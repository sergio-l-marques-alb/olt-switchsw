...
unsigned long flash_init (void)
{
   ...
   */
  size = flash_get_size((vu_char *)FLASH_BASE0_PRELIM, &flash_info[0]);
  if ( size ) {
...
static ulong flash_get_size (vu_char *addr, flash_info_t *info)
{
  ...
  switch ( devid ) {
    ...
    default:
      printf (" ### Vendor=%04X DevID=%04X ### ", vendor , devid );
      info->flash_id     = FLASH_UNKNOWN;
      info->sector_count = 0;
      info->size         = 0;
      return (0);
...
int flash_erase (flash_info_t *info, int s_first, int s_last)
{
  ...
  while ( (addr[0] & 0x80) != 0x80 ) {
    ...
    if ( (now - last) > 1000 ) {  /* every 1/2 second */
      serial_putc ('.');
      last = now;
    }
  }
...
int boot_update(flash_info_t *info , uchar *src_base )
{
  ...
  if ( *((ulong *)src_base) != 0x04040404 ) {
    printf("Error: boot update failed!\n");
  ...
  switch ( info->flash_id & FLASH_TYPEMASK ) {
    ...
    case FLASH_STM800DT:
    case FLASH_S29AL008J:
      rc=flash_erase (info, 0, 3); /* Erase first four blocks (4*64K) */
      if ( rc != 0 ) {
        printf("Error: boot update failed (when erasing the flash)!\n");
        return(-2);
      }

      /* Write first four blocks  */
      rc=write_buff (info, src_base , (ulong)CFG_FLASH_BASE , (64*4) * 1024 );
      if ( rc != 0 ) {
        printf("Error: boot update failed (writing to flash)!\n");
        return(-3);
      }
      break;
    ...
    case FLASH_STM800DB:
      {
        rc=flash_erase (info, 0, 6); /* Erase first four blocks (16+8+8+32+3*64K) */
        if ( rc != 0 ) {
          printf("Error: boot update failed (when erasing the flash)!\n");
          return(-2);
        }
