...
int do_updateboot ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
  ulong addr;
  ...
  if ((addr < 0x00010000) || (addr > 0x00FFFFFF)) {
	printf ("Use RAM as source!\n");
	return 1;
  }

  boot_update( flash_info , addr );
  return 0;
}

U_BOOT_CMD(
  updateboot, 2, 1, do_updateboot,
  "updateboot addr - save new boot from addr in flash\n",
  "addr - save new boot from addr in flash\n"
);
