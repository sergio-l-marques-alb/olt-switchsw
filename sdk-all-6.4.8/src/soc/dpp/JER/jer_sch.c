...
uint32
_jer_sch_rates_calculate_new_calendars(int unit, int core, int max_rate_increased, uint32 new_max, int is_max_psst_table, int *new_rate_configured);
uint32
_jer_sch_rates_calculate_new_divider(int unit, int core, int hr_calendar_table_num, int max_rate_increased, int new_max, int is_psst_table, int old_divider ,uint32 *new_divider);
...
/* This function should be called only during init sequence */
    uint32 reg_val, reg_val2, mem_val, quanta, credit, nof_ticks, hr_num,
           quanta_nof_bits, nof_available_hr = 0, psst_table_location, access_period, cal_length;
    uint8 valid;
    int rate;
    ...
    nof_available_hr = ARAD_EGR_NOF_Q_PAIRS;
    ...
    /* initalize quantas for each hr configured */
    for (psst_table_location = 0 ; psst_table_location < nof_available_hr; psst_table_location++)
    {
        SOCDNX_IF_ERR_EXIT(READ_SCH_PIR_SHAPER_CALENDAR_PSCm(unit, SCH_BLOCK(unit, core), psst_table_location, &mem_val));
        soc_mem_field_get(unit, SCH_PIR_SHAPER_CALENDAR_PSCm, &mem_val, HR_NUMf, &hr_num);

        SOCDNX_IF_ERR_EXIT(arad_sw_db_sch_priority_port_rate_get(unit, core, hr_num, &rate, &valid));
        if (valid > 0)
        {
            SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, 0, 1, &credit, &nof_ticks, &quanta_nof_bits));
            if (rate == 0)
            {
                quanta = 0;
            }
            ...
            {    
                SOCDNX_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(rate,credit,nof_ticks,&quanta));
    ...
    /* initalize tcg tables in a somewhat similar way */

    /* configcsst second table */
    ...
    )
{
    ...
    uint32 credit, reg_val, nof_ticks, quanta_nof_bits, quanta, offset,
    ...
    SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, hr_calendar_table_num, 1, &credit, &nof_ticks, &quanta_nof_bits));
    ...
    {
        SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calculate_new_calendars(unit, core, increasing_maximum_rate, (increasing_maximum_rate)? rate:max_rate_after_change , 1, &is_new_rate_configured));                       
    }
...
{
    uint32 reg_val, hr_calendar_table_num, credit, offset, index, mem_val, 
...
jer_ofp_rates_sch_tcg_shaper_rate_set(
    ...
    SOC_SAND_IN     uint32               rate)
{
    uint32  credit, reg_val ,nof_ticks, quanta_nof_bits, quanta, base_port_tc, hr_calendar_table_num,
    ...
    /* Generic function: get divider, cal_length, and access period, given which copy we're using */
    SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, hr_calendar_table_num, 0, &credit, &nof_ticks, &quanta_nof_bits));
    ...
    if (((quanta > 2047) && (increasing_maximum_rate)) || decreasing_unique_maximum_rate)
    {
        SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calculate_new_calendars(unit, core, increasing_maximum_rate, (increasing_maximum_rate)? rate:0 , 0, &is_new_rate_configured));                       
    }
...
jer_ofp_rates_sch_tcg_shaper_rate_get(
    ...
    SOC_SAND_OUT    uint32*               rate)
{
    uint32  reg_val, hr_calendar_table_num, quanta_nof_bits, credit, index, 
...
_jer_sch_rates_switch_calendars(int unit, int core, int new_calendar_table_num, int new_divider)
{
    uint32 mem_val, credit, nof_ticks, quanta_nof_bits, reg_val, cal_length_psst, cal_length_csst, hr_num, tcg_index,
    ...
    SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, new_calendar_table_num, 1, &credit, &nof_ticks, &quanta_nof_bits));    
    ...
    SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, new_calendar_table_num, 0, &credit, &nof_ticks, &quanta_nof_bits)); 
...
_jer_sch_rates_calculate_new_divider(int unit, int core, int hr_calendar_table_num, int max_rate_increased, int new_max, int is_psst_table, int old_divider ,uint32 *new_divider)
{
    uint32 credit, credit_worth, nof_ticks, quanta_nof_bits, new_quanta, nof_ticks_psst_table, nof_ticks_csst_table,
    ...
    {
        SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, hr_calendar_table_num, is_psst_table, &credit, &nof_ticks, &quanta_nof_bits));
        for (curr_divider = old_divider; curr_divider > 15 ; curr_divider--)
        {
            credit = SOC_SAND_DIV_ROUND_UP(credit_worth, curr_divider);
    		SOCDNX_SAND_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks,&new_quanta));
    		if(new_quanta <= 2047)
    		{
    			break;
    		}
    ...
    else /* maximum rate decreased - we can decrease the divider in order to get a better resolution */
    {
        uint32 new_max_rate_psst, new_max_rate_csst; 
        ...
        else /* increase divider to maximum possible, keeping the max rate quanta below 2047 */
        {
            SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, hr_calendar_table_num, 1, &credit, &nof_ticks_psst_table, &quanta_nof_bits));  
            SOCDNX_IF_ERR_EXIT(arad_sch_calendar_info_get(unit, core, hr_calendar_table_num, 0, &credit, &nof_ticks_csst_table, &quanta_nof_bits));
            curr_divider = old_divider;
            for (iterate_divider = curr_divider + 1; iterate_divider < 65; iterate_divider++)
            {
                credit = SOC_SAND_DIV_ROUND_UP(credit_worth, iterate_divider); 
                if (new_max_rate_psst > 0)
                {
                    SOCDNX_SAND_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks_psst_table,&new_quanta));
            		if(new_quanta > 2047)
            		{
                        break;
            		}                    
                }
                if (new_max_rate_csst > 0)
                {
            		SOCDNX_SAND_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks_csst_table,&new_quanta));
            		if(new_quanta > 2047)
            		{
            			break;
            		}  
...
_jer_sch_rates_calculate_new_calendars(int unit, int core, int max_rate_increased, uint32 new_max, int is_max_psst_table, int *new_rate_configured)
{
    ...
    SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calculate_new_divider(unit, core, hr_calendar_table_num, max_rate_increased, new_max, is_max_psst_table, old_divider, &new_divider));
