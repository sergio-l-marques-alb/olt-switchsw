  ...
  )
{
    ...
    rci_config.soc_dpp_fabric_rci_severity_factors[0] = 4;
    rci_config.soc_dpp_fabric_rci_severity_factors[1] = 5;
    rci_config.soc_dpp_fabric_rci_severity_factors[2] = 6;
    rci_config.soc_dpp_fabric_rci_high_score_fabric_rx_queue = 0x64;
    rci_config.soc_dpp_fabric_rci_high_score_fabric_rx_local_queue = 0x64;
    rci_config.soc_dpp_fabric_rci_threshold_num_of_congested_links = 0x1e;
    rci_config.soc_dpp_fabric_rci_high_score_congested_links = 0x64; 
  ...
  )
{
    ...
    soc_reg64_field32_set(unit, FDR_FDR_ENABLERS_REGISTER_1r, &reg64, FIELD_23_30f, 0x2);
    SOCDNX_IF_ERR_EXIT(WRITE_FDR_FDR_ENABLERS_REGISTER_1r(unit, reg64));

    SOCDNX_IF_ERR_EXIT(READ_FCR_FCR_ENABLERS_AND_FILTER_MATCH_INPUT_LINKr(unit, &reg_val));
    soc_reg_field_set(unit, FCR_FCR_ENABLERS_AND_FILTER_MATCH_INPUT_LINKr, &reg_val, FIELD_15_15f, 1);
    SOCDNX_IF_ERR_EXIT(WRITE_FCR_FCR_ENABLERS_AND_FILTER_MATCH_INPUT_LINKr(unit, reg_val));
    ...
    /* init EFMS_AGING_TIMER */
    SOCDNX_IF_ERR_EXIT(READ_FCR_EFMS_CONFIGURATIONr(unit, reg_above_64));
    soc_reg_above_64_field32_set(unit, FCR_EFMS_CONFIGURATIONr, reg_above_64, EFMS_AGING_TIMERf, 0x3);
  ...
  )
{
    ...
    if (fabric_pipe_type != soc_dpp_cosq_gport_fabric_pipe_egress) {
        ...
        SOCDNX_IF_ERR_EXIT(READ_FDT_IPT_0_WFQ_CONFIGr_REG32(unit, &reg_val));
        soc_reg_field_set(unit, FDT_IPT_0_WFQ_CONFIGr, &reg_val, field, weight);
        SOCDNX_IF_ERR_EXIT(WRITE_FDT_IPT_0_WFQ_CONFIGr_REG32(unit, reg_val));
    ...
    SOC_SAND_IN  soc_dpp_cosq_gport_egress_core_fifo_t   fifo_type)
{
    ...
    case soc_dpp_cosq_gport_egress_core_fifo_fabric_ucast:
        for (i = 0; i < SOC_JER_FABRIC_FDA_EGQ_NUMELS; ++i) {
            SOCDNX_IF_ERR_EXIT(READ_FDA_FDA_EGQ_WFQr_REG64(unit, i, &reg_val)); 
            soc_reg64_field32_set(unit, FDA_FDA_EGQ_WFQr, &reg_val, EGQ_N_WFQ_WEIGHT_FABFIF_0f, weight);
            SOCDNX_IF_ERR_EXIT(WRITE_FDA_FDA_EGQ_WFQr_REG64(unit, i, reg_val));
        }
        break;
    case soc_dpp_cosq_gport_egress_core_fifo_fabric_mcast:
        for (i = 0; i < SOC_JER_FABRIC_FDA_EGQ_NUMELS; ++i) {
  ...
  )
{
    ...
    switch (fifo_type) {
    case soc_dpp_cosq_gport_egress_core_fifo_fabric_ucast:
        SOCDNX_IF_ERR_EXIT(READ_FDA_FDA_EGQ_WFQr_REG64(unit, 0, &reg_val));
  ...
  )
{
    ...
    if (force == socDppFabricForceLocal0) {
    ...
    if (force == socDppFabricForceLocal1) {
    ...
    if (force == socDppFabricForceRestore) {
