...
{  
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, READ_EDB_EEDB_BANKm(unit, bank_id, MEM_BLOCK_ANY, internal_offset, data));
  ...
  if (key_prefix == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_RIF 
      ...
      if (key_prefix2 == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_NONE) {
          ...
          if (SOC_IS_ARADPLUS(unit))  {
              soc_mem_field32_set(unit, EPNI_AC_HALF_ENTRY_FORMATm, data, (is_location_at_start) ? AC_0_ACTION_USE_AS_DATA_BITf : AC_1_ACTION_USE_AS_DATA_BITf, 1);
          } else if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
