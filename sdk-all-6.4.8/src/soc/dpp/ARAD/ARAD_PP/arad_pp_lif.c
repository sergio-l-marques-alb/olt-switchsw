  ...
  )
{
    ...
    if (!ignore_key) {
        ...
        res = arad_pp_dbal_vtt_sw_db_get(unit, &sem_key, &nof_dbal_tables, dbal_table_ids);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

        /* Get the Extender key qualifiers */
        res = arad_pp_l2_lif_extender_key_to_qualifier(unit, extender_key, qual_vals);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
    if (is_ingress) {
        ...
        SHR_BITCOPY_RANGE(tbl_data.lif_table_entry,start_bit_off,data32_arr,0,56);
