    ...
    arad_pp_init_dynamic_kbp_result(int unit)
{
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_programs); i++){
        ...
        arad_pp_sw_db_flp_prog_app_to_index_get(unit, ARAD_PP_FLP_KEY_PROGRAM_IPV6UC_WITH_RPF, &ipv6rpf_prog_id);        
        if (flp_lookups_tbl.elk_lkp_valid) {
            ...
            /* configures the FLP */            
            if((i == ARAD_PP_FLP_KEY_PROGRAM_IPV4UC_WITH_RPF) ||                
               (i == ipv6rpf_prog_id)){
            ...
            if(i == ARAD_PP_FLP_KEY_PROGRAM_IPV4COMPMC_WITH_RPF){
                results_valid[1] = 1; /* RPF result for those program is located in result 1*/
                rpf_result_start_offset = res_sizes[3] + res_sizes[2];
                fec_res_found_bit_offset = ARAD_PP_FLP_KBP_MAX_ROP_PAYLOAD_SIZE_IN_BITS+6;
                is_rpf_lookup_needed = 1;
            }
    ...
    for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
        ...
        total_nof_bits_in_elk_res += irpp_qual_info.info.qual_nof_bits;
        kbp_qualifiers_values.buffer_lsb = irpp_qual_info.info.buffer_lsb - total_nof_bits_in_elk_res + res_sizes[i];        
  ...
  )
{
  ...
  */
  ...
  if (SOC_IS_JERICHO(unit))  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  220,  exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, IHB_PMF_GENERALr, REG_PORT_ANY, 0, PMF_KAPS_DELAY_IS_EVENf,  1));
