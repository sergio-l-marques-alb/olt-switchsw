  ...
  Arad_pp_error_desc_element_frwrd_bmact[] =
{
  ...
  },
  {
    ...
    "The range is: ARAD_PP_BMACT_ENTRY_TYPE_FRWRD - ARAD_PP_BMACT_ENTRY_TYPE_LEARN.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
  ...
  )
{
  ...
  res = arad_pp_fwd_decision_in_buffer_build( 
      unit,
      ARAD_PP_FRWRD_DECISION_TYPE_MC,
  ...
  if (bvid_info->b_fid_profile == ARAD_PP_BFID_EQUAL_TO_BVID) {
      fid_class = 1;
  }
  else { /* ARAD_PP_BFID_IS_0 */
      fid_class = 0;
  }
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_BMACT_BVID_INFO, bvid_info, 20, exit);
  ...
  )
{
  ...
  ARAD_PP_BMACT_BVID_INFO_clear(bvid_info);
  ...
  if (fid_class) {
      bvid_info->b_fid_profile = ARAD_PP_BFID_IS_0; /* shared */ 
  }
  else {
      bvid_info->b_fid_profile = ARAD_PP_BFID_EQUAL_TO_BVID; /* independent */ 
  }
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_BMACT_PBB_TE_VID_RANGE, pbb_te_bvids, 10, exit);
  ...
  )
{
  ...
  ARAD_PP_BMACT_PBB_TE_VID_RANGE_clear(pbb_te_bvids);
  ...
  )
{
  ...
  *asd_lcl = 0;
  ...
  if (flags & ARAD_PP_BMACT_ENTRY_TYPE_FRWRD) /* BMACT DB */
  {      
  ...
  if (flags & ARAD_PP_BMACT_ENTRY_TYPE_LEARN) /* MIM tunnel learn DB */
  {
   ...
   )
{
  ...
  if (bmac_key->flags & ARAD_PP_BMACT_ENTRY_TYPE_FRWRD) /* BMACT DB */
  {
  ...
  if (bmac_key->flags & ARAD_PP_BMACT_ENTRY_TYPE_LEARN) { 
      ...
      if (SOC_DPP_CONFIG(unit)->pp.test2) {
          ...
          if (SOC_IS_JERICHO(unit)) {
              ...
              int core = 0;
              res = soc_port_sw_db_core_get(unit, bmac_key->local_port_ndx, &core);
              SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

              key->param[2].value[0] |= core << ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_MAC_IN_MAC_TUNNEL;
    ...
    )
{
  ...
  ARAD_PP_FRWRD_DECISION_INFO_clear(&frwrd_decision_info);
  ARAD_PP_BMACT_ENTRY_INFO_clear(bmact_entry_info);
  ...
  if (flags & ARAD_PP_BMACT_ENTRY_TYPE_FRWRD) 
  {
    /*
  ...
  )
{
  ...
  /* Destination */
  if (bmac_key->flags & ARAD_PP_BMACT_ENTRY_TYPE_FRWRD) 
  {
  	  /* FRWRD */	
      if (bmac_key->flags & ARAD_PP_BMACT_ENTRY_TYPE_MC_DEST)
      {
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_BMACT_ENTRY_KEY, bmac_key, 10, exit);
  res = ARAD_PP_BMACT_ENTRY_INFO_verify(unit, bmact_entry_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
...
uint32
  ARAD_PP_BMACT_BVID_INFO_verify(
    SOC_SAND_IN  ARAD_PP_BMACT_BVID_INFO *info
  )
{
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_BMACT_BVID_INFO_verify()",0,0);
}

uint32
  ARAD_PP_BMACT_PBB_TE_VID_RANGE_verify(
    SOC_SAND_IN  ARAD_PP_BMACT_PBB_TE_VID_RANGE *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_BMACT_PBB_TE_VID_RANGE_verify()",0,0);
}

uint32
  ARAD_PP_BMACT_ENTRY_KEY_verify(
    SOC_SAND_IN  ARAD_PP_BMACT_ENTRY_KEY *info
  )
{
  uint32
    res = SOC_SAND_OK;
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->local_port_ndx, ARAD_PP_PORT_MAX, ARAD_PP_PORT_OUT_OF_RANGE_ERR, 50, exit);

  ARAD_PP_STRUCT_VERIFY(SOC_SAND_PP_MAC_ADDRESS, &(info->b_mac_addr), 60, exit);
  SOC_SAND_ERR_IF_ABOVE_MAX(info->flags, ARAD_PP_BMACT_ENTRY_TYPE_MC_DEST | ARAD_PP_BMACT_ENTRY_TYPE_FRWRD, SOC_SAND_PP_VLAN_ID_OUT_OF_RANGE_ERR, 70, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_BMACT_ENTRY_KEY_verify()",0,0);
}

uint32
  ARAD_PP_BMACT_ENTRY_INFO_verify(
    SOC_SAND_IN  int                      unit,
    SOC_SAND_IN  ARAD_PP_BMACT_ENTRY_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  SOC_SAND_CHECK_NULL_INPUT(info);
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->sa_learn_fec_id, SOC_DPP_DEFS_GET(unit, nof_fecs), ARAD_PP_FEC_ID_OUT_OF_RANGE_ERR, 12, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_BMACT_ENTRY_INFO_verify()",0,0);
}
