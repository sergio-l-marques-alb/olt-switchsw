...
/* 
    Function: arad_kbp_result_configurations_update
...
*/
uint32
    arad_kbp_result_configurations_update(int unit, ARAD_KBP_FRWRD_IP_DB_TYPE db_type, int results_size[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS], int results_valid[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS])
{
    int i;
    int total_size_valid_lookups = 0;
    int total_size_all_lookups = 0;
    ...
    if((results_valid[0]) || (results_valid[1]) || (results_valid[2]) || (results_valid[3])){
        Arad_kbp_gtm_lut_info[unit][db_type].result0_idx_ad_cfg = results_size[0];
    }
    if((results_valid[1]) || (results_valid[2]) || (results_valid[3])){
        Arad_kbp_gtm_lut_info[unit][db_type].result1_idx_ad_cfg = results_size[1];
    }
    if((results_valid[2]) || (results_valid[3])){
        Arad_kbp_gtm_lut_info[unit][db_type].result2_idx_ad_cfg = results_size[2];
    }    
    if(results_valid[3]){
        Arad_kbp_gtm_lut_info[unit][db_type].result3_idx_ad_cfg = results_size[3];
    }    

    for (i = 0; i < ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS; i++) {
        Arad_kbp_gtm_ltr_info[unit][db_type].res_data_len[i] = (results_valid[i] * results_size[i]);

        if (!results_valid[i]) {
            Arad_kbp_gtm_ltr_info[unit][db_type].res_format[i] = NLM_ARAD_NO_INDEX_NO_AD;
        } else {
            Arad_kbp_gtm_ltr_info[unit][db_type].res_format[i] = (results_size[i]/4);
            if( (results_size[i]*8)%32 > 0){
                Arad_kbp_gtm_ltr_info[unit][db_type].res_format[i]++;
            }            
        }
        total_size_valid_lookups += (results_valid[i] * results_size[i]);
        total_size_all_lookups += results_valid[i];
    }
    Arad_kbp_gtm_ltr_info[unit][db_type].res_total_data_len = 1 + total_size_valid_lookups;


    for (i = ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS - 1; i >= 0; i--) {
        if (results_valid) {
            break;
        } else{
            total_size_all_lookups -= results_size[i];
        }
    }    

    /* here we need to update the result size also for the ACLs that in between the FWD and the RPF */
    Arad_kbp_gtm_opcode_config_info[unit][db_type].rx_data_size = total_size_all_lookups;

exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_kbp_result_configurations_update", 0, db_type);
}
   ...
   )
{
    ...
    if (irpp_qual_info.info.qual_nof_bits <= 32) {
    ...
    opcode_info->rx_data_size += irpp_qual_info.info.qual_nof_bits/8;
    ...
    if(!(flags & ARAD_KBP_TABLE_ALLOC_CHECK_ONLY))
    {
        for(srch_idx = 0; srch_idx <= search_id; srch_idx++)
        {
            ...
            if (srch_idx != search_id)
            {
                ...
                Arad_kbp_ltr_config[unit][db_type].ltr_search[srch_idx].nof_key_segments = 1 + nof_shared_segments;
                Arad_kbp_ltr_config[unit][db_type].ltr_search[srch_idx].key_segment[0] = search_format->key_segment[0];
                Arad_kbp_ltr_config[unit][db_type].ltr_search[srch_idx].key_segment[1] = search_format->key_segment[1];
                Arad_kbp_ltr_config[unit][db_type].ltr_search[srch_idx].key_segment[2] = search_format->key_segment[2];
            }
        }
    }
    ...
    uint32     *ilkn_metaframe)
{
    ...
    *ilkn_port = -1;
    ...
    SOC_PBMP_ITER(ports_bm, port){
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 3, exit, soc_port_sw_db_local_to_pp_port_get(unit, port, &pp_port, &core_id));
        if (core != core_id) {
            continue;
        }
   ...
   int unit)
{
    ...
    KBP_TRY(kbp_device_set_property(AradAppData[unit]->device_p, KBP_DEVICE_PROP_INST_LATENCY, 1));
    ...
    ARAD_INIT_ELK *elk_ptr)
{
    ...
    if(!SAL_BOOT_PLISIM){
            ...
            if (res != 0 && elk->kbp_recover_enable) {
                res = arad_kbp_recover_run_recovery_sequence(unit, core, elk->kbp_mdio_id[core], elk->kbp_recover_iter);
            }
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
