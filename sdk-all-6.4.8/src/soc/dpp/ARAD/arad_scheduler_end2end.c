 ...
 */
static int __regions_cache_was_allocated = 0 ;
static uint8 *__regions_is_cl_cir = NULL ;
static uint8 *__regions_is_interdigitated = NULL ;
static int *__regions_p_is_oddEven = NULL ;
/*
 * Function:
 *   arad_sch_allocate_cache_for_per1k
 * Purpose:
 *   Internal routine to allocate cache that speeds up
 *   arad_sch_per1k_info_get_unsafe() from dynamic memory heap.
 * Input:
 *   Direct:
 *     int unit -
 ...
 *   None.
 * See also:
 *   tools\autocoder\DNXDefines\soc_dpp_defines.csv
 *   SOC_DPP_DEFS_MAX, SOC_DPP_DEFS_GET
 */
static int
arad_sch_allocate_cache_for_per1k(SOC_SAND_IN int unit)
{
  ...
  ret = 1 ;
  __regions_is_cl_cir = NULL ;
  __regions_is_interdigitated = NULL ;
  __regions_p_is_oddEven = NULL ;
  size =
    (SOC_MAX_NUM_DEVICES *
        (SOC_DPP_DEFS_GET(unit,nof_flows_per_pipe) / 1024) * SOC_DPP_DEFS_GET(unit,nof_cores)) ;
  total_size = sizeof(*__regions_is_cl_cir) * size ;
  __regions_is_cl_cir = sal_alloc(total_size,"per1k_cache") ;
  if (__regions_is_cl_cir == NULL)
  {
    goto exit ;
  }
  total_size = sizeof(*__regions_is_interdigitated) * size ;
  __regions_is_interdigitated = sal_alloc(total_size,"per1k_cache") ;
  if (__regions_is_interdigitated == NULL)
  {
    goto exit ;
  }
  total_size = sizeof(*__regions_p_is_oddEven) * size ;
  __regions_p_is_oddEven = sal_alloc(total_size,"per1k_cache") ;
  if (__regions_p_is_oddEven == NULL)
  {
    goto exit ;
  }
  /*
   ...
   */
  int_p = __regions_p_is_oddEven ;
  for (counter = 0 ; counter < size ; counter++)
  {
    *int_p++ = (int)(-1) ;
  }
  ret = 0 ;
  __regions_cache_was_allocated = 1 ;
  /*
   * Roll down to exit.
   */
exit:
  if (ret)
  {
    __regions_cache_was_allocated = 0 ;
    if (__regions_is_cl_cir)
    {
      sal_free(__regions_is_cl_cir) ;
    }
    if (__regions_is_interdigitated)
    {
      sal_free(__regions_is_interdigitated) ;
    }
    if (__regions_p_is_oddEven)
    {
      sal_free(__regions_p_is_oddEven) ;
    }
    __regions_is_cl_cir = NULL ;
    __regions_is_interdigitated = NULL ;
    __regions_p_is_oddEven = NULL ;
  ...
  )
{
  ...
  {
    if (!__regions_cache_was_allocated)
    {
      int err ;
     ...
     */
    {
      ...
      table_index =
        (unit * ((SOC_DPP_DEFS_GET(unit,nof_flows_per_pipe) / 1024) * SOC_DPP_DEFS_GET(unit,nof_cores))) +
                                                (k_flow_ndx * (SOC_DPP_DEFS_GET(unit,nof_cores))) + core ;
      if ((per1k_info->is_interdigitated == __regions_is_interdigitated[table_index]) &&
                     (per1k_info->is_odd_even == __regions_p_is_oddEven[table_index]) &&
                     (per1k_info->is_cl_cir == __regions_is_cl_cir[table_index]))
      {
        goto exit ;
      }
    }
   ...
   */
  {
    ...
    table_index =
      (unit * ((SOC_DPP_DEFS_GET(unit,nof_flows_per_pipe) / 1024) * SOC_DPP_DEFS_GET(unit,nof_cores))) +
                                (k_flow_ndx * (SOC_DPP_DEFS_GET(unit,nof_cores))) + core ;
    __regions_is_interdigitated[table_index] = per1k_info->is_interdigitated ;
    __regions_p_is_oddEven[table_index] = per1k_info->is_odd_even ;
    __regions_is_cl_cir[table_index] = per1k_info->is_cl_cir ;
  }
exit:
  ...
  else
  {
    ...
    already_loaded = 0 ;
    {
      ...
      table_index =
        (unit * ((SOC_DPP_DEFS_GET(unit,nof_flows_per_pipe) / 1024) * SOC_DPP_DEFS_GET(unit,nof_cores))) +
                                                (k_flow_ndx * (SOC_DPP_DEFS_GET(unit,nof_cores))) + core ;
      if (__regions_p_is_oddEven[table_index] != -1)
      {
        per1k_info->is_interdigitated = __regions_is_interdigitated[table_index];
        per1k_info->is_odd_even = __regions_p_is_oddEven[table_index];
        per1k_info->is_cl_cir = __regions_is_cl_cir[table_index];
        already_loaded = 1 ;
      }
  ...
  )
{
   ...
   */
  {
     ...
     * Important note:
     *   If 'fip_id' is SOC_TMC_MAX_FAP_ID then the object (input quartet or
