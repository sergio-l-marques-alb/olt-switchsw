                         ...
                         const portmod_port_add_info_t* add_info)
{
    ...
    for(i = 0 ; i < 3 ; i++)
    {
        ...
        if(core_num%2) {
            ((portmod_port_add_info_t*)add_info)->interface_config.flags |= PHYMOD_INTF_F_CORE_MAP_MODE_FLIP;
        }
                               ...
                               int phy_init_flags)
{
    ...
    if  ((config->speed >= 100000) &&
         (!(config->flags & PHYMOD_INTF_F_SET_CORE_MAP_MODE)) &&  
        ...
        for(i = 0 ; i < 3 ; i++)
        {
            rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_interface_config_set(
                             unit, port, PM_4X10_INFO(pm_info, i), my_config, 
                             phy_init_flags);
            _SOC_IF_ERR_EXIT(rv);
        }
        my_config->flags = save_flags;
    }
                              ...
                              const phymod_autoneg_control_t* an)
{
     ...
     */
     /* we should not recover any state once AN is disabled, user should call
      forced speed set to put the port into a valid state, thus removing this
      part of code */
#if 0
    if (!an->enable) {
...
{
    return (PM_4X25_DRIVER(pm_info)->f_portmod_port_speed_get(unit,
                            port, PM_4X25_INFO(pm_info), speed));
}
...
int pm12x10_port_ifg_set(int unit, int port, pm_info_t pm_info, int speed, soc_port_duplex_t duplex, int ifg,  int *real_ifg)
{
    ...
    *real_ifg = 0;
    ...
    _SOC_IF_ERR_EXIT
        (portmod_port_ability_local_get(unit, port, &ability));
    ...
    *real_ifg = ifg < 64 ? 64 : (ifg + 7) & (0x1f << 3);
    ...
    _SOC_IF_ERR_EXIT(pm12x10_port_duplex_get(unit, port, pm_info, &cur_duplex));
    _SOC_IF_ERR_EXIT(pm12x10_port_speed_get (unit, port, pm_info, &cur_speed));
    ...
    if (cur_speed == speed &&
        ...
        _SOC_IF_ERR_EXIT(PM_4X25_DRIVER(pm_info)->f_portmod_port_tx_average_ipg_set(unit,
                            port, PM_4X25_INFO(pm_info), *real_ifg));
    }
