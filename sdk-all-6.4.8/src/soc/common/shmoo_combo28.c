...
_shmoo_combo28_dqs2ck(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr)
{
    ...
    addr_ctrl_step = soc_phy_ddrc28_reg_field_get(unit, SOC_PHY_REG_AQ_L_MAX_VDL_ADDRr, data0, SOC_PHY_FLD_MAX_VDL_STEPf);
    
    if(addr_ctrl_ui == 3)
    {
        (*scPtr).sizeX = (*scPtr).endUI[4] + 1 + ((*scPtr).endUI[0] >> 1);
        position = ((*scPtr).endUI[4] - (*scPtr).endUI[3]) >> 1;
        (*scPtr).calibStart = (*scPtr).endUI[3] + 1 + position;
        ui = addr_ctrl_ui;
    }
    else if(addr_ctrl_ui > 0)
    {
        _combo28_calculate_ui_position((*scPtr).endUI[addr_ctrl_ui - 1] + 1 + addr_ctrl_step + ((*scPtr).endUI[0] >> 1), scPtr, &up);       /* +50% UI */
        position = up.position;
        ui = up.ui;
        if(ui < 3)
        {
            (*scPtr).calibStart = (*scPtr).endUI[ui - 1] + 1 + position;
            (*scPtr).sizeX = (*scPtr).endUI[ui + 1] + 1 + position;
        }
        else
        {
            (*scPtr).calibStart = (*scPtr).endUI[ui - 1] + 1 + position;
            (*scPtr).sizeX = (*scPtr).endUI[ui] + 1 + position + (*scPtr).endUI[0];
    ...
    {
        _combo28_calculate_ui_position(addr_ctrl_step + ((*scPtr).endUI[0] >> 1), scPtr, &up);                                              /* +50% UI */
        position = up.position;
        ui = up.ui;
        if(ui > 0)
        {
...
_shmoo_combo28_entry(int unit, int drc_ndx, combo28_shmoo_container_t *scPtr, uint32 mode)
{
    ...
    switch((*scPtr).shmooType)
    {
        case SHMOO_COMBO28_ADDR_CTRL_SHORT:
            if((*scPtr).dramType == SHMOO_COMBO28_DRAM_TYPE_GDDR5)
            {
                soc_combo28_calculate_step_size(unit, drc_ndx, &ss);
...
/*A06*/                         "A06. Compute VDL step size\n")));
        soc_combo28_calculate_step_size(unit, ndx, &ss);
