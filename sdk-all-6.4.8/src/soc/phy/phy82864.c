                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    if (port_num_lanes == 4) {
        if (pc->speed_max == PHY82864_SPD_40G) {
            speed_config->speed = PHY82864_SPD_40G;
            speed_config->line_interface =  SOC_PORT_IF_KR4;
        } else {
            speed_config->speed = PHY82864_SPD_100G;
            speed_config->line_interface =  SOC_PORT_IF_KR4;
        }
    } else if (port_num_lanes == 1) {
        speed_config->line_interface =  SOC_PORT_IF_SR;
        speed_config->speed = PHY82864_SPD_10G;
...
_phy_82864_init_pass2(int unit, soc_port_t port)
{
    ...
    sal_memset(interface_config.device_aux_modes, 0 , sizeof(phy82864_device_cfg_aux_mode_t));
    pCfg->device_cfg_aux_modes = (phy82864_device_cfg_aux_mode_t *)(interface_config.device_aux_modes) ;
    ...
    pCfg->device_cfg_aux_modes->BCM84793_capablity = 
        soc_property_port_get(
            unit, port, spn_PHY_PIN_COMPATIBILITY_ENABLE, FALSE);
    ...
    phy_mode_reverse = soc_property_port_get(unit, port, spn_PORT_PHY_MODE_REVERSE, 0);
    /* One time config for BCM82864 */
    pCfg->device_cfg_aux_modes->gearbox_100g_inverse_mode = phy_mode_reverse;
...
phy_82864_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    sal_memset(ability, 0, sizeof(*ability));
    if (pc->speed_max == PHY82864_SPD_100G) {
        pa_speed = SOC_PA_SPEED_100GB | SOC_PA_SPEED_40GB | SOC_PA_SPEED_10GB;
    } else if (pc->speed_max == PHY82864_SPD_40G) {
        pa_speed = SOC_PA_SPEED_40GB | SOC_PA_SPEED_10GB;
    } else if (pc->speed_max == PHY82864_SPD_10G) {
...
STATIC int
phy_82864_firmware_set(int unit, int port, int offset, uint8 *array,int datalen)
{
 ...
 */
STATIC int 
phy_82864_rx_vga_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
...
phy_82864_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    /* RX VGA */
    case SOC_PHY_CONTROL_RX_VGA:
        rv = phy_82864_rx_vga_set(pmc, port, intf, value);
        break;
    /* RX TAP */
    case SOC_PHY_CONTROL_RX_TAP1:
