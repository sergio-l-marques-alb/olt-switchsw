/*
 * $Id: sbZfCaQmQueueConfigEntryConsole.c,v 1.3 Broadcom SDK $
...
#include "sbZfCaQmQueueConfigEntryConsole.hx"



/* Print members in struct */
void
sbZfCaQmQueueConfigEntry_Print(sbZfCaQmQueueConfigEntry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmQueueConfigEntry:: enforcepolicemarkings=0x%01x"), (unsigned int)  pFromStruct->m_uEnforcePoliceMarkings));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmQueueConfigEntry:: dropthresh1=0x%03x"), (unsigned int)  pFromStruct->m_uDropThresh1));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmQueueConfigEntry:: flowcontrolthresh1=0x%03x"), (unsigned int)  pFromStruct->m_uFlowControlThresh1));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmQueueConfigEntry:: flowcontroltarget=0x%01x"), (unsigned int)  pFromStruct->m_uFlowControlTarget));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmQueueConfigEntry:: maxpages=0x%03x"), (unsigned int)  pFromStruct->m_uMaxPages));
  LOG_WARN(BSL_LS_SOC_COMMON,
...
char *
sbZfCaQmQueueConfigEntry_SPrint(sbZfCaQmQueueConfigEntry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaQmQueueConfigEntry:: enforcepolicemarkings=0x%01x", (unsigned int)  pFromStruct->m_uEnforcePoliceMarkings);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," dropthresh2=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh2);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaQmQueueConfigEntry:: dropthresh1=0x%03x", (unsigned int)  pFromStruct->m_uDropThresh1);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," flowcontrolthresh2=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh2);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaQmQueueConfigEntry:: flowcontrolthresh1=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlThresh1);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," flowcontrolenable=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlEnable);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaQmQueueConfigEntry:: flowcontroltarget=0x%01x", (unsigned int)  pFromStruct->m_uFlowControlTarget);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," flowcontrolport=0x%03x", (unsigned int)  pFromStruct->m_uFlowControlPort);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaQmQueueConfigEntry:: maxpages=0x%03x", (unsigned int)  pFromStruct->m_uMaxPages);
...
int
sbZfCaQmQueueConfigEntry_Validate(sbZfCaQmQueueConfigEntry_t *pZf) {
...
int
sbZfCaQmQueueConfigEntry_SetField(sbZfCaQmQueueConfigEntry_t *s, char* name, int value) {
