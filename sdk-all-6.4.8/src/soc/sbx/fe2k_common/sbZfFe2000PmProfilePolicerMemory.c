...
sbZfFe2000PmProfilePolicerMemory_Unpack(sbZfFe2000PmProfilePolicerMemory_t *pToStruct,
  ...
  (pToStruct)->bDropOnRed =  (uint32)  ((pFromData)[0] >> 5) & 0x01;

  /* Unpack Member: bBktENoDecrement */
  (pToStruct)->bBktENoDecrement =  (uint32)  ((pFromData)[0] >> 4) & 0x01;

  /* Unpack Member: bBktCNoDecrement */
  (pToStruct)->bBktCNoDecrement =  (uint32)  ((pFromData)[0] >> 3) & 0x01;

  /* Unpack Member: uLengthShift */
  (pToStruct)->uLengthShift =  (uint32)  ((pFromData)[0] ) & 0x07;

  /* Unpack Member: uEIR */
  (pToStruct)->uEIR =  (uint32)  ((pFromData)[2] >> 4) & 0x0f;
  (pToStruct)->uEIR |=  (uint32)  (pFromData)[1] << 4;

  /* Unpack Member: uCIR */
  (pToStruct)->uCIR =  (uint32)  (pFromData)[3] ;
  (pToStruct)->uCIR |=  (uint32)  ((pFromData)[2] & 0x0f) << 8;

  /* Unpack Member: bCouplingFlag */
  (pToStruct)->bCouplingFlag =  (uint32)  ((pFromData)[4] >> 7) & 0x01;

  /* Unpack Member: bRFC2698Mode */
  (pToStruct)->bRFC2698Mode =  (uint32)  ((pFromData)[4] >> 6) & 0x01;

  /* Unpack Member: bBlind */
  (pToStruct)->bBlind =  (uint32)  ((pFromData)[4] >> 5) & 0x01;

  /* Unpack Member: uBktCSize */
  (pToStruct)->uBktCSize =  (uint32)  ((pFromData)[4] ) & 0x1f;

  /* Unpack Member: uEBS */
  (pToStruct)->uEBS =  (uint32)  ((pFromData)[6] >> 4) & 0x0f;
  (pToStruct)->uEBS |=  (uint32)  (pFromData)[5] << 4;

  /* Unpack Member: uCBS */
  (pToStruct)->uCBS =  (uint32)  (pFromData)[7] ;
  (pToStruct)->uCBS |=  (uint32)  ((pFromData)[6] & 0x0f) << 8;
...
uint32 sbZfFe2000PmProfilePolicerMemory_Pack32( sbZfFe2000PmProfilePolicerMemory_t *pFrom, 
                                                ...
                                                uint32 nMaxToDataIndex)
{
  uint8 uBuffer[SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS*4];
  uint32 uIndex =0;
  uint32 *pBuffer = NULL;
  uint32 uNumberOfWords = SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS;
                                                 ...
                                                 uint32 nMaxToDataIndex)
{
  ...
  uint32 uBuffer[SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS*4];
  uint32 uNumberOfWords = SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_WORDS;
  uint32 uIndex =0;
  ...
  sbZfFe2000PmProfilePolicerMemory_Unpack(pToData , (uint8 *) &uBuffer[0], SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES);
  return uNumberOfWords;
}
