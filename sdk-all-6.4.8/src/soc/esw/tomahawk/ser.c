                         ...
                         uint32 *addr, int *second_pass)
{
    ...
    case MMU_WRED_CONFIG_PIPE2m:
        ...
        spci->mem_reported = MMU_WRED_CONFIG_PIPE3m;
        *addr &= _SOC_MMU_ADDR_ZERO_OUT_SEG_FIELD;
...
STATIC int
_soc_tomahawk_mem_index_remap(int unit, soc_mem_t mem, int index)
{
    switch (mem) {
        case L3_DEFIPm:
                              ...
                              char *prefix_str, int l2_mgmt_ser_fifo)
{
    ...
    case SOC_BLK_IPIPE:
        ...
        ser_info_table = _soc_bcm56960_a0_ip_mem_ser_info;
    ...
    case SOC_BLK_EPIPE:
        ...
        ser_info_table = _soc_bcm56960_a0_ep_mem_ser_info;
        break;
        ...
        if (soc_mem_field32_get(unit, mem, entry, VALIDf)) {
            ...
            if (regmem == _SOC_TH_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (spci.mem != INVALIDm) {
                        spci.flags |= SOC_SER_REG_MEM_KNOWN;
                        spci.index = _soc_tomahawk_mem_index_remap(unit,
                                                                   spci.mem,
                                                                   spci.index);
                        if (!spci.double_bit) {
    ...
    if ((mem_acc_type == _SOC_TH_ACC_TYPE_DUPLICATE) &&
        ...
        case SOC_BLK_MMU_XPE:
            ...
            case SOC_TH_MMU_BASE_TYPE_IPIPE:
                mmu_base_index = (soc_mem_base(unit, mem) >> 15) & 0x3; /* [16:15] */
            ...
            case SOC_TH_MMU_BASE_TYPE_SLICE:
                mmu_base_index = (soc_mem_base(unit, mem) >> 15) & 0x1; /* [15] */
                if (mmu_base_index == 0) {
...
{
    int i, j;
    _soc_mem_ser_en_info_t *ser_info_table[] = {
        _soc_bcm56960_a0_ip_mem_ser_info,
        _soc_bcm56960_a0_ep_mem_ser_info,
        _soc_bcm56960_a0_mmu_mem_ser_info,
        NULL
    ...
    for (i = 0; NULL != ser_info_table[i]; i++) {
        _soc_mem_ser_en_info_t *ser_info_list = ser_info_table[i];
        for (j = 0; INVALIDm != ser_info_list[j].mem; j++) {
            if (ser_info_list[j].mem == mem) {
                SOC_IF_ERROR_RETURN(
                    _soc_th_reg32_par_en_modify(unit,
                                                ser_info_list[j].en_reg,
                                                ser_info_list[j].en_fld,
                                                en));
                SOC_IF_ERROR_RETURN(
                    _soc_th_reg32_par_en_modify(unit,
                                                ser_info_list[j].ecc_1b_report_en_reg,
                                                ser_info_list[j].ecc_1b_report_en_fld,
                                                en_1b));
                return SOC_E_NONE;
            }
        }
...
#define _SER_TEST_MEM_TH_ALPM_MASK(num_uft_banks) ((num_uft_banks) == 4? 0x7FFF : 0x3FFF)
#define _SER_TEST_MEM_TH_ALPM_SHIFT(num_uft_banks) ((num_uft_banks) == 4? 15 : 14)
                                    ...
                                    int pipe_target)
{
    ...
    _soc_mem_ser_en_info_t *fifo_parity = NULL;
    ...
    * _soc_bcm56960_a0_mmu_mem_ser_info,
    */
    for (i = 0; _soc_th_ser_block_info[i].blocktype != 0; i++) {
        if ((_soc_th_ser_block_info[i].type == _SOC_TH_SER_TYPE_MEM) &&
            ((_soc_th_ser_block_info[i].blocktype == SOC_BLK_IPIPE) ||
             (_soc_th_ser_block_info[i].blocktype == SOC_BLK_EPIPE) ||
             (_soc_th_ser_block_info[i].blocktype == SOC_BLK_MMU_GLB))) {
            fifo_parity = _soc_th_ser_block_info[i].info;
            for (j = 0; fifo_parity[j].mem != INVALIDm; j++) {
                if ((fifo_parity[j].mem == mem) &&
...
{
    int i;
    for (i = 0; th_mems_test_field[i].mem != INVALIDm; i++) {
        if ((th_mems_test_field[i].mem == mem) &&
            ((th_mems_test_field[i].acc_type == soc_acc_type_target) ||
             (th_mems_test_field[i].acc_type == _SOC_ACC_TYPE_PIPE_ANY))
           ) {
            *test_field = th_mems_test_field[i].test_field;
            LOG_VERBOSE(BSL_LS_SOC_SER,
                (BSL_META_U(unit,
                              ...
                              int inject_only, int cmd, uint32 flags)
{
    ...
    _soc_generic_ser_info_t *tcams = _soc_th_tcam_ser_info[unit];
    _soc_mem_ser_en_info_t *fifo_parity = NULL;
    int i,j, p, error_count = 0, skip_count = 0, found_mem = FALSE;
        ...
        for (i = 0; _soc_th_ser_block_info[i].blocktype != 0; i++) {
            if ((_soc_th_ser_block_info[i].type == _SOC_TH_SER_TYPE_MEM) &&
                ...
                for (j = 0; fifo_parity[j].mem != INVALIDm; j++) {
                    if ((fifo_parity[j].mem == mem) &&
                        (soc_mem_index_count(unit, mem))) {
                        if (_soc_th_ser_block_info[i].blocktype == SOC_BLK_MMU_GLB) {
                            /* For MMU mems */
                            soc_acc_type_target = _SOC_ACC_TYPE_PIPE_ANY;
                        }
                        test_field = ECCf;
                        _soc_ser_find_test_field(unit, mem, soc_acc_type_target,
                                                 &test_field);
                        soc_ser_create_test_data(unit, tmp_entry, field_data,
                                                 fifo_parity[j].en_reg,
                                                 SOC_INVALID_TCAM_PARITY_BIT,
                                                 fifo_parity[j].en_fld,
                                                 mem, test_field, block,
                                                 REG_PORT_ANY,
                                                 soc_acc_type_target,
                                                 index, &test_data);
                        found_mem = TRUE;
                        if (SOC_REG_IS_VALID(unit, fifo_parity[j].ecc_1b_report_en_reg) &&
                            (INVALIDf != fifo_parity[j].ecc_1b_report_en_fld)) {
                            flags |= SOC_INJECT_ERROR_2BIT_ECC;
                        }
...
soc_th_ser_hardware_test(int unit, _soc_ser_test_t test_type) {
    uint32 tmp_entry[SOC_MAX_MEM_WORDS], field_data[SOC_MAX_MEM_FIELD_WORDS];
    ser_test_data_t test_data;
    int mem_failed = 0, mem_tests = 0, mem_skipped = 0, num_inst_to_test = 0;
    _soc_mem_ser_en_info_t *fifo_parity = NULL;
    int i, j;
    ...
    for (i = 0; _soc_th_ser_block_info[i].blocktype != 0; i++) {
        if ((_soc_th_ser_block_info[i].type == _SOC_TH_SER_TYPE_MEM) &&
            ...
            fifo_parity = _soc_th_ser_block_info[i].info;
            for (j = 0; fifo_parity[j].mem != INVALIDm; j++) {
                if (!SOC_MEM_IS_VALID(unit, fifo_parity[j].mem) ||
                    !soc_mem_index_count(unit, fifo_parity[j].mem) ||
                    (SOC_MEM_INFO(unit, fifo_parity[j].mem).flags &
                     SOC_MEM_FLAG_SER_PARITY_ENABLE_SKIP)) {
                    LOG_WARN(BSL_LS_SOC_SER,
                              (BSL_META_U(unit,
                                          "unit %d, mem %s is INVALID or not valid "
                                          "or parity is disabled for this mem !!\n"),
                               unit, SOC_MEM_NAME(unit, fifo_parity[j].mem)));
                    continue;
                }
                flags = 0; /* for every mem */
                mem_acc_type = SOC_MEM_ACC_TYPE(unit, fifo_parity[j].mem);
                if (_soc_th_ser_block_info[i].blocktype == SOC_BLK_MMU_GLB) {
                    num_inst_to_test = 1;
