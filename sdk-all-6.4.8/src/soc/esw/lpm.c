...
typedef struct soc_lpm_field_cache_s {
    ...
    SOC_LPM_CACHE_FIELD_CREATE(L3_DEFIPm, GLOBAL_HIGH0f);
    SOC_LPM_CACHE_FIELD_CREATE(L3_DEFIPm, GLOBAL_HIGH1f);
...
#if defined(BCM_TRIDENT2_SUPPORT)
            /* GLOBAL_HIGH is meaningless for lpm, use it to indicate OVERRIDE */
            if (SOC_MEM_FIELD_VALID(unit, L3_DEFIPm, GLOBAL_HIGH1f)) {
                if (SOC_MEM_OPT_F32_GET(unit, L3_DEFIPm, lpm_entry, GLOBAL_HIGH1f)){
                    *vrf = _SOC_HASH_L3_VRF_OVERRIDE;
                }
            }
#endif /* BCM_TRIDENT2_SUPPORT */
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_MEM_OPT_FIELD_VALID(u, L3_DEFIPm, GLOBAL_HIGH0f)) {
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_MEM_OPT_FIELD_VALID(u, L3_DEFIPm, GLOBAL_HIGH1f)) {
...
#if defined(BCM_TRIDENT2_SUPPORT)
        if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, GLOBAL_HIGH0f)) {
            SOC_LPM_CACHE_FIELD_ASSIGN(u, soc_lpm_field_cache_state[u] ,L3_DEFIPm, GLOBAL_HIGH0f);
            SOC_LPM_CACHE_FIELD_ASSIGN(u, soc_lpm_field_cache_state[u] ,L3_DEFIPm, GLOBAL_HIGH1f);
        }
#endif /* BCM_TRIDENT2_SUPPORT */
...
soc_fb_lpm128_deinit(int u)
{
    ...
    if (DBG_SOC_LPM128_INDEX_TO_PFX_GROUP_TABLE(u)) {
...
lpm128_idx_to_pfx_dump(int u, int before)
{
    ...
    before = before && DBG_SOC_LPM128_INDEX_TO_PFX_GROUP_TABLE(u);
    LOG_CLI((BSL_META_U(u, "%s:\n"), before ? "Before" : "Current"));
...
lpm128_state_dump(int u, int before)
{
    ...
    before = before && DBG_SOC_LPM128_STATE(u);
    LOG_CLI((BSL_META_U(u, "%s:\n"), before ? "Before" : "Current"));
    ...
    lpm_state_ptr = before ? DBG_SOC_LPM128_STATE(u) : SOC_LPM128_STATE(u);
    ...
    lpm_state_ptr = before ? DBG_SOC_LPM128_UNRESERVED_STATE(u) :
...
int lpm128_state_verify(int u)
{
    ...
    SOC_IF_ERROR_RETURN(soc_fb_lpm_table_sizes_get(u, &defip_table_size, NULL));
    ...
    if (NULL == DBG_SOC_LPM128_STATE(u) && SOC_LPM128_STATE(u)) {
    ...
    pfxmap2 = sal_alloc(defip_table_size * sizeof(int), "debug pfxmap2");
    if (pfxmap2 == NULL) {
        return SOC_E_MEMORY;
    }

    sal_memset(pfxmap, -1, sizeof(int) * defip_table_size);
    sal_memset(pfxmap2, -1, sizeof(int) * defip_table_size);
        ...
        if (start2 != -1) {
            ...
            if (SOC_LPM128_PFX_IS_V4(u, curr_pfx)) {
                ...
                if (next_pfx != -1) {
                    ...
                    if (end_idx != (defip_table_size - tcam_depth - 1)) {
                        LOG_ERROR(BSL_LS_SOC_LPM,
                                  (BSL_META_U(u,
                                  "End_idx caculation wrong: end_idx %d ,"\
                                  "should be %d\n"), end_idx,
                                  (defip_table_size - tcam_depth - 1)
                                  ));
                        error_count++;
                    }
                }
...
_lpm128_fb_shift_v4_pfx_down(int u, soc_lpm128_state_p lpm_state_ptr, int pfx)
{
     ...
     */
    erase = SOC_LPM128_PFX_IS_V6(u, prev_pfx);
    if (half_entry == 0) {
                            ...
                            void *e, int slot, int *is_deleted)
{
    ...
    LPM128_HASH_INSERT(u, ef, NULL, end);
    if ((rv = WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, end, ef)) < 0) {
        return rv;
    }
    if ((rv = _lpm128_fb_urpf_entry_replicate(u, end, ef, NULL)) < 0) {
        return rv;
    }
 ...
 */
STATIC int
_lpm128_free_slot_delete(int u, int pfx,
                         void *e, int slot, int *is_deleted)
{
    int         from_ent;
    int         to_ent;
...
soc_fb_lpm128_delete(int u, void *key_data, void* key_data_upr)
{
    ...
    if (rv == SOC_E_NONE) {
        ...
        rv = _lpm128_free_slot_delete(u, pfx, e, index, &is_deleted);
        if (rv >= 0) {
            if (type == socLpmEntryTypeV4) {
