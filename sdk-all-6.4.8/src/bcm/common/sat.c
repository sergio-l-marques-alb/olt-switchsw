...
#define SAT_NULL_CHECK(p, para_name) \
    if (p == NULL) { \
...
#define SAT_VALUE_CHECK(val, min, max, para_name) \
    if ((val) < (min) || (val) >= (max)) { \
        LOG_ERROR(BSL_LS_BCM_SAT, \
                  (BSL_META_U(unit, \
...
#define SAT_VALUE_MAX_CHECK(val, max, para_name) \
    if ((val) >= (max)) { \
        LOG_ERROR(BSL_LS_BCM_SAT, \
                  (BSL_META_U(unit, \
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        for (i = 0; i < BCM_SAT_GTF_MAX_STAMPS; i++) {
            ...
            if ((config->packet_edit[priority].stamps[i].stamp_type == bcmSatStampCounter8Bit) || 
                ...
                SAT_VALUE_CHECK(config->packet_edit[priority].stamps[i].inc_period_packets, _BCM_SAT_GTF_STAMP_INC_PERIOD_MIN, _BCM_SAT_GTF_STAMP_INC_PERIOD_MAX+1, "stamp_inc_period_packets");
            }
    ...
    )
{
    ...
    SAT_VALUE_CHECK(bw->rate, _BCM_SAT_GTF_BW_RATE_MIN, (_BCM_SAT_GTF_BW_RATE_MAX+1), "rate");
    SAT_VALUE_CHECK(bw->max_burst, _BCM_SAT_GTF_BW_BURST_MIN, (_BCM_SAT_GTF_BW_BURST_MAX+1), "max_burst");
