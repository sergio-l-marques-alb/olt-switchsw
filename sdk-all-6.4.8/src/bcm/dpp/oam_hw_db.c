...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (endpoint_info->gport != BCM_GPORT_INVALID) {
        ...
        uint32 flags =  (endpoint_info->mpls_out_gport == BCM_GPORT_INVALID)?  _BCM_DPP_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS |_BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS: 
        ...
        ret = _bcm_dpp_gport_to_hw_resources(unit, endpoint_info->gport,flags,
                                             &gport_hw_resources);
        BCMDNX_IF_ERR_EXIT(ret);
