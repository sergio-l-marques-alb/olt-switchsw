...
STATIC int 
dpp_am_pp_resource_setup_in_range_by_core_id(int unit, int first_res, int last_res, int *pool_id);
...
dpp_am_pp_resource_setup_in_range_by_core_id(int unit, int first_res, int last_res, int *pool_id){
    ...
    for (res_id = first_res; res_id < last_res ; res_id++)   {
        ...
        for (core_id = 0 ; core_id < nof_pools_for_res ; core_id++) {
            ...
            rv = _bcm_dpp_pp_resource_setup(unit, res_id, core_id, adj_pool_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
...
_bcm_dpp_resources_create(int unit, int nof_resource_pools, int nof_cosq_pools)
{
    ...
    if (!SOC_WARM_BOOT(unit)) { /* In WB, we only load from sw state. */
        ...
        rv = dpp_am_pp_resource_setup_in_range_by_core_id(unit, 0, dpp_am_res_count, &pool_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (template_id = 0 ; template_id < SOC_DPP_CONFIG(unit)->am.nof_am_template_ids ; template_id++)   {
            ...
            for (core_id = 0 ; core_id < nof_pools_for_template ; core_id++) {
                rv = _bcm_dpp_template_setup(unit, template_id, core_id, pool_id);
                BCMDNX_IF_ERR_EXIT(rv);
                pool_id++;
            }
        }
...
bcm_dpp_am_resource_to_nof_pools(int unit, int res_id, uint8 *nof_pools_per_res){
    ...
    switch (res_id) {
        ...
        case dpp_am_res_global_outlif:
            *nof_pools_per_res = (SOC_IS_JERICHO(unit)) ? _BCM_DPP_AM_NOF_GLOBAL_LIF_BANKS(unit) : BCM_DPP_AM_DEFAULT_NOF_POOLS;
            break;
 ...
 */
STATIC int
_bcm_dpp_am_id_to_pool_id_get(int unit, int core_id, int (*get_func)(int, int, int*), int res_template_id, int *pool_id){
    int rv;
    BCMDNX_INIT_FUNC_DEFS;
...
bcm_dpp_am_resource_id_to_pool_id_get(int unit, int core_id, int resource_id, int *pool_id){
    ...
    rv = _bcm_dpp_am_id_to_pool_id_get(unit, core_id, sw_state_access[unit].dpp.bcm.alloc_mngr_utils.resource_to_pool_map.get, resource_id, pool_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_am_template_id_to_pool_id_get(int unit, int core_id, int template_id, int *pool_id){
    ...
    rv = _bcm_dpp_am_id_to_pool_id_get(unit, core_id, sw_state_access[unit].dpp.bcm.alloc_mngr_utils.template_to_pool_map.get, template_id, pool_id);
    BCMDNX_IF_ERR_EXIT(rv);
 ...
 */
STATIC int 
_bcm_dpp_am_pool_id_to_id_get(int unit, int (*get_func)(int, int, int*), int nof_pools, int pool_id, int *res_template_id, int *core_id){
    int rv;
...
int 
dpp_am_res_silent_alloc(int unit, int core_id, int res_id, uint32 flags, int count, int *elem){
...
int
dpp_am_res_used_count_get(int unit, int core_id, int res_id, int *used_count){
    int rv, pool_id;
...
int 
dpp_am_res_silent_alloc_align(int unit, int core_id, int res_id, uint32 flags, int align, int offset, int count, int *elem){
    int rv = BCM_E_NONE, pool_id;
    BCMDNX_INIT_FUNC_DEFS;
...
int
dpp_am_res_alloc_tag(int unit, int core_id, int res_id, uint32 flags, const void *tag, int count, int *elem){
    int rv, pool_id;
    BCMDNX_INIT_FUNC_DEFS;
...
int
dpp_am_res_tag_set(int unit, int core_id, int res_id, uint32 flags, int offset, int count, const void *tag) {
    int rv, pool_id;
    BCMDNX_INIT_FUNC_DEFS;
...
int
dpp_am_res_tag_get(int unit, int core_id, int res_id, uint32 flags, int elem, const void *tag) {
    int rv, pool_id;
    BCMDNX_INIT_FUNC_DEFS;
...
int
dpp_am_res_set_pool(int unit, int core_id, int res_id, sw_state_res_allocator_t manager, 
...
int
dpp_am_template_free_group(int unit, int core_id, int template_id, int template, int nof_deductions, int *is_last){
    int rv, pool_id;
    BCMDNX_INIT_FUNC_DEFS;
