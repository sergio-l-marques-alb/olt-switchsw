...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id) {
    ...
    switch (res_id) {
        ...
        case dpp_am_res_ecmp_id:
           if ( !(soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"pon_wihout_rif_support",0)) && SOC_IS_ARAD(unit))  {
                p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
                p_info.start = 1;
    ...
    break;
        ...
        break;
		    ...
		    case dpp_am_res_ether_policer_1:
            /* mostly 'next available' with blocks up to 8[???] */
			
            p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
            p_info.start = SOC_IS_JERICHO(unit) ? 0 : 1; /* don't use global policer-0 in Arad*/;
            p_info.count = SOC_IS_JERICHO(unit) ? 8 : 8 - 1;
                               ...
                               int *elem)
{
    ...
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_etpp,flags,1,elem);
        ...
        int elem)
{
    ...
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SW_STATE_RES_ALLOC_WITH_ID,trap_id);
       ...
       /*Clears virtual bit before allocation*/
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,SW_STATE_RES_ALLOC_WITH_ID,trap_id);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        LOG_DEBUG(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                              ...
                              unit, trap_id, trap_index));
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_egress,SW_STATE_RES_ALLOC_WITH_ID,trap_index);    
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    else if (_BCM_TRAP_ID_IS_ETPP(trap_id))  
    {
        ...
        trap_id = _bcm_dpp_rx_trap_id_to_etpp(trap_id);
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_etpp,SW_STATE_RES_ALLOC_WITH_ID,trap_id);    
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_single_instance,SW_STATE_RES_ALLOC_WITH_ID,trap_id);    
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        int trap_id)
{
    ...
    /* If in the range of PPD user defined*/
    if ( (SOC_PPD_TRAP_CODE_USER_DEFINED_0 <= trap_id) && (trap_id <= _BCM_LAST_USER_DEFINE) ) { 
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SW_STATE_RES_ALLOC_WITH_ID,trap_id);

    } else if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id)) { /* If virtual bit is marked */
       /*Clears virtual bit before allocation*/
        trap_id = _BCM_TRAP_ID_TO_VIRTUAL(trap_id);
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_virtual,SW_STATE_RES_ALLOC_WITH_ID,trap_id);
        ...
        rv = _bcm_dpp_am_eg_trap_index_get(unit, 
                                           ...
                                           &trap_index); /* 0...NOF_TRAPS */
        BCMDNX_IF_ERR_EXIT(rv);
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_egress,SW_STATE_RES_ALLOC_WITH_ID,trap_index);    

    }
    else if (_BCM_TRAP_ID_IS_ETPP(trap_id)) 
    {
        trap_id = _bcm_dpp_rx_trap_id_to_etpp(trap_id);
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_etpp,SW_STATE_RES_ALLOC_WITH_ID,trap_id);  
    }
    else { /* Otherwise, use single instance pool (ingress) */
        rv = dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_single_instance,SW_STATE_RES_ALLOC_WITH_ID,trap_id);  
    }
        ...
        int elem)
{
    ...
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SW_STATE_RES_ALLOC_WITH_ID,snoop_cmnd);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int snoop_cmnd)
{
    ...
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SW_STATE_RES_ALLOC_WITH_ID,snoop_cmnd));
exit:
    BCMDNX_FUNC_RETURN;
}
        ...
        uint32 *trap_code)
{
    ...
    *trap_code = elem;
    ...
    BCMDNX_IF_ERR_EXIT( OAM_ACCESS.is_internally_allocated_trap.set(unit, *trap_code, 1););
        ...
        uint32 trap_code)
{
    ...
    BCMDNX_IF_ERR_EXIT(OAM_ACCESS.is_internally_allocated_trap.set(unit, trap_code, 0));
    ...
    int *policer_id)
{
	...
	default:
		...
		break;
	case 1:
		rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer_1,flags,size,policer_id);
		BCMDNX_IF_ERR_EXIT(rv);
		break;
	}
    ...
    int policer_id)
{
	...
	default:
		...
		break;
	case 1:
		rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer_1,size,policer_id);
		BCMDNX_IF_ERR_EXIT(rv);
		break;
	}
    ...
    int policer_id)
{
	...
	default:
		...
		break;
	case 1:
		BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_ether_policer_1,1,policer_id));
		break;
	}
        ...
        int *qos_id)
{
    ...
    int elem = *qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_map_encap_intpri_color,flags,1,&elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_map_encap_intpri_color,1,elem);
    BCMDNX_IF_ERR_EXIT(rv);
        ...
        int qos_id)
{
    ...
    int elem = qos_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_map_encap_intpri_color,1,elem));
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_cosq_egr_thresh_init(int unit, int core ,int template_init_id,bcm_dpp_cosq_egress_thresh_key_info_t *egr_thresh_data_init)
{
    ...
    /* We are not going to change the macro SOC_DPP_DEFS_MAX to aviod such cases */
...
int _bcm_dpp_am_template_user_defined_trap_exchange(int unit, int port, bcm_dpp_user_defined_traps_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if (*is_last) {
       ...
       rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SW_STATE_RES_ALLOC_WITH_ID,trap_id);
       BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (*is_allocated) {
       ...
       rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_trap_user_define,SW_STATE_RES_ALLOC_WITH_ID,1,&trap_id);
       BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_snoop_cmd_exchange(int unit, int port, bcm_dpp_snoop_t * data, int *old_snoop_cmd, int *is_last, int *new_snoop_cmd,int *is_allocated)
{
    ...
    if (is_last_temp) {
        /*is_last means not in use anymore, there for free allocation*/
        rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SW_STATE_RES_ALLOC_WITH_ID,snoop_cmd);
        BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (*is_allocated) {
        ...
        rc = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_res_snoop_cmd,SW_STATE_RES_ALLOC_WITH_ID,1,new_snoop_cmd);
        BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_port_tpid_class_init(int unit,int template_init_id,_bcm_dpp_port_tpid_class_info_t* init_data)
{
    ...
    for (index = 0; index < _DPP_AM_TEMPLATE_PORT_TPID_CLASS_MAX_ENTITIES(unit); index++) {
        ...
        BCMDNX_IF_ERR_EXIT(result);
        result = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_tpid_class_eg, flags, init_data, &is_allocated,&template);
        BCMDNX_IF_ERR_EXIT(result);
    }
...
int _bcm_dpp_am_template_port_tpid_class_eg_data_get(int unit, int port, _bcm_dpp_port_tpid_class_info_t * data)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class_eg,old_profile,data);
...
int _bcm_dpp_am_template_port_tpid_class_eg_exchange(int unit, int flags, int port, _bcm_dpp_port_tpid_class_info_t * data, int *old_template, int *is_last, int *template,int *is_allocated)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK))
    {
        rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class_eg,0,data,*old_template,is_last,template,is_allocated);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
        rc = dpp_am_template_exchange_test(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class_eg,0,data,*old_template,is_last,template,is_allocated);
    }
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_template_clear(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_tpid_class_eg));
exit:
    BCMDNX_FUNC_RETURN;
}
...
/* OAM Priority - Start */
int _bcm_dpp_am_template_oam_tx_priority_alloc(int unit, int flags, const SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES *data, int *is_allocated, int *profile_ndx)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_mpls_pwe_push_profile_alloc(int unit, int flags, const SOC_PPD_MPLS_PWE_PROFILE_DATA *data, int *is_allocated, int *mpls_pwe_profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_bfd_tos_ttl_profile_alloc(int unit, int flags, const SOC_PPD_BFD_IP_MULTI_HOP_TOS_TTL_DATA *data, int *is_allocated, int *tos_ttl_profile)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_l2_prog_trap_free(int unit, int is_virtual_traps_p_set, int type, int *template,int *is_last)
{
    ...
    *template = type;
    ...
    if (*is_last) {
        ...
        {
            /*is_last means not in use anymore, there for free allocation*/
            rc = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_res_trap_virtual, SW_STATE_RES_ALLOC_WITH_ID, type);
        BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_queue_rate_cls_exchange(int unit, int core, int queue, bcm_dpp_cosq_ingress_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls,int *is_allocated)
{
    ...
    if (core == BCM_CORE_ALL) {
        ...
        for (core_index = 0; core_index < (SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores - 1); core_index++) {
            rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID,dpp_am_template_queue_rate_cls,SHR_TEMPLATE_MANAGE_SET_WITH_ID,data_rate_cls,tmp_old_rate_cls,&tmp_is_last,new_rate_cls,is_allocated);
            BCMDNX_IF_ERR_EXIT(rc);
        }
    }
...
_bcm_dpp_am_template_queue_rate_cls_allocate_group(int unit, uint32 flags, bcm_dpp_cosq_ingress_rate_class_info_t *data_rate_cls, int ref_count, int *is_allocated, int *template)
{
    ...
    if (ref_count > 0) {
        ...
        rc = dpp_am_template_free_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_queue_rate_cls, *template, -ref_count, &is_last);
        BCMDNX_IF_ERR_EXIT(rc);
    }
...
_bcm_dpp_am_template_l2_flooding_mapping_get(int unit, SOC_PPD_LIF_ID lif_index,int *profile)
{
    ...
    if ((lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
        ...
        (lif_entry_info->type != SOC_PPD_LIF_ENTRY_TYPE_EXTENDER)) {
