                                  ...
                                  const bcm_field_extraction_field_t *extraction)
{
    ...
    for (index = 0, totalBits = 0;
        ...
        if (extraction[index].flags &
           ...
           tempAct.fld_ext[index].type = BCM_FIELD_ENTRY_INVALID;
    ...
    if (0 < maxCount) {
        for (offset = 0;
            ...
            if (!_BCM_DPP_FIELD_PPD_QUAL_VALID(entryDe_fld_ext_type)) {
                extraction[offset].qualifier = BCM_FIELD_ENTRY_INVALID;
                ...
                if ((bcmFieldQualifyCount > ppdQual) && (BCM_FIELD_ENTRY_INVALID != ppdQual)) {
                    extraction[offset].qualifier = ppdQual;
                } else {
                                              ...
                                              _bcm_dpp_field_ent_idx_t entry)
{
    ...
    BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.entryDe.entryCmn.entryGroup.get(unit, entry, &group));
    BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.groupD.hwHandle.get(unit, group, &hwHandle));
    ...
    for (index = 0; index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; index++) {
        ...
        } /* if (SOC_PPD_NOF_FP_ACTION_TYPES > entryDe_hwParam.type) */
    } /* for (all actions) */
                                  ...
                                  const char *prefix)
{
    ...
    if ((entryDe_entryFlags & (_BCM_DPP_FIELD_ENTRY_IN_HW |
        ...
        BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.groupD.hwHandle.get(unit, entryDe_entryGroup, &groupD_hwHandle));
    ...
    if (entryDe_entryFlags & _BCM_DPP_FIELD_ENTRY_IN_USE) {
        ...
        dumpedRanges = FALSE;
        {
            _bcm_dpp_field_qual_t entryDe_entryQual[SOC_PPD_FP_NOF_QUALS_PER_DB_MAX] ;
        ...
        for (index = 0; index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; index++) {
            ...
            if (_BCM_DPP_FIELD_PPD_ACTION_VALID(entryDe_type)) {
                ...
                BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.entryDe.deAct.bcmType.get(unit, entry, index, &entryDe_bcmType));
                if (!BCM_FIELD_ASET_TEST(actions, entryDe_bcmType)) {
                ...
                BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.entryDe.deAct.bcmType.get(unit, entry, index, &entryDe_bcmType));
                BCM_FIELD_ASET_ADD(actions, entryDe_bcmType);
            } /* if (action exists) */
        } /* for (index = 0; index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; index++) */
        if (0 == count) {
            /* nothing in qualifiers */
                                          ...
                                          bcm_field_action_t type)
{
    ...
    for (index = 0;
        ...
        if (type == entryDe_bcmType) {
            ...
            BCMDNX_IF_ERR_EXIT(FIELD_ACCESS.entryDe.deAct.hwParam.type.set(unit, entry, index, SOC_PPD_FP_ACTION_TYPE_INVALID));
                                   ...
                                   _bcm_dpp_field_ent_idx_t entry)
{
    ...
    for (index = 0; index < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX; index++) {
        ...
        FIELD_ACCESS.entryDe.deAct.hwParam.type.set(unit, entry, index, SOC_PPD_FP_ACTION_TYPE_INVALID);
        for (offset = 0; offset < SOC_PPD_FP_DIR_EXTR_MAX_NOF_FIELDS; offset++) {
                                     ...
                                     _bcm_dpp_field_ent_idx_t source)
{
    ...
    _bcm_dpp_field_ent_idx_t prev;
    uint32 entryDe_entryFlags;
    _bcm_dpp_field_entry_dir_ext_t sourceData;
    int unit;
                ...
                if ((COMPILER_64_HI(entryDe_qualData) !=
                    ...
                    LOG_WARN(BSL_LS_BCM_FP,
                             (BSL_META_U(unit,
                              ...
                              COMPILER_64_HI(entryDe_qualMask),
                              COMPILER_64_LO(entryDe_qualMask),
                              SOC_PPD_FP_QUAL_TYPE_to_string(entryDe_hwType),
                              entryDe_hwType,
                              offset,
            ...
            if (offset < SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX) {
                ...
                LOG_WARN(BSL_LS_BCM_FP,
                         (BSL_META_U(unit,
                          ...
                          ((entryDe_bcmType<bcmFieldActionCount)  && (entryDe_bcmType != BCM_FIELD_ENTRY_INVALID)) ?_bcm_dpp_field_action_name[entryDe_bcmType]:"-",
                          entryDe_bcmType,
                          SOC_PPD_FP_ACTION_TYPE_to_string(entryDe_type),
                          entryDe_type));
#else /* !_BCM_DPP_FIELD_GET_VERIFY_RETURN_ERROR */
                BCMDNX_ERR_EXIT_MSG(_BCM_DPP_FIELD_GET_VERIFY_ERROR,
