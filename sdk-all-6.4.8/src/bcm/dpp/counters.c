...
#define BCM_DPP_COUNTER_SOURCE_MATCH(unitData, crps_index, _src_core, _src_type, _command_id, _eg_type)       \
        if ((unitData->proc[crps_index].mode.src_type == _src_type) &&                                                     \
            ...
            (unitData->proc[crps_index].valid)) \
...
#define COUNTER_UPDATE_SELECT_PROC_INFO       0x8 
...
#define COUNTER_UPDATE_SELECT_ALL (COUNTER_UPDATE_SELECT_THREAD        | \
                                   ...
                                   COUNTER_UPDATE_SELECT_PROC_INFO     | \
                                   COUNTER_UPDATE_SELECT_BACKROUND_INFO| \
                                   COUNTER_UPDATE_SELECT_LIF_INFO)
STATIC int 
_bcm_dpp_counter_proc_select_update(int unit, 
                                    int engine, 
                                    uint32 select_bitmap,
                                    _bcm_dpp_counter_proc_info_t* proc_info) {
    int offset;
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_DPP_UNIT_CHECK(unit);
    if (select_bitmap & COUNTER_UPDATE_SELECT_PROC_INFO) {
...
STATIC int 
_bcm_dpp_counter_unit_select_update(int unit, 
                                    uint32 select_bitmap,
    ...
    if (!SOC_WARM_BOOT(unit)) {
        unitData->unit = unit;
        rv = COUNTERS_ACCESS.unit.set(unit, unit);
        BCMDNX_IF_ERR_EXIT(rv);

        if (select_bitmap & COUNTER_UPDATE_SELECT_THREAD) {
            rv = COUNTERS_ACCESS.running.set(unit, unitData->running);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = COUNTERS_ACCESS.bgWait.set(unit, unitData->bgWait);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        for (engine = 0; engine < (SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)); engine++) {
            rv = _bcm_dpp_counter_proc_select_update(unit, engine, select_bitmap, &(unitData->proc[engine]));
            BCMDNX_IF_ERR_EXIT(rv);
        }
        if (select_bitmap & COUNTER_UPDATE_SELECT_DIAGS) {
            for (fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++) {
                rv = COUNTERS_FIFO_ACCESS.set(unit, fifo, &(unitData->fifo[fifo]));
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
        if (select_bitmap & COUNTER_UPDATE_SELECT_PROC_INFO) {
...
STATIC int 
_bcm_dpp_counter_unit_update(int unit, 
                             _bcm_dpp_counter_state_t *unitData) {
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_DPP_UNIT_CHECK(unit);
    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_ALL, unitData);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
...
STATIC int 
_bcm_dpp_counter_proc_fill(int unit, int engine, _bcm_dpp_counter_proc_info_t* proc_info) {
    int offset;
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    BCM_DPP_UNIT_CHECK(unit);
...
STATIC int 
_bcm_dpp_counter_unit_fill(int unit, _bcm_dpp_counter_state_t **unitDataPtr) {
    ...
    unitData->unit = unit;
    rv = COUNTERS_ACCESS.running.get(unit, &unitData->running);
    BCMDNX_IF_ERR_EXIT(rv);
    rv = COUNTERS_ACCESS.bgWait.get(unit, &unitData->bgWait);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    for (engine = 0; engine < (SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)); engine++) {
        rv = _bcm_dpp_counter_proc_fill(unit, engine, &(unitData->proc[engine]));
        BCMDNX_IF_ERR_EXIT(rv);
    }
    for (fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++) {
...
STATIC int 
_bcm_dpp_counter_unit_check_and_fill(int unit, _bcm_dpp_counter_state_t **unitData) {
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        if (_bcm_dpp_counter_state_buffer[unit].dirty_bitmap) {
            rv = _bcm_dpp_counter_unit_select_update(unit, _bcm_dpp_counter_state_buffer[unit].dirty_bitmap, *unitData);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        rv = _bcm_dpp_counter_unit_fill(unit, unitData);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
_bcm_dpp_counter_state_set_all(_bcm_dpp_counter_state_t *unitData, int proc, uint64 counter) {
    ...
    for (ofs = 0; ofs < unitData->proc[proc].mode.num_counters; ofs += _SUB_COUNT) {
...
bcm_dpp_counter_state_diag_unmask(int unit) {
    ...
    /*restore data*/
    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD, unitData); 
    BCMDNX_IF_ERR_EXIT(rv);
    _bcm_dpp_counter_state_buffer[unit].dirty_bitmap &= ~(COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD);
    if (unitData->counter) {
        for (proc = 0; proc < unitData->num_counter_procs; proc++) {
            if (!unitData->proc[proc].valid) {
                continue;
            }
            for (ofs = 0; 
                 ofs < ((proc < SOC_DPP_DEFS_GET(unit, nof_counter_processors)) ?  
                        SOC_DPP_DEFS_GET(unit, counters_per_counter_processor) : 
                        SOC_DPP_DEFS_GET(unit, counters_per_small_counter_processor)); ofs += _SUB_COUNT) {
...
bcm_dpp_counter_state_diag_mask(int unit) {
    ...
    rv = _bcm_dpp_counter_unit_fill(unit, &unitData); /*save data*/
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /*write zero data*/
    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD, zeroUnitData);
    BCMDNX_IF_ERR_EXIT(rv);
    _bcm_dpp_counter_state_buffer[unit].dirty_bitmap |= COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD;
                              ...
                              unsigned int *depth)
{
    ...
    for (proc = 0; proc < unitData->num_counter_procs; proc++) {
        ...
        soc_sandResult = (MBCM_DPP_DRIVER_CALL((unit), mbcm_dpp_cnt_algorithmic_read, (unit, fifo_dma_index, counter_info_array, data_p)));
...
void _bcm_dpp_counter_bg(void *unitPtr) {
    ...
    if (unitData) {
        ...
        rv = _bcm_dpp_counter_unit_check_and_fill(unit, &unitData);
        BCMDNX_IF_ERR_EXIT(rv);
        /* mark unit as running */

        unitData->running = TRUE;
        rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_THREAD, unitData);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        while (unitData->running) {
             ...
             *  destroying a unit.
             */
            semTaken = (!sal_sem_take(COUNTERS_SYNC_ACCESS.bgSem, unitData->bgWait));
            ...
            if ((!unitData->diag.fifo_read_deferred) && (!unitData->background_disable)) {
                ...
                if (!mutexTaken) {
                    ...
                    rv = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_THREAD | COUNTER_UPDATE_SELECT_BACKROUND_INFO, unitData);
                    BCMDNX_IF_ERR_EXIT(rv);
                    continue;
                }
                ...
                mutexTaken = (0 == sal_mutex_give(COUNTERS_SYNC_ACCESS.cacheLock));
            ...
            unitData->background_active = FALSE;
            if (!(_bcm_dpp_counter_state_buffer[unit].dirty_bitmap & COUNTER_UPDATE_SELECT_THREAD) && 
                (!SOC_WARM_BOOT(unit))) {
                rv = COUNTERS_ACCESS.running.get(unit, &unitData->running);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        ...
        rv = _bcm_dpp_counter_unit_update(unit, unitData);
        BCMDNX_IF_ERR_EXIT(rv);
    }
exit:
                                ...
                                unsigned int num)
{
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_BACKROUND_INFO, unitData);
    BCMDNX_IF_ERR_EXIT(result);
                                 ...
                                 uint64 *counter)
{
    ...
    if (update) {
        ...
        for (ofs = 0; ofs < _OFS_COUNT; ofs++) {
            if (offsets & (1 << ofs)) {
                ...
                if (done) {
                    ...
                    result = COUNTERS_ACCESS.counter.set(unit, proc, index + ofs, counter_accum);
                    BCMDNX_IF_ERR_EXIT(result);                   
                } else {
                    result = COUNTERS_ACCESS.counter.set(unit, proc, index + ofs, *counter);
                    BCMDNX_IF_ERR_EXIT(result);                   
                    done = TRUE;
                }
            } /* if (offsets & (1 << ofs)) */
        ...
        for (ofs = 0; ofs < _OFS_COUNT; ofs++) {
            if (offsets & (1 << ofs)) {
                result = COUNTERS_ACCESS.counter.get(unit, proc, index + ofs, &counter_accum);
                BCMDNX_IF_ERR_EXIT(result);                   
...
_bcm_dpp_counter_native(_bcm_dpp_counter_state_t *unitData,
    ...
    if (update) {
        unitData->proc[proc].diag.cache_writes++;
        result = COUNTERS_ACCESS.counter.set(unit, proc, index + offset, *counter);
        BCMDNX_IF_ERR_EXIT(result);
    } else {
        unitData->proc[proc].diag.cache_reads++;
        result = COUNTERS_ACCESS.counter.get(unit, proc, index + offset, counter);
        BCMDNX_IF_ERR_EXIT(result);
    } 
exit:
    BCMDNX_FUNC_RETURN;
...
_bcm_dpp_counters_set_id_inside_lif_range(_bcm_dpp_counter_state_t *unitData, unsigned int proc, int set_id) {
    ...
    result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_lif_counting_range_get, (unit, unitData->proc[proc].mode.src_type, unitData->proc[proc].mode.command_id, &range));
    BCMDNX_IF_ERR_EXIT(result);
                        ...
                        uint64 *counter)
{
    ...
    unsigned int lfirst;
    BCMDNX_INIT_FUNC_DEFS;
    result = _bcm_dpp_counter_unit_check_and_fill(unit, &unitData);
    BCMDNX_IF_ERR_EXIT(result);
    if (!unitData->proc[proc].valid) {
    ...
    if (!((base + num - 1) < (unitData->proc[proc].mode.num_counters)) /*We keep this condition as backward compatibale - if the counter was allready scaled.*/ 
        && 
        !((base >= base_limit_down) && ((base + num - 1) < base_limit_up))) {
    ...
    result = _bcm_dpp_counter_unit_update(unit, unitData);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_counter_bg_thread_detach(_bcm_dpp_counter_state_t *unit_data){
    ...
    if (unit_data->running){
        ...
        BCMDNX_IF_ERR_EXIT(COUNTERS_ACCESS.running.set(unit, FALSE));
...
int _bcm_dpp_counter_dma_attach(_bcm_dpp_counter_state_t *unitData){
    ...
    uint32                sand_rv;
    int unit = unitData->unit;
    int dma_index = 0, dma_index_old = -1;
    int cache_length = 0;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(unitData);
    BCM_DPP_UNIT_CHECK(unit);
    ...
    if (SOC_IS_ARAD(unit)) {
        for (index = 0; index < SOC_DPP_DEFS_GET(unit, nof_counter_processors); index++) {
            if (unitData->proc[index].valid) {
                SOCDNX_SAND_IF_ERR_RETURN(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_engine_to_fifo_dma_index, (unit, index, &dma_index, &cache_length)));
                if (dma_index == dma_index_old) {
                    continue;
                }
                sand_rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_dma_set, ((unit), dma_index, cache_length));
                BCM_SAND_IF_ERR_EXIT(sand_rv);
                dma_index_old = dma_index;
            }
        }
    }
exit:
...
int _bcm_dpp_counter_dma_detach(_bcm_dpp_counter_state_t *unitData){
    ...
    uint32                sand_rv;
    int unit = unitData->unit;
    int dma_index = 0, dma_index_old = -1, cache_length = 0;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(unitData);
    BCM_DPP_UNIT_CHECK(unit);
    ...
    if (SOC_IS_ARAD(unit)) {
        for (index = 0; index < SOC_DPP_DEFS_GET(unit, nof_counter_processors); index++) {
            if (unitData->proc[index].valid) {
                ...
                sand_rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_dma_unset, (unit, dma_index));
                BCM_SAND_IF_ERR_EXIT(sand_rv);
                dma_index_old = dma_index;
            }
        }
...
bcm_dpp_counter_background_collection_enable_set(int unit, int enable){
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_counter_unit_check_and_fill(unit, &unitData));
    if(enable){
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_counter_unit_update(unit, unitData));
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_counter_background_collection_enable_get(int unit, int *enable){
    ...
    BCMDNX_IF_ERR_EXIT(COUNTERS_ACCESS.running.get(unit, enable));
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_counter_lif_range_set(
   ...
   SOC_SAND_U32_RANGE   *range
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_LIF_INFO | COUNTER_UPDATE_SELECT_PROC_INFO, unitData);
    BCMDNX_IF_ERR_EXIT(result);
   ...
   int                                  range_id) 
{
    ...
    int is_ingress = (source == SOC_TMC_CNT_SRC_TYPE_ING_PP);
    int max_lif_stack_to_count = SOC_TMC_CNT_LIF_COUNTING_NUMBER_OF_STACK_IDS(source);
    uint8 found = 0;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(unitData);
    ...
    for (found = 0, stack_level_to_count = bcmStatCounterLifStackId0, lif_counting_mask = 0; 
         lif_counting_mask < SOC_TMC_CNT_LIF_COUNTING_MASK_MAX(source); 
        ...
        for (stack_level = 0; stack_level < max_lif_stack_to_count; stack_level++) {
            if (SOC_TMC_CNT_LIF_COUNTING_MASK_TO_FIELD_VAL(range_id) == SOC_TMC_CNT_LIF_COUNTING_MASK_FIELD_VAL_GET(source, lif_counting_mask, stack_level)) {
                if (!found || 
   ...
   int                                  priority)
{
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_LIF_INFO, unitData);
    BCMDNX_IF_ERR_EXIT(result);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_counter_mode_config(_bcm_dpp_counter_state_t *unitData, soc_port_t crps_index, SOC_TMC_CNT_MODE_STATISTICS stat) {
    ...
    switch (stat) {
        case SOC_TMC_CNT_MODE_STATISTICS_COLOR_RES_ENQ_HI: /*SIMPLE_COLOR*/
            if (SOC_IS_ARAD(unit)) { /*Arad and above*/
...
bcm_dpp_counter_config_set(
    ...
    unitData->proc[crps_index].mode.num_counters = 
        (crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors)) ? 
        SOC_DPP_DEFS_GET(unit, counters_per_counter_processor) : 
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_PROC_INFO, unitData);
    BCMDNX_IF_ERR_EXIT(result);
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_counter_config_clear(int unit, soc_port_t crps_index, uint8 clear_cache){
    ...
    unitData->proc[crps_index].valid = FALSE;
...
{
    _bcm_dpp_counter_state_t *tempUnit;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        result = _bcm_dpp_counter_unit_fill(unit, &tempUnit);
        BCMDNX_IF_ERR_EXIT(result);
    }
    tempUnit->unit = unit;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (index = 0; index < tempUnit->num_counter_procs; index++) {
            ...
            tempUnit->proc[index].mode.num_counters = 
    ...
    result = _bcm_dpp_counter_unit_update(unit, tempUnit);
    BCMDNX_IF_ERR_EXIT(result);
    ...
    if (SOC_WARM_BOOT(unit)) {
        if (tempUnit->running == TRUE) {
            ...
            LOG_DEBUG(BSL_LS_BCM_CNT, (BSL_META_U(unit, "unit %d counter background thread reinit\n"), unit));
            result = _bcm_dpp_counter_dma_attach(tempUnit);
            BCMDNX_IF_ERR_EXIT(result);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_THREAD, tempUnit);
        BCMDNX_IF_ERR_EXIT(result);
...
_bcm_dpp_counter_egress_que_build_pointer(_bcm_dpp_counter_state_t *unitData,
    ...
    if (unitData->proc[proc].mode.mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_TM) {
        ...
        result = soc_port_sw_db_local_to_out_port_priority_get(unit, tm_port, &nof_priorities);
        BCMDNX_IF_ERR_EXIT(result);
