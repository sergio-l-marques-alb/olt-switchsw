...
#define BCM_IF_ERROR_RETURN_WITH_FREE(_rv_, e, p, max_idx) {    \
    ...
    e = (uint32 *) soc_cm_salloc(unit, SOC_MAX_MEM_FIELD_WORDS * 4, "ACL Control");
        if (!e) {
            return (BCM_E_MEMORY);
        }

        p = (uint32 **) soc_cm_salloc(unit, _FP_MAX_ENTRY_WIDTH * 4, "ACL Control");
        if (!p)
        {
            soc_cm_sfree(unit, e);
            return (BCM_E_MEMORY);
        ...
        {
            p[i]= (uint32 *) soc_cm_salloc (unit, SOC_MAX_MEM_FIELD_WORDS *4, "ACL Control");
            if (!p[i])
            {
                while (i > 0) {
