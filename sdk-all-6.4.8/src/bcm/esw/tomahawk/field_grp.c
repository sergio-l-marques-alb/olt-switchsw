                             ...
                             uint16 qual_count, uint16 key_size)
{
    ...
    */
    ...
    for (idx = 0; idx < qual_count; idx++) {
        ...
        qid = f_qual_arr[idx]->qid;
        ibus_qual_info[qid] =  stage_fc->input_bus.qual_list[qid];
    }
    ...
    for (idx = 0; idx < qual_count; idx++) {
        ...
        num_configs = 0;
        ibus_qual = stage_fc->input_bus.qual_list[qid];
    ...
    while (1) {
        ...
        for (ibus_qual = ibus_qual_info[qid]; 
             NULL != ibus_qual; ibus_qual = ibus_qual_info[qid]->next) {
        ...
        sal_memcpy(ibus_qual_info[qid],
                   stage_fc->input_bus.qual_list[qid],
                   sizeof(_field_qual_sec_info_t));
        ...
        while (idx1 < qual_count) {
            ...
            if (NULL == stage_fc->input_bus.qual_list[qid2]) {
                continue;
            }
            ...
            if (ibus_qual_info[qid2]->next) {
                ...
                ibus_qual_info[qid2] = 
                           stage_fc->input_bus.qual_list[qid2];
            }
        }
        ...
        for (part = 0; part < _FP_MAX_ENTRY_WIDTH; part++) {
            _FP_EXT_SELCODE_CLEAR(fg->ext_codes[part]);
        }
    }
                               ...
                               bcm_field_qset_t *qset_req, _field_group_t *fg)
{
     ...
     * input bus.
     */
    rv = _field_th_group_update_keysize(unit, stage_fc, f_qual_arr, 
    ...
    rv = _field_th_group_process_qset(unit, stage_fc, fg, 
