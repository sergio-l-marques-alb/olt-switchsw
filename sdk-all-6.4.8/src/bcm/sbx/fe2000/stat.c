                        ...
                        uint64 *value)
{
    ...
    case snmpIfInErrors:  /* RX Errors or Receive packets - non-error frames */
        ...
        STAT_ADD_BLOCK_AM(unit, port, fe2000CounterXmRxMtuExceededPktCnt, count);
...
bcm_fe2000_stat_init(int unit)
{
    ...
    if (BCM_FAILURE(result)) {
        LOG_ERROR(BSL_LS_BCM_STAT,
                  (BSL_META_U(unit,
                              "bcm_stat_init: sw_counter_init failed: %s\n"),
                   bcm_errmsg(result)));
        return result;
    }
    ...
    for (blki=0; blki < sizeof(init_blk) / sizeof(init_blk[0]); blki++) {
        ...
        if (BCM_FAILURE(result)) {
            LOG_ERROR(BSL_LS_BCM_STAT,
                      (BSL_META_U(unit,
                                  "%s(%d) -Could not attach counter block %d: %s\n"),
                                     ...
                                     bcm_stat_sbx_info_t *pCtlBlock)
{
    ...
    if (NULL == pCtlBlock) {
        LOG_DEBUG(BSL_LS_BCM_STAT,
                  (BSL_META_U(unit,
                              "%s: control block NULL; trying defaults\n"),
                   FUNCTION_NAME()));
