...
int field_evpn_esi_db(int unit, int group_priority) {
    ...
    result = bcm_field_qualify_InterfaceClassVPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, cint_field_evpn_info.inlif_profile_evpn, cint_field_evpn_info.inlif_profile_evpn_mask);
    if (BCM_E_NONE != result) {
...
int field_evpn_evpn_db(int unit, int group_priority, bcm_field_ForwardingType_t forwarding_type) {
    ...
    result = bcm_field_qualify_ForwardingType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, forwarding_type);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_qualify_ForwardingType\n");
        return result;
    }
    ...
    BCM_FIELD_ASET_ADD(grp_lsb.aset, bcmFieldActionLearnSrcPortNew);
    ...
    BCM_FIELD_ASET_ADD(grp_msb.aset, bcmFieldActionLearnInVPortNew);
