...
int mirror_with_erspan_example(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int span_id, 
    ...
    bshell(0,"m IPT_STAMPING_FTMH_OUTLIF_ENABLE STAMP_FTMH_OUTLIF_TYPE_EN=0x7775");
...
void erspan_set_eei_invalid(bcm_gport_t mirror_id)
{
    int offset;
    char cmd[300] = {0};
    ...
    if (BCM_GPORT_IS_MIRROR_MIRROR(mirror_id)) {
        ...
        /* This is temporary solution to set EEI, dedicated API will be provided in later version */
...
int mirror_into_tunnel_iproute(int unit, int in_vlan, int in_port, int dest_port, int mirror_port, bcm_if_t ip_tunnel, int erspan_value, 
    ...
    rv = basic_example(0,1,in_port,dest_port);
    if (rv != BCM_E_NONE) {
    ...
    /* set eei to invalid*/
    erspan_set_eei_invalid(dest.mirror_dest_id);
