...
/* $Id: cint_stg.c,v 1.7 Broadcom SDK $
...
stg(int unit, stg_info_s *info) {
    ...
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_PORT_ADD(pbmp, info->port_1);
    BCM_PBMP_PORT_ADD(pbmp, info->port_2);
           
    rv= bcm_vlan_port_add(unit, info->vlan_1, pbmp, pbmp);
    if (BCM_FAILURE(rv)) {
        printf("BCM FAIL %d: %s\n", rv, bcm_errmsg(rv));
        return CMD_FAIL;
    }

    rv = bcm_vlan_port_add(unit, info->vlan_2, pbmp, pbmp);
    if (BCM_FAILURE(rv)) {
        printf("BCM FAIL %d: %s\n", rv, bcm_errmsg(rv));
        return CMD_FAIL;
    }
 ...
 */
int
revert_stg(int unit, stg_info_s *info) {
    int rv;    
