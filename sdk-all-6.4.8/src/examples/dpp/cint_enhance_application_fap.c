 ...
 */
int create_voq_connector(int unit, int num_cos_levels, int destination_local_port, bcm_gport_t *voq_connector)
{
    bcm_error_t rv;
    int my_modid;
...
{    
    ...
    /*  1. Create VOQ connector */
    rv = create_voq_connector(unit,num_cos_level,destination_local_port,&mg1_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    rv = create_voq_connector(unit,num_cos_level,destination_local_port,&mg2_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
    ...
    rv = create_voq_connector(unit,num_cos_level,destination_local_port,&mg3_voq_connector);
    if (rv != BCM_E_NONE) {
        print rv;
        printf("(%s) \n",bcm_errmsg(rv));
        return rv;
