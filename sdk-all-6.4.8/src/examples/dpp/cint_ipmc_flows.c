...
int ipmc_flows_rif_ipmc_disabled(int unit, int inP, int outP, int outP2 ,int outP3 ,int vlan1, int vlan2, int vlan3, int vlan4){
    ...
    if (ipmc_ssm_enable) {
        ...
        rv = add_ingress_multicast_forwarding(unit, ipmc_flows_info1.ipmc_mc_groups[1], ipmc_flows_info1.forwarding_ports_1, 3, vlan2); /* 4.1.1 */
        if (rv != BCM_E_NONE) {
            printf("Error, add_ingress_multicast_forwarding \n");
            return rv;
        }
        rv = add_ingress_multicast_forwarding(unit, ipmc_flows_info1.ipmc_mc_groups[2], ipmc_flows_info1.forwarding_ports_1, 0, vlan2); /* 4.1.2 */
        if (rv != BCM_E_NONE) {
            printf("Error, add_ingress_multicast_forwarding \n");
            return rv;
        }
    ...
    */
    ...
    if (ipmc_ssm_enable) {
        ...
        rv = create_forwarding_entry_dip_sip(unit,ipmc_flows_info1.mc_ip_arr[0],0xc0a8000f/*sip*/ ,ipmc_flows_info1.ipmc_mc_groups[1],vlan1); /* 7.1.1 */
        if (rv != BCM_E_NONE) {
            printf("Error, create_forwarding_entry_dip\n");
            return rv;
        }
        rv = create_forwarding_entry_dip_sip(unit,ipmc_flows_info1.mc_ip_arr[0],0xc0a8000e/*sip*/ ,ipmc_flows_info1.ipmc_mc_groups[2],vlan1); /* 7.1.2 */
        if (rv != BCM_E_NONE) {
            printf("Error, create_forwarding_entry_dip\n");
            return rv;
        }
