/* $Id: cint_port_extender_dynamic_switching.c,v 1.6 Broadcom SDK $
...
*      cint> port_extender_dynamic_swithing_coe_service(0);
*      cint> port_extender_dynamic_swithing_coe_service_clean_up(0);
*/
...
/* Init coe port info */
int port_extender_dynamic_swithing_coe_info_init(uint32 index, uint32 coe_port, bcm_port_t coe_vlan, uint32 phy_port)
{
...
int port_extender_dynamic_swithing_coe_mapping_set(int unit, int coe_vlan, bcm_port_t coe_port, bcm_port_t phy_port)
{
    int rv = 0;
    bcm_port_extender_mapping_info_t mapping_info;

    mapping_info.vlan = coe_vlan;
    mapping_info.pp_port = coe_port;
    mapping_info.phy_port = phy_port;

    /* Map port and COE VLAN ID to in pp port */
    rv = bcm_port_extender_mapping_info_set(unit, BCM_PORT_EXTENDER_MAPPING_INGRESS, bcmPortExtenderMappingTypePortVlan, &mapping_info);
    if (rv != BCM_E_NONE) {
    ...
    if (enable) {
        for (index = 0; index < nof_coe_ports; index++) {
            if (coe_port_info[index].coe_port == coe_port) {
                rv = port_extender_dynamic_swithing_coe_mapping_set(unit, coe_port_info[index].coe_vlan, coe_port_info[index].coe_port, coe_port_info[index].phy_port);
                if (rv != BCM_E_NONE) {
...
int port_extender_dynamic_swithing_coe_service(int unit)
{
    ...
    rv = is_jericho_only(unit, &chip_get);
    if (rv != BCM_E_NONE) {
        printf("is_jericho_only failed $rv\n");
        return rv;
    }
    if (chip_get) {
    ...
    rv = is_qumran_mx_only(unit, &chip_get);
    if (rv != BCM_E_NONE) {
        printf("is_qumran_mx_only failed $rv\n");
        return rv;
    }
    if (chip_get) {
        if_set_force_local = 0;
    ...
    if (if_set_force_local == 0)  {
        ...
        bshell(0,"m IPT_FORCE_LOCAL_OR_FABRIC FORCE_LOCAL=1");
    }
