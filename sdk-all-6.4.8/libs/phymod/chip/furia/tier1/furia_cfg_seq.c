                       ...
                       const phymod_phy_inf_config_t* config)
{
    ...
    if (config_copy.interface_type == phymodInterfaceCR && 
    ...
    if(
       ...
       (curr_intf[0] == phymodInterfaceSR4 &&
        ...
        lane_map = FURIA_IS_DUPLEX(chip_id) ? 0xF : 0xFF;
            ...
            || prev_phy_type == FURIA_PHYTYPE_PROP) && 
            (phy_type == FURIA_PHYTYPE_FP || phy_type == FURIA_PHYTYPE_PROP) &&
            (config_copy.data_rate == SPEED_10G || config_copy.data_rate == SPEED_11G || 
             config_copy.data_rate == SPEED_25G || config_copy.data_rate == SPEED_27G)) 
    {
        /* USED TO MOVE FROM CR TO SR PER DIE
...
STATIC uint16_t _furia_get_phy_type(phymod_interface_t intf, uint32_t speed)
{
    ...
    switch (intf) {
        ...
        case phymodInterfaceXLAUI:
            if (((speed == SPEED_25G) || (speed == SPEED_27G)) && (intf == phymodInterfaceCR)) {
                phy_type = 0x1; 
            } else {
                phy_type = 0x4; 
            }
    ...
    switch (intf) {
        ...
        case phymodInterfaceCX4:
            if (speed == SPEED_10G || speed == SPEED_11G ||
                ...
                link_type = 0x4;
            } else if ( speed == SPEED_25G || speed == SPEED_27G) {
                link_type = 0x5;
            }
        break;
        case phymodInterfaceVSR:
