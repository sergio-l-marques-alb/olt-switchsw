...
typedef struct _sbZfC2PrCcTcamConfigEntry {
  uint32 m_uMask213_192;
  uint32 m_uMask191_160;
  uint32 m_uMask159_128;
  uint32 m_uMask127_96;
  uint32 m_uMask95_64;
  uint32 m_uMask63_32;
  uint32 m_uMask31_0;
  uint32 m_uData213_192;
  uint32 m_uData191_160;
  uint32 m_uData159_128;
  uint32 m_uData127_96;
  uint32 m_uData95_64;
  uint32 m_uData63_32;
  uint32 m_uData31_0;
  uint32 m_uValid;
...
#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK213_192(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) (pFromData)[51] ; \
           (nToData) |= (uint32) (pFromData)[52] << 8; \
           (nToData) |= (uint32) ((pFromData)[53] & 0x3f) << 16; \
          } while(0)

#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK191_160(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) (pFromData)[47] ; \
           (nToData) |= (uint32) (pFromData)[48] << 8; \
           (nToData) |= (uint32) (pFromData)[49] << 16; \
           (nToData) |= (uint32) (pFromData)[50] << 24; \
          } while(0)

#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK159_128(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) (pFromData)[43] ; \
           (nToData) |= (uint32) (pFromData)[44] << 8; \
           (nToData) |= (uint32) (pFromData)[45] << 16; \
           (nToData) |= (uint32) (pFromData)[46] << 24; \
          } while(0)

#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK127_96(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) (pFromData)[39] ; \
           (nToData) |= (uint32) (pFromData)[40] << 8; \
           (nToData) |= (uint32) (pFromData)[41] << 16; \
           (nToData) |= (uint32) (pFromData)[42] << 24; \
          } while(0)

#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK95_64(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) (pFromData)[35] ; \
           (nToData) |= (uint32) (pFromData)[36] << 8; \
           (nToData) |= (uint32) (pFromData)[37] << 16; \
...
#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_MASK63_32(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) (pFromData)[33] << 16; \
...
#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_DATA213_192(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           (nToData) = (uint32) ((pFromData)[24] >> 2) & 0x3f; \
           (nToData) |= (uint32) (pFromData)[25] << 6; \
           (nToData) |= (uint32) (pFromData)[26] << 14; \
          } while(0)
...
#define SB_ZF_C2PRCCTCAMCONFIGENTRY_GET_DATA191_160(nToData,pFromData,nMaxFromDataIndex) \
          do { \
           ...
           (nToData) |= (uint32) (pFromData)[23] << 22; \
           (nToData) |= (uint32) ((pFromData)[24] & 0x03) << 30; \
          } while(0)
