...
/*
 * $Id: g2p3_cls.h,v 1.48 Broadcom SDK $
 *
...
typedef struct soc_sbx_g2p3_cls_program_attribute_s {
    uint8     uNumber;        /* physical program number                    */
    uint8     uActive;        /* if 1 the program is active or its a backup */
    uint32    uBaseAddress;   /* Base Address of this program               */
    uint32    uLength;        /* Length of this program                     */
    soc_sbx_g2p3_cls_schema_enum_t sType; /* Schema type ingress or egress    */    
    uint8     uValid;         /* if 1 the program is valid                  */
} soc_sbx_g2p3_cls_prog_attr_t;

/* Schema Layout Configuration Structure */
typedef struct soc_sbx_g2p3_schema_layout_s {
    uint8     uNumRs;   /* number of rules sets this schema must support */
    uint32    uNumRulesPerRs; /* number of rules per rule set */
    uint8     uNumRcInstance; /* number of RCE instance this schema must use   */
    uint8     uBaseRcInstance;/* Base rc instance to start with. ZERO Based    */
    uint32    uBankedMode;
...
typedef uint8 soc_sbx_g2p3_bit_vector_t;
/**
...
{
  uint32 uMemTotal; /* Total memory per Rc */
  uint32 uRsMax;   /* Maximum rulesets per Rc */
  uint32 uMemUsed; /* Committed memory Usage */
  uint32 uRsUsed;  /* Committed Rs Usage */
  uint32 uMemTobeUsed; /* Extra Memory needed for next commit */
  uint32 uRsTobeUsed;  /* Extra Rs needed for next commit */
...
typedef struct soc_sbx_g2p3_cls_pattern_memory_s
{
  ...
  uint32                  uSaveResult[G2P3_MAX_CLS_RULESETS_PER_SCHEMA]\
                                       [G2P3_MAX_RC_INSTANCE];
...
typedef struct soc_sbx_g2p3_cls_comp_s
{
  ...
  uint8                          sbyProgNumber[SB_G2P3_FE_CLS_MAX_NUM_SCHEMA];
  ...
  void                      *clientData;
  uint32                  bDebug;
  uint32                  isC2; /* Caladan 2 runtime differentiation */
  ...
  uint32                  uSwapRs[G2P3_MAX_RC_INSTANCE];
  uint8                   isSwapRsUsed[G2P3_MAX_RC_INSTANCE];
} soc_sbx_g2p3_cls_t;
...
int
soc_sbx_g2p3_cls_rule_usable_capacity_get(uint32 unit,
                               soc_sbx_g2p3_cls_schema_enum_t sType,
                               uint32 *pRuleCount);
...
soc_sbx_g2p3_cls_template_fields_enable(
                               ...
                               soc_sbx_g2p3_ace_qset_t qset, uint8 bEnableAll);
