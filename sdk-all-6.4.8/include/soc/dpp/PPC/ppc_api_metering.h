...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  If True, then all the copies of a Multicast packet
   *  updates the meter processor (destination-oriented
   *  metering). Otherwise, only the first copy updates it
   *  (source-oriented metering). T20E has to be TRUE.
   */
  uint8 update_all_copies;

} SOC_PPC_MTR_GLBL_INFO;
...
typedef struct
{
  ...
  /*
   * Enable IPG compensation. 
   * If TRUE then a constant value of 20 will be added.
   * to packet size for meter calculations.
   * If FALSE then nothing is added.
   * This is relevant both for the ethernet policer and
   * for the MRPS.
   * This flag is global and so any group will change the
   * global flag and NOT a per-group flag.
   * Arad+ only
  */
  uint8 is_ipg_compensation_enabled;
...
typedef struct
{
  ...
  */
  ...
  uint32 ebs;
  /*
   *  Max Rate of 4 hierarchical meter, in Kbps
   *  Used only when global sharing is enabled
   *  to calculate shared rev-exp for the 4 meters
   *  ARAD only.
   */
  uint32 hierarchical_max_rate;
  /*
   *  Min Rate of 4 hierarchical meter, in Kbps
   *  Used only when global sharing is enabled
   *  to calculate shared rev-exp for the 4 meters
   *  ARAD only.
   */
  uint32 hierarchical_min_rate;
  /*
...
typedef struct
{
	...
	/* if policer0_color is yellow,
	   ...
	   */
    uint8 policer0_other_bucket_has_credits; 
	...
	/* if policer1_color is yellow,
	   indicate if green bucket has credits,
	   otherwise if yellow bucket has credits */
    uint8 policer1_other_bucket_has_credits;
...
void
  SOC_PPC_MTR_GLBL_INFO_clear(
    SOC_SAND_OUT SOC_PPC_MTR_GLBL_INFO *info
  );
...
void
  SOC_PPC_MTR_GLBL_INFO_print(
    SOC_SAND_IN  SOC_PPC_MTR_GLBL_INFO *info
  );
