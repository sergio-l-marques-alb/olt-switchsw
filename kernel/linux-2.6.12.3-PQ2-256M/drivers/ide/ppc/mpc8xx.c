...
extern void pq2_ptin_hwif_iops (ide_hwif_t *hwif);
#endif
...
#if defined(CONFIG_PTIN860T) || defined(CONFIG_PTIN857TIAD)
	/* Enable rising edge irq */
	pcmp->pcmc_per = 0x200000 >> (16 * _slot_);
#else
	...
	unsigned long data_port, unsigned long ctrl_port, int *irq)
{
	...
	*p = 0;
	...
	unicom_v1_hwif_iops (&ide_hwifs[data_port], ide_base);
        ide_hwifs[data_port].mmio= 2;
#endif
#ifdef CONFIG_BROUTER8
        brouter8_hwif_iops(&ide_hwifs[data_port]);
#endif
#ifdef CONFIG_PQ2_PTIN
        pq2_ptin_hwif_iops(&ide_hwifs[data_port]);
#endif
#if defined(CONFIG_UNICOM_V1) || defined(CONFIG_BROUTER8) || defined(CONFIG_PQ2_PTIN)
{
	volatile intctl_cpm2_t *ic = &cpm2_immr->im_intctl;
	/* make IRQ1 edge sensitive */
	ic->ic_siexr |= (1 << (14 - (SIU_INT_IRQ1 - ioport_dsc[data_port].irq)));
...
#if defined(CONFIG_PTIN860T) || defined(CONFIG_PTIN857TIAD)
        /* set level mode irq and memory mapped device in config reg */
	*((unsigned char *)(base + config_base)) = 0x40;
#else
