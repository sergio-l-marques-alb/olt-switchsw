/**************************************************************************************
 **************************************************************************************
 *  File Name     :  merlin_dino_dv_functions.c                                        *
 *  Created On    :  22/05/2014                                                       *
 *  Created By    :  Kiran Divakar                                                    *
 *  Description   :  APIs for Serdes IPs ported over from DV                          *
 *  Revision      :  $Id: merlin_dino_dv_functions.c 1356 2016-02-12 23:27:16Z cvazquez $ *
 *                                                                                    *
...
/** @file merlin_dino_dv_functions.c
...
err_code_t merlin_dino_config_shared_tx_pattern( const phymod_access_t *pa, uint8_t patt_length, const char pattern[]) {
...
err_code_t merlin_dino_tx_shared_patt_gen_en( const phymod_access_t *pa, uint8_t enable, uint8_t patt_length) {
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by _calc_patt_gen_mode_sel( pa, ) */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by _calc_patt_gen_mode_sel( pa, ) */
...
err_code_t merlin_dino_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_rx_lock) {
    if(!pmd_rx_lock) {
...
err_code_t merlin_dino_display_cl72_status( const phymod_access_t *pa ) {
  EFUN_PRINTF(("\n\n************************\n"                                                                                                                         ));
...
err_code_t merlin_dino_tx_disable( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_rx_restart( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_write_rx_afe( const phymod_access_t *pa, enum srds_rx_afe_settings_enum param, int8_t val) {
...
err_code_t merlin_dino_read_rx_afe( const phymod_access_t *pa, enum srds_rx_afe_settings_enum param, int8_t *val) {
  /* Assumes the micro is not actively tuning */
    if(!val) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t merlin_dino_config_tx_prbs( const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv) {
...
err_code_t merlin_dino_get_tx_prbs_config( const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
    uint8_t val;
...
err_code_t merlin_dino_tx_prbs_en( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_get_tx_prbs_en( const phymod_access_t *pa, uint8_t *enable) {
...
err_code_t merlin_dino_tx_prbs_err_inject( const phymod_access_t *pa, uint8_t enable) {
  /* PRBS Error Insert.
...
err_code_t merlin_dino_config_rx_prbs( const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
...
/* get PRBS Checker */
err_code_t merlin_dino_get_rx_prbs_config( const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
  uint8_t val;
...
err_code_t merlin_dino_rx_prbs_en( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_get_rx_prbs_en( const phymod_access_t *pa, uint8_t *enable) {
...
err_code_t merlin_dino_prbs_chk_lock_state( const phymod_access_t *pa, uint8_t *chk_lock) {
    if(!chk_lock) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t merlin_dino_prbs_err_count_ll( const phymod_access_t *pa, uint32_t *prbs_err_cnt) {
    uint16_t rddata;

    if(!prbs_err_cnt) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    EFUN(merlin_dino_pmd_rdt_reg(pa,TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t merlin_dino_prbs_err_count_state( const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    if(!prbs_err_cnt || !lock_lost) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    EFUN(merlin_dino_prbs_err_count_ll( pa, prbs_err_cnt));
...
err_code_t merlin_dino_dig_lpbk( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_dig_lpbk_rptr( const phymod_access_t *pa, uint8_t enable, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        /* DATA_IN_SIDE  - The side where data is fed into the RX serial input */
        if (enable) {
            EFUN(merlin_dino_loop_timing( pa, 0));
            EFUN(merlin_dino_loop_timing( pa, 1));     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
        } else {
            EFUN(merlin_dino_loop_timing( pa, 0));
        }
        break;
    ...
    case DIG_LPBK_SIDE:
        ...
        EFUN(merlin_dino_tx_rptr_mode_timing( pa, 1));          /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
        EFUN(wr_dig_lpbk_en(enable));                  /* 0 = diabled, 1 = enabled */
...
err_code_t merlin_dino_ull_from_dig_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
   switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_dino_dig_lpbk_rptr( pa, 0,mode));
        EFUN(merlin_dino_tx_ull_config( pa, 1));
        break;
    case DIG_LPBK_SIDE:
        EFUN(merlin_dino_dig_lpbk_rptr( pa, 0,mode));
        EFUN(merlin_dino_rx_ull_config( pa, 1));
        break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
...
err_code_t merlin_dino_nl_from_dig_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
    EFUN(merlin_dino_dig_lpbk_rptr( pa, 0,mode));
    return (ERR_CODE_NONE);
...
err_code_t merlin_dino_loop_timing( const phymod_access_t *pa, uint8_t enable) {
    ...
    if (enable) {
        ...
        EFUN(merlin_dino_delay_us(25));               /* Wait for tclk to lock to CDR */
    } else {
...
err_code_t merlin_dino_rmt_lpbk( const phymod_access_t *pa, uint8_t enable) {
    if (enable) {
        EFUN(merlin_dino_loop_timing( pa, enable));
        ...
        EFUN(merlin_dino_delay_us(50));        /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t merlin_dino_rmt_lpbk_from_ull( const phymod_access_t *pa ) {
    EFUN(merlin_dino_rmt_lpbk( pa, 0)); /* Reset TX_PI */
    EFUN(merlin_dino_rmt_lpbk( pa, 1)); /* Enable RMT LPBK */
    return (ERR_CODE_NONE);
}


/* Switch to remote loopback from NL mode in repeater */
err_code_t merlin_dino_rmt_lpbk_from_nl( const phymod_access_t *pa ) {
    EFUN(merlin_dino_rmt_lpbk( pa, 0)); /* Reset TX_PI */
    EFUN(merlin_dino_rmt_lpbk( pa, 1)); /* Enable RMT LPBK */
    return (ERR_CODE_NONE);
}
...
err_code_t merlin_dino_ull_from_rmt_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_dino_rx_rptr_mode_timing(1));  /* Enable jitter filter enable */
        EFUN(_ull_rx_mode_setup( pa, 1));
        break;
    case RMT_LPBK_SIDE:
        EFUN(merlin_dino_rmt_lpbk( pa, 0));         /* Reset TX_PI */
        EFUN(merlin_dino_tx_rptr_mode_timing( pa, 1));  /* Enable jitter filter enable */
...
err_code_t merlin_dino_nl_from_rmt_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_dino_rx_rptr_mode_timing(1));  /* Enable RX PI transfer */
        break;
    case RMT_LPBK_SIDE:
        EFUN(merlin_dino_rmt_lpbk( pa, 0));                 /* Reset TX_PI */
        EFUN(merlin_dino_tx_rptr_mode_timing( pa, 1));      /* Enable jitter filter enable */
...
err_code_t merlin_dino_tx_pi_freq_override( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val) {
    if (enable) {
...
err_code_t merlin_dino_tx_pi_jitt_gen( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(merlin_dino_tx_pi_freq_override( pa, enable, freq_override_val));
...
err_code_t merlin_dino_core_config_for_iddq( const phymod_access_t *pa ) {
...
err_code_t merlin_dino_lane_config_for_iddq( const phymod_access_t *pa ) {
...
err_code_t merlin_dino_uc_active_enable( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t merlin_dino_uc_reset( const phymod_access_t *pa, uint8_t enable) {
  if (enable) {
    ...
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD200, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD201, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD202, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD204, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD205, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD206, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD207, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD208, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD209, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD20A, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD20B, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD20C, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD20D, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD20E, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD211, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD212, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD213, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD214, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD215, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD216, 0x0007));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD217, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD218, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD219, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD21A, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD21B, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD220, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD221, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD224, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD226, 0x0000));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD225, 0x8803));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD228, 0x0101));
    EFUN(merlin_dino_pmd_wr_reg(pa,0xD229, 0x0000));
    ...
    EFUN(merlin_dino_poll_micro_ra_initdone( pa, 250));             /* Poll   status of  data RAM initialization */
...
err_code_t merlin_dino_map_lanes(const phymod_access_t *pa, uint8_t num_lanes, uint8_t const *tx_lane_map, uint8_t const *rx_lane_map) {
    uint8_t rd_val = 0;
...
err_code_t merlin_dino_core_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(_merlin_dino_core_clkgate( pa, 0));
        EFUN(wrc_afe_s_pll_pwrdn(0x0));
        EFUN(wrc_core_dp_s_rstb(0x1));
        break;
    case PWRDN:
        EFUN(wrc_core_dp_s_rstb(0x0));
        EFUN(merlin_dino_delay_ns(   500)); /* wait >50 comclk cycles  */
    ...
    case PWRDN_DEEP:
        ...
        EFUN(merlin_dino_delay_ns(   500)); /* wait >50 comclk cycles  */
                EFUN(wrc_afe_s_pll_pwrdn(0x1));
        EFUN(_merlin_dino_core_clkgate( pa, 1));
        break;
    default:
...
err_code_t merlin_dino_lane_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
  ...
  */
    ...
    case PWR_ON:
        ...
        EFUN(_merlin_dino_lane_clkgate( pa, 0));
        break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
        ...
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        EFUN(wr_ln_tx_s_pwrdn(0x1));
        EFUN(_merlin_dino_lane_clkgate( pa, 1));
...
err_code_t merlin_dino_isolate_ctrl_pins( const phymod_access_t *pa, uint8_t enable) {
    ...
    EFUN(merlin_dino_isolate_lane_ctrl_pins( pa, enable));
    EFUN(merlin_dino_isolate_core_ctrl_pins( pa, enable));
...
err_code_t merlin_dino_ucode_mdio_load( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
...
err_code_t merlin_dino_ucode_load_verify( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
...
/* Micro RAM Word Read */
uint16_t merlin_dino_rdw_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
   uint16_t rddata;
   if(!err_code_p) {
        return(0);
   }
...
/* Micro RAM Byte Read */
uint8_t merlin_dino_rdb_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    if(!err_code_p) {
        return(0);
    }
...
err_code_t merlin_dino_wrws_uc_ram( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
    return (merlin_dino_wrw_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t merlin_dino_wrbs_uc_ram( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
    return (merlin_dino_wrb_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Word Signed Read */
int16_t merlin_dino_rdws_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int16_t)merlin_dino_rdw_uc_ram( pa, err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t merlin_dino_rdbs_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int8_t)merlin_dino_rdb_uc_ram( pa, err_code_p, addr));
}
...
err_code_t merlin_dino_rdblk_uc_ram( const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {
    ...
    while (cnt--) {
        ...
        uint8_t    tmp = merlin_dino_rdb_uc_ram( pa, &error_code, addr++); /* Value read from the required Data RAM Address */
        if (error_code) {
