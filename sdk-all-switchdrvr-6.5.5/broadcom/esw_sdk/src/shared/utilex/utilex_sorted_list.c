 ...
 *   \param [in] _err1 - \n
 *     Error to report in case the 'get' operation fails.
 * \par INDIRECT OUTPUT
 *   * &head_place - \n
 ...
 *
 */
#define UTILEX_SORTED_LIST_HEAD_PLACE_GET(sorted_list_index, head_place, _err1) \
  res = UTILEX_SORTED_LIST_ACCESS_INFO.list_size.get(unit,sorted_list_index,&head_place) ; \
 ...
 *   * If operation fails then software goes to 'exit' with error index 'err1'.
 */
#define UTILEX_SORTED_LIST_TAIL_PLACE_GET(sorted_list_index, tail_place, _err1) \
  res = UTILEX_SORTED_LIST_ACCESS_INFO.list_size.get(unit,sorted_list_index,&tail_place) ; \
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, _err1, exit) ; \
  tail_place += 1 ;
...
#define UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(_sorted_list_index) \
  { \
    ...
    { \
      res = UTILEX_SORTED_LIST_ACCESS.occupied_lists.bit_get(unit, (int)_sorted_list_index, &bit_val) ; \
      SHR_IF_ERR_EXIT(res) ; \
    } \
    if (bit_val == 0) \
    { \
       ...
       * with error. \
       */ \
      SHR_IF_ERR_EXIT(SOC_SAND_FREE_FAIL) ; \
    } \
  }
/*!
 * \brief
 ...
 *   'exit' is assumed to be defined in the caller's scope.
 */
#define UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, _err1) \
...
*   May 18 2015
* FUNCTION:
*   Get value of 'tmp_data' pointer (See UTILEX_SORTED_LIST_T)
*   from handle.
* INPUT:
*   int                             unit -
...
utilex_sorted_list_get_tmp_data_ptr_from_handle(
  ...
  uint8 ** tmp_data_ptr_ptr)
{
  uint32 sorted_list_index, res;

  SHR_FUNC_INIT_VARS(unit);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, SHR_E_UNIT);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);

  *tmp_data_ptr_ptr =
    sw_state[unit]->dpp.soc.sand.sorted_list->
    lists_array[sorted_list_index]->list_data.tmp_data;
exit:
  SHR_FUNC_EXIT;
}
...
*   May 18 2015
* FUNCTION:
*   Get value of 'tmp_key' pointer (See UTILEX_SORTED_LIST_T)
*   from handle.
* INPUT:
*   SOC_SAND_IN  int                             unit -
*     Identifier of the device to access.
*   SOC_SAND_IN  UTILEX_SORTED_LIST_PTR        sorted_list -
*     Handle to the sorted list.
  ...
  SOC_SAND_OUT uint8 ** tmp_key_ptr_ptr)
{
  ...
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  *tmp_key_ptr_ptr =
    sw_state[unit]->dpp.soc.sand.sorted_list->
    lists_array[sorted_list_index]->list_data.tmp_key;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_get_tmp_key_ptr_from_handle()", 0, 0);
}
...
*   SOC_SAND_IN  UTILEX_SORTED_LIST_PTR        sorted_list -
*     Handle to the sorted list.
*   SOC_SAND_IN  UTILEX_SORTED_LIST_KEY*       const key -
...
*     in UTILEX_SORTED_LIST_T is allocated).
* REMARKS:
*     None.
* RETURNS:
*     OK or ERROR indication.
...
STATIC uint32 utilex_sorted_list_find_match_entry(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  SOC_SAND_IN UTILEX_SORTED_LIST_KEY * const key,
  SOC_SAND_IN UTILEX_SORTED_LIST_DATA * const data,
  SOC_SAND_IN uint8 first_match,
  SOC_SAND_OUT uint8 * found,
  SOC_SAND_OUT UTILEX_SORTED_LIST_ITER * prev_node,
  SOC_SAND_OUT UTILEX_SORTED_LIST_ITER * cur_node,
  SOC_SAND_OUT uint32 * index_in_indices);

STATIC uint32 UTILEX_SORTED_LIST_node_alloc(
...
*   SOC_SAND_IN  UTILEX_SORTED_LIST_PTR        sorted_list -
*     Handle to the sorted list.
*   SOC_SAND_INOUT  UTILEX_SORTED_LIST_ITER    *iter
*     iterator points to the entry to start traverse from.
*   SOC_SAND_OUT  UTILEX_SORTED_LIST_KEY       *const key -
*     the sorted list key returned
*   SOC_SAND_OUT  UTILEX_SORTED_LIST_DATA      data -
*     the sorted list data returned and associated with the key above.
*   SOC_SAND_INOUT UTILEX_SORTED_LIST_ITER     *next_or_prev -
...
*       use UTILEX_SORTED_LIST_ITER_BEGIN(iter)
*     - to check if the iterator get to the end of the table use.
*       use UTILEX_SORTED_LIST_ITER_END(unit,iter)
* RETURNS:
*     OK or ERROR indication.
*********************************************************************/
STATIC uint32 utilex_sorted_list_get_next_aux(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  SOC_SAND_IN UTILEX_SORTED_LIST_ITER curr,
  SOC_SAND_IN uint8 forward,
  SOC_SAND_OUT UTILEX_SORTED_LIST_KEY * const key,
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data,
  SOC_SAND_INOUT UTILEX_SORTED_LIST_ITER * next_or_prev);

STATIC uint32 UTILEX_SORTED_LIST_node_link_set(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  ...
  uint32 size)
{
  ...
  ARAD_TCAM_PRIO_LOCATION prio_location_1, prio_location_2;
  ...
  {     /* Data comparison */
    ...
    return ((prio_location_1.entry_id_first != prio_location_2.entry_id_first)
            || (prio_location_1.entry_id_last !=
                prio_location_2.entry_id_last)) ? 1 : 0;
  }
  else
  {
    /*
  ...
  SOC_SAND_IN uint32 max_nof_lists)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_INIT);

  res = UTILEX_SORTED_LIST_ACCESS.alloc(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);

  res = UTILEX_SORTED_LIST_ACCESS.lists_array.ptr_alloc(unit, max_nof_lists);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

  res = UTILEX_SORTED_LIST_ACCESS.max_nof_lists.set(unit, max_nof_lists);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);

  res = UTILEX_SORTED_LIST_ACCESS.in_use.set(unit, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);

  res =
    UTILEX_SORTED_LIST_ACCESS.occupied_lists.alloc_bitmap(unit,
                                                          max_nof_lists);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 60, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in UTILEX_SORTED_LIST_init()", 0, 0);
}
  ...
  SOC_SAND_IN int unit)
{
  ...
  uint32
    found,
    res, max_nof_lists, tmp_size, key_size, data_size, ptr_size, in_use;
  uint8 bit_val;
  uint8 is_allocated;
  int32 offset;
  ...
  res = UTILEX_SORTED_LIST_ACCESS.in_use.get(unit, &in_use);
  ...
  found = 0;
  offset = 0;
  for (sorted_list_index = 0; sorted_list_index < max_nof_lists;
       sorted_list_index++)
  {
    res =
      UTILEX_SORTED_LIST_ACCESS.occupied_lists.bit_get(unit,
                                                       sorted_list_index,
                                                       &bit_val);
    ...
    if (bit_val == 1)
    {
      ...
      found++;
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.is_allocated(unit,
                                                            sorted_list_index,
                                                            &is_allocated);
      ...
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.is_allocated(unit,
                                                             sorted_list_index,
                                                             &is_allocated);
       ...
       */
      res =
        UTILEX_SORTED_LIST_ACCESS_INFO.key_size.get(unit, sorted_list_index,
                                                    &key_size);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.memset(unit, sorted_list_index,
                                                      offset, key_size, 0);
       ...
       */
      res =
        UTILEX_SORTED_LIST_ACCESS_INFO.data_size.get(unit, sorted_list_index,
                                                     &data_size);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.ptr_size.get(unit, sorted_list_index,
                                                    &ptr_size);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
      tmp_size = SOC_SAND_MAX(ptr_size, data_size);
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.memset(unit,
                                                       sorted_list_index,
                                                       offset, tmp_size, 0);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_clear_all_tmps()", 0, 0);
}

uint32
utilex_sorted_list_create(
  SOC_SAND_IN int unit,
  SOC_SAND_INOUT UTILEX_SORTED_LIST_PTR * sorted_list_ptr,
  SOC_SAND_IN UTILEX_SORTED_LIST_INIT_INFO init_info)
{
  ...
  uint32
    found,
    res, data_size, list_size, key_size, null_ptr, ptr_size, max_nof_lists;
  uint32 in_use;
  uint8 bit_val;
  UTILEX_SORTED_LIST_PTR sorted_list;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_CREATE);
   ...
   */
  res = UTILEX_SORTED_LIST_ACCESS.in_use.set(unit, (in_use + 1));
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  /*
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS.occupied_lists.bit_set(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 16, exit);

  res = UTILEX_SORTED_LIST_ACCESS.lists_array.alloc(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 18, exit);
  /*
   * Note that legal handles start at '1', not at '0'.
   */
  UTILEX_SORTED_LIST_CONVERT_SORTEDLIST_INDEX_TO_HANDLE(sorted_list,
                                                        sorted_list_index);
  /*
  ...
  *sorted_list_ptr = sorted_list;
  ...
  soc_sand_utilex_sorted_list_info_clear(unit, sorted_list);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.prime_handle.set(unit, sorted_list_index,
                                                    init_info.prime_handle);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 22, exit);

  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.sec_handle.set(unit, sorted_list_index,
                                                  init_info.sec_handle);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.list_size.set(unit, sorted_list_index,
                                                 init_info.list_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.key_size.set(unit, sorted_list_index,
                                                init_info.key_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.data_size.set(unit, sorted_list_index,
                                                 init_info.data_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.get_entry_fun.set(unit, sorted_list_index,
                                                     init_info.get_entry_fun);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 32, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.set_entry_fun.set(unit, sorted_list_index,
                                                     init_info.set_entry_fun);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 34, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.cmp_func_type.set(unit, sorted_list_index,
                                                     init_info.cmp_func_type);
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.list_size.get(unit, sorted_list_index,
                                                 &list_size);
   ...
   */
  ptr_size =
    (soc_sand_log2_round_up(list_size + 2) +
     (SOC_SAND_NOF_BITS_IN_BYTE - 1)) / SOC_SAND_NOF_BITS_IN_BYTE;
  null_ptr = list_size + 1;
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.ptr_size.set(unit, sorted_list_index,
                                                ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 44, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.null_ptr.set(unit, sorted_list_index,
                                                null_ptr);
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.alloc(unit, sorted_list_index,
                                                  tmp_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 48, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.alloc(unit, sorted_list_index,
                                                 key_size);
  ...
  offset = 0;
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.keys.alloc(unit, sorted_list_index,
                                              list_size * key_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 52, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.keys.memset(unit, sorted_list_index,
                                               offset, list_size * key_size,
                                               0);
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.next.alloc(unit, sorted_list_index,
                                              (list_size + 2) * ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 56, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.next.memset(unit, sorted_list_index,
                                               offset,
                                               (list_size + 2) * ptr_size,
                                               0xFF);
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.prev.alloc(unit, sorted_list_index,
                                              (list_size + 2) * ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 56, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.prev.memset(unit, sorted_list_index,
                                               offset,
                                               (list_size + 2) * ptr_size,
                                               0xFF);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 58, exit);
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.data.alloc(unit, sorted_list_index,
                                              list_size * data_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 60, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.data.memset(unit, sorted_list_index,
                                               offset, list_size * data_size,
                                               0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 62, exit);
  {
     ...
     */
    UTILEX_SORTED_LIST_HEAD_PLACE_GET(sorted_list_index, head_place, 64);
    UTILEX_SORTED_LIST_TAIL_PLACE_GET(sorted_list_index, tail_place, 66);
    res =
      utilex_sorted_list_node_link_set(unit,
                                       sorted_list, head_place, tail_place);
  ...
  res = soc_sand_occ_bm_create(unit, &btmp_init_info, &memory_use);
  SOC_SAND_CHECK_FUNC_RESULT(res, 80, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.set(unit, sorted_list_index,
                                                  memory_use);
...
/* { */
  {
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.indices.alloc(unit, sorted_list_index,
                                                   list_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 84, exit);
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.num_elements_on_indices.set(unit,
                                                                 sorted_list_index,
                                                                 0);
...
#endif
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_create()", 0, 0);
}
  ...
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list)
{
  ...
  }
  {
     ...
     */
    uint32 head_place, tail_place;
    UTILEX_SORTED_LIST_HEAD_PLACE_GET(sorted_list_index, head_place, 54);
    UTILEX_SORTED_LIST_TAIL_PLACE_GET(sorted_list_index, tail_place, 56);
    res = utilex_sorted_list_node_link_set(unit,
                                           sorted_list,
                                           head_place, tail_place);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  }

  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.get(unit, sorted_list_index,
                                                  &memory_use);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 32, exit);
  res = soc_sand_occ_bm_clear(unit, memory_use);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_clear()", 0, 0);
}
...
utilex_sorted_list_destroy(
  ...
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list)
{
  uint32 res, sorted_list_index;
  SOC_SAND_OCC_BM_PTR memory_use;
  uint8 bit_val;
  uint32 in_use;
  uint8 is_allocated;

  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_DESTROY);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS.occupied_lists.bit_get(unit, sorted_list_index,
                                                     &bit_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);
  if (bit_val == 0)
  {
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS.occupied_lists.bit_clear(unit,
                                                       sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 14, exit);
  /*
   * Free the temps buffer.
   */
  res = UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 18, exit);
  res = UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

  res = UTILEX_SORTED_LIST_ACCESS_DATA.next.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  res = UTILEX_SORTED_LIST_ACCESS_DATA.keys.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);

  res = UTILEX_SORTED_LIST_ACCESS_DATA.prev.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 32, exit);
  res = UTILEX_SORTED_LIST_ACCESS_DATA.data.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 36, exit);

  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.indices.is_allocated(unit,
                                                        sorted_list_index,
                                                        &is_allocated);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);
  if (is_allocated)
  {
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.indices.free(unit, sorted_list_index);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.num_elements_on_indices.set(unit,
                                                                 sorted_list_index,
  ...
  res = UTILEX_SORTED_LIST_ACCESS.lists_array.free(unit, sorted_list_index);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 48, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_destroy()", 0, 0);
}
  ...
  SOC_SAND_OUT uint8 * success)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_ADD);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  res = utilex_sorted_list_find_match_entry(unit,
                                            sorted_list,
                                            key,
                                            data,
                                            FALSE,
  ...
  if (found)
  {
    SOC_SAND_SET_ERROR_CODE(UTILEX_SORTED_LIST_KEY_DATA_ALREADY_EXIST_ERR,
                            20, exit);
  }
  /*
   * Allocate new node.
   */
  res = utilex_sorted_list_node_alloc(unit,
                                      sorted_list,
                                      key,
                                      data,
                                      prev_node,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_entry_add()", 0,
                               0);
}
...
utilex_sorted_list_entry_add_by_iter(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  SOC_SAND_IN UTILEX_SORTED_LIST_ITER pos,
  ...
  SOC_SAND_OUT uint8 * success)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_ADD_BY_ITER);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.cmp_func_type.get(unit, sorted_list_index,
                                                     &cmp_func_type);
  ...
  {
    cmp_key_fun = (UTILEX_SORTED_LIST_KEY_CMP) soc_sand_os_memcmp;
  }
  /*
  ...
  {
    SOC_SAND_SET_ERROR_CODE(UTILEX_SORTED_LIST_ILLEGAL_ITER_ERR, 10, exit);
  }
  /*
  ...
  {
    res =
      utilex_sorted_list_get_next_aux(unit,
                                      sorted_list,
                                      pos, FALSE, NULL, NULL, &prev_node);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  }
  else
  {
    res =
      utilex_sorted_list_get_next_aux(unit,
                                      sorted_list,
                                      pos, TRUE, NULL, NULL, &next_node);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);
  }
  res =
    utilex_sorted_list_get_tmp_key_ptr_from_handle(unit, sorted_list,
                                                   &tmp_key_ptr);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 14, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.key_size.get(unit, sorted_list_index,
  ...
  {
    res = utilex_sorted_list_entry_value(unit,
                                         sorted_list,
                                         prev_node, tmp_key_ptr, NULL);
    SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);

    if (cmp_key_fun(tmp_key_ptr, key, key_size) > 0)
    {
      SOC_SAND_SET_ERROR_CODE(UTILEX_SORTED_LIST_ILLEGAL_ITER_ERR, 18, exit);
    }
  }
      ...
      && next_node != UTILEX_SORTED_LIST_ITER_BEGIN(unit, sorted_list))
  {
    res = utilex_sorted_list_entry_value(unit,
                                         sorted_list,
                                         next_node, tmp_key_ptr, NULL);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    if (cmp_key_fun(key, tmp_key_ptr, key_size) > 0)
    {
      SOC_SAND_SET_ERROR_CODE(UTILEX_SORTED_LIST_ILLEGAL_ITER_ERR, 22, exit);
    }
  }
  {
    ...
    if (is_allocated)
    {
       ...
       */
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.num_elements_on_indices.get(unit,
                                                                   sorted_list_index,
          ...
          || (next_node == UTILEX_SORTED_LIST_ITER_BEGIN(unit, sorted_list)))
      {
        from_top_index_in_indices = 0;
               ...
               || prev_node == UTILEX_SORTED_LIST_ITER_BEGIN(unit,
                                                             sorted_list))
      {
      ...
      else
      {
         ...
         */
        res =
          utilex_sorted_list_get_index_from_iter(unit, sorted_list,
                                                 prev_node,
                                                 &from_top_index_in_indices);
        SOC_SAND_CHECK_FUNC_RESULT(res, 32, exit);
        from_top_index_in_indices =
          num_elements_on_indices - 1 - from_top_index_in_indices;
      }
    }
  }
  res = utilex_sorted_list_node_alloc(unit,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_entry_add_by_iter()", 0, 0);
}
...
utilex_sorted_list_entry_update(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  SOC_SAND_IN UTILEX_SORTED_LIST_ITER iter,
  SOC_SAND_IN UTILEX_SORTED_LIST_KEY * const key,
  SOC_SAND_IN UTILEX_SORTED_LIST_DATA * const data)
{
  uint32 res;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_UPDATE);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  if (data)
  {
     ...
     */
    {
       ...
       */
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.data.memwrite(unit, sorted_list_index,
                                                     data, iter * data_size,
                                                     data_size);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
    }
  ...
  if (key)
  {
     ...
     */
    {
       ...
       */
      res =
        UTILEX_SORTED_LIST_ACCESS_DATA.keys.memwrite(unit, sorted_list_index,
                                                     key, iter * key_size,
                                                     key_size);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
    }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_entry_update()",
                               0, 0);
}
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data)
{
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.data.memread(unit, sorted_list_index, data,
                                                iter * data_size, data_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_entry_get_data()", 0, 0);
}
  ...
  SOC_SAND_IN UTILEX_SORTED_LIST_ITER iter)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_REMOVE_BY_ITER);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  if (is_indices_enabled)
  {
     ...
     */
    res =
      utilex_sorted_list_get_index_from_iter(unit, sorted_list, iter,
                                             &index_in_indices);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }
  res = utilex_sorted_list_get_next_aux(unit,
                                        sorted_list,
                                        iter, TRUE, NULL, NULL, &next);
  SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);
  res = utilex_sorted_list_get_next_aux(unit,
                                        sorted_list,
                                        iter, FALSE, NULL, NULL, &prev);
  SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  res = utilex_sorted_list_node_link_set(unit, sorted_list, prev, next);
  SOC_SAND_CHECK_FUNC_RESULT(res, 16, exit);

  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.null_ptr.get(unit, sorted_list_index,
                                                &null_ptr);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.ptr_size.get(unit, sorted_list_index,
   ...
   */
  {
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.next.memwrite(unit, sorted_list_index,
                                                   (uint8 *) & null_ptr,
                                                   iter * ptr_size, ptr_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  }
  {
    /*
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.prev.memwrite(unit, sorted_list_index,
                                                   (uint8 *) & null_ptr,
                                                   iter * ptr_size, ptr_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
  }
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.get(unit, sorted_list_index,
                                                  &memory_use);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
  res = soc_sand_occ_bm_occup_status_set(unit, memory_use, iter, FALSE);
  ...
  if (is_indices_enabled)
  {
    ...
    if (index_in_indices < (num_elements_on_indices - 1))
    {
      ...
      {
        res =
          UTILEX_SORTED_LIST_ACCESS_DATA.indices.get(unit, sorted_list_index,
                                                     ii, &node_ptr);
        SOC_SAND_CHECK_FUNC_RESULT(res, 38, exit);
        res =
          UTILEX_SORTED_LIST_ACCESS_DATA.indices.set(unit, sorted_list_index,
                                                     ii - 1, node_ptr);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_entry_remove_by_iter()", 0, 0);
}
...
utilex_sorted_list_entry_lookup(
  ...
  SOC_SAND_IN UTILEX_SORTED_LIST_DATA * const data,
  SOC_SAND_OUT uint8 * found,
  SOC_SAND_OUT UTILEX_SORTED_LIST_ITER * iter)
{
  uint32 curr_node, prev_node;
  uint32 res;
  uint32 index_in_indices;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_LOOKUP);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_entry_lookup()",
                               0, 0);
}
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_ENTRY_VALUE);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  if (key != NULL)
  {
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.keys.memread(unit, sorted_list_index,
                                                  key, iter * key_size,
                                                  key_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
  }
  if (data != NULL)
  {
     ...
     */
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.data.memread(unit, sorted_list_index,
                                                  data, iter * data_size,
                                                  data_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);
  }
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_entry_value()",
                               0, 0);
}
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_GET_NEXT);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("utilex_sorted_list_get_next()", 0, 0);
}
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_GET_PREV);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  UTILEX_SORTED_LIST_HEAD_PLACE_GET(sorted_list_index, head_place, 12);
  if (*iter == head_place
      /*
       * UTILEX_SORTED_LIST_HEAD_PLACE(&(sorted_list->init_info)) 
       */ )
  {
    goto exit;
  }
  res = utilex_sorted_list_get_next_aux(unit,
                                        sorted_list,
                                        *iter, FALSE, key, data, iter);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("utilex_sorted_list_get_prev()", 0, 0);
}
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_DATA * const data)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_GET_FOLLOW);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  res = utilex_sorted_list_get_next_aux(unit,
                                        sorted_list,
                                        *iter, forward, key, data, iter);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("utilex_sorted_list_get_follow()", 0, 0);
}
...
*   SOC_SAND_IN  int                         unit -
*     Identifier of the device to access.
*   SOC_SAND_IN  UTILEX_SORTED_LIST_INFO   *sorted_list -
  ...
  SOC_SAND_OUT uint32 * size)
{
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.get(unit, sorted_list_index,
                                                  &memory_use);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
  res = soc_sand_occ_bm_get_size_for_save(unit, memory_use, &bmp_size);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  total_size += bmp_size;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in soc_sand_hash_table_get_size_for_save()", 0, 0);
}
  ...
  SOC_SAND_OUT uint32 * actual_size_bytes)
{
   ...
   */
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.get(unit, sorted_list_index,
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.memwrite(unit, sorted_list_index,
                                                     cur_ptr, 0, tmp_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 22, exit);
  cur_ptr += tmp_size;

  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.memwrite(unit, sorted_list_index,
                                                    cur_ptr, 0, key_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  cur_ptr += key_size;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.get(unit, sorted_list_index,
                                                  &memory_use);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
  res = soc_sand_occ_bm_save(unit,
                             memory_use,
                             cur_ptr,
                             buffer_size_bytes - total_size, &cur_size);
  SOC_SAND_CHECK_FUNC_RESULT(res, 36, exit);
  cur_ptr += cur_size;
  total_size += cur_size;
...
*   SOC_SAND_IN  UTILEX_SORTED_LIST_SW_DB_ENTRY_GET  get_entry_fun,
*   SOC_SAND_IN  UTILEX_SORTED_LIST_SW_DB_ENTRY_SET  set_entry_fun,
*   SOC_SAND_IN  UTILEX_SORTED_LIST_KEY_CMP          cmp_key_fun,
*   SOC_SAND_OUT UTILEX_SORTED_LIST_PTR              *sorted_list_ptr -
  ...
  SOC_SAND_OUT UTILEX_SORTED_LIST_PTR * sorted_list_ptr)
{
   ...
   */
  soc_sand_os_memcpy(&local_init_info, cur_ptr,
                     sizeof(UTILEX_SORTED_LIST_INIT_INFO));
  cur_ptr += sizeof(UTILEX_SORTED_LIST_INIT_INFO);
   ...
   */
  res = utilex_sorted_list_create(unit, sorted_list_ptr, local_init_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_data.memread(unit, sorted_list_index,
                                                    (uint8 *) cur_ptr, 0,
                                                    tmp_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 22, exit);
  cur_ptr += tmp_size;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.tmp_key.memread(unit, sorted_list_index,
                                                   (uint8 *) cur_ptr, 0,
                                                   key_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  cur_ptr += key_size;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.next.memread(unit, sorted_list_index,
                                                (uint8 *) cur_ptr, 0,
                                                (list_size + 2) * ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);
  cur_ptr += ((list_size + 2) * ptr_size);
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.prev.memread(unit, sorted_list_index,
                                                (uint8 *) cur_ptr, 0,
                                                (list_size + 2) * ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
  cur_ptr += ((list_size + 2) * ptr_size);
   ...
   * Load bitmap 
   */
  res = soc_sand_occ_bm_load(unit, &cur_ptr, &memory_use);
  SOC_SAND_CHECK_FUNC_RESULT(res, 38, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.set(unit, sorted_list_index,
                                                  memory_use);
...
utilex_sorted_list_print(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list,
  SOC_SAND_IN char table_header[UTILEX_SORTED_LIST_HEADER_SIZE],
  SOC_SAND_IN UTILEX_SORTED_LIST_PRINT_VAL print_key,
  SOC_SAND_IN UTILEX_SORTED_LIST_PRINT_VAL print_data)
{
  uint32 prev, curr;
  uint32 sorted_list_index;
  uint32 res;
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_PRINT);
  UTILEX_SORTED_LIST_VERIFY_UNIT_IS_LEGAL(unit, 2);
  UTILEX_SORTED_LIST_CONVERT_HANDLE_TO_SORTEDLIST_INDEX(sorted_list_index,
                                                        sorted_list);
  UTILEX_SORTED_LIST_VERIFY_SORTEDLIST_IS_ACTIVE(sorted_list_index);
  ...
  UTILEX_SORTED_LIST_HEAD_PLACE_GET(sorted_list_index, prev, 12);
  res =
    utilex_sorted_list_get_tmp_data_ptr_from_handle(unit, sorted_list,
                                                    &tmp_data_ptr);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  res =
    utilex_sorted_list_get_tmp_key_ptr_from_handle(unit, sorted_list,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_print()", 0, 0);
}
...
{
  uint32 res, sorted_list_index, data_size;
  int32 cmp_res;
  UTILEX_SORTED_LIST_KEY_CMP cmp_key_fun;
  uint8 *tmp_data_ptr;
  UTILEX_SORTED_LIST_CMP_FUNC_TYPE cmp_func_type;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_data_cmp()", 0,
                               0);
}
  ...
  SOC_SAND_INOUT UTILEX_SORTED_LIST_ITER * next_or_prev)
{
   ...
   */
  ptr_long = 0;
  soc_sand_U8_to_U32(tmp_data_ptr, ptr_size, &ptr_long);
  ...
  *next_or_prev = ptr_long;
  goto exit;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_get_next_aux()",
                               0, 0);
}

uint32
soc_sand_utilex_sorted_list_info_clear(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list)
{
  uint32 sorted_list_index;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.data_size.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 8, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.get_entry_fun.set(unit, sorted_list_index,
                                                     0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.key_size.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.set_entry_fun.set(unit, sorted_list_index,
                                                     0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 14, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.list_size.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 16, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.prime_handle.set(unit, sorted_list_index,
                                                    0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 18, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.sec_handle.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.cmp_func_type.set(unit, sorted_list_index,
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.ptr_size.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 24, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.null_ptr.set(unit, sorted_list_index, 0);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 28, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in soc_sand_utilex_sorted_list_info_clear()", 0, 0);
}
...
soc_sand_db_prio_list_priority_value_encode(
  ...
  SOC_SAND_OUT uint8 * data)
{
  soc_sand_U32_to_U8(&priority, sizeof(uint32), data);
}
  ...
  UTILEX_SORTED_LIST_PTR sorted_list)
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  sal_printf
    ("%s(): Invocation %d Sorted_list %d - Display 'indices' of %d entries\r\n",
     __FUNCTION__, num_invocations, (int) sorted_list,
     num_elements_on_indices);
  for (ii = 0; ii < num_elements_on_indices; ii++)
  {
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.indices.get(unit, sorted_list_index, ii,
                                                 &node_ptr);
    SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  ...
  UTILEX_SORTED_LIST_PTR sorted_list)
{
  ...
  sal_printf("%s(): Invocation %d Sorted_list %d - \r\n",
             __FUNCTION__, num_invocations, (int) sorted_list);
  utilex_sorted_list_print(unit, sorted_list, "Sorted linked list",
                           soc_sand_print_key, soc_sand_print_data);
exit:
 ...
 */
uint32
soc_sand_exercise_01(
  int unit)
{
  uint32 num_elements, priority_profile, entry_id, res = SOC_SAND_OK;
  uint32 prio, priority[2][8];
  ...
  uint8 entry_added;
  ARAD_TCAM_PRIO_LOCATION prio_location;
  uint8 found_equal;
   ...
   */
  soc_sand_os_memset(&priorities_init_info, 0x0,
                     sizeof(UTILEX_SORTED_LIST_INIT_INFO));
  ...
  priorities_init_info.data_size =
    ARAD_TCAM_DB_LIST_DATA_SIZE * sizeof(uint8);
  ...
  res = utilex_sorted_list_create(unit, &priorities, priorities_init_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  for (entry_id = 0; entry_id < max_nof_entries; entry_id++)
  {
    soc_sand_db_prio_list_priority_value_encode(priority[priority_profile]
                                                [entry_id], key_buffer);
    ...
    res = utilex_sorted_list_entry_add(unit,
                                       priorities,
                                       key_buffer, data_buffer, &entry_added);
    SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  ...
  if (0)
  {
    ...
    res =
      utilex_sorted_list_find_higher_eq_key(unit, priorities, key_buffer,
                                            &iter, &found_equal);
    SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);
    sal_printf
  ...
  {
    /*
     * Now exercise bisect. Use utilex_sorted_list_find_lower_eq_key()
     */
    UTILEX_SORTED_LIST_ITER iter;
    ...
    soc_sand_db_prio_list_priority_value_encode(prio, key_buffer);
    res =
      utilex_sorted_list_find_lower_eq_key(unit, priorities, key_buffer,
                                           &iter, &index_on_indices,
                                           &found_equal);
    SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
    sal_printf
    ...
    res =
      utilex_sorted_list_get_num_elements(unit, priorities, &num_elements);
    SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
    sal_printf("%s(): Original num of element %d. Now, remove element %d\r\n",
               __FUNCTION__, num_elements, iter);
    res = utilex_sorted_list_entry_remove_by_iter(unit, priorities, iter);
...
uint32
soc_sand_utilex_sorted_list_info_print(
  SOC_SAND_IN int unit,
  SOC_SAND_IN UTILEX_SORTED_LIST_PTR sorted_list)
{
  uint32 res, key_size, list_size, data_size, sec_handle, sorted_list_index;
  int prime_handle;
  ...
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 12, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.prime_handle.get(unit, sorted_list_index,
                                                    &prime_handle);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 14, exit);
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.sec_handle.get(unit, sorted_list_index,
                                                  &sec_handle);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 16, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in soc_sand_utilex_sorted_list_info_print()", 0, 0);
}
...
*     UTILEX_SORTED_LIST_ACCESS_INFO.cmp_func_type
* RETURNS:
*   OK or ERROR indication.
*********************************************************************/
uint32
  ...
  SOC_SAND_OUT uint8 * found_equal)
{
  uint32
    sorted_list_index,
    matched,
    index_on_indices, top_index_on_indices, bottom_index_on_indices, res;
  uint8 is_allocated;
  uint32 num_elements;
  UTILEX_SORTED_LIST_ITER local_iter;
  UTILEX_SORTED_LIST_KEY element_key[ARAD_TCAM_DB_LIST_KEY_SIZE];
  UTILEX_SORTED_LIST_KEY_CMP cmp_key_fun;
  UTILEX_SORTED_LIST_CMP_FUNC_TYPE cmp_func_type;
  int32 compare_res;
  ...
  *found_equal = 0;
  ...
  if (!is_allocated)
  {
    res = UTILEX_SORTED_LIST_ILLEGAL_ITER_ERR;
    SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  }
  res =
    UTILEX_SORTED_LIST_ACCESS_INFO.cmp_func_type.get(unit, sorted_list_index,
  ...
  res = utilex_sorted_list_get_num_elements(unit, sorted_list, &num_elements);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 18, exit);
  if (num_elements == 0)
  {
    /*
   ...
   */
  res =
    utilex_sorted_list_get_iter_from_indices(unit, sorted_list,
                                             num_elements - 1, &local_iter);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 22, exit);
  res =
    utilex_sorted_list_entry_value(unit, sorted_list, local_iter,
                                   element_key, NULL);
  SOC_SAND_CHECK_FUNC_RESULT(res, 26, exit);
   ...
   */
  res =
    utilex_sorted_list_get_iter_from_indices(unit, sorted_list, 0,
                                             &local_iter);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
  res =
    utilex_sorted_list_entry_value(unit, sorted_list, local_iter,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_find_higher_eq_key()", 0, 0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_find_lower_eq_key()", 0, 0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_get_num_elements()", 0, 0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_is_indices_enabled()", 0, 0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_get_iter_from_indices()", 0, 0);
}
  ...
  SOC_SAND_OUT uint32 * index_in_indices)
{
  ...
  res =
    utilex_sorted_list_entry_value(unit, sorted_list, iter, element_key,
                                   NULL);
  SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);
  res =
    utilex_sorted_list_find_lower_eq_key(unit, sorted_list, element_key,
                                         &local_iter, index_in_indices,
                                         &found_equal);
  SOC_SAND_CHECK_FUNC_RESULT(res, 22, exit);
  if (!found_equal)
  {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_get_index_from_iter()", 0, 0);
}
...
utilex_sorted_list_node_link_set(
  ...
  SOC_SAND_IN uint32 node2)
{
  uint32 sorted_list_index, ptr_size, res;
  uint8 *tmp_data_ptr;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.ptr_size.get(unit, sorted_list_index,
                                                &ptr_size);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 14, exit);
  res = soc_sand_U32_to_U8(&node2, ptr_size, tmp_data_ptr);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  /*
   * if(SOC_DPP_WB_ENGINE_VAR_NONE == init_info_ptr->wb_var_index) 
   */
  ...
  res = soc_sand_U32_to_U8(&node1, ptr_size, tmp_data_ptr);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  /*
   * if(SOC_DPP_WB_ENGINE_VAR_NONE == init_info_ptr->wb_var_index) 
   */
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_node_link_set()", 0, 0);
}
...
*   SOC_SAND_OUT  UTILEX_SORTED_LIST_ITER       *next_node
*     iterator points to searched node or node after.
*   SOC_SAND_OUT  uint32                          *index_in_indices -
*     Only meaningful if the 'indices' feature is enabled ('indices' element
*     in UTILEX_SORTED_LIST_T is allocated). Index in 'indices'
  ...
  SOC_SAND_OUT uint32 * index_in_indices)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(UTILEX_SORTED_LIST_FIND_MATCH_ENTRY);
  /*
  ...
  *found = FALSE;
  UTILEX_SORTED_LIST_TAIL_PLACE_GET(sorted_list_index, prev, 12);
  ...
  do
  {
     ...
     */
    res = utilex_sorted_list_get_next_aux(unit, sorted_list, prev, FALSE,       /* Scan the list from the end - faster in TCAM for sorted entries */
                                          tmp_key_ptr, NULL, &curr);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
    ...
    compare_res =
      cmp_key_fun(tmp_key_ptr,
                  key, (key_size * sizeof(UTILEX_SORTED_LIST_KEY_TYPE)));
    /*
     * If key was found
     */
    if (compare_res == 0)
    {
      ...
      {
        res =
          utilex_sorted_list_data_cmp(unit,
                                      sorted_list, curr, data, &compare_res);
    ...
    else if (compare_res < 0)
    {
      ...
      *prev_node =
        (curr ==
         UTILEX_SORTED_LIST_ITER_END(unit, sorted_list)) ? null_ptr : curr;
      *cur_node = prev;
      *index_in_indices = curr_index;
      goto exit;
    }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR
    ("error in utilex_sorted_list_find_match_entry()", 0, 0);
}
...
*   SOC_SAND_IN  UTILEX_SORTED_LIST_PTR         sorted_list -
*     Handle to the sorted list.
*   SOC_SAND_IN  UTILEX_SORTED_LIST_KEY         *key -
*     The key to add into the sorted list
*   SOC_SAND_IN  UTILEX_SORTED_LIST_DATA        *data -
*     The data to add into the sorted list and to be associated with
*     the given key
*   SOC_SAND_OUT  uint8                           *success -
*     whether the add operation success, this may be false.
*     if after trying to relist the key UTILEX_SORTED_LIST_MAX_NOF_RELIST times
...
{
  uint32 new_node_ptr;
  uint32 index_in_indices, sorted_list_index, res;
  ...
  res =
    UTILEX_SORTED_LIST_ACCESS_DATA.memory_use.get(unit, sorted_list_index,
                                                  &memory_use);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  res = soc_sand_occ_bm_alloc_next(unit, memory_use, &new_node_ptr, found);
  if (*found == FALSE)
  {
    goto exit;
  }
   ...
   */
  {
    ...
    res =
      UTILEX_SORTED_LIST_ACCESS_DATA.data.memwrite(unit, sorted_list_index,
                                                   data,
                                                   new_node_ptr * data_size,
                                                   data_size);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 26, exit);
  }
  utilex_sorted_list_node_link_set(unit,
                                   sorted_list, prev_node, new_node_ptr);
  utilex_sorted_list_node_link_set(unit,
                                   sorted_list, new_node_ptr, next_node);
  {
    uint8 is_allocated;
    uint32 num_elements_on_indices;
    res =
    ...
    if (is_allocated)
    {
      ...
      {
        index_in_indices =
          num_elements_on_indices - from_top_index_in_indices - 1;
        /*
        ...
        else if ((int) index_in_indices < (int) (num_elements_on_indices - 1))
        {
          ...
          {
            res =
              UTILEX_SORTED_LIST_ACCESS_DATA.indices.get(unit,
                                                         sorted_list_index,
                                                         tmp_index,
                                                         &tmp_on_indices);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);
            res =
              UTILEX_SORTED_LIST_ACCESS_DATA.indices.set(unit,
                                                         sorted_list_index,
                                                         tmp_index + 1,
                                                         tmp_on_indices);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in utilex_sorted_list_node_alloc()",
                               0, 0);
