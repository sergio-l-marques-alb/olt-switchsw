...
shr_sw_state_memset(int unit, uint8 *location, int input, uint32 length, uint32 flags){
    ...
    if (is_modified && SW_STATE_DO_JOURNALING(unit, flags)){
...
*********************************************************************/
int
shr_sw_state_memcpy(int unit, uint8 *location, uint8 *input, uint32 length, uint32 flags, uint8 internal_use){
    ...
    if (SOC_IS_DONE_INIT(unit) && is_modified && SW_STATE_DO_JOURNALING(unit, flags)){
        SW_STATE_JOURNAL_INSERT(unit, location, length, location);
    }
...
shr_sw_state_pbmp_operation(int unit, _shr_pbmp_t *path, SW_ST_PBMP op, _shr_pbmp_t *input_pbmp,
    ...
    if (SW_STATE_DO_JOURNALING(unit, flags)){
...
shr_sw_state_alloc(int unit, uint8** ptr, uint32 size, uint32 flags, char* dbg_string) {
...
#ifdef CRASH_RECOVERY_SUPPORT
    soc_dcmn_cr_suppress(unit, dcmn_cr_no_support_sw_state_alloc);
#endif
    ...
    */
    ...
    shr_sw_state_memcpy(unit, (uint8*) &shr_sw_state_data_block_header[unit]->next_free_data_slot, (uint8*)&uint8_pvalue,
                        sizeof(uint8_pvalue), flags, TRUE);

    uint32_value = shr_sw_state_data_block_header[unit]->size_left - (alloc_size + sizeof(shr_sw_state_alloc_element_t));
    shr_sw_state_memcpy(unit, (uint8*) &shr_sw_state_data_block_header[unit]->size_left, (uint8*)&uint32_value,
                        sizeof(uint32_value), flags, TRUE);


    assert(UINTPTR_TO_PTR(PTR_TO_UINTPTR(p)) == p);

    uint32_value = size / 4; /* size in 32b */
    shr_sw_state_memcpy(unit, (uint8*) &p[0], (uint8*)&uint32_value,
                        sizeof(uint32_value), flags, TRUE);
    uint32_value = 0xcccccccc;
    shr_sw_state_memcpy(unit, (uint8*) &p[1], (uint8*)&uint32_value,
                        sizeof(uint32_value), flags, TRUE);
    uint32_value = 0xdddddddd;
    shr_sw_state_memcpy(unit, (uint8*) &p[2 + size / 4], (uint8*)&uint32_value,
                        sizeof(uint32_value), flags, TRUE);
...
/* } */
    ...
    shr_sw_state_memcpy(unit, (uint8*) ptr, (uint8*)&uint8_pvalue,
                        sizeof(uint8_pvalue), flags, TRUE);
    ...
    shr_sw_state_memcpy(unit, (uint8*) shr_sw_state_data_block_header[unit]->ptr_offsets_sp, (uint8*)&alloc_element_value,
                        sizeof(alloc_element_value), flags, TRUE);


    alloc_element_pvalue = shr_sw_state_data_block_header[unit]->ptr_offsets_sp - 1;
    shr_sw_state_memcpy(unit, (uint8*) &shr_sw_state_data_block_header[unit]->ptr_offsets_sp, (uint8*)&alloc_element_pvalue,
                        sizeof(alloc_element_pvalue), flags, TRUE);
...
/* } */
    ...
    while (p <= stack_base) {
        if (((uint8 *)p->ptr_value == ptr)) {
            ...
            uint8_pvalue = NULL;
            shr_sw_state_memcpy(unit, (uint8*) freed_location, (uint8*)&uint8_pvalue,
                                sizeof(uint8_pvalue), flags, TRUE);
            ...
            shr_sw_state_memcpy(unit, (uint8*)((uint32*)ptr - 2), (uint8*)uint8_pzero, size, flags, TRUE);
    ...
    if (p != shr_sw_state_data_block_header[unit]->ptr_offsets_sp + 1) {
        ...
        shr_sw_state_memcpy(unit, (uint8*)p, (uint8*)(&alloc_element_value), sizeof(alloc_element_value), flags, TRUE);
    }
    /* pop the stack */
    alloc_element_pvalue = shr_sw_state_data_block_header[unit]->ptr_offsets_sp + 1;
    shr_sw_state_memcpy(unit, (uint8*)&shr_sw_state_data_block_header[unit]->ptr_offsets_sp, (uint8*)(&alloc_element_pvalue), sizeof(alloc_element_pvalue), flags, TRUE);
...
#ifdef CRASH_RECOVERY_SUPPORT
    shr_sw_state_data_block_header_t  *temp_header;
#endif
    uint8                              is_issu=0; 
        ...
        case socSwStateDataBlockRestoreAndOveride:
           ...
           /* try recovering from a crash
            ...
            if (SOC_WARM_BOOT(unit)) {
                temp_header = shr_sw_state_data_block_header[unit];
                shr_sw_state_data_block_header[unit] = retrieved_data_block_header;
                rc = soc_dcmn_cr_recover(unit);
                shr_sw_state_data_block_header[unit] = temp_header;
                if (BCM_FAILURE(rc)) {
...
shr_sw_state_memmove(int unit, void *dst_void, const void *src_void, uint32 len, uint32 flags)
{
    ...
    if (dst < src) {
        while (len--) {
           shr_sw_state_memcpy(unit, (uint8*)(dst++), (uint8*)(src++), sizeof(unsigned char), flags, TRUE);
        }
    } else if (src < dst) {
        while (len--) {
           shr_sw_state_memcpy(unit, (uint8*)(dst+len), (uint8*)(src+len), sizeof(unsigned char), flags, TRUE);
        }
    }
    return dst_void;
}
...
int shr_sw_state_defrag(int unit, uint8*** old_ptr, uint32 flags) {
    ...
    */
    ...
    shr_sw_state_memcpy(unit, (uint8*)(shr_sw_state_data_block_header[unit]->ptr_offsets_sp + 1), (uint8*)temp_stack,
                        num_of_alloc_mem*sizeof(shr_sw_state_alloc_element_t), flags, TRUE);
    sal_free(temp_stack);
    ...
    while (p > shr_sw_state_data_block_header[unit]->ptr_offsets_sp) {
        ...
        if (((uint8 *)temp_ptr != curr_ptr)) {
            ...
            shr_sw_state_memcpy(unit, curr_ptr + alloc_size, uint8_pzero, (uint8*)temp_ptr - curr_ptr, flags, TRUE);
            sal_free(uint8_pzero);
            ...
            shr_sw_state_memcpy(unit, (uint8*)orig_ptr, (uint8*)(&uint8_pvalue), sizeof(uint8_pvalue), flags, TRUE);
            shr_sw_state_memcpy(unit, (uint8*)(&p->ptr_value), (uint8*)(&uint8_pvalue), sizeof(uint8_pvalue), flags, TRUE);
            ...
            while (p2 > shr_sw_state_data_block_header[unit]->ptr_offsets_sp) {
                if (((shr_sw_state_data_block_header[unit]->data_ptr + p2->ptr_offset) >= 
                    ...
                    shr_sw_state_memcpy(unit, (uint8*)&p2->ptr_offset, (uint8*)&uint32_value, sizeof(uint32_value), flags, TRUE);
                }
                p2--;
            }
    ...
    shr_sw_state_memcpy(unit, (uint8*)&shr_sw_state_data_block_header[unit]->next_free_data_slot, (uint8*)&curr_ptr, sizeof(curr_ptr), flags, TRUE);
    uint32_value = (uint8*)(shr_sw_state_data_block_header[unit]->ptr_offsets_sp) - curr_ptr;
    shr_sw_state_memcpy(unit, (uint8*)&shr_sw_state_data_block_header[unit]->size_left, (uint8*)&uint32_value, sizeof(uint32_value), flags, TRUE);
...
int shr_sw_state_shr_bitdcl_nof_bits_update(int unit, uint8 *ptr, uint32 num_of_bits, uint32 flags) {
    ...
    shr_sw_state_memcpy(unit, extra_bits_ptr, &uint8_value, sizeof(uint8_value), flags, TRUE);
    SOC_EXIT;
exit:
    SOC_FUNC_RETURN;
}
    ...
    switch(param.op) {
      ...
      case SW_ST_BIT_CLEAR:
        if (SW_STATE_DO_AUTOSYNC(1)){
            ...
            if (SW_STATE_DO_JOURNALING(unit, param.flags)){
