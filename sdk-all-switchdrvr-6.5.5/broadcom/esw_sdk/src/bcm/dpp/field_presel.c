                            ...
                            bcm_field_presel_t *presel)
{
    ...
    if (flags & _BCM_DPP_PRESEL_ALLOC_WITH_ID) {
        ...
        if (flags & _BCM_DPP_PRESEL_ALLOC_WITH_STAGE ) 
        {
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_presel_to_bitmap(unit,*presel, stage, &index, &stage_id)); 
        }
                           ...
                           bcm_field_stage_t  stage)
{
    ...
    {
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_presel_to_bitmap(unit,presel, stage, &index, &stage_id)); 
            presel_stage = ((stage == bcmFieldStageIngress) ? BCM_FIELD_QUALIFY_PRESEL_ADVANCED_MODE_STAGE_INGRESS : 
                        ((stage == bcmFieldStageEgress)  ?  BCM_FIELD_QUALIFY_PRESEL_ADVANCED_MODE_STAGE_EGRESS : BCM_FIELD_QUALIFY_PRESEL_ADVANCED_MODE_STAGE_HASH));
    }
    ...
    BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_field_presel_port_profile_clear_all(unitData,
                                                     (presel | BCM_FIELD_QUALIFY_PRESEL | presel_stage)));
                                 ...
                                 bcm_field_presel_t presel_id)
{
    ...
    result = _bcm_dpp_field_presel_alloc(unitData,
                                         stage,
                                         _BCM_DPP_PRESEL_ALLOC_WITH_ID | _BCM_DPP_PRESEL_ALLOC_WITH_STAGE ,
                                         &presel_id);
    _DPP_FIELD_UNIT_UNLOCK(unitData);
    BCMDNX_IF_ERR_EXIT(result);
exit:
                               ...
                               bcm_field_presel_t presel_id)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL_ADVANCED_MODE(presel_id)) 
    {
        stage = _BCM_DPP_FIELD_PRESEL_STAGE_FROM_ENTRY(presel_id);
        flags = _BCM_DPP_PRESEL_ALLOC_WITH_STAGE;
    }
    result = _bcm_dpp_field_presel_free(unitData, presel_id, flags, stage);
    _DPP_FIELD_UNIT_UNLOCK(unitData);
    BCMDNX_IF_ERR_EXIT(result);
exit:
    BCMDNX_FUNC_RETURN;
                               ...
                               _bcm_dpp_field_presel_idx_t presel)
{
    ...
    if (_BCM_DPP_FIELD_ENTRY_IS_PRESEL_ADVANCED_MODE(presel)) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_presel_to_bitmap(unit,presel, bcm_stage, &index, &stage)); 
        flags = _BCM_DPP_PRESEL_ALLOC_WITH_STAGE;
    }
