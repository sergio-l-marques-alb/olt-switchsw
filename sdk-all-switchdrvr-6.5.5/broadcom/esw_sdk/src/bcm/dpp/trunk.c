                               ...
                               bcm_trunk_member_t   *member)
{
    ...
    } else 
    {
        ...
        if((member->flags & BCM_TRUNK_MEMBER_INGRESS_DISABLE) != 0) 
        {
            ...
            /* check and resolve case of existing member with INGRESS_DISABLE flag */
            ...
            /* if has replications and has INGRESS_DISABLE flag */
        ...
        {
            LOG_INFO(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Added member (gport:%x) to trunk (id:%d)\n"), FUNCTION_NAME(), member->gport, tid));
    ...
    if (tid > ngroups && tid < ngroups + stk_ngroups) {
        ...
        if (TRUNK_TID_VALID(trunk_id)) 
        {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                LOG_INFO(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Deleted member(gport:%x) from trunk (id:%d)\n"), FUNCTION_NAME(), member->gport, tid));
            }
        } else {
            rv = BCM_E_NOT_FOUND;
        }
...
bcm_petra_trunk_bitmap_expand(int unit, bcm_pbmp_t *pbmp_ptr)
{
    ...
    for (tid = 0; tid < ngroups; tid++) {
        ...
        if (in_use == TRUE) {
            BCMDNX_ALLOC(lag_info, sizeof(*lag_info), "bcm_petra_trunk_bitmap_expand.lag_info");
            if(lag_info == NULL) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Memory allocation failue\n")));
            }
            rv = soc_ppd_lag_get(unit, tid, lag_info);
            ...
            if (BCM_FAILURE(rv)) {
                ...
                for (index = 0; ((index < lag_info->nof_entries) &&
                    ...
                    if (BCM_SUCCESS(rv) && BCM_GPORT_IS_MODPORT(modport) &&
                      ...
                      bcm_trunk_t *tid)
{
    ...
    *tid = BCM_TRUNK_INVALID;
    ...
    found = 0;
    for (t = 0; t < ngroups; t++) {
        BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.t_info.in_use.get(unit, t, &in_use));
        if (TRUE == in_use) {
            BCMDNX_ALLOC(lag_info, sizeof(*lag_info), "bcm_petra_trunk_find.lag_info");
            if(lag_info == NULL) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Memory allocation failue\n")));
            }
            SOC_PPC_LAG_INFO_clear(lag_info);
            BCMDNX_IF_ERR_EXIT(TRUNK_ACCESS.t_info.trunk_id.get(unit, t, &trunk_id));
    ...
    bcm_gport_t*         gport)
{
    ...
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("system_port_aggregate 0x%x is invalid"), system_port_aggregate));
    }
    ...
    if(!is_lag)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("system_port 0x%x is not a system port aggregate invalid"), system_port_aggregate));
    }
    ...
    if(!trunk_in_use)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("trunk ID %d is not in use"), lag_id));
    }
    ...
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("member ID %d greater or equal to number of members in trunk %d (contains %d members)"), lag_member_id, lag_id, lag_info.nof_entries));        
    }
