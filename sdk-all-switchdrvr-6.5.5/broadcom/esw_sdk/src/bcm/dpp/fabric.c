                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
  ...
  BCMDNX_NULL_CHECK(profile_array);
  {
    ...
    if (profile_count > SOC_DPP_DEFS_GET(unit, nof_fabric_links) * SOC_DPP_DEFS_GET(unit, nof_rci_levels)  || profile_count < 0) {
    ...
    for(ii = 0 ; ii < profile_count ; ii++)
    {
      ...
      if (profile_array[ii].num_links > SOC_DPP_DEFS_GET(unit, nof_fabric_links)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Number of links on profile is out of range")));
      }
    }
  }
  for (ii = 0 ; ii < profile_count ; ii++)
                                  ...
                                  int queue_id)
{
    ...
    case bcmFabricQueueMax:
        ...
        break;
    default:
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("contro type %d invalid"), control_type));
    }
        ...
        if (ftmh_info.action_ing != SOC_TMC_TDM_ING_ACTION_NO_CHANGE) {
            if (ftmh_info.is_mc) {
                ...
                if (SOC_IS_DPP_TDM_OPTIMIZE(unit)) {                                        
                    ...
                    BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_tm_to_local_port_get,(unit, core, ftmh_info.ftmh.opt_uc.dest_if, &local_port)));

                    BCM_GPORT_MODPORT_SET(editing->destination_port,modid,local_port);
                } else {
                      
                      rc = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_tm_to_local_port_get,(unit, core, ftmh_info.ftmh.standard_uc.dest_fap_port, &local_port));
                      BCMDNX_IF_ERR_EXIT(rc);
