                                   ...
                                   egr_vlan_xlate_entry_t *egr_vlan_xlate)
{
    ...
    if ((soc_EGR_VLAN_XLATEm_field32_get(unit, egr_vlan_xlate, 
        ...
        tpid_idx = soc_EGR_VLAN_XLATEm_field32_get(unit, egr_vlan_xlate, 
...
_bcm_tr2_mim_egr_vxlt_sd_tag_actions_delete(
    int unit, egr_vlan_xlate_entry_t *egr_vlan_xlate_entry)
{
                                   ...
                                   int vp, bcm_mim_vpn_t vpn)
{
    ...
    if ((mim_port->criteria == BCM_MIM_PORT_MATCH_PORT_VLAN) ||
        ...
        rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ANY, &xlate_index,
                            &vent, &return_vent, 0);
        if (BCM_E_NONE == rv) {
            if (mim_port->flags & BCM_MIM_PORT_REPLACE) {
                BCM_IF_ERROR_RETURN(
                    soc_mem_write(unit,
                                  VLAN_XLATEm, MEM_BLOCK_ALL, xlate_index, &vent));
            } else {
                return BCM_E_EXISTS;
            }
...
_bcm_tr2_mim_match_delete(int unit, int vp, bcm_mim_port_t mim_port)
{
    ...
    if (MIM_INFO(unit)->port_info[vp].flags & 
        ...
        if (SOC_IS_TRIDENT2X(unit) || SOC_IS_KATANA2(unit) || SOC_IS_TOMAHAWK(unit)) {
            soc_VLAN_XLATEm_field32_set(unit, &vent, SOURCE_TYPEf, 1);
        }
...
_bcm_tr2_mim_peer_port_config_add(int unit, bcm_mim_port_t *mim_port, 
    ...
    if (mim_port->flags & BCM_MIM_PORT_EGRESS_TUNNEL_SERVICE_REPLACE) {
        soc_EGR_VLAN_XLATEm_field32_set(
            unit, &egr_vlan_xlate_entry, MIM_ISID__ISIDf,
            (mim_port->egress_tunnel_service & 0xffffff));
    } else {
        soc_EGR_VLAN_XLATEm_field32_set(unit, &egr_vlan_xlate_entry,
    ...
    BCM_IF_ERROR_RETURN(soc_mem_insert(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ALL,
                                       &egr_vlan_xlate_entry));
...
_bcm_tr2_mim_peer_port_config_delete(int unit, int vp, bcm_mim_vpn_t vpn)
{
    ...
    isid = soc_EGR_VLAN_XLATEm_field32_get(unit, &egr_vlan_xlate_entry,
                                           MIM_ISID__ISIDf);
    ...
    soc_MPLS_ENTRYm_field32_set(unit, &ment, KEY_TYPEf, 3); /* MIM_ISID_SVP */
    soc_MPLS_ENTRYm_field32_set(unit, &ment, VALIDf, 0x1);
    soc_MPLS_ENTRYm_field32_set(unit, &ment, MIM_ISID__ISIDf, isid);
    soc_MPLS_ENTRYm_field32_set(unit, &ment, MIM_ISID__SVPf, vp);
    rv = soc_mem_delete(unit, MPLS_ENTRYm, MEM_BLOCK_ANY, &ment);
    BCM_IF_ERROR_RETURN(rv);
#if defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    ...
    BCM_IF_ERROR_RETURN(soc_mem_delete(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ANY,
                        &egr_vlan_xlate_entry));
...
bcm_tr2_mim_port_add(int unit, bcm_mim_vpn_t vpn, bcm_mim_port_t *mim_port)
{
           ...
           (!mim_use_default_reserve_svp)) {
            if (!(mim_port->flags & BCM_MIM_PORT_REPLACE)) {
                ...
                if (soc_feature(unit, soc_feature_mim_peer_sharing) &&
                    ...
                    mim_port->encap_id = nh_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
    ...
    if (mim_port->flags & BCM_MIM_PORT_REPLACE) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_tr2_mim_match_add(unit, mim_port, vp, vpn));
    } else {
...
port_cleanup:
    if (rv < 0) {
        ...
        if (!(mim_port->flags & BCM_MIM_PORT_REPLACE)) {
            if (!mim_use_default_reserve_svp) {
...
_bcm_tr2_mim_match_get(int unit, bcm_mim_port_t *mim_port, int vp)
{
    ...
    if ((MIM_INFO(unit)->port_info[vp].flags & 
        ...
        if (soc_feature(unit, soc_feature_ingress_failover)) {
            ...
            int                index = -1;
            sal_memset(&ment, 0, sizeof(mpls_entry_entry_t));
            soc_MPLS_ENTRYm_field32_set(unit, &ment, KEY_TYPEf, 1);
            soc_MPLS_ENTRYm_field32_set(unit, &ment, VALIDf, 1);
            soc_MPLS_ENTRYm_field32_set(unit, &ment, MIM_NVP__BVIDf,
                MIM_INFO(unit)->port_info[vp].match_tunnel_vlan);
            soc_mem_mac_addr_set(unit, MPLS_ENTRYm, &ment, MIM_NVP__BMACSAf,
                MIM_INFO(unit)->port_info[vp].match_tunnel_srcmac);
            BCM_IF_ERROR_RETURN(soc_mem_search(unit, MPLS_ENTRYm, MEM_BLOCK_ANY,
                                               &index, &ment, &ment, 0));
    ...
    egr_vlan_xlate_entry_t *egr_vlan_xlate_entry)
{
    ...
    if ((action_not_present == 0x1) || (action_present == 0x1)) {
        ...
        mim_port->flags |= 
                    BCM_MIM_PORT_EGRESS_SERVICE_TPID_REPLACE;
        tpid_idx = soc_EGR_VLAN_XLATEm_field32_get(unit, egr_vlan_xlate_entry,
                                                 MIM_ISID__SD_TAG_TPID_INDEXf);
        _bcm_fb2_outer_tpid_entry_get(unit, &mim_port->egress_service_tpid, tpid_idx);
    }
#endif
...
_bcm_td2_mim_match_vp_replace(int unit, int vp, int new_vp, int *old_vp)
{
    ...
    switch (mkey->flags) {
        ...
        case _BCM_MIM_PORT_TYPE_ACCESS_LABEL:
            ...
            soc_mem_lock(unit, mem);
            if (mkey->modid != -1) {
                soc_MPLS_ENTRYm_field32_set(unit, &ment, MODULE_IDf, mkey->modid);
                soc_MPLS_ENTRYm_field32_set(unit, &ment, PORT_NUMf, mkey->port);
            } else {
                soc_MPLS_ENTRYm_field32_set(unit, &ment, Tf, 1);
                soc_MPLS_ENTRYm_field32_set(unit, &ment, TGIDf, mkey->tgid);
            }
            soc_MPLS_ENTRYm_field32_set(
                unit, &ment, MPLS_LABELf, mkey->match_label);
        ...
        case _BCM_MIM_PORT_TYPE_PEER:
            ...
            soc_MPLS_ENTRYm_field32_set(unit, &ment, KEY_TYPEf, 0x1);/* MIM NVP */
            soc_MPLS_ENTRYm_field32_set(unit, &ment, MIM_NVP__BVIDf,
