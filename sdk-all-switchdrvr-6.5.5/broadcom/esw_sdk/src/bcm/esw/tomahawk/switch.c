...
STATIC int
_th_agm_stat_id_get(int unit, bcm_switch_agm_id_t agm_id, int agm_pid,
...
STATIC int
_th_agm_stat_detach(int unit, bcm_switch_agm_id_t agm_id, int agm_pid)
{
    soc_mem_t                  table;
...
STATIC int
_th_agm_stat_attach(int unit, bcm_switch_agm_id_t agm_id,
                         ...
                         bcm_switch_agm_info_t *agm_info)
{
    ...
    rv = _th_agm_stat_id_get(unit, agm_id, agm_pid, agm_info, &counter_id);
    if (BCM_FAILURE(rv)) {
        AGM_UNLOCK(unit);
        return rv;
    }
    ...
    rv = _th_agm_stat_attach(unit, agm_id, agm_pid, counter_id);
    if (BCM_FAILURE(rv)) {
    ...
    if (BCM_FAILURE(rv)) {
        (void)_th_agm_stat_detach(unit, agm_id, agm_pid);
...
bcm_th_switch_agm_destroy(int unit, bcm_switch_agm_id_t agm_id)
{
    ...
    rv = _th_agm_stat_detach(unit, agm_mnt->attr.agm_id, agm_mnt->agm_pool_id);
    if (BCM_FAILURE(rv)) {
        AGM_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_SWITCH, (BSL_META_U(unit,
