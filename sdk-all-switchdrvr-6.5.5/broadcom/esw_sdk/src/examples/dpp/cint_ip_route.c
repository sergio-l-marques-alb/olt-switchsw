...
int add_route(int unit, uint32 addr, uint32 mask, int vrf, int intf) {
  ...
  l3rt.l3a_vrf = vrf;
  if (lpm_raw_data_enabled || large_direct_lu_key_length) {
...
int kaps_raw_data_redirect(int unit) {
    ...
    if (!large_direct_lu_key_length) {
        ...
        uint8 data1[3], mask1[3];
        data1[0] = (lpm_payload & 0xff0000);
        data1[1] = (lpm_payload & 0x00ff00);
        data1[2] = (lpm_payload & 0x0000ff);
        ...
        int size=(large_direct_lu_key_length / 8);
        if (large_direct_lu_key_length % 8 !=0) {
            size++;
        }
        result = bcm_field_qualify_data(unit, action_entry, data_qual.qual_id, &data1, &mask1, size);    /*3*/
        if (BCM_E_NONE != result) {
