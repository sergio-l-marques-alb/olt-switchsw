...
int 
policer_set_get_example(int unit, int in_port, int packet_mode) {
	...
	rv = bcm_rate_bandwidth_set(unit, in_port, BCM_RATE_PKT_ADJ_HEADER_TRUNCATE|BCM_RATE_DLF|packet_mode_flag, 1800000, 5500);
	if (rv != BCM_E_NONE) {
    ...
    rv = bcm_rate_bandwidth_set(unit, in_port, BCM_RATE_UNKNOWN_MCAST|packet_mode_flag, 2300000, 7000);
    if (rv != BCM_E_NONE) {
	...
	rv = check_error_by_percent(2300000,kbits_sec,0.05);
	if (rv != BCM_E_NONE) {
    ...
    rv = bcm_rate_bandwidth_set(unit, in_port, BCM_RATE_PKT_ADJ_HEADER_TRUNCATE|BCM_RATE_MCAST|packet_mode_flag, 500000, 400);
    if (rv != BCM_E_NONE) {
    ...
    rv = bcm_rate_bandwidth_set(unit, in_port, BCM_RATE_PKT_ADJ_HEADER_TRUNCATE|BCM_RATE_BCAST|packet_mode_flag, 100000, 200);
    if (rv != BCM_E_NONE) {
...
traffic_example(int unit, int in_port, int out_port) {
	...
	/* set 1800MB rate for unknown unicast traffic */
	rv = bcm_rate_bandwidth_set(unit, in_port, BCM_RATE_DLF|BCM_RATE_PKT_ADJ_HEADER_TRUNCATE, 250000, 300);
	if (rv != BCM_E_NONE) {
		printf("Error in bcm_rate_bandwidth_set for BCM_RATE_DLF\n");
		return rv;
	}
 ...
 */
int 
aggregate_policer_traffic_example(int unit, int core ,int in_port, int out_port) {
	...
	/* 3. set port(s) to the group of policers*/
	rv = bcm_port_policer_set(unit,in_port,policer_id);
	if (rv != BCM_E_NONE) {
		printf("Error in bcm_port_policer_set, port =%d\n",in_port);
		return rv;
	}
