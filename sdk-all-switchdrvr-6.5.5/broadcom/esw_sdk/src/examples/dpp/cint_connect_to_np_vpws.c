...
int p_port_l3vpn = 13;
uint16 g_next_hop_index = 0x1122;
uint16 g_ulei_port=0x3344;

void fill_data_format_entry1(uint32 *data, uint16 nhi, uint16 lsb)
{
    data[0] = (nhi&0xffff)<<8 | (lsb&0xff) | (((g_ulei_port<<2)&0xff) << 24);
    data[1] = ((g_ulei_port<<2)&0x7f00)>>8;
}

int data_format_entry1_create(int unit, int *global_lif_id, uint32 next_eep, uint16 lsb)
{
    uint32 data[2];
    int rv = 0;
    ...
    /* fill information into data-entry */
    fill_data_format_entry1(data, g_next_hop_index, lsb);
...
int two_data_format_entry_create(int unit, int *global_lif_id, uint16 lsb)
{
    int rv;
    uint32 next_eep;
    int g_lif_id;
    ...
    rv = data_format_entry1_create(unit, global_lif_id, next_eep, lsb);
    if (rv != BCM_E_NONE) {
...
int config_ingress_l3vpn(int unit)
{
    ...
    rv = two_data_format_entry_create(unit, &global_lif_id, 0);
    if (rv != BCM_E_NONE) {
        printf("Error, data_format_entry_create\n");
        return rv;
    }
...
int config_ingress_vpn_bridge(int unit)
{
    ...
    data_entry_init(out_port_mpls,g_flow_id,1,g_out_fp,7,1,2);
    rv = two_data_format_entry_create(unit, &global_lif_id, 2);
    if (rv != BCM_E_NONE) {
        printf("Error, data_format_entry_create\n");
        return rv;
    }
