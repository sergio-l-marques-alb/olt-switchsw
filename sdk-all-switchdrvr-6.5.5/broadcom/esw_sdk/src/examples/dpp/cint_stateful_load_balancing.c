                        ...
                        /* in */  bcm_field_group_t group_post_hash) 
{
  ...
  /* Match IPv4 packets */
  result = bcm_field_qualify_HeaderFormat(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldHeaderFormatIp4AnyL2L3);
  if (BCM_E_NONE != result) {
...
int slb_create_stateful_ecmp(const int unit, const int in_port, const int out_port, const int ecmp_size)
{
  ...
  rc = ecmp_hashing_main(unit, in_port, out_port, ecmp_size);
  if (BCM_E_NONE != rc) {
