                         ...
                         uint32 *bank_ignore_mask)
{
    if (soc_feature(unit, soc_feature_sbus_format_v4)) {
        ...
        if (dst_blk != NULL) {
            *dst_blk = soc_feature(unit, soc_feature_blkid_extended_format) ? (header->v4.dst_blk >> 4) : header->v4.dst_blk;
        }
        if (src_blk != NULL) {
            *src_blk = 0;
        }
        if (acc_type != NULL) {
            *acc_type = (soc_feature(unit, soc_feature_blkid_extended_format)) ? (header->v4.dst_blk & 0xf) >>1 : header->v4.acc_type;
        }
