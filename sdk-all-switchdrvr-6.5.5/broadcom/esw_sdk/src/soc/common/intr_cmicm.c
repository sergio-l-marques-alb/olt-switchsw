...
soc_cmicm_fifo_dma_done(int unit, uint32 vchan)
{
...
#endif
    }else if (SOC_IS_JERICHO(unit) && (ch == oam_status_channel%N_DMA_CHAN))  {
#ifdef BCM_DPP_SUPPORT
        uint8 oam_is_init=0;
        uint8 bfd_is_init=0;
        uint32 rv;
...
#endif
    } else if (SOC_IS_JERICHO(unit) && (ch == oam_event_channel%N_DMA_CHAN))  {
#ifdef BCM_DPP_SUPPORT
        uint8 oam_is_init=0;
        uint8 bfd_is_init=0;
        uint32 rv;
    ...
    } else if (SOC_IS_JERICHO(unit) && (ch == olp_channel%N_DMA_CHAN))  {
#ifdef BCM_DPP_SUPPORT
