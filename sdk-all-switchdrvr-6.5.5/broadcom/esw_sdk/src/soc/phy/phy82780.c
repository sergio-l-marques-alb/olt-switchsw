/*
 * $Id: phy82780.c,v 1.1.2.25 Broadcom SDK $
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    if (port_num_lanes == 1) {
        ...
        } else if (pc->speed_max == PHY82780_SPD_42G) {
            speed_config->line_interface =  SOC_PORT_IF_SR4;
...
_phy_82780_init_pass1(int unit, soc_port_t port)
{
    ...
    sal_strncpy (dev_name, "Quarda28", sal_strlen("Quadra28"));
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        ...
        speed_config->higig_port = (PBMP_MEMBER(SOC_HG2_PBM(unit), port) || PBMP_MEMBER(PBMP_HG_ALL(unit), port)); 
...
_phy_82780_init_pass2(int unit, soc_port_t port)
{
    ...
    if (DEVID(pc) == Q28_ID_82780) { 
        ...
        sal_strncpy (dev_name, "BCM82780", sal_strlen("BCM82780"));
    } else if (DEVID(pc) == Q28_ID_82752) { 
        /* coverity[secure_coding] */
        sal_strncpy (dev_name, "BCM82752", sal_strlen("BCM82752"));
    } else if (DEVID(pc) == Q28_ID_82758) {
        /* coverity[secure_coding] */
        sal_strncpy (dev_name, "BCM82758", sal_strlen("BCM82758"));
    } else if (DEVID(pc) == Q28_ID_8278F) {
        /* coverity[secure_coding] */
        sal_strncpy (dev_name, "BCM8278F", sal_strlen("BCM8278F"));
    } else {
        /* coverity[secure_coding] */
        sal_strncpy (dev_name, "Quarda28", sal_strlen("Quadra28"));
    }
...
phy_82780_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    SOC_IF_ERROR_RETURN
        (phy_82780_power_set(pmc, intf, enable));
...
phy_82780_enable_get(int unit, soc_port_t port, int *enable)
{
    ...
    *enable = 0;
    ...
    intf = (pc->flags & PHYCTRL_SYS_SIDE_CTRL) ? PHY82780_SYS_SIDE : PHY82780_LINE_SIDE; 
    SOC_IF_ERROR_RETURN
        (phy_82780_power_get(pmc, intf, &ena_dis));
    *enable = ena_dis;
...
phy_82780_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    pmc = &pc->phymod_ctrl;
    SOC_IF_ERROR_RETURN (
        phy_82780_loopback_internal_pmd_set(pmc, PHY82780_LINE_SIDE, enable));
...
STATIC int
phy_82780_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
