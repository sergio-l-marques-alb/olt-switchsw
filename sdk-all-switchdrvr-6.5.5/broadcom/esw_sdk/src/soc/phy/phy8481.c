/*
 * $Id: phy8481.c,v 1.252 Broadcom SDK $
...
#define PHY8481_SET_REPEATER(_pc)\
        if ( PHY_IS_BCM8486X_AND_UP(_pc) ) { /* Mako and Orca supports 5G/2.5G speed */    \
            uint32   _xfi_2500x = 0;                                          \
                                                                              \
            _xfi_2500x = soc_property_port_get((_pc)->unit, (_pc)->port, spn_PHY_SERDES, 0);\
            if ( (_xfi_2500x == 2500 && PHY_IS_BCM8486X(_pc)) ||              \
                   (PHY_IS_BCM8488X(_pc) && (_xfi_2500x == 2500 || _xfi_2500x == 5000)) ) {                                                                    \
                PHY_FLAGS_CLR((_pc)->unit, (_pc)->port, PHY_FLAGS_REPEATER);  \
            } else {                                                          \
                /* As setting 5G/2.5G line side speed (over XFI mode),            *\                                                                          \
                |* phy_8481_link_up() will notify internal SerDes with 10G speed. *|                                                                          \
                \* set REPEATER flag to tell MAC driver don't notify speed again! */                                                                          \
                PHY_FLAGS_SET((_pc)->unit, (_pc)->port, PHY_FLAGS_REPEATER);  \
            }                                                                 \
        } else {                                                              \
                PHY_FLAGS_CLR((_pc)->unit, (_pc)->port, PHY_FLAGS_REPEATER);  \
        }
...
phy_8481_init(int unit, soc_port_t port)
{
    ...
    uint32         phy_long_xfi, xfi_2500x = 0, priority = 0;
    uint8          *fw = NULL;
    int            fw_len = 0;
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) ||
        ...
        if ( (! fw_ver) && (PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1)
            ...
            case  PHY_OUI_MODEL(PHY_BCM84888_OUI, PHY_BCM84888_MODEL) |
                ...
                FIRMWARE(pc) = bcm_84888_firmware;
                FIRMWARE_LEN(pc) = bcm_84888_firmware_size;
                pc->flags |= PHYCTRL_MDIO_BCST;
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        if ( PHY_IS_BCM8486X_AND_UP(pc) ) {    /* Mako and Orca supports 5G/2.5G speed */
            ...
            xfi_2500x = soc_property_port_get(unit, port,
                                                     spn_PHY_SERDES, 0);
            if ( (xfi_2500x == 2500 && PHY_IS_BCM8486X(pc)) || 
                   (PHY_IS_BCM8488X(pc) && (xfi_2500x == 2500 || xfi_2500x == 5000)) ) {
        ...
        if (PHY_IS_BCM8484X(pc)) {
            /* TX output squelch. Register bank = 0x1  offset = 0xE set bit[12]*/
            SOC_IF_ERROR_RETURN(
                _phy84834_xfi_reg_set(unit, pc, 0x1, 0xE, 0x1000));
        }
...
_phy_8481_copper_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if ( enable ) {         /* enable loopback */
        ...
        if ( lbmood ) {             /* already in loopback mode */
            return SOC_E_NONE;      /* do nothing               */
        }

        /* check 8486X's link status BEFORE getting the line side speed */
        if ( PHY_IS_BCM8486X(pc) ) {
            int  linkup;
            /* get line side link status, link-up or link-down */
            SOC_IF_ERROR_RETURN(_phy_8481_copper_link_get(unit, port, &linkup));
            if ( ! linkup ) {
                /* turn AutoNeg off if 8486x links down at line side */
                phy_8481_an_set(unit, port, FALSE);
                sal_udelay(100);  /* wait for the AN setting to take effect */
            }
        }

        /* get the line side speed to determine what registers to set */
        SOC_IF_ERROR_RETURN(_phy_8481_copper_speed_get(unit, port, &speed));
        if ( PHY_IS_BCM8486X_AND_UP(pc) ) {
            switch ( speed ) {
            case  2500 :
                PHY_FLAGS_SET(unit, port,
                              PHY_FLAGS_NO_SYS_LINE_SPD_SYNC);
                data16 = MGBASE_AN_CTRL_SPEED_2P5G;     break;
            case  5000 :
                data16 = MGBASE_AN_CTRL_SPEED_5G;       break;
            default :
                data16 = 0;
            }
        ...
        if ( speed >= 2500 ) {     /*  10G / 5G / 2.5G  */
            ...
            if ( PHY_IS_BCM8484X_AND_UP(pc) ) {
                if ( speed == 100 ) {
                    WRITE_PHY8481_MII_CTRLr(unit, pc, 0x7100);
                } else {
                    WRITE_PHY8481_MII_CTRLr(unit, pc, 0x5140);
                }
            } else {
...
phy_8481_xehg_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    if ( PHY_IS_BCM8486X_AND_UP(pc) & (! PHY_REPEATER(unit, port)) ) {
        /* Mako's MAC interface is in 2500BASE-X mode */
        *pif = SOC_PORT_IF_SGMII;
    } else {
        /* Mako's MAC interface is in XFI mode        */
...
_phy84834_top_level_cmd_set_v2(int unit,phy_ctrl_t *pc, uint16 cmd, uint16 arg[], int size)
{
    ...
    if(PHY_IS_BCM8484X(pc) && (cmd == PHY8485X_DIAG_CMD_WRITE_SHADOW_REG)) {
        flag = 1;
    }
    if(!flag) {
    soc_timeout_init(&to, 7000000, 0);
...
_phy84834_top_level_cmd_get_v2(int unit,phy_ctrl_t *pc, uint16 cmd, uint16 arg[], int size)
{
    ...
    if(PHY_IS_BCM8484X(pc) && (cmd == PHY8485X_DIAG_CMD_READ_SHADOW_REG)) {
        flag = 1;
    }
    if(!flag) {
    soc_timeout_init(&to, 7000000, 0);
...
_phy84834_xfi_reg_set_v2(int unit, phy_ctrl_t *pc, uint16 pre_tap, uint16 main_tap, uint16 post1_tap, uint16 post2_tap, uint16 HPF_en)
{
    ...
    if (pc->flags & PHYCTRL_SYS_SIDE_CTRL)
        cmd = (uint16)PHY84834_DIAG_CMD_SET_XFI_TX_FILTERS_GENERIC;
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        if (PHY_IS_BCM8485X_AND_UP(pc)) {
            ...
            post_tap1 = PHY8485X_PREEMPH_GET_POST_TAP1(value); 
            post_tap2 = PHY8485X_PREEMPH_GET_POST_TAP2(value);           
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        if (PHY_IS_BCM8485X_AND_UP(pc)) {
            SOC_IF_ERROR_RETURN(
                _phy84834_xfi_reg_get_v2(unit, pc, &pre_tap, &main_tap, &post_tap1, &post_tap2, &HPF_en));
        
            *value = 0;
            *value |= pre_tap;
            main_tap = ((temp | main_tap) << 4);
            post_tap1 = ((temp | post_tap1) << 10);
            HPF_en = ((temp | HPF_en) << 15);
            post_tap2 = ((temp | post_tap2) << 16);

            *value |= (main_tap | post_tap1 | HPF_en | post_tap2);
        }
        else {
            /* Register bank = 0x2  offset = 0xB value */
            SOC_IF_ERROR_RETURN(
    ...
    case SOC_PHY_CONTROL_EEE_AUTO:
        ...
        if (value) {
            if (PHY_IS_BCM8483X_AND_UP(pc)) {
                ...
                SOC_IF_ERROR_RETURN
                    (_phy84834_eee_mode_set(unit, pc, mode, 0x0000, 0x3d09, 0x047e ));
...
_phy_8481_copper_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    if ( PHY_IS_BCM8484X_AND_UP(pc) ) {
    	...
    	case 3 :
            *speed = 10000;   break;
    	case 4 :
            *speed = 2500;    return SOC_E_NONE;
    	case 5 :
            *speed = 5000;    return SOC_E_NONE;
    	default:
            *speed = 0;       break;
    	}
        ...
        if ( linkup ) {            /* if line side link is up, speed info has */
            return SOC_E_NONE;     /* been got from Core Config Status Reg.   */
        } 
        /* Core Config Status Register reports wrong speed when link is down, *\
        \* need to run the following legacy way to determine link-down speed. */
    }
    ...
    } else {   /* autoNego is no t enabled, forced speed */
                                    ...
                                    soc_port_ability_t *ability)
{
    ...
    if ( (mii_ana & MII_ANA_HD_10) || (mii_ana & MII_ANA_FD_10) ) {
        rv = SOC_E_PARAM;
    }
    ...
    if (PHY_IS_BCM8486X_AND_UP(pc)) {
        ...
        SOC_IF_ERROR_RETURN(
            READ_PHY8481_MGBASET_AN_CTRLr(unit, pc, &teng_an_ctrl) );
        if ( teng_an_ctrl & MGBASE_AN_CTRL_ADV_2P5G ) {
            ability->speed_full_duplex |= SOC_PA_SPEED_2500MB;
        }
        if ( teng_an_ctrl & MGBASE_AN_CTRL_ADV_5G ) {
                                    ...
                                    soc_port_ability_t *ability)
{
    ...
    if ( (ability->speed_half_duplex & SOC_PA_SPEED_10MB) || (ability->speed_full_duplex & SOC_PA_SPEED_10MB) ) {
        rv = SOC_E_PARAM;
    }

    if(PHY_IS_BCM8488X(pc)) {
        if ( (ability->speed_full_duplex & SOC_PA_SPEED_10GB) || (ability->speed_full_duplex & SOC_PA_SPEED_100MB) || 
                (ability->speed_half_duplex & SOC_PA_SPEED_100MB) || (ability->speed_full_duplex & SOC_PA_SPEED_1000MB) ||
                (ability->speed_half_duplex & SOC_PA_SPEED_1000MB) ) {
            /* 7.0x0020 [7:8]  */
            mii_gb_ctrl &= ~MII_ECR_BRSA;
            mii_gb_ctrl &= ~MII_ECR_BMLT3;
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY8481_TENG_AN_CTRLr(unit, pc, mii_gb_ctrl,
                                              MII_ECR_BRSA | MII_ECR_BMLT3));
        }
    ...
    /* 10GBASE-T AN Control Reg.(7.0x0020), Advert_10G_capability[12] */
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY8481_TENG_AN_CTRLr(unit, pc, mii_gb_ctrl,
...
{
    int          speed, an, an_done, is_line = 0;
    phy_ctrl_t  *pc;
    phy_ctrl_t  *int_pc;
        ...
        if ( ! LAST_IN_CHAIN(pc) ) {
            ...
            SOC_IF_ERROR_RETURN
                (soc_phyctrl_notify(unit, port, phyEventInterface,
                        (PHY_REPEATER(unit, port)) ? SOC_PORT_IF_XGMII
                                                   : SOC_PORT_IF_2500X));
            ...
            if ( PHY_IS_BCM8484X(pc) ) {
                ...
                /* Turn on TX. Register bank = 0x1  offset = 0xE bit[12]:0*/
                SOC_IF_ERROR_RETURN(
                    _phy84834_xfi_reg_set(unit, pc, 0x1, 0xE, 0x0));
                if ( is_line ) {
                    pc->flags &= ~PHYCTRL_SYS_SIDE_CTRL;
                    is_line = 0;
                }
         ...
         int tx)   /* tx or rx */
{
    ...
    if (PHY_IS_BCM8485X_AND_UP(pc)) {
        ...
        if (tx) {
            ...
            SOC_IF_ERROR_RETURN
                (WRITE_PHY8481_PMAD_REG(unit, pc,
                    PHY8485X_RX_PRBS_REGISTER, data16));
        }
    } else {
        if (poly_ctrl == 0) {
            data16 = 1;
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 *data)
{
    ...
    } else { /* Old way if FW is old */
            SOC_IF_ERROR_RETURN
                (phy_reg_xge_read(unit, pc, flags, reg_bank, dev_addr, reg_addr, data));
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 data)
{
    ...
    if (FLAGS(pc) & PHY8483X_ATOMIC_ACCESS_SHD) {
        ...
        SOC_IF_ERROR_RETURN
                (phy_reg_xge_write(unit, pc, flags, reg_bank, dev_addr, reg_addr, data));
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 data, uint16 mask)
{
    ...
    if (FLAGS(pc) & PHY8483X_ATOMIC_ACCESS_SHD) {
        ...
        SOC_IF_ERROR_RETURN
                (phy_reg_xge_modify(unit, pc, flags, reg_bank, dev_addr, reg_addr, data, mask));
    }
