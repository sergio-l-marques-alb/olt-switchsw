 ...
 * note the following:
 * soc_sand_low_level.c is using the the define USING_CHIP_SIM.
...
/*
 * Indicator:
 * TRUE  - SOC_SAND driver solved the big/little endian issue.
 * FALSE - SOC_SAND driver DID NOT solved the big/little endian issue.
 */
uint32
...
/*
 * Indicator:
 * TRUE  - SOC_SAND driver is access the device in big endian.
 * FALSE - SOC_SAND driver is access the device in little endian.
 */
 ...
 * Examples:
 *  + SOC_SAND_FE200  have only ECI. Hence, there is no need to change these functions.
 *  + SOC_SAND_FAP10M have ECI, UpLink and I2C interfaces.
...
*INPUT:
*  SOC_SAND_DIRECT:
*   SOC_SAND_IN     uint32     *array      - array of 32bits to write to
*   SOC_SAND_IN     uint32     offset      - offset to write to
*   SOC_SAND_IN     uint32     size        - array size (in bytes)
*  SOC_SAND_INDIRECT:
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*INPUT:
*  SOC_SAND_DIRECT:
*   SOC_SAND_INOUT  uint32     *array        - array of 32bits to write to
*   SOC_SAND_IN     uint32     *base_address - the beginning of the chip address space
*   SOC_SAND_IN     uint32     offset      - offset to read from chip
*   SOC_SAND_IN     uint32     size        - number (of bytes) to read from chip
*  SOC_SAND_INDIRECT:
*OUTPUT:
*  SOC_SAND_DIRECT:
*    Non-zero in case of an error
...
/*
 * {
 * 'soc_sand_eci_write', 'soc_sand_eci_read' are default implementation of the physical interface:
 ...
 * To connect those BSP function use 'soc_sand_set_physical_access_hook'.
...
/*
 * {
 * 'soc_sand_eci_write_and_print', 'soc_sand_eci_read_and_print' are debug implementation
 *  of the physical interface.
 *  They print the offset/address and call 'soc_sand_eci_write', 'soc_sand_eci_read'.
 *
