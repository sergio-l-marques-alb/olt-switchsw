  ...
  )
{
    ...
    soc_reg_t
        reg = INVALIDr;
    uint64 
        reg64_val = COMPILER_64_INIT(0, 0),
        field64_val = COMPILER_64_INIT(0, 0);
    ...
    reg = EGQ_REG_01A9r;
    SOCDNX_IF_ERR_EXIT(soc_reg_get(unit, reg, REG_PORT_ANY, 0, &reg64_val));
    soc_reg64_field_set(unit, reg, &reg64_val, FIELD_0_47f, field64_val);
    SOCDNX_IF_ERR_EXIT(soc_reg_set(unit, reg, REG_PORT_ANY, 0, reg64_val));
  ...
  )
{
    ...
    rv = soc_reg_above_64_get(unit, lbg_reg, REG_PORT_ANY, lbg_id, lbg_data);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    *active_modems = soc_reg_above_64_field32_get(unit, lbg_reg, lbg_data, LBRG_N_ACTIVE_MODEMSf);
    ...
    rv = soc_reg_above_64_set(unit, lbg_reg, REG_PORT_ANY, lbg_id, lbg_data);
    SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    *lbg_id = SOC_TMC_LB_LBG_INVALID;
    ...
    for (lbg_id_tmp = 0; lbg_id_tmp < (SOC_TMC_LB_LBG_MAX+1); lbg_id_tmp++) {
        rv = soc_reg_above_64_get(unit, lbg_reg, REG_PORT_ANY, lbg_id_tmp, lbg_data);
        SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    reorder_config->expected_sequence_number = soc_reg_above_64_field32_get(unit, lbg_reg, lbg_data, LBRG_N_SEQUENCE_NUMf);
  ...
  )
{
    ...
    SOC_TMC_LB_VALUE_MAX_CHECK(reorder_config->expected_sequence_number, QAX_LB_RX_LBG_SEQUENCE_MAX, "expected_sequence_number");
    ...
    soc_reg_above_64_field32_set(unit, lbg_reg, lbg_data, LBRG_N_SEQUENCE_NUMf, reorder_config->expected_sequence_number);
    rv = soc_reg_above_64_set(unit, lbg_reg, REG_PORT_ANY, lbg_id, lbg_data);
    SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    case socLbStatusIsInSync:
        reg = ILB_LBRG_CONFIGURATIONr;
        rv = soc_reg_above_64_get(unit, reg, REG_PORT_ANY, lbg_id, reg_abv64);
        SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    case socLbModemStatsPkts:
        ...
        rv = soc_reg_above_64_get(unit, reg, REG_PORT_ANY, modem_id, reg_abv64);
        SOCDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    soc_sand_u64_devide_u64_long(&u64_1, SOC_TMC_LB_EGR_MODEM_SHAPER_RATE_GRANULARITY, &u64_2);
    hw_rate = u64_2.arr[0];
  ...
  )
{
    ...
    *hw_hdr_compensation = soc_mem_field32_get(unit, modem_table, modem_data, HDR_COMPENSATIONf);
    ...
    soc_sand_u64_multiply_longs(hw_rate, SOC_TMC_LB_EGR_MODEM_SHAPER_RATE_GRANULARITY, &u64_1);
  ...
  )
{
    ...
    case socLbStatusIsEmptyFifo:
        ...
        *status = (SHR_BITGET(fld_val32, lbg_id) != 0);
        break;
    default:
        if (status_type < socLbStatusCount) {
            return BCM_E_UNAVAIL;
  ...
  )
{
    ...
    switch (type) {
    case socLbModemStatsPkts:
        reg = EPNI_LBG_PACKET_COUNTER_MODEM_LPr;

        rv = soc_reg64_get(unit, reg, REG_PORT_ANY, modem_id, &reg_val64);
        SOCDNX_IF_ERR_EXIT(rv);
        *value = soc_reg64_field_get(unit, reg, reg_val64, LBG_PACKET_COUNTER_MODEM_LPf);
        break;
    case socLbModemStatsOctets:
        reg = EPNI_LBG_BYTES_COUNTER_MODEM_LPr;

        rv = soc_reg64_get(unit, reg, REG_PORT_ANY, modem_id, &reg_val64);
        SOCDNX_IF_ERR_EXIT(rv);
        *value = soc_reg64_field_get(unit, reg, reg_val64, LBG_BYTES_COUNTER_MODEM_LPf);            
        break;
    default:
        if (type < socLbModemStatsCount) {
            return BCM_E_UNAVAIL;
  ...
  )
{
    ...
    SOC_REG_ABOVE_64_CLEAR(lbg_data);
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_get(unit, lbg_reg, REG_PORT_ANY, lbg_id, lbg_data));
    soc_reg_above_64_field32_set(unit, lbg_reg, lbg_data, LBRG_N_OUTPUT_PORTf, phy_port); 
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_set(unit, lbg_reg, REG_PORT_ANY, lbg_id, lbg_data));
