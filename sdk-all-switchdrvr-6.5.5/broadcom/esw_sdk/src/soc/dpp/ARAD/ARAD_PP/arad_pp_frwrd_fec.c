  ...
  )
{
  ...
  res = arad_pp_lag_hash_func_to_hw_val(unit, lag_glbl_hash_info.hash_func_id, &lag_hw_val);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  /* As the LAG and ECMP enumerations are different the HW values are matched. */
  if(lag_hw_val == arad_pp_frwrd_fec_hash_index_to_hw_val(glbl_hash_info->hash_func_id)) {
      SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_FEC_ECMP_LAG_HASH_MATCH_ERR, 20, exit);
  }
  ...
  if (SOC_IS_JERICHO_PLUS_A0(unit)) {
      /* As the LAG and ECMP enumerations are different the HW values are matched. */
      if(lag_hw_val == arad_pp_frwrd_fec_hash_index_to_hw_val(glbl_hash_info->hash_func_id_second_hier)) {
            SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_FEC_ECMP_LAG_HASH_MATCH_ERR, 40, exit);
      }
  ...
  )
{
   ...
   */
  ihb_pinfo_fer_tbl_data.ecmp_lb_key_count = (SOC_IS_JERICHO_PLUS_A0(unit)) ? port_hash_info->nof_headers :
                                                                              (port_hash_info->nof_headers-1);
  ...
  )
{
   ...
   */
  port_hash_info->nof_headers =  (SOC_IS_JERICHO_PLUS_A0(unit)) ?  (uint8)(ihb_pinfo_fer_tbl_data.ecmp_lb_key_count) :
