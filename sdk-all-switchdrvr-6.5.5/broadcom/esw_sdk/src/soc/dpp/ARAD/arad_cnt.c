 ...
 SOC_SAND_CHECK_FUNC_RESULT(res, 4, exit);
  ...
  if(SOC_IS_QAX(unit))
  {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1260,  exit, ARAD_REG_ACCESS_ERR,
        soc_reg_above_64_field32_modify(unit, CGM_TM_CTR_PTR_SETTINGSr, src_core, 0, TM_CTR_A_MC_ONCEf, ((info->replicated_pkts == SOC_TMC_CNT_REPLICATED_PKTS_FRWRD)? 0 : 1)));        
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1260,  exit, ARAD_REG_ACCESS_ERR,
        soc_reg_above_64_field32_modify(unit, CGM_TM_CTR_PTR_SETTINGSr, src_core, 0, TM_CTR_B_MC_ONCEf, ((info->replicated_pkts == SOC_TMC_CNT_REPLICATED_PKTS_FRWRD)? 0 : 1)));                  
  }
  else
  {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1260,  exit, ARAD_REG_ACCESS_ERR,
