...
int soc_jer_write_masks_set (int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_SCH_INDIRECT_WR_MASKr(unit,SOC_CORE_ALL, wr_mask_val));     
...
uint32 jer_kbp_init_jer_interface(int unit)
{
    ...
    SOC_DPP_CORES_ITER(_SHR_CORE_ALL, core) {
        rv = handle_sand_result(arad_kbp_ilkn_interface_param_get(unit, core, &ilkn_port, &ilkn_num_lanes, &ilkn_rate, &ilkn_metaframe));
        if (rv == SOC_E_NONE) {
...
int soc_jer_init_hw_interfaces_set(int unit)
{
    ...
    if (init->drc_info.enable) { 
        ...
        if (!SOC_IS_QAX(unit)) {
            SOCDNX_IF_ERR_EXIT(soc_jer_dram_recovery_init(unit)); 
        }
    }
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    if(init->elk.enable) {
...
STATIC int soc_jer_init_functional_init(int unit)
{
    ...
    if (SOC_IS_QAX(unit)) {
        if (SOC_DPP_CONFIG(unit)->emulation_system != 3) { /* QAX overrides */
            SOCDNX_IF_ERR_EXIT(soc_qax_init_overrides(unit));
        }
        /*SOC_EXIT;*/ 
    ...
    if (SOC_IS_QAX(unit)) {
        SOCDNX_IF_ERR_EXIT(soc_qax_pdq_dtq_contexts_init(unit));
    } else {
        SOCDNX_IF_ERR_EXIT(soc_jer_ipt_contexts_init(unit));
    }
    ...
    */
    SOCDNX_IF_ERR_EXIT(WRITE_IDR_REASSEMBLY_TIMEOUTr(unit, 0x800));
    ...
    int unit)
{
    ...
    if (SOC_IS_JERICHO_B0(unit) || SOC_IS_QMX_B0(unit) || SOC_IS_JERICHO_PLUS_A0(unit)) {
