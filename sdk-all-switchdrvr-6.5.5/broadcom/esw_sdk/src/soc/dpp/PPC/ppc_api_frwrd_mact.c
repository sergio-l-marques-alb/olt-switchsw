/* $Id: ppc_api_frwrd_mact.c,v 1.15 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_frwrd_mact.c
*
  ...
  )
{
  ...
  /* SOC_PPC_FRWRD_MACT_ENTRY_KEY_VAL_clear(&(info->key_val)); */
  SOC_SAND_MAGIC_NUM_SET;
exit:
  SOC_SAND_VOID_EXIT_AND_SEND_ERROR(0, 0, 0);
}
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "destination = 0x%08x.\n"), info->destination));
  LOG_CLI((BSL_META_U(unit,
                      "flow_label = 0x%08x%08x.\n"), info->flow_label[1], info->flow_label[0])); /* Compile doesn't want to cast to long long. */
