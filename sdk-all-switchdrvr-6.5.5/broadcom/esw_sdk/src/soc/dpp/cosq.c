  ...
  )
{
  ...
  if(SOC_IS_QAX(unit)){
    flow_id = BCM_COSQ_FLOW_ID_QAX_ADD_OFFSET(flow_id);
  }
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_flow_and_up_info_get,(unit, core_idx, flow_id, reterive_status, flow_and_up_info));
  SOCDNX_SAND_IF_ERR_RETURN(ret);

  if(SOC_IS_QAX(unit)){
    flow_and_up_info->sch_consumer.sub_flow[0].id = BCM_COSQ_FLOW_ID_QAX_SUB_OFFSET(flow_and_up_info->sch_consumer.sub_flow[0].id);
    ...
    if(flow_and_up_info->sch_consumer.sub_flow[1].is_valid){
        flow_and_up_info->sch_consumer.sub_flow[1].id -= QAX_FLOW_ID_OFFSET;
