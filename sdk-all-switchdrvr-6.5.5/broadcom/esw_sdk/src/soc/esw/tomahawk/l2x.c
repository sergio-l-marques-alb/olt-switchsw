/*
 * $Id: l2x.c, 2014/01/14 yiwenz Exp $
...
STATIC int
_soc_th_l2_bulk_age_entries_delete(int unit)
{
    int rv;
    ...
    if (SOC_CONTROL(unit)->l2x_mode == L2MODE_FIFO) {
        ...
        soc_mem_field32_set(unit, L2_BULKm, &new_mask, VALIDf, 1);
    } else {
...
STATIC int
_soc_th_l2_bulk_age_entries_hit_clear(int unit)
{
    int rv;
    l2_bulk_entry_t match_mask;
    l2_bulk_entry_t match_data;
...
_soc_th_l2_bulk_age(void *unit_vp)
{
    ...
    while((interval = soc->l2x_age_interval) != 0) {
        ...
        rv = _soc_th_l2_bulk_age_entries_delete(unit);

        if (!SOC_SUCCESS(rv)) {
            goto cleanup_exit;
        }
        rv = _soc_th_l2_bulk_age_entries_hit_clear(unit);
...
age_delay:
        rv = -1; /* timeout */
        if (interval > 4294) {
            m = (interval / 4294) * 1000;
            r = (interval % 4294) * 1000000;
            for (c = 0; c < m; c++) {
                rv = sal_sem_take(soc->l2x_age_notify, 4294000);
