...
static int
soc_esw_portctrl_core_config_update(int unit, int lport, int pport, uint32_t xphy_addr, int phy_idx) {
    ...
    for (i = 0; i < 4; i++) {
        lane_map.lane_map_rx[i] = ((rx_polarity >> (i*4)) & 0xf);
        lane_map.lane_map_tx[i] = ((tx_polarity >> (i*4)) & 0xf);
    }
...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag)
{
    ...
    if ((init_flag == PORTMOD_PORT_ADD_F_INIT_PASS2)  || (!init_flag)){
        ...
        SOC_IF_ERROR_RETURN(soc_esw_portctrl_frame_max_set(
            unit, pport, SOC_INFO(unit).max_mtu - 4));
...
soc_portctrl_phy_name_get(int unit, int port, char *phy_name)
{
    ...
    SOC_PBMP_ITER(diag_info.phys, phy){
        range_start = phy;
        break;
    }
