...
#define PM4x10_WB_BUFFER_VERSION        (5)
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
   ...
   /* initialize num of phys for each lane */
    ...
    if(!SOC_WARM_BOOT(unit)){
        ...
        for(i = 0 ; i < PM4X10_LANES_PER_CORE ; i++){
            nof_phys = 1;
            PM4x10_PORT_NUM_PHYS_SET(unit, pm_info, &nof_phys, i);
        }
...
STATIC
int _pm4x10_ext_phy_interface_check(int unit, int port,
                               ...
                               soc_port_if_t interface, int *is_valid)
{
    switch (interface_config->speed) {
        ...
        case 10000:
            if ((SOC_PORT_IF_ZR == interface) &&
                (interface_config->port_num_lanes == 1)) {
                *is_valid = 1;
            }
            break;
        default:
...
STATIC
int _pm4x10_interface_check(int unit, int port,
                               const portmod_port_interface_config_t *interface_config,
                               soc_port_if_t interface, int *is_valid)
{
    int is_higig;
    ...
    switch (interface_config->speed) {
        ...
        case 100000:
            if ((SOC_PORT_IF_CR10 == interface) ||
                ...
                (SOC_PORT_IF_CAUI_C2C == interface) ||
                (SOC_PORT_IF_CAUI_C2M == interface) ||
                (SOC_PORT_IF_VSR == interface)) {
                /* Support for both 4 lanes(Gear box mode) and 10 lanes (pass-through)
                with CAC2C, CAC2M, VSR for EXT PHY. */
                *is_valid = 1;
            }
            break;
        case 106000:
...
STATIC
int _pm4x10_ext_phy_default_interface_get(int unit, int port,
                               const portmod_port_interface_config_t *interface_config,
                               soc_port_if_t *interface)
{
    switch (interface_config->speed) {
                                ...
                                phymod_interface_t *interface_type)
{
    ...
    _SOC_IF_ERR_EXIT(_pm4x10_interface_check(unit, port, config, interface,
                                             &is_valid));
    if (nof_phys > 1) {
        /* EXT PHY interface */
        _SOC_IF_ERR_EXIT
            (_pm4x10_ext_phy_interface_check(unit, port, config, interface,
                                                 &is_valid));
    }
    ...
    if (!is_valid) {
        ...
        if (nof_phys > 1) {
            /* EXT PHY interface */
            _SOC_IF_ERR_EXIT
                (_pm4x10_ext_phy_default_interface_get(unit, port, config, &interface));
        }
    }

    _SOC_IF_ERR_EXIT(PM4x10_INTERFACE_TYPE_SET(unit, pm_info, interface,
                                               port_index));
                    ...
                    if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                        ...
                        if( !core_probed ){
                            ...
                            if(SOC_E_NONE != (_pm4x10_core_probe(unit,port, pm_info, &core_access, &probe, 1))) {
                                ...
                                PM4x10_PORT_NUM_PHYS_SET(unit, pm_info, &(PM_4x10_INFO(pm_info)->nof_phys[temp]), temp);
                                      ...
                                      portmod_port_interface_config_t* config)
{
    ...
    _SOC_IF_ERR_EXIT(PM4x10_IS_INTERFACE_RESTORE_GET(unit, pm_info,
    ...
    if(is_interface_restore) {
        ...
        if ((phy_interface_config.data_rate == 40000) &&
             PHYMOD_INTF_MODES_FIBER_GET(&phy_interface_config)) {
        ...
        /* portmod_port_phychain_interface_config_get() cannot distinguish among 100G SR10//LR10 KR10/CAUI.
       */
        if (phy_interface_config.data_rate >= 100000) {
            switch(phy_interface_config.interface_type) {
              case phymodInterfaceCAUI:
                if (prev_phymod_if == phymodInterfaceKR10)
                  phy_interface_config.interface_type = phymodInterfaceKR10; 
                break;
              case phymodInterfaceSR10:
                if (prev_phymod_if == phymodInterfaceLR10)
...
{   
    ...
    if (!SOC_WARM_BOOT(unit)) {
        PM4x10_PORT_LANE_CONNECTION_DB_SET(unit, pm_info, lane_conn, phyn-1, lane_index); 
        ...
        if (nof_phys <= phyn) {
            nof_phys = phyn + 1;
            PM4x10_PORT_NUM_PHYS_SET(unit, pm_info, &nof_phys, lane_index);
...
int pm4x10_xphy_lane_detach_from_pm (int unit, pm_info_t pm_info, int iphy, int phyn, portmod_xphy_lane_connection_t* lane_conn)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        PM4x10_PORT_LANE_CONNECTION_DB_SET(unit, pm_info, candidate_lane_conn, phyn-1, lane_index);
