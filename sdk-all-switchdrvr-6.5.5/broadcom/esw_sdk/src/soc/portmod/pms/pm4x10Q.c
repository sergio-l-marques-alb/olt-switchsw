...
int pm4x10Q_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if (phy_acc_port == port) { /* only done once per phy lane */
        ...
        _SOC_IF_ERR_EXIT(pm4x10_port_lane_map_get(unit, port, PM_4x10Q_INFO(pm_info)->pm4x10, &lane_map));
        orig_lane_map = lane_map;
        for (i = 0; i < PM4X10Q_LANES_PER_CORE; ++i) {
            idx = lane_map.lane_map_rx[i];
            lane_map.lane_map_tx[idx] = orig_lane_map.lane_map_tx[i];
            lane_map.lane_map_rx[i] = i;
        }
        _SOC_IF_ERR_EXIT(pm4x10_port_lane_map_set(unit, port, PM_4x10Q_INFO(pm_info)->pm4x10, &lane_map));
...
int pm4x10Q_port_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_port_phychain_link_status_get(phy_access, nof_phys, (uint32_t*) link));
