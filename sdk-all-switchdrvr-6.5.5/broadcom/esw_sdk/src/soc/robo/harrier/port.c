    ...
    switch (property) {
        ...
        case DRV_PORT_PROP_ENABLE_DROP_NON1Q:
            if ((rv = REG_READ_VLAN_CTRL3r(
                unit, (void *)&vlan_ctrl3_64)) < 0) {
                return rv;
            }
            soc_VLAN_CTRL3r_field_get(unit, (void *)&vlan_ctrl3_64, 
                EN_DROP_NON1Qf, &temp);
            ...
            soc_VLAN_CTRL3r_field_set(unit, (void *)&vlan_ctrl3_64, 
                EN_DROP_NON1Qf, &temp);
        ...
        case DRV_PORT_PROP_PAUSE_FRAME_BYPASS_TX:
            ...
            if ((rv = (DRV_SERVICES(unit)->reg_read)
                (unit, addr, (void *)&reg_value64, length)) < 0) {
                return rv;
            }
            (DRV_SERVICES(unit)->reg_field_get)
                (unit, reg_index, (void *)&reg_value64, fld_index, &temp);
            SOC_PBMP_WORD_SET(pbmp, 0, temp);
            if (enable) {
                SOC_PBMP_PORT_ADD(pbmp, port);
            } else {
        ...
        case DRV_PORT_PROP_DTAG_ISP_PORT:
            if ((rv = REG_READ_ISP_SEL_PORTMAPr(
                unit,(void *)&reg_value64)) < 0) {
                return rv;
            }

            soc_ISP_SEL_PORTMAPr_field_get(unit, (void *)&reg_value64,
                ISP_PORT_PBMPf, &temp);
            SOC_PBMP_WORD_SET(pbmp, 0, temp);
            ...
            */
            ...
            temp = SOC_PBMP_WORD_GET(pbmp, 0);
            soc_ISP_SEL_PORTMAPr_field_set(unit, (void *)&reg_value64,
                ISP_PORT_PBMPf, &temp);
...
drv_harrier_port_set(int unit, soc_pbmp_t bmp, uint32 prop_type, uint32 prop_val)
{
    ...
    switch (prop_type) {
        ...
        case DRV_PORT_PROP_SFLOW_INGRESS_RATE:
            ...
            soc_INGRESS_RMONr_field_get(unit, (void *)&reg_value64,
            ...
            soc_INGRESS_RMONr_field_set(unit, (void *)&reg_value64,
                EN_INGRESS_PORTMAPf, &temp);
        ...
        case DRV_PORT_STATUS_LINK_UP:
            ...
            if (port_lb_phy){
                ...
                DRV_REG_FIELD_GET(unit, INDEX(LNKSTSr), 
                        (void *)&reg_value64, INDEX(LNK_STSf), 
                        &temp);
                SOC_PBMP_WORD_SET(pbmp, 0, temp);
                if (SOC_PBMP_MEMBER(pbmp, port)) {
                    *val = TRUE;
