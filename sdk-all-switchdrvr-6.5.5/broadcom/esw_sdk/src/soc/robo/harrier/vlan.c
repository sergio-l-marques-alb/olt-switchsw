...
drv_harrier_port_vlan_set(int unit, uint32 port, soc_pbmp_t bmp)
{
    ...
    SOC_IF_ERROR_RETURN(
        REG_READ_PORT_EGCTLr(unit, port, (void *)&reg_value64));
    temp = SOC_PBMP_WORD_GET(bmp, 0);
    soc_PORT_EGCTLr_field_set(unit, (void *)&reg_value64, 
        PORT_EGRESS_ENf, &temp);
    SOC_IF_ERROR_RETURN(
        REG_WRITE_PORT_EGCTLr(unit, port, (void *)&reg_value64));
...
drv_harrier_port_vlan_get(int unit, uint32 port, soc_pbmp_t *bmp)
{
    ...
    SOC_IF_ERROR_RETURN(
        REG_READ_PORT_EGCTLr(unit, port, (void *)&reg_value64));
    soc_PORT_EGCTLr_field_get(unit, (void *)&reg_value64, 
        PORT_EGRESS_ENf, &temp);
                ...
                soc_pbmp_t bmp, uint32 val)
{
    ...
    SOC_IF_ERROR_RETURN(DRV_REG_READ(unit, 
            reg_addr, (void *)&reg_value64, reg_len));
    SOC_IF_ERROR_RETURN(DRV_REG_FIELD_GET(unit, 
            reg_index,(void *)&reg_value64, fld_index, &temp));
