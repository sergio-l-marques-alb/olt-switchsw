    ...
    uint8* mac, uint32 bpdu_idx)
{
    ...
    switch (mac_type) {
        ...
        case DRV_MAC_SECURITY_ADD:                                  
            rv = _drv_dino16_sec_mac_set
                (unit, SOC_PBMP_WORD_GET(pbmp, 0), (void *)&mac_field, mcast);
            return rv;
    ...
    SOC_IF_ERROR_RETURN(DRV_REG_FIELD_SET
        (unit, reg_index, (void *)&reg_v64, fld_index, (void *)&mac_field));
    if ((rv = DRV_REG_WRITE(unit, reg_addr, 
            (void *)&reg_v64, reg_len)) < 0) {
       return rv;
    }
    ...
    if ((mac_type == DRV_MAC_MULTIPORT_0) || 
        ...
        /* enable MULTIPORT Address 1 and 2 register
          ...
          */            
        if ((rv = REG_READ_GARLCFGr(unit, (void *)&reg_v64)) < 0) {
            return rv;
        }
        reg_v32 = 1;
        soc_GARLCFGr_field_set(unit, (void *)&reg_v64, 
            MPORT_ADDR_ENf, (uint32 *)&reg_v32);
        if ((rv = REG_WRITE_GARLCFGr(unit, (void *)&reg_v64)) < 0) {
            return rv;
        }
