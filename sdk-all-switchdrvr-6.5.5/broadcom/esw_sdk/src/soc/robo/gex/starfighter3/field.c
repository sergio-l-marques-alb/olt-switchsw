...
/* Policer _Mode */
#define FIELD_53134_POLICER_MODE_RFC2698    (0x0)
#define FIELD_53134_POLICER_MODE_RFC4115    (0x1)
#define FIELD_53134_POLICER_MODE_MEF        (0x2)
#define FIELD_53134_POLICER_MODE_DISABLE    (0x3)


STATIC int
_drv_sf3_fp_policer_support_check(int unit, int stage_id, drv_policer_config_t *policer_cfg)
{
    int rv;
...
STATIC int
_drv_sf3_fp_policer_config(int unit,  int stage_id, void *entry, drv_policer_config_t *policer_cfg)
{
    ...
    switch(policer_cfg->mode){
        case drvPolicerModeCommitted:
            cir_en = 1;
            mode = FIELD_53134_POLICER_MODE_RFC4115;
            action = 1;
            break;
        case drvPolicerModePassThrough:
            mode = FIELD_53134_POLICER_MODE_DISABLE;
            break;
        case drvPolicerModeSrTcm: /* RFC 2697 */
            policer_cfg->pkbits_sec = 0; /* clear it */
            cir_en = 1;
            eir_en = 1;
            mode = FIELD_53134_POLICER_MODE_MEF;
            coupling = 1;
            break;
        case drvPolicerModeTrTcm: /* RFC 2698 */
            cir_en = 1;
            eir_en = 1;
            mode = FIELD_53134_POLICER_MODE_RFC2698;
            action = 1;
            break;
        case drvPolicerModeTrTcmDs: /* RFC 4115 */
            cir_en = 1;
...
int
drv_sf3_fp_policer_control(int unit,  int stage_id, int op, void *entry, drv_policer_config_t *policer_cfg)
{
    int rv = SOC_E_NONE;
    drv_cfp_entry_t *drv_entry;
    uint32  temp;
    ...
    switch (op) {
        case DRV_FIELD_POLICER_MODE_SUPPORT:
            rv = _drv_sf3_fp_policer_support_check(unit, stage_id, policer_cfg);
            break;
        case DRV_FIELD_POLICER_CONFIG:
            rv =  _drv_sf3_fp_policer_config(unit, stage_id, entry, policer_cfg);
            break;
        case DRV_FIELD_POLICER_FREE:
            drv_entry = (drv_cfp_entry_t *)entry;
            sal_memset(drv_entry->meter_data, 0, sizeof(drv_entry->meter_data));
            temp = FIELD_53134_POLICER_MODE_DISABLE;
...
int
_drv_sf3_fp_counter_mode_support(int unit, int param0, void *mode)
{
    int policer_valid;
...
int
drv_sf3_fp_stat_type_get(int unit, int stage_id, drv_policer_mode_t policer_mode,
...
int
_drv_sf3_fp_stat_mode_support(int unit, int stage_id, drv_policer_mode_t policer_mode, void *mode)
{
    ...
    type1 = type2 = type3 =-1;
    rv = drv_sf3_fp_stat_type_get(unit, stage_id, policer_mode,
        f_stat, &type1, &type2, &type3);
    return rv;
}

int
drv_sf3_fp_stat_support_check(int unit, int stage_id, int op, int param0, void *mode)
{

    switch (op) {
        case _DRV_FP_STAT_OP_COUNTER_MODE:
            return _drv_sf3_fp_counter_mode_support(unit, param0, mode);
        case _DRV_FP_STAT_OP_STAT_MODE:
            return _drv_sf3_fp_stat_mode_support(unit, stage_id, param0, mode);
    }
    return SOC_E_NONE;
}

int
drv_sf3_fp_action_conflict(int unit, int stage_id,
