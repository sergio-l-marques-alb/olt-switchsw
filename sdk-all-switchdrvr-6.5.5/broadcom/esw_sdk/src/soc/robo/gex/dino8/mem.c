        ...
        uint32 *entry_1, uint32 flags)
{
    ...
    if (flags & DRV_MEM_OP_SEARCH_DONE) {
        ...
        for (count = 0; count < SOC_TIMEOUT_VAL; count++) {
            ...
            soc_ARLA_SRCH_RSLT_MACVIDr_field_get(unit, (void *)&entry0, 
            ...
            if (valid) {
                /* MAC Address */
                soc_ARLA_SRCH_RSLT_MACVIDr_field_get(unit, (void *)&entry0, 
                    ARLA_SRCH_MACADDRf, (void *)&temp_mac_field);
            ...
            if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, 
                DRV_MEM_FIELD_MAC, entry, (void *)&temp_mac_field)) < 0) {
                goto mem_search_valid_get;
            }
        ...
        uint32 *entry, uint32 *entry_1, uint32 flags, int *index)
{
    ...
    if ((flags & DRV_MEM_OP_SEARCH_DONE) || 
        ...
        SOC_IF_ERROR_RETURN(DRV_MEM_FIELD_GET
            ...
            (uint32 *)&l2_arl_entry, (void *)&rw_mac_field));
        SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, rw_mac_field);
        
        SOC_IF_ERROR_RETURN(DRV_MEM_FIELD_SET
            (unit, DRV_MEM_ARL, DRV_MEM_FIELD_MAC, 
             entry, (void *)&temp_mac_field));
    ...
    /* delete by MAC */    
        ...
        if (flags & DRV_MEM_OP_REPLACE) {
            ...
            if (temp_valid1) {
                ...
                COMPILER_64_ZERO(temp_mac_field);
                soc_ARLA_MACVID_ENTRY0r_field_get(unit, (void *)&entry0, 
            ...
            if (temp_valid2) {
                ...
                COMPILER_64_ZERO(temp_mac_field);
                soc_ARLA_MACVID_ENTRY1r_field_get(unit, (void *)&entry1, 
                    ARL_MACADDRf, (void *)&temp_mac_field);
                SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, temp_mac_field);
                soc_ARLA_MACVID_ENTRY1r_field_get(unit, (void *)&entry1, 
                    VID_Rf, &vid2);
...
{   
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mem_id == INDEX(L2_ARLm)) {
            if ((rv = REG_READ_MEM_ARL_LOWr(unit, (void *)&mem_data0)) < 0) {
                goto mem_read_exit;
            }
            if ((rv = REG_READ_MEM_ARL_HIGHr(unit, (void *)&mem_data1)) < 0) {
                goto mem_read_exit;
            }

            soc_MEM_ARL_LOWr_field_get(unit, (void *)&mem_data0, 
                MACADDRf, (void *)&temp_mac_field);                  
            SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, temp_mac_field);                    
            if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, DRV_MEM_FIELD_MAC, 
                (uint32 *)arl_mem, (void *)&temp_mac_field)) < 0) {
                goto mem_read_exit;
            }
            ...
            soc_MEM_ARL_HIGHr_field_get(unit, (void *)&mem_data1, 
                VID_Rf, &temp);                                
            if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, 
            ...
            if (temp_mac_addr[0] & 0x01) {
                ...
                soc_MEM_ARL_LOWr_field_get(unit, (void *)&mem_data0, 
                    PORTID_Rf, &temp);                                
                if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, 
                    DRV_MEM_FIELD_SRC_PORT, (void *)arl_mem, &temp)) < 0) {
                    goto mem_read_exit;
                }               
            }
            arl_mem ++;
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mem_id == INDEX(L2_ARLm)) {
            ...
            soc_MEM_ARL_LOWr_field_set(unit, (void *)&mem_data0, 
                MACADDRf, (void *)&temp_mac_field);
            SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, temp_mac_field);            
            ...
            soc_MEM_ARL_HIGHr_field_set(unit, (void *)&mem_data1, 
                VID_Rf, &temp);
            ...
            if (temp_mac_addr[0] & 0x01) {
                ...
                soc_MEM_ARL_LOWr_field_set(unit, (void *)&mem_data1, 
                    AGEf, &temp); 
            ...
            if ((rv = REG_WRITE_MEM_DATA_LOWr(unit, (void *)&mem_data0)) < 0) {
                goto mem_write_exit;
            }              
            if ((rv = REG_WRITE_MEM_DATA_HIGHr(unit, (void *)&mem_data1)) < 0) {
                goto mem_write_exit;
            }
            arl_mem ++;
        } 
...
drv_dino8_mem_insert(int unit, uint32 mem, uint32 *entry, uint32 flags)
{
    ...
    soc_ARLA_MACVID_ENTRY0r_field_set(unit, (void *)&entry_reg, 
        VID_Rf, &vid);

    /* MAC Address */
    if ((rv = DRV_MEM_FIELD_GET(unit, DRV_MEM_ARL, 
            DRV_MEM_FIELD_MAC, entry, (void *)&mac_field)) < 0) {
        goto mem_insert_exit;
    }
