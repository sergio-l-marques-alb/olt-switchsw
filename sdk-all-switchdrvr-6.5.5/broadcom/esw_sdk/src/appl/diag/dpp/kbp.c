...
cmd_dpp_kbp_test_ipv6_performance_measure(int unit, args_t *a)
{
    ...
    for(loopCounter = 0 ; loopCounter < (IPv6Number/(10*1000)) ; loopCounter++) /*measure performance for each 10k entries alone*/
    {
        startTime = sal_time_usecs();

        for(innerLoopCounter = 0 ; innerLoopCounter < 10*1000 ; innerLoopCounter++)
        {
            retVal = bcm_l3_route_add(0, &routesArray[innerLoopCounter + 10*1000*loopCounter]);
            if(retVal != 0)
            {
                printf("error occurred while adding route, index %d ,retVal %d\n",innerLoopCounter + 10*1000*loopCounter,retVal);
                return BCM_E_FAIL;
            }
        }
        ...
        printf("    %3dst 10K routes add done in : %d us\n",loopCounter+1,endTime-startTime);
        avgPerformanceArray[loopCounter] = endTime-startTime;
    }

    for(loopCounter = 0 ; loopCounter < (IPv6Number/(10*1000)) ; loopCounter++){ /*measure average performance*/
        sum += avgPerformanceArray[loopCounter];
        num++;
    }
    avgPerformance = sum/num;
    if(avgPerformance > 2000000.0){
        printf("ERROR : the average time for bcm_l3_route_add is higher than 2000000 us for 10k entries : %f us\n\n",avgPerformance);
        return BCM_E_FAIL;
    }
    else{
        printf("the average time for 10k bcm_l3_route_add : %f us\n\n",avgPerformance);
    }
    ...
    for(loopCounter = 0 ; loopCounter < (IPv6Number/(10*1000)) ; loopCounter++)
    {
        startTime = sal_time_usecs();
        ...
        printf("    %3dst 10K routes update done in : %d us\n",loopCounter+1,endTime-startTime);
        avgPerformanceArray[loopCounter] = endTime-startTime;
    }
    ...
    avgPerformance = sum/num;
    if(avgPerformance > 1500000.0){
        printf("ERROR : the average time for bcm_l3_route_add (Update) is higher than 1500000 us for 10k entries : %f us\n\n",avgPerformance);
        return BCM_E_FAIL;
    }
    else{
        printf("the average time for 10k bcm_l3_route_add (Update) : %f us\n\n",avgPerformance);
    }
    ...
    for(loopCounter = 0 ; loopCounter < (IPv6Number/(10*1000)) ; loopCounter++)
    {
        ...
        for(innerLoopCounter = 0 ; innerLoopCounter < 10*1000 ; innerLoopCounter++)
        {
            retVal = bcm_l3_route_delete(0, &routesArray[innerLoopCounter + 10*1000*loopCounter]);
            if(retVal != 0)
            {
        ...
        printf("    %3dst 10K routes delete done in : %d us\n",loopCounter+1,endTime-startTime);
        avgPerformanceArray[loopCounter] = endTime-startTime;
    }
    ...
    avgPerformance = sum/num;
    if(avgPerformance > 2500000.0){
        printf("ERROR : the average time for bcm_l3_route_delete is higher than 2500000 us for 10k entries : %f us\n\n",avgPerformance);
        return BCM_E_FAIL;
    }
    else{
...
cmd_dpp_kbp_test_ipv4_dc_lem(int unit, args_t *a)
{
	...
	uint32  lem_entry_num = 750000; /* 750K */
    ...
    if (ARG_CNT(a) > 0) {
		...
		parse_table_add(&pt,"LemEntryNum"       ,PQ_INT , (void *) (750000)             , &lem_entry_num        , NULL);
