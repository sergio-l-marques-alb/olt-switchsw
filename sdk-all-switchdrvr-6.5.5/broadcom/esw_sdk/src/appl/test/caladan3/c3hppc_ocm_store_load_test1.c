/* $Id: c3hppc_ocm_store_load_test1.c,v 1.8 Broadcom SDK $
...
static int g_nLrpStorePort;



int
c3hppc_ocm_store_load_test1__init(c3hppc_test_info_t *pc3hppcTestInfo, void *pUserData)
{
  int rc, nCopInstance;

  g_nLrpStorePort = ( sal_rand() % 2 ) ? 9 : 0;

  c3hppc_populate_with_defaults( pc3hppcTestInfo->nUnit, &(pc3hppcTestInfo->BringUpControl) );
  if ( g_nLrpStorePort == 9 ) {
    strcpy(pc3hppcTestInfo->BringUpControl.sUcodeFileName, "ocm_store_load_test1_p9.oasm");
  } else {
    strcpy(pc3hppcTestInfo->BringUpControl.sUcodeFileName, "ocm_store_load_test1_p0.oasm");
  }
...
int
c3hppc_ocm_store_load_test1__run(c3hppc_test_info_t *pc3hppcTestInfo, void *pUserData)
{
   ...
   * 23:16 --> contains the number of iterations necessary to fill up 9x16K OCM blocks
   * 15:8  --> contains the running count(incremented) of iterations necessary to store (only P0) to 9x16K OCM blocks
   *  7:0  --> contains the running count(incremented) of iterations necessary to load from 9x16K OCM blocks
   *****************************************************************************************************************************/
  for ( nIndex = 0; nIndex < g_nFlowTableSize; ++nIndex ) {
    /* 144(9x16) iterations -- see comment below for explanation */
    g_pFlowTable[nIndex].uData[0] = 0x00900000;
  }
...
int
c3hppc_ocm_store_load_test1__done(c3hppc_test_info_t *pc3hppcTestInfo, void *pUserData)
{
  ...
  uOcmEntryExpect.uData[0] = g_pFlowTable[0].uData[0] | 
                             ((g_pFlowTable[0].uData[0] >> 8) & 0xff00) |
                             ((g_pFlowTable[0].uData[0] >> 16) & 0xff);
  ...
  pOcmBlock = (c3hppc_64b_ocm_entry_template_t *) soc_cm_salloc(pc3hppcTestInfo->nUnit,
                                                                ...
                                                                "ocm_block");
  nSegment = C3HPPC_OCM_NUM_OF_TRANSFER_SIZES + 1;
  nOcmPort = c3hppc_ocm_map_lrp2ocm_port(g_nLrpStorePort);
  for ( nLrpPort = 0; nLrpPort < 1; ++nLrpPort ) {
    cli_out("\nChecking OCM contents for LRP Port %d\n", g_nLrpStorePort);
    c3hppc_ocm_dma_read_write( pc3hppcTestInfo->nUnit, nOcmPort, nSegment,
                               0, (nDmaBlockSize-1), 0, pOcmBlock->uData );
    nErrorCnt = 0;
    for ( nIndex = 0; nIndex < nDmaBlockSize; ++nIndex ) {
      uOcmEntryExpect.uData[1] = 0xaaaaa000 | (nIndex & 0x3ff);
      uOcmEntryExpect.uData[0] = 0xbbbbb000;
      if ( ((nIndex >> 10) % 8) != 0 ) {
      ...
      if ( pOcmBlock[nIndex].uData[1] != uOcmEntryExpect.uData[1] ||
        ...
        if ( nErrorCnt < 256 ) {
          cli_out("LrpPort%d OCM entry[%d] --> Actual: 0x%08x_%08x   Expect: 0x%08x_%08x\n",
