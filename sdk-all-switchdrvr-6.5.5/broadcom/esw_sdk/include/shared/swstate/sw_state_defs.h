...
#include <soc/dnx/dnx_sw_state.h>
...
#define SW_STATE_DO_JOURNALING(unit, flags)\
    (SOC_CR_ENABALED(unit) && SW_STATE_DO_AUTOSYNC(flags & SW_STATE_ALLOW_AUTOSYNC))
...
#define SHR_SW_STATE_SET(unit, location, input, type, flags, dbg_string)\
    shr_sw_state_memcpy(unit, (uint8 *) &location, (uint8 *) &input, sizeof(type), flags, 0);
#define SHR_SW_STATE_MEMSET(unit, location, input, type, flags, dbg_string)\
    shr_sw_state_memcpy(unit,(uint8 *) &location, (uint8 *) input, sizeof(type), flags, 0);
...
#define SW_STATE_MEMWRITE(unit, input, location, offset, length, flags, debug_str)\
    shr_sw_state_memcpy(unit,(uint8 *) (location + offset), (uint8 *) input, length, flags, 0);
