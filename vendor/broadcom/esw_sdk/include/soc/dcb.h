/*
 * $Id: dcb.h 1.28.20.2 Broadcom SDK $
...
typedef struct {
    ...
    soc_rx_reason_t *rx_egr_reason_map;
    void        (*init)(dcb_t *dcb);
    ...
    uint32      (*rx_untagged_get)(int unit, dcb_t *dcb);
    ...
    uint32      (*rx_egr_to_cpu_get)(dcb_t *dcb);
...
#define SOC_DCB_RX_UNTAGGED_GET(_u, _dcb)       \
        SOC_DCB(_u).rx_untagged_get(_u, _dcb)
...
#define SOC_DCB_RX_EGR_TO_CPU_GET(_u, _dcb)           \
        SOC_DCB(_u).rx_egr_to_cpu_get(_dcb)
