...
#define L2CP_DA_LSBS_BITMAP_EGRESS_SET(egress_profile_data, da_lsbs, handle_type)        \
  do {                                                                                   \
	  ...
	  if (handle_type == SOC_PPD_VSI_L2CP_HANDLE_TYPE_TUNNEL) {                      \
...
#define L2CP_DA_LSBS_BITMAP_INGRESS_SET(ingress_profile_data, da_lsbs, handle_type)      \
  do {                                                                                   \
	  ...
	  else if (handle_type == SOC_PPD_VSI_L2CP_HANDLE_TYPE_PEER) {                      \
	  ...
	  else if (handle_type == SOC_PPD_VSI_L2CP_HANDLE_TYPE_DROP) {                      \
...
typedef struct bcm_dpp_prog_trap_s {
    SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER qual;
...
extern SOC_PPD_FRWRD_MACT_ENTRY_KEY                
     *_bcm_l2_traverse_mact_keys[BCM_LOCAL_UNITS_MAX];
extern SOC_PPD_FRWRD_MACT_ENTRY_VALUE              
...
extern SOC_PPD_LLP_SA_AUTH_MAC_INFO              
