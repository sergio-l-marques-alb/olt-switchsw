...
/* $Id: sand_sorted_list.h,v 1.5 Broadcom SDK $
 * Reset the sorted list iterator to point to the beginning of the sorted list
 */
#define SOC_SAND_SORTED_LIST_ITER_BEGIN(list) \
  ((list)->init_info.list_size)
/*
 * Check if the sorted list iterator arrives to the end of the sorted list
 */
#define SOC_SAND_SORTED_LIST_ITER_END(list)  \
  ((list)->init_info.list_size + 1)
 ...
 */
typedef struct SOC_SAND_SORTED_LIST_INFO_T*  SOC_SAND_SORTED_LIST_PTR;
/*
...
typedef struct {
  ...
  */
  ...
  SOC_SAND_SORTED_LIST_KEY_CMP cmp_key_fun;

 /*
  * saving warm boot variable index for get/set of sorted_list variables.
  */
  int32 wb_var_index;
 /*
...
{
 /*
  * includes the full keys to sort the list according.
  */
  uint8 *keys;
 /*
  ...
  */
  uint8 *prev;
 /*
  * the data to hold in the linked list.
  */
  uint8 *data;
 /*
  ...
  */
  uint32 ptr_size;
 /*
  * available memory of the nodes, for efficient manipulation.
  */
...
/*typedef SOC_SAND_SORTED_LIST_INFO* SOC_SAND_SORTED_LIST_PTR;*/
...
*     information to use in sort to create the sorted list (size, data size..)
* REMARKS:
*   None.
* RETURNS:
*   OK or ERROR indication.
