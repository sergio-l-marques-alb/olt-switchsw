...
PRIVATE sbStatus_t  
sbFe2000PolicerTimerTickGet(sbFe2000Pm_t *pPm, sbFe2000PmPolicerRate_t rate, uint32 *pTimerTick);
...
PRIVATE sbStatus_t
sbFe2000PmLocProfileMemoryRead(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfFe2000PmProfileMemory_t *pProfile);

PRIVATE sbStatus_t
sbFe2000XtPmLocProfileMemoryRead(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfC2PmProfileMemory_t *pProfile);

PRIVATE sbStatus_t 
sbFe2000PmProfileAlloc(sbFe2000Pm_t *pPm, sbZfFe2000PmProfileMemory_t *pProfile, uint32 *pIndex);

PRIVATE sbStatus_t 
sbFe2000XtPmProfileAlloc(sbFe2000Pm_t *pPm, sbZfC2PmProfileMemory_t *pProfile, uint32 *pIndex);
...
PRIVATE sbStatus_t 
sbFe2000PmIEEECodingGet(uint32 uBytes, uint32 *pEncodedValue);


PRIVATE sbStatus_t
sbFe2000PmGroupGet(sbFe2000Pm_t *pPm, sbFe2000PmPolicerRate_t rate, uint32 *pGroup);
...
PRIVATE sbStatus_t
sbFe2000PmPolicerRefreshSet(sbFe2000Pm_t *pPm, uint32 uPolicerId, uint32 bEnable);

PRIVATE sbStatus_t
sbFe2000PolicerGroupRateGet(sbFe2000Pm_t *pPm, uint32 uTimerTick, sbFe2000PmPolicerRate_t *uRate);
...
PRIVATE sbStatus_t
sbFe2000PmPolicerGroupReadConfig(sbFe2000Pm_t *pPm,  uint32 uGroupId, 
...
{
  uint32 idx, count, shown;
  dq_t *pElt;


  LOG_CLI((BSL_META("Profiles:     profile_id:<num_policers, ref_count>\n")));
  ...
  for (idx = 0; idx < SB_FE_2000_PM_MAX_NUM_PROFILES; idx++) {
    ...
    if (count || pPm->profiledb[idx].uRefCount) {
      LOG_CLI((BSL_META("0x%03x:<%5d,%5d> "), idx, count, pPm->profiledb[idx].uRefCount));
      if ((shown++ & 3) == 3) {
        LOG_CLI((BSL_META("\n")));
      }
    }
...
sbStatus_t
sbFe2000PmRecoverPolicer(sbFe2000Pm_t *pPm, uint32 policer_id,
                         uint32 profile_id)
{
  uint32             group, group_max_id;
...
sbFe2000PmRecover(sbFe2000Pm_t *pPm)
{
  ...
  for (id=0; id < SB_FE_2000_PM_MAX_NUM_PROFILES; id++) {
    ...
    if (st != SB_OK) {
      LOG_CLI((BSL_META("Failed to read profile %d on recovery: 0x%08x\n"), 
               id, st));
      return st;
    }      
  }
 ...
 */
sbStatus_t 
sbFe2000PmInit(uint32 uHppFreq, sbFe2000Pm_t **pPmMgr, sbFe2000PmConfig_t *pConfig)
{
  sbFe2000Pm_t    *pPm;
  sbStatus_t            err = SB_OK;
  void                  *vp;
  uint32 uPolicerBase = 0;
  uint32 uValue = 0, uAddress =0;
  uint32 uNumPolicerGroups = 0;
  uint32 i =0;
  ...
  *pPmMgr = pPm = (sbFe2000Pm_t *)vp;
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("%s: Initialize PM_CONFIG1 \n"), FUNCTION_NAME()));
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("%s: Initialize PM_CONFIG0 \n"), FUNCTION_NAME()));
  ...
  default:
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("illegal policer bank selection: %d"), pPm->PolicerConfig.bank));
  ...
  if (SOC_WARM_BOOT((int)pPm->PolicerConfig.regSet)) {
    ...
    if (err != SB_OK) {
      LOG_CLI((BSL_META("Failed to recover soc_policer: 0x%08x\n"), err));
      return err;
    }
    
  } else {
    /* +1: C2 has an extra word and sbFe2000PmProfileMemoryWrite is common for FE2000 and C2 */
    uint32 id, uProfileWords[SB_ZF_FE_2000_PM_PROFMEMORY_SIZE_IN_WORDS + 1];
    ...
    for (id=0; id < SB_FE_2000_PM_MAX_NUM_PROFILES; id++) {
      ...
      if (err != SB_OK) {
        LOG_CLI((BSL_META("Failed to clear profile %d: 0x%08x\n"), id, err));
        return err;
      }
    }
    ...
    if(err != SB_OK) {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("error: null profile create failed\n")));
    }
  }
                      ...
                      uint32 *pIndex)
{
  sbStatus_t uStatus;
  uint32 grp=0xFF;
  uint32 uMinPolicerId = 0,uMaxPolicerId = 0;
  uint32 uAddress = 0, uValue =0;
  uint32 uPolicerId=0, uProfileId= 0xFFFF;
  uint8 bFound = 0;
  uint8   uPolicerBytes[SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES];
  uint8   uXtPolicerBytes[SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES];
  uint32  uBktCBits, tempBktCBits, uBktEBits, uBktEShift, tempCBS, tempEBS; 
  uint64  uBktCMask, uBktEMask;
  uint64  ulTimeStamp, ulBktE, ulBktC, ulBktTemp;
#if 0
  uint32  tempBktEBits;
#endif
  uint32  uBktESize; /* configured BktE size */
  ...
  if (SAND_HAL_IS_FE2KXT (pPm->PolicerConfig.regSet )) {
    ...
    if (uBktESize > (zGroupConfig.uTimestampOffset - zXtProfile.uBktCSize)) {
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("%s: Invalid Excess Burst Size. Max supported - %d Bytes \n"),
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("Policer Raw Set: ulBktE %x%08x  ulBktC %x%08x ulTimeStamp %x%08x\n"), 
            COMPILER_64_HI(ulBktE), COMPILER_64_LO(ulBktE), COMPILER_64_HI(ulBktC), COMPILER_64_LO(ulBktC), COMPILER_64_HI(ulTimeStamp), COMPILER_64_LO(ulTimeStamp)));
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("   MeterState %x%08x\n"), COMPILER_64_HI(zPolicer.uMeterState), COMPILER_64_LO(zPolicer.uMeterState)));
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("   uBktCBits %d   uBktEBits %d\n"), uBktCBits, uBktEBits));
...
{
  sbStatus_t uStatus;
  uint32 uProfileId=0,uGroupId=0,bFound=0;
  uint32 uAddress=0 , uValue =0,uMinPolicerId=0;
  sbFe2000PmPolicer_t *pPolicerRef = NULL;
  sbZfFe2000PmPolicerRawFormat_t zPolicer;
                                ...
                                uint32 uRateId)
{
  ...
  uint32 uPolicerId=0;
  sbZfFe2000PmGroupConfig_t zGroupConfig;
  uint32 uConfig=0, uValue=0, uAddress=0;
  char     buf[1024];
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("PmGroup%d Configuration reinited is %s\n"), uGroupId, &buf[0]));
...
sbStatus_t
sbFe2000ProfileCreate(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfFe2000PmProfileMemory_t *pProfile)
{
  sbStatus_t uStatus;
...
sbStatus_t
sbFe2000ProfileDelete(sbFe2000Pm_t *pPm, uint32 uProfileId)
{
  sbStatus_t uStatus;
...
sbFe2000NullProfileCreate(sbFe2000Pm_t *pPm)
{
  ...
  uint32 uProfileId = SB_FE_2000_PM_NULL_PROFILE_ID;
...
sbFe2000PmProfileAlloc(sbFe2000Pm_t *pPm, sbZfFe2000PmProfileMemory_t *pProfile, uint32 *pIndex)
{
  ...
  uint8                             uProfileBytes[SB_ZF_FE_2000_PM_PROFMEMORY_SIZE_IN_BYTES];
  uint32                            uId =0, bFound =0;
...
AllocMemChunk(sbFe2000Pm_t *pPm) 
{
  ...
  for (i = 0; i < pPm->uPolicersPerChunk; i++) {
    ...
    pPolicer[i].uPolicerId  = 0; 
    pPolicer[i].uProfileId  = 0; 
...
{
  pPolicer->uPolicerId  = 0; 
  pPolicer->uProfileId  = 0; 
...
{
  uint32 uTimerTick = 0;
  uint32 kbps, uRBps = 0; /* Bytes Per second */
  uint32 uNpuClockFreq =0;
...
sbStatus_t
sbFe2000PmLocProfileMemoryWrite(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfFe2000PmProfileMemory_t *pProfile)
{
  sbStatus_t uStatus;
  uint32 uProfileWords[SB_ZF_FE_2000_PM_PROFMEMORY_SIZE_IN_WORDS + 1];
  /* +1: C2 has an extra word and sbFe2000PmProfileMemoryWrite is common for FE2000 and C2 */
...
sbStatus_t
sbFe2000PmIEEECodingGet(uint32 uValue, uint32 *pEncodedValue)
{
  uint32 uExponent = 0;
  uint32 uBytes = uValue;
  uint32 uBFMantissa = 0, uBFExponent =0;
  uint32 bFound =0;
  ...
  *pEncodedValue = 0;
  ...
  if(uBytes <= 255) {
    ...
    if(!bFound) {
      LOG_CLI((BSL_META("ERROR: No valid conversion found for %d\n"), uValue));
      return SB_BAD_ARGUMENT_ERR_CODE;
      /* error */
    }
                        ...
                        uint32 *pIndex)
{
  ...
  uint32    uMinPolicerId = 0,uMaxPolicerId = 0;
  uint32    uAddress = 0, uValue =0;
  uint32    uTimerId=0, uProfileId= 0xFFFF;
  uint8     bFound = 0;
  uint8     uTimerBytes[SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_BYTES];
  uint8     uXtTimerBytes[SB_ZF_C2_PM_PROFTIMERMEMORY_SIZE_IN_BYTES];
  ...
  zTimer.uTime =  0;
  zTimer.uStarted =  pConfig->bStarted;
                     ...
                     sbZfFe2000PmOamTimerConfig_t *pConfig)
{
  ...
  uint8                               uTimerBytes[SB_ZF_FE_2000_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES];
  uint8                               uXtTimerBytes[SB_ZF_C2_PM_PROFPOLICERMEMORY_SIZE_IN_BYTES];
                         ...
                         uint32     *pIndex)
{
  ...
  uint32    uMinPolicerId = 0,uMaxPolicerId = 0;
  uint32    uAddress = 0, uValue =0;
  uint32    uSeqGenId=0, uProfileId= 0xFFFF;
  uint8     bFound = 0;
  uint8     uSeqGenBytes[SB_ZF_C2_PM_PROFSEQGENMEMORY_SIZE_IN_BYTES];
...
{
  sbStatus_t status;
  uint32 uTimerTick, uMtuTimeNs, uBits = 0;
  uint32 uNpuClockPeriod = 0; /* In nanosec */

  if (SAND_HAL_IS_FE2KXT (pPm->PolicerConfig.regSet ))
  {
    /* uNpuClockPeriod = SB_FE_2000XT_PM_NPU_CLOCK_PERIOD_NS; */
    if        (pPm->uHppFreq == 400000) {
        uNpuClockPeriod = 1024000.0/400.0;
    } else if (pPm->uHppFreq == 375000) {
        uNpuClockPeriod = 1024000.0/375.0;
    } else if (pPm->uHppFreq == 300000) {
        uNpuClockPeriod = 1024000.0/300.0;
    } else if (pPm->uHppFreq == 262500) {
        uNpuClockPeriod = 1024000.0/262.500;
    } else if (pPm->uHppFreq == 258333) {
        uNpuClockPeriod = 1024000.0/258.333;
    } else if (pPm->uHppFreq == 250000) {
        uNpuClockPeriod = 1024000.0/250.0;
    } else {
        SB_ASSERT(0);
    }
  }
  ...
  pGroupConfig->uTimestampOffset = 53-uBits; 
  ...
  /* JH: it's really uMtuTicks, change to this to get rid of 1 devide */
  ...
  if (pPm->PolicerConfig.uNumberOfPolicersPerGroup[uGroupId] == 0) {
    pGroupConfig->uRefreshPeriod = 0;
      ...
      pGroupConfig->uRefreshPeriod = (SB_FE_2000_PM_OAM_TIMER_TICK_NS*32) / 
      ((uNpuClockPeriod * 
       pPm->PolicerConfig.uNumberOfPolicersPerGroup[uGroupId])/32);
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("group %d: %d policers, Timer: %f ms per tick\n"), uGroupId,
            pPm->PolicerConfig.uNumberOfPolicersPerGroup[uGroupId],
            (pPm->PolicerConfig.uNumberOfPolicersPerGroup[uGroupId] * 
...
sbFe2000XtPmProfileAlloc(sbFe2000Pm_t *pPm, sbZfC2PmProfileMemory_t *pProfile, uint32 *pIndex)
{
  ...
  uint8                             uProfileBytes[SB_ZF_C2_PM_PROFMEMORY_SIZE_IN_BYTES];
  uint32                            uId =0, bFound =0;
...
sbStatus_t
sbFe2000XtProfileCreate(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfC2PmProfileMemory_t *pProfile)
{
  sbStatus_t uStatus;
...
sbStatus_t
sbFe2000XtPmLocProfileMemoryWrite(sbFe2000Pm_t *pPm, uint32 uProfileId, sbZfC2PmProfileMemory_t *pProfile)
{
  sbStatus_t uStatus;
  uint32 uProfileWords[SB_ZF_C2_PM_PROFMEMORY_SIZE_IN_WORDS];
