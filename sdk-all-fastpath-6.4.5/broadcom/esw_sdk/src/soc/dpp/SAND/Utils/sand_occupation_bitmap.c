  ...
  )
{
  ...
  else
  {
      if (bit_map->support_cache && bit_map->cache_enabled)
      {     
          res = SOC_DPP_WB_ENGINE_MEMSET_ARR(bit_map->unit, bit_map->wb_var_index + WB_ENGINE_OCC_BM_LEVELS_CACHE_BUFFER, init_val);
          SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      }
      else
      {
          res = SOC_DPP_WB_ENGINE_MEMSET_ARR(bit_map->unit, bit_map->wb_var_index + WB_ENGINE_OCC_BM_LEVELS_BUFFER, init_val);
  ...
  )
{
  ...
  {
      res = SOC_DPP_WB_ENGINE_SET_VAR(bit_map->unit, bit_map->wb_var_index + WB_ENGINE_OCC_BM_CACHE_ENABLED, &cached);
  ...
  */
  if (cached)
  {
      ...
      {
          res = SOC_DPP_WB_ENGINE_MEMCPY_ARR(bit_map->unit, bit_map->wb_var_index + WB_ENGINE_OCC_BM_LEVELS_CACHE_BUFFER, 
  ...
  )
{
  ...
  */
  ...
  {
      res = SOC_DPP_WB_ENGINE_MEMCPY_ARR(bit_map->unit, bit_map->wb_var_index + WB_ENGINE_OCC_BM_LEVELS_BUFFER, 
