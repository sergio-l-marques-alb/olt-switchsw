  ...
  )
{
  ...
  *multi_set_ptr = soc_sand_os_malloc_any_size(sizeof(SOC_SAND_MULTI_SET_INFO), "multi_set_ptr");
  soc_sand_SAND_MULTI_SET_INFO_clear(*multi_set_ptr);
  multi_set = *multi_set_ptr;

  multi_set->init_info.max_duplications = init_info.max_duplications;
  multi_set->init_info.get_entry_fun    = init_info.get_entry_fun;
  multi_set->init_info.set_entry_fun    = init_info.set_entry_fun;
  multi_set->init_info.nof_members      = init_info.nof_members;
  multi_set->init_info.member_size      = init_info.member_size;
  multi_set->init_info.prime_handle     = init_info.prime_handle;
  multi_set->init_info.sec_handle       = init_info.sec_handle;
  multi_set->init_info.wb_var_index     = init_info.wb_var_index;
  ...
  */
  ...
  {
      hash_init.wb_var_index = SOC_DPP_WB_ENGINE_VAR_NONE;
  }
  ...
  )
{
  ...
  */
  ...
  soc_sand_os_free_any_size(multi_set);
  ...
  )
{
  ...
  if (set_init->get_entry_fun) {
      get_entry_fun = set_init->get_entry_fun;
  }
  else
  {
      get_entry_fun = soc_sand_multi_set_default_get_entry;
  }
  if (set_init->set_entry_fun) {
      set_entry_fun = set_init->set_entry_fun;
  }
  else
  {
      set_entry_fun = soc_sand_multi_set_default_set_entry;
  }
  ...
  *data_indx = found_index;
  ...
  {
      res = SOC_DPP_WB_ENGINE_SET_ARR(set_init->prime_handle, set_init->wb_var_index + WB_ENGINE_MULTI_SET_NEXT_REF_COUNTER, 
                                      tmp_cnt, 
                                      *data_indx);
  ...
  )
{
  ...
  */
  if (set_init->max_duplications > 1)
  {
      ...
      {
          res = SOC_DPP_WB_ENGINE_MEMSET_ARR(set_init->prime_handle, set_init->wb_var_index + WB_ENGINE_MULTI_SET_NEXT_REF_COUNTER, 0x0);
                                   ...
                                   )
{
  ...
  info->multiset_data.hash_table = NULL;
