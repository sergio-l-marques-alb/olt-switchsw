...
soc_jer_lif_init(int unit){
    ...
    rv = soc_jer_lif_glem_access_entry_add(unit, SOC_PPC_INVALID_GLOBAL_LIF, 0);
    SOCDNX_IF_ERR_EXIT(rv);
...
soc_jer_lif_glem_access_entry_add(int unit, int global_lif_id, int egress_lif_id){
    int rv;
#ifdef PLISIM
    uint32 uint_global_lif_id, uint_egress_lif_id;
#else
...
soc_jer_lif_glem_access_write_to_machine(int unit, uint32 *buffer){
    ...
    /*rv = soc_jer_lif_glem_access_assert_management_request_complete(unit);
    SOCDNX_IF_ERR_EXIT(rv);*/
