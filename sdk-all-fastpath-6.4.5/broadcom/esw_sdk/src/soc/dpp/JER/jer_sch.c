...
/* This function should be called only during init sequence */
    uint32 reg_val, reg_val2, offset, rate, mem_val, quanta, credit, nof_ticks, hr_num,
    ...
    if(!(SOC_DPP_CONFIG(unit)->arad->init.dynamic_port_enable)) /* working without dynamic ports - should restrict calendar length*/
    {
        ...
        for (offset = 0; offset < ARAD_EGR_NOF_Q_PAIRS; offset++)
        {
            ...
            if (valid > 0)
            {
                /* update psc table - psst[nof_available_hr] = hr[offset].rate */
                SOCDNX_IF_ERR_EXIT(READ_SCH_PIR_SHAPER_CALENDAR_PSCm(unit, SCH_BLOCK(unit,core), nof_available_hr, &mem_val));
                soc_SCH_PIR_SHAPER_CALENDAR_PSCm_field32_set(unit, &mem_val, HR_NUMf, offset);
                SOCDNX_IF_ERR_EXIT(WRITE_SCH_PIR_SHAPER_CALENDAR_PSCm(unit, SCH_BLOCK(unit,core), nof_available_hr, &mem_val));
                nof_available_hr++;
            }
        }
        ...
        soc_reg_field_set(unit, SCH_PIR_SHAPERS_CONFIGURATIONr, &reg_val, PIR_SHAPERS_CAL_LENGTHf, (nof_available_hr > 0) ? (nof_available_hr - 1):0); /* set is length - 1 */
        SOCDNX_IF_ERR_EXIT(WRITE_SCH_PIR_SHAPERS_CONFIGURATIONr(unit, core, reg_val));

        /* write 0 to all other places in PSC table */
        
        for (offset = nof_available_hr; offset < ARAD_EGR_NOF_Q_PAIRS; offset++)
        {
            SOCDNX_IF_ERR_EXIT(READ_SCH_PIR_SHAPER_CALENDAR_PSCm(unit, SCH_BLOCK(unit,core), offset, &mem_val));
            soc_SCH_PIR_SHAPER_CALENDAR_PSCm_field32_set(unit, &mem_val, HR_NUMf, 0x0);
            SOCDNX_IF_ERR_EXIT(WRITE_SCH_PIR_SHAPER_CALENDAR_PSCm(unit, SCH_BLOCK(unit,core), offset, &mem_val));
        }
    }
    else /* using dynamic ports*/
    {
        nof_available_hr = ARAD_EGR_NOF_Q_PAIRS;
    }

    /* configure access period and cal length to both tables */
    ...
    if (!(SOC_DPP_CONFIG(unit)->arad->init.dynamic_port_enable)) /* working without dynamic ports- calc csst table length */
    {
        /*
        uint32 tcg_id, nof_priorities, csc_table_index = 0, tm_port, base_port_tc;
        int core_2, i;
        ...
        SOC_PBMP_ITER(ports_bm, port_i)
        {
            ...
            if (nof_priorities == 8)
            {
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_tm_port_get(unit, port_i, &tm_port, &core_2));
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, tm_port, &base_port_tc));
                *//* config 0 to 7*//*
                for (i = 0; i < ARAD_NOF_TCGS_IN_PS; i++)
                {
                     tcg_id = ARAD_SCH_PORT_TCG_ID_GET(base_port_tc, i);
                     SOCDNX_IF_ERR_EXIT(READ_SCH_CIR_SHAPER_CALENDAR_CSCm(unit, SCH_BLOCK(unit, core), csc_table_index, &mem_val));
                     soc_mem_field32_set(unit, SCH_CIR_SHAPER_CALENDAR_CSCm, &mem_val, PG_NUMf, tcg_id);
                     SOCDNX_IF_ERR_EXIT(WRITE_SCH_CIR_SHAPER_CALENDAR_CSCm(unit, SCH_BLOCK(unit, core), csc_table_index, &mem_val));
                     csc_table_index ++;
                }
            }
        }*/
        /* config cal length for CSC *//*
        SOCDNX_IF_ERR_EXIT(READ_SCH_CIR_SHAPERS_CONFIGURATIONr(unit, core, &reg_val));
        soc_reg_field_set(unit, SCH_CIR_SHAPERS_CONFIGURATIONr, &reg_val, CIR_SHAPERS_CAL_LENGTHf, (csc_table_index > 0) ? (csc_table_index-1):0);
        SOCDNX_IF_ERR_EXIT(WRITE_SCH_CIR_SHAPERS_CONFIGURATIONr(unit,core, reg_val));

        SOCDNX_IF_ERR_EXIT(READ_SCH_CIR_SHAPERS_CONFIGURATION_1r(unit, core, &reg_val));
        soc_reg_field_set(unit, SCH_CIR_SHAPERS_CONFIGURATION_1r, &reg_val, CIR_SHAPERS_CAL_LENGTH_1f, (csc_table_index > 0) ? (csc_table_index-1):0);
        SOCDNX_IF_ERR_EXIT(WRITE_SCH_CIR_SHAPERS_CONFIGURATION_1r(unit, core, reg_val));

        *//* write 0 to all other places in psc table */
        /*for (i = csc_table_index; i < ARAD_EGR_NOF_Q_PAIRS ; i++)
        {
             SOCDNX_IF_ERR_EXIT(READ_SCH_CIR_SHAPER_CALENDAR_CSCm(unit, SCH_BLOCK(unit, core), i, &mem_val));
             soc_mem_field32_set(unit, SCH_CIR_SHAPER_CALENDAR_CSCm, &mem_val, PG_NUMf, 0x0);
             SOCDNX_IF_ERR_EXIT(WRITE_SCH_CIR_SHAPER_CALENDAR_CSCm(unit, SCH_BLOCK(unit, core), i, &mem_val));
        }*/
    }
    ...
    )
{
    ...
    uint32 credit, reg_val, nof_ticks, quanta_nof_bits, quanta, offset,
           base_port_tc, hr_calendar_table_num, index, mem_val, max_rate_before_change, old_rate;
    int increasing_maximum_rate = 0 , decreasing_unique_maximum_rate = 0 , is_new_rate_configured = 0, maximum_multiplicity;
    uint8 valid;
    ...
    {
        SOCDNX_IF_ERR_EXIT(_jer_sch_rates_calculate_new_calendars(unit, core, increasing_maximum_rate, (increasing_maximum_rate)? rate:0 , 1, &is_new_rate_configured));                       
    }
    ...
    SOC_SAND_IN     uint32               rate)
{
    uint32  credit, reg_val ,nof_ticks, quanta_nof_bits, quanta, base_port_tc, hr_calendar_table_num,
            offset, index, mem_val , decreasing_unique_maximum_rate = 0, increasing_maximum_rate = 0, max_rate_before_change, old_rate;
    int maximum_multiplicity, is_new_rate_configured=0;
    uint8 valid;
    SOCDNX_INIT_FUNC_DEFS;
...
_jer_sch_rates_switch_calendars(int unit, int core, int new_calendar_table_num, int new_divider)
{
    uint32 mem_val, credit, nof_ticks, quanta_nof_bits, reg_val, cal_length_psst, cal_length_csst, hr_num, tcg_index, rate,
...
_jer_sch_rates_calculate_new_divider(int unit, int core, int hr_calendar_table_num, int max_rate_increased, int new_max, int is_psst_table, int old_divider ,uint32 *new_divider)
{
    ...
    if (max_rate_increased) /* max rate increased - we need to decrease the divider */
    {
        ...
        for (curr_divider = old_divider; curr_divider > 15 ; curr_divider--)
        {
    		...
    		SOCDNX_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks,&new_quanta));
    		if(new_quanta <= 2047)
    		{
    			break;
    		}
    ...
    else /* maximum rate decreased - we can decrease the divider in order to get a better resolution */
    {
        ...
        else /* increase divider to maximum possible, keeping the max rate quanta below 2047 */
        {
            ...
            for (iterate_divider = curr_divider + 1; iterate_divider < 65; iterate_divider++)
            {
                ...
                {
            		SOCDNX_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks_psst_table,&new_quanta));
            		if(new_quanta > 2047)
            		{
                        break;
            		}                    
                ...
                {
            		SOCDNX_IF_ERR_EXIT(arad_sch_port_rate_kbits_per_sec_to_qaunta(new_max ,credit,nof_ticks_csst_table,&new_quanta));
            		if(new_quanta > 2047)
            		{
            			break;
            		}  
