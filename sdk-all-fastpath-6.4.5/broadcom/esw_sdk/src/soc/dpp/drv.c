...
soc_dpp_init(int unit, int reset)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        rv = shr_sw_state_init(unit, 0x0, socSwStateDataBlockRestoreAndOveride, -1);
        SOCDNX_IF_ERR_RETURN(rv);
    }
    else{
        rv = shr_sw_state_init(unit, 0x0, socSwStateDataBlockRegularInit, SHR_SW_STATE_MAX_DATA_SIZE_IN_BYTES);
        SOCDNX_IF_ERR_RETURN(rv);
    }
