  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, core, &buffer));
    soc_reg_field_set(unit, IPS_IPS_GENERAL_CONFIGURATIONSr, &buffer, FMC_CREDITS_FROM_SCHf, info->credits_via_sch);
    SOCDNX_IF_ERR_EXIT(WRITE_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, core, buffer));
  ...
  )
{
    ...
    rv = READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, core, &buffer);
    SOCDNX_IF_ERR_EXIT(rv);
