  ...
  )
{
    ...
    /* Clear the DB bitmap */
    for(bitmap_array_idx = 0; bitmap_array_idx < ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS); ++bitmap_array_idx) {
        res = sw_state_access[unit].dpp.soc.arad.tm.pmf.pgm_db_pmb.set(unit, stage, prog_id, bitmap_array_idx, 0x0);
   ...
   SOC_SAND_IN  SOC_TMC_CNT_MODE_EG_TYPE eg_type)
{
    ...
    if (eg_type == SOC_TMC_CNT_MODE_EG_TYPE_VSI || eg_type == SOC_TMC_CNT_MODE_EG_TYPE_PMF) {
        res = arad_pmf_low_level_pgm_counter_set_unsafe(unit, command_id, eg_type); 
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    }
                ...
                if (SHR_BITGET(groups_lcl, pfg_ndx)) {
                    ...
                    for (qual_ndx = 0; qual_ndx < SOC_PPC_FP_NOF_QUALS_PER_PFG_MAX; qual_ndx++) {
                        if (pfg_info_current.qual_vals[qual_ndx].type == SOC_PPC_FP_QUAL_FWD_PRCESSING_PROFILE) {
                            ...
                            res = arad_pp_fp_control_get(unit, SOC_CORE_INVALID, &control_ndx, &control_info);
                            SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
  ...
  )
{
    ...
    if (stage == ARAD_FP_DATABASE_STAGE_EGRESS) {
        for (is_msb = FALSE; is_msb <= TRUE; is_msb++) {
            for (ce_group_ndx = 0; ce_group_ndx < ARAD_PMF_CE_NOF_GROUP_PER_LSB; ce_group_ndx++) {
                ce_table = arad_pmf_ce_table_get(unit, stage, is_msb, 0, 0); /* ce_ndx(last argument) is irrelevant */
    ...
    sal_memcpy(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_to],
        sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_from],
        sizeof(uint32) * ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS));
