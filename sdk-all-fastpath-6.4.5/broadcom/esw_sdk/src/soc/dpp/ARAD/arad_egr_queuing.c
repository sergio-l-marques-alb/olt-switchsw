  ...
  )
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_id) {
        ...
        {
            SOCDNX_IF_ERR_EXIT(arad_egq_ppct_tbl_get_unsafe(unit, core_id, base_q_pair, &egq_ppct_tbl_data));
            ...
            SOCDNX_IF_ERR_EXIT(arad_egq_ppct_tbl_set_unsafe(unit, core_id, base_q_pair, &egq_ppct_tbl_data));
        }
    }
    ...
    if(SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      ...
      res = arad_egr_dev_fc_set_unsafe(unit, 0  ,&dev_conf, &exact_dev_conf);
  ...
  )
{
  ...
  if(prio_ndx == ARAD_EGR_Q_PRIO_ALL) {
    ...
    /* CGM_CONGESTION_MANAGEMENT_MULTICAST_FLOW_CONTROL_PER_TC_THREHOSLDSr */
  ...
  )
{
    ...
    */
    ...
    for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_q_pairs; curr_q_pair++)
    {
        ...
        soc_EGQ_PCTm_field32_set(unit, pct_data, EGRESS_TCf, curr_q_pair-base_q_pair);
  ...
  )
{
    ...
    else if (threshold_type == soc_dpp_cosq_threshold_packet_descriptors || soc_dpp_cosq_threshold_available_packet_descriptors)
    {
  ...
  )
{
    ...
    {
         if(dev_thresh->thresh_type[threshold_type].reserved[prio].descriptors > EGQ_QDCT_PD_MAX_VAL)
        {
          SOC_SAND_SET_ERROR_CODE(ARAD_EGR_QDCT_PD_VALUE_OUT_OF_RANGE, 5, exit);
        }
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 1250, exit, READ_EGQ_QDCT_TABLEm(unit, EGQ_BLOCK(unit, core), threshold_type, mem));
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 1260, exit, WRITE_EGQ_QDCT_TABLEm(unit, EGQ_BLOCK(unit, core), threshold_type, mem));
    }
