  ...
  if (soc_property_get(unit, spn_FIELD_PRESEL_MGMT_ADVANCED_MODE, FALSE) ==0 ) 
  {
      ...
      if (!info->is_array_qualifier) {
          ...
          for (db_bmp_id = 0; db_bmp_id < ARAD_PMF_LOW_LEVEL_NOF_DBS_BMP; db_bmp_id++) {
              res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.get(unit, stage, pfg_ndx, db_bmp_id, &pfg_db_pmb);
              SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
              if (pfg_db_pmb) {
                LOG_ERROR(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
                                      "   Error in PFG %d destroy: Not all the Databases are deestroyed, BMP[%d] = 0x%x\n\r"), 
                           pfg_ndx, db_bmp_id, pfg_db_pmb));
  ...
  )
{
    ...
    if (is_default_db
        ...
        res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.default_db_pmb.get(unit, stage, db_id_ndx/(sizeof(uint32)*8), default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 11, exit);
        SHR_BITSET(default_db_pmb, db_id_ndx % (sizeof(uint32)*8));
        res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.default_db_pmb.set(unit, stage, db_id_ndx/(sizeof(uint32)*8), *default_db_pmb);
        ...
        for (pfg_ndx = 0; pfg_ndx < ARAD_PP_FP_NOF_PFGS; ++pfg_ndx) {
            if (SHR_BITGET(db_info->supported_pfgs_arad, pfg_ndx)) {
                res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.get(unit, stage, pfg_ndx, db_id_ndx/(sizeof(uint32)*8), pfgs_db_pmb);
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                SHR_BITSET(pfgs_db_pmb, db_id_ndx % (sizeof(uint32)*8));
                res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.set(unit, stage, pfg_ndx, db_id_ndx/(sizeof(uint32)*8), *pfgs_db_pmb);
  ...
  )
{
    ...
    uint32
        pfg_ndx,
        default_db_pmb[1],
        pfgs_db_pmb[ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS)],
        res;
    ...
    if (is_default_db) {
        res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.default_db_pmb.get(unit, stage, db_id_ndx/(sizeof(uint32)*8), default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 11, exit);
        SHR_BITCLR(default_db_pmb, db_id_ndx % (sizeof(uint32)*8));
        res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.default_db_pmb.set(unit, stage, db_id_ndx/(sizeof(uint32)*8), *default_db_pmb);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 13, exit);
    }
    else {
        for (pfg_ndx = 0; pfg_ndx < ARAD_PP_FP_NOF_PFGS; ++pfg_ndx) {
            if (SHR_BITGET(db_info->supported_pfgs_arad, pfg_ndx)) {
                res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.get(unit, stage, pfg_ndx, db_id_ndx/(sizeof(uint32)*8), pfgs_db_pmb);
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                SHR_BITCLR(pfgs_db_pmb, db_id_ndx % (sizeof(uint32)*8));
                res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.set(unit, stage, pfg_ndx, db_id_ndx/(sizeof(uint32)*8), *pfgs_db_pmb);
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 17, exit);
            }
        }
    ...
    */
       ...
       && (info->db_type != ARAD_PP_FP_DB_TYPE_SLB))
    {
        ...
        if(info->db_type != ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) {
            ...
            sal_memcpy(forbidden_db_bmp,
                sw_state[unit]->dpp.soc.arad.tm->pmf[stage].psl_info.default_db_pmb,
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  else 
  {
    ...
    for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
    {
        ...
        SHR_BITSET(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_ndx_duplicate], db_id_ndx);
  ...
  )
{
  ...
  for (pmf_pgm_ndx_used = pmf_pgm_ndx_min; pmf_pgm_ndx_used < pmf_pgm_ndx_max; ++pmf_pgm_ndx_used)
  {
      ...
      SHR_BITCLR(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_ndx_used], db_id_ndx);
      ...
      for (pmf_pgm_ndx_new = pmf_pgm_ndx_min; pmf_pgm_ndx_new < pmf_pgm_ndx_max; ++pmf_pgm_ndx_new)
      {
          ...
          if(sal_memcmp(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_ndx_new],
                sw_state[unit]->dpp.soc.arad.tm->pmf[stage].pgm_db_pmb[pmf_pgm_ndx_used],
                sizeof(uint32) * ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS) ) == 0) {
              /* Update the Preselector line's PMF program at SW DB and HW */
            ...
            if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_DBAL)) {
                if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_HANDLE_ENTRIES_BY_KEY)) {                    
                    ...
                    if (physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM) {
                      ...
                      lem_payload.dest = action.value[0] & 0xffff7; /*19 bit of the destination */
                      lem_payload.asd = action.value[1] & 0xffffff;/* 24 bit of the asd */                      
  ...
  )
{
  ...
  if ( (!is_large_direct_extraction) && (!use_fes)) {
      ...
      *success = SOC_SAND_SUCCESS;
      ...
      alloc_flags = ( database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_SINGLE_BANK) ? ARAD_PP_FP_KEY_ALLOC_USE_KEY_A : 0 ; 
      ARAD_ALLOC(alloc_info, ARAD_PP_FP_KEY_ALLOC_INFO, 1, "arad_pp_fp_database_create_unsafe.alloc_info");
      for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
      {
  ...
  for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
  {
    ...
    if(mode >= ARAD_PP_FP_RESOURCE_MODE_WITH_AVAILABLE_RESOURCES) {
      ...
      for(pfg_idx = 0; pfg_idx < ARAD_PMF_NOF_GROUPS; ++pfg_idx) {
        for(bitmap_block_idx = 0; bitmap_block_idx < ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS); ++bitmap_block_idx) {
            res = sw_state_access[unit].dpp.soc.arad.tm.pmf.psl_info.pfgs_db_pmb.get(
                    unit,
                    stage,
                    pfg_idx,
                    bitmap_block_idx,
                    &(info->available.pfgs_db_pmb[stage][pfg_idx][bitmap_block_idx])
                   );
  ...
  for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
  {
    ...
    for(line_idx = 0 ; line_idx < ARAD_PMF_NOF_LINES; line_idx++) 
    {
      ...
      if(valid)
      {
        ...
        for(bitmap_block_idx = 0; bitmap_block_idx < ARAD_BIT_TO_U32(ARAD_PMF_NOF_DBS); ++bitmap_block_idx) {
          res = sw_state_access[unit].dpp.soc.arad.tm.pmf.pgm_db_pmb.get(
                  unit,
                  stage,
                  info->presel[stage][line_idx].pmf_pgm,
                  bitmap_block_idx,
                  &(info->presel[stage][line_idx].db_bmp[bitmap_block_idx])
                );
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 340, exit); 
        }
