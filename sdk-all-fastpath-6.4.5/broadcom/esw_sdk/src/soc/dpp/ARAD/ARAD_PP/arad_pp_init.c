    ...
    SOC_SAND_IN  int                                unit)
{
    ...
    if (!SOC_IS_ARDON(unit) && SOC_IS_ARADPLUS(unit)) 
    {
        ...
        SOCDNX_IF_ERR_EXIT(READ_IHP_FLP_GENERAL_CFGr(unit, SOC_CORE_ALL, &reg_val_64));
        fld_val = 0x1; 
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_IHP_FLP_GENERAL_CFGr(unit, SOC_CORE_ALL, reg_val_64));
  ...
  )
{
  ...
  res = arad_pp_diag_init_unsafe(unit);
