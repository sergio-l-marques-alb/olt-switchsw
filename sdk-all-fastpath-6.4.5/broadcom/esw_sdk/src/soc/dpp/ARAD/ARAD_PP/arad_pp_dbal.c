...
uint32 
    arad_pp_dbal_flp_lookup_update(int unit, DBAL_PROGRAM_INFO table_program, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ARAD_PP_IHB_FLP_LOOKUPS_TBL_DATA flp_lookups_tbl;
    SOC_DPP_DBAL_TABLE_INFO* table = &(dbal_tables[unit][table_id]);
...
uint32 
    arad_pp_dbal_vt_lookup_update(int unit, DBAL_PROGRAM_INFO table_program, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
...
uint32 
    arad_pp_dbal_tt_lookup_update(int unit, DBAL_PROGRAM_INFO table_program, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
                                             ...
                                             SOC_DPP_DBAL_KEY_TO_TABLE keys_to_table_id[SOC_DPP_DBAL_PROGRAM_NOF_KEYS],uint8 use_32_bit_ce[SOC_DPP_DBAL_PROGRAM_NOF_KEYS] [SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], int nof_valid_keys)
{
    ...
    int                         curr_key_id, curr_qual_id, curr_table_program, table_id;
    uint32                      ce_rsrc[1];
    ...
    for (curr_key_id = 0; curr_key_id < nof_valid_keys; curr_key_id++){
        ...
        if (!table->is_table_initiated) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_dbal_program_to_tables_associate - table not initiated %d"), table_id));
        }
        ...
        curr_key = keys_to_table_id[curr_key_id].key_id;
        if(keys_to_table_id[curr_key_id].key_id > SOC_DPP_DBAL_PROGRAM_KEY_D) {
        ...
        for (curr_qual_id = 0; curr_qual_id < table->nof_qualifiers; curr_qual_id++){
            ...
            if (use_32_bit_ce) {
                curr_use_32_bit_ce = use_32_bit_ce[curr_key_id][curr_qual_id];
            }else {
        ...
        table->table_programs[curr_table_program].key_id = keys_to_table_id[curr_key_id].key_id;
        table->table_programs[curr_table_program].lookup_number = keys_to_table_id[curr_key_id].lookup_number;
        table->table_programs[curr_table_program].nof_bits_used_in_key = nof_bits_used_in_key;
        table->table_programs[curr_table_program].public_lpm_lookup_size = keys_to_table_id[curr_key_id].public_lpm_lookup_size;

        if (stage == ARAD_FP_DATABASE_STAGE_INGRESS_FLP) {
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_flp_lookup_update(unit, table->table_programs[curr_table_program], table_id));
        } else if(stage == ARAD_FP_DATABASE_STAGE_INGRESS_VT){
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_vt_lookup_update(unit, table->table_programs[curr_table_program], table_id));
        } else if(stage == ARAD_FP_DATABASE_STAGE_INGRESS_TT){
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_tt_lookup_update(unit, table->table_programs[curr_table_program], table_id));
        }        
...
{    
    ...
    for (curr_key_id = 0; curr_key_id < nof_valid_keys; curr_key_id++){
        ...
        table->table_programs[nof_table_programs].lookup_number = keys_to_table_id[curr_key_id].lookup_number;
        table->table_programs[nof_table_programs].nof_bits_used_in_key = nof_bits_used_in_key;
        if (stage == ARAD_FP_DATABASE_STAGE_INGRESS_FLP) {
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_flp_lookup_update(unit, table->table_programs[nof_table_programs], keys_to_table_id[curr_key_id].sw_table_id));
        } else if(stage == ARAD_FP_DATABASE_STAGE_INGRESS_VT){
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_vt_lookup_update(unit, table->table_programs[nof_table_programs], keys_to_table_id[curr_key_id].sw_table_id));
        } else if(stage == ARAD_FP_DATABASE_STAGE_INGRESS_TT){
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_tt_lookup_update(unit, table->table_programs[nof_table_programs], keys_to_table_id[curr_key_id].sw_table_id));
        }
        table->nof_table_programs ++;
    }
        ...
        for (i = first_table_id[stage]; i < last_table_id[stage]; i++) {
            ...
            for (j = 0; j < table->nof_table_programs; j++) {
                if ((table->table_programs[j].program_id == prog_id[stage]) && (table->table_programs[j].stage == actual_stages[stage])) {                    
                    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_key_dump(unit, i, table->table_programs[j].lookup_number, actual_stages[stage]));
                    found = 1;
                }
            }
        }
