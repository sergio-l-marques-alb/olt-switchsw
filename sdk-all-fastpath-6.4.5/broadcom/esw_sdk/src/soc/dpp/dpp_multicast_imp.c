...
)
{
  ...
  DPP_MC_ASSERT(mcds->nof_ingr_reps == mcds->nof_reps[0] && mcds->nof_reps[0] < DPP_MULT_MAX_INGRESS_REPLICATIONS &&
    (mcds->nof_egr_port_outlif_reps[0] | mcds->nof_egr_outlif_reps[0] | mcds->nof_egr_bitmap_reps) == 0);
...
)
{
    ...
    DPP_MC_ASSERT(mcds->nof_ingr_reps == 0 && mcds->nof_reps[core] < DPP_MULT_MAX_INGRESS_REPLICATIONS &&
      mcds->nof_egr_port_outlif_reps[core] + mcds->nof_egr_outlif_reps[core] + mcds->nof_egr_bitmap_reps == mcds->nof_reps[core]);
    rep->extra = rep->base = 0;
...
void dpp_mcds_clear_replications(dpp_mcds_base_t *mcds, const uint32 group_type)
{
    ...
    for (; i < SOC_DPP_DEFS_MAX(NOF_CORES); ++ i) {
        mcds->nof_egr_outlif_reps[i] = mcds->nof_egr_port_outlif_reps[i] = mcds->nof_reps[i] = 0;
    }
}
...
)
{
    ...
    DPP_MC_ASSERT(is_normal_egress || (mcds->nof_egr_port_outlif_reps[0] | mcds->nof_egr_outlif_reps[0] | mcds->nof_egr_bitmap_reps)  == 0);
...
)
{
  ...
  if (is_egress) {
      ...
      for (; size_left; --size_left) {
        ...
        if (port == _SHR_GPORT_INVALID) { /* CUD only replication */
          if (nof_active_cores > 1) { /* add to the appropriate core */
            if ((core = DPP_CUD2CORE_GET_CORE(mcds, cud)) == DPP_CUD2CORE_UNDEF_VALUE) {
              SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("CUD 0x%x is not mapped to a port/core"), cud));
            }
          }
          dpp_add_egress_replication_cud(mcds, core, cud);
          ...
          dpp_add_egress_replication_port_cud(mcds, core, cud, port);
        }
      }
      for (core = SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; core > 0;) {
          --core;
          DPP_MC_ASSERT(mcds->nof_reps[core] == (int32)mcds->nof_egr_port_outlif_reps[core] + mcds->nof_egr_outlif_reps[core] + mcds->nof_egr_bitmap_reps && mcds->nof_ingr_reps == 0);
    ...
    DPP_MC_ASSERT(mcds->nof_reps[0] == mcds->nof_ingr_reps && (mcds->nof_egr_port_outlif_reps[0] | mcds->nof_egr_outlif_reps[0] | mcds->nof_egr_bitmap_reps) == 0);
...
)
{
  ...
  while (entries_remaining) {
    SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry( /* add replications to mcds from cur_index entry */
      unit, 0, cur_index, DPP_MCDS_TYPE_VALUE_EGRESS, &reps_left, &nof_reps, next_entry));
    ++cur_index;
    --entries_remaining;
  ...
  *out_err = _SHR_E_NONE;
  ...
  if (DPP_MCDS_TYPE_IS_EGRESS(group_type)) { /* egress */
    ...
    if (dest == _SHR_GPORT_INVALID) { /*outlif only replication */
      if (mcds->cud2core) { /* get core from CUD */
          if ((core = DPP_CUD2CORE_GET_CORE(mcds, cud)) >= SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores) {
...
)
{
  ...
  if (mcds->nof_egr_port_outlif_reps[0] | mcds->nof_egr_outlif_reps[0] | mcds->nof_egr_bitmap_reps) {
    DPP_MC_ASSERT(0);
...
uint32 dpp_mult_process_egress_replications(
    ...
    SOC_SAND_OUT uint16        *nof_port_couples,        /* The number of replication couples with the same outlif and in Arad with the first port < 127 moved to */
...
{
  dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
  dpp_rep_data_t *rep = mcds->reps + DPP_MCDS_GET_REP_INDEX(core, 0);
  uint16 i;
  SOCDNX_INIT_FUNC_DEFS;
  ...
  *nof_port_couples = 0;
  *nof_remaining_ports = mcds->nof_egr_port_outlif_reps[core]; /* nof_remaining_ports holds the number of port+outlif replications not belonging to couples */
  *nof_outlif_replications = mcds->nof_egr_outlif_reps[core];
  *nof_bitmap_replications = mcds->nof_egr_bitmap_reps;
  DPP_MC_ASSERT(*nof_remaining_ports + *nof_outlif_replications + *nof_bitmap_replications == mcds->nof_reps[core] && mcds->nof_reps[core] <= DPP_MULT_MAX_EGRESS_REPLICATIONS);

  /* sorting is always needed for finding port+outlif couples */
  soc_sand_os_qsort(rep, mcds->nof_reps[core], sizeof(dpp_rep_data_t), dpp_rep_data_t_compare);

  if (!(SOC_DPP_CONFIG(unit)->tm.mc_mode & DPP_MC_ALLOW_DUPLICATES_MODE)) { /* need to check duplicate replications */
    for (i = mcds->nof_reps[core]; i > 1; --i) {
  ...
  DPP_MC_ASSERT(rep == mcds->reps + DPP_MCDS_GET_REP_INDEX(core, mcds->nof_reps[core]));
  *out_err = _SHR_E_NONE;
exit:
  SOCDNX_FUNC_RETURN;
}
...
)
{
  ...
  *egress_groups_open_data_ptr = NULL;
  ...
  { /* allocate memory for the arrays */
    ...
    SOCDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, egress_groups_open_data_ptr, sizeof(uint32) * ((SOC_DPP_CONFIG(unit)->tm.nof_mc_ids + 31) / 32), "mcds-egress-open"));
    if (nof_active_cores > 1 ) {
      const uint32 bytes_to_alloc = sizeof(uint32) * (mcds->max_egr_cud_field / DPP_CUD2CORE_CUDS_PER_WORD + 1);
      SOCDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &mcds->cud2core, bytes_to_alloc, "mcds-mc-prev_entries"));
      sal_memset(mcds->cud2core, DPP_CUD2CORE_INITIAL_BTYE_VALUE, bytes_to_alloc); /* init the allocated memory to no core */
    }
  }
  ...
  if (!SOC_WARM_BOOT(unit)) {
    ...
    if (SOC_IS_JERICHO(unit)) { /* Jericho specific configuration */
      ...
      for (i = 0; i < SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; ++i) {
        SOCDNX_IF_ERR_EXIT(WRITE_EGQ_MULTICAST_OFFSET_ADDRESSr(unit, i, DPP_MCDS_GET_EGRESS_GROUP_START(mcds, 0, i)));
      }
    }
  }
