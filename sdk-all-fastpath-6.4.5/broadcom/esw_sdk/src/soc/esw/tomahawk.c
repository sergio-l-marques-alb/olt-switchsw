...
#define _SOC_TH_DEFIP_TCAM_DEPTH 1024
    ...
    if (NULL == _soc_th_alpm_bkt_view_map[unit]) {
        if ((_soc_th_alpm_bkt_view_map[unit] =
            sal_alloc(sizeof(soc_mem_t) * SOC_TH_ALPM_MAX_BKTS,
                      "alpm_bkt_map")) == NULL) {
            return SOC_E_MEMORY;
        }
    }
    ...
    sal_memset(_soc_th_alpm_bkt_view_map[unit], INVALIDm,
               sizeof(soc_mem_t) * SOC_TH_ALPM_MAX_BKTS);

    return SOC_E_NONE;
}
#else /* BCM_TH_LATENCY_EXCLUDE */
int
soc_tomahawk_mem_config(int unit)
{
#define _SOC_TH_DEFIP_MAX_TCAMS   8
#define _SOC_TH_DEFIP_TCAM_DEPTH  1024
    ...
    if (SOC_SWITCH_BYPASS_MODE_NONE != latency) {
        ...
        /* fudge the figures for memories unsupported in both
                * Low latency L2 and Balanced L2+L3 modes */
        fpem_entries = 0;
        sop->memState[VLAN_MPLSm].index_max = -1;
        SOC_MEM_PTR(unit, VLAN_MPLSm) = 0;

        if (SOC_SWITCH_BYPASS_MODE_LOW == latency) {
            /* fudge the figures for memories unsupported in
                       * Low latency L2 mode */
            l3_entries = 0;
            fixed_l3_entries = 0;
            num_ecmp_rh_flowset_entries = 0;
...
#endif /* BCM_TH_LATENCY_EXCLUDE */

#ifdef ALPM_ENABLE
/* Verify ALPM Pivot table entries and Bucket before exit */
int
soc_tomahawk_alpm_scrub(int unit)
{
    ...
    int bkt_idx, bkt_ptr = 0, bkt_addr;
    int bank_num = 0, entry_num = 0, entry_count, bank_count;
    void *alpm_entry;
    uint32 rval;
    soc_mem_t alpm_mem, lpm_mem;
...
#define ALPM_GET_AVAIL_BANK(bank_num, bank_count, bank_disable) \
    ...
    if (soc_mem_read_range(unit, lpm_mem, MEM_BLOCK_ANY,
                           soc_mem_index_min(unit, lpm_mem),
                           soc_mem_index_max(unit, lpm_mem), lpm_tbl_ptr) < 0) {
    ...
    for (idx = 0; idx < tbl_size; idx++) {
        ...
        lpm_entry = soc_mem_table_idx_to_pointer(unit, lpm_mem, defip_entry_t *,
                                                 lpm_tbl_ptr, idx);
        ipv6 = soc_mem_field32_get(unit, lpm_mem, lpm_entry, MODE0f);
        step_count = ipv6 ? 1 : 2;
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            bkt_ptr = soc_mem_field32_get(unit, lpm_mem, lpm_entry, ALG_BKT_PTR0f);
            /* Get VRF */
            ...
            for (bkt_idx = 0; bkt_idx < bkt_count; bkt_idx++) {
                ...
                if (SOC_URPF_STATUS_GET(unit)) {
                    /* Read the other copy */
                    if (SOC_FAILURE(soc_mem_read(unit, alpm_mem, MEM_BLOCK_ANY,
                                    _soc_th_alpm_rpf_entry(unit, bkt_addr),
                                    alpm_entry))) {
                        goto free_lpm_table;
                    }
                }
...
_pair_128:
    ...
    for (idx = 0; idx < tbl_size; idx++) {
        ...
        lpm_128_entry = soc_mem_table_idx_to_pointer(unit, lpm_mem,
                                                     defip_pair_128_entry_t *,
                                                     lpm_tbl_ptr, idx);

        /* Verify if read LPM entry is IPv6-128 entry */
        if (0x03 != soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, MODE1_UPRf)) {
            continue;
        };

        /* Verify if LPM entry is valid */
        if (!soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, VALID1_LWRf) ||
            !soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, VALID0_LWRf) ||
            !soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, VALID1_UPRf) ||
            !soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, VALID0_UPRf)) {
            continue;
        };

        /* Get VRF */
        if (soc_th_alpm_128_lpm_vrf_get(unit, lpm_128_entry, &vrf_id, &vrf) < 0) {
            goto free_lpm_table;
        }
        ...
        soc_th_alpm_bank_db_type_get(unit, vrf, &bank_disable, &db_type);
        bkt_ptr = soc_mem_field32_get(unit, lpm_mem, lpm_128_entry, ALG_BKT_PTRf);
        ...
        for (bkt_idx = 0; bkt_idx < bkt_count; bkt_idx++) {
            ...
            /* Read entry from bucket memory */
            if (soc_mem_read(unit, L3_DEFIP_ALPM_IPV6_128m, MEM_BLOCK_ANY,
...
STATIC int _soc_tomahawk_clear_all_memory(int unit);
...
#define _SOC_TH_LINK_DELAY_REG_BLK    2            /* EPIPE BLOCKNUM */
    ...
    const char *lstr[] = {"normal", "balanced (L2/L3)", "low (L2)"};
#endif
    ...
    soc_reg_field_set(unit, TOP_BS_PLL1_CTRL_3r, &rval, NDIV_INTf, 100);
    soc_reg_field_set(unit, TOP_BS_PLL1_CTRL_3r, &rval, NDIV_FRACf, 0);
    ...
    LOG_CLI((BSL_META_U(unit, "*** unit %d: configured for %s latency\n"), unit, lstr[lmode]));
#endif
...
soc_tomahawk_port_speed_update(int unit, soc_port_t port, int speed)
{
    ...
    rval = 0;
    if (speed >= 100000 || IS_LB_PORT(unit, port)) {
        fval = 44;
    } else if (speed >= 50000) {
        fval = 27;
    } else if (speed >= 40000) {
...
STATIC int
_soc_tomahawk_clear_mmu_memory(int unit)
{
    ...
    for (pipe = 0; pipe < _TH_PIPES_PER_DEV; pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == TH_OVSB_TOKEN) {
                ...
                port = si->port_p2l_mapping[phy_port];
                idb_port = si->port_l2i_mapping[port];
                id = ((phy_port == 129) || (phy_port == 131)) ?
                     0xf : si->port_serdes[port];
            }
...
_soc_tomahawk_tdm_calculation(int unit, int initTime, _soc_tomahawk_tdm_t *tdm)
{
    ...
    for (index=0; index<TH_NUM_PHY_PORTS; index++) {
        _chip_pkg.state[index] = tdm->pm_encap_type[((index+1)/TH_NUM_PM_LNS)]==
...
_soc_tomahawk_misc_init(int unit)
{
    ...
    if (parity_enable) {
        if ((!SAL_BOOT_SIMULATION || SAL_BOOT_QUICKTURN) &&
            ...
            SOC_IF_ERROR_RETURN(_soc_tomahawk_clear_mmu_memory(unit));
        }
    ...
    if (!SAL_BOOT_XGSSIM) {
        ...
        SOC_IF_ERROR_RETURN(_soc_tomahawk_tdm_calculation(unit, TRUE, tdm));
        ...
        SOC_IF_ERROR_RETURN(_soc_tomahawk_tdm_idb_hsp_set(unit, UPDATE_ALL_PIPES));
        ...
        SOC_IF_ERROR_RETURN(_soc_tomahawk_tdm_mmu_hsp_set(unit, UPDATE_ALL_PIPES));
        SOC_IF_ERROR_RETURN(_soc_tomahawk_idb_init(unit));
    }
    ...
    if (SOC_PBMP_NOT_NULL(si->oversub_pbm)) {
        ...
        PBMP_ALL_ITER(unit, port) {
            fval = (si->port_speed_max[port] * 11875)/100000;
                                     ...
                                     _soc_mmu_rsvd_buffer_t *rsvd_buffer)
{
    ...
    int extreme_speed_asf_cells_lr[] = {47, 94, 188}; /* Line Rate */
                               ...
                               _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    int in_reserved = 0;
    int asf_rsvd = 0, egr_rsvd = 0; /* contains max value among all pipes */
    ...
    for (idx = 0; idx < NUM_PIPE(unit); idx++) {
        /* In Non-flex mode, additional reserve is only for ASF(CT) case */
        soc_th_mmu_additional_buffer_reserve(unit, idx, FALSE,
                                         ...
                                         int lossless)
{
    ...
    for (idx = 0; idx < NUM_PIPE(unit); idx++) {
        soc_th_mmu_additional_buffer_reserve(unit, idx, TRUE,
                                             &buf->rsvd_buffers[idx]);
        if (asf_rsvd < buf->rsvd_buffers[idx].asf_rsvd_cells) {
            asf_rsvd = buf->rsvd_buffers[idx].asf_rsvd_cells;
        }
                              ...
                              _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
        ...
        if (FALSE == si->flex_eligible) {
            pool_pg_headroom = buf_pool->prigroup_headroom / 2;
        } else {
            pool_pg_headroom = ing_rsvd;
        }
        limit = buf_pool->total - (((buf_pool->prigroup_guarantee) / 2 +
                                    pool_pg_headroom + asf_rsvd) +
                                   buf->headroom);
    ...
    for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
        ...
        limit = buf_pool->total - ((queue_guarantee >> 1) + asf_rsvd) - 44;
    ...
    for (idx = 0; idx < 4; idx++) {
        ...
        limit = buf_pool->total - (buf_pool->queue_guarantee / 2) - 44;
                                     ...
                                     uint32 *port_speed)
{
    ...
    if (lanes_ctrl->lanes == 1) {
        ...
        if (vco >= 25781) {
            ...
            *port_speed = higigPort ? 21000 : 20000;
        }
    } else {
...
STATIC uint32
_soc_map_port_speed_to_mmu_credit(uint32 port_speed)
{
    uint32 credit = 13;

    if (port_speed >= 100000) {
        credit = 44;
    } else if (port_speed >= 50000) {
        credit = 27;
    } else if (port_speed >= 40000) {
        credit = 25;
    } else if (port_speed >= 25000) {
        credit = 16;
    } else if (port_speed >= 20000) {
        credit = 18;
...
_soc_tomahawk_update_ep_credit(int unit, soc_th_port_lanes_t *lanes_ctrl)
{
    ...
    si = &SOC_INFO(unit);
    rc = _soc_tomahawk_flexed_port_speed_get(unit, lanes_ctrl, &port_speed);
    if SOC_FAILURE(rc) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Flexport : Unable to determine"
                              "flexed port details")));
        return rc;
    }
    fval = _soc_map_port_speed_to_mmu_credit(port_speed);
    soc_reg_field_set(unit, reg, &rval, CREDITf, fval);
    SOC_IF_ERROR_RETURN(WRITE_EGR_MMU_CELL_CREDITr(unit, port, rval));
    if (lanes_ctrl->lanes < lanes_ctrl->cur_lanes) { /* Ports being added */
        for (i = 0; i < lanes_ctrl->phy_ports_len; i++) {
            port = si->port_p2l_mapping[lanes_ctrl->phy_ports[i]];
            SOC_IF_ERROR_RETURN(WRITE_EGR_MMU_CELL_CREDITr(unit, port, rval));
        }
...
_soc_tomahawk_port_lanes_update_tdm(int unit, soc_th_port_lanes_t *lanes_ctrl)
{
    ...
    if (!SOC_PBMP_MEMBER(si->oversub_pbm, port_base)) {
        ...
        SOC_IF_ERROR_RETURN(_soc_tomahawk_tdm_calculation(unit, FALSE, tdm));
...
soc_tomahawk_edb_buf_reset(int unit, soc_port_t port, int reset)
{
    ...
    if (reset) {
        ...
        SOC_IF_ERROR_RETURN(READ_EGR_ENABLEm(unit, MEM_BLOCK_ALL, port, entry));
...
soc_tomahawk_port_lanes_set(int unit, soc_th_port_lanes_t *lanes_ctrl)
{
    ...
    /* Update port_init_speed info which is used by SerDes driver */
...
int
soc_th_mmu_config_shared_buf_recalc(int unit, int res, int thdi_shd,
                                    ...
                                    int post_update)
{
            ...
            for (idx = 0; idx < _TH_MMU_NUM_PG; idx++) {
                ...
                if (!soc_mem_field32_get(unit, mem, &pg_config_mem,
                    ...
                    cur_limit = soc_mem_field32_get(unit, mem, &pg_config_mem,
                                                    field);
                    if (TH_MMU_SHARED_LIMIT_CHK(cur_limit, thdi_shd/granularity,
                                                post_update)) {
    ...
    if (res & 0x2) { /* if (res & THDO) { */
        ...
        for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
            ...
            if ((cur_limit != 0) &&
                TH_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_db_shd/granularity,
                                        post_update)) {
            ...
            cur_limit = soc_reg_field_get(unit, reg, rval, field);
            if ((cur_limit != 0) &&
                TH_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_qe_shd/granularity,
                                        post_update)) {
                soc_reg_field_set(unit, reg, &rval, field,
        ...
        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
            ...
            for (idx = 0; idx < SOC_TH_MMU_CFG_QGROUP_MAX; idx++) {
                ...
                if (!soc_mem_field32_get(unit, mem, &cfg_qgrp, Q_LIMIT_DYNAMIC_CELLf)) {
                    ...
                    if (TH_MMU_SHARED_LIMIT_CHK(cur_limit, thdo_db_shd/granularity,
                                                post_update)) {
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            mem = SOC_MEM_UNIQUE_ACC(unit, base_mem)[pipe];
            for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
                index1 = SOC_TH_MMU_PIPED_MEM_INDEX(unit, port, mem, idx);
