...
int soc_sb2_ci_config(int unit, int ci)
{
    ...
    soc_reg_field_set(unit, CI_CONFIG6r, &rval, TRFCf, T0fgmss.trfc[0]);
...
int soc_saber2_and28_info_config(int unit, soc_sb2_and28_info_t *drc_info, 
    ...
    drc_info->dram_bitmap = soc_property_get(unit, spn_EXT_RAM_PRESENT, 0x1);
    ...
    drc_info->dram_freq = soc_property_get(unit, spn_EXT_RAM_FREQ, 800);
...
int soc_sb2_and28_dram_init_reset(int unit) 
{
    ...
    sal_memset(&drc_info, 0, sizeof(drc_info));
    
    /* Get drc_info parameters */
    SOC_IF_ERROR_RETURN(soc_saber2_and28_info_config(unit, &drc_info, &sdi));

    /* DRAM info set */
    SOC_IF_ERROR_RETURN(soc_and28_shmoo_dram_info_set(unit, &sdi));
    
    /* Shmoo callback function registration */
    SOC_IF_ERROR_RETURN(soc_and28_shmoo_interface_cb_register(unit, sb2_shmoo_cbi));    
    
    /* initiate master DRCs */
    SOC_IF_ERROR_RETURN(soc_and28_shmoo_phy_init(unit, ci));
...
int soc_sb2_and28_dram_init_config(int unit) 
{
    ...
    for (ci=0; ci<2; ci++) {
        SOC_IF_ERROR_RETURN(soc_sb2_and28_shmoo_dram_init(unit, ci, 0));
        SOC_IF_ERROR_RETURN(soc_sb2_ci_config(unit, ci));
        SOC_IF_ERROR_RETURN(soc_sb2_and28_post_shmoo_dram_init(unit, ci));
        ...
        SOC_IF_ERROR_RETURN(WRITE_BISTGENERALCONFIGURATIONSr(unit, ci, rval));

        SOC_IF_ERROR_RETURN(READ_BISTCONFIGr(unit, ci, &rval));
        soc_reg_field_set(unit, BISTCONFIGr, &rval, BIST_RESETBf, 1);
        SOC_IF_ERROR_RETURN(WRITE_BISTCONFIGr(unit, ci, rval));

    }

    ci=0;
    if (drc_info.auto_tune == 0x1) {
        SOC_IF_ERROR_RETURN(soc_and28_shmoo_ctl(unit, ci, -1, 0,
