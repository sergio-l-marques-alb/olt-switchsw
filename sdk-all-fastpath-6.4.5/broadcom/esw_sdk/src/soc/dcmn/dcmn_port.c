...
soc_dcmn_fabric_pms_add(int unit, int cores_num , int first_port, int use_mutex, int quads_in_fsrd, core_address_get_f address_get_func, void **alloced_buffer)
{
    ...
    for(i = 0; i < cores_num ; i++){
        ...
        port = first_port + i*4;
        if(SOC_PBMP_MEMBER(SOC_CONTROL(unit)->info.sfi.disabled_bitmap, port)) {
            continue;
        }
...
soc_dcmn_fabric_port_probe(int unit, int port, dcmn_port_init_stage_t init_stage, int fw_verify, dcmn_port_fabric_init_config_t* port_config)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_dcmn_port_phy_control_get(unit, port, -1, -1, 0, SOC_PHY_CONTROL_PREEMPHASIS, &pre_curr_def));
        ...
        {
            SOCDNX_IF_ERR_EXIT(soc_dcmn_port_phy_control_set(unit, port, -1, -1, 0, SOC_PHY_CONTROL_PREEMPHASIS, pre_curr));
        }
...
soc_dcmn_port_control_low_latency_llfc_set(int unit, soc_port_t port, int value)
{
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_CONFIG, (_BSL_SOCDNX_MSG("Low latency LLFC contorl is supported only by KR_FEC")));
    }
    if(value){
...
soc_error_t 
soc_dcmn_port_prbs_tx_enable_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;

    PHYMOD_PRBS_DIRECTION_TX_SET(flags);
    SOCDNX_IF_ERR_EXIT(portmod_port_prbs_enable_set(unit, port, mode, flags, value));
...
soc_error_t 
soc_dcmn_port_prbs_rx_enable_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_dcmn_port_prbs_tx_enable_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int *value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;

    PHYMOD_PRBS_DIRECTION_TX_SET(flags);
    SOCDNX_IF_ERR_EXIT(portmod_port_prbs_enable_get(unit, port, mode, flags, value));
...
soc_error_t 
soc_dcmn_port_prbs_rx_enable_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int *value)
{
    int flags = 0;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t 
soc_dcmn_port_prbs_rx_status_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int* value)
{
    ...
    SOCDNX_IF_ERR_EXIT(portmod_port_prbs_status_get(unit, port, mode, flags, &status));
...
soc_error_t 
soc_dcmn_port_prbs_tx_invert_data_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int *invert)
{
    int flags = 0;
    ...
    PHYMOD_PRBS_DIRECTION_TX_SET(flags);
    SOCDNX_IF_ERR_EXIT(portmod_port_prbs_config_get(unit, port, mode, flags, &config));
...
soc_error_t 
soc_dcmn_port_prbs_tx_invert_data_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int invert)
{
    int flags = 0;
    phymod_prbs_t config;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    SOCDNX_IF_ERR_EXIT(portmod_port_prbs_config_set(unit, port, mode, flags, &config));
...
soc_error_t 
soc_dcmn_port_prbs_polynomial_set(int unit, soc_port_t port, portmod_prbs_mode_t mode, int value)
{
    int flags = 0;
...
soc_error_t 
soc_dcmn_port_prbs_polynomial_get(int unit, soc_port_t port, portmod_prbs_mode_t mode, int *value)
{
    int flags = 0;
    phymod_prbs_t config;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_dcmn_port_loopback_set(int unit, soc_port_t port, soc_dcmn_loopback_mode_t loopback)
{
    ...
    if(current_lb != soc_dcmn_loopback_mode_none){
        ...
        SOCDNX_IF_ERR_EXIT(portmod_port_loopback_set(unit, port, portmod_lb_mode, 0));
    }
    if(loopback != soc_dcmn_loopback_mode_none){
        ...
        SOCDNX_IF_ERR_EXIT(portmod_port_loopback_set(unit, port, portmod_lb_mode, 1));
    }
