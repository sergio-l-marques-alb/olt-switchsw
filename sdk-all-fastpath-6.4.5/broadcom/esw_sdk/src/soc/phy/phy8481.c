...
phy_8481_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        if (phy_mdi_pair_map) {
          ...
          pair_swap_value = ((phy_mdi_pair_map & (0x3<<12)) >> 6) | ((phy_mdi_pair_map & (0x3<<8)) >> 4) | 
                    ((phy_mdi_pair_map & (0x3<<4)) >> 2) | (phy_mdi_pair_map & 0x3);
       
                if (PHY_IS_BCM8485X_AND_UP(pc)) {
                    SOC_IF_ERROR_RETURN
                        (PHY84834_TOP_LEVEL_CMD_SET(unit, pc, (uint16)PHY84834_DIAG_CMD_SET_PAIR_SWAP_GENERIC, &pair_swap_value, 1));
                } else 
                {
                    SOC_IF_ERROR_RETURN
