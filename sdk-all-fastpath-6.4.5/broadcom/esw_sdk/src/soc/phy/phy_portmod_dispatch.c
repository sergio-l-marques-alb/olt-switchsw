/*
 * $Id: phy_portmod_dispatch.c.c,v 1.23 2012/08/23 16:30:48  Exp $
...
typedef struct phy_null_saved_data_s
{
    ...
    portmod_portphy_ability_t null_phy_ability[SOC_MAX_NUM_PORTS];
...
phy_portmod_dispatch_ability_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if (NULL != saved_data[unit]) {
       portmod_ext_to_int_cmd_set(unit, port, portmodExtToInt_CtrlCode_Ability, &pdata);
...
phy_driver_t phy_ext_to_int_drv = {
    ...
    phy_portmod_dispatch_set,               /* phy_an_set                   */
