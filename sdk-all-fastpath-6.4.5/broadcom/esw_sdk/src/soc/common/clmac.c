/*
 * $Id: clmac.c,v 1.1.8.2 Broadcom SDK $
...
_mac_cl_drain_cells(int unit, soc_port_t port, int notify_phy)
{
    ...
    if (soc_mmu_is_hw_flush_capable(unit) == FALSE) {
    ...
    SOC_IF_ERROR_RETURN(soc_egress_drain_cells(unit, port, drain_timeout));

    /* Wait until TX fifo cell count is 0 */
    soc_timeout_init(&to, 250000, 0);
    for (;;) {
        SOC_IF_ERROR_RETURN(READ_CLMAC_TXFIFO_CELL_CNTr(unit, port, &rval64));
...
#if 0
/* This code is copied from XLMAC and this routine is expected to be called
 * by speed set */
/*
 * Function:
 ...
 */
STATIC int
_mac_cl_timestamp_delay_set(int unit, soc_port_t port)
{
    uint64 ctrl;
    int delay;

    DBG_100G_VERB(("mac_cl_timestamp_delay_set: unit %d port %s\n",
                   unit, SOC_PORT_NAME(unit, port)));

    SOC_IF_ERROR_RETURN(READ_CLMAC_TIMESTAMP_ADJUSTr(unit, port, &ctrl));

    /* 
     * CLMAC pipeline delay is : ( as in CLMAC specification)
     *          = (7.5 * TX line clock period) + (Timestamp clock period)
     */
    /* 2's complement signed value of pipeline delay in ns */
    delay = SOC_TIMESYNC_PLL_CLOCK_NS(unit) - ((15 * CLMAC_TX_LINE_RATE_NS ) / 2);
    soc_reg64_field32_set(unit, CLMAC_TIMESTAMP_ADJUSTr, &ctrl, 
                           TS_OSTS_ADJUSTf, delay );
    delay = SOC_TIMESYNC_PLL_CLOCK_NS(unit) - ((5 * CLMAC_TX_LINE_RATE_NS ) / 2);
    soc_reg64_field32_set(unit, CLMAC_TIMESTAMP_ADJUSTr, &ctrl, 
                           TS_TSTS_ADJUSTf, delay );

    /* 5x mode delay for clmac: 1 * TX line clock period */
    delay = 1 * CLMAC_TX_LINE_RATE_NS;
    soc_reg64_field32_set(unit, CLMAC_TIMESTAMP_ADJUSTr, &ctrl, 
 ...
 */
STATIC int
_mac_cl_port_mode_update(int unit, soc_port_t port, int to_hg_port)
{
    uint32              rval;
    uint64              val64;
    soc_pbmp_t          ctr_pbmp;
...
mac_cl_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    if (to_hg_port != -1) {
        ...
        /* CE -> HG */
        SOC_IF_ERROR_RETURN(_mac_cl_port_mode_update(unit, port, to_hg_port));
