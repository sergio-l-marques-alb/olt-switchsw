...
static uint64 gtf_stat_value[16];
#endif
  ...
  )
{
  ...
  write_to = 0;
  exp_payload_long[0] = 0;
  exp_payload_long[1] = 0;
  long_indx = 0;

  for (char_indx = 0; char_indx < SOC_SAT_PAYLOAD_MAX_PATTERN_NOF_U8; ++char_indx)
  {
    ...
    )
{
...
#ifdef BCM_SAT_GTF_STAT_FIX
    for (i = 0; i < 16; i++) {
        COMPILER_64_ZERO(gtf_stat_value[i]);
    }
#endif
    ...
    /* 3. number of system clock cycles in one second */
    ...
    )
{
    ...
    /* set packet gen for cir and eir */
    for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        COMPILER_64_ADD_32(gtf_stat_value[index], stat_val);
#endif
    ...
    )
{
    ...
    index = SAT_TX_PER_GENERATOR_INDX(gtf_id, 0);
    ...
    )
{
    ...
    uint32 payload_rpt_pattern[SOC_SAT_PAYLOAD_MAX_PATTERN_NOF_U32];
    uint32 payload_type;   
    ...
    uint32 pkt_add_tlv = 0, pkt_add_crc = 0;
    uint32 offset = 0;
    int i = 0;
    int priority = 0;
    ...
    if (config->payload.payload_type == socSatPayloadConstant8Bytes) {
        soc_sat_packet_config_exp_payload_uint8_to_long(config->payload.payload_pattern, payload_rpt_pattern);
        payload_type = 0;
    }
   ...
   /* For writing on dynamic tables (in the OAMP), have to enable dynamic memory access */
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in read OAMP_SAT_TX_EVC_PARAMS_ENTRY_1m\n"), soc_errmsg(rv)));
        goto exit;
    }
    /* PKT_HEADER_LSBf */
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in write OAMP_SAT_TX_EVC_PARAMS_ENTRY_1m\n"), soc_errmsg(rv)));
        goto exit;
    }
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in read OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m\n"), soc_errmsg(rv)));
        goto exit;
    }

    soc_mem_field_set(unit, OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m, tx_evc_params_entry_2_data, PKT_HEADER_MSBf, pkt_hdr_tbl_info.pkt_hdr_msb);
    soc_mem_field_set(unit, OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m, tx_evc_params_entry_2_data, PKT_HEADER_LENGTHf, &(pkt_hdr_tbl_info.pkt_hdr_len));
    soc_mem_field_set(unit, OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m, tx_evc_params_entry_2_data, RPT_PATTERNf, payload_rpt_pattern);
    soc_mem_field_set(unit, OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m, tx_evc_params_entry_2_data, PAYLOAD_TYPEf, &payload_type);
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in write OAMP_SAT_TX_EVC_PARAMS_ENTRY_2m\n"), soc_errmsg(rv)));
        goto exit;
    }
    ...
    for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, PKT_LENGTH_NEXTf, (uint32*)&(config->packet_edit[priority].packet_length[0]));
        ...
        soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, CRC_32_ENABLEf, &pkt_add_crc);
    ...
    )
{
    ...
    soc_reg_above_64_val_t data;
    int index;
    uint32 rate_mnt_nof_bits = 0, rate_exp_nof_bits = 0;
    ...
    rate_mnt_nof_bits = soc_reg_field_length(unit, OAMP_SAT_GEN_RATE_CONTROLr, THRESHOLD_UPPER_MNT__Nf);
    rate_exp_nof_bits = soc_reg_field_length(unit, OAMP_SAT_GEN_RATE_CONTROLr, THRESHOLD_UPPER_EXP__Nf);
    rv = _soc_sat_gtf_rate_to_internal_rate_convert(unit, 
    ...
    /* 2. calculate hardware values for low threshold */
    rv = _soc_sat_gtf_rate_to_internal_rate_convert(unit, 
    ...
    )
{
    ...
    config->rate_pattern_mode = soc_reg_above_64_field32_get(unit, OAMP_SAT_GEN_RATE_CONTROLr, data, BURST_MODE__Nf);
...
#ifdef BCM_SAT_GTF_STAT_FIX
        COMPILER_64_ADD_64(*value, gtf_stat_value[index]);

        COMPILER_64_ZERO(gtf_stat_value[index]);
#endif
    }
    ...
    )
{
    ...
    uint32 exp_rpt_pattern[SOC_SAT_PAYLOAD_MAX_PATTERN_NOF_U32];
    ...
    /* EXP_RPT_PATTERNf */
    offset = 64;
    soc_sat_packet_config_exp_payload_uint8_to_long(packet_info->payload.payload_pattern, exp_rpt_pattern);
    SHR_BITCOPY_RANGE(reg_above64_val, offset, exp_rpt_pattern, 0, 64);

    /* ADD_END_TLVf */
    if (packet_info->flags & SOC_SAT_CTF_PACKET_INFO_ADD_END_TLV) {
        field = 1;
        offset = 128;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 1);
    }
    /* ADD_CRCf */
    if (packet_info->flags & SOC_SAT_CTF_PACKET_INFO_ADD_CRC) {
        field = 1;
        offset  = 129;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 1);
    }
    ...
    if (packet_info->payload.payload_type == socSatPayloadConstant4Bytes) {
        ...
        field = 1;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 1);
        /* USE_PRBS_PAYLOADf  and USE_8_BYTE_PATTERNf */
        offset = 147;
        field = 0;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 2);
    }
        ...
        /* USE_4_BYTE_PATTERNf and USE_PRBS_PAYLOADf */
        offset = 146;
        field = 0;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 2);
        /* USE_8_BYTE_PATTERNf */
    ...
    )
{
    ...
    soc_mem_field32_set(unit, OAMP_SAT_RX_FLOW_PARAMSm, &data, CALC_EQUAL_FDV_BINSf, stat->use_global_bin_config);
    ...
    )
{
    ...
    if (reports->report_sampling_percent == 0){
        ...
        field = (reports->report_sampling_percent*255/100);
    }
