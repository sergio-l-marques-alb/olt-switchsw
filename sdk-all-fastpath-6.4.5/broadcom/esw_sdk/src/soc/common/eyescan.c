...
int is_eyescan_algorithm_legacy_mode(int unit, uint32 nof_ports, soc_port_t* ports)
{
        int legacy;
        phy_ctrl_t *pc;
    ...
    for(j = 0 ; j < nof_ports ; j++) {
        ...
        if(soc_feature(unit, soc_feature_portmod)) {
           ...
           intf = PHY_DIAG_INTF_DFLT;
            ...
            params.lane = lane_num ? lane_num[j] : -1;
            ...
            rv = portmod_port_phy_lane_access_get(unit, ports[j], &params, PHYMOD_CONFIG_MAX_CORES_PER_PORT, access, &nof_phys, NULL);
            SOC_IF_ERROR_RETURN(rv);

            for(idx = 0 ; idx < nof_phys ; idx++) {
                pa = &(access[idx]);

                if(ignore_down_ports) {
                ...
                if(line_rates) {
                    rv = portmod_port_interface_config_get(unit, ports[j], &if_config);
                    SOC_IF_ERROR_RETURN(rv);
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    for(i=0; i<nof_ports; i++ ) {
        if ( phy_acc[i].access.lane_mask == 0 ) {
            LOG_ERROR(BSL_LS_SOC_PHY, (BSL_META_U(unit, "Illegal lane_mask of 0 on port %d\n"), i));
            sal_free(phy_acc);
            return SOC_E_INTERNAL; 
        }
    }
    for(i=0; i<nof_ports; i++ ) {
        if ( lane_num == NULL ) {
            switch(phy_acc[i].access.lane_mask) {
                case 0x1: phy_acc[i].access.lane_mask = 1 << 0; break;
                case 0x2: phy_acc[i].access.lane_mask = 1 << 1; break;
                case 0x4: phy_acc[i].access.lane_mask = 1 << 2; break;
                case 0x8: phy_acc[i].access.lane_mask = 1 << 3; break;
                case 0xc: phy_acc[i].access.lane_mask = 1 << 2; break;
                default: phy_acc[i].access.lane_mask = 1; 
            }
        } else {
            phy_acc[i].access.lane_mask = 0x1 << *lane_num; 
        }
    }
    /*enable eyescan*/
    rv = phymod_diag_eyescan_run(phy_acc, line_rates, phy_acc_length, phymodEyescanModeLowBER, &eyescan_options);
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    /* Run eyescan */
    rv = phymod_diag_eyescan_run(phy_acc, NULL, phy_acc_length, phymodEyescanModeFast, NULL);
    ...
    soc_port_phy_eyescan_results_t* results /*array of result per port*/) 
{
    ...
    return (is_eyescan_algorithm_legacy_mode(unit, nof_ports, ports)?  
               _soc_port_phy_eyescan_run  : 
               (params->type == 1 ?  _soc_port_phy_eyescan_run_uc:
                                     _soc_port_phy_eyescan_run_lowber))
