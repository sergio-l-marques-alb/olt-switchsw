...
_bcm_l3_defip_deinit(unit)
{
    ...
    if (soc_property_get(unit, spn_L3_ALPM_ENABLE, 0)) {
        int rv = BCM_E_NONE;
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_th_alpm_deinit(unit);
        } else {
            rv = soc_alpm_deinit(unit);
        }
        if (BCM_SUCCESS(rv)) {
            LOG_VERBOSE(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit,
                                    "ALPM mode deinitialized\n")));
        } else {
            LOG_ERROR(BSL_LS_BCM_ALPM,
                      (BSL_META_U(unit,
                                  "ALPM mode deinitialization failed, "
                                  "retVal = %d\n"), rv));
        }
        return rv;
    }
    ...
    if (soc_property_get(unit, spn_L3_ALPM_ENABLE, 0)) {
        LOG_WARN(BSL_LS_BCM_ALPM,
                 (BSL_META_U(unit,
                             "ALPM mode support is not compiled. Please, "
                             "recompile the SDK with ALPM_ENABLE preprocessor variable "
                             "defined\n"))); 
    }
#endif /* ALPM_ENABLE */
