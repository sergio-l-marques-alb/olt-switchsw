                                 ...
                                 int *expanded, int *slice_master_idx)
{
    ...
    if (stage_fc->stage_id == _BCM_FIELD_STAGE_INGRESS) {
        for (vmap = 1; vmap < _FP_VMAP_CNT; ++vmap) {
            for (vslice = 0; vslice < slice_num; vslice++) {
                stage_fc->vmap[vmap][vslice] = stage_fc->vmap[0][vslice];
            }
        }
    }
...
_field_entry_info_retrieve(int unit, bcm_field_entry_t *eid, int *prio,
                           ...
                           bcm_field_stat_t *sid, bcm_policer_t *pid, 
                           uint8 *dvp_type, _field_entry_wb_t *f_ent_wb)
{
    ...
    if (fc->flags & _FP_STABLE_SAVE_LONG_IDS) {
        ...
        if (stat_present) {
            *sid |= buf[fc->scache_pos] & 0xF; /* SID_lo */
            if (pol_present) {
                *pid |= (buf[fc->scache_pos] & 0xF0) << 4; /* PID_hi */
            }
            fc->scache_pos++;
        ...
        if (fc->l2warm) {
            ...
            BCM_IF_ERROR_RETURN
                (_field_group_info_retrieve(unit, port, &gid, &priority, 
                                            &action_res_id, &group_flags, &qset, fc));
            if (gid != -1) {
...
{
    uint32  *e;                         /* Buffer to zero TCAM & Policy entry.*/
    uint32  **p;                        /* For policies  */
    int     i;
...
#define BCM_IF_ERROR_RETURN_WITH_FREE(op, e, p, max_idx) {\
         do  { int __rv__, __i__; \
         if ((__rv__ = (op)) < 0) { \
            sal_free(e);\
            for (__i__ = 0; __i__ < max_idx; __i__++) {\
                sal_free(p[__i__]); }\
            sal_free(p);\
            return(__rv__);\
         } }while(0); }
    ...
    e = (uint32 *) sal_alloc(SOC_MAX_MEM_FIELD_WORDS*4, "ACL Control");
        if (!e) {
            return (BCM_E_MEMORY);
        }

        p = (uint32 **) sal_alloc(_FP_MAX_ENTRY_WIDTH *4, "ACL Control");
        if (!p)
        {
            sal_free(e);
            return (BCM_E_MEMORY);
        ...
        {
            p[i]= (uint32 *) sal_alloc (SOC_MAX_MEM_FIELD_WORDS *4, "ACL Control");
            if (!p[i])
            {
                while (i > 0) {
                    sal_free(p[--i]);
    ...
    for (idx = 0; idx < parts_count; idx++) {
        ...
        if ((tcam_idx_old[idx] < tcam_idx_min) || (tcam_idx_old[idx] > tcam_idx_max) ||
            ...
            BCM_IF_ERROR_RETURN_WITH_FREE(BCM_E_PARAM, e, p, _FP_MAX_ENTRY_WIDTH);
        }

        /* Read policy entry from current tcam index. */
        rv = soc_mem_read(unit, policy_mem, MEM_BLOCK_ANY, tcam_idx_old[idx], p[idx]);
        BCM_IF_ERROR_RETURN_WITH_FREE(rv, e, p, _FP_MAX_ENTRY_WIDTH);
