...
_bcm_esw_l3_sync(int unit)
{
...
#endif
#ifdef BCM_TOMAHAWK_SUPPORT   
    SHR_BITDCL egflgbitmap2k[_SHR_BITDCLSIZE(BCM_L3_ECMP_MAX_2K_GROUPS)] = {0};
    int j;
    SHR_BITDCL eglevel1flgbitmap[_SHR_BITDCLSIZE(BCM_L3_ECMP_MAX_2K_GROUPS)] = {0};
    SHR_BITDCL eglevel2flgbitmap[_SHR_BITDCLSIZE(BCM_L3_ECMP_MAX_2K_GROUPS)] = {0};
#endif /* BCM_TOMAHAWK_SUPPORT */
#ifdef BCM_TRIUMPH_SUPPORT
    ...
    if (soc_feature(unit, soc_feature_l3) &&
        ...
        /* ECMP Level 1 flags */
        alloc_sz += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
        /* ECMP Level 2 flags */
        alloc_sz += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
   ...
   if (soc_feature(unit, soc_feature_vxlan)) {
...
#endif /* BCM_TRIDENT2_SUPPORT */ 
#ifdef BCM_TOMAHAWK_SUPPORT
    /* Determine ECMP_GROUP resorting flags for the additional 1K groups */
    if (soc_feature(unit, soc_feature_l3) &&
        soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
        alloc_sz += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
    }
    ...
    if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
        ...
        alloc_sz += sizeof(uint16) *
                    BCM_XGS3_L3_ECMP_MAX_GROUPS(unit);
        /* Group, member tuples */
        alloc_sz += (RH_ECMP_MEMBER_TBL_MAX * 
    ...
    if (soc_feature(unit, soc_feature_l3) && !soc_feature(unit, soc_feature_l3_no_ecmp)) {
        ...
        if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
            for (i = 0; i <= ecmp_tbl_ptr->idx_max; i++) {
                if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, i) > 0) {
                    SHR_BITSET(egbitmap2k, i);
                }
            }
            sal_memcpy(l3_scache_ptr, egbitmap2k,
...
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            ...
            sal_memcpy(l3_scache_ptr, egbitmap,
    ...
    if (soc_feature(unit, soc_feature_l3) && !soc_feature(unit, soc_feature_l3_no_ecmp)) {
        ...
        if (soc_feature(unit, soc_feature_l3_ecmp_2k_groups)) {
            for (i = 0; i <= ecmp_tbl_ptr->idx_max; i++) {
                if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, i) > 0 &&
                    ...
                    SHR_BITSET(egflgbitmap2k, i);
                }
            }
            sal_memcpy(l3_scache_ptr, egflgbitmap2k,
...
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            ...
            sal_memcpy(l3_scache_ptr, egflgbitmap,
                       SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS));  
            l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_MAX_ECMP_GROUPS);
        }
    }
        ...
        for (j = 0; j <= ecmp_tbl_ptr->idx_max; j++) {
            if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, j) > 0 &&
                (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit, j,
                               BCM_L3_ECMP_OVERLAY))) {
                SHR_BITSET(eglevel1flgbitmap, j);
            }
            if (BCM_XGS3_L3_ENT_REF_CNT(ecmp_tbl_ptr, j) > 0 &&
                (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit, j,
                               BCM_L3_ECMP_UNDERLAY))) {
                SHR_BITSET(eglevel2flgbitmap, j);
            }
        }
        sal_memcpy(l3_scache_ptr, eglevel1flgbitmap,
                       SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS));
        l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
        sal_memcpy(l3_scache_ptr, eglevel2flgbitmap,
                       SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS));
        l3_scache_ptr += SHR_BITALLOCSIZE(BCM_L3_ECMP_MAX_2K_GROUPS);
    }
    if (soc_feature(unit, soc_feature_l3) &&
        ...
        BCM_IF_ERROR_RETURN
                (bcm_th_l3_ecmp_rh_member_sync(unit, &l3_scache_ptr));
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
bcm_esw_l3_route_add(int unit, bcm_l3_route_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_ipmc_defip)) { 
         ...
         */
        if((info_local.l3a_expected_intf > _BCM_DEFIP_IPMC_EXPECTED_L3IIF_MAX) ||
...
bcm_esw_l3_route_get(int unit, bcm_l3_route_t *info)
{
    ...
    if (defip.defip_flags & BCM_L3_IPMC ) {
        _BCM_MULTICAST_GROUP_SET(info->l3a_mc_group, _BCM_MULTICAST_TYPE_L3,
                                 info->l3a_mc_group);
    }
        ...
        int intf_count, bcm_if_t *intf_array, int op, int count, bcm_if_t *intf)
{
...
#ifdef BCM_TOMAHAWK_SUPPORT
    bcm_if_t *intf_array_rh_copy = NULL, *new_intf_array = NULL;
    bcm_if_t *temp_intf_arr_ptr = NULL; 
    int intf_count_rh_copy = 0;
    int alloc_size;
    int cnt;
    uint32 temp_flags = 0;
    int pow = 0;
    int temp_max_paths = 0;
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (soc_feature(unit, soc_feature_riot)) {
            /*
             * Do nothing for now. if H-ECMP is enabled
             * with riot feature in future then we can handle
             * testing for these flags here.
             */
        } else
#endif
        {
        ...
        if (soc_feature(unit, soc_feature_hierarchical_ecmp)) {
            ...
            if ((ecmp->ecmp_group_flags & BCM_L3_ECMP_UNDERLAY) &&
                (ecmp->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)) {
                L3_UNLOCK(unit);
                return (BCM_E_PARAM);
            }
            /* When specified, max path should be within the limit for the mode */
            if (((BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode == 
                                              ECMP_MODE_HIERARCHICAL) ||
                (BCM_XGS3_L3_TBL(unit, ecmp_info).ecmp_mode ==
                                              ECMP_MODE_SINGLE_LOW_LATENCY)) &&
                (ecmp->dynamic_mode != BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)) {
                if ((ecmp->max_paths > 128) && !(ecmp->ecmp_group_flags & BCM_L3_ECMP_RH_OPT)) {
                    L3_UNLOCK(unit);
                    return (BCM_E_PARAM);
                }
            }
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
            ...
            if (op == BCM_L3_ECMP_MEMBER_OP_SET) {
                ...
                if (ecmp->dynamic_size < RH_OPT_MIN_PATHS ||
                    ecmp->dynamic_size > RH_OPT_MAX_PATHS) {
                    L3_UNLOCK(unit);
                    return BCM_E_PARAM;
                }
            }
            ...
            /* Keep a copy of the original intf_array */
            temp_intf_arr_ptr = intf_array;
            temp_flags = ecmp->flags;
            temp_max_paths = ecmp->max_paths;
            intf_array_rh_copy = sal_alloc(sizeof(bcm_if_t) * intf_count,
                                    "ECMP RH intf array");
            if (NULL == intf_array_rh_copy) {
                L3_UNLOCK(unit);
                return(BCM_E_MEMORY);
            }
            sal_memset(intf_array_rh_copy, 0, sizeof(bcm_if_t) * intf_count); 
            sal_memcpy(intf_array_rh_copy, intf_array, sizeof(bcm_if_t) * intf_count);
            ...
            rv = bcm_th_l3_egress_ecmp_rh_create(unit, ecmp, intf_count,
            ...
            /* Check if this is the last member of the ECMP group */
            if ((intf_count_rh_copy == 1) && (op == BCM_L3_ECMP_MEMBER_OP_DELETE)) {
                /*
                 * Dec ref cnt of each member from the ECMP group, where the member
                 * to be deleted appears dynamic_size times
                 */
                rv = _bcm_xgs3_l3_egress_intf_ref_count_update(unit, ecmp->dynamic_size,
                                                               new_intf_array, -1);
            ...
            /* intf_arr and count updated with RH entry set and count
             * before calling multipath functions to create RH entry
             * set in member table
             */
            intf_array = new_intf_array;
            intf_count = ecmp->dynamic_size;
            ecmp->flags = temp_flags;
            /* Do not sort if load balancing mode is RH on Tomahawk. */
            ecmp->ecmp_group_flags |= BCM_L3_ECMP_PATH_NO_SORTING;
            ecmp->ecmp_group_flags |= BCM_L3_ECMP_RH_OPT;
            ecmp->max_paths = ecmp->dynamic_size;
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
        ...
        if (BCM_FAILURE(rv)) {
            ...
            if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
                (ecmp->dynamic_mode == BCM_L3_ECMP_DYNAMIC_MODE_RESILIENT)) {
                intf_count = intf_count_rh_copy;
                intf_array = temp_intf_arr_ptr;
                sal_free(intf_array_rh_copy);
                sal_free(new_intf_array);
            }
#endif /* BCM_TOMAHAWK_SUPPORT */
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized) &&
            ...
            intf_count = intf_count_rh_copy;
            intf_array = temp_intf_arr_ptr;
            ecmp->max_paths = temp_max_paths;
            if (ecmp->max_paths == 0) {
                ecmp->max_paths = BCM_XGS3_L3_ECMP_MAX_PATHS(unit);
            }
            rv = bcm_th_ecmp_rh_set_intf_arr(unit, intf_count_rh_copy,
                            intf_array_rh_copy, 
                            (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                            ecmp->dynamic_size, ecmp->max_paths);
            rv = bcm_th_ecmp_lb_mode_set(unit, 
                                  (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
                                  BCM_ECMP_LB_MODE_RH);
            sal_free(intf_array_rh_copy);
            sal_free(new_intf_array);
            ...
            /* Reset the lb mode value only on Tomahawk */
            if (SOC_IS_TOMAHAWK(unit)) {
                rv = bcm_th_ecmp_lb_mode_set(unit, 
                                  (ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN),
    ...
    if ((soc_feature(unit, soc_feature_ecmp_resilient_hash) ||
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
            rv = bcm_th_ecmp_rh_get_multipath(unit, 
                ...
                BCM_XGS3_L3_ECMP_MAX(unit), old_intf_array, &old_intf_count,
                NULL);
            if (BCM_FAILURE(rv)) {
                goto clean_up;
            }
...
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            rv = _bcm_xgs3_l3_ecmp_egress_if_get(unit, ecmp_info, 
                                                 BCM_XGS3_L3_ECMP_MAX(unit),
                                                 old_intf_array,
                                                 &old_intf_count);
            if (BCM_FAILURE(rv)) {
    ...
    int *ecmp_member_count)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && 
        soc_feature(unit, soc_feature_l3)) {
        L3_LOCK(unit);
        rv = bcm_xgs3_l3_ecmp_get(unit, ecmp_info, ecmp_member_size, 
    ...
    bcm_l3_ecmp_member_t *ecmp_member)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) &&
        ...
        intf_array[ecmp_member_count] = ecmp_member->egress_if;
...
bcm_esw_l3_egress_ecmp_destroy(int unit, bcm_l3_egress_ecmp_t *ecmp) 
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (BCM_SUCCESS(rv) &&
            soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
            rv = bcm_th_l3_egress_ecmp_rh_destroy(unit, ecmp->ecmp_intf);
        }
        if (BCM_SUCCESS(rv) && SOC_IS_TOMAHAWK(unit)) {
            rv = bcm_th_ecmp_lb_mode_reset(unit, ecmp->ecmp_intf);
        }
        if (BCM_SUCCESS(rv) && soc_feature(unit, soc_feature_hierarchical_ecmp)) {
            BCM_XGS3_L3_ECMP_GROUP_FLAGS_RESET(unit, ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN,
                                               BCM_L3_ECMP_UNDERLAY);
            BCM_XGS3_L3_ECMP_GROUP_FLAGS_RESET(unit, ecmp->ecmp_intf - BCM_XGS3_MPATH_EGRESS_IDX_MIN,
                                               BCM_L3_ECMP_OVERLAY);
        }
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (!BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf)) {
            if (!soc_feature(unit, soc_feature_hierarchical_ecmp)) {
                ...
                /* In Hierarchical ecmp mode, member can be another ECMP group */
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (soc_feature(unit, soc_feature_ecmp_resilient_hash_optimized)) {
            if (rv == BCM_E_NOT_FOUND) {
                rv = bcm_th_l3_egress_rh_ecmp_find(unit, intf_count,
                                                   intf_array, 
                                                   &ecmp->ecmp_intf);
            }
        }
