...
/*
 * Function:
 *     _field_th_presel_stage_qualifiers_free
 * Purpose:
 *     De-initialize presel stage qaualifiers descriptors. 
 * Parameters:
 *     unit       - (IN) BCM device number.
 ...
 */
STATIC int
_field_th_presel_stage_qualifiers_free(int unit, _field_stage_t *stage_fc)
{
                           ...
                           _field_stage_t *stage_fc)
{
    ...
    /* Free qualifiers. */
    (void) (_field_th_presel_stage_qualifiers_free(unit, stage_fc));
                                 ...
                                 _field_stage_t *stage_fc)
{
    ...
    /* Deallocate Field Control Preselector information */
    _field_th_presel_info_free(unit, fc, stage_fc);
                                  ...
                                  uint32 *p_data, uint32 *p_mask) 
{
    ...
    uint32                    null_key[SOC_MAX_MEM_BYTES] = {0};
    ...
    /* Free the TCAM key and mask if no qualifiers are assigned to the entry. */
    if ((f_presel->lt_tcam.key != NULL) && (f_presel->lt_tcam.mask != NULL)) {
       if (!sal_memcmp(f_presel->lt_tcam.key, null_key,
                       f_presel->lt_tcam.key_size) &&
           !sal_memcmp(f_presel->lt_tcam.mask, null_key,
                       f_presel->lt_tcam.key_size)) {
           sal_free(f_presel->lt_tcam.key);
           sal_free(f_presel->lt_tcam.mask);
           f_presel->lt_tcam.key_size = 0;
           f_presel->lt_tcam.key = NULL;
           f_presel->lt_tcam.mask = NULL;
           LOG_DEBUG(BSL_LS_BCM_FP, (BSL_META_U(unit,
                "%s(): Freeing Key and Mask for Presel Entry[%d]."),
                __func__, f_presel->presel_id));
       }
    } 
    return (rv);
}
                                              ...
                                              int slice)
{
    ...
    /* Get logical table info for the default FP instance. */
    lt_info = stage_fc->lt_info[fg->instance][lt_id];
                                     ...
                                     _field_slice_t *fs)
{
    ...
    */
    for (fs_ptr = fg->slices; fs_ptr != NULL; fs_ptr = fs_ptr->next) {
        ...
        if (found == 1) {
           fg->lt_info->lt_part_pri[fs_ptr->slice_number] += 1;
        }
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_FP,
            (BSL_META_U(unit, "Error: LT Partition Reset failed for"
             ...
             fg->gid, fg->lt_info->lt_id, fs->slice_number));
    }
                                         ...
                                         int *lt_part_prio)
{
    ...
    for (group_fs = fg->slices; group_fs != NULL; group_fs = group_fs->next) {
        part_prio = fg->lt_info->lt_part_pri[group_fs->slice_number];
    }
