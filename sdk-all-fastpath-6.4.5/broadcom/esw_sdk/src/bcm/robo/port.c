 ...
 *      BCM_E_XXX
 * Notes:
 *      Available on Thunderbolt only for ROBO.
 */
int
 ...
 *      2. In Robo chip, the pri_map->color and cfi parameters are supported for TB only
    ...
    bcm_vlan_t vid, int pkt_pri, int cfi, bcm_priority_mapping_t *pri_map)
{
    ...
    if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
        /* currently, the cfi and color are used for TB only on ROBO chip for PCP2TC */
