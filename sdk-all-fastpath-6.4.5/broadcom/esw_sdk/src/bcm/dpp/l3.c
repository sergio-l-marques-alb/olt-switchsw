...
#define DPP_L3_LOCK_RELEASE \
    if ((unit) >= 0 && ((unit) < (BCM_MAX_NUM_UNITS)) && \
        ...
        && sal_mutex_give(sw_state_sync_db[(unit)].dpp.l3_lock)) { \
    ...
    */
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            _BCM_PETRA_L3_MULTIPLE_MYMAC_MODE(unit) = current_multiple_mymac_mode;
        }
    }
    ...
    int unit)
{
    ...
    if (init == TRUE) {
        ...
        BCMDNX_IF_ERR_EXIT(L3_ACCESS.dpp_l3_state.bcm_tunnel_intf_to_eep.free(unit));
        ...
        BCMDNX_IF_ERR_EXIT(L3_ACCESS.dpp_l3_state.free(unit));
        BCMDNX_IF_ERR_EXIT(L3_ACCESS.free(unit));
  ...
  )
{
    ...
    int lif_to_gport_flags = _BCM_DPP_LIF_TO_GPORT_EGRESS | _BCM_DPP_LIF_TO_GPORT_GLOBAL_LIF | _BCM_DPP_LIF_TO_GPORT_USE_SW_STATE;
    ...
    while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter)))
    {
        ...
        for(idx = 0; idx < nof_entries; idx++)
        {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, src_bind_ipv4s[idx].lif_ndx, lif_to_gport_flags, &entry.port));
            /* Invoke user callback. */
    ...
    for (vrf_ndx=BCM_L3_VRF_DEFAULT; vrf_ndx<vrf_max; vrf_ndx++)
    {
      ...
      while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&block_range.start.payload))
      {
        ...
        for(idx = 0; idx < nof_entries; idx++)
        {
          ...
          if (fec_ids[idx])
          {
              BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, fec_ids[idx], lif_to_gport_flags, &entry.port));
              if (vrf_ndx == BCM_L3_VRF_DEFAULT)
              {
    ...
    while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&block_range_key.start.payload))
    {
      ...
      for(idx = 0; idx < nof_entries; idx++)
      {
          ...
          rv = _bcm_l3_sand_ipv6_addr_to_bcm_ipv6_addr(unit,
                               ...
                               &(entry.ip6));
          BCMDNX_IF_ERR_EXIT(rv);
          BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_gport(unit, src_bind_ipv6s[idx].lif_ndx, lif_to_gport_flags, &entry.port));
