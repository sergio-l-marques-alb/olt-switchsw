    ...
    int unit)
{
    ...
    if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)) {
        ...
        rv = sw_state_access[unit].dpp.bcm.alloc_mngr_glif.free(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    rv = sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);
