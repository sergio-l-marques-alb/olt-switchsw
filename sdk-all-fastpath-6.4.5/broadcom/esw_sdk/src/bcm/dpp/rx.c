    ...
    int unit,SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO* prm_profile_info,bcm_rx_trap_config_t *config)
{
    ...
    if (prm_profile_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST) {        
        if (prm_profile_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_MC) {
            ...
            {
                _BCM_DPP_GPORT_ENCAP_VAL_SET(config->encap_id, _BCM_DPP_GPORT_ENCAP_TYPE_OUTLIF , _BCM_DPP_GPORT_ENCAP_USAGE_GENERAL ,  prm_profile_info->dest_info.frwrd_dest.additional_info.outlif.val);
...
bcm_petra_rx_detach(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(RX_ACCESS.free(unit));
    ...
    int trap_id)
{
    ...
    if (!_BCM_TRAP_ID_IS_EGRESS(trap_id_sw)) {    
        if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id_sw)) {
...
int bcm_petra_rx_trap_type_get(int unit, int flags, bcm_rx_trap_t type, int *trap_id)
{
    ...
    {    
        _bcm_dpp_rx_ppd_trap_get(unit, type, &trap_id_sw);
    ...
    if (!_BCM_TRAP_ID_IS_EGRESS(trap_id_sw)) {
        ...
        if (config->flags & (BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_DEST_MULTICAST)) {
            ...
            else
            {
                ...
                {
                    prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST_OUTLIF;
  
                    if ( BCM_GPORT_IS_SET(config->encap_id)) 
                    {
                        _bcm_dpp_field_gport_to_lif_convert(unit, config->encap_id, 0 /*out lif */, &out_lif_id);
                    }
                    else
                    {
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_POLICER) {
            ...
            if (SOC_IS_ARADPLUS_AND_BELOW(unit))
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_RX_TRAP_UPDATE_POLICER flag is unsupported for Arad Plus or Arad devices"))); 
            }
        ...
        if (config->flags & BCM_RX_TRAP_UPDATE_DEST) {
            ...
            if (config->flags & BCM_RX_TRAP_UPDATE_ENCAP_ID) 
            {
                ...
                rv = _bcm_dpp_field_gport_to_pp_port_tm_port_convert(unit, config->dest_port, &pp_port, &tm_port);
                BCM_IF_ERROR_RETURN(rv);
        ...
        if (config->flags & BCM_RX_TRAP_DROP_AND_LEARN) {
             prm_egr_profile_info.bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST;
             prm_egr_profile_info.out_tm_port = _BCM_DPP_TRAP_INVALID_PORT;
        }
    ...
    if (!_BCM_TRAP_ID_IS_EGRESS(trap_id_sw)) {    
        ...
        if (_BCM_TRAP_ID_IS_VIRTUAL(trap_id_sw)) {
            ...
            if (  prm_profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_FRWRD_OFFSET) {
                  ...
                  config->forwarding_header = prm_profile_info.processing_info.frwrd_offset_index; 
            } 
        }
        ...
        rv = _bcm_dpp_am_eg_trap_index_get(unit, 
                                           ...
                                           &trap_index); /* 0...NOF_TRAPS */
        BCMDNX_IF_ERR_EXIT(rv);
        rv =_bcm_dpp_am_template_trap_egress_data_get(unit, trap_id_sw, trap_index, &prm_egr_profile_info);
        BCM_IF_ERROR_RETURN(rv);
        ...
        if (prm_egr_profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST) {
            ...
            if (prm_egr_profile_info.out_tm_port == _BCM_DPP_TRAP_INVALID_PORT) 
            {
            ...
            {      
                ...
                if (SOC_IS_JERICHO(unit))
                {
                    ...
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Not supported: need to extract the core id.")));

                }
                else
                {
                    BCMDNX_IF_ERR_EXIT(bcm_petra_stk_my_modid_get(unit, &my_modid));
                    BCM_GPORT_MODPORT_SET(config->dest_port, my_modid, prm_egr_profile_info.out_tm_port); 
                }

                if (prm_egr_profile_info.bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_CUD) {
                    _BCM_DPP_GPORT_ENCAP_VAL_SET(config->encap_id, _BCM_DPP_GPORT_ENCAP_TYPE_OUTLIF , _BCM_DPP_GPORT_ENCAP_USAGE_GENERAL , prm_egr_profile_info.header_data.cud);       
                    config->flags |= BCM_RX_TRAP_UPDATE_ENCAP_ID;
                }
            }
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        }else
        {
            ...
            if (prm_profile_info.cmd.outlif.value != 0)
            {
                   ...
                   _BCM_DPP_GPORT_ENCAP_VAL_SET(config->encap_id, _BCM_DPP_GPORT_ENCAP_TYPE_OUTLIF , _BCM_DPP_GPORT_ENCAP_USAGE_GENERAL , prm_profile_info.cmd.outlif.value);
            }
        }
    ...
    uint8 device_access_allowed)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if (found) {
            BCMDNX_IF_ERR_EXIT(soc_port_sw_db_hdr_type_out_get(unit, port_i, &header_type_out)); 
                ...
                if (device_access_allowed) {
                    sand_rv = arad_ports_logical_sys_id_parse(unit, packetInfo.src_sys_port, 
                     ...
                     if (is_lag) {
                        ...
                        /* Get LAG info */
                        rv = _bcm_dpp_trunk_sw_db_get(unit, lag_id, &lag_info);
                        BCM_SAND_IF_ERR_EXIT(rv);
