...
_bcm_petra_mirror_detach(int unit)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    /* tear down this unit */
    if ( !BCM_FAILURE(MIRROR_ACCESS.ingress.free(unit))
        && !BCM_FAILURE(MIRROR_ACCESS.free(unit))) {
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if(mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_IP_GRE) {
            *outlif_valid = 1;
            ...
            tmp_global_outlif = _BCM_PETRA_L3_ITF_VAL_GET(mirror_dest->tunnel_id);
            ...
            if(mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_WITH_SPAN_ID) {/*span_id*/
                ...
                else if (SOC_IS_ARADPLUS(unit))/* ARAD+ or above*/
                {
...
bcm_petra_mirror_destination_tunnel_destroy(int unit, int global_lif, int local_outlif)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        SOC_PPD_EG_ENCAP_DATA_INFO_clear(&data_encap_entry);
        /* add enry to allocated place */
        soc_sand_rv = soc_ppd_eg_encap_data_lif_entry_add(soc_sand_dev_id,local_outlif,&data_encap_entry,FALSE,0);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    else
#endif /* SOC_IS_ARAD */
                                    ...
                                    bcm_mirror_destination_t *mirror_dest, uint8 set_dest)
{
    ...
    if ((flags = mirror_dest->flags & ~(BCM_MIRROR_DEST_WITH_ID | BCM_MIRROR_DEST_REPLACE | BCM_MIRROR_DEST_UPDATE_POLICER | BCM_MIRROR_DEST_DEST_MULTICAST |
                                        BCM_MIRROR_DEST_UPDATE_COUNTER | BCM_MIRROR_DEST_TUNNEL_IP_GRE | BCM_MIRROR_DEST_TUNNEL_WITH_ENCAP_ID | BCM_MIRROR_DEST_IS_SNOOP |
                                  ...
                                  bcm_gport_t mirror_dest)
{
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS_ACL) { /* remove reservation of reassembly context and recycle channel for port for other applications */
        ...
        /* disable mirroring */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_appl_set((unit), core, pp_port, 0));
    }
                                      ...
                                      bcm_mirror_options_t options)
{
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS_ACL) { /* enable of outbound mirroring (reserve reassembly context and recycle channel) for other applications */
        ...
        /* enable mirroring */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_appl_set((unit), core, pp_port, 1));
    }
