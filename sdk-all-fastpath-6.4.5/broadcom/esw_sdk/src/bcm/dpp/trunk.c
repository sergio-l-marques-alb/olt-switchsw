                                ...
                                bcm_trunk_member_t  *member_array )
{
    ...
    } else 
    {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_trunk_sw_db_set(unit, tid, &lag_info));
    }
                               ...
                               bcm_trunk_member_t   *member_array)
{
    ...
    if (TRUNK_TID_VALID(trunk_id)) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            rv = _bcm_dpp_trunk_sw_db_set(unit, tid, &lag_info);
            if (BCM_FAILURE(rv)) {
                               ...
                               bcm_trunk_member_t   *member)
{
    ...
    } else 
    {
        ...
        {
            LOG_ERROR(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Added member (gport:%x) to trunk (id:%d)\n"), FUNCTION_NAME(), member->gport, tid));
        ...
        rv = _bcm_dpp_trunk_sw_db_add(unit, tid, &lag_member);
        if (BCM_FAILURE(rv)) 
        {
            TRUNK_DB_UNLOCK(unit);
            BCMDNX_IF_ERR_EXIT(rv);
                                  ...
                                  bcm_trunk_member_t    *member)
{
    ...
    if (member->flags) {
        LOG_WARN(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: member->flags not supported. \n"), FUNCTION_NAME()));
    }
    ...
    if (tid > ngroups && tid < ngroups + stk_ngroups) {
        ...
        if (TRUNK_TID_VALID(trunk_id)) 
        {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                LOG_ERROR(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Deleted member(gport:%x) from trunk (id:%d)\n"), FUNCTION_NAME(), member->gport, tid));
                rv = _bcm_dpp_trunk_sw_db_remove(unit, tid, &lag_member);
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Failed to update software database for trunk (id:%d)\n"), FUNCTION_NAME(), tid));
                }
            }
        } else {
            rv = BCM_E_NOT_FOUND;
                                             ...
                                             bcm_trunk_t    tid)
{
    ...
    if(SOC_DPP_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, BCM_GPORT_MODPORT_MODID_GET(gport), modid)) {   
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, local_port, &tm_port, &core)));
                            ...
                            int         psc)
{
    ...
    if (TRUNK_TID_VALID(trunk_id)) {
        ...
        } else if (lag_info.lb_type == lb_type &&((!SOC_IS_ARADPLUS(unit)) || (SOC_IS_ARADPLUS(unit) && lag_info.is_stateful == is_stateful))) 
        {
            ...
            rv = _bcm_dpp_trunk_sw_db_set_trunk_attributes(unit, tid, lb_type, is_stateful);
            if (BCM_FAILURE(rv)) {
                        ...
                        int                 *member_count)
{
    ...
    if (!TRUNK_TID_VALID(trunk_id)) {
        ...
        if (tid > ngroups && tid < ngroups + stk_ngroups) { 
            ...
            rv = _bcm_dpp_trunk_sw_db_get(unit, tid, &lag_info);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_TRUNK, (BSL_META_U(unit, "%s: Failed (%s) to read trunk (id:%d) info \n"), FUNCTION_NAME(), bcm_errmsg(rv), tid));
            } else {
                rv = _bcm_petra_trunk_lb_type_to_psc(unit, lag_info.lb_type, &psc);
