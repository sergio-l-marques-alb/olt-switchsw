...
#define _DPP_STACK__MODID_TO_DOMAIN_SET(unit, domain, modid, val)   \
        \
        if ((val)) {\
            SHR_BITSET(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[domain], modid); \
        } else {\
            SHR_BITCLR(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[domain], modid); \
        }
 
#define _DPP_STACK__STK_TRUNK_DOMAINS_SET(unit, stk_trunk, domain, val);   \
        \
        if ((val)) {\
            SHR_BITSET(sw_state[unit]->dpp.bcm.stack->_domain_to_stk_trunk[stk_trunk], domain); \
        } else {\
            SHR_BITCLR(sw_state[unit]->dpp.bcm.stack->_domain_to_stk_trunk[stk_trunk], domain); \
        }
...
_bcm_petra_stk_detach(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(STACK_ACCESS.free(unit));
    ...
    int *mod_count)
{
    ...
    for (modid = 0; modid < _DPP_STACK_MAX_DEVICES; modid++) {
        ...
        bit_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[tm_domain], modid);
    ...
    int *domain_count)
{
    ...
    for (domain = 0; domain < _DPP_STACK_MAX_TM_DOMAIN; domain++) {
        ...
        bit_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_domain_to_stk_trunk[stk_tid], domain);
    ...
    int *stk_trunk_count)
{
    ...
    for (stk_tid_ndx = 0; stk_tid_ndx < _DPP_STACK_MAX_TM_DOMAIN; stk_tid_ndx++) {
        ...
        bit_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_domain_to_stk_trunk[stk_tid_ndx], domain);
    ...
    uint32 *is_same_domain)
{
    ...
    *is_same_domain = 0x0;
    ...
    for (domain = 0; domain < _DPP_STACK_MAX_TM_DOMAIN; domain++) {
        ...
        bit_local_modid_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[domain], local_modid);
        bit_sysport_modid_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[domain], sysport_modid);
    ...
    int *domain)
{
    ...
    *domain = _DPP_STACK_INVALID_DOMAIN;
    ...
    for (tmp_domain = 0; tmp_domain < _DPP_STACK_MAX_TM_DOMAIN; tmp_domain++) {
        ...
        bit_modid_val = SHR_BITGET(sw_state[unit]->dpp.bcm.stack->_modid_to_domain[tmp_domain], modid);
