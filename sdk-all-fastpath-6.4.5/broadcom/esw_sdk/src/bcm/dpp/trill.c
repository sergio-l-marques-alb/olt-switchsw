    ...
    int         *dt_nickname)
{
    ...
    if (phy_port_ptr->type != _bcmDppGportResolveTypeTrillMC) {
                                ...
                                int unit)
{
    ...
    rv = TRILL_ACCESS.trill_ports.ports.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = TRILL_ACCESS.trill_ports.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = TRILL_ACCESS.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int bcm_petra_trill_port_get(int unit,bcm_trill_port_t *trill_port){
    ...
    if (BCM_GPORT_TRILL_PORT_ID_IS_EGRESS(port_id)) {
        ...
        if (next_eep[0] > 0) {
            _BCM_PETRA_L3_ITF_SET(trill_port->egress_if,_BCM_PETRA_L3_ITF_ENCAP, next_eep[0]); 
        }
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (found) {
            ...
            /* Get wide entry flag */
            rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, lif_index, TRUE, &is_wide_entry));
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    int                 is_replace)
{
    ...
    if (is_egress) {
        ...
        if (!(trill_port->flags & BCM_TRILL_PORT_REPLACE)) {
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                /* identify type of eedb trill tunnel allocation according to LL eedb entry type: 
                   ...
                   default otherwise */
                rv = soc_ppd_eg_encap_entry_type_get(unit, _BCM_PETRA_L3_ITF_VAL_GET(trill_port->egress_if), &entry_type);
        ...
        global_ll_lif = _BCM_PETRA_L3_ITF_VAL_GET(trill_port->egress_if); 
...
/*      rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_eg_trill_entry_set,(unit,trill_eep,&trill_encap_info,ll_eep,ll_eep_is_valid)));*/
    ...
    int         reset_local_info)
{
    ...
    if ( (phy_port_ptr->type != _bcmDppGportResolveTypeTrillMC) && !(trill_port.flags & BCM_TRILL_PORT_LOCAL ) && !is_ecmp) {        
        /* Remove unicast Rbridge with transit functionality if BCM_TRILL_PORT_TUNNEL is not set */
        if (!(trill_port.flags & BCM_TRILL_PORT_TUNNEL)) {   
            soc_sand_rv = soc_ppd_frwrd_trill_unicast_route_remove(soc_sand_dev_id, phy_port_ptr->encap_id , &local_in_lif_id);  
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }          
        /* Remove SVP if BCM_TRILL_PORT_EGRESS is not set */
        if (!(trill_port.flags & BCM_TRILL_PORT_EGRESS)) {   
            soc_sand_rv = soc_ppd_frwrd_trill_ingress_lif_remove(soc_sand_dev_id, phy_port_ptr->encap_id , &local_in_lif_id);  
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
