                            ...
                            bcm_l3_nat_egress_t *nat_info, void *ign)
{
    ...
    if(flags & BCM_L3_NAT_EGRESS_SNAT)
    {
        ...
        cli_out("%-5d %-5s %-5d %-16s %-16s %-5d\n", 
                index, "SNAT", nat_info->nat_id, ip_str, 
                ip_mask_str, nat_info->src_port);
    }
...
_l3_cmd_nat_egress_show(int unit, args_t *a)
{
    ...
    cli_out("%-5s %-5s %-5s %-16s %-16s %-6s\n", "IDX", 
            "TYPE", "ID", "IP", "MASK", "L4Port");
    /* -1 is special value to say all entries */
...
_l3_cmd_alpm_debug_show(int unit, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "brief") == 0) {
        ...
        parse_table_add(&pt, "BucketuSage", PQ_DFL | PQ_BOOL, 0, 
                        (void *)&bkt_usage, 0); 
        parse_table_add(&pt, "InternelDebug", PQ_DFL | PQ_BOOL, 0, 
...
_l3_cmd_alpm_find(int unit, args_t *a)
{
    ...
    if (ip4addr != 0) {
        ...
        if (sal_memcmp(ip6addr, ip6zero, sizeof(bcm_ip6_t)) == 0) {
            return CMD_USAGE;
        } else if (ip_len == BCM_L3_IP6_MAX_NETLEN) {
            for_host = 1;
        }
    }
...
_l3_cmd_alpm_sanity_check(int unit, args_t *a)
{
    ...
    parse_table_init(unit, &pt);
    parse_table_add(&pt, "memory", PQ_DFL|PQ_STRING|PQ_STATIC, 0, &mem_name, 0);
    parse_table_add(&pt, "index",  PQ_DFL|PQ_INT, (void *)-1, &index, 0);
    ...
    cli_out("Processing ALPM sanity check for memory %s"
            " index %d\n", SOC_MEM_NAME(unit, mem), index);
    switch (mem) {
    case L3_DEFIPm:
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_th_alpm_bucket_sanity_check(unit, mem, index);
        } else 
#endif
        {
            rv = soc_alpm_bucket_sanity_check(unit, mem, index);
        }
        if (BCM_FAILURE(rv)) {
            cli_out("Bucket sanity check failed, mem %s "
                    "return %d\n", SOC_MEM_NAME(unit, mem), rv);
        }
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_th_alpm_pivot_sanity_check(unit, mem, index);
        } else 
#endif
        {
            rv = soc_alpm_pivot_sanity_check(unit, mem, index);
        }
        if (BCM_FAILURE(rv)) {
            cli_out("Pivot sanity check failed, mem %s "
                    "return %d\n", SOC_MEM_NAME(unit, mem), rv);
        }
        break;
...
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_th_alpm_128_bucket_sanity_check(unit, mem, index);
        } else 
#endif
        {
            rv = soc_alpm_128_bucket_sanity_check(unit, mem, index);
        } 
        if (BCM_FAILURE(rv)) {
            cli_out("Bucket sanity check failed, mem %s "
                    "return %d\n", SOC_MEM_NAME(unit, mem), rv);
...
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_th_alpm_128_pivot_sanity_check(unit, mem, index);            
        } else 
#endif
        {
            rv = soc_alpm_128_pivot_sanity_check(unit, mem, index);
        }
        if (BCM_FAILURE(rv)) {
            cli_out("Pivot sanity check failed, mem %s "
                    "return %d\n", SOC_MEM_NAME(unit, mem), rv);
    ...
    if (BCM_SUCCESS(rv)) {
        cli_out("Sanity check for memory %s index %d "
                "Passed\n", SOC_MEM_NAME(unit, mem), index);
    }
...
if_l3(int unit, args_t *a)
{
    ...
    case L3_SHOW:
        ...
        if ((SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIUMPH3(unit)) &&
            ...
            !soc_property_get(unit, spn_L3_ALPM_ENABLE, 0) && !ext_tcam_flag){
