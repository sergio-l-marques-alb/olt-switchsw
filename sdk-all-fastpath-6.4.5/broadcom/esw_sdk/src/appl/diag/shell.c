...
char sh_warmboot_usage[] =
...
#if defined(BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT)
    "Parameters: [on | off | show | shutdown | storage | usage | test_mode <on|off|dvapi|no_wb_test|allow_wb_test|skip <$count>>]\n"
    ...
    "test_mode supported for dpp chips when compiled with a destined flag:"
    "\t on - wb test after API calls\n"
    "\t dvapi - wb test after dvapi calls\n"
    "\t off - no tests\n"
#endif
    ;
#else 
