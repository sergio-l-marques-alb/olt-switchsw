...
int
brief_port_info(int unit, char *port_ref, bcm_port_info_t *info, uint32 flags)
{
...
esw_brief_port_info(int unit, int port, bcm_port_info_t *info, uint32 flags)
{
     ...
     */
#if defined(BCM_TOMAHAWK_SUPPORT) && !defined(BCM_TH_ASF_EXCLUDE)
    cli_out("%10s  %4s ", BCM_PORT_NAME(unit, port),
#else
    cli_out("%10s  %4s ", port_ref,
#endif
            !info->enable ? "!ena" :
               ...
               int st_port, uint32 flags)
{
    ...
    if (st_port) {
         ...
         */
        cli_out("%s(", ENCAP_MODE(info->encap_mode & 0x3));
...
if_esw_port(int u, args_t *a)
{
    ...
    if (ARG_CNT(a) == 0) {
        ...
        if (!sal_strcasecmp(_ARG_CUR(a), "detach")) {
            ...
            /*
            At present, 'asf' option in 'port' cmd is supported only for
            TH SKUs. This cmd could be easily extended to TD2 but this has
            to be tested on TD2 before making it available for the customer.
            */
            if (!soc_feature(u, soc_feature_asf_multimode)) {
                sal_free(info_all);
                sal_free(ability_all);
                return CMD_NOTIMPL;
            }
        ...
        if (info_all[p].autoneg) {
            ...
            if (info_all[p].duplex) {
                if (!(info_given.port_ability.speed_full_duplex & pa_speed)) {
                    cli_out("%s: port %s does not support %d Mbps full duplex\n",
                            ARG_CMD(a), BCM_PORT_NAME(u, p), info_all[p].speed);
                    continue;
                }
            } else {
                if (!(info_given.port_ability.speed_half_duplex & pa_speed)) {
                    cli_out("%s: port %s does not support %d Mbps half duplex\n",
                            ARG_CMD(a), BCM_PORT_NAME(u, p), info_all[p].speed);
                    continue;
                }
            }
...
if_esw_linkscan(int unit, args_t *a)
{
    ...
    us = us ? us : 
              soc_property_get(unit, spn_BCM_LINKSCAN_INTERVAL, 250000);
...
_phy_diag_loopback(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    if (mode_str) {
        if (sal_strcasecmp(mode_str, "remote") == 0) {
            ...
            cli_out("valid modes: remote,local and none\n");
            rv = CMD_FAIL;
        }
    }
...
_phy_diag_eyescan(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    if (!(is_eyescan_algorithm_legacy_mode(unit, nof_ports, ports)) || rv)
        return (rv);
#endif
...
int
phymod_sym_info(int unit, int port, int intermediate,
                phymod_phy_access_t *pm_acc, int core_num)
{
    phy_ctrl_t *pc;

    if (pm_acc == NULL || core_num < 0) {
        return -1;
    }
#if defined(PORTMOD_SUPPORT)
    if (soc_feature(unit, soc_feature_portmod)) {
        ...
        if (rv < 0 || core_num >= num_phy) {
            return -1;
        }
        /* Only a single core per port is currently supported */
        sal_memcpy(pm_acc, &phy_acc[core_num], sizeof(*pm_acc));
        return 0;
    }
#endif
...
phymod_sym_access(int u, args_t *a, int intermediate, soc_pbmp_t *pbm)
{
    ...
    DPORT_SOC_PBMP_ITER(u, *pbm, dport, p) {
        /* Set up per-port info (only a single core is currently supported) */
        if (phymod_sym_info(u, p, intermediate, &pm_acc, 0) < 0) {
            continue;
        }
    ...
    if (c != NULL && sal_strcasecmp(c, "info") == 0) {
        ...
        DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
            ...
            if(soc_feature(u, soc_feature_portmod)) {
                ...
                if (!is_legacy) {
                cli_out("%5s(%3d) %5x %5x %5x %5x %23s/%02d/%02d %10d %s \n",
                            SOC_PORT_NAME(u, p), p1,
                            core_info.phy_id0,
                            core_info.phy_id1,
                            core_acc.access.addr,
                            internal_core.access.addr,
                            phymod_core_version_t_mapping[core_info.core_version].key+17,
                            core_num,(range_start-1)%4,
                            an_timeout,
                            /*soc_phy_an_timeout_get(u, p), */
                            remark);
                } else {
                    cli_out("%5s(%3d) %5x %5x %5x %5x %23s %16d %s \n",
                        SOC_PORT_NAME(u, p), p1,
    ...
    if (ARG_CNT(a) == 0) {      /*  show information for all registers */
        ...
        if (ARG_CNT(a) == 0) {  /* no more args; show this register */
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if (soc_phy_is_c45_miim(u, p) && !intermediate && !int_ovrride) {
                    for (i = 0; i < COUNTOF(p_devad); i++) {
                        phy_devad = p_devad[i];
...
cmd_ibod_sync(int unit, args_t *a)
{
    ...
    if (!ARG_CNT(a)) {          /* Display settings */
        ...
        parse_eq_format(&pt);
        cli_out("Unit %d: IBOD sync event count: %d%d\n", unit,
