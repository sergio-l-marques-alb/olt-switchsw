    ...
    cmd_ppd_api_diag_lif_show(int unit, args_t* a)
{
    ...
    if(!sal_strncasecmp(lif_type, "in", strlen(lif_type))) {
        if((lif_id < 0) || (lif_id >= SOC_DPP_DEFS_GET(unit, nof_local_lifs))) {
            cli_out("in lif id:%d is out of range\n", lif_id);
            return CMD_OK;
        }
        if(global_flag == 1) {
        ...
        /* Existing approach
        SOC_PPC_LIF_ENTRY_PER_TYPE_INFO_print(&lif_entry_info.value, lif_entry_info.type);
        */
        ...
        switch(lif_entry_info.type) {
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP:
                switch(lif_entry_info.value.ac.service_type) {
                    ...
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2AC:
                        mem = IHP_LIF_TABLE_AC_P2P_TO_ACm;
                        break;
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PWE:
                        mem = IHP_LIF_TABLE_AC_P2P_TO_PBBm;
                        break;
                    case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PBB:
                        mem = IHP_LIF_TABLE_AC_P2P_TO_PWEm;
                        break;
                    default:
                        cli_out("AC Service Type:%d", lif_entry_info.value.ac.service_type);
                        break;
                }
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_PWE:
                if(lif_entry_info.value.pwe.service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_MP)
                    ...
                    cli_out("PWE Service Type:%d", lif_entry_info.value.pwe.service_type);
                break;
            case SOC_PPC_LIF_ENTRY_TYPE_ISID:
                if(lif_entry_info.value.isid.service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_MP)
                    ...
                    cli_out("ISID Service Type:%d", lif_entry_info.value.isid.service_type);
                break;
            ...
            case SOC_PPC_LIF_ENTRY_TYPE_TRILL_NICK:
                ...
                break;
            default:
                cli_out("Unsupported LIF type :%d", lif_entry_info.type);
                break;
        }
        cli_out("LIF Type:0x%04x mem:%d\n", lif_entry_info.type, mem);
        ...
        if((lif_id < 0) || (lif_id >= SOC_DPP_DEFS_GET(unit, nof_out_lifs))) {
            cli_out("out lif id:%d is out of range\n", lif_id);
            return CMD_OK;
        }
        if(global_flag == 1) {
            if((ret = _bcm_dpp_global_lif_mapping_global_to_local_get(unit,
        ...
        switch(access_type) {
            ...
            default:
                cli_out("Unsupported eedb access type :%d", access_type);
                break;
        }
        cli_out("Access Type:%d mem:%d\n", access_type, mem);
    ...
    cli_out("%s %s_lif:%d - Bank:%d Offset:%d\n",
            ((global_flag == 1) ? "global" : " local"), lif_type, lif_id, bank_id, internal_offset);
