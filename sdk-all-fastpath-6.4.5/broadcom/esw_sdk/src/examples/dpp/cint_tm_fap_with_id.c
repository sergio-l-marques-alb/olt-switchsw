...
/*
 * $Id: cint_tm_petra_with_id.c,v 1.5 Broadcom SDK $
 *
...
int c1_sched[4] = {56721, 56725, 56729, 56733};
...
int example_setup(void)
{
    ...
    for (idx=0; idx<C1_PETRA_CARD_NUM_PORTS; idx++) {
        ...
        BCM_GPORT_SCHEDULER_SET(gport_ucast_scheduler[idx], c1_sched[idx]);
        ...
        BCM_COSQ_GPORT_VOQ_CONNECTOR_SET(gport_ucast_voq_connector_group[idx], c1_voq_connector[idx]);
        ...
        BCM_GPORT_UNICAST_QUEUE_GROUP_SET(gport_ucast_queue_group[idx], c1_voq[idx]);
...
int example_verify(int unit)
{
    ...
    for (idx = 0; idx < C1_PETRA_CARD_NUM_PORTS; idx++) {
        ...
        rc = bcm_cosq_gport_bandwidth_get(unit,e2e_parent_gport[idx], 0,0,&kbits_sec_max,0);
        if (rc != BCM_E_NONE) {
        ...
        rc = bcm_cosq_gport_bandwidth_get(unit,e2e_gport[idx],0, 0,kbits_sec_max, 0);
        if (rc != BCM_E_NONE) {
