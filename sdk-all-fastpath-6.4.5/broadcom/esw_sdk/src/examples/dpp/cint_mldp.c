...
int mldp_run(int unit) {
    ...
    rv = bcm_port_control_set(0,cint_mldp_info.recycle_port, bcmPortControlOverlayRecycle, 1);  
    if (rv != BCM_E_NONE) {
    ...
    rv = mldp_create_l3_intf(unit, 
                           0, 
                           cint_mldp_info.p1_vlan, 
                           cint_mldp_info.p1_vlan_my_mac, 
                           &cint_mldp_info.p1_intf); 


    rv = mldp_create_l3_intf(unit, 
                           0,  
                           cint_mldp_info.p2_p3_vlan, 
                           cint_mldp_info.p2_p3_vlan_my_mac, 
                           &cint_mldp_info.p2_p3_intf);
    if (rv != BCM_E_NONE) {
...
mldp_mpls_mc_config(int unit){
    ...
    rv = mldp_create_l3_egress(unit, 
                          BCM_L3_EGRESS_ONLY, 
                          cint_mldp_info.p2_port, 
                          cint_mldp_info.p2_p3_vlan, 
                          label_array[0].tunnel_id,
                          cint_mldp_info.p2_mac, 
                          &egid, 
                          &cint_mldp_info.encap_id1);
    if (rv != BCM_E_NONE) {
        printf("Error, in create_l3_egress intf = %d\n", label_array[0].tunnel_id);
        return rv;
    }  
    ...
    rv = mldp_create_l3_egress(unit, 
                          BCM_L3_EGRESS_ONLY, 
                          cint_mldp_info.p3_port, 
                          cint_mldp_info.p2_p3_vlan, 
                          label_array[0].tunnel_id,
                          cint_mldp_info.p3_mac,  
                          &egid, 
                          &cint_mldp_info.encap_id2);
    if (rv != BCM_E_NONE) {
        printf("Error, in create_l3_egress intf = %d\n", label_array[0].tunnel_id);
        return rv;
    }  
...
int mldp_create_l3_intf(int unit, int flags, int vlan, bcm_mac_t my_mac_addr, int *intf) {
...
int mldp_create_l3_egress(int unit, uint32 flags, int port, int vlan, int ingress_intf, bcm_mac_t nh_mac_addr, int *intf, int *encap_id) {
