...
int vxlan_roo_create_l3_intf(int unit, uint32 flags, uint8 open_vlan, uint8 add_gport_to_vlan, bcm_gport_t port, int vlan, int vrf, bcm_mac_t mac_addr, int native_vsi_compensation, int *intf) {
  ...
  if(open_vlan) {
      rc = vlan__open_vlan_per_mc(unit, vlan, 0x1);
      if (rc != BCM_E_NONE) {
          printf("create_l3_intf: error in vlan__open_vlan_per_mc\n");
        return rc;
      }
  }
 ...
 */
int vxlan_roo_ipv4_tunnel_build_tunnels(int unit, int out_port, bcm_if_t ll_eep_id, int out_vlan, bcm_if_t* intf_ids, bcm_gport_t *tunnel_gports){
    int rv;
    int ing_intf_in;
    int ing_intf_out; 
    int fec[2] = {0x0,0x0};
 ...
 */
int vxlan_roo_run(int unit, int in_port, int out_port, int vpn_id, int add_routing_table_entry, int uc_rpf_mode){
    int rv;
    int vpn;
    int flags = 0;
  ...
  /* add native host entry: add routing host to host table to access network */
  ...
  if (g_vxlan_roo.enable_add_entry_routing_table) {
      ...
      create_l3_egress_params.vlan = out_vlan; /* out-vlan */
  ...
  rv = l2__port_vlan__create(unit, 0, out_port, g_vxlan_roo.native_out_vlan, g_vxlan_roo.native_out_vlan, &vlan_port_id);
  if (rv != BCM_E_NONE) {
