 ...
 * Parameters: 
 *     unit         -- unit number.
 *    lag_lb_key    -- Lag load balance key (the 16 MSBs of fer_lb_key's return value).
 *    dest_group_id -- destination Lag id.
 */
int lag_resolution(int unit, uint32 lag_lb_key, uint32 dest_group_id) {
   uint8  lag_range, lag_mode, lag_member;
   uint32 reg32_val = 0;
   int    rv;

   common_read_table_entry(unit, "IRR_LAG_TO_LAG_RANGE", dest_group_id, &reg32_val, 1);
   lag_range = get_field(reg32_val, 0, 7); /*RANGE*/
   lag_mode  = get_field(reg32_val, 8, 8); /*MODE*/

   if (lag_mode) {
	  common_read_table_entry(unit, "IRR_LAG_NEXT_MEMBER", dest_group_id, &reg32_val, 1);
	  lag_member = get_field(reg32_val, 0, 7); /*OFFSET*/
	  /* In practice, the Arad updates the next_member table as follows:
		*  soc_IRR_LAG_NEXT_MEMBERm_field32_set(unit,&reg32_val,OFFSETf,(lag_member+1)% lag_range);
		*  WRITE_IRR_LAG_NEXT_MEMBERm(unit,MEM_BLOCK_ALL,dest_group_id,&reg32_val);
	  */
   } else {
	  uint8 global_lag_mode;
	  reg32_val = common_read_reg(unit, "ECI_GLOBALF");
	  global_lag_mode = get_field(reg32_val, 20, 22);  /*LAG_MODE*/
	  if (global_lag_mode == 0) {  /*1024 groups of 16 ports each*/
		 uint32 index = ((lag_range << 7) & ((1 << 32) - (1 << 7))) + ((lag_lb_key >> 1) & ((1 << 7) - 1));
		 common_read_table_entry(unit, "IRR_SMOOTH_DIVISION", index, &reg32_val, 1);
		 if (lag_lb_key & 1) {
			lag_member = get_field(reg32_val, 4, 7);
		 } else {
			lag_member = get_field(reg32_val, 0, 3);
		 }
	  } else lag_member = (lag_range * lag_lb_key) >> 16;
   }
   common_read_table_entry(unit, "IRR_LAG_MAPPING", (dest_group_id << 4) + lag_member, &reg32_val, 1);
   return get_field(unit, reg32_val, 0, 17); /*DESTINATION*/
}
