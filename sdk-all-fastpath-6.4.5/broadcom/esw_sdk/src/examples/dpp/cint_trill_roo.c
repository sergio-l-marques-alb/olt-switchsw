/* $Id: cint_trill_roo.c,v 1.45 Broadcom SDK $
...
10,                  20,           30,
/* nicknames:
   my_nickname | remote rb nickname */
0x1234,          0xAA11
};
...
trill_roo_add_remote_rbridge_router(int unit, 
    ...
    if (verbose >= 2) {
        printf("Trill Next hop encapsulation created \n\r");
    }
    ...
    /*** build Trill FEC ***/
    bcm_l3_egress_t l3_egress_fec; 
    bcm_l3_egress_t_init(&l3_egress_fec); 
    uint32 flags; 

    l3_egress_fec.port = network_port; 
    trill_roo_egress_trill_gport_to_l3_intf(trill_port.trill_port_id, &(l3_egress_fec.intf)); /* fec point to trill eedb */
    flags = BCM_L3_INGRESS_ONLY; 

    rv = bcm_l3_egress_create(unit, flags, &l3_egress_fec, fec_if_id); 
    if (rv != BCM_E_NONE) {
...
trill_roo_run(int unit) {
    ...
    l3_egress_encap.gport = cint_trill_roo_info.provider_port; /* outgoing port */
    ...
    native_ll_encap_id = l3_egress_encap.encap_id; /* Contains LL eedb index (l3_if object. Type encap) */
    ...
    l3_egress.gport = cint_trill_roo_info.access_port;  /* outgoing port */
    ...
    ll_intf = l3_egress.l3_egress_id; /* Contain the fec_id  (l3_if object. Type FEC) */
    ll_encap_id = l3_egress.encap_id; /* Contains LL eedb index (l3_if object. Type encap) */
