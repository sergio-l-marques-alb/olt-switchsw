 ...
 *  Revision      :  $Id: eagle_tsc_functions.c 900 2015-02-05 21:25:55Z eroes $ *
 *                                                                                 *
    ...
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
...
#endif
        {
            ...
            USR_MEMSET(&osr_mode, 0, sizeof(eagle_tsc_osr_mode_st));
...
err_code_t eagle_tsc_display_eye_density_data( const phymod_access_t *pa, const struct eagle_tsc_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t buffer_size) {
    ...
    for (y = eyescan_options.vert_max-1;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
        ...
        for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
            ...
            for(z=1;z<=eyescan_options.hstep;z++) {
                if      (z!=1)                     {USR_PRINTF((" "));}
                else if (val)                      {USR_PRINTF(("%X",(uint32_t)val));}
                else if ((x%5)==0 && ((y+3)%5)==0) {USR_PRINTF(("+"));}
                else if ((x%5)!=0 && ((y+3)%5)==0) {USR_PRINTF(("-"));}
                else if ((x%5)==0 && ((y+3)%5)!=0) {USR_PRINTF((":"));}
                else                               {USR_PRINTF((" "));}
            }
...
err_code_t eagle_tsc_display_core_state_line( const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _eagle_tsc_display_core_state_no_newline( pa );
  USR_PRINTF(("\n"));
  return (err_code);
}

err_code_t eagle_tsc_display_core_state_legend(void) {
  USR_PRINTF(("\n"));
  USR_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t eagle_tsc_display_core_config( const phymod_access_t *pa ) {
    ...
    USR_PRINTF(("***********************************\n\n"));
    {
        ...
        USR_MEMSET(&core_cfg, 0, sizeof(core_cfg));
            EFUN(eagle_tsc_get_uc_core_config( pa, &core_cfg));
        {
            USR_PRINTF(("uC Config VCO Rate   = %d (~%d.%dGHz)\n",
               core_cfg.field.vco_rate, (((((uint16_t)core_cfg.field.vco_rate) * 250) +5500) / 1000),
               (((((uint16_t)core_cfg.field.vco_rate) * 250) +5500) % 1000)));
...
err_code_t eagle_tsc_display_lane_config( const phymod_access_t *pa ) {
  ...
  USR_MEMSET(&lane_cfg, 0, sizeof(struct eagle_tsc_uc_lane_config_st));
...
err_code_t eagle_tsc_display_core_state( const phymod_access_t *pa ) {
    ...
    {   uint16_t bin;
        int16_t temp;
        ESTM(bin = rdcv_avg_tmon_reg13bit());
        temp     = _bin_to_degC((bin>>3));
        USR_PRINTF(( "Average Die TMON_reg13bit = %d [ %3dC ]\n", bin, temp));
    }
    {
        uint8_t temp_idx;
        ESTM(temp_idx = rdcv_temp_idx());
        USR_PRINTF(( "Temperature Index         = %d\n",temp_idx));
    }
    ...
    ESTM(USR_PRINTF(("uC Core Status byte        = 0x%x\n\n", rdcv_status_byte())));

    {
        ...
        {
            EFUN(eagle_tsc_get_uc_core_config( pa, &core_cfg));
            ESTM(pll_range = rdc_ams_pll_range());
            ESTM(pll_div   = rdc_ams_pll_ndiv());
        } {
...
err_code_t eagle_tsc_display_lane_debug_status( const phymod_access_t *pa ) {
  ...
  USR_MEMSET(&ds, 0, sizeof(struct eagle_tsc_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dsd, 0, sizeof(struct eagle_tsc_usr_ctrl_disable_dfe_functions_st));
  USR_MEMSET(&dss, 0, sizeof(struct eagle_tsc_usr_ctrl_disable_functions_st));
  USR_MEMSET(&dssd, 0, sizeof(struct eagle_tsc_usr_ctrl_disable_dfe_functions_st));
...
err_code_t eagle_tsc_check_ucode_crc( const phymod_access_t *pa, uint16_t expected_crc_value, uint32_t timeout_ms) {
    ...
    if (err_code) {
      USR_PRINTF(("ERROR : DSC ready for command timed out. Previous uC command not finished yet\n"));
      return (err_code);
    }
    ESTM(calc_crc = rd_uc_dsc_data());
    if(calc_crc != expected_crc_value) {
...
err_code_t eagle_tsc_set_uc_core_config( const phymod_access_t *pa, struct eagle_tsc_uc_core_config_st struct_val) {
    {   uint8_t reset_state;
        ...
        if(reset_state < 7) {
            USR_PRINTF(("ERROR: eagle_tsc_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0\n"));
...
err_code_t eagle_tsc_set_uc_lane_cfg( const phymod_access_t *pa, struct eagle_tsc_uc_lane_config_st struct_val) {
    ...
    if(reset_state < 7) {
        USR_PRINTF(("ERROR: eagle_tsc_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0\n"));
...
#else
  {   uint16_t rddata;
      USR_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(eagle_tsc_delay_us(timeout_ms));
      } else {
      ...
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          ...
          USR_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      USR_PRINTF(("// ATE_LOG eagle_tsc_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t eagle_tsc_uc_core_var_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {
    ESTM(rddata = eagle_tsc_rdbc_uc_var( pa, &__err, addr));
...
err_code_t eagle_tsc_uc_lane_var_dump( const phymod_access_t *pa ) {
  ...
  USR_PRINTF(("********************************************\n"));
    ...
    for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
      ESTM(rddata = eagle_tsc_rdbl_uc_var( pa, &__err, addr));
...
/*
                ...
                */
                ...
                eagle_tsc_uc_lane_idx_to_system_id(uc_lane_id_str, cc);
...
err_code_t eagle_tsc_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp) {
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    USR_PRINTF((" Function 'eagle_tsc_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
