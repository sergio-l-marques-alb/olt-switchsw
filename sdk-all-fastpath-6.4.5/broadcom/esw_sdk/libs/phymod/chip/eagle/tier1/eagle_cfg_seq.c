  ...
  if(config->ref_clock == phymodRefClk156Mhz) {
    switch (config->data_rate) {
      case 5750 :   /* speed 5.75G ; fracn pll= 73.6    */
        *new_pll_div = EAGLE_TSC_pll_div_36p8x; *new_os_mode = 1; *vco_rate = 5750; /* vco_rate = 24 */
        break;
      ...
      default:
        PHYMOD_DEBUG_ERROR(("Unsupported speed :: %d :: at ref clk :: %d", config->data_rate, config->ref_clock));
