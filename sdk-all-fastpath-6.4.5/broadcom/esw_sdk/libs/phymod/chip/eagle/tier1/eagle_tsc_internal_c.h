 ...
 *  Revision      :  $Id: eagle_tsc_internal.c 900 2015-02-05 21:25:55Z eroes $ *
 *                                                                                 *
...
/*  SHANTA ADD HEADER IF COMPILE FAILS */
...
SDK_STATIC err_code_t _eagle_tsc_get_osr_mode( const phymod_access_t *pa, eagle_tsc_osr_mode_st *imode) {
    eagle_tsc_osr_mode_st mode;
     ...
     USR_MEMSET(&mode, 0, sizeof(eagle_tsc_osr_mode_st));
...
SDK_STATIC err_code_t _eagle_tsc_read_lane_state( const phymod_access_t *pa, eagle_tsc_lane_state_st *istate) {
  ...
  USR_MEMSET(&state, 0, sizeof(eagle_tsc_lane_state_st));
...
SDK_STATIC err_code_t _eagle_tsc_read_core_state( const phymod_access_t *pa, eagle_tsc_core_state_st *istate) {
  ...
  USR_MEMSET(&state, 0, sizeof(eagle_tsc_core_state_st));
...
static err_code_t _eagle_tsc_display_core_state_no_newline( const phymod_access_t *pa ) {
    eagle_tsc_core_state_st state;
    USR_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_eagle_tsc_read_core_state( pa, &state));
    USR_PRINTF(("%02d  "              ,  eagle_tsc_get_core()));
    ...
    USR_PRINTF(("    %3dC  "      ,  _bin_to_degC(    (state.avg_tmon>>3))));
