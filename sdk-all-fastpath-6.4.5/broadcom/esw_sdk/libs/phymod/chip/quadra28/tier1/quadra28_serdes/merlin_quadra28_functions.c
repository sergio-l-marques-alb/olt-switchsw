/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_quadra28_functions.c                                        *
 *  Created On    :  29/04/2013                                                    *
 *  Created By    :  Kiran Divakar                                                 *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_quadra28_functions.c 960 2015-03-11 21:43:05Z kirand $ *
 *                                                                                 *
...
/** @file merlin_quadra28_functions.c
...
#include "merlin_quadra28_functions.h"
#include "merlin_quadra28_field_access_c.h"
#include "merlin_quadra28_internal_c.h"
#include "merlin_quadra28_dv_functions.c"
...
err_code_t merlin_quadra28_display_eye_scan(const phymod_access_t *pa) {
    ...
    EFUN(merlin_quadra28_display_eye_scan_header( pa, 1));

    /* start horizontal acquisition */
    {   err_code_t err_code = merlin_quadra28_meas_eye_scan_start(pa, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((merlin_quadra28_meas_eye_scan_done( pa ), err_code));
        }
    }
    /* display stripes */
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = merlin_quadra28_read_eye_scan_stripe(pa, &stripe[0], &status);
            if (err_code) {
                EFUN((merlin_quadra28_meas_eye_scan_done( pa ), err_code));
            }
        }
        EFUN(merlin_quadra28_display_eye_scan_stripe( pa, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin_quadra28_meas_eye_scan_done( pa ));
    EFUN(merlin_quadra28_display_eye_scan_footer( pa, 1));
...
err_code_t merlin_quadra28_meas_lowber_eye( const phymod_access_t *pa, const struct merlin_quadra28_eyescan_options_st eyescan_options, uint32_t *buffer) {
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(merlin_quadra28_pmd_uc_cmd_with_data( pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            EFUN(merlin_quadra28_delay_us(1000));
            ESTM(status=rdv_usr_diag_status());
            EFUN_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0);
#else
        EFUN(merlin_quadra28_poll_diag_done( pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            merlin_quadra28_osr_mode_st osr_mode;
            ENULL_MEMSET(&osr_mode, 0, sizeof(merlin_quadra28_osr_mode_st));
            EFUN(_merlin_quadra28_get_osr_mode( pa, &osr_mode));
            if(osr_mode.tx_rx == MERLIN_QUADRA28_OSX1) {
                EFUN(merlin_quadra28_prbs_err_count_ll( pa, &errors));
            } else if(osr_mode.tx_rx == MERLIN_QUADRA28_OSX2) {
...
err_code_t merlin_quadra28_meas_eye_scan_start( const phymod_access_t *pa, uint8_t direction) {
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin_quadra28_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(merlin_quadra28_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}
...
err_code_t merlin_quadra28_read_eye_scan_stripe( const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = merlin_quadra28_poll_diag_eye_data(pa, &val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t merlin_quadra28_meas_eye_density_data(const phymod_access_t *pa, const struct merlin_quadra28_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  ...
  EFUN(merlin_quadra28_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DENSITY,2000));
  i = 0;hzcnt=0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    ...
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         ...
         EFUN(merlin_quadra28_poll_dsc_state_equals_uc_tune( pa, 2000));
...
/*eye_scan_status_t merlin_quadra28_read_eye_scan_status() */
...
err_code_t merlin_quadra28_meas_eye_scan_done( const phymod_access_t *pa ) {
  EFUN(merlin_quadra28_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}
...
err_code_t merlin_quadra28_start_ber_scan_test( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(merlin_quadra28_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin_quadra28_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t merlin_quadra28_eye_margin_proj(const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_quadra28_start_ber_scan_test( pa, ber_scan_mode, timer_control, max_error_control));
    ...
    if(verbose > 5) {
        do {
            EFUN(merlin_quadra28_delay_us(2000000));
            ESTM(sts = rdv_usr_diag_status());
            EFUN_PRINTF(("sts=%04x\n",sts));
        ...
        EFUN(merlin_quadra28_poll_diag_done( pa, &sts,timer_control*2000));
    }
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin_quadra28_read_ber_scan_data( pa,  &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,50));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin_quadra28_display_ber_scan_data(rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t merlin_quadra28_display_ber_scan_data (USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
    ...
    ********************************************* */
    ...
    if (verbose > 2) EFUN_PRINTF(("Exy=%.2f, Eyy=%.4f, Exx=%.2f, Ey=%.4f, Ex=%.2f, alpha=%.4f, beta=%.4f, alpha2=%.3f, sq_r=%.3f, sq_err1=%g, sq_err2=%g\n", Exy,Eyy,Exx,Ey,Ex,alpha,beta,alpha2,sq_r,sq_err1,sq_err2));
...
err_code_t merlin_quadra28_display_lane_state( const phymod_access_t *pa ) {
  err_code_t err_code = _merlin_quadra28_display_lane_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t merlin_quadra28_display_core_state_line( const phymod_access_t *pa ) {
  err_code_t err_code = _merlin_quadra28_display_core_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t merlin_quadra28_display_core_config( const phymod_access_t *pa ) {
    ...
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        struct   merlin_quadra28_uc_core_config_st core_cfg;
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
        EFUN(merlin_quadra28_get_uc_core_config( pa, &core_cfg));
...
err_code_t merlin_quadra28_display_lane_config( const phymod_access_t *pa ) {
    struct merlin_quadra28_uc_lane_config_st lane_cfg;
    ...
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                              merlin_quadra28_get_uc_lane_cfg( pa, &lane_cfg)       );
...
err_code_t merlin_quadra28_display_core_state( const phymod_access_t *pa ) {
    EFUN(merlin_quadra28_display_core_state_hdr( pa ));
    EFUN(merlin_quadra28_display_core_state_line( pa ));
    EFUN(merlin_quadra28_display_core_state_legend( pa ));
    return ERR_CODE_NONE;
}
...
err_code_t merlin_quadra28_display_lane_debug_status( const phymod_access_t *pa ) {
    /* startup */
    struct merlin_quadra28_usr_ctrl_disable_functions_st     ds;
    struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct merlin_quadra28_usr_ctrl_disable_functions_st     dss;
    struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(merlin_quadra28_get_usr_ctrl_disable_startup( pa, &ds));
    ...
    EFUN(                                                            merlin_quadra28_get_usr_ctrl_disable_startup_dfe( pa, &dsd) );
    ...
    EFUN(                                                           merlin_quadra28_get_usr_ctrl_disable_steady_state( pa, &dss) );
    ...
    EFUN(                                                      merlin_quadra28_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd) );
...
err_code_t merlin_quadra28_stop_uc_lane( const phymod_access_t *pa, uint8_t enable) {

  if (enable) {
    return(merlin_quadra28_pmd_uc_control( pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(merlin_quadra28_pmd_uc_control( pa, CMD_UC_CTRL_RESUME,50));
  }
}


err_code_t merlin_quadra28_stop_uc_lane_status( const phymod_access_t *pa, uint8_t *uc_lane_stopped) {
...
err_code_t merlin_quadra28_request_stop_rx_adaptation( const phymod_access_t *pa ) {
  ...
  return(merlin_quadra28_pmd_uc_control_return_immediate( pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t merlin_quadra28_pmd_uc_cmd_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  err_code_t err_code;
  uint16_t   cmddata;

  err_code = merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1( pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_quadra28 ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(merlin_quadra28_pmd_wr_reg(pa,DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */

  return(ERR_CODE_NONE);
}

err_code_t merlin_quadra28_pmd_uc_cmd( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {

  uint8_t uc_dsc_error_found;

  EFUN(merlin_quadra28_pmd_uc_cmd_return_immediate( pa, cmd, supp_info));    /* Invoke merlin_quadra28_uc_cmd and return control immediately */

  EFUN(merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1( pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_quadra28 ready for command */
...
err_code_t merlin_quadra28_pmd_uc_cmd_with_data_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data) {
  uint16_t   cmddata;
  err_code_t err_code;
...
err_code_t merlin_quadra28_pmd_uc_cmd_with_data( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {

    uint8_t uc_dsc_error_found;

    EFUN(merlin_quadra28_pmd_uc_cmd_with_data_return_immediate( pa, cmd, supp_info, data)); /* Invoke merlin_quadra28_uc_cmd and return control immediately */
...
err_code_t merlin_quadra28_pmd_uc_control_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(merlin_quadra28_pmd_uc_cmd_return_immediate( pa, CMD_UC_CTRL, (uint8_t) control));
}

err_code_t merlin_quadra28_pmd_uc_control( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_quadra28_pmd_uc_cmd( pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}

err_code_t merlin_quadra28_pmd_uc_diag_cmd( const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_quadra28_pmd_uc_cmd( pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t merlin_quadra28_rdbl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    {
        merlin_quadra28_uc_lane_info_st uc_lane_info;
        EPNULL_MEMSET(&uc_lane_info, 0, sizeof(uc_lane_info));
        EPFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EPFUN(merlin_quadra28_rdb_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));    /* Direct RAM access */
        }
        else {
            ...
            EPFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_READ_UC_LANE_BYTE, (uint8_t)addr, 50));          /* Use Serdes FW DSC Command Interface for reading RAM */
...
int8_t merlin_quadra28_rdbls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) merlin_quadra28_rdbl_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t merlin_quadra28_rdwl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  if (addr%2 != 0) {                                                                /* Validate even address */
...

    {
      ...
      EPFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EPFUN(merlin_quadra28_rdw_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));                    /* Direct RAM access */
      }
      else {
          if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                       /* uC command only support 8-bit addr */
              *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
          ...
          EPFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_READ_UC_LANE_WORD, (uint8_t)addr, 50));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t merlin_quadra28_rdwls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) merlin_quadra28_rdwl_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Lane Byte Write */
err_code_t merlin_quadra28_wrbl_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
...

    {
        ...
        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrb_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                      /* uC command only support 8-bit addr */
            ...
            return (merlin_quadra28_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_BYTE, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Lane Byte Signed Write */
err_code_t merlin_quadra28_wrbls_uc_var( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (merlin_quadra28_wrbl_uc_var( pa, addr, wr_val));
}

/* Micro RAM Lane Word Write */
err_code_t merlin_quadra28_wrwl_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    {
        merlin_quadra28_uc_lane_info_st uc_lane_info;
        USR_MEMSET(&uc_lane_info, 0, sizeof(merlin_quadra28_uc_lane_info_st));

        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrw_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size || addr > 255 ) {                   /* uC command only support 8-bit addr */
            ...
            return (merlin_quadra28_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_WORD, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Lane Word Signed Write */
err_code_t merlin_quadra28_wrwls_uc_var( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (merlin_quadra28_wrwl_uc_var( pa, addr,wr_val));
}
...
uint8_t merlin_quadra28_rdbc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  if(!err_code_p) {
...

    {
      ...
      EPFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EFUN(merlin_quadra28_rdb_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
      }
      else {
          ...
          EPFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_READ_UC_CORE_BYTE, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
          EPSTM(rddata = (uint8_t)rd_uc_dsc_data());                                            /* Read_Value read from uc_dsc_data field */
      }
    }
...
int8_t merlin_quadra28_rdbcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) merlin_quadra28_rdbc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t merlin_quadra28_rdwc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  if (addr%2 != 0) {                                                                /* Validate even address */
...

    {
      ...
      EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EFUN(merlin_quadra28_rdw_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
      }
      else {
          if (addr >= uc_lane_info.lane_ram_size) {
              *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
          ...
          EPFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_READ_UC_CORE_WORD, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t merlin_quadra28_rdwcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) merlin_quadra28_rdwc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Byte Write  */
err_code_t merlin_quadra28_wrbc_uc_var( const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {


    {
        merlin_quadra28_uc_lane_info_st uc_lane_info;
        ENULL_MEMSET(&uc_lane_info, 0, sizeof(merlin_quadra28_uc_lane_info_st));
        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrb_ram(pa, uc_lane_info.core_var_base_address+(uint16_t)addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size) {                 
            ...
            return (merlin_quadra28_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_BYTE, addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}
...
err_code_t merlin_quadra28_wrbcs_uc_var( const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (merlin_quadra28_wrbc_uc_var( pa, addr, wr_val));
}

/* Micro RAM Core Word Write  */
err_code_t merlin_quadra28_wrwc_uc_var( const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    {   merlin_quadra28_uc_lane_info_st uc_lane_info;
        ...
        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrw_ram(pa, uc_lane_info.core_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        } else {
            if (addr >= uc_lane_info.lane_ram_size) {                   
              return (_error(ERR_CODE_INVALID_RAM_ADDR));
            }
            return (merlin_quadra28_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_WORD, addr, wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Core Word Signed Write */
err_code_t merlin_quadra28_wrwcs_uc_var( const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(merlin_quadra28_wrwc_uc_var( pa, addr,wr_val));
}
...
err_code_t merlin_quadra28_set_uc_core_config( const phymod_access_t *pa, struct merlin_quadra28_uc_core_config_st struct_val) {
    {   uint8_t reset_state;
        ...
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin_quadra28_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0\n"));
            return (_error(ERR_CODE_CORE_DP_NOT_RESET));
        }
    }
    if(struct_val.vco_rate_in_Mhz > 0) {
        struct_val.field.vco_rate = (uint8_t)((struct_val.vco_rate_in_Mhz/250) - 22);
    }
...
err_code_t merlin_quadra28_set_usr_ctrl_core_event_log_level( const phymod_access_t *pa, uint8_t core_event_log_level) {
...
err_code_t merlin_quadra28_set_uc_lane_cfg( const phymod_access_t *pa, struct merlin_quadra28_uc_lane_config_st struct_val) {
    ...
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: merlin_quadra28_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0\n"));
...
err_code_t merlin_quadra28_set_usr_ctrl_lane_event_log_level( const phymod_access_t *pa, uint8_t lane_event_log_level) {
  return(wrv_usr_ctrl_lane_event_log_level(lane_event_log_level));
}

err_code_t merlin_quadra28_set_usr_ctrl_disable_startup( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte( pa, &set_val));
  return(wrv_usr_ctrl_disable_startup_functions_byte(set_val.byte));
}

err_code_t merlin_quadra28_set_usr_ctrl_disable_startup_dfe( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st set_val) {
...
err_code_t merlin_quadra28_set_usr_ctrl_disable_steady_state( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte( pa, &set_val));
  return(wrv_usr_ctrl_disable_steady_state_functions_byte(set_val.byte));
}

err_code_t merlin_quadra28_set_usr_ctrl_disable_steady_state_dfe( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_dfe_functions_byte( pa, &set_val));
...
err_code_t merlin_quadra28_get_uc_core_config(const phymod_access_t *pa, struct merlin_quadra28_uc_core_config_st *get_val)
{
...
err_code_t merlin_quadra28_get_usr_ctrl_core_event_log_level( const phymod_access_t *pa, uint8_t *core_event_log_level) {
...
err_code_t merlin_quadra28_get_uc_lane_cfg( const phymod_access_t *pa, struct merlin_quadra28_uc_lane_config_st *get_val) {
...
err_code_t merlin_quadra28_get_usr_ctrl_lane_event_log_level( const phymod_access_t *pa, uint8_t *lane_event_log_level) {
...
err_code_t merlin_quadra28_get_usr_ctrl_disable_startup( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t merlin_quadra28_get_usr_ctrl_disable_startup_dfe( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t merlin_quadra28_get_usr_ctrl_disable_steady_state( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t merlin_quadra28_get_usr_ctrl_disable_steady_state_dfe( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t merlin_quadra28_poll_diag_done( const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(loop>10) {
         EFUN(merlin_quadra28_delay_us(10*timeout_ms));
     }
 }
...
/* poll for microcontroller to populate the dsc_data register */
err_code_t merlin_quadra28_poll_diag_eye_data(const phymod_access_t *pa,uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 uint16_t dscdata;
 if(!data || !status) {
     return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(merlin_quadra28_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(merlin_quadra28_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
...
#else
  {   uint16_t rddata;
      EFUN_PRINTF(("// ATE_LOG merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(merlin_quadra28_delay_us(timeout_ms));
      } else {
          EFUN(merlin_quadra28_delay_ms(timeout_ms));
      }
      EFUN(merlin_quadra28_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          EFUN_PRINTF(("// ATE_LOG merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      EFUN_PRINTF(("// ATE_LOG merlin_quadra28_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t merlin_quadra28_poll_dsc_state_equals_uc_tune( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
  uint16_t dsc_state;
...
err_code_t merlin_quadra28_poll_st_afe_tx_fifo_resetb_equals_1( const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
...
err_code_t merlin_quadra28_tx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(merlin_quadra28_tx_rptr_mode_timing( pa, enable)); /* Enable jitter filter enable */
...
err_code_t merlin_quadra28_rx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(merlin_quadra28_rx_rptr_mode_timing(enable));
...
err_code_t merlin_quadra28_tx_rptr_mode_timing( const phymod_access_t *pa, uint8_t enable) {
        ...
        EFUN(merlin_quadra28_delay_us(25));                            /* Wait for TX_PI to settle */
    return (ERR_CODE_NONE);
}
...
err_code_t merlin_quadra28_reg_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(merlin_quadra28_pmd_rdt_reg(pa,addr,&rddata));
...
err_code_t merlin_quadra28_uc_core_var_dump( const phymod_access_t *pa ) {
  uint8_t addr;
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {
    ...
    ESTM_PRINTF(("%02x ", merlin_quadra28_rdbc_uc_var( pa, __ERR, addr)));
  }
  return (ERR_CODE_NONE);
}


err_code_t merlin_quadra28_uc_lane_var_dump( const phymod_access_t *pa ) {
  uint8_t     rx_lock, uc_stopped = 0;
  uint16_t    addr;

  merlin_quadra28_uc_lane_info_st lane_info;

  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",merlin_quadra28_get_lane(pa)));
  ...
  if (rx_lock == 1) {
      ...
      if (!uc_stopped) {
          EFUN(merlin_quadra28_stop_rx_adaptation( pa, 1));
      }
  } else {
      EFUN(merlin_quadra28_pmd_uc_control( pa, CMD_UC_CTRL_STOP_IMMEDIATE,200));
  }

    EFUN(merlin_quadra28_get_uc_ln_info(pa, &lane_info));

    for (addr = 0x0; addr < lane_info.lane_ram_size; addr++) {
      if (!(addr % 26))  {
        EFUN_PRINTF(("\n%04x ",addr));
      }
      ESTM_PRINTF(("%02x ", merlin_quadra28_rdbl_uc_var( pa, __ERR, addr)));
    }

  if (rx_lock == 1) {
      if (!uc_stopped) {
          EFUN(merlin_quadra28_stop_rx_adaptation( pa, 0));
      }
  } else {
      EFUN(merlin_quadra28_stop_rx_adaptation( pa, 0));
  }
...
err_code_t merlin_quadra28_display_diag_data( const phymod_access_t *pa, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin_quadra28_get_core(),merlin_quadra28_get_lane(pa)));
    EFUN(merlin_quadra28_version( pa, &api_version));
    ...
    if (rx_lock == 1) {
        ...
        EFUN(merlin_quadra28_display_lane_state_hdr( pa ));
        EFUN(merlin_quadra28_display_lane_state( pa ));
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(merlin_quadra28_read_event_log( pa, trace_mem,EVENT_LOG_HEX));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin_quadra28_display_eye_scan( pa ));
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(merlin_quadra28_reg_dump( pa ));
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin_quadra28_uc_core_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin_quadra28_uc_lane_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin_quadra28_display_lane_debug_status( pa ));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_merlin_quadra28_display_ber_scan_data( pa, ber_mode, timer_control,  err_threshold));
...
err_code_t merlin_quadra28_diag_access( const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin_quadra28_pmd_rdt_reg(pa,i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin_quadra28_pmd_rdt_reg(pa,addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin_quadra28_pmd_wr_reg(pa,addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_quadra28_rdbc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin_quadra28_rdbc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_quadra28_rdbl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin_quadra28_rdbl_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_quadra28_rdwc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin_quadra28_rdwc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_quadra28_rdwl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin_quadra28_rdwl_uc_var( pa, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_quadra28_rdbc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_quadra28_wrbc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_quadra28_rdbl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_quadra28_wrbl_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_quadra28_rdwc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_quadra28_wrwc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_quadra28_rdwl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_quadra28_wrwl_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _merlin_quadra28_rdb_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_merlin_quadra28_rdb_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _merlin_quadra28_rdb_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_merlin_quadra28_wrb_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _merlin_quadra28_rdw_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_merlin_quadra28_rdw_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _merlin_quadra28_rdw_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_merlin_quadra28_wrw_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin_quadra28_pmd_uc_cmd_with_data( pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_merlin_quadra28_display_ber_scan_data( pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    case SRDS_EN_BREAKPOINT: 
    case SRDS_GOTO_BREAKPOINT:
    case SRDS_RD_BREAKPOINT:
    case SRDS_DIS_BREAKPOINT:
    default: EFUN_PRINTF(("Invalid request type merlin_quadra28_diag_access\n"));
    }
...
err_code_t merlin_quadra28_read_event_log( const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(merlin_quadra28_event_log_stop( pa ));

    EFUN(merlin_quadra28_event_log_readmem( pa, trace_mem));

    EFUN(merlin_quadra28_event_log_display( pa, trace_mem, display_mode));

    return(ERR_CODE_NONE);
}

err_code_t merlin_quadra28_event_log_stop( const phymod_access_t *pa ) {
    ...
    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));

    return(ERR_CODE_NONE);
}

err_code_t merlin_quadra28_event_log_readmem( const phymod_access_t *pa, uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
    ...
    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));

    return(ERR_CODE_NONE);
}

err_code_t merlin_quadra28_event_log_display( const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
        ...
        while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
            ...
            if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                if (!is_last_event_timewrap) {                                                         /* display the rest of previous event info */
                    EFUN_PRINTF((","));
                    EFUN(_merlin_display_event( pa, prev_event,(uint8_t)prev_rr,prev_cursor,curr_cursor,post_cursor,supp_info));
                ...
                EFUN(merlin_quadra28_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t merlin_quadra28_display_state ( const phymod_access_t *pa ) {
    EFUN(merlin_quadra28_display_core_state( pa ));
    EFUN(merlin_quadra28_display_lane_state_hdr( pa ));
    EFUN(merlin_quadra28_display_lane_state( pa ));
    EFUN(merlin_quadra28_display_lane_state_legend( pa ));
    return(ERR_CODE_NONE);
}

err_code_t merlin_quadra28_display_config ( const phymod_access_t *pa ) {
    EFUN(merlin_quadra28_display_core_config( pa ));
    EFUN(merlin_quadra28_display_lane_config( pa ));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin_quadra28_read_die_temperature ( const phymod_access_t *pa, int16_t *die_temp) {
    ...
    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
...
err_code_t merlin_quadra28_read_die_temperature_double (USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    EFUN_PRINTF((" Function 'merlin_quadra28_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}

SDK_STATIC err_code_t _merlin_quadra28_get_osr_mode_define( const phymod_access_t *pa, merlin_quadra28_osr_mode_st_define *imode) {
    merlin_quadra28_osr_mode_st_define mode;

     USR_MEMSET(&mode, 0, sizeof(merlin_quadra28_osr_mode_st_define));
...
err_code_t merlin_quadra28_read_lane_state_define(const phymod_access_t *pa, merlin_quadra28_lane_state_st_define *istate) {
  ...
  USR_MEMSET(&state, 0, sizeof(merlin_quadra28_lane_state_st_define));
  ...
  USR_MEMSET(&osr_mode, 0, sizeof(merlin_quadra28_osr_mode_st_define));
  EFUN(_merlin_quadra28_get_osr_mode_define( pa, &osr_mode));
