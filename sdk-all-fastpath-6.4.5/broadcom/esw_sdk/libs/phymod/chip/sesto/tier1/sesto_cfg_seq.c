                            ...
                            unsigned char prg_eeprom)
{
    ...
    if (prg_eeprom) {
        eeprom_num_of_pages = (uint16_t)master_len/SESTO_M0_EEPROM_PAGE_SIZE;

        /* Start Page of EEPROM*/
        WRITE_SESTO_PMA_PMD_REG(pm_acc, SES_GEN_CNTRLS_GPREG14_ADR, 
                          0);
        WRITE_SESTO_PMA_PMD_REG(pm_acc, SES_GEN_CNTRLS_GPREG15_ADR, 
                          eeprom_num_of_pages);
    	/* Reset spi */
    	/* Assert Reset */
        READ_SESTO_PMA_PMD_REG(pm_acc, SES_GEN_CNTRLS_GEN_CONTROL2_ADR, 
                          gen_ctrl2.data);
        gen_ctrl2.fields.spi_rstb = 0;
        gen_ctrl2.fields.spi2x_rstb = 0;
        WRITE_SESTO_PMA_PMD_REG(pm_acc, SES_GEN_CNTRLS_GEN_CONTROL2_ADR, 
                          gen_ctrl2.data);
    ...
    boot_por.fields.spi_port_used = prg_eeprom;
                           ...
                           const phymod_phy_inf_config_t *config)
{
    ...
    if (core == SESTO_MERLIN_CORE) {
        ...
        if (cfg.data_rate == SESTO_SPD_20G) {
        ...
        if (cfg.data_rate == SESTO_SPD_10G) {
 ...
 */
int _sesto_config_dut_mode_reg(const phymod_phy_access_t *pa, const phymod_phy_inf_config_t* config) 
{
    ...
    if (config->data_rate != SESTO_SPD_100G) {
        READ_SESTO_PMA_PMD_REG(&pa->access, DP_SESTO_MODE_CTRL0_ADR, mode_ctrl0.data);
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
            if(config->data_rate == SESTO_SPD_10G) {
                if ((aux_mode->BCM84793_capablity && core == SESTO_MERLIN_CORE) || core == SESTO_FALCON_CORE) {
                    if (pa->access.lane_mask & (1 << port_idx)) {
                       is_lane_used =  sesto_is_lane_mask_set(pa, port_idx, config);
                    } else {
                       continue;
                    }
                } else { /*  For non BCM84793 with Merlin*/
                    if (pa->access.lane_mask & (1 << ((port_idx < 2) ? port_idx : (port_idx + 2)))) {
                       is_lane_used =  sesto_is_lane_mask_set(pa, port_idx, config);
                    } else {
                       continue;
                    }
                }
            } else if(config->data_rate == SESTO_SPD_40G && (!aux_mode->pass_thru)) {
                if (core == SESTO_MERLIN_CORE) {
                    /* MUX Mode*/
           ...
           if (!is_lane_used) {
            ...
            /* Program Modes */
            if (((config->data_rate == SESTO_SPD_40G || config->data_rate == SESTO_SPD_20G) && (port_idx == 0)) ||
                 ((config->data_rate == SESTO_SPD_10G) && (port_idx < 2))) {
                if (config->data_rate == SESTO_SPD_40G) {
                ...
                if (config->data_rate == SESTO_SPD_20G) {
                    if (aux_mode->pass_thru) {
                ...
                if (config->data_rate == SESTO_SPD_10G) {
                    if ((pa->access.lane_mask & 0x4) || (pa->access.lane_mask & 0x10)) { 
                        mode_ctrl0.fields.mode_multiport2_10g1 = 1;
                    } if ((pa->access.lane_mask & 0x8) || (pa->access.lane_mask & 0x20)) { 
    ...
    mode_ctrl0.fields.mode_100g_gbox_en = (config->data_rate == SESTO_SPD_100G) ? 1 : 0;
    mode_ctrl0.fields.mode_40g_passthru_en = (config->data_rate == SESTO_SPD_40G) && 
                                             (aux_mode->pass_thru);

    PHYMOD_DEBUG_VERBOSE(("MODE CTRL0 :0x%x\n", mode_ctrl0.data));

    WRITE_SESTO_PMA_PMD_REG(&pa->access, DP_SESTO_MODE_CTRL0_ADR, mode_ctrl0.data);
                        ...
                        SESTO_AN_MODE mode)
{
    ...
    if (config->data_rate != SESTO_SPD_100G) {
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
           ...
           if (!is_lane_used) {
            ...
            READ_SESTO_PMA_PMD_REG(acc, DP_SESTO_MODE_CTRL1_ADR, mode_ctrl1.data);
            /* Program Modes */
            if (((config->data_rate == SESTO_SPD_40G || config->data_rate == SESTO_SPD_20G) && (port_idx == 0)) ||
                     ((config->data_rate == SESTO_SPD_10G) && (port_idx < 2))) {
                ...
                } else if(config->data_rate == SESTO_SPD_10G) {
                    if (pa->access.lane_mask & 0x1) { 
                        mode_ctrl1.fields.mode_dmept1_en = (mode == SESTO_AN_PROP) ? 1 : 0;
    ...
    WRITE_SESTO_PMA_PMD_REG(acc, DP_SESTO_MODE_CTRL1_ADR, mode_ctrl1.data);
    return PHYMOD_E_NONE;
}
...
int _sesto_interface_set(const phymod_access_t *pa, const phymod_phy_inf_config_t* config)
{
    ...
    SES_GEN_CNTRLS_GPREG12_TYPE_T   gpreg12;
    SES_GEN_CNTRLS_GPREG13_TYPE_T   gpreg13;
    phymod_phy_inf_config_t cpy_config;
    uint16_t intf = config->interface_type;
    uint16_t dfe_en = 0;
    uint16_t dfe_lp_en = 0;

    PHYMOD_MEMSET(&gpreg11, 0, sizeof(gpreg11));
    PHYMOD_MEMSET(&gpreg12, 0, sizeof(gpreg12));
    PHYMOD_MEMSET(&gpreg13, 0, sizeof(gpreg13));
    PHYMOD_MEMCPY(&cpy_config, config, sizeof(phymod_phy_inf_config_t));
    SESTO_GET_IP(pa, cpy_config, ip);

    if (intf == phymodInterfaceSR4 || intf == phymodInterfaceSR10 || intf == phymodInterfaceER4 ||
        intf == phymodInterfaceSR || intf == phymodInterfaceLR || intf == phymodInterfaceER) {
        ...
        dfe_en = 1;
        dfe_lp_en = 0;
    } else if (intf == phymodInterfaceKR4 || intf == phymodInterfaceKR2 ||
    ...
    if (ip == SESTO_MERLIN_CORE) {
        READ_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GPREG12_ADR, gpreg12.data);
        gpreg12.fields.media_type_10g = media_type;
        gpreg12.fields.scrambling_dis_10g = 0; 
        gpreg12.fields.dfe_en_10g = dfe_en;
        WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GPREG12_ADR, gpreg12.data);
    } else {
        READ_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GPREG13_ADR, gpreg13.data);
        gpreg13.fields.media_type_20g = media_type;
        gpreg13.fields.scrambling_dis_20g = 0;
        gpreg13.fields.dfe_en_20g = dfe_en;
        gpreg13.fields.dfe_lp_en_20g = dfe_lp_en; 
        WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_GPREG13_ADR, gpreg13.data);
    }
    ...
    PHYMOD_DEBUG_VERBOSE(("GPREG13 data:%x\n",gpreg13.data));
    PHYMOD_DEBUG_VERBOSE(("GPREG12 data:%x\n",gpreg12.data));
    PHYMOD_DEBUG_VERBOSE(("GPREG11 data:%x\n",gpreg11.data));

    PHYMOD_IF_ERR_RETURN(
        _sesto_set_ieee_intf(pa, intf, &cpy_config, ip));
...
int _sesto_set_ieee_intf(const phymod_access_t *pa, uint16_t intf, const phymod_phy_inf_config_t *config, uint16_t ip)
{
    ...
    PHYMOD_IF_ERR_RETURN(
        _sesto_port_from_lane_map_get(pa, config, &port));
    
    READ_SESTO_PMA_PMD_REG(pa, (SES_GEN_CNTRLS_MICRO_SYNC_0_ADR + port), mod_sel.data);
    SESTO_GET_INTF_SIDE(pa, intf_side);
...
int _sesto_phy_interface_config_set(const phymod_phy_access_t* pa, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    phymod_phy_inf_config_t cfg;
    phymod_phy_inf_config_t cpy_config;
    SESTO_DEVICE_AUX_MODE_T *aux_mode;
    ...
    PHYMOD_IF_ERR_RETURN(sesto_get_chipid(&pa->access, &chip_id, &rev));
    /* CHIP ID 82792/82790/82796 merlin core on system side */
    if (chip_id == SESTO_CHIP_ID_2 || chip_id == SESTO_CHIP_ID_3 || chip_id == SESTO_CHIP_ID_4) {
    ...
    PHYMOD_IF_ERR_RETURN (
       _sesto_phy_interface_config_get(&pa->access, 0, &cfg));

    /* Configure Ref Clock*/
    PHYMOD_DEBUG_VERBOSE(("Configuring REF clock\n"));
    PHYMOD_IF_ERR_RETURN(
          _sesto_configure_ref_clock(&pa->access, config->ref_clock));
    
    /* Configure DUT MODE */
    PHYMOD_IF_ERR_RETURN(
        _sesto_config_dut_mode_reg(pa, &cpy_config));
    
    /* Enable UDMS for non-an MODE */
    PHYMOD_DEBUG_VERBOSE(("Configuring UDMS\n"));
    PHYMOD_IF_ERR_RETURN(
        _sesto_udms_config(pa, &cpy_config, SESTO_AN_NONE));

    /* Configure PLL Divider*/
    if ((config->data_rate != cfg.data_rate) && (config->ref_clock != cfg.ref_clock)) {
        PHYMOD_DEBUG_VERBOSE(("Configuring PLL divider\n"));
        PHYMOD_IF_ERR_RETURN(
            _sesto_config_pll_div(&pa->access, cpy_config));
    }
    PHYMOD_USLEEP(100000);

    /* Configure Interface */
    PHYMOD_IF_ERR_RETURN(
        _sesto_interface_set(&pa->access, &cpy_config));

    return PHYMOD_E_NONE;
}

int _sesto_phy_interface_config_get(const phymod_access_t *pa, uint32_t flags, phymod_phy_inf_config_t *config)
{
    ...
    if (clock == 0x2) {
        ...
        config->ref_clock = phymodRefClk174Mhz; 
    }
    PHYMOD_IF_ERR_RETURN(
       _sesto_get_ieee_intf(pa, ip, config, &intf_side));
    config->interface_type = intf_side;
    
    return PHYMOD_E_NONE;
}

int _sesto_phy_pcs_link_get(const phymod_access_t *pa, uint32_t *pcs_link)
{
    if (pcs_link) {
        *pcs_link = 0xffff;
    } else {
    ...
    PHYMOD_IF_ERR_RETURN(_sesto_get_pcs_link_status(pa, pcs_link));
...
int _sesto_core_dp_rstb(const phymod_access_t *pa, const phymod_phy_inf_config_t config)
{
    uint16_t lane = 0, lock = 0;
    uint16_t ip = 0, count = 0;
...
int _sesto_get_pcs_link_status(const phymod_access_t *pa, uint32_t *link_sts)
{
    ...
    *link_sts = 0xffff;
    ...
    PHYMOD_IF_ERR_RETURN(
         _sesto_pcs_link_monitor_enable_get(pa, &get_pcs));
    ...
    if (get_pcs) {
                ...
                if (ip == SESTO_FALCON_CORE) {
                    ...
                    if (fmon_live_link.fields.fmon0_live_stat & 1) { 
                        *link_sts &= 1;
                        ...
                        if (fmon_main_sts.fields.fmon0_main_stat & 4) {
                    ...
                    if (mmon_live_link.fields.mmon0_live_stat & 1) { 
                        *link_sts &= 1;
                        ...
                        if (mmon_main_sts.fields.mmon0_main_stat & 4) {
                            /* Loss of link : read error cntr*/
        ...
        PHYMOD_IF_ERR_RETURN(
                  _sesto_rx_pmd_lock_get(pa, link_sts));
    }
    return PHYMOD_E_NONE;
}
int _sesto_merlin_falcon_lane_map_get(const phymod_access_t *pa, const phymod_phy_inf_config_t *cfg,
...
int _sesto_falcon_lpbk_set(const phymod_access_t* pa, const phymod_phy_inf_config_t *config,
                           phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    PHYMOD_IF_ERR_RETURN(
        _sesto_merlin_falcon_lane_map_get(pa, config, &mer_lane_mask, &fal_lane_mask));
                ...
                if (fal_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_FALCON_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_rmt_lpbk(pa, enable));
                }
            }
            SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
        break;
                ...
                if (mer_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_MERLIN_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_dig_lpbk_rptr(pa, enable, DATA_IN_SIDE));
                }
            }
            SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
                ...
                if (fal_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_FALCON_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_dig_lpbk_rptr(pa, enable, DIG_LPBK_SIDE));
                    /* Disable the prbs_chk_en_auto_mode while seting digital loopback */
                    PHYMOD_IF_ERR_RETURN(wr_falcon_furia_sesto_prbs_chk_en_auto_mode((!enable)));
                }
            }
...
int _sesto_merlin_lpbk_set(const phymod_access_t* pa, const phymod_phy_inf_config_t *config,
                           phymod_loopback_mode_t loopback, uint32_t enable)
{
    uint16_t lane = 0;
    uint32_t mer_lane_mask = 0;
                ...
                if (mer_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_MERLIN_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_rmt_lpbk(pa, enable));
                } 
            }
            SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
        break;
                ...
                if (fal_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_FALCON_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_dig_lpbk_rptr(pa, enable, DATA_IN_SIDE));
                }
            }
            SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
                ...
                if (mer_lane_mask & (1 << lane)) {
                    PHYMOD_IF_ERR_RETURN(
                      _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                                SESTO_MERLIN_CORE, SESTO_DEV_PMA_PMD,
                                0, lane));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_dig_lpbk_rptr(pa, enable, DIG_LPBK_SIDE));
                    /* Disable the prbs_chk_en_auto_mode while seting digital loopback */
...
int _sesto_if_ctrl_frc_tx_disable(const phymod_access_t *pa, const phymod_phy_inf_config_t *config,
                                  uint32_t enable)
{
    uint16_t lane = 0;
    uint32_t mer_lane_mask = 0;
...
int _sesto_loopback_set(const phymod_access_t *pa, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    if (ip == SESTO_FALCON_CORE) {
       /* FALCON core loopback set */
       PHYMOD_IF_ERR_RETURN(
           _sesto_falcon_lpbk_set(pa, &config, loopback, enable));
    } else {
       /* MERLIN core loopback set */
       PHYMOD_IF_ERR_RETURN(
           _sesto_merlin_lpbk_set(pa, &config, loopback, enable));
    }

    if (!enable) {
        /* Clear any forces on Tx disables set by HW while disable */
        PHYMOD_IF_ERR_RETURN(_sesto_if_ctrl_frc_tx_disable(pa, &config, enable));
        /* Set bit 13 in GPREG11 for loopback change */
        SESTO_CHIP_FIELD_WRITE(pa, SES_GEN_CNTRLS_GPREG11, lpbk_change, 1);
    }
        ...
        if (lane_mask & (1 << lane)) {
            ...
            if (ip == SESTO_FALCON_CORE) {
                ...
                if (rx->vga.enable) {
                    ...
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_write_rx_afe(pa, RX_AFE_VGA, rx->vga.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 0));
                }
                /*dfe set*/
                for (indx = 0 ; indx < PHYMOD_NUM_DFE_TAPS; indx++){
                    if(rx->dfe[indx].enable){
                        PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 1));
                        PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_write_rx_afe(pa, RX_AFE_DFE1+indx, rx->dfe[indx].value));
                    } else {
                        PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 0));
                    }
                }
                ...
                if(rx->peaking_filter.enable){
                    /* first stop the rx adaption */
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_write_rx_afe(pa, RX_AFE_PF, rx->peaking_filter.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 0));
                }
                ...
                if(rx->low_freq_peaking_filter.enable){
                    /* first stop the rx adaption */
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_write_rx_afe(pa, RX_AFE_PF2, rx->low_freq_peaking_filter.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_stop_rx_adaptation(pa, 0));
                }
            } else {
                ...
                if (rx->vga.enable) {
                    /* first stop the rx adaption */
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_write_rx_afe(pa, RX_AFE_VGA, rx->vga.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 0));
                }
                /*dfe set*/
                ...
                for (indx = 0 ; indx < MAX_MERLIN_DFES_TAPS; indx++){
                    if(rx->dfe[indx].enable){
                        PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 1));
                        PHYMOD_IF_ERR_RETURN(merlin_sesto_write_rx_afe(pa, RX_AFE_DFE1+indx, rx->dfe[indx].value));
                    } else {
                        PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 0));
                    }
                }
                ...
                if(rx->peaking_filter.enable){
                    /* first stop the rx adaption */
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_write_rx_afe(pa, RX_AFE_PF, rx->peaking_filter.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 0));
                }
                ...
                if(rx->low_freq_peaking_filter.enable){
                    /* first stop the rx adaption */
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 1));
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_write_rx_afe(pa, RX_AFE_PF2, rx->low_freq_peaking_filter.value));
                } else {
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_stop_rx_adaptation(pa, 0));
                } 
            }
    ...
    for (lane = 0; lane < max_lane; lane ++) {
        if (lane_mask & (1 << lane)) {
            ...
            if (ip == SESTO_FALCON_CORE) {
                ...
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_read_rx_afe(pa, RX_AFE_VGA, &vga));
                rx->vga.value = vga; 

                /*dfe get*/
                for (indx = 0 ; indx < PHYMOD_NUM_DFE_TAPS; indx++){
                    PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_read_rx_afe(pa, (RX_AFE_DFE1+indx), &dfe));
                    rx->dfe[indx].value = dfe;
                }

                /*peaking filter get*/
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_read_rx_afe(pa, RX_AFE_PF, &pf));
                rx->peaking_filter.value = pf;
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_read_rx_afe(pa, RX_AFE_PF2, &low_freq_pf));
                ...
                PHYMOD_IF_ERR_RETURN(merlin_sesto_read_rx_afe(pa, RX_AFE_VGA, &vga));
                rx->vga.value = vga;

                /*dfe get*/
                for (indx = 0 ; indx < MAX_MERLIN_DFES_TAPS; indx++){
                    PHYMOD_IF_ERR_RETURN(merlin_sesto_read_rx_afe(pa, (RX_AFE_DFE1+indx), &dfe));
                    rx->dfe[indx].value = dfe;
                }

                /*peaking filter get*/
                PHYMOD_IF_ERR_RETURN(merlin_sesto_read_rx_afe(pa, RX_AFE_PF, &pf));
                rx->peaking_filter.value = pf;
                PHYMOD_IF_ERR_RETURN(merlin_sesto_read_rx_afe(pa, RX_AFE_PF2, &low_freq_pf));
                rx->low_freq_peaking_filter.value = low_freq_pf;
                break;
            }
        }
...
int _sesto_phy_status_dump(const phymod_access_t *pa)
{
    ...
    if (ip == SESTO_FALCON_CORE) { /* Falcon core status dump */
        ...
        PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_display_core_config(pa));
        PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_display_core_state(pa));
                break;
            }
        }
        ...
        PHYMOD_IF_ERR_RETURN(merlin_sesto_display_core_config(pa));
        PHYMOD_IF_ERR_RETURN(merlin_sesto_display_core_state(pa));
                break;
            }
        }
    }
    if (ip == SESTO_FALCON_CORE) { /* Falcon lanes status dump */
        for(lane = 0; lane < SESTO_MAX_FALCON_LANE; lane ++) {
            if (lane_mask & (1 << lane)) {
                ...
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_display_lane_state_hdr(pa));
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_display_lane_state(pa));
                PHYMOD_IF_ERR_RETURN(falcon_furia_sesto_display_lane_config(pa));
            }
        }
        SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
        ...
        for(lane = 0; lane < SESTO_MAX_MERLIN_LANE; lane ++) {
            if (lane_mask & (1 << lane)) {
                ...
                PHYMOD_IF_ERR_RETURN(merlin_sesto_display_lane_state_hdr(pa));
                PHYMOD_IF_ERR_RETURN(merlin_sesto_display_lane_state(pa));
                PHYMOD_IF_ERR_RETURN(merlin_sesto_display_lane_config(pa));
            }
        }
        SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
    }
...
int _sesto_tx_enable_set(const phymod_access_t *pa, int16_t enable) 
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
...
int _sesto_tx_enable_get(const phymod_access_t *pa, int16_t *enable)
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
    uint16_t lane = 0, max_lane = 0, reg_addr = 0;
...
int _sesto_tx_dp_reset(const phymod_access_t *pa)
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
...
int _sesto_rx_enable_set(const phymod_access_t *pa, int16_t enable) 
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
    uint16_t lane = 0, max_lane = 0;
...
int _sesto_rx_enable_get(const phymod_access_t *pa, int16_t *enable)
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
    uint16_t lane = 0, max_lane = 0;
...
int _sesto_rx_dp_reset(const phymod_access_t *pa)
{
    phymod_phy_inf_config_t config;
    uint16_t ip = 0;
    uint16_t lane = 0, max_lane = 0;
...
int _sesto_tx_lane_control_set(const phymod_access_t *pa,  phymod_phy_tx_lane_control_t tx_control)
{
    switch (tx_control) {
        ...
        case phymodTxReset: /* reset tx data path */
            PHYMOD_IF_ERR_RETURN(_sesto_tx_dp_reset(pa));
        break;
        case phymodTxSquelchOn: /* squelch tx */
            PHYMOD_IF_ERR_RETURN(_sesto_tx_enable_set(pa, SESTO_DISABLE));
        break;
        case phymodTxSquelchOff: /* squelch tx off */
            PHYMOD_IF_ERR_RETURN(_sesto_tx_enable_set(pa, SESTO_ENABLE));
        break;
        default:
            return PHYMOD_E_PARAM;
    }
...
int _sesto_rx_lane_control_set(const phymod_access_t *pa,  phymod_phy_rx_lane_control_t rx_control)
{
    switch (rx_control) {
        case phymodRxReset: /* reset rx data path */
            PHYMOD_IF_ERR_RETURN(_sesto_rx_dp_reset(pa));
            break;
        case phymodRxSquelchOn: /* squelch rx */
            PHYMOD_IF_ERR_RETURN(_sesto_rx_enable_set(pa, SESTO_DISABLE));
        break;
        case phymodRxSquelchOff: /* squelch rx off */
            PHYMOD_IF_ERR_RETURN(_sesto_rx_enable_set(pa, SESTO_ENABLE));
        break;
        default:
            return PHYMOD_E_PARAM;
    }
...
int _sesto_tx_lane_control_get(const phymod_access_t *pa,  phymod_phy_tx_lane_control_t *tx_control)
{
    ...
    switch (*tx_control) {
        ...
        break;
        case phymodTxSquelchOn: /* squelch tx */
            PHYMOD_IF_ERR_RETURN(_sesto_tx_enable_get(pa, &tx_ctrl));
            *tx_control = tx_ctrl;
        break;
        case phymodTxSquelchOff: /* squelch tx off */
            PHYMOD_IF_ERR_RETURN(_sesto_tx_enable_get(pa, &tx_ctrl));
...
int _sesto_rx_lane_control_get(const phymod_access_t *pa,  phymod_phy_rx_lane_control_t *rx_control)
{
    ...
    switch (*rx_control) {
        case phymodRxReset: /* reset rx data path */
            return PHYMOD_E_UNAVAIL;
        case phymodRxSquelchOn: /* squelch rx */
            PHYMOD_IF_ERR_RETURN(_sesto_rx_enable_get(pa, &rx_ctrl));
            *rx_control = (!rx_ctrl);
        break;
        case phymodRxSquelchOff: /* squelch rx off */
            PHYMOD_IF_ERR_RETURN(_sesto_rx_enable_get(pa, &rx_ctrl));
...
int _sesto_autoneg_ability_set(const phymod_access_t* pa, sesto_an_ability_t *an_ability)
{
    ...
    PHYMOD_IF_ERR_RETURN(_sesto_phy_interface_config_get(pa, 0, &config));
    
    if ((config.data_rate == SESTO_SPD_10G || config.data_rate == SESTO_SPD_20G) || 
        (config.data_rate == SESTO_SPD_100G && aux_mode->gearbox_100g_inverse_mode)) { 
        PHYMOD_DEBUG_ERROR(("Invalid config for CL73, AN does not support for 10G, 20G and 100G Inverse mode\n"));
...
int _sesto_autoneg_ability_get(const phymod_access_t* pa, sesto_an_ability_t *an_ability)
{
    return PHYMOD_E_NONE;
}
