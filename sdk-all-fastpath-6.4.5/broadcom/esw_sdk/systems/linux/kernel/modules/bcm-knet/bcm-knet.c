...
{
    /* Avoid reading this register */
    DEV_WRITE32(sinfo, CMICM_DMA_STAT_CLRr, DS_CMC_DESCRD_CMPLT_CLR(chan));
...
bkn_do_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    /*
     * Since we have to release our spinlock when servicing the Linux
     * network stack, we need do make sure that no other process
     * starts processing Rx DCBs.
     */
    if (sinfo->doing_rx) {
        return 0;
    }
    ...
    if (sinfo->rx[chan].use_rx_skb == 0) {
        /* Rx buffers are provided by BCM Rx API */
        dcbs_done = bkn_do_api_rx(sinfo, chan);
    } else {
        /* Rx buffers are provided by Linux kernel */
        dcbs_done = bkn_do_skb_rx(sinfo, chan, budget);
    }
...
bkn_rxtick(unsigned long context)
{
    ...
    /* In case we somehow missed one or more DMA descriptors */
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        if (sinfo->rx[chan].rate == 0) {
            unsigned long flags;
            ...
            if (bkn_do_rx(sinfo, chan, MAX_RX_DCBS) > 0) {
                bkn_rx_desc_done(sinfo, chan);
                sinfo->rx[chan].intr_miss++;
                DBG_WARN(("Interrupt check: Lost Rx descriptors\n"));
            }
...
bkn_proc_dstats_show(struct seq_file *m, void *v){
    ...
    list_for_each(list, &_sinfo_list) {
        ...
        seq_printf(m, "  Tx drop no link     %10u\n",
                        sinfo->tx.pkts_d_no_link);
        seq_printf(m, "  Tx drop oversize pkt %10u\n",
                        sinfo->tx.pkts_d_over_limit);
