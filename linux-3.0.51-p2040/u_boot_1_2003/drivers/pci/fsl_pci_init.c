					 ...
					 volatile pit_t *pi)
{
	...
	if ((pcie_cap == PCI_CAP_ID_EXP) && (pci_sz < sz)) {
		printf ("R0 bus_start: %llx phys_start: %llx size: %llx\n",
		...
		pci_sz = 1ull << __ilog2_u64(sz);
		if (sz) {
			printf ("R1 bus_start: %llx phys_start: %llx size: %llx\n",
				(u64)bus_start, (u64)phys_start, (u64)pci_sz);
			pci_set_region(r, bus_start, phys_start, pci_sz,
					PCI_REGION_MEM | PCI_REGION_SYS_MEMORY |
					PCI_REGION_PREFETCH);
	...
	printf ("R64 bus_start: %llx phys_start: %llx size: %llx\n",
	...
	pci_sz = 1ull << __ilog2_u64(sz);
	if (sz) {
		printf ("R2 bus_start: %llx phys_start: %llx size: %llx\n",
			(u64)bus_start, (u64)phys_start, (u64)pci_sz);
		pci_set_region(r, bus_start, phys_start, pci_sz,
				PCI_REGION_MEM | PCI_REGION_SYS_MEMORY |
				PCI_REGION_PREFETCH);
...
static void fsl_pcie_boot_master(pit_t *pi)
{
	...
	printf("PCIEBOOT - MASTER: Inbound window for slave's image; "
	...
	printf("PCIEBOOT - MASTER: Inbound window for slave's "
			"ucode and ENV; "
			"Local = 0x%llx, Bus = 0x%llx, Size = 0x%x\n",
...
void fsl_pci_init(struct pci_controller *hose, struct fsl_pci_info *pci_info)
{
	...
	printf("Outbound memory range: %llx:%llx\n", out_lo, out_hi);
	...
	for (r = 0; r < hose->region_count; r++)
		printf("PCI reg:%d %016llx:%016llx %016llx %08lx\n", r,
...
#define PEX_CSR0_LTSSM_SHIFT	2
		...
		if (ltssm == 1) {
			int i;
			printf("....PCIe link error. " "LTSSM=0x%02x.", ltssm);
			/* assert PCIe reset */
			...
			struct pci_controller *hose, int busno)
{
	...
	if (in_be32(&pci->pme_msg_det)) {
		out_be32(&pci->pme_msg_det, 0xffffffff);
		printf (" with errors.  Clearing.  Now 0x%08x",
			pci->pme_msg_det);
	}
