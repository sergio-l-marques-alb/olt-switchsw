...
#define TBICR_SETTINGS (TBICR_ANEG_ENABLE | TBICR_RESTART_ANEG | TBICR_FULL_DUPLEX | TBICR_SPEED1_SET) 
/*#define TBICR_SETTINGS (TBICR_PHY_RESET | TBICR_FULL_DUPLEX | TBICR_SPEED1_SET ) /*HUGO*/
...
static void dtsec_configure_serdes(struct fm_eth *priv)
{
	 ...
	 */
	tsec_local_mdio_write(phyregs, in_be32(&regs->tbipa), 0, TBI_TBICON,TBICON_CLK_SELECT);
	tsec_local_mdio_write(phyregs, in_be32(&regs->tbipa), 0, TBI_ANA,TBIANA_SGMII_ACK);
    tsec_local_mdio_write(phyregs, in_be32(&regs->tbipa), 0,TBI_CR, TBICR_SETTINGS);
