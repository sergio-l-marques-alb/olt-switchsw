...
static void permute (const char *,
		  char [][16][8],
		  char *);

static void round (int,
		  SR_UINT32 *);

static SR_INT32 f(SR_UINT32,
		  unsigned char []);

static SR_UINT32 ByteSwap(SR_UINT32 x);
...
static SR_INT32 (*sp)[64];	/* Combined S and P boxes */
    ...
    int mode)
{
    ...
    littleEndian = (srEndian() == SR_LITTLE_ENDIAN);


    if ((sp = (SR_INT32 (*)[64]) malloc(sizeof(SR_INT32) * 8 * 64)) == NULL) {
	return -1;
    }
    spinit();
...
{
    register int    i, j;
    register const char *ib;    /* ptr to input block */
    register char  *ob;   	/* ptr to output block */
    register char  *p, *q;
...
{
    register int    i;
    SR_UINT32       work[2];	/* Working data storage */
    SR_INT32        tmp;
