/* 
 * $Id: mpls.c,v 1.1 2011/04/18 17:11:11 mruas Exp $
                             ...
                             mpls_entry_entry_t *entry, int dual)
{
    ...
    soc_internal_read_reg(pcid_info,
          soc_reg_addr(pcid_info->unit, MPLS_ENTRY_HASH_CONTROLr,
                       REG_PORT_ANY, 0),
          tmp_hs);
    if (dual) {
    ...
    if (SOC_IS_TRIUMPH2(pcid_info->unit) || SOC_IS_APOLLO(pcid_info->unit) || SOC_IS_VALKYRIE2(pcid_info->unit)) {
        bits = soc_tr2_mpls_base_entry_to_key(pcid_info->unit, 
                                                   (uint32 *)entry, key);

        index = soc_tr2_mpls_hash(pcid_info->unit, hash_sel, bits, (uint32 *)entry, key);
    } else
#endif
    {
        ...
        index = soc_tr_mpls_hash(pcid_info->unit, hash_sel, bits, (uint32 *)entry, key);
    }
                                   ...
                                   mpls_entry_entry_t *entry)
{
    ...
    uint32 address = soc_mem_addr(pcid_info->unit, MPLS_ENTRYm,
                                  ...
                                  mpls_entry_entry_t *entry, uint32 *result)
{
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        index = bucket * 8 + slot;
        soc_internal_read_mem(pcid_info, soc_mem_addr(unit, MPLS_ENTRYm,
                                         SOC_MEM_BLOCK_ANY(unit, MPLS_ENTRYm),
                                         index), tmp);
    ...
    if (free_index != -1) {
        ...
        debugk(DK_VERBOSE, "write bucket %d, slot %d, index %d\n",
               index / 8, index % 8, index);
        soc_internal_write_mem(pcid_info, soc_mem_addr(unit, MPLS_ENTRYm,
                                          SOC_MEM_BLOCK_ANY(unit, MPLS_ENTRYm),
                                          index), (uint32 *)entry);
        return 0;
    }
