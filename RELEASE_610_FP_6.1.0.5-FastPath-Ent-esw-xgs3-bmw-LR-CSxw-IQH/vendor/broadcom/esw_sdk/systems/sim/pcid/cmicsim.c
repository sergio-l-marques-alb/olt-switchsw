/*
 * $Id: cmicsim.c,v 1.1 2011/04/18 17:11:11 mruas Exp $
...
soc_internal_read_mem(pcid_info_t *pcid_info, uint32 address, uint32 *data)
{
 ...
 found:
    ...
    if (SOC_IS_TUCANA(pcid_info->unit) && (mem != INVALIDm)) {
...
soc_internal_xgs3_table_dma(pcid_info_t *pcid_info)
{
    ...
    while (dma_count > 0) {
        ...
        for (i = 0; i < burst_count; i++) {
            soc_internal_read_bytes(pcid_info, table_addr, 
...
soc_internal_xgs3_tslam_dma(pcid_info_t *pcid_info)
{
    ...
    while (dma_count > 0) {
        ...
        for (i = 0; i < burst_count; i++) {
            ...
            soc_internal_write_bytes(pcid_info, table_addr, 
