/*
 * $Id: linux-kernel-bde.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
...
fixup_p2p_bridge(void)
{
    ...
    if ((dev = LINUX_PCI_FIND_DEVICE(BCM4716_VENDOR_ID, BCM4716PCIE_DEVICE_ID,
        NULL)) != NULL) {
        ...
        pci_read_config_dword(dev, BCM4716PCIE_DEV_CAP_REG, &tmp);
        if (debug >= 1)
            gprintk("DevCap (@%x): 0x%x%c\n", BCM4716PCIE_DEV_CAP_REG, tmp, (tmp & 0x7) != 1 ? ' ':'\n');
        if ((tmp & 0x7) != 1) {
            tmp16 = *((uint16 *)0xb800581C);                                                       
            if (debug >= 1)
                gprintk("spromData.MaxPayloadSize: 0x%x\n", tmp16);                                    
            if (((tmp16 & 0xe000) >> 13) != 0x1) {                                                 
                tmp161 = (tmp16 & ~0xe000) | (0x1 << 13);                                          
                *((uint16 *)0xb800581C) = tmp161;                                                  
                tmp16 = 0;                                                                         
                tmp16 = *((uint16 *)0xb800581C);                                                   
                if (debug >= 1)
                    gprintk("Enable spromData.MaxPayloadSize to 1 (256 bytes): 0x%x (%s w/ 0x%x)\n",   
                        tmp161, (((tmp16 & 0xe000) >> 13) == 0x1) ? "Success":"Fail", tmp16);          
            }                                                                                      
            pci_read_config_dword(dev, BCM4716PCIE_DEV_CAP_REG, &tmp);                             
            if (debug >= 1)
                gprintk("DevCap (@%x): now is 0x%x\n\n", BCM4716PCIE_DEV_CAP_REG, tmp);                
        }                                                                                          
                                                                                           
        tmp16 = *((uint16 *)0xb80054d8);                                                           
        if (debug >= 1) gprintk("DevControl (@%x): 0x%x\n", BCM4716PCIE_DEV_CTRL_REG, tmp16);                      
        if (!(tmp16 & MAX_PAYLOAD_256B) || !(tmp16 & MAX_READ_REQ_256B)) {                         
            tmp161 = tmp16 | MAX_PAYLOAD_256B | MAX_READ_REQ_256B;                                 
            *((uint16 *)0xb80054d8) = tmp161;                                                      
            tmp16 = 0;                                                                             
            tmp16 = *((uint16 *)0xb80054d8);                                                       
            if (debug >= 1) {
                gprintk("Enable DevControl MaxPayloadSize to 1 (256 bytes): 0x%x (%s w/ 0x%x)\n",      
                    tmp161, (tmp16 & MAX_PAYLOAD_256B) ? "Success":"Fail", tmp16);                     
...
/* VENDOR_BROADCOM ] */
    ...
    { BROADCOM_VENDOR_ID, BCM56332_DEVICE_ID, PCI_ANY_ID, PCI_ANY_ID },
    { BROADCOM_VENDOR_ID, BCM56330_DEVICE_ID, PCI_ANY_ID, PCI_ANY_ID },
...
#endif
        if (((phyidh == 0xFFFF) || (phyidh == 0x0)) & (dev < 1)) {
            /*second try SPI */
