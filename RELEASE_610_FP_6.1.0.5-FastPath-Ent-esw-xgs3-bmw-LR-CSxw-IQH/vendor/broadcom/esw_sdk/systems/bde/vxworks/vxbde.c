/*
 * $Id: vxbde.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
...
#define PLX_LAS_EN          0x01000000  /* Space Enable bit                */
...
_plx_las_baroff_get(pci_dev_t *dev)
{
    ...
    if (local_config_bar) {
         ...
         */
        if ((*(uint32 *)(local_config_bar + PLX_LAS0_BA)) & PLX_LAS_EN) {
            baroff = PCI_CONF_BAR2;
        } else if ((*(uint32 *)(local_config_bar + PLX_LAS1_BA)) & PLX_LAS_EN) {
...
probe_plx_local_bus(void)
{
    ...
    if (vxd->bde_dev.device != BM9600_DEVICE_ID) {
        printf("probe_plx_local_bus: wrong BME type: "
                "0x%x (vs 0x%x)\n", vxd->bde_dev.device, BM9600_DEVICE_ID);
        return -1;
    }
    return 0;
}
