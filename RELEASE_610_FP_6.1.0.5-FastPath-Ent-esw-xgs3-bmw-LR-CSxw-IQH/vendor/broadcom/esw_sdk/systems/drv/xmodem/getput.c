/*
 * $Id: getput.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
 *
 ...
 */
int
getbyte(int fildes, char *ch)		/* Buffered disk read */
{
...
readbyte(int seconds){
    ...
    if ((select(1, (fd_set *) &readfd,
		(fd_set *)0, (fd_set *)0, &tmout)) == 0){
	if (XDEBUG)
	    fprintf(LOGFP,
		    "DEBUG: readbyte TIMEOUT after %d seconds\n", seconds);
	return(TIMEOUT);
    }
...
/* 
 * get a buffer (length bufsize) from data stream -- timeout if
 * "seconds" elapses.  Read bunches of characters to save system
 * overhead; Further process data while kernel is reading stream
 * (calculating "checksum").  Try to nap long enough so kernel collects
 * 100 characters or so until we wake up unless TOOBUSY is set.
 */
...
int
readbuf(int bufsize,        /* number of chars to be read */
	int seconds,        /* timeout period for each read */
	int tmode,          /* transmission mode: TRUE if text */
	int amode,          /* transmission mode: TRUE if apple macintosh */
...
sendbyte(char data)
{
    ...
    /* write the byte (assume it goes NOW; no flushing needed) */
    if (write(1, &data, 1) != 1)  	
...
void
writebuf(char *buffer, int nbytes)
{
    if (XDEBUG)
	fprintf(LOGFP, "DEBUG: writebuf (%d bytes)\n", nbytes);
    /* write the buffer (assume no TIOCFLUSH needed) */    
...
napms (int milliseconds)
{
    ...
    taskDelay((sysClkRateGet() * milliseconds) / 1000);
}
 
/* set and restore tty modes for XMODEM transfers */
/* These routines are VERY vxWorks specific */

int ttyOptions, ttyNewOptions;	/* vxWorks tty options */
SIGVEC origSignal, newSignal;
...
setmodes(void)
{
    ...
    sigvec(SIGTERM, (SIGVEC *) NULL,  &origSignal);
...
{
    if (ioctl(0,FIOSETOPTIONS, ttyOptions) !=  0){ 
	if (!errcall)
...
{
#if 0
    ttyspeed = sioIoctl(0, SIO_BAUD_GET, &ttyspeed);
    if (ttyspeed == ERROR){
	ttyspeed = 9600;
	logit ("Can't determine line speed; assuming 9600 bps\n");
    }
#endif
    ttyspeed = 9600;
}
...
stopsig(void)
{
    ttyOptions = ioctl(0, FIOGETOPTIONS,0);
    ttyNewOptions = ttyOptions & ~OPT_MON_TRAP;
    ioctl(0, FIOSETOPTIONS, ttyNewOptions);
}
