/*
 * $Id: vxbde_alpha8245.c,v 1.1 2011/04/18 17:11:11 mruas Exp $
...
#ifdef LVL7_FIXUP
extern uint32 bcm_bde_soc_cm_memory_base;
#else
       ...
       uint8 pciRevID)
{
    ...
    /*
     * PLX bridge itself won't be part of _devices[]
...
#if defined(LVL7_ALPHA8245) && defined(LVL7_FIXUP)
        /* LVL7 start */
        if (((pciDevID == 0xb304) || (pciDevID == 0xb314) || 
              (pciDevID == 0xb504)) && (1 == _n_devices))
        {
          tmp = pci_config_getw(dev, PCI_CONF_INTERRUPT_LINE);
          tmp = (tmp & 0xffffff00) | 0; /* Interrupt line 0 */
          pci_config_putw(dev, PCI_CONF_INTERRUPT_LINE, tmp);
        }
        else
        {
          tmp = pci_config_getw(dev, PCI_CONF_INTERRUPT_LINE);
          tmp = (tmp & 0xffffff00) | 1; /* Interrupt line 1 */
          pci_config_putw(dev, PCI_CONF_INTERRUPT_LINE, tmp);
        }
        /* LVL7 end */
#else
printf("%s:line %d n_devices = %d, int_line = %d\n", __FUNCTION__, __LINE__, _n_devices, _bus.int_line);
...
int
vxbde_create_alpha8245(vxbde_bus_t *bus, 
         ibde_t **bde)
{
    if (_n_devices == 0) {
    _bus = *bus;
