/* 
 * $Id: chipsim.c,v 1.1 2011/04/18 17:11:10 mruas Exp $
 ...
 * File:        chipsim.c
 *
 *     The main pcid file; simulates BCM 5600, 05, 15, 70, 80 and 90.
 *
 * Requires:
 *     util.c routines
...
pcid_counter_activity(event_t * ev)
{
    ...
    PBMP_ITER(PBMP_PORT_ALL(pcid_info->unit), port) {
        ...
        for (i = 0; i < cmap->cmap_size; i++) {
            ...
            reginfo = &SOC_REG_INFO(pcid_info->unit, ctr_reg);
...
pcid_check_packet_input(event_t * ev)
{
    ...
    if ((fp = fopen("pkt", "r")) != 0) {
        ...
        for(i = 0; i < 8; i++) {
            pkt->dcbd[i] = 0x12345678; /* FB/ER 3 + 5 words of HG + PBI */
        }
#ifdef BCM_XGS_SUPPORT
        soc_higig_field_init(pcid_info->unit, pkt->dcbd);
