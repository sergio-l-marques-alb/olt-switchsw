/*
 * $Id: bcmx_ipmc.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
{
    int			    rv = BCM_E_NONE;/* Function return value.         */  
        ...
        if (BCM_SUCCESS(tmp_rv)) {
            ...
            if (BCM_FAILURE(convert_rv)) {
                ...
                if (bcm_ipmc.ts) {
                    ...
                    /* port */
                    convert_rv =
                        _bcmx_dest_from_unit_port(&data->port_tgid,
                                                  bcm_unit,
                                                  bcm_ipmc.port_tgid,
                                                  BCMX_DEST_CONVERT_DEFAULT);
                    if (BCM_FAILURE(convert_rv)) {
                      ...
                      void *user_data)
{
    ...
    BCMX_LOCAL_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_ipmc_traverse(bcm_unit, flags, cb, user_data);
        if (BCM_SUCCESS(tmp_rv)) {
            return tmp_rv;
        }
	    RV_REPLACE(tmp_rv, rv);
