/*
 * $Id: bcmx_l2.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
bcmx_l2_addr_add(bcmx_l2_addr_t *l2addr, bcmx_lplist_t *port_block)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        if (_bcmx_l2_addr_to_bcm(bcm_unit, &bcm_l2, l2addr, port_block) < 0) {
            ...
            if (BCM_SUCCESS(tmp_rv)) {   /* Note that a call has succeeded */
                call_made = TRUE;
            }
 ...
 *      bcmx_l2_addr_get
 * Notes:
 *      If port_block is non-NULL, then all units are called
 *      in order to assemble the list of blocked ports.  Otherwise, just
 *      the first successful call is used.  *port_block must be
                 ...
                 bcmx_lplist_t *port_block)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        if (BCM_SUCCESS(tmp_rv)) {   /* Successfully found addr */
            rv = BCM_E_NONE;
            if (first_time) { /* Call address conversion */
                ...
                if (port_block == NULL) { /* Done now */
                    break;
                }
            } else { /* Not first time; querying to get port block list */

                /*
                 * Otherwise, have to query all units to get full
                 * status of port blocking for the address.
                 */
                BCMX_LPLIST_PBMP_ADD(port_block,
                                     bcm_unit,
                                     bcm_l2.block_bitmap);
            }
        } else if ((tmp_rv != BCM_E_NOT_FOUND) &&
                   (tmp_rv != BCM_E_UNAVAIL) &&
                   (rv != BCM_E_NONE)) {
            rv = tmp_rv;   /* Real error, no success yet */
        }
    }
...

{
    ...
    int i, bcm_unit, tmp_rv;
    bcm_l2_addr_t bcm_l2;

    BCMX_UNIT_ITER(bcm_unit, i) {
        bcm_l2_addr_t_init (&bcm_l2, mac_addr, vid);
        tmp_rv = bcm_l2_addr_get(bcm_unit, mac_addr, vid, &bcm_l2);
        if (BCM_SUCCESS(tmp_rv)) { /* Device knows addr; front panel? */
            /* Get the port configuration for the device */
            if (bcm_l2_port_native(bcm_unit, bcm_l2.modid, bcm_l2.port) > 0) {
...
bcmx_l2_age_timer_get(int *age_seconds)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_l2_age_timer_get(bcm_unit, age_seconds);
...
bcmx_l2_bpdu_get(int index, bcm_mac_t *addr)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_l2_bpdu_get(bcm_unit, index, addr);
        if (BCM_SUCCESS(tmp_rv)) {
            return tmp_rv;
        }
        BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
...
bcmx_l2_learn_limit_get(bcmx_l2_learn_limit_t *limit)
{
    ...
    /* This is for a specific unit/port */
    if (bcm_unit >= 0) {
        tmp_rv = bcm_l2_learn_limit_get(bcm_unit, &bcm_l2);
        BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
    } else {
        /* Iterate in all units until first successful find */
        BCMX_UNIT_ITER(bcm_unit, i) {
            tmp_rv = bcm_l2_learn_limit_get(bcm_unit, &bcm_l2);
            BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
            if (BCM_SUCCESS(tmp_rv)) {
...
bcmx_l2_learn_class_get(int lclass, int *lclass_prio, uint32 *flags)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_l2_learn_class_get(bcm_unit, lclass, lclass_prio, flags);
        if (BCM_SUCCESS(tmp_rv)) {
            return tmp_rv;
        }
        BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
 ...
 * Notes:
 *      The logical port (bcm mod/port) is a system-wide information,
...
bcmx_l2_learn_port_class_get(bcmx_lport_t port, int *lclass)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        tmp_rv = bcm_l2_learn_port_class_get(bcm_unit, port, lclass);
        if (BCM_SUCCESS(tmp_rv)) {
            return tmp_rv;
        }
        BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
