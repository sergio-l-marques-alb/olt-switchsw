...
sal_alloc(unsigned int sz, char *s)
{
     ...
     */
#if defined(MIPS64)  /* Sibyte 1125 (NSX) */
    /* align to 2^3 = 8 byte boundary */
    if ((p = memalign(3, (sizeof (block_t) + size_words * 4))) == NULL) {
        return p;
    }
#else
...
#if defined(MIPS64)  /* Sibyte 1125 (NSX) */
    return memalign(3, sz);    /* align to 2^3 = 8 byte boundary */
#else
    return malloc(sz);
#endif
...
sal_alloc_stat(void *param)
{
    ...
    if (param) { /* Show with specific description */
        for (p = head; p != NULL; p = p->next) {
            if (strcmp(p->description, (char *)param) == 0) {
                grand_tot += p->size;
            }
        }
        printf("%s : total size 0x%x (%d) bytes allocated\n", 
               (char *)param, grand_tot, grand_tot);
