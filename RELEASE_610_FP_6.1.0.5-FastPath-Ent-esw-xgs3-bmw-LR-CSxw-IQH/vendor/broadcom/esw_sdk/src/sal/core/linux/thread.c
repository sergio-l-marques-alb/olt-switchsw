/*
 * $Id: thread.c,v 1.1 2011/04/18 17:11:04 mruas Exp $Copyright: Broadcom SDK $Copyright: Copyright 2009 Broadcom Corporation.
...
#else
    SET_USER_SCHED(thread, SAL_TO_POSIX_NICE_PRIO(ctrl->prio));
#endif
    smp_mb();
 ...
 *    Nothing
 * Notes:
 *    Since the actual delay period for the interruptible_sleep_on_timeout
 *    function depends on the Linux system tick (usually 1/100 sec), we 
 *    simply yield the processor until the specified number of usecs have 
 *    passed, whenever the delay period is less than a system tick.
 *    Should only be called from interrupt context with small values.
 */
void
