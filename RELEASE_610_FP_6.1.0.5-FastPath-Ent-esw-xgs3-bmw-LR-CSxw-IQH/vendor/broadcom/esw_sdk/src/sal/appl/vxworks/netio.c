/*
 * $Id: netio.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
int netio_rename(char *old_path, char *new_path)
{
	...
	if (ftpXfer(host, user, pass, "", "RNFR %s", st ? cmd : "", file,
		    &c_fd, NULL) == ERROR) {
	    DPRINT("netio_rename: FTP connection failed to host %s\n", host);
	    goto done;
	}

	if (ftpCommand(c_fd, "RNTO %s", PTR_TO_INT(new_path),
		       0, 0, 0, 0, 0) != FTP_COMPLETE) {
	    DPRINT("netio_rename: FTP completion failed to host %s\n", host);
	    goto done;
	}
	break;
    }
