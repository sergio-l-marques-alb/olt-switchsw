/*
 * $Id: common_rx.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
                              ...
                              bcm_rx_chan_t chan_id)
{
    ...
    if (!soc_feature(unit, soc_feature_cos_rx_dma)) {
        /* not DMA set up */
        return BCM_E_CONFIG;
    } else if (chan_id < 0 || chan_id >= BCM_RX_CHANNELS) {
        /* Verify the chan id */
        return BCM_E_PARAM;
...
rx_cleanup(int unit)
{
    ...
    if (rx_ctl[unit] != NULL) {
        if (SOC_UNIT_VALID(unit)) {
            /* Abort running DVs */
            if (soc_dma_abort(unit) < 0) {
                RX_DEBUG((DK_WARN, "RX: Error aborting DMA\n"));
            }
    ...
    if (!physical) {
        ...
        if (vp) {
            ...
            if (SOC_IS_SC_CQ(unit)){
                int grp;
                if (!_bcm_tr_subport_group_find(unit, vp, &grp)) {
                    if (grp != -1) {  	
    ...
    if (vni) {
        if (SOC_IS_TR_VL(unit)) {
            if (_bcm_vfi_used_get(unit, vni, _bcmVfiTypeMpls)) {
                _BCM_VPN_SET(*vpn, _bcmVfiTypeMpls, vni);
            } else if (_bcm_vfi_used_get(unit, vni, _bcmVfiTypeMim)) {
                _BCM_VPN_SET(*vpn, _bcmVfiTypeMim, vni);
            }
        }
    }
    *vpn = l_vpn;
    ...
    if (SOC_IS_RCPU_UNIT(unit)) {
        ...
        if (!soc_feature(unit, soc_feature_trunk_group_overlay) &&
            ...
            if (soc_feature(unit, soc_feature_trunk_extended)) {
                ...
                tid = -1;
                rv = _bcm_xgs3_trunk_get_port_property(unit, pkt->src_mod,
  	                                               src_port_tgid, &tid);
...
_bcm_common_rx_show(int unit)
{
    ...
    for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
        ...
        if (i < 16) {
            /* legacy support */
            if (!(BCM_RCO_F_COS_ACCEPT(i) & BCM_RCO_F_ALL_COS)) {
                /* Skip unused queues */
                continue;
            }
        }
