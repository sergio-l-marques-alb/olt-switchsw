/*
 * $Id: common_tx.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
_xgs3_tx(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    BCM_PBMP_ITER(tx_pbmp, port)
    {
        ...
        BCM_PBMP_AND(pkt->tx_upbmp, tx_upbmp);
#endif
    ...
    rv = bcm_common_tx_array(unit, packet_pointers_p, pkt_cnt, pkt->call_back,
        cookie);
    ...
    BCM_PBMP_ASSIGN(pkt->tx_l3pbmp, tx_l3pbmp);
             ...
             bcm_pkt_cb_f chain_done_cb, void *cookie, int per_pkt_cb)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        int port_count;
        /* On XGS3 we allow a larger DV chain to allow a single TX
           to all ports */
