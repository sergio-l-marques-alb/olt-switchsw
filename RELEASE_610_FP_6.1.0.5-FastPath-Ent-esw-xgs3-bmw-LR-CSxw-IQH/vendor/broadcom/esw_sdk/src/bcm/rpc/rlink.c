/*
 * $Id: rlink.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
_bcm_rlink_rx_handler(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    bp = buf;
    BCM_PACK_U8(bp, RLINK_MSG_NOTIFY);
    BCM_PACK_U8(bp, RLINK_TYPE_RX);
			    ...
			    bcm_port_info_t *info)
{
    ...
    bp = buf;
    BCM_PACK_U8(bp, RLINK_MSG_NOTIFY);
    BCM_PACK_U8(bp, RLINK_TYPE_LINK);
    BCM_PACK_U32(bp, unit);
                        ...
                        int reason)
{
    ...
    bp = buf;
    BCM_PACK_U8(bp, RLINK_MSG_NOTIFY);
    BCM_PACK_U8(bp, RLINK_TYPE_AUTH);
    BCM_PACK_U32(bp, unit);
    BCM_PACK_U32(bp, port);
...
_bcm_rlink_scan_add(cpudb_key_t cpu, int unit, rlink_type_t type)
{
    ...
    case RLINK_TYPE_L2:
        ...
        break;
    case RLINK_TYPE_RX:
        (void)bcm_rx_register(unit, "rlink",
			...
			rlink_msg_t msg, rlink_type_t type)
{
    ...
    bp = buf;
    BCM_PACK_U8(bp, msg);		/* RLINK_MSG_* */
    BCM_PACK_U8(bp, type);		/* RLINK_TYPE_* */
    BCM_PACK_U32(bp, unit);
...
bcm_client_linkscan_unregister(int unit, bcm_linkscan_handler_t f)
{
    ...
    if (rhand != NULL) {  /* Found an entry */
        if (last) {
            _bcm_rlink_send_control(rhand->cpu, rhand->cunit,
                                    RLINK_MSG_DELETE, RLINK_TYPE_LINK);
        }
        sal_free(rhand);
    }
			      ...
			      void *cookie)
{
    ...
    if (rhand != NULL) {  /* Found an entry to remove */
        if (last) {
            _bcm_rlink_send_control(rhand->cpu, rhand->cunit,
                                    RLINK_MSG_DELETE, RLINK_TYPE_L2);
        }
        sal_free(rhand);
    }
...
#if defined(BROADCOM_DEBUG)
int bcm_linkscan_dump(int unit);
