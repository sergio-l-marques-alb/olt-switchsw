/*
 * $Id: rpc.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
/* server rpc request: allocated from heap */
typedef struct _rpc_sreq_s {
    struct _rpc_sreq_s  *next;
    cpudb_key_t         cpu;
    uint8               *buf;
    void                *cookie;
...
static _rpc_sreq_t  *_rpc_sreq;
static _rpc_sreq_t  *_rpc_sreq_tail;
                ...
                void **cookie)
{
    ...
    atp_tx_data_free(buf);
#ifdef LVL7_FIXUP
  /* ALT - This fixup adds a timeout for RPC calls. The reference code assumes that
  **       if we loose the remote unit then detach logic will clear the semaphore.
  **       In our environment the task responsible for invoking the detach logic 
  **       may be the same task that is waiting for the RPC semaphore. This causes
  **       a deadlock. 
  **
  ...
  **
  **       I don't expect Brodcom to integrate this change in the reference code.
  */
    {
      int atp_timeout;
      int atp_retry_count;
      ...
      rpc_timeout = 2 * (atp_timeout * atp_retry_count);


      if (rv == BCM_E_NONE)
      {
        /* wait for reply */
        rv1 = sal_sem_take(req.sem, rpc_timeout);
        if (rv1 != 0)
        {
    ...
    /* wait for reply */
    sal_sem_take(req.sem, sal_sem_FOREVER);
    sal_sem_destroy(req.sem);
#endif
