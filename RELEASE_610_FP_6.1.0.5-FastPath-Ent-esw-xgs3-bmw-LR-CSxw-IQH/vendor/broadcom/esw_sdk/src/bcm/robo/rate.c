/*
 * $Id: rate.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
 ...
 */
static int bcm_robo_storm_en = FALSE;
...
int bcm_robo_rate_set(int unit, int pps, int flags)
{
    ...
    if (bcm_robo_storm_en == FALSE){
        bcm_robo_storm_en = TRUE;
        PBMP_ITER(tr_pbmp, port) {
            /* set the storm control rate_type & rate_limit also */
            BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->storm_control_enable_set)
                            (unit, port, bcm_robo_storm_en));
        }

    }

    /* set the storm control rate_type & rate_limit also */
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->storm_control_set)
                    (unit, tr_pbmp,
                    rate_type, rate_limit, burst_size));
...
int bcm_robo_rate_get(int unit, int *pps, int *flags)
{
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->storm_control_get)
    ...
    if (bcm_robo_storm_en == FALSE){
        bcm_robo_storm_en = TRUE;
         PBMP_E_ITER(unit, tmp_port) {
            /* set the storm control rate_type & rate_limit also */
            BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->storm_control_enable_set)
                            (unit, tmp_port, bcm_robo_storm_en));
        }
