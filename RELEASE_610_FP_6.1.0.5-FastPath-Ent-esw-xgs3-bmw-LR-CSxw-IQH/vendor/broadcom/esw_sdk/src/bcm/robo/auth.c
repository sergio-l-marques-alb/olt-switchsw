/*
 * $Id: auth.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#define AUTH_SUPPORT(unit)    \
        if (!SOC_PBMP_WORD_GET(auth_pbmp, 0))    \
...
bcm_robo_auth_init(int unit)
{
    ...
    for (i=0; i<SOC_PBMP_WORD_MAX; i++){
        SOC_PBMP_WORD_SET(auth_pbmp, i, value[i]);
    }
    AUTH_SUPPORT(unit);
    /*
    ...
    */
    ...
    BCM_PBMP_AND(bmp_all, auth_pbmp);
    
    BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->security_set
                    (unit, bmp_all, 
...
bcm_robo_auth_detach(int unit)
{
    ...
    */
    ...
    if (cb_cntl[unit].registered) {
        BCM_IF_ERROR_RETURN(
            bcm_linkscan_unregister(unit, _robo_auth_linkscan_cb)); 
...
bcm_robo_auth_mode_set(int unit, int port, uint32 mode)
{
    ...
    if (mode & BCM_AUTH_MODE_UNCONTROLLED) {
        ...
        BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->security_set
                        (unit, t_pbm,
                        DRV_SECURITY_PORT_UNCONTROLLED, 0));
        ...
        mask |= (mode & BCM_AUTH_BLOCK_INOUT) ?
                DRV_SECURITY_BLOCK_INOUT : DRV_SECURITY_BLOCK_IN;
        BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->security_set
                        (unit, t_pbm,
                        DRV_SECURITY_PORT_UNAUTHENTICATE, mask));
        ...
        mask |= (mode & BCM_AUTH_SEC_NONE) ? 
                ...
                (mode & BCM_AUTH_SEC_SA_MATCH) ? 
                        DRV_SECURITY_VIOLATION_SA_MATCH : 0;
        ...
        BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->security_set
                        (unit, t_pbm,
                        DRV_SECURITY_PORT_AUTHENTICATED, mask));
    } else {
        return BCM_E_PARAM;
    }
...
bcm_robo_auth_egress_get(int unit, int port, int *enable)
{
    ...
    BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->security_egress_get
                    (unit, port, (int *) &status));
    *enable = status;
...
bcm_robo_auth_mac_add(int unit, int port, bcm_mac_t mac)
{
   ...
   /* get the number of supported per port Max sec mac number in this unit */
    ...
    /* Get the first unused Sec Mac index */
    if (robo_auth_info[unit].portAuth[port].secMacNum == max_sec_mac){
        return BCM_E_FULL;
    }
    ...
    BCM_IF_ERROR_RETURN(DRV_SERVICES(unit)->mac_set
                    (unit, t_pbm, 
                    DRV_MAC_SECURITY_ADD, mac, 0));
    
    /* Update the S/W info */
    BCM_IF_ERROR_RETURN(_robo_auth_maclist_insert
                    (&robo_auth_info[unit].portAuth[port].macList, mac, &entry));
