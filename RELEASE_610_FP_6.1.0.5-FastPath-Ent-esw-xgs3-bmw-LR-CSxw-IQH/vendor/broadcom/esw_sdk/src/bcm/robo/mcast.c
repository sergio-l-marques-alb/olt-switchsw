/*
 * $Id: mcast.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
bcm_robo_mcast_init(int unit)
{
   ...
   /* get the number of supported Mcast gourps in this unit */
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mcast_init)(unit));
...
int bcm_robo_mcast_addr_add(int unit, bcm_mcast_addr_t *mcaddr)
{
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mcast_to_marl)
    ...
    if (SOC_MCAST_ADD_ALL_ROUTER_PORTS(unit)) {
        rv = bcm_mcast_port_get(unit, (uint8 *)_soc_mac_all_routers, mcaddr->vid,
                             &rtaddr);
        if (rv >= 0) {
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mcast_bmp_set)
            (unit, (uint32 *)&marl_entry, 
            t_pbm,
            DRV_MCAST_INDEX_ADD));
                  ...
                  bcm_mcast_addr_t *mcaddr)
{
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mcast_from_marl)
        (unit, (uint32 *)mcaddr, (uint32 *)&marl_result));
    
    BCM_PBMP_CLEAR(mcaddr->pbmp);
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mcast_bmp_get)
                    (unit, (uint32 *)&marl_result, &mcaddr->pbmp));
              ...
              bcm_pbmp_t *allRouterBmp)
{
    ...
    /*
      * For Robo, do we need create the All router mac address (01:00:5e:00:00:02).
    ...
    rv = bcm_mcast_port_get(unit, (uint8 *)_soc_mac_all_routers, vlanId,
                                &newmc);
    ...
    rv = bcm_mcast_port_add(unit, &newmc);

    if (rv == BCM_E_NOT_FOUND) {
        BCM_IF_ERROR_RETURN(bcm_mcast_addr_add(unit, &newmc));
    ...
    if (mcaddr != NULL) {
    BCM_IF_ERROR_RETURN(bcm_mcast_port_get(unit, mcMacAddr, vlanId, mcaddr));
    }
    return rv;
}             
               ...
               int srcPort)
{
    ...
    rv = bcm_mcast_port_remove(unit, &mcaddr);
    if (rv < 0) {
        return rv;
    } else {
        rv = bcm_mcast_port_get(unit, mcMacAddr, vlanId, &mcaddr);
    }
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (SOC_MCAST_ADD_ALL_ROUTER_PORTS(unit)) {
            rv = bcm_mcast_port_get(unit, (uint8 *)_soc_mac_all_routers, vlanId,
                                    &rtaddr);
            if (rv < 0) {
                bcm_mcast_addr_t_init(&rtaddr, (uint8 *)_soc_mac_all_routers, vlanId);
            }
            BCM_PBMP_REMOVE(mcaddr.pbmp, rtaddr.pbmp);
        }
        if (BCM_PBMP_IS_NULL(mcaddr.pbmp)) {
            BCM_IF_ERROR_RETURN(bcm_mcast_addr_remove(unit, mcMacAddr, vlanId));
...
bcm_robo_mcast_addr_remove_w_l2mcindex(int unit, bcm_mcast_addr_t *mcaddr)
{
    ...
    rv = bcm_l2_addr_get(unit, mcaddr->mac, mcaddr->vid, &l2addr);
    if (rv < 0) {
        return rv;
    }
    ...
    rv = bcm_mcast_addr_remove(unit, mcaddr->mac, mcaddr->vid);
...
bcm_robo_mcast_addr_add_w_l2mcindex(int unit, bcm_mcast_addr_t *mcaddr)
{
    ...
    soc_cm_debug(DK_L2TABLE, 
        "BCM API : bcm_robo_mcast_addr_add_w_l2mcindex().. \n");
    ...
    if (rv >= 0) {
        ...
        L2MC_USED_CLR(unit, l2addr.l2mc_index);
    }
    
    rv = bcm_mcast_addr_add(unit, mcaddr);
