/*
 * $Id: trunk.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
static int fe_trunk_patch_lock_init = FALSE;
...
#if defined(BCM_53242_A0) /* BCM_53242_A0 */
#define TRUNK_PSC_CHECK(unit, psc) \
    if ((psc) < BCM_TRUNK_PSC_SRCMAC ) { return BCM_E_BADID; } \
    else if ((psc) > BCM_TRUNK_PSC_SRCDSTIP) {return BCM_E_UNAVAIL; }
#else /* !BCM_53242_A0 */
#define TRUNK_PSC_CHECK(unit, psc) \
    if ((psc) < BCM_TRUNK_PSC_SRCMAC ) { return BCM_E_BADID; } \
    else if ((psc) > BCM_TRUNK_PSC_SRCDSTMAC) {return BCM_E_UNAVAIL; }
#endif /* !BCM_53242_A0 */
    ...
    if (tid != -1) {
        ...
        if (info->linkstatus) { /* linkup event */
            ...
            if (has_link) {
                ...
                rv = (DRV_SERVICES(unit)->trunk_set)
                        (unit, tid, trunk_pbmp, flag, 0);
                if (rv < 0) {
                    soc_cm_print("trunk_set rv = %d\n", rv);
                }
            ...
            rv = (DRV_SERVICES(unit)->trunk_set)
                ...
                if (rv != BCM_E_NONE) {
                    soc_cm_print("trunk_set failed rv=%d\n", rv);
                }            
            }
                            ...
                            mac_addr_t mac, trunk_private_t *t_info)
{
    ...
    if (SOC_IS_ROBO5324(unit)) {
        ...
        flag = DRV_TRUNK_FLAG_BITMAP;
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_get)
                    (unit, tid, &trunk_pbmp, flag, &hash_op));
    }
    ...
    if (t_info->dlf_port_used >= 0) {
        BCM_PBMP_PORT_ADD(mc_addr.pbmp, t_info->dlf_port_used);
    }
    soc_cm_debug(DK_VERBOSE, 
...
_robo_trunk_check_vlan(int unit, bcm_pbmp_t t_pbmp_new, bcm_pbmp_t t_pbmp_old)
{
    ...
    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->vlan_mode_get)
    ...
    if (!t_info->in_use) {
        ...
        if (SOC_IS_ROBO5324(unit)) {
            ...
            if (private_info_5324[unit][tid].in_use) {
                 ...
                 */
                flag = DRV_TRUNK_FLAG_BITMAP;
                if ((rv = (DRV_SERVICES(unit)->trunk_get)
                     ...
                     trunk_private_t *t_info)
{
    ...
    case BCM_TRUNK_PSC_SRCMAC:
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_hash_field_add)
                            (unit, DRV_TRUNK_HASH_FIELD_MACSA));
         ...
         *      Robo Chip.
         */
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_hash_field_remove)
                        (unit, 
    ...
    case BCM_TRUNK_PSC_DSTMAC:
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_hash_field_add)
                            (unit, DRV_TRUNK_HASH_FIELD_MACDA));
        /* remove the undefined psc 
         *  - below psc may not be suitable for all Robo chips but 
         *      implemented for general purpose on supporting every 
         *      Robo Chip.
    ...
    case BCM_TRUNK_PSC_SRCIP:
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_hash_field_add)
                            (unit, DRV_TRUNK_HASH_FIELD_IP_MACSA));
        /* remove the undefined psc 
         *  - below psc may not be suitable for all Robo chips but 
         *      implemented for general purpose on supporting every 
         *      Robo Chip.
    ...
    case BCM_TRUNK_PSC_DSTIP:
        BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_hash_field_add)
                            (unit, DRV_TRUNK_HASH_FIELD_IP_MACDA));
        /* remove the undefined psc 
         *  - below psc may not be suitable for all Robo chips but 
         *      implemented for general purpose on supporting every 
         *      Robo Chip.
        ...
        if (SOC_IS_ROBO5324(unit)) {
            ...
            if (hasfe) {            
                ...
                if (sync_to_hw) {   /* atleast one link is up */
                    ...
                    BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->trunk_set)
                            (unit, tid, trunk_pbmp, flag, 0));
    ...
    if (BCM_PBMP_NOT_NULL(new_trunk_pbmp)) {
        ...
        if (add_info->tm[dlf_index] == my_modid) {
            t_info->dlf_port_used = add_info->tp[dlf_index];
        } else {
             ...
             bcm_trunk_add_info_t *add_info)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_robo_trunk_gport_resolve(unit, add_info));

    rv = _bcm_robo_trunk_set(unit, tid, add_info, t_info);
    if (rv >= 0) {
...
bcm_robo_trunk_bitmap_expand(int unit, bcm_pbmp_t *pbmp_ptr)
{
    ...
    for (i = ti.trunk_id_min ; i <= ti.trunk_id_max ; i++){
        ...
        PBMP_ITER(t_pbmp, port) {
            if (PBMP_MEMBER(trunk_pbmp, port)) {
