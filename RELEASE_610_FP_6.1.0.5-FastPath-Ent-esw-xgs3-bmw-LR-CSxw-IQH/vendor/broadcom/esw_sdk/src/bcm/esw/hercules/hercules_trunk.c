/*
 * $Id: hercules_trunk.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
 ...
 */
int
bcm_hercules_trunk_set(int unit, bcm_trunk_t tid,
		       ...
		       trunk_private_t *t_info)
{
    ...
    ing_cntlr = SOC_IS_HERCULES1(unit) ? ING_CNTLr : ING_CTRLr;
	...
	add_info->psc = BCM_TRUNK_PSC_DEFAULT;
        ...
        if (SOC_IS_HERCULES15(unit)) {
            add_info->psc |= (BCM_TRUNK_PSC_MACSA | BCM_TRUNK_PSC_MACDA |
                              BCM_TRUNK_PSC_IPMACSA | BCM_TRUNK_PSC_IPMACDA);
        }
#endif /* BCM_HERCULES15_SUPPORT */
    }
    switch (add_info->psc & 0xf) {
...
#ifdef BCM_HERCULES15_SUPPORT
    if (!algo && SOC_IS_HERCULES15(unit)) {
        ...
        if (add_info->psc & BCM_TRUNK_PSC_IPTYPE) {
            soc_reg_field_set(unit, ING_HGTRUNKr, &val, EN_IPTYPEf, 1);
        }
        if (add_info->psc & BCM_TRUNK_PSC_IPVID) {
        ...
        if (add_info->psc & BCM_TRUNK_PSC_L4SS) {
            soc_reg_field_set(unit, ING_HGTRUNKr, &val, EN_L4SSf, 1);
        }
        if (add_info->psc & BCM_TRUNK_PSC_L4DS) {
        ...
        if (add_info->psc & BCM_TRUNK_PSC_TYPE) {
            soc_reg_field_set(unit, ING_HGTRUNKr, &val, EN_TYPEf, 1);
        }
        if (add_info->psc & BCM_TRUNK_PSC_VID) {
    ...
    if (bmap != 0) {
        PBMP_PORT_ITER(unit, port) {
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit,
                                soc_reg_addr(unit, ing_cntlr, port, 0), &val));
            oval = val;
...
bcm_hercules_trunk_mcast_join(int unit, bcm_trunk_t tid, bcm_vlan_t vid,
                              mac_addr_t mac, trunk_private_t *t_info)
{
