/*
 * $Id: tucana_vlan.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
bcm_tucana_vlan_port_get(int unit, bcm_vlan_t vid, pbmp_t *pbmp, pbmp_t *ubmp)
{
    ...
    if (pbmp != NULL) {
	SOC_PBMP_CLEAR(*pbmp);
	SOC_PBMP_WORD_SET(*pbmp, 0, soc_VLAN_TABm_field32_get(unit,
                           &vt, PORT_BITMAP_M0f));
	SOC_PBMP_WORD_SET(*pbmp, 1, soc_VLAN_TABm_field32_get(unit,
                           &vt, PORT_BITMAP_M1f));
    }
