/*
 * $Id: draco_ipmc.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
#define	IPMC_USED_SET(unit, n)                                 \
        {                                                      \
            SHR_BITSET(xgs_ipmc_info[unit].ipmc_allocated, n); \
	    xgs_ipmc_info[unit].ipmc_count++;                  \
        }
#define IPMC_USED_CLR(unit, n) \
        {                                                      \
            SHR_BITCLR(xgs_ipmc_info[unit].ipmc_allocated, n); \
	    xgs_ipmc_info[unit].ipmc_count--;                  \
        }
#define IPMC_USED_ISSET(unit, n) \
        SHR_BITGET(xgs_ipmc_info[unit].ipmc_allocated, n)
#define IPMC_USED_ZERO(unit)                                            \
        {                                                               \
	    sal_memset((char *)(xgs_ipmc_info[unit].ipmc_allocated), 0, \
		SHR_BITALLOCSIZE(xgs_ipmc_info[unit].ipmc_size));       \
	    xgs_ipmc_info[unit].ipmc_count = 0;                         \
        }
#define IPMC_ID(unit, id) \
        if ((id < 0) || (id >= xgs_ipmc_info[unit].ipmc_size)) \
            { return BCM_E_PARAM; }

#define IPMC_INIT(unit) \
        if (!xgs_ipmc_info[unit].ipmc_initialized) { return BCM_E_INIT; }
#define IPMC_INFO(unit)   (&xgs_ipmc_info[unit])
...
static _bcm_xgs_ipmc_t xgs_ipmc_info[BCM_MAX_NUM_UNITS];
...
_xgs_ipmc_create(int unit, int *ipmc_id)
{
    ...
    IPMC_LOCK(unit);
    for (i = 0; i < xgs_ipmc_info[unit].ipmc_size; i++) {
...
{
    int			rv, i;
    _bcm_xgs_ipmc_t	*info = IPMC_INFO(unit);
...
bcm_xgs_ipmc_get(int unit, int index, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (IPMC_USED_ISSET(unit, index)) {
	    ...
	    rv = _xgs_ipmc_info_get(unit,
                      xgs_ipmc_info[unit].ipmc_l3_index[index],
                      ipmc, &ipmc_entry);
...
bcm_xgs_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    xgs_ipmc_info[unit].ipmc_l3_index[ipmc_id] = l3cfg.l3c_hw_index;
...
bcm_xgs_ipmc_delete(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (!(ipmc->flags & BCM_IPMC_KEEP_ENTRY)) {
        ...
        xgs_ipmc_info[unit].ipmc_l3_index[ipmc_id] = -1;
        if (repl) {
                 ...
                 void *user_data)
{
    ...
    _bcm_xgs_ipmc_t *info;     /* Sw image pointer.           */
...
int
bcm_xgs_ipmc_egress_port_get(int unit, bcm_port_t port, mac_addr_t mac,
