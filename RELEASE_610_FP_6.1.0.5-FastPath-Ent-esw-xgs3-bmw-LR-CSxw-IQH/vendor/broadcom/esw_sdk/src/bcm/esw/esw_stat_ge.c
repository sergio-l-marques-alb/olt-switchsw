/*
 * $Id: esw_stat_ge.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
                 ...
                 int incl_non_ge_stat)
{
	...
	REG_ADD(unit, port, GRBYTr, count);	/* bytes rcvd */
        if (SOC_REG_IS_VALID(unit, RRBYTr)) {
...
#ifdef LVL7_FIXUP
                /*  EGRDROPPKTCOUNTr appears to be transient, and not working properly */
    	        REG_ADD(unit, port, HOLDr, count);	/* HOLD */
#endif
                _bcm_stat_counter_extra_get(unit, EGRDROPPKTCOUNTr, port,
                                            &count1);
...
/* ipOutNoRoutes */
	...
	REG_ADD(unit, port, GRPKTr, count);
	REG_ADD(unit, port, GTPKTr, count);
        if (SOC_REG_IS_VALID(unit, RRPKTr)) {
	...
	break;
#ifdef LVL7_FIXUP
    case snmpEtherStatsInOversizePkts:
        if (COUNT_OVR_ERRORS(unit)) {
            if (soc_feature(unit, soc_feature_stat_jumbo_adj)) {
                REG_ADD(unit, port, GROVRr, count);
            } else {
        ...
        break;
    case snmpEtherStatsOutOversizePkts:
        if (COUNT_OVR_ERRORS(unit)) {
            if (soc_feature(unit, soc_feature_stat_jumbo_adj)) {
                REG_ADD(unit, port, GTOVRr, count);
            } else {
