/*
 * $Id: esw_diffserv.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
                               ...
                               void *user_data) 
{
    ...
    for (dpid_idx = dpid_min; dpid_idx< dpid_max; dpid_idx++) {
        ...
        for (cfid_idx = 0; cfid_idx < CFID_MAX_COUNT; cfid_idx++) {
            cf_ptr = _ds_classifier_find(unit, dp_ptr, cfid_idx);
            /* Skip unused classifiers. */ 
            if (cf_ptr == NULL) {
                continue;
            }
            cf_info = *cf_ptr;
        ...
        rv = (*cb)(unit, dpid_idx, cfid_idx, &cf_info.cfe_clfr, 
