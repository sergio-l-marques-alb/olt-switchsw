/*
 * $Id: firebolt_vlan.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
_bcm_fb2_outer_tpid_entry_delete(int unit, int index) 
{
    ...
    if ((0 == OUTER_TPID_REF_COUNT(unit, index)) && 
        (SOC_IS_TRX(unit) && (0 == SOC_IS_TRIUMPH2(unit)) &&
         (0 == SOC_IS_APOLLO(unit)) && (0 == SOC_IS_ENDURO(unit)) && 
         (0 == SOC_IS_VALKYRIE2(unit)))) {
...
int
_bcm_fb2_port_evc_tpid_index_set(int unit, bcm_port_t port, uint16 tpid)
{
...
#endif /* BCM_TRIUMPH_SUPPORT */
#if defined(BCM_TRX_SUPPORT)
        if (SOC_IS_TRX(unit) && (0 == SOC_IS_TRIUMPH2(unit) &&
            (0 == SOC_IS_APOLLO(unit)) && (0 == SOC_IS_ENDURO(unit)) && 
            (0 == SOC_IS_VALKYRIE2(unit)))) {
...
_bcm_fb2_outer_tpid_tab_ref_count_add(int unit, int index, int count)
{
    ...
    OUTER_TPID_ENTRY(unit, index) += count; 
...
_vlan_profile_read(int unit, int index, _vlan_profile_t *entry)
{
    ...
    rv = _bcm_fb2_outer_tpid_entry_get(unit, &entry->outer_tpid, value); 
    BCM_IF_ERROR_RETURN(rv);
 ...
 */
static int
_bcm_xgs3_vlan_table_port_add(int unit, bcm_vlan_t vid,
                              pbmp_t pbmp, pbmp_t ubmp, int table)
{
    vlan_tab_entry_t            vt;
    int                         rv;
    bcm_pbmp_t                  cur_pbmp;
...
static int
_bcm_xgs3_vlan_port_get(int unit, bcm_vlan_t vid,
                         pbmp_t *pbmp, pbmp_t *ubmp, uint32 table)
{
    vlan_tab_entry_t            vt;
    ...
    for (index = soc_mem_index_min(unit, table);
         ...
         if (soc_mem_field32_get(unit, table, vlan_entry, VALIDf)) {
             ...
             if (soc_mem_field_valid(unit, table, VLAN_PROFILE_PTRf)) {
                 ...
                 tpid_index = soc_mem_field32_get(unit, VLAN_PROFILE_TABm,
                                    &VLAN_PROFILE_ENTRY(unit, profile_ptr), 
                                    OUTER_TPID_INDEXf);
                 OUTER_TPID_REF_COUNT(unit, tpid_index)++;
             }  
...
bcm_xgs3_vlan_create(int unit, bcm_vlan_t vid)
{
    ...
    if (soc_mem_field_valid(unit, L3_IIFm, IPMC_L3_IIFf)) {
        iif_entry_t iif_entry;
        sal_memcpy(&iif_entry,  soc_mem_entry_null(unit, L3_IIFm), 
                   sizeof(iif_entry_t));
        soc_mem_field32_set(unit, L3_IIFm, (uint32 *)&iif_entry, 
                            IPMC_L3_IIFf, vid);
        BCM_IF_ERROR_RETURN
            (soc_mem_write(unit, L3_IIFm, MEM_BLOCK_ALL, vid, 
                           (uint32 *)&iif_entry));               
    }
#endif
...
bcm_xgs3_vlan_port_add(int unit, bcm_vlan_t vid, pbmp_t pbmp, pbmp_t ubmp)
{
    if (SOC_IS_FBX(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_vlan_table_port_add(unit, vid, pbmp, ubmp, EGR_VLANm));
    }
    return _bcm_xgs3_vlan_table_port_add(unit, vid, pbmp, ubmp, VLAN_TABm);
}
...
bcm_xgs3_vlan_port_get(int unit, bcm_vlan_t vid, pbmp_t *pbmp, pbmp_t *ubmp)
{
    if (SOC_IS_FBX(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_vlan_port_get(unit, vid, pbmp, ubmp, EGR_VLANm));
    }
    return (_bcm_xgs3_vlan_port_get(unit, vid, pbmp, ubmp, VLAN_TABm));
}
...
bcm_xgs3_vlan_reload(int unit, vbmp_t *bmp, int *count)
{
 ...
 || defined(BCM_RAVEN_SUPPORT)
        ...
        if (SOC_IS_FIREBOLT2(unit) || SOC_IS_SC_CQ(unit)) {
            return _bcm_xgs3_vlan_table_reload(unit, bmp, count, VLAN_TABm);
        } else {
            return _bcm_xgs3_vlan_table_reload(unit, bmp, count, EGR_VLANm);
        }
    }
    return _bcm_xgs3_vlan_table_reload(unit, bmp, count, VLAN_TABm);
}
                                ...
                                bcm_vlan_control_vlan_t *control)
{
    ...
    if ((SOC_MEM_FIELD_VALID(unit, VLAN_TABm, L3_IIFf)) || 
        ...
        rv =  _bcm_tr_l3_intf_class_set(unit, vid, 
                                        control->if_class);
        BCM_IF_ERROR_RETURN(rv);
    }
    if (SOC_MEM_FIELD_VALID(unit, L3_IIFm, ALLOW_GLOBAL_ROUTEf)) {
...
/*
 * Function :
 *    _vlan_block_bitmap_substract
 * 
 * Purpose  :
 *    Substract selected mask from all other masks in the array. 
 *
 * Parameters:
 *     unit    - (IN)     BCM device number.
 *     array   - (IN/OUT) Bitmaps array 
 *     arr_sz  - (IN)     Array size. 
 *     mask    - (IN)     Mask to substract.
 *
 * Return :
 *    BCM_E_XXX
 */
int
_vlan_block_bitmap_substract(int unit, bcm_pbmp_t *array, 
...
{      
    ...
    for (idx = 0; idx < arr_size; idx++) {
        ...
        BCM_PBMP_REMOVE(array[idx], mask);
    }
    return (BCM_E_NONE);
}
...
_bcm_trx_vlan_block_set (int unit, bcm_vlan_t vid, bcm_vlan_block_t *block)
{
    ...
    /* Substruct the shortest mask from other array masks. */
    rv = _vlan_block_bitmap_substract(unit, array,
                                      _BCM_VLAN_BLOCK_LOOKUP_TYPES,
                                      hw_block.first_mask);
    BCM_IF_ERROR_RETURN(rv);
