/*
 * $Id: firebolt_ipmc.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
typedef struct _bcm_xgs3_ipmc_s
{
    ...
    int *ipmc_l3_index;
    int *ipmc_ip6;
    int ipmc_valid_as_hit; /* L3_IPMC "valid" field is used as hit-bit */
} _bcm_xgs3_ipmc_t;

#define IPMC_USED_SET(unit, n)                                  \
        {                                                       \
            SHR_BITSET(xgs3_ipmc_info[unit].ipmc_allocated, n); \
            xgs3_ipmc_info[unit].ipmc_count++;                  \
        }
#define IPMC_USED_CLR(unit, n)                                  \
        {                                                       \
            SHR_BITCLR(xgs3_ipmc_info[unit].ipmc_allocated, n); \
            xgs3_ipmc_info[unit].ipmc_count--;                  \
        }
#define IPMC_USED_ISSET(unit, n) \
        SHR_BITGET(xgs3_ipmc_info[unit].ipmc_allocated, n)
#define IPMC_USED_ZERO(unit)                                             \
        {                                                                \
            sal_memset((char *)(xgs3_ipmc_info[unit].ipmc_allocated), 0, \
                SHR_BITALLOCSIZE(xgs3_ipmc_info[unit].ipmc_size));       \
            xgs3_ipmc_info[unit].ipmc_count = 0;                         \
        }
#define IPMC_ID(unit, id) \
        if ((id < 0) || (id >= xgs3_ipmc_info[unit].ipmc_size)) \
            { return BCM_E_PARAM; }

#define IPMC_INIT(unit) \
        if (!xgs3_ipmc_info[unit].ipmc_initialized) { return BCM_E_INIT; }
#define IPMC_INFO(unit)   (&xgs3_ipmc_info[unit])
#define IPMC_GROUP_NUM(unit)   (xgs3_ipmc_info[unit].ipmc_size)
...
#define IPMC_VALID_AS_HIT(unit) (xgs3_ipmc_info[unit].ipmc_valid_as_hit)

static _bcm_xgs3_ipmc_t xgs3_ipmc_info[BCM_MAX_NUM_UNITS];
...
_rep_regs_t en_rep_regs[] = { {INVALIDf,       INVALIDf,    INVALIDm},
                              ...
                              {PORT23_1STPTRf,  PORT23_LASTf, MMU_IPMC_GROUP_TBL3m},                         
                              {PORT24_1STPTRf,  PORT24_LASTf, MMU_IPMC_GROUP_TBL3m},
                              {PORT25_1STPTRf,  PORT23_LASTf, MMU_IPMC_GROUP_TBL3m},
                              {PORT26_1STPTRf,  PORT24_LASTf, MMU_IPMC_GROUP_TBL3m},
			      {PORT27_1STPTRf,  PORT24_LASTf, MMU_IPMC_GROUP_TBL3m},
                              {PORT28_1STPTRf,  PORT24_LASTf, MMU_IPMC_GROUP_TBL3m},
                              {PORT29_1STPTRf,  PORT24_LASTf, MMU_IPMC_GROUP_TBL3m},
};
#endif
 ...
 */
STATIC int
_bcm_fb_ipmc_repl_reload(int unit)
{
    ...
    PBMP_ITER(PBMP_PORT_ALL(unit), port) {
        ...
        alloc_size = sizeof(int32) * IPMC_GROUP_NUM(unit);
...
_bcm_er_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (BCM_SUCCESS(rv)) {
        xgs3_ipmc_info[unit].ipmc_ip6[ipmc->ipmc_index] =
            (ipmc->flags & BCM_IPMC_IP6) ? 1: 0;

        xgs3_ipmc_info[unit].ipmc_l3_index[ipmc->ipmc_index] = ipmc->ipmc_index;
    }
 ...
 */
STATIC int
_bcm_er_ipmc_del(int unit, bcm_ipmc_addr_t *ipmc)
{
    int rv, hw_index;
...
_bcm_er_ipmc_replace(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (ipmc->flags & BCM_IPMC_IP6) {
        ...
        soc_l3x_lock(unit);
        WRITE_IPMC_GROUP_V6m(unit, MEM_BLOCK_ALL, hw_index, &ipmc_entry);
        soc_l3x_unlock(unit);
        ...
        soc_l3x_lock(unit);
        WRITE_IPMC_GROUP_V4m(unit, MEM_BLOCK_ALL, hw_index, &ipmc_entry);
        soc_l3x_unlock(unit);
    }
...
STATIC int
_xgs3_ipmc_info_get(int unit, int hw_index, int index,
                    bcm_ipmc_addr_t *ipmc, ipmc_entry_t *entry)
{
    ...
    if (soc_feature(unit, soc_feature_trunk_group_overlay)) {
        ...
        if (is_trunk) {
            if ((port_tgid == chk_val) && (mod == SOC_MODID_MAX(unit))) {
                no_src_check = TRUE;
            } else {
                mod = 0;
    ...
    if (ipmc->v) {
        if (SOC_IS_FBX(unit)) {
            ...
            if (index >= 0) {
                if (xgs3_ipmc_info[unit].ipmc_ip6[index]) {
            ...
            rv = bcm_xgs3_l3_get_by_index(unit, hw_index, &l3cfg);
                ...
                if (xgs3_ipmc_info[unit].ipmc_ip6[hw_index]) {
                    ipmc->flags |= BCM_IPMC_IP6;
                } else {
                    ipmc->flags &= ~BCM_IPMC_IP6;
                }
        ...
        if (SOC_IS_FBX(unit)) {
#if defined(BCM_FIREBOLT_SUPPORT)
            if (((index >= 0) && xgs3_ipmc_info[unit].ipmc_ip6[index]) ||
 ...
 */
STATIC int
_er_ipmc_min_mtu_get(int unit, pbmp_t pbmp, uint32 *min_mtu)
{
    bcm_port_t          port;
    uint32              mtu_port;
...
bcm_er_ipmc_port_mtu_update(int unit, bcm_port_t port)
{
    ...
    while(todo_count > 0) {
        ...
        for (idx = 0; idx < (end_dma_idx - start_dma_idx); idx++) {
            ...
            rv = _er_ipmc_min_mtu_get(unit, l3_pbmp, &new_min_mtu);
            if (rv < 0) {
                break;
            }
...
_xgs3_ipmc_write(int unit, int ipmc_id, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (soc_mem_field_length(unit, L3_IPMCm, MIN_MTUf)) {
        uint32  min_mtu_l3;
        BCM_IF_ERROR_RETURN(
            _er_ipmc_min_mtu_get(unit, ipmc->l3_pbmp, &min_mtu_l3));
...
{
    _bcm_xgs3_ipmc_t  *info = IPMC_INFO(unit);
    int i;
    ...
    info->ipmc_allocated =
        sal_alloc(SHR_BITALLOCSIZE(IPMC_GROUP_NUM(unit)), "IPMC-alloc");
    if (info->ipmc_allocated == NULL) {
        return BCM_E_MEMORY;
    }

    info->ipmc_l3_index =
        sal_alloc(IPMC_GROUP_NUM(unit) * sizeof(int), "IPMC-l3");
    ...
    info->ipmc_ip6 =
        sal_alloc(IPMC_GROUP_NUM(unit) * sizeof(int), "IPMC-v6");
...
bcm_fb_er_ipmc_get(int unit, int index, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (IPMC_USED_ISSET(unit, index)) {
        ...
        if (rv >= 0) {
            rv = _xgs3_ipmc_info_get(unit, xgs3_ipmc_info[unit].ipmc_l3_index[index],
...
bcm_fb_er_ipmc_lookup(int unit, int *index, bcm_ipmc_addr_t *ipmc)
{
    ...
    ipmc_id = SOC_IS_FBX(unit) ? l3cfg.l3c_hw_index : ipmc->ipmc_index;
    BCM_IF_ERROR_RETURN
        (_xgs3_ipmc_info_get(unit, ipmc_id, -1, ipmc, &ipmc_entry));
...
_bcm_fb_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        xgs3_ipmc_info[unit].ipmc_l3_index[ipmc->ipmc_index] =
            l3cfg.l3c_hw_index;
    }
    return (rv);
}
...
int
_bcm_fb_ipmc_del(int unit, bcm_ipmc_addr_t *ipmc)
{
    _bcm_l3_cfg_t       l3cfg; /* L3 ipmc entry.           */
    int                 rv;    /* Operation return status. */
...
bcm_fb_er_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if(ipmc->flags & BCM_IPMC_USE_IPMC_INDEX) {
        if (_BCM_MULTICAST_IS_SET(orig_data.ipmc_index)) {
            ...
            BCM_IF_ERROR_RETURN
                (_bcm_xgs3_multicast_ipmc_read(unit,  ipmc->ipmc_index, 
            ...
            if (!new_entry) {
                ...
                rv = bcm_fb_er_ipmc_get(unit, ipmc->ipmc_index, &orig_idx_data);
    ...
    if (BCM_FAILURE(rv)) {
        if (!new_entry) {
            if (SOC_IS_FBX(unit)) {
#if defined(BCM_FIREBOLT_SUPPORT)
                _bcm_fb_ipmc_del(unit, ipmc);
#endif /* BCM_FIREBOLT_SUPPORT */
            } else if (SOC_IS_EASYRIDER(unit)) {
#if defined(BCM_EASYRIDER_SUPPORT)
                _bcm_er_ipmc_del(unit, ipmc);
#endif /* BCM_EASYRIDER_SUPPORT */
            }
        }
...
bcm_fb_er_ipmc_delete(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (!(ipmc->flags & BCM_IPMC_KEEP_ENTRY)) {
        ...
        if (SOC_IS_FBX(unit)) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_er_ipmc_del(unit, ipmc));
#endif /* BCM_EASYRIDER_SUPPORT */
        }
        xgs3_ipmc_info[unit].ipmc_l3_index[ipmc_id] = -1;
        if (repl) {
    ...
    for (i = 0; i < IPMC_GROUP_NUM(unit); i++) {
        ...
        if (l3index != -1) {
            if (SOC_IS_FBX(unit)) {
                ...
                BCM_IF_ERROR_RETURN
                    (bcm_xgs3_l3_del_by_index(unit, l3index, flags));
#endif /* BCM_FIREBOLT_SUPPORT */
#if defined(BCM_EASYRIDER_SUPPORT)
            } else if (SOC_IS_EASYRIDER(unit)) {
                BCM_IF_ERROR_RETURN
                (_bcm_er_ipmc_del_by_index(unit, l3index, info->ipmc_ip6[i]));
#endif /* BCM_EASYRIDER_SUPPORT */
            }
            info->ipmc_l3_index[i] = -1;
        }
                 ...
                 void *user_data)
{
    ...
    for (idx = 0; idx < IPMC_GROUP_NUM(unit); idx++) {
        if (IPMC_USED_ISSET(unit, idx)) {
            ...
            rv = bcm_fb_er_ipmc_get(unit, idx, &entry);
            if (BCM_FAILURE(rv)) {
                break;
            }
            ...
            if (entry.flags & BCM_IPMC_HIT) {
                ...
                rv = bcm_fb_er_ipmc_delete(unit, &entry);
                if (BCM_FAILURE(rv)) {
                    break;
                }
...
int
bcm_fb_er_ipmc_egress_port_get(int unit, bcm_port_t port, mac_addr_t mac,
...
#if defined(BCM_BRADLEY_SUPPORT)
    if (soc_feature(unit, soc_feature_ipmc_repl_penultimate)) {
        return bcm_fb_ipmc_egress_intf_set(unit, ipmc_id, port, 0, NULL, 1);
    }
#endif /* BCM_BRADLEY_SUPPORT */
...
_bcm_xgs3_ipmc_reinit(int unit)
{
    ...
    if (SOC_IS_FB_FX_HX(unit)) {
        ...
        for (i = l3_min; i <= l3_max; i++) {
             ...
             info->ipmc_l3_index[ipmc_ptr] = i;
             info->ipmc_ip6[ipmc_ptr] = 0;
        }
        soc_cm_sfree(unit, ipmc_table);
        ...
        if ((rv = soc_mem_read_range(unit, L3_ENTRY_IPV6_MULTICASTm, MEM_BLOCK_ANY,
         ...
         */
        rv = _bcm_fb_ipmc_repl_reload(unit);
    }
...
_bcm_xgs3_ipmc_sw_dump(int unit)
{
    ...
    if (ipmc_info->ipmc_allocated != NULL) {
    ...
    if (ipmc_info->ipmc_l3_index != NULL) {
        idx = ipmc_info->ipmc_l3_index;
        for (i = 0, j = 0; i < IPMC_GROUP_NUM(unit); i++) {
            /* If zero, skip print */
            if (idx[i] < 0) {
                continue;
    ...
    if (ipmc_info->ipmc_ip6 != NULL) {
        idx = ipmc_info->ipmc_ip6;
        for (i = 0, j = 0; i < IPMC_GROUP_NUM(unit); i++) {
            /* If zero, skip print */
            if ((idx[i] == 0) || (idx[i] == -1)) {
                continue;
