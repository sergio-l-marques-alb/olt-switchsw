/*
 * $Id: firebolt_mcast.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
int
bcm_xgs3_mcast_addr_remove(int unit, mac_addr_t mac, bcm_vlan_t vid)
{
    int                 rv;
    bcm_l2_addr_t       l2addr;
...
bcm_xgs3_mcast_init(int unit)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        index_max = soc_mem_index_max(unit, L2MC_MCMEM(unit));
