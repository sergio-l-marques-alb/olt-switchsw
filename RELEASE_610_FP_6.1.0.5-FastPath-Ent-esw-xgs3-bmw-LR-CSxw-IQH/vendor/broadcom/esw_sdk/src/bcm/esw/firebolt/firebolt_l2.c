/*
 * $Id: firebolt_l2.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
int
bcm_fb_l2_addr_get(int unit, mac_addr_t mac, bcm_vlan_t vid,
                      bcm_l2_addr_t *l2addr)
{
 ...
 */
STATIC int 
_bcm_fb_repl_reg_config(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
...
_bcm_fb_l2_addr_replace_by_vlan_dest(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
    ...
    if (0 == (flags & BCM_L2_REPLACE_DELETE)) {
        BCM_IF_ERROR_RETURN(
            _bcm_fb_repl_reg_config(unit, flags, rep_st));
    }
    BCM_IF_ERROR_RETURN(
