/*
 * $Id: triumph2_multicast.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
 ...
 */
int
_bcm_tr2_multicast_ipmc_write(int unit, int ipmc_id, bcm_pbmp_t l2_pbmp,
 ...
 */
int
_bcm_tr2_multicast_ipmc_read(int unit, int ipmc_id, bcm_pbmp_t *l2_pbmp,
                             bcm_pbmp_t *l3_pbmp)
{
    uint32 entry[SOC_MAX_MEM_FIELD_WORDS];  /* hw entry bufffer. */
 ...
 */
int
bcm_tr2_multicast_create(int unit, uint32 flags, bcm_multicast_t *group)
{
    ...
    if (type == BCM_MULTICAST_TYPE_L2) {
        ...
        if ((type == BCM_MULTICAST_TYPE_VPLS) || 
            ...
            BCM_PBMP_REMOVE(l2_pbmp, SOC_PBMP_STACK_INACTIVE(unit));
             ...
             rv = _bcm_tr2_multicast_ipmc_write(unit, mc_index, l2_pbmp,
                                                l3_pbmp, TRUE);
            if (rv != BCM_E_NONE) {
                sal_memset(&egr_ipmc, 0, sizeof(egr_ipmc));
 ...
 */
int
bcm_tr2_multicast_destroy(int unit, bcm_multicast_t group)
{
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        ...
        PBMP_E_ITER(unit, port_iter) {
            rv = bcm_tr2_ipmc_egress_intf_set(unit, mc_index, port_iter,
                                             0, NULL);
            BCM_IF_ERROR_RETURN(rv);
        }
 ...
 */
int
bcm_tr2_multicast_subport_encap_get(int unit, bcm_multicast_t group, bcm_gport_t port,
 ...
 */
int
bcm_tr2_multicast_egress_set(int unit, bcm_multicast_t group, int port_count,
                            bcm_gport_t *port_array, bcm_if_t *encap_id_array)
{
    int mc_index, i, list_count, is_vpls = 0, rv = BCM_E_NONE;
    ...
    if ((0 == _BCM_MULTICAST_IS_L2(group)) && 
        ...
        (0 == _BCM_MULTICAST_IS_WLAN(group)) &&
    ...
    for (i = 0; i < port_count ; i++) {
        ...
        if (BCM_GPORT_IS_TRUNK(port_array[i])) {
            ...
            rv = _bcm_esw_gport_resolve(unit, port_array[i], &mod_out, &port_out,
                                        &trunk_id, &gport_id); 
            if (BCM_FAILURE(rv)) {
                sal_free(local_port_array);
                sal_free(temp_encap_list);
            ...
            if ((0 == IS_E_PORT(unit, port_out)) || (TRUE != modid_local)) {
                sal_free(local_port_array);
                sal_free(temp_encap_list);
                temp_encap_list  = NULL;
                return (BCM_E_PARAM);
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        for (i = 0; i < port_count; i++) {
            ...
            if (soc_feature(unit, soc_feature_sysport_remap)) {
                BCM_XLATE_SYSPORT_S2P(unit, &local_port_array[i]);
            }
 ...
 */
STATIC int     
_bcm_tr2_multicast_egress_get(int unit, bcm_multicast_t group, int port_max, 
                              ...
                              bcm_if_t *encap_id_array, int *port_count)
{
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        ...
        *port_count = 0;
        PBMP_E_ITER(unit, port_iter) {
            rv = bcm_tr2_ipmc_egress_intf_get(unit, mc_index, port_iter,
                                             port_max - *port_count, 
        ...
        /* Check for L2 ports */
        BCM_IF_ERROR_RETURN
            (_bcm_tr2_multicast_ipmc_read(unit, mc_index, &l2_pbmp,
                                          &l3_pbmp));
        PBMP_ITER(l2_pbmp, port_iter) {
            if (*port_count == port_max) {
                break;
 ...
 */
int     
bcm_tr2_multicast_egress_get(int unit, bcm_multicast_t group, int port_max, 
                            bcm_gport_t *port_array, bcm_if_t *encap_id_array,
                            int *port_count)
{
    int i;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_tr2_multicast_egress_get(unit, group, port_max, port_array,
 ...
 */
int
bcm_tr2_multicast_egress_add(int unit, bcm_multicast_t group,
                            bcm_gport_t port, bcm_if_t encap_id)
{
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        ...
        if (soc_feature(unit, soc_feature_sysport_remap)) {
            if (BCM_GPORT_IS_TRUNK(port)) {
                for (idx=0; idx<trunk_local_ports; idx++) {
                    BCM_XLATE_SYSPORT_S2P(unit, &trunk_member_port[idx]);
       	        }
            } else {
                BCM_XLATE_SYSPORT_S2P(unit, &port_out);
            }
        ...
        if (encap_id != BCM_IF_INVALID) {
            if (BCM_GPORT_IS_TRUNK(port)) {
                for (idx=0; idx<trunk_local_ports; idx++) {
                    rv =  _bcm_tr2_ipmc_egress_intf_add(unit, mc_index, 
                                        trunk_member_port[idx], encap_id);
                    if (rv < 0) {
                        while (idx--) {
                            (void) _bcm_tr2_ipmc_egress_intf_delete
                                (unit, mc_index, trunk_member_port[idx], 
                                 port_max, encap_id);
                        }
                        return rv;
                    }
                ...
                BCM_IF_ERROR_RETURN
                    (_bcm_tr2_ipmc_egress_intf_add(unit, mc_index,  
                                                   port_out, encap_id));
            }
            new_pbmp = &l3_pbmp;
        } else {
        ...
        rv = _bcm_tr2_multicast_ipmc_read(unit, mc_index, &l2_pbmp,
                                          &l3_pbmp);
        if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_FAILURE(rv) && (encap_id != BCM_IF_INVALID)) {
            if (BCM_GPORT_IS_TRUNK(port)) {
                 for (idx=0; idx<trunk_local_ports; idx++) {
                     (void)_bcm_tr2_ipmc_egress_intf_delete(unit, mc_index, 
                                                            trunk_member_port[idx], 
                                                            port_max, encap_id);
                 }
            } else {
                (void)_bcm_tr2_ipmc_egress_intf_delete(unit, mc_index, port_out,
		 			               port_max, encap_id);
            }
        }
    }
 ...
 */
int
bcm_tr2_multicast_egress_delete(int unit, bcm_multicast_t group,
                               bcm_gport_t port, bcm_if_t encap_id)
{
    ...
    if (!SOC_GPORT_IS_MODPORT(port) && !SOC_GPORT_IS_TRUNK(port)) {
        return BCM_E_PORT;
    } 
    ...
    if (_BCM_MULTICAST_IS_L2(group)) {
        ...
        /* Convert system port values to physical port values */
        if (soc_feature(unit, soc_feature_sysport_remap)) {
            if (BCM_GPORT_IS_TRUNK(port)) {
                for (idx=0; idx<trunk_local_ports; idx++) {
                    BCM_XLATE_SYSPORT_S2P(unit, &trunk_member_port[idx]);
        ...
        if (encap_id == BCM_IF_INVALID) {
            ...
            rv = _bcm_tr2_multicast_ipmc_read(unit, mc_index, 
                                              &l2_pbmp, &l3_pbmp);
            if (BCM_SUCCESS(rv)) {
                ...
                if (BCM_PBMP_NEQ(old_l2_pbmp, l2_pbmp)) {
                    rv = _bcm_tr2_multicast_ipmc_write(unit, mc_index,
        ...
        rv = _bcm_tr2_multicast_egress_get(unit, group, port_max, port_array, 
        ...
        if (BCM_GPORT_IS_TRUNK(port)) {
            for (idx=0; idx<trunk_local_ports; idx++) {		   
                 rv = _bcm_tr2_ipmc_egress_intf_delete(unit, mc_index, 
                                                       trunk_member_port[idx],
                                                       port_max, encap_id);
                 if (rv < 0) {
                     while (idx--) {
                         (void)_bcm_tr2_ipmc_egress_intf_add(unit, mc_index, 
                                                             trunk_member_port[idx],
                                                             encap_id);
                     }
                     return rv;
             ...
             rv =  _bcm_tr2_ipmc_egress_intf_delete(unit, mc_index, port_out,
                                                    port_max, encap_id);
        }
    }
    return rv;
 ...
 */
int
bcm_tr2_multicast_egress_delete_all(int unit, bcm_multicast_t group)
{
    return bcm_tr2_multicast_egress_set(unit, group, 0, NULL, NULL);
}
