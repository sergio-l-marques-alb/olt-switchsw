/*
 * $Id: esw_filter.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
_bcm_filter_compress(int unit, filter_t *f, int blk)
{
    ...
    if (SOC_BLOCK_IS_TYPE(unit, blk, SOC_BLK_XPIC)) {
...
_bcm_filter_op_install(int unit, filter_t *f, int blk)
{
    ...
    soc_mem_field32_set(unit, fc->fc_blk2rule[blk], &f->rule,
                        FSELf, index);
...
#ifdef BCM_TUCANA_SUPPORT
    if (SOC_IS_TUCANA(unit)) {
        if (SOC_BLOCK_IS_TYPE(unit, blk, SOC_BLK_GPIC) ||
