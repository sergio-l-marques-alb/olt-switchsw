/*
 * $Id: esw_rcpu.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
_bcm_rcpu_get_config(int unit)
{
    ...
    /*
     * Get the remote CMIC mac address.
     */
    /* configure default mac address for rmac */
    
    sal_memcpy(BCM_RCPU_CFG_LMAC(unit), rcpu_default_mac,
               sizeof(bcm_mac_t));
    BCM_RCPU_CFG(unit)->lmac[5] = unit;

    /* Set the default CPU MAC address */
    sal_memcpy(BCM_RCPU_CFG_SRC_MAC(unit), src_cpu_default_mac,
               sizeof(bcm_mac_t));

    /* Get the default VLAN and TPID */
    BCM_RCPU_CFG(unit)->vlan = soc_property_get(unit, spn_RCPU_VLAN_ID,
                                                BCM_VLAN_DEFAULT);
    ...
    if (soc_feature(unit, soc_feature_rcpu_priority)) {
        BCM_RCPU_CFG(unit)->dot1pri[0] = soc_property_get(unit, spn_DOT1PRI_COS0, 0);
        BCM_RCPU_CFG(unit)->mh_tc[0] = soc_property_get(unit, spn_MH_TC_COS0, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[0] = soc_property_get(unit, spn_CPU_TC_COS0, 0);
        BCM_RCPU_CFG(unit)->dot1pri[1] = soc_property_get(unit, spn_DOT1PRI_COS1, 0);
        BCM_RCPU_CFG(unit)->mh_tc[1] = soc_property_get(unit, spn_MH_TC_COS1, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[1] = soc_property_get(unit, spn_CPU_TC_COS1, 0);
        BCM_RCPU_CFG(unit)->dot1pri[2] = soc_property_get(unit, spn_DOT1PRI_COS2, 0);
        BCM_RCPU_CFG(unit)->mh_tc[2] = soc_property_get(unit, spn_MH_TC_COS2, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[2] = soc_property_get(unit, spn_CPU_TC_COS2, 0);
        BCM_RCPU_CFG(unit)->dot1pri[3] = soc_property_get(unit, spn_DOT1PRI_COS3, 0);
        BCM_RCPU_CFG(unit)->mh_tc[3] = soc_property_get(unit, spn_MH_TC_COS3, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[3] = soc_property_get(unit, spn_CPU_TC_COS3, 0);
        BCM_RCPU_CFG(unit)->dot1pri[4] = soc_property_get(unit, spn_DOT1PRI_COS4, 0);
        BCM_RCPU_CFG(unit)->mh_tc[4] = soc_property_get(unit, spn_MH_TC_COS4, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[4] = soc_property_get(unit, spn_CPU_TC_COS4, 0);
        BCM_RCPU_CFG(unit)->dot1pri[5] = soc_property_get(unit, spn_DOT1PRI_COS5, 0);
        BCM_RCPU_CFG(unit)->mh_tc[5] = soc_property_get(unit, spn_MH_TC_COS5, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[5] = soc_property_get(unit, spn_CPU_TC_COS5, 0);
        BCM_RCPU_CFG(unit)->dot1pri[6] = soc_property_get(unit, spn_DOT1PRI_COS6, 0);
        BCM_RCPU_CFG(unit)->mh_tc[6] = soc_property_get(unit, spn_MH_TC_COS6, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[6] = soc_property_get(unit, spn_CPU_TC_COS6, 0);
        BCM_RCPU_CFG(unit)->dot1pri[7] = soc_property_get(unit, spn_DOT1PRI_COS7, 0);
        BCM_RCPU_CFG(unit)->mh_tc[7] = soc_property_get(unit, spn_MH_TC_COS7, 0);
        BCM_RCPU_CFG(unit)->cpu_tc[7] = soc_property_get(unit, spn_CPU_TC_COS7, 0);
    }
    return BCM_E_NONE;
}
    ...
    *flags = 0; /* clear the flags */
    ...
    if (!BCM_PKT_TX_ETHER(pkt)) {
        ...
        if (SOC_DCB_TYPE(unit) == 14) {
            ...
            PBS_MH_V3_W2_SMOD_DPORT_COS_SET(pbh, pkt->src_mod, port,
                                            pkt->cos, prio_val, 0);
        } else if (SOC_DCB_TYPE(unit) == 16) {
            PBS_MH_V4_W2_SMOD_DPORT_COS_SET(pbh, pkt->src_mod, port,
                                            pkt->cos, pkt->prio_int, 0);
        } else {
            PBS_MH_V2_W2_SMOD_DPORT_COS_SET(pbh, pkt->src_mod, port,
                                            pkt->cos, pkt->prio_int, 0);
        }
...
rcpu1_init(int unit)
{
    ...
    soc_reg_field_set(unit, CMIC_PKT_PORTSr, &rval, PORTSf, 
                      SOC_PBMP_WORD_GET(BCM_RCPU_CFG(unit)->pbmp, 0));
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_PORTSr(unit, rval));
    
    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_PORTS_HIr, &rval, PORTS_HIf, 
                      SOC_PBMP_WORD_GET(BCM_RCPU_CFG(unit)->pbmp, 1));
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_PORTS_HIr(unit, rval));

    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_REASONr, &rval, REASONSf, ~0);
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_REASONr(unit, rval));

    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_REASON_DIRECTr, &rval, REASONSf, 0);
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_REASON_DIRECTr(unit, rval));
    ...
    /* enable all COS */
    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_COSr, &rval, COSf, 0xffff);
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_COSr(unit, rval));

    /* Set remote mac address */
    mac = BCM_RCPU_CFG_SRC_MAC(unit);
    rval = (mac[2] << 24) | (mac[3] << 16) | (mac[4] << 8) | mac[5];
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_RMACr(unit, rval));
    rval = (mac[0] << 8) | mac[1];
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_RMAC_HIr(unit, rval));

    /* Set cmic mac address */
    mac = BCM_RCPU_CFG_LMAC(unit);
    rval = (mac[2] << 24) | (mac[3] << 16) | (mac[4] << 8) | mac[5];
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_LMAC0_LOr(unit, rval));
    rval = (mac[0] << 8) | mac[1];
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_LMAC0_HIr(unit, rval));

    /* set CMIC EtherType and Signature */
    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_ETHER_SIGr, &rval, ETHERTYPEf, 
                      BCM_RCPU_CFG(unit)->ether_type);
    soc_reg_field_set(unit, CMIC_PKT_ETHER_SIGr, &rval, SIGNATUREf, 
                      BCM_RCPU_CFG(unit)->signature);
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_ETHER_SIGr(unit, rval));

    /* Set the vlan and tpid values */
    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_VLANr, &rval, TPIDf, 
                      BCM_RCPU_CFG(unit)->tpid);
    soc_reg_field_set(unit, CMIC_PKT_VLANr, &rval, VLAN_IDf, 
                      BCM_RCPU_CFG(unit)->vlan);
    SOC_IF_ERROR_RETURN(WRITE_CMIC_PKT_VLANr(unit, rval));

    /* Enable vlan/lmac1 checking */
    rval = 0;
    soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, LMAC0_MATCHf, 1);
    soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, VLAN_MATCHf, 1);
    soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, ENABLE_TOCPU_PACKETf, 1);
    soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, ENABLE_FROMCPU_PACKETf, 1);
    soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, ENABLE_SCHAN_REQUESTf, 1);
#if defined(BCM_TRX_SUPPORT) || defined(BCM_RAVEN_SUPPORT)
    if (soc_feature(unit, soc_feature_rcpu_priority)) {
        uint8 property;
        property = soc_property_get(unit, spn_MH_SRC_PID_ENABLE, 0);
        soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, MH_SRC_PID_ENABLEf, property);
        property = soc_property_get(unit, spn_CPU_TC_ENABLE, 0);
        soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, CPU_TC_ENABLEf, property);
        property = soc_property_get(unit, spn_MH_TC_MAP_ENABLE, 0);
        soc_reg_field_set(unit, CMIC_PKT_CTRLr, &rval, MH_TC_MAP_ENABLEf, property); 
        property = soc_property_get(unit, spn_DOT1PRI_MAP_ENABLE, 0);  
    ...
    /* program cos, forward reason, direct, mini, direct */
    /*
    ...
    if ( masterunit >= 0 ) {
      ...
      l2addr.port = soc_property_get(masterunit, spn_RCPU_PORT, 3);
    ...
    l2addr.modid = soc_property_get(unit, spn_RCPU_SLAVE_MODID, mymodid);
    if ( soc_property_get(unit, spn_RCPU_HIGIG_PORT, 0) ) {
      /** for higig, we want to do it on the higig port */
      l2addr.port = 1;  /* ge0 */
    } else {
        l2addr.port = soc_property_get(unit, spn_RCPU_PORT, 3);
    }
    bcm_l2_addr_add(unit, &l2addr);
    ...
    if ( masterunit >= 0 ) {
        ...
        /* Prepare VLAN translation on the RCPU port and CPU port if
           double tagging mode gets enabled for the master unit. This
           forces the service vlan to always be the RCPU vlan for RCPU
           protocol packets. */
    ...
    if (soc_feature(unit, soc_feature_rcpu_priority)) {
        for (i = 0; i < NUM_COS(unit); i++) {
            rval = 0;
            soc_reg_field_set(unit, CMIC_PKT_PRI_MAP_TABLEr, &rval, IEEE_802_1_Pf, 
                              BCM_RCPU_CFG(unit)->dot1pri[i]);
            soc_reg_field_set(unit, CMIC_PKT_PRI_MAP_TABLEr, &rval, MH_TCf, 
                              BCM_RCPU_CFG(unit)->mh_tc[i]);
            soc_reg_field_set(unit, CMIC_PKT_PRI_MAP_TABLEr, &rval, CPU_TCf, 
                              BCM_RCPU_CFG(unit)->cpu_tc[i]);
            BCM_IF_ERROR_RETURN(WRITE_CMIC_PKT_PRI_MAP_TABLEr(unit, i, rval));
        }
    }
#endif /* BCM_TRX_SUPPORT || BCM_RAVEN_SUPPORT */
...
#define BCM_TRX_CPU_QUEUE_MAX      31
    ...
    if (soc_feature(unit, soc_feature_rcpu_tc_mapping)) {
        ...
        for (tc = 0; tc < tc_total; tc++) {
            ...
            queue = soc_property_suffix_num_get(unit, tc, spn_CPU_QUEUE,
                                                "tc", 0);
...
STATIC int
_bcm_raptor_fill_cmic_hdr(int unit, rcpu1_cmic_pkt_hdr_t *p_rcpu_hdr, 
...
rcpu1_tx(int unit, bcm_pkt_t *pkt, void * cookie)
{
    ...
    if (BCM_RCPU_CFG(unit)->flags & BCM_RCPU_F_ETHER) {
        ...
        if (BCM_RCPU_CFG(unit)->flags & BCM_RCPU_F_PACK) {
            ...
            _bcm_raptor_fill_cmic_hdr(unit, (rcpu1_cmic_pkt_hdr_t *)tx_buf, 
...
rcpu1_schan_op(int unit, schan_msg_t *msg, int dwc_write, int dwc_read)
{
    ...
    /*sal_memcpy(buf_ptr, (void *)msg->bytes, 4 * dwc_write);*/
   ...
   _bcm_raptor_fill_cmic_hdr(unit, (rcpu1_cmic_pkt_hdr_t *)tx_buf, 
    ...
    for (num_retries = 0; num_retries < MAX_RETRY_ATTEMPTS; num_retries++) {
        ...
        if (soc_property_get(unit, spn_RCPU_USE_OOB, 1)) {
            ...
            rv = bcm_tx(remunit, new_pkt, (void*)new_pkt);
        }
        ...
        if (rv == SOC_E_NONE) {
            for (i = 0; i < 4 * dwc_read; i++) {
                msg->bytes[i] = schan_reply[cur_transid].data[i];
            }
            break;
        }
    }
...
_bcm_rcpu_raw_rx(int eth_unit, uint8 * pkt, int pkt_len)
{
	 ...
	 *     If FAIL, then set length appropriately
        ...
        if (rcpu_hdr->flags & BCM_RCPU_FLAG_FAIL) {
            ...
            rcpu_hdr->datalen = pkt_len - sizeof(rcpu1_cmic_pkt_hdr_t);
        }
        
        if (schan_reply[transid].waiting_for_reply == TRUE) {
            sal_memcpy(schan_reply[transid].data, 
                       pkt + sizeof(rcpu1_cmic_pkt_hdr_t), 
                       rcpu_hdr->datalen);
    ...
    /* fake partial hdr to real dcb (-8 is a result of this) */
    dcb = (dcb_t *) ((uint8*) pkt + 
...
STATIC void 
_rcpu_modid_chg_cb(int unit, int modid, void* userdata)
{
#if defined(BCM_RAPTOR_SUPPORT)
    rcpu1_init(unit);
#endif
...
_bcm_esw_rcpu_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_esw_stk_modid_chg_cb_register(unit, _rcpu_modid_chg_cb, NULL));
