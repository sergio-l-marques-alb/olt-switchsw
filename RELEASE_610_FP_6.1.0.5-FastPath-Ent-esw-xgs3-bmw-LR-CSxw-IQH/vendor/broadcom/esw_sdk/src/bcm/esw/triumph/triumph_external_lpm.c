/* $Id: triumph_external_lpm.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
_tr_ext_lpm_sw_dump(int unit)
{
    ...
    if (NULL != _TR_LPM_SW_IMAGE(unit, _TR_LPM_IPV4)) {
        soc_cm_print("     IPv4 Prefix entries\n");

        for (i = 0; i <  _TR_LPM_PREFIX_MAX(EXT_IPV4_DEFIPm); i++) {
    ...
    if (NULL != _TR_LPM_SW_IMAGE(unit, _TR_LPM_IPV6)) {
        soc_cm_print("     IPv6 Prefix entries \n" );
