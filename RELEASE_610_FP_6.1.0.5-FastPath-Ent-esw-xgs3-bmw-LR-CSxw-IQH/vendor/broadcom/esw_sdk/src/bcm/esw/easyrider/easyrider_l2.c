/*
 * $Id: easyrider_l2.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
bcm_er_l2_addr_delete(int unit, bcm_mac_t mac, bcm_vlan_t vid)
{
    ...
    if ((rv = soc_mem_delete(unit, L2_ENTRY_INTERNALm, COPYNO_ALL, 
                                                    &l2er_entry)) >= 0) {
        rv = soc_l2x_sync_delete(unit, (uint32 *)&l2er_lookup, l2_index);
    }
                                        ...
                                        soc_mem_t tmem, void *data)
{
    ...
    bcm_trunk_t                 ent_tid = 0,  tid_hi = 0;
    ...
    if (ent_port & BCM_TGID_TRUNK_INDICATOR(unit)) {
        tid_hi = ent_port & BCM_TGID_PORT_TRUNK_MASK_HI(unit);
        ent_tid = (ent_port & BCM_TGID_PORT_TRUNK_MASK(unit)) | (tid_hi >> 1);
    }
...
_bcm_er_l2_addr_replace_by_vlan_dest(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
    ...
    if (flags & BCM_L2_REPLACE_MATCH_VLAN) {
        ...
        dvp_info.vlan = rep_st->match_vid;
        vlan = rep_st->match_vid;
    }
    if (flags & BCM_L2_REPLACE_MATCH_DEST) {
        ...
        if (rep_st->isTrunk) {
            ...
            dvp_info.tid = rep_st->match_trunk;
            port_tgid = BCM_TRUNK_TO_TGIDf(unit, rep_st->match_trunk);
            mod = BCM_TRUNK_TO_MODIDf(unit, rep_st->match_trunk);
            new_tgid = rep_st->new_trunk;
        } else {
            dvp_info.by_port = TRUE;
            dvp_info.by_trunk = FALSE;
            dvp_info.port = rep_st->match_port;
            dvp_info.mod = rep_st->match_module;
            port_tgid = rep_st->match_port;
            mod = rep_st->match_module;
            new_tgid = rep_st->new_port;
        }
    }
    ...
    dvp_info.new_mod = rep_st->new_module;
    if (flags & BCM_L2_REPLACE_NEW_TRUNK) {
        dvp_info.new_port = -1;
        dvp_info.new_tid = rep_st->new_trunk;
    } else {
        dvp_info.new_port = rep_st->new_port;
        dvp_info.new_tid = -1;
    }
    ...
    (void) soc_er_l2x_delete_by(unit, mod, port_tgid, vlan, rep_st->new_module, 
                                new_tgid, op, 
...
int
_bcm_er_l2_addr_get(int unit, mac_addr_t mac, bcm_vlan_t vid,
                   bcm_l2_addr_t *l2addr, bcm_pbmp_t *pbm)
{
    l2_entry_internal_entry_t    l2er_entry, l2er_lookup;
...
int
bcm_er_l2_addr_get(int unit, mac_addr_t mac, bcm_vlan_t vid,
                   bcm_l2_addr_t *l2addr)
{
    bcm_pbmp_t pbm;
