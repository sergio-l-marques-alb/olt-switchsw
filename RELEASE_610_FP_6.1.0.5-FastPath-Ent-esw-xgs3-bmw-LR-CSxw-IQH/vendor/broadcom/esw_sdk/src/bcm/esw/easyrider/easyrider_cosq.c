/*
 * $Id: easyrider_cosq.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
                             ...
                             int average_time)
{
    ...
    SOC_IF_ERROR_RETURN
            (READ_WREDAVERAGINGTIMEr(unit, port, &val));
    soc_reg_field_set(unit, WREDAVERAGINGTIMEr, &val, EXPf, exp);
    SOC_IF_ERROR_RETURN
            (WRITE_WREDAVERAGINGTIMEr(unit, port, val));

    if (cosq >= _num_cosq[unit]) {
        return BCM_E_PARAM;
    } else if (cosq < 0) {
        cosq_start = 0;
    ...
    for (idx = cosq_start; idx < cosq_start + num_cosq; idx++) {
        ...
        SOC_IF_ERROR_RETURN(READ_EGRPKTLIMITCOSr(unit, port, idx, &pkt_limit));
        if (color & BCM_COSQ_DISCARD_COLOR_YELLOW) {
            ...
            SOC_IF_ERROR_RETURN(READ_WREDPARAMYELCOSr(unit, port, idx, &val));
            soc_reg_field_set(unit, WREDPARAMYELCOSr, &val, DROPSTARTPOINTf,
                              start);
            soc_reg_field_set(unit, WREDPARAMYELCOSr, &val, SLOPEf, slope);
            SOC_IF_ERROR_RETURN(WRITE_WREDPARAMYELCOSr(unit, port, idx, val));
        }
        if (color & BCM_COSQ_DISCARD_COLOR_RED) {
            limit = soc_reg_field_get(unit, EGRPKTLIMITCOSr, pkt_limit,
                                      XQREDLIMITf);
            start = (limit * xq_factor * drop_start) / 100 / 4;
            SOC_IF_ERROR_RETURN(READ_WREDPARAMREDCOSr(unit, port, idx, &val));
            soc_reg_field_set(unit, WREDPARAMREDCOSr, &val, DROPSTARTPOINTf,
                              start);
            soc_reg_field_set(unit, WREDPARAMREDCOSr, &val, SLOPEf, slope);
            SOC_IF_ERROR_RETURN(WRITE_WREDPARAMREDCOSr(unit, port, idx, val));
        }
        if (!color || (color & BCM_COSQ_DISCARD_COLOR_GREEN)) {
            limit = soc_reg_field_get(unit, EGRPKTLIMITCOSr, pkt_limit,
                                      XQMINENTRYf);
            start = (limit * xq_factor * drop_start) / 100 / 4;
            SOC_IF_ERROR_RETURN(READ_WREDPARAMCOSr(unit, port, idx, &val));
            soc_reg_field_set(unit, WREDPARAMCOSr, &val, DROPSTARTPOINTf,
                              start);
            soc_reg_field_set(unit, WREDPARAMCOSr, &val, SLOPEf, slope);
            SOC_IF_ERROR_RETURN(WRITE_WREDPARAMCOSr(unit, port, idx, val));
        }
    }
                             ...
                             int *average_time)
{
    ...
    /* Read the packet limits for this cos */
    SOC_IF_ERROR_RETURN
            (READ_EGRPKTLIMITCOSr(unit, port, cosq, &pkt_limit));
    /*
     * The limits are programmed as units of XQ multiples.
     * A unit equals 'xq_factor' XQ entries.
     */
