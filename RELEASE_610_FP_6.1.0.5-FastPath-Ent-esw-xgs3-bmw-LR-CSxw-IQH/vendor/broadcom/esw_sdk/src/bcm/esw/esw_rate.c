/*
 * $Id: esw_rate.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
                              ...
                              int bucketSz, uint32 refreshCnt)
{
    ...
    soc_mem_field32_set(unit, FP_STORM_CONTROL_METERSm, &entryMeter, 
                        BUCKETSIZEf, bucketSz);
    soc_mem_field32_set(unit, FP_STORM_CONTROL_METERSm, &entryMeter, 
                        REFRESHCOUNTf, refreshCnt);
    soc_mem_field32_set(unit, FP_STORM_CONTROL_METERSm, &entryMeter, 
                        BUCKETCOUNTf, 0);

    entryNum = (port << 2) + index;
    soc_mem_write(unit, FP_STORM_CONTROL_METERSm, MEM_BLOCK_ALL, 
                  entryNum, &entryMeter);
                                  ...
                                  int rps, int brs)
{
    ...
    if (mode == _BCM_TRX_RATE_PKT_MODE) {
        ...
        for (quantum = 4000; quantum >= 125; quantum >>= 1) {
            if (rps <= _BCM_TRX_RATE_MAX_REFRESH_COUNT * (8000 / quantum)) {
                break;
            }
        }
        if (quantum < 125) {
                              ...
                              uint32 *pRps, uint32 *pBrs)
{
    ...
    entryNum = (port << 2) + index;
    BCM_IF_ERROR_RETURN(
    soc_mem_read(unit, FP_STORM_CONTROL_METERSm, MEM_BLOCK_ANY,
                     entryNum, &entryMeter));

    refreshCnt = soc_mem_field32_get(unit, FP_STORM_CONTROL_METERSm, 
                                                  &entryMeter, REFRESHCOUNTf);
    bucketSz   = soc_mem_field32_get(unit, FP_STORM_CONTROL_METERSm, 
                                                  &entryMeter, BUCKETSIZEf);
    if (mode == _BCM_TRX_RATE_PKT_MODE) {
        SOC_IF_ERROR_RETURN
...
bcm_esw_rate_set(int unit, int pps, int flags)
{
    ...
    if (SOC_IS_XGS_SWITCH(unit)) {
        ...
        PBMP_E_ITER(unit, port) {
            ...
            SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, rate));
...
bcm_esw_rate_mcast_get(int unit, int *pps, int *flags, int port)
{
    ...
    if (SOC_IS_XGS_SWITCH(unit)) {
        ...
        if (IS_E_PORT(unit, port)) { /* not IPIC port */
            ...
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rate));
            *pps = 0;
            *flags = 0;
