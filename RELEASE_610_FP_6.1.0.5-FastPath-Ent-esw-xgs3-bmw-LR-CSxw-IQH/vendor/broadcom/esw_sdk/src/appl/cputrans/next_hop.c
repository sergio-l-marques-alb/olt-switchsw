/*
 * $Id: next_hop.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
#define NEXT_HOP_OUT(stuff)       TKS_DEBUG(TKS_DBG_NORMAL, stuff)
#define NEXT_HOP_DEBUG(stuff)     TKS_DEBUG(TKS_DBG_NH, stuff)
#define NH_VERY_VERBOSE(stuff)    TKS_DEBUG(TKS_DBG_NHV, stuff)
#else
#define NEXT_HOP_OUT(stuff)
#define NEXT_HOP_DEBUG(stuff)
#define NH_VERY_VERBOSE(stuff)
...
#else
static bcm_trans_ptr_t *next_hop_trans_ptr = NULL;
#endif
...
STATIC int _port_add(int unit, int port, int duplex);
 ...
 * Purpose:
 *      Set/get the COS and VLAN settings used by
 * Notes:
...
next_hop_stop(void)
{
    ...
    if (nh_thread_id != SAL_THREAD_ERROR) {
        ...
        if (nh_thread_id != SAL_THREAD_ERROR) {
            NEXT_HOP_DEBUG(("Warning:  NEXT_HOP thread did not exit\n"));
        }
    }
...
STATIC int
_port_add(int unit, int port, int duplex)
{
    int rv;
    int i;
    ...
    if (_unit_on_list(unit) < 0 && setup_done) {
...
handle_rx_queue(void)
{
    ...
    if (rx_queue != NULL) {
	    ...
	    pkt->next = NULL;
            ...
            NH_VERY_VERBOSE(("nh pkt " CPUDB_KEY_FMT
...
_rx_packet_enqueue(bcm_pkt_t *pkt)
{
    ...
    NH_VERY_VERBOSE(("next hop rx enqueue\n"));

    new_pkt = _rx_packet_alloc(pkt);
    if (new_pkt == NULL) {
        NEXT_HOP_DEBUG(("NH: no queue, resource\n"));
        return BCM_E_RESOURCE;
    }
                   ...
                   uint32 flags)
{
    ...
    NH_VERY_VERBOSE(("next hop tx enqueue\n"));
             ...
             void *cookie)
{
    ...
    for (i = 0; i < num_stk_ports; i++) {
        ...
        if (tmp_rv < 0) {
            NEXT_HOP_DEBUG(("NEXT_HOP: Failed to send to unit %d, port %d\n",
...
next_hop_rx_callback(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    NH_VERY_VERBOSE(("NEXT_HOP pkt in\n"));

    if (next_hop_lock == NULL || !setup_done ||
                              nh_thread_id == SAL_THREAD_ERROR) {
        NEXT_HOP_DEBUG(("exit: %p, %d, %p\n",
    ...
    if (!nh_tx_pkt_recognize(pkt->pkt_data[0].data, &pkt_type)) {
        NH_VERY_VERBOSE(("NEXT_HOP pkt not recognized\n"));
        return BCM_RX_NOT_HANDLED;
    }
    if (pkt_type != NEXT_HOP_PKT_TYPE) {    /* Is this an NEXT_HOP packet */
        NEXT_HOP_DEBUG(("NEXT_HOP pkt not proper type\n"));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    if (_rx_packet_enqueue(pkt) < 0) {
        NH_VERY_VERBOSE(("NH: Dropped RX pkt %d\n", seq_num));
        ++rx_pkt_drop_count;
        return BCM_RX_HANDLED;
    }
...
_rx_key_seq_seen(cpudb_key_t key, uint16 seq_num)
{
    ...
    if (local_idx < 0) {   /* New CPU key; hence new packet */
        ...
        if (local_idx < 0) {
            NEXT_HOP_DEBUG(("NEXT_HOP key rsrc err\n"));
            return BCM_E_RESOURCE;
        }
    } else {
        NH_VERY_VERBOSE(("Local idx %d\n", local_idx));
        if (_seq_num_seen(local_idx, seq_num)) {  /* Seen it */
            NH_VERY_VERBOSE(("NEXT_HOP pkt previously seen\n"));
            return TRUE;
        }
    }
...
next_hop_dump(void)
{
    NEXT_HOP_OUT(("Next Hop\n"));

    NEXT_HOP_OUT(("  %s.  %s.\n",
                  (next_hop_lock != NULL) ? "Initialized":"Not initialized",
                  setup_done ? "Running":"Not running"));
    NEXT_HOP_OUT(("  thread_priority   = %d\n", next_hop_thread_priority));
    NEXT_HOP_OUT(("  rx_priority       = %d\n", next_hop_rx_priority));
    NEXT_HOP_OUT(("  rx_queue_size     = %d\n", next_hop_rx_queue_size));
    NEXT_HOP_OUT(("  tx_queue_size     = %d\n", next_hop_tx_queue_size));
    NEXT_HOP_OUT(("  mtu               = %d\n", next_hop_mtu));
    NEXT_HOP_OUT(("  vlan              = %d\n", nh_vlan));
    NEXT_HOP_OUT(("  cos               = %d\n", nh_cos));
    NEXT_HOP_OUT(("  local_key         = " CPUDB_KEY_FMT "\n",
                  CPUDB_KEY_DISP(next_hop_local_key)));
    NEXT_HOP_OUT(("  rx_pkt_drop_count = %d\n", rx_pkt_drop_count));
    NEXT_HOP_OUT(("  tx_error_count    = %d\n", tx_error_count));
