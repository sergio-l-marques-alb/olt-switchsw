/*
 * $Id: atptrans_socket.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 *
...
#include <netinet/tcp.h>
#define ATPTRANS_SOCKET_SUPPORTED 1
#if !defined(_SOCKLEN_T) && !defined(__socklen_t_defined) && (VX_VERSION < 60)
 ...
 * Debug
 *
 * Macros used for debug information
 */
#if defined(BROADCOM_DEBUG)
#define ATP_SOCK_DEBUG(flags, stuff) TKS_DEBUG(TKS_DBG_ATP | (flags), stuff)
#define ATP_SOCK_ERR(stuff)          ATP_SOCK_DEBUG(TKS_DBG_ERR, stuff)
#define ATP_SOCK_WARN(stuff)         ATP_SOCK_DEBUG(TKS_DBG_WARN, stuff)
#define ATP_SOCK_VERB(stuff)         ATP_SOCK_DEBUG(TKS_DBG_VERBOSE, stuff)
#define ATP_SOCK_VVERB(stuff)        ATP_SOCK_DEBUG(TKS_DBG_ATPV, stuff)
#define ATP_SOCK_OUT(stuff)          TKS_DEBUG(TKS_DBG_ATP, stuff)
#else
#define ATP_SOCK_ERR(stuff)
#define ATP_SOCK_WARN(stuff)
#define ATP_SOCK_VERB(stuff)
#define ATP_SOCK_VVERB(stuff)
#define ATP_SOCK_OUT(stuff)
...
static int atp_sock_cfg_lport      = ATP_SOCK_SERVER_LPORT_DEFAULT;
...
recv_pkt_hdr(int fd, atp_sock_pkt_hdr_t *hdr)
{
    ...
    if (n_recv == sizeof(hdr_buffer)) {
        ...
        ATP_SOCK_VVERB(("ATP-Socket: Receive packet header on socket %d, "
        ...
        ATP_SOCK_VERB(("ATP-Socket: Connection socket %d received FIN\n", fd));
    } else if (n_recv < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: System error\n"));
    } else {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot read packet header\n"));
    }
...
recv_pkt_data(int fd, uint8 *buffer, int n_expected)
{
    ...
    if (n_expected == n_recv) {
        ATP_SOCK_VVERB(("ATP-Socket: Receive packet data on socket %d, "
                        "length %d\n", fd, n_expected));
        rv = BCM_E_NONE;
    } else if (n_recv == 0) {    /* EOF */
        ATP_SOCK_VERB(("ATP-Socket: Connection socket %d received FIN\n", fd));
        ...
        ATP_SOCK_ERR(("ATP-Socket ERROR: Expected packet data length %d, "
                      "received % bytes\n", n_expected, n_recv));
    }
...
send_pkt_data(int client_id, int fd, uint8 *buffer, int data_len)
{
    ...
    if (n_wrote < hdr_size) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot send packet header\n"));
        return BCM_E_FAIL;
    }
    ...
    if (n_wrote < data_len) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot send packet data, "
                      "payload length %d, sent %\n", data_len, n_wrote));
        return BCM_E_FAIL;
    }

    ATP_SOCK_VVERB(("ATP-Socket: Send packet on socket %d, "
...
rx_conn_thread(void *cookie)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Start RX Connection Thread, socket %d\n",
                   rx_sock));
    ...
    while (!(conn_ptr->flags & ATP_SOCK_CONN_ABORT)) {
        ...
        if (buffer == NULL) {
            ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot allocate receive "
                          "buffer\n"));
            rv = BCM_E_MEMORY;
            break;
        }
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Destroy connection to '%s', socket %d\n",
                    dest_ip_str, rx_sock));
...
conn_destroy(atp_sock_db_entry_t *db_ptr)
{
    ...
    if (i >= ATP_SOCK_STOP_RETRIES_MAX) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot destroy connection on "
                      "socket %d\n", db_ptr->conn.sock_fd));
        return BCM_E_FAIL;
    }
...
conn_create(atp_sock_db_entry_t *db_ptr, int sock_fd)
{
    ...
    if (conn_ptr->flags & ATP_SOCK_CONN_VALID) {
        ATP_SOCK_ERR(("ATP-Socket ERROR:  Connection already exists\n"));
        return BCM_E_EXISTS;
    }
    ...
    if (thread_id == SAL_THREAD_ERROR) {
        ...
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot create connection thread\n"));
        return BCM_E_FAIL;
    }
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Created connection to '%s', socket %d, "
               "thread '%s' (id = 0x%x)\n",
               dest_ip_str, conn_ptr->sock_fd,
               conn_ptr->rx_thread_name, conn_ptr->rx_thread_id));
...
conn_request(atp_sock_db_entry_t *db_ptr)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Connection request to '%s'\n", dest_ip_str));

    /* Open socket */
    if ((conn_sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot open socket (err=%d)\n",
                      conn_sock));
        return conn_sock;
    }
    ...
    server_addr.sin_port   = htons(atp_sock_cfg_lport);
    server_addr.sin_addr.s_addr = htonl(db_ptr->ip);

    if ((rv = connect(conn_sock, 
                      (struct sockaddr *) &server_addr, addr_len)) < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot connect to '%s' (err=%d)\n",
                      dest_ip_str, rv));
        close(conn_sock);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot create TX connection to '%s', "
    ...
    ATP_SOCK_VERB(("ATP-Socket: Connection established with '%s' on socket "
                   "%d\n", dest_ip_str, conn_sock));
                ...
                cpudb_key_t const cpu_key, bcm_ip_t ip, uint32 flags)
{
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Added CPU key " CPUDB_KEY_FMT "\n",
...
accept_connection(int listen_sock)
{
    ...
    if (rv < 0) {
        ATP_SOCK_VERB(("ATP-Socket:  Accept returned error on socket %d "
                       "(rv=%d)\n", listen_sock, rv));
        return BCM_E_FAIL;
    }
    ...
    ATP_SOCK_VERB(("ATP-Socket: Connection request from client '%s' on "
                   "socket %d\n", dest_ip_str, conn_sock));
    
    if (BCM_FAILURE(rv)) {
        ATP_SOCK_WARN(("ATP-Socket WARNING: Invalid IP address '%s', closing "
                       "socket %d\n", dest_ip_str, conn_sock));
        close(conn_sock);
        return BCM_E_NONE;
    }
    ...
    if ((db_ptr = db_ip_lookup(atp_sock_db_list, dest_ip)) == NULL) {
        if ((db_ptr = db_entry_create(&atp_sock_db_list,
            ...
            ATP_SOCK_UNLOCK;
            ATP_SOCK_WARN(("ATP-Socket ERROR: Cannot create new entry, "
                           "closing socket %d\n", conn_sock));
            close(conn_sock);
            return BCM_E_NONE;
        }
    ...
    if (db_ptr->conn.flags & ATP_SOCK_CONN_VALID) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_WARN(("ATP-Socket ERROR: Connection already exists, closing "
                 "socket %d\n", conn_sock));
        close(conn_sock);
        return BCM_E_NONE;
    }
    ...
    if (BCM_FAILURE(rv)) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_WARN(("ATP-Socket WARNING: Cannot create connection to '%s', "
                       "closing socket %d\n", dest_ip_str, conn_sock));
        close(conn_sock);
        return BCM_E_NONE;
    }
...
server_conn_thread(void *cookie)
{
    ...
    if ((rv = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot open Server listening socket "
                      "(err=%d)\n", rv));
    ...
    if ((rv = bind (listen_sock, 
                    (struct sockaddr *) &server_addr, addr_len)) < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot bind Server listening "
                      "socket %d (err=%d)\n", listen_sock, rv));
        close(listen_sock);
        atp_sock_server_thread_id = SAL_THREAD_ERROR;
        sal_thread_exit(rv);
    ...
    if ((rv = listen(listen_sock, ATP_SOCK_BACKLOG)) < 0) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot set Server listening mode "
                      "on socket %d (err=%d)\n", listen_sock, rv));
        close(listen_sock);
        atp_sock_server_thread_id = SAL_THREAD_ERROR;
        sal_thread_exit(rv);
    ...
    ATP_SOCK_VERB(("ATP-Socket: Server ready, listening on socket %d\n",
    ...
    ATP_SOCK_VERB(("ATP-Socket: Exit Server Thread '%s' (id = 0x%x), "
                   "close socket %d\n", atp_sock_server_thread_name,
                   atp_sock_server_thread_id, atp_sock_server_lsock));
...
cleanup (void)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Cleanup\n"));
...
init(void)
{
    ...
    if (atp_sock_mlock != NULL) {
        ...
        ATP_SOCK_VVERB(("ATP-Socket: Destroy mutex 0x%x\n", atp_sock_mlock));
        atp_sock_mlock = NULL;
    }
    if ((atp_sock_mlock = sal_mutex_create("ATP_Socket_mlock")) == NULL) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot create mutex\n"));
        return BCM_E_MEMORY;
    }
    
    /* Reset configuration parameters to default values */
    atp_sock_cfg_thread_pri = ATP_SOCK_THREAD_PRI_DEFAULT;
    atp_sock_cfg_lport = ATP_SOCK_SERVER_LPORT_DEFAULT;
...
atptrans_socket_server_start(void)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Start Server\n"));
    ...
    if (ATP_SOCK_SERVER_RUNNING) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_WARN(("ATP-Socket: Server thread is already running\n"));
        return BCM_E_EXISTS;
    }
    ...
    atp_sock_server_thread_id = sal_thread_create(atp_sock_server_thread_name,
                                                  ...
                                                  atp_sock_cfg_thread_pri,
                                                  server_conn_thread, 
                                                  (void *)&atp_sock_cfg_lport);

    if (atp_sock_server_thread_id == SAL_THREAD_ERROR) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot create Server thread\n"));
        return BCM_E_FAIL;
    }

    ATP_SOCK_UNLOCK;

    ATP_SOCK_VVERB(("ATP-Socket: Created Server Server Thread '%s' "
                    "(id = 0x%x)\n",
                    atp_sock_server_thread_name, atp_sock_server_thread_id));
...
atptrans_socket_server_stop(void)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Stop Server\n"));

    ATP_SOCK_LOCK;

    if (!ATP_SOCK_SERVER_RUNNING) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_VERB(("ATP-Socket: Server not running\n"));
        return BCM_E_NONE;
    }
    ...
    if (i >= ATP_SOCK_STOP_RETRIES_MAX) {
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot stop Server Thread in %d ms\n",
 ...
 *     listen_port     - Listening port for connection request
 * Returns:
 *     BCM_E_NONE  Success
 *     BCM_E_XXX   Failure
 * Note:
 ...
 *     3. A negative value will leave the parameter unchanged.
 */

int
atptrans_socket_config_set(int priority, int listen_port)
{
    int rv;
    ...
    ATP_SOCK_VERB(("ATP-Socket: Configuration set to priority %d, "
                   "listen_port %d\n",
                        ...
                        bcm_ip_t dest_ip, uint32 flags)
{
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Install CPU key "
                    CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(dest_cpu_key)));
    ...
    if ((db_ptr = db_entry_create(&atp_sock_db_list,
        ...
        ATP_SOCK_UNLOCK;
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot add new entry for CPU key "
                      CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(dest_cpu_key)));
        return BCM_E_FAIL;
    }
    ...
    ip_to_str(dest_ip_str, db_ptr->ip);
    ATP_SOCK_VERB(("ATP-Socket: Installed on CPU key " CPUDB_KEY_FMT ", '%s'\n",
...
atptrans_socket_uninstall(cpudb_key_t dest_cpu_key)
{
    ...
    if (!ATP_SOCK_INIT_DONE) {
        ATP_SOCK_ERR(("ATP-Socket ERROR:  Not initialized\n"));
        return BCM_E_FAIL;
    }

    ATP_SOCK_VVERB(("ATP-Socket: Uninstall CPU key "
                    CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(dest_cpu_key)));
    ...
    if ((db_ptr = db_cpu_key_lookup(atp_sock_db_list, 
                                    dest_cpu_key)) == NULL) {
        ATP_SOCK_UNLOCK;
        ATP_SOCK_ERR(("ATP-Socket ERROR: Cannot find valid entry for CPU key "
                      CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(dest_cpu_key)));
        return BCM_E_NOT_FOUND;
    }
    ...
    if (BCM_FAILURE(rv)) {
        ATP_SOCK_VERB(("ATP-Socket: Uninstalled failed\n"));
        return BCM_E_FAIL;
    }
...
atptrans_socket_local_cpu_key_set(cpudb_key_t const cpu_key)
{
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Set local CPU key " CPUDB_KEY_FMT "\n",
...
atptrans_socket_show(void)
{
    ...
    if (!ATP_SOCK_INIT_DONE) {
        ATP_SOCK_OUT(("ATP over Sockets not initialized\n"));
        return;
    }

    /* Server */
    ATP_SOCK_OUT(("ATP-Socket Server\n"));
    if (atptrans_socket_server_running()) {
        ATP_SOCK_OUT(("    Listening Port    %d\n", atp_sock_server_lport));
        ATP_SOCK_OUT(("    Listening Socket  %d\n", atp_sock_server_lsock));
        ATP_SOCK_OUT(("    Thread-Name       %s\n",
                      atp_sock_server_thread_name));
        ATP_SOCK_OUT(("    Thread-ID         0x%8.8x\n",
                      atp_sock_server_thread_id));
    } else {
        ATP_SOCK_OUT(("    Not running\n"));
    }
    ATP_SOCK_OUT(("\n"));

    /* Database List */
    ATP_SOCK_OUT(("ATP-Socket Database\n"));
    ATP_SOCK_OUT(("    CPU Key            IP Address       Flags  Socket  "
                  "Thread\n"));
    ATP_SOCK_OUT(("    -----------------  ---------------  -----  ------  "
                  "-----------\n"));
    ATP_SOCK_DB_FOREACH_ENTRY(atp_sock_db_list, db_ptr) {
        ip_to_str(ip_str, db_ptr->ip);
        ATP_SOCK_OUT(("    %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x  %-15s  0x%3.3x",
        ...
        /* Connection */
        if (db_ptr->conn.flags & ATP_SOCK_CONN_VALID) {
            ATP_SOCK_OUT(("  %6d  %-11s (0x%8.8x)",
                          db_ptr->conn.sock_fd,
                          db_ptr->conn.rx_thread_name,
                          db_ptr->conn.rx_thread_id));
        }
