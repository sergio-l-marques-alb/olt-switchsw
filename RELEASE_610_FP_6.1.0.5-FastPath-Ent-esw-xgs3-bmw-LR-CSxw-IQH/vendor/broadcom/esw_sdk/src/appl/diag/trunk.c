/*
 * $Id: trunk.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
_bcm_diag_trunk_show(int unit, args_t *a)
{
    ...
    if (num_tids) {
        ...
        if (num_fp_tids > 0) {
            printk("  %d front panel trunks, %d ports/trunk\n",
                   num_fp_tids, num_fp_ports); 
        }
        if (num_fabric_tids > 0) {
            printk("  %d fabric trunks, %d ports/trunk\n",
                   num_fabric_tids, num_fabric_ports); 
        }
    }
    ...
    for (tid = tid_min; tid <= tid_max; tid++) {
        ...
        printk("trunk %d: (%s, %d ports)", tid, 
               ((tid <= tid_fp_max) ? "front panel" : "fabric"),
