/*
 * $Id: pcktwatch.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
#ifdef GHS
#define d_packet_format(pfx, decode, pkt, count)
#endif
...
typedef struct pup_s {
    ...
    robo_dv_t   *pup_robo_dv;   /* DMA descriptor, ROBO SOC mode only */
...
STATIC void             pw_robo_done_intr(int unit, robo_dv_t *dv);
#endif
 ...
 */
{
	...
	pu->pu_log_free = pup->pup_next;
        if (pu->mode == PW_MODE_SOC) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                
                soc_robo_dma_dv_free(unit, pup->pup_robo_dv);
#endif
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
 ...
 */
{
    ...
    for (i = 0; i < pup_count; i++) {
        ...
        if (pu->mode == PW_MODE_SOC) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                 
                pup->pup_robo_dv = soc_robo_dma_dv_alloc(unit, DV_RX, 1);
 ...
 */
{
...
#endif
#if defined(BCM_ROBO_SUPPORT)
    robo_dcb_t  *robo_dcb = pup->pup_robo_dv->dv_dcb;
#endif
    char        pfx[64];
    ...
    if (report & REPORT_DMA) {
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_robo_dma_dump_dv(unit, pfx, pup->pup_robo_dv);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (report & REPORT_RAW) {
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_robo_dma_dump_pkt(unit, pfx,
    ...
    if (report & REPORT_DECODE) {
        ...
        d_packet_format(pfx, DECODE_ETHER, pup->pup_pkt,
                             SOC_DCB_XFERCOUNT_GET(unit, robo_dcb));
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
            d_packet_format(pfx, DECODE_ETHER, pup->pup_pkt,
...
pw_dump_packet_rx(int unit, pup_t *pup, uint32 report)
{
    ...
    if (report & REPORT_DMA) {
        if (pkt->_dv) {
            if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)                
                soc_robo_dma_dump_dv(unit, pfx, (robo_dv_t *)pkt->_dv);
#endif
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (report & REPORT_RAW) {
        ...
        if (SOC_IS_ROBO(unit)) {
#if defined(BCM_ROBO_SUPPORT)            
            soc_robo_dma_ether_dump(unit, pfx, pkt->_pkt_data.data, pkt->pkt_len, 0);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
 ...
 */
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        rv = soc_robo_dma_desc_add(robo_dv, (sal_vaddr_t)pup->pup_pkt,
                                   PW_PACKET_SIZE);
        if (rv < 0) {
        ...
        rv = soc_robo_dma_start(unit, robo_dv);
        if (rv < 0) {
 ...
 */
STATIC	void
pw_robo_done_intr(int unit, robo_dv_t *dv)
{
    pwu_t	*pu  = &pw_units[unit];
    pup_t	*pup = (pup_t *)dv->dv_pw_pup;
