/*
 * $Id: system.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
# elif defined(IDTRP334) || defined(MBZ) || defined(IDT438) || defined(NSX) || defined(METROCORE)
...
cmd_tcl(int unit, args_t *a)
{
    ...
    *envnext++ = ((sal_boot_flags_get() & BOOT_F_48PORT_BOARD) ?
          "P48=1" : "P48=0");
#ifdef UNIX
    *envnext++ = "UNIX=1";
#else
...
IMPORT int Xmodem(int, char**);
IMPORT void xmhelp(void);

char if_xmodem_usage[] =
    "Usage :\n\t"
    "  xmodem [-rb] [-sb] file\n\t"
    "          use XModem protocol to transfer files to/from console.\n";

cmd_result_t
if_xmodem(int unit, args_t * a)
{
    int i, rv = CMD_OK;
    char* args[3];
    ...
    for ( i = 0; i < 3; i++){
    args[i] = malloc(100);
    memset((void*)args[i], 0, 100);
    }
    if (2 != ARG_CNT(a)) {
#if defined(MOUSSE) || defined(IDTRP334) || \
    defined(MBZ) || defined(IDT438)
    xmhelp();
#endif
    return CMD_FAIL;
    } else {
    strcpy(args[1],ARG_GET(a));
    strcpy(args[2],ARG_GET(a));
    }
#if defined(MOUSSE) || defined(IDTRP334) || \
    defined(MBZ) || defined(IDT438)
    /* Send/Receive file */
    rv =  ( 0 == Xmodem(2, args)) ? CMD_OK : CMD_FAIL;
#endif
    for ( i = 0; i < 3; i++){
    free(args[i]);
    }
...
char shell_mode_usage[] =
  "\n    mode <m> where m is ESW, ROBO or BCMX\n";
#else
char shell_mode_usage[] =
  "\n    mode <m> where m is ESW, ROBO, BCMX or SBX\n";
#endif
...
char cmd_bsafe_usage[] =
    "bsafe <magic-flags>\n\t"
    "   This function is under development.\n";

/*
 * Function:    cmd_bsafe
 * Purpose: Exercise the BroadSAFE uHSM library
 * Returns: CMD_USAGE/CMD_FAIL/CMD_OK.
 */
cmd_result_t
cmd_bsafe(int unit, args_t *a)
{
    char *c;
    int flags, ret;
    extern int test_bsafe_main(int unit, int flags);
    ...
    ret = test_bsafe_main(unit, flags);

    printk("Return code: 0x%x\n", ret);
...
diag_shell(void)
{
            ...
            for (i = 0; i < soc_all_ndev; i++) {
		     ...
		     if ((sbx_soc_file = getenv("SBX_SOC_FILE")) != NULL){
		       ...
		       diag_rc_set(i, "sbx.soc");
                     ...
                     /*
                      * If successful, make first unit the default.
                      * Currently, a string with a carriage return is output
                      * on the same line by the driver attach routine.
                      */
                     if (soc_unit < 0) {
                         soc_unit = i;
                     }
    ...
    if ((script = sal_boot_script()) != NULL) {
        ...
        no_rc_warning = 0;
    } else {    /* Robo units do not use rc.soc */
#if defined(BCM_ROBO_SUPPORT)
        for (i = 0; i < soc_ndev; i++) {
            int rv;

#ifdef ETH_MII_DEBUG
            if (soc_cm_get_dev_type(i) & SOC_ETHER_DEV_TYPE) {
                continue;
            }
#endif
            if ((rv = system_init(i)) < 0) {
                printk("system_init: Failed on error %d\n", rv);
            }
        }

        sh_swap_unit_vars(soc_unit);

        if (soc_unit < 0) {
            printk("No default unit.\n");
        }
#endif
    }
...
system_init(int unit)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (soc_property_get_str(unit, spn_BCM_LINKSCAN_PBMP) == NULL) {
            if (SOC_IS_ROBO(unit) && SOC_IS_ROBO5324(unit)) {
