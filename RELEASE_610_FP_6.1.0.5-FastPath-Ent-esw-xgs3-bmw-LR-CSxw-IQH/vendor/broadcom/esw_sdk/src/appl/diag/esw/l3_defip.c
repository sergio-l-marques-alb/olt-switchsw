/*
 * $Id: l3_defip.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
diag_defip_test(int unit)
{
    ...
    char str[128],cmd;
    ip_addr_t ipaddr;
    int a, b, c, d, mask = 0, nibbles, lpm;
    ...
    while (fscanf(f, "%s", str) > 0) {
        ...
        if (add == 0) {
            sscanf(str, "%d.%d.%d.%d/%d", &a, &b, &c, &d, &mask);
        }else{
            sscanf(str, "%c %d.%d.%d.%d/%d", &cmd, &a, &b, &c, &d, &mask);
        }
        ipaddr = ((a << 24) & 0xFF000000) |
            ((b << 16) & 0xFF0000) | ((c << 8) & 0xFF00) |
            (d & 0xFF);
        ...
        if (add == 0 || cmd == 'a') {
            ...
            /* need vlid data here */
            sal_memset(cfg.defip_mac_addr, 0, sizeof(mac_addr_t)); 
            ...
            soc_cm_debug(DK_L3,
                         "lpm %d.%d.%d.%d for %d.%d.%d.%d subnet_len = %d\n", 
                         (lpm>>24)&0xff,
                         (lpm>>16)&0xff,
                         (lpm>>8)&0xff,
                         (lpm)&0xff,
