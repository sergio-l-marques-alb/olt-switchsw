...
if_esw_l2(int unit, args_t *a)
{
    ...
    arg_cbit = 0, arg_hit = 0, arg_replace = 0, arg_port = 0, arg_newport = 0, 
        arg_group = 0, arg_pending = 0, arg_limitexempt = 0;
	 ...
	 * 1 for each port.
	 */
        DPORT_SOC_PBMP_ITER(unit, pbmp, dport, p) {
	    bcm_l2_addr_t_init(&l2addr, arg_macaddr, arg_vlan);
	...
	hash_select = arg_hash;
        ...
        defined(BCM_TRX_SUPPORT)
	        ...
	        bits = soc_tr_l2x_param_to_key(unit, arg_macaddr, arg_vlan, key);
	        hash_bucket = soc_tr_l2x_hash(unit, hash_select, bits, key);
                if (hash_select2 >= 0) {
                    hash_bucket2 =
                        soc_tr_l2x_hash(unit, hash_select2, bits, key);
                }
            } else
#endif
	...
	printk("Hash[%d] of key "
        ...
        char *cachecmd = NULL;
        char str[16];
        mac_addr_t arg_macaddr, arg_macaddr_mask;
        int arg_setpri = 0, arg_bpdu = 0, arg_mirror = 0;
...
if_esw_bpdu(int unit, args_t *a)
{
    int                 idx, count;
    char 		*subcmd = NULL;
    mac_addr_t   arg_macaddr, maczero, mac;
    int		arg_index = 0;
    int			rv;
    parse_table_t	pt;
