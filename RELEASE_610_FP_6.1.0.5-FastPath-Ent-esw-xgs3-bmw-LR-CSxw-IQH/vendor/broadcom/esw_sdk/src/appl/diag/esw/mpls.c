/*
 * $Id: mpls.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
if_er_mpls(int unit, args_t *a)
{
    ...
    memset(mac, 0, sizeof(mac_addr_t));
    ...
    if (sal_strcasecmp(table, "vpn") == 0) {
        ...
        case MPLS_SHOW:
            ...
            for (i = 0; i < vpn_count; i++) {
                printk("%3d  %4d  %-4d      %2d          %2d         %3d       0x%08x\n",
                       vpnid, vpn_info.vlan, vpn_info.l3_intf, vpn_info.port,
...
_bcm_tr_mpls_cli_vpn_port_add(int unit, args_t * a)
{
    ...
    if (ifl & (BCM_MPLS_PORT_REPLACE | BCM_MPLS_PORT_WITH_ID)) {
        ...
        soc_cm_debug(DK_TESTS, "mpls port=%08x\n", port);
...
_bcm_tr_mpls_cli_tunnel_init_clear(int unit, args_t * a)
{
    ...
    soc_cm_debug(DK_TESTS, "Tunnel init clear intf_id=%d\n", intf_id);
...
_bcm_tr_mpls_cli_expmap_create(int unit, args_t * a)
{
    ...
    PT_PARSE(a, pt);
    flags |= BCM_MPLS_WITH_ID;
    soc_cm_debug(DK_TESTS, "exp_map_id %d\n", exp_map_id);
    PT_DONE(pt);
...
_bcm_tr_mpls_cli_expmap_set(int unit, args_t * a)
{
    ...
    map.pkt_pri = pkt_pri;
    map.pkt_cfi = pkt_cfi;
    soc_cm_debug(DK_TESTS, "exp_map_id %d priority %d exp %d\n"
...
_bcm_tr_mpls_cli_expmap_show(int unit, args_t * a)
{
    ...
    MPLS_CMD("ing") {
        ...
        soc_cm_debug(DK_TESTS, "exp_map_id %d priority %d Color %d\n"
