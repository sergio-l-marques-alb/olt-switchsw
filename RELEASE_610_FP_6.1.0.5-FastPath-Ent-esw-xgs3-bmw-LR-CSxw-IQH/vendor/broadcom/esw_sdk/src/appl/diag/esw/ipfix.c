/*
 * $Id: ipfix.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
cmd_ipfix(int unit, args_t *arg)
{
    ...
    if (!ARG_CNT(arg)) {                  /* Nothing passed */
        ...
        printk("Number of records received: %d\n", ipfix_report_count[unit]);
        return(CMD_OK);
    }
    ...
    if (!sal_strcasecmp(subcmd, "start")) {
        if (ipfix_is_running[unit]) {
            printk("%s: IPFIX thread already running.\n", ARG_CMD(arg));
            return CMD_OK;
        }
        ...
        if (!ipfix_is_running[unit]) {
            printk("%s: ERROR: IPFIX thread already stopped.\n", ARG_CMD(arg));
            return CMD_FAIL;
        }
        ...
        SAL_IP6_ADDR_TO_UINT32(config.src_ip6_mask, src_ip6_mask);
        SAL_IP6_ADDR_TO_UINT32(config.dst_ip6_mask, dst_ip6_mask);
        SAL_IP6_ADDR_TO_UINT32(config.tunnel_src_ip6_mask,
                               tunnel_src_ip6_mask);
        SAL_IP6_ADDR_TO_UINT32(config.tunnel_dst_ip6_mask,
                               tunnel_dst_ip6_mask);
        ...
        if (ARG_CNT(arg)) {
            ...
            printk("IPFIX Reporting on for: ");
            parse_mask_format(50, ipfix_report_table, ipfix_report[unit]);
            printk("IPFIX Reporting off for: ");
            parse_mask_format(50, ipfix_report_table, ~ipfix_report[unit]);
        }
    } else {
        return CMD_USAGE;
