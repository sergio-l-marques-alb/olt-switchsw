/*
 * $Id: port.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
char            *robo_interface_mode[] = {
    "NONE", "NULL", "MII", "GMII", "SGMII", "TBI", "XGMII", "RGMII",NULL
};
...
if_robo_port_stat(int unit, args_t *a)
{
    ...
    if (!(bcm_init_check(unit) & BCM_INIT_SEL_PORT)) {
    ...
    PBMP_ITER(pbm, p) {
        if (operation & SHOW) {
            ...
            if (operation & INGRESS ) {
                ...
                if (rv < 0) {
                    printk("%4s:", SOC_PORT_NAME(unit, p));
                    printk("\tIngress meter:  %s.\n",bcm_errmsg(rv));
                    continue;
                }
                if (rate) {
                    printk("%4s:", SOC_PORT_NAME(unit, p));
                    ...
                    printk("%4s:", SOC_PORT_NAME(unit, p));
                    header = 1;
                    printk("\tIngress meter:  value not set yet.\n");
                }
            }
            ...
            if (operation & EGRESS) { 
                ...
                if (rv < 0) {
                    ...
                    printk("\tEgress meter:  value not set yet.\n");
                    }
               } else {
                    rv = bcm_port_rate_egress_get(unit, p, &rate, &burst);
                 if (rv < 0) {
            ...
            if (operation & PER_QUEUE_EGRESS) { 
                if (queue_n >= 0) {
                    rv = bcm_cosq_port_bandwidth_get(unit, p, queue_n, &rate, &burst, 0);
                    if (rv < 0) {
                       printk("\tQueue #%d Egress meter:  %s.\n",queue_n, bcm_errmsg(rv));
                       continue;
                    }
                    if (rate) { 
                        if (!header) {
                        ...
                        printk("\tQueue #%d Egress meter:  %8d kbps %8d kbits max burst.\n",
                          queue_n, rate, burst);
                    } else {
                        if (!header) {
                          printk("%4s:", SOC_PORT_NAME(unit, p));
                        }
                        printk("\tQueue #%d Egress meter:  value not set yet.\n", queue_n);
                    }
                } else {
                    int i, current_q;
                    rv = bcm_cosq_config_get(unit, &current_q);
                    if (rv < 0) {
                       printk("\tPer Queue Egress meter:  %s.\n",bcm_errmsg(rv));
                       continue;
                    }

                    for (i = 0; i < current_q; i++) {
                        rv = bcm_cosq_port_bandwidth_get(unit, p, i, &rate, &burst, 0);
                        if (rv < 0) {
                           printk("\tQueue #%d Egress meter:  %s.\n",i,bcm_errmsg(rv));
                           continue;
                        }
            ...
            if (operation & INGRESS) {
                ...
                if (rv < 0) {
                    printk("%s: Error: bcm_port_rate_ingress_set() failed: %s\n",
                ...
                if (rv < 0) {
                    printk("%s: Error: bcm_port_rate_egress_set() failed: %s\n",
                       ARG_CMD(a), bcm_errmsg(rv));
                    return CMD_OK;
                }
...
#define SET     0x02
    ...
    PBMP_ITER(pbm, soc_port) {
        ...
        /* Show port sflow sample rate(s) */
            ...
            printk("%4s:", SOC_PORT_NAME(unit, soc_port));
               ...
               uint32 flags)
{
    ...
    printk(" %c%s ", info->linkstatus ? '*' : ' ', port_ref);
    ...
    if (flags & BCM_PORT_ATTR_LINKSCAN_MASK) {
        if (info->linkscan) {
            printk("LS(%s) ", robo_linkscan_mode[info->linkscan]);
        }
    }

    if (hg_port) {
        printk("%s/XAUI(", robo_encap_mode[info->encap_mode & 0x3]);
    ...
    if (!no_an_props) {
        if (flags & BCM_PORT_ATTR_SPEED_MASK) {
            printk("%s", robo_if_fmt_speed(speed_buf, info->speed));
        }
        if (flags & BCM_PORT_ATTR_DUPLEX_MASK) {
            printk("%s", info->duplex ? "FD" : "HD");
        }
        if (flags & BCM_PORT_ATTR_PAUSE_MASK) {
            printk("%s%s", info->pause_tx ? "+TXpau" : "",
                   info->pause_rx ? "+RXpau" : "");
        }
        printk(") ");
    }
    ...
    if (flags & BCM_PORT_ATTR_STP_STATE_MASK) {
        printk("STP(%s) ", robo_forward_mode[info->stp_state]);
    }
    ...
    if (flags & BCM_PORT_ATTR_INTERFACE_MASK) {
        if (info->interface >= 0 && info->interface < SOC_PORT_IF_COUNT) {
            printk("IF(%s) ", robo_interface_mode[info->interface]);
        }
    }
    ...
    if (flags & BCM_PORT_ATTR_PHY_MASTER_MASK) {
        ...
        info->phy_master != SOC_PORT_MS_NONE) {
            printk("PH(%s) ", robo_phymaster_mode[info->phy_master]);
        }
    }
...
if_robo_port(int u, args_t *a)
{
    ...
    bcm_port_medium_t ge_port_medium;
    ...
    if (!(bcm_init_check(u) & BCM_INIT_SEL_PORT)) {
        printk("%s: Error: bcm ports not initialized\n", ARG_CMD(a));
        return CMD_FAIL;
    }
...
#define _call_pdi(u, i, p, mp, s) \
    ...
    PBMP_ITER(pbm, p) {
        ...
        /* If fiber mode, set speed = 1000 and Full-duplex fixed*/
        if (IS_GE_PORT(u, p)) {
            bcm_port_medium_get(u, p, &ge_port_medium);

            if (ge_port_medium == BCM_PORT_MEDIUM_FIBER) {
                info_all[p].medium = BCM_PORT_MEDIUM_FIBER;
                info_all[p].speed = 1000;
                info_all[p].speed_max = 1000;
                info_all[p].duplex = BCM_PORT_DUPLEX_FULL;
            }
            else {
                info_all[p].medium = BCM_PORT_MEDIUM_COPPER;
           }
        }
...
if_robo_dtag(int unit, args_t *a)
{
    ...
    PBMP_ITER(pbmp, port) {
        ...
        printk("port %d:%s\tdouble tag mode %s, tpid 0x%x\n",
    ...
    if (c == NULL) {
        ...
        printk("port %d:%s\tdouble tag mode %s\n",
    ...
    PBMP_ITER(pbm, port) {
        ...
        if (cd.fuzz_len == 0) {
            printk("port %s: cable %s (%d pairs)\n",
               SOC_PORT_NAME(unit, port), statename[cd.state],
               cd.npairs);
        } else {
            printk("port %s: cable %s (%d pairs, length +/- %d metres)\n",
               SOC_PORT_NAME(unit, port), statename[cd.state],
               cd.npairs, cd.fuzz_len);
        }
        for (i = 0; i < cd.npairs; i++) {
            printk("\tpair %c %s, length %d metres\n",
                   'A' + i, statename[cd.pair_state[i]], cd.pair_len[i]);
        }
    }

    if ((rv = bcm_linkscan_mode_set_pbm(unit, pbm, BCM_LINKSCAN_MODE_SW)) < 0) {
        printk("%s: Failed to recover link scanning: PBM=%s: %s\n",
               ARG_CMD(a), SOC_PBMP_FMT(pbm, pfmt), bcm_errmsg(rv));
        return(CMD_FAIL);
    }
    return CMD_OK;
...
if_robo_ipg(int unit, args_t *a)
{
    ...
    PBMP_ITER(arg_pbmp, port) {
        printk("%-8.8s", SOC_PORT_NAME(unit, port));
...
if_robo_linkscan(int unit, args_t *a)
{
    ...
    if (ARG_CNT(a) == 1 &&
    sal_strcasecmp(_ARG_CUR(a), "off") == 0 &&
    !(bcm_init_check(unit) & BCM_INIT_SEL_LINKSCAN)) {
        (void) ARG_GET(a);
        return(CMD_OK);
    }
    ...
    if (ARG_CNT(a) == 0) {  /*  show information for all registers */
        PBMP_ITER(pbm, p) {
            ...
            if (intermediate) {
                ...
                SOC_IF_ERROR_RETURN(bcm_port_phy_drv_name_get(u, p, drv_name, 64));
            ...
            if ((!intermediate) && (soc_phy_is_c45_miim(u, p))) {
                for(i = 0; i < sizeof(p_devad)/sizeof(p_devad[0]); i++) {
                    ...
                    for (phy_reg = PHY_MIN_REG; phy_reg <= PHY_MAX_REG; \
                        ...
                        if (rv < 0) {
                            printk("\n");
                            printk("ERROR: Port %s: soc_miim_read failed: %s\n",
        ...
        if (ARG_CNT(a) == 0) {  /* no more args; show this register */
            PBMP_ITER(pbm, p) {
                ...
                if ((!intermediate) && (soc_phy_is_c45_miim(u, p))) {
                    for(i = 0; i < sizeof(p_devad)/sizeof(p_devad[0]); i++) {
                        phy_devad = p_devad[i];
                        printk("Port %s (PHY addr 0x%02x) DevAd %d(%s) Reg %d:",
...
if_robo_dscp(int unit, args_t *a)
{
    ...
    if ((s = ARG_GET(a)) == NULL) {
        PBMP_ITER(pbm, port) {
            printk("%d:%s dscp map:", unit, SOC_PORT_NAME(unit, port));
            for (srccp = 0; srccp < 63; srccp++) {
                rv = bcm_port_dscp_map_get(unit, port, srccp, &mapcp, 0);
                if (rv < 0) {
    ...
    PBMP_ITER(pbm, port) {
        rv = bcm_port_dscp_map_set(unit, port, srccp, mapcp, 0);
        if (rv < 0) {
            printk("%d:%s ERROR: dscp map set %d=%d failed: %s\n",
               unit, SOC_PORT_NAME(unit, port),
               srccp, mapcp, bcm_errmsg(rv));
            return CMD_FAIL;
        }
    }
    return CMD_OK;
...
if_stat(int unit, args_t* a)
{
    ...
    if (f_parse_fail) {
        printk("%s: Error: no specified port number.\n",ARG_CMD(a));
        return CMD_FAIL;
    }
...
cmd_result_t
if_robo_igmp(int unit, args_t * args)
{
    char *c;
    int rv;
    int enable = 0;
    ...
    if (ARG_CUR(args) != NULL) {
        ...
        if (!sal_strcasecmp(c, "init")) {
            if ((rv = bcm_igmp_snooping_init(unit)) < 0 ) {
                printk("bcm_igmp_snooping_init failed ERROR: %s\n",
                    bcm_errmsg(rv));
                return CMD_FAIL;
            }
            ...
            !sal_strcasecmp(c, "disable") ||
            !sal_strcasecmp(c, "no")) {
            if ((rv = bcm_igmp_snooping_enable_set(unit, 0)) < 0) {
                printk("bcm_igmp_snooping_enable_set failed ERROR: %s\n",
                    bcm_errmsg(rv));
                return CMD_FAIL;
            }
        } else if (!sal_strcasecmp(c, "on") ||
               !sal_strcasecmp(c, "enable") ||
               !sal_strcasecmp(c, "yes")) {
            if ((rv = bcm_igmp_snooping_enable_set(unit, 1)) < 0) {
                printk("bcm_igmp_snooping_enable_set failed ERROR: %s\n",
                    bcm_errmsg(rv));
                return CMD_FAIL;
            }
        ...
        if ((rv = bcm_igmp_snooping_enable_get(unit, &enable)) < 0) {
            printk("bcm_igmp_snooping_enable_get failed ERROR: %s\n",
                bcm_errmsg(rv));
            return CMD_FAIL;
        }
        if (enable) {
            printk("IGMP Snoop is enabled\n");
        } else {
            printk("IGMP Snoop is disabled\n");
        }
    }
...
if_robo_imp_protect(int unit, args_t * args)
{
    ...
    if (ARG_CUR(args) != NULL) {
        ...
        if ((min_rate > mid_rate) || (min_rate > max_rate)) {
            printk("The Min rate is larger than Midlle rate or Max rate.\n");
            return CMD_FAIL;
        }
        if (mid_rate > max_rate) {
            printk("The Middle rate is larger than Max rate.\n");
            return CMD_FAIL;
        }

        rv = soc_imp_prot_set(unit, min_rate, mid_rate, max_rate);
        if (rv < 0) {
            printk("soc_imp_prot_set : failed! \n");
            return CMD_FAIL;
        }
    } else {
...
char if_dos_attack_usage[] =
    ...
    "Usage :\n\t"
    "   dos [pbm] enable [0|1|2|...|14]\n\t"
    "   dos [pbm] disable [0|1|2|...|14]\n\t"
    "   dos [pbm] show\n\t"
    ...
    if (BCM_PBMP_EQ(pbm, all_pbm)) {
        if (!sal_strcasecmp(c, "show")) {
            ...
            switch (x) {
                ...
                case 14:
                    ...
                    break;
                default:
                    soc_cm_print("Case Number Invalid\n");
                    return CMD_FAIL;
                
            }

            /* Enable DoS Attack Function if not enabled */
            type = bcmSwitchDosAttackV4FirstFrag;
            rv = bcm_switch_control_get(unit, type, &enable);
            if (rv < 0) {
                soc_cm_print("SET ENABLE FAIL: rv = %s\n",soc_errmsg(rv));
                return CMD_FAIL;
            }
            if (!enable) {
            ...
            if (rv < 0) {
                soc_cm_print("SET DISABLE FAIL: rv = %s\n",soc_errmsg(rv));
                return CMD_FAIL;
            }
        }  else {
            return CMD_USAGE;
        ...
        if (!sal_strcasecmp(c, "show")) {
            ...
            PBMP_ITER(pbm, port) {
                ...
                if (!enable) {
                    rv = bcm_switch_control_port_set(unit, port, type, 1);
                    if (rv < 0) {
                        soc_cm_print("SET ENABLE FAIL: rv = %s\n",soc_errmsg(rv));
                        return CMD_FAIL;
                    }
...
#define PM_GET_NUMB(numb, str, args) \
    if (((str) = ARG_GET(args)) == NULL) { \
        return CMD_USAGE; \
    } \
    (numb) = parse_integer(str);
...
cmd_result_t
cmd_robo_pm(int unit, args_t *args)
{
    char*                       subcmd = NULL;
    int port, power_mode;
    bcm_pbmp_t  pbm;
    ...
    if(!sal_strcasecmp(subcmd, "show")) {
        PBMP_E_ITER(unit, port) {
            ...
            bcm_robo_port_phy_power_mode_get(unit, port, &power_mode);
            switch(power_mode) {
                case PHY_HIGH_PERFORMANCE_MODE:
                    soc_cm_print("Link is up. PHY Power Mode of port %d is High Performance Mode.\n", 
                        port);
                    break;
                case PHY_LOW_POWER_AB0_MODE:
                    soc_cm_print("Link is up. PHY Power Mode of port %d is Low Power AB0 Mode.\n", 
                        port);
                    break;
                case PHY_LOW_POWER_AB6_MODE:
                    soc_cm_print("Link is up. PHY Power Mode of port %d is Low Power AB6 Mode.\n", 
                        port);
                    break;
                case PHY_GREEN_POWER_MODE:
                    soc_cm_print("Link is up. PHY Power Mode of port %d is Green Mode.\n", port);
                    break;
                case PHY_AUTO_POWER_DOWN_MODE:
                    soc_cm_print("Link is down. PHY Power Mode of port %d is Auto Power-Down Mode.\n", 
                        port);
                    break;
                case PHY_STANDBY_POWER_DOWN_MODE:
                    soc_cm_print("Link is down. PHY Power Mode of port %d is Standby Power-Down Mode.\n", 
                        port);
                    break;
                case PHY_NON_STANDBY_POWER_DOWN_MODE:
                    soc_cm_print("Link is down. PHY Power Mode of port %d is non-Standby Power-Down Mode.\n", 
                        port);
                    break;
                default:
                    soc_cm_print("Port %d's PHY Power Mode is Unknown.\n", port);
                    break;
            }
        }
        return CMD_OK;
    ...
    if(!sal_strcasecmp(subcmd, "set")) {
        subcmd = ARG_GET(args);
        if (parse_pbmp(unit, subcmd, &pbm) < 0) {
            printk("%s: ERROR: unrecognized port bitmap: %s\n",
               ARG_CMD(args), subcmd);
            return CMD_FAIL;
        }
        PM_GET_NUMB(power_mode, subcmd, args);
        PBMP_ITER(pbm, port) {
            if ((power_mode < PHY_HIGH_PERFORMANCE_MODE) || 
                (power_mode > PHY_NON_STANDBY_POWER_DOWN_MODE)) {
                return CMD_USAGE;
            }
            bcm_robo_port_phy_power_mode_set(unit, port, power_mode);
        }
        return CMD_OK;;
    }
