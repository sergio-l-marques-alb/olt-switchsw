/*
 * $Id: decode.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_SUPPORT)
static decode_t d_oam_lm_table;		/* OAM LM Decode */
static decode_t d_oam_dm_table;		/* OAM DM Decode */
#endif
...
static char *
d_decode(const char *pfx, char *s, decode_t *decode, void *hdr, int l)
/*
 * Function: 	d_decode
 ...
 */
{
	...
	if (dl->dl_decode) {		/* Next level if there */
	    s = d_decode(pfx, s, dl->dl_decode, hdr, l);
	}
...
char *
d_ether_decode(const char *pfx, char *s, void *ehp, int el, decode_list_t *dl)
/*
...
char *
d_arp_decode(const char *pfx, char *s, void *ahp, int el, decode_list_t *dl)
/*
...
char *
d_oam_decode(const char *pfx, char *s, void *ahp, int el, decode_list_t *dl)
/*
 * Function:	p_arp_decode
 * Purpose: 	Format arp header into string.
 * Parameters:	pfx - output prefix
...
char *
d_oam_lm_decode(const char *pfx, char *s, void *ahp, int el, decode_list_t *dl)
/*
 * Function:	p_arp_decode
 * Purpose: 	Format arp header into string.
 * Parameters:	pfx - output prefix
...
char *
d_oam_dm_decode(const char *pfx, char *s, void *ahp, int el, decode_list_t *dl)
/*
 * Function:	p_arp_decode
 * Purpose: 	Format arp header into string.
 * Parameters:	pfx - output prefix
...
static decode_list_t	d_oam_type[] = {
    {0,  41, 	NULL, 	"Undefined"},
    {42, 42, &d_oam_lm_table, "LMR"},
    {43, 43, &d_oam_lm_table, "LMM"},
    {46, 46, &d_oam_dm_table, "DMR"},
    {47, 47, &d_oam_dm_table, "DMM"},
...
static decode_t d_oam_lm_table = {	/* OAM LMM/LMR */
    "LM", 
    d_oam_lm_decode,
    NULL,
    NULL, 
...
static decode_t d_oam_dm_table = { /* OAM DMM/DMR */
    "DM", 
    d_oam_dm_decode, 
    NULL,
    NULL, 
...
char *
d_ip_decode(const char *pfx, char *s, void *ihp, int il, decode_list_t *dl)
/*
 ...
 */
{
#if defined(JUPITER) || defined(IDTRP334)
    strcpy(s, "***NOT IMPL***");
#else
    ...
    s = d_vformat_dec(s, (unsigned char *)&ih->ip_src, sizeof(ih->ip_src), 
		      1, '.');
    s = d_strcat(s, "> dst<");
    s = d_vformat_dec(s, (unsigned char *)&ih->ip_dst, sizeof(ih->ip_src), 
		      1, '.');
...
char *
d_icmp_decode(const char *pfx, char *s, void *icp, int il, decode_list_t *dl)
/*
...
static	char	*
d_tcp_decode(const char *pfx, char *s, void *tcpp, int tl, decode_list_t *dl)
/*
...
#if defined(JUPITER) || defined(IDTRP334)
    s = d_strcat_dec(s, "Off<", 999, "> Flags<");
#else
...
int
d_packet_vformat(char *pfx, char *s, int type, void *p, int pl)
/*
 ...
 */
{
    switch(type) {
    case DECODE_ETHER:
	(void)d_decode(pfx, s, &d_ether_table, p, pl);
	break;
    case DECODE_IP:
	(void)d_decode(pfx, s, &d_ip_table, p, pl);
	break;
    case DECODE_TCP:
	(void)d_decode(pfx, s, &d_tcp_table, p, pl);
	break;
    case DECODE_ARP:
	(void)d_decode(pfx, s, &d_arp_table, p, pl);
	break;
...
int
d_packet_format(char *pfx, int type, void *p, int pl)
/*
 ...
 */
{
	...
	*s = '\0';
	ts = d_strcat(s, pfx);
	rv = d_packet_vformat(pfx, ts, type, p, pl);
	if (0 == rv) {
	    printk("%s\n", s);
	}
	free(s);
