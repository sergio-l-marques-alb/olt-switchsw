/*
 * $Id: field.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
static cmd_result_t counter_cmd(int unit, args_t *args);
static cmd_result_t counter_create_cmd(int unit, args_t *args);
static cmd_result_t counter_destroy_cmd(int unit, args_t *args);
static cmd_result_t counter_set_cmd(int unit, args_t *args);
static cmd_result_t counter_share_cmd(int unit, args_t *args);
static cmd_result_t counter_show_cmd(int unit, args_t *args);
...
static cmd_result_t meter_cmd(int unit, args_t *args);
static cmd_result_t meter_create_cmd(int unit, args_t *args);
static cmd_result_t meter_destroy_cmd(int unit, args_t *args);
static cmd_result_t meter_set_cmd(int unit, args_t *args);
static cmd_result_t meter_share_cmd(int unit, args_t *args);
static cmd_result_t meter_show_cmd(int unit, args_t *args);
...
static cmd_t field_cmd_list[] = {
    ...
    {"Counter", counter_cmd,
     "field counter {create | destroy | set | share | show} <args>",
     "Manage a field processor counter."},
    ...
    {"Meter", meter_cmd,
     "field meter {create | destroy | set | share | show} <args>",
     "Manage a field processor meter."},
...
static cmd_t field_counter_cmd_list[] = {
    {"Create", counter_create_cmd, 
     "field counter create <entry-id>", 
     "Create a counter and associate it with a field entry."},

    {"Destroy", counter_destroy_cmd, 
     "field counter destroy <entry-id>", 
     "Remove a counter association with a field entry."},

    {"Set", counter_set_cmd, 
     "field counter set <entry-id> <counter-num> Value=<value>", 
     "Set the value for a field counter."},

    {"SHare", counter_share_cmd, 
     "field counter share <src-entry-id> <dst-entry-id>", 
     "Share the counter of a field entry with another entry."},

    {"SHOW", counter_show_cmd, 
     "field counter show <entry-id> <counter-num>", 
     "Display the value for a field counter."}
};
...
static cmd_t field_meter_cmd_list[] = {
    {"Create", meter_create_cmd, 
     "field meter create <entry-id>", 
     "Create a meter and associate with a field entry."},

    {"Destroy", meter_destroy_cmd, 
     "field meter destroy <entry-id>", 
     "Remove a meter association with a field entry."},

    {"Set", meter_set_cmd, 
     "field meter set <entry-id> <meter-num> Rate=<kbits/sec> Burst=<kbits>", 
     "Set the rate parameters for a field meter."},

    {"SHare", meter_share_cmd, 
     "field meter share <src-entry-id> <dst-entry-id>", 
     "Share the meter of a field entry with another entry."},

    {"SHOW", meter_show_cmd, 
     "field meter show <entry-id> <meter-num>", 
     "Display the rate parameters for a field meter."}
};
...
static cmd_result_t
counter_cmd(int unit, args_t *args)
{
    return subcommand_execute(unit, args, 
                              field_counter_cmd_list, 
                              COUNTOF(field_counter_cmd_list));
}

static cmd_result_t
counter_create_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Counter create", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    rv = bcmx_field_counter_create(entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Counter create failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
counter_destroy_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Counter destroy", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    rv = bcmx_field_counter_destroy(entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Counter destroy failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
counter_set_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Counter set", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }
    ...
    if (get_integer(cmd, "Counter set", 
                    ARG_GET(args), &counter, "counter number",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }
    ...
    /* Parse option */
    parse_table_init(0, &pt);
    parse_table_add(&pt, "Value",  PQ_DFL | PQ_STRING,
                    0, &str_value,  NULL);
    ...
    if (str_value == NULL) {
        sal_printf("%s: Need value\n", cmd);
        return CMD_FAIL;
    }

    value = parse_uint64(str_value);

    parse_arg_eq_done(&pt);   /* No longer needed */

    rv = bcmx_field_counter_set(entry, counter, value);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Counter set failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
counter_share_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    src_entry, dst_entry;

    cmd = ARG_CMD(args);

    if (get_integer(cmd, "Counter share", 
                    ARG_GET(args), &src_entry, "source entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    if (get_integer(cmd, "Counter share", 
                    ARG_GET(args), &dst_entry, "destination entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    rv = bcmx_field_counter_share(src_entry, dst_entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Counter share failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
counter_show_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Counter show", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }
    ...
    rv = bcmx_field_counter_get(entry, counter, &value);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Counter show failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
  
    format_uint64(value_str, value);
    sal_printf("%s: entry=%d(0x%x), counter=%d, value=%s\n",
               cmd, entry, entry, counter, value_str);
...
entry_qualify_cmd(int unit, args_t *args)
{
    ...
    case bcmFieldQualifyInnerVlan:
        ...
        if (qid == bcmFieldQualifySrcModid) {
            rv = bcmx_field_qualify_SrcModid(entry, data, mask);
        } else if (qid == bcmFieldQualifyDstModid) {
            rv = bcmx_field_qualify_DstModid(entry, data, mask);
        } else if (qid == bcmFieldQualifyEtherType) {
        ...
        } else if (qid == bcmFieldQualifyLookupStatus) {
            rv = bcmx_field_qualify_LookupStatus(entry, data, mask);
        ...
        } else if (qid == bcmFieldQualifyPacketFormat) {
            rv = bcmx_field_qualify_PacketFormat(entry, data, mask);
...
static cmd_result_t
meter_cmd(int unit, args_t *args)
{
    return subcommand_execute(unit, args, 
                              field_meter_cmd_list, 
                              COUNTOF(field_meter_cmd_list));
}

static cmd_result_t
meter_create_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Meter create", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    rv = bcmx_field_meter_create(entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Meter create failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
meter_destroy_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    if (get_integer(cmd, "Meter destroy", 
                    ARG_GET(args), &entry, "entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }

    rv = bcmx_field_meter_destroy(entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Meter destroy failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
meter_set_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    cmd_result_t         ret_code;
    ...
    if (get_integer(cmd, "Meter set", 
                    ARG_GET(args), &meter, "meter number",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }
    ...
    parse_table_add(&pt,"Burst", PQ_DFL | PQ_INT, 0, &burst, NULL);
    if (!parseEndOk(args, &pt, &ret_code)) {
        return ret_code;
    }
    
    rv = bcmx_field_meter_set(entry, meter, rate, burst);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Meter set failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }

    sal_printf("%s: entry=%d(0x%x), meter=%d, rate=%d kbits/sec, "
               "burst=%d kbits\n",
               cmd, entry, entry, meter, rate, burst);

    return CMD_OK;
}

static cmd_result_t
meter_share_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    src_entry, dst_entry;
    ...
    if (get_integer(cmd, "Meter share", 
                    ARG_GET(args), &src_entry, "source entry id",
                    FALSE, 0, 0) != CMD_OK) {
        return CMD_FAIL;
    }
    ...
    rv = bcmx_field_meter_share(src_entry, dst_entry);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Meter share failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
...
static cmd_result_t
meter_show_cmd(int unit, args_t *args)
{
    int                  rv;
    char                 *cmd;
    bcm_field_entry_t    entry;
    ...
    rv = bcmx_field_meter_get(entry, meter, &rate, &burst);

    if (BCM_FAILURE(rv)) {
        sal_printf("%s: Meter show failed: %s\n", cmd, bcm_errmsg(rv));
        return CMD_FAIL;
    }
