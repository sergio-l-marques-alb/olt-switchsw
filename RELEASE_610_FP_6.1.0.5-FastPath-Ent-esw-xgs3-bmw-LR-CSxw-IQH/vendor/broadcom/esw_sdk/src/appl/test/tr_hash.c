/*
 * $Id: tr_hash.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
    ...
    for (ix = 0; ix < iterations; ix++) {
        for (bank = 0; bank < bank_count; bank++) {
            ...
            if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                ...
                num_bits = soc_tr2_vlan_xlate_base_entry_to_key
                               (unit, (uint32 *) &entry, key);
                soft_bucket = soc_tr2_vlan_xlate_hash(unit, hash, num_bits, 
                                                     (uint32 *) &entry, key);
            } else 
#endif    
            {
            ...
            if (dual == TRUE) {
                ...
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    SOC_IS_VALKYRIE2(unit)) {
                    soft_bucket = soc_tr2_vlan_xlate_bank_entry_hash
        ...
        for (ix = 0; ix < bucket_size; ix++) {
            ...
            if (ix == 0) {
                ...
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    ...
                    bucket = soc_tr2_vlan_xlate_hash(unit, hash, num_bits, 
...
#endif
                {
                    num_bits = soc_tr_vlan_xlate_base_entry_to_key
                                   (unit, (uint32 *) &(entry_tmp[ix]), key);
                    bucket = soc_tr_vlan_xlate_hash(unit, hash, num_bits, 
                                   (uint32 *) &(entry_tmp[ix]), key);
                }
    ...
    for (ix = 0; ix < iterations; ix++) {
        for (bank = 0; bank < bank_count; bank++) {
            ...
            if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                ...
                num_bits = soc_tr2_egr_vlan_xlate_base_entry_to_key
                               (unit, (uint32 *) &entry, key);
                soft_bucket = soc_tr2_egr_vlan_xlate_hash(unit, hash, num_bits, key);
            } else 
#endif    
            {
                num_bits = soc_tr_egr_vlan_xlate_base_entry_to_key
                               (unit, (uint32 *) &entry, key);
                soft_bucket = soc_tr_egr_vlan_xlate_hash(unit, hash, num_bits, key);
            }
            banks = 0;
            if (dual == TRUE) {
#ifdef BCM_TRIUMPH2_SUPPORT
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    SOC_IS_VALKYRIE2(unit) || SOC_IS_ENDURO(unit)) {
                    soft_bucket = soc_tr2_egr_vlan_xlate_bank_entry_hash
                                      (unit, bank, (uint32 *) &entry);
                } else
#endif
                {
        ...
        for (ix = 0; ix < bucket_size; ix++) {
            ...
            if (ix == 0) {
                ...
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    ...
                    num_bits = soc_tr2_egr_vlan_xlate_base_entry_to_key
                                   (unit, (uint32 *) &(entry_tmp[ix]), key);
                    bucket = soc_tr2_egr_vlan_xlate_hash(unit, hash, num_bits, key);
                } else
#endif
                {
                    num_bits = soc_tr_egr_vlan_xlate_base_entry_to_key
                                   (unit, (uint32 *) &(entry_tmp[ix]), key);
                    bucket = soc_tr_egr_vlan_xlate_hash(unit, hash, num_bits, key);
                }
                if (ad->opt_verbose) {
                    printk("Filling bucket %d\n", bucket);
                }
...
tr_mpls_test_hash(int unit, args_t *a, void *p)
{
    ...
    for (ix = 0; ix < iterations; ix++) {
        for (bank = 0; bank < bank_count; bank++) {
            ...
            if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                SOC_IS_VALKYRIE2(unit) || SOC_IS_ENDURO(unit)) {
                num_bits = soc_tr2_mpls_base_entry_to_key(unit, (uint32 *) &entry, key);
                soft_bucket = soc_tr2_mpls_hash(unit, hash, num_bits, 
                                               (uint32 *) &entry, key);
            } else
#endif
            {    
                num_bits = soc_tr_mpls_base_entry_to_key(unit, (uint32 *) &entry, key);
            ...
            if (dual == TRUE) {
                ...
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    SOC_IS_VALKYRIE2(unit) || SOC_IS_ENDURO(unit)) {
                    soft_bucket = soc_tr2_mpls_bank_entry_hash
                                      (unit, bank, (uint32 *) &entry);
                } else
#endif
                {
    ...
    while (iter--) {
        for (ix = 0; ix < bucket_size; ix++) {
            ...
            if (ix == 0) {
                ...
                if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    SOC_IS_VALKYRIE2(unit)) {
                    num_bits = soc_tr2_mpls_base_entry_to_key
                                   (unit, (uint32 *) &(entry_tmp[ix]), key);
                    bucket = soc_tr2_mpls_hash(unit, hash, num_bits, 
                                              (uint32 *) &(entry_tmp[ix]), key);
                } else
#endif
                {
