/*
 * $Id: bench.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
#define BENCH_BUFDMA_SIZE	0x100000
...
benchmark_test(int unit, args_t *a, void *pa)
{
    ...
    mac_addr_t		mac = { 0x00, 0x01, 0x10, 0x55, 0x66, 0x77 };
    mac_addr_t		maclkup = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
	...
	if (benchmark_begin(b, "DMA L2 table", "xfer",
		...
		soc_mem_read_range(unit, L2_ENTRY_INTERNALm, MEM_BLOCK_ANY,
	...
	*ip_ptr[0] = *ip_ptr[1] = 0xc0a80000; /* Delete in insert order */
	...
	if (benchmark_begin(b, "DMA L2 table", "xfer",
			    iter_dma)) {
	    for (i = 0; i < iter_dma; i++)
		soc_mem_read_range(unit, L2Xm, MEM_BLOCK_ANY,
	...
	benchmark_end(b);
#if defined(BCM_XGS3_SWITCH_SUPPORT) && defined(INCLUDE_L3)
    if (SOC_IS_FBX(unit) && !SOC_IS_HAWKEYE(unit) && SOC_MEM_IS_VALID(unit, L3_ENTRY_ONLYm) && (soc_mem_index_count(unit,L3_ENTRY_ONLYm) > 0)) {
        ...
        if (benchmark_begin(b, "Dma read L3_ENTRY bucket", "xfer",
            ...
            for (i = 0; i < iter_cmic; i++) {
                ...
                soc_mem_read_range(unit, L3_ENTRY_ONLYm, MEM_BLOCK_ANY,
