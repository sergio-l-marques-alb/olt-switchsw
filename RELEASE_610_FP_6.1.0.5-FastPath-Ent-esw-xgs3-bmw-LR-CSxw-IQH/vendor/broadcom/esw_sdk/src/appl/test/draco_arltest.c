/*
 * $Id: draco_arltest.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
mac_addr_t dl_mac_src = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
...
fb_l2_test_hash(int unit, args_t *a, void *p)
{
    ...
    for (ix = 0; ix < iterations; ix++) {
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            _bcm_tr_l2_to_l2x(unit, &entry, &addr);
            num_bits = soc_tr_l2x_base_entry_to_key(unit, (uint32 *) &entry, key);
            soft_bucket = soc_tr_l2x_hash(unit, hash, num_bits, key);
        } else
#endif /* BCM_TRX_SUPPORT */
        {
...
#ifdef BCM_TRX_SUPPORT
            if (SOC_IS_TRX(unit)) {
                _bcm_tr_l2_to_l2x(unit, &(entry_tmp[ix]), &addr);

                if (ix == 0) {
                    int num_bits;

                    num_bits = soc_tr_l2x_base_entry_to_key(unit, (uint32 *) &(entry_tmp[ix]), key);
                    bucket = soc_tr_l2x_hash(unit, hash, num_bits, key);
...
#ifdef BCM_TRX_SUPPORT
        if (SOC_IS_TRX(unit)) {
            _bcm_tr_l2_to_l2x(unit, &(entries[i]), &addr);
        } else
#endif /* BCM_TRX_SUPPORT */
        {
    ...
    PBMP_E_ITER(unit, i) {
        ...
        if (!bucket_full[i]) {
            ...
            if (r < 0) {
                ...
                if (SOC_IS_TRX(unit)) {
                    int num_bits;
                    num_bits = soc_tr_l2x_base_entry_to_key(unit, (uint32 *) cur_entry, key);
                    soft_bucket = soc_tr_l2x_hash(unit, ad->opt_hash, num_bits, key);
                } else
#endif
                {
...
draco_l3_test_hash(int unit, args_t *a, void *p)
{
    ...
    memcpy(&cfg.l3c_mac_addr, &ad->opt_base_mac, sizeof(mac_addr_t));
...
{
    uint32                      entry[SOC_MAX_MEM_WORDS];
    uint32                      entry_tmp[SOC_L3X_MAX_BUCKET_SIZE][SOC_MAX_MEM_WORDS];
    soc_mem_t                   mem = L3_ENTRY_IPV4_UNICASTm;
