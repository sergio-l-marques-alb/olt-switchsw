/*
 * $Id: rpacket.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
#include <soc/sbx/gu2/sbG2FeTableAccess.h>
#include <soc/sbx/gu2/sbG2Fe.h>
#endif /* def BCM_FE2000_P2_SUPPORT */
...
static  mac_addr_t      rp_mac_dest = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
static  mac_addr_t      rp_mac_src  = {0x00, 0x11, 0x22, 0x33, 0x44, 0x66};
...
rpacket_setup(int unit, p_t *p)
{
...
#ifdef BCM_FE2000_P2_SUPPORT
    sbG2Fe_t   *pFe;
    sbZfG2FeEgrPVid2Etc_t   zEgrPVid2Etc;
#endif /* def BCM_FE2000_P2_SUPPORT */
#ifdef BCM_FE2000_P3_SUPPORT
...
#ifdef BCM_FE2000_P2_SUPPORT
    case SOC_SBX_UCODE_TYPE_G2P2:
        pFe = SOC_SBX_G2FE_FROM_UNIT(unit);
        for (port_idx = 0; port_idx < num_test_ports; port_idx++) {
            rv = sbG2FeEgrPVid2EtcGet(pFe,
                                      p->p_port[port_idx],
                                      1,
                                      &zEgrPVid2Etc);
            zEgrPVid2Etc.ulStrip = TRUE;
            rv = sbG2FeEgrPVid2EtcSet(pFe,
                                      p->p_port[port_idx],
                                      1,
                                      &zEgrPVid2Etc);
        }
    ...
    case SOC_SBX_UCODE_TYPE_G2P3:
        for (port_idx = 0; port_idx < num_test_ports; port_idx++) {
            rv = soc_sbx_g2p3_epv2e_get(unit, 1, p->p_port[port_idx], &p3epv2e);
            p3epv2e.strip = TRUE;
            rv = soc_sbx_g2p3_epv2e_set(unit, 1, p->p_port[port_idx], &p3epv2e);
        }
        break;
#endif /* def BCM_FE2000_P3_SUPPORT */
    default:
    ...
    if (soc_feature(unit, soc_feature_field)) {
        ...
        if (SOC_IS_ESW(unit)) {
            ...
            BCM_PBMP_PORT_ADD(redirect_pbmp, p->p_port[0]);
            if (BCM_SUCCESS(rv)) {
...
#ifdef BCM_ROBO_SUPPORT
        rv = bcm_port_enable_set(unit, p->p_port[0], 1);
        if (BCM_FAILURE(rv)) {
            test_error(unit, "Unable to enable port %d: %s\n", p->p_port[0], bcm_errmsg(rv));
            test_rv = -1;
            goto done;
        }
#endif /* BCM_ROBO_SUPPORT */
 ...
 */
{
    ...
    (DRV_SERVICES(unit)->rate_set)
        ...
        imp_rate_default, imp_burst_size_default);
#endif /* BCM_ROBO_SUPPORT */
