/*
 * $Id: ctrtest.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
test_width32(int unit, soc_reg_t ctr_reg, soc_port_t port)
{
    ...
    if (soc_feature(unit, soc_feature_prime_ctr_writes)) {
        ...
        r = soc_reg32_write(unit, addr, ~0);
    }

    if ((r < 0) ||
	(r = soc_reg32_read(unit, addr, &val)) < 0) {
	test_error(unit,
		   "Can't access counter %s.%s: %s\n",
		   name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
	return -1;
    ...
    if (soc_feature(unit, soc_feature_counter_parity)) {
        val &= ((1 << soc_reg_field_length(unit, ctr_reg, COUNTf)) - 1);
    }
#endif
    }
...
test_width64(int unit, soc_reg_t ctr_reg, soc_port_t port)
{
    ...
    if (SOC_IS_ROBO(unit)) {
	   ...
	   return -1;
        ...
        name = SOC_REG_NAME(unit, ctr_reg);
        addr = soc_reg_addr(unit, ctr_reg, port, 0);
        if ((r = soc_reg64_write(unit, addr, val)) < 0 ||
	    (r = soc_reg64_read(unit, addr, &val)) < 0) {
	    test_error(unit,
		   "Can't access counter %s.%s: %s\n",
		   name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
	    return -1;
...
test_rw32(int unit, soc_reg_t ctr_reg, soc_port_t port)
{
    ...
    uint32            mask, val;
#if defined(BCM_ROBO_SUPPORT) || (BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
        ...
        if (SOC_IS_ROBO(unit)) {
 	    ...
 	    if (r < 0) {
            ...
            if (soc_feature(unit, soc_feature_prime_ctr_writes)) {
                ...
                r = soc_reg32_write(unit, addr, pattern_mask);
            }
            if (r < 0) {
                val = pattern_mask;
	        test_error(unit,
		       "Can't write counter %s.%s: %s\n",
		       name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
            } else if ((r = soc_reg32_read(unit, addr, &val)) < 0) {
	        test_error(unit,
		       "Can't access counter %s.%s: %s\n",
		       name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
            }
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
	    (void) soc_reg32_read(unit, addr, &reread);
#endif
           }
	...
	tmp = pattern;
        if (SOC_IS_ROBO(unit)) {
           ...
           if ((r = (DRV_SERVICES(unit)->reg_write)(unit, addr, &tmp, (bits/8))) < 0) {
            ...
            name = SOC_REG_NAME(unit, ctr_reg);
             addr = soc_reg_addr(unit, ctr_reg, port, 0);
	if ((r = soc_reg64_write(unit, addr, tmp)) < 0) {
            val = tmp;
	    test_error(unit,
		       "Can't write counter %s.%s: %s\n",
		       name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
	} else if ((r = soc_reg64_read(unit, addr, &val)) < 0) {
	    test_error(unit,
		       "Can't access counter %s.%s: %s\n",
		       name, SOC_PORT_NAME(unit, port), soc_errmsg(r));
             }
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
		(void) soc_reg64_read(unit, addr, &reread);
#endif
           }
