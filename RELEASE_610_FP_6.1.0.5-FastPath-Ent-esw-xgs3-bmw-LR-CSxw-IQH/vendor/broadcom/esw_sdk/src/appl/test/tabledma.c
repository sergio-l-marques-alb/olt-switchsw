/*
 * $Id: tabledma.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
td_test_data_init(int unit, int alloc)
{
    ...
    for (i = 0; i < td_test_data.entwords; i++) {
        if (td_test_data.tcammask[i]) {
            td_test_data.datamask[i] &= ~td_test_data.tcammask[i];
            has_tcammask = TRUE;
        }
    ...
    td_test_data.comp_fn     = meminfo->cmp_fn;
    if (td_test_data.comp_fn == _soc_mem_cmp_undef ||
        td_test_data.comp_fn == 0 ) {
...
td_test_dump_data(int unit, int entry_cnt)
{
...
#if 0 /* do we need to decode these randomly generated field? */
    soc_mem_entry_dump(unit, td_test_data.mem, td_test_data.datamask);
#else
    ...
    for (i = td_test_data.index_min;
        ...
        printk("    Source:  ");
#if 0 /* do we need to decode these randomly generated field? */
        soc_mem_entry_dump(unit, td_test_data.mem, s);
#else
        for (idx = 0; idx < td_test_data.entwords; idx++) {
            printk(" %08x", s[idx]);
        }
        ...
        printk("    DMA_ed:  ");
#if 0 /* do we need to decode these randomly generated field? */
        soc_mem_entry_dump(unit, td_test_data.mem, d);
#else
        for (idx = 0; idx < td_test_data.entwords; idx++) {
            printk(" %08x", d[idx]);
        }
    ...
    if (sal_memcmp(td_test_data.dma_ed, td_test_data.source,
        ...
        for (i = index_min; i <= index_max; ++i) {
            if (td_test_data.comp_fn(unit, d, s) != 0) {
                ...
                /* This code is to detect mismatches in "undefined" data */
                printk("Raw diffs:\n");
#endif
                printk("Source:");
