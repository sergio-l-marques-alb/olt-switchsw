/*
 * $Id: loopback.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
/* mac_addr_t lb_mac_default = {0x00, 0x00, 0x00, 0xde, 0xad, 0x00}; */
mac_addr_t lb_mac_src = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
mac_addr_t lb_mac_dst = {0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb};
    ...
    if (SOC_IS_XGS_SWITCH(lw->lw_unit)) {
        ...
        if (SOC_IS_XGS3_SWITCH(lw->lw_unit)) {
            ...
            PBMP_ST_ITER(lw->lw_unit, port) {
                if (SOC_IS_TRIUMPH2(lw->lw_unit) || SOC_IS_APOLLO(lw->lw_unit) 
                    ...
                    uint32 dst_port, uint32 vlan, uint32 *xghp)
{
    ...
    if (SOC_IS_HAWKEYE(unit) || ((SOC_IS_RAPTOR(unit) || SOC_IS_RAVEN(unit)) && (IS_E_PORT(unit, dst_port)))) {
...
lb_check_packet(loopback_test_t *lw, uint8 *tx_data, int tx_len,
                ...
                mac_addr_t mac_dst, mac_addr_t mac_src, int skip_vlan)
/*
 ...
 */
{
    ...
    while (!lw->lw_eoc_tx || !lw->lw_eoc_rx) {
        if (sal_sem_take(lw->lw_sema, lb_timeout_usec(lw->lw_unit))) {
...
lb_fill_packet_tx(loopback_test_t *lw, loopback_testdata_t *lp,
                  uint8 *packet,
                  int *tx_len, mac_addr_t mac_dst, mac_addr_t mac_src)
/*
 ...
 */
{
    ...
    int         c;
    int         rv, org_len;
    mac_addr_t  cur_dst_mac;
    mac_addr_t  cur_src_mac;
    pbmp_t      pbm;
#ifdef BCM_HIGIG2_SUPPORT
 ...
 */
{
    ...
    if ((rv = bcm_l2_addr_delete(unit, mac_addr, (vlan_id_t)vlan)) < 0) {
        if (rv != BCM_E_NOT_FOUND) {
            return rv;
        } /* else, non-existent entry */
    }

    sal_memcpy(arl->mac, mac_addr, sizeof(mac_addr_t));
    arl->vid = vlan;
    arl->flags = BCM_L2_STATIC;
    arl->port = p;
    return(bcm_l2_addr_add(unit, arl));
}
    ...
    *p = lw;                            /* Pass back cookie */
    ...
    if (soc_feature(unit, soc_feature_l2_user_table)) {
        ...
        if ((rv = bcm_l2_cache_set(unit, l2_index, &l2_cache_addr,
                                   &l2_index)) < 0) {
            test_error(lw->lw_unit,
                       "Failed to configure L2 User Enrty Table %s\n",
                       bcm_errmsg(rv));
            return(rv);
        }
    }
 ...
 */
{
    ...
    /* On 48-port board, do not attempt slow MAC loopback on cross-links */
    if ((sal_boot_flags_get() & BOOT_F_48PORT_BOARD) && 
        ((lp->lp_speed != LB_SPEED_MAX) && 
         (lp->lp_speed != LB_SPEED_1000FD))) {
        soc_port_t      xport;
        ...
        if (SOC_PBMP_MEMBER(lp->lp_pbm, xport) ||
            SOC_PBMP_MEMBER(lp->lp_ubm, xport)) {
            printk("WARNING: Skipping loopback on 48-port cross-link\n");
            SOC_PBMP_PORT_REMOVE(lp->lp_pbm, xport);
            SOC_PBMP_PORT_REMOVE(lp->lp_ubm, xport);
        }
    }
 ...
 */
{
    ...
    for (l_cur = lp->lp_l_start;
        ...
        for (cos_cur = lp->lp_cos_start;
             cos_cur <= lp->lp_cos_end;
             cos_cur++) {
            for (tx_offset = 0; tx_offset <= 3; tx_offset++) {
    ...
    for (tx = 0; tx <= 1; tx++) {
        for (channel = 0; channel < N_DMA_CHAN; channel++) {
            ...
            for (d = lp->lp_dv_start; d <= lp->lp_dv_end; d += lp->lp_dv_inc) {
                ...
                while (!rv) {
                    if (sal_sem_take(lw->lw_sema,
                                     lb_timeout_usec(lw->lw_unit))) {
