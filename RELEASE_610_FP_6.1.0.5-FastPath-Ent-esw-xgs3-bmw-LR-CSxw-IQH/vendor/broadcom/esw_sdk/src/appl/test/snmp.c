/*
 * $Id: snmp.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
    ...
    PBMP_PORT_ITER(b->unit, port) {
        ...
        for (object = 0; object < snmpValCount; object++) {
            ...
            if (rv == BCM_E_UNAVAIL) {
                ...
                printk("FAIL: u=%d:speed=%d:port=%d:object=%d:rv=%d: %s\n",
...
{
    printk("All Ports: Max speed=%d: %d failures; %d vars not implemented\n",
