/*
 * $Id: lb_util.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
/* mac_addr_t lbu_mac_default = {0x00, 0x00, 0x00, 0xde, 0xad, 0x00}; */
mac_addr_t lbu_mac_src = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
mac_addr_t lbu_mac_dst = {0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb};
                   ...
                   bcm_pkt_t *rx_info)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)) {
            crc_length = ((lp->crc_mode == LB2_CRC_MODE_MAC_REGEN) ? -4 : 0);
        } else {
    ...
    if (rv) {
        ...
        if (SOC_IS_ROBO(unit)){
#ifdef BCM_ROBO_SUPPORT
            soc_robo_dma_dump_pkt(unit, "  ", tx_data, tx_info->_pkt_data.len);
            printk("RX packet: len=%d\n", rx_info->pkt_len);
            soc_robo_dma_dump_pkt(unit, "  ", rx_data, rx_info->pkt_len);
#endif 
        } else {
#ifdef BCM_ESW_SUPPORT
    ...
    for (ix = 0; ix < lw->rx_pkt_cnt; ix++) {
        ...
        if ((ev = lbu_snake_pkt_match(lw, rx_info, &match_idx)) < 0) {
            if (match_idx >=0) {
                ...
                if (SOC_IS_ROBO(unit)){
#ifdef BCM_ROBO_SUPPORT
                    soc_robo_dma_dump_pkt(unit, "  ", BCM_PKT_IEEE(tx_info), 
                                 tx_info->_pkt_data.len);
#endif /* BCM_ROBO_SUPPORT */
                } else {
#ifdef BCM_ESW_SUPPORT                
...
lbu_set_l2_addr(int unit, bcm_port_t p, bcm_vlan_t vlan, 
                bcm_l2_addr_t *l2_addr, mac_addr_t mac_addr)
/*
 ...
 */
{
    ...
    sal_memcpy(l2_addr->mac, mac_addr, sizeof(mac_addr_t));
    l2_addr->vid = vlan;
                        ...
                        int connect)
{
    ...
    if (MAC_IS_MCAST(pc_info->dst_mac)) {
        ...
        if (connect) {
            ...
            if (NUM_MODID(unit) == 2) {
                if ((pc_info->to_port % (SOC_PORT_ADDR_MAX(unit) + 1)) > 31) {
                    l2_addr.port = (pc_info->to_port % 
                                    (SOC_PORT_ADDR_MAX(unit) + 1)) - 32;
...
lbu_port_init(loopback2_test_t *lw, loopback2_testdata_t *lp)
{
    ...
    if (lp->loopback == LB2_MODE_MAC) {
        ...
        /* On 48-port board, do not attempt slow MAC loopback on cross-link */
        if ((sal_boot_flags_get() & BOOT_F_48PORT_BOARD) && 
            ((lp->speed != LB_SPEED_MAX) && 
             (lp->speed != LB_SPEED_1000FD))) {
            soc_port_t  xport;

            if (unit == 0) {                    /* unit 0 reserves ge0 */
                xport = SOC_PORT(unit, ge, 0);
            } else {                            /* all others reserve ge1 */
                xport = SOC_PORT(unit, ge, 1);
            }

            if (SOC_PBMP_MEMBER(lp->pbm, xport) ||
                SOC_PBMP_MEMBER(lp->ubm, xport)) {
                printk("WARNING: Skipping loopback on 48-port cross-link\n");
                SOC_PBMP_PORT_REMOVE(lp->pbm, xport);
                SOC_PBMP_PORT_REMOVE(lp->ubm, xport);
            }
        }
        ...
        /* On 48-port board, do not attempt PHY loopback on cross-links */
        if (sal_boot_flags_get() & BOOT_F_48PORT_BOARD) {
            bcm_port_t  xport;
            ...
            if (BCM_PBMP_MEMBER(lp->pbm, xport) ||
                BCM_PBMP_MEMBER(lp->ubm, xport)) {
                printk("WARNING: Skipping loopback on 48-port cross-link\n");
                BCM_PBMP_PORT_REMOVE(lp->pbm, xport);
                BCM_PBMP_PORT_REMOVE(lp->ubm, xport);
            }
        }
        ...
        /* On 48-port board, do not attempt EXT loopback on cross-links */
        if (sal_boot_flags_get() & BOOT_F_48PORT_BOARD) {
            bcm_port_t  xport;
 ...
 */
{
    ...
    if (lp->ppt_start > SOC_DV_PKTS_MAX || lp->ppt_end > SOC_DV_PKTS_MAX) {
        test_error(unit, "Too many packets per trial (%d, %d).  Max is %d.\n",
               lp->ppt_start, lp->ppt_end, SOC_DV_PKTS_MAX);
        lp->ppt_end = lp->ppt_start = SOC_DV_PKTS_MAX;
        return -1;
    }
#ifdef BCM_ESW_SUPPORT
    if (!SOC_IS_ROBO(unit)) {
...
lbu_pkt_data_setup(loopback2_test_t *lw, bcm_pkt_t *pkt, bcm_vlan_t vlan,
                   mac_addr_t mac_dst, mac_addr_t mac_src, int seq_no)
{
    loopback2_testdata_t *lp = lw->cur_params;
    enet_hdr_t           *eh = NULL;
    int tx_len;
    ...
    packet_store((uint8*)&eh->en_tag_len, 2, (tx_len) << 16, 0);
                    ...
                    loopback2_test_t *lw)
{
    loopback2_testdata_t *lp = lw->cur_params;
    int                  i;
    mac_addr_t           cur_mac_dst, cur_mac_src;
    bcm_pkt_t           *pkt;
...
lbu_serial_wait(loopback2_test_t *lw)
{
    ...
    if (sal_sem_take(lw->sema, lbu_timeout_usec(unit))) {
...
#ifdef BCM_ROBO_SUPPORT
            soc_robo_dma_dump_pkt(lw->unit, "  ", lw->tx_pkts[ix]->_pkt_data.data, 
