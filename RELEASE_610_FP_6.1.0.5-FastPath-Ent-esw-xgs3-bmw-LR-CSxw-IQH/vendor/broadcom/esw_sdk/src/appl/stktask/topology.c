/*
 * $Id: topology.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
STATIC void
tp_weights_init(cpudb_ref_t db_ref, uint8 *weights)
{
    int s_idx, d_idx;
    ...
    CPUDB_FOREACH_ENTRY(db_ref, src_entry) {
        ...
        for (i = 0; i < src_entry->base.num_stk_ports; i++) {
            ...
            TP_WEIGHT(weights, s_idx, d_idx) = 1;
            ...
            if (sp->flags & CPUDB_SPF_DUPLEX) {
                if (!TP_REACHABLE(db_ref, d_idx, s_idx)) {
                    TP_WEIGHT(weights, d_idx, s_idx) = 1;
                    /* Below indicates is reachable and by what stk port */
...
STATIC void
tp_weights_process(cpudb_ref_t db_ref, uint8 *weights)
{
    int count;
 ...
 *      src_key             - Source CPU key
 *      dest_key            - Destination CPU key
