/*
 * $Id: brd_strata.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
_bcm_board_sl_topomap(int src_unit, int dest_modid, bcm_port_t *exit_port)
{
    ...
    TOPO_VERB(("SL topo map failed unit %d to mod id %d\n",
               src_unit, dest_modid));
...
_bcm_board_sl_modport(int unit, topo_cpu_t *tp_cpu)
{
    ...
    if (rv < 0) {
        TOPO_VERB(("SL modport %d: modport failed: %s\n",
                   unit, bcm_errmsg(rv)));
	if (rv == BCM_E_UNAVAIL) {
	    rv = BCM_E_NONE;
	}
	...
	if (punit != unit) {
        ...
        TOPO_VVERB(("SL modport map unit %d port %d mods %d\n",
...
_bcm_board_sl_loop(int unit, topo_cpu_t *tp_cpu)
{
	...
	if (!(l_entry->sp_info[i].flags &
	    ...
	    BCM_PBMP_PORT_ADD(cutports, port);
            ...
            TOPO_VERB(("BRD: Port %d.%d is a cut port\n", punit, port));
	    BCM_PBMP_PORT_ADD(cutports, port);
	}
    }
                 ...
                 bcm_port_t dflt_exit_port, int is_strata)
{
	...
	if (rv < 0 && rv != BCM_E_NOT_FOUND) {
            TOPO_WARN(("BRD: Could not l2 delete mac %x:%x:%x\n",
                       _bcm_sl_l2mac[i][3],
                       _bcm_sl_l2mac[i][4],
                       _bcm_sl_l2mac[i][5]));
	    return rv;
	...
	l2addr.modid = entry->dest_mod;
        ...
        if (use_dflt_port) {
            if (dflt_exit_port < 0) {
                TOPO_WARN(("BRD: CPU L2 map; not local but no dflt port"
                       " for unit %d and key " CPUDB_KEY_FMT_EOLN, unit,
	...
	l2addr.flags |= BCM_L2_STATIC;
        TOPO_VERB(("BRD: Adding L2 on %d for " CPUDB_KEY_FMT
                   " to mod %d port %d\n",
...
{
    TOPO_VERB(("TOPO: SL 5645 24FE+2GE board topology handler\n"));
...
{
    TOPO_VERB(("TOPO: SL 5665 48FE+4GE board topology handler\n"));
...
bcm_board_topo_48f2g_stk(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    ...
    TOPO_VERB(("TOPO: SL b2b 5645 48FE+2GE board topology handler\n"));
