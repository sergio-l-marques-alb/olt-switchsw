/*
 * $Id: topo_brd.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
_topo_stk_ports_update(cpudb_ref_t db_ref, topo_cpu_t *topo_cpu)
{
    ...
    for (i = 0; i < l_entry->base.num_stk_ports; i++) {
        ...
        if (l_entry->sp_info[i].flags & CPUDB_SPF_ETHERNET) {
            flags = 0; /* Disable - not a stack port anymore */
        } else if (l_entry->sp_info[i].flags & CPUDB_SPF_NO_LINK) {
            flags |= BCM_STK_INACTIVE | BCM_STK_NO_LINK;
        } else if (l_entry->sp_info[i].flags & CPUDB_SPF_INACTIVE) {
            ...
            if (l_entry->sp_info[i].flags & CPUDB_SPF_DUPLEX) {
...
int topo_board_initialize(cpudb_ref_t db_ref)
{
    switch (db_ref->local_entry->base.board_id) {
