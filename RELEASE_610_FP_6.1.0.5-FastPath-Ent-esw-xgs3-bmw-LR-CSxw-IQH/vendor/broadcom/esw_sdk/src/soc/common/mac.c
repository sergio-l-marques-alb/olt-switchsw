/*
 * $Id: mac.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
extern mac_driver_t soc_mac_ge;   /* GMACC */
...
extern mac_driver_t soc_mac_gx;   /* 12G/10G/2.5G/1G MAC */
#if defined(BCM_BIGMAC_SUPPORT) && defined(BCM_UNIMAC_SUPPORT)
extern mac_driver_t soc_mac_big_uni; /* 16G/13G/12G/10G/2.5G/1G MAC */
#endif /* BCM_BIGMAC_SUPPORT && BCM_UNIMAC_SUPPORT */
...
mac_fe_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (!enable) {
       ...
       if (SOC_IS_FIREBOLT2(unit)) {    
            /* Reset the MAC so that the error link state which may affect 
             * receive false carrier counter is cleared when the link
             * is up again. */
            SOC_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, GMACC0r, port, SRSTf, 1));
            SOC_IF_ERROR_RETURN
