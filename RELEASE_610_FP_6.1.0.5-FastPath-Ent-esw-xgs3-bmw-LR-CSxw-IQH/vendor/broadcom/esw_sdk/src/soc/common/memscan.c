/*
 * $Id: memscan.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
_soc_mem_scan_thread(void *unit_vp)
{
    ...
    err_max = soc_property_get(unit, spn_MEM_SCAN_MAXERR, 8);
	...
	for (mem = 0; mem < NUM_SOC_MEM; mem++) {
		...
		for (idx = soc_mem_index_min(unit, mem);
			...
			if (++err_count == err_max) {
			    soc_cm_debug(DK_ERR,
                                     "soc_mem_scan_thread: Too many errors\n");
			    MEM_UNLOCK(unit, mem);
			    goto cleanup_exit;
			}
		    } else {
