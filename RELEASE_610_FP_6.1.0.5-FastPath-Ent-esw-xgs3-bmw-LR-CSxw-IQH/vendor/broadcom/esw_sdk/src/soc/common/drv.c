...
char *
soc_block_port_name_lookup(soc_block_t blk)
{
    int         i;
...
#endif
    {
	return soc_block_port_name_lookup(blk);
    }
    return "?";
}

char *
soc_block_name_lookup(soc_block_t blk)
{
    int         i;
...
#endif
    {
	return soc_block_name_lookup(blk);
    }
    return "?";
}
...
soc_endian_config(int unit)
{
    ...
    if (SOC_IS_LYNX(unit) && SAL_BOOT_RTLSIM) { /* And what? */
        config |= ES_BIG_ENDIAN_CTR64_WORDS;
    }

    if (SOC_IS_RAVEN(unit) && SOC_IS_RCPU_UNIT(unit)) {
        config |= EN_BIG_ENDIAN_EB2_2B_SEL;
    }

    soc_pci_write(unit, CMIC_ENDIAN_SELECT, config);
}
...
soc_xgxs_reset(int unit, soc_port_t port)
{
    ...
    if (SOC_REG_FIELD_VALID(unit, reg, LCREFENf)) {
        ...
        soc_reg64_field32_set(unit, reg, &rval64, LCREFENf, lcpll ? 1 : 0);
