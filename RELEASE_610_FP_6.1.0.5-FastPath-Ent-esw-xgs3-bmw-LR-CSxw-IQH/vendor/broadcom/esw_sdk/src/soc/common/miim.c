/*
 * $Id: miim.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
{
    return soc_miim_write(unit, (uint8)phy_id,
...
{
    return soc_miim_read(unit, (uint8)phy_id, 
...
int
soc_miim_modify(int unit, uint8 phy_id, uint16 phy_reg_addr, 
                     ...
                     uint32 phy_reg_addr, uint16 phy_wr_data)
{
    ...
    return soc_miimc45_write(unit, (uint8)phy_id, dev_addr,
                    ...
                    uint32 phy_reg_addr, uint16 *phy_rd_data)
{
    ...
    return soc_miimc45_read(unit, (uint8)phy_id, dev_addr,
...
int
soc_miimc45_modify(int unit, uint8 phy_id, uint8 phy_devad,
...
int 
soc_miim_write(int unit, uint8 phy_id,
               uint8 phy_reg_addr, uint16 phy_wr_data)
{
    int                 rv = SOC_E_NONE;
...
int
soc_miim_read(int unit, uint8 phy_id,
              uint8 phy_reg_addr, uint16 *phy_rd_data)
{
    int                 rv = SOC_E_NONE;
#ifdef BCM_ESW_SUPPORT
    ...
    if (rv == SOC_E_TIMEOUT) {
        ...
        *phy_rd_data = (uint16)soc_pci_read(unit, CMIC_MIIM_READ_DATA);
        ...
        soc_cm_debug(DK_MIIM,
                     "soc_miim_read: read data=0x%04x\n", *phy_rd_data);
    }
...
int 
soc_miimc45_write(int unit, uint8 phy_id, uint8 phy_devad,
                  uint16 phy_reg_addr, uint16 phy_wr_data)
{
    int                 rv = SOC_E_NONE;
    assert(!sal_int_context());
...
int
soc_miimc45_read(int unit, uint8 phy_id, uint8 phy_devad,
                 uint16 phy_reg_addr, uint16 *phy_rd_data)
{
    int                 rv = SOC_E_NONE;    
    assert(!sal_int_context());
