/*
 * $Id: unimac.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
_soc_egress_metering_freeze(int unit, soc_port_t port, uint64 *key)
{
    ...
    case SOC_CHIP_BCM53314:
        rv = READ_EGRMETERINGCONFIGr(unit, port, &temp_32);
        if (SOC_SUCCESS(rv)) {
            rv = WRITE_EGRMETERINGCONFIGr(unit, port, 0);
        }
        COMPILER_64_SET(*key, 0, temp_32);
        break;
    ...
    case SOC_CHIP_BCM56334:
        ...
        if (SOC_SUCCESS(rv)) {
            rv = WRITE_EGRMETERINGCONFIG_64r(unit, port, temp_64);
        }     
        break;
...
STATIC int
_soc_egress_metering_thaw(int unit, soc_port_t port, uint64 key_64)
{
    int       rv;
    uint32    key_32;
    ...
    COMPILER_64_TO_32_LO(key_32, key_64);
    ...
    case SOC_CHIP_BCM53314:
        ...
        rv = WRITE_EGRMETERINGCONFIGr(unit, port, key_32);
        break;
#endif /* BCM_RAVEN_SUPPORT || BCM_SCORPION_SUPPORT */
#if defined(BCM_TRIUMPH_SUPPORT)
    case SOC_CHIP_BCM56624:
    ...
    case SOC_CHIP_BCM56334:
    case SOC_CHIP_BCM56685:
        rv = WRITE_EGRMETERINGCONFIG_64r(unit, port, key_64);
        break;
#endif /* BCM_TRIUMPH_SUPPORT */
    default:
        return SOC_E_NONE;
...
_mac_uni_drain_cells(int unit, soc_port_t port)
{
    ...
    for (;;) {
        ...
        if (new_cells < cur_cells) {                    /* Progress made */
            /* Restart timeout */
            soc_timeout_init(&to, lcc_drain_timeout[unit], 0);
            cur_cells = new_cells;
        }
    ...
    /* Restore egress metering configuration. */
    SOC_IF_ERROR_RETURN
        (_soc_egress_metering_thaw(unit, port, meter_key));
...
mac_uni_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (tx_ena == rx_ena && tx_ena == (enable ? 1 : 0)) {
        return SOC_E_NONE;
    }
...
mac_uni_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    if ((1000 == speed) || (2500 == speed)) {
        ...
        *duplex = soc_reg_field_get(unit, COMMAND_CONFIGr, 
                                    command_config, HD_ENAf);
    }
                ...
                soc_port_duplex_t  duplex, int ifg)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_TX_IPG_LENGTHr(unit, port, ipg));
    ...
    if ((cur_speed == speed) && ((soc_port_duplex_t)cur_duplex == duplex)) {
        return mac_uni_ipg_update(unit, port);
    }
                ...
                soc_port_duplex_t  duplex, int *ifg)
{
    ...
    DBG_GE_VERB(("mac_uni_ifg_get: unit %d port %s speed=%dMb duplex=%s ifg=%d\n",
                 unit, SOC_PORT_NAME(unit, port),
                 speed, duplex ? "True" : "False", *ifg));
    return SOC_E_NONE;
}
