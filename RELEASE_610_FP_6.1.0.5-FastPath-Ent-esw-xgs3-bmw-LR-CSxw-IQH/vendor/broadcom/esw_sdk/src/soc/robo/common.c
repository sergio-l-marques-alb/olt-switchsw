/*
 * $Id: common.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
soc_robo_anyreg_read(int unit, soc_regaddrinfo_t *ainfo, uint64 *data)
{
    ...
    if (len <= 4) {
        COMPILER_64_SET(*data, 0,*(uint32 *)data);
     }
...
soc_robo_reg_iterate(int unit, soc_reg_iter_f do_it, void *data)
{
    ...
    for (reg = 0; reg < NUM_SOC_ROBO_REG; reg++) {
        ...
        case soc_portreg:
            ...
            break;
        case soc_phy_reg:
            ainfo.block = SOC_REG_INFO(unit,reg).block;
            if (ainfo.block & (SOC_BLK_PORT|SOC_BLK_INTER|SOC_BLK_EXTER)) {
                    SOC_PBMP_CLEAR(bm);
                    SOC_BLOCK_ITER(unit, blk, ainfo.block) {
        ...
        case soc_genreg:
            ...
            if (ainfo.block & SOC_BLK_SYS) {
               if (numels == 1) {
                ...
                if (ainfo.block & (SOC_BLK_PORT|SOC_BLK_EXTER)) {
                    SOC_PBMP_CLEAR(bm);
                    SOC_BLOCK_ITER(unit, blk, ainfo.block) {
                        SOC_PBMP_OR(bm, SOC_BLOCK_BITMAP(unit, blk));
                    }
