 ...
 *
 * $Id: etc47xx.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
static void chiprxreset(struct bcm4xxx *ch);
static void chiprxinit(struct bcm4xxx *ch);
...
chipattach(etc_soc_info_t *etc, void *dev, void *regsva)
{
    ...
    /* XXX this assumes that we are running on a 47xx mips */
    /* with a swap window at 0x1000000 */
    dataoffset |= SB_SDRAM_SWAPPED;
#endif

    /* dma attach */
    sprintf(name, "et%d", (int)dev);
    if ((ch->di = dma_soc_attach(etc->et, dev, name, &regs->dmaregs,
        NTXD, NRXD, RXBUFSZ, NRXBUFPOST, HWRXOFF, 
        ddoffset, dataoffset, (uint32 *) &et_msg_level)) == NULL) {
        ET_ERROR(("et%d: chipattach: dma_attach failed\n", etc->unit));
        goto fail;
    }
    etc->txavail = (uint*) dma_soc_getvar(ch->di, "&txavail");
...
chiptx(struct bcm4xxx *ch, void *p0)
{
    ...
    error = dma_soc_txfast(ch->di, p0, 0);
...
chipstatsupd(struct bcm4xxx *ch)
{
     ...
     */
    etc->mib.tx_broadcast_pkts += R_REG(ch->dev, &regs->mib.tx_broadcast_pkts);
    etc->mib.tx_multicast_pkts += R_REG(ch->dev, &regs->mib.tx_multicast_pkts);
    etc->mib.tx_len_64 += R_REG(ch->dev, &regs->mib.tx_len_64);
    etc->mib.tx_len_65_to_127 += R_REG(ch->dev, &regs->mib.tx_len_65_to_127);
    etc->mib.tx_len_128_to_255 += R_REG(ch->dev, &regs->mib.tx_len_128_to_255);
    etc->mib.tx_len_256_to_511 += R_REG(ch->dev, &regs->mib.tx_len_256_to_511);
    ...
    etc->mib.tx_jabber_pkts += R_REG(ch->dev, &regs->mib.tx_jabber_pkts);
    etc->mib.tx_oversize_pkts += R_REG(ch->dev, &regs->mib.tx_oversize_pkts);
    etc->mib.tx_fragment_pkts += R_REG(ch->dev, &regs->mib.tx_fragment_pkts);
    etc->mib.tx_underruns += R_REG(ch->dev, &regs->mib.tx_underruns);
    etc->mib.tx_total_cols += R_REG(ch->dev, &regs->mib.tx_total_cols);
    etc->mib.tx_single_cols += R_REG(ch->dev, &regs->mib.tx_single_cols);
    etc->mib.tx_multiple_cols += R_REG(ch->dev, &regs->mib.tx_multiple_cols);
    etc->mib.tx_excessive_cols += R_REG(ch->dev, &regs->mib.tx_excessive_cols);
    etc->mib.tx_late_cols += R_REG(ch->dev, &regs->mib.tx_late_cols);
    etc->mib.tx_defered += R_REG(ch->dev, &regs->mib.tx_defered);
    etc->mib.tx_carrier_lost += R_REG(ch->dev, &regs->mib.tx_carrier_lost);
    etc->mib.tx_pause_pkts += R_REG(ch->dev, &regs->mib.tx_pause_pkts);
    etc->mib.rx_broadcast_pkts += R_REG(ch->dev, &regs->mib.rx_broadcast_pkts);
    etc->mib.rx_multicast_pkts += R_REG(ch->dev, &regs->mib.rx_multicast_pkts);
    etc->mib.rx_len_64 += R_REG(ch->dev, &regs->mib.rx_len_64);
    etc->mib.rx_len_65_to_127 += R_REG(ch->dev, &regs->mib.rx_len_65_to_127);
    etc->mib.rx_len_128_to_255 += R_REG(ch->dev, &regs->mib.rx_len_128_to_255);
    etc->mib.rx_len_256_to_511 += R_REG(ch->dev, &regs->mib.rx_len_256_to_511);
    ...
    etc->mib.rx_jabber_pkts += R_REG(ch->dev, &regs->mib.rx_jabber_pkts);
    etc->mib.rx_oversize_pkts += R_REG(ch->dev, &regs->mib.rx_oversize_pkts);
    etc->mib.rx_fragment_pkts += R_REG(ch->dev, &regs->mib.rx_fragment_pkts);
    etc->mib.rx_missed_pkts += R_REG(ch->dev, &regs->mib.rx_missed_pkts);
    etc->mib.rx_crc_align_errs += R_REG(ch->dev, &regs->mib.rx_crc_align_errs);
    etc->mib.rx_undersize += R_REG(ch->dev, &regs->mib.rx_undersize);
    etc->mib.rx_crc_errs += R_REG(ch->dev, &regs->mib.rx_crc_errs);
    etc->mib.rx_align_errs += R_REG(ch->dev, &regs->mib.rx_align_errs);
    etc->mib.rx_symbol_errs += R_REG(ch->dev, &regs->mib.rx_symbol_errs);
    etc->mib.rx_pause_pkts += R_REG(ch->dev, &regs->mib.rx_pause_pkts);
    etc->mib.rx_nonpause_pkts += R_REG(ch->dev, &regs->mib.rx_nonpause_pkts);
