/*
 * $Id: vlan.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 *
        ...
        case DRV_VLAN_MODE_TAG:
            ...
            if (SOC_IS_ROBO5324(unit)) {
                if (soc_feature(unit, soc_feature_mstp)){
                     ...
                     *    (for no tagging)
                     */
                    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, VLAN_CTRL1r, 0, 0);
                    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, VLAN_CTRL1r);
                    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
                    (unit, reg_addr, &reg_value,reg_len));

                    temp = 1;
                    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, VLAN_CTRL1r, &reg_value, EN_RSV_MCAST_V_TAGf, &temp));
                    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
                    (unit, reg_addr, &reg_value,reg_len));
                }                
            }
            if (SOC_IS_ROBO5324(unit)||SOC_IS_ROBO5348(unit) ||
                ...
                temp = 1;
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, VLAN_CTRL2r, &reg_value, EN_GMRP_GVRP_V_TAGf, &temp));
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
                    (unit, reg_addr, &reg_value,reg_len));
            }                
             ...
             *  - GMRP/GVRP frame won't sent to CPU without set this bit.
             */
            reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, VLAN_CTRL4r, 0, 0);
            reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, VLAN_CTRL4r);
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
                (unit, reg_addr, &reg_value,reg_len));
            temp = 1; /* Drop frame if ingress vid violation */
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, VLAN_CTRL4r, &reg_value, INGR_VID_CHKf, &temp));

            temp = 1;
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, VLAN_CTRL4r, &reg_value, EN_MGE_REV_GVRPf, &temp));
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, VLAN_CTRL4r, &reg_value, EN_MGE_REV_GMRPf, &temp));
 ...
 */
int 
drv_port_vlan_pvid_set(int unit, uint32 port, uint32 vid, uint32 prio)
{
    uint32	reg_addr, reg_value;
    int		reg_len;

    soc_cm_debug(DK_VLAN, 
    "drv_port_vlan_pvid_set: unit = %d, port = %d, vid = %d, priority = %d\n",
        unit, port, vid, prio);
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, DEFAULT_1Q_TAGr, port, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, DEFAULT_1Q_TAGr);
    reg_value = (vid | (prio << 13));
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
        (unit, reg_addr, &reg_value,reg_len));
 ...
 */
int 
drv_port_vlan_pvid_get(int unit, uint32 port, uint32 *vid, uint32 *prio)
{
    uint32	reg_addr, reg_value;
    int		reg_len;
    ...
    *prio = reg_value >> 13;
    soc_cm_debug(DK_VLAN, 
    "drv_port_vlan_pvid_get: unit = %d, port = %d, vid = %d, priority = %d\n",
        unit, port, *vid, *prio);
    return SOC_E_NONE;
}
...
drv_port_vlan_set(int unit, uint32 port, soc_pbmp_t bmp)
{
    ...
    if (SOC_IS_ROBO5348(unit)) {
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, PORT_VLAN_CTLr, port, 0);
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PORT_VLAN_CTLr);
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
        soc_robo_64_pbmp_to_val(unit, &bmp, &temp64);
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_VLAN_CTLr, (uint32 *)&reg_value64, PORT_EGRESS_ENf, (uint32 *)&temp64));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_VLAN_CTLr, (uint32 *)&reg_value64, PORT_EGRESS_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
    }else if (SOC_IS_ROBO53242(unit)||SOC_IS_ROBO53262(unit)) {
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, PORT_EGCTLr, port, 0);
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PORT_EGCTLr);
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
        temp = SOC_PBMP_WORD_GET(bmp, 0);
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_EGCTLr, (uint32 *)&reg_value64, PORT_EGRESS_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
    } else {
...
#endif /* STP_BY_EAP_BLK */
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_VLAN_CTLr, &reg_value, PORT_EGRESS_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, &reg_value,reg_len));
    }
...
drv_vlan_prop_set(int unit, uint32 prop_type, uint32 prop_val)
{
    ...
    switch (prop_type) {
      case DRV_VLAN_PROP_DROP_VTABLE_MISS:
