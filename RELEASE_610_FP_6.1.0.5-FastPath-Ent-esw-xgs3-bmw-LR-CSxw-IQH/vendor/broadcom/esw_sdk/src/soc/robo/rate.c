/*
 * $Id: rate.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 *
...
_drv_port_irc_set(int unit, uint32 port, uint32 limit, uint32 burst_size)
{
    ...
    if (limit == 0) { /* Disable ingress rate control */
        ...
        if (limit <= 1792) { /* 64KB ~ 1.792MB */
            ...
            temp = (limit /1024 ) + 27;
        } else if (limit <= 1024000){ /* 104MB ~ 1000MB */
            temp = (limit /8192) + 115;
        } else {
            temp = 255;
        }
    ...
    uint32 limit, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, 
    "drv_rate_set: unit = %d, bmp = %x, %s, limit = %dK, burst size = %dKB\n",
        unit, SOC_PBMP_WORD_GET(bmp, 0), 
        (direction - 1) ? "EGRESS" : "INGRESS", limit, burst_size);
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            ...
            PBMP_ITER(bmp, port) {
                if (SOC_IS_ROBO5348(unit)||SOC_IS_ROBO5347(unit)) {
                    SOC_IF_ERROR_RETURN(
                        _drv_bcm5348_port_irc_set(unit, port, limit, 
                            burst_size));
                } else {
                    SOC_IF_ERROR_RETURN(
                        _drv_port_irc_set(unit, port, limit, burst_size));
                }
            }
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            ...
            if (SOC_IS_ROBO5324(unit)||SOC_IS_ROBO5398(unit)||
                ...
                PBMP_ITER(bmp, port) {
                    SOC_IF_ERROR_RETURN(
                        _drv_port_erc_set(unit, port, limit, burst_size));
                }                
            }
    ...
    uint32 *limit, uint32 *burst_size)
{
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            if (SOC_IS_ROBO5348(unit)||SOC_IS_ROBO5347(unit)) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm5348_port_irc_get(unit, port, limit, burst_size));
            } else {
                SOC_IF_ERROR_RETURN(
                    _drv_port_irc_get(unit, port, limit, burst_size));
            }
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            if (SOC_IS_ROBODINO(unit)){
            ...
            if (SOC_IS_ROBO5324(unit)||SOC_IS_ROBO5398(unit)||
                ...
                SOC_IS_ROBO5347(unit) || SOC_IS_ROBO5395(unit)){
                SOC_IF_ERROR_RETURN(
                    _drv_port_erc_get(unit, port, limit, burst_size));
            }
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
            return SOC_E_UNAVAIL;
    ...
    soc_cm_debug(DK_PORT, 
    "drv_rate_get: unit = %d, port = %d, %s, limit = %dK, burst size = %dKB\n",
    unit, port, (direction - 1) ? "EGRESS" : "INGRESS", *limit, *burst_size);
