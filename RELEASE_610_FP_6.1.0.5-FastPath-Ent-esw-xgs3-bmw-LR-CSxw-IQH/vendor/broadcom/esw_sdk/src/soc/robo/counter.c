/*
 * $Id: counter.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
static int bcm53242_flag = SOC_BCM53242;
...
{
    return ((port * SOC_MAX_COUNTER_NUM) + 
...
_soc_robo_counter_illegal(int unit, soc_reg_t ctr_reg)
{
    soc_cm_print("soc_robo_counter_get: unit %d: ctr_reg %d (%s) is not a counter\n",
         unit, ctr_reg, SOC_ROBO_REG_NAME(unit, ctr_reg));
    /* assert(0); */
}
         ...
         int zero, uint64 *val)
{
    ...
    if ((SOC_IS_ROBO(unit) && (SOC_IS_ROBO5397(unit) || 
        ...
        counter_index = (shift_port * SOC_MAX_COUNTER_NUM) +
              (SOC_REG_INFO(unit, ctr_reg).ctr_idx);
    } else if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
        if (bcm53242_flag == SOC_BCM53202) {
            if (port >= 24) {
                shift_port = port-16;
            } else {
                shift_port = port;
            }
        } else if (bcm53242_flag == SOC_BCM53212) {
            if (port >= 24) {
                shift_port = port-8;
            } else {
                shift_port = port;
         ...
         uint64 *val)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, MIB_SNAPSHOT_CTLr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, MIB_SNAPSHOT_CTLr);
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, MIB_SNAPSHOT_CTLr, &reg_value, SNAPSHOT_PORTf, &temp));
    temp = 1;
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, MIB_SNAPSHOT_CTLr, &reg_value, SNAPSHOT_STDONEf, &temp));
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, ctr_reg, 0, 0);
    /* 
    ...
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, ctr_reg);
    if ((rv = (DRV_SERVICES(unit)->reg_read)
...
drv_counter_set(int unit, soc_pbmp_t bmp, uint32 counter_type, uint64 val)
{
    ...
    PBMP_ITER(bmp, port) {
        switch (counter_type) {
            ...
            soc_robo_counter_set(unit, port, RXEXCESSSIZEDISCr, val);
            ...
            if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
                soc_robo_counter_set(unit, port, PKTS1024TOMAXOCTETSr,val); 
            } else {
                soc_robo_counter_set(unit, port, PKTS1024TO1522OCTETSr,val);    
            }
            break;
...
drv_counter_get(int unit, uint32 port, uint32 counter_type, uint64 *val)
{
    ...
    switch (counter_type) 
    {
        ...
        case DRV_SNMP_IF_IN_DISCARDS:
            /* so far (Sep,27,2007), all robo chips on this mib counter is 
             *  not work. Here we return UNAVAIL for some listed robo chips.
             */
            if (SOC_IS_ROBO5324(unit) || SOC_IS_ROBO5324_A1(unit) ||
                SOC_IS_ROBO5396(unit)|| SOC_IS_ROBO5398(unit) || 
                SOC_IS_ROBO5397(unit)|| SOC_IS_ROBO5389(unit) || 
        ...
        /* RXEXCESSSIZEDISCr replaced by JUMBOPKTr for BCM53115, BCM53118 */
        } else {
            soc_robo_counter_get(unit, port, RXEXCESSSIZEDISCr, &count_tmp);
            COMPILER_64_ADD_64(count, count_tmp);
        }
        ...
        case DRV_SNMP_ETHER_STATS_PKTS_1024TO1518_OCT:
        if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
            soc_robo_counter_get(unit, port, PKTS1024TOMAXOCTETSr,&count);
        } else if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)) {
            soc_robo_counter_get(unit, port, PKTS1024TOMAXPKTOCTETSr,&count);            
        } else {
            soc_robo_counter_get(unit, port, PKTS1024TO1522OCTETSr,&count); 
        }
        break;
...
drv_counter_reset(int unit)
{
    ...
    if (SOC_IS_ROBO5396(unit)){
        PBMP_ITER(soc->counter_pbmp, port) {
            for (i = 0; i < PORT_CTR_NUM(unit, port); i++) {
                ...
                ctraddr = (DRV_SERVICES(unit)->reg_addr)
                    (unit, ctr_ref->reg, port, 0);
                ctr_new32 = 0;
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
    ...
    if (SOC_IS_ROBO53242(unit)||SOC_IS_ROBO53262(unit)){
        ...
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, RST_TABLE_MEMr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, RST_TABLE_MEMr, 0, 0);
        ...
        (DRV_SERVICES(unit)->reg_field_set)
            (unit, RST_TABLE_MEMr, &reg_value, RST_MIB_CNTf, &temp);
        ...
        temp = 1;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, GMNGCFGr, &reg_value, RST_MIB_CNTf, &temp));
    ...
    if (SOC_IS_ROBO5389(unit)){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &model, reg_len));
        reg_len = (DRV_SERVICES(unit)->reg_length_get)
            (unit, CHIP_REVIDr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)
            (unit, CHIP_REVIDr, 0, 0);
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &rev, reg_len));
    ...
    if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
        ...
        reg_value >>= 1;
        if (reg_value == 0x5) {
            bcm53242_flag = SOC_BCM53212;
        } else if (reg_value == 0x3) {
            bcm53242_flag = SOC_BCM53202;
        }
    }
    ...
    _soc_robo_counter_sw_table_set(unit, pbmp, 0);
    ...
    PBMP_ITER(soc->counter_pbmp, port) {
        for (i = 0; i < PORT_CTR_NUM(unit, port); i++) {
            ...
            if (SOC_IS_ROBO5397(unit)||SOC_IS_ROBO5395(unit) ||
                SOC_IS_ROBO53115(unit) || bcm5389a1_flag){
                /* 
                 * For BCM5397, BCM5395, CPU port (port 8, parsed from portbitmap)
                 * should change its value as 5 when calculate counter_index.
                 * For BCM53115, CPU port (port 8, parsed from portbitmap)
            ...
            if (ctr_ref->reg == TXPAUSEPKTSr) {
                ...
                (DRV_SERVICES(unit)->port_get)
                        (unit, port, DRV_PORT_PROP_DUPLEX, &fd);
                if (fd == DRV_PORT_STATUS_DUPLEX_FULL) {
                    ...
                    soc->counter_delta[counter_index] = 0;
                }
                soc->counter_hw_val[counter_index] = ctr_new;
            } else {
                COMPILER_64_ADD_64(soc->counter_sw_val[counter_index], ctr_diff);
...
soc_robo_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    if(SOC_IS_ROBO5396(unit)){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            ...
            (unit, GMNGCFGr, 0, 0), &val, (DRV_SERVICES(unit)->reg_length_get)
            (unit, GMNGCFGr)));
        temp = 1;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, GMNGCFGr, &val, RST_MIB_CNTf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, (DRV_SERVICES(unit)->reg_addr)
            (unit, GMNGCFGr, 0, 0), &val, (DRV_SERVICES(unit)->reg_length_get)
            (unit, GMNGCFGr)));
        ...
        switch(soc->counter_flags ) {
         ...
         SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
                    (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, MIB_SELECTr, 0, 0), &val64, 
                    (DRV_SERVICES(unit)->reg_length_get)
                    (unit, MIB_SELECTr)));    
        PBMP_ITER(soc->counter_pbmp, port) {
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, MIB_SELECTr, (uint32 *)&val64, 
                    mib_sel_field[port], &temp));
        }
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
                    (unit, (DRV_SERVICES(unit)->reg_addr)
                    (unit, MIB_SELECTr, 0, 0), (uint32 *)&val64, 
                    (DRV_SERVICES(unit)->reg_length_get)
                    (unit, MIB_SELECTr))); 
...
soc_robo_counter_attach(int unit)
{
    ...
    n_bytes = portNum * SOC_MAX_COUNTER_NUM * sizeof(uint64);
