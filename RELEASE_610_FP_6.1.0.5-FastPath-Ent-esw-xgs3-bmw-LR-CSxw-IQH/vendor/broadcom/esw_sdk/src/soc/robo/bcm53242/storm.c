/*
 * $Id: storm.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 *
    ...
    uint32 type, uint32 enable)
{
    ...
    if (type & DRV_STORM_CONTROL_BUCKET_2) {
        ...
        bucket_field = IRC_PKT_MASK_B2f;
    } else {
        bucket_reg = RCM_DATA1r;
        bucket_field = IRC_PKT_MASK_B1f;
    }
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, RCM_PORTr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, RCM_PORTr);
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, RCM_PORTr, &acc_ctrl, RCM_PORTf, &port);
    if ((rv = (DRV_SERVICES(unit)->reg_write)
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, RCM_CTLr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, RCM_CTLr);
    ...
    temp = MEM_TABLE_READ;
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, RCM_CTLr, &acc_ctrl, RCM_RWf, &temp);
        
    /* 1'b0 : Ingress Rate Control Memory access */
    temp = 0;
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, RCM_CTLr, &acc_ctrl, IE_INDf, &temp);
    
    /* Start Read Process */
    temp = 1;
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, RCM_CTLr, &acc_ctrl, RCM_RW_STRTDNf, &temp);
    if ((rv = (DRV_SERVICES(unit)->reg_write)
        (unit, reg_addr, &acc_ctrl, reg_len)) < 0) {
        goto storm_type_enable_set_exit;
    }
...
drv_bcm53242_storm_control_enable_set(int unit, uint32 port, uint8 enable)
{
    ...
    zero_value = 0;
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, RCM_DATA0r, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, RCM_DATA0r);
    if ((rv = (DRV_SERVICES(unit)->reg_write)
        (unit, reg_addr, &zero_value, reg_len)) < 0) {
    ...
    temp = 1;
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, RCM_DATA0r, &reg_value, IRC_DROP_ENf, &temp));
    
    /* Enable Ingress rate control bit */
    (DRV_SERVICES(unit)->reg_field_get)
        (unit, RCM_DATA0r, &reg_value, IE_RC_ENf, &temp);
    if (!temp) {
        temp = 1;
        (DRV_SERVICES(unit)->reg_field_set)
            (unit, RCM_DATA0r, &reg_value, IE_RC_ENf, &temp);
     ...
     */
    (DRV_SERVICES(unit)->reg_field_get)
        (unit, RCM_DATA0r, &reg_value, REF_UNITf, &temp);
    if (!temp) {
    ...
    (DRV_SERVICES(unit)->reg_field_get)
        (unit, RCM_DATA0r, &reg_value, REF_CNTSf, &temp);
    if (!temp) {
    ...
    uint32 limit, uint32 burst_size)
{
    ...
    if (SOC_PBMP_MEMBER(PBMP_GE_ALL(unit), port)) {
        ...
        if (limit == 0) { /* Disable storm suppression type */
            ...
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, bucket_reg, &reg_value, BUCKET_SIZEf, &temp));
            ...
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, bucket_reg, &reg_value, REF_UNITf, &ref_u));

            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, bucket_reg, &reg_value, REF_CNTSf, &temp));
    ...
    uint32 *limit, uint32 *burst_size)
{
    ...
    if (!loc_type) {
        *limit = 0;
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, RCM_DATA1r, &reg_value, REF_UNITf, &ref_u));
         
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, RCM_DATA1r, &reg_value, REF_CNTSf, &temp));
