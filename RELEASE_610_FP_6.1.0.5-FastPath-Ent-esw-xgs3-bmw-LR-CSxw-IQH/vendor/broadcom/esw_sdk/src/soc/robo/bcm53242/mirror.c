 ...
 */
int 
drv_bcm53242_mirror_set(int unit, uint32 enable, uint32 monitor_port, 
    soc_pbmp_t ingress_bmp, soc_pbmp_t egress_bmp)
{
    ...
    if (enable) { /* enable mirror */
        ...
        if (SOC_INFO(unit).port_num > 32) {
            COMPILER_64_SET(temp, 0 , 1);
            COMPILER_64_SHL(temp, monitor_port);

            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_set)
                (unit, MIRCAPCTLr, (uint32 *)&reg_value, SMIR_CAP_PORTf, (uint32 *)&temp));
        } else {
            temp32 = 0x1 << monitor_port;
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_set)
        ...
        SOC_IF_ERROR_RETURN(
            (DRV_SERVICES(unit)->reg_field_set)
            (unit, MIRCAPCTLr, (uint32 *)&reg_value, MIR_ENf, (uint32 *)&temp));
        SOC_IF_ERROR_RETURN(
            (DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, (uint32 *)&reg_value,reg_len));
    }
 ...
 */
int 
drv_bcm53242_mirror_get(int unit, uint32 *enable, uint32 *monitor_port, 
    soc_pbmp_t *ingress_bmp, soc_pbmp_t *egress_bmp)
{
    uint32      reg_addr, reg_len;
    uint64      reg_value, temp;
    ...
    if (temp32) {
        ...
        *enable = TRUE;
        ...
        if (SOC_INFO(unit).port_num > 32) {
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_get)
                (unit, MIRCAPCTLr, (uint32 *)&reg_value, 
                SMIR_CAP_PORTf, (uint32 *)&temp));
            soc_robo_64_val_to_pbmp(unit, &pbmp, temp);
        } else {
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_get)
                (unit, MIRCAPCTLr, (uint32 *)&reg_value, 
                SMIR_CAP_PORTf, (uint32 *)&temp32));
            SOC_PBMP_WORD_SET(pbmp, 0, temp32);
        }
