/*
 * $Id: sec.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 *
...
_drv_bcm53242_security_init(int unit)
{
    ...
    temp = 0;
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, EAP_GLO_CONr, &reg_value, EN_DIP_0f, &temp));
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, EAP_GLO_CONr, &reg_value, EN_DIP_1f, &temp));
...
{
    int             rv = SOC_E_NONE;
    uint32          reg_addr, reg_value, temp, flag = 0;
    int             reg_len;
        ...
        case DRV_SECURITY_PORT_UNCONTROLLED:
            ...
            */                
            ...
            PBMP_ITER(bmp, port) {
                ...
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, PORT_SEC_CONr, &reg_value, EAP_PORT_BLKf, &temp));
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, reg_addr, &reg_value, reg_len)) < 0) {
                    return rv;
                }
