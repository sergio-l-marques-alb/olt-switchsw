/*
 * $Id: cfp.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
...
/* 1Gb */
#define CFP_5348_METER_RATE_MAX (1024 * 1024)
/* 64Kbits/sec */
#define CFP_5348_METER_RATE_MIN 64
/* 128Mb */
#define CFP_5348_METER_BURST_MAX (128 * 1024)   
/* 64K */
...
_drv_cfp_meter_rate2chip(int unit, uint32 kbits_sec, uint32 *chip_val)
{
    ...
    if (kbits_sec > 1024 * 100 ) {
        *chip_val = (kbits_sec / (1024 * 8)) + 115;
    } else if (kbits_sec > 1792) {
    /* case 2 : greater than 1792K */
        *chip_val = (kbits_sec / 1024 ) + 27;
    } else {
...
{
    if (chip_val > 127) {
        *kbits_sec = (chip_val - 115) * 8 * 1024;
    } else if (chip_val > 28) {
        *kbits_sec = (chip_val - 27) * 1024;
    } else {
    ...
    switch (control_type) {
        case DRV_CFP_ENABLE:
            ...
            (DRV_SERVICES(unit)->reg_field_get)
                (unit, CFP_GLOBAL_CTLr, (uint32 *)&reg_64, 
                EN_CFP_MAPf, (uint32 *)&fld_64);
            COMPILER_64_ZERO(temp);
            fld_32 = 1;
            COMPILER_64_SET(temp, 0, fld_32);
            COMPILER_64_SHL(temp, param1);
            if (param2) {
                COMPILER_64_OR(fld_64, temp);
            } else {
                COMPILER_64_XOR(fld_64, temp);
            }
            (DRV_SERVICES(unit)->reg_field_set)
                (unit, CFP_GLOBAL_CTLr, (uint32 *)&reg_64, 
                EN_CFP_MAPf, (uint32 *)&fld_64);
