/*
 * $Id: port.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
#define SOC_MAX_NUM_UNITS        1    /* TBD */
...
static port_t   *soc_port_info[SOC_MAX_NUM_UNITS];

#define MEDIUM_MODE_SET(unit, port, medium) \
    PORT((unit), (port)).cur_medium = (medium)
#define PORT_MEDIUM_MODE(unit, port) \
    PORT((unit), (port)).cur_medium
#define IS_MEDIUM_MODE_COPPER(unit, port) \
    (PORT_MEDIUM_MODE((unit), (port)) == SOC_PORT_MEDIUM_COPPER) ? \
    TRUE : FALSE

#define PORT(unit, port)    soc_port_info[unit][port]
#define MAC_DRIVER(u, p)  (PORT(u, p).p_mac)
#define DRV_MAX_NUM_UNITS        16
#define PORT_INIT(unit) if (soc_port_info[unit] == NULL) { return SOC_E_INIT; }
    
/*  !!!! REMOVED already !!!! (for new robo linkscan)
 *  Function : _drv_port_ge_update
 *
 *  Purpose :
 *      Program MAC SW override regs when GE ports medium changes.
 *
 *  Parameters :
 *      unit   :   unit id
 *      port   :   port id. Should only GE port id.
 *      medium :   port medium.
 *
 *  Return :
 *      SOC_E_XXX
...
int
soc_robo_53242_phy_reset_callbck(int unit, _shr_port_t port, 
                                        void *user_data)
{
    phy_ctrl_t        *pc;

    phy_fe_ge_reset(unit, port, NULL);
    ...
    soc_cm_debug(DK_PORT,"soc_robo_53242_phy_reset_callbck port %d\n",port);
    return (SOC_E_NONE);
}
...
_drv_port_probe(int unit, soc_port_t p, int *okay)
{
    ...
    *okay = FALSE;
    ...
    if (soc_port_info[unit] == NULL) 
    {
        soc_port_info[unit] = 
            sal_alloc(sizeof(port_t) * SOC_ROBO_MAX_NUM_PORTS, "soc_port_info");
        if (soc_port_info[unit] == NULL) 
        {
            return SOC_E_MEMORY;
        }
        sal_memset(soc_port_info[unit], 0, sizeof(port_t) * SOC_ROBO_MAX_NUM_PORTS);
    }
    ...
    if (SOC_IS_ROBO53242(unit)) {
        ...
        if ((dev_id == BCM53242_DEVICE_ID) && (rev_id == BCM53242_B0_REV_ID)
                && IS_GE_PORT(unit, p)){
                soc_phy_reset_register(unit, p, 
                    soc_robo_53242_phy_reset_callbck, NULL, FALSE);
        }
    }            
    ...
    (DRV_SERVICES(unit)->port_get) \
            (unit, p, DRV_PORT_PROP_PHY_MEDIUM, &default_port_medium);
    PORT_MEDIUM_MODE(unit, p) = default_port_medium;
    ...
    if ((rv = MAC_INIT(PORT(unit, p).p_mac, unit, p)) < 0) {
        soc_cm_debug(DK_WARN | DK_PORT,
...
_drv_port_security_mode_set(int unit, uint32 port, uint32 mode, uint32 sa_num)
{
    ...
    temp = (mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) ?
            sa_num : 0x1fff;
    ...
    if (mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) {
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, MAC_SEC_CON4r, &reg_value, SW_SA_CNT_RSTf, &temp));
        if ((rv = (DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
    ...
    uint32 *prop_val)
{
    ...
    if ((mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) && (*prop_val)) {
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PORT_MAX_LEARNr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)
            (unit, PORT_MAX_LEARNr, port, 0);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
...
_drv_bcm5348_port_security_mode_set(int unit, uint32 port, uint32 mode, uint32 sa_num)
{
    ...
    if (mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) {
        ...
        if (SOC_IS_ROBO5348(unit)) {
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                        (unit, MAC_SEC_CON3r, (uint32 *)&reg_value64, 
                        SW_SA_CNT_RSTf, (uint32 *)&temp64));
        ...
        if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, reg_addr, &reg_value64, reg_len)) < 0) {
            return rv;
        }
    }
...
_drv_bcm5398_port_security_mode_set(int unit, uint32 port, uint32 mode, uint32 sa_num)
{
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, PORT_EAP_CONr, (uint32 *)&reg_value64, EAP_MODEf, &temp));
    if ((rv = (DRV_SERVICES(unit)->reg_write)
        (unit, reg_addr, (uint32 *)&reg_value64, reg_len)) < 0) {
        return rv;
    }
...
_drv_bcm53242_port_security_mode_set(int unit, uint32 port, uint32 mode, uint32 sa_num)
{
    ...
    if (change_eap) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, PORT_SEC_CONr, &reg_value, EAP_MODEf, &temp));
    }
    if (change_trap) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, PORT_SEC_CONr, &reg_value, SA_VIO_OPTf, &trap));
    }
    if ((rv = (DRV_SERVICES(unit)->reg_write)
        (unit, reg_addr, &reg_value, reg_len)) < 0) {
        return rv;
     ...
     */
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PORT_MAX_LEARNr);
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, PORT_MAX_LEARNr, port, 0);
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, reg_addr, &reg_value, reg_len)) < 0) {
        return rv;
    }
    ...
    if (mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) {
         ...
         *      2. set back to enable normal learned SA counter. 
         */
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, LRN_CNT_CTLr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, LRN_CNT_CTLr, 0, 0);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
        ...
        temp = port;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, LRN_CNT_CTLr, &reg_value, PORT_NUM_Rf, &temp));

        /* set access control reset bit to 0 with corresponding ports */
        temp = 3;
        (DRV_SERVICES(unit)->reg_field_set)
            (unit, LRN_CNT_CTLr, &reg_value, ACC_CTLf, &temp);
        
        /* start learning counter control Process */
        temp = 1;
        (DRV_SERVICES(unit)->reg_field_set)
            (unit, LRN_CNT_CTLr, &reg_value, START_DONEf, &temp);
        
        if ((rv = (DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            goto access_contol_exit;
        }
    ...
    uint32 *prop_val)
{
    ...
    *prop_val = FALSE;
    ...
    if ((mode == DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_NUM) && (*prop_val)) {
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PORT_MAX_LEARNr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, DYN_MAX_MAC_NOf, port, 0);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
    ...
    switch (property) {
        case DRV_PORT_PROP_ENABLE_RX:
            if (IS_FE_PORT(unit, port)) {
                addr = (DRV_SERVICES(unit)->reg_addr)(unit, TH_PCTLr, port, 0);
                length = (DRV_SERVICES(unit)->reg_length_get)(unit, TH_PCTLr);
                if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, addr, &port_ctrl, length)) < 0) {
                    return rv;
                }
                if (enable) {
                    temp = 0;
                ...
                if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)) {
                    (DRV_SERVICES(unit)->reg_field_set)
                        (unit, G_PCTLr, &port_ctrl, RX_DISf, &temp);
                } else {
                    (DRV_SERVICES(unit)->reg_field_set)
                        (unit, G_PCTLr, &port_ctrl, MIRX_DISf, &temp);
                }
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, addr, &port_ctrl, length)) < 0) {
                    return rv;
        ...
        case DRV_PORT_PROP_ENABLE_TX:
            if (IS_FE_PORT(unit, port)) {
                ...
                if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)) {
                    (DRV_SERVICES(unit)->reg_field_set)
                        (unit, G_PCTLr, &port_ctrl, TX_DISf, &temp);
                } else {
                    (DRV_SERVICES(unit)->reg_field_set)
                ...
                if (SOC_IS_ROBO5396(unit)){
                    ...
                    (DRV_SERVICES(unit)->reg_field_set)
                            (unit, TXQ_FLUSH_MODEr, &port_ctrl, 
                            DIS_NEW_TXDISf, &temp);
        ...
        case DRV_PORT_PROP_ENABLE_DROP_NON1Q:
            ...
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit) ||
                ...
                soc_robo_64_val_to_pbmp(unit, &pbmp, vlan_ctrl3_64);
                    ...
                    (DRV_SERVICES(unit)->reg_field_get)
                        (unit, VLAN_CTRL3r, (uint32 *)&vlan_ctrl3_64, 
                        EN_DROP_NON1Qf, &temp);
                    SOC_PBMP_WORD_SET(pbmp, 0, temp);
                }
                if (enable) {
                ...
                /* bcm53115, bcm53118 can apply this section */
                if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, addr, &vlan_ctrl3, length)) < 0) {
                    return rv;
                }
                if (enable) {
                    vlan_ctrl3 |= ( 1 << port);
        ...
        case DRV_PORT_PROP_PAUSE_FRAME_BYPASS_TX:
            ...
            if (property == DRV_PORT_PROP_PAUSE_FRAME_BYPASS_RX) {
                reg_index = RX_PAUSE_PASSr;
                fld_index = RX_PAUSE_PASSf;
            } else {
                reg_index = TX_PAUSE_PASSr;
                fld_index = TX_PAUSE_PASSf;
            }
            addr =  (DRV_SERVICES(unit)->reg_addr)
                (unit, reg_index, 0, 0);
            length = (DRV_SERVICES(unit)->reg_length_get)
        ...
        case DRV_PORT_PROP_DTAG_MODE:
            if (SOC_IS_ROBO5348(unit) ||SOC_IS_ROBO5347(unit) ||
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                    (unit, DTAG_GLO_CTLr, &reg_value, EN_DTAG_ISPf, &enable);
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, addr, &reg_value, length)) < 0) {
                    return rv;
                }
                ...
                (DRV_SERVICES(unit)->vlan_property_set)
                    (unit, DRV_VLAN_PROP_IDT_MODE_ENABLE, enable);
#else
                (DRV_SERVICES(unit)->vlan_property_set)
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                    (unit, VLAN_CTRL4r, &reg_value, EN_DOU_TAGf, &enable);
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, addr, &reg_value, length)) < 0) {
                    return rv;
                }
        ...
        case DRV_PORT_PROP_DTAG_ISP_PORT:
            ...
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)) {
                ...
                if (SOC_IS_ROBO5348(unit)) {
                (DRV_SERVICES(unit)->reg_field_get)
                        (unit, ISP_PORT_SEL_PBMr, (uint32 *)&reg_value64, 
                        ISP_PORT_PBMPf, (uint32 *)&temp64);
                soc_robo_64_val_to_pbmp(unit, &pbmp, temp64);
                } else { /* 5347 */
                    (DRV_SERVICES(unit)->reg_field_get)
                    (unit, ISP_PORT_SEL_PBMr, (uint32 *)&reg_value64, 
                ...
                */
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, ISP_SEL_PORTMAPr, (uint32 *)&reg_value64, 
                    ISP_PORT_PBMPf, &temp);
                SOC_PBMP_WORD_SET(pbmp, 0, temp);
                ...
                */
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, ISP_PORT_SEL_PBMr, &reg_value, ISP_PORT_PBMPf, &temp);
                if (enable) {
                    temp |= (1 << port);
                } else {
                    temp &= ~(1 << port);
        ...
        case DRV_PORT_PROP_EGRESS_PCP_REMARK:
            ...
            if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)){
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                    (unit, TRREG_CTRLr, &reg_value, PCP_RMK_ENf, &temp);
        ...
        case DRV_PORT_PROP_EGRESS_CFI_REMARK:
            ...
            if (SOC_IS_ROBO53115(unit)){
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                    (unit, TRREG_CTRLr, &reg_value, CFI_RMK_ENf, &temp);
    ...
    switch (property) {
        ...
        case DRV_PORT_PROP_DTAG_MODE:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit) ||
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, DTAG_GLO_CTLr, &reg_value, EN_DTAG_ISPf, &temp);
            } else if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)){
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, VLAN_CTRL4r, &reg_value, EN_DOU_TAGf, &temp);
            }
            *enable = temp;
            break;
        case DRV_PORT_PROP_DTAG_ISP_PORT:
            ...
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)) {
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, ISP_SEL_PORTMAPr,&reg_value, 
                    ISP_PORTMAPf, &temp);
...
_drv_sample_rate_get(int unit, uint32 val)
{
    ...
    SOC_FIND_FIELD(INGRESS_CFGf,
        SOC_REG_INFO(unit, INGRESS_RMONr).fields,
        SOC_REG_INFO(unit, INGRESS_RMONr).nFields,
        finfop);
    assert(finfop);

    /* Get the maximum value can be set in the register's field */
    cap = 1;
    for (i= 0; i < finfop->len; i++) {
        cap *= 2;
    }
...
drv_port_pri_mapop_set(int unit, int port, int op_type, 
                uint32 pri_old, uint32 pri_new, uint32 cfi_new)
{
    int     rv, length;
    uint32  addr, temp;
    uint32  reg_index = 0, fld_index = 0;
    ...
    switch (op_type) {
        ...
        case DRV_PORT_OP_NORMAL_TC2PCP:
             ...
             */
            if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)){
                reg_index = EGRESS_NRESE_PKT_TC2PCP_MAPr;
                
                /* assigning the field-id */
                switch(pri_old) {
                    case 0:
                        fld_index = PCP_FOR_RV0_TC0f;
                        break;
                    case 1:
                        fld_index = PCP_FOR_RV0_TC1f;
                        break;
                    case 2:
                        fld_index = PCP_FOR_RV0_TC2f;
                        break;
                    case 3:
                        fld_index = PCP_FOR_RV0_TC3f;
                        break;
                    case 4:
                        fld_index = PCP_FOR_RV0_TC4f;
                        break;
                    case 5:
                        fld_index = PCP_FOR_RV0_TC5f;
                        break;
                    case 6:
                        fld_index = PCP_FOR_RV0_TC6f;
                        break;
                    case 7:
                        fld_index = PCP_FOR_RV0_TC7f;
                        break;
                    default :
                        return SOC_E_PARAM;
                        break;
                ...
                addr =  (DRV_SERVICES(unit)->reg_addr)
                        (unit, EGRESS_NRESE_PKT_TC2PCP_MAPr, port, 0);
                length = (DRV_SERVICES(unit)->reg_length_get)
                        (unit, EGRESS_NRESE_PKT_TC2PCP_MAPr);
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                        (unit, reg_index, (uint32 *)&reg_value64, 
                        fld_index, &temp);
        ...
        case DRV_PORT_OP_OUTBAND_TC2PCP:
            ...
            if (SOC_IS_ROBO53115(unit)){
                ...
                switch(pri_old) {
                    case 0:
                        fld_index = PCP_FOR_RV1_TC0f;
                        break;
                    case 1:
                        fld_index = PCP_FOR_RV1_TC1f;
                        break;
                    case 2:
                        fld_index = PCP_FOR_RV1_TC2f;
                        break;
                    case 3:
                        fld_index = PCP_FOR_RV1_TC3f;
                        break;
                    case 4:
                        fld_index = PCP_FOR_RV1_TC4f;
                        break;
                    case 5:
                        fld_index = PCP_FOR_RV1_TC5f;
                        break;
                    case 6:
                        fld_index = PCP_FOR_RV1_TC6f;
                        break;
                    case 7:
                        fld_index = PCP_FOR_RV1_TC7f;
                        break;
                    default :
                        return SOC_E_PARAM;
                        break;
 ...
 */
 int 
 drv_port_oper_mode_set(int unit, uint32 bmp, uint32 oper_mode)
{
    ...
    switch (prop_type) {
        case DRV_PORT_PROP_SPEED:
            ...
            PBMP_ITER(bmp, port) {
                ...
                if (PORT(unit, port).p_mac != NULL) {
                    /* Set PHY registers anyway. */
                    if (PORT(unit, port).p_mac->md_speed_set != NULL) {
                        rv = MAC_SPEED_SET(
                            MAC_DRIVER(unit, port), unit, port, temp);
                    } else {
                        rv = SOC_E_UNAVAIL;
                    }
                } else {
        ...
        case DRV_PORT_PROP_DUPLEX:
            ...
            PBMP_ITER(bmp, port) {
                ...
                if (PORT(unit, port).p_mac != NULL) {
                    /* Set PHY registers anyway. */
                    if (PORT(unit, port).p_mac->md_duplex_set != NULL) {
                        rv = MAC_DUPLEX_SET(
                            MAC_DRIVER(unit, port), unit, port, temp);
                    } else {
                        rv = SOC_E_UNAVAIL;
        ...
        case DRV_PORT_PROP_TX_PAUSE:
            ...
            PBMP_ITER(bmp, port) {
                /* set TX PAUSE */
                if (PORT(unit, port).p_mac != NULL) {
                    if (PORT(unit, port).p_mac->md_pause_set != NULL) {
                        rv = MAC_PAUSE_SET(
                            MAC_DRIVER(unit, port), unit, port, prop_val, -1);
                    } else {
                        rv = SOC_E_UNAVAIL;
                    }
                } else {
        ...
        case DRV_PORT_PROP_RX_PAUSE:
            ...
            PBMP_ITER(bmp, port) {
                ...
                if (PORT(unit, port).p_mac != NULL) {
                    if (PORT(unit, port).p_mac->md_pause_set != NULL) {
                        rv = MAC_PAUSE_SET(
                            MAC_DRIVER(unit, port), unit, port, -1, prop_val);
                    } else {
                        rv = SOC_E_UNAVAIL;
                    }
                } else {
        ...
        case DRV_PORT_PROP_MAC_ENABLE:
            ...
            PBMP_ITER(bmp, port) {
                /* MAC register(s) should be set also */
                if (PORT(unit, port).p_mac != NULL) {
                    if (PORT(unit, port).p_mac->md_enable_set != NULL) {
                        rv = MAC_ENABLE_SET(
                            MAC_DRIVER(unit, port), unit, port, prop_val);
                    } else {
                        rv = SOC_E_UNAVAIL;
                    }
                } else {
        ...
        case DRV_PORT_PROP_IPG_FE:
            ...
            if (SOC_PBMP_EQ(bmp, PBMP_ALL(unit))) { /* per system */
                ...
                if (prop_val > 92) {
                    temp = 3;
                    (DRV_SERVICES(unit)->reg_field_set)
                        (unit, SWMODEr, &reg_value, IPGf, &temp);
                } else if (prop_val > 88) {
                    temp = 2;
                    (DRV_SERVICES(unit)->reg_field_set)
                        (unit, SWMODEr, &reg_value, IPGf, &temp);
        ...
        case DRV_PORT_PROP_MAC_LOOPBACK:
            ...
            PBMP_ITER(bmp, port) {
                if (PORT(unit, port).p_mac != NULL) {
                    if (PORT(unit, port).p_mac->md_lb_set != NULL) {
                        rv = MAC_LOOPBACK_SET(
                            MAC_DRIVER(unit,port), unit, port, prop_val);
                    } else {
                        rv = SOC_E_UNAVAIL;
        ...
        case DRV_PORT_PROP_DTAG_TPID:
            ...
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit) ||
                ...
                (DRV_SERVICES(unit)->reg_field_set)
                    (unit, ISP_VIDr, &reg_value, ISP_VLAN_DELIMITERf, &prop_val);
                if ((rv = (DRV_SERVICES(unit)->reg_write)
                    (unit, reg_addr, &reg_value, reg_len)) < 0) {
                    return rv;
                }
        ...
        case DRV_PORT_PROP_MAX_FRAME_SZ:
            PBMP_ITER(bmp, port) {
                if (PORT(unit, port).p_mac != NULL) {
                    if (PORT(unit, port).p_mac->md_frame_max_set != NULL) {
        ...
        case DRV_PORT_PROP_SFLOW_INGRESS_RATE:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                ...
                if (SOC_IS_ROBO5348(unit)) {
                    (DRV_SERVICES(unit)->reg_field_get)
                        (unit, INGRESS_RMONr, (uint32 *)&reg_value64, 
                ...
                if (prop_val) {
                    ...
                    PBMP_ITER(bmp, port) {
                        PORT(unit, port).ing_sample_rate = prop_val;
                    }
                    /* 
                    ...
                    PBMP_ITER(PBMP_ALL(unit), port) {
                        if (PORT(unit, port).ing_sample_rate) {
                            PORT(unit, port).ing_sample_rate = prop_val;
                        }
                    }
                } else {
        ...
        case DRV_PORT_PROP_SFLOW_EGRESS_RATE:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                ...
                if (prop_val) {
                     ...
                     */
                    PBMP_ITER(PBMP_ALL(unit), port) {
                        PORT(unit, port).eg_sample_rate = 0;
                    }

                    /* Only update software copy of the configured port. */
                    PORT(unit, conf_port).eg_sample_rate = prop_val;
                } else {
                    /* 
                     * Clear software copy of all ports. 
                     * Since there is only one port can be 
        ...
        case DRV_PORT_PROP_SFLOW_INGRESS_PRIO:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                 ...
                 */
                port = 0;
                PORT(unit, port).ing_sample_prio = prop_val;
            } else {
                return SOC_E_UNAVAIL;
            }
            break;
        case DRV_PORT_PROP_SFLOW_EGRESS_PRIO:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                 ...
                 */
                port = 0;
                PORT(unit, port).eg_sample_prio = prop_val;
            } else {
                return SOC_E_UNAVAIL;
            }
            break;
 ...
 *
 */
int drv_port_get(int unit, uint32 port, uint32 prop_type, uint32 *prop_val)
{
    int         rv = SOC_E_NONE;
    uint32     reg_value, reg_addr, reg_len, temp;
    ...
    switch (prop_type) {
        ...
        case DRV_PORT_PROP_TX_PAUSE:
            ...
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_pause_get != NULL) {
                    rv = MAC_PAUSE_GET(MAC_DRIVER(unit, port), 
        ...
        case DRV_PORT_PROP_PORT_ABILITY:
            ...
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_ability_get != NULL) {
                    rv = MAC_ABILITY_GET(
                        MAC_DRIVER(unit, port), unit, port, &mac_ability);
                } else {
                    rv = SOC_E_UNAVAIL;
                }
            } else {
        ...
        case DRV_PORT_PROP_INTERFACE:
            ...
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_interface_get != NULL) {
        ...
        case DRV_PORT_PROP_JAM:
            ...
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_pause_get != NULL) {
                    rv = MAC_PAUSE_GET(
                        MAC_DRIVER(unit, port), unit, port, (int *) prop_val,
                        &pause_rx);
                } else {
                    rv = SOC_E_UNAVAIL;
                }
        ...
        case DRV_PORT_PROP_MAC_LOOPBACK:
            ...
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_lb_get != NULL) {
                    rv = MAC_LOOPBACK_GET(
                        MAC_DRIVER(unit,port), unit, port, (int *) prop_val);
                } else {
                    rv = SOC_E_UNAVAIL;
        ...
        case DRV_PORT_PROP_DTAG_TPID:
            ...
            if (SOC_IS_ROBO5348(unit)||SOC_IS_ROBO5347(unit) ||
                ...
                (DRV_SERVICES(unit)->reg_field_get)
                    (unit, ISP_VIDr, &reg_value, ISP_VLAN_DELIMITERf, &temp);
                *prop_val = temp;
            } else {
                rv = SOC_E_UNAVAIL;
            }
        ...
        case DRV_PORT_PROP_MAX_FRAME_SZ:
            if (PORT(unit, port).p_mac != NULL) {
                if (PORT(unit, port).p_mac->md_frame_max_get != NULL) {
        ...
        case DRV_PORT_PROP_SFLOW_INGRESS_RATE:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                ...
                if (SOC_PBMP_MEMBER(tmp_pbmp, port)) {
                    *prop_val = PORT(unit, port).ing_sample_rate;
                } else {
                    *prop_val = 0;
                }
            } else {
                ...
                if (temp != port) {
                    *prop_val = 0;
                    ...
                    if (!temp) {
                        ...
                        *prop_val = PORT(unit, port).eg_sample_rate;
                    }
                }
            } else {
                return SOC_E_UNAVAIL;
        ...
        case DRV_PORT_PROP_SFLOW_INGRESS_PRIO:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
                *prop_val = PORT(unit, port).ing_sample_prio;
            } else {
                return SOC_E_UNAVAIL;
            }
            break;
        case DRV_PORT_PROP_SFLOW_EGRESS_PRIO:
            if (SOC_IS_ROBO5348(unit) || SOC_IS_ROBO5347(unit)||
                SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
                *prop_val = PORT(unit, port).eg_sample_prio;
            } else {
                return SOC_E_UNAVAIL;
            }
            break;
    ...
    switch (status_type)
    {
        case DRV_PORT_STATUS_LINK_UP:
            ...
            if (port_lb_phy){
                ...
                reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, LNKSTSr, 0, 0);
                reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, LNKSTSr);
                ...
                if (SOC_IS_ROBO5348(unit)||SOC_IS_ROBO5347(unit) ||
                    ...
                    if (SOC_IS_ROBO5348(unit)) {
                        (DRV_SERVICES(unit)->reg_field_get)
                            (unit, LNKSTSr, (uint32 *)&reg_value64, LNK_STSf, (uint32 *)&temp64);
                        soc_robo_64_val_to_pbmp(unit, &pbmp, temp64);
                    } else { /* 5347, 53242 */
                        (DRV_SERVICES(unit)->reg_field_get)
                            (unit, LNKSTSr, (uint32 *)&reg_value64, LNK_STSf, &temp);
                        SOC_PBMP_WORD_SET(pbmp, 0, temp);
                    }
                    if (SOC_PBMP_MEMBER(pbmp, port)) {
                        *val = TRUE;
           ...
           /* If link down, set default COPPER mode. */
            ...
            if (phy_medium != PORT_MEDIUM_MODE(unit, port)) {
                    
                    /* SW port medium update */
                    MEDIUM_MODE_SET(unit, port, phy_medium);
            }
        ...
        case DRV_PORT_STATUS_LINK_SPEED:
            ...
            if (SOC_IS_ROBO5324(unit)){
                 ...
                 */  
                int speed = 0;
                if ((rv = soc_phyctrl_speed_get(unit, port, &speed)) < 0 ){
                    return rv;
                }
                
                *val = (speed == 1000) ? DRV_PORT_STATUS_SPEED_1G :
        ...
        case DRV_PORT_STATUS_LINK_DUPLEX:
            ...
            if (SOC_IS_ROBO5324(unit)){
                reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, DUPSTSr, 0, 0);
                if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, reg_addr, &reg_value, 4)) < 0) {
                    return rv;
                }
                 ...
                 */  
                int duplex = 0;
                if ((rv = soc_phyctrl_duplex_get(unit, port, &duplex)) < 0 ){
                    return rv;
                }
...
drv_port_sw_mac_update(int unit, soc_pbmp_t bmp)
{
    ...
    PBMP_ITER(bmp, port) {
        SOC_IF_ERROR_RETURN
            (soc_phyctrl_speed_get(unit, port, &temp));
        SOC_IF_ERROR_RETURN
            (MAC_SPEED_SET(PORT(unit, port).p_mac, unit, port, temp));
            
        SOC_IF_ERROR_RETURN
            (soc_phyctrl_duplex_get(unit, port, &temp));
        SOC_IF_ERROR_RETURN
            (MAC_DUPLEX_SET(PORT(unit, port).p_mac, unit, port, temp));
    }
 ...
 */
int 
drv_bcm5396_port_get(int unit, uint32 port, 
                uint32 prop_type, uint32 *prop_val)
{
    int         rv = SOC_E_NONE, reg_len;
    uint32     reg_value, reg_addr, temp;
 ...
 */
int 
drv_bcm5398_port_get(int unit, uint32 port, 
                uint32 prop_type, uint32 *prop_val)
{
    int         rv = SOC_E_NONE;
    uint32     reg_value, reg_addr, reg_len, temp;
