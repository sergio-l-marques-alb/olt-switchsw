/*
 * $Id: cosq.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
 *
    ...
    (int unit, uint8 port, uint8 prio, uint8 queue_n)
{
    ...
    switch ( prio ) {        
        case 0:
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_set)(unit, TC2COS_MAPr, 
    ...
    (int unit, uint8 port, uint8 prio, uint8 *queue_n)
{
    ...
    switch ( prio ) {        
       case 0:
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_get)(unit, TC2COS_MAPr, 
                            &reg_value, PRT000_TO_QIDf, &temp));
            break;
        case 1:         
            SOC_IF_ERROR_RETURN(
 ...
 */
int 
drv_bcm5395_queue_mode_set(int unit, uint32 bmp, uint32 mode)
{
    int rv = SOC_E_NONE;
    uint32  reg_value, reg_addr, temp;
    int         reg_len;
    ...
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, QOS_TX_CTRLr, &reg_value, QOS_PRIORITY_CTRLf, &temp);
...
drv_bcm5395_queue_prio_remap_set(int unit, uint32 port, uint8 pre_prio, uint8 prio)
{
    ...
    PBMP_ITER(pbmp, p) {
        ...
        switch ( pre_prio ) {        
            case 0:
                SOC_IF_ERROR_RETURN(
                    (DRV_SERVICES(unit)->reg_field_set)(unit, PN_PCP2TCr, 
...
drv_bcm5395_queue_prio_remap_get(int unit, uint32 port, uint8 pre_prio, uint8 *prio)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, PN_PCP2TCr, p, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, PN_PCP2TCr);
    SOC_IF_ERROR_RETURN(
        (DRV_SERVICES(unit)->reg_read)(unit, reg_addr, &reg_value, reg_len));
    ...
    switch ( pre_prio ) {        
        case 0:
            SOC_IF_ERROR_RETURN(
                (DRV_SERVICES(unit)->reg_field_get)(unit, PN_PCP2TCr, 
                            &reg_value, TAG000_PRI_MAPf, &temp));
            break;
        case 1:         
            SOC_IF_ERROR_RETURN(
...
#ifdef LVL7_FIXUP /* Port Based QoS Fix */
...
drv_bcm5395_queue_rx_reason_set(int unit, uint8 reason, uint32 queue)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, CPU2COS_MAPr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, CPU2COS_MAPr);
    SOC_IF_ERROR_RETURN(
        (DRV_SERVICES(unit)->reg_read)(unit, reg_addr, &reg_value, reg_len));
    ...
    switch (reason) {
        case DRV_RX_REASON_MIRRORING:
            if (SOC_IS_ROBO5395(unit)) {
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, CPU2COS_MAPr, &reg_value, MIRf, &temp));
            } else if (SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit)) {
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, CPU2COS_MAPr, &reg_value, MIRROR_Rf, &temp));
            }
            break;
        case DRV_RX_REASON_SA_LEARNING:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, CPU2COS_MAPr, &reg_value, SA_LRNf, &temp));
            break;
        case DRV_RX_REASON_SWITCHING:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, CPU2COS_MAPr, &reg_value, SW_FLDf, &temp));
            break;
        case DRV_RX_REASON_PROTO_TERM:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, CPU2COS_MAPr, &reg_value, PRTC_TRMNTf, &temp));
            break;
        case DRV_RX_REASON_PROTO_SNOOP:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, CPU2COS_MAPr, &reg_value, PRTC_SNOOPf, &temp));
            break;
        case DRV_RX_REASON_EXCEPTION:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, CPU2COS_MAPr, &reg_value, EXCPT_PRCSf, &temp));
            break;
        default:
            return SOC_E_UNAVAIL;
    }
...
{
    uint32  reg_addr, reg_value, temp = 0;
    int     reg_len;
