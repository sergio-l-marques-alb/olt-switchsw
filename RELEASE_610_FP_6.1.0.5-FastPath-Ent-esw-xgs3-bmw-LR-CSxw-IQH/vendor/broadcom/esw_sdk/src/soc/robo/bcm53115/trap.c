...
drv_bcm53115_trap_set(int unit, soc_pbmp_t bmp, uint32 trap_mask)
{
    ...
    if (trap_mask & DRV_SWITCH_TRAP_IGMP_DISABLE){
        temp = 0;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_UKN_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_QRY_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_RPTLVE_ENf, &temp));
    } else {
    ...
    if (trap_mask & DRV_SWITCH_TRAP_MLD_DISABLE){
        temp = 0;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, MLD_QRY_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, MLD_RPTDONE_ENf, &temp));
    } else {
...
drv_bcm53115_trap_get(int unit, soc_port_t port, uint32 *trap_mask)
{
    ...
    *trap_mask = 0;
    ...
    if (temp){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_QRY_FWD_MODEf, &temp));
    ...
    if (temp){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_RPTLVE_FWD_MODEf, &temp));
        if (temp == BCM53115_REG_VALUE_TRAP){
    ...
    if (temp){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_UKN_FWD_MODEf, &temp));
        if (temp == BCM53115_REG_VALUE_TRAP){
            *trap_mask |= DRV_SWITCH_TRAP_IGMP_UNKNOW;
        }
    }
    ...
    if (temp){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, HL_PRTC_CTRLr, &reg_value, MLD_QRY_FWD_MODEf, &temp));
        if (temp == BCM53115_REG_VALUE_TRAP){
    ...
    if (temp){
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, HL_PRTC_CTRLr, &reg_value, MLD_RPTDONE_FWD_MODEf, &temp));
        if (temp == BCM53115_REG_VALUE_TRAP){
...
drv_bcm53115_snoop_set(int unit, uint32 snoop_mask)
{
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, HL_PRTC_CTRLr, &reg_value, DHCP_ENf, &temp));
     ...
     */
    if (snoop_mask & DRV_SNOOP_IGMP) {
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, NEW_CTRLr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, NEW_CTRLr, 0, 0);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
        temp = 1;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, NEW_CTRLr, &reg_value, IP_MCf, &temp));
        if ((rv = (DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
...
drv_bcm53115_snoop_get(int unit, uint32 *snoop_mask)
{
    ...
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, HL_PRTC_CTRLr);
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, HL_PRTC_CTRLr, 0, 0);
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, reg_addr, &reg_value, reg_len)) < 0) {
        return rv;
    }
