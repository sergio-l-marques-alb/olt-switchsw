                              ...
                              uint32 limit, uint32 burst_size)
{
    ...
    if (limit == 0) { /* Disable ingress rate control */
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_BUCKET0f, &temp));
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, BUCKET0_SIZEf, &temp));
                                 ...
                                 uint32 limit, uint32 burst_size)
{
    ...
    if (IS_CPU_PORT(unit, port)) {
        reg_addr = (DRV_SERVICES(unit)->reg_addr)
                    (unit, IMP_EGRESS_RATE_CTRL_CFG_REGr, 0, 0);
        reg_len = (DRV_SERVICES(unit)->reg_length_get)
                    (unit, IMP_EGRESS_RATE_CTRL_CFG_REGr);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
                (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, IMP_EGRESS_RATE_CTRL_CFG_REGr, &reg_value, 
                RATE_INDEXf, &temp));
        ...
        if ((SOC_IS_ROBO53115(unit)) && 
            ...
            reg_addr = (DRV_SERVICES(unit)->reg_addr)
                        (unit, IMP1_EGRESS_RATE_CTRL_CFG_REGr, 0, 0);
            reg_len = (DRV_SERVICES(unit)->reg_length_get)
                        (unit, IMP1_EGRESS_RATE_CTRL_CFG_REGr);
            if ((rv = (DRV_SERVICES(unit)->reg_read)
                    (unit, reg_addr, &reg_value, reg_len)) < 0) {
                return rv;
            }
            ...
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, IMP1_EGRESS_RATE_CTRL_CFG_REGr, &reg_value, 
                    RATE_INDEXf, &temp));
            ...
            if (limit == 0) { /* Disable ingress rate control */
                temp = 0;
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, PORT_ERC_CONr, &reg_value, EGRESS_RC_ENf, &temp));
            } else {    /* Enable ingress rate control */
                if (burst_size > (500 * 8)) { /* 500 KB */
                    return SOC_E_PARAM;
                }
            ...
            *burst_size = 0;
            ...
            if (temp ==0) {
                ...
                *burst_size = 0;
                ...
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                        (unit, PORT_ERC_CONr, &reg_value, REF_CNT_Rf, &temp));
                if (temp <= 28) {
                    *limit = temp * 64;
                } else if (temp <= 127) {
                    *limit = (temp -27) * 1024;
    ...
    uint32 limit, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, 
    "drv_bcm53115_rate_set: unit = %d, bmp = %x, %s, limit = %dK, \
     burst size = %dKB\n",
...
/*
 *  Function : drv_bcm53115_rate_config_get
 *
 *  Purpose :
    ...
    uint32 *limit, uint32 *burst_size)
{
    ...
    soc_cm_debug(DK_PORT, 
    "drv_bcm53115_rate_get: unit = %d, port = %d, %s, limit = %dK,\
      burst size = %dKB\n",
