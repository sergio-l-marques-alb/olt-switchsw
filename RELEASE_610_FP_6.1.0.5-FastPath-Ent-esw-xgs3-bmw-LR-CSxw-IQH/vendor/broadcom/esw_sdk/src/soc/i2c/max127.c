/*
 * $Id: max127.c,v 1.1 2011/04/18 17:11:05 mruas Exp $
...
max127_read_channel(int unit, int devno, int ch, max127_t *data)
{
    ...
    for (i = 0; i < soc_i2c_max127_iterations; i++) {
 ...
 */
STATIC int
max127_ioctl(int unit, int devno, int ch, void *data, int len)
{
    int i;

    if (ch < 0) { /* Just raw dump */
	for (i = 0; i < N_MAX127_CHAN; i++) {
#ifdef	COMPILER_HAS_DOUBLE
            max127_read_channel(unit, devno, i, &max127_vals[i]);
	    soc_cm_print("%s ch%d: %.3fV\t%s\t"
        ...
        max127_read_channel(unit, devno, ch, (max127_t *)data);
    }
