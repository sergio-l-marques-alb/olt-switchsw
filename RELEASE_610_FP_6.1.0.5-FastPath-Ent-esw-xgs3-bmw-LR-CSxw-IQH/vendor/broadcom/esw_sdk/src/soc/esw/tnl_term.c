/* $Id: tnl_term.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
int _soc_tunnel_term_shift_type_up(int unit, int type)
{
    ...
    SOC_TNL_TERM_IDX_TO_BLOCK_START(to_ent, to_block);
    ...
    if (SOC_TNL_TERM_BLOCK_SIZE != entry_width) {
        ...
        SOC_TNL_TERM_IDX_TO_BLOCK_START(from_ent, from_block);
 ...
 */ 
STATIC int
soc_tunnel_term_reinit_done(int unit)
{
    int idx;
    ...
    SOC_TNL_TERM_STATE_FENT(unit, prev_idx) =  tnl_term_table_size - \
                                               SOC_TNL_TERM_STATE_END(unit, prev_idx) - 1;
...
soc_tunnel_term_reinit(int unit)
{
    ...
    while (idx <= idx_max) {
        ...
        SOC_IF_ERROR_RETURN
            (_soc_tunnel_term_type_get(unit, &entry, NULL, &type));
        ...
        if (SOC_TNL_TERM_STATE_VENT(unit, type) == 0) {
            ...
            SOC_TNL_TERM_STATE_END(unit, type) = idx;
        } else {
            SOC_TNL_TERM_STATE_END(unit, type) = idx;
        }
    ...
    rv = soc_tunnel_term_reinit_done(unit);
    return (rv);
}
