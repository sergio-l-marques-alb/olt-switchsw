/*
 * $Id: triumph.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
STATIC _soc_parity_info_t _soc_tr_ep_parity_info[] = {
    ...
    { EGR_IPFIX_SESS_PAR_ERRf, EGR_IPFIX_SESS_PAR_ERRf, NULL,
      EGR_IPFIX_SESSION_TABLEm, _SOC_PARITY_INFO_TYPE_SINGLE_TABLE2,
...
STATIC _soc_parity_info_t _soc_tr_mmu_parity_info[] = {
    { AGING_CTR_PAR_ERR_ENf, AGING_CTR_PAR_ERRf, NULL,
      MMU_AGING_CTRm, _SOC_PARITY_INFO_TYPE_SINGLE_TABLE1,
      INVALIDr,
      INVALIDr, INVALIDr },
    { AGING_EXP_PAR_ERR_ENf, AGING_EXP_PAR_ERRf, NULL,
      MMU_AGING_EXPm, _SOC_PARITY_INFO_TYPE_SINGLE_TABLE1,
      INVALIDr,
      INVALIDr, INVALIDr },
...
{
    int block, table;
    uint32 addr, block_enable, regval, misc_cfg;
    _soc_parity_info_t *info;
...
_soc_triumph_process_parity_error(int unit)
{
    ...
    uint32 entry_idx, bitmap0, bitmap1;
    _soc_parity_info_t *info;
    soc_reg_t block_reg, reg;
    char *msg;
    ...
    for (block = 0; _soc_tr_parity_block_info[block].cpi_bit; block++) {
        ...
        for (table = 0; info[table].error_field != INVALIDf; table++) {
            ...
            case _SOC_PARITY_INFO_TYPE_SINGLE_TABLE2:
                ...
                entry_idx = soc_reg_field_get(unit, reg, status, ENTRY_IDXf);
                soc_cm_debug(DK_ERR,
                             "unit %d %s entry %d parity error\n",
                             unit, msg, entry_idx);
                break;
        ...
        reg = _soc_tr_parity_block_info[block].enable_reg;
        addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
        SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &block_enable));
...
int
soc_triumph_esm_intr_status(int unit)
{
...
STATIC int
_soc_triumph_esm_init_set_sram_tuning_result(int unit)
{
    soc_tcam_info_t *tcam_info;
    soc_reg_t reg;
    uint32 tune_data[2], addr, rval, fval;
    uint32 w2r_nops, r2w_nops, common_w2r_nops, common_r2w_nops;
    int freq, i;
    char name_str[20];
    static const struct {
    ...
    for (i = 0; i < 2; i++) { /* ES0, and ES1 */
        ...
        tune_data[i] = soc_property_get(unit, name_str, 0);
        if (tune_data[i]) {
            freq = (tune_data[i] >> SOC_TR_MEMTUNE_DDR_FREQ_SHIFT) &
                SOC_TR_MEMTUNE_DDR_FREQ_MASK;
            if (freq != tcam_info->sram_freq) {
                tune_data[i] = 0;
                soc_cm_print("Ignore %s, config was tuned at %d MHz, "
                             "current frequency is %d MHz\n",
                             name_str, freq, tcam_info->sram_freq);
            }
        }
    }

    /* make w2r_nops and r2w_nops the name on both sram interface */
    common_w2r_nops = SOC_TR_MEMTUNE_W2R_NOPS_MASK + 1;
    common_r2w_nops = SOC_TR_MEMTUNE_R2W_NOPS_MASK + 1;
    if (tune_data[0] && tune_data[1] &&
        soc_property_get(unit, "use_common_nops", 0)) {
        common_w2r_nops = (tune_data[0] >> SOC_TR_MEMTUNE_W2R_NOPS_SHIFT) &
            SOC_TR_MEMTUNE_W2R_NOPS_MASK;
        fval = (tune_data[1] >> SOC_TR_MEMTUNE_W2R_NOPS_SHIFT) &
            SOC_TR_MEMTUNE_W2R_NOPS_MASK;
        common_w2r_nops = common_w2r_nops > fval ? common_w2r_nops : fval;
        common_r2w_nops = (tune_data[0] >> SOC_TR_MEMTUNE_R2W_NOPS_SHIFT) &
            SOC_TR_MEMTUNE_R2W_NOPS_MASK;
        fval = (tune_data[1] >> SOC_TR_MEMTUNE_R2W_NOPS_SHIFT) &
            SOC_TR_MEMTUNE_R2W_NOPS_MASK;
        common_r2w_nops = common_r2w_nops > fval ? common_r2w_nops : fval;
    }

    for (i = 0; i < 2; i++) { /* ES0 and ES1 */
        if (tune_data[i]) {
            /* ESx_SRAM_CTL register */
            reg = ddr_reg[i].sram_ctl_reg;
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
            w2r_nops = common_w2r_nops;
            if (w2r_nops == SOC_TR_MEMTUNE_W2R_NOPS_MASK + 1) {
                w2r_nops = (tune_data[i] >> SOC_TR_MEMTUNE_W2R_NOPS_SHIFT) &
                    SOC_TR_MEMTUNE_W2R_NOPS_MASK;
            }
            soc_reg_field_set(unit, reg, &rval, NUM_W2R_NOPSf, w2r_nops);
            r2w_nops = common_r2w_nops;
            if (r2w_nops == SOC_TR_MEMTUNE_R2W_NOPS_MASK + 1) {
                r2w_nops = (tune_data[i] >> SOC_TR_MEMTUNE_R2W_NOPS_SHIFT) &
                    SOC_TR_MEMTUNE_R2W_NOPS_MASK;
            }
            if (!r2w_nops) {
                r2w_nops = 1; /* min r2w_nops encoding is 1, 0 means 4 nops */
            }
            soc_reg_field_set(unit, reg, &rval, NUM_R2W_NOPSf, r2w_nops);
            SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, rval));

            /* ESx_DTU_LTE_TMODE0 register */
            reg = ddr_reg[i].tmode0_reg;
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
            fval = (tune_data[i] >> SOC_TR_MEMTUNE_EM_LATENCY_SHIFT) &
            ...
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
            fval = (tune_data[i] >> SOC_TR_MEMTUNE_DDR_TX_OFFSET_SHIFT) &
            ...
            fval = (tune_data[i] >> SOC_TR_MEMTUNE_DDR_RX_OFFSET_SHIFT) &
            ...
            reg = ddr_reg[i].config_reg2;
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
            fval = (tune_data[i] >> SOC_TR_MEMTUNE_DDR_LATENCY_SHIFT) &
                SOC_TR_MEMTUNE_DDR_LATENCY_MASK;
            soc_reg_field_set(unit, reg, &rval, SEL_EARLY2_0f,
                              fval == 0 ? 1 : 0);
            soc_reg_field_set(unit, reg, &rval, SEL_EARLY1_0f,
                              fval == 1 ? 1 : 0);
            soc_reg_field_set(unit, reg, &rval, SEL_EARLY2_1f,
                              fval == 0 ? 1 : 0);
            soc_reg_field_set(unit, reg, &rval, SEL_EARLY1_1f,
                              fval == 1 ? 1 : 0);
            SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, rval));

            /* ESx_DDR36_CONFIG_REG3_IS register */
            reg = ddr_reg[i].config_reg3;
            addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
            fval = (tune_data[i] >> SOC_TR_MEMTUNE_OVRD_SM_SHIFT) &
                SOC_TR_MEMTUNE_OVRD_SM_MASK;
            if (fval) {
                fval = (tune_data[i] >> SOC_TR_MEMTUNE_PHASE_SEL_SHIFT) &
...
_soc_triumph_esm_init_set_tcam_tuning_result(int unit)
{
    ...
    if (tune_data[1]) {
        ...
        if (freq != tcam_info->tcam_freq) {
            ...
            soc_cm_print("Ignore %s0 and %s1, config was tuned at %d MHz, "
                         "current frequency is %d MHz\n",
        ...
        if (use_midl != cur_use_midl) {
            tune_data[0] = tune_data[1] = 0;
            soc_cm_print("Ignore %s0 and %s1, config was tuned using %s "
...
STATIC int
_soc_triumph_esm_init_stop_dk(int unit)
{
    soc_reg_t reg;
    uint32 addr, rval;
    int i;
...
STATIC int
_soc_triumph_esm_init_relock_dll(int unit)
{
    soc_reg_t reg;
    uint32 addr, rval;
    int fval, i;
    ...
    /* Relock DLL on ES0 and ES1 interfaces */
    for (i = 0; i < 2; i++) {
        reg = ddr_reg[i].config_reg1;
        addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
        SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
        ...
        SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, rval));
        sal_usleep(10000);
        sal_sprintf(name_str, "sram%d_dac_value", i);
    ...
    fval = soc_property_get(unit, "tcam_dac_value", 4);
    if (fval != -1) {
        reg = ETU_DDR72_CONFIG_REG2_ISr;
        addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
        SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
        soc_reg_field_set(unit, reg, &rval, BIASGEN_DAC_ENf, 1);
        soc_reg_field_set(unit, reg, &rval, BIASGEN_DAC_CTRLf, fval);
    ...
    fval = soc_property_get(unit, "tcam_ptr_dist", 2);
    reg = ETU_DDR72_CONFIG_REG1_ISr;
    addr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
    SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
    soc_reg_field_set(unit, reg, &rval, SEL_WRFIFO_PTR_CLKf, fval);
    SOC_IF_ERROR_RETURN(soc_reg32_write(unit, addr, rval));
...
STATIC int
_soc_triumph_esm_init_set_pvt(int unit, soc_reg_t reg)
{
    uint32 pvt_data, addr, rval, fval;
    char name_str[20];

    switch (reg) {
    case ETU_DDR72_CONFIG_REG2_ISr:
        sal_sprintf(name_str, "%s", spn_EXT_TCAM_PVT);
        break;
    case ES0_DDR36_CONFIG_REG2_ISr:
        sal_sprintf(name_str, "%s0", spn_EXT_SRAM_PVT);
        break;
    case ES1_DDR36_CONFIG_REG2_ISr:
        sal_sprintf(name_str, "%s1", spn_EXT_SRAM_PVT);
        break;
    default:
        return SOC_E_PARAM;
    }
...
int
soc_triumph_esm_init_pvt_comp(int unit, soc_reg_t reg, soc_reg_t status_reg)
{
    ...
    if (reg != ES0_DDR36_CONFIG_REG2_ISr) {
        ...
        /* Enable only the specified PVT cell */
        SOC_IF_ERROR_RETURN(soc_reg32_read(unit, addr, &rval));
...
_soc_triumph_esm_init_retune_tcam(int unit)
{
    ...
    soc_cm_print("Re-tuning TCAM interface\n");
    ...
    mt_data.verbose = FALSE;
    mt_data.mask[0] = 0xffffffff;
    mt_data.manual_settings = TRUE;
    ...
    if (SOC_IS_TRIUMPH(unit)) {
        soc_triumph_esm_intr_status(unit);
    }
#ifdef BCM_TRIUMPH2_SUPPORT
    else if (SOC_IS_TRIUMPH2(unit)) {
        soc_triumph2_esm_intr_status(unit);
    }
...
_soc_triumph_esm_init_clear_all_entries(int unit)
{
    ...
    if (SOC_CONTROL(unit)->ext_l2_shadow != NULL) {
...
soc_triumph_esm_init(int unit)
{
    soc_tcam_info_t *tcam_info;
    uint32 rval, to_usec;
    int rv, sram_mode, ad_mode, do_tcam_tuning, always_drive_dbus;
    ...
    tcam_info->subtype = soc_property_get(unit, spn_EXT_TCAM_DEV_TYPE, 0);
    if (tcam_info->subtype &&
        (tcam_info->subtype < 3 || tcam_info->subtype > 4)) {
        soc_cm_debug(DK_ERR, "ESM init: unit %d invalid device type %d\n",
                     unit, tcam_info->subtype);
        return SOC_E_PARAM;
    }
    ...
    if (soc_property_get(unit, "ext_tcam_use_midl", 1)) {
        soc_reg_field_set(unit, ETU_DDR72_CONFIG_REG1_ISr, &rval,
    ...
    /* 0: 1.5 clocks latency, 1: 2 clocks latency */
    ...
    rv = _soc_triumph_esm_init_set_sram_tuning_result(unit);
    ...
    do_tcam_tuning = rv == SOC_E_NOT_FOUND ? FALSE : TRUE;
    ...
    /* Toggle STOP_DK on SRAM interfaces */
    SOC_IF_ERROR_RETURN(_soc_triumph_esm_init_stop_dk(unit));

    /* Relock DLL to stabilize interfaces */
    SOC_IF_ERROR_RETURN(_soc_triumph_esm_init_relock_dll(unit));

    /* Set ES0 PVT */
    rv = _soc_triumph_esm_init_set_pvt(unit, ES0_DDR36_CONFIG_REG2_ISr);
    if (rv == SOC_E_NOT_FOUND) {
        SOC_IF_ERROR_RETURN
            (soc_triumph_esm_init_pvt_comp(unit, ES0_DDR36_CONFIG_REG2_ISr,
    ...
    /* Set ES1 PVT */
    rv = _soc_triumph_esm_init_set_pvt(unit, ES1_DDR36_CONFIG_REG2_ISr);
    if (rv == SOC_E_NOT_FOUND) {
        SOC_IF_ERROR_RETURN
            (soc_triumph_esm_init_pvt_comp(unit, ES1_DDR36_CONFIG_REG2_ISr,
    ...
    /* Set TCAM PVT */
    rv = _soc_triumph_esm_init_set_pvt(unit, ETU_DDR72_CONFIG_REG2_ISr);
    if (rv == SOC_E_NOT_FOUND) {
        SOC_IF_ERROR_RETURN
            (soc_triumph_esm_init_pvt_comp(unit, ETU_DDR72_CONFIG_REG2_ISr,
...
#ifdef BCM_TRIUMPH2_SUPPORT
    if (SOC_IS_TRIUMPH2(unit) && soc_feature(unit, soc_feature_esm_support)) {
...
/*************** Generic shadow table handling **************/
typedef struct _soc_shadow_entry_hdr_s {
    struct _soc_shadow_entry_hdr_s *prev;
    struct _soc_shadow_entry_hdr_s *next;
    int index;
    int chunk_index;
} _soc_shadow_entry_hdr_t;

typedef struct _soc_shadow_chunk_s {
    _soc_shadow_entry_hdr_t *chunk;
    int entries_in_used;
} _soc_shadow_chunk_t;

typedef struct _soc_shadow_s {
    _soc_shadow_entry_hdr_t **entries; /* array of entry pointers */
    _soc_shadow_entry_hdr_t **buckets; /* array of bucket link list pointers */
    _soc_shadow_entry_hdr_t *free_list;
    _soc_shadow_chunk_t *chunks;       /* array of chunk structure pointers */
    _soc_shadow_entry_hdr_t *free_chunk;
    uint32 (*hash_over_entry)(int unit, void *entry);
    uint32 (*hash_over_userdata)(void *userdata);
    int num_entries;            /* size of entries array */
    int num_buckets;            /* size of buckets link list array */
    int entries_per_chunk;      /* chunk size of each memory allocation */
    int bytes_per_entry;        /* size of table entry (not include hdr) */
    int unit;
} _soc_shadow_t;

STATIC int
_soc_shadow_init(int unit, int num_entries, int num_buckets,
                 int entries_per_chunk, int bytes_per_entry,
                 uint32 (*hash_over_entry)(int unit, void *entry),
                 uint32 (*hash_over_userdata)(void *userdata), void **shadow)
{
    _soc_shadow_t *ptr;
    int num_chunks, alloc_size;

    if (num_entries <= 0 || num_buckets <= 0 || entries_per_chunk <= 0 ||
        bytes_per_entry <= 0 || hash_over_entry == NULL ||
        hash_over_userdata == NULL || shadow == NULL) {
        return SOC_E_PARAM;
    }
    num_chunks = (num_entries + entries_per_chunk - 1) / entries_per_chunk;

    ptr = sal_alloc(sizeof(_soc_shadow_t), "soc_shadow_t");
    if (ptr == NULL) {
        return SOC_E_MEMORY;
    }
    sal_memset(ptr, 0, sizeof(_soc_shadow_t));

    alloc_size = sizeof(_soc_shadow_entry_hdr_t *) * num_entries;
    ptr->entries = sal_alloc(alloc_size, "soc_shadow_t entries");
    if (ptr->entries == NULL) {
        goto alloc_fail;
    }
    sal_memset(ptr->entries, 0, alloc_size);

    alloc_size = sizeof(_soc_shadow_entry_hdr_t *) * num_buckets;
    ptr->buckets = sal_alloc(alloc_size, "soc_shadow_t buckets");
    if (ptr->buckets == NULL) {
        goto alloc_fail;
    }
    sal_memset(ptr->buckets, 0, alloc_size);

    alloc_size = sizeof(_soc_shadow_chunk_t) * num_chunks;
    ptr->chunks = sal_alloc(alloc_size, "soc_shadow_t chunks");
    if (ptr->chunks == NULL) {
        goto alloc_fail;
    }
    sal_memset(ptr->chunks, 0, alloc_size);

    ptr->hash_over_entry = hash_over_entry;
    ptr->hash_over_userdata = hash_over_userdata;
    ptr->num_entries = num_entries;
    ptr->num_buckets = num_buckets;
    ptr->entries_per_chunk = entries_per_chunk;
    ptr->bytes_per_entry = bytes_per_entry;
    ptr->unit = unit;
    *shadow = ptr;
...
alloc_fail:
    if (ptr->entries != NULL) {
        sal_free(ptr->entries);
    }
    if (ptr->buckets != NULL) {
        sal_free(ptr->buckets);
    }
    if (ptr->chunks != NULL) {
        sal_free(ptr->chunks);
    }
...
STATIC int
_soc_shadow_deinit(void **shadow)
{
    _soc_shadow_t *ptr;
    int num_chunks, i;
    ...
    if (*shadow != NULL) {
        ...
        if (ptr->chunks != NULL) {
            num_chunks = (ptr->num_entries + ptr->entries_per_chunk - 1) /
                ptr->entries_per_chunk;
            for (i = 0; i < num_chunks; i++) {
                if (ptr->chunks[i].chunk != NULL) {
                    sal_free(ptr->chunks[i].chunk);
                }
            }
            sal_free(ptr->chunks);
        }
        if (ptr->free_chunk != NULL) {
            sal_free(ptr->free_chunk);
        }
        sal_free(ptr);
        *shadow = NULL;
    }
...
STATIC int
_soc_shadow_write(_soc_shadow_t *shadow, int index, void *data)
{
    _soc_shadow_entry_hdr_t *hdr, *chunk;
    unsigned char *ptr;
    int num_chunks, bytes_per_shadow_entry, alloc_size;
    int chunk_index, bucket_index, i;

    if (shadow == NULL) {
        return SOC_E_INIT;
    }
    if (index < 0 || index >= shadow->num_entries) {
        return SOC_E_PARAM;
    }

    if (shadow->entries[index] == NULL && data == NULL) {
        return SOC_E_NONE;
    }

    bytes_per_shadow_entry =
        (sizeof(_soc_shadow_entry_hdr_t) + shadow->bytes_per_entry +
         sizeof(void *) - 1) / sizeof(void *) * sizeof(void *);

    if (shadow->entries[index] != NULL) {
        /* there is an entry at the specified index already */

        hdr = shadow->entries[index];

        /* remove the old entry from hash bucket link list */
        if (hdr->next != NULL) {
        ...
        if (hdr->prev != NULL) {
            ...
            bucket_index = shadow->hash_over_entry(shadow->unit, &hdr[1]);
            shadow->buckets[bucket_index] = hdr->next;
        }

        /* the action is to delete */
        if (data == NULL) {
            /* remove the entry from entries table */
            shadow->entries[index] = NULL;

            /* add the entry to head of free_list */
            hdr->prev = NULL;
            hdr->next = shadow->free_list;
            if (hdr->next != NULL) {
                hdr->next->prev = hdr;
            }
            shadow->free_list = hdr;

            /* decrease per chunk entries_in_used count */
            chunk_index = hdr->chunk_index;
            shadow->chunks[chunk_index].entries_in_used--;

            /* free the chunk if all entries are not in used */
            if (!shadow->chunks[chunk_index].entries_in_used) {
                /* remove the chunk from chunks table */
                chunk = shadow->chunks[chunk_index].chunk;
                shadow->chunks[chunk_index].chunk = NULL;

                /* remove all entries in the chunk from free_list */
                ptr = (unsigned char *)chunk;
                for (i = 0; i < shadow->entries_per_chunk; i++) {
                    hdr = (_soc_shadow_entry_hdr_t *)ptr;
                    ptr += bytes_per_shadow_entry;

                    /* remove the entry from free_list */
                    if (hdr->next != NULL) {
                        hdr->next->prev = hdr->prev;
                    }
                    if (hdr->prev != NULL) {
                        ...
                        shadow->free_list = hdr->next;
                    }
                }

                /* free the chunk or cache it for later use */
                if (shadow->free_chunk == NULL) {
                    shadow->free_chunk = chunk;
                } else {
                    sal_free(chunk);
                }
            }
        ...
        /* this is a new entry at the specified index */

        /* no pre-allocated free entry available */
        if (shadow->free_list == NULL) {
            if (shadow->free_chunk == NULL) {
                /* need to allocate a new chunk */
                alloc_size = bytes_per_shadow_entry *
                    shadow->entries_per_chunk;
                chunk = sal_alloc(alloc_size, "_soc_shadow_chunk_t");
                if (chunk == NULL) {
                    return SOC_E_MEMORY;
                }
            } else {
                /* use the previously used pre-allocated chunk */
                chunk = shadow->free_chunk;
                shadow->free_chunk = NULL;
            }

            /* find an available chunk_index */
            num_chunks =
                (shadow->num_entries + shadow->entries_per_chunk - 1) /
                shadow->entries_per_chunk;
            for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
                if (shadow->chunks[chunk_index].chunk == NULL) {
                    shadow->chunks[chunk_index].chunk = chunk;
                    break;
                }
            }
            ...
            /* add all entries in the chunk to free_list */
            shadow->free_list = chunk;
            ptr = (unsigned char *)chunk;
            hdr = shadow->free_list;
            hdr->prev = NULL;
            for (i = 0; i < shadow->entries_per_chunk - 1; i++) {
                ptr += bytes_per_shadow_entry;
                hdr->next = (_soc_shadow_entry_hdr_t *)ptr;
        ...
        /* remove the new entry from free_list */
        hdr = shadow->free_list;
        shadow->free_list = hdr->next;
        shadow->entries[index] = hdr;
        shadow->chunks[hdr->chunk_index].entries_in_used++;
    }

    bucket_index = shadow->hash_over_entry(shadow->unit, data);
    if (bucket_index >= shadow->num_buckets) {
        return SOC_E_INTERNAL;
    }

    /* fill in data and add the new entry to hash bucket link list */
    sal_memcpy(&hdr[1], data, shadow->bytes_per_entry);
    hdr->index = index;
    hdr->prev = NULL;
    hdr->next = shadow->buckets[bucket_index];
    if (hdr->next != NULL) {
        hdr->next->prev = hdr;
    }
    shadow->buckets[bucket_index] = hdr;

    return SOC_E_NONE;
}

STATIC int
_soc_shadow_read(_soc_shadow_t *shadow, int index, void **data)
{
    _soc_shadow_entry_hdr_t *hdr;

    if (shadow == NULL) {
        return SOC_E_INIT;
    }
    if (index < 0 || index >= shadow->num_entries || data == NULL) {
        return SOC_E_PARAM;
    }
...
/*
 * Return value from match_func:
 *     SOC_E_NONE      - matched, do callback
 *     SOC_E_NOT_FOUND - not matched, no callback
 */
STATIC int
_soc_shadow_search(_soc_shadow_t *shadow, void *userdata, int stop_on_hit,
                   int (*match_func)(int unit, void *userdata, void *entry),
                   void (*callback)(int unit, void *userdata, int index,
                                    void *entry))
{
    _soc_shadow_entry_hdr_t *hdr, *hdr_next;
    uint32 bucket_idx;
    int rv, count;
    ...
    if (userdata == NULL || match_func == NULL || callback == NULL) {
        return SOC_E_PARAM;
    }

    count = 0;
    bucket_idx = shadow->hash_over_userdata(userdata);
    if (bucket_idx >= shadow->num_buckets) {
        return SOC_E_INTERNAL;
    }
    hdr = shadow->buckets[bucket_idx];
    while (hdr != NULL) {
        rv = match_func(shadow->unit, userdata, &hdr[1]);
        hdr_next = hdr->next;
        if (rv == SOC_E_NONE) {
            callback(shadow->unit, userdata, hdr->index, &hdr[1]);
            count++;
            if (stop_on_hit) {
                break;
            }
...
/*************** End of Generic shadow table handling **************/

/*************** EXT_L2_ENTRY shadow table handling **************/
typedef struct _soc_shadow_ext_l2_entry_userdata_s {
    sal_mac_addr_t mac;
    int module;
    int port;
    int include_static;
    void (*callback)(int unit, int index, ext_l2_entry_entry_t *entry);
} _soc_shadow_ext_l2_entry_userdata_t;

#define _SOC_EXT_L2_ENTRY_SHADOW_NUM_BUCKETS       256
#define _SOC_EXT_L2_ENTRY_SHADOW_ENTRIES_PER_CHUNK 64

STATIC uint32
_soc_shadow_ext_l2_entry_hash_over_entry(int unit, void *data)
{
    sal_mac_addr_t mac;
    uint32 crc;

    soc_mem_mac_addr_get(unit, EXT_L2_ENTRYm, data, MAC_ADDRf, mac);
    crc = _shr_crc32b(0, mac, 48);
    return crc & 0xff;
}

STATIC uint32
_soc_shadow_ext_l2_entry_hash_over_userdata(void *data)
{
    _soc_shadow_ext_l2_entry_userdata_t *userdata = data;
    uint32 crc;

    crc = _shr_crc32b(0, userdata->mac, 48);
    return crc & 0xff;
}

STATIC int
_soc_shadow_ext_l2_entry_match_mac(int unit, void *userdata, void *entry)
{
    sal_mac_addr_t mac_key, mac;
    int include_static, static_bit;

    sal_memcpy(mac_key, ((_soc_shadow_ext_l2_entry_userdata_t *)userdata)->mac,
               6);
    soc_mem_mac_addr_get(unit, EXT_L2_ENTRYm, entry, MAC_ADDRf, mac);
    if (!MAC_ADDR_EQUAL(mac_key, mac)) {
        return SOC_E_NOT_FOUND;
    }

    include_static =
        ((_soc_shadow_ext_l2_entry_userdata_t *)userdata)->include_static;
    static_bit = soc_mem_field32_get(unit, EXT_L2_ENTRYm, entry, STATIC_BITf);
    if (!include_static && static_bit) {
        return SOC_E_NOT_FOUND;
    }
...
STATIC int
_soc_shadow_ext_l2_entry_match_mac_port(int unit, void *userdata, void *entry)
{
    sal_mac_addr_t mac_key, mac;
    int module_key, module;
    int port_key, port;
    int include_static, static_bit;
    ...
    module_key = ((_soc_shadow_ext_l2_entry_userdata_t *)userdata)->module;
    module = soc_mem_field32_get(unit, EXT_L2_ENTRYm, entry, MODULE_IDf);
    if (module_key != module) {
        return SOC_E_NOT_FOUND;
    }

    port_key = ((_soc_shadow_ext_l2_entry_userdata_t *)userdata)->port;
    port = soc_mem_field32_get(unit, EXT_L2_ENTRYm, entry, PORT_NUMf);
    if (port_key != port) {
        return SOC_E_NOT_FOUND;
    }
...
STATIC void
_soc_shadow_ext_l2_entry_callback(int unit, void *userdata, int index,
                                  void *entry)
{
    ((_soc_shadow_ext_l2_entry_userdata_t *)userdata)->callback(unit, index,
                                                                entry);
}

int
soc_shadow_ext_l2_entry_deinit(int unit)
{
    ...
    return _soc_shadow_deinit(&soc->ext_l2_shadow);
}

int
soc_shadow_ext_l2_entry_write(int unit, int index, ext_l2_entry_entry_t *entry)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    int rv;
    ...
    if (soc->ext_l2_shadow == NULL) {
        SOC_IF_ERROR_RETURN
            (_soc_shadow_init(unit,
                              soc_mem_index_count(unit, EXT_L2_ENTRYm),
                              _SOC_EXT_L2_ENTRY_SHADOW_NUM_BUCKETS,
                              _SOC_EXT_L2_ENTRY_SHADOW_ENTRIES_PER_CHUNK,
                              sizeof(ext_l2_entry_entry_t),
                              _soc_shadow_ext_l2_entry_hash_over_entry,
                              _soc_shadow_ext_l2_entry_hash_over_userdata,
                              &soc->ext_l2_shadow));
    }

    MEM_LOCK(unit, EXT_L2_ENTRYm);
    rv = _soc_shadow_write(soc->ext_l2_shadow, index, entry);
    MEM_UNLOCK(unit, EXT_L2_ENTRYm);

    return rv;
}

int
soc_shadow_ext_l2_entry_read(int unit, int index, ext_l2_entry_entry_t **entry)
{
    soc_control_t *soc = SOC_CONTROL(unit);

    return _soc_shadow_read(soc->ext_l2_shadow, index, (void **)entry);
}

int
soc_shadow_ext_l2_entry_search(int unit, sal_mac_addr_t mac, int module,
                               int port, int include_static, int stop_on_hit,
                               void (*callback)(int unit, int index,
                                                ext_l2_entry_entry_t *entry))
{
    soc_control_t *soc = SOC_CONTROL(unit);
    _soc_shadow_ext_l2_entry_userdata_t userdata;

    sal_memcpy(userdata.mac, mac, 6);
    userdata.module = module;
    userdata.port = port;
    userdata.include_static = include_static;
    userdata.callback = callback;

    if (port == -1) {
        return _soc_shadow_search(soc->ext_l2_shadow, &userdata, stop_on_hit,
                                  _soc_shadow_ext_l2_entry_match_mac,
                                  _soc_shadow_ext_l2_entry_callback);
    } else {
        return _soc_shadow_search(soc->ext_l2_shadow, &userdata, stop_on_hit,
                                  _soc_shadow_ext_l2_entry_match_mac_port,
                                  _soc_shadow_ext_l2_entry_callback);
    }
}
/*************** End of EXT_L2_ENTRY shadow table handling **************/
...
soc_triumph_ext_l2_entry_update(int unit, int index, void *entry)
{
    ...
    SOC_IF_ERROR_RETURN(soc_shadow_ext_l2_entry_write(unit, index, entry));
