/*
 * $Id: er_cmdmem.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
soc_mem_cmd_op(int unit, soc_mem_t mem, soc_mem_cmd_t *cmd_info, int intr)
{
    ...
    uint32                     target_tab, cmd_reg_addr, cmd_reg_data = 0;
    int                        mcmd_ix;
    ...
    /* Read results reg */
    cmd_reg_addr = soc_reg_addr(unit, cmd_reg, 0 , 0);
	...
	/* Override schan write-suppression during warm reboot */
        ...
        if (use_intr) {
            ...
            if (sal_sem_take(SOC_CONTROL(unit)->memCmdIntr[mcmd_ix],
                ...
                if ((rv = soc_reg32_read(unit, cmd_reg_addr,
                                         &cmd_reg_data)) < 0) {
                    if (soc_cm_debug_check(DK_ERR)) {
            ...
            while ((rv = soc_reg32_read(unit, cmd_reg_addr,
                                        &cmd_reg_data)) >= 0) {
                /* Operation done? */
