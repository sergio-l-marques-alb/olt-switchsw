/*
 * $Id: firebolt.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
soc_firebolt_misc_init(int unit)
{
    ...
    if(SOC_IS_HAWKEYE(unit)) {
        /* Enable fp slice */
        SOC_IF_ERROR_RETURN(READ_FP_SLICE_ENABLEr(unit, &val));
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_SLICE_ENABLE_SLICE_0f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_SLICE_ENABLE_SLICE_1f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_SLICE_ENABLE_SLICE_2f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_SLICE_ENABLE_SLICE_3f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_LOOKUP_ENABLE_SLICE_0f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_LOOKUP_ENABLE_SLICE_1f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_LOOKUP_ENABLE_SLICE_2f, 1);
        soc_reg_field_set(unit, FP_SLICE_ENABLEr, &val, FP_LOOKUP_ENABLE_SLICE_3f, 1);
        SOC_IF_ERROR_RETURN(WRITE_FP_SLICE_ENABLEr(unit, val));

        SOC_IF_ERROR_RETURN(READ_ING_MISC_CONFIGr(unit, &val));
        soc_reg_field_set(unit, ING_MISC_CONFIGr, &val, USE_EPC_LINK_BMPf, 1);
        SOC_IF_ERROR_RETURN(WRITE_ING_MISC_CONFIGr(unit, val));
        ...
        SOC_IF_ERROR_RETURN(READ_EGR_ENABLE_SELECTr(unit, &val));
        soc_reg_field_set(unit, EGR_ENABLE_SELECTr, &val, ENABLE_SELECTf, 1);
        SOC_IF_ERROR_RETURN(WRITE_EGR_ENABLE_SELECTr(unit, val));

        /* Set test mode bits to 0x10 for all FP TCAM */
        val = 0x10;
        SOC_IF_ERROR_RETURN(WRITE_FP_NON_ROTATED_CAM_CONTROLr(unit, val));
        SOC_IF_ERROR_RETURN(WRITE_FP_ROTATED_CAM_CONTROLr(unit, val));
        /* QSGMII Pause Work aroud */
        val = 0xffffffff; /* all pbmp */
        SOC_IF_ERROR_RETURN(WRITE_CMIC_TX_PAUSE_OVERRIDE_CONTROLr(unit, val));
        SOC_IF_ERROR_RETURN(WRITE_CMIC_RX_PAUSE_OVERRIDE_CONTROLr(unit, val));
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_ip_ep_mem_parity)) {
        ...
        for (i = 0; i < 10 ; i++) {
            ...
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, soc_reg_addr(unit, reg[i], REG_PORT_ANY,
                                                                  0), &rval));
            soc_reg_field_set(unit, reg[i], &rval, PARITY_IRQ_ENf, 0);
            SOC_IF_ERROR_RETURN(soc_reg32_write(unit, soc_reg_addr(unit, reg[i], REG_PORT_ANY,                                                                   0), rval));
    ...
    if (SOC_IS_RAPTOR(unit) || SOC_IS_RAVEN(unit) || SOC_IS_HAWKEYE(unit)) {
        ...
        if (SOC_IS_RAVEN(unit) || SOC_IS_HAWKEYE(unit)) {
            if (SOC_PORT_VALID(unit, 3)) {
                soc_reg_field_set(unit,
                                  GPORT_CONFIGr,
                                  &rval,
                                  HGIG2_EN_S3f,
                                  IS_ST_PORT(unit, 3) ? 1 : 0);
...
int _soc_hx_ipipe_parity_error(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_ip_ep_mem_parity)) {
        ...
        for (i = 0; i < 10 ; i++) {
            ...
            SOC_IF_ERROR_RETURN(soc_reg32_read(unit, soc_reg_addr(unit, 
                                               stat_reg[i], REG_PORT_ANY, 0), &istat));
            if (soc_reg_field_get(unit, stat_reg[i], istat, PARITY_ERRf)) {
                ...
                SOC_IF_ERROR_RETURN(soc_reg32_read(unit, soc_reg_addr(unit, 
                                                   ctrl_reg[i], REG_PORT_ANY, 0), &val));
                soc_reg_field_set(unit, ctrl_reg[i], &val, PARITY_IRQ_ENf, 0);
                SOC_IF_ERROR_RETURN(soc_reg32_write(unit, soc_reg_addr(unit, 
                                                    ctrl_reg[i], REG_PORT_ANY, 0), val));
