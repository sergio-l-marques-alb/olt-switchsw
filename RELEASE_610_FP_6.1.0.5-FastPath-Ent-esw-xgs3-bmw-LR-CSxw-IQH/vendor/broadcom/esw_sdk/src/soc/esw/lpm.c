/* $Id: lpm.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
static soc_lpm_state_p soc_lpm_state[SOC_MAX_NUM_DEVICES];

#define IPV6_PFX_ZERO                33
#define MAX_PFX_ENTRIES              (3 * (64 + 32 + 2 + 1))
...
{                                                                            \
    ...
    d.ip0 = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR0f);        \
    d.ip1 = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR1f);        \
    d.mask0 =                                                                \
        soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK0f);       \
    d.mask1 =                                                                \
        soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK1f);       \
    if (soc_mem_field_valid(u, L3_DEFIPm, VRF_ID_0f)) {                      \
        d.vrf_id = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VRF_ID_0f); \
    } else {                                                                 \
        d.vrf_id = 0;                                                        \
    }                                                                        \
    d.index = tab_index;                                                     \
...
{                                                                            \
    soc_lpm_avl_state_t     d;                                               \
    d.ip0 = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR1f);        \
    d.mask0 =                                                                \
        soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK1f);       \
    d.ip1 = 0;                                                               \
    d.mask1 = 0x80000001;                                                    \
...
{
    if (soc_mem_field32_get(u, L3_DEFIPm, entry_data, MODE0f)) {        
        /* IPV6 entry */                                                
        if (soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID1f) &&   
            soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID0f)) {   
        ...
        if (soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID0f)) {   
...
void soc_fb_lpm_avl_delete(int u, void *key_data)
{
    ...
    d.ip0 = soc_mem_field32_get(u, L3_DEFIPm, key_data, IP_ADDR0f);
    d.mask0 = soc_mem_field32_get(u, L3_DEFIPm, key_data, IP_ADDR_MASK0f);

    if (soc_mem_field_valid(u, L3_DEFIPm, VRF_ID_0f)) {                      
        d.vrf_id = soc_mem_field32_get(u, L3_DEFIPm, key_data, VRF_ID_0f); 
    } else {
        d.vrf_id = 0;
    }                                                                        

    if (soc_mem_field32_get(u, L3_DEFIPm, key_data, MODE0f)) {
        d.ip1 = soc_mem_field32_get(u, L3_DEFIPm, key_data, IP_ADDR1f);
        d.mask1 =
            soc_mem_field32_get(u, L3_DEFIPm, key_data, IP_ADDR_MASK1f);
    } else {
        d.ip1 = 0;
        d.mask1 = 0x80000001;
    }
...
typedef uint32 _soc_fb_lpm_hash_entry_t[5];
...
#define FB_LPM_HASH_INDEX_MASK  (0x3FFF)
#define FB_LPM_HASH_IPV6_MASK   (0x8000)

#define SOC_FB_LPM_HASH_ENTRY_IPV6_GET(u, entry_data, odata)                 \
    odata[0] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR0f);     \
    odata[1] =                                                               \
        soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK0f);       \
    odata[2] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR1f);     \
    odata[3] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK1f);\
    if (soc_mem_field_valid(u, L3_DEFIPm, VRF_ID_0f)) {                      \
        odata[4] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VRF_ID_0f); \
    } else {                                                                 \
        odata[4] = 0;                                                        \
    }                                                                       
...
#define SOC_FB_LPM_HASH_ENTRY_IPV4_1_GET(u, entry_data, odata)               \
    odata[0] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR1f);     \
    odata[1] =                                                               \
        soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK1f);       \
    odata[2] = 0;                                                            \
    odata[3] = 0x80000001;                                                   \
    if (soc_mem_field_valid(u, L3_DEFIPm, VRF_ID_1f)) {                      \
        odata[4] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VRF_ID_1f); \
    } else {                                                                 \
        odata[4] = 0;                                                        \
    }                                                                       
...
#define LPM_HASH_INSERT(u, entry_data, tab_index)       \
    soc_fb_lpm_hash_insert(u, entry_data, tab_index, 0x4000, 0)
...
int soc_fb_lpm_hash_lookup(int u, void *key_data, int pfx, int *key_index)
{
    ...
    is_ipv6 = soc_mem_field32_get(u, L3_DEFIPm, key_data, MODE0f);
    if (is_ipv6) {
        SOC_FB_LPM_HASH_ENTRY_IPV6_GET(u, key_data, key_hash);
    } else {
        SOC_FB_LPM_HASH_ENTRY_IPV4_0_GET(u, key_data, key_hash);
...
soc_fb_lpm_ip4entry0_to_0(int u, void *src, void *dst, int copy_hit)
{
    ...
    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, MODE0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, MODE0f, ipv4a);
#if defined(BCM_TRX_SUPPORT) || defined(BCM_RAVEN_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, MODE_MASK0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, MODE_MASK0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, MODE_MASK0f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT || BCM_RAVEN_SUPPORT */

#if defined(BCM_TRX_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, GLOBAL_ROUTE0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, GLOBAL_ROUTE0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, GLOBAL_ROUTE0f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT */

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, IP_ADDR0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, IP_ADDR0f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, IP_ADDR_MASK0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, IP_ADDR_MASK0f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP0f, ipv4a);


    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, ECMP_COUNT0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP_COUNT0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP_COUNT0f, ipv4a);

        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP_PTR0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP_PTR0f, ipv4a);
    } else {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, NEXT_HOP_INDEX0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, NEXT_HOP_INDEX0f, ipv4a);
    }

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, PRI0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, PRI0f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, RPE0f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, RPE0f, ipv4a);

    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, VRF_ID_0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, VRF_ID_0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, VRF_ID_0f, ipv4a);
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, VRF_ID_MASK0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, VRF_ID_MASK0f, ipv4a);
    }

    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, DST_DISCARD0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, DST_DISCARD0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, DST_DISCARD0f, ipv4a);
    }
#if defined(BCM_TRX_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, CLASS_ID0f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, CLASS_ID0f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, CLASS_ID0f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT */
    if (copy_hit) {
...
soc_fb_lpm_ip4entry1_to_1(int u, void *src, void *dst, int copy_hit)
{
    ...
    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, VALID1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, VALID1f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, MODE1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, MODE1f, ipv4a);
#if defined(BCM_TRX_SUPPORT) || defined(BCM_RAVEN_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, MODE_MASK1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, MODE_MASK1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, MODE_MASK1f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT || BCM_RAVEN_SUPPORT */

#if defined(BCM_TRX_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, GLOBAL_ROUTE1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, GLOBAL_ROUTE1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, GLOBAL_ROUTE1f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT */

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, IP_ADDR1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, IP_ADDR1f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, IP_ADDR_MASK1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, IP_ADDR_MASK1f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP1f, ipv4a);


    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, ECMP_COUNT1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP_COUNT1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP_COUNT1f, ipv4a);

        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, ECMP_PTR1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, ECMP_PTR1f, ipv4a);
    } else {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, NEXT_HOP_INDEX1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, NEXT_HOP_INDEX1f, ipv4a);
    }

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, PRI1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, PRI1f, ipv4a);

    ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, RPE1f);
    soc_mem_field32_set(u, L3_DEFIPm, dst, RPE1f, ipv4a);

    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, VRF_ID_1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, VRF_ID_1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, VRF_ID_1f, ipv4a);
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, VRF_ID_MASK1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, VRF_ID_MASK1f, ipv4a);
    }

    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, DST_DISCARD1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, DST_DISCARD1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, DST_DISCARD1f, ipv4a);
    }
#if defined(BCM_TRX_SUPPORT)
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, CLASS_ID1f)) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, CLASS_ID1f);
        soc_mem_field32_set(u, L3_DEFIPm, dst, CLASS_ID1f, ipv4a);
    }
#endif /* BCM_TRX_SUPPORT */
    if (copy_hit) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, src, HIT1f);
...
soc_fb_lpm_dump(int u)
{
    ...
    for(i = max_pfx_len; i >= 0 ; i--) {
        ...
        soc_cm_print("PFX = %d P = %d N = %d START = %d END = %d VENT = %d FENT = %d\n",
        i,
        SOC_LPM_STATE_PREV(u, i),
        SOC_LPM_STATE_NEXT(u, i),
        SOC_LPM_STATE_START(u, i),
...
int _lpm_fb_urpf_entry_replicate(int u, int index, uint32 *e)
{
    ...
    ipv6 = soc_mem_field32_get(u, L3_DEFIPm, e, MODE0f);

    /* Reset destination discard bit. */
    if (SOC_MEM_FIELD_VALID(u, L3_DEFIPm, DST_DISCARD0f)) {
        soc_mem_field32_set(u, L3_DEFIPm, e, DST_DISCARD0f, 0);
        soc_mem_field32_set(u, L3_DEFIPm, e, DST_DISCARD1f, 0);
    }

    /* Set/Reset default gateway flag based on ip mask value. */
    mask0 = soc_mem_field32_get(u, L3_DEFIPm, e, IP_ADDR_MASK0f);
    mask1 = soc_mem_field32_get(u, L3_DEFIPm, e, IP_ADDR_MASK1f);

    if (!ipv6) {
        if (soc_mem_field32_get(u, L3_DEFIPm, e, VALID0f)) {
            soc_mem_field32_set(u, L3_DEFIPm, e, RPE0f, (!mask0) ? 1 : 0);
        }
        if (soc_mem_field32_get(u, L3_DEFIPm, e, VALID1f)) {
            soc_mem_field32_set(u, L3_DEFIPm, e, RPE1f, (!mask1) ? 1 : 0);
        }
    } else {
        def_gw_flag = ((!mask0) &&  (!mask1)) ? 1 : 0;
        soc_mem_field32_set(u, L3_DEFIPm, e, RPE0f, def_gw_flag);
        soc_mem_field32_set(u, L3_DEFIPm, e, RPE1f, def_gw_flag);
    }
...
#endif /* FB_LPM_TABLE_CACHED */
        SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, from_ent, e));
        v0 = soc_mem_field32_get(u, L3_DEFIPm, e, VALID0f);
        v1 = soc_mem_field32_get(u, L3_DEFIPm, e, VALID1f);
...
int _lpm_free_slot_delete(int u, int pfx, int ipv6, void *e, int slot)
{
    ...
    if (!ipv6) { /* IPV4 */
        ...
        if (soc_mem_field32_get(u, L3_DEFIPm, ef, VALID1f)) {
            ...
            soc_mem_field32_set(u, L3_DEFIPm, ef, VALID0f, 0);
            SOC_LPM_STATE_VENT(u, pfx) -= 1;
            SOC_LPM_STATE_FENT(u, pfx) += 1;
            SOC_LPM_STATE_END(u, pfx) -= 1;
        }
...
soc_fb_lpm_vrf_get(int unit, void *lpm_entry, int *vrf)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, L3_DEFIPm, VRF_ID_MASK0f)){
        vrf_id = soc_L3_DEFIPm_field32_get(unit, lpm_entry, VRF_ID_0f);

        /* Special vrf's handling. */
        if (soc_L3_DEFIPm_field32_get(unit, lpm_entry, VRF_ID_MASK0f)) { 
            *vrf = vrf_id;
...
_soc_fb_lpm_prefix_length_get(int u, void *entry, int *pfx_len) 
{
    ...
    if (ipv6) {
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, entry, IP_ADDR_MASK0f);
        if ((rv = _ipmask2pfx(ipv4a, &pfx)) < 0) {
            return(rv);
        }
        ipv4a = soc_mem_field32_get(u, L3_DEFIPm, entry, IP_ADDR_MASK1f);
        if (pfx) {
            if (ipv4a != 0xffffffff)  {
                return(SOC_E_PARAM);
            }
    ...
    switch (vrf_id) { 
      ...
      case SOC_L3_VRF_OVERRIDE:
          *pfx_len = pfx + 2 * (MAX_PFX_ENTRIES / 3);
          break;
      default:   
          *pfx_len = pfx +  (MAX_PFX_ENTRIES / 3);
          break;
    }
    return (SOC_E_NONE);
}
...
soc_fb_lpm_ipv6_delete_index(int u, int index)
{
    ...
    if (rv == SOC_E_NONE) {
        if ((soc_mem_field32_get(u, L3_DEFIPm, e, VALID0f)) &&
            (soc_mem_field32_get(u, L3_DEFIPm, e, MODE0f)) &&
            (soc_mem_field32_get(u, L3_DEFIPm, e, MODE1f)) &&
            (soc_mem_field32_get(u, L3_DEFIPm, e, VALID1f))) {
            uint32      ipv4a;
 
            ipv4a = soc_mem_field32_get(u, L3_DEFIPm, e, IP_ADDR_MASK0f);
            if ((rv = _ipmask2pfx(ipv4a, &pfx)) == SOC_E_NONE) {
                ipv4a = soc_mem_field32_get(u, L3_DEFIPm, e, IP_ADDR_MASK1f);
                if (pfx) {
                    if (ipv4a != 0xffffffff)  {
                        rv =SOC_E_PARAM;
                    }
