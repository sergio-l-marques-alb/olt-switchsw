/*
 * $Id: l2u.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
soc_l2u_insert(int unit, l2u_entry_t *entry, int index, int *index_used)
{
    ...
    if (index == -1) {
        ...
        soc_L2_USER_ENTRYm_field_get(unit, entry, MASKf, mask);

        if (mask[0] == 0xffffffff && (mask[1] & 0xffff) == 0xffff) {
            /* Search from high priority end */
            for (i = i_min; i <= i_max; i++) {
                rv = READ_L2_USER_ENTRYm(unit, MEM_BLOCK_ANY, i, &l2u_entry);
                if (SOC_SUCCESS(rv)) {
                    if (!soc_L2_USER_ENTRYm_field32_get(unit, &l2u_entry, 
                                                        VALIDf)) {
                        /* Found unused entry */
                        break;
                    }
...
soc_l2u_delete(int unit, l2u_entry_t *entry, int index, int *index_deleted)
{
    ...
    WRITE_L2_USER_ENTRYm(unit, MEM_BLOCK_ALL, index, &l2u_entry);
...
soc_l2u_delete_all(int unit)
{
    ...
    memset(&l2u, 0, sizeof(l2u));
    for (i = i_min; i <= i_max; i++) {
        WRITE_L2_USER_ENTRYm(unit, MEM_BLOCK_ALL, i, &l2u);
    }
