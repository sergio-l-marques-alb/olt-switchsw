/*
 * $Id: serdesrobo.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
phy_serdesrobo_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
	soc_cm_debug( DK_PHY,"%s() u=%d p=%d dpx=%d\n", __func__, unit, port, duplex ) ;
...
phy_serdesrobo_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
	soc_cm_debug( DK_PHY,"%s() u=%d p=%d sp=%d\n", __func__, unit, port, speed ) ;
	reg_addr = (DRV_SERVICES(unit)->reg_addr)( unit, SDMIICTLr, port, 0 ) ;
	reg_len  = (DRV_SERVICES(unit)->reg_length_get)( unit, SDMIICTLr ) ;
	SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
					     ( unit, reg_addr, &reg_value, reg_len) ) ;
	/* SDMIICTLr reference soc_MIICTLr_fields, doesn't provide field "Speed (MSB)" */
...
phy_serdesrobo_an_set(int unit, soc_port_t port, int an)
{
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
	soc_cm_debug( DK_PHY,"%s() u=%d p=%d an=%d\n", __func__, unit, port, an ) ;
	reg_addr = (DRV_SERVICES(unit)->reg_addr)( unit, SDMIICTLr, port, 0 ) ;
	reg_len  = (DRV_SERVICES(unit)->reg_length_get)( unit, SDMIICTLr ) ;
	SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
					     ( unit, reg_addr, &reg_value, reg_len) ) ;
...
phy_serdesrobo_an_get(int unit, soc_port_t port, int *an, int *an_done)
{
	...
	*an_done = (int) temp ;
	...
	soc_cm_debug( DK_PHY,"%s() u=%d p=%d an=%d an_done=%d\n",
			      __func__, unit, port, *an, *an_done ) ;
	return(SOC_E_NONE);
    }
...
phy_serdesrobo_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if ( SOC_IS_ROBO5396(unit) )  {
	soc_cm_debug( DK_PHY,"%s() u=%d p=%d lb=%d\n", __func__, unit, port, enable ) ;
	reg_addr = (DRV_SERVICES(unit)->reg_addr)( unit, SDMIICTLr, port, 0 ) ;
	reg_len  = (DRV_SERVICES(unit)->reg_length_get)( unit, SDMIICTLr ) ;
	SOC_IF_ERROR_RETURN( (DRV_SERVICES(unit)->reg_read)
					     ( unit, reg_addr, &reg_value, reg_len) ) ;
