/*
 * $Id: phy54880.c,v 1.1 2011/04/18 17:11:07 mruas Exp $
...
#define PHY_IEEE_MODE(unit, port)   PHY_COPPER_MODE((unit), (port))
#define SOC_PORT_MEDIUM_IEEE        SOC_PORT_MEDIUM_COPPER
#define PHY_FLAGS_IEEE              PHY_FLAGS_COPPER
#define SOC_PA_MEDIUM_IEEE          SOC_PA_MEDIUM_COPPER

#define PHY_BR_MODE(unit, port)     PHY_FIBER_MODE((unit), (port))
#define SOC_PORT_MEDIUM_BR          SOC_PORT_MEDIUM_FIBER
#define PHY_FLAGS_BR                PHY_FLAGS_FIBER
#define SOC_PA_MEDIUM_BR            SOC_PA_MEDIUM_FIBER
...
#define LONGREACH_ABILITY_ALL ( SOC_PHY_CONTROL_LONGREACH_ABILITY_10M_1PAIR |\
...
_phy_54880_reset_setup(int unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54880_SPARE_3r(unit, pc, 0, 1));
#endif

    /* LDS parameters */

    SOC_IF_ERROR_RETURN
        (PHY54880_REG_WRITE(unit, pc,  0x00, 0x0000, MII_AUX_REG, 0x0440));
...
phy_54880_init(int unit, soc_port_t port)
{
    ...
    br_config->lds_advert_ability = LONGREACH_ABILITY_ALL;
    ...
    PHY_FLAGS_SET(unit, port, PHY_FLAGS_IEEE);
    ...
    pc->halfout = soc_property_port_get(unit, port, spn_PHY_HALF_PWR_MODE, 0);
...
_phy_54880_medium_change(int unit, soc_port_t port, int force_update)
{
    ...
    if (medium == SOC_PORT_MEDIUM_IEEE) {
        ...
        if (PHY_IEEE_MODE(unit, port) || force_update) { /* Was IEEE */
            ...
            PHY_FLAGS_CLR(unit, port, PHY_FLAGS_IEEE);
            PHY_FLAGS_SET(unit, port, PHY_FLAGS_MEDIUM_CHANGE);
...
_phy_54880_br_pairs_set(int unit, soc_port_t port, int pairs)
{
    ...
    /* If pairs == 0 turn off BR and return, else turn on */
    if (pairs == 0) {
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54880_EXP_BR_LRE_MISC_CTRL1r(unit, pc, 0, 1));
        return SOC_E_NONE;
    } else {
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54880_EXP_BR_LRE_MISC_CTRL1r(unit, pc, 1, 1));
    }
...
_phy_54880_ieee_ability_advert_set(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_PHY54880_MII_GB_CTRLr(unit, pc,  &mii_gb_ctrl));
...
_phy_54880_br_ability_advert_set(int unit, soc_port_t port, uint32 ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54880_LDS_ADVAr(unit, pc, lds_adv,
        LDS_ADVA_PAUSE|LDS_ADVA_ASYM_PAUSE|LDS_ADVA_2_100|LDS_ADVA_4_100|
        LDS_ADVA_1_100|LDS_ADVA_1_10 | LDS_ADVA_2_10));
...
_phy_54880_ieee_ability_advert_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_PHY54880_MII_ANAr(unit, pc, &mii_ana));
    SOC_IF_ERROR_RETURN
        (READ_PHY54880_MII_GB_CTRLr(unit, pc, &mii_gb_ctrl));
...
_phy_54880_br_ability_advert_std_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if (lds_adv & (LDS_ADVA_1_10|LDS_ADVA_2_10)) {
      ability->speed_full_duplex |= SOC_PA_SPEED_10MB;
    }
    if (lds_adv & (LDS_ADVA_2_100|LDS_ADVA_4_100|LDS_ADVA_1_100)) {
      ability->speed_full_duplex |= SOC_PA_SPEED_100MB;
    }
...
_phy_54880_ieee_ability_remote_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_PHY54880_MII_STATr(unit, pc, &mii_stat));
    ...
    if ((mii_stat & MII_STAT_AN_DONE & MII_STAT_LA)
         ...
         */
        SOC_IF_ERROR_RETURN
            (READ_PHY54880_MII_ANPr(unit, pc, &mii_anp));
        SOC_IF_ERROR_RETURN
            (READ_PHY54880_MII_GB_STATr(unit, pc, &mii_gb_stat));
    ...
    if (mii_gb_stat & MII_GB_STAT_LP_1000FD) {
        ...
        phy_54880_ability_advert_get(unit, port, ability);
    }
    SOC_DEBUG_PRINT((DK_PHY,
         "phy_54880_ability_remote_get:unit=%d p=%d pause=%08x sp=%08x\n",
...
_phy_54880_br_ability_current_get(int unit, soc_port_t port, uint32 *ability)
{
    ...
    if (lre_ctrl & LRE_CTRL_LDSE) {
        ...
        if (!(lre_stat & LRE_STAT_LDS_DONE)) { /* LDS NOT complete */
            ...
            *ability = (ability_local & ability_remote & (3U<<5)) |
                       (((speed == 100) ? (1U<<2) : 1U) << (((unsigned) pairs) >> 1));
        }
    } else {
        *ability = 0;
    }
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_LONGREACH_LOCAL_ABILITY:
        ...
        br_config->lds_advert_ability = value & LONGREACH_ABILITY_ALL;
        break;
