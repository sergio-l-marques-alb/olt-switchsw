/*
 * $Id: phy5464robo.c,v 1.1 2011/04/18 17:11:07 mruas Exp $
...
/*
 * Function:
 *      _phy_5464_disable_process
 * Purpose:
 *      Special process for servicing GE PHY disable.
 *      1. the power down bit will be modified autometically by chip when 
 *          medium was switched between copper/fiber. (currently on 5461s and 
 *          5482s, PD bit will be clear when SD detected)
 * Parameters:
 *      unit - Switch unit #.
 *      port - Switch port #.
 * Returns:
 *      SOC_E_XXX
 * Note:
 *      1. only fiber medium PD should be reprogram again.
 *      2. PHY should be at disable status before calling this routine.
 */

STATIC int
_phy_5464_disable_process(int unit, soc_port_t port)
{
    phy_ctrl_t    *pc;
    ...
    if (pc->fiber.enable) {
        /* set the PD bit again if the PD bit was reset */
        SOC_IF_ERROR_RETURN(
            MODIFY_PHY5464_1000X_MII_CTRLr(unit, pc, MII_CTRL_PD, MII_CTRL_PD));        
    }
...
_robo_phy_5464_reset_setup(int unit, soc_port_t port)
{
    ...
    if (phy53115_an100tx_war){ /* config set to stop this workaround */
        ...
        if (PHY_REVISION_CHECK(pc, PHY_BCM53115_MODEL, 
            ...
            DPRINTF("%s,%d,Clear ge%d workaround sw-info!!\n", 
    ...
    if (PHY_MODEL_CHECK(pc, PHY_BCM53115_OUI, PHY_BCM53115_MODEL)){
        ...
        if ((rev_id == BCM53115_A0_REV_ID) || 
            ...
            DPRINTF("%s,%d,Reset ge%d IOP-WAR-#2 sw-info!!\n", 
                    __func__,__LINE__, port);
        }
    }
    ...
    if (PHY_MODEL_CHECK(pc, PHY_BCM53118_OUI, PHY_BCM53118_MODEL)){
        ...
        if ((rev_id == BCM53118_B0_REV_ID)){
            ...
            DPRINTF("%s,%d, ge%d BCM53118_B0 PHY-WAR-sw-info!!\n", 
                    __func__,__LINE__, port);
        }
    }
    ...
    if (PHY_MODEL_CHECK(pc, PHY_BCM53118_OUI, PHY_BCM53118_MODEL)){
        ...
        if ((rev_id == BCM53118_B1_REV_ID)){
            ...
            DPRINTF("%s,%d, ge%d BCM53118_B1 PHY-WAR-sw-info!!\n", 
                    __func__,__LINE__, port);
        }
    }
...
/* bcm5348/5347/53262 bcm5389/bcm5396 interface == sgmii */
   ...
   if (PHY_IS_BCM5461(pc)) {
        ...
        if (SOC_IS_ROBO5348(unit)||SOC_IS_ROBO5347(unit)|| SOC_IS_ROBO53262(unit)) {
            type_s = 1;
        }
    }
    ...
    if(PHY_IS_BCM5398(pc) ||PHY_IS_BCM5395(pc) ||PHY_IS_BCM53115(pc) ||
        PHY_IS_BCM53118(pc)){
        fiber_capable = 0;
    }

    if(SOC_IS_ROBODINO(unit)){
        /* currently no fiber solution for bcm5396 */
        fiber_capable = 0;
    }
    /*
...
robo_phy_5464_link_get(int unit, soc_port_t port, int *link)
{
    ...
    *link = FALSE;      /* Default return */
    if (PHY_FLAGS_TST(unit, port, PHY_FLAGS_DISABLE)) {
        SOC_IF_ERROR_RETURN(_phy_5464_disable_process(unit, port));
        return SOC_E_NONE;
    }
            ...
            if (SOC_IS_ROBO53115(unit)){
                ...
                if (phy53115_an100tx_war) {
                    if(PHY_REVISION_CHECK(pc, PHY_BCM53115_MODEL, 
                                    PHY_BCM53115_OUI, BCM53115_A0_REV_ID)){
                        if(speed == 100){
                            DPRINTF("%s,%d,Set ge%d WAR for phy53115_a0!!\n", 
                                    __func__,__LINE__, port);
                            rv = WRITE_PHY_REG(unit, pc, 0x17, 0x0fff);
                            rv |= WRITE_PHY_REG(unit, pc, 0x15, 0x20c0);
                            if (rv){
                                DPRINTF("%s,%d,Failed PHY Reg wrtie!\n", 
                                    __func__,__LINE__);
                            }
                        }
                    }
...
robo_phy_5464_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    if (SOC_IS_DRACO(unit) ||
        ...
        *pif = SOC_PORT_IF_RGMII; /* For Robo5324 GE ports */
    }
            ...
            soc_port_cable_diag_t *status)
{
    ...
    if (soc_property_get(unit, "cable_diag_hw", robo_phy_5464_cable_diag_HW)) {
        uint16              tmp, result;
...
void _phy53115_a0_iop_errata(int unit, soc_port_t port)
{
    ...
    if (regrw_rv) {
        DPRINTF("%s,%d,port%d, failed on writing to PHY reg!!\n", 
                __func__,__LINE__,port);
    } else {
        DPRINTF("%s,%d,port%d, IOP-WAR-#1 Done!!\n", __func__,__LINE__,port);
    }
}
...
void _phy53115_b0_iop_errata(int unit, soc_port_t port, int link)
{
    ...
    if (regrw_rv){
        DPRINTF("%s,%d,port%d failed on PHY Reg read!\n", 
                __func__,__LINE__,port);
    }
    ...
    if (link){  /* 2.1 link-up procedure */
        ...
        if (PHY53115_ERRATA_IOP_GET(port)){
            ...
            /* SW-info update */
            PHY53115_ERRATA_IOP_CLEAR(port);
            DPRINTF("%s,%d,port%d link-up! IOP-WAR-#2 removed!!\n", 
                    __func__,__LINE__,port);
        }
        phy53115_errata_iop_cnt[port] = 0;
        ...
        if (phy53115_errata_iop_cnt[port] >= 
            ...
            DPRINTF("%s,%d,port%d IOP-WAR-#2,HALFOUT change to 0x%04x!!\n", 
                    __func__,__LINE__,port, temp);
...
void _phy53115_a0_errata(int unit, soc_port_t port, int link)
{
    ...
    if (link){      /* for link up condition */
        ...
        if (! PHY53115_ERRATA_AN100TX_LINKERR_GET(port)){
            ...
            DPRINTF("%s,%d,checking WAR link-up condition!!\n", 
                    __func__,__LINE__);
        ...
        DPRINTF("%s,%d,ge%d,checked WAR link-down condition!!\n", 
                __func__,__LINE__, port);
    }
    
    DPRINTF("%s,%d,phy53115_a0 an100tx sw-info stat=0x%08x,link=0x%08x!!\n", 
                __func__,__LINE__, phy53115_errata_an100tx_status, 
                phy53115_errata_an100tx_linkup);
    DPRINTF("%s,%d,Set ge%d workaround for phy53115_a0!!\n", 
            __func__,__LINE__, port);
    /* enable the force AUTO-MDIX for this workaround */
    rv = MODIFY_PHY5464_MII_MISC_CTRLr(unit, pc, 0x0200, 0x0200);
    if ( rv != SOC_E_NONE) {
        DPRINTF("%s, failed on force AUTO-MDIX for this WAR.\n", __func__);
    }
...
void _phy53115_a0_errata_link_check(int unit, soc_port_t port, int link)
{
    ...
    if (link) {
        ...
        if (PHY53115_ERRATA_AN100TX_STAT_GET(port) && 
            ...
            DPRINTF("%s,%d,ge%d link-up at phy53115_a0 workaround !!\n", 
                    __func__,__LINE__, port);
            PHY53115_ERRATA_AN100TX_LINK_SET(port);
            DPRINTF("%s,%d,sw-info stat=0x%08x,link=0x%08x!!\n", 
                        __func__,__LINE__, phy53115_errata_an100tx_status, 
                        phy53115_errata_an100tx_linkup);
        ...
        if (PHY53115_ERRATA_AN100TX_STAT_GET(port) && 
                PHY53115_ERRATA_AN100TX_LINK_GET(port)){
            DPRINTF("%s,%d,ge%d reset to free phy53115_a0 workaround !!\n", 
                    __func__,__LINE__, port);
        
            robo_phy_5464_init(unit, port);
        } else if (phy53115_errata_an100tx_failed_cnt[port] > 
                    PHY53115_ERRATA_AN100TX_RESET_CNT){
            DPRINTF("%s,%d,ge%d reset for phy53115_a0 workaround failed!!\n", 
                    __func__,__LINE__, port);
            robo_phy_5464_init(unit, port);
        }
    }
...
robo_phy_5464_shadow_dump(int unit, soc_port_t port)
{
    ...
    for (i = 0; i <= 7; i++) {
        ...
        if (regrw_rv){
            soc_cm_print("%s,%d, failed on PHY reg read/write!\n", 
                    __func__, __LINE__);
        }
