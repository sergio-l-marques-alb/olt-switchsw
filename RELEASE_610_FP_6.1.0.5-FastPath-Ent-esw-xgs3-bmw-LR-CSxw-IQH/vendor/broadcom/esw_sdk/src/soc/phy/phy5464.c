/*
 * $Id: phy5464.c,v 1.1 2011/04/18 17:11:07 mruas Exp $
...
_phy_5464_reset_setup(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM53314(pc)) {
        ...
        if ((rev_id == BCM53310_B0_REV_ID) ||
            (rev_id == BCM53311_B0_REV_ID) ||
            (rev_id == BCM53312_B0_REV_ID) ||
            (rev_id == BCM53313_B0_REV_ID) ||
            (rev_id == BCM53314_B0_REV_ID)) {                
            SOC_IF_ERROR_RETURN
                (PHY5464_REG_WRITE(unit, pc, 0x00, 0x0f76, 0x15, 0x100));
            SOC_IF_ERROR_RETURN
                (PHY5464_REG_WRITE(unit, pc, 0x00, 0x0f75, 0x15, 0x29));
            SOC_IF_ERROR_RETURN
                (PHY5464_REG_WRITE(unit, pc, 0x00, 0x0f74, 0x15, 0xd087));
            SOC_IF_ERROR_RETURN
                (PHY5464_REG_WRITE(unit, pc, 0x00, 0x0000, 0x18, 0x0C00));
            SOC_IF_ERROR_RETURN
                (PHY5464_REG_WRITE(unit, pc, 0x00, 0x601f, 0x15, 0x0110));                            
        }            
    }
#endif
                    ...
                    soc_port_cable_diag_t *status)
{
    ...
    if (soc_property_get(unit, "cable_diag_hw", PHY_5464_CABLE_DIAG_HW)) {
        uint16              tmp, result;
