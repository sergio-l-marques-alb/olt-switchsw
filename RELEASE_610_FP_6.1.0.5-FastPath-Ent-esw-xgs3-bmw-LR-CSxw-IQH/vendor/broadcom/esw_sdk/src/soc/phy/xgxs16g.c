/*
 * $Id: xgxs16g.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 ...
 *
 * File:        xgxs16g.c
 * Purpose:     Broadcom 1000/2500/10000/12000/13000/16000 SerDes 
 *              (XGXS-16G)
 */
...
phy_xgxs16g_init(int unit, soc_port_t port)
{
    ...
    if (IS_XE_PORT(unit, port) || IS_HG_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(phy_xgxs16g_speed_set(unit, port, 10000));
    } else if (IS_GE_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(phy_xgxs16g_speed_set(unit, port, 1000));
    }

    if (PHY_EXTERNAL_MODE(unit, port) &&
        (IS_XE_PORT(unit, port) || IS_HG_PORT(unit, port))) {
        SOC_IF_ERROR_RETURN(phy_xgxs16g_an_set(unit, port, FALSE));
    } else {
        SOC_IF_ERROR_RETURN(phy_xgxs16g_an_set(unit, port, TRUE));
    }
                               ...
                               soc_port_ability_t *ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_XGXS16G_COMBO_IEEE0_MIISTATr(unit, pc, &mii_stat_combo));
    SOC_IF_ERROR_RETURN
        (READ_XGXS16G_GP_STATUS_XGXSSTATUS3r(unit, pc, &link_stat_gp));
