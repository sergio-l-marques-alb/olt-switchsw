/*
 * $Id: hl65.c,v 1.1 2011/04/18 17:11:07 mruas Exp $ 
...
_phy_hl65_combo_core_init(int unit, soc_port_t port)
{
    ...
    /* point to CL73 AN device*/
    SOC_IF_ERROR_RETURN
        (WRITE_HL65_AERBLK_AERr(unit,pc,0x3800));

    /* disable the CL73 device */
    SOC_IF_ERROR_RETURN
        (WRITE_HL65_IEEE0BLK_MIICNTLr(unit,pc,0));

    /* point back to combo core device */
    SOC_IF_ERROR_RETURN
        (WRITE_HL65_AERBLK_AERr(unit,pc,0x00));
        ...
        if (lane_map != 0x0123) {
            ...
            if (chk_map == 0xf) {
                SOC_IF_ERROR_RETURN     /* Enable RX Lane swap */
                   (MODIFY_HL65_XGXSBLK2_RXLNSWAPr(unit, pc, 0x8000 | hw_map,
                                                   0x80ff));
            } else {
    ...
    if (IS_XE_PORT(unit, port) || IS_HG_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(phy_hl65_speed_set(unit, port, 10000));
    } else if (IS_GE_PORT(unit, port)) {
        SOC_IF_ERROR_RETURN(phy_hl65_speed_set(unit, port, 1000));
    }
                                                                                
    if (PHY_EXTERNAL_MODE(unit, port) &&
        (IS_XE_PORT(unit, port) || IS_HG_PORT(unit, port))) {
        SOC_IF_ERROR_RETURN(phy_hl65_an_set(unit, port, FALSE));
    } else {
        SOC_IF_ERROR_RETURN(phy_hl65_an_set(unit, port, TRUE));
    }
...
_phy_hl65_fabric_init(int unit, soc_port_t port)
{
    ...
    if (serdes_id0 == HL_SERDES_ID0_REVID_C0) {
        ...
        /* XgxsBlk1 Lanectrl0 (8015) :: Clear CL36 and ED66  */
        SOC_IF_ERROR_RETURN
...
#if 000
	/* these values are written during speed set */
        /*
         * XgxsBlk1_laneCtrl1 (0x8016)
         * RX/TX DWSDR_DIV1 all lanes == 0xffff
         */
        SOC_IF_ERROR_RETURN
...
phy_hl65_link_get(int unit, soc_port_t port, int *link)
{
    ...
    *link = FALSE;
    ...
    if (!PHY_INDEPENDENT_LANE_MODE(unit, port)) {
        SOC_IF_ERROR_RETURN
            (READ_HL65_IEEE0BLK_MIISTATr(unit, pc, &mii_stat));
...
phy_hl65_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (PHY_INDEPENDENT_LANE_MODE(unit, port) && (speed > 2500)) {
        return SOC_E_PARAM;
    } 
...
phy_hl65_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    if (PHY_INDEPENDENT_LANE_MODE(unit, port)) {
        ...
        if ((xgxs20g_stat & AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_MASK) == 
                       AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_dr_20G_X4) {
            *speed=20000;
        } else if ((xgxs20g_stat & 
                               AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_MASK) == 
                       AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_dr_21G_X4) {
            *speed=21000;
        } else if ((xgxs20g_stat & 
                               AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_MASK) == 
                       AN73_PDET_XGXSSTATUS4_ACTUAL_SPEED_LN0_dr_25G_X4) {
            *speed=25000;
        }
                            ...
                            soc_port_ability_t *ability)
{
    ...
    an_adv = (mode & SOC_PA_SPEED_1000MB) ? CL73_AN_ADV2_LP_TECH_10G_KX4 : 0;
    an_adv = (mode & SOC_PA_SPEED_10GB) ? CL73_AN_ADV2_LP_TECH_1G_KX : 0;

    SOC_IF_ERROR_RETURN
        (MODIFY_CL73_IEEEB1_AN_ADV2r(unit, pc, an_adv,
                                     CL73_AN_ADV2_LP_TECH));

    switch (ability->pause) {
                        ...
                        soc_port_ability_t *ability)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_HL65_COMBO_IEEE0_MIISTATr(unit, pc, &mii_stat_combo));
    SOC_IF_ERROR_RETURN
        (READ_HL65_IEEE0BLK_MIISTATr(unit, pc, &mii_stat_ieee));

    SOC_DEBUG_PRINT((DK_PHY,
                     "u=%d p=%d mii_stat_combo=%04x mii_stat_ieee=%04x\n",
                     unit, port, mii_stat_combo, mii_stat_ieee));
    sal_memset(ability, 0, sizeof(*ability));
    mode = 0;
    if (((mii_stat_ieee & MII_STAT_LA) || (mii_stat_combo & MII_STAT_LA)) &&
...
phy_hl65_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_HC65_FABRIC_MODE(unit, port)) {
        ...
        if (enable) {
            SOC_IF_ERROR_RETURN(WRITE_HL65_DSC2B2_SM_CTRL0r(unit, pc, 0x0));
            SOC_IF_ERROR_RETURN(WRITE_HL65_DSC2BB_SM_CTRL0r(unit, pc, 0x0));
            ...
            SOC_IF_ERROR_RETURN(WRITE_HL65_DSC2B2_SM_CTRL0r(unit, pc, 0x821));
            SOC_IF_ERROR_RETURN(WRITE_HL65_DSC2BB_SM_CTRL0r(unit, pc, 0x821));
            SOC_IF_ERROR_RETURN
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_HL65_IEEE0BLK_MIICNTLr(unit, pc, lb_bit, lb_mask));
                                          ...
                                          uint32 value)
{
    ...
    /* CDED & EDEN off: 0x8017: 0x0 */
    SOC_IF_ERROR_RETURN
        (WRITE_HL65_XGXSBLK1_LANECTRL2r(unit, pc, 0));
...
_phy_hl65_control_prbs_tx_enable_set(int unit, soc_port_t port, uint32 value)
{
    ...
    if (PHY_HC65_FABRIC_MODE(unit, pc->port)) {
        return _phy_hl65_control_fabric_prbs_tx_enable_set(unit, port, value);
    } else {
        return _phy_hl65_control_xgxs_prbs_tx_enable_set(unit, port, value);
    }
    return SOC_E_NONE;
}
...
_phy_hl65_control_prbs_rx_status_get(int unit, soc_port_t port, uint32 *value)
{
    ...
    if (PHY_HC65_FABRIC_MODE(unit, pc->port)) {
        return _phy_hl65_control_fabric_prbs_rx_status_get(unit, port, value);
    } else {
        return _phy_hl65_control_xgxs_prbs_rx_status_get(unit, port, value);
    }
    return SOC_E_NONE;
}
                                         ...
                                         uint32 value)
{
     ...
     */
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit, pc, 0x0, 0x826E + (0x10 * pc->lane_num), &data));
    ...
    SOC_IF_ERROR_RETURN
        (HL65_REG_WRITE(unit, pc, 0x0, 0x826E + (0x10 * pc->lane_num), data));
                                ...
                                soc_phy_control_t type, uint32 value)
{
    ...
    /* 
     * high bit indicates 6.5 preemphasis programming - strip it from
     * programmed value
     */
    ivalue = value & 0x7fff;
    if (ivalue > 15) {
    ...
    if (PHY_HC65_FABRIC_MODE(unit, pc->port)) {
        /* If high bit set, program preemphasis for 6.5G */
        if (value & 0x8000) {
            SOC_IF_ERROR_RETURN
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_HL65_TXB_TX_OS_DRIVERr(unit, pc, data, mask));
    }
                                ...
                                soc_phy_control_t type, uint32 *value)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_HL65_TXB_TX_OS_DRIVERr(unit, pc, &data16));

    switch(type) {
    case SOC_PHY_CONTROL_PREEMPHASIS:
         *value = (data16 & TXB_TX_OS_DRIVER_PREEMPHASIS_POST_MASK) >>
                  TXB_TX_OS_DRIVER_PREEMPHASIS_POST_SHIFT;
         break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT:
         *value = (data16 & TXB_TX_OS_DRIVER_IDRIVER_MASK) >>
                  TXB_TX_OS_DRIVER_IDRIVER_SHIFT;
         break;
    case SOC_PHY_CONTROL_PRE_DRIVER_CURRENT:
         *value = (data16 & TXB_TX_OS_DRIVER_IPREDRIVER_MASK) >>
                  TXB_TX_OS_DRIVER_IPREDRIVER_SHIFT;
         break;
    case SOC_PHY_CONTROL_PRE_PREEMPHASIS:
         *value = (data16 & TXB_TX_OS_DRIVER_PREEMPHASIS_PRE_MASK) >>
                  TXB_TX_OS_DRIVER_PREEMPHASIS_PRE_SHIFT;
         break;
    default:
         /* should never get here */
         return SOC_E_PARAM;
...
STATIC int
_phy_hl65_tuning_done_get(int unit, soc_port_t port, uint16 *done)
{
    uint16      data;
    phy_ctrl_t *pc = INT_PHY_SW_STATE(unit, port);
    ...
    *done = 0;
     ...
     */
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,0x82B7 + (0x10 * pc->lane_num), &data));
                            ...
                            uint16 *dfe_tap_bin_avg)
{
	...
	/* uRegAddr = 0x82b5 + (uLaneAddr*0x10);
         ...
         */
        SOC_IF_ERROR_RETURN
            (HL65_REG_READ(unit,pc,0x0,0x82b5 + (0x10 * pc->lane_num), &data));
...
STATIC int
_phy_hl65_fine_tune(int unit, soc_port_t port)
{
    uint16   data, cnt;
    uint16   tuning_done;
    uint32   reg_addr;
     ...
     */
    reg_addr = 0x8260 + (pc->lane_num * 0x10);
    SOC_IF_ERROR_RETURN
     ...
     * rv = soc_bm9600_mdio_hc_read(unit, uPhyAddr, uLaneAddr, uRegAddr, &uData);
     */
    reg_addr = 0x8262 + (pc->lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    ...
    reg_addr = 0x821a + (pc->lane_num * 0x10);
     ...
     * uData = 0x8000; 
     */
    reg_addr =  0x821b + (pc->lane_num * 0x10);
     ...
     * uAvgVgaSum = 0;
     */
    reg_addr = 0x82b5 + (pc->lane_num * 0x10);
    avg_vga_sum = 0;
     ...
     * rv = soc_bm9600_mdio_hc_read(unit, uPhyAddr, uLaneAddr, uRegAddr, &uData);
     */
    reg_addr = 0x8215 + (pc->lane_num * 10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    ...
    for (cnt = 0; cnt < 50; cnt++) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_hl65_tuning_done_get(unit, port, &tuning_done));
...
STATIC int
_phy_hl65_control_serdes_driver_equalization_tune_status_far_end_get(
                                                               int unit, 
                                                               soc_port_t port, 
                                                               uint32 *value)
{
    ...
    SOC_IF_ERROR_RETURN
        (HL65_REG_WRITE(unit,pc,0x0,0x80B0 + (0x10 * pc->lane_num), saved_status_select));
    ...
    /* status = _bcm_bm9600_hypercore_tuning_done_get(unit, uPhyAddr, uLaneAddr, &bTuningDone);*/
    SOC_IF_ERROR_RETURN
        (_phy_hl65_tuning_done_get(unit, port, &tuning_done));
    ...
    if (!tuning_done) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_hl65_tuning_status_get(unit, port, &avg_vga, &avg_dfe));
        ...
        if (avg_dfe < 8) {
             ...
             */
            SOC_IF_ERROR_RETURN
                (HL65_REG_READ(unit,pc,0x0,0x82b8 + (0x10 * pc->lane_num), &data));
        ...
        if (!negative && (avg_dfe < 30) && (postc_metric < 15)) { 
            SOC_DEBUG_PRINT((DK_WARN, 
        ...
        SOC_IF_ERROR_RETURN(_phy_hl65_fine_tune(unit, port));
    }
    return SOC_E_NONE;
}
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_SERDES_DRIVER_TUNE:
        rv = _phy_hl65_control_serdes_driver_tune_set(unit, port, value);
        break;
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_SERDES_DRIVER_EQUALIZATION_TUNE_STATUS_FAR_END:
        rv = _phy_hl65_control_serdes_driver_equalization_tune_status_far_end_get(unit, port, value);
        break;
    default:
        rv = SOC_E_UNAVAIL;
        break; 
