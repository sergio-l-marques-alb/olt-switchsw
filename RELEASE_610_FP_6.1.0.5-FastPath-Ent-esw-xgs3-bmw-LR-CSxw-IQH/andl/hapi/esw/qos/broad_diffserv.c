                                                      ...
                                                      L7_ushort16         ethType)
{
     ...
     */
    result = hapiBroadPolicyRuleCopy(policy, confRule, &nonConfRule);  

    if (L7_SUCCESS == result)
    {
        /* clear all actions associated with non-conf rule */
        result = hapiBroadPolicyRuleActionClearAll(policy, nonConfRule);
    }
    ...
    if (L7_SUCCESS == result)
    {
        ...
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_COLORAWARE_COS:
            ...
            result = hapiBroadPolicyRuleQualifierAdd(policy, confRule, BROAD_FIELD_OCOS, &value, defaultMask);
            break;
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_COLORAWARE_IPDSCP:
            ...
            if (ethType == 0x86dd)
            {
              result = hapiBroadPolicyRuleQualifierAdd(policy, confRule, BROAD_FIELD_IP6_TRAFFIC_CLASS, &value, defaultMask);
            }
            else
            {
              result = hapiBroadPolicyRuleQualifierAdd(policy, confRule, BROAD_FIELD_DSCP, &value, defaultMask);
            }
            break;
    ...
    if ((L7_SUCCESS == result) && (direction == HAPI_QOS_INTF_DIR_IN))
    {
        ...
        result = hapiBroadPolicyRuleActionAdd(policy,
                                              confRule,
    ...
    if (L7_SUCCESS == result)
    {
        ...
        if (L7_SUCCESS == result)
        {
            result = hapiBroadPolicyRuleNonConfActionGet(policy,
                                                         confRule,
                                                         &nonConfAction,
                                                         &nonConfValue0,
                                                         &nonConfValue1,
        ...
        if (L7_SUCCESS == result)
        {
            result = hapiBroadPolicyRuleNonConfActionAdd(policy,
                                                         nonConfRule,
                                                         nonConfAction,
                                                         nonConfValue0,
                                                         nonConfValue1,
                                            ...
                                            DAPI_t             *dapi_g)
{
    ...
    classDefLen = osapiNtohs(pClassDefTLV->length);

    /* point to first rule */
    pRuleTLV = GET_SUB_TLV(pClassDefTLV,0);

    /* start a new rule */
    result = hapiBroadPolicyRuleAdd(diffId, &ruleId);
    if (L7_SUCCESS != result)
    {
        return result;
    }
    ...
    while (classDefLen > 0)
    {
        ...
        if (osapiNtohs(pRuleTLV->type) != L7_QOS_DIFFSERV_TLV_CLASS_RULE_DEF_TYPE)
        {
            return L7_FAILURE;
        }
        
        /* deny flag is only used for 'match not' class types, which we don't support */
        classRuleLen = osapiNtohs(pRuleTLV->length) - 4;    /* subtract 4 to account for denyFlag */
        ...
        while ((classRuleLen > 0) && (matchEvery == L7_FALSE))
        {
            ...
            case L7_QOS_DIFFSERV_TLV_CLASS_MATCH_DSTIP_TYPE:
                ...
                result = hapiBroadPolicyRuleQualifierAdd(diffId, ruleId,
                                                         BROAD_FIELD_DIP,
                                                         (L7_uchar8 *)&data32,
                                                         (L7_uchar8 *)&mask32);
                break;
    ...
    policyLen = osapiNtohs(pPolicyDefTLV->length);
    ...
    while (policyLen > 0)
    {
        ...
        switch (osapiNtohs(pPolicyTLV->type))
        {
        ...
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_ASSIGN_QUEUE_TYPE:
            result = hapiBroadPolicyRuleActionAdd(diffId, ruleId,
        ...
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_REDIRECT_TYPE:
            {
                ...
                if (L7_LAG_SLOT_NUM == redirUsp.slot)
                {
                    ...
                    if (hapiBroadQosAddDepends(diffId, &redirUsp, dapi_g) != L7_SUCCESS)
                        result = L7_FAILURE;
                }
            }
            break;
        ...
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_MIRROR_TYPE:
            {   /* Mirror Policy */
                ...
                {
                    /* This policy now depends upon the LAG as it's destination. */
                    if (hapiBroadQosAddDepends(diffId, &mirrorUsp, dapi_g) != L7_SUCCESS)
                        result = L7_FAILURE;
                }
            }
            break;
        ...
        case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_DEF_TYPE:
            ...
            policeLen = osapiNtohs(pPolicyTLV->length) - 8; /* subtract 8 to account for style/color */
            ...
            while (policeLen > 0)
            {
                ...
                switch (osapiNtohs(pPoliceActionTLV->type))
                {
                ...
                case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_EXCEED_TYPE:
                    ...
                    case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_ACT_DROP:
                        if (L7_SUCCESS == result)
                        {
                            result = hapiBroadPolicyRuleExceedActionAdd(diffId, ruleId,
                                                                        BROAD_ACTION_HARD_DROP,
                                                                        0,
                                                                        0,
                                                                        0);
                ...
                case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_NONCONFORM_TYPE:
                    ...
                    case L7_QOS_DIFFSERV_TLV_POLICY_ATTR_POLICE_ACT_DROP:
                        if (L7_SUCCESS == result)
                        {
                            result = hapiBroadPolicyRuleNonConfActionAdd(diffId, ruleId,
                                                                         BROAD_ACTION_HARD_DROP,
                                                                         0,
                                                                         0,
                                                                         0);
                ...
                policeLen -= (osapiNtohs(pPoliceActionTLV->length) + sizeof(pPoliceActionTLV->type) + sizeof(pPoliceActionTLV->length));
            ...
            if (L7_SUCCESS == result)
            {
                ...
                result = hapiBroadPolicyRuleMeterAdd(diffId, ruleId, &meterDef);
            }
            break;
        ...
        policyLen -= (osapiNtohs(pPolicyTLV->length) + sizeof(pPolicyTLV->type) + sizeof(pPolicyTLV->length));
        /* get next policy */
    ...
    if (groupId != HAPI_DOT1X_GROUP_ID_INVALID)
    {
      ...
      if (hapiBroadPolicyRuleQualifierAdd(diffId, ruleId,
    ...
    if ((osapiNtohs(pPolicyDefTLV->length)) && (L7_FALSE == policing))
    {
        /* add a counter for non-policing instances */
         result = hapiBroadPolicyRuleCounterAdd(diffId, ruleId, BROAD_COUNT_PACKETS, hapiPortPtr->bcm_port);
    }
    ...
    if ((L7_SUCCESS == result) && (ethType2))
    {
        ...
        result = hapiBroadPolicyRuleCopy(diffId, ruleId, &newRule);
        if (L7_SUCCESS == result)
        {
            /* update new rule with ETHTYPE2 value */
            result = hapiBroadPolicyRuleQualifierAdd(diffId, newRule,
                                                     BROAD_FIELD_ETHTYPE,
                                                     (L7_uchar8*)&ethType2,
                                                     defaultMask);
        }
                                      ...
                                      DAPI_t              *dapi_g)
{
  ...
  if (pTLV->type != L7_QOS_DIFFSERV_TLV_POLICY_ADD_TYPE)
  {
      return L7_FAILURE;
  }
  ...
  result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_PORT, policyId);

  if (L7_SUCCESS == result)
  {
    result = hapiBroadPolicyStageSet(*policyId, stage);
  ...
  while (instanceCount-- > 0)
  {
    ...
    if (pTLV->type != L7_QOS_DIFFSERV_TLV_CLSF_INST_TYPE)
    {
      result = L7_FAILURE;
      break;
    }
    ...
    if (pClassDefTLV->type != L7_QOS_DIFFSERV_TLV_CLASS_DEF_TYPE)
    {
      result = L7_FAILURE;
      break;
    }
    ...
    if (pPolicyDefTLV->type != L7_QOS_DIFFSERV_TLV_POLICY_DEF_TYPE)
    {
      result = L7_FAILURE;
      break;
    }
...
L7_RC_t hapiBroadQosDiffServInstAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    if (L7_TRUE == qosPort->aclds.policyInit[direction])
    {
    ...
    if (L7_SUCCESS != result)
    {
      ...
      qosPort->aclds.policyInit[direction] = L7_FALSE;
...
L7_RC_t hapiBroadQosDiffServInstDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    if (osapiNtohs(pTLV->type) != L7_QOS_DIFFSERV_TLV_INST_DEL_LIST_TYPE)
    {
        return L7_FAILURE;
    }
    ...
    if (L7_FALSE == qosPort->aclds.policyInit[direction])
    {
        LOG_MSG("No DiffServ policy applied on port %d\n", hapiPortPtr->bcmx_lport);
        hapiBroadQosSemGive(dapi_g);
        return L7_ERROR;
    }

    /* Remove the policy from this interface. */
    result = hapiBroadQosRemovePolicy(qosPort->aclds.policyId[direction], usp, dapi_g);
    if (L7_SUCCESS != result)
    {
        /* continue to clean-up even if an error occurs */
        LOG_MSG("DiffServ not removed from port %d\n", hapiPortPtr->bcmx_lport);
    }
    ...
    qosPort->aclds.policyInit[direction] = L7_FALSE;          /* update per-port QoS table */
    qosPort->aclds.policyId[direction]   = BROAD_POLICY_INVALID;
...
*
*********************************************************************/
static L7_RC_t hapiBroadQosDiffServIntfInboundStatAccum(HAPI_BROAD_QOS_PORT_t           *qosPortPtr,
                                                        DAPI_QOS_DIFFSERV_INTF_STAT_IN_t type,
    ...
    *value = 0;
	...
	for (i = 0; i < L7_DIFFSERV_INST_PER_POLICY_LIM; i++) 
	{
	  if ((qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_IN][i].inUse == L7_TRUE) && 
	     ...
	     rule = qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_IN][i].rule;
	}
...
#ifdef BROAD_ACCUMULATE_RULE_STATS
    if ((L7_DIFFSERV_INST_PER_POLICY_LIM == i) ||
        (BROAD_POLICY_RULE_INVALID == qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_IN][i+1]))
    {
        /* last instance in policy so sum all remaining rules */
        lastRule = numStats;
    }
    else
    {
        /* sum all rules up to the next policy */
        lastRule = qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_IN][i+1];
    }

    for (; rule < lastRule; rule++)
#endif
    {
        switch (type)
        {
        case DAPI_QOS_DIFFSERV_STAT_IN_OFFERED_PACKETS:
            if (L7_TRUE == stats[rule].meter)
            {
                /* offered = in_prof + out_prof */
                *value += stats[rule].statMode.meter.in_prof;
                *value += stats[rule].statMode.meter.out_prof;
            }
            else
            {
                *value += stats[rule].statMode.counter.count;
            }
            break;
...
*
*********************************************************************/
static L7_RC_t hapiBroadQosDiffServIntfOutboundStatAccum(HAPI_BROAD_QOS_PORT_t           *qosPortPtr,
                                                         DAPI_QOS_DIFFSERV_INTF_STAT_IN_t type,
                                                         BROAD_POLICY_STATS_t             stats[],
                                                         L7_uint32                        numStats,
                                                         L7_uint32                        instance,
                                                         L7_uint64                       *value)
{
    ...
    *value = 0;
...
#ifdef BROAD_ACCUMULATE_RULE_STATS
    if ((L7_DIFFSERV_INST_PER_POLICY_LIM == i) ||
        (BROAD_POLICY_RULE_INVALID == qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_OUT][i+1]))
    {
        /* last instance in policy so sum all remaining rules */
        lastRule = numStats;
    }
    ...
    {
        /* sum all rules up to the next policy */
        lastRule = qosPortPtr->aclds.ruleList[HAPI_QOS_INTF_DIR_OUT][i+1];
    }
...
L7_RC_t hapiBroadQosDiffServIntfStatInGet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    L7_uint32              instance;
    BROAD_POLICY_t         policy;
    BROAD_POLICY_STATS_t   stats[BROAD_MAX_RULES_PER_DIFFSERV_POLICY];
    L7_uint32              numStats = BROAD_MAX_RULES_PER_DIFFSERV_POLICY;
    ...
    if (L7_TRUE != qosPortPtr->aclds.policyInit[direction])
        result = L7_NOT_EXIST;

    if (L7_SUCCESS == result)
    {
        /* This API reads all the stats for a port, so it is possible to
         * call it once and cache the remaining values if performance
         * improvement is needed, at the expense of memory.
         */
        result = hapiBroadPolicyStatsGet(policy, &numStats, stats);
    }

    if (L7_SUCCESS == result)
    {
        result = hapiBroadQosDiffServIntfInboundStatAccum(qosPortPtr,
                                                          qosCmd->cmdData.diffServIntfStatInGet.statistic,
                                                          stats,
                                                          numStats,
                                                          instance,
                                                          &tmpVal);
...
L7_RC_t hapiBroadQosDiffServIntfStatOutGet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (L7_SUCCESS == result)
  {
      result = hapiBroadQosDiffServIntfOutboundStatAccum(qosPortPtr,
                                                         qosCmd->cmdData.diffServIntfStatOutGet.statistic,
                                                         stats,
                                                         numStats,
                                                         instance,
                                                         &tmpVal);
