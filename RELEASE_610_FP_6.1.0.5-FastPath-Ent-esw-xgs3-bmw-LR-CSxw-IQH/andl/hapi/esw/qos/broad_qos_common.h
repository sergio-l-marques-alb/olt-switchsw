...
typedef struct
{
    /* A port can have one of two policies applied. If the port is
     * not part of a LAG then the port policy applies. Otherwise,
     * the LAG policy applies.
     */
    L7_BOOL        policyApplied[HAPI_QOS_INTF_DIR_COUNT];  /* indicates if any policy applies */
    BROAD_POLICY_t policyInEffect[HAPI_QOS_INTF_DIR_COUNT]; /* actual policy that is applied */

    L7_BOOL        policyInit[HAPI_QOS_INTF_DIR_COUNT];     /* indicates if port policy is valid */
...
typedef struct
{
    ...
    L7_ulong32                      minBw[BCM_COS_COUNT];   /* PTin modified: L7_uchar8 */
    L7_ulong32                      maxBw[BCM_COS_COUNT];   /* PTin modified: L7_uchar8 */
