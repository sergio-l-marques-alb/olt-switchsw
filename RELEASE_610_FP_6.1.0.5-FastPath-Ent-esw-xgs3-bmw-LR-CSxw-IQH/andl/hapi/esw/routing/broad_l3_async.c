  ...
  do
  {
    ...
    switch (pNhop->wl.cmd)
    {
      ...
      case BROAD_L3_NH_CMD_MODIFY:
        ...
        bcmx_l3_egress_t_init(&bcmxEgrObj);
            ...
            !(pNhop->flags & BROAD_L3_NH_LOCAL))
        {
          ...
          bcmxEgrObj.vlan = pMac->key.vlanId; 

          if (pMac->target_is_trunk == L7_TRUE)
          {
            bcmxEgrObj.flags |= BCM_L3_TGID;
            bcmxEgrObj.trunk = pMac->trunkId; 
          }
          else
          {
            bcmxEgrObj.lport = hapiPortPtr->bcmx_lport;
          }
          bcmxEgrObj.intf = pNhop->l3_intf_id;
          memcpy(bcmxEgrObj.mac_addr, pMac->key.macAddr, sizeof(bcm_mac_t));
        }
        else
        {
          /* Next hop is not resolved or is local. Point it to CPU L3 interface */
          cpuHapiPortPtr = hapiBroadL3CpuHapiPortGet(dapi_g);
          bcmxEgrObj.vlan = HPC_STACKING_VLAN_ID;
          bcmxEgrObj.lport = cpuHapiPortPtr->bcmx_lport;
          bcmxEgrObj.intf = hapiBroadL3CpuIntfId;
          memcpy(bcmxEgrObj.mac_addr, hapiBroadL3CpuMac, sizeof(bcm_mac_t));
        }
        ...
        hapiBroadL3HwNhopAdd(flags, pNhop, &bcmxEgrObj, BROAD_L3_ENTRY_CACHE);
      ...
      case BROAD_L3_NH_CMD_DELETE:
        ...
        else
        {
          ...
          {
            hapiBroadL3HwNhopDelete(egressIntfId, BROAD_L3_ENTRY_CACHE);
          }
...
static void hapiBroadL3AsyncHostWlistProcess (DAPI_t *dapi_g)
{
  ...
  do
  {
    ...
    bcmx_l3_host_t_init(&bcmxHostInfo); 
 
    if (pHost->key.family == L7_AF_INET)
    {
       bcmxHostInfo.l3a_ip_addr = pHost->key.addrUsp.addr.ipv4;
    }
    else
    {
       ...
       bcmxHostInfo.l3a_flags |= BCM_L3_IP6;
       memcpy(&bcmxHostInfo.l3a_ip6_addr, &(pHost->key.addrUsp.addr.ipv6),
                sizeof (bcm_ip6_t));
    }
    ...
    switch (pHost->wl.cmd)
    {
      ...
      case BROAD_L3_HOST_CMD_MODIFY:
        ...
        {
          bcmxHostInfo.l3a_flags |= BCM_L3_REPLACE;
        }

        if (pTunNhop != L7_NULL)
        {
          bcmxHostInfo.l3a_intf = pTunNhop->egressId;
        }
        else
        {
          bcmxHostInfo.l3a_intf = pNhop->egressId;
        }

        if (pHost->flags & BROAD_L3_HOST_LOCAL)
        {
          bcmxHostInfo.l3a_flags |= BCM_L3_RPE;
          bcmxHostInfo.l3a_pri = HAPI_BROAD_INGRESS_LOCAL_IP_PRIORITY;
        }
        ...
        hapiBroadL3HwHostAdd(pHost, &bcmxHostInfo, BROAD_L3_ENTRY_CACHE);
        break;
      ...
      case BROAD_L3_HOST_CMD_DELETE:
        ...
        hapiBroadL3HwHostDelete(&bcmxHostInfo, BROAD_L3_ENTRY_CACHE);
...
static void hapiBroadL3AsyncRouteWlistProcess (DAPI_t *dapi_g)
{
  ...
  do 
  {
    ...
    bcmx_l3_host_t_init(&bcmxHostInfo);
    bcmx_l3_route_t_init(&bcmxRouteInfo);

    /* Form the BCMX structure */
    if (pRoute->key.family == L7_AF_INET6)
    {
      bcmxRouteInfo.l3a_flags |= BCM_L3_IP6;
      memcpy(&bcmxRouteInfo.l3a_ip6_net,  &pRoute->key.ipAddr, sizeof(bcm_ip6_t));
      memcpy(&bcmxRouteInfo.l3a_ip6_mask, &pRoute->key.ipMask, sizeof(bcm_ip6_t));
                ...
                sizeof(bcm_ip6_t)) == 0)
      {
        if (hapiBroadL3Ipv6Prefix128Supported() == L7_FALSE)
        {
          ...
          bcmxHostInfo.l3a_flags |= BCM_L3_IP6;
          memcpy(&bcmxHostInfo.l3a_ip6_addr, &bcmxRouteInfo.l3a_ip6_net,
                 sizeof (bcm_ip6_t));
        }
      }
    }
    ...
    {
      bcmxRouteInfo.l3a_subnet = pRoute->key.ipAddr.ipv4;
      bcmxRouteInfo.l3a_ip_mask = pRoute->key.ipMask.ipv4;
    ...
    switch (pRoute->wl.cmd)
    {
      case BROAD_L3_ROUTE_CMD_ADD:
        if (BROAD_L3_IS_ROUTE_ECMP(pRoute))
        {
          bcmxRouteInfo.l3a_intf = pEcmp->egressId;  
          bcmxRouteInfo.l3a_flags |= BCM_L3_MULTIPATH;
        }
        else 
        {
          ...
          {
            /* Reject route */
            bcmxRouteInfo.l3a_intf = hapiBroadL3CpuEgrNhopId;
          }
          else
          {
            bcmxRouteInfo.l3a_intf = pNhop->egressId;
          }
        }
        ...
        {
          bcmxHostInfo.l3a_intf = bcmxRouteInfo.l3a_intf;

          usl_bcmx_l3_host_add (&bcmxHostInfo, 1, &rv);
        ...
        {
          hapiBroadL3HwRouteAdd(pRoute, &bcmxRouteInfo, BROAD_L3_ENTRY_CACHE);
        }
      ...
      case BROAD_L3_ROUTE_CMD_MODIFY:
        ...
        {
          bcmxRouteInfo.l3a_intf = pNhop->egressId; /* Got to be valid */
        }
        else
        {
          /* Reject route */
         ...
         * updated.
         */
        bcmxRouteInfo.l3a_flags |= BCM_L3_REPLACE;
        /* Remove the route from WL */
        HAPI_BROAD_L3_ROUTE_WLIST_DEL(pRoute);
        HAPI_BROAD_L3_SEMA_GIVE(hapiBroadL3Sema);
      ...
      case BROAD_L3_ROUTE_CMD_DELETE:
        ...
        {
          bcmxHostInfo.l3a_intf = bcmxRouteInfo.l3a_intf;
          usl_bcmx_l3_host_delete (&bcmxHostInfo, 1, &rv);
        }
        else
        {
          hapiBroadL3HwRouteDelete(&bcmxRouteInfo, BROAD_L3_ENTRY_CACHE);
                                          ...
                                          DAPI_t *dapi_g)
{
  ...
  bcmx_l3_intf_t           intfInfo;
  bcmx_tunnel_terminator_t termInfo;
  bcmx_tunnel_initiator_t  initInfo;
  bcm_mac_t                mac_addr;
  BROAD_L3_NH_ENTRY_t     *pNhop;
  ...
  /* Common tunnel init */
  bcmx_l3_intf_t_init(&intfInfo);
  bcmx_tunnel_terminator_t_init(&termInfo);
  bcmx_tunnel_initiator_t_init(&initInfo);
  ...
  if (replace == L7_TRUE)
  {
    intfInfo.l3a_intf_id = pTunEntry->bcmL3IntfId; /* Got to be valid */

    rv = bcmx_l3_intf_get(&intfInfo); /* Intf must exists */
    if (rv != BCM_E_NONE)
    {
      HAPI_BROAD_L3_LOG_ERROR(rv);
    }

    intfInfo.l3a_flags |= (BCM_L3_REPLACE | BCM_L3_WITH_ID);
  ...
  intfInfo.l3a_vid = vlanId;
  memcpy(&intfInfo.l3a_mac_addr, mac_addr, sizeof(bcm_mac_t));
  ...
  HAPI_BROAD_L3_BCMX_DBG(rv, "Tunnel usl_bcmx_l3_intf_create returned (%s),"
                       "L3 intf id %d\n", bcm_errmsg(rv), intfInfo.l3a_intf_id);

  if (rv == BCM_E_NONE)
  {
    pTunEntry->bcmL3IntfId = intfInfo.l3a_intf_id;
  }
  else
  {
    pTunEntry->rv = rv;
  ...
  if (pTunEntry->bcmL3IntfId != HAPI_BROAD_INVALID_L3_INTF_ID)
  {
        ...
        (pTunEntry->key.tunnelMode == L7_TUNNEL_MODE_4OVER4))
    {
      ...
      if ((pNhop != L7_NULL) && (pNhop->flags & BROAD_L3_NH_RESOLVED))
      {
        ...
        rv = usl_bcmx_tunnel_initiator_set(&intfInfo, &initInfo);
...
static void hapiBroadL3AsyncTunnelModify (BROAD_L3_TUNNEL_ENTRY_t *pTunEntry)
{
           ...
           (pTunEntry->key.tunnelMode == L7_TUNNEL_MODE_4OVER4))
  {
    ...
    pNhop = pTunEntry->pXportNhop;
    intfInfo.l3a_intf_id = pTunEntry->bcmL3IntfId;
    ...
    else if (pTunEntry->hasInitiator == L7_TRUE)
    {
      ...
      rv = usl_bcmx_tunnel_initiator_clear(&intfInfo);
    ...
    else if (pTunnelEntry->key.tunnelMode == L7_TUNNEL_MODE_6TO4)
    {
      ...
      if ((pV4Nhop != L7_NULL) && (pV4Nhop->flags & BROAD_L3_NH_RESOLVED))
      {
        ...
        if(L7_IP6_IS_ADDR_6TO4(&pTunNhop->key.addrUsp.addr.ipv6))
        {
          ...
          initInfo.dip = pV4Nhop->key.addrUsp.addr.ipv4;
          initInfo.ttl = 255;
          ...
          if (rv == BCM_E_NONE)
          {
            p6to4->relayTunnelId = intfInfo.l3a_intf_id;
            ...
            pTunNhop->l3_intf_id = intfInfo.l3a_intf_id;
            *pMac = pV4Nhop->pMac;
          }
        }
        else
    ...
    for (index=0; index < HAPI_BROAD_L3_MAX_6TO4_NHOP; index++)
    {
      if (pTunnelEntry->tun6to4Nhops[index].pV6Nhop == pTunNhop)
      {
        ...
        if ((pNhop != L7_NULL) && (pTunNhop->ref_count == 1))
        {
          ...
          {
            intfInfo.l3a_intf_id = *pRelayTunnelId;
...
hapiBroadL3v6LinkLocalRouteAddDel(L7_BOOL install, DAPI_t *dapi_g)
{
  ...
  memcpy(&bcmxRouteInfo.l3a_ip6_net, addr, sizeof(bcm_ip6_t));
  memcpy(&bcmxRouteInfo.l3a_ip6_mask, mask, sizeof(bcm_ip6_t));
  ...
  if (install == L7_TRUE)
  {
        ...
        (haveLinkLocals == L7_FALSE))
    {
       ...
       */
      result = usl_bcmx_l3_route_add(&bcmxRouteInfo, 1, &rv);
  ...
  else
  {
    if (haveLinkLocals == L7_TRUE)
    {
      ...
      result = usl_bcmx_l3_route_delete(&bcmxRouteInfo, 1, &rv);
      if ((result == L7_FAILURE) || (rv != BCM_E_NONE))
      {
        HAPI_BROAD_L3_ASYNC_DBG("hapiBroadL3v6LinkLocalRouteAddDel:"
