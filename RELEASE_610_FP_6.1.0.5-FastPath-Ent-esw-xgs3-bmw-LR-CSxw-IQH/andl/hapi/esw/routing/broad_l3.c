    ...
    else if (L7_TRUE == IS_PORT_TYPE_CPU(dapiPortPtr))
    {
          ...
          (hapiBroadL3CpuEgrNhopId == HAPI_BROAD_INVALID_L3_INTF_ID))
      {
        ...
        result = sysapiHpcIfaceMacGet(L7_LOGICAL_VLAN_INTF, L7_VLAN_SLOT_NUM, 0,
                                      (L7_uchar8 *)hapiBroadL3CpuMac);
        if (result != L7_SUCCESS)
        {
          HAPI_BROAD_L3_LOG_ERROR (result);
        }

        bcm_l3_intf_t_init(&cpuIntfInfo);
        memcpy(cpuIntfInfo.l3a_mac_addr, hapiBroadL3CpuMac, sizeof(bcm_mac_t));
        cpuIntfInfo.l3a_vid = HPC_STACKING_VLAN_ID;
        cpuIntfInfo.l3a_flags |= (BCM_L3_ADD_TO_ARL | BCM_L3_L2ONLY);
        ...
        {
          hapiBroadL3CpuIntfId = cpuIntfInfo.l3a_intf_id;
          hapiPortPtr->l3_interface_id = cpuIntfInfo.l3a_intf_id;
          ...
          result = usl_bcmx_l3_egress_create(&egrFlags, &bcmxEgrObj, 1,
                                     ...
                                     DAPI_t *dapi_g)
{
  ...
  if (hapiBroadL3CpuIntfId != HAPI_BROAD_INVALID_L3_INTF_ID) 
  {
    ...
    result = usl_bcmx_l3_egress_destroy(&hapiBroadL3CpuEgrNhopId, 1, &rv);
    ...
    {
      LOG_MSG("hapiBroadL3RouterCardRemove: Failed deleting CPU nexthop %d\n", rv);
    }

    hapiBroadL3CpuEgrNhopId = HAPI_BROAD_INVALID_L3_INTF_ID;
    
    bcm_l3_intf_t_init(&cpuIntfInfo);
    cpuIntfInfo.l3a_intf_id = hapiBroadL3CpuIntfId;
                                  ...
                                  DAPI_t *dapi_g)
{
  ...
  enable = (((ipv4_enable == L7_TRUE) || (ipv6_enable == L7_TRUE))
            ? L7_TRUE : L7_FALSE);
#if 0 /* need this to be on for wireless switching builds; this was removed to keep it from being disabled */
  rv = bcmx_l3_enable_set(enable);
                                     ...
                                     DAPI_t *dapi_g)
{
  ...
  if (L7_TRUE == IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    ...
    rv = bcmx_vlan_mcast_flood_set(vid, BCM_VLAN_MCAST_FLOOD_NONE /*BCM_VLAN_MCAST_FLOOD_UNKNOWN*/ );
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR (rv);
    }
    ...
    {
      if (bcmx_vlan_control_vlan_get(vid, &vlanControl) == BCM_E_NONE)
      {
        vlanControl.flags |= BCM_VLAN_UNKNOWN_IP6_MCAST_TOCPU;

        rv = bcmx_vlan_control_vlan_set(vid, vlanControl);
        if (L7_BCMX_OK(rv) != L7_TRUE)
        {
          HAPI_BROAD_L3_LOG_ERROR (rv);
        }
    ...
    rv = bcmx_lplist_add(&lplist, cpuHapiPortPtr->bcmx_lport);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR (vid);
    }

    rv = bcmx_lplist_add(&lplist, hapiPortPtr->bcmx_lport);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR (vid);
    }

    rv = bcmx_vlan_port_add(vid, lplist, lplist);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR (vid);
    }

    rv = bcmx_lplist_free(&lplist);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR (vid);
    }

    /* Set STP state of routing port to forwarding. */
    rv = bcmx_port_stp_set(hapiPortPtr->bcmx_lport, BCM_STG_STP_FORWARD);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
     ...
     */
    rv = l7_bcmx_port_learn_set(hapiPortPtr->bcmx_lport, BCM_PORT_LEARN_FWD);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR(rv);
    }
    ...
    rv = l7_bcmx_port_untagged_vlan_set(hapiPortPtr->bcmx_lport, vid);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR(rv);
    }
    ...
    rv = l7_bcmx_port_discard_set(hapiPortPtr->bcmx_lport, BCM_PORT_DISCARD_NONE);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      HAPI_BROAD_L3_LOG_ERROR(rv);
    }
  ...
  intfInfo.l3a_intf_id = HAPI_BROAD_INVALID_L3_INTF_ID;
  memcpy(intfInfo.l3a_mac_addr, mac_addr, sizeof(bcm_mac_t));
  intfInfo.l3a_vid = vid;
  intfInfo.l3a_flags |= BCM_L3_ADD_TO_ARL;
  ...
  else
  {
    HAPI_BROAD_L3_DEBUG(broadL3Debug,
                        ...
                        "intf_id = %d\n", intfInfo.l3a_intf_id);
    hapiPortPtr->l3_interface_id = intfInfo.l3a_intf_id;
                                     ...
                                     DAPI_t *dapi_g)
{
  ...
  DAPI_PORT_t *dapiPortPtr;
  BROAD_PORT_t *hapiPortPtr;
  DAPI_QVLAN_MGMT_CMD_t cmdVlanPortConfig;
  bcm_vlan_t vid;
  bcmx_l3_intf_t intfInfo;
  int rv;
  L7_uint32 ip_addr = 0;
  ...
  if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr) == L7_TRUE)
  {
    ...
    /* Restore the pvid and acceptableFrameType to the original
     * setting for the port 
     */
    cmdVlanPortConfig.cmdData.portConfig.getOrSet = DAPI_CMD_SET;
    cmdVlanPortConfig.cmdData.portConfig.protocolModify = L7_FALSE;
    cmdVlanPortConfig.cmdData.portConfig.vlanIDModify = L7_TRUE;
    cmdVlanPortConfig.cmdData.portConfig.vlanID = hapiPortPtr->pvid;
    cmdVlanPortConfig.cmdData.portConfig.acceptFrameTypeModify = L7_TRUE;
    cmdVlanPortConfig.cmdData.portConfig.acceptFrameType =
      hapiPortPtr->acceptFrameType;
    cmdVlanPortConfig.cmdData.portConfig.ingressFilteringEnabledModify =
      L7_TRUE;
    cmdVlanPortConfig.cmdData.portConfig.ingressFilteringEnabled =
      hapiPortPtr->ingressFilteringEnabled;
    cmdVlanPortConfig.cmdData.portConfig.getOrSet = DAPI_CMD_SET;

    if ((result = hapiBroadQvlanPortConfig(usp,
                                           DAPI_CMD_QVLAN_PORT_CONFIG,
                                           &cmdVlanPortConfig,dapi_g))
        != L7_SUCCESS)
    {
      return result;
    }
    ...
    rv = l7_bcmx_port_learn_set(hapiPortPtr->bcmx_lport, flags);
  ...
  HAPI_BROAD_L3_DEBUG(broadL3Debug, "hapiBroadL3IntfDelete: Deleting %d (%s)\n",
                      intfInfo.l3a_intf_id, bcm_errmsg(rv));  
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
    HAPI_BROAD_L3_LOG_ERROR(rv);
  }
                                 ...
                                 DAPI_t *dapi_g)
{
    ...
    case DAPI_CMD_IPV6_ROUTE_ENTRY_ADD:
           ...
           (dapiCmd->cmdData.routeAdd.ip6Mask.in6.addr32[3] != 0)))
      {
        if (hapiBroadL3Ipv6Prefix128Supported() == L7_FALSE)
        {
          ...
          LOG_MSG("hapiBroadL3RouteEntryAdd: Ignoring IPv6 route with prefix len"
                " /65 through /127\n");
          return L7_SUCCESS;
        }
      }
   ...
   */
  if (hapiBroadL3RouteTableFull() == L7_TRUE)
  {
    hapiBroadL3AsyncWait(usp, cmd);
    ...
    if (hapiBroadL3RouteTableFull() == L7_TRUE)
    {
      HAPI_BROAD_L3_DEBUG(broadL3Debug, "hapiBroadL3RouteEntryAdd: table full");
      return L7_FAILURE;
    }  
  }
                                ...
                                DAPI_t *dapi_g)
{
   ...
   */
  if (hapiBroadL3HostTableFull() == L7_TRUE)
  {
    /* Wait for Async task to finish processing */
    hapiBroadL3AsyncWait(usp, cmd);
    ...
    if (hapiBroadL3HostTableFull() == L7_TRUE)
    {
      HAPI_BROAD_L3_DEBUG(broadL3Debug, "hapiBroadL3HostEntryAdd: table full");
      return L7_FAILURE;
    }
  }
                                           ...
                                           DAPI_t *dapi_g)
{
   ...
   */
  rv = bcmx_l2_addr_get(pMac->key.macAddr, pMac->key.vlanId, &bcmx_l2, L7_NULL);
                                         ...
                                         DAPI_t *dapi_g)
{
  ...
  for (entry=0; entry<dapiCmd->cmdData.arpQuery.numOfEntries; entry++)
  {
    ...
    if (L7_AF_INET6 == family)
    {
      inHw = usl_l3host_hit_flags_get(family,
    ...
    if (L7_NULL != pHostEntry)
    {
      ...
      if (L7_FALSE == notInHw)
      {
        ...
        bcmx_l3_host_t_init(&hostInfo);

        if (L7_AF_INET6 == family)
        {
          memcpy(&(hostInfo.l3a_ip6_addr),
                 &(dapiCmd->cmdData.arpQuery.ip6Addr[entry]),
        ...
        {
          hostInfo.l3a_ip_addr = dapiCmd->cmdData.arpQuery.ipAddr[entry];
...
hapiBroadL3TunnelNhopGet(BROAD_L3_NH_KEY_t *pTunNhopKey, DAPI_t * dapi_g)
{
  ...
  if (IS_USP_TYPE_TUNNEL(&pTunNhopKey->addrUsp.usp))
  {
    ...
    else if (pTunnelEntry->key.tunnelMode == L7_TUNNEL_MODE_6TO4)
    {
      ...
      /* Here if a tunnel next hop is not found. Do not increment ref count
       * for tunnel nexthop here. Thats done by the caller.
       */
                                      ...
                                      DAPI_t *dapi_g)
{
  ...
  memcpy(intfInfo.l3a_mac_addr, &(L7_ENET_VRRP_MAC_ADDR), sizeof(bcm_mac_t));
  intfInfo.l3a_mac_addr[5] = vrid;
  ...
  {
    hapiBroadL3HostPolicyInstall (usp, (L7_uchar8 *)intfInfo.l3a_mac_addr, dapi_g);
    hapiPortPtr->vrrp_interface_id[vrid] = intfInfo.l3a_intf_id;
  }
                                         ...
                                         DAPI_t *dapi_g)
{
  ...
  bcm_l3_intf_t_init(&intfInfo);
  intfInfo.l3a_intf_id = hapiPortPtr->vrrp_interface_id[vrid];
  ...
  memcpy(intfInfo.l3a_mac_addr, &(L7_ENET_VRRP_MAC_ADDR), sizeof(bcm_mac_t));
  intfInfo.l3a_mac_addr[5] = vrid;
  hapiBroadL3HostPolicyRemove (usp, (L7_uchar8 *)intfInfo.l3a_mac_addr, dapi_g);
...
L7_RC_t hapiBroadL3IcmpRedirVlanConfig(L7_ushort16 vlanId, L7_int32 enable)
{
  ...
  memset(&vlanControl, 0, sizeof(vlanControl));

  rv = bcmx_vlan_control_vlan_get(vlanId, &vlanControl);

  HAPI_BROAD_L3_BCMX_DBG(rv, "bcmx_vlan_control_vlan_get: returned %s",
                         bcm_errmsg(rv));
  ...
  if (enable)
  {
    vlanControl.flags |= BCM_VLAN_ICMP_REDIRECT_TOCPU;
  }
  else
  {
    vlanControl.flags &= ~BCM_VLAN_ICMP_REDIRECT_TOCPU;
                                  ...
                                  DAPI_t *dapi_g)
{
  ...
  HAPI_BROAD_L3_DEBUG(broadL3Debug,
                      "hapiBroadRouteUpdateMacLearn: Mac : "
