...
typedef struct BROAD_L3_NHOP_CACHE_s
{
  ...
  bcmx_l3_egress_t    *pBcmxInfo;      /* BCMX data assoicated with next hops */
...
typedef struct BROAD_L3_HOST_CACHE_s
{
  ...
  bcmx_l3_host_t    *pBcmxInfo;        /* BCMX data associated with hosts */
...
typedef struct BROAD_L3_ROUTE_CACHE_s
{
  ...
  bcmx_l3_route_t        *pBcmxInfo;   /* BCMX data associated with routes */
...
L7_RC_t hapiBroadL3CacheInit(void)
{
  ...
  size = sizeof(bcmx_l3_egress_t) * hapiBroadL3NhopCache.maxEntries;
  hapiBroadL3NhopCache.pBcmxInfo = osapiMalloc(L7_DRIVER_COMPONENT_ID, size);

  if (hapiBroadL3NhopCache.pBcmxInfo == L7_NULLPTR)
    return L7_FAILURE;
  ...
  size = sizeof(bcmx_l3_host_t) * hapiBroadL3HostCache.maxEntries;
  hapiBroadL3HostCache.pBcmxInfo = osapiMalloc(L7_DRIVER_COMPONENT_ID, size);

  if (hapiBroadL3HostCache.pBcmxInfo == L7_NULLPTR)
    return L7_FAILURE;
  ...
  size = sizeof(bcmx_l3_route_t) * hapiBroadL3RouteCache.maxEntries;
  hapiBroadL3RouteCache.pBcmxInfo = osapiMalloc(L7_DRIVER_COMPONENT_ID, size);

  if (hapiBroadL3RouteCache.pBcmxInfo == L7_NULLPTR)
    return L7_FAILURE;
                           ...
                           L7_BOOL cacheOrCommit)
{
  ...
  memcpy (&hapiBroadL3NhopCache.pBcmxInfo[cacheIdx],
          pBcmxInfo, sizeof (bcmx_l3_egress_t));
...
void hapiBroadL3NhopCacheCommit (void)
{
  ...
  pBcmxInfo = hapiBroadL3NhopCache.pBcmxInfo;
  ...
  {
    result = usl_bcmx_l3_egress_create(pFlags, pBcmxInfo,
      ...
      else
      {
        ...
        if ((pFlags[i] & BCM_L3_REPLACE) == L7_FALSE)
        {
          ...
          if (pEgrIntf[i] != HAPI_BROAD_L3_INVALID_EGR_ID)
          {
            ...
            usl_bcmx_l3_egress_destroy(&pEgrIntf[i], 1, &rv);
          }
        }
      }
    }
  ...
  {
    if (usl_bcmx_l3_egress_destroy(pEgrIntf, count, rv_array) == L7_SUCCESS)
    {
      for (i=0; i < count; i++)
      {
        rv = rv_array[i];
...
static void hapiBroadL3NhopCacheClear (void)
{
  ...
  size = sizeof(bcmx_l3_egress_t) * hapiBroadL3NhopCache.maxEntries;
  memset(hapiBroadL3NhopCache.pBcmxInfo, 0, size);
                            ...
                            L7_BOOL          cacheOrCommit)
{
  ...
  memcpy (&hapiBroadL3HostCache.pBcmxInfo[cacheIdx],
          pBcmxInfo, sizeof (bcmx_l3_host_t));
...
void hapiBroadL3HostCacheCommit (void)
{
  ...
  pBcmxInfo = hapiBroadL3HostCache.pBcmxInfo;
  ...
  {
    if (usl_bcmx_l3_host_add(pBcmxInfo, count, rv_array) != L7_SUCCESS)
    {
    ...
    for (i=0; i < count; i++)
    {
      ...
      HAPI_BROAD_L3_BCMX_DBG(rv_array[i], "usl_bcmx_l3_host_add returned (%s) "
                             ...
                             pBcmxInfo[i].l3a_intf, pBcmxInfo[i].l3a_flags);     

      if (pBcmxInfo[i].l3a_flags & BCM_L3_REPLACE)
      {
      ...
      if (rv_array[i] != BCM_E_NONE) /* Do not use L7_BCMX_OK() here */
      {
        ...
        usl_bcmx_l3_host_delete(&pBcmxInfo[i], 1, &rv);
      }
      else
      {
        if (pHost->rv != 0)
  ...
  {
    if (usl_bcmx_l3_host_delete(pBcmxInfo, count, rv_array) == L7_SUCCESS)
    {
      for (i=0; i < count; i++)
      {
        rv = rv_array[i];
        HAPI_BROAD_L3_BCMX_DBG(rv, "usl_bcmx_l3_host_delete returned (%s)"
                  "egress Id %d", bcm_errmsg(rv), pBcmxInfo[i].l3a_intf);
...
static void hapiBroadL3HostCacheClear (void)
{
  ...
  size = sizeof(bcmx_l3_host_t) * hapiBroadL3HostCache.maxEntries;
  memset(hapiBroadL3HostCache.pBcmxInfo, 0, size);
                            ...
                            L7_BOOL          cacheOrCommit)
{
  ...
  memcpy (&hapiBroadL3RouteCache.pBcmxInfo[cacheIdx], 
          pBcmxInfo, sizeof (bcmx_l3_route_t));
...
void hapiBroadL3RouteCacheCommit (void)
{
  ...
  pBcmxInfo = hapiBroadL3RouteCache.pBcmxInfo;
  ...
  {
    if (usl_bcmx_l3_route_add(pBcmxInfo, count, rv_array) != L7_SUCCESS)
    {
    ...
    for (i=0; i < count; i++)
    {
      ...
      if (rv_array[i] != BCM_E_NONE) /* Do not use L7_BCMX_OK() here */
      {
        ...
        usl_bcmx_l3_route_delete(&pBcmxInfo[i], 1, &rv);
      }
      else
      {
        if (pRoute->rv != 0)
  ...
  {
    if (usl_bcmx_l3_route_delete(pBcmxInfo, count, rv_array) == L7_SUCCESS)
    {
      for (i=0; i < count; i++)
      {
        rv = rv_array[i];
...
static void hapiBroadL3RouteCacheClear (void)
{
  ...
  size = sizeof(bcmx_l3_route_t) * hapiBroadL3RouteCache.maxEntries;
  memset(hapiBroadL3RouteCache.pBcmxInfo, 0, size);
