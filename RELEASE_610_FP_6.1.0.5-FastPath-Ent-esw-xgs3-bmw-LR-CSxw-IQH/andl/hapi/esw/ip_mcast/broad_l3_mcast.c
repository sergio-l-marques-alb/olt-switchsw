...
static void printfIPMC(bcmx_ipmc_addr_t *impc_info)
{
  int index;  
  sysapiPrintf("\n***********************************"); 
  ...
  if (impc_info->flags & BCM_IPMC_IP6) 
  {
    ...
    for(index  = 0; index < 16; index++)
        sysapiPrintf("%x ",impc_info->s_ip6_addr[index]);
    sysapiPrintf("\n");
    sysapiPrintf("\nGRP            :   "); 
    for(index  = 0; index < 16; index++)
        sysapiPrintf("%x ",impc_info->mc_ip6_addr[index]);
    sysapiPrintf("\n");
  }
  else 
  {
...
static void hapiBroadL3McastSetIpAddress(L7_inet_addr_t *srcIp, L7_inet_addr_t *grpIp, bcmx_ipmc_addr_t *ipmc)
{
  if ((srcIp->family == L7_AF_INET6) && (grpIp->family == L7_AF_INET6))
  {                                                                  
    memcpy((ipmc)->s_ip6_addr, (L7_uchar8 *)(srcIp)->addr.ipv6.in6.addr8, 16);  
    memcpy((ipmc)->mc_ip6_addr,(L7_uchar8 *)(grpIp)->addr.ipv6.in6.addr8, 16);
    (ipmc)->flags |= BCM_IPMC_IP6;                             
  }                                                                
  else                                                                 
  {                                                                  
    (ipmc)->s_ip_addr  = (srcIp)->addr.ipv4.s_addr;                 
    (ipmc)->mc_ip_addr = (grpIp)->addr.ipv4.s_addr;                    
  }
}
...
**    nonzero = log problem (emergency), display some data, invoke LOG_ERROR
*/
...
/*********************************************************************
*
* @purpose Enter L7 Multicast critical section.
*
*
* @param   none
*
...
*
*********************************************************************/
static void hapiBroadL3McastCritEnter (void)
{
  L7_RC_t rc;
  ...
  if (!hapiBroadL3McastSema)
  {
    ...
    hapiBroadL3McastSema = osapiSemaBCreate(OSAPI_SEM_Q_FIFO,OSAPI_SEM_FULL);
    if (!hapiBroadL3McastSema) 
    {
...
/*********************************************************************
*
* @purpose Exit L7 Multicast critical section.
*
*
* @param   none
*
...
*
*********************************************************************/
static void hapiBroadL3McastCritExit (void)
{
  L7_RC_t rc;
...
/*********************************************************************
*
* @purpose Return the RPF lport for a USP. 
*
* @param usp    - USP to resolve
* @param lport  - lport is returned upon success 
* @param dapi_g  
*
* @returns L7_SUCCESS - lport is returned
*          L7_FAILURE - lport is not returned 
*
* @notes  0.0.0 is assumed to mean the CPU port.
*
* @end
*
*********************************************************************/
static L7_RC_t hapiBroadRpfLportGet(DAPI_USP_t *usp, bcmx_lport_t *lport,DAPI_t *dapi_g)
{
  BROAD_PORT_t                 *rpfHapiPortPtr;
  ...
  *lport = 0;
  ...
  /* 
   * The Driver sets the usp to {0.0.0} on VLAN interface with unresolved RPF 
   * use the CPU USP/lport in this case
   */
  if (usp->unit == 0 && usp->slot == 0 && usp->port == 0)
  {
    ...
    result = dapiCpuUspGet (&tmpUsp);

    /* we need to have a usp to get the lport */
    if (result != L7_SUCCESS) return (result);

    rpfHapiPortPtr = HAPI_PORT_GET (&tmpUsp, dapi_g);
    HAPI_L3_MCAST_DEBUG("Enter: rpfHapiPortPtr assigned from cpuport %d.%d.%d\n",usp->unit,usp->slot,usp->port); 
  }
  else
  {
    if (isValidUsp(usp,dapi_g) == L7_FALSE) return L7_FAILURE;

    /* Only 0.0.0 from above, physical, and lag are allowed */
    if (!IS_SLOT_TYPE_PHYSICAL(usp,dapi_g) && !IS_SLOT_TYPE_LOGICAL_LAG(usp,dapi_g) && !IS_SLOT_TYPE_CAPWAP_TUNNEL(usp,dapi_g))
    {
      return L7_FAILURE;
    }
  
    rpfHapiPortPtr = HAPI_PORT_GET(usp, dapi_g);
    HAPI_L3_MCAST_DEBUG("Enter: rpfHapiPortPtr assigned from  %d.%d.%d\n",usp->unit,usp->slot,usp->port); 
  }

  if (!rpfHapiPortPtr) return L7_FAILURE;
 
  if (usp->slot == L7_LAG_SLOT_NUM)
  {
     *lport = rpfHapiPortPtr->hapiModeparm.lag.tgid;
  }
  else
  {
    *lport = rpfHapiPortPtr->bcmx_lport;
  }

  HAPI_L3_MCAST_DEBUG("SUCCESS: RPF lport = %x(%d) for %d.%d.%d\n",*lport,*lport,usp->unit,usp->slot,usp->port); 
                                        ...
                                        DAPI_t     *dapi_g)
{
  ...
  bcmx_lplist_t                 ipmc_l2_port_list;
  bcmx_lplist_t                 ipmc_l2_untag_list;
 ...
 dapiPortPtr = DAPI_PORT_GET(usp, dapi_g);
  ...
  {
    bcmx_lplist_init(&ipmc_l2_port_list,L7_MAX_INTERFACE_COUNT, 0);
    bcmx_lplist_init(&ipmc_l2_untag_list, L7_MAX_INTERFACE_COUNT, 0);
    ...
    */
    ...
    if (l2BitmapListCount)
    {
      if (l2BitmapList != L7_NULLPTR)
      {
        for (i = 0; i < l2BitmapListCount; i++)
        {
          ...
          {
             rv = bcmx_lplist_last_insert(&ipmc_l2_port_list, hapiPortPtr->bcmx_lport);
             ...
             {
               rv = bcmx_lplist_last_insert(&ipmc_l2_untag_list, hapiPortPtr->bcmx_lport);
             }
          }
          else if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr2))
          {
            ...
            if (lagMemberSet[0].inUse == L7_TRUE)
            {
              ...
              rv = bcmx_lplist_last_insert(&ipmc_l2_port_list, hapiLagMemberPortPtr->bcmx_lport);
              HAPI_L3_MCAST_DEBUG("Inserted port %d to the l2_port_list", hapiPortPtr->bcmx_lport);
              if (!BROAD_IS_VLAN_TAGGING(&searchUsp, *vid, dapi_g))
              {
                rv = bcmx_lplist_last_insert(&ipmc_l2_untag_list, hapiLagMemberPortPtr->bcmx_lport);
              }
            }
          }
        }
    ...
    rv = l7_bcmx_ipmc_set_l2_ports(ipmc_index, &ipmc_l2_port_list, &ipmc_l2_untag_list, *vid);
...
#if 0
    /* We have a LVL7_FIXUP inside the bcm_ipmc_add_l2_ports() to always
    ** enable replication for the VLAN. This saves us an RPC call.
    */

    /* add ipmc replication for the derived set of l2 ports */
    rv = bcmx_ipmc_repl_add(ipmc_index, ipmc_l2_port_list, *vid);
    if ((rv != BCM_E_EXISTS) && (L7_BCMX_OK(rv) != L7_TRUE))
    {
      LOG_ERROR(rv);
    }
    ...
    bcmx_lplist_free(&ipmc_l2_port_list);
    bcmx_lplist_free(&ipmc_l2_untag_list);
  }
}
                                       ...
                                       )
{
  ...
  rv = customx_ipmc_l3_port_delete (lport,
                                    &ipmc_index,
                                    1,
                                    vlan_id);
                                       ...
                                       L7_uint32  ttl)
{
  ...
  rv = customx_ipmc_l3_port_add (lport,
                                 &ipmc_index,
                                 1,
                                 vlan_id,
                                 ipmc_tagged,
...
static int hapiBroadMcastHardwareAddEntry(BroadGroupEntryType *entry,DAPI_t *dapi_g)
{
  ...
  bcmx_ipmc_addr_t ipmc_info;                /* BCMX IP Multicast information entry workspace */
  L7_RC_t          result;                   /* L7 Result code workspace */
  ...
  bcmx_ipmc_addr_t_init(&ipmc_info);
  ipmc_info.vid = entry->vlanId;
  /* Copy the addresses into the buffer */
  hapiBroadL3McastSetIpAddress(&(entry->srcAddr),&(entry->groupIp),&ipmc_info);
  /* Check for entry existing in hardware */
  if (entry->flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW)
  {
    /* We think it's in the hardware; ask hardware about the entry */
    if ((result = hapiBroadRpfLportGet(&(entry->rpf_physical_usp), &(ipmc_info.port_tgid),dapi_g)) == L7_SUCCESS)
    {     
      if (IS_SLOT_TYPE_LOGICAL_LAG(&(entry->rpf_physical_usp),dapi_g)) 
        ipmc_info.ts = 1;
      rv = bcmx_ipmc_find(&ipmc_info);
    }

    if (!L7_BCMX_OK(rv) || result != L7_SUCCESS)
    {
      /* Something went wrong accessing it in the hardware; complain */
      ...
      L7_LOGF (L7_LOG_SEVERITY_CRITICAL, L7_DRIVER_COMPONENT_ID,
               "bcmx_ipmc_find failed: group = %s, source = %s, vlan = %d, rv = %d\n",
               grpStr, srcStr, entry->vlanId, rv);
      HAPI_L3_MCAST_DEBUG("FAIL: rpf return=%d find return=%d\n",result,rv);
    }
  }
  ...
  if (L7_TRUE == IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    ...
    hapiPortPtr = HAPI_PORT_GET(usp,dapi_g);
    ipmc_info.port_tgid = hapiPortPtr->bcmx_lport;
  }
  ...
  */
  ...
  /* Enable index flag only if replace flag is not on, as both cannot be on at once */
  if (!(ipmc_info.flags & BCM_IPMC_REPLACE))
  { 
    HAPI_L3_MCAST_DEBUG("Enabling BCM_IPMC_USE_IPMC_INDEX for non strata devices");        
    ipmc_info.flags |= BCM_IPMC_USE_IPMC_INDEX;
  }
  /* Get the index */
  ipmc_info.ipmc_index = entry->ipmc_index;
  ...
  if ((rv != BCM_E_EXISTS) && (L7_BCMX_OK(rv) != L7_TRUE))
  {
    ...
    hapiBroadMcastFillAddressString((L7_uchar8*)(&(srcStr)),sizeof(srcStr),&(entry->srcAddr));
    L7_LOGF (L7_LOG_SEVERITY_CRITICAL, L7_DRIVER_COMPONENT_ID,
             "bcmx_ipmc_add %s: group = %s, source = %s, vlan = %d, port = %d, rv = %d\n",
      ...
      */
      ...
      if (BCM_E_EXISTS == rv)
      {
        ...
        L7_LOGF (L7_LOG_SEVERITY_ALERT, L7_DRIVER_COMPONENT_ID,
                 "bcmx_ipmc_add existing: group = %s, source = %s, vlan = %d, rv = %d\n",
                 grpStr, srcStr, entry->vlanId, rv);
      }
      ...
      if (entry->flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW_FAIL)
      {
        ...
        L7_LOGF (L7_LOG_SEVERITY_NOTICE, L7_DRIVER_COMPONENT_ID,
                 "bcmx_ipmc_add success (previously failed): group = %s, source = %s, vlan = %d, rv = %d\n",
                 grpStr, srcStr, entry->vlanId, rv);
      }
  ...
  /* free the ipmc_info that was allocated in bcmx_ipmc_addr_init() */
  bcmx_ipmc_addr_free (&ipmc_info);
  return rv;
}
...
static void hapiBroadMcastHardwareSetIfs(BroadGroupEntryType *entry,DAPI_t *dapi_g)
{
  ...
  if (entry->flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW)
  {
    ...
    {
      hapiBroadL3McastAddL2Ports (&(entry->rpf_usp),&(entry->groupIp),&(entry->srcAddr),&(entry->vlanId),entry->ipmc_index,
L7_NULLPTR, L7_NULL, dapi_g);
...
void hapiBroadRoutingMulticastMacFromIpAddr(L7_inet_addr_t *ipAddr, L7_uchar8 *macAddr)
{
  ...
  if (ipAddr->family == L7_AF_INET)
  {
    ...
    {
      LOG_MSG("hapiBroadRoutingMulticastMacFromIpAddr: Invalid IPv4Address");
  ...
  else if (ipAddr->family == L7_AF_INET6)
  {
    ...
    {
      LOG_MSG("hapiBroadRoutingMulticastMacFromIpAddr: Invalid IPv6Address");
      memset(macAddr, 0x00, L7_MAC_ADDR_LEN);
      return;
    }
...
/************** RPF interface modify code ***********************/
        if (dapiCmd->cmdData.mcastModify.outIntfDirection == DAPI_SNOOP_L3_NOTIFY_INGRESS)
        {
          ...
          {
           HAPI_L3_MCAST_DEBUG("\nTable Index %u L2 Bitmap of entries with rpf_us %u/%u/%u modified",
                               tableIndex, 
  ...
  if (tableIndex >= L7_L3_MCAST_ROUTE_TBL_SIZE_TOTAL)
  {
    ...
    */
    ...
    L7_LOGF (L7_LOG_SEVERITY_EMERGENCY, L7_DRIVER_COMPONENT_ID,
             "L7_L3_MCAST_ROUTE_TBL_SIZE_TOTAL (%d) reached, tryting to add more",
  ...
  if (set_group_address == L7_TRUE)
  {
    ...
    if (override_existing_entry == L7_TRUE)
    {
      ...
      if ((L7_BCMX_OK(rv) != L7_TRUE))
      {
        ...
        L7_LOGF (L7_LOG_SEVERITY_CRITICAL, L7_DRIVER_COMPONENT_ID,
                 "bcmx_ipmc_remove failed: group = %08X, source = %08X, vlan = %5d, rv = %d\n",
...
L7_RC_t hapiBroadRoutingIntfMcastAsyncDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  */
  ...
  /* Initialise bcmx_ipmc_addr_t */
  bcmx_ipmc_addr_t_init(&ipmc);
  ipmc.s_ip_addr = srcAddr.addr.ipv4.s_addr;
  ipmc.mc_ip_addr = groupIp.addr.ipv4.s_addr;
  ipmc.vid = vlan_id;
  hapiBroadL3McastSetIpAddress(&srcAddr, &groupIp, &ipmc);

  if (hapiBroadRpfLportGet(&(BroadGroupList[table_index].rpf_physical_usp), &(ipmc.port_tgid),dapi_g) != L7_SUCCESS)
  {
    hapiBroadL3McastCritExit (); 
    return L7_FAILURE;
  }
  
  if (IS_SLOT_TYPE_LOGICAL_LAG(&(BroadGroupList[table_index].rpf_physical_usp),dapi_g)) 
    ipmc.ts = 1;

  /* [Looking for the entry in the hardware table before deleting it seems redundant.] */
  /* Find the entry in the hardware table... */
  rv = bcmx_ipmc_find(&ipmc);
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
    if (BroadGroupList[table_index].flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW)
    {
      /* the entry *should* have been in the hardware table, but was not */
      /* since we're deleting it, not finding it is hardly fatal, but worth a warning */
      L7_LOGF (L7_LOG_SEVERITY_WARNING, L7_DRIVER_COMPONENT_ID,
               "bcmx_ipmc_find failed: group = %x, source = %x, vlan = %d, rv = %d\n",
               groupIp, srcAddr, vlan_id, rv);
      /* If we don't find it for search, not being able to delete later isn't so bad... */
      BroadGroupList[table_index].flags &= ~BROAD_GROUP_ENTRY_FLAGS_IN_HW;
      /* ...but better update the accounting */
      groupEntriesInHw--;
      groupWasDeleted = L7_TRUE;
    }
  }
  ...
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
    if (BroadGroupList[table_index].flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW)
    {
      ...
      L7_LOGF (L7_LOG_SEVERITY_ALERT, L7_DRIVER_COMPONENT_ID,
               "bcmx_ipmc_remove failed: group = %x, source = %x, vlan = %d, rv = %d\n",
               groupIp, srcAddr, vlan_id, rv);
    }
  }
  if (BroadGroupList[table_index].flags & BROAD_GROUP_ENTRY_FLAGS_IN_HW)
                                                 ...
                                                 DAPI_t *dapi_g)
{
  ...
  bcmx_ipmc_addr_t             ipmc; 
#if 0
  HAPI_L3_MCAST_DEBUG("Addr to be checked for in use");
  HAPI_L3_ADDR_PRINT_DEBUG(dapiCmd->cmdData.mcastUseQuery.srcIpAddr, "SRC");
  HAPI_L3_ADDR_PRINT_DEBUG(dapiCmd->cmdData.mcastUseQuery.mcastGroupAddr, "GRP");
#endif
  hapiBroadL3McastCritEnter ();
  ...
  ipmc.vid = vid;
  hapiBroadL3McastSetIpAddress(&srcAddr, &groupIp, &ipmc);
  if ((result = hapiBroadRpfLportGet(&(BroadGroupList[table_index].rpf_physical_usp),&(ipmc.port_tgid),dapi_g)) == L7_SUCCESS)
  {
    if (IS_SLOT_TYPE_LOGICAL_LAG(&(BroadGroupList[table_index].rpf_physical_usp),dapi_g)) 
      ipmc.ts = 1;

    dapiCmd->cmdData.mcastUseQuery.entryUsed  = usl_ipmc_inuse_get(&ipmc);
  }
  bcmx_ipmc_addr_free(&ipmc);
  hapiBroadL3McastCritExit ();
  return result;
...
L7_RC_t hapiBroadRoutingIntfMcastTtlSet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  */
  ...
  if (hapiPortPtr->l3_interface_id != HAPI_BROAD_INVALID_L3_INTF_ID)
  {
    ...
    if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr) == L7_TRUE)
    {
      memcpy(intfInfo.l3a_mac_addr, dapiPortPtr->modeparm.physical.macAddr, sizeof(bcm_mac_t));
      intfInfo.l3a_vid = hapiPortPtr->port_based_routing_vlanid;
    }
    else
    {
      memcpy(intfInfo.l3a_mac_addr, dapiPortPtr->modeparm.router.macAddr, sizeof(bcm_mac_t));
      intfInfo.l3a_vid = dapiPortPtr->modeparm.router.vlanID;
    }
    intfInfo.l3a_flags |= (BCM_L3_ADD_TO_ARL  | BCM_L3_REPLACE | BCM_L3_WITH_ID);
    intfInfo.l3a_ttl =  hapiPortPtr->multicast_ttl_limit;
    intfInfo.l3a_intf_id = hapiPortPtr->l3_interface_id; 
  ...
  if (dapiCmd->cmdData.mcastforwardConfig.family == L7_AF_INET)  
  {
    if (dapiCmd->cmdData.mcastIgmpConfig.enable == L7_TRUE)
    {
      ...
      if (!dapi_g->system->igmpSnoopingEnable)
      {
        ...
        switchFrame = L7_TRUE;
        hapiBroadConfigIgmpFilter(L7_TRUE, 0, HAPI_BROAD_INGRESS_MED_PRIORITY_COS, switchFrame, usp, dapi_g);
      }
      else 
      { 
         ...
         */
        hapiBroadConfigIgmpFilter(L7_FALSE, 0, HAPI_BROAD_INGRESS_MED_PRIORITY_COS, L7_FALSE, usp, dapi_g);
        hapiBroadConfigL3McastFilter(L7_TRUE);
...
L7_RC_t hapiBroadConfigL3McastFilter(L7_BOOL enableFilter)
{
  ...
  if ((enableFilter == L7_TRUE) && (mrpFilterId == BROAD_POLICY_INVALID))
  {
    do
    {
      if ((result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_SYSTEM, &mrpFilterId)) != L7_SUCCESS)
        break;

      /* Give DVMRP frames high proority. */
      if ((result = hapiBroadPolicyRuleAdd(mrpFilterId, &ruleId)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyRuleQualifierAdd(mrpFilterId, ruleId, BROAD_FIELD_ETHTYPE,(L7_uchar8 *)&ip_ethtype, exact_match)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyRuleQualifierAdd(mrpFilterId, ruleId, BROAD_FIELD_PROTO,igmp_proto, exact_match)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyRuleQualifierAdd(mrpFilterId, ruleId, BROAD_FIELD_SPORT,(L7_uchar8 *)&dvmrp_type, (L7_uchar8 *)&dvmrp_type_match)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyRuleActionAdd(mrpFilterId, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_HIGH_PRIORITY_COS, 0, 0)) != L7_SUCCESS)
        break;
      ...
      if ((result = hapiBroadPolicyRuleQualifierAdd(mrpFilterId, ruleId, BROAD_FIELD_PROTO,pim_proto, exact_match)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyRuleActionAdd(mrpFilterId, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_HIGH_PRIORITY_COS, 0, 0)) != L7_SUCCESS)
        break;

      if ((result = hapiBroadPolicyCommit(mrpFilterId)) != L7_SUCCESS)
        break;
...
L7_RC_t hapiBroadConfigL3V6McastFilter(L7_BOOL enableFilter)
{
  ...
  if ((enableFilter == L7_TRUE) && (Ip6MrpId == BROAD_POLICY_INVALID))
  {
    do
    {
      ...
      if (( result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_SYSTEM, &Ip6MrpId)) != L7_SUCCESS) 
        break;
      if (( result = hapiBroadPolicyRuleAdd(Ip6MrpId, &ruleId)) != L7_SUCCESS)
        break;
      if (( result = hapiBroadPolicyRuleQualifierAdd(Ip6MrpId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ip6_ethtype, exact_match)) != L7_SUCCESS) 
        break;
      if (( result = hapiBroadPolicyRuleQualifierAdd(Ip6MrpId, ruleId, BROAD_FIELD_IP6_NEXTHEADER,   pim6_proto, exact_match)) != L7_SUCCESS) 
        break;
      if (( result = hapiBroadPolicyRuleActionAdd(Ip6MrpId, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_HIGH_PRIORITY_COS, 0,0)) != L7_SUCCESS) 
        break;
    ...
    */
    ...
    { 
      ...
      { 
        ...
        rv = customx_ipmc_l2_port_add (lport[lport_idx],
                                       ipmc_group_index,
                                       num_ipmc_groups,
                                       vlan_id,
                                       tagged);
    ...
    } else
    {
      if (num_ipmc_groups > 0)
      {
        ...
        rv = customx_ipmc_l2_port_delete (lport[lport_idx],
                                          ipmc_group_index,
                                          num_ipmc_groups,
                                          vlan_id);
        if (L7_BCMX_OK(rv) != L7_TRUE)
  ...
  for (lport_idx = 0; lport_idx < lport_count; lport_idx++)
  {
    ...
    */
    if (num_ipmc_groups > 0)
    {
      if (add_remove)
      {
        rv = customx_ipmc_l3_port_add (lport[lport_idx],
                                       ipmc_group_index,
                                       num_ipmc_groups,
                                       vlan_id,
                                       tagged,
      ...
      {
        rv = customx_ipmc_l3_port_delete (lport[lport_idx],
                                          ipmc_group_index,
                                          num_ipmc_groups,
                                          vlan_id);
        if (L7_BCMX_OK(rv) != L7_TRUE)
...
/*********************************************************************
* @purpose  Check to see how many vlans are replicated on the specified group and port
*
* @param    @b{(input)} ipmc_index
* @param    @b{(input)} bcmx_lport
* @param    @b{(output)} *num_replicated_vlans
*
* @returns  L7_SUCCESS
*           L7_FAILURE
*
...
* @end
*********************************************************************/
L7_RC_t hapiBroadL3McastReplCheck(int ipmc_index, bcmx_lport_t bcmx_lport, L7_uint32 *num_replicated_vlans)
{
  L7_int32                      rv;
  L7_uint32                     i;
  bcm_vlan_vector_t             ipmc_vlan_vec;

  *num_replicated_vlans = 0;
  rv = bcmx_ipmc_repl_get(ipmc_index, bcmx_lport, ipmc_vlan_vec);
  if (rv != BCM_E_UNAVAIL)
  {
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
    ...
    */
    ...
    if (rpf_frame.usp.slot == L7_LAG_SLOT_NUM)
    {
      ...
      else
      {
        ...
        ts = TRUE;
        port_tgid = hapiPortPtr->hapiModeparm.lag.tgid;
      }
    }
    else
    {
      ...
      ts = FALSE;
      port_tgid = hapiPortPtr->bcmx_lport;
    }
    ...
    HAPI_L3_MCAST_DEBUG("Calling the usl_bcmx_ipmc_port_tgid_set ts=%d port_tgid=%x",ts,port_tgid);
    rv = usl_bcmx_ipmc_port_tgid_set(&ipmc, port_tgid);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      LOG_ERROR(rv);
    }
