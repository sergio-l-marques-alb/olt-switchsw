                                     ...
                                     DAPI_t      *dapi_g)
{
  ...
  l7_usl_port_cache_vlan_entry_set(usp->unit, usp->slot, hapiPortPtr->bcm_unit, hapiPortPtr->bcm_port,
                                   vlanId, is_tagged, L7_TRUE);
                                      ...
                                      DAPI_t     *dapi_g)
{
  ...
  L7_BOOL       tag;
  BROAD_PORT_t *hapiPortPtr;
  HAPI_BROAD_CUSTOM_BCMX_VLAN_CMD_t port_command;
  int rv;

  hapiPortPtr = HAPI_PORT_GET(usp, dapi_g);
  memset (&port_command, 0, sizeof (HAPI_BROAD_CUSTOM_BCMX_VLAN_CMD_t));
  ...
  */
  for (vlan_id = 1; vlan_id <= L7_PLATFORM_MAX_VLAN_ID; vlan_id++)
  {
    ...
    {
      BROAD_BCMX_CUSTOM_VLAN_MEMBER_SET (vlan_id, port_command.vlan_membership_mask);
      if (BROAD_IS_VLAN_TAGGING (usp, vlan_id, dapi_g))
      {
        BROAD_BCMX_CUSTOM_VLAN_MEMBER_SET (vlan_id, port_command.vlan_tag_mask);
      }
    }
  }
  ...
  */
  if (port_cmd == 1)
  {
    rv = l7_bcmx_port_vlan_member_add (hapiPortPtr->bcmx_lport, &port_command);
  ...
  */
  if (port_cmd == 0)
  {
    rv = l7_bcmx_port_vlan_member_delete (hapiPortPtr->bcmx_lport, &port_command);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      LOG_ERROR (rv);
    }
...
L7_RC_t hapiBroadPortAcceptableFrameTypeSet(DAPI_USP_t *usp, L7_uint32 acceptFrameType, DAPI_t *dapi_g)
{
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
  {
    ...
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        rv = l7_bcmx_port_discard_set(hapiLagMemberPortPtr->bcmx_lport, mode);
        if (L7_BCMX_OK(rv) != L7_TRUE)
        {
          LOG_ERROR(rv);
        }
  ...
  else
  {
    ...
    {
      rv = l7_bcmx_port_discard_set(hapiPortPtr->bcmx_lport, mode);
      if (L7_BCMX_OK(rv) != L7_TRUE)
      {
        LOG_ERROR(rv);
      }
...
L7_RC_t hapiBroadPortUntaggedVlanSet(DAPI_USP_t *usp, L7_ushort16 pvid, DAPI_t *dapi_g)
{
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
  {
    ...
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        rv = l7_bcmx_port_untagged_vlan_set(hapiLagMemberPortPtr->bcmx_lport,pvid);
        if (L7_BCMX_OK(rv) != L7_TRUE)
        {
          LOG_ERROR(pvid);
        }
  ...
  else
  {
        ...
        (hapiBroadSystemMirroringUSPIsProbe(usp,dapi_g) == L7_FALSE)) 
    {
      ...
      rv = l7_bcmx_port_untagged_vlan_set(hapiPortPtr->bcmx_lport, pvid);
      if (L7_BCMX_OK(rv) != L7_TRUE)
      {
        LOG_ERROR(pvid);
      }
...
L7_RC_t hapiBroadQvlanVlanCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  bcmx_lplist_t            ut_list, member_list;
  ...
  // PTin: Here the CPU is added to the vlan
  /* Add the cpu port to the vlan */
  if (CPU_USP_GET(&cpuUsp) == L7_FAILURE)
  {
    LOG_ERROR (vlanID);
  }
    
  rv = bcmx_lplist_init(&member_list, L7_MAX_INTERFACE_COUNT, 0);
  if (L7_BCMX_OK(rv) != TRUE)
  {
    LOG_ERROR (vlanID);
  }
    
  rv = bcmx_lplist_init(&ut_list, L7_MAX_INTERFACE_COUNT, 0);
  if (L7_BCMX_OK(rv) != TRUE)
  {
    LOG_ERROR (vlanID);
  }
  ...
  // PTin added: CPU inclusion conditioned (To Vlan 1 has to be included, in order to have dynamic LACP working)
  if (ptin_vlan_defs.include_cpu_intf || vlanID==1)
  {
    for (cpuUsp.port=0; cpuUsp.port < dapi_g->unit[cpuUsp.unit]->slot[cpuUsp.slot]->numOfPortsInSlot; cpuUsp.port++)
    {
      cpuHapiPortPtr = HAPI_PORT_GET(&cpuUsp, dapi_g);
      
      rv = BCMX_LPLIST_ADD(&member_list,cpuHapiPortPtr->bcmx_lport);
      if (L7_BCMX_OK(rv) != TRUE)
      {
        LOG_ERROR (vlanID);
      }
  ...
  rv = bcmx_vlan_port_add(vlanID, member_list, ut_list);
  if (L7_BCMX_OK(rv) != TRUE)
  {
    LOG_ERROR (vlanID);
  }
 
  bcmx_lplist_free(&member_list);
  bcmx_lplist_free(&ut_list);
...
#ifdef L7_IPV6_PACKAGE
  /* On T2/Apollo platforms, the tunnel hit qualifier is not supported.
     Because of which, we do not get tunneled OSPFv3 packets to CPU.
     As a workaround use BCM_VLAN_UNKNOWN_IP6_MCAST_TOCPU. This ensures
     that OSPFv3 packets are received fully-encapsulated at the CPU. */

  board_info = hpcBoardGet();
  if ((board_info != L7_NULL) &&
      ((board_info->npd_id == __BROADCOM_56634_ID) ||
       (board_info->npd_id == __BROADCOM_56524_ID) ||
       (board_info->npd_id == __BROADCOM_56636_ID)))
  {
    if (bcmx_vlan_control_vlan_get(vlanID, &vlanControl) == BCM_E_NONE)
    {
      vlanControl.flags |= BCM_VLAN_UNKNOWN_IP6_MCAST_TOCPU;

      rv = bcmx_vlan_control_vlan_set(vlanID, vlanControl);
      if (L7_BCMX_OK(rv) != L7_TRUE)
      {
        LOG_ERROR(rv);
      }
...
L7_RC_t hapiBroadQvlanVlanListCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  HAPI_BROAD_CUSTOM_BCMX_VLAN_CMD_t hapiL2VlanCpuCommand;
  ...
  memset(&hapiL2VlanCpuCommand, 0, sizeof(hapiL2VlanCpuCommand));
          ...
          )
	  {
	    vlanID = (vidx*8) + pos;
            ...
            */
            ...
            L7_VLAN_SETMASKBIT(hapiL2VlanCpuCommand.vlan_membership_mask, vlanID);
  ...
  {
    cpuHapiPortPtr = HAPI_PORT_GET(&cpuUsp, dapi_g);
    rv = customx_port_vlan_member_add(cpuHapiPortPtr->bcmx_lport, &hapiL2VlanCpuCommand);
    if (rv != BCM_E_NONE) 
    {
      SYSAPI_PRINTF(SYSAPI_LOGGING_HAPI_ERROR,
                    "Failed to set cpu vlan membership in %s:%d rv %d\n", 
                     __FUNCTION__, __LINE__, rv
                   );
      L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_DRIVER_COMPONENT_ID,
              "Failed to set cpu vlan membership rv %d\n", 
              rv
             );
    }
  }
...
#if 0
/* ALT - HACK */
      printf("Remove Port - VLAN = %d, USP = %d, %d, %d\n",
             vlanId,
             memberUsp.unit,
             memberUsp.slot,
             memberUsp.port);
...
L7_RC_t hapiBroadQvlanVlanPurge(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    /* Remove CPU ports from the VLAN.
    */
    bcmx_lplist_t          member_list;
    DAPI_USP_t             cpuUsp;
    BROAD_PORT_t          *cpuHapiPortPtr;
    ...
    rv = bcmx_vlan_port_remove(vid, member_list);
    if ((L7_BCMX_OK(rv) != TRUE) && (rv != BCM_E_NOT_FOUND))
    {
      LOG_ERROR (vid);
    }
    bcmx_lplist_free(&member_list);
...
L7_RC_t hapiBroadQvlanVlanListPurge(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    cpuHapiPortPtr = HAPI_PORT_GET(&cpuUsp, dapi_g);
    rv = customx_port_vlan_member_delete(cpuHapiPortPtr->bcmx_lport, &hapiL2VlanCpuCommand);
    if (rv != BCM_E_NONE) 
    {
      SYSAPI_PRINTF(SYSAPI_LOGGING_HAPI_ERROR,
                    "Failed to set cpu vlan membership in %s:%d rv %d\n", 
             ...
             if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
             {
               ...
               for (i = 0;i < L7_MAX_MEMBERS_PER_LAG;i++)
               {
                 if (lagMemberSet[i].inUse == L7_TRUE)
                 {
                   ...
                   l7_usl_port_cache_vlan_entry_set(lagMemberSet[i].usp.unit, lagMemberSet[i].usp.slot, hapiLagMemberPortPtr->bcm_unit, hapiLagMemberPortPtr->bcm_port, vlanId, tagged, L7_TRUE);
                 }
               }
             }
             else
             {
               l7_usl_port_cache_vlan_entry_set(usp->unit, usp->slot, hapiPortPtr->bcm_unit, hapiPortPtr->bcm_port, vlanId, tagged, L7_TRUE);
             }
          ...
          */
          if (remFromHw == L7_TRUE) 
          {
            ...
            */
            ...
            if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
            {
              ...
              for (i = 0;i < L7_MAX_MEMBERS_PER_LAG;i++)
              {
                if (lagMemberSet[i].inUse == L7_TRUE)
                {
                   ...
                   BROAD_HW_VLAN_TAGGING_CLEAR (&lagMemberSet[i].usp, vlanId, dapi_g);
                   hapiLagMemberPortPtr = HAPI_PORT_GET(&lagMemberSet[i].usp, dapi_g);
                   l7_usl_port_cache_vlan_entry_set(lagMemberSet[i].usp.unit, lagMemberSet[i].usp.slot,
                                                    hapiLagMemberPortPtr->bcm_unit ,hapiLagMemberPortPtr->bcm_port,
                                                 vlanId, L7_FALSE, L7_FALSE);
                }
              }
            }
            ...
            {
              l7_usl_port_cache_vlan_entry_set(usp->unit, usp->slot, hapiPortPtr->bcm_unit, hapiPortPtr->bcm_port, vlanId, L7_FALSE, L7_FALSE);
            }
...
L7_RC_t hapiBroadQvlanPortConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  BROAD_PORT_t          *hapiLagMemberPortPtr;
  L7_uint32              i;
  int                    ethertype[DAPI_QVLAN_NUM_OF_PROTOCOL_ENTRIES] = {0x0800, 0x0806, 0x8137};
    ...
    for (i = 0; i < DAPI_QVLAN_NUM_OF_PROTOCOL_ENTRIES; i++)
    {
      if (cmdVlanPortCfg->cmdData.portConfig.protocol[i] != hapiPortPtr->pbvlan_table[i])
      {
             ...
             (hapiPortPtr->hapiModeparm.physical.isMemberOfLag == L7_FALSE)))
        {
          ...
          {
            hapiBroadProtocolVlanRemove(usp, ethertype[i], dapi_g);
          }

          /* if a VLAN is now configured, add it */
          if (cmdVlanPortCfg->cmdData.portConfig.protocol[i] != 0)
          {
            hapiBroadProtocolVlanAdd(usp, ethertype[i], cmdVlanPortCfg->cmdData.portConfig.protocol[i], dapi_g);
          }
        }
      }
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
  {
    ...
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        /* if the user is trying to assign the default priority to queue 7 on a
           stacking build, we need to re-assign this to queue 6 because queue 7
           is a strict priority queue reserved for stacking traffic */
        if(priority == 7)
          priority = 6;
        rv = l7_bcmx_port_untagged_priority_set(hapiPortPtr->bcmx_lport, priority);
#else
        rv = l7_bcmx_port_untagged_priority_set(hapiPortPtr->bcmx_lport, priority);
#endif
        if (L7_BCMX_OK(rv) != TRUE)
...
L7_RC_t hapiBroadCosCommitDot1pParams(BROAD_PORT_t *hapiPortPtr, L7_uchar8 *dot1pMap)
{
  ...
  if (complete)
  {
    ...
    if (hapiBroadPolicyUtilLookup(dot1pMap, L7_DOT1P_MAX_PRIORITY+1, &cosqId) == L7_TRUE)
    {
    ...
    if (L7_FALSE == policyFound)   /* create new policy */
    {
      ...
      hapiBroadPolicyCreate(BROAD_POLICY_TYPE_COSQ, &cosqId);

      for (i = 0; i < L7_DOT1P_MAX_PRIORITY+1; i++)
      {
        L7_uchar8 dot1p = i;
        hapiBroadPolicyPriorityRuleAdd(cosqId, &ruleId, BROAD_POLICY_RULE_PRIORITY_LOWEST);
        hapiBroadPolicyRuleQualifierAdd(cosqId, ruleId, /*BROAD_FIELD_OCOS*/ BROAD_FIELD_ICOS, &dot1p, exactMask); /* PTin modified */
        hapiBroadPolicyRuleActionAdd(cosqId, ruleId, BROAD_ACTION_SET_COSQ, dot1pMap[i], 0, 0);
      }

      result = hapiBroadPolicyCommit(cosqId);

      //printf("%s(%d) hapiBroadPolicyCommit: result is %d\r\n",__FUNCTION__,__LINE__,result);

      if (L7_SUCCESS == result)
        (void)hapiBroadPolicyUtilAdd(dot1pMap, L7_DOT1P_MAX_PRIORITY+1, cosqId);

    }

    if (L7_SUCCESS == result)
    {  
      result = hapiBroadPolicyUtilApply(cosqId, hapiPortPtr->bcmx_lport);
    }
    ...
    {
      hapiBroadPolicyUtilDelete(cosqId);
    }
...
L7_RC_t hapiBroadQvlanMcastFloodModeSet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  //printf("%s(%d) vlan=%u mode=%u\r\n",__FUNCTION__,__LINE__,vlanId,mode);
  ...
  if (bcmx_vlan_mcast_flood_set(vlanId, bcmxMode) != BCM_E_NONE)
  {
    result = L7_FAILURE;
  }
...
L7_RC_t hapiBroadProtocolVlanAdd(DAPI_USP_t *usp, L7_ushort16 etherType, L7_ushort16 vlanId, DAPI_t *dapi_g)
{
  ...
  if (usp->slot == L7_LAG_SLOT_NUM)
  {
    ...
    for (lagMemberIdx = 0; lagMemberIdx < L7_MAX_MEMBERS_PER_LAG; lagMemberIdx++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[lagMemberIdx].inUse == L7_TRUE)
      {
        ...
        rv = bcmx_port_protocol_vlan_add(hapiLagMemberPortPtr->bcmx_lport,
                                         BCM_PORT_FRAMETYPE_ETHER2, 
                                         etherType, 
                                         vlanId);
  ...
  else
  {
    ...
    rv = bcmx_port_protocol_vlan_add(hapiPortPtr->bcmx_lport,
                                     BCM_PORT_FRAMETYPE_ETHER2, 
                                     etherType, 
                                     vlanId);
    if ((L7_BCMX_OK(rv) != L7_TRUE) && (rv != BCM_E_EXISTS))
    {
      LOG_ERROR(hapiPortPtr->bcmx_lport);
    }
  }
...
L7_RC_t hapiBroadProtocolVlanRemove(DAPI_USP_t *usp, L7_ushort16 etherType, DAPI_t *dapi_g)
{
  ...
  if (usp->slot == L7_LAG_SLOT_NUM)
  {
    ...
    for (lagMemberIdx = 0; lagMemberIdx < L7_MAX_MEMBERS_PER_LAG; lagMemberIdx++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[lagMemberIdx].inUse == L7_TRUE)
      {
        ...
        rv = bcmx_port_protocol_vlan_delete(hapiLagMemberPortPtr->bcmx_lport,
                                            BCM_PORT_FRAMETYPE_ETHER2, 
                                            etherType);
        if (L7_BCMX_OK(rv) != L7_TRUE)
        {
  ...
  else
  {
    ...
    rv = bcmx_port_protocol_vlan_delete(hapiPortPtr->bcmx_lport,
                                        BCM_PORT_FRAMETYPE_ETHER2, 
                                        etherType);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
...
{
  uint32  flags = (BCM_PORT_VLAN_MEMBER_EGRESS); /* always egress filter */
  int rc = BCM_E_NONE;
  ...
  rc = l7_bcmx_port_vlan_member_set(lport,flags);               
...
L7_RC_t hapiBroadVlanEgressFilterSet(bcmx_lport_t lport, L7_BOOL val, DAPI_t *dapi_g)
{
  ...
  {
    if (hapiBroadTgidToUspConvert(BCM_GPORT_TRUNK_GET(lport), &lagUsp, dapi_g) == L7_SUCCESS)
    {
      ...
      for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
      {
        if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
        {
          ...
          rc = l7_bcmx_port_vlan_member_get(hapiLagMemberPortPtr->bcmx_lport, &flags); 
          if (rc == BCM_E_NONE)
          {
            if (val)
            {
            ...
            rc = l7_bcmx_port_vlan_member_set(hapiLagMemberPortPtr->bcmx_lport,flags);               
          }
        }
      }
      hapiBroadLagCritSecExit();
  ...
  {
    rc = l7_bcmx_port_vlan_member_get(lport, &flags); 
    if (!L7_BCMX_OK(rc))
    {
      return L7_FAILURE;
    }
