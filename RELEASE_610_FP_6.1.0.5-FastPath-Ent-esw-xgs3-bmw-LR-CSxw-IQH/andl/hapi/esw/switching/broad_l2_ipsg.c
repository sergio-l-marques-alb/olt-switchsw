...
L7_RC_t hapiBroadIpsgDefaultPolicyInstall(DAPI_t *dapi_g)
{
  ...
  if (hapiBroadIpsgSupported())
  {
    ...
    hapiBroadPolicyCreate(BROAD_POLICY_TYPE_SYSTEM, &hapiSystem->ipsgPolicyId);
    priority = BROAD_POLICY_RULE_PRIORITY_DEFAULT; /* These rules have default priority
                                                      compared to other system rules. */
#else
    hapiBroadPolicyCreate(BROAD_POLICY_TYPE_IPSG, &hapiSystem->ipsgPolicyId);
    hapiBroadPolicyStageSet(hapiSystem->ipsgPolicyId, BROAD_POLICY_STAGE_LOOKUP);
    ...
    hapiBroadPolicyPriorityRuleAdd(hapiSystem->ipsgPolicyId, &ruleId, priority);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8*)&ip_ethtype, exact_match);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_PROTO, udp_proto, exact_match);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_DPORT, (L7_uchar8*)&dhcpc_dport, exact_match);

    hapiBroadPolicyPriorityRuleAdd(hapiSystem->ipsgPolicyId, &ruleId, priority);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8*)&ip_ethtype, exact_match);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_PROTO, udp_proto, exact_match);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_DPORT, (L7_uchar8*)&dhcps_dport, exact_match);
#endif

    /* Drop all other IP packets */
    hapiBroadPolicyPriorityRuleAdd(hapiSystem->ipsgPolicyId, &ruleId, priority);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8*)&ip_ethtype, exact_match);
    hapiBroadPolicyRuleActionAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);

#if HAPI_BROAD_IPSG_IPV6_SUPPORTED
    /* When we support IPv6 IPSG, we'll need to include a default rule to drop IPv6 traffic. */
    hapiBroadPolicyPriorityRuleAdd(hapiSystem->ipsgPolicyId, &ruleId, priority);
    hapiBroadPolicyRuleQualifierAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ipV6_ethtype, exact_match);
    hapiBroadPolicyRuleActionAdd(hapiSystem->ipsgPolicyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
#endif
...
L7_RC_t hapiBroadIntfIpsgClientAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (pIpsgEntry == L7_NULL)
  {
    ...
    if (hapiBroadPolicyCreate(BROAD_POLICY_TYPE_IPSG, &pIpsgEntry->policyId) == L7_SUCCESS)
    {
#if (L7_FEAT_IPSG_ON_IFP == 0)
      hapiBroadPolicyStageSet(pIpsgEntry->policyId, BROAD_POLICY_STAGE_LOOKUP);
#endif
      hapiBroadPolicyRuleAdd(pIpsgEntry->policyId, &ruleId);
      if (dapiCmd->cmdData.ipsgClientAdd.ip4Addr)
      {
        hapiBroadPolicyRuleQualifierAdd(pIpsgEntry->policyId, ruleId, BROAD_FIELD_SIP, (L7_uchar8 *)&dapiCmd->cmdData.ipsgClientAdd.ip4Addr, exact_match);
      }
#if HAPI_BROAD_IPSG_IPV6_SUPPORTED
      else
      {
        hapiBroadPolicyRuleQualifierAdd(pIpsgEntry->policyId, ruleId, BROAD_FIELD_IP6_SRC, dapiCmd->cmdData.ipsgClientAdd.ip6Addr.in6.addr8, exact_match);
      }
#endif
      ...
      {
        hapiBroadPolicyRuleQualifierAdd(pIpsgEntry->policyId, ruleId, BROAD_FIELD_MACSA, dapiCmd->cmdData.ipsgClientAdd.macAddr.addr, exact_match);
      }

      hapiBroadPolicyRuleActionAdd(pIpsgEntry->policyId, ruleId, BROAD_ACTION_PERMIT, 0, 0, 0);
