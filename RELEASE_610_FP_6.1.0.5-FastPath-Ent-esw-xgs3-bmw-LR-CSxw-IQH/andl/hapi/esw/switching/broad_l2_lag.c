...
// PTin added
extern L7_uint8 ptin_lag_pending_configs;
...
void hapiBroadLagAsyncDoneWait(void)
{
  ...
  osapiSemaTake(hapiBroadLagAsyncDoneSema,L7_WAIT_FOREVER);
}
  ...
  while (num_msgs--)
  {
    ...
    switch (lag_cmd.async_cmd)
    {
      case HAPI_BROAD_LAG_ASYNC_PORT_ADD:
        ...
        // PTin add
        if (ptin_lag_pending_configs>0)  ptin_lag_pending_configs--;
        break;
  ...
  for (vlan_id = 1; vlan_id <= L7_PLATFORM_MAX_VLAN_ID; vlan_id++)
  {
    if (BROAD_IS_VLAN_MEMBER(usp, vlan_id, dapi_g))
    {
      if (port_cmd == 1)
      {
        ...
        l7_usl_port_cache_vlan_entry_set(usp->unit, usp->slot, hapiPortPtr->bcm_unit,
                                         hapiPortPtr->bcm_port, vlan_id, tag, L7_TRUE);
      ...
      } else
      {
        ...
        l7_usl_port_cache_vlan_entry_set(usp->unit, usp->slot, hapiPortPtr->bcm_unit,
                                         hapiPortPtr->bcm_port, vlan_id, L7_FALSE, L7_FALSE);
      }
    }
  }
...
L7_RC_t hapiBroadL2LagPortInit(DAPI_PORT_t *dapiPortPtr)
{
  ...
  dapiPortPtr->cmdTable[DAPI_CMD_LAG_PENDING_CONFIGS] = (HAPICTLFUNCPTR_t)hapiBroadLagPendingConfigs;   // PTin added
...
{
  L7_uint8 *ret = (L7_uint8 *) data;

  *ret = ptin_lag_pending_configs;
...
L7_RC_t hapiBroadLagCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  */
  ...
  hapiPortPtr->hapiModeparm.lag.hashMode = 0;
...
L7_RC_t hapiBroadLagPortAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  // PTin add
  if (ptin_lag_pending_configs<(L7_uint8)-1)  ptin_lag_pending_configs++;
...
L7_RC_t hapiBroadLagPortAsyncAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  for (entry=0; entry < cmdLagPortAdd->cmdData.lagPortAdd.numOfMembers; entry++)
  {
    ...
    for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
    {
      if (lagMemberSet[searchEntry].inUse == L7_FALSE)
      {
        ...
        {
          printf("Adding %d.%d.%d to %d.%d.%d  (Up Time: %d)\n",
  ...
  */
  for (entry=0; entry < cmdLagPortAdd->cmdData.lagPortAdd.numOfMembers; entry++)
  {
    ...
    l7_bcmx_port_frame_max_set(lport, cmdLagPortAdd->cmdData.lagPortAdd.maxFrameSize);


    /* Port Priority 
    */
    rc = l7_bcmx_port_untagged_priority_set(lport, hapiLagPortPtr->priority);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      LOG_ERROR(hapiLagMemberPortPtr->bcmx_lport);
    }
    ...
    */
    ...
    rc = l7_bcmx_port_discard_set(lport, mode);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      LOG_ERROR(hapiLagMemberPortPtr->bcmx_lport);
    }
  ...
  for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
  {
    if (lagMemberSet[searchEntry].inUse == L7_TRUE)
    {
      ...
      bcmx_lplist_last_insert (&(trunk_info.ports), lport);
      if (memberCount == 0)
      {
        /* Since these are index into the list we can set it to 0 */
        trunk_info.dlf_port  = lport;
        trunk_info.mc_port   = lport;
        trunk_info.ipmc_port = lport;
      }
  ...
  if ((board_family != BCM_FAMILY_DRACO) &&
         (board_family != BCM_FAMILY_TUCANA))
      {    
          trunk_info.dlf_port  = -1;
          trunk_info.mc_port   = -1;
          trunk_info.ipmc_port = -1;
      }
  ...
  if (!BCMX_LPLIST_IS_EMPTY(&trunk_info.ports))
  {
    ...
    if (hapiLagPortPtr->hapiModeparm.lag.numMembers == 0)
    {
      ...
      usl_bcmx_trunk_learn_mode_set(tid, hapiLagPortPtr->locked);
    } else
    {
      tid = hapiLagPortPtr->hapiModeparm.lag.tgid;
    }

    trunk_info.psc = cmdLagPortAdd->cmdData.lagPortAdd.hashMode;

    if (0 == trunk_info.psc)
    {
        trunk_info.psc = BCM_TRUNK_PSC_SRCDSTIP;
    }
    
    /* then add the member ports */
    rc = usl_bcmx_trunk_set(tid, &trunk_info);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      LOG_ERROR (rc);
    }
...
L7_RC_t hapiBroadLagPortAsyncDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  bcm_chip_family_t             board_family=0; 
  L7_uint16 cond0, cond1, cond2, cond3, cond;
  ...
  */ 
  for (entry=0;entry<cmdLagPortDelete->cmdData.lagPortDelete.numOfMembers;entry++)
  {
    for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
    {
      if (lagMemberSet[searchEntry].inUse == L7_TRUE)  {
        ...
        if (hapiBroadDebugLag)
        {
          printf("%s(%d) lagMemberSet[%u].usp={%u,%u,%u}  cmdLagPortDelete->cmdData.lagPortDelete.memberSet[%u]={%u,%u,%u}\n", __FUNCTION__, __LINE__,
          ...
          printf("%s(%d) (lagMemberSet[searchEntry].usp.unit == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].unit): %u\n", __FUNCTION__, __LINE__,
                  cond1);
          printf("%s(%d) (lagMemberSet[searchEntry].usp.slot == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].slot): %u\n", __FUNCTION__, __LINE__,
                  cond2);
          printf("%s(%d) (lagMemberSet[searchEntry].usp.port == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].port): %u\n", __FUNCTION__, __LINE__,
                  cond3);
          printf("%s(%d) Total: %u\n", __FUNCTION__, __LINE__,cond);
        }

        // PTin: This clode block was never executed
        if ((lagMemberSet[searchEntry].usp.unit == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].unit) &&
        ...
        {  
          if (hapiBroadDebugLag)
          {
            printf("Preparing to Remove %d.%d.%d from %d.%d.%d (Up Time: %d)\n",
  ...
  for (entry=0;entry<cmdLagPortDelete->cmdData.lagPortDelete.numOfMembers;entry++)
  {
    for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
    {
      if (hapiBroadDebugLag)
      {
        ...
        printf("%s(%d) (lagMemberSet[searchEntry].inUse    == L7_TRUE): %u\n", __FUNCTION__, __LINE__,
                cond0);
        printf("%s(%d) (lagMemberSet[searchEntry].usp.unit == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].unit): %u\n", __FUNCTION__, __LINE__,
                cond1);
        printf("%s(%d) (lagMemberSet[searchEntry].usp.slot == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].slot): %u\n", __FUNCTION__, __LINE__,
          ...
          (lagMemberSet[searchEntry].usp.port == cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].port))
      {
        if (hapiBroadDebugLag)
        {
          printf("Removing %d.%d.%d from %d.%d.%d (Up Time: %d)\n",
                 cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].unit,
                 cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].slot,
                 cmdLagPortDelete->cmdData.lagPortDelete.memberSet[entry].port,
                 usp->unit,
  ...
  /* Initialize the lport list */
  memset (&trunk_info, 0, sizeof (bcmx_trunk_add_info_t));
  bcmx_lplist_init(&(trunk_info.ports), L7_MAX_MEMBERS_PER_LAG, BCMX_LP_UNIQ);
  ...
  for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
  {
    if (lagMemberSet[searchEntry].inUse == L7_TRUE)
    {
      ...
      rc = bcmx_lplist_last_insert (&(trunk_info.ports), lport);
      if (rc == BCM_E_NONE)
      {
 ...
 /* get the hardware psc type value */
  trunk_info.psc = cmdLagPortDelete->cmdData.lagPortDelete.hashMode;
  ...
  */
  for (entry=0; entry < cmdLagPortDelete->cmdData.lagPortDelete.numOfMembers; entry++)
  {
    ...
    rc = l7_bcmx_port_frame_max_set(lport, cmdLagPortDelete->cmdData.lagPortDelete.maxFrameSize);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      printf("Failed to set max frame on %d\n",lport);
    ...
    */
    ...
    /* Port Priority 
    */
    rc = l7_bcmx_port_untagged_priority_set(lport, hapiLagMemberPortPtr->priority);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      LOG_ERROR(hapiLagMemberPortPtr->bcmx_lport);
    }
...
L7_RC_t hapiBroadLagHashModeSet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (hapiLagPortPtr->hapiModeparm.lag.hashMode != cmdLagHashMode->cmdData.lagHashMode.hashMode)
  {
    ...
    {
      tid = hapiLagPortPtr->hapiModeparm.lag.tgid;
      rc = usl_bcmx_trunk_psc_set (tid, cmdLagHashMode->cmdData.lagHashMode.hashMode);
      if (L7_BCMX_OK(rc) != L7_TRUE)
      {
        LOG_ERROR (rc);
      }
...
L7_RC_t hapiBroadProtocolVlanLagPortAddNotify(DAPI_USP_t *memberUsp, DAPI_USP_t *lagUsp, DAPI_t *dapi_g)
{
  ...
  for (i = 0; i < DAPI_QVLAN_NUM_OF_PROTOCOL_ENTRIES; i++)
  {
    ...
    {
      hapiBroadProtocolVlanRemove(memberUsp, ethertype[i], dapi_g);
    }
    if (hapiLagPortPtr->pbvlan_table[i] != 0)
    {
      hapiBroadProtocolVlanAdd(memberUsp, ethertype[i], hapiLagPortPtr->pbvlan_table[i], dapi_g);
    }
  }
...
L7_RC_t hapiBroadProtocolVlanLagPortDeleteNotify(DAPI_USP_t *memberUsp, DAPI_USP_t *lagUsp, DAPI_t *dapi_g)
{
  ...
  for (i = 0; i < DAPI_QVLAN_NUM_OF_PROTOCOL_ENTRIES; i++)
  {
    ...
    if (hapiLagMemberPortPtr->pbvlan_table[i] != 0)
    {
      hapiBroadProtocolVlanAdd(memberUsp, ethertype[i], hapiLagMemberPortPtr->pbvlan_table[i], dapi_g);
    }
  }
