...
void CpuInterceptDebug(int enable)
{
...
int hapiBroadDebugPktIsFiltered(L7_uchar8 *pkt)
{
...
void hapiBroadCpuTxRxInit(DAPI_t *dapi_g)
{
  ...
  /* 
  ** On XGS2, only one cos queue is used for transmission. Most of the control
  ** packets (Bpdu's, IP control frames etc) received on the non-manager unit ports 
  ** are tunneled to the manager unit. Hence, manager should avoid transmitting frames
  ** on the cos queues where contention with the received control packets is possible.
  */
  if (hapiBroadXGS2Check() == L7_TRUE)
  {
    hapiBroadTxCosTable[BROAD_TX_PRIO_HIGH  ] = HAPI_BROAD_EGRESS_HIGH_PRIORITY_COS;
    hapiBroadTxCosTable[BROAD_TX_PRIO_NORMAL] = HAPI_BROAD_EGRESS_HIGH_PRIORITY_COS;
  }
  else
  {
...
L7_BOOL hapiBroadPktIsDhcp(L7_uchar8 *pkt)
{
  ...
  L7_ushort16         outertag_id, innertag_id;

  // PTin added
  // For DHCP detection
  outertag_id = (L7_ushort16) *((L7_ushort16 *) &pkt[12]);
  innertag_id = (L7_ushort16) *((L7_ushort16 *) &pkt[16]);
  if (outertag_id==0x8100 || outertag_id==0x88A8)  {
    if (innertag_id==0x8100 || innertag_id==0x88A8)  {
      offset = 4;
    }
    else  {
      offset = 0;
  ...
  if (memcmp(&pkt[offset+16], ip_ethtype, sizeof(ip_ethtype)) == 0)
  {
    if (memcmp(&pkt[offset+27], udp_proto, sizeof(udp_proto)) == 0)
    {
      if (memcmp(&pkt[offset+40], &dhcpc_dport, sizeof(dhcpc_dport)) == 0)
      {
        return L7_TRUE;
      }
      if (memcmp(&pkt[offset+40], &dhcps_dport, sizeof(dhcps_dport)) == 0)
      {
        return L7_TRUE;
      }
    }
  ...
  */
  ...
  if ( (*(L7_ushort16 *)&bcm_pkt.pkt_data->data[4+12] != osapiHtons(hapiBroadDvlanEthertype)) &&
     ...
     ) /* Is this correct way ? */
    {
        ...
        *(L7_ushort16 *)&bcm_pkt.pkt_data->data[12] = osapiHtons(L7_ETYPE_8021Q /*hapiBroadDvlanEthertype*/);
  ...
  else
  {
     ...
     {
        memcpy(&bcm_pkt.pkt_data->data[0], &bcm_pkt.pkt_data->data[4], 12);
        *(L7_ushort16 *)&bcm_pkt.pkt_data->data[12] = L7_ETYPE_8021Q /*hapiBroadDvlanEthertype*/;
...
//  }
  ...
  
  {
    ...
    case DAPI_FRAME_TYPE_NO_L2_EGRESS_DATA_TO_PORT:
      ...
      mirrorDapiPortPtr = DAPI_PORT_GET(usp, dapi_g);
...
bcm_rx_t hapiBroadReceive(L7_int32 unit, bcm_pkt_t *bcm_pkt, void *cookie)
{
  ...
  L7_ushort16          outertag_id, innertag_id;      // PTin added
  L7_int               dhcp_packet_offset=0;          // PTin added

  // PTin added
  time_run = osapiTimeMillisecondsGet();

  // PTin
  if (cpu_intercept_debug)
  {
    printf("%s(%d) Lowest level reception: (reason=%u [%u,%u,%u]) %u rxport:%u, srcport=%u, vid=%u\n", __FUNCTION__, __LINE__,
           bcm_pkt->rx_reason,bcm_pkt->rx_reasons.w[0],bcm_pkt->rx_reasons.w[1],bcm_pkt->rx_reasons.w[2],bcm_pkt->cos,
  ...
  // PTin added
  // For DHCP detection
  outertag_id = (L7_ushort16) *((L7_ushort16 *) &bcm_pkt->pkt_data->data[12]);
  innertag_id = (L7_ushort16) *((L7_ushort16 *) &bcm_pkt->pkt_data->data[16]);
  if (outertag_id==0x8100 || outertag_id==0x88A8)  {
    if (innertag_id==0x8100 || innertag_id==0x88A8)  {
      dhcp_packet_offset = 4;
    }
    else  {
      dhcp_packet_offset = 0;
    }
  }
  else  {
    dhcp_packet_offset = -4;
             ...
             inner inner TPID as 0x8100.*/
     {
       ...
       if (dapi_g->system->dvlanEnable)
       {
         ...
         if(hapiPortPtr->locked == L7_FALSE)
         {
           ...
           {
             innerVlanId = ((L7_ushort16) *((L7_ushort16 *)&(bcm_pkt->pkt_data->data[18]))) & ((L7_ushort16) 0x0FFF);
           }
         ...
         else
         {
           ...
           if (ether_type == FD_DVLANTAG_802_1Q_ETHERTYPE) /* DT frame */
           {
             innerVlanId = ((L7_ushort16) *((L7_ushort16 *)&(bcm_pkt->pkt_data->data[14]))) & ((L7_ushort16) 0x0FFF);
           }
  ...
  if(hapiBroadRoboVariantCheck() == __BROADCOM_53115_ID)
  {
    ...
    if (dapi_g->system->dvlanEnable)
    {
      innerVlanId = ((L7_ushort16) *((L7_ushort16 *)&(bcm_pkt->pkt_data->data[18]))) & ((L7_ushort16) 0xFFF);
    }
    if ( (outerVlanId > L7_MAX_VLAN_ID) ||
         (innerVlanId > L7_MAX_VLAN_ID)
       )
  ...
  if (hapiBroadXGS2Check() == L7_TRUE)
  {
     /* This code assumes that trunk128 mode is disabled on 5695. Tucana/Lynx
      * devices do not have trunk128 support.
      */
     if (bcm_pkt->flags & BCM_PKT_F_TRUNK) 
     {
        if (memcmp(bcm_pkt->pkt_data->data,lacPduMac,6) == 0)
        {
           /* if the packet is an LACPDU, then use the actual physical
            ** port is in the rx_unit and rx_port
            */
           lport = bcmx_unit_port_to_lport(bcm_pkt->rx_unit, bcm_pkt->rx_port);
           uport = BCMX_UPORT_GET(lport);
        ...
        {
          /* if the packet is not an LACPDU, then use the first physical
           ** port of the lag and the code later on will find its lag membership
           */
  
           tgid = bcm_pkt->src_trunk & 0x1F;
           ...
           for (lagUsp.port = 0; lagUsp.port < L7_MAX_NUM_LAG_INTF; lagUsp.port++)
           {
              ...
              {
                 dapiLagPortPtr = DAPI_PORT_GET(&lagUsp,dapi_g);
    
                 for (lagIndex=0; lagIndex < L7_MAX_MEMBERS_PER_LAG; lagIndex++)
                 {
                   if (dapiLagPortPtr->modeparm.lag.memberSet[lagIndex].inUse == L7_TRUE)
                   {
                     usp = dapiLagPortPtr->modeparm.lag.memberSet[lagIndex].usp;
                     gmrpUspCheck = usp;
                     break;
                   }
                 }
  ...
  else /* XGS3 devices */
  {
   ...
   if (weAreHawkeye == L7_TRUE) 
     ...
     if (uport == BCMX_UPORT_INVALID_DEFAULT)
     {
       L7_LOGFNC(L7_LOG_SEVERITY_INFO,
                      "hapiBroadModidModportToLportGet: failed to uport/lport (%d / %d) \n", uport, lport);
       return result;
     }
  ...
  */
  ...
  if (dot1xStatus == L7_DOT1X_PORT_STATUS_UNAUTHORIZED)
  {
    ...
    if (voiceVlanAuthDebug)
    {
      ...
      else if  ((!memcmp(srcPort, &bcm_pkt->pkt_data->data[dhcp_packet_offset+38], 2) &&
                !memcmp(dstPort, &bcm_pkt->pkt_data->data[dhcp_packet_offset+40], 2) &&
                (bcm_pkt->pkt_data->data[dhcp_packet_offset+46] == bootRequest)))
      {
    ...
    else if ( (unAuthMode == L7_ENABLE) && (!memcmp(srcPort, &bcm_pkt->pkt_data->data[dhcp_packet_offset+38], 2) &&
              !memcmp(dstPort, &bcm_pkt->pkt_data->data[dhcp_packet_offset+40], 2) &&
              (bcm_pkt->pkt_data->data[dhcp_packet_offset+46] == bootRequest)) )
    {
      /*
  ...
  */
      ...
      (isValidUsp(&gmrpUspCheck, dapi_g) == L7_TRUE))
  {
    ...
    if (hapiPortPtr->acceptFrameType == L7_DOT1Q_ADMIN_ONLY_VLAN_TAGGED && bcm_pkt->rx_untagged)
    {
      return BCM_RX_HANDLED;
    }
  }
  ...
  if ((memcmp(bcm_pkt->pkt_data->data, gvrpPduMac, L7_MAC_ADDR_LEN) == 0) && (!bcm_pkt->rx_untagged))
  {
#if defined(L7_METRO_PACKAGE) && defined(L7_DOT1AD_PACKAGE)
  ...
  /* Drop duplicate MLD packets. In some cases, two copies of an MLD packet are sent 
     to the CPU. One will be on a high COS (e.g. COS 4), and the other is switched 
     on the VLAN and typically comes to the CPU on COS 1. Drop MLD packets on COS 1. 
...
void hapiBroadReceiveTask(L7_uint32 numArgs, DAPI_t *dapi_g)
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      hapiBroadStatsCpuIncrement(pkt, frameLen, &pktRxMsg.usp, dapi_g, L7_TRUE, pktRxMsg.rx_untagged);
...
static void hapiBroadRxReasonCodeSet(BROAD_PKT_RX_MSG_t *pktRxMsg)
{
  ...
  for (reason = 0; reason < bcmRxReasonCount; reason++)
  {
    if (BCM_RX_REASON_GET(pktRxMsg->reasons, reason))
    {
      ...
      if (reason == bcmRxReasonIcmpRedirect)
        rxCode |= L7_MBUF_RX_REASON_ICMP_REDIR;
      else if (reason == bcmRxReasonL3DestMiss)
        rxCode |= L7_MBUF_RX_REASON_L3_MISS;
      else if (reason == bcmRxReasonSampleSource)
        rxCode |= L7_MBUF_RX_SAMPLE_SOURCE;
      else if (reason == bcmRxReasonSampleDest)
...
#ifdef L7_DOT1AG_PACKAGE
   /* if(!(user_data[5] >= 0x30 && user_data[5] <= 0x3F)) */
   {
#endif
...
void hapiBroadBpduTxTask(L7_uint32 numArgs, DAPI_t *dapi_g)
{
  ...
  while (1)
  {
    ...
    /*
    ** Check for GMRP, they must honor egress tagging
    */
...
#if 0
        SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR,
                       "%s %d: In %s call to 'bcm_packet_send_pbmp' rv = 0x%8x\n",
                            ...
                            DAPI_t *dapi_g)
{
  ...
  DAPI_PORT_t           *dapiPortPtr;
  DAPI_USP_t             searchUsp;
  static L7_BOOL         rt_da_found = L7_FALSE;
  static L7_uchar8       rt_da[6];
#endif
...
#ifdef L7_ROUTING_PACKAGE
  if ((board_family == BCM_FAMILY_DRACO) || (board_family == BCM_FAMILY_TUCANA))
  {
    /* XGS2 only: if the MAC SA is that of an L3 intf, pkt was routed to CPU */
    if (hapiBroadRxMacSaIsL3Intf(&user_data[6], dapi_g) == L7_TRUE)
    {
      return L7_TRUE;
    }
  }

  if (rt_da_found == L7_TRUE)
  {
    if (memcmp (user_data, rt_da, 6) == 0)
    {
      return L7_TRUE;
    }
#ifdef L7_WIRELESS_PACKAGE
     ...
     */
    return hapiBroadL3MacDestIsProxy(user_data,
                                     pktRxMsg->cmdInfo.cmdData.receive.vlanID);
#else /* !L7_WIRELESS_PACKAGE */
    return L7_FALSE;
  ...
  /* if src port is a router port, and MAC DA == router intf MAC, don't drop */
  if (dapiPortPtr->modeparm.physical.routerIntfEnabled == L7_TRUE)
  {
#ifdef HAPI_BROAD_PKT_DEBUG
    if (!hapiBroadDebugPktIsFiltered(user_data))
...
#endif
    if (memcmp(dapiPortPtr->modeparm.physical.macAddr, user_data, 6) == 0)
    {
      rt_da_found = L7_TRUE;
      memcpy (rt_da, user_data, 6);
      return L7_TRUE;
    }
    else
    {
  ...
  /* if src port is a VLAN router, and MAC DA == router intf MAC, don't drop */
  searchUsp.unit = L7_LOGICAL_UNIT;
  searchUsp.slot = L7_VLAN_SLOT_NUM;
  ...
  for (searchUsp.port = 0; searchUsp.port < L7_MAX_NUM_ROUTER_INTF; searchUsp.port++)
  {
    ...
    if (dapiPortPtr->modeparm.router.ipAddr  != 0)
    {
      if (dapiPortPtr->modeparm.router.vlanID == pktRxMsg->cmdInfo.cmdData.receive.vlanID)
      {
#ifdef HAPI_BROAD_PKT_DEBUG
        if (!hapiBroadDebugPktIsFiltered(user_data))
        {
...
#endif
        if (memcmp(dapiPortPtr->modeparm.router.macAddr, user_data, 6) == 0)
        {
          rt_da_found = L7_TRUE;
          memcpy (rt_da, user_data, 6);
          return L7_TRUE;
...
#ifdef L7_WIRELESS_PACKAGE
        else if (hapiBroadL3MacDestIsProxy(user_data,
                     pktRxMsg->cmdInfo.cmdData.receive.vlanID) == L7_TRUE)
        {
          /* Do not cache a proxied MAC address */
          return L7_TRUE;
        }
#endif /* L7_WIRELESS_PACKAGE */
        else
  ...
  // Evaluate if is a L2 Station move
  // I have marked these pakets
  if (pktRxMsg->cos==HAPI_BROAD_INGRESS_LOW_PRIORITY_COS)
  {
    // Only applicable to network interfaces
    if (pktRxMsg->usp.slot!=0 || pktRxMsg->usp.port>=SYSTEM_N_GPON_PORTS)
    {
      L7_uint8 *macAddr = &user_data[6];
      L7_uint16 tagId   = *((L7_uint16 *) (&user_data[12]));
      L7_uint16 vlanId  = *((L7_uint16 *) (&user_data[14])) & 0xfff;
     
      // Tag and vlan must be valid, and mac must be unicast
      if (tagId==0x8100 && vlanId>0 && vlanId<4096 && (macAddr[0] & 0x01)==0x00)
      {
        bcmx_l2_addr_t l2addr;
        bcmx_lplist_t  port_block;
  
        // If this VID+MAC is present in the L2 table,
        // and is static,
        // and belongs to another port
        if (bcmx_l2_addr_get(macAddr,vlanId,&l2addr,&port_block)==BCM_E_NONE &&
            (l2addr.flags & BCM_L2_STATIC) &&
            l2addr.lport!=hapiPortPtr->bcmx_lport)
        {
          // Remove only static entry
          // PTin management MACS will do the rest
          bcmx_l2_addr_delete(macAddr,vlanId);
        }
      }
    }
  }
...
L7_BOOL hapiBroadXgs3RxCheck(BROAD_PKT_RX_MSG_t *pktRxMsg, DAPI_t *dapi_g)
{
  ...
  if (BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonUnknownVlan) ||
      (BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonL2SourceMiss) &&
	    (hpcSoftwareLearningEnabled () != L7_TRUE) && (hapiPort->cpEnabled == L7_FALSE))||
      BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonL2DestMiss) ||
      BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonL2Cpu) ||
      BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonL3SourceMiss) ||
      BCM_RX_REASON_GET(pktRxMsg->reasons, bcmRxReasonL3SourceMove))
