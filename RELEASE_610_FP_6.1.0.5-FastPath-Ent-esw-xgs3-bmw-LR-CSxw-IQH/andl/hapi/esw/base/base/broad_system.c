...
// This is for "fastpath.shell dev"
L7_BOOL hapiBroadSystem_ptin_debug_enable = L7_FALSE;
void hapiBroadSystem_ptin_debug(L7_BOOL enable)
{
  hapiBroadSystem_ptin_debug_enable = enable;
}
...
L7_RC_t hapiBroadStdPortInit(DAPI_PORT_t *dapiPortPtr)
{
  ...
  dapiPortPtr->cmdTable[DAPI_CMD_SYSTEM_DHCP_CONFIG                ] = (HAPICTLFUNCPTR_t)hapiBroadSystemDhcpConfig;
  ...
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_HW_PHY_SET                   ] = (HAPICTLFUNCPTR_t)hapiBroadPtinPhySet;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_HW_RESOURCES                 ] = (HAPICTLFUNCPTR_t)hapiBroadPtinGetHwResources;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_VLAN_DEFS                    ] = (HAPICTLFUNCPTR_t)hapiBroadPtinVlanDefs;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_VLAN_XLATE                   ] = (HAPICTLFUNCPTR_t)hapiBroadPtinVlanTranslate;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_FPENTRY                      ] = (HAPICTLFUNCPTR_t)hapiBroadPtinFPentry;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_FLOW_COUNTERS                ] = (HAPICTLFUNCPTR_t)hapiBroadPtinFlowCounters;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_BCASTPKT_LIMIT               ] = (HAPICTLFUNCPTR_t)hapiBroadPtinBcastPktLimit;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_BITSTREAM_UPFWD              ] = (HAPICTLFUNCPTR_t)hapiBroadPtinBitstreamUpFwd;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_BITSTREAM_LAG_RECHECK        ] = (HAPICTLFUNCPTR_t)hapiBroadPtinBitstreamLagRecheck;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_GET_COUNTERS                 ] = (HAPICTLFUNCPTR_t)hapiBroadPtinGetCounters;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_CLEAR_COUNTERS               ] = (HAPICTLFUNCPTR_t)hapiBroadPtinClearCounters;
  dapiPortPtr->cmdTable[DAPI_CMD_PTIN_GET_COUNTERS_STATE           ] = (HAPICTLFUNCPTR_t)hapiBroadPtinStatisticStatus;
  /* PTin end */
    ...
    DAPI_t *dapi_g)
{
  ...
  if (ability_mask & DAPI_NEG_2500MB_FD) local_ability_mask |= BCM_PORT_ABIL_2500MB_FD;
...
L7_RC_t hapiBroadIntfIsolatePhyConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  bcm_port_t port;
#endif
  /* PTin added: FPGA TXDisable port map */
  const L7_uint8 tx_disable_bit[8] = { 4, 1, 0, 7, 3, 2, 6, 5 };
  L7_uint8 ge_index;
  /* PTin end */
  ...
  /* PTin added: TXDisable for SFPs */
  if (fpga_map!=L7_NULLPTR && fpga_map!=MAP_FAILED &&
      IS_PORT_TYPE_PHYSICAL(dapiPortPtr) &&
      (hapiPortPtr->bcm_port>=8 && hapiPortPtr->bcm_port<=15))
  {
    ge_index = hapiPortPtr->bcm_port-8;
    if (enable)
    {
      fpga_map->registo[FPGA_TXDISABLE_REG] &= ~((L7_uint8) 1<<tx_disable_bit[ge_index]);
    }
    else
    {
      fpga_map->registo[FPGA_TXDISABLE_REG] |=  ((L7_uint8) 1<<tx_disable_bit[ge_index]);
  ...
  if (dapiCmd->cmdData.snoopConfig.family == L7_AF_INET) /* IGMP Snooping */
  {
    if (dapiCmd->cmdData.snoopConfig.enable == L7_TRUE)
    {
      ...
      else
      {
         ...
         result = hapiBroadConfigIgmpFilter (L7_FALSE, vlanId, cos, L7_FALSE, usp, dapi_g);
      ...
      result = hapiBroadConfigIgmpFilter(L7_TRUE, vlanId, cos, switchIgmp, usp, dapi_g);
    }
    else
    {
      ...
      if (!dapi_g->system->mldEnable)
      {
        if (dapi_g->system->mldSnoopingEnable != L7_TRUE)
        {
          ...
          /* Since L3 MLD  is not yet enabled, filter does not exist */
           if ((result=hapiBroadConfigMldFilter(L7_TRUE,dapi_g)) == L7_SUCCESS)
#endif
           {
    ...
    else
    {
      if (!dapi_g->system->mldEnable)
      {
        if (dapi_g->system->mldSnoopingEnable != L7_FALSE)
        {
          ...
          if ((result=hapiBroadConfigMldFilter(L7_FALSE,dapi_g)) == L7_SUCCESS)
#endif
          {
...
L7_RC_t hapiBroadSystemDhcpConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (dapiCmd->cmdData.snoopConfig.family == L7_AF_INET) /* IGMP Snooping */
  {
    result = hapiBroadConfigDhcpFilter(dapiCmd->cmdData.snoopConfig.enable, dapiCmd->cmdData.snoopConfig.vlanId, usp, dapi_g);
  }
  return result;
}
...
// List of vlans for IGMP
static L7_ushort16 igmp_vlan_list[MAX_VLANS];
...
{
  uint8      port, i;
  bcm_pbmp_t eth_ports, msk_ports;
  L7_uint32  portbmp, portbmp_ref;

  BROAD_PORT_t          *hapiPortPtr, *hapiLagMemberPortPtr;
  DAPI_PORT_t           *dapiPortPtr;

  static L7_BOOL          first_time = L7_TRUE, snoop_enable=L7_FALSE;
  static L7_ushort16      cos_list[MAX_VLANS];
  static L7_uint32        portbmp_list[MAX_VLANS], portbmp_prev;
  static BROAD_POLICY_t   igmpId[MAX_VLANS][2];
  L7_uint8                igmp_index_free, igmp_index, index;
  L7_RC_t                 result_global=L7_SUCCESS, result;
  //static BROAD_POLICY_t   igmpId1 = BROAD_POLICY_INVALID, igmpId2 = BROAD_POLICY_INVALID;
  BROAD_POLICY_RULE_t     ruleId = BROAD_POLICY_RULE_INVALID;
  L7_ushort16             ip_ethtype2 = 0x8100;                         /* PTin modified */
  #if 1
  L7_ushort16             ip_ethtype1   = L7_ETYPE_IP;
  L7_uchar8               igmp_proto[]  = {IP_PROT_IGMP};
  #endif
  L7_uchar8               exact_match[] = {FIELD_MASK_NONE, FIELD_MASK_NONE, FIELD_MASK_NONE,
                                           FIELD_MASK_NONE, FIELD_MASK_NONE, FIELD_MASK_NONE};
  L7_uint8 payload_data[16] = {0x00, 0x00, 0x08, 0x00, 0x46, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00 };
  L7_uint8 payload_mask[16] = {0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00 };

  L7_uint8                ip_type = BROAD_IP_TYPE_NONIP;
  BROAD_METER_ENTRY_t     meterInfo;
  BROAD_POLICY_TYPE_t     policyType = BROAD_POLICY_TYPE_SYS_EXT;

  if (hapiBroadSystem_ptin_debug_enable)
    printf("%s(%d) Hello: (snoop_enable=%u) enableFilter=%u, vlanId=%u, cos=%u\r\n", __FUNCTION__, __LINE__,snoop_enable,enableFilter,vlanId,cos);
  ...
  if (first_time)
  {
    ...
    memset(igmp_vlan_list,0x00,sizeof(L7_ushort16)*MAX_VLANS);
    memset(cos_list,0xFF,sizeof(L7_ushort16)*MAX_VLANS);
    memset(portbmp_list,0x00,sizeof(L7_uint32)*MAX_VLANS);
    memset(igmpId,0xFF,sizeof(BROAD_POLICY_t)*MAX_VLANS*2);
    first_time = L7_FALSE;
  }

  // Correct cos
  if (cos>7)  cos = HAPI_BROAD_INGRESS_MED_PRIORITY_COS;

  // Rate limit the IGMP packets for 64 Kbps
  meterInfo.cir       = 64;
  meterInfo.cbs       = 64;
  meterInfo.pir       = 64;
  meterInfo.pbs       = 64;
  ...
  printf("%s(%d) usp={%d,%d,%d}\r\n",__FUNCTION__,__LINE__,usp->unit,usp->slot,usp->port);

  // Calculate list of ports (also considering lags and allports situation)
  portbmp_ref = 0;
  if (IS_PORT_TYPE_CPU(dapiPortPtr))
  {
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) CPU interface\r\n",__FUNCTION__,__LINE__);
    portbmp_ref = (enableFilter) ? 0 : 0xFFFFFF;
  }
  else if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
  {
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) lag interface\r\n",__FUNCTION__,__LINE__);
    hapiBroadLagCritSecEnter();
    portbmp_ref = 0;
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      ...
      {
        hapiLagMemberPortPtr = HAPI_PORT_GET(&dapiPortPtr->modeparm.lag.memberSet[i].usp, dapi_g);
        portbmp_ref |= (L7_uint32) 1 << hapiLagMemberPortPtr->bcm_port;
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) interface %d added\r\n",__FUNCTION__,__LINE__,hapiLagMemberPortPtr->bcm_port);
      }
    }
    hapiBroadLagCritSecExit();
  }
  ...
  {
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) Physical interface (%d) added\r\n",__FUNCTION__,__LINE__,hapiPortPtr->bcm_port);
    portbmp_ref = (L7_uint32) 1 << hapiPortPtr->bcm_port;
  }
  ...
  {
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) Local configuration\r\n", __FUNCTION__, __LINE__);
    igmp_index_free = (L7_uint8)-1;
    for (igmp_index=0; igmp_index<MAX_VLANS; igmp_index++)
    {
      // Save the first free index
      if (igmp_index_free>=MAX_VLANS &&
          (igmp_vlan_list[igmp_index]<2 || igmp_vlan_list[igmp_index]>4093))
        igmp_index_free = igmp_index;      
      // If there is a matching vlan, leave search cycle
      if (vlanId==igmp_vlan_list[igmp_index])  break;
    }
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) igmp_index=%u, igmp_index_free=%u\r\n", __FUNCTION__, __LINE__,igmp_index,igmp_index_free);
    // If Vlan was not found, use the first free index
    if (igmp_index>=MAX_VLANS)
    {
      /* Initial port bitmap */
      portbmp_prev = 0;
      ...
      {
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) New vlan (%u) in index %u\r\n", __FUNCTION__, __LINE__,vlanId,igmp_index_free);
        // IF there is no free index, return error
        if (igmp_index_free>=MAX_VLANS)  return L7_TABLE_IS_FULL;
        igmp_index = igmp_index_free;

        igmp_vlan_list[igmp_index] = vlanId;   // New vlan
        // Store port information
        portbmp_list[igmp_index] = portbmp_ref;
      }
      else
      {
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) Vlan %u don't exist. Nothing to do, because it was to be removed...\r\n", __FUNCTION__, __LINE__,vlanId);
    ...
    {
      /* Initial port bitmap */
      portbmp_prev = portbmp_list[igmp_index];

      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) Vlan %u found!\r\n", __FUNCTION__, __LINE__,vlanId);
      if (enableFilter)
      { 
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) Vlan %u will be reactivated!\r\n", __FUNCTION__, __LINE__,vlanId);
        portbmp_list[igmp_index] |= portbmp_ref;
        // No code
      }
      else
      {  
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) Vlan %u will be removed\r\n", __FUNCTION__, __LINE__,vlanId);
        // If port is valid, only remove one port... if not, remove all
        portbmp_list[igmp_index] &= ~portbmp_ref;
        // Only clear vlan, if there is no ports
        if (portbmp_list[igmp_index]==0)  igmp_vlan_list[igmp_index] = 0;    // Vlan to remove
      }
    }
  }
  else
  {
    /* Initial port bitmap */
    portbmp_prev = 0xffffff;

    snoop_enable = enableFilter;
    //if (!snoop_enable && enableFilter)  snoop_enable = L7_TRUE;

    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) Global configuration\r\n", __FUNCTION__, __LINE__);
    vlanId = 0;
    igmp_index = 0;
  }
  ...
  {
    // Only run all vlans, if configuration is global.
    // Otherwise, only configure one vlan
    if (vlanId!=0 && index!=igmp_index)  continue;

    // If this vlan is already configured, and there is no change on its priority, do nothing
//  if (snoop_enable && enableFilter &&
//      igmpId[index][0]!=BROAD_POLICY_INVALID && igmpId[index][1]!=BROAD_POLICY_INVALID &&
//      cos_list[index] == cos)
//    continue;
    ...
    #if 1
    // Deactivate current IGMP id
    if ( (igmpId[index][0] != BROAD_POLICY_INVALID) && (portbmp_prev & 0xffff00) )
    {
      hapiBroadPolicyDelete(igmpId[index][0]);
      igmpId[index][0] = BROAD_POLICY_INVALID;
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) Removed first policy\r\n", __FUNCTION__, __LINE__);
    }
    if ( (igmpId[index][1] != BROAD_POLICY_INVALID) && (portbmp_prev & 0x0000ff) )
    {
      hapiBroadPolicyDelete(igmpId[index][1]);
      igmpId[index][1] = BROAD_POLICY_INVALID;
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) Removed second policy\r\n", __FUNCTION__, __LINE__);
    }
    #endif

    // Clear priority
    cos_list[index] = (L7_uint8)-1;
    
    // Enable
    // Only create policies, if enables are TRUE, VLAN is valid and there is ports to be associated
    // Policies creation does not depend on enableFilter flag, but if there is associated ports (portbmp_list)
    if ( snoop_enable && /*enableFilter &&*/
         (igmp_vlan_list[index]>=2 && igmp_vlan_list[index]<=4093) &&
         portbmp_list[index]!=0)
    {
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) Final bitmap=0x%X\r\n",__FUNCTION__,__LINE__,portbmp_list[index]);

      // Convert port list, to the physical port list
      portbmp = portbmp_list[index];
      BCM_PBMP_CLEAR(eth_ports);
      BCM_PBMP_CLEAR(msk_ports);
      for (port=0; port<L7_MAX_PHYSICAL_PORTS_PER_UNIT; port++,portbmp>>=1) {
        if (portbmp & 1) {
          if (hapiBroadSystem_ptin_debug_enable)
            printf("%s(%d) Port %u added to eth ports\r\n", __FUNCTION__, __LINE__,port);
          BCM_PBMP_PORT_ADD(eth_ports,port);
        }
        BCM_PBMP_PORT_ADD(msk_ports,port);
      }

      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) snnop_enable and enableFilter is TRUE... [vlan=%u]\r\n", __FUNCTION__, __LINE__,igmp_vlan_list[index]);

      do
      {
        if (result == L7_SUCCESS && igmpId[index][0] == BROAD_POLICY_INVALID 
            && (portbmp_list[index] & 0xffff00) )
        {
          #if 0
          if (igmpId[index][0] != BROAD_POLICY_INVALID )
          {
            hapiBroadPolicyDelete(igmpId[index][0]);
            igmpId[index][0] = BROAD_POLICY_INVALID;
            if (hapiBroadSystem_ptin_debug_enable)
          ...
          if (hapiBroadSystem_ptin_debug_enable)
            printf("%s(%d) Going to (re)activate first policy\r\n", __FUNCTION__, __LINE__);
          #if defined(FEAT_METRO_CPE_V1_0)
          /*On bcm53115 with DOT1AD package, dot1ad related rules in TCAM will be
          ...
          {
           result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_DOT1AD_SNOOP, &igmpId[index][0]);
          }
          else
          {
            /* Don't create the policy now. */
            ...
            return result;
          result = hapiBroadPolicyCreate(policyType, &igmpId[index][0]);
          }
          #else
          result = hapiBroadPolicyCreate(policyType, &igmpId[index][0]);
          #endif
          ...
          if (result != L7_SUCCESS)  {
            result = L7_TABLE_IS_FULL;
            break;
          }
        
          /* give IGMP frames high priority and trap to the CPU. */
          result = hapiBroadPolicyRuleAdd(igmpId[index][0], &ruleId);
          if (result != L7_SUCCESS)  {
            break;
          }
        
          // Add inports rule
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][0], ruleId,
                                                   BROAD_FIELD_INPORTS,
                                                   (L7_uchar8 *)&eth_ports, (uint8 *) &msk_ports);
          if (result != L7_SUCCESS)  {
            break;
          }
          if (hapiBroadSystem_ptin_debug_enable)
            printf("%s(%d) Inports=0x%X\r\n", __FUNCTION__, __LINE__,portbmp_list[index]);
        
          // Add Vlan rule, only if vlan is valid
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][0], ruleId,
                                                   BROAD_FIELD_IVID,
                                                   (L7_uchar8 *)&igmp_vlan_list[index], exact_match);
          if (result != L7_SUCCESS)  {
            break;
          }
          if (hapiBroadSystem_ptin_debug_enable)
          ...
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][0], ruleId,
                                                   BROAD_FIELD_ETHTYPE,
                                                   (L7_uchar8 *)&ip_ethtype1, exact_match);
          if (result != L7_SUCCESS)  {
            break;
          }
          ...
          // PTin modified
          result = hapiBroadPolicyRuleActionAdd(igmpId[index][0], ruleId, 
                                                BROAD_ACTION_SET_COSQ, 
                                                HAPI_BROAD_INGRESS_MED_PRIORITY_COS, 0, 0);
        
          if ((result=hapiBroadPolicyRuleNonConfActionAdd(igmpId[index][0], ruleId, BROAD_ACTION_HARD_DROP,0,0,0))!=L7_SUCCESS)
            break;
          if ((result=hapiBroadPolicyRuleMeterAdd(igmpId[index][0], ruleId, &meterInfo))!=L7_SUCCESS)
            break;
          ...
          if ((result = hapiBroadPolicyCommit(igmpId[index][0])) != L7_SUCCESS)  {
            result = L7_TABLE_IS_FULL;
            break;
          }
        }
        ...
        //result = L7_SUCCESS;
        if (result == L7_SUCCESS && igmpId[index][1] == BROAD_POLICY_INVALID
            && (portbmp_list[index] & 0x0000ff))
        {
          #if 0
          if (igmpId[index][1] != BROAD_POLICY_INVALID )
          {
            hapiBroadPolicyDelete(igmpId[index][1]);
            igmpId[index][1] = BROAD_POLICY_INVALID;
            if (hapiBroadSystem_ptin_debug_enable)
          ...
          if (hapiBroadSystem_ptin_debug_enable)
            printf("%s(%d) Going to (re)activate second policy\r\n", __FUNCTION__, __LINE__);
          #if defined(FEAT_METRO_CPE_V1_0)
          /*On bcm53115 with DOT1AD package, dot1ad related rules in TCAM will be
            hit first for IGMP packets (as DOT1AD rules are only based on VLAN tags).
            As a result IGMP packets will not be trapped to CPU.
          ...
          {
           result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_DOT1AD_SNOOP, &igmpId[index][1]);
          }
          else
          {
            /* Don't create the policy now. */
            /* In CPE code if the IGMP Snooping is enabled a call will be made to this 
               function after dvlan is enabled */ 
            return result;
          result = hapiBroadPolicyCreate(policyType, &igmpId[index][1]);
          }
          #else
          result=hapiBroadPolicyCreate(policyType, &igmpId[index][1]);
          #endif
          ...
          /* give IGMP frames high priority and trap to the CPU. */
          result = hapiBroadPolicyRuleAdd(igmpId[index][1], &ruleId);
          if (result != L7_SUCCESS)  {
            break;
          }
    
          // Add inports rule
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][1], ruleId,
                                                   BROAD_FIELD_INPORTS,
                                                   (L7_uchar8 *)&eth_ports, (uint8 *) &msk_ports);
          if (result != L7_SUCCESS)  {
            break;
          ...
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][1], ruleId,
                                                   BROAD_FIELD_ETHTYPE,
                                                   (L7_uchar8 *)&ip_ethtype2, exact_match);
          if (result != L7_SUCCESS)  {
            break;
          }
          ...
          result = hapiBroadPolicyRuleQualifierAdd(igmpId[index][1], ruleId,
                                                   BROAD_FIELD_IP6_DST,
                                                   payload_data, payload_mask);
          if (result != L7_SUCCESS)  {
            break;
          }
    
          result = hapiBroadPolicyRuleActionAdd(igmpId[index][1], ruleId, 
                                                BROAD_ACTION_SET_COSQ, 
                                                HAPI_BROAD_INGRESS_MED_PRIORITY_COS, 0, 0);
  
          if ((result=hapiBroadPolicyRuleNonConfActionAdd(igmpId[index][1], ruleId, BROAD_ACTION_HARD_DROP,0,0,0))!=L7_SUCCESS)
            break;
          if ((result=hapiBroadPolicyRuleMeterAdd(igmpId[index][1], ruleId, &meterInfo))!=L7_SUCCESS)
            break;
          ...
          if ((result = hapiBroadPolicyCommit(igmpId[index][1])) != L7_SUCCESS)  {
            result = L7_TABLE_IS_FULL;
            break;
          }
        }
      ...
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) (Re)activation done. Proceeding...\r\n", __FUNCTION__, __LINE__);

      // In case of any error, remove any configuration done
      if (result != L7_SUCCESS)
      {
        if (hapiBroadSystem_ptin_debug_enable)
        ...
        if ( igmpId[index][0] != BROAD_POLICY_INVALID )
        {
          ...
          (void)hapiBroadPolicyDelete(igmpId[index][0]);  
          igmpId[index][0] = BROAD_POLICY_INVALID;
        }
        if ( igmpId[index][1] != BROAD_POLICY_INVALID )
        {
          // attempt to delete the policy in case it was created
          (void)hapiBroadPolicyDelete(igmpId[index][1]);  
          igmpId[index][1] = BROAD_POLICY_INVALID;
        }
        igmp_vlan_list[index] = 0;
        portbmp_list[index] = 0;
      }
      // When successfull, save CoS
      else
      {
        if (hapiBroadSystem_ptin_debug_enable)
          printf("%s(%d) Success with (re)activation!\r\n", __FUNCTION__, __LINE__);
        cos_list[index] = cos;
      }
    }
  ...
  if (vlanId==0)
  {
    //if (snoop_enable && !enableFilter)  snoop_enable = L7_FALSE;
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) global state: %u\r\n", __FUNCTION__, __LINE__,snoop_enable);
  }
...
L7_uint16 hapiBroadPtinIgmp_numberOfAvailableVlans(void)
{
  ...
  {
    if (igmp_vlan_list[i]>0 && igmp_vlan_list[i]<(L7_uint16)-1)  number++;
  }

  return (MAX_VLANS-number);
}

// List of vlans for DHCP
static L7_ushort16 dhcp_vlan_list[MAX_VLANS];

L7_RC_t hapiBroadConfigDhcpFilter(L7_BOOL enableFilter, L7_uint16 vlanId, DAPI_USP_t *usp, DAPI_t *dapi_g)
{
  static L7_BOOL          first_time = L7_TRUE;
  static BROAD_POLICY_t   dhcpId[MAX_VLANS][2];
  ...
  BROAD_POLICY_t      /*policyId,*/ policyId2;
  BROAD_POLICY_RULE_t ruleId  = BROAD_POLICY_RULE_INVALID;
  BROAD_METER_ENTRY_t meterInfo;

  L7_uint8  ip_type[]   = { BROAD_IP_TYPE_NONIP };
  L7_uint8 dstip6_data[16] = { 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00 };
  L7_uint8 dstip6_mask[16] = { 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00 };

  L7_uint8 srcip6_data_client[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x00 };
  L7_uint8 srcip6_data_server[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x43, 0x00, 0x00, 0x00, 0x00 };
  L7_uint8 srcip6_mask[16]        = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00 };
  L7_uint8 mask_vlan[2]           = { 0x0f, 0xf0 };
  ...
  if (first_time)
  {
    if (hapiBroadSystem_ptin_debug_enable)
      printf("%s(%d) First time\r\n", __FUNCTION__, __LINE__);
    memset(dhcp_vlan_list,0x00,sizeof(L7_ushort16)*MAX_VLANS);
    memset(dhcpId,0xFF,sizeof(BROAD_POLICY_t)*MAX_VLANS*2);
    first_time = L7_FALSE;
  }
  ...
  for (index=0; index<MAX_VLANS; index++)
  {
    ...
    if (index_free>=MAX_VLANS &&
        (dhcp_vlan_list[index]<2 || dhcp_vlan_list[index]>4093))
      index_free = index;
    // If there is a matching vlan, leave search cycle
    if (vlanId==dhcp_vlan_list[index])  break;
  }
  if (hapiBroadSystem_ptin_debug_enable)
  ...
  if (!enableFilter &&
      (/*dhcpId[index][0]!=BROAD_POLICY_INVALID ||*/ dhcpId[index][1]!=BROAD_POLICY_INVALID))
  {
    if (hapiBroadSystem_ptin_debug_enable)
    ...
    if (IS_PORT_TYPE_CPU(dapiPortPtr))
    {
      ...
      if (/*(dhcpId[index][0]!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromAll(dhcpId[index][0])!=L7_SUCCESS) ||*/
          (dhcpId[index][1]!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromAll(dhcpId[index][1])!=L7_SUCCESS))
        result = L7_FAILURE;
    }
    else if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
    {
      ...
      for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
      {
        if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
        {
          ...
          if (/*(dhcpId[index][0]!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromIface(dhcpId[index][0],lport)!=L7_SUCCESS) ||*/
              (dhcpId[index][1]!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromIface(dhcpId[index][1],lport)!=L7_SUCCESS))
          {
            hapiBroadLagCritSecExit();
            if (hapiBroadSystem_ptin_debug_enable)
    ...
    for (i = 0; i < L7_MAX_PHYSICAL_PORTS_PER_UNIT; i++)
    {
      ...
      if (/*(dhcpId[index][0]!=BROAD_POLICY_INVALID && hapiBroadPolicyIfaceCheck(dhcpId[index][0],lport)==L7_ALREADY_CONFIGURED) ||*/
          (dhcpId[index][1]!=BROAD_POLICY_INVALID && hapiBroadPolicyIfaceCheck(dhcpId[index][1],lport)==L7_ALREADY_CONFIGURED))
        break;
    }
  ...
  if (!enableFilter ||
      dhcp_vlan_list[index]!=vlanId ||
      /*dhcpId[index][0]==BROAD_POLICY_INVALID ||*/ dhcpId[index][1]==BROAD_POLICY_INVALID)
  {
    // If some policyId is not null, delete both
    if (dhcpId[index][0]!=BROAD_POLICY_INVALID || dhcpId[index][1]!=BROAD_POLICY_INVALID)
    {
      //hapiBroadPolicyDelete(dhcpId[index][0]);
      hapiBroadPolicyDelete(dhcpId[index][1]);
      hapiSystem->dhcpSnoopingPolicyId[index][0] = BROAD_POLICY_INVALID;
      hapiSystem->dhcpSnoopingPolicyId[index][1] = BROAD_POLICY_INVALID;
      //dhcpId[index][0] = BROAD_POLICY_INVALID;
      dhcpId[index][1] = BROAD_POLICY_INVALID;
      dhcp_vlan_list[index] = 0;
      if (hapiBroadSystem_ptin_debug_enable)
    ...
    {
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) enableFilter is TRUE... [vlan=%u]\r\n", __FUNCTION__, __LINE__,vlanId);
      /* PTin removed */
      #if 0
      policyId = dhcpId[index][0];
      if (result == L7_SUCCESS && dhcpId[index][0] == BROAD_POLICY_INVALID)
      {
        ...
        {    
          ...
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_INPORTS, (L7_uchar8 *)&inPorts, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_OVID, (L7_uchar8 *)&vlanId, mask_vlan);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ip_ethtype, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_PROTO, udp_proto, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_DPORT, (L7_uchar8*)&dhcpc_dport, exact_match);
          ...
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ip_ethtype, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_PROTO, udp_proto, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_DPORT, (L7_uchar8*)&dhcps_dport, exact_match);
          hapiBroadPolicyRuleActionAdd(policyId, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_MED_PRIORITY_COS, 0, 0);
          hapiBroadPolicyRuleActionAdd(policyId, ruleId, BROAD_ACTION_TRAP_TO_CPU, 0, 0, 0);
          hapiBroadPolicyRuleNonConfActionAdd(policyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
          hapiBroadPolicyRuleMeterAdd(policyId, ruleId, &meterInfo);
      ...
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) enableFilter is TRUE... [vlan=%u]\r\n", __FUNCTION__, __LINE__,vlanId);
      policyId2 = dhcpId[index][1];
      if (result == L7_SUCCESS && dhcpId[index][1] == BROAD_POLICY_INVALID)
      {
        policyId2 = BROAD_POLICY_INVALID;
        if ((result=hapiBroadPolicyCreate(policyType, &policyId2))==L7_SUCCESS)
        {
          hapiBroadPolicyPriorityRuleAdd(policyId2, &ruleId, BROAD_POLICY_RULE_PRIORITY_HIGHEST);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_INPORTS, (L7_uchar8 *)&inPorts, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_OVID, (L7_uchar8 *)&vlanId, mask_vlan);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&innertpid, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP_TYPE, ip_type, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP6_DST, dstip6_data, dstip6_mask);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP6_SRC, srcip6_data_client, srcip6_mask);
          hapiBroadPolicyRuleActionAdd(policyId2, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_MED_PRIORITY_COS, 0, 0);
          hapiBroadPolicyRuleActionAdd(policyId2, ruleId, BROAD_ACTION_TRAP_TO_CPU, 0, 0, 0);
          hapiBroadPolicyRuleNonConfActionAdd(policyId2, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
          hapiBroadPolicyRuleMeterAdd(policyId2, ruleId, &meterInfo);
          ...
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP_TYPE, ip_type, exact_match);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP6_DST, dstip6_data, dstip6_mask);
          hapiBroadPolicyRuleQualifierAdd(policyId2, ruleId, BROAD_FIELD_IP6_SRC, srcip6_data_server, srcip6_mask);
          hapiBroadPolicyRuleActionAdd(policyId2, ruleId, BROAD_ACTION_SET_COSQ, HAPI_BROAD_INGRESS_MED_PRIORITY_COS, 0, 0);
          hapiBroadPolicyRuleActionAdd(policyId2, ruleId, BROAD_ACTION_TRAP_TO_CPU, 0, 0, 0);
          hapiBroadPolicyRuleNonConfActionAdd(policyId2, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
          hapiBroadPolicyRuleMeterAdd(policyId2, ruleId, &meterInfo);
      
          if ((result=hapiBroadPolicyCommit(policyId2))!=L7_SUCCESS)
            result = L7_TABLE_IS_FULL;
        }
        else
        {
      ...
      if (hapiBroadSystem_ptin_debug_enable)
        printf("%s(%d) activation done. Proceeding... PolicyId={X,%d}\r\n", __FUNCTION__, __LINE__,/*policyId,*/policyId2);
      ...
      if (result==L7_SUCCESS)
      {
        ...
        #if 0
        if (policyId!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromAll(policyId)!=L7_SUCCESS)
        {
          result = L7_FAILURE;
        }
        else
        #endif
        if (policyId2!=BROAD_POLICY_INVALID && hapiBroadPolicyRemoveFromAll(policyId2)!=L7_SUCCESS)
        { 
          result = L7_FAILURE;
        }
      }
      ...
      if (result==L7_SUCCESS)
      {
          //dhcpId[index][0] = policyId;
          dhcpId[index][1] = policyId2;
          dhcp_vlan_list[index] = vlanId;
          //hapiSystem->dhcpSnoopingPolicyId[index][0] = policyId;
          hapiSystem->dhcpSnoopingPolicyId[index][1] = policyId2;
      }
      else
      {
        if (hapiBroadSystem_ptin_debug_enable)
        ...
        (void)hapiBroadPolicyDelete(policyId2);
        hapiSystem->dhcpSnoopingPolicyId[index][0] = BROAD_POLICY_INVALID;
        hapiSystem->dhcpSnoopingPolicyId[index][1] = BROAD_POLICY_INVALID;
        //dhcpId[index][0] = BROAD_POLICY_INVALID;
        dhcpId[index][1] = BROAD_POLICY_INVALID;
  ...
  if (result == L7_SUCCESS && enableFilter &&
      (/*dhcpId[index][0]!=BROAD_POLICY_INVALID ||*/ dhcpId[index][1]!=BROAD_POLICY_INVALID) &&
    ...
    if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
    {
      ...
      for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
      {
        ...
        if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
        {
          ...
          if (/*(dhcpId[index][0]!=BROAD_POLICY_INVALID && hapiBroadPolicyApplyToIface(dhcpId[index][0],lport)!=L7_SUCCESS) ||*/
              (dhcpId[index][1]!=BROAD_POLICY_INVALID && hapiBroadPolicyApplyToIface(dhcpId[index][1],lport)!=L7_SUCCESS))
          {  
            hapiBroadLagCritSecExit();
            if (hapiBroadSystem_ptin_debug_enable)
...
L7_uint16 hapiBroadPtinDhcp_numberOfAvailableVlans(void)
{
  ...
  for (i=0; i<MAX_VLANS; i++)
  {
    if (dhcp_vlan_list[i]>0 && dhcp_vlan_list[i]<(L7_uint16)-1)  number++;
  }
