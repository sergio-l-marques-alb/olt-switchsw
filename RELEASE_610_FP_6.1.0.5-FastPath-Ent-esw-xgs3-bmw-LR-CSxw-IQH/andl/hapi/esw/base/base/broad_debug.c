...
int hapiBroadDebugDuplexAdvertSet(int unit,int slot,int port,int duplex)
{
  ...
  if (duplex == 0)
  {
    ability_mask &= ~(BCM_PORT_ABIL_10MB_FD | BCM_PORT_ABIL_100MB_FD | BCM_PORT_ABIL_1000MB_FD | BCM_PORT_ABIL_2500MB_FD);
    ability_mask |=  (BCM_PORT_ABIL_10MB_HD | BCM_PORT_ABIL_100MB_HD | BCM_PORT_ABIL_1000MB_HD | BCM_PORT_ABIL_2500MB_HD);
  }
  else
  {
    ability_mask &= ~(BCM_PORT_ABIL_10MB_HD | BCM_PORT_ABIL_100MB_HD | BCM_PORT_ABIL_1000MB_HD | BCM_PORT_ABIL_2500MB_HD);
...
void hapiBroadDebugModifyBpduTrap(int enable)
{
  ...
  {
    hapiBroadPolicyRuleAdd(sysId2, &ruleId);
    hapiBroadPolicyRuleQualifierAdd(sysId2, ruleId, BROAD_FIELD_MACSA, zero_mac_addr, exact_match);
    hapiBroadPolicyRuleActionAdd(sysId2, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);

    hapiBroadPolicyRuleAdd(sysId2, &ruleId);
    hapiBroadPolicyRuleQualifierAdd(sysId2, ruleId, BROAD_FIELD_MACDA, zero_mac_addr, exact_match);
    hapiBroadPolicyRuleActionAdd(sysId2, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
  }
