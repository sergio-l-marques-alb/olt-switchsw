...
void hapiBroadPortLinkStatusTask(void)
{
  ...
  do
  {
    ...
    */
    ...
    else if ((dapiPortPtr->modeparm.physical.isLinkUp == L7_TRUE) && (link_msg.linkstatus == FALSE))
    {
        /* link is down */
        dapiPortPtr->modeparm.physical.isLinkUp = L7_FALSE;
        hapiBroadPortLinkDown(&usp, dapi_g);
...
#if 0 /* Move to CPU card Init */
  /* Loop through the CPU Ports on this slot */
  cpuSlotNum = dapi_g->unit[usp.unit]->cpuSlotNum;
  usp.slot = cpuSlotNum;

  for (cpuPortNum=dapi_g->unit[usp.unit]->slot[usp.slot]->cpuPortStartNum,cpuCount=0;
      cpuCount<dapi_g->unit[usp.unit]->slot[usp.slot]->numOfCpuPortsInSlot;
      cpuPortNum++,cpuCount++)
  {

    usp.port = cpuPortNum;

    dapiPortPtr = DAPI_PORT_GET(&usp);
