...
L7_RC_t usmDb1213SysDescrGet(L7_uint32 UnitIndex, L7_char8 *buf)
{
  ...
  {
    osapiStrncat( buf, "\n ", L7_CLI_MAX_STRING_LENGTH - (strlen(buf)+1) );
    osapiStrncat( buf, stat, L7_CLI_MAX_STRING_LENGTH - (strlen(buf)+1) );
  }
  return L7_SUCCESS;
}
...
L7_RC_t usmDb1213SysDescrSingleLineGet(L7_uint32 UnitIndex, L7_char8 *buf)
{
  ...
  rc = usmDb1213SysDescrGet(UnitIndex, buf);
  if ( L7_SUCCESS == rc )
  {
    dst = src = buf;
    len = strlen(buf) + 1;        /* include trailing '\0' */
    for (i = 0; i < len; i++)
    {
      if (*src == '\n')
      {
        src++;
        /* special case: also remove single blank space after newline */
        if (*src == ' ')
        {
          src++;
          i++;
...
L7_RC_t usmDb1213SysContactSet(L7_uint32 UnitIndex, L7_char8 *sysContact)
{
 ...
 if (usmDbStringAlphaNumericCheck(sysContact) != L7_SUCCESS)
 {
   return L7_FAILURE;
 }
...
L7_RC_t usmDb1213SysNameSet(L7_uint32 UnitIndex, L7_char8 *sysName)
{
 ...
 if (usmDbStringAlphaNumericCheck(sysName) != L7_SUCCESS)
 {
   return L7_FAILURE;
 }
...
L7_RC_t usmDb1213SysLocationSet(L7_uint32 UnitIndex, L7_char8 *sysLocation)
{
 if (strlen(sysLocation) > L7_SYS_SIZE-1)
  ...
  if (usmDbStringAlphaNumericCheck(sysLocation) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
