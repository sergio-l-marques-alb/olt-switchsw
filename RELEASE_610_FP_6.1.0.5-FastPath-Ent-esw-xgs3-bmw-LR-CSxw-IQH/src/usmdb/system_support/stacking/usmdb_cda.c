...
{
#if L7_FEAT_OVERRIDE_DST_IMG
   if ( 0 == strcmp(dstImage,DIM_BACKUP_IMAGE))
   {
     return cdaCodeUpdate (unit, srcImage, "image", L7_TRUE);
...
{
  return cdaImageActivate(unit, image);
}
...
/**************************************************************************
* @purpose  copy the image file from standby to supervisor
*
* @param    unit        secondary CFMs unitid
* @param    srcImage    Image to be copied
* @param    dstImage    Image to be updated
*
...
{
  return(cdaUpdateACFMwithSCFMImage(unit,srcImage,dstImage));
}
