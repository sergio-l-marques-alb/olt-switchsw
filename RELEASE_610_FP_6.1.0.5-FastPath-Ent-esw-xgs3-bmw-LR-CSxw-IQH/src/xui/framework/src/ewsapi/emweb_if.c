...
{
  return ewsContextNetHandle (context)->submitType;
}

char *emwebRepeatInitDetails (void *context, int count, int split, int max, boolean more)
{
  ewsContextNetHandle (context)->repeatCount = count;
  ewsContextNetHandle (context)->repeatCounter = 0;
  ...
  if (split != 0)
  {
    ...
    ewsContextNetHandle (context)->repeatSplitCount = split;
  }
  strcpy (ewsContextNetHandle (context)->repeatRetVal, "TRUE");
  if (max != 0)
  {
    ewsContextNetHandle (context)->repeatMax = max;
    ewsContextNetHandle (context)->repeatMore = more;
  }
  return (char *) &ewsContextNetHandle (context)->repeatCount;
}
...
char *emwebRepeatIsFirst (void *context)
{
  int cur_repeat;
  int num_rows = 0;
  int i;
  ...
  split = ewsContextNetHandle (context)->repeatSplitCount;
  count = ewsContextNetHandle (context)->repeatCount;

  cur_repeat = getRepeatForRow (++(ewsContextNetHandle (context)->repeatCounter), count, split);
  ...
  if (ewsContextNetHandle (context)->repeatCounter == num_rows + 1)
  {
    return ewsContextNetHandle (context)->repeatRetVal;
  }

  return NULL;
}

char *emwebRepeatIsLast (void *context)
{
  ...
  if (ewsContextNetHandle (context)->repeatCounter == num_rows)
  {
    return ewsContextNetHandle (context)->repeatRetVal;
  }
...
char *emwebRepeatIsMore (void *context)
{
  if (ewsContextNetHandle (context)->repeatMore == FALSE)
  {
    return NULL;
  }
  return ewsContextNetHandle (context)->repeatRetVal;
...
{
  *buf16 = ewsContextNetHandle (context)->numberBuf;
