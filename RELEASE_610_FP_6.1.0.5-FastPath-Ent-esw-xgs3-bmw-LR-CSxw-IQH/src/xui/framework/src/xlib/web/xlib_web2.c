...
static xLibBool_t xLibWeb2RecordError (void *cntx, xLibId_t oid, xLibRC_t rc, xLibS8_t * value);
...
{
  xLibWa_t *wap = (xLibWa_t *) emwebAppWaGet (webSrvCntx, sizeof (*wap));
...
void xLibWeb2Set (void *cntx)
{
  ...
  for (i = 0; i < wap->objCount; i++)
  {
        ...
        && (objp->flag & XLIB_IS_VALIDATE))
    {
      ...
      if (objp->rc != XLIBRC_SUCCESS)
      {
        err = xLibWeb2RecordError (wap, objp->oid, objp->rc, *objp->ui.web.var);
...
{
  xLibU16_t size;
  xLibWa_t *wap = (xLibWa_t *) emwebAppWaGet (servContext, sizeof (*wap));
...
char *xLibWeb2RepeatCount (void *cntx, xLibU32_t pageMax, xLibU32_t split)
{
  ...
  retval = emwebRepeatInitDetails (wap->ui.web.webCntxt, count, split, pageMax,
...
static xLibBool_t xLibWeb2RecordError (void *cntx, xLibId_t oid, xLibRC_t rc, xLibS8_t * value)
{
  ...
  xLibOEMErrorOutputNameGet (oid, (xLibS8_t *)name,value, rc, buf, sizeof (buf));
...
char *xLibWeb2ErrorMessageGet (void *servContext, xLibId_t oid, xLibU32_t nlsId, xLibRC_t rc)
{
  ...
  temp = (char *) xLibOEMErrorOutputNameGet (oid, name, value, rc, ptr, sizeof (ptr));
  if (NULL != temp)
  {
    if (*temp == '\r')
    {
...
{
  return emwebRepeatIsFirst (context);
}

char *xLibWeb2RepeatIsLast (void *context)
{
  return emwebRepeatIsLast (context);
}
