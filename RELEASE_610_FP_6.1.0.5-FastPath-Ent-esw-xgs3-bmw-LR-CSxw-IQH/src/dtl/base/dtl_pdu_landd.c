                                    ...
                                    void *dapiEventInfo)
{
  ...
  // PTin
  //printf("%s:%s(%d) I have received a PDU inside callback\n",__FILE__, __FUNCTION__, __LINE__);
  ...
  if (dtlMsgQueue && ((dtlMsgTail+1)%DTL_MSG_COUNT != dtlMsgHead))
  {
        ...
        != L7_SUCCESS)
    {
      ...
      LOG_MSG("DTL: pdu receive msg send failed, mrc=%d\n", mrc);
      dr = L7_FAILURE;
    }
  }
  else
  {
    /*exit critical section*/
    dtlExitCS();
    LOG_MSG("DTL: dtl pdu receive msg queue full , mrc=%d\n", mrc);
    dr = L7_FAILURE;
  }
