                                  ...
                                  DAPI_ROUTING_INTF_MGMT_CMD_t *dapiCmd)
{
  ...
  nimGetIntfAddress(ipCircDesc->ipCircuit.intIfNum, L7_SYSMAC_BIA,
  ...
  DTL_IP_TRACE("%s : intIfNum = %d, vlanID = %d \n",
                 __FUNCTION__,
                 ipCircDesc->ipCircuit.intIfNum, ipCircDesc->ipCircuit.vlanId);
                            ...
                            DAPI_ROUTING_INTF_MGMT_CMD_t *dapiCmd)
{
  ...
  DTL_IP_TRACE("%s : intIfNum = %d IP Address= %x, Mask = %x\n",
                 __FUNCTION__, intIfNum, ipAddr, ipMask);
...
L7_RC_t dtlIpv4RtrIntfModify(dtlRtrIntfDesc_t *ipCircDesc)
{
  ...
  DTL_IP_TRACE("%s. intIfNum %u, vlan %u, encaps %s, mtu %u, flags %#x\n",
                 __FUNCTION__,
                 ipCircDesc->ipCircuit.intIfNum, ipCircDesc->ipCircuit.vlanId,
                 (ipCircDesc->llEncapsType == L7_ENCAP_ETHERNET) ? "enet" : "snap",
                             ...
                             L7_IP_MASK_t ipMask)
{
  ...
  if (dtlIpv4AddrCmdBuild(intIfNum, ipAddr, ipMask, &dapiCmd) == L7_SUCCESS)
  {
    ...
    DTL_IP_TRACE("%s : intIfNum = %d, IP Addr =  %x, IP Mask = %x  \n",
                   __FUNCTION__,
                   intIfNum,  ipAddr,  ipMask);
...
L7_RC_t dtlIpv4MulticastIntfForwardModeSet(L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  DTL_IP_TRACE("%s : mode =  %s, intIfNum %d\n",
                 __FUNCTION__, mode?"enable":"disable", intIfNum);
...
L7_RC_t dtlIpv4LocalMulticastAddrAdd(L7_uint32 intIfNum, L7_IP_ADDR_t ipAddress)
{
  ...
  DTL_IP_TRACE("%s : intIfNum = %d, IP Addr = %x  \n",
                 routine_name,
                 intIfNum, ipAddress);
...
L7_RC_t dtlIpv4ArpEntryAdd(L7_arpEntry_t *pArp)
{
  ...
  DTL_IP_TRACE("%s : IP Addr = %x, intIfNum = %d, flags = %x, vlan = %d" \
                 " mac addr = %d:%d:%d:%d:%d:%d \n",
                 routine_name,
                 pArp->ipAddr, pArp->intIfNum, pArp->flags,pArp->vlanId,
...
L7_RC_t dtlIpv4ArpEntryQuery(L7_arpQuery_t *pArp)
{
  ...
  DTL_IP_TRACE("%s %d: %s : IP Addr = %x, vlan = %d, intf = %d\n", 
               __FILE__, __LINE__, routine_name,
               pArp->ipAddr, pArp->vlanId, pArp->intIfNum);
                                        ...
                                        DAPI_ROUTING_ROUTE_ENTRY_t *pRouteEntry)
{
  ...
  {
    DTL_IP_TRACE("%s : IP Addr = %x, Mask = %x. intIfNum = %d, vlan = %d, flags =  %x  \n",
                   routine_name,
                   pRoute->ipAddr, pRoute->subnetMask, pRoute->ecmpRoutes.equalCostPath[i].arpEntry.intIfNum,
...
L7_RC_t dtlIpv4VrrpVridAdd(L7_uint32 intIfNum, L7_uint32 vrID, L7_uint32 ipAddr)
{
  ...
  DTL_IP_TRACE("%s : intIfNum = %d, vrID =  %d",
                 routine_name,
                 intIfNum,  vrID);
