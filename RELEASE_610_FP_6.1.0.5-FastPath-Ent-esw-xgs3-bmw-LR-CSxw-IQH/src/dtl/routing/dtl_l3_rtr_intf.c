                               ...
                               DAPI_ROUTING_INTF_MGMT_CMD_t *dapiCmd)
{
  ...
  DTL_IP_TRACE("%s : intIfNum = %d, vlanID = %d \n",
                 __FUNCTION__,
                 ipCircuit->intIfNum, ipCircuit->vlanId);
  ...
  nimGetIntfAddress(ipCircuit->intIfNum, L7_SYSMAC_BIA,
...
L7_RC_t dtlRtrIntfModeSet(L7_uint32 intIfNum, L7_uint32 vlanId, L7_uint32 mode)
{
  ...
  DTL_IP_TRACE("%s : intIfNum %u, mode =  %s,  \n",
