...
xLibRC_t fpObjGet_SwitchingARPAclRule_RuleIP (void *wap, void *bufp)
{
  ...
  xLibStr256_t objRuleMACValue, nextObjRuleMACValue;
  ...
  if (owa.rc != XLIBRC_SUCCESS)
  {
    ...
    owa.l7rc = usmDbArpAclRuleNextGet(objARPACLValue,
                                     objRuleIPValue,
                                     objRuleMACValue, nextObjARPACLValue, &nextObjRuleIPValue,
                                     nextObjRuleMACValue);
  }
  else
  {
    ...
    do
    {
      owa.l7rc = usmDbArpAclRuleNextGet(objARPACLValue,
                                      ...
                                      objRuleMACValue, nextObjARPACLValue, &nextObjRuleIPValue,
                                      nextObjRuleMACValue);
     memcpy(objRuleMACValue, nextObjRuleMACValue, sizeof(nextObjRuleMACValue));
    }
...
xLibRC_t fpObjGet_SwitchingARPAclRule_RuleMAC (void *wap, void *bufp)
{
  ...
  if (owa.rc != XLIBRC_SUCCESS)
  {
    ...
    owa.l7rc = usmDbArpAclRuleGet (objARPACLValue, objRuleIPValue, nextObjRuleMACValue);
    if(owa.l7rc != L7_SUCCESS)
    {
      owa.l7rc = usmDbArpAclRuleNextGet(objARPACLValue,
                                        objRuleIPValue,
  ...
  FPOBJ_TRACE_NEW_KEY (bufp, nextObjRuleMACValue, owa.len);

  /* return the object value: RuleMAC */
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) nextObjRuleMACValue, strlen (nextObjRuleMACValue));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
...
xLibRC_t fpObjGet_SwitchingARPAclRule_ROWSTATUS (void *wap, void *bufp)
{
  ...
  owa.l7rc = usmDbArpAclRuleGet(keyARPACLValue,
                              keyRuleIPValue, keyRuleMACValue);
  if (owa.l7rc != L7_SUCCESS)
  {
