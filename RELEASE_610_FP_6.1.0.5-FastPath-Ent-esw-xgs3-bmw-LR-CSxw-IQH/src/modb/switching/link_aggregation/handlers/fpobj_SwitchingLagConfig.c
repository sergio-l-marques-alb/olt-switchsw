...
xLibRC_t fpObjSet_SwitchingLagConfig_IntfLagMember (void *wap, void *bufp)
{
  ...
  {
    if (usmDbDot3adMemberDeleteSet(L7_UNIT_CURRENT, tempVal, keyInterfaceValue) != L7_SUCCESS)
    { 
      owa.rc = XLIBRC_FAILURE;
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
  ...
  if (addFlag == XLIB_TRUE)
  {
    ...
    if (owa.l7rc == L7_SUCCESS)
    {
      owa.l7rc = usmDbDot3adMemberAddSet(L7_UNIT_CURRENT, objIntfLagMemberValue, keyInterfaceValue);
    }
