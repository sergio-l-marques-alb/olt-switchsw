...
xLibRC_t fpObjSet_routingswitchIPInterfaceConfig_netMask (void *wap, void *bufp)
{
  ...
  owa.l7rc = usmDbIpRtrIntfIPAddressSet(L7_UNIT_CURRENT, keyIfIndexValue, objIpAddrValue, objnetMaskValue);
  if (owa.l7rc != L7_SUCCESS)
  {
    switch (owa.l7rc)
    {
                                                           ...
                                                           void *bufp)
{
  ...
  if (usmDbIntfMaxIpMtuGet(keyIfIndexValue, &maxIpMtu) != L7_SUCCESS)
  {
    maxIpMtu = L7_L3_MAX_IP_MTU;
  }

  if(( objmtuValueValue >= L7_L3_MIN_IP_MTU ) && ( objmtuValueValue <= maxIpMtu ))
  {
    /* set the value in application */
...
* @return
*******************************************************************************/
xLibRC_t fpObjGet_routingswitchIPInterfaceConfig_netDirectBroadcasts (void *wap,
                                                                      void
                                                                      *bufp)
{
  fpObjWa_t kwa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
...
xLibRC_t fpObjGet_routingswitchIPInterfaceConfig_IsPrimaryConfigured (void *wap, void *bufp)
{
  ...
  xLibU32_t objIsPrimaryConfiguredValue;
  ...
  if (ipval != 0)
  {
    objIsPrimaryConfiguredValue = XLIB_TRUE;
  }
  else
  {
    objIsPrimaryConfiguredValue = XLIB_FALSE;
  }
