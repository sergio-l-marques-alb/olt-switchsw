                                                            ...
                                                            void *bufp)
{
  ...
  owa.rc = xLibBufDataGet (bufp,
                           (xLibU8_t *) objsysTransferFileNameLocalValue,
                           &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_EXIT (bufp, owa);
                                                            ...
                                                            void *bufp)
{
  ...
  if (!FPOBJ_CMP_STR256(activeImageName, USMDB_IMAGE1_NAME))
  {
     osapiStrncpySafe(objsysTransferBackupImageFileNameLocal, USMDB_IMAGE2_NAME, sizeof(objsysTransferBackupImageFileNameLocal));
  }
  else
  {
     osapiStrncpySafe(objsysTransferBackupImageFileNameLocal, USMDB_IMAGE1_NAME, sizeof(objsysTransferBackupImageFileNameLocal));
  }
...
xLibRC_t fpObjSet_baseSysTransfer_sysTransferHTTPDownStart (void *wap, void *bufp)
{
 ...
 if(usmDbTransferFileNameLocalGet(L7_UNIT_CURRENT,objsysTransferHTTPFileNameRemoteValue)!= L7_SUCCESS)
 {
  ...
  if (objsysTransferDownloadFileTypeValue == L7_FILE_TYPE_CODE)
  {
    ...
    {
#endif
      if (!FPOBJ_CMP_STR256(objsysTransferHTTPFileNameLocalValue, USMDB_IMAGE1_NAME))
      {
         /* Need to rename the file to image1 */
         owa.l7rc = usmDbHttpTransferFileRename(L7_UNIT_CURRENT, objsysTransferDownloadFileTypeValue, 
                                                                      objsysTransferHTTPFileNameRemoteValue, USMDB_IMAGE1_NAME);
         
      }
      else  if (!FPOBJ_CMP_STR256(objsysTransferHTTPFileNameLocalValue, USMDB_IMAGE2_NAME))
      {
         /* Need to rename the file to image2 */
         owa.l7rc = usmDbHttpTransferFileRename(L7_UNIT_CURRENT, objsysTransferDownloadFileTypeValue, 
                                                                      objsysTransferHTTPFileNameRemoteValue, USMDB_IMAGE2_NAME);
      }
#ifdef L7_TR069_PACKAGE
    }
    else
  ...
  #ifdef _L7_OS_ECOS_
   owa.l7rc =
    usmDbHttpTransferDownStartSet(L7_UNIT_CURRENT);
  #endif
  ...
  if (owa.l7rc == L7_SUCCESS)
  {
    owa.l7rc = usmDbHttpProcessTransferFile (L7_UNIT_CURRENT,objsysTransferDownloadFileTypeValue);
     /* Do any cleanup once the http download has finished. */
     usmDbHttpTransferFileCleanup(L7_UNIT_CURRENT);
  }
  else
  {
 ...
 if(objsysTransferDownloadFileTypeValue == L7_FILE_TYPE_CONFIG_SCRIPT)
 {
   if (!FPOBJ_CMP_STR256(objsysTransferHTTPFileNameLocalValue, L7_SCRIPTS_STARTUP_CONFIG_FILENAME))
   {
      owa.l7rc = usmDbHttpProcessConfigFile( L7_SCRIPTS_STARTUP_CONFIG_FILENAME );
   }
   else  if (!FPOBJ_CMP_STR256(objsysTransferHTTPFileNameLocalValue, L7_SCRIPTS_BACKUP_CONFIG_FILENAME))
   {
      owa.l7rc = usmDbHttpProcessConfigFile( L7_SCRIPTS_BACKUP_CONFIG_FILENAME );
   }
   else
   {
  ...
  owa.rc = XLIBRC_TRANSFER_SUCCESS;
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
}
/*******************************************************************************
 ...
 { switch (tempValue)  /* Transfer Status */
  {
    ...
    default:
         LOG_MSG("Unknown return code from tftp result" );
         break;
  }
 }
...
xLibRC_t fpObjGet_baseSysTransfer_sysTransferCompleteStatus(void *wap, void *bufp)
{
  ...
  L7_uint32 data = 0;
  int bytesTransfered = 0xFFFFFFFF; 
  xLibU32_t objsysTransferModeValue;
...
if (usmDbTransferInProgressGet (unit) == L7_TRUE)
  {
    if (direction == L7_TRANSFER_DIRECTION_DOWN)
    {
      ...
      {
  osapiSnprintf(msgBuffer, sizeof(msgBuffer), "Download Completed. %d bytes downloaded.<br>Image is being distributed to all units<br>Please wait.....", data);
  OSAPI_STRNCAT(objsysTransferResultValue,msgBuffer);
      }
      else
      {
  ...
  {
    usmDbTransferUploadResultCodeGet (unit, &direction);
    if (direction == L7_TRANSFER_CODE_SUCCESS)
    {
...
xLibRC_t fpObjSet_baseSysTransfer_sysTransferInProgress (void *wap, void *bufp)
{
  ...
  /* set the value in application */
  owa.l7rc = L7_SUCCESS;
  usmDbTransferInProgressSet (L7_UNIT_CURRENT, objsysTransferInProgressValue);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
                                                            ...
                                                            void *bufp)
{
  ...
  if (owa.rc == XLIBRC_SUCCESS)
  {
    if (objsysTransferUploadFileTypeValue == L7_FILE_TYPE_CONFIG_SCRIPT)
    {
      if( osapiStrncmp(objsysTransferScriptFileNameLocal, L7_SCRIPTS_TEMP_CONFIG_FILENAME, sizeof(L7_SCRIPTS_TEMP_CONFIG_FILENAME)) == 0 )
      {
      ...
      if( osapiStrncmp(objsysTransferScriptFileNameLocal, L7_SCRIPTS_STARTUP_CONFIG_FILENAME, sizeof(L7_SCRIPTS_STARTUP_CONFIG_FILENAME)) == 0 )
      {
        osapiStrncpy(objsysTransferScriptFileNameLocal,L7_SCRIPTS_STARTUP_CONFIG_FILENAME,15);
      }
      if( osapiStrncmp(objsysTransferScriptFileNameLocal, L7_SCRIPTS_BACKUP_CONFIG_FILENAME, sizeof(L7_SCRIPTS_BACKUP_CONFIG_FILENAME)) == 0 )
      {
