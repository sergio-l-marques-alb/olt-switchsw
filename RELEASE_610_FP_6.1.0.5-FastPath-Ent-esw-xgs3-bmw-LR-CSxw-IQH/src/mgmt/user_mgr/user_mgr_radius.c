                              ...
                              L7_BOOL *pChallengeFlag, L7_char8 *pChallengePhrase)
{
  ...
  if (userMgrRadiusRequestSend(pUserName, pPwdOrChallengeResponse, pState, &msg)
      == L7_FAILURE )
    return (L7_FAILURE);
...
L7_RC_t userMgrRadiusPhaseOneInit()
{
  ...
  do
  {
    ...
    {
      LOG_MSG("UserMgr: Unable to create msg queue\n");
      rc = (L7_FAILURE);
      break;
    }
    ...
    {
      LOG_MSG("Unable to create User Manager semaphore\n");
      rc = (L7_FAILURE);
      break;
    }
  } while ( 0 );
...
L7_RC_t userMgrRadiusPhaseTwoInit()
{
  ...
  {
    LOG_MSG("Unable to register User Manager for RADIUS response callback\n");
    rc = L7_FAILURE;
  }
