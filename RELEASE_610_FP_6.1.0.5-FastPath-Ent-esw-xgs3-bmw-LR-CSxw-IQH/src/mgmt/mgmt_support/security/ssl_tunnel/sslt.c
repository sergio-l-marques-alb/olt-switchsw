                                ...
                                L7_uint32 size)
{
  ...
  for (nwritten = L7_NULL;  nwritten < size;  nwritten += err)
  {
    ...
    if (err <= L7_NULL)
    {
      if (err < L7_NULL)
      {
        LOG_MSG("SSLT: Error on unsecure send\n");
      }
      break;
    }
  }
      ...
      switch(SSL_get_error(ssl, nwritten))
        {
          ...
          case SSL_ERROR_SSL:
            ...
            LOG_MSG("SSLT: SSL_ERROR_SSL on SSL_write\n");
          ...
          case SSL_ERROR_SYSCALL:
            ...
            /* Error, the SSL connection should be terminated */
            LOG_MSG("SSLT: SSL_ERROR_SYSCALL on SSL_write\n");
          ...
          default:
            LOG_MSG("SSLT: Unknown error on SSL_write\n");
