...
SSL_CTX *ssltContextCreate(void)
{
  ...
  do
  {
    ...
    if ((ssltServerCertFileGet(ssltGlobal.ssltCertificateNumber,
                               certFile) != L7_SUCCESS))
    {
      LOG_MSG("SSLT: Error getting certificate file for certficate number %u\n",
    ...
    if (cert == L7_NULLPTR)
    {
      LOG_MSG("SSLT: Error reading certificate file for certficate number %u\n",
              ssltGlobal.ssltCertificateNumber);
      break;
    }
    /* load into context */
    ...
    {
      L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSLT_COMPONENT_ID, "SSLT: Error loading private key from file. "
    ...
    if (!SSL_CTX_use_PrivateKey(ctx, pkey))
    {
      ...
      {
        LOG_MSG("SSLT: Did not find SSL PEM file %s%s, OK to continue\n", L7_SSLT_PEM_DIR, L7_SSLT_ROOT_PEM);
        break;
      }

      if (SSL_CTX_set_default_verify_paths(ctx) != 1)
      {
        LOG_MSG("SSLT: Error loading SSL PEM file and/or directory\n");
        break;
      }
  ...
  /* Set the supported protocols and options */
    ...
    {
      L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSLT_COMPONENT_ID, "SSLT: Error setting cipher list "
                                         "(no valid ciphers). Failed while setting cipher list.\n");
      break;
    }

    LOG_MSG("SSLT: Successfully loaded all required SSL PEM files\n");
    success = L7_TRUE;
...
static void ssltCertGenTask(L7_int32 numargs, L7_uint32 *argv )
{
  ...
  if (ssltServerCredentialsCreate(argv[0], argv[1],
                                  ...
                                  argv[2]) != L7_SUCCESS)
  {
    LOG_MSG("SSHD: ssltServerCredentialsCreate failed\n");
  }
                         ...
                         L7_uint32 days)
{
  ...
  {
      LOG_MSG("SSHD: Failed to create the ssltCertGenTask task\n");
      return L7_FAILURE;
  }
                                    ...
                                    L7_uint32 days)
{
  ...
  do
  {
    if (keyLength != 0)
    {
      ...
      LOG_MSG("SSLT: creating server RSA key pair...\n");
      rsa = RSA_generate_key(keyLength, RSA_F4, L7_NULLPTR, L7_NULLPTR);
      LOG_MSG("SSLT: verifying RSA key pair...\n");
      if ((rsa == L7_NULLPTR) || (!RSA_check_key(rsa)))
      {
        LOG_MSG("SSLT: ssltServerCredentialsCreate, RSA key generation failed\n");
        break;
      }
      pkey = EVP_PKEY_new();
      if (pkey == L7_NULLPTR)
      ...
      {
        LOG_MSG("SSLT: ssltServerCredentialsCreate, RSA key assignment failed\n");
        break;
      }
    ...
    } else
    {
      ...
      {
        osapiSemaGive(ssltCertSema);
        LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to read RSA key file\n");
        break;
      }
      osapiSemaGive(ssltCertSema);
    }
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to create subject name\n");
      break;
    }
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to assign certificate subject\n");
      break;
    }
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to create issuer name\n");
      break;
    }
    /* assign the issuer name */
    if (!X509_set_issuer_name(cert, issuer))
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to assign certificate issuer\n");
      break;
    }
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to set X509 version\n");
      break;
    }
    /* set the serial number to the certificate number */
    if (!ASN1_INTEGER_set(X509_get_serialNumber(cert), (long)number))
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to assign serial number\n");
      break;
    }
    /* set the public key, object does NOT take ownership of key object */
    if (!X509_set_pubkey(cert, pkey))
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to assign certificate public key\n");
      break;
    }
    /* set the valid dates */
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to set certificate valid days\n");
      break;
    }
    /* last step, self-sign with our private key */
    if (!X509_sign(cert, pkey, EVP_sha1()))
    {
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to sign certificate\n");
      break;
    }
    ...
    if (keyLength != 0)
    {
      ...
      {
        osapiSemaGive(ssltCertSema);
        LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to write RSA key file\n");
        break;
      }
      osapiSemaGive(ssltCertSema);
    }
    ...
    {
      osapiSemaGive(ssltCertSema);
      LOG_MSG("SSLT: ssltServerCredentialsCreate, failed to write server certificate file\n");
      break;
    }
    osapiSemaGive(ssltCertSema);
                                    ...
                                    L7_char8 *cert)
{
  ...
  do
  {
    ...
    if (certX509 == L7_NULLPTR)
    {
      LOG_MSG("SSLT: ssltServerCredentialsImport: failed to parse certificate\n");
      break;
    }
    ...
    if (ssltServerKeyRead(number, &pkey) != L7_SUCCESS)
    {
      LOG_MSG("SSLT: ssltServerCredentialsImport, failed to read RSA key file\n");
      break;
    }
    ...
    {
      LOG_MSG("SSLT: ssltServerCredentialsImport: failed to verify certficate against private key\n");
      break;
    }
    ...
    if (ssltServerCertFileGet(number, certFile) == L7_SUCCESS)
    {
      ...
      {
        LOG_MSG("SSLT: ssltServerCredentialsImport: failed to delete previous certificate\n");
      }
    }
    if (ssltServerCertWrite(number, certX509) != L7_SUCCESS)
    {
      LOG_MSG("SSLT: ssltServerCredentialsImport: failed to save certificate\n");
      break;
    }
