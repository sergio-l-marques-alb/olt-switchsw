                          ...
                          L7_uint32     length)
{
  ...
  do
  {
    ...
    if (type == SSHD_KEY_TYPE_RSA)
    {
      ...
      {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, RSA key generation failed\n");
        break;
      }
      /* assign RSA key pair */
      if (!EVP_PKEY_assign_RSA(pkey, rsa))
      {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, RSA key assignment failed\n");
        break;
      }
    ...
    } else if (type == SSHD_KEY_TYPE_DSA)
    {
      ...
      if (dsa == L7_NULLPTR)
      {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, DSA generate parameters failed\n");
        break;
      }
      if (DSA_generate_key(dsa) == 0)
      {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, DSA key generation failed\n");
        break;
      }
      /* assign DSA key pair */
      if (!EVP_PKEY_assign_DSA(pkey, dsa))
      {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, DSA key assignment failed\n");
        break;
      }
    ...
    {
      LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate, failed to write key file.\n");
