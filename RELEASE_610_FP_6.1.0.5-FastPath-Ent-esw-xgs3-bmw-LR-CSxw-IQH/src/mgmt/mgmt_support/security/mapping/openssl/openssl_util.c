...
L7_RC_t opensslInit(void)
{
  ...
  if ((opensslSemaSetup() != L7_SUCCESS) || !SSL_library_init())
  {
    LOG_MSG("OPENSSL: OpenSSL initialization failed!\n");
                                                                ...
                                                                int line)
{
  ...
  {
    LOG_MSG("OPENSSL: Could not delete the dynamic semaphore!\n");
...
L7_RC_t opensslSemaSetup(void)
{
  ...
  {
    LOG_MSG("OPENSSL: Could not allocate space for the mutex semaphore!\n");
...
void opensslSemaCleanup(void)
{
  ...
  {
    /* Nothing to do */
    LOG_MSG("OPENSSL: Did not need to clean up the SSL semaphores!\n");
  ...
  for (i = L7_NULL; i < CRYPTO_num_locks(); i++)
  {
    ...
    {
	  LOG_MSG("OPENSSL: Could not delete the SSL semaphores!\n");
    }
  }
...
opensslDiffieHellmanParamsRead(L7_char8 *file, DH **dh)
{
    ...
    if (fp == L7_NULLPTR)
    {
        LOG_MSG("OPENSSL: opensslFileOpen could not open %s\n", file);
        return L7_FAILURE;
    }
    ...
    if ((*dh) == L7_NULLPTR)
    {
        LOG_MSG("OPENSSL: PEM DH params not read\n");
