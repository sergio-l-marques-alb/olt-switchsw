...
const char *commandShowIpInterface(EwsContext ewsContext, uintf argc, const char * * argv, uintf index)
{
  ...
  if (usmDbIfAdminStateGet(unit, intIface, &val) == L7_SUCCESS)
  {
	...
	case L7_DIAG_DISABLE:
      sprintf(stat, pStrInfo_common_DiagDsbl_1);
	  break;
    case L7_ENABLE:
  ...
  if (usmDbIfPhysAddressGet(unit, intIface, strMacAddr) == L7_SUCCESS)
  {
    ...
    if(flagPrint == L7_TRUE)
    {
      ...
      else
      {
	    ...
	    if(routeTable.protocol !=RTO_STATIC )
	    {
               ...
               osapiConvertRawUpTime((osapiUpTimeRaw()-routeTable.updateTime),&routeUpdateTime);
...
static L7_uint32 interface;
  ...
  ewsTelnetWriteAddBlanks (1, 0, 52, 4, L7_NULLPTR, ewsContext,pStrInfo_routing_NetdirMulti);
  ...
  while (interface)
  {
       ...
       )
    {
      ...
      {
        osapiSnprintf(stat,sizeof(stat),"%-9.8s",strUtilEnableDisableGet(val, pStrInfo_common_Blank));
      }
      else
      {
      ...
      memset (stat, 0, sizeof(stat));
      if (usmDbIpMcastsFwdModeGet(unit, interface, &val) == L7_SUCCESS)
      {
        osapiSnprintf(stat,sizeof(stat),"%-9.8s",strUtilEnableDisableGet(val, pStrInfo_common_Blank));
      }
      else
...
const char *commandShowIpIrdp(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  while ((rc == L7_SUCCESS) && (count < CLI_MAX_SCROLL_LINES-6))
  {
        ...
        (usmDbRtrDiscIsValidIntf(unit, interface) == L7_TRUE))
    {
      ...
      {
        osapiSnprintf(buf, sizeof(buf), "%u", time);
        osapiSnprintf(stat, sizeof(stat), "%-10.8s", buf);
      }
      else
      {
        osapiSnprintf(stat, sizeof(stat), "%-10.8s", pStrErr_common_Err);
      }
      ewsTelnetWrite(ewsContext, stat);
...
const L7_char8 *commandShowIpVlan(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  {
    if (usmDbIfIntfAddressGet(intIfNum, L7_NULL, macAddr) == L7_SUCCESS)
    {
...
const L7_char8 *commandShowVlanInternal(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  static L7_uint32 intIfNum;         /* internal interface number of corresponding routing interface */
  static L7_RC_t rc;
  L7_uint32 i;
  L7_char8 stat[L7_CLI_MAX_STRING_LENGTH];
  L7_char8  cmdBuf[L7_CLI_MAX_STRING_LENGTH];
  ...
  L7_uint32 numArg;        
  L7_int32  count;
  L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];          /* name of routing interface */
  ...
  osapiSnprintf(stat, L7_CLI_MAX_STRING_LENGTH, "\r\nBase VLAN ID:  %u", usmDbBaseInternalVlanIdGet());
  ewsTelnetWrite(ewsContext, stat);

  osapiSnprintf(stat, L7_CLI_MAX_STRING_LENGTH, "\r\nAllocation policy:  %s\r\n", 
                (usmDbInternalVlanPolicyGet() == DOT1Q_INT_VLAN_POLICY_ASCENDING) ? "Ascending" : "Descending");
  ewsTelnetWrite(ewsContext, stat);
  ...
  {
    rc = usmDbInternalVlanFindFirst(&vlanId, &intIfNum);
    if (rc != L7_SUCCESS)
    {
      return cliSyntaxReturnPrompt (ewsContext, "");
    }
  ...
  ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, "VLAN ID  Routing Interface");
  ewsTelnetWrite(ewsContext,"\r\n-------  --------------------------------");
  ...
  while ((rc == L7_SUCCESS) && (count < CLI_MAX_SCROLL_LINES-6))
  {
    ...
    /* print routing interface name */
    rc = usmDbIfNameGet(unit, intIfNum, ifName);
    if (rc == L7_SUCCESS)
    {
      ewsTelnetPrintf (ewsContext, "%-33s ", ifName);
    }
    else 
    {
     sprintf(stat,"%-33s ", "");
    ...
    rc = usmDbInternalVlanFindNext(vlanId, &vlanId, &intIfNum);
    if (rc != L7_SUCCESS)
    {
      ewsCliDepth(ewsContext, cliPrevDepthGet(), argv);
      return cliSyntaxReturnPrompt (ewsContext, "");
