                                           ...
                                           L7_uint32 index)
{
  ...
  if (vlanTableShow == L7_TRUE)
  {
    ...
    for (; tempVid <= L7_DOT1Q_MAX_VLAN_ID; tempVid++)
    {
  ...
  if (vlanStatusShow == L7_TRUE)
  {
    ...
    for (; tempVid <= L7_DOT1Q_MAX_VLAN_ID; tempVid++)
    {
      ...
      if ((rc == L7_SUCCESS && val == L7_ENABLE) || loopFlag == L7_FALSE)
      {
        ...
        // PTin added
        /* Querier MRouter Vid */
        if (usmDbSnoopQuerierVlanXlateGet(tempVid, &xlateVid) == L7_SUCCESS)
        {
          cliFormat(ewsContext, pStrInfo_switching_SnoopQuerierVlanXlate);
          ewsTelnetPrintf (ewsContext, "%u", xlateVid);
        }
