...
L7_RC_t cliRunningConfigProtocolInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if (usmDbPbVlanGroupGetFirst(unit, &groupID) == L7_SUCCESS)
  {
    ...
    do
    {
      ...
      {
        osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, buf, sizeof(buf), pStrInfo_base_VlanProtoGrp_2, group_name);
    ...
    if (usmDbPbVlanIsValidIntf(unit, interface) == L7_TRUE)
    {
      ...
      do
      {
        ...
        while (usmDbPbVlanGroupPortGetNext(unit, groupID, intIfNum, &intIfNum) == L7_SUCCESS)
        {
          ...
          {
            rc1 = L7_SUCCESS;
            osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_ProtoVlanGrp, groupID);
            EWSWRITEBUFFER(ewsContext,stat);
          }
        }
      } while (usmDbPbVlanGroupGetNext(unit, groupID, &nextGroupID) == L7_SUCCESS);
