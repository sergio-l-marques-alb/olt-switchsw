    ...
    L7_char8 * fileName, L7_uint32 fileNameSize)
{
...
#ifdef L7_CHASSIS
  L7_uint32 intSlot;
  len = strlen(pStrInfo_base_Slot_4);
  pBuf = strstr(str,pStrInfo_base_Slot_4);
#else
  ...
  else
  {
    ...
    *unit = (L7_uint32)(*(unitNum) - '0');
    genericUnit = *unit;
#ifdef L7_CHASSIS
    if (usmDbSlotMapUSIntUnitSlotGet(*unit, &genericUnit, &intSlot) != L7_SUCCESS)
    {
      return L7_FAILURE;
    }
#endif
  ...
  /* copy the file name */
   ...
   if ( (strcmp(pBuf, USMDB_IMAGE1_NAME) == 0) || (strcmp(pBuf, USMDB_IMAGE2_NAME) == 0))
   {
...
L7_char8 urlHelpStr[4*L7_CLI_MAX_STRING_LENGTH];
  ...
  else if(usmDbIsAnImage(unit, (L7_char8 *)(argv[index+1])) == L7_TRUE)
  {
    /* upload */
    ...
    if ( (strcmp(unitUrlStr, USMDB_IMAGE1_NAME) == 0) ||
        (strcmp(unitUrlStr, USMDB_IMAGE2_NAME) == 0))
    {
      ...
      rc = usmDbFileCopy(unit, localFileName, unitUrlStr,L7_TRUE);
      if(rc != L7_SUCCESS)
      {
...
#else
        if (strstr(&unitUrlStr[0], pStrInfo_base_Slot_2) != NULL)
#endif
        {
...
#else
            sprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, buf, pStrInfo_base_CopyingToOnAllSlots, localFileName, fileName);
#endif
          ...
          else
          {
            ...
            {
              return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, pStrInfo_base_CopyOperationFailed);
            }
            sprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, buf, pStrInfo_base_CopyingToOnSlot, localFileName, fileName, unit);
            unit = chassisIntUnit;
#endif
          }
...
#ifdef L7_CHASSIS
  else if( (strcmp( argv[index+1], pStrInfo_common_StandbyImage1) == 0) || (strcmp( argv[index+1], pStrInfo_base_StandbyImage2) == 0))
  {
    commandCopyImage(ewsContext,argc, argv, index);
    return cliPrompt(ewsContext);
  }
#endif
  else
  {
    ...
    {
      OSAPI_STRNCPY_SAFE(localFileName, pStrInfo_common_Image1);
      rc = usmDbTransferUploadFileTypeSet(U_IDX, L7_FILE_TYPE_CODE);
      OSAPI_STRNCPY_SAFE(urlStr, argv[index+1]);
      setFileDatatype(pStrInfo_base_Code_3);
    }
    else if ((strcmp( argv[index+2], pStrInfo_common_Image1 ) == 0) ||
        (strcmp( argv[index+2], pStrInfo_common_Image2 ) == 0) )
    {
                              ...
                              L7_uint32 index, L7_BOOL archive, L7_char8 *host)
{
  ...
  else
  {
    ...
    else if(val == L7_FILE_TYPE_CODE)
    {
      ...
      dimActiveImageFileNameGet(activeFileName);
  ...
  if (usmDbTransferInProgressGet(unit))
  {
    ...
    else
    {
          ...
          (transMode == L7_TRANSFER_SCP) )
      {
        ...
        {
          if (transferFlag == L7_TRUE)
          {
            if (upload == L7_TRUE && transType == L7_TRANSFER_CODE_SUCCESS)
            {
              memset(perc_val_buf, 0x00, sizeof(perc_val_buf));
              perc_val = usmDbTransferCompleteStatus();
              osapiSnprintf(perc_val_buf, sizeof(perc_val_buf), "\r%d bytes transferred   ", perc_val);
          ...
          {
            usmDbTransferDownloadFileTypeGet(unit, &transType);
            if ((transType == L7_FILE_TYPE_CONFIG_SCRIPT) &&
                (transType == L7_TRANSFER_CODE_SUCCESS))
            {
              memset(perc_val_buf, 0x00, sizeof(perc_val_buf));
              perc_val = usmDbTransferCompleteStatus();
              osapiSnprintf(perc_val_buf, sizeof(perc_val_buf), "\r%d bytes transferred   ", perc_val);
      ...
      if (val >= L7_TRANSFER_FAILED)
      {
        if(upload == L7_FALSE) /*  transfer is a download*/
        {
          ...
          if(val == L7_TRANSFER_SUCCESS && upload == L7_FALSE)
          {
            rc = usmDbTransferDownloadFileTypeGet(unit, &val);
