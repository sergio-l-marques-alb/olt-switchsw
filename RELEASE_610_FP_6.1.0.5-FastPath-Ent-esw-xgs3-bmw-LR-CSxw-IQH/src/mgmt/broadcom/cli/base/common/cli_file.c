                            ...
                            const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (strcmp( argv[index+1], pStrInfo_base_BootSys ) == 0)
  {
...
#else
      cliConvertTo32BitUnsignedInteger(buf, &logSlot);
      usmDbSlotMapPLPhysicalSlotGet(logSlot, &phySlot);
      usmDbSlotMapUSUnitNumGet(phySlot, &unit);
#endif
    ...
    if((strcmp(argv[index+2],USMDB_IMAGE1_NAME)!=0) && (strcmp(argv[index+2],USMDB_IMAGE2_NAME) != 0))
    {
    ...
    rc = usmDbImageActivate(unit, fileName);
                                ...
                                const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if(firstTime == L7_TRUE)
  {
    ...
    osapiSnprintf(buf, sizeof(buf), "\r\n %4s  %10s  %10s  %17s  %17s \r\n", pStrInfo_base_Unit_3,
                  USMDB_IMAGE1_NAME, USMDB_IMAGE2_NAME, pStrInfo_base_BootCurrentActive, pStrInfo_base_BootNextActive);
#else
    osapiSnprintf(buf, sizeof(buf), "\r\n %4s  %10s  %10s  %17s  %17s \r\n", pStrInfo_base_Slot_2,
                  USMDB_IMAGE1_NAME, USMDB_IMAGE2_NAME, pStrInfo_base_BootCurrentActive, pStrInfo_base_BootNextActive);
#endif
...
/* j what about j ??*/
  ...
  {
    if (usmDbImageVersionGet(unitAry[j], USMDB_IMAGE1_NAME, &image1Version[0])
        != L7_SUCCESS)
    {
      OSAPI_STRNCPY_SAFE(image1Version, pStrInfo_base_None);
    }

    if (usmDbImageVersionGet(unitAry[j], USMDB_IMAGE2_NAME, &image2Version[0])
        != L7_SUCCESS)
    {
...
#else
    (void)usmDbSlotMapUSPhysicalSlotGet(unitAry[j],&phySlot);
    (void)usmDbSlotMapPLLogicalSlotGet(phySlot,&logSlot);
    osapiSnprintf(buf, sizeof(buf), "\r\n %4d  %10s  %10s  %17s  %17s ", logSlot, image1Version,
                  image2Version, activeImage, activatedImage);
#endif
                                 ...
                                 const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if ( (strcmp (argv[index+1], USMDB_IMAGE1_NAME ) != 0) &&
      (strcmp (argv[index+1], USMDB_IMAGE2_NAME ) != 0) )
  {
  ...
  rc = usmDbImageDescrSet((L7_char8 *)argv[index+1], (L7_char8 *)argv[index+2]);
