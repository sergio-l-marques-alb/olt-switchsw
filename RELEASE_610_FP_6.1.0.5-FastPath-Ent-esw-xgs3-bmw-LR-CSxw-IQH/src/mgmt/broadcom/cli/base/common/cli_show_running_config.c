...
L7_RC_t cliRunningConfigSysInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if ( usmDb1213SysDescrSingleLineGet(unit, stat)  == L7_SUCCESS)
  {
...
L7_RC_t cliRunningConfigSnmpInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  for ( count=0; count<L7_MAX_SNMP_COMM; count++ )
  {
    if (usmDbSnmpCommunityNameGet(unit, count, snmpCommunity) == L7_SUCCESS )
    {
      ...
      if (strcmp(snmpCommunity, FD_snmpComm[count].agentCommunityName ) != 0)
      {
          if (strcmp(snmpCommunity, "") != 0)
          {
    ...
    if (usmDbSnmpCommunityStatusGet(unit, count, &val) == L7_SUCCESS &&
        val != L7_SNMP_COMMUNITY_STATUS_DELETE)
    {
      if (usmDbSnmpCommunityIpAddrGet(unit, count, &val) == L7_SUCCESS )
      {
        if ((val !=  FD_snmpComm[count].agentCommunityClientIpAddr)||
      ...
      if (usmDbSnmpCommunityIpMaskGet(unit, count, &val) == L7_SUCCESS )
      {
        if ((val !=  FD_snmpComm[count].agentCommunityClientIpMask)||
            (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
        {
          memset(stat, 0x00, sizeof(stat));
          if (usmDbInetNtoa(val, stat) == L7_SUCCESS)
      ...
      if (usmDbSnmpCommunityAccessLevelGet(unit, count, &val) == L7_SUCCESS )
      {
        if ((val !=  FD_snmpComm[count].agentCommunityAccessLevel)||
            (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
        {
          switch ( val )
          {
    ...
    if (usmDbSnmpCommunityStatusGet(unit, count, &val) == L7_SUCCESS )
    {
      ...
      if ((val !=  FD_snmpComm[count].agentCommunityStatus) ||
          (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
        if (strcmp(snmpCommunity, pStrInfo_common_EmptyString) != 0)
        {
          switch ( val )       /* val = "Invalid","Enable","Disable","Delete" */
          {
            ...
            case L7_SNMP_COMMUNITY_STATUS_INVALID:
              osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, buf, sizeof(buf), pStrInfo_base_NoSnmpSrvrCommunityMode, snmpCommunity);
              EWSWRITEBUFFER(ewsContext, buf);
              break;
            case L7_SNMP_COMMUNITY_STATUS_DELETE:
              osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, buf, sizeof(buf), pStrInfo_base_NoSnmpSrvrCommunity, snmpCommunity);
              EWSWRITEBUFFER(ewsContext, buf);
              break;
            default:
              /* Do nothing */
  ...
  for ( count=0; count<L7_MAX_SNMP_COMM; count++ )
  {
    if (usmDbTrapManagerCommIpGet(unit, count, snmpCommunity) == L7_SUCCESS )
    {
          ...
          (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
        if (strcmp(snmpCommunity,pStrInfo_common_EmptyString)!=0)
        {
          osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, buf, sizeof(buf), pStrInfo_base_Snmptrap);
            ...
            if ((( val !=  FD_snmpTrapMgr[count].agentTrapMgrStatus )|| EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT) && (val != L7_ENABLE))
            {
              switch ( val )   /* val = "Invalid","Enable","Disable","Delete" */
              {
                case L7_SNMP_TRAP_MGR_STATUS_VALID:
                  OSAPI_STRNCPY_SAFE_ADD_BLANKS (1, 0, 0, 1, L7_NULLPTR, buf,pStrInfo_base_SnmptrapMode);
                  OSAPI_STRNCAT(buf," ");
                  OSAPI_STRNCAT(buf,snmpCommunity);
                  OSAPI_STRNCAT(buf," ");
                  OSAPI_STRNCAT(buf,stat);
...
L7_RC_t cliRunningConfigUsersSnmpv3Info(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  for (; tempInt < L7_MAX_LOGINS; tempInt++)
  {
    ...
    if (strcmp(buf, FD_CLI_WEB_DEFAULT_LOGIN_NAME) != 0)
    {
      ...
      {
        if(!(((strcmp(buf,pStrInfo_base_LoginUsrId) == 0) || (strcmp(buf,pStrInfo_base_Guest) == 0)) && (strlen(password) == 0)))
        {
          ...
          osapiStrncat(stat, " encrypted", sizeof(stat)-strlen(stat));
          EWSWRITEBUFFER(ewsContext,stat);
        }
      }
      memset(stat, 0x00, sizeof(stat));
      if (snmpUserSupported == L7_TRUE)
      {
            ...
            (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
        {
           if((strcmp(buf,pStrInfo_base_LoginUsrId) == 0) && (accessLevel != L7_AGENT_COMM_ACCESS_LEVEL_READ_WRITE))
            sprintf(stat,"\r\nusername snmpv3 accessmode \"%s\" readonly", buf);
          else if((strcmp(buf,pStrInfo_base_LoginUsrId) != 0) && (accessLevel != L7_AGENT_COMM_ACCESS_LEVEL_READ_ONLY))
            sprintf(stat,"\r\nusername snmpv3 accessmode \"%s\" readwrite", buf);
          EWSWRITEBUFFER(ewsContext,stat);
        }
      }

      /*  if ((strcmp(buf,"admin") != 0) && (strcmp(buf,"guest") != 0))
          {*/
          ...
          switch (auth)
            {
              ...
              sprintf(stat,"\r\nusername snmpv3 authentication \"%s\" md5", buf);  /* MD5 */
                break;
              case L7_SNMP_USER_AUTH_PROTO_HMACSHA:
              sprintf(stat,"\r\nusername snmpv3 authentication \"%s\" sha", buf);  /* SHA */
                break;
              case L7_SNMP_USER_AUTH_PROTO_NONE:
              sprintf(stat,"\r\nusername snmpv3 authentication \"%s\" none", buf); /* None */
              break;
            default:
              break;
          }
          ...
          switch (encrypt)
            {
              ...
              sprintf(stat,"\r\nusername snmpv3 encryption \"%s\" des",buf);
              EWSWRITEBUFFER(ewsContext,stat);
...
L7_RC_t cliRunningConfigNetworkInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  {
    if ((usmDbAgentIpIfAddressGet(unit, &val) == L7_SUCCESS) &&
        (val != FD_DHCPS_DEFAULT_NETWORKIP_ADDR))
    {
      memset (buf, 0, sizeof(buf));
      if (usmDbInetNtoa(val, buf) == L7_SUCCESS)
      {
        ...
        if (usmDbAgentIpIfNetMaskGet(unit, &val) == L7_SUCCESS)
        {
          memset (buf, 0, sizeof(buf));
          if (usmDbInetNtoa(val, buf) == L7_SUCCESS)
          {
        ...
        if (usmDbAgentIpIfDefaultRouterGet(unit, &val) == L7_SUCCESS)
        {
          memset (buf, 0, sizeof(buf));
          if (usmDbInetNtoa(val, buf) == L7_SUCCESS)
          {
...
L7_RC_t cliRunningConfigServicePortInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if (L7_SYSCONFIG_MODE_NONE == val)
  {
    if ((usmDbServicePortIPAddrGet(unit, &val) == L7_SUCCESS )&&
        ((val != FD_DHCPS_DEFAULT_NETWORKIP_ADDR)||(EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT)))
    {
      if (usmDbInetNtoa(val, buf) == L7_SUCCESS )
      {
        ...
        memset (buf, 0, sizeof(buf));
        if (usmDbServicePortNetMaskGet(unit, &val) == L7_SUCCESS )
        {
          if (usmDbInetNtoa(val, buf) == L7_SUCCESS )
          {
            OSAPI_STRNCAT(stat, buf);
        ...
        memset (buf, 0, sizeof(buf));
        if (usmDbServicePortGatewayGet(unit, &val) == L7_SUCCESS )
        {
          if (usmDbInetNtoa(val, buf) == L7_SUCCESS )
          {
            OSAPI_STRNCAT(stat, buf);
...
L7_RC_t cliRunningConfigRadiusInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if( usmDbRadiusServerFirstIPHostNameGet(unit, hostAddr, &type) == L7_SUCCESS )
  {
    do
    {
  ...
  if( usmDbRadiusAcctServerFirstIPHostNameGet(unit, 
                     hostAddr, &type) == L7_SUCCESS )
  {
    do
    {
...
L7_RC_t cliRunningPhysicalInterfaceInfo(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
    ...
    if (usmDbIfTypeGet(unit, interface, &val) == L7_SUCCESS)
    {
      ...
      if (usmDbIfAutoNegAdminStatusGet(unit, interface, &val1) == L7_SUCCESS)
      {
        if (val1 != FD_NIM_AUTONEG_ENABLE) /* If not the default value. */
        {
          ...
          if ((usmDbIfDefaultSpeedGet(unit, interface, &val) == L7_SUCCESS)&&
              ((val != portFactoryDefaultSpeed)|| (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT)))
          {
            switch ( val )
            {
              ...
              case L7_PORTCTRL_PORTSPEED_FULL_2P5FX:
                osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_Speed2500Full);
                break;
              case L7_PORTCTRL_PORTSPEED_FULL_1000SX:
                osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_Speed1000Full);
                break;
      ...
      usmDbSnoopIsValidIntf (unit, interface) == L7_TRUE)
  {
          ...
          L7_IGMP_SNOOPING_FEATURE_SUPPORTED) == L7_TRUE)
    {
      ...
      {
        if ((usmDbsnoopIntfApiVlanStaticMcastRtrGet (unit, interface, pvid, &vlanMtrMode, family) ==
              L7_SUCCESS) && (vlanMtrMode != L7_DISABLE))
        {
          if (vlanMtrMode == L7_ENABLE)
          {
            osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, stat, sizeof (stat), pStrInfo_base_SetIgmpMrouter, pvid);
            EWSWRITEBUFFER(ewsContext, stat);
          }
        }
      }
  ...
  if ((usmDbDot1sIsValidIntf(unit, interface) == L7_TRUE))
  {
    ...
    {
      cliShowCmdEnable(ewsContext,boolVal,FD_DOT1S_PORT_MODE,pStrInfo_base_SpanTreeEdgeport);
    }
    ...
    if(usmDbDot1sIntfBpduFloodGet(unit, interface, &boolVal) == L7_SUCCESS )
    {
      cliShowCmdTrue(ewsContext, boolVal, FD_DOT1S_BPDUFLOOD, pStrInfo_base_SpanTreeBpduflood);
    }
    ...
    do
    {
      ...
      if (usmDbDot1sMstiPortPriorityGet(unit, mstid, interface, &val) == L7_SUCCESS )
      {
        ...
        cliShowCmdInt(ewsContext, val, FD_DOT1S_PORT_PRIORITY, stat);
      }
...
L7_RC_t cliRunningSNTPInfo(EwsContext ewsContext, L7_uint32 unit)
{
      ...
      == L7_SUCCESS)
  {
    ...
    switch (status)
    {
      case L7_SNTP_SERVER_STATUS_INVALID:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsInvalid);
        break;
      case L7_SNTP_SERVER_STATUS_ACTIVE:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsActive);
        break;
      case L7_SNTP_SERVER_STATUS_NOT_IN_SERVICE:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsNotInService);
        break;
      case L7_SNTP_SERVER_STATUS_NOT_READY:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsNotReady);
        break;
      case L7_SNTP_SERVER_STATUS_CREATE_AND_GO:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsCreateAndGo);
        break;
      case L7_SNTP_SERVER_STATUS_CREATE_AND_WAIT:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsCreateAndWait);
        break;
      case L7_SNTP_SERVER_STATUS_DESTROY:
        osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, stat, sizeof(stat), pStrInfo_base_SntpSrvrStatusIsDestroy);
        break;
    }
    ...
    rc = usmDbSntpServerTableIpAddressGet(unit, bufIndex, valString);
    ...
    if (rc == L7_SUCCESS)
    {
      ...
      usmDbSntpServerTablePriorityGet(unit, bufIndex, &val1);
      usmDbSntpServerTableVersionGet(unit, bufIndex, &val2);
      usmDbSntpServerTablePortGet(unit, bufIndex, &val3);

      if ((val1 != FD_SNTP_DEFAULT_SERVER_PRIORITY) || (val2 != FD_SNTP_DEFAULT_VERSION) || (val3 != FD_SNTP_DEFAULT_SERVER_PORT))
      {
        show = L7_TRUE;
      }

      rc = usmDbSntpServerTablePriorityGet(unit, bufIndex, &val16);
      if ((rc == L7_SUCCESS)&&
          ((val16 != FD_SNTP_DEFAULT_SERVER_PRIORITY) || (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT) || (show == L7_TRUE)))
      {

        osapiSnprintf(buf, sizeof(buf), "%d ", val16);
        OSAPI_STRNCAT(stat, buf);

        rc = usmDbSntpServerTableVersionGet(unit, bufIndex, &val16);

        if ((rc == L7_SUCCESS)&&
            ((val16 != FD_SNTP_DEFAULT_VERSION) || (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT) || (show == L7_TRUE)))
        {

          osapiSnprintf(buf, sizeof(buf), "%d ", val16);
          OSAPI_STRNCAT(stat, buf);

          rc = usmDbSntpServerTablePortGet(unit, bufIndex, &val16);

          if ((rc == L7_SUCCESS) &&
              ((val16 != FD_SNTP_DEFAULT_SERVER_PORT) || (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT) || (show == L7_TRUE)))
          {
            osapiSnprintf(buf, sizeof(buf), "%d ", val16);
            OSAPI_STRNCAT(stat, buf);
...
L7_RC_t cliRunningSyslogInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if (( usmDbLogConsoleAdminStatusGet(unit, &val) == L7_SUCCESS ) &&
      (val != FD_LOG_DEFAULT_CONSOLE_ADMIN_STATUS || EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT  ))
  {
    switch ( val )
    {
...
const L7_char8 *cliShowRunningConfig(EwsContext ewsContext, const L7_char8 * * argv, L7_uint32 numArg, boolean chkFile)
{
  L7_int32 argOne=2;
  ...
  if (ewsContext->showRunningOutputFlag == L7_WRITE_CONFIG_SCRIPT)
  {
    ...
    if (chkFile && checkConfigScriptPresent(scrFileName) == L7_SUCCESS)
    {
      ...
      else
      {
        ...
        if ( numArg == 2 )
        {
          osapiSnprintf(strCurrentCommand, sizeof(strCurrentCommand), "%s %s \"%s\"", pStrErr_base_ShowRunningCfgScript_1, strMode1, scrFileName);
        }
        else
        {
  ...
  else
  {
   ...
   if (ewsContext->showRunningOutputFlag == L7_WRITE_CONFIG_SCRIPT)
   {
    ...
    /* et the number of lines for page. if it is 0, pagination not required */
    cliSyntaxBottom( ewsContext );
...
L7_RC_t cliShowRunningConfigExecute(EwsContext ewsContext)
{
  ...
  {
    memset (strUserName, 0, sizeof(strUserName));
    rc = usmDbPortUserGetFirst(unit, intf, strUserName);
    i = 0;
    while (i < L7_MAX_LOGINS)
    {
      ...
      else
      {
        ...
        memset (strUserName, 0, sizeof(strUserName));
        rc = usmDbPortUserGetNext(unit, intf, strUserNamePrev, strUserName);
      }
      i++;
    }
    rc = usmDbValidIntIfNumNext(intf, &intf);
