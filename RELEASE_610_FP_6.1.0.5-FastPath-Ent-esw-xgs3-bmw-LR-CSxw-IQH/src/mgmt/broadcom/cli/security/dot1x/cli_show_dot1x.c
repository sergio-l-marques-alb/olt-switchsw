                                 ...
                                 L7_uint32 index)
{
  ...
  if (numArgs == 0)
  {
    ...
    if (mode == L7_ENABLE)
    {
      sprintf( buf, pStrInfo_common_Enbld);
    }                                /* Enabled */
    else if (mode == L7_DISABLE)
    {
      sprintf( buf, pStrInfo_common_Dsbld);
    }                                   /* Disabled */
    else
    {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error,  ewsContext, pStrErr_security_Dot1xShowAdminMode);
  ...
  else if (numArgs == 2)
  {
    if (strcmp(argv[index+argToken],pStrInfo_common_Dot1xShowSummary) == 0)
    {
      ...
      while ( interface != 0 )
      {
        ...
        {
          sprintf(stat, cliDisplayInterfaceHelp(u, s, p));
          ...
          if ( rc  == L7_SUCCESS )
          {
            ...
            case L7_DOT1X_PORT_AUTO:
              sprintf(stat,"%-19s",pStrInfo_common_Auto2);    /* auto */
              break;
            case L7_DOT1X_PORT_FORCE_AUTHORIZED:
              sprintf(stat,"%-19s",pStrInfo_security_ForceAuthorizedDot1x);    /* force-authorized */
              break;
            case L7_DOT1X_PORT_FORCE_UNAUTHORIZED:
              sprintf(stat,"%-19s",pStrInfo_security_ForceUnAuthorizedDot1x);    /* force-unauthorized */
              break;
            case L7_DOT1X_PORT_AUTO_MAC_BASED:
              sprintf(stat,"%-19s",pStrInfo_security_MacBasedDot1x);    /* mac-based */
              break;
            default:
              break;
            }
          ...
          if ( rc == L7_SUCCESS )
          {
            ...
            case L7_DOT1X_PORT_AUTO:
              ...
              break;
            case L7_DOT1X_PORT_FORCE_AUTHORIZED:
              sprintf(stat,"%-19s",pStrInfo_security_ForceAuthorized_1);  /* forceauthorized */
              break;
            case L7_DOT1X_PORT_FORCE_UNAUTHORIZED:
              sprintf(stat,"%-19s",pStrInfo_security_ForceUnAuthorized_1);  /* forceunauthorized */
              break;
            case L7_DOT1X_PORT_AUTO_MAC_BASED:
              sprintf(stat,"%-19s",pStrInfo_security_MacBasedDot1x);    /* mac-based */
              break;
            default:
              sprintf(stat,"%-19s",pStrInfo_common_NotApplicable);  /* N/A */
              break;
            }
            ewsTelnetWrite(ewsContext,stat);
          }
          ...
          if (cap & L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
          {
            ...
            if ( usmDbDot1xPortReAuthEnabledGet(unit, interface, &mode) == L7_SUCCESS )
            {
              if (mode == L7_TRUE)
              {
                sprintf( stat, "%-17s", pStrInfo_common_True);
              }                                        /* true */
              else if (mode == L7_FALSE)
              {
                sprintf( stat, "%-17s", pStrInfo_common_False);
              }                                         /* false */
              ewsTelnetWrite(ewsContext,stat);
            }
            else
         ...
         {
           sprintf(stat,"%-17s",pStrInfo_common_NotApplicable);  /* N/A */
           ewsTelnetWrite(ewsContext,stat);
         }
         if (cap & L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
         {
         ...
         else
         {
          ...
          if (rc  == L7_SUCCESS)
          {
            ...
            {
              sprintf( stat, "%-14s", pStrInfo_security_Authorized);
            }                                              /* "Authorized" */
            else if (val == L7_DOT1X_PORT_STATUS_UNAUTHORIZED)
            {
              sprintf( stat, "%-14s", pStrInfo_common_UnAuthorized);
            }                                                /* "Unauthorized */
            else
            {
              sprintf( stat, "%-14s", pStrInfo_common_NotApplicable);
            }                                    /* N/A */
            ewsTelnetWrite(ewsContext,stat);
          }
          else
    ...
    else if (strcmp(argv[index+argToken],pStrInfo_common_IgmpProxyGrpsDetail) == 0)
    {
     ...
     /*}*/
        ...
        if ( usmDbDot1xPortKeyTransmissionEnabledGet(unit, interface, &mode) == L7_SUCCESS )
        {
          ...
          if (mode == L7_TRUE)
          {
            sprintf(stat, "%-16s", pStrInfo_common_True);
          }                                       /* true */
          else if (mode == L7_FALSE)
          {
            sprintf(stat, "%-16s", pStrInfo_common_False);
          }                                        /* false */
          ewsTelnetWrite(ewsContext, stat);
        }
        ...
        if ( usmDbDot1xPortAdminControlledDirectionsGet(unit, interface, &val) == L7_SUCCESS )
        {
          ...
          case L7_DOT1X_PORT_DIRECTION_BOTH:
            sprintf(stat, "%-16s", pStrInfo_common_Both_1);    /* both */
            break;
          case L7_DOT1X_PORT_DIRECTION_IN:
            sprintf(stat, "%-16s", pStrInfo_common_AclInStr);    /* in */
            break;
          default:
            sprintf(stat, "%-16s", pStrInfo_common_NotApplicable);    /* N/A */
            break;
          }
          ewsTelnetWrite(ewsContext, stat);
        }
           ...
           val == L7_DOT1X_PORT_AUTO_MAC_BASED )
        {
          ...
          do
          {
            ...
            if(usmDbDot1xLogicalPortVlanAssignmentGet(lIntIfNum,&val,&val1) == L7_SUCCESS)
            {
              ...
              {
              case L7_DOT1X_RADIUS_ASSIGNED_VLAN:
                ewsTelnetPrintf (ewsContext, "%-3u %s", val,pStrInfo_security_Dot1xVlanAssigned_Radius);
                break;
              case L7_DOT1X_UNAUTHENTICATED_VLAN:
                ewsTelnetPrintf (ewsContext, "%-3u %s", val,pStrInfo_security_Dot1xVlanAssigned_unauth);
                break;
              case L7_DOT1X_GUEST_VLAN:
                ewsTelnetPrintf (ewsContext, "%s ",pStrInfo_security_Dot1xVlanAssigned_Guest);
                break;
              case L7_DOT1X_VOICE_VLAN:
                ...
                break;
              case L7_DOT1X_DEFAULT_ASSIGNED_VLAN:
                ewsTelnetPrintf (ewsContext, "%-3u %s", val,pStrInfo_security_Dot1xVlanAssigned_Default);
                break;
                                      ...
                                      uintf index)
{
  ...
  { 
    if ( usmDbPortUserGetFirst(unit, interface, strUserName ) != L7_SUCCESS )
    {
    ...
    do
    {
      ...
      strcpy((L7_char8 *)strUserNamePrev, (L7_char8 *)strUserName);
      memset( strUserName, 0, sizeof(strUserName));
    } while (usmDbPortUserGetNext(unit, interface, strUserNamePrev, strUserName) == L7_SUCCESS);
  }
  else
  {
    ewsTelnetWriteAddBlanks (1, 0, 6, 6, L7_NULLPTR, ewsContext, pStrInfo_common_Usrs);
                  ...
                  val == L7_DOT1X_PORT_STATUS_AUTHORIZED)
               {
                   do
                   {
                           ...
                           (val == L7_DOT1X_APM_AUTHENTICATED)) 
                      {
                           ...
                           sprintf(buf, cliDisplayInterfaceHelp(unit, slot, port));
