 ...
 **********************************************************************
*
* @filename cp_web_customer_success.html
*
...
/*****************************************************************************
 ****************************************************************************/
void ewaFormServe_cpWebCustomSuccess ( EwsContext context, EwaForm_cpWebCustomSuccessP form )
{
  EwaNetHandle net;
  ...
  {
    form->value.cp_success_branding_image = osapiStrDup(buf);
    form->status.cp_success_branding_image |= (EW_FORM_INITIALIZED | EW_FORM_DYNAMIC);
  }
  ...
  if (L7_SUCCESS == usmDbCpdmCPConfigWebLogoutSuccessBrowserTitleTextGet(cpId,webId,tmp))
  {
    ...
    form->value.cp_logout_success_browser_title = osapiStrDup(buf);
    form->status.cp_logout_success_browser_title |= (EW_FORM_INITIALIZED | EW_FORM_DYNAMIC);
  }
  ...
  {
    usmWebConvertHexToUnicode(tmp,buf);
    form->value.cp_logout_success_page_title = osapiStrDup(buf);
    form->status.cp_logout_success_page_title |= (EW_FORM_INITIALIZED | EW_FORM_DYNAMIC);
  }

  /* logout success content/instruct text */
  memset(tmp, 0, sizeof(tmp));
  memset(buf, 0, sizeof(buf));
  if (L7_SUCCESS == usmDbCpdmCPConfigWebLogoutSuccessContentTextGet(cpId,webId,tmp))
  {
    usmWebConvertHexToUnicode(tmp,buf);
    form->value.cp_logout_success_instruct_text = osapiStrDup(buf);
    form->status.cp_logout_success_instruct_text |= (EW_FORM_INITIALIZED | EW_FORM_DYNAMIC);
  }
...
/******************************************************************************
******************************************************************************/
char *ewaFormSubmit_cpWebCustomSuccess ( EwsContext context, EwaForm_cpWebCustomSuccessP form )
{
  EwaNetHandle net;
  usmWeb_AppInfo_t appInfo;
  cpId_t cpId;
  ...
  if (form->status.Submit & EW_FORM_RETURNED)
  {
   ...
   /* logout success background image */
    memset(buf, 0, sizeof(buf));
    memcpy(buf, form->value.cp_logout_success_image,CP_FILE_NAME_MAX);
    if (osapiStrncmp(buf,WEB_DEF_NOSEL_IMAGE_NAME,CP_FILE_NAME_MAX) == 0)
    {
      memset(buf, 0, sizeof(buf));
    }
    if (L7_SUCCESS != usmDbCpdmCPConfigWebLogoutSuccessBackgroundImageNameSet(cpId,webId,buf))
    {
      appInfo.err.err_flag = L7_TRUE;
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                       pStrErr_common_FailedToSet, "Logout Success Background Image Name");
    }

    /* logout success browser title */
    if (CP_LOGOUT_SUCCESS_BROWSER_TITLE_TEXT_MAX < strlen(form->value.cp_logout_success_browser_title_hex))
    {
      appInfo.err.err_flag = L7_TRUE;
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                       pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessBrowserTitleText);
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1),
                       pStrInfo_security_MaxNumOfChars, 
                       CP_UI_LOGOUT_SUCCESS_BROWSER_TITLE_TEXT_MAX);
    }
    else
    {
      memset(buf, 0, sizeof(buf));
      memcpy(buf, form->value.cp_logout_success_browser_title_hex, strlen(form->value.cp_logout_success_browser_title_hex));
      if (L7_SUCCESS != usmDbCpdmCPConfigWebLogoutSuccessBrowserTitleTextSet(cpId,webId,buf))
      {
        appInfo.err.err_flag = L7_TRUE;
        osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                         pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessBrowserTitleText);
    ...
    /* logout success page title */
    if (CP_LOGOUT_SUCCESS_TITLE_TEXT_MAX < strlen(form->value.cp_logout_success_page_title_hex))
    {
      appInfo.err.err_flag = L7_TRUE;
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                       pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessPageTitleText);
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1),
                       pStrInfo_security_MaxNumOfChars, 
                       CP_UI_LOGOUT_SUCCESS_TITLE_TEXT_MAX);
    }
    else
    {
      memset(buf, 0, sizeof(buf));
      memcpy(buf, form->value.cp_logout_success_page_title_hex, strlen(form->value.cp_logout_success_page_title_hex));
      if (L7_SUCCESS != usmDbCpdmCPConfigWebLogoutSuccessTitleTextSet(cpId,webId,buf))
      {
        appInfo.err.err_flag = L7_TRUE;
        osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                         pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessPageTitleText);
    ...
    /* logout success content/instruct title */
    if (CP_LOGOUT_SUCCESS_CONTENT_TEXT_MAX < strlen(form->value.cp_logout_success_instruct_hex))
    {
      appInfo.err.err_flag = L7_TRUE;
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                       pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessInstructText);
      osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1),
                       pStrInfo_security_MaxNumOfChars, 
                       CP_UI_LOGOUT_SUCCESS_CONTENT_TEXT_MAX);
    }
    else
    {
      memset(buf, 0, sizeof(buf));
      memcpy(buf, form->value.cp_logout_success_instruct_hex, strlen(form->value.cp_logout_success_instruct_hex));
      if (L7_SUCCESS != usmDbCpdmCPConfigWebLogoutSuccessContentTextSet(cpId,webId,buf))
      {
        appInfo.err.err_flag = L7_TRUE;
        osapiSnprintfcat(appInfo.err.msg, (sizeof(appInfo.err.msg)-strlen(appInfo.err.msg)-1), "%s%s.\r\n", 
                         pStrErr_common_FailedToSet, pStrInfo_security_LogoutSuccessInstructText);
  ...
  if (form->value.refresh_key == CLEAR)
  {
    ...
    if (usmDbCpdmCPConfigWebAddDefaults(cpId,webId,langCode,CP_LOGOUT_SUCCESS_PAGE) != L7_SUCCESS)
    {
  ...
  memcpy(net->app_pointer, &appInfo, sizeof(usmWeb_AppInfo_t));
  ewsContextSendReply(context,"cp_web_custom_success.html");
  return NULL;
}
