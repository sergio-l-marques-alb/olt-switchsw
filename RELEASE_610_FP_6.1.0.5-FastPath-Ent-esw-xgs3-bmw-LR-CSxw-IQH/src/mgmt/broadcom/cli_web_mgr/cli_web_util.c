...
L7_RC_t cliWebPhaseOneInit(void)
{
  ...
  do
  {
    ...
    {
      LOG_MSG("%s: cliWebPhaseOneInit: %u: Failed to allocate memory for cliCommon.\n",
              __FILE__, __LINE__);
      return L7_FAILURE;
    }
    /*old SSH TELNET TAG _TOTAL_CONNECTIONS*/
    ...
    L7_web_java_task_id = osapiTaskCreate( "webJavaTask", L7_web_java_task, 0, 0,
                                           L7_DEFAULT_STACK_SIZE + (1024 * 32),
                                           L7_DEFAULT_TASK_PRIORITY,
                                           L7_DEFAULT_TASK_SLICE);

    if (L7_web_java_task_id == L7_ERROR)
      LOG_MSG("Failed to Create Web_Java Task.\n");
#endif
    ...
    if (L7_util_task_id == L7_ERROR)
      LOG_MSG("Failed to Create Util Task.\n");
    ...
    if (L7_cli_web_task_id == L7_ERROR)
      LOG_MSG("Failed to Create Cli_Web Task.\n");
...
L7_RC_t cliWebPhaseTwoInit(void)
{
  ...
  if (trapMgrRegisterCardEvent(L7_CLI_WEB_COMPONENT_ID,
    ...
    LOG_MSG("Failed registration for TRAPMGR CARD EVENTS\n");
          ...
          if (context != L7_NULL)
          {
            ...
            if (i != CLI_SERIAL_HANDLE_NUM)
            {
              ...
              for (j=0; j < strlen(redirBuf); j++)
              {
                if (redirBuf[j] == '\n' && (j == 0 || redirBuf[j-1] != '\r'))
                {
                  write(context->net_handle->socket, "\r", 1);
                }
                write(context->net_handle->socket, &redirBuf[j], 1);
              }
            }
            else
            {
              /* Display the message */
              write(cliWebIOStdOutFd, redirBuf, len);
            }
          }
        }
      }
...
void cliWebIORedirectIoToCLI(void)
{
  if (cliWebIORedirectInitialized == L7_TRUE)
  {
    ...
    LOG_MSG("Redirecting IO to CLI Redirect Task\n");
  }
}
...
void cliWebIORedirectIoToStdIO(void)
{
  if (cliWebIORedirectInitialized == L7_TRUE)
  {
    ...
    LOG_MSG("Redirecting IO to stdout\n");
  }
}
...
L7_RC_t cliWebIoRedirectStart(void)
{
  ...
  */
  ...
  if (cliIoRedirectToCliGet() != L7_TRUE)
  {
    ...
    LOG_MSG("IO Redirection To CLI Succeeded\n");
    return L7_SUCCESS;
  }
...
L7_RC_t cliWebIoRedirectStop(void)
{
  if (cliIoRedirectToCliGet() != L7_TRUE)
  {
    LOG_MSG("IO Redirection To Stdout Failed\n");
    return L7_FAILURE;
  }
  /* Restore CLI redirection */
  cliWebIORedirectIoToStdIO();
  cliIoRedirectToCliSet(L7_FALSE);
  LOG_MSG("IO Redirection To Stdout Succeeded\n");
