...
L7_RC_t cliReadBannerFile(L7_char8 *filename, L7_char8 *buffer)
{
  ...
  if((fileHandle=osapiFsOpen(filename))==L7_ERROR)
  {
    LOG_MSG("not able to open the file specified");
    return L7_ERROR;
  }
  ...
  {
    LOG_MSG("not able to close open file");
    return L7_ERROR;
  }
  ...
  {
    LOG_MSG(" File size is zero or file do not exists");
    return L7_ERROR;
  }
  ...
  {
	L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_CLI_WEB_COMPONENT_ID, " File size is greater "
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG("Not able to read the file");
    return L7_ERROR;
  }
...
L7_RC_t cliWriteBannerFile (L7_char8 * outputFileName)
{
  ...
  /*create temp file */
  filedesc = osapiFsOpen (outputFileName);
