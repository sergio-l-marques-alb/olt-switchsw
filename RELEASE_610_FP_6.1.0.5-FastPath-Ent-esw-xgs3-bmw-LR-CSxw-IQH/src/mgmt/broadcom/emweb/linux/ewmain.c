...
struct EwaNetHandle_s ewaNetHandleSpecialDiscardObject;
#endif /* EWCONFIG_OPTION_UPNP */
...
/* lvl7_P0006 start */
    if ((usmDbTransferSuspendMgmtAccessGet(U_IDX)) && (NULL != usmDbTransferContextGet(U_IDX)))
    {
    ...
    {
      cliWebAccessSemaFree ();
      // PTin: characters are detected here
      //printf("%s:%s(%d)\n",__FILE__, __FUNCTION__, __LINE__);   // PTIN: DO NOT DELETE
      sockstat = select( max_socket+1
                         ,FDSET_CAST(&read_ready)
                         ,FDSET_CAST(&write_ready)
                         ,NULL
  ...
  if ( connection != NULL )
  {
	...
	connection->connection_type = EW_CONNECTION_TELNET;
        ...
        {                                                                 /* has be be >= because it is not incremented until after this call */
          ...
          LOG_MSG("EMWEB (Telnet): Max number of Telnet login sessions exceeded\n");
          close( connection->socket );
          ewaFree( connection );
          connection = NULL;
          return connection;
    ...
    /* internal connection, always v4 */
      ...
      */
...
#endif /* L7_MGMT_SECURITY_PACKAGE */
      {
        ...
        LOG_MSG("EMWEB (SSH): Max number of SSH login sessions exceeded\n");
        close(connection->socket);
        ewaFree(connection);
        connection = NULL;
        return connection;
        ...
        */
...
#if defined(L7_IPV6_PACKAGE) || defined(L7_IPV6_MGMT_PACKAGE)
        {
          ...
          */
          ...
          {
          cliSSHLoginCheck(username, L7_NULL, L7_NULLPTR, connection->context);
          if (cliSSHLoginCheck(password, accessLevel, challengePhrase, connection->context) == L7_TRUE)
          {
            cliCommon[cliUtil.handleNum].loginStatus = L7_TRUE;
            cliCommon[cliUtil.handleNum].passwdExpStatus = L7_TRUE;

            if (cliPasswdExpireCheck (L7_NULL, connection->context)
                == L7_TRUE)
            {
  ...
  for ( s = 0; s < (int) MaxConnections; s++ )
  {
        ...
        && (Handle[s]->context->obtActive == L7_FALSE))
    {
      ...
      if (buffer != EWA_NET_BUFFER_NULL)
      {
        ...
        {
          read(bspapiConsoleFdGet(), (char *)ewaNetBufferDataGet(buffer), 1);
          bytes = 1;
        }
#if 0
        ...
        if (bytes > 0)
        {
          ...
          */
          ...
          if (status != EWS_STATUS_OK)
          {
            LOG_MSG("ewsNetHTTPReceive failure in NetReceiveLoop() - closing connection.\n");
            if ( connection->pending_xmit != EWA_NET_BUFFER_NULL )
            {
              ewaNetBufferFree( connection->pending_xmit );
              connection->pending_xmit = EWA_NET_BUFFER_NULL;
...
#endif /* defined (EW_CONFIG_OPTION_SSL_RSA) */
      {
        ...
        if (bytes_to_be_sent != 0)
        {
          ...
          if (connection->socket == bspapiConsoleFdGet())
          {
            ...
            if (fdout != -1)
            {
              write(fdout, (char*)ewaNetBufferDataGet( buffer ), bytes_to_be_sent);
