...
/**********************************************************************
* @purpose  Call LOG_MSG for the session disconnect
*
* @param    sessionId
*
* @returns  Nothing
...
void EwaDisconnectLogMsg(L7_uint32 sessionId)
{
  ...
  LOG_MSG("%s Session %d ended for user %s connected from %s\n",
...
EwaSessionInit()
{
  ...
  {
    LOG_MSG("EwaSessionInit %s(%d): unable to create semaphore!\n",
        __FILE__, __LINE__);
    return;
  }
...
EwaSessionTableTimeoutScan(void)
{
  ...
  if (ewaSessionDataLock() == L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("EwaSessionTableTimeoutScan %s(%d): unable to release semaphore!\n",
          __FILE__, __LINE__);
    }
  }
  else
  {
    LOG_MSG("EwaSessionTableTimeoutScan %s(%d): unable to obtain semaphore!\n",
        __FILE__, __LINE__);
  }
}
...
EwaSessionLookup(char* sid, L7_uint32 sessionType, L7_inet_addr_t inetAddr, L7_BOOL createFlag)
{
  ...
  if (ewaSessionDataLock() != L7_SUCCESS)
  {
    LOG_MSG("EwaSessionLookup %s(%d): unable to obtain semaphore!\n",
        __FILE__, __LINE__);
    return NULL;
  }
      ...
      else
      {
        ...
        if (sid != NULL)
        {
              ...
              (strcmp(session_table[i].sid, sid) == 0))
          {
            ...
            if (ewaSessionDataUnlock() != L7_SUCCESS)
            {
              LOG_MSG("EwaSessionLookup %s(%d): unable to release semaphore!\n",
                  __FILE__, __LINE__);
            }
  ...
  if ((nextavailable >= 0) && (createFlag == L7_TRUE))
  {
    ...
    {
      LOG_MSG("EwaSessionLookup %s(%d): HTTPS session denied.  Maximum sessions reached.\n",
              __FILE__, __LINE__);
      nextavailable = -1;
    }
    if ((sessionType == L7_LOGIN_TYPE_HTTP) && (httpSessionCount >= httpSessionsMax))
    {
      LOG_MSG("EwaSessionLookup %s(%d): HTTP session denied.  Maximum sessions reached.\n",
              __FILE__, __LINE__);
      nextavailable = -1;
    }
...
void EwaSessionRemove(char* sid)
{
  ...
  if (ewaSessionDataLock() != L7_SUCCESS)
  {
    LOG_MSG("EwaSessionRemove %s(%d): unable to obtain semaphore!\n",
        __FILE__, __LINE__);
    return;
  }
  ...
  if (ewaSessionDataUnlock() != L7_SUCCESS)
  {
    LOG_MSG("EwaSessionRemove %s(%d): unable to release semaphore!\n",
        __FILE__, __LINE__);
  }
...
EwaSessionNumActiveGet(void)
{
  ...
  if (ewaSessionDataLock() == L7_SUCCESS)
  {
    ...
    if (ewaSessionDataUnlock() != L7_SUCCESS)
    {
      LOG_MSG("EwaSessionNumActiveGet %s(%d): unable to release semaphore!\n",
          __FILE__, __LINE__);
    }
  }
  else
  {
    LOG_MSG("EwaSessionNumActiveGet %s(%d): unable to obtain semaphore!\n",
        __FILE__, __LINE__);
  }
  return numSessions;
}
