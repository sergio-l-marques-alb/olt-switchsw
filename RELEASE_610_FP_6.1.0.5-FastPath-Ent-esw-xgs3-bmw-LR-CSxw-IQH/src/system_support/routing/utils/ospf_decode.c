...
ospfv3LsaOptionsDecode(L7_char8 * options, outputFuncPtr output, void * context)
{
  ...
  if (opt == 0)
  {
    ...
    do {
      ...
      {
        strncat(outBuf, lsaOptDCBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, lsaOptRBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, lsaOptNBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, lsaOptMCBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, lsaOptEBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, lsaOptV6BitStr, sizeof(outBuf));
...
ospfv3LsaOptionsDecode(L7_char8 * options, outputFuncPtr output, void * context)
{
  ...
  if (opt == 0)
  {
    strncat(outBuf, lsaOptionsBitsDecode.noBitSetStr, sizeof(outBuf));
  } 
  else {
    int i;
    ...
    for (i = 7; i > 0 && opt; i--) {
      if (opt & bitMask.mask[i])
      {
        ...
        {
          strncat(outBuf, lsaOptionsBitsDecode.bitStr[i], sizeof(outBuf));
        }
        opt &= ~bitMask.mask[i];
        if (!opt)
        {
  ...
  strncat(outBuf, lsaOptionsBitsDecode.terminalStr, sizeof(outBuf));
  strncat(outBuf, "\r\n", sizeof(outBuf));
  output(context, outBuf);
  return L7_SUCCESS;
}
...
ospfv3RouterLsaBitsDecode(L7_char8 * rtrLsa, outputFuncPtr output, void * context)
{
  ...
  int bits = *rtrLsa & L7_OSPFV3_LSA_ROUTER_KNOWNBITS_MASK;
    ...
    do {
      ...
      {
        strncat(outBuf, rtrLsaNtBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, rtrLsaWBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, rtrLsaVBitStr, sizeof(outBuf));
      ...
      {
        strncat(outBuf, rtrLsaEBitStr, sizeof(outBuf));
      ...
      {
         strncat(outBuf, rtrLsaBBitStr, sizeof(outBuf));
                          ...
                          outputFuncPtr output, void * context, L7_uint32 lsaLength)
{
  ...
  if(flags)
  {
    ...
    {
      strncat(optBuf, aseLsaOptionsEBitStr, sizeof(optBuf));
    ...
    {
      strncat(optBuf, aseLsaOptionsFBitStr, sizeof(optBuf));
    ...
    {
      strncat(optBuf, aseLsaOptionsTBitStr, sizeof(optBuf));
...
ospfv3LsaPrefixOptionsDecode(L7_char8 *linkLsa, outputFuncPtr output, void * context)
{
  ...
  if (opt == 0)
  {
    ...
    do {
      ...
      {
        strncat(optBuf, linkLsaPfxOptionsPBitStr, sizeof(optBuf));
      ...
      {
        strncat(optBuf, linkLsaPfxOptionsMCBitStr, sizeof(optBuf));
      ...
      {
        strncat(optBuf, linkLsaPfxOptionsLABitStr, sizeof(optBuf));
      ...
      {
        strncat(optBuf, linkLsaPfxOptionsNUBitStr, sizeof(optBuf));
