 ...
 * ********************************************************************/
  ...
  static char __fileid[] = "$Header: /home/cvs/PR1003/PR1003/FASTPATH/RELEASE_610_FP_6.1.0.5-FastPath-Ent-esw-xgs3-bmw-LR-CSxw-IQH/src/system_support/routing/arp/arp.c,v 1.1 2011/04/18 17:10:52 mruas Exp $";
#endif
...
static t_ADR *_createAddrEntry(t_ARP *p_A, t_IPAddr *ipAddr, t_MACAddr *macAddr,
               ...
               t_Handle lowId, word queueSize, t_Handle *p_ObjId)
{
    ...
    if(!p_A->arpCacheSize)
        p_A->arpCacheSize = (word)ARP_DEF_CACHESIZE;
 ...
 *
 * @notes         Limit the time spent on this to 1 second. Don't want
...
e_Err ARP_PurgeOldEntries(t_Handle *p_ObjId)
{
  ...
  while ((e == E_OK) && p_Adr)
  {
    ...
    p_Adr = nextAdr;
    //CLEMOS if ((osapiTimeMillisecondsGet() - startTime) > ARP_MAX_WINNOW_TIME)
...
static e_Err ARP_ProxyArp(L7_uint32 flags, t_ARPFrame *arpFrame)
{
    ...
    L7_uint32 localProxyArpMode;  /* whether local proxy ARP is enabled on
                                      rx interface */
    ...
    if (rtoBestRouteLookup(destAddr, &route, L7_FALSE) == L7_SUCCESS)
    {
      ...
      nimGetIntfAddress(rxIntIfNum, L7_NULL, rxIfMacAddr);
...
e_Err ARP_Receive(t_Handle Id, void *p_Frame, unsigned int rxIntIfNum, word flags)
{
   ...
   else if(opCode == ARP_REQUEST)
   {
      ...
      if(vrrpFuncTable.vrrpMapGetVMac &&
         (vrrpFuncTable.vrrpMapGetVMac((L7_uchar8 *)arpFrame.dstIP, (L7_uchar8 *)tVMAC) == L7_SUCCESS))
      {
                             ...
                             t_MACAddr *macAddr, word flags, Bool create)
{
          ...
          (addrEnt->intfNum == flags))
      {
          ...
          if (memcmp(macAddr, addrEnt->macAddr, MACADDRLENGTH) != 0)
          {
              ...
              L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_IP_MAP_COMPONENT_ID,
                      "ARP received mapping for local IP address %s to MAC address %s. "
                      "Local IP address may be configured on another station."
                      " When we receive an ARP response with different MAC address from "
                      "another station with the same IP address as ours. This might be a "
   ...
   /* Don't update the ARP cache for an IP address used by VRRP as a virtual 
    * router address. */
   if (vrrpFuncTable.vrrpMapGetVMac &&
       (vrrpFuncTable.vrrpMapGetVMac(*ipAddr, tVMAC) == L7_SUCCESS))
   {
     return; 
   }  
...
#else
{
   ...
   if(vrrpFuncTable.vrrpMapGetVMac &&
      (vrrpFuncTable.vrrpMapGetVMac((L7_uchar8 *)arpFrame.srcIP, (L7_uchar8 *)tVMAC) == L7_SUCCESS))
...
static e_Err  _sendGratARP(t_ADR *p_Adr)
{
   ...
   for(srcAddrEnt = p_A->addrEnt; srcAddrEnt;
   srcAddrEnt = srcAddrEnt->next)
   {
       if (srcAddrEnt->intfNum == p_Adr->intfNum)
       {
           /*
           With secondaries, there can be more than one addrEnt for
           an interface, so find the one that matches the IP address
           in p_Adr to send the right address for gratuitous ARP
           */

           if (memcmp(srcAddrEnt->ipAddr, p_Adr->ipAddr, sizeof(t_IPAddr)) == 0)
           {
               break;
           }
       }
   ...
   memcpy(arpFrame.dstIP, p_Adr->ipAddr, IPADDRLENGTH);
   /* Gratuitous ARP by convention sets the destination MAC to all 1s */
   memcpy(arpFrame.dstMAC, bcstMacAddr, MACADDRLENGTH);
   len = F_AddToEnd(p_Frame, (byte *)&arpFrame, (word)sizeof(arpFrame));
   ASSERT(len);
