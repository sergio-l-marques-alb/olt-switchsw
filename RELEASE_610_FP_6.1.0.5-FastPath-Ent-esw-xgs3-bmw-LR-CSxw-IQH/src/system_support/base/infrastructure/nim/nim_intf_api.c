      ...
      do
      {
        if ((adminState == L7_ENABLE) || (adminState == L7_DISABLE) || (adminState == L7_DIAG_DISABLE))
        {
          if ( ( L7_ENABLE == adminState ) &&
               ( L7_FALSE  == linkDependencyIntfCanBeEnabled( intIfNum ) ) )
          {
            NIM_CRIT_SEC_WRITE_EXIT();
  ...
  else
  {
    ...
    if (rc == L7_SUCCESS)
    {
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_MSG("\nnimClearCounters: failed to notify interface %u stats reset event", intIfNum);
        }
      }
    }
  }
...
L7_RC_t nimSetIntfAutoNegoStatusCapabilities(L7_uint32 intIfNum, L7_uint32 negoCapabilities)
{
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    if (negoCapabilities == 0)
    {
      ...
      rc = dtlIntfSpeedSet(intIfNum, nimCtlBlk_g->nimPorts[intIfNum].configPort.cfgInfo.ifSpeed);
    }
  }
  return(rc);
}
