...
static avlTreeTables_t        *nimIfIndexTreeHeap = L7_NULL;
static nimIfIndexTreeData_t   *nimIfIndexDataHeap = L7_NULL;
...
L7_RC_t nimIfIndexPhaseOneInit()
{
  ...
  do
  {
    ...
    nimIfIndexTreeHeap = osapiMalloc(L7_NIM_COMPONENT_ID, sizeof(avlTreeTables_t)*platIntfTotalMaxCountGet());
    if (nimIfIndexTreeHeap == L7_NULLPTR)
    {
    ...
    {
      (void)memset(nimIfIndexTreeHeap,0x00,sizeof(avlTreeTables_t)*platIntfTotalMaxCountGet());
    }


    nimIfIndexDataHeap = osapiMalloc(L7_NIM_COMPONENT_ID, sizeof(nimIfIndexTreeData_t)*platIntfTotalMaxCountGet());
    if (nimIfIndexDataHeap == L7_NULLPTR)
    {
      NIM_LOG_ERROR("NIM: Unable to allocate resources\n");
      break; /* goto while */
    }
    ...
    {
      (void)memset(nimIfIndexDataHeap,0x00,sizeof(nimIfIndexTreeData_t)*platIntfTotalMaxCountGet());
    }

    avlCreateAvlTree(&nimIfIndexTreeData, nimIfIndexTreeHeap, nimIfIndexDataHeap, platIntfTotalMaxCountGet(),
                     sizeof(nimIfIndexTreeData_t), 0x10, sizeof(L7_uint32));
    (void)avlSetAvlTreeComparator(&nimIfIndexTreeData, avlCompareULong32);
