...
nimMigrateConfigV3V4Convert (nimConfigDataV3_t * pCfgV3, nimConfigDataV4_t * pCfgV4)
{
  ...
  {
      LOG_MSG("Migration from V3 to V4 cannot be supported\n");
  }
  ...
  */
  /*============================================================================*/
                                                             ...
                                                             
   {
       ...
       {
           LOG_MSG(" Failed to migrate interface config mask from version 3 to version 4\n");
           return L7_FAILURE;
       }
...
nimRel4_0FileMigrate (nimConfigData_t * pCfgData)
{
  ...
  if (pCfgPort == L7_NULL)
  {
    LOG_MSG ("Failed to get port config");
    return L7_ERROR;
  }
  ...
  for (slot = 0; slot < L7_MAX_SLOTS_PER_BOX_REL_4_0; slot++)
  {
    ...
    {
      LOG_MSG ("rel4_0NimSlotConfig[slot].numPorts %d out of bounds",
    ...
    for (port = 1; port <= rel4_0NimSlotConfig[slot].numPorts; port++, pCfgPortV1++)
    {
      ...
      if (pCfgPort != L7_NULL)
      {
        LOG_MSG ("Migrating NIM: slot %d port %d intIfNum %d intfType %d addrType = %d",
                 slot, port, intIfNum, intfType, pCfgPortV1->addrType);
                                             ...
                                             NIM_INTF_MASK_REL_4_3_t *destMask)
{
    ...
    {
        LOG_MSG("Migration of previous mask size %d is not supported in this release\n",
