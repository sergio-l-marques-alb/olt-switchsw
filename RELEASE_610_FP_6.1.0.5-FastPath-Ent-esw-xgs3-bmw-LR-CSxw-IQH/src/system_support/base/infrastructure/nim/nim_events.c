...
// PTin added
L7_BOOL nimNotifyIntf_done(void)
{
  L7_uint32 num;
...
L7_RC_t nimNotifyUserOfIntfChange(NIM_CORRELATOR_t correlator, NIM_EVENT_NOTIFY_INFO_t eventInfo)
{
  ...
  if (nimPhaseStatusCheck() != L7_TRUE)
  {
    ...
    NIM_LOG_MSG("NIM: event(%d),intf(%d),component(%d), in wrong phase\n",
                event,intIfNum,eventInfo.component);
  }
  else if (state == L7_INTF_UNINITIALIZED)
  {
        ...
        (event != L7_INACTIVE))
    {
      ...
      NIM_LOG_MSG("NIM: event(%d),intf(%d),component(%d), on non-existent intf\n",
                  event,intIfNum,eventInfo.component);
    }
    else
    {
      ...
      if (event < L7_LAST_PORT_EVENT)
      {
        switch (event)
        {
          case L7_ATTACH:
            ...
            {
              NIM_LOG_MSG("NIM: L7_ATTACH out of order for intIfNum(%d) unit %d slot %d port %hu\n",
                          eventInfo.intIfNum,usp.unit,usp.slot,usp.port);
              rc = L7_FAILURE;
              performCallback = L7_TRUE;
            }
            else if (nimIntfConfigApply(eventInfo.intIfNum) != L7_SUCCESS)
            {
              NIM_LOG_MSG("NIM: Failed to ATTACH unit %d slot %d port %hu\n", usp.unit,usp.slot,usp.port);
              rc = L7_FAILURE;
              performCallback = L7_TRUE;
            }
            else
          ...
          case L7_DETACH:
            ...
            {
              NIM_LOG_MSG("NIM: L7_DETACH out of order for intIfNum(%d) unit %d slot %d port %hu\n",
                          eventInfo.intIfNum,usp.unit,usp.slot,usp.port);
              rc = L7_FAILURE;
              performCallback = L7_TRUE;
            }
          ...
          case L7_DELETE:
            if (nimUtilIntfStateGet(eventInfo.intIfNum) != L7_INTF_CREATED)
            {
              NIM_LOG_MSG("NIM: L7_DELETE out of order for intIfNum(%d) unit %d slot %d port %hu\n",
                          eventInfo.intIfNum,usp.unit,usp.slot,usp.port);
              rc = L7_FAILURE;
              performCallback = L7_TRUE;
            }
          ...
          case  L7_UP:
            ...
            else
            {
              ...
              if (nimCtlBlk_g->nimPorts[macroPort].configPort.cfgInfo.adminState == L7_ENABLE)
              {
                ...
                {
                  LOG_MSG("NIM: failed to send a TRAP message\n");
                }
              }
            }
            break;
