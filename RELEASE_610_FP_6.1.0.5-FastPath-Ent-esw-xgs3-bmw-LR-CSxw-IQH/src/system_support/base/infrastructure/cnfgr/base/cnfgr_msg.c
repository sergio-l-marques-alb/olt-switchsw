    ...
    do {
        ...
        if ( msgRC == L7_SUCCESS && pMsgData != L7_NULLPTR ) {
            ...
            default:
                 ...
                 * message.
                 */
                LOG_MSG("cnfgrMsgHandler: Discarded msg on Q1 %08x\n", pMsgData->msgType);
                cnfgrMsgEIP = L7_FALSE;
            ...
            if (++logMsgCount <= CNFGR_MSG_LOG_MAX_COUNT_f ) {
                LOG_MSG("cnfgrMsgHandler: Bad status on Q1 or invalid message pointer %08x\n", msgRC);
            ...
            */
            ...
            if (msgRC == L7_SUCCESS && pMsgData != L7_NULLPTR) {
                ...
                default:
                    ...
                    */
                    if (++logMsgCount <= CNFGR_MSG_LOG_MAX_COUNT_f ) {
                        LOG_MSG("cnfgrMsgHandler: Discarded msg on Q2 %08x\n", pMsgData->msgType);
                    } /* endif logMsgCount threshold */
...
L7_RC_t cnfgrMsgInitialize()
{
    ...
    */
    ...
    if (pCnfgrMsgQ1Id == L7_NULLPTR)
    {
      ...
      LOG_MSG("cnfgrMsgInitialize: Failed to create Garp Q1, exiting....\n");
      ...
      msgRC = L7_ERROR;
        ...
        {
          /*  Log Error Message - cnfgrMsgInitialize: initialization failed   */
          LOG_MSG("cnfgrMsgInitialize: Failed to create Message Handler Q2, exiting....\n");
          ...
          msgRC = L7_ERROR;
            ...
            if (pCnfgrMsgHandlerTid == (L7_int32)L7_ERROR) {
                ...
                /* Log Error Message - cnfgrMsgInitialize: initialization failed */
                LOG_MSG("cnfgrMsgInitialize: Failed to create Message Handler Thread, exiting....\n");
...
L7_RC_t cnfgrMsgSend(CNFGR_IN CNFGR_MSG_DATA_t *pMsgData)
{
    ...
    case CNFGR_MSG_NOOP:
        ...
        /* invalid message */
    default:
        LOG_MSG("cnfgrMsgSend: Invalid Message %08x\n", pMsgData->msgType);
        break;
