...
/*
 *  Logging buffer for use when logging subsystem is not initialized
 */
static L7_char8 logInitBuf[L7_LOG_MESSAGE_LENGTH];
 ...
 *         in-memory log.
 *
 * @param  buffer @b{(input)} the buffer to log
 * @param  length @b{(input)} length of the buffer (but not the header)
 *
 * @returns  None
 *
 * @notes  This executes on the calling task thread. This routine may
...
void logMsgBuffer(L7_uchar8 * msgbuf, L7_uint32 msglen)
{
  L7_COMPONENT_IDS_t  component;
  L7_LOG_SEVERITY_t   severity;
  L7_BOOL logMem;
...
L7_RC_t logWriteInMemoryLogToFile(L7_char8 * fileName)
{
  ...
  if (fd == L7_ERROR)
  {
    ...
    {
      LOG_MSG("logWriteInMemoryLogToFile: osapiFsCreateFile failed");
    }
    fd = osapiFsOpen(fileName);
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("logWriteInMemoryLogToFile: osapiFsDeleteFile failed");
    }
    return L7_ERROR;
  }
