...
L7_RC_t sysapiSupportCfgFileSysInit(void)
{
  ...
  for (i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
    ...
    {
      LOG_MSG("Failed to malloc pSysapiSupportCfgTree[%u]\n", i);
      LOG_ERROR(1);
      return L7_ERROR;
    }
    else
...
static L7_RC_t sysapiSupportCfgFileSeparate(L7_char8 *cfgFile)
{
  ...
  {
    LOG_MSG ("Support configuration file size %u is toos small.\n",
             total_length);
    (void)osapiSemaGive(sysapiSupportCfgSema);
    return L7_ERROR;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG ("Failed to read support config file %s header\n", cfgFile);
    (void)osapiSemaGive(sysapiSupportCfgSema);
    return L7_ERROR;
  }
  ...
  */
  ...
  {
    LOG_MSG ("Support configuration file on disk size %u is less than expected size %u.\n",
  ...
  {
    LOG_MSG("sysapiSupportCfgFileSeparate: CRC check failed. 0x%x read and 0x%x calculated",
  ...
  */
  ...
  {
     LOG_MSG("Unexpected file name:  %s\n",cfgFileHdr.filename);
    (void)osapiFsClose (file_desc);
    (void)osapiSemaGive(sysapiSupportCfgSema);
    return L7_ERROR;
  }
  ...
  */
  ...
  {
    LOG_MSG("Unexpected target device:  %s\n",cfgFileHdr.targetDevice);
    (void)osapiFsClose (file_desc);
    (void)osapiSemaGive(sysapiSupportCfgSema);
    return L7_ERROR;
  }
  ...
  {
    LOG_MSG("Unrecognized big support config file version %d\n", cfgFileHdr.fileHdrVersion);
    (void)osapiSemaGive(sysapiSupportCfgSema);
    return L7_ERROR;
  }
                                  ...
                                  L7_char8 *buffer, L7_uint32 nbytes)
{
  ...
  */
  ...
  {
    LOG_MSG("File %s hdr length = %u is invalid expect %u\n", filename,
  ...
  */
  ...
  {
    LOG_MSG("Failed to update support cfg tree for component %d and file %s\n",
            component_id, cfgFileHdr->filename);
                                ...
                                void (*migrateBuild)(L7_uint32, L7_uint32, L7_char8 *))
{
  ...
  */
  ...
  if (fileSize < sizeof(L7_fileHdr_t))
    ...
    else
    {
      ...
      if ((savedVersion != version) || (fileSize != bufferSize))
      {
        ...
        else
        {
          ...
          {
            LOG_MSG ("File %s: same version (%u) but the sizes (%u->%u) differ\n",
                     fileName, version, fileSize, bufferSize);
          }
          LOG_MSG("Migrating support config file %s from version %u to %u\n", fileName, savedVersion, version);
      ...
      else
      {
        ...
        {
          LOG_MSG ("Failed to uncompress config data for component %d\n", component_id);
          return L7_ERROR;
        }
  ...
  if (buildDefaultFile == L7_TRUE)
  {
    ...
    LOG_MSG("Building defaults for file %s version %u\n",fileName, version);
    ...
    *checkSum = nvStoreCrc32((L7_uchar8 *)buffer, bufferSize-sizeof(L7_uint32));
    ...
    if (sysapiSupportCfgFileWrite(component_id, fileName, buffer, bufferSize) == L7_ERROR)
      LOG_MSG("Error on call to sysapiSupportCfgFileWrite routine on file %s\n", fileName);
  }
...
L7_RC_t sysapiSupportCfgFileIndexGet(const L7_char8 *fileName, L7_uint32 *pFileIndex)
{
  ...
  for (i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
    if (pSysapiSupportCfgTree[i] == L7_NULL)
    {
      LOG_MSG("pSysapiSupportCfgTree[%u] is NULL", i);
      return L7_ERROR;
    }
    else
    {
...
static L7_RC_t sysapiSupportCfgFileSeparateFilesV2(L7_int32 file_desc, L7_int32 total_length)
{
  ...
  for (i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_MSG("Failure to process cfg file descriptor: index %d\n", i);
      return L7_ERROR;
    }

    if (cfgFileDescr.marker != SYSAPI_SUPPORT_CFG_DESCR_MARKER)
    {
      LOG_MSG("Invalid cfgFileDescr Marker: %d\n", cfgFileDescr.marker);
      (void)osapiFsClose (file_desc);
      return L7_ERROR;
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_MSG("Failure to read config file header: index %d\n", i);
      return L7_ERROR;
    }

    {
      ...
      if (rc == L7_ERROR)
      {
        LOG_MSG("Failure to cfg file index for file %s\n", cfgFileHdr.filename);
        (void)osapiFsClose (file_desc);
        return L7_ERROR;
      }
      ...
      if (data_area == L7_NULL)
      {
        ...
        if (data_area == L7_NULL)
        {
          LOG_MSG("Failure to allocate memory for cfg for file %s\n", cfgFileHdr.filename);
          (void)osapiFsClose (file_desc);
          return L7_ERROR;
        }
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_MSG("Failure closing file %s\n", cfgFileHdr.filename);
        return L7_ERROR;
      }
                                          ...
                                          SYSAPI_DIRECTORY_CFG_t *pCfgTreeEntry)
{
  ...
  {
    LOG_MSG("Failed to compress support config file for component %d\n", component_id);
    return L7_ERROR;
  }
  ...
  if (dataArea == L7_NULL)
  {
    ...
    if (dataArea == L7_NULL)
    {
      LOG_MSG("Failed to allocate %ld for support config file %s\n", compressedFileSize, filename);
      return L7_ERROR;
    }
  }
