...
L7_RC_t supportDebugCategoryRegisterImpl(supportDebugCategory_t supportDebugCategory)
{
  ...
  {
    LOG_MSG("Attempt to register invalid category %d\n", 
            (L7_int32)category);
    return(L7_FAILURE);
  }
  ...
  {
    LOG_MSG("Attempt to register invalid component ID %d\n", 
  ...
  {
    LOG_MSG("supportRoutine is null for category %d and componentId %d\n", 
            (L7_int32) category,(L7_int32)componentId);
    return(L7_FAILURE);
  }
  ...
  {
    LOG_MSG("Category %d  component %d: Attempt to register null name\n",
            (L7_int32)category, (L7_int32)componentId);
    return(L7_FAILURE);
  }
  ...
  {
    LOG_MSG("Unable to get buffer for category %d, component %d, routine %s\n",
            supportDebugCategory.category, supportDebugCategory.componentId, 
  ...
  {
    LOG_MSG("Debug Help routine %s (%x) already registered\n", 
  ...
  {
    LOG_MSG("Failed to register entry for category %d, component %d, routine %s\n",
            supportDebugCategory.category, supportDebugCategory.componentId, 
            supportDebugCategory.info.supportRoutineName);
...
L7_RC_t supportDebugCategoryDeregisterImpl(supportDebugCategory_t supportDebugCategory)
{
  ...
  if ( (category == SUPPORT_CATEGORY_UNUSED) || (category >= SUPPORT_CATEGORY_LAST) )
  {
    LOG_MSG("Attempt to deregister invalid category %d\n", 
            (L7_int32)category);
    return(L7_FAILURE);
  }
  ...
  if ( (componentId == L7_FIRST_COMPONENT_ID) || (componentId >= L7_LAST_COMPONENT_ID))
  {
    LOG_MSG("Attempt to deregister invalid component ID %d\n", 
            (L7_int32)componentId);
    return(L7_FAILURE);
  }
  ...
  if (pEntry == L7_NULLPTR)
  {
    ...
    LOG_MSG("Deregistration failed: Debug Help routine %s (%x) not registered\n", 
            supportDebugCategory.info.supportRoutineName,(L7_int32)supportRoutine);
    return(L7_FAILURE);
  }
                                    ...
                                    supportDebugCategory_t *supportDebugCategory)
{
  ...
  {
    LOG_MSG("Null value passed for supportDebugCategory\n");
    return L7_FAILURE;
  }
  /* Is this a getFirst */
...
L7_RC_t supportDebugRegisterImpl(supportDebugDescr_t supportDebugDescr)
{
  ...
  if (debugHelp != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("Component %d: Attempt to register null string for name or help description\n", 
              (L7_int32)componentId);
      return(L7_FAILURE);
    }
...
L7_RC_t supportDebugDeregisterImpl(supportDebugDescr_t supportDebugDescr) 
{
  ...
  /* Registration not found */
  LOG_MSG("Deregistration failed: Component %d, Debug Help routine %s (%x) not registered\n", 
          componentId, supportDebugDescr.internal.debugHelpRoutineName,(L7_int32)debugHelp);
  return(L7_FAILURE);
...
L7_RC_t supportDebugSave(void)
{
  ...
  for (i=0; i < L7_LAST_COMPONENT_ID; i++)
  {
    if (supportDebugDescrList[i].userControl.notifySave != L7_NULL)
    {
      ...
      {
        LOG_MSG("support/debug Save call failed for component %d\n", 
                (L7_int32)supportDebugDescrList[i].componentId);
  ...
  {
    LOG_MSG("Failed to save support cfg file\n");
    return L7_FAILURE;
  }
...
void supportDebugClear(void)
{
  ...
  for (i=0; i < L7_LAST_COMPONENT_ID; i++)
  {
    if (supportDebugDescrList[i].userControl.clearFunc != L7_NULL)
    {
      ...
      {
        LOG_MSG("Clear routine failed for component %d\n", 
                (L7_int32)supportDebugDescrList[i].componentId);
        LOG_EVENT(supportDebugDescrList[i].componentId);
      }
    }  /*  clearFunc != L7_NULL */
...
L7_RC_t supportDebugCategoryListInit(void)
{
  ...
  {
    LOG_MSG("Cannot allocate memory for support debug category buffers");
    return L7_FAILURE;
  }
