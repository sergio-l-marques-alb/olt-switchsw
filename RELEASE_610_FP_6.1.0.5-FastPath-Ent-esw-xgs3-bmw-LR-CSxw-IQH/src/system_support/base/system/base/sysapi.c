...
#endif
L7_BOOL sysapiGlobalDataChangedFlag = L7_FALSE;
...
void sysapiInit (void (*box_reset)(void))
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_EMERGENCY, L7_IP_MAP_COMPONENT_ID,
...
L7_RC_t sysapiCfgFileReload(L7_uchar8* filename)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG("Failed to separate component config from %s\n", filename);
  }
  return rc;
}
...
L7_RC_t sysapiCfgFileSysInit(void)
{
  ...
  for (i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
    ...
    {
      LOG_MSG("Failed to malloc pSysapiCfgTree[%u]\n", i);
      LOG_ERROR(1);
      return L7_ERROR;
    }
    else
...
static L7_RC_t sysapiCfgFileSeparate(L7_char8 *cfgFile)
{
  ...
  {
    LOG_MSG ("Configuration file size %u is toos small.\n",
             total_length);
    (void)osapiSemaGive(sysapiCfgSema);
    return L7_ERROR;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG ("Failed to read config file %s header\n", cfgFile);
    (void)osapiSemaGive(sysapiCfgSema);
    return L7_ERROR;
  }
  ...
  */
  ...
  {
    LOG_MSG ("Configuration file on disk size %u is less than expected size %u.\n",
  ...
  {
    LOG_MSG("sysapiCfgFilesSeparate: CRC check failed. 0x%x read and 0x%x calculated",
  ...
  {
    LOG_MSG("Unrecognized big config file version %d\n", cfgFileHdr.fileHdrVersion);
    (void)osapiSemaGive(sysapiCfgSema);
    return L7_ERROR;
  }
    ...
    L7_char8 *buffer, L7_uint32 nbytes)
{
  ...
  {
    LOG_MSG("Failed to update cfg tree for component %d and file %s\n",
    ...
    L7_char8 *buffer, L7_uint32 buffer_length)
{
  ...
  {
    LOG_MSG("Failed to allocate %u for config file %s\n", nbytes, filename);
    return L7_ERROR;
  }
    ...
    void (*defaultBuild)(L7_uint32))
{
  ...
  *checkSum = nvStoreCrc32((L7_uchar8 *)buffer, bufferSize-sizeof(L7_uint32));
  ...
  {
    LOG_MSG("Error on call to sysapiCfgFileWrite routine on file %s\n", fileName);
  }
  return L7_SUCCESS;
}
...
L7_RC_t sysapiCfgFileIndexGet(const L7_char8 *fileName, L7_uint32 *pFileIndex)
{
  ...
  for(i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
    if (pSysapiCfgTree[i] == L7_NULL)
    {
      LOG_MSG("pSysapiCfgTree[%u] is NULL", i);
      return L7_ERROR;
    }
    else
    {
    ...
    L7_char8 **pUncompressedData, L7_uint32 *uncompressedBufLen)
{
  ...
  if (bufferAllocationNeeded == L7_TRUE )
  {
    ...
    {
      LOG_MSG("Failed to allocate %ld for config file compression buffer\n", compressionBufferSize);
      return L7_ERROR;
    }
    ...
    L7_char8 **pCompressedData,  L7_uint32 *compressedBufLen)
{
  ...
  if (Z_OK != retCode)
  {
    LOG_MSG("Failed to compress data - rc = %u (%s)\n", retCode,
...
static L7_RC_t sysapiCfgFileSeparateFilesV1(L7_int32 file_desc, L7_int32 total_length)
{
  ...
  for (i = 0; i < SYSAPI_MAX_FILE_ENTRIES; i++)
  {
...

    {
      ...
      {
        LOG_MSG("Failed to update cfg tree for component %d and file %s\n",
            cfgFileHdr.componentID, cfgFileHdr.filename);
    ...
    SYSAPI_DIRECTORY_CFG_t *pCfgTreeEntry)
{
  ...
  {
    LOG_MSG("Failed to compress config file for component %d\n", component_id);
    return L7_ERROR;
  }
  ...
  if (dataArea == L7_NULL)
  {
    ...
    if (dataArea == L7_NULL)
    {
      LOG_MSG("Failed to allocate %ld for config file %s\n", compressedFileSize, filename);
      return L7_ERROR;
    }
  }
