...
L7_RC_t sysapiHpcInit(void)
{
  ...
  {
    LOG_MSG("hpcDescriptorDbInit: bad return code from sysapiHpcDescriptorDbInit call.\n");
    return(result);
  }

  if ((result = hpcInit()) != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from hpcInit call.\n");
    return(result);
  }
...
{
  return(hpcIfaceMacGet(type, slot, port, mac_addr));
}
...
void sysapiHpcDiagPowerCtrl(L7_uint32 unit, L7_uint32 port)
{
  if ( L7_NOT_SUPPORTED == HPC_DIAG_POWER_CTRL(unit, port))
    LOG_MSG("sysapiHpcSfpPowerCtrl: Pwr ctrl is not supported\n");
}
                                     ...
                                     L7_BOOL present)
{
  if ( L7_NOT_SUPPORTED == HPC_SENSOR_10GBASET_PRSNCE(unit, xfpPort, present))
    LOG_MSG("sysapiHpcNotify10GBASETPresence: 10GBASET ctrl is not supported\n");
}
...
void sysapiHpcNotifySfpPlusPresence(L7_uint32 unit, L7_uint32 sfpPort)
{
  if ( L7_NOT_SUPPORTED == HPC_SENSOR_SFPPLUS_PRSNCE(unit, sfpPort))
    LOG_MSG("sysapiHpcNotifySFP+Presence: SFP+ ctrl is not supported\n");
...
{
  return hpcDriverProcessStackTopoChange(managerFailed);
}
...
{
  return hpcDriverProcessManagerLinkDown(reportingUnit, reportingUnitKey,
...
{
  return hpcDriverNotifyStandbyStatus(stbyStatus);
}
