...
extern L7_CNFGR_STATE_t mirrorState;
...
L7_RC_t mirrorIntfApplyConfigData(L7_uint32 intIfNum)
{
  ...
  else if(mirrorIsSrcConfigured(intIfNum, &sessionNum) == L7_TRUE)
  {
    ...
    {
      LOG_MSG("mirrorIntfApplyConfigData(): Failed to get direction for port %u\n", intIfNum);
      return L7_FAILURE;
    }
  ...
  {
    /* should never get here -- if source interface is configured , default direction 
    L7_MIRROR_BIDIRECTIONAL should also be configured */
    LOG_MSG ("\nmirrorIntfApplyConfigData: Invalid Configuration. Source port %d \
             Configured without configuring valid direction %d. Defaulting direction\
             to Bidirectional.",intIfNum,probeType);
    L7_INTF_SETMASKBIT(mirrorCfgData->sessionData[sessionNum-1].srcIngressMirrorType,intIfNum);
    L7_INTF_SETMASKBIT(mirrorCfgData->sessionData[sessionNum-1].srcEgressMirrorType,intIfNum);
  ...
  /*--------------------------------------------------------------------------*/
    ...
    if (firstSourcePort[sessionNum-1] == L7_TRUE)
	{
...
L7_RC_t mirrorIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event,NIM_CORRELATOR_t correlator)
{
 ...
 /* initialize staus.response.reason as unused */
  ...
  {
    LOG_MSG("mirrorIntfChangeCallback: Received an interface event callback while not in EXECUTE state.\n");
  }
...
void mirrorNimEventIntfNotify(L7_uint32 intIfDest,L7_PORT_EVENTS_t eventType)
{
  ...
  */
  ...
  {
    LOG_MSG("Mirroring: Failed to notify components of probe status change\n");
  }
}
