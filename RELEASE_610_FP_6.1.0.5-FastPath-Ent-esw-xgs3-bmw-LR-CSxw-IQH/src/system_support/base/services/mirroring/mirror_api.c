...
L7_uint32 intfList[L7_MAX_INTERFACE_COUNT];
                            ...
                            L7_uint32 mode, L7_MIRROR_DIRECTION_t probeType)
{
  ...
  if (rc != L7_SUCCESS)
  {
    if (mode == L7_ENABLE)
    {
      LOG_MSG(" mirrorSourcePortSet failed to enable source interface %d\
              in direction %d in session %d.", intIfNum, probeType,sessionNum);
    }
    else 
    {
      LOG_MSG(" mirrorSourcePortSet failed to disable source interface %d\
              in direction %d in session %d.", intIfNum, probeType,sessionNum);
    }
...
L7_RC_t mirrorSourcePortRemove(L7_uint32 sessionNum,L7_uint32 intIfNum)
{
  ...
  if (rc == L7_SUCCESS && mirrorDestPortGet(sessionNum, &dstIntIfNum) == L7_SUCCESS)
  {
	...
	{
	  if (L7_INTF_ISMASKBITSET(mirrorCfgData->sessionData[sessionNum-1].srcPorts,i))
	  {
    ...
    if (srcConfigured == L7_FALSE && mirrorCfgData->sessionData[sessionNum-1].mode == L7_ENABLE)
    {
	  ...
	  if (rc == L7_SUCCESS)
	  {
        MIRROR_TRACE("\n\nmirrorSourcePortRemove::Calling mirrorNimEventIntfNotify with L7_PROBE_TEARDOWN for Interface: %d.\n",dstIntIfNum);
...
L7_RC_t mirrorDestPortRemove(L7_uint32 sessionNum)
{
  ...
  nimMaskToList((NIM_INTF_MASK_t *)&tempIfMask_g, intfList, 
    ...
    {
      /* should never get here */
      MIRROR_TRACE("\n\n mirrorDestPortRemove:: Error!!Number of source interface : %d is greater than Maximum number of source interfaces allowed : %d.\n",
                   numSrcIntf,L7_MIRRORING_MAX_SRC_PORTS_PER_SESSION) ;
...
/* Create the data to send to DTL */
  ...
  MIRROR_TRACE("\n\nmirrorDestPortRemove::Sending Dtl Session Config command.Session :%d,Destination Interface: %d,:No.of Source Interfaces: %d",
  ...
  if (rc == L7_SUCCESS &&
	  ...
	  mirrorCfgData->sessionData[sessionNum-1].mode == L7_ENABLE)
  {
    MIRROR_TRACE("\n\nmirrorDestPortRemove::Calling mirrorNimEventIntfNotify with L7_PROBE_TEARDOWN for Interface: %d.\n",intIfNum);
