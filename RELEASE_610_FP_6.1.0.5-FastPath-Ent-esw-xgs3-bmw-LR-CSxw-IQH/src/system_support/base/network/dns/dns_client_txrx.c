                                 ...
                                 dnsNameServerEntry_t **server)
{
  ...
  if (entry == L7_NULLPTR)
  {
    ...
    LOG_MSG("DNS Client: Bad active server configuration\n");
    return L7_FAILURE;
  }
  ...
  if (entry->requests == 0)
  {
    ...
    {
      LOG_MSG("DNS Client: Failed to create socket\n");
      return L7_FAILURE;
    }
    ...
    {
      L7_LOGFNC( L7_LOG_SEVERITY_ALERT, "dnsNameServerOpen(): osapiSetsockopt SO_REUSEADDR failed errorno %d\n",
                   osapiErrnoGet());
      return L7_FAILURE;
    }
    ...
    if (family == L7_AF_INET)
    {
      ...
      {
        LOG_MSG("DNS Client: Set socket SO_BROADCAST option error");
      ...
      {
         L7_LOGFNC( L7_LOG_SEVERITY_WARNING, "dnsNameServerOpen(): osapiSetsockopt SO_BROADCAST failed\n");
      }
      ...
      {
        LOG_MSG("DNS Client: Bind error on addr 0x%X\n", serverAddr);
        osapiSocketClose(entry->socket);
        return L7_FAILURE;
      }
    }
    ...
    {
      LOG_MSG("DNS Client: Invalid Family");
      osapiSocketClose(entry->socket);
      return L7_FAILURE;
    }
  }
...
L7_RC_t dnsNameServerPacketReceive(void)
{
  ...
  if (status > 0)
  {
    ...
    for (i = 0; i < L7_DNS_NAME_SERVER_ENTRIES; i++)
    {
      ...
      {
        LOG_MSG("DNS Client: received invalid packet length - %u\n", length);
        continue;
      }
      ...
      {
        LOG_MSG("DNS Client: received invalid packet, not a response\n");
        continue;
      }
      ...
      {
        LOG_MSG("DNS Client: received invalid response packet\n");
      ...
      {
        LOG_MSG("DNS Client: received response for invalid request id %u\n", id);
        dnsOprData->counters.martians++;
        continue;
      }
      ...
      {
        LOG_MSG("DNS Client: received response for request id %u with mismatched question\n", id);
        dnsOprData->counters.unparsed++;
        continue;
      }
      ...
      {
        LOG_MSG("DNS Client: failed to send response message for request id %u\n", id);
        continue;
      }
    }
    osapiSemaGive(dnsSemaphore);
                                ...
                                L7_ushort16     length)
{
  ...
  if (family == L7_AF_INET)
  {
    if (osapiSocketSendto(server->socket,
                          ...
                          length,
                          L7_NULL,
                          (L7_sockaddr_t *)&saddr.u.sa4,sizeof(saddr),
                          &bytes) != L7_SUCCESS)
    {
      osapiSocketClose(server->socket);
      osapiSemaGive(dnsSemaphore);
      L7_LOG(L7_LOG_SEVERITY_ERROR, L7_DNS_CLIENT_COMPONENT_ID,
  ...
  {
    osapiSemaGive(dnsSemaphore);
    LOG_MSG("DNS Client: Failed to send all of the query\n");
    return L7_FAILURE;
  }
  server->requests++;
  if (family == L7_AF_INET)
