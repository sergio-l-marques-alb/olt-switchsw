                                          ...
                                          L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  if (nvStoreRegister(sntpNotifyFunctionList))
  {
    ...
    *pReason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    LOG_MSG("sntpCnfgrInitPhase2Process: Could not register with nvStore\n");
    return L7_ERROR;
  }

  sntpCnfgrStateSet(SNTP_PHASE_INIT_2);
#if defined (L7_STACKING_PACKAGE)
  sysapiHpcReceiveCallbackRegister(sntpHandleStackBroadcast, L7_SNTP_COMPONENT_ID);
#endif
  return L7_SUCCESS;
                                          ...
                                          L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  {
    LOG_MSG("%s: %d: sntpCnfgrInitPhase3Process: SNTP Cfg unable to read \
...
static void sntpCfgTask()
{
  ...
  while (L7_TRUE)
  {
    ...
    {
      LOG_MSG("sntpCfgTask: Bad status on receive message from cfg queue %08x", status);
    }
  } /* end while */
}
...
static L7_RC_t sntpCfgTaskCreate()
{
  ...
  if (sntpCfg == L7_NULL)
  {
    LOG_MSG("%s: %d: sntpCfgTaskCreate: Error allocating CFG data\n",
            __FILE__, __LINE__);
    cbData.correlator = L7_NULL;
    cbData.asyncResponse.rc = L7_ERROR;
  ...
  {
    LOG_MSG("sntpCfgTaskCreate: Error starting SNTP Client");
    cbData.correlator = L7_NULL;
    cbData.asyncResponse.rc = L7_ERROR;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    LOG_MSG("sntpCfgTaskCreate: SNTP unable to create message queue");
    cbData.correlator = L7_NULL;
    cbData.asyncResponse.rc = L7_ERROR;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    LOG_MSG("sntpCfgTaskCreate: Unable to create SNTP processing task\n");
    cbData.correlator = L7_NULL;
    cbData.asyncResponse.rc = L7_ERROR;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    LOG_MSG("sntpCfgTaskCreate: Unable to syncronize with SNTP Cfg processing task");
    return L7_FALSE;
  }
  return L7_SUCCESS;
}
