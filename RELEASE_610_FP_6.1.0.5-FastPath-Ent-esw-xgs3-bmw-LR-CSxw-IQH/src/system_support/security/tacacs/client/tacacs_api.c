                               ...
                               L7_uint32  correlator)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_DEBUG, L7_TACACS_COMPONENT_ID, "TACACS+: user name is too long.");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_DEBUG, L7_TACACS_COMPONENT_ID, "TACACS+: password is too long.");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_DEBUG, L7_TACACS_COMPONENT_ID, "TACACS+: user name in enable request is missing.");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_DEBUG, L7_TACACS_COMPONENT_ID,
            "TACACS+: received request from unregistered componentId %u.", request->component);
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_DEBUG, L7_TACACS_COMPONENT_ID,
  ...
  {
    LOG_MSG("TACACS+: authentication error, failed to send TACACS_EVENT_AUTH_REQUEST.");
    osapiSemaGive(tacacsSemaphore);
    return L7_FAILURE;
  }
                                ...
                                L7_uint32  correlator)
{
  ...
  if (tacacsNotifyList[request->component].notifyFunction == L7_NULLPTR)
  {
    LOG_MSG("TACACS+: received request from unregistered componentId %u.", request->component);
    osapiSemaGive(tacacsSemaphore);
    return L7_FAILURE;
  }
  ...
  if (tacacsAuthSessionGet(request, correlator, &session) != L7_SUCCESS)
  {
    LOG_MSG("TACACS+: authorization error, no authorization sessions available");
    osapiSemaGive(tacacsSemaphore);
    return L7_FAILURE;
  }
  ...
  if (osapiMessageSend(tacacsQueue,
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    LOG_MSG("TACACS+: authorization error, failed to send TACACS_EVENT_AUTH_REQUEST.");
    osapiSemaGive(tacacsSemaphore);
    return L7_FAILURE;
  }
                                ...
                                L7_uchar8  *serverAddress)
{
  ...
  if (i < L7_TACACS_MAX_SERVERS)
  {
    ...
    else
    {
      ...
      strcpy(tacacsCfg->servers[i].hostName.host.hostName,
                 serverAddress);
      tacacsCfg->servers[i].hostName.hostAddrType =
                         L7_IP_ADDRESS_TYPE_DNS;
      msg.event = TACACS_EVENT_HOSTNAME_RESOLVE;
      strcpy(msg.data.hostName, serverAddress);
      if (osapiMessageSend(tacacsQueue, &msg, sizeof(tacacsMsg_t),
                           L7_NO_WAIT, L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
      {
        LOG_MSG("TACACS+: DNS Resolution error, failed to send TACACS_EVENT_HOSTNAME_RESOLVE.");
      }
    }
    tacacsCfg->header.dataChanged = L7_TRUE;
    osapiSemaGive(tacacsSemaphore);
                                   ...
                                   L7_uchar8  *serverAddress)
{
  ...
  if (found == L7_TRUE && ipAddr != 0)
  {
    ...
    if (osapiMessageSend(tacacsQueue,
                         ...
                         L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
    {
      LOG_MSG("TACACS+: failed to send server remove message\n");
    }
