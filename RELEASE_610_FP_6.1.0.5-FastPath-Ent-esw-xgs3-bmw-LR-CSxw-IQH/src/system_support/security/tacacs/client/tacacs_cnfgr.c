...
L7_RC_t tacacsHandleCnfgrCmd(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if ( pCmdData != L7_NULL )
  {
    if (pCmdData->type == L7_CNFGR_RQST)
    {
      ...
      if ((request > L7_CNFGR_RQST_FIRST) && (request < L7_CNFGR_RQST_LAST))
      {
        ...
        switch (command)
        {
          case L7_CNFGR_CMD_INITIALIZE:
            switch (request)
            {
              ...
              default:
                ...
                LOG_MSG("TACACS+: Invalid Request in Configurator"
                    "Command Initialize.\n");
                break;
          ...
          default:
            ...
            LOG_MSG("TACACS+: Invalid Command/Event Pair.\n");
            break;
      ...
      else
      {
        ...
        LOG_MSG("TACACS+: Invalid Request.\n");
    ...
    else
    {
      ...
      LOG_MSG("TACACS+: Invalid Command Type.\n");
  ...
  else /* cmdData is NULL */
  {
    correlator = L7_NULL;
    reason = L7_CNFGR_ERR_RC_INVALID_CMD;
    LOG_MSG("TACACS+: Invalid Command Pointer.\n");
...
void tacacsCnfgrFiniPhase1Process()
{
  ...
  LOG_MSG("TACACS+: Failed Intialization.\n");
