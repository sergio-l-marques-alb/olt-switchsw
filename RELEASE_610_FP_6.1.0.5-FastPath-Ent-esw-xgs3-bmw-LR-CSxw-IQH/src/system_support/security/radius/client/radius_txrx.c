...
L7_RC_t radiusServerOpen(radiusServerEntry_t *newEntry)
{
    ...
    )
  {
    ...
    {
      LOG_MSG("%s(): dns resolution is not successful for server: %s ! ",
                 __FUNCTION__,
  ...
  */
  ...
  {
    LOG_MSG("RADIUS: %s(): Failed to create socket\n",__FUNCTION__);
    return L7_FAILURE;
  }
...
void radiusPacketReceive(void)
{
  ...
  */
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_RADIUS_COMPONENT_ID, "RADIUS: Could not get the Task "
    ...
    */
       ...
       serverEntry = serverEntry->nextEntry)
    {
      ...
      if (packetSock == L7_NULL)
      {
        ...
        {
          LOG_MSG("RADIUS: Could not allocate a buffer for the packet\n");
          return;
        }
      ...
      if(rc != L7_SUCCESS) 
      {
        ...
        switch(rxStatus) 
        {
            ...
            default:
                 LOG_MSG("RADIUS: (): packet receive failed with other status.\n"); 
                 break;
        }
        if(L7_ERROR == rc ) 
        {        
    ...
    for(i=0;i<radiusGlobalData.totalNwInterfaces;i++) 
    {
      ...
      if (packetIntf == L7_NULL)
      {
        ...
        {
          LOG_MSG("RADIUS: Could not allocate a buffer to receive on Intf Socket\n");
          return;
        }
      ...
      if(L7_SUCCESS != rc) 
      {
        LOG_MSG("%s(): Failed to receive on socket: %d bound on Local IP: %s, closing socket\n",
                     __FUNCTION__,interface[i].socket,
                     osapiInet_ntoa(interface[i].ipAddr));
  ...
  serverEntry = server;
    ...
    */
    ...
    if(L7_NULL != nwSocket.ipAddr)    
    {
      ...
      {
		L7_LOG(L7_LOG_SEVERITY_NOTICE,  L7_RADIUS_COMPONENT_ID, "RADIUS: Server address doesn't "
    ...
    */
    if (RADIUS_LG_BUF_SIZE >= (pktLength + (L7_RADIUS_MAX_SECRET+1)))
    {
      ...
      {
        LOG_MSG("RADIUS: (): Failed to process received packet \n");
        *status = L7_RADIUS_RX_STS_ERROR_CODE_MAX;
      }
    }
    else
    {
	  L7_LOG(L7_LOG_SEVERITY_NOTICE,  L7_RADIUS_COMPONENT_ID, "RADIUS: Buffer is too small "
                         ...
                         radiusRequest_t *request)
{
  ...
  {
	L7_LOG(L7_LOG_SEVERITY_NOTICE,  L7_RADIUS_COMPONENT_ID, "RADIUS: Failed to send the request. "
                     ...
                     L7_uint32 *ipAddr)
{
  ...
  if(L7_FAILURE == rc ) 
  {
    LOG_MSG("RADIUS: %s(): Could not find the ipAddr for specified socket : %d\n",
...
L7_RC_t radiusNwInterfaceSocketTimeStampPut(L7_uint32 interfaceSocket)
{
  ...
  if(L7_FAILURE == rc ) 
  {
    LOG_MSG("RADIUS: radiusNwInterfaceSocketTimeStampPut(): Could not find the \
...
{  
  ...
  {
    LOG_MSG("RADIUS: %s(): No room for socket in data base. \n",__FUNCTION__);
    return L7_FAILURE;
  }
...
L7_RC_t radiusNwInterfaceSocketClose(L7_uint32 ipAddr)
{
  ...
  if( L7_FAILURE == rc) 
  {
    LOG_MSG("RADIUS %s(): Could not find the socket bound on ip address: %s",
                     __FUNCTION__,osapiInet_ntoa(ipAddr));     
  }
  return rc;
}
