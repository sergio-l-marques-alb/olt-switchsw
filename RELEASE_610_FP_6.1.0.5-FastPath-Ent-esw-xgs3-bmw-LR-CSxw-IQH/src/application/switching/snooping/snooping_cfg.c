...
L7_RC_t snoopSave(void)
{
  ...
  if (snoopHasDataChanged() == L7_TRUE)
  {
         ...
         cbIndex++, pSnoopCB++)
    {
      ...
      {
        LOG_MSG("snoopSave: Error on call to sysapiCfgFileWrite file %s\n",
                pSnoopCB->snoopCfgData->cfgHdr.filename);
        rc = L7_FAILURE;
        break;
      }
...
L7_RC_t snoopRestore(void)
{
  ...
  while (pSnoopL3Entry)
  {
    if (snoopL3EntryDelete(&pSnoopL3Entry->snoopL3InfoDataKey.mcastGroupAddr,
        ...
        != L7_SUCCESS)
    {
      LOG_MSG("snoopRestore: Error cleaning the snoopL3Table\n");
      break;
    }
    memset(&mcastSrcAddr, 0x00, sizeof(L7_inet_addr_t));
    memset(&mcastGrpAddr, 0x00, sizeof(L7_inet_addr_t));
...
L7_RC_t snoopApplyConfigData(snoop_cb_t *pSnoopCB)
{
  ...
  if (snoopAdminModeApply(pSnoopCfg->snoopAdminMode, 0, 0, pSnoopCfg->snoopAdminIGMPPrio, pSnoopCB) != L7_SUCCESS)
  {
    LOG_MSG(
                                     ...
                                     snoop_cb_t *pSnoopCB)
{
  switch (pSnoopCB->family)
  {
    ...
    case L7_AF_INET6:
      ...
      break;
    default:
      LOG_MSG("snoopBuildDefaultIntfConfigData: Invalid family passed\n");
  }
}
                                         ...
                                         snoopIntfCfgData_t *pCfg)
{
  ...
  for(vIdx = 1 ; vIdx < (L7_DOT1Q_MAX_VLAN_ID + 1) ; vIdx++)
  {
    ...
    {
      L7_VLAN_CLRMASKBIT(pCfg->vlanStaticMcastRtr, vIdx);
    }
    else
    {
      L7_VLAN_SETMASKBIT(pCfg->vlanStaticMcastRtr, vIdx);
    }
  }
...
void snoopIGMPBuildDefaultConfigData(L7_uint32 ver)
{
  ...
  pSnoopCfg->snoopAdminIGMPPrio = SNOOP_IGMP_DEFAULT_PRIO;  /* PTin added */
...
{
  L7_uint32 vlanId = 0;
  L7_int port, xlate_once;                // PTin added
  L7_uint16 vlan_original, vlan_to_use;   // PTin added

  while (snoopNextVlanGet(vlanId, &vlanId) == L7_SUCCESS)
  {
    // PTin added
    xlate_once=L7_FALSE;
    vlan_original=vlanId;
    for (port=0; port<L7_SYSTEM_N_INTERF; port++)
    {
      // Determine original vlan
      if (ptin_oldvlan_translate_get(port,&vlan_to_use,vlan_original)==L7_SUCCESS)
      {
        vlanId = vlan_to_use;
        xlate_once=L7_TRUE;
      }
...
L7_RC_t snoopDebugRestore(void)
{
       ...
       cbIndex++, pSnoopCB++)
  {
    if (pSnoopCB->family == L7_AF_INET)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_MSG("snoopDebugRestore: IGMP Snooping Debug config restore failed");
        break;
      }
    }
    else if (pSnoopCB->family == L7_AF_INET6)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_MSG("snoopDebugRestore: MLD Snooping Debug config restore failed");
        break;
      }
    }
