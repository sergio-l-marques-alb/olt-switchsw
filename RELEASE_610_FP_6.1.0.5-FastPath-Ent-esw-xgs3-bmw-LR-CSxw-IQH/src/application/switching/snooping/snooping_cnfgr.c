 ...
 */
  ...
  else
  {
                         ...
                         L7_WAIT_FOREVER, L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
    {
       ...
       {
          LOG_MSG("snoopApiCnfgrCommand: Failed to give msgQueue semaphore\n"); 
       }
    }
  }
}
...
L7_RC_t snoopInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
    LOG_MSG("snoopInit: msgQueue creation error.\n");
  ...
  pSnoopEB->snoopAvlTreeSema = osapiSemaBCreate(OSAPI_SEM_Q_FIFO, OSAPI_SEM_FULL);
  if (pSnoopEB->snoopMsgQSema == L7_NULL || pSnoopEB->snoopAvlTreeSema == L7_NULL)
  {
    LOG_MSG("snoopInit: Unable to create snoop message queue semaphore\n");
    cbData.correlator = pCmdData->correlator;
    cbData.asyncResponse.rc = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    LOG_MSG("snoopInit: Failed to post message. msgqueue is full\n");
    cbData.correlator = pCmdData->correlator;
    cbData.asyncResponse.rc = L7_FAILURE;
  ...
  *pReason    = 0;
  ...
  if (snoopEBInit() != L7_SUCCESS)
  {
    ...
    *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    snoopRC   = L7_ERROR;
    LOG_MSG("snoopCnfgrInitPhase1Process: snoopEBInit failed\n");
    return snoopRC;
  }
  ...
  if (snoopCB == L7_NULLPTR)
  {
    ...
    *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    snoopRC   = L7_ERROR;
    LOG_MSG("snoopCnfgrInitPhase1Process: snoopCB Allocation failed\n");
    return snoopRC;
  }
                            ...
                            L7_IGMP_SNOOPING_FEATURE_SUPPORTED) == L7_TRUE)
  {
    if (snoopCBInit(cbIndex, L7_AF_INET) != L7_SUCCESS) 
    {
      ...
      *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
      snoopRC   = L7_ERROR;
      LOG_MSG("snoopCnfgrInitPhase1Process: snoopCB[%d] Allocation failed\n",
               cbIndex);
      return snoopRC;
    }
    cbIndex++;
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  if (nimRegisterIntfChange(L7_SNOOPING_COMPONENT_ID, snoopIntfChangeCallback) != L7_SUCCESS)
  {
    LOG_MSG("snoopCnfgrInitPhase2Process: IGMP Snooping: Unable to register with NIM\n");
  ...
  {
    LOG_MSG("snoopCnfgrInitPhase2Process: Unable to register for VLAN change callback\n");
  ...
  if (cnfgrIsFeaturePresent(L7_DOT1S_COMPONENT_ID, L7_DOT1S_FEATURE_ID) == L7_TRUE)
  {
    ...
    {
      LOG_MSG("snoopCnfgrInitPhase2Process: Unable to register for dot1s interface change callback\n");
  ...
  if (snoopCBGet(L7_AF_INET) != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("snoopCnfgrInitPhase2Process: Unable to register with sim for address change events\n");
    }
  }
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  {
    snoopRC = snoopCnfgrConfigFileReadApply(pSnoopCB, pResponse, pReason);
    if (snoopRC != L7_SUCCESS)
    {
      LOG_MSG("snoopCnfgrInitPhase3Process: Failed to read configuration file\n");
      return snoopRC;
    }
  }
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((snoopRC = snoopCnfgrInitPhase3Process( &response, &reason )) != L7_SUCCESS)
                {
...
L7_RC_t snoopEBInit(void)
{
  ...
  {
    LOG_MSG("snoopEBInit: Error allocating data for snoop AVL Tree \n");
    return L7_FAILURE;
  }
  ...
  // PTin added
  // Create Heap for IP AVL tree
  pSnoopEB->snoopIpTreeHeap = 
                          (avlTreeTables_t *)osapiMalloc(L7_SNOOPING_COMPONENT_ID, 
                          L7_MAX_GROUP_REGISTRATION_ENTRIES *
                          sizeof(avlTreeTables_t)); 
  pSnoopEB->snoopIpDataHeap = (snoopIpInfoData_t *)osapiMalloc(L7_SNOOPING_COMPONENT_ID, 
                              L7_MAX_GROUP_REGISTRATION_ENTRIES *
                              sizeof(snoopIpInfoData_t)); 

  if ((pSnoopEB->snoopIpTreeHeap == L7_NULLPTR) ||
      (pSnoopEB->snoopIpDataHeap == L7_NULLPTR)
     )
  {
    LOG_MSG("snoopEBInit: Error allocating data for snoop IP AVL Tree \n");
    return L7_FAILURE;
  }

  // AVL Tree creations - snoopIpAvlTree
  avlCreateAvlTree(&(pSnoopEB->snoopIpAvlTree),  pSnoopEB->snoopIpTreeHeap, 
                   pSnoopEB->snoopIpDataHeap, L7_MAX_GROUP_REGISTRATION_ENTRIES, 
                   sizeof(snoopIpInfoData_t), 0x10, 
                   sizeof(snoopIpInfoDataKey_t));
  // PTin end
  ...
  {
    LOG_MSG("snoopEBInit: Error allocating data for snoop L3 AVL Tree \n");
    return L7_FAILURE;
  }

  /* AVL Tree creations - snoopAvlTree*/
  avlCreateAvlTree(&(pSnoopEB->snoopL3AvlTree),  pSnoopEB->snoopL3TreeHeap,
                   pSnoopEB->snoopL3DataHeap, L7_MAX_GROUP_REGISTRATION_ENTRIES,
  ...
  */
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_WARNING, L7_SNOOPING_COMPONENT_ID,
  ...
  {
    LOG_MSG("snoopEBInit: Error allocating buffers for timers\n");
    return L7_FAILURE;
  }
  ...
  {
     LOG_MSG("snoopEBInit: Falied to allocate memory for snoopIntfInfo\n");
     return L7_FAILURE;
  }
  ...
  { 
    LOG_MSG("snoopEBInit: Falied to allocate memory for snoopIntfMapTbl\n");
    return L7_FAILURE;
  }
  ...
  /* Timer Initializations */
	...
	{
		LOG_MSG("snoopEBInit: Falied to allocate memory for Snooping timer buffers\n");
		return L7_FAILURE;
	}
  ...
  {
    LOG_MSG("snoopEBInit: App Timer Initialization Failed.\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("snoopEBInit: Entry App Timer free list alloc Failed.\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("snoopEBInit: Entry App Timer ctrl blk list alloc Failed.\n");
    return L7_FAILURE;
  }
  ...
  for (idx = 0; idx < SNOOP_ENTRY_TIMER_BLOCKS_COUNT; idx++)
  {
    ...
    {
      LOG_MSG("snoopEBInit: snoopEntry App Timer[%d] Initialization Failed.\n", idx);
      return L7_FAILURE;
    }
  ...
  if ((pSnoopEB->handleListMemHndl =
        (handle_member_t*) osapiMalloc (L7_SNOOPING_COMPONENT_ID,
                        ...
                        == L7_NULLPTR)
  {
    LOG_MSG("snoopEBInit: Error allocating Handle List Buffers\n");
    return L7_FAILURE;
  }
  if(handleListInit (L7_SNOOPING_COMPONENT_ID, maxTimers,
                  ...
                  != L7_SUCCESS)
  {
    LOG_MSG("snoopEBInit: Unable to create timer handle list\n");
    return L7_FAILURE;
  }
...
void snoopEBInitUndo(void)
{
  ...
  // PTin added
  // Destroy the IP AVL Tree
  if (pSnoopEB->snoopIpTreeHeap != L7_NULLPTR)
  {
    osapiFree(L7_SNOOPING_COMPONENT_ID, pSnoopEB->snoopIpTreeHeap);
    pSnoopEB->snoopIpTreeHeap = L7_NULLPTR;
  }
  
  if (pSnoopEB->snoopIpDataHeap != L7_NULLPTR)
  {
    osapiFree(L7_SNOOPING_COMPONENT_ID, pSnoopEB->snoopIpDataHeap);
    pSnoopEB->snoopIpDataHeap = L7_NULLPTR;
  }
  // PTin end
...
L7_RC_t snoopCBInit(L7_uint32 cbIndex, L7_uchar8 family)
{
  ...
  /* Control Block initialization */
   ...
   {
     LOG_MSG("snoopCBInit: Error allocating data for mrouter timer AVL Tree \n");
     return L7_FAILURE;
   }
   ...
   {
     LOG_MSG("snoopCBInit: Error allocating data for oper data AVL Tree \n");
     return L7_FAILURE;
   }
  ...
  {
    LOG_MSG("snoopCBInit: Memory insufficent for snoop configuration structure");
  ...
  {
    LOG_MSG(
    "snoopCBInit: Memory insufficent for snoop debug configuration structure");
    return L7_FAILURE; /* Memory Allocation failed! */
  }
...
L7_RC_t snoopStartTask(void)
{
  ...
  if (snoopEB.snoopTaskId == L7_ERROR)
  {
    LOG_MSG("snoopStartTask: Could not create task snoopTask\n");
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit (L7_IGMP_SNOOPING_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("snoopStartTask: Unable to initialize snoopTask()\n");
    return(L7_FAILURE);
  }
...
* @end
*********************************************************************/
L7_RC_t snoopCnfgrConfigFileReadApply(snoop_cb_t *pSnoopCB, 
                 L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  {
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    LOG_MSG("snoopCnfgrConfigFileReadApply: Error retrieving config sysapiCfgFileGet().\n");
    return L7_FAILURE;
  }
  ...
  {
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    LOG_MSG("snoopCnfgrConfigFileReadApply: Error applying config snoopApplyConfigData().\n");
    return L7_FAILURE;
  }
