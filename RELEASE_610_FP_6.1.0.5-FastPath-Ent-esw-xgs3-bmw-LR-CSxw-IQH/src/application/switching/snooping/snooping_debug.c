...
#define SNOOP_DEBUG_PACKET_RX_FORMAT "Pkt RX - Intf: %d/%d/%d(%d), Vlan_Id:%d SrcMac: %s DestMac: %s SrcIP: %s DestIP: %s Type: %s Group: %s\n"

#define SNOOP_DEBUG_PACKET_RX_DROP_FORMAT "Pkt RX - %s\n"
#define SNOOP_DEBUG_PACKET_TX_FORMAT "Pkt TX - Intf: %d/%d/%d(%d), Vlan_Id:%d SrcMac: %s DestMac: %s SrcIP: %s DestIP: %s Type: %s Group: %s\n"
  ...
  if (pSnoopCB->snoopDebugPacketTraceRxFlag == L7_TRUE)
  {
        ...
        == L7_SUCCESS)
    {
      SNOOP_USER_TRACE_RX(SNOOP_DEBUG_PACKET_RX_FORMAT,
                          unit, slot, port, mcastPacket->intIfNum, 
                             ...
                             L7_uchar8 *payload, L7_uchar8 family)
{
  ...
  if (intIfNum && usmDbUnitSlotPortGet(intIfNum, &unit, &slot, &port) == L7_SUCCESS)
  {
    SNOOP_USER_TRACE_TX(SNOOP_DEBUG_PACKET_TX_FORMAT,
                        unit, slot, port, intIfNum, vlanId, 
...
snoopDebugTraceEnable (void)
{
  ...
  {
    SNOOP_DEBUG_PRINTF ("SNOOP Debug Tracing is already Enabled.\n");
    return (L7_FAILURE);
  }

  snoopIsDebugEnabled = L7_TRUE;
  SNOOP_DEBUG_PRINTF ("SNOOP Debug Tracing is Enabled.\n");
