...
#define SNOOP_TRACE(traceFlag,family, __fmt__, __args__...)  { \
   if (snoopDebugTraceFlagCheck ((traceFlag),(family)) == L7_TRUE)\
   {\
     ...
     (void)osapiSnprintf (__buf1__, SNOOP_MAX_DEBUG_MSG_SIZE, __fmt__, ## __args__);  \
       ...
       time.hours, time.minutes, time.seconds, ((family) == L7_AF_INET)? "IGMP" : "MLD",\
