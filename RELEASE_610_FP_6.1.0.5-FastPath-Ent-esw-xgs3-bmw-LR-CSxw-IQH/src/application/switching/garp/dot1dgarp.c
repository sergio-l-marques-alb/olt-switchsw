...
static void garpIntfEventActiveProcess(L7_uint32 intIfNum);
...
static GARPStatus garpIfStateChange(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  {  
                         ...
                         L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
    {
        ...
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_GARP_COMPONENT_ID,
            "garpIfStateChange: QUEUE SEND FAILURE: intIfNum= %d, event = %d, correlator = %d."
            " The garpQueue is full, logs specifics of the message content like"
            " internal interface number, type of message etc.", intIfNum, event, correlator);  
        LOG_ERROR(intIfNum);
    }
    else
    {
  ...
  if (garpPduQueue != L7_NULL) 
  {
          ...
          ((garp_app==GARP_GMRP_APP) && (garpGetGmrpEnabled()==L7_ENABLE))) 
    {
       ...
       { 
         ...
         {
           L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_GARP_COMPONENT_ID,
               "GarpSendPDU: QUEUE SEND FAILURE: intIfNum= %d, vlan_id = %d, bufHandle = %d."
               " The garpPduQueue is full, logs specific of the GPDU, internal interface "
               "number, vlan id, buffer handle etc.", intIfNum, vlan_id, bufHandle); 
         }
         else
         {
             osapiSemaGive(garpInfo.msgQSema);
...
void garpCPTask(void)
{
  ...
  {
    LOG_MSG("garpCPTask: Failed to create msgQSema, exiting.\n");
    return;
  }
  ...
  {
    LOG_MSG("garpCPTask: Failed to create garpQueue or garpPduQueue , exiting.\n");
  }
  else
  {
    ...
    if (init_garp_apps() == GARP_SUCCESS)
    {
      ...
      while (1)
      {
         ...
         * data. Wait until data is available. */
         ...
         {
             LOG_MSG("Failure taking GARP message queue semaphore");
             continue;
         }
          ...
          if (status != L7_SUCCESS)
          {
              LOG_MSG("Bad status %d from garpQueue or garpPduQueue\n", status);
              continue;
          }
        ...
        case G_PORT_CONN:
        case G_PORT_DISCONN:
            sysapiPrintf("Reached G_PORT_CONN or G_PORT_DISCONN \n");
    ...
    {
      LOG_MSG("garpCPTask: Failed to initialize Garp, exiting.\n");
    } 
  }
...
L7_RC_t garpIntfChangeProcess(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  if (GARPCB != L7_NULL)
  {
        ...
        (garpIsValidIntfType(sysIntfType) == L7_TRUE))
    {
      ...
      case L7_ATTACH:
        rc = garpApplyIntfConfigData(intIfNum);
        break;
          ...
          case VLAN_DELETE_PENDING_NOTIFY:
               ...
               if(gvrp_event == 1)
               {
                 if (GarpBuildCommand(&cmd, GARP_GVRP_APP, GARP_DELETE_ATTRIBUTE, 
                  ...
                  {
                      LOG_MSG("GARP VLAN process: Cannot process vlan callback for vlanId %d, intIfNum %d, event %d\n", 
                              vlanId, intIfNum, event);
                  }
               }
              ...
              if (( mode==L7_ENABLE) && ( status==L7_ENABLE) )
              {
                 /* Prior to release K, this path invoked
                  ...
                  if (gmrpInstanceIndexFind(vlanId,&index) == L7_SUCCESS)
                  {
                      ...
                      {
                          LOG_MSG("GMRP VLAN process: Cannot process vlan callback for vlanId %d, intIfNum %d, event %d\n", 
                                  vlanId, intIfNum, event);
                      }
          ...
          case VLAN_DELETE_PORT_NOTIFY:
              ...
              if (( mode==L7_ENABLE) && ( status==L7_ENABLE) )
              {
                  ...
                  if (gmrpInstanceIndexFind(vlanId,&index) == L7_SUCCESS)
                  {
                      ...
                      {
                          LOG_MSG("GMRP VLAN process: Cannot process vlan change callback for vlanId %d, intIfNum %d, event %d\n", 
                                  vlanId, intIfNum, event);
                      }
...
GARPStatus GarpCreate( void *GarpCBInst)
{
  ...
  {
    LOG_MSG("GarpCreate: garpTaskSyncSema not created.");
    return GARP_FAILURE;
  }
  ...
  if (garpPtr->GarpCB_garpCCPTask == L7_ERROR)
  {
     ...
     LOG_MSG("GarpCreate(): unable to create GCCP task.\n");
  }
...
L7_RC_t garpIntfGVRPModeEnableProcess(L7_uint32 intIfNum, GARPCommand *cmd)
{
  ...
  {  
      ...
      while(L7_SUCCESS == dot1qNextStaticVlanGet(VlanId, &nextVlanId))
      {
          VlanId = nextVlanId;

          if (L7_SUCCESS == dot1qVlanMemberAndTypeGet(nextVlanId, 
                                                      i, &mode, &type))
          {            
              switch (mode) 
              {
...
L7_RC_t garpGarpModeProcess(GARPApplication application, L7_uint32 mode)
{
  ...
  L7_uint32 aqstatus,i;
    ...
    {
        LOG_MSG("unknown application: %d\n", application);
        return L7_FAILURE;
    }

    if ( (mode != GARP_ENABLE ) && (mode != GARP_DISABLE) )
    {
        LOG_MSG("unknown mode: %d\n", mode);
        return L7_FAILURE;
    }
