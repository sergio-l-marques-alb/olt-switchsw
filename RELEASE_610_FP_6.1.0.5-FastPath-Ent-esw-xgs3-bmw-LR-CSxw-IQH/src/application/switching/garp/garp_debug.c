...
static void garpTraceMsgs(L7_uchar8 *traceMsg)
{
    ...
    {                                                        
        LOG_COMPONENT_MSG(L7_GARP_COMPONENT_ID,traceMsg);
    }                                                                
    ...
    switch (msg->msgId)
    {
      ...
      case G_PORT_CONN:
        /* GARP connect port (span)*/
          GARP_TRACE_QUEUE_EVENTS("%s: G_PORT_CONN: port = %d\n", 
                           traceDirection, msg->port);
        break;

      case G_PORT_DISCONN:
          GARP_TRACE_QUEUE_EVENTS("%s: G_PORT_DISCONN: port = %d\n", 
                           traceDirection, msg->port);
        break;
      ...
      case G_INTF_STATE_CHANGE:
        GARP_TRACE_QUEUE_EVENTS("%s: G_INTF_STATE_CHANGE: port = %d, event= %d, correlator =%x\n",
                                traceDirection, msg->port, msg->msgData.nim.nim_event, 
      ...
      case G_VLAN_NOTIFY:
        GARP_TRACE_QUEUE_EVENTS("%s: G_VLAN_NOTIFY: intIfNum = %d, event = %d , \n",
                            traceDirection, msg->port, 
      ...
      case G_DOT1S_NOTIFY:
        GARP_TRACE_QUEUE_EVENTS("%s: G_DOT1S_NOTIFY: mstId = %d, intIfNum = %d, event = %d\n",
                                traceDirection, msg->msgData.dot1s.mstID, msg->port, 
...
void garpTraceMsgQueueUsage(void)
{
  ...
  if (osapiMsgQueueGetNumMsgs(garpQueue, &num) == L7_SUCCESS)
  {
      if (num > GARP_QUEUE_USAGE_FIFTY_PERCENT) 
      {
          ...
          {
              L7_LOG(L7_LOG_SEVERITY_DEBUG, L7_GARP_COMPONENT_ID,
 ...
 *********************************************************************/
 void garpDebugGmrpGidShow(L7_uint32 vlanId,L7_uint32 intIfNum)
 {
     ...
     {
         if (gid_find_port(application->g.gid, intIfNum, (void *) &my_port)!= L7_SUCCESS)
         {
             garpDebugGidShow(my_port);
         }
         else
