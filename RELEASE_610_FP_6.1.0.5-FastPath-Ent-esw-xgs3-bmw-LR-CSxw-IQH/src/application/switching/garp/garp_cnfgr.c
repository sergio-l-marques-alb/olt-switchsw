...
L7_RC_t garpCnfgrInitPhase1Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
      LOG_MSG("garpCnfgrInitPhase1Process: Unable to create garpCfgRWLock\n");
  ...
  {
      LOG_MSG("garpCnfgrInitPhase1Process: Unable to create gmrpCfgSema\n");
  ...
  {
      LOG_MSG("garpCnfgrInitPhase1Process: Unable to allocate memory\n");
...
L7_RC_t garpCnfgrInitPhase2Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if (nimRegisterIntfChange(L7_GARP_COMPONENT_ID, garpIntfChangeCallback) != L7_SUCCESS)
  {
    LOG_MSG("Unable to register port manager callback\n");
  }
  else
  {
    rc = L7_SUCCESS;
  ...
  {
    LOG_MSG("Unable to register MSTP callback\n");
  }
  ...
  {
    LOG_MSG("Unable to register Gvrp pdu receive callback\n");
    return L7_FAILURE;
  }
  ...
  if (sysNetRegisterPduReceive(&snEntry) != L7_SUCCESS)
  {
    LOG_MSG("Unable to register Gmrp pdu receive callback\n");
    return L7_FAILURE;
  }
...
L7_RC_t garpCnfgrInitPhase3Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  rc = sysapiCfgFileGet(L7_GARP_COMPONENT_ID, GARP_CFG_FILENAME, (L7_char8*)garpCfg, (L7_uint32)sizeof(garpCfg_t),
                        &garpCfg->checkSum, GARP_CFG_VER_CURRENT, garpBuildDefaultConfigData, garpMigrateConfigData);
  ...
  rc = sysapiCfgFileGet(L7_GMRP_COMPONENT_ID, GMRP_CFG_FILENAME, (L7_char8*)gmrpCfg, (L7_uint32)sizeof(gmrpCfg_t),
                        &gmrpCfg->checkSum, GMRP_CFG_VER_CURRENT, gmrpBuildDefaultConfigData, gmrpMigrateConfigData);
...
void garpCnfgrFiniPhase2Process(void)
{
  ...
  (void)nimRegisterIntfChange(L7_GARP_COMPONENT_ID, L7_NULLPTR);
