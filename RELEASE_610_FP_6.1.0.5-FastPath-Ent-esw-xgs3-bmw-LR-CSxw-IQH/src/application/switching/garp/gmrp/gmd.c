                         ...
                         L7_uint32 *created_at_index)
{
  ...
  if(rc==L7_NOT_EXIST)
  {
      if (mfdbEntryAdd(&memInfo) != L7_SUCCESS)
      {
  ...
  if (local_index == L7_NULL)
  {
    ...
    {
        LOG_MSG("GMRP failure undoing MFDB entry: vlan %d and address %s\n", vlanId, buf);
    }
  ...
  {
      LOG_MSG("GMRP failure adding DLL entry: vlan %d and address %s\n", vlanId, buf);
	  if (mfdbEntryDelete(&memInfo) != L7_SUCCESS)
      {
          LOG_MSG("GMRP failure undoing MFDB after DLL failure: vlan %d and address %s\n", vlanId, buf);
          osapiFree(L7_GMRP_COMPONENT_ID, local_index);
          return L7_FALSE;
       }
      garpInfo.infoCounters.gmrpRegistationCount--;
  ...
  {
      LOG_MSG("GMRP failure with DLL insertion: vlan %d and address %s\n", vlanId, buf);
