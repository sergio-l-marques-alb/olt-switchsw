...
L7_RC_t voiceVlanPortAdminModeGet(L7_uint32 intIfNum,VOICE_VLAN_CONFIG_TYPE_t *mode)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_DEBUG,L7_ISDP_COMPONENT_ID,"Invalid Params");
    return L7_FAILURE;
  }
...
L7_RC_t voiceVlanPortDscpSet(L7_uint32 intIfNum,L7_uint32 val)
{
  ...
  {
    LOG_MSG("%s:%d - Invalid Range for dscp val (%d). Should be between %d - %d \n",
...
L7_RC_t voiceVlanPortDeviceInfoFirstGet(L7_uint32 intIfNum,L7_uchar8 *mac_addr)
{
  ...
  {
    memset(mac_addr,0,L7_MAC_ADDR_LEN);
    /*LOG_MSG("%s:%d - Voice vlan admin mode not enabled on Iface[%d]", __FUNCTION__, __LINE__,intIfNum);*/
  ...
  {
    memset(mac_addr,0,L7_MAC_ADDR_LEN);
    /*LOG_MSG("%s:%d - No voice devices identified on the Iface[%d]", __FUNCTION__, __LINE__,intIfNum);*/
  ...
  */
  ...
  {
     memset(mac_addr,0,L7_MAC_ADDR_LEN);
     /*LOG_MSG("%s:%d - Could not find the first voice vlan device identified on the Iface[%d]", __FUNCTION__, __LINE__,intIfNum);*/
  ...
  {
    /*LOG_MSG(" 1 %s:%d - Could not find the first voice vlan device identified on the Iface[%d]", __FUNCTION__, __LINE__,intIfNum);*/
...
L7_RC_t voiceVlanPortDeviceInfoNextGet(L7_uint32 intIfNum,L7_uchar8 *mac_addr)
{
  ...
  {
     memset(mac_addr,0,L7_MAC_ADDR_LEN);
    LOG_MSG("%s:%d - No more than one voice devices identified on the Iface[%d]", __FUNCTION__, __LINE__,intIfNum);
     VOICE_VLAN_TRACE(VOICE_VLAN_TRACE_DEBUG,"%s:%d - No more than one voice devices identified on the Iface[%d]",
  ...
  */
  ...
  {
    memset(mac_addr,0,L7_MAC_ADDR_LEN);
     LOG_MSG("%s:%d - Could not find anymore voice vlan devices on the Iface[%d]", __FUNCTION__, __LINE__,intIfNum);
     VOICE_VLAN_TRACE(VOICE_VLAN_TRACE_DEBUG,"%s:%d - Could not find anymore voice vlan devices on the Iface[%d]",
                      __FUNCTION__,__LINE__,intIfNum);
    return L7_FAILURE;
  }
  else
