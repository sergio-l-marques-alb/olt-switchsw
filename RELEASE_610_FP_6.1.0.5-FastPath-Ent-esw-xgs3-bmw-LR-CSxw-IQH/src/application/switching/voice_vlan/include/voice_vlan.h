...
typedef enum
{
  VOICE_VLAN_TRACE_DISABLED = 0,
  VOICE_VLAN_TRACE_HIGH,
  VOICE_VLAN_TRACE_MEDIUM,
  VOICE_VLAN_TRACE_DEBUG
} voiceVlanTraceLevel_t;

extern voiceVlanTraceLevel_t voiceVlanTrace;
#define VOICE_VLAN_TRACE(level, __fmt__, __args__...)                \
  if (level <= (voiceVlanTrace))                                     \
  {                                                                  \
    L7_char8  __buf1__[256];                                         \
    osapiSnprintf(__buf1__, sizeof(__buf1__), __fmt__, ## __args__); \
...
* @end
*********************************************************************/
L7_RC_t voiceVlanPortVoipDeviceAdd(L7_uint32 intIfNum,L7_uchar8 *mac, L7_BOOL clientAuthState);
