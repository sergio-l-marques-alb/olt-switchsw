...
L7_RC_t voiceVlanDeviceInfoDBInit(L7_uint32 nodeCount)
{
  ...
  {
    LOG_MSG("voiceVlanDeviceInfoDBInit: Error allocating data for dot1x DB Tree \n");
    return L7_FAILURE;
  }
  ...
  voiceVlanDeviceInfo_t  newNode,*tmpNode=L7_NULLPTR,*retNode = L7_NULLPTR;
   ...
   tmpNode = voiceVlanDeviceInfoGet(mac_addr);
    ...
    if (tmpNode == L7_NULLPTR)
    {
      ...
      if (retNode == &newNode)
      {
        LOG_MSG("%s:%d Error Adding the node to the Voice Vlan Device Info Tree for mac address: %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x ,phyintf %d, voice vlan : %d \n",
