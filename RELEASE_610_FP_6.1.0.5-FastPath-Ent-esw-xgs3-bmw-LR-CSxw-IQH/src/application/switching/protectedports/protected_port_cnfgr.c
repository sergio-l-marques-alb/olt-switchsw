...
extern L7_INTF_MASK_t protectedPortAttachedPortMask;
                                            ...
                                            L7_CNFGR_ERR_RC_t * pReason)
{
    ...
    *pReason = 0;
    ...
    protectedPortIntfInfo = 
              ( protectedPortIntfInfo_t *) osapiMalloc(L7_PROTECTED_PORT_COMPONENT_ID,
                          sizeof(protectedPortIntfInfo_t) * L7_MAX_INTERFACE_COUNT);

    if ( (protectedPortCfg == L7_NULLPTR) || (protectedPortIntfInfo == L7_NULLPTR))
    {
      osapiFree(L7_PROTECTED_PORT_COMPONENT_ID, protectedPortCfg);
      osapiFree(L7_PROTECTED_PORT_COMPONENT_ID, protectedPortIntfInfo);
  ...
  memset((void *)protectedPortCfg, 0x00,
           (L7_uint32) sizeof(protectedPortCfg_t));
  memset((void *)protectedPortIntfInfo, 0x00,
           sizeof(protectedPortIntfInfo_t) * (L7_MAX_INTERFACE_COUNT));
  memset((void *)&protectedPortAttachedPortMask, 0x00,sizeof(protectedPortAttachedPortMask));

    if (osapiRWLockCreate(&protectedPortCfgRWLock,
                          OSAPI_RWLOCK_Q_PRIORITY) == L7_FAILURE)
    {
	L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_PROTECTED_PORT_COMPONENT_ID, "protectedPortCnfgrInitPhase1Process: Unable "
                                            ...
                                            L7_CNFGR_ERR_RC_t * pReason)
{
    ...
    *pReason = 0;
    ...
    if (nimRegisterIntfChange(L7_PROTECTED_PORT_COMPONENT_ID,
                              protectedPortIntfChangeCallback) != L7_SUCCESS)
    {
	L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_PROTECTED_PORT_COMPONENT_ID, "protectedPortCnfgrInitPhase2Process: Unable "
...
void protectedPortCnfgrFiniPhase1Process()
{
    ...
    if(protectedPortIntfInfo != L7_NULLPTR)
    {
        osapiFree(L7_PROTECTED_PORT_COMPONENT_ID, protectedPortIntfInfo);
        protectedPortIntfInfo = L7_NULLPTR;
    }
