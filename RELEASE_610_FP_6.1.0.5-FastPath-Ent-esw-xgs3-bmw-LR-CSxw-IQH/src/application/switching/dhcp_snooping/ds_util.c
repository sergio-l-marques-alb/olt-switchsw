...
// PTin added
static L7_RC_t ptin_dsDot1qVlanEgressPortsGet(L7_uint32 vlanID, NIM_INTF_MASK_t *mask)
{
  L7_int port;
  L7_uint16 vlan;
  NIM_INTF_MASK_t tmp_mask;

  // Clear mask
  memset(mask,0x00,sizeof(NIM_INTF_MASK_t));

  // Run all interfaces
  for (port = 0; port < L7_SYSTEM_N_INTERF; port++)
  {
    // Check if there is translation entry for this vlan and interface
    if (ptin_vlan_translate_get(port,vlanID,&vlan)!=L7_SUCCESS || vlan<=1 || vlan>=4094)
      continue;
  
    // Get list of ports for this vlan and port
    if (dot1qVlanEgressPortsGet(vlan, &tmp_mask) == L7_SUCCESS)
    {
      // OR interfaces
      L7_uint32 x;
      for (x = 0; x < L7_INTF_INDICES; x++) {
        if (tmp_mask.value[x])  mask->value[x] = tmp_mask.value[x];
      }
    }
  }
...
L7_RC_t dsVlanEnableApply(L7_uint32 vlanId, L7_uint32 vlanCfg)
{
  ...
  // PTin modified (dot1qVlanEgressPortsGet)
  if (ptin_dsDot1qVlanEgressPortsGet(vlanId, &portMask) == L7_SUCCESS)
  {
    for (i = 1; i < DS_MAX_INTF_COUNT; i++)
    {
...
L7_RC_t dsIntfEnable(L7_uint32 intIfNum)
{
  ...
  if (dtlDhcpSnoopingConfig(intIfNum, L7_TRUE) != L7_SUCCESS)
  {
    ...
    nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DHCP_SNOOPING_COMPONENT_ID,
            "Failed to enable DHCP snooping in hardware on interface %s",
            ifName);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
                             ...
                             NIM_CORRELATOR_t correlator)
{
  ...
  {
     LOG_MSG("DS: NIM event Enqueue failed.\n");
  }
  if (rc == L7_SUCCESS)
  {
...
L7_RC_t dsIntfChangeProcess(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  {
    LOG_MSG("%s: %d: dsIntfChangeProcess(): Received a interface callback while outside the EXECUTE state", __FILE__, __LINE__);
    rc = L7_FAILURE;
  }
  status.response.rc = rc;
  nimEventStatusCallback(status);
...
void dsLogEthernetHeader(L7_enetHeader_t *ethHdr, dsTraceDest_t traceDest)
{
  ...
  if (traceDest == DS_TRACE_LOG)
    L7_LOG(L7_LOG_SEVERITY_WARNING, L7_DHCP_SNOOPING_COMPONENT_ID, traceBuf);
...
L7_RC_t dsDbRemoteSave()
{
 ...
 if (osapiFsFileCreate(DHCP_SNOOPING_UPLOAD_FILE_NAME, &filedesc) == L7_SUCCESS)
 {
   ...
   while(dsBindingFind(&binding,L7_MATCH_GETNEXT) == L7_SUCCESS)
   {
     ...
     if (binding.bindingType == DS_BINDING_DYNAMIC)
     {
       ...
       {
         LOG_MSG (" Problem occured while writing DHCP Snooping database into NVRAM");
 ...
 {
  LOG_MSG (" Problem with file creation while writing DHCP Snooping database");
  return L7_FAILURE;
 }
 return L7_SUCCESS;
}
...
L7_RC_t dsDbRemoteRestore()
{
 ...
 if ( ( filedesc = osapiFsOpen(DHCP_SNOOPING_DOWNLOAD_FILE_NAME)) != L7_ERROR)
 {
...
void dsDbLocalSave()
{
 ...
 {
   LOG_MSG("dsDbSaveCfg: Error during sysapiCfgFileWrite %s\n",
             L7_DHCP_SNOOPING_CFG_FILENAME);
 }
...
void dsDbLocalRestore()
{
  ...
  if (dsCfgData->dsGlobalAdminMode == L7_ENABLE)
  {
    if (sysapiCfgFileGet( L7_DHCP_SNOOPING_COMPONENT_ID,
     ...
     {
        LOG_MSG("%s: %d: dsDb Restore problem: DHCP Sboop unable to read \
   ...
   for (dbIndex = 0; dbIndex < dbEntries; dbIndex ++)
   {
      if ( dsDbCfgData.dsBindingDb[dbIndex].ipAddr > 0)
      {
         ...
         rc = dsBindingAdd(DS_BINDING_DYNAMIC,
                      ...
                      dsDbCfgData.dsBindingDb[dbIndex].vlanId,
                      0,
...
void dsTftpUploadInit()
{
  ...
  {
    LOG_MSG ("DHCP snooping database saving on nvram failed");
    return;
  }
