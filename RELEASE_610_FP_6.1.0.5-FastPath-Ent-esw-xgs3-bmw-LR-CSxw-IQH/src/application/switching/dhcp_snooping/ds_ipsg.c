                     ...
                     L7_uint32 ipAddr)
{
  ...
  if (pNode == L7_NULLPTR)
  {
    ...
    pNode->leaseStart = osapiUpTimeRaw();

    if ( ipsgBindingHwAdd(intIfNum, ipAddr, macAddr) == L7_SUCCESS)
    {
...
L7_RC_t ipsgIntfEnableApply(L7_uint32 intIfNum, L7_BOOL addingMacFilter)
{
                                ...
                                L7_MATCH_GETNEXT, &pNode)) == L7_SUCCESS)
  {
    ...
    if (dtlIpsgIpv4ClientAdd(intIfNum, clientIpAddr, clientMacAddr) != L7_SUCCESS)
    {
      L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
      L7_uchar8 ipAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
                         ...
                         L7_enetMacAddr_t *macAddr)
{
  ...
  if (( rc=dtlIpsgIpv4ClientAdd(intIfNum, clientIpAddr, clientMacAddr)) != L7_SUCCESS)
  {
    L7_uchar8 ipAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
    L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
    static L7_uint32 lastMsg = 0;
