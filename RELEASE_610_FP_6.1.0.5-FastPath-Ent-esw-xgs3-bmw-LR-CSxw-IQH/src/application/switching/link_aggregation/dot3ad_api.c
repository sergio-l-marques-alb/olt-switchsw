...
// PTin added
L7_BOOL dot3adQueueIsEmpty(void)
{
  L7_uint32 numMsg;

  if (LACGetNumMsgs(&numMsg)!=L7_SUCCESS || numMsg>0)
    return L7_FALSE;
  ...
  while (rc == L7_SUCCESS)
  {
    if (nimGetIntfType(lagIntIfNum, &itype) == L7_SUCCESS)
    {
      ...
      if (itype == L7_LAG_INTF)
      {
        ...
        {
          LOG_MSG("dot3adAggSystemHashModeSet: Failed to set hash mode %d for lag %d", 
                   hashMode, lagIntIfNum);
        }
      }
    }
...
// PTin added
L7_uint8 dot3adLagPendingConfigsOnHW(void)
{
  L7_uint8 num;

  if (dtlDot3adPendingConfigsOnHW(&num)==L7_SUCCESS)
    return num;
  else
    return (L7_uint8)-1;
}
 ...
 * @end
 *********************************************************************/
L7_RC_t dot3adAdminModeSet(L7_uint32 intIfNum, L7_uint32 status)
{
  return LACIssueCmd(AGG_ADMIN_MODE, intIfNum, (void*)&status);
...
L7_RC_t dot3adLagStaticModeSet(L7_uint32 intIfNum, L7_uint32 isStatic)
{
  ...
  {
    LOG_MSG("Invalid mode for static lags%d: %u!\n",intIfNum, isStatic);
    return rc;
