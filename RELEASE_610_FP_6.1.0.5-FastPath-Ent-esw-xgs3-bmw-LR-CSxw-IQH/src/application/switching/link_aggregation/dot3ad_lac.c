...
L7_uint32 dot3adIntfChangeCallBackProcess(NIM_EVENT_COMPLETE_INFO_t eventInfo)
{
  ...
  {
  rc = nimGetIntfAdminState(intIfNum, &adminMode);
  if (rc != L7_SUCCESS)
  {
	return L7_SUCCESS;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    case L7_DOWN:
      ...
      case L7_CREATE:
        rc = dot3adIntfCreate(intIfNum,&eventInfo,&performNimCallback);
        break;

      case L7_ATTACH:
        rc = dot3adIntfAttach(intIfNum,&eventInfo,&performNimCallback);
        break;
...
void dot3ad_lac_task()
{
  ...
  rc = osapiTaskInitDone(L7_DOT3AD_TASK_SYNC);

  if (dot3ad_queue == L7_NULLPTR)
  {
    LOG_MSG("LAG: Unable to create msg queue for dot3ad_lac_task\n");
    return;
  }
...
void dot3ad_timer_task()
{
  ...
  {
    LOG_MSG("LAG: Unable to create msg queue for dot3ad_timer_task\n");
    return;
  }
...
{
  return osapiMsgQueueGetNumMsgs( dot3ad_queue, numMsg);
}
// PTin end
...
****/
  ...
  case AGG_ADMIN_MODE:
    rc = aggAdminModeSet(msg.intf, msg.intfData.p);
    break;
...
L7_RC_t aggCollDistDisable(L7_uint32 intf)
{
  ...
  */
  ...
  { 
    ...
    /*trace call*/
    TRACE_BEGIN(dot3adSystem.traceId,0xF2);
...
L7_RC_t aggCollDistEnable(L7_uint32 intf)
{
  ...
  */
  ...
  if (tmpCount == 1)/*first member in the trunk table*/
  {
    ...
    /*trace call*/
    TRACE_BEGIN(dot3adSystem.traceId,0xF2);
    TRACE_ENTER_BYTE(0x77);
    TRACE_ENTER_BYTE(attached_port->actorPortAggId);
    TRACE_ENTER_BYTE(attached_port->actorPortNum);
    TRACE_ENTER_BYTE(attached_port->partnerOperPortNumber);
    TRACE_ENTER_BYTE(attached_port->actorOperPortState);
    TRACE_ENTER_BYTE(attached_port->partnerOperPortState);
    TRACE_ENTER_BYTE(attached_port->selected);
    TRACE_ENTER_BYTE(attached_port->rxState);
    TRACE_ENTER_BYTE(attached_port->muxState);
    TRACE_ENTER_BYTE(attached_port->perState);
    TRACE_END();
  }
...
L7_RC_t aggAdminModeSet(L7_uint32 agg_intf, L7_uint32 status)
{
  ...
  if (a != L7_NULLPTR)
  {
    ...
    if (status == L7_ENABLE)
    {
	  ...
	  /* Enable lag port. Ensure nim/dtl call is made */
      rc = dot3adLihIntfAdminStateSet(a->aggId, L7_ENABLE);
        ...
        for (i = 0; i< a->currNumWaitSelectedMembers; i++)
          {
            ...
            if (p != L7_NULLPTR)
            {
		  rc = dot3adLihIntfAdminStateSet(p->actorPortNum, L7_ENABLE);
        }
	  }
              ...
              else
	{
	  ...
	  for (i = 0; i< a->currNumWaitSelectedMembers; i++)
	  {
		...
		if (p != L7_NULLPTR)
        {
		  rc = dot3adLihIntfAdminStateSet(p->actorPortNum, L7_DISABLE);
        }
	  }

	  /* Update the adminMode with the current status*/
	  a->adminMode = status;
	  /* Disable lag port. Ensure nim/dtl call is made*/
      rc = dot3adLihIntfAdminStateSet(a->aggId, L7_DISABLE);
...
L7_RC_t aggPortResetValues(dot3ad_port_t *p)
{
  ...
  p->actorPortPri = FD_DOT3AD_DEFAULT_PORT_PRIORITY;
  /*p->portIndividualAgg = L7_TRUE;*/
  /*
...
void dot3adCurrentWhileTimerExpiredToQueue(L7_uint32 portNum, L7_uint32 nullParm)
{
  ...
  if (rc == L7_ERROR)
  {
	LOG_MSG("dot3adTimerTask: Failed to put msg in timer queue....\n");
  }
                               ...
                               L7_uint32 *pIntIfNum)
{
  ...
  do
  {
    ...
    sprintf ((L7_char8 *)&(intfDescr.ifDescr), "%s %s %d",
                            ...
                            L7_STACKING_FEATURE_SUPPORTED) == L7_TRUE)
    {
      ...
      sprintf ((L7_char8 *)&(intfDescr.ifName), "%d/%d/%d",
               (L7_uchar8)L7_LOGICAL_UNIT, (L7_uchar8)platSlotLagSlotNumGet(), lagId);
    }
    else
    {
      sprintf ((L7_char8 *)&(intfDescr.ifName), "%d/%d",
               (L7_uchar8)platSlotLagSlotNumGet(), lagId);
    }

    memset((void*)&intfDescr.macroPort,0,sizeof(nimMacroPort_t));

    if ((rc = cmgrIfaceMacGet(intfIdInfo.type, lagUsp.unit,lagUsp.slot,lagId,intfDescr.macAddr.addr)) != L7_SUCCESS)
    {
      LOG_MSG("DOT3AD: failed to get the MAC address of LagID(%d)\n",lagId);
      break; /* goto while (0) */
    }
    ...
    {
      LOG_MSG("DOT3ad: failed to create the interface\n");
      break; /* goto while (0) */
    }
