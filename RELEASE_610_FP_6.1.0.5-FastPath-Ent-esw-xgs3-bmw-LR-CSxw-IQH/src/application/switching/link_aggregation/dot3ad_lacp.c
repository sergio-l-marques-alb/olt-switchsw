...
L7_RC_t dot3adCurrentWhileTimerExpired(L7_uint32 portNum, L7_uint32 nullParm)
{
  ...
  if (p->currentWhileTimer != L7_NULLPTR)
  {
    ...
    /*trace call*/
    TRACE_BEGIN(dot3adSystem.traceId,0xF1);
    TRACE_ENTER_BYTE(0xA6);
    TRACE_ENTER_BYTE(p->actorPortAggId);
    TRACE_ENTER_BYTE(p->actorPortNum);
    TRACE_ENTER_BYTE(p->partnerOperPortNumber);
    ...
    TRACE_ENTER_4BYTE(osapiUpTimeRaw());
    TRACE_END();
...
L7_RC_t dot3adRxMachinePortDisabledAction(dot3ad_port_t *p)
{
  ...
  if (moved == L7_TRUE)
  {
    ...
    if (L7_ERROR == rc)
    {
      /*trace call*/
      TRACE_BEGIN(dot3adSystem.traceId,0xF1);
      TRACE_ENTER_BYTE(lacpPortMoved);
      TRACE_ENTER_BYTE(p->actorPortAggId);
      TRACE_ENTER_BYTE(p->actorPortNum);
      TRACE_ENTER_BYTE(p->partnerOperPortNumber);
      TRACE_ENTER_BYTE(p->actorOperPortState);
  ...
  if (agg->isStatic == L7_TRUE)
  {
    ...
    if (p->rxState == EXPIRED)
    {
       ...
       if (L7_ERROR == rc)
       {
         ...
         /*trace call*/
         TRACE_BEGIN(dot3adSystem.traceId,0xF1);
         TRACE_ENTER_BYTE(lacpCurrentWhileTimerExpired);
         TRACE_ENTER_BYTE(p->actorPortAggId);
         TRACE_ENTER_BYTE(p->actorPortNum);
         TRACE_ENTER_BYTE(p->partnerOperPortNumber);
         TRACE_ENTER_BYTE(p->actorOperPortState);
       ...
       && !(p->partnerOperPortState & DOT3AD_STATE_LACP_ACTIVITY))
   {
     ...
     if (dot3adPerMachineIsEnabled(p) == L7_TRUE)
     {
       ...
       if (L7_ERROR == rc)
       {
         ...
         /*trace call*/
         TRACE_BEGIN(dot3adSystem.traceId,0xF1);
         TRACE_ENTER_BYTE(lacpDisablePerMachine);
         TRACE_ENTER_BYTE(p->actorPortAggId);
         TRACE_ENTER_BYTE(p->actorPortNum);
         TRACE_ENTER_BYTE(p->partnerOperPortNumber);
         TRACE_ENTER_BYTE(p->actorOperPortState);
     ...
     if (!p->begin && p->portEnabled && p->lacpEnabled)
     {
       ...
       if (dot3adPerMachineIsEnabled(p) == L7_FALSE )
       {
         ...
         if (L7_ERROR == rc)
         {
           ...
           /*trace call*/
           TRACE_BEGIN(dot3adSystem.traceId,0xF1);
           TRACE_ENTER_BYTE(lacpEnablePerMachine);
           TRACE_ENTER_BYTE(p->actorPortAggId);
           TRACE_ENTER_BYTE(p->actorPortNum);
           TRACE_ENTER_BYTE(p->partnerOperPortNumber);
           TRACE_ENTER_BYTE(p->actorOperPortState);
   ...
   if (p->partnerOperPortState & DOT3AD_STATE_LACP_TIMEOUT)
   {
     ...
     if (dot3adPerMachineIsSlow(p) == L7_TRUE)
     {
       ...
       if (L7_ERROR == rc)
       {
         ...
         /*trace call*/
         TRACE_BEGIN(dot3adSystem.traceId,0xF1);
         TRACE_ENTER_BYTE(lacpShortTimeOut);
         TRACE_ENTER_BYTE(p->actorPortAggId);
         TRACE_ENTER_BYTE(p->actorPortNum);
         TRACE_ENTER_BYTE(p->partnerOperPortNumber);
         TRACE_ENTER_BYTE(p->actorOperPortState);
   ...
   else
   {
     ...
     if (dot3adPerMachineIsFast(p) == L7_TRUE)
     {
       ...
       if (L7_ERROR == rc)
       {
         ...
         /*trace call*/
         TRACE_BEGIN(dot3adSystem.traceId,0xF1);
         TRACE_ENTER_BYTE(lacpLongTimeOut);
         TRACE_ENTER_BYTE(p->actorPortAggId);
         TRACE_ENTER_BYTE(p->actorPortNum);
         TRACE_ENTER_BYTE(p->partnerOperPortNumber);
         TRACE_ENTER_BYTE(p->actorOperPortState);
    ...
    if (p->selected == SELECTED)
    {
      ...
      if (dot3adMuxMachineIsAttachedState(p) == L7_TRUE)
      {
        ...
        if (L7_ERROR == rc)
        {
          ...
          /*trace call*/
          TRACE_BEGIN(dot3adSystem.traceId,0xF1);
          TRACE_ENTER_BYTE(lacpSelectedPartnerSyncTrue);
          TRACE_ENTER_BYTE(p->actorPortAggId);
          TRACE_ENTER_BYTE(p->actorPortNum);
          TRACE_ENTER_BYTE(p->partnerOperPortNumber);
          TRACE_ENTER_BYTE(p->actorOperPortState);
  ...
  else
  {
    ...
    if (dot3adMuxMachineIsCollDistState(p) == L7_TRUE)
    {
      ...
      if (L7_ERROR == rc)
      {
        ...
        /*trace call*/
        TRACE_BEGIN(dot3adSystem.traceId,0xF1);
        TRACE_ENTER_BYTE(lacpPartnerSyncFalse);
        TRACE_ENTER_BYTE(p->actorPortAggId);
        TRACE_ENTER_BYTE(p->actorPortNum);
        TRACE_ENTER_BYTE(p->partnerOperPortNumber);
        TRACE_ENTER_BYTE(p->actorOperPortState);
...
{
  return((p->periodicTimer->orig_count == DOT3AD_SLOW_PER_TIME * 1000) ?
         L7_TRUE : L7_FALSE);
  ...
  */
  ...
  if (agg->isStatic == L7_TRUE)
  {
	if (p->partnerOperPortState & DOT3AD_STATE_SYNCHRONIZATION)
    {
      if (p->selected == SELECTED)
      {
        /*partner is in sync with actor and actor has selected its aggregator*/
        /*send msg to LAC queue with lacpSelectedPartnerSyncTrue*/
        /*message E16*/
        {
          rc = LACIssueCmd(lacpSelectedPartnerSyncTrue, p->actorPortNum, L7_NULLPTR);
...
L7_RC_t dot3adSelectionLogicSelect(dot3ad_port_t *p)
{
  ...
  /*isSame is set to true to accomodate the first entry into the selected list*/
  /*as well as to init the variable for the loops to follow*/
  /*isSame has to remain L7_TRUE through out for a successful selection*/
  isSame = L7_TRUE;
    ...
    while (port != L7_NULLPTR)
    {
      if (port->muxState == WAITING && p->selected == SELECTED)
      {
        ...
        if (L7_ERROR == rc)
        {
          /*trace call*/
          TRACE_BEGIN(dot3adSystem.traceId,0xF1);
          TRACE_ENTER_BYTE(lacpSelectedReady);
          TRACE_ENTER_BYTE(p->actorPortAggId);
          TRACE_ENTER_BYTE(p->actorPortNum);
          TRACE_ENTER_BYTE(p->partnerOperPortNumber);
          TRACE_ENTER_BYTE(p->actorOperPortState);
...
L7_RC_t dot3adSelectionLogicUnselect(dot3ad_port_t *p)
{
  ...
  */
  ...
  if (L7_ERROR == rc)
  {
    ...
    /*trace call*/
    TRACE_BEGIN(dot3adSystem.traceId,0xF1);
    TRACE_ENTER_BYTE(lacpUnselected);
    TRACE_ENTER_BYTE(p->actorPortAggId);
    TRACE_ENTER_BYTE(p->actorPortNum);
    TRACE_ENTER_BYTE(p->partnerOperPortNumber);
    TRACE_ENTER_BYTE(p->actorOperPortState);
