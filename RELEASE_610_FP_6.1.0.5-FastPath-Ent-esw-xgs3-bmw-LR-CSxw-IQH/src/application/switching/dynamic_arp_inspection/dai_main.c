...
void daiTask(void)
{
  ...
  while (L7_TRUE)
  {
                                 ...
                                 sizeof(daiFrameMsg_t), L7_NO_WAIT) == L7_SUCCESS)
    {
      if (osapiWriteLockTake(daiCfgRWLock, L7_WAIT_FOREVER) == L7_SUCCESS)
      {
        daiFrameProcess(frameMsg.rxIntf, frameMsg.vlanId,
...
L7_RC_t daiIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  {
      LOG_MSG("DAI: NIM event Enqueue failed.\n");
...
L7_RC_t daiIntfChangeProcess(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  {
     LOG_MSG("%s: %d: daiIntfChangeProcess(): Received a interface callback while outside the EXECUTE state", __FILE__, __LINE__);
        rc = L7_FAILURE;
...
void daiRateLimitCheck(void)
{
  ...
  for (i = 1; i < DAI_MAX_INTF_COUNT; i++)
  {
    if((daiCfgData->intfCfg[i].rate_limit != L7_DAI_RATE_LIMIT_NONE) &&
       (daiIntfInfo[i].pktRxCount >= daiCfgData->intfCfg[i].rate_limit))
    {
...
//            "DAI Interface %s Error-Disabled!! Rate Limit %d pps with burst interval %d hit",
                     ...
                     L7_uchar8 *frame, L7_uint32 dataLen)
{
  ...
  daiFrameForward(intIfNum, vlanId, frame, dataLen);
}
                        ...
                        L7_uchar8 *frame, L7_ushort16 frameLen)
{
     ...
     (routingEnabled == L7_ENABLE))
  {
    ...
    else
    {
      ...
      if(arp_pkt->arp_op == L7_ARPOP_REPLY)
      {
        if (nimGetIntfAddress(intIfNum, L7_SYSMAC_BIA, intfMac) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DAI_COMPONENT_ID, 
                 "Failure to get Interface MAC address for %d",intIfNum);
          return L7_FAILURE;
        }
        ...
        if((memcmp(intfMac, eth_header->dest.addr, L7_ENET_MAC_ADDR_LEN) == 0) ||
           (vrrpFuncTable.vrrpMapGetVMac &&
            (vrrpFuncTable.vrrpMapGetVMac(arp_pkt->arp_tpa, (L7_uchar8 *)vMac) == L7_SUCCESS) &&
  ...
  {
    return daiFrameUnicast(fdbEntry.dot1dTpFdbPort, vlanId, frame, frameLen);
  }
                        ...
                        L7_uchar8 *frame, L7_ushort16 frameLen)
{
  ...
  if (dot1qVlanEgressPortsGet(vlanId, &portMask) == L7_SUCCESS)
  {
    ...
    {
      if (daiFrameSend(outgoingIf, vlanId, frame, frameLen) == L7_SUCCESS)
      {
        return L7_SUCCESS;
      }
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DAI_COMPONENT_ID, 
      ...
      if (L7_INTF_ISMASKBITSET(portMask, i))
      {
        ...
        if(i != intIfNum)
        {
             ...
             (activeState == L7_ACTIVE))
          {
            ...
            if (daiFrameSend(i, vlanId, frame, frameLen) != L7_SUCCESS)
            {
              daiInfo->debugStats.pktTxFailures++;
              rc = L7_FAILURE;
            }
                    ...
                    L7_uchar8 *frame, L7_ushort16 frameLen)
{
  ...
  return dtlIpBufSend(intIfNum, vlanId, 0, bufHandle);
}
