...
// PTin added
L7_BOOL dot1qQueueEmpty( void )
{
  if (dot1qQueueGetNumMsgs()==0)  return L7_TRUE;
  return L7_FALSE;
}
// PTin end
...
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*           L7_ADDR_INUSE if VLAN ID used for port based routing
*
* @notes    none
*
* @end
...
{  
    ...
    if ((requestor != DOT1Q_ADMIN) && (requestor != DOT1Q_GVRP)&& 
    ...
    /* Verify that the VLAN ID is not being used internally for a port based
     * routing interface */
    if (dot1qVlanIdInternal(vid))
    {
    ...
    msg.event =  
     (requestor == DOT1Q_GVRP || requestor == DOT1Q_WS_L2TUNNEL) ? dot1q_vlan_create_dynamic :  dot1q_vlan_create_static;


    rc = dot1qIssueCmd(&msg);
    if (rc != L7_SUCCESS)
    {
        LOG_MSG("Cannot put a message on dot1q msg Queue %d \n",rc);
    }
...
L7_RC_t dot1qVlanDelete(L7_uint32 vid, L7_uint32 requestor)
{
    ...
    msg.requestor = requestor;
	msg.event =  
	 (requestor == DOT1Q_GVRP || requestor == DOT1Q_WS_L2TUNNEL) ? dot1q_vlan_delete_dynamic :  dot1q_vlan_delete_static;
...
L7_RC_t   dot1qVlanMemberSet(L7_uint32 vid, L7_uint32 intIfNum, L7_uint32 mode, L7_uint32 requestor, DOT1Q_SWPORT_MODE_t swport_mode)
{
  ...
  {
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID, "Cannot get the switchport mode Interface: %d\n", intIfNum);
  }
      ...
      (mode == L7_DOT1Q_FIXED))
  {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID,
              "Port(%d) with switchport mode Access cannot participate in any vlan\n", intIfNum);
       return L7_FAILURE;
  }
  ...
  if (requestor!=DOT1Q_ADMIN)
  {
    ...
    if ((rc==L7_SUCCESS) && 
        ((vlan_type==L7_DOT1Q_ADMIN) && (vlan_mode==L7_DOT1Q_FORBIDDEN)))
    {
       return L7_FAILURE;
    }
  }
...
L7_RC_t   dot1qVlanTaggedMemberSet(L7_uint32 vid, L7_uint32 intIfNum, L7_uint32 tagMode)
{
    ...
    msg.data.intIfNum = intIfNum;
    msg.mode = tagMode;
    msg.event = dot1q_vlan_tagged_set_static; 
...
L7_RC_t   dot1qVlanStaticEgressPortsSet(L7_uint32 vlanId, NIM_INTF_MASK_t * mask)
{
    ...
    memcpy(&(vCfg->vlanCfg.staticEgressPorts), &tempMask, sizeof(NIM_INTF_MASK_t));
    dot1qCfg->hdr.dataChanged = L7_TRUE;
...
L7_RC_t   dot1qVlanStaticUntaggedPortsSet(L7_uint32 vlanId, NIM_INTF_MASK_t * mask)
{
    ...
    memcpy(&(vCfg->vlanCfg.taggedSet), &tempMask, sizeof(NIM_INTF_MASK_t));
    dot1qCfg->hdr.dataChanged = L7_TRUE;
...
L7_RC_t dot1qIntfInVIDnumGet(L7_uint32 vlanId, NIM_INTF_MASK_t *mMask, L7_uint32 *numOfElements)
{
  ...
  *numOfElements = 0;
  ...
  {
	  LOG_MSG("%s: nimGetHighestIntfNumber failed \n",__FUNCTION__);
	  return L7_FAILURE;
  }
...
L7_RC_t dot1qIntfListForValidMode(NIM_INTF_MASK_t * mask, DOT1Q_SWPORT_MODE_t swport_mode)
{
  ...
  while ( curr != 0 )
  {
    ...
    if (curr_mode != swport_mode )
    {
       LOG_MSG("Incorrect switchport mode for vlan participation. Mode: %d Interface: %d\n", swport_mode,  curr);
...
L7_RC_t   dot1qQportsVlanMembershipMaskSet(L7_uint32 intIfNum, L7_VLAN_MASK_t *mask, L7_uint32 mode, DOT1Q_SWPORT_MODE_t swport_mode)
{
  ...
  if (dot1qIntfIsValid(intIfNum) != L7_TRUE)
  {
	LOG_MSG("Invalid dot1q Interface: %d\n", intIfNum);
	return L7_FAILURE;
  }
  if (nimConfigIdMaskOffsetGet(intIfNum, &maskOffset) != L7_SUCCESS)
  {
	LOG_MSG("Could not get nimMaskIdOffset for Interface %d\n",  intIfNum);
	return L7_FAILURE;
  }
  ...
  if ((mode != L7_DOT1Q_FIXED) && 
      ...
      (mode != L7_DOT1Q_FORBIDDEN))
  {
	LOG_MSG("Incorrect mode for vlan participation. Mode: %d Interface: %d\n", mode,  intIfNum);
      return L7_FAILURE;
  }
  ...
  if (valid == L7_FALSE)
  {
	LOG_MSG("Intf is not configurable: Interface: %d\n", intIfNum);
    return L7_FAILURE;
  }
  ...
  if (!overrideMgmtVlanSet)
  {
      ...
      )
    {
      if (L7_VLAN_ISMASKBITSET(vidMask, mgmtVlanId))
      {
        ...
        rc1 = L7_FAILURE;
        LOG_MSG("Cannot set membership for user interface %d on management vlan %d\n", intIfNum,  mgmtVlanId);
      }
    }
  }
#endif
  ...
  for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
  {
	if (L7_VLAN_ISMASKBITSET(vidMask, vlanId))
	{
	  ...
	  if (valid == L7_FALSE) 
	  {
		...
		LOG_MSG("Cannot set membership for interface %d on non existant vlan %d\n", intIfNum,  vlanId);
		rc1 = L7_FAILURE;
...
L7_RC_t   dot1qQportsVlanTagMaskSet(L7_uint32 intIfNum, L7_VLAN_MASK_t *mask, L7_uint32 tagMode)
{
  ...
  if ((tagMode != L7_DOT1Q_TAGGED) && 
	  (tagMode != L7_DOT1Q_UNTAGGED))
  {
	LOG_MSG("Incorrect tagmode for vlan tagging. tagmode: %d Interface: %d\n", tagMode,  intIfNum);
	  return L7_FAILURE;
  }
  ...
  for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
  {
	if (L7_VLAN_ISMASKBITSET(vidMask, vlanId))
	{
	  ...
	  if (valid == L7_FALSE) 
	  {
		...
		/* reset the bit in the mask so that we do not access this in process side of the Q*/
		L7_VLAN_CLRMASKBIT(vidMask, vlanId);
		LOG_MSG("Cannot set tagging for interface %d on non existant vlan %d\n", intIfNum,  vlanId);
		rc1 = L7_FAILURE;
		continue; /*Continue to the next vlan id */
	  }
  ...
  for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
  {
    ...
    if (valid == L7_TRUE)                                       
    {
      ...
      while ( curr != 0 )
      {
        ...
        /* Set tagged as well as member properties for vlan/port*/
        if (NIM_INTF_ISMASKBITSET(vCfg->vlanCfg.forbiddenEgressPorts, curr) == L7_FALSE)
        {
            NIM_INTF_SETMASKBIT(vCfg->vlanCfg.staticEgressPorts, curr);
            NIM_INTF_SETMASKBIT(vCfg->vlanCfg.taggedSet, curr);
        }
            break;
          default:
            return L7_FAILURE;
...
L7_RC_t dot1qSwitchPortAccessVlanSet(L7_uint32 intIfNum, L7_uint32 vlanId, L7_uint32 requestor)
{
  ...
  if (requestor == DOT1Q_ADMIN)
  {
    ...
    if (dot1qMapIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR,L7_DOT1Q_COMPONENT_ID,"Intf(%d) not configurable",intIfNum );
      (void)osapiWriteLockGive(dot1qCfgRWLock);
      return rc;
    }
...
L7_RC_t   dot1qVlanCreateMask(L7_VLAN_MASK_t *vidMask)
{
    ...
    for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
    {
      if (L7_VLAN_ISMASKBITSET_POINTER(vidMask, vlanId))
      {
        ...
        /* Verify that the VLAN ID is not being used internally for a port based
         * routing interface */
        else if (dot1qVlanIdInternal(vlanId))
        {
          L7_uint32 intIfNum = 0;
          L7_uchar8 ifName[L7_NIM_IFNAME_SIZE+1];
          if (dot1qInternalVlanIdToIntIfNum(vlanId, &intIfNum) == L7_SUCCESS)
          {
          ...
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DOT1Q_COMPONENT_ID,
                  "Cannot create VLAN %u. VLAN ID is already in use for port-based "
                  "routing interface %s.",
                  vlanId, ifName);
          L7_VLAN_CLRMASKBIT_POINTER(vidMask, vlanId);
        }
	...
	if (rc != L7_SUCCESS)
	{
		LOG_MSG("Cannot put a message %d on dot1q msg Queue %d \n",dot1q_vlan_create_static_mask,rc);
	}
...
L7_RC_t   dot1qVlanDeleteMask(L7_VLAN_MASK_t *vidMask)
{
    ...
    if (rc != L7_SUCCESS)
    {
        LOG_MSG("Cannot put a message %d on dot1q msg Queue %d \n",dot1q_vlan_delete_static_mask,rc);
    }
