...
{
  return ((requestor == DOT1Q_GVRP) || (requestor == DOT1Q_WS_L2TUNNEL));
}
    ...
    if (numPorts > 0)
	{
		if (mode == L7_DOT1Q_FIXED)
		{
            ...
            { 
                ...
                LOG_MSG("dtl failure when adding ports to vlan id %d - portMask = %s \n",
                         vlanId, traceBuf);
            }
				 ...
				 (mode == L7_DOT1Q_FORBIDDEN))
		{
            ...
            { 
                ...
                LOG_MSG("dtl failure when deleting ports from vlan id %d - portMask = %s \n",
                         vlanId, traceBuf);
            }
		...
		if (rc != L7_SUCCESS)
		{
			LOG_MSG("Cannot update driver for interface  vlan id %d \n", vlanId);
			return L7_FAILURE;
		}
		else
		{
    ...
    if (numPorts > 0)
	{
		...
		if (action == L7_DOT1Q_TAGGED)
		{
            ...
            { 
                ...
                LOG_MSG("dtl failure when adding ports to tagged list for vlan id %d - portMask = %s \n", vlanId, traceBuf);
            }
		...
		else if (action == L7_DOT1Q_UNTAGGED)
		{
            ...
            { 
                ...
                LOG_MSG("dtl failure when deleting ports from tagged list for vlan id %d - portMask = %s \n", vlanId, traceBuf);
            }
...
void dot1qTask(void)
{
	...
	for (;;)
	{
        ...
        {
            LOG_MSG("dot1qTask: unsuccessful return code on receive from dot1qMsgQueue: %08x\n", rc);
        }
	}
}
...
// PTin added
L7_int32 dot1qQueueGetNumMsgs( void )
{
  L7_int32 num;
...
L7_RC_t dot1qDispatch(DOT1Q_MSG_t *msg)
{
	...
	case dot1q_vlan_tagged_set_static:
		dot1qVlanTaggedMemberSetModify(msg->vlanId, msg->data.intIfNum , msg->mode);
		break;
    ...
    case dot1q_vlan_delete_static_mask:
        ...
        break;
	default:
		LOG_MSG("Unknown Event %d received \n",msg->event); 
		break;
...
L7_RC_t dot1qVlanCreateProcess(L7_uint32 fdbId, L7_uint32 vlanId, L7_uint32 requestor)
{
	...
	L7_RC_t rc = L7_SUCCESS;
	L7_uint32 currentVlanCount, intIfNum, pvid;
    dot1qNotifyData_t vlanData;
    ...
    if ( (vlanId <= L7_DOT1Q_NULL_VLAN_ID) || (vlanId > L7_DOT1Q_MAX_VLAN_ID) )
    {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID,
                ...
                " This accommodates for reserved vlan ids. i.e. 4094 - x.", (int)vlanId);
        rc = L7_FAILURE;
    }
	...
	{
        LOG_MSG("Unable to apply VLAN creation request for VLAN ID %d\n", vlanId);
		rc = L7_FAILURE;
	...
	if (pDataEntry != L7_NULLPTR)
	{
		...
		LOG_MSG("Attempt to create a vlan (%d) that already exists \n",vlanId); 
    ...
    if (requestor == DOT1Q_ADMIN) 
    {
        ...
        {
            LOG_MSG("Error pointing to DOT1Q config data for VLAN %d\n", vlanId);
        }
    }
	...
	if ( (rc = vlanEntryAdd(pVlanTree, &createDataEntry) ) != L7_SUCCESS )
	{
		...
		LOG_MSG("Problem updating data for VLAN %d\n", (L7_int32)vlanId);
		rc = L7_FAILURE;
		osapiSemaGive(dot1qSem);
	...
	else
	{
		...
		{
			LOG_MSG("Problem updating transient data for VLAN %d\n", (int)vlanId);
		}
		...
		if (rc != L7_SUCCESS)
		{
			LOG_MSG("DTL call to create VLAN %d failed with rc %d\n", vlanId, (L7_int32)rc);

            /* Delete from the operational structure */
            if (vlanEntryDelete(pVlanTree,vlanId) != L7_SUCCESS)
            {
                LOG_MSG("Problem unrolling data for VLAN %d\n", (L7_int32)vlanId);
            }
        ...
        for (intIfNum = 1; intIfNum <= L7_MAX_INTERFACE_COUNT;intIfNum++)
        {
            ...
            if ((dot1qOperSwitchPortModeGet(intIfNum) == DOT1Q_SWPORT_MODE_ACCESS) && 
               (dot1qOperSwitchPortAccessVlanGet(intIfNum) == vlanId))
            {
               dot1qSwitchPortAccessModeSet(intIfNum, vlanId, DOT1Q_ADMIN);
               /* PVID will be set as part of acces vlan setting no need for further processing*/
               continue;
            }
            dot1qQportsVIDGet(intIfNum, L7_DOT1Q_DEFAULT_VID_INDEX, &pvid);
            if (pvid == vlanId)
            {
                (void)dot1qQportsVIDProcess(intIfNum, vlanId, 
...
L7_RC_t dot1qVlanDeleteProcess(L7_uint32 vlanId,L7_uint32 requestor)
{
	...
	DOT1Q_DEBUG_MSG("dot1qVlanDelete : vlan %d requestor %d \n", vlanId, requestor);
	/*Search the vlan DB */
	...
	{
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_DOT1Q_COMPONENT_ID,
	...
	{
		LOG_MSG("Not valid permision to delete the VLAN %d requestor %d\n",
				vlanId,requestor );
	...
	else
	{
		...
		if (rc != L7_SUCCESS)
		{
			LOG_MSG("VLAN Delete Call failed in driver for vlan %d\n", (L7_int32)vlanId);
		}

		/* Delete from the operational structure */
		if (vlanEntryDelete(pVlanTree,vlanId) != L7_SUCCESS)
		{
			LOG_MSG("Problem deleting data for VLAN %d\n", (L7_int32)vlanId);
		}

        /* Return the semaphore asap so as not to lock any other hopefuls */
        osapiSemaGive(dot1qSem); 
...
* @end
*********************************************************************/
L7_RC_t dot1qVlanTaggedMemberSetModify(L7_uint32 vlanId,L7_uint32 intIfNum, L7_uint32 tagMode )
{   
...
L7_RC_t dot1qVlanMSMaskModify(L7_uint32 vlanId, NIM_INTF_MASK_t *mask, L7_uint32 mode, DOT1Q_SWPORT_MODE_t swport_mode)
{
	...
	if (dot1qIsVlanDynamic(pDataEntry))
	{
		LOG_MSG("Dynamic entry %d can only be modified after it is converted to static",pDataEntry->vlanId);
		osapiSemaGive(dot1qSem);
		return L7_FAILURE;
	}
...
L7_RC_t dot1qVlanMemberSetMaskModify(L7_uint32 vlanId, NIM_INTF_MASK_t intfMask)
{
	...
	while (rc == L7_SUCCESS)
	{
	  ...
	  if (!NIM_INTF_ISMASKBITSET(pDataEntry->current.currentEgressPorts,curr) &&
		   NIM_INTF_ISMASKBITSET(intfMask,curr) && 
		  !NIM_INTF_ISMASKBITSET(pDataEntry->admin->vlanCfg.forbiddenEgressPorts, curr))
	  {
...
L7_RC_t dot1qDynamicToStaticVlanConvertProcess(L7_uint32 vlanId)
{
	...
	if (pDataEntry == NULL)
	{
		LOG_MSG("Cannot find vlan %d to delete\n", (int)vlanId);
		osapiSemaGive(dot1qSem);
		return(L7_FAILURE);
	...
	{
		LOG_MSG("Only Dynamically created vlans can be converted \n");
		osapiSemaGive(dot1qSem);
		return L7_FAILURE;
	}
...
void dot1qQportsVlanTagMaskProcess(L7_uint32 intIfNum, L7_VLAN_MASK_t *vlanMask, L7_uint32 tagMode)
{
  ...
  for (vlanId=1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
  {
	if (L7_VLAN_ISMASKBITSET(tempVlanMask, vlanId))
	{
	  ...
	  if (pDataEntry == L7_NULLPTR)
	  {
		...
		LOG_MSG("Cannot modify tagging of interface %d to non existance vlan %d\n", intIfNum,  vlanId);
	  ...
	  {
		LOG_MSG("Cannot modify tagging for port %d for vlan %d as this port is not a member of the vlan\n", 
				intIfNum,  vlanId);
		/*Clear the bit as we need to send notification based on the presence of the bit */
		L7_VLAN_CLRMASKBIT(tempVlanMask, vlanId);
		/*Process the next vlan id*/
  ...
  if (rc != L7_SUCCESS)
  {
	LOG_MSG("dtl request failure when modifying tagging for intIfNum %d \n", intIfNum );
	return;
  }
                                          ...
                                          L7_uint32 requestor)
{
  ...
  for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
  {
	if (L7_VLAN_ISMASKBITSET(localVlanMask, vlanId))
	{
	  ...
	  {
		/*Entry is NOT present*/
		LOG_MSG("Cannot modify membership of interface %d to non existance vlan %d\n", intIfNum,  vlanId);
		/*Clear the bit as we need to send notification based on the presence of the bit */
  ...
  if (rc != L7_SUCCESS)
  {
	LOG_MSG("dtl request failure when modifying vlan membership of intIfNum %d \n", intIfNum );
	return;
  }
...
void dot1qVlanCreateMaskProcess(L7_VLAN_MASK_t *vlanMask)
{
    ...
    for (intIfNum = 1; intIfNum <= L7_MAX_INTERFACE_COUNT;intIfNum++)
    {
      rc1 = dot1qQportsVIDGet(intIfNum, L7_DOT1Q_DEFAULT_VID_INDEX, &pvid);
      if ((rc1 == L7_SUCCESS) && (L7_VLAN_ISMASKBITSET(localVlanMask, pvid) && !(L7_VLAN_ISMASKBITSET(dot1qVlanCreateMaskFailure, pvid))))
      {
...
void dot1qVlanDeleteMaskProcess(L7_VLAN_MASK_t *vlanMask)
{
    ...
    for (vlanId = 1; vlanId <= L7_VLAN_MAX_MASK_BIT; vlanId++)
    {
      if (L7_VLAN_ISMASKBITSET(localVlanMask,vlanId))
      {
        ...
        if (pDataEntry->current.status == L7_DOT1Q_TYPE_DEFAULT)
        {
          LOG_MSG("Deleting the default VLAN \n");
        }
        if (pDataEntry->current.status != L7_DOT1Q_TYPE_ADMIN)
        {
          LOG_MSG("Not valid permision to delete the VLAN %d \n", vlanId);
          L7_VLAN_CLRMASKBIT(localVlanMask, vlanId);
          continue;
        }
        else
                                     ...
                                     L7_uint32 requestor)
{
   ...
   if ( (vlanId <= L7_DOT1Q_NULL_VLAN_ID) || (vlanId > L7_DOT1Q_MAX_VLAN_ID) )
   {
       LOG_MSG("Attempt to set access vlan with an invalid vlan id %d\n", (int)vlanId);
       return L7_FAILURE;
   }
   ...
   {
       /*Entry is already present*/
       LOG_MSG("Attempt to set access vlan with (%d) that does not exist \n",vlanId); 
       osapiSemaGive(dot1qSem); 
   ...
   rc = dot1qVlanTaggedMemberSetModify(vlanId,intIfNum, L7_DOT1Q_UNTAGGED);
   if (rc != L7_SUCCESS)
   {
       /* TBD rollback*/
       return rc;
