...
L7_RC_t pbVlanCnfgrInitPhase2Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if ((rc = nvStoreRegister(notifyFunctionList)) == L7_SUCCESS)
  {
    ...
    if ((rc = nimRegisterIntfChange(L7_PBVLAN_COMPONENT_ID, pbVlanIntfChangeCallback)) == L7_SUCCESS)
    {
      ...
      {
        L7_assert(1);
        L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_PBVLAN_COMPONENT_ID,
...
L7_RC_t pbVlanCnfgrUconfigPhase2(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  */
  ...
  /* JPR-TODO what really happens here??? */
...
void pbVlanCnfgrFiniPhase2Process()
{
  ...
  if ((rc = nvStoreRegister(notifyFunctionList)) == L7_SUCCESS)
  {
    ...
    if ((rc = nimDeRegisterIntfChange(L7_PBVLAN_COMPONENT_ID)) == L7_SUCCESS)
    {
      ...
      {
        L7_assert(1);
        LOG_MSG("Unable to deregister NIM callback\n");
      }
    }
    else
    {
      L7_assert(1);
      LOG_MSG("Unable to deregister pbVlan callback with vlans\n");
    }
  }
  else
  {
    L7_assert(1);
    LOG_MSG("Unable to deregister pbVlan callback with nvStore\n");
  }
  return;
}
