...
*
*********************************************************************/
L7_RC_t pbVlanGroupCreate(L7_char8 *Name)
{
  L7_uint32 i;
  ...
  for (i = 1; i <= L7_PBVLAN_MAX_NUM_GROUPS; i++)
  {
    if (pbVlanCfgData.group[i].inUse == L7_FALSE)
    {
      ...
      memcpy(pbVlanCfgData.group[i].name, Name, L7_PBVLAN_MAX_GROUP_NAME);
...
*
*********************************************************************/
L7_RC_t pbVlanGroupProtocolAdd(L7_uint32 groupId, L7_uint32 prtl)
{
  L7_uint32 i;
  L7_uint32 index;
  L7_uint32 ports;
...
L7_RC_t pbVlanGroupProtocolDelete(L7_uint32 groupId, L7_uint32 prtl)
{
  ...
  rc = pbVlanGroupPrtlIndexGet(prtl, &index);
  if (pbVlanCfgData.group[groupId].protocol[index] != prtl)
  {
    return L7_ERROR;
  }
...
L7_RC_t pbVlanGroupPortAdd(L7_uint32 groupId, L7_uint32 intIfNum)
{
  ...
  if ( ((groupId == L7_PBVLAN_NULL_GROUP) || (groupId > L7_PBVLAN_MAX_NUM_GROUPS))  ||
       (pbVlanCfgData.group[groupId].inUse == L7_FALSE)  ||
       ((nimGetIntfType(intIfNum, &intfType) != L7_SUCCESS) || (intfType != L7_PHYSICAL_INTF)) )
  {
    return L7_FAILURE;
  }
  ...
  else
  {
    ...
    for (i = 1; i <= L7_PBVLAN_MAX_NUM_GROUPS; i++)
    {
      ...
      {
        memset(list, 0x00, sizeof(L7_uint32)*(L7_MAX_INTERFACE_COUNT + 1));
        rc = nimMaskToList(&pbVlanCfgData.group[i].ports, list, &numPorts);

        count = 0;
        while (numPorts > L7_NULL)
        {
          if (list[count++] == intIfNum)
          {
            for (j = 0; j < L7_PBVLAN_MAX_CONFIGURABLE_PROTOCOLS; j++)
            {
...
L7_RC_t pbVlanGroupNameGet(L7_uint32 groupId, L7_char8 *buf)
{
  ...
  if (rc == L7_SUCCESS)
  {
    strncpy(buf, pbVlanCfgData.group[groupId].name, sizeof (pbVlanCfgData.group[groupId].name));
    rc = L7_SUCCESS;
  }
...
*
*********************************************************************/
L7_RC_t pbVlanGroupProtocolGet(L7_uint32 groupId, L7_uint32 *prtl)
{
  L7_uint32 i;
  L7_RC_t rc;
