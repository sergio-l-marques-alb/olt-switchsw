...
void *dot1s_timer_queue = L7_NULLPTR; /* reference to the dot1s timer message queue */
...
L7_RC_t dot1sSave(void)
{
	...
	{
	  LOG_MSG("dot1sSave deregistered\n");
	  return L7_FAILURE;
	}
...
L7_RC_t dot1sPortDefaultPopulate(DOT1S_PORT_COMMON_t *pPort, L7_uint32 intIfNum)
{
  ...
  pPort->diagnosticDisable = L7_FALSE;
...
* @end
*********************************************************************/
void dot1sStatGet(pCounterValue_entry_t c)
{
  c->status = L7_SUCCESS;
  c->cSize = C32_BITS;
...
L7_RC_t dot1sInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
    LOG_MSG("Unable to create dot1s task semaphore()\n");
  ...
  dot1s_timer_queue = (void*)osapiMsgQueueCreate("dot1s_timer_queue", DEFAULT_MSG_COUNT,
                                            (L7_uint32)sizeof(timerMsg));

  if ((dot1s_queue == L7_NULLPTR) ||
	  (dot1s_timer_queue == L7_NULLPTR))
  {
    LOG_MSG("dot1sInit: msgQueue creation error.\n");
    cbData.correlator = pCmdData->correlator;
    cbData.asyncResponse.rc = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  dot1s_timer_task_id = (L7_uint32)osapiTaskCreate("dot1s_timer_task", (void *)dot1s_timer_task, 0, 0,
                                        L7_DOT1S_STACK_SIZE,
  ...
  {
	LOG_MSG("dot1sInit: timer task creation error.\n");
	cbData.correlator = pCmdData->correlator;
    cbData.asyncResponse.rc = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  if (osapiWaitForTaskInit (L7_DOT1S_TIMER_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("Unable to initialize dot1s_timer_task()\n");
    return(L7_FAILURE);                                                   
  }

  dot1s_task_id = (L7_uint32)osapiTaskCreate("dot1s_task", (void *)dot1s_task, 0, 0,
                                             L7_DOT1S_STACK_SIZE,
                                             L7_DOT1S_DEFAULT_DOT1S_TASK_PRI,
                                             L7_DEFAULT_TASK_SLICE);
  if (dot1s_task_id == L7_ERROR)
  {
	LOG_MSG("dot1sInit: task creation error.\n");
	cbData.correlator = pCmdData->correlator;
    cbData.asyncResponse.rc = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
...
void dot1sInitUndo()
{
  ...
  if (dot1s_timer_queue != L7_NULLPTR)
    osapiMsgQueueDelete(dot1s_timer_queue);
