...
static char *dot1sRoleStringGet(DOT1S_PORT_ROLE_t role)
{
    switch (role)
    {
...
{
  L7_RC_t rc;
  if (p == L7_NULLPTR || p < &dot1sPort[0] || p > &dot1sPort[L7_MAX_INTERFACE_COUNT +1])
  {
        return L7_FAILURE;
  }
  ...
  {
	LOG_MSG("dot1s Event out of range %d \n", dot1sEvent);
	dot1sDepth--;
    return L7_FAILURE;
  }
  dot1sDepth--;
...
L7_RC_t dot1sRcvdMsgsClearAll()
{
  ...
  {
	if (dot1sPort[i].portNum != 0)
	{
	  dot1sPort[i].portInstInfo[DOT1S_CIST_INDEX].rcvdMsg = L7_FALSE;
	  /*now loop through all the MSTIs*/
	  for (j = 1; j <= L7_MAX_MULTIPLE_STP_INSTANCES; j++)
	  {
		if (dot1sInstanceMap[j].inUse == L7_TRUE)
		  dot1sPort[i].portInstInfo[j].rcvdMsg = L7_FALSE;
	  }
	}
  }
  return L7_SUCCESS;
...
L7_RC_t dot1sRcvdMsgsSet(DOT1S_PORT_COMMON_t *p, DOT1S_MSTP_ENCAPS_t 	*bpdu)
{
  ...
  {
	LOG_MSG("dot1sRcvdMsgsSet(): updtInfo CIST is L7_TRUE, discarding Msg on port %u\n", p->portNum);
	dot1sPortStats[p->portNum].rxDiscards++;
  }
  ...
  if (p->rcvdInternal == L7_TRUE)
  {
	...
	if (numMstiMsgs > 0)
	{
	  for (i=1;i<=numMstiMsgs;i++)
	  {
		...
		{
          LOG_MSG("dot1sRcvdMsgsSet(): updtInfo MSTI is L7_TRUE, discarding Msg on port %u\n", p->portNum);
	    }
...
L7_RC_t dot1sPrxReceiveAction(DOT1S_PORT_COMMON_t *p, DOT1S_MSTP_ENCAPS_t 	*bpdu)
{
  ...
  {
	  LOG_MSG("Invalid BPDU received on port (%d) \n", p->portNum);
	  return rc;
						...
						DOT1S_MSTP_ENCAPS_t 	*bpdu)
{
  ...
  if (bpdu != L7_NULL)
  {
	...
	if (L7_TRUE == dot1sIsConfigBPDU(bpdu))
	  dot1sPortStats[p->portNum].stpRx++;
	else if (L7_TRUE == dot1sIsRstBPDU(bpdu))
	  dot1sPortStats[p->portNum].rstpRx++;
	else if (L7_TRUE == dot1sIsMstBPDU(bpdu))
	  dot1sPortStats[p->portNum].mstpRx++;
    else
      dot1sPortStats[p->portNum].rxDiscards++;
  }
  return rc;
								   ...
								   DOT1S_MSTP_ENCAPS_t 	*bpdu)
{
  ...
  {  
      LOG_MSG("Cannot find inst number for Index %d \n", instIndex);
      return info;
  }
 ...
 /*       memcpy(mstiMsg.cistRootId.macAddr, bpdu->cistRootId.macAddr, L7_MAC_ADDR_LEN);          */
        ...
        msgPrio->dsgPortId = (bpdu->cistPortId & 0x0FFF) | portPri;
  ...
  else if (isDesignated == L7_TRUE)
  {
    ...
    if (comparison == SAME)
    {
       if (msgTimes->remHops != p->portInstInfo[instIndex].inst.msti.mstiPortTimes.remHops) 
       {
         info = SuperiorDesignatedInfo;
...
*/
  ...
  if (DOT1S_DEBUG(DOT1S_DEBUG_RCVD_INFO, instIndex))
	{
      SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port %d inst %d Rcvd Info While %d \n", p->portNum, instIndex, p->portInstInfo[instIndex].rcvdInfoWhile);
...
L7_RC_t dot1sPimAgedAction(DOT1S_PORT_COMMON_t *p, L7_uint32 instIndex)
{
  ...
  /* Do not do this if the infoIs variable is disabled as this means the port is transitioning from the init state*/
...
L7_RC_t dot1sPimReceiveAction(DOT1S_PORT_COMMON_t *p, L7_uint32 instIndex, DOT1S_MSTP_ENCAPS_t 	*bpdu)
{
  ...
  if (DOT1S_DEBUG(DOT1S_DEBUG_RCVD_INFO, instIndex))
  {
       DOT1S_DEBUG_MSG("Port(%d) Inst(%d) :rcvd Info = %d\n",p->portNum,instIndex,
                     p->portInstInfo[instIndex].rcvdInfo);
  }
...
L7_RC_t dot1sRolesCistUpdt()
{
  ...
  {
    isRoot = L7_FALSE;
	p = dot1sIntfFind(bestPortId & 0x0FFF);
	if (p != L7_NULLPTR && p->portNum != 0)
	{
  ...
  for (i = 1; i<= ((L7_uint32)L7_MAX_INTERFACE_COUNT); i++ )
  {
    ...
    /*in the same loop calculate the designated times for each port*/   /*13.26.25 e*/
	  ...
	  if ((changedMaster == L7_TRUE) && (p->portLinkState == L7_UP))
	  {
		...
		{
			LOG_MSG("Changed master is true setting agree et. al. port(%d) \n", p->portNum);
		}
		*/
		for (j = 1; j <= L7_MAX_MULTIPLE_STP_INSTANCES; j++)
		{
  ...
  if (p != L7_NULLPTR && p->portNum != 0)
  {
	...
	else if (p->portInstInfo[DOT1S_CIST_INDEX].infoIs == INFO_AGED)
	{
        ...
        if (DOT1S_PROTO_DEBUG() == L7_TRUE)
        {
            SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) CIST :Selected Role = DESIGNATED\n",p->portNum);
        }
		*/
    ...
    else if (p->portInstInfo[DOT1S_CIST_INDEX].infoIs == INFO_RECEIVED)
    {
						 ...
						 p->rcvdInternal))
    	  {
            ...
            /*if (DOT1S_PROTO_DEBUG() == L7_TRUE)
            {
                SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) CIST :Selected Role = ROOT\n",p->portNum);
            }
			*/
          ...
          { 
											...
											&p->portInstInfo[DOT1S_CIST_INDEX].inst.cist.cistPortPriority) != BETTER)
              {
                  ...
                  
                  {
                      ...
                      /*if (DOT1S_PROTO_DEBUG() == L7_TRUE)
                      {
                          SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) CIST :Selected Role = ALTERNATE\n",p->portNum);
                      } */
                      p->portInstInfo[DOT1S_CIST_INDEX].updtInfo = L7_FALSE;
                  ...
                  else 
                  {
                      ...
                      /*if (DOT1S_PROTO_DEBUG() == L7_TRUE)
                      {
                          SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) CIST :Selected Role = BACKUP\n",p->portNum);
                      }*/
                      p->portInstInfo[DOT1S_CIST_INDEX].updtInfo = L7_FALSE;
              ...
              else
              {
                  ...
                  /*if (DOT1S_PROTO_DEBUG() == L7_TRUE)
                  {
                       SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) CIST :Selected Role = DESIGNATED(Default)\n",
									 p->portNum);
                  } */
                  p->portInstInfo[DOT1S_CIST_INDEX].updtInfo = L7_TRUE;
                  p->portInstInfo[DOT1S_CIST_INDEX].agreed = L7_FALSE;
...
L7_RC_t dot1sRolesMstiUpdt(L7_uint32 instIndex)
{
  ...
  if (p != L7_NULLPTR && p->portNum != 0)
  {
	...
	if ((p->portInstInfo[DOT1S_CIST_INDEX].infoIs == INFO_RECEIVED) 
    ...
    else
    {
		...
		if (p->portInstInfo[instIndex].infoIs == INFO_AGED)
		{
          ...
          /*if (DOT1S_PROTO_INST_DEBUG(instIndex) == L7_TRUE)
          {
               SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) Inst(%d) :Selected Role = DESIGNATED\n",p->portNum,instIndex );
          } */
        ...
        else if (p->portInstInfo[instIndex].infoIs == INFO_RECEIVED)
        {
												   ...
												   p->portInstInfo[instIndex].InternalPortPathCost) == L7_TRUE)
		   {
              ...
              /*if (DOT1S_PROTO_INST_DEBUG(instIndex) == L7_TRUE)
              {
                   SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) Inst(%d) :Selected Role = ROOT\n",p->portNum,instIndex );
              } */
		   ...
		   else
           {
											  ...
											  &p->portInstInfo[instIndex].inst.msti.mstiPortPriority) != BETTER)
             {
                 ...
                 else
                 {
                    ...
                    /*if (DOT1S_PROTO_INST_DEBUG(instIndex) == L7_TRUE)
                    {
                        SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) Inst(%d) :Selected Role = BACKUP\n",p->portNum,instIndex );
                    } */
			 ...
			 else
             {
                 ...
                 /*if (DOT1S_PROTO_INST_DEBUG(instIndex) == L7_TRUE)
                 {
                       SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,"Port(%d) Inst(%d) :Selected Role = DESIGNATED(default)\n",p->portNum,instIndex );
                 } */
      ...
      if (dot1sIsPortActiveForInstance(instIndex, intIfNum) == L7_TRUE)
      {
        if (DOT1S_DEBUG(DOT1S_DEBUG_FLUSH,instIndex))
        {
          DOT1S_DEBUG_MSG("%s: Flushing Dynamic Filtering entries for port %d at %d",
                          __FUNCTION__,intIfNum,osapiTimeMillisecondsGet());
        }
		  ...
		  p->operEdge == L7_FALSE)
	  {
        ...
        if (DOT1S_DEBUG(DOT1S_DEBUG_DETECTED_TC,instIndex))
        {
          DOT1S_DEBUG_MSG("%s:Set tcprop for port (%d) ,inst (%d) to TRUE at %d.",
                          __FUNCTION__,p->portNum,instIndex,osapiTimeMillisecondsGet());
        }
      }
	}
  }
...
{	
  ...
  if (dot1sIdNotEqual(rootId, bestId))
  {
    ...
    if ((rc = dot1sInstNumFind(instIndex, &instNum)) != L7_FAILURE)
    {
      trapMgrStpNewRootElectedLogTrap(instNum, (L7_uchar8 *)&bestId);
    }
  }
...
L7_RC_t dot1sMstiMasterFlagCompute(DOT1S_PORT_COMMON_t *p, L7_uint32 instIndex)
{
  ...
  if ((p->portInstInfo[instIndex].role == ROLE_ROOT || 
	   p->portInstInfo[instIndex].role == ROLE_DESIGNATED) &&
	  (anyPortMaster == L7_TRUE || anyPortMastered == L7_TRUE))
  {
...
static L7_BOOL isAllSynced(L7_uint32 portNum, L7_uint32 instIndex, L7_BOOL role_alternate)
{
  ...
  */
	...
	for (i = 1; i<= ((L7_uint32)L7_MAX_INTERFACE_COUNT); i++)
	{
	  ...
	  if (dot1sPort[i].portUpEnabledNum != 0 &&
		  ((dot1sPort[i].portInstInfo[instIndex].selected != L7_TRUE ||
		  dot1sPort[i].portInstInfo[instIndex].role != dot1sPort[i].portInstInfo[instIndex].selectedRole ||
		  dot1sPort[i].portInstInfo[instIndex].updtInfo == L7_TRUE)  ||
		  dot1sPort[i].portInstInfo[instIndex].synced == L7_FALSE))
	  {

		if (dot1sPort[i].portInstInfo[instIndex].synced == L7_FALSE)
		{
		    if (role_alternate == L7_TRUE )
			{
			  if (dot1sPort[i].portInstInfo[instIndex].role == ROLE_ROOT)
			  {
				continue;
			  }
			}
			else if(portNum == dot1sPort[i].portNum)
			{
			  continue;
			}
...
L7_BOOL isRerooted(DOT1S_PORT_COMMON_t *p, L7_uint32 instIndex)
{
       ...
       (p->portInstInfo[instIndex].role == ROLE_ROOT))) 
  {
    ...
    {
      if (dot1sPort[i].portUpEnabledNum != 0 &&
        p->portNum != dot1sPort[i].portUpEnabledNum &&
        dot1sPort[i].portInstInfo[instIndex].rrWhile != 0 )
      {
      reRooted = L7_FALSE;
...
void dot1sLoopInconsistentSet(DOT1S_PORT_COMMON_t *p, L7_uint32 instIndex, L7_BOOL mode)
{
    ...
    if (mode == L7_FALSE)
    {
        dot1sPortStats[p->portNum].transitionsOutOfLoopInconsistentState++;
        LOG_MSG("SpanningTree-LoopGuard: LoopGuard Disabled: unblocking interface %d on MST instance %d\n", p->portNum, instIndex);
        trapMgrStpInstanceLoopInconsistentEndTrap(instIndex, p->portNum);

    }
    else
    {
        dot1sPortStats[p->portNum].transitionsIntoLoopInconsistentState++;
        LOG_MSG("SpanningTree-LoopGuard: LoopGuard blocking port: %d on MST instance: %d\n", 
                  p->portNum, instIndex);
                                            ...
                                            L7_uint32 intIfNum)
{
    ...
    dot1sInstNumFind(instIndex, &mstID);
    ...
    for (i = 0; i < numVlan; i++)
    {
      if (VIDList[i] == 0)
      {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1S_COMPONENT_ID,
          "%s :Port %d inst %d VIDList has element 0 numVlans %d \n", 
               __FUNCTION__, intIfNum, instIndex, numVlan);
        return L7_TRUE;
      }
...
L7_RC_t dot1sPortStateMachineInit(DOT1S_PORT_COMMON_t *p)
{
  L7_RC_t rc = L7_SUCCESS;
  L7_uint32 instIndex;
  ...
  for (instIndex = DOT1S_CIST_INDEX; instIndex <= L7_MAX_MULTIPLE_STP_INSTANCES; instIndex++)
  {
    if (dot1sInstanceMap[instIndex].inUse == L7_TRUE)
    {
      p->portInstInfo[instIndex].portState = L7_DOT1S_DISCARDING;
...
L7_RC_t dot1sResetBpduGuardEffect(DOT1S_PORT_COMMON_t *p)
{
  ...
  dot1sPortStateMachineInit(p);
  /*if (dot1sProcessGuardLinkUpChange(intIfNum) != L7_SUCCESS)
  {
    LOG_MSG("Failed to set BPDU Guard value FLASE for %d\n",intIfNum );
    return L7_FAILURE;
  }*/
  return L7_SUCCESS;
}
