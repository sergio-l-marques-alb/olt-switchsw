          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((pimdmRC = pimdmCnfgrInitPhase3Process(&response, 
                     &reason )) != L7_SUCCESS)
                {
                                    ...
                                    L7_CNFGR_ERR_RC_t *pReason)
{
  ...
  *pReason   = 0;
  ...
  /*--------------------------------------------------*/
  if (nimRegisterIntfChange(L7_FLEX_PIMDM_MAP_COMPONENT_ID, 
                            pimdmMapIntfChangeCallback) != L7_SUCCESS)
  {
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    PIMDM_MAP_DEBUG (PIMDM_MAP_DEBUG_FAILURES, "Error registering with NIM");
    return L7_ERROR;
  }
...
* @end
*********************************************************************/
L7_RC_t pimdmCnfgrInitPhase3Process(L7_CNFGR_RESPONSE_t *pResponse, 
                                                  L7_CNFGR_ERR_RC_t *pReason)
{
  L7_RC_t      pimdmRC;
                 ...
                 L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  {
    rc = sysapiCfgFileGet(L7_FLEX_PIMDM_MAP_COMPONENT_ID, L7_PIMDM_CFG_FILENAME,
      (L7_char8 *)pimdmMapCbPtr->pPimdmMapCfgData, sizeof(L7_pimdmMapCfg_t), 
      &pimdmMapCbPtr->pPimdmMapCfgData->checkSum, L7_PIMDM_CFG_VER_CURRENT, 
       pimdmBuildDefaultConfigData, pimdmMigrateConfigData);
  }
  else
  {
    rc = sysapiCfgFileGet(L7_FLEX_PIMDM_MAP_COMPONENT_ID, L7_PIMDM6_CFG_FILENAME,
      (L7_char8 *)pimdmMapCbPtr->pPimdmMapCfgData, sizeof(L7_pimdmMapCfg_t), 
      &pimdmMapCbPtr->pPimdmMapCfgData->checkSum, L7_PIMDM_CFG_VER_CURRENT, 
      pimdm6BuildDefaultConfigData, pimdmMigrateConfigData);
  }
  if (rc != L7_SUCCESS)
  {
    pimdmRC = L7_ERROR;
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    PIMDM_MAP_DEBUG (PIMDM_MAP_DEBUG_FAILURES, 
                      "Error retrieving PIMDM config file");
    return pimdmRC;
  }
