...
heapBuffPool_t mgmdV4HeapPoolList[] ={
  ...
  {MGMD_MAX_GROUPS * sizeof (avlTreeTables_t), 1,"MGMD_GROUP_TREE_HEAP", HEAP_BUFFPOOL_NO_OVERFLOW},
...
heapBuffPool_t mgmdProxyV4HeapPoolList[] ={
  ...
  {MGMD_HOST_MAX_GROUPS * sizeof (mgmd_host_group_t), 1,"MGMD_PROXY_GRP_DATA_HEAP", HEAP_BUFFPOOL_NO_OVERFLOW},

  /* MGMD-Proxy Host Source Record + AdminScope Database Buffers */
  {sizeof (mgmd_host_source_record_t), (MGMD_HOST_MAX_SRC_RECORDS + L7_MCAST_MAX_ADMINSCOPE_ENTRIES ),"MGMD_PROXY_SOURCE_N_ADMINSCOPE_RECORD_BUFFERS", HEAP_BUFFPOOL_NO_OVERFLOW},
  ...
  {sizeof (avlTreeTables_t) * MGMD_PROXY_MRT_MAX_IPV4_ROUTE_ENTRIES, 1,"MGMD_PROXY_MRT_TREE_HEAP", HEAP_BUFFPOOL_NO_OVERFLOW},

  /* AVL Data Heap Buffers for MGMD-Proxy MRT (S,G) Database */
  {MGMD_PROXY_MRT_MAX_IPV4_ROUTE_ENTRIES * sizeof (mgmdProxyCacheEntry_t), 1,"MGMD_PROXY_MRT_DATA_HEAP", HEAP_BUFFPOOL_NO_OVERFLOW},
...
L7_RC_t mgmdMemoryInit (L7_uchar8 addrFamily)
{
  ...
  {
    mgmdHeapPoolList = mgmdV4HeapPoolList;
  }
  else if (addrFamily == L7_AF_INET6)
  {
    extern heapBuffPool_t mgmdV6HeapPoolList[];

    mgmdHeapPoolList = mgmdV6HeapPoolList;
  }
  else
  {
...
L7_RC_t mgmdProxyMemoryInit (L7_uchar8 addrFamily)
{
  ...
  if (addrFamily == L7_AF_INET)
  {
    mgmdProxyHeapPoolList = mgmdProxyV4HeapPoolList;
  }
  else if (addrFamily == L7_AF_INET6)
  {
    extern heapBuffPool_t mgmdProxyV6HeapPoolList[];

    mgmdProxyHeapPoolList = mgmdProxyV6HeapPoolList;
  }
  else
  {
