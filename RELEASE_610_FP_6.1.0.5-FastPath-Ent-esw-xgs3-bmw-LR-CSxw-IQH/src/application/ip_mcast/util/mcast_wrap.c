...
L7_uint32  mcastIpMapRtrAdminModeGet(L7_uint32 familyType)
{
  ...
  {
    LOG_MSG("Invalid familyType %d",familyType);
  }
 return adminMode;
}
/*********************************************************************
                                      ...
                                      L7_inet_addr_t *inetAddr,L7_uchar8 actionFlag)
{
  ...
  if(familyType == L7_AF_INET)
  {
    ...
    {
      rc = dtlIpv4LocalMulticastAddrAdd(intIfNum, inetAddr->addr.ipv4.s_addr);
    }
    else if(actionFlag == L7_FALSE)
    {
      rc = dtlIpv4LocalMulticastAddrDelete(intIfNum, inetAddr->addr.ipv4.s_addr);
    }
  }
  else if (familyType == L7_AF_INET6)
  {
  ...
  {
    LOG_MSG("Invalid familyType, family = %d", familyType);
  }
  return rc;
}
