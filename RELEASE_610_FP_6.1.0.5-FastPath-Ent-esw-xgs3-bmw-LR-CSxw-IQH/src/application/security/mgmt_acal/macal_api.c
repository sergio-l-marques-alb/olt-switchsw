...
L7_RC_t macalListCreate(L7_char8 *name)
{
  ...
  if (macalCfgData->macalList.creationStatus == L7_CREATED)
  {
    ...
    {
      LOG_MSG("%s(): List %s already exists.\n", fnName, name);
      (void)osapiWriteLockGive(macalRwLock);
      return L7_SUCCESS;
    }
    LOG_MSG("%s: A list already exists. Only single list is allowed to exist in the system.\n", fnName);
    return L7_FAILURE;
  }
  /* List name validation */
  if (strlen(name) == L7_NULL)
  {
    LOG_MSG("%s: Empty list name not allowed\n", fnName);
    (void)osapiWriteLockGive(macalRwLock);
    return L7_FAILURE;
  }
  if (strlen(name) > MACAL_MAX_LISTNAME_LEN)
  {
    LOG_MSG("%s: Listname %s truncated to %u characters\n", fnName, name, MACAL_MAX_LISTNAME_LEN);
  }
...
L7_RC_t macalListDelete(L7_char8 *name)
{
  ...
  if (strcmp(macalCfgData->macalList.name, name) != L7_NULL)
  {
    LOG_MSG("%s: List %s does not exist.\n", fnName, name);
    (void)osapiWriteLockGive(macalRwLock);
    return L7_FAILURE;
  }
...
L7_RC_t macalRuleAdd(L7_char8* name, L7_uint32 *priority)
{
  ...
  if (*priority == MACAL_REQUEST_PRIORITY)
  {
    *priority = 1;
    ...
    {
      *priority = 0;
      LOG_MSG("%s: List %s has no rules left.\n", fnName, name);
      (void)osapiWriteLockGive(macalRwLock);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_MSG("%s: invalid priority value %u.\n", fnName, *priority);
    (void)osapiWriteLockGive(macalRwLock);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Rule with priority %u already exists. Overwriting it\n", fnName, *priority);
  }
...
L7_RC_t macalRuleSrcIpMaskAdd(L7_char8 *name, L7_uint32 priority, L7_uint32 ipAddress, L7_uint32 mask)
{
  ...
  {
    LOG_MSG("%s: List %s is active. It cannot be modified.\n", fnName, name);
    (void)osapiWriteLockGive(macalRwLock);
    return L7_FAILURE;
  }
...
L7_RC_t macalValidateListPriorityRule(const L7_char8 *fnName, L7_char8 *listName, L7_uint32 priority)
{
  ...
  {
    LOG_MSG("%s: Rule with priority %u does not exist.\n", fnName, priority);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
