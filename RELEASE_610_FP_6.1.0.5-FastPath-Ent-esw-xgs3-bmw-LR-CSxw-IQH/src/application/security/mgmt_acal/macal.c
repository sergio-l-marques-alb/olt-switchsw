...
L7_RC_t macalStartTask()
{
  ...
  {
    LOG_MSG("macalStartTask(): Could not create task macalTask\n");
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit (L7_MGMT_ACAL_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("Unable to initialize macalTask()\n");
    return L7_FAILURE;
  }
...
void macalTask()
{
  ...
  do
  {
    ...
    switch (msg.msgId)
    {
      ...
      default:
        LOG_MSG("macalTask(): invalid message type:%d. %s:%d\n", msg.msgId, __FILE__, __LINE__);
        break;
    }
  } while (1);
}
...
L7_RC_t macalSysnetRegisterDeregister(L7_BOOL bRegister)
{
  ...
  if ((bRegister == L7_TRUE) && (registered == L7_FALSE))
  {
    ...
    {
      LOG_MSG("%s: Failed to register with sysnet\n", fnName);
      return L7_FAILURE;
    }
  }
  else if ((bRegister == L7_FALSE) && (registered == L7_TRUE))
  {
    ...
    {
      LOG_MSG("%s: Failed to deregister with sysnet\n", fnName);
      return L7_FAILURE;
    }
  }
                                  ...
                                  L7_FUNCPTR_t continueFunc)
{
  ...
  LOG_MACAL_DEBUG_MSG("MACL Trace: Packet received on Vlan 0x%04X, I/F %d, SRC %d.%d.%d.%d, protocol %d, src_port %d, dest_port %d.",
                      pduInfo->vlanId, pduInfo->intIfNum, FMTIPADDR(ipHdr->iph_src), ipHdr->iph_prot, src_port, dest_port);
  ...
  for (i = 1; i <= MACAL_MAX_NUM_RULES; i++)
  {
    ...
    { /* Interface match is required and the interfaces don't match; continue to the next rule */
      LOG_MACAL_DEBUG_MSG("MACL Trace: Mismatch on rule %d. Rule interface %d, ingress interface %d.", i,
                          macalCfgData->macalList.rule[i].ifNum, intIfNum);
      continue;
    }
  } /* end for stmt */
