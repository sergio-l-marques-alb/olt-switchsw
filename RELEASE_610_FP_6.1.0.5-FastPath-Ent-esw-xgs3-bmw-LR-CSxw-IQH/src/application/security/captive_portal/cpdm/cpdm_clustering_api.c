                                         ...
                                         L7_uint32 intf, L7_uchar8 ifStatus, L7_uint32 ifUsers)
{
  ...
  if (ptrData != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("Failed to get first wireless interface.\n");
    }
                                            ...
                                            L7_enetMacAddr_t swMac)
{
  ...
  {
    LOG_MSG("%s: Failed to get switch %02x:%02x:%02x:%02x:%02x:%02x IP address.\n",
	    __FUNCTION__,
  ...
  {
    /* added entry */
    LOG_MSG("Added peer switch client %02x:%02x:%02x:%02x:%02x:%02x.\n",
