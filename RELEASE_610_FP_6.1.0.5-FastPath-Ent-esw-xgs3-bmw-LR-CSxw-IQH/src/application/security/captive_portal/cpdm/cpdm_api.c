...
#ifdef L7_MGMT_SECURITY_PACKAGE
{
  ...
  if (L7_SUCCESS == cpdmGlobalHTTPSecurePort1Get(&port))
  {
    ...
    {
      LOG_MSG("%s: Failed to update SSLT with default auxiliary port:%d\n", __FUNCTION__,port);
    }
  }
  if (L7_SUCCESS == cpdmGlobalHTTPSecurePort2Get(&port))
  {
    ...
    {
      LOG_MSG("%s: Failed to update SSLT with additional auxiliary port:%d\n", __FUNCTION__,port);
    }
  }
...
L7_RC_t cpdmGlobalHTTPPortSet(L7_uint32 httpPort)
{
  ...
  if (cpdmCfgData->cpGlobalData.httpPort != httpPort)
  {
    ...
    while (L7_SUCCESS == rc)
    {
      ...
      {
        loop_rc = L7_FAILURE;
        LOG_MSG(partialFmt, "No way to", httpPort, intfId);
      }
      else if (L7_SUCCESS != pCBs->cpSetAuthPort(intfId, httpPort))
      {
        loop_rc = L7_FAILURE;
        LOG_MSG(partialFmt, "Can't", httpPort, intfId);
      }
      rc = cpdmIntfStatusNextGet(intfId, &intfId);
    }
    rc = loop_rc;
...
L7_RC_t cpdmGlobalHTTPSecurePort1Set(L7_uint32 httpSecurePort)
{
  ...
  if (cpdmCfgData->cpGlobalData.httpsPort1 != httpSecurePort)
  {
    ...
    while (L7_SUCCESS == rc)
    {
      ...
      {
        LOG_MSG(partialFmt, "No way to", httpSecurePort, intfId);
      }
      else if (L7_SUCCESS != pCBs->cpSetAuthSecurePort1(intfId, httpSecurePort))
      {
        LOG_MSG(partialFmt, "Can't", httpSecurePort, intfId);
      }
      rc = cpdmIntfStatusNextGet(intfId, &intfId);
      if (L7_SUCCESS == rc)
      {
...
{ 
  ...
  if (L7_SUCCESS == rc)
  {
    ...
    if (L7_SUCCESS != rc)
    {
      LOG_MSG("Could not add default Web Id for CP Id %d",cpId);
      return rc;
    }  
  }
  ...
  {  
    do {
      ...
      if (L7_SUCCESS != rc)
      {
        LOG_MSG("Failed to create intf assoc status table for CP ID %d",cpId);
        break;
      }
      else
      {
...
{ 
  ...
  do 
  {
    ...
    if (CP_ID_MIN == cpId) 
    {
      LOG_MSG("Refusing to remove default ID");
      break;
    }
    ...
    {
      LOG_MSG("Can't update CP ID %d",  cpId);
      break;
    }
      ...
      cpdmCPConfigIntIfNumNextGet(cpId, intIfNum, &nextCpId, &intIfNum))
    {
      ...
      {
        LOG_MSG("Could not disable intf %d", intIfNum);
      }
    }
    ...
    if (L7_SUCCESS != rc)
    {
       LOG_MSG("Failed to delete intf assoc status table for CP ID %d",cpId);
       break;
    }
...
L7_RC_t cpdmCPConfigModeAllUpdate(void)
{
  ...
  while(L7_SUCCESS == cpdmCPConfigNextGet(cpId, &cpId))
  {
    ...
    {
      LOG_MSG("Can't update I/F modes for CP ID %d", cpId);
    }
  }
  return rc;
}
...
L7_RC_t cpdmCPConfigIntIfNumUpdate(L7_uint32 intIfNum, L7_CP_MODE_STATUS_t mode)
{
  ...
  switch(mode)
  {
    ...
    case L7_CP_MODE_DISABLED:
      ...
      break;
    default:
      LOG_MSG("%s: only ENABLED, DISABLED supported (got %d)", __FUNCTION__, mode );
      return rc;
  }
  ...
  do 
  {
    ...
    {
      LOG_MSG("Can't delete clients using interface %d",intIfNum);
      break;
    }
    ...
    if (L7_SUCCESS == cpdmIntfStatusGet(intIfNum))
    {
      ...
      {
        LOG_MSG("Could not %sable intf %d\n",
                (L7_CP_MODE_ENABLED == mode) ? "en" : "dis", intIfNum);
        break;
      }

      if (L7_SUCCESS != cpdmIntfStatusSetIsEnabled(intIfNum, enabling))
      {
        LOG_MSG("Could not set status of intf %d\n", intIfNum);
        break;
      }
    }
    rc = L7_SUCCESS;
...
{ 
  ...
  {
    LOG_MSG("Couldn't get mode for CP ID %d", cpId);
    return rc;
  }
  ...
  if (L7_CP_MODE_ENABLED == mode)
  {
    ...
    {
      LOG_MSG("Couldn't get global mode");
      return rc;
    }
    ...
    if (L7_CP_MODE_ENABLED==mode)
    {
      ...
      {
        LOG_MSG("%s: no associated interface for CP ID %d, can't enable\n",
                __FUNCTION__, cpId);
        mode = L7_CP_MODE_DISABLED;
...
L7_RC_t cpdmCPStatusBlockedAllUpdate(void)
{
  ...
  while(L7_SUCCESS == cpdmCPConfigNextGet(cpId, &cpId))
  {
    ...
    {
      LOG_MSG("Can't update blocking status for CP ID %d", cpId);
    }
  }
  return rc;
}
...
{ 
  ...
  case L7_CP_MODE_DISABLED:
    break;
  default:
    LOG_MSG("%s: only ENABLED, DISABLED supported (got %d)",
       __FUNCTION__, mode );
    return rc;
  }
...
L7_RC_t cpdmCPConfigAddDefaults(void)
{
  ...
  if (L7_SUCCESS != cpdmCPConfigNextGet(cpId, &cpId))
  {
    ...
    if (L7_SUCCESS != rc)
    {
      LOG_MSG("Could not add default CP ID");
      return rc;
    }
    rc = cpdmCPConfigNameSet(CP_ID_MIN, CP_DEF_NAME);
    if (L7_SUCCESS != rc)
    {
      LOG_MSG("Could not set name of default CP ID");
      return rc;
    }
  }
  return L7_SUCCESS;
...
L7_RC_t cpdmCPConfigIntIfNumAdd(cpId_t cpId, L7_uint32 intIfNum)
{
  ...
  if (L7_SUCCESS == rc)
  {
    ...
    if (L7_SUCCESS != rc)
    {
      LOG_MSG("Could not update CP data", intIfNum);
    }
  }  
...
L7_RC_t cpdmCopyImageDataToFs(L7_uchar8 *images)
{
  ...
  {
    fileNameLen = b2i(&images[idx+=fileSize]);
    ...
    fileSize = b2i(&images[idx+=fileNameLen]);
    rc = osapiFsWrite(fqf,&images[idx+=4],fileSize);
    if (L7_SUCCESS != rc)
    {
      LOG_MSG("%s: Failed to write file:%s.",__FUNCTION__,fileName); 
      return L7_FAILURE;
    }
  }
