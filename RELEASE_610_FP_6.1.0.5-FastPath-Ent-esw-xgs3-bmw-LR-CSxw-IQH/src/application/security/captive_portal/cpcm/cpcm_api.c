...
static avlTree_t         aipStatusTree;
                                   ...
                                   L7_uint32          intfId)
{
      ...
      if (cpClusterSwitchClientEntryByIPGet(ipAddr, &macMatched) == L7_SUCCESS)
      {
        if (memcmp(&macMatched, mac, sizeof(L7_enetMacAddr_t)) == 0)
        {
          if (cpClusterSwitchClientSwitchMACGet(macMatched, &swMac) == L7_SUCCESS)
          {
            ...
            {
              LOG_MSG("cpcmProcessAuthPreparation: Failed to send client auth request to switch %02x:%02x:%02x:%02x:%02x:%02x.\n",
              ...
              (usp.port == auth_usp.port))
          {
            ...
            if (pCBs->cpAuthenUnauthenClient(CP_CLIENT_AUTH, *mac, maxBandwidthUp, maxBandwidthDown) == L7_SUCCESS)
            {
              ...
              if (cpcmAIPStatusGet(ipAddr) == L7_SUCCESS) 
              {
                ...
                {
                  LOG_MSG("cpcmProcessAuthPreparation: AIP entry IP 0x%x updated intfId %d.\n", ipAddr, intfId);
                  return L7_SUCCESS;
                }
                else
                {
                  LOG_MSG("cpcmProcessAuthPreparation: Failed to update AIP entry IP 0x%x, intfId %d.\n", ipAddr, intfId);
                  return L7_FAILURE;      
                }
              }
              ...
              {
                LOG_MSG("cpcmProcessAuthPreparation: Failed to add AIP entry IP 0x%x.\n", ipAddr);    
              }
            ...
            {
              LOG_MSG("cpcmProcessAuthPreparation: can't reauthenticate client.\n");
              return L7_FAILURE;
            }
          }
        }
  ...
  */
      ...
      if (0==memcmp(&macMatched,mac,sizeof(L7_enetMacAddr_t)))
      {
             ...
             (L7_SUCCESS==cpdmClientConnStatusCpIdGet(mac,&cpId)))
        {
          if ((intfId==tmpIntfId) && (cpId == matchCpId))
          {
            ...
            {
              LOG_MSG("can't reauthenticate client\n");
              return L7_FAILURE;
            }
          }
        }
  ...
  if (L7_SUCCESS == cpcmAIPStatusGet(ipAddr)) 
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_DEBUG,L7_FLEX_CAPTIVE_PORTAL_COMPONENT_ID,
              "%s: AIP table entry IP 0x%x updated intfId %d.",
              __FUNCTION__,ipAddr,intfId);
      return L7_SUCCESS;
    }
    else
    {
      LOG_MSG("%s: Failed to update AIP table entry IP 0x%x, intfId %d.",
              __FUNCTION__,ipAddr,intfId);
      return L7_FAILURE;      
    }
  }
  ...
  {
    LOG_MSG("Can't add IP 0x%x for auth in prog\n", ipAddr);    
  }
                                             ...
                                             cpRedirIpAddr_t    ipAddr)
{
...
#if 0
  L7_LOGF(L7_LOG_SEVERITY_DEBUG,L7_FLEX_CAPTIVE_PORTAL_COMPONENT_ID,
          "%s: NIM wrapper for PrepAuth: i/f, ip = %d, 0x%08x",
          __FUNCTION__,intIfNum,ipAddr.ipAddr.redirIpv4Addr);
#endif
  ...
  if (L7_FAILURE == rc)
  {
    LOG_MSG("%s: cannot prepare work item\n", __FUNCTION__);
  }
  else
  {
...
L7_RC_t cpcmUserAuthRequest(cpUserAuth_t *user, CP_AUTH_STATUS_FLAG_t *flag)
{
  ...
  if (!user || !flag)
  {
    LOG_MSG("%s: Bad parms", __FUNCTION__);
    return status;
  }
  ...
  if (L7_SUCCESS==cpcmAIPStatusGet(ipAddr))
  {
    ...
    {
      CP_AUTH_STATUS_FLAG_t localFlag; 
      ...
      switch (localFlag)
      {
        case CP_TIMEOUT:
        {
          LOG_MSG("%s: Client IP 0x%08x auth timed out: TIMEOUT..",__FUNCTION__,ipAddr);
          break;
        }
        case CP_RESOURCE:
        {
          LOG_MSG("%s: Client IP 0x%08x no more resources: RESOURCES..",__FUNCTION__,ipAddr);
          break;
        }
        case CP_RADIUS_WIP:
        {
          LOG_MSG("%s: Client IP 0x%08x auth, radius waiting for AUTH command..",__FUNCTION__,ipAddr);
          break;
        }
        case CP_DENIED:
        {
          LOG_MSG("%s: Client IP 0x%08x auth, auth failed: DENIED..",__FUNCTION__,ipAddr);
          cpcmUserAuthFailed(ipAddr);
          break;
        }
        default:
        {
          LOG_MSG("%s: Client IP 0x%08x auth, default switch flag = %d..",__FUNCTION__,ipAddr,localFlag);
              break;
        }
      }
      *flag = localFlag;
  ...
  else
  {
    ...
    LOG_MSG("%s: User uid %s is neither connected nor in progress",
...
L7_RC_t cpcmRadiusVendorAttrProcess(CP_vsRadiusAttr_t *radiusAttr,
                                    CP_radiusEntry_t *radiusEntry)
{
  LOG_MSG("%s: Vendor-specific attr vendor %d, type: %d, length %d",
         __FUNCTION__,
  ...
  if (osapiNtohl(radiusAttr->vendor) == RADIUS_VENDOR_ID_WISPR)
  {
    switch (radiusAttr->vType)
    {
      ...
      default:
      {
        LOG_MSG("%s: Invalid Attribute type %d\n",__FUNCTION__,radiusAttr->vType);
        return L7_FAILURE;
      }
    }
  }
...
L7_RC_t cpcmUserAuthFailed(L7_IP_ADDR_t ipAddr)
{
  ...
  LOG_MSG("%s: Client IP 0x%08x auth failed: DENIED..",__FUNCTION__,ipAddr);

  memset(&macAddr,0,sizeof(L7_enetMacAddr_t));
  memset(&swMacAddr,0,sizeof(L7_enetMacAddr_t));
  memset(uid,0,sizeof(uid));
  cpcmAIPStatusMacAddrGet(ipAddr,&macAddr);
  cpcmAIPStatusUIDGet(ipAddr,uid);
  cpcmAIPStatusIntfIdGet(ipAddr,&intfId);
  cpdmCPConfigIntIfNumFind(intfId,&cpId);
                                 ...
                                 CP_radiusEntry_t * radiusEntry)
{
  ...
  do 
  {
    ...
    if (0 == sessionTimeout)
    {
      cpdmCPConfigSessionTimeoutGet(cpId,&sessionTimeout);
    }
    if (0 == idleTimeout)
    {
      cpdmCPConfigIdleTimeoutGet(cpId,&idleTimeout);
    }
    if (0 == maxBandwidthUp)
    {
      cpdmCPConfigUserUpRateGet(cpId,&maxBandwidthUp);
    }
    if (0 == maxBandwidthDown)
    {
      cpdmCPConfigUserDownRateGet(cpId,&maxBandwidthDown);
    }
    if (0 == maxInputOctets)
    {
      cpdmCPConfigMaxInputOctetsGet(cpId,&maxInputOctets);
    }
    if (0 == maxOutputOctets)
    {
      cpdmCPConfigMaxOutputOctetsGet(cpId,&maxOutputOctets);
    }
    if (0 == maxTotalOctets)
    {
      cpdmCPConfigMaxTotalOctetsGet(cpId,&maxTotalOctets);
    }
...
cpcmRADIUSGroupCheck(L7_IP_ADDR_t ipAddr, CP_radiusEntry_t * radiusEntry)
{
  ...
  if (0 < strlen(userGroups))
  {
    ...
    cpdmUserGroupEntryNameGet(gpId,groupName);
                         ...
                         L7_uint32          intfId)
{
  ...
  {
    LOG_MSG("cpcmAIPStatusAdd: Failed because either tree or MAC is NULL");
    return L7_FAILURE;
  }
  SEMA_TAKE(WRITE);   
...
L7_RC_t cpcmAIPStatusPwdSet(L7_IP_ADDR_t ipAddr, L7_uchar8 * pwd)
{
  ...
  do {
    ...
    if (L7_NULLPTR == pEntry)
      break;
    osapiStrncpySafe(pEntry->desc.pwd, pwd, L7_PASSWORD_SIZE+1);
    rc = L7_SUCCESS;
  } while(0);
  SEMA_GIVE(WRITE);   
