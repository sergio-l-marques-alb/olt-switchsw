...
diffServIntfInfo_t      *pDiffServIntfInfo_g = L7_NULLPTR;
...
L7_RC_t diffServSave(void)
{
  ...
  if (pDiffServCfgData_g->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    do
    {
      ...
      {
        LOG_MSG("%s %d: %s : Error while building contents of config file %s\n", 
      ...
      {
        LOG_MSG("%s %d: %s : Error on call to osapiFsWrite routine for config "
...
L7_RC_t diffServReplace(L7_diffServCfg_t *pCfgData)
{
  ...
  {
    LOG_MSG("%s: Failed clearing DiffServ config. The device may need to be reset.\n", 
            __FUNCTION__);
  }
...
L7_RC_t diffServApplyIntfConfigToMIB(L7_uint32 intIfNum)
{
  ...
  if (diffServMapIntfIsConfigurable(intIfNum, &pCfg) == L7_TRUE)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      ...
      if (rc == L7_SUCCESS)
      {
        LOG_MSG("%s: Error activating saved config for intIfNum %u -- using "
                "factory defaults instead\n", __FUNCTION__, intIfNum);
      }
      else
      {
        LOG_MSG("%s: Error activating saved config for intIfNum %u -- attempt "
         ...
         == L7_SUCCESS)
  {
    ...
    if ((rc == L7_SUCCESS) && (targetIntIfNum == intIfNum))
    {
      DIFFSERV_PRT(DIFFSERV_MSGLVL_LO_2,
                  "DiffServ: Found collateral policy %u,%u mirr/redir to intf %u\n", 
      ...
      if (dsDistillerPolicyEvaluate(policyIndex, adminMode, DSMIB_POLICY_EVAL_ALL_INTF, 
                                    L7_TRUE) != L7_SUCCESS)
      {
        LOG_MSG("%s: Error evaluating policy index %u (from target intf %u, event %u)\n",
                __FUNCTION__, policyIndex, intIfNum, event);
      }
  ...
  if (diffServMapIntfIsConfigurable(intIfNum, &pCfg) == L7_TRUE)
  {
        ...
        ifDirection++)
    {
      ...
      if ((rc != L7_SUCCESS) && (rc != L7_ERROR))
      {
        if (noMsg == L7_FALSE)
        {
          LOG_MSG("%s: Error deleting service interface %u %s\n",
                  __FUNCTION__, intIfNum,
...
L7_uint32 diffServIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event,NIM_CORRELATOR_t correlator)
{
  ...
  if (diffServDeregister_g.diffServIntfChangeCallback == L7_TRUE)
  {
  ...
  {
    LOG_MSG("%s called outside permitted phase (intf=%u, event=%u)\n", 
            __FUNCTION__, intIfNum, event);
    nimEventStatusCallback(status);
    return rc;
  }
  ...
  if (dsDistillerIntfEventNotify(intIfNum, event) != L7_SUCCESS)
  {
    ...
    LOG_MSG("%s: Error during distiller processing for intf %u, event %u\n", 
            __FUNCTION__, intIfNum, event);
  }
  ...
  case L7_PROBE_SETUP:
    ...
    COMPONENT_ACQ_SETMASKBIT(pDiffServIntfInfo_g[intIfNum].acquiredList, compId);
    break;
  ...
  case L7_PROBE_TEARDOWN:
    ...
    /* update acquired list */
    COMPONENT_ACQ_CLRMASKBIT(pDiffServIntfInfo_g[intIfNum].acquiredList, compId);
...
{
  if (COMPONENT_ACQ_ISMASKBITSET(pDiffServIntfInfo_g[intIfNum].acquiredList, 
...
L7_BOOL diffServMapIntfIsConfigurable(L7_uint32 intIfNum, L7_diffServIntfCfg_t **pCfg)
{
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(pDiffServCfgData_g->diffServIntfCfg[index].configId)) == L7_FALSE)
    {
      ...
      */
      LOG_MSG("Error accessing DIFFSERV config data for interface %u in diffServMapIntfIsConfigurable.\n", intIfNum);
      return L7_FALSE;
    }
  }
