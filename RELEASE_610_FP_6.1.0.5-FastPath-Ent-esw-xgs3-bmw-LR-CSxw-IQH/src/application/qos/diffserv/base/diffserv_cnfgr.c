                                        ...
                                        L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  do
  {
    ...
    /* Malloc space for DiffServ interface info structure  */
    pDiffServIntfInfo_g = 
      (diffServIntfInfo_t *)osapiMalloc(L7_FLEX_QOS_DIFFSERV_COMPONENT_ID, (L7_uint32)sizeof(diffServIntfInfo_t)*platIntfMaxCountGet());
    if (pDiffServIntfInfo_g == L7_NULLPTR)
    {
      errLine = __LINE__;
      break;
    }
    memset(pDiffServIntfInfo_g, 0, sizeof(diffServIntfInfo_t) * platIntfMaxCountGet());
    ...
    {
      LOG_MSG("%s: Unable to obtain DiffServ trace buffer\n", __FUNCTION__);
      errLine = __LINE__;
      break;
    } 
    else
  ...
  if ((rc != L7_SUCCESS) && (errLine != 0))
  {
    ...
    LOG_MSG("%s %u: %s: Error initializing DiffServ\n",
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  do
  {
    ...
    if (nimRegisterIntfChange(L7_FLEX_QOS_DIFFSERV_COMPONENT_ID, 
                              diffServIntfChangeCallback) != L7_SUCCESS)
    {
      errLine = __LINE__;
      break;
    }
    diffServDeregister_g.diffServIntfChangeCallback = L7_FALSE;
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  do
  {
    ...
    else
    {
      ...
      {
        LOG_MSG("%s: Error activating saved config -- using factory defaults instead\n",
                __FUNCTION__);
      } 
      else
      {
...
void diffServCnfgrFiniPhase1Process()
{
  ...
  {
    LOG_MSG("%s: Unable to delete DiffServ trace buffer\n", __FUNCTION__);
  }
  ...
  if (pDiffServIntfInfo_g != L7_NULLPTR)
  {
    osapiFree(L7_FLEX_QOS_DIFFSERV_COMPONENT_ID, pDiffServIntfInfo_g);
    pDiffServIntfInfo_g = L7_NULLPTR;
  }
...
void diffServCnfgrFiniPhase2Process()
{
  ...
  diffServDeregister_g.diffServIntfChangeCallback = L7_TRUE;
...
void diffServCnfgrFiniPhase3Process()
{
  ...
  {
    LOG_MSG("%s: Unconfig phase 2 function failed with resp=%u, reas=%u\n",
