...
L7_RC_t diffServPrivateMibInit(void)
{
  ...
  {
    LOG_MSG("%s: Unable to create private MIB index semaphore\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Unable to create private MIB AVL trees\n", __FUNCTION__);
    return L7_FAILURE;
  }
...
void diffServPrivateMibFini(void)
{
  ...
  if (dsmibIndexSemId != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("%s: Error freeing MIB index table semaphore (0x%8.8x)\n",
...
L7_RC_t dsmibMibTableSizeCheck(dsmibTableId_t tableId)
{
  ...
  {
    LOG_MSG("dsmibMibTableSizeCheck: table ID %u size %u exceeds maximum %u\n",
