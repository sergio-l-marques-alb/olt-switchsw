          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch (request)
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((rc = voipCnfgrInitPhase3Process(&response, &reason)) != L7_SUCCESS)
                {
                                  ...
                                  L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  {
    LOG_MSG("%s %u: %s: Error initializing VOIP component\n",
  ...
  *pReason   = L7_CNFGR_ERR_RC_FATAL;
  ...
  do
  {
    ...
    if (nimRegisterIntfChange(L7_FLEX_QOS_VOIP_COMPONENT_ID, 
                              voipIntfChangeCallback) != L7_SUCCESS)
    {
      errLine = __LINE__;
      break;
    }
  ...
  *pReason   = L7_CNFGR_ERR_RC_FATAL;
  ...
  do
  {
    ...
    else
    {
      ...
      {
        LOG_MSG("%s: Unable to apply saved config -- using factory defaults "
                "instead\n", __FUNCTION__);
      } 
      else
      {
...
void voipCnfgrFiniPhase3Process(void)
{
  ...
  {
    LOG_MSG("%s %u: %s: Error unconfiguring VOIP phase 2\n",
