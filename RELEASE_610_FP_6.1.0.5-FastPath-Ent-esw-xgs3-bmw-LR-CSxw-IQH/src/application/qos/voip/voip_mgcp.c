...
voipMgcpCaInsert( voipMgcpCa_t **list,const L7_uchar8 *ip, voipMgcpCa_t **ins)
{
  ...
  {
    LOG_MSG("voipMgcpCaInsert could not allocate memory\n"); 
    return L7_FAILURE;
  }
...
voipMgcpCallInsert(voipMgcpCa_t *list,const L7_uchar8 *ip,voipMgcpMessage_t *m,voipMgcpCall_t **ins)
{
  ...
  {
    LOG_MSG("voipMgcpCallInsert could not allocate memory\n");    
    return L7_FAILURE;
  }
...
L7_RC_t voipMgcpMessageInit (voipMgcpMessage_t **mgcp)
{
  *mgcp = (voipMgcpMessage_t *) osapiMalloc(L7_FLEX_QOS_VOIP_COMPONENT_ID, sizeof(voipMgcpMessage_t));
  if (*mgcp == L7_NULLPTR) 
  { 
    LOG_MSG("MGCP:Could not allocate memory\n");
    return L7_FAILURE; 
  }
...
voipMgcpCallUpdateSdp(voipFpEntry_t *entry, voipSdpMessage_t *sdp, L7_uint32 source)
{
  ...
  if (sdp == L7_NULLPTR) 
  {
    LOG_MSG("Should not happen!!! sdp = NULL\n");
    return L7_FAILURE;
  }
  ...
  if (size == 0)
  {
    LOG_MSG("Should not happen!!! sdp->m_medias = 0\n");
    return L7_FAILURE;
  }
  for(i = 0; i < size; i++)
  {
  ...
  if (source)
  {
    ...
    {
      LOG_MSG("osapiInetPton source failed ip = %s\n", sdp->c_connection->addr);
       return L7_FAILURE;
    }
  ...
  else
  {
     ...
     {
       LOG_MSG("osapiInetPton destn failed ip = %s\n", sdp->c_connection->addr);
       return L7_FAILURE;
     }
                                   ...
                                   const L7_uchar8 **headers)
{
  ...
  *headers = buf;
  ...
  {
    LOG_MSG("voipMgcpMessageStartlineParseReq : could not allocate memory\n");
    return L7_FAILURE;
  }
                                   ...
                                   const L7_uchar8 **headers)
{
  ...
  *headers = buf;
  ...
  if (sscanf (buf, "%d", &dest->returnCode) != 1)
  {
    ...
    LOG_MSG("voipMgcpMessageStartlineParseResp can't find return code!\n");
    return L7_FAILURE;
  }
                   ...
                   const L7_uchar8 **body)
{
  ...
  for (;;)
  {
    ...
    if (rc == L7_FAILURE) 
    {
      LOG_MSG("MGCP:voipMgcpHeadersParse no CRLF\n"); 
    ...
    {
       LOG_MSG("MGCP:voipMgcpHeadersParse no colon\n");
       return L7_FAILURE;            /* this is also an error case */
    }
    if (end <= colon_index) 
    {
       LOG_MSG("MGCP:voipMgcpHeadersParse message is incorrect\n");
       return L7_FAILURE;
    }
    ...
    voipClrncpy (hname, start, colon_index - start);
    {
      ...
      /* END of header is (end-1) if header separation is CR or LF */
        ...
        {
          LOG_MSG("should not happen, empty header %s!!!\n", hname);
                        ...
                        voipSdpMessage_t *sdp)
{
  ...
  if (MGCPMSG_IS_REQUEST(mgcp)) 
  {
    if (MGCPMSG_IS_CRCX(mgcp)) 
    {
      ...
      if (rv != L7_SUCCESS) 
      {
         LOG_MSG("voipMgcpCaInsert Failed\n");
         return L7_FAILURE;
      }
      if (mgcp->callId == L7_NULLPTR) 
      {
        LOG_MSG("voipMgcpMessageParseSdp : mgcp->call_id = NULL...\n");
        return L7_FAILURE;
      }
      if ((rv = voipMgcpCallLookup(caentry, mgcp->callId, &callentry)) == L7_SUCCESS) 
      {
          LOG_MSG("voipMgcpCallLookupCallId: Entry already exist...\n");
          return L7_FAILURE;
      }
    ...
    else if (MGCPMSG_IS_DLCX(mgcp)) 
    {
      ...
      if (rv == L7_FAILURE) 
      {
        LOG_MSG("DLCX received, but CA not found!\n");
        return L7_FAILURE;
      }
      /* Some DLCX don't have call id? */
      if (mgcp->callId == L7_NULLPTR) 
      {
        LOG_MSG("DLCX received, but without call id!\n");
        return L7_FAILURE;
      }
      rv = voipMgcpCallLookup(caentry, mgcp->callId , &callentry);
      if (rv == L7_FAILURE) 
      {
        LOG_MSG("DLCX received, but call id not found!\n");
        return L7_FAILURE;
      }
      ...
      if (rv == L7_SUCCESS) 
      {
        ...
        if (rv != L7_SUCCESS) 
        {
          LOG_MSG("MGCP: voipSetupMGCPSession(disable) failed\n");
        }
  ...
  else if (MGCPMSG_IS_STATUS_200(mgcp)) 
  {
    ...
    if (rv == L7_FAILURE) 
    {
      ...
      if (rv == L7_FAILURE) 
      {
         LOG_MSG("OK received, but CA not found!\n");
         return L7_FAILURE;
      }
      flag = 1;
    } 
    ...
    else 
    {
      ...
      if (rv == L7_SUCCESS) 
      {
        LOG_MSG("call found, call id = %s\n", callentry->msg->callId);
        if (mgcp->connectionId == L7_NULLPTR) 
        {
          LOG_MSG("!!!OK without connection id!!!\n");
          return L7_FAILURE;
        }
        /* TODO:
        ...
        */
        if (callentry->msg->connectionId == L7_NULLPTR) 
        {
          ...
          if (mgcp->connectionId == L7_NULLPTR) 
          {
            LOG_MSG("OK of CRCX received, but without connection id inside!!!!\n");
            return L7_FAILURE;
          }
          ...
          if(callentry->msg->connectionId == L7_NULLPTR)
          {
            LOG_MSG("Fail to allocate memory\n");
            return L7_FAILURE;
          } 
          ...
          if (sdp!=L7_NULLPTR) 
          {
            ...
            if (rv == L7_FAILURE) 
            {
              LOG_MSG("voipMgcpCallUpdateSdp retured failured\n");
              return rv;
            } 
            ...
            if (rv == L7_FAILURE) 
            {
              LOG_MSG("MGCP: Setup_MGCP_Session(enable) failed\n");
              rv = voipMgcpCallRemove(caentry, callentry);
              if (rv == L7_SUCCESS) 
              {
                 voipMgcpMessageFree(callentry->msg);
          ...
          {  
             LOG_MSG("OK of CRCX received, but NO SDP inside!!!!\n");
          }
                              ...
                              L7_uint32 length)
{
  ...
  if (tmp == L7_NULLPTR) 
  {
    LOG_MSG("Could not allocate memory.\n");
    return L7_FAILURE;
  }
  #endif
  ...
  if (rc == L7_FAILURE) 
  {
    ...
    LOG_MSG("voipMgcpMessageStartlineParse Failed\n");
    return L7_FAILURE;
  }
  tmp = (L7_char8 *) next;
  ...
  if (rc == L7_FAILURE) 
  {
    osapiFree (L7_FLEX_QOS_VOIP_COMPONENT_ID,beg);
    beg = L7_NULLPTR;
    LOG_MSG("voipMgcpHeadersParse Failed\n");
    voipMgcpMessageFree(mgcp);
    return L7_FAILURE;
  }
  tmp = (L7_uchar8 *) next;
  ...
  if(voipSdpMessageParse(sdp,start)!= L7_SUCCESS)
  {
    ...
    sdp = L7_NULLPTR;
    LOG_MSG("MGCP: voipSdpMessageParse failed\n");
    return L7_FAILURE;
  }
mgcp_parse:
