          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch (request)
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((rc = iscsiCnfgrInitPhase3Process(&response, &reason)) != L7_SUCCESS)
                {
                                    ...
                                    L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  do
  {
    ...
    if (osapiRWLockCreate(&iscsiRWLock, OSAPI_RWLOCK_Q_PRIORITY) == L7_FAILURE)
    {
      errLine = __LINE__;
      break;
    }
...
* @end
*********************************************************************/
L7_RC_t iscsiCnfgrInitPhase3Process(L7_CNFGR_RESPONSE_t *pResponse,
                                  L7_CNFGR_ERR_RC_t   *pReason)
{
  L7_RC_t               rc;
  L7_uint32             errLine;
  ...
  *pReason   = L7_CNFGR_ERR_RC_FATAL;
  ...
  do
  {
    ...
    else
    {
      ...
      {
        L7_LOG(L7_LOG_SEVERITY_WARNING, L7_FLEX_QOS_ISCSI_COMPONENT_ID, 
...
void iscsiCnfgrFiniPhase1Process(void)
{
  ...
  if (iscsiRWLock.handle != L7_NULLPTR)
  {
    (void)osapiRWLockDelete(iscsiRWLock);
  }
