...
extern osapiRWLock_t    iscsiRWLock;
      ...
      if (iscsiPktLen > 0)
      {
        if (bufferPoolAllocate(iscsiPacketBufferPoolId, &msg.u.iscsiPduReceiveParms.iscsiPacketBuffer) == L7_SUCCESS)
        {
          if (ISCSI_PACKET_BUFFER_SIZE > pkt_size)
          {
            ...
            {
              L7_LOG(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ISCSI_COMPONENT_ID, "iscsiPktIntercept(): message send failed");
              ISCSI_TRACE(ISCSI_TRACE_PDU_INTERCEPT, "iscsiPktIntercept() - message send failed\n");
            }
          }
          else
          {
            L7_LOG(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ISCSI_COMPONENT_ID, "iscsiPktIntercept(): received frame larger than ISCSI_PACKET_BUFFER_SIZE");
...
static void iscsiTimerCallback()
{
  ...
  if (osapiMessageSend(iscsiQueue,
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ISCSI_COMPONENT_ID, 
...
L7_RC_t iscsiPduReceiveProcess(L7_uint32 intIfNum, L7_uint32 pkt_size, L7_uchar8 *iscsiBuf)
{
  ...
  L7_uint32         ipHdrLen, ipPktLen, tcpHdrLen, iscsiPktLen, totalAhsLength, iscsiDataSegmentLength;
  ...
  if (ip_header->iph_prot == IP_PROT_TCP)
  {
    ...
    if (iscsiTargetTableLookUp(dstPort, dstAddr, &targetTableIndex) == L7_SUCCESS)
    {
      ...
      if (iscsiPktLen > 0)
      {
        ...
        if ((iscsiDataSegmentLength + ISCSI_BHS_LENGTH) <= iscsiPktLen)
        {
          switch (iscsiOpCode)
          {
        ...
        {
          ISCSI_TRACE(ISCSI_TRACE_PDU_RX, "iscsiPduReceiveProcess() - packet not large enough to contain iSCSI PDU ((iscsiDataSegmentLength + ISCSI_BHS_LENGTH) <= iscsiPktLen) is false :\n  iscsiDataSegmentLength = %d\n  ISCSI_BHS_LENGTH = %d\n  iscsiPktLen = %d\n", 
                      iscsiDataSegmentLength, ISCSI_BHS_LENGTH, iscsiPktLen);
        }
      }
      else
      {
        ISCSI_TRACE(ISCSI_TRACE_PDU_RX, "iscsiPduReceiveProcess() (iscsiPktLen > 0) is false : iscsiPktLen = %d\n", iscsiPktLen);
      }
    }
    else
    {
