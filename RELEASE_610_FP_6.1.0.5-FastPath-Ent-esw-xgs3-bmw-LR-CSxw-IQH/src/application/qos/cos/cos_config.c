...
void cosDefaultMappingConfigBuild(L7_uint32 intIfNum, L7_cosMapCfg_t *pCfgMap)
{
  ...
  for (i = 0; i < L7_QOS_COS_MAP_NUM_IPPREC; i++)
  {
    ...
    {
      LOG_MSG("%s: Cannot obtain default traffic class mapping for "
...
L7_RC_t cosConfigDataApply(void)
{
  ...
  {
    cosDefaultMappingConfigBuild(L7_ALL_INTERFACES, &pCfgGlob->cfg.mapping);
    LOG_MSG("%s: Global COS mapping table config is no longer valid, "
  ...
  for (cfgIndex = 1; cfgIndex < L7_COS_INTF_MAX_COUNT; cfgIndex++)
  {
    ...
    if (cosIntfIsConfigurable(intIfNum, &pCfgIntf) == L7_TRUE)
    {
      ...
      {
        LOG_MSG("%s: Failed applying config data to interface %u \n", 
...
L7_RC_t cosConfigIntfDataApply(L7_uint32 intIfNum, L7_cosCfgIntfParms_t *pCfgIntf)
{
  ...
  {
    LOG_MSG("%s: Failed applying COS map table config data to interface %u \n", 
            __FUNCTION__, intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Failed applying COS queue config data to interface %u \n", 
            __FUNCTION__, intIfNum);
    return L7_FAILURE;
  }
                                       ...
                                       L7_BOOL forceDtl)
{
  ...
  if (cosMapTableContentIsValid(intIfNum, &pCfg->mapping) != L7_TRUE)
  {
    ...
    LOG_MSG("%s: Intf %u COS map table config is no longer valid, "
...
L7_RC_t cosConfigIntfDataUnapply(L7_uint32 intIfNum, L7_cosCfgIntfParms_t *pCfgIntf)
{
  ...
  if (memcmp(&pCfgIntf->cfg.mapping, &intfCfg.cfg.mapping, sizeof(intfCfg.cfg.mapping)) != 0)
  {
        ...
        (pCfgIntf->cfg.mapping.intfTrustMode != L7_QOS_COS_MAP_INTF_MODE_TRUST_DOT1P))
    {
      ...
      {
        LOG_MSG("%s: Failed to apply default COS map table config to interface %u \n", 
                __FUNCTION__, intIfNum);
        return L7_FAILURE;
      }
    }
       ...
       (memcmp(pCfgIntf->cfg.queue, intfCfg.cfg.queue, sizeof(intfCfg.cfg.queue)) != 0) )
  {
    ...
    {
      LOG_MSG("%s: Failed to apply default COS queue config to interface %u \n", 
              __FUNCTION__, intIfNum);
      return L7_FAILURE;
    }
  }
...
L7_RC_t cosSave(void)
{
  ...
  {
    LOG_MSG("COS: Error writing config file %s\n", L7_COS_CFG_FILENAME);
  }
...
L7_RC_t cosRestore(void)
{
  ...
  {
    LOG_MSG("%s: Failed to restore COS factory defaults\n", __FUNCTION__);
    rc = L7_FAILURE;
  }
...
L7_BOOL cosIntfIsConfigurable(L7_uint32 intIfNum, L7_cosCfgIntfParms_t **ppCfgIntf)
{
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
        ...
        == L7_FALSE)
    {
       ...
       */
      LOG_MSG("cosIntfIsConfigurable: Error accessing COS config data for "
              "interface %u\n", intIfNum);
      return L7_FALSE;
    }
  }
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    for (cfgIndex = 1; cfgIndex < L7_COS_INTF_MAX_COUNT; cfgIndex++)
    {
                                 ...
                                 &configIdNull))
      {
        ...
        if (ppCfgIntf != L7_NULLPTR)
          *ppCfgIntf = &pCosCfgData_g->cosIntf[cfgIndex];
        return L7_TRUE;
      }
    }
  }
                              ...
                              NIM_CORRELATOR_t correlator)
{
  ...
  if (cosDeregister_g.cosIntfChangeCallback == L7_TRUE)
  {
    rc = L7_SUCCESS;
  }
  ...
  else if (COS_IS_READY == L7_TRUE)
  {
    ...
    case L7_PROBE_SETUP:
      ...
      COMPONENT_ACQ_SETMASKBIT(pCosIntfInfo_g[intIfNum].acquiredList, compId);
      break;
    ...
    case L7_PROBE_TEARDOWN:
      ...
      /* update acquired list */
      COMPONENT_ACQ_CLRMASKBIT(pCosIntfInfo_g[intIfNum].acquiredList, compId);
  ...
  {
    LOG_MSG("cosIntfChangeCallback called outside permitted phase\n");
    rc = L7_FAILURE;
...
{
  if (COMPONENT_ACQ_ISMASKBITSET(pCosIntfInfo_g[intIfNum].acquiredList, 
