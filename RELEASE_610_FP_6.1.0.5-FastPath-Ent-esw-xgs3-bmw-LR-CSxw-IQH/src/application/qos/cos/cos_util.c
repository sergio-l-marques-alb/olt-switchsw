                                      ...
                                      L7_uint32 val)
{
  ...
  {
    LOG_MSG("COS mapping: Unable to apply IP Precedence %u to traffic class %u "
            "on intf %u\n", prec, val, intIfNum);
    return L7_FAILURE;
  }
                                      ...
                                      L7_uint32 val)
{
  ...
  {
    LOG_MSG("COS mapping: Unable to apply IP DSCP %u to traffic class %u "
            "on intf %u\n", dscp, val, intIfNum);
    return L7_FAILURE;
  }
                                 ...
                                 L7_BOOL forceDtl)
{
  ...
  default:
    LOG_MSG("COS mapping: Invalid trust mode value (%u) on intf %u\n", 
            mode, intIfNum);
    return L7_FAILURE;
    /*PASSTHRU*/
  }
  ...
  for (i = 0; i < (L7_DOT1P_MAX_PRIORITY+1); i++)
  {
    if (mode == L7_QOS_COS_MAP_INTF_MODE_TRUST_DOT1P)
    {
      ...
      {
        LOG_MSG("COS mapping: Unable to obtain dot1p mapping table for intf %u\n",
                intIfNum);
        return L7_FAILURE;
      }
    }
  ...
  {
    LOG_MSG("COS mapping: Unable to apply trust mode \'%s\' on intf %u\n", 
            cosMapIntfModeStr[mode], intIfNum);
    return L7_FAILURE;
  }

  /* display debug message according to COS msgLvl setting */
  COS_PRT(msgLvlReqd, "\nCOS intf mode set to \'%s\' on intf %u\n\n",
          cosMapIntfModeStr[mode], intIfNum);
  switch (mode)
  {
  case L7_QOS_COS_MAP_INTF_MODE_UNTRUSTED:
                                ...
                                L7_uint32 wredDecayExp)
{
  ...
  {
    LOG_MSG("COS queueing: Unable to apply COS intf config on interface %u\n", 
            intIfNum);
    return L7_FAILURE;
  }

  /* display debug message according to COS msgLvl setting */
  COS_PRT(COS_MSGLVL_MED, "\nCOS intf config applied on intf %u\n", intIfNum);
                                 ...
                                 L7_cosQueueSchedParms_t *pQParms)
{
  ...
  {
    LOG_MSG("COS queueing: Unable to apply COS scheduler config on intf %u\n", 
            intIfNum);
    return L7_FAILURE;
  }

  /* display debug message according to COS msgLvl setting */
  COS_PRT(COS_MSGLVL_MED, "\nCOS scheduler config applied on intf %u\n", intIfNum);
  if (cosMsgLvlGet() >= COS_MSGLVL_LO)
  {
    COS_PRT(COS_MSGLVL_LO, "\n  minBw:  ");
                               ...
                               L7_qosCosDropParmsList_t *pVal)
{
  ...
  {
    LOG_MSG("COS queueing: Unable to apply COS drop parms on intf %u\n",
            intIfNum);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
