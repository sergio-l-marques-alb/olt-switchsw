...
// ----------------------------------------------------
   ...
   int  OpenIPC	(void)
   {
      ...
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Vai iniciar o modulo de comunicacoes IPC."); 
      init_ipc_lib ();
...
//      }
      ...
      if ((res=open_ipc (IPC_HW_FASTPATH_PORT, &CHMessageHandler, IPC_CH_TIMEOUT, &g_iInterfaceHW))!=S_OK)
      {
         DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Nao foi possivel abrir o canal de comunicacoes para atendimento de mensagens (%08X).", res);
//         close_ipc (g_iInterfaceSW);
         return res;
      }
     
      if ((res=open_ipc (0, NULL, IPC_CM_TIMEOUT, &g_iInterfaceSW))!=S_OK)
      {
         DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Nao foi possivel abrir o canal para o envio de mensagens (%08X).", res);
//         close_ipc (g_iInterfaceSW);
      ...
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Modulo de comunicacoes IPC iniciado com sucesso."); 
      return S_OK;
   } // OpenIPC (V1.0.0.060622)

   void CloseIPC	(void)
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Vai fechar o modulo de comunicacoes IPC."); 
      if (g_iInterfaceSW>=0)
         close_ipc (g_iInterfaceSW);
      if (g_iInterfaceHW>=0)
         close_ipc (g_iInterfaceHW);
      ...
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Modulo de comunicacoes IPC fechado com sucesso."); 
   } // CloseIPC (V1.0.0.060622)

   void EnableHandling (BOOLEAN enable)
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "(Des)activacao do processamento de mensagens do modulo de comunicacoes IPC (%d).", enable); 
...
int send_trap(int porto, int trap_type, int arg)
{
  ...
  ret=send_data(g_iInterfaceSW, IPC_CHMSG_TRAP_PORT, (ipc_msg *)&comando, (ipc_msg *)NULL);
  if(ret<0)
      printf("SENDTRAP to PORT %d (Canal =%d), Code = 0x%.4x, arg = 0x%08x (%d): ERROR = %d\n\r", IPC_CHMSG_TRAP_PORT, g_iInterfaceSW, trap_type,arg,arg, ret);
  else
      printf("SENDTRAP to PORT %d (Canal =%d), Code = 0x%.4x: arg = 0x%08x (%d)\n\r", IPC_CHMSG_TRAP_PORT, g_iInterfaceSW, trap_type,arg,arg);  
  return(ret);
}


/* Envia trap */
int send_trap_intf_alarm(int porto, int code, int status, int param)
{
  ipc_msg	comando;
  int ret;
  st_alarmGeral *alarm;
  ...
  ret=send_data(g_iInterfaceSW, IPC_CHMSG_TRAP_PORT, (ipc_msg *)&comando, (ipc_msg *)NULL);
  if(ret<0)
      printf("SENDTRAP to PORT %d: interface=%d, Code = 0x%.4x, status = %d: ERROR = %d\n\r", IPC_CHMSG_TRAP_PORT, porto, code, status, ret);
  return(ret);
}

int send_trap_gen_alarm(int porto, int code, int status, int param1, int param2)
{
  ipc_msg	comando;
  int ret;
  st_alarmGeral *alarm;
