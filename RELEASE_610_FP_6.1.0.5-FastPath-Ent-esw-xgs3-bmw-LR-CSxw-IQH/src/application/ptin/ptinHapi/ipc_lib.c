...
void init_ipc_lib(void)
{
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Vai iniciar a biblioteca ipc_lib."); 
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Biblioteca ipc_lib iniciada."); 
...
// ********************************************************************************
int open_ipc (int porto_rx, int  (*MessageHandler)(ipc_msg *inbuffer, ipc_msg *outbuffer), int timeout, int *handlerid)
{
   struct timeval tv;
   ...
   tv.tv_usec = 0;
   *handlerid = -1;
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Abertura de canal de comunicacao (%d, %08X, %d).", porto_rx, MessageHandler, timeout); 
   ...
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Nao existem canais disponiveis."); 
   ...
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Falhou ao criar o socket cliente (%08X).", errno); 
   ...
   if (getsockopt (ipc_canais[canal_id].socket_descriptor_cliente, SOL_SOCKET, SO_SNDBUF, (char*)&optVal, &optLen)==0)
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Socket cliente do canal %d, SO_SNDBUF=%d.", canal_id, optVal); 
   else
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "Nao obteve o valor de SO_SNDBUF no Socket cliente do canal %d (errno=%d).", canal_id, errno); 
   ...
   if(MessageHandler!=NULL && porto_rx>0 && porto_rx<IPCLIB_MAX_PORTNUMBER)	    //Se existir handler para a funcao de escuta inicializa
   {
      ...
      {
         DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Falhou ao criar o socket servidor (%08X).", errno); 
      ...
      hostbyname = gethostbyname("127.0.0.1");  //so aceita mensagens locais
      memcpy (&ipc_canais[canal_id].socketaddr_servidor.sin_addr.s_addr, hostbyname->h_addr, 
              sizeof(ipc_canais[canal_id].socketaddr_servidor.sin_addr.s_addr));
      ...
      {
         DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Falhou ao ligar o socket servidor ao porto de escuta (%08X).", errno); 
         close(ipc_canais[canal_id].socket_descriptor_cliente);    //liberta o descritor de cliente
...
//      // Definicao do Clone de escutas
      ...
      {
         DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_EMERGENCY, "Falhou na criacao do thread de atendimento de mensagens.");
         close(ipc_canais[canal_id].socket_descriptor_cliente);    //liberta o descritor de cliente
         close(ipc_canais[canal_id].socket_descriptor_servidor);   //liberta o descritor de servidor
         return SIR_ERROR(ERROR_FAMILY_IPC, ERROR_SEVERITY_EMERGENCY, ERROR_CODE_CREATECLONE);
      }
   ...
   *handlerid = canal_id;  //Retorna o handler correspondente ao canal de comunicacao alocado
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Canal aberto com sucesso (id=%d).", canal_id);
   return S_OK;
...
int close_ipc(int canal_id)
{
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Fecho de canal de comunicacoes (id=%d).", canal_id); 
   if (canal_id<0 || canal_id>=IPCLIB_MAX_CANAIS)
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "Identificador de canal invalido."); 
   ...
   {
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "O canal indicado nao esta atribuido."); 
   ...
   if ((res=close (ipc_canais[canal_id].socket_descriptor_cliente))!=0)
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "O socket cliente NAO foi fechado com sucesso (%d).", res);
   //destroi o descritor para o socket servidor
   if ((res=close (ipc_canais[canal_id].socket_descriptor_servidor))!=0)
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "O socket servidor NAO foi fechado com sucesso (%d).", res);
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "O canal %d foi fechado com sucesso.", canal_id);
   return S_OK;
...
// ********************************************************************************
int send_data (int canal_id, int porto_destino, ipc_msg *sendbuffer, ipc_msg *receivebuffer)
{
   int bytes, addr_len;
   int timeout;
...
//      }
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "Nao foi possivel enviar a mensagem (%08X).", errno); 
      return SIR_ERROR(ERROR_FAMILY_IPC, ERROR_SEVERITY_ERROR, ERROR_CODE_SENDFAILED);
   }
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Mensagem enviada pelo canal %d.", canal_id); 
...
//      }
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Aguarda resposta pelo canal %d.", canal_id); 
...
//   }
   if(timeout<=0)
   {                         
      DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_WARNING, "Timeout na recepcao da resposta no canal %d.", canal_id); 
      return SIR_ERROR(ERROR_FAMILY_IPC, ERROR_SEVERITY_WARNING, ERROR_CODE_TIMEOUT);
   }
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Devolve resposta recebida no canal %d.", canal_id); 
   return S_OK;
...
#endif
{
   ...
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Abertura do thread de atendimento de mensagens do canal %d.", canal);
      ...
      {                              
         ...
         if(ipc_canais[canal].processa_msg_handler(&inbuffer,&outbuffer)==0)
         {
           ...
           if (sendto(ipc_canais[canal].socket_descriptor_servidor, &outbuffer, (outbuffer.infoDim)+(7*sizeof(UINT)), 0,
            ...
            {
               DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_ERROR, "Nao foi possivel enviar a resposta pelo canal %d - %s ", canal, strerror(errno));
            }
            else
            {
               DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Enviada resposta pelo canal %d.", canal);
            }
         }
         else
         {
            DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_IPC, TRACE_SEVERITY_INFORMATIONAL, "Nao existe resposta 'a mensagem %08X no canal %d.", inbuffer.msgId, canal);
         }
      }
   } //while  
   DEBUGTRACE (TRACE_MODULE_ALL | TRACE_LAYER_APP, TRACE_SEVERITY_INFORMATIONAL, "Fecho do thread de atendimento de mensagens do canal %d.",  canal);
   return S_OK;
