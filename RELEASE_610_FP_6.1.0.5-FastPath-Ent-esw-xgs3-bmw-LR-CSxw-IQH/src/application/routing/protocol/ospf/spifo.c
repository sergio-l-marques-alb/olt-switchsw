...
static char __fileid[] = "$Header: /home/cvs/PR1003/PR1003/FASTPATH/RELEASE_610_FP_6.1.0.5-FastPath-Ent-esw-xgs3-bmw-LR-CSxw-IQH/src/application/routing/protocol/ospf/spifo.c,v 1.1 2011/04/18 17:10:29 mruas Exp $";
#endif
...
extern e_Err StubDefaultLsaOriginate(t_ARO *p_ARO);
extern e_Err NssaDefaultLsaOriginate(t_ARO *p_ARO);
...
static void DrCalculate(t_IFO *p_IFO);
...
e_Err IFO_Config(t_Handle IFO_Id, t_IFO_Cfg *p_Cfg)
{
   ...
   if(p_IFO->Cfg.Type == IFO_VRTL)
   {
      ...
      if((e = HL_Insert(bbArea->IfoHl, (byte*)&p_IFO->IfoId, p_IFO)) != E_OK)
      {
         p_IFO->Cfg.IfStatus = ROW_NOT_IN_SERVICE;         
         return e;
      }
   ...
   if(p_IFO->Cfg.IfStatus != ROW_NOT_IN_SERVICE)
   {
      ...
      if((e = HL_Insert(p_ARO->IfoHl, (byte*)&p_IFO->IfoId, p_IFO)) != E_OK)
      {
         p_IFO->Cfg.IfStatus = ROW_NOT_IN_SERVICE;         
         return e;
      }
                      ...
                      word offset, ulng *allocPktLen)
{
   ...
   *allocPktLen = 0;
   if(! maxBufLen)
   {
       LOG_MSG("Max allowed OSPF pkt len on intIfNum %d is zero", p_IFO->Cfg.IfIndex);
       return NULL;
   }
   ...
   if (p_B == NULL)
   {
       LOG_MSG("Unable to allocate buffer for OSPF packet in IFO_AllocPacket().");
       return NULL;
   }
...
e_Err IFO_SendHello(t_Handle Id, t_Handle NboId, Bool IsReplay)
{
  ...
  p_Hlo->Options |= p_RTO->Cfg.MulticastExtensions ? OSPF_OPT_MC_BIT : 0;
  ...
  A_SET_4B(p_IFO->Cfg.DrId,p_Hlo->DesignatedRouter);
  A_SET_4B(p_IFO->Cfg.BackupId,p_Hlo->BackupRouter);
...
void DoHelloTransition(t_IFO *p_IFO, e_IFO_Events Event)
{
   ...
   if(((t_ARO*)p_IFO->ARO_Id) == NULLP)
   {
       ...
       char ifName[L7_NIM_IFNAME_SIZE + 1];
       RTO_InterfaceName(p_IFO, ifName);
       LOG_MSG("DoHelloTransition: ARO_Id not set yet on Intf %s, Event %s\n", ifName, ifoEventNames[Event]);
...
{
   t_ARO *p_ARO = (t_ARO *) p_IFO->ARO_Id;
   SP_IPADR CurDr, CurBackup, NewDr, NewBackup, NewBackupTmp, PrevDr, PrevBackup;
   t_NBO *p_NBO;
   t_RTO *p_RTO = (t_RTO *) p_IFO->RTO_Id;
   e_Err er;
   byte  BkUpNewPriority, DrNewPriority, NewPriorityTmp;
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_DR_CALC)
   {
     ...
     osapiInetNtoa(CurDr, drid);
     osapiInetNtoa(CurBackup, bdrid);
     osapiSnprintf(traceBuf, OSPF_MAX_TRACE_STR_LEN,
                   "Current DR is %s. Current BDR is %s.", drid, bdrid);
     RTO_TraceWrite(traceBuf);
   }

   NewDr = NewBackup = NewBackupTmp = 0;
   BkUpNewPriority = DrNewPriority = NewPriorityTmp = 0;

   /* Check if any neighbor presents */
   if(HL_GetFirst(p_IFO->NboHl, (void *)&p_NBO) != E_OK)
   {
     if (p_RTO->ospfTraceFlags & OSPF_TRACE_DR_CALC)
     {
       RTO_TraceWrite("No neighbors");
     }
      /* no neighbor found */
      /* Set the local rtr as DR, if its priority is not 0 */
      if(p_IFO->Cfg.RouterPriority != 0) 
      {
         NewDr = p_IFO->DrId = LocRouterId;
         NewBackup = p_IFO->BackupId = 0;
      }
      else
         p_IFO->DrId = p_IFO->BackupId = 0;
   }
   else  /* at least one neighbor present */
   {
      do /* steps 2,3,4 */
      {
         PrevDr = p_IFO->DrId;
         PrevBackup = p_IFO->BackupId;
         
         NewDr = NewBackup = NewBackupTmp = 0;
         BkUpNewPriority = DrNewPriority = NewPriorityTmp = 0;

         /* Step 2 - calculate the Backup router */
         
         /* Start from the local router */
         if(p_IFO->Cfg.RouterPriority && (p_IFO->DrId != LocRouterId))
         {
            if(p_IFO->BackupId == LocRouterId)
            {
               /* if declared as a Backup */
               if((p_IFO->Cfg.RouterPriority > BkUpNewPriority) ||
                  ((p_IFO->Cfg.RouterPriority == BkUpNewPriority) &&
                  (LocRouterId > NewBackup)))
               {
                  NewBackup = LocRouterId;
                  BkUpNewPriority =  p_IFO->Cfg.RouterPriority;       
               }            
            }
            else if(!NewBackup) /* if no routers have declared as a Backup */
            {
               if((p_IFO->Cfg.RouterPriority > NewPriorityTmp) ||
                  ((p_IFO->Cfg.RouterPriority == NewPriorityTmp) &&
                  (LocRouterId > NewBackupTmp)))
               {
                  NewBackupTmp = LocRouterId;
                  NewPriorityTmp =  p_IFO->Cfg.RouterPriority;       
               }            
            }
         }
         ...
         while(er == E_OK)
         {
         /* Check only eligible to be DR neighbors that 
            in state greater or equal than 2-Way and not declared themselves as DR */
               ...
               (p_NBO->DrId != p_NBO->RouterId))
            {
               ...
               {
                  /* if declared as a Backup */
                  if((p_NBO->RouterPriority > BkUpNewPriority) ||
                     ((p_NBO->RouterPriority == BkUpNewPriority) &&
                     (p_NBO->RouterId > NewBackup)))
                  {
                     NewBackup = p_NBO->RouterId;
                     BkUpNewPriority =  p_NBO->RouterPriority;       
                  }            
               }
               else if(!NewBackup) /* if no routers have declared as a Backup */
               {
                  if((p_NBO->RouterPriority > NewPriorityTmp) ||
                     ((p_NBO->RouterPriority == NewPriorityTmp) &&
                     (p_NBO->RouterId > NewBackupTmp)))
                  {
                     NewBackupTmp = p_NBO->RouterId;
                     NewPriorityTmp =  p_NBO->RouterPriority;       
                  }
               }
            }
            er = HL_GetNext(p_IFO->NboHl, (void *)&p_NBO, p_NBO);
         ...
         if(!NewBackup)
            NewBackup = NewBackupTmp;
         
         /* set elected BACKUP */
         p_IFO->BackupId = NewBackup;
         
         /* Step 3 - calculate the Designated router */
         
         /* Start from the local router */
         if(p_IFO->Cfg.RouterPriority && (p_IFO->DrId == LocRouterId))
         {
            if((p_IFO->Cfg.RouterPriority > DrNewPriority) ||
               ((p_IFO->Cfg.RouterPriority == DrNewPriority) &&
               (LocRouterId > NewDr))) 
            {
               NewDr = LocRouterId;
               DrNewPriority =  p_IFO->Cfg.RouterPriority;       
            }            
         }
         ...
         {
         /* Check only eligible to be DR neighbors that 
         in state greater or equal than 2-Way and declared themselves 
            as DR */
            if(p_NBO->RouterPriority && 
               (p_NBO->State >= NBO_2WAY) &&
               (p_NBO->DrId == p_NBO->RouterId))
            {
               if((p_NBO->RouterPriority > DrNewPriority) ||
                  ((p_NBO->RouterPriority == DrNewPriority) &&
                  (p_NBO->RouterId > NewDr))) 
               {
                  NewDr = p_NBO->RouterId;
                  DrNewPriority =  p_NBO->RouterPriority;       
               }            
            }
            er = HL_GetNext(p_IFO->NboHl, (void *)&p_NBO, p_NBO);
         }
         ...
         /* step 4 - change in Dr or Backup */
      }         
      while(((NewDr == LocRouterId) && (PrevDr != LocRouterId))        ||
            ((NewBackup == LocRouterId) && (PrevBackup != LocRouterId)) ||
            ((NewDr != LocRouterId) && (PrevDr == LocRouterId))         ||
            ((NewBackup != LocRouterId) && (PrevBackup == LocRouterId)));

   } /* end of "if any neighbor exists" statement */
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_DR_CALC)
   {
     ...
     L7_uchar8 drid[OSAPI_INET_NTOA_BUF_SIZE];
     L7_uchar8 bdrid[OSAPI_INET_NTOA_BUF_SIZE];
     osapiInetNtoa(NewDr, drid);
     osapiInetNtoa(NewBackup, bdrid);
     osapiSnprintf(traceBuf, OSPF_MAX_TRACE_STR_LEN,
                   "New DR is %s. New BDR is %s.", drid, bdrid);
     RTO_TraceWrite(traceBuf);
   }

   /* set elected DR and BACKUP */
   if(NewDr == LocRouterId)
   {
      p_IFO->Cfg.DrId = p_IFO->Cfg.IpAdr;
   }
   else if(NewDr)
   {
      if (HL_FindFirst(p_IFO->NboHl, (byte *)&NewDr, (void *)&p_NBO) == E_OK)
      {
        p_IFO->Cfg.DrId = p_NBO->IpAdr;
      }
   }
   else /* DR was not elected */
      p_IFO->Cfg.DrId = 0;

   if(NewBackup == LocRouterId)
   {
      if(NewDr != LocRouterId)
         p_IFO->Cfg.BackupId = p_IFO->Cfg.IpAdr;
      else
         p_IFO->Cfg.BackupId = 0;
   }
   else if(NewBackup)
   {
      if (HL_FindFirst(p_IFO->NboHl, (byte *)&NewBackup, (void *)&p_NBO) == E_OK)
      {
        p_IFO->Cfg.BackupId = p_NBO->IpAdr;
      }
   }
   else /* BDR was not elected */
      p_IFO->Cfg.BackupId = 0;
         
   /* step 5 - check if the local router became to be DR or Backup */
   if(NewDr == LocRouterId)
      p_IFO->Cfg.State = IFO_DR;
   ...
   /* step 6 - if this IFO is NBMA, may be START event should be invoked */
   if((p_IFO->Cfg.Type == IFO_NBMA) && 
   ...
   if((NewDr != CurDr) || (NewBackup != CurBackup))
   {
      ...
      if((NewDr == LocRouterId) && (CurDr != LocRouterId))
      {
         /* Reoriginate the network LSA */                
         LsaReOriginate[S_NETWORK_LSA](p_ARO, (ulng)p_IFO);

         LsaReOriginate[S_GROUP_MEMBER_LSA](p_ARO, 0);
      }    
      else if (NewDr != LocRouterId)
      {
        /* If local router stops to be DR flush Network_LSA out */       
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_DR_CALC)
   {       
     RTO_TraceWrite("DR calculation done.");
   }
}
...
static void AuthPckCoding(t_IFO *p_IFO, void *p_Packet)
{
   ...
   switch(p_IFO->Cfg.AuType)
   {
      ...
      case IFO_CRYPT:      /* Cryptographic Authentication */
         ...
         /* Find the key */
         curtime = TICKS_TO_SECONDS(TIMER_SysTime());
