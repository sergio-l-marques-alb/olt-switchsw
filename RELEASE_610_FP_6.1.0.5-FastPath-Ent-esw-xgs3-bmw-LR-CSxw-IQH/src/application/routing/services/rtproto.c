                 ...
                 L7_uint32 intIfNum)
{
  ...
  */
  ...
  rc = ipMapArpReceive (intIfNum, (L7_uchar8 *) &arp_pkt);
  if (rc != L7_SUCCESS)
  {
    LOG_MSG ("rtLvl7ArpHandle: Bad rc (%d) from ipMapArpReceive\n", rc);
  }
...
L7_RC_t rtIpRecvCnfgrInitPhase1Process(void)
{
  ...
  */
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_MSG ("Buffer pool create failed\n");
    return L7_FAILURE;
  }
  ...
  */
  if (buff_count != num_buffers)
  {
    ...
    LOG_MSG ("Invalid buffer pool size: buff_count = %d, num_buffers = %d\n",
  ...
  {
    LOG_MSG ("Unable to malloc memory for network interfaces\n");
    return L7_FAILURE;
  }
