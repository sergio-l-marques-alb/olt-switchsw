...
L7_RC_t ipMapIpArpCacheSizeSet(L7_uint32 arpCacheSize)
{
  ...
  if ((arpCacheSize < L7_IP_ARP_CACHE_SIZE_MIN) ||
      (arpCacheSize > L7_IP_ARP_CACHE_SIZE_MAX))
    return L7_FAILURE;
...
L7_RC_t ipMapRouterIfMacAddressGet(L7_uint32 intIfNum, L7_uchar8 *mac_addr)
{
  ...
  if (nimGetIntfAddress(intIfNum, L7_SYSMAC_BIA, mac_addr) != L7_SUCCESS)
  {
    return(L7_FAILURE);
  }
                          ...
                          L7_uint32 intIfNum)
{
  ...
  if ((intIfNum == L7_INVALID_INTF) || ipMapIntfIsUp(intIfNum))
  {
     ...
     * happen later (ipMapRtrIntfStaticConfigApply()). */
    (void) ipMapStaticArpAddApply(ipAddress, intIfNum, pLLAddr);
  }
                                                          ...
                                                          ASYNC_EVENT_NOTIFY_INFO_t *response))
{
  ...
  static const char *routine_name = "ipMapRegisterRoutingIntfStateChange()";

  if (routerProtocol_ID >= L7_LAST_ROUTER_PROTOCOL)
  {
    IPMAP_ERROR("%s %d: %s:  ERROR: ipRegisterRoutingIntfDisable: Routing Protocol Id (%d) > " \
                "L7_LAST_ROUTER_PROTOCOL\n",
                __FILE__, __LINE__, routine_name, routerProtocol_ID);
    return L7_FAILURE;
  }
  ...
  strncpy((L7_char8 *)&(pIpIntfStateNotifyList[routerProtocol_ID].name), name,
