...
static L7_RC_t ipMapIntfEnable(L7_uint32 intIfNum);
...
/* The Linux 2.4 kernel does not accept an IPv4 MTU larger than 1500 bytes. */
#ifdef _L7_OS_LINUX_
...
#if 0
    /* This commented out. Driver does not set the IP MTU in hardware (at least 
     * not on broadcom hardware up to xgs iii). The IP MTU is associated with
     * a routing interface. The hardware can only apply the IP MTU to a physical
     * port. A physical port can be associated with many (vlan) routing interfaces. */
    if (pIpCktInfo[rtrIfNum].dtlIpIntfExists == L7_TRUE)
    {
      if (dtlIpv4MtuSet(intIfNum, ipMtu) != L7_SUCCESS)
      {
        L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
  ...
  if (_ipMapIntIfNumToRtrIntf(intIfNum, &rtrIfNum) == L7_SUCCESS)
  {
    if (ipMapIntfIsUp(intIfNum))
    {
      ...
      if (osapiIfIpv4IcmpSendDestinationUnreachableSet(intIfNum, mode) != L7_SUCCESS)
      {
        ...
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
                "IP stack failed to set the IP Unreachables mode on interface %s."
                " When issued changing the mode of sending ICMP Destination Unreachables"
                " to linux or Vxworks Stack where the support is not present.",
                 ifName);
        return L7_FAILURE;
      }
    }
  ...
  if (_ipMapIntIfNumToRtrIntf(intIfNum, &rtrIfNum) == L7_SUCCESS)
  {
    if (ipMapIntfIsUp(intIfNum))
    {
      ...
      if (osapiIfIpv4IcmpRedirectSendSet(intIfNum, mode) != L7_SUCCESS)
      {
        ...
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
                "IP stack failed to set the IP Redirects mode on interface %s.",
                ifName);
        return L7_FAILURE;
      }
    }
...
L7_RC_t ipMapRtrForwardingModeApply(L7_uint32 mode)
{
  if (mode == L7_DISABLE)
  {
    if(dtlIpv4ForwardingModeSet(mode) != L7_SUCCESS)
      LOG_MSG("Could not set forwarding mode in driver\n");
...
L7_RC_t ipMapRtrICMPEchoReplyModeApply (L7_uint32 mode)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
                "Failed to set Echo Reply mode in IP Stack."
                " When issued the changing the mode of sending "
                "ICMP echo-replies to linux or Vxworks stack "
                "where the support is not present.");
       return L7_FAILURE;
  }
...
static L7_RC_t ipMapIntfEnable(L7_uint32 intIfNum)
{
  ...
  if (!isUnnumbered)
  {
    ...
    if (rtoRouteAdd (&routeEntry) != L7_SUCCESS)
    {
      ...
      nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
      L7_LOGFNC(L7_LOG_SEVERITY_CRITICAL, 
  ...
  if (ipMapIntfSupports(intIfNum, IPMAP_INTFCAP_ARP) == L7_TRUE)
  {
    ...
    arp_reg.proxyArp = pCfg->flags & L7_RTR_INTF_PROXY_ARP;
    arp_reg.localProxyArp = pCfg->flags & L7_RTR_INTF_LOCAL_PROXY_ARP;
  ...
  /* tell clients */
  ipMapRoutingEventChangeNotify(intIfNum, L7_RTR_INTF_ENABLE, L7_FALSE, L7_NULL);
...
L7_RC_t ipMapIntfUpdate(L7_uint32 intIfNum, L7_uint32 *eventCompleted)
{
   ...
   * should be down. */
  if (ipMapMayEnableInterface(intIfNum))
  {
    ...
    if (pIpMapInfo->operIntf[intIfNum].lastNotify != L7_RTR_INTF_ENABLE)
    {
      ...
      {
        return ipMapIntfEnable(intIfNum);
      }
    }
  }
  else
...
L7_BOOL ipMapMayEnableInterface(L7_uint32 intIfNum)
{
    ...
    if (_ipMapRtrIntfConfigured(intIfNum) == L7_FALSE)
      return L7_FALSE;
...
{  
   ...
   * OSPFv2 has to come back and ask for MTU. */
                                      ...
                                      L7_BOOL isAsync, void *pData)
{
  ...
  if (isAsync)
  {
      ...
      {
          IPMAP_ERROR("Failure to get notification buffer for intIfNum %d, event %d\n", 
                      intIfNum, event);
          return L7_FAILURE;
      }  

      /* Start the event */
      osapiSemaTake(pIpMapInfo->eventHandler.registered_rwlock,L7_WAIT_FOREVER);
      if ( asyncEventSyncStart(pIpMapInfo->eventHandlerId, correlator, 
                               pCorrelatorData) != L7_SUCCESS)
      {
          IPMAP_ERROR("Failure to send notification for intIfNum %d, event %d\n", 
                      intIfNum, event);
          return L7_FAILURE;
      }
    ...
    if (pComplete->async_rc.rc != L7_SUCCESS)
    {
        if (pComplete->async_rc.reason == ASYNC_EVENT_REASON_RC_TIMEOUT)
        {
          ...
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_IP_MAP_COMPONENT_ID,
                  "Event timeout for event %s%s."
                  " IP MAP notifies other layer 3 applications when certain "
                  "events occur. These events include such things as routing "
                  "interface state changes and global routing state changes. "
                  "If an application does not acknowledge receipt of the event "
                  "within a timeout period, IP MAP logs this message.",
                  ipMapRouterEventNames[pEventData->type.rtrInfo.event], ifString);

          /* Figure out who didn't respond. */
       ...
       else
       {
         ...
         nimGetIntfName(pEventData->type.rtrInfo.intIfNum, L7_SYSNAME, ifName);
         L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_IP_MAP_COMPONENT_ID,
                 "Event failure reason %d for event %s on intIfNum %s",
                 pComplete->async_rc.reason, 
                 ipMapRouterEventNames[pEventData->type.rtrInfo.event], ifName);
       }
    }
...
void ipMapAsyncEventCompleteQueue( ipAsyncNotifyData_t *pEventData)
{
    ...
    if (rc != L7_SUCCESS)
    {
        LOG_MSG("ipMapAsyncEventCompleteQueue: osapiMessageSend failed\n");
    }
                                            ...
                                            L7_uint32 ipMask, L7_uint32 index)
{
    ...
    IPMAP_TRACE("%s %d: %s : intIfNum %d\n",
                __FILE__, __LINE__, routine_name, intIfNum);
  ...
  isUnnumbered = _ipMapIntfIsUnnumbered(intIfNum);
    ...
    for (i=0; i < L7_RTR_MAX_STATIC_ROUTES; i++)
    {
        if (route->rtrStaticRouteCfgData[i].inUse == L7_TRUE)
        {
            ...
            */
/* ROBRICE - don't think this works if for ECMP static routes where 
 * one or more next hops are on a secondary subnet. */
			for (nh = 0; (nh < L7_RT_MAX_EQUAL_COST_ROUTES) &&
                                      ...
                                      L7_IP_ADDR_t ipAddr, L7_IP_MASK_t ipMask)
{
  ...
  for (i=0; i < L7_RTR_MAX_STATIC_ROUTES; i++)
  {
    ...
    for (newNhIndex = 0, nh = 0; (nh < L7_RT_MAX_EQUAL_COST_ROUTES) &&
        (route->rtrStaticRouteCfgData[i].nextHops[nh].nextHopRtr != 0); nh++)
    {
                                        ...
                                        L7_IP_MASK_t subnetMask)
{
  ...
  {
      if ((ipMapMapIntfIsConfigurable(i, &pCfg) == L7_TRUE))
      {
        for (j=0; j < L7_L3_NUM_IP_ADDRS; j++)
        {
          ...
          if ((subnetMask & pCfg->addrs[j].ipMask & ipAddress) ==
              (subnetMask & pCfg->addrs[j].ipMask & pCfg->addrs[j].ipAddr))
          {
            return L7_TRUE;
          } 
        }
...
L7_RC_t ipMapRtrIntfDataGet( L7_uint32 intIfNum, dtlRtrIntfDesc_t *ipCircuitDesc)
{
  ...
  else if (sysIntfType == L7_PHYSICAL_INTF)
  {
    ...
    vlanId = dot1qInternalVlanIdGet(intIfNum);
  }
