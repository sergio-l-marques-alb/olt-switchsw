...
typedef struct ripIntfInfo_s     
{
  ...
  /* Bit mask indicating the components which have acquired the interface.
   * RIP may not use an interface which has been acquired by another component.
   * Indexed by component ID (L7_COMPONENT_IDS_t). We do not set a bit when
   * routing is enabled on an interface.
   */
...
typedef struct ripMapIntfEventMsg_s
{
    L7_uint32           intIfNum;       
    L7_uint32           event;          /* one of L7_PORT_EVENTS_t from NIM */
    NIM_CORRELATOR_t    correlator;
} ripMapIntfEventMsg_t;
...
typedef struct ripMapMsg_s
{
    ...
    union 
    {
      ripMapIntfEventMsg_t      intfEvent;
...
L7_RC_t ripMapIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator);
L7_RC_t ripMapIntfChangeProcess(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator);
