...
static L7_uint32 ripMapRoutesMax = L7_L3_ROUTE_TBL_SIZE_TOTAL;
...
L7_RC_t ripMapRouteTableInit()
{
  ...
  ripMapRouteTableTreeHeap = malloc (ripMapRoutesMax * sizeof (avlTreeTables_t));
  if (ripMapRouteTableTreeHeap == NULL)
  {
    LOG_MSG("Error: unable to allocate RIP Route Table tree heap.\n");
    return L7_FAILURE;
  }

  ripMapRouteDataHeap =  malloc (ripMapRoutesMax * sizeof (ripMapRouteData_t));
  if (ripMapRouteDataHeap == NULL)
  {
    LOG_MSG("Error: unable to allocate RIP Route Table data heap.\n");
    return L7_FAILURE;
  }
...
L7_RC_t ripMapRouteTableShow()
{
  ...
  if (ripMapRouteTableResourceRelease() != L7_SUCCESS)
    LOG_MSG("Could not release the RIP route table semaphore.\n");
