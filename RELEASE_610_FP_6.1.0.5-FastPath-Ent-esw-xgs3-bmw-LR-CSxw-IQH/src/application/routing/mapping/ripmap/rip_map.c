...
L7_RC_t ripMapStartTasks(void)
{
  ...
  {
    LOG_MSG("ripStartTasks: Failed to create RIP task\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("ripStartTasks: Unable to synchronize with RIP task\n");
    return L7_FAILURE;
  }
  /*-----------------------------------*/
  ...
  {
    LOG_MSG("ripStartTasks: Failed to create RIP processing task\n");
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit(  L7_RIP_PROCESSING_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("ripStartTasks: Unable to synchronize with RIP processing task\n");
    return L7_FAILURE;
  }
    ...
    if (status == L7_SUCCESS)
      {
        ...
        {
        case RIPMAP_INTF_EVENT:
          (void)ripMapIntfChangeProcess(Message.type.intfEvent.intIfNum,
...
L7_RC_t ripMapSave(void)
{
  ...
  if (pRipMapCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    {
      LOG_MSG("ripSave: Error during osapiFsWrite for config file %s\n", 
...
L7_RC_t ripMapApplyConfigData(void)
{
  ...
  */
  ...
  {
    LOG_MSG("ripApplyConfigData: Invalid RIP admin mode config value (%u)\n", 
            ripMode);
    L7_assert(1);
    return L7_FAILURE;
  }
  ...
  if (ripMapSplitHorizonModeApply(pRipMapCfgData->ripSplitHorizon) 
      != L7_SUCCESS)
  {
    LOG_MSG("ripApplyConfigData: Could not apply split horizon mode.\n");
  }
  if (ripMapAutoSummarizationModeApply(pRipMapCfgData->ripAutoSummarization) 
      != L7_SUCCESS)
  {
    LOG_MSG("ripApplyConfigData: Could not apply auto-summarization mode.\n");
  }
  if (ripMapHostRoutesAcceptModeApply(pRipMapCfgData->ripHostRoutesAccept) 
      != L7_SUCCESS)
  {
    LOG_MSG("ripApplyConfigData: Could not apply host routes accept mode.\n");
  }
  ...
  if (RIPMAP_IS_READY == L7_TRUE)
  {
    for (cfgIndex = 1; cfgIndex < L7_RIP_MAP_INTF_MAX_COUNT; cfgIndex++)
    {
            ...
            (ripMapIntfMayParticipate(intIfNum) == L7_TRUE))
        {
            ...
            {
                LOG_MSG("ripApplyConfigData: Failure to configure RIP interface " \
                          "%d\n", intIfNum);
            }
        } 
    }  /* cfgIndex */
  ...
  */
  ...
  {
    LOG_MSG("ripApplyConfigData: Could not apply Default Route Advertise mode.\n");
  }
  ...
  for (index = REDIST_RT_FIRST+1; index < REDIST_RT_LAST; index ++)
  {
    ...
    if (ripMapRouteRedistributeModeApply(index, 
        ...
        != L7_SUCCESS)
    {
      LOG_MSG("ripApplyConfigData: Could not apply Route Redist for routes from -- protocol.\n");
    }
  }
...
L7_RC_t ripMapIntfCreate(L7_uint32 intIfNum) 
{
  ...
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&pRipMapCfgData->intf[i].configId, &configId))
    {
...
L7_RC_t ripMapAdminModeQueue( L7_uint32 mode)
{
  ...
  {
    LOG_MSG("ripMapAdminModeQueue: osapiMessageSend failed\n");
  }
...
L7_RC_t ripMapIntfAdminModeQueue( L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  {
      LOG_MSG("ripMapIntfAdminModeQueue: osapiMessageSend failed\n");
  }
...
/*********************************************************************
* @purpose  Put interface events on the queue to be handled by RIP
*
...
L7_RC_t ripMapIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
      ...
      event != L7_PROBE_TEARDOWN)
  {
    ...
    status.component    = L7_RIP_MAP_COMPONENT_ID;
  ...
  {
      LOG_MSG("ripMapIntfChangeCallback: osapiMessageSend failed\n");
  }
...
*           from inside the IP Mapping layer. Since the RIP mapping layer 
*           is a "slave" to the IP Mapping layer, these events are 
*           handled via its registration to ipMapRegisterRoutingIntfStateChange().
* @notes    config entry point, must lock ripMapSemaphore to read/modify
*
*       
* @end
*********************************************************************/
L7_RC_t ripMapIntfChangeProcess(L7_uint32 intIfNum, L7_uint32 event, 
                                NIM_CORRELATOR_t correlator)
{
  ...
  /* Initialize the status for the NIM callback */
  status.intIfNum     = intIfNum;
  status.component    = L7_RIP_MAP_COMPONENT_ID;
  status.response.rc  = L7_SUCCESS;
  status.event        = event;
...
#if WPJ_PHASE_2
  /* Return if currently deregistered */
  if (ripMapDeregister.ripMapIntfChangeCallback == L7_TRUE)
  {
    nimEventStatusCallback(status);
    return rc;
  }
  ...
  /* Return if RIP map does not care about the interface */
  if (ripMapIntfIsValid(intIfNum) != L7_TRUE)
  {
    nimEventStatusCallback(status);
    return rc;
  ...
  if (RIPMAP_IS_READY == L7_TRUE)
  {
    ...
    case (L7_DETACH):
        /* Do nothing.  Wait for routing to be disabled on the interface */
      break;

    case (L7_DELETE):
      ripMapSemaTake();
      rc = ripMapIntfDelete(intIfNum);
      ripMapSemaGive();
      break;
    ...
    case L7_DOWN:
      /* do nothing here - key off of L7_NOT_ACTIVE instead */
      break;
    ...
    case L7_INACTIVE:
      /* NOTE:  This case also covers administratively disabled and 
       *        diagnostically disabled ports
       */
      if (L7_RT_BRIDGING_ROUTING_COEXISTANCE == L7_RT_BRIDGE_FIRST_THEN_ROUTE)
      {
        ripMapSemaTake();
    ...
    case L7_LAG_ACQUIRE:   
      ripMapSemaTake();
      prevAcquired = ripMapIntfAcquired(intIfNum);

      /* update aquired list */
      COMPONENT_ACQ_SETMASKBIT(pRipIntfInfo[intIfNum].acquiredList, 
                               L7_DOT3AD_COMPONENT_ID);

      /* If the interface was not previously acquired, bring RIP down. */
    ...
    case L7_LAG_RELEASE:  
      ripMapSemaTake();
      /* update aquired list */
      COMPONENT_ACQ_CLRMASKBIT(pRipIntfInfo[intIfNum].acquiredList, 
                               L7_DOT3AD_COMPONENT_ID);
  
      if (ripMapIntfAcquired(intIfNum) == L7_FALSE)
      {
        if (L7_RT_BRIDGING_ROUTING_COEXISTANCE == L7_RT_BRIDGE_FIRST_THEN_ROUTE)
        {
          if (ripMapIntfMayParticipate(intIfNum))
        ...
        {
          /* check if interface link state is up */
          if ((nimGetIntfLinkState(intIfNum, &ifState) == L7_SUCCESS) &&
              (ifState == L7_UP)) {
            if (ripMapIntfMayParticipate(intIfNum)) 
            {
              ripMapIntfAdminModeEnable(intIfNum);
    ...
    case L7_PROBE_TEARDOWN: 
      ripMapSemaTake();
      /* update aquired list */
      COMPONENT_ACQ_CLRMASKBIT(pRipIntfInfo[intIfNum].acquiredList, L7_PORT_MIRROR_COMPONENT_ID);
    ...
    case L7_SPEED_CHANGE:
      /* Nothing to do for RIP, since interface metric is already at minimum */
      break;
  ...
  {
    LOG_MSG("Received ripMapIntfChangeProcess outside of permitted phase");
  }
                                          ...
                                          ASYNC_EVENT_NOTIFY_INFO_t *pResponse)
{
  ...
  {
    LOG_MSG("ripMapRoutingEventChangeCallback: osapiMessageSend failed\n");
  }
                                        ...
                                        ASYNC_EVENT_NOTIFY_INFO_t *pResponse)
{
  ...
  case L7_RTR_INTF_CONFIG_CLEAR:
    ...
    if (ripMapIntfIsConfigurable(intIfNum, &pCfg) == L7_TRUE) 
    {
      ripMapIntfBuildDefaultConfigData(&pCfg->configId, pCfg);
      ripMapIntfAdminModeDisable(intIfNum);
    }
    ripMapSemaGive();
    break;
  ...
  if ((must_respond == L7_TRUE) && (event_completed == L7_TRUE))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_RIP_MAP_COMPONENT_ID,
              "RIP response to completion of event %s, seqno %u, on interface %u failed.",
              ipMapRouterEventNames[event], event_completion.correlator, intIfNum);
    }
  }
...
L7_RC_t ripMapIpAddressAdd(L7_uint32 intIfNum)
{
  ...
  {
    LOG_MSG("ripMapIpAddressChangeCallback: RIP app not initialized\n");
    return L7_FAILURE;
  }
...
void ripMapDebugShow(void)
{
  ...
  for (i = 1, pIntf = pRipIntfInfo; i <= L7_RIP_MAP_INTF_MAX_COUNT; i++, pIntf++)
  {
    RIP_MAP_PRT(RIP_MAP_MSGLVL_ON,
