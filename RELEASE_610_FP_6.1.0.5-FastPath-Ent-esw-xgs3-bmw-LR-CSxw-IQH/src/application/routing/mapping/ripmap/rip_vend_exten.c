...
/*
  if (mode == L7_ENABLE)
  {
    if (ripMapCtrl_g.taskCtrl.semAvail == L7_FALSE)
    {
      ...
      {
        LOG_MSG("ripMapExtenRipAdminModeSet: semaphore give error\n");
        return L7_FAILURE;
      }
    }
  }
  ...
  else  
  {
    if (ripMapCtrl_g.taskCtrl.semAvail == L7_TRUE)
    {
      ...
      {
        LOG_MSG("ripMapExtenRipAdminModeSet: semaphore take error\n");
      }
    }
  }
*/
                                                ...
                                                L7_OSPF_METRIC_TYPES_t matchType)
{
  ...
  if (ripMapIsInitialized() == L7_TRUE)
  {
    ...
    if (ripMapVendRouteRedistProtocolConvert(protocol, &rip_proto) == L7_SUCCESS)
    {
      ...
      LOG_MSG("Could not convert ripMap redistribution protocol to rip redistribution protocol.\n");
  }
...
L7_RC_t ripMapExtenRipPurgeRedistRoutes(void)
{
  ...
  while (ripMapRouteTableRouteGetNext(&routeData, &rtData) == L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("%s: Could not Delete route %x/%x from ripMap route table.\n",
  ...
  /* Release the ripmap internal redist route table resource. */
  if (ripMapRouteTableResourceRelease() != L7_SUCCESS)
    LOG_MSG("%s: Could not release the RIP route table semaphore.\n", __FUNCTION__);
                            ...
                            L7_uint32 rtFlags)
{
  ...
  {
    LOG_MSG("%s: Could not add route into ripMap route table.\n", routine_name);
  }
...
L7_RC_t ripMapExtenRouteDelete(L7_IP_ADDR_t network, L7_IP_MASK_t subnetMask)
{
  ...
  if (ripMapRouteTableRouteSearch(network, subnetMask, &routeData) == L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("%s: Could not Delete route from ripMap route table.\n", routine_name);
    }
  }
                               ...
                               L7_uint32 rtFlags)
{
  ...
  if (rcRtTypeConv == L7_SUCCESS)
  {
    ...
    else
    {
      ...
      {
        LOG_MSG("%s: Could not add route to ripMap route table.\n", routine_name);
      }
    }
  }
  else
...
L7_RC_t ripMapExtenIntfAuthTypeSet(L7_uint32 intIfNum, void *p, L7_uint32 authType)
{
  ...
  {
    LOG_MSG("Could not get Ip address list for interface %d", intIfNum);
    return L7_FAILURE;
  }
  ...
  for (count = 0; count < L7_L3_NUM_IP_ADDRS; count++)
  {
    ...
    {
      LOG_MSG("Could not set blank authen key for interface %d", intIfNum);
      return L7_FAILURE;
    }
  }
  return L7_SUCCESS; 
