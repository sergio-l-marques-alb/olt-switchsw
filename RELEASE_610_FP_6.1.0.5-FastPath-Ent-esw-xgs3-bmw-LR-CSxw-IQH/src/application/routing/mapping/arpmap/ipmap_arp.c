...
#include "arp_debug.h"
#include "arp.h"
#if (L7_IP_ARP_CACHE_SIZE_MIN > L7_IP_ARP_CACHE_SIZE_MAX)
#error Conflicting definitions for L7_IP_ARP_CACHE_SIZE_MIN, L7_IP_ARP_CACHE_SIZE_MAX
#endif
...
void ipMapArpCtxInit(void)
{
  ...
  pArpCtx->cacheMaxCt = (L7_uint32)L7_IP_ARP_CACHE_SIZE_MAX;
  pArpCtx->staticMaxCt = (L7_uint32)L7_IP_ARP_CACHE_STATIC_MAX;
  pArpCtx->gwTbl.treeEntryMax = (L7_uint32)IPM_ARP_GATEWAYS_MAX;
  pArpCtx->ipTbl.treeEntryMax = (L7_uint32)L7_IP_ARP_CACHE_SIZE_MAX;
...
L7_RC_t ipMapArpStartTasks(void)
{
  ...
  {
    LOG_MSG("%s: Failed to create ARP Reissue task\n", routine_name);
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit (L7_IPMAP_ARP_REISSUE_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("%s: Unable to initialize ARP Reissue task()\n", routine_name);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Failed to create ARP Callback task\n", routine_name);
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit (L7_IPMAP_ARP_CALLBACK_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("%s: Unable to initialize ARP Callback task\n", routine_name);
    return L7_FAILURE;
  }
  queueLengths[IPM_ARP_EXP_EVENT_QUEUE] = L7_IP_ARP_CACHE_SIZE_MAX;
  ...
  {
    LOG_MSG("%s: Failed to create ARP Timer Expire task\n", routine_name);
    return L7_FAILURE;
  }

  if (ipmArpTaskCreate() != L7_SUCCESS)
  {
      LOG_MSG("%s: Unable to start task to process ARP requests\n", routine_name);
      return L7_FAILURE;
  }
...
e_Err ipMapArpResCallbackInvoke(t_Handle userId, t_ARPResInfo *arpInfo)
{
  ...
  if(pCbInfo->pCallbackFn != L7_NULLPTR)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      ...
      {
        LOG_MSG("%s: Message queue overflow (one or more occurrences)", __FUNCTION__);
      }
                                 ...
                                 ipMapArpTableEvent_t event)
{
  ...
  /* handle special case of a gateway ARP entry being deleted
   *
   * NOTE:  Per design agreement, we MUST NOT allow an ARP entry to be removed
   *        from the device table if it is the next hop address for any route entries.
   *        Although uncommon, there are certain situations where a gateway ARP
   *        entry is removed from the ARP table.  A flag is stored in the
   *        gateway AVL entry indicating that the device ARP entry is a holdover
   *        from an earlier ARP table deletion.  The device ARP entry will be
   *        removed when this gateway table entry gets deleted.
   */
  if (ipMapArpGwTableSearch(arpEntry.ipAddr, pIntf->intIfNum, (L7_uint32)L7_MATCH_EXACT, &pGw) == L7_SUCCESS)
  {
    if ((event == IPM_ARP_TABLE_REMOVE) &&
        (pIp->inDevice == L7_TRUE))
    {
      pGw->holdover = L7_TRUE;
      memcpy((L7_uchar8 *)&arpEntry.macAddr.addr.enetAddr, nullMac, L7_ENET_MAC_ADDR_LEN);
      arpEntry.flags = 0;               /* i.e., 'unresolved' entry */
      gwHoldover = L7_TRUE;             /* set local flag to be checked below */
    }
  } /* endif GW table search */
...
L7_RC_t ipMapArpReissueTaskMsgSend(ipMapArpReissueMsg_t *msg)
{
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("%s: Message queue overflow (one or more occurrences)\n",
              routine_name);
    }
    /* task msg queue must be full -- record via debug error counter */
...
void ipMapArpReissueTask(void)
{
  ...
  {
    LOG_MSG("%s: Unable to create ARP Map Reissue Task msg queue\n", routine_name);
    L7_assert(1);
    return;
  }
...
void ipMapArpCallbackTask(void)
{
  ...
  {
    LOG_MSG("%s: Unable to create ARP Map Callback Task msg queue\n", routine_name);
    L7_assert(1);
    return;
  }
