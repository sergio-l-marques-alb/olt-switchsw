...
*
* @param    autoCostRefBw @b{(input)} the autocost referencebandwidth allowed
*
* @returns  L7_SUCCESS if configuration is successfully stored
...
{
  if (ospfMapOspfInitialized() == L7_TRUE) 
    return ospfMapExtenMulticastExtensionsGet(val);
...
{
  if (ospfMapOspfInitialized() == L7_TRUE) 
    return ospfMapExtenMulticastExtensionsSet(val);
...
{
  *mode = pOspfMapCfgData->rtr.opaqueLSAMode;
  return L7_SUCCESS;
}
...
{
  pOspfMapCfgData->rtr.opaqueLSAMode = mode;
  pOspfMapCfgData->cfgHdr.dataChanged = L7_TRUE;
...
/*********************************************************************
* @purpose  Set the OpaqueCapability to enabled.
*
...
L7_RC_t ospfMapOspfOpaqueLsaSupportSet(L7_uint32 opaqueEnabled)
{
    ...
    if (ospfMapOspfInitialized() == L7_TRUE)
      rc = ospfMapExtenOspfOpaqueCapabilitySet(opaqueEnabled);
