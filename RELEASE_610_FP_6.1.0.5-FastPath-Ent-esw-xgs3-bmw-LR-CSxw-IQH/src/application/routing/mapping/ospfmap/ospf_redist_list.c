...
static L7_uint32 ospfRedistRoutesMax = L7_L3_ROUTE_TBL_SIZE_TOTAL; 
...
L7_RC_t ospfMapRedistListCreate(void)
{
    ...
    if (ospfRedistRouteTreeHeap == NULL) {
        LOG_MSG("Error: unable to allocate OSPF redistribution tree heap.\n");
        return L7_FAILURE;
    }
    ...
    if (ospfRedistRouteDataHeap == NULL) {
        ...
        LOG_MSG("Error: unable to allocate OSPF redistribution data heap.\n");
        return L7_FAILURE;
    }
...
L7_RC_t ospfMapRedistListAdd(ospfRedistRoute_t *redistRoute)
{
    ...
    if (avlTreeCount(&ospfRedistRouteTree) >= ospfRedistRoutesMax) {
        LOG_MSG("SOFTWARE ERROR: Attempt to add to a full OSPF redistribution list\n.");
    }

    redistRoute->used = L7_TRUE;
    if (avlInsertEntry(&ospfRedistRouteTree, redistRoute) != NULL) {
        LOG_MSG("Error inserting route into OSPF redistribution list\n.");
    }
    else {
        rc = L7_SUCCESS;
    }
...
L7_RC_t ospfMapRedistListModify(ospfRedistRoute_t *redistRoute)
{
    ...
    if (routeInTree) {
        ...
        /* Shouldn't get here. */
        LOG_MSG("SOFTWARE ERROR: Failed to find route in ospfMapRedistRouteModify().\n");
    }
    osapiSemaGive(ospfRedistRouteTree.semId);
    return rc;
}
...
L7_BOOL ospfMapRedistRouteChanged(ospfRedistRoute_t *redistRoute)
{
    ...
    if (routeInTree) {
        ...
        /* Shouldn't get here. */
        LOG_MSG("SOFTWARE ERROR: Failed to find route in ospfMapRedistRouteChanged().\n");
    }
    osapiSemaGive(ospfRedistRouteTree.semId);
    return rc;
}
