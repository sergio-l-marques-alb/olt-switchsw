...
static void ospf_NeigborCB(t_Handle areaUserId, t_Handle objId,
                           t_Handle *p_nbUserId);
...
L7_RC_t ospfMapOspfVendorInit(void)
{
  ...
  L7_RC_t           cfgRc;
  L7_uint32         cfgData, adminMode, trapFlags, routingMode;
  t_SysLabel        ospfSysLabel;
  t_IFO             ifo;
  static const char *routine_name = "ospfMapOspfVendorInit()";
  ...
  callback.f_NeighborCreateInd = ospf_NeigborCB;
  ...
  {
    OSPFMAP_ERROR("%s %d: %s: Error registering for OSPF callbacks, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc );
    return L7_FAILURE;
  }
  ...
  {
    OSPFMAP_ERROR("%s %d: %s: Unable to establish management object, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc);
    return L7_FAILURE;
  }
  ...
  cfgRc |= ospfMapOspfAdminModeGet(&adminMode);
  if (cfgRc != L7_SUCCESS)
  {
    OSPFMAP_ERROR("%s %d: %s: Unable to gather router config parms\n",
                    __FILE__, __LINE__, routine_name);
    return L7_FAILURE;
  }

  routingMode = ipMapRtrAdminModeGet();
  rtrCfg.RouterStatus = ((routingMode == L7_ENABLE) && (adminMode == L7_ENABLE)) ?
    ROW_CREATE_AND_GO : ROW_CREATE_AND_WAIT;
  if ((erc = RTO_Config(ospfMapCtrl_g.RTO_Id, &rtrCfg)) != E_OK)
  {
    OSPFMAP_ERROR("%s %d: %s: Unable to configure OSPF router object, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc);
    return L7_FAILURE;
  }
  ...
  {
    OSPFMAP_ERROR("%s %d: %s: Error initializing OSPF MIB function, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc );
    return L7_FAILURE;
  }
  ...
  {
    OSPFMAP_ERROR("%s %d: %s: Error initializing OSPF MIB Bind function, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc );
    return L7_FAILURE;
  }
#endif
  ...
  {
    OSPFMAP_ERROR("%s %d: %s: Error registering for OSPF MIB callbacks, rc=%d\n",
                    __FILE__, __LINE__, routine_name, erc );
    return L7_FAILURE;
  }
                              ...
                              L7_uint32 netMask)
{
  ...
  OSPFMAP_TRACE("%s %d: %s : intIfNum = %d, ipAddr = %s, mask = %s\n",
                __FILE__, __LINE__, routine_name,
                intIfNum, ipAddrStr, netMaskStr);
  ...
  do
  {
    ...
    if (lm == 0)
    {
      OSPFMAP_ERROR("%s %d: %s: ERROR: OSPF_MAP: Unable to allocate local mgr instance for intf %d\n",
    ...
    {
      XX_Thaw();
      OSPFMAP_ERROR("%s %d: %s: ERROR: OSPF_MAP: Could not create OSPF intf %d, at capacity\n",
                    __FILE__, __LINE__, routine_name, intIfNum);
      break;
    }
                                  ...
                                  L7_uint32 neighbor)
{
  ...
  static const char *routine_name = "ospfMapOspfVirtIntfCreate()";
  ...
  do
  {
    ...
    if (cfgRc != L7_SUCCESS)
    {
      OSPFMAP_ERROR("%s: ERROR: OSPF_MAP: Unable to gather router virt intf config parms\n",
                    routine_name);
      break;                            /* exit control loop */
    }
    ...
    {
      OSPFMAP_ERROR("%s: ERROR: OSPF_MAP: Virtual IFO config error, rc=%d\n",
                    routine_name, erc);
      break;                            /* exit control loop */
    }
...
L7_RC_t ospfMapOspfIntfMetricChange(L7_uint32 intIfNum, L7_uint32 ifDataRate)
{
  ...
  if ((erc = IFO_MetricConfig_Pack(lm->IFO_Id, &mCfg)) != E_OK)
  {
    OSPFMAP_ERROR("%s %d: %s: ERROR: OSPF_MAP: Error changing interface %d metrics, rc=%d\n",
                  __FILE__, __LINE__, routine_name, intIfNum, erc);
  }
                                 ...
                                 L7_uint32 intIfNum)
{
  ...
  else
  {
    ...
    {
      OSPFMAP_ERROR("%s %d: %s: ERROR: OSPF_MAP: Interface address error, intIfNum=%d\n",
                    __FILE__, __LINE__, __FUNCTION__, intIfNum);
    }
  }
}
