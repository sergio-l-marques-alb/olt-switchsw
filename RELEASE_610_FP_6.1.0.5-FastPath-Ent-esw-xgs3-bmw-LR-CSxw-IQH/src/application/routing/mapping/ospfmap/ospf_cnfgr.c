...
static L7_RC_t ospfMapCnfgrInitPhase2Process( L7_CNFGR_RESPONSE_t *pResponse,
                                              L7_CNFGR_ERR_RC_t   *pReason );
static L7_RC_t ospfMapCnfgrInitPhase3Process( L7_CNFGR_RESPONSE_t *pResponse,
                                              L7_CNFGR_ERR_RC_t   *pReason );
...
void ospfMapApiCnfgrCommand(L7_CNFGR_CMD_DATA_t *pCmdData)
{
    ...
    {
        L7_LOG(L7_LOG_SEVERITY_EMERGENCY, L7_OSPF_MAP_COMPONENT_ID, 
...
void ospfCnfgrHandle(L7_CNFGR_CMD_DATA_t *pCmdData)
{
    ...
    switch ( command )
    {
        case L7_CNFGR_CMD_INITIALIZE:
            ospfMapRC = ospfCnfgrInitRequestHandle(request, &response, &reason);
            break;
                                          ...
                                          L7_CNFGR_ERR_RC_t *reason)
{
    ...
    switch ( request )
    {
        ...
        case L7_CNFGR_RQST_I_PHASE3_START:
            rc = ospfMapCnfgrInitPhase3Process(response, reason);
            if (rc != L7_SUCCESS)
            {
...
static L7_RC_t ospfMapInfoAllocate(void)
{
    ...
    memset(( void * )pOspfIntfInfo, 0,
           sizeof( ospfIntfInfo_t) * (L7_OSPF_INTF_MAX_COUNT + 1));
                                              ...
                                              L7_CNFGR_ERR_RC_t   *pReason )
{
    ...
    *pReason   = 0;
    ...
    if (nimRegisterIntfChange(L7_OSPF_MAP_COMPONENT_ID,
                              ospfMapIntfChangeCallback) != L7_SUCCESS)
    {
        OSPFMAP_ERROR("Unable to register with NIM for interface changes.\n");
        *pResponse = 0;
        *pReason   = L7_CNFGR_ERR_RC_FATAL;
        return L7_ERROR;
    }
...
static L7_RC_t ospfMapCnfgrInitPhase3Process( L7_CNFGR_RESPONSE_t *pResponse,
                                              L7_CNFGR_ERR_RC_t   *pReason )
{
    L7_CNFGR_QUERY_DATA_t ipMapState;
    *pResponse = L7_CNFGR_CMD_COMPLETE;
    *pReason   = 0;

    if (sysapiCfgFileGet(L7_OSPF_MAP_COMPONENT_ID, L7_OSPF_CFG_FILENAME,
                         (L7_char8 *)pOspfMapCfgData, sizeof(L7_ospfMapCfg_t),
                         &pOspfMapCfgData->checkSum, L7_OSPF_CFG_VER_CURRENT,
                         ospfBuildDefaultConfigData, ospfMigrateConfigData) != L7_SUCCESS)
    {
        OSPFMAP_ERROR("Error loading OSPF configuration.\n");
        *pResponse = 0;
        *pReason   = L7_CNFGR_ERR_RC_FATAL;
        return L7_ERROR;
    }

    /* ospfMapAppsInit() depends on IP MAP config data (for example, the
     * router ID and interface addresses). Make sure IP MAP
     * has finished phase 3 init before proceeding.
     */
    ipMapState.type = L7_CNFGR_QRY_TYPE_SINGLE;
    ipMapState.args.cid = L7_IP_MAP_COMPONENT_ID;
    ipMapState.request = L7_CNFGR_QRY_RQST_STATE;
    if (cnfgrApiQuery(&ipMapState) != L7_SUCCESS) {
        OSPFMAP_ERROR("Error getting IP MAP state in ospfMapCnfgrInitPhase3Process().\n");
        *pResponse = 0;
        *pReason   = L7_CNFGR_ERR_RC_FATAL;
        return L7_ERROR;
    }
    while ((ipMapState.data.state.current != L7_CNFGR_STATE_P3) &&
           (ipMapState.data.state.current != L7_CNFGR_STATE_WMU) &&
           (ipMapState.data.state.current != L7_CNFGR_STATE_E))
    {
        osapiSleepMSec(10);
        cnfgrApiQuery(&ipMapState);
    }
