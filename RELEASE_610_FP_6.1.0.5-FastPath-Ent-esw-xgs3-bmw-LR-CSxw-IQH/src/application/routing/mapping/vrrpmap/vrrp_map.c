...
L7_RC_t vrrpStartTasks(void)
{
  ...
  if (osapiWaitForTaskInit(L7_VRRP_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
        {
            LOG_MSG("%s: %d: Unable to synchronize VRRP daemon task\n", __FILE__, __LINE__);
            return L7_FAILURE;
        }
...
L7_RC_t vrrpSave(void)
{
  ...
  if (pVrrpMapCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    {
      LOG_MSG("vrrpSave: Error during osapiFsWrite for config file %s\n", 
...
void vrrpBuildDefaultConfigData(L7_uint32 ver)
{
  ...
  for (max = 0; max < L7_VRRP_MAX_VIRT_ROUTERS; max++)
  {
    pVrrpMapCfgData->virtRouter[max].vrid          = FD_VRRP_DEFAULT_VRID;
    pVrrpMapCfgData->virtRouter[max].priority      = FD_VRRP_DEFAULT_PRIORITY;
    pVrrpMapCfgData->virtRouter[max].preempt_mode  = FD_VRRP_DEFAULT_PREEMPT;
    pVrrpMapCfgData->virtRouter[max].adver_int     = FD_VRRP_DEFAULT_ADVERTISEMENT_INTERVAL;
    pVrrpMapCfgData->virtRouter[max].auth_types    = FD_VRRP_DEFAULT_AUTH_TYPE;
    pVrrpMapCfgData->virtRouter[max].vr_start_state= FD_VRRP_DEFAULT_START_STATE;
    for (ipAddrCount=0; ipAddrCount < L7_L3_NUM_IP_ADDRS ; ipAddrCount++)
    {
      pVrrpMapCfgData->virtRouter[max].ipaddress[ipAddrCount]  = FD_VRRP_DEFAULT_IP_ADDRESS;
    }
    for (k = 0; k < L7_VRRP_MAX_TRACK_INTF_PER_VR; k++)
    {
      pVrrpMapCfgData->virtRouter[max].vr_track_intf[k].intIfNum = 0;
      pVrrpMapCfgData->virtRouter[max].vr_track_intf[k].prio_dec = FD_VRRP_DEFAULT_TRACK_PRIORITY_DECREMENT;
      pVrrpMapCfgData->virtRouter[max].vr_track_intf[k].stateUp = L7_FALSE;
    }
    for (k = 0; k <L7_VRRP_MAX_TRACK_RT_PER_VR; k++)
    {
      pVrrpMapCfgData->virtRouter[max].vr_track_route[k].ipAddr = FD_VRRP_DEFAULT_IP_ADDRESS;
      pVrrpMapCfgData->virtRouter[max].vr_track_route[k].subnetMask = FD_VRRP_DEFAULT_IP_ADDRESS;
      pVrrpMapCfgData->virtRouter[max].vr_track_route[k].prio_dec = FD_VRRP_DEFAULT_TRACK_PRIORITY_DECREMENT;
      pVrrpMapCfgData->virtRouter[max].vr_track_route[k].reachable = L7_FALSE;
    }
    for (k = 0; k < L7_VRRP_MAX_AUTH_DATA; k++)
      pVrrpMapCfgData->virtRouter[max].auth_data[k]   = FD_VRRP_DEFAULT_AUTH_DATA_KEY_CHAR;
  }
...
L7_RC_t vrrpApplyConfigData(void)
{
  ...
  {
    LOG_MSG("vrrpApplyConfigData: Error registering with sysnet\n");
    return L7_FAILURE;
  }
  ...
  if (vrrpIntfIsConfigurable(intIfNum, &pIntfInfo) == L7_TRUE)
  {
    ...
    for(i=0;i<L7_VRRP_MAX_VIRT_ROUTERS;i++)
    {
        if(NIM_CONFIG_ID_IS_EQUAL(&pVrrpMapCfgData->virtRouter[i].configId, &configId))
        {
            ...
            pVrrpMapCfgData->virtRouter[i].vrid          = FD_VRRP_DEFAULT_VRID;
            pVrrpMapCfgData->virtRouter[i].priority      = FD_VRRP_DEFAULT_PRIORITY;
            pVrrpMapCfgData->virtRouter[i].preempt_mode  = FD_VRRP_DEFAULT_PREEMPT;
            pVrrpMapCfgData->virtRouter[i].adver_int     = FD_VRRP_DEFAULT_ADVERTISEMENT_INTERVAL;
            pVrrpMapCfgData->virtRouter[i].auth_types    = FD_VRRP_DEFAULT_AUTH_TYPE;
            pVrrpMapCfgData->virtRouter[i].vr_start_state= FD_VRRP_DEFAULT_START_STATE;

            for (ipAddrCount = 0; ipAddrCount < L7_L3_NUM_IP_ADDRS; ipAddrCount++)
            {
              pVrrpMapCfgData->virtRouter[i].ipaddress[ipAddrCount] = FD_VRRP_DEFAULT_IP_ADDRESS;
            }
            for (k = 0; k < L7_VRRP_MAX_TRACK_INTF_PER_VR; k++)
            {
              pVrrpMapCfgData->virtRouter[i].vr_track_intf[k].intIfNum = 0;
              pVrrpMapCfgData->virtRouter[i].vr_track_intf[k].prio_dec = FD_VRRP_DEFAULT_TRACK_PRIORITY_DECREMENT;
              pVrrpMapCfgData->virtRouter[i].vr_track_intf[k].stateUp = L7_FALSE;
            }
            for (k = 0; k < L7_VRRP_MAX_TRACK_RT_PER_VR; k++)
            {
              pVrrpMapCfgData->virtRouter[i].vr_track_route[k].ipAddr = 0;
              pVrrpMapCfgData->virtRouter[i].vr_track_route[k].subnetMask = 0;
              pVrrpMapCfgData->virtRouter[i].vr_track_route[k].prio_dec = FD_VRRP_DEFAULT_TRACK_PRIORITY_DECREMENT;
              pVrrpMapCfgData->virtRouter[i].vr_track_route[k].reachable = L7_FALSE;
            }
            for (k = 0; k < L7_VRRP_MAX_AUTH_DATA; k++)
              pVrrpMapCfgData->virtRouter[i].auth_data[k]   = FD_VRRP_DEFAULT_AUTH_DATA_KEY_CHAR;
        }
    }
...
void vrrpInfoShow(L7_uint32 intIfNum, L7_uchar8 vrid)
{
  ...
  {
    if (vrrpInfo->object->vrrpCfgInfo.vr_track_route[ipAddrCount].ipAddr != 0)
    {
      
      SYSAPI_PRINTF( SYSAPI_APPLICATION_LOGGING_IPMAP,  "\n %-16.15x/%-16.15x,%-6.5d,%-6.5d ",vrrpInfo->object->vrrpCfgInfo.vr_track_route[ipAddrCount].ipAddr,vrrpInfo->object->vrrpCfgInfo.vr_track_route[ipAddrCount].subnetMask, vrrpInfo->object->vrrpCfgInfo.vr_track_route[ipAddrCount].reachable, vrrpInfo->object->vrrpCfgInfo.vr_track_route[ipAddrCount].prio_dec ); 
