...
* @returns  L7_SUCCESS or L7_FALIURE
*
* @notes    Cannot set the VRRP priority if router is the address owner
*       
* @end
*********************************************************************/
...
{    
  ...
  if( L7_NULLPTR != (my_params = L7_vrrpParamsFetch (routerID,intIfNum)) )
  {
    rtrIpAddr = my_params->vrrpVirtRouterOperInfo.localAddr.ipAddr;
    if ((rtrIpAddr == my_params->vrrpCfgInfo.ipaddress[0]) && 
        (rtrIpAddr != 0) && 
        (priority !=  L7_VRRP_INTF_PRIORITY_MAX))
    {
      osapiSemaGive(VrrpConfigSema);   
      return L7_FAILURE;
    }

    my_params->vrrpVirtRouterOperInfo.oper_priority = priority; 
    if (priority != L7_VRRP_INTF_PRIORITY_MAX)
    {
      my_params->vrrpCfgInfo.priority = priority; 
...
/*********************************************************************
* @purpose  Function to assign the IP Address to the virtual router
*
* @param    routerID -  router Id
* @param    intIfNum - Interface Number
                              ...
                              L7_uint32 ipAddress)
{
  ...
  if( L7_NULLPTR != (my_params = L7_vrrpParamsFetch (routerID,intIfNum)))
  {
    if(my_params->vrrpVirtRouterOperInfo.vr_state == L7_VRRP_STATE_INIT)
    {
      ...
      /* Check that VR IP address is in same subnet as primary IP address 
       * on local interface. I guess we don't allow preconfiguration. */
      netmask = my_params->vrrpVirtRouterOperInfo.localAddr.ipMask;
      if ((ipAddress & netmask) != 
          (my_params->vrrpVirtRouterOperInfo.localAddr.ipAddr & netmask))
      {
        osapiSemaGive(VrrpConfigSema);
        return L7_FAILURE;
      }
    ...
    {
      /* If IP address used as a secondary address, return error  */
      for (i = 1; i < L7_L3_NUM_IP_ADDRS; i++)
      {
          if (my_params->vrrpCfgInfo.ipaddress[i] == ipAddress)
          {
      ...
      my_params->vrrpCfgInfo.ipaddress[0] = ipAddress;  
      if (L7_vrrpCheckPrecondition(my_params) == L7_SUCCESS)
      {
        L7_vrrpRouterStart(my_params);
      }
    }
  ...
  /* Call after giving up the semaphore */
  if (rc == L7_SUCCESS)
  {
    vrrpMaxPrioritySetIfOwner(intIfNum);
    return L7_SUCCESS;
  }
...
L7_RC_t L7_vrrpAdminModeSet(L7_BOOL adminMode) 
{
  ...
  if( adminMode == L7_TRUE)
  {
    ...
    {
      LOG_MSG("L7_vrrpAdminModeSet: Error registering with sysnet\n");
      return L7_FAILURE;
    }
...
{
	L7_RC_t				rc = L7_FAILURE;
	L7_uint32			i, FoundSubnet = 0, FoundSpace = 0;
	vrrpRouterInfo_t	*routerInfo;
	L7_rtrIntfIpAddr_t	addrs[L7_L3_NUM_IP_ADDRS];
	L7_BOOL				stopped = L7_FALSE;

  /* check this before taking VRRP semaphore */
  /* Verify that IP addr being assigned to VR is in same subnet as 
   * an address on the local interface. */ 
  if (ipMapRtrIntfCfgIpAddrListGet(intIfNum, addrs) == L7_SUCCESS)
  {
    for (i=0; i < L7_L3_NUM_IP_ADDRS; i++)
    {
      if (addrs[i].ipAddr == L7_NULL_IP_ADDR)
      {
        /* skip holes */
        continue;
      }
      if ((ipAddress & addrs[i].ipMask) == (addrs[i].ipAddr & addrs[i].ipMask))
      {
        FoundSubnet = 1;
        break;
      }
	...
	if (L7_NULLPTR != (routerInfo = L7_vrrpParamsFetch (routerID, intIfNum)))
	{
		...
		{
			/* Primary VRRP IP address should be configured*/
		...
		/*
		Do not reconfigure one of the already configured
		IP addresses
		*/
		...
		routerInfo->vrrpCfgInfo.ipaddress[FoundSpace] = ipAddress;
...
void vrrpMaxPrioritySetIfOwner(L7_uint32 intIfNum)
{
  ...
  L7_uint32 vIntIfNum, nextIntIfNum;
  L7_RC_t rc;
  L7_uint32 rtrIpAddr, rtrIntfMask, vrrpIpAddr;
  L7_uchar8 vrrpRtrPrio;
  nimUSP_t  usp;

  /* VRRP sem not held. So ok to call IP MAP */
  if (ipMapRtrIntfCfgIpAddressGet(intIfNum, &rtrIpAddr, &rtrIntfMask) != L7_SUCCESS)
  {
    return;
  }
  ...
  while (rc == L7_SUCCESS)
  {
    if (vIntIfNum == intIfNum)
    {
      /* Look at all virtual routers configured on this interface */

      /* Make sure VRRP Ip addr !=0, else prio will get set to 
          255 when vrrpIpAddr == rtrIpAddr == 0.0.0.0 */
      if ((L7_vrrpIpAddressGet(vrid, vIntIfNum, &vrrpIpAddr) == L7_SUCCESS ) && (vrrpIpAddr != 0))
      {
        if (L7_vrrpOperPriorityGet(vrid,intIfNum, &vrrpRtrPrio) == L7_SUCCESS)
        {
          if (vrrpIpAddr == rtrIpAddr)
          {
            /* VRRP router and routing interface's IP address match, so set the 
             * priority at 255 */
            if (vrrpRtrPrio != L7_VRRP_INTF_PRIORITY_MAX)
            {
              L7_vrrpOperPrioritySet(vrid,intIfNum, L7_VRRP_INTF_PRIORITY_MAX);
              if (nimGetUnitSlotPort(intIfNum, &usp)== L7_SUCCESS)
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_VRRP_MAP_COMPONENT_ID,
                      "Changing priority to %u for virtual router with VRID %u on interface %u/%u/%u."
                      " When the router is configured with the address being used as the virtual router"
                      " ID, the router’s priority is automatically set to the maximum value to ensure "
                      "that the address owner becomes the VRRP master.", 
                      L7_VRRP_INTF_PRIORITY_MAX, vrid, usp.unit, usp.slot, usp.port);
              }
            }
          }
          else
          {
            /* VRRP router and routing interface's IP address DO NOT match, so make sure
             the priority is NOT 255 */
            if (vrrpRtrPrio == L7_VRRP_INTF_PRIORITY_MAX)
            {
              L7_vrrpOperPrioritySet(vrid,intIfNum, FD_VRRP_DEFAULT_PRIORITY);

              if (nimGetUnitSlotPort(intIfNum, &usp) == L7_SUCCESS)
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_VRRP_MAP_COMPONENT_ID,
                        "Changing priority to %u for virtual router with VRID %u on interface %u/%u/%u."
                        " When the router is no longer the address owner, FASTPATH reverts the router’s"
                        " priority to the default.", 
                        FD_VRRP_DEFAULT_PRIORITY, vrid, usp.unit, usp.slot, usp.port);
              }
            }
          }
        }
      }
    } /* end of if (vIntIfNum == intIfNum) */
    rc = L7_vrrpNextGet(vrid, vIntIfNum, &nextVrid, &nextIntIfNum);
    vrid = nextVrid;
    vIntIfNum = nextIntIfNum;
  } /* end of while */
                                    ...
                                    L7_uchar8 prio_dec)
{
  ...
  routeTable.ipAddr = netAddr;
  while ( rtoNextRouteGet( &routeTable, L7_FALSE) == L7_SUCCESS)
  {
    if ((routeTable.subnetMask == mask) && (routeTable.ipAddr == netAddr) )
      routFound = L7_TRUE;
  }
  ...
  if( L7_NULLPTR != (my_params = L7_vrrpParamsFetchLocked (vrId, intIfNum, L7_FALSE)))
  {
    for (i = 0; i< L7_VRRP_MAX_TRACK_RT_PER_VR;i++)
    {
      ...
      else if ((my_params->vrrpCfgInfo.vr_track_route[i].ipAddr == 0)
          && (emptyPosition == -1))
      {
        emptyPosition = i;
      }
    ...
    {
      if (my_params->vrrpCfgInfo.vr_track_route[emptyPosition].ipAddr == 0)
      {
        if (routFound == L7_TRUE)
        {
      ...
      if (my_params->vrrpCfgInfo.vr_track_route[emptyPosition].reachable != L7_TRUE)
      {
        if ( my_params->vrrpVirtRouterOperInfo.oper_priority != L7_VRRP_INTF_PRIORITY_MAX)
        {
          (void) vrrpOperationalPriorityCalc(my_params);
        }
        my_params->vrrpCfgInfo.vr_track_route[emptyPosition].reachable = L7_FALSE;
      } /* end if reachability check */
...
L7_RC_t L7_vrrpOperTrackRoutePrioGet(L7_uchar8 vrId, L7_uint32 intIfNum,
    L7_uint32 ipAddr,L7_uint32 netMask, L7_uchar8 *prio_dec )
{
  vrrpRouterInfo_t   *my_params;
  L7_BOOL entryFound = L7_FALSE;
  L7_RC_t rc = L7_FAILURE;
  ...
  if( L7_NULLPTR != (my_params = L7_vrrpParamsFetchLocked (vrId,intIfNum, L7_FALSE)))
  {
    for (i = 0; i< L7_VRRP_MAX_TRACK_RT_PER_VR;i++)
    {
      if ((my_params->vrrpCfgInfo.vr_track_route[i].ipAddr == ipAddr) && 
...
L7_RC_t L7_vrrpOperTrackRouteReachabilityGet(L7_uchar8 vrId, L7_uint32 intIfNum,
    L7_uint32 ipAddr,L7_uint32 netMask, L7_BOOL *reachable )
{
  vrrpRouterInfo_t   *my_params;
  L7_BOOL entryFound = L7_FALSE;
  L7_RC_t rc = L7_FAILURE;
                                     ...
                                     L7_uint32 * ipAddr, L7_uint32 * mask )
{
  ...
  while (1)
  {
    ...
    /* get the minumum Route greater than the asked for */
    tmpIpAddr = (L7_uint32) (-1);
    tmpMask = (L7_uint32) (-1); 
    for (i = 0; i < L7_VRRP_MAX_TRACK_RT_PER_VR ; i++)
    {
      if (routerInfo->vrrpCfgInfo.vr_track_route[i].ipAddr != 0)
      {
        if (routerInfo->vrrpCfgInfo.vr_track_route[i].ipAddr < *ipAddr)
          continue;
        else if ((routerInfo->vrrpCfgInfo.vr_track_route[i].ipAddr == *ipAddr) &&
    ...
    if (tmpIpAddr != (L7_uint32) (-1))
      break;
    /* no route is just greater than the asked for */
    *vrId = routerInfo->vrrpCfgInfo.vrid;
    *intIfNum = routerInfo->vrrpVirtRouterOperInfo.intIfNum;
    routerInfo = L7_NULLPTR;
  }
