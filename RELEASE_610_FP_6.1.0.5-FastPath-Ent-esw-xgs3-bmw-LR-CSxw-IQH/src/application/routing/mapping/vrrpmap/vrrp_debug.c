...
#include "vrrp_util.h"
#include "vrrptask.h"
#define VRRP_DEBUG_PACKET_RX_FORMAT "Pkt RX - Intf:%d/%d/%d(%d),SrcIP:%s,DestIp:%s,Version:%d,PktType:%d,VrId: %d,Priority: %d,Count IP Addrs:%d,AuthType:%d,Adver int:%d,AuthData :%s,Addresses:%s"
#define VRRP_DEBUG_PACKET_TX_FORMAT "Pkt TX - Intf: %d/%d/%d(%d),SrcIP:%s,DestIp :%s,Version:%d,PktType:%d,VrId:%d,Priority:%d,Count IP Addrs:%d,AuthType:%d,Adver int:%d,AuthData :%s,Addresses:%s"
...
void vrrpDebugPacketRxTrace(L7_uint32 intIfNum, L7_uchar8* msg)
{
  ...
  /* Convert from intIfNum to USP */
    ...
    if (usmDbUnitSlotPortGet(intIfNum, &unit, &slot, &port) == L7_SUCCESS)
      ...
      unit, slot, port, intIfNum, srcIp,destIp, version, type, pdu->vrid, pdu->priority, pdu->naddr, pdu->auth_type, pdu->adver_int, auth_data, address);
      }
  return;
}
