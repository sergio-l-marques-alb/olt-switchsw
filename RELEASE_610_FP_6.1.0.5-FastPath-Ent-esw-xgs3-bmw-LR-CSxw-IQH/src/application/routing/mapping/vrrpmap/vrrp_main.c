...
void VRRPDaemonTask()
{
  ...
  do
  {
    ...
    case (vrrpMsgIntfEvent):
      (void)vrrpMapIntfChangeProcess(msg.type.intfEvent.intIfNum, msg.type.intfEvent.event, msg.type.intfEvent.correlator);
      break;
    ...
    case vrrpMsgBestRouteEvent:
        ...
        break;
    default:
      LOG_MSG("vrrpRecvTask: invalid mesage type. %s:%d\n", __FILE__, __LINE__);
      break;
    }       
  }while (1);
...
void VRRPAdvertTimerCheck( )
{
  ...
  vrrpMsg_t      msg;
    ...
    if ( (L7_vrrpAdminModeGet(&adminMode) == L7_SUCCESS) && (adminMode == L7_TRUE) )
    {
        if (!vrrp_Queue)
        {
            LOG_MSG("error  in VRRPAdvertTimerCheck() \n");
        }
        ...
        if (rc != L7_SUCCESS)
        {
          if (logMsgDisplayed == L7_FALSE)
          {
            ...
            LOG_MSG("Advt Timer Tick send failed (one or more occurrences).\n");
          }
...
void VRRPMasterCheck()
{
  ...
  L7_BOOL adminMode;
    ...
    if ( (L7_vrrpAdminModeGet(&adminMode) == L7_SUCCESS) && (adminMode == L7_TRUE) )
    {
        if (!vrrp_Queue)
        {
            LOG_MSG("error  in VRRPMasterCheck() \n");
        }
        ...
        if (rc != L7_SUCCESS)
        {
          ...
          {
            logMsgDisplayed = L7_TRUE;
            LOG_MSG("Master Check Timer Tick send failed (one or more occurrences).\n");
          }
          vrrpQueueOverflowCtr++;
        }
  }
...
L7_RC_t VRRPMasterTimeCheck()
{
  ...
  L7_double64     lastPacket_time, elapsed_upTime;
        ...
        else
        {
          ...
          if (((routerInfo->vrrpVirtRouterOperInfo.last_packet_recvtime.seconds == 0) &&
              ...
              ((routerInfo->vrrpVirtRouterOperInfo.last_packet_recvtime.seconds != 0) &&
               (lastPacket_time > (L7_double64)(3*(routerInfo->vrrpCfgInfo.adver_int)*SECONDS_FACTOR + skewTime))))
          {
            (routerInfo->vrrpVirtRouterOperInfo.num_time_become_master)++;
...
L7_RC_t vrrpPacketProcess(L7_netBufHandle netBufHandle,L7_uint32 intIfNum)
{
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_MSG("VRRP Packet Enqueue failed.");
  }
  return rc;
}
                           ...
                           L7_uint32 intIfNum, L7_uint32 recv_pktlen)
{
  ...
  /* First verifyy the vrid, whether the packet is meant for this router*/
              ...
              (vrrpRouterInfo_t*) L7_vrrpVirtualRouterFetch(vrrp_pkt->vrid,intIfNum)))
  {
    ...
    /* RFC requires that the source IP address be the primary IP address. 
     * The primary addresses have to be in the same subnet as the VRRP IP 
     * address. Thus the source address in any received packet must be in the 
     * same subnet as the primary address on our local interface. */
    netmask = routerInfo->vrrpVirtRouterOperInfo.localAddr.ipMask;
    if ((osapiNtohl(ip_hdr->iph_src) & netmask) != 
        (routerInfo->vrrpVirtRouterOperInfo.localAddr.ipAddr & netmask))
    {
      osapiSemaGive(VrrpConfigSema);
      return L7_FAILURE;
    }
