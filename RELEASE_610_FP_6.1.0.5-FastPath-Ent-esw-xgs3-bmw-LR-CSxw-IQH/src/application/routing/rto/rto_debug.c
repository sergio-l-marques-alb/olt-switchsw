...
void rtoRouteShow(L7_uint32 detail);
...
void rtoRouteShow(L7_uint32 detail)
{
    ...
    sysapiPrintf("tot_adds = %u, tot_dels = %u, dup_adds = %u, bad_adds = %u, "
    ...
    sysapiPrintf("\nNetwork          NetMask          Proto             Best   Pref   Gateway        IfNum  IfStat\n");
        ...
        while (routeInfo)
        {
            ...
            for (j = 0; j < routeInfo->numNextHops; j++)
            {
                if (j != 0)
                    sysapiPrintf("%-64s  ",  "");
                sysapiPrintf("%-15.15s  ",
                                 rtoLongToIP(routeInfo->nextHops[j].nextHopIP, buf));

                sysapiPrintf(" %3.1u   ", (unsigned int)routeInfo->nextHops[j].intIfNum);
                sysapiPrintf("  %2.1u  ", (unsigned int)routeInfo->nextHops[j].ifUp);
                sysapiPrintf("\n");
            }
...
again:
    ...
    for(i = 0; i < max_routes; i++)
    {
        ...
        printf("rto6RtTest(%u): adding ip %x mask %x ", rtoRouteCount(L7_FALSE),
