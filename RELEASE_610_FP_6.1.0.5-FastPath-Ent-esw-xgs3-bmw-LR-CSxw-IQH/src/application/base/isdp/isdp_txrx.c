                        ...
                        L7_uint32 intIfNum)
{
  ...
  {
    LOG_MSG("isdpPduTransmit(): could not get a buffer\n");
    return L7_FAILURE;
  }
...
L7_RC_t isdpTxRxHeaderTagRemove(L7_netBufHandle bufHandle)
{
  ...
  L7_uchar8 *offset;
  L7_uint32 diff;
  L7_uchar8 temp[ISDP_MAX_PDU_SIZE];
                       ...
                       sysnet_pdu_info_t *pduInfo)
{
  ...
  {
    LOG_MSG("Couldn't remove header %d\n", pduInfo->intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("Foreign pdu on interface %d\n", pduInfo->intIfNum);
    return L7_FAILURE;
  }
...
L7_RC_t isdpTcnTx(L7_uint32 intIfNum, L7_uchar8 version)
{
  ...
  sysapiRegistryGet(SERIAL_NUM, STR_ENTRY, (void*) pdu->isdp_device_id);
                                          ...
                                          L7_uchar8    **data_ptr)
{
  ...
  isdpTxRxTlvTypeLengthPut(ISDP_ADDRESS_TLV_TYPE,
                           sizeof(isdpTlvAddress_t) * pdu->isdp_address_number +
...
L7_RC_t isdpV2CheckSum(void* data, size_t length, L7_ushort16* checksum)
{
  ...
  {
    LOG_MSG("One of parameters is equal to NULL\n");
    return L7_FAILURE;
  }
