...
L7_BOOL isdpIntfIsConfigurable(L7_uint32 intIfNum, isdpIntfCfgData_t **pCfg)
{
  ...
  if (nimConfigIdGet(intIfNum, &cfgId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&cfgId, &(isdpCfg->cfg.intfCfgData[index].cfgId)) == L7_FALSE)
    {
      ...
      */
      LOG_COMPONENT_MSG(L7_ISDP_COMPONENT_ID,
                        "Error accessing isdp config data for interface %d in %s.\n",
                        intIfNum, __FUNCTION__);
    ...
    NIM_CORRELATOR_t correlator)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "isdpIntfChangeCallback is not ready");
  ...
  if (!(ISDP_IS_READY))
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Received an interface callback while outside the EXECUTE state");
    nimEventStatusCallback(status);
    return L7_FAILURE;
  }
  ...
  if (event != L7_CREATE      && event != L7_DELETE      &&
      ...
      event != L7_LAG_ACQUIRE && event != L7_PROBE_SETUP &&
      event != L7_LAG_RELEASE && event != L7_PROBE_TEARDOWN)
  {
    status.response.rc = L7_SUCCESS;
    nimEventStatusCallback(status);
    return L7_SUCCESS;
                                            ...
                                            L7_uint32 intfEvent)
{
  ...
  else if (intfEvent == L7_PROBE_SETUP)
  {
    COMPONENT_ACQ_SETMASKBIT(isdpIntfTbl[intIfNum].acquiredList,
...
isdpEntry_t * isdpEntryDeviceIdFind(isdpEntryKey_t key, L7_uchar8* deviceId)
{
  ...
  {
    if(strcmp(pEntry->key.deviceId, deviceId) == 0)
    {
      return pEntry;
    }
...
L7_RC_t isdpEntryAdd(isdpEntry_t * pEntry)
{
    ...
    {
      L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Param is NULL");
      return L7_FAILURE;
    }
    ...
    { /* found existing entry */
      ...
      if(rc != L7_SUCCESS)
      {
        L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "couldn't delete the entry");
        return rc;
      }
...
L7_RC_t isdpEntryDelete(isdpEntryKey_t key)
{
  ...
  if(pEntry == L7_NULLPTR)
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "entry not found");
    rc = L7_FAILURE;
  }
  ...
  if(rc == L7_FAILURE)
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "couldn't delete entry");
  }
...
L7_RC_t isdpFillEntry(isdpEntry_t *pIsdpEntry, isdpPacket_t *pdu, L7_uint32 intIfNum)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Param/s is/are NULL");
    return L7_FAILURE;
  }
...
L7_RC_t isdpModeApply(L7_uint32 mode)
{
  ...
  if (mode == L7_DISABLE)
  {
    ...
    if(rc != L7_SUCCESS)
    {
      L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "couldn't clean neighbors table!");
    }

    rc = isdpTxRxDeinit();
    if (rc != L7_SUCCESS)
    {
      L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Unable to deregister isdp callback with Sysnet!");
      rc = L7_FAILURE;
    }
  }
  else if (mode == L7_ENABLE)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Unable to register for isdp callback with Sysnet!");
      rc = L7_FAILURE;
    }
  }
...
L7_RC_t isdpIntfModeApply(L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  if (mode == L7_DISABLE)
  {
    ...
    if(rc != L7_SUCCESS)
    {
      L7_LOG(L7_LOG_SEVERITY_INFO, L7_ISDP_COMPONENT_ID, "Couldn't delete entries");
    }
  }
