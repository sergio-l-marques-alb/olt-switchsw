...
L7_RC_t sFlowRcvrAddressSet(L7_uint32 index, L7_inet_addr_t *inetAddr)
{
  ...
  else
  {
                         ...
                         L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
    {
      ...
      {
        LOG_MSG("sFlowRcvrAddressSet: Failed to give msgQueue semaphore\n");
      }
      osapiWriteLockGive(agent.sFlowCfgRWLock);
      return L7_SUCCESS;
    }
                          ...
                          L7_uint32 timeout)
{
                           ...
                           L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
  {
    if (osapiSemaGive(agent.sFlowMsgQSema) != L7_SUCCESS)
    {
      LOG_MSG("sFlowRcvrOnwerSet: Failed to give msgQueue semaphore\n");
    }
    osapiWriteLockGive(agent.sFlowCfgRWLock);
    return L7_SUCCESS;
  }
                  ...
                  L7_uint32 samplingRate)
{
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
                           ...
                           L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
    {
      if (osapiSemaGive(agent.sFlowMsgQSema) != L7_SUCCESS)
      {
        LOG_MSG("sFlowFsPacketSamplingRateSet: Failed to give msgQueue semaphore\n");
      }
      rc = L7_SUCCESS;
    }
  }
                           ...
                           L7_uint32 interval)
{
  ...
  if ((pPoller = sFlowPollerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
                           ...
                           L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
    {
      if (osapiSemaGive(agent.sFlowMsgQSema) != L7_SUCCESS)
      {
        LOG_MSG("sFlowCpIntervalSet: Failed to give msgQueue semaphore\n");
      }
      rc = L7_SUCCESS;
    }
  }
