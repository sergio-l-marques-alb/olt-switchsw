...
void dhcps_Task(void)
{
  ...
  /* set last save time */
  lastCfgSaveTicks = osapiUpTimeRaw();
  ...
  while(1)
  {
    ...
    if( 0 != L7_DHCPS_CFG_SAVE_INTERVAL_SECS &&
       (osapiUpTimeRaw() - lastCfgSaveTicks) > L7_DHCPS_CFG_SAVE_INTERVAL_SECS)
    {
       if(L7_TRUE == pDhcpsMapLeaseCfgData->cfgHdr.dataChanged)
       {
         ...
         /* save config data */
         if( dhcpsSaveLeaseCfg() == L7_SUCCESS)
  ...
  else
  {
    ...
    if(LEASE_DATA(pLease)->state == EXPIRED_LEASE)
    {
      ...
      else if(requestedIPAddr != 0)
      {
        ...
        /* excluded? */
        if(dhcpsCheckIfExcluded(requestedIPAddr, L7_TRUE) == L7_TRUE)
        {
          DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED,
                             ...
                             dhcpsOption_t **option82)
{
  ...
  while(msgOptionsLength > 0)
  {
    ...
    case DHCPS_OPTION_TYPE_REQUEST_IP_ADDR:
        ...
        if (0 == (*requestedIPAddr))
        {
          DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED, "dhcpsParseDhcpPacket dropping message with requestedIpAddr=0\n");
          pDhcpsInfo->dhcpsStats.numOfMalformedMessages++;
          return L7_FAILURE;
        }
        break;
                           ...
                           L7_BOOL isSysMgmtIntf)
{
    ...
    /* send via dtl */
    if(dtlIpBufSend(intIfNum, vlanId, 0, pBufHandle)==L7_SUCCESS)
    {
...
L7_RC_t dhcpsGetFreeIpAddr(dhcpsPoolNode_t * pPool, L7_IP_ADDR_t * pIpAddr)
{
  ...
  while(ipAddr < maxIpAddr)
  {
    if(dhcpsFindLeaseByIp(pPool,ipAddr, &pLease) == L7_SUCCESS)
    {
      ...
      {
        /* excluded? */
        if(dhcpsCheckIfExcluded(ipAddr, isManualExcluded) == L7_TRUE)
        {
          DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED,
    ...
    {
      if(dhcpsCheckIfExcluded(ipAddr, isManualExcluded) == L7_FALSE)
      {
                                        ...
                                        L7_uint32 incomingIfNum)
{
  ...
  if(dhcpsMatchPool(incomingSubnetAddr, &clientKey, &pPool) == L7_SUCCESS)
  {
    ...
    if(POOL_DATA(pPool)->poolType != MANUAL_POOL)
    {
      ...
      {
        DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED,
          "process BOOTP REQUEST no manual pool configured for bootp client\n");
        return L7_FAILURE;
      }
                                    ...
                                    dhcpsOption_t *option82)
{
  ...
  while (pLease != L7_NULLPTR)
  {
    ...
    if(dhcpsCheckIfExcluded(LEASE_DATA(pLease)->ipAddr, L7_FALSE) == L7_TRUE &&
  ...
  {
    /* excluded? */
    if(dhcpsCheckIfExcluded(ipAddrRequested, L7_TRUE) == L7_TRUE)
    {
      DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED,
        "Ignoring discover requested IP option (excluded IP)\n");
    }
                               ...
                               L7_uint32 leaseTime))
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DHCPS_MAP_COMPONENT_ID,
            "Registrar ID %d is already registered", (L7_int32)registrar_ID);
    return(L7_FAILURE);
  }
...
L7_RC_t dhcpsSendTypeNotifyDeRegister(L7_uint32 registrar_ID)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DHCPS_MAP_COMPONENT_ID,
            "Registrar ID %d is already unregistered", (L7_int32)registrar_ID);
    return(L7_FAILURE);
  }
