...
L7_RC_t trapMgrPhaseTwoInit()
{
  ...
  {
    LOG_MSG("trapMgrPhaseTwoInit(): Could not register with nvStore\n");
  }
  ...
  if (rc == L7_FAILURE)
  {
    LOG_MSG("TRAPMGR: Failed to register with CARDMGR\n");
  }

  /* register with Power Unit Manager for power module event change */
  if((rc = pumgrAllTrapsRegister(L7_TRAPMGR_COMPONENT_ID, trapMgrCardEventCallBack)) != L7_SUCCESS)
  {
    LOG_MSG("TRAPMGR: Failed to register with Power Unit Manager\n");
  }

  /* register with Fan Unit Manager for fan module event change */
  if((rc = fumgrAllTrapsRegister(L7_TRAPMGR_COMPONENT_ID, trapMgrCardEventCallBack)) != L7_SUCCESS)
  {
    LOG_MSG("TRAPMGR: Failed to register with Fan Unit Manager\n");
  }
...
void trapMgrLogTrapToLocalLog(L7_char8 *trapStringBuf)
{
  ...
  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_TRAPMGR_COMPONENT_ID, trapStringBuf);
...
L7_RC_t trapMgrRegister(trapMgrFunctionList_t *trapMgrFunctionList) {
  ...
  {
    LOG_MSG("trapMgr registrar_ID %d greater then L7_LAST_TRAP_ID\n", 
  ...
  {
    LOG_MSG("trapMgr registrar_ID %d already registered\n", (L7_int32)trapMgrFunctionList->registrar_ID);
    return(L7_FAILURE);
  }
...
L7_RC_t trapMgrDeregister(L7_TRAP_ID_t registrar_ID)
{
  ...
  {
    LOG_MSG("trapMgr registrar_ID %d not registered\n", registrar_ID);
    return(L7_FAILURE);
  }
...
/*          osapiTimerAdd(trapMgrStackMasterFailedLogTrapSend, trap.u.stackMasterFailed.oldUnitNumber, L7_NULL, 60000, &pTimerHolder);*/
          break;
