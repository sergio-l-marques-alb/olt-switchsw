...
L7_RC_t hpcConfigSet()
{
  ...
  (void) sal_config_set(spn_MEMLIST_ENABLE, "1");     /* PTin modified */
  (void) sal_config_set(spn_REGLIST_ENABLE, "1");
  (void) sal_config_set(spn_HELPCLI_ENABLE, "1");

#ifdef _L7_OS_LINUX_
  /* Use polling on Linux. Was a fixup before */
  if (sal_config_set(spn_SCHAN_INTR_ENABLE, "0") != 0) LOG_ERROR(33);
  if (sal_config_set(spn_MIIM_INTR_ENABLE, "0") != 0) LOG_ERROR(33);
#ifdef LVL7_RAPTOR
