...
void osapiSwVersionRegister(void)
{
  ...
  {
      int comma_len;
      fscanf(vpd_fd, "Vital Product Data:-\n");
      fscanf(vpd_fd, "Operational Code Image File Name - %s\n", swFileName);
      fscanf(vpd_fd,"Rel %s Ver %s Maint Lev %s Bld No %s",
             rel,ver,maint,bld);
      fscanf(vpd_fd, "\nTimestamp - %28c\n", swTimeStamp);
...
void osapiOSVersionRegister(void)
{
  ...
  if (fver != NULL)
  {
    fscanf(fver,"%s %s %s", linux_string, version_string, kern_string);
    fclose(fver);
  }
