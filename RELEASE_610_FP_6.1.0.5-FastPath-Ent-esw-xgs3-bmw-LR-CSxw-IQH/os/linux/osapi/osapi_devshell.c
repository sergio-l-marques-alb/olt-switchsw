...
L7_uint32 osapiAddressLookup(L7_char8 *funcName) {
  ...
  f = fopen("/usr/local/ptin/sbin/devshell_symbols.txt", "r");
#else

/* zcat does not work correctly under valgrind*/
#if defined(L7_TOOL_VALGRIND)
  f = fopen("/usr/local/ptin/sbin/devshell_symbols", "r");
  if (f == NULL) {
      /* f is NULL; this may be possible if valgrind is not running, try the zcat */
      f = popen("zcat /usr/local/ptin/sbin/devshell_symbols.gz 2>/dev/null", "r");
  }
#else
  f = popen("zcat /usr/local/ptin/sbin/devshell_symbols.gz 2>/dev/null", "r");
#endif
  ...
  while (!feof(f)) {
    fscanf(f, OSAPI_ADDRESS_LOOKUP_FMT_STRING, &sym_addr, buf);
...
L7_RC_t osapiFunctionLookup(L7_uint32 addr, L7_char8 *funcName,
  ...
  while (!feof(f)) {
    fscanf(f, OSAPI_ADDRESS_LOOKUP_FMT_STRING, &cur_addr, buf);
    if (cur_addr <= addr) {
...
void osapiLkup(L7_char8 *search_str) {
  ...
  // PTin added: to avoid devshell crashes
  if(search_str == NULL) {
    return;
  }
  // PTin end
