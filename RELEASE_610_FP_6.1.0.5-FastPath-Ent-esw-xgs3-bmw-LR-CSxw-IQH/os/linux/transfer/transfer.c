                             ...
                             L7_char8 *local_filename, L7_int32 direction)
{
   ...
   if (inet_address->family == L7_AF_INET)
   {
      if (osapiInetNtop(L7_AF_INET, (L7_uchar8 *)&(inet_address->addr.ipv4),
          ...
          == L7_NULL)
      {
        LOG_MSG("osapiInetNtop error for %s\n",server_ip);
        return L7_FAILURE;
      }
   }
   ...
   {
        LOG_MSG("Address Family is not Supported \n");
        return L7_FAILURE;
   }
   ...
   {
     LOG_MSG("tftp command failed for %s\n",server_ip);
     return (L7_FAILURE);
   }
   return rc;
}
...
#define LINUX_MEM_INFO_FILE_NAME "/proc/meminfo"
  ...
  if ((filedesc = osapiFsOpen(LINUX_MEM_INFO_FILE_NAME)) != L7_ERROR)
  {
    while (readCnt < 2)
    {
      ...
      if(readCnt == 1)
      {
        ...
        {
          LOG_MSG("Kernel Free Memory Check failure occured.%s %d\n", __FUNCTION__, __LINE__);
          osapiFsClose(filedesc);
          return L7_FAILURE;
        }
      }
   ...
   switch (transfer_ftype) {
      ...
      case L7_FILE_TYPE_CODE:
         snprintf(full_fname, sizeof(full_fname), "%s%s",
                  CONFIG_PATH, transfer_fname_local);
         break;
#ifdef L7_DHCP_SNOOPING_PACKAGE
    case L7_FILE_TYPE_DHCP_SNOOPING_BINDINGS:
   ...
   case L7_FILE_TYPE_TXTCFG:
         ...
         if(L7_SUCCESS != cliTxtCfgCopyConfig (SYSAPI_TXTCFG_FILENAME, full_fname, L7_FALSE))
         {
           /* indicate preparation copy failure */
            set_result_string("Error while preparing file for transfer: Copy failed!");
            set_upload_result_code(L7_TRANSFER_CODE_ERROR_STARTING);
            set_result(L7_TRANSFER_FAILED);
            return;
         }
 ...
 *
 *************************************************************************/
L7_RC_t validate_image(L7_uchar8 *fileName, L7_uchar8* errBufPtr, L7_uint32 maxlen)
{
  ...
  if ( filedesc < 0 )
  {
    osapiStrncpySafe(errBufPtr, "Error opening the image file \n", maxlen);
    return L7_FAILURE;
  }
  ...
  {
    osapiStrncpySafe(errBufPtr, "Image file is incomplete \n", maxlen);
    close(filedesc);
    return L7_FAILURE;
  }
     ...
     (osapiNtohl(stkHeader.tag2) == STK_TAG2))
  {
    ...
    {
      osapiStrncpySafe(errBufPtr, "Can't figure out image size \n", maxlen);
      close(filedesc);
      return L7_FAILURE;
    }
    ...
    {
      /*osapiStrncpySafe(errBufPtr, "Invalid STK checksum, file is corrupt \n", maxlen);*/
      snprintf(errBufPtr, maxlen, "Checksum failed, read %x, computed %x\n", 
	       osapiNtohs(stkHeader.crc), computed_crc);
      close(filedesc);
      return L7_FAILURE;      
    }
    /* Sanity-check sizes */
    ...
    {
      osapiStrncpySafe(errBufPtr, "Too many images in STK file to process \n", maxlen);
      close(filedesc);
      return L7_FAILURE;      
    }
    ...
    read(filedesc, &buffer[0], osapiNtohl(stkHeader.stk_header_size));
    ...
    {
      osapiStrncpySafe(errBufPtr, "Empty STK file \n", maxlen);
      close(filedesc);
      return L7_FAILURE;
    }
    ...
    {
      osapiStrncpySafe(errBufPtr, "STK file does not contain any images for this platform \n", maxlen);
      close(filedesc);
      return L7_FAILURE;
    }
  } 
  ...
  {
    osapiStrncpySafe(errBufPtr, "Unrecognized image format \n", maxlen);
    close(filedesc);
    return L7_FAILURE;
  }
  close(filedesc);
...
L7_RC_t transfer_process_code(char *fileName)
{
  /* We're only called on one task's thread; use static buffers to save
     stack space */
  static L7_char8 buf[0x80];
  static L7_char8 errBuf[0x80];

  L7_int32 rc;

  static L7_char8 old_image[40];
  ...
  rc = validate_image(buf, errBuf, sizeof(errBuf));

  if (rc != L7_SUCCESS)
  {
    set_result_string(errBuf);
    set_download_result_code(L7_TRANSFER_CODE_FAILURE);
    set_result(L7_OTHER_FILE_TYPE_FAILED);
    return (L7_FAILURE);
  }

#ifndef L7_CODE_UPDATE_TO_FLASH
  /* move the file to the CONFIG_PATH */

  snprintf(buf, sizeof(buf), "mv %s/%s %s >/dev/null 2>&1",
      DOWNLOAD_PATH, fileName, CONFIG_PATH);

  if (WEXITSTATUS(system(buf)) != 0) {

    /* indicate extraction failure... */
    set_result_string("Failure copying upgrade instructions!");
  ...
  /* Check if this image is known to DIM */

  dimIsAnImage(fileName, (int *)&rc);

  if(rc == DIM_IMAGE_DOESNOT_EXIST)
  {
    /* we need to delete the existing backup image */

    if( dimBackupImageFileNameGet(&old_image[0]) == 0)
    {
      osapiFsDeleteFile(old_image);
    }
  }

  /* add this image to dual image manager */

  if (dimImageAdd(fileName) != DIM_SUCCESS)
  {
   ...
   rc = cdaUnpackSTK(fileName, L7_TRUE);
   if (rc != L7_SUCCESS)
   {
   ...
   */
   rc = usmdbCdaCodeUpdate(CDA_ALL_UNITS, fileName, transfer_fname_local);
...
#define KERNEL_FREE_MEMORY 5000 /* KB */
   ...
   switch (transfer_ftype) {
      ...
      if ( 0 == strcmp(transfer_fname_local,DIM_BACKUP_IMAGE))
      {
      ...
      else if ( 0 == strcmp(transfer_fname_local,DIM_ACTIVE_IMAGE))
      {
      ...
      {
        strcpy(tmpFileName,transfer_fname_local);
      }

      if (L7_ERROR == rc)
      {
        set_result(L7_OTHER_FILE_TYPE_FAILED);
        set_result_string("Failure to identify destination image.");
...
**
***************************************************************************/
void L7_transfer_start(L7_uint32 direction)
{
   L7_char8 buf[80];
   L7_RC_t rc;
   L7_uint32 family;
   ...
   if ((rc = osapiSemaTake(L7_transferParmSem, L7_WAIT_FOREVER))
    == L7_SUCCESS) {
      ...
      if (!usmDbTransferInProgressGet(1)) {
          ...
          usmDbTransferInProgressSet(1,L7_TRUE);
...
L7_int32 L7_transfer_task()
{
 ...
 #endif
          /* Unmount tmpfs first. */
          system("umount " DOWNLOAD_PATH " >/dev/null 2>&1");
           ...
           if (WEXITSTATUS(system("mount -t tmpfs tmpfs " DOWNLOAD_PATH
             ...
             set_result_string("Make sure /tmp/ directory exists!");
             set_upload_result_code(L7_TRANSFER_CODE_ERROR_STARTING);
             set_download_result_code(L7_TRANSFER_CODE_ERROR_STARTING);
             set_result(L7_TRANSFER_FAILED);
...
*
* @notes    The executables are in STK file format. This
*           function attemmpts to retrive the version information
*           from the STK header.
*
* @end
*********************************************************************/
L7_RC_t osapiImageVersionGet(L7_char8 *image, L7_char8 *version)
{
  stkFileHeader_t stk_header;

  /* Verify that STK file is not corrupt. */

  if ( osapiFsRead (image, (L7_uchar8 *)&stk_header,
        sizeof(stkFileHeader_t)) == L7_ERROR)
  {
    /* We may get here if the STK file is very small or does not exist. */
    return L7_FAILURE;
  }

  if ( (stk_header.tag1 != STK_TAG1) || (stk_header.tag2 != STK_TAG2))
  {
    return L7_FAILURE;
  }

  if ( isalpha(stk_header.rel) != 0 )
    sprintf(version, "%c.%d.%d.%d",stk_header.rel, stk_header.ver,
       stk_header.maint_level, stk_header.build_num);
  else
    sprintf(version, "%d.%d.%d.%d",stk_header.rel, stk_header.ver,
       stk_header.maint_level, stk_header.build_num);
...
* @end
*********************************************************************/
L7_uint32 osapiBootCodeUpdate(char *fileName)
{
...
L7_RC_t osapiKernelUpdate(void)
{
  /* We're only called on one task's thread; use static buffers to save
   ...
   if (WEXITSTATUS(system(buf)) != 0)
   {
      LOG_MSG("Unable to extract Kernel \n");
      /* indicate write failure */
   ...
   if (WEXITSTATUS(system(buf)) != 0)
   {
      LOG_MSG("Unable to extract UPDATE script \n");
      /* indicate write failure */
