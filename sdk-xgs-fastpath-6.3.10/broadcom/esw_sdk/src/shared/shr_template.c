/*
 * $Id: shr_template.c,v 1.20.2.1 Broadcom SDK $
 * 
...
#define TEMPLATE_MSG(string) "%s[%d]%s" string, __FILE__, __LINE__, FUNCTION_NAME()
#define TEMPLATE_MSG1(string) "%s[%d]%s: " string, __FILE__, __LINE__, FUNCTION_NAME()
...
shr_mtemplate_dump(shr_mtemplate_handle_t handle, int template_type)
{
    ...
    if( template_type != -1)
    {
        ...
        {
            soc_cm_print("  templateType not exist (%d..%d)\n", 0, handle->templateTypeCount);
            return BCM_E_PARAM;
        }
    ...
    else
    {
        ...
        soc_cm_print("  template manager (%p)\n", (void*)handle);
    ...
    for (index = start_index; index < last_index; index++) {
        ...
        if (thisTemplate) {
            ...
            soc_cm_print("  template type %d (%s) template pool %d (%s)\n", index, &(thisTemplate->name[0]), thisTemplate->templatePoolId, &(thisPool->name[0]));
            soc_cm_print("  first template index = %d nof templates     = %d\n", thisPool->template_low_id, thisPool->template_count);
            soc_cm_print("  number of elements using this template type = %d\n", thisTemplate->refCount);
            soc_cm_print("  template data size %d \n", (int)(thisPool->data_size));
            ...
            if(template_type != -1) {                
                for (template = thisPool->template_low_id; template < thisPool->template_low_id + thisPool->template_count; template++) {
                    ...
                    if (BCM_E_NONE != result) {
                        soc_cm_print("  error retriving ref counter \n");
                        return BCM_E_FAIL;
                    }
                    ...
                    if (ref_count > 0 && SHR_TEMPLATE_MAX_BUFFER_SIZE_FOR_PRINT >= thisPool->data_size) {
                        ...
                        if (BCM_E_NONE != result) {
                            soc_cm_print("  error geting data for template ID %d\n", template);
                        } else{
                            soc_cm_print("  template id = %d ref counter = %d template data (hex): ", template, ref_count);
                            for (buff_counter = 0; buff_counter < thisPool->data_size; buff_counter++) {
                                soc_cm_print("%02x", buff[buff_counter]);
                            }
                            soc_cm_print("\n\n");
                        }
                    }                    
