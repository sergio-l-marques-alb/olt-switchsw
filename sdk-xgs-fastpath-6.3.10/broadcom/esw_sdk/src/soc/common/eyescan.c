/* $Id: eyescan.c,v 1.45 Broadcom SDK $
 * $Id: 
 *
...
STATIC soc_port_phy_eyescan_counter_cb_t
    counter_func[SOC_MAX_NUM_DEVICES][socPortPhyEyescanNofCounters] = { {{0}
                                                                         }
};

int
soc_port_phy_eyescan_counter_register(int unit,
                                      ...
                                      soc_port_phy_eyescan_counter_cb_t * cf)
{
    ...
    if (counter >= socPortPhyEyescanNofCounters || counter < 0) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                           (_SOC_MSG("Counter %d isn't supported"), counter));
    }
...
STATIC int
soc_port_phy_eyescan_check_bounds(int unit, soc_port_t port, uint32 inst,
                                  int flags,
                                  soc_port_phy_eyscan_counter_t counter,
                                  soc_port_phy_eye_bounds_t * bounds)
{
    ...
    if ((socPortPhyEyescanCounterRelativePhy != counter)
        && bounds->vertical_min < 0) {
        soc_cm_print("Counter mode doesn't support negative vertical_min. "
                     "Updated to 0.\n");
        bounds->vertical_min = 0;
    }
    ...
    if (!(flags & SRD_EYESCAN_FLAG_VERTICAL_ONLY)) {
        _SOC_IF_ERR_EXIT
            (soc_phyctrl_diag_ctrl(unit, port, inst,
                                   PHY_DIAG_CTRL_CMD,
                                   PHY_DIAG_CTRL_EYE_GET_MAX_LEFT_HOFFSET,
                                   (int *)(&l_hmax)));
        _SOC_IF_ERR_EXIT
            (soc_phyctrl_diag_ctrl(unit, port, inst, PHY_DIAG_CTRL_CMD,
    ...
    _SOC_IF_ERR_EXIT(soc_phyctrl_diag_ctrl(unit, port, inst,
                                           ...
                                           (int *)(&vmin)));
    _SOC_IF_ERR_EXIT(soc_phyctrl_diag_ctrl
                     (unit, port, inst, PHY_DIAG_CTRL_CMD,
                      PHY_DIAG_CTRL_EYE_GET_MAX_VOFFSET, (int *)(&vmax)));

    locked = 0;
    MIIM_UNLOCK(unit);

    if (bounds->vertical_min < vmin) {
        soc_cm_print("vertical_min smaller than min available. "
                     "Updated to %d.\n", vmin);
    ...
    if (!(flags & SRD_EYESCAN_FLAG_VERTICAL_ONLY)) {
        if (bounds->horizontal_min < l_hmax) {
            soc_cm_print("horizontal_min smaller than min available. "
                         "Updated to %d.\n", l_hmax);
            bounds->horizontal_min = l_hmax;
        }
        if (bounds->horizontal_max > r_hmax) {
            soc_cm_print("horizontal_max larger than max available. "
                         "Updated to %d.\n", r_hmax);
...
soc_port_phy_eyescan_enable(int unit, uint32 inst, int flags, soc_port_t port,
                            soc_port_phy_eyscan_counter_t counter, int enable,
                            int *voffset)
{
    int data;
    int p1_mapped[] =
        { 0, 1, 3, 2, 4, 5, 7, 6, 8, 9, 11, 10, 12, 13, 15, 14, 16, 17, 19, 18,
          20, 21, 23, 22, 24, 25, 27, 26, 28, 29, 31, 30 };
    int hoffset_init = 0;
    SOC_INIT_FUNC_DEFS;
...
STATIC int
soc_port_phy_eyescan_counter_clear(int unit, soc_port_t port, uint32 inst,
                                   soc_port_phy_eyscan_counter_t counter,
    ...
    if (NULL != counter_func[unit][counter].get_func) {
        ...
        switch (counter) {
            ...
            default:
                _SOC_EXIT_WITH_ERR
                    (SOC_E_PARAM,
                     (_SOC_MSG("Counter isn't supported by the device")));
                break;
        }
    }
...
STATIC int
soc_port_phy_eyescan_start(int unit, soc_port_t port, uint32 inst,
                           soc_port_phy_eyscan_counter_t counter)
{
    SOC_INIT_FUNC_DEFS;
...
STATIC int
soc_port_phy_eyescan_stop(int unit, soc_port_t port, uint32 inst,
                          soc_port_phy_eyscan_counter_t counter,
...
STATIC int
soc_port_phy_eyescan_counter_get(int unit, soc_port_t port, uint32 inst,
                                 soc_port_phy_eyscan_counter_t counter,
                                 uint32 * err_count)
{
    SOC_INIT_FUNC_DEFS;
...
int soc_port_phy_eyescan_run(int unit, uint32 inst,     /* phy instance containing phy number and interface */
                             int flags,
                             ...
                             results /*array of result per port */ )
{
#if 0
    int port = 0, index = 0;
#endif
    ...
    local_ports = (soc_port_t *) sal_alloc(sizeof(soc_port_t) * nof_ports,
                                           "eyecan local ports");
    ...
    if (params->counter >= socPortPhyEyescanNofCounters ||
        params->counter < 0) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                           (_SOC_MSG("Counter %d isn't supported"),
                            params->counter));
    }

    /*allocate & initialize local_lane_num array */
    local_lane_num = (int *)sal_alloc(sizeof(int) * nof_ports,
                                      "eyecan local_lane_num");
    if (NULL == local_lane_num) {
        _SOC_EXIT_WITH_ERR
            (SOC_E_MEMORY,
             (_SOC_MSG("Failed to allocate local_lane_num array")));
    }
    ...
    for (j = 0; j < nof_ports; j++) {
        if (SOC_PBMP_MEMBER(valid_ports_bitmap, ports[j])) {
            if (dev == PHY_DIAG_DEV_EXT) {
                if (lane_num != NULL) {
                    _SOC_IF_ERR_EXIT
                        (soc_phyctrl_redirect_control_get(unit, ports[j], dev, lane_num[j],
            ...
            _SOC_EXIT_WITH_ERR(SOC_E_PORT,
                               (_SOC_MSG("Invalid port %d"), ports[j]));
        }
    }
    ...
    start_time =
        (uint32 *) sal_alloc(sizeof(sal_usecs_t) * nof_ports,
                             "eyecan start time");
    if (NULL == start_time) {
        _SOC_EXIT_WITH_ERR(SOC_E_MEMORY,
                           (_SOC_MSG("Failed to allocate start_time array")));
    }

    /*allocate v_init_offset array */
    v_init_offset = (int *)sal_alloc(sizeof(int) * nof_ports,
                                     "eyecan v_init_offset");
    if (NULL == v_init_offset) {
        _SOC_EXIT_WITH_ERR
            (SOC_E_MEMORY,
             (_SOC_MSG("Failed to allocate v_init_offset array")));
    }
    for (j = 0; j < nof_ports; j++) {
    ...
    for (j = 0; j < nof_ports; j++) {
        ...
        if (local_ports[j] == -1)
            continue;
        _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_enable
                         (unit, inst, flags, local_ports[j], params->counter, 1,
                          &(v_init_offset[j])));
    }
    ...
    for (j = 0; j < nof_ports; j++) {
        ...
        if (local_ports[j] == -1)
            continue;
        _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_check_bounds
                         (unit, local_ports[j], inst, flags, params->counter,
                          &(params->bounds)));
    }
    ...
    /*scan */
    for (h = params->bounds.horizontal_min; h <= params->bounds.horizontal_max;
         h++) {
        ...
        for (v = params->bounds.vertical_max; v >= params->bounds.vertical_min;
             v--) {
            ...
            for (j = 0; j < nof_ports; j++) {
                ...
                results[j].run_time[h + SOC_PORT_PHY_EYESCAN_H_INDEX][v +
                                                                      SOC_PORT_PHY_EYESCAN_V_INDEX]
                    = 0;
                results[j].error_count[h + SOC_PORT_PHY_EYESCAN_H_INDEX][v +
                                                                         SOC_PORT_PHY_EYESCAN_V_INDEX]
                    = 0;
            }

            while ((under_threshold >= params->nof_threshold_links)
                ...
                if (first_run) {
                    ...
                    if ((desired_time >= params->time_upper_bound)
                        || (total_time >= params->time_upper_bound)) {
                    ...
                    if (params->sample_time > params->time_upper_bound) {
                        params->sample_time =
                            params->time_upper_bound - total_time;
                    }
                    total_time += params->sample_time;
                }
                ...
                for (j = 0; (j < nof_ports); j++) {
                    ...
                    if (first_run) {
                        ...
                        if (!(flags & SRD_EYESCAN_FLAG_VERTICAL_ONLY)) {
                            _SOC_IF_ERR_EXIT(soc_phyctrl_diag_ctrl
                                             (unit, local_ports[j], inst,
                                              PHY_DIAG_CTRL_CMD,
                                              PHY_DIAG_CTRL_EYE_SET_HOFFSET,
                                              (int *)(&h)));
                        }
                        _SOC_IF_ERR_EXIT
                            (soc_phyctrl_diag_ctrl(unit, local_ports[j], inst,
                                                   PHY_DIAG_CTRL_CMD,
                                                   PHY_DIAG_CTRL_EYE_SET_VOFFSET,
                                                   (int *)(&v)));     /*start measure */
                        _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_start
                                         (unit, local_ports[j], inst,
                                          params->counter));

                        if ((flags & SRD_EYESCAN_FLAG_DSC_PRINT)) {
                            soc_phyctrl_control_set(unit, local_ports[j],
                                                    SOC_PHY_CONTROL_DUMP, 1);
                        }
                    }

                    /*clear counters */
                    _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_counter_clear
                                     (unit, local_ports[j], inst,
                                      params->counter, &(start_time[j])));
                ...
                for (j = 0; j < nof_ports; j++) {
                    ...
                    /*stop measure */
                    _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_stop
                                     (unit, local_ports[j], inst,
                                      params->counter, &end_time));
                    _SOC_IF_ERR_EXIT(soc_port_phy_eyescan_counter_get
                                     (unit, local_ports[j], inst,
                                      params->counter, &err_count));

                    time_before = results[j].run_time[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                                     [v + SOC_PORT_PHY_EYESCAN_V_INDEX];

                    if (end_time > start_time[j]) {
                        results[j].run_time[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                           [v + SOC_PORT_PHY_EYESCAN_V_INDEX]
                            += (end_time - start_time[j]) / 1000;
                    } else {
                        results[j].run_time[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                           [v + SOC_PORT_PHY_EYESCAN_V_INDEX]
                            +=
                            ((SAL_USECS_TIMESTAMP_ROLLOVER - start_time[j]) +
                             end_time) / 1000;
                    }
                    results[j].error_count[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                          [v + SOC_PORT_PHY_EYESCAN_V_INDEX]
                        += (err_count);

                    if (results[j].
                        error_count[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                   [v + SOC_PORT_PHY_EYESCAN_V_INDEX]
                    ...
                    if (flags & SRD_EYESCAN_FLAG_DSC_PRINT) {
                        soc_cm_print("Added %d errors in %d miliseconds "
                                     "for port %d\n",
                                      err_count,
                                      (results[j].
                                       run_time[h + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                               [v + SOC_PORT_PHY_EYESCAN_V_INDEX]
                                       - time_before), local_ports[j]);
                    }
#if 0
                    port = local_ports[j];
                    index = j;
            ...
            if ((params->counter != socPortPhyEyescanCounterRelativePhy)
                && (desired_time < params->time_upper_bound)) {
    ...
    for (j = 0; j < nof_ports; j++) {
        ...
        /*error results */
        for (h = params->bounds.horizontal_min;
             h <= params->bounds.horizontal_max; h++) {
            if (h % params->sample_resolution != 0) {
                if (!(flags & SRD_EYESCAN_FLAG_VERTICAL_ONLY)) {
                    continue;
                }
            }
            for (v = params->bounds.vertical_min;
                 v <= params->bounds.vertical_max; v++) {
                if (v % params->sample_resolution != 0) {
                    continue;
                }
    ...
    if (v_init_offset) {
        ...
        for (j = 0; j < nof_ports; j++) {
            ...
            if (local_ports != NULL) {
                ...
                soc_port_phy_eyescan_enable(unit, inst, flags, local_ports[j],
                                            params->counter, 0,
                                            &(v_init_offset[j]));
            }
        }
                               ...
                               soc_port_phy_eyescan_results_t * res)
{
    ...
    pc = EXT_PHY_SW_STATE(unit, port); /* pc = NULL if no external phy*/
    ...
    if (hz_max < hz_min) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                           (_SOC_MSG
                            ("bounds->horizontal_max < bounds->horizontal_min")));
    }
    if (vt_max < vt_min) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                           (_SOC_MSG
                            ("bounds->vertical_max < bounds->vertical_min")));
    }
    ...
    if (hz_max != hz_min) {
        ...
        for (vt_i = vt_max; vt_i >= vt_min; vt_i--) {
            ...
            for (hz_i = hz_min; hz_i <= hz_max; hz_i++) {
                ...
                error_cnt = res->error_count[hz_i + SOC_PORT_PHY_EYESCAN_H_INDEX]
                        [vt_i + SOC_PORT_PHY_EYESCAN_V_INDEX];
                time_tot = res->run_time[hz_i + SOC_PORT_PHY_EYESCAN_H_INDEX]
                                        [vt_i + SOC_PORT_PHY_EYESCAN_V_INDEX];
            ...
            if (pc && pc->phy_id0 == 0x600d && pc->phy_id1 == 0x8500) {
                ...
                if (desc_idx < desc_low_ber_size + desc_size) {
                    ...
                    if (desc_idx < desc_size) {
                        ...
                        soc_cm_print(desc_low_ber[desc_idx - desc_size],
                ...
                if (desc_idx < desc_ber_size + desc_size) {
                    ...
                    if (desc_idx < desc_size) {
                        ...
                        soc_cm_print(desc_ber[desc_idx - desc_size],
...
_eyescan_util_round_real(COMPILER_DOUBLE original_value, int decimal_places)
{
    ...
    COMPILER_DOUBLE shift_digits[EYESCAN_UTIL_MAX_ROUND_DIGITS + 1] =
    ...
    if (decimal_places > EYESCAN_UTIL_MAX_ROUND_DIGITS) {
        soc_cm_print
            ("ERROR: Maximum digits to the right of decimal for rounding "
             "exceeded. Max %d, requested %d\n", EYESCAN_UTIL_MAX_ROUND_DIGITS,
             decimal_places);
        return 0.0;
    }
    ...
    /* convert to integer and back to COMPILER_DOUBLE 
     * to truncate undesired precision 
     */
...
#ifdef COMPILER_HAS_DOUBLE
STATIC int
_eye_margin_diagram_cal(int unit, EYE_DIAG_INFOt * pInfo,
                        soc_port_phy_eyescan_results_t * results)
{
    ...
    /* Find the highest data point to use, 
     * currently based on at least 1E-8 BER level
     */
    ...
    for (loop_index = 0; loop_index < pInfo->veye_cnt; loop_index++) {
        if (pInfo->total_errs[loop_index] == 0) {
            bers[loop_index] =
                (1000.0 /
                 (COMPILER_DOUBLE) pInfo->total_elapsed_time[loop_index]) /
                pInfo->rate;
        } else {
            bers[loop_index] =
                (COMPILER_DOUBLE) (1000.0 *
                                   (COMPILER_DOUBLE) pInfo->
                                   total_errs[loop_index]) /
                (COMPILER_DOUBLE) pInfo->total_elapsed_time[loop_index] /
                pInfo->rate;
        }
    ...
    if (start_n >= pInfo->veye_cnt) {
        outputs[0] =
            -_eyescan_util_round_real(log(bers[pInfo->veye_cnt - 1]) / log(10),
                                      1);
        outputs[1] = -100.0;
        outputs[2] = -100.0;
        outputs[3] = -100.0;
        ...
        for (loop_index = start_n; loop_index < stop_n; loop_index++) {
            SOC_DEBUG(SOC_DBG_VERBOSE,
                      ("\ttotal_errs[%d]=0x%08x\n", loop_index,
                       (int)pInfo->total_errs[loop_index]));
            SOC_DEBUG(SOC_DBG_VERBOSE,
        ...
        for (loop_index = start_n; loop_index < stop_n; loop_index++) {
            if (pInfo->mono_flags[loop_index] == 0) {
                Exy =
                    Exy +
                    margins[loop_index] * lbers[loop_index] /
                    (COMPILER_DOUBLE) delta_n;
                Eyy =
                    Eyy +
                    lbers[loop_index] * lbers[loop_index] /
                    (COMPILER_DOUBLE) delta_n;
                Exx =
                    Exx +
                    margins[loop_index] * margins[loop_index] /
                    (COMPILER_DOUBLE) delta_n;
        ...
        if (beta > 9.32) {
            beta_max = 1;
            soc_cm_print("\n\tWARNING: intermediate float variable "
                         "is maxed out, what this means is:\n");
            soc_cm_print("\t\t- The *extrapolated* minimum BER will be "
                         "reported as 1E-37.\n");
            soc_cm_print("\t\t- This may occur if the channel is near ideal "
                         "(e.g. test loopback)\n");
            soc_cm_print("\t\t- While not discrete, reporting an extrapolated "
        ...
        SOC_DEBUG(SOC_DBG_VERBOSE,
                  ("\t\tlog1e-12=%f, sq=%f\n", (COMPILER_DOUBLE) log(1e-12),
                   (COMPILER_DOUBLE) sqrt(-log(1e-12))));
        ...
        if (beta_max) {
            ...
            results->ext_better = 0;
            results->ext_results_int = (int)outputs[0];
            results->ext_results_remainder =
                    (((int)(outputs[0] * 100)) % 100);
        }
    }
 exit:
    SOC_FUNC_RETURN;
...
#ifdef COMPILER_HAS_DOUBLE
STATIC int _eye_margin_ber_cal(int unit, EYE_DIAG_INFOt * pInfo)
{
    COMPILER_DOUBLE bers[MAX_EYE_LOOPS];        /* computed bit error rate */
#if defined(BROADCOM_DEBUG)
    COMPILER_DOUBLE margins[MAX_EYE_LOOPS];     /* Eye margin @ each measurement */
    ...
    for (loop_var = 0; loop_var < pInfo->veye_cnt; loop_var++) {
        ...
        if (pInfo->total_errs[loop_var] == 0) {
            bers[loop_var] =
                1000.0 / (COMPILER_DOUBLE) pInfo->total_elapsed_time[loop_var] /
                pInfo->rate;
            bers[loop_var] /= 1000;

            SOC_DEBUG(SOC_DBG_VERBOSE,
                      ("BER @ %04f %% = 1e%04f (%d errors in %d miliseconds)\n",
                       (COMPILER_DOUBLE) ((pInfo->offset_max -
                                           loop_var) * eye_unit),
            ...
            bers[loop_var] =
                ((COMPILER_DOUBLE) 1000.0 *
                 ((COMPILER_DOUBLE) (pInfo->total_errs[loop_var]))) /
                (COMPILER_DOUBLE) pInfo->total_elapsed_time[loop_var] /
                pInfo->rate;
            ...
            SOC_DEBUG(SOC_DBG_VERBOSE,
                      ("BER @ %2.2f%% = 1e%2.2f (%d errors in %d miliseconds)\n",
                       ...
                       log10(bers[loop_var]), pInfo->total_errs[loop_var],
                       pInfo->total_elapsed_time[loop_var]));
        }
                                 ...
                                 soc_port_phy_eyescan_results_t * results)
{
    ...
    /* extrapolate */
    if (!(flags & SRD_EYESCAN_FLAG_AVOID_EXTRAPOLATION)
        && (flags & SRD_EYESCAN_FLAG_VERTICAL_ONLY)
        && (params->counter != socPortPhyEyescanCounterRelativePhy)) {

        for (i = 0; i < nof_ports; i++) {
            rv = soc_phyctrl_speed_get(unit, ports[i], &speed);
            if (rv != SOC_E_NONE) {
                _SOC_EXIT_WITH_ERR(rv,
                                   (_SOC_MSG("soc_phyctrl_speed_get failed")));
            }
            ...
            cnt = 0;
            for (vertical_i = params->bounds.vertical_min;
                 vertical_i <= params->bounds.vertical_max; vertical_i++) {
            ...
            for (vertical_i = params->bounds.vertical_min;
                ...
                veye_info.total_errs[cnt] =
                    results[i].
                    error_count[SOC_PORT_PHY_EYESCAN_H_INDEX]
                               [vertical_i + SOC_PORT_PHY_EYESCAN_V_INDEX];
                veye_info.total_elapsed_time[cnt] = 
                    results[i].
                    run_time[SOC_PORT_PHY_EYESCAN_H_INDEX]
                            [vertical_i + SOC_PORT_PHY_EYESCAN_V_INDEX];
