/*
 * $Id: intr_cmicm.c,v 1.107.16.1 Broadcom SDK $
...
soc_cmicm_intr_sbusdma_done(int unit, uint32 ch)
{
	...
	if (SOC_IS_CALADAN3(unit)) {
		...
		soc_cm_debug(DK_INTR, "Received unallocated sbusdma interrupt cmc %d ch %d !!\n",
			     cmc, ch);
	    } else {
		switch (op) {
...
soc_cmicm_chip_func_intr(int unit, uint32 val)
{
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        ...
        if (irqStat & _SOC_TD2_FUNC_INTR_MASK) {
            ...
            /* dispatch interrupt */
            soc_cm_debug(DK_INTR, "soc_cmicm_intr type 1 unit %d: dispatch\n",
                         unit);
...
soc_cmicm_rcpu_intr(int unit, soc_rcpu_intr_packet_t *intr_pkt)
{
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            /* dispatch interrupt */
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                soc_cm_print("soc_cmicm_rcpu_intr type 0 unit %d: dispatch %s\n",
                             unit, intr_handler[i].intr_name);
            }
...
check_rcpu_type1:
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                soc_cm_print("soc_cmicm_rcpu_intr type 1 unit %d: dispatch %s\n",
                              unit, intr_handler[i].intr_name);
            }
        }
    }
...
check_rcpu_type2:
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            /* dispatch interrupt, verbose only */
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                soc_cm_print("soc_cmicm_rcpu_intr type 2 unit %d: dispatch %s\n",
                             unit, intr_handler[i].intr_name);
            }
        }
    }
...
check_rcpu_type3:
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            /* dispatch interrupt, verbose only */
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                soc_cm_print("soc_cmicm_rcpu_intr type 3 unit %d: dispatch %s\n",
                             unit, intr_handler[i].intr_name);
            }
        }
    }
...
check_rcpu_type4:
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            /* dispatch interrupt, verbose only */
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                soc_cm_print("soc_cmicm_rcpu_intr type 4 unit %d: dispatch %s\n",
                             unit, intr_handler[i].intr_name);
            }
        }
    }
...
check_rcpu_cmc0:
    ...
    for (; intr_handler[i].mask; i++) {
        if (irqStat & intr_handler[i].mask) {
            ...
            if (SOC_DEBUG_CHECK(SOC_DBG_RCPU | SOC_DBG_INTR)) {
                /* dispatch interrupt, verbose only */
                soc_cm_print("soc_cmicm_rcpu_cmc0_intr type 0 unit %d: dispatch %s\n",
                             unit, intr_handler[i].intr_name);
            }
        }
    }
