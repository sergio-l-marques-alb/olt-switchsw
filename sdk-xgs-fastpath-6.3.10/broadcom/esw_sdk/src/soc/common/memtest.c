/*
 * $Id: memtest.c,v 1.45.2.1 Broadcom SDK $
...
ser_test_mem_write(int unit, ser_test_data_t *test_data)
{
    ...
    if (SOC_FAILURE(rv)) {
        soc_cm_debug(DK_ERR, "unit %d %s entry %d mem write error\n",
                ...
                _soc_ser_test_t test_type, int * error_count)
{
    ...
    if (!(SOC_MEM_INFO(unit,test_data->mem).flags & SOC_MEM_SER_FLAGS)) {
        /*Verbose output: DK_VERBOSE or DK_TEST Skipping memory: name*/
        return SOC_E_NONE;
    }
    /*Check that there is a meminfo structure for this memory.*/
    if (!SOC_MEM_IS_VALID(unit, test_data->mem)) {
        soc_cm_print("%s is not a valid memory for this platform. Skipping.\n",
                     test_data->mem_name);
        return SOC_E_MEMORY;
    }
    switch (test_type) {
    ...
    for (j = 0; j < numIndexesToTest; j++) {
        ...
        soc_cm_print("Injection Error for mem: %s field: %s\n",
                     test_data->mem_name,
                     SOC_FIELD_NAME(unit,test_data->test_field));
        (*error_count)++;
        continue;
        ...
        if ((SOC_MEM_INFO(unit, test_data->mem).flags & SOC_MEM_SER_FLAGS) ==
            ...
            if (0 != test_data->field_buf[0]) {
                (*error_count)++;
                soc_cm_debug(DK_ERR, "SER failed to clear mem %s index %d\n",
                            test_data->mem_name, test_data->index);
            }
        }else if (test_data->badData == test_data->field_buf[0]) {
            (*error_count)++;
            soc_cm_debug(DK_ERR, "SER failed to correct mem %s index %d field %s\n",
                        test_data->mem_name, test_data->index,
                        SOC_FIELD_NAME(unit, test_data->test_field));
        }
        else {
            soc_cm_debug(DK_VERBOSE, "SER corrected mem %s index %d\n",
                                test_data->mem_name, test_data->index);
        }
    }
    if (startErrorCount != *error_count) {
        soc_cm_print("SER failed to correct memory %s acc_type: %d field: %s %d:%d\n",
...
ser_test_cmd_generate(int unit, ser_test_data_t *test_data)
{
    ...
    if (test_data != NULL && test_data->mem_name != NULL &&
        ...
        if (ypipe) {
            soc_cm_print("s EGR_SBS_CONTROL PIPE_SELECT=1\n");
            soc_cm_print("s SBS_CONTROL PIPE_SELECT=1\n");
        }
            soc_cm_print("wr %s%s 1 1 %s=0\n",
                         pipe_str, test_data->mem_name, test_data->field_name);
            soc_cm_print("d %s%s 1 1\n", pipe_str, test_data->mem_name);
            if (test_data->tcam_parity_bit != -1) {
                soc_cm_print("s %s %s=0\n",
            ...
            soc_cm_print("wr nocache %s%s 1 1 %s=1\n",
                         pipe_str, test_data->mem_name, test_data->field_name);
            if (test_data->tcam_parity_bit != -1) {
                soc_cm_print("s %s %s=1\n",
                             SOC_REG_NAME(unit, test_data->parity_enable_reg),
                             SOC_FIELD_NAME(unit,
                                            test_data->parity_enable_field));
            } else {
            ...
            soc_cm_print("d %s%s 1 1\n", pipe_str, test_data->mem_name);
        if (ypipe) {
            soc_cm_print("s EGR_SBS_CONTROL PIPE_SELECT=0\n");
            soc_cm_print("s SBS_CONTROL PIPE_SELECT=0\n");
        }

    } else {
        soc_cm_print("ERROR: Attempted to print a cmd from missing data.\n");
    }
}
                      ...
                      int(*pipe_select)(int,int,int) )
{
    ...
    for (i = 0; overlays[i].mem != INVALIDm; i++) {
        ...
        if (mem_failed != last_failed) {
            soc_cm_print("%s failed overlay test.\n",
                         SOC_MEM_NAME(unit,test_data.mem));
        }
    }
    return mem_failed;
