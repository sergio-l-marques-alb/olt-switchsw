...
#define DBG_G_OUT(flags, stuff) SOC_DEBUG(SOC_DBG_GE | flags, stuff)
#define DBG_G_VERB(stuff) DBG_G_OUT(SOC_DBG_VERBOSE, stuff)
#define DBG_G_WARN(stuff) DBG_G_OUT(SOC_DBG_WARN, stuff)
...
soc_mac_probe(int unit, soc_port_t port, mac_driver_t **macdp)
{
	...
	if (SOC_IS_HURRICANE2(unit) && IS_XL_PORT(unit,port)) {
		*macdp = &soc_mac_xl;
	} else
#endif
#ifdef BCM_UNIMAC_SUPPORT
...
#endif
        {
            if (SOC_IS_TRIUMPH3(unit) || SOC_IS_CALADAN3(unit)) { /* For all GE's */
                *macdp = &soc_mac_x;
            } else {
                *macdp = &soc_mac_ge;
            }
                   ...
                   int value)
{
    ...
    DBG_G_VERB(("mac_ge_control_set: unit %d port %s type=%d value=%d\n",
                   ...
                   int *value)
{
    ...
    DBG_G_VERB(("mac_ge_control_get: unit %d port %s type=%d value=%d\n",
                unit, SOC_PORT_NAME(unit, port),
                type, *value));
    return SOC_E_NONE;
}            
