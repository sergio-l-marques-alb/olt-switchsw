/*
 * $Id: l2xmsg.c,v 1.37.106.2 Broadcom SDK $
                        ...
                        uint32 flags)
{
    ...
    soc_cm_debug(DK_ARL, "soc_tr3_l2x_sync_delete: unit=%d index=%d\n",
                 unit, index);
...
soc_tr3_l2x_stop(int unit)
{
    ...
    soc_cm_debug(DK_ARL, "soc_tr3_l2x_stop: unit=%d\n", unit);
    ...
    if (soc->l2x_pid != SAL_THREAD_ERROR) {
        ...
        while (soc->l2x_pid != SAL_THREAD_ERROR) {
            if (soc_timeout_check(&to)) {
                soc_cm_debug(DK_ERR, "soc_tr3_l2x_stop: thread will not exit\n");
                rv = SOC_E_INTERNAL;
                break;
            }
        }
...
_soc_tr3_l2x_thread(void *unit_vp)
{
    ...
    if (index_count_int <= 0) {
        soc_cm_debug(DK_VERBOSE, "soc_tr3_l2x_thread: Internal L2 table size is 0 \n");
...
skip_l2_int:
    ...
    if (index_count_ext1 <= 0) {
        soc_cm_debug(DK_VERBOSE, "soc_tr3_l2x_thread: External L2_1 table size is 0 \n");
...
skip_l2_ext1:
    ...
    if (index_count_ext2 <= 0) {
        soc_cm_debug(DK_VERBOSE, "soc_tr3_l2x_thread: External L2_2 table size is 0 \n");
...
skip_l2_ext2:
    ...
    while ((interval = soc->l2x_interval) != 0) {
        ...
        if (SOC_L2_DEL_SYNC_LOCK(soc) < 0) {
            soc_cm_debug(DK_ERR, "AbnormalThreadExit:soc_l2x_thread: unable to take mutex\n");
        ...
        if ((rv = soc_mem_read_range(unit, L2_ENTRY_1m,
            ...
            SOC_MEM_L2_INT_UNLOCK(unit);
            soc_cm_debug(DK_ERR,"AbnormalThreadExit:soc_l2x_thread: DMA failed: %s\n",
                         soc_errmsg(rv));
...
soc_tr3_l2mod_fifo_process(int unit, uint32 flags, l2_mod_fifo_entry_t *entry)
{
    ...
    soc_cm_debug(DK_ARL | DK_VERBOSE, "Processing L2 MOD FIFO message..\n");
    ...
    if (soc_L2_MOD_FIFOm_field32_get(unit, entry, EXTERNAL_L2_ENTRYf)) {
        if (soc_feature(unit, soc_feature_esm_support)) {
            if (soc_L2_MOD_FIFOm_field32_get(unit, entry, ENTRY_WIDTHf)) {
                ...
                soc_cm_debug(DK_ARL, "External L2 entry type 2.\n");
            } else {
                mem_type = EXT_L2_ENTRY_1m;
                soc_L2_MOD_FIFOm_field_get(unit, entry, L2_ENTRYf,
                                           l2_entry.ext_l2_entry_1.entry_data);

                soc_cm_debug(DK_ARL, "External L2 entry type 1.\n");
            }            
        } else {
            soc_cm_debug(DK_ERR, "Unexpected external L2 entry data in L2 MOD FIFO !!\n");
        }
    } else {
        if (soc_L2_MOD_FIFOm_field32_get(unit, entry, ENTRY_WIDTHf)) {
            soc_L2_MOD_FIFOm_field_get(unit, entry, L2_ENTRYf,
            ...
            mem_type = L2_ENTRY_2m;
            soc_cm_debug(DK_ARL, "Internal L2 entry type 2.\n");
        } else {
            soc_L2_MOD_FIFOm_field_get(unit, entry, L2_ENTRYf,
                                       l2_entry.l2_entry_1.entry_data);
            mem_type = L2_ENTRY_1m;
            soc_cm_debug(DK_ARL, "Internal L2 entry type 1.\n");
        }
    }
    ...
    if (mem_type == INVALIDm) {
        soc_cm_debug(DK_ERR, "Unable to determine L2 mem type !!\n");
        return;
    }
...
soc_tr3_l2_overflow_interrupt_handler(int unit)
{
    ...
    if (!SOC_CONTROL(unit)->l2_overflow_active) {
        soc_cm_debug(DK_ERR, "Received L2 overflow event with no app handler or"\
                     " processing inactive !!\n");
    }
...
_soc_tr3_l2e_ppa_update(int unit, soc_mem_t mem, int index, uint32 *entry)
{
    ...
    soc_cm_debug(DK_VERBOSE, "tr3_l2e_ppa %d: index=%d oldvlan=%d min:max %d:%d\n",
                 mem == EXT_L2_ENTRY_1m ? 1 : 2,
                 index, oldvlan, ppa_vlan->vlan_min[oldvlan],
                 ppa_vlan->vlan_max[oldvlan]);
    soc_cm_debug(DK_VERBOSE, "tr3_l2e_ppa %d: ent=0x%x mac=%x:%x:%x:%x:%x:%x "
                 "vlan=%d min:max %d:%d\n",
                 mem == EXT_L2_ENTRY_1m ? 1 : 2,
                 ppa_info[index].data,
