...
#include <sal/core/alloc.h>
	#define TDM_VERBOSE(a) SOC_DEBUG(SOC_DBG_VERBOSE | SOC_DBG_TDM, a)
	#define TDM_ERROR3(a,b,c,d) soc_cm_debug(DK_ERR, a, b, c, d)
	#define TDM_ERROR2(a,b,c) soc_cm_debug(DK_ERR, a, b, c)
	#define TDM_ERROR1(a,b) soc_cm_debug(DK_ERR, a, b)
	#define TDM_ERROR0(a) soc_cm_debug(DK_ERR, a)
	#define TDM_WARN3(a,b,c,d) soc_cm_debug(DK_WARN, a,b,c,d)
	#define TDM_WARN2(a,b,c) soc_cm_debug(DK_WARN, a,b,c)
	#define TDM_WARN1(a,b) soc_cm_debug(DK_WARN, a,b)
	#define TDM_WARN0(a) soc_cm_debug(DK_WARN, a)
...
void parse_mmu_tdm_tbl(int bw, int mgmt_bw, int mmu_tdm_tbl[256], int mmu_tdm_ovs_1[16], int mmu_tdm_ovs_2[16], int mmu_tdm_ovs_3[16], int mmu_tdm_ovs_4[16], int pipe)
{
   ...
   for (j=0; j<16; j++) {if (mmu_tdm_ovs_1[j]!=NUM_EXT_PORTS || mmu_tdm_ovs_2[j]!=NUM_EXT_PORTS || mmu_tdm_ovs_3[j]!=NUM_EXT_PORTS || mmu_tdm_ovs_4[j]!=NUM_EXT_PORTS) {oversub=TRUE;}}
	 ...
	 if (mmu_tdm_tbl[j]!=ACC_TOKEN) {TDM_VERBOSE3("TDM: PIPE: %s, MMU TDM TABLE, element #%0d, contains physical port #%0d\n", name, j, mmu_tdm_tbl[j]);}
		...
		TDM_VERBOSE2("TDM: PIPE: %s, MMU TDM TABLE, element #%0d, CONTAINS ACCESSORY TOKEN - assigned as ", name, j);
...
int TDM_scheduler(int wc_array[NUM_WC][4], enum port_speed speed[NUM_EXT_PORTS], int *cur_idx, int *pgw_tdm_idx, int *ovs_tdm_idx, int subp, int pgw_tdm_tbl[32], int ovs_tdm_tbl[32], int upperlimit, int iter1, int bw, int port_state_map[128], int iter3, int stop1, int swap_array[32], int *z, int subport, int op_flags[1])
{
	...
	if (num_ovs > 0)
	{
		...
		if ((l1_ovs_cnt==0) && (op_flags[0]==0)) TDM_ERROR0("TDM: _____ERROR: unable to partition oversubscription tokens\n");
		else TDM_VERBOSE1("TDM: _____VERBOSE: %0d oversub tokens assigned\n", l1_ovs_cnt);
	}
	...
	if (microsub==TRUE)
	{
		...
		{
			TDM_VERBOSE0("TDM: _____VERBOSE: shunting all 1G into oversub table\n");
			op_flags[0]=1;
		...
		if (op_flags[0]==1)
		{
			...
			TDM_VERBOSE1("TDM: _____VERBOSE: oversub tokens increased to %0d\n", l1_ovs_cnt);
			while (sm_iter < NUM_EXT_PORTS)
			{
				...
				TDM_VERBOSE0("TDM: ----------------------------------------------------------------\n");
				TDM_VERBOSE2("TDM: _____DEBUG: the present port is %0d, speed %0d\n", wc_array[*cur_idx][subp],speed[wc_array[*cur_idx][subp]]);
		...
		{
			TDM_VERBOSE0("TDM: _____VERBOSE: using per quadrant granularity reduction\n");
			...
			TDM_VERBOSE0("TDM: _____VERBOSE: scheduling all 1G in line rate as 1G granularity slots\n");
			...
			TDM_ERROR0("TDM: __________FATAL ERROR: cannot schedule 1G ports or unsupported configuration\n");
			return 0;
		}
	...
	else if ((num_lr_100 + num_lr_120 == 1) && (num_lr_10)+((num_lr_20)*2)+((num_lr_40)*4) < ((num_lr_100)*10)+((num_lr_120)*12))
	{
		...
		while (sm_iter < NUM_EXT_PORTS)
		{
			...
			TDM_VERBOSE0("TDM: ----------------------------------------------------------------\n");
			TDM_VERBOSE2("TDM: _____VERBOSE: the present port is %0d, speed %0d\n", wc_array[*cur_idx][subp], speed[wc_array[*cur_idx][subp]]);
...
int write_mmu_tdm_tbl(int pgw0[32], int pgw1[32], int ovs0[32], int ovs1[32], int mmu[256], int bucket1[16], int bucket2[16], int bucket3[16], int bucket4[16], int x_pipe, int port_state_map[128], int tsc[NUM_WC][4], int bw, enum port_speed speed[NUM_EXT_PORTS], int (*op_flags)[1])
{
	...
	for (j=0; j<16; j++)
	{
		bucket1[j]=NUM_EXT_PORTS;
		bucket2[j]=NUM_EXT_PORTS;
		bucket3[j]=NUM_EXT_PORTS;
		bucket4[j]=NUM_EXT_PORTS;
	}
	...
	*/
	...
	TDM_VERBOSE7("TDM: _____VERBOSE: OVS1-%0d, OVS10-%0d, OVS20-%0d, OVS40-%0d, OVS100-%0d, OVS120-%0d, OVS Variety - %0d\n", num_ovs_1, num_ovs_10, num_ovs_20, num_ovs_40, num_ovs_100, num_ovs_120, (ovs_1 + ovs_10 + ovs_20 + ovs_40 + ovs_100 + ovs_120));
