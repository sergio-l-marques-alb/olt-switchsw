/*
 * $Id: max664x.c,v 1.8 Broadcom SDK $
...
max664x_ioctl(int unit, int devno, int opcode, void* data, int len)
{
    ...
    if (opcode == MAX664X_OP_READ) {
        ...
        soc_cm_print("invalid command for max6649 - must be read or write\n");
        rv = SOC_E_UNAVAIL;
    }
...
max664x_temp_show(int unit, int dev, int force)
{
	...
	soc_cm_print("%s: ERROR: device not responding\n",
    ...
    for (cnt=0; cnt<2; cnt++) {
        ...
        if (force || difftemp >= MINDIFF) {
            ...
            if (difftemp > 20) {
                if (force) {
                    soc_cm_print("%s: NOTICE: temperature unavailable\n",
                                 soc_i2c_devname(unit, dev));
                    if (lasttemp > 0) {
                        ftemp = C2F(lasttemp);
                        soc_cm_print("%s: last temperature %d.%dC, %d.%dF\n",
                                     soc_i2c_devname(unit, dev),
            ...
            if (cnt==0) {
                soc_cm_print("%s: local temperature %dC, %dF\n",
                             soc_i2c_devname(unit, dev),
                             ctemp,
                             ftemp);
            } else {

                soc_cm_print("%s: sirius temperature %dC, %dF\n",
                             soc_i2c_devname(unit, dev),
                             ctemp,
                             ftemp);
            }
...
max664x_thread(void *unitp)
{
    ...
    soc_cm_print("unit %d: thermal monitoring completed\n", unit);
    sal_thread_exit(0);
}
	  ...
	  void* data, int len)
{
    ...
    if (rv < 0) {
        soc_cm_print("%s: ERROR: device did not initialize: %s\n",
                     soc_i2c_devname(unit, devno),
                     soc_errmsg(rv));
    }
    soc_cm_print("status=0x%x\n", maxreg.status);
...
soc_i2c_max664x_monitor(int unit, int enable, int nsecs)
{
    ...
    sal_thread_create("bcmTEMP",
                      ...
                      INT_TO_PTR(unit));
    soc_cm_print("unit %d: thermal monitoring enabled\n", unit);
}
