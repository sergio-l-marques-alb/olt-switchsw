/*
 * $Id: max127.c,v 1.6.562.3 Broadcom SDK $
...
#define MAX127_12BIT_CPU(x) ((x)  >> 4); 
#define MAX127_12BIT_DUT(x) ((((x) << 8) | ((x) >> 8)) >> 4);
#else
...
max127_read_channel(int unit, int devno, int ch, max127_t *data)
{
    ...
    if (soc_caladan3_is_board_rev_a()) {
        val = MAX127_12BIT_DUT(val);
    } else {
        val = MAX127_12BIT_CPU(val);
    }
#else
...
max127_ioctl(int unit, int devno, int cmd, void *data, int len)
{
    ...
    case I2C_ADC_SET_SAMPLES:
        ...
        break;
    default:
        soc_cm_print("Unit:%d Dev:%d Unsupported IOCTL Cmd %x\n", unit, devno, cmd);
        return SOC_E_PARAM;
    }
    return SOC_E_NONE;
