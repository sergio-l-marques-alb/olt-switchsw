/*----------------------------------------------------------------------
 * $Id: tscmod.c,v 1.160.2.8 Broadcom SDK $
 ...
 * Purpose:
 *      Set the current advertise1ment for auto-negotiation.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
...
STATIC int
_phy_tscmod_proxy_alive_get(int unit, soc_port_t port, uint32 *alive)
{
    phy_ctrl_t  *pc;
    tscmod_st   *tsc;
    TSCMOD_DEV_DESC_t *pDesc;
    ...
    if(tsc->ctrl_type&TSCMOD_CTRL_TYPE_UC_STALL) {
        *alive = 0 ;
    } else {
        tsc->diag_type = TSCMOD_DIAG_ANEG;
        tsc->per_lane_control = 1 << TSCMOD_DIAG_AN_MODE ;
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    */
    ...
    case SOC_PHY_CONTROL_RX_PEAK_FILTER_TEMP_COMP:
       ...
       rv |= SOC_E_NONE;
       break ;
    case SOC_PHY_CONTROL_MICRO_ALIVE:
        rv = SOC_E_NONE;
        break ;  
    default:
        rv = SOC_E_UNAVAIL;
   ...
   /* preventive care */
    ...
    case SOC_PHY_CONTROL_RX_PEAK_FILTER_TEMP_COMP:
        ...
        *value = (fw_mode == TSCMOD_UC_CTRL_OPT_PF_TEMP_COMP);
        break ;
    case SOC_PHY_CONTROL_MICRO_ALIVE:
        rv = _phy_tscmod_proxy_alive_get(unit, port, value) ;
        break ;
    default:
        rv = SOC_E_UNAVAIL;
        break;
...
_phy_tscmod_cfg_dump(int unit, soc_port_t port)
{
    ...
    soc_cm_print("force_cl72_st   0x%04x en=%0d md=%0d\n", 
                 FORCE_CL72_STATE(pc), FORCE_CL72_ENABLED(pc), FORCE_CL72_MODE(pc)) ;
    soc_cm_print("war rxlos    0x%04x cnt=%0d en=%0x rf_dis=%0x\n", DEV_CFG_PTR(pc)->sw_rx_los.state, 
                 DEV_CFG_PTR(pc)->sw_rx_los.count, DEV_CFG_PTR(pc)->sw_rx_los.enable, DEV_CFG_PTR(pc)->sw_rx_los.rf_dis);
