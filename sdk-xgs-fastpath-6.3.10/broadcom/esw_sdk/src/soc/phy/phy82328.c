...
_phy_82328_dbg_read(int32 unit, phy_ctrl_t *pc, uint32 addr, uint16 *data)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Read Addr=0x%04x Val=0x%04x\n", addr, *data));
	return rv;
...
{
	int32 rv;
    SOC_DEBUG_PRINT((DK_PHY, "Write Addr=0x%04x Val=0x%04x\n", addr, data));
...
_phy_82328_init_pass3(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "PHY82328 init pass3: u=%d p=%d\n", unit, port));
    pc = EXT_PHY_SW_STATE(unit, port);
    int_pc = INT_PHY_SW_STATE(unit, port);
	...
	if (PHY82328_SINGLE_PORT_MODE(pc)) {
		...
		if (logical_lane0 > PHY82328_NUM_LANES) {
			SOC_DEBUG_PRINT((DK_ERR, "PHY82328 bad auto-negotiation lane %d: u=%d p=%d"
	...
	if (MOD_AUTO_DETECT(pc)) {
		...
		SOC_DEBUG_PRINT((DK_ERR, "PHY82328 module auto detect enabled: u=%d p=%d\n",
						 unit, port));
		...
		if (SOC_FAILURE(rv)) {
			...
			SOC_DEBUG_PRINT((DK_ERR, "PHY82328 setting module auto detect failed: u=%d p%d\n",
							 unit, port));
		} 
	}
...
int32 phy_82328_init(int32 unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        SOC_DEBUG_PRINT((DK_PHY, "PHY82328 init pass3 completed: u=%d p=%d\n", unit, port));
...
_phy_82328_mod_auto_detect_process(int unit, soc_port_t port, uint16 csr) 
{
	...
	SOC_DEBUG_PRINT((DK_ERR, "PHY82328 module detected: u=%d p=%d csr=%x\n", 
					 unit, port, csr));
...
_phy_82328_mod_auto_detect_update(int unit, soc_port_t port) 
{
    ...
    if (ucode_csr & MGT_TOP_PMD_USR_GP_REGISTER_3_SPEED_OR_TYPE_CHANGE_MASK) {
		...
		if (drv_csr & MGT_TOP_PMD_USR_GP_REGISTER_3_SPEED_OR_TYPE_CHANGE_MASK) {
            ...
            SOC_DEBUG_PRINT((DK_ERR, "PHY82328 module removed u=%d p=%d\n", unit, port));
        }
    }
    ...
    if (int_pc != NULL) {
        if ((sys_intf->type == SOC_PORT_IF_GMII) || (sys_intf->type == SOC_PORT_IF_SGMII)) {
             ...
             * forcing interface and speed */
                ...
                soc_cm_print("Cur type:%s new type:%s\n", phy82328_intf_names[cur_type], 
...
phy_82328_an_set(int32 unit, soc_port_t port, int32 an)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_82328_an_set: u=%d p=%d an=%s\n",
    ...
    if (line_intf->speed > 1000) {
        ...
        if (dev_an != AN_EN(pc)) {
            SOC_DEBUG_PRINT((DK_ERR, "82328 device autonegotiation mismatch: u=%d p=%d an=%s\n",
...
phy_82328_an_get(int32 unit, soc_port_t port, int32 *an, int32 *an_done)
{
	...
	if ((line_intf->type == SOC_PORT_IF_GMII) || (line_intf->type == SOC_PORT_IF_SGMII)) {
        ...
        *an = ((data16 & PHY82328_AN_ENABLE_MSK) == PHY82328_AN_ENABLE_MSK);
        SOC_DEBUG_PRINT((DK_PHY,"PHY82328: AN Enable: 0x%x\n",data16));
        if (*an) {
            SOC_IF_ERROR_RETURN(READ_PHY82328_MMF_AN_REG(unit, pc,
                               PHY82328_AN_STS_REG, &data16));

            SOC_DEBUG_PRINT((DK_PHY, "PHY82328 AN Done: 0x%x\n",data16));
...
_phy_82328_tx_squelch(int32 unit, soc_port_t port, phy82328_intf_side_t side, int32 cfg_sql)
{
    ...
    if (PHY82328_SINGLE_PORT_MODE(pc)) {
        ...
        squelch = (cfg_sql) ? 1 : 0;
        SOC_IF_ERROR_RETURN(_phy_82328_tx_squelch_enable (unit, port, side, squelch));
        SOC_DEBUG_PRINT((DK_PHY, "82328 squelch %s side: u=%d p=%d \n", side?"SYSTEM":"LINE", unit, port));
    }
...
int32 phy82328_validate_preemphasis(uint8 pre, uint8 main, uint8 post1, uint8 post2) 
{
    ...
    if (pre > 10) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid PRE CURSOR TAP. It should be less than 10\n"));
        return SOC_E_PARAM;
    }
    if (main > 60) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid MAIN CURSOR TAP. It should be less than 60\n"));
        return SOC_E_PARAM;
    }
    if ((!post2to1) && (post1 > 18)) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid POST1 CURSOR TAP. It should be less than 18\n"));
        return SOC_E_PARAM;
    }
    if ((post2to1) && (post1 > 23)) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid POST1 CURSOR TAP. It should be less than 23\n"));
        return SOC_E_PARAM;
    }
    if (post2 > 5) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid POST2 TAP. It should be less than 5\n"));
        return SOC_E_PARAM;
    }
    if (main < (pre + post1 + post2 + 1)) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid MAIN TAP. It should be greater than (pre + post1 + post2 + 1)\n"));
        return SOC_E_PARAM;
    }
    if ((pre + main + post1 + post2) > 60) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid Value. pre + main + post1 + post2 should be less than 60\n"));
        return SOC_E_PARAM;
    }
    if ((!post2to1) && ((pre + post1) > 22)) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid Value. pre + post1 greater than 22\n"));
        return SOC_E_PARAM;
    }
    if (post2to1 && ((pre + post1) > 27)) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid Value. pre + post1 greater than 27\n"));
        return SOC_E_PARAM;
    }
    return SOC_E_NONE;  
}
                    ...
                    int addr, int count, void *data_array,uint32 ram_start)
{
    ...
    soc_cm_debug(DK_PHY,"phy_82328_bsc_read: u=%d p=%d addr=%04x\n",
                 unit, port, addr);
...
phy_82328_probe(int32 unit, phy_ctrl_t *pc)
{
    ...
    if (devid == PHY82328_ID_82328) {
        if (chip_rev == PHY82328A0_CHIP_REV) {
            ...
            SOC_DEBUG_PRINT((DK_PHY, "PHY82328 B0 Dev found")) ;
            pc->dev_name = dev_name_82328_b0;
        }
    } else if (devid == PHY82328_ID_82328F) {
        if (chip_rev == PHY82328A0_CHIP_REV) {
            SOC_DEBUG_PRINT((DK_PHY, "PHY82328F A0 Dev found"));
            pc->dev_name = dev_name_82328f_a0;
        } else if (chip_rev == PHY82328B0_CHIP_REV) {
            SOC_DEBUG_PRINT((DK_PHY, "PHY82328F B0 Dev found")) ;
            pc->dev_name = dev_name_82328f_b0;
        }
    } else if (devid == PHY82328_ID_82322) {
        pc->dev_name = dev_name_82322;
    } else {  /* not found */
        SOC_DEBUG_PRINT((DK_WARN, "port %d: BCM82xxx type PHY device detected, please use "
...
int32 phy82328_uc_dsc_rdy_tim_out(int32 unit, phy_ctrl_t *pc,  uint16 timeout_ms) 
{
    ...
    soc_cm_print("UC DSC rdy timeout\n");
    return SOC_E_INTERNAL;
...
int32 phy82328_serdes_poll_dsc_state_equals_uc_tune(int32 unit, phy_ctrl_t *pc, uint32 timeout_ms) 
{
    ...
    soc_cm_print("uc Tune State not occured:Timeout:%d\n",data16);
    return SOC_E_INTERNAL;
}    
...
_phy_82328_diag_eyescan(int32 unit, soc_port_t port, uint32 inst)
{
    ...
    soc_cm_print("Fast eyescan under Processing");
    for (hoffset = 31; hoffset>=-31; hoffset--) {
        rv = _phy_82328_diag_eyescan_hoffset_set(unit, port, hoffset);
        if (SOC_FAILURE(rv)) {
            SOC_DEBUG_PRINT((DK_ERR, "82328 failed in HOFFSET SET: u=%d p=%d err=%d\n",
                               unit, port, rv));
            goto eye_fail;
        }
        ...
        for (voffset=-31; voffset < 32; voffset++) {
            ...
            if (SOC_FAILURE(rv)) {
                SOC_DEBUG_PRINT((DK_ERR, "82328 failed in Read BER: u=%d p=%d err=%d\n",
                               unit, port, rv));
                goto eye_fail;
            }
    ...
    soc_cm_print("\n\n");
    soc_cm_print("-30  -25  -20  -15  -10  -5    0    5    10   15   20   25   30\n");
    soc_cm_print("-|----|----|----|----|----|----|----|----|----|----|----|----|-\n");
...
_phy_82328_diag_dsc(int32 unit, soc_port_t port, int32 intf, int32 lane)
{
    ...
    soc_cm_print("SNO OSR SIGDET UCV_CON RXLCK RXPPM CLK90 CLKP1 PFMAIN PFLOW DCOFFSET VGA P1LVLCTRL DFE1 DFE2 "
                 "DFE3 DFE4 DFE5 PE  ZE  ME  PO  ZO  MO TXPPM PRE MAIN POST1 POST2 \n");
    for (i = 0; i < lanes; i++) {
        dsc_cbp = &(dsc_cb[i]);
        soc_cm_print("%03d %03d %06d %07d %05d %05d %05d %05d %06d %05d %08d "
           ...
           dsc_cbp->clk90, dsc_cbp->clkp1, dsc_cbp->pf_main, dsc_cbp->pf_low,
           dsc_cbp->dc_offset,dsc_cbp->vga,dsc_cbp->p1_lvl_ctrl,dsc_cbp->dfe1,
           dsc_cbp->dfe2,dsc_cbp->dfe3,dsc_cbp->dfe4,dsc_cbp->dfe5,
           dsc_cbp->pe,dsc_cbp->ze,dsc_cbp->me, dsc_cbp->po, dsc_cbp->zo, dsc_cbp->mo,
           dsc_cbp->tx_ppm, dsc_cbp->txfir_pre, dsc_cbp->txfir_main, dsc_cbp->txfir_post1, dsc_cbp->txfir_post2);
    }
    return SOC_E_NONE;
}
                            ...
                            soc_port_ability_t *ability)
{
	...
	line_intf = &(LINE_INTF(pc));
        ...
        if ((line_intf->type != SOC_PORT_IF_KR4) && (line_intf->type != SOC_PORT_IF_KR)) {
            return SOC_E_NONE;
        }
