...
_phy_54640_reset_setup(int unit, soc_port_t port)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        if (index & 0x80) {
            PHY54640_FLAGS(pc) |= PHY54640_PHYA_REV;
        } else {
            PHY54640_FLAGS(pc) &= ~PHY54640_PHYA_REV;
        }
        ...
        PHY54640_DEV_SET_BASE_ADDR(pc); /* phy address of port 0 */
...

{
    ...
    if (PHY_IS_BCM54640E(pc)) {
        /* Write register 0x17 with the top level reg offset to write (handled in PHY54640_REG_WRITE) */
        /* Write register 0x15 the value to write to the top level register offset */
        rv = PHY54640_REG_WRITE(unit, pc, 0, 0x0D01, 0x15, 0x0003);
        if (SOC_FAILURE(rv)) {
        ...
        rv = pc->write(unit, pc->phy_id, 0x1c, 0xd040);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
        ...
        rv = pc->write(unit, pc->phy_id, 0x1c, 0xd020);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
    ...
    if (PHY_IS_BCM54640E(pc)) {
        ...
        rv = PHY54640_REG_WRITE(unit, pc, 0, 0x0D01, 0x15, 0x0001);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
        ...
        rv = pc->write(unit, pc->phy_id, 0x1c, 0xd04f);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
        ...
        rv = pc->write(unit, pc->phy_id, 0x1c, 0xd00f);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
        ...
        rv = pc->write(unit, pc->phy_id, 0x1c, 0xd000);
        if (SOC_FAILURE(rv)) {
            pc->phy_id =  PHY54640_DEV_PHY_ID_ORIG(pc);
            INT_MCU_UNLOCK(unit);
            return rv;
    ...
    pc->phy_id =  PHY54640_SLICE_ADDR(pc, 2);
...

{
    ...
    pc->phy_id =  PHY54640_SLICE_ADDR(pc, 1);
