...
drv_gex_cfp_init(int unit)
{
    ...
    if (SOC_IS_STARFIGHTER(unit) || SOC_IS_POLAR(unit) || 
        ...
        * BCM_NORTHSTAR _SUPPORT ||  BCM_NORTHSTARPLUS_SUPPORT 
        */
#ifdef BCM_NORTHSTARPLUS_SUPPORT
    if (SOC_IS_NORTHSTARPLUS(unit)) {
            ...
            drv_cfp_entry_t* entry, uint32* act_param)
{
    ...
    defined(BCM_NORTHSTARPLUS_SUPPORT)
        ...
        case DRV_CFP_ACT_LOOPBACK:
            (DRV_SERVICES(unit)->cfp_field_get)(unit, DRV_CFP_RAM_ACT, 
                DRV_CFP_FIELD_ACTION_LOOPBACK_EN, entry, &fld_val);
            if (fld_val) {
                *act_param = TRUE;
            } else {
                *act_param = FALSE;
        ...
        case DRV_CFP_ACT_STP_BYPASS:
            (DRV_SERVICES(unit)->cfp_field_get)(unit, DRV_CFP_RAM_ACT, 
                DRV_CFP_FIELD_ACTION_STP_BYPASS, entry, &fld_val);
            if (fld_val) {
                *act_param = TRUE;
            } else {
                *act_param = FALSE;
        ...
        case DRV_CFP_ACT_EAP_BYPASS:
            (DRV_SERVICES(unit)->cfp_field_get)(unit, DRV_CFP_RAM_ACT, 
                DRV_CFP_FIELD_ACTION_EAP_BYPASS, entry, &fld_val);
            if (fld_val) {
                *act_param = TRUE;
            } else {
                *act_param = FALSE;
        ...
        case DRV_CFP_ACT_VLAN_BYPASS:
            (DRV_SERVICES(unit)->cfp_field_get)(unit, DRV_CFP_RAM_ACT, 
                DRV_CFP_FIELD_ACTION_VLAN_BYPASS, entry, &fld_val);
            if (fld_val) {
                *act_param = TRUE;
            } else {
                *act_param = FALSE;
        ...
        for (i = 0; i < CFP_53115_UDF_NUM_MAX; i++) {
            if ((drv53115_udf_info[i].valid) && 
                ...
                rv = drv_gex_cfp_field_set(unit, mem_type, 
                    ...
                    &temp_val);
                if (rv  < 0) {
                    soc_cm_print("%s FAIL on field set. \n",FUNCTION_NAME());
                    return rv;
                }
