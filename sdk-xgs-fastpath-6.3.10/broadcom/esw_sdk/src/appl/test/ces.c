...
ces_test_unstruct(int u, args_t *a, void *p)
{
	...
	printk("Creating %s test services...", bcm_ces_test_encap_data[e].name);
	...
	for (i = 0;i < BCM_CES_TEST_UNSTRUCT_MAX;i++) {
	    ...
	    if (rc != BCM_E_NONE) {
		printk("Service %2d create: Failed\n", service);
		return -1;
	    }
	    ...
	    if (rc != BCM_E_NONE) {
		printk("Service %2d enable: Failed\n", service);
		return -1;
	    }
	}
	...
	printk("Verifying CES data path...\n");
	...
	printk("Verifying CES broken data path...\n");
