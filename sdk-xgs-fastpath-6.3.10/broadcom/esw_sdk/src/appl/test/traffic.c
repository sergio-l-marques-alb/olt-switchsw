/*
 * $Id: traffic.c,v 1.116 Broadcom SDK $
...
_traffic_xgs3_modport_set(int unit, int modid, int portA, int portB)
{
    ...
    if (SOC_IS_RAPTOR(unit)) {
        ...
        soc_cm_debug(DK_ERR, "portA %d portB %d\n", portA, portB);
        idx = modid;
    } else if (SOC_IS_RAVEN(unit) || SOC_IS_TRIUMPH2(unit) || 
...
traffic_test_alloc(int unit)
{
    ...
    PBMP_ITER(test->ports, port) {
        if ((rv = bcm_port_info_save(unit, port,
            ...
            printk("traffic_init: port %s: could not get port info: %s\n",
        ...
        if (rv < 0) {
            printk("traffic_test_free: ARL delete by port unsuccessful on port %s: could not set port info: %s\n",
        ...
        if (IS_ST_PORT(unit, port) && SOC_IS_FBX(unit)) {
            ...
            if (modid < 0) {
                printk("traffic_init: port %s: could not get modid\n",
                       SOC_PORT_NAME(unit, port));
            }
            if (SOC_IS_HBX(unit)) {
    ...
    if (test->cleanup) {
        ...
        if (!SAL_BOOT_QUICKTURN) {
            ...
            if (SOC_IS_ESW(unit)) {
                ...
                robo_counter_val_set_by_port(unit, test->ports, tmp64);
#endif
            }
        }
        ...
        if (!SOC_IS_XGS12_FABRIC(unit)) {
            PBMP_ITER(test->ports, port) {
                ...
                printk("TRAFFIC: restoring ports: %s, %s\n",
        ...
        PBMP_ITER(test->ports, port) {
           ...
           if(IS_FE_PORT(unit, port)) {
            ...
            if ((rv = bcm_port_info_restore(unit, port,
                                        &test->saved_port_info[port])) < 0) {
                printk("traffic_test_free: port %s: could not set port info: %s\n",
                       SOC_PORT_NAME(unit, port),
                       bcm_errmsg(rv));
                goto done;
            }
            ...
            if (IS_ST_PORT(unit, port) && SOC_IS_FBX(unit)) {
                ...
                modid = _traffic_xgs3_test_modid_get(unit, port);
                if (modid < 0) {
                    printk("traffic_test_free: port %s: could not get modid\n",
                           SOC_PORT_NAME(unit, port));
                    goto done;
                }
...
traffic_test_send_frames(traffic_test_t* test, int port_pair_index)
{
    ...
    if (pktA == NULL || pktB == NULL) {
        printk("Error initializing packet buffers\n");
        goto error;
    }
    ...
    if (SOC_IS_XGS3_SWITCH(unit)) {
        ...
        if (IS_ST_PORT(unit, portA) || IS_ST_PORT(unit, portB)) {
            ...
            if (modid < 0) {
                printk("ERROR: port %s: could not get modid\n",
    ...
    for (i=0; i < test->npkts; i++) {
        ...
        if (rv < 0) {
            printk("ERROR: port %s: bcm_tx A #%d failed: %s\n",
        ...
        if (rv < 0) {
            printk("ERROR: port %s: bcm_tx B #%d failed: %s\n",
                              ...
                              bcm_port_t portB, traffic_test_t *test)
{
    ...
    if (peer_port >= SOC_MAX_NUM_PORTS) {
        printk("peer_port num %d too large \n", peer_port );
        return BCM_E_PORT;
    }
    if (port >= SOC_MAX_NUM_PORTS) {
        printk("port num %d too large \n", port );
        return BCM_E_PORT;
    }
    ...
    if (rv < 0) {
        printk("Failed to create RX secure association \n");
        return BCM_E_PORT;
    }
    ...
    if (rv < 0) {
        printk("Failed to create TX secure association \n");
        return BCM_E_PORT;
    }
    test->macsec_info->ports[port].cur_an = new_an;
...
_macsec_poll_thread(void *data)
{
    ...
    traffic_macsec_user_data_t  port_info;
    int         poll_interval, total_time = 0;
    pbmp_t      sw_key_update_pbm;
    pbmp_t      hw_key_update_pbm;
    int         wake_time_ms;
    ...
    port_info.unit = unit;
    SOC_PBMP_CLEAR(sw_key_update_pbm);
    SOC_PBMP_CLEAR(hw_key_update_pbm);
    ...
    if (test->cleanup) {
        PBMP_ITER(pbm, port) {
            ...
            if (peer_port >= SOC_MAX_NUM_PORTS) {
                printk("peer_port num %d  >=  %d : MACSEC clean failed\n", 
            ...
            if (bcm_macsec_port_config_get(unit, port, &config) < 0) {
                printk("Port %s MACSEC clean failed\n", 
                                    SOC_PORT_NAME(unit, port));
                continue;
            }
    ...
    printk("MACSEC Poll task exiting ...\n");
    return;
}
...
traffic_test_setup_l2addr(int unit, bcm_l2_addr_t* l2addr, int vid, int port)
{
    ...
    if ((rv = bcm_stk_my_modid_get(unit, &l2addr->modid)) < 0) {
        printk("ERROR: could not get modid: %s\n", bcm_errmsg(rv));
        return rv;
    }
    ...
    if ((rv = bcm_l2_addr_add(unit, l2addr)) < 0) {
        printk("ERROR: l2 entry add failed: "
                             ...
                             traffic_mode_t mode, traffic_speed_t speed)
{
    ...
    if (bcm_port_info_save(unit, portA, infoA) < 0) {
        printk("ERROR: traffic setup unit %d, ports %s, %s: "
               "%s info save failed\n",
               unit,
               SOC_PORT_NAME(unit, portA),
               SOC_PORT_NAME(unit, portB),
    ...
    {
                printk("ERROR: traffic setup unit %d, ports %s: "
                       "ability get failed\n",
    ...
    case SPEED_42GIG:
        if ((infoA->port_ability.speed_full_duplex & SOC_PA_SPEED_42GB) &&
            ...
            infoA->local_ability.speed_half_duplex =
                infoB->local_ability.speed_half_duplex = 0;
        } else if ((infoA->port_ability.speed_half_duplex & SOC_PA_SPEED_42GB) &&                    (infoB->port_ability.speed_half_duplex & SOC_PA_SPEED_42GB)) {
            infoA->duplex = FALSE;
            infoB->duplex = FALSE;
            infoA->local_ability.speed_full_duplex =
                infoB->local_ability.speed_full_duplex = 0;
    ...
    if (speed == SPEED_MAX) {
        /* Only autoneg enable on external test, when MAX speed is
         * selected for Internal or Internal phy tests, compute the
         * speed as max capability.
         */
        ...
        if (infoA->speed < infoB->speed) {
            printk("NOTICE: port %s only capable of speed %d, setting "
    ...
    if (((rv = bcm_port_speed_set(unit, portA, infoA->speed)) < 0) || 
        ((rv = bcm_port_speed_set(unit, portB, infoB->speed)) < 0) ||
        ((rv = bcm_port_duplex_set(unit, portA, infoA->duplex)) < 0) ||
        ...
        printk("ERROR: traffic setup unit %d, ports %s, %s:"
               ...
               SOC_PORT_NAME(unit, portA),
               bcm_errmsg(rv));
        printk("%s: speed = %d, duplex = %d, loopback = %d\n",
               SOC_PORT_NAME(unit, portA),
               infoA->speed, infoA->duplex, infoA->loopback); 
        printk("%s: speed = %d, duplex = %d, loopback = %d\n",
               SOC_PORT_NAME(unit, portB), 
               infoB->speed, infoB->duplex, infoB->loopback); 
        return -1;
    }
    if ((rv = bcm_port_info_restore(unit, portB, infoB)) < 0) {
     ...
     if (_traffic_setup_macsec(unit, portA, portB, test) < 0) {
        printk("ERROR: MACSEC traffic setup unit %d, ports %s, %s: Failed\n",
               unit,
               SOC_PORT_NAME(unit, portA),
               SOC_PORT_NAME(unit, portB));
        return -1;
    ...
    if (SOC_IS_XGS12_FABRIC(unit)) {
        ...
        if (rv < 0) {
            printk("ERROR: vlan %d: add all ports failed: %s\n",
        ...
        if ((mode == TRAFFIC_INTERNAL) ||
            ...
            if (bcm_vlan_create(unit, config->vidA) < 0) {
                printk("ERROR: ports %s, %s vlan %d: cannot create vlan\n",
                       SOC_PORT_NAME(unit, portA),
                       SOC_PORT_NAME(unit, portB),
                       config->vidA);
            }
            ...
            if (bcm_port_untagged_vlan_set(unit, portA, config->vidA) < 0) {
                printk("ERROR: port %s vlan %d: cannot set default vlan\n",
                       SOC_PORT_NAME(unit, portA),
                       config->vidA);
            }
...
traffic_show_number(uint64 v)
{
    ...
    if (*s) {
        printk("%d.%02d%s", value / prec, value % prec, s);
    } else {
        printk("%d", value / prec);
    }
#endif
...
traffic_show_measured_rate(uint64 v, int interval, int bytes)
{
    ...
    if (*s) {
        printk("%d.%02d%s", val32 / prec, val32 % prec, s);
    } else {
        printk("%d", val32 / prec);
    }
#endif
...
traffic_show_runtime(uint32 value)
{
    if (value >= 12*4*7*60*60*24) {
        ...
        printk("%d years", value);
    } else if (value >= 4*7*60*60*24 ) {
        value /= 4*7*60*60*24 ;
        printk("%d months", value);
    } else if (value >= 7*60*60*24 ) {
        value /= 7*60*60*24 ;
        printk("%d weeks", value);
    } else if (value >= 60*60*24) {
        value /= 60*60*24 ;
        printk("%d days", value);
    } else if (value >= 60*60) {
        value /= 60*60;
        printk("%d hours", value);
    } else if (value >= 60) {
        value /= 60;
        printk("%d minutes", value);
    } else {
        printk("%d seconds", value) ;
    }
}
...
traffic_run_test(traffic_test_t* test)
{
    ...
    printk("TRAFFIC: unit %d ports %s (%s), %d packets, mode %s\n",
           unit,
           bmstr,
    ...
    PBMP_ITER(test->ports, port) {
        ...
        if (lastportA < 0) {    /* this will be a port A */
            ...
            if (!(portA_ability.speed_full_duplex &
                ...
                printk("WARNING: no common speed between ports %s, %s:"
                       " skip %s\n",
    ...
    if (lastportA != -1) {
        BCM_PBMP_PORT_REMOVE(test->ports, lastportA);
        printk("WARNING: odd number of ports, will not use port %s\n",
               SOC_PORT_NAME(unit, lastportA));
    }

    printk("TRAFFIC: Port pairing:");
    PBMP_ITER(test->ports, port) {
    ...
    PBMP_ITER(test->ports, port) {
        ...
        printk("TRAFFIC: setting up ports %s, %s: ",
               SOC_PORT_NAME(unit, test->port_pair[port].portA),
               SOC_PORT_NAME(unit, test->port_pair[port].portB));
    ...
    printk("TRAFFIC: waiting for link on ports %s (%s)\n",
    ...
    if (SOC_IS_ROBO(unit)) {    
#ifdef BCM_ROBO_SUPPORT
        if (soc_robo_counter_set32_by_port(unit, test->ports, 0) < 0) {
	        printk("ERROR: Clear counters failed\n");
	        return -1;
        }
#endif
    } else {
    ...
    PBMP_ITER(test->ports, port) {
        ...
        printk("TRAFFIC: starting %d byte frames on ports %s, %s\n",
               test->pkt_size,
               SOC_PORT_NAME(unit, test->port_pair[port].portA),
               SOC_PORT_NAME(unit, test->port_pair[port].portB));
    ...
    if (test->runtime <= test->poll_interval) {
        ...
        printk("NOTICE: test will poll in %d seconds"
    ...
    if (SOC_IS_ROBO(unit)) {    
#ifdef BCM_ROBO_SUPPORT
        if (soc_robo_counter_status(unit, &curr_counter_flags, &curr_counter_interval,
                           &curr_counter_pbm) < 0) {
            printk("ERROR: Failed to get counter collection status\n");
            return -1;
        }
#endif
    } else {
    ...
    if (SOC_IS_ROBO(unit)) {    
#ifdef BCM_ROBO_SUPPORT
        if (soc_robo_counter_start(unit, curr_counter_flags, 
                          test->poll_interval * 2000000, 
                          test->ports) < 0) {
            printk("ERROR: Failed to start counter collection\n");
            return -1;
        }
#endif
    } else {
    ...
    while (runtime <= test->runtime) {
        ...
        if (waitloop == 1) {
            printk("TRAFFIC: mode %s, %d byte frames, runtime ",
                   traffic_mode_str[test->mode], test->pkt_size);
            traffic_show_runtime(interval);
            printk(" of ");
            traffic_show_runtime(test->runtime);
            printk("\n");
            printk("TRAFFIC: press Ctrl-C to stop test\n");
        } else {
            printk("TRAFFIC: running... (%d%% complete)\n",
        ...
        PBMP_ITER(test->ports, port) {
...

            {
                ...
                printk("Port %s (", SOC_PORT_NAME(unit, port));
                if (speed >= 1000) {
                    if (speed % 1000) {
                        printk("%d.%dG %s", speed / 1000,
                               (speed % 1000) / 100,
                               duplex ? "FD" : "HD");
                    } else {
                        printk("%dG %s", speed / 1000,
                               duplex ? "FD" : "HD");
                    }
                } else if (speed == 0) {
                    printk("-");
                } else {
                    printk("%dM %s", speed, duplex ? "FD" : "HD");
                }
                printk("): %s\n",
                ...
                if (drpktl == 0 || dtpktl == 0 || dtbytl == 0 || drbytl == 0) {
                    printk("ERROR: traffic stopped on ports %s, %s\n",
                           SOC_PORT_NAME(unit, portA),
                           SOC_PORT_NAME(unit, portB));
                    notraffic = 1;
                }
                ...
                if (notraffic && test->stop_on_fail) {
                    ...
                    if (SOC_IS_ROBO(unit)) {    
#ifdef BCM_ROBO_SUPPORT
                       (void) soc_robo_counter_start(unit, curr_counter_flags,
    ...
    if (SOC_IS_ROBO(unit)) {    
        ...
        if (soc_robo_counter_start(unit, curr_counter_flags,
            ...
            printk("ERROR: Failed to restore counter collection\n");
            return -1;
        }
#endif
    } else {
...
traffic_test_init(int unit, args_t *a, void **pa)
{
    ...
    if (SOC_IS_NORTHSTAR(unit)||SOC_IS_NORTHSTARPLUS(unit)) {
        ...
        if (SOC_PBMP_NOT_NULL(wan_pbmp)) {
            SOC_PBMP_REMOVE(dfl_portbitmap, wan_pbmp);
        }
    }
#endif
...
traffic_test_done(int unit, void *pa)
{
    ...
    if (traf_test[unit]->cleanup) {
        ...
        printk("WARNING: CleanupOnExit disabled: "
               "(H/W configuration still active for debug)\n");
        printk("*** Zero EPC_LINK to stop traffic.\n");
        printk("*** Reboot switch when done.\n");
    }
