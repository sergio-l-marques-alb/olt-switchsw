        ...
        if (++lw->rx_pkt_cnt >= num_pkts) {
            ...
            if (!lw->sema_woke) {
                ...
                if (sal_sem_give(lw->sema)) {
                    printk("Warning: LB RX done give failed\n");
                }
            }
        }
...
lbu_port_monitor_task(int unit, bcm_rx_cb_f callback, void *cookie)
{
    ...
    if ((soc_property_get_str(unit, spn_RCPU_PORT) == NULL) && (BCM_RX_INIT_DONE(unit))) {
        printk("lbu monitor:  RX already running (u=%d)\n", unit);
        return BCM_E_INIT;
    }
#else
                   ...
                   bcm_pkt_t *rx_info)
{
    ...
    if (SOC_IS_XGS12_FABRIC(unit)) {
        ...
        if (soc_higig_field_get(unit, rx_hdr, HG_start) != 
            SOC_HIGIG_START) {
            printk("ERROR: HIGIG START not detected (rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, rx_hdr, HG_hgi) != 
            SOC_HIGIG_HGI) {
            printk("ERROR: HIGIG identifier not detected (rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, tx_hdr, HG_vlan_pri) != 
            ...
            printk("ERROR: HIGIG VLAN priority miscompare (tx=%x,rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, tx_hdr, HG_vlan_cfi) != 
            ...
            printk("ERROR: HIGIG VLAN cfi miscompare (tx=%x,rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, tx_hdr, HG_vlan_id) != 
            ...
            printk("ERROR: HIGIG VLAN id miscompare (tx=%x,rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, tx_hdr, HG_opcode) != 
            ...
            printk("ERROR: HIGIG opcode miscompare (tx=%x,rx=%x)\n",
        ...
        if (soc_higig_field_get(unit, tx_hdr, HG_cos) != 
            ...
            printk("ERROR: HIGIG cos miscompare (tx=%x,rx=%x)\n",
 ...
 */
{
    ...
    if ((rv = bcm_l2_addr_delete(unit, mac_addr, vlan)) < 0) {
        if (rv != BCM_E_NOT_FOUND) {
            soc_cm_print("bcm_l2_addr_delete FAILED\n");
            return rv;
                  ...
                  int connect)
{
    ...
    soc_cm_debug(DK_TESTS, "lbu_connect_ports: rv = %d\n", rv);
    return rv;
}
...
lbu_port_init(loopback2_test_t *lw, loopback2_testdata_t *lp)
{
    ...
    soc_cm_debug(DK_TESTS, "Saving ports\n");
    if (lbu_save_port(lw, lp) < 0) {
        return (-1);
    }
    
    /* Port setup */
    soc_cm_debug(DK_TESTS, "Port setup, bitmap %s\n",
    ...
    if (lp->loopback == LB2_MODE_MAC) {
        soc_cm_debug(DK_TESTS, "MAC loopback at speed %s\n", 
        ...
        soc_cm_debug(DK_TESTS, "PHY loopback at speed %s\n", 
                     lb2_parse_speed[lp->speed]);
...
lbu_init(loopback2_test_t *lw, loopback2_testdata_t *lp)
{
    ...
    if (!lp->inject) {
        soc_cm_debug(DK_TESTS, "Port monitor init\n");
...
#ifdef BCM_XGS3_SWITCH_SUPPORT
        uint32  hc;
        soc_cm_debug(DK_TESTS, "Hash select reset to CRC32 low\n");
...
snake_show_number(uint64 size)
{
    ...
    if (s) {
        printk("%d.%02d%s", value / 100, value % 100, s);
    } else {
    ...
    PBMP_ITER(sel_pbm, port) {
        ...
        if (!stats[port].initialized) {
            ...
            if (port_error < 0) {
                ...
                if ((dralnl > tolerance) && 
                    (IS_FE_PORT(unit, port) || IS_GE_PORT(unit, port))) {
                    printk("ALN Error Detected\n");
                } else if (drdiscl > tolerance) {
                    printk("Discard Error Detected\n");
                } else if (ddroppedl > 0) {
                    printk("Dropped Error Detected\n");
                } else if (stopped < 0) {
                    printk("FAIL->NO TRAFFIC\n");
                } else {
                    printk("PASSED\n");
                }
 ...
 */
{
    ...
    for (ix = 0; ix < lw->port_count; ix++) {
        ...
        soc_cm_debug(DK_TESTS, "Port %s Tx Count = 0x%x\n", 
        ...
        if (((IS_FE_PORT(unit, thisPort) || IS_GE_PORT(unit, thisPort)) &&
            ...
            soc_cm_debug(DK_TESTS, "Port %s Rx Count = 0x%x\n", 
                         SOC_PORT_NAME(unit, thisPort), tempStat);
            ...
            if ((tempStat > tolerance) && (tempStat < (~tolerance + 1))) {
                COMPILER_64_TO_32_LO(tempStat, stats[thisPort].tpkt);
                printk("\t0x%x packets transmitted by port %s\n",
                       tempStat, SOC_PORT_NAME(unit, thisPort));
                COMPILER_64_TO_32_LO(tempStat, stats[thisPort].rpkt);
                printk("\t0x%x packets received by port %s\n", 
                       tempStat, SOC_PORT_NAME(unit, thisPort));
...
lbu_send_packets(int unit, bcm_pkt_t **pkts, int ppt, loopback2_test_t *lw)
{
    ...
    switch (lw->cur_params->send_mech) {
        ...
        for (i = 0; i < ppt; i++ ) {
            if ((rv = bcm_tx(unit, pkts[i], NULL)) < 0) {
                printk("lbu_send_packets: ERROR: port%d: bcm_tx:%s\n",
        ...
        if ((rv = bcm_tx_array(unit, pkts, ppt, NULL, NULL)) < 0) {
            printk("lbu_send_packets (array): ERROR: port %d; ppt %d; "
    ...
    case LBD_SEND_LLIST:
    default:
        printk("lbu_send_packets: Unsupported send mechanism\n");
        return BCM_E_UNAVAIL;
        break;
    }
...
lbu_serial_wait(loopback2_test_t *lw)
{
    ...
    if (sal_sem_take(lw->sema, lw->timeout_usec)) {
        printk("Time-out waiting for completion Tx(%s) Rx(%s)\n",
               SOC_PORT_NAME(unit, lw->tx_port),
               SOC_PORT_NAME(unit, lw->rx_port));
        printk("Receive count is %d; expecting %d pkts.\n",
    ...
    */
    ...
    for (ix = 0; ix < lw->tx_ppt; ix++) {
        ...
        if (lbu_packet_compare(lw, lw->tx_pkts[ix], next_rx) < 0) {
            printk("Compare error on packet %d in chain\n", ix);
            return (-1);
        }
...
lbu_dump_tx_packets(loopback2_test_t *lw)
{
    ...
    printk("Transmitted packets\n");
    for (ix = 0; ix < lw->tx_ppt; ix++) {
        printk("TX packet %d: len=%d\n", ix, lw->tx_pkts[ix]->_pkt_data.len);
 ...
 */
{
    ...
    if (sal_sem_take(lw->sema, lw->timeout_usec)) {
        printk("Time-out waiting for snake completion\n");
        printk("Receive count is %d; expecting %d pkts.\n",
               lw->rx_pkt_cnt, lw->tx_ppt);
    ...
    if (lp) {
        if (!lp->inject) {
            ...
            if ((rv = bcm_rx_stop(lw->unit, NULL)) < 0) {
                printk("lbu_done: could not stop packet driver: %s\n",
                       bcm_errmsg(rv));
            }
