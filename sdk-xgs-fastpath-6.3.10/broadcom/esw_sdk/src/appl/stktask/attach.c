/*
 * $Id: attach.c,v 1.22 Broadcom SDK $
...
#define AT_OUT(stuff) TKS_DEBUG(TKS_DBG_NORMAL, stuff)
#define AT_VERB(stuff) TKS_DEBUG(TKS_DBG_VERBOSE, stuff)
#else
#define AT_OUT(stuff)
#define AT_VERB(stuff)
...
_bcm_stack_detach_attach(cpudb_ref_t db_ref)
{
    ...
    if (db_ref->local_entry == db_ref->master_entry) {
        AT_OUT(("STACK: master on %s (%d cpu%s, %d unit%s)\n",
                keybuf,
        ...
        AT_OUT(("STACK: slave on %s (%d cpus, master %s)\n",
                keybuf, db_ref->num_cpus, keymast));
    }

    if (dt_unit > 0) {
        AT_OUT(("STACK: detach %d unit%s\n",
                dt_unit, dt_unit == 1 ? "" : "s"));
    }

    if (at_cpu > 0) {
        AT_OUT(("STACK: attach %d unit%s on %d cpu%s\n",
                at_unit, at_unit == 1 ? "" : "s",
                at_cpu, at_cpu == 1 ? "" : "s"));
    }
}
...
{
    assert(_bcm_stack_attach_thread_active >= 0);
    assert(_bcm_stack_attach_thread_active < 2);
