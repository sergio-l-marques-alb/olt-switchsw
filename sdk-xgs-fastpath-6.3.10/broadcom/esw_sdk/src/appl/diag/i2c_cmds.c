...
cmd_adc(int unit, args_t *a)
{
    ...
    if(!sal_strncasecmp(function,"samples",sal_strlen(function))){
	    ...
	    printk("ERROR: failed to set samples count.\n");
        }
	return CMD_OK;
    }
...
cmd_muxsel(int unit, args_t *a)
{
    ...
    if ((dev_name == NULL) || (sal_strncmp(dev_name, "mux6", 4) != 0) || 
        ...
        printk("Valid devices are mux6, mux7 \n");
        return CMD_FAIL;
    }
    ...
    if (rv == 0) {
        ...
        if (soc_ndev > 0) {
            if (!(SOC_CONTROL(0)->soc_flags & SOC_F_INITED)) {
                soc_cm_print("ERROR: SOC, BCM not initialized yet!\n");
                return NULL;
            }
...
cmd_i2c(int unit, args_t *a)
{
    ...
    soc_i2c_show_log(unit, TRUE);
        ...
        if (!soc_i2c_is_attached(unit)) {
            printk("i2c raw read/write commands require probe to be called first\n");
            return CMD_USAGE;
        }
        op = ARG_GET(a);
        if (op == NULL) {
...
int bb_caladan3_current(int unit, int bx)
{
    ...
    for(i = 0; bb_cal3_current[i].adc ; i++){
        ...
        if (cpu_mode) {
            ...
            if (adc < 0) {
                printk("Device open failed for %s\n", bb_vs_all[bx][i].adc);
                return (CMD_FAIL);
            }
...
#ifdef COMPILER_HAS_DOUBLE
        printk("%s%s%-2.3f Amps (ADC: %2.3f)\n",
               bb_cal3_current[i].function,
               ical >= 0 ? "  " : " ",
               ical, adc_data.val);
#else
        printk("%s%s%-d.%03d Amps (ADC: %03d) \n",
               bb_cal3_current[i].function,
                   ical >= 0 ? "  " : " ",
...
int bb_caladan3_voltage(int unit, int bx, char *source, char* value)
{
...
#ifdef COMPILER_HAS_DOUBLE
            printk("%-.20s\t%2.3f Volts \n",
                   bb_vs_all[bx][i].function,
                   adc_data.val);
#else
            printk("%-.20s\t%d.%03d Volts \n",
                   bb_vs_all[bx][i].function,
    ...
    if( value ) {
       ...
       if (!sal_strncasecmp(value,"high",sal_strlen(value))) {
           printk("Shifting %s voltage to Margin High\n", source);
           cmd = I2C_SMM_SET_MARGIN_HIGH;
       } else if (!sal_strncasecmp(value,"low",sal_strlen(value))) {
           printk("Shifting %s voltage to Margin low\n", source);
       ...
       if (cmd > 0) {
            ...
            } else
            {
                if ( (bcm_i2c_ioctl(unit, dac,
                                    cmd,
                                    &volts, bb_vs_config[bx][i].chan) < 0)) {
                    printk("Error: failed to reset VDD_%s DAC to %s value.\n",
...
#undef BB_DEBUG
    ...
    if(!sal_strncasecmp(function, "current", sal_strlen(function))){
        ...
        if( !(baseboard->bbcmd_flags & BB_CURR) || bb_vs_config_len[bx] == 0) {
            printk("Error: %s does not support Current Measurement subsystem.\n",
               BaseboardName);
            return CMD_FAIL;
        }
...
cmd_pio(int unit, args_t *a)
{
    ...
    if ((sal_strcmp(field_name, "all") != 0) 
           && (dev != -1) && (port != -1)) {
        printk("Cannot mix field and dev,port options");
        return(CMD_FAIL);
    }
    if (all_fields && (data != -1)) {
        printk("Cannot write to all fields with same value");
        return(CMD_FAIL);
    }
    if ((dev == -1) && (port == -1)) { 
            ...
            if (!cpui2c_mode) {
                ...
                if (rv == SOC_E_NONE) {
                    if ((data >= 0) && (data <= (1<<size)-1)) {
                        ...
                        data &= (1 << size) - 1;
                        wr_data |= data << pos;
                        printk("writing port dev %d %d <- %d(0x%x)\n",
