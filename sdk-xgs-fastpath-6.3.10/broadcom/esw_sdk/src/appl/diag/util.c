/*
 * $Id: util.c,v 1.193 Broadcom SDK $
...
{
    if (!isint(str)) {
        printk("WARNING: truncated malformed address \"%s\"\n", str);
    }
...
format_spec_long_integer(char *buf, uint32 *val, int nval)
{
    ...
    if (i == 0 && val[i] < 10) {        /* Only a single word < 10? */
       ...
       sal_sprintf(buf, "0x%08X", val[i]);   /* Print first word */
    }
                ...
                if(is_phy || is_ci) {
                  ...
                  DPORT_SOC_PBMP_ITER(unit, bmall, p, tp) {
                    ...
                    if (SOC_IS_SBX_BME(unit)) {
                      /* BME devices the sci/sfi ports could be mixed */
                      if (p == port) {
                          plast = p;
                          break;
                      }
...
_parse_port(int unit, char *s, soc_port_t *portp)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        max_port = SOC_ROBO_MAX_NUM_PORTS;
    } else
...
int parse_gport(int unit, char *s, soc_port_t *portp)
{
    ...
    if (isint(parse_port_str)) {
        ...
        rv = _parse_port(unit, parse_port_str, &port_val);
    }
...
dump_l2_addr(int unit, char *pfx, bcm_l2_addr_t *l2addr)
{
    ...
    printk("%smac=%02x:%02x:%02x:%02x:%02x:%02x vlan=%d GPORT=0x%x",
           pfx,
    ...
    if (!SOC_IS_ARAD(unit) && BCM_GPORT_IS_SET(l2addr->port)) {
        if (BCM_FAILURE(_bcm_esw_l2_gport_parse(unit, l2addr, &gport_params))) {
            printk("Unknown GPORT 0x%x \n", l2addr->port);
            return;
        }
#define L2_ADDR_GPORT_FORMAT_PRINT(_fmt, _gport) \
        printk(" port=%s", format_gport(_fmt, _gport))
...
#undef L2_ADDR_GPORT_FORMAT_PRINT
    ...
    if (l2addr->flags & BCM_L2_MCAST) {
        printk(" MCast=%d", l2addr->l2mc_group);
    }
    ...
    if (l2addr->group) {
        ...
        if(SOC_IS_ARAD(unit)) {
			printk(" encap_id=0x%x", l2addr->encap_id);
		}
#endif
    printk("\n");
}
...
dump_l2_cache_addr(int unit, char *pfx, bcm_l2_cache_addr_t *l2caddr)
{
    ...
    printk(" vlan=%d", l2caddr->vlan);
    if (l2caddr->vlan_mask != BCM_L2_VID_MASK_ALL) {
        printk("/0x%03x", l2caddr->vlan_mask);
    }

    if (l2caddr->src_port_mask != 0) {
        printk(" srcport=%d", l2caddr->src_port);
        if (l2caddr->src_port_mask != BCM_L2_SRCPORT_MASK_ALL) {
            printk("/0x%02x", l2caddr->src_port_mask);
        }
    }

    if (l2caddr->flags & BCM_L2_CACHE_TRUNK) {
        printk(" trunk=%d", l2caddr->dest_trunk);
    } else {
        printk(" modid=%d port=%d/%s",
               l2caddr->dest_modid,
               l2caddr->dest_port,
    ...
    if (SOC_IS_TRX(unit)) {
        printk(" lookup_class =%d", l2caddr->lookup_class);
    }
                  ...
                  soc_pbmp_t pall)
{
    ...
    if (SOC_PBMP_EQ(bm, pall)) {        /* all ports of this type */
        sal_sprintf(dest + sal_strlen(dest), ",%s", pname);
        return;
    }
    ...
    DPORT_SOC_PBMP_ITER(unit, pall, dport, tport) {        /* a subset of ports */
        ...
        if (SOC_PBMP_MEMBER(pbmp, tport)) {
            if (pfirst < 0) {
                pfirst = plast = DPORT_FROM_DPORT_INDEX(unit, dport, port);
            } else {
                plast = DPORT_FROM_DPORT_INDEX(unit, dport, port);
            }
            continue;
        }
        if (pfirst >= 0) {
            sal_sprintf(dest + sal_strlen(dest), ",%s%d", pname, pfirst);
            if (pfirst != plast) {
                sal_sprintf(dest + sal_strlen(dest), "-%s%d", pname, plast);
            }
            pfirst = plast = -1;
        }
    }
...
format_pbmp(int unit, char *buf, int bufsize, soc_pbmp_t pbmp)
{
    ...
    if (SOC_PBMP_NOT_NULL(tpbm)) {
        ...
        if (SOC_PBMP_MEMBER(pbmp, CMIC_PORT(unit)) != 0) {
            sal_strncpy(tmp, ",cpu", FORMAT_PBMP_MAX -1);
        } else {
            tmp[0] = tmp[1] = 0;
        }

        _format_pbmp_type(unit, tmp, pbmp, "ce", PBMP_CE_ALL(unit));
        if(SOC_IS_SAND(unit)) {
            _format_pbmp_type(unit, tmp, pbmp, "xl", PBMP_XL_ALL(unit));
        }
        _format_pbmp_type(unit, tmp, pbmp, "fe", PBMP_FE_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "ge", PBMP_GE_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "xe", PBMP_XE_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "hg", PBMP_HG_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "spi", PBMP_SPI_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "sci", PBMP_SCI_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "sfi", PBMP_SFI_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "spi_s", PBMP_SPI_SUBPORT_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "hg_s", PBMP_HG_SUBPORT_ALL(unit));
        _format_pbmp_type(unit, tmp, pbmp, "il", PBMP_IL_ALL(unit));
...
int
parse_cos_range(int unit, char *s, soc_cos_t *min, soc_cos_t *max)
{
            ...
            *s = snum;
            ...
            if (port_num_blktype > 1) {
                for (i = 0; i <  port_num_blktype; i++) {
                    ...
                    if (SOC_BLOCK_IN_LIST(block_types,
                  ...
                  soc_block_types_t mask)
{
    ...
    if (SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit)) {
	blk_names = soc_sbx_block_names;
	blk_port_names = soc_sbx_block_port_names;
    } 
    else
#endif
    {
...
parse_memory_name(int unit, soc_mem_t *result, char *str, int *copyno, unsigned *array_index)
{
    ...
    if (SOC_IS_ROBO(unit)){
        ...
        for (mem = 0; mem < NUM_SOC_ROBO_MEM; mem++) {
            ...
            if (sal_strcasecmp(tmp, SOC_ROBO_MEM_NAME(unit, mem)) == 0) {
                break;
            }

            if (sal_strcasecmp(tmp, SOC_ROBO_MEM_UFNAME(unit, mem)) == 0) {
                break;
            }

            if (SOC_ROBO_MEM_UFALIAS(unit, mem) != NULL &&
                sal_strcasecmp(tmp, SOC_ROBO_MEM_UFALIAS(unit, mem)) == 0) {
                break;
            }
        }
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (cp != NULL) {
#if defined(BCM_ROBO_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (arr_ind != 0xffffffff) {
        ...
        if (!SOC_MEM_IS_ARRAY(unit, mem)) {
            printk("ERROR: an array index was specified for a memory which is not an array\n");
            return -1;
        } else if  (arr_ind >= SOC_MEM_NUMELS(unit, mem)) {
            printk("ERROR: array index %u is out of range, should be under %u\n",
                   arr_ind, SOC_MEM_NUMELS(unit, mem));
            return -1;
        }
    } else if (array_index != NULL && SOC_MEM_IS_ARRAY(unit, mem)) {
        /* no array index specified, but this is a memory array */
        printk("ERROR: an array index was not specified for a memory array\n");
        return -1;
    }
    ...
    if (numblk >= 0) {
        ...
        if (matched != numblk) {
            if (SOC_IS_ROBO(unit)) {
#ifdef BCM_ROBO_SUPPORT                  
                printk("ERROR: memory %s.%d is not valid (not enough blocks)\n",
                   SOC_ROBO_MEM_UFNAME(unit, mem), numblk);
#endif
            } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRAB_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_DFE_SUPPORT)
            printk("ERROR: memory %s.%d is not valid (not enough blocks)\n",
    ...
    *result = mem;
    ...
    if (blk != COPYNO_ALL && !SOC_MEM_BLOCK_VALID(unit, mem, blk)) {
        if (SOC_IS_ROBO(unit)){
#ifdef BCM_ROBO_SUPPORT    
            printk("ERROR: memory %s.%s (block %d) is not valid\n",
               SOC_ROBO_MEM_UFNAME(unit, mem),
               SOC_BLOCK_NAME(unit, blk),
               blk);
#endif
        } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRAB_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
        printk("ERROR: memory %s.%s (block %d) is not valid\n",
...
parse_memory_index(int unit, soc_mem_t mem, char *val_str)
{
    ...
    if (!sal_strcasecmp(val_str, "min")) {
            return soc_robo_mem_index_min(unit, mem);
        }

        if (!sal_strcasecmp(val_str, "max")) {
            return soc_robo_mem_index_max(unit, mem);
        }

        index = parse_integer(val_str);

        if (!soc_robo_mem_index_valid(unit, mem, index)) {
            printk("WARNING: index %d out of range for memory %d\n",
                index, (int)mem);
        }
#endif        
    } else {
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
        if (!sal_strcasecmp(val_str, "min")) {
    ...
    if (!soc_mem_index_valid(unit, mem, index)) {
        printk("WARNING: index %d out of range for memory %s\n",
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_PETRAB_SUPPORT) || defined(BCM_DFE_SUPPORT)
    if (SOC_MEM_IS_ARRAY(unit, mem)) {
        ...
        if (maip) {
            if (!sal_strcasecmp(val_str, "min")) {
                ...
                return maip->numels - 1;
            }
            array_index = parse_integer(val_str);
            if (array_index >= maip->numels) {
                printk("WARNING: array index %u out of range for memory %s, changing it to %u\n",
...
format_port_mode(char *buf, int bufsize, soc_port_mode_t mode, int abbrev)
{
    ...
    for (i = 0; mode != 0 && pm_map[i].str != NULL; i++) {
        ...
        if ((mode & pm_map[i].pm) == pm_map[i].pm) {
            ...
            sal_strncpy(buf, pm_map[i].str, bufsize);
                    ...
                    _port_ability_map_t *pa_map)
{
    ...
    for (i = 0; ability != 0 && pa_map[i].str != NULL; i++) {
        if ((ability & pa_map[i].pa) == pa_map[i].pa) {
            ...
            sal_strncpy(buf, pa_map[i].str, bufsize);
 ...
 *        or
 *     bcmFieldQualifyCount if badly formed string
 */
bcm_field_qualify_t
 ...
 *        or
 *     bcmFieldActionCount if badly formed string
 */
bcm_field_action_t
...
parse_field_decap(char *str)
{
    ...
    for (decap = 0; decap < bcmFieldDecapCount; decap++) {
        ...
        /* Test for whole name of the action */
        sal_strncpy(lng_str, "bcmFieldDecap", FP_DECAP_STR_SZ - 1);
        sal_strncat(lng_str, tbl_str,
                FP_DECAP_STR_SZ - 1 - sal_strlen("bcmFieldDecap"));
...
#define SWITCH_CONTROL_STR_SZ 100
#define SWITCH_CONTROL_PREFIX  "bcmSwitch"
/*
 * Function:
 *     parse_switch_control
 * Purpose:
 *     Turn CLI input into a bcm_switch_control value.
 *
 * Returns:
 *     bcm_switch_control_t value that matches the input string
 *        or
 *     bcmSwitch__Count if badly formed string
 */
bcm_switch_control_t
parse_switch_control(char *str)
{
    bcm_switch_control_t control;
    char *control_text[bcmSwitch__Count] = {BCM_SWITCH_CONTROL_STR};
    char                 tbl_str[SWITCH_CONTROL_STR_SZ];
    char                 lng_str[SWITCH_CONTROL_STR_SZ];

    if (str == NULL) {
        return bcmSwitch__Count;
    }
    ...
    for (control = 0; control < bcmSwitch__Count; control++) {
        sal_memset(tbl_str, 0, sizeof(char) * SWITCH_CONTROL_STR_SZ);
        sal_memset(lng_str, 0, sizeof(char) * SWITCH_CONTROL_STR_SZ);
        /* Test for the suffix only */
        sal_strncpy(tbl_str, control_text[control], SWITCH_CONTROL_STR_SZ - 1);
        if (!sal_strcasecmp(tbl_str, str)) {
            break;
        }
        /* Test for whole name of the action */
        sal_strncpy(lng_str, SWITCH_CONTROL_PREFIX, SWITCH_CONTROL_STR_SZ - 1);
        sal_strncat(lng_str, tbl_str,
                SWITCH_CONTROL_STR_SZ - 1 - sal_strlen(SWITCH_CONTROL_PREFIX));
        if (!sal_strcasecmp(lng_str, str)) {
            break;
        }
    }
...
/*
 * Function: 
 *     format_switch_control
 * Purpose:
 *    Format a bcm_switch_control_t variable for output.
 * Parameters:
 *    buf       - character buffer where string is written
 ...
 */
char *
format_switch_control(char *buf, bcm_switch_control_t control)
{
    char *control_text[bcmSwitch__Count] = {BCM_SWITCH_CONTROL_STR};

    assert(buf != NULL);

    if (0 <= control && control < bcmSwitch__Count) {
        sal_sprintf(buf, "%s%s", SWITCH_CONTROL_PREFIX,control_text[control]);
    } else {
        sal_sprintf(buf, "invalid control value=%#x", control);
    }
...
#ifdef INCLUDE_L3
#define TUNNEL_TYPE_STR_SZ 100
#define TUNNEL_TYPE_PREFIX  "bcmTunnelType"
/*
 * Function:
 *     parse_tunnel_type
 * Purpose:
 *     Turn CLI input into a bcm_tunnel_type value.
 *
 * Returns:
 *     bcm_tunnel_type_t value that matches the input string
 *        or
 *     return bcmTunnelTypeCount if badly formed string
 */
bcm_tunnel_type_t
parse_tunnel_type(char *str)
{
    bcm_tunnel_type_t tunnel_type;
    char *tunnel_type_text[bcmTunnelTypeCount] = {BCM_TUNNEL_TYPE_STR};    

    char                 tbl_str[TUNNEL_TYPE_STR_SZ];
    char                 lng_str[TUNNEL_TYPE_STR_SZ];

    if (str == NULL) {
        return bcmTunnelTypeCount;
    }
    ...
    /*
     * COVERITY
     * coverity complains of mixed enums being using for 'control'.
     * two enums are used intentionally as per the declaraion. 
     */ 
     /* coverity[mixed_enums] */
    for (tunnel_type = 0; tunnel_type < bcmTunnelTypeCount; tunnel_type++) {
        sal_memset(tbl_str, 0, sizeof(char) * TUNNEL_TYPE_STR_SZ);
        sal_memset(lng_str, 0, sizeof(char) * TUNNEL_TYPE_STR_SZ);
        /* Test for the suffix only */
        sal_strncpy(tbl_str, tunnel_type_text[tunnel_type], TUNNEL_TYPE_STR_SZ - 1);
        if (!sal_strcasecmp(tbl_str, str)) {
            break;
        }
        /* Test for whole name of the action */
         ...
         * COVERITY
         *
         * Coverity complains that the source string might be longer than
         * the requested length, but the length is determined from the
         * source string length.  Coverity may not be able to determine
         * this.
         */ 
        /*    coverity[buffer_size]    */
        sal_strncpy(lng_str, TUNNEL_TYPE_PREFIX,
                sal_strlen(TUNNEL_TYPE_PREFIX) + 1);
        sal_strncat(lng_str, tbl_str,
                TUNNEL_TYPE_STR_SZ - 1 - sal_strlen(TUNNEL_TYPE_PREFIX));
        if (!sal_strcasecmp(lng_str, str)) {
            break;
        }
    }
 ...
 *     format_tunnel_type
 * Purpose:
 *    Format a bcm_tunnel_type_t variable for output.
 * Parameters:
 *    buf       - character buffer where string is written
 *    control     - Field control value
 * Returns:
 ...
 */
char *
format_tunnel_type(char *buf, bcm_tunnel_type_t control)
{
    char *control_text[bcmSwitch__Count] = {BCM_TUNNEL_TYPE_STR};

    assert(buf != NULL);

    if (0 <= control && control < bcmSwitch__Count) {
        sal_sprintf(buf, "%s%s", TUNNEL_TYPE_PREFIX,control_text[control]);
    } else {
        sal_sprintf(buf, "invalid control value=%#x", control);
    }
...
int soc_phy_fw_acquire_default(const char *file_name, uint8 **fw, int *fw_len)
{
    ...
    if (sal_ferror(fp)) {
        printk("ERROR: Can't read from file : %s\n", file_name);
...
bcm_pkt_dump(int unit, bcm_pkt_t *pkt, int dump_data)
{
    ...
    soc_cm_print("Packet dump:  %p.  Data in %d blocks.\n", (void *)pkt,
                 pkt->blk_count);
    ...
    soc_cm_print("  unit %d. %s %d. sm %d. dp %d. dm %d. cos %d. "
                 "prio_int %d.\n",
    ...
    soc_cm_print("  matched %d. classification-tag %d. timestamp %d.\n",
                 pkt->rx_matched,
    ...
    CHK_FLAG(pkt, BCM_TX_CRC_FORCE_ERROR, offset, line,
             "crc_force_error ");
    soc_cm_print("  tx flags-->%s\n", line);

    offset = 0;
    CHK_FLAG(pkt, BCM_RX_CRC_STRIP, offset, line, "crc_strip ");
    CHK_FLAG(pkt, BCM_PKT_F_NO_VTAG, offset, line, "vtag_strip ");
    soc_cm_print("  rx flags-->%s\n", line);

    soc_cm_print("  dma chan %d. adr pkt_data %p. idx %d. dv %p\n",
                pkt->dma_channel, (void *)&pkt->_pkt_data, pkt->_idx, pkt->_dv);
    soc_cm_print("  hghdr: 0x%08x%08x%08x. sltag 0x%x. vtag 0x%x\n",
                 ((uint32 *)(pkt->_higig))[0], ((uint32 *)(pkt->_higig))[1],
                 ((uint32 *)(pkt->_higig))[2], ((uint32 *)(pkt->_sltag))[0],
                 ((uint32 *)(pkt->_vtag))[0]);
