...
int fw_upgrade_file(void *fw){    	
	...
	rv = _soc_ea_firmware_ver_get(f->unit, &fw_ver);	
	if (CMD_OK != rv){
	...
	if (fw_ver == fw_ver_file){
		switch (f->fw_file_type){
			case FW_FILE_TYPE_BOOT:
				printk("Soc boot firmware also is the latest version!\n");
				break;
			case FW_FILE_TYPE_APP:
				printk("Soc app firmware also is the latest version!\n");
				break;
			case FW_FILE_TYPE_PERS:
				printk("Soc pers firmware also is the latest version!\n");
				break;
			default:
				break;	
			}
	...
	rv = soc_ea_firmware_upgrade(f->unit, f->fw_file_type, len, buff);   	
	fclose(fpointer);	
	if (CMD_OK != rv){		
