/*
 * $Id: regex.c,v 1.23 Broadcom SDK $
                                    ...
                                    void *user_data)
{
    ...
    *pos = '\0';
    ...
    printk("dst_ip          =   %d.%d.%d.%d\n",
                      ...
                      (report->dip >> 8) & 0xff,
                      (report->dip >> 0) & 0xff);
    printk("sip6            =   %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    printk("dip6            =   %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    printk("start_timestamp =   %d, x10 ms\n", report->start_timestamp);
    printk("last_timestamp  =   %d, x10 ms\n", report->last_timestamp);
    printk("refresh_timestamp = %d, x10 ms\n", report->last_timestamp);
    return;
}
                                 ...
                                 void *user_data)
{
    ...
    printk("------------------------------------------\n");
    printk("\tEngine-ID: 0x%x, HW index %d\n", engine, enghwid);
                              ...
                              void *user_data)
{
    ...
    if (eng_hwidx >= 0) {
        ...
        /*
        printk("Engine ID %d, hw index %d, engine count %d\n", engid, eng_hwidx, l_engine_count[unit]);
        */
    }
#else
    return CMD_NOTIMPL;
...
static int dump_regex_session_info(int unit)
{
    ...
    if (rv != 0) {
        printk("Session info not found\n");
    } else {
        printk("Session Table Size      = %d\n", regex_info.session_size);
        printk("Session Table IPv4 Free = %d\n", regex_info.session_free_size_ipv4);
        printk("Session Table IPv6 Free = %d\n", regex_info.session_free_size_ipv6);
        printk("Policy Table Size       = %d\n", regex_info.policy_size);
        printk("Policy Table Free Size  = %d\n", regex_info.policy_free_size);
    }
...
static int dump_regex_match_stats(int unit)
{
    ...
    printk("Sig match packets received      = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match packets sent          = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match packets dropped       = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match bytes matched         = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match matched flows         = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match unmatched flows       = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match total match           = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match cross sig flags       = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match fragments received    = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match in packet error       = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match flow tracker error    = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match packet length error   = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match L4 checksum error     = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match flow done packet drop = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match flow timestamp error  = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match flow packet num error = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
    printk("Sig match ECC error             = %08x:%08x\n", COMPILER_64_HI(val[i]), COMPILER_64_LO(val[i])); i++;
