...
#else
    bcm_mac_t mac1 = {0xe0, 0xfc, 0x00, 0x00, 0x00, 0x00};
#endif
...
fip_handle_flogi(bcm_pkt_t *info)
{
    ...
    printk("FIP FLOGI packet: time=0x%x, OXID=0x%4x, CNA port=%d\n", 
        sal_time_usecs(), fip_flogi_pending[entry].oxid, fip_flogi_pending[entry].port);
    printk("SRCMAC: 0x%04x%04x%04x \n",
...
fip_tx_els_reply(int unit, bcm_pkt_t *info)
{
    ...
    printk("ELS reply packet: time=0x%x, OXID=0x%4x, CNA port=%d\n", sal_time_usecs(), oxid, fip_flogi_pending[entry].port);
    printk("DSTMAC: 0x%04x%04x%04x \n",
        *(uint16 *)(fip_flogi_pending[entry].sa + 0),
        *(uint16 *)(fip_flogi_pending[entry].sa + 2),
        *(uint16 *)(fip_flogi_pending[entry].sa + 4));
...
fcoe_flogi_timer(void *p_owner, void *p2, void *p3, void *p4, void *p5)
{
    ...
    if (p_fip_flogi->used) {
        printk("fcoe_flogi_timer: OXID = %x\n", p_fip_flogi->oxid);
...
_print_fcmap_cfg(bcm_fcmap_port_config_t *cfg) 
{
    if (cfg != NULL) {
        ...
        printk("Ingress Mapper Bypass => %d\n", cfg->ingress_mapper_bypass);
        printk("Ingress FC Mapper Table Input => %d\n", cfg->ingress_map_table_input);
        printk("Ingress FC CRC Mode => %d\n", cfg->ingress_fc_crc_mode);
        printk("Ingress VFT Hdr Processing mode => %d\n", cfg->ingress_vfthdr_proc_mode);
        printk("Ingress VLAN TAG Processing mode => %d\n", cfg->ingress_vlantag_proc_mode);

        printk("Egress Mapper Bypass => %d\n", cfg->egress_mapper_bypass);
        printk("Egress FC Mapper Table Input => %d\n", cfg->egress_map_table_input);
        printk("Egress FC CRC Mode => %d\n", cfg->egress_fc_crc_mode);
        printk("Egress VFT Hdr Processing mode => %d\n", cfg->egress_vfthdr_proc_mode);
        printk("Egress VLAN TAG Processing mode => %d\n", cfg->egress_vlantag_proc_mode);
...
int fcoe_start(int unit, int fc_port)
{
    ...
    cfg.egress_fc_crc_mode = cfg.ingress_fc_crc_mode = 1;
#if FIP_VERSION_1
