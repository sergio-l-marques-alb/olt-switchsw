...
rx_conn_thread(void *cookie)
{
    ...
    while (!(conn_ptr->flags & ATP_SOCK_CONN_ABORT)) {
        ...
        if (buffer == NULL) {
            ATP_SOCK_ERR(("AbnormalThreadExit:%s ERROR: Cannot allocate receive "
                          "buffer\n", thread_name));
            rv = BCM_E_MEMORY;
            break;
        }
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Destroy connection to '%s', socket %d\n",
                    dest_ip_str, rx_sock));
    if (BCM_FAILURE(rv)) {
        ATP_SOCK_ERR(("AbnormalThreadExit:%s\n", thread_name)); 
    }   
    sal_thread_exit(rv);
...
server_conn_thread(void *cookie)
{
    ...
    ATP_SOCK_VERB(("ATP-Socket: Exit Server Thread '%s', "
...
atptrans_socket_server_start(void)
{
    ...
    ATP_SOCK_VVERB(("ATP-Socket: Created Server Thread '%s'\n",
                    atp_sock_server_thread_name));
