    ...
    bcm_ptp_telecom_g8265_quality_level_t QL)
{
    ...
    if (ci.slaveonly) {
        ...
        if (telecom.network_option != (QL /0x10)) {
            PTP_LOG("ITU-T G.781 synchronization networking option changed.\n");
        }
    ...
    uint8 flags)
{
    ...
    case bcmPTPUDPIPv4:
        PTP_LOG("Master IP Address (IPv4): %d.%d.%d.%d\n",
        ...
        break;
    case bcmPTPUDPIPv6:
        PTP_LOG("Master IP Address (IPv6): %02x%02x:%02x%02x:%02x%02x:"
