...
#define _MBOX_ERROR(errmsg)      soc_cm_print("%s() FAILED %s\n", \
                                           FUNCTION_NAME(), errmsg)

#define _MBOX_ERROR_FUNC(func)   soc_cm_print("%s() FAILED "   \
                                           func " returned %d : %s\n",  \
                                           FUNCTION_NAME(), rv, bcm_errmsg(rv))

#define _MBOX_WARN(warnmsg)      soc_cm_print("%s() failed %s\n",  \
                                           FUNCTION_NAME(), warnmsg)

#define _MBOX_WARN_FUNC(func)    soc_cm_print("%s() failed " \
                                           func " returned %d : %s\n",  \
                                           FUNCTION_NAME(), rv, bcm_errmsg(rv))
...
_bcm_mbox_rx_response_free(int unit, uint8 *resp_data)
{
    ...
    soc_cm_print("Invalid CMICM rx response free (%p)\n", (void *)resp_data);
...
_bcm_mbox_debug_poll(void* owner, void* time_as_ptr, void *unit_as_ptr, void *unused_2, void* unused_3)
{
    ...
    if (out_idx) {
        ...
        if (_cmicm_debug_flags) {
            soc_cm_print("%s", output_debugbuf);
        }
    }
