/*
 * $Id: vlan.c,v 1.464.2.8 Broadcom SDK $
...
STATIC int _bcm_vlan_port_add(int unit, bcm_vlan_t vid, pbmp_t pbmp,
                           pbmp_t ubmp, pbmp_t ing_pbmp, soc_field_t *dlf_type);
...
bcm_esw_vlan_create(int unit, bcm_vlan_t vid)
{
    ...
    if (rv == BCM_E_NONE) {
        ...
        rv = _bcm_vlan_port_add(unit, vid, stacked, empty_pbmp, stacked,NULL);
    }
#endif  /* BCM_VLAN_NO_AUTO_STACK */
    BCM_UNLOCK(unit);
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                BCM_IF_ERROR_RETURN(_bcm_esw_multicast_ipmc_read(unit, mc_index, 
                                                                 &l2_pbmp, 
                                                                 &l3_pbmp));

                SOC_PBMP_OR(l2_pbmp, orig_pbmp);
                BCM_IF_ERROR_RETURN(_bcm_esw_multicast_ipmc_write(unit, 
                                             mc_index, l2_pbmp, l3_pbmp, TRUE));
            }
        }
    }
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */
...
bcm_esw_vlan_port_add(int unit, bcm_vlan_t vid, pbmp_t pbmp, pbmp_t ubmp)
{
    ...
    BCM_LOCK(unit);
    rv = _bcm_vlan_port_add(unit, vid, pbmp, ubmp, pbmp,NULL);
    BCM_UNLOCK(unit);
...
_bcm_vlan_port_remove(int unit, bcm_vlan_t vid, pbmp_t pbmp)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        int              mc_index;
        bcm_multicast_t  mc_group;
        pbmp_t           l2_pbmp, l3_pbmp, l2_l3_pbmp;
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
...
#if defined(BCM_ENDURO_SUPPORT)
                if (SOC_IS_ENDURO(unit)) {
                    rv = bcm_enduro_vlan_vp_update_vlan_pbmp(unit, vid,
                            &l2_l3_pbmp); 
                } else 
#endif /* BCM_ENDURO_SUPPORT */                
                {
                    rv = bcm_tr2_update_vlan_pbmp(unit, vid,
                            &l2_l3_pbmp, _BCM_VLAN_PORT_DELETE); 
                }
                if (rv < 0) {
                    BCM_UNLOCK(unit);
                    return rv;
            ...
            if (_BCM_MULTICAST_IS_NIV(mc_group) ||
                    _BCM_MULTICAST_IS_EXTENDER(mc_group)) {
                rv = bcm_tr2_update_vlan_pbmp(unit, vid, &l2_l3_pbmp, _BCM_VLAN_PORT_DELETE); 
                if (rv < 0) {
                    BCM_UNLOCK(unit);
                    return rv;
                }
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                SOC_PBMP_OR(*pbmp, l2_pbmp);
            }
            SOC_PBMP_ASSIGN(*ing_pbmp, *pbmp);
        }
    }
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */

    VLAN_VERB(("VLAN %d: bcm_vlan_port_get: vid %d. pbm 0x%x upbm 0x%x.\n",
                            ...
                            bcm_port_t port, int stp_state)
{
    ...
    BCM_LOCK(unit);
    rv = _bcm_vlan_stp_set(unit, vid, local_port, stp_state);
    BCM_UNLOCK(unit);
                            ...
                            bcm_port_t port, int *stp_state)
{
    ...
    BCM_LOCK(unit);
    rv = _bcm_vlan_stp_get(unit, vid, local_port, stp_state);
    BCM_UNLOCK(unit);
    ...
    bcm_policer_t *policer)
{
    ...
    *policer = 0;
    rv = _bcm_esw_get_policer_from_table(unit, VLAN_TABm, vlan, &vt, 
                                                 policer, 0); 
    if (BCM_FAILURE(rv)) {
        VLAN_ERR(("%s : Unable to read vlan table entry\n", \
                                                 FUNCTION_NAME()));
        return (rv);
    }
    return rv;
    ...
    bcm_policer_t policer)
{
    ...
    if (BCM_FAILURE(rv)) {
        VLAN_ERR(("%s : Unable to add policer to vlan table entry\n", \
                                                 FUNCTION_NAME()));
        soc_mem_unlock(unit, VLAN_TABm);
        return (rv);
    }
                                                      ...
                                                      int *stop)
{
    ...
    *stop = FALSE;
    ...
    return usr_cb_st->usr_cb(unit, gport,
                             trvs_str->outer_vlan, 
                             trvs_str->inner_vlan, 
                             trvs_str->action,
                             trvs_str->user_data);
                                    ...
                                    bcm_vlan_t old_vid_high)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_translation_range)) {
        ...
        if (SOC_IS_TRX(unit)) {
            int rv = BCM_E_NONE;
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
            if (SOC_IS_TRIUMPH3(unit) || SOC_IS_KATANA(unit)) {
                                   ...
                                   bcm_port_t *port_out)
{
    ...
    int  id;
#if defined(BCM_KATANA2_SUPPORT)
    int is_local_subport = 0, my_modid = 0, pp_port;
#endif
                              ...
                              bcm_vlan_control_port_t type, int * arg)
{
    ...
    if (SOC_IS_FBX(unit)) {
        ...
        case bcmVlanTranslateEgressMissUntaggedDrop:
            if ((soc_feature(unit, soc_feature_untagged_vt_miss)) &&
                (soc_feature(unit,soc_feature_vlan_translation))) {
                SOC_IF_ERROR_RETURN (soc_reg32_get(unit, egr_register, port, 
                                     0, &evc));
                *arg = soc_reg_field_get(unit, egr_register, evc,
                                         VT_MISS_UT_DROPf);
                return BCM_E_NONE;
                     ...
                     bcm_pbmp_t remove_ports)
{
...
#ifdef BCM_TRIUMPH3_SUPPORT
    int pbmp_update=0, ing_pbmp_update=0;
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        ...
        pbmp_update = _stk_vlan_bitmap_update(unit, remove_ports, old_ports, &new_ports);
        if (pbmp_update) {
                              ...
                              bcm_vlan_control_vlan_t control)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_ctrl)) {
        ...
        rv = bcm_esw_vlan_control_vlan_selective_set(unit, vid, valid_fields, 
                                                    &control);
    }
            ...
            void        *to_entry)
{
   ...
   for (f = memp_from->nFields - 1; f >= 0; f--) {
        ...
        /* soc_cm_print(":%d:%s: ==>", f,soc_fieldnames[fieldp_from->field]); */
#if !defined(SOC_NO_NAMES)
            ...
            (strstr(soc_fieldnames[fieldp_from->field], ":") == NULL)) {
             if (soc_mem_field_valid(unit,
                 ...
                 if (fieldp_to->len < fieldp_from->len) {
                    soc_cm_print("Warning:%d:%s: Mismatch in length %d:%d ==>", 
                     ...
                     soc_cm_print("Warning:%d:%s: More lengthy field %d:%d==>", 
                                  f,soc_fieldnames[fieldp_from->field],
                                  fieldp_from->len,fieldp_to->len); 
                 }
                 ...
                 soc_mem_field_set(unit,to_table,
                                   (uint32 *)to_entry ,
                                   fieldp_from->field, fval);
                 /* soc_cm_print("Matched \n"); */
             } /* soc_cm_print("Not Matched \n"); */
        } /* soc_cm_print("Skipped \n"); */
#endif
   }
   return BCM_E_NONE;
}
                       ...
                       int flags)
{
    ...
    if (soc_feature(unit, soc_feature_wlan) && BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        if ((flags & BCM_VLAN_PORT_INGRESS_ONLY) && 
            ...
            rv = bcm_td_vlan_vp_group_set(unit,VLAN_TABm,vlan,
                    BCM_GPORT_VP_GROUP_GET(port),TRUE);
            BCM_IF_ERROR_RETURN(rv); 
        }
...
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */
    {
        ...
        if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
            ...
            if ((!umc_do_not_add) && (bc_idx != umc_idx)) {
                group_types[1] = UMC_IDXf;
            }
            if ((!uuc_do_not_add) && (uuc_idx != umc_idx) && (uuc_idx != bc_idx)) {
        ...
        if (SOC_IS_FBX(unit)) {
            BCM_LOCK(unit);
            rv = _bcm_vlan_port_add(unit, vlan, pbmp, ubmp, ing_pbmp,group_types);
            BCM_UNLOCK(unit);
                       ...
                       int *flags)
{
    ...
    if (soc_feature(unit, soc_feature_wlan) && BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        *flags=0;
        if (soc_feature(unit, soc_feature_vp_group_ingress_vlan_membership)) {
            rv = bcm_td_vlan_vp_group_get(unit,VLAN_TABm,vlan,
                        BCM_GPORT_VP_GROUP_GET(port),&enable);
            BCM_IF_ERROR_RETURN(rv);
            if (enable) {
...
_bcm_vlan_reinit(int unit)
{
    ...
    if (0 == _BCM_VBMP_LOOKUP(vi->bmp, vi->defl)) {
        VLAN_WARN(("Warm Reboot: Default VLAN %4d does not exist!\n",
                  vi->defl));
    }
