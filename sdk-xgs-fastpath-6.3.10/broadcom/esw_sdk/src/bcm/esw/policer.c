                  ...
                  _global_meter_policer_control_t *policer_control)
{
    ...
    if (pol_cfg->flags & BCM_POLICER_MIXED_MICRO_MACRO) {
        ...
        if (!(BCM_SUCCESS(rv))) {
            soc_cm_debug(DK_ERR,"Unable to access macro flow table at the index provided\n");
            return rv;
        }
        ...
        if (macro_flow_meter_index > 0) {
            ...
            rv = _bcm_esw_policer_decrement_ref_count(unit,
                                                      macro_meter_policer);
            if (BCM_FAILURE(rv)) {
                soc_cm_debug(DK_ERR,"%u : Unable to decrement ref count",
            ...
            rv = WRITE_SVM_MACROFLOW_INDEX_TABLEm(unit, MEM_BLOCK_ANY,
                                          index, &macro_flow_entry);
            if (!(BCM_SUCCESS(rv))) {
                soc_cm_debug(DK_ERR,"%s : Unable to write to macro flow table at index \
                                provided\n", FUNCTION_NAME());
                return rv;
            }
        }
    ...
    switch (mode) {
        ...
        case bcmPolicerGroupModeTrafficType:
        {
            *npolicers = 3;
            ...
            rv =  _bcm_esw_policer_set_offset_table_map(7, &table_offset[0], 
                      &value[0], 
                      &mode_attr->uncompressed_attr_selectors_v.offset_map[0]); 
            BCM_IF_ERROR_RETURN(rv);
            break;
                                   ...
                                   bcm_policer_t *policer_id, int *npolicers)
{
    ...
    if (BCM_FAILURE(rv)) {
        POLICER_VERB(("%s : Unable to create policer for given mode %d \n",\
                           FUNCTION_NAME(),mode));
        return (rv);
    }
        ...
        case bcmPolicerGroupModeTypeCascade:
            for (i = 0; i < num_selectors; i++) {
                if (attr_selectors[i].policer_offset >= num_pools) {
                    POLICER_VERB(("%s : Invalid policer offset  \n",\
                                                         FUNCTION_NAME()));
                    return BCM_E_PARAM;
                }    
...
{    
    ...
    if (BCM_FAILURE(rv)) {
        POLICER_VERB(("%s : Failed to get info related to UDF Id \n",\
                                                            FUNCTION_NAME()));
        return rv;
    }
    ...
    if (!((udf_offset_node->hw_bmap & 0x1) || (udf_offset_node->hw_bmap & 0x2))) {
        POLICER_VERB(("%s : UDF is not properly configured for policer use \n",\
                                                            FUNCTION_NAME()));
        return BCM_E_PARAM;
    }    
#endif
    if (attr_selectors[0].width > 8) {
        POLICER_VERB(("%s : Invalid offset and width passed for UDF \n",\
                                                            FUNCTION_NAME()));
        return (BCM_E_PARAM);
    }   
    ...
    uint32 *mode_id)
{
    ...
    if (attr_selectors[0].attr != bcmPolicerGroupModeAttrUdf) {
       return (BCM_E_UNAVAIL);
        ...
        if (BCM_FAILURE(rv)) {
            POLICER_VERB(("%s : Unable to create policer group for type=%d \n",\
                                FUNCTION_NAME(),type));
            return (rv);
        }
    }
    *mode_id = *mode_id + 1;
    POLICER_VERB(("%s : Created policer group with mode id 0x%x \n",\
                       FUNCTION_NAME(), *mode_id));
    return rv;
    ...
    uint32 mode_id)
{
    ...
    if (mode_info.reference_count > 0) {
        POLICER_ERR(("%s : Policer group mode is still in use \n", FUNCTION_NAME()));
        return (BCM_E_BUSY);
    } else {    
                                ...
                                uint32 action_id)
{
    ...
    if (policer_control->action_id == action_id) {
        GLOBAL_METER_UNLOCK(unit);
        POLICER_VERB(("%s : Action Id passed is same as the one in \
