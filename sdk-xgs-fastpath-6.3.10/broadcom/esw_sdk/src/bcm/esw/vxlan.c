/*
 * $Id: vxlan.c,v 1.25.2.2 Broadcom SDK $
            ...
            bcm_stat_flex_table_info_t *table_info)
{
    ...
    if ( !(_BCM_VXLAN_VPN_INVALID == vpn) &&
        ...
        BCM_IF_ERROR_RETURN 
                 (_bcm_td2_vxlan_vpn_is_eline(unit, vpn, &isEline_data));
                            ...
                            bcm_vpn_t           vpn)
{
    ...
    if (err_code[bcmStatFlexDirectionIngress] == BCM_E_NONE) {
        if (err_code[bcmStatFlexDirectionEgress] == BCM_E_NOT_FOUND) {
            return BCM_E_NONE;
        } else {
            return err_code[bcmStatFlexDirectionEgress];
        }
    } else if (err_code[bcmStatFlexDirectionIngress] == BCM_E_NOT_FOUND){
        if (err_code[bcmStatFlexDirectionEgress] == BCM_E_NONE) {
            return BCM_E_NONE;
        } else {
            return BCM_E_NOT_FOUND;
        }
    } else {
        return err_code[bcmStatFlexDirectionIngress];
    }
