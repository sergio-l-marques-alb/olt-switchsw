/*
 * $Id: l3.c,v 1.866.2.27 Broadcom SDK $
...
_bcm_fb_lpm128_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (idx  = read_reverse ? (idx_end - 1) : idx_start; 
        ...
        for (tmp_idx = 0; tmp_idx < ((ipv6) ? 1 : 2); tmp_idx++) {
            ...
            if (ipv6 && b128 != 1) {
                soc_cm_debug(DK_ERR, "b128 traverse: entry is not b128\n");
...
_bcm_xgs3_l3_free_resource(int unit)
{
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        if(BCM_TR3_ESM_HOST_TBL_PRESENT(unit)) {
            ...
            if (BCM_FAILURE(rv)) {
                L3_ERR(("Error in freeing ESM host tbl state %d\n", rv));
            }
        }
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_l3_ip4_options_profile)) {
       ...
       if (BCM_FAILURE(rv)) {
           L3_ERR(("Error in freeing IP4 options profile %d\n", rv));
       }
    }
#endif /* BCM_TRIDENT2_SUPPORT */
...
uint32 protection_flags=0;
   ...
   /* Store api flags within egr object flags */
    ...
    return (BCM_E_NONE);
cleanup:
    rflag = BCM_L3_REPLACE | BCM_L3_WITH_ID;
...
#ifdef BCM_KATANA_SUPPORT
    if (SOC_IS_KATANA(unit) && SOC_DEBUG_CHECK(DK_L3)) {
...
#endif /* BCM_TRIUMPH2_SUPPORT */
#if defined(BCM_TRIDENT2_SUPPORT)
    static _bcm_l3_hw_calls_t     td2_hw_call;         /* Trident2 hw calls. */
...
#endif /* BCM_TRIDENT_SUPPORT */
#if defined(BCM_TRIDENT2_SUPPORT)
        sal_memcpy(&td2_hw_call, &td_hw_call, sizeof(_bcm_l3_hw_calls_t));
        td2_hw_call.l3_add       = _bcm_td2_l3_add;
        td2_hw_call.l3_get       = _bcm_td2_l3_get;
        td2_hw_call.l3_del       = _bcm_td2_l3_del;
        td2_hw_call.l3_traverse  = _bcm_td2_l3_traverse;
        td2_hw_call.l3_del_match = _bcm_td2_l3_del_match;
        td2_hw_call.l3_get_by_idx = _bcm_td2_l3_get_by_idx;

        td2_hw_call.lpm_get       = _bcm_l3_lpm_get;
        td2_hw_call.lpm_add       = _bcm_l3_lpm_add;
        td2_hw_call.lpm_del       = _bcm_l3_lpm_del;
        td2_hw_call.lpm_update_match  = _bcm_l3_lpm_update_match;

#ifdef ALPM_ENABLE
        /* If ALPM is enabled */
        if (soc_property_get(unit, spn_L3_ALPM_ENABLE, 0)) {
            td2_hw_call.lpm_add       = _bcm_td2_alpm_add;
            td2_hw_call.lpm_get       = _bcm_td2_alpm_get;
            td2_hw_call.lpm_del       = _bcm_td2_alpm_del;
            td2_hw_call.lpm_update_match  = _bcm_td2_alpm_update_match;
        }
#endif

        td2_hw_call.ipmc_get     = _bcm_td2_l3_ipmc_get;
        td2_hw_call.ipmc_add     = _bcm_td2_l3_ipmc_add;
        td2_hw_call.ipmc_del     = _bcm_td2_l3_ipmc_del;
        td2_hw_call.ipmc_get_by_idx   = _bcm_td2_l3_ipmc_get_by_idx;

        td2_hw_call.proxy_nh_add = _bcm_td2_proxy_nh_add;
        td2_hw_call.proxy_nh_get = _bcm_td2_proxy_nh_get;
...
#ifdef BCM_TRIDENT2_SUPPORT
                if (SOC_IS_TRIDENT2(unit)) {
                    BCM_XGS3_L3_HW_CALL(unit) = &td2_hw_call;
...
void dump_ecmp_info(int unit)
{
    ...
    _bcm_l3_ecmp_info_t *ptr;
     ...
     soc_cm_print("Dumping ecmp_info\n");
     ptr = BCM_XGS3_L3_TBL_PTR(unit, ecmp_info);
     if (ptr) {
         soc_cm_print("ecmp_max_paths - %d ecmp_inuse - %d\n",
                      ptr->ecmp_max_paths, ptr->ecmp_inuse);
     }

     tbl_ptr = BCM_XGS3_L3_TBL_PTR(unit, ecmp_grp);
     if (tbl_ptr) {
         soc_cm_print("ECMP GROUP INFO:\n");
         soc_cm_print("idx_min: %d idx_max: %d idx_maxused: %d\n",
                       tbl_ptr->idx_min, tbl_ptr->idx_max,
                       tbl_ptr->idx_maxused);
         soc_cm_print("HASH and REFCOUNT for each ECMP GROUP\n");

         for (idx = tbl_ptr->idx_min; idx < tbl_ptr->idx_max; idx++) {
             if (tbl_ptr->ext_arr[idx].ref_count != 0) {
                 soc_cm_print("[idx: %d ref: %d, hash: %d],", idx,
     ...
     for (idx = tbl_ptr->idx_min; idx < tbl_ptr->idx_maxused; idx++) {
         ...
         soc_cm_print("groud id - %d  count - %d rv - %d\n", idx, member_count, rv);
     }
     ...
     if (tbl_ptr) {
         ...
         soc_cm_print("idx_min: %d idx_max: %d idx_maxused: %d\n",
                       tbl_ptr->idx_min, tbl_ptr->idx_max,
                       tbl_ptr->idx_maxused);
         soc_cm_print("HASH and REFCOUNT for each ECMP \n");
