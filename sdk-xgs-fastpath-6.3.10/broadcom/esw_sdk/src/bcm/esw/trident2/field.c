/* $Id: field.c,v 1.58.2.2 Broadcom SDK $
                               ...
                               bcm_field_IpType_t type)
{
    ...
    switch (type) {
      ...
      case bcmFieldIpTypeMim:
          ...
          break;
      default:
          FP_VVERB(("FP(unit %d) vverb: entry=%d qualifying on Invalid Iptype=%#x\n))",
                   unit, f_ent->eid, type));
          return (BCM_E_UNAVAIL);
    }
                           ...
                           )
{
    ...
    if (soc_feature(unit, soc_feature_field_ingress_two_slice_types)) {
        if (stage_fc->stage_id == _BCM_FIELD_STAGE_INGRESS) {
            FP_VVERB(("FP(unit %d) vverb: _bcm_field_td2_scache_sync() -"
                        "section  Writing  HintIds information @ byte %d.\n",
                        unit, fc->scache_pos));
