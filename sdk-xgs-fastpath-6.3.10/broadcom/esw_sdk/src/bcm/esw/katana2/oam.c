     ...
     */
        ...
        if (h_data_p->is_remote) {
            ...
            */
            ...
            if (BCM_FAILURE(rv)) {
                OAM_ERR(("OAM(unit %d) Error:BHH local_endpoint_delete failled "
                "(EP=%d) - %s.\n", unit, ep_id, bcm_errmsg(rv)));
                return (rv);
            }
            h_data_p->in_use = 0;
...
bcm_kt2_oam_init(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        OAM_ERR(("OAM(unit %d) Error: Creating BHH pool failed \n", unit));
        _bcm_kt2_oam_control_free(unit, oc);
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        OAM_ERR(("OAM(unit %d) Error: Reserving BHH endpoints from mep_pool \
                 failed \n", unit));
        _bcm_kt2_oam_control_free(unit, oc);
        return (rv);
    }
    ...
    if ( oc->event_thread_id ) {
        ...
        while (oc->event_thread_id != NULL) {
            if (sal_time_usecs() < timeout) {
                ...
                /*timeout*/
                OAM_ERR(("BHH event thread did not exit.\n"));
                return BCM_E_INTERNAL;
            }
        }
    }
                                ...
                                _bcm_oam_hash_key_t  *hash_key)
{
    ...
    if ( endpoint_info->level != _BCM_OAM_EP_LEVEL_MAX ) {
        OAM_ERR(("OAM(unit %d) Error: EP Level should be equal to %d\n",
        unit, _BCM_OAM_EP_LEVEL_MAX));
        return (BCM_E_PARAM);
    }
    ...
    */
    if ( endpoint_info->flags & 
         (BCM_OAM_ENDPOINT_LOSS_MEASUREMENT | BCM_OAM_ENDPOINT_DELAY_MEASUREMENT)
            ...
            if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, endpoint_info->intf_id)) {
                OAM_ERR(("OAM(unit %d) Error: bcm_oam_endpoint_info_t.intf type"
    ...
    if (is_remote) {
        ...
        if (BCM_FAILURE(rv)) {
            ...
            OAM_ERR(("OAM(unit %d) Error: EP MA INDEX alloc"
                     "failed EP:%d %s.\n", unit, endpoint_info->id,
                                                     bcm_errmsg(rv)));
            return (rv);
        }
        ...
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("OAM(unit %d) Error: reading EGR_L3_NEXT_HOP,\
                for EP=%d %s.\n", unit, endpoint_info->id, bcm_errmsg(rv)));
            _bcm_kt2_oam_endpoint_cleanup(unit, 0, *hash_key, hash_data);
            return (rv);
        }
        ...
        rv = soc_mem_write(unit, EGR_L3_NEXT_HOPm, MEM_BLOCK_ALL, 
                egr_nh_index, &egr_nh_entry);
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("OAM(unit %d) Error: writing EGR_L3_NEXT_HOP,\
                for EP=%d %s.\n", unit, endpoint_info->id, bcm_errmsg(rv)));
            _bcm_kt2_oam_endpoint_cleanup(unit, 0, *hash_key, hash_data);
            return (rv);
        }
...
_bcm_kt2_oam_bhh_hw_init(int unit)
{
    ...
    if(BCM_FAILURE(rv)) {
        OAM_ERR(("BHH(unit %d) Error:hw init. Write CPU_CONTROL_M Reg %s.\n",
                unit, bcm_errmsg(rv)));
        _BCM_OAM_UNLOCK(oc);
        return (rv);
    }
...
/*          soc_cm_print("%s: ****** OAM BHH State ******\n", FUNCTION_NAME());*/
    ...
    if (BCM_FAILURE(rv)) { 
        soc_cm_debug(DK_ERR, "Thread Exit:%s\n", thread_name);  
    }
    sal_thread_exit(0);
}
