...
bcmi_xgs5_udf_ctrl_init(int unit)
{
   ...
   /* Allocating memory of UDF control structure */
    ...
    if soc_feature(unit, soc_feature_flex_hashing) {
        udf_ctrl->flags |= BCMI_XGS5_UDF_CTRL_FLEXHASH;
    }
#endif
                              ...
                              bcmi_xgs5_udf_offset_info_t *offset_info)
{
    ...
    int bcmi_xgs5_udf_offsets_policer[]  = {0, 1};
    ...
    if ((hints != NULL) && (hints->flags & BCM_UDF_CREATE_O_FLEXHASH)) {
        ...
        max_offsets = COUNTOF(bcmi_xgs5_udf_offsets_policer);
        offset_order = &bcmi_xgs5_udf_offsets_policer[0];
    ...
    rv = bcmi_xgs5_udf_offset_reserve(unit, max_chunks, offset_array);
    BCM_IF_ERROR_RETURN(rv);
                             ...
                             int base, int offset)
{
    ...
    for (ix = 0; ix < BCMI_XGS5_UDF_CTRL_MAX_UDF_CHUNKS(unit); ix++) {
        ...
        soc_mem_field32_set(unit, mem, &offset_buf, base_f, base);
        soc_mem_field32_set(unit, mem, &offset_buf, offset_f, offset / gran);
                              ...
                              bcm_udf_pkt_format_info_t *pkt_format)
{
    ...
    if (hints != NULL) {
        ...
        if ((hints->flags & BCM_UDF_CREATE_O_FLEXHASH) &&
            (!(UDF_CTRL(unit)->flags & BCMI_XGS5_UDF_CTRL_FLEXHASH))) {
            return BCM_E_PARAM;
        }
    }
    ...
    bcm_udf_id_t *udf_id)
{
    ...
    rv = bcmi_xgs5_udf_sanitize_inputs(unit, udf_info, hints, NULL);
    BCM_IF_ERROR_RETURN(rv);
    ...
    if (hints != NULL) {
        ...
        if (hints->flags & BCM_UDF_CREATE_O_POLICER_GROUP) {
            offset_info->flags |= BCMI_XGS5_UDF_OFFSET_POLICER;
        }
    }
    ...
    bcm_udf_pkt_format_id_t *pkt_format_id)
{
    ...
    /* Sanitize input parameters */
    rv = bcmi_xgs5_udf_sanitize_inputs(unit, NULL, NULL, pkt_format_info);
    BCM_IF_ERROR_RETURN(rv);
