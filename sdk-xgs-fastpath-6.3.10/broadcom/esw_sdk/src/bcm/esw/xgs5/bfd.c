                               ...
                               bfd_udp_header_t *udp)
{
    ...
    if (v6) {
        ...
        BFD_ENCAP_PACK_U32(cur_ptr, udp->length);
        BFD_ENCAP_PACK_U32(cur_ptr, ipv6->next_header);
    } else {
        /* IPv4 Pseudo-Header
         *     Source address
         *     Destination address
        ...
        BFD_ENCAP_PACK_U16(cur_ptr, ipv4->protocol);
                              ...
                              bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (!endpoint_info->local_discr) {
        soc_cm_debug(DK_ERR, "BFD local discriminator cannot be zero\n");
        return BCM_E_PARAM;
    }
    ...
    if (!endpoint_info->local_detect_mult) {
        ...
        soc_cm_debug(DK_VERBOSE, "BFD Detect Multiplier Set to Default Value:%d\n",
                                       ...
                                       bcm_bfd_auth_simple_password_t *sp)
{
    ...
    if ((sp->length == 0) || 
        ...
        soc_cm_debug(DK_ERR, "BFD simple password length must be from 1 to 16 bytes\n");
        return BCM_E_PARAM;
    }
