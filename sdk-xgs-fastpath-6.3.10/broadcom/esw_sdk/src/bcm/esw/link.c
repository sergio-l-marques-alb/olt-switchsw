/*
 * $Id: link.c,v 1.203.2.5 Broadcom SDK $
...
_bcm_esw_linkscan_update_port(int unit, uint32 flags, int port)
{
        ...
        if (SOC_IS_HURRICANE (unit) || SOC_IS_TD_TT (unit) ||
            ...
            if(soc_feature(unit, soc_feature_eee)) {
                ...
                if (MAC_CONTROL_GET(port_info->p_mac, unit, port,
                    SOC_MAC_CONTROL_EEE_ENABLE, &mac_val) == BCM_E_NONE) {
                    if (new_link && 
                        (bcm_esw_port_ability_remote_get(unit, port, 
                                               &remote_advert) == SOC_E_NONE)) {
                        /* Maintain link up timer. 
                         * EEE WA : delay EEE enable in MAC by 1sec 
                         * once the link is up
                         */
                        soc_cm_debug(DK_LINK | DK_VERBOSE,
   ...
   if (SOC_IS_KATANA2(unit)) {
       soc_cm_debug(DK_LINK, "p=%d Applied DOWNSIZER \n", port);
