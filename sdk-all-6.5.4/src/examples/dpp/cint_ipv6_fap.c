...
int ipv6_host_fec_per_outrif(int unit, int in_port, int out_port){
    ...
    l3host.l3a_modid = 0;
    /* encap_id == 0 for IPv6 host entries in KAPS (Jericho and above) */
    l3host.encap_id = is_device_or_above(unit, JERICHO) ? 0 : encap_id[0];
...
int ipv6_mc_prefix(int unit, int port0, int port1, int port2, int port3, int table_id) {
    ...
    for (prefix_length=128; prefix_length >= 0; prefix_length-=4) {
        ...
        ipv6_and_mask(l3rts_ipv6_mc_traverse[interface_counter].mc_ip6_addr, ip6, l3rts_ipv6_mc_traverse[interface_counter].mc_ip6_mask);
