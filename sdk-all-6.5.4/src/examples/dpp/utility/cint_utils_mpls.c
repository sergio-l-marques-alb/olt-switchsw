...
void init_outer_label(int unit, bcm_mpls_egress_label_t* label_conf,
    ...
    if (mpls_tunnel_properties->force_flags) {
        label_conf->flags = mpls_tunnel_properties->flags;
    } else {
        label_conf->flags |= (BCM_MPLS_EGRESS_LABEL_TTL_SET | BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT | mpls_tunnel_properties->flags);
    }
