...
int public_private_config(int unit, int port1, int port2, int port3, int port4) {
 ...
 int rc, rv;
    ...
    for (i=0; i<num_ports; i++) { 
        ...
        create_l3_egress_s l3eg;
        l3eg.out_gport = egress_port;
        l3eg.next_hop_mac_addr = mac_l3_egress;
        l3eg.out_tunnel_or_rif = ingress_intf[i];
    ...
    for (i=0; i<2; i++) {
        bcm_l3_host_t l3host;
        bcm_l3_host_t_init(&l3host);
        l3host.l3a_ip_addr = host[i];
        l3host.l3a_intf = egress_intf[i];
        rc = bcm_l3_host_add(unit, l3host);
        if (rc != BCM_E_NONE) {
        printf ("bcm_l3_host_add failed: %d \n", rc);
        }
