...
void print_packet_rate32(uint32 counter_result, char *counter_name, int clock_ratio) {
    cli_out("%-40s = %u Kpps", counter_name, (counter_result) * clock_ratio / 1000);
    cli_out(" (ok)\n");
}
...
int gtimer_reg(int unit, char* block_name, soc_reg_t *cfg_reg, soc_reg_t *tri_reg) {
   ...
   else if (!sal_strncasecmp(block_name, "CGM", strlen(block_name))) {
      if (cfg_reg != NULL)
...
int gtimer_enable(int unit, char* block_name, int period_num) {
   ...
   } else if (!sal_strncasecmp(block_name, "IPS", strlen(block_name))) {
...
cmd_dpp_gtimer(int unit, args_t *a) {
   ...
   while (NULL != option) {
      if (!sal_strncasecmp(option, "?", strlen(option))) {
         ...
         if (sal_strcmp(var_name, block_name) != 0) {
            cli_out("block_name was not enabled!\n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
         }
