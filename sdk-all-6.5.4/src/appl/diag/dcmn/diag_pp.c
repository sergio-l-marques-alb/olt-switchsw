    ...
    cmd_ppd_api_diag_lif_show(int unit, args_t* a)
{
    ...
    if (is_ingress) {
        ...
        SOC_PPC_EG_ENCAP_ENTRY_INFO encap_entry_info;
...
{   
	...
	if (0 >= nof_arg) {
	    cli_out(
	            ...
	            "\n\t\t[print][advanced]           - Print out the current configuration of tables and DB."
	...
	if (print_db == TRUE ) {
        arad_kbp_sw_config_print(unit, print_advanced);
	}
