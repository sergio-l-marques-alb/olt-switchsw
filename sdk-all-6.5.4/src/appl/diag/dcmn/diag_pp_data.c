...
static void dpp_attribute_set(rhlist_t *qual_list, char *attribute, uint32 data, uint32 mask, int data_size)
{
    ...
    if(rhlist_add_tail(qual_list, attribute, RHID_TO_BE_GENERATED, &temp) == -SOC_SAND_ERR) {
...
static uint32 dpp_dump_flp_selection(int unit, dpp_export_pp_stage_info_t *stage)
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, READ_IHP_DBG_FLP_PROGRAM_SELECTION_CAM_LINEr(unit, 0, &flp_selects));
    ...
    for(i = 0; i < SOC_DPP_DEFS_GET(unit, nof_flp_program_selection_lines); i++) {
        ...
        if((stage->presels[i].qual_list = rhlist_create("qual_list", sizeof(signal_output_t), 0)) == NULL) {
            continue;
        }
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  44,  exit, ARAD_REG_ACCESS_ERR,WRITE_IHP_DBG_FLP_PROGRAM_SELECTION_CAM_LINEr(unit, 0, flp_selects));
...
static int dpp_dump_flp_programs(int unit,  dpp_export_pp_stage_info_t *stage)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IHP_DBG_FLP_SELECTED_PROGRAMr, 0, 0, DBG_FLP_SELECTED_PROGRAMf, &progs_bmp));
...
static uint32 dpp_dump_vt_selection(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32 res = CMD_OK;
    ARAD_PP_IHP_VTT1ST_KEY_PROGRAM_SELECTION_CAM_TBL_DATA    vt_prog_selection;
    int i;
...
static int dpp_dump_vt_programs(int unit,  dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;
    int i;
    char *program_name;
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_VTT_PROGRAM_ENCOUNTEREDr(unit, 0, &reg_val64));
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_IHP_VTT_PROGRAM_ENCOUNTEREDr(unit, 0, reg_val64));
...
static int dpp_dump_vt(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;

    /* Create FLP sub-sections: Program-Selection */
    res = dpp_dump_vt_selection(unit, stage);

    /* Create FLP sub-sections: Programs */
    res = dpp_dump_vt_programs(unit, stage);
...
static uint32 dpp_dump_tt_selection(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32 res = CMD_OK;
    ARAD_PP_IHP_VTT2ND_KEY_PROGRAM_SELECTION_CAM_TBL_DATA    tt_prog_selection;
    int i;
...
static int dpp_dump_tt_programs(int unit,  dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;
    int i;
    char *program_name;
...
static int dpp_dump_tt(int unit, dpp_export_pp_stage_info_t *stage)
{
    uint32         res = CMD_OK;

    /* Create FLP sub-sections: Program-Selection */
    res = dpp_dump_tt_selection(unit, stage);

    /* Create FLP sub-sections: Programs */
    res = dpp_dump_tt_programs(unit, stage);
...
dpp_export_pp_build_db(int unit, dpp_export_pp_info_t *dpp_export_pp_info)
{
    ...
    dpp_dump_vt(unit, &dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_VT]);

    dpp_dump_tt(unit, &dpp_export_pp_info->stages[SOC_PPC_FP_DATABASE_STAGE_INGRESS_TT]);
...
static cmd_result_t pp_scheme_export(int unit, void *curTop)
{
    ...
    RHDATA_SET_STR(curStage, "name", "IRPP.VT");
    ...
    RHDATA_SET_STR(curStage, "name", "IRPP.TT");
...
cmd_result_t cmd_diag_pp_signals_get(int unit, args_t* a)
{
    ...
    switch(command) {
        ...
        case SIG_COMMAND_STRUCT:
            res = dpp_sigstruct_print(device, match_n, print_flags);
            break;
        case SIG_COMMAND_PARAM:
            res = dpp_sigparam_print(device, match_n, print_flags);
            break;
        case SIG_COMMAND_INTERNAL:
            res = dpp_qual_signal_print(device, &match_m, print_flags);
            break;
