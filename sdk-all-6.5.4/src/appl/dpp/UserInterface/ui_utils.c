/* $Id: ui_utils.c,v 1.10 Broadcom SDK $
...
*DATE: 06/JAN/2002
*FUNCTION:
*  Send a string to the shell (after it has been instructed
*  to wait on this system's special shell pipe).
*CALLING SEQUENCE:
*  send_to_shell(string_to_send,len)
*INPUT:
*  SOC_SAND_DIRECT:
*    string_to_send -
*      char *. To a string which ends with "\n".
*    len -
*      unsigned int. Number of characters in the string, including
*      the ending "\n".
*  SOC_SAND_INDIRECT:
*
*OUTPUT:
  ...
  )
{
  ...
  int
    ...
    int
      shell_task_id ;
  proc_name = "send_to_shell" ;
  /*
   * The next three instructions should be placed at the
   * caller, not in the main loop.
   */
  shell_task_id = taskNameToId("tShell") ;
  shell_stdin = ioTaskStdGet(shell_task_id,0) ;
  shell_stdout = ioTaskStdGet(shell_task_id,1) ;
#if DEBUG_PRINT
#if !defined(__DUNE_GTO_BCM_CPU__)
/* { */
  proc_print("ioTaskStdGet. shell_stdin = %d",proc_name,1,
            shell_stdin,0) ;
  proc_print("ioTaskStdGet. shell_stdout = %d",proc_name,1,
            shell_stdin,0) ;
/* } */
#endif
#endif
  /*
   * Send to shell, character by character.
   */
  for (ui = 0 ; ui < len ; ui++)
  {
    count = d_write(shell_stdin,string_to_send++,1,FALSE) ;
    if (count == ERROR)
    {
      break ;
    }
...
/* { */
  ret =
    d_write(ioGlobalStdGet(STD_OUT),out_string,num_to_send,TRUE) ;
  if (ret != ERROR)
  {
    ret = 0 ;
  }
...
#if defined(__VXWORKS__) && !defined(__DUNE_GTO_BCM_CPU__) && !USE_SIMPLE_PROMPT
/* { */
  static char Prompt_string[20] = "";
/* } */
#endif
/**/
STATIC char*
...
/* { */
  ...
  else
  {
    ...
    if( bsp_card_is_same_family(LINE_CARD_GFA_FAP20V, card_type))
    {
#if LINK_FAP20V_LIBRARIES
      if (is_line_card_ftg_connected())
      {
        sal_sprintf(Prompt_string, "GFA-FTG,%02d>", get_slot_id());
      }
      else if (is_line_card_tgs_connected())
      {
        sal_sprintf(Prompt_string, "GFA-TGS,%02d>", get_slot_id());
      }
      else
      {
        ...
        sal_sprintf(
          Prompt_string,
          "GFA-%s,%02d>",
          get_front_card_type_name(front_card_type),
          get_slot_id()
        );
      }
#endif /*#if LINK_FAP20V_LIBRARIES*/
    }
    else if( bsp_card_is_same_family(LINE_CARD_GFA_FAP21V, card_type))
    {
    ...
    else if(LOAD_BOARD_PB == card_type)
    {
      /* Load Board must be before GFA, 
       * since LINE_CARD_GFA_PETRA_X matches also load board Soc_petra 
       */
      sal_sprintf(Prompt_string, "LoadBoardPetra,%02d>", get_slot_id());
    }
#if LINK_PETRA_LIBRARIES
    else if( bsp_card_is_same_family(card_type,LINE_CARD_GFA_PETRA_X) )
    {
      if (utils_ptg_is_line_card_ptg_connected())
      {
        sal_sprintf(Prompt_string, "GFA-PTG,%02d>", get_slot_id());
      }
      else if (utils_ptg_is_line_card_phy_connected())
      {
        sal_sprintf(Prompt_string, "GFA-PHY,%02d>", get_slot_id());
      }
#if LINK_TIMNA_LIBRARIES || LINK_T20E_LIBRARIES
      else if (tevb_is_timna_connected())
      {
        sal_sprintf(Prompt_string, "GFA-TMN,%02d>", get_slot_id());
      }
#endif
      else
      {
        sal_sprintf(Prompt_string, "GFA-Alone,%02d>", get_slot_id());
      }
    }
#endif
    else if(LINE_CARD_01 == card_type)
    {
      sal_sprintf(Prompt_string, "Line12x1G,%02d>", get_slot_id());
    }
    else if(LINE_CARD_FAP10M_B == card_type)
    {
      sal_sprintf(Prompt_string, "Line_B12x1G,%02d>", get_slot_id());
    }
    else if(FABRIC_CARD_01 == card_type)
    {
      sal_sprintf(Prompt_string, "Fabric-MD,%02d>", get_slot_id());
    }
    else if(LOAD_BOARD_FE600 == card_type)
    {
      sal_sprintf(Prompt_string, "LoadBoardFE600,%02d>", get_slot_id());
    }
    else if(FABRIC_CARD_FE600_01 == card_type)
    {
      sal_sprintf(Prompt_string, "Fe600,%02d>", get_slot_id());
    }
    else
    {
      sal_sprintf(Prompt_string, "Dune,%02d>", get_slot_id());
    }
    return Prompt_string;
  }
/* } */
  ...
  )
{
  ...
  assert(0);
      ...
      set_cli_end_time(time_from_startup) ;
  #endif
#endif
  Cli_input_fd = -1 ; 
