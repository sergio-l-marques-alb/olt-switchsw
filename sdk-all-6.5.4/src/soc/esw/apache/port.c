...
#define AP_FLEX_SPEED_MAX_DEFAULT    42000
...
if (0) {
    ...
    if (speed_max > 42000) {
        ...
        speed_max = AP_FLEX_SPEED_MAX_DEFAULT;
#endif
    }
    ...
    AP_INFO(unit)->port_max = ports_max;

} else {

    AP_INFO(unit)->speed_max = 106000;
 ...
 */
int
soc_ap_port_oversub_get(int unit, int phy_port, soc_port_t logical_port,
                          int *oversub)
{
    soc_pbmp_t pbmp;
    int oversub_mode = 0;
...
_soc_ap_speed_valid(int unit, int phy_port, int lanes, int speed)
{
    ...
    switch (speed) {
        case 1000:
            ...
            break;
        case 2500:
            speed_mask = AP_PHY_PORT_RATE_2_5;
            break;
        case 5000:
            speed_mask = AP_PHY_PORT_RATE_5;
            break;
        case 10000:
        ...
        case 20000:
            ...
            break;
        case 25000:
            speed_mask = AP_PHY_PORT_RATE_25;
            break;
        case 40000:
            speed_mask = AP_PHY_PORT_RATE_40;
            break;
        case 50000:
            speed_mask = AP_PHY_PORT_RATE_50;
            break;
        case 100000:
            speed_mask = AP_PHY_PORT_RATE_100;
            break;
        case 11000:
        ...
        case 21000:
            ...
            break;
        case 27000:
            speed_mask = AP_PHY_PORT_RATE_27;
            break;
        case 42000:
            speed_mask = AP_PHY_PORT_RATE_42;
            break;
        case 53000:
            speed_mask = AP_PHY_PORT_RATE_53;
            break;
        case 106000:
            speed_mask = AP_PHY_PORT_RATE_106;
            break;
        case 127000:
            speed_mask = AP_PHY_PORT_RATE_127;
            break;
        default:
            LOG_ERROR(BSL_LS_SOC_PORT,
                        (BSL_META_U(unit,
...
/*
 * Function:
 *      _soc_ap_port_resource_oversub_validate
 * Purpose:
 ...
 */
STATIC int
_soc_ap_port_resource_oversub_validate(int unit,
                                         int nport,
                                         soc_port_resource_t *resource,
                                         soc_info_misc_t *post_si)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        rv = soc_ap_port_oversub_get(unit, phy_port, pr->logical_port,
                                       &oversub);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_PORT,
                      (BSL_META_U(unit,
                                 ...
                                 soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (!(pr->flags & SOC_PORT_RESOURCE_SPEED)) {
            ...
            SOC_IF_ERROR_RETURN
                (_soc_ap_phy_port_flex_valid(unit, pr->physical_port));

            /* Check lane assignment */
            SOC_IF_ERROR_RETURN
                (_soc_ap_phy_port_lanes_valid(unit, pr->physical_port,
                                              pr->num_lanes));
            /* Check speed (port rate) */
            SOC_IF_ERROR_RETURN
                (_soc_ap_speed_valid(unit, pr->physical_port,
    ...
    /* Check port mappings */
    SOC_IF_ERROR_RETURN
        (_soc_ap_port_mapping_validate(unit, nport, resource));
...
int
soc_ap_port_resources_to_portlist(int unit,
                                  int nport, soc_port_resource_t *resource,
                                  ...
                                  int *post_count, int *post_port_list, int *post_port_flags)
{
    ...
    rv = soc_ap_port_resource_validate(unit, nport, resource);
    if (SOC_FAILURE(rv)) {
...
soc_apache_flexport_scache_sync(int unit)
{
    ...
    var_size = (sizeof(int) * AP_FLEX_MAX_NUM_PORTS);
    alloc_get = alloc_size;
...
soc_apache_flexport_scache_recovery(int unit)
{
    ...
    for (i = 0; i < AP_FLEX_MAX_NUM_PORTS; i++) {
        if (si->port_p2l_mapping[i] == -1) {
            continue;
        }
        SOC_PBMP_PORT_ADD(si->xpipe_pbm, si->port_p2l_mapping[i]);
        SOC_PBMP_PORT_ADD(si->pipe_pbm[0], si->port_p2l_mapping[i]);
    }
...
soc_apache_flexport_sw_dump(int unit) {
    ...
    for (phy_port = 1; phy_port < AP_FLEX_MAX_NUM_PORTS; phy_port++) {
