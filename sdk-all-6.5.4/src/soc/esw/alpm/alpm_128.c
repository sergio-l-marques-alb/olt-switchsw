                                   ...
                                   defip_aux_table_entry_t *shadow, int w_index)
{
    ...
    if (urpf) {
        ...
        tmp = soc_mem_field32_get(u, mem, entry, ALG_HIT_IDXf);
        if (tmp) {
            tmp = _soc_th_alpm_rpf_entry(u, tmp);
            soc_mem_field32_set(u, mem, entry, ALG_HIT_IDXf, tmp);
        }
    }
 ...
 */
int
_soc_th_alpm_128_bucket_move(int u, soc_mem_t mem,
                             int log_bkt_from,int log_bkt_to)
{
...
#define _ALPM_IF_ERROR_GOTO_FREE(_rv) \
    ...
    for (i = 0; i < pa->count; i ++) {
        ...
        if (pfx->index > 0) {
            ...
            rv = soc_mem_read_no_cache(u, 0, mem, MEM_BLOCK_ANY,
                                       pfx->index, bufp);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
    ...
    /* Connect TCAM entry to new bucket */
    rv = _soc_th_alpm_128_bucket_link(u, v6, pivot_index, log_bkt_to);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_th_alpm_128_bucket_link failed, pivot %d bkt %d\n"),
...
_rollback:
    if (SOC_FAILURE(rv)) {
        ...
        rv2 = _soc_th_alpm_128_bucket_link(u, v6, pivot_index, log_bkt_from);
                            ...
                            alpm_mem_prefix_array_t *pa, int *new_index)
{
    ...
    for (i = 0; i < pa->count; i ++) {
        ...
        if (prefix->index >= 0) {
            rv = soc_mem_read_no_cache(u, 0, mem, MEM_BLOCK_ANY, prefix->index,
                            ...
                            uint32 *bpm_len, uint32 *bpm)
{
    ...
    if (tmp_pyld->bkt_ptr) {
        ...
        rv = _soc_th_alpm_128_lpm_ent_init(u, bufp, mem, v6, pi->vrf_id, pi->log_bkt,
        ...
        *bpm_len = ((payload_t *)(tmp_pyld->bkt_ptr))->len;
        ...
        rv = soc_mem_read(u, L3_DEFIP_PAIR_128m, MEM_BLOCK_ANY,
                          soc_th_alpm_logical_idx(u, L3_DEFIP_PAIR_128m,
                          SOC_ALPM_128_DEFIP_TO_PAIR(u, old_tcam_index >> 1), 1),
                          lpm_entry);
        /* Need to reset ALG_HIT_IDX to 0 */
        soc_mem_field32_set(u, L3_DEFIP_PAIR_128m, lpm_entry, ALG_HIT_IDXf, 0);
    }
    ...
    _soc_th_alpm_128_lpm_ent_key_init(u, pivot, length, pi->vrf, v6,
                                      lpm_entry, 0);
...
static int
_soc_th_alpm_128_bucket_split(int u, alpm_pfx_info_t *pi,
                              int *new_tcam_index, int *index_ptr)
{
    ...
    /* Move prefix enteries(from new trie) to new pivot */
    prefix_array.v6_key = L3_DEFIP_MODE_128;
    ...
    if (old_index == NULL) {
        rv = SOC_E_MEMORY;
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   old_index, pi->log_bkt, pi->vrf, v6);
        return rv;
    }
    ...
    *new_tcam_index = soc_th_alpm_physical_idx(u, L3_DEFIP_PAIR_128m,
    ...
    tcam_index = SOC_ALPM_128_ADDR_LWR(u, *new_tcam_index) << 1;
                    ...
                    int tcam_index)  /* return key location */
{
    ...
    if (rv == SOC_E_FULL) {
        ...
        if (PIVOT_BUCKET_COUNT(ALPM_TCAM_PIVOT(u, tcam_index)) < hit) {
            ...
            SOC_IF_ERROR_RETURN(rv);
            rv = _soc_th_alpm_128_bucket_move(u, mem, log_bkt, log_bkt_new);
            SOC_IF_ERROR_RETURN(rv);
            log_bkt = log_bkt_new;
            soc_mem_field32_set(u, mem, alpm_data, SUB_BKT_PTRf,
                                ALPM_SUB_BKT(u, log_bkt));
    ...
    if (split) {
        ...
        pi.vrf_id           = vrf_id;
        pi.vrf              = vrf;
        rv = _soc_th_alpm_128_bucket_split(u, &pi, &new_tcam_index, index_ptr);
        if (rv != SOC_E_NONE) {
           return rv;
        }
        /* New Pivot */
    ...
    if (0) {
        ...
        soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, &aux_entry, DB_TYPEf,
                            db_type + 1);
        soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, &aux_entry, RPEf,
                            default_route ? 1 : 0);
        soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, &aux_entry, ALG_HIT_IDXf,
...
static int
_soc_th_alpm_128_bkt_count(int u, soc_mem_t mem, int log_bkt, int vrf, void *bufp)
{
    int bank_num;
    int entry_num;
    int bank_count;
                    ...
                    int tcam_index, int key_index)  /* TCAM entry data */
{
    ...
    defip_alpm_ipv6_128_entry_t alpmv6_entry, alpmv62;
    defip_aux_scratch_entry_t aux_entry;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    soc_mem_t mem;
    void      *bufp, *bufp2;
...
#ifdef CHECK_CLEAN_BKT
            assert(_soc_th_alpm_128_bkt_count(u, mem, delete_log_bkt, vrf, bufp)
                                                                    == 0);
#endif /* CHECK_CLEAN_BKT */
        }
    }
                                 ...
                                 int do_urpf)
{
    ...
    for (i = 0; i < ent_num; i++) {
        ...
        SOC_IF_ERROR_RETURN(soc_mem_read_no_cache(u, 0, mem, MEM_BLOCK_ANY,
                                                  index, (void *)&bufp));
...
int _lpm_128_fb_entry_shift(int u, int from_ent, int to_ent)
{
    ...
    if (rv >= 0) {
        rv = _lpm_128_fb_urpf_entry_replicate(u, to_ent, e);
    }
    if (rv < 0) {
    ...
    for (lpm_idx = min; lpm_idx <= max; lpm_idx++) {
        ...
        if (!SOC_MEM_OPT_F32_GET_128(u, L3_DEFIP_PAIR_128m, lpm_entry, VALID0_LWRf)) {
            ...
            rv = _soc_mem_alpm_read_cache(u, cmem, SOC_BLOCK_ANY,
                                          lpm_idx, &sw_ent);
            if (SOC_SUCCESS(rv)) {
                rv = soc_mem_read_no_cache(u, 0, cmem, SOC_BLOCK_ANY,
                                           lpm_idx, &hw_ent);
                if (SOC_SUCCESS(rv)) {
