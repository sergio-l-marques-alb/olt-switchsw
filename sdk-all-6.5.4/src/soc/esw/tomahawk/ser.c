...
soc_tomahawk_ser_enable_all(int unit, int enable)
{
    ...
    _soc_th_ser_info_t *ser_info_list;
    ...
    for (rbi = 0; ; rbi++) {
        ...
        /* Assign ser_info_list based on rb->info */
        ser_info_list = rb->info;
        /* For TH+ override ser info definitions for CLPORT, XLPORT */
#ifdef BCM_TOMAHAWKPLUS_SUPPORT
        if (SOC_IS_TOMAHAWKPLUS(unit)) {
            if (SOC_BLK_CLPORT == rb->blocktype) {
                ser_info_list = _soc_thplus_pm_clp_ser_info;
            }
        }
#endif
        rv = _soc_tomahawk_ser_enable_info(unit, block_info_idx, rb->id, port,
                                           rb->enable_reg, &rval64,
                                           ser_info_list, INVALIDm, enable);
