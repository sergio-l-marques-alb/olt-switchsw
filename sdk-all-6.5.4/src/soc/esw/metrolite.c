...
int _ml_config_id[SOC_MAX_NUM_DEVICES] = {-1};
...
int _soc_metrolite_get_cfg_num(int unit, int *new_cfg_num);
extern int
...
int _soc_ml_flexio_scache_sync(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_metrolite_get_cfg_num(unit, &cfg_num));
    _ml_config_id[unit] = cfg_num;
...
int _soc_metrolite_misc_init(int unit)
{
    ...
    if ((soc_feature(unit, soc_feature_linkphy_coe) &&
         ...
         my_modid_list[1] = 1;
         my_modid_valid[1] = 1;
         my_modid_port_base_ptr[1] = SOC_ML_SUBPORT_PP_PORT_INDEX_MIN;
    }
    ...
    si->lp_block_max = SOC_ML_LINKPHY_BLOCK_MAX;
    si->lp_ports_max = SOC_ML_MAX_LINKPHY_PORTS;
    si->lp_streams_per_slice_max = SOC_ML_MAX_STREAMS_PER_SLICE;
...
/* Enable back pressure status from MMU for lossless mode */
    ...
    /* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    for (sub_port = SOC_ML_MIN_SUBPORT_INDEX;
...
void
soc_metrolite_pbmp_init(int unit, ml_pbmp_t ml_pbmp) 
{
    int port;
    soc_pbmp_t          my_pbmp_xport_xe;
    ...
    if (_soc_metrolite_get_cfg_num(unit, &ml_port_details_index) != SOC_E_NONE) {
        LOG_DEBUG(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
...
/* port group will be implemented later */
    ...
    while(ml_selected_port_details[loop_index].port_speed != 0) {
        ...
        for (port =  ml_selected_port_details[loop_index].start_port_no;
            ...
            if((dev_id == BCM56271_DEVICE_ID) && (ml_port_details_index > 2)){
...
void
soc_metrolite_subport_init(int unit)
{
    ...
    available_subport = SOC_ML_MAX_SUBPORTS;
    prev_port = 0;
    si->port_num_subport[prev_port] = 0;
    si->port_subport_base[prev_port] = SOC_ML_MIN_SUBPORT_INDEX;
    ...
    if (SOC_PBMP_NOT_NULL(si->linkphy_allowed_pbm)) {
        ...
        SOC_PBMP_ITER(si->linkphy_allowed_pbm, port) {
            if (port < SOC_MAX_NUM_PORTS) {
                ...
                s1_xlp0_index += SOC_ML_MAX_LINKPHY_STREAMS_PER_PORT;
    ...
    int unit, int port, int blktype, int *block, int *index)
{
  ...
  STATIC int
_soc_metrolite_bond_info_init(int unit)
{
#ifdef BCM_METROLITE_SUPPORT
  if (SOC_IS_METROLITE(unit)) {
      return soc_metrolite_bond_info_init(unit);
  }
#endif
...
soc_functions_t soc_metrolite_drv_funs = {
    ...
    _soc_metrolite_bond_info_init      /* soc_bond_options_init_f */
};
