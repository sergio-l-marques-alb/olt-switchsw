...
uint32 soc_jer_sch_tbls_init(int unit)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "init_tbls_to_zero", !(
        ...
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_FC_MAP_FCMm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_CH_NIF_RATES_CONFIGURATION_CNRCm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_ONE_PORT_NIF_CONFIGURATION_OPNCm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_CH_NIF_CALENDAR_CONFIGURATION_CNCCm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_FLOW_SUB_FLOW_FSFm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_FLOW_DESCRIPTOR_MEMORY_STATIC_FDMSm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
        SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_FLOW_TO_FIP_MAPPING_FFMm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
    }
    ...
    SOCDNX_SAND_IF_ERR_EXIT(dcmn_fill_table_with_entry(unit, SCH_PS_8P_RATES_PSRm, SCH_BLOCK(unit, SOC_CORE_ALL), table_entry));
...
LOG_ERROR(BSL_LS_SOC_DRAM,(BSL_META_U(unit, "mem %s 1\n"), SOC_MEM_NAME(unit, IHP_RECYCLE_COMMANDm)));
    ...
    SOCDNX_SAND_IF_ERR_EXIT(jer_fill_and_mark_memory(unit, IHP_VTT_LLVPm, table_default_vtt));
    SOCDNX_SAND_IF_ERR_EXIT(jer_fill_and_mark_memory(unit, IHP_VSI_LOW_CFG_2m, table_default_vsi));
    SOCDNX_SAND_IF_ERR_EXIT(jer_fill_and_mark_memory(unit, IHP_PINFO_LLRm, table_default_pinfo));
...
static int soc_jerplus_ips_tbls_init(int unit)
{
     ...
     */
    SOCDNX_SAND_IF_ERR_EXIT(jer_fill_and_mark_memory(unit, IPS_FLWIDm, table_entry));

     /*
     *  all devices is set to FMS enabled, EFMS is in bypass mode by default
     */
    soc_mem_field32_set(unit, IPS_FMSBYPm, table_entry, FMS_BYP_BMPf, 0x0000); /*1 bit for each device, 16 devices in row */
    SOCDNX_SAND_IF_ERR_EXIT(jer_fill_and_mark_memory(unit, IPS_FMSBYPm, table_entry));
...
static int soc_jerplus_fdt_tbls_init(int unit)
{
...
int soc_jerplus_tbls_init(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_control_dynamic_mem_writes(unit, jer_dynamic_mem_enable_regs, 0)); /* enable dynamic memory writes */
