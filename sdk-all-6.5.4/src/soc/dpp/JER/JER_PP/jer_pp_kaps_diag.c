...
int jer_kaps_show_db_stats( int unit, uint32 dbal_table_id ) {
    ...
    for ( j = 0; j < 2 ; j++ ) {
        ...
        if ( ( tbl_idx < JER_KAPS_IP_NOF_TABLES ) && ( tbl_idx >= 0 ) ) {
            ...
            if ((table_cfg_p.db_id == JER_KAPS_IP_FWD_PRIVATE_DB_ID) || (table_cfg_p.db_id == JER_KAPS_IP_RPF_PRIVATE_DB_ID))
            {
...
int jer_pp_kaps_tcam_bist(int unit)
{
    ...
    for (blk_id = JER_KAPS_RPB_BLOCK_INDEX_START; blk_id <= JER_KAPS_RPB_BLOCK_INDEX_END; blk_id++) {
        for(cam_index = 0; cam_index < 2; cam_index++) {
            ...
            data[3] = cam_index + 1;
            res = jer_pp_kaps_write_command(jer_kaps_kaps_xpt_p_get(unit),
                                            blk_id,
                                            KAPS_CMD_WRITE,
                                            KAPS_FUNC0,
            ...
            res = jer_pp_kaps_read_command(jer_kaps_kaps_xpt_p_get(unit),
                                           blk_id,
                                           KAPS_CMD_READ,
                                           KAPS_FUNC0,
...
int jer_pp_kbp_sdk_ver_test(int unit)
{
    ...
    const char* kbp_sdk_ver_arr[] =       {"KBP SDK 1.3.11", "KBP SDK 1.4.4"};
    const uint32 sw_sdk_ver_arr_6_5[]  = {0x65100000, 0x65300000};
    const uint32 sw_sdk_ver_arr_6_4[]  = {0x64700000, 0x64b00000};
