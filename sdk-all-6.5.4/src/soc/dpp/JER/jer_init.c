...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
uint32 jer_kbp_init_jer_interface(unit)
{
    uint32 offset;
...
{
    soc_reg_above_64_val_t reg_above_64_val;
    uint32 nof_pipes, is_dual_fap, is_dram, reg_val, is_pcp;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    SOCDNX_IF_ERR_EXIT(READ_IPT_PACKET_LATENCY_MEASURE_CFGr(unit, reg_above_64_val));
    soc_reg_above_64_field32_set(unit, IPT_PACKET_LATENCY_MEASURE_CFGr, reg_above_64_val, EN_LATENCY_DROPf, 0);
    soc_reg_above_64_field32_set(unit, IPT_PACKET_LATENCY_MEASURE_CFGr, reg_above_64_val, EN_LATENCY_ERR_REPORTf, 0);
    SOCDNX_IF_ERR_EXIT(WRITE_IPT_PACKET_LATENCY_MEASURE_CFGr(unit, reg_above_64_val));
    ...
    if (!SOC_WARM_BOOT(unit)) {
...
#endif
       {
           ...
           if (res) {
               ...
               SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_soc_bist_all,(unit, (res == 2 ? 1 : 0))));
           } else if (SOC_IS_QAX_A0(unit) || SOC_IS_KALIA(unit)) {
