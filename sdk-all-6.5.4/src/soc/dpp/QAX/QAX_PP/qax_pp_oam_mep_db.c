...
#define QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_OPCODE_MASK       (SOC_SAND_BITS_MASK(0, QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_OPCODE_SIZE - 1))
...
#define QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_OAM_BFD_MASK      (SOC_SAND_BITS_MASK(0, QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_OAM_BFD_SIZE - 1))
...
#define QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_PE_PROFILE_MASK  (SOC_SAND_BITS_MASK(0, QAX_PP_OAMP_CLS_FLEX_CRC_TCAM_PE_PROFILE_SIZE - 1))
...
int _arad_pp_oam_internal_no_cache_read_oamp_mep_DBm(int unit, uint32 flags, int blk, uint32 index, void *data) {
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        rv = soc_mem_read_no_cache(unit, flags, OAMP_MEP_DBm, blk, index, data);
    }
    return rv;
}
...
qax_pp_oam_bfd_flexible_verification_set(int unit, SOC_PPC_OAM_BFD_FLEXIBLE_VERIFICATION_INFO *info) {
    ...
    soc_reg_above_64_val_t reg_data;
    uint32 crc_lsb_mask[SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE / 4], flex_crc_tcam_key;
    int byte;
    ...
    if (info->mep_idx != -1) {
        ...
        soc_OAMP_MEP_DB_EXT_DATA_HDRm_field32_set(unit, reg_data, CHECK_CRC_VALUE_1f /* Currently only 1 crc value is supported */, info->crc_info.crc16_val);
        soc_OAMP_MEP_DB_EXT_DATA_HDRm_field32_set(unit, reg_data, CHECK_CRC_VALUE_2f /* This field is used as "in use" indication */, 1);
    ...
    if (info->mask_tbl_index != -1) {
        ...
        sal_memset(crc_lsb_mask, 0, sizeof(crc_lsb_mask));

        /* Convert the mask from uint8 array to a uint32 array (to avoid big/little endian bugs). */
        for (byte = 0; byte < SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE; byte++) {
            crc_lsb_mask[byte / 4] += info->crc_info.mask.msb_mask[byte] << (byte % 4);
        }

        /* Write the fields. */
        soc_mem_field_set(unit, OAMP_FLEX_VER_MASK_TEMPm, reg_data, PER_BIT_MASKf, crc_lsb_mask);
    ...
    if (info->crc_tcam_index != -1) {
        ...
        soc_mem_field32_set(unit, OAMP_CLS_FLEX_CRC_TCAMm, reg_data, MASKf, 0/* Currently all bits are used */);

        /* The data is built of {CRCVal1/2(1),Mask_Sel(3)}. We always use CRCval1 (0x0), so we can just write the mask. */
        soc_mem_field32_set(unit, OAMP_CLS_FLEX_CRC_TCAMm, reg_data, DATf, info->crc_tcam_info.mask_tbl_index);
...
qax_pp_oam_bfd_flexible_verification_get(int unit, SOC_PPC_OAM_BFD_FLEXIBLE_VERIFICATION_INFO *info) {
    ...
    crc_is_in_use = soc_OAMP_MEP_DB_EXT_DATA_HDRm_field32_get(unit, reg_data, CHECK_CRC_VALUE_2f /* This field is used as "in use" indication */);
    ...
    info->crc_info.crc16_val = soc_OAMP_MEP_DB_EXT_DATA_HDRm_field32_get(unit, reg_data, CHECK_CRC_VALUE_1f /* Currently only 1 crc value is supported */);
    ...
    if (soc_mem_field32_get(unit, OAMP_CLS_FLEX_CRC_TCAMm, reg_data, VALIDf)) {
        ...
        /* The data is built of {CRCVal1/2(1),Mask_Sel(3)}. We always use CRCval1 (0x0), so we can just return the mask. */
        info->crc_tcam_info.mask_tbl_index = soc_mem_field32_get(unit, OAMP_CLS_FLEX_CRC_TCAMm, reg_data, DATf);
    ...
    /* Write the fields. */
    soc_mem_field_get(unit, OAMP_FLEX_VER_MASK_TEMPm, reg_data, PER_BIT_MASKf, crc_lsb_mask);
    ...
    for (byte = 0; byte < SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE; byte++) {
        info->crc_info.mask.msb_mask[byte] = (crc_lsb_mask[byte / 4] >> (byte % 4)) & 0xff;
    }
...
qax_pp_oam_bfd_flexible_verification_delete(int unit, SOC_PPC_OAM_BFD_FLEXIBLE_VERIFICATION_INFO *info) {
    ...
    if (info->mep_idx != -1) {
        ...
        if ((entry_type != SOC_PPC_OAM_MEP_TYPE_EXT_DATA_HDR)
            || (!soc_OAMP_MEP_DB_EXT_DATA_HDRm_field32_get(unit, reg_data, CHECK_CRC_VALUE_2f /* This field is used as "in use" indication */))) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("CRC is not used in this entry.")));
        }
...
qax_pp_oam_bfd_mep_db_ext_data_set(int unit, SOC_PPC_OAM_BFD_MEP_DB_EXT_DATA_INFO *info) {
    ...
    int bank, entry_idx, remaining_bits;
    ...
    if (info->mep_idx > ARAD_PP_OAM_OAMP_MEP_DB_MEP_ENTRIES_NOF(unit)) {
    ...
    if (info->data_size_in_bits > 0) {
        ...
        SHR_BITCOPY_RANGE(buf, 0, info->data, 0, SOC_PPC_OAM_BFD_MEP_DB_EXT_DATA_FIRST_HDR_SIZE_BITS);
         ...
         */
        for (remaining_bits = extra_bits, current_entry = 0 ;
                remaining_bits > 0 ;
            ...
            SHR_BITCOPY_RANGE(buf,
                              ...
                              SOC_SAND_MIN(remaining_bits, SOC_PPC_OAM_BFD_MEP_DB_EXT_DATA_EXTRA_ENTRY_SIZE_BITS)
                              );
...
qax_pp_oam_bfd_mep_db_ext_data_delete(int unit, int extension_idx){
    ...
    nof_extra_entries   = soc_mem_field32_get(unit, OAMP_MEP_DB_EXT_DATA_HDRm, reg_data, EXT_DATA_LENGTHf) - 1 /*Not counting the header entry */;
    crc_in_use          =  soc_mem_field32_get(unit, OAMP_MEP_DB_EXT_DATA_HDRm, reg_data, CHECK_CRC_VALUE_2f);
