...
)
{
  ...
  DPP_MC_ASSERT(mcds->nof_unoccupied <= QAX_LAST_MCDB_ENTRY);
}
...
)
{
  ...
  dst_entry->word2 |= src_entry->word1 & ~QAX_MCDS_LAST_WORD_KEEP_BITS_MASK;
...
STATIC void _qax_mcds_remove_free_entries_block_from_list(qax_mcds_t *mcds, dpp_free_entries_block_list_t *list, uint32 block, const dpp_free_entries_block_size_t block_size)
{
  ...
  DPP_MC_ASSERT(block <= QAX_LAST_MCDB_ENTRY && block > 0);
...
{
  DPP_MC_ASSERT(mcds && mcdb_index > 0 && mcdb_index <= QAX_LAST_MCDB_ENTRY);
  ...
  qax_mcds_is_empty_free_entries_block_list(const qax_mcds_t *mcds, const dpp_free_entries_block_list_t *list)
{
  if (list->first == DPP_MC_FREE_ENTRIES_BLOCK_LIST_EMPTY)
    return 1;
  DPP_MC_ASSERT(list->first <= QAX_LAST_MCDB_ENTRY);
  return 0;
}
...
uint32 qax_mcds_get_free_entries_block_from_list(qax_mcds_t *mcds, dpp_free_entries_block_list_t *list, int to_remove)
{
  ...
  if (block == DPP_MC_FREE_ENTRIES_BLOCK_LIST_EMPTY)
    return 0;
  DPP_MC_ASSERT(block <= QAX_LAST_MCDB_ENTRY);
  DPP_MC_ASSERT(QAX_MCDS_GET_TYPE(mcds, block) == DPP_MCDS_TYPE_VALUE_FREE_BLOCK_START);
...
)
{
    ...
    table_size = QAX_LAST_MCDB_ENTRY + 1;
    ...
    if (end < start)  { /* if the region should be empty */
        start = QAX_LAST_MCDB_ENTRY + 1;
        end = QAX_LAST_MCDB_ENTRY;
    }
...
)
{
    ...
    if (use_dma) { /* use DMA */
        ...
        SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, TAR_MCDBm, MEM_BLOCK_ANY, 0, QAX_LAST_MCDB_ENTRY, alloced_mem));
        ...
        if (do_not_read) { /* if not in warm boot, the MCDB and IRDB tables that we have just filled */
            for (i = QAX_LAST_MCDB_ENTRY + 1 ; i ; --i) {
                *dest = mcds->free_value;
                ++dest;
            }
        } else {
            SOCDNX_IF_ERR_EXIT(soc_mem_read_range(unit, TAR_MCDBm, MEM_BLOCK_ANY, 0, QAX_LAST_MCDB_ENTRY, dest));
            for (i = table_size; i; --i) {
 ...
 */
STATIC uint32 qax_mcds_get_replications_from_entry(
    SOC_SAND_IN    int     unit,
...
)
{
    ...
    if (DPP_MCDS_TYPE_IS_INGRESS(entry_type)) { /* get replications and next entry from an ingress format */
        ...
        switch (format) { /* select memory format based on the format field */
          case 0:
          	if (!SOC_IS_QUX(unit)) {
            temp = soc_mem_field32_get(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, BMP_PTRf);
            ...
            *next_entry = soc_mem_field32_get(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, LINK_PTRf);
            }
            else {
            	/* workaoundfor QUX only */
            	*next_entry = 131071;
            }
            break;
 ...
 */
STATIC void qax_mcds_clear_replications(qax_mcds_t *mcds, const uint32 group_type)
{
...
)
{
    ...
    if (do_clear) {
        qax_mcds_clear_replications(mcds, DPP_MCDS_TYPE_VALUE_EGRESS);
    }
 ...
 */
uint32
  qax_mcds_copy_replications_from_egress_block( 
    SOC_SAND_IN  int                           unit,
    SOC_SAND_IN  uint8                         do_clear,    /* if zero, replications will be added in addition to existing ones, otherwise previous replications will be cleared */
...
)
{
    ...
    if (DPP_MCDS_TYPE_IS_USED(group_entry_type) && !DPP_MCDS_TYPE_IS_START(group_entry_type)) {
        if (group_entry_type == DPP_MCDS_TYPE_VALUE_INGRESS) { /* egress groups do not have used consecutive blocks */
            ...
            for (entry = mcdb_index; qax_mcds_is_consecutive_format(mcds, next_entry); entry = next_entry) {
                next_entry = entry + 1;
                ++size;
...
)
{
  ...
  for (mcid = 0; mcid <= QAX_LAST_MCDB_ENTRY; ++mcid) {
    ...
    *does_exist = FALSE;
    ...
    if (!is_egress) { /* ingress group */
        if (QAX_MCDS_GET_TYPE(mcds, mcid) == DPP_MCDS_TYPE_VALUE_INGRESS_START) {
            DPP_MC_ASSERT(mcid == QAX_MCDS_GET_PREV_ENTRY(mcds, mcid)); /* verify that mcid is a start of a linked list */
            *does_exist = TRUE;
        }
    } else { /* egress group */
...
)
{
    ...
    if (found_block_size == size) { /* can relocate all the block to the allocated block */
        ...
        free_alloced_block = 0;
        SOCDNX_IF_ERR_EXIT(qax_mcds_copy_replications_from_egress_block( /* set mcdb with the block's entries */
            unit, TRUE, start, size, &cud2, &list_next));

        SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list( /* replace block by a linked list */
            unit, DPP_MCDS_TYPE_VALUE_INGRESS, prev_entry, list_next, found_block_start, found_block_size));
...
)
{
    ...
    *group_size = 0;
    ...
    if (do_clear) {
      qax_mcds_clear_replications(mcds, group_type); /* reset the returned replications to none */
    }
...
{
    qax_mcds_t *mcds = dpp_get_mcds(unit);
    const uint32 entry_index = QAX_MCDS_GET_EGRESS_GROUP_START(mcds, group_mcid); /* table index of the first entry in the group */
    ...
    const dpp_mc_replication_t *inp_rep = reps;
    dpp_mc_replication_t *rep, stack_reps[10], *reps_to_remove = stack_reps, *dest_reps, *bm_reps;
    SOCDNX_INIT_FUNC_DEFS;

    DPP_MC_ASSERT(DPP_MCDS_TYPE_VALUE_EGRESS_START == QAX_MCDS_ENTRY_GET_TYPE(mcdb_entry)); /* type should exactly match entry */
    if (nof_reps > 10) { /* if stack reps is too small for the input, allocate memory for input replications */
        reps_to_remove = NULL;
        SOCDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &reps_to_remove, sizeof(*reps_to_remove) * nof_reps, "dppreps2remove"));
    }
    ...
    for (i = 0; i < nof_reps; ++i, ++inp_rep) {
        if (inp_rep->dest & ARAD_MC_EGR_IS_BITMAP_BIT) { /* bitmap + outlif replication */
            *--bm_reps = *inp_rep;
            bm_reps->dest &= ~ARAD_MC_EGR_IS_BITMAP_BIT;
        } else {
            *(dest_reps++) = *inp_rep;
        }
    }
    ...
    DPP_MC_ASSERT(dest_reps == bm_reps && nof_reps > nof_dest_reps);
    ...
    if (rep->dest == _SHR_GPORT_INVALID) { /*outlif only, or port+outlif replication */
        ...
        if (rep->cud == DPP_MC_EGR_OUTLIF_DISABLED) { /* Invalid value, used to mark no replication in hardware */
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("invalid replication to delete")));
        }
    } else if (rep->dest & ARAD_MC_EGR_IS_BITMAP_BIT) { /* bitmap + outlif replication */
        bitmap = rep->dest & ~ARAD_MC_EGR_IS_BITMAP_BIT;
        ...
        if (rep->dest == DPP_MULT_EGRESS_PORT_INVALID) { /* Invalid value, used to mark no replication in hardware */
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("invalid replication to delete")));
        }
    }
    ...
    for (; entries_left; --entries_left) {
        ...
        switch (format = soc_mem_field32_get(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, ENTRY_FORMATf)) {
          ...
          case 0:
            ...
            if (is_outlif_cud_replication && soc_mem_field32_get(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, OUTLIF_1f) == rep->cud) {
                ...
                if (rep->dest == port) {
                    found = 1;
                    soc_mem_field32_set(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, PP_DSP_1Af, DPP_MULT_EGRESS_PORT_INVALID); /* remove replication */
                    if (soc_mem_field32_get(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, PP_DSP_1Bf) == ARAD_MULT_EGRESS_SMALL_PORT_INVALID) {
                        remove_entry = 1;
                    }
                } else if (rep->dest < ARAD_MULT_EGRESS_SMALL_PORT_INVALID && rep->dest == soc_mem_field32_get(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, PP_DSP_1Bf)) {
                    found = 1;
                    soc_mem_field32_set(unit, TAR_MCDB_EGRESS_BITMAP_POINTER_FORMATm, mcdb_entry, PP_DSP_1Bf, ARAD_MULT_EGRESS_SMALL_PORT_INVALID); /* remove replication */
          ...
          case 1:
            ...
            if (is_bitmap_cud_replication && bitmap == soc_mem_field32_get(unit, TAR_MCDB_EGRESS_DOUBLE_CUD_FORMATm, mcdb_entry, BMP_PTRf) && 
                ...
                soc_mem_field32_set(unit, TAR_MCDB_EGRESS_DOUBLE_CUD_FORMATm, mcdb_entry, BMP_PTRf, DPP_MC_EGR_BITMAP_DISABLED); /* remove replication */ 
            ...
            case 3:
              ...
              if (is_outlif_only_replication) {
                  ...
                  if (outlif1 == rep->cud) {
                      found = 1;
                      soc_mem_field32_set(unit, TAR_MCDBm, mcdb_entry, OUTLIF_1f, DPP_MC_EGR_OUTLIF_DISABLED); /* remove replication */
                      if (outlif2 == DPP_MC_EGR_OUTLIF_DISABLED) { /* replication not disabled for this entry part */
                          remove_entry = 1;
                      }
                  } else if (outlif2 == rep->cud) {
                      found = 1;
                      soc_mem_field32_set(unit, TAR_MCDBm, mcdb_entry, OUTLIF_2f, DPP_MC_EGR_OUTLIF_DISABLED); /* remove replication */
          ...
          case 5:
            if (is_outlif_cud_replication) {
                ...
                if (port1 == rep->dest && soc_mem_field32_get(unit, TAR_MCDB_EGRESS_DOUBLE_CUD_FORMATm, mcdb_entry, OUTLIF_1f) == rep->cud) {
                    found = 1;
                    soc_mem_field32_set(unit, TAR_MCDB_EGRESS_DOUBLE_CUD_FORMATm, mcdb_entry, PP_DSP_1f, DPP_MULT_EGRESS_PORT_INVALID); /* remove replication */
                    ...
                    found = 1;
                    soc_mem_field32_set(unit, TAR_MCDB_EGRESS_DOUBLE_CUD_FORMATm, mcdb_entry, PP_DSP_2f, DPP_MULT_EGRESS_PORT_INVALID); /* remove replication */
          ...
          default: /* formats 6,7 */
            if (is_outlif_only_replication) {
                ...
                if (outlif1 == rep->cud) {
                    ...
                    found = 1;
                    soc_mem_field32_set(unit, TAR_MCDBm, mcdb_entry, OUTLIF_3f, DPP_MC_EGR_OUTLIF_DISABLED); /* remove replication */
        ...
        if (format >= 4) {
            if (format & 1) {
                ...
                DPP_MC_ASSERT(next_index < mcds->ingress_bitmap_start);
            } else {
        ...
        DPP_MC_ASSERT(DPP_MCDS_TYPE_VALUE_EGRESS == QAX_MCDS_ENTRY_GET_TYPE(mcdb_entry)); /* type should exactly match entry */
    }
    ...
    if (remove_entry) { /* If we want to remove the entry since no replication was left in it */
        ...
        if (cur_index == entry_index) { /* the first entry in the group needs to be removed */
            ...
            if (next_index == DPP_MC_EGRESS_LINK_PTR_END) { /* this is the only entry in the group, just update it */
                ...
                qax_self_replication_set(unit, group_mcid , FALSE);
            ...
            qax_mcdb_entry_t *prev_entry = QAX_GET_MCDB_ENTRY(mcds, prev_index);
        ...
        if (try_to_copy_the_next_entry) { /* If the next entry is the end of the group or has a pointer, we can copy it on top of this entry */
            qax_mcdb_entry_t *next_entry = QAX_GET_MCDB_ENTRY(mcds, next_index);
            uint32 next_format = soc_mem_field32_get(unit, TAR_MCDBm, next_entry, ENTRY_FORMATf); 
            DPP_MC_ASSERT(QAX_MCDS_ENTRY_GET_TYPE(next_entry) == DPP_MCDS_TYPE_VALUE_EGRESS);
            if (next_format < 4 || !(next_format & 1)) { /* If the next entry has a pointer or is the last entry in the group */
                ...
                mcdb_entry->word1 &= ~QAX_MC_ENTRY_MASK_VAL;
                mcdb_entry->word1 |= (next_entry->word1 & QAX_MC_ENTRY_MASK_VAL);
...
#if 0 /* QAX implementation */
/*
 * Read a bitmap from the egress MC bitmap table.
 */
uint32 qax_egq_vlan_table_tbl_get(
    SOC_SAND_IN   int    unit,
 ...
 */
uint32 qax_egq_vlan_table_tbl_set(
    SOC_SAND_IN   int     unit,
    SOC_SAND_IN   uint32  entry_offset,
...
/* Exchange the contents of two qax_mcds_t structs */
STATIC INLINE void dpp_exchange_qax_rep_data_t(qax_rep_data_t *a, qax_rep_data_t *b)
{
    qax_rep_data_t temp = *a;
    *a = *b;
    *b = temp;
}
#endif /* 0 QAX implementation */
...
)
{
    ...
    if (mcds->linked_list != QAX_MC_INGRESS_LINK_PTR_END) { /* is a pointer entry needed at the end of the block */
        ...
        if (rep_type == QAX_MCDS_REP_TYPE_DEST) {
            ...
            soc_mem_field32_set(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, CUD_0f, cud1); /* set the 1st CUD */
            soc_mem_field32_set(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, CUD_1f, cud2); /* set the 2nd CUD */
            soc_mem_field32_set(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, BMP_PTRf, /* set the bitmap pointer */
              mcds->ingress_bitmap_start + dest * SOC_DPP_CONFIG(unit)->qax->nof_ingress_bitmap_bytes);
            soc_mem_field32_set(unit, TAR_MCDB_BITMAP_REPLICATION_ENTRYm, mcdb_entry, FORMATf, 0); /* set the entry format */
        }
        /* set the (hardware) forward link from the current entry to the next one in the list, which is the previous added entry */
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mc_end_linked_list(unit)); /* Finish writing the linked list, activating it in one write */
...
exit:
    if (mcds->state == QAX_MCDS_STATE_STARTED) {  /* free the linked list written so far on error or when running out of mcdb entries */
        ...
        qax_mcdb_free_linked_list_till_my_end(unit, mcds->linked_list, DPP_MCDS_TYPE_VALUE_EGRESS, list_end);
    } else {
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list( /* build the group, including the first entry */
      unit, DPP_MCDS_TYPE_VALUE_EGRESS_START, mcid, DPP_MC_EGRESS_LINK_PTR_END, 0, 0));
    if (mcds->out_err) { /* If failed due to lack of memories, do the same */
        SOC_EXIT;
    }
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list(unit, DPP_MCDS_TYPE_VALUE_EGRESS_START, group_mcid,
      DPP_MC_EGRESS_LINK_PTR_END, 0, 0));
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mult_remove_replications(unit, nof_reps, reps));
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list( /* build the group, including the first entry */
        unit, DPP_MCDS_TYPE_VALUE_EGRESS_START, group_mcid, DPP_MC_EGRESS_LINK_PTR_END, 0, 0));
    if (mcds->out_err) { /* If failed due to lack of free entries */
        ...
        SOCDNX_IF_ERR_EXIT(qax_mcds_remove_replications_from_group(unit, group_mcid, nof_reps, reps));
        mcds->out_err = _SHR_E_NONE;
 ...
 */
uint32 qax_mult_eg_bitmap_group_zero(
    SOC_SAND_IN  int         unit,
...
{
    SOCDNX_INIT_FUNC_DEFS;
    SOCDNX_IF_ERR_EXIT(qax_mult_eg_bitmap_group_zero(unit, multicast_id_ndx, 1));
exit:
    SOCDNX_FUNC_RETURN;
}
...
)
{
    SOCDNX_INIT_FUNC_DEFS;
    SOCDNX_NULL_CHECK(group);
    if (multicast_id_ndx >= QAX_MC_EGR_NOF_BITMAPS) {
...
{
    SOCDNX_INIT_FUNC_DEFS;
    SOCDNX_IF_ERR_EXIT(qax_mult_eg_bitmap_group_zero(unit, multicast_id_ndx, 0));
exit:
    SOCDNX_FUNC_RETURN;
}
...
*********************************************************************/
uint32 qax_mult_eg_bitmap_local_port_2_bitmap_bit_index(
    SOC_SAND_IN  int                 unit,
...
/*********************************************************************
*   Add port replications to an Egress-Multicast bitmap group. - replaced by qax_mult_eg_bitmap_group_bm_add
*********************************************************************/
uint32 qax_mult_eg_bitmap_group_port_add(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mult_eg_bitmap_local_port_2_bitmap_bit_index(unit, port, &bit_index));
...
*********************************************************************/
uint32 qax_mult_eg_bitmap_group_bm_add(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
...
/*********************************************************************
*   Removes a port replication of an egress multicast bitmap group. - replaced by qax_mult_eg_bitmap_group_bm_remove
*********************************************************************/
uint32 qax_mult_eg_bitmap_group_port_remove(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
...
*   Add port replications from a bitmap to an Egress-Multicast bitmap group.
*********************************************************************/
uint32 qax_mult_eg_bitmap_group_bm_remove(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
...
)
{
    ...
    if (do_clear) {
        qax_mcds_clear_replications(mcds, DPP_MCDS_TYPE_VALUE_INGRESS);
    }
    ...
    for (; size_left; --size_left, ++reps) {
        cud1 = reps->encap1 ? reps->encap1 : SOC_DPP_CONFIG(unit)->tm.ingress_mc_max_cud;
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list(unit, DPP_MCDS_TYPE_VALUE_INGRESS_START, mcid, QAX_MC_INGRESS_LINK_PTR_END, 0, 0));
    if (mcds->out_err) { /* If failed due to lack of memories, do the same */
        SOC_EXIT;
    }
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list(unit, DPP_MCDS_TYPE_VALUE_INGRESS_START, mcid,
      QAX_MC_INGRESS_LINK_PTR_END, 0, 0));
    if (mcds->out_err) { /* If failed, do some error code fixing */
        if (out_err != NULL) {
            if (mcds->out_err == _SHR_E_PARAM) {
...
)
{
    ...
    /* remove the given replication from the mcds */
    SOCDNX_IF_ERR_EXIT(qax_mult_remove_replications(unit, nof_reps, dpp_reps));
    if (mcds->out_err) { /* If the replication was not found, exit */
        SOC_EXIT;
    }
    ...
    SOCDNX_IF_ERR_EXIT(qax_mcds_set_linked_list( /* build the group, including the first entry */
        unit, DPP_MCDS_TYPE_VALUE_INGRESS_START, mcid, QAX_MC_INGRESS_LINK_PTR_END, 0, 0));
    if (mcds->out_err) { /* If failed due to lack of free entries */
#ifndef QAX_EGRESS_MC_DELETE_FAILS_ON_FULL_MCDB
        /* If we can not reconstruct the group due to MCDB being full, just remove the replication/entry */
        SOCDNX_IF_ERR_EXIT(qax_mcds_remove_replications_from_group(unit, mcid, nof_reps, dpp_reps)); 
        mcds->out_err = _SHR_E_NONE;
#endif /* ARAD_EGRESS_MC_DELETE_FAILS_ON_FULL_MCDB */
    }
    if (mcds->out_err == _SHR_E_NONE && old_group_entries != QAX_MC_INGRESS_LINK_PTR_END) {
        /* free previous linked list of the group not used any more */
        SOCDNX_IF_ERR_EXIT(qax_mcdb_free_linked_list(unit, old_group_entries, DPP_MCDS_TYPE_VALUE_INGRESS));
    }
