...
STATIC uint8
	arad_kbp_result_sizes_is_default(int unit, int *result_actual_sizes)
{
	int i;
	int *result_default_size; 
    ...
    arad_kbp_result_sizes_configurations_init(int unit, ARAD_KBP_FRWRD_IP_DB_TYPE db_type, int results[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS])
{
    ...
    if (db_type > ARAD_KBP_FRWRD_DB_NOF_TYPES || db_type == ARAD_KBP_FRWRD_DB_TYPE_IPV4_DC) {
        /* IPV4 DC was configured at init - don't need to update it*/
		ARAD_DO_NOTHING_AND_EXIT;
    }

	is_default = arad_kbp_result_sizes_is_default(unit, results_size);
	if (is_default) {
		ARAD_DO_NOTHING_AND_EXIT;
	}
	...
	if (ARAD_KBP_ENABLE_IPV4_DC) {
		results_size[2] += ARAD_KBP_IPV4DC_RES2_PAD_BYTES;
   ...
   )
{
	...
	switch (search_id) {
        ...
        case 2:
			...
			if (db_type == ARAD_KBP_FRWRD_DB_TYPE_IPV4_DC) {
				lut_info.result2_idx_ad_cfg -= ARAD_KBP_IPV4DC_RES2_PAD_BYTES;
			}
            break;
        case 3:
	...
	if (db_type == ARAD_KBP_FRWRD_DB_TYPE_IPV4_DC) {
		if (search_id == 2) {
			ltr_db_info.res_data_len[search_id] -= ARAD_KBP_IPV4DC_RES2_PAD_BYTES;
		}
		if (search_id == 3) {
