     ...
     )
{
    ...
    if (SOC_IS_ARADPLUS(unit) && SOC_IS_JERICHO_AND_BELOW(unit))  {
        /* In ARADPLUS and jericho, the "empty" lif is a lif with the ISID format MSBs (since ISID lifs are not used) with the AC "use as data"
         bits enabled for both half entries (In case half the lif becomes an AC lif, the other lif shouldn't be used yet). */
        soc_mem_field32_set(unit, EDB_AC_HALF_ENTRY_FORMATm, table_entry, AC_0_ACTION_USE_AS_DATA_BITf, 0);
        soc_mem_field32_set(unit, EDB_AC_HALF_ENTRY_FORMATm, table_entry, AC_1_ACTION_USE_AS_DATA_BITf, 1);
...
uint32 
arad_pp_eg_encap_access_entry_location_info_get(int unit, int outlif, soc_mem_t *mem, uint32 *bank_id, uint32 *internal_offset){
    uint32 entry_offset;
    ...
    is_top_bank_entry = entry_offset >= ARAD_PP_EG_ENCAP_ACCESS_NOF_EEDB_ENTRIES(unit);
  ...
  )
{
  ...
  push_command_upper_limit_val = SOC_IS_JERICHO_B0_AND_ABOVE(unit) ? ARAD_PP_EG_ENCAP_MPLS1_COMMAND_PUSH_QAX_UPPER_LIMIT_val : ARAD_PP_EG_ENCAP_MPLS1_COMMAND_PUSH_UPPER_LIMIT_val;
  ...
  )
{
  ...
  tbl_data->ipv4_tos_index = soc_mem_field32_get(unit, mem, data, IP_TUNNEL_ACTION_IPV4_TTL_INDEXf);

  
  tbl_data->ipv4_ttl_index = soc_mem_field32_get(unit, mem, data, IP_TUNNEL_ACTION_IPV4_TOS_INDEXf);
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
    mem = EDB_TRILL_FORMAT_HALF_ENTRYm;
  } else {
  ...
  if (SOC_IS_JERICHO(unit)) {
    tbl_data->outlif_profile = soc_mem_field32_get(unit, mem, data, TRILL_0_ACTION_OUTLIF_PROFILEf); 
  } 
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
    soc_mem_field32_set(unit, mem, data, TRILL_0_ACTION_OUTLIF_PROFILEf, tbl_data->outlif_profile);
  }
