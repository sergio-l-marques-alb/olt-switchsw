...
soc_dpp_init(int unit, int reset)
{
    ...
    ha_init(unit, 1 /* HA enabled */, "HA", stable_size, SOC_WARM_BOOT(unit)? 0 : 1);
#else
    ha_init(unit, 0 /* HA disabled */, "HA", stable_size, 0);
#endif
#endif
...
/* } */
    ...
    if (soc_property_get(unit, spn_HA_HW_JOURNAL_MODE, HW_LOG_MODE_Disabled) != HW_LOG_MODE_Disabled) {
