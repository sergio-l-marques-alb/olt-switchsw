...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm4x25", NULL, VERSION(5), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
                ...
                if( PM_4x25_INFO(pm_info)->nof_phys[temp] >= 2 ){
                    ...
                    for(i=0 ; i<nof_ext_phys ; i++) {
                        ...
                        if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                            ...
                            if ((probe == 0) && (xphy_core_info.core_access.type == phymodDispatchTypeCount)){
                               ...
                               phymod_phy_inf_config_t *phy_interface_config)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_intf_mode_from_port_intf(unit, interface_config->interface,
    ...
    switch (interface_config->speed) {
        ...
        case 11000:
            if (interface_config->port_num_lanes == 1){
                /* Fiber mode is not supported in hg mode. */
                PHYMOD_INTF_MODES_FIBER_CLR(phy_interface_config);

                /* if not a supported interface type, default to supported one. */
                if ( phy_interface_config->interface_type != phymodInterfaceKR &&
                     phy_interface_config->interface_type != phymodInterfaceCR &&
                     phy_interface_config->interface_type != phymodInterfaceLR &&
                     phy_interface_config->interface_type != phymodInterfaceXFI )
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(PM_4x25_INFO(pm_info)->nof_phys[port_index] > 0) {
        ...
        _SOC_IF_ERR_EXIT(portmod_line_intf_from_config_get(config, &interface));
        _SOC_IF_ERR_EXIT(portmod_intf_to_phymod_intf(unit, config->speed, interface, &phy_config->interface_type));
        _SOC_IF_ERR_EXIT(_pm4x25_speed_to_interface(unit, port, config, phy_config));
        if (nof_phys > 1)
