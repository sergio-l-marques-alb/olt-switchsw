...
int dnx_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "dnx_fabric", NULL, VERSION(3), 1, SOC_WB_ENGINE_PRE_RELEASE);
    SOCDNX_IF_ERR_EXIT(rv);
...
int dnx_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    {   
        ...
        if (pm_info->type == portmodDispatchTypeDnx_fabric_o_nif) {
            SOCDNX_IF_ERR_EXIT(READ_NBIH_NIF_PMH_FGS_CFGr(unit, port - 256, &reg_val));
            soc_reg_field_set(unit, NBIH_NIF_PMH_FGS_CFGr, &reg_val, FGS_RX_SLOW_READ_RATE_LANE_Nf, slow_read_rate);
            SOCDNX_IF_ERR_EXIT(WRITE_NBIH_NIF_PMH_FGS_CFGr(unit, port - 256, reg_val));
        } else {
