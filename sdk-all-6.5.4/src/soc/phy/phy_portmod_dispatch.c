...
phy_portmod_dispatch_an_set(int unit, soc_port_t port, int an_enable) {
    ...
    sal_memset(&local_ability, 0, sizeof(portmod_port_ability_t));
    rv = portmod_port_ability_local_get(unit, port, &local_ability);
    if (rv != SOC_E_NONE) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit, "phy_portmod_dispatch_an_set: portmod_port_ability_local_get failed")));
        return rv;
    }
    ...
    if (IS_GE_PORT(unit,port)) {
        ...
        an.num_lane_adv = 1;
        local_ability.speed_full_duplex = SOC_PA_SPEED_1000MB | SOC_PA_SPEED_100MB |
                                            SOC_PA_SPEED_10MB;
    }
    ...
    rv = portmod_ext_to_int_cmd_set(unit, port, portmodExtToInt_CtrlCode_AbilityAdvert, &local_ability);
    if (rv != SOC_E_NONE) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit, "phy_portmod_dispatch_an_set: portmod_port_ability_advert_set failed")));
        return rv;
    }

    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                     "phy_portmod_dispatch_an_set an set from ext to int phy \n"
                     ...
                     local_ability.pause,
                     local_ability.interface,
                     local_ability.medium, 
                     local_ability.loopback,
                     local_ability.flags));
...
int
phy_portmod_dispatch_ability_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    portmod_port_ability_t local_ability;

    portmod_ext_to_int_cmd_get(unit, port, portmodExtToInt_CtrlCode_Ability, &local_ability);
...
phy_driver_t phy_ext_to_int_drv = {
    ...
    phy_portmod_dispatch_ability_get,       /* phy_ability_advert_get       */
    phy_portmod_dispatch_ability_get,       /* phy_ability_remote_get       */
    phy_portmod_dispatch_ability_get,       /* phy_ability_local_get        */
