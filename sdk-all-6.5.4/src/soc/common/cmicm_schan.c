/*
 * $Id: cmicm_schan.c,v 1.00 Broadcom SDK $
...
typedef struct cmicm_schan_ch_s {
    sal_spinlock_t lock;
    int timeout;
    uint8 ch[SOC_MAX_NUM_DEVICES][CMIC_CMC_NUM_MAX+1];
}cmicm_schan_ch_t;


STATIC cmicm_schan_ch_t  _cmicm_schan_ch;
STATIC int _soc_schan_multi_cmc = 0;
                   ...
                   int dwc_read, int intr)
{
   ...
   if (!(SAL_BOOT_PLISIM || SAL_BOOT_BCMSIM)) {
    ...
    do {
        ...
        if (soc_schan_timeout_check(unit, &rv, msg, cmc, 0) == TRUE) {
            break;
        }
...
_cmicm_schan_ch_get(int unit, int *cmc)
{
    ...
    sal_spinlock_lock(_cmicm_schan_ch.lock);
    for (i = 0; i <= CMIC_CMC_NUM_MAX; i++) {
        if (_cmicm_schan_ch.ch[unit][i] & 0x01) {
            rv = SOC_E_NONE;
            *cmc = i;
            _cmicm_schan_ch.ch[unit][i] &= ~(0x01);
            break;
        }
    }
    sal_spinlock_unlock(_cmicm_schan_ch.lock);
    return rv;
}
...
int cmicm_schan_ch_get(int unit, int *cmc)
{
    ...
    soc_timeout_init(&to, _cmicm_schan_ch.timeout, 100);
...
int cmicm_schan_ch_try_get(int unit, int cmc)
{
    ...
    sal_spinlock_lock(_cmicm_schan_ch.lock);
    do {
        if (_cmicm_schan_ch.ch[unit][cmc] & 0x01) {
            rv = SOC_E_NONE;
            _cmicm_schan_ch.ch[unit][cmc] &= ~(0x01);
            break;
        }
    } while (!soc_timeout_check(&to));
    sal_spinlock_unlock(_cmicm_schan_ch.lock);
...
int cmicm_schan_ch_init(int unit, int timeout)
{
   ...
   _cmicm_schan_ch.lock = sal_spinlock_create("schan Lock");

   if (_cmicm_schan_ch.lock == NULL) {
        return SOC_E_MEMORY;
   }
