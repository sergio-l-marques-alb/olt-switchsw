    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (select_bitmap & COUNTER_UPDATE_SELECT_LIF_INFO) {
            for (is_ingress = 0; is_ingress < SOC_TMC_CNT_LIF_COUNTING_NOF_SOURCES; is_ingress++) {
                for (command_id = 0; command_id < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES; command_id++) {
                    rv = COUNTERS_ACCESS.lif_ranges.set(unit, is_ingress, command_id, &unitData->lif_ranges[is_ingress][command_id]);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
            }
        }
...
_bcm_dpp_counter_unit_fill(int unit, _bcm_dpp_counter_state_t **unitDataPtr, uint32 select_bitmap, bcm_dpp_counter_proc_specific_data_s * specific_proc_data) {
    ...
    if (select_bitmap & COUNTER_UPDATE_SELECT_LIF_INFO)
    {
        for (is_ingress = 0; is_ingress < SOC_TMC_CNT_LIF_COUNTING_NOF_SOURCES; is_ingress++) {
            for (command_id = 0; command_id < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES; command_id++) {
                rv = COUNTERS_ACCESS.lif_ranges.get(unit, is_ingress, command_id, &unitData->lif_ranges[is_ingress][command_id]);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
...
bcm_dpp_statistic_interface_config(int unit){
    ...
    for(index = 0 ; index < 2 ; index++){
        ...
        soc_sandRes = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_epni_regs_set,(unit, src_type, src_core, eg_type, index));
...
bcm_dpp_counter_lif_range_is_allocated(int unit, uint8* is_allocated) {
    ...
    *is_allocated = FALSE;
    ...
    for (is_ingress = 0; is_ingress < SOC_TMC_CNT_LIF_COUNTING_NOF_SOURCES && *is_allocated != TRUE; is_ingress++) {
        for (range_id = 0; range_id < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES && *is_allocated != TRUE; range_id++) {
...
bcm_dpp_counter_lif_range_source_is_allocated(int unit, SOC_TMC_CNT_SRC_TYPE source, int range_id, uint8* is_allocated) {
    ...
    if (range_id < 0 || range_id >= SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES) {
...
bcm_dpp_counter_lif_range_get(
   ...
   SOC_SAND_U32_RANGE   *range
    ...
    if (counting_profile < 0 || counting_profile >= SOC_TMC_CNT_LIF_COUNTING_NOF_PROFILES_PER_SOURCE) {
    ...
    for (range_index = 0; range_index < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES; range_index++) {
        if (counting_profile == unitData->lif_ranges[is_ingress][range_index].counting_profile) {
            break;
        }
    }
    if (range_index >= SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES) {
...
bcm_dpp_counter_lif_range_set(
   ...
   SOC_SAND_U32_RANGE   *range
    ...
    BCMDNX_NULL_CHECK(range);
    nof_resereved_lifs = is_ingress ? BCM_DPP_AM_LOCAL_IN_LIF_NOF_RESERVED_LIFS : SOC_DPP_CONFIG(unit)->l3.nof_rifs;;
    ...
    if (is_ingress) {
        result = _bcm_dpp_am_local_inlif_counting_profile_set(unit, counting_profile, range->start, (range->end - range->start) + 1);
        BCMDNX_IF_ERR_EXIT(result);
    } else {
        result = _bcm_dpp_am_local_outlif_counting_profile_set(unit, counting_profile, range->start, (range->end - range->start) + 1);
        BCMDNX_IF_ERR_EXIT(result);
    }
   ...
   int                  *base_val) 
{
    ...
    if (command_id < 0 || command_id >= SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES) {
...
_bcm_dpp_counter_lif_counting_priority_default_set(_bcm_dpp_counter_state_t *unitData) {
    ...
    for (is_ingress = 0; is_ingress < SOC_TMC_CNT_LIF_COUNTING_NOF_SOURCES; is_ingress++) {
        for (range_id = 0; range_id < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES; range_id++) {
...
_bcm_dpp_counter_unit_state_verify(_bcm_dpp_counter_state_t *unitData, soc_port_t crps_index) {
    ...
    /* in QAX - for SRC EPNI the base val must be in 2K resolution. see register EPNI_COUNTERS_BASE */
    if(SOC_IS_QAX(unit))
    {
...
bcm_dpp_counter_init(int unit)
{
    ...
    memset(&glb_bg_thread_and_dma_data, 0, SOC_MAX_NUM_DEVICES*sizeof(bcm_dpp_counter_bg_thread_and_dma_data_t));
        ...
        if (unitData->proc[proc_index].mode.mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_TM || 
            ...
            for (system_mc = 0; system_mc < 2; system_mc++) {
                ...
                for (dp = 0; dp < SOC_TMC_NOF_DROP_PRECEDENCE; dp++) {
                    ...
                    BCMDNX_IF_ERR_EXIT(result);
                    if ((SOC_IS_QAX(unit) == TRUE) && (unitData->proc[proc_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EPNI) && (dp != 0))
                    {                       
                    ...
                    if((SOC_IS_QAX(unit) == TRUE) && (unitData->proc[proc_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EPNI))
                    {
                        match = TRUE;
                    }
