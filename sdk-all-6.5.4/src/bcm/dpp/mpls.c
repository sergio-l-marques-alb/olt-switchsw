    ...
    int unit)
{
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && soc_property_get(unit, spn_MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE, 0)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("mpls_egress_label_extended_encapsulation_mode is not supported for QAX and JerichoPlus")));
    }
    ...
    if (!remote_lif) {
        ...
        if (mpls_port.egress_label.label != BCM_MPLS_LABEL_INVALID) {
            ...
            if (is_ingress && !is_egress) {
                ...
                int free_egress_tunnel_label_push_profile = 0;

                 /* free egress tunnel in case its push profile is non-zero and the below soc property is set */
                free_egress_tunnel_label_push_profile = soc_property_get(unit, spn_MPLS_BIND_PWE_WITH_MPLS_ONE_CALL, 0);
                rv = _bcm_petra_mpls_tunnel_free_push_profile(unit, gport_hw_resources.global_out_lif, 1 /*is_pwe*/,free_egress_tunnel_label_push_profile);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
...
/* Parameters:
   ...
   is_pwe: whether the out lif is a pwe entry in the eedb
   free_egress_tunnel_label_push_profile: whether the push profile of the mpls tunnel label in the pwe entry
   should be freed given is_pwe==1, soc property mpls_bind_pwe_with_mpls_one_call == 1*/
int _bcm_petra_mpls_tunnel_free_push_profile(
    int unit,
    int eep,
    uint8 is_pwe,
    uint8 free_egress_tunnel_label_push_profile)
{
    ...
    SOC_PPC_EG_ENCAP_EEP_TYPE eep_type;
    uint32 push_profile;
    ...
    if (free_egress_tunnel_label_push_profile) {
        if (!is_pwe && !soc_property_get(unit, spn_MPLS_BIND_PWE_WITH_MPLS_ONE_CALL, 0)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,\
            (_BSL_BCM_MSG("is_pwe and soc property mpls_bind_pwe_with_mpls_one_call should be set in order to free egress tunnel label push profile.")));
        }
    }
    ...
    eep_type = is_pwe ? SOC_PPC_EG_ENCAP_EEP_TYPE_LIF_EEP : SOC_PPC_EG_ENCAP_EEP_TYPE_TUNNEL_EEP;
    ...
    entry_type = is_pwe ? SOC_PPC_EG_ENCAP_ENTRY_TYPE_PWE : SOC_PPC_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP;
        ...
        if (encap_entry_info[indx].entry_type == entry_type) {
            ...
            for (cur_indx = 0; cur_indx < cur_count; ++cur_indx) {
                if (is_pwe) {
                    ...
                    /* If PWE is bonded with mpls, free the mpls' push profile */
                    if (free_egress_tunnel_label_push_profile && egress_tunnel_label_push_profile != 0) {
                        egress_tunnel_label_push_profile = encap_entry_info[indx].entry_val.pwe_info.egress_tunnel_label_info.tunnels[0].push_profile;
                        rv = _bcm_dpp_am_template_mpls_push_profile_free(unit,
                                                                         egress_tunnel_label_push_profile,
                                                                         &is_last);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
                }
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if ((label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_WITH_ID) && (label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_REPLACE)) {
            ...
            rv = _bcm_petra_mpls_tunnel_free_push_profile(unit, eep, 0/* is pwe */,0);
            BCMDNX_IF_ERR_EXIT(rv);
        }
...
int _bcm_gport_mpls_is_valid_replace(int unit, bcm_mpls_port_t *new_mpls_port, bcm_mpls_port_t *old_mpls_port, bcm_mpls_label_t old_egress_vc, uint32 old_push_profile){
    ...
    if (soc_property_get(unit, spn_PWE_TERMINATION_PORT_MODE_ENABLE, 0) &&
          (old_mpls_port->criteria == BCM_MPLS_PORT_MATCH_LABEL_PORT) ){
         _bcm_dpp_gport_hw_resources gport_hw_resources;
	 _bcm_dpp_gport_sw_resources gport_sw_resources;
         int        is_ingress;
         SOC_PPC_PORT   soc_ppc_port;
         SOC_PPC_PORT_INFO  port_info;
          int       core;
          rv = _bcm_dpp_gport_to_hw_resources(unit, old_mpls_port->mpls_port_id, _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF, &gport_hw_resources);
         ...
         if (!BCM_GPORT_IS_LOCAL(new_mpls_port->port)) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("local port must be specified when working at port match mode")));
          }

         BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, BCM_GPORT_LOCAL_GET(new_mpls_port->port), &soc_ppc_port, &core)));       

         BCM_SAND_IF_ERR_EXIT(soc_ppd_port_info_get(unit, core, soc_ppc_port, &port_info));
          if(port_info.vlan_domain != gport_sw_resources.in_lif_sw_resources.match2) {
              BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("when working at BCM_MPLS_PORT_MATCH_LABEL_PORT mode, vlan_domain could not be updated")));              
          }
    }
exit:
    BCMDNX_FUNC_RETURN;
...
int _bcm_dpp_mpls_port_pwe_set(
   ...
   SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info) {
    ...
    if (mpls_port->criteria == BCM_MPLS_PORT_MATCH_LABEL_PORT) {
         ...
         pwe_additional_info.vlan_domain = port_info.vlan_domain;
         pwe_additional_info.vlan_domain_valid = 1;
    }
    if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
        update = 1;
