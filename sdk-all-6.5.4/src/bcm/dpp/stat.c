...
{
    int rv = BCM_E_NONE;
    uint32 valid_flags = BCM_STAT_COUNTER_CLEAR_CONFIGURATION | BCM_STAT_COUNTER_CACHE | BCM_STAT_COUNTER_SYMMETRICAL_MULTICORE_ENGINE | BCM_STAT_COUNTER_IGNORE_COLORS | BCM_STAT_COUNTER_IGNORE_DISPOSITION;
    int index, offset;
    ...
    int priority)
{
    ...
    if(SOC_IS_QAX(unit) && source->type == bcmStatCounterSourceEgressTransmitOutlif)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported API for source device type ans source OUT_LIF")));
    }
    BCMDNX_NULL_CHECK(source);
    if (source->type == bcmStatCounterSourceIngressInlif) {
        counter_source = SOC_TMC_CNT_SRC_TYPE_ING_PP;
