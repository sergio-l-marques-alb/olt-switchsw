...
int _bcm_ml_port_lanes_set(int unit, bcm_port_t port, int lanes)
{
    ...
    if ((_ml_config_id[unit] == (BCM56272_DEVICE_ID_OFFSET_CFG + 1)) ||
        (_ml_config_id[unit] == (BCM56272_DEVICE_ID_OFFSET_CFG + 2))) {
        LOG_DEBUG(BSL_LS_BCM_PORT,
                 (BSL_META_U(unit,
                            "Flex Operation not supported on unit %d: Int cfg %d\n"),
                             unit, _ml_config_id[unit]));
    ...
    if (IS_MXQ_PORT(unit, port)) {
        ...
        if ((new_lanes == 4) &&
            (_ml_config_id[unit] == (BCM56271_DEVICE_ID_OFFSET_CFG + 2))) {
            return BCM_E_PARAM; 
        }
    }
