...
typedef struct ls_cntl_s {
    ...
    sal_mutex_t         lc_handle_lock;        /* Synchronization handle function */
} ls_cntl_t;
...
#define LC_HANDLE_LOCK(unit) \
        sal_mutex_take(link_control[unit]->lc_handle_lock, sal_mutex_FOREVER);

#define LC_HANDLE_UNLOCK(unit) \
        sal_mutex_give(link_control[unit]->lc_handle_lock);
...
_bcm_esw_link_fault_get(int unit, int port, int *fault)
{
    ...
    if (SOC_PBMP_MEMBER(sop->lc_pbm_fc, port)) {
        if (!(SOC_USE_PORTCTRL(unit))) {
            ...
            /* If FC link fault exists, return else continue checking fault status */
            if (*fault) {
                return BCM_E_NONE;
            }
        }
 ...
 *      port  - Port to process.
 *      lc_lock_flag - LC_LOCK is unlocked in this routine when lc_lock_flag == 0
 * Returns:
 *      BCM_E_XXX
 */

STATIC int
_bcm_esw_linkscan_update_port(int unit, uint32 flags, int port, int *lc_lock_flag)
{
    soc_persist_t *sop = SOC_PERSIST(unit);
    ls_cntl_t *lc = link_control[unit];
   ...
   if (SOC_IS_KATANA2(unit) && (!SOC_IS_SABER2(unit))) {
    ...
    if (notify) {
        ...
        LC_UNLOCK(unit);
        *lc_lock_flag = 0;
        /* if we don't unlock, there will be deadlock in handle functions */
...
_bcm_esw_linkscan_update(int unit, uint32 flags, pbmp_t pbm)
{
    ...
    int rv;
    bcm_port_t port;
    /* If lc_lock_flag is 0,  LC_LOCK is unlocked before
    * calling callback functions in routine _bcm_esw_linkscan_update_port.
    */
    int lc_lock_flag = 1;
    ...
    PBMP_ITER(pbm, port) {
        ...
        LC_LOCK(unit);
        /* Keep callback routines with port link status updating */
        LC_HANDLE_LOCK(unit);
        lc_lock_flag = 1;

        save_link_change = sop->lc_pbm_link_change;

        rv = _bcm_esw_linkscan_update_port(unit, flags, port, &lc_lock_flag);
...
_bcm_esw_linkscan_thread(int unit)
{
    ...
    int               rv = BCM_E_NONE;
    soc_port_t        port;
    int               link = 0, rx_en = 0, hw_linkscan_mode = 0;
#if defined(BCM_KATANA2_SUPPORT)
    soc_port_t        pp_port;
#endif
    ...
    if (!lc->lc_warm_boot) {
        ...
        PBMP_ITER(PBMP_PORT_ALL(unit), port) {
            ...
            if ((!BCM_FAILURE(rv)) && link) {
                ...
                if ((!BCM_FAILURE(rv)) && (!rx_en)) {
                    /* When PHY is UP and MAC is down, then MAC has to be
                     * reconfigured by syncing it with PHY.
                     */
...
bcm_esw_linkscan_init(int unit)
{
    ...
    lc->lc_handle_lock = sal_mutex_create("bcm_link_handle_LOCK");
    if (lc->lc_handle_lock == NULL) {
        sal_free(lc);
        return BCM_E_MEMORY;
    }
...
bcm_esw_linkscan_mode_set(int unit, bcm_port_t port, int mode)
{
    ...
    case BCM_LINKSCAN_MODE_HW:
        BCM_IF_ERROR_RETURN(_bcm_port_link_get(unit, port, 0, &link_up));

        /* syncup pbm with cmic_miim_link_status registesrs,
        to avoid misleading up/down notification */
        SOC_IF_ERROR_RETURN(soc_linkscan_hw_link_get(unit, &tmp_pbm));
...
int _bcm_esw_lc_check_init(unit)
{
    LC_CHECK_INIT(unit);
    return BCM_E_NONE;
}
...
bcm_esw_linkscan_detach(int unit)
{
    ...
    if (lc->lc_handle_lock != NULL) {
        sal_mutex_destroy(lc->lc_handle_lock);
        lc->lc_handle_lock = NULL;
    }
