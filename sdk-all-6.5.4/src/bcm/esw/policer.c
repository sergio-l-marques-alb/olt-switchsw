...
shr_aidxres_list_handle_t  meter_alloc_list_handle[BCM_UNITS_MAX]\
          [BCM_POLICER_GLOBAL_METER_MAX_POOL * BCM_POLICER_GLOBAL_METER_MAX_BANKS_PER_POOL] = {{NULL}}; 
...
{
    int num_banks = 1; /* default. 1-pool:1-bank */

    if (SOC_IS_APACHE(unit)) {
        num_banks = 2; /* 1-pool:2-banks */
    }
    ...
    bcm_policer_svc_meter_mode_t svc_meter_mode)
{
    ...
    mode_type = global_meter_offset_mode[unit][svc_meter_mode].meter_attr.mode_type_v;
    if ((mode_type == udf_mode) || 
...
static pkt_attr_bit_pos_t sb2_pkt_attr_bit_pos[pkt_attr_count] =  { 
                                       {0, 0}, {1, 1}, {2, 4}, {5, 10},
    ...
    pkt_attr_bits_t    pkt_attr_bits_v)
{
    ...
    if (SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit)) {
        pkt_attr_bit_pos = &sb2_pkt_attr_bit_pos[0];
    ...
    if (pkt_attr_bits_v.int_pri != 0) {
        int start_bit = 0; 
        if ((mode_type_v == compressed_mode) && (SOC_IS_SABER2(unit) ||
                                                            SOC_IS_APACHE(unit))) {
            start_bit = 32;
        } else if ((mode_type_v == compressed_mode) && SOC_IS_KATANA2(unit)) { 
            start_bit = 33;
        } else {
    ...
    bcm_policer_svc_meter_mode_t *svc_meter_mode)
{
    ...
    if (uncompressed_attr_selectors_v->uncompressed_attr_bits_selector &
        ...
        if (SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit)) {
            port_attr_size = BCM_POLICER_SVC_METER_SB2_ING_PORT_ATTR_SIZE; 
        } else if (SOC_IS_KATANA2(unit)) {
            port_attr_size = BCM_POLICER_SVC_METER_KT2_ING_PORT_ATTR_SIZE; 
        }
    ...
    if (uncompressed_attr_selectors_v->uncompressed_attr_bits_selector &
        ...
        if (SOC_IS_KATANA2(unit) || SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit)) {
    ...
    for (index = 1; index < BCM_POLICER_SVC_METER_MAX_MODE; index++) {
        if (_bcm_policer_svc_meter_get_mode_info(
            ...
            if ((((pkt_attr_bits->cng) || (pkt_attr_bits->int_pri)) &&
                 ((config_pkt_attr->cng) || (config_pkt_attr->int_pri) || (config_pkt_attr->short_int_pri))) || \
    ...
    uint32                       *total_bits)
{
    ...
    if (udf_pkt_attr->type == bcmPolicerGroupModeTypeNormal) {
        mode_type = udf_mode;
    } else if (udf_pkt_attr->type == bcmPolicerGroupModeTypeCascade) {
        mode_type = udf_cascade_mode;
    } else {
        mode_type = udf_cascade_with_coupling_mode;
    }
    ...
    for (index = 1;index < BCM_POLICER_SVC_METER_MAX_MODE; index++) {
        if (_bcm_policer_svc_meter_get_mode_info(unit, index, &mode_info) == 
            ...
            if ((udf_attr->offset == udf_pkt_attr->offset) &&
                (udf_attr->width == udf_pkt_attr->width) &&
                (udf_attr->num_selectors == udf_pkt_attr->num_selectors) &&
                (udf_attr->type == udf_pkt_attr->type) &&
                (sal_memcmp(&udf_attr->offset_map[0],
                            &udf_pkt_attr->offset_map[0],
                            (udf_pkt_attr->num_selectors * sizeof(offset_table_entry_t))) == 0)) {
...
int  _bcm_esw_policer_increment_ref_count(int unit, bcm_policer_t policer) 
{
    ...
    if ((global_meter_offset_mode[unit][offset_mode].meter_attr.mode_type_v == 
        cascade_mode) ||
        (global_meter_offset_mode[unit][offset_mode].meter_attr.mode_type_v ==
        udf_cascade_mode) ||
        (global_meter_offset_mode[unit][offset_mode].meter_attr.mode_type_v ==
...
{    
    ...
    if (!(attr_selectors[0].flags & BCM_POLICER_ATTR_WIDTH_OFFSET)) {
        return (BCM_E_PARAM);
    }
#if defined(BCM_KATANA2_SUPPORT) || defined(BCM_APACHE_SUPPORT)
    ...
    /* width = 0 means ING_SVM_PKT_ATTR_SELECTOR_KEY_Xr is not updated
     * with required values, makes no sense */
    if ((attr_selectors[0].width > 8) || (attr_selectors[0].width <= 0)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER, (BSL_META_U(unit,
    ...
    if (attr_selectors[0].offset >= SVC_METER_UDF1_MAX_BIT_POSITION) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER, (BSL_META_U(unit,
     ...
     * For eg. if offset=31, width can be 1 (selecting msb of udf1) making sum 32 */
    if ((attr_selectors[0].offset + attr_selectors[0].width)
    ...
    attr_selectors[0].offset += 1;  /* 0-th bit is used for UDF drop bit */ 
    ...
    rv = _bcm_esw_policer_udf_group_set_offset_map(unit, type, total_policers, 
                                                   num_selectors,
                                                   attr_selectors,
                                                   mode_attr);
    if (BCM_FAILURE(rv)) {
    ...
    mode_attr->udf_pkt_attr_selectors_v.offset = attr_selectors[0].offset;
    mode_attr->udf_pkt_attr_selectors_v.width = attr_selectors[0].width;
    mode_attr->udf_pkt_attr_selectors_v.num_selectors = num_selectors;
    mode_attr->udf_pkt_attr_selectors_v.type = type;
    mode_attr->udf_pkt_attr_selectors_v.udf_id = attr_selectors[0].udf_id;
    ...
    GLOBAL_METER_LOCK(unit);
    rv = _bcm_policer_svc_meter_create_udf_mode(unit, 
                             &mode_attr->udf_pkt_attr_selectors_v,
                             group_mode,
                             mode_id,
                             0);
    if (BCM_FAILURE(rv) && (rv != BCM_E_EXISTS)) {
        ...
        mode_attr->mode_type_v = mode_type_v;

        rv = _bcm_policer_svc_meter_reserve_mode(unit, *mode_id,
                                                 group_mode, mode_attr); 
        if (BCM_FAILURE(rv)) {
            GLOBAL_METER_UNLOCK(unit);
            sal_free(mode_attr);
            return rv;
    ...
    uint32 *mode_id)
{
    ...
    /* 
     * Attribute Selectors is used for selecting one of the policers created in 
    ...
    /* Currently only udf is supported */
    if (attr_selectors[0].attr != bcmPolicerGroupModeAttrUdf) {
       return (BCM_E_UNAVAIL);
    } else {
    ...
    uint32 *actual_num_selectors)
{
    ...
    *total_policers = mode_info.no_of_policers;
    if (mode_info.meter_attr.mode_type_v == udf_cascade_with_coupling_mode) {
        *total_policers = mode_info.no_of_policers / 2;
    }    

    *flags =  0;

    switch (mode_info.meter_attr.mode_type_v) {
        case udf_mode:
        {
            udf_attr = &mode_info.meter_attr.udf_pkt_attr_selectors_v;
            *type = udf_attr->type;
            *actual_num_selectors = udf_attr->num_selectors;
            ...
            for (i = 0; i < BCM_SVC_METER_MAP_SIZE_256; i++)   {
                if (udf_attr->offset_map[i].meter_enable == 1)  {
                    ...
                    attr_selectors[j].policer_offset = udf_attr->offset_map[i].offset;
                    attr_selectors[j].udf_id = udf_attr->udf_id;
                    attr_selectors[j].width = udf_attr->width;
                    attr_selectors[j].flags |= BCM_POLICER_ATTR_WIDTH_OFFSET;
                    attr_selectors[j].attr = bcmPolicerGroupModeAttrUdf;
                    attr_selectors[j].offset = udf_attr->offset;

                    if (attr_selectors[j].offset > 0)   {
                        attr_selectors[j].offset = udf_attr->offset - 1;
                    }
                    j++;
                }
            }
        ...
        {
            udf_attr = &mode_info.meter_attr.udf_pkt_attr_selectors_v;
            pool_offset_limit = (udf_attr->type == bcmPolicerGroupModeTypeCascade) ?
                                        BCM_POLICER_GLOBAL_METER_MAX_POOL :
            ...
            *actual_num_selectors = udf_attr->num_selectors;
            ...
            for (i = 0; i < BCM_SVC_METER_MAP_SIZE_256; i++)   {
                if (udf_attr->offset_map[i].meter_enable == 1)  {
                    ...
                    if (soc_feature(unit, soc_feature_global_meter_pool_priority_descending))   {
                        attr_selectors[j].policer_offset =
                                                pool_offset_limit - udf_attr->offset_map[i].pool - 1;
                    } else {
                        attr_selectors[j].policer_offset = udf_attr->offset_map[i].pool;
                    }
                    j++;
                }
            }
    ...
    uint32 *npolicers)
{
    ...
    if (*npolicers !=  1) {
        ...
        switch (mode_info.meter_attr.mode_type_v) {
            ...
            case cascade_with_coupling_mode:
            case udf_cascade_with_coupling_mode:
               direction = GLOBAL_METER_ALLOC_HORIZONTAL;
               mode = bcmPolicerGroupModeCascadeWithCoupling;
               break;
 ...
 * Purpose:
 *     Get the list of policer members for a given policer group represented by its base policer id. 
 * Parameters:
 *     unit                  - (IN) unit number 
 *     policer_id            - (IN) Base policer Id
 *     member_max            - (IN) Maximum number of policers to return in the member_array parameter
 *     member_array          - (OUT) Place to store policer members of the given policer group 
 *                             represented by its base_policer_id. Memory need to be allocated by 
 *                             the end user before calling the API. 
 *     member_count          - (OUT) Place to store total number of policers configured in the given 
 *                             policer group represented by base_policer_id. 

 * Returns:
 *     BCM_E_XXX 
 */
int bcm_esw_policer_group_get (int unit, bcm_policer_t base_policer_id, int member_max, 
                                bcm_policer_t *member_array, int *member_count)
{
    int rv = BCM_E_NONE;
        ...
        case bcmPolicerGroupModeIntPriCascadeWithCoupling:
        {
            for (index = 0; index < indices_to_fill; index++) {
                ...
                member_array[index] = base_policer_id + 
                            (policer_control->offset[policer_offset] * SOC_INFO(unit).global_meter_max_size_of_pool);
            }
            break;
        }
        default:
                           ...
                           _global_meter_policer_control_t *policer_control) 
{
   ...
   if (SOC_MEM_FIELD_VALID(unit, SVM_METER_TABLEm, POLICY_TABLE_INDEXf)) {
    ...
    /* Set the grp_mode for policer control. In case offset_mode=0, 
       global_meter_offset_mode[0][0].group_mode will have by default 0 (bcmPolicerGroupModeSingle) */
    policer_control->grp_mode = global_meter_offset_mode[unit][offset_mode].group_mode;
    ...
    if((soc_feature(unit, soc_feature_global_meter_mef_10dot3)) &&
        ...
        for (index = 0; index < num_pools; index++) {
...
int _bcm_policer_config_reinit_from_table(int unit, soc_mem_t table,void *data)
{
    ...
    int index = 0, max_index = 0, offset_mode = 0;
    _global_meter_policer_control_t *policer_control = NULL;
    int rv = BCM_E_NONE;
    ...
    for ( index = 0; index <= max_index; index++) {
        ...
        if ((policer & BCM_POLICER_GLOBAL_METER_INDEX_MASK) > 0) {
            ...
            if ((global_meter_offset_mode[unit][offset_mode].meter_attr.mode_type_v == 
                 ...
                 * bandwidth sharing.
                 */
                rv = _bcm_global_meter_policer_get(unit, policer,
                                                   &policer_control);
            } else {
                                 ...
                                 uint32  selector_for_bit_x_field_value[8])
{
    ...
    udf_attr->offset = selector_for_bit_x_field_value[0];
    udf_attr->type = bcmPolicerGroupModeTypeNormal;
    ...
    for (mode = 1; mode < BCM_POLICER_SVC_METER_MAX_MODE; mode++) { 
        ...
        if (!COMPILER_64_IS_ZERO(selector_key_value))
        {
            ...
            if (mode_type == udf_cascade_mode) {
                udf_attr = 
                   &global_meter_offset_mode[unit][mode].meter_attr.udf_pkt_attr_selectors_v;
                udf_attr->type = bcmPolicerGroupModeTypeCascade;
                global_meter_offset_mode[unit][mode].meter_attr.mode_type_v = 
                                                               udf_cascade_mode;
            }    
            if (mode_type == udf_cascade_with_coupling_mode) {
                udf_attr = 
                   &global_meter_offset_mode[unit][mode].meter_attr.udf_pkt_attr_selectors_v;
                udf_attr->type = bcmPolicerGroupModeTypeCascadeWithCoupling;
                global_meter_offset_mode[unit][mode].meter_attr.mode_type_v = 
                                                 udf_cascade_with_coupling_mode;
            }    
        }
...
_bcm_esw_global_meter_policer_sync(int unit)
{
    ...
    int policer_id = 0;
    uint8  *policer_state;
    uint32 npolicers, group_mode, udf_id;
    int mode;
    int size = 0, count = 0;
    if (!soc_feature(unit, soc_feature_global_meter)) {
        return BCM_E_UNAVAIL;
    ...
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_1)   {
        ...
        for (mode = 0; mode < BCM_POLICER_SVC_METER_MAX_MODE; mode++)   {
            if (global_meter_offset_mode[unit][mode].used == 1) {
                ...
                group_mode = (uint32)global_meter_offset_mode[unit][mode].group_mode;
                sal_memcpy(policer_state, &group_mode, sizeof(uint32));
                policer_state += sizeof(uint32);

                udf_id = global_meter_offset_mode[unit][mode].meter_attr.udf_pkt_attr_selectors_v.udf_id;
            ...
            if (recovered_ver >= BCM_WB_VERSION_1_1)  {
                for (mode = 0; mode < BCM_POLICER_SVC_METER_MAX_MODE; mode++)   {
                    if (global_meter_offset_mode[unit][mode].used == 1) {
                        ...
                        sal_memcpy(&group_mode, policer_state, sizeof(uint32));
                        global_meter_offset_mode[unit][mode].group_mode = group_mode;
                        policer_state+=(sizeof(uint32));

                        sal_memcpy(&udf_id, policer_state, sizeof(uint32));
                        global_meter_offset_mode[unit][mode].meter_attr.udf_pkt_attr_selectors_v.udf_id = udf_id;
                        policer_state += (sizeof(uint32));

                        /* handling mode_type_v/type for udf based policers */
                        if (global_meter_offset_mode[unit][mode].meter_attr.mode_type_v == udf_mode) {
                            if (group_mode == bcmPolicerGroupModeCascade)   {
                                global_meter_offset_mode[unit][mode].meter_attr.mode_type_v = udf_cascade_mode;
                                global_meter_offset_mode[unit][mode].meter_attr.
                                        udf_pkt_attr_selectors_v.type = bcmPolicerGroupModeTypeCascade;
                            } else if (group_mode == bcmPolicerGroupModeCascadeWithCoupling)    {
                                global_meter_offset_mode[unit][mode].meter_attr.mode_type_v = udf_cascade_with_coupling_mode;
                                global_meter_offset_mode[unit][mode].meter_attr.
                                        udf_pkt_attr_selectors_v.type = bcmPolicerGroupModeTypeCascadeWithCoupling;
                            }
                        } else {
                            if ((group_mode == bcmPolicerGroupModeCascade) ||
                                (group_mode == bcmPolicerGroupModeCascadeWithCoupling) ||
