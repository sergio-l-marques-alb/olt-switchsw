    ...
    int mc_index, bcm_pbmp_t e_pbmp, int is_l3)
{
    ...
    BCM_PBMP_ITER(e_pbmp, port_iter) {
        rv = _bcm_esw_ipmc_port_trunkid_get(unit, port_iter, &tgid);
        if (BCM_FAILURE(rv)) {
        ...
        if (tgid != BCM_TRUNK_INVALID) {
            ...
            if (i == count) {
                ...
                first_member_array[count].first_member = port_iter;
                is_first_member = TRUE;
                count++;
            }
            ...
            if (is_first_member) {
                rv = _bcm_esw_ipmc_egress_intf_set_for_trunk_first_member(
                         unit, mc_index, port_iter, 0, NULL,
                         is_l3, FALSE, tgid);
            } else {
...

{
    ...
    bcm_module_t mod_out;
    bcm_port_t port_out, port_gport;
    bcm_trunk_t trunk_id;
...
#ifdef BCM_TRIDENT2_SUPPORT
        /* if this is a virtual NH enap id, then it
            ...
            BCM_PBMP_ITER(mc_eligible_pbmp, port_iter) {
                ...
                if (BCM_MC_PER_TRUNK_REPL_MODE(unit)) {
                    if (trunk_id != BCM_TRUNK_INVALID) {
                        ...
                        if (first_member == -1) {
                            rv = _bcm_esw_ipmc_egress_intf_set_for_trunk_first_member(
                                     unit, mc_index, port_iter, list_count,
                                     temp_encap_list, is_l3, TRUE, trunk_id);
                            trunk_pipe_first_member_array[pos][pipe] = port_iter;
                        } else {
                            rv = _bcm_esw_ipmc_egress_intf_set_for_same_pipe_member(
                                     unit, mc_index, port_iter, first_member,
                                     trunk_id);
                        }
                    } else {
                        rv = _bcm_esw_ipmc_egress_intf_set_for_trunk_first_member(
...
#ifdef BCM_KATANA2_SUPPORT
             if (SOC_IS_KATANA2(unit)) {
                 BCM_IF_ERROR_RETURN(bcm_kt2_sw_hw_queue(unit, &queue_array[i]));
...
#endif /* BCM_KATANA2_SUPPORT */
             {
                 BCM_IF_ERROR_RETURN(bcm_kt_sw_hw_queue(unit, &queue_array[i]));
                                ...
                                bcm_multicast_t group)
{
    ...
    if (port_count) {
        rv = _bcm_kt_ipmc_subscriber_egress_intf_delete_all(unit, remap_index,
                port_array, encap_array,
                q_array,
                &q_count_decrease);
        if (BCM_SUCCESS(rv)){
            if (q_count_decrease){
                for ( index  = 0; index <  port_count; index++){
#ifdef BCM_KATANA2_SUPPORT
                    if (SOC_IS_KATANA2(unit)){
                        rv = bcm_kt2_cosq_port_get(unit, q_array[index], &temp_port);
                    } else
#endif
                    if (SOC_IS_KATANA(unit)){
                        rv = bcm_kt_cosq_port_get(unit, q_array[index], &temp_port);
                    } else {
                        goto failure;
                    }
                    if (BCM_FAILURE(rv)) {
                    ...
                    _bcm_kt_ipmc_port_ext_queue_count_decrement(unit, mc_index, temp_port );
                    rv = (_bcm_esw_ipmc_set_remap_group(unit, mc_index,
                                temp_port, 0));
                    if (BCM_FAILURE(rv)) {
                        goto failure;
