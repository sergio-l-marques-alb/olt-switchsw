...
#if defined(BCM_TRIDENT_SUPPORT) || defined(BCM_GREYHOUND_SUPPORT)
STATIC int
_bcm_esw_mirror_dest_tunnel_flags_get(int unit, int egress, int mtp_index,
                                      uint32 *flags)
{
    ...
    if (soc_feature(unit, soc_feature_mirror_encap_profile)) {
        ...
        offset = mtp_index * max_num_trunk_ports;
        BCM_IF_ERROR_RETURN
                                       ...
                                       int flags)
{
    ...
    if (mdest_flags & BCM_MIRROR_DEST_INT_PRI_SET) {
        ...
        if (soc_feature(unit, soc_feature_egr_mirror_true)) {
            if (flags & BCM_MIRROR_PORT_EGRESS_TRUE) {
                BCM_IF_ERROR_RETURN
                    (READ_EGR_EP_REDIRECT_EM_MTP_INDEXm(unit, MEM_BLOCK_ANY, offset,
...
_bcm_esw_mirror_reload(int unit, int directed)
{
...
#endif /* BCM_TRIUMPH2_SUPPORT */
    {
        for (idx = 0; idx < MIRROR_CONFIG(unit)->ing_mtp_count; idx++) {
            ...
            if (soc_feature(unit, soc_feature_mirror_encap_profile)) {
                BCM_IF_ERROR_RETURN
                    (_bcm_esw_mirror_dest_tunnel_flags_get(unit, FALSE, idx,
                                                           &mirror_dest.flags));
            }
#endif
            ...
            if (soc_feature(unit, soc_feature_mirror_flexible) &&
                MIRROR_MTP_METHOD_IS_DIRECTED_FLEXIBLE(unit)) {
                BCM_IF_ERROR_RETURN(
                    _bcm_td_mirror_destination_pri_recover(unit, &mirror_dest,
                                                           idx,
        ...
        for (idx = 0; idx < MIRROR_CONFIG(unit)->egr_mtp_count; idx++) {
            ...
            if (soc_feature(unit, soc_feature_mirror_encap_profile)) {
                BCM_IF_ERROR_RETURN
                    (_bcm_esw_mirror_dest_tunnel_flags_get(unit, TRUE, idx,
                                                           &mirror_dest.flags));
            }
#endif
                        ...
                        uint32 flags)
{
    ...
    if (soc_feature(unit, soc_feature_mirror_ing_egr_single)) {
