...
int sw_state_journal_transaction_start(int unit) {
    SOC_INIT_FUNC_DEFS;

    sw_state_journal[unit]->is_logging = 1;
    SOC_EXIT;
...
int
sw_state_journal_transaction_end(int unit)
{
    SOC_INIT_FUNC_DEFS;

    sw_state_journal[unit]->is_logging = 0;
    SOC_EXIT;
...
int sw_state_journal_insert(int unit, uint32 offset, uint32 length, uint8 *data){
    ...
    /* exit if not in logging mode */
    if (!sw_state_journal[unit]->is_logging) {
        SOC_EXIT;
    }
    ...
    _SOC_IF_ERR_EXIT(sw_state_access[unit].dcmn.crash_recovery.transaction_mode.get(unit, &tr_mode));
