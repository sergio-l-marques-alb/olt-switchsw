...
#ifdef CRASH_RECOVERY_SUPPORT
int shr_sw_state_backup_in_roll_back_journal(int unit, uint8* current_pointer_in_sw_state, int size, uint8 *data) 
{
    uint64                                offset; 
    ...
    if (SOC_CR_ENABALED(unit)) {
        ...
        _SOC_IF_ERR_EXIT(sw_state_journal_insert(unit, offset, size, (uint8*) data));
    }
...
#define SW_STATE_JOURNAL_INSERT(unit, current_pointer_in_sw_state, size, data)\
    shr_sw_state_backup_in_roll_back_journal(unit, current_pointer_in_sw_state, size, (uint8 *) data)
...
shr_sw_state_memset(int unit, uint8 *location, int input, uint32 length, uint32 flags){
    ...
    if (is_modified && SW_STATE_DO_JOURNALING(flags)){
        SW_STATE_JOURNAL_INSERT(unit, location, length, location);
    }
...
*********************************************************************/
int
shr_sw_state_memcpy(int unit, uint8 *location, uint8 *input, uint32 length, uint32 flags){
...
shr_sw_state_pbmp_operation(int unit, _shr_pbmp_t *path, SW_ST_PBMP op, _shr_pbmp_t *input_pbmp,
    ...
    if (SW_STATE_DO_JOURNALING(flags)){
        SW_STATE_JOURNAL_INSERT(unit, (uint8 *) path, sizeof(bcm_pbmp_t), (uint8 *) path);
    }
    switch (op){
    case SW_ST_PBMP_NEQ :
...
*********************************************************************/
int
shr_sw_state_alloc(int unit, uint8** ptr, uint32 size, uint32 flags) {
    ...
    */
    ...
    if (shr_sw_state_data_block_header[unit]->size_left < alloc_size + SW_STATE_MARGIN_OF_SAFETY) {
       _SOC_IF_ERR_EXIT(shr_sw_state_defrag(unit, &ptr));
    }
    ...
    /* next free data slot roll back journal */
    if (SW_STATE_DO_JOURNALING(flags)){
        SW_STATE_JOURNAL_INSERT(unit, (void*) &shr_sw_state_data_block_header[unit]->next_free_data_slot, sizeof(uint8*),
                                (void*) &shr_sw_state_data_block_header[unit]->next_free_data_slot);
    }

    /* update next free data slot */
    shr_sw_state_data_block_header[unit]->next_free_data_slot += alloc_size;

    /* next free data slot auto sync support */
...
static int shr_sw_state_issu_layout_nodes_diff_print(
   ...
   shr_sw_state_ds_layout_node_t    *updated_layout_node) {
     ...
     for (i = retrieved_node_first_child; i < retrieved_node_last_child; i++) {
         is_exist = 0;
         for (j = updated_node_first_child; j < updated_node_last_child; j++) {
     ...
     for (i = updated_node_first_child; i < updated_node_last_child; i++) {
         is_exist = 0;
         for (j = retrieved_node_first_child; j < retrieved_node_last_child; j++) {
...
void shr_sw_state_block_dump(int unit, char *file_name, char *mode)
{
    ...
    /* masking crash recovery variables */
    sal_memset((buff_to_print + 
                ((uint8 *) (&(sw_state[unit]->dcmn.crash_recovery)) - (uint8 *) shr_sw_state_data_block_header[unit]->data_ptr)),
                0x0, sizeof(sw_state[unit]->dcmn.crash_recovery));
    ...
    switch(param.op) {
      ...
      case SW_ST_BIT_CLEAR:
        if (SW_STATE_DO_AUTOSYNC(1)){
            ...
            if (SW_STATE_DO_JOURNALING(param.flags)){
                SW_STATE_JOURNAL_INSERT(unit, data_pointer, data_size, data_pointer);
            }
        }
        break;
      ...
      case SW_ST_BIT_RANGE_SET:
        if (SW_STATE_DO_AUTOSYNC(1)){
            data_pointer = ((uint8*) &(param.sw_state_bmp)[param.sw_state_bmp_first / SHR_BITWID]);
            data_size = SHR_BITALLOCSIZE(param.sw_state_bmp_first + param._range);
            if (SW_STATE_DO_JOURNALING(param.flags)){
                SW_STATE_JOURNAL_INSERT(unit, data_pointer, data_size, data_pointer);
            }
        }
