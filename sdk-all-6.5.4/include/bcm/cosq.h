...
#define BCM_COSQ_BW_PACKET_MODE 0x00000020 /* The bandwidth specified is in
                                              ...
                                              kbits per second */
#define BCM_COSQ_BW_NOT_COMMIT  0x00000040 /* Set rate to SW DB only, commit to
                                              HW is done when calling the API
                                              with the same GPort type and
                                              without NOT_COMMIT flag. */
...
#define BCM_BCM_COSQ_GPORT_SIZE_SRAM        0x0010     /* SRAM configuration */
#define BCM_BCM_COSQ_GPORT_SIZE_DRAM_BOUND  0x0020     /* DRAM bound
                                                          configuration */
#define BCM_COSQ_GPORT_SIZE_SRC_VSQ_POOL1   0x0040     /* Configure the pool-1
                                                          source VSQ, else
                                                          configure the pool-0
                                                          source VSQ. */
#define BCM_BCM_COSQ_GPORT_SIZE_RECOVERY_FAILURE 0x0080     /* Configure recovery
                                                          failure settings */
#define BCM_BCM_COSQ_GPORT_SIZE_RECOVERY    0x0100     /* Configure recovery
                                                          settings */
#define BCM_BCM_COSQ_GPORT_SIZE_GLOBAL      0x0200     /* Configure global
                                                          settings */
...
typedef enum bcm_cosq_stat_e {
    ...
    bcmCosqStatMaxCount = 56            /* this should be the last one. */
} bcm_cosq_stat_t;
...
typedef enum bcm_bst_stat_id_e {
    ...
    bcmBstStatIdHeadroomPool = 9,       /* BST Tracing the Headroom Pool Usage
                                           Count */
    bcmBstStatIdMaxCount = 10           /* Must be the last. Not a usable value. */
} bcm_bst_stat_id_t;
...
/* Type of a resource allocated. */
typedef enum bcm_cosq_allocation_resource_e {
    bcmReservationResourceBufferDescriptors = 0, /* Resource reservation of buffer
                                           descriptors. */
    bcmReservationResourceOcbBuffers = 1, /* OCB resource reservation in buffers. */
    bcmReservationResourceBufferDescriptorBuffers = 2, /* Resource reservation of buffer
                                           descriptor buffers. */
    bcmReservationResourceBytes = 3     /* Resource reservation in bytes. */
} bcm_cosq_allocation_resource_t;
...
typedef struct bcm_cosq_resource_amounts_s {
    ...
    uint32 max_shared_pool; /* maximum amount of the resource in the shared
                               pool. */
    uint32 max_headroom;    /* maximum amount of the resource in the lossless
                               headroom. */
...
#define BCM_COSQ_PFC_GPORT_COUNT    16         
...
#define BCM_COSQ_INGRESS_PORT_DROP_ETH_MAP_KEY(pcp, dei)  ( ((pcp << 1) | dei) & 0xF ) 
#define BCM_COSQ_INGRESS_PORT_DROP_TM_MAP_KEY(tc, dp)  ( ((tc << 2) | dp) & 0x3F ) 
