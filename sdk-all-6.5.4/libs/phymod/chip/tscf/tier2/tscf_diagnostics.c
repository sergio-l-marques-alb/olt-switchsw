    ...
    for (i=0; i< PATTERN_MAX_SIZE; i++) 
    {
      ...
      { 
        ...
        case (0):
            ...
            break;
        default:
            PHYMOD_DEBUG_ERROR(("Pattern format error!"));
            break;
       }
      }
       if (i*32+j == pattern->pattern_len)
...
int tscf_phy_pmd_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    ...
    if (!cmd_str) {
        ...
        type = (uint32)TEFMOD_DIAG_ALL;
    } else if (!PHYMOD_STRCMP(cmd_str,"STD")){
        type = (uint32)TEFMOD_DIAG_DSC_STD;
    } else {
        type = (uint32)TEFMOD_DIAG_STATE;
    }
...
int tscf_phy_pcs_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    phymod_phy_access_t phy_copy;
...
{
    PHYMOD_IF_ERR_RETURN(tefmod_fec_correctable_counter_get(&phy->access, count));
    return PHYMOD_E_NONE;
}

int tscf_phy_fec_uncorrectable_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    PHYMOD_IF_ERR_RETURN(tefmod_fec_uncorrectable_counter_get(&phy->access, count));
    return PHYMOD_E_NONE;
}
