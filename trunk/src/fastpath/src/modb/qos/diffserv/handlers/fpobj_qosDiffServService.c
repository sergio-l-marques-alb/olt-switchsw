...
xLibRC_t fpObjSet_qosDiffServService_InPolicyAdd (void *wap, void *bufp)
{
  ...
  if (usmDbDiffServServiceGet(L7_UNIT_CURRENT, keyIfIndexValue, keyIfDirectionValue) != L7_SUCCESS)
  {
    if (usmDbDiffServIsAclIntfInUse(L7_UNIT_CURRENT, keyIfIndexValue, keyIfDirectionValue ) == L7_TRUE &&
      usmDbFeaturePresentCheck( L7_UNIT_CURRENT, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
    ...
    if (rc == L7_FAILURE)
    {
          owa.rc = XLIBRC_DIFFSERV_ACL_FAILURE;
          return owa.rc;
    }
