...
#define L7_TIMERANGE_NAME_SIZE    32
#define L7_CLI_MAX_STRING_LENGTH  256

#define L7_CLI_MAX_SECURITY_PROTOCOL_STRING  10
#define L7_CLI_MAX_STRING_DURATION  10
...
typedef struct
{
  ...
  unsigned int ifTypeMask;
}ews_ifContext_t;

typedef struct
{
  /* internal interface number  */
  unsigned int vlanPorts[L7_MAX_VLANS];
  unsigned int vlanPorts_count;
}ews_vlanContext_t;
...
typedef struct
{
  char mailServerAddr[L7_HOST_NAME_LEN_MAX];
} ews_mailServerContext_t;
...
/* Structure to save the Wireless WDS Group ID info */
typedef struct
{
unsigned long groupId;
}ews_wirelessWdsApGroupConfigContext_t;
...
/* Structure to save Device Location building info */
typedef struct
{
  unsigned long bldngNum;
}ews_wirelessDevLocBldngConfigContext_t;

/* Structure to save Device Location floor info */
typedef struct
{
  unsigned long bldngNum;
  unsigned long flrNum;
}ews_wirelessDevLocFlrConfigContext_t;

/* Structure to save Dot1x User Id */
typedef struct
{
  unsigned long userIdx;
}ews_dot1xAuthServUserConfigContext_t;

/* Structure to save the time range mode info*/
typedef struct
{
  char strTimeRangeName[L7_TIMERANGE_NAME_SIZE];
}ews_timeRangeContext_t;
