...
{
  EwsCliCommandP depth2, depth2a, depth3, depth4, depth5, depth6, depth7, depth8, depth9, depth10;
#ifdef OSPF_STUB_ROUTER
  EwsCliCommandP treeRouterLsa;
  L7_char8 startupRange[L7_CLI_MAX_STRING_LENGTH];
  L7_char8 sumMetricRange[L7_CLI_MAX_STRING_LENGTH];
#endif
  L7_char8 buf[L7_CLI_MAX_STRING_LENGTH];
  L7_char8 help[L7_CLI_MAX_STRING_LENGTH];

#ifdef L7_NSF_PACKAGE    
  L7_char8 gracePeriodRange[L7_CLI_MAX_STRING_LENGTH];
#endif
...
#ifdef OSPF_STUB_ROUTER
  /* Feature commented out. Brought in from a customer branch, but not included 
   * in FP 6.3. No web or SNMP support yet. */
  /* max-metric router-lsa */
  depth2 = ewsCliAddNode(depth1, "max-metric", "Originate router LSAs with maximum link metrics.", NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  treeRouterLsa = ewsCliAddNode(depth2, "router-lsa", "Originate router LSAs with maximum link metrics.", commandMaxMetric, 
                                4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  depth4 = ewsCliAddNode(treeRouterLsa, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);

  /* create nodes for the normal syntax: on-startup <duration> */
  depth4 = ewsCliAddNode(treeRouterLsa, "on-startup", "Originate maximum link metrics on startup.", NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, 
                         L7_NO_COMMAND_SUPPORTED, L7_STATUS_NORMAL_ONLY);
  osapiSnprintf(startupRange, sizeof(startupRange), "<%d-%d> ", 
                L7_OSPF_STUB_ROUTER_DURATION_MIN, L7_OSPF_STUB_ROUTER_DURATION_MAX);
  depth5 = ewsCliAddNode(depth4, startupRange, "Enter length of startup period in seconds.", NULL, 
                         6, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, 
                         L7_OPTIONS_NODE, treeRouterLsa, L7_NO_COMMAND_SUPPORTED, L7_STATUS_NORMAL_ONLY);

  /* create nodes for the NO form syntax: on-startup (parameter <duration> not allowed) */
  depth4 = ewsCliAddNode(treeRouterLsa, "on-startup", "Originate maximum link metrics on startup.", NULL, 
                         6, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_OPTIONS_NODE, 
                         treeRouterLsa, L7_NO_COMMAND_SUPPORTED, L7_STATUS_NO_ONLY);

  depth4 = ewsCliAddNode(treeRouterLsa, "summary-lsa", "Override metric in summary LSAs.", NULL, 
                         6, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, 
                         L7_OPTIONS_NODE, treeRouterLsa, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH); 
  osapiSnprintf(sumMetricRange, sizeof(sumMetricRange), "<%d-%d> ", L7_OSPF_STUB_ROUTER_SUMLSA_MET_MIN, 
                L7_OSPF_STUB_ROUTER_SUMLSA_MET_MAX);
  depth5 = ewsCliAddNode(depth4, sumMetricRange, "Metric to use in summary LSAs", NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_OPTIONS_NODE, treeRouterLsa);
#endif   /* OSPF_STUB_ROUTER */

  /* (no) nsf [planned-only] */
  depth2 = ewsCliAddNode(depth1, "nsf", "Configure graceful restart", commandOspfNsf, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
#ifdef L7_NSF_PACKAGE
  depth3 = ewsCliAddNode(depth2, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  depth3 = ewsCliAddNode(depth2, "planned-only", "Do graceful restart only for planned restarts", NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
  depth4 = ewsCliAddNode(depth3, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);

  /* (no) nsf restart-interval <seconds> */
  depth3 = ewsCliAddNode(depth2, "restart-interval", "Configure the grace period", commandNsfRestartInterval,
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  osapiSnprintf(gracePeriodRange, L7_CLI_MAX_STRING_LENGTH, "<%d-%d>", 
                L7_OSPF_RESTART_INTERVAL_MIN, L7_OSPF_RESTART_INTERVAL_MAX);
  depth4 = ewsCliAddNode(depth3, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_NO_ONLY);
  depth4 = ewsCliAddNode(depth3, gracePeriodRange, "Length of the grace period in seconds", NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
  depth5 = ewsCliAddNode(depth4, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);

#endif
  /* (no) nsf helper [ planned-only ] */
  depth3 = ewsCliAddNode(depth2, "helper", "Configure graceful restart helpful neighbor", commandNsfHelper,
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  depth4 = ewsCliAddNode(depth3, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  depth4 = ewsCliAddNode(depth3, "planned-only", "Be helpful only for planned restarts", NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
  depth5 = ewsCliAddNode(depth4, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
  depth4 = ewsCliAddNode(depth3, "disable", "Disable helpful neighbor support", commandNsfHelperDisable,
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
  depth5 = ewsCliAddNode(depth4, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);

  /* (no) nsf helper strict-lsa-checking */
  depth4 = ewsCliAddNode(depth3, "strict-lsa-checking", "Terminate graceful restart helper on topology change", 
                         commandNsfHelperStrictLsaChecking,
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  depth5 = ewsCliAddNode(depth4, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);

  /* Allow optional "ietf" keyword to accept cisco syntax */
  depth2a = ewsCliAddNode(depth2, "ietf", "Configure graceful restart", commandOspfNsf, 
                          4, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID, L7_NO_COMMAND_SUPPORTED, L7_STATUS_BOTH);
  ewsCliAddTree(depth2a, depth2);
#ifdef L7_NSF_PACKAGE
  depth3 = ewsCliAddNode(depth2a, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 
                         2, L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
#endif
...
{
  EwsCliCommandP clearIpOspf, clearIpOspfRedist, clearIpOspfCounters, clearIpOspfStubRouter;
  ...
  /* clear ip ospf counters */
  clearIpOspfStubRouter = ewsCliAddNode(clearIpOspf, "stub-router", "Exit stub router mode",
                                      commandClearIpOspfStubRouter, 2, L7_KEY_RESTRICTED_NODE, 
                                      L7_OSPF_MAP_COMPONENT_ID);
  cr = ewsCliAddNode(clearIpOspfStubRouter, pStrInfo_common_Cr, pStrInfo_common_NewLine, NULL, 2, 
                     L7_KEY_RESTRICTED_NODE, L7_OSPF_MAP_COMPONENT_ID);
