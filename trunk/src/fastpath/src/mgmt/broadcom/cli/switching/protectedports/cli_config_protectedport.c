                                                     ...
                                                     const L7_char8 * * argv, L7_uint32 index)
{
 ...
 if (cliIsAlphaNum(name) != L7_SUCCESS)
  ...
  else if (ewsContext->commType == CLI_NO_CMD)
  {
    ...
    if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
    {
      if (usmdbProtectedPortGroupIntfMaskGet(unit, groupId, &intIfMask) == L7_SUCCESS)
      {
        rc = usmDbIntIfNumTypeFirstGet(unit, USM_PHYSICAL_INTF, 0, &interface);
          while (rc == L7_SUCCESS)
          {
            if(L7_INTF_ISMASKBITSET(intIfMask, interface))
            {
            ...
            rc = usmDbIntIfNumTypeNextGet(unit, USM_PHYSICAL_INTF, 0,interface, &interface);
          }
      }
      /* Set the name */
      rc = usmdbProtectedPortGroupNameSet(unit,groupId,name);
                           ...
                           const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (usmDbFeaturePresentCheck(0, L7_PROTECTED_PORT_COMPONENT_ID,
                                 L7_PROTECTED_PORT_MAX_GROUPS) == L7_TRUE )
  {
    if (numArg != 1)
    {
  ...
  for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
  {
    ...
    if (unit == 0)
    {
      ...
      else
      {
        status = L7_FAILURE;
        ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_ErrCouldNot, ewsContext, pStrErr_common_UnitId_1);
        continue;
      }
    }
    ...
    if(usmdbProtectedPortIntfIsValid(unit,interface) != L7_SUCCESS)
    {
      status = L7_FAILURE;
      ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
      ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrErr_switching_InvalidProtectedPort);
      continue;
    }
    ...
    if (ewsContext->commType == CLI_NORMAL_CMD)
    {
      ...
      if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          ...
          {
            /* Failed to change the mode */
            ewsTelnetWriteAddBlanks (1, 2, 0, 0, L7_NULLPTR, ewsContext,
