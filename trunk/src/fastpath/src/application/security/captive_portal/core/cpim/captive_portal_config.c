...
L7_RC_t cpNewOprData(void) 
{
  ...
  do 
  {
    ...
    {
      avlTreeTables_t *treeHeap = L7_NULLPTR;
      void            *dataHeap = L7_NULLPTR;

      treeHeap = (avlTreeTables_t *)osapiMalloc(L7_FLEX_CAPTIVE_PORTAL_COMPONENT_ID,
                                                sizeof(avlTreeTables_t)*CP_CLIENT_CONN_STATUS_MAX);
      dataHeap = osapiMalloc(L7_FLEX_CAPTIVE_PORTAL_COMPONENT_ID,
      ...
      memset((void *)dataHeap, 0, (sizeof(cpdmCkptData_t))*CP_CLIENT_CONN_STATUS_MAX);
      avlCreateAvlTree(&cpdmOprData->cpCkptTree, treeHeap, dataHeap, CP_CLIENT_CONN_STATUS_MAX,
                       (L7_uint32)sizeof(cpdmCkptData_t), 0x10, sizeof(L7_enetMacAddr_t));
                         ...
                         L7_CP_MODE_REASON_t * pCode )
{
  ...
  do 
  {
    ...
    *pCode = L7_CP_MODE_REASON_ADMIN;   
    ...
    /* 
     CP_DLOG(CPD_LEVEL_LOG, "..select call parms: addr, rMode, ifNum, inUse, reason = 0x%08x, %d, %d, %d, %d",
    ...
    */
    ...
    if (L7_FAILURE == rc)
    {
      ...
      /*  CP_DLOG(CPD_LEVEL_LOG, "..addr select failed, reason = %d", reason); */
    }
  } while(0);
  return rc;
}
...
L7_RC_t  cpSetOprMode(L7_uint32 enableOrDisable)
{
  ...
  do 
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: garbage in: %u\n", __FUNCTION__, enableOrDisable);
      break;
    }
    if (L7_NULLPTR == cpdmCfgData)
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: no cfg data?\n", __FUNCTION__);
      break;
    }
