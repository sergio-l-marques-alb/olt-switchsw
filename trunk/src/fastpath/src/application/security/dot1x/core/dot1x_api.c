/*********************************************************************
*
* (C) Copyright Broadcom Corporation 2003-2007
*
**********************************************************************
* @filename  dot1x_api.c
*
* @purpose   dot1x APIs
*
* @component dot1x
*
* @comments  none
*
* @create    04/08/2003
*
* @author    mfiorito
*
* @end
**********************************************************************/

#include "dot1x_include.h"
#include "dot1q_api.h"
#include "dot1ad_l2tp_api.h"

extern L7_VLAN_MASK_t dot1xGuestVlanMask;
extern L7_ushort16 *dot1xGuestVlanIntfCount;

/*********************************************************************
* @purpose  Set administrative mode setting for dot1x
*
* @param    adminMode @b{(input)} dot1x admin mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xAdminModeSet(L7_uint32 adminMode)
{
  L7_uint32 event;

  if (dot1xCfg->dot1xAdminMode == adminMode)
    return L7_SUCCESS;

  if (adminMode == L7_ENABLE)
  {
    dot1xCfg->dot1xAdminMode = L7_ENABLE;
    event = dot1xMgmtAdminModeEnable;
  }
  else 
  {
    dot1xCfg->dot1xAdminMode = L7_DISABLE;
    event = dot1xMgmtAdminModeDisable;
  }

  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(event, L7_NULL, &adminMode);
}

/*********************************************************************
* @purpose  Get administrative mode setting for dot1x
*
* @param    *adminMode @b{(output)} dot1x admin mode
*
* @returns  L7_SUCCESS
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xAdminModeGet(L7_uint32 *adminMode)
{
  *adminMode = dot1xCfg->dot1xAdminMode;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get initialize value for a port
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    *initialize @b{(output)} initialize value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value indicates whether a port is being initialized
*           due to a management request
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortInitializeGet(L7_uint32 intIfNum, L7_BOOL *initialize)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Setting this value to TRUE causes an action.  It is not actually
   * stored in the config structure.  So, just return what's stored
   * in the operational structure.
   */
  *initialize = dot1xPortInfo[intIfNum].initialize;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set initialize value for a port
*
* @param    intIfNum   @b{(input)} internal interface number
* @param    initialize @b{(input)} initialize value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value is set to L7_TRUE by management in order to force
*           initialization of a port.  It is re-set to L7_FALSE after
*           initialization has completed.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortInitializeSet(L7_uint32 intIfNum, L7_BOOL initialize)
{
  dot1xPortCfg_t *pCfg;
  L7_RC_t rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Admin mode must be Enable and Port Mode must be Auto (i.e. not ForceAuthorized
   * or ForceUnauthorized)
   */
  if (dot1xCfg->dot1xAdminMode != L7_ENABLE || 
      dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_FORCE_AUTHORIZED ||
      dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_FORCE_UNAUTHORIZED )
    return L7_FAILURE;
 
  if (dot1xPortInfo[intIfNum].paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
    return L7_NOT_SUPPORTED;

  if (initialize == L7_TRUE)
    rc = dot1xIssueCmd(dot1xMgmtPortInitializeSet, intIfNum, &initialize);

  return rc;
}

/*********************************************************************
* @purpose  Get reauthentication value for a port
*
* @param    intIfNum        @b{(input)} internal interface number
* @param    *reauthenticate @b{(output)} reauthentication value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value indicates whether a port is being reauthenticated
*           due to a management request.  Per the MIB, this object always
*           returns FALSE when read.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReauthenticateGet(L7_uint32 intIfNum, L7_BOOL *reauthenticate)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Per the MIB, this object always returns FALSE when read */
  *reauthenticate = L7_FALSE;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set reauthentication value for a port
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    reauthenticate @b{(input)} reauthentication value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value is set to L7_TRUE by management in order to force
*           reauthentication of a port.  It is re-set to L7_FALSE after
*           reauthentication has completed.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReauthenticateSet(L7_uint32 intIfNum, L7_BOOL reauthenticate)
{
  dot1xPortCfg_t *pCfg;
  L7_RC_t rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Admin mode must be Enable and Port Mode must be Auto (i.e. not ForceAuthorized
   * or ForceUnauthorized)
   */
  if (dot1xCfg->dot1xAdminMode != L7_ENABLE ||
      dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_FORCE_AUTHORIZED
      || dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_FORCE_UNAUTHORIZED)
    return L7_FAILURE;

  if(dot1xPortInfo[intIfNum].paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  
  if (reauthenticate == L7_TRUE)
    rc = dot1xIssueCmd(dot1xMgmtPortReauthenticateSet, intIfNum, &reauthenticate);

  return rc;
}

/*********************************************************************
* @purpose  Get admin value of controlled directions
*
* @param    intIfNum                   @b{(input)} internal interface number
* @param    *adminControlledDirections @b{(output)} controlled directions value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments Since uni-directional control is currently not supported,
*           this will always return L7_DOT1X_PORT_DIRECTION_BOTH.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAdminControlledDirectionsGet(L7_uint32 intIfNum,
                                              L7_DOT1X_PORT_DIRECTION_t *adminControlledDirections)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *adminControlledDirections = pCfg->adminControlledDirections;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set admin value of controlled directions
*
* @param    intIfNum                  @b{(input)} internal interface number
* @param    adminControlledDirections @b{(input)} controlled directions value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments Uni-directional control is currently not supported.
*           The dot1xAuthAdminControlledDirections MIB object is supported as
*           read-only.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAdminControlledDirectionsSet(L7_uint32 intIfNum,
                                              L7_DOT1X_PORT_DIRECTION_t adminControlledDirections)
{
  return L7_NOT_SUPPORTED;
}

/*********************************************************************
* @purpose  Get port control mode
*
* @param    intIfNum     @b{(input)} internal interface number
* @param    *portControl @b{(output)} port control mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortControlModeGet(L7_uint32 intIfNum, L7_DOT1X_PORT_CONTROL_t *portControl)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *portControl = pCfg->portControlMode;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set port control mode
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    portControl @b{(input)} port control mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortControlModeSet(L7_uint32 intIfNum, L7_DOT1X_PORT_CONTROL_t portControl)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->portControlMode = portControl;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortControlModeSet, intIfNum, &portControl);
}

/*********************************************************************
* @purpose  Get quiet period value
*
* @param    intIfNum     @b{(input)} internal interface number
* @param    *quietPeriod @b{(output)} quiet period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The quietTime is the initialization value for quietWhile,
*           which is a timer used by the Authenticator state machine
*           to define periods of time in which it will not attempt to
*           acquire a Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortQuietPeriodGet(L7_uint32 intIfNum, L7_uint32 *quietPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *quietPeriod = pCfg->quietPeriod;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set quiet period value
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    quietPeriod @b{(input)} quiet period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The quietPeriod is the initialization value for quietWhile,
*           which is a timer used by the Authenticator state machine
*           to define periods of time in which it will not attempt to
*           acquire a Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortQuietPeriodSet(L7_uint32 intIfNum, L7_uint32 quietPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  pCfg->quietPeriod = quietPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortQuietPeriodSet, intIfNum, &quietPeriod);
}

/*********************************************************************
* @purpose  Get tx period value
*
* @param    intIfNum  @b{(input)} internal interface number
* @param    *txPeriod @b{(output)} tx period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The txPeriod is the initialization value for txWhen,
*           which is a timer used by the Authenticator state machine
*           to determine when to send an EAPOL EAP Request/Identity
*           frame to the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortTxPeriodGet(L7_uint32 intIfNum, L7_uint32 *txPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *txPeriod = pCfg->txPeriod;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set tx period value
*
* @param    intIfNum @b{(input)} internal interface number
* @param    txPeriod @b{(input)} tx period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The txPeriod is the initialization value for txWhen,
*           which is a timer used by the Authenticator state machine
*           to determine when to send an EAPOL EAP Request/Identity
*           frame to the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortTxPeriodSet(L7_uint32 intIfNum, L7_uint32 txPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->txPeriod = txPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortTxPeriodSet, intIfNum, &txPeriod);
}


/*********************************************************************
* @purpose  Get Supplicant timeout value
*
* @param    intIfNum     @b{(input)} internal interface number
* @param    *suppTimeout @b{(output)} Supplicant timeout
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The suppTimeout is the initialization value for aWhile,
*           which is a timer used by the Authenticator state machine
*           to time out the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortSuppTimeoutGet(L7_uint32 intIfNum, L7_uint32 *suppTimeout)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *suppTimeout = pCfg->suppTimeout;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set Supplicant timeout value
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    suppTimeout @b{(input)} Supplicant timeout
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The suppTimeout is the initialization value for aWhile,
*           which is a timer used by the Authenticator state machine
*           to time out the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortSuppTimeoutSet(L7_uint32 intIfNum, L7_uint32 suppTimeout)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->suppTimeout = suppTimeout;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortSuppTimeoutSet, intIfNum, &suppTimeout);
}

/*********************************************************************
* @purpose  Get Authentiation Server timeout value
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *serverTimeout @b{(output)} Authentication Server timeout
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The serverTimeout is the initialization value for aWhile,
*           which is a timer used by the Authenticator state machine
*           to time out the Authentiation Server.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortServerTimeoutGet(L7_uint32 intIfNum, L7_uint32 *serverTimeout)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *serverTimeout = pCfg->serverTimeout;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set Authentiation Server timeout value
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    serverTimeout @b{(input)} Authentication Server timeout
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The serverTimeout is the initialization value for aWhile,
*           which is a timer used by the Authenticator state machine
*           to time out the Authentiation Server.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortServerTimeoutSet(L7_uint32 intIfNum, L7_uint32 serverTimeout)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->serverTimeout = serverTimeout;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortServerTimeoutSet, intIfNum, &serverTimeout);
}

/*********************************************************************
* @purpose  Get Maximum Request value
*
* @param    intIfNum @b{(input)} internal interface number
* @param    *maxReq  @b{(output)} maximum request value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The maxReq value is the number of times the Authenticator
*           state machine will retransmit an EAPOL EAP Request/Identity
*           before timing out the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMaxReqGet(L7_uint32 intIfNum, L7_uint32 *maxReq)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *maxReq = pCfg->maxReq;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set Maximum Request value
*
* @param    intIfNum @b{(input)} internal interface number
* @param    maxReq   @b{(input)} maximum request value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The maxReq value is the number of times the Authenticator
*           state machine will retransmit an EAPOL EAP Request/Identity
*           before timing out the Supplicant.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMaxReqSet(L7_uint32 intIfNum, L7_uint32 maxReq)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->maxReq = maxReq;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortMaxReqSet, intIfNum, &maxReq);
}

/*********************************************************************
* @purpose  Get the Reauthentication period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *reAuthPeriod  @b{(output)} reauthentication period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The reAuthPeriod is the initialization value for reAuthWhen,
*           which is a timer used by the Authenticator state machine to
*           determine when reauthentication of the Supplicant takes place.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReAuthPeriodGet(L7_uint32 intIfNum, L7_uint32 *reAuthPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *reAuthPeriod = pCfg->reAuthPeriod;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the Reauthentication period
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    reAuthPeriod  @b{(input)} reauthentication period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The reAuthPeriod is the initialization value for reAuthWhen,
*           which is a timer used by the Authenticator state machine to
*           determine when reauthentication of the Supplicant takes place.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReAuthPeriodSet(L7_uint32 intIfNum, L7_uint32 reAuthPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->reAuthPeriod = reAuthPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortReAuthPeriodSet, intIfNum, &reAuthPeriod);
}

/*********************************************************************
* @purpose  Get the Reauthentication mode
*
* @param    intIfNum        @b{(input)} internal interface number
* @param    *reAuthEnabled  @b{(output)} reauthentication mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The reAuthEnabled mode determines whether reauthentication
*           of the Supplicant takes place.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReAuthEnabledGet(L7_uint32 intIfNum, L7_BOOL *reAuthEnabled)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *reAuthEnabled = pCfg->reAuthEnabled;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the Reauthentication mode
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    reAuthEnabled  @b{(input)} reauthentication mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The reAuthEnabled mode determines whether reauthentication
*           of the Supplicant takes place.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortReAuthEnabledSet(L7_uint32 intIfNum, L7_BOOL reAuthEnabled)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->reAuthEnabled = reAuthEnabled;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortReAuthEnabledSet, intIfNum, &reAuthEnabled);
}

/*********************************************************************
* @purpose  Get the Key Transmission mode
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *keyTxEnabled @b{(output)} key transmission mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The keyTranmissionEnabled mode determines whether key
*           transmission to the Supplicant takes place.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortKeyTransmissionEnabledGet(L7_uint32 intIfNum, L7_BOOL *keyTxEnabled)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *keyTxEnabled = pCfg->keyTxEnabled;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the Key Transmission mode
*
* @param    intIfNum     @b{(input)} internal interface number
* @param    keyTxEnabled @b{(input)} key transmission mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The keyTranmissionEnabled mode determines whether key
*           transmission to the Supplicant takes place.  Note that
*           key transmission is not supported.  The dot1xAuthKeyTxEnabled
*           MIB object is supported as read-only.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortKeyTransmissionEnabledSet(L7_uint32 intIfNum, L7_BOOL keyTxEnabled)
{
  return L7_NOT_SUPPORTED;
}

/*********************************************************************
* @purpose  Get the dot1x protocol version of a port
*
* @param    intIfNum         @b{(input)} internal interface number
* @param    *protocolVersion @b{(output)} dot1x protocol version of this port
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortProtocolVersionGet(L7_uint32 intIfNum,
                                    L7_DOT1X_PAE_PORT_PROTOCOL_VERSION_t *protocolVersion)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *protocolVersion = dot1xPortInfo[intIfNum].paeProtocolVersion;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the port's dot1x capabilities (Supplicant and/or Authenticator)
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *capabilities @b{(output)} dot1x capabilities bitmask
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortPaeCapabilitiesGet(L7_uint32 intIfNum, L7_uchar8 *capabilities)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *capabilities = pCfg->paeCapabilities;

 return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the port's dot1x capabilities (Supplicant or Authenticator)
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    paeCapabilities  @b{(input)} dot1x capabilities bitmask
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xPortPaeCapabilitiesSet(L7_uint32 intIfNum,
                                    L7_uchar8 paeCapabilities)
{
  dot1xPortCfg_t *pCfg;
  L7_uint32 tmpCapabilities, session;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if ( pCfg->paeCapabilities == paeCapabilities)
  {
     return L7_SUCCESS;
  }
  if ( (paeCapabilities != L7_DOT1X_PAE_PORT_AUTH_CAPABLE)&&
       (paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
     )
  {
     return L7_FAILURE;
  }
  if ( (paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)&&
       ( (dot3adIsLagMember(intIfNum) == L7_TRUE) || 
         (mirrorIsDestConfigured(intIfNum, &session) == L7_TRUE)
       )
     )
  {
     return L7_FAILURE;
  }

  #if defined(FEAT_METRO_CPE_V1_0)
  #if defined L7_DOT1AD_PACKAGE
  {
    L7_uint32 nniIntf;
    dot1adNniInterfaceGet(&nniIntf);
    if ( ((nniIntf==intIfNum) &&
          (paeCapabilities == L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
         ) ||
         ((nniIntf!=intIfNum) && 
          (paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
         )
      )
    {
      return L7_FAILURE;
    }
  }
 #endif
 #endif



  DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS,intIfNum,"%s: PAE capability set to %d on  port-%d \n",
                    __FUNCTION__,paeCapabilities,intIfNum);

  pCfg->paeCapabilities = paeCapabilities;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  if (dot1xCfg->dot1xAdminMode == L7_ENABLE)
  {
    tmpCapabilities = paeCapabilities;
    return dot1xIssueCmd(dot1xMgmtPortPaeCapabilitySet, intIfNum, &tmpCapabilities);
  }
  else
  {
    return L7_SUCCESS;
  }

}

/*********************************************************************
* @purpose  Get the current state of the port's Authenticator PAE state machine
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *authPaeState @b{(output)} Authenticator PAE state
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthPaeStateGet(L7_uint32 intIfNum, L7_DOT1X_APM_STATES_t *authPaeState)
{
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *authPaeState = logicalPortInfo->apmState + 1;
  }
  else
  {
      rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Get the current state of the port's Backend Authentication state machine
*
* @param    intIfNum          @b{(input)} internal interface number
* @param    *backendAuthState @b{(output)} Backend Authentication state
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortBackendAuthStateGet(L7_uint32 intIfNum, L7_DOT1X_BAM_STATES_t *backendAuthState)
{
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_RC_t  rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *backendAuthState = logicalPortInfo->bamState + 1;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();

  return rc;
}

/*********************************************************************
* @purpose  Get operational value of controlled directions
*
* @param    intIfNum                    @b{(input)} internal interface number
* @param    *operControlledDirections   @b{(output)} controlled directions value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments Since uni-directional control is currently not supported,
*           this will always return L7_DOT1X_PORT_DIRECTION_BOTH.
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortOperControlledDirectionsGet(L7_uint32 intIfNum,
                                             L7_DOT1X_PORT_DIRECTION_t *operControlledDirections)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *operControlledDirections = dot1xPortInfo[intIfNum].operControlledDirections;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get operational value of controlled directions
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *portStatus   @b{(output)} port authentication status
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthControlledPortStatusGet(L7_uint32 intIfNum, L7_DOT1X_PORT_STATUS_t *portStatus)
{
  if (dot1xPortInfo == L7_NULLPTR)
    return L7_FAILURE;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if( dot1xPortInfo[intIfNum].portEnabled == L7_TRUE)
  {
    *portStatus = dot1xPortInfo[intIfNum].portStatus;
  } 
  else
  {
    *portStatus  = L7_DOT1X_PORT_STATUS_NA;
  }
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAPOL frames received
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolFramesRx @b{(output)} EAPOL frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapolFramesRx(L7_uint32 intIfNum, L7_uint32 *eapolFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapolFramesRx = dot1xPortStats[intIfNum].authEapolFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAPOL frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolFramesTx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapolFramesTx(L7_uint32 intIfNum, L7_uint32 *eapolFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapolFramesTx = dot1xPortStats[intIfNum].authEapolFramesTx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAPOL Start frames received
*
* @param    intIfNum            @b{(input)} internal interface number
* @param    *eapolStartFramesRx @b{(output)} EAPOL start frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapolStartFramesRx(L7_uint32 intIfNum, L7_uint32 *eapolStartFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapolStartFramesRx = dot1xPortStats[intIfNum].authEapolStartFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAPOL Logoff frames received
*
* @param    intIfNum             @b{(input)} internal interface number
* @param    *eapolLogoffFramesRx @b{(output)} EAPOL logoff frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapolLogoffFramesRx(L7_uint32 intIfNum, L7_uint32 *eapolLogoffFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapolLogoffFramesRx = dot1xPortStats[intIfNum].authEapolLogoffFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP Response/Identity frames received
*
* @param    intIfNum           @b{(input)} internal interface number
* @param    *eapRespIdFramesRx @b{(output)} EAP Resp/Id frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapRespIdFramesRx(L7_uint32 intIfNum, L7_uint32 *eapRespIdFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapRespIdFramesRx = dot1xPortStats[intIfNum].authEapRespIdFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP Response frames received
*
* @param    intIfNum             @b{(input)} internal interface number
* @param    *eapResponseFramesRx @b{(output)} EAP Response frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapResponseFramesRx(L7_uint32 intIfNum, L7_uint32 *eapResponseFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapResponseFramesRx = dot1xPortStats[intIfNum].authEapResponseFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP Request/Identity frames transmitted
*
* @param    intIfNum          @b{(input)} internal interface number
* @param    *eapReqIdFramesTx @b{(output)} EAP Req/Id frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapReqIdFramesTx(L7_uint32 intIfNum, L7_uint32 *eapReqIdFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapReqIdFramesTx = dot1xPortStats[intIfNum].authEapReqIdFramesTx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP Request frames transmitted
*
* @param    intIfNum        @b{(input)} internal interface number
* @param    *eapReqFramesTx @b{(output)} EAP Request frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapReqFramesTx(L7_uint32 intIfNum, L7_uint32 *eapReqFramesTx)
{
 if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapReqFramesTx = dot1xPortStats[intIfNum].authEapReqFramesTx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of invalid EAPOL frames received
*
* @param    intIfNum              @b{(input)} internal interface number
* @param    *invalidEapolFramesRx @b{(output)} Invalid EAPOL frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortInvalidEapolFramesRx(L7_uint32 intIfNum, L7_uint32 *invalidEapolFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *invalidEapolFramesRx = dot1xPortStats[intIfNum].authInvalidEapolFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP length error frames received
*
* @param    intIfNum                @b{(input)} internal interface number
* @param    *eapLengthErrorFramesRx @b{(output)} EAP length error frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortEapLengthErrorFramesRx(L7_uint32 intIfNum, L7_uint32 *eapLengthErrorFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *eapLengthErrorFramesRx = dot1xPortStats[intIfNum].authEapLengthErrorFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get protocol version number carried in the most recently
*           received EAPOL frame
*
* @param    intIfNum               @b{(input)} internal interface number
* @param    *lastEapolFrameVersion @b{(output)} last EAPOL version received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortLastEapolFrameVersion(L7_uint32 intIfNum, L7_uint32 *lastEapolFrameVersion)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *lastEapolFrameVersion = dot1xPortStats[intIfNum].authLastEapolFrameVersion;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the source MAC address carried in the most recently
*           recently received EAPOL frame
*
* @param    intIfNum               @b{(input)} internal interface number
* @param    *lastEapolFrameVersion @b{(output)} last source MAC address
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortLastEapolFrameSourceGet(L7_uint32 intIfNum, L7_enetMacAddr_t *lastEapolFrameSource)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  memcpy(lastEapolFrameSource->addr,
         dot1xPortStats[intIfNum].authLastEapolFrameSource.addr,
         L7_MAC_ADDR_LEN);
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get port operational mode
*
* @param    intIfNum     @b{(input)} internal interface number
* @param    *portMode    @b{(output)} port operational mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortOperControlModeGet(L7_uint32 intIfNum, L7_DOT1X_PORT_CONTROL_t *portMode)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if(dot1xPortInfo[intIfNum].portEnabled == L7_TRUE)
    *portMode = dot1xPortInfo[intIfNum].portControlMode;
  else
    *portMode = L7_DOT1X_PORT_NA; 
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Clear dot1x stats for specified port
*
* @param    intIfNum     @b{(input)} internal interface number
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortStatsClear(L7_uint32 intIfNum)
{
  L7_RC_t rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  rc = dot1xIssueCmd(dot1xMgmtPortStatsClear, intIfNum, L7_NULLPTR);

  return rc;
}

/*********************************************************************
* @purpose  Get number of times the FSM enters the CONNECTING state from any other state
*
* @param    intIfNum                @b{(input)} internal interface number
* @param    *authEntersConnecting   @b{(output)} number of times CONNECTING state entered
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthEntersConnectingGet(L7_uint32 intIfNum, L7_uint32 *authEntersConnecting)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authEntersConnecting = dot1xPortStats[intIfNum].authEntersConnecting;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of times the FSM transitions from  CONNECTING to DISCONNECTED
*           as a result of receiving an EAPOL-Logoff message
*
* @param    intIfNum                       @b{(input)} internal interface number
* @param    *authEapLogoffsWhileConnecting @b{(output)} number of logoffs
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthEapLogoffsWhileConnectingGet(L7_uint32 intIfNum, L7_uint32 *authEapLogoffsWhileConnecting)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authEapLogoffsWhileConnecting = dot1xPortStats[intIfNum].authEapLogoffsWhileConnecting;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from CONNECTING to AUTHENTICATING,
*           as a result of an EAP-Response/Identity message being received from the Supplicant
*
* @param    intIfNum                  @b{(input)} internal interface number
* @param    *authEntersAuthenticating @b{(output)} number of EAP-Response/Identity messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthEntersAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authEntersAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authEntersAuthenticating = dot1xPortStats[intIfNum].authEntersAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to AUTHENTICATED,
*           as a result of the Backend Authentication state machine indicating successful
*           authentication of the Supplicant
*
* @param    intIfNum                            @b{(input)} internal interface number
* @param    *authAuthSuccessWhileAuthenticating @b{(output)} number of successes
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthSuccessWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthSuccessWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthSuccessWhileAuthenticating = dot1xPortStats[intIfNum].authAuthSuccessWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to ABORTING,
*           as a result of the Backend Authentication state machine indicating authentication
*           timeout
*
* @param    intIfNum                             @b{(input)} internal interface number
* @param    *authAuthTimeoutsWhileAuthenticating @b{(output)} number of timeouts
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthTimeoutsWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthTimeoutsWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthTimeoutsWhileAuthenticating = dot1xPortStats[intIfNum].authAuthTimeoutsWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to HELD,
*           as a result of the Backend Authentication state machine indicating authentication
*           failure
*
* @param    intIfNum                         @b{(input)} internal interface number
* @param    *authAuthFailWhileAuthenticating @b{(output)} number of failures
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthFailWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthFailWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthFailWhileAuthenticating = dot1xPortStats[intIfNum].authAuthFailWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to ABORTING,
*           as a result of a reauthentication request
*
* @param    intIfNum                            @b{(input)} internal interface number
* @param    *authAuthReauthsWhileAuthenticating @b{(output)} number of reauthentication requests
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthReauthsWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthReauthsWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthReauthsWhileAuthenticating = dot1xPortStats[intIfNum].authAuthReauthsWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to ABORTING,
*           as a result of an EAPOL-Start message being received from the Supplicant
*
* @param    intIfNum                              @b{(input)} internal interface number
* @param    *authAuthEapStartsWhileAuthenticating @b{(output)} number of EAP-Start messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthEapStartsWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthEapStartsWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthEapStartsWhileAuthenticating = dot1xPortStats[intIfNum].authAuthEapStartsWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATING to ABORTING,
*           as a result of an EAPOL-Logoff message being received from the Supplicant
*
* @param    intIfNum                              @b{(input)} internal interface number
* @param    *authAuthEapLogoffWhileAuthenticating @b{(output)} number of EAP-Logoff messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthEapLogoffWhileAuthenticatingGet(L7_uint32 intIfNum, L7_uint32 *authAuthEapLogoffWhileAuthenticating)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthEapLogoffWhileAuthenticating = dot1xPortStats[intIfNum].authAuthEapLogoffWhileAuthenticating;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATED to CONNECTING,
*           as a result of a reauthentication request
*
* @param    intIfNum                           @b{(input)} internal interface number
* @param    *authAuthReauthsWhileAuthenticated @b{(output)} number of reauthentication requests
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthReauthsWhileAuthenticatedGet(L7_uint32 intIfNum, L7_uint32 *authAuthReauthsWhileAuthenticated)
{
 if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthReauthsWhileAuthenticated = dot1xPortStats[intIfNum].authAuthReauthsWhileAuthenticated;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATED to CONNECTING,
*           as a result of an EAPOL-Start message being received from the Supplicant
*
* @param    intIfNum                             @b{(input)} internal interface number
* @param    *authAuthEapStartsWhileAuthenticated @b{(output)} number of EAPOL-Start messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthEapStartsWhileAuthenticatedGet(L7_uint32 intIfNum, L7_uint32 *authAuthEapStartsWhileAuthenticated)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authAuthEapStartsWhileAuthenticated = dot1xPortStats[intIfNum].authAuthEapStartsWhileAuthenticated;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM transitions from AUTHENTICATED to DISCONNECTED,
*           as a result of an EAPOL-Logoff message being received from the Supplicant
*
* @param    intIfNum                             @b{(input)} internal interface number
* @param    *authAuthEapLogoffWhileAuthenticated @b{(output)} number of EAPOL-Logoff messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthAuthEapLogoffWhileAuthenticatedGet(L7_uint32 intIfNum, L7_uint32 *authAuthEapLogoffWhileAuthenticated)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
     return L7_FAILURE;

  *authAuthEapLogoffWhileAuthenticated = dot1xPortStats[intIfNum].authAuthEapLogoffWhileAuthenticated;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM sends an initial Access-Request packet to the
*           Authentication server
*
* @param    intIfNum              @b{(input)} internal interface number
* @param    *authBackendResponses @b{(output)} number of Access-Request packets
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendResponsesGet(L7_uint32 intIfNum, L7_uint32 *authBackendResponses)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendResponses = dot1xPortStats[intIfNum].authBackendResponses;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM receives an initial Access-Challenge packet from
*           the Authentication server
*
* @param    intIfNum                     @b{(input)} internal interface number
* @param    *authBackendAccessChallenges @b{(output)} number of Access-Challenge packets
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendAccessChallengesGet(L7_uint32 intIfNum, L7_uint32 *authBackendAccessChallenges)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendAccessChallenges = dot1xPortStats[intIfNum].authBackendAccessChallenges;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM sends an EAP-Request packet
*
* @param    intIfNum                              @b{(input)} internal interface number
* @param    *authBackendOtherRequestsToSupplicant @b{(output)} number of EAP-Request packets
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendOtherRequestsToSupplicantGet(L7_uint32 intIfNum, L7_uint32 *authBackendOtherRequestsToSupplicant)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendOtherRequestsToSupplicant = dot1xPortStats[intIfNum].authBackendOtherRequestsToSupplicant;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM receives a response from the Supplicant to an
*           initial EAP-Request, and the response is something other than EAP-NAK
*
* @param    intIfNum                                  @b{(input)} internal interface number
* @param    *authBackendNonNakResponsesFromSupplicant @b{(output)} number of non-Nak responses
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendNonNakResponsesFromSupplicantGet(L7_uint32 intIfNum, L7_uint32 *authBackendNonNakResponsesFromSupplicant)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendNonNakResponsesFromSupplicant = dot1xPortStats[intIfNum].authBackendNonNakResponsesFromSupplicant;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM receives an EAP-Success message from the
*           Authentication Server
*
* @param    intIfNum                  @b{(input)} internal interface number
* @param    *authBackendAuthSuccesses @b{(output)} number of EAP-Success messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendAuthSuccessesGet(L7_uint32 intIfNum, L7_uint32 *authBackendAuthSuccesses)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendAuthSuccesses = dot1xPortStats[intIfNum].authBackendAuthSuccesses;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the number of times the FSM receives an EAP-Failure message from the
*           Authentication Server
*
* @param    intIfNum              @b{(input)} internal interface number
* @param    *authBackendAuthFails @b{(output)} number of EAP-Success messages
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortAuthBackendAuthFailsGet(L7_uint32 intIfNum, L7_uint32 *authBackendAuthFails)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *authBackendAuthFails = dot1xPortStats[intIfNum].authBackendAuthFails;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Determine if the interface type is valid to participate in dot1x
*
* @param    sysIntfType              @b{(input)} interface type
*
* @returns  L7_TRUE
* @returns  L7_FALSE
*
* @comments
*       
* @end
*********************************************************************/
L7_BOOL dot1xIsValidIntfType(L7_uint32 sysIntfType)
{
  if (sysIntfType != L7_PHYSICAL_INTF)
    return L7_FALSE;

  return L7_TRUE;
}

/*********************************************************************
* @purpose  Determine if the interface is valid to participate in dot1x
*
* @param    intIfNum              @b{(input)} internal interface number
*
* @returns  L7_TRUE
* @returns  L7_FALSE
*
* @comments
*       
* @end
*********************************************************************/
L7_BOOL dot1xIsValidIntf(L7_uint32 intIfNum)
{
  L7_INTF_TYPES_t sysIntfType;

  if ( (nimGetIntfType(intIfNum, &sysIntfType) != L7_SUCCESS) || 
       (dot1xIsValidIntfType(sysIntfType) != L7_TRUE) )
    return L7_FALSE;

  return L7_TRUE;
}


/*********************************************************************
* @purpose  Return Internal Interface Number of next valid interface for
*           dot1x.
*
* @param    intIfNum  @b{(input)}   Internal Interface Number
* @param    pNextintIfNum @b{(output)}  pointer to Next Internal Interface Number,
*
* @returns  L7_SUCCESS  if success
* @returns  L7_ERROR    if interface does not exist
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xNextValidIntf(L7_uint32 intIfNum, L7_uint32 *pNextIntIfNum)
{
  L7_RC_t rc;

  rc = nimNextValidIntfNumberByType(L7_PHYSICAL_INTF, intIfNum, pNextIntIfNum);

  return(rc);
}

/*********************************************************************
* @purpose  Return Internal Interface Number of the first valid interface for
*           dot1x.
*
* @param    pFirstIntIfNum @b{(output)}  pointer to first internal interface number
*
* @returns  L7_SUCCESS  if success
* @returns  L7_ERROR    if interface does not exist
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xFirstValidIntfNumber(L7_uint32 *pFirstIntIfNum)
{
  L7_RC_t rc;

  rc = nimFirstValidIntfNumberByType(L7_PHYSICAL_INTF, pFirstIntIfNum);

  return(rc);

}

/*********************************************************************
* @purpose  Set administrative mode setting for dot1x Vlan Assignment
*
* @param    mode @b{(input)} radius vlan assignment mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xVlanAssignmentModeSet (L7_uint32 mode)
{
  if (dot1xCfg->vlanAssignmentMode == mode)
    return L7_SUCCESS;

  if(mode == L7_ENABLE)
  {
    dot1xCfg->vlanAssignmentMode = L7_ENABLE;
  }
  else
  {
    dot1xCfg->vlanAssignmentMode = L7_DISABLE;
  }
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get administrative mode setting for dot1x Vlan Assignment
*
* @param    mode @b{(input)} radius vlan assignment mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xVlanAssignmentModeGet(L7_uint32 *mode)
{
  *mode = dot1xCfg->vlanAssignmentMode;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set administrative mode setting for dot1x dynamic Vlan creation 
*
* @param    mode @b{(input)} radius dynamic vlan creation mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xDynamicVlanCreationModeSet (L7_uint32 mode)
{
  if (dot1xCfg->dynamicVlanMode== mode)
    return L7_SUCCESS;

  if(mode == L7_ENABLE)
  {
    dot1xCfg->dynamicVlanMode= L7_ENABLE;
  }
  else
  {
    dot1xCfg->dynamicVlanMode= L7_DISABLE;
  }
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get administrative mode setting for dot1x dynamic Vlan creation
*
* @param    mode @b{(input)} radius dynamic vlan creation mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xDynamicVlanCreationModeGet(L7_uint32 *mode)
{
  *mode = dot1xCfg->dynamicVlanMode;
  return L7_SUCCESS;
}


/*********************************************************************
* @purpose  Get the Vlan Assigned to a port by dot1x.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *vlanId       @b{(output)} vlan Id
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortVlanAssignedGet(L7_uint32 intIfNum, L7_uint32 *vlanId)
{
  dot1xPortCfg_t *pCfg;
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_RC_t  rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    if(logicalPortInfo->vlanAssigned != 0)
      *vlanId = logicalPortInfo->vlanAssigned;
    else if(logicalPortInfo->unauthVlan != 0)
      *vlanId = logicalPortInfo->unauthVlan;
    else if(logicalPortInfo->guestVlanId !=  0)
      *vlanId = logicalPortInfo->guestVlanId;
    else if(logicalPortInfo->voiceVlanId !=  0)
      *vlanId = logicalPortInfo->voiceVlanId;
    else if((logicalPortInfo->defaultVlanId != 0) && (logicalPortInfo->isMonitorModeClient == L7_TRUE))
      *vlanId = logicalPortInfo->defaultVlanId;
    else
      *vlanId = logicalPortInfo->vlanId;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Get the username that authenticated on a port.
*
* @param    intIfNum @b((input)) the specified interface
* @param    username @b((output))    Username
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xPortUsernameGet(L7_uint32 intIfNum,
                             L7_uchar8 *username)
{
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_RC_t  rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE || username == L7_NULL)
      return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR && logicalPortInfo->apmState == APM_AUTHENTICATED)
  {
    memcpy(username,logicalPortInfo->dot1xUserName,
           logicalPortInfo->dot1xUserNameLength);
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();

  return rc;
}

/*********************************************************************
* @purpose  Get the reason vlan was assigned to a port by dot1x.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *reason       @b{(output)} reason vlan was assigned to the port by dot1x
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortVlanAssignedReasonGet(L7_uint32 intIfNum, L7_uint32 *reason)
{
  dot1xPortCfg_t *pCfg;
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_RC_t  rc = L7_SUCCESS;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    if(logicalPortInfo->vlanAssigned != 0)
      *reason = L7_DOT1X_RADIUS_ASSIGNED_VLAN;
    else if(logicalPortInfo->unauthVlan != 0)
      *reason = L7_DOT1X_UNAUTHENTICATED_VLAN;
    else if(logicalPortInfo->guestVlanId != 0)
      *reason = L7_DOT1X_GUEST_VLAN;
    else if(logicalPortInfo->voiceVlanId != 0)
      *reason = L7_DOT1X_VOICE_VLAN;
    else if((logicalPortInfo->defaultVlanId != 0) && (logicalPortInfo->isMonitorModeClient == L7_TRUE))
      *reason = L7_DOT1X_MONITOR_MODE_VLAN;   
    else if(logicalPortInfo->vlanId != 0)
      *reason = L7_DOT1X_DEFAULT_ASSIGNED_VLAN;
    else
      *reason = L7_DOT1X_NOT_ASSIGNED;
  }
  else
  { 
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Set the Guest Vlan Id for the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *guestVlanId  @b{(input)} guest vlan Id
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*           L7_ERROR      if MAB has been enabled on the port
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xAdvancedGuestPortsCfgSet(L7_uint32 intIfNum,L7_uint32 guestVlanId)
{
  dot1xPortCfg_t *pCfg;
  L7_uint32 rc = L7_SUCCESS;
  L7_uint32 currentGuestVlanId = 0;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  currentGuestVlanId = pCfg->guestVlanId;

  /* Guest vlan is not supported on Supplicant ports */
  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_FAILURE;
  }

  if(guestVlanId != 0)
  {
    /* check if mab has been enabled on the port. do not allow guets vlan to be configured */
    if (pCfg->mabEnabled == L7_ENABLE)
    {
       return  L7_ERROR;
    }
    if(pCfg->guestVlanId == guestVlanId)
      return L7_SUCCESS;
    if (currentGuestVlanId == 0)
    {
      dot1xGuestVlanIntfCount[guestVlanId]++;
      L7_VLAN_SETMASKBIT(dot1xGuestVlanMask, guestVlanId);
    }
    else
    {
      dot1xGuestVlanIntfCount[currentGuestVlanId]--;
      if (dot1xGuestVlanIntfCount[currentGuestVlanId] == 0)
      {
        L7_VLAN_CLRMASKBIT(dot1xGuestVlanMask, currentGuestVlanId);
      }
      dot1xGuestVlanIntfCount[guestVlanId]++;
      L7_VLAN_SETMASKBIT(dot1xGuestVlanMask, guestVlanId);
    }
   
    pCfg->guestVlanId = guestVlanId;
    dot1xCfg->cfgHdr.dataChanged = L7_TRUE;
    rc = dot1xIssueCmd(dot1xMgmtPortGuestVlanModeEnable,intIfNum,&pCfg->guestVlanId);
    
  }
  else
  {
    if (currentGuestVlanId != 0)
    {
      dot1xGuestVlanIntfCount[currentGuestVlanId]--;
      if (dot1xGuestVlanIntfCount[currentGuestVlanId] == 0)
      {
        L7_VLAN_CLRMASKBIT(dot1xGuestVlanMask, currentGuestVlanId);
      }
    }
    pCfg->guestVlanId = guestVlanId;
    dot1xCfg->cfgHdr.dataChanged = L7_TRUE;
    rc = dot1xIssueCmd(dot1xMgmtPortGuestVlanModeDisable, intIfNum, &pCfg->guestVlanId);
    
  }

  return rc;
}
/*********************************************************************
* @purpose Get the Guest Vlan Id for the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *guestVlanId  @b{(intput)} guest vlan Id
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xAdvancedGuestPortCfgGet(L7_uint32 intIfNum,L7_uint32 *guestVlanId)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *guestVlanId = pCfg->guestVlanId;
  
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the Guest Vlan Period for the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    guestVlanPeriod  @b{(output)} guest vlan Period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xAdvancedGuestVlanPeriodSet(L7_uint32 intIfNum,L7_uint32 guestVlanPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Guest vlan is not supported on Supplicant ports */ 
  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->guestVlanPeriod = guestVlanPeriod;
  
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;
  (void) dot1xIssueCmd(dot1xMgmtPortGuestVlanPeriod, intIfNum, &guestVlanPeriod);

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the Guest Vlan Period for the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *guestVlanPeriod  @b{(output)} guest vlan Period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xAdvancedGuestVlanPeriodGet(L7_uint32 intIfNum,L7_uint32 *guestVlanPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  * guestVlanPeriod = pCfg->guestVlanPeriod;

  return L7_SUCCESS;
}


/*********************************************************************
* @purpose  Set max users value
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    maxUsers @b{(input)} max users
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The maxUsers is the maximum number of hosts that can be 
*           authenticated on a port using mac based authentication
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMaxUsersSet(L7_uint32 intIfNum, L7_uint32 maxUsers)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  pCfg->maxUsers = maxUsers;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortMaxUsersSet, intIfNum, &maxUsers);
}

/*********************************************************************
* @purpose  Get max users value
*
* @param    intIfNum  @b{(input)} internal interface number
* @param    *maxUsers @b{(output)} max users per port
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The maxUsers is the maximum number of hosts that can be 
*           authenticated on a port using mac based authentication
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMaxUsersGet(L7_uint32 intIfNum, L7_uint32 *maxUsers)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *maxUsers = pCfg->maxUsers;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set initialize value for a logical port
*
* @param    lIntIfNum   @b{(input)} Logical internal interface number
* @param    initialize @b{(input)} initialize value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value is set to L7_TRUE by management in order to force
*           initialization of a port.  It is re-set to L7_FALSE after
*           initialization has completed.
*       
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortInitializeSet(L7_uint32 lIntIfNum, L7_BOOL initialize)
{
  dot1xPortCfg_t *pCfg;
  L7_uint32 physPort;
  L7_RC_t rc = L7_SUCCESS;

  physPort = dot1xPhysPortGet(lIntIfNum);
  if (dot1xIsValidIntf(physPort) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /* Admin mode must be Enable and Port Mode must be Auto (i.e. not ForceAuthorized
   * or ForceUnauthorized and PAE capability should be Authenticator)
   */

   if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
   {
     return L7_NOT_SUPPORTED;
   }

  if (dot1xCfg->dot1xAdminMode != L7_ENABLE ||
      dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_FORCE_AUTHORIZED ||
      dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_FORCE_UNAUTHORIZED)
    return L7_FAILURE;

  if (initialize == L7_TRUE)
  {
    rc = dot1xIssueCmd(dot1xMgmtLogicalPortInitializeSet, lIntIfNum, &initialize);
  }

  return rc;
}

/*********************************************************************
* @purpose  Set reauthentication value for a logical port
*
* @param    lIntIfNum       @b{(input)} logical interface number
* @param    reauthenticate @b{(input)} reauthentication value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments This value is set to L7_TRUE by management in order to force
*           reauthentication of a port.  It is re-set to L7_FALSE after
*           reauthentication has completed.
*       
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortReauthenticateSet(L7_uint32 lIntIfNum, L7_BOOL reauthenticate)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t rc = L7_SUCCESS;

  physPort = dot1xPhysPortGet(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;
  
  /* Admin mode must be Enable and Port Mode must be Auto (i.e. not ForceAuthorized
   * or ForceUnauthorized)
   */
  if (dot1xCfg->dot1xAdminMode != L7_ENABLE ||
      dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_FORCE_AUTHORIZED ||
      dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_FORCE_UNAUTHORIZED)
    return L7_FAILURE;

  /* PAE role should be Authenticator */
  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  if (reauthenticate == L7_TRUE)
  {
    rc = dot1xIssueCmd( dot1xMgmtLogicalPortReauthenticateSet, lIntIfNum, &reauthenticate);
  }

  return rc;
}

/*********************************************************************
* @purpose  Returns Session time
*
* @param    lIntIfNum @b((input)) the specified interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_ERROR    if interface does not exist
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xPortSessionTimeGet( L7_uint32 lIntIfNum, L7_uint32 *pval)
{
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo)
  {
    *pval = osapiUpTimeRaw() - logicalPortInfo->sessionTime;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the first logical port for the physcial interface
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    lIntIfNum    @b((output)) the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortFirstGet(L7_uint32 intIfNum,L7_uint32 *lIntIfNum)
{
  dot1xPortCfg_t *pCfg;
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_uint32 next;
  L7_RC_t  rc = L7_FAILURE;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
    return rc;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return rc;

  if(dot1xCfg->dot1xAdminMode != L7_ENABLE)
    return rc;

  if (pCfg->portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED ||
      pCfg->portControlMode == L7_DOT1X_PORT_AUTO)
  {
     dot1xLogicalPortInfoTakeLock();
     logicalPortInfo = dot1xLogicalPortInfoFirstGet(intIfNum,lIntIfNum);
     if(logicalPortInfo != L7_NULLPTR)
     {
         if ((pCfg->portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED) && (logicalPortInfo->inUse != L7_TRUE)) 
         {
              /*rc = L7_FAILURE;*/
             /* try to get next valid logical Interface*/
             next = *lIntIfNum;
             logicalPortInfo = dot1xLogicalPortInfoGetNextNode(intIfNum,&next);
             while(logicalPortInfo!=L7_NULLPTR)
             {
                 if (logicalPortInfo->inUse == L7_TRUE)
                     break;
                  logicalPortInfo = dot1xLogicalPortInfoGetNextNode(intIfNum,&next);
             }
             if (logicalPortInfo != L7_NULLPTR) 
             {
                 *lIntIfNum = next;
                 rc = L7_SUCCESS;
             }
             else
                 rc = L7_FAILURE;

         }
         else
         {
             if ((pCfg->portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED) && 
                 (logicalPortInfo->inUse == L7_TRUE)&&
                 (dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)) 
                 rc = L7_SUCCESS;         
             else if ((pCfg->portControlMode == L7_DOT1X_PORT_AUTO)&& 
                      (dot1xPortInfo[intIfNum].portEnabled == L7_TRUE) &&
                      (dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO)) 
                     rc = L7_SUCCESS;
             else
                 rc = L7_FAILURE; 
         }
     }
     dot1xLogicalPortInfoGiveLock();
  }
  return rc;
}

/*********************************************************************
* @purpose  Returns the first logical port for the physcial interface
*
* @param    lIntIfNum    @b((input)) the logical interface
* @param    nextIntf    @b{(ouput)}  the next interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortNextGet(L7_uint32 lIntIfNum,L7_uint32 *nextIntf)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_uint32 next;
  L7_RC_t  rc = L7_FAILURE;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return rc;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return rc;

  if (pCfg->portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
  {
    next = lIntIfNum;
    dot1xLogicalPortInfoTakeLock();
    if(dot1xLogicalPortInfoGetNextNode(physPort,&next)!=L7_NULLPTR)
    {
      *nextIntf = next;
      rc = L7_SUCCESS;
    }
    dot1xLogicalPortInfoGiveLock();
  }
  return rc;
}

/*********************************************************************
* @purpose  Returns the Supplicant Mac address for the logical interface
*
* @param    lIntIfNum    @b((input)) the specified interface
* @param    macAddr      @b((output)) Mac Address of the supplicant
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortSupplicantMacAddrGet(L7_uint32 lIntIfNum,
                                             L7_uchar8 *macAddr)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    memcpy(macAddr,&logicalPortInfo->suppMacAddr,L7_ENET_MAC_ADDR_LEN);
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}


/*********************************************************************
* @purpose  Returns the PAE state for the logical interface
*
* @param    lIntIfNum    @b((input)) the specified interface
* @param    authPaeState @b((output)) PAE state
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortPaeStateGet(L7_uint32 lIntIfNum,
                                    L7_uint32 *authPaeState)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);
  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *authPaeState = logicalPortInfo->apmState + 1;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the Backend state for the logical interface
*
* @param    lIntIfNum    @b((input)) the specified interface
* @param    state        @b((output)) backend state
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortBackendAuthStateGet(L7_uint32 lIntIfNum,
                                            L7_uint32 *state)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;
  
  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *state = logicalPortInfo->bamState + 1;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the Vlan assigned for the  logical interface
*
* @param    lIntIfNum    @b((input)) the specified interface
* @param    vlan        @b((output)) vlan assigned to the logical interface
* @param    mode        @b((output)) mode of assignment Radius/unauthenticated/Default
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortVlanAssignmentGet(L7_uint32 lIntIfNum,
                                          L7_uint32 *vlanId,
                                          L7_uint32 *mode)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    if(logicalPortInfo->vlanAssigned != 0)
    {
      *vlanId = logicalPortInfo->vlanAssigned;
      *mode = L7_DOT1X_RADIUS_ASSIGNED_VLAN;
    }
    else if(logicalPortInfo->unauthVlan != 0)
    {
      *vlanId = logicalPortInfo->unauthVlan;
      *mode = L7_DOT1X_UNAUTHENTICATED_VLAN;
    }
    else if(logicalPortInfo->guestVlanId != 0)
    {
      *vlanId = logicalPortInfo->guestVlanId;
      *mode = L7_DOT1X_GUEST_VLAN;
    }
    else if(logicalPortInfo->voiceVlanId != 0)
    {
      *vlanId = logicalPortInfo->voiceVlanId;
      *mode = L7_DOT1X_VOICE_VLAN;
    }
    else if((logicalPortInfo->defaultVlanId != 0) && 
            (logicalPortInfo->isMonitorModeClient == L7_TRUE))
    {
      *vlanId = logicalPortInfo->defaultVlanId;
      *mode = L7_DOT1X_MONITOR_MODE_VLAN; 
    }
    else
    {
      *vlanId = logicalPortInfo->vlanId;
      *mode = L7_DOT1X_DEFAULT_ASSIGNED_VLAN;
    }
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the User Name for the  logical interface
*
* @param    lIntIfNum    @b((input)) the specified interface
* @param    userName     @b((output)) user name for the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortUserNameGet(L7_uint32 lIntIfNum,
                                    L7_uchar8 *userName)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    memcpy(userName,logicalPortInfo->dot1xUserName,
           logicalPortInfo->dot1xUserNameLength);
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the filter id for the  logical interface
*
* @param    lIntIfNum  @b((input)) the specified interface
* @param    filter     @b((output)) filter id for the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortFilterIdGet(L7_uint32 lIntIfNum,
                                    L7_uchar8 *filter)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    memcpy(filter,logicalPortInfo->filterName,DOT1X_FILTER_NAME_LEN);
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Set Unathenticated vlan 
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    vlanId      @b{(input)} Unathenticated vlan
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments 
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortUnauthenticatedVlanSet(L7_uint32 intIfNum, L7_uint32 vlanId)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }

  if (vlanId==pCfg->unauthenticatedVlan ) 
  {
      return L7_SUCCESS;
  }

  pCfg->unauthenticatedVlan = vlanId;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(dot1xMgmtPortUnauthenticatedVlanSet, intIfNum, &vlanId);
}

/*********************************************************************
* @purpose  Get UnatuthenticatedVlan value
*
* @param    intIfNum  @b{(input)} internal interface number
* @param    *vlanId @b{(output)} Unatuthenticated Vlan
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments 
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortUnauthenticatedVlanGet(L7_uint32 intIfNum, L7_uint32 *vlanId)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *vlanId = pCfg->unauthenticatedVlan;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the port's session timeout value
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *sessionTimeout @b{(output)} session timeout value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortSessionTimeoutGet(L7_uint32 intIfNum, L7_uint32 *sessionTimeout)
{
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *sessionTimeout = logicalPortInfo->sessionTimeout;
  }
  else
  {
      rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the session timeout value for the  logical interface
*
* @param    lIntIfNum  @b((input)) the specified interface
* @param    sessiontimeout  @b((output)) session timeout for the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortSessionTimeoutGet(L7_uint32 lIntIfNum,
                                    L7_uint32 *session_timeout)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *session_timeout = logicalPortInfo->sessionTimeout;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Get the ports termination action value
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *terminationAction @b{(output)} termination action value
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortTerminationActionGet(L7_uint32 intIfNum, L7_uint32 *terminationAction)
{
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *terminationAction = logicalPortInfo->terminationAction + 1 ;
  }
  else
  {
      rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the termination Action for the  logical interface
*
* @param    lIntIfNum  @b((input)) the specified interface
* @param    terminationAction  @b((output)) termination Action for the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xLogicalPortTerminationActionGet(L7_uint32 lIntIfNum,
                                    L7_uint32 *terminationAction)
{
  L7_uint32 physPort;
  dot1xPortCfg_t *pCfg;
  L7_RC_t  rc = L7_SUCCESS;
  dot1xLogicalPortInfo_t *logicalPortInfo;

  physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);

  if (dot1xIsValidIntf(physPort) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(physPort, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  dot1xLogicalPortInfoTakeLock();
  logicalPortInfo = dot1xLogicalPortInfoGet(lIntIfNum);
  if(logicalPortInfo != L7_NULLPTR)
  {
    *terminationAction = logicalPortInfo->terminationAction + 1;
  }
  else
  {
    rc = L7_FAILURE;
  }
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
* @purpose  Returns the logical port for the corresponding supplicant Mac Address
*
* @param    mac_addr    @b{(input)} supplicant mac address to be searched
* @param    lIntIfNum    @b((output)) the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments for SNMP
*
* @end
*********************************************************************/
L7_RC_t dot1xClientMacAddressGet(L7_enetMacAddr_t *macAddr,L7_uint32 *lIntIfNum)
{

    if (dot1xCfg->dot1xAdminMode != L7_ENABLE) 
        return L7_FAILURE;

    return dot1xMacAddrInfoFind(macAddr,lIntIfNum);
}

/*********************************************************************
* @purpose  Returns the logical port for the next supplicant Mac Address
*           in the mac address database
*
* @param    mac_addr    @b{(input)} supplicant mac address to be searched
* @param    lIntIfNum    @b((output)) the logical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments for SNMP
*
* @end
*********************************************************************/
L7_RC_t dot1xClientMacAddressNextGet(L7_enetMacAddr_t *macAddr,L7_uint32 *lIntIfNum)
{
    if (dot1xCfg->dot1xAdminMode != L7_ENABLE) 
        return L7_FAILURE;

    return dot1xMacAddrInfoFindNext(macAddr,lIntIfNum);
}

/*********************************************************************
* @purpose  Returns the physical port corresponding to the logical interface
*
* @param    lIntIfNum    @b((input)) the logical interface
* @param    physport    @b{(ouput)}  the physical interface
*
* @returns  L7_SUCCESS  if success
* @returns  L7_FAILURE  if other failure
*
* @comments none
*
* @end
*********************************************************************/
L7_RC_t dot1xPhysicalPortGet(L7_uint32 lIntIfNum,L7_uint32 *physPort)
{
  *physPort = DOT1X_PHYSICAL_PORT_GET(lIntIfNum);
  return L7_SUCCESS;
}
/*********************************************************************
*
* @purpose  Callback from DTL informing about an unauthorized address
*
* @param    L7_uint32        intIfNum  @b((input)) Internal interface number
* @param    L7_enetMacAddr_t macAddr   @b((output)) MAC address
* @param    L7_ushort16      vlanId    @b((output)) VLAN ID
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @notes    none
*
* @end
*********************************************************************/
L7_RC_t dot1xUnauthAddrCallBack( L7_uint32 intIfNum, L7_enetMacAddr_t macAddr, L7_ushort16 vlanId )
{
  dot1xPortCfg_t *pCfg;
  dot1xUnauthCallbackParms_t      parms;

  /* If admin mode not enabled, don't process request */
  if (dot1xCfg->dot1xAdminMode != L7_ENABLE)
    return L7_FAILURE;

  /* If it is not a valid interface drop the request */
  if ( dot1xIsValidIntf( intIfNum ) != L7_TRUE )
  {
    return L7_FAILURE;
  }

  /* check if Guest vlan is enabled */
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }

  memcpy(&parms.macAddr,&macAddr.addr,L7_ENET_MAC_ADDR_LEN);
  parms.vlanId = vlanId;

  /* send the message to the dot1x component */
  return dot1xIssueCmd(dot1xUnauthAddrCallBackEvent,intIfNum,(void *)&parms);
}

/*********************************************************************
*
* @purpose  Enable/Disable Voice Vlan Device authorization.
*
* @param    L7_uint32        intIfNum  @b((input)) Internal interface number
* @param    L7_enetMacAddr_t macAddr   @b((output)) MAC address
* @param    L7_ushort16      vlanId    @b((output)) VLAN ID
* @param    L7_BOOL          flag       @b{(input)} TRUE - Enable, FALSE - DISABLE
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @notes    None
*
* @end
*********************************************************************/
L7_RC_t dot1xPortVoiceVlanAuthMode( L7_uint32 intIfNum, 
                                    L7_enetMacAddr_t macAddr, 
                                    L7_ushort16 vlanId,
                                    L7_BOOL  flag)
{
  dot1xPortCfg_t *pCfg;
  dot1xVoiceVlanModeParms_t      parms;

  /* If admin mode not enabled, don't process request */
  if (dot1xCfg->dot1xAdminMode != L7_ENABLE)
    return L7_FAILURE;

  /* If it is not a valid interface drop the request */
  if ( dot1xIsValidIntf( intIfNum ) != L7_TRUE )
  {
    return L7_FAILURE;
  }
  
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }

  /* check if interfac mode is mac-based */
  if (pCfg->portControlMode != L7_DOT1X_PORT_AUTO_MAC_BASED)
  {
    return L7_FAILURE;
  }

  /* check to see if the vlan id is valid */
  if (dot1qStaticVlanCheckValid(vlanId) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }

  memcpy(&parms.macAddr,&macAddr.addr,L7_ENET_MAC_ADDR_LEN);
  parms.vlanId = vlanId;
  parms.flag = flag;

  /* send the message to the dot1x component */
  return dot1xIssueCmd(dot1xVoiceVlanModeEvent,intIfNum,(void *)&parms);
}


/* Supplicant APIs Start here */

/*********************************************************************
* @purpose  Get the Port.s Max start value
*
*@param    intIfNum     @b{(input)} internal interface number
* @param    maxStart   @b((output)) Max start value
*
* @returns  L7_SUCCESS
*
* @notes    The max start value represents the maximum number of
*           successive EAPOL start messages that will be sent before 
*           the Supplicant assumes that there is no Authenticator is
*           present.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortMaxStartGet(L7_uint32 intIfNum, 
                                         L7_uint32 *maxStart)
{
  dot1xPortCfg_t *pCfg;
  
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;
 
  *maxStart = pCfg->supplicantPortCfg.maxStart;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the Port.s Max start value
*
*@param    intIfNum     @b{(input)} internal interface number
* @param    maxStart   @b((input)) Max start value
*
* @returns  L7_SUCCESS
*
* @notes    The max start value represents the maximum number of successive
*           EAPOL start messages that will be sent before the Supplicant
*           assumes that there is no Authenticator is present.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortMaxStartSet(L7_uint32 intIfNum, 
                                     L7_uint32 maxStart)
{

  dot1xPortCfg_t *pCfg;
 
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if ( pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  if ( maxStart < L7_DOT1X_PORT_MIN_MAX_START ||
       maxStart > L7_DOT1X_PORT_MAX_MAX_START  
     )
  {
    return L7_FAILURE;
  }
  
  pCfg->supplicantPortCfg.maxStart = maxStart;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Sets the Port.s Supplicant Mode
*
* @param  intIfNum     @b{(input)} internal interface number
* @param  *controlMode  @b((input)) Supplicant control mode.
*
* @returns  L7_SUCCESS
*
* @notes    none
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortControlModeGet(L7_uint32 intIfNum,
                                            L7_uint32 *controlMode)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
    return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;
 
  *controlMode = pCfg->supplicantPortCfg.portControlMode;
  
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Sets the Supplicant port's control Mode
*
* @param    intIfNum    @b{(input)} internal interface number
* @param    portControl @b((input)) Supplican't control mode.
*
* @returns  L7_SUCCESS
*
* @notes    none
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortControlModeSet(L7_uint32 intIfNum,
                                           L7_uint32 portControl)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  if ( portControl < L7_DOT1X_PORT_FORCE_UNAUTHORIZED ||
       portControl > L7_DOT1X_PORT_FORCE_AUTHORIZED)
  {
    return L7_NOT_SUPPORTED;
  }
  if (pCfg->supplicantPortCfg.portControlMode == portControl)
  {
    return L7_SUCCESS;
  }
 
  pCfg->supplicantPortCfg.portControlMode = portControl;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;


  return dot1xIssueCmd(dot1xMgmtSupplicantPortControlModeSet, intIfNum, &portControl);

}

/*********************************************************************
* @purpose  Get the start period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *startPeriod  @b{(output)} start period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The startPeriod is the initialization value for startWhen,
*           which is a timer used by the Supplicant PAE state machine to
*           determine when to send next start message to the Authenticator.
*
* @end
*********************************************************************/
L7_RC_t dot1xPortSupplicantPortStartPeriodGet(L7_uint32 intIfNum,
                                           L7_uint32 *startPeriod)
{

 dot1xPortCfg_t *pCfg;

 if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
   return L7_FAILURE;

 if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
   return L7_FAILURE;

  *startPeriod = pCfg->supplicantPortCfg.startPeriod;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Set the start period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    startPeriod  @b{(input)} start period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The startPeriod is the initialization value for startWhen,
*           which is a timer used by the Supplicant PAE state machine to
*           determine when to send next start message to the Authenticator.
*
* @end
*********************************************************************/
L7_RC_t dot1xPortSupplicantPortStartPeriodSet(L7_uint32 intIfNum,
                                           L7_uint32 startPeriod)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if ( pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  if ( startPeriod < L7_DOT1X_PORT_MIN_START_PERIOD ||
       startPeriod > L7_DOT1X_PORT_MAX_START_PERIOD
     )
  {
    return L7_FAILURE;
  }

  pCfg->supplicantPortCfg.startPeriod = startPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the Held period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *heldPeriod  @b{(output)} Held period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The heldPeriod is the initialization value for heldWhile,
*           which is a timer used by the Supplicant PAE state machine to
*           determine when to send start message on previous authentication 
*           failure.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortHeldPeriodGet(L7_uint32 intIfNum,
                                             L7_uint32 *heldPeriod)
{

  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *heldPeriod = pCfg->supplicantPortCfg.heldPeriod;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Set the Held period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    heldPeriod  @b{(input)} Held period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The heldPeriod is the initialization value for heldWhile,
*           which is a timer used by the Supplicant PAE state machine to
*           determine when to send start message on previous authentication
*           failure.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortHeldPeriodSet(L7_uint32 intIfNum,
                                          L7_uint32 heldPeriod)
{

  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if ( pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  if ( heldPeriod < L7_DOT1X_PORT_MIN_HELD_PERIOD ||
       heldPeriod > L7_DOT1X_PORT_MAX_HELD_PERIOD
     )
  {
    return L7_FAILURE;
  }

  pCfg->supplicantPortCfg.heldPeriod = heldPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get the Auth period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    authPeriod  @b{(output)} Auth period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The authPeriod is the initialization value for authWhile,
*           which is a timer used by the Supplicant Backend state machine to
*          determine when to abort the authentication process on no response from
*          Authenticator.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortAuthPeriodGet(L7_uint32 intIfNum,
                                     L7_uint32 *authPeriod)
{

  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *authPeriod = pCfg->supplicantPortCfg.authPeriod;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Set the Auth period
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    authPeriod  @b{(input)} Auth period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The authPeriod is the initialization value for authWhile,
*           which is a timer used by the Supplicant Backend state machine to
*           determine when to abort the authentication process on no response
*           from Authenticator.
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortAuthPeriodSet(L7_uint32 intIfNum,
                                               L7_uint32 authPeriod)
{

  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if ( pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  if ( authPeriod < L7_DOT1X_PORT_MIN_AUTH_PERIOD ||
       authPeriod > L7_DOT1X_PORT_MAX_AUTH_PERIOD
     )
  {
    return L7_FAILURE;
  }

  pCfg->supplicantPortCfg.authPeriod = authPeriod;
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Set the user name 
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    userName  @b{(input)} user Name period
* @param    userNameLen  @b{(input)} user Name Length
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortUserNameSet(L7_uint32 intIfNum,
                                         L7_uchar8 * userName,
                                         L7_uint32 userNameLen)
{

  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
  {
    return L7_FAILURE;
  }
  if ( pCfg->paeCapabilities != L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
  {
    return L7_NOT_SUPPORTED;
  }
  memset (pCfg->supplicantPortCfg.dot1xSupplicantUserName,
          '\0',
          sizeof(pCfg->supplicantPortCfg.dot1xSupplicantUserName));
  if ( userNameLen > 0)
  {
    memcpy (pCfg->supplicantPortCfg.dot1xSupplicantUserName,
          userName,
          userNameLen);

  pCfg->supplicantPortCfg.dot1xSupplicantUserNameLen = userNameLen;
 }
 else
 {
   pCfg->supplicantPortCfg.dot1xSupplicantUserNameLen = 0;
 }

  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return L7_SUCCESS;

}



/*********************************************************************
* @purpose  Get the Supplicant user name
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    userName       @b{(output)} Auth period
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortUserNameGet(L7_uint32 intIfNum,
                                     L7_uchar8 *userName)
{

  dot1xPortCfg_t *pCfg;
  L7_uint32 usrIndex = 0;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  if (pCfg->supplicantPortCfg.dot1xSupplicantUserNameLen > 0)
  {
    if (userMgrLoginIndexGet((L7_char8 *)pCfg->supplicantPortCfg.dot1xSupplicantUserName,
                             &usrIndex) != L7_SUCCESS)
    {
      return L7_FAILURE;
    }

  }

  if ( pCfg->supplicantPortCfg.dot1xSupplicantUserNameLen > 0)
  {
    memcpy (userName, 
          pCfg->supplicantPortCfg.dot1xSupplicantUserName,
          pCfg->supplicantPortCfg.dot1xSupplicantUserNameLen);
  }
  else
  {
    return L7_FAILURE;
  }
       
  return L7_SUCCESS;

}


/*********************************************************************
* @purpose  Get the current state of the port's Supplicant PAE
*           state machine
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *suppPaeState @b{(output)} Authenticator PAE state
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortPaeStateGet(L7_uint32 intIfNum,
                                 L7_DOT1X_SPM_STATES_t *suppPaeState)
{

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *suppPaeState = dot1xSupplicantPortInfo[intIfNum].spmState;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get the current state of the port's Supplicant Backend 
*           state machine
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *suppSbmState @b{(output)} Backend Machine state
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortSbmStateGet(L7_uint32 intIfNum,
                                 L7_DOT1X_SBM_STATES_t *suppSbmState)
{

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *suppSbmState = dot1xSupplicantPortInfo[intIfNum].sbmState;

  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get the current Authrization status
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    *suppSpmStatus @b{(output)} Port status
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortSpmStatusGet(L7_uint32 intIfNum,
                                 L7_uchar8 *suppSpmStatus)
{

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  if (dot1xCfg->dot1xAdminMode == L7_DISABLE)
  {
    *suppSpmStatus = L7_DOT1X_PORT_STATUS_AUTHORIZED;
    return L7_SUCCESS;
  }

  *suppSpmStatus = dot1xSupplicantPortInfo[intIfNum].supplicantPortStatus;

  return L7_SUCCESS;

}


/* Supplicant port Stats retreival APIs */

/*********************************************************************
* @purpose  Get number of EAPOL frames received
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolFramesRx @b{(output)} EAPOL frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolFramesRx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolFramesRx = dot1xPortStats[intIfNum].suppEapolFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAPOL frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolFramesTx @b{(output)} EAPOL frames transmitted 
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolFramesTx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolFramesTx = dot1xPortStats[intIfNum].suppEapolFramesTx;
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get number of EAPOL Start frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolStartFramesTx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolStartFramesTx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolStartFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolStartFramesTx = dot1xPortStats[intIfNum].suppEapolStartFramesTx;
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get number of EAPOL LOGOFF frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolLogoffFramesTx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolLogoffFramesTx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolLogoffFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolLogoffFramesTx = dot1xPortStats[intIfNum].suppEapolLogoffFramesTx;
  return L7_SUCCESS;

}
/*********************************************************************
* @purpose  Get number of EAPOL Response/Id frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolRespIdFramesTx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolRespIdFramesTx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolRespIdFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolRespIdFramesTx = dot1xPortStats[intIfNum].suppEapRespIdFramesTx;
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get number of EAPOL Response frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolResponseFramesTx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolResponseFramesTx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolResponseFramesTx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolResponseFramesTx = dot1xPortStats[intIfNum].suppEapResponseFramesTx;
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get number of EAPOL Request/Id frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolReqIdFramesTx @b{(output)} EAPOL frames received 
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolReqIdFramesRx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolReqIdFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolReqIdFramesRx = dot1xPortStats[intIfNum].suppEapReqIdFramesRx;
  return L7_SUCCESS;

}
/*********************************************************************
* @purpose  Get number of EAPOL Request frames transmitted
*
* @param    intIfNum       @b{(input)} internal interface number
* @param    *eapolRequestFramesRx @b{(output)} EAPOL frames transmitted
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSuppicantPortEapolRequestFramesRx(L7_uint32 intIfNum,
                                         L7_uint32 *eapolReqFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapolReqFramesRx = dot1xPortStats[intIfNum].suppEapReqFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of invalid EAPOL frames received
*
* @param    intIfNum              @b{(input)} internal interface number
* @param    *invalidEapolFramesRx @b{(output)} Invalid EAPOL frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortInvalidEapolFramesRx(L7_uint32 intIfNum,
                                  L7_uint32 *invalidEapolFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *invalidEapolFramesRx = dot1xPortStats[intIfNum].suppInvalidEapolFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get number of EAP length error frames received
*
* @param    intIfNum                @b{(input)} internal interface number
* @param    *eapLengthErrorFramesRx @b{(output)} EAP length error frames received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortEapLengthErrorFramesRx(L7_uint32 intIfNum, 
                                     L7_uint32 *eapLengthErrorFramesRx)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *eapLengthErrorFramesRx = dot1xPortStats[intIfNum].suppEapLengthErrorFramesRx;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get protocol version number carried in the most recently
*           received EAPOL frame
*
* @param    intIfNum               @b{(input)} internal interface number
* @param    *lastEapolFrameVersion @b{(output)} last EAPOL version received
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortLastEapolFrameVersion(L7_uint32 intIfNum, 
                                      L7_uint32 *lastEapolFrameVersion)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  *lastEapolFrameVersion = dot1xPortStats[intIfNum].suppLastEapolFrameVersion;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the source MAC address carried in the most recently
*           recently received EAPOL frame
*
* @param    intIfNum               @b{(input)} internal interface number
* @param    *lastEapolFrameVersion @b{(output)} last source MAC address
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xSupplicantPortLastEapolFrameSourceGet(L7_uint32 intIfNum, 
                                 L7_enetMacAddr_t *lastEapolFrameSource)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE)
      return L7_FAILURE;

  memcpy(lastEapolFrameSource->addr,
         dot1xPortStats[intIfNum].suppLastEapolFrameSource.addr,
         L7_MAC_ADDR_LEN);
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the port autherization status.
*
* @param    intIfNum               @b{(input)} internal interface number
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_RC_t dot1xPortIsAuthorized(L7_uint32 intIfNum)
{
  dot1xPortCfg_t *pCfg;
  L7_DOT1X_PORT_STATUS_t portStatus;
  L7_uchar8 suppStatus;

  if (dot1xIsValidIntf(intIfNum) == L7_TRUE)
  {
    if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    {
      return L7_SUCCESS;
    }
    if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
    {
      if ((dot1xPortAuthControlledPortStatusGet(intIfNum, &portStatus) != L7_SUCCESS) ||
          (portStatus == L7_DOT1X_PORT_STATUS_UNAUTHORIZED))
      {
        return L7_FAILURE;
      }
    }
    else if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
    {
      if ((dot1xSupplicantPortSpmStatusGet(intIfNum, &suppStatus) != L7_SUCCESS) ||
          (suppStatus == L7_DOT1X_PORT_STATUS_UNAUTHORIZED))
      {
        return L7_FAILURE;
      }
    }
    return L7_SUCCESS;
  }
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Get the configured MAB value on the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    mabEnabled   @b{(output)} value detrmining if MAB 
*                                      has been enabled on the port 
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMABEnabledGet(L7_uint32 intIfNum, L7_uint32 *mabEnabled)
{
  dot1xPortCfg_t *pCfg;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  *mabEnabled = pCfg->mabEnabled;
  return L7_SUCCESS;

}

/*********************************************************************
* @purpose  Get the operational MAB value on the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    mabEnabled   @b{(output)} value detrmining if MAB 
*                                     has been operationally 
*                                     enabled on the port 
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortOperMABEnabledGet(L7_uint32 intIfNum, L7_uint32 *mabEnabled)
{
  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  *mabEnabled = dot1xPortInfo[intIfNum].mabEnabled;
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Set the MAB value on the port.
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    mabEnabled   @b{(output)} boolean value detrmining if MAB 
*                                      has been configured on the port 
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
* @results  L7_REQUEST_DENIED     if port control mode of the port is 
*                                 not mac-based  
* @results  L7_ERROR              if guest vlan has been configured on 
*                                 the same port.      
*
* @comments
*       
* @end
*********************************************************************/
L7_RC_t dot1xPortMABEnabledSet(L7_uint32 intIfNum, L7_uint32 mabEnabled)
{
  dot1xPortCfg_t *pCfg;
  L7_uint32 event = 0;

  if (dot1xIsValidIntf(intIfNum) != L7_TRUE) 
      return L7_FAILURE;

  if (dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE)
    return L7_FAILURE;

  /*invalid input*/
  if (mabEnabled != L7_ENABLE && mabEnabled != L7_DISABLE)
    return L7_FAILURE;

  /* check if guest vlan has been configured for the port */
  if (pCfg->guestVlanId != 0)
  {
    return L7_ERROR;
  }

  if (mabEnabled == L7_ENABLE)
  {
    pCfg->mabEnabled = L7_ENABLE;
    event =  dot1xMgmtPortMABEnabledSet;
  }
  else 
  {
    pCfg->mabEnabled = L7_DISABLE;
    event = dot1xMgmtPortMABDisabledSet;  
  }
  return dot1xIssueCmd(event, intIfNum, &mabEnabled);;
}

/*********************************************************************
* @purpose  Determine if a client is authenticated on an interface
*
* @param    intIfNum      @b{(input)} internal interface number
* @param    macAddr       @b{(input)} client's MAC address
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments
*
* @end
*********************************************************************/
L7_BOOL dot1xPortClientAuthenticationGet(L7_uint32 intIfNum, L7_uchar8 *macAddr)
{
  L7_uint32               adminMode;
  L7_DOT1X_PORT_CONTROL_t portControl;
  L7_DOT1X_APM_STATES_t   authPaeState;
  L7_uint32               lIntIfNum;
  L7_uint32               physPort;

  /* If dot1x is not enabled, client is considered authenticated */
  if ((L7_SUCCESS != dot1xAdminModeGet(&adminMode)) ||
      (L7_DISABLE == adminMode))
  {
    return L7_TRUE;
  }

  if (L7_SUCCESS != dot1xPortControlModeGet(intIfNum, &portControl))
  {
    return L7_TRUE;
  }

  if (L7_DOT1X_PORT_FORCE_AUTHORIZED == portControl)
  {
    /* If port is force authorized, client is considered authenticated */
    return L7_TRUE;
  }
  else if (L7_DOT1X_PORT_AUTO == portControl)
  {
    /* if port has been authenticated, all clients are authorized */
    if ((L7_SUCCESS == dot1xPortAuthPaeStateGet(intIfNum, &authPaeState)) &&
        (L7_DOT1X_APM_AUTHENTICATED == authPaeState))
    {
      return L7_TRUE;
    }
  }
  else if (L7_DOT1X_PORT_AUTO_MAC_BASED == portControl)
  {
    /*
     * if
     * 1. MAC address is found on a logical interface
     * 2. the logical interface belongs to this physical port
     * 3. the logical interface is authenticated
     * then the client is authenticated
     */
    if ((L7_SUCCESS == dot1xClientMacAddressGet((L7_enetMacAddr_t *)macAddr, &lIntIfNum)) &&
        (L7_SUCCESS == dot1xPhysicalPortGet(lIntIfNum, &physPort)) &&
        (physPort == intIfNum) &&
        (L7_SUCCESS == dot1xLogicalPortPaeStateGet(lIntIfNum, &authPaeState)) &&
        ((L7_DOT1X_APM_AUTHENTICATED == authPaeState)))
    {
      return L7_TRUE;
    }
  }

  return L7_FALSE;
}
/*********************************************************************
* @purpose  Set Monitor mode setting for dot1x
*
* @param    monitorMode @b{(input)} dot1x Monitor mode
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xMonitorModeSet(L7_uint32 monitorMode)
{
  L7_uint32 event;
  if (dot1xCfg->monitorMode == monitorMode)
    return L7_SUCCESS;

  if(monitorMode == L7_ENABLE)
  {
    dot1xCfg->monitorMode = L7_ENABLE;
    event = dot1xMgmtAdminMonitorModeEnable;
  }
  else
  {
    dot1xCfg->monitorMode = L7_DISABLE;
    event = dot1xMgmtAdminMonitorModeDisable;
  }
  dot1xCfg->cfgHdr.dataChanged = L7_TRUE;

  return dot1xIssueCmd(event, L7_NULL, &monitorMode);
}

/*********************************************************************
* @purpose  Get Monitor mode setting for dot1x
*
* @param    *monitorMode @b{(output)} dot1x monitor mode
*
* @returns  L7_SUCCESS
*
* @comments    
*       
* @end
*********************************************************************/
L7_RC_t dot1xMonitorModeGet(L7_uint32 *monitorMode)
{
  if(monitorMode == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  *monitorMode = dot1xCfg->monitorMode;
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the number of dot1x Monitor mode clients
 *          
 * @param    pCount @b((output)) ptr to the number of monitor mode clients  
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xMonitorModeNumClientsGet(L7_uint32 *pCount) 
{
  L7_RC_t rc;

  if(pCount == L7_NULLPTR)
  {
    return L7_FAILURE;
  }
  dot1xLogicalPortInfoTakeLock();
  rc = dot1xAuthMonitorModeClientsNumGet(pCount);
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/*********************************************************************
 * @purpose  Get the number of dot1x Non Monitor mode clients
 *          
 * @param    pCount @b((output)) ptr to the number of non monitor mode
 *                               clients  
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xNonMonitorModeNumClientsGet(L7_uint32 *pCount)
{
  L7_RC_t rc;

  if(pCount == L7_NULLPTR)
  {
    return L7_FAILURE;
  }
  dot1xLogicalPortInfoTakeLock();
  rc = dot1xAuthNonMonitorModeClientsNumGet(pCount);
  dot1xLogicalPortInfoGiveLock();
  return rc;
}

/******************************************************************************
 * @purpose  Gets the next History Log interface index
 *
 * @param    intIfNum   @b{(input/output)} Physical Interface Number
 *
 * @returns  L7_SUCCESS
 * @returns  L7_FAILURE  If there are no entries
 *
 * @notes
 *
 * @end
 *******************************************************************************/
L7_RC_t dot1xAuthHistoryLogIfIndexNextGet(L7_uint32 *intIfNum)
{
  L7_RC_t rc;

  if(intIfNum == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  rc = dot1xAuthHistoryLogInterfaceNextGet(intIfNum);
  dot1xAuthHistoryLogInfoGiveLock();

  return rc;
}


/******************************************************************************
 * @purpose  Gets the next History Log entry indexs
 *
 * @param    intIfNum   @b{(input/output)} Physical Interface Number
 * @param    entryIndex @b{(input/output)} EntryIndex
 *
 * @returns  L7_SUCCESS
 * @returns  L7_FAILURE  If there are no entries
 *
 * @notes
 *
 * @end
 *******************************************************************************/
L7_RC_t dot1xAuthHistoryLogIndexNextGet(L7_uint32 *intIfNum,
                                        L7_uint32 *entryIndex)
{
  L7_RC_t rc;

  if((intIfNum == L7_NULLPTR) || (entryIndex == L7_NULLPTR))
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  rc = dot1xAuthHistoryLogEntryIndexNextGet(intIfNum, entryIndex);
  dot1xAuthHistoryLogInfoGiveLock();

  return rc;
}



/******************************************************************************
 * @purpose  Gets the next History Log entry indexs in reverse order
 *
 * @param    intIfNum   @b{(input/output)} Physical Interface Number
 * @param    entryIndex @b{(input/output)} EntryIndex
 *
 * @returns  L7_SUCCESS
 * @returns  L7_FAILURE  If there are no entries
 *
 * @notes
 *
 * @end
 *******************************************************************************/
L7_RC_t dot1xAuthHistoryLogReverseIndexNextGet(L7_uint32 *intIfNum,
                                               L7_uint32 *entryIndex)
{
  L7_RC_t   rc=L7_SUCCESS;
  L7_uint32 ifIndex;
  L7_uint32 entryId1=L7_NULL, entryId2=L7_NULL;
  L7_BOOL   isEntryExist = L7_FALSE;
 
  if((intIfNum == L7_NULLPTR) || (entryIndex == L7_NULLPTR))
  {
    return L7_FAILURE;
  }

  ifIndex  = *intIfNum;

  if(ifIndex == L7_NULL)
  {
    rc = dot1xAuthHistoryLogIfIndexNextGet(&ifIndex);
  }
    

  if(rc == L7_SUCCESS)
  {
    do
    { 
      dot1xAuthHistoryLogInfoTakeLock();
  
      while(dot1xAuthHistoryLogInfoNextEntryIndexGet(ifIndex, &entryId1) == L7_SUCCESS)
      {
        if(entryId1 == *entryIndex)
        {
          break;
        }  
        entryId2 = entryId1;
        isEntryExist = L7_TRUE;
      }
 
      if(isEntryExist == L7_TRUE)
      {
        *intIfNum = ifIndex; 
        *entryIndex = entryId2;
        rc = L7_SUCCESS;
        dot1xAuthHistoryLogInfoGiveLock();
        break; 
      }
      else
      {
        rc = L7_FAILURE;
      }
      dot1xAuthHistoryLogInfoGiveLock();
      entryId1 = L7_NULL;
    }while((rc = dot1xAuthHistoryLogIfIndexNextGet(&ifIndex)) == L7_SUCCESS);
  }

  return rc;
}


/*********************************************************************
 * @purpose  Purge all dot1x auth history log entries for the given
 *           interface
 *
 * @param    intIfNum   @b{(input)} Physical Interface Number
 *
 * @returns L7_SUCCESS
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes
 * @end
 *
 **********************************************************************/
L7_RC_t dot1xAuthHistoryLogInterfacePurge(L7_uint32 intIfNum)
{
  L7_uint32 ifIndex=0, entryIndex=0;
  L7_RC_t rc;

  dot1xAuthHistoryLogInfoTakeLock();

  rc = dot1xAuthHistoryLogEntryIndexNextGet(&ifIndex, &entryIndex);
  if(rc != L7_SUCCESS)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }

  do
  {
    if(intIfNum == ifIndex)
    {
      if(dot1xAuthHistoryLogEntryDelete(ifIndex, entryIndex) != L7_SUCCESS)
      {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID, 
          "Failed to delete Dot1x Auth History Log Entry for Iface:[%d] entryIndex:[%d]\n", ifIndex, entryIndex);
      }
    }
    rc = dot1xAuthHistoryLogEntryIndexNextGet(&ifIndex, &entryIndex);
  }while(rc == L7_SUCCESS);

  dot1xAuthHistoryLogInfoGiveLock();

  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Purge all dot1x auth history log entries
 *
 * @param    void
 *
 * @returns L7_SUCCESS
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes
 *
 * @end
 *
 **********************************************************************/
L7_RC_t dot1xAuthHistoryLogPurgeAll()
{
  L7_uint32 ifIndex=0, entryIndex=0;
  L7_RC_t rc;

  dot1xAuthHistoryLogInfoTakeLock();
  rc = dot1xAuthHistoryLogEntryIndexNextGet(&ifIndex, &entryIndex);
  if(rc != L7_SUCCESS)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }

  do
  {
    if(dot1xAuthHistoryLogEntryDelete(ifIndex, entryIndex) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID, 
                  "Failed to delete Dot1x Auth History Log Entry for Iface:[%d] entryIndex:[%d]\n", ifIndex, entryIndex);
    }
    rc = dot1xAuthHistoryLogEntryIndexNextGet(&ifIndex, &entryIndex);
  }while(rc == L7_SUCCESS);

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}


/*********************************************************************
 * @purpose  Get the timestamp from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pTimeStamp @b{(output)} reference to the Reason Code
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogTimestampGet(L7_uint32 intIfNum,
                                        L7_uint32 entryIndex,
                                        L7_uint32 *pTimeStamp) 
{
  dot1xAuthHistoryLogInfo_t *pData;

  if(pTimeStamp == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();

  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  { 
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pTimeStamp = pData->timeStamp;

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the VlanId from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pVlanId    @b{(output)} reference to the VLANID
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogVlanIdGet(L7_uint32 intIfNum,
                                     L7_uint32 entryIndex,
                                     L7_ushort16 *pVlanId) 
{
  dot1xAuthHistoryLogInfo_t *pData;
  
  if(pVlanId == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();

  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pVlanId = pData->vlanId;
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the reasonCode from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pReasonCode @b{(output)} Reference to the Reason Code
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogReasonCodeGet(L7_uint32 intIfNum,
                                         L7_uint32 entryIndex,
                                         L7_uint32 *pReasonCode) 
{
  dot1xAuthHistoryLogInfo_t *pData;

  if(pReasonCode == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pReasonCode = pData->reasonCode;

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}


/*********************************************************************
 * @purpose  Get the accessStatus from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pStatus    @b{(output)} Reference to the Access Status
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogAccessStatusGet(L7_uint32 intIfNum,
                                           L7_uint32 entryIndex,
                                           L7_uint32 *pStatus) 
{
  dot1xAuthHistoryLogInfo_t *pData;
 
  if(pStatus == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pStatus = pData->accessStatus;

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}


/*********************************************************************
 * @purpose  Get the dot1x authentication Status from the dot1x Auth 
 *           History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pStatus    @b{(output)} Reference to the Auth Status
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogAuthStatusGet(L7_uint32 intIfNum,
                                         L7_uint32 entryIndex,
                                         L7_uint32 *pStatus) 
{
  dot1xAuthHistoryLogInfo_t *pData;

  if(pStatus == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pStatus = pData->authStatus;

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the supplicant Mac Address from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pMacAddress @b{(output)} Reference to the Mac Address
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogSupplicantMacAddressGet(L7_uint32        intIfNum,
                                                   L7_uint32        entryIndex,
                                                   L7_enetMacAddr_t *pMacAddress) 
{
  dot1xAuthHistoryLogInfo_t *pData;
 
  if(pMacAddress == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  
  memcpy(pMacAddress, &pData->suppMacAddr, L7_ENET_MAC_ADDR_LEN);
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}


/*********************************************************************
 * @purpose  Get the FilterID for the Dot1x Radius Accept Packet
 *           located in the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pFilterId  @b{(output)} Reference to the filter Id
 * @param    pFilterLen @b{(input/output)} Reference to filter Length
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogFilterIdGet(L7_uint32 intIfNum,
                                       L7_uint32 entryIndex,
                                       L7_uchar8 *pFilterIdName, 
                                       L7_uint32 *pFilterLen) 
{
  dot1xAuthHistoryLogInfo_t *pData;

  if((pFilterIdName == L7_NULLPTR) || (pFilterLen == L7_NULLPTR))
  {
    return L7_FAILURE;
  }
 
  if(*pFilterLen == 0)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();
  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }

  memset(pFilterIdName, 0x00, *pFilterLen);
  memcpy(pFilterIdName, pData->filterName, min(*pFilterLen, DOT1X_FILTER_NAME_LEN));
  *pFilterLen = min(*pFilterLen, DOT1X_FILTER_NAME_LEN);

  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the Vlan Assigned from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pVlanId    @b{(output)} reference to the VLANID
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogVlanIdAssignedGet(L7_uint32 intIfNum,
                                             L7_uint32 entryIndex,
                                             L7_ushort16 *pVlanId) 
{
  dot1xAuthHistoryLogInfo_t *pData;
  
  if(pVlanId == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();

  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pVlanId = pData->vlanIdAssigned;
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the Vlan Assigned Type from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pVlanAssignedType @b{(output)} reference to the VLAN 
                                            assigned Type.
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogVlanAssignedTypeGet(L7_uint32 intIfNum,
                                               L7_uint32 entryIndex,
                                               L7_uint8  *pVlanAssignedType) 
{
  dot1xAuthHistoryLogInfo_t *pData;
  
  if(pVlanAssignedType == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();

  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pVlanAssignedType = pData->vlanAssignedType;
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

/*********************************************************************
 * @purpose  Get the MAB enable status from the dot1x Auth History table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 * @param    pMABStatus @b{(output)} MAB enable status
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogMABStatusGet(L7_uint32 intIfNum,
                                        L7_uint32 entryIndex,
                                        L7_uint8  *pMABStatus)
{
  dot1xAuthHistoryLogInfo_t *pData;
  
  if(pMABStatus == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  dot1xAuthHistoryLogInfoTakeLock();

  if ((pData = dot1xAuthHistoryLogInfoGet(intIfNum, 
                                          entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE;
  }
  *pMABStatus = pData->isMABClient;
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}


/*********************************************************************
 * @purpose  Check if the dot1x auth history log entry exists in History
 *           table
 *          
 * @param    intIfNum   @b{(input)} Physical Interface Number
 * @param    entryIndex @b{(input)} EntryIndex
 *
 * @returns L7_SUCCESS  
 * @returns L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogEntryIsValid(L7_uint32 intIfNum,
                                        L7_uint32 entryIndex)
{
  dot1xAuthHistoryLogInfo_t *pData;

  dot1xAuthHistoryLogInfoTakeLock();
  if((pData = dot1xAuthHistoryLogInfoGet(intIfNum,
                                         entryIndex)) == L7_NULLPTR)
  {
    dot1xAuthHistoryLogInfoGiveLock();
    return L7_FAILURE; 
  }
  dot1xAuthHistoryLogInfoGiveLock();
  return L7_SUCCESS;
}

 /*********************************************************************
 * @purpose  Return Reason in String format for the given entry
 *          
 * @param    intIfNum   (input)  - Interface
 *           entryIndex (input)  - Entry Index
 *           reasonCode (input)  - Reason Code
 *           strReason  (output) - Reason in String format
 *           strLen     (input/output) - Length of Reason String
 *
 * @returns L7_SUCCESS/L7_FAILURE
 *
 * @comments
 *
 * @notes 
 *
 * @end
 *
 *********************************************************************/
L7_RC_t dot1xAuthHistoryLogReasonStringGet(L7_uint32  intIfNum,
                                           L7_uint32  entryIndex,
                                           L7_uint32  reasonCode,
                                           L7_char8  *strReason,
                                           L7_uint32 *strLen)
{
   L7_char8 buf[L7_CLI_MAX_STRING_LENGTH * 2]={0};
   dot1xAuthHistoryLogInfo_t *pData;
   L7_uint32 len=0;

   if((strReason == L7_NULLPTR) || (strLen == L7_NULLPTR))
   {
     return L7_FAILURE;
   }

   if(*strLen == 0)
   {
     return L7_FAILURE;
   }
 
   pData = dot1xAuthHistoryLogInfoGet(intIfNum, entryIndex);
   if(pData == L7_NULLPTR)
   {
     return L7_FAILURE;
   }

   switch(reasonCode)
   {
     case  DOT1X_AUTH_REASON_SUPPLICANT_TIMEOUT:
           osapiSnprintf(buf, sizeof(buf), "Dot1x Supplicant Timeout");             
           break;
     case  DOT1X_AUTH_REASON_EAPOL_TIMEOUT:                                 /*2*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x EAPOL Timeout");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_REQ_TIMEOUT:                            /*3*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Request Timeout%s", 
                            (pData->isMABClient == L7_TRUE)?" for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_AUTH_FAILURE:                           /*4*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Authentication Failure %s",
                                       (pData->isMABClient == L7_TRUE)?"for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_AUTH_COMM_FAILURE:                      /*5*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Authentication Comm Failure %s", 
                                         (pData->isMABClient == L7_TRUE)?"for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_CHALLENGE_PROCESS_INVALID_NAS_PORT:     /*6*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Challenge Invalid NAS Port");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_CHALLENGE_PROCESS_WRONG_EAP_MSG:        /*7*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Challenge Wrong EAP Message");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_REQUEST_SEND_MSG_ERROR:                 /*8*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Request Message Send Error");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_ACCEPT_PROCESS_INVALID_NAS_PORT:        /*9*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Accept Process - Invalid NAS Port %s",
                                        (pData->isMABClient == L7_TRUE)?"for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_ACCEPT_PROCESS_WRONG_EAP_MSG:           /*10*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Accept Process - Wrong EAP Message %s",
                                        (pData->isMABClient == L7_TRUE)?"for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_ACCEPT_FILTER_ASSIGNMENT_FAILURE:       /*11*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Accept Process - Filter[%s] Assignment Failure %s", 
                          pData->filterName, (pData->isMABClient == L7_TRUE)?"for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_ACCEPT_DIFFSERV_NOT_PRESENT:            /*12*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Accept Process - Diffserv Feature Not Present, fail to authenticate%s",
                                        (pData->isMABClient == L7_TRUE)?" MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_ACCEPT_VLAN_ASSIGNMENT_FAILURE:         /*13*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Accept Process - VLAN [%d] Assignment Failure, fail to authenticate%s", 
                                       pData->vlanId, (pData->isMABClient == L7_TRUE)?" MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_VLAN_ASSIGNMENT_FEATURE_NOT_ENABLED:           /*14*/
           osapiSnprintf(buf, sizeof(buf), "Authentication Successful, VLAN Assignment Feature Not Present%s",  
                                  (pData->isMABClient == L7_TRUE)?" for a MAB Client":"");       
           break;
     case  DOT1X_AUTH_REASON_RADIUS_SUCCESS:                                /*15*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Radius Authentication Successful%s", (pData->isMABClient == L7_TRUE)?" for a MAB Client":"");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_AUTH_USER_NOT_FOUND:                     /*16*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Process - Local User Not Found");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_AUTH_USER_NO_ACCESS:                     /*17*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Process - Local User No Access");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_AUTH_MD5_VALIDATION_FAILURE:             /*18*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Process - MD5 Validation Failure");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_AUTH_INVALID_EAP_TYPE:                   /*19*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Process - Invalid EAP Type");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_FAILURE:                                 /*20*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Failure");             
           break;
     case  DOT1X_AUTH_REASON_LOCAL_SUCCESS:                                 /*21*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Local Authentication Successful");             
           break;
     case  DOT1X_AUTH_REASON_RADIUS_INVALID_RADIUS_STATUS:                  /*22*/
           osapiSnprintf(buf, sizeof(buf), "RADIUS response Process Failed! Invalid Radius Status Type");             
           break; 
     case  DOT1X_AUTH_REASON_GUEST_VLAN_TIMER_EXPIRY:                        /*23*/
           osapiSnprintf(buf, sizeof(buf), "Dot1x Authentication due to Guest VLAN Timer Expiry");             
           break;
     case  DOT1X_AUTH_REASON_AUTH_METHOD_UNDEFINED:                          /*24*/ 
           osapiSnprintf(buf, sizeof(buf), "Failed to Authenticate due to Undefined Auth Method");             
           break;
     case  DOT1X_AUTH_REASON_AUTH_METHOD_REJECT:                             /*25*/    
           osapiSnprintf(buf, sizeof(buf), "Failed to Authenticate, Auth method is configured as 'Reject'");             
           break;
     case  DOT1X_AUTH_REASON_AUTH_METHOD_INVALID:                            /*26*/ 
           osapiSnprintf(buf, sizeof(buf), "Failed to Authenticate due to Invalid Auth Method");             
           break;
     case  DOT1X_AUTH_REASON_AUTH_METHOD_NOT_CONFIGURED:                     /*27*/ 
           osapiSnprintf(buf, sizeof(buf), "Failed to Authenticate, Auth method is not configured");             
           break;
     default:
           osapiSnprintf(buf, sizeof(buf), "Invalid Dot1x Auth History Log Reason Code[%d]\n\r",reasonCode);

           L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID,
                   "Invalid Dot1x Auth History Log Reason Code[%d]\n\r",reasonCode);
           break;
   }

   len = osapiStrnlen(buf, sizeof(buf));
   if(*strLen < len)
   {
     len = *strLen;
   }
   else
   {
     *strLen = len;
   }
   osapiStrncpy(strReason, buf, len); 
   return L7_SUCCESS;
}



