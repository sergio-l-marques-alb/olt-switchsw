...
L7_RC_t rlimTunnelOpCreate(L7_uint32 tunnelId)
{
  ...
  if (rc != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "failed to create tunnel%u in NIM (rc=%u)\n", tunnelId, rc);
    return rc;
  }
...
void rlimTunnelOpDelete(L7_uint32 intIfNum, L7_uint32 tunnelId)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "invalid tunnelId/intIfNum combo\n");
    return;
  }
...
void rlimTunnelOpUpdate(L7_uint32 tunnelId)
{
  ...
  if ((top->flags & RLIM_TNNLOP_FLAG_DELETE_IN_PROGRESS) != 0)
  {
                       ...
                       RLIM_TNNLOP_FLAG_DISABLE_IN_PROGRESS)) == 0)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                "failed to delete tunnel%u in NIM (rc=%u)\n", tunnelId, rc);
      }
    }
  }
}
...
L7_RC_t rlimTunnelEnable(L7_uint32 tunnelId)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "failed to get src/dst addrs for tunnel%u\n", tunnelId);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "failed to get nexthop info for tunnel%u\n", tunnelId);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "failed to create tunnel%u in stack\n", tunnelId);
  }
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "failed to create tunnel%u vi DTL\n", tunnelId);
    }
  }
  ...
  if (rc == L7_SUCCESS && RLIM_TUNNEL_CANBE_IP4_INTF(tcp->tunnelMode))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "failed to enable routing on tunnel%u\n", tunnelId);
    }
  }
...
void rlimTunnelDisable(L7_uint32 tunnelId)
{
                     ...
                     RLIM_TNNLOP_FLAG_IP6_ROUTING)) == 0)
  {
    if ((top->flags & RLIM_TNNLOP_FLAG_DTL_INTF_CREATED) != 0)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                "dtlTunnelDelete failed for tunnel%u\n", tunnelId);
      }
    }
    ...
    if ((top->flags & RLIM_TNNLOP_FLAG_OSAPI_INTF_CREATED) != 0)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                "osapiTunnelIfDelete failed for tunnel%u\n", tunnelId);
      }
    }
...
void rlimTunnelIntfDeleteProcess(L7_uint32 eventIntIfNum)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "cannot get configId for deleted interface");
    return;
  }
                          ...
                          L7_uint32 localIntIfNum)
{
  ...
  switch (tcp->localAddr.addrType)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "invalid addrtype for enabling ip4 on tunnel (%u)",
              tcp->localAddr.addrType);
      break;
  }
...
void rlimTunnelForceIp4Down(L7_uint32 tunnelId)
{
  ...
  switch (tcp->localAddr.addrType)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "invalid addrtype for disabling ip4 on tunnel (%u)",
              tcp->localAddr.addrType);
      break;
  }
