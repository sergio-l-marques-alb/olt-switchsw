    ...
    if (status == L7_SUCCESS)
    {
      switch (Message.msgId)
      {
        ...
        case DHCPRELAYMAP_INTF_EVENT:
          ...
          break;
        case DHCPRELAYMAP_NIM_STARTUP:
          if (osapiSemaTake(ihInfo->ihLock, L7_WAIT_FOREVER) == L7_SUCCESS)
          {
            dhcpRelayNimStartupProcess(Message.type.nimStartupEvent.startupPhase);
            osapiSemaGive(ihInfo->ihLock);
          }
          break;
        default:
...
{
  L7_uint32 intIfNum = 0,rtrIfNum;           /* routing interface where client is attached */
  ...
  L7_uchar8 traceBuf[IH_TRACE_LEN_MAX];
  L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
  dhcpRelayIntfValue_t relayCIDOptioncheck,relayCIDinsertMode;


  /* We first try to get the interface number as we have to check for
   * the correct options in the relay info echoed from server.
  ...
  relayCIDOptioncheck= pDhcpRelayCfgData->relayrtrIntfCfgData[rtrIfNum].cidOptionIntfCheckMode;
  relayCIDinsertMode = pDhcpRelayCfgData->relayrtrIntfCfgData[rtrIfNum].circuitIDIntfOptionMode;

  if(cnfgrIsFeaturePresent(L7_DHCP_RELAY_COMPONENT_ID, L7_RELAY_OPTION82_INTF_FEATURE_ID) == L7_TRUE) 
  {
    if((relayCIDinsertMode == L7_RELAY_INTF_ENABLE) || 
     ((relayCIDinsertMode == L7_RELAY_INTF_NOT_CONFIGURED) 
                     && (pDhcpRelayCfgData->circuitIDOptionMode == L7_ENABLE)))
    {
      rc = dhcpRelayOption82Strip(dhcpPacket, &dhcpPacketLength, &clientPort);
      if ((relayCIDOptioncheck == L7_RELAY_INTF_ENABLE) ||
        ((relayCIDOptioncheck == L7_RELAY_INTF_NOT_CONFIGURED)
                && (pDhcpRelayCfgData->cidOptionCheckMode == L7_ENABLE)))
      {
       if ((rc != L7_SUCCESS) && (ihInfo->ihTraceFlags & IH_TRACE_PACKET))
       {
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        osapiSnprintf(traceBuf, IH_TRACE_LEN_MAX,
                      "IP Helper dropping %u byte message from interface %s due to option-82"
                      " check fail in server reply message.",dhcpPacketLength, ifName);
        ihTraceWrite(traceBuf);
       }
      } 
    }
  ...
  else if (pDhcpRelayCfgData->circuitIDOptionMode == L7_ENABLE) 
  {
    /* parse option 82 to get physical port to client. Continue even if
     * this fails provided no check is to be done for the returned value*/
                                  ...
                                  L7_uint32 vlanId)
{
  ...
  dhcpRelayIntfValue_t relayCIDinsertMode;
  ...
  relayCIDinsertMode = pDhcpRelayCfgData->relayrtrIntfCfgData[rtrIntfNum].circuitIDIntfOptionMode;
...
#define IH_MAX_NS 8   
  ...
  if(nextOption <= pktEnd)
  {
    /* To handle the case when a malicious IP packet with wrong length
     * is processed (fixes crash) */
...
* @returns  void
*
* @notes    Transfer to DHCP relay thread
*
* @end
*********************************************************************/
void dhcpRelayIntfStartupCallback(NIM_STARTUP_PHASE_t startupPhase)
{
  L7_RC_t rc;
  udpRelayMsg_t dhcpMessage;

  memset(&dhcpMessage, 0, sizeof(udpRelayMsg_t) );
  dhcpMessage.msgId = DHCPRELAYMAP_NIM_STARTUP;
  dhcpMessage.type.nimStartupEvent.startupPhase = startupPhase;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DHCP_RELAY_COMPONENT_ID,
            "Failed to enqueue NIM startup event %d.", startupPhase);
  }
}

/*********************************************************************
* @purpose  Invoke NIM startup processing
*
* @param    startupPhase     create or activate
*
* @returns  void
*
* @notes    IP helper only cares about L7_CREATE and L7_DELETE
*
* @end
*********************************************************************/
L7_RC_t dhcpRelayNimStartupProcess(NIM_STARTUP_PHASE_t startupPhase)
{
  if (startupPhase == NIM_INTERFACE_CREATE_STARTUP)
  {
    dhcpRelayNimCreateStartup();
    return L7_SUCCESS;
  }
  else if (startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    /* DHCP relay doesn't care about interface state */
    nimStartupEventDone(L7_DHCP_RELAY_COMPONENT_ID);  
    return L7_SUCCESS;
  }
  L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DHCP_RELAY_COMPONENT_ID,
          "Processing unexpected NIM startup callback %d", startupPhase);
  return L7_FAILURE;
}
...
* @end
*********************************************************************/
L7_RC_t dhcpRelayNimCreateStartup(void)
{
  L7_RC_t rc;
  L7_uint32 intIfNum;
  ...
  while (rc == L7_SUCCESS)
  {
    if (dhcpRelayIsValidIntf(intIfNum))
    {
      if (dhcpRelayIntfCreate(intIfNum) != L7_SUCCESS)
      {
        L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
  ...
  PORTEVENT_SETMASKBIT(ihInfo->ihNimEventMask, L7_CREATE);
  PORTEVENT_SETMASKBIT(ihInfo->ihNimEventMask, L7_DELETE);
  nimRegisterIntfEvents(L7_DHCP_RELAY_COMPONENT_ID, ihInfo->ihNimEventMask);
