...
L7_RC_t gmrpCreateGmr(L7_uint32 vlanId)
{
  ...
  if (gmrpInstanceEmptyIndexGet(vlanId, &emptySlotIndex) != L7_SUCCESS)
  {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_GARP_COMPONENT_ID,
              "Failed to find empty gmrpInstance for new vlan %d\n", vlanId);
      return L7_FAILURE;
  }
  /*create the gmr*/
...
L7_BOOL gmrpMapIntfConfigEntryGet(L7_uint32 intIfNum, gmrpIntfCfgData_t **pCfg)
{
  ...
  if ((nextIndex < L7_GARP_MAX_INTF) &&
      NIM_CONFIG_ID_IS_EQUAL(&gmrpCfg->intf[nextIndex].configId, &configIdNull))
  {
    i = nextIndex;
  }
  else
...
*
*           gmrpConfig is zeroed in gmrpBuildDefaultConfigData() at startup.
*           So no need to zero reg entry and vlan masks at startup. If
*           interface is created, deleted, and recreated, then mask bit for
*           the interface is cleared when the interface is deleted.
*
*
* @end
*
...
L7_RC_t gmrpIntfCreate(L7_uint32 intIfNum)
{
  ...
  /* Since GMRP does not support binary config storage, it cannot
   * have assigned a config structure for this interface yet. If one
   * exists, holler. */
  if (gmrpMapTbl[intIfNum] != 0)
  {
    L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
    nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_GMRP_COMPONENT_ID,
  ...
  /* Find available config entry */
  if (gmrpMapIntfConfigEntryGet(intIfNum, &pCfg) != L7_TRUE)
  {
    L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
    nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_GMRP_COMPONENT_ID,
            "Failed to get configuration entry for GMRP interface %s.",
            ifName);
    return L7_FAILURE;
  }
...
{
  gmrpIntfCfgData_t *pCfg;
  L7_uint32 maskOffset, cfgIdx, entry, vid;
  GMRP_REG_ENTRY_t *staticEntry;
  GMRP_SERVICE_ENTRY_t *fwdAllEntry;
  GMRP_SERVICE_ENTRY_t *fwdUnregEntry;
  ...
  /* These are all cleared on interface delete rather than interface
   * create in order to avoid this heavy processing at startup. */
  for (entry = 0; entry < L7_MAX_GROUP_REGISTRATION_ENTRIES; entry++)
  {
    staticEntry = &gmrpCfg->cfg.staticEntry[entry];
    NIM_INTF_CLRMASKBIT(staticEntry->fixedReg, maskOffset);
    NIM_INTF_CLRMASKBIT(staticEntry->forbiddenReg, maskOffset);
  }

  for (vid = 0; vid < L7_MAX_VLANS; vid++)
  {
    fwdAllEntry = &gmrpCfg->cfg.staticForwardAll[vid];
    NIM_INTF_CLRMASKBIT(fwdAllEntry->fixedReg, maskOffset);
    NIM_INTF_CLRMASKBIT(fwdAllEntry->forbiddenReg, maskOffset);

    fwdUnregEntry = &gmrpCfg->cfg.staticForwardUnreg[vid];
    NIM_INTF_CLRMASKBIT(fwdUnregEntry->fixedReg, maskOffset);
    NIM_INTF_CLRMASKBIT(fwdUnregEntry->forbiddenReg, maskOffset);
  }
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(gmrpCfg->intf[index].configId)) == L7_FALSE)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_GMRP_COMPONENT_ID,
          "Error accessing GMRP config data for interface %s in gmrpMapIntfIsConfigurable."
...
L7_RC_t gmrpEntryDelete(L7_uint32 vlanId, L7_uchar8 *macAddr)
{
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_GARP_COMPONENT_ID,
              "GMRP failed to delete MFDB entry: vlan %d and address %s\n",
              vlanId, buf);
  }
...
void gmrpEntriesGet(L7_uint32 vlanId)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_GARP_COMPONENT_ID,
            "Could not find a gmrp instance for this vlan id %u\n",vlanId);
    return;
  }
