...
void dot3adBuildConfigData(L7_uint32 ver)
{
  ...
  simMacAddrGet(dot3adCfg.cfg.dot3adSystem.actorSys.addr);
...
L7_RC_t dot3adPhysIntfDefaultBuild( L7_uint32 intIfNum, L7_uint32 ver, dot3ad_port_t *intf, L7_uint32 *idx)
{
  ...
  intf->begin       = L7_FALSE;
  intf->actorAdminPortKey = L7_NULL;
                                  ...
                                  dot3ad_agg_t *agg, L7_uint32 *idx)
{
  ...
  agg->collectorMaxDelay = FD_DOT3AD_DEFAULT_LACP_MAXDELAY;
...
L7_RC_t dot3adIntfProcessIntfUp(L7_uint32 intIfNum)
{
  ...
  if (agg == L7_NULLPTR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT3AD_COMPONENT_ID," Cannot find aggregator(%d) for this interface(%s)", p->actorOperPortKey, ifName);
    return L7_FAILURE;
  }
...
L7_RC_t dot3adIntfAttach(L7_uint32 intIfNum)
{
  ...
  if (nimGetIntfType(intIfNum, &type) != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT3AD_COMPONENT_ID,
                      "%s:failed to get intf type for intf %s",
                         __FUNCTION__, ifName);
  }
  else if (type == L7_PHYSICAL_INTF)
  {
...
L7_RC_t dot3adIntfDelete(L7_uint32 intIfNum,NIM_EVENT_COMPLETE_INFO_t *status,L7_BOOL *callback)
{
  ...
  do
  {
    ...
    else if (type == L7_LAG_INTF)
    {
      ...
      dtlDot3adLagDelete(intIfNum); /* PTin added: trunks */
    }
    else
    {
      rc = L7_SUCCESS;
...
L7_RC_t dot3adConfigCompleteCallback(L7_uint32 event)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT3AD_COMPONENT_ID,"CONFIG COMPLETE FOR DOT3AD\n");
