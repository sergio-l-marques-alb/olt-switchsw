    ...
    if (dot1qIntfIsSettable(intIfNum) == L7_TRUE)
    {
      ...
      while (pDataEntry != L7_NULLPTR)
      {
        ...
        if (pDataEntry->current.status == L7_DOT1Q_DYNAMIC ||
            ...
            pDataEntry->current.status == L7_DOT1Q_IP_INTERNAL_VLAN)
        {
        ...
        if (msNum != 0 || tsNum != 0)
        {
            ...
            {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                        "DTL call failure adding intf %s to vlan %d\n", ifName, pDataEntry->vlanId);
            }
        }
...
L7_RC_t dot1qIntfUnapplyConfigData(L7_uint32 intIfNum)
{
                             ...
                             (queryData.data.state == L7_INTF_DETACHING)))
  {
    ...
    {
       L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
               "dtl failure reverting to default state for intf %s\n", ifName);
    }
    ...
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(dot1qCfg->Qports[index].configId)) == L7_FALSE)
    {
       ...
       */
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID,
                "Error accessing DOT1Q config data for interface %s in dot1qMapIntfIsConfigurable."
...
L7_RC_t dot1qRestore(void)
{
  ...
  while (rc == L7_SUCCESS)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "dtl request failure when deleting intf %s", ifName);
    }

    /* Need to set the vlan ingress filtering to default */
    rc = dtlDot1qPortConfig(intIfNum,
                           (DTL_L2_DOT1Q_PORT_CONFIG_INGRESS_FILTER|
                            DTL_L2_DOT1Q_PORT_CONFIG_ACCEPT_FRAME_TYPE),
                            FD_DOT1Q_DEFAULT_PVID,
                            FD_DOT1Q_DEFAULT_ACCEPT_FRAME_TYPE,
                            FD_DOT1Q_DEFAULT_INGRESS_FILTER_PORT);
    if (rc != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1Q_COMPONENT_ID,
              "dtl failure while modifying ingress filter to default for intf %s",
              ifName);
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
            "Failures occurred in setting default VLAN configuration\n");
...
L7_BOOL dot1qVlanIsConfigurable(L7_uint32 vid, dot1qVlanCfgData_t **pVCfg)
{
    ...
    if ( vid != dot1qCfg->Qvlan[index].vlanId)
    {
       ...
       */
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                "Error accessing DOT1Q config data for VLAN %d in dot1qVlanIsConfigurable\n",
                vid);
        return L7_FALSE;
...
L7_RC_t   dot1qVlanTaggedMemberSetCfgUpdate(L7_uint32 vid, L7_uint32 intIfNum, L7_uint32 tagMode)
{
    ...
    for (i = start; i < end; i++)
    {
        ...
        /* Check if intIfNum is configured member of VLAN */
        if (!NIM_INTF_ISMASKBITSET(vCfg->vlanCfg.staticEgressPorts, maskOffset))
            continue;
...
void dot1qBuildTestConfigData(void)
{
  ...
  /*-------------------------------*/
    ...
    if (dot1qCnfgrStateCheck() == L7_TRUE)
    {
      ...
      while (rc == L7_SUCCESS)
      {
        if ((brc = dot1qMapIntfIsConfigurable(intIfNum, &pCfg)) == L7_TRUE)
        {
