...
/*************
 ** Message sent to Card Insert/Remove task.
 *************/

typedef enum
{
  L7_CMGR_CARD_INSERT = 1,
  L7_CMGR_CARD_REMOVE,
  L7_CMGR_CARD_SYNC,
  L7_CMGR_CARD_CREATE,
  L7_CMGR_CARD_DELETE,
  L7_CMGR_LOCAL_UNIT_IS_MANAGER,     /* Sent by UM */
  L7_CMGR_LOCAL_UNIT_IS_NOT_MANAGER, /* Sent by UM */
  L7_CMGR_CARD_CREATE_COMPLETE,
  L7_CMGR_CARD_CONFIG_SLOTS,
  L7_CMGR_CARD_CONFIG_PORTS,
  L7_CMGR_UNIT_CONNECTED,          /* Sent by UM */
  L7_CMGR_UNIT_DISCONNECTED,       /* Sent by UM */
  L7_CMGR_STARTUP_CREATE,
  L7_CMGR_STARTUP_ACTIVATE, 
  L7_CMGR_HW_APPLY_EVENT           /* Sent by Cnfgr */
} cmgrWorkerEvent_t;

typedef struct 
{
  cmgrWorkerEvent_t event; 
  ...
  union 
  {
    ...
    struct
    {
      L7_LAST_STARTUP_REASON_t systemStartupReason;
      L7_BOOL                  mgrFailover;
      L7_uint32                lastMgrUnitNum;
...
/*********************************************************************
 *
 * @purpose Async notification from the driver that a card insert has
 *          failed.
 *
 * @params  event {(input)} Card insert failure info
 *
 * @returns  none
 *
 * @notes   Will issue card unplug for the failed cards
 *
 * @end
 *
 *********************************************************************/
void cmgrCardInsertFailureCallback(hpcStackEventMsg_t event);
