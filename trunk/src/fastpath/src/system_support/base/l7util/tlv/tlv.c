                    ...
                    L7_tlvHandle_t *pTlvHandle)
{
  ...
  *pTlvHandle = L7_TLV_HANDLE_INVALID;          /* init to invalid value */
  ...
  if (TlvFirstRegister == L7_FALSE)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_SIM_COMPONENT_ID, "tlvRegister: Unable to create TLV "
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_SIM_COMPONENT_ID, "tlvRegister: All TLV blocks currently "
...
L7_RC_t tlvComplete(L7_tlvHandle_t tlvHandle)
{
  ...
  tlvType = osapiNtohl(*(L7_uint32 *)L7_TLV_BLK_DATA_START(pTlvCtrl));
  tlvLen = (totalLen - L7_TLV_ENTRY_SIZE());
  ...
  if (pTlvCtrl->traceId != L7_TLV_TRACE_ID_INVALID)
  {
    ...
    TRACE_ENTER_BYTE((L7_uchar8)pTlvCtrl->numOpen);
    TRACE_ENTER_BYTE(0);
    TRACE_ENTER_4BYTE(tlvType);
    TRACE_ENTER_4BYTE(tlvLen);
    TRACE_END();
  }
                 ...
                 L7_uint32 *pTlvLen)
{
  ...
  *pTlvLen = L7_TLV_BLK_DATA_LENGTH(pTlvCtrl);
  ...
  if (pTlvCtrl->traceId != L7_TLV_TRACE_ID_INVALID)
  {
    ...
    TRACE_ENTER_BYTE((L7_uchar8)pTlvCtrl->numOpen);
    TRACE_ENTER_BYTE(0);
    TRACE_ENTER_4BYTE(osapiNtohl((*ppTlv)->type));
    TRACE_ENTER_4BYTE(*pTlvLen);
    TRACE_END();
  }
                 ...
                 L7_uchar8 *pValue)
{
  ...
  if (pTlvCtrl->traceId != L7_TLV_TRACE_ID_INVALID)
  {
    ...
    TRACE_ENTER_BYTE((L7_uchar8)pTlvCtrl->numOpen);
    TRACE_ENTER_BYTE(0);
    TRACE_ENTER_4BYTE(type);
    TRACE_ENTER_4BYTE(length);
    TRACE_END();
  }
  ...
  pTlvHdr->type = osapiHtonl(type);
  pTlvHdr->length = osapiHtonl(length);
  ...
  for (i = 0; i < pTlvCtrl->numOpen; i++)
  {
    ...
    oldLength = osapiNtohl(pOpenHdr->length);
    pOpenHdr->length = osapiHtonl((oldLength + totalLength));
  }
                ...
                L7_uchar8 *pValue)
{
  ...
  if (pTlvCtrl->traceId != L7_TLV_TRACE_ID_INVALID)
  {
    ...
    TRACE_ENTER_BYTE(0);
    TRACE_ENTER_4BYTE(type);
    TRACE_ENTER_4BYTE(pTlvCtrl->nextOffset);
    TRACE_END();
  }
...
L7_RC_t tlvClose(L7_tlvHandle_t tlvHandle, L7_uint32 type)
{
  ...
  if (pTlvCtrl->traceId != L7_TLV_TRACE_ID_INVALID)
  {
    ...
    TRACE_ENTER_BYTE(0);
    TRACE_ENTER_4BYTE(type);
    TRACE_ENTER_4BYTE(osapiNtohl(pOpenHdr->length));
    TRACE_END();
  }
   ...
   */
  if (osapiNtohl(pOpenHdr->type) != type)
    return L7_FAILURE;
  ...
  for (i = 0; i < numEnt; i++)
  {
    ...
    if (count >= (numEnt-i))
    {
      ...
      else  /* manually format output */
      {
        ...
        case L7_TLV_TRACE_CODE_CLOSE:
          SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                        "Type=0x%8.8x ", *(L7_uint32 *)pOffset);
          pOffset += sizeof(L7_uint32);
          SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                        "Len=0x%8.8x ", *(L7_uint32 *)pOffset);
          break;
        ...
        case L7_TLV_TRACE_CODE_QUERY:
          ...
          pOffset += sizeof(L7_uint32);
          SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                        "TotLen=0x%8.8x ", *(L7_uint32 *)pOffset);
          break;
        ...
        case L7_TLV_TRACE_CODE_OPEN:
          ...
          pOffset += sizeof(L7_uint32);
          SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                        "NxtOfst=0x%8.8x ", *(L7_uint32 *)pOffset);
          break;
...
static void tlvCtrlBlockInit(L7_tlvCtrl_t *pTlvCtrl)
{
  ...
  if (pTlvCtrl == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tlvCtrlBlockInit: TLV control pointer parm is null.\n");
    return;
  }
                       ...
                       L7_uint32 *pEntrySize)
{
  ...
  remainLen = L7_TLV_HEADER_SIZE + osapiNtohl(pTlv->length);
...
L7_RC_t tlvCtrlShow(L7_tlvHandle_t tlvHandle)
{
  ...
  valLong = osapiNtohl(pHdr->type);
  SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                "Primary TLV type          : 0x%8.8x (%u)\n",
                valLong, valLong);
  valLong = osapiNtohl(pHdr->length);
  SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                "Primary TLV length        : 0x%8.8x (%u)\n",
                valLong, valLong);
  ...
  for (i = 0; i < pTlvCtrl->numOpen; i++)
  {
    ...
    jmax = L7_TLV_ENTRY_SIZE() + min(8, osapiNtohl(pHdr->length));
    for (j = 0; j < jmax; j++)
    {
