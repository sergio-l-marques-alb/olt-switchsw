/*
 * $Id: switch.c 1.433.2.46 Broadcom SDK $
                                    ...
                                    int arg)
{
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) ||
        SOC_IS_VALKYRIE2(unit) || SOC_IS_ENDURO(unit) || SOC_IS_KATANA(unit) || SOC_IS_HURRICANE(unit)) {
                          ...
                          bcm_switch_control_t type, int arg)
{
    ...
    if ((info.hash_concat == 1) && (info.concat_f != -1)) {
        soc_reg_field_set(unit, 
                         info.reg, &hash_control, info.concat_f, 1);
    }
                          ...
                          bcm_switch_control_t type, int *arg)
{
    ...
    *arg  = soc_reg_field_get(unit, info.reg, hash_control, info.offset_f);
    if (info.concat_f != -1) {
        concat = soc_reg_field_get(unit, info.reg, hash_control, info.concat_f);
    }
...
_bcm_switch_sync_port_select_set(int unit, uint32 val)
{
    ...
    SOC_IF_ERROR_RETURN(READ_CMIC_MISC_CONTROLr(unit, &rval));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CMIC_MISC_CONTROLr(unit, rval));
    }
    else if (SOC_IS_KATANA(unit)) {
      static int16 _L1_port_val_map[] = {
...
_bcm_switch_sync_port_select_get(int unit, uint8 type, uint32 *val)
{
    ...
    if (SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit)) {
      ...
      SOC_IF_ERROR_RETURN(READ_TOP_MISC_CONTROLr(unit, &rval));
      if (type) { /* primary (0) */
          port = soc_reg_field_get(unit, TOP_MISC_CONTROLr, rval,
                                 L1_CLK0_RECOVERY_MUX_SELf);
          *val = port+1;
      } else { /* backup (1) */
          port = soc_reg_field_get(unit, TOP_MISC_CONTROLr, rval,
                           ...
                           int arg)
{
    ...
    defined(BCM_HURRICANE_SUPPORT) || defined(BCM_KATANA_SUPPORT)
        ...
        case bcmSwitchSynchronousPortClockSourceBkup:
            if (soc_feature(unit, soc_feature_gmii_clkout)) {
                ...
                if (SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit)) {
                    ...
                    return _bcm_switch_sync_port_backup_select_set(unit,(uint32)
arg);
                }
                else
#endif
