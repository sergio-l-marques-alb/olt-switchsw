/*
 * $Id: stat.c 1.124.6.16 Broadcom SDK $
...
_bcm_esw_stat_detach(int unit)
{
    ...
    if (NULL != _bcm_stat_ovr_control) {
        ...
        free_units = TRUE;
        for (i = 0; i < BCM_MAX_NUM_UNITS; i++) {
            if(!SHR_BITNULL_RANGE(&_bcm_stat_ovr_control[unit], 0,
                      SHR_BITALLOCSIZE(SOC_MAX_NUM_PORTS))) {
                free_units = FALSE;
                break;
            }
...
bcm_esw_stat_init(int unit)
{
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            if ((IS_GE_PORT(unit,port) || IS_FE_PORT(unit,port))
                ...
                if (SOC_REG_IS_VALID(unit, FRM_LENGTHr)) {
                    SOC_IF_ERROR_RETURN (WRITE_FRM_LENGTHr(unit, port, 
                                    _bcm_stat_ovr_threshold[unit][port]));
                }
            }
    ...
    defined(BCM_HURRICANE_SUPPORT)
            if (IS_XQ_PORT(unit, port) && (SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit))) {
                SOC_IF_ERROR_RETURN
...
#if defined (BCM_KATANA_SUPPORT)
        } else if (SOC_IS_KATANA(unit) && IS_MXQ_PORT(unit,port)) {
            SOC_IF_ERROR_RETURN
                    (WRITE_X_GPORT_CNTMAXSIZEr(unit, port, value));
#endif
#if defined (BCM_ENDURO_SUPPORT) || defined(BCM_HURRICANE_SUPPORT)
        } else if (IS_XQ_PORT(unit, port) && (SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit))) {
            SOC_IF_ERROR_RETURN
