/*
 * $Id: management.c 1.1.2.31 Broadcom SDK $
 *
    ...
    bcm_pbmp_t top_bitmap)
{
    ...
    STACK_DATA(unit, ptp_id).mgmt_available = _bcm_ptp_sem_create("BCM PTP mgmt", sal_sem_BINARY, 0);
    rv = _bcm_ptp_sem_give(STACK_DATA(unit, ptp_id).mgmt_available);
    if (BCM_FAILURE(rv)) {
    ...
    int* resp_len)
{
    ...
    rv = _bcm_ptp_sem_take(STACK_DATA(unit, ptp_id).mgmt_available, PTP_MGMT_RESPONSE_TIMEOUT_US);
    if (BCM_FAILURE(rv)) {
...
/* static char holding_thread_name[100]; */
...

{
    ...
    if (stack_p->int_state.mboxes->status[0] != MBOX_STATUS_EMPTY) {
        ...
        /*            stack_p->int_state.mboxes->status[0], holding_thread_name, this_thread_name); */
...
release_return:
    ...
    /* Add 5 ms in order to delay the next bcm_ptp_sem_take */
    sal_usleep(5000);  
    return rv;
}
...
_bcm_ptp_construct_signaling_msg(
	...
	int *udp_payload_len)
{
    static uint16 sequenceID = 0;

    memset(udp_payload, 0, 34);  /* blank PTP header */
    udp_payload[0] = _bcmPTPmessageType_signaling;
    udp_payload[1] = 2;
	...
	int *tlv_payload_len)
{
    ...
    tlv_payload[1] = _bcmPTPtlvType_request_unicast_transmission;
...
{
    tlv_payload[0] = 0;  /* TLV type: */
    tlv_payload[1] = _bcmPTPtlvType_cancel_unicast_transmission;
    tlv_payload[2] = 0;  /* Length: */
    tlv_payload[3] = 2;
    tlv_payload[4] = (type << 4);
    tlv_payload[5] = 0;
...
{
    tlv_payload[0] = 0;  /* TLV type: */
    tlv_payload[1] = _bcmPTPtlvType_grant_unicast_transmission;
    tlv_payload[2] = 0;  /* Length: */
    tlv_payload[3] = 8;
    tlv_payload[4] = (type << 4);
    tlv_payload[5] = interval;
...
{
    tlv_payload[0] = 0;  /* TLV type: */
    tlv_payload[1] = _bcmPTPtlvType_ack_cancel_unicast_transmission;
    tlv_payload[2] = 0;  /* Length: */
    tlv_payload[3] = 2;
    tlv_payload[4] = (type << 4);
    tlv_payload[5] = 0;
