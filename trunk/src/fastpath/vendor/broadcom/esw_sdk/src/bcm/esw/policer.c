/*
 * $Id: policer.c 1.6.6.15 Broadcom SDK $
    ...
    switch (meter_attr->mode_type_v)
    {
        ...
        case cascade_mode:
            BCM_IF_ERROR_RETURN(_bcm_policer_svc_meter_create_cascade_mode(
                    unit,
                    meter_attr->uncompressed_attr_selectors_v,
                    group_mode, num_pol, svc_meter_mode, &total_bits));
            break;
   ...
   /* Delete policer from policers hash. */
    ...
    if (offset_mode) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, SVM_MACROFLOW_INDEX_TABLEm,
                                                       MACROFLOW_INDEXf)) {
            soc_SVM_MACROFLOW_INDEX_TABLEm_field_get(unit, &macro_flow_entry,
        ...
        if (macro_flow_meter_index > 0) {
            ...
            rv = _bcm_esw_policer_decrement_ref_count(unit, macro_meter_policer);
            if (!(BCM_SUCCESS(rv))) {
                POLICER_VERB(("%s : Unable to decrement ref count for macro \
                              meter provided\n", FUNCTION_NAME()));
                return rv;
            }
            macro_flow_meter_index = 0; /* clear macro flow index table entry */
