/*
 * $Id: internal_stack.c 1.1.2.17 Broadcom SDK $
 * 
...
_bcm_kt_ptp_hw_init(int unit, int uc_num)
{
    ...
    soc_cm_print("1588 COSQ for unit %d is %d\n", unit, ptp_cosq);
    ...
    bcm_ptp_stack_id_t ptp_id)
{
    ...
    stack_p->int_state.mboxes->num_buffers = soc_ntohl(BCM_PTP_MAX_BUFFERS);
    ...
    stack_p->int_state.log = soc_cm_salloc(unit, sizeof(_bcm_ptp_internal_stack_log_t)
    ...
    for (c=max_num_cores-1; c >= 0; c--) {
        soc_cm_print("Trying PTP on core %d\n", c);
