/*
 * $Id: l3.c 1.605.2.78 Broadcom SDK $
                          ...
                          bcm_if_t *intf)
{
    ...
    uint32  entry_type=0;              /* Next Hop Entry_type */
        ...
        ((defined (BCM_EASYRIDER_SUPPORT) || defined(BCM_TRIUMPH_SUPPORT)) \
         && defined(BCM_MPLS_SUPPORT))
   ...
   /* Store api flags within egr object flags */
    ...
    if ( soc_feature(unit, soc_feature_trill) && (egr->flags & BCM_L3_TRILL_ONLY)){
         L3_IF_ERROR_CLEANUP_ELSE_RETURN(bcm_td_trill_egress_set(unit, index));
    }
#endif /* BCM_TRIDENT_SUPPORT */
    ...
    if (soc_feature(unit, soc_feature_mpls_failover)) {
         if (egr->failover_id > 0 && egr->failover_id < 1024) {
                 L3_IF_ERROR_CLEANUP(bcm_tr2_failover_prot_nhi_create( unit, index ));
                 L3_IF_ERROR_CLEANUP(bcm_tr2_failover_prot_nhi_set(
