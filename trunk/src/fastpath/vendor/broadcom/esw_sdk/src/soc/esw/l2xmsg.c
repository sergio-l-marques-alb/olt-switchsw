/*
 * $Id: l2xmsg.c 1.39.2.1 Broadcom SDK $
...
{                                                           \
    ...
    soc_cm_debug(DK_ARL, "set_entry_callback: u:%d i=%d\n", \
                _u_, _index_);                             \
}
#define L2X_ENTRY_CALLBACK_CLR(_u_, _index_)                \
{                                                           \
    SHR_BITCLR(l2x_data[(_u_)].callback_map, (_index_));    \
    soc_cm_debug(DK_ARL, "clr_entry_callback: u:%d i=%d\n", \
                _u_, _index_);                             \
}
...
{
    int         io, in, hit_bit, delete_marked, callback_suppress;
    soc_field_t         valid_bit = VALID_BITf;
    uint32              *old_p, *new_p;
...
_soc_l2x_thread(void *unit_vp)
{
    ...
    if (index_count <= 0) {
        soc_cm_debug(DK_ERR, "soc_l2x_thread: table size is 0 \n");
        soc_event_generate(unit, SOC_SWITCH_EVENT_THREAD_ERROR, 
    ...
    if (shadow_tab == NULL || chunk_buf == NULL || 
        ...
        soc_cm_debug(DK_ERR, "soc_l2x_thread: not enough memory \n");
        soc_event_generate(unit, SOC_SWITCH_EVENT_THREAD_ERROR, 
