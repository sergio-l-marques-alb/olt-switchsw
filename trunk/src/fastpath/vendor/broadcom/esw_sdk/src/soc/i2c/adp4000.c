/*
 * $Id: adp4000.c 1.6.64.1 Broadcom SDK $
 ...
 */
STATIC int
adp4000_setmin_max(int unit, int devno, int set_max, int idx)
{
    uint8 data8;
    ...
    return adp4000_write(unit, devno, I2C_ADP4000_CMD_VOUT_CMD,
			     &data8, 1);
}

#define ADP4000_SETMAX(unit,devno, idx)\
   adp4000_setmin_max(unit, devno, 1, idx)
#define ADP4000_SETMIN(unit,devno, idx)\
   adp4000_setmin_max(unit, devno, 0, idx)
	   ...
	   int opcode, void* data, int len)
{
	...
	dac_params[len].step = 
	    (dac_params[len].max - dac_params[len].min) / 
               (ADP4000_MAX_DACVAL - ADP4000_MIN_DACVAL);
