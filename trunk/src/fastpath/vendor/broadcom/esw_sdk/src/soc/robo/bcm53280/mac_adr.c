/*
 * $Id: mac_adr.c 1.7.122.1 Broadcom SDK $
 *
 ...
 */
int 
drv_bcm53280_mac_set(int unit, soc_pbmp_t pbmp, uint32 mac_type, 
        uint8* mac, uint32 bpdu_idx)
{
    ...
    SOC_IF_ERROR_RETURN(DRV_REG_FIELD_SET(unit, reg_index, 
            (uint32 *)&reg_v64, fld_index, (uint32 *)&mac_field));
    SOC_IF_ERROR_RETURN(DRV_REG_WRITE(unit, reg_addr, 
            (uint32 *)&reg_v64, reg_len));
        ...
        if (_TB_MAC_ADDR_IS_NULL(mac)){
            if (mac_type == DRV_MAC_MULTIPORT_0){
        	    SOC_IF_ERROR_RETURN(REG_READ_GRPADDR2r(unit, 
        	            (uint32 *)&reg_v64));
        	    soc_GRPADDR2r_field_get(unit, (uint32 *)&reg_v64, 
        	            GRP_ADDRf, (uint32 *)&mac_field);
            } else {
        	    SOC_IF_ERROR_RETURN(REG_READ_GRPADDR1r(unit, 
        	            (uint32 *)&reg_v64));
        	    soc_GRPADDR1r_field_get(unit, (uint32 *)&reg_v64, 
        	            GRP_ADDRf, (uint32 *)&mac_field);
            }
 ...
 */
int 
drv_bcm53280_mac_get(int unit, uint32 val, uint32 mac_type, 
