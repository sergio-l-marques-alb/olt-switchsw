/*
 * $Id: memtune.c 1.109.2.2 Broadcom SDK $
...
/*
 * Local (portable) randomizer functions.
 */

static uint32 _prng_w = 6483;
static uint32 _prng_z = 31245;
	
STATIC void
_mem_srand(uint32 seed)
{
    _prng_z = (seed & 0xffff);
    _prng_w = (seed >> 16);

    /* Seeds cannot be zero */
    if (_prng_z == 0) {
        _prng_z = 6483;
    }
    if (_prng_w == 0) {
...
_mem_rand(void)
{
    _prng_z = 36969 * (_prng_z & 65535) + (_prng_z >> 16);
    _prng_w = 18000 * (_prng_w & 65535) + (_prng_w >> 16);

    return (_prng_z << 16) + _prng_w;  /* 32-bit result */
}

#ifdef BCM_EASYRIDER_SUPPORT
STATIC int
_soc_er_ddr_phase_sel_prog_hw_cb(soc_memtune_data_t *mt_data)
{
    ...
    if (mt_ctrl->phase_sel_cur != -1) {
        SOC_IF_ERROR_RETURN(soc_reg32_get(mt_ctrl->unit, er_mt_data->ddr_reg3, 
                            REG_PORT_ANY, 0, &rval));
        oval = rval;
        ...
        if (rval != oval) {
            SOC_IF_ERROR_RETURN(soc_reg32_set(mt_ctrl->unit, er_mt_data->ddr_reg3, 
                                REG_PORT_ANY, 0, rval));
        }
    }
...
STATIC int
_soc_er_ddr_latency_prog_hw_cb(soc_memtune_data_t *mt_data)
{
    soc_memtune_ctrl_t *mt_ctrl = mt_data->mt_ctrl;
    soc_er_memtune_data_t *er_mt_data = mt_ctrl->data;
...
STATIC int
_soc_er_ddr_txrx_prog_hw_cb(soc_memtune_data_t *mt_data)
{
    soc_memtune_ctrl_t *mt_ctrl = mt_data->mt_ctrl;
    soc_er_memtune_data_t *er_mt_data = mt_ctrl->data;
...
_soc_er_tune_generate_config(soc_memtune_data_t *mt_data)
{
    ...
    for (ix = 0; ix < iter_count; ix ++) {
        ...
        if (_mem_config_set(config_str, val_str) < 0) {
            return SOC_E_MEMORY;
        }
    }
...
_soc_memtune_txrx_analyze(soc_memtune_data_t *mt_data)
{
    ...
    if (fail_count == offset_count) {
        if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_FAIL_MATRIX) {
            soc_cm_print("Memory tuning %s failure count: all fail\n", buf);
        }
    ...
    if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_FAIL_MATRIX) {
        soc_cm_print("Memory tuning %s failure count:\n", buf);
        ...
        for (tx_offset = mt_ctrl->tx_offset_min;
            ...
            for (rx_offset = mt_ctrl->rx_offset_min;
                ...
                if (mt_ctrl->fail_array[index] >= 0) {
                    ...
                    soc_cm_print(" (%d)\t", -mt_ctrl->fail_array[index]);
                }
                index++;
            }
        }
    ...
    if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_ANALYSIS) {
        soc_cm_print("Memory tuning %s analysis "
...
_soc_memtune_ddr_main(soc_memtune_data_t *mt_data)
{
    ...
    if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_TEST_NAME) {
        soc_cm_print("Memory tuning test: unit %d %s interface:\n",
                     mt_ctrl->unit, mt_ctrl->intf_name);
    }
        ...
        for (i = 0; i < mt_ctrl->result_count; i++) {
            ...
            if (result->fail_count != offset_count) {
                ...
                if (result->ddr_latency != -1) {
                    soc_cm_print(" %2d  ,", result->ddr_latency);
                } else {
                    soc_cm_print(" **  ,");
                }
    ...
    if (result->fail_count == offset_count) {
        soc_cm_print("ERROR - no passing \"eye\" was found, \n"
        ...
        if (result->height < mt_ctrl->tx_pass_threshold ||
            ...
            soc_cm_print("!!! SRAM tuning result does not pass qualified "
                         "threshold !!!\n");
        }
        rv = SOC_E_NONE;
        if (mt_data->config) {
...
_soc_memtune_tcam_main(soc_memtune_data_t *mt_data)
{
    ...
    if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_TEST_NAME) {
        soc_cm_print("TCAM tuning test: unit %d %s interface:\n",
                     mt_ctrl->unit, mt_ctrl->intf_name);
    }
    ...
    if (result->fail_count != offset_count) {
        ...
        if (SOC_FAILURE(rv) || mt_ctrl->cur_fail_count) {
            ...
            if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_SELECTION) {
                ...
                if (mt_ctrl->invert_txclk_cur != -1) {
                    soc_cm_print("invert_txclk=%d ",
                                 mt_ctrl->invert_txclk_cur);
                }
                if (mt_ctrl->invert_rxclk_cur != -1) {
                    soc_cm_print("invert_rxclk=%d ",
                                 mt_ctrl->invert_rxclk_cur);
                }
                if (mt_ctrl->tx_offset_cur != -1) {
                    soc_cm_print("TX offset=%d ", mt_ctrl->tx_offset_cur);
    ...
    if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_SUMMARY) {
        ...
        for (i = 0; i < mt_ctrl->result_count; i++) {
            ...
            if (result->invert_txclk != -1) {
                soc_cm_print(" %c  ,", result->invert_txclk ? 'I' : ' ');
            } else {
                soc_cm_print(" ** ,");
            }
            if (result->invert_rxclk != -1) {
                soc_cm_print(" %c  ,", result->invert_rxclk ? 'I' : ' ');
            } else {
                soc_cm_print(" ** ,");
            }
            if (result->fail_count != offset_count) {
    ...
    if (result->fail_count == offset_count) {
        ...
        if (result->height < mt_ctrl->tx_pass_threshold ||
            result->width < mt_ctrl->rx_pass_threshold) {
            soc_cm_print("!!! TCAM tuning result does not pass qualified "
                         "threshold !!!\n");
        }
        rv = SOC_E_NONE;
        if (mt_data->config) {
...
_soc_er_mem_interface_tune(int unit, soc_memtune_data_t *mt_data)
{
    ...
    mt_ctrl->prog_hw1_fn = _soc_er_ddr_txrx_prog_hw_cb;
    mt_ctrl->prog_hw2_fn = _soc_er_ddr_latency_prog_hw_cb;
    mt_ctrl->prog_hw3_fn = _soc_er_ddr_phase_sel_prog_hw_cb;
    ...
    mt_ctrl->r2w_nops_min = -1;
    mt_ctrl->r2w_nops_max = -1;
    mt_ctrl->w2r_nops_min = -1;
...
_soc_tr_mem_interface_tune_lvl2(soc_memtune_data_t *mt_data)
{
    ...
    for (dt = -delta_max; dt <= delta_max; dt++) {
        for (dr = -delta_max; dr <= delta_max; dr++) {
            ...
            if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_FAIL ||
                ...
                for (i = start; i <= end; i++) {
                    soc_cm_print("SRAM%d TX=%d RX=%d ", i,
                                 tx_offset_cur[i], rx_offset_cur[i]);
                    if (em_latency_cur[i] != -1) {
                        soc_cm_print("EM lat=%d ", em_latency_cur[i]);
                    }
                    soc_cm_print(": Fail count=%d ", fail_count[i]);
                }
                soc_cm_print("\n");
            }
    ...
    for (i = start; i <= end; i++) {
        ...
        if (!has_fail[i]) {
            ...
            if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_FAIL_MATRIX) {
                soc_cm_print("Memory tuning unit %d %s interface",
                             mt_ctrl->unit, mt_ctrl->intf_name);
                if (mt_ctrl->em_latency_cur != -1) {
                    soc_cm_print(" em_latency=%d", mt_ctrl->em_latency_cur);
                }
                soc_cm_print(" failure count: all pass\n");
            }
        } else {
            /* Analyze and print result matrix */
        ...
        if (result->fail_count == offset_count[i]) {
            ...
            if (mt_ctrl->flags & SOC_MEMTUNE_CTRL_FLAGS_SHOW_SUMMARY) {
                soc_cm_print("%s OldTX=%d OldRX=%d ",
                             mt_ctrl->intf_name, tx_offset[i], rx_offset[i]);
                if (em_latency_min[i] != -1) {
                    soc_cm_print("OldEmLat=%d ", em_latency[i]);
                }
                soc_cm_print("NewTX=%d NewRX=%d ",
                             mt_ctrl->tx_offset_cur, mt_ctrl->rx_offset_cur);
                if (em_latency_min[i] != -1) {
                    soc_cm_print("NewEmLat=%d ", mt_ctrl->em_latency_cur);
                }
                soc_cm_print("Delta=%d ", mt_data->delta[i]);
                if (result->fail_count == 0) {
                    soc_cm_print("no failure seen in tested points\n");
                } else {
                    soc_cm_print("Area=%d Width=%d Height=%d\n",
                                 result->width * result->height,
                                 result->width, result->height);
                }
            }
            ...
            if (result->fail_count > 0 &&
                ...
                soc_cm_print("ERROR - \"eye\" of size %d * %d is smaller "
