/* $Id: phy84334.c 1.17.2.6 Broadcom SDK $
...
_phy_84334_no_reset_setup(int unit, soc_port_t port, int fiber)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_phy_84334_reset_setup: u=%d p=%d medium=%s\n",
                     unit, port,
...
phy_84334_init(int unit, soc_port_t port)
{
    ...
    int        fiber_preferred;
    bmacsec_dev_addr_t macsec_dev_addr, phy_dev_addr;
    int         mmi_mdio_addr, port_index, macsec_enable, phy_ext_boot;
    uint32      phy_mdi_pair_map;
    int         status;

    SOC_DEBUG_PRINT((DK_PHY, "phy_84334_init: u=%d p=%d\n",
                     unit, port));
  ...
  if (PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_DEFAULT ||
    ...
    if (mmi_mdio_addr == -1) {
        soc_cm_debug(DK_WARN, "phy_84334_init: "
    ...
    phy_dev_addr    = SOC_MACSECPHY_MDIO_ADDR(unit, pc->phy_id, 1); 
    macsec_dev_addr = SOC_MACSECPHY_MDIO_ADDR(unit, pc->macsec_dev_addr, 1);

    macsec_enable = soc_property_port_get(unit, port, spn_MACSEC_ENABLE, 0);
    pc->macsec_enable = macsec_enable;


    rv = bmacsec_phy84334_phy_mac_addr_set(phy_dev_addr, macsec_dev_addr, 
                                   pc->macsec_dev_port, bmacsec_io_mmi1_cl45_10gbt);

    if(rv != BMACSEC_E_NONE) {
        SOC_DEBUG_PRINT((DK_WARN, "phy_84334_init: bmacsec_phy_mac_addr_set "
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84334_init: "
  ...
  if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
    ...
    /*soc_cm_print("PASS 2: iter=%d, u= %d, p = %d\n",ITER(pc), pc->unit, pc->port ); */
  ...
  if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
    ...
    /* soc_cm_print("PASS 3: iter=%d, u= %d, p = %d\n",ITER(pc), pc->unit, pc->port ); */
...
phy_84334_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (pc->automedium || PHY_FIBER_MODE(unit, port)) {
        ...
        SOC_DEBUG_PRINT((DK_PHY, "phy_84334_enable_set: "
...
phy_84334_firmware_set(int unit, int port, int offset, uint8 *data,int len)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY,"PHY84334 firmware upgrade successful:"
                             "u=%d p=%d\n", unit, port));
