/*
 * $Id: mspi.c 1.5.2.6 Broadcom SDK $
...
int soc_mspi_config(int unit, int device, int cpol, int cpha) {
    ...
    if (SOC_IS_KATANA(unit)) {
        if(device != -1) {
            ...
            switch (device) {
                case MSPI_FLASH:
                    soc_cm_debug(DK_VERBOSE,"MSPI: Selecting Flash\n");
                    selval = 0;
                    break;
                case MSPI_LIU:
                    soc_cm_debug(DK_VERBOSE, "MSPI: Selecting LIU\n");
                    selval = 1;
                    break;
                case MSPI_DPLL:
                    soc_cm_debug(DK_VERBOSE, "MSPI: Selecting DPLL\n");
                    selval = 2;
                    break;
                case MSPI_EXTRA:
                    soc_cm_debug(DK_VERBOSE, "MSPI: Selecting Extra Mux\n");
                    selval = 3;
                    break;
                default:
                    /* What Device? */
