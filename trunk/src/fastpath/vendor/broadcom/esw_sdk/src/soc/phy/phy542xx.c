/*
 * $Id: phy542xx.c 1.5.4.21 Broadcom SDK $ 
...
#define PHY_BCM542XX_REV_BO(rev) ((rev & 0x7) == 0x1)
...
extern int
phy_54380_ecd_cable_diag_init(int unit, soc_port_t port);
extern int
...
{
    int primary, offset, phy_id_orig;
    phy_ctrl_t    *pc, *pc_base;
    ...
    pc_base = EXT_PHY_SW_STATE(unit, primary);

    /* Offset is the phy port index */
    SOC_IF_ERROR_RETURN
    ...
    */
    ...
    /* back up the original phy_id */
    phy_id_orig = pc->phy_id;

    /* replace it with qsgmii phy id */
    pc->phy_id = PHY_BCM54282_QSGMII_DEV_ADDR(pc_base->phy_id);    

    /* QSGMII FIFO Elasticity */
    phy_bcm542xx_qsgmii_reg_write(unit, pc, 
                                      offset /*dev_port*/,
                                      0x8300, 0x12, 0x0006);
    pc->phy_id =  phy_id_orig;   
    ...
    /* In case of QSGMII devices, there is a LPI pass through mode
       ...
       (phy_bcm542xx_qsgmii_reg_write(PHY_BCM54282_QSGMII_DEV_ADDR(pc_base->phy_id), 
                                      offset,
                                      0x833e, 0x0e, 0xc000));
    */
    return SOC_E_NONE;
...
phy_bcm542xx_power_seq_war(int unit, soc_port_t port)
{
    soc_port_t     primary;
    phy_ctrl_t    *pc, *pc_base;
    uint16 phy_id_orig, phy_base_addr;
    ...
    phy_id_orig = pc->phy_id;
    phy_base_addr = pc_base->phy_id;

    _phy_bcm542xx_get_model_rev(unit, pc_base, &oui, &model, &rev);
     ...
     */
    if( pc->phy_id == pc_base->phy_id) {
        /* POWER SEQUENCE FIX */
 
        num_ports = 8;   
        if (model == PHY_BCM54240_MODEL) {
            num_ports = 4;
        }
        for (index = 0; index < num_ports; ++index) {
            pc->phy_id = phy_base_addr + index;
                         ...
                         int copper_enable)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        ...
        SOC_DEBUG_PRINT((DK_WARN, "phy_bcm542xx_reset_setup: Config property 'phy_port_primary_and_offset' not set for u=%d, p=%d\n", unit, port));
                         ...
                         int copper_enable)
{
    ...
    */
    ...
    /* save original phy_id */
    phy_id_orig = pc->phy_id;
    pc->phy_id = pc_base->phy_id;
...
phy_bcm542xx_init(int unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN(
        phy_54380_ecd_cable_diag_init(unit, port));
...
phy_bcm542xx_reset(int unit, soc_port_t port, void *user_arg)
{
    ...
    int rv;
#if 0
    rv = phy_bcm542xx_reset(pc->phy_id);
#else
                         ...
                         soc_port_mdix_status_t *status)
{
    ...
    if(PHY_COPPER_MODE(unit, port)) {
        SOC_IF_ERROR_RETURN(
            (PHY_READ_BCM542XX_MII_ESRr(unit, pc, &mii_esrr)));
        if (mii_esrr & 0x2000) {
...
phy_bcm542xx_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    /* Enable TX SOP & RX SOP capability in 10BaseT speed. */
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_MPLS_CONTROL) {
        ...
        if (conf->mpls_control.flags & SOC_PORT_PHY_TIMESYNC_MPLS_CONTROL_WORD_ENABLE) {
            mpls_control_reg_tx |= PHY_BCM542XX_TOP_1588_MPLS_TX_CNTL_REG_CW_EN;
            mpls_control_reg_rx |= PHY_BCM542XX_TOP_1588_MPLS_RX_CNTL_REG_CW_EN;
        }
        ...
        phy_bcm542xx_direct_reg_modify(unit, pc, 
                         ...
                         | PHY_BCM542XX_TOP_1588_MPLS_TX_CNTL_REG_CW_EN 
...
phy_bcm542xx_timesync_config_get(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_MPLS_CONTROL) {
        ...
        if ( (mpls_control_reg_rx & PHY_BCM542XX_TOP_1588_MPLS_RX_CNTL_REG_CW_EN)
            /*&& (mpls_control_reg_tx & PHY_BCM542XX_TOP_1588_MPLS_TX_CNTL_REG_CW_EN)*/) {
...
/* CLOCK CONTROLS  */
        case SOC_PHY_CONTROL_CLOCK_ENABLE:
            ...
            phy_bcm542xx_clock_enable_set(unit, pc, offset, value);
        ...
        case SOC_PHY_CONTROL_CLOCK_SECONDARY_ENABLE:
            ...
            phy_bcm542xx_clock_secondary_enable_set(unit, pc, offset, value);
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_CLOCK_ENABLE:
            SOC_IF_ERROR_RETURN
                (soc_phyctrl_offset_get(unit, port, &offset));
            phy_bcm542xx_clock_enable_get(unit, pc, offset, value);
            break;

        case SOC_PHY_CONTROL_CLOCK_SECONDARY_ENABLE:
            SOC_IF_ERROR_RETURN
                (soc_phyctrl_offset_get(unit, port, &offset));
            phy_bcm542xx_clock_secondary_enable_get(unit, pc, offset, value); 
            break;
...
phy_driver_t phy_542xxdrv_ge = {
    "542XX (MACSec PHY) Gigabit PHY Driver",
    phy_bcm542xx_init,
    phy_fe_ge_reset,
    phy_bcm542xx_link_get,
    phy_bcm542xx_enable_set,
