 ...
 *
 *   The topology map used for bcm_topo_port_get() is an two
                    ...
                    bcm_trunk_add_info_t *ta, void *user_data)
{
    ...
    if (ta->num_ports > 1) {
        ...
        if (tid >= ti->trunk_fabric_id_min) {
            ...
            if (BCM_SUCCESS(rv)) {
                BOARD_VERB(("Board trunk u:%d tid:%d\n", unit, tid));
                rv = bcm_trunk_set(unit, tid, ta);
                if (BCM_SUCCESS(rv)) {
...
_board_program_vertex(int unit, int modid, bcm_gport_t port, void *user_data)
{
    ...
    do {
        ...
        BOARD_VERB(("Board modmap mod=%d u:%d,%d\n", modid, unit, port));
