...
*/
#define HAPI_BROAD_EGRESS_NORMAL_PRIORITY_COS (HAPI_BROAD_EGRESS_HIGH_PRIORITY_COS - 1)
...
#define HAPI_BROAD_INVALID_MODID                      (-1)
#define HAPI_BROAD_INVALID_MODPORT                    (-1)
...
typedef struct
{
  ...
  */
  L7_ushort16             pbvlan_table[L7_PBVLAN_MAX_CONFIGURABLE_PROTOCOLS];
  ...
  */
  ...
  /* DoS Policy Info. */
  L7_BOOL                 pingFloodPolicyApplied;
  L7_BOOL                 smurfAttackPolicyApplied;
  L7_BOOL                 synAckPolicyApplied;
...
/* LLPF PORT POINTERS */ 
#ifdef L7_LLPF_PACKAGE
  L7_uint32 llpfBlockProtoEnable;
#endif
  L7_BOOL   isdpEnable;
} BROAD_PORT_t;
...
struct BROAD_SYSTEM_s
{
  ...
  BROAD_POLICY_t         sysId1, sysId2, arpPolicyId;
  BROAD_POLICY_t         dhcpSnoopingPolicyId;          /* Trap DHCPS/DHCPC traffic and rate limit */
  BROAD_POLICY_t         dhcpDefaultPolicyId;           /* If snooping is disabled, just raise */
  ...
  BROAD_POLICY_t         isdpSysId;
#ifdef L7_LLPF_PACKAGE 
 /* policies for LLPF */
  BROAD_POLICY_t         llpfPolicyIds[HAPI_BROAD_LLPF_MAX_POLICIES];
#endif
};
...
/* Add a elem to TLV*/
#define BROAD_ADD_TLV_ELEM(buffer, type, len, value) \
  do { \
    L7_ushort16 _val; \
    _val = osapiHtons((type)); \
    memcpy((buffer), &(_val), sizeof(L7_ushort16)); \
    (buffer) += sizeof(L7_ushort16); \
    _val = osapiHtons((len)); \
    memcpy((buffer), &(_val), sizeof(L7_ushort16)); \
    (buffer) += sizeof(L7_ushort16); \
...
/* Get next elem from TLV */
#define BROAD_GET_NEXT_TLV_ELEM(buffer, type, len, data) \
  do { \
    L7_ushort16 _val; \
    memcpy(&_val, (buffer), sizeof(L7_ushort16)); \
    (type) = osapiNtohs(_val); \
    (buffer) += sizeof(L7_ushort16); \
    memcpy(&_val, (buffer), sizeof(L7_ushort16)); \
    (len) = osapiNtohs(_val); \
    (buffer) += sizeof(L7_ushort16); \
...
* @returns L7_RC_t result
*
* @notes   Should be called only on manager unit
* @end
*
*********************************************************************/
