				  ...
				  struct resource *rsrc)
{
	...
	printk("%s:%d|%s|pcicsrbar:0x%8.8X\n",__FILE__,__LINE__,__func__,pcicsrbar);
...
int __init mpc83xx_add_bridge(struct device_node *dev)
{
	...
	pr_debug("%d Adding PCI host bridge %s\n",__LINE__,dev->full_name);
...
u64 fsl_pci_immrbar_base(struct pci_controller *hose)
{
	...
	if (is_mpc83xx_pci) {
		...
		for (i = 0; i < 4; i++) {
			...
			{
				u64 val = (u64)in_le32(&in[i].barh) << 32 |
					    in_le32(&in[i].barl);
				printk("%s:%d:%s val:%llX\n",__FILE__,__LINE__,__func__,val);
				return val;
			}
		}
