 ...
 *  Revision      :  $Id: falcon_furia_internal.c 844 2015-01-12 20:58:16Z kirand $ *
 *                                                                                 *
...
#endif
err_code_t falcon_furia_log_full_pmd_state ( const phymod_access_t *pa, struct falcon_furia_detailed_lane_status_st *lane_st) {
    ...
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_AMP, &tmp)); lane_st->amp_ctrl = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_PRE, &tmp)); lane_st->pre_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_MAIN, &tmp)); lane_st->main_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_POST1, &tmp)); lane_st->post1_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_POST2, &lane_st->post2_tap));
    EFUN(falcon_furia_read_tx_afe( pa, TX_AFE_POST3, &lane_st->post3_tap));
    ...
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_D,&reg_data));
    lane_st->ddq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_P,&reg_data));
    lane_st->ppq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_L,&reg_data));
    lane_st->llq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_PD,&reg_data));
    lane_st->dp_hoffset = (uint8_t)dist_cw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_LD,&reg_data));
    ...
    ESTM(lane_st->heye_left = _eye_to_mUI(rdv_usr_sts_heye_left()));
    ESTM(lane_st->heye_right = _eye_to_mUI(rdv_usr_sts_heye_right()));
    ESTM(lane_st->veye_upper = _eye_to_mV(rdv_usr_sts_veye_upper(), 0));
    ESTM(lane_st->veye_lower = _eye_to_mV(rdv_usr_sts_veye_lower(), 0));
    ...
    lane_st->prbs_chk_order = (reg_data==0)?7:
                              ...
                              (reg_data==5)?31:
                              (reg_data==6)?58:0;
    EFUN(falcon_furia_prbs_chk_lock_state( pa, &lane_st->prbs_chk_lock));
    EFUN(falcon_furia_prbs_err_count_ll( pa, &lane_st->prbs_chk_errcnt));
...
err_code_t falcon_furia_disp_full_pmd_state ( const phymod_access_t *pa, struct falcon_furia_detailed_lane_status_st *lane_st, uint8_t num_lanes) {
    uint8_t i;
    err_code_t err_code = 0;
    USR_PRINTF(("\n\n"));
    USR_PRINTF(("------------------------------------------------------------------------\n"));
...
err_code_t _rptr_mode_timing( const phymod_access_t *pa ) {
    ...
    err_code = falcon_furia_delay_us(25);                     /* Wait for TX_PI to settle */
    return(err_code);
  }

/* Setup the Ultra low latency clk and datapath [Return Val = Error Code (0 = PASS)] */
err_code_t _ull_mode_setup( const phymod_access_t *pa ) {
    ...
    err_code = falcon_furia_delay_us(1024); 
    ...
    err_code = falcon_furia_poll_st_afe_tx_fifo_resetb_equals_1( pa, 1);  /* Check if FIFO reset is reported to be released within 1ms time interval */
    if (err_code) {
      return (err_code);
    }
...
char* _status_val_2_str( const phymod_access_t *pa, uint8_t val) {
	switch (val) {
	case 3:
		return ("MAX");
	case 2:
...
char* _update_val_2_str( const phymod_access_t *pa, uint8_t val) {
	switch (val) {
	case 2:
		return ("DEC");
	case 1:
...
/* convert float12 to uint32 */
 uint32_t _float12_to_uint32( const phymod_access_t *pa, uint8_t byte, uint8_t multi) {
...
err_code_t _get_rx_dfe2( const phymod_access_t *pa, int8_t *val) {
 ...
 err_code_t _get_tx_pre( const phymod_access_t *pa, int8_t *val) {
	ESTM(*val = rd_txfir_pre_after_ovr());
 return (ERR_CODE_NONE);
}

 err_code_t _get_tx_main( const phymod_access_t *pa, int8_t *val) {
	ESTM(*val = rd_txfir_main_after_ovr());
 return (ERR_CODE_NONE);
}

 err_code_t _get_tx_post1( const phymod_access_t *pa, int8_t *val) {
	ESTM(*val = rd_txfir_post_after_ovr());
 return (ERR_CODE_NONE);
}

 err_code_t _get_tx_post2( const phymod_access_t *pa, int8_t *val) {
...
err_code_t _falcon_furia_get_osr_mode( const phymod_access_t *pa, falcon_furia_osr_mode_st *imode) {
    falcon_furia_osr_mode_st mode;

     USR_MEMSET(&mode, 0, sizeof(falcon_furia_osr_mode_st));
...
err_code_t _falcon_furia_read_lane_state( const phymod_access_t *pa, falcon_furia_lane_state_st *istate) {  
  ...
  USR_MEMSET(&state, 0, sizeof(falcon_furia_lane_state_st));  
  ...
  USR_MEMSET(&osr_mode, 0, sizeof(falcon_furia_osr_mode_st));  
  EFUN(_falcon_furia_get_osr_mode( pa, &osr_mode));
  ...
  state.p1_lvl = _ladder_setting_to_mV((int8_t)state.p1_lvl, 0);
  ESTM(state.m1_lvl = rd_rx_phase_thresh_sel());
  state.m1_lvl = _ladder_setting_to_mV((int8_t)state.m1_lvl, 0);
...
err_code_t _falcon_furia_display_lane_state_no_newline( const phymod_access_t *pa ) {     
  ...
  EFUN(_falcon_furia_read_lane_state( pa, &state));
 
  lane_idx = falcon_furia_get_lane(pa); 
...
err_code_t _falcon_furia_read_core_state( const phymod_access_t *pa, falcon_furia_core_state_st *istate) {  
  ...
  err_code_t  err_code;
  falcon_furia_core_state_st state;
  struct falcon_furia_uc_core_config_st core_cfg;

  USR_MEMSET(&state, 0, sizeof(falcon_furia_core_state_st));
  USR_MEMSET(&core_cfg, 0, sizeof(struct falcon_furia_uc_core_config_st));

  if(!istate) 
	  return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  err_code = falcon_furia_get_uc_core_config( pa, &core_cfg);
  if (err_code) {
    return (err_code);
  }
...
err_code_t _falcon_furia_display_core_state_no_newline( const phymod_access_t *pa ) {
  ...
  EFUN(_falcon_furia_read_core_state( pa, &state));
  USR_PRINTF(("%02d  ",falcon_furia_get_core(pa)));
  ...
  USR_PRINTF(("   x%02x   ",state.afe_hardware_version));
  ...
  USR_PRINTF(("   0x%04x  ", state.avg_tmon));
...
#define BFMASK(n) ((1<<((n)))-1)
 ...
 void _update_uc_lane_config_st( const phymod_access_t *pa, struct  falcon_furia_uc_lane_config_st *st) {
 ...
 void _update_usr_ctrl_disable_functions_st( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st *st) {
 ...
 void _update_usr_ctrl_disable_dfe_functions_st( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_dfe_functions_st *st) {
...
/* word to fields, for display */
 void _update_uc_core_config_st( const phymod_access_t *pa, struct falcon_furia_uc_core_config_st *st) {
...
/* fields to word, to write into uC RAM */
 void _update_uc_core_config_word( const phymod_access_t *pa, struct falcon_furia_uc_core_config_st *st) {
  uint16_t in = 0;
 ...
 void _update_uc_lane_config_word( const phymod_access_t *pa, struct falcon_furia_uc_lane_config_st *st) {
 ...
 void _update_usr_ctrl_disable_functions_byte( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st *st) {
 ...
 void _update_usr_ctrl_disable_dfe_functions_byte( const phymod_access_t *pa, struct  falcon_furia_usr_ctrl_disable_dfe_functions_st *st) {
