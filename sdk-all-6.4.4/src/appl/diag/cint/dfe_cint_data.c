/*
 * $Id: dfe_cint_data.c,v 1.15 Broadcom SDK $
 *
...
STATIC int 
dfe_multicast_multi_get(
    int unit, 
    uint32 flags, 
    uint32 ngroups, 
    bcm_multicast_t *groups, 
    SHR_BITDCL* dest_array)
{
    int i, j, rv;
       
    bcm_fabric_module_vector_t  *dest_matrix = (bcm_fabric_module_vector_t *) sal_alloc(ngroups * sizeof(bcm_fabric_module_vector_t), "dfe_multicast_multi_set");
    ...
    rv = bcm_fabric_multicast_multi_get(unit, flags, ngroups, groups, dest_matrix);
    for(i=0; i<ngroups; i++)
    {
        for(j=0; j<4; j++)
        {
            dest_array[i*4 + j] = dest_matrix[i][j];
        }
    }
    ...
    SHR_BITDCL* dest_array)
{
    ...
    for(i=0; i<ngroups; i++)
    {
        for(j=0; j<4; j++)
        {
            dest_matrix[i][j] = dest_array[i*4 + j];
        }
    }

    return bcm_fabric_multicast_multi_set(unit, flags, ngroups, groups, dest_matrix);
...
CINT_FWRAPPER_CREATE_RP5(int, int, 0, 0,
                         ...
                         uint32,uint32,ngroups,0,0,
                         bcm_multicast_t*,bcm_multicast_t,groups,1,0,
                         SHR_BITDCL*,SHR_BITDCL,dest_array,1,0);
...
static cint_function_t __cint_dfe_functions[] = 
    {
        ...
        CINT_FWRAPPER_ENTRY(dfe_multicast_multi_get),
        CINT_FWRAPPER_ENTRY(dfe_multicast_multi_set),
