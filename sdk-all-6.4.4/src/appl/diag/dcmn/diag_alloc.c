...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    {  "Local Common InLif"                    , "Common_Inlif"  , "LCLif"  , dpp_am_res_local_inlif_common             , 1                        , NULL                              , NULL                             },
    {  "Local Wide InLif"                      , "Wide_Inlif"    , "LWLif"  , dpp_am_res_local_inlif_wide               , 1                        , NULL                              , NULL                             },
    {  "Local OutLif"                          , "Local_Outlif"  , "LOLif"  , dpp_am_res_local_outlif                   , 1                        , NULL                              , NULL                             },
...
cmd_result_t diag_alloc_gport_info_get(int unit, 
    ...
    ret = _bcm_dpp_gport_to_global_lif(unit, 
                          gport_info->gport_id, 
                          &(gport_info->lif_id), 
...
cmd_diag_alloc_sw_common_eval_object(int unit, 
                                        ...
                                        int pool_idx) { 
    int res_id_start = -1; /* first pool id */
    ...
    res_id_start = pools[pool_idx].pool_id_start;
    nbrOfPools    = pools[pool_idx].number_of_pools;

    /* for each pool id, display ids */
    for (res_id=res_id_start; res_id<res_id_start+nbrOfPools; ++res_id) {
...
cmd_result_t cmd_diag_alloc_my_in_test(int unit)
{
    ...
    inlif_info.app_type = bcm_dpp_am_ingress_lif_app_mpls_term;
...
cmd_result_t cmd_diag_alloc_my_out_test(int unit)
{
       ...
       uint32 flags = 0; /* BCM_DPP_AM_LIF_FLAG_WITH_ID*/
        ...
        outlif_info.glif = 21; /* pre-alloced Global LIF ID*/ 
        ...
        outlif_info.glif = 22; /* pre-alloced Global LIF ID*/ 
        rv = _bcm_dpp_am_local_outlif_alloc(unit, flags, &outlif_info);
        if (rv != BCM_E_NONE) {
            return CMD_FAIL;
        }
        ...
        outlif_info.glif = global_lif_id; /* pre-alloced Global LIF ID*/ 
        rv = _bcm_dpp_am_local_outlif_alloc(unit, flags, &outlif_info);
        if (rv != BCM_E_NONE) {
            return CMD_FAIL;
        }
    ...
    outlif_info.app_alloc_info.application_type = 0;
    outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED;
    outlif_info.glif = 25; /* pre-alloced Global LIF ID*/ 
    rv = _bcm_dpp_am_local_outlif_alloc(unit, flags, &outlif_info);
    if (rv != BCM_E_NONE) {
        return CMD_FAIL;
    }
    ...
    outlif_info.app_alloc_info.application_type = 0;
    outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED;
    outlif_info.glif = 26; /* pre-alloced Global LIF ID*/ 
    rv = _bcm_dpp_am_local_outlif_alloc(unit, flags, &outlif_info);
    if (rv != BCM_E_NONE) {
        return CMD_FAIL;
    }
    ...
    rv = _bcm_dpp_am_local_outlif_dealloc(unit, lif_id[0]);
    if (rv != BCM_E_NONE) {
        return CMD_FAIL;
    }
...
cmd_result_t cmd_diag_alloc_eedb_banks(int unit)
{
    ...
    for (bank_id_iter = 0 ; bank_id_iter < _BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_HALF_BANKS ; bank_id_iter++) 
    {
        ...
        cli_out(" Phase= %d , is extended = %d, Type=%d  \n\r", eedb_bank_info.phase, eedb_bank_info.is_extended , eedb_bank_info.type  );
