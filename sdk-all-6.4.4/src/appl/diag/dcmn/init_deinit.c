...
"  ElkMdioId=<value>      ELK MDIO id number. (default=0x101)\n\n"
#endif /* defined(BCM_ARAD_SUPPORT) && defined(INCLUDE_KBP) && !defined(BCM_88030) */
...
STATIC int appl_dcmn_linkscan_init(int unit, int warm_boot){
    ...
    if (warm_boot) {
        ...
        /* Turn linkscan on */
        linkscan_interval = soc_property_get(unit, spn_BCM_LINKSCAN_INTERVAL, 250000);
...
appl_dcmn_init(int unit, appl_dcmn_init_param_t* init_param) {
...
#ifdef BCM_ARAD_SUPPORT	
	/*Load the uKernel if spn_L2_LEARN_LIMIT_MODE is pon tunnel mode */
    if (soc_property_get(unit, spn_MCS_LOAD_UC0, 0)) { 
    ...
    /* call to sysconf_chip_override cause kernel build error */
            ...
            if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit)) {
                sal_snprintf(mcs_load, sizeof(mcs_load) -1, "mcsload 0 %s_0pon.srec ResetUC=true InitMCS=true StartUC=true StartMSG=true;", dev_rev);
            } else {
                sal_snprintf(mcs_load, sizeof(mcs_load) -1, "mcsload 0 %s_0.srec ResetUC=true InitMCS=true StartUC=true StartMSG=true;", dev_rev);
            }
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    /* init KBP device */
    if (SOC_DPP_IS_ELK_ENABLE(unit) && (!init_param->no_elk_device) && SOC_IS_ARAD(unit)) {
        LOG_INFO(BSL_LS_BCM_INIT,
                 (BSL_META_U(unit,
                             "%d: Init KBP interface.\n"), unit));
        rv = dpp_kbp_init_kbp_interface(unit, init_param->elk_mdio_id, init_param->elk_ilkn_rev);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_SHELL,
                      (BSL_META_U(unit,
                                  "KBP device init Failed:\n")));
            return rv;
        }
    }
#endif 
    ...
    if (SOC_IS_ARAD(unit)){
        /* init linkscan */
        if ( (!init_param->no_linkscan) && !SOC_IS_JERICHO(unit)){
            LOG_INFO(BSL_LS_BCM_INIT,
                     (BSL_META_U(unit,
                                 "%d: Init LinkScan Appl.\n"), unit));
...
int appl_dcmn_deinit(int unit, appl_dcmn_init_param_t* init_param) 
{
    ...
    /* deinit linkscan */
    if (SOC_IS_ARAD(unit) && !SOC_IS_JERICHO(unit) && !init_param->no_linkscan) {
...
appl_dcmn_init_usage_parse(int unit, args_t *a, appl_dcmn_init_param_t *init_param)
{
    ...
    parse_table_add(&pt, "ElkMdioId", PQ_INT, (void *) ARAD_KBP_APPL_MDIO_DEFAULT_ID,
