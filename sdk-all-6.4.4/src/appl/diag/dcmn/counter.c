...
/*for PetraB only*/
STATIC char *
dpp_counter_name_get(int unit, soc_reg_t ctr_reg)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        return soc_dpp_petra_nif_ctr_names[ctr_reg];
    }
#endif /* BCM_PETRAB_SUPPORT */
...
/*
 * Function:
 *      do_resync_dpp_counters
 * Purpose:
 *      Resynchronize the previous counter values (stored in dcmn_counter_val)
 *      with the actual counters.
 * Note: for PetraB only
 */
STATIC int
do_resync_dpp_counters(int unit, soc_pbmp_t pbmp)
{
#if defined (BCM_PETRAB_SUPPORT) 
    soc_port_t          port;
    int                 i;
    ...
    SOC_PBMP_ITER(pbmp, port) {
        ...
        for (i = 0; i < cmap->cmap_size; i++) {
            ...
            if ((reg != INVALIDr) &&
                (soc_counter_get(unit, port, reg, 0, &val) == SOC_E_NONE)) {
                dpp_counter_val_set(unit, port, reg, 0, val);
            }
        }
    }
                ...
                int ar_idx, int flags)
{
    ...
    } else if (!_SOC_CONTROLLED_COUNTER_USE(unit, port) && !SOC_IS_PETRAB(unit)) {
   ...
   if(ctr_reg >= NUM_SOC_REG) {
        ...
        ) 
    {
        ...
        if (SOC_IS_PETRAB(unit)) {
             char    *ctr_name;
             ctr_name = dpp_counter_name_get(unit, ctr_reg);
            if (ctr_name == NULL) {
                return;
             }
             sal_sprintf(cname, "%s.%s", ctr_name, SOC_PORT_NAME(unit, port));
            ...
            if (cmap == NULL) {
                ...
                for (index = 0; soc->controlled_counters[index].controlled_counter_f != NULL; index++) {
                    ...
                    if (SOC_IS_ARAD(unit)) {
                        soc_error_t rv;
                        int enable = 0;
                        rv = soc_arad_stat_controlled_counter_enable_get(unit, port, index, &enable);
...
cmd_dpp_counter(int unit, args_t *a)
{
    ...
    if (sync) {
        ...
        if(SOC_IS_PETRAB(unit)) {
            do_resync_dpp_counters(unit, pbmp);
        }
        return CMD_OK;
    }
...
#endif /* BCM_PETRAB_SUPPORT || BCM_ARAD_SUPPORT */
