/*
 * $Id: parse.c,v 1.47 Broadcom SDK $
 ...
 * Notes:       In case of multiple matches, an exact match
 ...
 */
{
    ...
    case PQ_PORTABIL:
        ...
        if (sal_strcmp(s, "?") == 0 || parse_port_ability(s, &pa)) {
            ...
            cli_out("Examples: 21g,1000full,100,10,pause 100,pause_tx,pause_rx\n");
            return(-2);
        }
        v = (void *)&pa;
        vs = sizeof(pa);
 ...
 */
{
    ...
    while (pq_cur < pt->pt_cnt) {
        ...
        switch(PQ_TYPE(pq->pq_type)) {
        case PQ_INT64:
            COMPILER_64_ZERO(*(uint64*)ds);
            COMPILER_64_OR(*(uint64*)ds, *(uint64*)defl);
            break;
        case PQ_INT:
        case PQ_INT8:
        case PQ_INT16:
 ...
 */
{
    return(parse_do_lookup(s, (void *)t, size, cnt, '\0'));
}
