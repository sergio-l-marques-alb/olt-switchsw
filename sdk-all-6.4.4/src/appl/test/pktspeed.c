...
pktspeed_run_test(int tx_test, pktspeed_param_t *psp)
{
    ...
    if (tx_test) {
        ...
        if (!SOC_IS_ARAD(psp->unit)) {   
            ...
            bcm_cosq_gport_bandwidth_set(psp->unit, sch_gport, 0, 0, 10000000, 0);
            BCM_GPORT_LOCAL_SET(egq_gport, 0);
            bcm_cosq_gport_bandwidth_set(psp->unit, egq_gport, 0, 0, 10000000, 0);
        }
