...
STATIC uint32 last_allocated_sid_2; /* for MY_STATION_TCAM_2 */
...
_bcm_tr_l2_station_control_init(int unit)
{
    ...
    last_allocated_sid = (_BCM_L2_STATION_ID_BASE - 1);
    ...
    if (soc_feature(unit, soc_feature_riot)) {
        ...
        last_allocated_sid_2 = (_BCM_L2_STATION_ID_2_BASE - 1);
        max_entries = soc_mem_index_count(unit, tcam_mem);
...
#ifdef BCM_RIOT_SUPPORT
    if (_BCM_L2_STATION_ID_OVERLAY(sid)) {
...
_bcm_tr_l2_from_l2x(int unit, bcm_l2_addr_t *l2addr, l2x_entry_t *l2x_entry)
{
...
#if defined(INCLUDE_L3)
        if (_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DEST_TYPE_f) == 2) {
            int vp;
...
#endif /* INCLUDE_L3 */
        if (_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, T_f)) {
            /* Trunk group */
    ...
    if (soc_feature(unit, soc_feature_l2_entry_used_as_my_station)
        ...
        if ((_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DEST_TYPE_f) == 1)
                 && (_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DESTINATION_f) 
                     & _BCM_L2_DEST_STATION_HIT)) {
            if ((_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DESTINATION_f)
                     & _BCM_L2_DEST_STATION_IPV4)
                 || (_BCM_L2X_MEMACC_FIELD32_GET(unit, l2x_entry, DESTINATION_f)
                     & _BCM_L2_DEST_STATION_IPV6)
                  ...
                  int key_only)
{
    ...
    if (BCM_MAC_IS_MCAST(l2addr->mac)) {
        ...
        if (_BCM_MULTICAST_IS_SET(l2addr->l2mc_group)) {
            ...
            soc_L2Xm_field32_set(unit, l2x_entry, L2MC_PTRf, l2addr->l2mc_group);
        }
    } else {
        bcm_port_t      port = -1;
...
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWK(unit)) {
            rv = soc_tomahawk_hash_bank_info_get(unit, L2Xm, bank,
                                                 &entries_per_bank,
                                                 &entries_per_row,
                                                 &entries_per_bucket,
                                                 &bank_base, &bucket_offset);
            ...
            bucket = soc_th_l2x_bank_entry_hash(unit, bank,
                                                 (uint32 *)l2x_entry);
        } else
#endif
        {
...
_bcm_l2_station_non_tcam_reload(int unit, _bcm_l2_station_control_t *sc) 
{
    ...
    if ((mac_field[0] > 0) || (mac_field[1] > 0)) {
        ...
        if (SOC_IS_KATANA2(unit)) {
            entry_index = sc->entries_total + sc->port_entries_total +
                            sc->olp_entries_total + index;
        }
#endif
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_fp_based_oam)) {
            entry_index = sc->entries_total + sc->olp_entries_total + index;
        }
#endif
        sc->entry_arr[entry_index] = s_ent_p;
        s_ent_p->hw_index = 0;
...
#ifdef BCM_RIOT_SUPPORT
        if (overlay == _BCM_L2_STATION_ENTRY_OVERLAY) {
            s_ent_p->sid = ++last_allocated_sid_2;
...
bcm_tr_l2_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_tr_l2_memacc_init(unit));
                             ...
                             int *cf_count)
{
    ...
    *cf_count = 0;
    ...
    for (bank = 0; bank < num_banks; bank++) {
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            BCM_IF_ERROR_RETURN
                (soc_tomahawk_hash_bank_info_get(unit, L2Xm, bank,
                                                 &entries_per_bank,
                                                 &entries_per_row,
                                                 &entries_per_bucket,
                                                 &bank_base, &bucket_offset));
        ...
        for (index = 0; index < DEFAULT_L2DELETE_CHUNKS; index++) {
            ...
            if (soc_L2Xm_field32_get(unit, l2x_entry, VALIDf)) {
                if (soc_L2Xm_field32_get(unit, l2x_entry, KEY_TYPEf) ==
                    ...
                    inner_vlan = soc_L2Xm_field32_get(unit, l2x_entry, IVIDf);
                ...
                /* Scorpion uses index to L3_NEXT_HOP as VPG */
                    ...
                    BCM_GPORT_TRUNK_SET(port_1, 
                        soc_L2Xm_field32_get(unit, l2x_entry, TGIDf));
                } else {
                ...
                if (SOC_MEM_FIELD_VALID(unit, mem, VPG_TYPE_1f) && 
                    ...
                    BCM_GPORT_TRUNK_SET(port_2, 
                        soc_L2Xm_field32_get(unit, l2x_entry, TGID_1f));
                } else {
...
#ifdef BCM_RIOT_SUPPORT
    if (_BCM_L2_STATION_OVERLAY(station) && !_BCM_VPN_VFI_IS_SET(station->vlan)) {
        soc_mem_field32_set(unit, tcam_mem, ent_p->tcam_ent,
                            VFI_IDf, station->vlan);
        soc_mem_field32_set(unit, tcam_mem, ent_p->tcam_ent,
                            VFI_ID_MASKf, station->vlan_mask);
        soc_mem_field32_set(unit, tcam_mem, ent_p->tcam_ent,
...
int _bcm_olp_port_mac_get(int unit, bcm_port_t port, bcm_mac_t *mac)
{
    int dglp;
    int rv = BCM_E_NONE;
    bcm_module_t modid;
    ...
    rv = _bcm_check_olp_dglp(unit, dglp, mac);
    if(rv == BCM_E_EXISTS) {
        return BCM_E_NONE;
    }
    return rv;
...
STATIC int
_bcm_olp_mac_addr_mask_get(int unit, uint32 *lowest_mac_addr, uint32 *mac_mask)
{
    _bcm_l2_station_control_t *sc;           /* Station control structure. */
    egr_olp_config_entry_t eolp_cfg;
    bcm_mac_t  olp_mac;
    ...
    max_entries = soc_mem_index_count(unit, EGR_OLP_CONFIGm);
    mac_mask[0] = 0;
    mac_mask[1] = 0;

    for (; index < max_entries; index++) {
        SOC_IF_ERROR_RETURN(
              READ_EGR_OLP_CONFIGm(unit, MEM_BLOCK_ANY, 
                                           index, &eolp_cfg));
        soc_mem_mac_addr_get(unit, EGR_OLP_CONFIGm, &eolp_cfg, 
                                                    MACSAf, olp_mac);
...
_bcm_l2_xgs_mac_set(int unit, int index, bcm_mac_t xgs_mac) 
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_olp_mac_addr_mask_get(unit, mac_addr, mac_mask));
...
_bcm_l2_station_entry_install(int unit, _bcm_l2_station_entry_t *ent_p)
{
    ...
    rv = _bcm_l2_station_tcam_mem_get(unit,
              _BCM_L2_STATION_ID_OVERLAY(ent_p->sid), &tcam_mem);
    if (BCM_FAILURE(rv)) {
        return (rv);
    }
    ...
    if ((SOC_IS_KATANA2(unit) || 
       ...
       ((station->flags & BCM_L2_STATION_OAM) || 
        ...
        if (_BCM_L2_STATION_OVERLAY(station)) {
            ...
            ParamCheck(unit, tcam_mem, VFI_IDf, station->vlan);
            ParamCheck(unit, tcam_mem, VFI_ID_MASKf, station->vlan_mask);
        } else
#endif /* BCM_RIOT_SUPPORT */
        {
                      ...
                      bcm_l2_station_t *station)
{
    ...
    if (station->flags & BCM_L2_STATION_WITH_ID) {
        if ((last_allocated_sid < *station_id) &&
            (*station_id <= _BCM_L2_STATION_ID_MAX)) {
            last_allocated_sid = *station_id;
        }
        sid = *station_id;
    } else {
...
#ifdef BCM_RIOT_SUPPORT
        if (_BCM_L2_STATION_OVERLAY(station)) {
            sid = ++last_allocated_sid_2;

            if (_BCM_L2_STATION_ID_2_MAX < sid) {
                last_allocated_sid_2 = _BCM_L2_STATION_ID_2_MAX;
...
#endif /* BCM_RIOT_SUPPORT */
        {
            ...
            if (_BCM_L2_STATION_ID_MAX < sid) {
                last_allocated_sid = _BCM_L2_STATION_ID_MAX;
...
bcm_tr_l2_station_delete(int unit, int station_id)
{
    ...
    if (station_id == last_allocated_sid) {
        --last_allocated_sid;
    }
...
bcm_tr_l2_station_get(int unit, int station_id, bcm_l2_station_t *station)
{
...
#endif
    rv = _bcm_l2_station_tcam_mem_get(unit,
             _BCM_L2_STATION_ID_OVERLAY(station_id), &tcam_mem);
    if (BCM_FAILURE(rv)) {
        SC_UNLOCK(sc);
        return (rv);
    }
    ...
    if (overlay) {
        station->vlan
            = soc_mem_field32_get(unit, tcam_mem, entry, VFI_IDf);

        station->vlan_mask
            = soc_mem_field32_get(unit, tcam_mem, entry, VFI_ID_MASKf);
    } else
#endif /* BCM_RIOT_SUPPORT */
    {
        station->vlan
    ...
    if (SOC_IS_KATANAX(unit)
        ...
        if (SOC_MEM_FIELD_VALID(unit, tcam_mem, SOURCE_FIELDf)) {
            ...
            if (soc_feature(unit, soc_feature_riot) && (src_type & 0x1)) {
                ...
                _bcm_vp_gport_dest_fill(unit, src_mask_field, &src_mask);
            } else
#endif /* BCM_RIOT_SUPPORT */
