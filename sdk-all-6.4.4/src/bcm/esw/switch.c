...
/* #if defined(BCM_TRIDENT2PLUS_SUPPORT)
    ...
    if (BCM_GPORT_IS_SET(port)) {
        ...
        if(soc_feature(unit, soc_feature_separate_ing_lport_rtag7_profile)) {
            ...
            info->idx = index; */
            /* Last set of entries reserved for Front-Panel ports, hence adjust the index */
            info->idx = 
                          ...
                          bcm_switch_control_t type, int arg)
{
        ...
        if (BCM_GPORT_IS_PROXY(port) ||
            ...
            if ((info.hash_concat == 1) && (info.concat_f != -1)) {
                soc_mem_field32_set(unit,
                                    info.regmem, hash_control_entry,
                                    info.concat_f, 1);
            } else if ((info.hash_concat == 0) && (info.concat_f != -1)){
                soc_mem_field32_set(unit,
                                    info.regmem, hash_control_entry,
                                    info.concat_f, 0);
            }
    ...
    if (SOC_REG_IS_VALID(unit, info.regmem)) {
        ...
        if ((info.hash_concat == 1) && (info.concat_f != -1)) {
            ...
            soc_reg_field_set(unit,
                             info.regmem, &hash_control, info.concat_f, 0);
        }
        SOC_IF_ERROR_RETURN
            (soc_reg32_set(unit,
