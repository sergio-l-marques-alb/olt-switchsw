...
defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_SABER2_SUPPORT)
    uint32  rval;
#endif
 #if defined(BCM_HURRICANE2_SUPPORT) || defined(BCM_GREYHOUND_SUPPORT) ||  defined(BCM_SABER2_SUPPORT)
    int val, index;
#endif
...
defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_SABER2_SUPPORT)
    ...
    if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) || SOC_IS_SABER2(unit)) {
        for(index=0;
