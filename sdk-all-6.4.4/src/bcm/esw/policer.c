...
int _bcm_esw_policer_group_create(int unit, bcm_policer_group_mode_t mode,
                                   int skip_pool, bcm_policer_map_t *offset_map,
                                   bcm_policer_t *policer_id,
                                   int *npolicers)
{
    int rv = BCM_E_NONE;
...
{    
    ...
    if (BCM_FAILURE(rv) && (rv != BCM_E_EXISTS)) {
        ...
        if (mode_attr->udf_pkt_attr_selectors_v.type 
                               == bcmPolicerGroupModeTypeNormal) {
            mode_attr->mode_type_v = udf_mode;
        } else if (mode_attr->udf_pkt_attr_selectors_v.type 
                              == bcmPolicerGroupModeTypeCascade) {
            mode_attr->mode_type_v = udf_cascade_mode;
        } else {
            mode_attr->mode_type_v = udf_cascade_with_coupling_mode;
        }
    ...
    uint32 mode_id)
{
    ...
    if (_bcm_policer_svc_meter_get_mode_info(unit, mode_id, &mode_info)
                                                                     != BCM_E_NONE) {
        return (BCM_E_PARAM);
    }
    ...
    uint32 *actual_num_selectors)
{
    ...
    if (_bcm_policer_svc_meter_get_mode_info(unit, mode_id - 1, &mode_info)
                                                                     != BCM_E_NONE) {
        return (BCM_E_PARAM);
    }
        ...
        case udf_cascade_with_coupling_mode: 
            ...
            *actual_num_selectors = udf_attr->num_selectors;
            ...
            for (i = 0; i < num_selectors; i++) {
                ...
                if (udf_attr->offset > 0) {
                    attr_selectors[i].offset = udf_attr->offset - 1;
                } else {
                    attr_selectors[i].offset = udf_attr->offset;
                }    
                attr_selectors[i].width = udf_attr->width;
                attr_selectors[i].udf_id = udf_attr->udf_id;
                attr_selectors[i].flags |= BCM_POLICER_ATTR_WIDTH_OFFSET;
                attr_selectors[i].attr = bcmPolicerGroupModeAttrUdf;
    ...
    uint32 *npolicers)
{
    ...
    GLOBAL_METER_LOCK(unit);
    rv = _global_meter_policer_id_alloc(unit, direction,
                            (int *)npolicers, policer_id, skip_pool,
                            &pid_offset[0]);
    if (!(BCM_SUCCESS(rv))) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_VERBOSE(BSL_LS_BCM_POLICER, (BSL_META_U(unit,
                                 ...
                                 uint32  selector_for_bit_x_field_value[8])
{
    ...
    for (entry_index = 0; entry_index < 256; entry_index++) {
        entry = soc_mem_table_idx_to_pointer(unit, 
                              ...
                              buf, entry_index);
        soc_mem_field_get(unit, SVM_OFFSET_TABLEm, (void *)entry,
                           OFFSETf, (uint32 *)&udf_attr->offset_map[entry_index].offset);
        soc_mem_field_get(unit, SVM_OFFSET_TABLEm, (void *)entry,
                           METER_ENABLEf, (uint32 *)&udf_attr->offset_map[entry_index].meter_enable);
        soc_mem_field_get(unit, SVM_OFFSET_TABLEm, (void *)entry,
                           POOL_OFFSETf, (uint32 *)&udf_attr->offset_map[entry_index].pool);
