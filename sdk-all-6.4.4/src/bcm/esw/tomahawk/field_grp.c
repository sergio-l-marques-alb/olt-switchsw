    ...
    for (idx = 0; idx < qual_count; idx++) {
        ...
        for (ibus_qual_info = stage_fc->input_bus.qual_list[qid];
            ...
            LOG_VERBOSE(BSL_LS_BCM_FP, (BSL_META_U(unit,
                ...
                "\t\tSec_1=%d Sec_val_1=%d Sec_2=%d Sec_val_2=%d\n"
                "\t\tSec_3=%d Sec_val_3=%d Sec_4=%d Sec_val_4=%d\n"
                                   ...
                                   _field_group_t **next_group)
{
    ...
    *next_group = NULL;  
    for (; 
         fg != NULL && (instance == fg->instance) && (stage_id == fg->stage_id);
         fg = fg->next) {
...
_field_th_lt_default_entry_install(int unit, bcm_field_entry_t lt_entry)
{
    ...
    if (dirty) {
        ...
        rv = _field_th_lt_default_tcam_entry_install(unit, lt_entry);
    } else {
                                   ...
                                   _field_group_t *fg, _field_lt_slice_t *lt_fs)
{
    ...
    if (fg->flags & _FP_GROUP_PRESELECTOR_SUPPORT) {
       ...
       for (idx = 0; idx < _FP_PRESEL_ENTRIES_MAX_PER_GROUP; idx++) {
           ...
           BCM_IF_ERROR_RETURN
              (_bcm_field_th_lt_entry_config_set(unit, stage_fc, fg, lt_fs,
                                                 f_presel));

           /* Install the LT Entry in HW. */
           rv = _bcm_field_th_lt_entry_install(unit, f_presel);
           BCM_IF_ERROR_RETURN(rv);
       }
    } else {
    ...
    while (NULL != fs) {
        ...
        if (empty) {
            ...
            for (slice = 0; slice < count; slice++) {
                ...
                BCM_IF_ERROR_RETURN(_field_th_ingress_slice_clear(unit, fg, fs_ptr));
                                         ...
                                         _field_group_t *fg)
{
    ...
    static soc_mem_t lt_action_mem[_FP_MAX_NUM_PIPES+1] = {
 ...
 */
STATIC int
_field_th_ingress_logical_table_map_write(int unit,
                                          _field_stage_t *stage_fc,
                                          _field_group_t *fg)
{
    soc_field_t field;              /* HW entry field.                  */
    ...
    static soc_reg_t lt_config_reg[_FP_MAX_NUM_PIPES+1] = {
