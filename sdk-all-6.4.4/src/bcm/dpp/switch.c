...
_bcm_dpp_switch_detach(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(SWITCH_ACCESS.free(unit));
...
/************************************************************************/
    ...
    case bcmSwitchEcnDelayMeasurementThreshold:
     ...
     case bcmSwitchPortVlanTranslationAdvanced:
       /* such fields only in Jericho */
       if (SOC_IS_JERICHO(unit))  {
           SOC_IF_ERROR_RETURN(READ_IHP_VTT_GENERAL_CONFIGS_0r(unit, REG_PORT_ANY, &reg_val));
           soc_reg_field_set(unit, IHP_VTT_GENERAL_CONFIGS_0r, &reg_val, FORCE_VEC_FROM_LIFf, arg?0x1:0x0);
           SOC_IF_ERROR_RETURN(WRITE_IHP_VTT_GENERAL_CONFIGS_0r(unit, REG_PORT_ANY,reg_val));
       }else{
           unsupported_switch = TRUE;
       }
       break;
    ...
    case bcmSwitchEgressKeepSystemHeader:
        ...
        if ((bcm_type == bcmSwitchPortHeaderType) || (SOC_IS_ARAD_B0_AND_ABOVE(unit))) {
            if (bcm_type == bcmSwitchPortHeaderType) {
                BCMDNX_IF_ERR_EXIT(soc_port_sw_db_hdr_type_out_get(unit,port,&soc_tmcport_header_type));
    ...
    bcm_switch_lif_property_config_t *lif_config)
{
    ...
    if ( (lif_property == bcmLifPropertyEEDBBankPhase) || (lif_property == bcmLifPropertyEEDBBankExtension) )
    {
         ...
         if ( ((lif_property == bcmLifPropertyEEDBBankPhase) && !BCM_DPP_AM_LOCAL_OUT_LIF_BANK_IN_RANGE(eedb_bank_id) ) ||
              ((lif_property == bcmLifPropertyEEDBBankExtension) && !BCM_DPP_AM_LOCAL_OUT_LIF_REGULAR_BANK_IN_RANGE(eedb_bank_id) ))
         {
    ...
    rif_max_id = soc_property_get(unit, spn_RIF_ID_MAX, _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK) ;
    ...
    if (lif_property==bcmLifPropertyEEDBBankPhase) 
    {
    ...
    bcm_switch_control_info_t *info)
{
    ...
    {
    case bcmSwitchForwardingLayerMtuFilter:
        soc_sand_rv = arad_pp_vsi_egress_mtu_check_enable_get(unit, FALSE /*Forwarding Layer*/, key.index, &enable);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        info->value = enable;
        break;
    case bcmSwitchLinkLayerMtuFilter:
        soc_sand_rv = arad_pp_vsi_egress_mtu_check_enable_get(unit, TRUE /*Link Layer*/, key.index, &enable);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        info->value = enable;
        break;
    case bcmSwitchForwardingLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_get(unit, FALSE /*Forwarding Layer*/, key.index, (uint32*)&(info->value));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    case bcmSwitchLinkLayerMtuSize:
        soc_sand_rv = arad_pp_vsi_egress_mtu_get(unit, TRUE /*Link Layer*/, key.index, (uint32*)&(info->value));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        break;
    default:
        break;
