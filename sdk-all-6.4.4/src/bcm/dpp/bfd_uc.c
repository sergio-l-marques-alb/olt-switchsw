...
/* Itmh Header */
typedef struct bfd_itmh_header_t {
    uint8           ptch_byte0;     
    uint8           ptch_byte1;
    uint16          itmh_word0;
    uint16          itmh_word1;
 ...
 * @return int 
 */
STATIC int _bcm_petra_bfd_uc_m_hop_set_classifer(int unit, bcm_bfd_endpoint_info_t *endpoint_info, bfd_endpoint_config_t *endpoint_config) {
    int rv;
    uint32 soc_sand_rv;
    ...
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) {
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_INIT_TRAP_INFO_TRAP_IDS, &trap_code, SOC_PPD_BFD_TRAP_ID_UC_IPV6); 
        BCMDNX_IF_ERR_EXIT(rv);
    } else {
    ...
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) {
        ...
        classifier_mep_entry.flags = SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT | SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED;
    ...
    classifier_mep_entry.mep_type = SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV6_M_HOP;  /* As far as the classifier is concerned this may as welll be an IPv4 entry.*/
    ...
    BCMDNX_IF_ERR_EXIT(rv); 
    discr_msbs =  endpoint_info->local_discr >> SOC_PPD_BFD_RX_YOUR_DISCRIMINATOR_RANGE_BIT_START;
...
STATIC int _bcm_petra_bfd_uc_m_hop_remove_classifer_entry(int unit, bfd_endpoint_config_t *endpoint_config) {
    ...
    SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_clear(&classifier_mep_entry);
    classifier_mep_entry.flags = SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT | SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED;
    classifier_mep_entry.your_discriminator = endpoint_config->info.local_discr & SOC_PPD_BFD_DISCRIMINATOR_TO_LIF_START_MASK;
    classifier_mep_entry.lif = _BCM_OAM_INVALID_LIF;
    classifier_mep_entry.mep_type = SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV6_M_HOP;  /* As far as the classifier is concerned this may as welll be an IPv4 entry.*/
                             ...
                             uint8 r_subclass, uint16 *r_len)
{
    ...
    *r_len = bcm_ntohs(reply.s.len);
    ...
    rv = _bcm_petra_bfd_uc_convert_uc_error(reply.s.data);
...
STATIC uint8 *
_bcm_petra_bfd_uc_l2_header_pack(uint8 *buffer, bfd_l2_header_t *l2)
{
    uint32  tmp;
    int     i;

    for (i = 0; i < _BCM_BFD_MAC_ADDR_LENGTH; i++) {
...
{
    _SHR_PACK_U8(buffer, itmh->ptch_byte0);
    _SHR_PACK_U8(buffer, itmh->ptch_byte1);
    _SHR_PACK_U16(buffer, itmh->itmh_word0);
    _SHR_PACK_U16(buffer, itmh->itmh_word1);
...
STATIC int
_bcm_petra_bfd_uc_l2_header_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info,
                            ...
                            uint16 etype, bfd_l2_header_t *l2)
{
    ...
    l3_intf.l3a_intf_id = l3_egress.vlan;
    if (BCM_FAILURE(bcm_l3_intf_get(unit, &l3_intf))) {
        return BCM_E_PARAM;
    }    

    /* Destination and Source MAC */
    sal_memcpy(l2->dst_mac, l3_egress.mac_addr, _BCM_BFD_MAC_ADDR_LENGTH);
    sal_memcpy(l2->src_mac, l3_intf.l3a_mac_addr, _BCM_BFD_MAC_ADDR_LENGTH);
     ...
     */    
    if (BCM_FAILURE(bcm_port_tpid_get(unit, port, &tpid))) {
        return BCM_E_INTERNAL;
    }
    
    l2->outer_vlan_tag.tpid     = tpid;
    if (BCM_SUCCESS(bcm_port_untagged_vlan_get(unit, port, &vid))) {
        if ((vid == l3_intf.l3a_vid) ||
            (packet_flags & (_BCM_BFD_ENCAP_PKT_HG | _BCM_BFD_ENCAP_PKT_HG2))) {
    ...
    if (l3_intf.l3a_inner_vlan != 0) {
        if (BCM_FAILURE(bcm_port_inner_tpid_get(unit, port, &tpid))) {
            return BCM_E_INTERNAL;
        }
...
STATIC int
_bcm_petra_bfd_uc_itmh_header_get(int unit, int tx_port, bfd_itmh_header_t *itmh)
{
    itmh->ptch_byte0 = 0x50;
    itmh->ptch_byte1 = 0;
    itmh->itmh_word0 = 0x000C;
    itmh->itmh_word1 = (tx_port & 0x00ff);    
                               ...
                               uint8 *buffer)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_petra_bfd_uc_l2_header_get(unit, endpoint_info, packet_flags,
                                     tx_port, etype, &l2));

    
    /* Always build L2 Header */
    BCM_IF_ERROR_RETURN
        (_bcm_petra_bfd_uc_itmh_header_get(unit, tx_port, &itmh));
    ...
    cur_ptr = _bcm_petra_bfd_uc_l2_header_pack(cur_ptr, &l2);
                                        ...
                                        bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    /* Set BFD Session in HW for single-hop*/
    if (!update || (endpoint_info->flags & BCM_BFD_ENDPOINT_ENCAP_SET)) {
        if ((endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) != BCM_BFD_ENDPOINT_MULTIHOP) {
            rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_bfd_one_hop_lem_entry_add, (unit, 1, endpoint_info->local_discr, endpoint_info->id, is_accelerate)); 
    		BCMDNX_IF_ERR_EXIT(rv);
        }
        encap = 1;
    }
    ...
    if (encap) {
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_encap_create(unit, endpoint_config,
                                        msg_sess.encap_data));
    }
    ...
    /** RX */
    if (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP && !update) {
        /* Use the OAM classifier*/
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_m_hop_set_classifer(unit, endpoint_info,endpoint_config));
    }
...
bcm_petra_bfd_uc_endpoint_destroy(int unit, bcm_bfd_endpoint_t endpoint)
{
    ...
    if ((endpoint_config->info.flags & BCM_BFD_ENDPOINT_MULTIHOP) != BCM_BFD_ENDPOINT_MULTIHOP) {
        rv= MBCM_PP_DRIVER_CALL(unit, mbcm_pp_bfd_one_hop_lem_entry_remove, (unit, endpoint_config->info.local_discr)); 
