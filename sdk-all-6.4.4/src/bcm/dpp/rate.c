...
{
    SOC_PPD_MTR_BW_PROFILE_INFO policer;
    int port_i;
    SOC_PPD_MTR_ETH_TYPE  types[5];
    int nof_types = 0;
    int type_indx = 0;
    int soc_sand_rv = 0;
    int rv = BCM_E_NONE;
    ...
    SOC_PPD_MTR_BW_PROFILE_INFO_clear(&policer);
    ...
    if (kbits_sec != 0 && kbits_burst != 0) {

        dpp_meter = &(SOC_DPP_CONFIG(unit))->meter;
        if (kbits_sec < dpp_meter->policer_min_rate) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (DPP_RATE_MSG("rate is lower than the device min-rate. rate can be either 0 or above the min-rate \n")));
        }
    ...
    if(flags & BCM_RATE_DLF) {
        types[nof_types++] = SOC_PPD_MTR_ETH_TYPE_UNKNOW_UC;
    }
    if(flags & BCM_RATE_UNKNOWN_MCAST) {
        types[nof_types++] = SOC_PPD_MTR_ETH_TYPE_UNKNOW_MC;
    }
    if(flags & BCM_RATE_MCAST) {
        types[nof_types++] = SOC_PPD_MTR_ETH_TYPE_KNOW_MC;
    }
    if(flags & BCM_RATE_BCAST) {
        types[nof_types++] = SOC_PPD_MTR_ETH_TYPE_BC;
    }
    if(flags & BCM_RATE_UCAST) {
        types[nof_types++] = SOC_PPD_MTR_ETH_TYPE_KNOW_UC;
    }
                           ...
                           uint32 *kbits_sec, uint32 *kbits_burst)
{
    ...
    /* check flags */
    if (flags & (BCM_RATE_SALF|BCM_RATE_RSVD_MCAST|BCM_RATE_CTRL_BUCKET_1|BCM_RATE_CTRL_BUCKET_2)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (DPP_RATE_MSG("not supported flags \n")));
    }
