...
_bcm_petra_vxlan_resources_get(
    ...
    *fec_index = -1;
    ...
    if(ingress) {
        ...
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit,vxlan_port->match_tunnel_id,global_in_lif, local_in_lif,NULL,NULL,&is_local);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (vxlan_port->egress_tunnel_id != 0) {
        /* get match tunnel (terminator) */
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit,vxlan_port->egress_tunnel_id,global_out_lif,NULL,local_out_lif,NULL,&is_local);
        BCMDNX_IF_ERR_EXIT(rv);
    }
