...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (endpoint_info->gport != BCM_GPORT_INVALID) {
		int global_lif, local_out_lif,local_in_lif; 
		/* In Jericho the OAM-LIF in the ingress is the global LIF and at the egress it is the local LIF.*/
		ret = _bcm_dpp_gport_to_global_and_local_lif(unit,endpoint_info->gport,&global_lif, &local_in_lif, &local_out_lif,NULL,   &is_local);
        BCMDNX_IF_ERR_EXIT(ret);

        if (endpoint_info->mpls_out_gport != BCM_GPORT_INVALID) {
			classifier_mep_entry->lif = global_lif;
			ret = _bcm_dpp_gport_to_global_and_local_lif(unit, endpoint_info->mpls_out_gport, &global_lif, NULL, &local_out_lif, NULL,   &is_local);
			BCMDNX_IF_ERR_EXIT(ret);
			classifier_mep_entry->passive_side_lif = local_out_lif; /* In Arad this will equal the global  out LIF anyways.*/
...
int _bcm_dpp_endpoint_counters_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_LIF_ID lif) {
    ...
    if (endpoint_info->lm_counter_base_id > 0) {
        ...
        for (crps_index = 0; crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors); crps_index++) {           
            ...
            if ( SOC_IS_JERICHO(unit)?
                 (we_val ==  endpoint_info->lm_counter_base_id  / (SOC_DPP_DEFS_GET(unit,counters_per_counter_processor) * ((format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS)  +1))):
                 
                 (src_type == ing_cnt_type) || (src_type == egr_cnt_type)) /* In Arad if the source type is egress/ingress OAM..*/ {
...
int _bcm_dpp_oam_fifo_interrupt_handler(int unit) {
    ...
    soc_sand_rv = soc_ppd_oam_event_fifo_read(unit, &rmeb_db_ndx_internal, event_id_bitmap, &valid, event_found.event_data);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
STATIC int _bcm_oam_default_endpoint_validity_checks(int unit, const bcm_oam_endpoint_info_t *endpoint_info, ARAD_PP_OAM_DEFAULT_EP_ID * default_id) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Default MEP currently unavailable for Jericho\n"))); 
    }
