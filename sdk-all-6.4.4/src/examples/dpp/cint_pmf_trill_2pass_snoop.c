/* $Id: cint_pmf_trill_2pass_snoop.c,v 1.6 Broadcom SDK $
...
int trill_2pass_field_group_set( int unit, bcm_gport_t snoop_trap_gport_id, bcm_field_group_t *group) 
{
  int result;
  int auxRes;
  ...
  for (class_port = 0; class_port <= 1; ++class_port) {
      ...
      if (class_port == 0) {
          result = bcm_field_action_add(unit, snoop_ent, bcmFieldActionSnoop, snoop_trap_gport_id, 0);
          if (BCM_E_NONE != result) {
...
/* 
 * Set the In-LIF to be equal to the EEP (1st LEM lookup result): 
                                ...
                                /* in */ int trap_id) 
{
  ...
  uint16 dq_offset[2] = {19 /* LIF located in LEM-Lookup-Result[34:19] */, 0};
  int dq_length[2] = {16 /* 2 Bytes */, 1};
  ...
  result = bcm_field_qualify_ForwardingType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeTrill);
  if (BCM_E_NONE != result) {
      printf("Error in bcm_field_qualify_ForwardingType\n");
      return result;
  }
  ...
  result = bcm_field_qualify_L2SrcHit(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 0x0, 0x1);
  if (BCM_E_NONE != result) {
      printf("Error in bcm_field_qualify_L2SrcHit\n");
      return result;
  }
