 ...
 *          2.   Calls multicast_vlan_port_add() to update the multicast group with the new gport. 
 * 4.  Delete VSI
...
int open_ingress_mc_group_update_id(int unit, int* mc_group_id){
    int rv = BCM_E_NONE;
    int flags;
    bcm_multicast_t mc_group = *mc_group_id;
...
int vswitch_create(int unit, bcm_vlan_t  *new_vsi, int with_id){
     ...
     */
    rv = open_ingress_mc_group(unit, vsi1);
    if (rv != BCM_E_NONE) {
