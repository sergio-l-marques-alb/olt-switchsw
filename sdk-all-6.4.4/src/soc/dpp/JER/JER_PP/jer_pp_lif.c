...
soc_jer_lif_init(int unit){
    ...
    nof_direct_banks = SOC_DPP_JER_CONFIG(unit)->pp.rif_id_max / JER_PP_EG_ENCAP_NOF_ENTRIES_PER_HALF_BANK;
...
soc_jer_lif_glem_access_entry_by_key_get(int unit, int global_lif_id, int *egress_lif_id, uint8 *accessed, uint8 *is_found){
    ...
    *fld_val = 0x1;
     ...
     */
    if (SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit) && SOC_INFO(unit).ha_hw_log_access.polling) {
        rv = (SOC_INFO(unit).ha_hw_log_access.polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, EDB_GLEM_DIAGNOSTICSr, EDB_BLOCK(unit), 0, GLEM_DIAGNOSTICS_LOOKUPf, 0));
        SOCDNX_SAND_IF_ERR_EXIT(rv);
    } else {
        /* No HW Log - Do Polling */
...
soc_jer_lif_glem_access_write_to_machine(int unit, uint32 *buffer){
    ...
    if ((SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit) && SOC_INFO(unit).ha_hw_log_access.polling)) {
        rv = SOC_INFO(unit).ha_hw_log_access.polling( unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, EDB_GLEM_INTERRUPT_REGISTER_ONEr, REG_PORT_ANY, 0, GLEM_MANAGEMENT_COMPLETEDf, 1);
        SOCDNX_SAND_IF_ERR_EXIT(rv);
    } else {
