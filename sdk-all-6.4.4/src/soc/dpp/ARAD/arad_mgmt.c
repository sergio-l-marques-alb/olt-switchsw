...
#define ARAD_MGMT_IQM_OCBPRM_MANTISSA_NOF_BITS      (7)
  ...
  arad_mgmt_module_init(void)
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_MGMT_MODULE_INIT);
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, MESH_TOPOLOGY_MESH_STATUSESr, REG_PORT_ANY, 0, MESH_STATUS_2f, &buffer_1));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, MESH_TOPOLOGY_THRESHOLDSr, REG_PORT_ANY, 0, THRESHOLD_0f, &buffer_2));
  ...
  )
{
  ...
  conf[conf_idx].val = (enable == TRUE ? 0x1 : 0x1);
  conf[conf_idx].instance_id = REG_PORT_ANY;
  conf[conf_idx].polling_func = NULL;
  conf[conf_idx].failure_func = NULL;
...
#define ARAD_MAX_BUFFERS_PER_PACKET 32
    uint32 mru = SOC_DPP_CONFIG(unit)->arad->init.dram.dbuff_size * ARAD_MAX_BUFFERS_PER_PACKET;
    uint32 entry = 0;
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    ...
    soc_IDR_CONTEXT_MRUm_field32_set(unit, &entry, ORG_SIZEf, mru + 1);
    soc_IDR_CONTEXT_MRUm_field32_set(unit, &entry, SIZEf, mru - 1);
    ...
    )
{
    ...
    res = soc_sand_break_to_mnt_exp_round_up(value,
            ARAD_MGMT_IQM_OCBPRM_MANTISSA_NOF_BITS,
...
/* This function takes the field value of OCB_QUE_BUFF_SIZE_EN_TH_[0/1]f and OCB_QUE_SIZE_EN_TH_[0/1]f,
  ...
  )
{
  ...
  exact_info->th_buffers[0] = arad_mgmt_ocb_prm_field_value_to_exact_value(unit, field_val);
  ...
  exact_info->th_buffers[1] = arad_mgmt_ocb_prm_field_value_to_exact_value(unit, field_val);
  ...
  )
{
  ...
  info->th_buffers[0] = arad_mgmt_ocb_prm_field_value_to_exact_value(unit, field_val);;

  field_val = soc_mem_field32_get(unit, IQM_OCBPRMm, &reg_val, OCB_QUE_BUFF_SIZE_EN_TH_1f);
  info->th_buffers[1] = arad_mgmt_ocb_prm_field_value_to_exact_value(unit, field_val);;
    ...
    SOC_SAND_IN  int       unit)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 90, exit, READ_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, &reg_val));
    field = 0x0;
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit, WRITE_IPS_IPS_GENERAL_CONFIGURATIONSr(unit, REG_PORT_ANY, reg_val));
    ...
    SOC_SAND_IN  int       unit)
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 8, exit, READ_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, &reg_val));
    field = 0x1;
    soc_reg_field_set(unit, MMU_GENERAL_CONFIGURATION_REGISTERr, &reg_val, WAIT_FINISH_CUR_STATE_BEFORE_STOP_FOR_REFRESHf, field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 9, exit, WRITE_MMU_GENERAL_CONFIGURATION_REGISTERr(unit, reg_val));
    ...
    for (index=0 ; index < ARAD_CNT_NOF_PROCESSOR_IDS(unit) ; index++) {
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  65,  exit, ARAD_REG_ACCESS_ERR,READ_CRPS_CRPS_GENERAL_CFGr(unit,  index, &reg_val));
        field = 0x1;
        /* QPTS - "active counters" counter isn't accurate when using "Qsize"    */ 
        soc_reg_field_set(unit, CRPS_CRPS_GENERAL_CFGr, &reg_val, CRPS_N_ACT_CNT_VALIDATE_ENf, field);
        /* QPTS - Ovth memory bypass bug    */
        soc_reg_field_set(unit, CRPS_CRPS_GENERAL_CFGr, &reg_val, CRPS_N_OVTH_MEM_RFRSH_BPASS_ENf, field);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,WRITE_CRPS_CRPS_GENERAL_CFGr(unit,  index,  reg_val));
    }    
...
/*
 * Note: This procedure is based on soc_fe1600_drv_temperature_monitor_get(). Apparently,
 * they both use the same silicon piece.
 */
int 
   arad_mgmt_temp_pvt_get(int unit, int temperature_max, soc_switch_temperature_monitor_t *temperature_array, int *temperature_count)
{
    int i, rv;
    uint32 reg32_val;
    int curr;
