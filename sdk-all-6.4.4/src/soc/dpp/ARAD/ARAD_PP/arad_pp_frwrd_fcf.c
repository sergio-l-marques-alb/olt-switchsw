...
/* result index for lookup in SEM, not really matter,
   as lookup result is found/not found
 */
#define ARAD_PP_FRWRD_FC_SEM_RESULT_INDX (0xae2)



/* building key with VFI, reserved bit from the VFT header is taken as well
   this bit considered as zer
 */
#define ARAD_PP_FRWRD_FC_VFT_VAL_TO_KEY_FIX(_vfi)  ((_vfi)<<1); /* lsb is reserved*/

#define ARAD_PP_FRWRD_FC_VFT_VAL_FROM_KEY_FIX(_vfi)  ((_vfi)>>1); /* lsb is reserved*/
  ...
  Arad_pp_procedure_desc_element_frwrd_fcf[] =
{
   ...
   */
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_FCF_GLBL_INFO_GET_UNSAFE),
  ...
  Arad_pp_error_desc_element_frwrd_fcf[] =
{
  ...
  {
    ARAD_PP_FRWRD_FCF_ROUTE_STATUS_OUT_OF_RANGE_ERR,
    "ARAD_PP_FRWRD_FCF_ROUTE_STATUS_OUT_OF_RANGE_ERR",
    "The parameter 'route_status' is out of range. \n\r "
    "The range is: 0 - ARAD_PP_NOF_FRWRD_FCF_ROUTE_STATUSS-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
...
/* given FCF key, build IPv4 key, so later IPv4 functions will be used */
/* static */ uint32
  arad_pp_fcf_route_key_to_ip_key(
    SOC_SAND_IN  int                         unit,
    SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ROUTE_KEY    *fcf_route_key,
  ...
  )
{
    ...
    if (ARAD_LPM_CUSTOM_LOOKUP_ENABLED(unit)) { /* custom feature LPM lookup */
            *ip_vrf_id = fcf_route_key->vfi; /* should be always 0 */
            ip_route_key->ip_address = fcf_route_key->d_id; /* {db_id(1), 12'b0, InLIF(16), TC(3)} */
    ...
    *ip_vrf_id = (ARAD_PP_FRWRD_IP_FC_VRF);    
    ...
    /* prefix is 12 + 1 + route-key prefix in range [0:8]*/
...
/* static */ uint32
  arad_pp_fcf_ip_key_to_fc_key(
    SOC_SAND_IN  int                         unit,
    SOC_SAND_OUT  ARAD_PP_FRWRD_FCF_ROUTE_KEY   *fcf_route_key,
...
/* static */ uint32
  arad_pp_fcf_key_sem_op(
    SOC_SAND_IN  int                         unit,
    SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ROUTE_KEY    *fcf_route_key,
    SOC_SAND_IN  uint8                          db_op, /* add,remove,get*/
  ...
  )
{
    ...
    else if (db_op == ARAD_PP_FRWRD_FC_OP_REMOVE) { /* remove */
  ...
  )
{
  ...
  /* check prefix, can be either 24 (LOCAL) or 8 (REMOTE)*/
  ...
  if(fcf_route_key->prefix_len == ARAD_PP_FRWRD_FCF_D_ID_BITS){
      ...
      lem_key->param[1].value[0] = ARAD_PP_FRWRD_FC_VFT_VAL_TO_KEY_FIX(fcf_route_key->vfi); /* lsb is reserved*/
...
/* parse lem key into FCF route key */
/* static*/
  void
    arad_pp_fcf_route_key_from_lem_key(
      SOC_SAND_IN ARAD_PP_LEM_ACCESS_KEY *key,
      SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ROUTE_KEY *route_key
    )
{
...
/* static */ uint32
  arad_pp_fcf_frwrd_decision_to_lem_payload(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_FRWRD_DECISION_INFO    *frwrd_info,
  ...
  )
{
  ...
  if(frwrd_info->additional_info.outlif.type != ARAD_PP_OUTLIF_ENCODE_TYPE_NONE) {
      payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
  }
  else if(frwrd_info->additional_info.eei.type != ARAD_PP_EEI_TYPE_EMPTY) {
...
/* static */ uint32
  arad_pp_fcf_frwrd_decision_from_lem_payload(
     ...
     SOC_SAND_OUT ARAD_PP_FRWRD_DECISION_INFO    *frwrd_info
  )
{
  uint32
  ...
  ARAD_PP_FRWRD_DECISION_INFO_clear(frwrd_info);
...
/* static */ uint32
  arad_pp_fcf_route_info_to_lem_payload(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ROUTE_INFO   *fcf_route_info,
     SOC_SAND_OUT ARAD_PP_LEM_ACCESS_PAYLOAD     *payload
  )
{
  uint32 
...
/* LEM payload parse + accessed */
/* static */ uint32
  arad_pp_fcf_route_info_from_lem_payload(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_LEM_ACCESS_PAYLOAD     *payload,
     SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ROUTE_INFO   *fcf_route_info,
  ...
  )
{
  ...
  ARAD_PP_FRWRD_FCF_ROUTE_INFO_clear(fcf_route_info);
...
/* static */ uint32
  arad_pp_fcf_zoning_key_to_lem_key(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ZONING_KEY   *fcf_zone_key,
     SOC_SAND_OUT ARAD_PP_LEM_ACCESS_KEY         *lem_key
  )
{
...
/* parse lem key into FCF zoning key */
/* static*/
  void
    arad_pp_fcf_zone_key_from_lem_key(
      SOC_SAND_IN ARAD_PP_LEM_ACCESS_KEY *key,
      SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ZONING_KEY *zone_key
    )
{
...
/* static */ uint32
  arad_pp_fcf_zone_info_to_lem_payload(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ZONING_INFO  *fcf_zone_info,
     SOC_SAND_OUT ARAD_PP_LEM_ACCESS_PAYLOAD     *payload
  )
{
  uint32 
...
/* static */ uint32
  arad_pp_fcf_zone_info_from_lem_payload(
     SOC_SAND_IN  int                         unit,
     SOC_SAND_IN  ARAD_PP_LEM_ACCESS_PAYLOAD     *payload,
     SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ZONING_INFO  *fcf_zone_info,
     SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ROUTE_STATUS *route_status
  )
{
  uint32 
  ...
  ARAD_PP_FRWRD_FCF_ZONING_INFO_clear(fcf_zone_info);
...
/* add/get/remove zoning/routing key to LEM*/
/* static */ uint32
  arad_pp_fcf_key_lem_op(
    ...
    SOC_SAND_INOUT  ARAD_PP_FRWRD_FCF_ROUTE_INFO   *fcf_route_info,
    SOC_SAND_INOUT  ARAD_PP_FRWRD_FCF_ZONING_INFO  *fcf_zone_info,
    SOC_SAND_OUT uint8                          *found,
    SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE       *success,
    SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ROUTE_STATUS *route_status
  )
{
    ...
    /* do the operation, add, remove or get  */
    if (db_op == ARAD_PP_FRWRD_FC_OP_ADD) { /* add */
...
/* static */
  uint32
    arad_pp_frwrd_fcf_lpm_route_get(
      ...
      SOC_SAND_OUT  ARAD_PP_FRWRD_FCF_ROUTE_INFO              *fcf_route_info,
      SOC_SAND_OUT ARAD_PP_FRWRD_FCF_ROUTE_STATUS            *pending_type,
      SOC_SAND_OUT uint8                                     *found
    )
{
 ARAD_PP_IPV4_LPM_MNGR_INFO
...
/* 
 * internal function to check if this VFI is indenpendent 
 * i.e. can be commited/cached/cleared without affecting other VFIs 
 * this derived if the VFI use it's own patricia tree 
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_FCF_GLBL_INFO_SET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(glbl_info);

  if (glbl_info->enable) {
      /* set Custom ether type */
  ...
  )
{
    ...
    /* ARAD_PP_FRWRD_FCF_ROUTE_LOCAL_DOMAIN --> add to sem :: prefix: 8 or 24 */
    if (route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_LOCAL_DOMAIN) {
        res = arad_pp_fcf_key_sem_op(unit,route_key,ARAD_PP_FRWRD_FC_OP_ADD,success,0);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    }
    /* SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN --> add to LPM, ::prefix has to be 0-8 */
    if (route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_DOMAIN || route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN_NPORT) {
        res = arad_pp_frwrd_fcf_lpm_route_add(unit,route_key,route_info,success);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }
    /* ARAD_PP_FRWRD_FCF_ROUTE_HOST --> add to LEM, ::prefix has to 8 or 24  */
    if (route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_HOST || route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_HOST_NPORT) {
        res = arad_pp_fcf_key_lem_op(unit,route_key,0,ARAD_PP_FRWRD_FC_OP_ADD,(ARAD_PP_FRWRD_FCF_ROUTE_INFO*)route_info,0,0,success,0);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    }
exit:
  ...
  )
{
  ...
  if (route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_LOCAL_DOMAIN) {
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      *route_status = ARAD_PP_FRWRD_FCF_ROUTE_STATUS_COMMITED;
  }
  /* SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN --> add to LPM, ::prefix has to be 0-8 */
  if (route_key->flags & ARAD_PP_FRWRD_FCF_ROUTE_DOMAIN || route_key->flags & SOC_PPC_FRWRD_FCF_ROUTE_DOMAIN_NPORT) {
  ...
  )
{
    uint32
      res = SOC_SAND_OK;
    ARAD_PP_FRWRD_FCF_ROUTE_KEY       fcf_route_key;
    ...
    ARAD_PP_FRWRD_FCF_ROUTE_KEY_clear(&fcf_route_key);
  ...
  )
{
  ...
  res = arad_pp_fcf_key_lem_op(unit,0,zoning_key,ARAD_PP_FRWRD_FC_OP_ADD,0,(ARAD_PP_FRWRD_FCF_ZONING_INFO*)routing_info,0,success,0);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  ...
  )
{
  ...
  ARAD_PP_FRWRD_FCF_ZONING_INFO_clear(routing_info);
  ...
  )
{
    ...
    uint32                            nof_entries;
    ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION        action;
    ARAD_PP_FRWRD_FCF_ZONING_KEY key_mask;
    ...
    ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_clear(&action);
    soc_sand_SAND_TABLE_BLOCK_RANGE_clear(&block_range);
    ARAD_PP_FRWRD_FCF_ZONING_KEY_clear(&key_mask);


    /* set mask to max value so all bits are consider */
    if (flags & ARAD_PP_FRWRD_FCF_ZONE_REMOVE_BY_VFT) {
        key_mask.vfi = ARAD_PP_FRWRD_FCF_VFT_MAX;
    }
    if (flags & ARAD_PP_FRWRD_FCF_ZONE_REMOVE_BY_D_ID) {
        key_mask.d_id = ARAD_PP_FRWRD_FCF_D_ID_MAX;
    }
    if (flags & ARAD_PP_FRWRD_FCF_ZONE_REMOVE_BY_S_ID) {
        key_mask.s_id = ARAD_PP_FRWRD_FCF_S_ID_MAX;
    }

    /* map key to lem format */
  ...
  )
{
    ...
    if (enable) {
        ...
        /* create new programs for N_Port */
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_npv_programs_init(unit));

    } else {

        /* disable existing programs for N_Port */
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_n_port_programs_disable(unit));        
    }

    /* enable/disable zoning for existing fcf programs */
  ...
  )
{
    ...
    /* change mode for existing fcf programs in the FLP */
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_fcoe_vsan_mode_set(unit, is_vsan_from_vsi));

    /* change mode for existing fcf programs in the VTT */
    SOCDNX_IF_ERR_EXIT(arad_pp_vtt_fcf_vsan_mode_update(unit, is_vsan_from_vsi));
...
/*********************************************************************
*     Clear fcf routing tables for all VFIs.
 *     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
...
uint32
  ARAD_PP_FRWRD_FCF_GLBL_INFO_verify(
...
uint32
  ARAD_PP_FRWRD_FCF_ROUTE_KEY_verify(
...
uint32
  ARAD_PP_FRWRD_FCF_ROUTE_INFO_verify(
...
uint32
  ARAD_PP_FRWRD_FCF_ZONING_KEY_verify(
...
uint32
  ARAD_PP_FRWRD_FCF_ZONING_INFO_verify(
    SOC_SAND_IN  ARAD_PP_FRWRD_FCF_ZONING_INFO *info
  )
{
	return 0;
