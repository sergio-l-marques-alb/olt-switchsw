  ...
  )
{
  ...
  else
  {
       ...
       * eei encoding = 24 bits = {EEI identifier[23:20], HI[19:16], out_lif[15:0]}
  ...
  )
{
  ...
  else if(fwd_decision->additional_info.eei.type == ARAD_PP_EEI_TYPE_OUTLIF) /* outlif */
  {
    ...
    eei_type = ARAD_PP_EEI_IDENTIFIER_ENCAP_VAL;  
     ...
     && fwd_decision->additional_info.eei.type != ARAD_PP_NOF_EEI_TYPES && fwd_decision->additional_info.eei.type != ARAD_PP_EEI_TYPE_RAW)
  {
    ...
    res = soc_sand_bitstream_set_any_field(
            &(eei_type),
            ARAD_PP_ASD_BIT_POSITION_IDENTIFIER,
            ARAD_PP_EEI_IDENTIFIER_NOF_BITDS,
            &(asd_buffer_lcl)
          );
...
/* mask for ac mp lif learn data, with FEC learn data: nof bit in ac mp lif learn data - MSB.
   REMINDER: MSB indicate type of learning: FEC or UC FLOW */
#define ARAD_PP_AC_MP_LIF_FEC_LEARN_BUFFER_MASK(unit)  ((1 << (SOC_DPP_DEFS_GET(unit, ac_mp_lif_learn_data_nof_bits) -1)) -1)
/* mask for ip tunnel lif learn data, with FEC learn data: nof bits in ip tunnel lif learn data - MSB */
#define ARAD_PP_IP_TUNNEL_LIF_FEC_LEARN_BUFFER_MASK(unit)  ((1 << (SOC_DPP_DEFS_GET(unit, ip_tunnel_lif_learn_data_nof_bits) -1)) -1)
/* mask for ac mp lif learn data, with UC flow learn data: nof bit in ac mp lif learn data - MSB - static lsbs */
#define ARAD_PP_AC_MP_LIF_UC_FLOW_LEARN_BUFFER_MASK(unit)  ((1 << (SOC_DPP_DEFS_GET(unit, ac_mp_lif_learn_data_nof_bits) -1 - SOC_DPP_DEFS_GET(unit, ac_mp_lif_uc_flow_learn_data_static_lsbs))) -1)
/* mask for ip tunnel lif learn data, with flow learn data: nof bit in ip tunnel lif learn data - MSB - static lsbs */
#define ARAD_PP_IP_TUNNEL_LIF_UC_FLOW_LEARN_BUFFER_MASK(unit)  ((1 << (SOC_DPP_DEFS_GET(unit, ip_tunnel_lif_learn_data_nof_bits) -1 - SOC_DPP_DEFS_GET(unit, ip_tunnel_lif_uc_flow_learn_data_static_lsbs))) -1)
        ...
        )
{
    ...
    /* set dest id */
        ...
        /* for ac mp lif */
        if (lif_entry_type == ARAD_PP_LIF_ENTRY_TYPE_AC) {
            /* MSB is set, FEC learn data for ac mp lif */
            if (learn_buffer >> ARAD_PP_AC_MP_LIF_LEARN_BUFFER_MSB(unit) == 1) {
                fwd_decision->dest_id = (learn_buffer & ARAD_PP_AC_MP_LIF_FEC_LEARN_BUFFER_MASK(unit));
                fwd_decision->type = ARAD_PP_FRWRD_DECISION_TYPE_FEC;
            }
            /* MSB is not set, UC flow learn data for ac mp lif */
            else {
                fwd_decision->dest_id = ((learn_buffer & ARAD_PP_AC_MP_LIF_UC_FLOW_LEARN_BUFFER_MASK(unit)) << SOC_DPP_DEFS_GET(unit, ac_mp_lif_uc_flow_learn_data_static_lsbs));
        ...
        /* for ip tunnel lif */
            ...
            if (learn_buffer >> ARAD_PP_IP_TUNNEL_LIF_LEARN_BUFFER_MSB(unit) == 1) {
                fwd_decision->dest_id = (learn_buffer & ARAD_PP_IP_TUNNEL_LIF_FEC_LEARN_BUFFER_MASK(unit));
                fwd_decision->type = ARAD_PP_FRWRD_DECISION_TYPE_FEC;
            } 
            /* MSB is not set, UC flow learn data for ip tunnel lif */
            else {
                fwd_decision->dest_id = ((learn_buffer & ARAD_PP_IP_TUNNEL_LIF_UC_FLOW_LEARN_BUFFER_MASK(unit)) << SOC_DPP_DEFS_GET(unit, ip_tunnel_lif_uc_flow_learn_data_static_lsbs));
                fwd_decision->type = ARAD_PP_FRWRD_DECISION_TYPE_UC_FLOW;
            }
        }
    }
