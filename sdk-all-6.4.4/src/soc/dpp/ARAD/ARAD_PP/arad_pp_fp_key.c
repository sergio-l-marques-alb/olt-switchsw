       ...
       )
{
    ...
    const ARAD_SW_DB_PMF_CE ce_array_kbp_rif_match[] = {
                {1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_IRPP_IN_RIF}
               };
	
	const ARAD_SW_DB_PMF_CE ce_array_kbp_ipv4_mc_match[] = {
		{1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_IRPP_VRF},
		{1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_IRPP_IN_RIF},
		{1,   0, 0, 31, 0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_DIP},
		{1,   0, 0, 31,  0, 0, 0, SOC_PPC_FP_QUAL_HDR_IPV4_SIP}
    ...
    switch (table_id) {
        ...
        case ARAD_KBP_FRWRD_TBL_ID_INRIF_MAPPING:
            (*nof_ces) = sizeof(ce_array_kbp_rif_match)/sizeof(ARAD_SW_DB_PMF_CE);
            sal_memcpy(ce_array,&ce_array_kbp_rif_match,sizeof(ARAD_SW_DB_PMF_CE)* (*nof_ces) );
            break;

        case ARAD_KBP_FRWRD_TBL_ID_IPV4_MC:
            (*nof_ces) = sizeof(ce_array_kbp_ipv4_mc_match)/sizeof(ARAD_SW_DB_PMF_CE);
            sal_memcpy(ce_array,&ce_array_kbp_ipv4_mc_match,sizeof(ARAD_SW_DB_PMF_CE)* (*nof_ces) );
            break;
    default:
        return -1;
    }
       ...
       )
{
    ...
    /* Get the database Info */
     ...
     if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_DBAL) {
        ...
        /* get relevant programs for the DB */
        res = arad_sw_db_pmf_db_prog_get(unit, stage, db_id, exist_progs);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
        ...
        res = arad_sw_db_pgm_ce_array_get(unit, stage, db_prog, db_prog_info.cycle, ce_array);
...
{    
    ...
    ARAD_SW_DB_PMF_CE ce_array[ARAD_PMF_LOW_LEVEL_NOF_CE_IN_PROG_MAX_ALL_LEVELS] = {{0}};
