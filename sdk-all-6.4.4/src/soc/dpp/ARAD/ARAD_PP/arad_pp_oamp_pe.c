...
STATIC void _arad_pp_oamp_pe_programs_usage_init(SOC_SAND_IN  int  unit) {
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
		...
		if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop) {
			programs[unit][ARAD_PP_OAMP_PE_PROGS_BFD_IPV4_SINGLE_HOP].instruction_func = _arad_pp_oamp_pe_bfd_ipv4_single_hop;
...
17    nop
18   #a: copy 20 bytes
19    #b: if tmp_reg2 == 0, jump to tmp_reg3 / *  tmp_reg2 holds the encapsulation #c * /
20    nop
21    nop
22    copy 4 bytes / *  VLAN TAG1 * /
23    if (tmp_reg2 - 2) == 1, jump to tmp_reg3 / *  encapsulation == 1, jump to #c * /
24    nop
25    nop
26    copy 4 bytes / * VLAN TAG2 * /
27    #c:put 64 to tmp_reg1
28    nop
29    nop
30    tmp_reg1 <= tmp_reg - 4, copy in_fifo to buffer, loop till alu_out != 0
31    copy 2 bytes (CCM PDU 73, 74)
32    TmpReg1 <= LENGTH - BYTES_RD / *  can be 1,5,9 * /
33    nop
34    nop
35    if (TmpReg1 - 1) == 0, jump to tmp_reg3 / *  NO TLV * /
36    put #d to tmp_reg3
37    nop
38    nop
39    copy two lower bytes
40    copy two higher bytes
41    if TmpReg1- 5 == 0, jump to tmp_reg3
42    nop
43    nop
44    copy two lower bytes
45    copy two higher bytes
46    #d:put 8'h0 to buffer, set EOP
47   copy rest of packet
...
/*  #3	put #c to tmp_reg3 */
    ...
    SET_OAMP_PE_CMP2_ACT_BITS(reg_field, ARAD_PP_OAMP_PE_CMP_ACT_NEVER);
    SET_OAMP_PE_INST_SRC_BITS(reg_field, ARAD_PP_OAMP_PE_INST_SRC_NXT_JMP);
    SET_OAMP_PE_INST_CONST_BITS(reg_field, fst_inst_addr + 27); /*  should be #c */
    soc_OAMP_PE_PROGRAMm_field_set(unit, reg_data, PROG_DATAf, reg_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 13, exit,
                                 WRITE_OAMP_PE_PROGRAMm(unit, MEM_BLOCK_ALL, last_program_pointer[unit]++, reg_data));
...
/*  #10 copy bytes 15,16, rd in_fifo */
    ...
    SET_OAMP_PE_CMP2_ACT_BITS(reg_field, ARAD_PP_OAMP_PE_CMP_ACT_NEVER);
    SET_OAMP_PE_INST_SRC_BITS(reg_field, ARAD_PP_OAMP_PE_INST_SRC_NXT_JMP);
    SET_OAMP_PE_INST_CONST_BITS(reg_field, (fst_inst_addr + 46));
    soc_OAMP_PE_PROGRAMm_field_set(unit, reg_data, PROG_DATAf, reg_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 13, exit,
                                 WRITE_OAMP_PE_PROGRAMm(unit, MEM_BLOCK_ALL, last_program_pointer[unit]++, reg_data));
    ...
    SET_OAMP_PE_CMP2_ACT_BITS(reg_field, ARAD_PP_OAMP_PE_CMP_ACT_ALWAYS);
    SET_OAMP_PE_INST_SRC_BITS(reg_field, ARAD_PP_OAMP_PE_INST_SRC_NXT_JMP);
    SET_OAMP_PE_INST_CONST_BITS(reg_field, fst_inst_addr + 19);  /*  Should be #b */
    soc_OAMP_PE_PROGRAMm_field_set(unit, reg_data, PROG_DATAf, reg_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 13, exit,
                                 WRITE_OAMP_PE_PROGRAMm(unit, MEM_BLOCK_ALL, last_program_pointer[unit]++, reg_data));
...
/*  #19=#b: if tmp_reg2 == 0, jump to tmp_reg3 ** tmp_reg2 holds the encapsulation, tmp_reg3 is #c */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
...
/*  #23	if (tmp_reg2 - 2) == 0, jump to tmp_reg3 ** encapsulation == 1, jump to #c */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
    SET_OAMP_PE_OP1_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_TMP_REG2);
    SET_OAMP_PE_OP2_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_INST_VAR);
...
/*  #33 put #d to tmp_reg3  */
    ...
    SET_OAMP_PE_CMP2_ACT_BITS(reg_field, ARAD_PP_OAMP_PE_CMP_ACT_NEVER);
    SET_OAMP_PE_INST_SRC_BITS(reg_field, ARAD_PP_OAMP_PE_INST_SRC_NXT_JMP);
    SET_OAMP_PE_INST_CONST_BITS(reg_field, fst_inst_addr + 46); /*  should be #d */
    soc_OAMP_PE_PROGRAMm_field_set(unit, reg_data, PROG_DATAf, reg_field);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 13, exit,
                                 WRITE_OAMP_PE_PROGRAMm(unit, MEM_BLOCK_ALL, last_program_pointer[unit]++, reg_data));
...
/*  #36	if (TmpReg1 + 1) == 0, jump to tmp_reg3 ** NO TLV */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
    SET_OAMP_PE_OP1_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_TMP_REG1);
    SET_OAMP_PE_OP2_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_INST_VAR);
...
/*  #39 copy one lower bytes, rd in_fifo */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
    SET_OAMP_PE_IN_FIFO_RD_BITS(reg_field, 1);
    SET_OAMP_PE_FEM1_SEL_BITS(reg_field, 0Xb);
...
/*  #41	if TmpReg1- 3 == 0, jump to tmp_reg3 ** one TLV */
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_REG_ABOVE_64_CLEAR(reg_field);
    SET_OAMP_PE_OP1_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_TMP_REG1);
    SET_OAMP_PE_OP2_SEL_BITS(reg_field, ARAD_PP_OAMP_PE_OP_SEL_INST_VAR);
...
/*  #47, copy rest of packet (default instruction)*/
	SOC_SAND_SOC_IF_ERROR_RETURN(res, 13, exit,
								 WRITE_OAMP_PE_PROGRAMm(unit, MEM_BLOCK_ALL, last_program_pointer[unit]++, reg_default_instr));
