...
#define ARAD_PP_SLB_HASH_OFFSET_BIT_NUM           4
...
STATIC uint32 SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION_verify(SOC_SAND_IN int unit, SOC_SAND_IN SOC_PPC_SLB_OBJECT *untyped_obj)
{
  const SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION *object = (const SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION*)untyped_obj;
...
STATIC uint32 SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET_verify(SOC_SAND_IN int unit, SOC_SAND_IN SOC_PPC_SLB_OBJECT *untyped_obj)
{
  const SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET *object = (const SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET*)untyped_obj;

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  SOC_SAND_ERR_IF_ABOVE_MAX(object->offset, _arad_pp_slb_max_val_for_bits(ARAD_PP_SLB_HASH_OFFSET_BIT_NUM), 0, 10, exit);
...
STATIC uint32 _arad_pp_slb_init_hashing(SOC_SAND_IN int unit)
{
  ...
  SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION function;

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

  SOC_PPC_SLB_CLEAR(&function, SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_FUNCTION);

  /* Set the default hash function to be 0x8423. */
...
STATIC uint32 _arad_pp_slb_init_aging(SOC_SAND_IN int unit)
{
  ...
  /*        Note that the device starts with aging disabled, so it might not matter - */
  /*        since when aging is enabled, the time will be set as well. */
...
/*
 * Initialize SLB program variables. 
 *  
 * Details: 
 * 1. Set ENABLE_STANDARD_FORMATS to 0. This means that the LEM format is treated as SLB. 
 * 2. Set ENABLE_LOOKUP to 1. 
 * 3. Set the AND_MASK to 0xf. 
 * 4. Set the OR_MASK to the slb_prefix. 
 */
STATIC uint32 _arad_pp_slb_init_program_variables(SOC_SAND_IN int unit, SOC_SAND_IN uint32 slb_prefix)
{
  ...
  for (program_idx = 0; program_idx < ARAD_PP_SLB_NOF_PROGRAMS; program_idx++) {
    ...
    soc_mem_field32_set(unit, IHB_CONSISTENT_HASHING_PROGRAM_VARIABLESm, reg_val, ITEM_40_40f /* ENABLE_STANDARD_FORMATSf */, 0);
    soc_mem_field32_set(unit, IHB_CONSISTENT_HASHING_PROGRAM_VARIABLESm, reg_val, ENABLE_LOOKUPf, 1);
  ...
  )
{
  ...
  if (enable) {
    ...
    res = _arad_pp_slb_init_program_variables(unit, slb_prefix);
...
uint32 
  _arad_pp_slb_slb_action_to_lem_action(
    SOC_SAND_IN     int                                          unit,
    SOC_SAND_IN     SOC_PPC_SLB_TRAVERSE_ACTION *                   action,
    SOC_SAND_OUT    ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION *            mact_traverse_action
  )
{
  uint32 rv;
  ...
  ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_clear(mact_traverse_action);
  ...
  )
{
  ...
  ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION mact_action;
...
{
  uint32 rv;
  ARAD_PP_FRWRD_MACT_AGING_INFO aging_info;
  ...
  )
{
  ...
  SOC_PPC_SLB_OBJECT_TYPE valid_types[] = { 
    ...
    SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_FUNCTION,
    SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_LAG_HASH_SEED,
    SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_ECMP_HASH_SEED,
    SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_OFFSET,
  };
  uint32 nof_valid_types = 5;
  ...
  )
{
  ...
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_SLB_ENTRY_AGING_TIME_IN_SECONDS:
    ...
    break;
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_FUNCTION:
    {
      SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION *hash_function_cfg = (SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_FUNCTION*)configuration_item;
  ...
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_ECMP_HASH_SEED:
    {
      SOC_PPC_SLB_CONFIGURATION_ITEM_ECMP_HASH_SEED *ecmp_hash_seed_function_cfg = (SOC_PPC_SLB_CONFIGURATION_ITEM_ECMP_HASH_SEED*)configuration_item;
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHB_CONSISTENT_HASHING_LB_KEY_CFGr, SOC_CORE_ALL, 0, CONSISTENT_HASHING_ECMP_LB_KEY_SEEDf,  (uint32)ecmp_hash_seed_function_cfg->seed));
    }
    break;
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_OFFSET:
    {
      SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET *hash_offset_cfg = (SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET*)configuration_item;
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IHB_CONSISTENT_HASHING_LB_KEY_CFGr, SOC_CORE_ALL, 0, CONSISTENT_HASHING_LAG_LB_KEY_SHIFTf,  (uint32)hash_offset_cfg->offset));
    }
    break;
  default:
    SOC_SAND_VERIFY(FALSE);
  ...
  )
{
  ...
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_ECMP_HASH_SEED:
    {
      SOC_PPC_SLB_CONFIGURATION_ITEM_ECMP_HASH_SEED *ecmp_hash_seed_function_cfg = (SOC_PPC_SLB_CONFIGURATION_ITEM_ECMP_HASH_SEED*)configuration_item;
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  60,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IHB_CONSISTENT_HASHING_LB_KEY_CFGr, SOC_CORE_ALL, 0, CONSISTENT_HASHING_ECMP_LB_KEY_SEEDf, &field_val));
      ecmp_hash_seed_function_cfg->seed = field_val;
    }
    break;
  case SOC_PPC_SLB_OBJECT_TYPE_CONFIGURATION_ITEM_HASH_OFFSET:
    {
      SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET *hash_offset_cfg = (SOC_PPC_SLB_CONFIGURATION_ITEM_HASH_OFFSET*)configuration_item;
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  70,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, IHB_CONSISTENT_HASHING_LB_KEY_CFGr, SOC_CORE_ALL, 0, CONSISTENT_HASHING_LAG_LB_KEY_SHIFTf, &field_val));
