  ...
  )
{
   ...
   */
  for (local_port = 0; (local_port < ARAD_NOF_LOCAL_PORTS) && (!is_tm_pmf_per_port_mode); local_port++) {
  ...
  res = arad_sw_db_pgm_program_rsrc_set(unit, ARAD_FP_DATABASE_STAGE_INGRESS_PMF, &reserved_progs);
  ...
  res = arad_pmf_prog_select_eth_init(unit, ARAD_FP_DATABASE_STAGE_INGRESS_PMF, is_tm_pmf_per_port_mode, &init_info);
  ...
  res = arad_pmf_prog_select_eth_init(unit, ARAD_FP_DATABASE_STAGE_EGRESS, FALSE, &init_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 140, exit);

#ifdef BCM_88660_A0
  if (SOC_IS_ARADPLUS(unit) && (!(SOC_IS_ARDON(unit)))) {
      /* Identical init for SLB as at egress for program selection */
      res = arad_pmf_prog_select_eth_init(unit, ARAD_FP_DATABASE_STAGE_INGRESS_SLB, FALSE, &init_info);
      SOC_SAND_CHECK_FUNC_RESULT(res, 140, exit);
  }
#endif /* BCM_88660_A0 */
  ...
  )
{
  ...
  if ((SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1)&&(soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) == 0)) {
  ...
  if (soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE) == 0) {
      ...
      if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 0) {
          res = arad_pmf_low_level_ce_key_construction_unsafe(
              ...
              pmf_pgm_ndx,
              in_port_profile,
              (SOC_IS_JERICHO(unit) ? sizeof(Jericho_pmf_key_construction_info_tm) : sizeof(Arad_pmf_key_construction_info_tm))
              / sizeof(ARAD_PMF_KEY_CONSTR),
              SOC_IS_JERICHO(unit) ? Jericho_pmf_key_construction_info_tm : Arad_pmf_key_construction_info_tm
              );
          SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
          ...
          res = arad_pmf_low_level_fes_action_construction_unsafe(
              ...
              pmf_pgm_ndx,
              in_port_profile,
              (SOC_IS_JERICHO(unit) ? sizeof(Jericho_pmf_fes_construction_info_tm) : sizeof(Arad_pmf_fes_construction_info_tm))
              / sizeof(ARAD_PMF_FES_CONSTR),
              SOC_IS_JERICHO(unit) ? Jericho_pmf_fes_construction_info_tm : Arad_pmf_fes_construction_info_tm
              );
          SOC_SAND_CHECK_FUNC_RESULT(res, 80, exit);
  ...
  pgm_info.copy_pgm_var += (((0x3 << 4 /* Snoop stength */) + (ARAD_PP_TRAP_CODE_INTERNAL_IHP_TIMNA_PREFIX >> 4 /* Prefix base */)) 
  ...
  )
{
  ...
  soc_IHB_PMF_PROGRAM_SELECTION_CAMm_field32_set(unit, pgm_sel_tbl_data, PARSER_LEAF_CONTEXTf, ARAD_PMF_PGM_SEL_PARSER_PROFILE_RAW);
     ...
     /* ARAD mode */
      ...
      if (SOC_IS_JERICHO(unit)) {
          ...
          for (line_ndx = 0; line_ndx < pmf_key_construction_info_length; line_ndx++) {
              ...
              else if ((pmf_key_construction_info[line_ndx].is_msb == 0) && (pmf_key_construction_info[line_ndx].ce_id == 1)) {
                  pmf_key_construction_info[line_ndx].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ONES;
              }
      ...
      if (SOC_DPP_CONFIG(unit)->pp.oam_statistics == 1) {
          ...
          pmf_key_construction_info[16].qual_nof_bits = 2;
          pmf_key_construction_info[16].qual_lsb = 0;

          pmf_key_construction_info[15].qual_type = SOC_PPC_FP_QUAL_TRAP_QUALIFIER_FHEI;
          pmf_key_construction_info[15].qual_nof_bits = 16;
          pmf_key_construction_info[15].qual_lsb = 0;

         /* pmf_key_construction_info[17].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ONES;
          pmf_key_construction_info[17].qual_nof_bits = 2;
          pmf_key_construction_info[17].qual_lsb = 0;

          pmf_key_construction_info[18].qual_type = SOC_PPC_FP_QUAL_OAM_ID;
          pmf_key_construction_info[18].qual_nof_bits = 17;
          pmf_key_construction_info[18].qual_lsb = 0; */

          /*delete lines 17 and 18*/
          for (i=19; i<pmf_key_construction_info_length; i++) {
              pmf_key_construction_info[i-2]=pmf_key_construction_info[i];
          }
  ...
  )
{
  ...
  res = arad_pmf_low_level_fes_action_construction_unsafe(
            ...
            pmf_pgm_ndx,
            ARAD_PMF_PORT_PROFILE_RAW,
            (sizeof(Arad_pmf_fes_construction_info_xgs) / sizeof(ARAD_PMF_FES_CONSTR)),
            Arad_pmf_fes_construction_info_xgs
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 80, exit);
  ...
  )
{
  ...
  uint8
    ...
    for (local_port = 0; local_port < ARAD_NOF_LOCAL_PORTS ; local_port++) {
  ...
  )
{
  ...
  ARAD_PMF_PGM_INFO_clear(&pmg_info);
  
  pmg_info.bytes_to_rmv.header_type = ARAD_PMF_PGM_BYTES_TO_RMV_HDR_1ST;
  pmg_info.fc_type = SOC_TMC_PORTS_FC_TYPE_NONE;
  pmg_info.header_profile = ARAD_PMF_PGM_HEADER_PROFILE_ETHERNET;
