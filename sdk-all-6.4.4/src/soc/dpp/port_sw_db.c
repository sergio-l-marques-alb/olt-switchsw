...
#define PORT_CORE       (PORT_INFO.core)
...
STATIC soc_error_t soc_port_sw_db_core_set(int unit, soc_port_t port, uint32 core /*zero based*/) ;
...
soc_port_sw_db_init(int unit) 
{
    ...
    for (port = 0; port < SOC_MAX_NUM_PORTS; port++) {
        ...
        PORT_INFO.header_type_out = SOC_TMC_PORT_HEADER_TYPE_NONE;
        PORT_INFO.header_type_in = SOC_TMC_PORT_HEADER_TYPE_NONE;
    }
...
soc_port_sw_db_port_add(int unit,  int core_of_port, soc_port_t port,uint32 channel, uint32 flags, soc_port_if_t interface, soc_pbmp_t phy_ports) 
{
        ...
        for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; port_i++) {
            if (logical_ports_info[unit][port_i].valid) {
                ...
                if (interface == phy_ports_info[unit][first_phy_port].interface_type 
                    && core_of_first_phy_port == phy_ports_info[unit][first_phy_port].core ) {
    ...
    if (1 == PORT_PHY_INFO.channels_count) {
        ...
        SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PORT_SW_DB_PHY_PORT_CORE, &core_of_first_phy_port, PORT_PHY));
        ...
        if (SOC_PORT_IF_CPU != interface && SOC_PORT_IF_RCY != interface) {
            SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PORT_SW_DB_PHY_PORT_CORE, &core_of_first_phy_port, PORT_PHY)) ;
            if (core_of_first_phy_port != core_of_port) {
                SOCDNX_EXIT_WITH_ERR(
                    SOC_E_PARAM,
                    (_BSL_SOCDNX_MSG("Logical port %d: All logical ports on same first_phy_port (%d) must have the same core"), port, PORT_PHY)) ;
            }
        }
    }    
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_core_set(unit, port, core_of_port)) ;
...
soc_port_sw_db_port_remove(int unit, soc_port_t port) 
{
    ...
    if (header_type_in == BCM_SWITCH_PORT_HEADER_TYPE_ETH) {
        SOC_PBMP_PORT_REMOVE(SOC_INFO(unit).ether.bitmap, port);
    } else if (header_type_in == BCM_SWITCH_PORT_HEADER_TYPE_STACKING) {
        SOC_PBMP_PORT_REMOVE(SOC_INFO(unit).st.bitmap, port);
    }
...
STATIC soc_error_t
soc_port_sw_db_core_set(int unit, soc_port_t port, uint32 core /*zero based*/) 
{
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PORT_SW_DB_LOGICAL_PORTS_CORE, &core, port)) ;
...
soc_port_sw_db_protocol_offset_get(int unit, soc_port_t port, uint32 flags, uint32 *offset) 
{
    ...
    if (flags == SOC_PROTOCOL_OFFSET_FLAGS_FORCE_ILKN && SOC_PORT_IF_ILKN != interface_type) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PORT, (_BSL_SOCDNX_MSG("Port %d isn't an ILKN port"),port));
    }
    ...
    for(j = 0; j < 12; ){
        ...
        for(i = 0 ; i < table_line->num_of_ports; i ++){
            ...
            case 9:
                ...
                break;
            case 10:
                phy_port_core = phy_ports_info[unit][table_line->ports[i]].core;
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    if (SOC_PORT_IF_CPU == phy_ports_info[unit][table_line->ports[i]].interface_type) {
                        for (port = 0; port < SOC_MAX_NUM_PORTS; ++port) {
                            uint32 is_valid;
                            soc_port_if_t if_type;
                            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_is_valid_port_get(unit, port, &is_valid));
                            if (is_valid) {
                                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port, &if_type));
                                if (SOC_PORT_IF_CPU == if_type) {
                                    logical_port_core = logical_ports_info[unit][port].core;
