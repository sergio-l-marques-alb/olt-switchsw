...
#define _MAX_KEY_LEN_   (_MAX_KEY_LEN_48_)
 ...
 */
inline unsigned int _key_get_bits(unsigned int *key, 
 ...
 */
inline int _key_shift_left(unsigned int *key, unsigned int shift)
{
 ...
 * eg., for key size of 48, word0-[bits 48-32] word1-[bits31-0]
 */
inline int _key_shift_right(unsigned int *key, unsigned int shift)
{
    unsigned int index=0;
 ...
 * eg., for key size of 48, word0-[bits 48-32] word1-[bits31-0]
 */
inline int _key_append(unsigned int *key, 
                unsigned int *length,
...
inline int _bpm_append(unsigned int *key, 
                unsigned int *length,
                unsigned int skip_addr,
                unsigned int skip_len)
{
...
int _print_trie_node(trie_node_t *trie, void *datum)
{
	...
	LOG_CLI((BSL_META("trie: %p, type %s, skip_addr 0x%x skip_len %d count:%d bpm:0x%x Child[0]:%p Child[1]:%p\n"),
                 (void *) trie, (trie->type == PAYLOAD)?"P":"I",
                 trie->skip_addr, trie->skip_len, 
                 trie->count, trie->bpm,  (void *)trie->child[0].child_node,  (void *)trie->child[1].child_node));
    }
    return SOC_E_NONE;
}
...
static int _trie_preorder_traverse(trie_node_t *trie, trie_callback_f cb, void *user_data)
{
    ...
    if (SOC_SUCCESS(rv)) {
        rv = _trie_preorder_traverse(trie->child[0].child_node, cb, user_data);
    }
    if (SOC_SUCCESS(rv)) {
        rv = _trie_preorder_traverse(trie->child[1].child_node, cb, user_data);
    }
    return rv;
}
...
static int _trie_inorder_traverse(trie_node_t *trie, trie_callback_f cb, void *user_data)
{
    ...
    if (SOC_SUCCESS(rv)) {
        rv = _trie_inorder_traverse(trie->child[1].child_node, cb, user_data);
    }
    return rv;
}
 ...
 *     trie_traverse_find
 * Purpose:
 *     Preorder traverse to find a specified node. Stop traversing after find the node.
 * Return:
 *     SOC_E_NONE: Find the key in this trie
 *     SOC_E_NOT_FOUND: Don't find the key in this trie
 */
int trie_traverse_find(trie_node_t *trie, trie_callback_f cb, 
                  void *user_data)
{
    int rv = SOC_E_NONE;
    if (!cb) {
    ...
    if (SOC_FAILURE(rv)) {
        rv = trie_traverse_find(trie->child[0].child_node, cb, user_data);
    }
    if (SOC_FAILURE(rv)) {
        rv = trie_traverse_find(trie->child[1].child_node, cb, user_data);
    }
    return rv;
}
    ...
    if (length > trie->skip_len) {
        if (lcp == trie->skip_len) {
            bit = (key[KEY_BIT2IDX(length - lcp)] & \
                   TP_SHL_IN32(1,(length - lcp - 1) % _NUM_WORD_BITS_)) ? 1:0;
            if (dump) {
 ...
 */
inline int _trie_find_lpm(trie_node_t *trie,
			  unsigned int *key,
			  unsigned int length,
			  trie_node_t **payload,
			  trie_callback_f cb,
 ...
 *     trie_node_mem_alloc
 * Purpose:
 *     Alloc a trie node. If memory pool is enable, then use hpcm. Otherwise, use dynamic alloc.
 */
int trie_node_mem_alloc(trie_node_t **node, void **hpcm_elem_handle)
{
    int rv = SOC_E_NONE;
    if (mem_pool_enable) {
        if (!node || !hpcm_elem_handle) {
            rv = SOC_E_PARAM;
        } else {
            rv = hpcm_sl_alloc_payload(0, g_trie_node_hpcm, (void **)node, hpcm_elem_handle);
        }
    } else {
        if (!node) {
            rv = SOC_E_PARAM;
        } else {
            *node = sal_alloc(sizeof(trie_node_t), "trie_node_1");
        }
    }
    return rv;
}
 ...
 *     trie_node_mem_free
 * Purpose:
 *     Free a trie node. If memory pool is enable, then use hpcm. Otherwise, use dynamic free.
 */
int trie_node_mem_free(trie_node_t *node)
{
    int rv = SOC_E_NONE;
    ...
    if (!node) {
        ...
        if (mem_pool_enable) {
            rv = hpcm_sl_free_payload(0, g_trie_node_hpcm, (void *)node->hpcm_elem_handle);
        } else {
            sal_free(node);
        }
    }
...
static int _trie_internal_node_mem_free_cb(trie_node_t *trie, void *datum)
{
    if (trie->type == INTERNAL) {
        trie_node_mem_free(trie);     
    }
    return SOC_E_NONE;
}

int trie_internal_node_free(trie_node_t *trie)
{
    if (trie == NULL) {
        return SOC_E_PARAM;     
    }
    return _trie_traverse(trie, _trie_internal_node_mem_free_cb, NULL, _TRIE_POSTORDER_TRAVERSE);
}
...
{
    int rv = SOC_E_NONE, lsb=0, msbpos=0, lsbpos=0, bit=0, index;
    ...
    if (msb) {
		...
		/* (lsbpos == lsb-1) is only true for first node (loop) here */
                skip_node = payload;
		hpcm_elem_handle = payload->hpcm_elem_handle;
                /* init memory */
		sal_memset(skip_node, 0, sizeof(trie_node_t));
		skip_node->hpcm_elem_handle = hpcm_elem_handle;
            } else {
		        /* other nodes need to be created */
                rv = trie_node_mem_alloc(&skip_node, &hpcm_elem_handle);
                if (SOC_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META("%s: Failed to alloc skip node %d:%s !!!\n"), 
                               FUNCTION_NAME(), rv, soc_errmsg(rv)));
                    if (child) {
                        trie_internal_node_free(child);
                    }
                    return rv;
                }
                /* init memory */
			...
			trie_node_t **child /* child pointer if the child is modified */)
{
    ...
    *child = NULL;
    ...
    if (lcp == length) { /* guaranteed: length < _MAX_SKIP_LEN_ */
        ...
        rv = trie_node_mem_alloc(&node, &hpcm_elem_handle);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("%s: Failed to alloc internal node %d:%s !!!\n"), 
                       FUNCTION_NAME(), rv, soc_errmsg(rv)));
            return rv;
        }
        _CLONE_TRIE_NODE_(node, trie);
        node->hpcm_elem_handle = hpcm_elem_handle;
                ...
                trie_node_t *payload)
{
	...
	if (trie->v6_key) {
        ...
        if (child && SOC_SUCCESS(rv)) { /* chande the old child pointer to new child */
            trie->trie = child;
        }
    }
    ...
    *child = NULL;
	 ...
	 */ 
        ...
        /* the node has 2 children, update it to internal node 
               * we need to clear the bpm bit of itself before convert it to internal
               */
                _BITCLR(trie->bpm, 0);
                rv = trie_node_mem_alloc(&node, &hpcm_elem_handle);
                if (SOC_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META("%s: Failed to alloc node to replace of the double child node %d:%s !!!\n"), 
                               FUNCTION_NAME(), rv, soc_errmsg(rv)));
                    return rv;
                }
                _CLONE_TRIE_NODE_(node, trie);
		     ...
		     */
                    ...
                    if (SOC_FAILURE(rv)) {
                        LOG_ERROR(BSL_LS_SOC_COMMON,
                                  (BSL_META("%s: Failed to alloc node to replace of the single child node %d:%s !!!\n"), 
                                   FUNCTION_NAME(), rv, soc_errmsg(rv)));
                        return rv;
                    }
...
static int _trie_split_len_get(trie_node_t  *trie,
		       unsigned int *pivot,
		       unsigned int *length,
		       ...
		       unsigned int *bpm)
{
    ...
    if (!trie || !pivot || !length || max_count == 0) return SOC_E_PARAM;
	...
	if (((*length > max_split_len) && (trie->count != max_count))) {
		...
		if ((((*length - max_split_len) > trie->skip_len) && (trie->skip_len == 0))) {
		     ...
		     */
		    /* another possibility is we are spliting to a pair of bucket, 
		     * in this case, simply chose this node as the split point
		     */
		    return rv;
		}
	    ...
	    rv = _trie_split_len_get(trie->child[bit].child_node, 
			     pivot, length, max_count, max_split_len, bpm);
	}
 ...
 *     trie_split_len_get
 * Purpose:
 *     Get the length of sub trie if this trie split
 */
int trie_split_len_get(trie_t *trie,
	       const unsigned int max_split_len,
               unsigned int *length)
{
    int rv = SOC_E_NONE;
    unsigned int max_count=0;
    unsigned int pivot[BITS2WORDS(_MAX_KEY_LEN_144_)];
    unsigned int bpm[BITS2WORDS(_MAX_KEY_LEN_144_)];

    if (!trie || !length) return SOC_E_PARAM;
    ...
    *length = 0;
    ...
    if (trie->trie) {
    	...
    	if (trie->v6_key) {	    
    	    rv = _trie_v6_split_len_get(trie->trie, pivot, length, 
    			     max_count, max_split_len, bpm);
    	} else {
    	    rv = _trie_split_len_get(trie->trie, pivot, length, 
    			     max_count, max_split_len, bpm);
    	}
    }
    return rv;
...
{
    int bit=0, rv=SOC_E_NONE;
    int pair_split_optimization = FALSE;
    void *hpcm_elem_handle = NULL;
	 ...
	 */
	if (split_to_pair) {
	    if ((trie->count < max_count) && 
		!(((trie->child[0].child_node != NULL) && (trie->child[0].child_node->count == trie->count)) ||
		  ((trie->child[1].child_node != NULL) && (trie->child[1].child_node->count == trie->count)))) {
		pair_split_optimization = TRUE;
            } else if ((trie->count == max_count) && (trie->child[0].child_node != NULL) && (trie->child[1].child_node != NULL) &&
                       ((SOC_SBX_POWER_OF_TWO(trie->child[0].child_node->count) && (trie->child[0].child_node->count >= (max_count/2))) ||
                        (SOC_SBX_POWER_OF_TWO(trie->child[1].child_node->count) && (trie->child[1].child_node->count >= (max_count/2))))) {
		pair_split_optimization = TRUE;
	    } else {
		pair_split_optimization = FALSE;
	    }
	} else {
	    pair_split_optimization = FALSE;
	}

	if (((*length > max_split_len) && (trie->count != max_count)) || pair_split_optimization) {
	    /* the pivot is getting too long, we better split at this node for
	     * better bucket capacity efficiency if we can. We can split if 
	     * the trie node has a count != max_count, which means the 
	     * resulted new trie will not have all pivots (FULL)
		...
		*state = TRIE_SPLIT_STATE_PAYLOAD_SPLIT_DONE;
		...
		if ((((*length - max_split_len) > trie->skip_len) && (trie->skip_len == 0)) || pair_split_optimization) {
		    /* the length is longer than max_split_len, and the trie->skip_len is 0,
		     * so the best we can do is use the node as the split point
		     */
		    /* another possibility is we are spliting to a pair of bucket, 
        ...
        rv = trie_node_mem_alloc(split_node, &hpcm_elem_handle);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("%s: Failed to alloc intelnal node as split point %d:%s !!!\n"), 
                       FUNCTION_NAME(), rv, soc_errmsg(rv)));
            return rv;
        }
		...
		(*split_node)->hpcm_elem_handle = hpcm_elem_handle;
		if ((*length - max_split_len) > trie->skip_len) {
		    /* the length is longer than the max_split_len, and the trie->skip_len is
		     * shorter than the difference (max_split_len pivot is not covered by this 
		     * node but covered by its parent, the best we can do is split at the branch
               ...
               uint8 payload_node_split)
{
    ...
    *length = 0;
	...
	if (trie->v6_key) {	    
	    ...
	    if (split_count == max_count) {
		/* split gives same trie, this is only possible when split into pair */
		assert(split_to_pair);
		*split_trie_root = trie->trie;
		...
		if (SOC_SUCCESS(rv)) {
		    ...
		    *split_trie_root = node;
            ...
            LOG_CLI((BSL_META("!!!! Failed to split the trie error:%d state: %d !!!\n"),
                     rv, state));
        }
    } else {
        rv = SOC_E_PARAM;
...
{
    trie_t *trie = sal_alloc(sizeof(trie_t), "trie-head");
	...
	/* coverity[dead_error_begin] */
        ...
        if (SOC_SUCCESS(rv)) {
            LOG_CLI((BSL_META("\n Split Trie Pivot: 0x%08x 0x%08x Length: %d Root: %p \n"),
                     pivot[0], pivot[1], length,  (void *)newroot));
            ...
            for(index=0; index < numkey && rv == SOC_E_NONE; index++) {
                ...
                if (rv != SOC_E_NONE) {
                    ...
                    if (rv != SOC_E_NONE) {
                        LOG_CLI((BSL_META("SEARCH: Key=0x%x 0x%x len %d SEARCH idx:%d failed on both trie!!!!\n"), 
...
int tmu_taps_trie_ut(int id, unsigned int seed)
{
    ...
    for(index=0; index < numkey && rv == SOC_E_NONE; index++) {
...
#ifdef LOG
        LOG_CLI((BSL_META("+ Inserted Key=0x%x 0x%x vpn=0x%x pfx=0x%x Len=%d idx:%d\n"), 
        ...
        rv = trie_insert(trie, &pyld[index].key[0], NULL, pyld[index].len, &pyld[index].node);
        if (rv != SOC_E_NONE) {
            LOG_CLI((BSL_META("FAILED to Insert Key=0x%x 0x%x vpn=0x%x pfx=0x%x Len=%d idx:%d\n"), 
                     pyld[index].key[0], pyld[index].key[1], vrf,
                     pyld[index].key[1] & MASK(pyld[index].len - _VRF_LEN_), 
                     pyld[index].len, index));
        }
...
#endif
            if (rv != SOC_E_NONE) {
                LOG_CLI((BSL_META("SEARCH: Key=0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n"), 
                         pyld[index].key[0], pyld[index].key[1],
                         pyld[index].len, index));
                break;
            } else {
                ...
                if (pyld[index].key[0] != result_key[0] ||
                    ...
                    LOG_CLI((BSL_META(" Found key mismatches with the expected Key !!!! \n")));
                    rv = SOC_E_FAIL;
                }
#ifdef VERBOSE
                LOG_CLI((BSL_META("Lkup[%d] key/len: 0x%x 0x%x/%d Found Key/len: 0x%x 0x%x/%d \n"),
    ...
    while(num_deleted < numkey && rv == SOC_E_NONE) {
        ...
        if (pyld[index].len != 0xFFFFFFFF) {
            ...
            if (rv == SOC_E_NONE) {
                ...
                if (rv == SOC_E_NONE) {
#ifdef LOG
                    LOG_CLI((BSL_META("Deleted Key=0x%x 0x%x Len=%d idx:%d Num-Key:%d\n"), 
                             pyld[index].key[0], pyld[index].key[1], 
                    ...
                    for (index=0; index < numkey; index++) {
                        ...
                        rv = trie_search(trie, &pyld[index].key[0], pyld[index].len, &pyldptr);
                        if (rv != SOC_E_NONE) {
                            LOG_CLI((BSL_META("ALL SEARCH after delete: Key=0x%x 0x%x len %d SEARCH idx:%d failed!!!!\n"), 
                                     pyld[index].key[0], pyld[index].key[1],
                                     pyld[index].len, index));
                            break;
                        } else {
                    ...
                    LOG_CLI((BSL_META("Deleted Key=0x%x 0x%x Len=%d idx:%d FAILED!!!\n"), 
                             pyld[index].key[0], pyld[index].key[1], 
                             pyld[index].len, index));
                    break;
                }
...
int tmu_taps_bpm_trie_ut(int id, unsigned int seed)
{
        ...
        if (rv != SOC_E_NONE) {
            ...
            if (SOC_SUCCESS(rv) && ++pivot < _MAX_TEST_PIVOTS_) {
                ...
                if (SOC_SUCCESS(rv)) {
                    ...
                    LOG_CLI((BSL_META("BPM for split pivot: 0x%x 0x%x / %d = [0x%x 0x%x] \n"),
                             pivot_pyld[pivot].key[0], pivot_pyld[pivot].key[1],
	...
	if (pyld[pfx].len >= 1) {
	    ...
	    if (SOC_FAILURE(rv)) {
		LOG_CLI((BSL_META("!!!!! BPM search Test FAILED to propagate add len=0!!!!!\n")));
		assert(0);
	    }
		...
		LOG_CLI((BSL_META("!!!!! BPM search Test FAILDED after propagate add !!!!!\n")));
		assert(0);		
	    }
		...
		LOG_CLI((BSL_META("!!!!! BPM search Test FAILDED after propagate delete !!!!!\n")));
		assert(0);		
	    }
	...
	LOG_CLI((BSL_META("!!!!! BPM search Test 2 Skipped after tried %d times!!!!!\n"), _MAX_NUM_PICK));	
    } else {
