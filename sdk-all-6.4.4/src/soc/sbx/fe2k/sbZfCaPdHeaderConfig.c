/*
 * $Id: sbZfCaPdHeaderConfig.c,v 1.2 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfCaPdHeaderConfig.hx"
...
sbZfCaPdHeaderConfig_Pack(sbZfCaPdHeaderConfig_t *pFrom,
  ...
  int i;
  int size = SB_ZF_CAPDHEADERCONFIG_SIZE_IN_BYTES;
  ...
  return SB_ZF_CAPDHEADERCONFIG_SIZE_IN_BYTES;
}
...
sbZfCaPdHeaderConfig_Unpack(sbZfCaPdHeaderConfig_t *pToStruct,
  ...
  (pToStruct)->m_uSpare0 =  (uint32)  ((pFromData)[3] >> 2) & 0x3f;

  /* Unpack Member: m_uLengthUnits */
  (pToStruct)->m_uLengthUnits =  (uint32)  ((pFromData)[3] ) & 0x03;

  /* Unpack Member: m_uSpare1 */
  (pToStruct)->m_uSpare1 =  (uint32)  ((pFromData)[2] >> 7) & 0x01;

  /* Unpack Member: m_uLengthSize */
  (pToStruct)->m_uLengthSize =  (uint32)  ((pFromData)[2] >> 4) & 0x07;

  /* Unpack Member: m_uSpare2 */
  (pToStruct)->m_uSpare2 =  (uint32)  ((pFromData)[2] >> 1) & 0x07;

  /* Unpack Member: m_uLengthPosition */
  (pToStruct)->m_uLengthPosition =  (uint32)  (pFromData)[1] ;
  (pToStruct)->m_uLengthPosition |=  (uint32)  ((pFromData)[2] & 0x01) << 8;

  /* Unpack Member: m_uSpare3 */
  (pToStruct)->m_uSpare3 =  (uint32)  ((pFromData)[0] >> 6) & 0x03;

  /* Unpack Member: m_uBaseLength */
  (pToStruct)->m_uBaseLength =  (uint32)  ((pFromData)[0] ) & 0x3f;
