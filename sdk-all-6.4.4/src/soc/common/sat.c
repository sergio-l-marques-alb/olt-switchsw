    ...
    )
{
    ...
    for (i = 0; i < pkt_edit->number_of_stamps; i++) {
        ...
        if (tmp_stamp->stamp_type == socSatStampConstant2Bit) {
            tmp_stamp_offset_bits = tmp_stamp->offset - 3;  /* hw adjust  */
            stamp_field_info->stamp_2_ena = 1;
            stamp_field_info->stamp_2_offset = tmp_stamp_offset_bits/8; 
            stamp_field_info->stamp_2_shift = tmp_stamp_offset_bits%8;
            stamp_field_info->stamp_2_value = tmp_stamp->value;
        }
        else if (tmp_stamp->stamp_type == socSatStampCounter8Bit) {
            tmp_stamp_offset_bits = tmp_stamp->offset + 7;  /* hw adjust  */
            stamp_field_info->counter_8_ena = 1;
            stamp_field_info->counter_8_offset = tmp_stamp_offset_bits/8;
            stamp_field_info->counter_8_shift = tmp_stamp_offset_bits%8;
            ...
            stamp_field_info->counter_16_offset = tmp_stamp_offset_bits/8;
            stamp_field_info->counter_16_shift = tmp_stamp_offset_bits%8;
    ...
    )
{
   ...
   /* For writing on dynamic tables (in the OAMP), have to enable dynamic memory access */
    ...
    for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        ...
        field_data = config->packet_edit[priority].packet_length[0];
        soc_mem_field_set(unit, OAMP_SAT_TX_GEN_PARAMSm, tx_gen_params_data, PKT_LENGTH_NEXTf, &field_data);
