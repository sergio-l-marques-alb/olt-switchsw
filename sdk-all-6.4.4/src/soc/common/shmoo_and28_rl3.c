...
_and28_rl3_zq_calibration(int unit, int phy_ndx)
{
    ...
    p_drive = 4;    /* WILL BECOME 8 BELOW */
    n_drive = 4;    /* WILL BECOME 8 BELOW */
    ...
    if((p_drive << 1) >= SHMOO_AND28_RL3_MAX_ZQ_CAL_RANGE)
    {
        p_drive = SHMOO_AND28_RL3_MAX_ZQ_CAL_RANGE - 1;
        LOG_ERROR(BSL_LS_SOC_DDR, (BSL_META_U(unit, "     WARNING: ZQ calibration error (P limit) - Manual IO programming required for correct operation\n")));
        /* return SOC_E_FAIL; */
    }
    else
    {
    ...
    DDR34_RL3_SET_FIELD(data, DDR34_RL3_PHY_CONTROL_REGS, ZQ_CAL, ZQ_DRIVE_P, p_drive);
    WRITE_DDR34_RL3_PHY_CONTROL_REGS_ZQ_CALr(unit, phy_ndx, data);
    sal_usleep(SHMOO_AND28_RL3_SHORT_SLEEP);
    ...
    if((n_drive << 1) >= SHMOO_AND28_RL3_MAX_ZQ_CAL_RANGE)
    {
        n_drive = SHMOO_AND28_RL3_MAX_ZQ_CAL_RANGE - 1;
        LOG_ERROR(BSL_LS_SOC_DDR, (BSL_META_U(unit, "     WARNING: ZQ calibration error (N limit) - Manual IO programming required for correct operation\n")));
        /* return SOC_E_FAIL; */
    }
    else
    {
...
_shmoo_and28_rl3_entry(int unit, int phy_ndx, and28_rl3_shmoo_container_t *scPtr, uint32 mode)
{
    ...
    /*R08*/ if(shmoo_dram_info.sim_system_mode)
            ...
            data = 0;
            DDR34_RL3_SET_FIELD(data, DDR34_RL3_PHY_BYTE_LANE_0, READ_CONTROL, MODE, 1);
            ...
            data = 0;
            DDR34_RL3_SET_FIELD(data, DDR34_RL3_PHY_BYTE_LANE_0, VDL_CONTROL_RD_QKP, FORCE, 1);
            temp = (((*scPtr).size1000UI * 3) / 4000) + SHMOO_AND28_RL3_RD_DQS_VDL_OFFSET;       /* 75% + Offset */
            if(temp > SHMOO_AND28_RL3_MAX_VDL_LENGTH)
            {
