...
_soc_mem_array_sbusdma_write(int unit, soc_mem_t mem, unsigned array_index_start, 
                             ...
                             int clear_buf_ent) /* clear_buf_ent is not used as SLAM DMA only supports a one entry buffer */
{
    soc_control_t *soc = SOC_CONTROL(unit);
    uint32        start_addr;
    uint32        count = 0;
    ...
    for (array_index = array_index_start; array_index <= array_index_end; ++array_index) {
        ...
        if (mem_clear) {
            soc_cm_sflush(unit, buffer, WORDS2BYTES(data_beats));
        } else {
                                    ...
                                    int copyno, int force_entry_size, int host_entries, void *host_buff)
{
    ...
    else if (SOC_IS_ARAD(unit) || SOC_IS_DFE(unit)) {
