...
STATIC int
_xlmac_timestamp_delay_set(int unit, soc_port_t port, int timesync_pll_clock_ns, int speed)
{
    uint64 ctrl;
    uint32 clk_rate;
    uint32 tx_clk_ns;
    ...
    soc_reg64_field32_set(unit, XLMAC_TIMESTAMP_ADJUSTr, &ctrl,
                   TS_OSTS_ADJUSTf, 
                   (timesync_pll_clock_ns - ((11 * tx_clk_ns ) / 2)));
...
int xlmac_encap_set(int unit, soc_port_t port, int flags, portmod_encap_t encap)
{
    ...
    soc_reg64_field32_set(unit, XLMAC_RX_CTRLr, &reg_val,
                          STRICT_PREAMBLEf, encap == SOC_ENCAP_IEEE ? 1 : 0);
...
int xlmac_tx_mac_sa_get(int unit, soc_port_t port, sal_mac_addr_t mac)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_TX_MAC_SAr(unit, port, &rval64));
...
int xlmac_tx_average_ipg_get(int unit, soc_port_t port, int *val)
{
    ...
    *val = soc_reg64_field32_get(unit, XLMAC_TX_CTRLr, reg_val, AVERAGE_IPGf);
...
int xlmac_clear_rx_lss_status_set(int unit, soc_port_t port, int lcl_fault, int rmt_fault)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_CLEAR_RX_LSS_STATUSr(unit, port, &reg_val));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_CLEAR_RX_LSS_STATUSr(unit, port, reg_val));
...
xlmac_duplex_set(int unit, soc_port_t port, int duplex)
{
    LOG_VERBOSE(BSL_LS_SOC_10G,
                (BSL_META_U(unit,
                 ...
                 unit, SOC_PORT_NAME(unit, port),
                 duplex ? "Full" : "Half"));
    return (duplex) ? SOC_E_NONE: SOC_E_PARAM;
}
...
int xlmac_frame_spacing_stretch_set(int unit, int port, int spacing)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_TX_CTRLr(unit, port, &mac_ctrl));
...
int xlmac_lag_failover_loopback_set(int unit, int port, int val)
{
    ...
    _SOC_IF_ERR_EXIT(READ_XLMAC_LAG_FAILOVER_STATUSr(unit, port, &rval));

    soc_reg64_field32_set (unit, XLMAC_LAG_FAILOVER_STATUSr, &rval, 
                                            LAG_FAILOVER_LOOPBACKf, val);
    _SOC_IF_ERR_EXIT(WRITE_XLMAC_LAG_FAILOVER_STATUSr(unit, port, rval));
...
int xlmac_drain_cell_get (int unit, int port, int *rx_pfc_en, 
                          int *llfc_en, int *rx_pause, int *tx_pause)
{   
    uint64 rval;

    SOC_IF_ERROR_RETURN(READ_XLMAC_PFC_CTRLr(unit, port, &rval));
    *rx_pfc_en = soc_reg64_field32_get(unit, XLMAC_PFC_CTRLr, 
                                   rval, RX_PFC_ENf);

    SOC_IF_ERROR_RETURN(READ_XLMAC_LLFC_CTRLr(unit, port, &rval));
    *llfc_en = soc_reg64_field32_get(unit, XLMAC_LLFC_CTRLr, 
                                 rval, RX_LLFC_ENf);

    SOC_IF_ERROR_RETURN(READ_XLMAC_PAUSE_CTRLr(unit, port , &rval));
    *rx_pause  = soc_reg64_field32_get (unit, XLMAC_PAUSE_CTRLr,
                                                 rval, RX_PAUSE_ENf);
    *tx_pause  = soc_reg64_field32_get (unit, XLMAC_PAUSE_CTRLr,
                                                 rval, TX_PAUSE_ENf);

    return (0);
}    

int xlmac_drain_cell_stop (int unit, int port, int rx_pfc_en, 
                           int llfc_en, int rx_pause, int tx_pause)
{   
    uint64 rval;
    ...
    soc_reg64_field32_set(unit, XLMAC_TX_CTRLr, &rval, DISCARDf, 0);
    soc_reg64_field32_set(unit, XLMAC_TX_CTRLr, &rval, EP_DISCARDf, 0);
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_PAUSE_CTRLr(unit, port , rval));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_LLFC_CTRLr(unit, port, rval));
...
{   
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_TX_CTRLr(unit, port, &rval));
    soc_reg64_field32_set(unit, XLMAC_TX_CTRLr, &rval, DISCARDf, 1);
    soc_reg64_field32_set(unit, XLMAC_TX_CTRLr, &rval, EP_DISCARDf, 1);
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_TX_CTRLr(unit, port, rval));
...
int xlmac_txfifo_cell_cnt_get (int unit, int port, uint32* fval)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_TXFIFO_CELL_CNTr(unit, port, &rval));
...
int xlmac_e2ecc_hdr_get (int unit, int port, uint32_t *words)
{
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_E2ECC_MODULE_HDR_0r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_E2ECC_MODULE_HDR_1r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_E2ECC_DATA_HDR_0r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_XLMAC_E2ECC_DATA_HDR_1r(unit, port, &rval64));
...
int xlmac_e2ecc_hdr_set (int unit, int port, uint32_t *words)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_E2ECC_MODULE_HDR_0r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_E2ECC_MODULE_HDR_1r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_E2ECC_DATA_HDR_0r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XLMAC_E2ECC_DATA_HDR_1r(unit, port, rval64));
