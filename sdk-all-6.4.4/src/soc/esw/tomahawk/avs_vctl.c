...
#define MUX_0 0
th_avs_vparams_t
th_avs_vs_config = {1 << MUX_0, I2C_ADC_0, I2C_ADC_CH0, "pwctrl2"};
#endif

#if defined(_SOC_AVS_TH_VOLTAGE_CALIBRATE_USE_I2C) || defined(_SOC_AVS_TH_VOLTAGE_SET_USE_I2C)
static dac_calibrate_t
th_avs_dac_params = {0,"Core",-1,1,0,0,ADP4000_MIN_DACVAL,ADP4000_MAX_DACVAL-0x3,ADP4000_MID_DACVAL,1};
#endif

#if defined(_SOC_AVS_TH_VOLTAGE_CALIBRATE_USE_I2C)
STATIC soc_error_t
_soc_th_i2c_avs_voltage_calibrate(int unit)
{
    int adc, dac, rv, mux;
    i2c_adc_t  adc_data;
    ...
    if((rv = soc_i2c_device(unit, mux)->driver->
                write(unit, mux, 0, &th_avs_vs_config.mux, 1)) < 0) {
        LOG_ERROR(BSL_LS_SOC_AVS,
            (BSL_META_U(unit,
    ...
    /* Open ADC device */
    if((adc = soc_i2c_devopen(unit, th_avs_vs_config.adc, 0, 0)) < 0) {
        LOG_ERROR(BSL_LS_SOC_AVS,
            (BSL_META_U(unit,
                        "Could not open %s:%s\n"),
                        th_avs_vs_config.adc, soc_errmsg(adc)));
        return SOC_E_FAIL;
    }

    /* Open the correct DAC */
    if ( (dac = soc_i2c_devopen(unit, th_avs_vs_config.dac, 0, 0)) < 0) {
        LOG_ERROR(BSL_LS_SOC_AVS,
            (BSL_META_U(unit,
                        "Could not open %s:%s\n"),
                        th_avs_vs_config.dac, soc_errmsg(dac)));
        return SOC_E_FAIL;
    }

    /* Set DAC calibration table */
    if ((rv = soc_i2c_device(unit, dac)->driver->ioctl(unit, dac,
                I2C_DAC_IOC_SET_CALTAB, (void*)&th_avs_dac_params, 0) < 0)) {
        LOG_ERROR(BSL_LS_SOC_AVS,
            (BSL_META_U(unit,
    ...
    if((rv = soc_i2c_device(unit, adc)->driver->ioctl(unit, adc,
                I2C_ADC_QUERY_CHANNEL,&adc_data, th_avs_vs_config.chan)) < 0) {
        LOG_ERROR(BSL_LS_SOC_AVS,
            (BSL_META_U(unit,
...
STATIC soc_error_t
_soc_th_i2c_avs_voltage_get(int unit, uint32 *val)
{
    int adc, rv;
    i2c_adc_t  adc_data;
...
STATIC soc_error_t
_soc_th_i2c_avs_voltage_set(int unit, uint32 val)
{
    int rv, dac, mux;
#ifdef COMPILER_HAS_DOUBLE
...
{
#if defined(_SOC_AVS_TH_VOLTAGE_CALIBRATE_USE_I2C)
   return _soc_th_i2c_avs_voltage_calibrate(unit);
#endif
...
{
#if defined(_SOC_AVS_TH_VOLTAGE_GET_USE_I2C)
   return _soc_th_i2c_avs_voltage_get(unit, voltage);
#endif
...
{
#if defined(_SOC_AVS_TH_VOLTAGE_SET_USE_I2C)
   return _soc_th_i2c_avs_voltage_set(unit, voltage);
#endif
