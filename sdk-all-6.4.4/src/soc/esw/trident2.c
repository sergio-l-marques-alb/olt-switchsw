...
_soc_trident2_get_prio_map(int unit, int port, uint16 *pri_mask)
{
    ...
    *pri_mask = 0;
    ...
    (void)soc_property_port_get_csv(unit, port, name,
...
_soc_trident2_tdm_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        _chip_pkg.clk_freq = (si -> frequency > 760)?(760):(si->frequency);
                    ...
                    for (j = 0; ipep_fifo_parity[j].mem != INVALIDm; j++) {
                        if (ipep_fifo_parity[j].mem == mem) {
                            soc_ser_create_test_data(unit, tmp_entry, field_data,
                                                     ...
                                                     index, &test_data);
                            found_mem = TRUE;
                            if (SOC_REG_IS_VALID(unit, ipep_fifo_parity[j].ecc_1b_report_en_reg) &&
                                (INVALIDf != ipep_fifo_parity[j].ecc_1b_report_en_fld)) {
