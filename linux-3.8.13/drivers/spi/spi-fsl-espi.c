...
#define PTIN_SPI	
#ifdef PTIN_SPI    
#define SPI_CLK_DELAY 0xFF
#define SPI_CLK_DELAY_DISABLE 0xFE
    if (spi->mode == SPI_CLK_DELAY)
{		cs->hw_mode=hw_mode; /*neste modo so alteramos o clock*/
printk("spi->mode  SPI_CLK_DELAY enable  \n " );
		cs->hw_mode |= (CSMODE_BEF(1) | CSMODE_AFT(1));
		fsl_espi_change_mode(spi);
return 0;
}
    if (spi->mode == SPI_CLK_DELAY_DISABLE )
{		cs->hw_mode=hw_mode; /*neste modo so alteramos o clock*/
printk("spi->mode  SPI_CLK_DELAY disable  \n " );
		cs->hw_mode &= ~(CSMODE_BEF(1) | CSMODE_AFT(1));
		fsl_espi_change_mode(spi);
return 0;
}
