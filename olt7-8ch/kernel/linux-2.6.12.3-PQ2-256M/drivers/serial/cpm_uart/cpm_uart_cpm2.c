...
void smc1_lineif(struct uart_cpm_port *pinfo)
{
	 ...
	 *(short *)&cpm2_immr->im_dprambase[PROFF_SMC1_BASE]= PROFF_SMC1;
...
void smc2_lineif(struct uart_cpm_port *pinfo)
{
	...
	{
	//	volatile smc_uart_t * smcup= (volatile smc_uart_t *) & cpm2_immr->im_dprambase[PROFF_SMC2];
//	printk("%s @ %d\n",__FUNCTION__,__LINE__);
	/* SMC2 is on PC15 tx, PD4 rx */
		io->iop_ppara |= 0x00C00000;
        	io->iop_psora |= 0x00C00000; 
        /*	io->iop_pdira &=~0x00C00000;  */
	...
	/* desabilitar a função "MAX_IDL" */
  //      smcup->smc_maxidl= 0;     /* Maximum idle characters */
/* clear BRKLEN e BRKEC */
//	smcup->smc_brklen= smcup->smc_brkec= 0;
  //      smcup->smc_brklen= 0;     /* Last received break length */
  //      smcup->smc_brkec= 0;      /* rcv'd break condition counter */
/* set BRKCR to 0x0001; if a STOP TRANSMIT COMMAND is issue, one break character is sent. */
  //      smcup->smc_brkcr= 0x0001;      /* xmt break count register */
	}
	*(short *)&cpm2_immr->im_dprambase[PROFF_SMC2_BASE]= PROFF_SMC2;

	/* Wire BRG2 to SMC2 */
//	#define JAY_BRG8 1
	cpm2_immr->im_cpmux.cmx_smr &= 0xf0;
//	cpm2_immr->im_cpmux.cmx_smr |= JAY_BRG8;
...
int cpm_uart_allocbuf(struct uart_cpm_port *pinfo, unsigned int is_con)
{
	...
	if(1==pinfo->port.line) printk("%s @ %d|dp_offset:0x%X|dp_mem:0x%X|mem_addr:0x%X|mem_addr_bus:0x%X\n",__FUNCTION__,__LINE__,dp_offset,dp_mem,mem_addr,dma_addr);/*??jpm*/
	return 0;
}
