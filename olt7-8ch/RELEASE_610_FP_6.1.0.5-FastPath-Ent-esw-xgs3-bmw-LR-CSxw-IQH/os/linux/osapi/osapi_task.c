...
L7_RC_t osapiTaskPrioritySet(L7_int32 TaskID, L7_int32 Priority)
{
  ...
  old_prio = osapiTask->osapi_prio;

  osapiTask->osapi_prio = Priority;

  pthread_attr_getschedpolicy(&(osapiTask->attr), &policy);

  osapiTask->pthread_prio = osapi_scale_prio(Priority, policy);

  if (osapiTask->cur_prio == old_prio)
  {

    osapiTask->cur_prio = osapiTask->pthread_prio;

    pthread_attr_getschedparam(&(osapiTask->attr), &(osapiTask->param));

    osapiTask->param.sched_priority = osapiTask->cur_prio;

    pthread_attr_setschedparam(&(osapiTask->attr), &(osapiTask->param));
...
#define LVL7_CPU_UTIL_TICKSPERSECOND 100
/*********************************************************************
...
* @returns  none
*
* @comments Could not get floating point to work correctly
*           so all numbers are increased by 1000 to use integer arithetic.
*
* @end
*
*********************************************************************/
...
{
  char taskStatFileName[40];
  osapi_task_t *curTask;
  int           fpPid = getpid();
  unsigned int  curCpuTime, cpuTime;
  ...
  L7_uint32 LengthInSeconds, index;
  L7_int32 i, PercentSum, ExpectedTotalTicks;
  L7_BOOL insertFlag = L7_FALSE;


  lvl7percentutiltbl_idx = 0;

  LengthInSeconds    = LVL7_CPU_UTIL_SECONDS;
  ExpectedTotalTicks = LVL7_CPU_UTIL_SECONDS * LVL7_CPU_UTIL_TICKSPERSECOND;
  lvl7taskSema = osapiSemaBCreate(OSAPI_SEM_Q_FIFO, OSAPI_SEM_FULL);

  /* initialize list of tasks */
  memset(&bigIdList, 0x00, sizeof(bigIdList));
  ...
  for (curTask = task_list_head; curTask != NULL;
      curTask = curTask->chain_next)
  {
    bigIdList[bigListTotal].taskId = (int)curTask;
    bigListTotal++;
  }
  pthread_cleanup_pop(1);

  /*Add semaphore protection for sorts 72053 FCJ */
  ...
  do
  {
    osapiSleep(LVL7_CPU_UTIL_SECONDS);
    ...
    elapsed_time = (t.tv_sec - oldtime.tv_sec)   * LVL7_CPU_UTIL_TICKSPERSECOND
                 + (t.tv_nsec - oldtime.tv_nsec) * LVL7_CPU_UTIL_TICKSPERSECOND
                 / 100000000.0;
        ...
        curTask = curTask->chain_next)
    {
      ...
      if (cpuTime)
      {
        percent = (cpuTime * LVL7_CPU_UTIL_SCALAR * 100) / elapsed_time;
        if (percent >= LVL7_CPU_UTIL_SCALAR * 100)
           percent = (LVL7_CPU_UTIL_SCALAR * 100) - 1; /* 99.99% */
        PercentSum += percent;
        curTask->prevCpuTime = curCpuTime;
      }

      insertFlag = L7_FALSE;
      if (bigIdListFind((int)curTask, &index) == L7_FALSE)
      {
        index = bigIdListInsert((int)curTask);
        insertFlag = L7_TRUE;
      }

      bigIdList[index].lvl7percentutiltbl[lvl7percentutiltbl_idx] = percent;
      bigIdList[index].lvl7percentutiltbl_idx++;
      if (bigIdList[index].lvl7percentutiltbl_idx > (((LVL7_CPU_UTIL_MINUTES * 60) / LVL7_CPU_UTIL_SECONDS) - 1))
      {
        bigIdList[index].lvl7percentutiltbl_idx = 0;
      }
    ...
    { /* fixme: this code should calculate how long task has been idle */
      if (( osapiTaskIdVerify( bigIdList[i].taskId) != L7_SUCCESS) &&
          (lvl7percentutiltbl_idx > 0) &&
          (bigIdList[i].lvl7percentutiltbl[lvl7percentutiltbl_idx-1] == 0))
      {
        bigIdListDelete(bigIdList[i].taskId);
      }
      else if (osapiTaskIdVerify( bigIdList[i].taskId) != L7_SUCCESS)
      { /* store a zero for this 5 second slot */
        bigIdList[i].lvl7percentutiltbl[lvl7percentutiltbl_idx] = 0;
      }
    }

    lvl7percentutiltbl[lvl7percentutiltbl_idx] = PercentSum; 
    lvl7percentutiltbl_idx++;
    if (lvl7percentutiltbl_idx > (((LVL7_CPU_UTIL_MINUTES * 60) / LVL7_CPU_UTIL_SECONDS) - 1))
    {
      lvl7percentutiltbl_idx = 0;
    }
    osapiSemaGive(lvl7taskSema);
