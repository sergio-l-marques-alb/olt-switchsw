...
#define OSAPI_STACK_TRACE_LOG "sysStackTrace.dump"
...
#define osapiMallocArraySize 5000
static osapiMemAllocType *lcp = NULL;
#endif
...
*
*************************************************************************/
void osapiDebugMallocSummary(L7_COMPONENT_IDS_t compId)
{
  ...
  bzero(componentName, sizeof(componentName));

  printf ("Component                    CurrentAllocated          MaxAllocated \n");

  if(compId == 0)
  {
    for(compId = 1; compId < L7_LAST_COMPONENT_ID; compId++)
    {
      memset(componentName, L7_EOS ,L7_CLI_MAX_STRING_LENGTH);
...
void osapiDebugMemoryCheck (void)
{
  ...
  L7_uint32 max_memory;
  pthread_t self;
  L7_uint32 min_memory;
  L7_uint32 mem_addr;
  ...
  /* HACK --- Use system function to get memory size.
  */
  min_memory = 0;
  max_memory = 0xF0000000;
...

  {
    ...
    while (mem_alloc != 0)
    {
      ...
      if ((mem_addr > max_memory) || (mem_addr < min_memory))
      {
      ...
      if (((L7_uint32) mem_alloc->next > max_memory) ||
          ((L7_uint32) mem_alloc->next < min_memory))
      {
...
void osapiDebugMemoryStats (void)
{
  ...
  for (i = 0; i < num_tasks; i++)
  {
    ...
    printf("Task id: 0x%x -- Task Name: %32s -- Blocks: %6d -- Size: %d\n",
              task_id_table[i],
              task_name,
              task_id_count[i],
              task_id_total[i]);
...
{
  pthread_t self;
  L7_uint32 numAllocEntries = osapiMallocArraySize;
  ...
  if( (state == 1) && (lcp == NULL))
    lcp = malloc(numAllocEntries * sizeof(osapiMemAllocType));

  /* Disable osapiDebugMallocDetail() and free allocated memory */
  else if( (state == 0) && (lcp != NULL))
  {
    free(lcp);
    lcp = NULL;
  }
...
/**************************************************************************
*
* @purpose  Display information about allocated memory blocks -- FCJ & RJM.
*
...
* @param    filePtr - If 0 is specified, then allocations are printed out to stdout
*                     else the allocations are written to the specified file pointer.
*
* @returns  none
*
						   ...
						   FILE *filePtr)
{
  ...
  if (lcp == L7_NULL) 
  {
    fprintf(fp, "RealTime osapiDebugMallocDetail() is not enabled !\n");
    fprintf(fp, "Use osapiDebugMallocDetailEnable(1 or 0) to enable and disable.\n");
  }
  else
  {
    self = pthread_self();
    ...
    while (mem_alloc != 0)
    {
      ...
      if (((comp_id == 0) || (mem_alloc->component_id == comp_id)) &&
          ...
          (mem_alloc->time_stamp > time_stamp))
      {
        if (cnt < osapiMallocArraySize)
          memcpy(&lcp[cnt++], mem_alloc, sizeof(osapiMemAllocType));
...
void osapiFree(L7_COMPONENT_IDS_t compId, void * memory)
{
  ...
  */
  ...
  osapiMallocMemUsage[compId].currentMemUsage = (osapiMallocMemUsage[compId].currentMemUsage) - (mem_alloc->length);
                         ...
                         L7_uint32 line_number )
{
  ...
  if ( address != NULL )
  {
    ...
    ** if required
    **/
    osapiMallocMemUsage[compId].currentMemUsage = osapiMallocMemUsage[compId].currentMemUsage + numberofbytes;

    if(osapiMallocMemUsage[compId].maxMemUsage < osapiMallocMemUsage[compId].currentMemUsage)
    {
      osapiMallocMemUsage[compId].maxMemUsage = osapiMallocMemUsage[compId].currentMemUsage;
    }
                         ...
                         L7_uint32 line_number)
{
  ...
  if ( address != NULL )
  {
    ...
    ** if required
    **/
    osapiMallocMemUsage[compId].currentMemUsage = osapiMallocMemUsage[compId].currentMemUsage + numberofbytes-old_size;
...
L7_RC_t osapi_nvstore_save(void)
{
  ...
  speed_t speed;
  int baudrate;
#if !defined(BAUDRATE_USE_BOOTENV)
  FILE *baudrate_cfg;
#else
  char cmd[80];
#endif
  ...
  if (system(cmd) == -1) {
    LOG_MSG("Could not save baudrate to boot environment...");
  }
#endif
#endif
  return(L7_SUCCESS);
...
void osapi_nvstore_register(void)
{
  ...
  if (nvStoreRegister(nvStoreList) != L7_SUCCESS) {
    LOG_MSG("nvStoreRegister failed!");
  }
...
*
*************************************************************************/
L7_RC_t osapiBaudRateChange(L7_uint32 rate)
{
#if (L7_SERIAL_COM_ATTR)
  struct termios tio;
  speed_t speed;
...
/**
 * ----------------------------------------------------------------------
 * Function name: osapiTimeMillisecondsGet64
 * ----------------------------------------------------------------------
 * Description:     
 *   
 * ----------------------------------------------------------------------
...
/**
 * ----------------------------------------------------------------------
 * Function name: osapiTimeMillisecondsGet
 * ----------------------------------------------------------------------
 * Description:     
 *   
 * ----------------------------------------------------------------------
...
/**
 * ----------------------------------------------------------------------
 * Function name: osapiTimeMillisecondsDiff
 * ----------------------------------------------------------------------
 * Description:     
 *   
 * ----------------------------------------------------------------------
...
struct timespec   tp;
 ...
 tp.tv_sec  = 0;
 tp.tv_nsec = 0;
 clock_settime(CLOCK_MONOTONIC /*CLOCK_REALTIME*/, &tp);  /* PTin modified: Clock */
 localClockError=0.0;
}
...
* @end
*********************************************************************/
L7_double64 osapiNTPTimeGet()
{
  L7_clocktime current;
  L7_int32     signedIntSec, signedIntNan;
  osapiClockTimeRaw(&current);

  signedIntSec = (L7_int32)current.seconds;
  signedIntNan = (L7_int32)current.nanoseconds;

  /* Jan 1970 in NTP seconds + UTC seconds till uptime + current uptime = NTC now */
  return JAN_1970_OFFSET + (L7_double64)signedIntSec + ((L7_double64)(signedIntNan) / 1000000000.0) + localClockError;
}


/**********************************************************************
* @purpose  Update the local clock.
*
* @param    start @b{(input)} time when we received the NTP packet
* @param    offset @b{(input)} the correction to be applied.
*
* @returns  L7_SUCCESS if the correction was applied.
*           L7_ALREADY_CONFIGURED if correction not applied.
*
* @notes    This routine calculates the local correction to apply
*           using the start time, the current time, and the offset
*           (correction).
*           If this time is greater than the SNTP_MIN_CORRECTION,
*           the correction is applied, otherwise not.
*           This serves to minimize the number of corrections applied
*           to the clock to perhaps once a day for most computer clocks.
*
* @end
*********************************************************************/

L7_RC_t osapiNTPTimeSet(L7_double64 start, L7_double64 offset)
{

  L7_double64 timeNow = osapiNTPTimeGet();

  /** Our processing time is the difference between the start
  * time and time now.
  */
  L7_double64 diff = (SNTP_CLOCK_PHI * (timeNow - start)) + offset;

  if (osapiFabs(diff) >= SNTP_MIN_CORRECTION)
  {
    localClockError += diff;
...
L7_uchar8 osapiGetChoice()
{
  ...
  do
  {
    ...
    rc = read(bspapiConsoleFdGet(), string, sizeof(string));
    if ( rc  <= 0 )
    {
      continue;
    }
...
void osapiInputTask(void)
{
  ...
  while (L7_TRUE)
  {
    ...
    {
      inputSize = read(STDIN_FILENO, &inputBuffer[0], sizeof(inputBuffer));
...
L7_RC_t osapiCpuUtilizationStringGet(L7_char8 *cpuUtilBuffer, L7_uint32 bufferSize)
{
  ...
  n += sysapiTaskUtilPrint(&cpuUtilBuffer[n], bufferSize - n, L7_TRUE);
  return rc;
}
...
{
  L7_RC_t   rc = L7_FAILURE;
  L7_uchar8 command[128] = {0};
  L7_uchar8 memFilename[] = EXEC_PATH"meminfo.out";
...
#define MAX_FILE_LENGTH 624
  ...
  read(fd, buffer, MAX_FILE_LENGTH);
  close(fd);
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    osapiSnprintf(command,sizeof(command),"du -ks / > %s", memFilename);
    system(command);

    fp = fopen(memFilename,"r");
    if (fp != L7_NULL) 
    {
      fscanf(fp,"%u", &fsTotal); 
      fclose(fp);
    ...
    /*    from total. (leaving rootfs, tmpfs, & procfs)    */
    osapiSnprintf(command,sizeof(command),"du -ks %s > %s",
                  CONFIG_PATH, memFilename);
    system(command);

    fp = fopen(memFilename,"r");
    if (fp != L7_NULL) 
    {
      fscanf(fp,"%u", &jffsTotal); 
      fclose(fp);
    }
...
* @end
*********************************************************************/
void osapiLogStackTraceFlash(L7_char8 * format, ...)
{
  ...
  if (osapiStackDumpLogMode == L7_TRUE) 
  {
	SYSAPI_PRINTF(SYSAPI_LOGGING_ALWAYS,
				  "Saving stack trace dump of all the tasks...\n");
    fp = fopen(OSAPI_STACK_TRACE_LOG, "w");
    if (fp == L7_NULL) 
    {
      return;
    }
    ...
    fprintf(fp, "Stack trace dump created %s\n\n",
            ctime((void *)&currTime));
