...
/* Trace Flags for L3 IPM */
#define IPM_TRACE_SECONDARY 0x10
L7_uint32 ipmTraceFlags;
...
void l3intfAddRoute(L7_uint32 network, L7_uint32 gateway, L7_uint32 netmask) {
   ...
   if ((ioctl(sock, SIOCADDRT, &rtEntry) < 0) && (errno != EEXIST)) {
      ...
      L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID, 
             "Error adding or deleting a default gateway in the kernel’s routing "
             "table (the function is really osapiRawMRouteAdd()/Delete())\n");
      close(sock);
      return;
                    ...
                    L7_uchar8 *pVmac)
{
	...
	{
		LOG_MSG("Failed to Retrieve Interface Flags\n");
	}
...
void l3intfVrrpToMaster(L7_uint32 intIfNum)
{
   ...
   do {
        ...
        {
           LOG_MSG("Failed to retrieve parent interface usp\n");
        }
...
ipmRouterIfInitPhase2Process(L7_uint32 max_interfaces)
{
    ...
    if (rc != L7_SUCCESS)
    {
        LOG_MSG("Failed to register with Sysnet PDU Intercept (AF_INET/SYSNET_INET_IN)\n");
    }
    sysnetPduIntercept.addressFamily = AF_INET;
    ...
    if (rc != L7_SUCCESS)
    {
        LOG_MSG("Failed to register with Sysnet PDU Intercept (AF_INET/SYSNET_INET_ARP_IN)\n");
    }
#else
                          ...
                          L7_uchar8 * mac )
{
   ...
   {
	  LOG_MSG("ipmRouterIfCreate: RouterIntfInitDone == L7_FALSE\n");
      return L7_ERROR;
   }
 
   if ((index < 1) || (index > MaxRouterIf))
   {
	  LOG_MSG("rtrIntfNum %d: invalid\n", index);
      return L7_ERROR;
   }
   ...
   {
	   LOG_MSG("ip 0x%08x is an invalid interface address\n", ip);
	   return L7_ERROR;
   }
...
L7_RC_t ipmRouterIfRemove(L7_uint32 intIfNum)
{
   ...
   if (RouterIntfInitDone == L7_FALSE)
   {
	  LOG_MSG("ipmRouterIfRemove: RouterIntfInitDone == L7_FALSE\n");
      return L7_ERROR;
   }
 
   if (_ipMapIntIfNumToRtrIntf(intIfNum, &index) != L7_SUCCESS)
   {
	  LOG_MSG("intIfNum %d: Could not retrieve rtrIntf\n", intIfNum);
      return L7_ERROR;
   }
   ...
   if (osapiIfNameStringGet(intIfNum, ifr.ifr_name,IFNAMSIZ) != L7_SUCCESS) {
     ...
     LOG_MSG("ipmRouterIfRemove: could not get interface name intIfNum %d\n",
                           intIfNum);
     return (0);
   ...
   {
	  LOG_MSG("rtrIntfNum %d: Port state invalid\n", index);
      return L7_FAILURE;
   }

   if (osapiIfNameStringGet(intIfNum, ifr.ifr_name, IFNAMSIZ) != L7_SUCCESS)
   {
     LOG_MSG("Unable to get interface name for interface %u\n", intIfNum);
     return L7_ERROR; 
   }
   ...
   {
     LOG_MSG("%s: SIOCGIFADDR %s", __FUNCTION__, strerror(errno));
   }
   else
   {
   ...
   if(ipstkIfAddrDel((L7_uchar8 *)ifr.ifr_name, ipa) != L7_SUCCESS)
     { /* delete IF addr */
       LOG_MSG("%s: SIOCDIFADDR %s", __FUNCTION__, strerror(errno));
     }
   }
   ifInfo->port_state = L3INTF_INVALID;
   ifInfo->ip_addr[0] = 0;
   ...
   do
   {
    ...
    if(ifInfo->tap_fd < 0)
    {
      L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID, 
             ...
             " The /dev/tap file is missing, or, if not using the LVL7 reference kernel binary, "
     ...
     *the ifr strcutre (with the name already filled out)
     *to create the corresponding net interface
     */
    ifr.ifr_flags = IFF_TAP | IFF_NO_PI;
    if(ioctl(ifInfo->tap_fd,TUNSETIFF,&ifr) < 0)
    {
     LOG_MSG("Unable to create corresponding dtl net ifc\n");
     close(ifInfo->tap_fd);
     ifInfo->tap_fd = -1;
     rc = L7_ERROR;
     break;
                                    ...
                                    L7_uint32 netmask, L7_uint32 index)
{
	...
	if (RouterIntfInitDone == L7_FALSE)
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrAdd: RouterIntfInitDone == L7_FALSE\n");
		}
		LOG_MSG("ipmRouterIfSecondaryAddrAdd: RouterIntfInitDone == L7_FALSE\n");
		return L7_ERROR;
	}
	...
	if (_ipMapIntIfNumToRtrIntf(intIfNum, &rtrIfNum) != L7_SUCCESS)
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrAdd: intIfNum %d: ipMapIntIfNumToRtrIntf failed\n",
				   intIfNum);
		}
		LOG_MSG("intIfNum %d: Could not retrieve rtrIntf\n", intIfNum);
		return L7_ERROR;
	...
	if (ipmTraceFlags & IPM_TRACE_SECONDARY)
	{
		printf("ipmRouterIfSecondaryAddrAdd: rtrIfNum = %d\n", rtrIfNum);
	}
	...
	if ((rtrIfNum < 1) || (rtrIfNum > MaxRouterIf)) 
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrAdd: rtrIfNum %d invalid\n", rtrIfNum);
		}
		LOG_MSG("rtrIfNum %d invalid\n", rtrIfNum);
		return L7_ERROR;
	}
	...
	if ((ip == 0) || (ip >= 0xe0000000))
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrAdd: ip 0x%08x invalid\n", ip);
		}
		LOG_MSG("ip 0x%08x invalid interface address\n", ip);
		return L7_ERROR;
	}
	...
	do
	{
		...
		{
			LOG_MSG("intIfNum %d: could not get interface name\n", intIfNum);
			return L7_FAILURE;
		}
		...
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrAdd: intIfNum %d, ifname %s\n",
				   intIfNum, ifr.ifr_name);
		}
    if(ipstkIfAddrSet(ifr.ifr_name,ip) != L7_SUCCESS)
    {
    L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID,
                                       ...
                                       L7_uint32 netmask, L7_uint32 index) 
{
	...
	if (RouterIntfInitDone == L7_FALSE) 
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: RouterIntfInitDone == L7_FALSE\n");
		}
		LOG_MSG("ipmRouterIfSecondaryAddrAdd: RouterIntfInitDone == L7_FALSE\n");
		return L7_ERROR;
	}
	...
	if (_ipMapIntIfNumToRtrIntf(intIfNum, &rtrIfNum) != L7_SUCCESS) 
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: intIfNum %d: ipMapIntIfNumToRtrIntf failed\n",
				   intIfNum);
		}
		LOG_MSG("intIfNum %d: Could not retrieve rtrIntf\n", intIfNum);
		return L7_ERROR;
	...
	if (ipmTraceFlags & IPM_TRACE_SECONDARY)
	{
		printf("ipmRouterIfSecondaryAddrDelete: rtrIfNum = %d\n", rtrIfNum);
	}
	...
	if ((rtrIfNum < 1) || (rtrIfNum > MaxRouterIf)) 
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: rtrIfNum %d invalid\n", rtrIfNum);
		}
		LOG_MSG("rtrIfNum %d invalid\n", rtrIfNum);
		return L7_ERROR;
	}
	...
	if ((ip == 0) || (ip >= 0xe0000000))
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: ip 0x%08x invalid\n", ip);
		}
		LOG_MSG("ip 0x%08x invalid interface address\n", ip);
		return L7_ERROR;
	}
	...
	if (ifInfo->port_state == L3INTF_INVALID) 
	{
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: rtrIfNum %d: interface state invalid\n",
				   rtrIfNum);
		}
		LOG_MSG("rtrIfNum %d: Interface state invalid\n", rtrIfNum);
		return L7_FAILURE;
	}
	...
	if (ipmTraceFlags & IPM_TRACE_SECONDARY)
	{
		printf("ipmRouterIfSecondaryAddrDelete: deleting %s\n", ifr.ifr_name);
	}
	...
	if(ipstkIfAddrDel(ifr.ifr_name, ip) != L7_SUCCESS)
	{
		...
		if (ipmTraceFlags & IPM_TRACE_SECONDARY)
		{
			printf("ipmRouterIfSecondaryAddrDelete: could not get flags for %s\n", ifr.ifr_name);
		}
		LOG_MSG("Failed to Retrieve Interface Flags for %s\n", ifr.ifr_name);
	}
...
L7_RC_t ipmRouterIfMetricGet(L7_uint32 intIfNum, L7_uint32 *metric)
{
   ...
   if (osapiIfNameStringGet(intIfNum, ifr.ifr_name,IFNAMSIZ) != L7_SUCCESS) {
	  ...
	  LOG_MSG("ipmRouterIfMetricGet: intIfNum %d: could not get interface name\n",
				   intIfNum);
      return (L7_FAILURE);
   ...
   {
     LOG_MSG("ipmRouterIfMetricGet: intIfNum %d, %s link not active\n",
			 intIfNum, ifr.ifr_name);
     return(L7_ERROR);
   }
   ...
   if(ipstkIfMetricGet(ifr.ifr_name,metric) != L7_SUCCESS) {
      LOG_MSG("intIfNum %d, %s: Failed to Retrieve Interface Metric\n",
			  intIfNum, ifr.ifr_name);
      return (L7_FAILURE);
...
/*********************************************************************
*
* @purpose  Set the IPM trace flags level.
*
* @param    void
*
* @returns  L7_SUCCESS or L7_FAILURE
...
* @end
*********************************************************************/
void ipmTraceFlagsSet(L7_uint32 traceFlags)
{
    ipmTraceFlags = traceFlags;
    return;
}
...
ipmRouterIfDrvUp ( L7_uint32 intIfNum)
{
   ...
   if(nimGetIntfType(intIfNum, &itype) != L7_SUCCESS){
       LOG_MSG("intIfNum %d: unable to get interface type\n", intIfNum);
       return L7_ERROR; 
   }
   switch(itype){
   ...
   case L7_LOOPBACK_INTF:
     ...
     if (ioctl(sock, SIOCSIFHWADDR, &ifr) == -1) {
       ...
       L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID,
           "intIfNum %d, %s: Failed to Set Interface Hardware Address."
           " Trouble adding VRRP IP or MAC address(es) to a Linux network interface.",
               intIfNum, ifr.ifr_name);
       rc = L7_ERROR;
     }
   ...
   sprintf(procfs_message, 
           "/proc/sys/net/ipv6/conf/%s/accept_ra", &ifr.ifr_name[0]);
   osapi_proc_set(procfs_message, "0");
   sprintf(procfs_message,
           "/proc/sys/net/ipv6/conf/%s/forwarding", &ifr.ifr_name[0]);
