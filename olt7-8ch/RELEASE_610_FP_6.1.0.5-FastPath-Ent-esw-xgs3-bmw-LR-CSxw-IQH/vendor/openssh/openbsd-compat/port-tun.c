...
#else
        osapiSnprintf(name, sizeof(name), "/dev/%s%d", tunbase, tun);
#endif /* L7_SSHD */
        fd = open(name, O_RDWR);
...
#else
            osapiSnprintf(name, sizeof(name), "/dev/%s%d",
#endif /* L7_SSHD */
                tunbase, tun);
#ifndef L7_SSHD
            if ((fd = open(name, O_RDWR)) >= 0)
#else
            if ((fd = osapiFsOpen(name)) >= 0)
#endif /* L7_SSHD */
                break;
        }
    } else {
...
#else
    osapiSnprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s%d", tunbase, tun);
#endif /* L7_SSHD */
...
#else
int
sys_tun_infilter(int cn, struct Channel *c, char *buf, int len)
#endif /* L7_SSHD */
{
...
#else
u_char *
sys_tun_outfilter(int cn, struct Channel *c, u_char **data, u_int *dlen)
#endif /* L7_SSHD */
{
    u_char *buf;
    u_int32_t *af;
