...
#else
    osapiSnprintf(buf, sizeof(buf), "bad-auth-msg-%d", type);
#endif /* L7_SSHD */
    return (buf);
}
...
auth1_process_password(Authctxt *authctxt, char *info, size_t infolen)
{
    ...
    packet_check_eom();
#else
    password = packet_get_string(authctxt->cn, &dlen);
    packet_check_eom(authctxt->cn);

    debugl7(SYSLOG_LEVEL_DEBUG1, "auth1_process_password: user %s, passwd \"%s\"\n", authctxt->user, password);
#endif /* L7_SSHD */
...
#else
    if ((n = BN_new()) == NULL)
        fatal_cn(authctxt->cn, "do_authloop: BN_new failed");
    packet_get_bignum(authctxt->cn, n);
    packet_check_eom(authctxt->cn);
#endif /* L7_SSHD */
...
#else
    client_user = packet_get_string(authctxt->cn, &ulen);

    /* Get the client host key. */
    client_host_key = key_new(authctxt->cn, KEY_RSA1);
    bits = packet_get_int(authctxt->cn);
    packet_get_bignum(authctxt->cn, client_host_key->rsa->e);
    packet_get_bignum(authctxt->cn, client_host_key->rsa->n);
#endif /* L7_SSHD */
...
#else
    osapiSnprintf(info, infolen, " ruser %.100s", client_user);
#endif /* L7_SSHD */
...
auth1_process_tis_challenge(Authctxt *authctxt, char *info, size_t infolen)
{
    ...
    packet_write_wait();
#else
    packet_start(authctxt->cn, SSH_SMSG_AUTH_TIS_CHALLENGE);
    packet_put_cstring(authctxt->cn, challenge);
    xfree(challenge);
...
auth1_process_tis_response(Authctxt *authctxt, char *info, size_t infolen)
{
    ...
    packet_check_eom();
#else
    response = packet_get_string(authctxt->cn, &dlen);
    packet_check_eom(authctxt->cn);
#endif /* L7_SSHD */
...
do_authloop(Authctxt *authctxt)
{
    ...
    packet_write_wait();
#else
    packet_start(authctxt->cn, SSH_SMSG_FAILURE);
    packet_send(authctxt->cn);
    packet_write_wait(authctxt->cn);
#endif /* L7_SSHD */
...
#else
            fatal_cn(authctxt->cn, "Access denied for user %s.",authctxt->user);
#endif /* L7_SSHD */
        }
#endif /* _UNICOS */
...
#else
            packet_disconnect(authctxt->cn, "Authentication rejected for uid %d.",
                authctxt->pw == NULL ? -1 : authctxt->pw->pw_uid);
#endif /* L7_SSHD */
            authenticated = 0;
        }
...
#else
        if (authenticated &&
            !auth_root_allowed(authctxt->cn, meth->name)) {
#endif /* L7_SSHD */
            authenticated = 0;
...
#else
            packet_disconnect(authctxt->cn, msg);
#endif /* L7_SSHD */
        }
#endif
...
do_authentication(Authctxt *authctxt)
{
    ...
    packet_check_eom();
#else
    packet_read_expect(authctxt->cn, SSH_CMSG_USER);

    /* Get the user name. */
    user = packet_get_string(authctxt->cn, &ulen);
    packet_check_eom(authctxt->cn);
#endif /* L7_SSHD */
...
#else
    if ((authctxt->pw = PRIVSEP(getpwnamallow(authctxt->cn, user))) != NULL)
#endif /* L7_SSHD */
        authctxt->valid = 1;
    else {
    ...
    packet_write_wait();
#else
    packet_start(authctxt->cn, SSH_SMSG_SUCCESS);
    packet_send(authctxt->cn);
    packet_write_wait(authctxt->cn);
#endif /* L7_SSHD */
}
