...
typedef struct _bcm_port_info_s {
    ...
    bcm_filterid_t      p_ut_filter;    /* Untagged priority filter rules */
#endif
#ifdef BCM_METER_SUPPORT
    /* For non-TUCANA, implement port-based ingress rate meter using FFP */
    int                 meter_dpid;     /* Diffserve datapath */
    int                 meter_cfid;     /* Diffserve classifier */
#endif
...
extern int _bcm_gport_modport_api2hw_map(int, bcm_module_t, bcm_port_t,
                                         bcm_module_t *, bcm_port_t *);
