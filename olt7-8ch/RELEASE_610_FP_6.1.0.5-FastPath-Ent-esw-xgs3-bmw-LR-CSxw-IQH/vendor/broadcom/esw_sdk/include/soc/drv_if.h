/*
 * $Id: drv_if.h,v 1.1 2011/04/18 17:10:57 mruas Exp $
...
typedef struct {
    ...
    /* port priority mapping operation(PCP2TC / TC2PCP) set */
    int (*port_pri_mapop_set)(int unit, int port, int op_type, 
                uint32 pri_old, uint32 pri_new, uint32 cfi_new);
                
    /* port priority mapping operation(PCP2TC / TC2PCP) set */
    int (*port_pri_mapop_get)(int unit, int port, int op_type, 
                uint32 pri_old, uint32 *pri_new, uint32 *cfi_new);
    ...
    /* trap and snoop */
    /* trap enable/disable */
    int (*trap_set)(int unit, soc_pbmp_t bmp, uint32 trap_mask);
    ...
    int (*rate_config_get)(int unit, uint32 port, uint32 config_type, 
        uint32 *value);
    int (*rate_set)(int unit, soc_pbmp_t pbmp, uint8 queue_n, int direction, 
        uint32 limit, uint32 burst_size);
    int (*rate_get)(int unit, uint32 port, uint8 queue_n, int direction, 
        uint32 *limit, uint32 *burst_size);
...
extern int drv_mirror_get(int unit, uint32 *enable, uint32 *monitor_port, 
                                            soc_pbmp_t *ingress_bmp, soc_pbmp_t *egress_bmp);
extern int drv_port_pri_mapop_set(int unit, int port, int op_type, 
                        uint32 pri_old, uint32 pri_new, uint32 cfi_new);
extern int drv_port_pri_mapop_get(int unit, int port, int op_type, 
                        uint32 pri_old, uint32 *pri_new, uint32 *cfi_new);
...
extern int drv_mstp_port_get(int unit, uint32 mstp_gid, 
                                        uint32 port, uint32 *port_state);
extern int drv_trap_set(int unit, soc_pbmp_t bmp, uint32 trap_mask);
...
extern int drv_rate_config_get(int unit, uint32 port, 
                                        uint32 config_type, uint32 *value);
extern int drv_rate_set(int unit, soc_pbmp_t bmp, uint8 queue_n, int direction, 
                                        uint32 limit, uint32 burst_size);
extern int drv_rate_get(int unit, uint32 port, uint8 queue_n, int direction, 
                                        uint32 *limit, uint32 *burst_size);
