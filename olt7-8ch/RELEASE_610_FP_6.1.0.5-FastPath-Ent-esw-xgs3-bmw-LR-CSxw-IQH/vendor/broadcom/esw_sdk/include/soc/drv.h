/*
 * $Id: drv.h,v 1.1 2011/04/18 17:10:57 mruas Exp $
...
#define SOC_IS_HAWKEYE(unit)    (SOC_INFO(unit).chip & SOC_INFO_HAWKEYE)
#else
...
#define SOC_IS_BRADLEY(unit)    (SOC_INFO(unit).chip & SOC_INFO_HB_GW)
#else
...
#define SOC_IS_ENDURO(unit)    (SOC_INFO(unit).chip & SOC_INFO_ENDURO)
...
#define SOC_IS_TRIUMPH2(unit)    (SOC_INFO(unit).chip & SOC_INFO_TRIUMPH2)
#else
#define SOC_IS_TRIUMPH2(unit)  (0)
#endif

#ifdef  BCM_APOLLO_SUPPORT
#define SOC_IS_APOLLO(unit)    (SOC_INFO(unit).chip & SOC_INFO_APOLLO)
#else
#define SOC_IS_APOLLO(unit)  (0)
#endif

#ifdef  BCM_VALKYRIE2_SUPPORT
#define SOC_IS_VALKYRIE2(unit)    (SOC_INFO(unit).chip & SOC_INFO_VALKYRIE2)
#else
#define SOC_IS_VALKYRIE2(unit)  (0)
#endif

#ifdef  BCM_SIRIUS_SUPPORT
#define SOC_IS_SIRIUS(unit)    (SOC_INFO(unit).chip & SOC_INFO_SIRIUS)
#else
...
#define SOC_UNIT_VALID(unit)            (((unit) >= 0) &&                  \
...
#define PORT_DUALMODID_VALID(_u_, _p_) \
         if ((NUM_MODID(_u_) == 2) && (_p_ > 31)) {return SOC_E_PORT;}       
...
#define CI_BLOCK(unit, instance)        ((((instance) >= 0) && ((instance) < SOC_MAX_NUM_CI_BLKS)) ? SOC_INFO(unit).ci_block[(instance)] : (-1))
...
#define SOC_INFO_FX_HX          (SOC_INFO_FELIX_ANY | SOC_INFO_HELIX_ANY | \
                                 SOC_INFO_RAPTOR | SOC_INFO_RAVEN | SOC_INFO_HAWKEYE)
