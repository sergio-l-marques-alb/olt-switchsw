/*
 * $Id: acl_field.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 ...
 */
#define ACL_FIELD_IS_INIT()                                                 \
    if (acl_field_control == NULL) {                                        \
        ACL_ERR(("ACL Error: ACL Field  not initialized\n"));               \
        return BCM_E_INIT;                                                  \
    }
...
/* Local Prototypes */
STATIC _acl_field_group_t *_acl_field_group_first(_acl_field_control_t *fc);
STATIC _acl_field_group_t *_acl_field_group_next(_acl_field_control_t *fc);
...
_acl_field_init(void)
{
    ...
    if (field_control == NULL) {
        ACL_ERR(("ACL Error: allocation failure for ACL Field control.\n"));
        return BCM_E_MEMORY;
    }
...
{
    ACL_VVERB(("ACL _acl_field_detach()\n"));
    ...
    if (BCM_FAILURE(_acl_field_group_destroy_all())) {
        ACL_ERR(("ACL Error: Failure in _acl_field_group_destroy_all()\n"));
    }
                        ...
                        _acl_field_group_t **ret_group)
{
    ...
    ACL_VVERB(("ACL _acl_field_group_create()\n"));

    /* Allocate a ACL field group. */
    group = sal_alloc(sizeof(_acl_field_group_t), "ACL field group");
    if (group == NULL) {
        ACL_ERR(("ACL Error: Allocation error for ACL field group\n"));
        return BCM_E_MEMORY;
    }
    ...
    while (field_control->prio_prev > 0 && BCM_FAILURE(retval)) {
        ACL_VERB(("ACL Calling FP group create pri=%d\n",
    ...
    if (BCM_FAILURE(retval)){
        ACL_ERR(("ACL Error: Group creation error (prio=%d, gid=%d)\n",
...
_acl_field_group_remove_all(void) {
    ...
    ACL_VVERB(("ACL _acl_field_group_remove_all()\n"));
    ...
    while (group != NULL) {
        ...
        if (BCM_FAILURE(retval)) {
            ACL_ERR(("ACL Error: Failed to remove entries in group ID=%d\n",
                      group->gid));
            return retval;
        }
        group = group->next;
...
_acl_field_group_destroy_all(void) {
    ...
    ACL_VVERB(("ACL _acl_field_group_destroy_all()\n"));
    assert(acl_field_control != NULL);
    ...
    while (acl_field_control->group_list != NULL) {
        ...
        if (BCM_FAILURE(retval)) {
            ACL_ERR(("ACL Error: _acl_field_group_destroy_all() failure\n"));
            return retval;
        }
    }
...
_acl_field_group_destroy(_acl_field_control_t *control,
    ...
    ACL_VVERB(("ACL _acl_field_group_destroy(gid=%d)\n", gid));
    assert(control != NULL);

    /* If the group list is empty, group ID can't be found. */
    if (control->group_list == NULL) {
        ACL_ERR(("ACL Error: No groups available to delete?\n"));
    ...
    while (group != NULL) {
        if (group->gid == gid) {
            ...
            if (BCM_FAILURE(retval)) {
                ACL_ERR(
                ("ACL Error: Failed to destroy entries in group ID=%d\n",
                 group->gid));
                return retval;
            }
            ...
            if (BCM_FAILURE(retval)) {
                ACL_ERR(("ACL Error: group ID=%d not destroyed\n", gid));
                return retval;
            }
            return BCM_E_NONE;
        }
    ...
    ACL_ERR(("ACL Error: group ID=%d not found to be destroyed\n", gid));
    return BCM_E_NOT_FOUND;
}
                        ...
                        bcma_acl_rule_t *rule)
{
    ...
    ACL_VVERB(("ACL _acl_field_entry_create(gid=%d, list_id=%d, rule_id=%d)\n",
    ...
    if (entry == NULL) {
        ...
        bcmx_field_entry_destroy(eid);
        ACL_ERR(("ACL Error: _acl_field_entry_t allocation failure\n"));
        return BCM_E_MEMORY;
    }
                              ...
                              bcm_field_entry_t eid, _acl_field_entry_t *entry)
{
    ...
    ACL_VVERB(("ACL _acl_field_entry_qualify_vlan(gid=%d, rule_id=%d)\n",
                                   ...
                                   _acl_field_entry_t *entry)
{
   ...
   ACL_VVERB(("ACL _acl_field_entry_qualify_l4srcport(gid=%d, rule_id=%d)\n",
               group->gid, rule->rule_id));
    ...
    for (rule_idx = 0; rule_idx < org_count; rule_idx++) {
       ...
       rv = bcmx_field_qualify_L4SrcPort(eid_cur, 
        ...
        for (idx = 1; idx < count; idx++) {
            ...
            if (entry == NULL) {
                ACL_ERR(
                ("ACL Error: _acl_field_entry_t allocation failure\n"));
                rv =  BCM_E_MEMORY;
                goto srcport_error;
            }
                                   ...
                                   _acl_field_entry_t *entry)
{
    ...
    ACL_VVERB(("ACL _acl_field_entry_qualify_l4dstport(gid=%d, rule_id=%d)\n",
               group->gid, rule->rule_id));     
    /* Create the necessary Field range checker. */
...
_acl_field_entry_remove_all(_acl_field_group_t *group) 
{
    ...
    ACL_VVERB(("ACL _acl_field_entry_remove_all(gid=%d)\n",
               group->gid));
    entry = group->entry_list;
    while (entry != NULL) {
        retval = bcmx_field_entry_remove(entry->eid);
        if (BCM_FAILURE(retval)) {
           ACL_ERR(("ACL Error: Entry ID=%d not removed\n", entry->eid));
           return retval;
        }
        entry = entry->next;
    }
                         ...
                         bcm_field_entry_t eid) 
{
    ...
    assert(group != NULL);
    ACL_VVERB(("ACL _acl_field_entry_remove(gid=%d, eid=%d)\n",
               group->gid, eid));
    ...
    ACL_ERR(("ACL Error: Entry ID=%d not found to be removed\n", eid));
    return BCM_E_NOT_FOUND;
}
...
_acl_field_entry_destroy_all(_acl_field_group_t *group) 
{
    ...
    ACL_VVERB(("ACL _acl_field_entry_destroy_all(gid=%d)\n",
               group->gid));

    while (group->entry_list != NULL) {
        retval = _acl_field_entry_destroy(group, group->entry_list->eid);
        if (BCM_FAILURE(retval)) {
            ACL_ERR(("ACL Error: _acl_field_entry_destroy() failure\n"));
            return retval;
        }
    }
                         ...
                         bcm_field_entry_t eid) 
{
    ...
    assert(group != NULL);
    ACL_VVERB(("ACL _acl_field_entry_destroy(gid=%d, eid=%d)\n",
               group->gid, eid));

    /* If the entry list is empty, Entry ID can't be found. */
    if (group->entry_list == NULL) {
        ACL_ERR(("ACL Error: No entries available to delete?\n"));
        return BCM_E_NOT_FOUND;
    }
    ...
    if (group->entry_list->eid == eid) {
        ...
        if (BCM_FAILURE(retval)) {
            ACL_ERR(("ACL Error: Entry ID=%d not destroyed\n", eid));
            return retval;
        }
        return BCM_E_NONE;
    }
    ...
    ACL_ERR(("ACL Error: Entry ID=%d not found to be destroyed\n", eid));
    return BCM_E_NOT_FOUND;
}
...
{
    assert(rule);
    ACL_VVERB(("ACL _acl_field_entry_action_add(rule_id=%d, eid=%d)\n",
                rule->rule_id, eid));
...
_acl_field_merge(_acl_control_t *acl_control)
{
    ...
    ACL_VVERB(("ACL _acl_field_merge()\n"));
    ACL_FIELD_IS_INIT();
         ...
         acl_link = _acl_next(acl_control))
    {
        ...
        {
            ACL_VERB(("ACL Merging Rule ID=%d\n", rule->rule_id));
            ...
            while (group != NULL) {
                ACL_VERB(("ACL Setting group ID=%d\n", group->gid));
            ...
            if (BCM_FAILURE(retval)) {
                ACL_VERB(("ACL Creating another group\n"));
                retval = _acl_field_group_create(acl_field_control, qset,
                                                 &group);
                if (BCM_FAILURE(retval)) {
                    bcmx_lplist_free(&lplist_full);
                    ACL_ERR(("ACL Error: Can't create group for rule_id=%d\n",
                             rule->rule_id));
                    return retval;
                }
                ACL_VERB(("ACL Group (gid=%d) created.\n", group->gid));
            }
...
_acl_field_install(void)
{
    ...
    ACL_VVERB(("ACL _acl_field_install()\n"));
    ACL_FIELD_IS_INIT();
    ...
    for (group = acl_field_control->group_list;
         ...
         group = group->next)
    {
        ACL_VERB(("ACL: Installing Group ID=%d\n", group->gid));
        /* For each entry in group. */
        for (entry = group->entry_list; entry != NULL; entry = entry->next) {
            /* Install the entry */
            ACL_VERB(("ACL: Installing Entry ID=%d\n", entry->eid));
            retval = bcmx_field_entry_install(entry->eid);
            if (BCM_FAILURE(retval)) {
                ACL_ERR(("ACL Error: ACL Field Entry ID=%d install failure.\n",
                         entry->eid));
                return retval;
            }
        }
...
_acl_field_rule_remove(bcma_acl_rule_id_t rule_id) 
{
    ...
    ACL_VVERB(("ACL _acl_field_rule_remove(rule_id=%d)\n", rule_id));
    ACL_FIELD_IS_INIT();
                           ...
                           bcm_field_entry_t *eid) 
{
    ...
    ACL_VVERB(("ACL _acl_field_rule_entry_find(rule_id=%d)\n", rule_id));
    ...
    ACL_VVERB(("ACL END _acl_field_rule_entry_find(rule_id=%d) NOT FOUND\n", rule_id));
    return BCM_E_NOT_FOUND;
}
...
_acl_field_show(void) {
    ...
    ACL_OUT(("acl_field={group_numb=%d, ", group_count));
...
_acl_field_group_show(_acl_field_group_t *group)
{
    ...
    ACL_VVERB(("ACL _acl_field_group_show(gid=%d)\n", group->gid));
    assert(group != NULL);
    ACL_OUT(("\tgroup={gid=%d,\n", group->gid));
...
_acl_field_entry_show(_acl_field_entry_t *entry)
{
    ...
    assert(entry != NULL);
    ACL_VVERB(("ACL _acl_field_entry_show(eid=%d)\n", entry->eid));
    ACL_OUT(("entry={eid=%d, rule_id=%d, pri=%d},", entry->eid,
