/*
 * $Id: cpudb.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
_cpudb_clear(cpudb_ref_t db_ref, int keep_local)
{
    ...
    if (saved) {
        ...
        for (i = 0; i < entry->base.num_stk_ports; i++) {
            /* Clear all flags except no link flag */
            entry->sp_info[i].flags &= CPUDB_SPF_NO_LINK;
        }
    }
...
{
    return ((db_ref != NULL) && (db_ref->magic == CPUDB_MAGIC));
}
