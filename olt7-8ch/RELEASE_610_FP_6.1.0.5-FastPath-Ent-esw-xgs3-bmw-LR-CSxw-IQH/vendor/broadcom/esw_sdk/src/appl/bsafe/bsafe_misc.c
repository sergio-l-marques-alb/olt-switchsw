 ...
 * $Id: bsafe_misc.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 */
...
void *(*g_bsafe_copy_from_user)(void *, const void *, size_t) = OS_COPY_FROM_USER;
void *(*g_bsafe_copy_to_user)(void *, const void *, size_t) = OS_COPY_TO_USER;
void bsafe_swap_dummy(BSAFE_BIGNUM * src, int size);
void (*g_bsafe_swap_bignum)(BSAFE_BIGNUM *, int) = bsafe_swap_dummy;


void bsafe_set_copy_from_user (void *(*ptr)(void *, const void *, size_t));
void bsafe_set_copy_to_user (void *(*ptr)(void *, const void *, size_t));
void bsafe_set_swap_bignum(void (*ptr)(BSAFE_BIGNUM *, int));
...
#else
#define byteCopy(src,dest, size) OS_MEMCPY(dest,src,size)
...
void bsafe_swap_bn(unsigned char * buffer, int size)
{
...
void initialize_kek_akey_localstatus(void * dev_context)
{
	...
	OS_MEMSET(g_kekloc_used, BSAFE_KEY_FREE, sizeof(g_kekloc_used) );
	OS_MEMSET(g_akeyloc_used, BSAFE_KEY_FREE, sizeof(g_akeyloc_used) );
