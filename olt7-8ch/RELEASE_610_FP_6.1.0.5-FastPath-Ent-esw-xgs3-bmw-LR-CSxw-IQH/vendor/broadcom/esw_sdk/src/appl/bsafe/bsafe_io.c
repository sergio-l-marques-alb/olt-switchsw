 ...
 * $Id: bsafe_io.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 */
...
int allocate_inputbuf(bsafe_context_t * cmd_context, int command,  
		                ...
		                void (* callback)(bsafe_context_t *), int output_cmd_size);
...
int bsafe_clean(void * dev_context)
{
	return BSAFE_SUCCESS;
}
...
int bsafe_ioctl(void * dev_context, int command, void *cmdio)
{
	...
	/* play safe */
	OS_MEMSET(cmd_context,0x00,sizeof(bsafe_context_t));
...
#define ALWAYS_RET_SUCCESS	0
#if ALWAYS_RET_SUCCESS
		 ...
		 void (* callback)(bsafe_context_t *), int output_cmd_size)
{
	...
	OS_MEMSET(cmd_context->ocmd,0x00, sizeof(bsafe_ocmd_t));
...
void bsafe_clr_key_out(bsafe_context_t * cmd_context)
{
	...
	bsafe_clr_key_io_t *cmdio;
	bsafe_clr_key_input_t *input_cmd = NULL;
...
int bsafe_user_random_in(bsafe_context_t  * cmd_context, void * arg)
{
	...
	allocate_inputbuf(cmd_context, UHSM_USER_RANDOM, 
		...
		bsafe_user_random_out,
		sizeof(bsafe_user_random_output_t));

	input_cmd->rngType = SWAP4BYTES(cmdio->rngType);
	COPY_FROM_USER(input_cmd->random, cmdio->random, sizeof(input_cmd->random));
...
extern void BN2BIGNUM(BSAFE_BIGNUM * src, int size);
int bsafe_le_bignum(bsafe_context_t  * cmd_context, void * arg)
{
