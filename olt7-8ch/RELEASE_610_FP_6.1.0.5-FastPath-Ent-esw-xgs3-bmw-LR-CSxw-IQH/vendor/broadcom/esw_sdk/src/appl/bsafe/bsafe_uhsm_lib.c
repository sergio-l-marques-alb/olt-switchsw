 ...
 * $Id: bsafe_uhsm_lib.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 */
...
int bsafe_open(int unit, int reset)
{
	int fd = unit;
	return bsafe_strata_open(fd, reset);
}

int bsafe_close(int fd) 
{
	return bsafe_strata_close(fd);
}
				...
				BSAFE_BIGNUM *sig_begin_s /* 20 bytes */) 
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_DLIES_GEN_KS_IOCTL,&cmd);
			...
			unsigned short *sessionid_r)
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_DLIES_MSG_DEC_IOCTL,&cmd);
				...
				bsafe_dsa_pub_t *kdi_pub_out /* 404 bytes  */)
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_DEV_KDI_PUBLIC_IOCTL,&kdi_cmd);

	kdc_cmd.kdc_cp = kdc_pub_out;
	bsafe_strata_ioctl(fd,BSAFE_UHSM_DEV_KDC_PUBLIC_IOCTL,&kdc_cmd);
				...
				BSAFE_BIGNUM *sig_userauth_s /* 20 bytes */)
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USERAUTH_IOCTL,&cmd);
				...
				unsigned char *op /* len of op == p_len */ )
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_PKE_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_3DES_HMACSHA1_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_INIT_DEVICEKEY_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_SELFTEST_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_KEK_STATUS_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_AKEY_STATUS_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_CLR_KEY_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_LD_USERKEY_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_LD_CFG_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_FIPS_CLR_IOCTL,&cmd);
...
int bsafe_user_status( int fd , unsigned int *SALeventstatus )
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_STATUS_IOCTL,&cmd);
...
int bsafe_user_nop( int fd , unsigned char *datain,unsigned int data_len, unsigned char * dataout )
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_NOP_IOCTL,&cmd);
...
int bsafe_user_random( int fd , unsigned int rngtype, unsigned char * random /* 20 bytes */ )
{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_RANDOM_IOCTL,&cmd);
...

{
	...
	bsafe_strata_ioctl(fd,BSAFE_UHSM_USER_SHA1_IOCTL,&cmd);
				...
				BSAFE_BIGNUM * one	/* 8 bytes */)
{
	...
	if (one[0] == 0x01) /* huh  0x01020304 0x05060708 */
		bsafe_strata_ioctl(fd,BSAFE_LE_BIGNUM_IOCTL,&cmd);
	else if (one[7] == 0x01) /* huh 0x0807060504030201 */
		bsafe_strata_ioctl(fd,BSAFE_BN_2_BIGNUM_IOCTL,&cmd);
