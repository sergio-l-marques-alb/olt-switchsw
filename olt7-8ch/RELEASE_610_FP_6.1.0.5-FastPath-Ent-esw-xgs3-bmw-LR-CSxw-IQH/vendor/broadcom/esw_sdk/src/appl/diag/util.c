/*
 * $Id: util.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
int
parse_macaddr_hex(char *str, mac_addr_t macaddr)
{
    char *s;
    int i;
...
int
parse_macaddr(char *str, mac_addr_t macaddr)
{
    char *s;
    int i;
...
void
format_macaddr(char buf[SAL_MACADDR_STR_LEN], mac_addr_t macaddr)
{
    int i;
...
void
increment_macaddr(mac_addr_t macaddr, int amount)
{
    int i, v;
    for (i = 5; i >= 0; i--) {
    ...
    if (!sal_strcasecmp(val_str, "min")) {
        ...
        if (!soc_robo_mem_index_valid(unit, mem, index)) {
            printk("WARNING: index %d out of range for memory %s\n",
                index, SOC_ROBO_MEM_UFNAME(unit, mem));
        }
#endif        
    } else {
#ifdef BCM_ESW_SUPPORT    
...
{
    char *action_text[bcmFieldActionCount] = BCM_FIELD_ACTION_STRINGS;
