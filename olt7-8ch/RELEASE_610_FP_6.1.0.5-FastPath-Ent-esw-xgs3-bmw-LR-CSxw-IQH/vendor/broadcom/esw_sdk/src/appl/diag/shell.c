/*
 * $Id: shell.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
 ...
 */
{
	...
	if ((cmd->c_usage) &&
        (soc_property_get(u, spn_HELPCLI_ENABLE, 1))) {
	    printk("Usage (%s): %s", cmd->c_cmd, cmd->c_usage);
	} else if (soc_property_get(u, spn_HELPCLI_ENABLE, 1)){
 ...
 */
{
    ...
    if (!soc_property_get(u, spn_HELPCLI_ENABLE, 1)) {
        return (CMD_OK);
    }
    /* Provide easy method to print out all commands */
    if ((1 == ARG_CNT(a)) && !sal_strcasecmp(_ARG_CUR(a), "print-manual")) {
	ARG_NEXT(a);
        /* Print common commands */
	printk("\nCommands common to all modes:\n");
	for (cmd = bcm_cmd_common;
             cmd < &bcm_cmd_common[bcm_cmd_common_cnt]; cmd++) {
	    printk("\n\nCOMMAND: %s\tDescription:%s\n\n%s",
		   cmd->c_cmd,
		   cmd->c_help[0] == '.' ? &cmd->c_help[1] : cmd->c_help,
		   cmd->c_usage);
	}
	printk("\nCommands for current mode:\n");
	for (cmd = cur_cmd_list; cmd < &cur_cmd_list[cur_cmd_cnt]; cmd++) {
	    printk("\n\nCOMMAND: %s\tDescription:%s\n\n%s",
		   cmd->c_cmd,
		   cmd->c_help[0] == '.' ? &cmd->c_help[1] : cmd->c_help,
		   cmd->c_usage);
    ...
    while ((c = ARG_GET(a)) != NULL) {			/* Look it up */
        clist = cur_cmd_list;
        ccnt = cur_cmd_cnt;
	cmd = (cmd_t *)parse_lookup(c, clist,
				    sizeof(cmd_t), ccnt);
	if (!cmd) {
            clist = bcm_cmd_common;
            ccnt = bcm_cmd_common_cnt;
            cmd = (cmd_t *)parse_lookup(c, bcm_cmd_common,
                                        sizeof(cmd_t), ccnt);
            if (!cmd) {
...
#ifdef BCM_ESW_SUPPORT
        soc_phy_list_get(phy_list, sizeof(phy_list)/sizeof(phy_list[0]), &phys_in_list);
 ...
 */
{
	...
	char *ellipsis;
        ...
        do {
            ...
            for (i = 0; i < BCM_MODULE__COUNT; i++) {
                if (!sal_strcasecmp(bcm_module_name(u, i), c)) {
                    found = 1;
                    flags |= (1 << i);
                    break;
                }
        ...
        if (flags == 0 || usage) {
            if (flags == 0) {
		printk("%s: missing flags:\n", ARG_CMD(a));
	    }
            printk("Valid flags are: \n");
            for (i = 0; i < BCM_MODULE__COUNT; i++) {
                printk("    %s\n", bcm_module_name(u, i));
            }
            return CMD_FAIL;
	...
	if ((bcm_rv = bcm_init_selective(u, flags)) < 0) {
	    ...
	    printk("%s: Unable to initialize %s%s (flags 0x%x): %s\n",
		   ARG_CMD(a), c, ellipsis, flags, bcm_errmsg(bcm_rv));
	    rv = CMD_FAIL;
	}
    }
