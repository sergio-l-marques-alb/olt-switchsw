                ...
                int ar_idx, uint64 val)
{
    ...
    if (counter_val[unit] == NULL) {
    	int	n;
    	n = SOC_MAX_COUNTER_NUM * SOC_MAX_NUM_PORTS;
                ...
                int ar_idx, int flags)
{
    ...
    if (soc_robo_counter_get(unit, port, ctr_reg, &val) < 0) {
        return;
    }
            ...
            if (reg != INVALID_Rr) {
                if (SOC_REG_INFO(unit, reg).flags & SOC_REG_FLAG_ARRAY) {
                    for (ar_idx = 0;
                         ...
                         ar_idx++) {
                        if ((rv = (DRV_SERVICES(unit)->counter_get) \
                                               (unit, port, reg, &val)) < 0) {
                            return rv;
                        }
