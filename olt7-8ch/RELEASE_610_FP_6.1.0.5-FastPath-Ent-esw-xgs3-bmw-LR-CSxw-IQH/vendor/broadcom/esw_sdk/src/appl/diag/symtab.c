/*
 * $Id: symtab.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
parse_symbolic_reference(int unit, soc_regaddrlist_t *alist, char *ref)
{
    ...
    soc_block_t regblktype, bmask, portblktype;
    soc_reg_t reg;
    pbmp_t bm;
	...
	*range1++ = '\0';
    ...
    soc_cm_debug(DK_SYMTAB, "symtab: index %d:%d type %d regblktype %#x\n",
                 minidx, maxidx, reginfo->regtype, regblktype);
	...
	if (range2) {
        ...
        if (SOC_IS_ROBO(unit)) {
            ...
            if (regblktype & SOC_BLK_SYS) {
                ...
                if (! range1) {
                    if (regblktype & (SOC_BLK_PORT||SOC_BLK_EXTER)) {
                        SOC_PBMP_CLEAR(bm);
                        SOC_BLOCK_ITER(unit, blk, regblktype) {
                            SOC_PBMP_OR(bm, SOC_BLOCK_BITMAP(unit, blk));
                        }
