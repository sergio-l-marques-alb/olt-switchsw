/* 
 * $Id: reg.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
 ...
 */
int
_reg_get_by_type(int unit, uint32 regaddr, soc_regtype_t regtype,
 ...
 */
int
_reg_set_by_type(int unit, uint32 regaddr, soc_regtype_t regtype,
		 uint64 regval)
{
    int             rv = CMD_OK, r;
    uint32          val32;
        ...
        if (rt->type == soc_cpureg) {
            ...
            for (i = 0; i < alist.count; i++) {
                ...
                if (!isint(valstr)) {  /* Must modify registers */
                     ...
                     */
                    if (mod) {
                        rv = _reg_get_by_type(unit, ainfo->addr,
                ...
                if (rv == CMD_OK) {
                    rv = _reg_set_by_type(unit, ainfo->addr,
