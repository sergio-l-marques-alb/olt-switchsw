/*
 * $Id: acl.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
STATIC int _bcmx_acl_rule_add(bcma_acl_list_id_t list_id, bcma_acl_rule_t *rule);
...
bcmx_cmd_acl(int unit, args_t *args) {
    ...
    if (param != NULL) {
        if (sal_strcmp(param, "?") == 0)
        {
            ...
            if ((cmd->c_usage != NULL) &&
                (soc_property_get(unit, spn_HELPCLI_ENABLE, 1))) {
                sal_printf("Usage:  %s\n", cmd->c_usage);
            }
            if ((cmd->c_help != NULL) && 
                (soc_property_get(unit, spn_HELPCLI_ENABLE, 1))){
...
bcmx_cmd_acl_list_create(int unit, args_t *args) {
    ...
    if (list_p == NULL) {
        printk("ACL: list ID=%d allocation failure\n", list_id);
        return CMD_FAIL;
    }
    ...
    if (str) {
        if (BCM_FAILURE(bcmx_lplist_parse(&list_p->lplist, str))) {
            printk("Can't parse UserPortList \"%s\"\n", str);
            return CMD_USAGE;
        }
        /* Read the priority */
        str = ARG_GET(args);
...
bcmx_cmd_acl_list_destroy(int unit, args_t *args) {
    ...
    if (BCM_FAILURE(bcma_acl_remove(list_id))) {
        printk("ACL: failed to destroy list ID=%d \n", list_id);
        return CMD_FAIL;
    }

    printk("ACL: List ID=%d destroyed\n", list_id);
...
bcmx_cmd_aclrule(int unit, args_t *args) {
    ...
    ACL_VVERB(("BEGIN bcmx_cmd_aclrule()\n"));
    ...
    if (ethertype > 0xffff) {
        printk("Ethertype=%#x too large (>0xffff).\n", ethertype);
        sal_free(rule);
        return CMD_FAIL;
    }
    ...
    if (BCM_FAILURE(bcma_acl_rule_add(list_id_cur, rule))) {
        printk("ACL: rule add failed (rule_id=%d).\n", rule->rule_id);
        sal_free(rule);
        return CMD_FAIL;
    }
...
_bcmx_acl_rule_alloc(void) {
    ...
    ACL_VVERB(("BEGIN _bcmx_acl_rule_alloc()\n"));
    rule_new = sal_alloc(sizeof(bcma_acl_rule_t), "acl rule");
    if (rule_new == NULL) {
        printk("ACL Error: allocation failure\n");
        return NULL;
    }
