/*
 * $Id: counter.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
                ...
                int ar_idx, uint64 val)
{
    ...
    debugk(DK_COUNTER,
	   ...
	   SOC_REG_NAME(unit, ctr_reg), ar_idx,
	   port,
	   ind,
	   COMPILER_64_HI(val),
	   COMPILER_64_LO(val));
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        switch (rx_tx) {
            ...
            case TDBGC:
                ...
                end = (counter == -1) ? (SOC_IS_EASYRIDER(unit) ? snmpValCount
                    : snmpBcmCustomTransmit12) : counter + 1;
                for (count_index = start; count_index < end; count_index++) {
                    a->a_arg = flags_index;
                    set_triggers(unit, port, count_index, a);
                }
        ...
        /* Only TDBGCn_SELECT for ER is per port */
        if (!SOC_IS_EASYRIDER(unit) || rx_tx == RDBGC) {
            break;
        }
    }
    return 0;
