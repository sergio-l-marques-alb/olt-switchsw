/* 
 * $Id: brd_common.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
#if 0
#define BCM_BOARD_MAX_MODID  128  /* may be 5675s in system */
#else
                      ...
                      int start, int flags)
{
    ...
    for (i = 0; i < entry->base.num_stk_ports; i++) {
         ...
         */
        trunk.num_ports = 0;
        trunk.tp[trunk.num_ports++] = entry->base.stk_ports[i].port;
        for (j = i+1; j < entry->base.num_stk_ports; j++) {
            ...
            SHR_BITSET(pseen, j);
            trunk.tp[trunk.num_ports++] = entry->base.stk_ports[j].port;
        }
