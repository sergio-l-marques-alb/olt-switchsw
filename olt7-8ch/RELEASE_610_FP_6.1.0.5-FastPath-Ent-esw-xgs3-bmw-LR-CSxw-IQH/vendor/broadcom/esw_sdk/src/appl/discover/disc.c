/*
 * $Id: disc.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
#else
static bcm_trans_ptr_t *disc_trans_ptr = NULL;
#endif
...
probe_pkt_process(cpudb_ref_t db_ref, disc_pkt_t *probe_pkt)
{
    ...
    entry_buf = data_start + PROBE_ENTRY_OFS(0);
    drop_pkt = TRUE;
    switch (probe_pkt_dseq_num_check(db_ref, entry_buf, entry_count)) {
    case PROBE_PKT_OK:
        drop_pkt = FALSE;
        break;
    case PROBE_PKT_DROP:
    ...
    } else if ((disc_abort_rv == DISC_RESTART_REQUEST) && (local_idx < 0)) {
        /*
...
route_entry_process(uint8 *buf, cpudb_entry_t *entry, int version)
{
    ...
    if (!(entry->flags & CPUDB_F_BASE_INIT_DONE)) {
        ...
        sal_memcpy(entry->base.mac, buf, sizeof(bcm_mac_t));
        buf += sizeof(bcm_mac_t);
        UNPACK_U32_INCR(buf, entry->base.dseq_num);
        UNPACK_U32_INCR(buf, entry->flags);
        entry->flags               |= CPUDB_F_BASE_INIT_DONE;
        UNPACK_U32_INCR(buf, entry->base.slot_id);
        if (entry->db_ref->local_entry->base.flags & CPUDB_BASE_F_CHASSIS) {
            DISC_VERB(("Found slot %d\n", entry->base.slot_id));
        }
        UNPACK_U32_INCR(buf, entry->base.master_pri);

        sal_memcpy(entry->base.app_data, buf, CPUDB_APP_DATA_BYTES);
        buf += CPUDB_APP_DATA_BYTES;
        entry->base.board_id = cpudb_board_id_unknown;
        entry->base.flags    = 0;
        /* Get application data: Board ID and Flag */
        if (version >= DISCOVERY_VERSION_2) {
#if defined(DISCOVER_APP_DATA_BOARDID)
            uint8 *app_buf = entry->base.app_data;
            UNPACK_U32_INCR(app_buf, entry->base.board_id);
            UNPACK_U32_INCR(app_buf, entry->base.flags);
        ...
        UNPACK_U32_INCR(buf, entry->base.dest_unit);
        UNPACK_U32_INCR(buf, entry->base.dest_port);
