 /*
  * $Id: ct_tun.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
#define TUNNEL_DEBUG(stuff)          TKS_DEBUG(TKS_DBG_TUNNEL, stuff)
#define TUNNEL_WARN(stuff)           TKS_DEBUG(TKS_DBG_WARN, stuff)
#define TUNNEL_ERR(stuff)            TKS_DEBUG(TKS_DBG_ERR, stuff)
#define TUNNEL_VERBOSE(stuff)        TKS_DEBUG(TKS_DBG_TUNNELV, stuff)
#else
#define TUNNEL_DEBUG(stuff)
#define TUNNEL_WARN(stuff)
#define TUNNEL_ERR(stuff)
#define TUNNEL_VERBOSE(stuff)
...
#define CT_TUNNEL_RX_HDR_BYTES (2 + 4 + (11*1) + (2*4) + (3*1) + \
                                (CT_TUNNEL_RX_REASONS_WORDS * 4) \
                                 + (2*4) + 4 + \
                                2 + (2*1) + 2 + (2*1) + (11*4))
...
ct_rx_tunnel_pkt_pack(bcm_pkt_t *pkt, uint8 *pkt_data)
{
    ...
    CT_INCR_PACK_U8(pkt_data, pkt->src_port);
    CT_INCR_PACK_U8(pkt_data, pkt->src_mod & 0xFF);
    CT_INCR_PACK_U8(pkt_data, pkt->dest_port);
    ...
    CT_INCR_PACK_U8(pkt_data, pkt->src_trunk);
    ...
    for (i=0; i<CT_TUNNEL_RX_REASONS_WORDS; i++) {
        CT_INCR_PACK_U32(pkt_data, (uint32)pkt->rx_reasons.w[i]);
    }
                        ...
                        uint8 **payload_start, int *payload_len)
{
    ...
    CT_INCR_UNPACK_U8(pkt_data, pkt->src_port);
    CT_INCR_UNPACK_U8(pkt_data, pkt->src_mod);
    CT_INCR_UNPACK_U8(pkt_data, pkt->dest_port);
    ...
    CT_INCR_UNPACK_U8(pkt_data, pkt->src_trunk);
    ...
    for (i=0; i<CT_TUNNEL_RX_REASONS_WORDS; i++) {
        CT_INCR_UNPACK_U32(pkt_data, pkt->rx_reasons.w[i]);
    }
...
_ct_rx_tunnel_filter_default(int unit, bcm_pkt_t *pkt)
{
    ...
    if (bcm_port_config_get(unit, &port_config) < 0) {
        TUNNEL_ERR(("Tunnel filter: could not get port config "
                    " for %d\n", unit));
        return BCM_CPU_TUNNEL_NONE;
    }
...
ct_rx_tunnelled_pkt_handler(cpudb_key_t src_key, uint8 *payload, int len)
{
    ...
    TUNNEL_VERBOSE(("Tunnel handler from " CPUDB_KEY_FMT_EOLN,
                    CPUDB_KEY_DISP(src_key)));

    if (payload == NULL || len < CT_TUNNEL_RX_HDR_BYTES) {
        TUNNEL_WARN(("Tunnel RX pkt: no payload (%p, len %d)\n",
    ...
    if (rv != BCM_E_NONE) {
        TUNNEL_VERBOSE(("Tunnel RX pkt: remote pkt alloc failed "
    ...
    if (rv < 0) {
        TUNNEL_ERR(("Tunnel RX pkt: unpack failed\n"));
    ...
    if (bcm_unit < 0) {
        TUNNEL_DEBUG(("Tunnel RX pkt: unit/cpu not found: %s.%d;"
                     CPUDB_KEY_FMT_EOLN, keybuf, rx_pkt->rx_unit,
                     CPUDB_KEY_DISP(src_key)));
        bcm_rx_remote_pkt_free(rx_pkt);
        return;
    }
    ...
    TUNNEL_VERBOSE(("Tunnel handler bcm_u %d. u %d p %d. sm %d sp %d cos %d.\n",
    ...
    if (rv < 0) {
        TUNNEL_WARN(("Tunnel RX pkt: rx enqueue failed %d: %s\n",
                      rv, bcm_errmsg(rv)));
        bcm_rx_remote_pkt_free(rx_pkt);
    }
...
#define TUNNEL_TX_HEADER_BYTES \
        (2 + (7*1) + 4 + 1 + 1 + 4 + (2 * PBMP_BYTES) + 1 + 1 + 4 + \
         2 + (11*4))
...
ct_tx_tunnel_setup(void)
{
    ...
    if (rv < 0) {
        TUNNEL_ERR(("Could not init tunnel thread %d: %s",
                    rv, bcm_errmsg(rv)));
        return rv;
    }
#endif
    ...
    if (rv < 0) {
        TUNNEL_ERR((
            "Tunnel: Failed to register for tx tunnel %d: %s\n",
            rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if (rv < 0) {
        TUNNEL_ERR((
            "Tunnel: Failed to register for tx best effort %d: %s\n",
            rv, bcm_errmsg(rv)));
        return rv;
    }
    ...
    if (rv < 0) {
        TUNNEL_ERR((
            "Tunnel: Failed to register tunnel TX with BCM layer "
            "%d: %s\n", rv, bcm_errmsg(rv)));
        return rv;
    }
             ...
             bcm_cpu_tunnel_mode_t mode)
{
    ...
    TUNNEL_VERBOSE(("CT Tunnel pkt out to (%d, %d), flags 0x%x, mode %d\n",
    ...
    rv = bcm_unit_subtype_get(dest_unit, dest_key_str,
                               sizeof(dest_key_str));
    if (rv < 0) {
        TUNNEL_WARN(("Tunnel TX: Could not get dest key string, %d: %s\n",
                    rv, bcm_errmsg(rv)));
    ...
    if (rv < 0) {
        TUNNEL_WARN(("Tunnel TX: Could not parse dest key string, %d: %s\n",
                    rv, bcm_errmsg(rv)));
        atp_tx_data_free(alloc_ptr);
        return rv;
    }

    TUNNEL_VERBOSE(("CT Tunnel dest key %x %x %x %x %x %x\n",
                    dest_key.key[0],
                    dest_key.key[1],
                    dest_key.key[2],
                    dest_key.key[3],
    ...
    if (rv < 0) {
        TUNNEL_VERBOSE(("Tunnel TX: Error sending %d: %s\n",
                        rv, bcm_errmsg(rv)));
        atp_tx_data_free(alloc_ptr);
    }
                            ...
                            void *cookie)
{
    ...
    TUNNEL_VERBOSE(("CT Tunnel pkt in from " CPUDB_KEY_FMT " cli %d, len %d\n",
                    CPUDB_KEY_DISP(src_key), client_id, payload_len));

    if (payload == NULL) {
        /* Unsupported forwarding mode */
        TUNNEL_WARN(("Tunnel TX: Received segmented packet to "
                     "forward.  Unsupported\n"));
    ...
    if (rv < 0) {
        TUNNEL_VERBOSE(("Tunnel TX: Error forwarding %d: %s\n",
                        rv, bcm_errmsg(rv)));
    }
