/*
 * $Id: atp.c,v 1.1 2011/04/18 17:10:59 mruas Exp $
...
#else
static bcm_trans_ptr_t *_atp_trans_ptr = NULL;
#endif
 ...
 *      atp_cos_vlan_set
 * Purpose:
 *      Set the default cos and vlan for clients
 * Parameters:
 *      cos       - COS value to use as default if valid
 ...
 *      atp_cos_vlan_get
 * Purpose:
 *      Get the default cos and vlan for clients
 * Parameters:
 *      cos        - (OUT) Pointer where to store cos value
 ...
 *      cos         - The COS to use sending pkts for this client
 *                    (Unless overridden by flags in transmit)
 *      vlan        - The VLAN to use sending pkts for this client
 * Returns:
 *      BCM_E_XXX
 * Notes:
...
atp_dump(int verbose)
{
    ...
    FOREACH_CLIENT(client, i) {
        ATP_OUT(("Client %d. fl 0x%x. cos %d. vl %d. bet sn %d\n",
                 client->client_id, client->flags, client->cos,
