/*
 * $Id: traffic.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
...
/* Unicast test */
static mac_addr_t traf_unicast_mac_base = {0,0,0,0,0xfe,0};

/* Unicast test */
static mac_addr_t traf_broadcast_mac_base = {0xff,0xff,0xff,0xff,0xff,0xff};
...
_traffic_xgs3_modport_set(int unit, int modid, int portA, int portB)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        /* Traffic test on Higig port not supported */
        return BCM_E_NONE;
    }
...
#ifdef BCM_ESW_SUPPORT
        uint32 bitmap, multiplier = 0;
        rv = soc_xgs3_port_to_higig_bitmap(unit, portA, &bitmap);
        for (multiplier = 0 ; multiplier < 9 ; multiplier++) {
            if (bitmap & (1 << multiplier)) {
                break;
            }
        }
        idx = (multiplier * (SOC_MODID_MAX(unit) + 1)) + modid;
#endif
    } else {
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        if (SOC_IS_TR_VL(unit) && !SOC_IS_TRIUMPH2(unit) && 
            ...
            !SOC_IS_ENDURO(unit)) {
#ifdef BCM_ESW_SUPPORT
            rv = soc_xgs3_port_to_higig_bitmap(unit, portB, &hg_reg);
#endif
        }
...
traffic_test_alloc(int unit)
{
    ...
    PBMP_ITER(test->ports, port) {
        ...
        if (!(SOC_IS_EASYRIDER(unit) && SAL_BOOT_SIMULATION)) {
        ...
        if (IS_ST_PORT(unit, port) && SOC_IS_FBX(unit)) {
            ...
            if ((rv = READ_MODPORT_MAPm(unit, MEM_BLOCK_ANY, idx, 
                                        &test->saved_modport_map[port])) < 0) {
                printk("traffic_init: port %s: could not get modport map: %s\n",
                       SOC_PORT_NAME(unit, port), bcm_errmsg(rv));
            }
        }
    }
        ...
        PBMP_ITER(test->ports, port) {
           ...
           if(IS_FE_PORT(unit, port)) {
            ...
            if (IS_ST_PORT(unit, port) && SOC_IS_FBX(unit)) {
...
#ifdef BCM_ESW_SUPPORT
                    uint32 bitmap, multiplier = 0;
                    rv = soc_xgs3_port_to_higig_bitmap(unit, port, &bitmap);
                    for (multiplier = 0 ; multiplier < 9 ; multiplier++) {
                        if (bitmap & (1 << multiplier)) {
                            break;
                        }
                ...
                rv = WRITE_MODPORT_MAPm(unit, MEM_BLOCK_ANY, idx, 
...
traffic_test_send_frames(traffic_test_t* test, int port_pair_index)
{
    ...
    if (SOC_IS_XGS12_FABRIC(unit) ||
        ...
        ehdrA->en_tag_len  = soc_htons(test->pkt_size);
        ...
        ehdrB->en_tag_len  = soc_htons(test->pkt_size);
