/*
 * $Id: loopback2.c,v 1.1 2011/04/18 17:11:00 mruas Exp $
 ...
 */
{
    ...
    debugk(DK_TESTS, "lb_mac_test[%d]: Starting ....\n", unit);
    ...
    if (SOC_IS_EASYRIDER(unit)) { 
        /*
         * For HG-HG device, install L2 entry only for port hg0
         * Even though port 0 is treated as a stack port (hg) it behaves
         * like a front panel Ethernet port in some respects and does
         * lookups and the packet forwarding is based on the lookup
         * result.
         */
        if (IS_ST_PORT(unit, 0) && BCM_PBMP_MEMBER(pbm, 0)) {
            BCM_PBMP_PORT_ADD(tpbm, 0);
        }
    }
#endif
    if (BCM_PBMP_NOT_NULL(tpbm)) { 
         ...
         */
        debugk(DK_TESTS, "Setting up ARL\n");
 ...
 */
{
	...
	PBMP_ITER(lp->pbm, p) {
		...
		test_error(unit,
            ...
            if (SOC_IS_DRACO(unit) && IS_HG_PORT(unit, p)) {
