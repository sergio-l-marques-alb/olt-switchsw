/*
 * $Id: bcmx_port.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
                       ...
                       int *prio)
{
    ...
    if (port == BCMX_LPORT_ETHER_ALL) {
        ...
        BCMX_UNIT_ITER(bcm_unit, i) {
            tmp_rv = bcm_port_dscp_map_get(bcm_unit, bcm_port_all, 
                                           srccp, mapcp, prio);
            BCMX_RV_REPLACE_OK(tmp_rv, rv, BCM_E_UNAVAIL);
            if (BCM_SUCCESS(rv)) {
                return rv;
        ...
        rv = bcm_port_dscp_map_get(bcm_unit, bcm_port,
                                   srccp, mapcp, prio);
    }
...
bcmx_port_learn_get(bcmx_lport_t port, uint32 *flags)
{
    ...
    return bcm_port_learn_get(bcm_unit, bcm_port,
                              flags);
}
...
bcmx_port_learn_set(bcmx_lport_t port, uint32 flags)
{
  ...
  int         tmp_rv;
    ...
    if (BCM_SUCCESS(_bcmx_dest_to_unit_port(port, &bcm_unit, &bcm_port,
                                 BCMX_DEST_CONVERT_DEFAULT)))
    {
    return bcm_port_learn_set(bcm_unit, bcm_port,
                              flags);
}
    /* Virtual port, set in all units */
    BCMX_UNIT_ITER(bcm_unit, i) {
...
bcmx_port_vlan_member_get(bcmx_lport_t port, uint32 *flags)
{
    ...
    return bcm_port_vlan_member_get(bcm_unit, bcm_port,
                                    flags);
}
...
bcmx_port_vlan_member_set(bcmx_lport_t port, uint32 flags)
{
  ...
  if (BCM_SUCCESS(_bcmx_dest_to_unit_port(port, &bcm_unit, &bcm_port,
                               BCMX_DEST_CONVERT_DEFAULT)))
  {
    return bcm_port_vlan_member_set(bcm_unit, bcm_port,
                                    flags);
}
  /* Virtual port, set in all units */
  BCMX_UNIT_ITER(bcm_unit, i) {
                     ...
                     bcmx_lplist_t lplist)
{
    ...
    BCMX_UNIT_ITER(dest_unit, i) {
        ...
        rv = bcm_port_egress_set(dest_unit, src_port, (int)src_mod, pbmp);
        if (rv < 0) {
            return rv;
        }
    }
                     ...
                     bcmx_lplist_t *lplist)
{
    ...
    BCMX_UNIT_ITER(dest_unit, i) {
        rv = bcm_port_egress_get(dest_unit, src_port,
                                 (int)src_mod, &pbmp);
    if (rv == BCM_E_NONE) {
