/*
 * $Id: bcmx_bcmx.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
 ...
 * Returns:
 *     Pointer to port information struct, if successful.
 *     NULL, on failure to allocate struct.
 * Notes:
 *     Assumes lock is held.
 */
...
bcmx_lport_next(bcmx_lport_t lport)
{
    ...
    if (!BCMX_INIT() || !BCMX_IS_LPORT(lport)) {
        return BCMX_LPORT_INVALID;
    }
...
bcmx_device_attach(int bcm_unit)
{
    ...
    BCM_PBMP_ITER(config.all, port) {
        ...
        /* Add port */
        if (_bcmx_port_add(bcm_unit, port, lport[i++], port_flags) == NULL) {
            BCMX_CONFIG_UNLOCK;
            return BCM_E_RESOURCE;
        }
    }
    ...
    for (i  = 0; i < BCMX_PORT_HASH_COUNT; i++) {
        ...
        while (port_info != NULL) {
            BCMX_OUT((" %d: lport=0x%x unit=%d port=%d uport=%d flags=0x%x "
                      ...
                      port_info->port.uport, port_info->port.flags,
