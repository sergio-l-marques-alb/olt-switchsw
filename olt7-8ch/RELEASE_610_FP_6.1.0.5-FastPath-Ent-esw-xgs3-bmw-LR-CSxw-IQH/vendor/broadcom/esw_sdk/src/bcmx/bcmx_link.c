/*
 * $Id: bcmx_link.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#define LSX_CHECK_INIT if (lsx_lock == NULL) \
...
bcmx_linkscan_enable_set(int us)
{
    ...
    if (us < 0) {  /* Sync:  See notes above */
        ...
        /* Try to get current enable setting for some unit */
        BCMX_UNIT_ITER(bcm_unit, i) {
            rv = bcm_linkscan_enable_get(bcm_unit, &interval);
            if (rv == BCM_E_NONE) {
                found = TRUE;
                break;
            }
        ...
        BCMX_UNIT_ITER(bcm_unit, i) {
            ...
            if (tmp_rv == BCM_E_NONE) {
                *us = lsx_interval = tmp_us;
                break;  /* Found someplace, not initialized */
            }
        }
    }

    if (lsx_interval < 0) { /* Still unitialized */
        return BCM_E_FAIL;  /* Could not be determined */
    }
