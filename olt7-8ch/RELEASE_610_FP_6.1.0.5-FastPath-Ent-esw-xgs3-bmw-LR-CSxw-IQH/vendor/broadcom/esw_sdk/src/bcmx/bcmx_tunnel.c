/*
 * $Id: bcmx_tunnel.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#define BCMX_TUNNEL_RV_REPLACE(_trv, _rv)    \
...
bcmx_tunnel_terminator_add(bcmx_tunnel_terminator_t *info)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        if(BCM_PBMP_NOT_NULL(bcm_tunnel.pbmp)) {
            ...
            BCMX_TUNNEL_RV_REPLACE(tmp_rv, rv);
        }
    }
    return rv;
}
...
bcmx_tunnel_terminator_get(bcmx_tunnel_terminator_t *info)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        if (BCM_FAILURE(tmp_rv)) {
            ...
            tmp_rv = BCMX_LPLIST_PBMP_ADD(&info->ports, bcm_unit,
                                                  bcm_tunnel.pbmp);
            BCMX_TUNNEL_RV_REPLACE(tmp_rv, rv);
        }
    }
