/*
 * $Id: bcmx_mpls.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#define BCMX_MPLS_RV_REPLACE(_trv, _rv)    \
...
bcmx_mpls_init(void)
{
    ...
    BCMX_UNIT_ITER(bcm_unit, i) {
        ...
        BCMX_MPLS_RV_REPLACE(tmp_rv, rv);
    }
...
bcmx_mpls_port_block_set(bcm_vpn_t vpn, bcmx_lport_t lport, int enable)
{
    ...
    rv = bcm_mpls_port_block_set(bcm_unit, vpn, bcm_port, enable);
...
bcmx_mpls_port_block_get(bcm_vpn_t vpn, bcmx_lport_t lport, int *enable)
{
    ...
    rv = bcm_mpls_port_block_get(bcm_unit, vpn, bcm_port, enable);
...
bcmx_mpls_vpn_add(bcm_vpn_t vpn, bcmx_mpls_vpn_t *vpn_info)
{
    ...
    _bcmx_mpls_vpn_t_to_bcm(vpn_info, &bcm_vpn_info);
    unit = bcmx_lport_bcm_unit(vpn_info->lport);
    rv = bcm_mpls_vpn_add(unit, vpn, &bcm_vpn_info);
...
bcmx_mpls_vpn_delete(bcm_vpn_t vpn, bcmx_mpls_vpn_t *vpn_info)
{
    ...
    _bcmx_mpls_vpn_t_to_bcm(vpn_info, &bcm_vpn_info);
    unit = bcmx_lport_bcm_unit(vpn_info->lport);
    rv = bcm_mpls_vpn_delete(unit, vpn, &bcm_vpn_info);
 ...
 * Parameters:
 *      mswitch - (OUT)L3 MPLS label switching info
 * Returns:
 *      BCM_E_XXXX
 */
int
bcmx_mpls_switch_get(int max, bcmx_mpls_switch_t *mapping, int *count)
{
