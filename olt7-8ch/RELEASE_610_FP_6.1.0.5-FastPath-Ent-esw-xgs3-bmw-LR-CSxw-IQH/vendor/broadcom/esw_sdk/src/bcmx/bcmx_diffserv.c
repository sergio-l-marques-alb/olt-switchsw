/*
 * $Id: bcmx_diffserv.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
 ...
 * Notes
 *       Attempt to create for all units; return any error that occurs,
 *       but continue with later units.  On error, caller must unwind with
 *       datapath_delete if desired.
 *
 *       The DS lock is taken exactly when dsu != NULL
 * Returns
 *      BCM_E_FAIL means the DPID was not created and no units
 *      were added.  Other errors may mean some units were added,
 *      but others had failures.
 */
 ...
 *      units, ignoring not found and unavail errors.
 */
