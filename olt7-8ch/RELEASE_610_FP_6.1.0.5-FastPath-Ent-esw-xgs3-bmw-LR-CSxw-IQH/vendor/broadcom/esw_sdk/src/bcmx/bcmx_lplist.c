/*
 * $Id: bcmx_lplist.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#define _MIN_COUNT 16

/* This is the increment when reallocating */
#define _MIN_INCREMENT 16
 ...
 *      At least _MIN_COUNT ports will be allocated.
...
bcmx_lplist_add(bcmx_lplist_t *list, bcmx_lport_t lport)
{
    ...
    if (BCMX_LPLIST_COUNT(list) + 1 >= list->lp_alloc) {
        if (_bcmx_lplist_realloc(list, list->lp_alloc + _MIN_INCREMENT)
            != BCM_E_NONE) {
            return BCM_E_MEMORY;
        }
    }
...
bcmx_lplist_check(bcmx_lplist_t *list)
{
    ...
    if (list->lp_alloc > _ABSOLUTE_MAX_LEN) {
        return -4;
    }
