/*
 * $Id: idxres_mdb.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
 ...
 *  pessimal' for a given maximum block size -- for example, 4096 byte banks
 *  provides the least waste for the case of largest block = 132 bytes.
 *
 ...
 *  and 2048 element block free lists), though this will have minimal impact if
 *  not using the join on alloc feature.
 *
 ...
 *  depending upon whether the element is the head of a block or oen of the
                       ...
                       shr_mdb_elem_index_t second)
{
    ...
    res->elem[second + newCount - 1].count = oldCount - newCount;
}
                    ...
                    shr_mdb_elem_index_t *head)
{
    ...
    if (_MDB_BLOCK_NOT_HEAD != res->elem[*head].list) {
        ...
        MDB_VVERB((MDB_MSG("(%08X,&(%08X)) early leave (head)\n"),
                   (uint32)res,
                   *head));
        return;
    }
    if (0 != res->elem[*head].count) {
        ...
        *head -= (res->elem[*head].count - 1);
        MDB_VVERB((MDB_MSG("(%08X,&(%08X)) early leave (tail)\n"),
                   (uint32)res,
                   *head));
        return;
    }
                        ...
                        shr_mdb_elem_bank_index_t count)
{
    ...
    if ((currElem > block) ||
        ...
        _mdb_block_frag_and_free(res, currElem);
        MDB_ERR((MDB_MSG1("mdb %08X: unable to alloc %d element block at %08X"
               ...
               int lock)
{
    ...
    tempList->lastBankSize = elems - (banks * bankSize);
                   ...
                   shr_mdb_block_info_t *blockInfo)
{
    ...
    if (BCM_E_NONE == result) {
        ...
        if (handle->elem[block].list < handle->freeLists) {
    ...
    if (BCM_E_NONE == result) {
        ...
        blockInfo->list = handle->elem[block].list;
        blockInfo->size = handle->elem[block].count;
    }
