/*
 * $Id: idxres_afl.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
 ...
 *  things like printing status.  I plan to take these out or replace them with
 *  some appropriate debugging output; for now, they're for my purposes only.
 *  Defining IDXRES_DEBUG will violate rules against using stdio, while making
 *  sure it's undefined will inhibit the code that uses stdio at compile time.
 *
 *  If I leave this in, the printf call will be replaced by an appropriate
 *  macro, but I'm not sure what's appropriate.
 *
...
#undef AIDXRES_DEBUG
#ifdef AIDXRES_DEBUG
#define AIDXRES_DUMP(stuff) sal_printf stuff
...
_aidxres_dump_list(const shr_aidxres_list_handle_t list){
    ...
    unsigned int _index;
    unsigned int _max;
    AIDXRES_DUMP(("Resource list attributes\n"));
    AIDXRES_DUMP(("   first = %08X   last = %08X   low = %08X   high = %08X\n",
                  list->first,
                  list->last,
                  list->valid_low,
                  list->valid_high));
    AIDXRES_DUMP(("    free = %08X  alloc = %08X  bMax = %08X  bRMax = %08X\n",
                  list->free_count,
                  list->alloc_count,
                  list->bank_max,
                  list->bank_rem_max));
    AIDXRES_DUMP(("    sCnt = %08X",
                  list->sublist_count));
    for (_sublist = 0; _sublist < list->sublist_count; _sublist++) {
        AIDXRES_DUMP(("\nFree banks sublist %02X", _sublist));
        AIDXRES_DUMP(("    head=%08X  free=%08X",
