/*
 * $Id: common_link.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
#define LS_TRUNK_SW_FAILOVER_TRIGGER(_u, _a, _s) \
    _LS_CALL(LS_CONTROL_DRV(_u), ld_trunk_swfailover_trigger, \
             ((_u), (_a), (_s)))
...
_bcm_linkscan_hw_interrupt(int unit)
{
    ...
    LS_HW_INTERRUPT(unit, &tmp_pbmp);

    /* Apply to HW link scanned ports only */
    BCM_PBMP_AND(tmp_pbmp, lc->pbm_hw);

    if (BCM_PBMP_NEQ(lc->pbm_hw_upd, tmp_pbmp)) {
        /* Update set of ports to be re-scanned */
        BCM_PBMP_ASSIGN(lc->pbm_hw_upd, tmp_pbmp);
        lc->hw_change = 1;

        /*
         * Wake up master thread to notice changes - required if using 
         * hardware link scan 
         */
        if (lc->sema != NULL) {
...
_bcm_linkscan_thread(int unit)
{
    ...
    while ((interval = lc->interval_us) != 0) {
        ...
        if (lc->hw_change) {
            ...
            BCM_PBMP_ASSIGN(change, PBMP_PORT_ALL(unit));
            BCM_PBMP_AND(change, lc->pbm_hw_upd);
            BCM_PBMP_XOR(change, lc->pbm_link);
            BCM_PBMP_REMOVE(change, PBMP_CMIC(unit));
            ...
            /* Check to make sure that CMIC reported link status is 
             * actually in sync with external or internal PHY.
             */

            /* Don't care override links */
            BCM_PBMP_NEGATE(change, lc->pbm_override_link);

            /* Get SW state of HW linkscanned ports */
            BCM_PBMP_ASSIGN(sw_link, lc->pbm_link);
            BCM_PBMP_AND(sw_link, lc->pbm_hw);
            BCM_PBMP_AND(sw_link, change);

            /* Get HW value (CMIC_LINK_STAT) */
            BCM_PBMP_ASSIGN(hw_link, lc->pbm_hw_upd);
            BCM_PBMP_AND(hw_link, change);

            if (BCM_PBMP_NEQ(sw_link, hw_link)) {
#if defined(BROADCOM_DEBUG)
                char pfmt[SOC_PBMP_FMT_LEN];
                LINK_VERB(("sw scan = %s\n", SOC_PBMP_FMT(lc->pbm_sw, pfmt)));
                LINK_VERB(("sw link = %s\n", SOC_PBMP_FMT(sw_link, pfmt)));
                LINK_VERB(("hw scan = %s\n", SOC_PBMP_FMT(lc->pbm_hw, pfmt)));
                LINK_VERB(("hw link = %s\n", SOC_PBMP_FMT(hw_link, pfmt)));
#endif /* BROADCOM_DEBUG */
                /* External PHY may be still auto-negotiating.
                 * Try scanning port status with SW linkscan. */ 
                interval = lc->interval_us;
            }
