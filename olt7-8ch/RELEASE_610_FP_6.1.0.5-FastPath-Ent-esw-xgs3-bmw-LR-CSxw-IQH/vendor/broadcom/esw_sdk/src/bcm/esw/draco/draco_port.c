/*
 * $Id: draco_port.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
                            ...
                            bcm_port_cfg_t *cfg)
{
    if (!SOC_IS_TUCANA(unit) && !SOC_IS_LYNX(unit)) {
        cfg->pc_new_pri = soc_PORT_TABm_field32_get(unit, pv, NEW_PRIf);
    }
    ...
    if (soc_l2x_frozen_cml_get(unit, port, &cfg->pc_cml) < 0) {
			    ...
			    bcm_port_cfg_t *cfg)
{
    ...
    if (!SOC_IS_TUCANA(unit) && !SOC_IS_LYNX(unit)) {
        soc_PORT_TABm_field32_set(unit, pv, NEW_PRIf, cfg->pc_new_pri);
    }
    ...
    if (soc_l2x_frozen_cml_set(unit, port, cfg->pc_cml, &repl_cml) < 0) {
...
bcm_draco_port_cfg_get(int unit, bcm_port_t port, bcm_port_cfg_t *cfg)
{
    ...
    if (IS_E_PORT(unit, port)) {
        ...
        if (SOC_IS_DRACO15(unit)) {
            ...
            if (SOC_IS_TUCANA(unit) || SOC_IS_LYNX(unit)) {
                cfg->pc_new_pri = soc_reg64_field32_get(unit, PRTABLE_DEFAULTr,
                                                        prreg, PRIf);
            }
#endif
        }
