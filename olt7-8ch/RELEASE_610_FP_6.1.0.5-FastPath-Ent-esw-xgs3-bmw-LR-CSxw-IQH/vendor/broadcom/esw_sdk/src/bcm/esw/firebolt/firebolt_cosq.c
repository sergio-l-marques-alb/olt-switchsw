/*
 * $Id: firebolt_cosq.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
    ...
    defined(BCM_FELIX_SUPPORT)  || defined(BCM_FIREBOLT2_SUPPORT)
	    ...
	    /* per-queue packet red and yellow drop */
            ...
            if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) ||
                SOC_IS_HAWKEYE(unit)) { /* Granularity limitation */
    ...
    if (soc_feature(unit,soc_feature_bucket_support)) {
...
#endif /* BCM_FIREBOLT2_SUPPORT || BCM_RAVEN_SUPPORT */
        {
            ...
            soc_reg_field_set(unit, regindex, &regval,
                              MIN_REFRESHf, refresh_rate);
#ifdef LVL7_FIXUP
            /* We want as small a burst size as possible... */
            soc_reg_field_set(unit, regindex,
                              &regval, MIN_HI_THD_SELf, kbits_sec_min ? 2 : 0);
            soc_reg_field_set(unit, regindex,
                              &regval, MIN_LO_THD_SELf, kbits_sec_min ? 1 : 0);
#else
            soc_reg_field_set(unit, regindex,
                             ...
                             int drop_slope)
{
    ...
    int i, is_local, cosq_start, num_cosq;
    uint32 min_thresh, n;
    ...
    if (BCM_GPORT_IS_SET(port)) {
        BCM_IF_ERROR_RETURN
            (_bcm_esw_gport_resolve(unit, port, &modid,
    ...
    for (i = cosq_start; i < (cosq_start + num_cosq); i++) { 
        ...
        BCM_IF_ERROR_RETURN(soc_reg_field32_modify
                           (unit, SIMPLEREDCONFIGr, local_port, _cosfld[i], 1));
                             ...
                             int *drop_slope)
{
    ...
    BCM_IF_ERROR_RETURN(READ_SIMPLEREDCONFIGr(unit, local_port, &rval));                          if (soc_reg_field_get(unit, SIMPLEREDCONFIGr, rval, _cosfld[cosq])) {
        if (color == BCM_COSQ_DISCARD_COLOR_GREEN) {
            BCM_IF_ERROR_RETURN(READ_CNGCOSPKTLIMIT1r(unit, port, cosq, &rval));
            *drop_start = soc_reg_field_get(unit, CNGCOSPKTLIMIT1r, rval, 
                                            CNGPKTSETLIMIT1f) * 100 / n; 
            BCM_IF_ERROR_RETURN(READ_CNG1COSDROPRATEr(unit, port, cosq, &rval));
            *drop_slope = -1 * _bcm_rv_hw_drop_prob_to_api_val[rval];
        } else {
            BCM_IF_ERROR_RETURN(READ_CNGCOSPKTLIMIT0r(unit, port, cosq, &rval));
            *drop_start = soc_reg_field_get(unit, CNGCOSPKTLIMIT0r, rval, 
                                            CNGPKTSETLIMIT0f) * 100 / n; 
            BCM_IF_ERROR_RETURN(READ_CNG0COSDROPRATEr(unit, port, cosq, &rval));
            *drop_slope = -1 * _bcm_rv_hw_drop_prob_to_api_val[rval];
        }
    } 
    return BCM_E_NONE;
