/* $Id: triumph_field.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
 ...
 */
#include <soc/defs.h>
#if defined(BCM_TRIUMPH_SUPPORT) && defined(BCM_FIELD_SUPPORT)
...
STATIC soc_tcam_partition_type_t _fp_tcam_partitions[] =  { 
...
STATIC soc_mem_t ext_counter_mems[] = {
...
_bcm_field_tr_hw_clear(int unit, _field_stage_t *stage_fc)
{
    ...
    switch (stage_fc->stage_id) {
      case _BCM_FIELD_STAGE_INGRESS:
          ...
          BCM_IF_ERROR_RETURN(rv);
          rv = soc_mem_clear(unit, FP_STORM_CONTROL_METERSm, COPYNO_ALL, TRUE);
          BCM_IF_ERROR_RETURN(rv);
      ...
      case _BCM_FIELD_STAGE_EXTERNAL:
          ...
          for (idx = 0; idx < stage_fc->tcam_slices; idx++) {
              if (ext_data_mask_mems[idx] != 0) {
                  rv = soc_mem_clear((unit), ext_data_mask_mems[idx], 
                                     COPYNO_ALL, TRUE);
                  BCM_IF_ERROR_RETURN(rv);
              }
              if (ext_data_mems[idx] != 0) {
                  rv = soc_mem_clear((unit), ext_data_mems[idx], 
                                     COPYNO_ALL, TRUE);
                  BCM_IF_ERROR_RETURN(rv);
              }
              if (ext_mask_mems[idx] != 0) {
                  rv = soc_mem_clear((unit), ext_mask_mems[idx], COPYNO_ALL, TRUE);
                  BCM_IF_ERROR_RETURN(rv);
              }
              rv = soc_mem_clear((unit), ext_policy_mems[idx], COPYNO_ALL, TRUE);
              BCM_IF_ERROR_RETURN(rv);
          }
...
_bcm_field_tr_external_init(int unit, _field_stage_t *stage_fc)
{
    ...
    for (i = 0; i < stage_fc->tcam_slices; i++) {
        ...
        size = soc_mem_index_count(unit, ext_policy_mems[i]);
        fs->entry_count = size;
        fs->free_count  = size;
        if (size) {
    ...
    rv = soc_profile_mem_create(unit, &mem, 1, &stage_fc->ext_act_profile);
    if (BCM_FAILURE(rv)) {
        _bcm_field_stage_entries_free(unit, stage_fc);
        return (rv);
    }
                                        ...
                                        int  *idx)
{
    ...
    return soc_tcam_part_index_to_mem_index(unit,
                                            _fp_tcam_partitions[slice_number],
...
_field_tr_external_tcam_write(int unit, _field_entry_t *f_ent, int index)
{
    ...
    switch (slice) {
      ...
      case _FP_EXT_ACL_144_IPV6:
          ...
          soc_mem_field_set(unit, ext_data_mask_mems[slice], 
                            buf, DATAf, tcam->key_hw);
          /* 
           * External TCAM: mask polarity is reversed.
           *     Use this instead of soc_mem_field_set
           */
          soc_mem_mask_field_set(unit, ext_data_mask_mems[slice], 
                                 buf, MASKf, tcam->mask_hw);

          rv = soc_mem_write(unit, ext_data_mask_mems[slice], 
      ...
      case _FP_EXT_ACL_L2_IPV6:
           ...
           *     Use this instead of soc_mem_field_set
           */
          soc_mem_mask_field_set(unit, ext_mask_mems[slice], 
                                 buf, MASKf, tcam->mask_hw);
          rv = soc_mem_write(unit, ext_mask_mems[slice], 
                             MEM_BLOCK_ALL, 0, buf);
          BCM_IF_ERROR_RETURN(rv);

          /* Write the data */
          sal_memset(buf, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
          soc_mem_field_set(unit, ext_data_mems[slice], 
                            buf, DATAf, tcam->key_hw);

          rv =  soc_mem_write(unit, ext_data_mems[slice], 
                              MEM_BLOCK_ALL, index, buf);
          BCM_IF_ERROR_RETURN(rv);
          break;
      default:
                                       ...
                                       _field_entry_t *f_ent)
{
    ...
    policy_mem = ext_policy_mems[f_ent->fs->slice_number];
    ...
    for (fa = f_ent->actions; fa != NULL; fa = fa->next) {
        switch (fa->action) {
          ...
          case bcmFieldActionGpVlanCosQNew:
              rv = _bcm_field_trx_action_get(unit, EXT_IFP_ACTION_PROFILEm, f_ent, 
                                             f_ent->slice_idx, fa, ext_act_profile);
              BCM_IF_ERROR_RETURN(rv);
              break;
          default:
              rv = _bcm_field_trx_action_get(unit, policy_mem, f_ent, 
                                      ...
                                      _field_entry_t *f_ent)
{
    ...
    BCM_IF_ERROR_RETURN(soc_tcam_part_index_to_mem_index(unit, 
                            _fp_tcam_partitions[f_ent->fs->slice_number], 
...
_field_tr_external_entry_clear(int unit, int slice, int index)
{
    ...
    switch (slice) {
      ...
      case _FP_EXT_ACL_L2_IPV6:
          ...
          BCM_IF_ERROR_RETURN(rv);
          /* Write the data */
          soc_mem_write(unit, ext_data_mems[slice], 
                        MEM_BLOCK_ALL, index, buf);
          BCM_IF_ERROR_RETURN(rv);
          break;
      default:
    ...
    /* Clear policy entry */
    rv =  soc_mem_write(unit, ext_policy_mems[slice],
                        MEM_BLOCK_ALL, index, buf);
    return (BCM_E_NONE);
}
                                       ...
                                       int index1, int index2)
{
    ...
    uint32  policy[SOC_MAX_MEM_FIELD_WORDS];
    soc_mem_t policy_mem;
    int counter_index1, counter_index2;
    int rv;
...
_bcm_field_tr_external_counters_collect(int unit, _field_stage_t *stage_fc)
{
    ...
    while (soc_mem_index_max(unit, ext_counter_mems[table_id]) == -1) {
        /* Skip Empty tables */
    ...
    for (idx = min_idx; idx <= max_idx; idx += _FP_64_COUNTER_BUCKET) {
        ...
        soc_tcam_part_index_to_mem_index(unit, _fp_tcam_partitions[table_id], 
                                         idx, EXT_FP_CNTRm, 
                                         &unified_start_index);
    ...
    if (retval == BCM_E_NONE) {
        if (max_idx >= (soc_mem_index_max(unit, ext_counter_mems[table_id]))) {
                      ...
                      int idx, uint64 *packet_count, uint64 *byte_count)
{
    ...
    hw_val[0] = hw_val[1] = 0;
    cntrs64_buf = &stage_fc->_field_x64_counters[idx];
    soc_mem_field_get(unit, counter_x_mem, mem_x_buf, BYTE_COUNTERf, hw_val);
    ...
    /* Packet counter. */
    cntrs32_buf = &stage_fc->_field_x32_counters[idx];
    soc_mem_field_get(unit, counter_x_mem, mem_x_buf, PACKET_COUNTERf, hw_val);
