/*
 * $Id: draco_l2.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
_bcm_draco_age_reg_config(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
    ...
    if (rep_st->isTrunk) {
        match_mod = BCM_TRUNK_TO_MODIDf(unit, rep_st->match_trunk);
        match_port = BCM_TRUNK_TO_TGIDf(unit, rep_st->match_trunk);
        if (sync_op == SOC_L2X_PORTMOD_DEL) {
            sync_op = SOC_L2X_TRUNK_DEL;
        }
    } else {
        match_mod = rep_st->match_module;
        match_port = rep_st->match_port;
    }
    ...
    rv = soc_l2x_port_age(unit, age_val, INVALIDr, 0);
    if (BCM_FAILURE(rv)) {
        SOC_L2_DEL_SYNC_UNLOCK(soc);
        return rv;
    }
 ...
 */
STATIC int 
_bcm_draco_repl_reg_config(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
        ...
        for (ix = 0; ix < chunksize; ix++) {
            ...
            if (dest_match) {
                ...
                if (rep_st->isTrunk) {
                    ...
                    if (rep_st->match_module != 
                           soc_L2Xm_field32_get(unit, l2xep, MODULE_IDf)) {
                        continue;
                    }
                    if (port_tgid != rep_st->match_port) {
                        continue;
                    }
                }
            }
...
_bcm_draco_l2_addr_replace_by_vlan_dest(int unit, uint32 flags, _bcm_l2_replace_t *rep_st)
{
    ...
    if (0 == (flags & BCM_L2_REPLACE_DELETE)) {
        if (SOC_IS_DRACO15(unit)) {
            BCM_IF_ERROR_RETURN(
                _bcm_draco_repl_reg_config(unit, flags, rep_st));
        } else {
...
int
bcm_draco_l2_addr_get(int unit, mac_addr_t mac, bcm_vlan_t vid,
                      bcm_l2_addr_t *l2addr)
{
