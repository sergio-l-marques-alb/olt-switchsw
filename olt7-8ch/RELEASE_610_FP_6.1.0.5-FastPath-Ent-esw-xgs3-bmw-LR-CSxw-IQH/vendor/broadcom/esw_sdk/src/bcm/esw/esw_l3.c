/*
 * $Id: esw_l3.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
/* #ifdef BCM_XGS_SWITCH_SUPPORT
    if (SOC_IS_DRACO15(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_xgs_l3_reinit(unit));
    }
   #endif */
...
bcm_esw_l3_host_find(int unit, bcm_l3_host_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        if (info->l3a_ip_addr == INADDR_NONE) {
            L3_UNLOCK(unit);
...
#ifdef BCM_TRIUMPH2_SUPPORT
    if (soc_feature(unit, soc_feature_wlan) && 
        BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, info->l3a_intf)) {
        /* Get DVP using the egress object ID and construct a gport */
        egr_l3_next_hop_entry_t next_hop;
        _bcm_gport_dest_t dest;
        int nh_idx = info->l3a_intf - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        BCM_IF_ERROR_RETURN
            (READ_EGR_L3_NEXT_HOPm(unit, MEM_BLOCK_ANY, nh_idx, &next_hop));
        dest.wlan_id = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &next_hop, DVPf);
        dest.gport_type = _SHR_GPORT_TYPE_WLAN_PORT;
...
bcm_esw_l3_host_add(int unit, bcm_l3_host_t *info)
{
    ...
    if ((info->l3a_lookup_class > SOC_ADDR_CLASS_MAX(unit)) || 
        (info->l3a_lookup_class < 0)) {
        return (BCM_E_PARAM);
    }

    if(!BCM_PRIORITY_VALID(info->l3a_pri)) {
        return (BCM_E_PARAM);
    }
    if (BCM_GPORT_IS_SET(info->l3a_port_tgid)) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_esw_l3_gport_resolve(unit, info->l3a_port_tgid, 
                                      &(info->l3a_port_tgid),
                                      &(info->l3a_modid), 
                                      &(info->l3a_port_tgid), 
                                      &(info->l3a_flags)));
    } else {
        PORT_DUALMODID_VALID(unit, info->l3a_port_tgid);
    }
    /* Check if host entry already exists. */
    L3_LOCK(unit);
    info_orig = *info;
    rv = bcm_esw_l3_host_find(unit, &info_orig);
    if ((BCM_SUCCESS(rv)) && (0 == (info->l3a_flags & BCM_L3_REPLACE))) {
    ...
    sal_memset(&l3cfg, 0, sizeof(_bcm_l3_cfg_t));
    sal_memcpy(l3cfg.l3c_mac_addr, info->l3a_nexthop_mac, sizeof(bcm_mac_t));
    l3cfg.l3c_intf = info->l3a_intf;
    l3cfg.l3c_modid = info->l3a_modid;
    l3cfg.l3c_port_tgid = info->l3a_port_tgid;
    l3cfg.l3c_prio = info->l3a_pri;
    l3cfg.l3c_flags = info->l3a_flags;
    l3cfg.l3c_vrf = info->l3a_vrf;
    l3cfg.l3c_lookup_class = info->l3a_lookup_class;
...
bcm_esw_l3_route_add(int unit, bcm_l3_route_t *info)
{
    ...
    sal_memcpy(defip.defip_mac_addr, info->l3a_nexthop_mac,
               sizeof(bcm_mac_t));
    defip.defip_intf       = info->l3a_intf;
    defip.defip_modid      = info->l3a_modid;
    defip.defip_port_tgid  = info->l3a_port_tgid;
    defip.defip_vid        = info->l3a_vid;
    defip.defip_flags      = info->l3a_flags;
    defip.defip_prio       = info->l3a_pri;
    defip.defip_vrf        = info->l3a_vrf;
    defip.defip_tunnel_option = (info->l3a_tunnel_option & 0xffff);
    defip.defip_mpls_label = info->l3a_mpls_label;
    defip.defip_lookup_class      = info->l3a_lookup_class;
                         ...
                         bcm_if_t *intf)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        if (BCM_GPORT_IS_SET(egr->port)) {
            if (BCM_GPORT_IS_WLAN_PORT(egr->port)) {
                /* Resolve the WLAN gport and store the VP in egr->encap_id */
                rv = _bcm_esw_gport_resolve(unit, egr->port, &(egr->module), 
                                            &(egr->port), &(egr->trunk), 
                                            &(egr->encap_id));  
            } else {
                rv = _bcm_esw_l3_gport_resolve(unit, egr->port, &(egr->port), 
                                               &(egr->module), &(egr->trunk), 
                                               &(egr->flags));
            }
            BCM_IF_ERROR_RETURN(rv);
        } else {
            PORT_DUALMODID_VALID(unit, egr->port);
        }
        L3_LOCK(unit);
        rv = bcm_xgs3_l3_egress_create(unit, flags, egr, intf);
        L3_UNLOCK(unit);
    }
#endif  /* BCM_XGS3_SWITCH_SUPPORT */
    return (rv);
...
bcm_esw_l3_egress_get (int unit, bcm_if_t intf, bcm_l3_egress_t *egr) 
{
    ...
    if (soc_feature(unit, soc_feature_wlan) && 
        ...
        egr_l3_next_hop_entry_t next_hop;
        _bcm_gport_dest_t dest;
        int nh_idx = intf - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        BCM_IF_ERROR_RETURN
            (READ_EGR_L3_NEXT_HOPm(unit, MEM_BLOCK_ANY, nh_idx, &next_hop));
        dest.wlan_id = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &next_hop, DVPf);
        dest.gport_type = _SHR_GPORT_TYPE_WLAN_PORT;
        rv = _bcm_esw_gport_construct(unit, &dest, &(egr->port));
    } else
#endif
    {
                                 ...
                                 bcm_if_t *intf)
{
    ...
    if (SOC_IS_XGS3_SWITCH(unit) && soc_feature(unit, soc_feature_l3)) {
        ...
        L3_LOCK(unit);
        rv = bcm_xgs3_l3_egress_find(unit, egr, intf);
        L3_UNLOCK(unit);
    }
#endif  /* BCM_XGS3_SWITCH_SUPPORT */
    return (rv);
...
STATIC int
_bcm_vrf_flex_stat_hw_index_set(int unit, uint32 handle,
                               ...
                               _bcm_flex_stat_t *fs_arr)
{
    ...
    if ((nstat <= 0) || (nstat > _bcmFlexStatIngressBytes)) {
        return BCM_E_PARAM;
    }
...
bcm_esw_l3_source_bind_add(int unit, bcm_l3_source_bind_t *info)
{
    ...
    if (info->rate_id != 0) {
        ...
        soc_VLAN_MACm_field32_set(unit, &vment,
                      MAC_IP_BIND__IPFIX_FLOW_METER_IDf, info->rate_id);
    }
                                            ...
                                            bcm_l3_source_bind_t *info)
{
    ...
    info->rate_id = soc_VLAN_MACm_field32_get(unit, vment,
                                  MAC_IP_BIND__IPFIX_FLOW_METER_IDf);
