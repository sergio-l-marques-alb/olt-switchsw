/*
 * $Id: easyrider_mpls.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
			     ...
			     bcm_mpls_circuit_t *vc_info)
{
    ...
    if (vc_info->flags & BCM_MPLS_DST_TRUNK) {
        ...
        l2ue.dest_modid = BCM_TRUNK_TO_MODIDf(unit, vc_info->dst_trunk);
        l2ue.dest_trunk = BCM_TRUNK_TO_TGIDf(unit, vc_info->dst_trunk);
        l2ue.flags |= BCM_L2_CACHE_TRUNK;
    } else { 
