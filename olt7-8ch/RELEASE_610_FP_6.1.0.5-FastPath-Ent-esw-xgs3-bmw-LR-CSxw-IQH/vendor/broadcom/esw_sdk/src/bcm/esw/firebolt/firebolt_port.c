/*
 * $Id: firebolt_port.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
                          ...
                          bcm_port_cfg_t *port_cfg)
{
    ...
    if (soc_l2x_frozen_cml_get(unit, port, &port_cfg->pc_cml) < 0) {
    ...
    } else if (soc_mem_field32_get(unit, mem, &ptab_entry, MIRRORf)) {
...
_bcm_xgs3_port_table_write(int unit, bcm_port_t port,
                           bcm_port_cfg_t *port_cfg)
{
    int repl_cml, cml;             /* CPU managed learning mode. */   
    port_tab_entry_t ptab_entry;   /* Port table entry.          */
    soc_mem_t mem = PORT_TABm;     /* Port table memory.         */
    int tbl_index = port;          /* Port table index.          */
    int rv;
     ...
     */
    if (soc_l2x_frozen_cml_set(unit, port, port_cfg->pc_cml, &repl_cml) < 0) {
        cml = port_cfg->pc_cml;
    } else {
        cml = repl_cml;
    }
...
#ifdef BCM_TRX_SUPPORT
    if (SOC_IS_TRX(unit)) {
        soc_mem_field32_set(unit, mem, &ptab_entry, CML_FLAGS_MOVEf, cml);
    ...
    if (IS_CPU_PORT(unit, port)) {
        if (SOC_MEM_FIELD_VALID(unit, IPORT_TABLEm, HIGIG_PACKETf)) {
            soc_IPORT_TABLEm_field32_set(unit, &ptab_entry, HIGIG_PACKETf, 1);
        }
        if (SOC_MEM_FIELD_VALID(unit, IPORT_TABLEm, PORT_TYPEf)) {
            soc_IPORT_TABLEm_field32_set(unit, &ptab_entry, PORT_TYPEf, 1);
        }
        BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, IPORT_TABLEm, port,
                                               &ptab_entry));
    }
 ...
 *      unit       - (IN)SOC unit number.
 *      port       - (IN)Port number.
 *      port_cfg   - (OUT) API port info structure
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_xgs3_trunk_table_read(int unit, bcm_port_t port,
                           bcm_port_cfg_t *port_cfg)
{
    int    src_trk_idx;                            /*Source Trunk table index.*/
    source_trunk_map_table_entry_t trunk_map_entry;/*Trunk table entry buffer.*/

    /* Initialization. */
    src_trk_idx = 0;
    sal_memset(&trunk_map_entry, 0, sizeof(source_trunk_map_table_entry_t));

    /* Get module id for unit. */
    BCM_IF_ERROR_RETURN(bcm_esw_stk_my_modid_get(unit, &src_trk_idx));

    /* Calculate index to trunk map table. */
#ifdef BCM_TRX_SUPPORT
    if (soc_feature(unit, soc_feature_trunk_group_overlay)) {
        src_trk_idx = (src_trk_idx * (SOC_PORT_ADDR_MAX(unit) + 1)) + port;
    } else
#endif
    {
        src_trk_idx = (src_trk_idx << SOC_TRUNK_BIT_POS(unit)) + port;
    }
    ...
    /* Check if port is part of trunk */
    port_cfg->pc_trunk = 
        (1 == soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                                  &trunk_map_entry, PORT_TYPEf));

    /* Get trunk group id. */
    port_cfg->pc_tgid = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
 ...
 * Purpose:
 *      Write trunk table entry from API structure.
 * Parameters:
 *      unit       - (IN)SOC unit number.
 *      port       - (IN)Port number.
 *      port_cfg   - (IN) API port info structure
 * Returns:
 *      BCM_E_XXX
 */
STATIC int
_bcm_xgs3_trunk_table_write(int unit, bcm_port_t port,
                            bcm_port_cfg_t *port_cfg)
{
    int    src_trk_idx;                            /*Source Trunk table index.*/
    source_trunk_map_table_entry_t trunk_map_entry;/*Trunk table entry buffer.*/
    ...
    soc_mem_field32_set(unit, SOURCE_TRUNK_MAP_TABLEm, &trunk_map_entry,
                        TGIDf, port_cfg->pc_tgid);

    /* Set port is part of trunk flag. */
    soc_mem_field32_set(unit, SOURCE_TRUNK_MAP_TABLEm, &trunk_map_entry,
                        PORT_TYPEf, ((port_cfg->pc_trunk == 1) ? 1 : 0));

    /* Write entry to hw. */
    SOC_IF_ERROR_RETURN(BCM_XGS3_MEM_WRITE(unit, SOURCE_TRUNK_MAP_TABLEm,
                                           src_trk_idx, &trunk_map_entry));
...
_bcm_fb_port_cfg_init(int unit, bcm_port_t port, bcm_vlan_data_t *vlan_data)
{
    ...
    soc_field_t reg_fields[2] = {HIGIG_PACKETf, EN_EFILTERf};
    uint32 reg_values[2] = {0, 1};
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        ...
        reg_fields[0] = PORT_TYPEf;
    } else if (SOC_IS_ENDURO(unit)) {
        egr_port_reg = EGR_PORTr;
        iegr_port_reg = IEGR_PORTr;
        reg_fields[0] = PORT_TYPEf;
    }
#endif

    /* Set port is higig flag. */
    if (IS_ST_PORT(unit,port)) {
        reg_values[0] = 1;
    }

    /* Configure internal loopback port */
    ...
    BCM_IF_ERROR_RETURN 
        (soc_reg_fields32_modify(unit, egr_port_reg, port,
                                 sizeof(reg_fields) /sizeof(int),
    ...
    if (SOC_IS_TRX(unit)) {
        ...
        BCM_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, egr_port_reg, port, 
                                    VT_PORT_GROUP_IDf, port));
    }
#endif

#if defined(BCM_RAPTOR_SUPPORT)
    /* for Raptor directed mirroring should always be 1 */
    if (SOC_IS_RAPTOR(unit) || SOC_IS_RAVEN(unit) || SOC_IS_HAWKEYE(unit)) {
        BCM_IF_ERROR_RETURN 
            (soc_reg_field32_modify(unit, egr_port_reg, 
                                    port, EM_SRCMOD_CHANGEf, 1));
    }
#endif
    ...
    /* Set port type. */
    soc_PORT_TABm_field32_set(unit, &ptab, reg_fields[0], reg_values[0]);
    ...
    if (IS_CPU_PORT(unit,port)) {
        ...
        soc_IPORT_TABLEm_field32_set(unit, &ptab, reg_fields[0], 1);

#ifdef BCM_TRIUMPH_SUPPORT
        if (SOC_IS_TR_VL(unit) || SOC_IS_TRIUMPH2(unit)) {
            soc_IPORT_TABLEm_field32_set(unit, &ptab, DUAL_MODID_ENABLEf, 
                                         dual_modid);
        }
#endif
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, IPORT_TABLEm,
                                          MEM_BLOCK_ALL, port, &ptab));

        /* EGR_PORT and IEGR_PORT have identical formats */
        BCM_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, iegr_port_reg, port, reg_fields[0], 1));

#ifdef BCM_TRIUMPH_SUPPORT
        if (SOC_IS_TR_VL(unit)) {
            BCM_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, iegr_port_reg, port, 
                                        DUAL_MODID_ENABLEf, dual_modid));
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_RAVEN_SUPPORT) || defined (BCM_SCORPION_SUPPORT)
                           ...
                           uint32 *kbits_pause, uint32 *kbits_resume)
{
    ...
    switch (soc_reg_field_get(unit, BKPMETERINGCONFIGr,
                              regval, DISCARD_THDf)) {
      case 0:
          bucket = (pause * 7) / 4;
          break;
                           ...
                           uint32 *kbits_pause, uint32 *kbits_resume)
{
    ...
    uint32 granularity, rate, flags = 0;
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_RAPTOR_SUPPORT) || defined(BCM_SCORPION_SUPPORT)
        if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || 
            SOC_IS_HAWKEYE(unit) || SOC_IS_SC_CQ(unit)) {
            uint32 miscval;
...
bcm_xgs3_port_cfg_get(int unit, bcm_port_t port, bcm_port_cfg_t *port_cfg)
{
    ...
    /* Read trunk information. */
    BCM_IF_ERROR_RETURN(_bcm_xgs3_trunk_table_read(unit, port, port_cfg));
...
bcm_xgs3_port_cfg_set(int unit, bcm_port_t port, bcm_port_cfg_t *port_cfg)
{
    ...
    /* Write trunk table. */
    BCM_IF_ERROR_RETURN (_bcm_xgs3_trunk_table_write(unit, port, port_cfg));
...
_bcm_fb2_port_tpid_set(int unit, bcm_port_t port, uint16 tpid)
{
     ...
     * The fist insertion and second insertion will return
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        ...
        rv = READ_SYSTEM_CONFIG_TABLEm(unit, MEM_BLOCK_ANY, sys_index, &systab);
        if (BCM_SUCCESS(rv)) {
