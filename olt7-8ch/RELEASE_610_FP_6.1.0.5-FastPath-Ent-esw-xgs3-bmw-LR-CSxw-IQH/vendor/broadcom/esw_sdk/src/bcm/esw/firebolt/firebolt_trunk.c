/*
 * $Id: firebolt_trunk.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
#define _BCM_FABRIC_TRUNK_NUM_MAX   8
        ...
        for (; i <= imax; i++) {
            ...
            if (localcount > 0) {
                if (my_modid_bmap & (1 << ((i & 0xf) % member_count)))  {
                    SOC_PBMP_PORT_REMOVE(pbmp, localports[(i & 0xf) % localcount]);
                    index_count++;
                }
            }
                                    ...
                                    int hw_count, bcm_port_t *ftp)
{
    ...
    if (hw_count) { /* Enabling HG trunk failover */
        SOC_IF_ERROR_RETURN
            (soc_xgs3_port_num_to_higig_port_num(unit, failport, &hg_failport));
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        ...
        COMPILER_64_SET(oval64, COMPILER_64_HI(val64), COMPILER_64_LO(val64));
                                   ...
                                   int *array_count)
{
    ...
    if (SOC_PBMP_MEMBER(fail_ports, hg_failport)) {
        SOC_IF_ERROR_RETURN
...
_bcm_xgs3_trunk_hwfailover_read(int unit, bcm_port_t failport,
                                int array_size, bcm_port_t *ftp,
                                bcm_module_t *ftm, int *array_count)
{
    port_lag_failover_set_entry_t fail_entry;
    uint32 val;
...
{
    int tix, mod_is_local, failover_count, tid_ix;
    bcm_port_t tp_port;
    ...
    /* Record trunk data */
    hwft->num_ports = add_info->num_ports;
    for (tix = 0; tix < add_info->num_ports; tix++) {
        hwft->flags[tix] = add_info->flags;
        hwft->psc[tix] = add_info->psc;
        hwft->modport[tix] = add_info->tp[tix];
        if (!hg_trunk) {
            hwft->modport[tix] |= (add_info->tm[tix] << 8);
        }
    }

    /* No flags in trunk_set means no failover */
    failover_count = (add_info->flags) ? add_info->num_ports : 0;

    /* Configure new trunk info */
    for (tix = 0; tix < add_info->num_ports; tix++) {
        BCM_IF_ERROR_RETURN
            (_bcm_esw_modid_is_local(unit, add_info->tm[tix],
                                     &mod_is_local));
        if (!mod_is_local) {
            continue;
        }
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_trunk_hwfailover_set(unit, tid, hg_trunk, 
                       add_info->tp[tix], add_info->tm[tix],
                       add_info->psc, add_info->flags, failover_count,
                   ...
                   trunk_private_t *t_info)
{
    ...
    if (t_info->in_use) {
        ...
        for (i = 0; i < max_ports; i++) {
            mod = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                               _xgs_modulef[i]);
    ...
    for (i = 0; i < max_ports; i++) {
        ...
        soc_TRUNK_GROUPm_field32_set(unit, &tg_entry,
                                     _xgs_portf[i], port);
        if (i < add_info->num_ports) {
            BCM_IF_ERROR_RETURN(
                _bcm_xgs3_trunk_set_port_property(unit, mod, port, tid));
        }
    }
                       ...
                       trunk_private_t *t_info)
{
    ...
    bcm_port_t                  op[BCM_TRUNK_MAX_PORTCNT];
    int                         max_ports = BCM_SWITCH_TRUNK_MAX_PORTCNT;
    bcm_trunk_chip_info_t       chip_info;
    ...
    if (soc_feature(unit, soc_feature_trunk_group_size)) {        /* TG_SIZE field exists */
        i = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry, TG_SIZEf);
        tg_size = i + 1;
        for (i = 0; i < tg_size; i++) {
            mod = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                               _xgs_modulef[i]);
                           ...
                           trunk_private_t *t_info)
{
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
        ...
        for (i = 0; i < port_limit; i++)
        {
            ...
            BCM_IF_ERROR_RETURN(bcm_esw_stk_modmap_map(unit,
                   ...
                   trunk_private_t *t_info)
{
    ...
    if (soc_feature(unit, soc_feature_trunk_group_size)) {        /* TG_SIZE field exists */
        ...
        for (i = 0; i < t_data->num_ports; i++) {
            om[i] = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                                 _xgs_modulef[i]);
            op[i] = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                                 _xgs_portf[i]);
        }
    } else {
        /* need to discern the number or unique mod/port entries */
        ocount = 0;
        for (i = 0; i < max_ports; i++) {
            mod = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                               _xgs_modulef[i]);
                          ...
                          mac_addr_t mac, trunk_private_t *t_info)
{
    ...
    sal_memcpy(mc_addr.mac, mac, sizeof(mac_addr_t));
    mc_addr.vid = vid;
        ...
        for (k = j = 0; k < 2; k++) {
             ...
             if (nmp == 0) {
                 ...
                 if (SOC_IS_RAVEN(unit)) {
                     ...
                     BCM_IF_ERROR_RETURN(
                         soc_xgs3_port_to_higig_bitmap(unit, swft->modport[0], &p));
                     BCM_IF_ERROR_RETURN(
                         soc_xgs3_higig_bitmap_to_higig_port_num(p, &p));
                 }
             }
             ...
             for (i = 0; i < 4; i++) {
                  ...
                  if (nmp > 1) {
                      ...
                      if (SOC_IS_RAVEN(unit)) {
                          ...
                          BCM_IF_ERROR_RETURN(
                              soc_xgs3_port_to_higig_bitmap(unit, swft->modport[j], &p));
                          BCM_IF_ERROR_RETURN(
                              soc_xgs3_higig_bitmap_to_higig_port_num(p, &p));
                      }
                  }
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid < _BCM_FP_TRUNK_NUM_MAX + 1) {
            ...
            if (nmp == 0) {
                ...
                m = swft->modport[0] >> 8;
                p = swft->modport[0] & 0xff;
            }

            SOC_PBMP_CLEAR(new_trunk_pbmp);
            for (i = j = 0; i < max_ports; i++) {
                 if (m == swf->mymodid) {
                     SOC_PBMP_PORT_ADD(new_trunk_pbmp, p);
                 }
...
_xgs3_trunk_reinit(int unit, int ngroups_fp, trunk_private_t *trunk_info)
{
    ...
    if (SOC_IS_FB_FX_HX(unit) || SOC_IS_EASYRIDER(unit)) {
        ...
        bcm_port_t hw_ports[BCM_TRUNK_MAX_PORTCNT];
        trunk_group_entry_t tg_entry;
        bcm_module_t my_modid, hw_mods[BCM_TRUNK_MAX_PORTCNT];
        bcm_module_t om[BCM_TRUNK_MAX_PORTCNT];
        pbmp_t op[BCM_TRUNK_MAX_PORTCNT];
        int max_ports = BCM_SWITCH_TRUNK_MAX_PORTCNT;
        int i, num_mod, tgsize;
        ...
        for (tid = 0; tid < ngroups_fp; tid++, trunk_info++) {
             ...
             for (i = 0; i < max_ports; i++) {
                  ...
                  hw_ports[i] = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                                             _xgs_portf[i]);
                  hw_mods[i] = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                                            _xgs_modulef[i]);
                  for (j = 0; j < num_mod; j++) {
                       if (om[j] == hw_mods[i]) {
                           SOC_PBMP_PORT_ADD(op[j], hw_ports[i]);
                           found = 1;
                           break;
                       }
                  }
                  if (!found) {
                      SOC_PBMP_PORT_ADD(op[num_mod], hw_ports[i]);
                      om[num_mod++] = hw_mods[i];
                  }
             }
             ...
             for (i = 0; i < num_mod; i++) {
                  ...
                  if (my_modid == i) {
                      SOC_PBMP_OR(trunk_pbmp, op[i]);
                  }
                  SOC_PBMP_COUNT(op[i], num_ports);
                  t_data->num_ports += num_ports;
             }
             ...
             t_data->psc = soc_TRUNK_GROUPm_field32_get(unit, &tg_entry,
                                                        RTAGf);
             if (t_data->psc <= 0) {
                 t_data->psc = BCM_TRUNK_PSC_DEFAULT;
             }
                          ...
                          trunk_private_t *trunk_info)
{
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
         ...
         if (SOC_IS_FB_FX_HX(unit)) { 
             ...
             SOC_IF_ERROR_RETURN
                (READ_HG_TRUNK_BITMAPr(unit, tid, &hg_cntl_entry));
             SOC_PBMP_WORD_SET(hg_pbmp, 
                               0,
                               soc_reg_field_get(unit, 
                                                 HG_TRUNK_BITMAPr, 
         ...
         if (SOC_IS_FB_FX_HX(unit)) {
             ...
             if (SOC_PBMP_NOT_NULL(pbmp)) {
                 SOC_IF_ERROR_RETURN
                     (READ_HG_TRUNK_GROUPr(unit, tid, &hg_trunk_group));
                 ...
                 if (SOC_IS_SC_CQ(unit) &&
                     (t_data->num_ports > BCM_SWITCH_TRUNK_MAX_PORTCNT)) {
                     SOC_IF_ERROR_RETURN
                         (READ_HG_TRUNK_GROUP_HIr(unit, tid,
                                                  &hg_trunk_group_hi));
                     for (i = BCM_SWITCH_TRUNK_MAX_PORTCNT;
                          i < t_data->num_ports; i++) {
                         t_data->tm[i] = 0;
         ...
         t_info->psc = t_data->psc;
         t_info->in_use = TRUE;
         rv = _bcm_xgs3_trunk_swfailover_fabric_set(unit,
                                tid - _BCM_FABRIC_TRUNK_SWF_TID_OFFSET,
                                                    t_data->psc,
                                                    pbmp);
         TRUNK_UNLOCK(unit);
