/*
 * $Id: tucana_cosq.c,v 1.1 2011/04/18 17:11:02 mruas Exp $
...
#ifndef LVL7_FIXUP
/* This fixup allows us to use this API to set the min_bw on individual FE ports */
                                 ...
                                 int average_time)
{
     ...
     * as previously programmed in mmu init
     */
    rc = _bcm_tucana_cosq_get_cos_xq(unit, port, cos_xq);
    if (rc != BCM_E_NONE) {
        return rc;
    }
    ...
    for (idx = cosq_start; idx < cosq_start + num_cosq; idx++) {
        ...
        if (color & BCM_COSQ_DISCARD_COLOR_RED) {
            SOC_IF_ERROR_RETURN
                (READ_EGRPKTLIMITCNGCOSr(unit, port, idx, &val));
            ...
            SOC_IF_ERROR_RETURN
                (READ_WREDCNGPARAMETERCOSr(unit, port, idx, &val));
            soc_reg_field_set(unit, WREDCNGPARAMETERCOSr, &val,
                              DROPSTARTPOINTf, start);
            soc_reg_field_set(unit, WREDCNGPARAMETERCOSr, &val, SLOPEf, slope);
            SOC_IF_ERROR_RETURN
                (WRITE_WREDCNGPARAMETERCOSr(unit, port, idx, val));
        }
        if (!color || (color & BCM_COSQ_DISCARD_COLOR_GREEN)) {
            start = (limit * drop_start) / 100;
            SOC_IF_ERROR_RETURN(READ_WREDPARAMETERCOSr(unit, port, idx, &val));
            soc_reg_field_set(unit, WREDPARAMETERCOSr, &val, DROPSTARTPOINTf,
                              start);
            soc_reg_field_set(unit, WREDPARAMETERCOSr, &val, SLOPEf, slope);
            SOC_IF_ERROR_RETURN(WRITE_WREDPARAMETERCOSr(unit, port, idx, val));
        }
    }
                                 ...
                                 int *average_time)
{
    ...
    switch (color) {
        ...
        case BCM_COSQ_DISCARD_COLOR_RED:
            SOC_IF_ERROR_RETURN
                    (READ_EGRPKTLIMITCNGCOSr(unit, port, cosq, &val));
            cng_percent = soc_reg_field_get(unit, EGRPKTLIMITCNGCOSr,
                                            val, PKTSETLIMITf);
            if (cng_percent == 0x2) {
               /* 75% of XQ limit */
            ...
            SOC_IF_ERROR_RETURN
                    (READ_WREDCNGPARAMETERCOSr(unit, port, cosq, &val));
        ...
        default: /* BCM_COSQ_DISCARD_COLOR_GREEN */
            SOC_IF_ERROR_RETURN
                    (READ_WREDPARAMETERCOSr(unit, port, cosq, &val));
