/*
 * $Id: esw_rx.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
...
/* CPU_COS_MAP_KEY format for TRIUMP and SCORPION */
...
_bcm_trx_cpu_cos_map_key [] =
{
    ...
    /* Below are available for TRIUMPH only */
...
bcm_esw_rx_cosq_mapping_reasons_get(int unit, bcm_rx_reasons_t * reasons)
{
    ...
    if (SOC_IS_TRX(unit)) {
        ...
        for (ix = 0; ix < sz; ix++) {
             if (_bcm_trx_cpu_cos_map_key[ix] != bcmRxReasonInvalid) {
                 BCM_RX_REASON_SET(*reasons, _bcm_trx_cpu_cos_map_key[ix]);
             }
        }
    ...
    *prio = 
    ...
    for (ix = 0; ix < (masklowlen + maskhighlen); ix++) {
         ...
         maskbitset = (ix >= masklowlen)? (mask_high & (1 << (ix - masklowlen))) :
                                      (mask_low & (1 << ix));
         if (maskbitset) {
             BCM_RX_REASON_SET(*reasons_mask, _bcm_trx_cpu_cos_map_key[ix]);
         }
         if (keybitset) {
             BCM_RX_REASON_SET(*reasons, _bcm_trx_cpu_cos_map_key[ix]);
         }
                            ...
                            bcm_cos_queue_t cosq)
{
    ...
    for (bit = 0; bit < sz; bit++) {
         ...
         ridx  = _bcm_trx_cpu_cos_map_key[bit];
...
bcm_esw_rx_rate_set(int unit, int pps)
{
    ...
    if (RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit)) {
        if (soc_feature(unit, soc_feature_packet_rate_limit)) {
	      ...
	      return bcm_en_cosq_port_pps_set(unit, CMIC_PORT(unit),
					      -1, pps);
            }
#endif
#ifdef BCM_TRX_SUPPORT
...
#ifdef BCM_ENDURO_SUPPORT
            if (SOC_IS_ENDURO(unit)) {
              return bcm_en_cosq_port_burst_set(unit, CMIC_PORT(unit),
                                              -1, burst);
            }
#endif
#ifdef BCM_TRX_SUPPORT
...
bcm_esw_rx_cos_rate_set(int unit, int cos, int pps)
{
    ...
    if (RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit)) {
		...
		for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
                        BCM_IF_ERROR_RETURN
			  (bcm_en_cosq_port_pps_set(unit, CMIC_PORT(unit), i,
						    pps));
		}
	      } else {
                    BCM_IF_ERROR_RETURN
		      (bcm_en_cosq_port_pps_set(unit, CMIC_PORT(unit), cos,
						pps));
	      }
	      return BCM_E_NONE;
            }
...
bcm_esw_rx_cos_burst_set(int unit, int cos, int burst)
{
    ...
    if (RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit)) {
        if (soc_feature(unit, soc_feature_packet_rate_limit)) {
		    ...
		    for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
                        BCM_IF_ERROR_RETURN
			  (bcm_en_cosq_port_burst_set(unit, CMIC_PORT(unit),
						      i, burst));
		    }
	        } else {
                    BCM_IF_ERROR_RETURN
		      (bcm_en_cosq_port_burst_set(unit, CMIC_PORT(unit), cos,
						  burst));
	        }
	        return BCM_E_NONE;
            }
