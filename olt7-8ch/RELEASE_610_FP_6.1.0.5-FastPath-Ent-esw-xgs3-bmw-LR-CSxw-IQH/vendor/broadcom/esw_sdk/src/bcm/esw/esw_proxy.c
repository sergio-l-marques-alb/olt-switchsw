/*
 * $Id: esw_proxy.c,v 1.1 2011/04/18 17:11:01 mruas Exp $
 ...
 */
STATIC int 
_bcm_esw_proxy_gport_resolve(int unit, bcm_gport_t gport, bcm_port_t *port_out,
                             bcm_module_t *modid_out, int isLocal)
{
    ...
    if (isLocal) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_gport_modport_api2hw_map(unit, modid, port,
                                          &modid, port_out));
    } else {
        *port_out = port;
        *modid_out = modid;
                         ...
                         int enable)
{
    ...
    if (BCM_GPORT_IS_SET(client_port)) {
        bcm_module_t    modid;
        BCM_IF_ERROR_RETURN(
            _bcm_esw_proxy_gport_resolve(unit, client_port, &client_port,
                                         &modid, 1));
    }
