...
/*
 * $Id: stg.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
#if defined(BCM_5324)
static int bcm5324_stg_status[SOC_MAX_NUM_PORTS];
#endif /* BCM_5324 */

#ifdef STP_BY_EAP_BLK

static sal_mutex_t stp_8021x_state_Mutex = NULL;
static int bcm_shadow_stg_status[SOC_MAX_NUM_PORTS];
/* First array is STP, the other is 802.1x */
int stp_8021x_block_state[2][SOC_MAX_NUM_PORTS]; /* block = 1, non-block = 0 */
extern bcm_pbmp_t  bcm5324_pvlan_value[SOC_MAX_NUM_PORTS];
bcm_pbmp_t  stp_non_forward_pbmp;
                ...
                bcm_port_t port, int stp_state)
{
    ...
    int             reg_len;
    int rv = BCM_E_NONE;
    uint32      ctrl_reg_addr, ctrl_reg_value;
    int    ctrl_reg_len;
    uint32          field_id;
    ...
    ctrl_reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, EAP_GLO_CONr);
    ctrl_reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, EAP_GLO_CONr, port, 0);
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, ctrl_reg_addr, &ctrl_reg_value, ctrl_reg_len)) < 0) {
        return rv;
    }
    ...
    switch(stp_state) {
        case BCM_STG_STP_DISABLE:
            ...
            if (SOC_IS_ROBO53115(unit)){
                ...
                field_id = EN_MAC_BPDUf;
            }
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, EAP_GLO_CONr, 
                    &ctrl_reg_value, field_id, 
                    &temp));
            if (SOC_IS_ROBO53115(unit)){
                field_id = EN_RMCf;
            } else {
                field_id = EN_MAC_02_04_0Ff;
            }
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                    (unit, EAP_GLO_CONr, 
                    &ctrl_reg_value, field_id, 
            ...
            stp_8021x_block_state[0][port] = 1; /* STP block */
        ...
        case BCM_STG_STP_FORWARD:
            BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mstp_port_set)
                    (unit, stg, port, DRV_PORTST_FORWARD));
            bcm_robo_port_vlan_add(unit, port);
            bcm_robo_stp_8021x_lock(unit);
            if (stp_8021x_block_state[1][port]) {
                temp = 1;
            } else {
                temp = 0;
            }
            stp_8021x_block_state[0][port] = 0; /* STP non-block */
            bcm_robo_stp_8021x_unlock(unit);
    ...
    bcm_shadow_stg_status[port] = stp_state;
    ...
    if (SOC_IS_ROBO5324(unit)) {
        port_status = (stp_state == BCM_STG_STP_DISABLE) ? DRV_PORTST_DISABLE :
                (stp_state == BCM_STG_STP_BLOCK) ? DRV_PORTST_DISABLE :
                (stp_state == BCM_STG_STP_LEARN) ? DRV_PORTST_DISABLE :
                (stp_state == BCM_STG_STP_FORWARD) ? DRV_PORTST_FORWARD :
                DRV_PORTST_DISABLE;
        bcm5324_stg_status[port] = stp_state;
    } else 
#endif /* BCM5324_SUPPORT_LACP */
    {
        port_status = (stp_state == BCM_STG_STP_DISABLE) ? DRV_PORTST_DISABLE :
                (stp_state == BCM_STG_STP_BLOCK) ? DRV_PORTST_BLOCK :
                (stp_state == BCM_STG_STP_LEARN) ? DRV_PORTST_LEARN :
                (stp_state == BCM_STG_STP_FORWARD) ? DRV_PORTST_FORWARD :
                DRV_PORTST_LISTEN;
        if (SOC_IS_ROBO5324(unit)) {
            /* 
                ...
                bcm_port_t port, int *stp_state)
{
    ...
    *stp_state = bcm_shadow_stg_status[port];
...
#ifdef BCM5324_SUPPORT_LACP
    if (SOC_IS_ROBO5324(unit)) {
        *stp_state = bcm5324_stg_status[port];
    } else 
#endif /* BCM5324_SUPPORT_LACP */    
    {
        if (SOC_IS_ROBO5324(unit)) {
            ...
            *stp_state = bcm5324_stg_status[port];
            ...
            BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->mstp_port_get)
                        (unit, stg, port, &port_status));
...
int bcm_robo_stg_init(int unit)
{
...
#ifdef STP_BY_EAP_BLK
    {
        ...
        BCM_PBMP_CLEAR(stp_non_forward_pbmp);
        if (stp_8021x_state_Mutex != NULL) {
            sal_mutex_destroy(stp_8021x_state_Mutex);
            stp_8021x_state_Mutex = NULL;
        }
        stp_8021x_state_Mutex = sal_mutex_create("STP_8021X");
        if (stp_8021x_state_Mutex == NULL) {
            return BCM_E_MEMORY;
        }
        for (i=0; i < SOC_MAX_NUM_PORTS; i++) {
            bcm_shadow_stg_status[i] = BCM_STG_STP_FORWARD;
            stp_8021x_block_state[0][i] = 0; /* Non-block */
            stp_8021x_block_state[1][i] = 0; /* Non-block */
        ...
        if (SOC_IS_ROBO5324(unit)) {
            BCM_PBMP_ITER(PBMP_ALL(unit), port){
                bcm5324_pvlan_value[port] = PBMP_ALL(unit);
            }
        }
    ...
    if (SOC_IS_ROBO5324(unit)) {
        ...
        PBMP_ALL_ITER(unit, port) {
            (DRV_SERVICES(unit)->mstp_port_set)
                    (unit, 1, port, DRV_PORTST_FORWARD);
            bcm5324_stg_status[port] = BCM_STG_STP_FORWARD;
        }
			   ...
			   bcm_port_t port, int stp_state)
{
    bcm_stg_info_t  *si = &robo_stg_info[unit];
    int             rv;
    soc_cm_debug(DK_STP, "BCM API : bcm_robo_stg_stp_set()..port=%d,state=%d\n",
                    port, stp_state);
     ...
     *   for get/set stp status 
     */
    if (IS_CPU_PORT(unit, port)){
        if (stp_state != BCM_STG_STP_FORWARD){
            soc_cm_debug(DK_WARN | DK_STP, "Warning: CPU port should be "
                        "at Forward STP state\n");
			   ...
			   bcm_port_t port, int *stp_state)
{
    ...
    soc_cm_debug(DK_STP, "BCM API : bcm_robo_stg_stp_get()..\n");
    /* 
     * CPU should be always been Forward status, and not suitable 
     *   for get/set stp status 
     */
...
{
    sal_mutex_take(stp_8021x_state_Mutex, sal_mutex_FOREVER);
}

void
bcm_robo_stp_8021x_unlock(int unit)
{
    sal_mutex_give(stp_8021x_state_Mutex);
}
...
bcm_robo_port_vlan_remove(int unit, bcm_port_t rem_port)
{
    ...
    BCM_PBMP_PORT_ADD(stp_non_forward_pbmp, rem_port);

    BCM_PBMP_ITER(PBMP_ALL(unit), port){
        BCM_PBMP_CLEAR(pbmp);
        (DRV_SERVICES(unit)->port_vlan_get)(unit, port, &pbmp);
        BCM_PBMP_PORT_REMOVE(pbmp, rem_port);
        (DRV_SERVICES(unit)->port_vlan_set)(unit, port, pbmp);
    }
...
bcm_robo_port_vlan_add(int unit, bcm_port_t add_port)
{
    ...
    BCM_PBMP_PORT_REMOVE(stp_non_forward_pbmp, add_port);
