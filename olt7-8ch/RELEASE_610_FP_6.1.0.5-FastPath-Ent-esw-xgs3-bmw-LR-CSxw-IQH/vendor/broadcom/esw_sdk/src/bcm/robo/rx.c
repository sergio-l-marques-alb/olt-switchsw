/*
 * $Id: rx.c,v 1.1 2011/04/18 17:11:03 mruas Exp $
...
STATIC void        robo_rx_intr_process_packet(int unit, robo_dv_t *dv, robo_dcb_t *dcb,
                                  bcm_pkt_t *pkt);
...
STATIC void        robo_rx_dv_fill(int unit, int chan, robo_dv_t *dv);
STATIC robo_dv_t   *robo_rx_dv_alloc(int unit, int chan, int dv_idx);
...
static bcm_rx_cfg_t _rx_dflt_cfg = {
    ...
    robo_rx_dflt_alloc,          /* alloc function */
    robo_rx_dflt_free,           /* free function */
    0                       /* flags */
};
 ...
 */
int
bcm_erobo_rx_sched_register(int unit, bcm_rx_sched_cb sched_cb)
{
...
bcm_robo_rx_start(int unit, bcm_rx_cfg_t *cfg)
{
    ...
    if (cfg) {  /* Use default if config not specified. */
        ...
        if (!cfg->rx_alloc) {
            rx_ctl[unit]->user_cfg.rx_alloc = robo_rx_dflt_alloc;
        }
        if (!cfg->rx_free) {
            rx_ctl[unit]->user_cfg.rx_free = robo_rx_dflt_free;
        }
...
STATIC INLINE int
robo_rx_chain_start(int unit, int chan, robo_dv_t *dv)
{
    int rv = BCM_E_NONE;
    ...
    if ((rv = soc_robo_dma_start(unit, dv)) < 0) {
...
STATIC INLINE int
robo_rx_chain_start_or_sched(int unit, int chan, robo_dv_t *dv)
{
    int global_usecs = 0;
    int system_usecs = 0;
...
STATIC INLINE int
robo_rx_update_dv(int unit, int chan, robo_dv_t *dv)
{
...
STATIC void
robo_rx_done_packet(int unit, robo_dv_t *dv, robo_dcb_t *dcb)
{
...
STATIC INLINE void
robo_pkt_len_get(int unit, int chan, bcm_pkt_t *pkt, robo_dv_t *dv)
{
...
_pkt_robo_reasons_get(int unit, bcm_pkt_t *pkt, uint32 reasons)
{
    ...
    if (SOC_IS_ROBO5395(unit) || SOC_IS_ROBO53242(unit) ||
        ...
        if (reasons & BCM_ROBO_RX_REASON_MIRROR) {
            BCM_RX_REASON_SET(pkt->rx_reasons, bcmRxReasonL2Cpu);
            pkt->rx_reason = bcmRxReasonL2Cpu;
            (DRV_SERVICES(unit)->queue_rx_reason_get)
...
STATIC  void
robo_rx_intr_process_packet(int unit, robo_dv_t *dv, robo_dcb_t *dcb, bcm_pkt_t *pkt)
{
    volatile rx_callout_t *rco;
    bcm_rx_t              handler_rc;
    int                   chan;
    ...
    if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
        ...
        pkt->rx_cpu_cos = dv->dv_cosq_53242;
...
robo_rx_process_packet(int unit, bcm_pkt_t *pkt)
{
    ...
    /* Mask for complier ok */
        if (soc_cm_debug_check(DK_VERBOSE + DK_DMA)) {
            soc_robo_dma_dump_dv(unit, "rx dv: ", dv);
        }
    }
...
robo_rx_channel_dv_setup(int unit, int chan)
{
    ...
    for (i = 0; i < RX_CHAINS(unit, chan); i++) {
        if ((dv = robo_rx_dv_alloc(unit, chan, i)) < 0) {
            for (j = 0; j < i; j++) {
...
STATIC int
robo_rx_dv_add_pkt(int unit, volatile bcm_pkt_t *pkt, int idx, robo_dv_t *dv)
{
    /* Setup DCB for the Packet, including ETHER Header+Payload+CRC:
     * Always into packet
     */
    SOC_IF_ERROR_RETURN(soc_robo_dma_desc_add(dv, (sal_vaddr_t)pkt->alloc_ptr,
                                         pkt->_pkt_data.len));
...
STATIC void
robo_rx_dv_fill(int unit, int chan, robo_dv_t *dv)
{
    int         i;
    volatile bcm_pkt_t   *pkt;
    ...
    soc_robo_dma_dv_reset(DV_RX, dv);
...
robo_rx_dv_alloc(int unit, int chan, int dv_idx)
{
    ...
    if ((dv = soc_robo_dma_dv_alloc(unit, DV_RX, RX_DCB_PER_DV(unit, chan)))
                                  == NULL) {
        sal_free(dv_info);
        return NULL;
    }
    ...
    clr_len = sizeof(robo_dcb_t) * RX_DCB_PER_DV(unit, chan);
    ...
    FOREACH_SETUP_CHANNEL(unit, chan) {
        ...
        if (soc_cm_debug_check(DK_VERBOSE)) {
            for (i = 0; i < RX_CHAINS(unit, chan); i++) {
                ...
                if (dv == NULL || !soc_robo_dma_dv_valid(dv)) {
