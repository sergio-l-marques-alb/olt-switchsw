/*
 * $Id: dma.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
soc_dma_abort(int unit)
{
    ...
    for (c = 0; c < soc->soc_max_channels; c++) {
        sc = &soc->soc_channels[c];
        soc_dma_abort_channel(unit, c);
...
soc_dma_dv_alloc(int unit, dvt_t op, int cnt)
{
    ...
    if ((cnt == soc->soc_dv_size) && (soc->soc_dv_free_cnt > 0)) {
        ...
        dv->dv_dmabuf = soc_cm_salloc(unit, SOC_DV_DMABUF_SIZE,
                                      "sdma_dmabuf_alloc");
...
{
    SOC_IF_ERROR_RETURN(soc_dma_abort(unit));
#ifdef  BCM_XGS3_SWITCH_SUPPORT
    if (soc_feature(unit, soc_feature_txdma_purge)) {
