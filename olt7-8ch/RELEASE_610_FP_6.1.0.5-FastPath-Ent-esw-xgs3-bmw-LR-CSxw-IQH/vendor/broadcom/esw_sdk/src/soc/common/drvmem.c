/*
 * $Id: drvmem.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
                      ...
                      soc_field_t field, uint32 value)
{
    ...
    soc_meminfo_field32_force(meminfo, entry, field, value);
}
 ...
 */
void
soc_meminfo_field32_force(soc_mem_info_t *meminfo, void *entry,
                          soc_field_t field, uint32 value)
{
    soc_field_info_t    *fieldinfo;
    int                 len;
    ...
    soc_meminfo_field_set(meminfo, entry, field, &value);
}
                       ...
                       soc_field_t field, uint32 *fldbuf)
{
    ...
    rfldbuf = soc_meminfo_field_get(meminfo, entbuf, field, fldbuf);
                       ...
                       soc_field_t field, uint32 *fldbuf)
{
    ...
    if (dc_val) {
        ...
        soc_meminfo_field_set(meminfo, entbuf, field, buf);
    }
    else 
#endif
    {
...
soc_mem_addr(int unit, soc_mem_t mem, int blk, int index)
{
    ...
    if (((blk>=32)?(mip->blocks_hi & (1 << (blk&0x1F))):(mip->blocks & (1 << blk))) && !(mip->flags & SOC_MEM_FLAG_MONOLITH)) {
        ...
        mip->base |= ((SOC_BLOCK2OFFSET(unit, blk) >> 4) & 0x3) << SOC_BLOCK_MSB_BP;
    } else {
        blkoff = 0;
    }
