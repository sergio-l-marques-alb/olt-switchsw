/*
 * $Id: mem.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
uint32 fifo_delay_value = 15 * MILLISECOND_USEC;
...
_soc_mem_cmp_vlan_xlate_tr(int unit, void *ent_a, void *ent_b)
{
    ...
    type_a = soc_VLAN_XLATEm_field32_get(unit, ent_a, KEY_TYPEf);
    type_b = soc_VLAN_XLATEm_field32_get(unit, ent_b, KEY_TYPEf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    if ((type_a == 4) || (type_a == 7)) {
        return _soc_mem_cmp_vlan_mac_tr(unit, ent_a, ent_b);
    }

    type_a = soc_VLAN_XLATEm_field32_get(unit, ent_a, GLPf);
    type_b = soc_VLAN_XLATEm_field32_get(unit, ent_b, GLPf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_VLAN_XLATEm_field32_get(unit, ent_a, INCOMING_VIDSf);
    type_b = soc_VLAN_XLATEm_field32_get(unit, ent_b, INCOMING_VIDSf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);
...
_soc_mem_cmp_egr_vlan_xlate_tr(int unit, void *ent_a, void *ent_b)
{
    ...
    type_a = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_a, PORT_GROUP_IDf);
    type_b = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_b, PORT_GROUP_IDf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_a, IVIDf);
    type_b = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_b, IVIDf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_a, OVIDf);
    type_b = soc_EGR_VLAN_XLATEm_field32_get(unit, ent_b, OVIDf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);
...
_soc_mem_cmp_mpls_entry_tr(int unit, void *ent_a, void *ent_b)
{
    ...
    type_a = soc_MPLS_ENTRYm_field32_get(unit, ent_a, PORT_NUMf);
    type_b = soc_MPLS_ENTRYm_field32_get(unit, ent_b, PORT_NUMf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_MPLS_ENTRYm_field32_get(unit, ent_a, MODULE_IDf);
    type_b = soc_MPLS_ENTRYm_field32_get(unit, ent_b, MODULE_IDf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_MPLS_ENTRYm_field32_get(unit, ent_a, Tf);
    type_b = soc_MPLS_ENTRYm_field32_get(unit, ent_b, Tf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);

    type_a = soc_MPLS_ENTRYm_field32_get(unit, ent_a, MPLS_LABELf);
    type_b = soc_MPLS_ENTRYm_field32_get(unit, ent_b, MPLS_LABELf);
    SOC_MEM_COMPARE_RETURN(type_a, type_b);
        ...
        for (f = memp->nFields - 1; f >= 0; f--) {
            ...
            if (memp->flags & SOC_MEM_FLAG_MULTIVIEW) {
                ...
                if (strstr(soc_fieldnames[fieldp->field], memp->views[key_type]) ||
                    (strcmp(memp->views[key_type], memp->views[0]) == 0 &&
             ...
             int index_min, int index_max, void *buffer)
{
    ...
    start_addr = soc_mem_addr(unit, mem, copyno, index_min);
             ...
             int index_begin, int index_end, void *buffer)
{
    ...
    if (index_begin > index_end) {
        start_addr = soc_mem_addr(unit, mem, copyno, index_end);
        count = index_begin - index_end + 1;
    } else {
        start_addr = soc_mem_addr(unit, mem, copyno, index_begin);
    ...
    soc_cm_sflush(unit, (void *)buffer, WORDS2BYTES(data_beats) * count);
...
#define	SOC_MEM_ADDR_STAGE(_addr) (((_addr) >> 24) & 0xf)
	...
	rv = WRITE_ING_HW_RESET_CONTROL_2r(unit, 0);
	if (rv < 0) {
	    break;
	}
             ...
             int index_min, int index_max, void *buffer)
{
    ...
    do {
        ...
        start_addr = soc_mem_addr(unit, mem, copyno,
                                  (index_min+=block_count));
...
#if defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
        /* required on XGS3. Optional on other devices */
        schan_msg.writecmd.header.dstblk = ((schan_msg.writecmd.address >> SOC_BLOCK_BP)
             & 0xf) | (((schan_msg.writecmd.address >> SOC_BLOCK_MSB_BP) & 0x3) << 4);
                       ...
                       void *key, void *result, int *index_ptr)
{
    ...
    schan_msg.gencmd.address = soc_mem_addr(unit, mem, copyno, 0);
    ...
    rv = soc_schan_op(unit, &schan_msg, entry_dw + 2, entry_dw + 2, 1);
                        ...
                        int recurse_depth)
{
    ...
    for (bix = 0; bix < 2; bix++) {
        ...
        /* Bucket half is based on bunk id.      */
        ...
        /* Iterate over entries int the half-bucket.. */
                          ...
                          int recurse_depth)
{
    ...
    case MPLS_ENTRYm:
	    ...
	    return soc_mem_bank_insert(unit, mem, 0, copyno,
                                       entry_data, old_entry_data);
    ...
    case VLAN_XLATEm:
        ...
        /* Fall thru for Triumph (VLAN_XLATE & VLAN_MAC are a shared table) */
    case VLAN_MACm:
	rv = soc_mem_bank_insert(unit, mem, 2, copyno,
                                 entry_data, old_entry_data);
        if (rv != SOC_E_FULL) {
            return rv;
        }
	rv = soc_mem_bank_insert(unit, mem, 1, copyno,
                                 entry_data, old_entry_data);
        if (rv != SOC_E_FULL) {
            return rv;
        }
...
_soc_mem_pop(int unit, soc_mem_t mem, int copyno, void *entry_data)
{
    ...
    schan_msg.popcmd.address = soc_mem_addr(unit, mem, copyno, 0);
    ...
    rv = soc_schan_op(unit, &schan_msg, 2, entry_dw + 1, 1);
...
_soc_mem_push(int unit, soc_mem_t mem, int copyno, void *entry_data)
{
    ...
    schan_msg.pushcmd.address = soc_mem_addr(unit, mem, copyno, 0);
    ...
    rv = soc_schan_op(unit, &schan_msg, entry_dw + 2, entry_dw + 1, 1);
                        ...
                        int host_entries, void *host_buf)
{
    ...
    addr = soc_reg_addr(unit, fifo_dma[chan].sbus_addr, REG_PORT_ANY, 0);
    rval = soc_mem_addr(unit, mem, copyno, 0);
    soc_pci_write(unit, addr, rval);

    addr = soc_reg_addr(unit, fifo_dma[chan].hostmem_addr, REG_PORT_ANY, 0);
    rval = soc_cm_l2p(unit, host_buf);
    soc_pci_write(unit, addr, rval);

    addr = soc_reg_addr(unit, fifo_dma[chan].read_ptr, REG_PORT_ANY, 0);
    soc_pci_write(unit, addr, rval);

    addr = soc_reg_addr(unit, fifo_dma[chan].threshold, REG_PORT_ANY, 0);
    rval = 0;
    soc_reg_field_set(unit, fifo_dma[chan].threshold, &rval, ADDRESSf,
                      host_entries / 16);
    soc_pci_write(unit, addr, rval);

    addr = soc_reg_addr(unit, fifo_dma[chan].cfg, REG_PORT_ANY, 0);
    rval = 0;
    data_beats = soc_mem_entry_words(unit, mem);
    soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval, BEAT_COUNTf,
                      data_beats);
    soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval, HOST_NUM_ENTRIES_SELf,
                      sel);
    soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval, TIMEOUT_COUNTf, 200);
    if (soc_feature(unit, soc_feature_multi_sbus_cmds)) {
        ...
        if (spacing) {
            soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval,
                              MULTIPLE_SBUS_CMD_SPACINGf, spacing);
            soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval,
                              ENABLE_MULTIPLE_SBUS_CMDSf, 1);
        }
    ...
    rval = 0;
    soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval, ENABLEf, 1);
    soc_reg_field_set(unit, fifo_dma[chan].cfg, &rval, ENABLE_VALf, 1);
    soc_pci_write(unit, addr, rval);
...
_soc_mem_fifo_dma_get_read_ptr(int unit, int chan, void **host_ptr, int *count)
{
    ...
    addr = soc_reg_addr(unit, fifo_dma[chan].write_ptr, REG_PORT_ANY, 0);
    ...
    if (read_ptr == write_ptr) {
        ...
        if (!soc_reg_field_get(unit, CMIC_FIFO_RD_DMA_DEBUGr,
                               debug, fifo_dma[chan].overflow_fld)) {
            return SOC_E_EMPTY;
        }
    }
    ...
    data_beats = soc_reg_field_get(unit, fifo_dma[chan].cfg, rval,
                                   BEAT_COUNTf);
    host_entries = 1 << (soc_reg_field_get(unit, fifo_dma[chan].cfg, rval,
                                           HOST_NUM_ENTRIES_SELf) + 6);
...
_soc_mem_fifo_dma_advance_read_ptr(int unit, int chan, int count)
{
    ...
    /* Clear overflow (wrap/full) bit */
    addr = soc_reg_addr(unit, CMIC_FIFO_RD_DMA_DEBUGr, REG_PORT_ANY, 0);
    debug = 0;
    soc_reg_field_set(unit, CMIC_FIFO_RD_DMA_DEBUGr,
                      &debug, BIT_POSf, fifo_dma[chan].overflow_bit);
                       ...
                       int host_entries, void *host_buf)
{
    ...
    if (soc_feature(unit, soc_feature_fifo_dma)) {
        return _soc_mem_fifo_dma_start(unit, chan, mem, copyno, host_entries,
                                       host_buf);
    }
#endif /* BCM_TRX_SUPPORT || BCM_SIRIUS_SUPPORT */
