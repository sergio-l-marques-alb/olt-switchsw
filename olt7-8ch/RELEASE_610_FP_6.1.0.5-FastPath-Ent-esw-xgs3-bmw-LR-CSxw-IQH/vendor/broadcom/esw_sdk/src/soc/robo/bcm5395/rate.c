/*
 * $Id: rate.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
 *
    ...
    uint32 limit, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, 
    "drv_bcm5395_rate_set: unit = %d, bmp = %x, %s, limit = %dK, \
     burst size = %dKB\n",
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm5395_port_irc_set(unit, port, limit, burst_size));
            }
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm5395_port_erc_set(unit, port, limit, burst_size));
            }    
            break;
...
/*
 *  Function : drv_bcm5395_rate_config_get
 *
 *  Purpose :
    ...
    uint32 *limit, uint32 *burst_size)
{
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            SOC_IF_ERROR_RETURN(
                _drv_bcm5395_port_irc_get(unit, port, limit, burst_size));
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            SOC_IF_ERROR_RETURN(
                _drv_bcm5395_port_erc_get(unit, port, limit, burst_size));
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
            return SOC_E_UNAVAIL;
        default:
    ...
    soc_cm_debug(DK_PORT, 
    "drv_bcm5395_rate_get: unit = %d, port = %d, %s, limit = %dK,\
      burst size = %dKB\n",
