/*
 * $Id: trap.c,v 1.1 2011/04/18 17:11:09 mruas Exp $
 *
...
drv_bcm5395_trap_set(int unit, soc_pbmp_t bmp, uint32 trap_mask)
{
    ...
    if (trap_mask & DRV_SWITCH_TRAP_IGMP) {    
        temp = 1;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_IP_ENf, &temp));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, HL_PRTC_CTRLr, &reg_value, IGMP_FWD_MODEf, &temp));
    } else {
