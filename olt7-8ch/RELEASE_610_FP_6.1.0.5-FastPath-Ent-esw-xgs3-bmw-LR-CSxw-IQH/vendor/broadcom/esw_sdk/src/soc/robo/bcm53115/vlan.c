...
/* Customer EVR IDs */
#ifdef LVL7_FIXUP
typedef struct drv_vlan_evt_table_info_s 
{
   uint16 out_vid;
   uint16 out_op;
   uint16 in_vid;
   uint16 in_op;
   int evr_id;
}drv_vlan_evt_table_info_t;

typedef struct drv_vlan_evt_db_info_s 
{
  int count;
  drv_vlan_evt_table_info_t egr_vt_table[DRV_EVRID_CUSTOMER_ACTION_COUNT]; 
}drv_vlan_evt_db_info_t;

drv_vlan_evt_db_info_t egr_vt_table_db[SOC_ROBO_MAX_NUM_PORTS];

#define IS_VALID_CUSTOM_EGRVT_DB_ENTRY_ID(_id)    \
                ((_id) >= 0 && (_id) < DRV_EVRID_CUSTOMER_ACTION_COUNT)
#define EGRVT_CUSTOM_DB_ID_TO_REAL_ID(_db_id)    \
                ((_db_id) + DRV_EVRID_CUSTOMER_ACTION_FIRST)
#define EGRVT_CUSTOM_REAL_ID_TO_DB_ID(_real_id)    \
                ((_real_id) - DRV_EVRID_CUSTOMER_ACTION_FIRST)
#endif
...
#ifdef  LVL7_FIXUP
#define EVR_SWDB_OP_FLAG_MODIFY  4
#endif
/* global var for 
...
#ifdef LVL7_FIXUP
int _drv_bcm53115_evr_table_freeid_get(int unit, int port, int *free_id, int out_op, int out_vid,
                                       int in_op, int in_vid)
{
  int     i;
    
  drv_vlan_evt_table_info_t *temp_evr_sw_db_entry;
     
  /* for bcm53115, this routine is port based DB search */
  assert(port <= SOC_ROBO_MAX_NUM_PORTS);
  
  /* check if sw_db is empty */
  if (egr_vt_table_db[port].count == 0)
  {
    *free_id = 0; 
    return FALSE;
  }
    
  if (egr_vt_table_db[port].count == DRV_EVRID_CUSTOMER_ACTION_COUNT)
  {
    *free_id = DRV_EVRID_CUSTOMER_ACTION_COUNT;
    return FALSE;
  }

#if 0
  for (i = 0; i < DRV_EVRID_CUSTOMER_ACTION_COUNT; i++)
  {
    temp_evr_sw_db_entry = egr_vt_table_db[port].egr_vt_table + i;
    
    if ((temp_evr_sw_db_entry->out_op == out_op) && (temp_evr_sw_db_entry->out_vid == out_vid) &&
        (temp_evr_sw_db_entry->in_op == in_op) && (temp_evr_sw_db_entry->in_vid == in_vid))
    {
      *free_id = i;
      return TRUE;
    }    
  ...
  {
    temp_evr_sw_db_entry = egr_vt_table_db[port].egr_vt_table + i;
    if (temp_evr_sw_db_entry->evr_id == -1)
    {
      *free_id = i;
      break;
    }
  ...
  /* check if sw_db is full */
  if (i == DRV_EVRID_CUSTOMER_ACTION_COUNT)
  {
    *free_id = DRV_EVRID_CUSTOMER_ACTION_COUNT;
  }
...
#ifdef LVL7_FIXUP
void _drv_bcm53115_evr_table_sw_db_update(int op, int port, int real_id, int out_op,
                                          int out_vid, int in_op, int in_vid)
{
  drv_vlan_evt_table_info_t *upd_entry;
  int db_id;
  ...
  db_id =  EGRVT_CUSTOM_REAL_ID_TO_DB_ID(real_id);
 
  if (!(IS_VALID_CUSTOM_EGRVT_DB_ENTRY_ID(db_id)))
  {
    soc_cm_debug(DK_ERR, "%s: invalid case on both id \n", __func__);
    return;   
  }
  upd_entry =  egr_vt_table_db[port].egr_vt_table + db_id;
  
  switch(op)
  {
    case EVR_SWDB_OP_FLAG_INSERT :
      /* insert to the front of fast_id node */
      upd_entry->out_op = out_op;
      upd_entry->out_vid = out_vid;
      upd_entry->in_op = in_op;
      upd_entry->in_vid = in_vid;
      upd_entry->evr_id = real_id;
      egr_vt_table_db[port].count ++;
    
      break;
    
    case EVR_SWDB_OP_FLAG_DELETE : 
      sal_memset(upd_entry, 0x00 , sizeof(drv_vlan_evt_table_info_t));
      upd_entry->evr_id = -1;
      egr_vt_table_db[port].count --;
      break;
    
     case EVR_SWDB_OP_FLAG_MODIFY :
      /*Modify the entry and donot increment the count*/
      upd_entry->out_op = out_op;
      upd_entry->out_vid = out_vid;
      upd_entry->in_op = in_op;
      upd_entry->in_vid = in_vid;
...
int _drv_bcm53115_evr_entry_set(int unit, uint32 port, int ent_id, 
    ...
    soc_cm_debug(DK_VLAN, "%s: port=%d,entry_id=%d,out_op=%d,in_op=%d\n", 
            __func__, port, ent_id, out_op, in_op);
#ifdef LVL7_FIXUP
    if ((ent_id < 0) || 
            (ent_id > (DRV_EVRID_CUSTOMER_ACTION_FIRST + 
                    DRV_EVRID_CUSTOMER_ACTION_COUNT - 1))){
...
int _drv_bcm53115_evr_init(int unit){
    ...
    int     retry;
#ifdef LVL7_FIXUP
/*SDK patch on 5.5.3 to set default EVR entry.
  TODO: need to remove while merging latest SDK (>5.5.3)*/
...
#ifdef LVL7_FIXUP
   sal_memset(egr_vt_table_db, 0, sizeof(drv_vlan_evt_db_info_t) * SOC_ROBO_MAX_NUM_PORTS);
   for (i=0; i<SOC_ROBO_MAX_NUM_PORTS; i++)
   {
     for (j=0; j<DRV_EGRESS_V2V_NUM_PER_PORT; j++)
     {
        egr_vt_table_db[i].egr_vt_table[j].evr_id = -1; 
     }
   }
#endif
...
/* The following 2 functions are are re-defined here */
 *  Function : _drv_bcm53115_evr_entry_set
 *      - to set a Egress VLAN Remark(EVR) table entry.
 *
 *  Parameter :
                ...
                uint32 sp_vid, uint32 pri, uint32 mode)
{
    ...
    soc_cm_debug(DK_VLAN, 
            ...
            __func__, vt_type, port, cvid, sp_vid);
    switch (vt_type){
...
#ifdef LVL7_FIXUP
/*
 *  Function : drv_vlan_evt_entry_add
 *
 *  Purpose :
 *      Add a Egress VLAN entry to EGR table.
 *
 *  Parameters :
 *      unit        :   RoboSwitch unit number.
 *      port        :   port id. (not used for ingress VT)
 *      evr_id      :   Classification ID
 *      out_op      :   Outer tagged operation
 *      out_vid     :   Outer vlan
 *      in_op       :   Inner tagged operation
 *      in_vid      :   inner_vid
 *
 *  Return :
 *      SOC_E_NONE
 ...
 */
int
drv_bcm53115_vlan_evt_entry_add(int unit, int port, int *evr_id, int out_op, int out_vid,
                                int in_op, int in_vid)
{  
  int rv = SOC_E_NONE;
  uint32 free_id;
  ...
  soc_cm_debug(DK_VLAN, 
               "%s: unit=%d, port=%d, out_vid=%d out_op=%d in_vid=%d in_op=%d\n",
               __func__, unit, port, out_vid, out_op, in_vid, in_op);
 
  /* can't work if the iDT_mod is not enabled */
  if (!(device_dtag_mode & DRV_VLAN_INTELLIGENT_DT_MODE))
  {
     /* warnning message */
     soc_cm_debug(DK_WARN,
                 "EVR on port=%d can't work -> incorrect DT_mode!\n",
                  port);
     return SOC_E_CONFIG;
  }

  if (_drv_bcm53115_evr_table_freeid_get(unit, port, &free_id, out_op, out_vid, 
                                         in_op, in_vid))
  {     /* exist */
    *evr_id = EGRVT_CUSTOM_DB_ID_TO_REAL_ID(free_id);
    soc_cm_debug(DK_VLAN,
                 "%s:EVR for port=%d,evr_id=%d is existed! Can't Add it!\n",
                 __func__, port, *evr_id);
    return SOC_E_EXISTS;
  }
  else 
  {    
    /* not exist */
    /* check if full */
    if (free_id == DRV_EVRID_CUSTOMER_ACTION_COUNT)
    {
      soc_cm_debug(DK_VERBOSE | DK_VLAN,
                   "%s:EVR for port=%d is existed! Can't Add it!\n",
                   __func__, port);
      return SOC_E_FULL;
    }
  ...
  assert(IS_VALID_CUSTOM_EGRVT_DB_ENTRY_ID(free_id));
  *evr_id = EGRVT_CUSTOM_DB_ID_TO_REAL_ID(free_id);

  out_vid &= DRV_EVR_VID_MASK;
  in_vid &= DRV_EVR_VID_MASK;
  
  rv = _drv_bcm53115_evr_entry_set(unit, port, *evr_id, 
                                   out_op, out_vid,
                                   in_op, in_vid);
 
  assert(rv == SOC_E_NONE);
  _drv_bcm53115_evr_table_sw_db_update(EVR_SWDB_OP_FLAG_INSERT, port, *evr_id, 
                                       out_op, out_vid, in_op, in_vid);
  return rv;
}

/*
 *  Function : drv_vlan_evt_entry_delete
 *
 *  Purpose :
 ...
 */
int
drv_bcm53115_vlan_evt_entry_delete(int unit, int port, int evr_id)
{  
  int rv = SOC_E_NONE;
  uint32 db_id;

  db_id = EGRVT_CUSTOM_REAL_ID_TO_DB_ID(evr_id);
  ...
  soc_cm_debug(DK_VLAN, 
               "%s: unit=%d, port=%d, evr_id=%d, db_id=%d\n",
               __func__, unit, port, evr_id, db_id);
  
  rv = _drv_bcm53115_evr_entry_set(unit, port, evr_id, 
                                   DRV_EVR_OP_FLAG_ASIS, 0,
                                   DRV_EVR_OP_FLAG_ASIS, 0);
  assert(rv == SOC_E_NONE);
  _drv_bcm53115_evr_table_sw_db_update(EVR_SWDB_OP_FLAG_DELETE, port, evr_id, 0,0,0,0);
 ...
 */
int 
drv_bcm53115_vlan_evt_entry_get(int unit, int port, int evr_id, uint16 *out_op, uint16 *out_vid,
                                uint16 *in_op, uint16 *in_vid)
{
  int rv = SOC_E_NONE;
  int db_id;
  ...
  rv = _drv_bcm53115_evr_entry_get(unit, port, evr_id, out_op, out_vid, in_op, in_vid);

  return rv; 
}
/*
 *  Function : drv_vlan_evt_entry_modify
 *
 *  Purpose :
 *      Modify an existing Egress VLAN entry in EGR table.
 *
 *  Parameters :
 *      unit        :   RoboSwitch unit number.
 *      port        :   port id. (not used for ingress VT)
 ...
 */
int
drv_bcm53115_vlan_evt_entry_modify(int unit, int port, int evr_id, int out_op, int out_vid,
                                int in_op, int in_vid)
 {  
  int rv = SOC_E_NONE;
  uint32 db_id;
  ...
  rv = _drv_bcm53115_evr_entry_set(unit, port, evr_id, 
                                   ...
                                   in_op, in_vid);
  assert(rv == SOC_E_NONE);
  _drv_bcm53115_evr_table_sw_db_update(EVR_SWDB_OP_FLAG_MODIFY, port, evr_id,
                                       out_op, out_vid, in_op, in_vid);
                                ...
                                uint32 port, uint32 prop_val)
{
    ...
    soc_cm_debug(DK_VLAN, 
            ...
            __func__, unit, prop_type, vid, port, prop_val);
    switch(prop_type){
                                ...
                                uint32 port, uint32 *prop_val)
{
    ...
    soc_cm_debug(DK_VLAN, 
            ...
            __func__, unit, prop_type, vid, port);
...
drv_bcm53115_vlan_prop_set(int unit, uint32 prop_type, uint32 prop_val)
{
    ...
    soc_cm_debug(DK_VLAN, 
            ...
            __func__, unit, prop_type, prop_val);
    switch (prop_type) {
                ...
                uint32 port_n, uint32 *val)
{
    ...
    soc_cm_debug(DK_VLAN, "%s: unit=%d, prop=%d, port=%d\n", 
            __func__, unit, prop_type, port_n);
