/*
 * $Id: reg.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
drv_reg_read(int unit, uint32 addr, void *data, int len)
{
    ...
    if (rv < 0) {
        soc_cm_print("drv_reg_read: error reading unit=0x%x addr=0x%x\n", 
                     unit, addr);
    }
...
drv_reg_addr(int unit, uint32 reg, int port, int index)
{
    ...
    switch (regtype) {
        case soc_genreg:
            if (regblktype & SOC_BLK_SYS) {
                base = (0x0000FFFF & (offset + index * nByte));        
            } else if (!(regblktype & SOC_BLK_PORT)) {
                base = (0x0000FFFF & (offset + port * nByte));
            } else if (regblktype & SOC_BLK_EPIC){
                        base = (0x0000FFFF & (offset + port * nByte));
            } else if (regblktype & SOC_BLK_CPIC){
                if (SOC_IS_ROBO5324(unit)) {
                    /* 
                     * Since the offset of IMP port is a fixed value (24).
                     * But there are 3 value of NUM_FE_PORT(), 
                     * 8, 16 and 24 for BCM5320/21/24 respectively.
                     * So, add hardcode here. 
                     */
                    if (numels > NUM_FE_PORT(unit)) {
                     ...
                     * So, add hardcode NUM_FE_PORT(unit) here. 
                     */
                    if (numels > NUM_FE_PORT(unit)) {
                            base = (0x0000FFFF & (offset + 
                            (port-24) * nByte));
            ...
            } else if (regblktype & SOC_BLK_GPIC){
                if (SOC_IS_ROBO5324(unit)) {
                    /* 
                     * Since the offset of GE ports is a fixed value (24+1).
                     * But there are 3 value of NUM_FE_PORT(), 
                     * 8, 16 and 24 for BCM5320/21/24 respectively.
                     * So, add hardcode here. 
                     */
        ...
        case soc_portreg:
            ...
            if ((regblktype & SOC_BLK_GPIC) && !(regblktype & SOC_BLK_EPIC)) {
                if (SOC_IS_ROBO5348(unit)|| SOC_IS_ROBO5347(unit)) {
                        port = port-NUM_FE_PORT(unit);
                }
                /* 
                 * Since the offset of IMP port is a fixed value (24).
                 * But there are 3 value of NUM_FE_PORT(), 
