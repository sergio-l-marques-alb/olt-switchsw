...
_drv53115_trunk_enable_set(int unit, uint32 enable)
{
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
        (unit, MAC_TRUNK_CTLr, &reg_value, EN_TRUNK_LOCALf, &temp));
...
_drv53115_trunk_enable_get(int unit, int tid, uint32 *enable)
{
    ...
    if (temp){
         ...
         bmp = 0;
         SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                (unit, TRUNK_GRP_CTLr, &reg_value, EN_TRUNK_GRPf, &bmp));
	 if(bmp) {
            *enable = 1;
        }else {
            *enable = 0;
                ...
                soc_pbmp_t *bmp, uint32 flag, uint32 *hash_op)
{
    ...
    if (flag & DRV_TRUNK_FLAG_BITMAP) {
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &reg_value,reg_len));
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
            (unit, TRUNK_GRP_CTLr, &reg_value, EN_TRUNK_GRPf, &temp));
...
drv_bcm53115_trunk_hash_field_add(int unit, uint32 field_type)
{
    ...
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
        (unit, reg_addr, &reg_value,reg_len));
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, MAC_TRUNK_CTLr, &reg_value, HASH_SELf, &temp));
...
{
    uint32	reg_addr, reg_value, temp, current_hash;
    int		reg_len;

    soc_cm_debug(DK_VERBOSE, 
        "drv_bcm53115_trunk_hash_field_remove: unit = %d, field type = %d\n",
        unit, field_type);
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, MAC_TRUNK_CTLr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, MAC_TRUNK_CTLr);
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
        (unit, reg_addr, &reg_value,reg_len));
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, MAC_TRUNK_CTLr, &reg_value, HASH_SELf, &temp));
