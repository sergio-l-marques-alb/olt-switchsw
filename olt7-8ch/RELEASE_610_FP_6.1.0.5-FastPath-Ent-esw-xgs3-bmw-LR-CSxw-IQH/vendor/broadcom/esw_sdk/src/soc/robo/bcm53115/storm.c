    ...
    uint32 type, uint32 enable)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)
            (unit, BC_SUP_RATECTRL_Pr, port, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)
            (unit, BC_SUP_RATECTRL_Pr);
    ...
    if (bcast_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, bcast_fld_index, &temp));
    }
    if (mcast_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, mcast_fld_index, &temp));
    }
    if (dlf_fld_index) {
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, dlf_fld_index, &temp));
    }
...
_drv_bcm53115_storm_control_type_enable_get(int unit, uint32 port, uint32 *type)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, BC_SUP_RATECTRL_Pr, port, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, BC_SUP_RATECTRL_Pr);
    if ((rv = (DRV_SERVICES(unit)->reg_read)
        (unit, reg_addr, &reg_value, reg_len)) < 0) {
        return rv;
    }
    ...
    *type = 0;
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_BCAST_SUPf, &temp)); 
    if (temp) {
        *type |= DRV_STORM_CONTROL_BCAST;
    }
   
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_MCAST_SUPf, &temp));
    if (temp) {
        *type |= DRV_STORM_CONTROL_MCAST;
    }
    
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_DLF_SUPf, &temp));    
    if (temp) {
        *type |= DRV_STORM_CONTROL_DLF;
    }

    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
        (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_RSV_MLTCST_SUPf, &temp));
    if (temp) {
...
drv_bcm53115_storm_control_enable_set(int unit, uint32 port, uint8 enable)
{
    ...
    temp = enable;
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_BUCKET1f, &temp));

    /* Enable Suppression bit of Storm Control */ 
    SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, BC_SUP_RATECTRL_Pr, &reg_value, EN_STORM_SUPf, &temp));
    ...
    PBMP_ITER(bmp, port) {
        if (limit == 0) { /* Disable storm suppression type */
            ...
            if (burst_size) {
                ...
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                        (unit, BC_SUP_RATECTRL_Pr, &reg_value, 
                        BUCKET1_SIZEf, &temp));
            }
