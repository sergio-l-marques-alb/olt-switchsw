...
{
    uint32  reg_len, reg_addr, reg_value, temp, i;
    uint16  dev_id;
    uint8   rev_id;
    int     en_temp;
    ...
    if (reset_type == V2V_SW_RESET) {
        ...
        reg_len = (DRV_SERVICES(unit)->reg_length_get)
                        (unit, RST_TABLE_MEM1r);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)
                        (unit, RST_TABLE_MEM1r, 0, 0);
        reg_value = 0;
        temp = 1;
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, RST_TABLE_MEM1r, &reg_value, RST_VLAN2VLANf, &temp));
...
drv_bcm53242_vlan_vt_delete_all(int unit, uint32 vt_type)
{
    ...
    case DRV_VLAN_XLAT_INGRESS_TRAN :
        ...
        tmp = 0;
        tmp_rv = (DRV_SERVICES(unit)->vlan_property_get)
                                ...
                                uint32 port, uint32 prop_val)
{
    ...
    case DRV_VLAN_PROP_ING_VT_SPTPID:    /* ingress SP TPID */
        field = ISP_VLAN_DELIMITERf;
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, ISP_VIDr, port, 0);
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, ISP_VIDr, &reg_value, ISP_VLAN_DELIMITERf, &field_val32));
...
drv_bcm53242_vlan_prop_get(int unit, uint32 prop_type, uint32 *prop_val)
{
    ...
    case DRV_VLAN_PROP_SP_TAG_TPID:
        ...
        *prop_val = temp & 0xFFFF;
        ...
        break;
    case DRV_VLAN_PROP_DOUBLE_TAG_MODE:
        reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, DTAG_GLO_CTLr);
        reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, DTAG_GLO_CTLr, 0, 0);
        if ((rv = (DRV_SERVICES(unit)->reg_read)
            (unit, reg_addr, &reg_value, reg_len)) < 0) {
            return rv;
        }
                ...
                soc_pbmp_t bmp, uint32 val)
{
    ...
    case    DRV_VLAN_PROP_V2V_PORT :
        ...
        reg_index = VLAN2VLAN_CTLr;
        fld_index  = VLAN2VLAN_CTLf;
        break;
    case    DRV_VLAN_PROP_MAC2V_PORT :
        reg_index = MAC2VLAN_CTLr;
        fld_index  = MAC2VLAN_CTLf;
        break;
    case    DRV_VLAN_PROP_PROTOCOL2V_PORT :
        reg_index = PROTOCOL2VLAN_CTLr;
        fld_index  = PROTOCOL2VLAN_CTLf;
        break;
    case    DRV_VLAN_PROP_TRUST_VLAN_PORT :
        reg_index = TRUST_CVLANr;
        fld_index  = TRUST_CVLANf;
        break;
    default :
        return SOC_E_UNAVAIL;
        break;
