/*
 * $Id: cosq.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
 *
 ...
 */
int 
drv_bcm53242_queue_mode_set(int unit, uint32 bmp, uint32 mode)
{
    int rv = SOC_E_NONE;
    ...
    (DRV_SERVICES(unit)->reg_field_set)
        (unit, QOS_CTLr, (uint32 *)&reg_value64, SCHEDULE_SELECTf, &temp);
...
drv_bcm53242_queue_count_set(int unit, uint32 port_type, uint8 count)
{
    ...
    if ((rv = (DRV_SERVICES(unit)->dev_prop_get)
        (unit, DRV_DEV_PROP_COSQ_NUM, &max_numq)) < 0) {
        return rv;
    }
    ...
    uint32 mapping_type, uint8 state)
{
    ...
    case DRV_QUEUE_MAP_NONE:
        ...
        if (SOC_INFO(unit).port_num > 32) {
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                (unit, QOS_1P_ENr, (uint32 *)&reg_value64, QOS_1P_ENf, (uint32 *)&temp64));
            ...
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                (unit, QOS_1P_ENr, (uint32 *)&reg_value64, QOS_1P_ENf, (uint32 *)&temp));
    ...
    case DRV_QUEUE_MAP_PRIO:
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_write)
            (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
        /* disable QOS_TOS_DIF_ENr */
        if (state) {
            reg_addr = (DRV_SERVICES(unit)->reg_addr)
                (unit, QOS_TOS_DIF_ENr, 0, 0);
            reg_len = (DRV_SERVICES(unit)->reg_length_get)
                (unit, QOS_TOS_DIF_ENr);
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_read)
                (unit, reg_addr, (uint32 *)&reg_value64,reg_len));
            if (SOC_INFO(unit).port_num > 32) {
                SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
    ...
    case DRV_QUEUE_MAP_DFSV:
        ...
        /* don't need to disable QOS_1P_ENr, the priority of QOS_TOS_DIF_ENr > QOS_1P_ENr */
        break;
...
drv_bcm53242_queue_rx_reason_set(int unit, uint8 reason, uint32 queue)
{
    ...
    reg_addr = (DRV_SERVICES(unit)->reg_addr)(unit, QOS_REASON_CODEr, 0, 0);
    reg_len = (DRV_SERVICES(unit)->reg_length_get)(unit, QOS_REASON_CODEr);
    SOC_IF_ERROR_RETURN(
        (DRV_SERVICES(unit)->reg_read)(unit, reg_addr, &reg_value, reg_len));
    ...
    switch (reason) {
        case DRV_RX_REASON_MIRRORING:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
                (unit, QOS_REASON_CODEr, &reg_value, PRI_MIRRORf, &temp));
            break;
        case DRV_RX_REASON_SA_LEARNING:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, QOS_REASON_CODEr, &reg_value, PRI_SA_LEARNf, &temp));
            break;
        case DRV_RX_REASON_SWITCHING:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, QOS_REASON_CODEr, &reg_value, PRI_SWITCHf, &temp));
            break;
        case DRV_RX_REASON_PROTO_TERM:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, QOS_REASON_CODEr, &reg_value, PRI_PROTOCOL_TERMf, &temp));
            break;
        case DRV_RX_REASON_PROTO_SNOOP:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, QOS_REASON_CODEr, &reg_value, PRI_PROTOCOL_SNOOPf, &temp));
            break;
        case DRV_RX_REASON_EXCEPTION:
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_set)
            (unit, QOS_REASON_CODEr, &reg_value, PRI_EXCEPTf, &temp));
            break;
        default:
            return SOC_E_UNAVAIL;
    }
