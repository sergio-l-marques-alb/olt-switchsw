/*
 * $Id: et_soc.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
    ...
    if (events & INTR_ERROR) {
        ...
        if ((*chops->errors)(ch)) {
             ...
             */
                ...
                (DRV_SERVICES(imp_unit)->rate_set)
                     (imp_unit, t_pbm, no_que, DRV_RATE_CONTROL_DIRECTION_EGRESSS, 
                      imp_min_rate, IMP_PORT_BURST_SIZE);            
...
et_soc_sendup(et_soc_info_t *et, robo_dv_t *dv)
{
    ...
    uchar eabuf[32];
    uchar rxhdr[HWRXOFF];
    uint8 *payload = (uint8 *)dv->dv_dcb->dcb_vaddr + HWRXOFF;
    uint16 dev_id;
    uint8 rev_id;
    ...
    sal_memcpy((void *)rxhdr, (void *)dv->dv_dcb->dcb_vaddr, HWRXOFF);
    ...
    if ((payload[2*ENET_MAC_SIZE] == (ENET_DEFAULT_BRCMID >> 8)) &&
        ...
        if (len == (uint16)(dv->dv_len + ENET_BRCM_TAG_SIZE + 4 
                     + timestamp_size)) {
    ...
    } else if (SOC_IS_ROBO53115(soc_mii_unit) || SOC_IS_ROBO53118(soc_mii_unit)) {
...
soc_imp_prot_thread(void *param)
{
    ...
    SOC_PBMP_CLEAR(t_pbm);
    SOC_PBMP_PORT_ADD(t_pbm, imp_port);
    (DRV_SERVICES(unit)->rate_set)
        (unit, t_pbm, no_que, DRV_RATE_CONTROL_DIRECTION_EGRESSS, 
        imp_max_rate, IMP_PORT_BURST_SIZE);
        ...
        switch(imp_state) {
            case IMP_PORT_STATE_RATE_MAX:
                if (( delta / (cur_time - imp_last_time)) > 
                    ...
                    (DRV_SERVICES(unit)->rate_set)
                        (unit, t_pbm, no_que, DRV_RATE_CONTROL_DIRECTION_EGRESSS, 
                        imp_mid_rate, IMP_PORT_BURST_SIZE);
