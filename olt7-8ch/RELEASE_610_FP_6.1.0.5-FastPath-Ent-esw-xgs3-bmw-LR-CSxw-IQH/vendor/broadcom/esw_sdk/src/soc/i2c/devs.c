/*
 * $Id: devs.c,v 1.1 2011/04/18 17:11:05 mruas Exp $
...
#ifdef LVL7_FIXUP
/* BCM59101 Powered Ethernet chip driver */
...
#ifdef LVL7_FIXUP
    I2CDEV(POE, 7, bcm59101),
    I2CDEV(POE, 8, bcm59101),
#endif
...
soc_i2c_probe(int unit)
{
    ...
    char		*devdesc;
    int         i2c_nvram_skip, i2c_hclk_skip, i2c_poe_power;
    ...
    i2c_nvram_skip = soc_property_get(unit, spn_I2C_NVRAM_SKIP, 0);
    i2c_hclk_skip = soc_property_get(unit, spn_I2C_HCLK_SKIP, 0);
    i2c_poe_power = soc_property_get(unit, spn_I2C_POE_POWER, 0);

    found = 0; /* Number of installed I2C devices */

    /* Probe the currently configured (i.e. MUX'd) I2C bus. */
    /* soc_cm_print("soc_i2c_probe: Probing for I2C devices\n"); */
    for (devid = 0; devid < num_i2c_devices; devid++) {
        if ( (i2c_nvram_skip) && /* check property and skip NVRAM */
	     (i2c_devices[devid].saddr == I2C_NVRAM_SADDR0) &&
	     (i2c_devices[devid].driver == &_soc_i2c_eep24c64_driver)) {
	    continue;
	} 
	else if ( (!i2c_nvram_skip) &&/* check property and skip XFP */
		  (i2c_devices[devid].saddr == I2C_XFP_SADDR0) &&
		  (i2c_devices[devid].driver == &_soc_i2c_xfp_driver)) {
	    continue;
	} 
	else if ( (i2c_hclk_skip) &&/* check property and skip HCLK */
	...
	else if ( (!i2c_hclk_skip) &&/* check property and skip POE */
        ...
        else if ( (i2c_poe_power) && /* check property and set init power level */        
	...
	if (SOC_E_NONE ==
	    ...
	    i2cbus->devs[devid] = &i2c_devices[devid];
