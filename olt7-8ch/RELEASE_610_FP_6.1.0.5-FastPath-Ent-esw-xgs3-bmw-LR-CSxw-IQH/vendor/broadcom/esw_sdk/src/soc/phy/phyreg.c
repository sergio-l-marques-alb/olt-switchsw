/*
 * $Id: phyreg.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 *data)
{
    ...
    if (flags & SOC_PHY_REG_1000X) {
        if (reg_addr <= 0xffef) {
            ...
            /* Map 1000X page */
            SOC_IF_ERROR_RETURN
                (WRITE_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, 0xfffc), 0x7c00));
            SOC_IF_ERROR_RETURN
                (READ_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, 0xfffc), &blk_sel));
            SOC_IF_ERROR_RETURN
                (WRITE_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, 0xfffc), blk_sel | 0x8001));

            /* Read 1000X IEEE register */
            SOC_IF_ERROR_RETURN
                (READ_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, reg_addr), data));

           /* Restore IEEE mapping */
            SOC_IF_ERROR_RETURN
                (WRITE_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, 0xfffc), 
                                (blk_sel & 0xfffe) | 0x8000));
        } else {
            rv = SOC_E_PARAM;
        }
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 data)
{
    ...
    if (flags & SOC_PHY_REG_1000X) {
        if (reg_addr <= 0xffef) {
            ...
            /* Write 1000X IEEE register */
            SOC_IF_ERROR_RETURN
                (WRITE_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, reg_addr), data));
                ...
                uint8 dev_addr, uint16 reg_addr, uint16 data, uint16 mask)
{
    ...
    if (flags & SOC_PHY_REG_1000X) {
        if (reg_addr <= 0xffef) {
            ...
            /* Modify 1000X IEEE register */
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY_REG(unit, pc, SOC_PHY_CLAUSE45_ADDR(dev_addr, reg_addr), data, mask));
