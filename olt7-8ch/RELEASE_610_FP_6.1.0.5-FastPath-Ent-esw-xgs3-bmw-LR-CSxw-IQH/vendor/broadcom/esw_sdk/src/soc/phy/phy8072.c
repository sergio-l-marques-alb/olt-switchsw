...
/*
 * $Id: phy8072.c,v 1.1 2011/04/18 17:11:07 mruas Exp $
...
_8073_A0_xaui_check(int unit, int port)
{
     ...
     * XAUI Workaround is not needed for Rev A1
     */
...
phy_8072_init(int unit, soc_port_t port)
{
    ...
    if (phy_ext_rom_boot) {
        ...
        if (PMDA_ROM_LOAD_8072ID == (data16 & 0xFFF0) || 
            ...
            soc_cm_debug(DK_ERR,"u=%d p=%d External ROM Boot Complete\n",
                            unit, port);
        } else {
            soc_cm_debug(DK_WARN,
                           ...
                           soc_port_ability_t *ability)
{
    ...
    ability->pause |= (pause & AN_ADVERT_PAUSE) ? SOC_PA_PAUSE : 0;
    ability->pause |= (pause & AN_ADVERT_PAUSE_ASYM) ? SOC_PA_PAUSE_ASYMM : 0;
...
{   
    ...
    if (pc->phy_id1 == PHY_ID1_8073) {
        ...
        if (ability->pause & SOC_PA_PAUSE) {
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY8072_AN_REG(unit, pc, AN_ADVERT_0_REG,
                                        AN_ADVERT_PAUSE, AN_ADVERT_PAUSE));
        } else {
            /* If PAUSE is not specified, clear it. */
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY8072_AN_REG(unit, pc, AN_ADVERT_0_REG, 
                                       0x0, AN_ADVERT_PAUSE));
        }
        ...
        if (ability->pause & SOC_PA_PAUSE_ASYMM) {
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY8072_AN_REG(unit, pc, AN_ADVERT_0_REG, 
                               AN_ADVERT_PAUSE_ASYM, AN_ADVERT_PAUSE_ASYM));
        } else {
            /* If PAUSE_ASYM is not specified, clear it. */
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY8072_AN_REG(unit, pc, AN_ADVERT_0_REG, 
                                       0x0, AN_ADVERT_PAUSE_ASYM));
        }
    }
...
phy_8072_ability_remote_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    ability->pause     |= (lp_abil & AN_ADVERT_PAUSE) ? SOC_PA_PAUSE : 0;
    ability->pause     |= (lp_abil & AN_ADVERT_PAUSE_ASYM) ? 
                          SOC_PA_PAUSE_ASYMM : 0;
    ability->interface = SOC_PA_INTF_XGMII;
    ability->medium    = SOC_PA_MEDIUM_FIBER;
    ability->loopback  = SOC_PA_LB_PHY;
    SOC_IF_ERROR_RETURN
