/*
 * $Id: phyfege.c,v 1.1 2011/04/18 17:11:08 mruas Exp $
...
#ifdef BCM_ROBO_SUPPORT
    if (SOC_IS_ROBO(unit) && IS_FE_PORT(unit,port)) {
        uint16 mii_aux;
        SOC_IF_ERROR_RETURN
            (READ_PHY_REG(unit, pc, MII_AUX_REG, &mii_aux));

        mii_aux &= ~MII_AUX_LD;
        mii_aux |= enable ? MII_AUX_LD : 0;

        SOC_IF_ERROR_RETURN
            (WRITE_PHY_REG(unit, pc, MII_AUX_REG, mii_aux));
    }
#endif /* BCM_ROBO_SUPPORT */
