/*
 * $Id: post.c,v 1.1 2011/04/18 17:11:04 mruas Exp $
...
_soc_raptor_port_probe(int unit, soc_port_t p, mac_driver_t **macdp)
{
    ...
    *macdp = NULL;
    ...
    soc_cm_debug(DK_VERBOSE, "Init port %d PHY...\n", p);
    ...
    soc_cm_debug(DK_VERBOSE, "Init port %d MAC...\n", p);
...
#define TABLE_HAS_UT_BITMAP(unit, table)                        \
    ((SOC_IS_EASYRIDER(unit) && (table) == VLAN_TABm)   ||      \
...
_soc_raptor_post_run(int unit, uint32 *tx_pkt)
{
    ...
    soc_cm_debug(DK_VERBOSE, "Running post on unit %d \n", unit);
    ...
    for(count = 0; count < SOC_RAPTOR_POST_MAX_COUNT; count++) {
        ...
        PBMP_ITER(fe_pbmp, p) {
            ...
            SOC_IF_ERROR_RETURN(soc_phyctrl_loopback_set(unit, p, TRUE));
	...
	/* macd is for the last FE port. But it should be fine */
        ...
        SOC_IF_ERROR_RETURN(soc_phyctrl_loopback_set(unit, p, FALSE));
    }
...
soc_post_init(int unit)
{
    ...
    runpost = soc_property_get(unit, spn_POST_ENABLE, 1);
    if (runpost) {
