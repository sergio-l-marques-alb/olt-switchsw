/*
 * $Id: bcm-core.c,v 1.1 2011/04/18 17:11:10 mruas Exp $
...
_startup(void)
{
	...
	if (bcore_sysconf_attach(unit) < 0) {
	    gprintk("ERROR: PCI device attach failed\n");
	    return -1;
	}
    }
...
bcore_init(int unit)
{
	...
	SYSTEM_INIT_CHECK(bcm_ds_init(unit), "Diffserv init");
    }
