/*
 * $Id: bcm-net.c,v 1.1 2011/04/18 17:11:10 mruas Exp $
...
{
    return ((struct bnet_priv_t *)dev->priv)->bdev;
}
...
bnet_tx(struct sk_buff *skb, struct net_device *dev)
{
    ...
    struct bnet_priv_t *pd = (struct bnet_priv_t *)bnet_dev[0].ndev->priv;
...
bnet_proc_init(void)
{
    struct proc_dir_entry *stats_root = NULL;
    char scratch[64];
    pbmp_t pbm;
    bcm_port_t port;
...
_init(void)
{
    ...
    if ((dev = bnet_init(bnet_dev_mac)) == NULL) {
        return -1;
    }
    bnet_dev[0].ndev = dev;
    bnet_init_bdev_pbmp(&bnet_dev[0]);
    ((struct bnet_priv_t *)dev->priv)->bdev = &bnet_dev[0];
    ...
    for (i = 1; bnet_dev[i].pmask; i++) {
        ...
        ((struct bnet_priv_t *)dev->priv)->bdev = &bnet_dev[i];
    }
