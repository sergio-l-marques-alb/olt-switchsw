/* 
 * $Id: vlan.c,v 1.1 2011/04/18 17:11:11 mruas Exp $
                             ...
                             vlan_xlate_entry_t *entry, int dual)
{
    ...
    soc_internal_read_reg(pcid_info,
          soc_reg_addr(pcid_info->unit, VLAN_XLATE_HASH_CONTROLr,
                       REG_PORT_ANY, 0),
          tmp_hs);
    if (dual) {
    ...
    // PTin added: SOC_IS_VALKYRIE2(pcid_info->unit)
    if (SOC_IS_TRIUMPH2(pcid_info->unit) || SOC_IS_APOLLO(pcid_info->unit) || SOC_IS_ENDURO(pcid_info->unit) || SOC_IS_VALKYRIE2(pcid_info->unit)) {
        bits = soc_tr2_vlan_xlate_base_entry_to_key(pcid_info->unit, 
                                                   (uint32 *)entry, key);

        index = soc_tr2_vlan_xlate_hash(pcid_info->unit, hash_sel, bits, 
                                       (uint32 *) entry, key);
    } else
#endif
    {
                                   ...
                                   vlan_xlate_entry_t *entry)
{
    ...
    uint32 address = soc_mem_addr(pcid_info->unit, VLAN_XLATEm,
    ...
    soc_internal_read_mem(pcid_info, address, (uint32 *) entry);
                                    ...
                                    vlan_xlate_entry_t *entry)
{
    ...
    soc_internal_write_mem(pcid_info, address, (uint32 *) entry);
                                  ...
                                  vlan_xlate_entry_t *entry, uint32 *result)
{
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        index = bucket * 8 + slot;
        soc_internal_read_mem(pcid_info, soc_mem_addr(unit, VLAN_XLATEm,
                                         SOC_MEM_BLOCK_ANY(unit, VLAN_XLATEm),
                                         index), tmp);
    ...
    if (free_index != -1) {
        ...
        debugk(DK_VERBOSE, "write bucket %d, slot %d, index %d\n",
               index / 8, index % 8, index);
        soc_internal_write_mem(pcid_info, soc_mem_addr(unit, VLAN_XLATEm,
                                          SOC_MEM_BLOCK_ANY(unit, VLAN_XLATEm),
                                          index), (uint32 *)entry);
        return 0;
    }
                                 ...
                                 egr_vlan_xlate_entry_t *entry, int dual)
{
    ...
    soc_internal_read_reg(pcid_info,
          soc_reg_addr(pcid_info->unit, EGR_VLAN_XLATE_HASH_CONTROLr,
                       REG_PORT_ANY, 0),
          tmp_hs);
    if (dual) {
    ...
    // PTin added: SOC_IS_VALKYRIE2(pcid_info->unit)
    if (SOC_IS_TRIUMPH2(pcid_info->unit) || SOC_IS_APOLLO(pcid_info->unit) || SOC_IS_ENDURO(pcid_info->unit) || SOC_IS_VALKYRIE2(pcid_info->unit)) {
        bits = soc_tr2_egr_vlan_xlate_base_entry_to_key(pcid_info->unit, 
                                                       (uint32 *)entry, key);

        index = soc_tr2_egr_vlan_xlate_hash(pcid_info->unit, hash_sel, bits, key);
    } else
#endif
    {
        ...
        index = soc_tr_egr_vlan_xlate_hash(pcid_info->unit, hash_sel, bits, key);
    }
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        soc_internal_read_mem(pcid_info, 
                              soc_mem_addr(unit, EGR_VLAN_XLATEm,
                                     SOC_MEM_BLOCK_ANY(unit, EGR_VLAN_XLATEm),
                                           index), tmp);
                                      ...
                                      uint32 *result)
{
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        index = bucket * 8 + slot;
        soc_internal_read_mem(pcid_info, soc_mem_addr(unit, EGR_VLAN_XLATEm,
                                         SOC_MEM_BLOCK_ANY(unit, EGR_VLAN_XLATEm),
                                         index), tmp);
        if (soc_mem_field32_get(unit, EGR_VLAN_XLATEm, tmp, VALIDf)) {
            if (soc_mem_compare_key(unit, EGR_VLAN_XLATEm, entry, tmp) == 0) {
