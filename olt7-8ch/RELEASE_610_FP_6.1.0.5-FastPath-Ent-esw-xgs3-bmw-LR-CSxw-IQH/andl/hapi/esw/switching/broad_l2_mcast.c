...
void hapiBroadL2McastReInit(void)
{
  ...
  for (i = 0; i < maxEntries; i++)
  {
    ...
    memset(&mcastGroupList[i].mcMacAddr,0,sizeof(bcmx_l2_addr_t));     
...
*
*********************************************************************/
L7_RC_t hapiBroadL2McastUSPJoin(bcmx_l2_addr_t l2addr, bcm_vlan_t vid, DAPI_USP_t *usp, DAPI_t *dapi_g)
{
      ...
      hapiBroadL2McastLagDistributionSupported())
  {
    ...
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        bcmx_lplist_add(&addPorts, lport);
      }
    }
    hapiBroadLagCritSecExit ();

    rv = usl_bcmx_multicast_join_ports(&l2addr, addPorts);
    bcmx_lplist_free(&addPorts);
  }
  else
  {
    if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
    ...
    rv = usl_bcmx_multicast_egress_add(&l2addr, lport);
...
/*********************************************************************
*
* @purpose Remove a USP (physical or LAG or L2 tunnel) from a L2 mcast entry
*
* @returns L7_RC_t result
*
* @end
*
*********************************************************************/
L7_RC_t hapiBroadL2McastUSPLeave(bcmx_l2_addr_t l2addr, bcm_vlan_t vid, DAPI_USP_t *usp, DAPI_t *dapi_g)
{
  BROAD_PORT_t               *hapiLagMemberPortPtr, *hapiPortPtr;
  L7_uint32                   i;
  bcmx_lport_t                lport = 0;
      ...
      hapiBroadL2McastLagDistributionSupported())
  {
    /* This is a LAG on an XGS3 or later device. All member ports
       are in the mcast group. */
    bcmx_lplist_init(&removePorts,0,0);
    hapiBroadLagCritSecEnter ();
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        bcmx_lplist_add(&removePorts, lport);
      }
    }
    hapiBroadLagCritSecExit ();

    rv = usl_bcmx_multicast_leave_ports(&l2addr, removePorts);
    bcmx_lplist_free(&removePorts);
  }
  else
  {
    hapiPortPtr = HAPI_PORT_GET(&localUsp, dapi_g);
    ...
    rv = usl_bcmx_multicast_egress_delete(&l2addr, lport);
  }
...
L7_RC_t hapiBroadGarpGroupRegModify(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    hapiBroadL2McastCritSecExit();
    SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR, "\n%s %d: Failure in %s (vlan=%u)!\n",
                   __FILE__, __LINE__, __FUNCTION__,garpMgmt->cmdData.groupRegModify.vlanId);
    return L7_FAILURE;
  }
  ...
  if (index == maxEntries)
  {
    ...
    bcmx_l2_addr_init(&mcastGroupList[index].mcMacAddr, 
    ...
    /* To allow wlan virtual ports to be part of L2 MC groups, they need to use l3_ipmc table resources (under the covers) */
    if (hapiBroadWlanHardwareFeatureSupport(dapi_g)) 
    {
#ifdef L7_WIRELESS_PACKAGE
    _BCM_MULTICAST_GROUP_SET(group, _BCM_MULTICAST_TYPE_WLAN, mcastGroupList[index].ipmc_index);
#else
    _BCM_MULTICAST_GROUP_SET(group, _BCM_MULTICAST_TYPE_L2, mcastGroupList[index].ipmc_index);
#endif
    }
    else
    {
    ...
    mcastGroupList[index].mcMacAddr.l2mc_index = group;

    /* Add MAC addr to hw ARL table */
    mcastGroupList[index].mcMacAddr.flags |= BCM_L2_STATIC | BCM_L2_MCAST | BCM_L2_REPLACE_DYNAMIC;
    rv = usl_bcmx_multicast_addr_add(&mcastGroupList[index].mcMacAddr);
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      (void)usl_bcmx_multicast_addr_remove(&mcastGroupList[index].mcMacAddr);
      hapiBroadL2McastCritSecExit();
      result =  L7_FAILURE;
      SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR, "\n%s %d: Failure in %s!\n",
                     __FILE__, __LINE__, __FUNCTION__);
  ...
  while (hapiBroadMgmNextPortGet(index, firstPort, &localUsp) == L7_SUCCESS)
  {
    ...
    if (port >= garpMgmt->cmdData.groupRegModify.numOfPorts)
    {
      ...
      if (hapiBroadL2McastUSPLeave(mcastGroupList[index].mcMacAddr, mcastGroupList[index].mcMacAddr.vid, &localUsp, dapi_g) != L7_SUCCESS)
      {
        hapiBroadL2McastCritSecExit();
        result =  L7_FAILURE;
        SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR, "\n%s %d: Failure in %s!\n",
  ...
  for (port = 0; port < garpMgmt->cmdData.groupRegModify.numOfPorts; port++)
  {
    ...
    {
      if (hapiBroadL2McastUSPJoin(mcastGroupList[index].mcMacAddr, mcastGroupList[index].mcMacAddr.vid, &localUsp, dapi_g) != L7_SUCCESS)
      {
        hapiBroadL2McastCritSecExit();
        result =  L7_FAILURE;
        SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR, "\n%s %d: Failure in %s!\n",
...
L7_RC_t hapiBroadGarpGroupRegDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  for (index = 0; index < maxEntries; index++)
  {
    ...
    {
      rv = usl_bcmx_multicast_addr_remove(&mcastGroupList[index].mcMacAddr);
      numMcastGroups--;
...
void hapiBroadMgmPortVlanAddNotify (DAPI_USP_t *usp, L7_ushort16 vlan_id, DAPI_t *dapi_g)
{
  ...
  for (index = 0; index < maxEntries; index++)
  {
    ...
    {
      if (hapiBroadL2McastUSPJoin(mcastGroupList[index].mcMacAddr,
...
void hapiBroadMgmPortVlanRemoveNotify (DAPI_USP_t *usp, L7_ushort16 vlan_id, DAPI_t *dapi_g)
{
  ...
  for (index = 0; index < maxEntries; index++)
  {
    ...
    if (MGM_IS_PORT_MEMBER(&mcastGroupList[index], usp))
    {
      if (hapiBroadL2McastUSPLeave(mcastGroupList[index].mcMacAddr,
                                   mcastGroupList[index].mcMacAddr.vid,
                                   usp, dapi_g) != L7_SUCCESS)
      {
        SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR, "\n%s %d: Failure in %s!\n",
  ...
  if (hapiBroadL2McastLagDistributionSupported() == L7_TRUE)
  {
    ...
    for (index = 0; index < maxEntries; index++)
    {
      ...
      if (MGM_IS_PORT_MEMBER(&mcastGroupList[index], lagUsp))
      {
        ...
        {
          /* time to commit */
          rv = customx_port_multicast_join_groups(port, mgmBcmxMcastGroups);
  ...
  if (hapiBroadL2McastLagDistributionSupported() == L7_TRUE)
  {
    ...
    for (index = 0; index < maxEntries; index++)
    {
      ...
      if (MGM_IS_PORT_MEMBER(&mcastGroupList[index], lagUsp))
      {
        ...
        {
          /* time to commit */
          rv = customx_port_multicast_leave_groups(port, mgmBcmxMcastGroups);
...
{
  /* just remove the port, then re-add it to set the tagging appropriately */
#if 0
  /* These updates are unnecessary for any chip after StrataII (which we no longer support). */
