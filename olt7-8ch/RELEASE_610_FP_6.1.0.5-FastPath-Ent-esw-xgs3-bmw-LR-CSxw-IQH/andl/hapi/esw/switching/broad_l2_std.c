...
#include "bcmx/stg.h"
#include "bcmx/auth.h"
#include "bcmx/bcmx_int.h"  /* PTin added */
...
#include "ptin_hapi.h"    /* PTin added */

L7_BOOL ptin_learnEnabled[L7_MAX_INTERFACE_COUNT];    /* PTin added */
...
/* Double Vlan Tag Ethertype initialized to 802.1q*/
L7_ushort16 hapiBroadDvlanEthertype = FD_DVLANTAG_DEFAULT_ETHERTYPE;
...
static  bcmx_lplist_t protectedGroupList;        /* list of protected ports */
...
// PTin added
L7_uint8 ptin_lag_pending_configs = 0;
...
L7_RC_t hapiBroadL2StdPortInit(DAPI_PORT_t *dapiPortPtr)
{
  ...
  dapiPortPtr->cmdTable[DAPI_CMD_ADDR_SET_LEARN_MODE          ] = (HAPICTLFUNCPTR_t)hapiBroadAddrSetLearnMode;  /* PTin added */
...
L7_RC_t hapiBroadSystemDot1sInstanceDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  L7_uint32              instNumber;
  L7_int32              rc;
  L7_int32              oldnumVlans;
  L7_uint32             vlanIndex, stg_index;
  bcm_vlan_t           *oldVlans = L7_NULLPTR;
  ...
  /* dis-associate all VLANs w/ this stg */
  /* Get the existing list of vlans for this instance */
  rc = usl_stg_vlan_list_get(stg, &oldVlans, &oldnumVlans);

  if (BCM_E_NOT_FOUND == rc)
  {
    (void)hapiBroadDot1sBcmStgMapClear(instNumber);
    return(L7_SUCCESS);
  ...
  /* Loop through and delete the removed vlans from the instance */
  for (vlanIndex = 0; vlanIndex < oldnumVlans; vlanIndex++)
  {
    /* Call BCMX to remove VLAN from STG instance */
    rc = usl_bcmx_stg_vlan_remove (stg, oldVlans[vlanIndex]);

#ifdef HAPI_BROAD_DOT1S_DEBUG
    if (hapiDot1sDebug)
    {
      SYSAPI_PRINTF( SYSAPI_LOGGING_ALWAYS,
                     "  Removing VLAN %d from STG ID %d\n",
                     oldVlans [vlanIndex], stg);
    }
#endif
    ...
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      ...
      /* Free the vlan list */
      usl_stg_vlan_list_destroy(oldVlans, oldnumVlans);
...
L7_RC_t hapiBroadSystemDot1sInstanceVlanAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  rc = usl_bcmx_stg_vlan_add (stg, vlanId);
...
L7_RC_t hapiBroadSystemDot1sInstanceVlanRemove(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  rc = usl_bcmx_stg_vlan_remove(stg, vlanId);
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr) == L7_TRUE)
  {
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        rc = bcmx_stg_stp_set (stg, lport, stgState);
  ...
  while (num_msgs--)
  {
    ...
    switch (dot1s_cmd.async_cmd)
    {
      case HAPI_BROAD_DOT1S_STATE_SET:
        ...
        processTime = osapiTimeMillisecondsGetOffset(dot1s_cmd.enqueTime);
...
void hapiBroadDot1sPortStateCopy(DAPI_USP_t *destUsp, DAPI_USP_t *srcUsp, DAPI_t *dapi_g)
{
  ...
  for (index = 0; index < HAPI_MAX_MULTIPLE_STP_INSTANCES; index++)
  {
    if (broadDot1sMapping[index].stg != BCM_STG_INVALID)
    {
      ...
      /* Call BCMX to set the state for a port in a STG instance */
      rc = bcmx_stg_stp_set (stg, lport, hapiSrcPortPtr->hw_dot1s_state[index]);
      if (L7_BCMX_OK(rc) != L7_TRUE)
      {
        LOG_ERROR(rc);
      }
                                         ...
                                         DAPI_t *dapi_g)
{
  ...
  for (index = 0; index < HAPI_MAX_MULTIPLE_STP_INSTANCES; index++)
  {
    if (broadDot1sMapping[index].stg != BCM_STG_INVALID)
    {
      ...
      /* Call BCMX to set the state for a port in a STG instance */
      rc = bcmx_stg_stp_set (stg, lport, state);
      if (L7_BCMX_OK(rc) != L7_TRUE)
      {
        LOG_ERROR(rc);
      }
...
L7_RC_t hapiBroadIntfDoubleVlanTagConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (dapiCmd->cmdData.doubleVlanTagConfig.getOrSet == DAPI_CMD_SET)
  {
            ...
            (dapiCmd->cmdData.doubleVlanTagConfig.enable == L7_TRUE))
    {
        ...
        if(HAPI_BROAD_DTAG_XGS2)
            mode = HAPI_BROAD_DTAG_MODE_INTERNAL;
        else
        {
          // PTin modified
          mode = HAPI_BROAD_DTAG_MODE_INTERNAL /*HAPI_BROAD_DTAG_MODE_EXTERNAL*/;
          /* Need TR specific condition */
        ...
        /* But this function even manipulates the default TPID,with 0x8100 on Triupmh*/
        result = usl_bcmx_dvlan_mode_set(mode,dapiCmd->cmdData.doubleVlanTagConfig.etherType);
        if (result != L7_SUCCESS)
            return result;

       /* To get ride of the above problem, calling one more USL function to 
          Set the default TPID per system */
        if (isTriumphFamily == L7_TRUE)
        {
          result = usl_bcmx_dvlan_default_tpid_set(dapiCmd->cmdData.doubleVlanTagConfig.etherType);
          if (result != L7_SUCCESS)
            return result;
        }
    ...
    {
         /* Should be TR specific */
         if (isTriumphFamily == L7_TRUE)
         {
           // PTin modified
            result = usl_bcmx_dvlan_mode_set(HAPI_BROAD_DTAG_MODE_EXTERNAL /*HAPI_BROAD_DTAG_MODE_EXTERNAL*/,
                                          dapiCmd->cmdData.doubleVlanTagConfig.etherType);
            if (result != L7_SUCCESS)
              return result;
         }

        result = usl_bcmx_dvlan_mode_set(HAPI_BROAD_DTAG_MODE_NONE,
                                         dapiCmd->cmdData.doubleVlanTagConfig.etherType);
        if (result != L7_SUCCESS)
            return result;

        /* Should be TR specific. The above call is sets the system TPID to 0x8100 in triumph.
           Placing a work around here.This call sets the system TPID to configured value.
        */
        #if 0
        if (isTriumphFamily == L7_TRUE)
        {
  ...
  do
  {
    if (dapiCmd->cmdData.doubleVlanTagConfig.enable != L7_TRUE)
    {
       ...
       */
        ...
        if((dtagPortCount == 0))
           mode = HAPI_BROAD_DTAG_MODE_NONE;
        else if (HAPI_BROAD_DTAG_XGS2)
            mode = HAPI_BROAD_DTAG_MODE_INTERNAL;
        else
        {
          // PTin modified
          mode = HAPI_BROAD_DTAG_MODE_INTERNAL /*HAPI_BROAD_DTAG_MODE_EXTERNAL*/;
          /* For Triumph this should be Internal. */
          if (isTriumphFamily == L7_TRUE)
          {
            mode = HAPI_BROAD_DTAG_MODE_INTERNAL;
           }
    ...
    rc = l7_bcmx_port_dtag_mode_set(hapiPortPtr->bcmx_lport,mode);
    ...
    if ( (mode == HAPI_BROAD_DTAG_MODE_INTERNAL)  /*||
         (mode == HAPI_BROAD_DTAG_MODE_NONE) */
       )
    {
      rc = l7_bcmx_port_tpid_set(hapiPortPtr->bcmx_lport,
                               dapiCmd->cmdData.doubleVlanTagConfig.etherType);
...
void hapiBroadLearnSet(DAPI_USP_t *usp, L7_uint32 flags, DAPI_t *dapi_g)
{
  ...
  L7_uint32             lp, p;
  DAPI_USP_t           *usp_lag;
  DAPI_PORT_t          *dapiPortPtr_lag;
  BROAD_PORT_t         *hapiPortPtr_lag;
  bcmx_lport_t          lport_lag;
  // PTin end
  ...
  // PTin added
  // learning is disabled, make sure flag BCM_PORT_LEARN_ARL is deactivated!
  if (!ptin_learnEnabled[port] && (flags & BCM_PORT_LEARN_ARL))
    flags &= ~((L7_uint32) BCM_PORT_LEARN_ARL);
  // PTin end

  /* assumes this function is only called w/ physical and LAG ports */
  if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    rc = l7_bcmx_port_learn_set(lport, flags);
    if (L7_BCMX_OK(rc) != L7_TRUE)
    {
      SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR,
    ...
    // PTin added
    // If this physical port belongs to a lag, verify if all lag ports have the learning deactivated
    if (hapiPortPtr->hapiModeparm.physical.isMemberOfLag)
    {
      usp_lag         = &(hapiPortPtr->hapiModeparm.physical.lagUsp);
      dapiPortPtr_lag = DAPI_PORT_GET(usp_lag, dapi_g);
      hapiPortPtr_lag = HAPI_PORT_GET(usp_lag, dapi_g);
      lport_lag       = hapiPortPtr_lag->bcmx_lport;

      L7_BOOL lag_learning_active = L7_FALSE;
      hapiBroadLagCritSecEnter ();
      for (i = 0; i < L7_MAX_MEMBERS_PER_LAG && !lag_learning_active; i++)
      {
        if (dapiPortPtr_lag->modeparm.lag.memberSet[i].inUse == L7_TRUE)
        {
          hapiLagMemberPortPtr = HAPI_PORT_GET(&dapiPortPtr_lag->modeparm.lag.memberSet[i].usp, dapi_g);
          lp = hapiLagMemberPortPtr->bcmx_lport;
          p = BCMX_LPORT_BCM_PORT(lp);

          if (p<SYSTEM_N_PORTS)  continue;
          if (ptin_learnEnabled[p]) lag_learning_active = L7_TRUE;
        }
      }
      hapiBroadLagCritSecExit ();
      usl_bcmx_trunk_learn_mode_set(hapiPortPtr_lag->hapiModeparm.lag.tgid, (lag_learning_active) ? BCM_PORT_LEARN_ARL : 0);
    }
  }
  else if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr))
  {
    ...
    hapiBroadLagCritSecExit ();
    usl_bcmx_trunk_learn_mode_set(hapiPortPtr->hapiModeparm.lag.tgid, (flags == BCM_PORT_LEARN_CPU) ? 1 : 0);
  }
...
L7_RC_t hapiBroadAddrMacAddressEntryAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  mac_addr_t                    macAddr;
  L7_int32                      rc=0;
  L7_uchar8                     virtualMac[5] = {0x00, 0x00, 0x5E, 0x00, 0x01};
  ...
  } else if (dapiCmd->cmdData.macAddressEntryAdd.flags == DAPI_ADDR_FLAG_MANAGEMENT)
  {
	...
	/* Add the Virtual mac with L3 bit set. */
	if (memcmp((void *)macAddr, (void *)virtualMac, sizeof(virtualMac)) == 0) 
	{
	  l2Addr.flags = (BCM_L2_STATIC | BCM_L2_L3LOOKUP | BCM_L2_REPLACE_DYNAMIC);
	}
	else
	{
...
L7_RC_t hapiBroadAddrMacAddressEntryDelete(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
//  printf("Not found entry %02X:%02X:%02X:%02X:%02X:%02X VLAN=%u flags=0x%X\r\n",
//         macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5],
//         dapiCmd->cmdData.macAddressEntryDelete.vlanID,dapiCmd->cmdData.macAddressEntryDelete.flags);
  ...
  } else if (rc == BCM_E_NONE)
  {
    ...
    if (rc!=BCM_E_NONE)
    {
//    printf("Error deleting address: %02X:%02X:%02X:%02X:%02X:%02X VLAN=%u flags=0x%X rc=%d(%s)\r\n",
//           macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5],
//           dapiCmd->cmdData.macAddressEntryDelete.vlanID,dapiCmd->cmdData.macAddressEntryDelete.flags,
//           rc,bcm_errmsg(rc));
    }
  ...
  {
//  printf("Other error: %02X:%02X:%02X:%02X:%02X:%02X VLAN=%u flags=0x%X rc=%d(%s)\r\n",
//         macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5],
//         dapiCmd->cmdData.macAddressEntryDelete.vlanID,dapiCmd->cmdData.macAddressEntryDelete.flags,
//         rc,bcm_errmsg(rc));
  }
...
L7_RC_t hapiBroadAddrAgingTime(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    /* if the agingTime is zero, flush all dynamic entries on this VLAN */
    if (ageTime == 0)
    {
...
L7_RC_t hapiBroadAddrSetLearnMode(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (dapiCmd->cmdData.portAddressSetLearnMode.getOrSet == DAPI_CMD_SET)
  {
    ...
    {
      // Save new state
      // It should be done before changing effectively the state, because it depends on this variable
      ...
      if (!(hapiPortPtr->l2FlushInProgress))
      {
        ...
        else
        {
          ...
          // Clear physical port entries in L2 table
          ...
          if (hapiPortPtr->hapiModeparm.physical.isMemberOfLag)
          {
            ...
            // If no interface of this lag is learning, clear all lag entries from L2 table
            if (!lag_learning_active)
              usl_bcmx_l2_addr_remove_by_trunk(hapiPortPtr_lag->hapiModeparm.lag.tgid,L7_FALSE);
          }
        }
      }
    }
...
L7_RC_t hapiBroadAddrFlushAll(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  usl_bcmx_l2_addr_remove_all(L7_TRUE);  /* PTin modified */
...
L7_RC_t hapiBroadAddrMacFilterAdd(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (hapiBroadAddrMacFilterFind(macAddr, vlanID, &idx) != L7_SUCCESS)
  {
    ...
    /* create a new policy */
    hapiBroadPolicyCreate(BROAD_POLICY_TYPE_SYSTEM, &policyId);
    hapiBroadPolicyRuleAdd(policyId, &ruleId);
    hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_MACDA, macAddr.addr, exact_match);
    hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_OVID, (L7_uchar8*)&vlanID, exact_match);
    hapiBroadPolicyRuleActionAdd(policyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0); /* matching rules should drop the packet */
...
volatile L7_BOOL hapiPtinL2Freeze=L7_FALSE;
osapiTimerDescr_t *hapiPtinL2FreezeTimeoutDesc = L7_NULL;

void hapiPtinL2FreezeTimeoutCallback(L7_uint32 arg1, L7_uint32 arg2)
{
  if (hapiPtinL2Freeze)
  {
    printf("Going to release timer\r\n");

    /* Unfreeze L2 learning */
    bcmx_l2_addr_thaw();
    printf("Released\r\n");

    hapiPtinL2Freeze = L7_FALSE;
  }
}
...
void hapiBroadAddrMacUpdate(void *bcmx_l2, int insert, DAPI_t *dapi_g)
{
...
#if 0
  L7_uint32 call_rate, time_delta;
  L7_clocktime ct_now;
  L7_uint64 time_now;
  static L7_uint64 time_old=0;
  static L7_uint32 calls_made=0;

  /* Measure current instant of time */
  osapiClockTimeRaw ( &ct_now );
  time_now = (L7_uint64) ct_now.seconds*1000ULL + (L7_uint64) ct_now.nanoseconds/1000000ULL;

  /* Init */
  if ( hapiPtinL2Freeze || (time_old == 0) || (time_now < time_old) )
  {
    //printf("Init done\r\n");
    calls_made = 0;
    time_old = time_now;
  ...
  else
  {
    ...
    time_delta = (L7_uint32) (time_now - time_old);
  
    /* If sampling period is lower than 2 seconds, increment number of function calls */
    if (time_delta<2000)
    {
      calls_made++;
    }
    ...
    {
      /* Calculate function calls rate */
      call_rate = (calls_made*1000UL) / time_delta;

      /* If more than 50 calls were done, freeze learning */
      if (call_rate>50)
      {
        /* Release Timer */

        //osapiTimerFree(hapiPtinL2FreezeTimeoutDesc);
        //hapiPtinL2FreezeTimeoutDesc = L7_NULL;

        //bcmx_l2_addr_freeze();
        hapiPtinL2Freeze = L7_TRUE;

        printf("We have an overloading situation... starting timer!\r\n");

        /* Restart timer */
        osapiTimerAdd((void *) hapiPtinL2FreezeTimeoutCallback,0,0,2000,&hapiPtinL2FreezeTimeoutDesc);
      }
...
void hapiBroadAddrMacUpdateLearn(bcmx_l2_addr_t *bcmx_l2_addr, DAPI_t *dapi_g)
{
  ...
  /* Ignore entries that we added to block clients for dot1x. These entries should exist
     only on the bcm unit off of which the client exists. Here we determine the blocked
     dot1x clients by checking the discard flags. */
  if ((bcmx_l2_addr->flags & BCM_L2_DISCARD_SRC) &&
      (bcmx_l2_addr->flags & BCM_L2_DISCARD_DST))
  {
    return;
  }
...
void hapiDebugMacStatsPrint()
{
  if (usl_is_present())
  {
    printf("Use the usl_macsync_stats_print() instead for stacking builds\n");
    return;
  }
...
#else
  if (bde->num_devices(BDE_ALL_DEVICES) == 1) 
#endif
  {
	return result;
  }
...
void hapiBroadL2AddrFlushTask(L7_uint32 numArgs, DAPI_t *dapi_g)
{
  ...
  while (1)
  {
    ...
    {
      /* no flush requested */
      LOG_MSG("hapiL2AddrFlushQueue: failed in recv message\n");
      continue;
    }
    ...
    BCMX_LPLIST_ITER(hapiBroadFlushTasklpList, lport, count)
    {
      ...
      if (BCMX_LPORT_VALID(lport))
      {
        ...
        (void) usl_bcmx_l2_addr_remove_by_port(lport,L7_TRUE);  /* PTin modified */
      }
    }
      ...
      if (hapiBroadFlushTgidList[index] != 0)
      {
        for (bitpos = 0;bitpos < 32;bitpos++)
        {
          if ( hapiBroadFlushTgidList[index] & (1 << bitpos)) 
          {
            ...
            (void) usl_bcmx_l2_addr_remove_by_trunk (tgid, L7_TRUE);  /* PTin modified */
            count++;
          }
        }
      }
...
L7_RC_t hapiBroadDhcpPolicyInstall(DAPI_t *dapi_g)
{
  ...
  hapiBroadPolicyCreate(policyType, &policyId);
  hapiSystem->dhcpSnoopingPolicyId[0] = policyId;
  ...
  //hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_DROP, (L7_uchar8*)&drop_packet, exact_match);
  ...
  // PTin added: For double-tagged packets
  hapiBroadPolicyRuleAdd(policyId, &ruleId);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&innertpid, exact_match);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP_TYPE, ip_type, exact_match);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP6_DST, dstip6_data, dstip6_mask);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP6_SRC, srcip6_data_client, srcip6_mask);
  //hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_DROP, (L7_uchar8*)&drop_packet, exact_match);
  hapiBroadPolicyRuleActionAdd(policyId, ruleId, BROAD_ACTION_TRAP_TO_CPU, 0, 0, 0);
  hapiBroadPolicyRuleNonConfActionAdd(policyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
  hapiBroadPolicyRuleMeterAdd(policyId, ruleId, &meterInfo);
  ...
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP_TYPE, ip_type, exact_match);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP6_DST, dstip6_data, dstip6_mask);
  hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_IP6_SRC, srcip6_data_server, srcip6_mask);
  //hapiBroadPolicyRuleQualifierAdd(policyId, ruleId, BROAD_FIELD_DROP, (L7_uchar8*)&drop_packet, exact_match);
  hapiBroadPolicyRuleActionAdd(policyId, ruleId, BROAD_ACTION_TRAP_TO_CPU, 0, 0, 0);
  hapiBroadPolicyRuleNonConfActionAdd(policyId, ruleId, BROAD_ACTION_HARD_DROP, 0, 0, 0);
  hapiBroadPolicyRuleMeterAdd(policyId, ruleId, &meterInfo);
  ...
  {
    result = hapiBroadPolicyRemoveFromAll(policyId);
    if (L7_SUCCESS != result)
       return result;
  }
  #endif
  ...
  if (hapiBroadRaptorCheck() == L7_FALSE) /* Can't do this on raptor */
  {
    ...
    result = hapiBroadPolicyCommit(policyId);
    if (result==L7_SUCCESS)  hapiSystem->dhcpDefaultPolicyId[0] = policyId;
    ...
    result = hapiBroadPolicyCommit(policyId);
    if (result==L7_SUCCESS)  hapiSystem->dhcpDefaultPolicyId[1] = policyId;
  }
  else
  {
                                        ...
                                        DAPI_t     *dapi_g)
{
  ...
  for (i=0; i<120; i++)
  {
    for (j=0; j<2; j++)
    {
      if (BROAD_POLICY_INVALID != hapiSystem->dhcpSnoopingPolicyId[i][j])
      {
        lport = hapiPortPtr->bcmx_lport;
        if (enabled == L7_TRUE)
        {
          rc = hapiBroadPolicyApplyToIface(hapiSystem->dhcpSnoopingPolicyId[i][j], lport);
        }
        else 
        {
          rc = hapiBroadPolicyRemoveFromIface(hapiSystem->dhcpSnoopingPolicyId[i][j], lport);
        }
      }
    }
  }
									   ...
									   void *data, DAPI_t *dapi_g)
{
  ...
  rv = bcmx_lplist_init(&protectedGroupList, L7_MAX_INTERFACE_COUNT, 0);
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
	return L7_FAILURE;
  }
  ...
  for (index = 0; index < L7_MAX_INTERFACE_COUNT; index++)
  {
	...
	rv = bcmx_lplist_add(&protectedGroupList, hapiPortPtr->bcmx_lport);
	if (L7_BCMX_OK(rv) != L7_TRUE)
	{
	  (void)bcmx_lplist_free(&protectedGroupList);
	  return L7_FAILURE;
	}
  ...
  rv = usl_bcmx_protected_group_set(protectedGroupList);
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
    SYSAPI_PRINTF(SYSAPI_LOGGING_HAPI_ERROR,
                                       ...
                                       void *data, DAPI_t *dapi_g)
{
  ...
  /* Add all the ports in the list */
  BCMX_FOREACH_LPORT(tmpPort)
  {
    rv = bcmx_lplist_add(&protectedGroupList, tmpPort);
	if (L7_BCMX_OK(rv) != L7_TRUE)
	{
      (void)bcmx_lplist_free(&protectedGroupList);
	  return L7_FAILURE;
  ...
  hapiPortPtr = HAPI_PORT_GET(usp, dapi_g);
  rv = bcmx_port_egress_set(hapiPortPtr->bcmx_lport, 0, protectedGroupList);
  if (L7_BCMX_OK(rv) != L7_TRUE)
  {
    result = L7_FAILURE;
  }
...
void hapiBroadL2FlushRequest(BROAD_L2ADDR_FLUSH_t flushReq)
{
  do {
    ...
    else
    {
      ...
      LOG_MSG("Invalid Flush type %d requested\n",flushReq.flushtype);
                         ...
                         DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
    {
      if (dapiPortPtr->modeparm.lag.memberSet[i].inUse == L7_TRUE)
      {
        ...
        if (dapiCmd->cmdData.doubleVlanTagConfig.getOrSet == DAPI_CMD_SET)
        {
          if (dapiCmd->cmdData.doubleVlanTagConfig.enable == L7_TRUE)
          {
            rv = bcmx_port_tpid_add (hapiLagMemberPortPtr->bcmx_lport,
                                         dapiCmd->cmdData.doubleVlanTagConfig.etherType,
                                         0);
          }
          ...
          {
             rv = bcmx_port_tpid_delete (hapiLagMemberPortPtr->bcmx_lport,
                              dapiCmd->cmdData.doubleVlanTagConfig.etherType);
  ...
  else if(IS_PORT_TYPE_PHYSICAL(dapiPortPtr) == L7_TRUE)/*Physical Ports */
  {
    if (dapiCmd->cmdData.doubleVlanTagConfig.getOrSet == DAPI_CMD_SET)
    {
      if (dapiCmd->cmdData.doubleVlanTagConfig.enable == L7_TRUE)
      {
        rv = bcmx_port_tpid_add (hapiPortPtr->bcmx_lport,
                                 dapiCmd->cmdData.doubleVlanTagConfig.etherType,
                                 0);
      }
      else
      {
        rv = bcmx_port_tpid_delete (hapiPortPtr->bcmx_lport,
                                    dapiCmd->cmdData.doubleVlanTagConfig.etherType);
      }
    }
  }
                         ...
                         DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    hapiBroadDvlanEthertype = dapiCmd->cmdData.doubleVlanTagConfig.etherType;
    if (dtagPortCount) /* This operation depends on dvlan enabled ports */
    {
    result = usl_bcmx_dvlan_default_tpid_set(
               dapiCmd->cmdData.doubleVlanTagConfig.etherType);
        if (result != L7_SUCCESS)
            return result;
   /* This change, modifies the default TPID of the system */
   }
 }
 return result;
}
