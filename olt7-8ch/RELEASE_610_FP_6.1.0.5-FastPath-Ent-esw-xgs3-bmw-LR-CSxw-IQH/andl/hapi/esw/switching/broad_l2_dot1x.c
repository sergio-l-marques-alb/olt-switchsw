...
{
    return crc32(0L, (L7_uchar8*)pTLV, pTLV->length + sizeof(pTLV->type) + sizeof(pTLV->length));
}
...
L7_RC_t hapiBroadIntfDot1xAuthModeUpdate( DAPI_USP_t *usp, BROAD_PORT_t *hapiPortPtr, DAPI_t *dapi_g )
{
  ...
  {
    rc = l7_bcmx_port_learn_set( hapiPortPtr->bcmx_lport, 0); /* Do not forward or learn */
    if ( L7_BCMX_OK( rc ) != L7_TRUE )
    {
      SYSAPI_PRINTF( SYSAPI_LOGGING_HAPI_ERROR,
...
L7_RC_t hapiBroadDot1xViolationPolicyCreate(DAPI_t *dapi_g)
{
  ...
  {
    result = L7_FAILURE;
    if (hapiBroadPolicyCreate(BROAD_POLICY_TYPE_SYSTEM, &hapiSystem->dot1xViolationPolicyId) == L7_SUCCESS)
    {
      hapiBroadPolicyRuleAdd(hapiSystem->dot1xViolationPolicyId, &ruleId);
      hapiBroadPolicyRuleQualifierAdd(hapiSystem->dot1xViolationPolicyId,
                                      ruleId,
                                      BROAD_FIELD_LOOKUP_STATUS,
                                      (L7_uchar8 *)&lookupStatusUnresolvedSa,
                                      (L7_uchar8 *)&lookupStatusUnresolvedSa);
      hapiBroadPolicyRuleActionAdd(hapiSystem->dot1xViolationPolicyId,
                                   ruleId,
  ...
  if (hapiBroadDot1xVlanAssignmentSupported())
  {
    if (dapiCmd->cmdData.dot1xClientAdd.vlanId != 0)
    {
      ...
      else
      {
          ...
          if (hapiBroadPolicyCreate(BROAD_POLICY_TYPE_PORT, 
                                    &pClient->vlanAssignmentPolicyId) == L7_SUCCESS)
          {
              hapiBroadPolicyStageSet(pClient->vlanAssignmentPolicyId, BROAD_POLICY_STAGE_LOOKUP);
            ...
            /* We need to create two rules here because of how the VFP VLAN action works.
              ...
              hapiBroadPolicyRuleAdd(pClient->vlanAssignmentPolicyId, &ruleId);
              hapiBroadPolicyRuleQualifierAdd(pClient->vlanAssignmentPolicyId, 
                                              ruleId, 
                                              BROAD_FIELD_MACSA, 
                                              dapiCmd->cmdData.dot1xClientAdd.macAddr.addr, 
                                              exact_match);
              hapiBroadPolicyRuleQualifierAdd(pClient->vlanAssignmentPolicyId, 
                                              ...
                                              untaggedPktFormat, 
                                              exact_match);
              hapiBroadPolicyRuleActionAdd(pClient->vlanAssignmentPolicyId,
                                           ruleId,
...
{
    HAPI_BROAD_CUSTOM_BCMX_DOT1X_CLIENT_CMD_t clientCmd;
    L7_int32 rv;

    *timedout = L7_FALSE;
    memset(&clientCmd,0,sizeof(HAPI_BROAD_CUSTOM_BCMX_DOT1X_CLIENT_CMD_t));
    memcpy(clientCmd.mac_addr,macAddr.addr,L7_ENET_MAC_ADDR_LEN);
    clientCmd.vlan_id = vid;
    clientCmd.timedout_flg = L7_FALSE;

   rv= customx_dot1x_client_timeout_get(port,&clientCmd);
...
L7_RC_t hapiBroadIntfDot1xClientBlock(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (customx_dot1x_client_block(hapiPortPtr->bcmx_lport, &clientCmd) == BCM_E_NONE)
  {
    result = L7_SUCCESS;
  }
  else
...
L7_RC_t hapiBroadIntfDot1xClientUnblock(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (customx_dot1x_client_unblock(hapiPortPtr->bcmx_lport, &clientCmd) == BCM_E_NONE)
  {
    result = L7_SUCCESS;
  }
  else
