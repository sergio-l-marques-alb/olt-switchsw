...
L7_RC_t hapiBroadIscsiPolicyRuleCounterAdd(BROAD_POLICY_t policy, 
                                           ...
                                           DAPI_t *dapi_g)
{
  ...
  if ((result = CPU_USP_GET(&dapiUsp)) == L7_SUCCESS)
  {
    ...
    result = hapiBroadPolicyRuleCounterAdd(policy, rule, BROAD_COUNT_PACKETS, hapiPortPtr->bcm_port);
  }

  if (result != L7_SUCCESS)
  {
    if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_NONE) 
      sysapiPrintf("error in hapiBroadIscsiPolicyRuleCounterAdd() - result = %u policy = %u rule = %u \n", result, policy, rule);
  }
  return result;
}
...
L7_RC_t hapiBroadQosIscsiTargetPortAdd(DAPI_USP_t * usp, DAPI_CMD_t cmd, void *data, DAPI_t * dapi_g)
{
  ...
  if (hapiBroadQosIscsiTargetPortPolicyAllocate(&index) != L7_SUCCESS)
  {
    ...
    L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_QOS_ISCSI_COMPONENT_ID, "Failed to allocate target port policy table entry");
    return L7_FAILURE;
  }
  ...
  do
  {
    if ((rc = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_ISCSI, &policyId)) != L7_SUCCESS)
    {
      if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
    ...
    {
      if ((rc = hapiBroadPolicyPriorityRuleAdd(policyId, &ruleId1, BROAD_POLICY_RULE_PRIORITY_HIGHEST)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_DPORT, port, exact_match)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      {
        if ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_DIP, ipAddr, exact_match)) != L7_SUCCESS)
        {
          if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, opcodeField[opcodeFieldIdx], &opcode, &opcode_mask)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if ((rc = hapiBroadPolicyRuleActionAdd(policyId, ruleId1, BROAD_ACTION_SET_COSQ, iscsiCmd->cmdData.iscsiTargetPortAdd.traffic_class, 0, 0)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if ((rc = hapiBroadPolicyRuleActionAdd(policyId, ruleId1, BROAD_ACTION_COPY_TO_CPU, 0, 0, 0)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if (iscsiCmd->cmdData.iscsiTargetPortAdd.taggingEnabled == L7_TRUE)
      {
        ...
        {
          if ((rc = hapiBroadPolicyRuleActionAdd(policyId, ruleId1, action, iscsiCmd->cmdData.iscsiTargetPortAdd.tagData, 0, 0)) != L7_SUCCESS)
          {
            if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      if ((rc = hapiBroadIscsiPolicyRuleCounterAdd(policyId, ruleId1, dapi_g)) != L7_SUCCESS)
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
      ...
      rc = hapiBroadPolicyRuleCopy(policyId, ruleId1, &ruleId2);
      if (L7_SUCCESS == rc)
      {
          /* update new rule with iscsi Logout opcode value */
          opcode = ISCSI_OPCODE_LOGOUT_REQ;
          if ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId2, opcodeField[opcodeFieldIdx], &opcode, &opcode_mask)) != L7_SUCCESS)
          {
            if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
              sysapiPrintf(" - error from hapiBroadPolicyRuleQualifierAdd() for BROAD_FIELD_ISCSI_OPCODE - rc = %u\n", rc);        
            break;
...
L7_RC_t hapiBroadQosIscsiConnectionAdd(DAPI_USP_t * usp, DAPI_CMD_t cmd, void *data, DAPI_t * dapi_g)
{
  ...
  do
  {
    ...
    if ((rc = hapiBroadPolicyPriorityRuleAdd(policyId, &ruleId1, BROAD_POLICY_RULE_PRIORITY_DEFAULT)) != L7_SUCCESS)
    {
      if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
        sysapiPrintf(" - error from hapiBroadPolicyPriorityRuleAdd() - rc = %u\n", rc);        
      break;
    ...
    if (((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_SIP, initiatorIpAddr, exact_match)) != L7_SUCCESS) ||
        ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_DIP, targetIpAddr, exact_match)) != L7_SUCCESS) ||
        ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_SPORT, initiatorPort, exact_match)) != L7_SUCCESS) ||
        ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId1, BROAD_FIELD_DPORT, targetPort, exact_match)) != L7_SUCCESS))
    {
      if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
    ...
    if ((rc = hapiBroadPolicyRuleActionAdd(policyId, ruleId1, BROAD_ACTION_SET_COSQ, iscsiCmd->cmdData.iscsiConnectionAdd.traffic_class, 0, 0)) != L7_SUCCESS)
    {
      if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
        sysapiPrintf(" - error from hapiBroadPolicyRuleActionAdd() for BROAD_ACTION_SET_COSQ - rc = %u, traffic_class = %u\n", 
                     rc, iscsiCmd->cmdData.iscsiTargetPortAdd.traffic_class);        
    ...
    if (iscsiCmd->cmdData.iscsiConnectionAdd.taggingEnabled == L7_TRUE)
    {
      ...
      if (action < BROAD_ACTION_LAST)
      {
        if ((rc = hapiBroadPolicyRuleActionAdd(policyId, ruleId1, action, iscsiCmd->cmdData.iscsiConnectionAdd.tagData, 0, 0)) != L7_SUCCESS)
        {
          if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
            sysapiPrintf(" - error from hapiBroadPolicyRuleActionAdd() - rc = %u, action = %u, tagData = %u\n", 
                         rc, action, iscsiCmd->cmdData.iscsiTargetPortAdd.tagData);        
    ...
    if (L7_SUCCESS == rc)
    {
      ...
      if (((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId2, BROAD_FIELD_DIP, initiatorIpAddr, exact_match)) != L7_SUCCESS) ||
          ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId2, BROAD_FIELD_SIP, targetIpAddr, exact_match)) != L7_SUCCESS) ||
          ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId2, BROAD_FIELD_DPORT, initiatorPort, exact_match)) != L7_SUCCESS) ||
          ((rc = hapiBroadPolicyRuleQualifierAdd(policyId, ruleId2, BROAD_FIELD_SPORT, targetPort, exact_match)) != L7_SUCCESS))
      {
        if (hapiBroadIscsiDebugLevel() > ISCSI_DEBUG_LOW)
...
L7_RC_t hapiBroadQosIscsiCounterGet(DAPI_USP_t * usp, DAPI_CMD_t cmd, void *data, DAPI_t * dapi_g)
{
  ...
  if (hapiBroadQosIscsiConnectionCounterGetFirst(qosCmd->cmdData.iscsiConnectionCounterGet.identifier, &policyId, &index) == L7_SUCCESS)
  {
    do
    {
      rc = hapiBroadPolicyStatsGet(policyId, &numStats, stats);
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        {
          temp.high = temp.high +
                      ((stats[broadIscsiConnectionPolicyTable[index].ruleId[i]].statMode.counter.count >> 32) & 0xfffffff);
          temp.low  = temp.low +
                      ((stats[broadIscsiConnectionPolicyTable[index].ruleId[i]].statMode.counter.count) & 0xfffffff);
        }
      }
...
static void hapiBroadIscsiConnectionTableEntryShow(L7_uint32 i)
{
  ...
  numStats = 2;
  if (hapiBroadPolicyStatsGet(broadIscsiConnectionPolicyTable[i].iscsiPolicy, &numStats, stats) == L7_SUCCESS)
  {
    osapiSnprintf(ruleStatsString, sizeof(ruleStatsString), "%llu", stats[0].statMode.counter.count);
  }
  else
  {
...
static void hapiBroadIscsiTargetPortTableEntryShow(L7_uint32 i)
{
  ...
  numStats = 2;
  if (hapiBroadPolicyStatsGet(broadIscsiTargetPortPolicyTable[i].iscsiPolicy, &numStats, stats) == L7_SUCCESS)
  {
    osapiSnprintf(ruleStatsString, sizeof(ruleStatsString), "%llu", stats[0].statMode.counter.count);
  }
  else
