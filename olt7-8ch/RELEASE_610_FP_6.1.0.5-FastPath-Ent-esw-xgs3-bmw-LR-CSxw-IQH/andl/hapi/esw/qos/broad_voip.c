...
static L7_RC_t hapiBroadQosVoIPApplyPolicyToIface(DAPI_USP_t *usp, DAPI_t *dapi_g, L7_BOOL addDel)
{
    ...
    if (BROAD_PORT_IS_LAG(hapiPortPtr))
    {
        ...
        for (i = 0; i < L7_MAX_MEMBERS_PER_LAG; i++)
        {
            if (L7_TRUE == lagMemberSet[i].inUse)
            {
                ...
                if(hapiBroadPolicyIfaceCheck(voipProfilePolicy, lagMemberPtr->bcmx_lport )== 
    ...
    {
      if(hapiBroadPolicyIfaceCheck(voipProfilePolicy, hapiPortPtr->bcmx_lport )== 
                                                                L7_ALREADY_CONFIGURED)
      {
        return L7_SUCCESS;
      }
...
*
*********************************************************************/
L7_RC_t    hapiBroadVoipPolicyRuleCounterAdd(BROAD_POLICY_t policy, 
    ...
    for (dapiUsp.slot=0;dapiUsp.slot<dapi_g->unit[dapiUsp.unit]->numOfSlots;dapiUsp.slot++)
    {
          ...
          (IS_SLOT_TYPE_PHYSICAL(&dapiUsp, dapi_g)                       == L7_TRUE))
      {
             ...
             dapiUsp.port++)
        {
          ...
          hapiPortPtr = HAPI_PORT_GET(&dapiUsp, dapi_g);
          result = hapiBroadPolicyRuleCounterAdd(policy, rule, BROAD_COUNT_PACKETS, hapiPortPtr->bcmx_lport);
          if (result !=L7_SUCCESS)
          {
             continue;
          }
                                ...
                                DAPI_t *dapi_g)
{
  ...
  if (voipCmd->cmdData.voipSession.val == L7_TRUE) 
  {
     ...
     if (hapiBroadVoipPolicyFind(voipCmd->cmdData.voipSession.srcIpAddr,
                                  ...
                                  &index, &voipId) == L7_SUCCESS) 
     {
       result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_PORT, &voipId);
       if (L7_SUCCESS != result)
       return result;
     
     }
    
     hapiBroadPolicyRuleAdd(voipId, &rule1);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule1, BROAD_FIELD_ETHTYPE,ip_ethtype, exact_match);
     if ((protocol == L7_QOS_VOIP_PROTOCOL_SCCP) || (protocol == L7_QOS_VOIP_PROTOCOL_H323))
        hapiBroadPolicyRuleQualifierAdd(voipId, rule1, BROAD_FIELD_PROTO, tcp_proto, exact_match);
     else
        hapiBroadPolicyRuleQualifierAdd(voipId, rule1, BROAD_FIELD_PROTO, udp_proto, exact_match);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule1, BROAD_FIELD_DIP, dstIpAddr, exact_match);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule1, BROAD_FIELD_DPORT,dstL4port, exact_match);
     hapiBroadPolicyRuleActionAdd(voipId, rule1,BROAD_ACTION_SET_COSQ,FD_VOIP_COS_QUEUE,0,0);
     hapiBroadVoipPolicyRuleCounterAdd(voipId,rule1,dapi_g);  
   
     hapiBroadPolicyRuleAdd(voipId, &rule2);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule2, BROAD_FIELD_ETHTYPE,ip_ethtype, exact_match);
     if ((protocol == L7_QOS_VOIP_PROTOCOL_SCCP) || (protocol == L7_QOS_VOIP_PROTOCOL_H323))
        hapiBroadPolicyRuleQualifierAdd(voipId, rule2, BROAD_FIELD_PROTO, tcp_proto, exact_match);
     else
        hapiBroadPolicyRuleQualifierAdd(voipId, rule2, BROAD_FIELD_PROTO, udp_proto, exact_match);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule2, BROAD_FIELD_SIP, dstIpAddr, exact_match);
     hapiBroadPolicyRuleQualifierAdd(voipId, rule2, BROAD_FIELD_SPORT, dstL4port,exact_match);
     hapiBroadPolicyRuleActionAdd(voipId, rule2,BROAD_ACTION_SET_COSQ, FD_VOIP_COS_QUEUE,0,0);
     hapiBroadVoipPolicyRuleCounterAdd(voipId,rule2,dapi_g);
...
L7_RC_t hapiBroadVoipControlPacketAdd(L7_uint32 protocol,L7_ushort16 port)
{
  ...
  hapiBroadPolicyRuleAdd(voipProfilePolicy, &rule1);
  hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule1, BROAD_FIELD_ETHTYPE,ip_ethtype, exact_match);

  if ((protocol == L7_QOS_VOIP_PROTOCOL_SCCP) || (protocol == L7_QOS_VOIP_PROTOCOL_H323))
    hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule1, BROAD_FIELD_PROTO, tcp_proto, exact_match);
  else
    hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule1, BROAD_FIELD_PROTO, udp_proto, exact_match);
  
  hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule1, BROAD_FIELD_SPORT, sport, exact_match);
  hapiBroadPolicyRuleActionAdd(voipProfilePolicy, rule1,BROAD_ACTION_COPY_TO_CPU,0,0,0);
  hapiBroadPolicyRuleActionAdd(voipProfilePolicy, rule1,BROAD_ACTION_SET_COSQ, FD_VOIP_COS_QUEUE,0,0);


  hapiBroadPolicyRuleAdd(voipProfilePolicy, &rule2);
  hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule2, BROAD_FIELD_ETHTYPE,ip_ethtype, exact_match);

   if ((protocol == L7_QOS_VOIP_PROTOCOL_SCCP) || (protocol == L7_QOS_VOIP_PROTOCOL_H323))
    hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule2, BROAD_FIELD_PROTO, tcp_proto, exact_match);
  else
    hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule2, BROAD_FIELD_PROTO, udp_proto, exact_match);

  hapiBroadPolicyRuleQualifierAdd(voipProfilePolicy, rule2, BROAD_FIELD_DPORT, sport, exact_match);
  hapiBroadPolicyRuleActionAdd(voipProfilePolicy, rule2,BROAD_ACTION_COPY_TO_CPU,0,0,0);
  hapiBroadPolicyRuleActionAdd(voipProfilePolicy, rule2,BROAD_ACTION_SET_COSQ, FD_VOIP_COS_QUEUE,0,0);
                                ...
                                DAPI_t *dapi_g)
{
  ...
  {  
    ...
    {
      result = hapiBroadPolicyCreate(BROAD_POLICY_TYPE_PORT, &voipProfilePolicy);
      if (L7_SUCCESS != result)
        return result;
                                 ...
                                 DAPI_t *dapi_g)
{
  ...
  if (hapiBroadVoipPolicyFind( voipCmd->cmdData.voipStats.srcIpAddr,
                               ...
                               &index, &voipId) == L7_ALREADY_CONFIGURED)
  {
    result = hapiBroadPolicyStatsGet(voipId, &numStats, stats);
    if (voipCmd->cmdData.voipStats.pHitcount != L7_NULLPTR)
    {
      /* accumulate stats of two rules */
      temp.high = ((stats[broadVoipTable[index].ruleId[0]].statMode.counter.count >>32) & 0xfffffff) +
                   ((stats[broadVoipTable[index].ruleId[1]].statMode.counter.count >>32) & 0xfffffff);
      temp.low  = ((stats[broadVoipTable[index].ruleId[0]].statMode.counter.count) & 0xfffffff) +
                   ((stats[broadVoipTable[index].ruleId[1]].statMode.counter.count) & 0xfffffff);
