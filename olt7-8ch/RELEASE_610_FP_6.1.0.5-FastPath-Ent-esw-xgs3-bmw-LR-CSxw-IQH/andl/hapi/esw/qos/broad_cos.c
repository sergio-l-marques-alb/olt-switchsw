...
static void hapiBroadQosCosResetDscpMapping(bcmx_lport_t port)
{
    ...
    {
        /* 5695: Remove dscp mappings for specified port */
        rv = bcmx_port_dscp_map_set(port, RESET_CP, RESET_CP, 0);
    } else
    {
...
static L7_RC_t hapiBroadQosCosIntfUntrusted(BROAD_PORT_t *hapiPortPtr, L7_uchar8 defTc, L7_BOOL l2Only)
{
    ...
    if (hapiBroadPolicyUtilLookup((L7_uchar8*)&ruleComposite, sizeof(ruleComposite), &cosqId) == L7_TRUE)
    {
    ...
    if (L7_FALSE == policyFound)   /* create new policy */
    {
        ...
        hapiBroadPolicyRuleActionAdd(cosqId, ruleId, BROAD_ACTION_SET_COSQ, defTc, 0, 0);
        ...
        { 
          ...
          hapiBroadPolicyRuleQualifierAdd(cosqId, ruleId, BROAD_FIELD_IP_TYPE, nonip, exact_match);
        }

        result = hapiBroadPolicyCommit(cosqId);

        if (L7_SUCCESS == result)
            result = hapiBroadPolicyUtilAdd((L7_uchar8*)&ruleComposite, sizeof(ruleComposite), cosqId);
...
static L7_RC_t hapiBroadQosCosRemoveIpPrec(BROAD_PORT_t *hapiPortPtr)
{
    ...
    {
        hapiBroadPolicyUtilRemove(qosPortPtr->cos.precId, hapiPortPtr->bcmx_lport);
    ...
    {
        hapiBroadPolicyUtilRemove(qosPortPtr->cos.precDefaultPolicyId, hapiPortPtr->bcmx_lport);
...
static L7_RC_t hapiBroadQosCosIntfTrustIpPrec(BROAD_PORT_t *hapiPortPtr, L7_uchar8 *precMap, L7_uchar8 defTc)
{
    ...
    if (hapiBroadPolicyUtilLookup(tmpPrecMap, L7_QOS_COS_MAP_NUM_IPPREC+1, &precId) == L7_TRUE)
    {
    ...
    if (L7_FALSE == policyFound)   /* create new policy */
    {
        ...
        hapiBroadPolicyCreate(BROAD_POLICY_TYPE_COSQ, &precId);

        for (i = 0; i < L7_QOS_COS_MAP_NUM_IPPREC; i++)
        {
            L7_uchar8 ipPrec = BROAD_IPPREC_TO_TOS(i);  /* cast to uchar8 and convert to tos */

            hapiBroadPolicyPriorityRuleAdd(precId, &ruleId, BROAD_POLICY_RULE_PRIORITY_LOWEST);
            hapiBroadPolicyRuleQualifierAdd(precId, ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ipv4, exactMask);
            hapiBroadPolicyRuleQualifierAdd(precId, ruleId, BROAD_FIELD_DSCP, &ipPrec, precMask);
            hapiBroadPolicyRuleActionAdd(precId, ruleId, BROAD_ACTION_SET_COSQ, precMap[i], 0, 0);
        }

        result = hapiBroadPolicyCommit(precId);

        if (L7_SUCCESS == result)
            result = hapiBroadPolicyUtilAdd(tmpPrecMap, L7_QOS_COS_MAP_NUM_IPPREC+1, precId);
    }

    if (L7_SUCCESS == result)
        result = hapiBroadPolicyUtilApply(precId, hapiPortPtr->bcmx_lport);
    ...
    if (L7_SUCCESS == result)
    {
        ...
        if (hapiBroadPolicyUtilLookup(defaultPolicyKey, sizeof(defaultPolicyKey), &defaultPolicyId) == FALSE)
        {
          hapiBroadPolicyCreate(BROAD_POLICY_TYPE_COSQ, &defaultPolicyId);
          ...
          * override mappings added at a later time */
          hapiBroadPolicyPriorityRuleAdd(defaultPolicyId, &ruleId, BROAD_POLICY_RULE_PRIORITY_LOWEST);
          hapiBroadPolicyRuleActionAdd(defaultPolicyId, ruleId, BROAD_ACTION_SET_COSQ, defTc, 0, 0);

          result = hapiBroadPolicyCommit(defaultPolicyId);

          if (L7_SUCCESS == result)
              result = hapiBroadPolicyUtilAdd(defaultPolicyKey, sizeof(defaultPolicyKey), defaultPolicyId);
        }

        if (L7_SUCCESS == result)
            result = hapiBroadPolicyUtilApply(defaultPolicyId, hapiPortPtr->bcmx_lport);
...
static L7_RC_t hapiBroadQosCosIntfRateShape(BROAD_PORT_t *dstPortPtr, L7_uint32 egrRate)
{
    ...
    if (L7_QOS_COS_INTF_SHAPING_RATE_UNITS == L7_RATE_UNIT_PERCENT)
    {
      ...
      kbits_sec = ((egrRate * portSpeed) / 100); 
    }
    else
    {
    ...
    kbits_burst = kbits_sec / 50; 
     
    rv = l7_bcmx_port_rate_egress_set(dstPortPtr->bcmx_lport, kbits_sec, kbits_burst);
    if (L7_BCMX_OK(rv) != L7_TRUE)
      result = L7_FAILURE;
...
static L7_RC_t hapiBroadQosCosEgressBwConfig(BROAD_PORT_t *dstPortPtr, HAPI_BROAD_COS_PORT_t *cosData)
{
  ...
  rv = l7_bcmx_cosq_port_sched_set(dstPortPtr->bcmx_lport, schedulerMode, weights, 0, minKbps, maxKbps);
  if (L7_BCMX_OK(rv) == L7_FALSE)
  {
    result = L7_FAILURE;
  }
...
static L7_RC_t hapiBroadQosCosQueueWeightsConfig(BROAD_PORT_t *dstPortPtr, HAPI_BROAD_COS_PORT_t *cosData)
{
  ...
  L7_uint32     schedulerMode;
  L7_uint32     portSpeed;              // PTin added
...
/* scale weights so they fit within valid range */
  ...
  if (L7_SUCCESS == result)
  {
    if (hapiBroadRoboVariantCheck() == __BROADCOM_53115_ID)
    {
      ...
      {
        rv = l7_bcmx_cosq_port_sched_set(dstPortPtr->bcmx_lport,BCM_COSQ_STRICT, weights, NO_DELAY, minKbps, maxKbps);
        if (L7_BCMX_OK(rv) == L7_FALSE)
        {
          result = L7_FAILURE;
        }
      ...
      {
        /* apply the scheduling policy to the port */
        rv = l7_bcmx_cosq_port_sched_set(dstPortPtr->bcmx_lport, BCM_COSQ_WEIGHTED_ROUND_ROBIN,
                                         weights, NO_DELAY, minKbps, maxKbps);
        if (L7_BCMX_OK(rv) == L7_FALSE)
        {
          result = L7_FAILURE;
        }
    ...
    else
    {
      /* apply the scheduling policy to the port */
      schedulerMode = hapiBroadCosQueueWDRRSupported() ? BCM_COSQ_DEFICIT_ROUND_ROBIN : BCM_COSQ_WEIGHTED_ROUND_ROBIN;
      rv = l7_bcmx_cosq_port_sched_set(dstPortPtr->bcmx_lport, schedulerMode, weights, NO_DELAY, minKbps, maxKbps);
      if (L7_BCMX_OK(rv) == L7_FALSE)
      {
        result = L7_FAILURE;
      }
...
static L7_RC_t hapiBroadQosCosApplyPolicy(BROAD_PORT_t *dstPortPtr, BROAD_PORT_t *srcPortPtr)
{
    ...
    case DAPI_QOS_COS_INTF_MODE_TRUST_IPDSCP:
        ...
        /* Use the port specific dscp mapping table for the 5695
         * and the global mapping table for other devices. Reset
         * the dirty bit upon success */
         hapiBroadGetSystemBoardFamily(&board_family);
         if (board_family == BCM_FAMILY_DRACO)
         {
            ...
            if (result == L7_SUCCESS)
               qosPortPtr->cos.dscpMapDirty = L7_FALSE;
         } else
         {
          result = hapiBroadQosCosIntfTrustIpDscp(dstPortPtr,
                                                  qos->dscpMap,
...
L7_RC_t hapiBroadQosCosIpDscpToTcMap(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    if (board_family == BCM_FAMILY_DRACO)
    {
      ...
      if (DAPI_QOS_COS_INTF_MODE_TRUST_IPDSCP == qosPortPtr->cos.trustMode)
        result = hapiBroadQosCosApplyPolicyToIface(usp, dapi_g);
    }
    else
    {
...
L7_RC_t hapiBroadQosCosIntfTrustModeConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
    ...
    case DAPI_QOS_COS_INTF_MODE_TRUST_IPDSCP:
        ...
        if(board_family == BCM_FAMILY_DRACO)
        {
            /* 5695: Use port specific mapping table */
            if(memcmp(pMapTable->ip_dscp_traffic_class, qosPortPtr->cos.dscpMap, sizeof(qosPortPtr->cos.dscpMap)))
            {
                for (i = 0; i < L7_QOS_COS_MAP_NUM_IPDSCP; i++)
                    qosPortPtr->cos.dscpMap[i] = pMapTable->ip_dscp_traffic_class[i];
...
static L7_RC_t hapiBroadQosCosWredApply(DAPI_USP_t *usp)
{
    ...
    DAPI_PORT_t                    *dapiPortPtr;
    HAPI_BROAD_QOS_PORT_t          *qosPortPtr;
    HAPI_BROAD_CUSTOM_BCMX_PORT_WRED_SET_t      parms;
    int                            colorIndex, rv;
    L7_uint32                      cosIndex;
    DAPI_USP_t                     *lagUsp;
    ...
    rv = customx_port_wred_set(parms.bcm_gport, &parms);
    if (!L7_BCMX_OK(rv)) 
    {
        return L7_FAILURE;
    }
