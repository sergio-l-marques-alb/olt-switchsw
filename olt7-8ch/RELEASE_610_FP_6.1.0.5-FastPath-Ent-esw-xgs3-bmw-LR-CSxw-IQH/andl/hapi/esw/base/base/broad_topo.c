...
int lvl7_topo_board_program(cpudb_ref_t db_ref, topo_cpu_t *topo_cpu,int *rv)
{
  ...
  if (customer_topo_board_program != L7_NULLPTR)
  {
    ...
    dapiTraceGeneric("customer_topo_board_program rv=%d handled=%d",*rv,handled);
  }
  ...
  /* Chassis LM board is programed below using board id */
    ...
    *rv = bcm_board_topo_sdk_xgs3_24g(topo_cpu, db_ref);
    dapiTraceGeneric("bcm_board_topo_sdk_xgs3_24g rv=%d",*rv);
    handled = 1;
#ifdef INCLUDE_RCPU
  } else if ( soc_ndev == 2 && 
              SOC_IS_RAPTOR(0) && 
    ...
    *rv = lvl7_bcm_board_topo_sdk_dual_cpu(topo_cpu, db_ref);
    dapiTraceGeneric("lvl7_bcm_board_topo_sdk_dual_cpu rv=%d",*rv);
    handled = 1;
  } else if ( soc_ndev == 2 && 
              SOC_IS_RAVEN(0) && 
              SOC_IS_RAVEN(1)) {
    ...
    *rv = bcm_board_topo_xgs3_20x(topo_cpu, db_ref);
    dapiTraceGeneric("bcm_board_topo_xgs3_20x rv=%d",*rv);
    handled = 1;    
    ...
    *rv = lvl7_bcm_board_topo_sdk_xgs3_48g(topo_cpu, db_ref);
    dapiTraceGeneric("lvl7_bcm_board_topo_sdk_xgs3_48g rv=%d",*rv);
    handled = 1;
  }
    ...
    switch (db_ref->local_entry->base.board_id) {
      case cpudb_board_id_lm_56800_12x:
		*rv = bcm_board_topo_lm_56800_12x(topo_cpu, db_ref);
        dapiTraceGeneric("bcm_board_topo_lm_56800_12x rv=%d",*rv);
        handled = 1;
        break;
      default:
		handled = 0;
