                        ...
                        void (*msg_recv_notify)(L7_uint32 receive_id, L7_uchar8* buffer, L7_uint32 msg_len))
{
  ...
  */
#ifndef L7_ROBO_SUPPORT
  {
     ...
     cpudb = hpcBroadLocalCpudbCreate ();
     topology_mod_ids_assign(cpudb);
     topo_board_setup (cpudb);
  }
#endif
...
/* Added this for non-stackable, it was lifted from hpcBroadTransportInit */
...
#endif
  {
    ...
    hpcLocalUnitIdentifierMacGet((L7_enetMacAddr_t *)&(cpu_key.key));
  ...
  usl_policy_init();    /* policy manager init */
