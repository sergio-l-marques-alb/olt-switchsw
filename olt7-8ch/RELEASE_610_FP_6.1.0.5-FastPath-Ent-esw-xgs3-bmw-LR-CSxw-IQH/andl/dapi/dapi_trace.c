...
void dapiTraceInit(L7_uint32 numEntries,L7_uint32 width)
{
  ...
  {
    LOG_MSG("DAPI: unable to create the trace buffer\n");
    dapiTraceHandle = ~0;
  }
  else
  {
    dapiTraceMaskSet(DAPI_TRACE_GENERIC_MASK);
...
L7_RC_t dapiTraceFini(void)
{
  ...
  if (dapiTraceHandle != ~0)
  {
    ...
    {
      LOG_MSG("DAPI: unable to delete the trace buffer\n");
      rc = L7_FAILURE;
    }
    else
    {
...
L7_char8 *dapiCommandToStringGet(DAPI_CMD_t cmd)
{
  ...
  switch (cmd)
  {
    ...
    case DAPI_CMD_SYSTEM_DHCP_CONFIG:
      str = "DAPI_CMD_SYSTEM_DHCP_CONFIG";
      break;
  ...
  case DAPI_CMD_INTF_DOT1X_CLIENT_TIMEOUT:
    ...
    case DAPI_CMD_LAG_PENDING_CONFIGS:
      str = "DAPI_CMD_LAG_PENDING_CONFIGS";
      break;
    // PTin end
...
void dapiTraceModeApply(L7_BOOL enable)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG("dapiTraceModeApply: Unable to apply dapi trace mode (%u) handle=0x%8.8x\n",
