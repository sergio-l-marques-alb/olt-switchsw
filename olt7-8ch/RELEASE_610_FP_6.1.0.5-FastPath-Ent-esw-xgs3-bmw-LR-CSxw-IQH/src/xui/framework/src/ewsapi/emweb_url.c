...
static char *ewaURLTransferHook(EwsContext context, char *url)
{
  ...
  {
    /* if the http download is in progress but not started by currrent context, then redirect to status page */
    if(ewaFileDownloadGetProgressGlobal() && ewaFileDownloadGetProgress(context) == FALSE)
    	return emwebTransferProgressCheckDefaultUrl;  	  
  	return url;
  }

  if(emwebTransferProgressCheckCallback == NULL) return url;
  ...
  /* download is started by current context, serve the url to finish the download */ 
  return url;
}
