...
static xLibU32_t getUpperLimit (xLibU32_t intArray[], xLibU32_t noElems, xLibU32_t qint)
{
  xLibU32_t index, temp;
  for (index = 0; index < noElems; index++)
  {
    if (intArray[index] == -32767)
    {
      continue;
    }
    if (intArray[index] == qint)
    {
      intArray[index] = -32767;
    }
    else if (intArray[index] - qint == 1)
    {
      temp = intArray[index];
      intArray[index] = -32767;
      return getUpperLimit (intArray, noElems, temp);
    }
  }
  return qint;
}
