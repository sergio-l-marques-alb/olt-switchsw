...
xLibRC_t fpObjGet_baselogCfg_LogInMemoryMsgIndex (void *wap, void *bufp)
{
  ...
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) & nextObjLogInMemoryMsgIndexValue,
                           sizeof (objLogInMemoryMsgIndexValue));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
}
...
xLibRC_t fpObjGet_baselogCfg_LogInMemoryMsgText (void *wap, void *bufp)
{
  ...
  xLibU32_t keyLogInMemoryMsgIndexNextValue=0;
  ...
  if (kwa.rc != XLIBRC_SUCCESS)
  {
    ...
    keyLogInMemoryMsgIndexValue = 0;

    if(usmDbLogMsgInMemoryGet (L7_UNIT_CURRENT, keyLogInMemoryMsgIndexValue,
                             objLogInMemoryMsgTextValue )!=L7_SUCCESS) 
    {
  ...
  /* get the value from application */
    ...
    FPOBJ_TRACE_CURRENT_KEY (bufp, &keyLogInMemoryMsgIndexValue, kwa.len);
    keyLogInMemoryMsgIndexNextValue = keyLogInMemoryMsgIndexValue;
    owa.l7rc = 

     usmDbLogMsgInMemoryGetNext( L7_UNIT_CURRENT, keyLogInMemoryMsgIndexValue,
            objLogInMemoryMsgTextValue,&keyLogInMemoryMsgIndexNextValue);
