...
xLibRC_t fpObjSet_routingConfigRipRedistribute_Metric (void *wap, void *bufp)
{
    ...
    /* set the value in application */
    owa.l7rc =
        usmDbRip2RouteRedistributeMetricSet (L7_UNIT_CURRENT, keySourceValue,
        ...
        void *bufp)
{
    ...
    /* set the value in application */
    owa.l7rc =
        usmDbRip2RouteRedistributeMatchTypeSet (L7_UNIT_CURRENT, keySourceValue,
                objMatchInternalValue, L7_OSPF_METRIC_TYPE_INTERNAL);
    if (owa.l7rc != L7_SUCCESS)
    {
        owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
...
xLibRC_t fpObjSet_routingConfigRipRedistribute_DistList (void *wap, void *bufp)
{
    ...
    /* set the value in application */
    owa.l7rc =
        usmDbRip2RouteRedistributeFilterSet (L7_UNIT_CURRENT, keySourceValue, L7_ENABLE,
                objDistListValue);
    if (owa.l7rc != L7_SUCCESS)
    {
        owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
...
xLibRC_t fpObjGet_routingConfigRipRedistribute_Match (void *wap, void *bufp)
{
  ...
  if (keySourceValue == XUI_REDIST_RT_OSPF)
  {
    ...
    {
      osapiStrncat(objMatchValue, "Internal <br>", sizeof(objMatchValue));
    }

    if (objMatchType & L7_OSPF_METRIC_TYPE_EXT1)
    {
      osapiStrncat(objMatchValue, "External Type 1 <br>", sizeof(objMatchValue));
    }

    if (objMatchType & L7_OSPF_METRIC_TYPE_EXT2)
    {
      osapiStrncat(objMatchValue, "External Type 2 <br>", sizeof(objMatchValue));
    }

    if (objMatchType & L7_OSPF_METRIC_TYPE_NSSA_EXT1)
    {
      osapiStrncat(objMatchValue, "NSSA External Type 1 <br>", sizeof(objMatchValue));
    }

    if (objMatchType & L7_OSPF_METRIC_TYPE_NSSA_EXT2)
    {
      osapiStrncat(objMatchValue, "NSSA External Type 2", sizeof(objMatchValue));
    }
  }
  else 
  {
