...
L7_int32    captivePortalStackSize(void)
{
  return L7_DEFAULT_STACK_SIZE * 2 ; /* Oh this is so cheap.. how could you? */
}
...
void captivePortalClientAuthHandle(void)
{
  ...
  {
    LOG_MSG("captivePortalClientAuthHandle: CP message queue not created.\n");
    return;
  }

  memset(&msg, 0, sizeof(captivePortalTaskMsg));
  if (osapiMessageSend(pCaptivePortalMsgQueue, &msg, sizeof(captivePortalTaskMsg),
                       L7_NO_WAIT, L7_MSG_PRIORITY_HI) != L7_SUCCESS)
  {
    LOG_MSG("captivePortalClientAuthHandle: osapiMessageSend failed.\n");
  }
...
captive_portal_dolisten(void)
{
  ...
  while (0 == captive_portal_exitflag)
  {
    ...
    do 
    {
      if ((msg_rx_rc == L7_SUCCESS) && (msg.msgID == CP_CLUSTER_MSG_ID_STATS_UPDATE))
      {
        if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
        {
          if ((cpdmGlobalModeGet(&cpMode) == L7_SUCCESS) && (cpMode == L7_CP_MODE_ENABLED))
          {
            cpClusterStatsUpdateMsgProcess(msg.type.statsMsg.buf, msg.type.statsMsg.msgLen);
          }
        }
      }
      ...
      {
        LOG_MSG("%serror with ntwk intf capabilities\n", label);
        break;
      }
      ...
      if (L7_ENABLE == mode)
      {
        ...
        else
        {
            ...
            if (nimGetIntfAddress(servIntIfNum, L7_NULL, intfMac) != L7_SUCCESS)
            {
                break;
            }
#endif
          ...
          (L7_CP_MODE_ENABLED == mode))
      {
		...
		default:
	      ...
	      {
		LOG_MSG("%s: Transaction type %d either faulty, or queue corrupted!",
			__FUNCTION__, trType);
		continue;
	      }
		...
		case CP_CONN_NEW:
		  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
		  {
		    ...
		    {
		      LOG_MSG("%s: Can't send connected client %02x:%02x:%02x:%02x:%02x:%02x update msg",
		  ...
		  if (peerSwTree.count > 0)
		  {
		    ...
		    {
		      LOG_MSG("%s: Can't send client %02x:%02x:%02x:%02x:%02x:%02x notif msg",
		...
		case CP_CONN_DELETE:
		  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
		  {
		    ...
		    {
		      LOG_MSG("%s: Can't send client %02x:%02x:%02x:%02x:%02x:%02x delete msg",
			      __FUNCTION__, client.addr[0], client.addr[1], client.addr[2], 
			      client.addr[3], client.addr[4], client.addr[5]);
		    }
		  }
		  ...
		  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
		  {
		    ...
		    {
		      LOG_MSG("%s: Can't get peer client switch", __FUNCTION__);
		    }
		    else if (cpClusterSwitchClientEntryDelete(client, swMac) != L7_SUCCESS)
		    {
		      LOG_MSG("%s: Can't delete peer client internally",  __FUNCTION__);
		    }
		  }
		...
		case CP_CONN_DEAUTH:
		  while (L7_SUCCESS == cpdmClientConnStatusDeauthGet(&swMac, &client))
		  {
		    if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
		    {
		      ...
		      {
			LOG_MSG("%s: Can't send deauth client contrllr cmd.", __FUNCTION__);
		      }
		    }
		  }
		  break;
        ...
        if (clusterSupportGet() == L7_SUCCESS)
        {
          if (cpClusterFlagGet(&flag) == L7_SUCCESS)
          {
            if (flag & CP_CLUSTER_PEER_JOINED)
            {
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to send client notification msg to peer %02x:%02x:%02x:%02x:%02x:%02x.\n",
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear peer joined flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_CONTROLLER_ELECTED)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
              {
                ...
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send connected clients update msg to controller.\n");
                }
                ...
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send instance interface update msg.\n");
                }
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear controller elected flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_INST_STATUS_SEND)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
              {
                ...
                if (cpClusterInstIntfUpdateMsgSend(cpdmOprData->cpLocalSw.cpId, 0,
                                                   cpdmOprData->cpCtrlMacAddr) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send cpId %d instance update.\n",
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear instance status send flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_INST_INTF_STATUS_SEND)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
              {
                ...
                if (cpClusterInstIntfUpdateMsgSend(cpdmOprData->cpLocalSw.cpId, cpdmOprData->cpLocalSw.intf,
                                                   cpdmOprData->cpCtrlMacAddr) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send interface %d update.\n",
                          cpdmOprData->cpLocalSw.intf);
                }
              }
              cpdmOprData->cpLocalSw.cpId = 0;
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear instance interface status send flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_DEAUTH_INST_CLIENTS_SEND)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
              {
                ...
                if (cpClusterControllerCmdMsgSend(macAddr, macAddr, cpdmOprData->cpLocalSw.cpId,
                                                  0, L7_FALSE) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send deauth instance %d clients controller command.\n",
                          cpdmOprData->cpLocalSw.cpId);
                }
              }
              cpdmOprData->cpLocalSw.cpId = 0;
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear deauth instance clients send flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_DEAUTH_ALL_CLIENTS_SEND)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
              {
                ...
                {
                  LOG_MSG("%s: Failed to send deauth all clients controller command.", __FUNCTION__);
                }

                if (cpdmClientConnStatusPeerPurge() != L7_SUCCESS)
                {
                  LOG_MSG("%s: Failed to purge internal map of all clients", __FUNCTION__);
                }
                
              }
              if (cpClusterFlagClear(CP_CLUSTER_DEAUTH_ALL_CLIENTS_SEND) != L7_SUCCESS)
              {
                LOG_MSG("%s: Failed to clear deauth all clients send flag.", __FUNCTION__);
              }
            }
            ...
            if (flag & CP_CLUSTER_INST_BLOCK_STATUS_SEND)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
              {
                ...
                if (cpClusterControllerCmdMsgSend(macAddr, macAddr, 0, cpdmOprData->cpLocalSw.cpId,
                                                  L7_TRUE) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send instance %d block status controller command.\n",
                          cpdmOprData->cpLocalSw.cpId);
                }
              }
              cpdmOprData->cpLocalSw.cpId = 0;
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear instance block status send flag.\n");
              }
            }
            ...
            if (flag & CP_CLUSTER_CLIENT_AUTH_REPLY_SEND)
            {
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to send client auth reply msg.\n");
              }
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear client auth reply send flag.\n");
              }
            }
          }
          ...
          if (cpClusterResendFlagGet(&resendFlag) == L7_SUCCESS)
          {
            if (resendFlag & CP_CLUSTER_RESEND_REQ_CLIENT_NOTIFY)
            {
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to send client notification resend request msg.\n");
              }
              if (cpClusterResendFlagClear(CP_CLUSTER_RESEND_REQ_CLIENT_NOTIFY) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear client notification resend request flag.\n");
              }
            }
            ...
            if (resendFlag & CP_CLUSTER_RESEND_REQ_CONN_CLIENTS)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
              {
                ...
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send connected clients resend request msg.\n");
                }
              }
              if (cpClusterResendFlagClear(CP_CLUSTER_RESEND_REQ_CONN_CLIENTS) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear connected clients resend request flag.\n");
              }
            }
            ...
            if (resendFlag & CP_CLUSTER_RESEND_REQ_INST_INTF)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
              {
                ...
                {
                  LOG_MSG("captive_portal_dolisten: Failed to send instance interface resend request msg.\n");
                }
              }
              if (cpClusterResendFlagClear(CP_CLUSTER_RESEND_REQ_INST_INTF) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear instance interface resend request flag.\n");
              }
            }
            ...
            if (resendFlag & CP_CLUSTER_RESEND_CLIENT_NOTIFY)
            {
              if (cpClusterClientNotificationMsgPeerSend(cpdmOprData->cpNewPeer) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to resend client notification msg to peer %02x:%02x:%02x:%02x:%02x:%02x.\n",
                        cpdmOprData->cpNewPeer.addr[0], cpdmOprData->cpNewPeer.addr[1], cpdmOprData->cpNewPeer.addr[2],
                        cpdmOprData->cpNewPeer.addr[3], cpdmOprData->cpNewPeer.addr[4], cpdmOprData->cpNewPeer.addr[5]);
              }
              memset(&cpdmOprData->cpNewPeer, 0, sizeof(L7_enetMacAddr_t));
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear client notification resend flag.\n");
              }
            }
            ...
            if (resendFlag & CP_CLUSTER_RESEND_CONN_CLIENTS)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
              {
                ...
                if (cpClusterConnClientsUpdateMsgSend(macAddr, L7_TRUE,
                                                      cpdmOprData->cpCtrlMacAddr) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to resend connected clients update msg.\n");
                }
              }
              if (cpClusterResendFlagClear(CP_CLUSTER_RESEND_CONN_CLIENTS) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear connected clients resend flag.\n");
              }
            }
            ...
            if (resendFlag & CP_CLUSTER_RESEND_INST_INTF)
            {
              if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
              {
                ...
                if (cpClusterInstIntfUpdateMsgSend(cpId, 0, cpdmOprData->cpCtrlMacAddr) != L7_SUCCESS)
                {
                  LOG_MSG("captive_portal_dolisten: Failed to resend instance interface update msg.\n");
                }
              }
              if (cpClusterResendFlagClear(CP_CLUSTER_RESEND_INST_INTF) != L7_SUCCESS)
              {
                LOG_MSG("captive_portal_dolisten: Failed to clear instance interface resend flag.\n");
              }
            }
          }
          ...
          if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
          {
            ...
            if ((end_time - start_time) >= cpdmCfgData->cpGlobalData.peerSwitchStatsReportInterval)
            {
              ...
              {
                LOG_MSG("captive_portal_dolisten: Failed to send stats update msg.\n");
              }
              start_time = osapiUpTimeRaw();
            }
          }
      ...
      if (clusterSupportGet() == L7_SUCCESS)
      {
        if (cpClusterFlagGet(&flag) == L7_SUCCESS)
        {
          if (flag & CP_CLUSTER_CONFIG_SEND)
          {
            ...
            {
              LOG_MSG("captive_portal_dolisten: Failed to send config sync msg.\n");
            }
            memset(&cpdmOprData->cpCfgPeerTx, 0, sizeof(L7_enetMacAddr_t));
            if (cpClusterFlagClear(CP_CLUSTER_CONFIG_SEND) != L7_SUCCESS)
            {
              LOG_MSG("captive_portal_dolisten: Failed to clear CP config send flag.\n");
            }
          }
          ...
          if (flag & CP_CLUSTER_CONFIG_RX)
          {
            ...
            {
              LOG_MSG("captive_portal_dolisten: Failed to clear CP config received flag.\n");
            }
          }
        }
      }
...
captive_portal_main(int argc, char *argv[])
{
  ...
  LOG_MSG("%s: task 0x%08x exiting (rc=%d)\n", __FUNCTION__, my_pid, rc);
...

{
  ...
  do
  {
    ...
    {
      LOG_MSG("%s: wait for task init failed.\n", __FUNCTION__); 
      break;
    }
    rc = L7_SUCCESS;
  } while ( 0 );
...
captivePortalTaskWake(void)
{
  ...
  return  osapiMessageSend(pCaptivePortalMsgQueue, &msg, sizeof(msg),
