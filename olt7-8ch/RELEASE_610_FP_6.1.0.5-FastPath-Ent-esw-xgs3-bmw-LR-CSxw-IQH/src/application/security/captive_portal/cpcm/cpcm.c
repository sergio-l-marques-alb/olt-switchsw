...
L7_RC_t cpcmTryAuth(void)
{
  ...
  while (L7_TRUE == fAuthBusy)
  {
        ...
        authsAttempted++) 
    {
      ...
      {
        LOG_MSG("%s: Client IP 0x%08x no more connections: RESOURCES..",__FUNCTION__,ipAddr);
      ...
      {
        LOG_MSG("%s: Can't get verify mode from interface ID (%d)",__FUNCTION__, desc.intfId);
        continue;
      }
      ...
      if (CP_VERIFY_MODE_LOCAL==verifyMode)
      {
        ...
        {
          LOG_MSG("%s: Client IP 0x%08x auth failed: Not a member of group id %d",
      ...
      if (CP_WIP==desc.flag)
      {
        switch (verifyMode)
        {
          ...
          case CP_VERIFY_MODE_RADIUS:
          {
              ...
              {
                  LOG_MSG("%s: unable to post authentication to RADIUS", __FUNCTION__);
          ...
          default: 
          {
              LOG_MSG("%s: unknown verify mode (%d)", verifyMode, __FUNCTION__);
          ...
          */
              ...
              (L7_SUCCESS==cpdmClientConnStatusAdd(&desc.macAddr,desc.uid,desc.intfId,ipAddr)))
          {
                       ...
                       sizeof(L7_enetMacAddr_t)) == 0)
            {
              ...
              {
                LOG_MSG("%s: Failed to update status of client %02x:%02x:%02x:%02x:%02x:%02x.",
          ...
          {
            LOG_MSG("%s: Client IP 0x%08x failed to add entry to client connection table.",
                    __FUNCTION__,ipAddr);
          }
        }
        else
        {
          LOG_MSG("%s: unable to send cpAuthenUnauthenClient message", __FUNCTION__);
        }
        continue;
      }
      else
