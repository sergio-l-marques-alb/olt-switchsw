...
L7_RC_t dot1xAdvancedGuestPortsCfgSet(L7_uint32 intIfNum,L7_uint32 guestVlanId)
{
  ...
  if(L7_SUCCESS != (dot1qVlanCheckValid(guestVlanId)))
     return L7_FAILURE;
...
L7_RC_t dot1xPortUnauthenticatedVlanSet(L7_uint32 intIfNum, L7_uint32 vlanId)
{
  ...
  /* is this check necessary? how can we support pre configuration with this check?*/
  if (vlanId != FD_DOT1X_PORT_UNAUTHENTICATED_VLAN)
  {
     if(L7_SUCCESS != (dot1qVlanCheckValid(vlanId)))
        return L7_FAILURE;
  }
     ...
     if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
      ...
      else if (pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_SUPP_CAPABLE)
      {
        if ((dot1xSupplicantPortSpmStatusGet(intIfNum, &suppStatus) == L7_SUCCESS) &&
         (suppStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED))
        {
          return L7_SUCCESS;
        }
      }
...
L7_RC_t dot1xPortMABEnabledSet(L7_uint32 intIfNum, L7_uint32 mabEnabled)
{
  ...
  /*check to see if the port control mode is mac-based*/
  if ((pCfg->portControlMode != L7_DOT1X_PORT_AUTO_MAC_BASED) && 
       (dot1xPortInfo[intIfNum].portControlMode != L7_DOT1X_PORT_AUTO_MAC_BASED))
  {
