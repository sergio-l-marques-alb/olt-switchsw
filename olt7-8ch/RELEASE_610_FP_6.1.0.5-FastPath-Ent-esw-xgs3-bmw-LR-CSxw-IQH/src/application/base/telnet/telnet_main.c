...
L7_RC_t telnetInitialize(telnetParams_t *telnetParams)
{
  ...
  {
    LOG_MSG("telnetInitialize: unable to add session; socket=%d\n",telnetParams->sock);
    return L7_ERROR;
  }
  ...
  if (tcgetattr(telnetParams->sock, &(session[sessionIndex].term.termios_exit)) < 0)
  {
    LOG_MSG("telnetInitialize: unable to get terminal parameters; index=%d socket=%d\n",sessionIndex,telnetParams->sock);
  }
...
L7_int32 telnetRemoteConnect(struct in_addr addr, L7_uint32 port)
{
  ...
  if (obtRc == L7_ERROR)
  {
    LOG_MSG("telnetRemoteConnect: unable to create server socket\n");
    return L7_ERROR;
  }
  ...
  if (obtRc != L7_SUCCESS)
  {
    LOG_MSG("telnetRemoteConnect: unable to connect to remote host\n");
...
L7_RC_t telnetCharModeSet(ioHandle_t *ioInfo)
{
  ...
  #if (L7_SERIAL_COM_ATTR)
  if (tcsetattr(ioInfo->clientSock, TCSAFLUSH, &(session[sessionIndex].term.termios_raw)) < 0)
  {
    LOG_MSG("telnetCharModeSet: unable to set character mode terminal parameters; socket=%d\n",ioInfo->clientSock);
  }
  #endif
  ...
  return L7_SUCCESS;
    ...
    {
      if (ioctl (ioInfo->clientSock, FIOOPTIONS, OPT_RAW | OPT_ECHO) < 0)
        LOG_MSG("telnetCharModeSet: unable to set character mode terminal parameters\n");
    }
    else if (ioctl (ioInfo->clientSock, FIOOPTIONS, OPT_RAW) < 0)
        LOG_MSG("telnetCharModeSet: unable to set character mode terminal parameters\n");
...
L7_RC_t telnetLineModeSet(ioHandle_t *ioInfo)
{
  ...
  {
    LOG_MSG("telnetLineModeSet: unable to set line mode terminal parameters; socket=%d\n",ioInfo->clientSock);
  }
  #endif
  ...
  return L7_SUCCESS;
    ...
    if (session[sessionIndex].modes.noechoMode == L7_TRUE)
    {
      if (ioctl (ioInfo->clientSock, FIOOPTIONS, OPT_TERMINAL & ~OPT_ECHO) < 0)
        LOG_MSG("telnetLineModeSet: unable to set line mode terminal parameters\n");
    }
...
#ifdef _L7_OS_LINUX_
  #if (L7_SERIAL_COM_ATTR)
 if (tcsetattr(clientSock, TCSAFLUSH, &(session[sessionIndex].term.termios_exit)) < 0)
 {
   LOG_MSG("telnetExitModeSet: unable to set exit mode terminal parameters;index=%d socket=%d\n",sessionIndex,clientSock);
 }
 #endif
#endif

#ifdef _L7_OS_VXWORKS_
  if (ioctl (clientSock, FIOOPTIONS, OPT_TANDEM) < 0)
    LOG_MSG("telnetExitModeSet: unable to set exit mode terminal parameters\n");
#endif

  telnetSessionIndexRemove(clientSock);
...
L7_RC_t telnetConEscape(ioHandle_t *ioInfo)
{
  ...
  if (telnetSessionIndexFind(ioInfo->clientSock, &sessionIndex) != L7_SUCCESS)
  {
    LOG_MSG("OBT: Error finding session for socket %d for escape processing.", ioInfo->clientSock);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("OBT: Error reading socket %d for escape processing in session %d.", ioInfo->clientSock, sessionIndex);
    return L7_FAILURE;
  }
...
L7_RC_t telnetTtypeSend(ioHandle_t *ioInfo)
{
  ...
  {
    LOG_MSG("telnetTtype: unable to get terminal type...defaulting to \"ansi\"\n");
