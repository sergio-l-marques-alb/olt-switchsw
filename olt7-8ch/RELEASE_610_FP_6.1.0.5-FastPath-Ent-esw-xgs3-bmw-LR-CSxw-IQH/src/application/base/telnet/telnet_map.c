...
L7_RC_t telnetMapAppsInit(void)
{
    ...
    {
        LOG_MSG("%s %d: %s: Unable to create Outbound Telnet data control semaphore\n",
...
L7_RC_t telnetSave(void)
{
  if (pTelnetMapCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    {
      LOG_MSG("telnetSave: Error during osapiFsWrite for config file %s\n", 
...
L7_RC_t telnetApplyConfigData(void)
{
  ...
  {
    LOG_MSG("telnetApplyConfigData: Error invalid admin mode\n");
    return L7_FAILURE;
  }
...
void telnetProcTask( void )
{
    ...
    while (L7_TRUE)
    {
        ...
        {
            LOG_MSG("telnetProcTask: Bad status on receive message from proc queue %08x\n", status);
        }
    } /* endwhile */
