...
dosIntfInfo_t *dosIntfInfo = L7_NULLPTR;
...
void doSBuildDefaultConfigData(L7_uint32 ver)
{
  ...
  doSCfgData->systemMaxICMPSize =     FD_DOS_DEFAULT_MAXICMP_SIZE;
  doSCfgData->systemMaxICMPv6Size =   FD_DOS_DEFAULT_MAXICMP_SIZE;
...
L7_RC_t doSApplyConfigData(void)
{
                            ...
                            L7_DOSCONTROL_TCPOFFSET_FEATURE_ID) == L7_TRUE)
  {
     ...
     {
        LOG_MSG("Failure setting system Offset DoS mode to %d\n", doSCfgData->systemTCPOffsetMode);
        rc = L7_FAILURE;
     }
  }
                            ...
                            L7_DOSCONTROL_ICMPV4_FEATURE_ID) == L7_TRUE)
  {
     if (dtlDoSICMPSet(doSCfgData->systemICMPMode, doSCfgData->systemMaxICMPSize) != L7_SUCCESS) 
     {
        LOG_MSG("Failure setting system ICMPv4 DoS mode to %d\n", doSCfgData->systemICMPMode);
        rc = L7_FAILURE;
     }
  }
                            ...
                            L7_DOSCONTROL_ICMPV6_FEATURE_ID) == L7_TRUE)
  {
     ...
     {
        LOG_MSG("Failure setting system ICMPv6 DoS mode to %d\n", doSCfgData->systemICMPMode);
        rc = L7_FAILURE;
     }
  }
...
L7_RC_t dosStartTask()
{
  ...
  {
    LOG_MSG("Could not create task dosTask\n");
    return L7_FAILURE;
  }

  if (osapiWaitForTaskInit (L7_DOS_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("Unable to initialize dosTask()\n");
    return(L7_FAILURE);
  }
