...
L7_RC_t sFlowPollerTimerStart(SFLOW_poller_t *pPoller, L7_uint32 interval)
{
  if (pPoller->timerData.ctrPollTimer != L7_NULL)
  {
    if (appTimerUpdate(agent.timerCB, &pPoller->timerData.ctrPollTimer,
                       ...
                       != L7_SUCCESS)
    {
      LOG_MSG("sFlowPollerTimerStart: Timer could not be updated for dsIndex %u instance %u interval %u\n",
                  ...
                  = handleListNodeStore(agent.handle_list, (void *)pPoller)) == 0)
  {
    ...
    L7_LOG(L7_LOG_SEVERITY_WARNING, L7_SFLOW_COMPONENT_ID, 
  ...
  if ((pPoller->timerData.ctrPollTimer
        ...
        == L7_NULL)
  {
    LOG_MSG("sFlowPollerTimerStart: Could not start poller timer for dsIndex %u instance %u\n",
...
L7_RC_t sFlowPollerTimerStop(SFLOW_poller_t *pPoller)
{
  ...
  if (sFlowTimerDestroy(&pPoller->timerData.ctrPollTimer,
                        ...
                        != L7_SUCCESS)
  {
    LOG_MSG("sFlowPollerTimerStop: Failed to stop poller timer for dsIndex %u instance %u\n",
             pPoller->key.dsIndex, pPoller->key.sFlowInstance);
    return L7_FAILURE;
  }
...
void sFlowPollerTimerExpiry(void *param)
{
  ...
  {
    LOG_MSG("sFlowPollerTimerExpiry: Failed to retrieve handle \n");
    return;
  }
  ...
  {
    LOG_MSG("sFlowPollerTimerExpiry: Failed to restart poller timer for dsIndex %u\n", 
            pPoller->key.dsIndex);
  }
}
/*************************************************************************
