...
L7_RC_t isdpModeSet(L7_uint32 mode)
{
  ...
  {
    LOG_MSG("Unsupported mode %d\n", mode);
    rc = L7_FAILURE;
  }
  else if(isdpCfg == L7_NULLPTR)
  {
    LOG_MSG("Configuration isn't initialized\n");
    rc = L7_FAILURE;
  }
  else
  {
...
L7_RC_t isdpTimerGet(L7_uint32 *timer)
{
  ...
  if (timer == L7_NULLPTR)
  {
    LOG_MSG("Parameter is equal to NULL\n");
    rc = L7_FAILURE;
  }
  else if(isdpCfg == L7_NULLPTR)
  {
...
L7_RC_t isdpHoldTimeSet(L7_uint32 holdTime)
{
  ...
  {
    LOG_MSG("Unsupported holdTime %d\n", holdTime);
    rc = L7_FAILURE;
  }
  else if(isdpCfg == L7_NULLPTR)
  {
...
L7_RC_t isdpIntfModeSet(L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  {
    osapiSnprintf(logBuf, sizeof(logBuf),"Unsupported interface %d\n", intIfNum);
    L7_LOG(L7_LOG_SEVERITY_DEBUG,L7_ISDP_COMPONENT_ID,logBuf);
    rc = L7_FAILURE;
  }
  ...
  else
  {
        ...
        (mode == L7_DISABLE && pCfg->mode != L7_DISABLE))
    {
      ...
      if(rc != L7_SUCCESS)
      {
        L7_LOG(L7_LOG_SEVERITY_ERROR ,L7_ISDP_COMPONENT_ID,"isdpIntfModeApply failed\n");
      }
    }
  }
    ...
    L7_uchar8 *nextDeviceId)
{
  ...
  if (isdpIsValidIntf(intIfNum) != L7_TRUE)
  {
    LOG_MSG("Unsupported interface %d\n", intIfNum);
    rc = L7_FAILURE;
  }
  else
  {
    ...
    L7_uchar8 *platform)
{
  ...
  {
    LOG_MSG("Bad parameters, interface %d\n", intIfNum);
    rc = L7_FAILURE;
  }
  else
  {
...
L7_RC_t isdpDebugTracePacketTxModeSet(L7_uint32 txTraceMode)
{
  ...
  {
    LOG_MSG("Unsupported txTraceMode %d\n", txTraceMode);
    rc = L7_FAILURE;
  }
...
L7_RC_t isdpDebugTracePacketTxModeGet(L7_uint32 *txTraceMode)
{
  ...
  {
    LOG_MSG("There is null pointer as param\n");
    rc = L7_FAILURE;
  }
  else
  {
...
L7_RC_t isdpDebugTracePacketRxModeSet(L7_uint32 rxTraceMode)
{
  ...
  {
    LOG_MSG("Unsupported rxTraceMode %d\n", rxTraceMode);
    rc = L7_FAILURE;
  }
...
L7_RC_t isdpDebugTraceEventsModeSet(L7_uint32 eventsTraceMode)
{
  ...
  {
    LOG_MSG("Unsupported eventsTraceMode %d\n", eventsTraceMode);
    rc = L7_FAILURE;
  }
...
L7_RC_t isdpDeviceIdFormatGet(L7_uint32 *deviceIdFormat)
{
  ...
  {
    *deviceIdFormat = ISDP_DEVICE_ID_SERIAL;
  }
...

{
  ...
  else if(index < 0 || index >= isdpDataCountEntryTree())
  {
    LOG_MSG("Index is out of range %d\n", index);
    rc = L7_FAILURE;
  }
  else
  {
