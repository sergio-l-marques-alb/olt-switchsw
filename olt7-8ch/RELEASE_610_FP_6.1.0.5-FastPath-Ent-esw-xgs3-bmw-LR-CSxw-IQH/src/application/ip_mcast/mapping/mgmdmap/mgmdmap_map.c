...
static void mgmdMapIntfChangeProcess(L7_uint32 intIfNum, 
...
static void mgmdMapNimEventProcess(mgmdMapCB_t *mgmdMapCbPtr, L7_uint32 intIfNum, 
                                   L7_uint32 event);
      ...
      switch(QIndex)
      {
        ...
        case MGMD_EVENT_Q:
        {
           switch (msg.msgId)
           {
             ...
             case (MGMDMAP_INTF_CHANGE_MSG):
               mgmdMapIntfChangeProcess(msg.u.mgmdMapIntfChangeParms.intIfNum, 
                                        msg.u.mgmdMapIntfChangeParms.event, 
                                        msg.u.mgmdMapIntfChangeParms.correlator);
               break;
...
void mgmdResetdataChanged(void)
{
  ...
  (void)mgmdMapCtrlBlockGet(L7_AF_INET6, &mgmdMapCbPtr);
  mgmdMapCbPtr->pMgmdMapCfgData->cfgHdr.dataChanged = L7_FALSE;
  return;
}
/*********************************************************************
...
L7_RC_t mgmdIntfCreate(mgmdMapCB_t *mgmdMapCbPtr, L7_uint32 intIfNum) 
{
                                ...
                                &pCfg) == L7_FALSE)
  {
    ...
    else
    {
      ...
      ** report failure                                                */
      MGMD_MAP_DEBUG (MGMD_MAP_DEBUG_FAILURES, "Unsuccessful in assigning config table entry for this intf\n");
      mgmdRC = L7_FAILURE;
    }
  }
  else
                                ...
                                ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  {
  case L7_RTR_ENABLE:
    if ((mgmdMapAdminModeGet(mgmdMapCbPtr->familyType, &mode) == L7_SUCCESS) && 
         mode == L7_ENABLE)
    {
  ...
  case L7_RTR_INTF_DISABLE_PENDING:
    if ((mgmdMapMgmdIntfInitialized(mgmdMapCbPtr->familyType, intIfNum) == 
        L7_SUCCESS) ||(mgmdMapMgmdProxyIntfInitialized(mgmdMapCbPtr->familyType, 
        intIfNum) == L7_SUCCESS))
    {
  ...
  case L7_RTR_INTF_ENABLE:
    if (((mgmdMapInterfaceModeGet(mgmdMapCbPtr->familyType, intIfNum, &mode) == 
          L7_SUCCESS) && mode == L7_ENABLE)||
        ((mgmdMapProxyInterfaceModeGet(mgmdMapCbPtr->familyType, intIfNum, &mode) ==
          L7_SUCCESS) && mode == L7_ENABLE))
    {
      if (mgmdMapIntfIsConfigurable(mgmdMapCbPtr, intIfNum , &pCfg) == L7_TRUE)
      {
  ...
  case L7_RTR_INTF_SECONDARY_IP_ADDR_ADD:
    /* Nothing to do on secondary address addition */
    break;

  case L7_RTR_INTF_SECONDARY_IP_ADDR_DELETE:
    /* Nothing to do on secondary address deletion */
    break;

  case L7_RTR_INTF_CONFIG_CLEAR:
    /* Clear the configuration for the interface */
    if (mgmdMapIntfIsConfigurable(mgmdMapCbPtr, intIfNum, &pCfg))
    {
      rc = mgmdIntfBuildDefaultConfigData(mgmdMapCbPtr, pCfg);
    }
    break;
  default:
    rc = L7_SUCCESS;
...
/*********************************************************************
* @purpose  Propogate Interface notifications to MGMD Map task
*
* @param    intIfNum    @b{(input)} Interface number
* @param    event       @b{(input)} Event type
...
* @end
*********************************************************************/
L7_RC_t mgmdMapIntfChangeCallback(L7_uint32 intIfNum, 
                                  L7_uint32 event, NIM_CORRELATOR_t correlator)
{                          
  mgmdMapMsg_t msg;
      ...
      ((intIfNum != 0) && (mgmdIntfIsValid(intIfNum) != L7_TRUE)))
  {
    ...
    status.component    = L7_FLEX_MGMD_MAP_COMPONENT_ID;
  ...
  msg.msgId = MGMDMAP_INTF_CHANGE_MSG;

  msg.u.mgmdMapIntfChangeParms.intIfNum = intIfNum;
  msg.u.mgmdMapIntfChangeParms.event = event;
  msg.u.mgmdMapIntfChangeParms.correlator = correlator;
...
* @end
*********************************************************************/
void mgmdMapIntfChangeProcess(L7_uint32 intIfNum, 
                              L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  L7_uint32    cbIndex = L7_NULL;
  L7_uint32    protoMax = L7_NULL;
                       ...
                       cbIndex < protoMax; cbIndex++, mgmdMapCbPtr++)
  {
    ...
    mgmdMapNimEventProcess(mgmdMapCbPtr, intIfNum, event);
  }
  status.intIfNum        = intIfNum;
  status.component       = L7_FLEX_MGMD_MAP_COMPONENT_ID;
  status.event           = event;
...
* @end
*********************************************************************/
void mgmdMapNimEventProcess(mgmdMapCB_t *mgmdMapCbPtr, L7_uint32 intIfNum, L7_uint32 event)
{
  L7_uint32 mode, rtrIfNum;
  L7_BOOL prevAcquired, currAcquired;
  L7_RC_t rc = L7_SUCCESS;
  mgmdIntfCfgData_t *intfCfg = L7_NULLPTR;
  mgmdMapCB_t *mgmdMapCb = mgmdMapCbPtr;
  ...
  {
    MGMD_MAP_DEBUG(MGMD_MAP_DEBUG_FAILURES,
              "event for intIfNum=%d while not in Execute or INIT_PHASE_3.\n", 
              intIfNum);
  }
  else if (mgmdIntfIsValid(intIfNum) == L7_TRUE)
  {
    switch (event)
    {
      case L7_UP:
      ...
      case L7_ATTACH:
        /* do nothing here, apply interface 
         * configuration when router interface event processed
         */
        break;
      ...
      case L7_ACTIVE:
                                   ...
                                   intIfNum, &rtrIfNum) == L7_SUCCESS)
        {
          ...
          COMPONENT_ACQ_NONZEROMASK(mgmdMapCbPtr->
              pMgmdInfo->mgmdIntfInfo[rtrIfNum].acquiredList, currAcquired);

          /* If the interface is not currently acquired by some component,
          ...
          if (currAcquired == L7_FALSE)
          {
            ...
            {
               (void)mgmdMapIntfModeEnable(mgmdMapCbPtr, intIfNum, MGMD_ROUTER_INTERFACE);
            }
            else if ((mgmdMapProxyInterfaceModeGet(mgmdMapCbPtr->familyType, 
                      intIfNum, &mode) == L7_SUCCESS) && mode == L7_ENABLE)
            {   
              (void)mgmdMapIntfModeEnable(mgmdMapCbPtr, intIfNum, MGMD_PROXY_INTERFACE);
            }
          }
        }
        break;
      ...
      case L7_LAG_ACQUIRE:
                                            ...
                                            intIfNum, &rtrIfNum) == L7_SUCCESS)
        {
          ...
          COMPONENT_ACQ_NONZEROMASK(mgmdMapCbPtr->
              pMgmdInfo->mgmdIntfInfo[rtrIfNum].acquiredList, prevAcquired);

          /* update aquired list */
          COMPONENT_ACQ_SETMASKBIT(mgmdMapCbPtr->
              pMgmdInfo->mgmdIntfInfo[rtrIfNum].acquiredList, 
      ...
      case L7_LAG_RELEASE:
        ...
        {
          /* update aquired list */
          COMPONENT_ACQ_CLRMASKBIT(mgmdMapCbPtr->
              pMgmdInfo->mgmdIntfInfo[rtrIfNum].acquiredList, 
              L7_DOT3AD_COMPONENT_ID);
...
L7_RC_t mgmdApplyDebugConfigData(void)
{
       ...
       familyIndex++)
  {
    ...
    { 
       ...
       {
         mgmdDebugTraceFlagSet(familyIndex+1, flagIndex, L7_TRUE);
       }
       else
       {
         mgmdDebugTraceFlagSet(familyIndex+1, flagIndex, L7_FALSE);
       }
    }
  }
  return rc;
