          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((mcastRC = mcastCnfgrInitPhase3Process( &response, 
                                                    &reason )) != L7_SUCCESS)
                {
                                    ...
                                    L7_CNFGR_ERR_RC_t *pReason)
{
  ...
  *pReason   = 0;
  ...
  strncpy((L7_uchar8 *)&(mcastGblVariables_g.mcastMapInfo.eventHandler.name),
                                     ...
                                     L7_CNFGR_ERR_RC_t *pReason )
{
  ...
  *pReason   = 0;
  ...
  if (nimRegisterIntfChange(L7_FLEX_MCAST_MAP_COMPONENT_ID, 
                            mcastMapIntfChangeCallback) != L7_SUCCESS)
  {
    MCAST_MAP_DEBUG (MCAST_MAP_DEBUG_FAILURES, "\nMCAST_MAP: Failed to register MCAST Map callback with NIM.\n");
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    return L7_FAILURE;
  }
                                     ...
                                     L7_CNFGR_ERR_RC_t *pReason )
{
  ...
  *pReason   = 0;
  ...
  if (sysapiCfgFileGet(L7_FLEX_MCAST_MAP_COMPONENT_ID, L7_MCAST_CFG_FILENAME, 
     (L7_char8 *)&mcastGblVariables_g.mcastMapCfgData, sizeof(L7_mcastMapCfg_t),
     &mcastGblVariables_g.mcastMapCfgData.checkSum, L7_MCAST_CFG_VER_CURRENT,
      mcastMapBuildDefaultConfigData, mcastMigrateConfigData) != L7_SUCCESS)
  {
    MCAST_MAP_DEBUG (MCAST_MAP_DEBUG_FAILURES, "\nMCAST_MAP:Failed to retrieve config file \n");
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    return L7_FAILURE;
  }
