...
/* Iterate through all destinations */
  while (pData != L7_NULLPTR)
  {
    ...
    {
      if (L7_INET_ADDR_COMPARE(&route->neighbor->nbrAddr, nbrIpAddr) == 0)
        rts++;
    }
    pData = pNextData;      
  }
 ...
 * one from the caller */
  for (tempIfNum = *rtrIfNum;(tempIfNum < MCAST_MAX_INTERFACES); 
      tempIfNum++)
  {
                                ...
                                L7_ulong32 *pruneExpTime)
{
  ...
  L7_int32 min_lifetime = DVMRP_AVERAGE_PRUNE_LIFETIME;	
  L7_uint32 now;	
                           ...
                           L7_inet_addr_t *pruneSrcMask)
{
  ...
  entry = dvmrp_cache_lookup (dvmrpCB,pruneSrc, pruneGrp);
  if ((entry != L7_NULLPTR) && (SLLNumMembersGet(&(entry->ll_prunes)) > L7_NULL))
  {
    rc = L7_SUCCESS;
  }
                               ...
                               L7_inet_addr_t *pruneSrc, L7_inet_addr_t *pruneSrcMask)
{
  ...
  dvmrpTreeKey_t key;
  L7_uint32 skipentries = 0;
  L7_char8 src[DVMRP_STRING_SIZE], grp[DVMRP_STRING_SIZE], msk[DVMRP_STRING_SIZE];
  ...
  while (L7_NULLPTR != entry)
  {
    ...
    if (SLLNumMembersGet(&(entry->ll_prunes)) == L7_NULL)
    {
      entry = entryNext;
      continue;
    }
