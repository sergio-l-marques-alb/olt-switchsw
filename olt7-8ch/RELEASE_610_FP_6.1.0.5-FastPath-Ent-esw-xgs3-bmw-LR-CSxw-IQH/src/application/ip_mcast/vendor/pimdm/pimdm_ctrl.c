                          ...
                          pimdmSendJoinPruneData_t *pJPData)
{
  ...
  L7_BOOL pktType = pJPData->pktType;
...
void pimdmJPBundleTimerExpiryHandler(void *pParam)
{
  ...
  {
    LOG_MSG ("PIMDM pimdmJPBundleTimer is NULL, But Still Expired");
    return;
  }
                             ...
                             L7_inet_addr_t *nbrAddr)
{
  ...
  if ((mrtEntry = pimdmMrtEntryGet (pimdmCB, srcAddr, grpAddr)) == L7_NULLPTR)
  {
    ...
    /* If the entry is timed out and you receive a State Refresh Message
     * from your upstream interface, Do the following:
     * 1. Create the MRT Entry to establish the State information.
     * 2. Compute the OIF List.
     * 3. If OIF is nonNull and Prune Indicator Bit is Set, send a Graft Upstream.
     *    Else, Send a Prune Upstream and Set the PLT.
     */
    ...
    /* Check if the OIF List is NULL, else Send a Graft upstream */
    BITX_IS_EMPTY (&mrtEntry->oifList, oifIsEmpty);
    if (oifIsEmpty != L7_TRUE)
    {
      pimdmUpstrmEventInfo_t upstrmFsmEventInfo;

      if (PIMDM_BIT_TST (stRfrMsg->reserved, PIMDM_STRFR_PRUNE_INDICATOR_BIT) != 0)
      {
    ...
    else
    {
       ...
       */
      pimdmUpstrmFsmExecute (PIMDM_UPSTRM_EVT_OIF_NULL,
                             mrtEntry, &upstrmFsmEventInfo);
    }
