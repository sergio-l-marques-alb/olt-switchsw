                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason   = 0;
  ...
  memset(( void * )ipMapIntfInfo, 0, 
           sizeof(ipMapIntfInfo_t) * (L7_IPMAP_INTF_MAX_COUNT + 1));  
  ...
  if (rtoCnfgrInitPhase1Process(L7_L3_ROUTE_TBL_SIZE_TOTAL, L7_IP_NHRES_MAX) != L7_SUCCESS)
  {
    *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    ipMapRC     = L7_ERROR;
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason   = 0;
  ...
  if (nimRegisterIntfChange(L7_IP_MAP_COMPONENT_ID, ipMapIntfChangeCallback) != L7_SUCCESS)
  {
...
/*    L7_assert(1); */
  ...
  {
    IPMAP_ERROR_SEVERE("%s %d: %s:  ERROR: Unable to register best route callback with RTO\n",
                       __FILE__, __LINE__, routine_name );
    *pResponse = 0;
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    ipMapRC    = L7_ERROR;
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  static const char *routine_name = "ipMapCnfgrInitPhase3Process()";
  *pResponse = L7_CNFGR_CMD_COMPLETE;
  *pReason   = 0;
  ipMapRC   = L7_SUCCESS;
  ...
  if (sysapiCfgFileGet(L7_IP_MAP_COMPONENT_ID, 
                       L7_IP_CFG_FILENAME, (L7_char8 *)ipMapCfg, 
                       sizeof(L7_ipMapCfg_t), &ipMapCfg->checkSum, 
                       L7_IP_CFG_VER_CURRENT, ipBuildDefaultConfigData,
                       ipmapMigrateConfigData) != L7_SUCCESS)
  {
    IPMAP_ERROR_SEVERE("%s %d: %s: Error retrieving file\n",
                       __FILE__, __LINE__, routine_name);
    *pResponse = 0;
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    ipMapRC   = L7_ERROR;
  ...
  if (sysapiCfgFileGet(L7_IP_MAP_COMPONENT_ID, 
                       ...
                       sizeof(L7_rtrStaticRouteCfg_t), &route->checkSum, 
                       L7_IP_STATIC_ROUTES_CFG_VER_CURRENT, 
                       ipStaticRtsBuildDefaultConfigData,
                       ipStaticRtsMigrateConfigData) != L7_SUCCESS)
  {
    IPMAP_ERROR_SEVERE("%s %d: %s: Error retrieving file\n",
                       __FILE__, __LINE__, routine_name);
    *pResponse = 0;
  ...
  if (sysapiCfgFileGet(L7_IP_MAP_COMPONENT_ID, 
                       ...
                       sizeof(L7_rtrStaticArpCfg_t), &arp->checkSum, 
                       L7_IP_STATIC_ARP_CFG_VER_CURRENT, 
                       ipStaticArpBuildDefaultConfigData,
                       ipStaticArpMigrateConfigData) != L7_SUCCESS)
  {
    IPMAP_ERROR_SEVERE("%s %d: %s: Error retrieving file\n",
                       __FILE__, __LINE__, routine_name);
    *pResponse = 0;
...
void ipMapCnfgrFiniPhase1Process()
{
  ...
  rtoCnfgrFiniPhase1Process(L7_L3_ROUTE_TBL_SIZE_TOTAL);
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                if ((ipMapRC = ipMapCnfgrInitPhase3Process( &response, &reason )) != L7_SUCCESS)
                {
