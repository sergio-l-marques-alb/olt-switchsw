...
L7_RC_t ipStartTasks(void)
{
    ...
    {
        L7_LOG(L7_LOG_SEVERITY_ALERT, L7_IP_MAP_COMPONENT_ID,
...
/*********************************************************************
* @purpose  Saves Ip user config file to NVStore
*
* @param    void
*
* @returns  L7_SUCCESS
...
L7_RC_t ipSave(void)
{
  ...
  static const char *routine_name = "ipSave()";

  IPMAP_TRACE("%s %d: %s : \n", __FILE__, __LINE__, routine_name);


  if (ipMapCfg->cfgHdr.dataChanged == L7_TRUE)
  {
    ipMapCfg->cfgHdr.dataChanged = L7_FALSE;
    ipMapCfg->checkSum = nvStoreCrc32((L7_char8 *)ipMapCfg,
                                      sizeof(L7_ipMapCfg_t) - sizeof(ipMapCfg->checkSum));

    if (sysapiCfgFileWrite(L7_IP_MAP_COMPONENT_ID,
                           L7_IP_CFG_FILENAME,
                           (L7_char8 *)ipMapCfg,
                           sizeof(L7_ipMapCfg_t)) != L7_SUCCESS)
    {
      IPMAP_ERROR("%s %d: %s: Error on call to osapiFsWrite routine on config file %s\n",
                  __FILE__, __LINE__, routine_name, L7_IP_CFG_FILENAME);
      return L7_ERROR;
    }
  }

  if (route->cfgHdr.dataChanged == L7_TRUE)
  {
    route->cfgHdr.dataChanged = L7_FALSE;
    route->checkSum = nvStoreCrc32((L7_char8 *)route,
                                   sizeof(L7_rtrStaticRouteCfg_t) - sizeof(route->checkSum));

    /* call save NVStore routine */
    if (sysapiCfgFileWrite(L7_IP_MAP_COMPONENT_ID,
                           L7_IP_STATIC_ROUTES_CFG_FILENAME, (L7_char8 *)route,
                           sizeof (L7_rtrStaticRouteCfg_t)) != L7_SUCCESS)
    {
      IPMAP_ERROR("%s %d: %s: Error on call to osapiFsWrite routine on config file %s\n",
                  __FILE__, __LINE__, routine_name, L7_IP_STATIC_ROUTES_CFG_FILENAME);
      return L7_ERROR;
    }
  }

  if (arp->cfgHdr.dataChanged == L7_TRUE)
  {
    arp->cfgHdr.dataChanged = L7_FALSE;
    arp->checkSum = nvStoreCrc32((L7_char8 *)arp,
                                 sizeof(L7_rtrStaticArpCfg_t) - sizeof(arp->checkSum));

    /* call save NVStore routine */
    if (sysapiCfgFileWrite(L7_IP_MAP_COMPONENT_ID,
                           L7_IP_STATIC_ARP_CFG_FILENAME, (L7_char8 *)arp,
                           sizeof (L7_rtrStaticArpCfg_t)) != L7_SUCCESS)
    {
      IPMAP_ERROR("%s %d: %s: Error on call to osapiFsWrite routine on config file %s\n",
                  __FILE__, __LINE__, routine_name, L7_IP_STATIC_ARP_CFG_FILENAME);
      return L7_ERROR;
    }
  }
...
void ipBuildDefaultConfigData(L7_uint32 ver)
{
  ...
  strcpy(ipMapCfg->cfgHdr.filename, L7_IP_CFG_FILENAME);
  ...
  ipMapCfg->ip.ipArpCacheSize        =  FD_IP_DEFAULT_ARP_CACHE_SIZE;
...
{
  static const char *routine_name = "ipIntfBuildDefaultConfigData()";
...
void ipStaticRtsBuildDefaultConfigData(L7_uint32 ver)
{
  ...
  strcpy(route->cfgHdr.filename, L7_IP_STATIC_ROUTES_CFG_FILENAME);
...
{
  /* verify config structure maximum is not exceeded */
  #if (FD_RTR_MAX_STATIC_ARP_ENTRIES < L7_RTR_MAX_STATIC_ARP_ENTRIES)
  #error The FD_RTR_MAX_STATIC_ARP_ENTRIES definition value is too low!
  #endif
  ...
  strcpy(arp->cfgHdr.filename, L7_IP_STATIC_ARP_CFG_FILENAME);
...
L7_RC_t ipMapIntfCreate(L7_uint32 intIfNum, L7_uint32 *eventCompleted)
{
  ...
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&ipMapCfg->ckt[i].configId, &configId))
    {
  ...
  else if (ipMapIntfSupports(intIfNum,
                             IPMAP_INTFCAP_ROUTING_CONFIGURABLE) != L7_TRUE)
  {
    /*
     * Some interfaces can only have routing enabled internally,
     * not from persistent config. Initialize disabled.
     */
    pCfg->flags &= ~L7_RTR_INTF_ADMIN_MODE_ENABLE;
  }
                                ...
                                NIM_CORRELATOR_t correlator)
{
  ...
  if (ipMapIsValidIntf(intIfNum) != L7_TRUE)
  {
    if (ipMapTraceFlags & IPMAP_TRACE_NIM_EVENTS)
    {
       ...
       osapiSnprintf(traceBuf, IPMAP_TRACE_LEN_MAX, "IP MAP ignoring NIM event %s for invalid IPMap intf %u.",
              nimGetIntfEvent(event), intIfNum);
  ...
  if (pIpMapInfo->operIntf[intIfNum].nimParms.correlator != L7_NULL)
      {
          ...
          LOG_MSG("More than one outstanding NIM callback on interface %d\n", intIfNum);
      LOG_MSG("First correlator is %x. Second correlator is %x\n",
              pIpMapInfo->operIntf[intIfNum].nimParms.correlator, correlator);
          rc = L7_FAILURE;
          status.response.rc  = rc;
          if (ipMapTraceFlags & IPMAP_TRACE_NIM_EVENTS)
          {
              L7_uchar8 traceBuf[IPMAP_TRACE_LEN_MAX];
              osapiSnprintf(traceBuf, IPMAP_TRACE_LEN_MAX, "IP MAP ignoring 2nd NIM event %s for interface %u.",
                      nimGetIntfEvent(event), intIfNum);
              ipMapTraceWrite(traceBuf);
          }
          nimEventStatusCallback(status);
  ...
  if (IPMAP_IS_READY)
  {
      ...
      {
          L7_uchar8 traceBuf[IPMAP_TRACE_LEN_MAX];
          osapiSnprintf(traceBuf, IPMAP_TRACE_LEN_MAX, "IP MAP processing NIM event %s for interface %u.",
                  nimGetIntfEvent(event), intIfNum);
          ipMapTraceWrite(traceBuf);
      }
    switch (event)
    ...
    case (L7_ATTACH):
        if (ipMapIntfInfo[intIfNum].intfAcquired == L7_TRUE)
            break;
    ...
    case (L7_DETACH):
      ...
      case (L7_ACTIVE):
        if (ipMapIntfInfo[intIfNum].intfAcquired != L7_TRUE)
        {
            ipMapIntfInfo[intIfNum].intfAcquired = L7_FALSE;
            ipMapIntfUpdate(intIfNum, &event_completed);
        }
        break;
      ...
      case (L7_INACTIVE):
        if (ipMapIntfInfo[intIfNum].intfAcquired != L7_TRUE)
        {
          rc = ipMapIntfUpdate(intIfNum, &event_completed);
        }
        break;

      case (L7_LAG_ACQUIRE):
      case (L7_PROBE_SETUP):
          /* LAG members do not act as independent interfaces */
          COMPONENT_ACQ_NONZEROMASK(ipMapIntfInfo[intIfNum].acquiredList,
                                    prevAcquired);
          if (event == L7_LAG_ACQUIRE)
              COMPONENT_ACQ_SETMASKBIT(ipMapIntfInfo[intIfNum].acquiredList,
          ...
          {
            ipMapIntfInfo[intIfNum].intfAcquired = L7_TRUE;
            rc = ipMapIntfUpdate(intIfNum, &event_completed);
          }
          break;
      ...
      case (L7_PROBE_TEARDOWN):
          ...
          if (event == L7_LAG_RELEASE)
              COMPONENT_ACQ_CLRMASKBIT(ipMapIntfInfo[intIfNum].acquiredList,
                                       L7_DOT3AD_COMPONENT_ID);
          ...
          COMPONENT_ACQ_NONZEROMASK(ipMapIntfInfo[intIfNum].acquiredList,
                                    currAcquired);
          if ((prevAcquired == L7_TRUE) && (currAcquired == L7_FALSE))
          {
              ipMapIntfInfo[intIfNum].intfAcquired = L7_FALSE;
              ipMapIntfUpdate(intIfNum, &event_completed);
          }
      ...
      case (L7_SPEED_CHANGE):
        /* Interface metric is handled by the individual routing protocols */
        break;
  ...
  {
    LOG_MSG("Received ipMapIntfChangeCallback outside of permitted phase");
  }
  ...
  if (event_completed == L7_TRUE)
  {
     /* Return event completion status  */
      ...
      {
          L7_uchar8 traceBuf[IPMAP_TRACE_LEN_MAX];
          osapiSnprintf(traceBuf, IPMAP_TRACE_LEN_MAX, "IP MAP responding to NIM event %s intf %u.",
                        nimGetIntfEvent(event), intIfNum);
          ipMapTraceWrite(traceBuf);
      }
      nimEventStatusCallback(status);
  ...
  else
  {
    /* save for later nimEventStatusCallback */
      ...
      {
          L7_uchar8 traceBuf[IPMAP_TRACE_LEN_MAX];
          osapiSnprintf(traceBuf, IPMAP_TRACE_LEN_MAX, "IP MAP deferring response to NIM event %u intf %u.",
                  event, intIfNum);
          ipMapTraceWrite(traceBuf);
      }
  }
...
L7_RC_t ipMapArpEntryModify(L7_arpEntry_t *pArpEntry)
{
  ...
  IPMAP_TRACE("%s : IP Address %#x and intIfNum %d\n",
              routine_name, pArpEntry->ipAddr, pArpEntry->intIfNum);
...
void ipMapForwardingTask(void)
{
    ...
    for (;;)
    {
        ...
        */
        ...
        {
            L7_LOG(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
...
{
  L7_int32 ipMapQLen;
  osapiMsgQueueGetNumMsgs(ipMapProcess_Queue, &ipMapQLen);
  SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_IPMAP,
    ...
    if (ipMapProcess_Queue != L7_NULL)
    {
      ...
      if (status == L7_SUCCESS)
      {
        ...
        switch (Message.msgId)
        {
          ...
          /* This path manipulates only ARP and RTO data. Don't take IP MAP lock. */
             ipMapBestRoutesGet();
          break;
  ...
  */
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    if ((pduInfo.vlanId == simMgmtVlanIdGet()) &&
        dot1qIntfIsVlanMember(pduInfo.vlanId, pduInfo.intIfNum))
    {
...
L7_BOOL ipMapMapIntfIsConfigurable(L7_uint32 intIfNum, L7_rtrCfgCkt_t **pCfg)
{
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(ipMapCfg->ckt[index].configId)) == L7_FALSE)
    {
      ...
      */
      LOG_MSG("Error accessing IPMAP config data for interface %d in ipMapMapIntfIsConfigurable.\n", intIfNum);
      return L7_FALSE;
    }
  }
