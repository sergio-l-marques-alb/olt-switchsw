...
static int ipMapArpIpIntfCompare(const void *a, const void *b, size_t len)
{
  ...
  {
    LOG_MSG("Invalid key length in ipMapArpIpIntfCompare\n" );
...
L7_RC_t ipMapArpGwTableCreate(void)
{
  ...
  pArpCtx->gwTbl.treeHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, treeHeapSize);
  if (pArpCtx->gwTbl.treeHeap == L7_NULLPTR)
  {
    LOG_MSG("%s: Unable to allocate Gateway Table tree heap\n", routine_name);
    return L7_FAILURE;
  }

  pArpCtx->gwTbl.dataHeapSize = dataHeapSize;
  pArpCtx->gwTbl.dataHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, dataHeapSize);
  if (pArpCtx->gwTbl.dataHeap == L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, pArpCtx->gwTbl.treeHeap);
    pArpCtx->gwTbl.treeHeap = L7_NULLPTR;
    LOG_MSG("%s: Unable to allocate Gateway Table data heap\n", routine_name);
    return L7_FAILURE;
  }
  ...
  if(TIMER_InitSec(1, pArpCtx, &pArpCtx->gwArpRqstTimer) != E_OK)
    LOG_MSG("%s: Failed to init gwArpRqstTimer\n", routine_name);
...
L7_RC_t ipMapArpGwTableDelete(void)
{
  ...
  if (pArpCtx->gwTbl.treeData.semId != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("%s: Could not delete AVL tree semaphore, id=0x%8.8x\n",
  ...
  if (pArpCtx->gwTbl.dataHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, pArpCtx->gwTbl.dataHeap);
                              ...
                              L7_BOOL staticRouteGw)
{
  ...
  if (pNode == L7_NULLPTR)
  {
    ...
    if (pNode == L7_NULLPTR)
    {
      ...
      LOG_MSG("%s: Cannot find new ARP Gateway entry for %s intf %d\n", 
              routine_name, (char *)ipStr, intIfNum);
      L7_assert(1);                     /* need to know about this */
  ...
  if (pNode->refCnt == 0)
  {
    ...
    else
    {
      ...
      if (holdover == L7_TRUE)
      {
        ...
        {
          LOG_MSG("%s: Could not delete ARP entry 0x%8.8x\n",
                  routine_name, ipAddr);
        }
      }
    }
...
L7_RC_t ipMapArpIpTableCreate(void)
{
  ...
  pArpCtx->ipTbl.treeHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, treeHeapSize);
  if (pArpCtx->ipTbl.treeHeap == L7_NULLPTR)
  {
    LOG_MSG("%s: Unable to allocate ARP IP Table tree heap\n", routine_name);
    return L7_FAILURE;
  }

  pArpCtx->ipTbl.dataHeapSize = dataHeapSize;
  pArpCtx->ipTbl.dataHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, dataHeapSize);
  if (pArpCtx->ipTbl.dataHeap == L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, pArpCtx->ipTbl.treeHeap);
    LOG_MSG("%s: Unable to allocate ARP IP Table data heap\n", routine_name);
    return L7_FAILURE;
  }
...
L7_RC_t ipMapArpIpTableDelete(void)
{
  ...
  if (pArpCtx->ipTbl.dataHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, pArpCtx->ipTbl.dataHeap);
...
L7_RC_t ipMapArpIpTableInsert(ipMapArpIpNode_t *pIpInfo, ipMapArpIpNode_t **ppIp)
{
  ...
  *ppIp = L7_NULLPTR;
  ...
  do
  {
    ...
    {
      /* entry add failed */
      LOG_MSG("%s: Failure adding %s intf %d to ARP table\n", 
    ...
    else if (pNode != L7_NULLPTR)
    {
      ...
      LOG_MSG("%s: Entry for %s intf %d already exists in ARP table\n", 
              routine_name, (char *)ipStr, pIpInfo->intIfNum);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not inserted in table */
      break;                            /* exit control loop */
    ...
    {
      /* cannot find new entry */
      LOG_MSG("%s: Cannot find new ARP table entry for %s intf %d\n", 
              routine_name, (char *)ipStr, pIpInfo->intIfNum);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not inserted in table */
      break;                            /* exit control loop */
    ...
    if (ipMapArpIpAgeListAdd(pNode) != L7_SUCCESS)
    {
      ...
      LOG_MSG("%s: Failure to add %s intf %d to ARP age list\n", 
              routine_name, (char *)ipStr, pIpInfo->intIfNum);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not inserted in table */
      break;                            /* exit control loop */
    ...
    *ppIp = pNode;
    ...
    if (ipMapArpDeviceTableApply(pNode, IPM_ARP_TABLE_INSERT) != L7_SUCCESS)
    {
      ...
      LOG_MSG("%s: Failure adding %s in device ARP table\n", 
                              ...
                              ipMapArpTableEvent_t event)
{
  ...
  do
  {
    ...
    {
      LOG_MSG("%s: Wrong table entry obtained for addr %s intf %d\n", 
              routine_name, (char *)ipStr, pIpInfo->intIfNum);
      L7_assert(1);                     /* need to know about this */
    ...
    {
      /* just log a message, but do not change outcome of table operation */
      LOG_MSG("%s: Failure updating %s intf %d in device ARP table\n", 
              routine_name, (char *)ipStr, pIpInfo->intIfNum);
    }
...
L7_RC_t ipMapArpIpTableRemove(ipMapArpIpNode_t *pIpInfo)
{
  ...
  do
  {
    ...
    if (pNode->key != pIpInfo->key || pNode->intIfNum != pIpInfo->intIfNum)
    {
      LOG_MSG("%s: Wrong table entry obtained for addr %s\n", 
              routine_name, (char *)ipStr);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not deleted from table */
      break;                            /* exit control loop */
    ...
    {
      LOG_MSG("%s: Failure to remove %s intf %d from ARP table\n", 
              routine_name, (char *)ipStr, pNode->intIfNum);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not removed from table */
      break;                            /* exit control loop */
    }
...
L7_RC_t ipMapArpIpTableOldestRemove(L7_BOOL allowGw)
{
  ...
  do
  {
    ...
    if (ipMapArpIpTableRemoveCommon(pNode) != L7_SUCCESS)
    {
      LOG_MSG("%s: Failure to remove %s from ARP table\n", 
              routine_name, (char *)ipStr);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not removed from table */
      break;                            /* exit control loop */
...
static L7_RC_t ipMapArpIpTableRemoveCommon(ipMapArpIpNode_t *pNode)
{
  ...
  {
    /* just log a message, but do not change outcome of table operation */
    LOG_MSG("%s: Failure stopping ARP entry timers for %s intf %d\n", 
            routine_name, (char *)ipStr, pNode->intIfNum);
  }
  ...
  {
    /* just log a message, but do not change outcome of table operation */
    LOG_MSG("%s: Failure removing %s intf %d in device ARP table\n", 
            routine_name, (char *)ipStr, pNode->intIfNum);
  }
...
L7_BOOL ipMapArpIpTableFullCheck(void)
{
  ...
  {
    LOG_MSG("%s: ARP table has overflowed\n", routine_name);
    L7_assert(1);                       /* need to know about this */
    /* continue processing */
  }
  if (ipMapArpCtx_g.staticCurrCt > ipMapArpCtx_g.staticMaxCt)
  {
    LOG_MSG("%s: ARP table static entries have overflowed\n", routine_name);
    L7_assert(1);                       /* need to know about this */
    /* continue processing */
  }
...
L7_RC_t ipMapArpIpTableEntryStaticConvert(ipMapArpIpNode_t *pIpInfo)
{
  ...
  do
  {
    ...
    {
      LOG_MSG("%s: Failure to remove %s from ARP age list\n", 
              routine_name, (char *)ipStr);
      L7_assert(1);                     /* need to know about this */
      rc = L7_FAILURE;                  /* node not removed from table */
      break;                            /* exit control loop */
