...
L7_RC_t ipMapArpInitPhase1Process(void)
{
  ...
  if ((erc = ARP_Init(p_Prm, (t_Handle)NULLP, (t_Handle)NULLP, 0, &ipMapArpCtx_g.arpHandle)) 
      != E_OK)
  {
    LOG_MSG("IPM_ARP: Failed to initialize L3 ARP Manager, rc=%d\n",
            erc);
    if (erc == E_NOMEMORY)
    {
      LOG_MSG("Out of memory in routing heap.");
    }
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("IPM_ARP: Could not create gateway table, rc=%d\n", rc);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("IPM_ARP: Could not create ARP IP lookup table, rc=%d\n", rc);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("IPM_ARP: Could not create ARP component lock\n");
    return L7_FAILURE;
  }
...
L7_RC_t ipMapArpInitPhase2Process(void)
{
  ...
  {
    LOG_MSG("IPM_ARP: Could not register ARP Query callback with DTL, rc=%d\n", 
            rc);
    return L7_FAILURE;
  }
...
void ipMapArpFiniPhase1Process(void)
{
  ...
  if ((rc = ipMapArpGwTableDestroy()) != L7_SUCCESS)
    LOG_MSG("IPM_ARP: Could not destroy gateway table, rc=%d\n", rc);

  if ((rc = ipMapArpIpTableDestroy()) != L7_SUCCESS)
    LOG_MSG("IPM_ARP: Could not destroy ARP IP lookup table, rc=%d\n", rc);
    ...
    if (ipAddr != 0)
    {
      ...
      case RTO_ADD_ROUTE:
        ...
        if (ipMapArpGwTableInsert(ipAddr, intIfNum, staticRouteGw) != L7_SUCCESS)
        {
          ...
          LOG_MSG("Unable to insert %s in ARP gateway table\n", (char *)ipStr);
        }
        break;
      ...
      case RTO_DELETE_ROUTE:
        if (ipMapArpGwTableRemove(ipAddr, intIfNum) != L7_SUCCESS)
        {
          L7_uchar8 ipStr[OSAPI_INET_NTOA_BUF_SIZE];
          osapiInetNtoa(ipAddr, ipStr);
          LOG_MSG("Unable to remove %s from ARP gateway table\n", (char *)ipStr);
        }
        break;
                             ...
                             ipMapArpIntfRegParms_t *pParms)
{
  ...
  {
    LOG_MSG("IPM_ARP: Out of resource for intf register (obj=0x%8.8x)\n",
            pParms->intfHandle);
    IPM_ARP_SEMA_GIVE(&ipMapArpCtx_g.arpLock);
    return L7_ERROR;
  }
  ...
  {
    ipMapArpIntfInUse_g[i] = L7_NULL;
    LOG_MSG("IPM_ARP: Unable to allocate local intf instance for obj=0x%8.8x\n",
            pParms->intfHandle);
    IPM_ARP_SEMA_GIVE(&ipMapArpCtx_g.arpLock);
    return L7_ERROR;
  }
  ...
  {
    LOG_MSG("IPM_ARP: OLL_Insert failed for intf %u, rc=%d\n",
            pIntf->index, erc);
    IPM_ARP_SEMA_GIVE(&ipMapArpCtx_g.arpLock);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("IPM_ARP: ARP_AddInterface failed for intf %u, rc=%d\n",
            pIntf->index, erc);
    IPM_ARP_SEMA_GIVE(&ipMapArpCtx_g.arpLock);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("IPM_ARP: ARP_AddLocalAddr failed for IP addr 0x%8.8x " \
...
L7_RC_t ipMapArpIntfUnregister(L7_uint32 intIfNum)
{
  ...
  {
    LOG_MSG("IPM_ARP: ARP_DelLocalAddr failed for IP addr 0x%8.8x " \
            "(intf %u), rc=%d\n", pIntf->ipAddr, pIntf->index, erc);
    rc = L7_FAILURE;
    /* keep going */
  }
  ...
  if ((erc = ARP_DelInterface(ipMapArpCtx_g.arpHandle, (t_LIH)pIntf->index)) 
      != E_OK)
  {
    LOG_MSG("IPM_ARP: ARP_DelInterface failed for intf %u, rc=%d\n",
            pIntf->index, erc);
    rc = L7_FAILURE;
    /* keep going */
  }
  ...
  {
    LOG_MSG("IPM_ARP: OLL_Extract failed for intf %u, rc=%d\n",
            pIntf->index, erc);
    rc = L7_FAILURE;
    /* keep going */
  }
                                        ...
                                        L7_uint32 secondaryMask)
{
  ...
  */
                ...
                (t_MACAddr *)(byte *)pIntf->macAddr, (word)pIntf->index)) != E_OK)
  {
    ...
    LOG_MSG("IPM_ARP: ARP_AddLocalAddr failed for IP addr %-15.15s " \
                                          ...
                                          L7_uint32 secondaryMask)
{
  ...
  {
    osapiInetNtoa((L7_uint32)secondaryIp, ipStr);
    LOG_MSG("IPM_ARP: ARP_DelLocalAddr failed for IP addr %-15.15s " \
        "(intf %u), rc=%d\n", ipStr, pIntf->index, erc);
    IPM_ARP_SEMA_GIVE(&ipMapArpCtx_g.arpLock);
    return L7_FAILURE;
  }
