...
static L7_RC_t ospfMapIntfActivate(L7_uint32 intIfNum);
static L7_RC_t ospfMapIntfEventProcess(L7_uint32 intIfNum, 
                                       L7_uint32 event,
...
{
    static const char *routine_name = "ospfRestoreProcess()";
    ...
    if (ospfMapAppsInit() != L7_SUCCESS)
    {
      L7_LOG(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID, 
             "Failure initializing OSPF during OSPF restore.");
    }
...
L7_RC_t ospfDefaultIfType(L7_uint32 intIfNum, L7_uint32 *pIfType)
{
    ...
    {
      OSPFMAP_ERROR("%s %d: ERROR: invalid internal interface ID\n",
                      __FILE__, __LINE__);
      return L7_FAILURE;
    }
...
L7_RC_t ospfApplyConfigData(void)
{
    ...
    static const char *routine_name = "ospfApplyConfigData()";
    ...
    {
        ospfMapOspfAdminModeEnable();
    }
    else if (pOspfMapCfgData->rtr.ospfAdminMode == L7_DISABLE)
    {
        ospfMapOspfAdminModeDisable();
    }
    else
    {
        OSPFMAP_ERROR("%s %d: %s: ERROR:  Cannot decipher OSPF admin mode configuration\n",
                      __FILE__, __LINE__, routine_name);
        L7_assert(1);
        return(L7_FAILURE);
    }
    ...
    ospfMapOspfOpaqueLsaSupportSet(pOspfMapCfgData->rtr.opaqueLSAMode);
        ...
        if ( pOspfMapCfgData->area[i].inUse == L7_TRUE)
        {
                ...
                != L7_SUCCESS)
            {
                OSPFMAP_ERROR("%s %d: %s: ERROR: Inserting area %x in the MIB table\n",
                              __FILE__, __LINE__, routine_name, pOspfMapCfgData->area[i].area);
            }
        }
    } /*  i < L7_MAX_OSPF_AREAS */             
...
L7_RC_t ospfMapAppsInit(void)
{
    ...
    static const char *routine_name = "ospfMapAppsInit()";

    OSPFMAP_TRACE("%s %d: %s : \n", __FILE__, __LINE__, routine_name);

    /*-------------------------------------*/
    /*   Initialize the ospf vendor stack  */
    /*-------------------------------------*/
    if ((ospfMapRouterIDGet(&routerId) == L7_SUCCESS) &&
        (routerId > 0))
    {
        if (ospfMapOspfVendorInit() != L7_SUCCESS)
        {
            OSPFMAP_ERROR("%s %d: %s: ERROR:  Cannot initialize the OSPF Stack!!!\n",
                          __FILE__, __LINE__, routine_name);
            L7_assert(1);
            return(L7_FAILURE);
        }
...
L7_BOOL ospfMapMapIntfIsConfigurable(L7_uint32 intIfNum, ospfCfgCkt_t **pCfg)
{
    ...
    if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
    {
        if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(pOspfMapCfgData->ckt[index].configId)) == L7_FALSE)
        {
            ...
            */
            LOG_MSG("Error accessing OSPFMAP config data for interface %d in ospfMapMapIntfIsConfigurable.\n", intIfNum);
            return L7_FALSE;
        }
    }
...
static L7_RC_t ospfMapIntfCreate(L7_uint32 intIfNum) 
{
    ...
    for (i = 1; i < L7_OSPF_INTF_MAX_COUNT; i++ )
    {
        if (NIM_CONFIG_ID_IS_EQUAL(&pOspfMapCfgData->ckt[i].configId, &configId))
        {
            ospfMapMapTbl[intIfNum] = i;
            break;
        }
...
/*********************************************************************
*
* @purpose  Handle the transition of an interface to the active state.
*
* @param    intIfNum @b{(input)} internal interface number
*
...
static L7_RC_t ospfMapIntfActivate(L7_uint32 intIfNum)
{
    ...
    /* Do not respond to link state changes if interface has been
     * acquired by another component in a manner such that it should
     * not be able to participate independently in OSPF.
     */
    if ((L7_RT_BRIDGING_ROUTING_COEXISTANCE != L7_RT_BRIDGE_FIRST_THEN_ROUTE) ||
        (pOspfIntfInfo[intIfNum].ospfInitialized != L7_TRUE) ||
        (pOspfIntfInfo[intIfNum].intfAcquired == L7_TRUE)) {
        return L7_SUCCESS;
    }
    ...
    if (pCfg->metric == L7_OSPF_INTF_METRIC_NOT_CONFIGURED)
    {
        ...
        {
            return ospfMapIntfMetricApply(intIfNum, ifMetric);
        }
    }
    return L7_SUCCESS;
}
...
/*********************************************************************
* @purpose  Put interface events on the queue to be handled by the OSPF
*           mapping thread.
*
...
* @end
*********************************************************************/
L7_RC_t ospfMapIntfChangeCallback(L7_uint32 intIfNum, 
                                  L7_uint32 event, 
                                  NIM_CORRELATOR_t correlator)
{
    L7_RC_t rc;
    ...
    OSPFMAP_TRACE("%s %d: %s : intIfNum = %d, event = %d\n", 
                  __FILE__, __LINE__, "ospfMapIntfChangeCallback()", intIfNum, event);

    /* Make an early return for events we are not interested in */
    if (event !=  L7_CREATE &&
        event !=  L7_DELETE &&
        event !=  L7_ACTIVE &&
        event !=  L7_INACTIVE &&
        ...
        )
    {
      ...
      status.component    = L7_OSPF_MAP_COMPONENT_ID;
      status.event        = event;
    ...
    {
        OSPFMAP_ERROR("ospfMapIntfChangeCallback: osapiMessageSend failed\n");
    }
...
* @param    void        not used
* @param    intIfNum    @b{(input)} internal Interface number
* @param    event       @b{(input)} port event
...
* @notes    from insdie the IP Mapping layer. Since the OSPF mapping layer 
* @notes    is a "slave" to the IP Mapping layer, these events are 
* @notes    handled via its registration to ipMapRegisterRoutingIntfStateChange.
*
* @notes    OSPF acknowledges completion of event handling to NIM at the
*           end of this function. It is not necessary to defer acknowledgement
*           to NIM until the OSPF protocol engine has completed its processing
*           of the event. This is because events that require a deferred 
*           acknowledgement are also propagated to OSPF through the IP MAP
*           component and processed in ospfMapRoutingEventProcess(). IP MAP
*           holds off its acknowledgement to NIM until IP MAP has received
*           acknowledgement from its registered users, including OSPF.
*
*       
* @end
*********************************************************************/
                                       ...
                                       NIM_CORRELATOR_t correlator)
{
    ...
    static const char *routine_name = "ospfMapIntfEventProcess()";
    ...
    OSPFMAP_TRACE("%s %d: %s : intIfNum = %d, event = %d\n", 
                  __FILE__, __LINE__, routine_name, intIfNum, event);
    ...
    if (ospfMapReadyForIntfEvents() == L7_FALSE)
    {
        LOG_MSG("%s called outside of allowed phase\n", routine_name);
    }
    ...
    case (L7_CREATE):
        rc = ospfMapIntfCreate(intIfNum);
        break;

    case (L7_ATTACH):
        /* handled by ipmap callbacks */
        break;

    case (L7_DELETE):
        rc = ospfMapIntfDelete(intIfNum);
        break;

    case (L7_ACTIVE):
        rc = ospfMapIntfActivate(intIfNum);
        break;

    case (L7_INACTIVE):
        /* This case also covers administratively disabled and diagnostically 
         * disabled ports.
         */
        if (L7_RT_BRIDGING_ROUTING_COEXISTANCE  ==  L7_RT_BRIDGE_FIRST_THEN_ROUTE)
        {
            if (pOspfIntfInfo[intIfNum].ospfInitialized == L7_TRUE)
            {

                /* Do not respond to link state changes if interface has been
                   acquired by another component in a manner such that it should
                   not be able to participate independently in OSPF */
                if (pOspfIntfInfo[intIfNum].intfAcquired == L7_FALSE)
                {
    ...
    case (L7_LAG_ACQUIRE):     /* check if previously acquired */
        COMPONENT_ACQ_NONZEROMASK(pOspfIntfInfo[intIfNum].acquiredList, prevAcquired);

        /* update aquired list */
        COMPONENT_ACQ_SETMASKBIT(pOspfIntfInfo[intIfNum].acquiredList, L7_DOT3AD_COMPONENT_ID);
        ...
        if (prevAcquired == L7_FALSE)
        {
            ...
            /* Signal the interface as being unavailable to OSPF */
            pOspfIntfInfo[intIfNum].intfAcquired = L7_TRUE;
        }
        break;

    case (L7_PROBE_SETUP):         /* check if previously acquired */
        COMPONENT_ACQ_NONZEROMASK(pOspfIntfInfo[intIfNum].acquiredList, prevAcquired);

        /* update aquired list */
        COMPONENT_ACQ_SETMASKBIT(pOspfIntfInfo[intIfNum].acquiredList, L7_PORT_MIRROR_COMPONENT_ID);
        ...
        if (prevAcquired == L7_FALSE)
        {
            /* LAG members do not act as independently as router interfaces */
            if (pOspfIntfInfo[intIfNum].ospfInitialized == L7_TRUE)
            {
                ospfMapOspfIntfDown(intIfNum);
            }
    ...
    case (L7_LAG_RELEASE): 
        /* update aquired list */
        COMPONENT_ACQ_CLRMASKBIT(pOspfIntfInfo[intIfNum].acquiredList, L7_DOT3AD_COMPONENT_ID);

        /* check if currently acquired */
        COMPONENT_ACQ_NONZEROMASK(pOspfIntfInfo[intIfNum].acquiredList, currAcquired);
        ...
        if (currAcquired == L7_FALSE)
        {
            ...
            if ( ospfMapOspfInitialized() == L7_TRUE)
            {
                ...
                {
                    if (ospfMapIntfEffectiveAdminModeGet(intIfNum, &intfMode) == L7_SUCCESS)
                    {
            ...
            /* Signal the interface as being available to OSPF */
            pOspfIntfInfo[intIfNum].intfAcquired = L7_FALSE;
        }
    ...
    case (L7_PROBE_TEARDOWN):      
        /* update aquired list */
        COMPONENT_ACQ_CLRMASKBIT(pOspfIntfInfo[intIfNum].acquiredList, L7_PORT_MIRROR_COMPONENT_ID);
    ...
    case (L7_SPEED_CHANGE):
        ...
        if (ospfMapMapIntfIsConfigurable(intIfNum, &pCfg) == L7_TRUE)
        {
            ...
            if (pCfg->metric == L7_OSPF_INTF_METRIC_NOT_CONFIGURED)
            {
                if (ospfMapIntfMetricCalculate(intIfNum, &ifMetric) == L7_SUCCESS)
                {
                    rc = ospfMapIntfMetricApply(intIfNum, ifMetric);
                }
            }
        }
        break;
    ...
    default:
      /* no action required for events OSPF does not care about, so simply
       * return success.
       */
        /* status.response.reason = NIM_ERR_RC_IGNORED; */
        rc = L7_SUCCESS;
    }
                                           ...
                                           ASYNC_EVENT_NOTIFY_INFO_t *response)
{
    ...
    OSPFMAP_TRACE("%s %d: %s : intIfNum = %d, state = %d \n", __FILE__, 
                  __LINE__, routine_name, intIfNum, event);
                                           ...
                                           ASYNC_EVENT_NOTIFY_INFO_t *response)
{
    ...
    case L7_RTR_INTF_DISABLE_PENDING:
        if (ospfAdminMode == L7_ENABLE)
        {
            if (ospfMapOspfIntfExists(intIfNum) == L7_TRUE)
            {
                ...
                {
                    ospfMapIpAddressDeleteCallBack(intIfNum);
                }
            }
        }
        break;
    ...
    case L7_RTR_INTF_CONFIG_CLEAR:
        if (ospfMapMapIntfIsConfigurable(intIfNum, &pCfg) == L7_TRUE)
        {
            ospfIntfBuildDefaultConfigData(&pCfg->configId, pCfg);
        }
        break;
...
/*********************************************************************
* @purpose  This routine is called if the IP address of an interface
*           is deleted        
*
...
* @returns  L7_SUCCESS or L7_FALIURE
*
* @notes    This routine gets called when some event happens such that
*           the underlying OSPF function operate.  These events include:
*               - obtaining a valid IP address for the interface
*               - deleting a valid IP address for the interface
*               - obtaining a valid Router ID for the system
*       
* @end
*********************************************************************/
...
void ospfProcTask(void)
{
    ...
    while (L7_TRUE)
    {
        ...
        if (status == L7_SUCCESS)
        {
          ...
          switch (Message.msg)
            {
            case OSPF_INTF_EVENT:
                osapiSemaTake(ospfMapCtrl_g.cfgSema, L7_WAIT_FOREVER); 
                ospfMapIntfEventProcess(Message.type.intfEvent.intIfNum,
...
/*********************************************************************
* @purpose  Process OSPF admin mode events
*
...
void ospfMapAdminModeProcess(L7_uint32 mode)
{
  if (mode == L7_ENABLE)
  {
    (void) ospfMapOspfAdminModeEnable();
  }
  else if (mode == L7_DISABLE)
  {
    (void) ospfMapOspfAdminModeDisable();
  }
}
