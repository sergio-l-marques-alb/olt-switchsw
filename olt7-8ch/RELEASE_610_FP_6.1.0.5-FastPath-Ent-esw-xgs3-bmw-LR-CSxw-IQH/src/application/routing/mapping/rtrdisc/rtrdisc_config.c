...
void rtrDiscIntfBuildDefaultConfigData(rtrDiscIntfCfgData_t *pCfg)
{
  ...
  NIM_CONFIG_ID_COPY(&(pCfg->configId), &configIdNull);
#endif
...
{
  static const char *routine_name = "rtrDiscSave()";
  ...
  if (rtrDiscCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    {
      RTR_DISC_ERROR("%s %d: %s: Error on call to osapiFsWrite routine on config file %s\n",
                    __FILE__, __LINE__, routine_name, L7_RTR_DISC_FILENAME);
