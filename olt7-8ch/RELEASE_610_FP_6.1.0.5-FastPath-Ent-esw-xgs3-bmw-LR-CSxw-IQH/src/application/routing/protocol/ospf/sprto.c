...
static char __fileid[] = "$Header: /home/cvs/PR1003/PR1003/FASTPATH/RELEASE_610_FP_6.1.0.5-FastPath-Ent-esw-xgs3-bmw-LR-CSxw-IQH/src/application/routing/protocol/ospf/sprto.c,v 1.1 2011/04/18 17:10:29 mruas Exp $";
#endif
...
static void LsaOrgNssaBulk(t_Handle Id, t_S_AsExternalCfg *p_Entry);
...
e_Err RTO_Cleanup(t_Handle Id)
{
   ...
   while(e == E_OK)
   {
      ...
      AVLH_Browse(p_ARO->GroupMembLsaHl, (BROWSEFUNC)FlushLsa, (ulng)p_ARO);
      /* NSSA Change */
      AVLH_Browse(p_ARO->NssaLsaHl, (BROWSEFUNC)FlushLsa, (ulng)p_ARO);
      AVLH_Browse(p_ARO->LinkOpaqueLsaHl, (BROWSEFUNC)FlushLsa, (ulng)p_ARO);
      ...
      if(HL_FindFirst(p_RTO->RtbHl, (byte *)&VpnCos, (void *)&p_RTB) == E_OK)
      {
         ...
         while(er == E_OK)
         {
            ...
            if (RTB_Find(p_RTB, (t_RoutingTableKey *)&rtbKey, &p_Rte, 0) == E_OK)
            {
               if(p_RTO->Clbk.p_RoutingTableChanged && (p_Rte->isRejectRoute == TRUE))
               {
                  p_RTO->Clbk.p_RoutingTableChanged(p_RTO->Clbk.f_RtmAsyncEvent[0] ? 
                              ...
                              p_Rte->isRejectRoute);
                  AVL_Delete(p_RTB->RtbNtBt, (byte *)&p_Rte->DestinationId, NULL);
   ...
   while(e == E_OK)
   {
      AVL_Browse(p_RTB->RtbRtBt, RteDelete, (ulng)p_RTB);
      AVL_Browse(p_RTB->RtbNtBt, RteDelete, (ulng)p_RTB);
 ...
 e_Err RTO_Config(t_Handle RTO_Id, t_S_RouterCfg *p_Cfg)
{
   ...
   switch(p_Cfg->RouterStatus)
   {
      case  ROW_ACTIVE:
         ...
         if((p_RTO->Cfg.RouterStatus == ROW_CREATE_AND_WAIT) &&
            (!p_RTO->Cfg.AdminStat))
               id_state = 1;
      ...
      case  ROW_NOT_IN_SERVICE:
         ...
         p_RTO->LsdbOverload = FALSE;    /* LSDB not full */
      ...
      case  ROW_CHANGE:
         ...
         */
         ...
         if(p_RTO->Cfg.StagedPathComp && !p_RTO->Cfg.ItemsNumInTurn)
            p_RTO->Cfg.ItemsNumInTurn = ITEMS_IN_TURN_DEF;
   ...
   if((p_Cfg->RouterStatus == ROW_CREATE_AND_GO) || (id_state == 1))
   {
 ...
 * @end
 * ********************************************************************/
e_Err RTO_OpaqueCapability_Set(t_Handle RTO_Id, L7_uint32 opaqueEnabled)
{
  t_RTO   *p_RTO = (t_RTO *)RTO_Id;
...
e_Err RTO_ShowStatistics(t_Handle RTO_Id)
{
  ...
  /* Browse all AROs */
   ...
   while(e == E_OK)
   {
      ...
      AVLH_GetEntriesNmb(p_ARO->GroupMembLsaHl, &num[6]);      /* group-membership-LSAs */
      AVLH_GetEntriesNmb(p_ARO->LinkOpaqueLsaHl, &num[7]);     /* Local-Link-Opaque-LSAs */
      ...
      sprintf(text, "   Router %d, Network %d, NetSummary %d, ASBR Summary %d, LSAs Num: GroupMemb %d, Link Opaque %d, Area Opaque %d",
               (int)num[2], (int)num[3], (int)num[4],(int) num[5], (int)num[6], (int)num[7], (int)num[8]);
      XX_Print(text);
   ...
   while(e == E_OK)
   {
      AVL_GetEntriesNmb(p_RTB->RtbRtBt, &num[0]);
      AVL_GetEntriesNmb(p_RTB->RtbNtBt, &num[1]);

      RtbSize = 0;
      for(er = AVL_GetFirst(p_RTB->RtbRtBt, (void**)&p_Rte); er == E_OK; er = AVL_GetNext(p_RTB->RtbRtBt,  (void**)&p_Rte))
         RtbSize += sizeof(t_RoutingTableEntry) + p_Rte->PathNum*sizeof(t_OspfPathPrm) +
         p_Rte->ErNum*sizeof(t_ErPrm);

      for(er = AVL_GetFirst(p_RTB->RtbNtBt,  (void**)&p_Rte); er == E_OK; er = AVL_GetNext(p_RTB->RtbNtBt,  (void**)&p_Rte))
         RtbSize += sizeof(t_RoutingTableEntry) + p_Rte->PathNum*sizeof(t_OspfPathPrm) +
         p_Rte->ErNum*sizeof(t_ErPrm);

      sprintf(text,"   RTB:%ld, Routers Num:%d, Networks Num:%d, Size:%ld, TotalCompTime:%ld, AsExtCompTime:%ld, SummCompTime:%ld",
         p_RTB->VpnCos, (int)num[0], (int)num[1], RtbSize,
         p_RTB->RtComputeTime,
...
e_Err RtbPrint(t_Handle RTO_Id, t_VpnCos VpnCos)
{
   ...
   osapiSnprintf(text,200,"OSPF: Routing table:");
   RTO_TraceWrite(text);

   ntLookup = TRUE;
   e = AVL_GetFirst(p_RTB->RtbNtBt, (void *)&p_Rte);
   if(e != E_OK)
   {
   ...
   while(e == E_OK)
   {
      ...
      if(ntLookup)
      {
         e = AVL_GetNext(p_RTB->RtbNtBt, (void *)&p_Rte);
         if(e != E_OK)
         {
            ntLookup = FALSE;
            e = AVL_GetFirst(p_RTB->RtbRtBt, (void *)&p_Rte);
...
/*********************************************************************
 * @purpose          RTO LSA Database Print
 *
 *
 * @param RTO_Id     @b{(input)}  Router Id
 * @param LsType     @b{(input)}  LSA type
 *
 * @returns          n/a
 *
 * @notes
...
void RTO_LsaDbPrint(t_Handle RTO_Id, byte LsType)
{
   ...
   memset(&Lsa,0,sizeof(t_S_LsDbEntry));
   Lsa.LsdbType = LsType;

   osapiSnprintf(text,200,"OSPF: Router:");
   XX_Print(text);

   e = HL_GetFirst(p_RTO->AroHl, (void *)&p_ARO);
   while(e == E_OK)
   {
      sprintf(text, "Area: %ld", p_ARO->AreaId);
      XX_Print(text);

      while(ARO_LsaDatabase(p_ARO, &Lsa) == E_OK)
      {
         osapiSnprintf(text,200," Type: %s, LsaId: %d.%d.%d.%d, RouterId: %d.%d.%d.%d, SeqNum: %d.%d.%d.%d, Age: %x, Checksum %d, Options: %x, Content: %.25s",
            LsaTypes[Lsa.LsdbType],
            ADR_PRINT(Lsa.LsdbLsid),
            ADR_PRINT(Lsa.LsdbRouterId),
            ADR_PRINT(Lsa.LsdbSequence),
            Lsa.LsdbAge,
            Lsa.LsdbChecksum,
             Lsa.LsdbOptions,
...
{
    L7_uint32 i, msecSinceBoot, secSinceBoot, msecs;
...
e_Err 
RTO_CalcForwardingAddr(t_RTO * p_RTO, SP_IPADR *nextHop, SP_IPADR *fwdAddr)
{
  t_RoutingTableEntry *p_Rte;
  t_RoutingTableKey rtbKey;
  t_RTB *p_RTB;
   ...
   if (!replaceFormer && (p_RTO->ActiveLsas >= OSPF_MAX_LSAS))
   {
       if (!p_RTO->LsdbOverload)
       {
         L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_OSPF_MAP_COMPONENT_ID,
                 ...
                 "Link state database contains %u LSAs."
                 " When the OSPFv2 LSDB becomes full, OSPFv2 logs this "
                 "message. OSPFv2 reoriginates its router LSAs with the"
                 " metric of all non-stub links set to the maximum value"
                 " to encourage other routers to not compute routes through"
                 " the overloaded router.", OSPF_MAX_LSAS);
