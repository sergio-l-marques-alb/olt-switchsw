...
void dsApiCnfgrCommand(L7_CNFGR_CMD_DATA_t * pCmdData)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_ALERT, L7_DHCP_SNOOPING_COMPONENT_ID, 
                                 ...
                                 L7_CNFGR_ERR_RC_t *pReason)
{
  ...
  *pReason    = 0;
  ...
  if (nimRegisterIntfChange(L7_DHCP_SNOOPING_COMPONENT_ID,
                            dsIntfChangeCallback) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_DHCP_SNOOPING_COMPONENT_ID,
            "DHCP Snooping: Failed to register with NIM" );
