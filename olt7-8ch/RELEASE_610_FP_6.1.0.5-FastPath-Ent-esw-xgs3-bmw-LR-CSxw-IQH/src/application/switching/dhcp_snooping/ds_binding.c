                     ...
                     L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint32 intIfNum)
{
  ...
  if (pNode == L7_NULLPTR)
  {
    ...
    pNode->ipAddr = ipAddr;
    pNode->vlanId = vlanId;
    pNode->innerVlanId = innerVlanId;   /* PTin added */
                               ...
                               dsBindingType_t type )
{
  ...
  if ( pNode != L7_NULLPTR)
  {
    ...
    dsBinding->remLease = ((pNode->leaseTime + pNode->leaseStart)-(osapiUpTimeRaw()))/60;
                             ...
                             dhcpSnoopBinding_t *extBinding)
{
  ...
  extBinding->innerVlanId = binding->innerVlanId;   /* PTin added */
  ...
  {
    if ((binding->leaseStart + binding->leaseTime) > osapiUpTimeRaw())
    {
      /* time remaining on lease */
      extBinding->remLease = 
        (((binding->leaseStart + binding->leaseTime) - osapiUpTimeRaw()));
    }
  }
  return L7_SUCCESS;
}
...
L7_RC_t dsBindingLeaseSet(L7_enetMacAddr_t *macAddr, L7_uint32 leaseTime)
{
  ...
  binding->leaseStart = osapiUpTimeRaw();
  return L7_SUCCESS;
}
...
/*********************************************************************
* @purpose  Start the DHCP snooping lease timer. On expiration, see
...
/*********************************************************************
*
* @purpose  Stop the DHCP snooping lease timer. 
*
* @returns  L7_SUCCESS
*
* @notes    
...
L7_RC_t dsLeaseCheck(void)
{
  ...
  while (dsBindingTreeSearch(&macAddr, L7_MATCH_GETNEXT, &binding) == L7_SUCCESS)
  {
    ...
    else if (binding->bindingType == DS_BINDING_TENTATIVE)
    {
      ...
      if (binding->leaseStart + (120) < osapiUpTimeRaw())
      {
    ...
    /* dynamic binding */
    else if ((binding->leaseStart + (binding->leaseTime)) < osapiUpTimeRaw())
    {
      dsBindingRemove(&macAddr);
      dsInfo->debugStats.bindingsRemoved++;
    }
...
void dsBindingTableShow(void)
{
  ...
  while (dsBindingTreeSearch(&macAddr, L7_MATCH_GETNEXT, &binding) == L7_SUCCESS)
  {
    ...
    if (binding->bindingType == DS_BINDING_DYNAMIC)
    {
      leaseAge = osapiUpTimeRaw() - binding->leaseStart;
