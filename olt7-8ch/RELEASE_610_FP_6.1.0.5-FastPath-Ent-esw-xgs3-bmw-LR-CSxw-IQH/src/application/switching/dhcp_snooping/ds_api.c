...
L7_RC_t dsIntfBurstIntervalSet(L7_uint32 intIfNum, L7_uint32 burst_interval)
{
  ...
  if((burst_interval < L7_DS_BURST_INTERVAL_MIN) ||
                           ...
                           L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint32 intIfNum)
{
  ...
  rc = dsBindingAdd(DS_BINDING_STATIC, macAddr, ipAddr, vlanId, innerVlanId, intIfNum);
                      ...
                      L7_ushort16 vlanId)
{
  L7_BOOL found = L7_FALSE;
   ...
   if (dsBindingExists(macAddr, ipAddr, vlanId) == L7_SUCCESS)
     found = L7_TRUE;
                        ...
                        L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint32 intIfNum)
{
  ...
  {
    if (dsStaticBindingAdd(&macAddr, ipAddr, vlanId, innerVlanId, intIfNum) != L7_SUCCESS)
...
/*********************************************************************
* @purpose  Add a DHCP packet filter rule
*
* @param    adminMode   @b{(input)} IGMP Snooping admin mode
*           vlanId      @b{(input)} Vlan ID
*
* @returns  L7_SUCCESS
*
...
* @end
*********************************************************************/
L7_RC_t dsVlanFilterAdmin(L7_uint32 adminMode, L7_uint32 intf, L7_uint16 vlanId)
{
  /* Tell driver to enable IGMP Snooping */
  if (adminMode)
  {
    return dtlDhcpSnoopEnable(L7_AF_INET, intf, vlanId);
  }

  return dtlDhcpSnoopDisable(L7_AF_INET, intf, vlanId);
}
