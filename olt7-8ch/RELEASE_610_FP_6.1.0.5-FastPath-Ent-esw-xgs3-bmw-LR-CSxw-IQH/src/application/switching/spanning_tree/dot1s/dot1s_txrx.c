...
static 	L7_uchar8 temp[DOT1S_MAX_BPDU_SIZE];
						  ...
						  L7_uint32 intIfNum)
{
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_CRITICAL, L7_DOT1S_COMPONENT_ID,
  ...
  if (rc != L7_SUCCESS)
  {
	...
	dot1sPortStats[intIfNum].txDiscards++;
  }
  return rc;
						 ...
						 sysnet_pdu_info_t *bpduInfo)
{
  ...
  {
	LOG_MSG("dot1sBpduReceive is deregistered\n");
	return L7_FAILURE;
  }
  if (!(DOT1S_IS_READY))
  {
  ...
  if (DOT1S_DEBUG_COMMON(DOT1S_DEBUG_RCVD_BPDU))
  {
    ...
    DOT1S_DEBUG_MSG("%s: Port %d Received BPDU at %d (msec) \n",__FUNCTION__, 
                     bpduInfo->intIfNum,osapiTimeMillisecondsGet());
  }
   ...
   rc = dot1sTxRxHeaderTagRemove(bufHandle);
    ...
    if (rc == L7_SUCCESS)
	{
	  ...
	  if (L7_SUCCESS != rc)
	  {
		...
		LOG_MSG("dot1sBpduReceive(): Unable to put BPDU on Msg Queue\n");
	  }
	}
	else
	{
	  dot1sPortStats[intIfNum].rxDiscards++;
      LOG_MSG("dot1sBpduReceive(): cannot get buffer from buff pool\n");
	}
...
L7_RC_t dot1sTcnTx(DOT1S_PORT_COMMON_t *p)
{
  ...
  memset((void *)pdu, 0x00, DOT1S_MAX_BPDU_SIZE);
...
L7_RC_t dot1sConfigTx(DOT1S_PORT_COMMON_t *p)
{
  ...
  if (rc == L7_SUCCESS)
  {
	...
	dot1sPortStats[p->portNum].stpTx++;
  }
  return rc;
}
/*********************************************************************
...
L7_RC_t dot1sMstpTx(DOT1S_PORT_COMMON_t *p)
{
  ...
  switch (p->portInstInfo[DOT1S_CIST_INDEX].role)
  {
	...
	default:
	  ...
	  LOG_MSG("dot1sMstpTx(): CIST Role Disabled.Port(%d) Inst(%d)\n",p->portNum,DOT1S_CIST_INDEX);
	  return L7_FAILURE;
	  break;
  ...
  */
  ...
  if (bridge->ForceVersion < 3)
  {
	...
	if (rc == L7_SUCCESS)
	{
	  ...
	  dot1sPortStats[p->portNum].rstpTx++;
	}
  }
  else
  {
	...
	if (rc == L7_SUCCESS)
	{
	  ...
	  dot1sPortStats[p->portNum].mstpTx++;
	}
  }
  return rc;
}
...
L7_netBufHandle dot1sTxRxNetworkConvert(L7_netBufHandle bufHandle)
{
  ...
  memset((void *)unpaddedPdu, 0x00, DOT1S_MAX_BPDU_SIZE);
