...
* @returns  void
*
* @notes    LOG_MSG on error. Note the responsibility of this function is to
...
static L7_RC_t garpTimerListDeleteElement(garpTimerDescr_t *pTimer, garpTimerList_t *timerList)
{
	...
	if (pTimer == L7_NULL)
	{
		LOG_MSG("Error: pTimer is L7_NULL\n");
		return rc;
	}
	...
	if (timerList == L7_NULL)
	{
		LOG_MSG("Error:Timer Type (%d) of the timer (%x) invalid :timerList cannot be found \n", pTimer->timerType , pTimer);
		return rc;
	}
    ...
    if (timerList->head == L7_NULL) 
    {
        ...
        LOG_MSG("Error: attempting to delete from an empty timer list: list type %d\n",
                timerList->listType);
        return rc;
    }
...
L7_RC_t garpTimerFree(garpTimerDescr_t *pTimer)
{
    ...
    {
        LOG_MSG("Failed to free timer: timerType %d, port %d, vid %d, status %d, expiry %d\n",
                  ...
                  GARP_TIMER_TYPE_t timer_type)
{
    ...
    if (pTimer->timer_status != GARP_TIMER_UNDEFINED) 
    {
        ...
        {
            LOG_MSG("Attempt to update an unrecognized timer: status %d, type %d, port %d, vid %d\n",
