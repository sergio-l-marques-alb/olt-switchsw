...
L7_RC_t  garpJoinTimeSet(L7_uint32 intIfNum, L7_uint32 time)
{
    ...
    {
        return_value = garpTimeApply(intIfNum, time, GARP_JOIN_TIME);
    }
...
L7_RC_t garpLeaveTimeSet(L7_uint32 intIfNum, L7_uint32 time)
{
    ...
    if ((time >= GARP_LEAVE_TIMER_MIN_VAL) && 
        (time <= GARP_LEAVE_TIMER_MAX_VAL)) 
    {
        return_value = garpTimeApply(intIfNum, time, GARP_LEAVE_TIME);
    }
...
L7_RC_t garpLeaveAllTimeSet(L7_uint32 intIfNum, L7_uint32 time)
{
    ...
    if ((time >= GARP_LEAVEALL_TIMER_MIN_VAL) && 
        (time <= GARP_LEAVEALL_TIMER_MAX_VAL)) 
    {
        return_value = garpTimeApply(intIfNum, time, GARP_LEAVEALL_TIME);
    }
