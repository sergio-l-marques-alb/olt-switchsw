...
extern L7_RC_t isAttributeRegistered(Gid_machine *machine, L7_int32  last_gid_used);
...
static L7_BOOL gid_create_gid(Garp *application, L7_uint32 port_no, void **gid)
{
   ...
   *gid = L7_NULL;
   ...
   {
      LOG_MSG("gid_create_gid(): unable to allocate memory for application %d, port %x.\n",
   ...
   {
       LOG_MSG("gid_create_gid(): unable to allocate memory for machines on Port %x.\n",port_no);
  ...
  */
  if (gid->join_timer_running == L7_TRUE)
  {
	  if (garpTimerFree(&(gid->join_timer_instance)) != L7_SUCCESS)
		  LOG_MSG("%s: join timer instance free failure for port %d\n", 
  ...
  if (garpTimerFree(&(gid->leave_timer_instance)) != L7_SUCCESS)
	  LOG_MSG("%s: leave timer instance free failure for port %d\n", 
              __FUNCTION__,gid->port_no);
  }
...
static Gid *gid_remove_port(Gid *my_port)
{
    ...
    /*LOG_MSG("GID: Removing Port # %x from the ring.\n",my_port->port_no);*/
  ...
  L7_BOOL   returnVal = L7_FALSE;
    ...
    for (;;)
    {
        if (gidtt_machine_active(&check_port->machines[gid_index]))
        {
            ...
            {
                LOG_MSG("GID: All Ports are used.\n");
                returnVal = L7_FALSE;
                break;
            }
                            ...
                            == application->gid)
        {
            *found_index = gid_index;
            returnVal = L7_TRUE;
            LOG_MSG("GID: Ports at index %x is not used.\n",gid_index);
            break;
        }
    } /* end for */
   return returnVal;
