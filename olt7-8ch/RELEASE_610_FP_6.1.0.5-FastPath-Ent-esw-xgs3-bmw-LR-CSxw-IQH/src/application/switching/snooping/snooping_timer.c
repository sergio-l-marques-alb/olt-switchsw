  ...
  if (timerCBHandle == L7_NULL)
{
  ...
  else
  {
    ...
    {
      LOG_MSG("snoopTimerProcess: Invalid timer handle received %d", timerCBHandle);
      return;
    }
    /* Invoke all the expired timers here */
                      ...
                      L7_uint32 timerValue, snoop_cb_t *pSnoopCB)
{
  ...
  // PTin added
  // Do nothing for static entries
  if (snoopEntry!=L7_NULLPTR && snoopEntry->staticEntry)
  {
    return;
  ...
  if (timerType == SNOOP_GROUP_MEMBERSHIP)
  {
         ...
         == L7_NULLPTR)
    {
      /* We come here when a leave is received and the intIfnum is a 
         ...
         Simply return. */
      LOG_MSG("snoopTimerUpdate: Group membership timer not found \n");
      return;
    }

    if (pTimerData->grpTimer == L7_NULL)
    {
      LOG_MSG("snoopTimerUpdate: Group membership timer not running\n");
    }
    ...
    if (appTimerUpdate(snoopEntry->timerCB, &pTimerData->grpTimer, 
                       ...
                       != L7_SUCCESS)
    {
      LOG_MSG("snoopTimerUpdate: Failed to update group membership timer \n");
    }
  ...
  else if (timerType == SNOOP_MAX_RESPONSE)
  {
    ...
    if (snoopGeneralQuerySend(intIfNum, vlanId, pSnoopCB, 
                              SNOOP_LEAVE_PROCESS_QUERY) != L7_SUCCESS)
    {
      LOG_MSG("snoopTimerUpdate: Failed to send general query \n");
    }
         SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
  ...
  else if (timerType == SNOOP_MCAST_ROUTER_ATTACHED)
  {
    ...
    if (pmrtrTimerData != L7_NULLPTR)
    {
      ...
      {
        LOG_MSG("snoopTimerUpdate: Failed to update mrtr timer \n");
      }
      SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
    ...
    else  /* Have a valid mrtr timer */
    {
      ...
      if (snoopTimerStart(L7_NULL, intIfNum, vlanId, timerType, pSnoopCB) 
                            != L7_SUCCESS)
      {
        LOG_MSG("snoopTimerUpdate: Failed to re-start mrtr timer\n", timerValue);
      }
                    ...
                    snoop_cb_t *pSnoopCB)
{
  ...
  if(timerType == SNOOP_GROUP_MEMBERSHIP)
  {
    ...
    if (SLLDelete(&snoopEntry->ll_timerList, (L7_sll_member_t *)&snoopTimerData)
                  != L7_SUCCESS)
    {
      LOG_MSG("snoopTimerStop: Failed to delete group timer node\n");
    }
  }
  else if (timerType == SNOOP_MCAST_ROUTER_ATTACHED)
  {
      ...
      if (pSnoopMrtrTimerData->mrtrTimer != L7_NULL)
      {
        if (snoopTimerDestroy(pSnoopCB->snoopExec->timerCB,
                               &pSnoopMrtrTimerData->mrtrTimer, 
                                ...
                                != L7_SUCCESS)
        {
           LOG_MSG("snoopTimerStop: Failed to stop mrtr timer \n");
        }
      } /* timer running ? */
      if (avlDeleteEntry(&pSnoopCB->snoopMrtrTimerAvlTree, pSnoopMrtrTimerData)
                        == L7_NULL)
      {
        LOG_MSG("snoopTimerStop: Failed to delete mrtr timer node \n");
      }
    }/* Valid mrtr timer */
  }
}
...

{
  ...
  if(timerType == SNOOP_GROUP_MEMBERSHIP)
  {
    ...
    timerValue = snoopCheckPrecedenceParamGet(vlanId,intIfNum, 
     ...
     {
       LOG_MSG("snoopTimerStart: Could not start timer. Insufficient memory. \n");
       return L7_FAILURE;
     }
            ...
            == 0)
     {
       ...
       LOG_MSG("snoopTimerStart: Could not get the handle node to store the timer data.\n");
       return L7_FAILURE;
     }
     ...
     if(pTimerData->grpTimer == NULL)
     {
        /* Free the previously allocated bufferpool */
        bufferPoolFree(pSnoopCB->snoopExec->snoopTimerBufferPoolId, (L7_uchar8 *)pTimerData);
        LOG_MSG("snoopTimerStart: Could not Start the Group timer.\n");
        return L7_FAILURE;
     }
     ...
     {
       /* Free the previously allocated bufferpool */
       LOG_MSG("snoopTimerStart: Could not add new timer data node \n");
       if (appTimerDelete(snoopEntry->timerCB, pTimerData->grpTimer) != L7_SUCCESS)
       {
         LOG_MSG("snoopTimerStart: Failed to delete timer \n");
       }
       bufferPoolFree(pSnoopCB->snoopExec->snoopTimerBufferPoolId, (L7_uchar8 *)pTimerData);
       return L7_FAILURE;
     } 
  ...
  else if (timerType == SNOOP_MCAST_ROUTER_ATTACHED)
  {
    ...
    if (pData != L7_NULLPTR)
    {
      LOG_MSG("snoopTimerStart: Mrtr Timer node already present \n");
      return L7_FAILURE;
    }
    ...
    if (pData == L7_NULLPTR)
    {
      LOG_MSG("snoopTimerStart: Mrtr Timer node not found after inserting \n");
      return L7_FAILURE;
    }
           ...
           == 0)
    {
      ...
      LOG_MSG("snoopTimerStart: Could not get the handle node to store the mrtr timer data.\n");
      return L7_FAILURE;
    }
    ...
    if(pData->mrtrTimer == NULL)
    {
      /* Free the previously allocated tree node */
      (void)avlDeleteEntry(&pSnoopCB->snoopMrtrTimerAvlTree, pData);
      LOG_MSG("snoopTimerStart: Could not Start the mrtr timer.\n");
      return L7_FAILURE;
    }
    SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
...
void snoopMrtrExpiry(void *param)
{
  ...
  {
    LOG_MSG("snoop_mrtr_expiry: Failed to retrieve handle \n");
    return;
  }
  ...
  /* Remove the interface from the snoopEntry only if static mrouter is 
   ...
   {
     LOG_MSG("snoop_group_membership_expiry: Failed to remove interface from snoop entry \n");
   }
}
...
void snoopGroupMembershipExpiry(void *param)
{
  ...
  if (pTimerData == L7_NULLPTR)
  {
    LOG_MSG("snoop_group_membership_expiry: Failed to retrieve handle \n");
    return;
  }

  // PTin added
  // Do nothing for static entries
  if (pTimerData->snoopEntry->staticEntry)
  {
    return;
  }
  // PTin end
  ...
  if ((pSnoopCB = snoopCBGet(pTimerData->snoopEntry->snoopInfoDataKey.family)) 
                             == L7_NULLPTR)
  {
    LOG_MSG("snoopGroupMembershipExpiry: Invalid Control Block \n");
    return;
  }
  ...
  LOG_TRACE(LOG_CTX_PTIN_IGMP,"Grp Timer Expiry - Vlan %d Mac %02X:%02X:%02X:%02X:%02X:%02X intIfNum %d",
             shortVid,
             mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],
             pTimerData->intIfNum);
...
L7_RC_t snoopTimerDataDestroy (L7_sll_member_t *ll_member)
{
  ...
  if (pTimerData->grpTimer != L7_NULL)
  {
    ...
    if (snoopTimerDestroy(pTimerData->snoopEntry->timerCB,
                          ...
                          != L7_SUCCESS)
    {
      LOG_MSG("snoop_timer_data_destroy: Failed to destroy timer \n");
    }
  }
                               ...
                               L7_uint32 interval)
{
  ...
  {
    LOG_MSG("snoopQuerierTimerStart: Invalid Data\n");
    return L7_FAILURE;
  }
  /* Get CB */
  ...
  if (timerType == SNOOP_QUERIER_QUERIER_EXPIRY_TIMER)
  {
    if (snoopOperEntry->snoopQuerierInfo.snoopQuerierTimerData.querierExpiryTimer 
                                         != L7_NULL)
    {
      LOG_MSG("snoopQuerierTimerStart: Timer already running\n");
    }
    ...
    {
      /* Free the previously allocated bufferpool */
      LOG_MSG("snoopQuerierTimerStart: Could not get the handle node to store the expiry timer data.\n");
      return L7_FAILURE;
    }
    ...
    if ((snoopOperEntry->snoopQuerierInfo.snoopQuerierTimerData.querierExpiryTimer
          ...
          == L7_NULL)
    {
      LOG_MSG("snoopQuerierTimerStart: could not start Querier Expiry timer for vlan %d family %d\n",
              snoopOperEntry->vlanId, pSnoopCB->family);
    }
    SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
                "Querier Timer Start - Vlan %d interval %d", \
  ...
  else if (timerType == SNOOP_QUERIER_QUERY_INTERVAL_TIMER)
  {
    ...
    {
      LOG_MSG("snoopQuerierTimerStart: could not start Query interval for vlan %d family %d\n",
              snoopOperEntry->vlanId, pSnoopCB->family);
    }
    SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
                "Query Timer Start - Vlan %d interval %d", \
                 snoopOperEntry->vlanId, interval);
  }
  else
  {
    LOG_MSG("snoopQuerierTimerStart: Invalid timer\n");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
                                ...
                                L7_uint32 interval)
{
  ...
  if (snoopOperEntry == L7_NULLPTR)
  {
    LOG_MSG("snoopQuerierTimerUpdate: Invalid Data\n");
    return L7_FAILURE;
  }
  /* Get CB */
  pSnoopCB = (snoop_cb_t *)snoopOperEntry->cbHandle;
  if (timerType == SNOOP_QUERIER_QUERIER_EXPIRY_TIMER)
  {
    if (snoopOperEntry->snoopQuerierInfo.snoopQuerierTimerData.querierExpiryTimer 
                                         == L7_NULL)
    {
      LOG_MSG("snoopQuerierTimerUpdate: Timer not running\n");
    }
    ...
    {
      LOG_MSG("snoopQuerierTimerUpdate: Failed to update Querier Expiry timer for vlan %d family %d\n",
              snoopOperEntry->vlanId, pSnoopCB->family);
    }
    SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
                "Querier Timer Update - Vlan %d interval %d", \
                 snoopOperEntry->vlanId, interval);
  }
  else if (timerType == SNOOP_QUERIER_QUERY_INTERVAL_TIMER)
  {
    ...
    {
      LOG_MSG("snoopQuerierTimerUpdate: Failed to update Query interval for vlan %d family %d\n",
              snoopOperEntry->vlanId, pSnoopCB->family);
    }
    SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
                "Query Timer Update - Vlan %d interval %d", \
                 snoopOperEntry->vlanId, interval);
  }
  else
  {
    LOG_MSG("snoopQuerierTimerUpdate: Invalid timer\n");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
                              ...
                              snoopQuerierTimerType_t timerType)
{
  ...
  if (snoopOperEntry == L7_NULLPTR)
  {
    LOG_MSG("snoopQuerierTimerStop: Invalid Data\n");
    return L7_FAILURE;
  }
  ...
  if (timerType == SNOOP_QUERIER_QUERIER_EXPIRY_TIMER)
  {
    ...
    if (snoopTimerDestroy(pSnoopCB->snoopExec->timerCB,
                          ...
                          != L7_SUCCESS)
    {
      LOG_MSG("snoopQuerierTimerStop: Failed to stop Querier Expiry timer for vlan %d family %d",
              snoopOperEntry->vlanId, pSnoopCB->family);
      return L7_FAILURE;
    }
  }
  else if (timerType == SNOOP_QUERIER_QUERY_INTERVAL_TIMER)
  {
    ...
    if (snoopTimerDestroy(pSnoopCB->snoopExec->timerCB,
                          ...
                          != L7_SUCCESS)
    {
      LOG_MSG("snoopQuerierTimerStop: Failed to stop Query interval timer for vlan %d family %d",
              snoopOperEntry->vlanId, pSnoopCB->family);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_MSG("snoopQuerierTimerStop: Invalid timer\n");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
...
void snoopQuerierExpiry(void *param)
{
  ...
  {
    LOG_MSG("snoopQuerierExpiry: Failed to retrieve handle \n");
    return;
  }
...
void snoopQuerierQueryExpiry(void *param)
{
  ...
  if (pSnoopOperEntry == L7_NULLPTR)
  {
    LOG_MSG("snoopQuerierQueryExpiry: Failed to retrieve handle \n");
    return;
  }
  ...
  SNOOP_TRACE(SNOOP_DEBUG_TIMER, pSnoopCB->family, \
              "Periodic Query Timer Expiry. Vlan %d", \
              pSnoopOperEntry->vlanId);
