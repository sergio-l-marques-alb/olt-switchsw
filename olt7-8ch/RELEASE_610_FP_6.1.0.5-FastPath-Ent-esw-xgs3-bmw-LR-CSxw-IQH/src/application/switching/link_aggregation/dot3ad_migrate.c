...
dot3adBuildConfigDataV2 (dot3adCfgV2_t * pCfgV2)
{
  ...
  if (simGetSystemIPMacType () == L7_SYSMAC_BIA)
    simGetSystemIPBurnedInMac (pCfgV2->cfg.dot3adSystem.actorSys.addr);
  else
    simGetSystemIPLocalAdminMac (pCfgV2->cfg.dot3adSystem.actorSys.addr);
...
dot3adBuildConfigDataV3 (dot3adCfgV3_t * pCfgV3)
{
  ...
  if (simGetSystemIPMacType() == L7_SYSMAC_BIA)
    simGetSystemIPBurnedInMac(pCfgV3->cfg.dot3adSystem.actorSys.addr);
  else
    simGetSystemIPLocalAdminMac(pCfgV3->cfg.dot3adSystem.actorSys.addr);
...
dot3adBuildConfigDataV4 (dot3adCfgV4_t * pCfgV4)
{
  ...
  if (simGetSystemIPMacType() == L7_SYSMAC_BIA)
    simGetSystemIPBurnedInMac(pCfgV4->cfg.dot3adSystem.actorSys.addr);
  else
    simGetSystemIPLocalAdminMac(pCfgV4->cfg.dot3adSystem.actorSys.addr);
