...
L7_RC_t fdbSysMacDelEntry(L7_uchar8 *macAddr, L7_uint32 vlanId, L7_uint32 intIfNum, L7_uchar8 entryType)
{
  ...
  fdbMsg.reflect_to_hw = L7_FALSE;    /* PTin added */
...
L7_RC_t fdbAddEntry(fdbMeberInfo_t *fdbMemberInfo)
{
  ...
  fdbMsg.msgsType  = FDB_ADD;
  fdbMsg.reflect_to_hw = L7_TRUE;   /* PTin added */
...
//    }
    ...
    {
      printf("osapiMessageSend failed\r\n");
    }
  }
  else
  {
...
void fdbTask( void )
{
  ...
  L7_uint32 fdbMaxEntries, fdbEntries;

  fdbMaxTableEntriesGet(&fdbMaxEntries);
  ...
  else
  {
    while (1)
    {
      ...
      case FDB_ADD:
          /* PTin added */
          fdbEntries=fdbActiveEntries();

          /* If table is full, do nothing */
          if (fdbEntries>=fdbMaxEntries)
          {
            //printf("%s(%d) Table is full\r\n",__FUNCTION__,__LINE__);
            break;
          }

          if (message.reflect_to_hw)
          {
        ...
        case FDB_DEL:
          ...
          if (message.reflect_to_hw)
          {
            if (dtlFdbMacAddrDelete(message.mac_addr,
                                    message.intIfNum,
                                    message.vlanId,
                                    message.entryType,
                                    L7_NULL)!=L7_SUCCESS)
