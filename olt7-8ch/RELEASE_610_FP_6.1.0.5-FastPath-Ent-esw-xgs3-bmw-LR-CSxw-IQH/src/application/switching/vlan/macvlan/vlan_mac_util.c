...
L7_RC_t vlanMacAddApply(L7_enetMacAddr_t mac, L7_uint32 vlanId)
{
    ...
    if(rc != L7_SUCCESS)
    {
        L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_VLAN_MAC_COMPONENT_ID, "vlanMacAddApply: Failed to add an entry. "
...
void vlanMacDeleteApply(L7_enetMacAddr_t mac, L7_uint32 vlanId)
{
    ...
    if (nodep && nodep->vlanId == vlanId)
    {
        ...
        {
			L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_VLAN_MAC_COMPONENT_ID, "vlanMacDeleteApply: Unable to delete an "
            ...
            while (nodep)
            {
              if(nodep->inUse == (L7_uchar8)L7_FALSE)
              {
                if (nodep->vlanId == vlanId)
                {
                  ...
                  {
                    delnode = nodep;
                       L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_VLAN_MAC_COMPONENT_ID, "vlanMacVlanChangeCallback: Failed to"
                ...
                if(nodep->inUse == (L7_uchar8)L7_TRUE)
                {
                  if (nodep->vlanId == vlanId)
                  {
                    if(dtlVlanMacEntryDelete(nodep->mac, nodep->vlanId) != L7_SUCCESS)
                             L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_VLAN_MAC_COMPONENT_ID, "vlanMacVlanChangeCallback: "
