...
L7_RC_t dsDistillerInit(void)
{
  ...
  {
    LOG_MSG("%s: Unable to allocate Distiller control memory\n", routine_name);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Unable to create OLL AVL tree semaphore\n", routine_name);
    return L7_FAILURE;
  }
...
L7_RC_t dsDistillerReset(void)
{
            ...
            avlSearchLVL7(pDsDstlCtrl->pOLL, pNode, AVL_NEXT)) != L7_NULLPTR))
  {
    ...
    {
      LOG_MSG("dsDistillerReset: Unable to delete TLV instances for policy %u, "
              "intf %u\n", pNode->key.policyIndex, pNode->intIfNum);
...
static L7_RC_t dsDistillerCtrlInit(void)
{
  ...
  for (i = 1, key = 1; i <= L7_DIFFSERV_POLICY_LIM; i++)
  {
    ...
    for (j = 1; j <= L7_DIFFSERV_INST_PER_POLICY_LIM; j++)
    {
      ...
      if (key > keymax)
      {
        LOG_MSG("dsDistillerInit: Key-to-index table exceeds maximum (%u)\n",
                keymax);
        return L7_FAILURE;
      }
         ...
         (nextIntf == intIfNum))
  {
                                 ...
                                 &policyIndex) == L7_SUCCESS)
    {
                                           ...
                                           L7_FALSE)) != L7_SUCCESS)
      {
        ...
        LOG_MSG("dsDistillerIntfEventNotify: error (rc=%u) evaluating policy "
                "index %u, intf %u, direction %u\n", rc2, policyIndex, 
                intIfNum, ifDirection);
        rc = L7_FAILURE;
      }
    }
  } /* endwhile */
  ...
  while (1)
  {
        ...
        (evalIntIfNum == intIfNum))
    {
        ...
        if (linkState == L7_ACTIVE)
        {
              ...
              (attachIsValid != L7_TRUE))
          {
            ...
            L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                    ...
                    "policy \'%s\', intIfNum %u, direction %u."
      ...
      DIFFSERV_PRT(DIFFSERV_MSGLVL_LO_2,
                   "DiffServ: Eval policy=%u intf=%u reissue=%u tearDown=%u buildUp=%u\n",
                   policyIndex, intIfNum, reissueTlv, needTearDown, needBuildUp);
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_MSG("%s: build up or tear down error for "
                "policy %u, intIfNum %u, direction %u\n",
                __FUNCTION__, policyIndex, intIfNum, ifDirection);
        break;
      }
