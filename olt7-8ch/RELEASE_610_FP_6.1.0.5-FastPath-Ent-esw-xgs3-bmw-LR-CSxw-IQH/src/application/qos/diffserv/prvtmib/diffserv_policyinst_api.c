  ...
  do
  {
    ...
    if (rc == L7_SUCCESS)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        {
          LOG_MSG("diffServPolicyInstCreate: Error deleting row after create "
...
L7_RC_t diffServPolicyInstDeleteAll(L7_uint32 policyIndex)
{
         ...
         (nextPolicyIndex == policyIndex))
  {
    ...
    {
      LOG_MSG("%s: could not delete instance %u from policy index %u\n", 
                              ...
                              L7_BOOL   bypassEval)
{
  ...
  do
  {
    ...
    else
    {
      ...
      {
        LOG_MSG("diffServPolicyInstDelete: couldn't delete row AVL node for "
  ...
  *ppRow = pRow;
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
    {
      ...
      {
        pRow->mib.rowStatus = restoreStatus;
        LOG_MSG("diffServPolicyInstObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
    }
    else
...
void dsmibPolicyInstIndexNextUpdate(L7_uint32 policyIndex)
{
  ...
  if (pRow == L7_NULLPTR)
  {
    LOG_MSG("dsmibPolicyInstIndexNextUpdate: ERROR -- could not find "
...
void dsmibPolicyInstIndexNextUndo(L7_uint32 policyIndex)
{
  ...
  if (pRow == L7_NULLPTR)
  {  
    LOG_MSG("dsmibPolicyInstIndexNextUndo: ERROR -- could not find "
            "policy row for index %u\n", policyIndex);
    return;                                     /* ERROR -- should not occur */
  }
  ...
  {  
    LOG_MSG("dsmibPolicyInstIndexNextUndo: WARNING -- attempting to decrement "
