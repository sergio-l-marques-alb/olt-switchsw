  ...
  do
  {
    ...
    if (rc == L7_SUCCESS)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        {
          LOG_MSG("diffServPolicyAttrCreate: Error deleting row after create "
                                    ...
                                    L7_uint32 policyInstIndex)
{
         ...
         (nextPolicyInstIndex == policyInstIndex))
  {
    ...
    if (dsmibPolicyAttrDelete(policyIndex, policyInstIndex, policyAttrIndex, L7_TRUE)
        != L7_SUCCESS)
    {
      LOG_MSG("%s: could not delete attribute %u from policy instance %u,%u\n", 
                              ...
                              L7_BOOL   bypassEval)
{
  ...
  do
  {
    ...
    else
    {
      ...
      {
        LOG_MSG("diffServPolicyAttrDelete: couldn't delete row AVL node for attribute "
                "%u, instance %u, in policy \'%s\'\n", 
                policyAttrIndex, policyInstIndex, (char *)policyName);
      }
      else
      {
        LOG_MSG("diffServPolicyAttrDelete: couldn't delete row AVL node for "
                "indexes %u,%u,%u\n", policyIndex, policyInstIndex, 
                policyAttrIndex);
      }
      rc = L7_FAILURE;
  ...
  *ppRow = pRow;
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
    {
      ...
      {
        pRow->mib.rowStatus = restoreStatus;
        LOG_MSG("diffServPolicyAttrObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
    }
    else
                                    ...
                                    L7_uint32 policyInstIndex)
{
  ...
  if (pRow == L7_NULLPTR)
  {
    LOG_MSG("dsmibPolicyAttrIndexNextUpdate: ERROR -- could not find "
                                  ...
                                  L7_uint32 policyInstIndex)
{
  ...
  if (pRow == L7_NULLPTR)
  {  
    LOG_MSG("%s: ERROR -- could not find policy instance row for indexes %u,%u\n", 
  ...
  {  
    LOG_MSG("%s: WARNING -- attempting to decrement an indexNext value "
