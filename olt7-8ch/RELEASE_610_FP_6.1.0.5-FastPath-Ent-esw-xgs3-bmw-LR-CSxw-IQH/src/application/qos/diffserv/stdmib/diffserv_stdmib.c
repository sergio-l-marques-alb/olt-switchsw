...
L7_RC_t diffServStandardMibInit(void)
{
  ...
  {
    LOG_MSG("%s: Error creating standard MIB semaphore\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("%s: Error creating standard MIB AVL trees\n", __FUNCTION__);
    return L7_FAILURE;
  }
#endif
...
void diffServStandardMibFini(void)
{
  ...
  if (dsStdMibSemId != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("%s: Error freeing standard MIB index table semaphore (0x%8.8x)\n",
