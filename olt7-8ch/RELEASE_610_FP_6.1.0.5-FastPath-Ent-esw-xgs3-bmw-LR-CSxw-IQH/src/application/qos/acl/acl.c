...
{ 
  ...
  "Vid", "Vids", "Vide", "V2id", "V2ids", "V2ide"
                             ...
                             L7_tlvHandle_t tlvHandle, aclId_t *pAclIdent)
{
  ...
  if (L7_NULLPTR == pAclIdent)
  {
    ...
    if (totalRuleCount > L7_ACL_MAX_RULES_PER_MULTILIST)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FLEX_QOS_ACL_COMPONENT_ID, 
              "Total number of ACL rules (%u) exceeds max (%u) on intf %u %s. "
  ...
  /* only write the implicit 'deny all' rule into TLV if a 'match every' type
   * of rule has not already been used for this set of ACLs
   *
   * if any MAC ACL was used, this is treated as a deny all MAC (i.e. all 
...
L7_RC_t aclBuildTLVDelete(L7_uint32 intf, L7_uint32 vlan, L7_uint32 aclDir)
{
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    /* update ACL correlator table for withdrawn correlators */
    for (i = 0; i < pDirInfo->tlvCorrCount; i++)
    {
                           ...
                           L7_uint32 vlan, L7_tlvHandle_t tlvHandle)
{
  ...
  *ruleCount = 0;
  ...
  /* do not process any IP ACL rules if any IP or MAC 'match every' rules 
  ...
  while ((rc == L7_SUCCESS) && (needDenyAll == L7_TRUE))
  {
    ...
    if ( (p->configMask & cfgMask) != 0 )
    {
      ...
      {
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_ASSIGN_QUEUEID) == L7_TRUE)
        { 
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_REDIRECT_INTF) == L7_TRUE)
        { 
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_MIRROR_INTF) == L7_TRUE)
        { 
      ...
      if ( (p->every == L7_TRUE) ||
           (aclIsTlvMaskedRuleValid(aclnum,rulenum) != L7_TRUE) )
      { 
      ...
      {
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_DSTIP) == L7_TRUE)
        { 
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_DSTIPV6) == L7_TRUE)
        { 
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_DSTPORT) == L7_TRUE)
        { 
        ...
        if ( (aclImpIsFieldConfigured(aclnum,rulenum,ACL_DSTSTARTPORT) == L7_TRUE) &&
             (aclImpIsFieldConfigured(aclnum,rulenum,ACL_DSTENDPORT) == L7_TRUE) )
        { 
          memset(&dstL4Port, 0, sizeof(dstL4Port));
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_FLOWLBLV6) == L7_TRUE)
        { 
        ...
        if ( (aclImpIsFieldConfigured(aclnum,rulenum,ACL_IPPREC) == L7_TRUE) ||
             (aclImpIsFieldConfigured(aclnum,rulenum,ACL_IPTOS ) == L7_TRUE) ||
             (aclImpIsFieldConfigured(aclnum,rulenum,ACL_IPDSCP) == L7_TRUE) )
        { 
          /* only include field if nonzero mask is specified */
          if (p->tosmask != 0)
          {
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_PROTOCOL) == L7_TRUE)
        { 
          /* only include field if nonzero mask is specified */
          if (p->protmask != 0)
          {
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_SRCIP) == L7_TRUE)
        { 
          /* only include field if nonzero mask is specified */
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_SRCIPV6) == L7_TRUE)
        { 
          /* only include field if nonzero prefix length is specified */
        ...
        if (aclImpIsFieldConfigured(aclnum,rulenum,ACL_SRCPORT) == L7_TRUE)
        { 
        ...
        if ( (aclImpIsFieldConfigured(aclnum,rulenum,ACL_SRCSTARTPORT) == L7_TRUE) &&
             (aclImpIsFieldConfigured(aclnum,rulenum,ACL_SRCENDPORT) == L7_TRUE) )
        { 
          memset(&srcL4Port, 0, sizeof(srcL4Port));
...
L7_BOOL aclIsTlvMaskedRuleValid(L7_uint32 aclnum, L7_uint32 rulenum) 
{
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_DSTIP) == L7_TRUE)
  {
    if (p->dstIp.v4.mask == 0)
    {
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_DSTIPV6) == L7_TRUE)
  {
    if (p->dstIp.v6.in6PrefixLen == 0)
    {
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_IPTOS) == L7_TRUE)
  {
    if (p->tosmask == 0)
    {
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_PROTOCOL) == L7_TRUE)
  {
    if (p->protmask == 0)
    {
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_SRCIP) == L7_TRUE)
  {
    if (p->srcIp.v4.mask == 0)
    {
  ...
  if (aclIsRuleFieldConfigured(acl_ptr,rulenum,ACL_SRCIPV6) == L7_TRUE)
  {
    if (p->srcIp.v6.in6PrefixLen == 0)
    {
  ...
  while ((pAcl = avlSearchLVL7(pTree, &aclnum, AVL_NEXT)) != L7_NULLPTR)
  {
    ...
    while ((pRule != L7_NULLPTR) && (pRule->last != L7_TRUE))
    {
      ...
      if ((rc == L7_SUCCESS) && (ruleIntIfNum == intIfNum))
      {
        ...
        {
          LOG_MSG("ACL %u: Error updating device with latest configuration\n", 
                  aclnum);
        }
                            ...
                            L7_ACL_TYPE_t aclType, L7_uint32 aclId)
{
  ...
  if (aclImpIsInterfaceInUse(intIfNum, aclDir) == L7_TRUE)
  {
    ...
    {
      LOG_MSG("Removed %s ACL %u from intf %u %s, but unable to "
              "apply remaining ACLs to device\n", 
              acl_type_str[aclType], aclId, intIfNum,
                                ...
                                L7_uint32 seqNum )
{
  ...
  return( aclImpVlanDirAdd( vlanNum, aclDir, aclType, aclnum, seqNum ) );
}
                             ...
                             L7_uint32     aclId )
{
  ...
  if ( L7_TRUE == aclImpIsVlanInUse( vlanNum, aclDir ) )
  {
    ...
    {
      LOG_MSG("Removed %s ACL %u from vlan %u %s, but unable to "
              "apply remaining ACLs to device\n",
...
/*********************************************************************
*
* @purpose  Ask registered applications if it is okay to delete an ACL.
*
* @param    aclnum      ACL associated with the event
*
...
*
*********************************************************************/
L7_RC_t aclImpDeleteApprovalQuery(L7_uint32 aclnum)
{
  L7_uint32             i, maxEnt;
  aclCallbackEntry_t    *pTableStart, *pCbEntry;
   ...
   *       strange about this ACL, so allow it to be deleted without
   *       querying the registrants.
   */
  if (aclImpCommonNameStringGet(aclnum, &aclType, aclName) != L7_SUCCESS)
    return L7_SUCCESS;                  /* see note above! */

  /* initiate ACL delete approval callbacks */
  pTableStart = pAclCallbackInfo_g->table[ACL_CALLBACK_TABLE_ID_DELETE_APPROVAL].pStart;
  maxEnt      = pAclCallbackInfo_g->table[ACL_CALLBACK_TABLE_ID_DELETE_APPROVAL].maxEntries;
  ...
  for (i = 0, pCbEntry = pTableStart; i < maxEnt; i++, pCbEntry++)
  {
    if (L7_NULLPTR != pCbEntry->funcPtr)
    {
       ...
       * prevent the ACL from being deleted.
       */
      funcPtr = (L7_FUNCPTR_t) pCbEntry->funcPtr;
      rc = (funcPtr)(aclType, aclnum, aclName);
      if (L7_SUCCESS != rc)
      {
        return L7_FAILURE;
      }
...
L7_RC_t aclCallbackInit(void)
{
  ...
  {
    LOG_MSG("%s: Unable to allocate ACL callback info structure\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  if (pMem == L7_NULLPTR)
  {
    LOG_MSG("%s: Unable to allocate ACL Route Filter callback table\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  if (pMem == L7_NULLPTR)
  {
    LOG_MSG("%s: Unable to allocate ACL Notify Event callback table\n", __FUNCTION__);
    return L7_FAILURE;
  }
  memset(pMem, 0, memSize);
  ...
  /* malloc space for the ACL Delete Approval callback registration table */
  maxEnt = ACL_CALLBACK_REGISTRANTS_MAX_DELETE_APPROVAL;
  memSize = maxEnt * sizeof(aclCallbackEntry_t); 
  pMem = osapiMalloc(L7_FLEX_QOS_ACL_COMPONENT_ID, memSize);
  if (pMem == L7_NULLPTR)
  {
    LOG_MSG("%s: Unable to allocate ACL Delete Approval callback table\n", __FUNCTION__);
    return L7_FAILURE;
  }
  memset(pMem, 0, memSize);
  pTable = &pAclCallbackInfo_g->table[ACL_CALLBACK_TABLE_ID_DELETE_APPROVAL];
  pTable->pStart = (aclCallbackEntry_t *)pMem;
  pTable->maxEntries = maxEnt;
...
void aclLogTask(void)
{
  ...
  while (1)
  {
    ...
    {
      LOG_MSG("%s unable to obtain ACL write lock\n", __FUNCTION__);
    }
...
L7_RC_t aclSemaTake(void *semId, L7_int32 timeout, char *file, L7_ulong32 line)
{
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_MSG("Semaphore take failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
...
L7_RC_t aclSemaGive(void *semId, char *file, L7_ulong32 line)
{
  ...
  {
    l7utilsFilenameStrip(&file);
    LOG_MSG("Semaphore give failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
            file, line, rc, (L7_uint32)semId);
  }
  return rc;
}
...
L7_RC_t aclReadLockTake(osapiRWLock_t rwlock, L7_int32 timeout, char *file, L7_ulong32 line)
{
  ...
  {
    l7utilsFilenameStrip(&file);
    LOG_MSG("Read lock take failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
...
L7_RC_t aclReadLockGive(osapiRWLock_t rwlock, char *file, L7_ulong32 line)
{
  ...
  {
    l7utilsFilenameStrip(&file);
    LOG_MSG("Read lock give failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
            file, line, rc, (L7_uint32)rwlock.handle);
  }
  return rc;
}
...
L7_RC_t aclWriteLockTake(osapiRWLock_t rwlock, L7_int32 timeout, char *file, L7_ulong32 line)
{
  ...
  {
    l7utilsFilenameStrip(&file);
    LOG_MSG("Write lock take failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
            file, line, rc, (L7_uint32)rwlock.handle);
  }
  return rc;
}
...
L7_RC_t aclWriteLockGive(osapiRWLock_t rwlock, char *file, L7_ulong32 line)
{
  ...
  {
    l7utilsFilenameStrip(&file);
    LOG_MSG("Write lock give failure for file %s:%lu, rc=%u, id=0x%8.8x\n",
            file, line, rc, (L7_uint32)rwlock.handle);
  }
  return rc;
}
