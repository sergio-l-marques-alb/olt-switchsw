...
L7_RC_t voipSdpConnectionInit (voipSdpConnection_t **conn)
{
  *conn = (voipSdpConnection_t *)osapiMalloc(L7_FLEX_QOS_VOIP_COMPONENT_ID ,(sizeof (voipSdpConnection_t)));
  ...
  if (*conn == L7_NULLPTR)
  {
    LOG_MSG("voipSdpConnectionInit Could not allocate memeory\n");
    return L7_FAILURE;
  }
...
L7_RC_t voipSdpMediaInit (voipSdpMedia_t **media)
{
  *media = (voipSdpMedia_t *)osapiMalloc(L7_FLEX_QOS_VOIP_COMPONENT_ID , (sizeof (voipSdpMedia_t)));
  ...
  if (*media == L7_NULLPTR)
  {
    LOG_MSG("voipSdpMediaInit Could not allocate memeory\n");    
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("voipSdpMediaInit(connections) Could not allocate memeory\n");
...
L7_RC_t voipSdpMessageInit (voipSdpMessage_t **sdp)
{
  ...
  if (*sdp == L7_NULLPTR) 
  {
    LOG_MSG("voipSdpMessageInit : Could not allocate memory\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("voipSdpMessageInit (m_medias) Could not allocate memory\n"); 
