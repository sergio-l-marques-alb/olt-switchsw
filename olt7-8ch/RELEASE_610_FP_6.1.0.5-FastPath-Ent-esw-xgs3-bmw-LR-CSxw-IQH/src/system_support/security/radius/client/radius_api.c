                                                   ...
                                                   L7_uint32 attributesLen))
{
  ...
  {
    LOG_MSG("RADIUS: Registrar ID %u greater then last component ID\n", registrar_ID);
    return L7_FAILURE;
  }
  ...
  if ((L7_uint32)radiusNotifyList[registrar_ID].notify_radius_resp != L7_NULL)
  {
    ...
    LOG_MSG("RADIUS: Registrar ID %u already registered\n", registrar_ID);
  }
...
L7_RC_t radiusResponseDeregister( L7_COMPONENT_IDS_t componentId)
{
  ...
  {
    LOG_MSG("radiusResponseDeregister registrar_ID %d greater than L7_LAST_COMPONENT_ID\n", componentId);
    return(L7_FAILURE);
  }

  if (radiusNotifyList[componentId].registrar_ID == L7_NULL)
  {
    LOG_MSG("radiusResponseDeregister registrar_ID %d not registered\n", componentId);
    return(L7_FAILURE);
  }
                               ...
                               L7_uint32 length)
{
  ...
  {
    LOG_MSG("RADIUS: could not get buffer");
    return L7_FAILURE;
  }
                               ...
                               L7_COMPONENT_IDS_t componentID)
{
  ...
  {
    LOG_MSG("RADIUS: radiusUserAuthenticate(): Invalid paramters.\n");
    return L7_FAILURE;
  }

  memset((void *)serverName, L7_NULL, sizeof(serverName));
  osapiStrncpySafe(serverName,L7_RADIUS_SERVER_DEFAULT_NAME_AUTH,sizeof(serverName));

  RADIUS_DLOG(RD_LEVEL_INFO,"%s(): Sending the message to named Server: %s \
                     ( via interface: %s IPAddr_type: %d componentID: %d correlator: %d)",
                     __FUNCTION__,serverName,osapiInet_ntoa(L7_NULL),
                     L7_IP_ADDRESS_TYPE_UNKNOWN,componentID,correlator);


  return radiusUserAuthenticateMsg (userName,password,state,correlator,
                     componentID,serverName,L7_IP_ADDRESS_TYPE_UNKNOWN,L7_NULL);
}
                               ...
                               L7_uchar8 *ipAddr)
{
  ...
  {
    LOG_MSG("RADIUS: %s(): Invalid paramters.\n",__FUNCTION__);
    return L7_FAILURE;
  }
                               ...
                               L7_uchar8 *ipAddr)
{
  ...
  if(L7_NULLPTR == userName || L7_NULLPTR == password ||
                     L7_NULLPTR == serverName )
  {
     LOG_MSG("RADIUS: %s(): Invalid parameters\n",__FUNCTION__);
     return L7_FAILURE;
  }
  ...
  {
    LOG_MSG("RADIUS: radiusUserAuthenticate(): FAiled to get lock\n");
    return L7_FAILURE;
  }
  else
  {
    ...
    if( L7_IP_ADDRESS_TYPE_IPV4 == type && L7_NULLPTR != ipAddr)
    {
      ...
      if( radiusNwInterfaceSocketOpen((osapiInet_addr(ipAddr)),&socket)
                     == L7_FAILURE)
      {
        LOG_MSG("RADIUS: %s(): Failed to open.",__FUNCTION__);
    ...
    if( radiusServerNameEntryIndexGet(serverName, &index ) != L7_SUCCESS ||
        radiusServerAuthCurrentEntryGet(index, usageType, &serverEntry) != L7_SUCCESS ||
        L7_NULLPTR == serverEntry )
    {
      LOG_MSG("RADIUS: radiusUserAuthenticateMsg(): Could not get the named \
                    %s  Server'\n",serverName);
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      return L7_FAILURE;
    }
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_NOTICE,  L7_RADIUS_COMPONENT_ID, "RADIUS: Could not allocate requestInfo. "
                                 ...
                                 L7_COMPONENT_IDS_t componentID)
{
  ...
  return radiusAccessRequestMsgSend(vpList,correlator,componentID,
                     serverName,L7_IP_ADDRESS_TYPE_UNKNOWN,L7_NULL);
}
                                ...
                                L7_uchar8 *ipAddr)
{
  ...
  {
     LOG_MSG("%s(): Invalid pointer to serverName\n",__FUNCTION__);
  ...
  else
  {
    ...
    if( radiusServerNameEntryIndexGet(serverName, &index ) != L7_SUCCESS ||
        ...
        L7_NULLPTR == serverEntry )
    {
      LOG_MSG("%s(): Named radius server:%s not found! \n",__FUNCTION__, serverName);
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      radiusAttrValuePairListFree(vpList);
      return L7_FAILURE;
    }
  ...
  if(bufferPoolAllocate(radius_sm_bp_id, (L7_uchar8 **)(&requestInfo)) != L7_SUCCESS)
  {
    LOG_MSG("%s(): Could not allocate requestInfo\n",__FUNCTION__);
    radiusAttrValuePairListFree(vpList);
    return L7_FAILURE;
  }
                                ...
                                L7_uchar8 *ipAddr)
{
  ...
  {
    LOG_MSG("RADIUS: radiusAccessRequestNamedSend(): Invalid Parameters. \n");
    return L7_FAILURE;
  }

  RADIUS_DLOG(RD_LEVEL_INFO,"%s(): Sending the message to named Server: %s \
                     ( via interface: %s IPAddr_type: %d componentID: %d correlator: %d)",
                     __FUNCTION__,serverName,ipAddr,type,componentID,correlator);
                              ...
                              L7_uchar8 *ipAddr)
{
  ...
  if(L7_NULLPTR == serverName)
  {
     LOG_MSG("RADIUS: radiusAccountingMsgStart(): Invalid pointer to serverName\n");
     return L7_FAILURE;
  }

  if( L7_IP_ADDRESS_TYPE_IPV4 != type && L7_IP_ADDRESS_TYPE_UNKNOWN != type)
  {
    LOG_MSG("RADIUS: radiusAccountingMsgStart(): Invalid parameters\n");
    return L7_FAILURE;
  }

  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("RADIUS: radiusAccountingNamedStart(): FAiled to get lock\n");
    return L7_FAILURE;
  }
  else
  {
    ...
    if( L7_IP_ADDRESS_TYPE_IPV4 == type && L7_NULLPTR != ipAddr)
    {
      ...
      {
        LOG_MSG("RADIUS: radiusAccountingMsgStart(): Failed to open.\n");
        (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
        return L7_FAILURE;
      }
    }
    ...
    if( radiusAcctServerNameEntryIndexGet(serverName, &index ) != L7_SUCCESS ||
        ...
        L7_NULLPTR == serverEntry )
    {
      LOG_MSG("RADIUS: radiusAccountingNamedStart(): Could not get the '%s'\n",serverName);
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      return L7_FAILURE;
    }
    (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  {
    L7_LOG(L7_LOG_SEVERITY_NOTICE,  L7_RADIUS_COMPONENT_ID, "RADIUS: Could not allocate"
                             ...
                             radiusValuePair_t *vpList)
{
  ...
  memset((void *)serverName, L7_NULL, sizeof(serverName));
  osapiStrncpySafe(serverName,L7_RADIUS_SERVER_DEFAULT_NAME_ACCT,sizeof(serverName));
                             ...
                             L7_uchar8 *ipAddr)
{
  ...
  if(L7_NULLPTR == serverName )
  {
     LOG_MSG("RADIUS: radiusAccountingMsgStop(): Invalid \
                     pointer to serverName\n");
     return L7_FAILURE;
  }

  if( L7_IP_ADDRESS_TYPE_IPV4 != type && L7_IP_ADDRESS_TYPE_UNKNOWN != type )
  {
    LOG_MSG("RADIUS: radiusAccountingMsgStop(): Invalid parameters\n");
    return L7_FAILURE;
  }
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_MSG("RADIUS: radiusAccountingMsgStop(): FAiled to get lock\n");
    return L7_FAILURE;
  }
  else
  {
    ...
    if( L7_IP_ADDRESS_TYPE_IPV4 == type  && L7_NULLPTR != ipAddr)
    {
      ...
      if( radiusNwInterfaceSocketOpen((osapiInet_addr(ipAddr)),&socket) == L7_FAILURE)
      {
        LOG_MSG("RADIUS: radiusAccessRequestMsgSend(): Failed to open.\n");
        (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
        return L7_FAILURE;
      }
    }
    ...
    if( radiusAcctServerNameEntryIndexGet(serverName, &index ) != L7_SUCCESS ||
        ...
        L7_NULLPTR == serverEntry )
    {
      LOG_MSG("RADIUS: radiusAccountingNamedStop(): Could not get the 'Default RADIUS Acct. Server'\n");
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      return L7_FAILURE;
    }
    (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
      ...
      { /* Server specific value is fetched */
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusMaxNoOfRetransGet(): Failed to find server");
          return L7_FAILURE;
        }
      }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
      ...
      {  /* Server specific value is fetched */
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusTimeOutDurationGet(): Failed to find server");
          return L7_FAILURE;
        }
      }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
      ...
      {  /* Server specific value is fetched */
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusServerDeadtimeGet(): Failed to find server");
          return L7_FAILURE;
        }
      }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
      ...
      {  /* Server specific value is fetched */
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusServerSourceIPGet(): Failed to find server");
          return L7_FAILURE;
        }
      }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
  ...
  */
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (serverEntry != L7_NULLPTR)
    {
      ...
      {
        LOG_MSG("RADIUS: %s(): Name already in use.!",__FUNCTION__);
               ...
               L7_IP_ADDRESS_TYPE_t type, L7_uint32 *portNum)
{
  ...
  {
    LOG_MSG(" RADIUS: radiusAcctServerPortNumGet(): Invalid parameters \n");
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
      ...
      {  /* Server specific value is fetched */
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusHostNameServerSharedSecretGet(): Failed to find server");
          return L7_FAILURE;
        }
      }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
               ...
               L7_IP_ADDRESS_TYPE_t type, L7_uint32 entryType)
{
  ...
  {
    LOG_MSG("RADIUS(): radiusHostNameServerEntryTypeSet() %s : Invalid parameters\n",__FUNCTION__);
    return L7_FAILURE;
  }
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (serverEntry == L7_NULLPTR)
    {
      LOG_MSG("RADIUS(): radiusHostNameServerEntryTypeSet() : Specified Server Not found\n");
      return L7_FAILURE;
    }
    if(type == L7_IP_ADDRESS_TYPE_IPV4)
    {
                           ...
                           L7_uchar8 *decrKeyLen)
{
    ...
    */
    ...
    if (left % RADIUS_AUTHENTICATOR_LENGTH) {
        LOG_MSG("RADIUS: Invalid ms key len\n");
        return L7_FAILURE;
    }
    ...
    {
        LOG_MSG("RADIUS: Failed to decrypt MPPE key\n");
        return L7_FAILURE;
    }
                     ...
                     L7_IP_ADDRESS_TYPE_t type, L7_char8 *serverName)
{
  ...
  {
    LOG_MSG("RADIUS: radiusServerAuthNameSet(): Invalid Parameters \n");
    return L7_FAILURE;
  }
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (serverEntry == L7_NULLPTR)
    {
      LOG_MSG("RADIUS: radiusServerAuthNameSet(): The specified server: %s of \
                     type: %d desn't exist\n",serverAddr,type);
      /* Entry doesn't exist */
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      return L7_FAILURE;
                     ...
                     L7_IP_ADDRESS_TYPE_t type, L7_char8 *serverName)
{
  ...
  {
    LOG_MSG("RADIUS: radiusServerAcctNameSet(): Invalid Parameters \n");
    return L7_FAILURE;
  }
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if(L7_SUCCESS == rc)
    {
      ...
      if( L7_NULL != currentEntry &&
          serverEntry != currentEntry)
      {
        LOG_MSG("RADIUS: radiusServerAcctNameSet(): Name already in use.\n");
        rc = L7_ALREADY_CONFIGURED;
      }
      else
      {
               ...
               L7_IP_ADDRESS_TYPE_t type, L7_char8 *serverName)
{
  ...
  {
    LOG_MSG("RADIUS: radiusServerHostNameGet() Invalid Parameters\n");
    return L7_FAILURE;
  }
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    {
      LOG_MSG("RADIUS:radiusServerHostNameGet(): Not found the server entry for the host: %s\n", serverAddr);
    }
               ...
               L7_IP_ADDRESS_TYPE_t type, L7_char8 *serverName)
{
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (L7_NULLPTR == serverEntry  )
    {
      LOG_MSG("RADIUS:%s Not found the server entry for the host: %s\n", __FUNCTION__,serverAddr);
    }
...
L7_RC_t radiusNamedServerAddrGet(L7_char8 *serverName, L7_RADIUS_SERVER_USAGE_TYPE_t usageType, L7_uchar8 *pServerAddr, L7_IP_ADDRESS_TYPE_t *pType)
{
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    *pType = L7_IP_ADDRESS_TYPE_UNKNOWN;
    ...
    if ( rc != L7_SUCCESS )
    {
      LOG_MSG("RADIUS: %s(): No server found with this name!\n",__FUNCTION__);
    }
    else
    {
    ...
    *pType = L7_IP_ADDRESS_TYPE_UNKNOWN;
    ...
    else
    {
      (void)radiusServerAcctCurrentEntryGet(index,&currentEntry);
       if(L7_NULLPTR == currentEntry)
       {
         LOG_MSG("RADIUS: %s(): Could not get current server!\n",__FUNCTION__);
       }
       else
       {
         *pType = currentEntry->radiusServerDnsHostName.hostAddrType;
                     ...
                     radiusServerType_t type, L7_uint32 *ipAddr)
{
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (L7_NULL == serverEntry)
    {
      LOG_MSG("%s(): DNS server not found! ",__FUNCTION__);
    }
                                ...
                                L7_uint32  *localMask)
{
  ...
  if (RADIUS_LOCK_TAKE(radiusTaskSyncSema, L7_WAIT_FOREVER) == L7_SUCCESS)
  {
    ...
    if (radiusClient != L7_NULLPTR)
    {
        ...
        {
          (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
          LOG_MSG("radiusLocalConfigMaskGet(): Failed to find server");
          return L7_FAILURE;
        }
      (void)RADIUS_LOCK_GIVE(radiusTaskSyncSema);
      return L7_SUCCESS;
