                               ...
                               radiusRequest_t *request)
{
  ...
  {
    LOG_MSG("RADIUS: Access-Reject failed validation, id=%u\n", packet->header.id);
    bufferPoolFree(radius_lg_bp_id, (L7_uchar8 *)packet);
  }
       ...
       */
        ...
        if( osapiNtohl(vendorId) == RADIUS_VENDOR_ID_MICROSOFT) 
        {
           ...
           */
    	   ...
    	   {
               LOG_MSG("RADIUS: MS attribute type =%d\n",msKey->type);
    	   }
...
L7_RC_t radiusRequestInfoProcess(radiusRequestInfo_t *requestInfo)
{
  ...
  {
    LOG_MSG("RADIUS (): request is dropped as it has no reference to current server\n");
    ...
    if ((requestInfo->requestType == RADIUS_REQUEST_TYPE_AUTH_VP))
    {
      ...
      */
      if ((pClient != L7_NULL) && (pClient->authOperMode == L7_ENABLE))
      {
        ...
        radiusServerAuthCurrentEntryGet(requestInfo->serverNameIndex, usageType, &currentEntry);
  ...
  {
    LOG_MSG("RADIUS: Could not allocate a request buffer\n");
  }
  ...
  */
  ...
  LOG_MSG("RADIUS: radiusRequestInfoProcess(): Could not \
