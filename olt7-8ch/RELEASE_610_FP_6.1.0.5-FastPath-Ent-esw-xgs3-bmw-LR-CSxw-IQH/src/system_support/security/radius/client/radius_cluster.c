...
L7_RC_t radiusClusterMsgRecv (L7_uchar8 *packet, L7_uint32 packetLen)
{
  ...
  {
    LOG_MSG("Packet received in invalid state %s.\n", 
  ...
  while (pdu.offset < packetLen)
  {
    ...
    {
      LOG_MSG("Received corrupt Cluster-RADIUS-Client-Global-Message, reading element 0x%x.\n",
              element.elementId);
      return L7_FAILURE;
    }
    switch (element.elementId)
    {
      ...
      case L7_IE_RADIUS_CLIENT_NAS_IP_ADDR_TYPE:
        ...
        {
          LOG_MSG("Invalid IP Address format received. Expecting IPv4 only addresses.\n");
          return L7_FAILURE;
        }
        break;
      case L7_IE_RADIUS_CLIENT_SERVER_TYPE:
        ...
        {
          LOG_MSG("Cannot add new RADIUS server %s of type %d.", buffer, serverType);
          return L7_FAILURE;
        }
      ...
      default:
        ...
        LOG_MSG("Invalid element 0x%x received in the message.\n",
                            element.elementId);
        break;
    }
  }
...
void radiusClusterCfgCopy (void)
{
  ...
  if (radiusIssueCmd (RADIUS_ACCT_ADMIN_MODE_SET, L7_NULL,
                            ...
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set RADIUS Accounting Admin Mode.\n");
  }
  ...
  if (radiusIssueCmd (RADIUS_MAX_RETRANS_SET, &hostIPAddr,
                            ...
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set RADIUS Max. Number of retransmissions.\n");
  }
  hostIPAddr.hostAddrType = L7_IP_ADDRESS_TYPE_IPV4;
  hostIPAddr.host.ipAddr  = ALL_RADIUS_SERVERS;
  authServerInfo.localGlobal = L7_RADIUSGLOBAL; /* Always Global for now. */
  ...
  if (radiusIssueCmd (RADIUS_TIMEOUT_SET, &hostIPAddr,
                            ...
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set RADIUS Timeout Duration.\n");
  }
  ...
  if(radiusIssueCmd (RADIUS_DEADTIME_SET, &hostIPAddr,
                            ...
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set RADIUS DeadTime Duration.\n");
  }
  hostIPAddr.hostAddrType = L7_IP_ADDRESS_TYPE_IPV4;
  hostIPAddr.host.ipAddr  = ALL_RADIUS_SERVERS;
  authServerInfo.localGlobal = L7_RADIUSGLOBAL;
  ...
  if (radiusIssueCmd (RADIUS_SOURCEIP_SET, &hostIPAddr,
                            ...
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set Source IP Address and Mode.\n");
  }
  ...
  if (radiusIssueCmd (RADIUS_AUTH_SECRET_SET, &hostIPAddr,
                            ...
                            L7_SUCCESS)
  {
        LOG_MSG("Failed to set RADIUS Auth Secret.\n");
  }
  ...
  if (radiusIssueCmd (RADIUS_ATTRIBUTE_4_SET, &nasIPAddr, (void *)&nasMode) !=
                            L7_SUCCESS)
  {
    LOG_MSG("Failed to set RADIUS NAS IP Address and Mode.\n");
  }
  ...
  for (;index >= 0; index--)
  {
    ...
    if (radiusIssueCmd (RADIUS_AUTH_IPADDR_ADD, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to add Authentication server to Db.\n");
      continue;
    }
    ...
    if (radiusIssueCmd (RADIUS_AUTH_PORT_SET, hostname, (void *)&port) !=
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to set RADIUS Auth Port.\n");
    }
    ...
    if (radiusIssueCmd (RADIUS_MAX_RETRANS_SET, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to set Max Retrans per server to Db.\n");
    ...
    if (radiusIssueCmd (RADIUS_TIMEOUT_SET, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to add Timeout per server to Db.\n");
    ...
    if (radiusIssueCmd (RADIUS_DEADTIME_SET, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to add Dead Time per server to Db.\n");
    ...
    if (radiusIssueCmd (RADIUS_SOURCEIP_SET, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to Source IP per server to Db.\n");
  ...
  for (;index >= 0; index--)
  {
    ...
    if (radiusIssueCmd (RADIUS_ACCT_IPADDR_ADD, hostname,
                            ...
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to add Accounting server to Db.\n");
      continue;
    }
    osapiSemaTake(radiusServerDbSyncSema, L7_WAIT_FOREVER);
    port = serverConfig->radiusServerConfigUdpPort;
    if (radiusIssueCmd (RADIUS_ACCT_PORT_SET, hostname, (void *)&port) !=
                            L7_SUCCESS)
    {
      LOG_MSG("Failed to set RADIUS Acct Port.\n");
    }
    if (strlen (serverConfig->radiusServerConfigSecret) > 0)
    {
      if (radiusIssueCmd (RADIUS_ACCT_SECRET_SET, hostname,
                            ...
                            L7_SUCCESS)
      {
        LOG_MSG("Failed to set RADIUS Acct Secret.\n");
      }
    }
  }
                            ...
                            L7_uint32 msgLength, L7_uchar8 *msgBuf)
{
  switch (msgType)
  {
    case RADIUS_CLUSTER_CONFIG_MSG_TYPE:
      ...
      {
        LOG_MSG("Failed to parse RADIUS Client Configuration message.\n");
      }
      break;
    default:
      LOG_MSG("RADIUS Client module received unknown clustering configuration message.\n");
      break;
  }
...
void radiusClusterEventCallback(clusterEvent event, clusterMemberID_t * memID)
{
  ...
  if (!memID)
  {
    LOG_MSG("cluster Memeber ID is NULL!\n");
    return;
  }
  ...
  switch (event)
  {
    ...
    case CLUSTER_EVENT_CONTROLLER_ELECTED:
      ...
      break;
    default:
      LOG_MSG("Unknown event dropped without handling.\n");
      break;
  }
...
void radiusCfgSendCallback(clusterMemberID_t * pID)
{
  ...
  if (!pID)
  {
    LOG_MSG("Cluster member ID is NULL!.\n");
    return;
  }
  ...
  {
     LOG_MSG("%s: msg send failed to radiusClusteringQueue!\n", __FUNCTION__);
  }
...
static L7_RC_t radiusClusterCfgSendHandle(clusterMemberID_t * memberID)
{
  ...
  if(rc != L7_SUCCESS)
  {
    LOG_MSG("Cluster Message send failed to member ID = %02x:%02x:%02x:%02x:%02x:%02x\n",
  ...
  while (authSrvrCount > 0)
  {
    ...
    do
    {
      ...
      {
        LOG_MSG("Authentication Server Type Get Failed!");
      }
      /* server type */
      tempChar = (L7_uchar8) tempInt;
      RADIUS_PKT_INT8_SET(tempChar, pdu.bufPtr, pdu.offset);
      ...
      {
        LOG_MSG("Authentication Server Port Get Failed!");
      }
      ...
      {
        LOG_MSG("Authentication Server Name Get Failed!");
      }
      ...
      {
        LOG_MSG("Authentication Server secret Get Failed!");
      }
      ...
      {
        LOG_MSG("Authentication Server Priority Get Failed!");
      }
      ...
      {
        LOG_MSG("Authentication Server Message Usage Type Get Failed!");
      }
      ...
      {
        LOG_MSG("Authentication Server Message Authentication Mode Get Failed!");
      }
      ...
      {
        LOG_MSG("Local Config Mask Get Failed!");
      }
      ...
      {
        LOG_MSG("Max Number of retransmissions Get Failed!");
      }
      ...
      {
        LOG_MSG("Time out Duration Get Failed!");
      }
      ...
      {
        LOG_MSG("Dead time Parameter Get Failed!");
      }
      ...
      {
        LOG_MSG("Source IP address Parameter Get Failed!");
      }
          ...
          (authSrvrCount == 0))
      {
        ...
        if(rc != L7_SUCCESS)
        {
          LOG_MSG("Cluster Message send failed to member ID  = %02x:%02x:%02x:%02x:%02x:%02x line %d\n",
                  memberID->addr[0], memberID->addr[1],
                  memberID->addr[2], memberID->addr[3],
  ...
  while (acctSrvrCount > 0)
  {
    ...
    do
    {
      ...
      {
        LOG_MSG("Accounting Server Type Get Failed!");
      }
      /* server type */
      tempChar = (L7_uchar8) tempInt;
      RADIUS_PKT_INT8_SET(tempChar, pdu.bufPtr, pdu.offset);
      ...
      {
        LOG_MSG("Accounting Server Port Get Failed!");
      }
      RADIUS_PKT_ELEMENT_HDR_SET(element,L7_IE_RADIUS_CLIENT_SERVER_PORT_TYPE,
                            L7_IE_RADIUS_CLIENT_SERVER_PORT_SIZE,
                            pdu.bufPtr, pdu.offset);
      ...
      {
        LOG_MSG("Accounting Server Name Get Failed!");
      }
      RADIUS_PKT_ELEMENT_HDR_SET(element, L7_IE_RADIUS_CLIENT_GROUP_NAME_TYPE,
                            L7_IE_RADIUS_CLIENT_GROUP_NAME_SIZE,
                            pdu.bufPtr, pdu.offset);
      ...
      {
        LOG_MSG("Accounting Server secret Get Failed!");
      }
      RADIUS_PKT_ELEMENT_HDR_SET(element, L7_IE_RADIUS_CLIENT_SERVER_SECRET_TYPE,
                            L7_IE_RADIUS_CLIENT_SERVER_SECRET_SIZE,
                            pdu.bufPtr, pdu.offset);
      ...
      {
        LOG_MSG("Accounting Server Priority Get Failed!");
      }
      RADIUS_PKT_ELEMENT_HDR_SET(element, L7_IE_RADIUS_CLIENT_SERVER_PRIORITY_TYPE,
                            L7_IE_RADIUS_CLIENT_SERVER_PRIORITY_SIZE ,
                            pdu.bufPtr, pdu.offset);
...
static void radiusClusterQueueMsgHandle(radiusClusterMessage_t * pMsg)
{
  if (L7_NULLPTR == pMsg)
  {
    LOG_MSG("Cluster message is NULL!\n");
    return;
  }
  ...
  switch(pMsg->mtype)
  {
    ...
    case RADIUS_CLUSTER_CFG_SEND:
      ...
      {
        LOG_MSG("Invalid state %s to send cluster config.\n",
        radiusClusterEventToStringConvert(clusterOprData.state));
      }
      break;
    ...
    case RADIUS_CLUSTER_CFG_RX_START:
      ...
      {
        LOG_MSG("Invalid state %s to RX cluster config.\n", 
                radiusClusterEventToStringConvert(clusterOprData.state));
      }
      break;
    ...
    case RADIUS_CLUSTER_CFG_RX_END:
      ...
      {
        LOG_MSG("Invalid state %s for RX ending\n",
                radiusClusterEventToStringConvert(clusterOprData.state));
        /*
    ...
    case RADIUS_CLUSTER_CFG_RX_APPLY:
      ...
      {
        LOG_MSG("Invalid state %s to apply data.\n",
                radiusClusterEventToStringConvert(clusterOprData.state));
      }
      break;
    ...
    case RADIUS_CLUSTER_CFG_RX_ABORT:
      ...
      {
        LOG_MSG("Invalid state %s to handle RX Abort message.\n", 
                radiusClusterEventToStringConvert(clusterOprData.state));
      }
      break;

    default:
      LOG_MSG("Invalid state %s, to hanlde msg %d dropping message.\n",
...
void radius_cluster_task(void)
{
  ...
  {
    LOG_MSG("RADIUS_CLUSTER: Could not register event notification callback!!\n");
  }
  ...
  if (L7_SUCCESS != clusterMsgRegister(RADIUS_CLUSTER_CONFIG_MSG_TYPE, 
                                       ...
                                       radiusClusterCfgMsgCallback))
  {
    LOG_MSG("RADIUS_CLUSTER: Register radius config message with cluster failed.\n");
  }
  ...
  if (L7_SUCCESS != clusterConfigRegister(CLUSTER_CFG_ID(RADIUS),
                                          ...
                                          radiusCfgRxCallback))
  {
    LOG_MSG("RADIUS_CLUSTER: Couldn't register cfg callbacks!\n");
  }
