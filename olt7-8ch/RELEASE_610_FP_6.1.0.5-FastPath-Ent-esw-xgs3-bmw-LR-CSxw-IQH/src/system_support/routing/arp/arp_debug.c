...
#define ARP_DEBUG_PACKET_TX_FORMAT "Pkt TX -  Intf: %d/%d/%d(%d),SrcMac: %s DestMac: %s SrcIP: %s DestIP: %s Type: %s H/wType: %s ProType: %s"
...
void arpDebugPacketTxTrace(L7_uint32 intIfNum,t_ARPFrame *arpFrame)
{
  ...
  if (nimGetUnitSlotPort(intIfNum, &usp) == L7_SUCCESS)
  {
    ...
    ARP_USER_TRACE(ARP_DEBUG_PACKET_TX_FORMAT,
        unit, slot, port, intIfNum, srcMac, destMac, srcIp, dstIp,type, hardwareType, protocolType);
  }
