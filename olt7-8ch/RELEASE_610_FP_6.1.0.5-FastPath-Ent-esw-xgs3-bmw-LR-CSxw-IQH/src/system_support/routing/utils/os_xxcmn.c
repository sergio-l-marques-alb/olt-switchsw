...
static char __fileid[] = "$Header: /home/cvs/PR1003/PR1003/FASTPATH/RELEASE_610_FP_6.1.0.5-FastPath-Ent-esw-xgs3-bmw-LR-CSxw-IQH/src/system_support/routing/utils/os_xxcmn.c,v 1.1 2011/04/18 17:10:53 mruas Exp $";
#endif
   ...
   for(length = strlen(string), index = 0; length > 0; length -= len, index +=len)
   {
       ...
       if(L7_DEFAULT_THREAD)
       {
           ...
           if( (tmp = (byte *)XX_MallocChunk(len + 1)) )
           {
               ...
               if (e != E_OK)
               {
                   LOG_COMPONENT_MSG(L7_FLEX_BGP_MAP_COMPONENT_ID, "OS_XX_Print: unable to send message for printing");
               }
           }
       }
       else
...
char *fptr2name(void *func_ptr, char *func_name, ulng name_size)
{
  ...
  if (osapiFunctionLookup((L7_uint32) func_ptr, func_name, name_size,
               &offset) != L7_SUCCESS)
  {
    strncpy(func_name, "Unk", name_size);
  }
  return func_name;
}
...
void XX_DisplayQueue(t_Handle threadID, ulng queueId, ulng resolveNames, long maxDepth)
{
  ...
  osapiMsgQueueGetNumMsgs(msgQueue, &currLen);
...
e_Err OS_XX_KillThread(t_Handle threadId, Bool taskContext)
{
   ...
   if(taskContext == TRUE)
   {
      ...
      if (e != E_OK)
      {
          L7_LOGNC(L7_LOG_SEVERITY_ERROR, "OS_XX_KillThread: unable to kill thread");
      }
   }
   else
   {
...
static Bool debugQueues = FALSE;
void DebugQueues(Bool val)
{
  debugQueues = val;
}
