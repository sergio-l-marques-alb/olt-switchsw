                          ...
                          L7_uint32 * buffer_count)
{
  ...
  */
  ...
  {
    LOG_MSG("we reached the maximum number (%d) of buffer pools\n", NumBufferPools);
    return L7_FAILURE;
  }
...
L7_RC_t bufferPoolTerminate (L7_uint32  buffer_pool_id)
{
  ...
  {
      LOG_MSG("Cannot delete buffer pool- pool id = %x\n", buffer_pool_id);
      return L7_FAILURE;
  }
...
void bufferPoolFree (L7_uint32 buffer_pool_id, L7_uchar8 * buffer_addr)
{
  ...
  if (buffer_addr == NULL)
  {
	  LOG_MSG("bufferPoolFree: ID %d, buffer_addr NULL!\n", buffer_pool_id);
	  return;
  }
  ...
  if (buffer_pool_id == 0)
  {
    for (i = 0; i < MaxBufferPools; i++)
    {
      ...
      */
      ...
      printf("Tot. Buffs: %d, Free Buffs: %d, Buff. Size: %d, Num Allocs: %d, Num Empty: %d\n",
             pool->total,
             pool->free_count,
             pool->buf_size,
             pool->num_allocs,
