...
{
  bzero((L7_uchar8 *)rootp,sizeof(avlTreeTables_t));
...
void * avlRemoveEntry( avlTree_t *avl_tree, void *item)
{
   ...
   int depth;
   char pathMap[L7_AVL_MAX_DEPTH+2];
   avlTreeTables_t *path[L7_AVL_MAX_DEPTH+2];               
   ...
   path[L7_LEFT] = &avl_tree->root;

   /* Find item to remove */
   for ( depth = 1; depth < L7_AVL_MAX_DEPTH; depth++)
   {
     ...
     {
       node = node->link[L7_RIGHT];
       pathMap[depth] = L7_RIGHT;
     }
     else if (diff == AVL_LESS_THAN)
     {
       node = node->link[L7_LEFT];
       pathMap[depth] = L7_LEFT;
     }
     if (node == NULL)
       return NULL;
   }
   ...
   if (node->link[L7_RIGHT] == NULL)
   {
     *nodePtr = node->link[L7_LEFT];
     if (*nodePtr)
       (*nodePtr)->balance = 0;
   }
   else
   { 
     avlTreeTables_t *tempNode = node->link[L7_RIGHT];
     if (tempNode->link[L7_LEFT] == NULL)
     {
       tempNode->link[L7_LEFT] = node->link[L7_LEFT];
     ...
     {
       avlTreeTables_t *node2 = tempNode->link[L7_LEFT];
       int baseDepth = depth++;
               
       pathMap[depth] = 0;
       path[depth++] = tempNode;

       while (node2->link[L7_LEFT] != NULL)
       {
         tempNode = node2;
         node2 = tempNode->link[L7_LEFT];
         pathMap[depth] = 0;
         path[depth++] = tempNode;
       }
       ...
       node2->link[L7_LEFT] = node->link[L7_LEFT];
       tempNode->link[L7_LEFT] = node2->link[L7_RIGHT];
       node2->link[L7_RIGHT] = node->link[L7_RIGHT];
                                   ...
                                   L7_BOOL *done)
{
  ...
  otherDirection = (direction == L7_LEFT) ? L7_RIGHT : L7_LEFT;
  weight = (direction == L7_LEFT) ? L7_LEFT_WEIGHT : L7_RIGHT_WEIGHT;
  otherWeight = (direction == L7_LEFT) ? L7_RIGHT_WEIGHT : L7_LEFT_WEIGHT;
  direction = (direction == L7_LEFT) ? L7_LEFT : L7_RIGHT;
  ...
  else
  {
    ...
    if ((tempNode->balance == 0) || 
        ((direction == L7_RIGHT) && (tempNode == NULL))) 
    {
...
void ** avlFindEntry(avlTree_t *avl_tree, void *item)
{
   ...
   node = node2 = baseNode->link[L7_LEFT];

   /* empty tree case */
   if (node == NULL)
   {
     avl_tree->count++;
     newNode = baseNode->link[L7_LEFT] = avlNewNewNode(avl_tree);
     newNode->data = item;
     newNode->link[L7_LEFT] = NULL;
     newNode->link[L7_RIGHT] = NULL;
     newNode->balance = L7_LEFT;
     return((void *)(&newNode->data));
   }

   /* find match and return or create new node and break */
   for (depth = 0; depth < L7_AVL_MAX_DEPTH; depth++)  
   {
     ...
     if (diff == AVL_LESS_THAN)
     {
       ...
       newNode = node2->link[L7_LEFT];
       if (newNode == NULL)
       {
         newNode = avlNewNewNode (avl_tree);
         node2->link[L7_LEFT] = newNode;
         break;
       }
     }
     ...
     else if (diff == AVL_GREATER_THAN)
     {
       ...
       newNode = node2->link[L7_RIGHT];
       if (newNode == NULL)
       {
         newNode = avlNewNewNode (avl_tree);
         node2->link[L7_RIGHT] = newNode;
         break;
       }
     }
     /* Found it */
              ...
              avlTree_t *avl_tree, int direction)
{
  ...
  int weight =  (direction == L7_LEFT) ? L7_LEFT_WEIGHT: L7_RIGHT_WEIGHT;
  ...
  if (((weight == L7_LEFT_WEIGHT) && (node->balance >= 0)) ||
      ((weight == L7_RIGHT_WEIGHT) && (node->balance <= 0)))
  {
    node->balance += weight;
    return;
  }
  ...
  if (baseNode != &avl_tree->root && node == baseNode->link[L7_RIGHT]) 
    baseNode->link[L7_RIGHT] = shiftNode;
  else
    baseNode->link[L7_LEFT] = shiftNode;
...
{
  int otherDirection = (direction == L7_LEFT) ? L7_RIGHT: L7_LEFT;
                  ...
                  avlTreeTables_t *node2)
{
  ...
  int direction = (otherDirection == L7_LEFT) ? L7_RIGHT: L7_LEFT;
...
void * avlAddEntry( avlTree_t *avl_tree, void *item)
{
  ...
  if (new_entry==NULL)
  {
    ...
    exactEntry = avlSearchLVL7(avl_tree,item,AVL_EXACT);
    if (exactEntry!=L7_NULL)
    {
      return exactEntry;
    }
    else
