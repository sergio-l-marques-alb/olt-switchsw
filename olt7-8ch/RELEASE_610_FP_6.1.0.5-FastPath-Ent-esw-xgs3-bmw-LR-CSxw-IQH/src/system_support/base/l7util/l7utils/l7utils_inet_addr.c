...
/*********************************************************************
* @purpose  Validate a Mask address
*
...
*
*********************************************************************/
static L7_int32 inetMaskLengthGet(L7_int32 mask)
{
                             ...
                             L7_uint32 lineNum)
{
    ...
    {
      LOG_MSG ("INET_ADDR:Invalid FamilyType - %d from %s(%d)", inetAddr->family,
        fileName, lineNum);
    }
                             ...
                             L7_uint32 lineNum)
{
    ...
    {
        LOG_MSG ("INET_ADDR:Invalid FamilyType - %d from %s(%d)", addr->family,
        fileName, lineNum);
    }
    return L7_FALSE;
}
...
L7_RC_t inetMaskToMaskLen(L7_inet_addr_t *mask, L7_uchar8 *masklen)
{
    ...
    *masklen =  0;
    if (mask->family == L7_AF_INET) 
    {
        ...
        if(tempMaskLen == -1)
        {
          LOG_MSG("INETADDR: Invalid mask = %s", inetAddrPrint(mask, addrBuff));
          return L7_FAILURE;
        }
       *masklen= tempMaskLen;
    } 
    ...
    {
        LOG_MSG ("INET_ADDR: Invalid FamilyType - %d", mask->family);
    }
    return L7_SUCCESS;
}
...
L7_RC_t inetMaskLenToMask(L7_uchar8 family, L7_uchar8 masklen, 
    ...
    if (mask == L7_NULLPTR ) 
    {
        LOG_MSG ("INET_ADDR:Invalid input maks ");
        return L7_FAILURE;
    }
    ...
    if (family == L7_AF_INET) 
    {
        if (masklen > 32)
        {
            LOG_MSG ("INET_ADDR:Invalid MaskLength - %d", masklen);
            return L7_FAILURE;
        }
        if (masklen != 0) 
        {
    ...
    {
        LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", family);
        return L7_FAILURE;
    }
    return L7_SUCCESS;
}
...
L7_RC_t inetAddrHtop(L7_inet_addr_t *addr, L7_uchar8 *string)
{
    ...
    if (addr->family == L7_AF_INET) 
    {
        a = (L7_uchar8 *)&addr->addr.ipv4.s_addr;
        osapiSnprintf(string, L7_IP4_STR_LEN, "%u.%u.%u.%u", a[0], a[1], a[2], a[3]);
    } 
    else if (addr->family == L7_AF_INET6) 
    {
...
L7_RC_t inetAddrNtoh(L7_inet_addr_t * addr, L7_inet_addr_t * addr_h)
{
    ...
    {
        LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", addr->family);
    }
    return L7_SUCCESS;
}
...
L7_BOOL inetIsMaskValid(L7_inet_addr_t *pMask)
{
  ...
  {
    LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", pMask->family);
  }
  return L7_FALSE;
}
...
L7_RC_t inetAddressAnd(L7_inet_addr_t *src1, L7_inet_addr_t *src2, L7_inet_addr_t *dest)
{
  ...
  {
    LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", src1->family);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
...
L7_RC_t inetAddressNot(L7_inet_addr_t *src, L7_inet_addr_t *dest)
{
  ...
  {
    LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", src->family);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
                                       ...
                                       L7_inet_addr_t *addr2, L7_uint32 mask2)
{
  ...
  {
    LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", addr1->family);
  }
  return L7_FALSE;
}
...
L7_RC_t inet6AddressGet(L7_inet_addr_t *inetAddr, L7_in6_addr_t *in6Addr)
{
    ...
    {
        LOG_MSG ("INET_ADDR:Invalid FamilyType - %d", inetAddr->family);
        return L7_FAILURE;
    }
    return L7_SUCCESS;
}
