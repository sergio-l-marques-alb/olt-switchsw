...
// PTin added
int pdu_intercept_debug = 0;

void PduInterceptDebug(int enable)
{
  pdu_intercept_debug = enable;
...
L7_RC_t sysNetRegisterPduReceive(sysnetNotifyEntry_t *snEntry)
{
  ...
  {
    LOG_MSG("SYSNET: Invalid Registration Function\n");
    return(L7_FAILURE);
  }
  ...
  for (i = 0; i < FD_CNFGR_SYSNET_MAX_REGISTRATIONS; i++)
  {
    if (snEntry->type ==  SYSNET_PKT_RX_REASON)
      if (sysnetNotifyList.sysnetNotifyEntries[i].u.rxReason
           == snEntry->u.rxReason)
      {
        LOG_MSG("SYSNET Registration already exists\n");
        return(L7_ERROR);
      }
  ...
  if (i < FD_CNFGR_SYSNET_MAX_REGISTRATIONS)
  {
    ...
    case SYSNET_PKT_RX_REASON:
      ...
      break;
    default:
      LOG_MSG("SYSNET Unknown Entry Type\n");
      return(L7_FAILURE);
    }
  ...
  {
    LOG_MSG("SYSNET Registration Failed\n");
    return(L7_FAILURE);
  }
...
L7_RC_t sysNetDeregisterPduReceive(sysnetNotifyEntry_t *snEntry)
{
  ...
  {
    LOG_MSG("SYSNET Deregistration Failed\n");
    return(L7_FAILURE);
  }
                               ...
                               L7_uchar8 *data)
{
  ...
  {
    /* This is an Vlan tagged Frame */
    pEtype =  data + L7_ENET_HDR_SIZE + (L7_8021Q_ENCAPS_HDR_SIZE*2);
    bcopy(pEtype, (L7_uchar8 *) &protocol_type, sizeof(L7_ushort16)); /*Endian*/

    temp16 = osapiNtohs(protocol_type);
    protocol_type = temp16;

    offset  =  L7_ENET_HDR_SIZE + (L7_8021Q_ENCAPS_HDR_SIZE*2);
  }
  // PTin end
                                    ...
                                    L7_char8 *data)
{
  ...
  if (protocol_type == L7_ETYPE_8021Q)
  {
    L7_uchar8 *newDataStart = sysapiNetMbufGetDataStart(bufHandle) + L7_ENET_HDR_SIZE + L7_8021Q_ENCAPS_HDR_SIZE;
    L7_uint16 *innerTag     = (L7_uint16 *) newDataStart;
    L7_uint16 length = sysapiNetMbufGetDataLength(bufHandle);
    
    // Correct inner vlan
    pduInfo->innerVlanId = (L7_uint32) innerTag[1] & 0xFFF;
    // Strip inner tag
    memmove((void *)newDataStart, 
            (void *)newDataStart + L7_8021Q_ENCAPS_HDR_SIZE, 
            sysapiNetMbufGetDataLength(bufHandle) - (L7_ENET_HDR_SIZE + 2*L7_8021Q_ENCAPS_HDR_SIZE));
    sysapiNetMbufSetDataLength(bufHandle, length-L7_8021Q_ENCAPS_HDR_SIZE );

    bcopy(newDataStart, (L7_uchar8 *) &protocol_type, sizeof(L7_ushort16));
  }
  // PTin added - END
...
//  L7_uchar8 dmac_bc[6]={0xff,0xff,0xff,0xff,0xff,0xff};

  // PTin
  if (pdu_intercept_debug)
  {
    printf("%s(%d) I have received a PDU: SrcPort=%u, OuterVlan=%u, InnerVlan=%u\n", __FUNCTION__, __LINE__,pduInfo->intIfNum,pduInfo->vlanId,pduInfo->innerVlanId);
  }
  ...
  if (pdu_intercept_debug && rc==L7_SUCCESS)
  {
    printf("%s(%d) sysNetNotifyPduReceive stopped\n", __FUNCTION__, __LINE__);
  }
                                       ...
                                       L7_uint32 exitPort)
{
  ...
  if (intIfNum == 0)
  {
    if (vlanId == 0)
    {
      ...
      LOG_MSG("Circuit Error - vlan id & intIfNum = 0\n");
      return 0;
    }
    else
    {
  ...
  else
  {
    ...
    {
      LOG_MSG("Error - Unrecognized Encapsulation type\n");
      return 0;
    }
  }
                                ...
                                L7_netBufHandle *pBufHandle, L7_char8 *data)
{
  ...
  else if (nimGetIntfAddress(intIfNum, L7_NULL, (L7_char8 *)macHdr.src.addr ) != L7_SUCCESS)
  {
    SYSAPI_NET_MBUF_FREE(*pBufHandle);
	LOG_MSG("No source MAC address found\n");
    rc = L7_FAILURE;
    return rc;
  }
                    ...
                    L7_ushort16 etype, L7_uint32 datalen)
{
  ...
  {
    LOG_MSG("Invalid exit port - could not resolve exit intfNum\n");
    return;
  }
...
* @param    *hookAction      @b{(output)} Action taken by the hook intercept call (optional parm)
*
