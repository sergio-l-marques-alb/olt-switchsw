...
L7_RC_t dnsTaskStart()
{
  ...
  {
    LOG_MSG("Unable to initialize dnsRxTask()\n");
    return L7_FAILURE;
  }
...
void dnsTask()
{
  ...
  do
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_MSG("dnsTask(): osapiMessageReceive failed. %s:%d\n", 
              __FILE__, __LINE__);
      continue;
    }
    switch (msg.msgId)
    {
      ...
      default:
        LOG_MSG("dnsTask(): invalid message type:%d. %s:%d\n", 
