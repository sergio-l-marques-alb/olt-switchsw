...
L7_RC_t autoInstallStartStopSet(L7_BOOL start)
{
  ...
  {
    LOG_MSG("%s: LINE %d: incorrect input: %d\n",
  ...
  {
    LOG_MSG("%s (%d): configuration is not initialized\n",
  ...
  if (autoInstallRunTimeData->startStop != start)
  {
    ...
    {
      LOG_MSG("autoInstallStartStopSet(): message send failed! input = %d\n", start);
    }
...
L7_RC_t autoInstallCurrentStatusGet(L7_uchar8 *status)
{
  ...
  switch (autoInstallRunTimeData->currentState)
  {
    ...
    default:
      LOG_MSG("%s: LINE %d: invalid AutoInstall status %d\n",
              __FUNCTION__, __LINE__,
                                  ...
                                  L7_RC_t (*notifyFn)())
{
  ...
  {
    LOG_MSG("%s: LINE %d: invalid Component Id %d for registration.\n",
            __FUNCTION__, __LINE__, compId);
    return L7_FAILURE;
  }
  if (notifyFn == L7_NULLPTR)
  {
    LOG_MSG("%s: LINE %d: invalid Callback function by component Id %d for registration.\n",
            __FUNCTION__, __LINE__, compId);
    return L7_FAILURE;
  }
...
L7_RC_t autoInstallNotifyDeregister(L7_COMPONENT_IDS_t compId)
{
  ...
  if (compId <= L7_FIRST_COMPONENT_ID || compId >= L7_LAST_COMPONENT_ID)
  {
    LOG_MSG("%s: LINE %d: invalid Component Id %d for deregistration.\n",
            __FUNCTION__, __LINE__, compId);
    return L7_FAILURE;
  }
  for (index = 0; index < AUTO_INSTALL_NUM_EVENT_REGISTRATIONS_MAX; index ++)
