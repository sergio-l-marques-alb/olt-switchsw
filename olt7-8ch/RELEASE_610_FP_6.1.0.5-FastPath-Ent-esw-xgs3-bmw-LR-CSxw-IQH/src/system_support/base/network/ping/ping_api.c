...
pingVrIdSet( L7_ushort16 handle, L7_ushort16 vrId )
{
    ...
    {
        L7_LOGFNC( L7_LOG_SEVERITY_WARNING, "Invalid vrId to pingVrIdSet" );
        return L7_FAILURE;
    }
    ...
    if ( pingHandleToEntry( handle, &entry ) != L7_SUCCESS )
    {
        L7_LOGFNC( L7_LOG_SEVERITY_WARNING, "Invalid handle to pingVrIdSet" );
        osapiWriteLockGive( pingSessionLock_g );
        return L7_FAILURE;
    }
    entry->vrfId = vrId;
...
pingSessionStart( L7_ushort16 handle )
{
    ...
    {
        L7_LOGFNC( L7_LOG_SEVERITY_WARNING, "Invalid state - cannot start session");
        osapiWriteLockGive( pingSessionLock_g );
        return L7_FAILURE;
    }
