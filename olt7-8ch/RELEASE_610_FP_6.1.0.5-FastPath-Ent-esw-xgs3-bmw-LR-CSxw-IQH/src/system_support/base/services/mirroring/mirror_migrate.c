...
mirrorMigrateConfigV1V2Convert (mirrorCfgDataV1_t * pCfgV1, mirrorCfgDataV2_t * pCfgV2)
{
  ...
  if (sysIntfType != L7_PHYSICAL_INTF) 
  {
      ...
      if (sysIntfType != 0) 
      {
          LOG_MSG("Cannot migrate source interface %d/%d: invalid type (%d)\n", 
  ...
  if (sysIntfType != L7_PHYSICAL_INTF) 
  {
      ...
      if (sysIntfType != 0) 
      {
          LOG_MSG("Cannot migrate destination interface %d/%d: invalid type (%d)\n", 
                  slot, port, sysIntfType);
      }
  }  
  else
...
mirrorMigrateConfigV2V3Convert (mirrorCfgDataV2_t * pCfgV2, mirrorCfgDataV3_t * pCfgV3)
{
  ...
  if (sysIntfType != L7_PHYSICAL_INTF) 
  {
      ...
      if (sysIntfType != 0) 
      {
          LOG_MSG("Cannot migrate source interface : invalid type (%d)\n", sysIntfType);
      }
  }  
  else
  {
  ...
  if (sysIntfType != L7_PHYSICAL_INTF) 
  {
      ...
      if (sysIntfType != 0) 
      {
          LOG_MSG("Cannot migrate dest interface : invalid type (%d)\n", sysIntfType);
      }
  }
  else
  {
