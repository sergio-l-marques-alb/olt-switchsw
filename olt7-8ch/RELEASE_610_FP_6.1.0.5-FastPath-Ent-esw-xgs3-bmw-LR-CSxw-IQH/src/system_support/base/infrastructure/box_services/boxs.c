...
L7_RC_t boxsInitialize(void)
{
  ...
  {
    L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Failed to Create Box Services Semaphore.\n");
    return L7_FAILURE;
  }
      ...
      cnfgrIsFeaturePresent(L7_BOX_SERVICES_COMPONENT_ID, L7_BXS_XFP_STATUS_FEATURE_ID))
  {
    ...
    {
      L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Failed to Create Box Services Response Queue.\n");
      return L7_FAILURE;
    }
    ...
    {
      L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Failed to Create Box Services Request Queue.\n");
      return L7_FAILURE;
    }
    ...
    {
      L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Failed to Create Box Services Request Task.\n");
      return L7_FAILURE;
    }
    ...
    {
      L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Failed to Create Box Services Response Task.\n");
      return L7_FAILURE;
    }
  }
...
* @end
*********************************************************************/
L7_RC_t boxsHPCReceiveCallback(boxsMsg_t *msg, L7_uint32 length)
{
  boxsMsg_t toQ;
...
L7_RC_t boxsLocalDataRead(boxsData_t *boxsData)
{
  ...
  for (i = 0; i < min(fanCount, L7_MAX_FANS_PER_UNIT); i++) 
  {
	  ...
	  {
		  L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Error %d on call to sysapiHpcLocalFanDataGet(%d)\n",rc, i );
		  return L7_FAILURE;
	  }
  }
  ...
  for (i = 0; i < min(powerSupplyCount, L7_MAX_PW_MODS_PER_UNIT); i++) 
  {
	  ...
	  {
		  L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Error %d on call to sysapiHpcLocalPowerSupplyDataGet(%d)\n",rc, i );
		  return L7_FAILURE;
	  }
  }
  ...
  for (i = 0; i < min(sfpCount, L7_MAX_SFPS_PER_UNIT); i++) 
  {
	  ...
	  {
		  L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Error %d on call to sysapiHpcLocalSfpDataGet(%d)\n",rc, i );
		  return L7_FAILURE;
	  }
  }
  ...
  for (i = 0; i < min(xfpCount, L7_MAX_XFPS_PER_UNIT); i++) 
  {
	  ...
	  {
		  L7_LOGFNC(L7_LOG_SEVERITY_INFO, "Error %d on call to sysapiHpcLocalXfpDataGet(%d)\n",rc, i );
		  return L7_FAILURE;
	  }
  }
...
void boxsIssueTemperatureChangeEvent(L7_uint32 unitNum, L7_uint32 itemNum, BOXS_TEMPERATURE_CROSS_RANGE_EVENT_t trapEvent)
{
	...
	{
		LOG_MSG("Failed to send temperature change trap, errcode %d, unit %d item %d event %d\n",
    ...
    if (trapEvent == BOXS_EVENT_TEMP_ABOVE_THRESHOLD)
    {
      ...
      {
          LOG_MSG("Failed to send temperature rising trap, errcode %d, unit %d item %d event %d\n",
                         rc,  unitNum, itemNum, trapEvent);
      }
    }
}
...
void boxsIssueFanStateChangeEvent(L7_uint32 unitNum, L7_uint32 itemNum, BOXS_ITEM_EVENT_t trapEvent) 
{
	...
	{
		LOG_MSG("Failed to send fan state change trap, errcode %d, unit %d item %d event %d\n",
					   rc,  unitNum, itemNum, trapEvent);
	}
    /* Customer trap */
...
void boxsIssuePowerSupplyStateChangeEvent(L7_uint32 unitNum, L7_uint32 itemNum, BOXS_ITEM_EVENT_t trapEvent)
{
	...
	{
		LOG_MSG( "Failed to send power supply state change trap, errcode %d, unit %d item %d event %d\n",
					   rc,  unitNum, itemNum, trapEvent);
	}
    /* Customer trap */
...
void boxsIssueSFPStateChangeEvent(L7_uint32 unitNum, L7_uint32 itemNum, L7_uint32 port, BOXS_ITEM_EVENT_t trapEvent)
{
	...
	{
		LOG_MSG( "Failed to send SFP state change trap, errcode %d, unit %d item %d event %d\n",
					   rc,  unitNum, itemNum, trapEvent);
	}
}
...
void boxsIssueXFPStateChangeEvent(L7_uint32 unitNum, L7_uint32 itemNum, L7_uint32 port, BOXS_ITEM_EVENT_t trapEvent)
{
	...
	{
		LOG_MSG( "Failed to send SFP+/XFP state change trap, errcode %d, unit %d item %d event %d\n",
					   rc,  unitNum, itemNum, trapEvent);
	}
}
...
void boxsRspTask(void)
{
  ...
  if (boxsReqQ == L7_NULLPTR)
  {
    L7_LOGFNC(L7_LOG_SEVERITY_INFO, "boxs Req Task: Failed to create");
    return;
  }
...
void boxsReqTask(void)
{
  ...
  if (boxsRspQ == L7_NULLPTR)
  {
    L7_LOGFNC(L7_LOG_SEVERITY_INFO, "boxs Req Task: Failed to create, no Response Queue");
    return;
  }
