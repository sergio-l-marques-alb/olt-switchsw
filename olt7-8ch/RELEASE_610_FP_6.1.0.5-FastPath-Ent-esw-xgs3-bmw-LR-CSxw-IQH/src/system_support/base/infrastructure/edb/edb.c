...
L7_RC_t edbRegister(void)
{
  ...
  if (nimRegisterIntfChange(L7_EDB_COMPONENT_ID, edbObjectPortChangeCallback) != L7_SUCCESS)
    return L7_FAILURE;
...
L7_RC_t edbObjectUnitJoinCallback(L7_uint32 unit)
{
  ...
  if (unitMgrUnitTypeGet(unit, &unit_type) == L7_SUCCESS)
  {
    ...
    {
	  L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_SNMP_COMPONENT_ID, "EDB Callback: Unit Join: %d. "
											  "A new unit has joined the stack.\n", unit);

      /* send the unit creation message */
      if (osapiMessageSend(edbMessageQueue, (void*)&temp_message, sizeof(temp_message), L7_WAIT_FOREVER, L7_MSG_PRIORITY_NORM ) == L7_SUCCESS)
      {
...
L7_RC_t edbObjectUnitLeaveCallback(L7_uint32 unit)
{
  ...
  LOG_MSG("EDB Callback: Unit Leave: %d.\n", unit);
...
L7_RC_t edbObjectUnitJoin(L7_uint32 unit)
{
  ...
  if (edbObjectCreate(&unit_p) == L7_SUCCESS)
  {
    ...
    if (edbObjectChildInsert(unit_p, edbTree_g) == L7_SUCCESS)
    {
      ...
      LOG_MSG("EDB Failure: Error renumbering tree after adding Unit %d\n", unit);
    }
    else
    {
      LOG_MSG("EDB Failure: Error adding Unit %d\n", unit);
      /* remove the unit */
...
L7_RC_t edbObjectUnitLeave(L7_uint32 unit)
{
  ...
  {
    LOG_MSG("EDB Failure: Could not find Unit %d to remove.\n", unit);
...
L7_RC_t edbObjectDestroy(L7_EDB_OBJECT_t *object_p)
{
  ...
  if (object_p == L7_NULLPTR ||
      ...
      object_p->prevSibling != L7_NULLPTR)
  {
    LOG_MSG("EDB:edbObjectDestroy failure: arguments are NULL\n");
    return L7_FAILURE;
  }
  ...
  while (object_p->child != L7_NULLPTR)
  {
    ...
    {
      LOG_MSG("EDB:edbObjectDestroy failure: Could not remove children.\n");
      return L7_ERROR;
    }
  }
...
L7_RC_t edbObjectRemove(L7_EDB_OBJECT_t *object_p)
{
  ...
  if (object_p == L7_NULLPTR)
  {
    LOG_MSG("EDB:edbObjectRemove failure: object_p == L7_NULLPTR\n");
    return L7_FAILURE;
  }
...
L7_RC_t edbObjectReindex(L7_EDB_OBJECT_t *object_p, L7_uint32 *index)
{
  ...
  if (object_p == L7_NULLPTR)
  {
    LOG_MSG("EDB:edbObjectReindex failure: arguments are NULL\n");
    return L7_FAILURE;
  }
  ...
  *index += 1;
  ...
  if (object_p->child != L7_NULLPTR)
  {
    ...
    while (temp_p != L7_NULLPTR)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_MSG("EDB:edbObjectReindex failure: recursive call failed\n");
        return rc;
      }
      temp_p = temp_p->nextSibling;
    }
