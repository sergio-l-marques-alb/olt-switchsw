    ...
    else
    {
      ...
      */
                                                                   ...
                                                                   cdcmpdu->slotId))
      {
        ...
        LOG_MSG("cmgrReceiveCardPlugIn(): Card mismatch \n");
      }
                              ...
                              SYSAPI_CARD_INDEX_t cardIndex)
{
  ...
  if((cardType == SYSAPI_CARD_TYPE_LINE) ||
     (cardType == SYSAPI_CARD_TYPE_CONTROL))
  {
    if(boardIdValidate_g == L7_TRUE)
    {
      /* Get the Board Id of Control / Line Module */
      if(L7_SUCCESS == (rc = sysapiHpcModuleBoardIdGet(phySlot, &boardId)))
      {
        /* Verify the board Id against the supported board Ids */
        rc = authenticateBoardId(unit, phySlot, boardId,cardType, cardIndex);
      ...
      {
        CMGR_DEBUG(CMGR_LOG_FLAG, "cmgrProcessCardPlugIn: Failure reading BoardId on slot %d\n", phySlot);
        LOG_MSG("Failure reading BoardId on slot %d\n", phySlot);
        return rc;
      }
    }

    /* clear any pending alarms against this card */
    cmgrAlarmClear(cardType, L7_ALARM_TYPE_MODULE_UNPLUG, cardIndex);
    cmgrAlarmClear(cardType, L7_ALARM_TYPE_MODULE_FAILURE, cardIndex);
    cmgrAlarmClear(cardType, L7_ALARM_TYPE_MODULE_MISMATCH, cardIndex);
    cmgrAlarmClear(cardType, L7_ALARM_TYPE_SLOT_POWERED_OFF, cardIndex);
    /* clear any pending card unsupported alarm */
    cmgrCardUnSupportedAlarmClear(phySlot);
  }
...
void cmgrReceiveCardUnplug(cmgr_cmpdu_t *cmpdu)
{
 ...
 if ((cdcmpdu->slotId < 0) ||
  ...
  if (cmgrState == L7_CNFGR_STATE_P3 ||
      cmgrState == L7_CNFGR_STATE_E)
  {
    cmgrAlarmRaise (cdcmpdu->cardType, cdcmpdu->cardStatus, cdcmpdu->cardIndex);
                              ...
                              SYSAPI_CARD_TYPE_t cardType, SYSAPI_CARD_INDEX_t cardIndex)
{
  ...
  /* raise module unplug alarm */
  if(cardType != SYSAPI_CARD_TYPE_INVALID)
    cmgrAlarmRaise (cardType, L7_ALARM_TYPE_MODULE_UNPLUG, cardIndex);

  rc = cmgrCMDBCardTypeSet(unit, slot, SYSAPI_CARD_TYPE_INVALID);
  rc = cmgrCMDBModuleIndexSet(unit, slot, SYSAPI_CARD_INDEX_INVALID);

  /* set power OFF if it is an LM slot */
  if(cardType == SYSAPI_CARD_TYPE_LINE)
  {
    if(cardIndex <= CMGR_MAX_LMS_PER_UNIT_f)
    {
      /* Update LM LED status */
      sysapiHpcLedUpdate (phySlot, SYSAPI_CARD_EVENT_UNPLUG);
    }
    else
    {
      rc = L7_FAILURE;
      LOG_MSG("cmgrProcessCardUnplug: Failure, wrong Card index %d \n", cardIndex);
    }
  }
                               ...
                               SYSAPI_CARD_TYPE_t cardType, SYSAPI_CARD_INDEX_t cardIndex)
{
  ...
  /* raise module failure alarm */
  if(cardType != SYSAPI_CARD_TYPE_INVALID)
    cmgrAlarmRaise (cardType, L7_ALARM_TYPE_MODULE_FAILURE, cardIndex);
  ...
  if(cardType == SYSAPI_CARD_TYPE_LINE)
  {
    if(cardIndex <= CMGR_MAX_LMS_PER_UNIT_f)
    {
      /* In case of card failure, Turn OFF power to LM slot */
      (void) cmgrSlotPowerSet (unit, cmgrCMDBPhySlotIdGet(unit, slot), POWER_OFF, POWER_SYSTEM);

      /* Update LM LED status */
      sysapiHpcLedUpdate (cmgrCMDBPhySlotIdGet(unit, slot), SYSAPI_CARD_EVENT_FAILURE);
    }
    else
      return L7_FAILURE;
  }
...
L7_RC_t cmgrReceiveStatusUpdate(cmgr_cmpdu_t* cmpdu)
{
      ...
      (cmpdu->unit > CMGR_MAX_UNITS_f))
  {
    CMGR_DEBUG(CMGR_LOG_FLAG, "cmgrReceiveStatusUpdate: unit:%d Out of range\n",
               cmpdu->unit);
    LOG_MSG( "cmgrReceiveStatusUpdate: unit:%d Out of range\n", cmpdu->unit);
    return L7_ERROR;
  }
  ...
  if (cmgrCMDBUnitConnectedGet(cmpdu->unit) == L7_FALSE)
  {
        ...
        (thisUnit == mgmtUnit))
    {
      CMGR_DEBUG(CMGR_LOG_FLAG,
                 ...
                 cmpdu->unit);
      LOG_MSG("cmgrReceiveStatusUpdate: unit:%d Unconnected\n", cmpdu->unit);
    }
    return L7_ERROR;
  }
    ...
    if (smCmpdu->slotStatus == L7_TRUE)
    {
      ...
      else
      {
        CMGR_DEBUG(CMGR_LOG_FLAG,
                   ...
                   cmpdu->unit, smCmpdu->phySlotId, smCmpdu->slotId);
        LOG_MSG("cmgrReceiveStatusUpdate: Card plugin for unit:%d phySlotId:%d slotId:%d- FAILED\n",
                cmpdu->unit, smCmpdu->phySlotId, smCmpdu->slotId);
      }
    }
    else
...
*
* @param  cmgr_cmpdu_statusUpdate_t *cdcmpdu (input)
*
* @returns L7_SUCCESS,     if success
                        ...
                        SYSAPI_CARD_INDEX_t cardIndex)
{
  ...
  L7_uint32 mgrUnit, unit, phySlot;
  ...
  switch (cardType)
  {
    ...
    case SYSAPI_CARD_TYPE_LINE:
    {
      switch (alarmType)
      {
        case L7_ALARM_TYPE_MODULE_UNPLUG:
        {
          switch (cardIndex)
          {
            case SYSAPI_CARD_INDEX_1:
              alarmId = L7_ALARM_ID_LM0_UNPLUGGED;
              strncpy (addInfo, "LM0 Unplugged", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_2:
              alarmId = L7_ALARM_ID_LM1_UNPLUGGED;
              strncpy (addInfo, "LM1 Unplugged", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_3:
              alarmId = L7_ALARM_ID_LM2_UNPLUGGED;
              strncpy (addInfo, "LM2 Unplugged", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_4:
              alarmId = L7_ALARM_ID_LM3_UNPLUGGED;
              strncpy (addInfo, "LM3 Unplugged", CMGR_ALARM_DESC_SIZE);
              break;
            default:
              CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Line Module Index %d unavailable\n",
                            cardIndex);
              LOG_MSG ("cmgrAlarmRaise: Line Module Index %d unavailable\n", cardIndex);
              break;
          }
        }
        break;
        case L7_ALARM_TYPE_MODULE_UNSUPPORTED:
        {
          switch (cardIndex)
          {
            case SYSAPI_CARD_INDEX_1:
              alarmId = L7_ALARM_ID_SLOT2_MODULE_UNSUPPORTED;
              strncpy (addInfo, "Unsupported Module in slot 2", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_2:
              alarmId = L7_ALARM_ID_SLOT3_MODULE_UNSUPPORTED;
              strncpy (addInfo, "Unsupported Module in slot 3", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_3:
              alarmId = L7_ALARM_ID_SLOT4_MODULE_UNSUPPORTED;
              strncpy (addInfo, "Unsupported Module in slot 4", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_4:
              alarmId = L7_ALARM_ID_SLOT5_MODULE_UNSUPPORTED;
              strncpy (addInfo, "Unsupported Module in slot 5", CMGR_ALARM_DESC_SIZE);
              break;
            default:
              LOG_MSG ("cmgrAlarmRaise: Line Module Index %d unsupported\n", cardIndex);
              break;
          }
        }
        break;

        case L7_ALARM_TYPE_MODULE_FAILURE:
        {
          switch (cardIndex)
          {
            case SYSAPI_CARD_INDEX_1:
              alarmId = L7_ALARM_ID_LM0_FAILED;
              strncpy (addInfo, "LM0 Failed", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_2:
              alarmId = L7_ALARM_ID_LM1_FAILED;
              strncpy (addInfo, "LM1 Failed", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_3:
              alarmId = L7_ALARM_ID_LM2_FAILED;
              strncpy (addInfo, "LM2 Failed", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_4:
              alarmId = L7_ALARM_ID_LM3_FAILED;
              strncpy (addInfo, "LM3 Failed", CMGR_ALARM_DESC_SIZE);
              break;
            default:
              CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Line Module Index %d unavailable\n",
                            cardIndex);
        ...
        break;
      case L7_ALARM_TYPE_MODULE_MISMATCH:
      {
        switch (cardIndex)
        {
          case SYSAPI_CARD_INDEX_1:
            alarmId = L7_ALARM_ID_LM0_BOARD_ID_MISMATCH;
            strncpy (addInfo, "LM0 BoardId Mismatch", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_2:
            alarmId = L7_ALARM_ID_LM1_BOARD_ID_MISMATCH;
            strncpy (addInfo, "LM1 BoardId Mismatch", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_3:
            alarmId = L7_ALARM_ID_LM2_BOARD_ID_MISMATCH;
            strncpy (addInfo, "LM2 BoardId Mismatch", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_4:
            alarmId = L7_ALARM_ID_LM3_BOARD_ID_MISMATCH;
            strncpy (addInfo, "LM3 BoardId Mismatch", CMGR_ALARM_DESC_SIZE);
            break;
          default:
            CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Line Module Index %d unavailable\n",
                          cardIndex);
      ...
      case L7_ALARM_TYPE_SLOT_POWERED_OFF:
      {
        switch (cardIndex)
        {
          case SYSAPI_CARD_INDEX_1:
            alarmId = L7_ALARM_ID_LM0_IS_INSERTED_IN_POWER_OFF_SLOT;
            strncpy (addInfo, "LM0 in Powered OFF slot", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_2:
            alarmId = L7_ALARM_ID_LM1_IS_INSERTED_IN_POWER_OFF_SLOT;
            strncpy (addInfo, "LM1 in Powered OFF slot", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_3:
            alarmId = L7_ALARM_ID_LM2_IS_INSERTED_IN_POWER_OFF_SLOT;
            strncpy (addInfo, "LM2 in Powered OFF slot", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_4:
            alarmId = L7_ALARM_ID_LM3_IS_INSERTED_IN_POWER_OFF_SLOT;
            strncpy (addInfo, "LM3 in Powered OFF slot", CMGR_ALARM_DESC_SIZE);
            break;
          default:
            CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Line Module Index %d unavailable\n",
                          cardIndex);
      ...
      default:
        CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Line Module status %d unknown\n",
                      alarmType);
        LOG_MSG ("cmgrAlarmRaise: Line Module status %d unknown\n", alarmType);
        break;
      } /* Line module status */
    } /* Line Module */
      break;
    ...
    {
      /* These events are taken care by Fan unit manager */
      /* Do nothing */
      break;
    }
    ...
    {
      /* These events are taken care by Power unit manager */
      /* Do nothing */
      break;
    }

    /* standby control module event */
    case SYSAPI_CARD_TYPE_CONTROL:
    {
      if (alarmType ==  L7_ALARM_TYPE_MODULE_UNSUPPORTED)
      {
        switch (cardIndex)
        {
          case SYSAPI_CARD_INDEX_1:
            alarmId = L7_ALARM_ID_SLOT0_MODULE_UNSUPPORTED;
            strncpy (addInfo, "Unsupported Module in slot 0", CMGR_ALARM_DESC_SIZE);
            break;
          case SYSAPI_CARD_INDEX_2:
            alarmId = L7_ALARM_ID_SLOT1_MODULE_UNSUPPORTED;
            strncpy (addInfo, "Unsupported Module in slot 1", CMGR_ALARM_DESC_SIZE);
            break;
          default:
            break;
         }
      ...
      if (unitMgrMgrNumberGet(&mgrUnit) != L7_SUCCESS)
        return L7_FAILURE;
      if(L7_SUCCESS !=
         (sysapiHpcSlotMapMPPhysicalSlotGet(cardType, cardIndex, &phySlot)))
         return L7_FAILURE;
      if(L7_SUCCESS != (sysapiHpcSlotMapUSUnitNumGet(phySlot, &unit)))
         return L7_FAILURE;
      ...
      if(unit != mgrUnit)
      {
        ...
        {
          case L7_ALARM_TYPE_MODULE_UNPLUG:
            alarmId = L7_ALARM_ID_STBY_CM_UNPLUGGED;
            strncpy (addInfo, "Standby CFM Unplugged", CMGR_ALARM_DESC_SIZE);
            break;
          case L7_ALARM_TYPE_MODULE_FAILURE:
            alarmId = L7_ALARM_ID_STBY_CM_FAILED;
            strncpy (addInfo, "Standby CFM Failed", CMGR_ALARM_DESC_SIZE);
            break;
          default:
            CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: standby CFM status %d unknown\n",
                          alarmType);
            LOG_MSG ("cmgrAlarmRaise: standby CFM status %d unknown\n", alarmType);
            break;
        }
      }
    }
    ...
    default:
      CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmRaise: Unexpected cardtype %d\n", cardType);
      LOG_MSG("cmgrAlarmRaise: Unexpected cardtype %d\n", cardType);
      break;
  }
  if (alarmId != L7_ALARM_ID_UNKNOWN)
  {
    ...
    /* raise alarm */
    rc =  alarmMgrAlarmRaise(alarmId, alarmTimeStamp, addInfo);
  }

  CMGR_DEBUG(CMGR_LOG_FLAG, "cmgrAlarmRaise: alarmId:%d Info:%s\n", alarmId, addInfo);
  return rc;
}
                        ...
                        SYSAPI_CARD_INDEX_t cardIndex)
{
  ...
  switch (cardType)
  {
    ...
    case SYSAPI_CARD_TYPE_LINE:
    {
        ...
        case L7_ALARM_TYPE_MODULE_UNPLUG:
        {
          switch (cardIndex)
          {
            ...
            case SYSAPI_CARD_INDEX_4:
              ...
              break;
            default:
              CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmClear: Line Module Index %d unavailable",
                            cardIndex);
              LOG_MSG("cmgrAlarmClear: Line Module Index %d unavailable", cardIndex);
              break;
          }
          break;
        }
      ...
      case L7_ALARM_TYPE_MODULE_MISMATCH:
        {
          switch (cardIndex)
          {
            case SYSAPI_CARD_INDEX_1:
              alarmId = L7_ALARM_ID_LM0_BOARD_ID_MISMATCH;
              strncpy (addInfo, "LM0 Board Id mismatch", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_2:
              alarmId = L7_ALARM_ID_LM1_BOARD_ID_MISMATCH;
              strncpy (addInfo, "LM1 Board Id mismatch", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_3:
              alarmId = L7_ALARM_ID_LM2_BOARD_ID_MISMATCH;
              strncpy (addInfo, "LM2 Board Id mismatch", CMGR_ALARM_DESC_SIZE);
              break;
            case SYSAPI_CARD_INDEX_4:
              alarmId = L7_ALARM_ID_LM3_BOARD_ID_MISMATCH;
              strncpy (addInfo, "LM3 Board Id mismatch", CMGR_ALARM_DESC_SIZE);
              break;
            default:
              CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmClear: Line Module Index %d unavailable",
                            cardIndex);
      ...
      case L7_ALARM_TYPE_SLOT_POWERED_OFF:
        ...
        default:
        {
          CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmClear: Line Module alarmType %d unknown",
                        alarmType);
          LOG_MSG ("cmgrAlarmClear: Line Module alarmType %d unknown", alarmType);
          break;
        }
      } /* Line module status */
    } /* Line Module */
      ...
      if(unit != mgrUnit)
      {
        ...
        switch (alarmType)
        {
          ...
          case L7_ALARM_TYPE_MODULE_FAILURE:
            ...
            break;
          default:
            CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmClear: standby CFM status %d unknown",
                          alarmType);
            LOG_MSG("cmgrAlarmClear: standby CFM status %d unknown", alarmType);
            break;
        }
      }
      break;
    ...
    default:
      CMGR_DEBUG (CMGR_LOG_FLAG, "cmgrAlarmClear: Unexpected cardtype %d", cardType);
      LOG_MSG ("cmgrAlarmClear: Unexpected cardtype %d", cardType);
      break;
  }
  ...
  CMGR_DEBUG(CMGR_LOG_FLAG, "cmgrAlarmClear: alarmId:%d Info:%s\n", alarmId, addInfo);
  return rc;
}
