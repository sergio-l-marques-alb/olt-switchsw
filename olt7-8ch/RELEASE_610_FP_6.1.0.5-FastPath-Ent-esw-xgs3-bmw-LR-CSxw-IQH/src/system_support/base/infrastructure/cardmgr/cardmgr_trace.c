...

{
  ...
  {
    LOG_MSG("CMGR: unable to create the trace buffer\n");
    cmgrTraceHandle = ~0;
  }
  else
  {
...
void cmgrTraceFini(void)
{
  ...
  if (cmgrTraceHandle != ~0)
  {
    ...
    {
      LOG_MSG("CMGR: unable to delete the trace buffer\n");
    }
    else
    {
      cmgrTraceHandle = ~0;
...
void cmgrTraceModeApply(L7_BOOL enable)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG("cmgrTraceModeApply: Unable to apply CMGR trace mode (%u), "
