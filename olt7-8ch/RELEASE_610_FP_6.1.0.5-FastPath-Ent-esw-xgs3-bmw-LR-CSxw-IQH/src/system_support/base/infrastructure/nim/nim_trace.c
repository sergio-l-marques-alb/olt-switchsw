...
static L7_uint32 nimTraceIntIfNum = 0;
                       ...
                       NIM_HANDLE_t       handle)
{
  ...
  if ((nimTraceHandle != ~0) &&
      ...
      ((nimTraceIntIfNum == 0) || (intIfNum == nimTraceIntIfNum)))
  {
...
L7_RC_t nimTracePortEventFormat(L7_ushort16 traceId, L7_uchar8 *pDataStart)
{
  ...
  cnfgrApiComponentNameGet((L7_COMPONENT_IDS_t)((info >> 24) & 0xff), compName);
  SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                "%s, ", compName);
...
void nimTraceModeApply(L7_BOOL enable)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_MSG("nimTraceModeApply: Unable to apply NIM trace mode (%u), "
...
void nimTraceHelp(void)
{
  ...
  SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
                "%-30s - 0 for all intf, >0 for an intf\n","nimTraceIntfNumSet()");
