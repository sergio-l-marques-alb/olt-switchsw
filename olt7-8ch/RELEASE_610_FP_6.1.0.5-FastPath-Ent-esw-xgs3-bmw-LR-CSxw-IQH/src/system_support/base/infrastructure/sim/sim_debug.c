...
#define PING_DEBUG_PACKET_RX_FORMAT "Pkt RX - Intf: %d/%d/%d(%d), SRC_IP:%s, DEST_IP:%s, Type:%s"
#define PING_DEBUG_PACKET_TX_FORMAT "Pkt TX - Intf: %d/%d/%d(%d), SRC_IP:%s, DEST_IP:%s, Type:%s"
...
void pingDebugPacketRxTrace(L7_uint32 intIfNum, L7_uchar8 *buff)
{
  ...
  if (usmDbUnitSlotPortGet(intIfNum, &unit, &slot, &port) == L7_SUCCESS)      
  {
    PING_USER_TRACE(PING_DEBUG_PACKET_RX_FORMAT, 
                      unit, slot, port,intIfNum,srcIp,destIp,type);
  }
...
int simDebugSetSystemConfigMode(int mode)
{
  ...
  else
  {
    ...
    {
      if(mode==L7_SYSCONFIG_MODE_NONE)
       {
         usmDbDhcp_release (0);
       }
    }         	
