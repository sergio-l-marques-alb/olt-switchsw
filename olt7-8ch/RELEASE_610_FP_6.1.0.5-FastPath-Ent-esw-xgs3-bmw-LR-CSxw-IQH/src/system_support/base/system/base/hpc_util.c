...
static lmSlotPowerCfgData_t lmSlotPower_config_data;
...
L7_RC_t hpcInit(void)
{
  ...
  if (hpcHardwareCfgDataRequired() == L7_TRUE)
  {
    ...
    {
      LOG_MSG("hpcInit: bad return code from hpcCfgDataRetrieve() call.\n");
    }
  }
  else
  {
  ...
  {
    LOG_MSG("hpcInit: bad return code from hpcApplyCfgData() call.\n");
    return(L7_ERROR);
  }
  ...
  {
    LOG_MSG("hpcInit: bad return code from hpcHardwareInit() call.\n");
    return(L7_ERROR);
  }

  if (hpcDriverAsfInit() != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from hpcDriverAsfInit() call.\n");
    return(L7_ERROR);
  }
 ...
 /* special debug code */
  ...
  /* initialize the platform specific HeartBeat Handler */
  if (sysapiHpcHeartBeatHndlrInit() != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from sysapiHpcHeartBeatHndlrInit() call\n");
    return(L7_ERROR);
  }

  /* initialize the platform specific HPC Sensor Handler */
  if (sysapiHpcSensorHndlrInit() != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from sysapiHpcSensorHndlrInit() call\n");
    return(L7_ERROR);
  }

  /* initialize the platform specific HPC Power Handler */
  if (sysapiHpcPwrHndlrInit() != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from sysapiHpcPwrHndlrInit() call.\n");
    return(L7_ERROR);
  }

  if (L7_SUCCESS != (result = sysapiHpcEventHandlerInit()))
  {
    LOG_MSG("hpcInit: bad return code from hpcEventHandlerInit call.\n");
    return(result);
  }
 
  if (L7_SUCCESS != (result = sysapiHpcLedHandlerInit()))
  {
    LOG_MSG("hpcInit: bad return code from hpcLedHandlerInit call.\n");
    return(result);
  }
  
  /* initialize the platform specific Redundancy Controller (HRC) */
  if (sysapiHpcHrcInit() != L7_SUCCESS)
  {
    LOG_MSG("hpcInit: bad return code from sysapiHpcHrcInit() call\n");
    return(L7_ERROR);
  }
...
L7_RC_t hpcIfaceMacGet(L7_INTF_TYPES_t type, L7_uint32 slot, L7_uint32 port, L7_uchar8 *mac_addr)
{
  ...
  {
    if(hpcHardwareIfaceMacGet(type, slot, port, mac_addr, pLocalUnitDescriptor) == L7_SUCCESS)
    {
      rc = L7_SUCCESS;
    }
  }
...
void hpcStackEventCallback(hpcStackEventMsg_t stack_event)
{
  ...
  {
    LOG_MSG("hpcStackEventCallback: unknown event (%d)\n", event);
    return;
  }
  ...
  switch (event)
  {
    ...
    case HPC_EVENT_LOCAL_STACK_PORT_LINK_STATE:
    {
      break;
    }
    case HPC_EVENT_STACK_TOPOLOGY_CHANGE:
    {
      break;
    }
    default:
                                      ...
                                      void (*notify)(hpcStackEventMsg_t eventMsg))
{
  ...
  {
    LOG_MSG("%s: registrarId %d out of range\n", __FUNCTION__, registrarId);
    rc = L7_FAILURE;
  }
  else if ((event >= HPC_EVENT_LAST) || (event <= 0))
  {
    LOG_MSG("%s: eventID %d out of range\n", __FUNCTION__, event);
    rc = L7_FAILURE;
  }
  else
  {
  ...
  if (hpcHardwareCfgDataRetrieve(&hpc_config_data, sizeof(hpc_config_data)) == L7_SUCCESS)
 {
    ...
    if (hpc_config_data.crc == crc)
    {
      ...
      if (hpc_config_data.version != HPC_CONFIG_VER_CURRENT)
      {
        ...
        LOG_MSG("hpcCfgDataRetrieve: version mismatch in retrieved config data.\n");
    ...
    else
    {
      ...
      LOG_MSG("hpcCfgDataRetrieve: bad crc in retrieved config data.\n");
      result = hpcCfgDataRestore();
    }
  }
  else
  {
    ...
    LOG_MSG("hpcCfgDataRetrieve: error from hpcHardwareCfgDataRetrieve().\n");
    result = hpcCfgDataRestore();
  }
  return(result);
}
...
void hpcCfgDataSetDefault(void)
{
  ...
  for(index =0; index < platLineModulesTotalMaxPerUnitGet(); index++)
  {
    lmSlotPower_config_data.lmSlotPower[index].status  = POWER_ON;
    lmSlotPower_config_data.lmSlotPower[index].control = POWER_ADMIN;
  }
}
...
L7_RC_t hpcCfgDataApply(hpcCfgData_t *cfg_data)
{
  ...
  do
  {
    ...
    {
      LOG_MSG("hpcCfgDataApply: error from hpcHardwareAdminPrefSet().\n");
    }
    ...
    {
      LOG_MSG("hpcCfgDataApply: error from hpcHardwareUnitNumberSet().\n");
      break;
    }
    for (stack_port_index = 0; stack_port_index < L7_MAX_STACK_PORTS_PER_UNIT; stack_port_index++)
    {
      if (cfg_data->stack_ports[stack_port_index].inuse == L7_TRUE)
      {
        ...
        {
          LOG_MSG("hpcCfgDataApply: error from hpcHardwareStackPortSet().\n");
          break;
        }
      }
    }
                                     ...
                                     L7_uint32 registrarID)
{
  if ((registrarID >= L7_LAST_COMPONENT_ID) | (registrarID <= 0))
  {
    LOG_MSG("hpcCardEventCallbackRegister: registrarID %d out of range\n", registrarID);
    return(L7_FAILURE);
  }
  else
  {
    ...
    {
      LOG_MSG("hpcCardEventCallbackRegister: bad return code from hpcHardwareCardEventCallbackRegister() for registrarID %d\n", registrarID);
      return(L7_FAILURE);
    }
    return(L7_SUCCESS);
  }
...
{
#ifdef L7_CHASSIS
  L7_uint32 phySlotNum;
  L7_uint32 unitNum;

  sysapiHpcCurrentSlotGet(&phySlotNum);
  (void)sysapiHpcSlotMapUSUnitNumGet(phySlotNum, &unitNum);
                            ...
                            POWER_STATUS_t status, POWER_CONTROL_t source)
{
  ...
  SYSAPI_CARD_INDEX_t     moduleIndex;
    ...
    /* Get Module Type/index for a given physical Slot */
  if((rc = sysapiHpcSlotMapMPModuleInfoGet(phySlot, &moduleType,
                                           &moduleIndex)) != L7_SUCCESS)
  {
    return rc;
  }
  if(SYSAPI_CARD_TYPE_LINE != moduleType)
  {
    return L7_FAILURE;
  }

  /* check if this is really news to us */
  if ((lmSlotPower_config_data.lmSlotPower[moduleIndex].status != status)
      || (lmSlotPower_config_data.lmSlotPower[moduleIndex].control != source))
  {
    if ((rc = hpcPwrSlotPowerStatusSet(phySlot, status, source)) == L7_SUCCESS)
    /* if application of change to hardware found no problems... */
    {
      /* Update the database */
      lmSlotPower_config_data.lmSlotPower[moduleIndex].status = status;
      lmSlotPower_config_data.lmSlotPower[moduleIndex].control = source;
    }
  }
  return(rc);
                            ...
                            POWER_STATUS_t *pStatus, POWER_CONTROL_t *pSource)
{
  ...
  *pStatus = lmSlotPower_config_data.lmSlotPower[moduleIndex].status;
  *pSource = lmSlotPower_config_data.lmSlotPower[moduleIndex].control;
...
L7_RC_t hpcStackPortSet(L7_uint32 slot, L7_uint32 port, L7_BOOL stacking_enable)
{
  ...
  */
  ...
  {
    LOG_MSG("hpcStackPortSet: Tried to add more than L7_MAX_STACK_PORTS_PER_UNIT stack ports.\n");
    rc = L7_FAILURE;
  }
  return(rc);
} 
                                   ...
                                   L7_uint32 registrarID)
{
  if ((registrarID >= L7_LAST_COMPONENT_ID) | (registrarID <= 0))
  {
    LOG_MSG("hpcReceiveCallbackRegister: registrarID %d out of range\n", registrarID);
    return(L7_FAILURE);
  }
  else
  {
...
L7_RC_t hpcBroadcastMessageSend(L7_uint32 registrarID, L7_uint32 msg_length, L7_uchar8* buffer)
{
  ...
  if ((registrarID >= L7_LAST_COMPONENT_ID) | (registrarID <= 0))
  {
    LOG_MSG("hpcBroadcastMessageSend: registrarID %d out of range\n", registrarID);
  }
  else
  {
...
L7_RC_t hpcMessageSend(L7_uint32 registrarID, L7_uint32 unit, L7_uint32 msg_length, L7_uchar8* buffer)
{
  ...
  if ((registrarID >= L7_LAST_COMPONENT_ID) | (registrarID <= 0))
  {
    LOG_MSG("hpcMessageSend: registrarID %d out of range\n", registrarID);
  }
  else
  {
    /* check if this registrar has registered with the transport */
    if (hpcMessageReceiveNotifyList[registrarID].registrarID == registrarID)
    {
      ...
      { 
        LOG_MSG("Mesg len %d more than HPC can send from registrarID %d\n", msg_length, registrarID);        
      }   
...
void hpcTransportMessageRecvHandler(L7_uint32 receive_id, L7_uchar8* buffer, L7_uint32 msg_len)
{
  ...
  {
    LOG_MSG("hpcTransportMessageRecvHandler: receive_id %d out of range\n", receive_id);
  }
  else
  {
    if (hpcMessageReceiveNotifyList[receive_id].registrarID == receive_id)
    {
      ...
      {
        hpcMessageReceiveNotifyList[receive_id].notify(buffer, msg_len);
...
void hpcDebugTransportTestCb(L7_uchar8* buffer, L7_uint32 msg_len)
{
...
void hpcDebugRemoteDevshellCb(L7_uchar8* buffer, L7_uint32 msg_len)
{
  void *command_text;
  L7_uint32 thread_id;
...
void hpcDebugIfaceMacGet(L7_INTF_TYPES_t type, L7_uint32 slot, L7_uint32 port)
{
  ...
  if (hpcIfaceMacGet(type, slot, port, mac) == L7_SUCCESS)
  {
    printf("The resulting mac for (%d, %d) is ", slot, port);
...
{
  return hpcHardwareProcessStackTopoChange(managerFailed);
}
...
{
  return hpcHardwareProcessManagerLinkDown(reportingUnit, reportingUnitKey, 
                                           stkPortHpcIndex, nhopCpuKey, 
                                           managerFailed);
}
...
{
  return hpcHardwareNotifyStandbyStatus(stbyStatus);
}
