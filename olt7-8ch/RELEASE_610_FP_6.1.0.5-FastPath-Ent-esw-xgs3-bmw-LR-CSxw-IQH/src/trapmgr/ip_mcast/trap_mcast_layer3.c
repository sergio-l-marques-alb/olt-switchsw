...
L7_RC_t trapMgrPimTrapNeighborLoss(L7_uint32 pimNeighborLossIfIndex, L7_uint32 pimNeighborUpTime, L7_BOOL isPimSm)
{
  ...
  {
    if (!isPimSm)
    sprintf(trapStringBuf, pimNeighborLoss_str, pimNeighborLossIfIndex);
    else
    {
      sprintf(trapStringBuf, pimSmNeighborLoss_str, pimNeighborUpTime);
    }
...
L7_RC_t trapMgrDvmrpTrapNeighborLoss(L7_uint32 dvmrpInterfaceLocalAddress, L7_int32 dvmrpNeighborState)
{
  ...
  if (trapMgrCfgData.trapDvmrp & L7_DVMRP_TRAP_NEIGHBOR_LOSS)
  {
    ...
    case L7_DVMRP_NEIGHBOR_STATE_ONEWAY:
      sprintf(dvmrpNeighborStateBuf, dvmrpNeighborStateOneway);
      break;
    case L7_DVMRP_NEIGHBOR_STATE_ACTIVE:
      sprintf(dvmrpNeighborStateBuf, dvmrpNeighborStateActive);
      break;
    case L7_DVMRP_NEIGHBOR_STATE_IGNORING:
      sprintf(dvmrpNeighborStateBuf, dvmrpNeighborStateIgnoring);
      break;
    case L7_DVMRP_NEIGHBOR_STATE_DOWN:
      sprintf(dvmrpNeighborStateBuf, dvmrpNeighborStateDown);
      break;
    default:
      sprintf(dvmrpNeighborStateBuf, defaultvalue);
      break;
    }
