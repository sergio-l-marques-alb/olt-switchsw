                                ...
                                L7_uchar8 *taggedSet)
{
  ...
  else
  {
      ...
      if (memberSet == L7_NULLPTR)
      {
          ...
          LOG_MSG("%s: Port %d Member set is NULL \n",__FUNCTION__, intIfNum);
          return L7_SUCCESS;
                            ...
                            L7_BOOL   pbvlan_cmd)
{
  ...
  */
  ...
  prc = dtlPbVlanProtocolGet(prtl, &prot);
  if (prc != L7_SUCCESS) 
  {
    return L7_FAILURE;
  }
  ...
  */
  if (pbvlan_cmd == L7_TRUE) 
  {
    ...
    */
    pbvlanTable [intIfNum][prot] = vlanid;
  } else
  {
    /* We are deleting an existing protocol.
    */
    pbvlanTable [intIfNum][prot] = L7_PBVLAN_NULL_VLANID;
  }
                             ...
                             L7_uint32 *prot)
{
    switch (prtl)
    {
    case DTL_PROTOCOL_HEX_IP:
      *prot = DAPI_QVLAN_PROTOCOL_IP;
      break;

    case DTL_PROTOCOL_HEX_ARP:
      *prot = DAPI_QVLAN_PROTOCOL_ARP;
      break;

    case DTL_PROTOCOL_HEX_IPX:
      *prot = DAPI_QVLAN_PROTOCOL_IPX;
      break;
