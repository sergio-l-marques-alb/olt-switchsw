                      ...
                      void *dapiEventInfo)
{
  ...
  // PTin added
  dtlPTinL2Callback(ddusp,family,cmd,event,dapiEventInfo);
  // PTin end
  ...
  else if ( DAPI_EVENT_ADDR_DOT1X_UNAUTH == event )
  {
    ...
    dtlDot1xIntfClientBlock(intIfNum, dei->cmdData.unsolIntfMacUnknownAddr.macAddr, dei->cmdData.unsolIntfMacUnknownAddr.vlanID);
