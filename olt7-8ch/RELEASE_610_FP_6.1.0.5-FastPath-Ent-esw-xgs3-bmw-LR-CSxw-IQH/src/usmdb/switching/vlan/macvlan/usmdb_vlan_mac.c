...
L7_RC_t usmDbVlanMacAdd(L7_uint32 unitIndex, L7_enetMacAddr_t mac, 
                        L7_uint32 vlanId)
{
    if(vlanMacIsMacAddrTypeMulticast(mac) == L7_TRUE)
        return L7_NOT_SUPPORTED;

    return vlanMacAdd(mac, vlanId);
}
...
L7_RC_t usmDbVlanMacDelete(L7_uint32 unitIndex, L7_enetMacAddr_t mac, 
                           L7_uint32 vlanId)
{
    return vlanMacDelete(mac, vlanId);
}
...
{
    return vlanMacGetNext(mac, nextmac, nextvlanId);
}

/* End Functional Declarations */
