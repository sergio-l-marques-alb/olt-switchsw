...
* @end
*********************************************************************/
L7_RC_t usmDbIpArpSwitchClear(L7_uint32 UnitIndex) 
{
  return ipMapIpArpSwitchClear(); 
}
                                   ...
                                   L7_IP_MASK_t subnetMask)
{
  ...
  {
      return(ipMapRtrIntfIpAddressSet(intIfNum,  ipAddress, subnetMask));
  }
}
     ...
     { 
		 ...
		 if (count > USMDB_VLAN_ROUTING_MAX_LOOPS)
		 {
			 ...
			 */
			 ...
			 LOG_MSG("NIM creation of VLAN routing intf %d not completed, timeout\n", intIfNum);
...
{
  return dot1qInternalVlanIdToIntIfNum(vlanId, intIfNum);
}
