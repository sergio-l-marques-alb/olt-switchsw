...
*          same subnet.
*
* @param L7_uint32 ipaddr1   internet address 1
* @param L7_uint32 netmask1  netmask 1
* @param L7_uint32 ipaddr2   internet address 2
...
{
  if ((ipaddr1 == 0) || (ipaddr2 == 0))
    return L7_SUCCESS;

  if (netmask1 == 0)
    netmask1 = osapiHtonl(0xffffff00);

  if (netmask2 == 0)
    netmask2 = osapiHtonl(0xffffff00);
  ...
  if(inet_address != L7_NULL)
  {
    if((strstr(address, ".") != L7_NULL) && (strstr(address, ":") == L7_NULL))
    {
      ...
      if ((rc =usmDbDnsIpAddressValidate((L7_char8 *)hostAddr, &ipAddr)) != L7_SUCCESS)
      {
        ...
        if (usmDbHostNameValidate(hostAddr)!= L7_SUCCESS)
        {
          return L7_FAILURE;
        }
        ipAddr = 0;
                                ...
                                L7_uint32 Index, L7_uint32 *intIfNum)
{
  ...
  /* Chassis is implemented as stack in a box 
   * this function converts user entered u/s to internal u/s 
   */
  rc = usmDbSlotMapLogicalSlotToIntUnitSlot(UnitIndex, SlotIndex, &intUnit, &intSlot);
  if (rc != L7_SUCCESS)
  {
    return rc;
  }
                             ...
                             L7_uint32 *Index)
{
  ...
  if (rc == L7_SUCCESS)
  {
    *Index = usp.port;
     ...
     /* Chassis is implemented as stack in a box
      * this function converts internal u/s to user entered u/s
      * for stacking builds this function returns internal u/s
      */

    rc = usmDbSlotMapPhysicalSlotToLogicalSlotGet(usp.unit, usp.slot, UnitIndex, SlotIndex);
  }
...
* @end
*********************************************************************/
void usmDbDhcp_release(L7_uint32 UnitIndex)
{
  l7_dhcp_release();
}
...
L7_RC_t usmDbIpSubnetMaskGet(L7_uint32 *mask, L7_uint32 plen)
{
  ...
  if((plen > 32) || (plen <1 )) return L7_FAILURE;
  
  switch(plen/8)
  {
    case 1: *mask = 0xff000000;
            break;
             /*passthru*/ 
  ...
  switch (plen%8)
  {
    ...
    case 6 :
    case 7 :
             i = ip6MapMaskByte[plen%8];
             *mask = *mask |(i<< ((3-(plen/8))*8));
             return L7_SUCCESS;
             /*passthru*/
    default :
             return L7_FAILURE;
