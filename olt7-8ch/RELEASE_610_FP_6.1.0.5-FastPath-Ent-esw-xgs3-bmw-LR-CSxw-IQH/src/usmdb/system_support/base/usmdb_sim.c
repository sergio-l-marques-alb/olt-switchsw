...
L7_RC_t usmDbTransferUpStartSet(L7_uint32 UnitIndex)
{
  ...
  L7_transfer_start(L7_TRANSFER_DIRECTION_UP);
  return L7_SUCCESS;
}
 ...
 *
 *********************************************************************/
void usmDbTransferInProgressSet(L7_uint32 UnitIndex, L7_BOOL val)
{
  simTransferInProgressSet(val);
  return;
}
 ...
 * @end
 *********************************************************************/
L7_RC_t usmDbSystemIPAndNetMaskSet(L7_uint32 UnitIndex, L7_uint32 ipAddr, L7_uint32 netMask, L7_uint32 *errorNum)
{
  L7_RC_t rc = L7_FAILURE;
  L7_uint32 oldIpAddr;

  /* check for conflicts in the system */
  if (usmDbIPConfigConflict(UnitIndex, FD_CNFGR_NIM_MIN_CPU_INTF_NUM, ipAddr, netMask, errorNum) == L7_TRUE)
    return rc;


  if (usmDbAgentIpIfAddressGet(UnitIndex, &oldIpAddr) == L7_SUCCESS)         /* save old IP address */
  {
    usmDbAgentIpIfAddressSet(UnitIndex, 0);                                  /* Zero the IP address */
    if (usmDbAgentIpIfNetMaskSet(UnitIndex, netMask) == L7_SUCCESS)        /* try to set the netmask */
    {
      if (usmDbAgentIpIfAddressSet(UnitIndex, ipAddr) == L7_SUCCESS)           /* try to set the IP address */
      {
    rc = L7_SUCCESS;     /* set succeeded */
      }
    }
  }
  ...
  if (rc != L7_SUCCESS)                                        /* One of the IP Set's failed */
  {
    *errorNum = 7248;                                           /* 7248: "Operation Failed" */
    usmDbAgentIpIfAddressSet(UnitIndex, oldIpAddr);            /* set IP to old value */
  }
...
{
  simSetSystemIPAddr(ipAddr1, L7_FALSE);

  return L7_SUCCESS;
}

L7_RC_t usmDbAgentIpMaskIfAddressSet(L7_uint32 UnitIndex, L7_uint32 ipAddr1, L7_uint32 mask)
{
  simSetSystemIPMaskAddr(ipAddr1,mask,L7_FALSE);
...
{
  simSetSystemIPGateway(val, L7_FALSE);
...
{
  simSetServPortIPGateway(val, L7_FALSE);
 ...
 * @end
 *********************************************************************/
L7_RC_t usmDbServicePortIPAndNetMaskSet(L7_uint32 UnitIndex, L7_uint32 ipAddr, L7_uint32 netMask, L7_uint32 *errorNum)
{
  L7_RC_t rc = L7_FAILURE;
  L7_uint32 oldIpAddr;

  /* check for conflicts in the system */
  if (usmDbIPConfigConflict(UnitIndex, FD_CNFGR_NIM_MIN_SERV_PORT_INTF_NUM, ipAddr, netMask, errorNum) == L7_TRUE)
    return rc;


  if (usmDbServicePortIPAddrGet(UnitIndex, &oldIpAddr) == L7_SUCCESS)      /* save old IP address */
  {
    usmDbServicePortIPAddrSet(UnitIndex, 0);                               /* Zero the IP address */
    if (usmDbServicePortNetMaskSet(UnitIndex, netMask) == L7_SUCCESS)      /* try to set the netmask */
    {
      if (usmDbServicePortIPAddrSet(UnitIndex, ipAddr) == L7_SUCCESS)      /* try to set the IP address */
      {
    rc = L7_SUCCESS;
      }
    }
  ...
  {
    *errorNum = 7248;                                           /* 7248: "Operation Failed" */
    usmDbServicePortIPAddrSet(UnitIndex, oldIpAddr);           /* set IP to old value */
  }
...
{
  L7_BOOL force = L7_FALSE;
  simSetServPortIPAddr(ipAddr1, force);
...
{
  L7_BOOL force = L7_FALSE;
  simSetServPortIPNetMask(val, force);
    ...
    while (rc == L7_SUCCESS)
    {
      if(usmDbDot1adInterfaceTypeGet(UnitIndex,intIfNum, &intfType) == L7_SUCCESS)
      {
        ...
        if (intfType == DOT1AD_INTFERFACE_TYPE_NNI)
        {
          ...
          {
            LOG_MSG("usmDbMgmtVlanIdSet: Failed to set pvid of intIfNum %d to vlan %d", intIfNum, val);
          }
        ...
        {
          LOG_MSG("usmDbMgmtVlanIdSet: Failed to set membership of intIfNum %d to vlan %d", intIfNum, val);
        }

        if (usmDbVlanMemberSet(UnitIndex, oldVlan, intIfNum, L7_DOT1Q_NORMAL_REGISTRATION, DOT1Q_SWPORT_MODE_GENERAL) != L7_SUCCESS)
        {
          LOG_MSG("usmDbMgmtVlanIdSet: Failed to remove membership of intIfNum %d to vlan %d", intIfNum, oldVlan);
        }
