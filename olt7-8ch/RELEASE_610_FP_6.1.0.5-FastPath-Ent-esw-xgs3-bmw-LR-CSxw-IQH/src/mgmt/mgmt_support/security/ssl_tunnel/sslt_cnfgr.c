...
void ssltApiCnfgrCommand(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  else
  {
    if (ssltIssueCmd(SSLT_CNFGR_EVENT, pCmdData) != L7_SUCCESS)
	  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSLT_COMPONENT_ID, "ssltApiCnfgrCommand: Failed calling "
...
L7_RC_t ssltInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
    LOG_MSG("ssltInit: msgQueue creation error.\n");
  ...
  if (ssltIssueCmd(SSLT_CNFGR_EVENT, pCmdData) != L7_SUCCESS)
    LOG_MSG("ssltInit: Failed calling ssltIssueCmd.\n");
