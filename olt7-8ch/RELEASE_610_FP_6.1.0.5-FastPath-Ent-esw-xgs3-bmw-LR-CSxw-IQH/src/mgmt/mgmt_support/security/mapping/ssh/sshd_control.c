...
sshdStartTasks(void)
{
    ...
    {
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Unable to create the task semaphore. "
    ...
    {
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Unable to create the startup semaphore. "
    ...
    */
    ...
    {
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Failed to create the sshd task. "
    ...
    {
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Unable to initialize the sshd task. "
...
sshdListenTaskCleanup(void)
{
    ...
    */
    ...
    if (sshdGlobal.sshdListenTaskId != L7_NULL)
    {
        ...
        while (sshd_exited != 1)
        {
            LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "exited = %d\n", sshd_exited);
            osapiSleep(1);
        }
    ...
    {
		L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "sshdListenTask not running, can't delete. The "
...
static void sshdEventKeyGenTask(L7_uint32 numargs, L7_uint32 *argv )
{
  ...
  {
	L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "sshdKeyFileCreate failed. The SSHD key "
															  "file(s) were not able to be created.\n");
  }
  else
  {
	  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Key file created. The SSHD key file(s)i"
  ...
  {
	L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "SSHD: sshdListenTaskRefresh failed. The "
                                    ...
                                    L7_uint32     length)
{
  ...
  {
	  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Failed to create the sshdEventKeyGenTask task."
											 " The task for generation of the SSHD keys could not be started.\n");
      return L7_FAILURE;
  }
  else
  {
	  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Created the sshdEventKeyGenTask task. The "
...
sshdFillMsg(void *data, sshdMsg_t *msg)
{
    ...
    */
    ...
    switch (msg->event)
    {
        ...
        default:
			L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_FLEX_SSHD_COMPONENT_ID, "Invalid event received. An event"
...
sshdDispatchCmd(sshdMsg_t msg)
{
    ...
    switch (msg.event)
    {
        case SSHD_ADMIN_MODE_SET:
            ...
            if (rc != L7_SUCCESS)
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventAdminModeSet failed, event=%u\n", msg.event);
            }
            else
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventAdminModeSet success, event=%u\n", msg.event);
            }
            osapiSemaGive(sshdGlobalSema);
            break;
        ...
        case SSHD_PROTO_LEVEL_SET:
            ...
            if (rc != L7_SUCCESS)
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventProtoLevelSet failed, event=%u\n", msg.event);
            }
            else
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventProtoLevelSet success, event=%u\n", msg.event);
            }
            osapiSemaGive(sshdGlobalSema);
    ...
    case SSHD_PORT_NUMBER_SET:
          ...
          if (rc != L7_SUCCESS)
          {
            LOG_MSG("SSHD: sshdEventPortNumberSet failed, event=%u\n", msg.event);
          }
          osapiSemaGive(sshdGlobalSema);
          /* Cannot take semaphore here because it will block the completion of sshdListenTaskCleanup */
          ...
          if (rc != L7_SUCCESS)
          {
            LOG_MSG("SSHD: sshdListenTaskRefresh failed, event=%u\n", msg.event);
          }
          break;
    ...
    case SSHD_KEY_GENERATE_EVENT:
            ...
            if (rc != L7_SUCCESS)
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventKeyGenerate failed, event=%u\n", msg.event);
            }
            else
            {
                LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "sshdEventKeyGenerate success, event=%u\n", msg.event);
            }
            break;
...
sshdEventAdminModeSet(L7_uint32 mode)
{
    ...
    if (mode != sshdGlobal.sshdOperMode)
    {
        if (mode == L7_ENABLE)
        {
            LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "Admin mode enable sshdListenTask");
        ...
        else if (mode == L7_DISABLE)
        {
            ...
            */
            ...
            LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "Admin mode disable sshdListenTask");
...
sshdEventProtoLevelSet(L7_uint32 protoLevel)
{
    ...
    if (protoLevel != sshdGlobal.sshdProtoLevel)
    {
        ...
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "protoLevel changed to %d", protoLevel);
    }
    else
    {
        LOG_COMPONENT_MSG(L7_FLEX_SSHD_COMPONENT_ID, "protoLevel %d unchanged", protoLevel);
    }
