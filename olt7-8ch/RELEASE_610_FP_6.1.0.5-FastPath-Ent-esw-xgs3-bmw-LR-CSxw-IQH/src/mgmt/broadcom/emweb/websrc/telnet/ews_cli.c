...
void ewsCliPrintHelp( EwsContext context, const char * str, int startPos);
/*
...
)
{
  ...
  for (; i < EWS_CLI_POSSIBLE_WIDTH; i++)
  {
    ewsTelnetWrite(context, " ");
  }
  ...
  if (full_help)
  {
    if (description != NULL)
    {
      ewsCliPrintHelp(context, description, EWS_CLI_POSSIBLE_WIDTH);
    }
  }
                        ...
                        char * dataTypeErrorMessage)
{
  ...
  for(i = 0 ; (*np_match == NULL || *ambiguous == FALSE) && parent->optionNodes[i] != NULL && i < MAX_OPTIONS_PER_NODE; i++)
  {
  ...
  do
  {
    ...
    if (*cp == '\0')
    {
      ...
      if (*datap == EWS_CLI_CHAR_POSSIBLE)
      {
        ...
        ewsCliPossible(context, full_help, mp->command, mp->description, &column);
        mp = (EwsCliCommandP) EWS_LINK_NEXT(mp->siblings);
        *help_printed = TRUE;
        continue;
      }
  ...
  case EWS_CLI_CHAR_ENTER:              /*lvl7_P0006*/
    ...
    else
    {
      ...
      /* If normal command doesn't match, try to search for hidden
         command Tree */
      strcpy(lastToken, cli->argv[cli->argc - 1]);
...
void ewsCliPrintHelp( EwsContext context, const char * str, int startPos)
{
  ...
  int plength = EWS_CLI_POSSIBLE_WIDTH -startPos;
  for ( i =0 ; i<plength; i++)
  {
    ewsTelnetWrite(context," ");
  }
  memset (tmpStr, 0,2);
  if( length + EWS_CLI_POSSIBLE_WIDTH > EWS_CLI_COLUMN_WIDTH)
  {
    k = EWS_CLI_COLUMN_WIDTH - EWS_CLI_POSSIBLE_WIDTH -1;
    ...
    for( j =0; j<=k; j++)
    {
      sprintf(tmpStr,"%c", str[j]);
      ewsTelnetWrite(context,tmpStr);
    }
    ewsTelnetWrite(context,pStrInfo_common_CrLf);
    ewsCliPrintHelp(context, str+k+1,0);
