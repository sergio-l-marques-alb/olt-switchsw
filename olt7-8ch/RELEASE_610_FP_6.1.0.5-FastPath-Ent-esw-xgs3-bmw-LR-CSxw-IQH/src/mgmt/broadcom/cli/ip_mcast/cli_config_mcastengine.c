...
const L7_char8 *commandIpMulticastTtlThreshold(EwsContext ewsContext, L7_uint32 argc,const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (( ttlThreshold >= L7_MCAST_TTL_THRESHOLD_MIN) && ( ttlThreshold <= L7_MCAST_TTL_THRESHOLD_MAX))
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR,  ewsContext, pStrInfo_ipmcast_SpecifiedIntfIsNotValidForSettingTtlThresh);
      }
    }
  }
  else
...
const L7_char8 *commandIpMcastBoundary(EwsContext ewsContext, L7_uint32 argc,const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if(ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      case L7_FAILURE:
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrErr_ipmcast_AddEntryToMcastBoundaryTbl_1);

      case L7_TABLE_IS_FULL:
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPrompt (ewsContext, pStrInfo_ipmcast_MaxNumOfAdminScopeEntriesReached);

      case L7_ERROR:
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPromptAddBlanks (0, 1, 0, 0, pStrErr_common_UserInput,  ewsContext, pStrInfo_ipmcast_GrpAddrHasHostBitsSet);

      case L7_ALREADY_CONFIGURED:
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPrompt (ewsContext, pStrInfo_ipmcast_BoundaryAlreadyExistsOnIntf);
  ...
  else if(ewsContext->commType == CLI_NO_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      {
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPromptAddBlanks (0, 2, 0, 1, L7_NULLPTR,  ewsContext, pStrInfo_ipmcast_UnableToDelAdminScopeBoundary);
      }

      if ( rc == L7_ALREADY_CONFIGURED )
      {
        cliSyntaxTop(ewsContext);
        return cliSyntaxReturnPrompt (ewsContext, pStrInfo_ipmcast_NoSuchBoundaryExistsOnIntf);
      }
    }
  }
