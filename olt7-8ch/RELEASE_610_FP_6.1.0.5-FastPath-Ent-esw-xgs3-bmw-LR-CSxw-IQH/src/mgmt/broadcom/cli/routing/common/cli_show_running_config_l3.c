...
L7_RC_t cliRunningConfigArpInfo(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  {
    cliShowCmdInt(ewsContext,val,L7_IP_ARP_CACHE_SIZE_DEF,pStrInfo_routing_ArpCachesize);
  }
...
L7_RC_t cliRunningConfigInternalVlan(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  vlanId = usmDbBaseInternalVlanIdGet();
  ...
  if ((usmDbInternalVlanPolicyGet() != FD_DOT1Q_INTERNAL_VLAN_POLICY) || 
      (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
  {
    osapiSnprintf(buf, sizeof(buf), "\r\nvlan internal allocation policy %s", 
                  (usmDbInternalVlanPolicyGet() == DOT1Q_INT_VLAN_POLICY_ASCENDING) ? 
    ...
    L7_uint32 interface)
{
    ...
    if (usmDbIfEncapsulationTypeGet(interface, &encapType) == L7_SUCCESS)
  {
            ...
            if (usmDbVrrpOperVrrpModeGet(unit,vrId,interface,&val) == L7_SUCCESS)
          {
            ...
            do
            {
              ...
              if ((ipAddr !=  0 ) && (mask != 0))
              {
                ...
                memset(buf, 0, sizeof(buf));
                (void) usmDbInetNtoa(mask, buf);
                maskLength = usmCliMaskLength (mask);
                osapiSnprintf(stat, sizeof(stat),  "%d ", maskLength);
                OSAPI_STRNCAT(strFinal, stat);
                
                osapiSnprintfAddBlanks (1, 0, 0, 1, L7_NULLPTR, buf, sizeof(buf),  pStrInfo_routing_IpVrrpTrackRoute, vrId,strFinal);
                if ((priority != FD_VRRP_DEFAULT_TRACK_PRIORITY_DECREMENT) || (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
                {
                  osapiSnprintf(stat, sizeof(stat),pStrInfo_routing_decrement,priority);
                  OSAPI_STRNCAT_ADD_BLANKS (0, 0, 1, 0, L7_NULLPTR, buf,  stat);
    ...
    {
      /* rtrdiscovery config info */
      /* By default, Router Discovery mode is true, so 'ip irdp' command 'll
         not appear in the 'show run' output. */
    ...
    {
      cliShowCmdInt(ewsContext,val,FD_RTR_DEFAULT_ADV_LIFETIME,pStrInfo_routing_IpIrdpHoldtime);
    }
    if (usmDbRtrDiscMinAdvIntervalGet(unit, interface, &val) == L7_SUCCESS)
    {
      cliShowCmdInt(ewsContext,val,FD_RTR_DEFAULT_MIN_ADV_INTERVAL,pStrInfo_routing_IpIrdpMinadvertinterval);
    }
    ...
    {
      ival = FD_RTR_DEFAULT_ADV_ADDR;
      cliShowCmdIpAddr(ewsContext, L7_AF_INET, &val, &ival, pStrInfo_routing_IpIrdpAddr_1);
      }
    }
