 ...
 **********************************************************************
 *
 * @filename src/mgmt/cli/routing/rip/cli_debug_rip.c
 *
 * @purpose Debug commands for CLI for RIP
 *
...
/*********************************************************************
*
* @purpose  To configure rip packet debug flag
*
...
*
*********************************************************************/
const L7_char8 *commandDebugRipPacket(EwsContext ewsContext,
                                      L7_uint32 argc,
                                      const L7_char8 * * argv,
                                      L7_uint32 index)
{
  ...
  if (numArg != 0)
  {
    if (mode == L7_ENABLE)
    {
      ewsTelnetWriteAddBlanks (1, 0, 0, 0, pStrErr_common_IncorrectInput,  ewsContext, pStrErr_routing_DebugRipPkt_1);
    }
    else
    {
      ewsTelnetWriteAddBlanks (1, 0, 0, 0, pStrErr_common_IncorrectInput,  ewsContext, pStrErr_routing_DebugRipPktNo);
    }
  ...
  if (mode == L7_ENABLE)
  {
    rc = usmDbRipMapPacketDebugTraceFlagSet(L7_TRUE);
  }
  else
  {
    rc = usmDbRipMapPacketDebugTraceFlagSet(L7_FALSE);
  }
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    {
      ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR,  ewsContext, pStrErr_routing_DebugRipPktDsbld);
    }
