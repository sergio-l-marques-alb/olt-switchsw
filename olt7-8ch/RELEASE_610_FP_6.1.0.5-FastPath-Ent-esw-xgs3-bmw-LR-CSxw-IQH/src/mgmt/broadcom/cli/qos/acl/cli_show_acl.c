...
const char *commandShowIpAccessList(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if ( cliNumFunctionArgsGet() == 0)     /* parameter check */
  {
    ...
    while(rc == L7_SUCCESS && numwrites < (CLI_MAX_SCROLL_LINES-6))
    {
      if (currentAclInfoPrinted == L7_FALSE)
      {
        ...
        {
          ewsTelnetPrintf (ewsContext, "%-34s", aclName);
        }
        else
        {
          ewsTelnetPrintf (ewsContext, "%-34d", aclId);
        }
      ...
      if (currentDirectionPrinted == L7_FALSE)
      {
        if ((intfList[direction].count > 0) || (vlanList[direction].count > 0))
        {
          ...
          {
          case L7_INBOUND_ACL:
            sprintf(buf, "%-12s", pStrInfo_qos_Inbound_1);
            break;
          case L7_OUTBOUND_ACL:
            sprintf(buf, "%-12s", pStrInfo_qos_Outbound_1);
            break;
          default:
  ...
  else if(cliNumFunctionArgsGet() == 1)      /* parameter check */
  {
    if (cliGetCharInputID() != CLI_INPUT_EMPTY)
    {
      /* we are in the middle of displaying the output */
      if(L7_TRUE == cliIsPromptRespQuit())
      {
        ewsCliDepth(ewsContext, cliPrevDepthGet(), argv);
        return cliPrompt(ewsContext);
    ...
    /* this section is executed for both the first and successive times */
    {
      ...
      if (usmDbQosAclIsFieldConfigured(unit, aclId,rulenum,ACL_DSTPORT) == L7_TRUE)
      {
        ...
        {
        case 53:
          strcpy(protStr, pStrInfo_qos_53);
          strcat(protStr, pStrInfo_qos_Domain_1);
          strcat(protStr, ")");
          break;
        case 7:
          strcpy(protStr, pStrInfo_qos_7);
          strcat(protStr, pStrInfo_qos_Echo);
          strcat(protStr, ")");
          break;
        case 21:
          strcpy(protStr, pStrInfo_qos_21);
          strcat(protStr, pStrInfo_common_Ftp);
          strcat(protStr, ")");
          break;
        case 20:
          strcpy(protStr, pStrInfo_qos_20);
          strcat(protStr, pStrInfo_qos_Ftpdata);
          strcat(protStr, ")");
          break;
        case 80:
          strcpy(protStr, pStrInfo_qos_80);
          strcat(protStr, pStrInfo_qos_WwwHttp);
          strcat(protStr, ")");
          break;
        case 25:
          strcpy(protStr, pStrInfo_qos_25);
          strcat(protStr, pStrInfo_qos_Smtp);
          strcat(protStr, ")");
          break;
        case 161:
          strcpy(protStr, pStrInfo_qos_161);
          strcat(protStr, pStrInfo_common_Snmp_1);
          strcat(protStr, ")");
          break;
        case 23:
          strcpy(protStr, pStrInfo_qos_23);
          strcat(protStr, pStrInfo_common_Telnet);
          strcat(protStr, ")");
          break;
        case 69:
          strcpy(protStr, pStrInfo_qos_69);
          strcat(protStr, pStrInfo_common_Tftp_1);
          strcat(protStr, ")");
          break;
        default:
          sprintf(protStr, "%d", val);
  ...
  if(numArgs == 0)     /* parameter check */
  {
    ...
    while((rc == L7_SUCCESS) && numwrites < (CLI_MAX_SCROLL_LINES-6))
    {
      if (currentAclInfoPrinted == L7_FALSE)
      {
        ...
        ewsTelnetPrintf (ewsContext, "%-6d", rules);
        currentAclInfoPrinted = L7_TRUE;
      }
      ...
      if (currentDirectionPrinted == L7_FALSE)
      {
        if ((intfList[direction].count > 0) || (vlanList[direction].count > 0))
        {
          ...
          {
          case L7_INBOUND_ACL:
            sprintf(buf, "%-10s", pStrInfo_qos_Inbound_1);
            break;
          case L7_OUTBOUND_ACL:
            sprintf(buf, "%-10s", pStrInfo_qos_Outbound_1);
            break;
          default:
            /* unknown direction enumeration */
            sprintf(buf, "        ");
  ...
  if(numArgs == 0)     /* parameter check */
  {
    ...
    while((rc == L7_SUCCESS) && numwrites < (CLI_MAX_SCROLL_LINES-6))
    {
      if (currentAclInfoPrinted == L7_FALSE)
      {
        ...
        ewsTelnetPrintf (ewsContext, "%-32s", macAclName);
