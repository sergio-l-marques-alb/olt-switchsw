...
const L7_char8 *commandShowNetwork(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  L7_uchar8 strIpv6Addr[L7_CLI_MAX_STRING_LENGTH];
   ...
   {
     osapiSnprintf(stat, sizeof(stat), "%s", "Always Up");
   }
   else
   {
...
const L7_char8 *commandShowServicePort(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (numArg != 0)
  {
   ...
   cliFormat(ewsContext, "Interface Status");
   memset (stat, 0, sizeof(stat));
   osapiSnprintf(spName, L7_SERVICE_PORT_MAX_LENGTH, "%s%d", bspapiServicePortNameGet(), 0);
    ...
    if ((strcmp(buf, pStrInfo_common_EmptyString ) != 0) && (rc == L7_SUCCESS))
      ...
      if(rc == L7_SUCCESS)
      {
        if(memcmp(&ipv6_address, " ", sizeof(L7_in6_addr_t)) != 0)
        {
          ...
          if(strcmp(buf1, "::") ==0)
          {
            ewsTelnetPrintf (ewsContext, "%-20s ",pStrInfo_common_Space);
          }
          else
          {
            if ( strlen(buf1) <=  20 )
    ...
    if (usmDbFeaturePresentCheck(unit, L7_SNOOPING_COMPONENT_ID, L7_IGMP_SNOOPING_FEATURE_PER_VLAN) == L7_TRUE)
      ...
      else
      {
        cliFormat(ewsContext, pStrInfo_base_VlansEnbldForMldSnooping);
         ...
         for (; nextvid <= L7_DOT1Q_MAX_VLAN_ID ; nextvid++)
         {
...
const char *commandShowMacAddressTableMulticast(EwsContext ewsContext, uintf argc, const char * * argv, uintf index)
{
   ...
   {                                                                     /* if our question has been answered */
    ...
    ewsTelnetWriteAddBlanks (1, 0, 72, 0, L7_NULLPTR, ewsContext,pStrInfo_base_Fwd_1);
    ewsTelnetWriteAddBlanks (1, 0, 6, 0, L7_NULLPTR, ewsContext,pStrInfo_base_MacAddrSrcTypeDescIntfIntf);
      ewsTelnetWrite(ewsContext,"\r\n-----------------------  ------  -------  --------------  ---------  ---------\r\n");
   }
   else
   {
      ...
      memset(&entry, 0, sizeof(usmdbMfdbEntry_t));
        ...
        ewsTelnetWriteAddBlanks (1, 0, 6, 0, L7_NULLPTR, ewsContext,pStrInfo_base_MacAddrSrcTypeDescIntfIntf);
            ...
            OSAPI_STRNCAT_ADD_BLANKS (0, 0, 0, 1, L7_NULLPTR, interfaceListFor2, pStrInfo_common_Fwd_2);
            if (numFwd2 <= 0)
            {
              emptyFwd = L7_TRUE;
              memset(interfaceListFor2, 0, sizeof(interfaceListFor2));
            ...
            if((numFwd > 0) && (numFlt == 0))
              ewsTelnetPrintf (ewsContext, "%-9s  ", interfaceListFor);
            else if((numFlt >0) && (numFwd ==0))
              ewsTelnetPrintf (ewsContext, "%-9s  ", interfaceListFilt);
            else if ((numFwd > 0 ) && (numFlt > 0))
            {
              ...
              ewsTelnetPrintf (ewsContext, "%-9s  ",pStrInfo_common_Space);

           if(numFwd2 >0)
             ewsTelnetPrintf (ewsContext, "%-9s", interfaceListFor2);
           else
              ewsTelnetPrintf (ewsContext, "%-9s ",pStrInfo_common_Space);
              ...
              if(fwdFiltBoth == L7_TRUE)
               {
                 if(listFor[j] != 0)
                 {
                   usmDbUnitSlotPortGet(listFor[j], &u, &s, &p);
                   osapiSnprintf(stat, sizeof(stat),"%-6s", cliDisplayInterfaceHelp(u, s, p));
                   flag=L7_TRUE;
                   numFwd--;
                 ...
                 else if(listFilt[j]!= 0)
                 {
                   ...
                   {
                     strcpy(interfaceListSrc, interfaceListFilt);
                     fwdFiltBothFlag = L7_FALSE;
                   }
                   else
                   {
                     usmDbUnitSlotPortGet(listFilt[j], &u, &s, &p);
                     osapiSnprintf(stat, sizeof(stat),"%-6s", cliDisplayInterfaceHelp(u, s, p));
                     flag=L7_TRUE;
                     numFlt--;
                     OSAPI_STRNCAT(interfaceListSrc, stat);
               ...
               {
                 usmDbUnitSlotPortGet(listFor[i], &u, &s, &p);
                 osapiSnprintf(stat, sizeof(stat),"%-6s", cliDisplayInterfaceHelp(u, s, p));
                 flag=L7_TRUE;
                 numFwd--;
                 OSAPI_STRNCAT(interfaceListSrc, stat);
               ...
               {
                 usmDbUnitSlotPortGet(listFilt[i], &u, &s, &p);
                 osapiSnprintf(stat, sizeof(stat),"%-6s", cliDisplayInterfaceHelp(u, s, p));
                 flag=L7_TRUE;
                 numFlt--;
                 OSAPI_STRNCAT(interfaceListSrc, stat);
               ...
               if(listFor2[i] != 0)
               {
                 usmDbUnitSlotPortGet(listFor2[i], &u, &s, &p);
                 osapiSnprintf(stat, sizeof(stat), "%-6s", cliDisplayInterfaceHelp(u, s, p));
                 flag=L7_TRUE;
                 numFwd2--;
  ...
  for (count=0; count < CLI_MAX_SCROLL_LINES-6; count++)
  {
    if (interface == 0)
    {
          ...
          sysIntfType != L7_TUNNEL_INTF)
      {
         ...
         {
            case L7_DISABLE:
              osapiSnprintf(stat, sizeof(stat),"%-7s ",pStrInfo_common_Dsbl_1);
              break;
            case L7_ENABLE:
              osapiSnprintf(stat, sizeof(stat),"%-7s ",pStrInfo_common_Enbl_1);
              break;
            case L7_DIAG_DISABLE:
              osapiSnprintf(stat, sizeof(stat),"%-7s ", pStrInfo_base_DiagDsbl);
              break;
            default:
              osapiSnprintf(stat, sizeof(stat),"%-7s ",pStrInfo_common_Enbl_1);
         }
         ...
         if (usmDbFeaturePresentCheck(unit, L7_POLICY_COMPONENT_ID, L7_POLICY_PORT_FLOW_CONTROL_FEATURE_ID) == L7_TRUE)
         {
            ...
            ewsTelnetPrintf (ewsContext, "%-6s",strUtilEnableDisableGet(val,pStrInfo_common_Enbl_1));
         }
		 else
		 {
		 ...
		 else
         {
             ...
             * so check if this interface is a LAG interface, if so print N/A
            */
             ewsTelnetPrintf (ewsContext, "%-6s ", pStrInfo_common_NotApplicable);
         }
      }
      }
...
const L7_char8 *commandShowHardware(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  {                                                                      /* if our question has been answered */
   ...
   else
   {
     ...
     memset (buf, 0, sizeof(buf));
     rc = usmDb1213SysDescrSingleLineGet(unit, buf);
     if (rc == L7_SUCCESS)
     {
...
const L7_char8 *commandShowMacAddrTable(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
   ...
   else
   {
      ...
      memset (stat, 0, sizeof(stat));
         if (fdb_type == L7_IVL)
         {
        ewsTelnetWriteAddBlanks (1, 0, 6, 3, L7_NULLPTR, ewsContext,pStrInfo_base_MacAddrIntfIfIdxStatus_1);
            ewsTelnetWrite(ewsContext,"\r\n-----------------------  ---------  -------  ------------");
        ewsTelnetWrite(ewsContext,pStrInfo_common_CrLf);
        osapiSnprintf(mac_address, sizeof(mac_address),
            "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
        ...
        itype == L7_TUNNEL_INTF)
    {
      ...
      else
      {
        ...
        ewsTelnetPrintf (ewsContext, "%-15s", strUtil64toa (high, low, buf, sizeof (buf)));
         ...
         {
        cliFormat(ewsContext, pStrInfo_base_PktsRcvdGreater1522);
        ewsTelnetPrintf (ewsContext, "%-15s", strUtil64toa (high, low, buf, sizeof (buf)));
      }
        ...
        ewsTelnetPrintf (ewsContext, "%-15s", strUtil64toa (high, low, buf, sizeof (buf)));
         ...
         {
        cliFormat(ewsContext, pStrInfo_base_PktsRxAndTxRange1523And2047);
        ewsTelnetPrintf (ewsContext, "%-15s", strUtil64toa (high, low, buf, sizeof (buf)));
         }
...
const L7_char8 *commandShowSntpServer(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
   ...
   else
   {
      ...
      if (usmDbSntpServerAddressTypeGet (unit, &val) == L7_SUCCESS)
      {
         ...
         {
            /* If address type is unknown, print server ip address
             * as blank */
      ...
      {
        osapiSnprintf(buf, sizeof(buf), pStrInfo_common_Diffserv_4);
      }
      ...
      else
      {
            ...
            if (addressType == L7_SNTP_ADDRESS_UNKNOWN)
        {
          osapiSnprintf(buf,sizeof(buf),pStrInfo_common_Unknown_2 );
            ...
            if (addressType == L7_SNTP_ADDRESS_IPV4)
        {
          osapiSnprintf(buf,sizeof(buf),pStrInfo_base_Ipv4_1);
        }
            if (addressType == L7_SNTP_ADDRESS_DNS)
        {
 ...
 * @notes
 *
 * @cmdsyntax   stacking environment:  show port description <unit/slot/port>
 * @cmdsyntax   non-stacking environment:  show port description <slot/port>
 *
 * @cmddescript
 * @end
 *
    ...
    const L7_char8 * * argv, L7_uint32 index)
{
...
#endif
    SHOW_RUNNING_CONFIG,
    SHOW_LOGGING_BUFFERED,
    SHOW_TRAPLOG,
#ifdef LVL7_DEBUG_BREAKIN
    SHOW_CONSOLE_BREAKIN,
#endif
    SHOW_DEBUGGING
  } localOptions;
  ...
  usmDbTrapLogEntry_t trapLog;
  L7_uint32 first=0;
  static L7_uint32 entryIndex = 0, numEntries=0;
  static L7_uint32 startingEntryIndex = 0;
  static localOptions currentOption = SHOW_VERSION;
#ifdef L7_ISDP_PACKAGE
  ...
  L7_uint32 nextInterface, intfSpeed;
  static L7_uint32 interface;
  usmDbTimeSpec_t  ts;
  static L7_uint32 idx;
#ifndef FEAT_METRO_CPE_V1_0
  ...
  static L7_uint32 ptr;
  static L7_uchar8 string[L7_CLI_MAX_STRING_LENGTH*2]; /* protect ourselves from strings greater than what the MAX is */
  static L7_uint32 logIndex, bufIndex=0;
  L7_char8 valString[L7_CLI_MAX_STRING_LENGTH];
  L7_ADMIN_MODE_t status;
  L7_LOG_SEVERITY_t severity;
  L7_ushort16 port;
    ...
    for(count = 0; count < CLI_MAX_SCROLL_LINES - 6; count++)
    {
      ...
      do
      {
        ...
        else
        {
          ...
          flag = L7_TRUE;
#ifdef L7_ISDP_PACKAGE
          currentOption = SHOW_ISDP_NEIGHBORS;
#else
          currentOption = SHOW_LOGGING;
#endif
          cliSetCharInputID(CLI_INPUT_NOECHO, ewsContext, argv);
          cliAlternateCommandSet(pStrInfo_base_ShowTechSupportCmd);
          ewsContext->unbufferedWrite = L7_FALSE;
  ...
  if(currentOption == SHOW_LOGGING)
  {
    ...
    for(count = 0; count < CLI_MAX_SCROLL_LINES-6; count++)
    {
      ...
      {
        flag = L7_TRUE;
        currentOption = SHOW_LOGGING_BUFFERED;
        cliSetCharInputID(CLI_INPUT_NOECHO, ewsContext, argv);
        cliAlternateCommandSet(pStrInfo_base_ShowTechSupportCmd);
        ewsContext->unbufferedWrite = L7_FALSE;
        return pStrInfo_common_Name_2;   /* --More-- or (q)uit */
  ...
  if(currentOption == SHOW_TRAPLOG)
  {
    ...
    {
      flag = L7_TRUE;
      currentOption = SHOW_RUNNING_CONFIG;
      cliSetCharInputID(CLI_INPUT_NOECHO, ewsContext, argv);
      cliSyntaxBottom(ewsContext);
      cliAlternateCommandSet(pStrInfo_base_ShowTechSupportCmd);
      ewsContext->unbufferedWrite = L7_FALSE;
    ...
    while (numWrites < CLI_MAX_SCROLL_LINES-6)
    {
      ...
      if (((count == startingEntryIndex + 1) && (numEntries < TRAPMGR_TABLE_SIZE))
  ...
  if(currentOption == SHOW_RUNNING_CONFIG)
  {
    ...
    ewsContext->unbufferedWrite = L7_FALSE;
#ifdef LVL7_DEBUG_BREAKIN
   currentOption = SHOW_CONSOLE_BREAKIN;
#else
   currentOption = SHOW_DEBUGGING;
#endif
                                 ...
                                 L7_uint32 index)
{
   ...
   if ((numArg != 1) && (firstDisplay == L7_TRUE) && (firstDcDisplay == L7_TRUE)) 
   {
     ...
     {
        sprintf(buf, pStrInfo_base_DomainNameNotConfig);
        ewsTelnetWrite(ewsContext, buf);
     }
     ...
     if (rc == L7_SUCCESS && (strcmp (domain, "\0") == 0))
     {
        sprintf(buf, pStrInfo_base_DomainListNotConfig);
        ewsTelnetWrite(ewsContext, buf);
     }
     else if (rc == L7_SUCCESS)
     {
     ...
     if (rc == L7_SUCCESS)
     {
        if ( enable == L7_TRUE )
        {
           sprintf(buf, pStrInfo_base_Enabled);
           ewsTelnetWrite(ewsContext, buf);
        }
        else
        {
           sprintf(buf, pStrInfo_base_Disabled);
           ewsTelnetWrite(ewsContext, buf);
        }
     }
     bzero(buf, sizeof(buf));
     ...
     {
        sprintf(buf, pStrInfo_base_NameServersNotConfig);
        ewsTelnetWrite(ewsContext, buf);
     }
   ...
   {
     sprintf(buf, pStrInfo_base_ConfigHostNameIPMapping);
     ewsTelnetWrite(ewsContext, buf);


     sprintf(buf, pStrInfo_base_HostAddress);
     ewsTelnetWrite(ewsContext, buf);
     sprintf(buf, pStrInfo_base_ShowHostsLine);
     ewsTelnetWrite(ewsContext, buf);
   }
   ...
   if (numArg == 1)
   {
      ...
      {
         sprintf(buf, pStrInfo_base_HostNameNotConfig);
         ewsTelnetWrite(ewsContext, buf);
      }
   }
   else if (numArg == 0)
   ...
   {
     bzero(buf, sizeof(buf));
     sprintf(buf, pStrInfo_base_HostTotalElapsedTypeAddress);
     ewsTelnetWrite(ewsContext, buf);
     bzero(buf, sizeof(buf));
     sprintf(buf, pStrInfo_base_ShowHostsLine_2);
     ewsTelnetWrite(ewsContext, buf);
   }
   ...
   if (numArg == 1)
   {
     ...
     {
        sprintf(buf, pStrInfo_base_HostNameNotMapped);
        ewsTelnetWrite(ewsContext, buf);
     }
   }
   else if (numArg == 0)
