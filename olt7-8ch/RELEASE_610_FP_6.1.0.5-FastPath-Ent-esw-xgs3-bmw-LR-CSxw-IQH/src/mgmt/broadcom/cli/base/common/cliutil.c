...
L7_char8 *cliProcessStringDisplay(L7_char8 * buffer, L7_uint32 len)
{
  ...
  osapiStrncpySafe(tmpString, buf, min(bufLength+1, (sizeof(tmpString)-1)));
  return tmpString;
}
/*********************************************************************
*
...
* @param  *buf   @b{(input)) pointer to user-input string
* @param  *prefix  @b{(output)) pointer to the prefix index
* @param  *mask  @b{(output)) pointer to the mask.
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
    ...
    L7_uint32 * mask)
{
  ...
  memcpy (prefix,strIPaddr, L7_IP6_ADDR_LEN);
...
L7_BOOL cliPasswdExpireCheck (L7_char8 * enteredInfo, EwsContext context)
{
  ...
  if (usmDbLoginSessionUserGet(unit, cliLoginSessionIndexGet(), currentUser) == L7_SUCCESS)
  {
...
L7_BOOL configExpiredPasswd(EwsContext context, L7_char8 * enteredInfo, L7_uint32 * pCallCnt)
{
  ...
  if (!(*pCallCnt))
  {
    ...
    {
      strncpy(cliCommon[cliUtil.handleNum].password, enteredInfo, sizeof(cliCommon[cliUtil.handleNum].password));
    }

    /* Check for minimum length */
    rc = usmDbPasswdLengthValid (0, enteredInfo, L7_FALSE);
    if (rc == L7_FAILURE)
    {
      usmDbMinPassLengthGet(&minPassLen);
      osapiSnprintfAddBlanks (1, 0, 0, 0, pStrErr_common_Error, buf, sizeof(buf), pStrErr_base_CfgPasswdLenError, minPassLen, L7_PASSWORD_SIZE-1);
      ewsTelnetWrite( context, buf);
  ...
  if (strcmp(enteredInfo, cliCommon[cliUtil.handleNum].password) == 0)
  {
    ...
    {
      *(pCallCnt) = 0;
      osapiSnprintfAddBlanks (1, 0, 0, 0, pStrErr_common_CouldNot, buf, sizeof(buf), pStrInfo_base_SetUsrPasswdPleaseAvoidReusingOldPasswds);
      ewsTelnetWrite( context, buf);
      ewsTelnetWrite (context, pStrInfo_base_NewPasswd);
      ewsFlushAll(context);
                               ...
                               L7_uint32 *ival, L7_IP_ADDRESS_TYPE_t *addrType)
{
  ...
  else if (rc == L7_FAILURE)
  {
    /* Invalid Host Name*/
    cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput,
           context, pStrErr_base_InvalidHostName);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
...
L7_int32 addIncludeVid(L7_uchar8 *buffer, L7_uint32 firstVid, L7_uint32 lastVid, L7_int32 cmdLen, L7_char8 * commandString)
{
  ...
  if ( 0 == cmdLen )
  {
    cmdLen     = sprintf(buffer, commandString);
    commaString = "";
  }
...
L7_int32 addIncludeIntf(L7_uchar8 *buffer, L7_uint32 startIntf, L7_uint32 endIntf, L7_int32 cmdLen, L7_char8 * commandString)
{
  ...
  {
    usmDbUnitSlotPortGet(startIntf, &unit, &slot, &port);
    ...
    usmDbUnitSlotPortGet(endIntf, &unit, &slot, &port);
