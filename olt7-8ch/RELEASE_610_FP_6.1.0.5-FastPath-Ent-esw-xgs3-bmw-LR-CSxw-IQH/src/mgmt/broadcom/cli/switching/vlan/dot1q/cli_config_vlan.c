      ...
      (strcmp(argv[index+argMode], pStrInfo_common_Auto2) == 0))
  {
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      if (usmDbSwPortMonitorIsDestConfigured(unit,interface,L7_NULLPTR)
          == L7_TRUE)
      {
        return cliSyntaxReturnPromptAddBlanks (0, 1, 0, 0, pStrErr_common_Cant,  ewsContext, pStrErr_switching_CantCfgureProbePort);
      }

      nimGetIntfType(interface, &itype);
      if (itype == L7_CPU_INTF)
      {
        return cliSyntaxReturnPrompt (ewsContext, pStrErr_switching_CantCfgureCpuIntf);
      }
      else if (itype == L7_LOGICAL_VLAN_INTF)
      {
        return cliSyntaxReturnPrompt (ewsContext, pStrErr_common_CantCfgureVlanRoutingIntf);
      }
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      if (rc == L7_ERROR)
      {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrErr_switching_CantSetVlanMbrshipOnDynVlan);
      }
    }
  }
  else
...
const L7_char8 *commandVlanName(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    ...
    else
    {
      ...
      rc = usmDbVlanNameSet(unit, vlanID, buf);
...
const L7_char8 *commandVlanTagging(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  {
    return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, NULL, ewsContext, pStrErr_switching_FailedToSetVlanTagging);
  }
                                 ...
                                 const L7_char8 ** argv, L7_uint32 index)
{
  ...
  L7_char8 *strVlanList;
  L7_uint32 unit;
  L7_uint32 numArg, i, mgmt_vlanid = 0;
  L7_uint32 vlanCount = 0;                 /* No. of Vlans to be created */
  L7_uint32 vlanList[L7_MAX_VLANS];        /* List of Vlan IDs to be created */
  L7_char8 buf[L7_CLI_MAX_STRING_LENGTH];
  L7_uchar8 tempBuf[L7_CLI_MAX_LARGE_STRING_LENGTH];
  L7_BOOL flag = L7_TRUE;
  L7_BOOL atleastOneFailure = L7_FALSE;
  L7_VLAN_MASK_t cliVlanList;
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
	...
	{
	  if (flag == L7_TRUE && atleastOneFailure == L7_TRUE)
	  {
  ...
  {
    rc = usmDbMgmtVlanIdGet(unit, &mgmt_vlanid);         /*check for mgmt VLAN*/
    if (mgmt_vlanid == vlanID) {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, NULL, ewsContext,
                                      pStrErr_switching_CantRemoveMgmtVlan);
    }

    if (vlanID == L7_DOT1Q_DEFAULT_VLAN) {                        /*check for default VLAN*/
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error,
                    ewsContext, "Cannot delete default VLAN");
    }
