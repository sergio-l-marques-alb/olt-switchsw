...
/*********************************************************************
*
* @purpose  Configures the Snooping Querier mrouter vlan address
*
*
* @param EwsContext ewsContext
* @param L7_uint32 argc
...
* @notes none
*
* @cmdsyntax    [no] set igmp querier <vlan-id> rvlan <vlan-id>
* @cmdsyntax    [no] set mld querier <vlan-id> rvlan <vlan-id>
*
* @cmdhelp
*
* @cmddescript  This command sets and clears the snooping querier address.
...
*
*********************************************************************/
const L7_char8 *commandSetSnoopingQuerierVlanXlate(EwsContext ewsContext, L7_uint32 argc,
                                                   const L7_char8 * * argv, L7_uint32 index)
{
  L7_RC_t rc = L7_FAILURE;
  L7_uint32 numArg, vlanId;
  L7_uint32 xlateVlanId;
  L7_int32 argxlateVlanId=1, argVlanid = -1;
  L7_uchar8 family = L7_AF_INET;
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    /* Extract the router vlan id */
    if (cliConvertTo32BitUnsignedInteger((L7_char8 *)argv[index+argxlateVlanId], &xlateVlanId) != L7_SUCCESS)
    {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, pStrErr_common_InvalidInteger);
    }

    if (xlateVlanId < L7_DOT1Q_MIN_VLAN_ID || xlateVlanId> L7_DOT1Q_MAX_VLAN_ID)
    {
      return cliSyntaxReturnPromptAddBlanks (0, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrErr_common_InvalidVlanIdSpecified);
    }
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      rc = usmDbSnoopQuerierVlanXlateSet(vlanId, xlateVlanId);
    }
  }
  else if (ewsContext->commType == CLI_NO_CMD)
  {
    ...
    // Default xlateVlanId is the same as vlanId
    xlateVlanId = vlanId;
  ...
  {
    return cliSyntaxReturnPrompt (ewsContext, pStrInfo_switching_IgmpSnoopingQuerierVlanXlate);
  }
  ...
  if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    if (rc != L7_SUCCESS)
    {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, 
                                           L7_NULLPTR, 
                                           ewsContext, 
                                           pStrErr_switching_SetSnoopingQuerierVlanXlate);
    }
  }
...
* @notes none
*
* @cmdsyntax    [no] set igmp querier <vlan-id> rvlan <vlan-id> address <ip-address>
* @cmdsyntax    [no] set mld querier <vlan-id> rvlan <vlan-id> adress <ip-address>
*
* @cmdhelp
*
* @cmddescript  This command sets and clears the snooping querier address.
                                                     ...
                                                     const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      printf("%s@%s@%d: VlanId=%u, ipAddr=\"%s\"\n",__FILE__, __FUNCTION__, __LINE__,vlanId, ipAddr);
