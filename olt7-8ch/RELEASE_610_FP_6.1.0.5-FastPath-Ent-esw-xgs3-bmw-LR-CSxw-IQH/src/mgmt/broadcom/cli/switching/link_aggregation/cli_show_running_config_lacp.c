 ...
 /* Begin display port-channel system hash mode */
  ...
  while ( rc == L7_SUCCESS )
  {
    if ( usmDbDot3adIsConfigured(unit, interface) == L7_TRUE )
    {
      ...
      {
        if (( strcmp(stat, FD_DOT3AD_NAME) != 0 )|| EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT)
        {
                                   ...
                                   L7_DOT3AD_HASHMODE_INTF_FEATURE_ID) == L7_TRUE)
      {
        if (usmDbDot3adLagHashModeGet(unit, interface, &hashMode) ==  L7_SUCCESS)
        {
          ...
          {
            OSAPI_STRNCPY_SAFE_ADD_BLANKS (1, 0, 0, 1, L7_NULLPTR, buf, pStrInfo_base_LagLoadBalance);
            osapiSnprintf(stat, sizeof(stat), "%d %s", hashMode, cliDisplayInterfaceHelp(u,s,p));
            OSAPI_STRNCAT(buf, stat);
            EWSWRITEBUFFER(ewsContext,buf);
          }
        }
                      ...
                      ((val != portFactoryDefaultSpeed)|| (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT)))
                  {
                    switch ( val )
                    {
                      ...
                      case L7_PORTCTRL_PORTSPEED_FULL_2P5FX:
                        osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, stat1, sizeof(stat1), pStrInfo_base_Speed2500Full);
                        break;
                      case L7_PORTCTRL_PORTSPEED_FULL_1000SX:
                        osapiSnprintfAddBlanks (1, 0, 0, 0, L7_NULLPTR, stat1, sizeof(stat1), pStrInfo_base_Speed1000Full);
                        break;
...
L7_RC_t cliRunningInterfaceConfigLACPInfo(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
  ...
  if ( (usmDbIntfTypeGet(interface, &itype) == L7_SUCCESS) && (itype == L7_PHYSICAL_INTF) )
  {
    ...
    if(rc == L7_SUCCESS)
    {
      cliShowCmdInt(ewsContext, keyValue, interface, pStrInfo_base_LacpActorAdminKey_1);
    }
