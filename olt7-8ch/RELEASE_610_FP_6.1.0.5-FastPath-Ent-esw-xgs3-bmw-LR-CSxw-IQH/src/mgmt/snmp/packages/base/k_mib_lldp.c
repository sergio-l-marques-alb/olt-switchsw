                           ...
                           SR_INT32 lldpStatsTxPortNum)
{
  ...
  if ((searchType == EXACT ? 
       (usmDbPhysicalIntIfNumberCheck(USMDB_UNIT_CURRENT, intIfNum) != L7_SUCCESS) :
       (usmDbPhysicalIntIfNumberCheck(USMDB_UNIT_CURRENT, intIfNum) != L7_SUCCESS &&
        usmDbGetNextPhysicalIntIfNumber(intIfNum, &intIfNum) != L7_SUCCESS)))
  {
    ZERO_VALID(lldpStatsTxPortEntryData.valid);
    return(NULL);
  }

  if (usmDbExtIfNumFromIntIfNum(intIfNum, &lldpStatsTxPortEntryData.lldpStatsTxPortNum) != L7_SUCCESS)
  {
    ZERO_VALID(lldpStatsTxPortEntryData.valid);
    return(NULL);
  }
                           ...
                           SR_INT32 lldpStatsRxPortNum)
{
  ...
  if (usmDbExtIfNumFromIntIfNum(intIfNum, &lldpStatsRxPortEntryData.lldpStatsRxPortNum) != L7_SUCCESS)
  {
    ZERO_VALID(lldpStatsRxPortEntryData.valid);
    return(NULL);
  }
