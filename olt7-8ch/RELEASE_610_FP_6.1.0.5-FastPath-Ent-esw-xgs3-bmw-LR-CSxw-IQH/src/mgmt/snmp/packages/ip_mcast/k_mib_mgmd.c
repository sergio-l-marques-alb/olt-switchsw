                           ...
                           SR_INT32 mgmdRouterCacheIfIndex)
{
  ...
  L7_inet_addr_t routerCacheAddr4,routerCacheAddr6,routerAddr;
  L7_inet_addr_t lastReportIp;
  static L7_uchar8 familyType=L7_AF_INET;

  ZERO_VALID(mgmdRouterCacheEntryData.valid); 
  inetAddressZeroSet(L7_AF_INET,&routerCacheAddr4);
  inetAddressZeroSet(L7_AF_INET6,&routerCacheAddr6);
  ...
  if(familyType==L7_AF_INET && mgmdRouterCacheAddress->length!=0)
  {
    ...
    inetAddressSet(L7_AF_INET,&routerCacheAddress,&routerCacheAddr4);
  }
  /*get the ipv6 address from octetString*/
  else if(familyType==L7_AF_INET6 && mgmdRouterCacheAddress->length!=0)
  {
    routerCacheAddr6.family=L7_AF_INET6;
    tempIntIfNumV6 = mgmdRouterCacheIfIndex;
    memcpy(routerCacheAddr6.addr.ipv6.in6.addr8,mgmdRouterCacheAddress->octet_ptr,16);
  }
  ...
  else if(searchType == NEXT)
  {
    if(familyType==L7_AF_INET)
    {
      ...
      if (((usmDbExtIfNumTypeCheckValid(USMDB_UNIT_CURRENT,USM_ROUTER_INTF, 0, tempIntIfNumV4) != L7_SUCCESS) &&
          ...
          ((snmpMgmdCacheEntryGet(USMDB_UNIT_CURRENT, &routerCacheAddr4, intIfNum, nominator,familyType) != L7_SUCCESS) &&
           (snmpMgmdCacheEntryNextGet(USMDB_UNIT_CURRENT, &routerCacheAddr4, &intIfNum, nominator,familyType) != L7_SUCCESS)) ||
    ...
    if(familyType==L7_AF_INET6 )
    {
      ...
      if (((usmDbExtIfNumTypeCheckValid(USMDB_UNIT_CURRENT,USM_ROUTER_INTF, 0, tempIntIfNumV6) != L7_SUCCESS) &&
          ...
          ((snmpMgmdCacheEntryGet(USMDB_UNIT_CURRENT, &routerCacheAddr6, intIfNum, nominator,familyType) != L7_SUCCESS) &&
           (snmpMgmdCacheEntryNextGet(USMDB_UNIT_CURRENT, &routerCacheAddr6, &intIfNum, nominator,familyType) != L7_SUCCESS)) ||
  ...
  if(familyType==L7_AF_INET)
  {
    ...
    inetCopy(&routerAddr,&routerCacheAddr4);
    if(SafeMakeOctetString(&mgmdRouterCacheEntryData.mgmdRouterCacheAddress,(L7_uchar8*)&routerCacheAddr4.addr.ipv4.s_addr,
                           sizeof(routerCacheAddr4.addr.ipv4.s_addr))==L7_FALSE)
      return(NULL);
  }
  else if(familyType==L7_AF_INET6)  
  {
    ...
    inetCopy(&routerAddr, &routerCacheAddr6);
    if( SafeMakeOctetString(&mgmdRouterCacheEntryData.mgmdRouterCacheAddress, 
               routerCacheAddr6.addr.ipv6.in6.addr8, sizeof( routerCacheAddr6.addr.ipv6.in6.addr8))==L7_FALSE)
      return(NULL);
  }
