...
typedef struct
{
  ...
  L7_uint32 optDnsServerIpAddr;                    /* IP addres of DNS server, got from option 6 field */
...
/*******************************************
*  Common network information used by dhcpv6
********************************************/
typedef enum
{
...
typedef struct dhcp6cCtlMsg_s
{
  L7_MGMT_PORT_TYPE_t   portType;
  L7_SYSCFG_MODE_t      configType;
} dhcp6cCtlMsg_t;

#define DHCP6_CTL_PKT_LEN	sizeof(dhcp6cCtlMsg_t)

/*****************************************/

/* Indices values for vlan masks */
/* Number of bytes in mask */
#define L7_VLAN_INDICES   ((L7_PLATFORM_MAX_VLAN_ID) / (sizeof(L7_uchar8) * 8) + 1)
#define L7_VLAN_MAX_MASK_BIT   L7_PLATFORM_MAX_VLAN_ID
