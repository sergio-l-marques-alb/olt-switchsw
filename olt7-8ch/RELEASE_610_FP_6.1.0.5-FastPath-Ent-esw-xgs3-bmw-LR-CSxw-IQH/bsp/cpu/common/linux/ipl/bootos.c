...
int fp_main(int argc, char *argv[])
{
  ...
  */
  ...
  rc = 10;
  L7_LOG(L7_LOG_SEVERITY_NOTICE, L7_BSP_COMPONENT_ID, 
...
void sigsegv_handler (int sig, siginfo_t * info, void * v)
{
  ...
  */
  ...
  /* Figure out if this segfault is a stack overflow, and, if so, which
     task it's from. */
  rc = osapiWhichStack((L7_uint32)(info->si_addr), which_stack, 20);
  if (rc == L7_SUCCESS)
  {
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  stack_t sig_stack;
	...
	system("sh fastpath.ver");
	printf("\nInitializing FASTPATH...\n");
  ...
  */
  ...
  bspCpuInit();  /* Perform CPU specific initialization */
	...
	switch (speed) {
		...
		default:
			printf("unknown speed (%d)! %d\n",speed,L7_SERIAL_COM_ATTR);
			exit(-1);
			break;
	}
