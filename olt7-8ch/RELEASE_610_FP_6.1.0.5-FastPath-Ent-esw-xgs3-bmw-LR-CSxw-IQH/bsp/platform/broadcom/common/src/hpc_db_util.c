...
L7_RC_t hpcHardwareLocalUnitIdentifierMacGet(L7_enetMacAddr_t *mac)
{
  ...
  */
  ...
  {
    if (sysapiRegistryGet(SYS_MAC, MAC_ENTRY, (void *)&sys_mac) != L7_SUCCESS)
    {
      LOG_MSG("hpcHardwareLocalUnitIdentifierMacGet: Error on sysapiRegistryGet with key SYS_MAC\n");
      rc = L7_ERROR;
    }
  }
                               ...
                               HPC_UNIT_DESCRIPTOR_t *local_unit_desc)
{
  ...
  if (sysapiRegistryGet(SYS_MAC, MAC_ENTRY, ( void * )&sysMac ) != L7_SUCCESS)
  {
    return(rc);
  }
  ...
  case SYSAPI_MAC_POLICY_MGMT_UNIT_POOL:
    for (i=0; i < uni_mac_allocation_table_num_entries; i++)
    {
      ...
      {
        // PTIN FLAG: This is where the MAC address is defined for each interface
        sysapiSetMacAddress(mac_addr, sysMac, port /*uni_mac_allocation_table[i].system_base_mac_offset*/);
        rc = L7_SUCCESS;
      }
    }
    break;
  case SYSAPI_MAC_POLICY_REMOTE_UNIT_POOL:
    ...
    if (slot < hpc_data_ptr->num_mac_allocation_table_entries)
    {
        ...
        if (port >= 1)
        {
          ...
          if (p < hpc_data_ptr->mac_allocation_table[slot].num_mac_addrs_allocated_to_slot)
          {
            ...
            */
            ...
            sysapiSetMacAddress(mac_addr, sysMac, totalPortMacOffset);
            rc = L7_SUCCESS;
          }
        }
      }
      ...
      {
        sysapiSetMacAddress(mac_addr, sysMac, hpc_data_ptr->mac_allocation_table[slot].system_base_mac_offset);
        rc = L7_SUCCESS;
      }
    }
    break;
...
L7_RC_t hpcLocalCardDbInit(void *board_info)
{
  ...
  L7_uint32 temp32;
  L7_uchar8 macCount = 32;
  L7_char8  mac[6] = {0, 1, 1, 2, 2, 3};
  ...
  */
  bspCpuReadMac(mac, &macCount);
  bspCpuReadSerialNum(serialString);

  sysapiRegistryPut(SERIAL_NUM,    STR_ENTRY, &serialString);
  sysapiRegistryPut(SYS_MAC_COUNT, U8_ENTRY,  &macCount);
  sysapiRegistryPut(SYS_MAC,       MAC_ENTRY, &mac);
