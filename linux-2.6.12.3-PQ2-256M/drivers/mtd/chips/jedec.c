...
/*XXX*/
static char *map_info2str(struct map_info *map,char *str)
{
        char *rv="(NULL)";
        if(((map)) && ((str)))
        {
                str[0]='\0x00';
                sprintf(str,"(name:%s,size:0x%X,phys:0x%X)", (((map->name))?map->name:"(NULL)"), map->size, map->phys);
                rv= str;
        }
        return rv;
...
static struct mtd_info *jedec_probe(struct map_info *map)
{
   ...
   {
        char str[128];
        /*XXX*/printk("%s @ %s @ %d|map:%d\n", __FILE__,__FUNCTION__,__LINE__,map_info2str(map,str));
   }
