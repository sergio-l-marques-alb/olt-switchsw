...
#define _BCM_NUMBER_OF_MC_RESERVED_TRAPS               (256) /*Number of IHP_RESERVED_MC, Mc reserved condition traps*/
#define _BCM_NUMBER_OF_PROG_TRAPS                      (4) /*Number of IHP_GENERAL_TRAP, program traps*/

#define _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL    _BCM_NUMBER_OF_MC_RESERVED_TRAPS+_BCM_NUMBER_OF_PROG_TRAPS
...
{
    _BCM_L2_CACHE_INDEX_TYPE_INVALID, /*Invalid must be first (must be zero) for ISSU safety*/
    _BCM_L2_CACHE_INDEX_TYPE_RESERVED_MC,      /*Virtual trap reserved Multi-cast IHP_RESERVED_MC*/
    _BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC,  /*Type when the set/get was called for L2CP traps*/
    _BCM_L2_CACHE_INDEX_TYPE_PROG,              /*Virtual Traps programable IHP_GENERAL_TRAP*/
    _BCM_L2_CACHE_INDEX_TYPE_MAX
}_bcm_l2_cache_index_type_e;

/*Each entry in index control type hold the needed information of bcm_l2_cache_set/get
   To perform "get" and read from correct rigister function need to know :
   index_type: For which trap type the get was called
   mac_lsb: 8 bit lsb of DA address of trap
   port: ingress prot which packet was recieved from
   profile: programable trap profile*/
typedef struct _bcm_l2_cache_index_control_s
{
    _bcm_l2_cache_index_type_e  index_type;
    int                         mac_lsb;
    int                         port;    
    int                         prog_profile;
}_bcm_l2_cache_index_control_t;

/*The control index array structure, maps index to values for bcm_l2_cache_set/get
    To preform a correct get function need port, mac_lsb and type of virtual trap, this what SW DB hold*/
typedef struct _bcm_l2_cache_index_s
{
   _bcm_l2_cache_index_control_t     index_entry[_BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL]; 
...
typedef struct l2_data_s {
    ...
    _bcm_l2_cache_index_t     index_control;   /*Used for bcm_l2_cache_set/get index parameter*/
} l2_data_t;
