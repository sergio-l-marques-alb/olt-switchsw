...
*  Revision      :  $Id: phy8806x_tsc_functions.c 1062 2015-06-02 22:12:41Z kirand $ *
*                                                                                 *
...
err_code_t phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 (const phymod_access_t *pa, uint32_t timeout_ms){
	...
	{   uint16_t rddata;
	    ...
	    if (timeout_ms < 100)
		    EFUN(phy8806x_tsc_delay_us(timeout_ms));
	    else
...
err_code_t phy8806x_tsc_read_die_temperature (const phymod_access_t *pa, int16_t *die_temp){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_UC_DBG, CMD_UC_DBG_DIE_TEMP, 50));
