...
int qax_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    for (i = 0; i < QAX_NOF_GRAPHIC_COUNTERS; ++i) {
       ...
       else if (SOC_E_NONE != soc_reg_get(unit, qax_counters[i].reg, 0, qax_counters[i].reg_index, &counters_data[i])) { 
           LOG_ERROR(BSL_LS_APPL_SHELL,
                     (BSL_META_U(unit,
                                 "Failed to read counter: %s\n"), qax_counters[i].reg_name)); 
       }
...
STATIC
cmd_result_t
_cmd_dpp_serdes_str_get(int unit, pbmp_t phy_ports, char *serdes_str, uint32 qsgmii_ind, int is_over_fab)
{
    ...
    BCM_PBMP_COUNT(phy_ports, phy_count);
...
cmd_dpp_nif(int unit, args_t *a) {
    ...
    BCM_PBMP_ITER(pbmp_nif, port) {
            ...
            if (one_clk_time_measured_int == -1)
            {
                ...
                if (SOC_PORT_IF_ILKN == interface_type) {
                    ...
                    if (SOC_DPP_IS_ILKN_PORT_OVER_FABRIC(unit, ilkn_id)) {
                        ...
                        serdes_freq_int = ilkn_o_fab_rate_int;
                        serdes_freq_remainder = ilkn_o_fabe_rate_reminder;
        ...
        result = _cmd_dpp_serdes_str_get(unit, phys, serdes_num_str_buf, qsgmii_ind, is_over_fab);
        if (result != CMD_OK) {
            return result;
        }
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            for (i = 0; i < nof_lanes; ++i) {
                ...
                if ( (SOC_PORT_IF_ILKN != interface_type) && PORTMOD_IS_LEGACY_PHY_GET(&(phy[nof_pm-1].access)) ) { 
...
dpp_diag_egr_calendars(int unit, args_t *a) {
    ...
    if (res == CMD_OK) {
        ...
        BCM_PBMP_ITER(valid_ports, port) {
            ...
            if (nif_type == SOC_PORT_IF_ERP || SOC_PORT_IS_VIRTUAL_RCY_INTERFACE(flags)) {
                continue;
            }
...
dpp_diag_resource_allocation(int unit, args_t *a)
{
    ...
    if (res == CMD_OK) {
        if (!sal_strncasecmp(type, "src", strlen(type))) {
            soc_dpp_ingress_congestion_mgmt_t ingress_congestion_mgmt;
            uint32 dram_not_used = 0, ocb_not_used = 0;
            int core_index;
            uint32 width = 7;

            cli_out("VSQ source based resource allocation\n");
            cli_out("====================================\n\n");

            BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_index) {
                char str_total_memory[2][11] = {"", ""},
                     str_pool_0_size[2][11] = {"", ""},
                     str_pool_1_size[2][11] = {"", ""},
                     str_headroom_size[2][11] = {"", ""},
                     str_reserved_size[2][11] = {"", ""},
                     str_not_used_size[2][11] = {"", ""};
...
dpp_diag_cosq_print_modport_to_sysport_to_voq_mapping(int unit, int by_sys, int by_mod, int by_voq) {
    ...
    uint8 is_valid, is_sw_only = FALSE;
    uint32 base_queue = 0;
...
dpp_diag_prge_last(int unit, args_t *a) {
    ...
    for (core_id=first_core; core_id < last_core; core_id++) {
        ...
        if(arad_egr_prog_editor_print_chosen_program(unit, first_instruction, FALSE /* NOT raw output */) != SOC_E_NONE) {
            cli_out("Not found.\n");
            continue;
        }
    }
