/*
 * $Id: counter.c,v 1.39 Broadcom SDK $
    ...
    if (_SOC_CONTROLLED_COUNTER_USE(unit, port)) {
         ...
         if (SOC_CONTROL(unit)->counter_interval != 0 && (COUNTER_IS_COLLECTED(SOC_CONTROL(unit)->controlled_counters[ctr_reg]))) {
            ...
            if (SOC_CONTROL(unit)->controlled_counters[ctr_reg].controlled_counter_f != NULL) {
                rv = SOC_CONTROL(unit)->controlled_counters[ctr_reg].controlled_counter_f(unit, SOC_CONTROL(unit)->controlled_counters[ctr_reg].counter_id, port, &val);
            } else {
                return;
            }
        ...
        ) 
    {
        ...
        if (_SOC_CONTROLLED_COUNTER_USE(unit, port)) {
            ...
            if (&SOC_CONTROL(unit)->controlled_counters != NULL) {
                            sal_sprintf(cname, "%-33s %-8s :", SOC_CONTROL(unit)->controlled_counters[ctr_reg].short_cname,
                                           SOC_PORT_NAME(unit, port));
            } 
        } else {
            /* DMA Counters */
                if (strlen(SOC_REG_NAME(unit, ctr_reg)) > 13) {
            ...
            if (cmap == NULL) {
                ...
                for (index = 0; soc->controlled_counters[index].controlled_counter_f != NULL; index++) {
                    ...
                    if (SOC_IS_ARAD(unit)) {
                        soc_error_t rv;
                        int enable = 0;
                        rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_stat_controlled_counter_enable_get,(unit, port, index, &enable));
    ...
    if (ARG_CNT(a)) {
        if (!sal_strcasecmp(_ARG_CUR(a), "info")) {
            ...
            result = bcm_dpp_counter_diag_info_get(unit,
                                                   ...
                                                   &value);
            if (BCM_E_NONE == result) {
                cli_out("  background wait    = %12d microseconds\n", value);
            } else {
                cli_out("  background wait    = [error %d reading]\n", result);
            }
            result = bcm_dpp_counter_diag_info_get(unit,
            ...
            if ((strlen(_ARG_CUR(a)) > 1) &&
                (!sal_strncasecmp(_ARG_CUR(a), "cached", strlen(_ARG_CUR(a))))) {
                cached = TRUE;
                ARG_NEXT(a);
            } else {
                cached = FALSE;
            ...
            if (strlen(_ARG_CUR(a)) > 4) {
                if (!sal_strncasecmp(_ARG_CUR(a), "withproc", strlen(_ARG_CUR(a)))) {
                    cached = BCM_DPP_COUNTER_WITH_PROC;
                    ARG_NEXT(a);
                } else if (!sal_strncasecmp(_ARG_CUR(a), "withid", strlen(_ARG_CUR(a)))) {
