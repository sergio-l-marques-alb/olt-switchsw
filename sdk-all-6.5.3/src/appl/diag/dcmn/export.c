...
diag_dnx_export_dump(int unit, args_t* a){
    ...
    if(filename != NULL) {
        /* Start Log */
        sprintf(shell_cmd, "log file=%s", filename);
...
diag_dnx_export_dump_usage(int unit){
    cli_out("'dump [PSRC=<port number> DATA=<data>] [CORE=<core>] ' - sends a packet and creates export dump; \n\
