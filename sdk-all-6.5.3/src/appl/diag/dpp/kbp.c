...
int dpp_kaps_init(int unit) {
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "kaps_arm_enable", 0) && !SOC_IS_JERICHO_PLUS(unit)) {
...
cmd_dpp_kbp_test_ipv4_random(int unit, args_t *a)
{
    ...
    for (i = 0; i < entry_num/entry_num_print_mod; i++)
    {
        ...
        if ((cached_entries_num > 0) && (SOC_IS_QAX(unit) && !SOC_DPP_IS_ELK_ENABLE(unit))) {
    ...
    if ((cached_entries_num > 0) && (SOC_IS_JERICHO_PLUS(unit) && !SOC_DPP_IS_ELK_ENABLE(unit))) {
        bcm_switch_control_set(unit, bcmSwitchL3RouteCommit, 1);
        if (cache_value == 0) {
            /*deinit the caching mechanism*/
            bcm_switch_control_set(unit, bcmSwitchL3RouteCache, 0);
...
STATIC cmd_result_t
cmd_dpp_kbp_test_ipv4_uc_scale_capacity(int unit, args_t *a)
{
    int             j;
    ...
    if ((SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) == 0) {
...
cmd_dpp_kbp(int unit, args_t *a)
{
    ...
    } else if (!sal_strcasecmp(func, "test_ipv4_scale_capacity")) {

        return cmd_dpp_kbp_test_ipv4_uc_scale_capacity(unit, a);
