...
#define SOC_ALPM_MOVE_PIVOT_FULL_128(u, src, dst)\
    do {\
        ...
        int tcam = SOC_ALPM_128_ADDR_LWR((_dst)) << 1;\
        /* Track full-entry move */\
        ALPM_TCAM_PIVOT((u), tcam) = \
                ALPM_TCAM_PIVOT((u), SOC_ALPM_128_ADDR_LWR((_src)) << 1);\
        if (ALPM_TCAM_PIVOT((u), tcam)) {\
            PIVOT_TCAM_INDEX(ALPM_TCAM_PIVOT((u), tcam)) = tcam;\
            soc_th_alpm_lpm_move_bu_upd((u), \
                PIVOT_BUCKET_INDEX(ALPM_TCAM_PIVOT((u), tcam)), tcam);\
        }\
        ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR((_src)) << 1) = NULL;\
    } while (0)
...
static int soc_th_alpm_128_lpm_insert(int u, void *entry_data, int *index);
...
_soc_th_alpm_128_lpm_ent_init(int unit, void *alpm_entry, soc_mem_t mem,
                              ...
                              void *lpm_entry);
static int
_soc_th_key_lpm_128_ent_init(int unit, uint32 *key, int len, int vrf, int ipv6,
                             defip_pair_128_entry_t *lpm_entry, int nh_ecmp_idx,
                             int init);

static int soc_th_alpm_128_vrf_delete(int u, int vrf, int v6);

static int
_soc_th_alpm_128_init_pivot(int u, alpm_pfx_info_t *pi, trie_t *pfx_trie,
                            uint32 *pivot, uint32 length,
                            trie_node_t *split_trie_root,
                            defip_pair_128_entry_t *lpm_entry, uint32 *bpm_len);

static int
_soc_th_alpm_128_move_inval(int u, soc_mem_t mem,
                            alpm_mem_prefix_array_t *pa, int *new_index);

/* Extern */
extern int soc_th_get_alpm_banks(int unit);
extern int soc_alpm_cmn_mode_get(int u);
extern int _soc_th_alpm_free_pfx_trie(int u, trie_t *pfx_trie, trie_t *bkt_trie,
...
extern void soc_th_alpm_lpm_move_bu_upd(int u, int bucket_id, int pivot_index);
extern int soc_th_alpm_lpm_delete(int u, void *key_data);

/* Debug */
int alpm_128_split_count;
               ...
               int sw_find)
{
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        if (hit) { /* Entry is found in the bktid */
            ...
            _soc_th_alpm_128_mem_ent_init(u, key_data, e, 0, mem, 0, 0, *bktid);
            /* Perform lookup */
...
static int _soc_th_alpm_128_update_bpm_len(int u, int new_tcam_index, int bpm_len)
{
...
_soc_th_alpm_128_tcam_shadow_ent_init(int u, int r_index, void *entry,
                                   defip_aux_table_entry_t *shadow, int w_index)
{
    ...
    SOC_IF_ERROR_RETURN(soc_mem_read(u, s_mem, MEM_BLOCK_ANY,
                        SOC_ALPM_128_ADDR_LWR(r_index), shadow));
    SOC_IF_ERROR_RETURN(soc_mem_read(u, s_mem, MEM_BLOCK_ANY,
                        SOC_ALPM_128_ADDR_UPR(r_index), shadow + 1));
    ...
    rv = soc_th_alpm_128_lpm_vrf_get(u, entry, &vrf, &pfx);
    SOC_IF_ERROR_RETURN(rv);
    ...
    if (urpf) {
        ...
        tmp = soc_mem_field32_get(u, mem, entry, ALG_SUB_BKT_PTRf);
        /* modify only valid bucket pointer */
        if (tmp) {
            soc_mem_field32_set(u, mem, entry, ALG_SUB_BKT_PTRf, tmp);
        }
    }
                          ...
                          void *entry)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_th_alpm_128_tcam_shadow_ent_init(u, s_index, entry,
                        &shadow[0], index));
...
static int
_soc_th_alpm_128_move_relink(int u, int v6, int pivot_index, int bktid)
{
    int             rv = SOC_E_NONE;
    int             lpm_index;
    ...
    lpm_index = soc_th_alpm_logical_idx(u, lpm_mem,
                    SOC_ALPM_128_DEFIP_TO_PAIR(pivot_index >> 1), 1);

    rv = soc_mem_read(u, lpm_mem, MEM_BLOCK_ANY, lpm_index, &lpm_entry);
    SOC_IF_ERROR_RETURN(rv);
    soc_mem_field32_set(u, lpm_mem, &lpm_entry, ALG_BKT_PTRf, bidx);
    soc_mem_field32_set(u, lpm_mem, &lpm_entry, ALG_SUB_BKT_PTRf, sbidx);
    ...
    if (SOC_URPF_STATUS_GET(u)) {
        int _urpf_index = (soc_mem_index_count(u, lpm_mem) >> 1) +
                            ...
                            SOC_ALPM_128_DEFIP_TO_PAIR(pivot_index >> 1), 1);
        rv = READ_L3_DEFIP_PAIR_128m(u, MEM_BLOCK_ANY, _urpf_index, &lpm_entry);
        SOC_IF_ERROR_RETURN(rv);
...
/* Move Bucket ID from_bkt to Bucket ID to_bkt */
int
_soc_th_alpm_128_move_trie(int u, int v6, int from_bkt, int to_bkt)
{
...
#define _ALPM_IF_ERROR_GOTO_FREE(_rv) \
    ...
    /* Connect TCAM entry to new bucket */
    rv = _soc_th_alpm_128_move_relink(u, v6, pivot_index, to_bkt);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
    ...
    rv = _soc_th_alpm_128_move_inval(u, mem, pa, new_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
...
_rollback:
    if (SOC_FAILURE(rv)) {
        ...
        rv2 = _soc_th_alpm_128_move_relink(u, v6, pivot_index, from_bkt);
...
static void
_soc_th_128_alpm_rollback_pivot_add(int u, defip_pair_128_entry_t *plpm_entry,
                                    ...
                                    alpm_pivot_t *pivot_pyld)
{
    ...
    rv = soc_th_alpm_128_lpm_delete(u, plpm_entry);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                            ...
                            alpm_mem_prefix_array_t *pa, int *new_index)
{
    ...
    defip_alpm_ipv6_128_entry_t alpmv6_entry;
    ...
    rb_bufp = sal_alloc(entsz * pa->count, "rb_bufp");
    rb_sip_bufp = sal_alloc(entsz * pa->count, "rb_sip_bufp");
                            ...
                            defip_pair_128_entry_t *lpm_entry, uint32 *bpm_len)
{
    ...
    if (tmp_pyld->bkt_ptr) {
        ...
        rv = _soc_th_alpm_128_lpm_ent_init(u, bufp, mem, v6, pi->vrf_id, pi->bktid,
        ...
        *bpm_len = ((payload_t *)(tmp_pyld->bkt_ptr))->len;
        ...
        rv = soc_mem_read(u, L3_DEFIP_PAIR_128m, MEM_BLOCK_ANY,
                          soc_th_alpm_logical_idx(u, L3_DEFIP_PAIR_128m,
                                SOC_ALPM_128_DEFIP_TO_PAIR(old_tcam_index >> 1),
                                1),
                          lpm_entry);
    }
    ...
    _soc_th_key_lpm_128_ent_init(u, pivot, length, pi->vrf, v6,
                                 lpm_entry, 0, 0);
...
static int
_soc_th_alpm_128_split_trie(int u, alpm_pfx_info_t *pi,
                            int *new_tcam_index, int *index_ptr)
{
    trie_node_t *split_trie_root;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    int         v6;
    ...
    uint32      pivot[5];
    int         key_index;
    defip_pair_128_entry_t alpmv6_entry, alpmv6_sip_entry;
    ...
    /* prefix trie */
    pfx_trie = VRF_PREFIX_TRIE_IPV6_128(u, pi->vrf);
    ...
    rv = trie_split(trie, _MAX_KEY_LEN_144_, FALSE,
    ...
    rv = _soc_th_alpm_128_init_pivot(u, pi, pfx_trie, pivot,
                                     length, split_trie_root, &lpm_entry,
                                     &bpm_len);
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "could not initialize pivot")));
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   new_index, pi->bktid, pi->vrf, v6);
        return rv;
    }
    ...
    old_index = sal_alloc(sizeof(*old_index)*prefix_array.count, "old_index");
    if (old_index == NULL) {
        rv = SOC_E_MEMORY;
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   old_index, pi->bktid, pi->vrf, v6);
        return rv;
    }
    ...
    for (i = 0; i < prefix_array.count; i ++) {
         ...
         * bucket, no point reading */
            ...
            if (SOC_FAILURE(rv)) {
                ...
                (void) _soc_th_alpm_rollback_bkt_move(u, L3_DEFIP_MODE_128,
                                pi->key_data, mem, old_pivot, pi->pivot_pyld,
                                &prefix_array, new_index, new_insert);
                sal_free(old_index);
                return rv;
    ...
    rv = soc_th_alpm_128_lpm_insert(u, &lpm_entry, new_tcam_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
    ...
    *new_tcam_index = soc_th_alpm_physical_idx(u, L3_DEFIP_PAIR_128m,
                                               *new_tcam_index, v6);

    rv = _soc_th_alpm_128_update_bpm_len(u, *new_tcam_index, bpm_len);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_alpm_insert: Unable to init bpm_len "
                              "for index %d\n"), *new_tcam_index));
        _soc_th_128_alpm_rollback_pivot_add(u, &lpm_entry, pi->key_data,
    ...
    if (new_insert == -1) {
        ...
        rv = _soc_th_alpm_insert_in_bkt(u, mem, PIVOT_BUCKET_INDEX(old_pivot),
                            bank_disable, pi->alpm_data, e, &key_index, v6);
...
{
    alpm_pivot_t *pivot_pyld, *old_pivot;
    defip_aux_scratch_entry_t aux_entry;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    uint32      prefix[5], length;
    int         v6, vrf, vrf_id;
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                 (BSL_META_U(u, "_soc_alpm_128_insert: prefix create failed\n")));
        return rv;
    }
    ...
    if (rv == SOC_E_FULL) {
        ...
        hit = _soc_th_alpm_bkt_entry_cnt(u, L3_DEFIP_MODE_128);
        if (hit > 4) {
            hit = 4;
        }
        if (PIVOT_BUCKET_COUNT(ALPM_TCAM_PIVOT(u, tcam_index)) < hit) {
            ...
            SOC_IF_ERROR_RETURN(rv);
            rv = _soc_th_alpm_128_move_trie(u, v6, bktid, bucket_new);
            SOC_IF_ERROR_RETURN(rv);
            bktid = bucket_new;
            soc_mem_field32_set(u, mem, alpm_data, SUB_BKT_PTRf,
                                ALPM_BKT_SIDX(bktid));
    ...
    if (SOC_FAILURE(rv)) {
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pfx_payload,
                                   new_index, bktid, vrf, v6);
        return rv;
    }
    ...
    if (split) {
        ...
        pi.pivot_pyld       = pivot_pyld;
        pi.alpm_data        = alpm_data;
        pi.alpm_sip_data    = alpm_sip_data;
        pi.bktid            = bktid;
        pi.vrf_id           = vrf_id;
        pi.vrf              = vrf;
        rv = _soc_th_alpm_128_split_trie(u, &pi, &new_tcam_index, index_ptr);
        if (rv != SOC_E_NONE) {
           return rv;
        }
        /* New Pivot */
...
static int
_soc_th_alpm_128_bkt_count(int u, soc_mem_t mem, int bkt_idx, void *bufp)
{
    int bank, entry;
    int addr, rv;
...
_soc_th_alpm_128_delete(int u,
                    void *key_data, int vrf_id, int vrf, int bktid,
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        if (bktid == 0) {
            ...
            rv = _soc_th_alpm_128_find(u, mem, key_data, vrf_id, vrf, bufp,
                                       &tcam_index, &bktid, &key_index, TRUE);
        } else {
        ...
        if (!default_route) {
            ...
            rv = _soc_th_key_lpm_128_ent_init(u, prop_pfx, bpm_pfx_len, vrf, v6,
                                              &lpm_entry, 0, 1);
            ...
            (void) _soc_th_key_lpm_128_ent_init(u, prop_pfx, length, vrf, v6,
                                                &lpm_entry, 0, 0);
            ...
            if ((bpm_pfx_len == 0)  && SOC_FAILURE(rv2)) {
                ...
                rv = _soc_th_key_lpm_128_ent_init(u, prefix, bpm_pfx_len, vrf,
                                                  v6, &lpm_entry, 0, 1);
            }
            if (SOC_FAILURE(rv)) {
                 LOG_ERROR(BSL_LS_SOC_ALPM,
        ...
        if ((pivot_pyld->len != 0) && (trie->trie == NULL)) { /* Empty bucket */
            ...
            /* Initialize an lpm entry to perform a match */
            _soc_th_key_lpm_128_ent_init(u, hw_key, pivot_pyld->len, vrf, v6,
                                         &lpm_entry, 0, 1);
        ...
        if ((pivot_pyld->len != 0) && (trie->trie == NULL)) { /* Empty bucket */
            ...
            if (SOC_FAILURE(rv)) {
                 LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
                          "_soc_th_alpm_128_delete: Unable to release"
...
#ifdef CHECK_CLEAN_BKT
            assert(_soc_th_alpm_128_bkt_count(u, mem, delete_bucket, bufp)
                                                                    == 0);
#endif /* CHECK_CLEAN_BKT */
        }
    }
...
soc_th_alpm_128_vrf_add(int u, int vrf, int v6)
{
    ...
    /* Initialize an lpm entry to perform a match */
    _soc_th_key_lpm_128_ent_init(u, key, 0, vrf, v6, lpm_entry, 0, 1);
    ...
    rv = soc_th_alpm_128_lpm_insert(u, &tmp_lpm, &index);
    if (SOC_FAILURE(rv)) {
        goto _err_exit;
    }
...
soc_th_alpm_128_vrf_delete(int u, int vrf, int v6)
{
    ...
    if (SOC_FAILURE(rv)) {
         LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                  "soc_th_alpm_bs_free: unable to free bucket %d"
                  " vrf %d/%d\n"), bktid, vrf, v6));
    }
 ...
 */
int
soc_th_alpm_128_insert(int u, void *entry_data, uint32 src_flags, int ent_index,
                    int pivot_info)
{
    /* Search Result buffer */
    defip_alpm_ipv6_128_entry_t alpmv6_entry, alpmv6_sip_entry;
    ...
    SOC_IF_ERROR_RETURN(
        _soc_th_alpm_128_mem_ent_init(u, entry_data, bufp, bufp2, mem, src_flags,
                                      &default_route, bktid));
    ...
    /* Global High Prefixes reside in TCAM directly, in Parallel mode */
     if (ALPM_PREFIX_IN_TCAM(u, vrf_id) || (ipmc_route)) { /* Insert into TCAM directly and return */
        ...
        rv = soc_th_alpm_128_lpm_insert(u, entry_data, &index);
        if (SOC_SUCCESS(rv)) {
                ...
                int *cookie)
{
    ...
    *index_ptr = 0;
    ...
    /* Find and update the entry */
    rv = _soc_th_alpm_128_find(u, mem, key_data, vrf_id, vrf, bufp,
                               &tcam_index, &bktid, index_ptr, TRUE);
    SOC_ALPM_LPM_UNLOCK(u);
    if (SOC_FAILURE(rv)) {
        *cookie = tcam_index;
                                 ...
                                 int do_urpf)
{
    ...
    /* Found */
    if (alpm_len != -1) {
        rv = _soc_th_alpm_128_lpm_ent_init(u, &alpm_bufp, mem, v6, vrf,
...
#define ALPM_128_KEY_FIELD_NUM  4
    ...
    /* Fill in AUX Scratch and perform PREFIX Operation */
    SOC_IF_ERROR_RETURN(soc_th_alpm_128_lpm_vrf_get(u, key_data, &vrf_id, &vrf));
    rv = _soc_th_alpm_128_find_best_match(u, key_data, e, vrf, &tcam_index,
...
soc_th_alpm_128_delete(int u, void *key_data, int key_idx, int pivot_info)
{
    ...
    if (ALPM_PREFIX_IN_TCAM(u, vrf_id) || (ipmc_route)) { /* TCAM Direct lookup */
        ...
        if (SOC_SUCCESS(rv)) {
            /* Update debug counter for GLobal */
            if (vrf_id == SOC_L3_VRF_OVERRIDE) {
        ...
        rv = _soc_th_alpm_128_delete(u, key_data, vrf_id, vrf, bktid,
                                     tcam_index, key_idx);
    }
    return(rv);
}
                            ...
                            uint16 *key_index)
{
    ...
    while(index != FB_LPM_128_HASH_INDEX_NULL) {
        uint32  e[SOC_MAX_MEM_FIELD_WORDS];
 ...
 */
static
int _lpm_128_fb_urpf_entry_replicate(int u, int index, uint32 *e)
{
    ...
    if (mask0 && mask1 && mask2 && mask3) {
        SOC_MEM_OPT_F32_SET_128(u, L3_DEFIP_PAIR_128m, e, RPEf, def_gw_flag);
    }

    /* Write entry to the second half of the tcam. */
    return _soc_th_alpm_128_write_pivot(u, MEM_BLOCK_ANY, index + src_tcam_offset,
                                 index, e);
}
...
int _lpm_128_fb_entry_shift(int u, int from_ent, int to_ent)
{
    ...
    LPM_128_HASH_INSERT(u, e, to_ent);
    SOC_IF_ERROR_RETURN(_soc_th_alpm_128_write_pivot(u, MEM_BLOCK_ANY, to_ent,
                        from_ent, e));
    SOC_IF_ERROR_RETURN(_lpm_128_fb_urpf_entry_replicate(u, to_ent, e));
...
int _lpm_128_free_slot_delete(int u, int pfx, int ipv6, void *e, int slot)
{
    ...
    if (to_ent != from_ent) {
        ...
        LPM_128_HASH_INSERT(u, ef, to_ent);
        /* coverity[copy_paste_error : FALSE] */
        if ((rv = _soc_th_alpm_128_write_pivot(u, MEM_BLOCK_ANY, to_ent, from_ent, ef)) < 0)
        {
            return rv;
        }
        if ((rv = _lpm_128_fb_urpf_entry_replicate(u, to_ent, ef)) < 0) {
            return rv;
        }
    }
    ...
    ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(tmp) << 1) =
                      ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(slot) << 1);
    if (ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(tmp) << 1)) {
        PIVOT_TCAM_INDEX(ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(tmp) << 1)) =
                         SOC_ALPM_128_ADDR_LWR(tmp) << 1;
        soc_th_alpm_lpm_move_bu_upd(u,
            PIVOT_BUCKET_INDEX(ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(tmp) << 1)),
            SOC_ALPM_128_ADDR_LWR(tmp) << 1);
    }
    ALPM_TCAM_PIVOT(u, SOC_ALPM_128_ADDR_LWR(slot) << 1) = NULL;

    sal_memcpy(ef, soc_mem_entry_null(u, L3_DEFIP_PAIR_128m),
               soc_mem_entry_words(u,L3_DEFIP_PAIR_128m) * 4);
    LPM_128_HASH_INSERT(u, ef, from_ent);
    if ((rv = _soc_th_alpm_128_write_pivot(u, MEM_BLOCK_ANY, from_ent, from_ent,
                                        ef)) < 0) {
        return rv;
    }
    if ((rv = _lpm_128_fb_urpf_entry_replicate(u, from_ent, ef)) < 0) {
        return rv;
    }
 ...
 */
int
soc_th_alpm_128_lpm_vrf_get(int unit, void *lpm_entry, int *vrf, int *mem_vrf)
{
    int vrf_id;
    ...
    if (SOC_MEM_OPT_FIELD_VALID_128(unit, L3_DEFIP_PAIR_128m,
                                            VRF_ID_MASK0_LWRf)){
        vrf_id = SOC_MEM_OPT_F32_GET_128(unit, L3_DEFIP_PAIR_128m, lpm_entry,
                                         VRF_ID_0_LWRf);
        *mem_vrf = vrf_id;
 ...
 */
static int
soc_th_alpm_128_lpm_insert(int u, void *entry_data, int *lpm_index)
{
    int         pfx;
    int         index;
    int         ipv6;
    ...
    SOC_ALPM_LPM_LOCK(u);
    rv = _soc_th_alpm_128_lpm_match(u, entry_data, e, &index, &pfx, &ipv6, 0, 0);
    if (rv == SOC_E_NOT_FOUND) {
    ...
    if (rv == SOC_E_NONE) {
        ...
        LOG_INFO(BSL_LS_SOC_ALPM,
                 ...
                 "soc_th_alpm_128_lpm_insert: %d %d\n"),
                  index, pfx));
        if (!found) {
            LPM_128_HASH_INSERT(u, entry_data, index);
        }
        rv = _soc_th_alpm_128_write_pivot(u, MEM_BLOCK_ANY, index, index,
                                   entry_data);
        if (rv >= 0) {
            rv = _lpm_128_fb_urpf_entry_replicate(u, index, entry_data);
        }
    }
...
soc_th_alpm_128_lpm_delete(int u, void *key_data)
{
    ...
    SOC_ALPM_LPM_LOCK(u);
    rv = _soc_th_alpm_128_lpm_match(u, key_data, e, &index, &pfx, &ipv6, 0, 0);
    if (rv == SOC_E_NONE) {
        LOG_INFO(BSL_LS_SOC_ALPM,
                 (BSL_META_U(u,
                                 ...
                                 int alpm_idx )    /* ALPM entry index       */
{
    ...
    /* Get VRF */
    SOC_IF_ERROR_RETURN(soc_th_alpm_128_lpm_vrf_get(unit, lpm_entry, &vrf_int,
                                                    &vrf));
 ...
 */
static int
soc_th_alpm_128_warmboot_vrf_add(int unit, int v6, int vrf, int idx, int bkt_ptr)
{
    int           rv = SOC_E_NONE;
    uint32        length;
    uint32        key[5] = {0, 0, 0, 0, 0};
    ...
    /* Initialize an lpm entry to perform a match */
    _soc_th_key_lpm_128_ent_init(unit, key, 0, vrf, v6, lpm_entry, 0, 1);
    ...
    SOC_MEM_OPT_F32_SET_128(unit, L3_DEFIP_PAIR_128m, lpm_entry, ALG_BKT_PTRf,
                                       ALPM_BKT_IDX(unit, bkt_ptr));
    SOC_MEM_OPT_F32_SET_128(unit, L3_DEFIP_PAIR_128m, lpm_entry, ALG_SUB_BKT_PTRf,
                                       ALPM_BKT_SIDX(bkt_ptr));
                                ...
                                int alpm_idx)
{
    ...
    tcam_idx = soc_th_alpm_physical_idx(unit, L3_DEFIP_PAIR_128m, tcam_idx, ipv6);
    tcam_idx = SOC_ALPM_128_ADDR_LWR(tcam_idx) << 1;

    /* Get ALPM table memory. */
    SOC_ALPM_TABLE_MEM_128(unit, ipv6, mem);

    /* Get VRF */
    SOC_IF_ERROR_RETURN(soc_th_alpm_128_lpm_vrf_get(unit, lpm_entry, &vrf_int, &vrf));
    ...
    if (!VRF_TRIE_INIT_COMPLETED(unit, vrf, ipv6)) {
        ...
        rv = soc_th_alpm_128_warmboot_vrf_add(unit, ipv6, vrf, tcam_idx, bktid);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                     (BSL_META_U(unit,
                                 "VRF %d/%d trie init \n"
                             ...
                             void *lpm_entry)
{
    ...
    SOC_IF_ERROR_RETURN
        (_soc_th_alpm_128_lpm_prefix_length_get(unit, lpm_entry, &pfx_len, 0, 0));
...
soc_th_alpm_128_warmboot_lpm_sw_dump(int unit)
{
    ...
    for(i = 0; i < hash128->index_count; i++) {
        if (hash128->table[i] != FB_LPM_HASH_INDEX_NULL) {
            LOG_CLI((BSL_META_U(unit,
 ...
 */
int
_soc_th_alpm_128_sanity_check(int u, int min, int max, int check_sw, int *bkt_ptr_arr)
{
    int             rv = SOC_E_NONE;
    ...
    for (lpm_idx = min; lpm_idx <= max; lpm_idx++) {
        ...
        if (SOC_SUCCESS(rv)) {
            ...
            rv = _soc_th_alpm_128_find(u, alpm_mem, &lpm_bmp,
                                       vrf_id, vrf, e,
        ...
        for (ent = 0; ent < ent_num; ent++) {
            (void) _soc_th_alpm_mem_index(u, alpm_mem, bkt_ptr, ent, bank_disable,
                                          &alpm_idx);
            ...
            rv = _soc_th_alpm_128_find(u, alpm_mem, &lpm_entry2,
                                       vrf_id, vrf, e,
                                       &fd_lpm_idx, &fd_bkt_ptr,
                                       &fd_alpm_idx, FALSE);
