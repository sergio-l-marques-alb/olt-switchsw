...
void soc_th_alpm_lpm_move_bu_upd(int u, int bucket_id, int pivot_index)
{
    ...
    int bkt_ptr = ALPM_BKT_IDX(u, bucket_id);
...
{
    int rv;
    rv = soc_mem_alpm_lookup(u, mem, ALPM_BKT_IDX(u, bktid),
                             MEM_BLOCK_ANY,
                             bank_disable,
    ...
    if (SOC_TH_ALPM_SCALE_CHECK(u, v6)) {
        return soc_mem_alpm_lookup(u, mem, ALPM_BKT_IDX(u, bktid) + 1,
...
_soc_th_alpm_bs_bitmap_get_next(int u, int start, alpm_bkt_bmp_t *alpm_bb)
{
    ...
    for (i = start + 1; i < SOC_TH_ALPM_MAX_BKTS(u); i++) {
...
#define _ALPM_IF_ERROR_GOTO_FREE(_rv) \
    ...
    fb      = ALPM_BKT_IDX(u, from_bkt);
    fsb     = ALPM_BKT_SIDX(from_bkt);
    tob     = ALPM_BKT_IDX(u, to_bkt);
                       ...
                       uint32 bank_disable, int *mem_index)
{
    ...
    int alpm_bkt_bits = SOC_TH_ALPM_BUCKET_BITS + 1;
    switch (mem) {
    ...
    if (SOC_IS_TOMAHAWK(u)) {
        alpm_bkt_bits = SOC_TH_ALPM_BUCKET_BITS;
    }
    ...
    ent_start_bit = (alpm_bkt_bits + bkt_start_bit);
                        ...
                        int *index_ptr)
{
    ...
    i -= SOC_ALPM_BS_BKT_USAGE_COUNT(u, ALPM_BKT_IDX(u, pi->bktid));
                        ...
                        int *key_index, int v6)
{
    ...
    rv = soc_mem_alpm_delete(u, mem, ALPM_BKT_IDX(u, delete_bucket), MEM_BLOCK_ALL,
                             bank_disable, bufp2, e, key_index);
    if (SOC_SUCCESS(rv)) {
        return rv;
    }
    if (SOC_TH_ALPM_SCALE_CHECK(u, v6)) {
        return soc_mem_alpm_delete(u, mem, ALPM_BKT_IDX(u, delete_bucket) + 1,
                    ...
                    int tcam_index, int key_index)  /* TCAM entry data */
{
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        if (SOC_URPF_STATUS_GET(u)) {
            ...
            delete_bucket = SOC_TH_ALPM_RPF_BKT_IDX(u, ALPM_BKT_IDX(u, delete_bucket));
                   ...
                   int pivot_info)
{
    ...
    if (ent_index != -1) {
        bktid = ALPM_BKTID(ALPM_BKT_IDX(u,
                           SOC_TH_ALPM_BKT_ENTRY_TO_IDX(u, ent_index)),
    ...
    if (pivot_info & SOC_ALPM_LOOKUP_HIT) {
        ...
        bucket_idx = ALPM_BKT_IDX(u, SOC_TH_ALPM_BKT_ENTRY_TO_IDX(u, ent_index));
                             ...
                             int do_urpf)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_ALPM,
                ...
                ALPM_BKT_IDX(u, *bucket_index), ALPM_BKT_SIDX(*bucket_index)));
    ...
    LOG_VERBOSE(BSL_LS_SOC_ALPM,
                ...
                "for Length %d\n"),
                SOC_MEM_NAME(u, mem), ALPM_BKT_IDX(u, *bucket_index),
    ...
    for (i = 0; i < ent_num; i++) {
        ...
        rv = _soc_th_alpm_mem_index(u, mem, ALPM_BKT_IDX(u, *bucket_index),
                            ...
                            int do_urpf)
{
    ...
    if (SOC_SUCCESS(rv)) {
        LOG_VERBOSE(BSL_LS_SOC_ALPM,
                    ...
                    tcam_index, ALPM_BKT_IDX(u, bucket_index),
...
soc_th_alpm_delete(int u, void *key_data, int key_idx, int pivot_info)
{
    ...
    if (ALPM_PREFIX_IN_TCAM(u, vrf_id) || (ipmc_route)) {
        ...
        bucket_idx = ALPM_BKT_IDX(u, SOC_TH_ALPM_BKT_ENTRY_TO_IDX(u, key_idx));
...
soc_th_alpm_dbg_urpf(int u)
{
    ...
    for (bkt = 0; bkt <= SOC_TH_ALPM_MAX_BKTS(u) / 2; bkt++) {
...
_free_mem:
    ...
    if (index != -1) {
        ...
        LOG_CLI(("%-24s%d\n", "Bucket:", ALPM_BKT_IDX(u, bktid)));
        LOG_CLI(("%-24s%d\n", "Sub-Bucket:", ALPM_BKT_SIDX(bktid)));
        LOG_CLI(("%-24s%d\n", "Bank:", index & (_shr_popcount(bnkbits) - 1)));
        bktid = (_shr_popcount(bnkbits) + 1) / 2 + alpm_bkt_bits;
        LOG_CLI(("%-24s%d\n", "Entry:", index >> bktid));
        LOG_CLI(("\n"));
    }
            ...
            for (ent = 0; ent < ent_num; ent++) {
                ...
                if (SOC_FAILURE(rv) ||
                    ...
                    LOG_ERROR(BSL_LS_SOC_ALPM,
                              ...
                              ALPM_BKT_IDX(u, fd_bkt_ptr), ALPM_BKT_SIDX(fd_bkt_ptr),
                               ...
                               int *max_entries, int *min_entries)
{
    ...
    bkt_divisor = 4 / soc_alpm_cmn_banks_get(unit);  /* 2 or 4 bank mode */

    /* following legacy two variables are only for min_ent*/
    num_ent     = SOC_TH_ALPM_MAX_BKTS(unit) / 2;  /* Number of buckets */
    ...
    if (route_type == L3_DEFIP_MODE_V4) {
        ...
        if (SOC_URPF_STATUS_GET(unit)) {
            if (soc_alpm_cmn_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                 ...
                 * physical buckets
                 */
                num_bkt        = ((v4_ent / 2) > (SOC_TH_ALPM_MAX_BKTS(unit) / 2)) ?
                                     (SOC_TH_ALPM_MAX_BKTS(unit) / 2) : (v4_ent / 2);
                max_ent        = num_bkt * bkt_max_ent;
                min_ent        = num_ent * (bkt_max_ent * 2 / 3);
            } else if (soc_alpm_cmn_mode_get(unit) == SOC_ALPM_MODE_PARALLEL) {
                num_bkt        = ((v4_ent / 2) > (SOC_TH_ALPM_MAX_BKTS(unit) / 2 * 2))
                                ? (SOC_TH_ALPM_MAX_BKTS(unit) / 2 * 2) : (v4_ent / 2);
                max_ent        = num_bkt * bkt_max_ent / 2;
                min_ent        = num_ent * (bkt_max_ent / 2 / 3) * 2;
            } else {
                /* calculate private DIP */
                num_bkt        = ((v4_ent / 4) > (SOC_TH_ALPM_MAX_BKTS(unit) / 2)) ?
                                     (SOC_TH_ALPM_MAX_BKTS(unit) / 2) : (v4_ent / 4);
            ...
            if (soc_alpm_cmn_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                if (l3_alpm_ipv4_double_wide[unit]) {
                    num_bkt        = (v4_ent > SOC_TH_ALPM_MAX_BKTS(unit) / 2) ?
                                        (SOC_TH_ALPM_MAX_BKTS(unit) / 2) : v4_ent;
                    max_ent        = num_bkt * bkt_max_ent * 2;
                    min_ent        = num_ent * (bkt_max_ent * 2 * 85 / 100);
                } else {
                    num_bkt        = (v4_ent > SOC_TH_ALPM_MAX_BKTS(unit)) ?
                                          SOC_TH_ALPM_MAX_BKTS(unit) : v4_ent;
                    max_ent        = num_bkt * bkt_max_ent;
        ...
        if (SOC_URPF_STATUS_GET(unit)) {
            ...
            if (soc_alpm_cmn_mode_get(unit) == SOC_ALPM_MODE_COMBINED) {
                ...
                num_bkt        = (v6_64b_ent > SOC_TH_ALPM_MAX_BKTS(unit) / 2) ?
                                       (SOC_TH_ALPM_MAX_BKTS(unit) / 2) : v6_64b_ent;
                max_ent        = num_bkt * bkt_max_ent * 2;
