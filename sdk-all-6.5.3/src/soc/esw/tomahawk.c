...
soc_th_subsidiary_ports_get(int unit, soc_port_t port, soc_pbmp_t *pbmp)
{
    ...
    /*
     * config.bcm specified is not flexport compliant, i.e. does
     * not contain :i/disabled ports or :speed_cap max speed
     * specification
     */
    ...
    for (lane_index = 0; lane_index < num_lanes; lane_index++) {
        if (-1 != si->port_p2l_mapping[phy_port + lane_index]) {
            SOC_PBMP_PORT_ADD(pbm,
                              si->port_p2l_mapping[phy_port + lane_index]);
        }
    }
...
#define _SOC_TH_LINK_DELAY_REG_BLK    2            /* EPIPE BLOCKNUM */
    ...
    _soc_th_iss_log_to_phy_bank_map_shadow[unit] = map;
    SOC_IF_ERROR_RETURN
...
_soc_tomahawk_misc_init(int unit)
{
    ...
    if (dev_id == BCM56968_DEVICE_ID) {
        ...
        if (SOC_PBMP_NEQ(si->ether.bitmap, si->management_pbm)) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                        (BSL_META_U(unit,
                            "Port config error!! 56968 supports only HiGig"
                            "ports.\n")));
            return SOC_E_CONFIG;
        }
    }
...
#define DEFAULT_PG_NUM 7  /* Default Priority Group Num */
    ...
    for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_SHARED_LIMITr(unit, idx, rval));

        rval = 0;
        soc_reg_field_set(unit, MMU_THDM_MCQE_POOL_YELLOW_SHARED_LIMITr,
                          &rval, YELLOW_SHARED_LIMITf, limit/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_YELLOW_SHARED_LIMITr(unit, idx, rval));

        rval = 0;
        soc_reg_field_set(unit, MMU_THDM_MCQE_POOL_RED_SHARED_LIMITr,
                          &rval, RED_SHARED_LIMITf, limit/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_RED_SHARED_LIMITr(unit, idx, rval));

        rval = 0;
        soc_reg_field_set(unit, MMU_THDM_MCQE_POOL_RESUME_LIMITr,
                          &rval, RESUME_LIMITf, limit/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_RESUME_LIMITr(unit, idx, rval));

        rval = 0;
        soc_reg_field_set(unit, MMU_THDM_MCQE_POOL_YELLOW_RESUME_LIMITr,
                          &rval, YELLOW_RESUME_LIMITf, limit/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_YELLOW_RESUME_LIMITr(unit, idx, rval));

        rval = 0;
        soc_reg_field_set(unit, MMU_THDM_MCQE_POOL_RED_RESUME_LIMITr,
                                       &rval, RED_RESUME_LIMITf, limit/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDM_MCQE_POOL_RED_RESUME_LIMITr(unit, idx, rval));
    }
    ...
    for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
        ...
        soc_reg_field_set(unit, MMU_THDR_DB_RESUME_COLOR_LIMIT_SPr, &rval,
                          ...
                          (limit - (default_mtu_cells * 2) + 7)/8);
        SOC_IF_ERROR_RETURN
            (WRITE_MMU_THDR_DB_RESUME_COLOR_LIMIT_SPr(unit, idx, rval));
                                    ...
                                    int post_update)
{
    ...
    if (res & 0x1) { /* if (res & THDI) { */
        ...
        for (idx = 0; idx < _TH_MMU_NUM_POOL; idx++) {
            ...
            if ((cur_limit != 0) &&
                TH_MMU_SHARED_LIMIT_CHK(cur_limit, thdi_shd/granularity,
                                        post_update)) {
