...
static int
_soc_gh_l2_entry_limit_update(int unit, uint32 *l2, 
    uint32 *ptm_tbl, uint32 ptm_entry_dw, 
    uint32 *vvm_tbl, uint32 vvm_entry_dw, uint32 *s_count)
{
...
soc_gh_l2_entry_limit_count_update(int unit)
{
    ...
    for (chnk_idx = index_min; chnk_idx <= index_max; chnk_idx += chunk_size) {
         ...
         for (idx = 0; idx <= (chnk_idx_max - chnk_idx); idx++) {
              ...
              SOC_IF_ERROR_RETURN(_soc_gh_l2_entry_limit_update(unit,
                  l2, ptm_buf, ptm_entry_dw, vvm_buf, vvm_entry_dw,
                  &s_count));    
         } /* End for index */
    } /* End for chnk_idx */
    ...
    if (soc_feature(unit, soc_feature_l2_overflow_bucket)) {
        ...
        SOC_IF_ERROR_RETURN(READ_L2_LEARN_CONTROLr(unit, &l2_learn));
        if (soc_reg_field_get(unit, L2_LEARN_CONTROLr, 
            ...
            buf = soc_cm_salloc(unit, table_chnk_sz, "l2x_overflow");
            ...
            rv =  soc_mem_read_range(unit, mem, MEM_BLOCK_ANY, index_min, 
                                  index_max, buf);
            if (rv < 0) {
                SOC_L2X_MEM_UNLOCK(unit);
                LOG_ERROR(BSL_LS_SOC_L2,
