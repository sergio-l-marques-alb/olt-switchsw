...
#define _APACHE_MMU_MAX_PACKET_BYTES    12228 /* bytes */
#define _MAVERICK_MMU_MAX_PACKET_BYTES  9416  /* bytes */
...
STATIC int
soc_apache_fc_config_init(int unit)
{
    int rv;
    soc_reg_t reg;
    soc_port_t port;
    uint32 values[2] = {1, 1};
    soc_field_t fields[2] = {MERGE_MODE_ENf, PARALLEL_FC_ENf};
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        values[0] = soc_property_port_get(unit, port, "port_fc_merge_mode_en", 1);
        values[1] = soc_property_port_get(unit, port, "port_fc_parallel_fc_en", 1);

        rv = soc_reg_fields32_modify(unit, reg, port, 2, fields, values);
        SOC_IF_ERROR_RETURN(rv);
    }
                               ...
                               _soc_ser_sram_info_t *sram_info)
{
    ...
    case L3_DEFIP_ALPM_ECCm:
        sram_info->ram_count = SOC_APACHE_NUM_EL_SHARED;
        base_bucket = ((index >> 2) & SOC_APACHE_ALPM_BKT_MASK);
        base = index & 0x3;
        ...
        for (i = 0; i < SOC_APACHE_NUM_EL_SHARED; i++) {
            ...
            sram_info->mem_indexes[i][0] =
                (index & SOC_APACHE_ALPM_BKT_MASK) |
                (bkt_offset << 2) | base;
        }
        sram_info->ram_count = SOC_APACHE_NUM_EL_SHARED;
        base_bucket = ((index  >> 2) & SOC_APACHE_ALPM_BKT_MASK);
...
int
soc_apache_mem_index_remap(int unit, soc_mem_t mem, int index)
{
    switch (mem) {
        case L3_DEFIPm:
        ...
        case L3_DEFIP_PAIR_128_DATA_ONLYm:
            return soc_apache_l3_defip_index_remap(unit, mem, index);
        default:
            return index;
    }
}
            ...
            if (regmem == _SOC_APACHE_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (spci.mem != INVALIDm) {
                        ...
                        spci.flags |= SOC_SER_REG_MEM_KNOWN;
                        spci.index = soc_apache_mem_index_remap(unit, spci.mem, index);
                    }
                    else {
...
#ifdef ALPM_ENABLE
int
soc_apache_alpm_scrub(int unit)
{
    ...
    if (SOC_TH_ALPM_SCALE_CHECK(unit, 1)) {
        step_count = 2;
    } else {
        step_count = 1;
    }
...
#define IS_100G_DISABLED(clp_block) !((SOC_BOND_INFO(unit)->clmac_map) & (1 << (clp_block)))
...
soc_apache_port_config_bandwidth_check(int unit)
{
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        ...
        if ((oversub_mode == 1) && (pgw_linerate_bandwidth != 0)) {
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit, "Linerat emode: Oversub BW for PGW %d is not 0.\n"), pgw));
            rv = SOC_E_FAIL;
        }
...
typedef struct soc_apache_tdm_s {
    ...
    struct {
        ...
        int mmu_ovs_group_speed_spacing[APACHE_MMU_OVS_GROUP_COUNT]; /* Speed spacing for each speed */
...
static const soc_reg_t ap_mmu_ovs_group_wt_regs[APACHE_MMU_OVS_WT_GROUP_COUNT] = {
                      ...
                      soc_apache_tdm_t *apache_tdm)
{
    ...
    uint32 same_spacing, speed_spacing;
                             ...
                             int lossless)
{
    ...
    /* Maverick and FB5 needs to have different Max MTU size.
     * The feature flag soc_feature_mmu_hqos_four_level is
     * used to check for these devices.
     */
    if (!soc_feature(unit, soc_feature_mmu_hqos_four_level)) { 
        devcfg->max_pkt_byte = _MAVERICK_MMU_MAX_PACKET_BYTES;
    } else {
        devcfg->max_pkt_byte = _APACHE_MMU_MAX_PACKET_BYTES;
    }
    ...
    devcfg->mmu_total_cell = _APACHE_MMU_TOTAL_CELLS  - fifo_entry_count -
      port_count  - (lossless ? 10 : 0);
...
_soc_apache_default_lossless_pg_headroom(int unit, soc_port_t port)
{
    ...
    if (SOC_PBMP_MEMBER(si->oversub_pbm, port)) {
        if (si->port_speed_max[port] >= 100000) {
            return 687;
        }else if (si->port_speed_max[port] >= 500000) {
             return 352;    
                               ...
                               soc_port_t input_port)
{
    ...
    if (input_port == -1) {
        ...
        for (idx = 0; idx < SOC_APACHE_MMU_CFG_QGROUP_MAX; idx++) {
            ...
            mem = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_CONFIG_QGROUPm, 
                    MMU_THDU_YPIPE_CONFIG_QGROUPm);
            mem1 = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_OFFSET_QGROUPm, 
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        mem = SOC_APACHE_PMEM(unit, port, THDI_PORT_SP_CONFIG_Xm, 
        ...
        mem = SOC_APACHE_PMEM(unit, port, THDI_PORT_PG_CONFIG_Xm, 
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        mem = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_CONFIG_QUEUEm, 
                                       MMU_THDU_YPIPE_CONFIG_QUEUEm);
        mem1 = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_OFFSET_QUEUEm, 
                                        MMU_THDU_YPIPE_OFFSET_QUEUEm);
        mem2 = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_Q_TO_QGRP_MAPm, 
        ...
        for (idx = 0; idx < _APACHE_MMU_NUM_POOL; idx++) {
            ...
            limit = buf_pool->total - buf_pool->queue_guarantee ;
            mem2 = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_CONFIG_PORTm, 
                                           MMU_THDU_YPIPE_CONFIG_PORTm);
            index1 = SOC_APACHE_MMU_PIPED_MEM_INDEX(unit, port, mem2, idx);
            mem3 = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_RESUME_PORTm, 
...
soc_apache_mmu_config_shared_buf_recalc(int unit, uint32 spid, int shared_size, int flags)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        /* Per  port per pool unicast */
        idx = spid;
        mem = SOC_APACHE_PMEM(unit, port, MMU_THDU_XPIPE_CONFIG_PORTm, 
                                       MMU_THDU_YPIPE_CONFIG_PORTm);
        index1 = SOC_APACHE_MMU_PIPED_MEM_INDEX(unit, port, mem, idx);
        sal_memset(entry0, 0, sizeof(mmu_thdu_xpipe_config_port_entry_t));
...
_soc_apache_mmu_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_apache_fc_config_init(unit));
...
soc_apache_port_asf_set(int unit, soc_port_t port, int speed)
{
    ...
    if (!soc_property_port_get(unit, port, "pfc_optimized_settings", 1)) {
        ...
        if (speed <= 11000) {
            ...
            pfc_watermark = IS_OVERSUB_PORT(unit,port) ? 61 : 61;
        }
    }
