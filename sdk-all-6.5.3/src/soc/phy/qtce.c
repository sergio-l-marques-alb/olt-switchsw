...
qtce_show_serdes_info(phy_ctrl_t *pc, qtce_dev_info_t *pInfo, phymod_core_info_t *core_info)
{
    ...
    sal_strncpy(pInfo->name,"QTCE-", strlen("QTCE-"));
                          ...
                          soc_port_ability_t *ability)
{
    ...
    if(ability->speed_half_duplex) {
        if(ability->speed_full_duplex) {
            LOG_WARN(BSL_LS_SOC_PHY,(BSL_META_U(pc->unit,
               "phy_qtce_ability_advert_get:unit=%d p=%d full half duplex=%08x %08x coexist.\n"),
               unit, port, ability->speed_full_duplex, ability->speed_half_duplex));
        } 
        ...
        } else if(ability->speed_half_duplex & SOC_PA_SPEED_2500MB) {
        ...
        if(ability->speed_full_duplex & SOC_PA_SPEED_1000MB) {
            ...
            PHYMOD_AN_CAP_SGMII_SET(&phymod_autoneg_ability);
            phymod_autoneg_ability.sgmii_speed = phymod_CL37_SGMII_10M;
        } else if(ability->speed_full_duplex & SOC_PA_SPEED_2500MB) {
            PHYMOD_AN_CAP_CL37BAM_SET(&phymod_autoneg_ability);
        }
...
STATIC
int
qtce_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
...
phy_qtce_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_FEC_OFF:
        rv = qtce_fec_enable_set(pmc, 0);
        break;
    case SOC_PHY_CONTROL_FEC_ON:
        rv = qtce_fec_enable_set(pmc, 1);
        break;
