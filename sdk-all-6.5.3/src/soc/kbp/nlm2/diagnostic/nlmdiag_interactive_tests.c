    ...
    )
{
	...
	{
		if(miscData_p->m_searchType[psNum] != NLMDEV_STANDARD &&
			miscData_p->m_searchType[psNum] != NLMDEV_SAHASRA)
			return NLMERR_FAIL;
    ...
    NlmDevSSReg *NetLData_p)
{
    ...
    for(idx = 0; idx < NLMDEV_SS_RMP_AB; idx++)
    {
        if( (NetLData_p->m_ss_result_map[idx]  != NLMDEV_MAP_PE0) &&
        	 (NetLData_p->m_ss_result_map[idx] != NLMDEV_MAP_PE1) )
            return NLMERR_FAIL;
	...
	)
{
	...
	for( iter = 0; iter < NLMDEV_NUM_PARALLEL_SEARCHES; iter++ )
	{
		...
		if( NLMDEV_HIT == cmpRslts_p->m_hitOrMiss[ iter ] )
		{
