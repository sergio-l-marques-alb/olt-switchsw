...
soc_dpp_str_prop_parse_logical_port_mim(int unit, SOC_PPC_LIF_ID *p_val_incoming, SOC_PPC_AC_ID *p_val_outgoing)
{
    ...
    *p_val_outgoing = soc_property_suffix_num_get(unit, 0, propkey, "out", (SOC_IS_QAX(unit) ? 8192: 4096));
...
{    
    ...
    {
        soc_sand_rv = chip_sim_em_init(unit, SOC_SAND_DEV_ARAD);
        SOCDNX_IF_ERR_EXIT(soc_sand_rv);
    }
...
soc_dpp_prop_parse_ingress_congestion_management_pdm_extensions_get(int unit, soc_dpp_config_pdm_extension_t *pdm_extensions)
{
    ...
    if (SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) {
        ...
        if (pdm_extensions->max_st_vsq >= SOC_TMC_ITM_VSQ_GROUPD_SZE(unit)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("MAX ST-VSQ in PDM extension %d is higher than number of ST-VSQ %d."), pdm_extensions->max_st_vsq, SOC_TMC_ITM_VSQ_GROUPD_SZE(unit)));
        }
        ...
        pdm_extensions->max_st_vsq = SOC_TMC_ITM_VSQ_GROUPD_SZE(unit);
...
/* } */
    ...
    /* re-config soc property (soc property was alreadu loaded before init */
    /* overide it here mainly to allow regression system modes */
    stable_size = soc_property_get(unit, spn_STABLE_SIZE,
              SHR_SW_STATE_MAX_DATA_SIZE_IN_BYTES);

#ifdef BCM_WARM_BOOT_SUPPORT
    rv = soc_stable_size_update(unit, stable_size);
    SOCDNX_IF_ERR_RETURN(rv);
#endif
