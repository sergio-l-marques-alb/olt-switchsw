...
{ 
    ...
    SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_port_nif_ilkn_pbmp_get, (unit, port, id, &phy_pbmp)));
...
STATIC int
soc_jer_ilkn_serdes_qmlfs_set( int unit, soc_pbmp_t* phy_pbmp, SHR_BITDCL* serdes_qmlfs)
{
    int i;
...
soc_jer_str_prop_parse_ucode_port(int unit, soc_port_t port, soc_jer_ucode_port_config_t* port_config)
{
    ...
    if (propval){
        ...
        if(port_config->tm_port == 0xFFFFFFFF && (!(port_config->is_sif)) ) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("TM port not specify for (\"%s\") for %s"), propval, propkey));
        }
...
soc_jer_ports_config(int unit) 
{
    ...
    soc_pbmp_t phy_ports, ports_bm,pon_port_bm;
    soc_port_if_t interface;
    ...
    for (port_i = 0; port_i < SOC_DPP_DEFS_GET(unit, nof_logical_ports); ++port_i) 
    {
        ...
        /* set ilkn serdes qmlfs */
        /*for ILKN over Fabric leave all serdes QMLFs to be taken by Eth ports*/
        if(port_config.interface == SOC_PORT_IF_ILKN && !SOC_JER_NIF_IS_ILKN_IF_OVER_FABRIC(unit, port_config.protocol_offset)) {
            serdes_qmlfs = SOC_DPP_CONFIG(unit)->jer->nif.ilkn_qmlf_resources[port_config.protocol_offset].serdes_qmlfs;
            SOCDNX_IF_ERR_EXIT(soc_jer_ilkn_serdes_qmlfs_set(unit, &(port_config.phy_pbmp), serdes_qmlfs));
        }

        if (dcmn_device_block_for_feature(unit,DCMN_NIF_SERDES_FEATURE)) {

            /*QAUD 9 is invalid*/
            if(SOC_PBMP_MEMBER(port_config.phy_pbmp, 37) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 38) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 39) ||
               SOC_PBMP_MEMBER(port_config.phy_pbmp, 40)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("device %s cant mapped to quad 9"), 
                                                  soc_dev_name(unit)));
            }
        ...
        /*ilkn >=5 blocked */
        if (dcmn_device_block_for_feature(unit,DCMN_ILKN_PORTS_FEATURE)) {
            if (port_config.interface == SOC_PORT_IF_ILKN &&
                port_config.protocol_offset >= 4) {
    ...
    if (!SOC_WARM_BOOT(unit)) 
    {
        ...
        SOC_PBMP_ITER(ports_bm, port_i) {
            ...
            if (dcmn_device_block_for_feature(unit,DCMN_DC_FEATURE)) {
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port_i, &interface));
    ...
    int unit)
{
    ...
    if (SOC_IS_QUX(unit)) {
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_SBUS_RING_MAP_104_111r(unit, 0x66666666));
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_SBUS_RING_MAP_112_119r(unit, 0x02000066));
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_SBUS_RING_MAP_120_127r(unit, 0x00000000));
