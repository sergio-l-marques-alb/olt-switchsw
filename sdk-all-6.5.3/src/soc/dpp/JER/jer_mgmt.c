  ...
  )
{
    ...
    data = 0;
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, READ_IPS_CRVSm(unit, MEM_BLOCK_ANY, fap_id / JER_CR_VAL_BMP_NOF_BITS, &data));
    *fap_bitmap = soc_mem_field32_get(unit, IPS_CRVSm, &data, CR_VAL_BMPf);
    ...
    soc_mem_field32_set(unit, IPS_CRVSm, &data, CR_VAL_BMPf, *fap_bitmap); 
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, WRITE_IPS_CRVSm(unit, MEM_BLOCK_ANY, fap_id /JER_CR_VAL_BMP_NOF_BITS, &data)); 
  ...
  )
{
    ...
    if (credit_worth_local != credit_worth_remote) {
        ...
        soc_mem_field32_set(unit, IPS_CRVSm, &data, CR_VAL_BMPf, 0); 
        ...
        soc_mem_field32_set(unit, IPS_CRVSm, &data, CR_VAL_BMPf, 0xffff); 
        res = arad_fill_table_with_entry(unit, IPS_CRVSm, MEM_BLOCK_ANY, &data); 
...
int jer_mgmt_nof_block_instances(int unit, soc_block_types_t block_types, int *nof_block_instances) 
{
    ...
    *nof_block_instances = 0;
    ...
    switch(block_types[0]) {
        ...
        case SOC_BLK_MXQ:    
            *nof_block_instances = SOC_JER_NOF_INSTANCES_MXQ;
            break;
    }
        ...
        switch(block_types[0]) {
            ...
            case SOC_BLK_PEM:    
                *nof_block_instances = SOC_JER_NOF_INSTANCES_PEM;
                break;
        case SOC_BLK_KAPS:    
            *nof_block_instances = SOC_JER_NOF_INSTANCES_KAPS;
            break;
        }
    }    
   /* for new blocks apear in JERICHO_PLUS*/
    if (*nof_block_instances == 0 && SOC_IS_JERICHO_PLUS_A0(unit)) {
        switch(block_types[0]) {

         case SOC_BLK_KAPS_BBS:    
            *nof_block_instances = SOC_JER_NOF_INSTANCES_KAPS_BBS;
            break;
        }
    }    

    if (*nof_block_instances == 0 && SOC_IS_QUX(unit)) {
        switch(block_types[0]) {
            case SOC_BLK_NIF:
                *nof_block_instances = SOC_JER_NOF_INSTANCES_NIF;
                break;
        }
    }

    if (*nof_block_instances == 0 && SOC_IS_FLAIR(unit)) {
        switch(block_types[0]) {
            case SOC_BLK_PLL: 
                *nof_block_instances = SOC_JER_NOF_INSTANCES_PLL;
                break;
        }
    }
