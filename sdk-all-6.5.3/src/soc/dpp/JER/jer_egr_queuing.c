                ...
                if (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON) {
                    ...
                    if (IS_TDM_PORT(unit, port)) {
                        if (offset == 0) {
                            ...
                            egr_if = 16;
                            other_egr_if = 17;
                        } else if ((offset == 2) && SOC_IS_QAX(unit)) {
                            egr_if = 2;
                            other_egr_if = 3;
                        } else if ((offset == 3) && SOC_IS_QAX(unit)) {
                            egr_if = 4;
                            other_egr_if = 5;
                        } else {
                            SOCDNX_EXIT_WITH_ERR(SOC_E_RESOURCE, (_BSL_SOCDNX_MSG("Failed to allocate egr interface, Interlaken ID not supported in ILKN dedicated mode")));
                        }
                    } else {
                        if (offset == 0){
                            egr_if = 1;
        ...
        if (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON) {
            if (interface_type == SOC_PORT_IF_ILKN) {
                ...
                if (protocol_offset == 0) {
                   egr_if = 0;
                } else if (protocol_offset == 1){
                   egr_if = 16;
                } else if (protocol_offset == 2){
                   egr_if = 2;
                } else if (protocol_offset == 3){
                   egr_if = 4;
                } 
            }
        }
