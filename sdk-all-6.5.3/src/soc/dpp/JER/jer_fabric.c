  ...
  )
{
    ...
    rci_config.soc_dpp_fabric_rci_threshold_num_of_congested_links = 0x15;
    ...
    if (SOC_IS_JERICHO_AND_BELOW(unit)){ /*in QAX UC egress core fifos does not exist*/
    ...
    /* Set TXQ to FDA queues default thresholds */
    if (SOC_IS_QAX(unit)){	
        SOCDNX_IF_ERR_EXIT(READ_TXQ_LOCAL_FIFO_CFGr(unit, above_64_reg_val));
        soc_reg_above_64_field32_set(unit, TXQ_LOCAL_FIFO_CFGr, above_64_reg_val, SRAM_DTQ_LOC_EGQ_FC_THf, 72);
        soc_reg_above_64_field32_set(unit, TXQ_LOCAL_FIFO_CFGr, above_64_reg_val, SRAM_DTQ_LOC_GEN_RCI_THf, 72);
        soc_reg_above_64_field32_set(unit, TXQ_LOCAL_FIFO_CFGr, above_64_reg_val, DRAM_DTQ_LOC_EGQ_FC_THf, 115);
        soc_reg_above_64_field32_set(unit, TXQ_LOCAL_FIFO_CFGr, above_64_reg_val, DRAM_DTQ_LOC_GEN_RCI_THf, 115);
        SOCDNX_IF_ERR_EXIT(WRITE_TXQ_LOCAL_FIFO_CFGr(unit, above_64_reg_val));
    }
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit)){
        /*Elij - Need to init relevant registers for OTCR, probably p[1..3]_otcr*/
        soc_reg64_field32_set(unit, FDR_FDR_ENABLERS_REGISTER_1r, &reg64, FIELD_4_11f, 0x2);
    }else{
    ...
    if (!SOC_IS_KALIA(unit) && !SOC_IS_JERICHO_PLUS_A0(unit)) {/*elij - temporary bypass since this is part of Kalia part 3 code*/
...
/*
   Elij - for the above, this is the implementation using modify:
   ---------------------------------------------------------------
   reg = SOC_IS_QAX(unit) ? FDR_REG_011D_1r : FDR_REG_011Er;

   if(SOC_DPP_CONFIG(unit)->arad->init.fabric.is_128_in_system) {
       soc_field_t field_arr [1] = {FIELD_0_0f};
       uint32 value_arr [2] = {0x0};
       SOCDNX_IF_ERR_EXIT(soc_reg_fields32_modify(unit, reg, REG_PORT_ANY, field_arr, value_arr));
   } else {
       soc_field_t field_arr [2] = {FIELD_0_0f, FIELD_4_22f};
       uint32 value_arr [2] = {0x1, 0x1800};
       SOCDNX_IF_ERR_EXIT(soc_reg_fields32_modify(unit, reg, REG_PORT_ANY, 2, field_arr, value_arr));
   }

    Elij - eventually I think the code is too different (due to the loop required in QAX) so if I have SOC_IS_QAX anyhow - the naming issue is solved
    --------------------------------------------------------------------------------------------------------------------------------
*/
    ...
    if (!is_mesh)
    {
        if (!SOC_IS_QAX(unit)) {
            ...
            for (i = 0; i < SOC_DPP_DEFS_GET(unit, nof_instances_fmac); i++) {
                SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAC_REG_0065r(unit, i, &reg_val)); 
                soc_reg_field_set(unit, FMAC_FMAC_REG_0065r, &reg_val, FIELD_4_7f, 0x0);
                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAC_REG_0065r(unit, i, reg_val));
            }
        }
    }
    ...
    if (SOC_IS_KALIA(unit)) {/*elij - temporary bypass till BRDC_FMAC will be supported*/
  ...
  )
{
    ...
    if (fabric->fabric_pcp_enable) {
        ...
        SOCDNX_IF_ERR_EXIT(READ_FDR_FDR_ENABLERS_REGISTER_2r_REG32(unit, &regval));
        soc_reg_field_set(unit, FDR_FDR_ENABLERS_REGISTER_2r, &regval, PCP_ENABLEDf, 1);
        SOCDNX_IF_ERR_EXIT(WRITE_FDR_FDR_ENABLERS_REGISTER_2r_REG32(unit, regval));
  ...
  )
{
    ...
    soc_fabric_dtq_mode_type_t dtq_mode;
    ...
    SOCDNX_IF_ERR_EXIT(READ_FDR_FDR_ENABLERS_REGISTER_2r_REG32(unit, &reg_val));
    soc_reg_field_set(unit, FDR_FDR_ENABLERS_REGISTER_2r, &reg_val, PARALLEL_DATA_PATHf, nof_pipes - 1);
    SOCDNX_IF_ERR_EXIT(WRITE_FDR_FDR_ENABLERS_REGISTER_2r_REG32(unit, reg_val));
     ...
     */
    if (SOC_IS_QAX(unit)){ /*In QAX, mapping pipes to DTQ are determined according to 3 fixed modes*/
        ...
        if (SOC_DPP_IS_MESH(unit)){
            ...
            strict_priority_2_val = SOC_FABRIC_PIPE_STRICT_PRIORITY_2_NONE; /*No pipe for tdm*/
             ...
             default:
                 SOCDNX_EXIT_WITH_ERR(SOC_E_UNIT, (_BSL_SOC_MSG("dtq mode is not supported in QAX")));
                 break;
             }
        }

        /*Set DTQ mode according to map_type*/
        SOCDNX_IF_ERR_EXIT(READ_TXQ_TXQ_GENERAL_CONFIGURATIONr(unit, &reg_val));
        soc_reg_field_set(unit, TXQ_TXQ_GENERAL_CONFIGURATIONr, &reg_val, DTQ_MODEf, dtq_mode); /* 0 = Single Queue, 1 = UC/MC, 2 = UC/HMC/LMC*/
        soc_reg_field_set(unit, TXQ_TXQ_GENERAL_CONFIGURATIONr, &reg_val, Q_0_Q_12_INTERLEAVE_ENf, 1); /*Since we guarantee each dtq queue is going to a specific, unique, pipe -> we can always enable Q0Q12InterleaveEn */
        soc_reg_field_set(unit, TXQ_TXQ_GENERAL_CONFIGURATIONr, &reg_val, Q_1_Q_2_INTERLEAVE_ENf, 1);  /*Since we guarantee each dtq queue is going to a specific, unique, pipe -> we can always enable Q1Q2InterleaveEn */
        SOCDNX_IF_ERR_EXIT(WRITE_TXQ_TXQ_GENERAL_CONFIGURATIONr(unit, reg_val));
  ...
  )
{
    ...
    switch (fifo_type) {
    case soc_dpp_cosq_gport_egress_core_fifo_fabric_ucast:
        SOCDNX_IF_ERR_EXIT(READ_FDA_FDA_EGQ_FABRIC_RCIr_REG32(unit, 0, &reg_val));
         ...
         for (i = 0; i < links_count; ++i) {
            ...
            if (!SOC_IS_QAX(unit)) {
                ...
                *field = soc_reg_field_get(unit, FMAC_LINK_TOPO_MODE_REG_0r, reg32_val, LINK_TOPO_MODE_1f);
                ...
                SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAC_REG_0065r(unit, fmac_index, &reg32_val)); 

                *field = soc_reg_field_get(unit, FMAC_FMAC_REG_0065r, reg32_val, FIELD_0_3f);
                SHR_BITSET(field, fmac_inner_link);
                soc_reg_field_set(unit, FMAC_FMAC_REG_0065r, &reg32_val, FIELD_0_3f, *field);

                *field = soc_reg_field_get(unit, FMAC_FMAC_REG_0065r, reg32_val, FIELD_4_7f);
                SHR_BITCLR(field, fmac_inner_link);
                soc_reg_field_set(unit, FMAC_FMAC_REG_0065r, &reg32_val, FIELD_4_7f, *field);

                SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAC_REG_0065r(unit, fmac_index, reg32_val));
            }
        }
    }
exit:
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, MESH_TOPOLOGY_REG_01B9r, REG_PORT_ANY, 0, FIELD_0_31f, &neg_watermark));
