...
jer_pp_kaps_dma_thread(void *cookie)
{
    ...
    uint32                      num_of_entries_done = 0;
    uint64                     *kaps_tcm_p = NULL;
    uint32                      next_write_max_address;
    ...
    while (1) {
        ...
        if (kaps_tcm_dma_state[unit].terminate) {/*Terminate the thread*/
            /* Terminate the thread only after all pending writes are done to avoid race conditions */
            terminate = 1;
            force_dma = 1;
        }
        ...
        if ((num_of_entries_to_dma >= JER_PP_KAPS_MIN_ENTRIES) || (force_dma && num_of_entries_to_dma)) {
...
uint32 jer_pp_xpt_dma_state(int unit, uint32 print_status, uint32 enable_dma_thread, uint32 wait_arm)
{
    ...
    if (print_status) {
        cli_out("DMA is enabled: %d. Thread terminated: %d.\n",
                kaps_dma_enabled[unit], kaps_tcm_dma_state[unit].terminate);
