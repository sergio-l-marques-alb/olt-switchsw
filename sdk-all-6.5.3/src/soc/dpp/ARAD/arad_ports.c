  ...
  )
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0) == 0) {
        if (SOC_PBMP_MEMBER(pon_port_bm, tm_port_id))   
        {
            ...
            for (tunnel_id = 0; tunnel_id < ARAD_PORTS_VIRTUAL_PORT_PON_MAX_TUNNEL_ID; tunnel_id++) 
            {
                res = READ_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(tm_port_id,tunnel_id), &tbl_data_32b);
                SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 142, exit);
                /* Set System-Port. The System-Port is set per Virtual Port */
                soc_IHP_VIRTUAL_PORT_TABLEm_field32_set(unit, &tbl_data_32b, VIRTUAL_PORT_SRC_SYS_PORTf, sys_port);
                res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, IHP_BLOCK(unit, core), ARAD_PORTS_VIRTUAL_PORT_PON_ENCODED(tm_port_id,tunnel_id), &tbl_data_32b);
  ...
  )
{
    ...
    if (in_enable)
    {
        ...
        if (!skip_config)
        {
            ...
            if((arad_port_header_type_is_injected_2(header_type) && (port % 256) == ARAD_OAMP_PORT_ID) &&
                SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG((unit))->pp.oam_statistics)
            {
  ...
  )
{
    ...
    */
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "preserving_dscp_enabled", 0) && info->flags & ARAD_PORTS_PRESERVING_DSCP_PORT_PP_PORT) {
    ...
    if (SOC_DPP_CONFIG_CUSTOM_PP_PORT_IS_VENDOR2()) {
        if (PBMP_MEMBER(vendor_pp_port_bm, port_ndx)) {
            ...
            for (curr_q_pair = base_q_pair; curr_q_pair < base_q_pair + nof_pairs; curr_q_pair++) {
                ...
                pct_tbl_data.prog_editor_profile = ARAD_PRGE_TM_SELECT_CUSTOMER_CONN_TO_NP;
