 ...
 */
    ...
    #define KBP_SCHAN_JER_OPCODE_64_IHB_WRITE            ((core == 2 ) ? 0x37882000 : (core ? 0x35e82000 : 0x35e02000))
    #define KBP_SCHAN_JER_ADDR_IHB_MSB_WRITE_ADDR        0x0000024f
    #define KBP_SCHAN_JER_ADDR_IHB_LSB_WRITE_ADDR        0x0000023f

    #define KBP_SCHAN_JER_OPCODE_32_IHB_WRITE            ((core == 2 ) ? 0x37880200 : (core ? 0x35e80200 : 0x35e00200))
    #define KBP_SCHAN_JER_ADDR_ROP_OPCODE_ADDR           0x0000025f
    #define KBP_SCHAN_JER_ADDR_ROP_TRIGGER_ADDR          0x0000023e    

    #define KBP_SCHAN_JER_OPCODE_IHB_ROP_READ            ((core == 2 ) ? 0x2F881400 : (core ? 0x2de81400 : 0x2de01400))
        ...
        )
{
    ...
    if ( ARAD_KBP_CPU_DATA_REGISTER_BYTE_LEN >= cbw_cmp_data->cbw_data.data_len) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            arad_kbp_buffer_endians_fix(unit, lsb_data, TRUE); 
        } else {
    ...
    if ((cbw_cmp_data->type == NLM_ARAD_CB_INST_CMP1) || 
        ...
        /* reversed buffer*/
        if (SOC_IS_JERICHO(unit)) {
            arad_kbp_buffer_endians_fix(unit, above_64_cmp_data, TRUE); 
        } else {
