...
*   SOC_SAND_IN  ARAD_SCH_PORT_ID  tm_port -
  ...
  )
{
  ...
  /* Access WRITE_SCH_PORT_ENABLE_PORTEN (jer/arad) - Access specific core. */
...
*     Details: in the H file. (search for prototype)
*
*     See also: arad_sch_port_sched_get_unsafe()
*********************************************************************/
uint32
  ...
  )
{
  ...
  else
  {
    ...
    res = arad_sch_port_hr_group_get(unit, core, tm_port, &group); /* get the new group */
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  res = arad_sch_calendar_info_get(unit, core, 0, 1, &credit, &nof_ticks, &quanta_nof_bits);
   ...
   SOC_SAND_IN   int    core)
{
    ...
    uint32 offset, mem_val, quanta, credit, nof_ticks, quanta_nof_bits;
    int rate;
    uint8 valid;
  ...
  )
{
  ...
  res = arad_sch_calendar_info_get(unit, core, 0, 0, &credit, &nof_ticks, &quanta_nof_bits);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 2, exit);
...
arad_sch_calendar_info_get(int unit, int core, int hr_calendar_num ,  int is_priority_rate_calendar ,
                          uint32 *credit, uint32 *nof_ticks, uint32 *quanta_nof_bits)
{
    uint32 reg_val, field_val, credit_worth, device_ticks_per_sec, access_period, cal_length, divider;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if (hr_calendar_num == 0) /* in jericho two copies of calendar are available */
    {
    ...
    *credit = SOC_SAND_DIV_ROUND_UP(credit_worth, divider);
