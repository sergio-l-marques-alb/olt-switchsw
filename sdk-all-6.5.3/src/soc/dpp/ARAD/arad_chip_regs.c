    ...
    arad_fast_reg_access_init(int unit)
{
    ...
    for (i = 0; i < ARAD_FAST_REG_TYPE_LAST; i++) {
        if (g_fast_reg_info_table[unit][i].reg != INVALIDr) {
            ...
            {
                if((i == ARAD_FAST_REG_IHP_ISA_INTERRUPT_REGISTER) || (ARAD_FAST_REG_IHP_ISA_MANAGEMENT_UNIT_FAILURE)){
                    port = 0;
                }
                if((i == ARAD_FAST_REG_IHP_ISB_INTERRUPT_REGISTER) || (ARAD_FAST_REG_IHP_ISB_MANAGEMENT_UNIT_FAILURE)){
                    port = 1;
                }
            }
...
arad_fast_reg_get(int unit, soc_reg_t reg, int acc_type, int addr, int block, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.fast_reg_get)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.fast_reg_get(unit, reg, acc_type, addr, block, data);
            }
        }
    }
#endif /* CRASH_RECOVERY_SUPPORT */
...
/*     Use crash recovery defined callback for access*/
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.fast_reg_set)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.fast_reg_set(unit, reg, acc_type, addr, block, data);
            }
        }
    }
#endif /* CRASH_RECOVERY_SUPPORT */
  ...
  )
{
  ...
  if (SOC_IS_DONE_INIT(unit))
  {
      if (BCM_UNIT_DO_HW_READ_WRITE(unit))
      {
          if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.polling)
          {
              return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.polling(
                   unit,
                   time_out,
                   min_polls,
                   reg,
