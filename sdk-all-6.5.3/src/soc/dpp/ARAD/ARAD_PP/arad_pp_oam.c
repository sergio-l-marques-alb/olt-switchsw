...
#define _ARAD_PP_OAM_MPLS_TCAM_SUPPORTED_OPCODE(opcode) \
  ((opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_CCM) || (opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LMM) || \
   ...
   (opcode == SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBR))
     ...
     )
{
    ...
    /* Connect DMA engine #3 to OAMP. Different registration for Arad+ and Jericho */
    if (SOC_IS_JERICHO(unit)) 
    {
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_ECI_FIFO_DMA_SELr(unit, &reg64));
        COMPILER_64_SET(field64,0,DMA_SRC_OAM_EVENT_FIFO);
        soc_reg64_field_set(unit, ECI_FIFO_DMA_SELr, &reg64, FIFO_DMA_3_SELf, field64);
    ...
    res = _soc_mem_sbus_fifo_dma_start_memreg(unit, ARAD_OAM_DMA_CHANNEL_USED_EVENT,
    ...
    soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH3_RD_DMA_CFGr, &reg, ENDIANESSf, endian !=0);
    ...
    soc_cmicm_intr0_enable(unit, IRQ_CMCx_FIFO_CH_DMA(ARAD_OAM_DMA_CHANNEL_USED_EVENT));
	 ...
	 )
{
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_ECI_FIFO_DMA_SELr(unit, &reg64));
    COMPILER_64_SET(field64,0,DMA_SRC_OAM_REPORT_FIFO);
    soc_reg64_field_set(unit, ECI_FIFO_DMA_SELr, &reg64, FIFO_DMA_2_SELf, field64);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_ECI_FIFO_DMA_SELr(unit, reg64));
    ...
    res = _soc_mem_sbus_fifo_dma_start_memreg(unit, ARAD_OAM_DMA_CHANNEL_USED_STAT_EVENT,
                                             FALSE /*is_mem*/, 0, OAMP_INTERRUPT_MESSAGEr, MEM_BLOCK_ALL,
                                             0, host_mem_num_entries, report_dma_host_memory[unit]);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 31, exit);

    /* Setting registers not properly set by dma_start_memreg()*/
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_CMIC_CMC0_FIFO_CH2_RD_DMA_HOSTMEM_THRESHOLDr(unit, &reg));
    soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_HOSTMEM_THRESHOLDr, &reg, THRESHOLDf, dma_threshold);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_CMIC_CMC0_FIFO_CH2_RD_DMA_HOSTMEM_THRESHOLDr(unit, reg)); 

    reg=0;
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, READ_CMIC_CMC0_FIFO_CH2_RD_DMA_CFGr(unit, &reg));
    ...
    soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_CFGr, &reg, TIMEOUT_COUNTf, timeout);
    soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_CFGr, &reg, ENDIANESSf, endian !=0);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 31, exit, WRITE_CMIC_CMC0_FIFO_CH2_RD_DMA_CFGr(unit, reg)); 


    /* Unmask fifo dma channle 2 interrupts*/
    soc_cmicm_intr0_enable(unit, IRQ_CMCx_FIFO_CH_DMA(ARAD_OAM_DMA_CHANNEL_USED_STAT_EVENT));
  ...
  )
{
  ...
  if (is_bfd) {
      ...
      if (mirror_trap_code != SOC_PPC_TRAP_CODE_INTERNAL_OAM_ITMH) {
  ...
  )
{
  ...
  for (jr_mp_type_idx = 0; jr_mp_type_idx<=(SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)&&(!is_bfd)); jr_mp_type_idx++) {
      oam1_key.mp_type_jr = (is_bfd ? SOC_PPC_OAM_MP_TYPE_JERICHO_BFD : jr_mp_types[jr_mp_type_idx]);
            ...
            for (mp_type = 0; mp_type < SOC_PPC_OAM_MP_TYPE_COUNT_PLUS; mp_type++) {
                ...
                case SOC_PPC_OAM_MP_TYPE_MIP_OR_PASSIVE_MATCH_PLUS:
                    ...
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)
                        ...
                        if ( (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) && is_passive ==0) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
                            if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
                                oam1_key.mp_profile = ARAD_PP_OAM_PROFILE_PASSIVE;
                            } else {
  ...
  )
{
  ...
  if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type) ||
      ...
      if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)){
         ...
         if(!(classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY)){
            res = soc_jer_pp_oam_oem1_mep_add(unit,classifier_mep_entry, &oem1_prev_payload,&oem1_payload,1,update == 1);
            SOC_SAND_SOC_CHECK_FUNC_RESULT_ERR_VAL(res, 222, exit, res);
         }
      } else {
          ...
          if (update == 1) {
              ...
              if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
                  if (found && (SOC_DPP_CONFIG(unit)->pp.oam_hierarchical_lm)) {
                      ...
                      uint32 new_counter_temp = oem1_payload.counter_ndx; /* Save the newly desired counter index */
                      /* Update the MP-Map */
                      res = soc_jer_pp_oam_oem1_mep_add(unit, classifier_mep_entry, &oem1_prev_payload, &oem1_payload, 0 /* passive */, update == 1);
                      SOC_SAND_SOC_CHECK_FUNC_RESULT_ERR_VAL(res, 222, exit, res);
                      /* Use the original counter index */
                      oem1_payload.counter_ndx = oem1_prev_payload.counter_ndx;
                      /* Set in HW   */
                                                                ...
                                                                uint8                    update )
{
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_above_64);
    /*enter RMEP DB entry*/
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit, READ_OAMP_RMEP_DBm(unit, MEM_BLOCK_ANY, rmep_index, reg_above_64));
...
exit:
 ...
 uint32 
     arad_pp_oam_oamp_read_pph_heaser(
     SOC_SAND_IN  int  unit,
     ...
     )
{
    ...
    SOC_SAND_INIT_ERROR_DEFINITIONS(0);
    /* fill the PPH header*/
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, SOC_IS_QAX(unit) ? READ_OAMP_REG_0130r(unit, &reg64): READ_OAMP_TX_PPHr(unit, &reg64));
    ...
    pph_header->field4 = 0;
    pph_header->field5 = 0;
    pph_header->field6 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, field6);
    pph_header->field7 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, field7);
    pph_header->field8 = 0;
    pph_header->field9 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, field9);
    pph_header->field10 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, field10); 
  ...
  )
{
    ...
    /* fill the PPH header*/
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, arad_pp_oam_oamp_read_pph_heaser(unit, 1,_ARAD_PP_OAM_FWD_CODE_CPU,0,&pph_header));

/*
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_OAMP_TX_PPHr(unit, &reg64));

    pph_header.field_1 = 1;  
    pph_header.field_2 =_ARAD_PP_OAM_FWD_CODE_CPU;
    pph_header.field_3 = 0;
    pph_header.field_4 = 0;
    pph_header.field_5 = 0;
    pph_header.field_6 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, TX_PPH_IN_LIF_ORIENTATIONf);
    pph_header.field_7 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, TX_PPH_UNKNOWN_DAf);
    pph_header.field_8 = 0;
    pph_header.field_9 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, TX_PPH_VSI_OR_VRFf);
    pph_header.field_10 = soc_reg64_field32_get(unit, OAMP_TX_PPHr, reg64, TX_PPH_IN_LIF_OR_IN_RIFf); 
  ...
  )
{
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_SAND_SOC_IF_ERROR_RETURN(res,12,exit,_ARAD_PP_OAM_INTERNAL_READ_OAMP_MEP_DB_LM_DBm(unit,MEM_BLOCK_ANY, lm_info->entry_id +1,reg_data));
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_data);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 12, exit, _ARAD_PP_OAM_INTERNAL_READ_OAMP_MEP_DB_LM_STATm(unit, MEM_BLOCK_ANY, lm_info->entry_id + 2, reg_data));
       ...
       )
{
    ...
    if (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, arad_pp_oam_oamp_read_pph_heaser(unit, 0,_ARAD_PP_OAM_FWD_CODE_MPLS,udh_size,&pph));

/*
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_OAMP_TX_PPHr(unit, &reg64));
        pph.field_1 = 0;
        pph.field_2 = _ARAD_PP_OAM_FWD_CODE_MPLS;  
        pph.field_3 = udh_size;
        pph.field_4 = 0;
        pph.field_5 = 0;
        pph.field_6 = soc_reg64_field32_get(unit, OAMP_TX_PPHr,reg64, TX_PPH_IN_LIF_ORIENTATIONf);
        pph.field_7 = soc_reg64_field32_get(unit, OAMP_TX_PPHr,reg64, TX_PPH_UNKNOWN_DAf);
        pph.field_8 = 0;
        pph.field_9 = soc_reg64_field32_get(unit, OAMP_TX_PPHr,reg64, TX_PPH_VSI_OR_VRFf);
        pph.field_10 = soc_reg64_field32_get(unit, OAMP_TX_PPHr,reg64, TX_PPH_IN_LIF_OR_IN_RIFf);
 ...
 * @return soc_error_t 
 */
soc_error_t arad_pp_oam_interrupt_init_internal(int unit, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data) {
    int dma_threshold;
    uint32 field_32;
    uint32 use_dma = 0;
    ...
    if (use_dma) {        
        ...
        res = READ_CMIC_CMC0_FIFO_CH3_RD_DMA_NUM_OF_ENTRIES_VALID_IN_HOSTMEMr(unit, &field_32) ;
        SOCDNX_IF_ERR_EXIT(res);
        ...
        if (num_entries > 0) {
            ...
            res = _soc_mem_sbus_fifo_dma_set_entries_read(unit, ARAD_OAM_DMA_CHANNEL_USED_EVENT, num_entries);
            SOCDNX_IF_ERR_EXIT(res);
        } 
        ...
        res =  WRITE_CMIC_CMC0_FIFO_CH3_RD_DMA_STAT_CLRr(unit, field_32);
        SOCDNX_IF_ERR_EXIT(res);
 ...
 * OAM 
 */
soc_error_t arad_pp_report_oam_interrupt_init_internal(int unit, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data) {
    int dma_threshold;
    uint32 field_32;
    int  num_entries=0,res;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if (SOC_DPP_CONFIG(unit)->pp.oam_use_report_fifo_dma) {
        ...
        /* Now copy to host memory to internal buffer*/
        res = READ_CMIC_CMC0_FIFO_CH2_RD_DMA_NUM_OF_ENTRIES_VALID_IN_HOSTMEMr(unit, &field_32) ;
        SOCDNX_IF_ERR_EXIT(res);
        num_entries = soc_reg_field_get(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_NUM_OF_ENTRIES_READ_FRM_HOSTMEMr, field_32, ENTRYCOUNTf);
        ...
        if (num_entries > 0) {
            ...
            res = _soc_mem_sbus_fifo_dma_set_entries_read(unit, ARAD_OAM_DMA_CHANNEL_USED_STAT_EVENT, num_entries);
            SOCDNX_IF_ERR_EXIT(res);
        } 

        /* Clear the interrupt*/
        soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_STAT_CLRr, &field_32, HOSTMEM_TIMEOUTf, 1);
        soc_reg_field_set(unit, CMIC_CMC0_FIFO_CH2_RD_DMA_STAT_CLRr, &field_32, HOSTMEM_OVERFLOWf, 1);
        res =  WRITE_CMIC_CMC0_FIFO_CH2_RD_DMA_STAT_CLRr(unit, field_32);
        SOCDNX_IF_ERR_EXIT(res);
       ...
       )
{
    ...
    {
        res = arad_pp_report_oam_interrupt_init_internal(unit, &interrupt_data); 
        SOCDNX_IF_ERR_EXIT(res);
    }
    else /* SOC_PPC_OAM_DMA_EVENT_TYPE_EVENT */
    {
        res = arad_pp_oam_interrupt_init_internal(unit, &interrupt_data); 
        SOCDNX_IF_ERR_EXIT(res);
    }
    ...
    *found = 0;
    ...
    if (*found) { /* Entry found in OEM-2, construct OAM-2 key and print */
        ...
        res = sw_state_access[unit].dpp.bcm.oam.y1731_opcode_to_intenal_opcode_map.get(unit,
                                        key_params->opcode, &act_key.opcode);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
