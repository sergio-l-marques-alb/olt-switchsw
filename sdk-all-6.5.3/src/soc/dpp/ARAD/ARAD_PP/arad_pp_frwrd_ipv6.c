    ...
    )
{
    ...
    if(SOC_IS_ARADPLUS_AND_BELOW(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0))){
        use_tcam = 1;
    }
    ...
    )
{
    ...
    * {VRF(14:0) in 2287:272, IP-MC-address(119:0) 271:144, Source-IP(119:0) in 143:16 IN-RIF(11:0) in 15:0} 
    ...
    SHR_BITCOPY_RANGE(data->param[2].value, 0 /* LSB */, route_key->group.address, 0 /* LSB */, data->param[2].nof_bits);

    ARAD_PP_FRWRD_IP_TCAM_LEM_KEY_ENCODE(unit, frwrd_table_id, 2, 0 /* Copy the bits after */, mask);
    SHR_BITCOPY_RANGE(mask->param[2].value, 0 /* LSB */, route_key_mask.address, 0 /* LSB */, mask->param[2].nof_bits);
    ...
    )
{
    ...
    * {VRF(14:0) in 2287:272, IP-MC-address(119:0) 271:144, Source-IP(119:0) in 143:16 IN-RIF(11:0) in 15:0} 
...

    {
        ...
        /* Param 2  - IPv6 address */
        ARAD_PP_FRWRD_IP_TCAM_LEM_KEY_DECODE(unit, frwrd_table_id, 2, route_key->group.address, data, mask);
...
#endif
  {
      res = arad_pp_frwrd_ipv6_uc_or_vpn_dbal_route_remove(unit,0, /* vrf ndx */&(route_key->subnet));
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit); 
      *success = SOC_SAND_SUCCESS;
  }
