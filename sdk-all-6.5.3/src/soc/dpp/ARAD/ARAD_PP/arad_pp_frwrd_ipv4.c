...
{
    uint32 payload = 0, res, prefix_len, priority;
    SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_to_tcam;
    ...
    is_to_tcam = (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0)) || (SOC_IS_ARADPLUS_AND_BELOW(unit));
    ...
    )
{
    uint32 res;
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    int is_to_tcam = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0) || SOC_IS_ARADPLUS_AND_BELOW(unit);
#endif
    SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    SOC_SAND_SUCCESS_FAILURE             success;
  ...
  )
{
  ...
  for(indx = 0; indx < nof_entries_lcl; ++indx)
  {
      ...
      if(routes_status){
		  ...
		  if (read_vals[indx].flags & ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED) {
	...
	if (ARAD_KBP_ENABLE_ANY_IPV4UC_PROGRAM ) {
        ...
        if (ARAD_KBP_ENABLE_IPV4_DC ) {            
            ...
            if (is_scale) {
                ...
                /* TBD-DC add support for get block*/
                LOG_CLI((BSL_META("get block to Double capacity implementation missing\n")));
                frwrd_table_id = ARAD_KBP_FRWRD_TBL_ID_IPV4_DC;
                goto exit;
            }
        }
