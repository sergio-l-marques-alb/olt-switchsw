...
arad_pp_flp_dbal_kaps_table_prefix_get(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 *table_prefix, uint32 *table_prefix_len)
{
    ...
    } else if (table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_SHORT_KAPS) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_flp_dbal_kaps_table_prefix_get - kaps table_id is disabled")));
    }
    ...
    arad_pp_flp_dbal_ipv4mc_bridge_tcam_table_create(int unit)
{
    ...
    if (!is_table_initiated) {
        ...
        DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);
        /* MSB */
        qual_info[0].qual_type = SOC_PPC_FP_QUAL_HDR_IPV4_SIP;
        qual_info[1].qual_type = SOC_PPC_FP_QUAL_HDR_IPV4_DIP;        
        qual_info[2].qual_type = SOC_PPC_FP_QUAL_FID;
    ...
    if (!is_table_initiated) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0)) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV4MC_KAPS, table_prefix, table_prefix_len,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM, 5, SOC_DPP_DBAL_ATI_NONE, qual_info, "FLP IPv4 MC TCAM"));
        }else{
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
    ...
    arad_pp_flp_dbal_vpws_tagged_program_tables_init(int unit)
{
    ...
    for (is_single_tagged=0; is_single_tagged<=1; is_single_tagged++) {
        ...
        if (!is_table_initiated) {
            DBAL_QUAL_INFO_CLEAR(&qual_info, SOC_PPC_FP_NOF_QUALS_PER_DB_MAX);        
            qual_info[0].qual_type = SOC_PPC_FP_QUAL_IRPP_IN_LIF;
            qual_info[1].qual_type = SOC_PPC_FP_QUAL_HDR_VLAN_TAG_ID;
            if (!is_single_tagged) {
                /* Double Tagged */
                qual_info[2].qual_type = SOC_PPC_FP_QUAL_HDR_INNER_2ND_VLAN_TAG_ID;
            }

            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id,
                                                         DBAL_PREFIX_NOT_DEFINED, 0,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, 2, SOC_DPP_DBAL_ATI_NONE, qual_info, 
   ...
   )
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
        /* Search in the special KAPS tables */
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, PROG_FLP_IPV4COMPMC_WITH_RPF, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP,keys_to_table_id, NULL, 4));
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, PROG_FLP_IPV4UC_PUBLIC, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP,
                                                                 keys_to_table_id, NULL, 3));
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, PROG_FLP_IPV6UC, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, keys_to_table_id, NULL, 1));
   ...
   )
{
    ...
    {        
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, PROG_FLP_IPV6MC, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, keys_to_table_id, NULL, 2));
    }
#else
    {
