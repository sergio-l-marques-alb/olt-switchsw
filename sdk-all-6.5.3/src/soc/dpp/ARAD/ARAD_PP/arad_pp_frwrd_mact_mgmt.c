    ...
    )
{
  ...
  ARAD_PP_LEM_ACCESS_PAYLOAD     
      lem_payload;
  SOC_PPC_FRWRD_MACT_EVENT_BUFFER mact_event_buffer;
  ...
  {    
    ...
    soc_sand_U8_to_U32(buff, SOC_PPC_FRWRD_MACT_LEARN_MSG_EVENT_LEN, (uint32*)&(mact_event_buffer.buff));    
    mact_event_buffer.buff_len = SOC_PPC_FRWRD_MACT_EVENT_BUFF_MAX_SIZE;

    res = soc_ppd_frwrd_mact_event_parse(unit,(SOC_PPC_FRWRD_MACT_EVENT_BUFFER*)&mact_event_buffer,c_event);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
...
soc_error_t arad_pp_frwrd_mact_learning_dma_unset(SOC_SAND_IN int unit)
{
    ...
    soc_cmicm_intr0_disable(unit, IRQ_CMCx_FIFO_CH_DMA(ARAD_LEARNING_DMA_CHANNEL_USED));
