...
#define ARAD_PP_FLP_PROG_SEL_CAM_P2P_BASE                  1 /* 1 through 4 */
#define ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_SINGLE        5
#define ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_DOUBLE        6
...
#define ARAD_PP_FLP_PROG_SEL_CAM_IPV4_DC                   26 /* IPv4 Double capacity in external tcam */
   ...
   )
{
    ...
    for (is_single_tag = 1; is_single_tag!=(uint8)(-1); is_single_tag--) {
        ...
        ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, prog_id, prog_id);

        res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, 
                                                                   is_single_tag ? ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_SINGLE : ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_DOUBLE, 
                                                                   &prog_selection_cam_tbl);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
        ...
        if (is_single_tag) {
            ...
            prog_selection_cam_tbl.forwarding_header_qualifier = 0; /*Outer Tag (2 LSBs) is dont care, Inner tag (2 MSBs) is None, Outer tag is priority is masked */
            prog_selection_cam_tbl.forwarding_header_qualifier_mask = 0x1C;
        }

        prog_selection_cam_tbl.program = prog_id;

        res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, 
                                                                   is_single_tag ? ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_SINGLE : ARAD_PP_FLP_PROG_SEL_CAM_VPWS_TAGGED_DOUBLE, 
                                                                   &prog_selection_cam_tbl);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit); 
    }
...
/* when ARAD_KBP_ENABLE_P2P_EXTENDED is enabled, we will use only service type 000 and 001 (AC to PWE & AC to AC) */
    ...
    for (i = 0; i < num_of_selections; i++) {
        ...
        if ((service_type[i] == ARAD_PP_FLP_SERVICE_TYPE_PWE_P2P) && soc_property_get(unit, spn_VPWS_TAGGED_MODE, 0)) { 
            ...
            prog_selection_cam_tbl.forwarding_header_qualifier = 0; /* Outer Tag (2 LSBs) and Inner tag (2 MSBs) are None, Outer tag is priority is masked */
            prog_selection_cam_tbl.forwarding_header_qualifier_mask = 4;
        }
   ...
   )
{
  ...
  /* For static, program id and app id is same */
  ARAD_PP_FLP_SW_DB_FLP_PROGRAM_ID_SET(unit, PROG_FLP_IPV4_DC, PROG_FLP_IPV4_DC);

  res = arad_pp_ihb_flp_program_selection_cam_tbl_get_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4_DC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4_DC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
exit:
   ...
   )
{
  ...
  prog_selection_cam_tbl.program                        = PROG_FLP_ETHERNET_ING_LEARN;
  prog_selection_cam_tbl.valid                          = 1;
  res = arad_pp_ihb_flp_program_selection_cam_tbl_set_unsafe(unit, ARAD_PP_FLP_PROG_SEL_CAM_IPV4MC_BRIDGEV4MC, &prog_selection_cam_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
exit:
   ...
   )
{
  ...
  if (!(SOC_DPP_CONFIG(unit)->pp.pon_application_enable)) {
      ...
      if (soc_property_get(unit, spn_VPWS_TAGGED_MODE, 0)) {
          arad_pp_flp_prog_sel_cam_vpws_tagged(unit);
          arad_pp_flp_dbal_vpws_tagged_program_tables_init(unit);
          arad_pp_flp_process_vpws_tagged(unit);
      } 		  
  }
  ...
  {      
      ...
      if (SOC_IS_JERICHO(unit)) {
          if(ARAD_KBP_ENABLE_IPV4_DC){      
              arad_pp_flp_prog_sel_cam_ipv4_dc(unit);
              arad_pp_flp_dbal_ipv4_dc_program_tables_init(unit);
              arad_pp_flp_process_ipv4_dc(unit);
          }
      }
#endif
	  ...
	  if(SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode != BCM_IPMC_SSM_DISABLE) /*SSM is enabled is KAPS or TCAM */{
      ...
      if (SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable) {
...
#endif 
          {
              ...
              arad_pp_flp_instruction_rsrc_set(unit, PROG_FLP_IPV4COMPMC_WITH_RPF);
              arad_pp_flp_dbal_ipv4mc_tcam_tables_init(unit);
          }      
      ...
      arad_pp_flp_process_coupling_lsr(unit);
        ...
        if (SOC_DPP_CONFIG(unit)->trill.mode)
        {
            ...
            arad_pp_flp_key_const_TRILL_mc(unit);        
            arad_pp_flp_lookups_TRILL_mc(unit,ingress_learn_enable,
                                         ARAD_TCAM_ACCESS_PROFILE_INVALID);
            if (SOC_DPP_CONFIG(unit)->trill.transparent_service){
                arad_pp_flp_dbal_trill_program_tcam_tables_init(unit);
            }
  ...
  if (!(SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable)) {
      ...
      /* this function update the SW state about CE usage*/
      arad_pp_flp_instruction_rsrc_set(unit, program_id);
      arad_pp_flp_dbal_ipv4mc_tcam_tables_init(unit);
