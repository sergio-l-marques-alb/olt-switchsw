    ...
    *success = TRUE;
    ...
    while(tmp_size > 0) 
    {
        ...
        else /* Only estimate the size according to the best case scenario */
        {
            ...
            if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ce_nibble_mis_align_ignore", 0)) /*if enabled - backward compatible mode (user responsibility only)*/
                qual_ce_info->size_cons[qual_ce_info->nof_ce] = arad_pmf_low_level_ce_if_must_use_16ce(unit, qual_type, stage)?ARAD_PP_FP_KEY_CE_SIZE_16 : ARAD_PP_FP_KEY_CE_SIZE_ANY;
            else
            ...
            if (SOC_IS_ARADPLUS(unit))
            {
                if (is_equal) 
                {
                    ...
                    if (SOC_IS_JERICHO_PLUS(unit))
                    {
                        ...
                        soc_reg_above_64_field_set(unit, IHB_XOR_MASKSr, reg_above_64, XOR_MASK_0f, field_above_64);
      ...
      for(alloc_exact = 1; alloc_exact >= 0; --alloc_exact) {
          for (cycle = cycle_first; cycle <= cycle_last; ++cycle) {
              ...
              for(ce_index = 0; ce_index  < nof_ce_const; ++ce_index) {
                  ...
                  if(!soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ce_nibble_mis_align_ignore", 0)) /*if enabled - backward compatible mode (user responsibility only)*/
                  {
                      /* signal qualifiers has nibble alignment. 16bit ce have nibble alignment, but 32bit ce have byte alignment.
                       * we must block the allocation in case 32bit ce is allocated to handle nibble-but-not-byte-aligned qualifier */
                      size_cons =  arad_pmf_low_level_ce_if_must_use_16ce(unit, stage, ce_cons[ce_index].qual_type) ? ARAD_PP_FP_KEY_CE_SIZE_16 : size_cons;
                  }
                  LOG_DEBUG(BSL_LS_SOC_FP,
                            (BSL_META_U(unit,
                                        "   "
                  ...
                  if(!success  && 
                      ...
                      LOG_DEBUG(BSL_LS_SOC_FP,
                                (BSL_META_U(unit,
                                            "   "
                                            "Attempt #2: PMF-Program: %d, cycle: %d, "
                                            "key:%d "
                                            "try to allocate qual %15s for db:%d size_cons %d\n\r"), prog_ndx, cycle, key_id[0], SOC_PPC_FP_QUAL_TYPE_to_string(ce_cons[ce_index].qual_type),db_id, size_cons));
