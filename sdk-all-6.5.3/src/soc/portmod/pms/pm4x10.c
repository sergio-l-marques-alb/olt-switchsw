 ...
 */
STATIC
int _pm4x10_speed_to_interface(
                     int unit, int port,
                     const portmod_port_interface_config_t *interface_config,
                        ...
                        int phy_init_flags)
{
    ...
    _SOC_IF_ERR_EXIT
         (_pm4x10_speed_to_interface(unit, port, config, &config_temp));
    ...
    /* Check whether the interface type applied to the most-external PHY is
     * legit or not. Do not check if the function call is from the external
     * PHY to program the internal SerDes.
     */
    if (phy_init_flags != PORTMOD_INIT_F_INTERNAL_SERDES_ONLY){
        _SOC_IF_ERR_EXIT(portmod_port_interface_type_verify(config->speed,
                                                      config->port_num_lanes,
                                                      is_higig,
                                                      config_temp.interface));
    }
    ...
    if (!(config->flags & (PHYMOD_INTF_F_SET_SPD_TRIGGER | PHYMOD_INTF_F_UPDATE_SPEED_LINKUP))) {
        ...
        if (PORTMOD_PORT_IS_DEFAULT_TX_PARAMS_UPDATED(PM_4x10_INFO(pm_info)->port_config[port_index].port_dynamic_state)) {
            phymod_tx_t_init(&updated_tx_params);
