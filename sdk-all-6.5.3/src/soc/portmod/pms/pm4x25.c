...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm4x25", NULL, VERSION(4), 1, SOC_WB_ENGINE_PRE_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
                    ...
                    for(i=0 ; i<nof_ext_phys ; i++) {
                        ...
                        if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                            ...
                            if ((PORTMOD_PORT_ADD_F_INIT_PASS1_GET(add_info) || init_all) &&
                                (xphy_core_info.is_initialized & PHYMOD_CORE_INIT_F_EXECUTE_PASS1))
                                continue;

                            if ((PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)|| init_all) &&
                                (xphy_core_info.is_initialized & PHYMOD_CORE_INIT_F_EXECUTE_PASS2))
                                continue;
                            ...
                            if (init_all) {
                                xphy_core_info.is_initialized = (PHYMOD_CORE_INIT_F_EXECUTE_PASS2 | PHYMOD_CORE_INIT_F_EXECUTE_PASS1);
...
int _pm4x25_pm_enable(int unit, int port, pm_info_t pm_info, int port_index, const portmod_port_add_info_t* add_info, int enable)
{
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        if(enable) {
            ...
            /* Get Serdes OOR */
            if(ref_out == TRUE) {  /* this PM is master PM, can't reset*/
 ...
 */
STATIC
int _pm4x25_speed_to_interface(int unit, int port,
                               ...
                               phymod_phy_inf_config_t *phy_interface_config)
{
    ...
    switch (interface_config->speed) {
        case 1000:
			...
			PHYMOD_INTF_MODES_FIBER_GET(interface_config)? 
				phymodInterface1000X: phymodInterfaceSGMII;
            break;
        case 25000:
            if (phy_interface_config->interface_type == phymodInterfaceSR){
                 PHYMOD_INTF_MODES_FIBER_SET(phy_interface_config);
            } else if (phy_interface_config->interface_type == phymodInterfaceCR){
            ...
            break;
        case 10000:
            /* here it is assumed that pm4x25.c is bundled with TSCF, which only supports
             * 1-lane 10G and interface types KR/CR/LR/XFI/SFI/SFPDAC.
             */
            if (interface_config->port_num_lanes == 1){
                ...
                {
                    if (phy_interface_config->interface_type == phymodInterfaceSFI) {
                        PHYMOD_INTF_MODES_FIBER_SET(phy_interface_config);
                    } else if (phy_interface_config->interface_type == phymodInterfaceCR){
                        PHYMOD_INTF_MODES_FIBER_CLR(phy_interface_config);
                        PHYMOD_INTF_MODES_COPPER_SET(phy_interface_config);
                    } else if (phy_interface_config->interface_type == phymodInterfaceXFI) {
                        PHYMOD_INTF_MODES_FIBER_CLR(phy_interface_config);
                    } else {
                        PHYMOD_INTF_MODES_FIBER_CLR(phy_interface_config);
                        PHYMOD_INTF_MODES_BACKPLANE_SET(phy_interface_config);
                    ...
                    phy_interface_config->interface_type =
                    PHYMOD_INTF_MODES_FIBER_GET(interface_config)?
                        phymodInterfaceSFI : phymodInterfaceXFI;
                    break;
                }
            }
        case 40000:
            switch (interface_config->port_num_lanes) {
                case 4:
                    if (is_higig){
                                     ...
                                     int all_phy)
{
    ...
    int port_index=0;
    uint32 bitmap;
    portmod_port_interface_config_t *interface_config = NULL;
    soc_port_if_t interface = SOC_PORT_IF_NULL;
    ...
    /* Check whether the interface type applied to the most-external PHY is legit or not.
     * Do not check if the function call is from the external PHY to program the internal
     * SerDes.
     */
    if (all_phy != PORTMOD_INIT_F_INTERNAL_SERDES_ONLY){
    ...
    if(!(PM_4x25_INFO(pm_info)->in_pm12x10)){
       ...
       _SOC_IF_ERR_EXIT
            (_pm4x25_speed_to_interface(unit, port, config, &phy_config));
                         ...
                         const phymod_operation_mode_t phy_op_mode)
{
    ...
    *port_init_config = *f_init_config;
    ...
    if(an.an_mode != phymod_AN_MODE_NONE) {
        ...
        /* update an information before proceeding further */
        SOC_IF_ERROR_RETURN(_pm4x25_an_mode_update (unit, port, pm_info, port_index, interface_config->port_num_lanes));
    }
