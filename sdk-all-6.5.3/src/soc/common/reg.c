...
soc_reg64_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            if(SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg64_get)
            {
                return SOC_CONTROL(unit)->Hw_Log_List.Access_cb.reg64_get(unit, reg, port, index, data);
            }
        }
    }
#endif /* CRASH_RECOVERY_SUPPORT */
...
soc_reg_rawport_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    if((SOC_INFO(unit).reg_access.reg64_get) || /* User defined */
       ...
       (SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit) &&
        SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_get)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Use soc_reg_get \n")));
        return SOC_E_FAIL;
...
/* Don't handle Special Accesses */
    if((SOC_INFO(unit).reg_access.reg64_set) || /* User defined */
       (SOC_REG_IS_ABOVE_64(unit, reg)) ||
       (SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit) &&
        SOC_CONTROL(unit)->Hw_Log_List.Access_cb.soc_reg_set)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Use soc_reg_set \n")));
        return SOC_E_FAIL;
    ...
    *blk = 0;/* not really needed, just to avoid coverity defect */
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype))) {
        ...
        case SOC_BLK_KAPS_BBS:
            block = KAPS_BBS_BLOCK(unit, port);
            break;
        case SOC_BLK_ILB:
