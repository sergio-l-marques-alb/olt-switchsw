              ...
              int copyno, int index, void *entry_data)
{
    ...
    if ((flags & SOC_MEM_DONT_MAP_INDEX) &&
        (mem == L3_DEFIPm ||
         ...
         mem == L3_DEFIP_HIT_ONLY_Ym ||
         mem == L3_DEFIP_HIT_ONLYm)) {
         entry_num_max = soc_mem_index_max(unit, L3_DEFIPm) +
                                   ...
                                   int copyno, int index_min, int index_max, void *buffer, int vchan)
{
    ...
    assert(buffer != NULL);
#ifdef BROADCOM_DEBUG
    if (soc_property_get(unit, "dma_mem_debug_enable", 0)) {
...
{
    static int ic = 0;
    static _soc_hash_bank_info_t bi[SOC_MAX_NUM_DEVICES];
    static SHR_BITDCL *bkt_trace[SOC_MAX_NUM_DEVICES] = {NULL};
    static SHR_BITDCL *bkt_trace2[SOC_MAX_NUM_DEVICES] = {NULL};
    static soc_mem_t l3_mem = L3_ENTRY_IPV6_MULTICASTm;
    ...
    if (bank_num_start == SOC_MEM_HASH_BANK_ALL) {
        ...
        SOC_IF_ERROR_RETURN(_soc_mem_shared_hash_init
            (unit, mem, &bkt_trace[unit], &bi[unit], 0, NULL, NULL));
    }

    bkt_trace2[unit] = bkt_trace[unit];
    if (SOC_MULTI_HASH_MOVE_ALGORITHM(unit) == MULTI_MOVE_MODE_DEPTH_FAST) {
        bkt_trace2[unit] = bkt_trace[unit] + _SHR_BITDCLSIZE(bi[unit].numb);
    }

    for (bix = 0; bix < bi[unit].num_banks; bix++) {
        cbix = bix % bi[unit].num_banks;
        bank_num = bi[unit].bank_nums[cbix];
        ...
        if (SHR_BITGET(bkt_trace[unit], bucket)) {
            continue;
        }
        SHR_BITSET(bkt_trace[unit], bucket);
        SHR_BITSET(bkt_trace2[unit], bucket);
        buckets[mark++] = bucket;
        ...
        for (ent = 0; ent < NUM_ENT; ent += ent_incr) {
            ...
            rv = soc_mem_generic_insert(unit, orig_mem, copyno,
                                        bi[unit].banks & ~(1 << bank_num),
                                        orig_entry[ent], NULL, &move_bkt);

            moved = 0;
            if (SOC_SUCCESS(rv)) {
                move_bkt /= _soc_mem_shared_hash_entries_per_bkt(orig_mem);
                if (SHR_BITGET(bkt_trace2[unit], move_bkt)) {
                    rv = soc_mem_generic_delete(unit, orig_mem, MEM_BLOCK_ANY,
                                                bi[unit].banks & ~(1 << bank_num),
...
cleanup:
    for (ent_incr = 0; ent_incr < mark; ent_incr++) {
        SHR_BITCLR(bkt_trace2[unit], buckets[ent_incr]);
    }
    if (bank_num_start == SOC_MEM_HASH_BANK_ALL) {
        sal_free(bkt_trace[unit]);
    }
    return rv;
}
