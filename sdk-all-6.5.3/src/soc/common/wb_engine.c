...
soc_wb_engine_restore(int unit, int engine_id, uint32 buffer_id, uint16 recovered_ver)
{
    ...
    if (buffer->version > recovered_ver) {
        ...
        if (buffer->upgrade_func != NULL) {
            (*(buffer->upgrade_func))(unit, recovered_ver, buffer->version);
        }
    }
        ...
        if (version != recovered_ver && is_only_copy_workaround) {
            ...
            for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
                ...
                if (tmp_scache_first_offset == -1) {
                    tmp_scache_first_offset = variables[var_ndx].offset;
                }
                variables[var_ndx].data_orig = tmp_scache_copy + variables[var_ndx].offset;
            }
        ...
        if (version != recovered_ver) {
            ...
            if (is_only_copy_workaround) {
                ...
                for (var_ndx = 0; var_ndx < soc_wb_engine_nof_vars[unit][engine_id]; var_ndx++) {
                    ...
                    variables[var_ndx].data_orig = NULL;
                }
            }
