...
int ipv4_tunnel_build_tunnels_erspan(int unit, int out_port, bcm_if_t* intf_id, bcm_gport_t *tunnel_gport, bcm_mac_t next_hop_mac){
    ...
    rv = create_l3_egress(unit,flags,out_port,0,tunnel_itf1,next_hop_mac, &fec[0], &encap_id[0]); 
    if (rv != BCM_E_NONE) {
...
int mirror_with_erspan_example_iproute(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int span_id, 
    ...
    /* add inbound mirror to send out of the create ip tunnel with  span encapsulation */
    rv = mirror_into_tunnel_iproute(unit, in_vlan, in_port, out_port, mirror_port, intf_id, span_id, ll_vlan_id, ll_tpid, ll_pcp, next_hop_mac);
    if (rv != BCM_E_NONE) {
        printf("Error, ipv4_tunnel_add_routes, in_port=%d, \n", in_port);
    }
...
void erspan_set_eei_invalid(int unit, bcm_gport_t mirror_id)
{
    ...
    if (BCM_GPORT_IS_MIRROR_MIRROR(mirror_id)) {
        ...
        /**/
        sprintf(cmd, "mod IPT_ARAD_SNP_MIR_STAMP_EN %d 1 PPH_EEI_EXT_EN=0xffffff", offset);
        bshell(unit, cmd);

        /**/
        sprintf(cmd, "mod IPT_ARAD_SNP_MIR_STAMP_VALUES %d 1 PPH_EEI_EXT=0x11", offset);
        bshell(unit,cmd);

        /* ERSPAN mirroring for routed packet in inbound mode,
           indicate that mirrored packet should be bridged and not routed.
           Instead of Forwarding-offset != 0 (some termination), we'll reset to 0 (no changes to original packet) */
        sprintf(cmd, "mod IPT_ARAD_SNP_MIR_STAMP_EN %d 1 PPH_BASE_EN=0xFE0000000000", offset); 
        bshell(unit,cmd);   
...
int mirror_into_tunnel(int unit, int in_vlan, int in_port, int dest_port, int mirror_port, bcm_if_t ip_tunnel, int erspan_value, 
    ...
    /* set eei to invalid*/
    erspan_set_eei_invalid(unit, dest.mirror_dest_id);
