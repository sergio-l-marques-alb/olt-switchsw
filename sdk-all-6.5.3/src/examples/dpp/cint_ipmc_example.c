...
int run_with_ports_ipv4_mc_example(int unit, int iport, int oport){
    ...
    /* Create the IP MC Group */
    bcm_multicast_destroy(unit,ipmc_index);
    bcm_multicast_t mc_id = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_index, dest_local_port_id, nof_dest_ports, vlan, TRUE);
...
int ipv4_mc_best_match_example(int unit, int in_port, int out_port){
    ...
    for(group_index = 0; group_index < nof_groups; ++group_index) {
        ...
        ipmc_indexes[group_index] = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_indexes[group_index], dest_local_port_id, nof_dest_ports_per_group, out_vlan, FALSE);
...
int ipv6_mc_example(int unit){
	...
	/* Create the IPMC group, same as we did for IPv6	 */
	bcm_multicast_t mc_id = create_ip_mc_group(unit, BCM_MULTICAST_EGRESS_GROUP, ipmc_index, dest_local_port_id, vlan, FALSE);
