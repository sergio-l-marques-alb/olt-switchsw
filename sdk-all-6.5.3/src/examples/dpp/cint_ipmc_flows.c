 ...
 */
int create_forwarding_entry_dip_sip_inner(int unit, bcm_ip_t mc_ip, bcm_ip_t src_ip, int ipmc_index, int vlan, int mc_bridge){
    int rv = BCM_E_NONE;
...
int create_forwarding_entry_dip_sip(int unit, bcm_ip_t mc_ip, bcm_ip_t src_ip, int ipmc_index, int vlan){
    return create_forwarding_entry_dip_sip_inner(unit,mc_ip,src_ip,ipmc_index,vlan,0);
}
...
int ipmc_flows_ipmc_disabled_l3_mcast_l2_per_rif(int unit, int inP, int outP, int outP2 ,int outP3 ,int vlan1, int vlan2, int vlan3, int vlan4){
     ...
     */
    rv = create_forwarding_entry_dip_sip_inner(unit,ipmc_flows_info1.mc_ip_arr[0],0x0,ipmc_flows_info1.ipmc_mc_groups[0],vlans[0],1);
    if (rv != BCM_E_NONE) {
    ...
    rv = create_forwarding_entry_dip_sip_inner(unit,ipmc_flows_info1.mc_ip_arr[2],ipmc_flows_info1.src_ip_arr[2],ipmc_flows_info1.ipmc_mc_groups[2],vlans[2],1);
    if (rv != BCM_E_NONE) {
        printf("Error, create_forwarding_entry_dip_sip\n");
        return rv;
    }
...
int ipmc_flows_rif_ipmc_disabled(int unit, int inP, int outP, int outP2 ,int outP3 ,int vlan1, int vlan2, int vlan3, int vlan4){
    ...
    */
     ...
     * defining entries for LEM which will forward the packet to the relevant mc group
     */
    rv = create_forwarding_entry_dip_sip_inner(unit, ipmc_flows_info1.mc_ip_arr[0], 0x0, ipmc_flows_info1.ipmc_mc_groups[0], vlan1,1); /* 7.1 */
    if (rv != BCM_E_NONE) {
        printf("Error, create_forwarding_entry_dip\n");
        return rv;
    }
    ...
    if (ipmc_ssm_enable) {
        ...
        rv = create_forwarding_entry_dip_sip_inner(unit, ipmc_flows_info1.mc_ip_arr[0], 0xc0a8000f/*sip*/ ,ipmc_flows_info1.ipmc_mc_groups[1], vlan1,1); /* 7.1.1 */
        if (rv != BCM_E_NONE) {
            printf("Error, create_forwarding_entry_dip\n");
            return rv;
        }
        rv = create_forwarding_entry_dip_sip_inner(unit, ipmc_flows_info1.mc_ip_arr[0], 0xc0a8000e/*sip*/ ,ipmc_flows_info1.ipmc_mc_groups[2], vlan1,1); /* 7.1.2 */
        if (rv != BCM_E_NONE) {
            printf("Error, create_forwarding_entry_dip\n");
            return rv;
        }
