...
int set_counter_source_and_engines(int unit, int *counter_base, int port) {
   ...
   if (current_lm_counter_base[core] % NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE == 0) {
      ...
      counter_config.source.pointer_range.start = (current_lm_counter_base[core] / NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE) * NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE;
      counter_config.source.pointer_range.end = ((current_lm_counter_base[core] / NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE) + 1) * NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE - 1;
  ...
  if (is_up) {
      ...
      /* The MAC address that the CCM packets are sent with*/
        ...
        if (is_itmh_mc) {
            ...
            int unit = 0;
            print bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, &mc_group);
            print bcm_multicast_ingress_add(unit, mc_group, 200, 130);
            print bcm_multicast_ingress_add(unit, mc_group, 201, 140);
...
int hierarchical_lm_counters_setup(int unit,
    ...
    int engine_index, counter_index, i;
    bcm_stat_counter_engine_t counter_engine;
    bcm_stat_counter_config_t counter_config;
    bcm_color_t colors[] = { bcmColorGreen, bcmColorYellow, bcmColorRed, bcmColorBlack };
    uint8 drop_fwd[] = { 0, 1 };
    uint32 NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE = 0x1000;
    int index1, index2;
    ...
    for (engine_index = 0; engine_index < num_engines; engine_index++) {
        ...
        counter_config.source.core_id = BCM_CORE_ALL;
        ...
        engine_arr[engine_index] = counter_engine.engine_id;
        for (i = 1; i <= num_counters_per_engine; i++) {
            counter_base_ids[(engine_index * num_counters_per_engine) + i] = counter_config.source.pointer_range.start + 16 * i;
        }
    }
}
