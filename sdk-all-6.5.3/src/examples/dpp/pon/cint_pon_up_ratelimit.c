...
int pon_up_ratelimit_setup(int unit)
{
    ...
    for (idx = 0; idx < nof_n_1_services; idx++) {
        ...
        rv = bcm_field_entry_policer_attach(unit, entry, 0, pon_n_1_service_info[idx].policer_id);
        if (rv != BCM_E_NONE) {
        ...
        rv = bcm_field_entry_policer_attach(unit, entry, 1, pon_llid_up_ratelimit.policer_id);
        if (rv != BCM_E_NONE) {
            printf("Error in bcm_field_entry_policer_attach with policer_id %d $rv\n", pon_llid_up_ratelimit.policer_id);
            print rv;
            return rv;
