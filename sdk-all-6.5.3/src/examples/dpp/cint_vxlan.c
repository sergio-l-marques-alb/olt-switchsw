 ...
 * in_tunnel: ip tunnel terminator gport
 * out_tunnel: ip tunnel initiator gport 
...
int vxlan_add_port(int unit, uint32 vpn, bcm_gport_t in_port, bcm_gport_t in_tunnel, bcm_gport_t out_tunnel, bcm_if_t egress_if, uint32 flags, bcm_gport_t *vxlan_port_id){
    ...
    if (is_device_or_above(unit, JERICHO)) {
        ...
        vxlan_port.network_group_id = g_vxlan.vxlan_network_group_id; 
        ...
        if (out_tunnel == 0){ 
            ...
            if (!g_vxlan.set_egress_orientation_using_vxlan_port_add) {
                rv = bcm_port_class_set(unit, out_tunnel, bcmPortClassForwardEgress, g_vxlan.vxlan_network_group_id); 
                if (rv != BCM_E_NONE) {
    ...
    if(verbose >= 2){
        printf("added vxlan port: ip tunnel termination gport:0x%08x ",in_tunnel);
        if(egress_if !=0 ) {
            printf("FEC entry (contains ip tunnel encapsulation information):0x%08x ",egress_if);
        }
        else
        {
            printf("ip tunnel initiator gport:0x%08x",out_tunnel);
        }
        printf("vpn::0x%08x\n\r",vpn);
        printf("vxlan lif orientation: network_group_id = %d\n\r", g_vxlan.vxlan_network_group_id);
    }
...
int vxlan_example(int unit, int in_port, int out_port, int vpn_id){
    ...
    if (g_vxlan.dip_sip_vrf_using_my_vtep_index_mode) {
        ...
        rv = vxlan_add_port(unit,vpn,in_port,in_tunnel_gports[0],out_tunnel_gports[1],out_tunnel_intf_ids[1],vxlan_port_flags,&vxlan_port_id[0]);
        if (rv != BCM_E_NONE) {
...
int vxlan_mc_bud_node_example(int unit, int in_port, int de_encap_out_port, int route_out_port, int recycle_port)
{
    ...
    rv = vxlan_add_port(unit,vpn, recycle_port, ip_tunnel_overlay_mc_tunnel, 0, g_vxlan.vxlan_fec_intf_id, 0, &vxlan_port_id);
    if (rv != BCM_E_NONE) {
