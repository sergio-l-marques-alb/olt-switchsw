...
int lsr_field_egress_counter_set( int unit) 
{
   ...
   if (BCM_E_NONE != result) {
    ...
    bcm_field_action_core_config_t core_config[2]={{0,stat_id,sysport_gport},{0,stat_id,sysport_gport}};
    
    /*Para 0 is 3, means setting Counter pointer with VSI value
    (Counter-Profile is '3' ? System-Headers-Record.PPH.VSI :)*/
    result =  bcm_field_action_config_add(unit, eg_ent, bcmFieldActionStat, 2, core_config);
    if (BCM_E_NONE != result) {
