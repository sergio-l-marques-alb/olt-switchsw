...
_bcm_apache_drop_vector_mask_default_set (int unit)
{
    ...
    uint32 entry[][3] = {
                            ...
                            {0x0000B201, 0x00000800, 0x00B013F0},
                            {0x0000B201, 0x0000E800, 0x00F013F0},
...
STATIC int
_bcm_fp_oam_mpls_router_alert_label_get(_mpls_label_t *mpls)
{
    return _bcm_fp_oam_mpls_label_get(SHR_BHH_MPLS_ROUTER_ALERT_LABEL,
                                      0, 0, 0, mpls);
}
                                ...
                                int *mpls_count, bcm_if_t *l3_intf_id)
{
    ...
    if (packet_flags & _BHH_ENCAP_PKT_MPLS_ROUTER_ALERT) {
        _bcm_fp_oam_mpls_router_alert_label_get(&mpls[count++]);
    }
                                 ...
                                 bcm_cos_t int_pri)
{
    ...
    rv = _bcm_olp_l2_hdr_get(unit, 0, &olp_l2_header);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_OAM, (BSL_META_U(unit, "OAM Error: "
                             ...
                             bcm_cos_t  int_pri)
{
    ...
    if (packet_flags & _BHH_ENCAP_PKT_MPLS_ROUTER_ALERT) {
        BCM_IF_ERROR_RETURN
            (_bcm_fp_oam_mpls_router_alert_label_get(&mpls_r_alert));
        counter_info->oam_replacement_offset += 4;
    }
 ...
 */
STATIC int
_bcm_fp_oam_scache_alloc(int unit) {
    _bcm_fp_oam_control_t *oc;
    ...
    *oam_scache = (*oam_scache + sizeof(int));
    ...
    for (idx = 0; idx < ep_count; idx++) {
        ...
        *oam_scache = (*oam_scache + sizeof(int));
        ...
        rv = _bcm_fp_oam_endpoint_gport_resolve(unit, ep_info, &sglp,
                            &dglp, &trunk_id, &svp, &is_vp_valid,
                            &trunk_member);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
                      (BSL_META_U(unit,
...
_bcm_fp_oam_bhh_reinit(int unit)
{
    ...
    if (oc->ukernel_not_ready == 0) {
        ...
        if (_BCM_FP_OAM_PM_BHH_DATA_COLLECTION_MODE_IS_PROCESSED(oc)) {
            ...
            for(itr = 0; itr < _BCM_OAM_MAX_PM_PROFILES; itr++) { 
                sal_memcpy(&(pmc->profile_info[itr]), oam_scache, sizeof(_bcm_fp_oam_pm_profile_int_info_t));
                oam_scache += sizeof(_bcm_fp_oam_pm_profile_int_info_t);
            }
        }
    } 
                                ...
                                bcm_oam_endpoint_info_t *endpoint_info)
{
    ...
    /*Validate CCM period */
    BCM_IF_ERROR_RETURN(_bcm_fp_oam_period_validate
    ...
    if (is_remote) {
        ...
        /* Resolve given endpoint gport value to SGLP and DGLP values. */
        rv = _bcm_fp_oam_endpoint_gport_resolve(unit, endpoint_info, &sglp, 
                            &dglp, &trunk_id, &svp, &is_vp_valid, 
                            &trunk_member);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_OAM,
        ...
        if(!is_replace) {
            ...
            BCM_IF_ERROR_RETURN(
                    _bcm_fp_oam_l2_header_get(unit,    hash_data,
            ...
            /* Pack control message data into DMA buffer */
            buffer     = oc->bhh_dma_buffer;
            buffer_ptr = bhh_sdk_msg_session_update_pack(buffer, &msg_sess);
            buffer_len = buffer_ptr - buffer;
...
bcm_fp_oam_loopback_add(int unit, bcm_oam_loopback_t *loopback_p)
{
    ...
    rv = _bcm_fp_oam_bhh_encap_create(unit,
            ...
            &encap_type,
            &msg.encap_length,
            &counter_info, 7); /*Send with default INT_PRI of 7 */
    if(BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_BCM_OAM,
                (BSL_META_U(unit,
...
uint8* _bcm_fp_oam_pm_dma_buffer_from_endpoint_type_get(int unit, 
                                                _bhh_fp_oam_ep_data_t *h_data_p)
{
    _bcm_fp_oam_control_t *oc = NULL;
    int rv = BCM_E_NONE;
...
int _bcm_fp_oam_pm_dma_buffer_and_len_from_class_get(int unit,
                                                  uint8 class, 
                                                  uint8 **buffer,
                                                  int *buffer_len)
{
                                    ...
                                    uint8 r_subclass, uint16 *r_len)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_fp_oam_pm_dma_buffer_and_len_from_class_get(
                                                        unit, s_class,
                                                        &dma_buffer, 
                                                        &dma_buffer_len));
...
int _bcm_fp_oam_pm_profile_exists(int unit, 
...
int
_bcm_fp_oam_pm_profile_delete(int unit, bcm_oam_pm_profile_t profile_id, 
                              int all) 
{
                              ...
                              bcm_oam_pm_profile_t profile_id)
{
    ...
    /* Pack control message data into DMA buffer */
    buffer = _bcm_fp_oam_pm_dma_buffer_from_endpoint_type_get(unit, h_data_p);
    buffer_ptr = shr_oam_pm_msg_ctrl_profile_attach_pack(buffer,
                                                         &msg_profile_attach);
    buffer_len = buffer_ptr - buffer;
...
void _bcm_fp_oam_copy_stats_msg_to_stats_ptr(
...
int
bcm_fp_oam_pm_profile_attach_get(
        int unit,
        bcm_oam_endpoint_t endpoint_id,
        bcm_oam_pm_profile_t *profile_id)
{
                                  ...
                                  uint32 read_index)
{
    ...
    for (event_type = bcmOAMEventBHHRawData; event_type <=
         bcmOAMEventMplsLmDmRawData; event_type++) { 
...
bcm_oam_pm_raw_data_t *_bcm_fp_oam_pm_raw_data_info_get(int unit, 
                                  bcm_oam_event_type_t event_type)
{
    int rv = BCM_E_NONE;
    _bcm_fp_oam_control_t *oc;
                                             ...
                                             mos_msg_data_t *event_msg)
{
    ...
    if (event_type != bcmOAMEventBHHRawData) {
        return; 
    }
    ...
    for (event_handler_p = oc->pm_event_handler_list_p;
        ...
        if (SHR_BITGET(event_handler_p->event_types.w,
                    event_type)) {
            raw_data = _bcm_fp_oam_pm_raw_data_info_get(oc->unit,
                    event_type);
            dma_buffer = _bcm_fp_oam_pm_dma_buffer_get(oc->unit, 
                    event_type);
            if (!raw_data) {
            ...
            is_updated = _bcm_fp_oam_update_dma_buffer_into_raw_buffer(
...
#ifdef BCM_WARM_BOOT_SUPPORT
            if (!SOC_WARM_BOOT(unit)) {
                rv = _bcm_fp_oam_scache_alloc(unit);
