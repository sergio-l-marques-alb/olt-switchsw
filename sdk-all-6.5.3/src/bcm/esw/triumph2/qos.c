    ...
    for (idx = min_idx; idx < max_idx; idx++) {
        ...
        if (mem == L3_IIFm && field == TRUST_DSCP_PTRf) {
            ...
            if (field_len == _BCM_TR_L3_TRUST_DSCP_PTR_BIT_SIZE) { 
                ...
                if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit)) { 
                    if (hw_prof_idx == 0x7f) {
                        continue;
                    } 
                } else {
...
bcm_tr2_qos_init(int unit)
{
...
#endif
#ifdef BCM_SABER2_SUPPORT
    int service_pri_map_profiles = 0;
#endif
#ifdef BCM_KATANA2_SUPPORT
    ...
    if (NULL == qos_info->rqe_queue_offset_hw_idx) {
        qos_info->rqe_queue_offset_hw_idx =
           sal_alloc(sizeof(uint32) * rqe_queue_offset_profiles, "rqe_queue_offset_hw_idx");
    ...
    if (NULL == qos_info->rqe_queue_offset_refcount) {
        qos_info->rqe_queue_offset_refcount =
           sal_alloc(sizeof(uint32) * rqe_queue_offset_profiles, "rqe_queue_offset_refcount");
...
{
    egr_pri_cng_map_entry_t  egr_pri_cng_map[_BCM_QOS_MAP_CHUNK_EGR_DSCP];
...
_bcm_td2p_egr_qos_combo_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id) 
{
    ...
    egr_pri_cng_map_entry_t  *egr_pri_cng_map;
    ...
    alloc_size = (_BCM_QOS_MAP_CHUNK_EGR_DSCP * 
                  sizeof(egr_pri_cng_map_entry_t));
    ...
    rv = soc_mem_read_range(unit, EGR_PRI_CNG_MAPm, MEM_BLOCK_ANY, 
                           index, 
    ...
    entry = &egr_pri_cng_map[offset];
    soc_EGR_PRI_CNG_MAPm_field32_set(unit, entry, PRIf, map->pkt_pri);
    soc_EGR_PRI_CNG_MAPm_field32_set(unit, entry, CFIf, map->pkt_cfi); 
        ...
        for (pri = 0, no_match_flag =0;
            ...
            value = soc_reg_field_get(unit, RQE_QUEUE_OFFSETr,
                                         entry[pri]/* regval[priority]*/,
                                          profile_field[id]/* profile  being set*/);
...
_bcm_rqe_queue_offset_map_table_sw_delete (int unit, int id)
{
    ...
    sal_memset(rqe_queue_offset_profile, 0,
                       _BCM_QOS_MAP_CHUNK_RQE_QUEUE_OFFSET_MAP);
    ...
    if (soc_feature(unit, soc_feature_vlan_pri_cfi_action)) {
		...
		PBMP_PORT_ITER(unit, port)
		{
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
                ...
                old_index = soc_mem_field32_get(unit, EGR_VLAN_CONTROL_3m,
                ...
                if (old_index == index)
                {
                    ...
                    soc_mem_field32_set(unit, EGR_VLAN_CONTROL_3m, &entry,
                            IPRI_CFI_SELf, 0);
                    rv = WRITE_EGR_VLAN_CONTROL_3m(unit, MEM_BLOCK_ANY, port,
                            &entry);
                    BCM_IF_ERROR_RETURN(rv);
                }
            } else
#endif
            {
                ...
                if (old_index == index)
                {
                    soc_reg_field_set(unit, EGR_VLAN_CONTROL_3r, &rval, IPRI_MAPPING_PTRf, 0);
...
bcm_tr2_qos_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id)
{
    ...
    case _BCM_QOS_MAP_TYPE_RQE_QUEUE_OFFSET_MAP:
        if (!_BCM_QOS_RQE_QUEUE_OFFSET_MAP_USED_GET(unit, id)) {
            ...
            /* Modify what's needed */
            soc_reg_field_set(unit, RQE_QUEUE_OFFSETr,
                  &rqe_queue_offset_profile[map->int_pri], profile_field[hw_idx],
                  map->queue_offset);
             ...
             */
            _bcm_rqe_queue_offset_map_table_compare(unit, entry, hw_idx, &profile_offset);

            if (profile_offset == hw_idx) {
               ...
               /* Add new chunk and store new HW index */
                ...
                QOS_INFO(unit)->rqe_queue_offset_refcount[hw_idx]++;
                QOS_INFO(unit)->rqe_queue_offset_hw_idx[hw_idx] = hw_idx;
            } else {
                ...
                QOS_INFO(unit)->rqe_queue_offset_hw_idx[hw_idx] = profile_offset;
        ...
        if (_BCM_QOS_GPORT_IS_VFI_VP_PORT(port) ||
            ...
            if (ing_map != -1) { /* -1 means no change */
                ...
                if (SOC_IS_TD2_TT2(unit)) { /* Get l3 iif profile entry */
                    uint32 l3_index, old_profile_index = 0;
                    void *entries[1];
                    iif_profile_entry_t l3_iif_profile;
                    iif_entry_t entry;
                    ...
                    if (BCM_SUCCESS(rv)) {
                        ...
                        rv = _bcm_l3_iif_profile_entry_get(unit, 
                                              l3_index, 1, entries);
                        old_profile_index = l3_index;
                        if (BCM_SUCCESS(rv)) {
                            (void)soc_mem_field32_set(unit, L3_IIF_PROFILEm,
                                                      &l3_iif_profile,
                                                      TRUST_DSCP_PTRf, index);
                        
                            _bcm_l3_iif_profile_entry_update(unit, entries,
                                                                 &l3_index);
                            if (BCM_SUCCESS(rv)) {
                                if (l3_index != old_profile_index) {
                                    soc_mem_field32_set(unit, L3_IIFm, 
                                                        (uint32 *)&entry,
                                                        L3_IIF_PROFILE_INDEXf, 
                                                        l3_index);
                                    /* write l3_iif entry to hw */
                                    rv = soc_mem_write(unit, L3_IIFm, 
                                                       MEM_BLOCK_ALL, l3_iif,
                                                       (uint32 *)&entry);
                                    /* remove the old entry */
                                    if ((BCM_SUCCESS(rv)) && 
                                            old_profile_index) {
                                        _bcm_l3_iif_profile_entry_delete(unit, 
                                                           old_profile_index);
                                    } else {
                                        /* remove the new entry as hw write
                                           of new entry is failed.
                                         */
                                        _bcm_l3_iif_profile_entry_delete(unit, 
                                                                    l3_index);
                                    }
                  ...
                  case _BCM_QOS_MAP_TYPE_DSCP_TABLE:
                    if (SOC_IS_KATANA(unit)) {
                        ...
                        rv = soc_mem_read_range(
                                 unit, DSCP_TABLEm, MEM_BLOCK_ANY, index,
                        ...
                        rv = soc_mem_write_range(
                                 unit, DSCP_TABLEm, MEM_BLOCK_ANY,
        ...
        if (SOC_IS_GREYHOUND(unit) || SOC_IS_HURRICANE3(unit) || \
            ...
            if (map_id != cur_id) {
                ...
                if (SOC_MEM_FIELD_VALID(unit, EGR_VLANm, REMARK_CFIf)) {
                    soc_EGR_VLANm_field32_set(unit, &egr_vtab, REMARK_CFIf, 1);
                }
                rv = soc_mem_write(unit, EGR_VLANm,MEM_BLOCK_ALL, vid, &egr_vtab);
            }
        }
                            ...
                            int int_pri, bcm_color_t color)
{
    ...
    old_profile_index =
        soc_mem_field32_get(unit, SOURCE_VPm, &svp, TRUST_DOT1P_PTRf) * 16;
...
_bcm_tr2_vp_dscp_map_mode_get(int unit, bcm_gport_t port, int *mode)
{
    ...
    rv = READ_SOURCE_VPm(unit, MEM_BLOCK_ANY, vp, &svp);
    if (soc_mem_field_valid(unit,SOURCE_VPm,TRUST_DSCP_V4f)) {
        *mode = soc_SOURCE_VPm_field32_get(unit, &svp, TRUST_DSCP_V4f);
    }
    return rv;
}
