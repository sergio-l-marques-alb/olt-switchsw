...
/* 
 * KT2 - Pool of per LinkPhy block internal stream Ids.
 * Each LinkPhy block has internal streams ids in the range of 
 * 0 to 128. Each LinkPhy block supports upto maximum 4 LinkPhy
 * ports. the internal stream id allocation for a port is done
 */
shr_idxres_list_handle_t
_bcm_kt2_blk_int_stream_id_pool[BCM_MAX_NUM_UNITS][_BCM_KT2_LINKPHY_NUM_BLKS];
...
bcm_kt2_subport_wb_scache_alloc(int unit, uint16 version, int *alloc_sz)
{
    ...
    if (SOC_IS_METROLITE(unit)) {
        subport_group_max = _BCM_ML_SUBPORT_GROUP_MAX;
        pp_port_index_max = _BCM_ML_SUBPORT_PP_PORT_INDEX_MAX;
    } else
#endif 
#if defined(BCM_SABER2_SUPPORT)
...
#if defined(BCM_METROLITE_SUPPORT)
    if (SOC_IS_METROLITE(unit)) {
        max_start_addr = _BCM_ML_LINKPHY_TX_DATA_BUF_START_ADDR_MAX;
        min_end_addr   = _BCM_ML_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
        max_end_addr   = _BCM_ML_LINKPHY_TX_DATA_BUF_END_ADDR_MAX;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
...
#if defined(BCM_METROLITE_SUPPORT)
    if (SOC_IS_METROLITE(unit)) {
        linkphy_block_max = _BCM_ML_LINKPHY_BLOCK_MAX;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
...
_bcm_linkphy_subport_init(int unit)
{
    ...
    if (SOC_IS_METROLITE(unit)) {
        subport_group_max = _BCM_ML_SUBPORT_GROUP_MAX;
        subport_port_max = _BCM_ML_SUBPORT_PORT_MAX;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
   ...
   */
    ...
    if (SOC_IS_METROLITE(unit)) {
        ...
        for (index = 0; index < _BCM_ML_MAX_LINKPHY_PORTS; index++) {
            start_index = _BCM_ML_MAX_INT_STREAMS_PER_LINKPHY_PORT * index;
            end_index   =
                (_BCM_ML_MAX_INT_STREAMS_PER_LINKPHY_PORT * index) +
                _BCM_ML_MAX_INT_STREAMS_PER_LINKPHY_PORT -1;
...
#endif
    {
        for (index = 0; index < _BCM_KT2_LINKPHY_NUM_BLKS; index++) {
            start_index = 0;
            end_index = _BCM_KT2_LINKPHY_PER_BLK_STREAMS_MAX - 1;
            rv = shr_idxres_list_create(
                                &(_bcm_kt2_blk_int_stream_id_pool[unit][index]),
                                start_index, end_index,
                                start_index, end_index,
    ...
    for (i = 0; i < config->num_streams; i++) {
        ...
        if (SOC_IS_METROLITE(unit)) {
            if (config->stream_id_array[i] >=
                    (ext_stream_id_base +
                     (ML_MAX_STREAMS_PER_SLICE * (lp_block + 1)))) {
                LOG_ERROR(BSL_LS_BCM_SUBPORT,
                        (BSL_META_U(unit,
...
#if defined(BCM_METROLITE_SUPPORT)
        if (SOC_IS_METROLITE(unit)) {
            start_addr =  _BCM_ML_LINKPHY_TX_STREAM_START_ADDR_OFFSET +
                (_BCM_ML_LINKPHY_TX_DATA_BUF_END_ADDR_MIN + 1) *
                blk_int_stream_id[i];
            end_addr = start_addr + _BCM_ML_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
        } else 
#endif
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
...
#if defined(BCM_METROLITE_SUPPORT)
    if (SOC_IS_METROLITE(unit)) {
        pp_port_index_min = _BCM_ML_SUBPORT_PP_PORT_INDEX_MIN;
        pp_port_index_max = _BCM_ML_SUBPORT_PP_PORT_INDEX_MAX;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
...
#if defined(BCM_METROLITE_SUPPORT)
    if (SOC_IS_METROLITE(unit)) {
        max_streams = ML_MAX_STREAMS_PER_SLICE;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
...
#if defined(BCM_METROLITE_SUPPORT)
    if (SOC_IS_METROLITE(unit)) {
        min_subport_index = ML_MIN_SUBPORT_INDEX;
    } else
#endif
#if defined(BCM_SABER2_SUPPORT)
    if(SOC_IS_SABER2(unit)) {
...
#if defined(BCM_METROLITE_SUPPORT)
        if (SOC_IS_METROLITE(unit)) {
            end = start + (ML_MAX_LINKPHY_STREAMS_PER_PORT - 1);
        } else
#endif
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
...
{
    uint32 rval1 = 0;
    uint32 port_num = 0, blk_int_stream_id = 0;
    ...
    if (rval1) {
        ...
        port_num = soc_reg_field_get(unit, RXLP_INTERRUPT_DATA_SOURCEr, rval1, PORT_NUMBERf);
    ...
    for (error_type = bcmSubportLinkphyRxErrorMaxFrameSize;
        ...
        if (BCM_SUBPORT_LINKPHY_RX_ERROR_GET(errors, error_type)) { 
            ...
            if (data_log_valid && data_log_enabled) {
                ...
                sal_memcpy(error_data.fragment_data, intr_data_mem, BCM_SUBPORT_LINKPHY_MAX_FRAGMENT_SIZE);
            }
