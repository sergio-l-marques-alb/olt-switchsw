    ...
    for (loop = 0;loop < ML_MAX_PORTS_PER_BLOCK; loop++) {
        ...
        if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), block_port)) {
            ...
            if (block < 1) {
                ...
                soc_metrolite_xl_port_speed_get(unit, block_port, &speed);
            }
...
void soc_ml_port_bitmap_add(int unit,soc_port_t port,int block,
        int speed, int block_speed, int new_lanes) {
    int         blk=0;
    soc_info_t  *si = &SOC_INFO(unit);
    ...
    if (block == 0) {
        ...
        if (SOC_PBMP_MEMBER(my_pbmp_xport_xe, port) || 
            ...
            /* In individual lane mode, when speed is less than 40G, 
               then eagle port needs to come up in mixed mode. 
               In mixed mode, hg is not supported */
            if ((block_speed < 40000) && (new_lanes == 4)) {
                SOC_PBMP_PORT_ADD(si->xe.bitmap, port);
                SOC_PBMP_PORT_REMOVE(si->xe.disabled_bitmap, port);
                speed = 10000;
            } else {
...
int _bcm_ml_port_lanes_set(int unit, bcm_port_t port, int lanes)
{
    ...
    if (IS_MXQ_PORT(unit, port)) {
        ...
        if ((new_lanes == 4) && 
                ((_ml_config_id[unit] == (BCM56272_DEVICE_ID_OFFSET_CFG + 5)) || 
                 (_ml_config_id[unit] == (BCM56271_DEVICE_ID_OFFSET_CFG + 1)))) {
                /* Nitesh: Equivalent Config for Metrolite ? */
            LOG_DEBUG(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "Lane 4 not supported on N2 flex unit %d: port %d cfg %d\n"),
                     unit, port, _ml_config_id[unit]));
    ...
    for (loop = 0;loop < ML_MAX_PORTS_PER_BLOCK; loop++) {
        block_port=(*ml_block_ports[unit])[block][loop];
        max_speed = bcm56270_speed[unit][_ml_config_id[unit]][block_port - 1];
        if((IS_HG_PORT(unit, block_port)) && (max_speed == 10000)) {
            max_speed = 11000;
        }
        block_speed += max_speed;
    }
    if (block_speed < (*ml_port_speeds[unit])[block][new_lanes-1]) {
        LOG_CLI((BSL_META_U(unit,
                        "Total BW of block %d is less than %d\n"),block_speed,
                    (*ml_port_speeds[unit])[block][new_lanes-1]));
        return BCM_E_PARAM;
    }
    ...
    for (loop = 0; loop < ML_MAX_PORTS_PER_BLOCK; loop++) {
        ...
        if (block < 1) {
            ...
            /* Add exception for eagle port due to mixed mode */
            soc_metrolite_xl_port_speed_get(unit, block_port,
        ...
        if (old_ports[old_lanes-1][loop] == new_ports[new_lanes-1][loop] ) {
            ...
            soc_ml_port_bitmap_add(unit,block_port,block,
                    si->port_speed_max[block_port], block_speed, new_lanes);
        }
        if (old_ports[old_lanes-1][loop]  > new_ports[new_lanes-1][loop]) {
