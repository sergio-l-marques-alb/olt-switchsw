...
extern int _ml_config_id[SOC_MAX_NUM_DEVICES];
#endif
...
_bcm_sb2_port_oamp_enable(int unit, bcm_port_t port, int enable)
{
    ...
    if (enable == TRUE) {
	 ...
	 if (SOC_IS_METROLITE(unit)){
        ...
        BCM_IF_ERROR_RETURN(bcm_katana2_cosq_reconfigure_flexio(unit, port));
#if defined(BCM_METROLITE_SUPPORT)
        if (SOC_IS_METROLITE(unit)){
	...
	if (SOC_IS_METROLITE(unit)){
            ...
            port_speed = bcm56270_speed[unit][_ml_config_id[unit]][first_port];
	}else
#endif
	{
            first_port = (*sb2_block_ports[unit])[block][0];
