...
{
	uint32 packets= kbits/_DPP_POLICER_PACKETS_TO_BYTES_FACTOR;
	packets *= _DPP_POLICER_KBITS_TO_BYTES_FACTOR;
	return packets;
}

STATIC uint32 _bcm_petra_policer_translate_packets_to_kbits(uint32 packets)
{
	uint32 kbits = packets/_DPP_POLICER_KBITS_TO_BYTES_FACTOR;
	kbits *= _DPP_POLICER_PACKETS_TO_BYTES_FACTOR;
	return kbits;
}
 ...
 *  
 */
STATIC void _bcm_dpp_policer_correct_config_rates(bcm_policer_config_t *pol_cfg, int divide)
{
    int i = 0;
    uint32 *rates[6];
   ...
   ) 
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        if (pol_cfg.flags & BCM_POLICER_MODE_PACKETS) {
            ...
            _bcm_dpp_policer_correct_config_rates(&pol_cfg, TRUE);
   ...
   ) 
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        if (dpp_profile_info->is_packet_mode) {
            _bcm_dpp_policer_correct_config_rates(pol_cfg, FALSE);
        }
    }
#endif /* BCM_88660_A0 */
...
STATIC void _bcm_petra_meter_profile_min_max_rates_get(
   ...
   uint32  *max_rate) 
{
    if (dpp_profile_info.max_cir != SOC_PPC_BW_PROFILE_IR_MAX_UNLIMITED) {
        *max_rate = dpp_profile_info.max_cir;
    } else {
        *max_rate = dpp_profile_info.cir;
    }
    if ((dpp_profile_info.max_eir != SOC_PPC_BW_PROFILE_IR_MAX_UNLIMITED) && (*max_rate < dpp_profile_info.max_eir)) {
...
STATIC int _bcm_petra_hierarchical_meters_max_min_rates_get_and_update(
   ...
   SOC_PPC_MTR_BW_PROFILE_INFO   *dpp_profile_info) 
{
    SOC_PPC_MTR_BW_PROFILE_INFO   dpp_other_profile_info, dpp_other_exact_profile_info;
    int i;
    uint32 profile;
   ...
   bcm_policer_t *policer_id) 
{
    ...
    if (pol_cfg->flags & BCM_POLICER_WITH_ID) {
		...
		/* meter id: bits 0-12 in PB, 0-14 in ARAD and above
		   group (0/1): bit 13 in PB, 15 in ARAD and above
    ...
    *policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, meter, act_meter_group, act_meter_core);
    ...
    for (meter_id_incr = 0; meter_id_incr < nof_meters; ++meter_id_incr) {
        ...
        rv = bcm_petra_meter_profile_alloc(unit, act_meter_core, act_meter_group, meter + meter_id_incr, &sw_state_info, 
										   &dpp_profile_info, &new_profile, &old_profile, change_profile);
        if (rv == BCM_E_NOT_FOUND) {
                           ...
                           void *cookie) 
{
	...
	/*ethernet policer not supported for Jericho*/
	if (SOC_IS_JERICHO(unit)) {
		BCM_EXIT;
	}
   ...
   bcm_policer_t policer_id) 
{
	...
	if (SOC_IS_JERICHO(unit)) {
		for (pkt_type = 0; pkt_type < SOC_PPC_NOF_MTR_ETH_TYPES; ++pkt_type) {
			BCMDNX_IF_ERR_EXIT(MBCM_PP_DRIVER_CALL(unit, mbcm_pp_mtr_eth_policer_glbl_profile_map_set, (unit, core_id, pp_port, pkt_type, policer_indx + type_offset[pkt_type])));
		}
	}else{
   ...
   bcm_policer_color_decision_t *policer_color_decision)
{
	...
	policer_color_decision->policer0_other_bucket_has_credits = internal_info->policer0_other_bucket_has_credits;
	policer_color_decision->policer1_color = internal_info->policer1_color;
	policer_color_decision->policer1_other_bucket_has_credits = internal_info->policer1_other_bucket_has_credits;
