  ...
  if (status == BCM_E_NONE) {
      ...
      if (is_found_tmp) {
        ...
        if (SOC_IS_JERICHO_PLUS(unit)) {
              ...
              status  = _bcm_dpp_am_template_out_rif_profile_index_get(unit, &outrif_profile, &outrif_profile_index);
              if ((status == BCM_E_NONE) || ((status == BCM_E_NOT_FOUND) && (flags & _BCM_PETRA_L3_INTF_IGNORE_NOT_FOUND))) {
                  BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_out_rif_profile_dealloc(unit, outrif_profile_index, &is_last));
                  /* No need to delete profile since this rif profile is 0 */
              } else {
                  BCMDNX_ERR_EXIT_VERB_MSG(status, (_BSL_BCM_MSG("Out-RIF profile index get failed")));
              }
        }
      }
  }
