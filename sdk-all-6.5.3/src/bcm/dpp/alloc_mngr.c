...
bcm_dpp_am_oam_trap_code_upmep_ftmh_header_alloced_check(
        ...
        uint32 trap_code)
{
    return dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_oam_trap_code_upmep_ftmh_header, 1, trap_code);
}
                                             ...
                                             bcm_dpp_rx_virtual_traps_t *virtual_traps_p)
{
    ...
    if(!(flags & BCM_DPP_AM_TEMPLATE_FLAG_CHECK)) {
        ...
        if (trap_id != 0) {
            new_trap_code = (*template-1) + SOC_PPC_TRAP_CODE_RESERVED_MC_0;
    ...
    else
    {
        ...
        if ( (*template != 0) && (virtual_traps_p == NULL) && (*is_allocated) ){
            rc = dpp_am_res_check_all(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_trap_virtual,_BCM_NUMBER_OF_VIRTUAL_TRAPS,0);
