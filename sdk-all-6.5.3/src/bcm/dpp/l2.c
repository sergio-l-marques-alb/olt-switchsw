...
/*L2 Cache invalid index for SW DB of index cotrol  (_bcm_l2_cache_index_t)*/
#define _BCM_DPP_L2_CACHE_INVALID_INDEX -1
...
* Purpose:    Check of certian entry present in SW DB, comparison by all entry params
* Params:
* unit         -  which unit is it
...
* prog_profile - (IN) programable trap profile to comapre
                                            ...
                                            int mac_lsb,int port, int prog_profile,int *index_used_p)
{
    ...
    int i;
    BCMDNX_INIT_FUNC_DEFS;
    *index_used_p = _BCM_DPP_L2_CACHE_INVALID_INDEX; /*invalidate index*/
                       ...
                       bcm_l2_cache_addr_t *addr)
{
    ...
    BCMDNX_IF_ERR_EXIT(
    _bcm_dpp_l2_cache_index_values_get(unit,&indx_type,&mac_lsb,&port,&prog_trap_profile,index));
    ...
    if(indx_type == _BCM_L2_CACHE_INDEX_TYPE_PROG) 
    {
        ...
        if(port_trap_info.trap_enable_mask & (SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << prog_trap_profile)) {
            ...
            /* get snoop info */
            soc_sand_rv = soc_ppd_trap_snoop_profile_info_get(soc_sand_dev_id,prog_trap_profile + SOC_PPC_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0,&snoop_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
{
    int rv = BCM_E_NONE;
    int index_of_entry_in_sw_db = _BCM_DPP_L2_CACHE_INVALID_INDEX;
    _bcm_l2_cache_index_control_t index_entry;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if( index_of_entry_in_sw_db != _BCM_DPP_L2_CACHE_INVALID_INDEX)
    {
                       ...
                       int is_for_delete)
{
        ...
        (addr->vlan_mask == (bcm_vlan_t)(-1))) 
    {
        ...
        if (addr->flags & BCM_L2_CACHE_TUNNEL) {
            ...
            if ((trap_id == SOC_PPC_TRAP_CODE_ETH_L2CP_PEER) || (trap_id == SOC_PPC_TRAP_CODE_ETH_L2CP_DROP)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Single action per API is supported")));
            }
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_id == SOC_PPC_TRAP_CODE_ETH_L2CP_PEER)){
            handle_type = SOC_PPC_VSI_L2CP_HANDLE_TYPE_PEER;
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_id == SOC_PPC_TRAP_CODE_ETH_L2CP_DROP)){
        ...
        (addr->ethertype_mask == 0x0) && (addr->vlan_mask == 0)) 
    {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            ...
            if ( (BCM_GPORT_IS_TRAP(addr->dest_port)) && (_BCM_TRAP_ID_IS_VIRTUAL(trap_id)) )
            {
                ...
                rv =  soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id,virtual_traps_p.soc_ppc_trap_code,&_bcm_trap_info.trap);
                BCM_SAND_IF_ERR_EXIT(rv);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    *size = _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL;
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}
