 ...
 )
{
    ...
    if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && (SOC_JER_PP_EG_ENCAP_IP_TUNNEL_SIZE_PROTOCOL_TEMPLATE_ENABLE == 1)) {        
        ...
        if (is_replace) {
            int old_is_last; 
            int old_encapsulation_mode = ipv4_encap_info->dest.encapsulation_mode_index; 
            ...
            rv = bcm_dpp_am_template_ip_tunnel_encapsulation_mode_alloc(
                    unit, 
                    0, /* flags */
                    &ip_tunnel_size_protocol_template,
                    &mapping_is_new, 
...
int _bcm_dpp_is_soc_property_termination_support(int unit, bcm_tunnel_terminator_t* tunnel)
{
    ...
    is_ipv6_tunnel = _bcm_petra_tunnel_type_is_ipv6 (unit,tunnel->type);
    if (is_ipv6_tunnel) {
        if (!soc_property_get(unit, spn_BCM886XX_IPV6_TUNNEL_ENABLE, 1)){
            BCMDNX_ERR_EXIT_MSG(BCM_E_DISABLED, (_BSL_BCM_MSG("IPv6 tunnel is disabled")));
        } 
    }
    else {
 ...
 )
{
    ...
    /* update type */
    rv = _bcm_petra_tunnel_ipv4_type_to_ppd(unit,tunnel->type, _bcm_tunnel_subtype_none, ipv4_encap_info, (tunnel->flags & BCM_TUNNEL_REPLACE));
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *tunnel_eep)
{
    ...
    SOC_PPC_EG_ENCAP_IPV4_ENCAP_INFO ipv4_encap_info;
    int global_lif_id = 0;
    int is_tunneled = 0, local_out_lif=0, flags = 0;
    int eep_alloced=0;
...
int bcm_dpp_eg_encap_data_entry_alloc(int unit, SOC_PPC_EG_ENCAP_DATA_INFO *data_info, int *global_lif_id, int *local_lif_id)
{
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL,(_BSL_BCM_MSG("Data-format dynamically alloc is not support for ARAD+ and below")));
    } else {
        /* get data-format global lif dynamically and add it to EEDB */
    ...
    int *local_lif_id)
{
    ...
    sal_memcpy((uint8 *)data_info.data_entry, (uint8 *)data, 8);    

    BCMDNX_IF_ERR_EXIT(bcm_dpp_eg_encap_data_entry_alloc(unit, &data_info, global_lif_id, local_lif_id));
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if ((conn_to_np_enable == 1) && (tunnel->type == bcmTunnelTypeCount)) {
        ...
        rv = bcm_petra_data_entry_alloc(unit, data, &tunnel->tunnel_id, &tunnel->dscp_map);
    } else {
...
bcm_petra_tunnel_initiator_clear(
    ...
    bcm_l3_intf_t *intf)
{
    int global_lif_id, local_out_lif, is_tunneled;
    SOC_PPC_EG_ENCAP_ENTRY_INFO encap_entry_info[SOC_PPC_NOF_EG_ENCAP_EEP_TYPES_MAX];
    ...
    /* ERSPAN / RSPAN*/
    else if (encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_DATA && SOC_PPD_EG_ENCAP_DATA_FORMAT_GET(soc_sand_dev_id,&(encap_entry_info->entry_val.data_info))) {
   ...
   /* check if this tunnel is allocated */
    ...
    if (!is_ipv6_tunnel) {
        ...
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && (SOC_JER_PP_EG_ENCAP_IP_TUNNEL_SIZE_PROTOCOL_TEMPLATE_ENABLE == 1)) {
            ...
            bcm_dpp_am_template_ip_tunnel_encapsulation_mode_free(unit, encap_entry_info[0].entry_val.ipv4_encap_info.dest.encapsulation_mode_index, &is_last_ip_tunnel_encapsulation_mode); 
            ...
            if (is_last_ip_tunnel_encapsulation_mode) {
                ...
                rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_eg_encap_ip_tunnel_size_protocol_template_set, 
                                          (unit, encap_entry_info[0].entry_val.ipv4_encap_info.dest.encapsulation_mode_index, &ip_tunnel_size_protocol_template)); 
                BCMDNX_IF_ERR_EXIT(rv);
            }
    ...
    SOC_SAND_IF_ERR_RETURN(soc_sand_rv);
                 ...
                 && (SOC_PPD_EG_ENCAP_DATA_FORMAT_GET(soc_sand_dev_id,&(encap_entry_info->entry_val.data_info))))
        {
            ...
            if (SOC_PPC_EG_ENCAP_EEDB_IS_FORMAT_ERSPAN(&(encap_entry_info->entry_val.data_info))) {
                ...
                tunnel->span_id = SOC_PPC_EG_ENCAP_DATA_ERSPAN_FORMAT_SPAN_ID_GET(soc_sand_dev_id,&(encap_entry_info->entry_val.data_info));
            } else { /* RSPAN*/
 ...
 ) 
{
    ...
    /* Indicate it's IP-LIF-Dummy for Explicit NULL mpls label */
    if (SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->pp.explicit_null_support &&
        (info->flags & BCM_TUNNEL_TERM_TUNNEL_WITH_ID) &&
        ((SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id) == BCM_GPORT_TUNNEL_ID_GET(info->tunnel_id))) {
        term_key->flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP_DUMMY;
    }

    /* vrf is used as termination key in DIP+SIP+VRF mode
       and in VXLAN tunnel mode: */
    if (SOC_IS_JERICHO(unit) 
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if(term_key.flags & SOC_PPC_RIF_IP_TERM_FLAG_KEY_ONLY) {
        ...
        soc_sand_rv =
        soc_ppd_rif_ip_tunnel_term_add(soc_sand_dev_id, SOC_PPC_RIF_IP_TUNNEL_TERM_ADD_LIF_ID_IS_FEC, &term_key, local_inlif,&term_info,  NULL,&success);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int _bcm_petra_tunnel_terminator_config_verify(int unit) {
    BCMDNX_INIT_FUNC_DEFS;
...
int _bcm_petra_tunnel_terminator_config_add_verify( 
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_petra_tunnel_terminator_config_verify(unit));
...
int bcm_petra_tunnel_terminator_config_get( 
    ...
    BCMDNX_IF_ERR_EXIT(
       _bcm_petra_tunnel_terminator_config_verify(unit));
    ...
    if (found) {
        config_action->tunnel_class = lif_index;
    } else {
        BCMDNX_IF_ERR_EXIT(BCM_E_NOT_FOUND);
    }
