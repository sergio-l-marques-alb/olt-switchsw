...
*  Revision      :  $Id: falcon_tsc_internal.c 1334 2016-01-28 01:06:11Z kirand $ *
*                                                                                 *
...
static uint8_t _falcon_tsc_rdb_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;

	if (!err_code_p)
		return 0;

	EPSTM(rddata = falcon_tsc_rdb_uc_ram (pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Word Read */
static uint16_t _falcon_tsc_rdw_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
...
#endif
	EPSTM(rddata = falcon_tsc_rdw_uc_ram (pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Byte Write  */
static err_code_t _falcon_tsc_wrb_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val){

	return falcon_tsc_wrb_uc_ram (pa, addr, wr_val);                              /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
static err_code_t _falcon_tsc_wrw_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){
	if (addr % 2 != 0)                                                              /* Validate even address */
		return _error(ERR_CODE_INVALID_RAM_ADDR);
	return falcon_tsc_wrw_uc_ram (pa, addr, wr_val);                                     /* Use Micro register interface for writing RAM */
}
...
SDK_STATIC err_code_t _falcon_tsc_get_osr_mode (const phymod_access_t *pa, falcon_tsc_osr_mode_st *imode){
	falcon_tsc_osr_mode_st mode;

	ENULL_MEMSET(&mode, 0, sizeof(falcon_tsc_osr_mode_st));
...
SDK_STATIC err_code_t _falcon_tsc_read_lane_state (const phymod_access_t *pa, falcon_tsc_lane_state_st *istate){
	...
	falcon_tsc_lane_state_st state;
	uint8_t ladder_range = 0;
	uint16_t reg_data;

	ENULL_MEMSET(&state, 0, sizeof(falcon_tsc_lane_state_st));

	if (!istate)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);

	EFUN(_falcon_tsc_pmd_lock_status (pa, &state.rx_lock, &state.rx_lock_chg));
	...
	{  falcon_tsc_osr_mode_st osr_mode;
	   ENULL_MEMSET(&osr_mode, 0, sizeof(falcon_tsc_osr_mode_st));
	   EFUN(_falcon_tsc_get_osr_mode (pa, &osr_mode));
	...
	EFUN(_falcon_tsc_sigdet_status (pa, &state.sig_det, &state.sig_det_chg));
	ESTM(state.rx_ppm = rd_cdr_integ_reg() / 84);
	EFUN(falcon_tsc_get_clk90_offset (pa, &state.clk90));
	EFUN(falcon_tsc_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_LD, &reg_data));
...
SDK_STATIC err_code_t _falcon_tsc_read_core_state (const phymod_access_t *pa, falcon_tsc_core_state_st *istate){
	falcon_tsc_core_state_st state;
	struct falcon_tsc_uc_core_config_st core_cfg;

	ENULL_MEMSET(&state, 0, sizeof(falcon_tsc_core_state_st));
	ENULL_MEMSET(&core_cfg, 0, sizeof(struct falcon_tsc_uc_core_config_st));
	...
	{  int16_t die_temp = 0;
	   EFUN(falcon_tsc_read_die_temperature (pa, &die_temp));
	...
	EFUN(_falcon_tsc_pll_lock_status (pa, &state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _falcon_tsc_display_core_state_no_newline (const phymod_access_t *pa){
	...
	EFUN(_falcon_tsc_read_core_state (pa, &state));
	EFUN_PRINTF(("%02d  ",  falcon_tsc_get_core()));
	...
	EFUN(_falcon_tsc_display_pll_to_divider (pa,   state.pll_div));
...
SDK_STATIC err_code_t _falcon_tsc_pll_lock_status (const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg){

	uint16_t rddata;

	EFUN(falcon_tsc_pmd_rdt_reg (pa, PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _falcon_tsc_pmd_lock_status (const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg){

	uint16_t rddata;

	EFUN(falcon_tsc_pmd_rdt_reg (pa, PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _falcon_tsc_sigdet_status (const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg){
	uint16_t rddata;

	EFUN(falcon_tsc_pmd_rdt_reg (pa, SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _update_uc_lane_config_st (const phymod_access_t *pa, struct  falcon_tsc_uc_lane_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_functions_st (const phymod_access_t *pa, struct falcon_tsc_usr_ctrl_disable_functions_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_st (const phymod_access_t *pa, struct falcon_tsc_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = st->byte;
...
static err_code_t _update_uc_core_config_st (const phymod_access_t *pa, struct falcon_tsc_uc_core_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_uc_core_config_word (const phymod_access_t *pa, struct falcon_tsc_uc_core_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_uc_lane_config_word (const phymod_access_t *pa, struct falcon_tsc_uc_lane_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_functions_byte (const phymod_access_t *pa, struct falcon_tsc_usr_ctrl_disable_functions_st *st){
	uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_byte (const phymod_access_t *pa, struct  falcon_tsc_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = 0;
...
static err_code_t _falcon_tsc_display_pll_to_divider (const phymod_access_t *pa, uint8_t val){
...
static err_code_t _falcon_tsc_display_ber_scan_data (const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	uint8_t range250;
	falcon_tsc_osr_mode_st osr_mode;
	struct falcon_tsc_uc_core_config_st core_config;

	ENULL_MEMSET(&core_config, 0, sizeof(core_config));
	ENULL_MEMSET(&osr_mode, 0, sizeof(osr_mode));

	EFUN(falcon_tsc_get_uc_core_config (pa, &core_config));
	EFUN(_falcon_tsc_get_osr_mode (pa, &osr_mode));
	...
	for (i = 0; i < cnt; i++) {
		...
		EFUN(falcon_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
		...
		EFUN(falcon_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
	...
	EFUN_PRINTF(("\n"));
	EFUN(falcon_tsc_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END, 0x00, 2000));
