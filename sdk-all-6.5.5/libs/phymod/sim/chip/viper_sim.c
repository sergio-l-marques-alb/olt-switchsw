...
STATIC uint32_t
viper_sim_default_data_get(uint32_t addr)
{
    uint32_t devad, reg;
...
STATIC uint32_t
viper_sim_write_adjust(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    uint32_t devad, reg, val;
    uint32_t sgmii_mode = 0, speed = 0, is_2p5_g = 0, duplex_status = 0;
    ...
    if (devad == 0) {
        ...
        case 0xc050:
            ...
            viper_sim_write(pms_data, addr + 1, 0x3);
            break;
        case 0x0000:
            /* Set SPEED_STATUS and DUPLEX_STATUS  in status reg 0x8304  */
            viper_sim_read(pms_data, addr + 0x8304, &val);
            speed = (((data >> 6) & 0x1) << 1) | ((data >> 13) & 0x1);
            duplex_status = ((data >> 8) & 0x1);
            val = (val & 0xfffffffd) | ((duplex_status & 0x1) << 2) | (0x1 << 1);
            val = (val & 0xffffffc7) | ((speed & 0x3) << 3);
            viper_sim_write(pms_data, addr + 0x8304, val);
            break;
        case 0x8300:
            /* Set SGMII_MODE in status reg 0x8304 */
            viper_sim_read(pms_data, addr + 0x4, &val);
            sgmii_mode = ((data & 0x1)==1)?0:1;
            val = (val & 0xfffffffe) | (sgmii_mode & 0x1) | (0x1 << 1);
            viper_sim_write(pms_data, addr + 0x4, val);
            break;
        case 0x8308:
            /* Set SPEED_STATUS in status reg 0x8304 */
            viper_sim_read(pms_data, addr - 0x4, &val);
            is_2p5_g = ((data & 0x1f)==0x10)?1:0;
            if (is_2p5_g) {
                speed = 0x3;
            }
            val = (val & 0xffffffc7) | ((speed & 0x3) << 3) | (0x1 << 1);
            viper_sim_write(pms_data, addr - 0x4, val);
            break;
        default:
            break;
...
viper_sim_read(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t *data)
{
    ...
    if (addr < VIPER_BLK) {
        ...
        (void)viper_sim_read(pms_data, VIPER_BLK, &blk);
        /* IEEE bit */
        if (addr & 0x10) {
            blk |= 0x8000;
        } else {
        ...
        if (addr != VIPER_AER && addr != VIPER_BLK) {
            (void)viper_sim_read(pms_data, VIPER_AER, &aer);
            addr |= (aer << 16);
        }
    } else {
    ...
    for (idx = 0; idx < pms_data->entries_used; idx++) {
        ...
        if (pse->addr == addr) {
            *data = pse->data;
            DBG_VERB(("viper_sim_read 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, *data));
            return PHYMOD_E_NONE;
        }
    }
    ...
    *data = viper_sim_default_data_get(addr);
...
viper_sim_write(phymod_sim_data_t *pms_data, uint32_t addr, uint32_t data)
{
    ...
    if (addr != VIPER_AER && addr != VIPER_BLK) {
        ...
        if (lane > 3) {
            ...
            if (lane == 4 || lane == 6) {
                ...
                (void)viper_sim_write(pms_data, addr, data);
    ...
    data = viper_sim_write_adjust(pms_data, addr, data);
    ...
    for (idx = 0; idx < pms_data->entries_used; idx++) {
        ...
        if (pse->addr == addr) {
            pse->data &= ~mask;
            pse->data |= data;
            DBG_VERB(("viper_sim_write 0x%08"PRIx32" = 0x%04"PRIx32"\n",
                      addr, pse->data));
            return PHYMOD_E_NONE;
        }
    }
    ...
    if (data == viper_sim_default_data_get(addr)) {
        return PHYMOD_E_NONE;
    }
    ...
    DBG_VERB(("viper_sim_write 0x%08"PRIx32" = 0x%04"PRIx32" (new)\n",
              addr, pse->data));
