...
bcm_dpp_counter_state_diag_mask(int unit) {
    ...
    alloc_size = sizeof(uint64) * _SUB_COUNT * ((SOC_DPP_DEFS_GET(unit, nof_counter_processors) * SOC_DPP_DEFS_GET(unit, counters_per_counter_processor)) + 
                              ...
                              unsigned int *depth)
{
    ...
    for (proc = 0; proc < unitData->num_counter_procs; proc++) {
        ...
        if ((info->mode.format == SOC_TMC_CNT_FORMAT_PKTS) || 
            (info->mode.format == SOC_TMC_CNT_FORMAT_BYTES) || 
            (info->mode.format == SOC_TMC_CNT_FORMAT_MAX_QUEUE_SIZE) || 
            (info->mode.format == SOC_TMC_CNT_FORMAT_IPT_LATENCY)) {
            continue;
        }
                             ...
                             unsigned int * finalProc)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    while (num > 0) {
        _bcm_dpp_modify_proc_and_index(unitData, &proc, &index, FALSE);
        info = &(unitData->proc[proc]);
        if (unitData->num_counter_procs <= proc){
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d invalid counter processor ID %u"), unit, proc));
        }
...
_bcm_dpp_counters_set_id_inside_lif_range(_bcm_dpp_counter_state_t *unitData, unsigned int proc, int set_id) {
    ...
    if (unitData->lif_ranges[is_ingress][command_id].counting_profile == BCM_STAT_LIF_COUNTING_PROFILE_NONE) {
                        ...
                        uint64 *counter)
{
    ...
    select_bitmap = COUNTER_UPDATE_SELECT_PROC_INFO_SPECIFIC_DATA | COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_BACKROUND_INFO | COUNTER_UPDATE_SELECT_LIF_INFO;
    ...
    if (!((base + num - 1) < (unitData->proc[proc].mode.num_counters)) && !((base >= base_limit_down) && ((base + num - 1) < base_limit_up)))
    {
        ...
        {
            if ( ((proc+1) < (SOC_DPP_DEFS_GET(unit, nof_counter_processors))) && (unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP) && (num == 5) )
            {
        ...
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d counter processor %u set %u, counters %u..%u, invalid, valid range of sets is %u..%u"),
                              unit,
                              proc,
                              set_id,
                              base,
                              ...
                              (unitData->proc[proc].mode.num_sets * (unitData->proc[proc].mode.we_val + 1))));
        }
    }
    ...
    if (!cache_only || SOC_TMC_CNT_FORMAT_IS_WIDE(unitData->proc[proc].mode.format)) {
    ...
    uint8                       *channel)
{
    int unit = unitData->cunit;
    int result = BCM_E_NONE;
    int channel_number, nof_crps_pci_cmc_used = 0;    
    ...
    /* This is workaround: in case of working with multiple CMCs for the CPU, we will save at least one channel in the PCI_CMC (SOC_PCI_CMC(unit)) to another application */
    /* this is made because there might be a bug in the interupt/MSI mechanism when the channel do not belong to the PCI_CMC */
    /* since CRPS use polling and not interrupts, it will not face the bug */
    ...
    {   
        for(fifo = 0; fifo < SOC_DPP_DEFS_GET(unit, nof_counter_fifos); fifo++)
        {
            ...
            BCMDNX_IF_ERR_EXIT(result);            
            /* update a counter that holds how much CRPS channels connected to PCI_CMC  */
            if((channel_number >= SOC_PCI_CMC(unit)*NOF_DMA_FIFO_PER_CMC) && (channel_number < SOC_PCI_CMC(unit)*NOF_DMA_FIFO_PER_CMC + NOF_DMA_FIFO_PER_CMC) )
            {
                nof_crps_pci_cmc_used++;
            }
        }        
    }
    result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_mgmt_dma_fifo_channel_free_find, (unit, ((nof_crps_pci_cmc_used >= (NOF_DMA_FIFO_PER_CMC-1))? TRUE:FALSE), &channel_number)); 
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_counter_attach_init(_bcm_dpp_counter_state_t *unitData){
    ...
    /* verify that bg thread is not runnig and attach to DMA */
...
bcm_dpp_statistic_interface_config(int unit){
    ...
    for(command_id = 0; command_id < NUM_OF_COUNTERS_CMDS; command_id++) {
        ...
        BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_index) {
            soc_sandRes = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_cnt_epni_regs_set,(unit, src_type, core_index, eg_type, command_id));            
            result = handle_sand_result(soc_sandRes);
            BCMDNX_IF_ERR_EXIT(result);  
            /* lergacy mode configure only core0, new mode configures core0 and core1 in the same way*/
       ...
       SOC_TMC_CNT_Q_SET_SIZE *q_set_size)
{
    ...
    *command_base_offset = 0;
    ...
    lastVoqMin[lastVoqMinId] += (unitData->proc[crps_index].mode.num_sets * (*lastVoqSize));
    lastVoqMin[SOC_TMC_CNT_FORMAT_PKTS_AND_BYTES] = 
        SOC_SAND_MAX(lastVoqMin[SOC_TMC_CNT_FORMAT_PKTS_AND_BYTES], SOC_SAND_MIN(lastVoqMin[SOC_TMC_CNT_FORMAT_PKTS], lastVoqMin[SOC_TMC_CNT_FORMAT_BYTES]));
   ...
   ) 
{
    ...
    if(!SOC_IS_JERICHO_PLUS(unit) || source == SOC_TMC_CNT_SRC_TYPE_ING_PP)
    {
        *range_id = range_index;
    }
    else
    {
        /* T.B.D - need to read HW in order to know which counter command and stif id was set */
    }
...
bcm_dpp_counter_lif_range_set(
   ...
   SOC_SAND_U32_RANGE   *range
    ...
    if ((range->start > range->end) || 
        ...
        (range->end >= SOC_DPP_DEFS_GET(unit, nof_local_lifs))) {
    ...
    {
        /* T.B.D - add new check that range is not overlapping   */
        /* need prepare function that pull the ids from DB and call the LIF-MGMT for the range and verify no range is set (except from the profile that is just configure */

        /* T.B.D - add a check that the range in 2K resolution */
    ...
    else
    {
        ...
        for (range_index = 0; range_index < SOC_TMC_CNT_LIF_COUNTING_NOF_RANGES_PER_SRC(unit); range_index++) {
            if (counting_profile == unitData->lif_ranges[is_ingress][range_index].counting_profile) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("LIF counting profile %d for source %s is allready in use."), counting_profile, SOC_TMC_CNT_SRC_TYPE_to_string(source)));
            }
        }        
    }
    ...
    if (is_ingress) {
        ...
        result = _bcm_dpp_am_local_outlif_counting_profile_set(unit, range_id, range->start, (range->end - range->start) + 1 /* T.B.D - range->double_entry*/);
        BCMDNX_IF_ERR_EXIT(result);
    }
    ...
    /*Set the range*/
    result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_lif_counting_range_set, (unit, source, range_id, range));
    BCMDNX_IF_ERR_EXIT(result);
...
_bcm_dpp_counter_profile_mapping_table_init(_bcm_dpp_counter_state_t *unitData) 
{
    ...
    memset(lif_profile_mapping_table, BCM_STAT_LIF_COUNTING_PROFILE_NONE, 
        SOC_MAX_NUM_DEVICES*(SOC_TMC_CNT_LIF_COUNTING_MAX_NOF_RANGES_PER_SOURCE + SOC_TMC_CNT_LIF_COUNTING_MIN_NOF_RANGES_PER_SOURCE)*sizeof(int));
...
_bcm_dpp_counter_oam_verify(_bcm_dpp_counter_state_t *unitData) {
    ...
    for (index = 0; (index < unitData->num_counter_procs); index++) {
        if (unitData->proc[index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_OAM) {
            if (unitData->proc[index].mode.command_id == 0) {
                if(first_oam_eng_proc == -1){
                    first_oam_eng_proc = index;
                }
                if ((index >= 2) && (first_oam_eng_proc < 2)) {
                    LOG_ERROR(BSL_LS_BCM_CNT,
                              (BSL_META_U(unit,
                ...
                if ((index >= 2) && (first_oam_ing_proc < 2)){
                    LOG_ERROR(BSL_LS_BCM_CNT,
                              (BSL_META_U(unit,
...
bcm_dpp_counter_config_set(
    ...
    used_dma = ((unitData->proc[crps_index].valid & BCM_DPP_COUNTER_PROC_INFO_VALID) && (!SOC_TMC_CNT_FORMAT_IS_WIDE(unitData->proc[crps_index].mode.format)));    
    ...
    using_dma = !SOC_TMC_CNT_FORMAT_IS_WIDE(format);    
...
bcm_dpp_counter_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (index = 0; index < tempUnit->num_counter_procs; index++) {
            ...
            if (result != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_CNT, (BSL_META_U(unit, "unit %d counter %d failed to read counter replicated packets property. processors init failed\n"), unit, index));
                    BCMDNX_IF_ERR_EXIT(result);
            }
            if (!SOC_TMC_CNT_FORMAT_IS_WIDE(tempUnit->proc[index].mode.format)) {
                int fifo = 0, cache_length = 0;
...
_bcm_dpp_counter_egress_que_build_pointer(_bcm_dpp_counter_state_t *unitData,
    ...
    for (cosq = 0 ; cosq < nof_priorities; cosq++) {
        ...
        if (unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPES_EGQ_TM) {
             ...
             */ 
            lval = (((out_port_field + cosq) & 0xff) << 4) | ((priority & 0x7) << 1) | is_mc;
        ...
        if (((base >= (unitData->proc[proc].mode.num_counters * unitData->proc[proc].mode.we_val)) && 
               ((base + counters_in_set - 1) < (unitData->proc[proc].mode.num_counters * (unitData->proc[proc].mode.we_val + 1))))) {
                         ...
                         unsigned int *set_cnt)
{
    ...
    *set_cnt = 0;
    ...
    for (proc_index = 0; proc_index < unitData->num_counter_procs; proc_index++) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_counters_per_set(unitData, proc_index, &counters_in_set));
...
_bcm_dpp_modify_proc_and_index(_bcm_dpp_counter_state_t * unitData, unsigned int *proc, unsigned int * finalIndex, uint8 subCount)
{
    ...
    /* modification of the ptr and proc should be made only if the source is ING_PP and the group size=5 and the we_val !=0 */
    if ((unitData->proc[*proc].mode.src_type != SOC_TMC_CNT_SRC_TYPE_ING_PP) || (groupSize != 5))
    {
        SOC_EXIT;
    }
