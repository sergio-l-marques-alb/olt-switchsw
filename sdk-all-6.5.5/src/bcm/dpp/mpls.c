    ...
    int unit)
{
    ...
    /* Forbid usage of MPLS swap coupling devices. */
    if (soc_property_get(unit, spn_MPLS_CONTEXT_SPECIFIC_LABEL_ENABLE, 0)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, \
                            (_BSL_BCM_MSG("MPLS swap coupling is disabled.")));
    }
    ...
    if (SOC_IS_JERICHO(unit) && 
         soc_property_get(unit, spn_MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE, 0)) {
        ...
        int outlif_profile_index = 0;

        outlif_profile_index = SOC_IS_JERICHO_PLUS(unit) ? 3 : 1;

        for (i = 1; i <= 4; i++) {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                               ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_JER, i/*entry*/,
                                                                               1 /*value*/,outlif_profile_index/*outlif-profile1*/);
    ...
    */
    ...
    if (SOC_DPP_CONFIG(unit)->pp.custom_feature_mpls_port_termination
        ...
        for (i = 0; i < 3; i++) {
            ...
            BCM_GPORT_TUNNEL_ID_SET(port_termination_tunnel_switch.tunnel_id, SOC_DPP_CONFIG(unit)->pp.custom_feature_mpls_port_termination_lif_id[i]);
            ...
            BCM_MPLS_INDEXED_LABEL_SET(port_termination_tunnel_switch.label, _BCM_PETRA_MPLS_LABEL_FOR_PORT_TERMINATION, i + 1);
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (indx = 0; indx < num_labels; ++indx) {
        ...
        if ((label_array[indx].flags & (BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE)) &&
            (!(label_array[indx].flags & (BCM_MPLS_EGRESS_LABEL_ENTROPY_ENABLE)))) {
        ...
        if ((label_array[indx].flags & (BCM_MPLS_EGRESS_LABEL_ACTION_VALID)) && (label_array[indx].action & BCM_MPLS_EGRESS_ACTION_SWAP_OR_PUSH)) {
            if (!(SOC_IS_JERICHO_B0(unit) || SOC_IS_QMX_B0(unit) || (SOC_IS_JERICHO_PLUS(unit) && !SOC_IS_QAX_A0(unit)))) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_MPLS_EGRESS_ACTION_SWAP_OR_PUSH action suppoted only on Jericho_B0 and Jericho Plus devices")));
            }
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    if (!SOC_DPP_CONFIG(unit)->arad->pp_op_mode.mpls_info.mpls_coupling_enable && 
    ...
    int *label_count)
{
    ...
    if (!SOC_DPP_CONFIG(unit)->arad->pp_op_mode.mpls_info.mpls_coupling_enable &&
         encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_DATA) {
        /* NULL data entry */
        label_array[0].action = BCM_MPLS_EGRESS_ACTION_NOP;
        *label_count = 1;
    }
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        if ((!SOC_IS_JERICHO_PLUS(unit)) &&
                            ...
                            soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_get,(unit, SOC_OCC_MGMT_TYPE_OUTLIF, \
                                                                                              ...
                                                                                              &(encap_entry_info[indx].entry_val.mpls_encap_info.outlif_profile),&val));
                            BCMDNX_IF_ERR_EXIT(soc_sand_rv);
                            if (val) { /* if ELI bit is on, add the relevant flag */
    ...
    )
{
    ...
    /* In case key support l3-interface, flag must be set - NOT SUPPORTED YET */    
    ...
    /* RIF as part of they key feature is not supported yet */
    ...
    SOC_PPC_MPLS_LABEL_RIF_KEY *rif_key)
{
    ...
    /* In case key support l3-interface, flag must be set - CURRENTLY NOT SUPPORTED */    
    ...
    if (info->flags & (BCM_MPLS_SWITCH_LOOKUP_SECOND_LABEL | BCM_MPLS_SWITCH_FRR)) { /* Coupling */
        ...
        rif_key->flags |= (info->flags & BCM_MPLS_SWITCH_FRR) ? SOC_PPC_RIF_MPLS_LABEL_FRR:SOC_PPC_RIF_MPLS_LABEL_COUPLING;
    }
    ...
    if (rif_key->flags & SOC_PPC_RIF_MPLS_LABEL_COUPLING && SOC_DPP_CONFIG(unit)->pp.mpls_label_index_enable) {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (err_code == _BCM_MPLS_TERM_LABEL_RANGE_INTERSECT || err_code == _BCM_MPLS_TERM_LABEL_RANGE_EXIST) {
            ...
            if (info->label != range_action_info.label_low && range_profile_info.mpls_label_range_use_base) {
