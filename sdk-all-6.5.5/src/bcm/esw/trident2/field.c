/* $Id: field.c,v 1.61 Broadcom SDK $
...
#define BCM_IF_ERROR_RETURN_WITH_FREE(_rv_, e, p, max_idx) {    \
    ...
    e = (uint32 *) soc_cm_salloc(unit, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32), "ACL Control");
        if (!e) {
            return (BCM_E_MEMORY);
        }

        p = (uint32 **) soc_cm_salloc(unit, _FP_MAX_ENTRY_WIDTH * sizeof(uint32*), "ACL Control");
        if (!p)
        {
            soc_cm_sfree(unit, e);
            return (BCM_E_MEMORY);
        ...
        {
            p[i]= (uint32 *) soc_cm_salloc (unit, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32), "ACL Control");
            if (!p[i])
            {
                while (i > 0) {
                    soc_cm_sfree(unit, p[--i]);
                }
        ...
        if (((SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2PLUS(unit)))
            ...
            if ((fg->flags & _FP_GROUP_SPAN_DOUBLE_SLICE)
                ...
                if (SOC_MEM_IS_VALID(unit, FP_GLOBAL_MASK_TCAM_Xm) &&
                    ...
                    BCM_IF_ERROR_RETURN_WITH_FREE(rv, e,
                                                  p, _FP_MAX_ENTRY_WIDTH);
