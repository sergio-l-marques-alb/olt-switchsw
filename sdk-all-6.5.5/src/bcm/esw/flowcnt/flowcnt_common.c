...
_bcm_esw_flowcnt_selectors_cmp(int unit, soc_mem_t table, 
    ...
    bcm_stat_group_mode_attr_selector_t *src_attr_selectors,
...
{ 
    ...
    *group_index = profile_group;
    ...
    if (fc_pgroup->available == 0) {
        ...
        sal_memcpy(fc_pgroup->attr_selectors, 
                   attr_selectors, num_selectors *
                   sizeof(bcm_stat_group_mode_attr_selector_t));
...
_bcm_esw_flowcnt_init(int unit)
{
    ...
    local_scache_size = sizeof(uint32) * MAX_FLOWCNT_SCACHE_SIZE;
    ...
    local_scache_size += sizeof(_bcm_flowcnt_warmboot_info_t);
    local_scache_size += sizeof(_bcm_flowcnt_warmboot_group_info_t) *
    ...
    if (SOC_WARM_BOOT(unit)) {
        sal_memcpy(&local_scache_ptr[unit][0],
                   ...
                   MAX_FLOWCNT_SCACHE_SIZE*sizeof(uint32));
...
{    
    ...
    *actual_num_selectors = fc_pgroup->num_selectors;
    ...
    for (i = 0 ; i < effect_num; i++) {
         sal_memcpy(&attr_selectors[i], 
                    &fc_pgroup->attr_selectors[i], 
                    sizeof(bcm_stat_group_mode_attr_selector_t));
    }
    _FLOWCNT_UNLOCK(fc);
...
int _bcm_esw_flowcnt_sync(int unit)
{
    ...
    scache_size = MAX_FLOWCNT_SCACHE_SIZE*sizeof(uint32);
    ...
    flowcnt_wb_info = (_bcm_flowcnt_warmboot_info_t *)
                      (&flowcnt_scache_ptr[unit][0] +
                       scache_size);
    scache_size += sizeof(_bcm_flowcnt_warmboot_info_t);
