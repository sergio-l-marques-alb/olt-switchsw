                    ...
                    uint32 dst_port, uint32 vlan, uint32 *xghp)
{
    ...
    if (SOC_IS_TOMAHAWK(unit) && !soc_feature(unit, soc_feature_higig2)) {
...
lb_setup_dcbs(uint8 **packets, dv_t *dv, int l, int c, pbmp_t pbm,
              ...
              uint32 dmod, uint32 *hgh, int dma_mode)
/*
 ...
 */
{
    ...
    lb_setup_dcbs(lw->lw_tx_packets, dv, tx_len, chain,
                  ...
                  cos, dport, dmod, xghp, lw->lw_dma_mode);
 ...
 */
{
    ...
    PBMP_ITER(lp->lp_pbm, p) {
         ...
         */
#ifdef BCM_CALADAN3_SUPPORT
            /* Loopback test doesnt work on XL port */
            if (SOC_IS_CALADAN3(unit) && IS_XL_PORT(unit, p)) {continue;}
#endif
 ...
 */
{
    ...
    PBMP_ITER(pbm, port) {
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(unit) && lw->lw_c3_lrp_bypass) {
            if (IS_XL_PORT(unit,port)) {continue;  /* Does not work on a0/a1 */}
        ...
        if (IS_HG_PORT(unit, port)) {
            lp->lp_hg_mode = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE,
 ...
 */
{
    ...
    PBMP_ITER(lp->lp_pbm_tx, port) {
        ...
        if (SOC_IS_CALADAN3(unit)) {
            /* Loopback test doesnt work on XL port */
            if (IS_XL_PORT(unit, port)) {continue;}
            if (lw->lw_c3_lrp_bypass) {
                soc_sbx_caladan3_sws_cmic_to_port_mac_loopback(unit, port, 0);
            }
            if (IS_HG_PORT(unit, port)) {
                lp->lp_hg_mode = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE, 0) ?
                                 2 : 1;
            }
        }
#endif
