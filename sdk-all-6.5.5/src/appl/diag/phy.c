...
_phy_diag_loopback(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        if (mode == 3) {  
            rv = port_diag_ctrl(unit, port, inst,
                                   PHY_DIAG_CTRL_SET,
...
#ifdef BCM_CALADAN3_SUPPORT
        rv = soc_sbx_caladan3_port_is_channelized_subport(u, p,
            &is_channelized, &req_phy);

        if (rv) {
            cli_out("soc_sbx_caladan3_port_is_channelized_subport port %d failed: %d\n",
                    p, rv);
        } 
        else if (is_channelized) {
            continue;
        }
#endif
        p1 = p;
...
#ifdef PORTMOD_SUPPORT
        if(soc_feature(u, soc_feature_portmod)) {
           ...
           /* check if the external phy is a legacy phy */
            local_user_access = (portmod_default_user_access_t *)(core_acc.access.user_acc);
...
_phy_mt2(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(cmd_str, "listreg") == 0) {
        ...
        void *buffer, *buf;
        char *fn = NULL, file_name[81], *cbuf;
        parse_table_t pt;
        int lscan_time;
        int buf_size = 4096; /* NULL termination taken care of internally */
        ...
        buf = INT_TO_PTR(((PTR_TO_INT(buffer) + 0x3) & ~0x3));

        cbuf = (char *) buf;
        buf_size -= (PTR_TO_INT(buffer) & 0x3) + 4;
        cbuf[0] = buf_size & 0xff;
        cbuf[1] = (buf_size >> 4) & 0xff;
        cbuf[2] = (buf_size >> 8) & 0xff;
        cbuf[3] = (buf_size >> 12) & 0xff;
        buf = INT_TO_PTR(PTR_TO_INT(buf) + 0x4);
            ...
            if (intermediate) {
                ...
                if(soc_feature(u, soc_feature_portmod)) {
                    ...
                    if ( !is_legacy ) {
                        ...
                        pname = phymod_core_version_t_mapping[core_info.core_version].key+strlen("phymodCoreVersion");
                        namelen = strlen(pname);
                        ...
                        if (EXT_PHY_SW_STATE(u, p) &&
                            ...
                            rv = EXT_PHY_SW_STATE(u, p)->read(u, phy_addr,
                                                              ...
                                                              (phy_devad,
                                                               phy_reg),
                                                              &phy_data);
                        } else if(soc_feature(u, soc_feature_portmod)) {
...
#ifdef PORTMOD_SUPPORT
                            rv = portmod_port_phy_reg_read(u,p,0,phy_reg,&phy_data32);
                            phy_data = (uint16)phy_data32;
#endif
                        } else {
                            rv = soc_miim_read(u, phy_addr, phy_reg, &phy_data);
