    ...
    )
{
  ...
  if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit)) {
    if (SOC_IS_JERICHO(unit)) {
      res = _bcm_dpp_global_lif_mapping_local_to_global_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_INGRESS,ipv4_info->lif_ndx,&global_lif);
      if(BCM_E_NONE != res){
        return res;
      }
      ARAD_PP_FRWRD_DECISION_FEC_SET(unit, &frwrd_info, global_lif);
    } else {
      ...
      if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) &&
          ...
          if (src_bind_info->vsi_id) {
            ...
            res = soc_ppd_frwrd_ipv4_vrf_route_add(unit, 
                                                 ...
                                                 &route_key,
                                                 (SOC_IS_JERICHO(unit)?global_lif:src_bind_info->lif_ndx),
                                                 0,
                                                 success);
          } else {
            /* P2P service */
    ...
    )
{
    ...
    res = _bcm_dpp_global_lif_mapping_local_to_global_get(unit,
                                    _BCM_DPP_GLOBAL_LIF_MAPPING_INGRESS,
                                    src_bind_info->lif_ndx, &global_lif);
