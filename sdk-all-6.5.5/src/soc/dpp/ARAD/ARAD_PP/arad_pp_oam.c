...
#define _BCM_DPP_OAM_OAMP_MEP_DB_BANKS_NOF(_unit) (SOC_IS_QAX(_unit)?12:1)
 ...
 */
static ARAD_PP_INTERNAL_OPCODE_INIT   internal_opcode_init[SOC_PPC_OAM_OPCODE_MAP_COUNT] = {{0}};
     ...
     )
{
    ...
    if (SOC_IS_JERICHO(unit)) 
    {
        host_mem_size_in_bytes = soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_BUFFER_SIZE, 0);        
    }
    else
    {
        /* In Arad+ both soc props should be supported, so check which one is on.*/
        host_mem_size_in_bytes = (soc_property_get(unit, spn_OAMP_FIFO_DMA_BUFFER_SIZE, 0))? soc_property_get(unit, spn_OAMP_FIFO_DMA_BUFFER_SIZE, 0) :
            soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_BUFFER_SIZE, 0);        
    }
    dma_threshold = SOC_DPP_CONFIG(unit)->pp.oam_dma_event_threshold;
    
    timeout = soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_TIMEOUT, 0);
    if (!SOC_IS_JERICHO(unit)) {
        /* Due to documentation mistake, Arad+ should support the ..._event_interface... soc properties as well */
        if (!host_mem_size_in_bytes) {
            host_mem_size_in_bytes = soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_BUFFER_SIZE, 0);
        }
        if (!timeout) {
            timeout = soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_TIMEOUT, 0);
	 ...
	 )
{
    ...
    host_mem_size_in_bytes = soc_property_get(unit, spn_OAMP_FIFO_DMA_REPORT_INTERFACE_BUFFER_SIZE, 0);
    timeout = soc_property_get(unit, spn_OAMP_FIFO_DMA_REPORT_INTERFACE_TIMEOUT, 0);
 ...
 * 2/18/2016 
 * Builds tcam Entry for MPLS in Jericho and masks md_level for 
 * mpls_ignore_mdl 
 */
STATIC
  void
  ...
  )
{
    ...
    if (key->mdl == _ARAD_PP_OAM_Y1731_DEFAULT_MDLEVEL) {
        ...
        soc_sand_bitstream_set_any_field(&key->mdl, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN  - 15, 3, entry->value);
        soc_sand_bitstream_set_any_field(&mask,     sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN  - 15, 3, entry->mask);
        mask = _ARAD_PP_OAM_TCAM_ENTRY_BUILD_SET_MASK_FOR_VERSION_BY_OPCODE(key->opcode);
        val = 0; /* Version(5b) = 0 */
        soc_sand_bitstream_set_any_field(&val, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 20, 5, entry->value);
        soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 20, 5, entry->mask);

        if (key->opcode == (SOC_PPC_OAM_ETHERNET_PDU_OPCODE_COUNT - 1)) {
            mask = 0; /* OpCode(8b) - dont cares */
            val = 0;
            soc_sand_bitstream_set_any_field(&val, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 28, 8, entry->value);
            soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 28, 8, entry->mask);
        } else {
            mask = 0xFFFFFFFF; /* OpCode(8b) */
            soc_sand_bitstream_set_any_field(&key->opcode, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 28, 8, entry->value);
            soc_sand_bitstream_set_any_field(&mask,        sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 28, 8, entry->mask);
        }
    }
}
  ...
  )
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "y1711_enabled",0) == 1) {
        ...
        mask = 0xFFFFFFFF;
        val = 0xe;
        soc_sand_bitstream_set_any_field(&val, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 32, 20, entry->value);
		soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 32, 20, entry->mask);

        mask = 0xFFFFFFFF;
        val = 0x101;
        soc_sand_bitstream_set_any_field(&val, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 44, 12, entry->value);
		soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 44, 12, entry->mask);


        /* 2 entries should be added 1 for LBM (opcode 0xc) 1 for LBR (opcode 0xb) */
        mask = 0xFFFFFFFF;
        if (key->your_disc) {
            val = 0x0c;
        } else {
        ...
        soc_sand_bitstream_set_any_field(&val, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 52, 8, entry->value);
		soc_sand_bitstream_set_any_field(&mask, sizeof(uint32) * 8 * ARAD_TCAM_ENTRY_MAX_LEN - 52, 8, entry->mask);
                  ...
                  for (opcode = 1; opcode < SOC_PPC_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
                      ...
                      switch (action) {
                             case _ARAD_PP_OAM_ACTION_TRAP_ERROR_PASSIVE: {
                              ...
                              oam_payload.mirror_profile = (oam_mirror_profile >> 24) & 0xff;
...
/* TOD for Jericho handled separatly*/
  ...
  if (SOC_IS_JERICHO(unit) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl", 0) == 1)) {
      /* TCAM initialization - MPLS */
      res = arad_pp_oam_tcam_init_mpls(unit, is_bfd);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  }
 ...
 * together. 
 * If  the counter increment bit (from profile_data) is 1 for at 
 * least one of the above opcodes then count all of them. 
                                                                                   ...
                                                                                   SOC_SAND_INOUT  _oam_oam_a_b_table_buffer_t       *oamb_buffer)
{
    ...
    for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
        ...
        for (internal_opcode = 1; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++) {
            ...
            if (oam_payload.sub_type == _ARAD_PP_OAM_SUBTYPE_LM && (profile_data->flags & SOC_PPC_OAM_LIF_PROFILE_FLAG_COUNTED) == 0) {
                oam_payload.sub_type = _ARAD_PP_OAM_SUBTYPE_DEFAULT_OAM_MESSAGE;
            }
            res =_arad_pp_oam_set_counter_disable(unit, internal_opcode, (&oam_payload), &profile_data->mep_profile_data, profile_data->is_piggybacked);
            SOCDNX_SAND_IF_ERR_EXIT(res);
                                                                    ...
                                                                    SOC_SAND_IN SOC_PPC_OAM_LIF_PROFILE_DATA       *profile_data_acc)
{
    ...
    if (classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT ) {
        /* Insert inject entries, but only for endpoints (not MIPs)*/
  ...
  )
{
  ...
  if ((classifier_mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) && update != 1) {
      res = arad_pp_oam_my_cfm_mac_set_unsafe(unit, classifier_mep_entry->port_core, &classifier_mep_entry->dst_mac_address, classifier_mep_entry->port);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  }
  ...
  if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type) ||
      ...
      } else if (found && (SOC_DPP_CONFIG(unit)->pp.oam_hierarchical_lm)) {
          /* Hierarchical LM   */
          h_lm_2nd_mep = 1;
      }
      ...
      if(!(classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY))
      {
          if ((SOC_DPP_CONFIG(unit)->pp.oam_hierarchical_lm) && (h_lm_2nd_mep)) {
              /*
               ...
               */
              uint32 new_counter_temp = oem1_payload.counter_ndx; /* Save the newly desired counter index */
              ...
              /* Now revert back to the newly desired counter, update the
                 your-disc bit and add the new entry */
              oem1_payload.counter_ndx = new_counter_temp;
              oem1_key.your_discr = 1 - oem1_key.your_discr;
              res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload);
              SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
          }
          else {
      ...
      if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)
        &&(classifier_mep_entry->passive_side_lif != _BCM_OAM_INVALID_LIF)) {
          ...
          if (update == 1) {
              oem1_payload = oem1_prev_payload;
              oem1_payload.counter_ndx = classifier_mep_entry->counter + (SOC_IS_JERICHO(unit) ? soc_jer_pp_oam_counter_pointer_addition(unit, classifier_mep_entry, 0 /*is_active (adding passive side here) */) : 0); 

              res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload);
              SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
          } else {
              if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
                  if (found && SOC_DPP_CONFIG(unit)->pp.oam_hierarchical_lm) {
                      /*
                       * Hierarchical LM - MDL hierarcy
                       * The different counters are retrieved by looking up in OEM1 with
                       * your-disc bit different between keys.
                       * Need to update the MP-Map in both entries though.
                       */
                      ...
                      /* Now revert back to the newly desired counter and update the
                         your-disc bit and add the new entry */
                      oem1_payload.counter_ndx = new_counter_temp;
                      oem1_key.your_discr = 1;
                      res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload);
                      SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
                  }
                  else {
  ...
  )
{
  ...
  if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) { /* Ethernet and Y1731 */
      if (classifier_mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) { /* Ethernet only use MY_MAC */
         res = arad_pp_oam_my_cfm_mac_delete_unsafe(unit, classifier_mep_entry->port_core, &classifier_mep_entry->dst_mac_address, classifier_mep_entry->port);
         SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
      }
  ...
  )
{
  ...
  SOC_SAND_IF_ERR_EXIT(_arad_pp_oam_bfd_mep_db_mem_by_entry_type_get(unit, mep_db_entry->mep_type, &mem));
  ...
  if ((mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ||
      ...
      (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE)) { /* Only relevant for Ethernet and Y1731 */
  ...
  )
{
 ...
 LOG_DEBUG(BSL_LS_SOC_OAM,
  ...
  if ((mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ||
      (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
      (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE)) { /* Only relavant for Ethernet and Y1731 */
  /* The function soc_sand_pp_mac_address_struct_to_long writes to indecies 0 and 1 of the second parameter only */
  /* coverity[overrun-buffer-val : FALSE] */    
  ...
  ) 
{
    ...
    {
        if ((SOC_IS_JERICHO(unit) && dpp_pp->oam_use_event_fifo_dma) ||
            (!SOC_IS_JERICHO(unit) && dpp_pp->oam_use_fifo_dma))
        { /* use DMA */
            fill_interrupt_message = arad_pp_oam_fifo_get_interrupt_mesage_dma_internal;
        }
        else /* without DMA */
...
{
    soc_field_t field6 = SOC_IS_QAX(unit) ? FIELD_6_7f: TX_PPH_IN_LIF_ORIENTATIONf;
    soc_field_t field7 = SOC_IS_QAX(unit) ? FIELD_11_11f: TX_PPH_UNKNOWN_DAf;
    soc_field_t field9 = SOC_IS_QAX(unit) ? FIELD_12_27f: TX_PPH_VSI_OR_VRFf;
    soc_field_t field10 = SOC_IS_QAX(unit) ? FIELD_28_45f: TX_PPH_IN_LIF_OR_IN_RIFf;
       ...
       )
{
    ...
    if (next_is_itmh) {
		...
		header_size += _insert_itmh(unit, pkt_header, fwd_dst_info, dp, tc, 0, 0, (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP || mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE)); 
    }
       ...
       )
{
    ...
    case SOC_PPC_OAM_MEP_TYPE_ETH_OAM: field_name = TRAP_N_ETHf;
        break;
    case SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP: field_name = TRAP_N_1731O_MPLSTPf;
        break;
    case SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE: field_name = TRAP_N_1731O_PWEf;
        break;
    case SOC_PPC_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP:
...
soc_error_t arad_pp_oam_interrupt_init_internal(int unit, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data, int cmc, int ch) {
    ...
    {
        use_dma = SOC_DPP_CONFIG(unit)->pp.oam_use_fifo_dma;
        dma_threshold = SOC_DPP_CONFIG(unit)->pp.oam_dma_threshold;
    }
  ...
  )
{
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        found_field[1] = SOC_IS_JERICHO_PLUS(unit) ? OEMA_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMA_LAST_LOOKUP_SLAVE_FOUNDf;
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        found_field[1] = SOC_IS_JERICHO_PLUS(unit) ? OEMB_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMB_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
    ...
    )
{
    ...
    *val = 0;
    if(SOC_IS_JERICHO(unit)) {
        ...
        last_lookup_fields[1] = SOC_IS_JERICHO_PLUS(unit) ? OEMA_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMA_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
        last_lookup_regs[0] = IHB_OEMA_LAST_LOOKUPr;
    ...
    if(SOC_IS_JERICHO(unit)) {
        ...
        last_lookup_fields[1] = SOC_IS_JERICHO_PLUS(unit) ? OEMB_LAST_LOOKUP_SLAVE_RESULT_FOUNDf : OEMB_LAST_LOOKUP_SLAVE_FOUNDf;
        nof_last_lookup_regs = 2;
    } else {
        last_lookup_regs[0] = IHB_OEMB_LAST_LOOKUPr;
