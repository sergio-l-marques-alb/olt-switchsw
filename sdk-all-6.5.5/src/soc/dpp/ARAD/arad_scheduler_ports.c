  ...
  )
{
  ...
  if (!is_port_hr_val)
  {
      reg_idx = ARAD_REG_IDX_GET(base_port_tc, ARAD_SCH_PORT_NOF_PORTS_PER_FORCE_FC_REG_LINE);

      SOC_SAND_SOC_IF_ERROR_RETURN(res, 900, exit, READ_SCH_FORCE_PORT_FC_REGISTERr(unit, core, reg_idx, &fld_val));
      
      start_idx = ARAD_FLD_IDX_GET(base_port_tc, ARAD_SCH_PORT_NOF_PORTS_PER_FORCE_FC_REG_LINE);
      ...
      for (priority_i = 0; priority_i < nof_priorities; priority_i++)
      {
        ...
        SOC_SAND_SET_BIT(fld_val, 1, start_idx+priority_i);
      }
  ...
  if (is_port_hr_val)
  {
      ...
      {    
        /* No coverity here, the shift value can't be greater than 32 */
        /* coverity[large_shift:FALSE] */ 
        SOC_SAND_SET_BIT(fld_val, 0, start_idx+priority_i);
      }
