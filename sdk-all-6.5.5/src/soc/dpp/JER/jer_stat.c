    ...
    if(!IS_SFI_PORT(unit,port)) {
        ...
        COMPILER_64_ZERO(*val);
            ...
            case soc_jer_counters_rx_eth_stats_drop_events:
                if ((!IS_IL_PORT(unit,port) || (IS_IL_PORT(unit,port) && SOC_IS_QAX(unit))) && !IS_QSGMII_PORT(unit, port)) {
                    SOC_DPP_ALLOW_WARMBOOT_WRITE(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_prd_drop_count_get,(unit, port, &val_32)), rv); 
                    SOCDNX_IF_ERR_EXIT(rv);
...
soc_jer_stat_controlled_counter_enable_get(int unit, soc_port_t port, int index, int *enable, int *printable) {
    ...
    *printable = 1;
    ...
    } else if (IS_IL_PORT(unit, port))
    {
        ...
        /*ILKN*/
        if (!(soc->controlled_counters[index].flags & flags) && !SOC_IS_QAX(unit)) 
        {
            *enable = 0;
        }
    }
