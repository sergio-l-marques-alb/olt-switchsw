...
soc_jer_egr_interface_alloc(int unit, soc_port_t port) 
{
    ...
    if((master_port == port) || SOC_PORT_IS_LB_MODEM(flags)) { /* First port in interface */
        ...
        if (ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON && interface_type == SOC_PORT_IF_ILKN) {
            SOC_SAND_SET_BIT(egr_if, 0x0, 0); /*ILKN tdm interleave egr interface*/
            if (!(IS_TDM_PORT(unit, port) || is_tdm_queuing))
            {
...
#define SOC_JER_CAL_LANES_OTHERS   4 /*Lanes*/
#define SOC_JER_CAL_LANES_OLP      1 /*Lanes*/
#define SOC_JER_CAL_LANES_OAMP     1 /*Lanes*/
#define SOC_JER_CAL_LANES_CPU      1 /*Lanes*/
#define SOC_JER_CAL_LANES_RCY      1 /*Lanes*/
#define SOC_JER_CAL_LANES_SAT      4 /*Lanes*/
#define SOC_JER_CAL_LANES_IPSEC    4 /*Lanes*/
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->link_bonding_enable) 
    {
        ...
        if (!(is_special_int)) 
        {
            ...
            /* get first lane */
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit, port, &first_lane));
            /* get shifted lane id to check if it is falcon or eagle*/
            rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_qsgmii_offsets_remove, (unit, first_lane, &shifted_lane));
            --shifted_lane;
            if(unit > BCM_MAX_NUM_UNITS) {
                SOCDNX_EXIT_WITH_ERR_NO_MSG(SOC_E_UNIT);
            }
    ...
    */
    SOCDNX_IF_ERR_EXIT(arad_ofp_rates_fixed_len_cal_build(unit, lanes, dummy_start + nof_padding_if + 1, calendar_length, SOC_JER_MAX_CAL_LENGTH, 1, calendar->slots));
