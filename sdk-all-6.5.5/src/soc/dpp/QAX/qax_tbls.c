...
int soc_qax_tbls_init(int unit)
{
    ...
    soc_jer_tbls_zero_init(unit, qax_tbls_excluded_mem_list);
        
    SOCDNX_IF_ERR_EXIT(arad_tbl_access_init_unsafe(unit)); /* init arad_fill_table_with_variable_values_by_caching() */
    SOCDNX_IF_ERR_EXIT(soc_jer_control_dynamic_mem_writes(unit, qax_dynamic_mem_enable_regs, 1)); /* enable dynamic memory writes */
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_control_dynamic_mem_writes(unit, qax_dynamic_mem_disable_regs, 0)); /* disable dynamic memory writes */
