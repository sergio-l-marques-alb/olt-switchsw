/*
 * $Id: bigmac.c,v 1.46 Broadcom SDK $
...
_mac_big_init(int unit, soc_port_t port)
{
...
#if defined(BCM_BRADLEY_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (soc_feature(unit, soc_feature_higig2) && IS_HG_PORT(unit, port) &&
        soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE,
                              SOC_IS_SIRIUS(unit) ? 1 : 0))  {
	/* sirius default to higig2 mode for higig interface */
     ...
     * Sirius will use the default values.
     */

#if defined(BCM_SIRIUS_SUPPORT)
    if (SOC_IS_SIRIUS(unit)) {
       soc_reg64_field32_set(unit, MAC_RXCTRLr, &rx_ctrl, STRIPCRCf, 1);
