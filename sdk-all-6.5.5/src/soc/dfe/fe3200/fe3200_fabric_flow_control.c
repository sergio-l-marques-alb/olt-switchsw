/*
 * $Id: fe3200_fabric_flow_control.c,v 1.8.24.2 Broadcom SDK $
 *
...
soc_fe3200_fabric_links_link_type_set(int unit, soc_dcmn_fabric_pipe_t pipe, soc_dfe_fabric_link_fifo_type_index_t type_index, 
    ...
    /* is_fe1, is_fe3 flags are irrelevant, haven't removed them to be compatible with fe1600 function */
    int i,blk_id;
    ...
    for (i = 0; i<links_count ; i++)
    {
        ...
        /*Setting DCH and DCL to the same value*/
                                          ...
                                          int is_rx, int is_tx, int is_fe1, int is_fe3, uint32 links_count_max, soc_port_t* links, uint32* links_count)
{
    ...
    /*Assuming link types over DCH and DCL are synced (checking DCH only*/
    for (blk_id = 0; blk_id < SOC_DFE_DEFS_GET(unit, nof_instances_dch); blk_id++)
    {
...
soc_error_t soc_fe3200_fabric_flow_control_tx_bypass_llfc_threshold_set(int unit,soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int fe1, int fe3, int value)
{
    ...
    /*if asymmetric fe13: in DCL 2/3 (here do it for all DCLs: 0-3) we mark DCM0/1 as DCMB (The DCM which asymmetrical traffic comes from)*/
    /* quads 26,35 - the quads moved from FE to FAP in fe13, look at DCMB registers to determine their threshold*/
        ...
        for (i=0; i < nof_pipes ;i++)
        {
            if (SOC_DCMN_FABRIC_PIPE_IS_SET(pipe_lcl, i))
            {
                ...
                else
                {
                    ...
                    SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, DCL_DCMB_TYPE_1_GCI_TH_Pr, idx, i, dcl_reg_val));
                }
            }
        }
    }
...
soc_error_t soc_fe3200_fabric_flow_control_tx_rci_threshold_get(int unit, bcm_fabric_link_threshold_type_t type, soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int fe1, int fe3, int* value)
{
    ...
    *pipe_lcl = pipe;
    ...
    for (i=0 ; i < nof_pipes; i++)
    {
        ...
        break; /* value is already set, no need to keep looping*/
        }
    }
    ...
    *pipe_lcl = pipe;
    ...
    {    
        ...
        { 
            ...
            break; /* value is set, no need to keep looping */
        }
    }
...
soc_error_t soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_validate(int unit, soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int value)
{
    ...
    /* validate that value is divisble by 4*/
    if (value % 4 != 0)
    {
...
soc_error_t soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(int unit, soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int is_fe1, int is_fe3, int value, int to_check_counter_overflow)
{
    ...
    /* set an enviorment for when they fix 3rd pipe issues */
...
soc_error_t soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_get(int unit, soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int is_fe1, int* value)
{
    ...
    else if (SOC_DCMN_FABRIC_PIPE_IS_SET(&pipe,2))
    {
        ...
        *value = SOC_FE3200_DCM_FIFO_SIZE_BUFFER_SIZE - fifo_size_pipe_0 - fifo_size_pipe_1; /* fifo size for pipe 2 is derived from buffer size minus pipe 0 fifo size minus pipe 1 fifo size */
        if (*value < 0)
        {
            *value = 0;
        }
...
/* This function checks that there is no violation of thresholds combination
 * Currently checks combination of: Tx fifo size, Mid fifo size and TX->Mid flow control */
STATIC soc_error_t
soc_fe3200_fabric_flow_control_mid_tx_prevent_counter_per_link_overflow (int unit, soc_dfe_fabric_link_fifo_type_index_t type_index, soc_dcmn_fabric_pipe_t pipe, int fe1, int fe3)
{
    ...
    /*  FE3200 CIG errata - load balancing wraparound.
        if user enables DCL->DCM flow control, they should set DCM (mid) fifo size and drop thresholds accordingly to avoid following situation:
        if DCL[per link] + DCM[x4] are full with traffic dedicated for one link, the counter sent from CIG to RTP for this link will overflow
        (happens if traffic for one link > 4095 fragments) and will look empty => RTP will send more traffic for this link.
        This is relevant to case of: 1-pipe, FE2, DCL->DCM flow control enabled   */
    if ((nof_pipes == 1) && SOC_DCMN_FABRIC_PIPE_IS_SET(&pipe, 0) && SOC_DFE_IS_FE2(unit)) {
        ...
        /*if DCL->DCM flow control is enabled*/
        if (dcl_almost_full_threshold <= dcl_fifo_size) {
            ...
            /*size of counter passed from CIG to CGM is 12 bits => max value is 4095*/
