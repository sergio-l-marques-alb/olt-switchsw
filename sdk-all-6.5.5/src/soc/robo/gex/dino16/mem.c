    ...
    if (flags & DRV_MEM_OP_SEARCH_DONE) {
        ...
        for (count = 0; count < SOC_TIMEOUT_VAL; count++) {
            ...
            if ((valid == 1) && (!COMPILER_64_IS_ZERO(entry0))) {
                ...
                /* MAC Address */
                soc_ARLA_SRCH_RSLT_MACVID_0r_field_get(unit, (void *)&entry0, 
            ...
            if ((valid1 == 1) && (!COMPILER_64_IS_ZERO(entry1))) {
                ...
                /* MAC Address */
                soc_ARLA_SRCH_RSLT_MACVID_1r_field_get(unit, (void *)&entry1, 
                    ARLA_SRCH_MACADDRf, (void *)&temp_mac_field);
                /* VLAN ID */
                soc_ARLA_SRCH_RSLT_MACVID_1r_field_get(unit, (void *)&entry1, 
                    ARLA_SRCH_RSLT_VIDf, &vid_rw);
    ...
    /* delete by MAC */    
        ...
        for (i = 0; i < ROBO_DINO_L2_BUCKET_SIZE; i++) {
            ...
            *index = i;
            ...
            COMPILER_64_ZERO(temp_mac_field);
            soc_ARLA_MACVID_ENTRY0r_field_get(unit, (void *)mac_vid, 
                ARL_MACADDRf, (void *)&temp_mac_field);
            SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, temp_mac_field);
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {        
        ...
        /* get result */
        if ((rv = REG_READ_ARLA_VTBL_ENTRYr(unit, (void *)&reg_v64)) < 0) {
            goto mem_read_exit;
        }

        soc_ARLA_VTBL_ENTRYr_field_get(unit, (void *)&reg_v64, 
            VALID_Rf, &temp);
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        soc_ARLA_VTBL_ENTRYr_field_set(unit, (void *)&reg_v64, 
            VALID_Rf, &temp);
        ...
        if ((rv = REG_WRITE_ARLA_VTBL_ENTRYr(unit, (void *)&reg_v64)) < 0) {
                goto mem_write_exit;
        }
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mem_id == INDEX(L2_ARLm)) {
            ...
            DRV_REG_FIELD_GET(unit, reg_low, (void *)&mem_data0, 
            ...
            DRV_REG_FIELD_GET(unit, reg_low, (void *)&mem_data0, 
                PRIORITY_Rf, &temp);                    
            if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, 
                DRV_MEM_FIELD_PRIORITY, (void *)arl_mem, &temp)) < 0) {
                goto mem_read_exit;
            }
            ...
            DRV_REG_FIELD_GET(unit, reg_high, (void *)&mem_data1, 
                AGEf, &temp);
            if ((rv = DRV_MEM_FIELD_SET(unit, DRV_MEM_ARL, 
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mem_id == INDEX(L2_ARLm)) {
            ...
            DRV_REG_FIELD_SET(unit, reg_low, (void *)&mem_data0, 
                MACADDRf, (void *)&temp_mac_field);                           
            SAL_MAC_ADDR_FROM_UINT64(temp_mac_addr, temp_mac_field);            
            ...
            DRV_REG_FIELD_SET(unit, reg_low, (void *)&mem_data0, VID_Rf, 
                (uint32 *)&temp); 
            ...
            DRV_REG_FIELD_SET(unit, reg_high, (void *)&mem_data1, 
                AGEf, &temp);
            ...
            if ((rv = REG_WRITE_MEM_FRM_DATA0r
                (unit, (void *)&mem_data0)) < 0) {
                goto mem_write_exit;
            }              
            if ((rv = REG_WRITE_MEM_FRM_DATA1r
                (unit, (void *)&mem_data1)) < 0) {
                goto mem_write_exit;
            }
            arl_mem ++;
        } 
