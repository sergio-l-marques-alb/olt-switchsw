...
#define _SF3_WRED_PROFILE_ZERO_RESERVED         (1)

/* Invalid default wred profile index */
#define _SF3_DEFAULT_WRED_PROFILE_INVALID       (-1)
...
STATIC uint32   sf3_wred_usage;
STATIC drv_wred_config_t sf3_drv_wred[DRV_SF3_WRED_NUM];
STATIC int      sf3_wred_default_index;
STATIC int      sf3_wred_profile_index[SOC_ROBO_MAX_NUM_PORTS][DRV_SF3_COS_QUEUE_NUM][4];

#define _DRV_SF3_WRED_TABLE_OP_READ     (0)
#define _DRV_SF3_WRED_TABLE_OP_WRITE    (1)

/* Get the maximum cells of  the queue */ 
STATIC int
_drv_sf3_queue_max_cell_get(int unit, soc_port_t port, int queue, 
            uint32 *cell)
{
...
STATIC int
_drv_sf3_wred_map_set(int unit, uint32 wred_idx, drv_wred_map_info_t *map)
{
    int i, j, port;
            ...
            if (tc_map & (0x1 << i)) {
                for (j = 0; j < 2; j++) {
                    if (flow_mark_map & (0x1 << j)) {
                        ...
                        temp = _DRV_SF3_WRED_TABLE_OP_WRITE;
                        soc_TC2RED_PROFILE_TABLEr_field_set(unit, &reg_val,
                            TC2RED_TABLE_WR_RDf, &temp);
                        SOC_IF_ERROR_RETURN(
                            REG_WRITE_TC2RED_PROFILE_TABLEr(unit, &reg_val));
                        /* Remove the old profile index */
                        old_idx = sf3_wred_profile_index[port][i][(dei << 1 | j)];
                        if (sf3_wred_usage & (0x1 << old_idx)) {
                            sf3_drv_wred[old_idx].refer_count--;
                        }
                        /* record the new profile index value */
                        sf3_wred_profile_index[port][i][(dei << 1 | j)] = 
                            wred_idx;
                        sf3_drv_wred[wred_idx].refer_count++;
...
STATIC int
_drv_sf3_wred_config_set(int unit, int index,
                    int cosq, drv_wred_config_t *config)
{
    uint32  reg_val, fld_val;
    uint32  cell;
    int i;

    if (index >= DRV_SF3_WRED_NUM) {
        return SOC_E_PARAM;
    }
    ...
    if (index != sf3_wred_default_index) {

        /* Check if max > queue max depth */
        SOC_IF_ERROR_RETURN(
            _drv_sf3_queue_max_cell_get(unit, -1, cosq, &cell));
        cell *= DRV_SF3_CELL_UNIT; 
    ...
    for (i = MAX_HW_DROP_VALUE; i >= 0; i--) {
        if (config->drop_prob >= _drv_sf3_max_drop_prob_table[i]) {
            break;
        }
    }
    fld_val = i;
    ...
    /* MAX threshold */
    fld_val = config->max_threshold / DRV_SF3_CELL_UNIT;
    soc_RED_PROFILE_Nr_field_set(unit, &reg_val, RED_MAX_THDf, &fld_val);

    /* MIN Threshold */
    fld_val = config->min_threshold / DRV_SF3_CELL_UNIT;
    ...
    SOC_IF_ERROR_RETURN(
        drv_sf3_wred_control_get(unit, -1, -1,
            DRV_WRED_CONTROL_AQD_EXPONENT, &fld_val));
    if (fld_val != config->gain) {
        fld_val = config->gain;
        SOC_IF_ERROR_RETURN(
            drv_sf3_wred_control_set(unit, -1, -1,
                DRV_WRED_CONTROL_AQD_EXPONENT, fld_val));
    }
...
STATIC int
_drv_sf3_wred_default_index_set(int unit, int index)
{
    uint32 reg_val, fld_val;
    if (index >= DRV_SF3_WRED_NUM) {
        return SOC_E_PARAM;
...
int
drv_sf3_wred_init(int unit)
{
#if _SF3_WRED_PROFILE_ZERO_RESERVED
    uint32 reg_val = 0, cell_num = 0;
#endif /* _SF3_WRED_PROFILE_ZERO_RESERVED */
    ...
    sf3_wred_usage = 0;
    sal_memset(sf3_drv_wred, 0, 
        (sizeof(drv_wred_config_t) * DRV_SF3_WRED_NUM));
    sf3_wred_default_index = _SF3_DEFAULT_WRED_PROFILE_INVALID;
    sal_memset(sf3_wred_profile_index, 0, (sizeof(sf3_wred_profile_index)));
#if _SF3_WRED_PROFILE_ZERO_RESERVED
    ...
    SOC_IF_ERROR_RETURN(
        _drv_sf3_queue_max_cell_get(unit, 0, 0, &cell_num));
    soc_RED_PROFILE_Nr_field_set(unit, &reg_val, RED_MAX_THDf, &cell_num);
    SOC_IF_ERROR_RETURN(
        REG_WRITE_RED_PROFILE_Nr(unit, 0, &reg_val));
    sf3_drv_wred[0].refer_count = (SOC_ROBO_MAX_NUM_PORTS * 
        DRV_SF3_COS_QUEUE_NUM * 4);
#endif /* _SF3_WRED_PROFILE_ZERO_RESERVED */
...
int
drv_sf3_wred_config_create(int unit, uint32 flags, 
            drv_wred_config_t *config, int *wred_id)
{
    ...
    for (i = 0; i < DRV_SF3_WRED_NUM; i++) {
        if (!(sf3_wred_usage & (0x1 << i))) {
            continue;
        }
        
        if ((sf3_drv_wred[i].drop_prob == config->drop_prob) &&
            (sf3_drv_wred[i].max_threshold == config->max_threshold) &&
            (sf3_drv_wred[i].min_threshold == config->min_threshold)) {
            *wred_id = i;
            found = TRUE; 
            break;
        }
    ...
    if (!found) {
        for (i = 0; i < DRV_SF3_WRED_NUM; i++) {
            if (!(sf3_wred_usage & (0x1 << i))) {
                sf3_wred_usage |= (0x1 << i);
                break;
            }
        }

        if (i == DRV_SF3_WRED_NUM) { 
            return SOC_E_RESOURCE;
        }

        sal_memcpy(&(sf3_drv_wred[i]), config, sizeof(drv_wred_config_t));
    } 
    ...
    if (config->flags & DRV_WRED_CONFIG_FLAGS_DEFAULT_PROFILE) {
        sf3_wred_default_index = i;
        SOC_IF_ERROR_RETURN(
            _drv_sf3_wred_default_index_set(
                        unit, sf3_wred_default_index));
    }
    ...
    sf3_drv_wred[i].hw_index = i;
    sf3_drv_wred[i].refer_count = 0;

    /* Write the configuration back */
    sal_memcpy(config, &(sf3_drv_wred[i]), sizeof(drv_wred_config_t));
...
int
drv_sf3_wred_config_set(int unit, int wred_id, drv_wred_config_t *config)
{
    drv_wred_config_t       *drv_config;
    if ((config == NULL) || (wred_id >= DRV_SF3_WRED_NUM))  {
        return SOC_E_PARAM;
    }

    if ((sf3_wred_usage & (0x1 << wred_id)) == 0) {
        return SOC_E_PARAM;
    }

    drv_config = &sf3_drv_wred[wred_id];
    ...
    if (config->flags & DRV_WRED_CONFIG_FLAGS_DEFAULT_PROFILE) {
        if (sf3_wred_default_index != wred_id) {
            sf3_wred_default_index = wred_id;
            SOC_IF_ERROR_RETURN(
                _drv_sf3_wred_default_index_set(
                            unit, sf3_wred_default_index));
        }
    ...
    SOC_IF_ERROR_RETURN(
        _drv_sf3_wred_config_set(unit, wred_id, -1, config));
...
int
drv_sf3_wred_config_get(int unit, int wred_id, drv_wred_config_t *config)
{
    if ((config == NULL) || (wred_id >= DRV_SF3_WRED_NUM))  {
        return SOC_E_PARAM;
    }
    ...
    sal_memcpy(config, &sf3_drv_wred[wred_id], sizeof(drv_wred_config_t));
    return SOC_E_NONE;
}
...
int
drv_sf3_wred_config_destroy(int unit, int wred_id)
{
    drv_wred_config_t       *drv_config;
    uint32      reg_val, fld_val;
    
    if (wred_id >= DRV_SF3_WRED_NUM)  {
        return SOC_E_PARAM;
    }
    ...
    sf3_wred_usage &= ~(0x1 << wred_id);

    /* Invalid Default WRED index */
    if (wred_id == sf3_wred_default_index) {
        sf3_wred_default_index = _SF3_DEFAULT_WRED_PROFILE_INVALID;
    }
    ...
    SOC_IF_ERROR_RETURN(
        _drv_sf3_queue_max_cell_get(unit, 0, 0, &fld_val));
    soc_RED_PROFILE_Nr_field_set(unit, &reg_val, RED_MAX_THDf, &fld_val);
    SOC_IF_ERROR_RETURN(
...
int
drv_sf3_wred_map_attach(int unit, int wred_id, drv_wred_map_info_t *map)
{
    drv_wred_config_t       *drv_config;
    int                     rv = SOC_E_NONE;
    ...
    if (drv_config->refer_count == 0) {
        ...
        SOC_IF_ERROR_RETURN(
            _drv_sf3_wred_config_set(unit, wred_id, map->cosq, drv_config));
    } 

    /* Configure the WRED mapping table */
    SOC_IF_ERROR_RETURN(
        _drv_sf3_wred_map_set(unit, drv_config->hw_index, map)); 

    /* Enable per-port WRED function */
    rv = drv_sf3_wred_control_set(unit, map->port, -1,
...
int
drv_sf3_wred_map_deattach(int unit, int wred_id, drv_wred_map_info_t *map)
{
#ifndef _SF3_WRED_PROFILE_ZERO_RESERVED    
    int                     i, found;
    uint32                  wred_idx;
#endif /* _SF3_WRED_PROFILE_ZERO_RESERVED */    
...
#if _SF3_WRED_PROFILE_ZERO_RESERVED
    SOC_IF_ERROR_RETURN(
        _drv_sf3_wred_map_set(unit, 0, map));
#else /* !_SF3_WRED_PROFILE_ZERO_RESERVED */
    found = FALSE;
    for (i = (DRV_SF3_WRED_NUM - 1); i >= 0; i--) {
        if (!(sf3_wred_usage & (0x1 << i))) {
            found = TRUE;
            wred_idx = i;
            break;
    ...
    if (found) {
        return (_drv_sf3_wred_map_set(unit, wred_idx, map));
    }
#endif /* _SF3_WRED_PROFILE_ZERO_RESERVED */    
...
int
drv_sf3_wred_map_get(int unit, int *wred_id, drv_wred_map_info_t *map)
{
    ...
    if (map->flags & DRV_WRED_CONFIG_FLAGS_DEFAULT_PROFILE) {
        ...
        if (_SF3_DEFAULT_WRED_PROFILE_INVALID != sf3_wred_default_index) {
            *wred_id = sf3_wred_default_index;
            return SOC_E_NONE;
        }
    ...
    idx = sf3_wred_profile_index[port][cosq][(dei << 1 | flow_mark)]; 
#if _SF3_WRED_PROFILE_ZERO_RESERVED
    if (idx == 0) {
        return SOC_E_NOT_FOUND;
    }
...
int
drv_sf3_wred_control_set(int unit, soc_port_t port, int queue, 
            drv_wred_control_t type, uint32 value)
{
    ...
    switch (type) {
        ...
        case DRV_WRED_CONTROL_AQD_PERIOD:
            if (value > DRV_SF3_WRED_MAX_AQD_PERIOD) {
                return SOC_E_PARAM;
            }
            SOC_IF_ERROR_RETURN(
                REG_READ_RED_AQD_CONTROLr(unit, &reg_val));
            fld_val = value / DRV_SF3_WRED_AQD_PERIOD_UNIT;
...
int
drv_sf3_wred_control_get(int unit, soc_port_t port, int queue, 
            drv_wred_control_t type, uint32 *value)
{
    uint32  reg_val, fld_val = 0;
    ...
    switch (type) {
        ...
        case DRV_WRED_CONTROL_AQD_PERIOD:
            ...
            soc_RED_AQD_CONTROLr_field_get(unit, &reg_val, 
                AQD_PERIODf, &fld_val);
            *value = fld_val * DRV_SF3_WRED_AQD_PERIOD_UNIT;
            break;
        case DRV_WRED_CONTROL_AQD_EXPONENT:
            SOC_IF_ERROR_RETURN(
                REG_READ_RED_EXPONENTr(unit, &reg_val));
        ...
        case DRV_WRED_CONTROL_MAX_QUEUE_SIZE:
            ...
            SOC_IF_ERROR_RETURN(
                _drv_sf3_queue_max_cell_get(unit, port, queue, &fld_val));
            *value = fld_val * DRV_SF3_CELL_UNIT;
            break;
        default:
            return SOC_E_UNAVAIL;
    }
...
int
drv_sf3_wred_counter_enable_set(int unit, soc_port_t port, int queue,
...
int
drv_sf3_wred_counter_enable_get(int unit, soc_port_t port, int queue,
    drv_wred_counter_t type, int *enable) {
...
int
drv_sf3_wred_counter_set(int unit, soc_port_t port, int queue,
...
int
drv_sf3_wred_counter_get(int unit, soc_port_t port, int queue,
        drv_wred_counter_t type, uint64 *value)
{
    uint32 reg_val;
