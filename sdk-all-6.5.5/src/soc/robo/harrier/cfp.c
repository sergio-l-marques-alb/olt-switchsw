...
_drv_harrier_cfp_tcam_fill(int unit, uint32 data)
{
    ...
    for (i = 0; i < mem_len; ) {
        if ((rv = REG_WRITE_CFP_DATAr(unit, (i/2), (void *)&reg_val64)) < 0) {
...
_drv_harrier_cfp_read(int unit, uint32 ram_type, uint32 index, drv_cfp_entry_t *entry)
{
    ...
    case DRV_CFP_RAM_TCAM:
        ...
        break;
    case DRV_CFP_RAM_ACT:
        if ((rv = REG_READ_ACT_POL_DATAr(unit, (void *)&reg_val64)) < 0) {
            goto cfp_read_exit;
        }
...
_drv_harrier_cfp_write(int unit, uint32 ram_type, uint32 index, drv_cfp_entry_t *entry)
{
    ...
    case DRV_CFP_RAM_ACT:
        ...
        COMPILER_64_SET(reg_val64, reg_val32_hi, reg_val32_lo);
        if ((rv = REG_WRITE_ACT_POL_DATAr(unit, (void *)&reg_val64)) < 0) {
            goto cfp_write_exit;
        }
        break;
    case DRV_CFP_RAM_METER:
            ...
            uint32 *param2)
{
    ...
    switch (control_type) {
        case DRV_CFP_ENABLE:
            ...
            if ((rv = REG_READ_CFP_EN_CTRLr(unit, (void *)&reg_64)) < 0) {
                return rv;
            }
            soc_CFP_EN_CTRLr_field_get(unit, (void *)&reg_64,
            ...
            uint32 param2)
{
    ...
    switch (control_type) {
        case DRV_CFP_ENABLE:
            ...
            soc_CFP_EN_CTRLr_field_set(unit, (void *)&reg_64,
                EN_CFP_MAPf, (uint32 *)&fld_32);

            if ((rv = REG_WRITE_CFP_EN_CTRLr(unit, (void *)&reg_64)) < 0) {
                return rv;
            }
            break;
        case DRV_CFP_FLOOD_TRUNK:
