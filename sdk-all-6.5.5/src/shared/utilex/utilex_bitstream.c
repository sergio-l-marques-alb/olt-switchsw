  ...
  uint32 nof_bits)
{
  ...
  for (idx = start_bit_aligned; idx < start_bit_aligned + nof_bits; ++idx)
  {
    ...
    parity =
      (~((~parity) ^ (SAL_BIT(idx % UTILEX_NOF_BITS_IN_UINT32) & buffer))) & SAL_BIT(idx % UTILEX_NOF_BITS_IN_UINT32);
  }
  ...
  uint32 silent)
{
  ...
  utilex_bitstream_get_field(bit_stream, 8, 12, &field);
  if (field != 0x234)
  {
    pass &= 0;
  }
  ...
  )
{
  ...
  for (line_i = 0; line_i < not_lines; ++line_i)
  {
    LOG_CLI((BSL_META("[%4u-%4u]"),
             UTILEX_MIN((line_i + 1) * nof_longs_per_line * (int) UTILEX_NOF_BITS_IN_UINT32 - 1,
                        size * (int) UTILEX_NOF_BITS_IN_UINT32 - 1),
             line_i * nof_longs_per_line * (int) UTILEX_NOF_BITS_IN_UINT32));
    for (long_i = UTILEX_MIN((line_i + 1) * nof_longs_per_line - 1, size - 1);
  ...
  uint32 * output_buffer)
{
  ...
  else
  {
    ...
    {
      nof_bits_iter = nof_bits_iter_acc > UTILEX_NOF_BITS_IN_UINT32 ? UTILEX_NOF_BITS_IN_UINT32 : nof_bits_iter_acc;

      res = utilex_bitstream_set_field(output_buffer,
                                       start_bit + iter * UTILEX_NOF_BITS_IN_UINT32,
                                       nof_bits_iter, *(input_buffer + iter));
      SHR_IF_ERR_EXIT(res);
    }
  }
  ...
  uint32 * output_buffer)
{
  ...
  if ((end / UTILEX_NOF_BITS_IN_UINT32) == (start_bit / UTILEX_NOF_BITS_IN_UINT32))
  {
     ...
     */
    field =
      UTILEX_GET_BITS_RANGE(*(input_buffer + (start_bit / UTILEX_NOF_BITS_IN_UINT32)), end % UTILEX_NOF_BITS_IN_UINT32,
  ...
  else
  {
    for (iter = 0, nof_bits_iter = nof_bits; iter < nof_words; ++iter, nof_bits_iter -= UTILEX_NOF_BITS_IN_UINT32)
    {
      res = utilex_bitstream_get_field(input_buffer,
                                       start_bit + iter * UTILEX_NOF_BITS_IN_UINT32,
                                       nof_bits_iter >
                                       UTILEX_NOF_BITS_IN_UINT32 ? UTILEX_NOF_BITS_IN_UINT32 : nof_bits_iter,
                                       output_buffer + iter);
      SHR_IF_ERR_EXIT(res);
    }
  }
  ...
  uint32 * output_value)
{
  ...
  *output_value = 0;
  ...
  SHR_IF_ERR_EXIT(utilex_bitstream_get_field(tmp_output_value,
                                   buf_sizes - (start_bit_msb % UTILEX_NOF_BITS_IN_BYTE + nof_bits),
                                   nof_bits, output_value)) ;
