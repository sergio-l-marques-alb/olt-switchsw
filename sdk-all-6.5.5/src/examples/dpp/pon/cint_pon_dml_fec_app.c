...
int pon_voip_pselect_setup(int unit, bcm_pbmp_t in_pbmp)
{
    ...
    if (BCM_E_NONE != result)
    {
        ...
        auxRes = bcm_field_presel_destroy(unit, presel_id_voip);
        return result;
    }
  
    result = bcm_field_qualify_Stage(unit, presel_id_voip | BCM_FIELD_QUALIFY_PRESEL, bcmFieldStageIngress);
    if (BCM_E_NONE != result)
    {
    ...
    result = bcm_field_qualify_InPorts(unit, presel_id_voip | BCM_FIELD_QUALIFY_PRESEL, pbm, pbm_mask);
    if (BCM_E_NONE != result)
    {
    ...
    result = bcm_field_qualify_L2DestHit(unit, presel_id_voip | BCM_FIELD_QUALIFY_PRESEL, 0x1, 0x01);
    if (BCM_E_NONE != result)
    {
