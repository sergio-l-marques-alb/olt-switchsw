;
; $Id: sdk56601.asm,v 1.7 Broadcom SDK $
;
...
; 
;
; This is the default program for the (12+1 port) BCM56601 SDKs.
; To start it, use the following commands from BCM:
;
;       led load sdk56601.hex
;       led auto on
;       led start
;
...
;       ONE, ZERO       Green
;
; There are 2 LEDS for the HG/10G port
; There is two bit per higig/10G LED with the following colors:
;       ZERO, ZERO      Black
;       ZERO, ONE       Amber
;       ONE, ZERO       Green
;
; There is padding added for the unused higig port link/activity LEDs
;
; Each chip drives only its own LEDs and needs to write them in the order:
;       A01, L01, L02, A02, ..., L13, A13, PADDING
...
;
; L13 reflects higig/10G link enable/status:
;       Black: no link
;       amber: Enable
;       Green: 10 Gb/s
; A13 reflects port 13 activity
;       Black: idle
;       Green: RX (pulse extended to 1/3 sec)
;       Amber: TX (pulse extended to 1/3 sec, takes precedence over RX)
;       Green/Amber alternating at 6 Hz: both RX and TX
...
up1_5:
        ...
        jnc     no_link
        call    led_green
        call    activity        ; Check activity on this port
        jmp     next_port 
...
up4:
        ...
        call    led_black ; pad unused higig link LED
        call    led_black ; pad unused higig activity LED
