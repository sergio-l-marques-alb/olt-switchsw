...
#define SW_STATE_IS_DEINIT_CHECK() \
    do{ \
        if (SOC_IS_DETACHING(_unit)) {\
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(_unit,\
            "sw state ERROR: trying to free during deinit. for debug, use breakpoint on _sw_state_access_debug_hook\n")));\
            return _SHR_E_DISABLED;\
        }\
    } while(0)
...
#define SW_STATE_OUT_OF_BOUND_CHECK(__index, __max) \
    do{ \
        if (__index > __max) {\
            _sw_state_access_debug_hook(4);\
            LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(_unit,\
            "sw state ERROR: INDEX OUT OF BOUND, trying to access out of array's bounds. for debug, use breakpoint on _sw_state_access_debug_hook\n")));\
            return _SHR_E_NONE;\
        }\
    } while(0)
