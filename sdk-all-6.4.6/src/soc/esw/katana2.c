...
_soc_kt2_l1_port_mx_lane_map_init(int unit)
{
    ...
    for (port = 1; port < COUNTOF(_kt2_l1_port_mx_lane_map); port++) {
        ...
        for (i = 0; i < 4; i++) {
             ...
             mxqblock_i = (*kt2_port_to_mxqblock[unit])[port+i-1];
             ...
             /* coverity[overrun-local : FALSE] */
            _kt2_l1_port_mx_lane_map[port+i].lane_num = lane[i];
        }
...
int _soc_katana2_mmu_init_helper_dyn(int unit, int port, int ext_mem_enable)
{
    ...
    mmu_thdo_qconfig_cell_entry_t       mmu_thdo_qconfig_cell_entry={{0}};
    mmu_thdo_qoffset_cell_entry_t       mmu_thdo_qoffset_cell_entry={{0}};
    uint32        rval = 0, egr_fifo_depth = 0, threshold = 0;
    ...
    BCM_IF_ERROR_RETURN(READ_DEQ_EFIFO_CFGr(unit,port,&rval));
    if (si->port_speed_max[port] <= 1000) {
        ...
        threshold = (IS_EXT_MEM_PORT(unit, port)) ? 12 : 0;
    } else if (si->port_speed_max[port] <= 13000) {
        egr_fifo_depth = (IS_EXT_MEM_PORT(unit, port)) ? 56 : 10;
        threshold = (IS_EXT_MEM_PORT(unit, port)) ? 49 : 0;
    } else {
        egr_fifo_depth = (IS_EXT_MEM_PORT(unit, port)) ? 112 : 16;
        threshold = (IS_EXT_MEM_PORT(unit, port)) ? 85 : 0;
    }
    ...
    BCM_IF_ERROR_RETURN(WRITE_DEQ_EFIFO_CFGr(unit,port,rval));
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (SOC_WARM_BOOT(unit)) {
        /*Note: At warmboot retreival mxqspeeds should not be changed,
         *which will affect speed settings during flex-io operation.
         *Skipping mxqspeeds modification for warmboot case.*/
    } else
#endif
    {
...
soc_katana2_subport_init(int unit)
{
    ...
    if (SOC_PBMP_NOT_NULL(si->linkphy_allowed_pbm)) {
        ...
        */
        ...
        s1_xlp0_index = 0;
        s1_xlp1_index = KT2_MAX_SUBPORTS;
            ...
            if (port < SOC_MAX_NUM_PORTS) {
                ...
                if (lp_block) {
                    if ((num_subports_second_half + num_subport) <=
                         KT2_MAX_LINKPHY_SUBPORTS_PER_PORT) {
...
*/
    ...
    if ((rev_id == BCM56450_B0_REV_ID) || (rev_id == BCM56450_B1_REV_ID)) {
        /* Allow the RQE to use all eight queues on B0 and later devices */
        rval = 0;
             ...
             switch(cfg_num) {
                 ...
                 case BCM56450_DEVICE_ID_OFFSET_CFG + 14:
                          ...
                          sal_memcpy(tdm,
                                     &kt2_tdm_56450A2_ref[0],
                                     sizeof(kt2_tdm_56450A2_ref));
                          kt2_tdm_replace(tdm, tdm_size, 21, KT2_IDLE, 0);
                          kt2_tdm_replace(tdm, tdm_size, 17, KT2_IDLE, 0);
                          break;
                 ...
                 case BCM56450_DEVICE_ID_OFFSET_CFG + 15:
                          ...
                          memcpy(tdm,
                                 ...
                                 sizeof(kt2_tdm_56450A2_ref));
                          kt2_tdm_replace(tdm, tdm_size ,21, KT2_IDLE, 0);
                          kt2_tdm_replace(tdm, tdm_size, 13, KT2_IDLE, 0);
                          kt2_tdm_replace(tdm, tdm_size, 9, 60, 0);
                          tdm_index=0;
                          for (outer_index = 0; outer_index < 2;
                               outer_index++) {
                               for (port = 9; port <= 12 ; port++) {
                                    while(tdm[tdm_index] != 60) {
                                          tdm_index++;
                                    }
                                    tdm[tdm_index] = port;
                               }
                 ...
                 case BCM56452_DEVICE_ID_OFFSET_CFG + 5:
                        LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                                            "kt2_tdm_B_56452_5 \n")));
                        sal_memcpy(tdm,
                                   &kt2_tdm_56452B_ref[0],
                                   sizeof(kt2_tdm_56452B_ref));
                        kt2_tdm_replace(tdm, 90, 1, 60, 0);
                        kt2_tdm_replace(tdm, 90, 5, 60, 0);
                        kt2_tdm_replace(tdm, 90, 26, 60, 0);
                        for (inner_index=1; inner_index <= 4; inner_index++) {
                             for (outer_index=0; outer_index< 6;
                                  outer_index++) {
                                  port = inner_index + outer_index*4;
                                  kt2_tdm_replace(tdm, 90, 60, port, 1);
                             }
                        }
                        kt2_tdm_replace(tdm, 90, 60, KT2_IDLE, 0);
                        break;
...
int _soc_katana2_flexio_scache_retrieve(int unit)
{
    ...
    if (is_upgrade) {
        if ((bcm56450_tdm_info[cfg_num].tdm_size !=
            kt2_scache_tdm_info.tdm_size) ||
            (bcm56450_tdm_info[cfg_num].row != kt2_scache_tdm_info.row) ||
            (bcm56450_tdm_info[cfg_num].col != kt2_scache_tdm_info.col)) {
            LOG_WARN(BSL_LS_SOC_COMMON,
                       (BSL_META_U(unit,
    ...
    if (recover_default_tdm) {
        ...
        _soc_katana2_default_tdm_recover(unit, (uint32 *) kt2_scache_tdm);
        kt2_tdm_display(unit, (uint32 *) (&kt2_scache_tdm[0]),
                        kt2_scache_tdm_info.tdm_size,
                        kt2_scache_tdm_info.row,
                        kt2_scache_tdm_info.col);
    ...
    scache_offset += (sizeof(kt2_scache_tdm[0]) * scache_tdm_size);

    sal_memcpy(&kt2_scache_speed, &flexio_scache_ptr[scache_offset],
               sizeof(kt2_scache_speed));
