                           ...
                           soc_field_t *field_list)
{
    ...
    case ING_VP_VLAN_MEMBERSHIPm:
        ...
        if (soc_feature(unit, soc_feature_robust_hash)) {
            if (ROBUSTHASH(unit)->ing_vp_vlan_member.enable == 1) {
                if ((val_bits % 8) != 0) {
                    byte_align = 0;
                }
            }
     ...
     case EGR_VP_VLAN_MEMBERSHIPm:
         ...
         */
        if (soc_feature(unit, soc_feature_robust_hash)) {
            if (ROBUSTHASH(unit)->egr_vp_vlan_member.enable == 1) {
                if ((val_bits % 8) != 0) {
                    byte_align = 0;
                }
            }
...
soc_td2_ing_vp_vlan_member_entry_hash(int unit, int hash_sel, int bank, uint32 *entry)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        if (ROBUSTHASH(unit)->ing_vp_vlan_member.enable == 1) {
            SOC_IF_ERROR_RETURN
                (soc_robust_hash_get(unit, &(ROBUSTHASH(unit)->ing_vp_vlan_member),
                                     bank, key, key_nbits));
...
soc_td2_egr_vp_vlan_member_entry_hash(int unit, int hash_sel, int bank, uint32 *entry)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        if (ROBUSTHASH(unit)->egr_vp_vlan_member.enable == 1) {
            SOC_IF_ERROR_RETURN
                (soc_robust_hash_get(unit, &(ROBUSTHASH(unit)->egr_vp_vlan_member),
                                     bank, key, key_nbits));
            key_nbits += (key_nbits % 8) ? 4: 0;
        }
        key_nbits += ROBUST_HASH_KEY_SPACE_WIDTH;
