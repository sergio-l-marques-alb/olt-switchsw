...
{
    uint32 rval, fval, port_rename = 0;
     ...
     * - Port is a XE port and we are setting speed to 100G
     */
    if (IS_CL_PORT(unit, port) && (si->port_init_speed[port] == 100000) &&
        (speed < 100000)) {
        ...
        port_rename = 1;
               ...
               (si->port_init_speed[port] < 100000) && (speed == 100000)) {
        /* Convert to CE ports */
    ...
    /*
     * Update soc_info to reflect current port speed needed for
     * OS computations
     */
    ...
    if (SOC_PBMP_NOT_NULL(SOC_INFO(unit).oversub_pbm)) {
        ...
        /*
         * Reprogram OS scheduler table to account for port speed change.
         * Not needed for management ports.
         */
        if (!SOC_PBMP_MEMBER(PBMP_MANAGEMENT(unit), port)) {
            ...
            if (lanes_ctrl.class != lanes_ctrl.cur_class) {
                SOC_IF_ERROR_RETURN
...
_soc_tomahawk_tdm_calculation(int unit, int initTime, _soc_tomahawk_tdm_t *tdm)
{
    ...
    if (TRUE == initTime) {
        _chip_pkg.soc_vars.th.cal_universal_en = soc_property_get
                                                     (unit,
                                                      spn_FABRIC_PORT_ENABLE,
                                                      1);
    }
...
_soc_tomahawk_misc_init(int unit)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        if (si->port_speed_max[port] >= 100000 || IS_LB_PORT(unit, port)) {
            fval = (freq == CCLK_FREQ_850MHZ) ? 36 : 44;
        } else if (si->port_speed_max[port] >= 50000) {
            fval = (freq == CCLK_FREQ_850MHZ) ? 23 :27;
        } else if (si->port_speed_max[port] >= 40000) {
            fval = (freq == CCLK_FREQ_850MHZ) ? 19 :25;
        } else if (si->port_speed_max[port] >= 25000) {
            fval = (freq == CCLK_FREQ_850MHZ) ? 15 :16;
        } else if (si->port_speed_max[port] >= 20000) {
            fval = (freq == CCLK_FREQ_850MHZ) ? 16 :18;
        } else {
            fval = (freq == CCLK_FREQ_850MHZ) ? 11 :13;
        }
...
soc_th_port_speed_supported(int unit, soc_port_t port, int speed)
{
    ...
    default:
        ...
        } else if (speed >= 10000) { /* 10G, HG11, 25G, HG27 */
            if (1 != num_lanes) {
                return SOC_E_PARAM;
            }
        } else {
                                     ...
                                     uint32 *port_speed)
{
    ...
    if (TH_FLEXPORT_SPEED_SET_NUM_LANES == lanes_ctrl->lanes) {
        if (lanes_ctrl->speed > SOC_INFO(unit).port_speed_max[port]) {
            return SOC_E_CONFIG;
        } else {
...
_soc_tomahawk_update_ep_credit(int unit, soc_th_port_lanes_t *lanes_ctrl)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_EGR_MMU_CELL_CREDITr(unit, port, rval));
    if ((TH_FLEXPORT_SPEED_SET_NUM_LANES != lanes_ctrl->lanes) &&
        (lanes_ctrl->lanes < lanes_ctrl->cur_lanes)) { /* Ports being added */
        for (i = 0; i < lanes_ctrl->phy_ports_len; i++) {
            port = si->port_p2l_mapping[lanes_ctrl->phy_ports[i]];
            SOC_IF_ERROR_RETURN(WRITE_EGR_MMU_CELL_CREDITr(unit, port, rval));
        }
...
_soc_tomahawk_port_lanes_update_tdm(int unit, soc_th_port_lanes_t *lanes_ctrl)
{
    ...
    if (!SOC_PBMP_MEMBER(si->oversub_pbm, port_base)) {
        ...
        rv = _soc_tomahawk_update_ep_credit(unit, lanes_ctrl);
        if (SOC_FAILURE(rv)){
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "FAILED in"
                                  "_soc_tomahawk_update_ep_credit\n")));
            return rv;
        }
