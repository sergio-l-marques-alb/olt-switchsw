/*
 * $Id: sbZfCaQmBufferState1EntryConsole.c,v 1.3 Broadcom SDK $
...
sbZfCaQmBufferState1Entry_Print(sbZfCaQmBufferState1Entry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmBufferState1Entry:: drop=0x%01x"), (unsigned int)  pFromStruct->m_uDrop));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" crcmode=0x%01x"), (unsigned int)  pFromStruct->m_uCrcMode));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" headerlength=0x%02x"), (unsigned int)  pFromStruct->m_uHeaderLength));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaQmBufferState1Entry:: continuebyte=0x%02x"), (unsigned int)  pFromStruct->m_uContinueByte));
  LOG_WARN(BSL_LS_SOC_COMMON,
...
char *
sbZfCaQmBufferState1Entry_SPrint(sbZfCaQmBufferState1Entry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
