...
int pmOsILKN_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    int nof_lanes, start, index, port_i, num_pipes, i, field_index, reg_index, reverse_i = 0;
    SOC_INIT_FUNC_DEFS;
    ...
    if ((add_info->ilkn_core_id == 0) || (PORTMOD_PORT_FLAGS_ELK_GET(ilkn_data->flags))) {
    ...
    if ((add_info->ilkn_core_id == 1) || (nof_lanes > OS_ILKN_LANES_PER_CORE_TWO_ACTIVE_PORTS)) {
    ...
    if (add_info->ilkn_core_id) {
        num_pipes = (nof_lanes - 1 < OS_ILKN_SLE_MAX_NOF_PIPES / 2) ? nof_lanes - 1 : OS_ILKN_SLE_MAX_NOF_PIPES / 2;
  	} else {
  	    num_pipes = (nof_lanes - 1 < OS_ILKN_SLE_MAX_NOF_PIPES) ? nof_lanes - 1 : OS_ILKN_SLE_MAX_NOF_PIPES;
    }
    start = (add_info->ilkn_core_id) ? OS_ILKN_SLE_MAX_NOF_PIPES / 2 : 0;
	
    _SOC_IF_ERR_EXIT(READ_ILKN_SLE_TX_PIPE_ENABLEr(unit, port, &reg_val)); 
    for (i = start; i < start + num_pipes; ++i) {
