/*
 * $Id: phy522x.c,v 1.56 Broadcom SDK $
...
phy_522x_mdix_set(int unit, soc_port_t port, soc_port_mdix_t mode)
{
    ...
    force_auto = PHY_IS_BCM5248(pc) | PHY_IS_BCM5324(pc) |
                 PHY_IS_BCM5324A1(pc) | PHY_IS_BCM5348(pc) |
                 PHY_IS_BCM53242(pc) |PHY_IS_BCM53262(pc) |
                 PHY_IS_ROBO_EPHY_65(pc);
...
phy_522x_mdix_get(int unit, soc_port_t port, soc_port_mdix_t *mode)
{
    ...
    if ((PHY_IS_BCM5248(pc) || PHY_IS_BCM5324(pc) || PHY_IS_BCM5324A1(pc) ||
        PHY_IS_BCM5348(pc) || PHY_IS_BCM53242(pc) || PHY_IS_BCM53262(pc) ||
        PHY_IS_ROBO_EPHY_65(pc)) &&
        (*mode == SOC_PORT_MDIX_AUTO)) {
                             ...
                             soc_phy_control_t type, uint32 value)
{
    ...
    if (!PHY_IS_ROBO_EPHY_65(pc) && !PHY_IS_BCM5248(pc)) {
        return SOC_E_UNAVAIL;
    }
