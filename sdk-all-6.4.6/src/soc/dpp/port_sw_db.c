...
soc_port_sw_db_port_remove(int unit, soc_port_t port) 
{
    ...
    if (!is_virtual) {
        ...
        if (PORT_PHY_INFO.interface_type == SOC_PORT_IF_CPU) {
            blk = SOC_PORT_IDX_INFO(unit, 0, PORT_INFO_INDEX(PORT_PHY_INFO.interface_type, 0)).blk;
        } else {
...
soc_port_sw_db_free_tm_ports_get(int unit, int core, soc_pbmp_t* tm_ports)
{
    ...
    SOC_PBMP_ITER(ports_bm, port) {
        SOC_PBMP_PORT_REMOVE(*tm_ports, PORT_INFO.tm_port);
    }
