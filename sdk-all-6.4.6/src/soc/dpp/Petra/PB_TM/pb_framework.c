/* $Id: pb_framework.c,v 1.9 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       soc_pb_framework.c
*
* MODULE PREFIX:  soc_pb_framework
*
* FILE DESCRIPTION:
*
* REMARKS:
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PB_GET_ERR_TEXT);
  ...
  if (!err_found)
  {
...
#ifdef PB_PP
    if (error_code >= SOC_SAND_PB_PP_START_ERR_NUMBER)
    {
       soc_sand_linear_find_error(
         (uint32)error_code,
         soc_pb_pp_get_errors_ptr(),&error_desc_element_ptr
       );
    ...
    if (!err_found)
    {
      ...
      soc_sand_linear_find_error(
        (uint32)error_code,
        soc_pb_errors_ptr_get(),&error_desc_element_ptr) ;
  ...
  if (ex != no_err)
  {
    soc_sand_error_handler(ex, "error in soc_pb_get_err_text()", exit_place,0,0,0,0,0) ;
  }
  return ex ;
}
...
*CALLING SEQUENCE:
*  disp_pb_result(result.error_id,current_line_ptr)
*INPUT:
*  SOC_SAND_DIRECT:
  ...
  )
{
  ...
  proc_id = soc_sand_get_proc_id_from_error_word(soc_pb_api_result) ;
  ...
  if (soc_sand_get_error_code_from_error_word(soc_pb_api_result) != SOC_SAND_OK)
  {
    soc_sand_os_printf(
      "\n\r"
      "**>\'%s\' -\n\r"
      ...
      proc_name,soc_pb_api_result, err_name, err_text,
