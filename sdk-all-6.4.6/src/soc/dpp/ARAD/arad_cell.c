...
soc_arad_sr_cell_format(int unit, const vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf) 
{
    ...
    if (SOC_DPP_CONFIG(unit)->arad->init.fabric.is_fe600) {
        ...
        /*VSC256 cell*/
        rc = soc_vsc256_build_header(unit, cell, buf_size_bytes - (ARAD_SR_DATA_CELL_PAYLOAD_LENGTH/8), buf + (ARAD_SR_DATA_CELL_PAYLOAD_LENGTH/(8*sizeof(uint32))));
        SOCDNX_IF_ERR_EXIT(rc);
    }
  ...
  if (is_inband)
  {
    ...
    if (!is_memory) /* Direct command */
    {
      for(i = 0; i < nof_words; ++i) {
        ...
        maddr = soc_reg_addr_get(unit, regs[i], port_or_copyno[i], index[i], TRUE, &block, &at);
