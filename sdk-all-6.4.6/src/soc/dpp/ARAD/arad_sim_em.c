...
CHIP_SIM_EM_BLOCK
Arad_em_blocks[SOC_MAX_NUM_DEVICES][ARAD_EM_TABLE_ID_LAST + 1];


struct
{

  CHIP_SIM_EM_BLOCK* blocks[SOC_MAX_NUM_DEVICES];
            ...
            )
{
  ...
  *success = TRUE;
  ...
  multi_set = &Em.blocks[unit][id].multi_set[0];
  ...
  status = chip_sim_em_key_map_set_entry(
    unit, 
    0, 
    Em.blocks[unit][id].base,
    key_id,
    Em.blocks[unit][id].data_nof_bytes,
    data8
    );
  if (status != OK)
  {
                    ...
                    )
{
  ...
  Em.blocks[unit]  =  em_blocks;


  if(device_type == SOC_SAND_DEV_ARAD) {
    sal_memset(Em.blocks[unit], 0x0, sizeof(CHIP_SIM_EM_BLOCK) * ARAD_EM_TABLE_ID_LAST+1);
    Em.blocks[unit][ARAD_EM_TABLE_ID_LAST].read_result_address = INVALID_ADDRESS;
  }

  for (block_p=Em.blocks[unit]; block_p->read_result_address!=INVALID_ADDRESS; block_p++)
  {
      block_p->base = NULL ;
  }
  ...
  if(device_type == SOC_SAND_DEV_ARAD) {
    ...
    Em.blocks[unit][ARAD_EM_TABLE_LEM].offset = ARAD_CHIP_SIM_LEM_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_LEM].key_size = ARAD_CHIP_SIM_LEM_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_LEM].data_nof_bytes = ARAD_CHIP_SIM_LEM_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_LEM].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_LEM].end_address = ARAD_CHIP_SIM_LEM_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_LEM].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].offset = ARAD_CHIP_SIM_ISEM_A_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].key_size = ARAD_CHIP_SIM_ISEM_A_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].data_nof_bytes = ARAD_CHIP_SIM_ISEM_A_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].end_address = ARAD_CHIP_SIM_ISEM_A_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_A].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].offset = ARAD_CHIP_SIM_ISEM_B_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].key_size = ARAD_CHIP_SIM_ISEM_B_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].data_nof_bytes = ARAD_CHIP_SIM_ISEM_B_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].end_address = ARAD_CHIP_SIM_ISEM_B_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_ISEM_B].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].offset = ARAD_CHIP_SIM_ESEM_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].key_size = ARAD_CHIP_SIM_ESEM_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].data_nof_bytes = ARAD_CHIP_SIM_ESEM_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].end_address = ARAD_CHIP_SIM_ESEM_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_ESEM].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].offset = ARAD_CHIP_SIM_RMAPEM_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].key_size = ARAD_CHIP_SIM_RMAPEM_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].data_nof_bytes = ARAD_CHIP_SIM_RMAPEM_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].end_address = ARAD_CHIP_SIM_RMAPEM_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_RMAPEM].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].offset = ARAD_CHIP_SIM_OEMA_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].key_size = ARAD_CHIP_SIM_OEMA_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].data_nof_bytes = ARAD_CHIP_SIM_OEMA_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].end_address = ARAD_CHIP_SIM_OEMA_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_A].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].offset = ARAD_CHIP_SIM_OEMB_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].key_size = ARAD_CHIP_SIM_OEMB_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].data_nof_bytes = ARAD_CHIP_SIM_OEMB_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].end_address = ARAD_CHIP_SIM_OEMB_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_OAMEM_B].read_result_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].offset = ARAD_CHIP_SIM_GLEM_BASE;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].key_size = ARAD_CHIP_SIM_GLEM_KEY;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].data_nof_bytes = ARAD_CHIP_SIM_GLEM_PAYLOAD;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].start_address = 0;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].end_address = ARAD_CHIP_SIM_GLEM_TABLE_SIZE;
    Em.blocks[unit][ARAD_EM_TABLE_GLEM].read_result_address = 0;
  }
             ...
             )
{
  ...
  for (blk_ndx = 0; blk_ndx < block_cnt; blk_ndx++) {
    ...
    nof_addresses = block_p->end_address - block_p->start_address + 1;
      ...
      multi_set_init_info.wb_var_index = SOC_DPP_WB_ENGINE_VAR_CHIP_SIM_MULTI_SET +
          (blk_ndx * WB_ENGINE_MULTI_SET_INNER_VARS_NUM);
             ...
             )
{
  ...
  for (block_p=Em.blocks[unit]; block_p->read_result_address!=INVALID_ADDRESS; block_p++)
  {
      if (NULL != block_p->base)
      {
        CHIP_SIM_FREE(block_p->base) ;
        soc_sand_multi_set_destroy(unit, &(block_p->multi_set[0]));
      }
      block_p->base = NULL;
...
*****************************************************/
uint32
chip_sim_em_init(uint32 device_id, SOC_SAND_DEVICE_TYPE device_type)
{
  ...
  case SOC_SAND_DEV_PB:
    indirect_blocks  =  Soc_pb_em_blocks[device_id];
    block_cnt = SOC_PB_EM_TABLE_ID_LAST;
    break;
  case SOC_SAND_DEV_ARAD:
    indirect_blocks  =  Arad_em_blocks[device_id];
  ...
  chip_sim_em_init_chip_specifics(device_id, device_type, indirect_blocks);
  
  chip_sim_em_malloc(device_id, indirect_blocks, block_cnt);

  rv = soc_dpp_wb_engine_init_buffer(device_id,SOC_DPP_WB_ENGINE_BUFFER_CHIP_SIM);

  if (rv) {
      SOC_SAND_SET_ERROR_CODE(SOC_PPD_FUNC_UNSUPPORTED_ERR, 15, exit);
  }
