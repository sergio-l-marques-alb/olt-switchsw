  ...
  )
{
  ...
  switch(esem_key->key_type)
  {
    ...
    case ARAD_PP_ESEM_KEY_TYPE_VD_VSI_CVID:
      /* We are not going to change the macros because of such cases */
  ...
  )
{
  ...
  if (SOC_IS_DONE_INIT(unit))
  {
      if (BCM_UNIT_DO_HW_READ_WRITE(unit))
      {
          /* Crash Recovery is on - No need to check for failure: the value wasn't written yet to the HW. it was just stored in journal.*/
          ARAD_PP_DO_NOTHING_AND_EXIT;
      }
  }
