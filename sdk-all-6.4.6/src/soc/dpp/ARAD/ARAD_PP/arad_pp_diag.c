  ...
  )
{
 ...
 /* forwarding lookup forwarding decision*/
  ...
  /*dbg: fer2lbp_eq2_out_lif [15 : 0] */
  if (SOC_IS_JERICHO(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID, 2, 1, 62, 45, 124);
  } else  if (SOC_IS_ARADPLUS(unit)) {
      if (SOC_IS_ARDON(unit)) {
  ...
  )
{
  ...
  SOC_CHECK_DIAG_LAST_PACKET_VALIDITY(ARAD_PP_DIAG_ENCAP_INFO_GET, core_id);
  ...
  )
{
  ...
  *valid = FALSE;
  ...
  else if (db_info->lkup_num == 2)
  {
    ...
    /* is found */
    if (SOC_IS_JERICHO(unit)) {
        ARAD_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID, 7, 0, 47, 0, 5);
    } else {
  ...
  ) 
{
  ...
  SOC_CHECK_DIAG_LAST_PACKET_VALIDITY(ARAD_PP_DIAG_EGRESS_VLAN_EDIT_INFO_GET, core_id);
