    ...
    )
{
    ...
    if(ARAD_TCAM_LOCATION_ENCODING_BANK_LSB_IN_SECOND_UINT8 > 24) {
        location->entry = 0;
    } else {
        location->entry = (((uint32) key[1] & ((1 << (ARAD_TCAM_LOCATION_ENCODING_BANK_LSB_IN_SECOND_UINT8))-1)) << 8);
    }
    location->entry |= key[0];
    if(ARAD_TCAM_LOCATION_ENCODING_BANK_LSB_IN_SECOND_UINT8 >= 32) {
        location->bank_id = 0;
    } else {
    ...
    )
{
  ...
  if(entry_size == ARAD_TCAM_BANK_ENTRY_SIZE_80_BITS) {
      ...
      line[0] = base_address/2; /* lsb is writen to bank 2n + 1*/
      act_action_bitmap_ndx = 1;
  }
  else{ /* 160/320 */ 
  ...
  /* action bit represents which action entry is avaiable, */
    ...
    )
{
  ...
  hash_tbl = arad_sw_db_tcam_db_entry_id_to_location_get(unit);
  ...
  *found = FALSE;
  ...
  if (is_found == TRUE)
  {
      ...
      for (bank_ndx = 0; (bank_ndx < nof_banks) && (is_found == FALSE); bank_ndx++) {
          ...
          bank_bm = arad_sw_db_tcam_db_entries_used_get(unit, tcam_db_id, bank_id);
    ...
    )
{
  ...
  if (is_for_update && insertion_rule_respected) {
      ...
      list = arad_sw_db_tcam_db_priorities_get(unit, tcam_db_id);
      ...
      current  = SOC_SAND_SORTED_LIST_ITER_END(list);
      ...
      found_prev = FALSE;
      while (!priority_found && (current != SOC_SAND_SORTED_LIST_ITER_BEGIN(list))) 
      {
        node_priority = arad_tcam_db_prio_list_priority_value_decode(key_buffer_current);
        /* 
         * Apply the insertion-order rule: 
         *  - Stop the search for the 1st highest node only if
         *  the node priority is higher (and not higher or equal)
         *  that the entry to insert
         */
        if ((arad_tcam_db_block_entry_insertion_rule_respected_get(unit, tcam_db_id, &insertion_rule_respected) && (priority >= node_priority))
            || ((!arad_tcam_db_block_entry_insertion_rule_respected_get(unit, tcam_db_id, &insertion_rule_respected)) && (priority > node_priority)))
        {
            priority_found = TRUE;
            break;
        }
      ...
      found_node = current;
      if (  found_node != SOC_SAND_SORTED_LIST_ITER_BEGIN(list)  ) { 
          found_prev = TRUE;
    ...
    )
{
  ...
  prio_list = arad_sw_db_tcam_db_priorities_get(unit, tcam_db_id);
    ...
    )
{
  ...
  bank_bm = arad_sw_db_tcam_bank_entries_used_get(unit, bank_id, FALSE);
  res = soc_sand_occ_bm_get_next_in_range(
          unit,
          bank_bm,
          start,
  ...
  *place_chosen = *place_first_free;
  ...
  if ((*found) && (!is_inserted_top)) {
      ...
      bank_bm = arad_sw_db_tcam_bank_entries_used_get(unit, bank_id, TRUE /* is_inverse */);
    ...
    )
{
  ...
  if (!prio_node_found) {
    ...
    if (current != SOC_SAND_SORTED_LIST_ITER_END(prio_list)) {
    ...
    if (current != SOC_SAND_SORTED_LIST_ITER_BEGIN(prio_list)) {
    ...
    )
{
  ...
  bank_bm = arad_sw_db_tcam_db_entries_used_get(unit, tcam_db_id, location->bank_id);
 ...
 * however, the driver mainatinsmore logical owners (e.g.
  ...
  )
{
  ...
  uint8
      is_inverse;
  uint32
      occ_bm_var_id_base[2] = {
        SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_TCAM_BANK_ENTRIES_USED,
        SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_TCAM_BANK_ENTRIES_USED_INVERSE
      };
  ...
  else
  {
    ...
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 17, exit);
    for (is_inverse = 0; is_inverse < ARAD_TCAM_NOF_OCCUPATIONS_BITMAPS; is_inverse++) {
        occ_bm[is_inverse] = arad_sw_db_tcam_bank_entries_used_get(
                           unit,
                           bank_id,
                           is_inverse
                         );
        ...
        {
		  sal_memset(&occ_bm_restore_info, 0x0, sizeof(ARAD_TCAM_OCC_BM_RESTORE));
		
          /* save info that is used for wb restoration */
          occ_bm_restore_info.is_exist = FALSE;
          res = arad_sw_db_tcam_occ_bm_restore_set(unit, bank_id, is_inverse, occ_bm_restore_info);
          SOC_SAND_CHECK_FUNC_RESULT(res, 18, exit);

          /* disable wb for that bm */
          res = soc_dpp_wb_engine_disable_dynamic_occ_bm(unit, occ_bm_var_id_base[is_inverse] + (bank_id * WB_ENGINE_OCC_BM_INNER_VARS_NUM));
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  if (create_bitmap)
  {
    ...
    for (is_inverse = 0; is_inverse < ARAD_TCAM_NOF_OCCUPATIONS_BITMAPS; is_inverse++) {
        ...
        occ_bm_init_info.wb_var_index = occ_bm_var_id_base[is_inverse] + (bank_id * WB_ENGINE_OCC_BM_INNER_VARS_NUM);
        occ_bm_init_info.unit    = unit;
		/* save info that is used for wb restoration */
        occ_bm_restore_info.init_info = occ_bm_init_info;
        occ_bm_restore_info.is_exist = TRUE;
        res = arad_sw_db_tcam_occ_bm_restore_set(unit, bank_id, is_inverse, occ_bm_restore_info);
        ...
        /* enable wb for that bm */
        res = soc_dpp_wb_engine_enable_dynamic_occ_bm(unit, occ_bm_var_id_base[is_inverse] + (bank_id * WB_ENGINE_OCC_BM_INNER_VARS_NUM), occ_bm[is_inverse]);
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 31, exit);

        arad_sw_db_tcam_bank_entries_used_set(unit, bank_id, is_inverse, occ_bm[is_inverse]);
    }
 ...
 * COVERITY
 *
 * The variable occ_bm is pointed-to inside arad_sw_db_tcam_bank_entries_used_set.
 */
  ...
  )
{
  ...
  /* initialize with WB support */
  priorities_init_info.wb_var_index  = SOC_DPP_WB_ENGINE_VAR_TCAM_DB_PRIORITY + (tcam_db_id * WB_ENGINE_SORTED_LIST_INNER_VARS_NUM); 
  priorities_init_info.unit     = unit;

  res = soc_sand_os_memset(&sorted_list_restore_info, 0, sizeof(sorted_list_restore_info));
  SOC_SAND_CHECK_FUNC_RESULT(res, 666, exit);

  /* save info that is used for wb restoration */
  sorted_list_restore_info.init_info = priorities_init_info;
  sorted_list_restore_info.is_exist = TRUE;
  res = arad_sw_db_tcam_sorted_list_restore_set(unit, tcam_db_id, sorted_list_restore_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  res = soc_sand_sorted_list_create(unit, priorities, priorities_init_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

  /* enable wb for that hash table */
  res = soc_dpp_wb_engine_enable_dynamic_sorted_list(unit, SOC_DPP_WB_ENGINE_VAR_TCAM_DB_PRIORITY + (tcam_db_id * WB_ENGINE_SORTED_LIST_INNER_VARS_NUM), *priorities[0]);
  ...
  )
{
  ...
  /* save info that is used for wb restoration */
  sorted_list_restore_info.is_exist = FALSE;
  res = arad_sw_db_tcam_sorted_list_restore_set(unit, tcam_db_id, sorted_list_restore_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);

  res = soc_dpp_wb_engine_disable_dynamic_sorted_list(unit, SOC_DPP_WB_ENGINE_VAR_TCAM_DB_PRIORITY + (tcam_db_id * WB_ENGINE_SORTED_LIST_INNER_VARS_NUM));
            SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 35, exit);
  ...
  )
{
  ...
  if (access_profile_array_id == 0) {
      ...
      occ_bm_init_info.wb_var_index = SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_TCAM_DB_BANK_ENTRIES_USED + 
          ((tcam_db_id * SOC_DPP_DEFS_MAX_TCAM_NOF_BANKS + bank_id) * WB_ENGINE_OCC_BM_INNER_VARS_NUM);
      occ_bm_init_info.unit    = unit;
      /* save info that is used for wb restoration */
      occ_bm_restore_info.init_info = occ_bm_init_info;
      occ_bm_restore_info.is_exist = TRUE;
      res = arad_sw_db_tcam_db_bank_occ_bm_restore_set(unit, tcam_db_id, bank_id, occ_bm_restore_info);
      SOC_SAND_CHECK_FUNC_RESULT(res, 25, exit);
      ...
      /* enable wb for that bm */
      res = soc_dpp_wb_engine_enable_dynamic_occ_bm(unit, occ_bm_init_info.wb_var_index, occ_bm);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 31, exit);

      arad_sw_db_tcam_db_entries_used_set(unit, tcam_db_id, bank_id, occ_bm);
  }
  ...
  )
{
    ...
    for (ndx = 0; ndx < SOC_SAND_BIT(shift); ++ndx)
    {
      ...
      if (nof_dbs == 0)
      {
        ...
        for (is_inverse = 0; is_inverse < ARAD_TCAM_NOF_OCCUPATIONS_BITMAPS; is_inverse++) {
            ...
            occ_bm = arad_sw_db_tcam_bank_entries_used_get(unit, bank_id, is_inverse);
  ...
  for (access_profile_array_id = 0; access_profile_array_id < nof_access_profiles; access_profile_array_id++) {
      ...
      *bitmap_lcl = pd_tbl_data.bitmap;
      ...
      if (access_profile_array_id == 0) {
          /* Disable the occupation bitmap */
          res = arad_sw_db_tcam_db_bank_occ_bm_restore_get(unit, tcam_db_id, bank_id, &occ_bm_restore_info);
          SOC_SAND_CHECK_FUNC_RESULT(res, 65, exit);

          if (occ_bm_restore_info.is_exist) {
              /* disable wb for that bm */
              res = soc_dpp_wb_engine_disable_dynamic_occ_bm(unit, occ_bm_restore_info.init_info.wb_var_index);
              SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit);

              occ_bm = arad_sw_db_tcam_db_entries_used_get(unit, tcam_db_id, bank_id);
              soc_sand_occ_bm_destroy(unit, occ_bm);
