    ...
    SOC_SAND_IN  ARAD_DBG_RST_DOMAIN      rst_domain)
{
    ...
    uint64
        ...
        soft_init_reg_val_orig_wo_clp_xlp,
        fld64_val;
   ...
   LOG_VERBOSE(BSL_LS_SOC_INIT,
    ...
    soft_init_reg_val_orig_wo_clp_xlp = soft_init_reg_val;
   ...
   LOG_VERBOSE(BSL_LS_SOC_INIT,
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
        res = arad_kbp_recover_rx_shut_down(unit, elk->kbp_mdio_id);
    ...
    if (is_ingr && is_egr) {
        ...
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, XLP_0_INITf, fld64_val, soft_init_reg_val, 26, exit);
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, XLP_1_INITf, fld64_val, soft_init_reg_val, 27, exit);
    ...
    if (is_ingr) {    
       ...
       LOG_VERBOSE(BSL_LS_SOC_INIT,
                   (BSL_META_U(unit,
                               "%s(): Validate Data Path is clean- ingr.\n"), FUNCTION_NAME()));

        res = arad_polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, IQM_GLOBAL_RESOURCE_COUNTERS_2r, REG_PORT_ANY, 0, OC_BDB_COUNTf, 0x0);
        if (soc_sand_update_error_code(res, &ex ) != no_err) {
           LOG_ERROR(BSL_LS_SOC_INIT,
                     (BSL_META_U(unit,
                                 "%s(): Error Validating Dtat Path is clean: IQM_GLOBAL_RESOURCE_COUNTERS_2r, OC_BDB_COUNTf.\n"), FUNCTION_NAME()));
        }

        res = arad_polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, IQM_GLOBAL_RESOURCE_COUNTERS_BD_2r, REG_PORT_ANY, 0, FR_BD_COUNTf, 0x0);
        if (soc_sand_update_error_code(res, &ex ) != no_err) {
            LOG_ERROR(BSL_LS_SOC_INIT,
                      (BSL_META_U(unit,
                                  "%s(): Error Validating Dtat Path is clean: IQM_GLOBAL_RESOURCE_COUNTERS_BD_2r, FR_BD_COUNTf.\n"), FUNCTION_NAME()));
         }

        res = arad_polling(unit, ARAD_TIMEOUT, ARAD_MIN_POLLS, IQM_OCCUPIED_UNICAST_DBUFFS_COUNTERr, REG_PORT_ANY, 0, OC_DB_COUNT_UCf, 0x0);
        if (soc_sand_update_error_code(res, &ex ) != no_err) {
            LOG_ERROR(BSL_LS_SOC_INIT,
                      (BSL_META_U(unit,
                                  "%s(): Error Validating Dtat Path is clean: IQM_OCCUPIED_UNICAST_DBUFFS_COUNTERr, OC_DB_COUNT_UCf.\n"), FUNCTION_NAME()));
        }
    }
   ...
   LOG_VERBOSE(BSL_LS_SOC_INIT,
    ...
    if (is_ingr && is_egr) { 
        ...
        COMPILER_64_SET(fld64_val, 0, 0x1);
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, CLP_0_INITf, fld64_val, soft_init_reg_val_orig_wo_clp_xlp, 24, exit);
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, CLP_1_INITf, fld64_val, soft_init_reg_val_orig_wo_clp_xlp, 25, exit);
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, XLP_0_INITf, fld64_val, soft_init_reg_val_orig_wo_clp_xlp, 26, exit);
        ARAD_FLD_TO_REG64(ECI_BLOCKS_SOFT_INITr, XLP_1_INITf, fld64_val, soft_init_reg_val_orig_wo_clp_xlp, 27, exit);
    }
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  110,  exit, ARAD_REG_ACCESS_ERR,WRITE_ECI_BLOCKS_SOFT_INITr_REG64(unit, soft_init_reg_val_orig_wo_clp_xlp));
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    res = arad_kbp_recover_rx_enable(unit, elk->kbp_mdio_id);
