/* $Id: ppc_api_general.c,v 1.22 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_general.c
*
* MODULE PREFIX:  ppc
*
* FILE DESCRIPTION:
  ...
  )
{
  ...
  case SOC_PPC_PKT_HDR_STK_TYPE_FC_ENCAP_ETH:
    str = "fc_with_encap_eth";
  break;
  case SOC_PPC_PKT_HDR_STK_TYPE_FC_STD_ETH:
    str = "fc_without_encap_eth";
  break;
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "trap_code: %s\n\r"),SOC_PPC_TRAP_CODE_to_string(info->trap_code)));
  LOG_CLI((BSL_META_U(unit,
