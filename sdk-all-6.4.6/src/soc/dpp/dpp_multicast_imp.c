...
void dpp_add_egress_replication_bitmap(
  ...
  const uint32     cud2,
  const uint32     bm_id,
  const uint8      increase_nof_bm_reps /* should the number of bitmap replications be increased */
)
{
    dpp_rep_data_t *rep;
        ...
        for (; size_left; --size_left) {
            ...
            if (port == _SHR_GPORT_INVALID) { /* CUD only replication */
                ...
                for (core = SOC_DPP_CONFIG(mcds->unit)->core_mode.nof_active_cores; core > 0;) {
                    --core;
                    dpp_add_egress_replication_bitmap(mcds, core, cud, cud2, port & ~ARAD_MC_EGR_IS_BITMAP_BIT, core ? FALSE : TRUE);
                }
...
)
{
  ...
  while (entries_remaining) {
    SOCDNX_IF_ERR_EXIT(mcds->get_replications_from_entry( /* add replications to mcds from cur_index entry */
      unit, 0, TRUE, TRUE, cur_index, DPP_MCDS_TYPE_VALUE_EGRESS, cud2, &reps_left, &nof_reps, next_entry));
    ++cur_index;
    --entries_remaining;
  ...
  if (!SOC_WARM_BOOT(unit)) {
    ...
    if (SOC_IS_JERICHO(unit)) { /* Jericho specific configuration */
      ...
      for (i = 0; i < SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; ++i) {
        ...
        soc_reg_field_set(unit, EGQ_MULTICAST_OFFSET_ADDRESSr, &r32, MCDB_OFFSETf, DPP_MCDS_GET_EGRESS_GROUP_START(mcds, 0, i));
