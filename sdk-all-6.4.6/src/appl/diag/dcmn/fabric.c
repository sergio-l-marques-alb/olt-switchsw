/*
 * $Id: fabric.c,v 1.11 Broadcom SDK $
 *
...
diag_dnx_fabric_dpp_support(int unit, int diag_id)
{
    switch (diag_id)
    {
        ...
        case diag_dnx_fabric_type_force:
            return (SOC_IS_JERICHO(unit)) ? TRUE : FALSE;
            break;
        default:
            return FALSE;
    }
...
diag_dnx_fabric_connectivity(int unit, args_t *a) {
   int       rc, i, array_size, port_offset=0;
   bcm_fabric_link_connectivity_t links_connectivity_array[SOC_MAX_NUM_PORTS];
   int logical;
   ...
   for (i = 0; i < array_size; i++) {
      if (BCM_FABRIC_LINK_NO_CONNECTIVITY != links_connectivity_array[i].link_id) {
...
#ifdef BCM_DPP_SUPPORT
         if (SOC_IS_DPP(unit)) {
             port_offset = SOC_DPP_DEFS_GET(unit, first_fabric_link_id); /*offset for QMX*/
            logical = SOC_DPP_FABRIC_LINK_TO_PORT(unit, i + port_offset);
         } else
#endif
         {
            logical = i;
         ...
         cli_out(" %03d  |     %03d      |     %04d      |     %03d     |     %s\n", i + port_offset , logical, links_connectivity_array[i].module_id, links_connectivity_array[i].link_id, dev_type);
      }
   }
...
diag_dnx_fabric_link(int unit, args_t *a) {
   ...
   if (!sal_strncasecmp(option, "all", strlen(option))) {
      option = ARG_GET(a);
      if (!option) {
#ifdef BCM_DFE_SUPPORT
         if (SOC_IS_DFE(unit)) {
      ...
      for (i = 0; i < array_size; i++) {
         ...
         if (SOC_IS_ARAD(unit)) {
             bcm_port_t port = i;
             port += SOC_DPP_DEFS_GET(unit, first_fabric_link_id); /*offset for QMX*/
            cli_out(" %3d(%3d) |", (SOC_DPP_FABRIC_LINK_TO_PORT(unit, port)), port);
         } else
#endif
         {
            cli_out(" %8d |", i);
      ...
      if (!sal_strncasecmp(option, "clear", strlen(option))) {
    ...
    diag_dnx_fabric_thresholds(int unit, args_t* a)
{
    ...
    if (SOC_IS_DPP(unit)) {
        ...
        if (!sal_strncasecmp(direction_s,"rx", strlen(direction_s))) {
            is_rx = 1;
        } else if (!sal_strncasecmp(direction_s,"tx", strlen(direction_s))) {
            is_tx = 1;
        } else if (!sal_strncasecmp(direction_s,"middle", strlen(direction_s))) {
            is_middle = 1;
        } else {
            cli_out("invalid option: %s\n", direction_s);
            parse_arg_eq_done(&pt);
...
{
    cli_out("'fabric force <fabric/core0/core1/default>' - Map packets with all destination IDs to fabric/local core 0/local core 1.\n"); 
}
...
diag_dnx_fabric_force(int unit, args_t *a) 
{
    ...
    if (SOC_IS_DPP(unit)) {
        ...
        if (!sal_strncasecmp(dest_str, "fabric", strlen(dest_str))) {
            force = socDppFabricForceFabric;
        } else if(!sal_strncasecmp(dest_str, "core0", strlen(dest_str))) {
                   force = socDppFabricForceLocal0;
        } else if (!sal_strncasecmp(dest_str, "core1", strlen(dest_str))) {
                   force = socDppFabricForceLocal1;
        } else if (!sal_strncasecmp(dest_str, "default", strlen(dest_str))) {
