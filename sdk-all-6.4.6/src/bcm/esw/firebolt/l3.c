...
_bcm_xgs3_ecmp_grp_hash_calc(int unit, void *buf_ecmp, uint16 *hash)
{
    ...
    if ((NULL == buf_ecmp) || (NULL == hash)) {
        return (BCM_E_PARAM);
    }
    BCM_IF_ERROR_RETURN(_bcm_xgs3_l3_ecmp_group_alloc(unit, &buf));
    sal_memcpy (buf, buf_ecmp, BCM_XGS3_L3_ECMP_MAX_PATHS(unit) * sizeof(int));
...
_bcm_xgs3_ecmp_grp_cmp(int unit, void *buf, int grp_idx, int *cmp_result)
{
    ...
    if (BCM_XGS3_L3_ECMP_GROUP_FLAGS_ISSET(unit, grp_idx, 
                      BCM_L3_ECMP_PATH_NO_SORTING)) {
        _shr_sort(ecmp_grp, ecmp_count, sizeof(int), _bcm_xgs3_cmp_int);
    }
             ...
             if (egr->mpls_label != BCM_MPLS_LABEL_INVALID) {
              ...
              if (egr->mpls_label != BCM_MPLS_LABEL_INVALID ) {
                  ...
                  if (BCM_SUCCESS(bcm_tr_mpls_tunnel_intf_valid (unit, index))) {
                      ...
                      if (entry_type == 1) {  
                          L3_IF_ERROR_CLEANUP(
                              bcm_tr_mpls_egress_entry_modify(unit, index, 0, 0));
                      }
                  } 
              }
        }
                                 ...
                                 bcm_if_t *intf_array, int *intf_count)
{
    ...
    if (soc_feature(unit, soc_feature_vp_lag)) {
        ...
        tbl_ptr  = BCM_XGS3_L3_TBL_PTR(unit, ecmp_grp);
        if ((tbl_ptr->split_idx > 0)) {
            if ((grp_idx >= tbl_ptr->split_idx) &&
                (grp_idx < tbl_ptr->split_idx + max_vp_lags)) {
                return (BCM_E_PARAM);
            }
        } else
#endif
        ...
        if (BCM_GPORT_IS_SUBPORT_PORT(port)) {
            ...
            if ((soc_feature(unit, soc_feature_linkphy_coe) ||
                ...
                if (SOC_IS_SABER2(unit)) {
                    if (((port < _BCM_SB2_SUBPORT_PP_PORT_INDEX_MIN) || 
                       (port > _BCM_SB2_SUBPORT_PP_PORT_INDEX_MAX))) {
                        return BCM_E_PORT;
                    } 

                } else if ((port >= _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN) &&
...
_bcm_xgs3_nh_entry_parse(int unit, uint32 *ing_entry_ptr,
        uint32 *egr_entry_ptr, uint32 index, bcm_l3_egress_t *nh_entry)
{
    ...
    */
    ...
    if (soc_feature(unit, soc_feature_nat)) {
        ...
        if (BCM_XGS3_L3_INGRESS_INTF_MAP_MODE_ISSET(unit) &&
            ...
            sal_memset(&vlan_mpls, 0, sizeof(vlan_mpls));
            SOC_IF_ERROR_RETURN(
                soc_mem_read(unit, VLAN_MPLSm, MEM_BLOCK_ANY,
                             nh_entry->vlan, &vlan_mpls));
