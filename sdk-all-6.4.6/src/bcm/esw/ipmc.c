/*
 * $Id: ipmc.c,v 1.152 Broadcom SDK $
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            ...
            SOC_IF_ERROR_RETURN
                (READ_MMU_REPL_LIST_TBL_PIPE0m(unit, MEM_BLOCK_ANY, 0,
                                               &repl_list_entry));
            if (soc_mem_field_valid(unit, MMU_REPL_LIST_TBLm,
                                    NEXT_HOP_INDEX_0f)) {
           ...
           soc_MMU_REPL_LIST_TBLm_field32_set(unit, &repl_list_entry,
                                              MODE_1_BITMAPf, 0);
           SOC_IF_ERROR_RETURN
                 (WRITE_MMU_REPL_LIST_TBL_PIPE0m(unit, MEM_BLOCK_ALL,
                                                 0, &repl_list_entry));
           return BCM_E_NONE;
        } else
#endif /* BCM_TOMAHAWK_SUPPORT */
...
_bcm_esw_ipmc_gport_construct(int unit, bcm_ipmc_addr_t *data)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) ||
        ...
        if ((data->mod_id == mymodid + 1) && !(data->ts)) {
