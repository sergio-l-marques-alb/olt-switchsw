...
bcm_th_switch_agm_init(int unit)
{
    ...
    if (SOC_WARM_BOOT(unit)) {
        BCM_IF_ERROR_RETURN(bcm_th_switch_agm_deinit(unit));
    }
                         ...
                         bcm_switch_agm_info_t *agm_info)
{
    ...
    soc_mem_field32_set(unit, AGM_MONITOR_TABLEm, &entry, SCALEf,
                        _th_agm_itvl2scale(unit, agm_info->period_interval));
                             ...
                             int enable)
{
    ...
    if (enable && AGM_INFO(unit, agm_id).attr.period_num > 0 &&
        ...
        rv = bcm_esw_time_interface_add(0, &time_intf);
        if (BCM_FAILURE(rv) && rv != BCM_E_EXISTS) {
            AGM_UNLOCK(unit);
            return rv;
        }
        ...
        (void) bcm_esw_time_capture_get(0, time_intf.id, &time1);
...
bcm_th_switch_agm_stat_clear(int unit, bcm_switch_agm_id_t agm_id)
{
    ...
    group_size = AGM_INFO(unit, agm_id).attr.num_members;
    nstat = group_size * AGM_INFO(unit, agm_id).attr.period_num;
    AGM_UNLOCK(unit);
 ...
 * Purpose:
 *      Retrieve an aggregation group id on which the monitor entry is attached
 */
int
