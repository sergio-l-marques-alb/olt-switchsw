...
_kt2_rep_regs_t kt2_rqe_rep_regs[] = { 
   { INVALIDf,    INVALIDm},
    ...
    {PORT69_LASTf, MMU_IPMC_GROUP_TBL1m},
...
#define REPL_PORT_CHECK(unit, port) \
    ...
    if (SOC_IS_SABER2(unit)) { \
        if (!IS_PORT(unit, port) && \
                ...
                !(port >= _BCM_SB2_SUBPORT_PP_PORT_INDEX_MIN) && \
                !(port <= _BCM_SB2_SUBPORT_PP_PORT_INDEX_MAX)) { \
            return BCM_E_PARAM; \
        } \
                            ...
                            bcm_port_t port, int count)
{
    ...
    BCM_IF_ERROR_RETURN
    (_bcm_kt_ipmc_mmu_mc_remap_ptr(unit, ipmc_id, &old_remap_id, TRUE));
              ...
              if (!BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, startnh->encap_id)) {
                 ...
                 /* Next hop is used */
                  encap_id =   startnh->encap_id -
                           BCM_XGS3_DVP_EGRESS_IDX_MIN + IPMC_SB2_NHOP_ID_MIN;
              }
              soc_MMU_EXT_MC_QUEUE_LIST0m_field32_set(unit, 
                                                   &qlist0_entry[repl_index],
                                                        repl_ptr[repl_count],
...
bcm_tr3_ipmc_repl_detach(int unit)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        if (soc_property_get(unit, spn_MULTICAST_PER_TRUNK_REPLICATION, 0)) {
            BCM_IF_ERROR_RETURN(bcm_td2p_aggregation_id_list_detach(unit));
            BCM_IF_ERROR_RETURN(bcm_td2p_aggid_trunk_map_detach(unit));
        }
    }
#endif
...
_bcm_td2p_repl_port_agg_map_init(int unit)
{
    ...
    if (soc_property_get(unit, spn_MULTICAST_PER_TRUNK_REPLICATION, 0)) {
        ...
        BCM_IF_ERROR_RETURN(bcm_td2p_aggregation_id_list_init(unit));
        BCM_IF_ERROR_RETURN(bcm_td2p_aggid_trunk_map_init(unit));
    }
#endif /* BCM_TRIENT2PLUS_SUPPORT */
                ...
                if (SOC_IS_KATANA2(unit)) {
                    ...
                    SOC_IF_ERROR_RETURN(soc_mem_clear(unit, 
                        ...
                        repl_map_idx_pp =  (soc_mem_index_count(unit,
                               MMU_REPL_MAP_TBLm) - _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN);
                    } 
                    ...
                    if (SOC_IS_SABER2(unit)) {
                        repl_map_idx_pp =  _BCM_SB2_SUBPORT_PORT_MAX;
                ...
                int start_ptr, int intf_count)
{
    ...
    if (SOC_IS_KATANA2(unit)) {
        ...
        if (soc_mem_field_valid(unit, MMU_REPL_GRP_TBL0m, MEMBER_BMP_PORTSf)) {
            member_bitmap_width0 =  soc_mem_field_length(unit,
                    MMU_REPL_GRP_TBL0m, MEMBER_BMP_PORTSf);
        } else {
        member_bitmap_width0 =  soc_mem_field_length(unit,
                MMU_REPL_GRP_TBL0m, MEMBER_BMP_PORTS_119_0f);
        }
