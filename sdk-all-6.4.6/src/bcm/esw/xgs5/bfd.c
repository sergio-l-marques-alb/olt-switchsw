...
bcmi_xgs5_bfd_hw_init(int unit)
{
    ...
    soc_reg_field_set(unit, BFD_RX_UDP_CONTROLr, &val,
        BFD_UDP_PORTM_HOPf, SHR_MICRO_BFD_DEST_PORT);
                               ...
                               bfd_udp_header_t *udp)
{
    ...
    if (v6) {
        ...
        /*
         * Added 0 before protocol value to compute the checksum
         * accurately
         */
                              ...
                              bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (BCM_GPORT_INVALID != endpoint_info->gport) {
        ...
        if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
            LOG_ERROR(BSL_LS_BCM_BFD,
                      (BSL_META_U(unit,
                                  "\n BFD is not supported on TRUNK GPORT\n")));
            return (BCM_E_PARAM);
        }
    ...
    if ((BCM_SUCCESS(rv)) || trunk_id != BCM_TRUNK_INVALID) {
        ...
        if (MICRO_BFD_DST_MAC(l3_egress.mac_addr)){
            is_micro_bfd = 1;
        }
