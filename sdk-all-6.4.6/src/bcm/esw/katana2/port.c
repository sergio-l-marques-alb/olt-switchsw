            ...
            int unit,uint8 mxqblock ,uint8 mxqblock_port,uint32 port_speed)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_linkscan_hw_port_init(unit, mxqblock_port));
    soc_linkscan_continue(unit);
    return SOC_E_NONE;
}
static 
...
/* -------------------------------------------------  */
    ...
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
        ...
        BCM_IF_ERROR_RETURN(bcm_katana2_cosq_reconfigure_flexio(unit, mxqblock_port));
    }
...
int _bcm_kt2_cosq_port_ext_buf_set(int unit, bcm_port_t port, int enable)
{
    ...
    SOC_IF_ERROR_RETURN(soc_kt_port_flush(unit, port, 0));
