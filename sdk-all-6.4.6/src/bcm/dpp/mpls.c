...
int _bcm_mpls_port_delete(
   ...
   bcm_gport_t          port_id
    ...
    if (BCM_GPORT_IS_MPLS_PORT(port_id) && (is_ingress || is_egress) && (lif_usage != _bcmDppLifTypeMplsPort)) { /* PWE */
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    if (is_member || !is_local) {
        rv = _bcm_mpls_port_delete(unit, mpls_port_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    bcm_vpn_t vpn)
{
    ...
    SOC_PPD_LIF_ID local_lif_index_simple;
    ...
    for (iter = 0; iter != -1;) {
        ...
        /* fill next port */
        rv = _bcm_mpls_port_delete(unit, port_val);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (is_ingress) {
        /* Ensure InLIF is PWE */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(soc_sand_dev_id, gport_hw_resources.local_in_lif, -1, &lif_usage, NULL) );
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, gport_hw_resources.local_in_lif, TRUE, &is_wide_entry));
            BCMDNX_IF_ERR_EXIT(rv);

            mpls_port->flags2 |= (is_wide_entry) ? BCM_MPLS_PORT2_INGRESS_WIDE : 0;
        }
    ...
    /* get protection info */
    if (gport_type == _bcmDppGportResolveTypeFec) {
        _bcm_dpp_am_fec_alloc_usage usage;
        rv = bcm_dpp_am_fec_get_usage(unit, _BCM_PPD_GPORT_FEC_TO_WORK_FEC(dest_id.dest_val), &usage);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_egress) {
        rv = _bcm_dpp_lif_usage_get(soc_sand_dev_id, -1, gport_hw_resources.local_out_lif, NULL, &lif_usage);
        BCMDNX_IF_ERR_EXIT(rv);
        if (lif_usage != _bcmDppLifTypeMplsPort) {
        ...
        if (!is_ingress) {
            ...
            _BCM_PETRA_L3_ITF_SET(mpls_port->egress_tunnel_if, _BCM_PETRA_L3_ITF_ENCAP, global_lif_next_eep);
    ...
    if (((mpls_port->failover_port_id != BCM_GPORT_TYPE_NONE) && is_ingress && !is_egress) ||
        ...
        if (phy_port->type == _bcmDppGportResolveTypePhyEEP) {
        ...
        if (phy_port->type == _bcmDppGportResolveTypeFecVC) {
            ...
            mpls_port->egress_label.label = _BCM_DPP_ENCAP_ID_VC_GET(phy_port->encap_id);
            push_profile = _BCM_DPP_ENCAP_ID_PUSH_PROFILE_GET(phy_port->encap_id);
        }
        else { 
            if (is_egress) {
    ...
    if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1)) {
        ...
        if (is_ingress) {
            soc_sand_rv = soc_ppd_l2_lif_pwe_get(unit, mpls_port->match_label, &lif_idx, &pwe_info, &found);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            if (pwe_info.orientation == SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB) {
                mpls_port->flags |= BCM_MPLS_PORT_NETWORK;
            }
        } else if (is_egress) {
            if (SOC_IS_ARADPLUS_AND_BELOW(unit) && SOC_DPP_CONFIG(unit)->arad->pp_op_mode.split_horizon_filter_enable) {
                SOC_SAND_PP_HUB_SPOKE_ORIENTATION orientation;
                soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_get(unit, encap_id, &orientation);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if (orientation == SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB) {
                    mpls_port->flags |= BCM_MPLS_PORT_NETWORK;
                }
            }
        }
    ...
    )
{
    ...
    *inner_flags = 0;
    ...
    if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0) && (tunnel_switch->flags & BCM_MPLS_SWITCH_EVPN_IML)) {
    ...
    int eep)
{
    ...
    soc_sand_rv =
        soc_ppd_eg_encap_entry_get(soc_sand_dev_id,
                               SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, local_lif_id, 0,
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (indx = 0; indx < num_labels; ++indx) {
        ...
        if (_BCM_PETRA_L3_ITF_IS_RIF(label_array[indx].l3_intf_id) && label_array[indx].l3_intf_id >= _BCM_PETRA_L3_NOF_ITFS) {
        ...
        if (label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_REPLACE){
            ...
            BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, local_lif_id, NULL, &usage) );
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if(!eep_alloced) {
            ...
            if (SOC_IS_JERICHO(unit))
            {
                ...
                rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, flags, &global_lif_id, NULL, &local_out_lif_info, TRUE);
        ...
        _BCM_PETRA_L3_ITF_SET(label_array[indx].tunnel_id,_BCM_PETRA_L3_ITF_ENCAP,global_lif_id);
        if (is_bind_entries) {
            _BCM_PETRA_L3_ITF_SET(label_array[indx+1].tunnel_id,_BCM_PETRA_L3_ITF_ENCAP,global_lif_id);
        }
        /* EEP allocation } */
        ...
        if (((indx+1+is_bind_entries) == num_labels) & _BCM_PETRA_L3_ITF_IS_ENCAP(label_array[indx+is_bind_entries].l3_intf_id)) { /* Special case: interface in last label is Tunnel */
            if (SOC_IS_JERICHO(unit))
            {
                rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_EGRESS, (_BCM_PETRA_L3_ITF_VAL_GET(label_array[indx+is_bind_entries].l3_intf_id)), &next_eep_local_lif);
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (SOC_IS_JERICHO(unit))
            {
                rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_EGRESS, (_BCM_PETRA_L3_ITF_VAL_GET(label_array[indx+1+is_bind_entries].tunnel_id)), &next_eep_local_lif);
                BCMDNX_IF_ERR_EXIT(rv);
                next_eep[0] = (uint32)next_eep_local_lif;
            }
            else
        ...
        if (label_action_type == _bcm_dpp_mpls_egress_action_type_swap) { /* if swap */
            ...
            soc_sand_rv = 
                soc_ppd_eg_encap_data_lif_entry_add(soc_sand_dev_id, local_lif_id, &mpls_data_info, TRUE, next_eep[0]);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_if_t intf)
{
    ...
    if (SOC_IS_JERICHO(unit)) 
    {
        ...
        rv = _bcm_dpp_gport_delete_global_and_local_lif(unit,global_lif_id,_BCM_GPORT_ENCAP_ID_LIF_INVALID,local_lif_id,FALSE);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else    
    {
    ...
    /* Remove entry from SW DB */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_match_delete(unit,local_lif_id));
    ...
    int unit)
{
    ...
    for (indx = 0; indx < SOC_DPP_CONFIG(unit)->l2.nof_lifs; indx+=2 /* MPLS always even */) {
        ...
        if (lif_usage == _bcmDppLifTypeMplsTunnel) {
            ...
            _BCM_PETRA_L3_ITF_SET(eep, _BCM_PETRA_L3_ITF_ENCAP, global_lif);
    ...
    int *label_count)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(soc_sand_dev_id, -1, local_lif_id, NULL, &lif_usage));
    if ((lif_usage != _bcmDppLifTypeMplsTunnel) 
    ...
    soc_sand_rv =
        soc_ppd_eg_encap_entry_get(soc_sand_dev_id,
                               SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, local_lif_id, 2,
                               encap_entry_info, next_eep, &nof_entries);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    *label_count = 0;
    ...
    for (indx = 0; indx < nof_entries; ++indx) {
        ...
        if (indx == 0) {
            _BCM_PETRA_L3_ITF_SET(tunnel_id,_BCM_PETRA_L3_ITF_ENCAP,global_lif_id);
        } else {
            if (SOC_IS_JERICHO(unit)) {
        ...
        if (encap_entry_info[indx].entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) {  /* mpls encap */
            ...
            for (cur_indx = 0; cur_indx < cur_count; ++cur_indx) {
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, local_lif_id, FALSE, &is_wide_entry));
                    BCMDNX_IF_ERR_EXIT(rv);

                    label_array[*label_count].flags |= is_wide_entry ? BCM_MPLS_EGRESS_LABEL_WIDE : 0;
                }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    soc_sand_rv = soc_ppd_frwrd_fec_entry_get(unit, fec_id, &protect_type, &working_fec, &protect_fec, &protect_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, local_in_lif_id, TRUE, &is_wide_entry));
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (update) {/* update check LIF already allocated */
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                /* Allocate the global and local lif */
                rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, alloc_flags, &global_lif_id, &local_inlif_info, NULL, TRUE);
                BCM_IF_ERROR_RETURN(rv);
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            /* Special case for mLDP: termination type is concluded according to LIF-ID. LIF-ID is determined by SOC property */
            if (SOC_DPP_CONFIG(unit)->pp.mldp_support && (info->flags & BCM_MPLS_SWITCH_LOOKUP_NONE) &&
                (lif_indx == SOC_DPP_CONFIG(unit)->pp.global_mldp_dummy_lif_id[1])) {
                    inner_flags |= _BCM_DPP_MPLS_LIF_TERM_GAL;
            }
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_mpls_tunnel_lif_term_profile_exchange(unit,
                                               local_in_lif_id,
                                               inner_flags,
                                               ((rif_key.flags & SOC_PPD_RIF_MPLS_LABEL_EXPECT_BOS)>0),
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if (rif_key.flags & SOC_PPD_RIF_MPLS_LABEL_LOOKUP_NONE) {
           ...
           BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, local_in_lif_id, -1, &lif_usage, NULL) );
        ...
        soc_sand_rv =
            soc_ppd_rif_mpls_label_map_remove(soc_sand_dev_id, &rif_key, (SOC_PPD_LIF_ID*) &local_in_lif_id);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        if (SOC_IS_JERICHO(unit)) 
        {
            /*dealloc local+global lif*/
            rv = _bcm_dpp_gport_delete_global_and_local_lif(unit,global_lif_id,local_in_lif_id,_BCM_GPORT_ENCAP_ID_LIF_INVALID,FALSE);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else
        {
        ...
        rv = _bcm_dpp_in_lif_match_delete(unit, local_in_lif_id); /*Global lif*/
        BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_gport_mpls_is_valid_replace(int unit, bcm_mpls_port_t *new_mpls_port, bcm_mpls_port_t *old_mpls_port, bcm_mpls_label_t old_egress_vc, uint32 old_push_profile){
    ...
    if ((!n_is_protected) && (new_mpls_port->egress_label.label != old_egress_vc)) {
    ...
    if ((!n_is_protected) && ((new_push_profile != old_push_profile) || (rv != BCM_E_NONE))) {
    ...
    if (_BCM_MULTICAST_IS_SET(new_mpls_port->failover_mc_group)) {
        ...
        if (old_mpls_port->failover_id == _BCM_PROTECTION_POINTER_INVALID) {
    ...
    uint8 is_one_plus_one_protection)
{
    ...
    } else if (_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if)) {
    ...
    if (intf_global_lif != 0) {
        if (SOC_IS_JERICHO(unit) && !is_one_plus_one_protection && ((_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if) && _BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) \
            || ((mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP) || mpls_port->flags & BCM_MPLS_PORT_COUNTED)) ) { /* add the COUNTED flag*/
    ...
    if (is_egress_protection) {
        /* TBD - There will be a need to define protection_pointer & protection_path in SOC_PPD_EG_ENCAP_PWE_INFO
        eep_pwe_info.protection_pointer = mpls_port->egress_failover_id; 
        eep_pwe_info.protection_path = _BCM_PPD_GPORT_IS_EGRESS_WORKING_PORT(mpls_port) ? 0 : 1; 
        */ 
    }
    ...
    SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info)
{
    ...
    _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;
    _BCM_GPORT_PHY_PORT_INFO *temp_phy_port=NULL;    
    uint32 lif_types,pwe_lif_types = 0;
    uint32 sys_port = 0;
    SOC_PPD_LIF_ID local_in_lif = 0;
    int local_out_lif = 0, global_lif_id = 0;
    int is_lag=0;
    ...
    if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1)) {
        ...
        if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
            pwe_info.lif_profile = old_pwe_info->outlif_profile;
        }
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_set,
                                 (unit, SOC_OCC_MGMT_TYPE_INLIF, SOC_OCC_MGMT_INLIF_APP_ORIENTATION,
    ...
    */
    if(encap_with_id) {
        ...
        rv = _bcm_dpp_gport_l2_intf_get_lif_from_port_id(mpls_port, is_1_to_1_protected, is_mc_ingress, (SOC_PPD_LIF_ID*) &global_lif_id, BCM_GPORT_MPLS_PORT);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (is_forward_group) { /* Override previous value */
            fec_id = global_lif_id;
        }

        /* In Jericho and REPLACE, check that the CASCADED/not-CASCADED matches. */
        if (SOC_IS_JERICHO(unit) && update == 1) {
            _bcm_dpp_am_fec_alloc_usage old_fec_usage;
            _bcm_dpp_am_fec_alloc_usage fec_usage = mpls_port->flags2 & BCM_MPLS_PORT2_CASCADED ? _BCM_DPP_AM_FEC_ALLOC_USAGE_CASCADED : _BCM_DPP_AM_FEC_ALLOC_USAGE_STANDARD;
    ...
    if (is_ingress || is_egress) {
        ...
        */
            ...
            if(!update) {
                ...
                /* We will allocate outlif if one of the following is true:                                                                                                                                                                           .
                   ...
                   If PWE is P2P and not protected - then we don't need the Outlif. */
                    if(((mpls_port->flags & BCM_MPLS_PORT_COUNTED) || (mp) || (is_1_to_1_protected) || (is_1_plus_1) || (is_egress_protection) || (!_BCM_PETRA_L3_ITF_IS_FEC(mpls_port->egress_tunnel_if))) && is_egress){
                ...
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    if (allocate_egress) {
                        if (is_ingress) {
                            rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit,pwe_lif_types,lif_flags,1,(SOC_PPD_LIF_ID*)&global_lif_id,NULL);
                            BCMDNX_IF_ERR_EXIT(rv);
                ...
                rv = _bcm_dpp_in_lif_additional_flags_set(unit, local_in_lif, (eep_required) ? 0 : _BCM_DPP_INFLIF_MATCH_INFO_MPLS_PORT_NO_OUTLIF);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                if (is_1_to_1_protected && encap_with_id && is_egress) { /* Update EEP only if it's protected */
                    eep_required = 1;
        ...
        if(is_1_to_1_protected) {
            ...
            rv = bcm_dpp_gport_mgmt_fec_remote_lif_set(unit, fec_id, remote_lif);
            BCMDNX_IF_ERR_EXIT(rv);
        }
            ...
            if(!remote_lif) /* Tunnel protected */ {
                ...
                if (update) {
                    ...
                    if (gport_hw_resources.local_out_lif == _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                        rv = _bcm_dpp_sw_db_hash_vlan_find(unit,
                                            &(mpls_port->mpls_port_id), 
                                            (shr_htb_data_t*)(void *)&temp_phy_port,FALSE);    
                        if (!temp_phy_port || BCM_FAILURE(rv)) {
                            return BCM_E_INTERNAL;
                        }
                        if (temp_phy_port->type == _bcmDppGportResolveTypeFecVC) {
                                /* In case of FEC_VC push profile is allocated */
                                old_pwe_info->push_profile = _BCM_DPP_ENCAP_ID_PUSH_PROFILE_GET(temp_phy_port->encap_id);      
                        }
    ...
    if(is_forward_group) {
        ...
        if(!update) {
            if (with_id) {
                rv = _bcm_dpp_gport_l2_intf_get_lif_from_port_id(mpls_port, is_1_to_1_protected, is_mc_ingress, &fec_id, BCM_GPORT_MPLS_PORT);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    */ 
    ...
    if(_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) {
        ...
        if (_BCM_PETRA_L3_ITF_IS_ENCAP(mpls_port->egress_tunnel_if)) {
            ...
            if (mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP || (mpls_port->flags & BCM_MPLS_PORT_COUNTED)) {
                phy_port->type = _bcmDppGportResolveTypeFecEEP;
    ...
    if(eep_required == 1 && !remote_lif && is_egress) {
        rv = _bcm_dpp_mpls_port_pwe_eep_set(unit, mpls_port, local_out_lif,
                                            push_profile, old_pwe_info, is_egress_protection, is_1_plus_1);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(!remote_lif && is_ingress) {
      ...
      if(_BCM_DDP_IS_MP_VPN(vpn) && !is_gal) {
          ...
          if(_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id)) {
              ...
              rv = _bcm_dpp_in_lif_match_learn_gport_set(unit, local_in_lif, mpls_port->mpls_port_id);
              BCMDNX_IF_ERR_EXIT(rv);
          }
          /* protected case, where need to configure only LIF,
             in this case failover-id is invalid, but failover_port_id includes the FEC id to learn */
              ...
              rv = _bcm_dpp_in_lif_match_learn_gport_set(unit, local_in_lif, mpls_port->failover_port_id);
              BCMDNX_IF_ERR_EXIT(rv);
          }
          /* protected-tunnel: learn FEC + VC */
              ...
              if (_BCM_PETRA_L3_ITF_IS_ENCAP(mpls_port->egress_tunnel_if)) {
                ...
                if ((is_ingress && is_egress && !(mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP)) || is_forward_group) {
          ...
          pwe_info.is_extended = DPP_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_INGRESS_WIDE);

          soc_sand_rv = soc_ppd_l2_lif_pwe_add(unit, in_vc_label, local_in_lif, &pwe_info, &failure_indication);
          BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
          SOC_SAND_IF_FAIL_RETURN(failure_indication);

          /* store match cretiria used for this LIF */
          /* store gport this LIF is mapped to */
          rv = _bcm_dpp_in_lif_mpls_match_add(unit,mpls_port,local_in_lif);
          BCMDNX_IF_ERR_EXIT(rv);

          /* store vsi */
          rv = _bcm_dpp_in_lif_match_vsi_set(unit, local_in_lif, vpn);
          BCMDNX_IF_ERR_EXIT(rv);
      }
      else /* p2p */ 
      {
    ...
    if(update && !remote_lif && is_ingress) {
        ...
        if(mpls_port->match_label != mpls_port_o.match_label){
            ...
            soc_sand_rv = soc_ppd_l2_lif_pwe_remove(unit, in_vc_label, TRUE/*remove from SEM*/, is_gal, &local_in_lif_o);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            if(local_in_lif_o != local_in_lif) {
                rv = _bcm_dpp_in_lif_match_delete(unit,local_in_lif_o);
                BCMDNX_IF_ERR_EXIT(rv);

                rv = _bcm_dpp_in_lif_match_vsi_set(unit, local_in_lif_o, 0);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
    ...
    if (update) {
        if (!with_id) {
            ...
            if (gport_hw_resources.local_out_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
                ...
                _BCM_GPORT_PHY_PORT_INFO *phy_port=NULL;
                rv = _bcm_dpp_sw_db_hash_vlan_find(unit, &(mpls_port->mpls_port_id), \
                                    (shr_htb_data_t*)(void *)&phy_port,FALSE);    
                if (!phy_port || BCM_FAILURE(rv)) {
                    return BCM_E_INTERNAL;
                }
                if (phy_port->type == _bcmDppGportResolveTypeFecVC) {
                        /* In case of FEC_VC push profile is allocated */
                        old_push_profile = _BCM_DPP_ENCAP_ID_PUSH_PROFILE_GET(phy_port->encap_id); 
                        old_egress_vc = _BCM_DPP_ENCAP_ID_VC_GET(phy_port->encap_id) ;    
                }
            }
