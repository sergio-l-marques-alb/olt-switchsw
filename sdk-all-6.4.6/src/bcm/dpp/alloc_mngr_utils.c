...
bcm_dpp_am_attach(int unit)
{
    ...
    if(!SOC_WARM_BOOT(unit)) {
        ...
        rv = sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.is_allocated(unit, &is_allocated);
        BCMDNX_IF_ERR_EXIT(rv);

        if(!is_allocated) {
            rv = sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.alloc(
                unit,
        ...
        rv = sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.is_allocated(unit, &is_allocated);
        BCMDNX_IF_ERR_EXIT(rv);

        if(!is_allocated) {
            rv = sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.alloc(
                unit,
...
dpp_am_pp_resource_setup_in_range_by_core_id(int unit, int first_res, int last_res, int *pool_id, bcm_dpp_am_pool_info_t *p_info){
    ...
    for (res_id = first_res; res_id < last_res ; res_id++)   {
        ...
        if(!SOC_WARM_BOOT(unit)){
            ...
            rv = sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.set(unit, res_id, *pool_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
   ...
   BCMDNX_ALLOC(_bcm_dpp_template_info[unit], (sizeof(bcm_dpp_am_template_info_t) * nof_template_pools), "am template info");
    ...
    for (template_id = 0 ; template_id < SOC_DPP_CONFIG(unit)->am.nof_am_template_ids ; template_id++)   {
        ...
        if(!SOC_WARM_BOOT(unit)) {
            ...
            rv = sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.set(unit, template_id, pool_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
...
bcm_dpp_am_resource_id_to_pool_id_get(int unit, int core_id, int resource_id, int *pool_id){
    ...
    rv = _bcm_dpp_am_id_to_pool_id_get(unit, core_id, sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.get, resource_id, pool_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_am_template_id_to_pool_id_get(int unit, int core_id, int template_id, int *pool_id){
    ...
    rv = _bcm_dpp_am_id_to_pool_id_get(unit, core_id, sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.get, template_id, pool_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_am_pool_id_to_resource_id_get(int unit, int pool_id, int *res_id, int *core_id){
    ...
    rv = _bcm_dpp_am_pool_id_to_id_get(unit, sw_state_access[unit].dpp.bcm.alloc_mngr.resource_to_pool_map.get, 
...
bcm_dpp_am_pool_id_to_template_id_get(int unit, int pool_id, int *template_id, int *core_id){
    ...
    rv = _bcm_dpp_am_pool_id_to_id_get(unit, sw_state_access[unit].dpp.bcm.alloc_mngr.template_to_pool_map.get, 
...
dpp_am_res_alloc_align_tag(int unit, int core_id, int res_id, uint32 flags, int align, int offset, const void *tag, int count, int *elem){
    ...
    rv = shr_res_alloc_align_tag(unit, pool_id, flags, align, offset, tag, count, elem);
    BCMDNX_IF_ERR_EXIT(rv);
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (!SOC_WARM_BOOT(unit)) {
        rv = bcm_dpp_wb_template_dealloc_update(unit, pool_id, template, TRUE);
        BCMDNX_IF_ERR_EXIT(rv);
    }
#endif
exit:
