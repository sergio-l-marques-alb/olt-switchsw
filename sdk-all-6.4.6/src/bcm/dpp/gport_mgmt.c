...
_bcm_dpp_in_lif_p2p_match_set(int unit, int in_lif, int is_p2p, int peer_gport)
{
    ...
    if (is_p2p) {
        lif_info->match_key[in_lif].additional_flags |= _BCM_DPP_INFLIF_MATCH_INFO_P2P;
    } else {
        lif_info->match_key[in_lif].additional_flags &= (~_BCM_DPP_INFLIF_MATCH_INFO_P2P);
    }
    ...
    if (!SOC_DPP_IS_SYS_RSRC_MGMT_ADVANCED(unit)) {
        lif_info->match_key[in_lif].peer_gport = (is_p2p) ? peer_gport : 0;
    }
...
_bcm_dpp_in_lif_p2p_match_get(int unit, int in_lif, int *is_p2p, int *peer_gport)
{
    ...
    *is_p2p = (lif_info->match_key[in_lif].additional_flags & _BCM_DPP_INFLIF_MATCH_INFO_P2P) ? 1 : 0;
    *peer_gport = lif_info->match_key[in_lif].peer_gport;
    ...
    if (BCM_GPORT_IS_SET(gport)) {
        ...
        if (SOC_DPP_IS_VALID_ENCAP_ID(gport)) {
            ...
            if(SOC_DPP_ENCAP_TYPE_GET(gport) == _DPP_ENCAP_ID_MC_VAL) {
                ...
                /* it can be forward */
                gport_parse_info->type = _bcmDppGportParseTypeForwardFec;
            } else if(SOC_DPP_ENCAP_ID_IS_PUSH_PROFILE(gport)) {            
...
_bcm_dpp_gport_l2_intf_calculate_port_id(int unit, int act_fec, SOC_PPD_LIF_ID lif_id, void *gport, int gport_type){
    ...
    case BCM_GPORT_EXTENDER_PORT:
        ...
        break;
    default:
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("Invalid gport type in _bcm_dpp_gport_l2_intf_calculate_port_id")));
    }
