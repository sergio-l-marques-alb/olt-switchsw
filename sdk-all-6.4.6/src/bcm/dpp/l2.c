...
{ 
    ...
    if (l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_MC){
        ...
        if (!SOC_DPP_CONFIG(unit)->trill.mc_id) {
            ...
            rv = _bcm_dpp_port_encap_from_fwd_decision(unit, &gport_dummy, &(l2addr->encap_id), &(l2_entry_val->frwrd_info.forward_decision), _BCM_DPP_GPORT_ENCAP_USAGE_GENERAL); 
            BCMDNX_IF_ERR_EXIT(rv);            
        }
    }
...
bcm_petra_l2_egress_create(int unit, bcm_l2_egress_t *egr)
{
    ...
    /* Update SW DB */
    rv = _bcm_dpp_out_lif_overlay_ll_match_add(unit, local_ll_outlif);
    BCMDNX_IF_ERR_EXIT(rv);
