...
int ipv4_mc_example_trap(int unit, int in_port, int out_port){
	bcm_error_t rv = BCM_E_NONE;
	int ingress_intf;
    ...
    int egr_flags = 0;
    bcm_multicast_t mc_id;
    bcm_gport_t mc_gport; /*This mc id traps to BCM_GPORT_BLACK_HOLE*/
    int nof_entries = 6;
    int idx;
    ...
    bcm_mac_t mac_address_next_hop_l  = {0x00, 0x00, 0x00, 0x00, 0xcd, 0x1d}; /* next_hop_mac1 */
    int ing_intf_l;
    bcm_gport_t mc_gport_l; /*This mc id traps to BCM_GPORT_LOCAL_CPU*/
    bcm_multicast_t mc_id_l;
    ...
    int trap_id_handle_r, trap_id_handle_l;
    bcm_rx_trap_config_t config_r, config_l;
    bcm_rx_trap_type_create(unit,0/*flags*/,bcmRxTrapUserDefine,&trap_id_handle_r);
    bcm_rx_trap_type_create(unit,0/*flags*/,bcmRxTrapUserDefine,&trap_id_handle_l);

    /*change dest port for trap - drop */
    bcm_rx_trap_config_t_init(&config_r);
    config_r.flags |= (BCM_RX_TRAP_UPDATE_DEST|BCM_RX_TRAP_REPLACE);
    config_r.trap_strength = 0; /*FOR USER DEFINE set as part of destination */
    config_r.dest_port = BCM_GPORT_BLACK_HOLE;
    rv = bcm_rx_trap_set(unit,trap_id_handle_r,&config_r);

    /*change dest port for trap - send to cpu*/
    bcm_rx_trap_config_t_init(&config_l);
    config_l.flags |= (BCM_RX_TRAP_UPDATE_DEST|BCM_RX_TRAP_REPLACE);
    config_l.trap_strength = 0; /*FOR USER DEFINE set as part of destination */
    config_l.dest_port = BCM_GPORT_LOCAL_CPU;
    rv = bcm_rx_trap_set(unit,trap_id_handle_l,&config_l);

    /*Associate the traps with the gports*/
    BCM_GPORT_TRAP_SET(mc_gport, trap_id_handle_r, 7, 0);
    BCM_GPORT_TRAP_SET(mc_gport_l, trap_id_handle_l, 7, 0);
    ...
    /*
     * create egress objects with different traps
     */
    ...
    /* create egress object with drop trap */
    /*ing_intf_r,mac_address_next_hop_r doesn't affect IPMc packets  */
    rv = create_l3_egress(unit,egr_flags,mc_gport,0,ing_intf_r,mac_address_next_hop_r, &fec[eg_itf_index], &encap_id[eg_itf_index]);
    if (rv != BCM_E_NONE) {
       printf("Error, create egress object, on port=%d, \n", out_port);
    ...
    if(print_level >= 2) {
        printf("created egress object 0x%08x with trap set to drop packets  \n", fec[eg_itf_index]);
    }
    ++eg_itf_index;
...
0     */
    ...
    /* create egress objects with send to local cpu trap */
    egr_flags = BCM_L3_RPF;
    rv = create_l3_egress(unit,egr_flags,mc_gport_l,0,ing_intf_r,mac_address_next_hop_r, &fec[eg_itf_index], &encap_id[eg_itf_index]);
    if (rv != BCM_E_NONE) {
       printf("Error, create egress object, on port=%d, \n", out_port);
    }
    if(print_level >= 2) {
        printf("created egress object 0x%08x with send to local cpu trap \n", fec[eg_itf_index]);
    }
    ++eg_itf_index;
