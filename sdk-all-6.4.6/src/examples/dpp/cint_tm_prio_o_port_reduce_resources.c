...
/*
 * $Id: cint_tm_prio_o_port_reduce_resources.c, v 1.4 Broadcom SDK $
 *
 ...
 * The mapping can adjust bu using changing the above functions and setting OFFSET_START_OF_VOQ, OFFSET_START_OF_VOQ_CONNECTORS.
 ...
 * These queues devided to 2 trunks and each trunk has its own BW (trunk1 - 1.5Mb/s, trunk2 - 1.5Mb/s).
 ...
 * ISQ configuration, half of the isqs will configured to 1.5Mb/s and the other half to 2.5Mb/s
 *   isq_bw = 1500000;
 *   rv = ardon_shaping_configuration(unit, nof_isqs/2, MAX_COS, isq_bw);
 *   if (BCM_FAILURE(rv)) {
...
int ISQ_QUEUE_START = 3072;
int ISQ_QUEUE_END   = 4095;
...
int convert_modport_to_base_voq_connector(int local_port,int ingress_modid)
{
    ...
    return g_info.offset_start_of_voq_connector + local_port * g_info.nof_devices * MAX_COS + ingress_modid * MAX_COS;
}
...
int appl_ardon_stk_init(int unit)
{
     ...
     * Loop on ports start from idx = 1 since this cint assume idx = 0 ==> tm_internal_pkt_port is already defined
     */     
    for (modid = 0; modid < g_info.nof_devices; modid++) {
        for (idx = 1; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
            ...
            /* print pbmp; */
            rv = bcm_port_egress_set(unit,  g_info.tm_internal_pkt_port, modid,  pbmp);
            if (BCM_FAILURE(rv)) {
                printf("Error, in bcm_port_egress_set.\n");
                return rv;
            }
...
int cosq_init_ing_egr_alloc(int unit) 
{
    ...
    for (idx = 0; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
        ...
        for (cos = 0; cos < g_info.num_cos; cos++) {
            ...
            BCM_GPORT_SCHEDULER_SET(g_info.gport_ucast_scheduler[idx][cos], (64 * 1024) + (idx * g_info.num_cos + cos) * 4);
    ...
    for (modid = 0; modid < g_info.nof_devices; modid++) {
        for (idx = 0; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
            ...
            /* If both devices are control by a central CPU, no need to allocate with ID, since we use the ID allocated by the BCM */
		...
		voq_connector_id  = convert_modport_to_base_voq_connector(port, modid);
                ...
                rv = bcm_port_get(unit, g_info.dest_local_gport[idx], &dummy_flags, &interface_info, &mapping_info) ;
                if (BCM_FAILURE(rv)) {
                  printf("_bcm_petra_cosq_core_get connector idx:%d failed. Error %d\n", idx, rv);
                  return rv ;
                }
            ...
            if ((local_erp_port == port) && (modid != g_info.my_modid)) {
               continue;
            }
            config.port = g_info.dest_local_gport[idx] ;
            config.flags = flags ;
...
int cosq_init_ing_egr_connect(int unit, int mymodid) 
{
    ...
    int local_erp_port = -1, num_erp_ports;
    int int_flags;
    int cos;
