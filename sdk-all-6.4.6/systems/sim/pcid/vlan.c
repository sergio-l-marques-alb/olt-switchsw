                             ...
                             vlan_xlate_entry_t *entry, int dual)
{
    ...
    if (SOC_IS_TD2_TT2(pcid_info->unit)) {
        bits = soc_td2_vlan_xlate_base_entry_to_key(pcid_info->unit, entry,
                                                    key);
...
#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TD2_TT2(pcid_info->unit)) {
        bits = soc_td2_egr_vlan_xlate_base_entry_to_key(pcid_info->unit,
                                                        entry, key);
                                     ...
                                     int dual)
{
    ...
    addr = soc_reg_addr_get(pcid_info->unit, reg, REG_PORT_ANY, 0, FALSE, &blk,
                            &acc_type);
