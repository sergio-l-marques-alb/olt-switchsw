 ...
 */
bcm_gport_t erp_gport[BCM_PIPES_MAX];
...
int_ports_dump(int unit)
{
    ...
    rv = bcm_port_internal_get(unit, flags, BCM_PIPES_MAX, erp_gport, &int_port_count);
   ...
   for(i=0 ; i<int_port_count ; i++) {
       printf("ERP gport(0x%08x) port_cnt(%d)\n", erp_gport[i], int_port_count);
   }

    rv = l2__olp_port__get(unit, &olp_gport);
    if (rv != BCM_E_NONE) {
        printf("Error, l2__olp_port__get\n");
        return rv;
    }

    printf("OLP gport(0x%08x)\n", olp_gport);
