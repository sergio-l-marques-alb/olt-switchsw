 ...
 *                                                                                                                                                  *                                                                                                                                           * 
 * The CINT provides an example of AC 1:1 Protection for UC & MC traffic.                                                                           *
 *                                                                                                                                                  *
...
ac_1to1_coupled_protection_s g_ac_1to1_coupled_protection = { 
        ...
        /*  AC configurations
           ...
           {{ 201,      6,         0,          0   }, /* Out-ACs */
            ...
            {0x00, 0x11, 0x00, 0x00, 0x00, 0x13},   /* MAC Address */
            20,                                     /* VSI */
            0,                                      /* FEC failover_id */
            0};                                     /* Egress failover_id */
...
int ac_1to1_coupled_protection_init(int unit, ac_1to1_coupled_protection_s *params) {
    ...
    rv = bcm_failover_create(unit, BCM_FAILOVER_FEC, &g_ac_1to1_coupled_protection.fec_failover_id);
    if (rv != BCM_E_NONE) {
    ...
    /* Create a Egress failover ID for MC */
    rv = bcm_failover_create(unit, BCM_FAILOVER_ENCAP, &g_ac_1to1_coupled_protection.egress_failover_id);
    if (rv != BCM_E_NONE) {
        printf("Error, ac_1to1_coupled_protection_init failed during Egress bcm_failover_create, rv - %d\n", rv);
        return rv;
...
int ac_1to1_coupled_protection__set_in_ac(int unit, ac_1to1_coupled_protection_s *ac_1to1_coupled_protection_info) {
    ...
    bcm_if_t encap_id;
    int rv, mc_group, enable;

    /* Consider using Ingress_only flag */
...
int ac_1to1_coupled_protection__set_protection_acs(int unit, ac_1to1_coupled_protection_s *ac_1to1_coupled_protection_info) {
    ...
    out_ac.egress_failover_id = ac_1to1_coupled_protection_info->egress_failover_id;
    ...
    bcm_vlan_port_t_init(&out_ac);
    out_ac.flags = BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_EGRESS_PROTECTION;
    out_ac.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;
    ...
    out_ac.vsi = ac_1to1_coupled_protection_info->vsi;
    out_ac.vlan_port_id = ac_1to1_coupled_protection_info->out_ac[FAILOVER_PROTECTING].vlan_port_id + 1;
    out_ac.failover_id = ac_1to1_coupled_protection_info->fec_failover_id;
 ...
 *   protection_ac_s: Configuration info for a single In-AC.
 */
void ac_1to1_coupled_protection__struct_get(int unit, ac_1to1_coupled_protection_s *param){
 ...
 */
int ac_1to1_coupled_protection__start_run(int unit, ac_1to1_coupled_protection_s *params) {
    ...
    mact_properties.mac_address = g_ac_1to1_coupled_protection.mac_address;
 ...
 */
int ac_1to1_coupled_protection_with_ports__start_run(int unit, int in_port, int out_port_working, int out_port_protecting) {
    int rv;

    ac_1to1_coupled_protection_s param;

    ac_1to1_coupled_protection__struct_get(unit, &param);
    ...
    return ac_1to1_coupled_protection__start_run(unit, &param);
}


/* Performs failover setting for the two protection Out-ACs
 *  
 * INPUT: is_egress_protection  - Determine if the setting is for Egress-Protection or FEC-Protection
 *        is_enable - The failover state to set. 1: Working Path, 0: Protecting Path.
 */
int ac_1to1_coupled_protection_with_ports__failover_set(int unit, int is_egress_protection, int is_enable) {

    int rv;
    bcm_failover_t  failover_id = (is_egress_protection) ? g_ac_1to1_coupled_protection.egress_failover_id : g_ac_1to1_coupled_protection.fec_failover_id;

    rv = bcm_failover_set(unit, failover_id, is_enable);    
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_failover_set failover_id:  0x%08x, is_egress: %d, is_enable: %d \n", failover_id, is_egress_protection, is_enable);
        return rv;
    }
}
