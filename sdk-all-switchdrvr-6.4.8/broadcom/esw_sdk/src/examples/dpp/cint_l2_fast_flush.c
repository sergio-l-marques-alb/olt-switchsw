 ...
 *            using the function set_port_to_ring(0);
 *
 *
 *  Step 2:
 ...
 *          set_port_to_ring(0, g_gport[1], g_failover_id[1], erp_mode);
 ...
 *          perform_fast_flush(0, g_gport[2]);
 *      Calling sequence:
...
int create_ring_protection_ports(int *units_ids, int nof_units, int out_sysport, int in_sysport)
{
    ...
    /* Create the vlan - ports as defined in cint_vswitch_metro_mp.c */
    rv = vswitch_metro_mp_run_with_defaults_multi_device(units_ids, nof_units, port_1, port_2, port_3);
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
    	return rv;
    }
