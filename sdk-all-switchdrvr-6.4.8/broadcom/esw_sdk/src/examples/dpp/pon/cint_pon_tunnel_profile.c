/* $Id: cint_pon_tunnel_profile.c,v 1.3 Broadcom SDK $
...
* 
* File: cint_pon_tunnel_profile.c
...
*  7. Add VLAN range info to PON PP port if necessary:
...
*        - call bcm_port_pon_tunnel_add()
*  9. Map tunnel id 1002/1003 to PON PP port 13 (indirectly mapped to PON tunnel profile id 1).
*        - call bcm_port_pon_tunnel_map_set()
...
*  13. Add VLAN range info to PON PP port if necessary:
*        - Call bcm_vlan_translate_action_range_add() with action bcmVlanActionCompressed.
*  14. Create PON PP port with different PON tunnel profile id 2.
*        - call bcm_port_pon_tunnel_add()
*  15. Map tunnel id 1004/1005 to PON PP port 13 (indirectly mapped to PON tunnel profile id 2).
*        - call bcm_port_pon_tunnel_map_set()
*  16. Set TPIDs of PON pp port 21.
*        - call bcm_port_tpid_delete_all()
*        - call bcm_port_tpid_add()
*        - call bcm_port_inner_tpid_set()
*  17. Set VLAN domain of PON pp port 21 to 21.
*        - call bcm_port_class_set()
*  18. Set PON pp port 21 to discard all packets.
...
*
*     Tunnel-ID 1001 SVLAN 20 PON_PP_PORT 5 <------------------------------------> SVLAN 210
*     Tunnel-ID 1001 SVLAN 20 CVLAN 21 PON_PP_PORT 5 <---------------------------> SVLAN 202 CVLAN 21
*     Tunnel-ID 1001 SVLAN range 50~60 PON_PP_PORT 5 <---------------------------> SVLAN 500
*
*     Tunnel-ID 1002 SVLAN 30 PON_PP_PORT 13 <-----------------------------------> SVLAN 300
*     Tunnel-ID 1002 SVLAN 30 CVLAN 31 PON_PP_PORT 13 <--------------------------> SVLAN 303 CVLAN 31
*     Tunnel-ID 1002 SVLAN range 80~90 PON_PP_PORT 13 <--------------------------> SVLAN 800
*
*     Tunnel-ID 1003 SVLAN 30 PON_PP_PORT 13 <-----------------------------------> SVLAN 310
*     Tunnel-ID 1003 SVLAN 30 CVLAN 31 PON_PP_PORT 13 <--------------------------> SVLAN 304 CVLAN 31
*     Tunnel-ID 1003 SVLAN range 80~90 PON_PP_PORT 13 <--------------------------> SVLAN 1000
*
*     Tunnel-ID 1004 SVLAN 40 PON_PP_PORT 21 <-----------------------------------> SVLAN 400
*     Tunnel-ID 1005 SVLAN 40 PON_PP_PORT 21 <-----------------------------------> SVLAN 410
*
* Traffic:
...
*
*  4. PON Port 5 PON PP Port 5 Tunnel-ID 1001 SVLAN 20 <-CrossConnect-> NNI Port 129 SVLAN 210
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1001
*              -   VLAN tag: VLAN tag type 0x8100, VID = 20
...
*
*  5. PON Port 5 PON PP Port 5 Tunnel-ID 1001 SVLAN 20 CVLAN 21 <-CrossConnect-> NNI Port 129 SVLAN 202 CVLAN 21
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1001
*              -   VLAN tag: VLAN tag type 0x8100, VID = 20, VLAN tag type 0x9100, VID = 21
...
*
*  6. PON Port 5 PON PP Port 5 Tunnel-ID 1001 SVLAN range 50~60 <-CrossConnect-> NNI Port 129 SVLAN 500
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1001
*              -   VLAN tag: VLAN tag type 0x8100, VID = 55
...
*
*  7. PON Port 5 PON PP Port 13 Tunnel-ID 1002 SVLAN 30 <-CrossConnect-> NNI Port 129 SVLAN 300
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1002
...
*
*  8. PON Port 5 PON PP Port 13 Tunnel-ID 1002 SVLAN 30 CVLAN 31 <-CrossConnect-> NNI Port 129 SVLAN 303 CVLAN 31
...
*              -   ethernet header with any DA, SA 
*              -   VLAN tag: VLAN tag type 0x8100, VID = 30, VLAN tag type 0x9100, VID = 31
*  9. PON Port 5 PON PP Port 13 Tunnel-ID 1002 SVLAN range 80~90 <-CrossConnect-> NNI Port 129 SVLAN 800
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1002
...
*
*  10. PON Port 5 PON PP Port 13 Tunnel-ID 1003 SVLAN 30 <-CrossConnect-> NNI Port 129 SVLAN 310
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1003
*              -   VLAN tag: VLAN tag type 0x8100, VID = 30
...
*
*  11. PON Port 5 PON PP Port 13 Tunnel-ID 1003 SVLAN 30 CVLAN 31 <-CrossConnect-> NNI Port 129 SVLAN 304 CVLAN 31
...
*              -   ethernet header with any DA, SA 
*              -   VLAN tag: VLAN tag type 0x8100, VID = 30, VLAN tag type 0x9100, VID = 31
*  12. PON Port 5 PON PP Port 13 Tunnel-ID 1003 SVLAN range 800~900 <-CrossConnect-> NNI Port 129 SVLAN 1000
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1003
...
*
*  13. PON Port 5 PON PP Port 21 Tunnel-ID 1004 SVLAN 40 <-CrossConnect-> NNI Port 129 SVLAN 400
...
*
*  14. PON Port 5 PON PP Port 21 Tunnel-ID 1005 SVLAN 40 <-CrossConnect-> NNI Port 129 SVLAN 410
*    - Upstream is dropped, becasue PON pp port 21 drops all packets.
*        - From PON port:
*              -   ethernet header with any DA, SA 
*              -   Tunnel ID:1005
...
*      BCM> cint examples/dpp/pon/cint_pon_tunnel_profile.c
*      BCM> cint
*      cint> pon_tunnel_service_init(unit, pon_port, nni_port);
...
int pon_tunnel_profile_set(int unit, bcm_port_t port_pon)
{
    int rv = 0;
    int index;
    int j;
    ...
    for (index = 0; index < num_of_tunnel_profile; index++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            if ((pon_tunnel_profile[index].vlan_range[j].outer_vlan_low != BCM_VLAN_INVALID) &&
                (pon_tunnel_profile[index].vlan_range[j].inner_vlan_low != BCM_VLAN_INVALID)) {
                action.dt_outer = bcmVlanActionCompressed;
                action.new_outer_vlan = pon_tunnel_profile[index].vlan_range[j].outer_vlan_low;
                action.dt_inner = bcmVlanActionCompressed;
                action.new_inner_vlan = pon_tunnel_profile[index].vlan_range[j].inner_vlan_low;                
            } else if (pon_tunnel_profile[index].vlan_range[j].outer_vlan_low != BCM_VLAN_INVALID) {
                action.ot_outer = bcmVlanActionCompressed;
                action.new_outer_vlan = pon_tunnel_profile[index].vlan_range[j].outer_vlan_low;
            } else if (pon_tunnel_profile[index].vlan_range[j].inner_vlan_low != BCM_VLAN_INVALID) {
                action.it_inner = bcmVlanActionCompressed;
                action.new_outer_vlan = pon_tunnel_profile[index].vlan_range[j].inner_vlan_low;
            } else {
                continue;
            }
            ...
            rv = bcm_vlan_translate_action_range_add(
                         unit, 
                         tunnel_port_id, 
                         pon_tunnel_profile[index].vlan_range[j].outer_vlan_low, 
                         pon_tunnel_profile[index].vlan_range[j].outer_vlan_high, 
                         pon_tunnel_profile[index].vlan_range[j].inner_vlan_low, 
                         pon_tunnel_profile[index].vlan_range[j].inner_vlan_high, 
                         &action);
            if (rv != BCM_E_NONE)
            {
...
int pon_tunnel_service_init(int unit, bcm_port_t port_pon, bcm_port_t port_nni)
{
    int rv = 0;
    int index;
    uint32 flags;
    ...
    /* PON Port 5 Tunnel-ID 1003 SVLAN 40 <-CrossConnect-> NNI Port 129 SVLAN 410*/
    index++;
    pon_tunnel_mapping[index].service_mode = otag_to_otag2;
    ...
    pon_tunnel_profile_set(unit, port_pon);
...
int pon_tunnel_mapping_1_1_service(int unit)
{
    ...
    for (index = 0; index < num_of_tunnel_mapping; index++)
    {
        ...
        /* Create NNI LIF */
        nni_lif_create(unit, nni_port, nni_lif_type, is_with_id, down_ovlan, down_ivlan, &nni_gport, &encap_id);
