 ...
 * @return int 
 */
int oam_eth_o_mpls_o_eth_run_with_defaults(int unit, int ac_port, int pwe_port, int ac_inner_vlan, int ac_outer_vlan)
{
	int rv;
	...
	BCM_OAM_OPCODE_SET(action, 1); /* CCM*/
	BCM_OAM_ACTION_SET(action, bcmOAMActionUcFwd);
	BCM_OAM_ACTION_SET(action, bcmOAMActionMcFwd);
	BCM_GPORT_LOCAL_SET(action.destination, ac_port);

	rv =  bcm_oam_endpoint_action_set(0, ep.id, & action); 
	if (rv != BCM_E_NONE) {
		printf("Error in bcm_oam_endpoint_action_set\n");
		print rv;
		return rv;
	}
