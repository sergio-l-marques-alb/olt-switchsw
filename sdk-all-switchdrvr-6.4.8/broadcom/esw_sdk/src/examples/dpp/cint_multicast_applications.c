...
/*    
 * This application creates Ingress multicast that replicates a single copy to each 
 * destionation FAP (holding a member port in that MC group). 
...
int device_level_scheduled_multicast(int unit,int multicast_id,bcm_gport_t* destinations,int* cuds,int nof_destinations)
{
    ...
    for (index = 0; index < nof_destinations; index++) {
         if (BCM_GPORT_IS_LOCAL(destinations[index])) {
             rv = bcm_stk_my_modid_get(unit,&modid);
             if (rv != BCM_E_NONE) {
                 printf("(%s) \n",bcm_errmsg(rv));
                 return rv;
             }
         } else if (BCM_GPORT_IS_MODPORT(destinations[index])) {
             modid = BCM_GPORT_MODPORT_MODID_GET(destinations[index]);
         } else {
             /* System port */
         ...
         rv = bcm_stk_gport_sysport_get(unit,temp[0],&dest_erp);
         if (rv != BCM_E_NONE) {
             printf("(%s) \n",bcm_errmsg(rv));
             return rv;
         }
    ...
    for (index = 0; index < nof_destinations; index++) {
        if (BCM_GPORT_IS_LOCAL(destinations[index])) {
            port = BCM_GPORT_LOCAL_GET(destinations[index]);
        } else if (BCM_GPORT_IS_MODPORT(destinations[index])) {
            port = BCM_GPORT_MODPORT_PORT_GET(destinations[index]);
        } else {
            /* System port */
            rv = bcm_stk_sysport_gport_get(unit,destinations[index],&(temp[0]));
            if (rv != BCM_E_NONE) {
            ...
            port = BCM_GPORT_MODPORT_PORT_GET(temp[0]);
        }
        /* CUD on multicast ingress must be the multicast egress group id */
        cud = cuds[index];                             
        BCM_GPORT_LOCAL_SET(temp[0],port);
        rv = bcm_multicast_egress_add(unit,multicast_group,temp[0],cud);
        if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }      
