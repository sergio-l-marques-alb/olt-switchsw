                                ...
                                /* out */ bcm_field_group_t *group) 
{
  ...
  bcm_field_presel_create_id(unit, 0);
	bcm_field_qualify_ForwardingType(unit, 0 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeMpls);
   ...
   * Take the 1st byte of the header after MPLS that is after Ethernet header
   */
  bcm_field_data_qualifier_t_init(&ip_version); 
  ip_version.offset_base = bcmFieldDataOffsetBaseSecondHeader; /* Header after MPLS over Eth */
