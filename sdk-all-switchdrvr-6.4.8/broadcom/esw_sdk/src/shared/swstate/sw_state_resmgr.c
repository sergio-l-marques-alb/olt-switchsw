...
#define RES_UNIT_CHECK(_unit) \
    {                           \
        ...
        if (RESMGR_ACCESS.is_allocated(_unit, &_exists)) { return _SHR_E_INTERNAL;} \
        if (!_exists) { \
            LOG_ERROR(BSL_LS_SOC_COMMON, \
...
#define RES_POOL_VALID_CHECK(_unit, _pool) \
    {   \
        uint16 _resPoolCount;  \
        if (RESMGR_ACCESS.resPoolCount.get(_unit, &_resPoolCount)) { return _SHR_E_INTERNAL;} \
...
#define RES_POOL_EXIST_CHECK(_unit, _pool) \
    {   \
        uint8 _exists;  \
        if (RESMGR_ACCESS.pool.is_allocated(_unit, _pool, &_exists)) { return _SHR_E_INTERNAL;} \
        if (!_exists) { \
            LOG_ERROR(BSL_LS_SOC_COMMON, \
...
#define RES_TYPE_VALID_CHECK(_unit, _type) \
    {   \
        uint16 _resTypeCount;  \
        if (RESMGR_ACCESS.resTypeCount.get(_unit, &_resTypeCount)) { return _SHR_E_INTERNAL;} \
...
#define RES_TYPE_EXIST_CHECK(_unit, _type) \
    {   \
        uint8 _exists;  \
        if (RESMGR_ACCESS.res.is_allocated(_unit, _type, &_exists)) { return _SHR_E_INTERNAL;} \
        if (!_exists) { \
            LOG_ERROR(BSL_LS_SOC_COMMON, \
...
sw_state_res_tag_set(int unit,
    ...
    _sw_state_res_type_desc_t thisType;
    int result = _SHR_E_NONE;
    LOG_DEBUG(BSL_LS_SOC_COMMON, (BSL_META("(%d, %d, %p, %d, %d) enter\n"), unit, res_id, (void*)tag, offset, count));
    ...
    if (_sw_state_res_alloc_mgrs[thisPool.resManagerType].tag_set) {
        ...
        /* not supported by this allocator */
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META("allocator type %s does not support tagged alloc\n"), _sw_state_res_alloc_mgrs[thisPool.resManagerType].name));
        result = _SHR_E_UNAVAIL;
    }
...
sw_state_res_tag_get(int unit,
    ...
    _sw_state_res_type_desc_t thisType;
    int result = _SHR_E_NONE;
    LOG_DEBUG(BSL_LS_SOC_COMMON, (BSL_META("(%d, %d, %p, %d) enter\n"), unit, res_id, (void*)tag, element));
