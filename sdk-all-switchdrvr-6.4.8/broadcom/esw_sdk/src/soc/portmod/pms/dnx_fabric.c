...
int dnx_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "dnx_fabric", NULL, VERSION(2), 1, SOC_WB_ENGINE_PRE_RELEASE);
...
int dnx_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    for (i = 0; i < phy_count; ++i) {
        core_access[0].access.lane_mask |= 1 << i;
    }
    SOCDNX_IF_ERR_EXIT(portmod_port_phychain_core_init(core_access, fabric_data->nof_phys, &core_config, &status)); 
...
int dnx_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(portmod_access_get_params_t_init(unit, &params));
    phy_access.access.lane_mask = 0;
    for (phyn = 0; phyn < phys_count; phyn++) {
        params.phyn = phyn;
        phy_access.access.lane_mask |= 1 << phyn;
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &nof_phys, NULL));
...
int dnx_fabric_port_encoding_set(int unit, int port, pm_info_t pm_info, uint32 properties, portmod_port_pcs_t encoding)
{
    ...
    for (i = 0; i < (is_bypass ? 4 : 1); i++)
    {
        SOCDNX_IF_ERR_EXIT(READ_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index + i, &reg64_val));
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_FMAC_RSF_CONFIGURATIONr(unit, fmac_blk_id_masked, phy_index + i, reg64_val));
    }
                                    ...
                                    phymod_phy_access_t* phy_access_arr, int* nof_phys, int* is_most_ext)
{
    ...
    if (is_bypass) {
        ...
        phy_access_arr[0].access.lane_mask = 0xf;
...
int dnx_fabric_port_cl72_set(int unit, int port, pm_info_t pm_info, uint32_t enable)
{
...
int dnx_fabric_port_cl72_get(int unit, int port, pm_info_t pm_info, uint32_t* enable)
{
    phymod_phy_access_t phy_access[1];
    int chain_length = 0;
    uint32_t tmp_enable;
