...
int soc_jer_ocb_dram_buffer_conf_calc(int unit) 
{
    ...
    /* OCB_0, OCB_1, FMC_0 and FMC_1 transitions needs to be on 16k alignment when they are enabled, since they are consecutive, verifying that their sizes are aligned to 16k is sufficient */
    if ((ocb_0_size % (16 * 1024) != 0) || (ocb_1_size % (16 * 1024) != 0) || (fmc_0_size % (16 * 1024) != 0) || (fmc_1_size % (16 * 1024) != 0)){
    ...
    if ( ((int)mnmc_0_size < 0) || ((int)mnmc_1_size < 0) ){
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Invalid configuration, insufficent dram memory to support configuration - try to reduce dbuff size")));  
    }
    ...
    /* handle overflow of start and end range for invalid ranges */
    if (ocb_0_size == 0) 
    {
        /* due to a HW issue needed to change dbuff ranges in this way, and although it may appear size is 1, it is essentially 0 so no need to modify size (same goes for ocb_1 and fmc_0/1 below) */
        dbuffs->ocb_0.start = 0;
        dbuffs->ocb_0.end = 0;
        dbuffs->ocb_1.start = 1;
        dbuffs->ocb_1.end = 1;
        dbuffs->fmc_0.start = 2;
        if (fmc_0_size == 0) 
        {
            dbuffs->fmc_0.end = 2;
            dbuffs->fmc_1.start = 3;
            dbuffs->fmc_1.end = 3;
            dbuffs->mnmc_0.start = 4;
        }
    }
