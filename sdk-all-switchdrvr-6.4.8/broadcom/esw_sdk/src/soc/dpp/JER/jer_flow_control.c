  ...
  )
{
    ...
    rv = soc_reg_field32_modify(unit, NBIH_FC_RESETr, REG_PORT_ANY, FC_RESETf, 0);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    for(interface_number = 0; interface_number < SOC_TMC_FC_NOF_OOB_IDS; interface_number++)
    {
        ...
        if(tm->fc_oob_mode[interface_number] & SOC_DPP_FC_CAL_MODE_RX_ENABLE)
        {
           ...
           /*  Configure polarity for ILKN FC indications */
            ...
            cal_mode_ndx = (tm->fc_oob_type[interface_number] == SOC_TMC_FC_CAL_TYPE_ILKN ||
                            tm->fc_oob_type[interface_number] == SOC_TMC_FC_CAL_TYPE_COE ||
                            tm->fc_oob_type[interface_number] == SOC_TMC_FC_CAL_TYPE_E2E) ? 
  ...
  )
{
    ...
    rv = soc_port_sw_db_first_phy_port_get(unit, port, &first_phy /*one based*/);
    SOCDNX_IF_ERR_EXIT(rv);

    rv = soc_jer_qsgmii_offsets_remove(unit, first_phy, &first_lane);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    if (info->gen_src == SOC_TMC_FC_GEN_SRC_NIF) {
        ...
        rv = READ_NBIH_FC_PFC_QMLF_MASKr(unit, reg_offset, &pfc_map_rx_mlf_reg);
        SOCDNX_IF_ERR_EXIT(rv);
  
        soc_reg_field_set(unit, NBIH_FC_PFC_QMLF_MASKr, &pfc_map_rx_mlf_reg, rx_mlf_pfc_fld[fld_offset], info->nif_cls_bitmap);

        rv = WRITE_NBIH_FC_PFC_QMLF_MASKr(unit, reg_offset, pfc_map_rx_mlf_reg);
        SOCDNX_IF_ERR_EXIT(rv);
    }
    /* Set the bit in the MLF PFC bitmap } */
  ...
  )
{
    ...
    if (info->gen_src == SOC_TMC_FC_GEN_SRC_NIF) {
        ...
        rv = READ_NBIH_FC_TX_GEN_LLFC_FROM_QMLF_MASKr(unit, llfc_map);
        SOCDNX_IF_ERR_EXIT(rv);
        if (info->nif_enable) {
        ...
        rv = WRITE_NBIH_FC_TX_GEN_LLFC_FROM_QMLF_MASKr(unit, llfc_map);
        SOCDNX_IF_ERR_EXIT(rv);
    }
    /* Set the bit in the MLF LLFC bitmap } */
  ...
  )
{
    ...
    if (info->gen_src == SOC_TMC_FC_GEN_SRC_NIF) {
        ...
        info->nif_cls_bitmap = soc_reg_field_get(unit, NBIH_FC_PFC_QMLF_MASKr, pfc_map_rx_mlf_reg, rx_mlf_pfc_fld[fld_offset]);
    }
    /* Get the bit in the MLF PFC bitmap } */
  ...
  )
{
    ...
    rv = soc_reg_above_64_field64_modify(unit, ECI_BLOCKS_SOFT_INITr, REG_PORT_ANY, 0, BLOCKS_SOFT_INIT_55f,  field64);    /* Init block for CFC */
    SOCDNX_IF_ERR_EXIT(rv);
    COMPILER_64_SET(field64,0,0x0);
    rv = soc_reg_above_64_field64_modify(unit, ECI_BLOCKS_SOFT_INITr, REG_PORT_ANY, 0, BLOCKS_SOFT_INIT_55f,  field64);
    SOCDNX_IF_ERR_EXIT(rv);
    /* Enable the FC after configuring it } */
  ...
  )
{
    ...
    if (if_type == SOC_PORT_IF_ILKN) {  
        /* LLFC */
        if (info->llfc.clear > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_hrf_max)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("llfc.clear is out of range")));
        }

        if (info->llfc.set > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_hrf_max)) {
        ...
        /* LLFC */
        if (info->llfc.clear > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_max)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("llfc.clear is out of range")));
        }

        if (info->llfc.set > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_max)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("llfc.set is out of range")));
        }

        /* PFC */
        if (info->pfc.clear > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_max)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("pfc.clear is out of range")));
        }

        if (info->pfc.set > SOC_DPP_DEFS_GET(unit, fc_rx_mlf_max)) {
  ...
  )
{
    ...
    if (interface_type == SOC_PORT_IF_ILKN) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit, port, &first_phy /*one based*/));
        SOCDNX_IF_ERR_EXIT(soc_jer_qsgmii_offsets_remove(unit, first_phy, &first_lane));
        
        llfc_reg = (first_lane < SOC_JERICHO_NOF_PORTS_NBIH) ?  NBIH_RX_MLF_LLFC_THRESHOLDS_CONFIGr : NBIL_RX_MLF_LLFC_THRESHOLDS_CONFIGr;
        pfc_reg = (first_lane < SOC_JERICHO_NOF_PORTS_NBIH) ?  NBIH_RX_MLF_PFC_THRESHOLDS_CONFIGr : NBIL_RX_MLF_PFC_THRESHOLDS_CONFIGr;
        reg_port = (first_lane < SOC_JERICHO_NOF_PORTS_NBIH) ? REG_PORT_ANY : first_lane > (2 * SOC_JERICHO_NOF_LANES_PER_NBI); 
        reg_offset = ((first_lane - 1) % SOC_JERICHO_NOF_LANES_PER_NBI) / NUM_OF_LANES_IN_PM;
  ...
  )
{
    ...
    case ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS:
        vsq_ndx_tmp += (ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_SIZE)*2;
    ...
    case ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS:
        vsq_ndx_tmp += ARAD_ITM_VSQ_GROUP_CTGRY_SIZE*2;
    ...
    case ARAD_ITM_VSQ_GROUP_LLFC:
        vsq_ndx_tmp = first_phy + (is_ocb_only?148:0);
	    break;
    default:
        break;
    }
  ...
  )
{
    ...
    *fc_index = tx_cal.fc_index;
    ...
    if (fc_src == SOC_TMC_FC_GEN_CAL_SRC_STE) {
        ...
        *vsq_group = SOC_TMC_ITM_VSQ_GROUP_STTSTCS_TAG; /* Sttstcs VSQ */
        *cal_tx_src_type = SOC_TMC_FC_SRC_TYPE_VSQ;
    }
    else if (fc_src == SOC_TMC_FC_GEN_CAL_SRC_LLFC_VSQ) {
        *vsq_group = SOC_TMC_ITM_VSQ_GROUP_SRC_PORT;    /* Src Port VSQ */
        *cal_tx_src_type = SOC_TMC_FC_SRC_TYPE_VSQ;
    }
    else if (fc_src == SOC_TMC_FC_GEN_CAL_SRC_PFC_VSQ) {
        *vsq_group = SOC_TMC_ITM_VSQ_GROUP_PG;          /* PG VSQ */
        *cal_tx_src_type = SOC_TMC_FC_SRC_TYPE_VSQ;
    }
    else if (fc_src == SOC_TMC_FC_GEN_CAL_SRC_GLB_RCS) {
        *cal_tx_src_type = SOC_TMC_FC_SRC_TYPE_GLB;     /* GLB */
    }
    else if (fc_src == SOC_TMC_FC_GEN_CAL_SRC_LLFC) {
        *cal_tx_src_type = SOC_TMC_FC_SRC_TYPE_NIF;     /* NIF */
    }
    else {
  ...
  )
{
    ...
    if (fc_status_key->intf_type == SOC_TMC_FC_INTF_TYPE_NIF) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit, fc_status_key->port, &first_phy /*one based*/));
        SOCDNX_IF_ERR_EXIT(soc_jer_qsgmii_offsets_remove(unit, first_phy, &first_lane));
        first_phy--;
        first_lane--; /* first_lane returned is one-based */
    ...
    switch(res_type) {
    case SOC_TMC_FC_SRC_TYPE_GLB:  /* Global */
        /* 1.1 IQM block*/
        ...
        break;
    case SOC_TMC_FC_SRC_TYPE_VSQ:  /* VSQ A/B/C/D/E/F */
        /* A/B/C/D */
        if ((vsq_type >= SOC_TMC_ITM_VSQ_GROUP_CTGRY) && (vsq_type <= SOC_TMC_ITM_VSQ_GROUP_STTSTCS_TAG)) {
            ...
            for (i = 0; i < 2; i++) {
                llfc_src_id = (i == 0) ? src_id : src_id + 148;
        ...
        break;
    case SOC_TMC_FC_SRC_TYPE_NIF:  /* Nif */
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_qsgmii_offsets_remove(unit, (first_phy+1), &first_lane));
    first_lane--; /* first_lane returned is one-based */
  ...
  )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_valid_ports_get(unit, 0, &pbmp));
    SOC_PBMP_ITER(pbmp, port_i) {
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_first_phy_port_get(unit, port_i, &first_phy_i /*one based*/));
        first_phy_i--;
  ...
  )
{
    ...
    {
        if (fc_status_key->calendar_fc_type == SOC_TMC_FC_CALENDAR_TYPE_LLFC) /* LLFC */
        {
            reg = NBIH_ILKN_RX_LLFC_FROM_RX_CNTr;
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, port, &reg_val32));
            fc_status_info->nbi_ilkn_rx_llfc_cnt = reg_val32;

            reg = NBIH_ILKN_TX_LLFC_STOP_TX_CNTr;
            SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, reg, REG_PORT_ANY, port, &reg_val32));
            fc_status_info->nbi_ilkn_llfc_stop_tx_cnt = reg_val32;            
        }
        else if (fc_status_key->calendar_fc_type == SOC_TMC_FC_CALENDAR_TYPE_CHFC) /* CHFC */
        {
            /* Parse tx calendar entry */
            ...
            *reg_val32_hi = COMPILER_64_HI(reg_val64);
            ...
            case SOC_TMC_FC_REC_CAL_DEST_GENERIC_PFC:
                SOCDNX_IF_ERR_EXIT(jer_fc_pfc_generic_bitmap_get(unit, port, fc_index, &pfc_bitmap));
        ...
        *fld_val32 = soc_reg64_field32_get(unit, reg, reg_val64, fld);
        ...
        else if (fc_status_key->calendar_fc_type == SOC_TMC_FC_CALENDAR_TYPE_CHFC) /* CHFC */
        {
            ...
            *reg_val32_lo = COMPILER_64_LO(reg_val64);
            ...
            /* 2.4 EGQ block */
            SOCDNX_IF_ERR_EXIT(jer_fc_pfc_generic_bitmap_get(unit, port, calendar_id, &pfc_bitmap));
            SOCDNX_IF_ERR_EXIT(_jer_fc_egq_pfc_status_info_get(unit, TRUE, pfc_bitmap.core, 0, 0, fc_status_info));
        }
        else {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("Not support calendar_fc_type(%d)\n"), fc_status_key->calendar_fc_type));
  ...
  )
{
    ...
    soc_field_t
        fld = INVALIDf, 
        fld_hcfc_crc_err_arr[] = {HCFC_OOB_RX_0_CRC_ERR_CTR_0f, 
    ...
    if (fc_status_key->direction == SOC_TMC_FC_DIRECTION_GEN) {
        ...
        case SOC_TMC_FC_REC_CAL_DEST_GENERIC_PFC:
            ...
            SOCDNX_IF_ERR_EXIT(jer_fc_pfc_generic_bitmap_get(unit, port+6, fc_index, &pfc_bitmap));
            SOCDNX_IF_ERR_EXIT(_jer_fc_egq_pfc_status_info_get(unit, TRUE, pfc_bitmap.core, 0, 0, fc_status_info));
            break;
        default:
        ...
        switch(intf_type)
        {
        case SOC_TMC_FC_INTF_TYPE_ILKN: /* ILKN OOB */
            reg = CFC_OOB_RX_ERRr;
            ...
            break;
        case SOC_TMC_FC_INTF_TYPE_SPI: /* SPI OOB */
            reg = CFC_OOB_RX_ERRr;
            ...
            break;
        case SOC_TMC_FC_INTF_TYPE_HCFC: /* HCFC OOB */
            reg = CFC_OOB_RX_ERRr;
            ...
            for (i = 0; i < 5; i++) {
                SOCDNX_IF_ERR_EXIT(soc_reg64_get(unit, reg, REG_PORT_ANY, 0, &reg_val64));
                fc_status_info->cfc_hcfc_rx_crc_err_cnt[i] = soc_reg64_field32_get(unit, reg, reg_val64, fld_hcfc_crc_err_arr[i]);
            }
