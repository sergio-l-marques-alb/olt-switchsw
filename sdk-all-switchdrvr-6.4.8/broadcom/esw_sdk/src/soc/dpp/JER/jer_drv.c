...
soc_jer_ilkn_phy_pbmp_get(int unit, soc_port_t port, uint32 id, uint32 default_nof_lanes, soc_jer_ucode_port_config_t* config)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_jer_nif_ilkn_pbmp_get(unit, port, id, &phy_pbmp));
...
{
    uint32 flags = 0,is_fiber_perf, is_pon, init_vid_enabled;
    uint32 nof_channels = 0;
    int port_i;
    int primary_port = 0, port_offset = 0;
    soc_jer_ucode_port_config_t port_config;
    int rv, core, val, is_channelized;
    soc_info_t          *si;
    ...
    int unit)
{
    ...
    prop_value = soc_property_get(unit, prop_key, 4096);
    /* Both multiples of 4k, and multiple of 4k -1 are acceptable. */
    if (prop_value % 4096 == 4095) {
        prop_value++;
    }
    if (prop_value % 4096 != 0) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Soc property RIF_ID_MAX must be a multiple of 4096.")));
    }
...
int soc_jer_specific_info_config_direct(int unit)
{
    ...
    SOC_DPP_CONFIG(unit)->jer->nif.ilkn_over_fabric[0] = soc_property_suffix_num_get(unit, 4, spn_USE_FABRIC_LINKS_FOR_ILKN_NIF, "ilkn", 0);
    SOC_DPP_CONFIG(unit)->jer->nif.ilkn_over_fabric[1] = soc_property_suffix_num_get(unit, 5, spn_USE_FABRIC_LINKS_FOR_ILKN_NIF, "ilkn", 0);
