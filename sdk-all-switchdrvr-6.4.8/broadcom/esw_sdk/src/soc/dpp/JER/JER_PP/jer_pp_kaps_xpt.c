...
/*#define JER_KAPS_XPT_DMA_THREAD_PRINTS_ENABLED*/
/*#define JER_KAPS_XPT_REG_PRINTS_ENABLED*/
/*#define JER_KAPS_XPT_ARM_HOST_PRINTS_ENABLED*/
...
/*DMA related defines*/
#define JER_kaps_dma_enabled            0
#define JER_KAPS_MEM_FLAGS             0
 ...
 *Base address + entry size = 2**JER_KAPS_CONFIG_CFIFO_ENTRY_SIZE * 8 (64 bits to bytes) * (number of entries - 1). (-1 because we want the last valid address)
  */
#define JER_KAPS_CONFIG_CFIFO_MAX_ADD_8      (JER_KAPS_CONFIG_CFIFO_BASE_8 + 8*(JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES))

#define JER_PP_KAPS_DMA_SLEEP            1000

#define JER_PP_KAPS_MIN_ENTRIES          100

#define JER_PP_KAPS_XPT_THREAD_PRIO      50
...
STATIC uint32 kaps_dma_enabled = 0;

/* Entries in the host buffer */
STATIC VOLATILE uint32 kaps_host_buffer_entries_insert_offset[SOC_SAND_MAX_DEVICE];
/* Entries in the DMA thread buffer */
STATIC VOLATILE uint32 last_num_of_entries_to_dma[SOC_SAND_MAX_DEVICE];
/*kaps_host_buffer_base_[a/b]_p points to the allocated space in the host*/
/*we have a double buffer a and b*/
/*The main thread only writes to kaps_host_buffer_base_main_p*/
/*A secondary thread switches the main_p to the other buffer and copies the current buffer using SBUS DMA to KAPS TCM*/
STATIC VOLATILE uint64 *kaps_host_buffer_base_main_p[SOC_SAND_MAX_DEVICE];
STATIC uint64 *kaps_host_buffer_base_a_p[SOC_SAND_MAX_DEVICE];
STATIC uint64 *kaps_host_buffer_base_b_p[SOC_SAND_MAX_DEVICE];
...
/*The state variables required for the sbus dma thread to sync its operation with the main (soc + kbp_sdk) thread*/
typedef struct kaps_tcm_dma_state_s {
    uint8                       terminate;/*used to signal termination for the DMA_thread*/
    sal_mutex_t                 tcm_dma_lock; /*This lock is used to lock changes for kaps_host_buffer_base_main_p and kaps_host_buffer_entries_insert_offset*/
    sal_thread_t                tid;
} kaps_tcm_dma_state_t;

STATIC VOLATILE kaps_tcm_dma_state_t kaps_tcm_dma_state[SOC_SAND_MAX_DEVICE];
                                            ...
                                            uint8 *bytes)
{
    ...
    uint32 reg_val, field_val, temp_nbytes;

    uint64 *kaps_host_buffer_base_temp_p;
    ...
    sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);

    counter = 0;
    /*Avoid overflow, allocate larger buffers if this happens often, currently the buffer sizes are JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES each*/
    while (kaps_host_buffer_entries_insert_offset[unit] + 1 + (nbytes + 7)/8 > JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES) {
        counter++;
#ifdef JER_KAPS_XPT_ARM_HOST_PRINTS_ENABLED
        LOG_CLI((BSL_META_U(0,"%s() Waiting for DMA thread to free buffer, attempt: %d.\n"),FUNCTION_NAME(), counter));
#endif
        sal_mutex_give(kaps_tcm_dma_state[unit].tcm_dma_lock);
        if (counter > 1000) {
            LOG_CLI((BSL_META_U(0,"Buffer is full, DMA failed to clear it in 1000 attempts.\n")));
            return SOC_SAND_ERR;
        }
        sal_usleep(JER_PP_KAPS_DMA_SLEEP);
        sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);
    }

    /*Removing volatile within mutex protection*/
    kaps_host_buffer_base_temp_p = (uint64 *)kaps_host_buffer_base_main_p[unit];

    kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]] = nbytes;
    COMPILER_64_SHL(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]], 32);
    COMPILER_64_OR(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]], reg_val);
    kaps_host_buffer_entries_insert_offset[unit]++;

    temp_nbytes = nbytes%8;
    if (temp_nbytes > 0 ) {
        kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]] = 0;
        for (i = 0; i < temp_nbytes; i++) {
            COMPILER_64_SHL(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]], 8);
            COMPILER_64_OR(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]],  (uint64)bytes[i]);
        }

        kaps_host_buffer_entries_insert_offset[unit] += 1;
    }

    temp_ndwords = nbytes/NOF_UINT8_IN_UINT64;
    for ( i = 0; i < temp_ndwords; i++) {
        for (k = 0; k < NOF_UINT8_IN_UINT64; k++) {
            COMPILER_64_SHL(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]], 8);
            COMPILER_64_OR(kaps_host_buffer_base_temp_p[kaps_host_buffer_entries_insert_offset[unit]],  (uint64)bytes[temp_nbytes + 8*i + k]);
        }
...
uint32 jer_pp_xpt_wait_arm_done(int unit)
{
    ...
    if (kaps_dma_enabled) {
        /* Need to take mutex for the kaps_host_buffer_entries_insert_offset check */
        sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);
        do {
            sal_mutex_give(kaps_tcm_dma_state[unit].tcm_dma_lock);
            ...
            sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);
        } while ((read_pointer_index != write_pointer_index) || kaps_host_buffer_entries_insert_offset[unit] || response_depth || command_depth || last_num_of_entries_to_dma[unit]);
                                          ...
                                          uint32 next_write_max_address)
{
    ...
    while ( /*Regular write, make sure we are behind the pointer before write, and ahead of it after write
        ...
        (kaps_tcm_write_p[unit] < kaps_tcm_read_p[unit]) && (next_write_max_address >= kaps_tcm_read_p[unit])
...
/* Function:    jer_pp_kaps_dma_thread
 * Purpose:     Handle Asynchronous dma host to KAPS TCM actions
 * Parameters:  cookie (IN) - unit number
 * Returns:     none
 */
STATIC void
jer_pp_kaps_dma_thread(void *cookie)
{
    int                         unit = PTR_TO_INT(cookie);
    int                         rv = BCM_E_NONE;
    char                        thread_name[SAL_THREAD_NAME_MAX_LEN];
    sal_thread_t	            thread;
    uint32                      num_of_entries_to_dma;
    uint32                      num_of_entries_done;
    uint64                     *kaps_tcm_p;
    uint32                      next_write_max_address;
    uint32                      num_of_entries_to_dma_iter;
    uint32                      force_dma = 1; /* Always DMA after being awaken, regardless of number of entries in buffer */
    uint8                       wrap_around = 0;

    while (1) {
        /*Take the mutex and switch the host to the other buffer if necessary*/
        sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);
        if (kaps_tcm_dma_state[unit].terminate) {/*Terminate the thread*/
            /* Free mutex */
            kaps_tcm_dma_state[unit].tid = NULL;
            sal_mutex_give(kaps_tcm_dma_state[unit].tcm_dma_lock);
            sal_mutex_destroy(kaps_tcm_dma_state[unit].tcm_dma_lock);
            kaps_tcm_dma_state[unit].tcm_dma_lock = NULL;

            /* Free Memory */
            soc_cm_sfree(unit, kaps_host_buffer_base_a_p[unit]);
            kaps_host_buffer_base_a_p[unit] = NULL;
            soc_cm_sfree(unit, kaps_host_buffer_base_b_p[unit]);
            kaps_host_buffer_base_b_p[unit] = NULL;
            kaps_host_buffer_base_main_p[unit] = NULL;

            sal_thread_exit(0);
        }
        /*Only change buffers if the current buffer is not empty*/
        num_of_entries_to_dma = kaps_host_buffer_entries_insert_offset[unit];
        last_num_of_entries_to_dma[unit] = num_of_entries_to_dma;
        if ((num_of_entries_to_dma >= JER_PP_KAPS_MIN_ENTRIES) || force_dma) {
            num_of_entries_done = 0;
            kaps_host_buffer_entries_insert_offset[unit] = 0;
            if (kaps_host_buffer_base_main_p[unit] == kaps_host_buffer_base_a_p[unit]) {
                kaps_host_buffer_base_main_p[unit] = kaps_host_buffer_base_b_p[unit];
                kaps_tcm_p = kaps_host_buffer_base_a_p[unit];
            } else {
                kaps_host_buffer_base_main_p[unit] = kaps_host_buffer_base_a_p[unit];
                kaps_tcm_p = kaps_host_buffer_base_b_p[unit];
            }
        }
        sal_mutex_give(kaps_tcm_dma_state[unit].tcm_dma_lock);

        /*DMA the entries in this buffer if we have entries to transmit, else sleep*/
        if ((num_of_entries_to_dma >= JER_PP_KAPS_MIN_ENTRIES) || force_dma) {
            force_dma = 0;
            /*DMA the entries*/
            while (num_of_entries_done != num_of_entries_to_dma) {
                next_write_max_address = kaps_tcm_write_p[unit] + (num_of_entries_to_dma - num_of_entries_done)*8;

                /*write to the end of the available memory, before we wrap around*/
                if (next_write_max_address > JER_KAPS_CONFIG_CFIFO_MAX_ADD_8) {
                    num_of_entries_to_dma_iter = (JER_KAPS_CONFIG_CFIFO_MAX_ADD_8 - kaps_tcm_write_p[unit])/8 + 1;
                    next_write_max_address = JER_KAPS_CONFIG_CFIFO_MAX_ADD_8;
                    wrap_around = 1;
                } else { /*Simply write the remaining entries*/
                    num_of_entries_to_dma_iter = (num_of_entries_to_dma - num_of_entries_done);
                    wrap_around = 0;
                }

                /*Retrieve and wait for the read pointer if necessary*/
                rv = jer_pp_kaps_retrieve_read_p(unit, next_write_max_address);
                if (rv != SOC_SAND_OK) {
                    LOG_VERBOSE(BSL_LS_SOC_L3,
                       (BSL_META_U(unit,
                                   "%s(): Failed to retrieve read pointer from TCM.\n"),
                                   FUNCTION_NAME()
                        ));
                    break;
                }

#ifdef JER_KAPS_XPT_DMA_THREAD_PRINTS_ENABLED
                LOG_CLI((BSL_META_U(0, "%d entries to add. %d total to dma. %d already done. %d kaps_tcm_write_p\n"), num_of_entries_to_dma_iter, num_of_entries_to_dma, num_of_entries_done, kaps_tcm_write_p[unit]));
#endif /*JER_KAPS_XPT_DMA_THREAD_PRINTS_ENABLED*/

                /*Note DMA unintuitive behavior, the base index is in bytes, while the difference between indexes is in 64bits*/
                rv = soc_mem_array_write_range(unit, JER_KAPS_MEM_FLAGS, KAPS_TCMm, 0, KAPS_BLOCK(unit), kaps_tcm_write_p[unit],
                                               kaps_tcm_write_p[unit] + num_of_entries_to_dma_iter - 1,  &kaps_tcm_p[num_of_entries_done]);
                if (rv != SOC_SAND_OK) {
                    LOG_VERBOSE(BSL_LS_SOC_L3,
                   (BSL_META_U(unit,
                               "%s(): Failed to dma write to TCM.\n"),
                               FUNCTION_NAME()));
                     sal_thread_exit(0);
                }

                /*Advance the relevant pointer*/
                if (wrap_around) {
                    kaps_tcm_write_p[unit] = JER_KAPS_CONFIG_CFIFO_BASE_8;
                } else {
                    kaps_tcm_write_p[unit] += num_of_entries_to_dma_iter * 8;/*Multiplied by 8 due to 8 bytes per entry*/
                }

                num_of_entries_done += num_of_entries_to_dma_iter;
            }
        } else {
            /*Sleep*/
            sal_usleep(JER_PP_KAPS_DMA_SLEEP);
    ...
    kaps_tcm_dma_state[unit].tid = SAL_THREAD_ERROR;

    sal_thread_exit(0);
}

/* Function:    jer_pp_kaps_xpt_arm_thread_init
 * Purpose:     Initialize the KAPS memory to ARM TCM DMA thread
 * Parameters:  unit (IN)   - Device Number
 * Returns:     BCM_E_XXX
 */
int
jer_pp_kaps_xpt_arm_thread_init(int unit)
{
    uint32 reg_val, rv;
    uint32 write_pointer_index;
    sal_thread_t temp_thread;
    ...
    if (kaps_tcm_dma_state[unit].tid == NULL) {
        /* not initialized before, or thread deliberately terminated and destroyed the mutex*/
        kaps_tcm_dma_state[unit].tcm_dma_lock = sal_mutex_create("KAPS DMA mutex");
        if(kaps_tcm_dma_state[unit].tcm_dma_lock == NULL) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
                                  "KAPS DMA mutex create failed. \n")));
            SOCDNX_IF_ERR_EXIT_MSG(SOC_SAND_GEN_ERR, (_BSL_SOCDNX_MSG("Memory allocation failure.")));
        }
    }

    kaps_tcm_dma_state[unit].terminate = 0;

    /*Initialize the thread if we are in init or if recovering from error*/
    if ((kaps_tcm_dma_state[unit].tid == SAL_THREAD_ERROR) || (kaps_tcm_dma_state[unit].tid == NULL)) {
        temp_thread = sal_thread_create("kapsDMA",
                                          SAL_THREAD_STKSZ,
                                          JER_PP_KAPS_XPT_THREAD_PRIO,
                                          jer_pp_kaps_dma_thread,
                                          INT_TO_PTR(unit));
        if ((temp_thread == NULL) || (temp_thread == SAL_THREAD_ERROR)) {
            LOG_ERROR(BSL_LS_BCM_L3,
                      (BSL_META_U(unit,
                                  "KAPS DMA thread create failed. \n")));

            SOCDNX_IF_ERR_EXIT_MSG(SOC_SAND_GEN_ERR, (_BSL_SOCDNX_MSG("DMA thread Creation failure.")));
        }

        kaps_tcm_dma_state[unit].tid = temp_thread;
    }
...
uint32 jer_pp_xpt_arm_init(int unit)
{
    ...
    /* First reset the R5 */
    rv = soc_reg_field32_modify(unit, KAPS_R_5_CORE_CONTROLr, KAPS_BLOCK(unit), CPU_HALT_Nf, 0);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
    ...
    rv = soc_reg_fields32_modify(unit, KAPS_R_5_CORE_CONTROLr, KAPS_BLOCK(unit), 2, fields, values);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
    ...
    field_val = 0x7;
    rv = soc_reg_field32_modify(unit, KAPS_R_5_CORE_CONFIGAr, KAPS_BLOCK(unit), ECC_ENABLEf, field_val);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
    ...
    rv = soc_reg_fields32_modify(unit, KAPS_R_5_CORE_MEM_CONTROLr, KAPS_BLOCK(unit), 9, fields, values);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
    ...
    if (!SAL_BOOT_PLISIM) {
        ...
        for(;;){
            /* get command word register */
            rv = READ_KAPS_R_5_CORE_MEM_CONTROLr(unit, &reg_val);
            if (rv != SOC_E_NONE) {
                return KBP_INTERNAL_ERROR;
            }
            /* validate against expected value */
    ...
    reg_val = 0;
    rv = WRITE_KAPS_CONFIG_CFIFOr(unit, 0/*Array Index*/, reg_val);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
    ...
    reg_val = 0;
    rv = WRITE_KAPS_CONFIG_RFIFOr(unit, 0/*Array Index*/, reg_val);
    if (rv != SOC_E_NONE) {
        return KBP_INTERNAL_ERROR;
    }
...
STATIC uint32 jer_pp_xpt_dma_thread_init(int unit)
{
    ...
    sal_sprintf(buffer_name, "KAPS DMA buffer A, Unit %d", unit);
    kaps_host_buffer_base_a_p[unit] = (uint64 *)soc_cm_salloc(unit, JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES*sizeof(uint64), buffer_name);
    if (kaps_host_buffer_base_a_p[unit] == NULL) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_MEMORY, (_BSL_SOCDNX_MSG("Error, failed to allocate memory for KAPS XPT")));
    }
    sal_memset(kaps_host_buffer_base_a_p[unit], 0,  JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES*sizeof(uint64));

    sal_sprintf(buffer_name, "KAPS DMA buffer B, Unit %d", unit);
    kaps_host_buffer_base_b_p[unit] = (uint64 *)soc_cm_salloc(unit, JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES*sizeof(uint64), buffer_name);
    if (kaps_host_buffer_base_b_p[unit] == NULL) {
        soc_cm_sfree(unit, kaps_host_buffer_base_a_p[unit]);
        SOCDNX_EXIT_WITH_ERR(SOC_E_MEMORY, (_BSL_SOCDNX_MSG("Error, failed to allocate memory for KAPS XPT")));
    }
    sal_memset(kaps_host_buffer_base_b_p[unit], 0,  JER_JER_KAPS_CONFIG_CFIFO_NUM_ENTRIES*sizeof(uint64));

    kaps_host_buffer_base_main_p[unit] = kaps_host_buffer_base_a_p[unit];

    jer_pp_kaps_xpt_arm_thread_init(unit);
...
uint32 jer_pp_xpt_init(int unit, void **xpt)
{
    ...
    *xpt = soc_sand_os_malloc(sizeof(JER_KAPS_XPT), "kaps_xpt");
    ...
    if (kaps_dma_enabled) {
        rv = jer_pp_xpt_dma_thread_init(unit);
        if (rv != SOC_E_NONE) {
...
STATIC uint32 jer_pp_xpt_dma_thread_deinit(int unit)
{
    ...
    sal_mutex_take(kaps_tcm_dma_state[unit].tcm_dma_lock, sal_mutex_FOREVER);
    kaps_tcm_dma_state[unit].terminate = 1;
    sal_mutex_give(kaps_tcm_dma_state[unit].tcm_dma_lock);
...
uint32 jer_pp_xpt_dma_state(int unit, uint32 print_status, uint32 enable_dma_thread)
{
    ...
    if (print_status) {
        cli_out("DMA is enabled: %d. Thread terminated: %d.\n",
                kaps_dma_enabled, kaps_tcm_dma_state[unit].terminate);
        ...
        cli_out("tcm_write_p: %d. tcm_read_p: %d. kaps_host_buffer_entries_insert_offset: %d.\n",
                 write_p,           read_p,       kaps_host_buffer_entries_insert_offset[unit]);
    }

    if (enable_dma_thread != kaps_dma_enabled) {
        if (enable_dma_thread) {
            SOCDNX_IF_ERR_EXIT(jer_pp_xpt_dma_thread_init(unit));
        } else {
            SOCDNX_IF_ERR_EXIT(jer_pp_xpt_dma_thread_deinit(unit));
        }
        kaps_dma_enabled = enable_dma_thread;
    }
