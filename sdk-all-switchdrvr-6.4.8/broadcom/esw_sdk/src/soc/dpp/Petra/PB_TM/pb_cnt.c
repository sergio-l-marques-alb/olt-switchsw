  ...
  )
{
  ...
  */
  ...
  {
    SOC_PB_FLD_SET(regs->iqm.cnt_processor_config4_reg.crps_queue_shift[proc_id],info->q_set_size,1160,exit);
  
    SOC_PB_FLD_SET(regs->iqm.cnt_processor_shift_cfg1_reg[proc_id].crps_base_q,info->mode_eg.base_val,1170,exit);
  ...
  )
{
  ...
  else /* Ingress */
  {
    if(info->src_type == SOC_PB_CNT_SRC_TYPE_VOQ)
    {
      SOC_SAND_ERR_IF_ABOVE_MAX(info->q_set_size, SOC_PB_CNT_VOQ_PARAMS_Q_SET_SIZE_MAX, SOC_PB_CNT_Q_SET_SIZE_OUT_OF_RANGE_ERR , 23, exit);
      SOC_SAND_ERR_IF_ABOVE_MAX(info->mode_eg.base_val, SOC_PB_CNT_QUEUE_NDX_MAX, SOC_PB_CNT_START_Q_OUT_OF_RANGE_ERR, 29, exit);
    }
...

{
  ...
  if (info->src_type == SOC_PB_CNT_SRC_TYPE_VOQ)
  {
    ...
    case SOC_PB_CNT_Q_SET_SIZE_1_Q_FLD_VAL:
      info->q_set_size = SOC_PB_CNT_Q_SET_SIZE_1_Q;
      break;

    case SOC_PB_CNT_Q_SET_SIZE_2_Q_FLD_VAL:
      info->q_set_size = SOC_PB_CNT_Q_SET_SIZE_2_Q;
      break;

    case SOC_PB_CNT_Q_SET_SIZE_4_Q_FLD_VAL:
      info->q_set_size = SOC_PB_CNT_Q_SET_SIZE_4_Q;
      break;

    case SOC_PB_CNT_Q_SET_SIZE_8_Q_FLD_VAL:
      info->q_set_size = SOC_PB_CNT_Q_SET_SIZE_8_Q;
      break;
  ...
  )
{
  ...
  res = soc_convert_enum_val2_q_set_size(
    ...
    &q_set_size_val);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
   counter_id = ((queue_ndx - info.mode_eg.base_val)/q_set_size_val);
...
*     The computation of the queue_ndx: g(cnt_ndx) * info.q_set_size 
  ...
  )
{
  ...
  *queue_ndx = cnt_ndx * q_set_size_val + info.mode_eg.base_val;
