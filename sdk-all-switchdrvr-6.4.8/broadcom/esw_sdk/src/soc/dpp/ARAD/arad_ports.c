  ...
  )
{
    ...
    */
    ...
    if (pp_port == ARAD_OAMP_PORT_ID &&
        ((soc_property_get(unit, spn_NUM_OAMP_PORTS, 0) == 1) || soc_property_get(unit, spn_SAT_ENABLE, 0))) {
    ...
    )
{
    ...
    /* was changed because irr tables doesn't take to consideration disabled members */
...
uint32 alloc_reassembly_context_and_recycle_channel_unsafe(int unit, int core_id, uint32 out_pp_port, uint32 *channel, uint32 *reassembly_context)
{
  ...
  *channel = 0;
  ...
  if (out_info.header_type_out == SOC_TMC_PORT_HEADER_TYPE_ETH) {
    ...
    SOC_SAND_IN ARAD_PORTS_APPLICATION_MAPPING_INFO *info)
{
  ...
  {    
    ...
    res = WRITE_IHP_VIRTUAL_PORT_TABLEm(unit, MEM_BLOCK_ANY, xgs_mac_ext_encode, &tbl_data_32b);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, pp_port, &flags));
