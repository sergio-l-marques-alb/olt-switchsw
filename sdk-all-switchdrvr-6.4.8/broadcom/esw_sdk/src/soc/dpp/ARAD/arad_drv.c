...
soc_arad_default_config_general_get(int unit)
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        dpp_pp->oam_dma_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_THRESHOLD, 0x20);
        ...
        dpp_pp->oam_dma_event_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_THRESHOLD, dpp_pp->oam_dma_threshold);
        ...
        if (!SOC_IS_JERICHO(unit)) { /* Arad+ only */
            ...
            dpp_pp->oam_dma_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_THRESHOLD, ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY+1);
            if (dpp_pp->oam_dma_threshold > ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY) {
                dpp_pp->oam_dma_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_THRESHOLD, ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY);
                dpp_pp->oam_dma_threshold = (dpp_pp->oam_dma_threshold > ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY)?
                                                ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY: dpp_pp->oam_dma_threshold;
            }
        }
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        dpp_pp->bfd_ipv4_single_hop = soc_property_get(unit, spn_BFD_IPV4_SINGLE_HOP_EXTENDED, 1 ) && SOC_IS_ARADPLUS(unit); /* Default value is 1 for Arad+, 0 otherwise.*/
    } else {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        else if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_over_mpls_ignore_mdl",0)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                                 (_BSL_SOCDNX_MSG("OAM over MPLS ignor MDL currently is not supported for this device.")));
        }
        ...
        else if (soc_property_get(unit, spn_OAM_MAID_11_BYTES_ENABLE, 0)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                                 (_BSL_SOCDNX_MSG(spn_OAM_MAID_11_BYTES_ENABLE" currently is not supported for this device.")));
        }
        ...
        else if (soc_property_get(unit, spn_BFD_EXTENDED_IPV4_SRC_IP, 0)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                                 (_BSL_SOCDNX_MSG(spn_BFD_EXTENDED_IPV4_SRC_IP" currently is not supported for this device.")));
        }
...
soc_arad_allocate_rcy_port(int unit, int core, soc_port_t *port)
{
    ...
    for (tchannel = 0; tchannel < SOC_DPP_MAX_NOF_CHANNELS; ++tchannel) { /* find the first free channel */
        SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.tm.tm_info.rcy_channel_ilkn_offset.get(unit, core, tchannel, &offset_i));
        if ((!channels[tchannel]) && (offset_i == ARAD_PORT_INVALID_PROTOCOL_OFFSET)) {
            break;
        }
    }
...
soc_arad_info_config(int unit) 
{
    ...
    dpp_arad->init.ports.use_trunk_as_ingress_mc_dest = soc_property_get(unit, spn_USE_TRUNK_AS_INGRESS_MC_DESTINATION, 0);
