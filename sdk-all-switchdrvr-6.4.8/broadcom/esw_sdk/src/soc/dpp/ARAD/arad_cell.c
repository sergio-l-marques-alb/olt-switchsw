/* $Id: arad_cell.c,v 1.28 Broadcom SDK $
...
/*
 * The function tranlate from the structure sr_link_list to soc_fabric_inband_route_t
 */
STATIC int
...
/*
 * The function get a 32-register of a route data and parse it into a struct soc_fabric_inband_route_t
 */
STATIC int
...
)
{
  ...
  if(!fields) {/*i.e. the entry is invalid*/
    sr_link_list.dest_entity_type = SOC_SAND_FIP_ENTITY;/* And hence, the number of hops will be zero.*/
  }
  ...
  if(is_fe1600) {
    if(!is_group){
      ...
      for(;;) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 1110, exit, READ_FDT_INTERRUPT_REGISTERr(unit, &tmp_interrupt_reg));
  ...
  )
{
  ...
  {
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 1310, exit, READ_FDR_CPU_DATA_CELL_B_PRIMARYr(unit, data));/* Is it regs->fdr.cpu_data_cell_b_reg[0].addr.base) ??? */
    sal_memcpy(ack, data, ARAD_DATA_CELL_RECEIVED_BYTE_SIZE);
  } else if (success_c)
  {
...
STATIC uint32
  arad_sr_rcv_cell(
    ...
    SOC_SAND_OUT uint32                       *size, /*Unit: bytes. only in case of not inband*/
    SOC_SAND_OUT uint8                        *success
  )
{
  uint32
...
uint32
  arad_transaction_with_fe1600(
    ...
    SOC_SAND_IN  int32                        *port_or_copyno,
    SOC_SAND_IN  int32                        *index,
    SOC_SAND_IN  uint32                       *data_in, /*If inband, each 4 entries are 1 unit of 128 bits to be taken*/
    SOC_SAND_IN  uint32                       size,  /* in bytes if not inband, number of regs/mems if inband */
    SOC_SAND_IN  uint8                        is_write,
    SOC_SAND_IN  uint8                        is_inband,
    SOC_SAND_IN  uint8                        is_memory,
    SOC_SAND_OUT soc_reg_above_64_val_t       *data_out, /* The out data will be in units of 128 bits each one*/
    SOC_SAND_OUT uint32                       *data_out_size /* In case of read not inband the actual size read in bytes*/
  )
{
  uint8
    success = FALSE;
