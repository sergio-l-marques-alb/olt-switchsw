...
{
  uint32  res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_EXPLICIT_MAPPING_MODE_INFO_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  SOC_SAND_CHECK_NULL_INPUT(info);

  res = arad_ipq_explicit_mapping_mode_info_verify(
    unit,
    info
  );
  ...
  res = arad_ipq_explicit_mapping_mode_info_set_unsafe(
    unit,
    info
  );
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_ipq_explicit_mapping_mode_info_set()",0,0);
}
...
{
  uint32  res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_EXPLICIT_MAPPING_MODE_INFO_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_ipq_explicit_mapping_mode_info_get()",0,0);
}
  ...
  )
{
  uint32
    res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_TRAFFIC_CLASS_MULTICAST_PRIORITY_MAP_SET);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_ipq_traffic_class_multicast_priority_map_set()",0,0);
}
  ...
  )
{
  uint32
    res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_IPQ_TRAFFIC_CLASS_MULTICAST_PRIORITY_MAP_GET);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_ipq_traffic_class_multicast_priority_map_get()",0,0);
}
