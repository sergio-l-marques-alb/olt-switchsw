    ...
    )
{
  ...
  if (is_for_update && insertion_rule_respected) {
      ...
      found_node = current;
      if (  found_node != SOC_SAND_SORTED_LIST_ITER_BEGIN(unit,list)  ) { 
          found_prev = TRUE;
...
arad_pp_ihb_tcam_action_tbl_cache_after_move_update(
    ...
    */
    ...
    for (is_second_bank_ndx = 0; is_second_bank_ndx <= move_couple;  is_second_bank_ndx++)
    {
        res = arad_tcam_tbl_calc_action_banks_and_lines(unit, bank_ndx + is_second_bank_ndx, source_entry_start_ndx, &action_bitmap_ndx, src_line, src_action_tbl);
        SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
        res = arad_tcam_tbl_calc_action_banks_and_lines(unit, bank_ndx + is_second_bank_ndx, dest_entry_start_ndx, &action_bitmap_ndx, dest_line, dest_action_tbl);
