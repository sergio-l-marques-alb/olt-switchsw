...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lif_table_get_block()", 0, 0);
}
  ...
  )
{
  ...
  ARAD_PP_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lif_table_entry_get()", lif_ndx, 0);
}
  ...
  )
{
  ...
  ARAD_PP_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lif_table_entry_update()", lif_ndx, 0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_LIF_TABLE_ENTRY_ACCESSED_INFO_GET);
...
exit_semaphore:
  ...
  ARAD_PP_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_lif_table_entry_accessed_info_get()", lif_ndx, 0);
}
