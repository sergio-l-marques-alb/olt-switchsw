...
/* $Id: arad_pp_eg_ac.c,v 1.25 Broadcom SDK $
...
#define ARAD_PP_EG_AC_NOF_TAGS_MAX                               (ARAD_PP_VLAN_TAGS_MAX)
  ...
  Arad_pp_procedure_desc_element_eg_ac[] =
{
   ...
   */
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_INFO_GET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_EG_AC_MP_INFO_GET_UNSAFE),
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_INFO_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_INFO_SET_VERIFY);

  SOC_SAND_ERR_IF_ABOVE_NOF(out_ac_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_AC_INFO, unit, ac_info, 20, exit);

#ifdef BCM_88660_A0
  /* In case of BIG AC, the out_ac_ndx must be even to represent the start of the whole EEDB line.  */
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_INFO_GET_VERIFY);
  SOC_SAND_ERR_IF_ABOVE_NOF(out_ac_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_INFO_GET_UNSAFE);
  SOC_SAND_CHECK_NULL_INPUT(ac_info);
  ARAD_PP_EG_AC_INFO_clear(ac_info);
  ...
  for (vid_idx = 0; vid_idx < ARAD_PP_VLAN_TAGS_MAX; vid_idx++)
  {
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_MP_INFO_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_MP_INFO_SET_VERIFY);
  SOC_SAND_SET_ERROR_CODE(ARAD_PP_FEATURE_NOT_SUPPORTED_ERR, 35, exit);
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_MP_INFO_GET_VERIFY);
  SOC_SAND_SET_ERROR_CODE(ARAD_PP_FEATURE_NOT_SUPPORTED_ERR, 35, exit);
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_AC_MP_INFO_GET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(out_ac_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_AC_VBP_KEY, vbp_key, 20, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_AC_INFO, unit, ac_info, 30, exit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_AC_VBP_KEY, vbp_key, 10, exit);
  ...
  )
{
   ...
   *	ESEM Key
   */
  esem_key.key_type = ARAD_PP_ESEM_KEY_TYPE_VD_VSI_CVID; /*ARAD_PP_ESEM_KEY_TYPE_VD_CVID*/ 
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(out_ac_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_AC_ID_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_AC_CEP_PORT_KEY, cep_key, 20, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_AC_INFO, unit, ac_info, 30, exit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_AC_CEP_PORT_KEY, cep_key, 10, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_AC_VBP_KEY_verify()",0,0);
}

uint32
  ARAD_PP_EG_AC_CEP_PORT_KEY_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_AC_CEP_PORT_KEY_verify()",0,0);
}

uint32
  ARAD_PP_EG_VLAN_EDIT_CEP_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_VLAN_EDIT_CEP_INFO_verify()",0,0);
}

uint32
  ARAD_PP_EG_VLAN_EDIT_VLAN_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_VLAN_EDIT_VLAN_INFO_verify()",0,0);
}

uint32
  ARAD_PP_EG_AC_VLAN_EDIT_INFO_verify(
  ...
  )
{
  ...
  for (ind = 0; ind < 2; ++ind)
  {
    ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_VLAN_EDIT_VLAN_INFO, &(info->vlan_tags[ind]), 10, exit);
  }

  /*
   * In Arad nof_tags is always 2
   */
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_AC_VLAN_EDIT_INFO_verify()",0,0);
}

uint32
  ARAD_PP_EG_AC_INFO_verify(
    SOC_SAND_IN  int                unit,
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_AC_VLAN_EDIT_INFO, &(info->edit_info), 10, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_PROTECTION_INFO, unit, &(info->protection_info), 20, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_AC_INFO_verify()",0,0);
}

uint32
  ARAD_PP_EG_AC_MP_INFO_verify(
    SOC_SAND_IN  ARAD_PP_EG_AC_MP_INFO *info
  )
{
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(0);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_AC_MP_INFO_verify()",0,0);
}
