...
#define _ARAD_PP_OAM_OPCODE_STAMP_OFFSET(opcode)                                                                   \
    (( (opcode==SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LMM) ||            \
      ...
      (opcode==SOC_PPD_OAM_ETHERNET_PDU_OPCODE_SLM) ? 8 : (opcode==SOC_PPD_OAM_ETHERNET_PDU_OPCODE_SLR)? 16: \
      ...
      for (mep_type = 0; mep_type < nof_mp_types_to_set; mep_type++) {
          ...
          for (mip_type = 0; mip_type <nof_mp_types_to_set; mip_type++) {
              ...
              if (is_bfd) {
                  ...
                  res = sw_state_access[unit].dpp.bcm.oam.y1731_opcode_to_intenal_opcode_map.get(unit, SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM, &internal_opcode);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                  res = _arad_pp_oam_set_counter_disable(unit ,SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM, internal_opcode, (&oam_payload), &profile_data->mep_profile_data, 0);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                  *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                  ...
                  for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
                      ...
                      for (opcode = 1; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT;  opcode++) {
                          ...
                          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                          oam1_key.opcode = internal_opcode;
                          res = _arad_pp_oam_set_counter_disable(unit ,opcode, internal_opcode, (&oam_payload), &profile_data->mep_profile_data, 0);
                          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          oam_payload.meter_disable = (SHR_BITGET(meter_disable_lcl, internal_opcode) != 0);
                          if (is_my_cfm_mac) {
  ...
  )
{
    ...
    *meter_disable_lcl = profile_data->mep_profile_data.meter_disable;
    ...
    res = _arad_pp_oam_set_counter_disable(unit, opcode, internal_opcode, oam_payload, &profile_data->mep_profile_data, profile_data->is_piggybacked);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  )
{
  ...
  /* OAM */
	  ...
	  res = sw_state_access[unit].dpp.bcm.oam.trap_info.trap_ids.get(unit, SOC_PPD_OAM_TRAP_ID_ERR_LEVEL, &trap_code_level);
	  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 247, exit); 
              ...
              for (mp_type = 0; mp_type < SOC_PPD_OAM_MP_TYPE_COUNT_PLUS; mp_type++) {
                  ...
                  case SOC_PPD_OAM_MP_TYPE_MIP_OR_PASSIVE_MATCH_PLUS:
                      ...
                      if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)
    					  ...
    					  res = _arad_pp_oam_set_counter_disable(unit, opcode, internal_opcode, (&oam_payload), &profile_data->mip_profile_data, 0);
                          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          oam_payload.meter_disable = (SHR_BITGET(meter_disable_lcl, internal_opcode) != 0);
  ...
  if (is_upmep && !is_passive) {
      ...
      for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
          ...
          for (opcode = 1; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT; opcode++) {
              ...
              if (is_accelerated) {
                  res =_arad_pp_oam_set_counter_disable(unit, opcode, internal_opcode, (&oam_payload), &profile_data_acc->mep_profile_data, profile_data_acc->is_piggybacked);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
              } else {
                  res = _arad_pp_oam_set_counter_disable(unit, opcode, internal_opcode, (&oam_payload), &profile_data->mep_profile_data, profile_data->is_piggybacked);
                  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 15, exit);
  ...
  )
{
  ...
  uint8   found;
  uint32 original_lifetime_units, original_lifetime;
#if SOC_DPP_IS_EM_HW_ENABLE
  ...
  COMPILER_64_ZERO(entry);
  COMPILER_64_ZERO(val64);
  /*enter RMEP DB entry*/  
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit, READ_OAMP_RMEP_DBm(unit, MEM_BLOCK_ANY, rmep_index, &entry));
  original_lifetime_units = soc_mem_field32_get(unit,OAMP_RMEP_DBm,&entry, LIFETIME_UNITSf);
  original_lifetime = soc_mem_field32_get(unit,OAMP_RMEP_DBm,&entry, LIFETIMEf);

  COMPILER_64_SET(val64, 0, lifetime);

  /* When updating the RMEP the last_ccm_lifetime counter should be left as is except for the following exceptions:
     1. When changing the lifetime units. In this case the meaning of last_ccm_lifetime will be changed, so the existing value is not relavent.
     2. When going from low period to high period. The scanner machine decrements the period until it reaches zero.
        So for example when updating from 30ms to 300ms the RMEP_DB entry will expect a packet within the last 30 ms (at most). 
        The last_ccm_lifetime   must thus be reset*/
  /* For new endpoints (!update) last_ccm_lifetime will be set anyays, but this may be ignored, depending on rmep_db_entry->last_ccm_lifetime_valid_on_create.
     If it's 0 then the scanner machine ignores the value of last_ccm_lifetime.*/
  soc_mem_field64_set(unit, OAMP_RMEP_DBm,&entry, LIFETIMEf, val64);
  if (!update || original_lifetime_units !=lifetime_units || lifetime>original_lifetime ) {
      soc_mem_field64_set(unit, OAMP_RMEP_DBm,&entry, LAST_CCM_LIFETIMEf, val64);
      if (!update) {
          /* When updateing, the LAST_CCM_LIFETIME_VALID should remain untouched.*/
          COMPILER_64_SET(val64, 0, (rmep_db_entry->last_ccm_lifetime_valid_on_create)); 
          soc_mem_field64_set(unit,OAMP_RMEP_DBm, &entry, LAST_CCM_LIFETIME_VALIDf, val64);
      }
  }

  COMPILER_64_SET(val64, 0, lifetime_units);
  soc_mem_field64_set(unit,OAMP_RMEP_DBm, &entry, LIFETIME_UNITSf, val64);
  if (rmep_db_entry->loc_clear_threshold == 0) {
      COMPILER_64_SET(val64, 0, 3);
      soc_mem_field64_set(unit, OAMP_RMEP_DBm,&entry, LOC_CLEAR_LIMTf, val64);
      COMPILER_64_SET(val64, 0, 0);
      soc_mem_field64_set(unit,OAMP_RMEP_DBm, &entry, LOC_CLEAR_ENABLEf, val64);
  }
  else {
  ...
  if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
      ...
      soc_mem_field64_set(unit,OAMP_RMEP_DBm, &entry, EVENT_MASKf, val64);
      COMPILER_64_SET(val64, 0, rmep_db_entry->is_state_auto_handle);
      soc_mem_field64_set(unit,OAMP_RMEP_DBm, &entry, STATE_AUTO_HANDLEf, val64);
  }
  ...
  ) 
{
    ...
    uint32 (*fill_interrupt_message) (int, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA *) = (use_dma && SOC_IS_ARADPLUS(unit))?
       ...
       )
{
    ...
    if (!loopback_info->is_period_in_kpps) {
        result_period = loopback_info->tx_period * tiks_per_ms;
    }
    else {
        /*
           Period is in kilo-packets per sec.
           kilo-packets/sec == packets/ms so the period would be the division of
           tiks/ms by that number
         */
        result_period = SOC_SAND_DIV_ROUND_UP(tiks_per_ms, loopback_info->tx_period);
    }
    ...
    if (loopback_info->has_tst_tlv) {
        ...
        soc_reg_above_64_field_set(unit,OAMP_TST_LBM_CTRLr, reg_data, TST_LBM_TLV_PATTERN_TYPEf, reg_field);
        /* Setup TLV type code */
        reg_field[0] = 32; /* Test-TLV */
        soc_reg_above_64_field_set(unit,OAMP_TST_LBM_CTRLr, reg_data, TST_LBM_TLV_TYPEf, reg_field);
    }
    else {
       ...
       )
{
    ...
    if (!loopback_info->is_period_in_kpps) {
        loopback_info->tx_period =
            SOC_SAND_DIV_ROUND(reg_field[0], arad_chip_kilo_ticks_per_sec_get(unit));
    }
    else {
        loopback_info->tx_period =
            SOC_SAND_DIV_ROUND(arad_chip_kilo_ticks_per_sec_get(unit), reg_field[0]);
    }
...
soc_error_t arad_pp_oam_interrupt_init_internal(int unit, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data) {
    ...
    if (SOC_DPP_CONFIG(unit)->pp.oam_use_fifo_dma) {
        dma_threshold = SOC_DPP_CONFIG(unit)->pp.oam_dma_threshold; 
