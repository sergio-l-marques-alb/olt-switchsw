...
{
    const ARAD_PMF_CE ce_array_ipv4_match[6] = {
    ...
    const ARAD_PMF_CE ce_array_efp_match[8] = {
    ...
    case ARAD_PP_FP_VT_MATCH_MODE_IPV4:
        nof_ces_in_key = 6;
        sal_memcpy(ce_array,&ce_array_ipv4_match,sizeof(ARAD_PMF_CE)* nof_ces_in_key );
        break;
    case ARAD_PP_FP_VT_MATCH_MODE_EFP:
        nof_ces_in_key = 8;
        sal_memcpy(ce_array,&ce_array_efp_match,sizeof(ARAD_PMF_CE)* nof_ces_in_key );
        break;
    default:
        break;
    }
...
soc_error_t
  arad_pp_fp_is_qual_in_qual_arr(
      SOC_SAND_IN  int                   unit,
      SOC_SAND_IN  ARAD_PP_FP_QUAL_TYPE  *qual_types,
      SOC_SAND_IN  uint32                nof_qual_types,
      SOC_SAND_IN  ARAD_PP_FP_QUAL_TYPE  search_qual,
    ...
    *found = FALSE;
    ...
    for (qual_idx = 0; qual_idx < nof_qual_types; qual_idx++, qual_types++) {
        ...
        if (SOC_PPC_FP_IS_QUAL_TYPE_USER_DEFINED(*qual_types)) {
            ARAD_PP_FP_CONTROL_INDEX index;
            ARAD_PP_FP_CONTROL_INFO info;
            uint32 sand_res;

            ARAD_PP_FP_CONTROL_INDEX_clear(&index);
            ARAD_PP_FP_CONTROL_INFO_clear(&info);

            index.type = ARAD_PP_FP_CONTROL_TYPE_HDR_USER_DEF;
            index.val_ndx = *qual_types - SOC_PPC_FP_QUAL_HDR_USER_DEF_0;
            sand_res = arad_pp_fp_control_get(unit, SOC_CORE_INVALID, &index, &info);
            SOCDNX_SAND_IF_ERR_EXIT(sand_res);

            /* info.val[ARAD_PP_FP_CONTROL_TYPE_HDR_USER_DEF_PARTIAL_QUAL_NDX] - base qualifier for the UDF. */
            if ((info.val[0] == SOC_TMC_NOF_PMF_CE_SUB_HEADERS) && (info.val[3 /* ARAD_PP_FP_CONTROL_TYPE_HDR_USER_DEF_PARTIAL_QUAL_NDX */] == search_qual)) {
                *found = TRUE;
                SOC_EXIT;
            }
        }
  ...
  )
{
    ...
    *found = 0;
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        if ((stage == ARAD_FP_DATABASE_STAGE_INGRESS_PMF) && (cycle_ndx == 0) && !(flags & ARAD_PP_FP_KEY_ALLOC_USE_KEY_A))  {
                 ...
                 || (is_high_and_low_diff_fgs_allowed && Arad_pp_fp_place_const_convert[place_ndx][5])))
            {
                ...
                else
                {
                    ...
                    res = arad_pp_fp_key_alloc_key_id(
                             ...
                             ARAD_PP_FP_KEY_ALLOC_KEY_CHECK_ONLY | (flags & ARAD_PP_FP_KEY_ALLOC_USE_KEY_A ), /* Check-only */
                             &key_id[0],
                             &success
                          );
                    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
  ...
  *nof_quals = 0;
  ...
  for(qual_indx = 0; (qual_indx < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX) && (qual_types[qual_indx]!= ARAD_PP_NOF_FP_QUAL_TYPES) && (qual_types[qual_indx]!= BCM_FIELD_ENTRY_INVALID); ++qual_indx)
  {
    /*
  ...
  )
{
  ...
  if (  (stage == ARAD_FP_DATABASE_STAGE_INGRESS_FLP)) {
      ...
      if (dip_sip_sharing_is_sharing_enabled_for_program(unit, prog_result)) {
          for (i = 0; i < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; i++) {
        ...
        for (direct_ext_loop_indx = 0; (direct_ext_loop_indx < (1 + is_for_direct_extraction)) && direct_ext_2_needed; direct_ext_loop_indx++) {
            *found = TRUE;
            ...
            for(qual_indx = 0;  (qual_indx < nof_quals) && (*found); ++qual_indx) 
            {
              ...
              if (is_slb_hash_in_quals && stage == ARAD_FP_DATABASE_STAGE_INGRESS_PMF) {
       ...
       )
{
    ...
    /* Get the database Info */
     ...
     if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_DBAL) {
         ...
         SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
     }else if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_STAGE_VT_EXTENDED){
        res = arad_pp_fp_vt_match_key_ces_get(unit, fp_database_info.internal_table_id, ce_array, nof_ces ); /*curently only 1 mode*/
    ...
    for (key_ndx = 0; key_ndx <= is_key_320b; key_ndx++) {
        ...
        for(ce_indx = 0; ce_indx < nof_ces_to_handle; ++ce_indx) {
            ...
            LOG_DEBUG(BSL_LS_SOC_FP,(BSL_META_U(unit,"   Key value-buffer: direction:%d, ce-id:%d, type:%s \n\r"), direction, 
                       ce_ids[ce_indx], ARAD_PP_FP_QUAL_TYPE_to_string(sw_db_ce.qual_type)));
