       ...
       )
{
    ...
    SOCDNX_INIT_FUNC_DEFS;
    if (lif_counting_mask < 0 || lif_counting_mask > SOC_TMC_CNT_LIF_COUNTING_MASK_MAX(source)) {
    ...
    if ((lif_stack_to_count < -1 && source == SOC_TMC_CNT_SRC_TYPE_EPNI) ||
        ...
        (lif_stack_to_count >= SOC_TMC_CNT_LIF_COUNTING_NUMBER_OF_STACK_IDS(source))) {
        LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "Invalid lif stack to count %d.\n"), lif_stack_to_count));
        SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
    }
    for (stack_level = 0; stack_level < SOC_TMC_CNT_LIF_COUNTING_NUMBER_OF_STACK_IDS(source); stack_level++) {
        counting_mask[stack_level] = SOC_TMC_CNT_LIF_COUNTING_MASK_FIELD_VAL_GET(source, lif_counting_mask, stack_level);
...
{   
    ...
    for (stack_level = 0; stack_level < ((source == SOC_TMC_CNT_SRC_TYPE_ING_PP) ? 4 : 3); stack_level++) {
        counting_mask[stack_level] = SOC_TMC_CNT_LIF_COUNTING_MASK_FIELD_VAL_GET(source, lif_counting_mask, stack_level);
        if (counting_mask[stack_level] < 0 || counting_mask[stack_level] > 3) {
            LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "Invalid lif stack %d in lif counting stack level %d, .\n"), stack_level, counting_mask[stack_level]));
            SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
...
)
{
    ...
    if (format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS) {
        ...
        COMPILER_64_SET(*byte_cnt,
           (COMPILER_64_HI(field_val) >> JER_CNT_BYTE_CNT_SHIFT(format)) ,
           ((JER_CNT_BYTE_CNT_MASK_LO(format) & COMPILER_64_HI(field_val)) << (32 - JER_CNT_BYTE_CNT_SHIFT(format))) |
