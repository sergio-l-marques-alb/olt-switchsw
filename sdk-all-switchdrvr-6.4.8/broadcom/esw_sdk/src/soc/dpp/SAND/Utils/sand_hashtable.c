...
/*********************************************************************
* NAME:
*   soc_sand_hash_table_get_hashs_array_ptr_from_handle
* TYPE:
*   PROC
* DATE:
*   May 31 2015
* FUNCTION:
*   Get value of 'hashs_array' pointer (See SOC_SAND_HASH_TABLE_INFO)
*   from handle.
* INPUT:
...
*   SOC_SAND_INOUT  SOC_SAND_HASH_TABLE_INFO        **hashs_array_ptr_ptr -
*     This procedure loads pointed memory by address of corersponding
*     element in array of pointers 'hashs_array' (of specified hash table).
*     This is the address to an element of type SOC_SAND_HASH_TABLE_INFO.
* REMARKS:
*   This procedure is exceptional. It is added here so we can use
*   the buffer pointed by 'hashs_array' as a work space whose address
*   is passed to various utilities.
*   This procedure should be TAKEN OUT OF THIS CODE after the whole
*   'warm boot printout' module is replaced (file sw_state_dump.c). It
*   breaks the rule of 'no pointers' for the 'new sw state' module. In
*   any case, users of this procedure are assumed to ONLY READ from
*   and NOT WRITE into pointed SOC_SAND_HASH_TABLE_INFO structure.
*   
* RETURNS:
  ...
  )
{
  ...
  *hashs_array_ptr_ptr = sw_state[unit]->dpp.soc.sand.hash_table->hashs_array[hash_table_index] ;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_sand_hash_table_get_hashs_array_ptr_from_handle()",0,0);
}
...
*   May 18 2015
* FUNCTION:
*   Get value of 'tmp_buf' pointer (See SOC_SAND_HASH_TABLE_T)
*   from handle.
* INPUT:
*   SOC_SAND_IN  int                            unit -
*     Identifier of the device to access.
...
*   May 18 2015
* FUNCTION:
*   Get value of 'tmp_buf2' pointer (See SOC_SAND_HASH_TABLE_T)
*   from handle.
* INPUT:
*   SOC_SAND_IN  int                            unit -
*     Identifier of the device to access.
...
*   SOC_SAND_IN     int                           unit -
*     Identifier of the device to access.
*   SOC_SAND_IN  SOC_SAND_HASH_TABLE_PTR          *hash_table_ptr -
  ...
  )
{
  ...
  *hash_table_ptr = hash_table ;
  ...
  if (init_info.hash_function != NULL || init_info.rehash_function != NULL )
  {
    SOC_SAND_SET_ERROR_CODE(SOC_SAND_VALUE_OUT_OF_RANGE_ERR, 8, exit);
  }
   ...
   */
  res = HASH_TABLE_ACCESS_INFO.hash_function.get(unit, hash_table_index, &hash_function) ;
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 66, exit) ;
  if (!hash_function)
  {
    hash_function = soc_sand_hash_table_simple_hash ;
    res = HASH_TABLE_ACCESS_INFO.hash_function.set(unit, hash_table_index, hash_function) ;
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 70, exit) ;
  }
  /*
   * if the rehash function is not given then use the default rehash function.
   */
  res = HASH_TABLE_ACCESS_INFO.rehash_function.get(unit, hash_table_index, &rehash_function) ;
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 74, exit) ;
  if (!rehash_function)
  {
    rehash_function = soc_sand_hash_table_simple_rehash ;
    res = HASH_TABLE_ACCESS_INFO.rehash_function.set(unit, hash_table_index, rehash_function) ;
