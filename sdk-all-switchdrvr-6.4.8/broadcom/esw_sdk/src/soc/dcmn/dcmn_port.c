...
/** 
 *  @brief get phymod access structure for mdio core via function "soc_dcmn_mdio_phy_access_get"
 *  @param unit-
 *  @param addr - core address
 *  @param access - function output. the phymod core access
 */
...
soc_dcmn_port_phy_control_set(int unit, soc_port_t port, int phyn, int lane, int is_sys_side, soc_phy_control_t type, uint32 value)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_dcmn_port_phy_control_type_validate(unit, type));
...
soc_error_t
soc_dcmn_port_phy_control_type_validate(int unit, soc_phy_control_t type)
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    switch (type) 
    {
        case SOC_PHY_CONTROL_PREEMPHASIS:
            SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,(_BSL_SOCDNX_MSG("For TX FIR use SOC_PHY_CONTROL_TX_FIR_ [PRE / MAIN / POST]")));
            break;
        default:
        break;
    }
