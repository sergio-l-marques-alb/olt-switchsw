 ...
 * TYPE DEFS *
 *************/
static dcmn_block_control_info_t dcmn_intr_blocks_control_info[SOC_MAX_NUM_DEVICES][NOF_DNX_BLOCKS];
...
dcmn_set_block_control_regs_info_cb(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    ...
    dcmn_block_control_info_t *config = m->config;
    soc_reg_t reg=ainfo->reg;
    soc_reg_info_t reg_info = SOC_REG_INFO(unit, reg);   
    soc_block_type_t block = REG_FIRST_BLK_TYPE(reg_info.block) - SOC_BLK_FIRST_DNX;

    if (SOC_REG_FIELD_VALID(unit,reg,CPU_BYPASS_ECC_PARf)) {
    if (config->gmo_reg != INVALIDr) {
...
void dcmn_collect_blocks_control_info(int unit, dcmn_block_control_info_t *map,dcmn_block_control_info_t *config)
{
    ...
    for (i=0;i<nof_ints;i++) {
        ...
        block = REG_FIRST_BLK_TYPE(reg_info.block) - SOC_BLK_FIRST_DNX;
    ...
    soc_reg_iterate(unit, dcmn_set_block_control_regs_info_cb, &m);
}
...
int
dcmn_disable_block_ecc_check(int unit, int interrupt_id, soc_mem_t mem, soc_reg_above_64_val_t value, soc_reg_above_64_val_t orig_value)
{
    soc_block_t block ;
    ...
    if (orig_value!=NULL) {
        SOCDNX_IF_ERR_EXIT(soc_reg_above_64_get(unit, ecc1_monitor_mem_reg,  REG_PORT_ANY, 0, orig_value));
    }
    else
    {
        soc_interrupt_db_t *ints_db = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info;
        ...
        SOCDNX_IF_ERR_EXIT(dcmn_get_cnt_reg_values(unit, get_cnt_reg_type(unit,cnt_reg), cnt_reg,&cntf, &cnt_overflowf, &addrf,&addr_validf));
        if (addr_validf) {
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_set(unit,ecc1_monitor_mem_reg, REG_PORT_ANY, 0,value));
