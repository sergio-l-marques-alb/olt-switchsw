...
/*
 * $Id: g2p3_lpm.c,v 1.60 Broadcom SDK $
 *
...
struct lpmWork_s {
  ...
  uint8                 nodeCached;   /* node above is cached         */ 
  ...
  uint8                 pyldCached;   /* Payload above is cached      */ 
  uint8                 pfxLen;       /* Prefix length                */
...
struct lpmPruneWork_s {
  ...
  uint8                 *slices;      /* pointer to previous slice               */
...
typedef struct {
  uint32        numWork;          /* number of work item used */
  uint32        maxWork;          /* maximum number of work item admitted*/ 
  uint32        numPruneWork;     /* number of add prune work items */
  /* work item pool */  
...
static
sbBool_t
lpmIsSingletonPath(lpmNode_p_t node, uint8 slice);
...
extractPayload(g2p3_lpmPayloadHandle_t *pMain, /* main payload */
               ...
               uint32 *pDropMaskLo,              /* port drop mask */
               uint32 *pDropMaskHi,
...
static
uint32
lpmEncodeSkip(g2p3_lpmAddress_t addr, uint8 nbits);
#endif
...
refDevPayloads(g2p3_lpmDPMHandle_t dpm,
               uint32 *pCount, lpmNode_p_t node,
               uint8 payKind,
               void *payload, g2p3_lpmClassifierId_t clsA,
               g2p3_lpmClassifierId_t clsB, uint32 dropMaskLo,
               uint32 dropMaskHi);
...
static
sbStatus_t
lpmFinalCleanup(g2p3_lpmHandle_t lpm, const uint8 *slices,
...
lpmAddRoute(g2p3_lpmHandle_t lpm, lpmNode_p_t *pTrie,
            uint8 thisSlice, uint8 *nextSlices,	/* zero-terminated */
            g2p3_lpmAddress_t addr, uint8 length,
...
lpmDropDown(g2p3_lpmHandle_t lpm, g2p3_lpmAddress_t addr,
            ...
            uint32 *pDropMaskLo, uint32 *pDropMaskHi,
...
static
sbStatus_t
devPayloadsRec(g2p3_lpmDPMHandle_t dpm, uint32 *pCount, lpmNode_p_t node,
               sbBool_t ref, /* ref if true; deref otherwise */
               uint8 payKind, /* where to stop */
                 ...
                 sbBool_t ipv6)
{
    ...
    hostAllocBytes = (sizeof(g2p3_lpm_t)          /* lpm struct */
                      + RND_TO_WORD(sliceSlots) * sizeof(uint8)  /* slices */
             ...
             const uint8 *slices)
{
    ...
    case LPM_NODE_SPLIT:
    {
        ...
        if (node->dpsBelow == 0) {
            ...
            uint32 address = node->data.split.children[0] == NULL;
                       ...
                       uint8 Length)
{
    ...
    lpmWork_p_t work;
    unsigned lcp;
    uint8 incrLength=0, decLength=Length, pk;
    lpmPath_p_t path;
    uint32 len1, len2;
    ...
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("\n %s : Addr %x: Len %d"), FUNCTION_NAME(), COMPILER_64_LO(addr), Length));
#endif
    lpm->async = FALSE;
    path->node = lpm->trie;
    path->parent = NULL;
        ...
        case LPM_NODE_TABLE:
          ...
          case LPM_NODE_SPLIT:
            ...
            if (path->seen) {
              ...
              *(path+1) = *path;
              path->seen++;
              path->child = (COMPILER_64_BITTEST(addr, (decLength - 1)));
          ...
          case LPM_NODE_SKIP:
          {
            uint8 skipLen = path->node->tag.skipLen + 1;
            uint8 skipLenDiff = path->slice - skipLen;
            uint32 skipAddr = path->node->data.skip.address;
            ...
            if(path->seen) {
              ...
              *(path + 1) = *path;
              ++path->seen;
              COMPILER_64_SET(uuAddr2, 0, path->node->data.skip.address);
    ...
    while (node) {
        ...
        switch (node->tag.type) {
            ...
            case LPM_NODE_SPLIT:
                ...
                if (status != SB_OK)
                    return (status);
                COMPILER_64_SET(uuVal, 0, 1);
                COMPILER_64_SHL(uuVal, (lpm->addrBits - length));
                COMPILER_64_OR(uuVal, addr);
            ...
            case LPM_NODE_SKIP:
                COMPILER_64_SET(uuVal, 0, node->data.skip.address);
                COMPILER_64_SHL(uuVal, (lpm->addrBits - length - (node->tag.skipLen + 1)));
                COMPILER_64_OR(addr, uuVal);
                       ...
                       uint8 length)
{
    ...
    do {
         ...
         * the declared length
         */
        COMPILER_64_SET(uuAddrTmp, 0, 1);
        COMPILER_64_SHL(uuAddrTmp, lpm->addrBits - length);
        COMPILER_64_SUB_32(uuAddrTmp, 1);
        COMPILER_64_NOT(uuAddrTmp);
        COMPILER_64_AND(uuAddrTmp, addr);
        lAddr =  uuAddrTmp;
        auxAddr =  lAddr;

        COMPILER_64_SHR(lAddr, lpm->addrBits - length);
               ...
               g2p3_lpmPayloadKind_t payloadKind, lpmWork_p_t work)
{
    ...
    while (node != NULL && ((node->dpsBelow > 0) || lpmIsDead(node))) {
        if (lpmIsDead(node)) {
            ...
            case LPM_NODE_SPLIT:
                if(work) {
                    SPLIT_WORK_OFFSET(work, COMPILER_64_BITTEST(addr, length - 1));
                }
                
                node = node->data.split.children[COMPILER_64_BITTEST(addr,(length - 1))];
                --length;
                break;
            case LPM_NODE_SKIP:
                COMPILER_64_SET(uuTmp, 0, node->data.skip.address);
                lcp = lcplen(addr, length, uuTmp,
                             node->tag.skipLen + 1);
               ...
               lpmWork_p_t work, lpmNode_p_t parent)
{
    ...
    uint32 dpsPre;
    uint8 lenDiff=0;
    g2p3_lpmAddress_t addr2 = COMPILER_64_INIT(0,0);
    int bit = 0;
    g2p3_lpmSramAddr        lastTblAddr=0;
    sbBool_t                lastTblSingleton=FALSE;
    ...
    case LPM_NODE_TABLE:
        ...
        if(work) {
          COMPILER_64_SET(addr2, 0, work->dest);
          lastTblAddr = work->lastTblAddr;
          lastTblSingleton = work->lastTblSingleton;
    ...
    case LPM_NODE_SKIP:
        {
            uint8 length2 = trie->tag.skipLen + 1;
            COMPILER_64_SET(addr2, 0, trie->data.skip.address);
		...
		return SB_LPM_ADDRESS_NOT_FOUND;
            ...
            if(work) {
              ...
              SKIP_WORK_OFFSET(work, COMPILER_64_LO(addr2), 
    ...
    case LPM_NODE_SPLIT:
	{
	    bit = COMPILER_64_BITTEST(addr,(length - 1));
    ...
    if (status == SB_OK) {
        uint32 dpsPost = lpmDpsBelow(trie);
        uint8 cachePoint = 0;
        ...
        if((work) && (!work->nodeCached)) {
           ...
           switch(trie->tag.type) {
               case LPM_NODE_SKIP:
                 work->dest -= (COMPILER_64_LO(addr2) << lenDiff);
               ...
               case LPM_NODE_TABLE:
                 work->slice = 0;
                 work->slices--;
                 work->dest = COMPILER_64_LO(addr2);/*previous points address */
                 work->lastTblAddr = lastTblAddr;
                 work->lastTblSingleton = lastTblSingleton;
                 break;
...
g2p3_lpmAdd(g2p3_lpmHandle_t lpm, g2p3_lpmAddress_t addr, 
            ...
            g2p3_lpmPayloadKind_t payloadKind,uint8 l3cached)
{
...
#if LPM_LPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("g2p3_lpmAdd: addr 0x%02x%08x/%02d, payload = %p\n"),
              COMPILER_64_HI(addr), COMPILER_64_LO(addr), length, payload));
#endif
    
    COMPILER_64_SHR(addr, lpm->addrBits - length);
...
g2p3_lpmRemove(g2p3_lpmHandle_t lpm, g2p3_lpmAddress_t addr,
               uint8 length, g2p3_lpmPayloadKind_t payloadKind,
               uint8 l3cached)
{
    lpmWork_p_t work = NULL;
    sbStatus_t  status = SB_OK;
    ...
    #if LPM_LPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("g2p3_lpmRemove: addr 0x%02x%08x/%02d\n"),
              COMPILER_64_HI(addr), COMPILER_64_LO(addr), length));
    #endif
...
static
sbStatus_t
refDevPayloads(g2p3_lpmDPMHandle_t dpm, uint32 *pCount, lpmNode_p_t node,
               uint8 payKind, void *payload, g2p3_lpmClassifierId_t clsA,
               g2p3_lpmClassifierId_t clsB, uint32 dropMaskLo,
               uint32 dropMaskHi)
{
    sbStatus_t status;
...
static
sbStatus_t
derefDevPayloads(g2p3_lpmDPMHandle_t dpm, uint32 *pCount,
                 lpmNode_p_t node,
                 uint8 payKind, void *payload, g2p3_lpmClassifierId_t clsA,
                 g2p3_lpmClassifierId_t clsB, uint32 dropMaskLo,
                 uint32 dropMaskHi)
              ...
              lpmWork_p_t work)
{
    ...
    uint32 rmDropMaskLo, rmDropMaskHi;
    ...
    if (*pTrie == NULL
        ...
        *pTrie = newNode;
        ...
        uint32 dpsPre = lpmDpsBelow(*pTrie);
        ...
        status = lpmAddPayload(lpm, &((*pTrie)->data.payload.child),
                               ...
                               dropMaskHi, work);
        if (status == SB_OK) {
            uint32 dpsPost = lpmDpsBelow(*pTrie);
            SB_ASSERT(dpsPost >= dpsPre);
        ...
        switch (node->tag.type) {
            case LPM_NODE_PAYLOAD:
                if (node->tag.payloadKind == G2P3_LPM_PAYLOAD_CLS_C) {
                    ...
                    *pClsC =
                      (g2p3_lpmClassifierId_t)(uint32)node->data.payload.desc;
                }
                else {
            ...
            case LPM_NODE_SKIP:
                ...
                COMPILER_64_SET(addr2, 0, node->data.skip.address);
...
g2p3_lpmAddC(g2p3_lpmHandle_t lpm, g2p3_lpmAddress_t addr,
                 uint8 length, void **payMain, g2p3_lpmClassifierId_t *pClsA,
                 g2p3_lpmClassifierId_t *pClsB, uint32 *pDropMaskLo,
                 uint32 *pDropMaskHi, g2p3_lpmClassifierId_t *pClsC)
{
    sbStatus_t status;
    ...
    if (cC == 0) {
        ...
        *pClsC = payloadDevAddr - lpm->deviceMemA->startAddr;
        ...
        /* add CLS_C payload */
        status = g2p3_lpmAdd (lpm, addr, length, (void *)(uint32)*pClsC,
            ...
            lpmWork_p_t work)
{
    ...
    /* add at end of trie */
    if (*pTrie == NULL) {
        uint64 uuAddrTmp, uuAddrMsk = COMPILER_64_INIT(0,1);
        ...
        COMPILER_64_SHL(uuAddrMsk, skip);
        COMPILER_64_SUB_32(uuAddrMsk, 1);
        uuAddrTmp =  addr;
        COMPILER_64_SHR(uuAddrTmp, (length-skip));
        COMPILER_64_AND(uuAddrTmp, uuAddrMsk);
        newNode->data.skip.address = COMPILER_64_LO(uuAddrTmp);
    ...
    if (((*pTrie)->tag.type == LPM_NODE_TABLE)) {
      if ((*pTrie)->tag.singleton) {
        ...
        COMPILER_64_SET(addr2, 0, (*pTrie)->data.table.child->data.skip.address);
        lcp = lcplen(addr, length, addr2, length2);
        if ((lcp < length2)
    ...
    case LPM_NODE_SKIP:
         ...
         */
        {
            ...
            COMPILER_64_SET(addr2, 0, (*pTrie)->data.skip.address);
            lcp = lcplen(addr, length, addr2, length2);
            ...
            if (lcp == length2) {
                ...
                newNode->tag.state = LPM_NODE_STATE_LIVE;
                COMPILER_64_SHL(uuAddrMsk, lcp);
                COMPILER_64_SUB_32(uuAddrMsk, 1);
                uuAddrTmp =  addr;
                COMPILER_64_SHR(uuAddrTmp, (length - lcp));
                COMPILER_64_AND(uuAddrTmp, uuAddrMsk);
                newNode->data.skip.address = COMPILER_64_LO(uuAddrTmp);
                ...
                if (status == SB_OK) {
                    *pTrie = newNode;
                    ...
                    (*pTrie)->tag.skipLen += lcp;
                    (*pTrie)->data.skip.address = COMPILER_64_LO(addr2);
                    SB_ASSERT(g2p3_lpmHostMemFree(lpm->hostMem, newNode,
                                                   sizeof(lpmNode_t))
                           == SB_OK);
                           /* 			    checkSkipNode(*pTrie); */
                ...
                if (length2 == 1) {
                    ...
                    newNode->tag.state = LPM_NODE_STATE_LIVE;
                    newNode->data.skip.address =
                        COMPILER_64_LO(addr2) & ((1 << (length2 - 1)) - 1);
                   ...
                   lpmWork_p_t work)
{
    ...
    uint32 dropMaskLo, dropMaskHi, oldDropMaskLo, oldDropMaskHi;
    g2p3_lpmPayloadHandle_t payloadHandle;
...
#if LPM_LPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("g2p3_lpmUpd: addr 0x%02x%08x/%02d, payload = %p\n"),
              COMPILER_64_HI(addr), COMPILER_64_LO(addr), length, newPayload));
#endif
    COMPILER_64_SHR(addr, lpm->addrBits - length);
    status = lpmFindPayload(lpm, &node, &oldPayload, &oldClsA, &oldClsB,
                            &oldDropMaskLo, &oldDropMaskHi, addr,
    ...
    if (payloadKind == G2P3_LPM_PAYLOAD_DROPM_LO) {
        extractPayload(&payload, &clsA, &clsB,
                       (uint32 *)&node->data.payload.desc,
                       &dropMaskHi, payloadKind, newPayload);
    }
    else if (payloadKind == G2P3_LPM_PAYLOAD_DROPM_HI) {
        extractPayload(&payload, &clsA, &clsB,
                       &dropMaskLo, (uint32 *)&node->data.payload.desc,
                       payloadKind, newPayload);
    }
    else
                ...
                if (path->node->tag.state != LPM_NODE_STATE_DEAD) {
                    ...
                    if (path->node->tag.payloadKind ==
                        ...
                        path->clsC = (g2p3_lpmClassifierId_t)
                            (uint32)path->node->data.payload.desc;
                    ...
                    if (lpm->status != SB_OK) {
                        LOG_WARN(BSL_LS_SOC_COMMON,
                                 (BSL_META("g2p3_lpmDPMAddress non-OK on: payload dev addr 0x%08x, payload = %p, clsa = %x, clsbGu = %x, masklo = %x, maskhi = %x\n"),
            ...
            case LPM_NODE_SKIP: {
                ...
                case 0:
                    ...
                    *(path + 1) = *path;
                    ...
                    ++lpm->pathLength;
                    if (lpm->resetCommit || lpmAnyDirty(path)) {
                        uint32 skipAmt = skipAddr << skipLenDiff;
                ...
                case 1:
                    --lpm->pathLength;
                    if (lpm->resetCommit || lpmAnyDirty(path)) {
                        uint32 skipAmt = (skipAddr + 1) << skipLenDiff;
...
commitDelPruneWorkItem(g2p3_lpmHandle_t lpm)
{
    ...
    SB_ASSERT(LPMNODE_SIZE_IN_BYTES == sizeof(uint32));
...
#if LPM_LPM_DEBUG
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META("LPM commit: calling g2p3_lpmDmaFlush\n")));
#endif
        TIME_STAMP_START;
...
g2p3_lpmInitCommit(g2p3_lpmHandle_t lpm)
{
    ...
    if(g2p3_is_fast_update(workmgr)){
	/* commit all pending workitems so that
	 * we will do normal update next.
	 */
	status = g2p3_lpmCommit(lpm, NULL, NULL);
	if (status != SB_OK) {
	    return status;
	}
    }
    assert(!g2p3_is_fast_update(workmgr));

    /* resetCommit = TRUE and non-fast update will
     * force all tables to be DMAed again (calling
     * commitPath with resetCommit == TRUE)
     */
    lpm->resetCommit = TRUE;
    return g2p3_lpmCommit(lpm, NULL, NULL);
}
               ...
               g2p3_lpmPayloadKind_t payloadKind, void *payload)
{
    ...
    case G2P3_LPM_PAYLOAD_CLS_A:
        *pClsA = ((g2p3_lpmClassifierId_t) (uint32) payload);
        break;
    case G2P3_LPM_PAYLOAD_CLS_B:
        *pClsB = ((g2p3_lpmClassifierId_t) (uint32) payload);
        break;
    case G2P3_LPM_PAYLOAD_DROPM_LO:
        auxMask = ((uint32)payload >> 12) & 0xFFF;
        for (i = 0; i < 12; i++) {
            j = (1 << i);
            if (auxMask & j) {
                if ((uint32)payload & j)
       ...
       g2p3_lpmAddress_t addr2, unsigned len2)
{
    ...
    uint64 uuMsk = COMPILER_64_INIT(0,1);

    COMPILER_64_SHL(uuMsk, len);
    COMPILER_64_SUB_32(uuMsk, 1);
    COMPILER_64_SHR(addr1, (len1 - len));
    COMPILER_64_AND(addr1, uuMsk);
    COMPILER_64_SHR(addr2, (len2 - len));
    COMPILER_64_AND(addr2, uuMsk);
    diff =  addr1;
    COMPILER_64_XOR(diff, addr2);
    while (!COMPILER_64_IS_ZERO(diff)) {		/* there are faster but not-so-portable
                                 * ways */
        COMPILER_64_SHR(diff,1);
        --len;
    }
    return len;
}
                ...
                g2p3_lpmWalkHandle_t WalkHandle)
{
   ...
   if(pDesc->initdone == 0 ) {
    ...
    pDesc->pathLength = 1;
       ...
       if( (COMPILER_64_EQ(curraddr, pDesc->prevlpm))  &&
           (currlen == COMPILER_64_LO(pDesc->prevlen)) ) {
              /*
    ...
    while (pDesc->pathLength > 0) {
        if (pDesc->path->node == NULL) {
            ...
            {
                uint64 uuTmp;
                uuTmp =  pDesc->path->addr;
                  if(!found) {
                      if(pDesc->initdone == 0) {
                        if( (COMPILER_64_EQ(curraddr, uuTmp)  &&
            ...
            {
                uint64 uuTmp2;
                COMPILER_64_SET(uuTmp2, 0, 1);
                *(pDesc->path+1) = *pDesc->path;
                pDesc->path->node = pDesc->path->node->data.split.children[1];
                COMPILER_64_SHL(uuTmp2, ((lpm->addrBits) - (pDesc->path->len+1)));
                COMPILER_64_OR(pDesc->path->addr, uuTmp2);
            ...
            case LPM_NODE_SKIP: {
                uint8 skipLen   = pDesc->path->node->tag.skipLen + 1;
                g2p3_lpmAddress_t skipAddr;

                COMPILER_64_SET(skipAddr, 0,pDesc->path->node->data.skip.address);
                switch (pDesc->path->seen) {
                case 0:
                        COMPILER_64_SHL(skipAddr, 
                                        ((lpm->addrBits) - 
                                         (pDesc->path->len + skipLen)));
                        COMPILER_64_OR(pDesc->path->addr, skipAddr);
                        pDesc->path->len += skipLen;
                                 ...
                                 lpmNode_p_t newnode)
{
    ...
    if(work){       
        SB_ASSERT(newnode);
        SB_ASSERT(workmgr);
        if (workmgr->numPruneWork >= G2P3_MAX_SINGLETON_SLICES) {
#if LPM_LPM_DEBUG
    LOG_WARN(BSL_LS_SOC_COMMON,
             (BSL_META("\n %s : numPruneWork %d out of range, work aborted\n"), FUNCTION_NAME(), workmgr->numPruneWork));
#endif
            return;
        }
    ...
    for(uIdx = 0; uIdx < G2P3_LPM_STATS_UPD_MAX; uIdx++) {
      for(cIdx = 0; cIdx < G2P3_LPM_STATS_TYPE_MAX; cIdx++) {
        for(tIdx = 0; tIdx < G2P3_LPM_STATS_MAX_IDX; tIdx++) {
            COMPILER_64_ZERO(stats->counter[uIdx][cIdx][tIdx]);    
        }
      }
    }
}

/*#define LPM_STATS_ROLL_THLD ((1 << (sizeof(uint64) * CHAR_BIT))-1)*/
                      ...
                      lpmStatsUpdateType_t uType)
{
    ...
    COMPILER_64_SET(lpm_stats_roll_thld, 0xffffffff, 0xfffffffe);
    if(COMPILER_64_GE(stats->counter[uType][type][G2P3_LPM_STATS_IDX], lpm_stats_roll_thld)) {
        COMPILER_64_ZERO(stats->counter[uType][type][G2P3_LPM_STATS_IDX]);    
        COMPILER_64_ADD_32(stats->counter[uType][type][G2P3_LPM_STATS_ROLL_IDX], 1);
    } else {
        COMPILER_64_ADD_32(stats->counter[uType][type][G2P3_LPM_STATS_IDX], 1);
    }
    return SB_OK;
}
                         ...
                         sbBool_t verbose)
{
    ...
    LOG_CLI((BSL_META("\n Number of Route Add [0x%x%08x] Rollover [0x%x%08x]"),
             COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
             [G2P3_LPM_STATS_TYPE_ADD][G2P3_LPM_STATS_IDX]),
             COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
             [G2P3_LPM_STATS_TYPE_ADD][G2P3_LPM_STATS_IDX]),
             COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
    ...
    LOG_CLI((BSL_META("\n Number of Route Delete [0x%x%08x] Rollover [0x%x%08x]"),
             COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
    ...
    LOG_CLI((BSL_META("\n Number of Route Update [0x%x%08x] Rollover [0x%x%08x]"),
             COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
    ...
    LOG_CLI((BSL_META("\n Number of Commit [0x%x%08x] Rollover [0x%x%08x]"),
             COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_SUCCESS]\
    ...
    if(verbose) {
      LOG_CLI((BSL_META("\n Number of Route Add Requests [0x%x%08x] Rollover [0x%x%08x]"),
               COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_ADD][G2P3_LPM_STATS_IDX]),
               COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_ADD][G2P3_LPM_STATS_IDX]),
               COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_ADD][G2P3_LPM_STATS_ROLL_IDX]),
               COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
      ...
      LOG_CLI((BSL_META("\n Number of Delete Requests [0x%x%08x] Rollover [0x%x%08x]"),
               COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_DEL][G2P3_LPM_STATS_IDX]),
               COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_DEL][G2P3_LPM_STATS_IDX]),
      ...
      LOG_CLI((BSL_META("\n Number of Update Requests [0x%x%08x] Rollover [0x%x%08x]"),
               COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_UPDATE][G2P3_LPM_STATS_IDX]),
               COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_UPDATE][G2P3_LPM_STATS_IDX]),
      ...
      LOG_CLI((BSL_META("\n Number of Commit Requests [0x%x%08x] Rollover [0x%x%08x]"),
               COMPILER_64_HI(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_COMMIT][G2P3_LPM_STATS_IDX]),
               COMPILER_64_LO(stats->counter[G2P3_LPM_STATS_UPD_REQ]\
               [G2P3_LPM_STATS_TYPE_COMMIT][G2P3_LPM_STATS_IDX]),
