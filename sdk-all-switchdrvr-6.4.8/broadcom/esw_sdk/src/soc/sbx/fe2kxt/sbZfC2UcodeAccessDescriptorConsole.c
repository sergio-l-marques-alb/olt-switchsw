...
sbZfC2UcodeAccessDescriptor_Print(sbZfC2UcodeAccessDescriptor_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2UcodeAccessDescriptor:: rwn=0x%01x"), (unsigned int)  pFromStruct->m_nRWN));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" port=0x%02x"), (unsigned int)  pFromStruct->m_nPort));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" addr=0x%07x"), (unsigned int)  pFromStruct->m_nAddr));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" data=0x%04x"), (unsigned int)  pFromStruct->m_nData));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
