...
sbZfC2PmLrMeterRequest_Print(sbZfC2PmLrMeterRequest_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PmLrMeterRequest:: resv0=0x%03x"), (unsigned int)  pFromStruct->m_uResv0));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" pid=0x%06x"), (unsigned int)  pFromStruct->m_uPolicerId));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" resv1=0x%04x"), (unsigned int)  pFromStruct->m_uResv1));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" blnd=0x%01x"), (unsigned int)  pFromStruct->m_uBlind));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("C2PmLrMeterRequest:: dred=0x%01x"), (unsigned int)  pFromStruct->m_uDropOnRed));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META(" plen=0x%04x"), (unsigned int)  pFromStruct->m_uPacketLength));
  LOG_WARN(BSL_LS_SOC_COMMON,
...
char *
sbZfC2PmLrMeterRequest_SPrint(sbZfC2PmLrMeterRequest_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;
