 ...
 *
 * $Id: sbFe2000Util.c,v 1.10 Broadcom SDK $
 *
 ******************************************************************************/
...
uint8
sbFe2000UtilPvtCompensation(sbhandle userDeviceHandle, uint32 uMmInstance)
{
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META("Calibrating MM%d PVT Compensation\n"),uMmInstance));
  ...
  if ((uRetry == 0) && !uDone) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("MM%d PVT compensation timeout\n"), uMmInstance));
    bError = TRUE;
  }
  
  if (SAND_HAL_GET_FIELD(CA, MM_PVT_STATUS, PVT_COMP_ERROR, uData)) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("MM%d PVT compensation error, MM_PVT_STATUS=0x%x\n"), uMmInstance,
              uData));
