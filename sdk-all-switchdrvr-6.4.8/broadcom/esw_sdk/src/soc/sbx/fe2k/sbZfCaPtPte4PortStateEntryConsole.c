/*
 * $Id: sbZfCaPtPte4PortStateEntryConsole.c,v 1.3 Broadcom SDK $
...
#include "sbZfCaPtPte4PortStateEntryConsole.hx"



/* Print members in struct */
void
sbZfCaPtPte4PortStateEntry_Print(sbZfCaPtPte4PortStateEntry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: pktinprogress=0x%01x"), (unsigned int)  pFromStruct->m_uPktInProgress));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: doubledequeued=0x%01x"), (unsigned int)  pFromStruct->m_uDoubleDequeued));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: residuebyte=0x%01x"), (unsigned int)  pFromStruct->m_uResidueByte));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: lastpage=0x%01x"), (unsigned int)  pFromStruct->m_uLastPage));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: nextlength=0x%02x"), (unsigned int)  pFromStruct->m_uNextLength));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: remainingbytes=0x%02x"), (unsigned int)  pFromStruct->m_uRemainingBytes));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: pktlength=0x%04x"), (unsigned int)  pFromStruct->m_uPktLength));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("CaPtPte4PortStateEntry:: sourcequeue=0x%02x"), (unsigned int)  pFromStruct->m_uSourceQueue));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
char *
sbZfCaPtPte4PortStateEntry_SPrint(sbZfCaPtPte4PortStateEntry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  uint32 WrCnt = 0x0;

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: pktinprogress=0x%01x", (unsigned int)  pFromStruct->m_uPktInProgress);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: doubledequeued=0x%01x", (unsigned int)  pFromStruct->m_uDoubleDequeued);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," eopextraburst=0x%01x", (unsigned int)  pFromStruct->m_uEopExtraBurst);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: residuebyte=0x%01x", (unsigned int)  pFromStruct->m_uResidueByte);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: lastpage=0x%01x", (unsigned int)  pFromStruct->m_uLastPage);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: nextlength=0x%02x", (unsigned int)  pFromStruct->m_uNextLength);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: remainingbytes=0x%02x", (unsigned int)  pFromStruct->m_uRemainingBytes);
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: pktlength=0x%04x", (unsigned int)  pFromStruct->m_uPktLength);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," continuebyte=0x%02x", (unsigned int)  pFromStruct->m_uContinueByte);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"CaPtPte4PortStateEntry:: sourcequeue=0x%02x", (unsigned int)  pFromStruct->m_uSourceQueue);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
...
int
sbZfCaPtPte4PortStateEntry_Validate(sbZfCaPtPte4PortStateEntry_t *pZf) {
...
int
sbZfCaPtPte4PortStateEntry_SetField(sbZfCaPtPte4PortStateEntry_t *s, char* name, int value) {
