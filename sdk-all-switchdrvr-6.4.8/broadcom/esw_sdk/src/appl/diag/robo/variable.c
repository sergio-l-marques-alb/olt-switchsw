 /*
 * $Id: variable.c,v 1.10 Broadcom SDK $
 ...
 */
{
    ...
    snprintf(v_str, sizeof(v_str), "%d", value);
 ...
 */
{
    ...
    snprintf(v_str, sizeof(v_str), "0x%04x", value);
    return(var_set(name, v_str, local, system));
}
 ...
 */
{
    ...
    if (do_all) {
    	...
    	for (l = 1, vs = var_scope_list; vs; vs = vs->vs_next, l++) {
    	    snprintf(l_str, sizeof(l_str), "Scope Level %d", l);
