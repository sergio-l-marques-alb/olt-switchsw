...
/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    {  "SW handles of policer core 1"         , "Policer1"      , "POL_1"  , dpp_am_res_ether_policer_1                , NULL                              , NULL                              },
...
cmd_result_t cmd_diag_alloc_my_in_counter_profile_test(int unit)
{
      ...
      rv = _bcm_dpp_am_local_outlif_counting_profile_set(unit, counter_profile,9000, 10000);
      if (rv != BCM_E_NONE) {
        return CMD_FAIL;
     }
...
*/
    ...
    if (rv != BCM_E_NONE) {
        ...
        if (with_protection)
        {
            outlif_info.app_alloc_info.pool_id = dpp_am_res_eg_out_ac;
            outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED | BCM_DPP_AM_OUT_LIF_FLAG_WIDE | BCM_DPP_AM_OUT_LIF_FLAG_ODD_HALF;
            outlif_info.glif = 5014; /* pre-alloced Global LIF ID*/ 
            outlif_info.failover_id = 5;
            
            rv = _bcm_dpp_am_local_outlif_alloc(unit, flags, &outlif_info);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
            cli_out(" Outlif alloced . Global id=%d, local lif id= %d , ext_id=%d \n\r",outlif_info.glif,  outlif_info.base_lif_id, outlif_info.ext_lif_id );
            lif_id[2] = outlif_info.base_lif_id;
            outlif_info.glif = 5015; /* pre-alloced Global LIF ID*/ 
            outlif_info.failover_id = 5;
            ...
            outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED | BCM_DPP_AM_OUT_LIF_FLAG_WIDE | BCM_DPP_AM_OUT_LIF_FLAG_EVEN_HALF;
            outlif_info.glif = 5017; /* pre-alloced Global LIF ID*/ 
            outlif_info.failover_id = 5;
            ...
            rv = _bcm_dpp_am_local_outlif_dealloc(unit, lif_id[2]);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
             cli_out(" Outlif dealloced . lif id= %d , \n\r", lif_id[2]);
            ...
            outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED | BCM_DPP_AM_OUT_LIF_FLAG_WIDE | BCM_DPP_AM_OUT_LIF_FLAG_ODD_HALF;
            outlif_info.glif = 5018; /* pre-alloced Global LIF ID*/ 
            outlif_info.failover_id = 5;
            ...
            outlif_info.local_lif_flags = BCM_DPP_AM_OUT_LIF_FLAG_COUPLED | BCM_DPP_AM_OUT_LIF_FLAG_WIDE | BCM_DPP_AM_OUT_LIF_FLAG_EVEN_HALF;
            outlif_info.glif = 5019; /* pre-alloced Global LIF ID*/ 
            outlif_info.failover_id = 5;
            outlif_info.base_lif_id = 8200;

            rv = _bcm_dpp_am_local_outlif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &outlif_info);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }
            cli_out(" Outlif alloced . Global id=%d, local lif id= %d , ext_id=%d \n\r",outlif_info.glif,  outlif_info.base_lif_id, outlif_info.ext_lif_id );
...
cmd_result_t cmd_diag_alloc_failover_link_list(int unit)
{
    ...
    {
        rv = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_LOCAL_OUTLIF_FAILOVER_LINK_LIST_DB, &failover_link_list, failover_id_iter);
        if (rv != BCM_E_NONE) {
                return CMD_FAIL;
        }
        ...
        {
            rv = failover_linked_list_print(unit,&failover_link_list);
            if (rv != BCM_E_NONE) {
                return CMD_FAIL;
            }   
        }
