...
appl_dpp_cosq_diag_init(int unit)
{
    ...
    int int_flags, core_idx, core_id;
    ...
    for (modid_idx = 0; modid_idx < g_dii.nof_devices; modid_idx++) {
        for (idx = 0; idx < g_dii.num_ports; idx++) {
            ...
            if(domain == my_domain) {
                    for (tc_i = 0; tc_i < g_dii.num_cos; tc_i++) {
...
{
    int rv = BCM_E_NONE;
    bcm_field_group_config_t grp, grp2, grp3, grp4, grp5, grp6;
    ...
    bcm_field_data_qualifier_t data_qual, data_qual2, data_qual3, data_qual4, data_qual5, data_qual6 ; 
    bcm_field_group_t entry_groups[15]={122,123,122,122,127,122,122,124,124,124,124,124,125,126,126};
    ...
    for (entry_ndx = 0; entry_ndx < nof_itmh_entries; entry_ndx++) {
     ...
     case 9:  /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
      ...
      rv = bcm_field_direct_extraction_action_add(unit,
                                                  ent,
                                                  ext_action,
                                                  (entry_ndx == 5 || entry_ndx == 13 || entry_ndx == 14 ) ? 2 : 1 /* count */,
                                                  (entry_ndx == 5 || entry_ndx == 13 || entry_ndx == 14 ) ? ext_field2 : &ext_field );
