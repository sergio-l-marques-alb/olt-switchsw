            ...
            if(soc_feature(unit, soc_feature_portmod)) {
                ...
                char    lnstr[32], *pname, namelen; 
                    ...
                    if (!SAL_BOOT_SIMULATION) {
                        ...
                        rv = portmod_port_core_access_get(unit, p, -1, MAX_PORT_CORES, core_acc, &nof_cores, NULL);
                        if (BCM_FAILURE(rv)) {
                       ...
                       /* check if the external phy is a legacy phy */
                        local_user_access = (portmod_default_user_access_t *)(core_acc[0].access.user_acc);

                        /* ASSERT local_user_access not equal to NULL */
                        if (IS_SFI_PORT(unit, p)) {
                            is_legacy = 0;
                        } else {
                            is_legacy = local_user_access->is_legacy_phy_present;
                        }
                    ...
                    pname = phymod_core_version_t_mapping[core_info.core_version].key+17;
                    namelen = strlen(pname);

                    sal_snprintf(lnstr, sizeof(lnstr), "%s", pname); 
                    sal_snprintf(lnstr+namelen-2, sizeof(lnstr)-(namelen-2), "-%s/%02d/", pname+namelen-2, 0); 
...
_phy_jer_nif_measure(int unit, bcm_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane) {
    ...
    if (SOC_IS_QAX(unit)) {
        cli_out("ERROR: access to registers should be fixed for QAX at places we used _REG(32|64) access routines\n");               
        return BCM_E_FAIL;
    }
    ...
    *one_clk_time_measured_remainder = ((((total_time_measured_int*10000) + (total_time_measured_remainder)) / (sync_sth_cnt * vco_div)) % 10000);
    ...
    /* Get oversample */
    rv = soc_jer_portmod_calc_os(unit, phy_access, &os_int, &os_remainder);
    if (rv != BCM_E_NONE) {
        goto exit;
    }
