                                       ...
                                       )
{
  ...
  uint32
    prm_profile_ndx;
  SOC_PPD_LIF_COS_AC_PROFILE_INFO
    prm_profile_info;
  ...
  SOC_PPD_LIF_COS_AC_PROFILE_INFO_clear(&prm_profile_info);
                                       ...
                                       )
{
  ...
  SOC_PPD_LIF_COS_AC_PROFILE_INFO_print(&prm_profile_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LIF_COS_PROFILE_INFO_clear(&prm_profile_info);
...
{   
  ...
  SOC_PPD_LIF_COS_PROFILE_INFO_print(&prm_profile_info);
  ...
  )
{
  ...
  uint32
    prm_map_tbl_ndx;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_L2_KEY
    prm_map_key;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY
    prm_map_value;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_L2_KEY_clear(&prm_map_key);
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY_clear(&prm_map_value);
  ...
  )
{
  ...
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY_print(&prm_map_value);
  ...
  )
{
  ...
  uint32
    prm_map_tbl_ndx;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_IP_KEY
    prm_map_key;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY
    prm_map_value;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_IP_KEY_clear(&prm_map_key);
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY_clear(&prm_map_value);
                                        ...
                                        )
{
  ...
  uint32
    prm_profile_ndx;
  SOC_PPD_LIF_COS_PWE_PROFILE_INFO
    prm_profile_info;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LIF_COS_PWE_PROFILE_INFO_clear(&prm_profile_info);
                                        ...
                                        )
{
  ...
  send_string_to_screen("--> profile_info:", TRUE);
  SOC_PPD_LIF_COS_PWE_PROFILE_INFO_print(&prm_profile_info);
  ...
  )
{
  ...
  uint32
    prm_map_tbl_ndx;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_MPLS_KEY
    prm_map_key;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY
    prm_map_value;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_MPLS_KEY_clear(&prm_map_key);
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY_clear(&prm_map_value);
...
{   
  ...
  uint32   
    prm_map_tbl_ndx;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_TC_DP_KEY   
    prm_map_key;
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY   
    prm_map_value;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_TC_DP_KEY_clear(&prm_map_key);
  SOC_PPD_LIF_COS_PROFILE_MAP_TBL_ENTRY_clear(&prm_map_value);
                                    ...
                                    )
{
  ...
  use_l3 = prm_opcode_type & SOC_PPD_LIF_COS_OPCODE_TYPE_L3;
  use_tc_dp = prm_opcode_type & SOC_PPD_LIF_COS_OPCODE_TYPE_TC_DP;
  ...
  if (use_tc_dp)
  {
    prm_opcode_type = SOC_PPD_LIF_COS_OPCODE_TYPE_TC_DP;
  }
  else
  {
    prm_opcode_type = SOC_PPD_LIF_COS_OPCODE_TYPE_L2;
  }
  if (use_l3)
  {
    prm_opcode_type |= SOC_PPD_LIF_COS_OPCODE_TYPE_L3;
  }
  ...
  )
{
  ...
  SOC_SAND_PP_IPV6_TC   
    prm_ipv6_tos_ndx;
  SOC_PPD_LIF_COS_OPCODE_ACTION_INFO
    prm_action_info;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_LIF_COS_OPCODE_ACTION_INFO_clear(&prm_action_info);
  ...
  )
{
  ...
  SOC_PPD_LIF_COS_OPCODE_ACTION_INFO_print(&prm_action_info);
  ...
  )
{
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_LIF_TABLE_ENTRY_UPDATE_LIF_TABLE_ENTRY_UPDATE_LIF_ENTRY_INFO_TYPE_ID,1))
  {
	  ...
	  if(new_type == SOC_PPD_LIF_ENTRY_TYPE_AC && prm_lif_entry_info.type == SOC_PPD_LIF_ENTRY_TYPE_EMPTY){
		  prm_lif_entry_info.value.ac.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_INFO;
		  prm_lif_entry_info.value.ac.learn_record.learn_info.type = SOC_PPD_FRWRD_DECISION_TYPE_UC_PORT;
		  prm_lif_entry_info.value.ac.learn_record.learn_info.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_AC;
		  prm_lif_entry_info.value.ac.orientation = SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE;
		  prm_lif_entry_info.value.ac.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF;
	  }
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_LIF_TABLE_ENTRY_UPDATE_LIF_TABLE_ENTRY_UPDATE_LIF_ENTRY_INFO_VALUE_ISID_DEFAULT_FRWRD_DEFAULT_FRWD_TYPE_ID,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
	if(prm_lif_entry_info.type == SOC_PPD_LIF_ENTRY_TYPE_PWE)
    {
      prm_lif_entry_info.value.pwe.default_frwrd.default_frwd_type = param_val->numeric_equivalent;
    }
    if(prm_lif_entry_info.type == SOC_PPD_LIF_ENTRY_TYPE_ISID)
    {
  ...
  if (prm_lif_entry_info.value.ac.service_type != SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP)
  {
