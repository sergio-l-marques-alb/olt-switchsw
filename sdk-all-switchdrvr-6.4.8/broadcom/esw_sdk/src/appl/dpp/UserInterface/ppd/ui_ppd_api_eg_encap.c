/* $Id: ui_ppd_api_eg_encap.c,v 1.10 Broadcom SDK $
                                   ...
                                   )
{
  ...
  SOC_PPD_EG_ENCAP_RANGE_INFO_clear(&prm_range_info);
  bank_ndx = SOC_PPD_EG_ENCAP_NOF_BANKS(unit);
                                   ...
                                   )
{
  ...
  SOC_PPD_EG_ENCAP_RANGE_INFO_print(&prm_range_info);
...
/********************************************************************
*  Function handler: ac_entry_add (section eg_encap)
********************************************************************/
int
ui_ppd_api_eg_encap_ac_entry_add(
                                 CURRENT_LINE *current_line
                                 )
{
  uint32
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_eg_encap");
  soc_sand_proc_name = "soc_ppd_eg_encap_ac_entry_add";

  unit = soc_ppd_get_default_unit();

  /* Get parameters */
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_ENCAP_AC_ENTRY_ADD_AC_ENTRY_ADD_LIF_EEP_NDX_ID,1))
  {
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_ENCAP_AC_ENTRY_ADD_AC_ENTRY_ADD_LIF_EEP_NDX_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_lif_eep_ndx = (uint32)param_val->value.ulong_value;
  }
  else
  {
    send_string_to_screen(" *** SW error - expecting parameter lif_eep_ndx after ac_entry_add***", TRUE);
    goto exit;
  }

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_ENCAP_AC_ENTRY_ADD_AC_ENTRY_ADD_AC_ID_ID,1))
  {
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_ENCAP_AC_ENTRY_ADD_AC_ENTRY_ADD_AC_ID_ID);
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_ac_id = (uint32)param_val->value.ulong_value;
  }

  /* Call function */
  ret = soc_ppd_eg_encap_ac_entry_add(
    unit,
    prm_lif_eep_ndx,
    prm_ac_id
    );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
  {
    send_string_to_screen(" *** soc_ppd_eg_encap_ac_entry_add - FAIL", TRUE);
    soc_sand_disp_result_proc(ret, "soc_ppd_eg_encap_ac_entry_add");
    goto exit;
  }
  ...
  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_SWAP_INFO_clear(&prm_swap_info);
                                  ...
                                  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_PWE_INFO_clear(&prm_pwe_info);
  ...
  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_POP_INFO_clear(&prm_pop_info);
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_ENCAP_POP_COMMAND_ENTRY_ADD_POP_COMMAND_ENTRY_ADD_POP_INFO_ETHERNET_INFO_SYS_VSI_ID,1))
  {
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;
    prm_pop_info.ethernet_info.out_vsi = (uint32)param_val->value.ulong_value;
  }
  ...
  {
    prm_pop_info.pop_type = SOC_PPD_MPLS_COMMAND_TYPE_POP_INTO_ETHERNET;
  }
                                  ...
                                  )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_VSI_ENCAP_INFO_clear(&prm_vsi_encap_info);
                                   ...
                                   )
{
  ...
  uint32
    prm_tunnel_eep_ndx;
  SOC_PPD_EG_ENCAP_MPLS_ENCAP_INFO
    prm_mpls_encap_info;
  uint32
    idx,
    nof_ll_reps = 1,
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_MPLS_ENCAP_INFO_clear(&prm_mpls_encap_info);
                                   ...
                                   )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_IPV4_ENCAP_INFO_clear(&prm_ipv4_encap_info);
                                 ...
                                 )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_LL_INFO_clear(&prm_ll_encap_info);
  ...
  )
{
  ...
  uint32
    prm_profile_ndx;
  SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO
    prm_profile_info;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO_clear(&prm_profile_info);
  ...
  )
{
  ...
  SOC_PPD_EG_ENCAP_PUSH_PROFILE_INFO_print(&prm_profile_info);
                                      ...
                                      )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_PUSH_EXP_KEY_clear(&prm_exp_key);
                                      ...
                                      )
{
  uint32
    ret;
  SOC_PPD_EG_ENCAP_PWE_GLBL_INFO
    prm_glbl_info;
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_ENCAP_PWE_GLBL_INFO_clear(&prm_glbl_info);
                                      ...
                                      )
{
  ...
  SOC_PPD_EG_ENCAP_PWE_GLBL_INFO_print(&prm_glbl_info);
                    ...
                    )
{
  ...
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_ENCAP_AC_ENTRY_ADD_AC_ENTRY_ADD_ID,1))
  {
    ui_ret = ui_ppd_api_eg_encap_ac_entry_add(current_line);
  }
