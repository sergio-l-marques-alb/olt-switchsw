...
diag_pp_received_packet_bridge_test(int unit, int core_id) {
    ...
    rv = soc_port_sw_db_local_to_tm_port_get(unit, SOC_IS_JERICHO(unit) ? 200 : 0, &expected_pkt_info.in_tm_port, &expected_core);
    if ( rv < 0) {
        cli_out("Error: diag_pp_received_packet_bridge_test() local_to_pp Failed:\n");
        return rv;
    }
...
diag_pp_lif_lkup_info_bridge_test(int unit, int core_id) {
    ...
    expected_lif_lkup_info.base_index = (SOC_IS_JERICHO(unit)) ? 2 : 4096;
...
diag_pp_received_packet_router_test(int unit, int core_id) {
    ...
    rv = soc_port_sw_db_local_to_pp_port_get(unit, SOC_IS_JERICHO(unit) ? 200 : 0, &expected_pkt_info.in_tm_port, &expected_core);
    if ( rv < 0) {
        cli_out("Error: diag_pp_received_packet_router_test() local_to_pp Failed:\n");
        return rv;
    }
...
diag_pp_frwrd_trace_info_router_test(int unit, int core_id)
{
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_TRAP].frwrd_decision.additional_info.eei.val.outlif = (SOC_IS_JERICHO(unit)) ? 0x3f000 : 4096;
    ...
    expected_frwrd_trace_info.frwrd[SOC_PPC_DIAG_FRWRD_DECISION_PHASE_ING_RESOLVED].frwrd_decision.additional_info.eei.val.outlif = (SOC_IS_JERICHO(unit)) ? 0x3f000 : 4096;
...
diag_pp_encap_info_router_test(int unit, int core_id)
{
    ...
    expected_encap_info.eep[1] = (SOC_IS_JERICHO(unit)) ? 8192 : 4096;
...
diag_pp_lem_lkup_info_router_test(int unit, int core_id)
{
    ...
    expected_lem_lkup_info.value.host.frwrd_decision.dest_id = (SOC_IS_JERICHO(unit)) ? 4096 : 1024;
...
diag_pp_encap_info_mpls_test(int unit, int core_id)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        expected_encap_info.eep[0] = 8192;
    ...
    if (SOC_IS_JERICHO(unit)) {
        expected_encap_info.eep[1] = 16384;
    } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
...
diag_pp_received_packet_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    rv = soc_port_sw_db_local_to_tm_port_get(unit,  SOC_IS_JERICHO(unit) ? in_local_port : 0, &expected_pkt_info.in_tm_port, &expected_core);
    if ( rv < 0) {
        cli_out("Error: diag_pp_received_packet_vpls_test() local_to_pp Failed:\n");
        return rv;
    }
...
diag_pp_encap_info_vpls_test(int unit, int core_id, uint32 ac_to_pwe)
{
    ...
    if (ac_to_pwe) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            expected_encap_info.out_ac = 24576;
        } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
