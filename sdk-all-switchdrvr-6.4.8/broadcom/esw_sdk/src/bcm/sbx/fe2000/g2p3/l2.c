...
_bcm_fe2000_g2p3_l2addr_hw_add(int unit, bcm_l2_addr_t *l2addr)
{
	...
	maclog[my_log_id].fte=sbx_mac_data.pid;
	...
	maclog[my_log_id].fte=sbx_mac_data.pid;
        ...
        assert(SOC_SBX_NODE_ADDRESSABLE(unit, node));
    ...
    if (SOC_SBX_STATE(unit)->cache_l2 == FALSE) {
      ...
      rv = soc_sbx_g2p3_cmac_update(unit,
				    l2addr->mac,
				    l2addr->vid,
				    &sbx_mac_data);
    ...
    if (BCM_FAILURE(rv)) {
        ...
        if (SOC_SBX_STATE(unit)->cache_l2 == FALSE) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_L2,
                          (BSL_META_U(unit,
                                      "failed to commit L2 remove rv=%d(%s)\n"), 
                           rv, bcm_errmsg(rv)));
            }
        }
    }
                             ...
                             bcm_l2_addr_t *l2addr)
{
    ...
    maclog[my_log_id].modid=l2addr->modid;
    maclog[my_log_id].vid=l2addr->vid;
    maclog[my_log_id].fte=mac_data.pid;
            ...
            if (BCM_SUCCESS(rv)) {
                ...
                if (BCM_GPORT_MIM_PORT == type) {
                    ...
                    LOG_WARN(BSL_LS_BCM_L2,
                             (BSL_META_U(unit,
                                         "PID 0x%x in gport range, but unexpected type %d\n"),
                              mac_data.pid, type));
                }
            } else {
                LOG_ERROR(BSL_LS_BCM_L2,
            ...
            if (BCM_FAILURE(rv)) {
                ...
                if (BCM_FAILURE(status)) {
                    ...
                    if (vpncb.flags & BCM_MPLS_VPN_VPLS) {
                        BCM_GPORT_MPLS_PORT_ID_SET(l2addr->port, mac_data.pid);
                        rv = BCM_E_NONE;
                    } else {
                        LOG_ERROR(BSL_LS_BCM_L2,
                                  (BSL_META_U(unit,
                                     ...
                                     int qidunion)
{
    ...
    if (SOC_SBX_STATE(unit)->cache_l2 == FALSE) {
        ...
        if (BCM_FAILURE(rv)) {
            ...
            LOG_ERROR(BSL_LS_BCM_L2,
                      (BSL_META_U(unit,
                                  "failed to commit L2 add rv=%d(%s)\n"),
                       rv, bcm_errmsg(rv)));
            return rv;
        }
    }
...
_bcm_fe2000_g2p3_l2_age_remove(int unit, int *mackey, bcm_l2_addr_t *payload)
{
    ...
    if (bmac) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_L2,
                    (BSL_META_U(unit,
                                "Remove L2 aged entry MAC=" L2_6B_MAC_FMT 
                                 "  VLAN=%d: %s\n"),
