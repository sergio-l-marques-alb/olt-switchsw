                                  ...
                                  soc_sbx_g2p3_ft_t         *fte)
{
    ...
    if (sb_ret != SB_OK) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_IPMC,
                    (BSL_META_U(unit,
                                "unit %d: [%s] error(%s) returned from soc_sbx_g2p3_ipv6mcxxcomp_get()\n"),
                     unit, FUNCTION_NAME(),
                     bcm_errmsg(status)));
        return status;
    }
    ...
    if (sb_ret != SB_OK) {
        ...
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ft_get\n"),
                   unit, FUNCTION_NAME(),
                   bcm_errmsg(status)));
        return status;
    }
                                  ...
                                  soc_sbx_g2p3_ft_t         *fte)
{
    ...
    if (sb_ret != SB_OK) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_IPMC,
                    (BSL_META_U(unit,
                                "unit %d: [%s] error(%s) returned from soc_sbx_g2p3_ipv4mcxxcomp_get()\n"),
                     unit, FUNCTION_NAME(),
                     bcm_errmsg(status)));
        return status;
    }
                              ...
                              bcm_ipmc_addr_t        *data)
{
    ...
    if (flag_null_s_addr) {
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcg_get\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status)));
            return status;
        }
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcsg_get\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status)));
            return status;
        }
    ...
    if (flag_null_s_addr) {
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcg_update for ipmc-index(0x%x)\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status), data->group));
            return status;
        }
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcsg_update for ipmc-index(0x%x)\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status), data->group));
            return status;
        }
    ...
    if (sb_ret != SB_OK) {
        ...
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ft_set for ipmc-index(0x%x) fte(0x%x)\n"),
                   unit, FUNCTION_NAME(),
                   bcm_errmsg(status),
                   data->group, ftidx));
        return status;
                              ...
                              bcm_ipmc_addr_t        *data)
{
    ...
    if (flag_null_s_addr) {
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcgcomp_get\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status)));
            return status;
        }
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcsgcomp_get\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status)));
            return status;
        }
    ...
    if (flag_null_s_addr) {
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcgcomp_update for ipmc-index(0x%x)\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status), data->group));
            return status;
        }
        ...
        if (sb_ret != SB_OK) {
            ...
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcsgcomp_update for ipmc-index(0x%x)\n"),
                       unit, FUNCTION_NAME(),
                       bcm_errmsg(status), data->group));
            return status;
        }
                             ...
                             int                     remove_egress)
{
    ...
    if (ipmc_group_valid) {    
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling v6mcxxget(" IPV6_FMT ")," IPV6_FMT "),%d)\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status),
                       IPV6_PFMT(data->s_ip6_addr), 
                       IPV6_PFMT(data->mc_ip6_addr), mc_vsi));
            return status;
    ...
    if (flag_null_s_addr) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcg_remove\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
    } else {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcsg_remove\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
    }
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv6mcxx_commit\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
   ...
   BCM_IF_ERROR_RETURN
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_vlan_ft_base_get\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ft_set\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
                             ...
                             int                     remove_egress)
{
    ...
    if (ipmc_group_valid) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcxx_get(0x%x,0x%x,%d)\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status),
                       data->s_ip_addr, data->mc_ip_addr, mc_vsi));
            return status;
        }
    ...
    if (flag_null_s_addr) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcg_remove\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
    } else {
        ...
        if (BCM_FAILURE(status)) {
                        LOG_ERROR(BSL_LS_BCM_IPMC,
                                  (BSL_META_U(unit,
                                              "unit %d: [%s] error(%s) calling soc_sbx_g2p3_ipv4mcsg_remove\n"),
                                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
    }
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling ipv4mc commit\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
...
_bcm_fe2000_g2p3_ipmc_flush_cache(int unit) 
{
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling ipv4mcg commit\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
    ...
    if (BCM_FAILURE(status)) {
        LOG_ERROR(BSL_LS_BCM_IPMC,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) calling ipv4mcsg commit\n"),
                   unit, FUNCTION_NAME(), bcm_errmsg(status)));
        return status;
    }
    ...
    if (soc_sbx_g2p3_v6_ena(unit)) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling ipv6mcg commit\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_IPMC,
                      (BSL_META_U(unit,
                                  "unit %d: [%s] error(%s) calling ipv6mcsg commit\n"),
                       unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
     }
