/*
 * $Id: mpls.c,v 1.138 Broadcom SDK $
                                     ...
                                     bcm_mpls_port_t         *mpls_port)
{
    ...
    if ((l3_fe->fe_my_modid == modid || is_trunk) &&
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit, "error %d (%s) reading mpls-ete(0x%x)\n"),
                       status, bcm_errmsg(status),
                       ohi_ent.eteptr));
            return status;
        }
    ...
    if (vpn_sap->vc_res_alloced || (mpls_port->flags & BCM_MPLS_PORT_REPLACE)) {
        ...
        case BCM_MPLS_PORT_MATCH_LABEL:
            {
                ...
                if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(l3_fe->fe_unit, "failure(%s) reading l2-ete-get(0x%x)\n"),
                               bcm_errmsg(status),
                               mpls_tunnel_hw_ete.l2ete));
                    return status;
                }
        ...
        if (modid == l3_fe->fe_my_modid || is_trunk) {
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit, "error(%s) writing l2ete(0x%x)\n"),
                           bcm_errmsg(status),
                           vpn_sap->vc_l2ete_hw_idx.ete_idx));
                return status;
            }
        ...
        if (modid == l3_fe->fe_my_modid || is_trunk) {
            ...
            if (BCM_FAILURE(status)) {
                ...
                if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(l3_fe->fe_unit, "error(%s) invalidating l2ete(0x%x)\n"),
                               bcm_errmsg(status),
                               vpn_sap->vc_l2ete_hw_idx.ete_idx));
                    /* Do not return */
                }
                                         ...
                                         _fe2k_vpn_control_t     *vpnc)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_MPLS,
                (BSL_META_U(l3_fe->fe_unit, "Setting vsi=0x%x fte 0x%x oi=0x%x qid=0x%x (mc)\n"),
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit, "[%s] error(%s) writing fte(0x%x)\n"),
                                              ...
                                              soc_sbx_g2p3_etel2_t *mpls_tunnel_hw_l2ete)
{
    ...
    if (!vpn_sap->l2ete_res_shared) {
        ...
        {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit, "vc-l2-ete shared with tunnel, but DROP flag set\n")));
    ...
    if (!vpn_sap->l2ete_res_shared) {
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit, "error(%s) writing mpls-vc-l2ete(0x%x)\n"),
                       bcm_errmsg(status),
                       vpn_sap->vc_l2ete_hw_idx.ete_idx));
            return status;
        }
                                                      ...
                                                      _fe2k_vpn_sap_t    *vpn_sap)
{
    ...
    if (oh_ent.eteptr != l3_fe->fe_raw_ete_idx) {
        ...
        if (vpn_sap->vc_l2ete_hw_idx.ete_idx) {
            ...
            LOG_VERBOSE(BSL_LS_BCM_MPLS,
                        (BSL_META_U(l3_fe->fe_unit, "invalidate etel2=0x%x\n"),
                         vpn_sap->vc_l2ete_hw_idx.ete_idx));
        }
                                           ...
                                           bcm_mpls_port_t         *mpls_port)
{
    ...
    if(!SOC_SBX_CFG(l3_fe->fe_unit)->mplstp_ena) {
        ...
        if (((vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) && l3_fe->umc_ft_offset) &&
            ...
            LOG_VERBOSE(BSL_LS_BCM_MPLS,
                        (BSL_META_U(l3_fe->fe_unit, "writing UMC fte_idx=0x%04x\n"), fte_idx));
            status = soc_sbx_g2p3_ft_set(l3_fe->fe_unit, fte_idx, &ft_ent);

            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit, "error(%s) writing unknown mc fte(0x%x)\n"),
                           bcm_errmsg(status), fte_idx));
                return status;
            }
        }
                                      ...
                                      uint8  is_pw_failover)
{
    ...
    if ((vpn_sap->vc_vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) && is_pw_failover) {
        _bcm_fe2000_g2p3_mpls_vpws_pw_fo_connect(l3_fe, vpnc, vpn_sap,
                                                        mpls_port, 1);
    }
    return BCM_E_NONE;
}
...
/*
 * Function:
 *   _bcm_fe2000_g2p3_mpls_vpws_port_failover_update
 * Purpose:
...
{
    int               status;
    soc_sbx_g2p3_ft_t fte_to_update, fte_fo;
    LOG_DEBUG(BSL_LS_BCM_MPLS,
              (BSL_META_U(unit,
                          "fte(update):0x%x fte(fo):0x%x fo_id:0x%x\n"),
               fte_idx_to_update, fte_idx_fo, failover_id));
    soc_sbx_g2p3_ft_t_init(&fte_to_update);
    soc_sbx_g2p3_ft_t_init(&fte_fo);

    status = soc_sbx_g2p3_ft_get(unit, fte_idx_to_update, &fte_to_update);
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
    ...
    if (failover_id != 0) {
        status = soc_sbx_g2p3_ft_get(unit, fte_idx_fo, &fte_fo);
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(unit,
                                  "Could not read fte :0x%x \n"),
    ...
    status = soc_sbx_g2p3_ft_set(unit, fte_idx_to_update, &fte_to_update);
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
...
_bcm_fe2000_g2p3_mpls_vpws_pw_fo_connect(_fe2k_l3_fe_instance_t *l3_fe,
                                          ...
                                          uint8                 connect)
{
    _fe2k_vpn_sap_t  *vpws_uni_vpn_sap, *mpls_pw_sap;
    uint32          vpws_uni_fti, fo_fti, vpws_uni_ft_offset;
    uint32          vpws_pw_fo_ft_base, vpws_pw_fo_ft, vpls_pw_fo_ft_base;
    uint32          fo_size = 0;
    bcm_failover_t    pw_fo_id;
    bcm_gport_t       pw_fo_port_id;
    soc_sbx_g2p3_ft_t vpws_pw_fo_fte;
    ...
    if (mpls_pw_port == NULL) {
        ...
        status = _bcm_fe2000_mpls_vpws_port_sap_get(vpnc,
    ...
    if (status == BCM_E_NONE) {
        ...
        soc_sbx_g2p3_ft_t_init(&vpws_pw_fo_fte);

        COMPILER_REFERENCE(rv);
        if (connect) {
            soc_sbx_g2p3_ft_t_init(&vpws_pw_fo_fte);
            rv = soc_sbx_g2p3_ft_get(l3_fe->fe_unit, vpws_uni_fti,  &vpws_pw_fo_fte);
            rv = soc_sbx_g2p3_ft_set(l3_fe->fe_unit, vpws_pw_fo_ft, &vpws_pw_fo_fte);
            fo_size = soc_sbx_g2p3_rr_table_size_get(l3_fe->fe_unit);
            ...
            _bcm_fe2000_g2p3_mpls_vpws_fte_connect(l3_fe, vpws_pw_fo_ft, fo_fti, 1);
        } else {
            _bcm_fe2000_g2p3_mpls_vpws_fte_connect(l3_fe, vpws_pw_fo_ft, fo_fti, 0);
            rv = soc_sbx_g2p3_ft_set(l3_fe->fe_unit, vpws_pw_fo_ft, &vpws_pw_fo_fte);
        }
    } 
    return status;
...
int
_bcm_fe2000_g2p3_mpls_lp_write(int unit, uint32 logicalPort,
                               _fe2k_vpn_sap_t   *vpn_sap,
                               bcm_mpls_port_t   *mpls_port,
                               int                action)
{
    ...
    for (index = 0; index < num_ports; index++) {
        ...
        if ((SOC_SBX_CFG(l3_fe->fe_unit)->mplstp_ena) && 
            ...
            if (mpls_port->flags & BCM_MPLS_PORT_INNER_VLAN_PRESERVE) {
                soc_sbx_g2p3_p2e_t hw_p2etc;

                status = soc_sbx_g2p3_p2e_get(l3_fe->fe_unit, port, &hw_p2etc);
                if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(l3_fe->fe_unit, 
                ...
                status = soc_sbx_g2p3_p2e_set(l3_fe->fe_unit, port, &hw_p2etc);
                if (BCM_FAILURE(status)) {
                    LOG_ERROR(BSL_LS_BCM_MPLS,
                              (BSL_META_U(l3_fe->fe_unit, 
                                       ...
                                       uint32 connect)
{
    ...
    if(!SOC_SBX_CFG(l3_fe->fe_unit)->mplstp_ena) {
        for (idx=0; l3_fe->umc_ft_offset && idx < _BCM_FE2K_VPWS_MAX_SAP; idx++) {
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(l3_fe->fe_unit, "error(%s) writing port%d UMC fte(0x%x) for unknown mc\n"),
                           bcm_errmsg(rv), idx+1, fti));
                return rv;
            }
        }
                                     ...
                                     int                      ete_allocated)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit, "error %s in getting etel2[0x%x]\n"),
    ...
    if (ete_allocated == 0) {
        ...
        _FE2K_ALL_VPN_SAP_PER_MPLS_ETE(mpls_sw_ete, vpn_sap) {
            ...
            if (vpnc && (vpnc->vpn_flags & BCM_MPLS_VPN_VPWS) &&
                ...
                if (num_vpn_ports == 2 ) {
                    status = _bcm_fe2000_mpls_vpws_fte_connect(l3_fe, vpnSaps, 
                                               _FE2K_MPLS_PORT_MATCH_UPDATE, 0);
                    if (BCM_FAILURE(status)) {
                        LOG_ERROR(BSL_LS_BCM_MPLS,
                                  (BSL_META_U(unit,
           ...
           if (reconnect) {
               ...
               status = _bcm_fe2000_mpls_vpws_fte_connect(l3_fe, vpnSaps, 
                                              _FE2K_MPLS_PORT_MATCH_UPDATE, 1);
               if (BCM_FAILURE(status)) {
                   LOG_ERROR(BSL_LS_BCM_MPLS,
                             (BSL_META_U(unit,
                                           ...
                                           uint32                    count)
{
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) reading l3-fte(0x%x)\n"),
    ...
    for (idx = 0; idx < count; idx++) {
        ...
        if (status != SOC_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "unit %d: [%s] error(%s) programming mpls-fte(0x%x)\n"),
                       l3_fe->fe_unit, FUNCTION_NAME(),
                       bcm_errmsg(status), fte_idx));
            goto error_done;
        }
                                        ...
                                        bcm_if_t                  encap_id)
{
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) reading ohi2etc(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(),
                   bcm_errmsg(status),
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) reading encap-ete(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(),
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) writing encap-ete(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(),
                   bcm_errmsg(status), hw_ohi2etc.eteptr));
        return BCM_E_NONE;
    }
                                 ...
                                 bcm_mpls_tunnel_switch_t *info)
{
    ...
    if (egr_hw_fte.lag) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) getting modid for node(0x%x) port(0x%x)\n"),
                       l3_fe->fe_unit, FUNCTION_NAME(),
                                       ...
                                       uint32                    lpidx)
{
    ...
    if (SOC_SBX_CFG(unit)->mplstp_ena) {
        ...
        if (lpidx == 0) {
            ...
            status = soc_sbx_g2p3_lp_get(l3_fe->fe_unit,
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                                      "Failed to get lp 0x%x: %d %s\n"),
                           lsr_label_info.lpidx, status, bcm_errmsg(status)));
                return status;
            }
        }
        /* XXX: TBD:
        ...
        if (BCM_FAILURE(status)) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(unit,
                                  "Failed to program lp 0x%x policer 0x%x"
                                   ": %d (%s)\n"),
                       lsr_label_info.lpidx, info->policer_id,
                       status, bcm_errmsg(status)));
            return status;
        }
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) sbG2FeMplsLsrSet label(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(),
                                       ...
                                       uint32                    lpidx)
{
        ...
        if (lpidx == 0) {
            ...
            if (BCM_FAILURE(status)) {
                LOG_ERROR(BSL_LS_BCM_MPLS,
                          (BSL_META_U(unit,
                                      "Failed to get lp 0x%x: %d %s\n"),
                           ler_info.lpidx, status, bcm_errmsg(status)));
                return status;
            }
        }
    ...
    if (status != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(unit,
                              "unit %d: [%s] error(%s) soc_sbx_g2p3_l2e_set label(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(),
                   bcm_errmsg(status), (int)info->label));
    }
                                           ...
                                           bcm_mpls_tunnel_switch_t *info)
{
    ...
    soc_sbx_g2p3_l2e_t_init(&label2etc);
                                              ...
                                              _fe2k_l3_ete_t          *mpls_ete)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) reading mpls-ete(0x%x)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME(), bcm_errmsg(status),
                   mpls_ete->l3_ete_hw_idx.ete_idx));
        return status;
    }
    ...
    if (action_flags & _FE2K_MPLS_ETE_MODIFY__UCAST_IP_ETE_CHANGED) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "unit %d: [%s] error %s in setting MPLS ete in HW"),
                       l3_fe->fe_unit, FUNCTION_NAME(), bcm_errmsg(status)));
            return status;
        }
    ...
    if (action_flags & _FE2K_MPLS_ETE_MODIFY__L2_ETE_CHANGED) {
        ...
        if (status != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_MPLS,
                      (BSL_META_U(l3_fe->fe_unit,
                                  "unit %d: [%s] error(%s) writing mpls-L2-ete(0x%x)\n"),
                       l3_fe->fe_unit, FUNCTION_NAME(), bcm_errmsg(status),
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit, "error %s in getting etel2[0x%x]\n"),
                   bcm_errmsg(status), mpls_hw_ete.l2ete));
        return status;
    }
    ...
    } _FE2K_ALL_VPN_SAP_PER_MPLS_ETE_END(mpls_ete, vpn_sap);
                                    ...
                                    _fe2k_l3_ete_t         *v4_ete)
{
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] mpls tunnel ete not found on interface\n"),
                   l3_fe->fe_unit, FUNCTION_NAME()));
        return status;
    }
    LOG_VERBOSE(BSL_LS_BCM_MPLS,
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] tunnel ete not be enabled (could not set DMAC)\n"),
                   l3_fe->fe_unit, FUNCTION_NAME()));
        return status;
    }
    ...
    if (status != BCM_E_NONE) {
        LOG_ERROR(BSL_LS_BCM_MPLS,
                  (BSL_META_U(l3_fe->fe_unit,
                              "unit %d: [%s] error(%s) in mapping ohi (0x%x) "
                                ...
                                uint32 egrFlags)
{
    ...
    _fe2k_vpn_sap_t *vpn_sap = NULL;
    _fe2k_vpn_sap_t *vpn_sap_temp = NULL;
    ...
    _FE2K_ALL_VPNC(l3_fe, vpnc) {
        _FE2K_ALL_VPN_SAP_PER_VPNC(vpnc, vpn_sap_temp) {
            if (gport == vpn_sap_temp->vc_mpls_port_id) {
                vpn_sap = vpn_sap_temp;
                found = TRUE;
                break;
            }
        } _FE2K_ALL_VPN_SAP_PER_VPNC_END(vpnc, vpn_sap_temp);
        if (found) {
            break;
        }
