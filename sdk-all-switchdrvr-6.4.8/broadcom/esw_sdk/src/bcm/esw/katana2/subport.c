 ...
 * 
 */
shr_idxres_list_handle_t _bcm_sb2_int_stream_id_pool[BCM_MAX_NUM_UNITS][_BCM_SB2_MAX_LINKPHY_PORTS];
#endif
...
#define _BCM_SB2_LINKPHY_BLOCK_MAX     1 /* Only one RXLP/TXLP block */
...
#if defined(BCM_SABER2_SUPPORT)
                if (SOC_IS_SABER2(unit)) {
                    BCM_IF_ERROR_RETURN(_bcm_linkphy_port_rx_int_stream_map_port_get(unit, port, 
                                int_stream_id_1, &int_stream_id_1));
                    BCM_IF_ERROR_RETURN(_bcm_linkphy_port_rx_int_stream_map_port_get(unit, port, 
                                int_stream_id_2, &int_stream_id_2));
                    /* Index for _bcm_sb2_int_stream_id_pool runs from 0 to 3, whereas port runs from
                     * 1 to 4. Hence doing port -1 to index the pool 
                     */
                    /* Reserve those internal streams in that port's pool. */
                    BCM_IF_ERROR_RETURN(shr_idxres_list_reserve(_bcm_sb2_int_stream_id_pool[unit][port-1],
                                                                int_stream_id_1, int_stream_id_1));
                    BCM_IF_ERROR_RETURN(shr_idxres_list_reserve(_bcm_sb2_int_stream_id_pool[unit][port-1],
                                                                int_stream_id_2, int_stream_id_2));
                }
                                         ...
                                         int enable)
{
    ...
    uint32 max_start_addr = _BCM_KT2_LINKPHY_TX_DATA_BUF_START_ADDR_MAX;
    uint32 min_end_addr   = _BCM_KT2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
    uint32 max_end_addr   = _BCM_KT2_LINKPHY_TX_DATA_BUF_END_ADDR_MAX;

    mem = TXLP_STREAM_ADDR_MAP_TABLEm;

#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        max_start_addr = _BCM_SB2_LINKPHY_TX_DATA_BUF_START_ADDR_MAX;
        min_end_addr   = _BCM_SB2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
        max_end_addr   = _BCM_SB2_LINKPHY_TX_DATA_BUF_END_ADDR_MAX;
    }
#endif
                                             ...
                                             uint16 *int_stream_id)
{
    ...
    /* Ideally this API can be extended to Katana2 as well */
    if (!SOC_IS_SABER2(unit)) {
        return BCM_E_NONE;
    }
...
_bcm_linkphy_subport_stat_callback(int unit)
{
    ...
    int linkphy_block_max   = _BCM_KT2_LINKPHY_BLOCK_MAX;

#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        linkphy_block_max = _BCM_SB2_LINKPHY_BLOCK_MAX;
    }
#endif
...
#endif
        {
            counter_id += SOC_INFO(unit).port_linkphy_s1_base[port];
            counter_id = (block) ? counter_id - 128 : counter_id;
        }
...
_bcm_linkphy_subport_init(int unit)
{
   ...
   */
    ...
    if (SOC_IS_SABER2(unit)) {
        ...
        for (index = 0; index < _BCM_SB2_MAX_LINKPHY_PORTS; index++) {
                              ...
                              bcm_gport_t *gport)
{
    ...
    uint16 int_stream_id[BCM_SUBPORT_CONFIG_MAX_STREAMS] = {0, 0};
    ...
    for (i = 0; i < config->num_streams; i++) {
        ...
        if (SOC_IS_SABER2(unit)) {
            if (config->stream_id_array[i] >=
                    (ext_stream_id_base +
                    (SB2_MAX_STREAMS_PER_SLICE * (lp_block + 1)))) {
                LOG_ERROR(BSL_LS_BCM_SUBPORT,
                        (BSL_META_U(unit,
        ...
        */
        ...
        dev_int_stream_id[i] = ext_stream_id[i] + dev_int_stream_id_base;
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
            ...
            rv = shr_idxres_list_alloc(_bcm_sb2_int_stream_id_pool[unit][port -1],
                                       (shr_idxres_element_t *)(&(dev_int_stream_id[i])));
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_SUBPORT,
                        (BSL_META_U(unit,
                                    "*ERROR: Internal stream id alloc failed for stream_id %d - %s\n"),
                         config->stream_id_array[i], bcm_errmsg(rv)));
            }
        }
#endif
        int_stream_id[i] = (lp_block) ? dev_int_stream_id[i] - 128 : dev_int_stream_id[i];
        ...
        if (dev_int_stream_id[i] > soc_mem_index_max(unit,
            ...
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                       ...
                       dev_int_stream_id[i], port, dev_int_stream_id_base, int_stream_id[i]));
    ...
    *********************************/
    for (i = 0; i < config->num_streams; i++) {
        ...
        */
        ...
        start_addr =  _BCM_KT2_LINKPHY_TX_STREAM_START_ADDR_OFFSET + 
            (_BCM_KT2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN + 1) *
            int_stream_id[i];
        end_addr = start_addr + _BCM_KT2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
            start_addr =  _BCM_SB2_LINKPHY_TX_STREAM_START_ADDR_OFFSET + 
                (_BCM_SB2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN + 1) *
                int_stream_id[i];
            end_addr = start_addr + _BCM_SB2_LINKPHY_TX_DATA_BUF_END_ADDR_MIN;
        }
#endif
 
        rv = _bcm_linkphy_port_tx_stream_addr_map_set(unit,
                port, int_stream_id[i], start_addr, end_addr, enable);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                                  "ERROR: TX data buffer address set failed.\n"
                                   "port %d, internal stream_id = %d, start_addr = %d "
                                   "end_addr = %d.\n"), port, int_stream_id[i], start_addr, end_addr));
        ...
        rv = _bcm_linkphy_port_rx_int_stream_map_port_set(unit,
                port, ext_stream_id[i], int_stream_id[i], enable);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                                 ...
                                 bcm_gport_t subport_port_gport)
{
    ...
    */
    ...
    for (i = 0; i < num_streams; i++) {
        ...
        int_stream_id[i] = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_EXT_STREAM_ID_GET(
        ...
        if (SOC_IS_SABER2(unit)) {
            BCM_IF_ERROR_RETURN(_bcm_linkphy_port_rx_int_stream_map_port_get(unit, port, 
                        int_stream_id[i], &(int_stream_id[i])));
        }
#endif
        dev_int_stream_id[i] = int_stream_id[i] + dev_int_stream_id_base;
        mem = DEVICE_STREAM_ID_TO_PP_PORT_MAPm;
        if (dev_int_stream_id[i] > soc_mem_index_max(unit,mem)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                       ...
                       dev_int_stream_id_base, int_stream_id[i], pp_port_index));
            return BCM_E_PARAM;
        }
        ...
        if (SOC_REG_PORT_VALID(unit, reg, port)) {
            ...
            if (!(SAL_BOOT_SIMULATION || SOC_WARM_BOOT(unit))) {
                BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, reg,
                            port, FLUSH_STREAM_IDf, int_stream_id[i]));
                BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, reg,
                    ...
                    if (soc_timeout_check(&timeout)) {
                        LOG_ERROR(BSL_LS_BCM_SUBPORT,
                                  (BSL_META_U(unit,
                                   ...
                                   int_stream_id[i], subport_port_gport,
                                   port, pp_port_index));
        ...
        rv = _bcm_linkphy_port_tx_int_to_ext_stream_map_set(unit, 
                port, 0, int_stream_id[i], disable);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                                  "ERROR: TX internal to external stream mapp clear failed.\n"
                                  "port %d, internal stream_id = %d, external stream id = %d.\n"),
                       port, int_stream_id[i], 0));
            return rv;
        }

        rv = _bcm_linkphy_port_tx_stream_addr_map_set(unit,
                        port, int_stream_id[i], 0, 0, disable);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_SUBPORT,
                      (BSL_META_U(unit,
                                  "ERROR: TX data buffer address clear failed.\n"
                                   "port %d, internal stream_id = %d, start_addr = %d "
                                   "end_addr = %d.\n"), port, int_stream_id[i], 0, 0));
            return rv;
        }
        ...
        BCM_IF_ERROR_RETURN(
            soc_mem_field32_modify(unit, mem, dev_int_stream_id[i], INT_PRIf, 0));
        BCM_IF_ERROR_RETURN(
                              ...
                              _bcm_kt2_subport_info_t *subport_info)
{
    ...
    if (subport_info->port_type == _BCM_KT2_SUBPORT_TYPE_SUBTAG) {
...
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
            rv = soc_linkphy_port_blk_idx_get(unit, subport_info->port, &lp_block, &lp_index);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_SUBPORT,
                        (BSL_META_U(unit,
                                    "ERROR: LinkPHY block, index get failed for port %d\n"),
        ...
        for (i = 0; i < subport_port_config.num_streams; i++) {
            ...
            if (SOC_IS_SABER2(unit)) {
                BCM_IF_ERROR_RETURN(_bcm_linkphy_port_rx_int_stream_map_port_get(unit, subport_info->port,
                            int_stream_id, &int_stream_id));
            }
#endif
                             ...
                             bcm_subport_group_linkphy_config_t *linkphy_config)
{
    ...
    soc_info_t *si = &SOC_INFO(unit);
    int max_streams = _BCM_KT2_LINKPHY_PER_PORT_STREAMS_MAX;
...
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        max_streams = SB2_MAX_STREAMS_PER_SLICE;
    }
#endif 
...
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit)) {
            end = start + (SB2_MAX_LINKPHY_STREAMS_PER_PORT - 1);
        }
#endif
        SOC_IF_ERROR_RETURN
