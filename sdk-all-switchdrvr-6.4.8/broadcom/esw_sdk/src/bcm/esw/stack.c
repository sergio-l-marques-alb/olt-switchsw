/*
 * $Id: stack.c,v 1.354 Broadcom SDK $
...
typedef struct module_id_mask_s {
    int mod_base;    /* module base for extra modules */
...
bcm_esw_stk_my_modid_set(int unit, int my_modid)
{
	...
	PBMP_ST_ITER(unit, port) {
        ...
        if (cpu_hg_index != -1) {
            BCM_IF_ERROR_RETURN
                (soc_mem_read(unit, PORT_TABm, MEM_BLOCK_ANY, cpu_hg_index, &pte));
            if (soc_mem_field_valid(unit, PORT_TABm, MY_MODIDf)) {
                soc_PORT_TABm_field32_set(unit, &pte, MY_MODIDf, my_modid);
            }
            BCM_IF_ERROR_RETURN
                (soc_mem_write(unit, PORT_TABm, MEM_BLOCK_ALL, cpu_hg_index, &pte));
        }
...
bcmi_esw_src_modid_base_index_bk_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2PLUS(unit)) {
        _BCM_SRC_MOD_PORT_TABLE_USED_SET_RANGE(unit, 0, 106);
    } else {
        _BCM_SRC_MOD_PORT_TABLE_USED_SET_RANGE(unit, 0,
        ...
        for (i = 0; i < COUNTOF(src_modbase_mems); i++) {
            if(soc_mem_is_valid(unit, src_modbase_mems[i])) {
                ...
                if (src_modbase_mems[i] == MODID_BASE_PTRm) {
                    mod_idx = modid % 64;  /* lower 6 bit */
                    SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                        MODID_BASE_PTRm, mod_idx, MODID_BASEf, 0));
                } else 
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
                {
                    SOC_IF_ERROR_RETURN(soc_mem_field32_modify(unit, 
                        src_modbase_mems[i], modid, BASEf, 0));
                }
            }
        }
    }
                                                  ...
                                                  int mod_id, int valid, int base_idx)
{
    ...
    if(!(flags & BCM_TD2PLUS_MODID_TYPE_COE)) {
        return BCM_E_INTERNAL;
    }
...
{
    int modid = 0, base_index = 0, i = 0, rv;
    ...
    BCM_IF_ERROR_RETURN
        (_src_modid_base_index_alloc(unit, modid, n_ports,
                                         &base_index));
    ...
    rv = _bcm_esw_stk_update_mod_config_by_type(unit, BCM_TD2PLUS_MODID_TYPE_COE, 
                                                i+1, modid, 1, base_index);

    if (BCM_FAILURE(rv)) {
        BCM_IF_ERROR_RETURN
            (_src_modid_base_index_free(unit, modid, 
...
int _bcm_esw_stk_modid_range_config_add(int unit, bcm_stk_modid_config_t *mod_cfg)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_stk_multi_nexthops_module_mask_get(mod_cfg->num_mods, &mod_mask));
    ...
    for (mod_idx = modid; mod_idx < modid + mod_cfg->num_mods; mod_idx++) {
        rv = _src_modid_base_index_alloc(unit, mod_idx, n_ports, &base_index);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
...
int _bcm_esw_stk_modid_range_config_get(int unit, bcm_stk_modid_config_t *mod_cfg)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_esw_stk_modid_range_info_get(unit, &mod_cfg->modid, &mod_mask));
    if(!mod_mask) {
        mod_cfg->modid = BCM_MODID_INVALID;
    }

    BCM_IF_ERROR_RETURN
        (_bcm_esw_stk_multi_nexthops_modules_num_get(mod_mask, &mod_cfg->num_mods));

    if(mod_cfg->modid >= 0) {
        mod_cfg->num_ports = src_modid_base_index_bk[unit]->num_ports[mod_cfg->modid];
    }
        ...
        bcm_stk_modid_type_t modid_type)
{
    ...
    /* Parameter check */
    if (modid_type != bcmStkModidTypeMultiNextHops) {
        return BCM_E_PARAM;
    }
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_stk_multi_nexthops_modules_num_get(mod_mask, &num_mods));
 ...
 */
int 
_bcm_esw_stk_modid_config_delete(int unit, int modid)
{
    ...
    /* Program the relevant registers */
    _bcm_esw_stk_update_mod_config_by_type(unit, BCM_TD2PLUS_MODID_TYPE_COE, 
                                           i+1, modid, 0, 0);
                                ...
                                bcm_stk_modid_config_t *mod_cfg)
{
    ...
    case bcmStkModidTypeMultiNextHops:
        if (soc_feature(unit, soc_feature_multi_next_hops_on_port)) {
            BCM_IF_ERROR_RETURN
                (_bcm_esw_stk_modid_range_config_delete(unit, 
                     mod_cfg->modid_type));
        } else {
            return BCM_E_UNAVAIL;
        }
                              ...
                              bcm_stk_modid_config_t *modIdInfo)
{
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe)) {
        ...
        /* Make sure the caller has provided a valid type of module */
        if(!(type & BCM_TD2PLUS_MODID_TYPE_COE)) {
            return BCM_E_INTERNAL;
        }
