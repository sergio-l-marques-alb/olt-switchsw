...
/*****************************************************************************/
    {
        ...
        if (SOC_IS_TRIUMPH3(unit) && !SOC_IS_TRIDENT2PLUS(unit)) {
    ...
    if ( clk_src == bcmTimeSynceClockSourcePrimary ) {
        ...
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, EGR_L1_CLK_RECOVERY_CTRLr,
                                    REG_PORT_ANY, PRI_PORT_SELf, phy_port));
        /* Turn off TSC div11 */
        BCM_IF_ERROR_RETURN(
        ...
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, EGR_L1_CLK_RECOVERY_CTRLr,
                                    REG_PORT_ANY, BKUP_PORT_SELf, phy_port));
        /* Turn off TSC div11 */
        BCM_IF_ERROR_RETURN(
            _bcm_time_synce_tsc_divisor_turn_off(unit, lport));
        /* TOP_MISC_CONTROL */
                              ...
                              bcm_time_synce_divisor_setting_t *divisor)
{
    ...
    case bcmTimeSynceClockSourceSecondary:
        ...
        if (val == 0) { /* source is Port */
            ...
            SOC_IF_ERROR_RETURN(
                 READ_EGR_L1_CLK_RECOVERY_CTRLr(unit, &rval));
            phy_port = soc_reg_field_get(unit, EGR_L1_CLK_RECOVERY_CTRLr, rval,
                                         BKUP_PORT_SELf);
            divisor->index = SOC_INFO(unit).port_p2l_mapping[phy_port];
        } else { /* source is PLL */
            divisor->index = val - 1;
