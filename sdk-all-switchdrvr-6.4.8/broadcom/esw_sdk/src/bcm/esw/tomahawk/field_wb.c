            ...
            switch (tlv2.type) {
                ...
                case _bcmFieldInternalStatArr:
                    _FP_XGS3_ALLOC(f_st->stat_arr,
                                  (tlv2.length * (static_type_map[tlv2.type].size)),
                                   "Field stat array");
                                   ...
                                   uint32* e_buf, _field_action_t *act)
{
    ...
    switch (act ->action) {
           ...
           case bcmFieldActionUntaggedPacketPrioirtyNew:
...
_field_group_recover(int unit, _field_group_t *fg)
{
    ...
    while (tlv.type != _bcmFieldInternalEndStructGroup) {
        ...
        switch (tlv.type)
        {
            ...
            case _bcmFieldInternalQsetUdf:
                 ...
                 for (idx = 0; idx < ((static_type_map[tlv.type].size)
                      * tlv.length); idx++) {
    ...
    /* Get QSET. */
    sal_memcpy(qset_copy.w, fg->qset.w, SHR_BITALLOCSIZE(BCM_FIELD_QUALIFY_MAX));

    /* Update Qualifier Ids for qualifiers after bcmFieldqualifyCount */

    if (qual_count != -1) {
        for (idx = qual_count; idx < _bcmFieldQualifyCount; idx ++) {
            if (BCM_FIELD_QSET_TEST(qset_copy, idx)){
                 SHR_BITCLR(fg->qset.w, idx);
                 SHR_BITSET(fg->qset.w, ( (idx - qual_count)
                            + bcmFieldQualifyCount));
            }
        }
    }
...
_field_wb_em_emredirect_set_recover(int unit, _field_entry_t *f_ent,  uint32 *ebuf)
{
   _bcm_field_action_offset_t a_offset;
    ...
    BCM_IF_ERROR_CLEANUP(_field_stage_control_get(unit,_BCM_FIELD_STAGE_EXACTMATCH, &stage_fc));
