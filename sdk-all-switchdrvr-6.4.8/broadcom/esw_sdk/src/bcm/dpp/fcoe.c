...
STATIC uint32 _bcm_petra_foce_route_info_to_ppd(
    ...
    /* FEC + MAC --> forward decision = FEC + ARP (EEI)*/
        ...
        rv = _bcm_dpp_gport_fix_fec_type(unit, BCM_L3_ITF_VAL_GET(route->intf), SOC_PPD_FEC_TYPE_IP_UC);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
STATIC uint32 _bcm_petra_foce_route_info_from_ppd(
    ...
    if (frwrd_type == 0 || frwrd_type == 2) {
        ...
        rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&route->port,NULL,&(route_info->frwrd_decision),-1/*encap_usage*/);
        BCMDNX_IF_ERR_EXIT(rv);
    }
   ...
   )
{
    ...
    rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&zone->dest_port,NULL,&(zone_info->frwrd_decision),-1/*encap_usage*/);
    BCMDNX_IF_ERR_EXIT(rv);
