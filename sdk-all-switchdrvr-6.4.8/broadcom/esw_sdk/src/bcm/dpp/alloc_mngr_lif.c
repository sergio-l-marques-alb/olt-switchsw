...
STATIC uint32
_bcm_dpp_am_global_lif_bank_get(int unit, uint32 sync_flags, int *bank);
...
typedef enum bcm_dpp_am_global_lif_bank_bit_e {
    bcm_dpp_am_global_lif_bank_bit_ingress_allocated,  /* An ingress entry has been allocated in this bank. */
    bcm_dpp_am_global_lif_bank_bit_egress_allocated,   /* An egress entry has been allocated in this bank. */
    bcm_dpp_am_global_lif_bank_bit_sync,               /* A sync entry has been allocated in this bank, and every entry in it is 
                                                           allocated (or deallocated) in both ingress and egress. */
    bcm_dpp_am_global_lif_bank_bit_count
} bcm_dpp_am_global_lif_bank_bit_t;

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_INGRESS_ALLOCATED(_bank_bmp) \
            (DPP_IS_FLAG_SET(_bank_bmp, (1 << bcm_dpp_am_global_lif_bank_bit_ingress_allocated)))

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_EGRESS_ALLOCATED(_bank_bmp)  \
            (DPP_IS_FLAG_SET(_bank_bmp, (1 << bcm_dpp_am_global_lif_bank_bit_egress_allocated)))

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_ALLOCATED(_bank_bmp) \
            (_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_INGRESS_ALLOCATED(_bank_bmp) || _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_EGRESS_ALLOCATED(_bank_bmp))

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_SYNC(_bank_bmp) \
            (DPP_IS_FLAG_SET(_bank_bmp, (1 << bcm_dpp_am_global_lif_bank_bit_sync)))


#define _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_INGRESS_ALLOCATED(_bank_bmp) \
            (_bank_bmp) |= (1 << bcm_dpp_am_global_lif_bank_bit_ingress_allocated)

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_EGRESS_ALLOCATED(_bank_bmp)  \
            (_bank_bmp) |= (1 << bcm_dpp_am_global_lif_bank_bit_egress_allocated)

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_SYNC(_bank_bmp) \
            (_bank_bmp)  |= (1 << bcm_dpp_am_global_lif_bank_bit_sync)

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_INGRESS_ALLOCATED(_bank_bmp) \
            (_bank_bmp) &= ~(1 << bcm_dpp_am_global_lif_bank_bit_ingress_allocated)

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_EGRESS_ALLOCATED(_bank_bmp)  \
            (_bank_bmp) &= ~(1 << bcm_dpp_am_global_lif_bank_bit_egress_allocated)

#define _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_SYNC(_bank_bmp) \
            (_bank_bmp)  &= ~(1 << bcm_dpp_am_global_lif_bank_bit_sync)
...
STATIC uint32
_bcm_dpp_am_egress_encap_internal_dealloc(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, uint8 is_sync, int count, int object, uint8 *is_last);
/* Egress encapsulation functions } */
...
STATIC uint32
_bcm_dpp_am_ingress_lif_internal_dealloc(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, uint8 is_sync, int count, int object, uint8 *is_last);
/* Ingress LIF functions } */
...
_bcm_dpp_am_ingress_lif_dealloc(int unit, bcm_dpp_am_ingress_lif_alloc_info_t* allocation_info, int count, int object)
{
    ...
    /* Deallocate object */
    rv = _bcm_dpp_am_ingress_lif_internal_dealloc(unit,allocation_info, FALSE, count, object, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_egress_encap_dealloc(int unit, bcm_dpp_am_egress_encap_alloc_info_t* allocation_info, int count, int object)
{
    ...
    /* Allocate object */
    rv = _bcm_dpp_am_egress_encap_internal_dealloc(unit,allocation_info,FALSE,count,object, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_sync_lif_dealloc(int unit, bcm_dpp_am_sync_lif_alloc_info_t* allocation_info, int count, int object)
{
    ...
    if (is_ingress) {
        rv = _bcm_dpp_am_ingress_lif_internal_dealloc(unit, &(allocation_info->ingress_lif), TRUE, count, object, NULL);
    ...
    if (is_egress) {
        rv = _bcm_dpp_am_egress_encap_internal_dealloc(unit, &(allocation_info->egress_lif), TRUE, count, object, NULL);
...
_bcm_dpp_am_global_lif_init(int unit){
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        /* Allocate bank phase bitmap. */
        count = bcm_dpp_am_global_lif_bank_bit_count * _BCM_DPP_AM_NOF_GLOBAL_LIF_BANKS(unit);
        rv = sw_state_access[unit].dpp.bcm.alloc_mngr_glif.global_lif_bank_status.alloc_bitmap(unit, count);
        BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_internal_alloc(int unit, uint32 alloc_flags, uint32 sync_flags, int *global_lif){
    ...
    if (alloc_flags & (SW_STATE_RES_ALLOC_WITH_ID)) {
        int object_id;
...
_bcm_dpp_am_global_lif_internal_dealloc(int unit, uint32 sync_flags, int global_lif_id){
    ...
    if (sync_flags & _BCM_DPP_AM_SYNC_LIF_EGRESS) {
        ...
        clear_bank = clear_bank || (flags & (SW_STATE_RES_FREED_TYPE_LAST_ELEM | SW_STATE_RES_FREED_POOL_LAST_ELEM));
    }
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_status_get
 * Purpose:
 *     Given a bank id, returns the bank status info bmp, encoded according to bcm_dpp_am_global_lif_bank_bit_t
 *  
 * Parameters:
 *     unit         - (IN) Device number
 *     bank_id      - (IN) Bank id to be checked.
 *     bank_type    - (OUT)Bmp of the banks status.
 * Returns: 
 *     BCM_E_NONE       - If the check was succesful.
 *     BCM_E_*          - Otherwise.
 */
STATIC uint32 
_bcm_dpp_am_global_lif_bank_status_get(int unit, int bank_id, uint32 *bank_type){
    uint32 rv;
    BCMDNX_INIT_FUNC_DEFS;

    /* Get bank type from ingress. We could have asked for egress as well since it's symmetric. */
    rv = sw_state_access[unit].dpp.bcm.alloc_mngr_glif.global_lif_bank_status.bit_range_read(unit,
        (bank_id*bcm_dpp_am_global_lif_bank_bit_count),
        0,
        bcm_dpp_am_global_lif_bank_bit_count,
        bank_type);
    BCMDNX_IF_ERR_EXIT(rv);
 ...
 *     _bcm_dpp_am_global_lif_bank_status_get
 * Purpose:
 *     Given a bank id and bank status encoded according to bcm_dpp_am_global_lif_bank_bit_t, sets the bank status info bmp.
 *  
 * Parameters:
 *     unit         - (IN) Device number
 *     bank_id      - (IN) Bank id to be checked.
 *     bank_type    - (IN) Bmp of the banks status.
 * Returns: 
 *     BCM_E_NONE       - If the check was succesful.
 *     BCM_E_*          - Otherwise.
 */
STATIC uint32 
_bcm_dpp_am_global_lif_bank_status_set(int unit, int bank_id, uint32 bank_type){
    uint32 rv;
    BCMDNX_INIT_FUNC_DEFS;

    /* Get bank type from ingress. We could have asked for egress as well since it's symmetric. */
    rv = sw_state_access[unit].dpp.bcm.alloc_mngr_glif.global_lif_bank_status.bit_range_write(unit,
        (bank_id*bcm_dpp_am_global_lif_bank_bit_count),
        0,
        bcm_dpp_am_global_lif_bank_bit_count,
        &bank_type);
...
_bcm_dpp_am_global_lif_bank_is_sync_bank(int unit, int bank_id, uint8 *is_sync){
    ...
    /* Get bank type from ingress. We could have asked for egress as well since it's symmetric. */
    rv = _bcm_dpp_am_global_lif_bank_status_get(unit, bank_id, &bank_status);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Sync banks use the "sync" bank type. */
    *is_sync = _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_SYNC(bank_status);
...
_bcm_dpp_am_global_lif_is_legal_bank_by_type(int unit, int bank_id, uint8 is_ingress, uint8 is_sync, uint32 bank_alloc_flags, uint8 *is_legal) {
    ...
    if (_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_SYNC(bank_status)) {
        /* Only sync entries or with_id entries can be allocated in sync banks. */
        *is_legal = is_sync || with_id;
    } else if ((is_ingress && _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_INGRESS_ALLOCATED(bank_status))
               || (!is_ingress && _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_EGRESS_ALLOCATED(bank_status))) {
        /* Only non sync entries can be allocated in non sync banks. */
        *is_legal = !is_sync;            
    } else {
        /* Banks is not allocated in this direction. */
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_is_free
 * Purpose:
 *      Check whether the given bank has free entries and can be used for allocation.
 *  
 * Parameters:
 *     unit             - (IN) Device number
 *     sync_flags       - (IN) Sync flags for the entry. _BCM_DPP_AM_SYNC_LIF_INGRESS, _BCM_DPP_AM_SYNC_LIF_EGRESS or both for sync entry. 
 *     bank_id          - (IN) Bank id to be checked.
 *     is_free          - (OUT)Whether the bank has free entries and can be used for allocation.
 * Returns: 
 *     BCM_E_NONE       - If the check was succesful.
 *     BCM_E_*          - Otherwise.
 */
STATIC uint32
_bcm_dpp_am_global_lif_bank_is_free(int unit, uint32 sync_flags, int bank_id, uint8 *is_free){
    uint32 rv;
    uint8 is_ingress, is_egress;
    int used_count;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    /* Check the ref count of the bank. For sync banks, the ref count should be the same in both ingress and egress. */
    rv = dpp_am_res_used_count_get(unit, bank_id, (is_ingress) ? dpp_am_res_global_inlif : dpp_am_res_global_outlif, &used_count);
    BCMDNX_IF_ERR_EXIT(rv);

    *is_free = (used_count < _BCM_DPP_AM_GLOBAL_LIF_NOF_ENTRIES_PER_BANK);
...
_bcm_dpp_am_global_lif_update_bank_and_type(int unit, uint32 sync_flags, int bank_id){
    ...
    /* Only change sync status if the bank has not already been allocated on either side. */
    if (!_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_ALLOCATED(bank_status) && is_ingress && is_egress) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_SYNC(bank_status);
    }

    /* Update ingress status. */
    if (is_ingress) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_INGRESS_ALLOCATED(bank_status);
    ...
    if (is_egress) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_SET_EGRESS_ALLOCATED(bank_status);
    ...
    rv = _bcm_dpp_am_global_lif_bank_status_set(unit, bank_id, bank_status);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_dpp_am_global_lif_clear_bank_and_type(int unit, uint32 sync_flags, int bank_id) {
    ...
    /* Update ingress status. */
    if (is_ingress) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_INGRESS_ALLOCATED(bank_status);
    ...
    /* Update egress status. */
    if (is_egress) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_EGRESS_ALLOCATED(bank_status);
        LOG_VERBOSE(BSL_LS_BCM_COMMON, (BSL_META_U(unit, "unit %d, Egress global lif bank #%d has been deallocated.\n"), 
                                     unit, bank_id));
    }

    /* Only change sync status if the bank has been deallocated on both sides. */
    if (!_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_ALLOCATED(bank_status)) {
        _BCM_DPP_AM_GLOBAL_LIF_BANK_CLEAR_SYNC(bank_status);
    }
...
_bcm_dpp_am_global_lif_allocate_new_bank(int unit, uint32 sync_flags, int *bank_id, uint8 *is_success) {
    ...
    uint8 is_ingress_free, is_egress_free;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    *is_success = FALSE;
    ...
    /* Set both sides to free */
    is_ingress_free = TRUE;
    is_egress_free = TRUE;
    ...
    for (bank_id_iter = _BCM_DPP_AM_NOF_GLOBAL_LIF_BANKS(unit) - 1 ; bank_id_iter >= 0 ; bank_id_iter--) {
        rv = _bcm_dpp_am_global_lif_bank_status_get(unit, bank_id_iter, &bank_status);
        BCMDNX_IF_ERR_EXIT(rv);

        if (is_ingress) {
            is_ingress_free = !_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_INGRESS_ALLOCATED(bank_status);
        }

        if (is_egress) {
            is_egress_free = !_BCM_DPP_AM_GLOBAL_LIF_BANK_IS_EGRESS_ALLOCATED(bank_status);
        }

        /* Bank is free, see if it can be allocated. */
        if (is_ingress_free && is_egress_free) {
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_get
 * Purpose:
 *      To be called when trying to allocate an entry without the WITH_ID flag.
 *      Will return an allocated bank with free entries matching the entry's type.
 *  
 * Parameters:
 *     unit             - (IN) Device number
 *     sync_flags       - (IN) Sync flags of the allocated entry. _BCM_DPP_AM_SYNC_LIF_INGRESS, _BCM_DPP_AM_SYNC_LIF_EGRESS or both for sync entry. 
 *     banks_id         - (OUT)A free bank matching the entry. Will be -1 if no banks found.
 * Returns: 
 *     BCM_E_NONE       - If the check was succesful.
 *     BCM_E_*          - Otherwise.
 */
...
_bcm_dpp_am_global_lif_bank_get(int unit, uint32 sync_flags, int *bank) {
    uint32 rv;
    int bank_id_iter;
    uint8 is_ingress, is_egress, is_legal, is_free = 0;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    *bank = -1; /* Set to an illegal value, in case no bank is legal. */
    ...
    for (bank_id_iter = _BCM_DPP_AM_NOF_GLOBAL_LIF_BANKS(unit) - 1 ; bank_id_iter >= 0 ; bank_id_iter--) {
        ...
        /* If the bank fits, check if it has free entries. */
        if (is_legal) {
            rv = _bcm_dpp_am_global_lif_bank_is_free(unit, sync_flags, bank_id_iter, &is_free);
            BCMDNX_IF_ERR_EXIT(rv);
            if (is_free) {
                *bank = bank_id_iter;
                break;
 ...
 * Parameters:
 *     unit             - (IN) Device number
 *     alloc_flags      - (IN) SW_STATE_RES_* flags.
 *     sync_flags       - (IN) Sync flags of the allocated entry. _BCM_DPP_AM_SYNC_LIF_INGRESS, _BCM_DPP_AM_SYNC_LIF_EGRESS or both for sync entry. 
...
_bcm_dpp_am_global_lif_internal_alloc_by_type(int unit, uint32 alloc_flags, uint32 sync_flags, int *global_lif, uint8 *alloc_success) {
    ...
    *alloc_success = FALSE;
    ...
    rv = _bcm_dpp_am_global_lif_bank_get(unit, sync_flags, &bank);
    BCMDNX_IF_ERR_EXIT(rv);

    if (bank < 0) {
        /* No free allocated banks. Allocation failed. */
        BCM_EXIT;
    }

    rv = _bcm_dpp_am_global_lif_res_alloc(unit, alloc_flags, sync_flags, bank, global_lif);
    BCMDNX_IF_ERR_EXIT(rv);
