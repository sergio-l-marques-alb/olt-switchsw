...
#define _BCM_BFD_ENDPOINT_INFO_BFD_PERIOD_TO_MEP_DB_TX_RATE(bfd_period) \
    (SOC_SAND_DIV_ROUND(bfd_period * 100, SOC_IS_ARADPLUS_AND_BELOW(unit) ? 333 : 167) -1) 
...
int _bcm_bfd_rmep_db_entry_struct_get(int unit, SOC_PPD_OAM_OAMP_RMEP_DB_ENTRY *rmep_db_entry, bcm_bfd_endpoint_info_t *endpoint_info,
    ...
    if (classifier_mep_entry->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_TIMEOUT_SET_EXPLICITLY) {
 ...
 * @param mep_db_entry 
 */
void _bcm_bfd_enpoint_info_to_mep_db_entry(int unit,const bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry ) {
    ...
    case bcmBFDTunnelTypePweGal:
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            mep_db_entry->bfd_pwe_ach = SOC_PPD_BFD_ACH_TYPE_GACH_CC;
        }
        else {
    ...
    case bcmBFDTunnelTypePweTtl:
        ...
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_PWE_ACH ) {
            mep_db_entry->bfd_pwe_ach = SOC_PPD_BFD_ACH_TYPE_PWE_CW;
        }
        break; 
    case bcmBFDTunnelTypePweControlWord:
        mep_db_entry->mep_type = SOC_PPD_OAM_MEP_TYPE_BFD_O_PWE;
    ...
    if (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP) { /* ONE HOP IPV4 */
...
int _bcm_bfd_mep_db_entry_struct_set_pointers(int unit, const bcm_bfd_endpoint_info_t *endpoint_info, const bcm_bfd_endpoint_info_t *existing_endpoint_info, 
                                              const SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *existing_mep_db_entry,
                                              SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, _bcm_bfd_mep_db_entry_pointers_to_free * pointers_to_free){
   int is_allocated;
    uint32 soc_sand_rv;
    int rv;
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeUdp) || (endpoint_info->type == bcmBFDTunnelTypeMpls)) {
        ...
        if (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP ||  (endpoint_info->type == bcmBFDTunnelTypeMpls) ) {
            is_allocated = 0;
            if (existing_endpoint_info==NULL) {
                ...
                pointers_to_free->free_tos_ttl_profile =0;
            }
...
/* Fill MEP-entry struct
   When called by endpoint_create (update) tx_period should be freed by here, thus original pointer is given as input.
    This is due to a HW bug: if the pointer remains the same but the rate is changed then packets will not be sent for a few seconds.
	The solution is to allocate a new tx rate and then free the old one.
   Last paramater should be set to -1 otherwise (not under "endpoint update").*/
int _bcm_bfd_mep_db_entry_struct_set(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int original_tx_period_pointer) {
    ...
    tx_rate = SOC_SAND_DIV_ROUND(endpoint_info->bfd_period * 100, temp_index); /* Tx Rate is in 3.33 (JER - 1.67) ms units */
	--tx_rate; /* TX rate in the HW is one lower then what was calculated above.*/
    rv = _bcm_dpp_am_template_bfd_tx_rate_profile_alloc(unit, 0/*flags*/, &tx_rate, &is_allocated, &temp_index);
	if (rv == BCM_E_MEMORY && original_tx_period_pointer!=-1) {
		/* Current entry should be freed.*/
		rv = _bcm_dpp_am_template_bfd_tx_rate_profile_free(unit, original_tx_period_pointer, &is_last);
		BCMDNX_IF_ERR_EXIT(rv); 
		/* now try again*/
		rv = _bcm_dpp_am_template_bfd_tx_rate_profile_alloc(unit, 0/*flags*/, &tx_rate, &is_allocated, &temp_index);
		original_tx_period_pointer = -1;
	}
    BCMDNX_IF_ERR_EXIT(rv);
    mep_db_entry->ccm_interval = (uint8)temp_index;
    if (is_allocated) {
	...
	if (original_tx_period_pointer!=-1) {
		/* free the original entry.*/
		/* See comment at the start of the function.*/
		rv = _bcm_dpp_am_template_bfd_tx_rate_profile_free(unit, original_tx_period_pointer, &is_last);
		BCMDNX_IF_ERR_EXIT(rv); 
        if (is_last) {
            soc_sand_rv = soc_ppd_oam_bfd_tx_rate_set(unit, original_tx_period_pointer, 0);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    ...
    if (PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type) && original_tx_period_pointer==-1) {
        /* When updating the endpoint the tx_gport may not be updated anyways.
           In that case it is assumed that this hasn't been freed and thus shouldn't be allocated again.*/
        int local_port, ignored;
...
int _bcm_bfd_mep_db_entry_struct_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
    ...
    if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop && SOC_IS_ARADPLUS_A0(unit) 
        && mep_db_entry->mep_type== SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP && ((mep_db_entry->tos_ttl_profile==_BCM_BFD_IPV4_SINGLE_HOP_TTL_PROFILE)
    ...
    case SOC_PPD_OAM_MEP_TYPE_BFD_O_PWE:
        ...
        if (mep_db_entry->bfd_pwe_ach == SOC_PPD_BFD_ACH_TYPE_PWE_CW && endpoint_info->type==bcmBFDTunnelTypePweRouterAlert ) {
 ...
 * @return int 
 */
int _bcm_mep_db_entry_dealloc2(int unit, uint32 mep_index, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, const _bcm_bfd_mep_db_entry_pointers_to_free * pointers_to_free) {
    int is_last;
    uint32 soc_sand_rv;
    int rv;
    SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES tx_itmh_attributes;
...
/* Free all MEP-entry struct relevant info
   When this is called by endpoint_create (update) the tx_rate, local_port_2_sys_port should not be freed.
   Everything should be freed only when this function is called from endpoint_destroy()*/
int _bcm_mep_db_entry_dealloc(int unit, uint32 mep_index, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int called_from_ep_destroy) {
    int is_last;
    uint32 soc_sand_rv;
    int rv;
    SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES tx_itmh_attributes;
    ...
    /* To avoid grabage tx during/after the deallocation, first disable the endpoint packets creation */
    rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_bfd_acc_endpoint_tx_disable, (unit, mep_index, mep_db_entry));
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP) {
        ...
        rv = BFD_ACCESS.ipv4_multi_hop_acc_ref_counter.set(unit, ipv4_multi_hop_acc_ref_counter);
        BCMDNX_IF_ERR_EXIT(rv);
        if (ipv4_multi_hop_acc_ref_counter == 0 && called_from_ep_destroy) {
            SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES_clear(&tx_itmh_attributes);
            soc_sand_rv = soc_ppd_oam_bfd_tx_ipv4_multi_hop_set(unit, &tx_itmh_attributes);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
        ...
        rv = _bcm_dpp_am_template_oam_tx_priority_free(unit, mep_db_entry->priority, &is_last);
        BCMDNX_IF_ERR_EXIT(rv);
        if (is_last && called_from_ep_destroy) {
            SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES tx_itmh_attributes;
            SOC_PPD_OAMP_TX_ITMH_ATTRIBUTES_clear(&tx_itmh_attributes);
            soc_sand_rv = soc_ppd_oam_oamp_tx_priority_registers_set(unit, mep_db_entry->priority, &tx_itmh_attributes);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if ((mep_db_entry->tos_ttl_profile == _BCM_BFD_IPV4_SINGLE_HOP_TTL_PROFILE) || ((SOC_DPP_CONFIG(unit)->pp.bfd_echo_min_interval== 1) &&
                                                       (mep_db_entry->tos_ttl_profile == _BCM_BFD_IPV4_ECHO_MIN_INTERVAL_PROFILE))) {
		int tmp;
		rv = BFD_ACCESS.s_hop.ref_counter.get(unit, &tmp);
		BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type) && called_from_ep_destroy) {
        rv = _bcm_dpp_am_template_oam_local_port_2_sys_port_free(unit,mep_db_entry->local_port, &is_last);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
int _bcm_bfd_classifier_mep_entry_struct_set(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, int dip_ndx) {
    ...
    classifier_mep_entry->flags |= ((endpoint_info->flags & BCM_BFD_ENDPOINT_EXPLICIT_DETECTION_TIME) != 0) ? SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_TIMEOUT_SET_EXPLICITLY : 0;
 ...
 * SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY. This function can be used
 * in both cases.
 */
                                      ...
                                      uint8 *is_acc_to_oamp, uint8 *is_oamp_server)
{
    ...
    rv = _bcm_bfd_acceleration_to_oamp_by_trap_and_flags_check(
                ...
                SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED,
                SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_SERVER,
...
/* In warmboot oam init is called first and inits DBs, so we don't need to init them from BFD */
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_CPU, SOC_PPD_TRAP_CODE_OAM_CPU);
        BCMDNX_IF_ERR_EXIT(rv);
    }
        _BCM_OAM_ENABLED_GET(oam_is_init);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (!oam_is_init) {
            rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_CPU, &trap_id_sw);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            rv = OAM_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_OAM_TRAP_ID_CPU, trap_id_sw /*transfering the SW id*/);
            BCMDNX_IF_ERR_EXIT(rv);

            /* Increasing trap code counter in order to for this trap not to be deleted with another profile */
            rv = _bcm_dpp_oam_traps_ref_counter_increase(unit, trap_id_sw);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
            rv = OAM_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_OAM_TRAP_ID_CPU, &trap_id_sw);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_CPU, trap_id_sw);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_OAMP_IPV4, SOC_PPD_TRAP_CODE_TRAP_BFD_O_IPV4);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_OAMP_MPLS, SOC_PPD_TRAP_CODE_TRAP_BFD_O_MPLS);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_OAMP_PWE, SOC_PPD_TRAP_CODE_TRAP_BFD_O_PWE);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP, SOC_PPD_TRAP_CODE_TRAP_BFDCC_O_MPLS_TP);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv6_enable) {
            if (bfd_ipv6_trap_port != -1) {
                trap_id_sw = SOC_PPC_TRAP_CODE_BFD_IPV6_YOUR_DISCR_NOT_FOUND;
                BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,
                                                                        trap_id_sw,
                                                                        &trap_code_converted));
                ...
                rv = BFD_ACCESS.trap_info.trap_ids.set(unit, SOC_PPD_BFD_TRAP_ID_UC_IPV6, SOC_PPD_TRAP_CODE_TRAP_BFD_O_IPV6);
                BCMDNX_IF_ERR_EXIT(rv);
                rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_UC_IPV6, &trap_id_sw);
                BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_bfd_endpoint_validity_checks(int unit, bcm_bfd_endpoint_info_t *endpoint_info_lcl,
    ...
    if (!(*is_endpoint_acc_to_oamp)) {
        ...
        } else if (endpoint_info_lcl->bfd_period <4) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                 (_BSL_BCM_MSG("Error: bfd_period 0 - 3 unavailable.")));
        }
...
int _bcm_bfd_endpoint_replace_validity_checks(int unit, bcm_bfd_endpoint_info_t *endpoint_info_lcl,
    ...
    if (endpoint_info_lcl->tx_gport != BCM_GPORT_INVALID || existing_endpoint_info->tx_gport != BCM_GPORT_INVALID ) {
        if ((endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_IN_HW)) {
            rv = _bcm_dpp_gport_compare(unit, endpoint_info_lcl->tx_gport, existing_endpoint_info->tx_gport, _bcmDppGportCompareTypeSystemPort, &port_is_equal);
...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    uint32 snp_strength, fwd_strength;
    ...
    rv = OAM_ACCESS.y1731_opcode_to_intenal_opcode_map.get(unit,
        SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM,
        &internal_bfd_opcode);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if (is_endpoint_acc_to_oamp) {
            ...
            */
            ...
            if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_original_update_procedure", 0)) {
                /* Currently two implementations for updating the MEP-DB. The original one and the new one.
                   This is the original update which should be removed when the new one is verified. Here the MEP-DB is temporarily disabled.*/
                original_tx_rate = mep_db_entry.ccm_interval; 

                rv = _bcm_mep_db_entry_dealloc(unit, endpoint_info_lcl->id, &mep_db_entry,0 /* do not free tx rate, local_port_2_sys_port*/);
                BCMDNX_IF_ERR_EXIT(rv);
                SOC_PPD_OAM_OAMP_MEP_DB_ENTRY_clear(&mep_db_entry);
                rv = _bcm_bfd_mep_db_entry_struct_set(unit, endpoint_info_lcl, &mep_db_entry,original_tx_rate );
                BCMDNX_IF_ERR_EXIT(rv);
                /*set entry in mep db*/
                ...
                SOC_PPD_OAM_OAMP_MEP_DB_ENTRY new_mep_db_entry;
                _bcm_bfd_mep_db_entry_pointers_to_free pointers_to_free = {0}; /* Initialize to 0: free nothing*/
                SOC_PPD_OAM_OAMP_MEP_DB_ENTRY_clear(&new_mep_db_entry);
                ...
                _bcm_bfd_enpoint_info_to_mep_db_entry(unit, endpoint_info_lcl,&new_mep_db_entry);
        ...
        if (is_default) {
            ...
            if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
                ...
                if (endpoint_info_lcl->remote_gport != BCM_GPORT_INVALID) {
                    ...
                    rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_CPU, &trap_code);
                    BCMDNX_IF_ERR_EXIT(rv);
                }
            ...
            if (ref_counter_tos != 0) {
                ...
                rv = BFD_ACCESS.s_hop.tos_value.get(unit, &tos);
                BCMDNX_IF_ERR_EXIT(rv);
                if (tos != endpoint_info_lcl->ip_tos) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                            (_BSL_BCM_MSG("Error: Supporting single IP TOS value per device. Existing value: %d")
                             , tos));
                }
                ...
                if (!(endpoint_info_lcl->flags & BCM_BFD_ECHO) && (is_1_hop_extended ==0))
				 {
					...
					unit,0, &classifier_mep_entry, &profile_data, &profile_data_acc_dummy);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    ...
                    if (endpoint_info_lcl->remote_gport != BCM_GPORT_INVALID) {
                        ...
                        fwd_strength = _BCM_OAM_BFD_DEFAULT_FWD_STRENGTH_GET(unit);
                    }
                    else {
                        trap_code = 0;
                        snp_strength = 0;
            ...
            if (is_endpoint_acc_to_oamp) {
                ...
                */
                ...
                if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_original_update_procedure", 0)) {
                    rv = _bcm_bfd_mep_db_entry_struct_set(unit, endpoint_info_lcl, &mep_db_entry, -1);
                    BCMDNX_IF_ERR_EXIT(rv);
                } else {
                    _bcm_bfd_mep_db_entry_pointers_to_free pointers_to_free_dummy = {0}; /* Initialize to 0: free nothing*/

                    /* First, translate from endpoint_info to mep_db_entry, excluding pointers*/
                    _bcm_bfd_enpoint_info_to_mep_db_entry(unit, endpoint_info_lcl,&mep_db_entry);
...
int bcm_petra_bfd_endpoint_destroy(
   ...
   bcm_bfd_endpoint_t endpoint) {
    ...
    if (is_default) {
        ...
        if (is_last) {
            rv = _bcm_dpp_oam_mep_profile_trap_data_free(unit, &profile_data.mep_profile_data, &classifier_mep_entry, 1/*is_mep*/);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        if (classifier_mep_entry.remote_gport != BCM_GPORT_INVALID) {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            if (no_traps && is_internal_trap) {
                /* Internal traps should be destroyed when no more references
                   to them exist */
                ...
                /* Once the trap is freed, its ID may be re-allocated
                   externally so unmark it as internal */
    ...
    is_single_hop_extended =((SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop) && classifier_mep_entry.mep_type==SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP);
    ...
    if (is_endpoint_acc_to_oamp) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_original_update_procedure", 0)) {
            ...
            rv = _bcm_mep_db_entry_dealloc(unit, endpoint, &mep_db_entry, 1 /*called frome ep destroy*/);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
...
int bcm_petra_bfd_clear(int unit) {
    ...
    rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_OAMP_IPV4, &trap_id_sw);   
    BCMDNX_IF_ERR_EXIT(rv);
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted );
    BCMDNX_IF_ERR_EXIT(rv);

    rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_OAMP_MPLS, &trap_id_sw);   
    BCMDNX_IF_ERR_EXIT(rv);
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_OAMP_PWE, &trap_id_sw);
    BCMDNX_IF_ERR_EXIT(rv);
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted);
    BCMDNX_IF_ERR_EXIT(rv);

    rv = BFD_ACCESS.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP, &trap_id_sw);
    BCMDNX_IF_ERR_EXIT(rv);
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_rx_get_converted_trap_hw_id(unit,trap_id_sw, &trap_code_converted));
    rv = bcm_petra_rx_trap_type_destroy(unit, trap_code_converted );
    BCMDNX_IF_ERR_EXIT(rv);
