...
{ 
    ...
    if (l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_MC){
        ...
        if (!SOC_DPP_CONFIG(unit)->trill.mc_id) {
            ...
            rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit, &gport_dummy, &(l2addr->encap_id), &(l2_entry_val->frwrd_info.forward_decision), BCM_FORWARD_ENCAP_ID_OUTLIF_USAGE_GENERAL); 
            BCMDNX_IF_ERR_EXIT(rv);            
        }
    }
        ...
        if(l2_entry_val->frwrd_info.forward_decision.additional_info.trap_info.action_profile.trap_code == _BCM_PETRA_UD_DROP_TRAP){
            ...
            rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&(l2addr->port),NULL,&(l2_entry_val->frwrd_info.forward_decision), -1/*encap_usage*/);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
    else
    {
        ...
        rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&(l2addr->port),&(l2addr->encap_id),&(l2_entry_val->frwrd_info.forward_decision),BCM_FORWARD_ENCAP_ID_OUTLIF_USAGE_GENERAL);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if(trap_info->strength != 0 && (trap_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST)) { 
        if(trap_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_MC) {
            ...
            if (trap_info->dest_info.frwrd_dest.type == SOC_PPD_FRWRD_DECISION_TYPE_DROP) {
                ...
                rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&(addr->dest_port),NULL,&(trap_info->dest_info.frwrd_dest), -1/*encap_usage*/);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
