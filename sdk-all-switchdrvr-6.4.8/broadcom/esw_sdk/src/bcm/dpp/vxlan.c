...
bcm_petra_vxlan_port_sw_state_add(
    ...
    int update)
 {
    ...
    /* update lif usage */
    rv = _bcm_dpp_lif_sw_resources_lif_usage_set(unit, local_in_lif, local_out_lif, _bcmDppLifTypeVxlan, _bcmDppLifTypeVxlan);
    BCMDNX_IF_ERR_EXIT(rv);

    if (local_in_lif != -1) {
        rv = _bcm_dpp_lif_sw_resources_vsi_set(unit, local_in_lif, l2vpn);
        BCMDNX_IF_ERR_EXIT(rv);
    }   
...
bcm_petra_vxlan_port_sw_state_remove(
    ...
    int out_lif)
 {
    ...
    rv = _bcm_dpp_lif_sw_resources_lif_usage_set(unit, in_lif, out_lif, _bcmDppLifTypeIpTunnel, _bcmDppLifTypeIpTunnel);
    BCMDNX_IF_ERR_EXIT(rv);

    if (in_lif != -1) {
        rv = _bcm_dpp_lif_sw_resources_vsi_set(unit, in_lif, 0);
        BCMDNX_IF_ERR_EXIT(rv);
    }
