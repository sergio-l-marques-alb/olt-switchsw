    ...
    int unit)
{
    ...
    SOC_PPD_LIF_ID                          local_trill_in_lif_id = 0;
    bcm_port_t                              port;
            ...
            if (global_trill_in_lif_id != -1) {
                if (SOC_IS_JERICHO(unit)) {
                    ...
                    rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, &global_trill_in_lif_id, 
                                                                   &local_in_lif_info, NULL, FALSE);
                    BCMDNX_IF_ERR_EXIT(rv);

                    local_trill_in_lif_id = local_in_lif_info.base_lif_id;

                } else {
                    rv = bcm_dpp_am_l2_ac_alloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_VSI, BCM_DPP_AM_FLAG_ALLOC_WITH_ID, (SOC_PPD_LIF_ID*)&global_trill_in_lif_id);
                    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_trill_port_get(int unit,bcm_trill_port_t *trill_port){
    ...
    if (BCM_GPORT_TRILL_PORT_ID_IS_EGRESS(port_id)) {
        ...
        global_out_lif_id = SOC_DPP_TRILL_OUTLIF_FROM_ENCAP_ID(encap_id); 
        ...
        rv = soc_ppd_eg_encap_entry_get(unit, 
                                        SOC_PPD_EG_ENCAP_EEP_TYPE_TRILL, 
                                        local_out_lif_id,
                                        1, /* depth */
                                        encap_entry_info, 
                                        next_eep, 
...
int bcm_petra_trill_cleanup(int unit) {
    ...
    BCM_DPP_UNIT_CHECK(unit);
    if (SOC_IS_PETRAB(unit)) {
        status = _bcm_dpp_trill_sw_db_hash_trill_route_info_destroy(unit); 
    ...
    int                 is_replace)
{
    ...
    if (is_egress) {
        ...
        int trill_global_out_lif_id = 0, trill_local_out_lif_id = 0;
        ...
        if (trill_port->flags & BCM_TRILL_PORT_WITH_ID) {
            ...
            if (!SOC_DPP_ENCAP_ID_IS_TRILL_OUTLIF(encap_id)) {
            ...
            trill_global_out_lif_id = SOC_DPP_LIF_FROM_TRILL_OUTLIF_ENCAP_ID(encap_id); 
        ...
        if (!(trill_port->flags & BCM_TRILL_PORT_REPLACE)) {
             ...
             * - for Trill tunnel eedb entry in ROO application
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, outlif_alloc_flags, &trill_global_out_lif_id, NULL, &outlif_info, TRUE);
                BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = (MBCM_PP_DRIVER_CALL(unit,mbcm_pp_eg_trill_entry_set,(unit,trill_local_out_lif_id,&trill_encap_info,local_ll_lif,ll_eep_is_valid)));
        ...
        BCM_GPORT_TRILL_PORT_ID_SET(trill_port->trill_port_id,   /* gport */
                                    SOC_DPP_ENCAP_ID_FROM_TRILL_OUTLIF(trill_global_out_lif_id) /* gport's encap_id */
                                    ); 
        BCM_EXIT;
    }
    ...
    if (!is_replace) {
        ...
        rv = _bcm_dpp_trill_sw_db_hash_trill_info_find(unit,
                                           (sw_state_htb_key_t) &(fecless_port.trill_port_id),
        ...
        if (working_fec.type == SOC_PPC_FEC_TYPE_TRILL_MC) {
            ...
            if ( !(trill_port->flags & BCM_TRILL_PORT_LOCAL ) ) {
                ...
                if (!(trill_port->flags & BCM_TRILL_PORT_TUNNEL)) {
                    soc_sand_rv = soc_ppd_frwrd_trill_unicast_route_add(soc_sand_dev_id, local_lif_id, trill_port->name, &fwd_decision, &success);  
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    SOC_SAND_IF_FAIL_RETURN(success);
                }
                ...
                if (!(trill_port->flags & BCM_TRILL_PORT_EGRESS)) {        
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, 0, &global_lif_id, &local_in_lif_info, NULL, FALSE);
                        BCMDNX_IF_ERR_EXIT(rv);

                        local_lif_id = local_in_lif_info.base_lif_id;
                    } else {
                        rv = bcm_dpp_trill_lif_alloc(unit,0,(SOC_PPD_LIF_ID*)&global_lif_id);
                        BCMDNX_IF_ERR_EXIT(rv);
    ...
    int         reset_local_info)
{
    ...
    unsigned int                soc_sand_dev_id;
    uint32                      soc_sand_rv  = SOC_SAND_OK;
    SOC_PPD_LIF_ID                  local_in_lif_id=0;
    ...
    if (BCM_GPORT_TRILL_PORT_ID_IS_EGRESS(trill_port_id)) {
        ...
        /* get the outlif from the encap */
        trill_global_out_lif = SOC_DPP_TRILL_OUTLIF_FROM_ENCAP_ID(encap_id); 
        ...
        /* read entry from HW */
        rv = soc_ppd_eg_encap_entry_get(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_TRILL, trill_local_out_lif, 1, 
        ...
        if (SOC_IS_JERICHO(unit)) {
            rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, trill_global_out_lif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, trill_local_out_lif, TRUE);
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
        ...
        rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_TRILL, trill_local_out_lif); 
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if ( (phy_port.type != _bcmDppGportResolveTypeTrillMC) && !(trill_port.flags & BCM_TRILL_PORT_LOCAL ) && !is_ecmp) {        
        ...
        if (!(trill_port.flags & BCM_TRILL_PORT_TUNNEL)) {   
            soc_sand_rv = soc_ppd_frwrd_trill_unicast_route_remove(soc_sand_dev_id, phy_port.encap_id , &local_in_lif_id);  
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }          
                                            ...
                                            bcm_trill_multicast_adjacency_t *trill_multicast_adjacency)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        int eep; 
        rv = _bcm_l3_encap_id_to_eep(unit, trill_multicast_adjacency->encap_intf, &eep);
        mac_auth_info.expect_adjacent_eep = (uint32) eep; 
        BCMDNX_IF_ERR_EXIT(rv);
    }
