...
{
    int rv = BCM_E_NONE;
	bcm_policer_t policer_id_core0;
	bcm_policer_t policer_id_core1;
    ...
    DPP_POLICER_UNIT_LOCK(unit);
	if (SOC_IS_JERICHO(unit)) {
		rv = bcm_dpp_am_policer_alloc(unit, 0, 0, *npolicers, &policer_id_core0);
		BCMDNX_IF_ERR_EXIT(rv);
		rv = bcm_dpp_am_policer_alloc(unit, 1, 0, *npolicers, &policer_id_core1);
		BCMDNX_IF_ERR_EXIT(rv);
		if (policer_id_core0 != policer_id_core1) {
			 BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (DPP_POLICER_MSG("cores allocated different policers id\n")));
		}
		*policer_id = policer_id_core0;
	} else{
...
{
    int policer_indx, max_policer_indx;
	...
	max_policer_indx = SOC_IS_JERICHO(unit) ? _DPP_POLICER_MIN_POLICER_ID_JERICHO : _DPP_POLICER_MIN_POLICER_ID;
    ...
    soc_sand_dev_id = (unit);
    if (policer_indx > _DPP_POLICER_MAX_POLICER_ID || policer_indx < max_policer_indx) {
    ...
    rv = bcm_dpp_am_policer_is_alloc(unit, 0, 0, policer_indx);
    if (rv != BCM_E_EXISTS) {
	...
	if (SOC_IS_JERICHO(unit)) {
		soc_sand_rv = jer_pp_mtr_eth_policer_glbl_profile_set(soc_sand_dev_id, policer_indx, &policer_info);
		BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
	}else{
...
bcm_petra_eth_policer_destroy(int unit, bcm_policer_t policer_id) {
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    /* MARK AS UNUSED */
    rv = bcm_dpp_am_policer_dealloc(unit, 0, 0, 1, policer_id);
    BCMDNX_IF_ERR_EXIT(rv);
   ...
   bcm_policer_t policer_id) 
{
    ...
    if (policer_id != 0) {
        ...
        for (plc_indx = policer_indx + 1; plc_indx < policer_indx + num_to_alloc; ++plc_indx) {
            rv = bcm_dpp_am_policer_is_alloc(unit, 0, 0, plc_indx);
            if (rv != BCM_E_EXISTS) {
	...
	if (SOC_IS_JERICHO(unit)) {
		...
		SOC_SAND_IF_ERR_EXIT(bcm_port_get(unit, gport, &dummy_flags, &interface_info, &mapping_info));
		...
		BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
			for (pkt_type = 0; pkt_type < SOC_PPD_NOF_MTR_ETH_TYPES; ++pkt_type) {
				soc_sand_rv = jer_pp_mtr_eth_policer_glbl_profile_map_set(
				   unit,
				   core_id,
				   port_i,
				   pkt_type,
