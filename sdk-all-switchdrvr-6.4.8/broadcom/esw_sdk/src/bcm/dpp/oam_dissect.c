...
int _bcm_oam_generic_endpoint_validity_checks(int unit, const bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            correct_flags2 |= (BCM_OAM_ENDPOINT2_REMOTE_UPDATE_STATE_DISABLE
                               | BCM_OAM_ENDPOINT2_RX_REMOTE_EVENT_DISABLE
                               | BCM_OAM_ENDPOINT2_RX_REMOTE_DEFECT_AUTO_UPDATE
                               | BCM_OAM_ENDPOINT2_RDI_ON_RX_RDI
                               | BCM_OAM_ENDPOINT2_RDI_CLEAR_ON_LOC_CLEAR
                               | BCM_OAM_ENDPOINT2_RDI_ON_LOC);
            if  (endpoint_info->flags2 & BCM_OAM_ENDPOINT2_REMOTE_UPDATE_STATE_DISABLE &&
                ...
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                    (_BSL_BCM_MSG("Error: when BCM_OAM_ENDPOINT2_REMOTE_UPDATE_STATE_DISABLE"
                                                  " flag is set BCM_OAM_ENDPOINT2_RX_REMOTE_EVENT_DISABLE,"
            ...
            if ((endpoint_info->flags & BCM_OAM_ENDPOINT2_RX_REMOTE_EVENT_DISABLE)
                && (endpoint_info->flags & BCM_OAM_ENDPOINT2_RX_REMOTE_DEFECT_AUTO_UPDATE)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                    (_BSL_BCM_MSG("Error: when BCM_OAM_ENDPOINT2_RX_REMOTE_EVENT_DISABLE"
                                                  " is set, BCM_OAM_ENDPOINT2_RX_REMOTE_DEFECT_AUTO_UPDATE"
                                                  " flag may not be set.\n")));
        ...
        if (is_accelerated) {
            ...
            correct_flags2 |= (BCM_OAM_ENDPOINT2_RDI_FROM_RX_DISABLE | BCM_OAM_ENDPOINT2_RDI_FROM_LOC_DISABLE)
                                * SOC_IS_ARADPLUS(unit);

            if (endpoint_info->flags & BCM_OAM_ENDPOINT_RDI_TX
                && endpoint_info->flags2 & BCM_OAM_ENDPOINT2_RDI_FROM_RX_DISABLE) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                    (_BSL_BCM_MSG("When the flag BCM_OAM_ENDPOINT_RDI_TX is set,"
                                                  " the flag2 BCM_OAM_ENDPOINT2_RDI_FROM_RX_DISABLE"
                                                  " must not be set. ")));
            }
...
int _bcm_oam_endpoint_local_replace_validity_check(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl,
    ...
    if (is_accelerated) {
        if (endpoint_info_lcl->tx_gport != BCM_GPORT_INVALID
            || existing_endpoint_info->tx_gport != BCM_GPORT_INVALID) {
        ...
        if  ((endpoint_info_lcl->vlan != existing_endpoint_info->vlan)
...
int _bcm_oam_loopback_validity_check(int unit, bcm_oam_loopback_t *loopback_ptr,
    ...
    if ( loopback_ptr->flags & ~(BCM_OAM_LOOPBACK_UPDATE|BCM_OAM_LOOPBACK_PERIOD_IN_KPPS) ) {
