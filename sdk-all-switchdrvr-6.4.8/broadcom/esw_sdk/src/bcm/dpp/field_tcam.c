...
/* coverity[uninit_use_in_call] */
            ...
            for (index = 0; index < actCount; index++) {
                if (BCM_GPORT_INVALID != lookupOutLif[index]) {
                    ...
                    {
                        BCMDNX_IF_ERR_EXIT(_bcm_dpp_field_gport_to_local_lif_convert(unit,
                                lookupOutLif[index],
                                TRUE,
...
#endif /* def BCM_ARAD_SUPPORT */ 
            case bcmFieldActionRedirect:
                /* This action is not supported in Jericho*/
                if (SOC_IS_JERICHO(unit)) 
                {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL,(_BSL_BCM_MSG_NO_UNIT("bcmFieldActionRedirect in Egress stage is unsupported in Jericho")));
                }
                /* param0 = module */
                /* param1 = GPORT or port number */
