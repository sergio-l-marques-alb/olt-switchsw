...
_bcm_petra_is_cross_connect_lif(int unit,
                                SOC_PPD_LIF_ID local_in_lif_index)
{
    ...
    bcm_gport_t    port)
{
        ...
        if ((!is_ingress_only) && (!SOC_IS_JERICHO(unit) || (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1) != 2))) {
            if(vlan_port.flags & BCM_VLAN_PORT_NETWORK) {
                ...
                if (split_horizon_filter_enable) {
                    if (SOC_IS_PETRAB(unit)) {
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(soc_sand_dev_id,gport_hw_resources.local_out_lif,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else {
                        /* ARAD */
                ...
                if(split_horizon_filter_enable){
                    /* if egress hairpin is enabled set egress state as well*/
                    if (SOC_IS_PETRAB(unit)) {
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(soc_sand_dev_id,gport_hw_resources.local_out_lif,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else {
                        /* ARAD */
    ...
    /* 
		...
		rv = _bcm_dpp_lif_sw_resources_learn_gport_get(unit, gport_hw_resources.local_in_lif, &learn_gport);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    else if (lif_info->type & SOC_PPD_LIF_ENTRY_TYPE_EXTENDER) {
        lif_info->value.extender.vsid = vsi;
        lif_info->value.extender.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_VSI; 
        lif_info->value.extender.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP;

        /* vswitch is MP with learning */
        lif_info->value.extender.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_SYS_PORT;
		rv = _bcm_dpp_lif_sw_resources_learn_gport_get(unit, gport_hw_resources.local_in_lif, &learn_gport);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (lif_info->value.extender.learn_record.learn_info.type != SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT) {
            lif_info->value.extender.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_INFO;
        }
    }
    ...
    /* set map to vsi */
    rv = _bcm_dpp_lif_sw_resources_vsi_set(unit, gport_hw_resources.local_in_lif, vsi);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_gport_t    port)
{
    ...
    if(lif_info->type & (SOC_PPC_LIF_ENTRY_TYPE_AC|SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP|SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP)){
        ...
        lif_info->value.ac.learn_record.learn_type = SOC_PPD_L2_LIF_AC_LEARN_DISABLE;
        lif_info->value.extender.service_type = SOC_PPD_L2_LIF_AC_SERVICE_TYPE_AC2AC;

        /* no protection & use egress-tunnel*/
        lif_info->value.extender.default_frwrd.default_frwd_type = SOC_PPD_L2_LIF_DFLT_FRWRD_SRC_LIF;
    ...
    /* set map to NULL VSI */
    rv = _bcm_dpp_lif_sw_resources_vsi_set(unit, gport_hw_resources.local_in_lif, 0);
    BCMDNX_IF_ERR_EXIT(rv);
...
{    
    ...
    *vsi = 0;
    ...
    /* from LIF to VSI */
    rv= _bcm_dpp_lif_to_vsi(unit,gport_hw_resources.local_in_lif,&vsi_val, TRUE, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
...
{
    int rv, is_local;
    SOC_PPD_LIF_ID local_lif_index1, local_lif_index2;
    ...
    bcm_vswitch_cross_connect_t *gports)
{
    ...
    rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, (int)lif_index, -1, &in_lif_usage, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_DPP_IS_SYS_RSRC_MGMT_ADVANCED(unit)) {
        ...
        rv = _bcm_dpp_gport_and_encap_from_fwd_decision(unit,&gports->port2,&gports->encap2,frwrd_decision,encap_usage);
        BCMDNX_IF_ERR_EXIT(rv);
    }
