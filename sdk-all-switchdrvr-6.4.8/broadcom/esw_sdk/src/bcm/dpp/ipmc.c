  ...
  )
{
    ...
    if (found == SOC_SAND_FALSE) {
        ...
        if (data->flags & BCM_IPMC_REPLACE) {/*Only print message if we intended to find entry*/
            LOG_INFO(BSL_LS_BCM_IPMC, (BSL_META_U(unit, "IPV4 MC entry not found")));
  ...
  )
{
    ...
    if (found == SOC_SAND_FALSE) {
        /* Entry not found */
        LOG_INFO(BSL_LS_BCM_IPMC, (BSL_META_U(unit, "IPV6 MC entry not found\n")));
        BCM_RETURN_VAL_EXIT(BCM_E_NOT_FOUND);
    }
  ...
  )
{
    ...
    if (data->s_ip_addr == 0) {
        ...
        if (is_ipmc_ssm_enabled) {
            ...
            tcam_route_key.group_prefix_len = bcm_ip_mask_length(data->mc_ip_mask);
  ...
  )
{
    ...
    if (!found) {
        LOG_INFO(BSL_LS_BCM_IPMC, (BSL_META_U(unit, "IPV4 MC (bridged) entry not found\n")));
        BCM_RETURN_VAL_EXIT(BCM_E_NOT_FOUND);
    }
