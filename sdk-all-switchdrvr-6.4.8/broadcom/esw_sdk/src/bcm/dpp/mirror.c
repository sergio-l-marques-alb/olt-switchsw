                                ...
                                bcm_port_t destPort,int* mirrorId)
{
    ...
    SOC_PB_ACTION_CMD_MIRROR_INFO cmd_mirrorInfo;
...
bcm_petra_mirror_init(int unit)
{
    ...
    SOC_PB_ACTION_CMD_MIRROR_INFO mirror_info;
    uint8 is_allocated;
...
bcm_petra_mirror_destination_erspan_lif_alloc(int unit, uint32 alloc_flags, int *global_outlif, int *local_outlif) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, alloc_flags, global_outlif, NULL, &local_outlif_info, TRUE);
        ...
        *local_outlif = *global_outlif; /* If the WITH_ID flag is set, the lif value will be in the global lif */
        rv = bcm_dpp_am_eg_data_erspan_alloc(unit, alloc_flags, (SOC_PPD_LIF_ID*)local_outlif);
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        if(mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_IP_GRE) {
            *outlif_valid = 1;
            ...
            rv = _bcm_dpp_global_lif_mapping_global_to_local_get(unit, _BCM_DPP_GLOBAL_LIF_MAPPING_EGRESS, tmp_global_outlif, (int*)&ip_tunnel_id);
            BCMDNX_IF_ERR_EXIT(rv);
...
bcm_petra_mirror_destination_tunnel_destroy(int unit, int global_lif, int local_outlif)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        if (SOC_IS_JERICHO(unit)) {
            rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_outlif, TRUE);
            BCMDNX_IF_ERR_EXIT(rv);
        } else if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        /* add enry to allocated place */
        soc_sand_rv = soc_ppd_eg_encap_entry_remove(soc_sand_dev_id, SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, local_outlif);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    else
#endif /* SOC_IS_ARAD */
                                    ...
                                    bcm_mirror_destination_t *mirror_dest, uint8 set_dest)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        info.prob = 0x3ff; /* 100% probability of mirror execution in Soc_petra. */
    }
    ...
    if ((flags = mirror_dest->flags & ~(BCM_MIRROR_DEST_WITH_ID | BCM_MIRROR_DEST_REPLACE | BCM_MIRROR_DEST_UPDATE_POLICER | BCM_MIRROR_DEST_DEST_MULTICAST |
                                        ...
                                        | (SOC_IS_JERICHO(unit) ? BCM_MIRROR_DEST_EGRESS_ADD_ORIG_SYSTEM_HEADER : 0)))) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        info.add_orig_head = ((mirror_dest->flags & BCM_MIRROR_DEST_EGRESS_ADD_ORIG_SYSTEM_HEADER) ? 1 : 0);
    }

    info.cmnd_type = mirror_dest->flags & BCM_MIRROR_DEST_IS_SNOOP ? SOC_TMC_CMD_TYPE_SNOOP : SOC_TMC_CMD_TYPE_MIRROR; /*set command type*/
...
exit:
	...
	/* coverity[unchecked_value] */
        if (mirror_dest->flags & BCM_MIRROR_DEST_IS_SNOOP) {
            _bcm_dpp_am_snoop_dealloc(unit,0,action_profile_id,0);
        } else {
                                     ...
                                     bcm_gport_t mirror_dest_id)
{
    ...
    info.cmnd_type = (BCM_GPORT_IS_MIRROR_SNOOP(mirror_dest_id) ? SOC_TMC_CMD_TYPE_SNOOP : SOC_TMC_CMD_TYPE_MIRROR);
                                 ...
                                 bcm_mirror_destination_t *mirror_dest)
{
    ...
    info.cmnd_type = BCM_GPORT_IS_MIRROR_SNOOP(mirror_dest_id); /*0 for mirror and 1 for snoop*/
    ...
    if (!is_created) {
        BCMDNX_ERR_EXIT_MSG(is_created, (_BSL_BCM_MSG("specified mirroring/snooping destination ID %u is not created"), (unsigned)action_profile_id));
    }
    ...
    if (SOC_IS_PETRAB(unit)) {
        if (info.cmd.meter_ptr_low.enable) { /* lower bank meter is used, single core in Petra */
            mirror_dest->policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, info.cmd.meter_ptr_low.value, 0, 0/*only one core in PetraB*/);
        } else if (info.cmd.meter_ptr_up.enable) { /* upper bank meter is used, single core in Petra */
            mirror_dest->policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, info.cmd.meter_ptr_up.value, 1, 0/*only one core in PetraB*/);
        }
    }
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        mirror_dest->flags |= (info.add_orig_head ? BCM_MIRROR_DEST_EGRESS_ADD_ORIG_SYSTEM_HEADER : 0);
                                  ...
                                  bcm_gport_t mirror_dest)
{
    ...
    if (flags & BCM_MIRROR_PORT_INGRESS) { /* handle inbound mirroring */
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_dflt_get((unit), core, pp_port, &dflt_mirroring_info));
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_dflt_set((unit), core, pp_port, &dflt_mirroring_info));
    }
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */

        /* check that the destination profile is the specified one */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_dflt_get((unit), core, pp_port,  &outbound_mirror_config));
        /* check retrieved destination validity */
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_dflt_set((unit), core, pp_port, &outbound_mirror_config));
    }
                                       ...
                                       bcm_gport_t mirror_dest)
{
    ...
    if (flags & BCM_MIRROR_PORT_INGRESS) { /* handle inbound mirroring */
        ...
        /* check that the destination profile is the specified one */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_vlan_get((unit), core, pp_port, vlan, &mirror_profile));
        if (!mirror_profile) {
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_vlan_remove((unit), core, pp_port, vlan));
    }
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */       

        /* check that the destination profile is the specified one */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_vlan_get((unit), core, pp_port, vlan,  &outbound_mirror_config));
        ...
        /* call dpp to remove the port-vlan mirroring */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_vlan_remove((unit), core, pp_port, vlan));
    }
                               ...
                               bcm_mirror_port_info_t *info)
{
    ...
    if (flags & (BCM_MIRROR_PORT_EGRESS | BCM_MIRROR_PORT_EGRESS_ACL)) {
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_info_get(unit, core, pp_port, &ppd_port_info));
                                           ...
                                           bcm_mirror_options_t options)
{
   ...
   uint32 action_profile_id;
    ...
    if (flags & BCM_MIRROR_PORT_INGRESS) { /* handle inbound mirroring */
        ...
        /* call dpp to set the port mirroring to the given profile */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_vlan_add((unit),core,
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */
        ...
        SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_mirror_port_vlan_add((unit), core,
          pp_port, vlan, &success, &outbound_mirror_config));
        if (success != SOC_SAND_SUCCESS) {
                                    ...
                                    bcm_mirror_options_t *options)
{
    ...
    *mirror_dest_count = 0;
    ...
    if (flags & BCM_MIRROR_PORT_INGRESS) { /* handle inbound mirroring */
        uint32 action_profile_id;
        /* get the destination profile */
        SOC_SAND_IF_ERR_RETURN(soc_ppd_llp_mirror_port_vlan_get((unit), core, pp_port, vlan, &action_profile_id));
        /* check retrieved destination validity */
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS) { /* handle outbound mirroring */
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_petra_is_mirror_dest_created(unit, outbound_mirror_config.mirror_command, &is_created));
        if (!is_created) { /* verify that the destination already exists */
            BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("retrieved mirroring destination ID %u is not created"), (unsigned)outbound_mirror_config.mirror_command));
        }
        if (*mirror_dest_count >= mirror_dest_size) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("output size is too small")));
        }
        BCM_GPORT_MIRROR_SET(mirror_dest[*mirror_dest_count], (bcm_gport_t)outbound_mirror_config.mirror_command);
        ++*mirror_dest_count;
        options->copy_strength = outbound_mirror_config.mirror_strength;
        options->forward_strength = outbound_mirror_config.forward_strength;
    }
