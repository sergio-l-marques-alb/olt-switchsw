...
/*SPD_ILLEGAL         57*/ { 0, QMOD_PMA_OS_MODE_5,  QMOD_PLL_MODE_DIV_40,  0, 0,  0}
...
int qmod_tx_lane_control(PHYMOD_ST* pc, int enable, tx_lane_disable_type_t tx_dis_type);
...
int qmod_pmd_lock_get(PHYMOD_ST* pc, int* lockStatus)
{
  PMD_X1_STSr_t  reg_pmd_x1_sts;
  QMOD_DBG_IN_FUNC_INFO(pc);
  READ_PMD_X1_STSr(pc, &reg_pmd_x1_sts);
...
#ifdef _SDK_QMOD_
int qmod_autoneg_set(PHYMOD_ST* pc,
    qmod_an_ability_t *an_ability_st) /* AUTONEG_SET */
{
   return PHYMOD_E_NONE;
}
#endif /* _SDK_QMOD_*/
...
<p>Call directly </p>
*/
int qmod_autoneg_control_get(PHYMOD_ST* pc, qmod_an_control_t *an_control) 
{
  return PHYMOD_E_NONE;
}
...
int qmod_autoneg_local_ability_get(PHYMOD_ST* pc, qmod_an_ability_t *an_ability_st)
{
  return PHYMOD_E_NONE;
}
...
int qmod_autoneg_remote_ability_get(PHYMOD_ST* pc, qmod_an_ability_t *an_ability_st)
{
  return PHYMOD_E_NONE;
}
...
#ifdef _SDK_QMOD_
int qmod_autoneg_control(PHYMOD_ST* pc, qmod_an_control_t *an_control)
{
  return PHYMOD_E_NONE;
} /* qmod_autoneg_control */
#endif /* _SDK_QMOD_*/
...
*/
int qmod_tx_loopback_control(PHYMOD_ST* pc, int enable, int starting_lane, int port_type)           /* TX_LOOPBACK_CONTROL  */
{
...
*/
int qmod_tx_lane_control(PHYMOD_ST* pc, int enable, tx_lane_disable_type_t tx_dis_type)         /* TX_LANE_CONTROL */
{
  ...
  if (enable) {
     ...
     /* data = 0 ;  */
     if(tx_dis_type & QMOD_TX_LANE_TRAFFIC ){
        MISCr_ENABLE_TX_LANEf_SET(reg_misc_ctrl, 0);
     } else if (tx_dis_type & QMOD_TX_LANE_RESET) {
...
*/
int qmod_revid_read(PHYMOD_ST* pc, int *revid)              /* REVID_READ */
{
...
int qmod_init_pcs(PHYMOD_ST* pc)              /* INIT_PCS */
{
  ...
  qmod_tx_lane_control(pc, 1, 0);         /* TX_LANE_CONTROL */
...
*/
int qmod_get_pcs_link_status(PHYMOD_ST* pc, int *link)
{
  return PHYMOD_E_NONE;
}
...
*/
int qmod_fecmode_set(PHYMOD_ST* pc, int fec_enable)              /* FECMODE_SET */
{
  return PHYMOD_E_NONE;
}
...
*/
 /*
  MAIN_LN_SWPr_SET(reg, pcs_map);
  MAIN_LN_SWPr_LOG0_TO_PHY_LNSWAP_SELf_SET(reg, pcs_map << 0);
  MAIN_LN_SWPr_LOG1_TO_PHY_LNSWAP_SELf_SET(reg, pcs_map << 2);
  MAIN_LN_SWPr_LOG2_TO_PHY_LNSWAP_SELf_SET(reg, pcs_map << 4);
  MAIN_LN_SWPr_LOG3_TO_PHY_LNSWAP_SELf_SET(reg, pcs_map << 6);
 */
...
*/
int qmod_pmd_lane_swap_tx(PHYMOD_ST *pc, uint32_t tx_lane_map_0, uint32_t tx_lane_map_1, uint32_t tx_lane_map_2, uint32_t tx_lane_map_3)
{
  ...
  tx_lane_map_0 = tx_lane_map_0 & 0xf;
  tx_lane_map_1 = tx_lane_map_1 & 0xf;
  tx_lane_map_2 = tx_lane_map_2 & 0xf;
  tx_lane_map_3 = tx_lane_map_3 & 0xf;
...
int _qmod_configure_sc_speed_configuration(PHYMOD_ST* pc)
{
  ...
  else if(pc->sc_mode == QMOD_SC_MODE_ST_OVERRIDE) {
  }
...
int qmod_credit_control(PHYMOD_ST* pc, int enable)   /* CHECK_SC_STATS */
{
  return PHYMOD_E_NONE;
}
#endif /* _DV_TB_ */
...
#ifdef _DV_TB_
int qmod_prbs_tx_enable_set(PHYMOD_ST* pc, int enable)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
#ifdef _DV_TB_
int qmod_prbs_rx_invert_data_set(PHYMOD_ST* pc, int invert_data)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
#ifdef _DV_TB_
int qmod_prbs_rx_check_mode_set(PHYMOD_ST* pc, int check_mode)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
#ifdef _DV_TB_
int qmod_prbs_tx_invert_data_set(PHYMOD_ST* pc, int invert_data)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
#ifdef _DV_TB_
int qmod_prbs_tx_polynomial_set(PHYMOD_ST* pc, prbs_polynomial_type_t prbs_polynomial)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
#ifdef _DV_TB_
int qmod_prbs_rx_polynomial_set(PHYMOD_ST* pc, prbs_polynomial_type_t prbs_polynomial)                /* PRBS_MODE */
{
  return PHYMOD_E_NONE;
...
int qmod_set_qport_spd(PHYMOD_ST* pc)
{
  ...
  switch(pc->sub_port) {
     case 0: SC_X4_QSGMII_SPDr_SPEED_SP0f_SET(reg_qsgmii_spd, pc->subp_speed);  break ;
     case 1: SC_X4_QSGMII_SPDr_SPEED_SP1f_SET(reg_qsgmii_spd, pc->subp_speed);  break ;
     case 2: SC_X4_QSGMII_SPDr_SPEED_SP2f_SET(reg_qsgmii_spd, pc->subp_speed);  break ;
     case 3: SC_X4_QSGMII_SPDr_SPEED_SP3f_SET(reg_qsgmii_spd, pc->subp_speed);  break ;
     default: break ;
  }
...
int qmod_disable_get (PHYMOD_ST *pc, uint32_t* enable)
{
  SC_X4_CTLr_t reg_sc_ctrl;
...
@details get port speed configured
*/
int qmod_speed_id_get ( PHYMOD_ST *pc,  int *speed_id) 
{
  ...
  READ_SC_X4_FI_SP1_RSLVD_SPDr(pc,&sc_final_sp1_resolved_speed);
  READ_SC_X4_FI_SP2_RSLVD_SPDr(pc,&sc_final_sp2_resolved_speed);
  READ_SC_X4_FI_SP3_RSLVD_SPDr(pc,&sc_final_sp3_resolved_speed);

  sp_spd_id = SC_X4_FI_SP0_RSLVD_SPDr_SPEEDf_GET(sc_final_sp0_resolved_speed);

  sp_spd_id = sp_spd_id ||
    (SC_X4_FI_SP1_RSLVD_SPDr_SPEEDf_GET(sc_final_sp1_resolved_speed) << 8);

  sp_spd_id = sp_spd_id ||
    (SC_X4_FI_SP2_RSLVD_SPDr_SPEEDf_GET(sc_final_sp2_resolved_speed) << 8);

  *speed_id = sp_spd_id || SC_X4_FI_SP3_RSLVD_SPDr_SPEEDf_GET(sc_final_sp3_resolved_speed);
...
*/
int qmod_set_spd_intf(PHYMOD_ST *pc, qmod_spd_intfc_type_t spd_intf, int no_trig)
{
  ...
  PHYMOD_IF_ERR_RETURN (_qmod_wait_sc_stats_set(pc));
  }
  return PHYMOD_E_NONE;
}
