...
int tscf_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tefmod_pmd_osmode_set(&phy->access, spd_intf, os_mode));
...
int tscf_phy_autoneg_ability_get(const phymod_phy_access_t* phy, phymod_autoneg_ability_t* an_ability_get_type){
    ...
    if ((TEFMOD_FEC_NOT_SUPRTD == value.an_fec) || (TEFMOD_FEC_SUPRTD_NOT_REQSTD == value.an_fec)) {
        an_ability_get_type->an_fec = 0;
    } else {
        an_ability_get_type->an_fec = TEFMOD_FEC_SUPRTD_REQSTD;
    }       
...
{
    uint16_t baseP1, baseP2, nextP1, nextP2;
    PHYMOD_IF_ERR_RETURN
       (tefmod_autoneg_lp_status_get(&phy->access, &baseP1, &baseP2, &nextP1, &nextP2));
    ...
    an_ability_get_type->an_fec = (baseP2 >> 14) & 3;
...
int tscf_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if ((num_lane == 4) || PHYMOD_AN_F_ALLOW_PLL_CHANGE_GET(an)){
        single_port_mode = 1;
    }        
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    phy_copy.access.lane_mask = 0x1 << start_lane;
