...
int _madura_phy_prbs_config_set(const phymod_phy_access_t* phy, uint32_t flags , const phymod_prbs_t* prbs)
{
    ...
    MADURA_GET_CORE_SLICE_INFO(pa, ip, config, &cast_type, &mcast_val);

    PHYMOD_DEBUG_VERBOSE(("IP:%s Max_lane:%d lanemask:0x%x cast Type:%x flags=%x\n", 
                (ip == MADURA_FALCON_CORE)?"FALCON":"FALCON_2X", max_lane, lane_mask, cast_type,flags));
...
int _madura_phy_prbs_enable_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t enable)
{
    ...
    PHYMOD_DEBUG_VERBOSE(("\n IP:%s side=%d Max_lane:%d lanemask:0x%x cast type:%d enable=%x\n", 
                (ip == MADURA_FALCON_CORE)?"FALCON":"FALCON_2X", side,max_lane, lane_mask, cast_type,enable));
        ...
        for (lane = 0; lane < max_lane; lane ++) {
            if (lane_mask & (1 << lane)) {/*if starts*/
                ...
                /*only for standalone to disable system side state!!*/
