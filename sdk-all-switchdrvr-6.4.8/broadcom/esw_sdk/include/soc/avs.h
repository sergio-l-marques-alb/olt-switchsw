...
typedef struct soc_avs_custom_xbmp_s{
    uint32 cent_xbmp[SOC_AVS_NUM_CENT_XBMP];
    uint32 rmt_xbmp[SOC_AVS_NUM_RMT_XBMP];    
}soc_avs_custom_xbmp_t;

typedef struct soc_avs_custom_margin_s{
    int32 vmargin_low;
    int32 vmargin_high;
}soc_avs_custom_margin_t;

typedef struct soc_avs_functions_s{
    int (*init)(int unit, soc_avs_info_t* avs_info ); /* initialization function */
    int (*voltage_set)(int unit,uint32 voltage); /* core voltage set function */
    int (*voltage_get)(int unit,uint32* voltage); /* core voltage get function */
    int (*deinit)(int unit); /* deinitialization function */
...
#endif
    soc_avs_custom_xbmp_t   *avs_custom_xbmp;
...
#define SOC_AVS_XBMP(unit)          (SOC_AVS_CONTROL(unit)->avs_custom_xbmp)
#define SOC_AVS_MARGIN(unit)        (SOC_AVS_CONTROL(unit)->avs_custom_margin)
...
#define SOC_AVS_INT(_x_) ((int32)((_x_)*SOC_AVS_S1))
...
extern int _soc_avs_predict_vpred(int unit, uint32 dac_code_low, uint32
...
extern int soc_avs_custom_margin_get(int unit, 
            soc_avs_custom_margin_t *avs_custom_margin);            
extern int soc_avs_custom_margin_set(int unit, 
            soc_avs_custom_margin_t *avs_custom_margin);            
extern int soc_avs_temperature_get(int unit, int32 *temperature);
