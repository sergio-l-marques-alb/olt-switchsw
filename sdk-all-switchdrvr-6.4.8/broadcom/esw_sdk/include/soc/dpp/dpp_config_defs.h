...
typedef struct soc_dpp_defines_s {
    ...
    uint32     soc_dpp_defs_nof_failover_ingress_ids;                 /* Number of available failover entries of type Ingress, the last two values are reserved upto Arad due to Facility protection constraints */
    uint32     soc_dpp_defs_failover_ingress_last_hw_id;              /* Last Ingress Failover ID that can be used for HW access. Also used by the drop per In-LIF mechanism. */
    ...
    uint32     soc_dpp_defs_fid_nof_bits;                             /* The length of fid in bits */
    uint32     soc_dpp_defs_fc_rx_mlf_hrf_max;                        /* Maximum number of valid entries in Rx High Rate FIFO */
    uint32     soc_dpp_defs_fc_rx_mlf_max;                            /* Maximum number of valid entries in Rx FIFO */
    uint32     soc_dpp_defs_scm_qp_tcg_cr_to_add_nof_bits;            /* The length in bits of the fields CR_TO_ADD of qpairs and TCG calendar */
    uint32     soc_dpp_defs_nof_epni_isid;                            /* Number of EPNI_ISID_TABLE entry */
    uint32     soc_dpp_defs_source_routed_cells_header_offset;        /* offset in the header fields of sr cells  (bytes) */
    uint32     soc_dpp_defs_egq_qdct_pd_max_val;                      /* Max value for PD reserved threshold */
    uint32     soc_dpp_defs_egr_prge_nof_prog_sel_tcam_entries;       /* No. of program selection TCAM entries */
    uint32     soc_dpp_defs_nof_out_vsi;                              /* Number of out vsis */
} soc_dpp_defines_t;
...
#define SOC_DPP_DEFS_MAX5(x0,x1,x2,x3,x4)                    SOC_DPP_DEFS_MAX2(SOC_DPP_DEFS_MAX2(SOC_DPP_DEFS_MAX2(SOC_DPP_DEFS_MAX2(x0, x1),x2),x3),x4)
#define SOC_DPP_DEFS_MAX(dpp_define)                 SOC_DPP_DEFS_MAX5(SOC_DPP_GET_QAX(dpp_define),SOC_DPP_GET_ARDON(dpp_define),SOC_DPP_GET_JERICHO(dpp_define),SOC_DPP_GET_PETRAB(dpp_define),SOC_DPP_GET_ARAD(dpp_define))
