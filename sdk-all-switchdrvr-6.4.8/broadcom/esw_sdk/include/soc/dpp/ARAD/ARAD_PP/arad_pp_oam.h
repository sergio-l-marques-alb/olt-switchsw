...
typedef struct
{
  uint16 ref_count[ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT_NUM]; 
} ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT;

typedef struct
{
  ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT ref_count;
...
} ARAD_PP_OAM_ACTION_KEY_PARAMS;

typedef enum {
    JER_PP_OAM_API_DMA_EVENT_TYPE_EVENT,
    JER_PP_OAM_API_DMA_EVENT_TYPE_STAT_EVENT,

    JER_PP_OAM_API_DMA_EVENT_TYPE_NOF
} JER_PP_OAM_API_DMA_EVENT_TYPE;


/* Callback type for event handlers */
typedef int (*dma_event_handler_cb_t)(int, JER_PP_OAM_API_DMA_EVENT_TYPE, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA *) ;
...
typedef struct {
    ...
    uint8                           tx_period;            /* Tx period (msec or kpps). 0 indicates one shot. */
    uint8                           is_period_in_kpps;    /* units for tx_period */
 ...
 * INPUT:
 *   SOC_SAND_IN  int                   unit -
 *   SOC_SAND_IN  JER_PP_OAM_API_DMA_EVENT_TYPE event_type - for statistics in Jericho
 * REMARKS:
