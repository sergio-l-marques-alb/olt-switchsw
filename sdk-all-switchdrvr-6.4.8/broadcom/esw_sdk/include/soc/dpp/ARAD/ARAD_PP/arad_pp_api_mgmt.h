 ...
 *   - commit,cache function can be called only with ARAD_PP_FRWRD_IP_ALL_VRFS_ID
 * if FALSE then each VRF use seperate memory for routes management 
 *   - the maximum routes for VRF x is max_routes_in_vrf[x]
 *   - cache function can be called only with ARAD_PP_FRWRD_IP_ALL_VRFS_ID
...
typedef struct
{
  ...
  /* 
   *  MPLS coupling enables lookup of two mpls copuled.
   */ 
  uint8 mpls_coupling_enable;
  /*
