/*
 *         
 * $Id: tsce16_diagnostics.c,v 1.0 2016/09/27 12:00:00 Exp $
 * 
...
int tsce16_phy_pmd_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    ...
    } else if (type==TEMOD16_DIAG_DSC_STD) {
        PHYMOD_DIAG_OUT(("    +--------------------------------------------------------------------+\n"));
...
int tsce16_phy_pcs_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY        (&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN (temod16_diag_disp(&phy_copy.access, (char*)arg));
