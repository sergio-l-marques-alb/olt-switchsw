  ...
  for (i = 0; i < device->block_num; i++)
  {
    ...
    for (k = 0; k < cur_pp_block->stage_num; k++)
    {
      ...
      for (j = 0; j < cur_pp_stage->number; j++, qual_signal++)
      {
        ...
        {
          sprintf(qual_name, "%s.Offset-%db.Size-%db", qual_signal->name, qual_signal->offset - offset, size);
          return _SHR_E_NONE;
        }
      }
    }
...
static void pp_dump_parser_print_single(int unit, void *curTop, int depth, int base, int macro, int first, int last, int pfc, int plc)
{
    uint32         res = CMD_OK;
    char *entry_n;
...
static uint32 pp_dump_parser_programs(int unit, void *curTop, int depth)
{
    int i_entry, i_seg;
    ...
    int flags)
{
    ...
    if(device == NULL)
    {
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "NULL Device\n")
    }
    PRT_TITLE_SET("Qualifiers List");
