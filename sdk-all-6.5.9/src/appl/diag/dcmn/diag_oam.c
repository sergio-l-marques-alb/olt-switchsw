...
void parse_diag_oam_counters_oamp_set_opts(int unit, args_t *a, char *type_str, char *filter_str, int *value)
{
    ...
    if (next_arg && (!sal_strncasecmp(next_arg, "type=", 5))) {
        len=strlen(next_arg);
        sal_strncpy(type_str, next_arg+5, len-5);
    }

    next_arg= ARG_GET(a);
    if (next_arg && (!sal_strncasecmp(next_arg, "filter=", 7))) {
        len=strlen(next_arg);
        sal_strncpy(filter_str, next_arg+7, len-7);
    }

    next_arg= ARG_GET(a);
    if (next_arg && (!sal_strncasecmp(next_arg, "value=", 6))) {
...
void parse_diag_oam_counters_oamp_opts(int unit, args_t *a, int *set , int *show , int *help , int *oamp, int *set_type, int *set_filter, int *set_value)
{
    ...
    if (next_arg && (!sal_strncasecmp(next_arg, "set", 3))) {
        *set = 1;
    ...
    } else if (next_arg && (!sal_strncasecmp(next_arg, "show", 4))) {
        *show = 1;
    } else if (next_arg && (!sal_strncasecmp(next_arg, "help", 4))) {
        *help = 1;
    } else {
        *oamp = 1;
    } 
