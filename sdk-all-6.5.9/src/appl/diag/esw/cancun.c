...
static const char* soc_cancun_version_prefix_string[CANCUN_SOC_FILE_LOAD_STATUS_NUM] =
                   CANCUN_VERSION_PREFIX_INITIALIZER;
...
cmd_result_t _cmd_cancun_status(int unit) {
    ...
    if(cc->flags & SOC_CANCUN_FLAG_DEBUG_MODE) {
        ...
        cli_out("Skip Ver Match: %s\n",
                (cc->flags & SOC_CANCUN_FLAG_SKIP_VERSION_MATCH)? "ON": "OFF");
        if (soc_feature(unit,soc_feature_flex_flow)) {
...
void _cancun_version_string_get(char* ver_string, uint32 ver) {
    ...
    /* Version flags decode */
    if(ver & CANCUN_VERSION_FLAG_TEST) {
        sal_strcpy(p, "*");
    }

    /* Version type and number decode */
    ver &= CANCUN_VERSION_MASK;

    num = (ver >> CANCUN_VERSION_OFFSET_PREFIX) & 0xF;
    if(num >= CANCUN_VERSION_PREFIX_NUM) {
        sal_strcat(p, soc_cancun_version_prefix_string \
                   [CANCUN_VERSION_PREFIX_INVALID]);
        goto _end_cancun_version_string_get;
    }
    sal_strcat(p, soc_cancun_version_prefix_string[num]);
    sal_strcat(p, " ");
    ...
    num = (ver >> CANCUN_VERSION_OFFSET_BUILD) & 0xFF;
    sal_itoa(buf_str, num, 10, 0, 2);
    sal_strcat(p, buf_str);
    sal_strcat(p, ".");

    num = (ver >> CANCUN_VERSION_OFFSET_CANCUN) & 0xFF;
    sal_itoa(buf_str, num, 10, 0, 2);
    sal_strcat(p, buf_str);

_end_cancun_version_string_get:
    sal_strcat(p, "\0");
}

void _soc_cancun_default_filename(int unit, char *filename, int file_type) {
#ifndef NO_FILEIO
#define _SOC_CANCUN_DEFAULT_PATHS_NUM   (6)
