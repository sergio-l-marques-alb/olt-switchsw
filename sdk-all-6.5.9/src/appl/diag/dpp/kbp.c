...
/*Enables/Disables KAPS hitbits per DBAL table*/
STATIC cmd_result_t
cmd_dpp_kaps_hitbit_table_toggle(int unit, args_t *a)
{
    int rv = CMD_OK,
        enable = 0;
    parse_table_t pt;
    ...
    if (!SOC_IS_JERICHO_PLUS(unit)) {
        cli_out("Error(%d): KAPS HITBIT is only available in Jericho Plus/QAX/QUX and above\n", unit);
        return CMD_FAIL;
    }
    parse_table_init(0, &pt);

    if (ARG_CNT(a) > 0) {
        parse_table_add(&pt, "Table", PQ_STRING, (void *)0, &table_name, NULL);
        parse_table_add(&pt, "Enable", PQ_INT, (void *)0, &enable, NULL);
    ...
    /*Find a table that matches the provided name*/
    for (table_id = 0; table_id < SOC_DPP_DBAL_SW_NOF_TABLES; table_id++) {
        rv = sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, table_id, &dbal_table);
        if (rv != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Failed to get dbal table id %d.\n"), table_id));
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }

        if (dbal_table.is_table_initiated) {
            rv = sal_strcasecmp(dbal_table.table_name, table_name);
            if (rv == 0) {
                break;
            }
        }
    ...
    if (table_id == SOC_DPP_DBAL_SW_NOF_TABLES) {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Failed to find initialized dbal table name matching %s.\n"), table_name));
        parse_arg_eq_done(&pt);
        return CMD_FAIL;
    }
    ...
    if ((enable != 0) && (enable != 1)) {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Enable must be either 1 (enable) or 0 (disable).\n")));
        return CMD_FAIL;
    }

    if (dbal_table.additional_table_info == enable) {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Table %s hitbits already set to enable=%d.\n"), dbal_table.table_name, enable));
        return CMD_FAIL;
    }

    /*Clear table to make sure no memory leaks when switching KAPS table hitbit modes*/
    rv = arad_pp_dbal_table_clear(unit, table_id);
    if (rv != SOC_SAND_OK) {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Failed to clear KAPS table before switching HITBIT mode.\n")));
        return CMD_FAIL;
    }

    dbal_table.additional_table_info = enable;

    rv = sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.set(unit, table_id, &dbal_table);
    if (rv != SOC_SAND_OK) {
        LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META_U(unit, "Error: Failed to set dbal table id %d.\n"), table_id));
        return CMD_FAIL;
    }

    cli_out("Unit %d: Successfully %s KAPS hitbits for DBAL table: %s (%d) \n", unit, enable ? "enabled" : "disabled", dbal_table.table_name, table_id);
...
*/
        ...
        else if(!sal_strcasecmp(func, "kaps_hitbit")) {
            return cmd_dpp_kaps_hitbit_table_toggle(unit, a);
        }
        else {
