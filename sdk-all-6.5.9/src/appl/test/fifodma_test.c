...
static uint32
fifodma_soc_chk_big_endian(int unit)
{
    uint32 big_endian = 0;
/* 
    int big_pio, big_packet, big_other;
    if (fifodma_data.param_debug) {
        (void) soc_endian_get(unit, &big_pio, &big_packet, &big_other);
        cli_out("\nsoc_endian_get big_pri %0d, big_pkt %0d, big_other %0d",
                big_pio, big_packet, big_other);
    } */

    /* CMICX L2_MOD_FIFO is big endian */
#if defined(BCM_TRIDENT3_SUPPORT) && defined(BCM_CMICX_SUPPORT)
    if (soc_feature(unit, soc_feature_cmicx) && SOC_IS_TRIDENT3X(unit)) {
        big_endian = 1;
    }
#endif
    cli_out("\nL2_MOD_FIFOm endianless %d", big_endian);
                           ...
                           unsigned int start_index, unsigned int num_entries)
{
    ...
    big_endian = fifodma_soc_chk_big_endian(unit);
