...
/* #define FRAG_INDEX_SIZE 32768 */
#define FRAG_INDEX_SIZE 1000
...
typedef struct sbt_mem_s {
    ...
    int             count;
    int             frag_count;
    int             frag_index_max[FRAG_INDEX_SIZE];
    int             frag_index_min[FRAG_INDEX_SIZE];
...
const char sb_test_usage[] =
    ...
    "SlamEnable=<1/0>    -  slam enable; default is 0\n"
    "SlamLowToHigh=<1/0> -  slam direction; default is LowToHigh 1\n"
    "CheckData=<1/0>     -  check data integrity; default is 1\n"
    ;
#endif


static int
sb_parse_test_params(int unit, args_t *a)
{
    parse_table_t pt;
    int count_arg;
    int i, n;
    ...
    sb_test_mem[0].count = soc_mem_index_count(unit, L2Xm);
    ...
    cli_out("\nDesc          = %0d",  sb_test_data.desc_mode);
    cli_out("\nNumMem        = %0d",  sb_test_data.num_mem);
    cli_out("\nMem[0]        = %s",   sb_test_mem[0].mem_str);
    /* cli_out("\nStart         = %0d",  sb_test_mem[0].index_min); */
    /* cli_out("\nCount         = %0d",  count_arg); */
    cli_out("\nFrag          = %0d",  sb_test_mem[0].frag_count);
    /* cli_out("\nRateCalcInt   = %0d",  sb_test_data.rate_calc_int); */
    /* cli_out("\nSeed          = %0d",  sb_test_data.rnum); */
    cli_out("\nEccAsData     = %0d",  sb_test_data.ecc_as_data);
    cli_out("\nCmcChanBitmap = 0x%0x",sb_test_data.cmc_ch_bm);
    cli_out("\nDmaWrEn       = %0d",  sb_test_data.dma_wr_en);
    cli_out("\nDmaRdEn       = %0d",  sb_test_data.dma_rd_en);
    cli_out("\nTestMode      = %0d",  sb_test_data.test_mode);
    /* cli_out("\nVerbose       = %0d",  sb_test_data.verbose); */
    /* cli_out("\ndebug         = %0d",  sb_test_data.debug); */
    /* cli_out("\nSlamEnable    = %0d",  sb_test_data.slam_en); */
    /* cli_out("\nSlamLowToHigh = %0d",  sb_test_data.slam_dir); */
    cli_out("\nCheckData     = %0d",  sb_test_data.check_data);
    cli_out("\n -----------------------------------------------------\n\n");

    cli_out("CMIC INFO: SOC_PCI_CMCS_NUM       = %0d\n", SOC_PCI_CMCS_NUM(unit));
    cli_out("CMIC INFO: SOC_SBUSDMA_CH_PER_CMC = %0d\n", SOC_SBUSDMA_CH_PER_CMC);
    cli_out("CMIC INFO: SOC_SBUSDMA_CH_MAX     = %0d\n\n", SOC_SBUSDMA_CH_MAX);
    ...
    if (soc_feature(unit, soc_feature_cmicm) &&
        ...
        if ((sb_test_data.num_mem < 1) ||
            (sb_test_data.num_mem > SOC_SBUSDMA_CH_MAX) ||
    ...
    for (n = 0; n < sb_test_data.num_mem; n++) {
        ...
        if (sb_test_mem[n].frag_count <= sb_test_data.num_mem) {
            ...
            if (sb_test_mem[n].frag_count > FRAG_INDEX_SIZE) {
                sb_test_mem[n].frag_count = FRAG_INDEX_SIZE;
                cli_out("Reduced the fragment count to %d\n",
                        sb_test_mem[n].frag_count);
            }
        ...
        cli_out("TEST INFO: Mem[%0d] ", n);
        cli_out("name = %s ", sb_test_mem[n].mem_str);
        cli_out("copyno = %0d ", sb_test_mem[n].copyno);
        cli_out("index_count = %0d ", sb_test_mem[n].count);
        cli_out("index_min = %0d ", sb_test_mem[n].index_min);
        cli_out("index_max = %0d ", sb_test_mem[n].index_max);
        cli_out("frag_count = %0d ", sb_test_mem[n].frag_count);
        cli_out("frag_index_min = %0d ", sb_test_mem[n].frag_index_min[0]);
        cli_out("frag_index_max = %0d ", sb_test_mem[n].frag_index_max[0]);
        cli_out("\n");
    }
    ...
    cli_out("TEST INFO: cmc_ch_bm  = 0x%0x\n", sb_test_data.cmc_ch_bm);
...
sb_test_write(int unit)
{
    ...
    int n, done = FALSE;
    char str[80];
    sal_thread_t pid_sbusdma[3 * SOC_SBUSDMA_CH_PER_CMC];
...
sb_test_main(int unit, args_t *a, void *pa)
{
    int rv = SOC_E_NONE;
    int n;
    if (sb_test_data.bad_input == 1) {
        return BCM_E_FAIL;
    }

    /* reserve all channels */
    if (sb_test_data.dma_wr_en || sb_test_data.dma_rd_en) {
    ...
    if (sb_test_data.check_data == TRUE) {
        for (n = 0; n < sb_test_data.num_mem; n++) {
            if (sb_test_verify(unit, n) != SOC_E_NONE) {
                rv = BCM_E_FAIL;
                test_error(unit, "SBUS DMA failed: inconsistency observed\n");
                if (!sb_test_data.debug) {
                    sb_test_dump_data(unit, n, TD_DEFAULT_DUMP_ENTRY_COUNT);
...
sb_test_init(int unit, args_t *a, void **pa)
{
    ...
    assert((SOC_PCI_CMCS_NUM(unit) * SOC_SBUSDMA_CH_PER_CMC <= SOC_SBUSDMA_CH_MAX));

    if (sb_parse_test_params(unit, a) != BCM_E_NONE) {
        return BCM_E_FAIL;
    }

    /* set sb_test_mem[n].frag_count */
    frag = 0;
    for (i = 0; i < SOC_SBUSDMA_CH_MAX; i++) {
        if (sb_test_data.cmc_ch_bm & (0x1 << i)) {
            LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Testing %0d-th bit CMC %0d channel %0d\n"),
                             i, i / SOC_SBUSDMA_CH_PER_CMC,
    ...
    for (n = 1; n < sb_test_data.num_mem; n++) {
        sb_test_mem[n].frag_count = 1;
    }
    cli_out("TEST INFO: num_mem    = %0d\n", sb_test_data.num_mem);
    cli_out("TEST INFO: ch_count   = %0d\n", frag);
    cli_out("TEST INFO: frag_count = %0d\n\n", sb_test_mem[0].frag_count);
    ...
    /* construct sb_test_mem[n].mem_cmc_ch_bm and sb_test_data.ch_mem_map[n] */
    frag = 0;
    for (i = 0; i < SOC_SBUSDMA_CH_MAX; i++) {
    ...
    /* check if dma is supported by test memory */
    for (n = 0; n < sb_test_data.num_mem; n++) {
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
