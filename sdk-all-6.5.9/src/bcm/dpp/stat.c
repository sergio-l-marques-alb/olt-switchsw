    ...
    bcm_stat_counter_config_t *config)
{
    ...
    if (SOC_IS_QAX(unit) && config->source.pointer_range.start == 0) {
    /* HLM supported devices cannot use first counter processor range for OAM */
       if (config->source.engine_source ==  bcmStatCounterSourceIngressOam) {
           BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("First counter proccessor range cannot be used for source bcmStatCounterSourceIngressOam.")));
       }
       if (config->source.engine_source ==  bcmStatCounterSourceEgressOam){
           BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("First counter proccessor range cannot be used for source bcmStatCounterSourceEgressOam.")));
       }
    }
