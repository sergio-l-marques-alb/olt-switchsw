...
bcm_petra_knet_cleanup(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_knet_filter_traverse(unit, _petra_trav_filter_clean, NULL));
...
bcm_petra_knet_netif_create(int unit, bcm_knet_netif_t *netif)
{
    ...
    if (netif->type == BCM_KNET_NETIF_T_TX_CPU_INGRESS) {
        netif_create.netif.ptch[0] = 0xd0;
        netif_create.netif.ptch[1] = netif->port;
    }
        ...
        BCMDNX_IF_ERR_EXIT(bcm_petra_stk_gport_sysport_get(unit, netif->port, &sysport));
        if (SOC_IS_JERICHO(unit)) {
            /* DPP_HDR_ITMH_FWD_TYPE_UNICAST_DIRECT */
            itmh.base.jer_dest_info.dest_info_hi = 0x1;
            itmh.base.jer_dest_info.dest_info_mi = (BCM_GPORT_SYSTEM_PORT_ID_GET(sysport) >> 8) & 0xff;
            itmh.base.jer_dest_info.dest_info_lo = BCM_GPORT_SYSTEM_PORT_ID_GET(sysport) & 0xff;
        } else {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported device type\n")));
        }

        netif_create.netif.ptch[0] = 0x50;
        netif_create.netif.ptch[1] = 0x0;
        sal_memcpy(netif_create.netif.itmh, &itmh.base.raw.bytes[0], DPP_HDR_ITMH_BASE_LEN);
    }
...
bcm_petra_knet_filter_create(int unit, bcm_knet_filter_t *filter)
{
    ...
    if (filter->match_flags & BCM_KNET_FILTER_M_REASON) {
        ...
        if ((reason != 0x20) && (reason_hi != 0x8)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported Rx reason %x reason_hi %x\n")));
        }
        if ((reason == 0x20) && (reason_hi == 0x8)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported Rx reason Mirror and Snoop at the same time\n")));
        }
    }
    if (filter->match_flags & BCM_KNET_FILTER_M_SRC_MODPORT) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported filter to match source module port\n")));
    }
    if (filter->match_flags & BCM_KNET_FILTER_M_SRC_MODID) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported filter to match source module id\n")));
    }
    if (filter->match_flags & BCM_KNET_FILTER_M_ERROR) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("KNET: Unsupported filter to match error bit\n")));
    }
    ...
    if (SOC_DCB_TYPE(unit) == 28) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_INGPORT) {
            uint32 source_system_port_aggregate = 0;
            uint32 source_system_port_aggregate_mask = 0;
            ...
            if (BCM_GPORT_IS_TRUNK(filter->m_ingport)) {
                ...
                source_system_port_aggregate_mask =0x0;
                source_system_port_aggregate_mask |= ((lag_port_member_id_mask << sys_lag_port_id_nof_bits) & (SOC_SAND_BIT(13) - SOC_SAND_BIT(sys_lag_port_id_nof_bits) + SOC_SAND_BIT(13)));
            } else {
                bcm_gport_t modport;
                bcm_module_t modid;
                bcm_port_t port;
		 ...
		 BCMDNX_IF_ERR_EXIT(bcm_petra_port_gport_get(unit, filter->m_ingport, &modport));
                modid = BCM_GPORT_MODPORT_MODID_GET(modport);
                port = BCM_GPORT_MODPORT_PORT_GET(modport);
                bcm_trunk_find(unit, modid, port, (bcm_trunk_t *)&lag_id);
                if (lag_id != BCM_TRUNK_INVALID) {
                    int index;
                    is_lag_not_phys = 1;
                    BCMDNX_IF_ERR_EXIT(bcm_petra_trunk_get(unit, lag_id, &trunk_info, max_member_count, trunk_port_members, &member_count));
                    for (index = 0; index < member_count; index++)
                    {
                        if (modport == trunk_port_members[index].gport)
                        {
                    ...
                    bcm_gport_t sysport;
                    is_lag_not_phys =0;
		     BCMDNX_IF_ERR_EXIT(bcm_petra_stk_gport_sysport_get(unit, modport, &sysport));
                    sys_phys_port_id = BCM_GPORT_SYSTEM_PORT_ID_GET(sysport);
                }
            }
            arad_ports_logical_sys_id_build(unit, is_lag_not_phys, (uint32)lag_id, lag_port_member_id, sys_phys_port_id, &source_system_port_aggregate);
            dcb_data->spa = source_system_port_aggregate;
            dcb_mask->spa = source_system_port_aggregate_mask;
        }
        if (filter->match_flags & BCM_KNET_FILTER_M_REASON) {
            if (reason){
...
bcm_petra_knet_filter_get(int unit, int filter_id, bcm_knet_filter_t *filter)
{
    ...
    if (dcb_mask->spa & 0xffff) {
        filter->match_flags |= BCM_KNET_FILTER_M_INGPORT;
        filter->m_ingport = dcb_data->spa;
    }
