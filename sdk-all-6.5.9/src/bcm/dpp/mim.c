    ...
    bcm_mim_port_t * mim_port)
{
    ...
    if (spb_is_supported) {
        if (((soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 1) == 0) && (mim_port->network_group_id > 1)) || (mim_port->network_group_id > 3)) {
           BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG(
               "network_group_id valid values are 0-3 for split_horizon_forwarding_groups_mode=1/2 and 0-1 for split_horizon_forwarding_groups_mode=0")));
        }
    } else {
