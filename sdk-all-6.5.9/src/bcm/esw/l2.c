...
/*
 * Function:
 *    bcmi_l2_addr_tsn_sr_flowset_ref_dec
 * Purpose:
 *    Decrease reference count for the specified TSN/SR flowset
 *    associated it to a L2 deletion
 * Parameters:
 *    unit      - unit number.
    ...
    uint32 *entry_del)
{
    ...
    int l2x_mode = SOC_CONTROL(unit)->l2x_mode;
    ...
    if (soc_feature(unit, soc_feature_tsn)) {
        if (l2addr_del.tsn_flowset != 0) {
            ...
            if (l2x_mode == L2MODE_POLL) {
                /* Clear shadow copy for TSN_CIRCUIT_IDf */
                soc_L2Xm_field32_set(unit, (l2x_entry_t *)entry_del,
                                     TSN_CIRCUIT_IDf, 0);
            }
        }
    }
    ...
    if (soc_feature(unit, soc_feature_tsn_sr)) {
        if (l2addr_del.sr_flowset != 0) {
            ...
            if (l2x_mode == L2MODE_POLL) {
                /* Clear shadow copy for SR_FLOW_IDf */
                soc_L2Xm_field32_set(unit, (l2x_entry_t *)entry_del,
                                     SR_FLOW_IDf, 0);
            }
        }
                          ...
                          void *fn_data)
{
    ...
    if (pflags & SOC_L2X_ENTRY_TSN_SR_FLOWSET_REF_CNT) {
        if (soc_feature(unit, soc_feature_tsn)) {
            if (entry_del != NULL) {
                (void)bcmi_l2_addr_tsn_sr_flowset_ref_dec(unit,
                                                          (uint32 *)entry_del);
            }
        }
    } else
