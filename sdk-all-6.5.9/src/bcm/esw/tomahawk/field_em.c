                                   ...
                                   uint32 *abuf)
{
    ...
    BCM_IF_ERROR_RETURN(soc_profile_mem_get(unit,
                             ...
                             *ap_idx, 1, entries));
        /* for TH and TH2 EM, policy_data needs
           to be filled from LSB to MSB */
        act_offset = 0;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if(soc_feature(unit, soc_feature_td3_style_fp)) {
        mem = EXACT_MATCH_DOUBLEm;
        mem_wide = EXACT_MATCH_QUADm;
...
_bcm_field_td3_em_key_attributes_init(int unit, _field_stage_t *stage_fc)
{
    ...
    int idx;
    soc_mem_t mem;                      /* Memory Identifier.        */
    static soc_mem_t em_key_attributes_mem[3] = {
        EXACT_MATCH_KEY_ATTRIBUTES_PIPE0m,
        EXACT_MATCH_KEY_ATTRIBUTES_PIPE1m,
        EXACT_MATCH_KEY_ATTRIBUTESm
    }; /* EM KEY Attributes Memory Identifier.        */
    ...
    for (idx = 0; idx < 3; idx++) {
        ...
        /* Mode 128: entry 0 */
        /* Read entry into SW buffer. */
        BCM_IF_ERROR_RETURN(soc_mem_read(unit, em_key_attributes_mem[idx],
                                    MEM_BLOCK_ANY, 0, em_key_attr_entry));

        /* Mode 128: Set BUCKET_MODE. */
        soc_mem_field32_set(unit, em_key_attributes_mem[idx],
        ...
        /* Install entry in hardware. */
        BCM_IF_ERROR_RETURN(soc_mem_write(unit, em_key_attributes_mem[idx],
