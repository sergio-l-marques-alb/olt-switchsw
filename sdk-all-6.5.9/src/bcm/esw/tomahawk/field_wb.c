...
_field_em_group_entries_sync(int unit, _field_group_t *fg)
{
    ...
    static soc_mem_t em_td3_entry_nar_mem[_FP_MAX_NUM_PIPES+1] = {
        EXACT_MATCH_DOUBLE_PIPE0m,
        EXACT_MATCH_DOUBLE_PIPE1m,
        INVALIDm,
        INVALIDm,
        EXACT_MATCH_DOUBLEm
    };
    static soc_mem_t em_td3_entry_wide_mem[_FP_MAX_NUM_PIPES+1] = {
        EXACT_MATCH_QUAD_PIPE0m,
        EXACT_MATCH_QUAD_PIPE1m,
        INVALIDm,
        INVALIDm,
        EXACT_MATCH_QUADm
    };
    ...
    if (!soc_feature(unit, soc_feature_td3_style_fp)) {
        ...
        em_nar_mem = em_td3_entry_nar_mem;
        em_wide_mem = em_td3_entry_wide_mem;
    }
        ...
        while  (count  != (num_attr * ent_count)) {
            ...
            if (SOC_IS_TOMAHAWKX(unit)) {
                ...
                rv = _bcm_field_th_val_get(f_ent->tcam.key_hw, tbuf,
                                          TD3_EM_MODE320_KEY_PART0_SIZE, 57);
                BCM_IF_ERROR_CLEANUP(rv);
                ...
                rv = _bcm_field_th_val_get(f_ent_part->tcam.key_hw, tbuftemp, 0, 48);
                BCM_IF_ERROR_CLEANUP(rv);
                rv = _bcm_field_th_val_set(tbuf, tbuftemp, 57, 48);
                BCM_IF_ERROR_CLEANUP(rv);
                soc_mem_field_set(unit, mem, bufp, MODE320__KEY_1_ONLYf, tbuf);
                rv = _bcm_field_th_val_get(f_ent_part->tcam.key_hw, tbuf, 48,
                                                 TD3_EM_MODE320_KEY_PART2_SIZE);
                BCM_IF_ERROR_CLEANUP(rv);
                soc_mem_field_set(unit, mem, bufp, MODE320__KEY_2_ONLYf, tbuf);
                rv = _bcm_field_th_val_get(f_ent_part->tcam.key_hw, tbuf, 153, 7);
                BCM_IF_ERROR_CLEANUP(rv);
                soc_mem_field_set(unit, mem, bufp, MODE320__KEY_3_ONLYf, tbuf);
            }
            ...
            case bcmFieldActionMirrorEgress:
                ...
                for (mirror_id = 0; mirror_id < BCM_MIRROR_MTP_COUNT;
                    ...
                    if (hw_idx_array[hw_count] != -1) {
                        fa_temp->hw_index = hw_idx_array[hw_count];
                        if (fa_temp ->next != NULL) {
...
_field_entry_recover(int unit, _field_group_t *new_fg)
{
    ...
    uint32 act_data[2] = {0};             /* Action data */
