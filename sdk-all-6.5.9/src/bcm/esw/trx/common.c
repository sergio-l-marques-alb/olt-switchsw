                              ...
                              bcm_port_t port, int tunnel_term)
{
    ...
    my_station_profile_1_entry_t entry_prof;
#if defined(BCM_TRIDENT3_SUPPORT)
    my_station_tcam_entry_t *my_station_shadow_2 =
        (my_station_tcam_entry_t *)info->my_station2_shadow;
#endif
    ...
    if (soc_feature(unit, soc_feature_mysta_profile)) {
        ...
        entry_2 = (uint32 *)&my_station_shadow_2[index];

        if (soc_feature(unit, soc_feature_mysta_profile)) {
            uint32 dest_type;
            old_pid = soc_mem_field32_dest_get(unit, mem, entry_2,
                    DESTINATIONf, &dest_type);
            if (dest_type != SOC_MEM_FIF_DEST_MYSTA) {
                soc_mem_unlock(unit, mem);
                return BCM_E_NOT_FOUND;
            }
            rv = _bcm_l2_mysta_profile_entry_get(unit, &entry_prof, 1, old_pid);
            if (BCM_FAILURE(rv)) {
                soc_mem_unlock(unit, mem);
                return rv;
            }
            ...
            rv = _bcm_l2_mysta_profile_entry_delete(unit, 1, old_pid);
        }

        /* Delete station tcam 2 */
        entry_2 = (uint32 *)&my_station_shadow_2[index];
        sal_memset(entry_2, 0, sizeof(my_station_tcam_entry_t));
        if (BCM_SUCCESS(rv)) {
            rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, index, entry_2);
        }
        soc_mem_unlock(unit, mem);
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_mysta_profile)) {
        ...
        for (index = 0; index < num_entries; index++) {
            ...
            if (i == entry_words) {
                ...
                if (soc_feature(unit, soc_feature_mysta_profile)) {
                    uint32 new_pid;
                    rv = _bcm_l2_mysta_profile_entry_add(unit, &entry_prof, 1, &new_pid);
                    if (BCM_SUCCESS(rv)) {
                        soc_mem_field32_dest_set(unit, mem, entry, DESTINATIONf,
                                SOC_MEM_FIF_DEST_MYSTA, new_pid);
                        rv = _bcm_l2_mysta_profile_entry_delete(unit, 1, old_pid);
