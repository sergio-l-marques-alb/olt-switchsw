                                                ...
                                                int stm_index)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, OUTER_TPID_VERIFYf)) {
        soc_LPORT_TABm_field32_set(unit, &lport_entry, OUTER_TPID_VERIFYf, 1);
    } else {
...
_bcm_coe_subtag_subport_default_egr_lport_entry_set(int unit, int port)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {  
        LOG_CLI((BSL_META_U(unit,
                        "Subport Ports: TODO pri_unmap set\n")));
        return BCM_E_NONE;
    }
                                              ...
                                              int arg)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) { 
        /* subport TPID program requires CCH mappings,will be enabled 
          after CCH development */
        return BCM_E_NONE;
    }
