...
/*
 * Function:
 *      _bcm_esw_portctrl_ingress_dest_enable
 * Purpose:
 *      Enables/disables all forwarding to ports in the last stage
 ...
 */
STATIC int
_bcm_esw_portctrl_ingress_dest_enable(int unit, bcm_pbmp_t pbmp, int enable)
{
    ...
    if (enable) {
        BCM_PBMP_OR(dest_e_pbmp, pbmp);
    } else {
        BCM_PBMP_REMOVE(dest_e_pbmp, pbmp);
    }
...
STATIC int
_bcm_esw_portctrl_ingress_dest_port_enable(int unit, bcm_port_t port, int enable)
{
    bcm_pbmp_t pbmp;
    BCM_PBMP_PORT_SET(pbmp, port);
    return _bcm_esw_portctrl_ingress_dest_enable(unit, pbmp, enable);
}
                             ...
                             int flags, int enable)
{
    ...
    if (enable) {
        ...
        if (soc_feature(unit, soc_feature_ingress_dest_port_enable)) {
            BCM_IF_ERROR_RETURN
                (_bcm_esw_portctrl_ingress_dest_port_enable(unit, port, 1));
        }
    } else {
        /*
         * PORT DOWN
        ...
        if (soc_feature(unit, soc_feature_ingress_dest_port_enable)) {
            BCM_IF_ERROR_RETURN
                (_bcm_esw_portctrl_ingress_dest_port_enable(unit, port, 0));
        }
        ...
        PBMP_ITER(lcl_pbmp, port) {
            ...
            BCM_IF_ERROR_RETURN
                (bcmi_esw_portctrl_probe(unit, port, 
                        PORTMOD_PORT_ADD_F_INIT_CORE_PROBE, port_add_info, &okay));
            count++;
        }
...
bcmi_esw_portctrl_loopback_set(int unit, bcm_gport_t port, int loopback)
{
    ...
    if (!is_loopback_set) {
        ...
        if (local_port == -1) {
            ...
            if (soc_flag) {
                if ((loopback == BCM_PORT_LOOPBACK_MAC) ||
                       ((loopback == BCM_PORT_LOOPBACK_NONE) && (mac_lpbk == BCM_PORT_LOOPBACK_MAC))) {
                       BCM_IF_ERROR_RETURN(soc_egress_enable(unit, pport, 0));
                       BCM_IF_ERROR_RETURN(soc_port_credit_reset(unit, pport));
                       BCM_IF_ERROR_RETURN(soc_egress_enable(unit, pport, 1));
                }
            }
