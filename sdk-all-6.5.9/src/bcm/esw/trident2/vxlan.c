...
#if defined(BCM_TRIDENT3_SUPPORT)
#define VXLAN_DVP_EVXLT_KEY_VFI 1
#endif
...
bcm_td2_vxlan_udpSourcePort_get(int unit, int *hashEnable)
{
    ...
    if (soc_feature(unit, soc_feature_cancun)) {
        /* TD3TBD
         * FLEX_EDITOR_FIXED_HDR_TUNNEL_0_PROFILE_TABLE.UDP_USE_SOURCE_PORT_SEL
         */
        SOC_IF_ERROR_RETURN(READ_FLEX_EDITOR_VXLAN_CONTROL_64r(unit, &reg64));
        *hashEnable =
            soc_reg64_field32_get(unit, FLEX_EDITOR_VXLAN_CONTROL_64r,
                reg64, VXLAN_FLAGSf) == 0x8 ? 1 : 0;
...
_bcm_td2_vxlan_reinit(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (port_type == 1) {                   /* Trunk */
            tgid = soc_mem_field32_get(unit, mem, entry, TGIDf);
        }
    ...
    if (info->flags & BCM_VXLAN_VPN_WITH_VPNID) {
        ...
        if (soc_feature(unit, soc_feature_base_valid)) {
            ...
            soc_mem_field32_set(unit, mmem, &ment,
                DATA_TYPEf, _BCM_VXLAN_KEY_TYPE_VNID_VFI);
        } else
#endif
        {
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE) {
            ...
            if (rv == BCM_E_NONE) {
                vnid = soc_mem_field32_get(unit, vxlate_mem,
                            &vxlate_entry, VXLAN_VFI__VN_IDf);
                soc_mem_field32_set(unit, mmem, &ment, VXLAN_VN_ID__VN_IDf, vnid);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
                if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
        ...
        soc_mem_field32_set(unit, mmem, &ment, VXLAN_VN_ID__VN_IDf, info->vnid);
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_base_valid)) {
            soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry, BASE_VALID_0f, 3);
            soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry, BASE_VALID_1f, 7);
            soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry,
                                    DATA_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI);
        } else
#endif
        {
        ...
        soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry, VXLAN_VFI__VFIf, vfi_index);
        soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry, VXLAN_VFI__VN_IDf, info->vnid);
		
		if (soc_mem_field_valid(unit, vxlate_mem, KEY_TYPEf)) {
            soc_mem_field32_set(unit, vxlate_mem, &vxlate_entry,
        ...
        rv = _bcm_td2_vxlan_egr_xlate_entry_get(unit, vfi_index, -1, vxlate_entry);
        if (rv == BCM_E_NONE) {
            vnid = soc_mem_field32_get(unit, vxlate_mem, &vxlate_entry, VXLAN_VFI__VN_IDf);
            /* Delete mpls_entry */
...
_bcm_td2_vxlan_egr_xlate_entry_get(int unit, int vfi, int vp, uint32 *vxlate_entry)
{
    ...
    if (soc_feature(unit, soc_feature_base_valid)) {
        ...
        soc_mem_field32_set(unit, vxlate_mem, &key_vxlate, BASE_VALID_0f, 3);
        soc_mem_field32_set(unit, vxlate_mem, &key_vxlate, BASE_VALID_1f, 7);
        if (vp != -1) {
            ...
            soc_mem_field32_set(unit, vxlate_mem, &key_vxlate,
                                DATA_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP);
        } else {
            soc_mem_field32_set(unit, vxlate_mem, &key_vxlate,
                                KEY_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI);
            soc_mem_field32_set(unit, vxlate_mem, &key_vxlate,
...
bcm_td2_vxlan_vpn_get( int unit, bcm_vpn_t l2vpn, bcm_vxlan_vpn_config_t *info)
{
    ...
    if (rv == BCM_E_NONE) {
        ...
        info->vnid = soc_mem_field32_get(unit, vxlate_mem, &vxlate_entry, VXLAN_VFI__VN_IDf);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
    ...
    bcm_tunnel_terminator_t *tnl_info, uint32 *tr_ent, uint8 out_mode, int clean_flag)
{
    ...
    soc_mem_field32_set(unit, mem, tr_ent, VXLAN_DIP__DIPf, tnl_info->dip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
    ...
    soc_mem_field32_set(unit, mem, tr_ent, VXLAN_DIP__IGNORE_UDP_CHECKSUMf,
                                         ...
                                         int multicast_flag)
{
    ...
    if (rv == SOC_E_NONE) {
        ...
        if (net_receiver != soc_mem_field32_get(unit, mem,
            vxlate_entry, VXLAN_DIP__NETWORK_RECEIVERS_PRESENTf)) {
            soc_mem_field32_set(unit, mem, vxlate_entry,
...
bcm_td2_vxlan_tunnel_terminator_update(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if (tnl_info->flags & BCM_TUNNEL_TERM_TUNNEL_DEACTIVATE) {        
        ...
        if (vxlan_info->vxlan_tunnel_term[tunnel_idx].dip != 0) {
            soc_mem_field32_set(unit, mem, vxlate_entry, VXLAN_DIP__IGNORE_UDP_CHECKSUMf,
                   (vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag
                   & _BCM_VXLAN_TUNNEL_TERM_UDP_CHECKSUM_ENABLE) ? 0x0 : 0x1);
            if (vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag &
                _BCM_VXLAN_TUNNEL_TERM_USE_OUTER_DSCP) {
                soc_mem_field32_set(unit, mem, vxlate_entry, VXLAN_DIP__USE_OUTER_HEADER_PHBf, 0x1);
            } else if (vxlan_info->vxlan_tunnel_term[tunnel_idx].activate_flag &
                _BCM_VXLAN_TUNNEL_TERM_USE_OUTER_PCP) {
                soc_mem_field32_set(unit, mem, vxlate_entry, VXLAN_DIP__USE_OUTER_HEADER_PHBf, 0x2);
            }
        ...
        for (i = 0; i < nent; i++) {
            ...
            if (soc_feature(unit, soc_feature_base_valid)) {
                if (soc_mem_field32_get(unit, vxlate_mem,
                        vtabp, BASE_VALID_0f) != 3) {
                    continue;
                }
                if (soc_mem_field32_get(unit, vxlate_mem,
                        vtabp, BASE_VALID_1f) != 7) {
                    continue;
                }
...
_bcm_td2_vxlan_egress_dvp_set(int unit, int vp, int drop, bcm_vxlan_port_t *vxlan_port)
{
    ...
    if(SOC_MEM_FIELD_VALID(unit,EGR_DVP_ATTRIBUTEm,VXLAN__EVXLT_KEY_SELf)) {
        soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTEm,
            &egr_dvp_attribute, VXLAN__EVXLT_KEY_SELf, VXLAN_DVP_EVXLT_KEY_VFI);
    }
#endif
...
_bcm_td2_vxlan_egr_xlate_entry_reset(int unit, bcm_vpn_t vpn)
{
    ...
    if (soc_feature(unit, soc_feature_base_valid)) {
        ...
        soc_mem_field32_set(unit, vxlate_mem, &vxlate, BASE_VALID_0f, 3);
        soc_mem_field32_set(unit, vxlate_mem, &vxlate, BASE_VALID_1f, 7);
    ...
    soc_mem_field32_set(unit, vxlate_mem, &vxlate, VXLAN_VFI__VFIf, vfi);
                         ...
                         int network_port_flag )
{
    ...
    if (network_port_flag) {
       ...
       if (action_not_present == 1) {
            ...
            vxlan_vpn_info->egress_service_vlan = soc_mem_field32_get(unit,
       ...
       switch (action_present) {
         ...
         case 4:
            ...
            vxlan_vpn_info->egress_service_vlan = soc_mem_field32_get(unit,
                            vxlate_mem, vxlate_entry, VXLAN_VFI__SD_TAG_VIDf);
            vxlan_vpn_info->pkt_pri = soc_mem_field32_get(unit,
                vxlate_mem, vxlate_entry, VXLAN_VFI__SD_TAG_NEW_PRIf);
            vxlan_vpn_info->pkt_cfi = soc_mem_field32_get(unit,
       ...
       if ((vxlan_vpn_info->flags & BCM_VXLAN_VPN_SERVICE_VLAN_ADD) ||
            ...
            _bcm_fb2_outer_tpid_entry_get(unit,
                &vxlan_vpn_info->egress_service_tpid, tpid_index);
       }
    } else {
       action_present = 
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            type_field, _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW);
                            ...
                            bcm_vpn_t vpn, int drop)
{
    ...
    if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
        ...
        if (SOC_IS_TRIDENT3(unit)) {
            rv = bcmi_td3_l3_nh_assoc_ol_ul_link_replace(unit, nh_ecmp_index, 
                vp_nh_ecmp_index);
        } else
#endif
        {
...
STATIC int
_bcm_td2_vxlan_match_tunnel_entry_reset(int unit, uint32 *ment, uint32 *return_ment)
{
    int rv = BCM_E_UNAVAIL;
    int index = 0;   
    rv = soc_mem_search(unit, MPLS_ENTRYm, MEM_BLOCK_ANY, &index,
...
_bcm_td2_vxlan_match_add(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_VLAN ) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_base_valid)) {
            soc_mem_field32_set(unit, mem, ment_svp, BASE_VALID_0f, 3);
            soc_mem_field32_set(unit, mem, ment_svp, BASE_VALID_1f, 7);
            soc_mem_field32_set(unit, mem, ment_svp,
                DATA_TYPEf, _BCM_VXLAN_KEY_TYPE_TUNNEL);
        } else
#endif
        {
        ...
        soc_mem_field32_set(unit, mem, ment_svp, VXLAN_SIP__SIPf, tunnel_sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
...
#endif
        soc_mem_field32_set(unit, mem, ment_svp, VXLAN_SIP__SVPf, vp);
        ...
        if (vxlan_vpn_type) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
            if (soc_feature(unit, soc_feature_base_valid)) {
                soc_mem_field32_set(unit, vxlate_mem, &ment_evt, BASE_VALID_0f, 3);
                soc_mem_field32_set(unit, vxlate_mem, &ment_evt, BASE_VALID_1f, 7);
            ...
            soc_mem_field32_set(unit, vxlate_mem, &ment_evt, VXLAN_VFI__VFIf, vfi_index);
            soc_mem_field32_set(unit, vxlate_mem, &ment_evt, VXLAN_VFI__DVPf, vp);
            soc_mem_field32_set(unit, vxlate_mem, &ment_evt, VXLAN_VFI__VN_IDf, vxlan_port->vnid);
            /* Configure EGR_VLAN_XLATE - SD_TAG setting */
            if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
                ...
                if (1 == action_present || 4 == action_present ||
                    ...
                    tpid_index = soc_mem_field32_get(unit,
                                     vxlate_mem, &old_vxlate_entry, VXLAN_VFI__SD_TAG_TPID_INDEXf);
                    rv = _bcm_fb2_outer_tpid_entry_delete(unit, tpid_index);
                    if ((rv != BCM_E_NONE) && (rv != BCM_E_NOT_FOUND)) {
                        goto match_add_clean1;
                    }            
            ...
            soc_mem_field32_set(unit, mem, &ment_vnid, VXLAN_VN_ID__SIPf, tunnel_sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
                VLAN_CHK_ID(unit, vxlan_port->match_vlan);
                vxlan_info->match_key[vp].match_vlan = vxlan_port->match_vlan;
            ...
            soc_mem_field32_set(unit, mem, &ment_vnid, VXLAN_VN_ID__VN_IDf, vxlan_port->vnid);
...
_bcm_td2_vxlan_match_delete(int unit,  int vp , bcm_vxlan_port_t vxlan_port, bcm_vpn_t vpn)
{
    ...
    if  (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        ...
        soc_mem_field32_set(unit, mem, &ment_svp, VXLAN_SIP__SIPf,
        ...
        if (vxlan_vpn_type) {            
           ...
           rv = soc_mem_search(unit, vxlate_mem, MEM_BLOCK_ANY, &index,
            ...
            /* set mpls_entry table */
            soc_mem_field32_set(unit, mem, &ment_vnid, VXLAN_VN_ID__SIPf, 
                                        vxlan_info->vxlan_tunnel_term[tunnel_index].sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
                soc_mem_field32_set(unit, mem, &ment_vnid, VXLAN_VN_ID__OVIDf,
...
#define _BCM_VLXLT_KTYPE_ASSIGN(_ktype, _k) \
    ...
    switch(mkey->flags) {
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VNID:
            ...
            soc_mem_field32_set(unit, mem, &ment, VXLAN_SIP__SIPf, tunnel_sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            ...
            /* Replace entry's VP value */
            *old_vp = soc_mem_field32_get(unit, mem, &ment, VXLAN_SIP__SVPf);
            soc_mem_field32_set(unit, mem, &ment, VXLAN_SIP__SVPf, new_vp);
            /* Insert new entry */
...
#define _BCM_VLXLT_KTYPE_ASSIGN(_ktype, _k) \
    ...
    switch(mkey->flags) {
        ...
        case _BCM_VXLAN_PORT_MATCH_TYPE_VNID:
            ...
            /* Get VP value representing VP LAG */
            *vp_lag_vp = soc_mem_field32_get(unit, mem, &ment, VXLAN_SIP__SVPf);
            break;
        default:
            return BCM_E_PARAM;
    }
                                 ...
                                 int *index)
{
    ...
    if (soc_feature(unit, soc_feature_base_valid)) {
        ...
        soc_mem_field32_set(unit, mem, vent, BASE_VALID_1f, 7);
        soc_mem_field32_set(unit, mem, vent,
            DATA_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_DIP);
    } else
#endif
    {
        mem = VLAN_XLATEm;
    ...
    soc_mem_field32_set(unit, mem, vent, VXLAN_DIP__DIPf, vxlan_dip);
