...
char *label_type_array[5] = { "special_label_type_NONE",
                                  ...
                                  "Special_label_type_RAL",
                                  "Special_label_type_SPECIAL",
                                  "Special_label_type_ELI"};
#endif
...
STATIC int _bcm_tr_get_ing_mpls_index_size(int unit);
STATIC int bcmi_egr_mpls_exp_pri_mapping_num_exp_map_get(int unit);
STATIC int
bcmi_td3_soc_mem_fields32_modify(int unit, soc_mem_t mem, int index,
                        soc_field_t field, uint32 value);
STATIC int
bcmi_egr_dvp_attribute_nwgroup_get(int unit, int vp, int *nw_group);

STATIC int
bcmi_cmh_svp_attributes_set(int unit, int entry_type, int svp, int set_flag,
        int mpls_flags)
{
    int rv = BCM_E_NONE;
    uint32 svp_attrs_0_prof_idx = 0;
    uint32 svp_attrs_1_prof_idx = 0;
    uint32 flow_sel_ctrl_id_0 = 0;
    source_vp_2_entry_t svpattrs_0;
    svp_attrs_1_entry_t svpattrs_1;

    /*VPWS or VPLS SVP ATTRIBUTES*/
    if ((entry_type == 1) || (entry_type == 2)) {
        if((set_flag == TRUE) && (mpls_flags & BCM_MPLS_PORT_EGRESS_TUNNEL)) {
            svp_attrs_0_prof_idx = 2;
            flow_sel_ctrl_id_0 = 1;

            if(mpls_flags & BCM_MPLS_PORT_CONTROL_WORD) {
                svp_attrs_1_prof_idx = 2;
            } else {
                svp_attrs_1_prof_idx = 1;
            }
        }
    }

    /*Write SVP_ATTR_0,SVP_ATTRS_1 tables*/
    BCM_IF_ERROR_RETURN(READ_SVP_ATTRS_1m(unit, MEM_BLOCK_ALL,
                svp,&svpattrs_1));
    soc_SVP_ATTRS_1m_field_set(unit, &svpattrs_1,
            CTRL_PROFILE_IDXf, &svp_attrs_1_prof_idx);
    rv = WRITE_SVP_ATTRS_1m(unit, MEM_BLOCK_ALL, svp, &svpattrs_1);
    if( rv != BCM_E_NONE) {
        return rv;
    }

    BCM_IF_ERROR_RETURN(READ_SOURCE_VP_2m(unit, MEM_BLOCK_ALL,
                svp,&svpattrs_0));
    soc_SOURCE_VP_2m_field_set(unit, &svpattrs_0,
            CTRL_PROFILE_IDXf, &svp_attrs_0_prof_idx);
    soc_SOURCE_VP_2m_field_set(unit, &svpattrs_0,
            PKT_FLOW_SELECT_CTRL_ID_0f, &flow_sel_ctrl_id_0);
    rv = WRITE_SOURCE_VP_2m(unit, MEM_BLOCK_ALL, svp, &svpattrs_0);
    if( rv != BCM_E_NONE) {
        return rv;
    }
...
#if defined(BCM_TRIDENT3_SUPPORT)
STATIC int
bcmi_cmh_vfi_profile_set(int unit, uint32 *vfi_profile_idx)
{
    int rv = BCM_E_NONE;
    uint32 pkt_flow_sel_ctrl_id_2 = 4;
    vfi_profile_entry_t vfi_profile;
    void *entries[1];

    sal_memset(&vfi_profile, 0, sizeof(vfi_profile_entry_t));
    soc_VFI_PROFILEm_field_set(unit, &vfi_profile,
            PKT_FLOW_SELECT_CTRL_ID_2f, &pkt_flow_sel_ctrl_id_2);
    entries[0] = &vfi_profile;
    rv = _bcm_vfi_profile_entry_add(unit, entries, 1, vfi_profile_idx);
...
STATIC int
bcmi_egr_mpls_exp_pri_mapping_num_exp_map_get(int unit)
{
    int num_exp_map = 0;

#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        num_exp_map = soc_mem_index_count(unit,
                EGR_MPLS_PRI_MAPPINGm) / 64;
    } else
#endif
    {
        num_exp_map = soc_mem_index_count(unit,
                        ...
                        soc_field_t field, uint32 value)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_mem_field32_fit(unit, mem, field, value));
    ...
    if (field == DESTINATIONf) {
        ret_val = soc_mem_field32_dest_get(unit, mem, buf,
                    DESTINATIONf, &dest_type);

        if ((dest_type == SOC_MEM_FIF_DEST_DVP) &&
                (value != ret_val)) {
            soc_mem_field32_dest_set(unit, mem, buf,
                        DESTINATIONf, SOC_MEM_FIF_DEST_DVP, value);
            value_changed = 1;
        }
    } else {
        if(value != soc_mem_field32_get(unit, mem, buf, field)) {
                value_changed = 1;
                soc_mem_field32_set(unit, mem, buf, field, value);
        }
    }
...
bcmi_egr_dvp_attribute_nwgroup_get(int unit, int vp, int *nw_group)
{
    ...
    if (BCM_SUCCESS(rv)) {
        if (soc_mem_field32_get(unit, EGR_DVP_ATTRIBUTEm,
                    &egr_dvp_attribute, COMMON__DVP_NETWORK_GROUPf)) {
            *nw_group =  soc_mem_field32_get(unit, EGR_DVP_ATTRIBUTEm,
                    &egr_dvp_attribute, COMMON__DVP_NETWORK_GROUPf);
        }
    }
    return rv;
...
int _bcm_tr_get_ing_mpls_index_size(int unit)
{
    int num_ing_exp_map;

    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        num_ing_exp_map = 32;/*soc_mem_index_count(unit, PHB_MAPPING_TBL_3m) / 8;*/
    } else {
        num_ing_exp_map = soc_mem_index_count(unit, ING_MPLS_EXP_MAPPINGm) / 8;
    }

    return num_ing_exp_map;
}
...
_bcm_tr_mpls_free_resource(int unit, _bcm_tr_mpls_bookkeeping_t *mpls_info)
{
...
#if defined(BCM_APACHE_SUPPORT) || defined(BCM_TOMAHAWK2_SUPPORT)  || defined(BCM_TRIDENT3_SUPPORT)
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (mpls_info->egr_vc_swap_lbl_action) {
            sal_free(mpls_info->egr_vc_swap_lbl_action);
            mpls_info->egr_vc_swap_lbl_action = NULL;
        }
    }
#endif

#if defined(BCM_SPECIAL_LABEL_SUPPORT)
    if (soc_feature(unit, soc_feature_mpls_special_label)) {
        (void) bcmi_mpls_special_label_deinit(unit);
    }
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        egr_sequence_number_table_entry_t td3_pw_init_entry;
        num_pw_init = soc_mem_index_count(unit, EGR_SEQUENCE_NUMBER_TABLEm);
        /* Clear the EGR Counters */
        sal_memset(&td3_pw_init_entry, 0, sizeof(egr_sequence_number_table_entry_t));
        for (index = 0; index < num_pw_init; index++) {
            rv = WRITE_EGR_SEQUENCE_NUMBER_TABLEm(unit, MEM_BLOCK_ANY, index,
                    &td3_pw_init_entry);
            if (rv < 0) {
                return rv;
            }
...
_bcm_tr_mpls_scache_size_get(int unit, int *size)
{
    ...
    num_ing_exp_map = _bcm_tr_get_ing_mpls_index_size(unit);
#ifdef BCM_APACHE_SUPPORT
    if (soc_feature(unit, soc_feature_mpls_exp_to_phb_cng_map)) {
    ...
    num_egr_l2_exp_map = bcmi_egr_mpls_exp_pri_mapping_num_exp_map_get(unit);
    num_pw_term = 0;
    if (SOC_MEM_IS_VALID(unit, ING_PW_TERM_COUNTERSm)) {
        num_pw_term = soc_mem_index_count(unit, ING_PW_TERM_COUNTERSm);
    }
    ...
    if (SOC_MEM_IS_VALID(unit, mem)) {
        num_pw_init = soc_mem_index_count(unit, mem);
    }
...
_bcm_tr_mpls_source_vp_recover(int unit)
{
    ...
    int rv = BCM_E_NONE;
    int value = 0, vp;
    int dscp_map_chunk = 0, ing_pri_cng_chunk = 0;
    int vp_count = 0;
    soc_mem_t port_tbl;

#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        ing_pri_cng_chunk = 64;
        dscp_map_chunk = 64;
    } else
#endif
    {
        ing_pri_cng_chunk = 16;
        dscp_map_chunk = 64;
    }

    vp_count = soc_mem_index_count(unit, SOURCE_VPm);
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        port_tbl = LPORT_TABm;
    } else {
        port_tbl = PORT_TABm;
    ...
    for (vp = 0; vp < vp_count; vp++) {
        ...
        if (_BCM_MPLS_SVP_PROFILE_SET_USED_GET(unit, vp)) {
            ...
            if (!soc_mem_field_valid(unit, SOURCE_VPm, TRUST_DSCP_PTRf) ||
                !soc_mem_field_valid(unit, port_tbl, TRUST_DOT1P_PTRf) ||
...
_bcm_tr_mpls_associated_data_recover(int unit)
{
    ...
    for (i = idx_min; i <= idx_max; i++) {
        ...
        if (soc_EGR_L3_NEXT_HOPm_field32_get(unit, &nh_entry, nh_entry_type_field) != 1) {
...
bcmi_tr_mpls_egr_vc_and_swap_table_hash_recover(int unit)
{
    ...
    for (idx = imin; idx < num_vc + imin; idx++) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            action = MPLS_INFO(unit)->egr_vc_swap_lbl_action[table_idx];
        } else
#endif
        {
            action = 0;
        ...
        BCM_IF_ERROR_RETURN(bcmi_tr_mpls_vc_swap_hash_index_get(unit, 
            NULL , action, NULL, NULL, &vc_entry, &hash_idx));
...
#if defined(BCM_TRIDENT3_SUPPORT)
         if (soc_feature(unit, soc_feature_td3_style_mpls)) {
             egr_ip_tunnel_mpls_double_wide_entry_t tnl_entry_dbl;
             rv = READ_EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm(unit, MEM_BLOCK_ANY,
                     tnl_index, &tnl_entry_dbl);
             if (rv < 0) {
                 return rv;
             }
             if (soc_mem_field32_get(unit, EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm,
                         &tnl_entry_dbl, DATA_TYPEf) == 0x14) {
                 /* Increment Ref count */
...
_bcm_tr_mpls_match_key_recover(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        SOC_IF_ERROR_RETURN(soc_mem_read(unit,
                                         mem, MEM_BLOCK_ANY, i, vent));
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            if ((soc_mem_field32_get(unit, mem, vent, BASE_VALID_0f) == 0) ||
                    (soc_mem_field32_get(unit, mem, vent, BASE_VALID_1f) == 0)) {
                continue;
            }
        } else {
        ...
        if ((soc_mem_field32_get(unit, mem, vent, MPLS_ACTIONf) & 0x1) != 0x1) {
            continue;
        }    
            ...
            if (soc_mem_field32_get(unit, mem, vent, KEY_TYPEf) ==
             TR_VLXLT_HASH_KEY_TYPE_OVID) {
                ...
                mpls_info->match_key[vp].match_vlan =
                    soc_mem_field32_get(unit, mem, vent, OVIDf);
                if (soc_mem_field32_get(unit, mem, vent, Tf) == 0x1) {
                    mpls_info->match_key[vp].trunk_id = 
                              soc_mem_field32_get(unit, mem, vent, TGIDf);
                } else {
                    mpls_info->match_key[vp].port = 
                            soc_mem_field32_get(unit, mem, vent, PORT_NUMf);
                    mpls_info->match_key[vp].modid = 
                           soc_mem_field32_get(unit, mem, vent, MODULE_IDf);
                }
                bcm_tr_mpls_port_match_count_adjust(unit, vp, 1);
        } else if (soc_mem_field32_get(unit, mem, vent, KEY_TYPEf) ==
             TR_VLXLT_HASH_KEY_TYPE_IVID) {
                ...
                mpls_info->match_key[vp].match_inner_vlan =
                    soc_mem_field32_get(unit, mem, vent, IVIDf);
                if (soc_mem_field32_get(unit, mem, vent, Tf) == 0x1) {
                    mpls_info->match_key[vp].trunk_id = 
                              soc_mem_field32_get(unit, mem, vent, TGIDf);
                } else {
                ...
                mpls_info->match_key[vp].match_subport_pkt_vid =
                              ...
                              LLTAG__LLVIDf);
                mpls_info->match_key[vp].match_inner_vlan = 
                              soc_mem_field32_get(unit, mem, vent, LLTAG__IVIDf);
                if (soc_mem_field32_get(unit, mem, vent, Tf) == 0x1) {
                    mpls_info->match_key[vp].trunk_id =
                              soc_mem_field32_get(unit, mem, vent, TGIDf);
                } else {
                ...
                mpls_info->match_key[vp].match_subport_pkt_vid =
                              ...
                              LLTAG__LLVIDf);
                mpls_info->match_key[vp].match_vlan = 
                              soc_mem_field32_get(unit, mem, vent, LLTAG__OVIDf);
                if (soc_mem_field32_get(unit, mem, vent, Tf) == 0x1) {
                    mpls_info->match_key[vp].trunk_id =
                              soc_mem_field32_get(unit, mem, vent, TGIDf);
                } else {
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            if ((soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry, BASE_VALID_0f) == 0) ||
                    (soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry, BASE_VALID_1f) == 0)) {
                continue;
            }
        } else {
        ...
        if (stable_size > alloc_sz) {
...
#if  defined(BCM_TRIDENT3_SUPPORT)
            if(soc_feature(unit, soc_feature_td3_style_mpls)) {
                num_ip_tnl = soc_mem_index_count(unit,
...
#if defined(BCM_TRIDENT3_SUPPORT)
             /*Recover VC SWAP LABEL Actions in TD3*/
            if (recovered_ver >= BCM_WB_VERSION_1_9) {
                if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                    sal_memcpy(mpls_info->egr_vc_swap_lbl_action, mpls_state,
                            num_vc * sizeof(int));
                    mpls_state += num_vc * sizeof(int);
                }
            } else {
                additional_scache_sz += (num_vc * sizeof(int));
            }
#endif
#ifdef BCM_TRIUMPH2_SUPPORT
            ...
            if (soc_feature(unit, soc_feature_mpls_special_label)) {
                ...
                if (recovered_ver >= BCM_WB_VERSION_1_11) {
                    ...
                    num_entry =
                        soc_mem_index_count(unit, SPECIAL_LABEL_CONTROLm);
                    for (i = 0; i < num_entry; i++) {
                        sal_memcpy((&(ptr[i].ref_count)), mpls_state,
                                   sizeof(int));
                        mpls_state += sizeof(int);
                        sal_memcpy((&(ptr[i].cancun_added)), mpls_state,
                                   sizeof(int));
                        mpls_state += sizeof(int);
                    }
                } else {
                    /* Ask for space for new two variables */
                    additional_scache_sz += (num_entry * 2 * sizeof(int));
                }
            }
#endif
...
#endif /* BCM_SABER2_SUPPORT */
       /*
         * For TD3 VCSWAP Label action
         */
        if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_9) {
            if(soc_feature(unit, soc_feature_td3_style_mpls)) {
                alloc_sz += SHR_BITALLOCSIZE(soc_mem_index_count(unit,
                            EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm));
            }
        ...
        if (soc_feature(unit, soc_feature_mpls_special_label)) {
            ...
            if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_11) {
                num_entry =
                    soc_mem_index_count(unit, SPECIAL_LABEL_CONTROLm);
                /* Ask for space for new two variables */
                alloc_sz += (num_entry * 2 * sizeof(int));
            }
        }
#endif
 ...
 */
    ...
    if (soc_feature(unit, soc_feature_mpls_special_label)) {
        ...
        if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_11) {
            num_entry =
                soc_mem_index_count(unit, SPECIAL_LABEL_CONTROLm);
            alloc_sz += sizeof(bcmi_mpls_special_label_control_t) * num_entry;
        }
    }
...
#if defined(BCM_TRIDENT3_SUPPORT)
    /* Sync the scache for the VCSWAP Label action*/
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_9) {
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            sal_memcpy(mpls_state, mpls_info->egr_vc_swap_lbl_action,
                    num_vc * sizeof(int));
            mpls_state += num_vc * sizeof(int);
        }
    }
    ...
    if (soc_feature(unit, soc_feature_mpls_special_label)) {
        ...
        if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_11) {
            ...
            num_entry =
                soc_mem_index_count(unit, SPECIAL_LABEL_CONTROLm);
            for (i = 0; i < num_entry; i++) {
                sal_memcpy(mpls_state, (&(ptr[i].ref_count)),
                           sizeof(int));
                mpls_state += sizeof(int);
                sal_memcpy(mpls_state, (&(ptr[i].cancun_added)),
                           sizeof(int));
                mpls_state += sizeof(int);
            }
        }
                                          ...
                                          int end_idx)
{
    ...
    size = count * soc_mem_entry_words(unit, L3_IIFm) * 4;
    ...
    for (idx = 0; idx < count; idx++) {
         ...
         */
        sal_memcpy(hw_entry, &write_entry, soc_mem_entry_words(unit, L3_IIFm) * 4);
    }
...
bcm_tr_mpls_init(int unit)
{
    ...
    int rv=0, i, num_vp, num_vpws, num_vc, num_vrf, num_iif, num_trunk;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    /*To store label actions of vc swap index*/
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        mpls_info->egr_vc_swap_lbl_action =
            sal_alloc((num_vc * sizeof(int)), "egr_vc_swap_lbl_action");
        if (mpls_info->egr_vc_swap_lbl_action == NULL) {
            _bcm_tr_mpls_free_resource(unit, mpls_info);
            return BCM_E_MEMORY;
        }
    }
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        sal_memset(mpls_info->egr_vc_swap_lbl_action, 0,
                (num_vc * sizeof(int)));
    }
#endif
#ifdef BCM_TRIUMPH2_SUPPORT
     ...
     */
    sal_memset(pri_map, 0, (SOC_MAX_MEM_FIELD_WORDS) * 4);
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
    ...
    for (pkt_cfi = 0; pkt_cfi <= 1; pkt_cfi++) {
        for (pkt_pri = 0; pkt_pri <= 7; pkt_pri++) {
            ...
            soc_mem_field32_set(unit, mem, pri_map, pri_field, 
                                pkt_pri);
            soc_mem_field32_set(unit, mem, pri_map, CNGf,
                                pkt_cfi);
            rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, i, pri_map);
            if (rv < 0) {
                _bcm_tr_mpls_free_resource(unit, mpls_info);
                return rv;
            }
...
#if defined(BCM_SPECIAL_LABEL_SUPPORT)
    if (soc_feature(unit, soc_feature_mpls_special_label)) {
        rv = bcmi_mpls_special_label_init(unit);
        if (rv != BCM_E_NONE) {
            _bcm_tr_mpls_free_resource(unit, mpls_info);
        }
    }
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (!SOC_WARM_BOOT(unit)) {
            rv = soc_reg_field32_modify(unit, RTAG7_HASH_CONTROLr,
                    REG_PORT_ANY, USE_MPLS_STACK_FOR_HASHINGf, 0);
...
_bcm_tr_mpls_hw_clear(int unit)
{
    ...
    rv = bcm_esw_mpls_tunnel_initiator_clear_all(unit);
    if (BCM_FAILURE(rv) && (BCM_E_NONE == rv_error)) {
        rv_error = rv;
    }
...
#if defined(BCM_TRIDENT3_SUPPORT)
                /*VFI_PROFILE setting as per CMH rules*/
                bcmi_cmh_vfi_profile_set(unit, &vfi_profile_idx);
                if(vfi_profile_idx != 0) {
                    soc_VFIm_field32_set(unit, &vfi_entry,
                            VFI_PROFILE_PTRf, vfi_profile_idx);
                }
#endif
            } else {
                                 ...
                                 uint32 flags, int nh_index, int *is_local, int is_vpws, int drop)
{
    ...
    if (egr_nh_info->entry_type == 0x1) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm,
                    &egr_nh, L3__INTF_NUMf, egr_nh_info->intf_num);
        } else {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm,
        ...
        if (soc_feature(unit, soc_feature_mpls_enhanced)) {
              ...
              if (soc_feature(unit, soc_feature_multiple_split_horizon_group)) {
                  if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                      rv = soc_mem_field32_modify(unit, EGR_DVP_ATTRIBUTEm,
                              egr_nh_info->dvp, COMMON__DVP_NETWORK_GROUPf,
                              egr_nh_info->dvp_network_group_id);
                      if (rv < 0) {
                          return rv;
                      }
    ...
    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                        nh_entry_type_field, egr_nh_info->entry_type);
        ...
        if (nhop_cursorPtr->egr_if == egr_if) {
            for (vp_cursorPtr = nhop_cursorPtr->vp_head_ptr;  vp_cursorPtr != NULL; vp_cursorPtr = vp_cursorPtr->link) {
                ...
                if (ing_nh_info.trunk == -1) {
                    if (soc_feature(unit, soc_feature_generic_dest)) {
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_LAG, 0);
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_DGPP,
                                ing_nh_info.module << SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS | ing_nh_info.port);
                    } else {
                    ...
                    if (soc_feature(unit, soc_feature_generic_dest)) {
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_DGPP, 0);
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_LAG, ing_nh_info.trunk);
                    } else {
                        soc_mem_field32_set(unit, ING_L3_NEXT_HOPm,
                ...
                if (ing_nh_info.trunk == -1) {
                    ...
                    if (soc_feature(unit, soc_feature_generic_dest)) {
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &initial_ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_LAG, 0);
                        soc_mem_field32_dest_set(unit, ING_L3_NEXT_HOPm, &initial_ing_nh,
                                DESTINATIONf, SOC_MEM_FIF_DEST_DGPP,
                                ing_nh_info.module << SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS | ing_nh_info.port);
                                                           ...
                                                           int nh_index)
{
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (MPLS_INFO(unit)->egr_vc_swap_lbl_action != NULL) {
            action = MPLS_INFO(unit)->egr_vc_swap_lbl_action[vc_swap_index];
        }
    } else
#endif
    {
    ...
    BCM_IF_ERROR_RETURN(
        bcmi_tr_mpls_vc_swap_hash_index_get(unit, NULL, action, NULL, NULL,
                                            &vc_entry, &hash_idx));
...
_bcm_tr_mpls_vc_and_swap_table_index_reset (int unit, int vc_swap_index)
{
    ...
    BCM_IF_ERROR_RETURN(bcmi_tr_mpls_vc_swap_hash_index_get(
        unit, NULL, action, NULL, NULL, &vc_entry, &idx));
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (MPLS_INFO(unit)->egr_vc_swap_lbl_action != NULL) {
            mpls_info->egr_vc_swap_lbl_action[vc_swap_index] =
                _BCM_MPLS_ACTION_NOOP;
        }
    }
#endif
                                       ...
                                       int hw_map_idx, int vc_swap_index)
{
    ...
    if (mpls_port != NULL) {
         ...
         spl_label_action = info->egress_label.spl_label_push_type;
         action = info->action;
#ifdef BCM_TOMAHAWK2_SUPPORT        
         if (soc_feature(unit, soc_feature_mpls_ecn) && 
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            soc_mem_field32_set(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                    vc_entry, SPECIAL_LABEL_PUSH_TYPEf, spl_label_action);
        }
...
_bcm_tr_mpls_vc_and_swap_table_entry_adjust (int unit, int vp)
{
    ...
    rv = bcmi_tr_mpls_vc_swap_hash_index_get(unit, NULL, action, NULL, NULL,
                                             &vc_entry, &hash_idx);
    _BCM_MPLS_CLEANUP(rv);
    hash_input = &(bcmi_vc_swap_label_hash[unit][hash_idx]);
                   ...
                   uint32 pw_seq_number, _bcm_tr_egr_nh_info_t  *egr_nh_info)
{
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        ...
        num_pw_mask = soc_mem_index_count(0, EGR_SEQUENCE_NUMBER_TABLEm) - 1;
        ...
        sal_memset(&td3_pw_init_entry, 0, sizeof(egr_sequence_number_table_entry_t));
        soc_EGR_SEQUENCE_NUMBER_TABLEm_field_set(unit, &td3_pw_init_entry, SEQUENCE_NUMBERf,
                (uint32 *)&pw_seq_number);

        pw_init_cnt = EGR_PW_SEQNUM_VCSWAP_OFFSET + pw_init_cnt;
        WRITE_EGR_SEQUENCE_NUMBER_TABLEm(unit, MEM_BLOCK_ALL, pw_init_cnt,
                &td3_pw_init_entry);
    } else 
#endif
    {
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (SOC_MEM_IS_VALID(unit, EGR_SEQUENCE_NUMBER_TABLEm)) {
            egr_sequence_number_table_entry_t td3_pw_init_entry;
...
bcm_tr_mpls_port_untagged_profile_reset(int unit, bcm_port_t port)
{
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        /*Reset TAG_ACTION_PROFILE_PTR to 1 in TD3*/
        rv = _bcm_esw_port_tab_set_without_portlock(unit, port, _BCM_CPU_TABS_NONE,
                TAG_ACTION_PROFILE_PTRf, 1);
    } else {
        rv = _bcm_esw_port_tab_set_without_portlock(unit, port, _BCM_CPU_TABS_NONE,
...
_bcm_tr_mpls_port_mtu_set(int unit, int vp, int mtu)
{
    ...
    rv = READ_EGR_DVP_ATTRIBUTEm(unit, MEM_BLOCK_ALL, vp,
            &egr_dvp_attribute);
    if (BCM_SUCCESS(rv)) {
                            ...
                            _bcm_mpls_old_nh_info_t *old_nh_info)
{
    ...
    *local_port = 0;
    *is_local = 0;
    ing_nh_info.mtu = 0; /*Default, especially if MTU_SIZE is not valid */ 
    if (mpls_port->mtu != 0) {
...
_bcm_tr_mpls_l2_nh_info_delete(int unit, int nh_index)
{
    ...
    if (soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh, nh_entry_type_field) == 0x1) {
        /* Egressing into an MPLS tunnel */
        if (soc_feature(unit, soc_feature_mpls_enhanced)) {
              old_macda_idx = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh,
                                                         MPLS__MAC_DA_PROFILE_INDEXf);
    ...
    /*Read trunk_id,phy_port,modid*/
    if (soc_feature(unit, soc_feature_generic_dest)) {
        dest_val = soc_mem_field32_dest_get(unit, ING_L3_NEXT_HOPm,
                &ing_nh, DESTINATIONf, &dest_type);
        if (dest_type == SOC_MEM_FIF_DEST_LAG) {
            /* Trunk group */
            trunk_id = dest_val & SOC_MEM_FIF_DGPP_TGID_MASK;
            is_trunk = 1;
        } else {
            phys_port = dest_val & SOC_MEM_FIF_DGPP_PORT_MASK;
            modid     = (dest_val & SOC_MEM_FIF_DGPP_MOD_ID_MASK) >>
                            ...
                            int nh_index)
{
    ...
    BCM_IF_ERROR_RETURN (soc_mem_read(unit, EGR_L3_NEXT_HOPm, MEM_BLOCK_ANY, 
                                      nh_index, &egr_nh));
    entry_type =  soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh, nh_entry_type_field);
    ...
    if (entry_type == 0x2) {
        ...
        if (soc_feature(unit, soc_feature_generic_dest)) {
            ...
            if (dest_type == SOC_MEM_FIF_DEST_LAG) {
                ...
                port_in = dest_val & SOC_MEM_FIF_DGPP_PORT_MASK;
                mod_in  = (dest_val & SOC_MEM_FIF_DGPP_MOD_ID_MASK) >>
                    SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS;
            }
        } else {
            is_trunk = soc_ING_L3_NEXT_HOPm_field32_get(unit, &ing_nh, Tf);
            trunk_id = soc_ING_L3_NEXT_HOPm_field32_get(unit, &ing_nh, TGIDf);
            if(!is_trunk) {
        ...
        uint32 *vent, uint32 *return_ent)
{
    ...
    value = soc_mem_field32_get(unit, mem, return_ent, MPLS_ACTIONf);
    ...
    /*set default tag action profile ptr*/
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        soc_mem_field32_set(unit, mem, vent, XLATE__TAG_ACTION_PROFILE_PTRf, 1);
    }
#endif
                                                                     ...
                                                                     mpls_entry_entry_t *return_ment)
{
    ...
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        soc_MPLS_ENTRYm_field32_set(unit, return_ment, BASE_VALID_0f, 3);
        soc_MPLS_ENTRYm_field32_set(unit, return_ment, BASE_VALID_1f, 7);
    } else {
...
STATIC int
_bcm_tr_mpls_match_vlan_entry_set(int unit, bcm_mpls_port_t *mpls_port, uint32 *vent)
{
                       ...
                       int hw_update)
{
    ...
    if (mpls_port->criteria == BCM_MPLS_PORT_MATCH_PORT_VLAN ) {
        ...
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
                                        TR_VLXLT_HASH_KEY_TYPE_OVID);
        soc_mem_field32_set(unit, mem, vent, OVIDf,
        ...
        if (soc_feature(unit, soc_feature_vp_sharing)) {
            ...
            if (vp_info.flags & _BCM_VP_INFO_SHARED_PORT) {
                ...
                soc_mem_field32_set(unit, mem, vent, VFIf, vfi);
            }
        }
        ...
        soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
                                    TR_VLXLT_HASH_KEY_TYPE_IVID);
        soc_mem_field32_set(unit, mem, vent, IVIDf,
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
         ...
         soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
          ...
          soc_mem_field32_set(unit, mem, vent, OTAGf,
                                             mpls_port->match_vlan);
          if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
               soc_mem_field32_set(unit, mem, vent, Tf, 1);
               soc_mem_field32_set(unit, mem, vent, TGIDf, trunk_id);
        ...
        if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
            ...
            if (is_local_subport) {
                ...
                if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                    rv = _bcm_esw_port_tab_set(unit, local_port,
                            _BCM_CPU_TABS_BOTH,
                            PORT_OPERATIONf, 0x1);
                } else {
                    rv = _bcm_esw_port_tab_set_without_portlock(
        ...
        if (is_local) {
            ...
            /*TD3:VT_PORT_TYPE_SELECT_1f*/
            if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                rv = _bcm_esw_port_tab_set(unit, port_out,
                        _BCM_CPU_TABS_BOTH,
                        VT_PORT_TYPE_SELECT_1f, 1);
            } else {
                rv = soc_mem_field32_modify(unit, PORT_TABm, port_out,
        ...
        soc_mem_field32_set(unit, mem, vent, LLTAG__DISABLE_VLAN_CHECKSf, 1);
        ...
        soc_mem_field32_set(unit, mem, vent, LLTAG__LLVIDf,
                                    mpls_port->match_subport_pkt_vid);

        if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
            soc_mem_field32_set(unit, mem, vent, LLTAG__Tf, 1);
            soc_mem_field32_set(unit, mem, vent, LLTAG__TGIDf, trunk_id);
            mpls_info->match_key[vp].trunk_id = trunk_id;
        } else {
                          ...
                          int sw_clear)
{
    ...
    if (mpls_info->match_key[vp].flags == _BCM_MPLS_PORT_MATCH_TYPE_NONE) {
        ...
        if (mpls_info->match_key[vp].modid != -1) {
            soc_mem_field32_set(unit, mem, vent, MODULE_IDf,
                                        mpls_info->match_key[vp].modid);
            soc_mem_field32_set(unit, mem, vent, PORT_NUMf,
                                        mpls_info->match_key[vp].port);
        } else {
            soc_mem_field32_set(unit, mem, vent, Tf, 1);
            soc_mem_field32_set(unit, mem, vent, TGIDf,
                                        mpls_info->match_key[vp].trunk_id);
        }
    ...
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe)) {
        ...
        if (mpls_info->match_key[vp].flags == 
            ...
            soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
                                        KT2_PT_XLT_KEY_TYPE_LLVID_OVID);   
            soc_mem_field32_set(unit, mem, vent, LLTAG__OVIDf,
                                    mpls_info->match_key[vp].match_vlan);
        } else {
            soc_mem_field32_set(unit, mem, vent, KEY_TYPEf,
                                        KT2_PT_XLT_KEY_TYPE_LLVID_IVID); 
            soc_mem_field32_set(unit, mem, vent, LLTAG__IVIDf,
                                    mpls_info->match_key[vp].match_inner_vlan);
        }
        ...
        soc_mem_field32_set(unit, mem, vent, LLTAG__LLVIDf,
                                    mpls_info->match_key[vp].match_subport_pkt_vid);
        if (mpls_info->match_key[vp].modid != -1) {
            soc_mem_field32_set(unit, mem, vent, LLTAG__MODULE_IDf,
                                        mpls_info->match_key[vp].modid);
            soc_mem_field32_set(unit, mem, vent, LLTAG__PORT_NUMf,
                                        mpls_info->match_key[vp].port);
        } else {
            soc_mem_field32_set(unit, mem, vent, LLTAG__Tf, 1);
            soc_mem_field32_set(unit, mem, vent, LLTAG__TGIDf,
                                        mpls_info->match_key[vp].trunk_id);
        }
        rv = soc_mem_delete(unit, mem, MEM_BLOCK_ANY, vent);
        BCM_IF_ERROR_RETURN(rv);
        if (is_local) {
            ...
            if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                rv = _bcm_esw_port_tab_set(unit, pp_port,
                        _BCM_CPU_TABS_BOTH,
                        VT_KEY_TYPEf, KT2_PT_XLT_KEY_TYPE_OVID);
            } else {
...
/*
 * Function:
 *  _bcm_tr_mpls_failover_nw_port_match_get
 * Purpose:
 ...
 */
int
_bcm_tr_mpls_failover_nw_port_match_get(int unit, bcm_mpls_port_t *mpls_port,
                                         int vp, mpls_entry_entry_t *return_ment)
{
    ...
    if (mpls_port != NULL) {
        if (mpls_port->criteria == BCM_MPLS_PORT_MATCH_LABEL_PORT) {
            ...
            if (BCM_GPORT_IS_TRUNK(mpls_port->port)) {
                soc_MPLS_ENTRYm_field32_set(unit, &ment, Tf, 1);
                soc_MPLS_ENTRYm_field32_set(unit, &ment, TGIDf, mpls_port->port);
            } else {
                soc_MPLS_ENTRYm_field32_set(unit, &ment, MODULE_IDf, mod_out);
                soc_MPLS_ENTRYm_field32_set(unit, &ment, PORT_NUMf, port_out);
            }
...
bcmi_mpls_update_backup_svp(int unit, int vp, int dvp)
{
    ...
    if (soc_feature(unit, soc_feature_generic_dest)) {
        soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp,
                DESTINATIONf, SOC_MEM_FIF_DEST_DVP, dvp);
    } else {
        ...
        case BCM_MPLS_PORT_MATCH_PORT_VLAN_STACKED:
            ...
            if (soc_feature(unit, soc_feature_dscp_map_per_port)) {
                ...
                if (index_min == 0) {
                    ...
                    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                        rv = _bcm_esw_port_tab_get(unit, local_port,
                                TRUST_DSCP_PTRf,
                                &value);
                    } else {
                    ...
                    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                        alloc_size = dscp_map_chunk *
                                     sizeof(phb_mapping_tbl_2_entry_t);
                    } else {
                        alloc_size = dscp_map_chunk *
                    ...
                    /* Read the port chunk */
                    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                        rv = soc_mem_read_range(unit, PHB_MAPPING_TBL_2m, MEM_BLOCK_ANY, 
            ...
            if (soc_feature(unit, soc_feature_color_prio_map_profile)) {
                ...
                if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                    rv = _bcm_esw_port_tab_get(unit, local_port, TRUST_DOT1P_PTRf,
                            &value);
                } else {
            ...
            if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                rv = _bcm_esw_port_tab_get(unit, local_port, TRUST_DSCP_V4f,
                        &value);
            } else {
            ...
            if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                rv = _bcm_esw_port_tab_get(unit, local_port, TRUST_DSCP_V6f,
                        &value);
            } else {
...
_bcm_tr_mpls_vpws_port_add(int unit, bcm_vpn_t vpn, bcm_mpls_port_t *mpls_port)
{
    ...
    uint32 dest_type, dest_val;
    soc_field_t dest_field = 0;
    egr_dvp_attribute_entry_t  egr_dvp_attribute;

    if (SOC_MEM_FIELD_VALID(unit, SOURCE_VPm,DESTINATIONf)) {
        dest_field = DESTINATIONf;
    }
        ...
        if ( vpn != BCM_MPLS_VPWS_VPN_INVALID) {
             ...
             if (!(mpls_port->flags & BCM_MPLS_PORT_REPLACE)) {
                 if (BCM_SUCCESS(_bcm_esw_failover_mpls_check(unit,
                                                              mpls_port))) {
                     if (vp2 == -1 && vp1 != -1 && vp3_read != -1) {
                         /* Add new primary port: failover port should be set into vp2  */
                         vp2 = vp3_read;
                         vp3_read = -1;
                         _bcm_tr_mpls_vpws_vp_map_set(unit, vpn_id, vp1,
                                                      vp2, vp3_read);
                     }
                 }
             }
        ...
        switch (vp_valid_flag) {
                             ...
                             if (mpls_port->flags & BCM_MPLS_PORT_NETWORK) {
                                 ...
                                 if (SOC_MEM_IS_VALID(unit, EGR_DVP_ATTRIBUTEm)) {
                                     BCM_IF_ERROR_RETURN(WRITE_EGR_DVP_ATTRIBUTEm(unit,
                                                 MEM_BLOCK_ALL, vp2, &egr_dvp_attribute));
                                 }
                             } else {
                                 active_vp = vp1;
                                 vp_valid_flag = 1;
                                 if (SOC_MEM_IS_VALID(unit, EGR_DVP_ATTRIBUTEm)) {
                                     BCM_IF_ERROR_RETURN(WRITE_EGR_DVP_ATTRIBUTEm(unit,
                                                 MEM_BLOCK_ALL, vp1, &egr_dvp_attribute));
                                 }
                             }
         ...
         case 0x1:    /* Only VP1 is valid */	
                             ...
                             if (SOC_MEM_IS_VALID(unit, EGR_DVP_ATTRIBUTEm)) {
                                 BCM_IF_ERROR_RETURN(WRITE_EGR_DVP_ATTRIBUTEm(unit,
                                             MEM_BLOCK_ALL, active_vp, &egr_dvp_attribute));
                             }
                             break;
                                       ...
                                       if (active_vp == vp2) {
                                            ...
                                            if (BCM_SUCCESS(rv)) {
                                                 soc_SOURCE_VPm_field32_set(unit, &svp1,
                                                                ENTRY_TYPEf, 0x2);
                                                 if (soc_feature(unit, soc_feature_generic_dest)) {
                                                     soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp1,
                                                             DESTINATIONf, SOC_MEM_FIF_DEST_DVP, vp2);
                                                 } else {
                                                     soc_SOURCE_VPm_field32_set(unit, &svp1,
                                                             DVPf, vp2);
                                                 }
                                            ...
                                            if (soc_feature(unit, soc_feature_generic_dest)) {
                                                soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp3,
                                                        DESTINATIONf, SOC_MEM_FIF_DEST_DVP, vp1);
                                            } else {
                                                soc_SOURCE_VPm_field32_set(unit, &svp3,
                                                        DVPf, vp1);
                                            }
                                            ...
                                            if (BCM_SUCCESS(rv)) {
                                                 ...
                                                 if (soc_feature(unit, soc_feature_generic_dest)) {
                                                     soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp1,
                                                             DESTINATIONf, SOC_MEM_FIF_DEST_DVP, vp3);
                                                 } else {
                                                     soc_SOURCE_VPm_field32_set(unit, &svp1,
                                                             DVPf, vp3);
                                                 }
       ...
       if (mpls_port->flags & BCM_MPLS_PORT_FAILOVER) {
                  ...
                  if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
                      ...
                      BCM_IF_ERROR_RETURN(rv);
                        ...
                        if (mpls_port->flags & BCM_MPLS_PORT_SERVICE_TAGGED) {
                             ...
                             if (SOC_MEM_FIELD_VALID(unit, SOURCE_VPm, TPID_ENABLEf)) {
                        ...
                        if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                             ...
                             if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                                 ...
                                 sal_memset(svp_attrs_1, 0,
                                         sizeof(uint32) * SOC_MAX_MEM_WORDS);
                                 soc_SVP_ATTRS_1m_field_set(unit,
                                         &svp_attrs_1, TPID_ENABLEf,
                                         (uint32 *)&tpid_enable);
                                 BCM_IF_ERROR_RETURN(
                                         WRITE_SVP_ATTRS_1m(unit,
                                             MEM_BLOCK_ALL, vp1,
                                             &svp_attrs_1));
                             }
                            ...
                            if (soc_feature(unit,
                             ...
                             switch (vp_valid_flag) {
                                  case 1:
                                      if (soc_feature(unit, soc_feature_generic_dest)) {
                                          soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp1,
                                                  DESTINATIONf, SOC_MEM_FIF_DEST_DVP, 0);
                                      } else {
                                  ...
                                  case 3:
                                       if (vp3_read != -1) {
                                           if (soc_feature(unit, soc_feature_generic_dest)) {
                                               soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp1,
                                                       DESTINATIONf, SOC_MEM_FIF_DEST_DVP, vp3_read);
                                           } else {
                             ...
                             if (soc_mem_field_valid(unit, SOURCE_VPm, SVP_PROFILE_INDEXf)) {
                                 if(mpls_port->flags & BCM_MPLS_PORT_EGRESS_TUNNEL) {
                                     soc_SOURCE_VPm_field32_set(unit, &svp1, SVP_PROFILE_INDEXf, 3);
                                 }
                             }
                             ...
                             if ((rv >= 0) && (vp_valid_flag == 3)) {
                                 if (soc_feature(unit, soc_feature_generic_dest)) {
                                     soc_mem_field32_dest_set(unit, SOURCE_VPm, &svp2,
                                             DESTINATIONf, SOC_MEM_FIF_DEST_DVP, vp1);
                                 } else {
                             ...
                             if ((rv >= 0) && (vp3_read != -1)) {
                                 ...
                                 if (soc_feature(unit, soc_feature_generic_dest)) {
                                     ...
                                     if (dest_type == SOC_MEM_FIF_DEST_DVP) {
                                         if (vp1 != dest_val) {
                                             rv = bcmi_td3_soc_mem_fields32_modify(unit,
                                                     SOURCE_VPm, vp3_read, dest_field, vp1);
                                         }
                                     }
                                 } else {
                      ...
                      BCM_IF_ERROR_RETURN(rv);
                        ...
                        if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                            ...
                            if (soc_feature(unit,
                             ...
                             if (soc_mem_field_valid(unit, SOURCE_VPm, SVP_PROFILE_INDEXf)) {
                                 if(mpls_port->flags & BCM_MPLS_PORT_EGRESS_TUNNEL) {
                                     soc_SOURCE_VPm_field32_set(unit, &svp2, SVP_PROFILE_INDEXf, 3);
                                 }
                             }
                             rv = WRITE_SOURCE_VPm(unit, MEM_BLOCK_ALL, active_vp, &svp2);
                      ...
                      BCM_IF_ERROR_RETURN(rv);
                        ...
                        if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                            ...
                            if (soc_feature(unit,
                             ...
                             if (soc_mem_field_valid(unit, SOURCE_VPm, SVP_PROFILE_INDEXf)) {
                                 if(mpls_port->flags & BCM_MPLS_PORT_EGRESS_TUNNEL) {
                                     soc_SOURCE_VPm_field32_set(unit, &svp3, SVP_PROFILE_INDEXf, 3);
                                 }
                             }
         ...
         if (soc_feature(unit,soc_feature_td3_style_mpls)) {
             int entry_type = 2;
             rv = bcmi_cmh_svp_attributes_set(unit, entry_type, active_vp,
                     TRUE, mpls_port->flags);
             if (rv < 0) {
                 goto vpws_cleanup;
             }
         }
            ...
            if (!(mpls_port->flags & BCM_MPLS_PORT_REPLACE)) {
                ...
                if (vp_valid_flag) {
                    if (vp1 == active_vp) {
                        (void) _bcm_vp_free(unit, _bcmVpTypeMpls, 1, vp1);
                        if (vpn != BCM_MPLS_VPWS_VPN_INVALID) {
                            (void) _bcm_tr_mpls_vpws_vp_map_set(unit, vpn_id, -1, vp2, vp3_read);
                        }
                    }
                    if (vp2 == active_vp) {
                        ...
                        if (vpn != BCM_MPLS_VPWS_VPN_INVALID) {
                            (void) _bcm_tr_mpls_vpws_vp_map_set(unit, vpn_id, vp1, -1, vp3_read);
                        }
                    }
                }
            }
...
_bcm_tr_mpls_vpls_port_add(int unit, bcm_vpn_t vpn, bcm_mpls_port_t *mpls_port)
{
            ...
            if ((nh_index != 0)  &&  (nh_index < soc_mem_index_count(unit, ING_L3_NEXT_HOPm))) {
               ...
               if (soc_feature(unit, soc_feature_generic_dest)) {
                   ...
                   if (dest_type == SOC_MEM_FIF_DEST_LAG) {
                       ...
                       port_in   = dest_val & SOC_MEM_FIF_DGPP_PORT_MASK;
                       module_id = (dest_val & SOC_MEM_FIF_DGPP_MOD_ID_MASK) >>
                           SOC_MEM_FIF_DGPP_MOD_ID_SHIFT_BITS;
                   }
    ...
    || defined(BCM_TRIDENT_SUPPORT)
        if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TRIDENT2X(unit) 
             || SOC_IS_TRIDENT(unit) || SOC_IS_TRIDENT3X(unit)) {
            sal_memset(&dvp2, 0, sizeof(ing_dvp_2_table_entry_t));
        }
#endif /* BCM_TRIDENT2_SUPPORT or BCM_TRIUMPH3_SUPPORT or BCM_TRIDENT_SUPPORT */
        /*Set EGR_DVP_ATTRIBUTE indexed by vp to zero*/
        if (SOC_MEM_IS_VALID(unit, EGR_DVP_ATTRIBUTEm)) {
            BCM_IF_ERROR_RETURN(WRITE_EGR_DVP_ATTRIBUTEm(unit,
                        MEM_BLOCK_ALL, vp, &egr_dvp_attribute));
        }
    ...
    || defined(BCM_TRIDENT_SUPPORT)
        ...
        if (soc_feature(unit,soc_feature_td3_style_mpls)) {
            int entry_type = 1;
            rv = bcmi_cmh_svp_attributes_set(unit, entry_type, vp, TRUE,
                    mpls_port->flags);
            if (rv < 0) {
                goto vpls_cleanup;
            }
...
bcm_tr_mpls_port_add(int unit, bcm_vpn_t vpn, bcm_mpls_port_t *mpls_port)
{
    ...
    if (soc_mem_field_valid(unit, MPLS_ENTRYm, PW_CC_TYPEf)) {
        if (mpls_port->vccv_type >=
            (1 << soc_mem_field_length(unit, MPLS_ENTRYm, PW_CC_TYPEf))) {
            return BCM_E_PARAM;
        }
    }
...
_bcm_tr_mpls_port_delete(int unit, bcm_vpn_t vpn, int vp)
{
    ...
    if ((vpn == BCM_MPLS_VPWS_VPN_INVALID) || (vpn == BCM_MPLS_VPLS_VPN_INVALID)) {
       ...
       _BCM_MPLS_VPN_GET(vpn_id, _BCM_MPLS_VPN_TYPE_VPWS, vpn);
        ...
        if (vp1 != -1) {
         ...
         if ((soc_feature(unit, soc_feature_td3_style_mpls)) &&
                 (soc_feature(unit,
                     soc_feature_multiple_split_horizon_group))) {
             rv = soc_mem_field32_modify(unit, EGR_DVP_ATTRIBUTEm,
                     vp, COMMON__DVP_NETWORK_GROUPf, 0);
             if (rv < 0) {
                 return rv;
             }
         }
        ...
        if (vpless_failover_port) {
            ...
            int dvp_num_nni = 0; /* network side dvp number*/
            ...
            if (vp3_read != -1) {
                ...
                if (soc_feature(unit, soc_feature_generic_dest)) {
                    dest_val = soc_mem_field32_dest_get(unit, SOURCE_VPm,
                            &svp_uni, DESTINATIONf, &dest_type);
                    if (dest_type == SOC_MEM_FIF_DEST_DVP) {
                        dvp_num_nni = dest_val;
                    }
                } else {
        ...
        if (vp == vp1) {
            ...
            if (soc_feature(unit, soc_feature_generic_dest)) {
                ...
                temp_vp3 = soc_SOURCE_VPm_field32_get(unit, &svp2, DVPf);
            }
               ...
               if ((vp2 != -1) && 
                   ...
                   if (soc_feature(unit, soc_feature_generic_dest)) {
                       dest_val = 0;
                       rv = bcmi_td3_soc_mem_fields32_modify(unit,
                               SOURCE_VPm, vp2, dest_field, dest_val);
                   } else {
               ...
               if ((rv >= 0) && (vp3_read != -1)) {
                   ...
                   if (vp1 == temp_vp3) {
                       if (soc_feature(unit, soc_feature_generic_dest)) {
                           dest_val = 0;
                           rv = bcmi_td3_soc_mem_fields32_modify(unit,
                                   SOURCE_VPm, vp3_read, dest_field, dest_val);
                       } else {
                           rv = soc_mem_field32_modify(unit, SOURCE_VPm,
                                   vp3_read, DVPf, 0);
                       }
                ...
                if (rv >= 0 && (failover_port == FALSE)) {
                    if (vp1 != -1) {
                        if (soc_feature(unit, soc_feature_generic_dest)) {
                            dest_val = 0;
                            rv = bcmi_td3_soc_mem_fields32_modify(unit,
                                    SOURCE_VPm, vp1, dest_field, dest_val);
                        } else {
                            rv = soc_mem_field32_modify(unit, SOURCE_VPm,
                                    vp1, DVPf, 0);
                        }
            ...
            if (rv >= 0) {
                if (soc_feature(unit, soc_feature_generic_dest)) {
                    dest_val = 0;
                    rv = bcmi_td3_soc_mem_fields32_modify(unit,
                            SOURCE_VPm, vp3, dest_field, dest_val);
                } else {
                    rv = soc_mem_field32_modify(unit, SOURCE_VPm,
                            vp3, DVPf, 0);
                }
            ...
            if (BCM_SUCCESS(rv) &&
                ...
                if (failover_nh_index > 0) {
                    ...
                    if ((dvp > 0) && (fo_vp != -1)) {
                        if (soc_feature(unit, soc_feature_generic_dest)) {
                            rv = bcmi_td3_soc_mem_fields32_modify(unit,
                                   SOURCE_VPm, dvp, dest_field, fo_vp);
                        } else {
                            rv = soc_mem_field32_modify(unit, SOURCE_VPm,
                                    dvp, DVPf, fo_vp);
                        }
            ...
            if (match_label_delete) {
                ...
                /* Update source vp in mpls entry on primary vp deletion */
                if (fo_vp != -1) {
                    (void) _bcm_esw_mpls_match_failover_update(unit, vp, fo_vp);
                }
                (void) bcm_tr_mpls_match_clear (unit, vp);
            }
        }
        ...
        if (rv >= 0) {
            ...
            if ((vp == vp1) && (vpless_failover_port == FALSE)) {
                ...
                rv = WRITE_SOURCE_VPm(unit, MEM_BLOCK_ALL, vp1, &svp1);
                if (rv >= 0) {
                    ing_dvp_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_table_entry_t));
                    rv = WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, vp1, &dvp_tmp);
                }

                /*Reset CMH SVP ATTRIBUTES*/
                if (soc_feature(unit,soc_feature_td3_style_mpls)) {
                    rv = bcmi_cmh_svp_attributes_set(unit, 2, vp1,
                            FALSE, 0);
                    if (rv < 0) {
                        return rv;
                    }
                ...
                if (rv >= 0 &&
                    ...
                    ing_dvp_2_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_2_table_entry_t));
                    rv = WRITE_ING_DVP_2_TABLEm(unit, MEM_BLOCK_ALL, vp1, &dvp_tmp);
                }
#endif
                ...
                if (rv >= 0) {
                    ing_dvp_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_table_entry_t));
                    rv = WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, vp2, &dvp_tmp);
                }

                /*Reset CMH SVP ATTRIBUTES*/
                if (soc_feature(unit,soc_feature_td3_style_mpls)) {
                    rv = bcmi_cmh_svp_attributes_set(unit, 2, vp2,
                            FALSE, 0);
                    if (rv < 0) {
                        return rv;
                    }
                ...
                if (rv >= 0 &&
                    ...
                    ing_dvp_2_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_2_table_entry_t));
                    rv = WRITE_ING_DVP_2_TABLEm(unit, MEM_BLOCK_ALL, vp2, &dvp_tmp);
                }
#endif
            } else if (vp == vp3) {
                sal_memset(&svp3, 0, sizeof(source_vp_entry_t));
                ...
                if (rv >= 0) {
                    ing_dvp_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_table_entry_t));
                    rv = WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, vp3, &dvp_tmp);
                }

                /*Reset CMH SVP ATTRIBUTES*/
                if (soc_feature(unit,soc_feature_td3_style_mpls)) {
                    rv = bcmi_cmh_svp_attributes_set(unit, 2, vp3,
                            FALSE, 0);
                    if (rv < 0) {
                        return rv;
                    }
                ...
                if (rv >= 0 &&
                    ...
                    ing_dvp_2_table_entry_t dvp_tmp;
                    sal_memset(&dvp_tmp, 0, sizeof(ing_dvp_2_table_entry_t));
                    rv = WRITE_ING_DVP_2_TABLEm(unit, MEM_BLOCK_ALL, vp3, &dvp_tmp);
                }
#endif
                if (vp1 != -1) {
                    if (vp2 != -1) {
                        ...
                        if (soc_feature(unit, soc_feature_generic_dest)) {
                            dest_val = 0;
                            rv = bcmi_td3_soc_mem_fields32_modify(unit, SOURCE_VPm,
                                    vp1, dest_field, dest_val);
                        } else {
    ...
    || defined(BCM_TRIDENT_SUPPORT)
        ...
        if (vpless_failover_port == FALSE) {
            ...
            /*Reset CMH SVP ATTRIBUTES*/
            if (soc_feature(unit,soc_feature_td3_style_mpls)) {
                rv = bcmi_cmh_svp_attributes_set(unit, 1, vp,
                        FALSE, 0);
                if (rv < 0) {
                    return rv;
                }
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        rv = bcmi_egr_dvp_attribute_nwgroup_get(unit, vp, &nw_group);
        if (rv == BCM_E_NONE) {
            mpls_port->network_group_id = nw_group;
        }
    }
#endif
...
#ifdef BCM_TRIDENT3_SUPPORT
    egr_ip_tunnel_mpls_double_wide_entry_t tnl_entry_double;
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_MEM_IS_VALID(unit, EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm)) {
            BCM_IF_ERROR_RETURN(
                    READ_EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm(unit,
                        MEM_BLOCK_ANY,
                        tnl_index, &tnl_entry_double));
...
#if defined(BCM_TRIDENT3_SUPPORT)
    /*SWAP Action is similar to PUSH in TD3*/
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        if (MPLS_INFO(unit)->egr_vc_swap_lbl_action != NULL) {
            *label_action =
                MPLS_INFO(unit)->egr_vc_swap_lbl_action[vc_swap_idx];
        }
    } else
#endif
    {
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if(soc_feature(unit, soc_feature_td3_style_mpls)) {
        label_select = soc_mem_field32_get(unit,
                EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                &vc_entry, LABEL_SELECTf);
    }
#endif
...
bcm_tr_mpls_egress_entry_mac_replace(int unit, int nh_index, bcm_l3_egress_t *egr)
{
    ...
    entry_type = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &egr_nh, nh_entry_type_field);
...
bcm_tr_mpls_l3_label_delete (int unit, int index)
{
    ...
    entry_type = 0; /* Normal */
    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                        nh_entry_type_field, entry_type);
...
bcm_tr_mpls_swap_nh_info_add (int unit, bcm_l3_egress_t *egr, int nh_index, uint32 flags)
{
    ...
    rv = _bcm_tr_mpls_vc_and_swap_table_entry_set(unit, NULL, NULL, egr,
             _BCM_MPLS_EGRESS_LABEL_PRESERVE(unit, egr->mpls_flags) ?
             _BCM_MPLS_ACTION_PRESERVED : _BCM_MPLS_ACTION_SWAP,
             &vc_entry, hw_map_idx, vc_swap_index);
    _BCM_MPLS_CLEANUP(rv);
#if defined(BCM_TRIUMPH2_SUPPORT) \
    ||  defined(BCM_ENDURO_SUPPORT) || defined(BCM_TRIDENT_SUPPORT)
    if (SOC_IS_TRIUMPH2(unit)
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if(soc_feature(unit, soc_feature_td3_style_mpls)) {
        label_select = soc_mem_field32_get(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                &vc_entry, LABEL_SELECTf);
    }
#endif
...
bcm_tr_mpls_l3_nh_info_add(int unit, bcm_mpls_tunnel_switch_t *info, int *nh_index)
{
    ...
    rv = _bcm_tr_mpls_vc_and_swap_table_entry_set(unit, NULL, info, NULL,
         _BCM_MPLS_EGRESS_LABEL_PRESERVE(unit, info->egress_label.flags) ?
         _BCM_MPLS_ACTION_PRESERVED : _BCM_MPLS_ACTION_SWAP,
         &vc_entry, hw_map_idx, egr_nh_info.vc_swap_index);
     _BCM_MPLS_CLEANUP(rv);
...
#if defined(BCM_SWAP_TO_SELF_SUPPORT)
    if (soc_feature(unit, soc_feature_mpls_swap_label_preserve)) {
        uint32 mpls_label_action = 0, label_select = 0;
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            label_select = soc_mem_field32_get(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                   &vc_entry, LABEL_SELECTf);
...
#endif
        {
            mpls_label_action = soc_mem_field32_get(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm,
                                   &vc_entry, MPLS_LABEL_ACTIONf);
        }

        if (label_select || (mpls_label_action == _BCM_MPLS_ACTION_PRESERVED)) {
             info->egress_label.flags |= BCM_MPLS_EGRESS_LABEL_PRESERVE;
        }
    }
    if (soc_feature(unit, soc_feature_td3_style_mpls)) {
        info->egress_label.spl_label_push_type = soc_mem_field32_get(unit,
            EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, &vc_entry,
            SPECIAL_LABEL_PUSH_TYPEf);
    }
...
bcm_tr_mpls_egress_object_modify(int unit, int nh_index, int new_entry_type)
{
    ...
    if ((old_entry_type == 0) && (new_entry_type == 1)) {
        ...
        soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &new_egr_nh,
                            nh_entry_type_field, new_entry_type);
...
bcm_tr_mpls_tunnel_switch_delete_all(int unit)
{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            if ((soc_mem_field32_get(unit, MPLS_ENTRYm, mentp, BASE_VALID_0f) == 0) ||
                (soc_mem_field32_get(unit, MPLS_ENTRYm, mentp, BASE_VALID_1f) == 0)) {
                continue;
            }
        } else {
...
bcm_tr_mpls_exp_map_create(int unit, uint32 flags, int *exp_map_id)
{
    ...
    if (flags & BCM_MPLS_EXP_MAP_INGRESS) {
        ...
        num_exp_map = _bcm_tr_get_ing_mpls_index_size(unit);
        ...
        if (flags & BCM_MPLS_EXP_MAP_WITH_ID) {
            ...
            if (!_BCM_EGR_MPLS_MAP_USED_GET(unit, table_num)) {
                ...
                if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                    BCM_IF_ERROR_RETURN(
                            bcm_td3_qos_map_create(unit, (uint32 *)&index, 64));
                } else
#endif
                {
        ...
        /* EGR_MPLS_EXP_PRI_MAPPINGm profiles indexed by EXP */
         num_exp_map = bcmi_egr_mpls_exp_pri_mapping_num_exp_map_get(unit);
                        ...
                        bcm_mpls_exp_map_t *map)
{
...
#if defined(BCM_TRIDENT3_SUPPORT)
    egr_mpls_pri_mapping_entry_t exp_mpls_pri_entry;
#endif
    egr_mpls_exp_pri_mapping_entry_t exp_pri_entry;
    uint32 ing_pri_entry[SOC_MAX_MEM_FIELD_WORDS]; 
    int max_int_pri = 15;
    soc_mem_t mem;
    soc_field_t pri_field;
    int mode = 0;
    ...
    if ((exp_map_id & _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_MASK) == 
        ...
        rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, index, ing_pri_entry);
        if (BCM_FAILURE(rv)) {
            return rv;
        }
        soc_mem_field32_set(unit, mem, ing_pri_entry, pri_field, map->priority);
        soc_mem_field32_set(unit, mem, ing_pri_entry, CNGf, cng);

        if (SOC_MEM_FIELD_VALID(unit, mem, DSCPf)) {
            if (map->dscp >= (1 << soc_mem_field_length(unit, mem, DSCPf))) {
                return BCM_E_PARAM;
            }
            soc_mem_field32_set(unit, mem, ing_pri_entry, DSCPf, map->dscp);
        }
        rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, index, ing_pri_entry);
        if (BCM_FAILURE(rv)) {
            return rv;
        }
    } else if 
        ...
        if (!_BCM_EGR_MPLS_MAP_USED_GET(unit, table_num)) {
...
#ifdef BCM_TRIDENT3_SUPPORT
            if (soc_feature(unit, soc_feature_td3_style_mpls)) {
                rv = bcm_td3_mpls_combo_add(unit, BCM_QOS_MAP_EGRESS, map, exp_map_id,
                        MPLS_INFO(unit)->egr_mpls_hw_idx[table_num], 64);
...
#endif
            {
                ...
                if (rv == SOC_E_NONE) {
                    /* reference count increment occurs in entry_update only in dma mode,
                     * hence deecrease the profile ref_count increased by entry_update */
                    rv = _bcm_egr_mpls_combo_map_entry_write_mode_get(unit, index, &mode);
                    if ((rv == SOC_E_NONE) && (mode == SOC_PROFILE_MEM_WRITE_DMA_MODE)) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            BCM_IF_ERROR_RETURN(READ_EGR_MPLS_PRI_MAPPINGm(unit, MEM_BLOCK_ANY,
                        index, &exp_mpls_pri_entry));
            soc_EGR_MPLS_PRI_MAPPINGm_field32_set(unit, &exp_mpls_pri_entry,
                    NEW_PRIf, map->pkt_pri);
            soc_EGR_MPLS_PRI_MAPPINGm_field32_set(unit, &exp_mpls_pri_entry,
                    NEW_CFIf, map->pkt_cfi);
                        ...
                        bcm_mpls_exp_map_t *map)
{
    ...
    if ((exp_map_id & _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_MASK) == 
        ...
        map->priority = soc_mem_field32_get(unit, mem, ing_pri_entry, pri_field);
        cng = soc_mem_field32_get(unit, mem, ing_pri_entry, CNGf);
        map->color = _BCM_COLOR_DECODING(unit, cng);
        if (SOC_MEM_FIELD_VALID(unit,mem,DSCPf)) {
            map->dscp = soc_mem_field32_get(unit, mem, ing_pri_entry, DSCPf);
        }
    } else if
        ((exp_map_id & _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_MASK) == 
         _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_EGRESS) {
        ...
        if (soc_feature(unit, soc_feature_td3_style_mpls)) {
            BCM_IF_ERROR_RETURN(READ_EGR_MPLS_PRI_MAPPINGm(unit, MEM_BLOCK_ANY,
                        index, &exp_mpls_pri_entry));
            map->pkt_pri = soc_mem_field32_get(unit, EGR_MPLS_PRI_MAPPINGm,
                    &exp_mpls_pri_entry, NEW_PRIf);
            map->pkt_cfi = soc_mem_field32_get(unit, EGR_MPLS_PRI_MAPPINGm,
                    &exp_mpls_pri_entry, NEW_CFIf);
        } else
#endif
        {
            BCM_IF_ERROR_RETURN(READ_EGR_MPLS_EXP_PRI_MAPPINGm(unit, MEM_BLOCK_ANY,
    ...
    for (i = 0; i < num_trunk; i++) {
        if (mpls_info->trunk_vp_map[i] != -1) {
            ...
            if (soc_feature(unit, soc_feature_mpls_special_label)) {
                ...
                for (i = 0; i < num_entry; i++) {
                    if (ptr[i].valid) {
                        LOG_CLI((BSL_META_U(unit,"Index = %d : Label Value = 0x%x    Label Mask= 0x%x   Label Type = %s  ref_count= %d\n"),
                           i, ptr[i].label_value, ptr[i].label_mask, label_type_array[ptr[i].label_type], ptr[i].ref_count));
                    }
                }
            }
#endif
...
/*
 * Function:
 *        bcm_td3_mpls_php_info_add
 * Purpose:
 *        Add VC_AND_SWAP_TABLE for a specific EGR_NH Index
 * Parameters:
 *           IN :  Unit
 *           IN/OUT : Tunnel Switch Pointer
 *           IN :  nh_index
...
int
bcmi_td3_mpls_php_vcswap_info_add(int unit, bcm_mpls_tunnel_switch_t *php_info, int *nh_index)
{
    bcm_l3_egress_t nh_info;
    _bcm_tr_egr_nh_info_t egr_nh_info;
    egr_mpls_vc_and_swap_label_table_entry_t vc_entry;
    int rv = BCM_E_NONE, temp_nh_index = -1;
    uint32 mpath_flag=0;
    egr_nh_info.vc_swap_index = -1;
     ...
     * increment egress object reference count. 
     */
    if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, php_info->egress_if)) {
        rv = bcm_xgs3_get_nh_from_egress_object(unit, php_info->egress_if,
                &mpath_flag, 1, &temp_nh_index);
    ...
    rv = _bcm_tr_mpls_get_vc_and_swap_table_index(unit, 0, NULL, php_info, NULL,
            _BCM_MPLS_ACTION_PRESERVED, &egr_nh_info.vc_swap_index);

    BCM_IF_ERROR_RETURN(rv);

    /* Program the MPLS_VC_AND_SWAP table entry */
    rv = _bcm_tr_mpls_vc_and_swap_table_entry_set(unit, NULL, php_info, NULL,
            _BCM_MPLS_ACTION_PRESERVED,
            &vc_entry, 0, egr_nh_info.vc_swap_index);
    _BCM_MPLS_CLEANUP(rv);
    ...
    if (egr_nh_info.vc_swap_index != -1) {
        ...
        rv = soc_mem_field32_modify(unit, EGR_L3_NEXT_HOPm, *nh_index, 
                MPLS__VC_AND_SWAP_INDEXf, egr_nh_info.vc_swap_index);
        _BCM_MPLS_CLEANUP(rv);
    }
...
/*
 * Function:
 *       bcmi_td3_mpls_php_vcswap_info_delete 
 * Purpose:
 *        Delete  VC_AND_SWAP_TABLE for a specific EGR_NH Index
 * Parameters:
 *           IN :  Unit
 *           IN :  nh_index
 * Returns:
...
int
bcmi_td3_mpls_php_vcswap_info_delete(int unit, int nh_index,int type)
{
    int rv = BCM_E_NONE, old_vc_swap_idx = -1;
    egr_l3_next_hop_entry_t egr_nh;
    ...
    /* Clear VC_SWAP_INDEX in EGR_L3_NEXT_HOP entry */
    if (old_vc_swap_idx != -1) {
        rv = soc_mem_field32_modify(unit, EGR_L3_NEXT_HOPm, nh_index, 
                MPLS__VC_AND_SWAP_INDEXf, 0);
