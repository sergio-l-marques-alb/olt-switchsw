...
#define PRP_FORWARDING_ENABLE(_u_)\
        (TSN_BKINFO(_u_)->gh2_prp_forwarding_bk_info.prp_forwardin_enable)
    ...
    bcmi_tsn_sr_seqnum_slice_t slice)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == slice) {
        return BCM_E_PARAM;
    }
    ...
    bcmi_tsn_sr_seqnum_slice_t *slice)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == slice || winsize <= 0 ||
        !TSN_IS_POWER_OF_2(winsize)) {
        return BCM_E_PARAM;
    }
    ...
    int *actual)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == history || NULL == slice ||
    ...
    uint32 reset_mode)
{
    ...
    if (reset_mode == BCM_TSN_CONTROL_SR_SEQNUM_WINDOW_RESET_MODE_HW1) {
        ...
        if (winsize == 1) {
            /* Special case: 1 bit (it's the firts bit in this byte) */
            history[0] = 0;
        } else {
            sal_memset(history, 0xff, BITS2BYTES(winsize));
            history[BITS2BYTES(winsize) - 1] &= ~0x80;
        }
    } else if (reset_mode == BCM_TSN_CONTROL_SR_SEQNUM_WINDOW_RESET_MODE_SW) {
        /* Considered done (nothing to do) for SW reset mode */
        return BCM_E_NONE;
    } else {
...
bcmi_gh2_tsn_sr_flows_get_num_flows(int unit, int *rx_flows, int *tx_flows)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == rx_flows || NULL == tx_flows) {
        return BCM_E_PARAM;
    }
    ...
    int *fidx)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == is_tx || NULL == fidx) {
        return BCM_E_PARAM;
    }
    ...
    bcm_tsn_sr_rx_flow_config_t *cfg)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == cfg) {
        return BCM_E_PARAM;
    }
    ...
    bcmi_tsn_sr_rx_flow_t *config)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == config || idx < 0 ||
    ...
    /* Sequence numer history window size (also already validated) */
    if (cfg->seqnum_history_win_size == 1) {
        /* Special case: 1-bit */
        v = 0;
    } else {
    ...
    bcm_tsn_sr_tx_flow_status_t *status)
{
    ...
    if (!SOC_UNIT_VALID(unit) || NULL == status || idx < 0 ||
        idx >= SRFLOWS_BKINFO(unit).max_flows_tx) {
        return BCM_E_PARAM;
    }
    ...
    bcm_tsn_sr_port_config_t *new_config)
{
    ...
    for (ptab = 0; ptab < COUNTOF(mem_list); ptab++) {
        ...
        rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, port, entry);
        assert(SOC_E_NONE == rv);
    }
}
    ...
    int init_error)
{
    ...
    if (0 == soc_property_get(unit, spn_SR_PRP_ENABLE, 0)) {
        return;
    }
    ...
    int unit)
{
    ...
    /* skip prp-init if sdk not enable this feature */
    PRP_FORWARDING_ENABLE(unit) = soc_property_get(unit, spn_SR_PRP_ENABLE, 0);
    ...
    /* Get prp loopback port */
    BCM_IF_ERROR_RETURN(soc_gh2_prp_loopback_port_get(unit, &port));
    ...
    PRP_FORWARDING_ERROR_RETURN(
        bcm_esw_port_gport_get(
            unit,
            PRP_FORWARDING_REDIRECT_PORT(unit),
            &PRP_FORWARDING_REDIRECT_GPORT(unit)));
    PRP_FORWARDING_ERROR_RETURN(
        bcmi_gh2_tsn_sr_port_config_get(
            unit, PRP_FORWARDING_REDIRECT_GPORT(unit), &old_config));
    ...
    bcm_tsn_flow_config_t *config)
{
    ...
    /* parameter check */
    if (!SOC_UNIT_VALID(unit) || NULL == config || idx < 0 ||
        idx >= TSN_FLOWS_MAX_NUM_OF_FLOWS) {
        return BCM_E_PARAM;
    }
    ...
    uint32 *hw_index)
{
    ...
    *hw_index = sw_idx * GH2_PRI_MAP_TSN_ENT_SIZE;
    ...
    uint32 *sw_idx)
{
    ...
    /* Parameter validation */
    if (NULL == sw_idx ||
        GH2_PRI_MAP_TSN_MASK & hw_index ||
        GH2_PRI_MAP_TSN_TAB_SIZE * GH2_PRI_MAP_TSN_ENT_SIZE <= hw_index) {
        return BCM_E_PARAM;
    }
    *sw_idx = hw_index >> GH2_PRI_MAP_TSN_SHIFT;
    ...
    int val)
{
    ...
    for (ptab = 0; ptab < COUNTOF(mem_list); ptab++) {
        ...
        PBMP_ITER(port_pbmp, port) {
            ...
            MEM_LOCK(unit, mem);
            rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, port, &data);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_TSN,
                          (BSL_META_U(unit, "Failed to read mem[%s]"),
                                      SOC_MEM_NAME(unit, mem)));
            ...
            rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, port, &data);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_TSN,
                          (BSL_META_U(unit, "Failed to write mem[%s]"),
                                      SOC_MEM_NAME(unit, mem)));
...
bcmi_gh2_tsn_mtu_init(int unit)
{
    ...
    /* the mtu config is not used, but to occupied profile memory id 0 as invalid */
    config.size = 0;
    config.drop = 0;
...
bcmi_gh2_tsn_stu_init(int unit)
{
    ...
    /* the stu config is not used, but to occupied profile memory id 0 as invalid */
    config.size = 0;
    config.drop = 0;
    ...
    bcm_tsn_pri_map_config_t *config)
{
    ...
    for (i = 0; i < config->num_map_entries; i++) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, EGR_MTU_CHECKm,
                                MTU_PROFILE_INDEXf)) {
            rv = soc_mem_field32_get(unit, EGR_MTU_CHECKm, &egr_entry,
                                     MTU_PROFILE_INDEXf);
        } else {
            return BCM_E_INTERNAL;
        }
        BCM_IF_ERROR_RETURN(rv);
        config->map_entries[i].profile_id = rv;
    }
    return BCM_E_NONE;
}
    ...
    bcm_tsn_pri_map_config_t *config)
{
    ...
    for (i = 0; i < config->num_map_entries; i++) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, EGR_STU_CHECKm,
                                STU_PROFILE_INDEXf)) {
            rv = soc_mem_field32_get(unit, EGR_STU_CHECKm, &egr_entry,
                                     STU_PROFILE_INDEXf);
        } else {
            return BCM_E_INTERNAL;
        }
...
bcmi_gh2_tsn_event_flow_stat_get(
    ...
    uint32 flow_number)
{
    int             min, max, rv, fidx, min_cd_val = 0, cd_val;
    ...
    if (flowstate != NULL && (flow_number < bkinfo->max_rx_flows)) {
        return BCM_E_PARAM;
    }
...
bcmi_gh2_tsn_event_poll(int unit)
{
    ...
    if (bkinfo->window_reset_mode ==
        ...
        if (bkinfo->sn_reset_min_cd == 0) {
            ...
            (void)bcmi_gh2_tsn_event_flow_stat_get(
                unit, bcmTsnEventTypeSrSeqNumWindowAgeOut, NULL, 0);
        }
    ...
    if (!SHR_BITNULL_RANGE(bkinfo->wrong_lana_fbmp, 0, bkinfo->max_rx_flows)) {
        ...
        if (bkinfo->wla_tick <= 0) {
            (void)bcmi_gh2_tsn_event_mask(unit, bcmTsnEventTypeSrWrongLanA, 1);
            bkinfo->wla_tick = 0;
        }
    }
    if (!SHR_BITNULL_RANGE(bkinfo->wrong_lanb_fbmp, 0, bkinfo->max_rx_flows)) {
        /* check the wrong lana ageout ticks to enable the interrupt mask */
        if (bkinfo->wlb_tick <= 0) {
            (void)bcmi_gh2_tsn_event_mask(unit, bcmTsnEventTypeSrWrongLanB, 1);
            bkinfo->wlb_tick = 0;
        }
    }
    EVENT_MGMT_UNLOCK(unit);
    ...
    uint8 ep_cnt_err)
{
    ...
    if (ip_cnt_err) {
        multiple = 1;
        idx = -1;
        BCM_IF_ERROR_RETURN(READ_IP2_SR_TH_INTR_STATUSr(unit, &rval32));
        clear_field = rval32;
        for (i = 0; i < COUNTOF(gh2_event_cnt_thr_ip); i++) {
            cnt_th_info = &gh2_event_cnt_thr_ip[i];
            if (soc_reg_field_get(unit, IP2_SR_TH_INTR_STATUSr, rval32,
                                  cnt_th_info->sts_field)) {
                if (ce_cb) {
                    (void)ce_cb(unit, cnt_th_info->stat_type,
                                cnt_th_info->th_src, multiple, idx);
                }
                soc_reg_field_set(unit, IP2_SR_TH_INTR_STATUSr, &clear_field,
                                  cnt_th_info->sts_field, 0);
            }
        }
        /* Clear status */
        BCM_IF_ERROR_RETURN(WRITE_IP2_SR_TH_INTR_STATUSr(unit, clear_field));
    }
    if (ep_cnt_err) {
        multiple = 1;
        idx = -1;
