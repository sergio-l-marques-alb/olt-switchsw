...
/* Clear all SVM_METER_TABLEm */
STATIC int
bcmi_gh2_global_meter_meter_table_clear(int unit)
{
    ...
    rv = soc_mem_clear(unit, meter_mem, MEM_BLOCK_ALL, TRUE);
...
/* Clear a range (base_index ~ base_index + numbers) of SVM_METER_TABLEm */
STATIC int
bcmi_gh2_global_meter_meter_table_range_clear(
    int unit,
    int base_index,
    uint32 numbers)
{
    ...
    soc_mem_lock(unit, meter_mem);
    rv = soc_mem_write_range(unit, meter_mem, MEM_BLOCK_ALL, base_index,
...
/* TCAM fields mapping of bcm_policer_group_mode_t */
    ...
    pkt_tcam_attr_bits_t *pkt_attr_bits_v)
{
    ...
    for (attr = bcmPolicerGroupModeAttrFieldIngressColor;
        ...
        if (pkt_attr_selector_enable &&
            ...
            GH2_POL_FUNCTION_DEBUG(unit, "attr %d", attr);
            GH2_POL_FUNCTION_DEBUG(unit, "value 0x%x", value);
            GH2_POL_FUNCTION_DEBUG(unit, "value_mask 0x%x", value_mask);

            bcmi_gh2_group_mode_attr_value_remapping(
                attr, value, value_mask, &hw_value, &hw_value_mask);
            GH2_POL_FUNCTION_DEBUG(unit, "hw_value 0x%x", hw_value);
            GH2_POL_FUNCTION_DEBUG(unit, "hw_value_mask 0x%x", hw_value_mask);
        } else {
            hw_value = 0;
            hw_value_mask = 0;
        }
        ...
        /* Write value_mask */
        field = gh2_policer_group_mode_field_list[attr * 2 + 1];
        if (SOC_MEM_FIELD_VALID(unit, meter_mem, field)) {
            soc_mem_field_width_fit_set(
                unit, meter_mem, (uint32 *)&tcam, field, &hw_value_mask);
        }
    }
    ...
    int skip_read)
{
    ...
    BCM_IF_ERROR_RETURN(
        soc_mem_write(unit, mem, MEM_BLOCK_ALL, hw_idx, mem_data));
...
STATIC const bcmi_global_meter_dev_info_t bcmi_gh2_global_meter_dev_info = {
    ...
    bcmi_gh2_global_meter_meter_table_clear,        /* mem_clear */
    bcmi_gh2_global_meter_meter_table_range_clear,  /* mem_range_clear */
