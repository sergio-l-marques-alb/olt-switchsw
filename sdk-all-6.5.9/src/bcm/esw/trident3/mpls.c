...

{
    ...
    for (; i<2; i++) {
        ...
        rv = bcmi_mpls_special_label_egress_delete(unit,
            ...
            bcm_mpls_special_label_push_type_t push_type)
{
    ...
    if (element->gport != BCM_GPORT_INVALID) {
        ...
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, element->egr_obj_id) ||
            ...
            if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, element->egr_obj_id)) {
                nh_index = element->egr_obj_id - BCM_XGS3_EGRESS_IDX_MIN;
            } else {
                nh_index = element->egr_obj_id - BCM_XGS3_DVP_EGRESS_IDX_MIN;
            }
        } else {
            return BCM_E_PARAM;
        }
 ...
 */
int
bcmi_mpls_special_label_get_slot(int unit, int label_type, uint32 mpls_label,
            ...
            bcm_mpls_special_label_t label_info)
{
    ...
    /* Sandeep:  Still thinking whether to really check for mask. */
    ...
    BCM_IF_ERROR_RETURN(bcmi_mpls_special_label_get_slot(unit, label_type,
