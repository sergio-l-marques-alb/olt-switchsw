        ...
        int clear_action)
{
    ...
    if (SOC_SUCCESS(rv)) {
        soc_mem_field32_set(unit, mem, entry, CTRL_PROFILE_IDX_1f, 
                             ctrl_profile1);
        ...
        int *replaced_vp)
{
    ...
    if (info->valid_elements & BCM_FLOW_MATCH_FLOW_PORT_VALID) {
        if (_BCM_FLOW_IS_FLEX_VIEW(info)) {
            ...
            if (info->flow_handle == SOC_FLOW_DB_FLOW_ID_LEGCAY_VXLAN) {
                if (new_entry_add == FALSE && replaced_vp != NULL) {
                    *replaced_vp = soc_mem_field32_get(unit, mem, entry, 
                                           VXLAN_SIP__SVPf);
                }
                soc_mem_field32_set(unit, mem, entry, VXLAN_SIP__SVPf, vp);
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT_VLAN ) {
        ...
        if (info->valid_elements & BCM_FLOW_MATCH_SIP_V6_VALID) {
            if (SOC_MEM_FIELD_VALID(unit,mem_id,IPV6__SIPf)) {
                ...
                _bcm_flow_frag_field_info_t f_info[2];
                uint32 ip6_value[_BCM_FLOW_IPV6_ADDR_WORD_LEN];
                uint32 ip6_field[_BCM_FLOW_IPV6_ADDR_WORD_LEN];

                /* check if any fragment field */
                BCM_IF_ERROR_RETURN(_bcm_flow_fragment_field_info_get(unit, mem_id,
                                 "IPV6__SIP", f_info));

                /* validate the max bit position */
                if (((f_info[0].pos_max/32) >= _BCM_FLOW_IPV6_ADDR_WORD_LEN) ||
                    ((f_info[1].pos_max/32) >= _BCM_FLOW_IPV6_ADDR_WORD_LEN)) {
                    return BCM_E_INTERNAL;
                }
                _bcm_flow_ip6_addr_t_2_mem_field(info->sip6, ip6_value);
                BCM_IF_ERROR_RETURN(_bcm_flow_usr_to_frag_field_convert(ip6_value,
                     f_info[0].pos_min, f_info[0].pos_max, ip6_field));
                soc_mem_field_set(unit, mem_id, entry, f_info[0].id, ip6_field);
                BCM_IF_ERROR_RETURN(_bcm_flow_usr_to_frag_field_convert(ip6_value,
                     f_info[1].pos_min, f_info[1].pos_max, ip6_field));
                soc_mem_field_set(unit, mem_id, entry, f_info[1].id, ip6_field);
            }
        }
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT) {
        ...
        if (_BCM_FLOW_IS_FLEX_VIEW(info) &&
            ...
            rv = _bcm_flow_match_sgpp_ctrl_set(unit,info,FALSE);
            BCM_IF_ERROR_RETURN(rv);
        }
    ...
    if (!(info->options & BCM_FLOW_MATCH_OPTION_REPLACE)) {
        ...
        } else if (view_info.mem == MPLS_ENTRYm) {
    ...
    bcm_flow_logical_field_t *field)
{
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_PORT) {
        ...
        if (_BCM_FLOW_IS_FLEX_VIEW(info) &&
            info->valid_elements & BCM_FLOW_MATCH_PORT_VALID) {
            rv = _bcm_flow_match_sgpp_ctrl_set(unit,info,TRUE);
            BCM_IF_ERROR_RETURN(rv);
        }
                             ...
                             bcm_flow_logical_field_t *field)
{
        ...
        if (mem == VLAN_XLATE_1_DOUBLEm || mem == VLAN_XLATE_2_DOUBLEm) {
            ...
            switch (key_type) {
                case _BCM_FLOW_MATCH_KEY_TYPE_VXLAN_VNID:
                    ...
                    info->flow_handle = SOC_FLOW_DB_FLOW_ID_LEGCAY_VXLAN;
    ...
    if (info->criteria == BCM_FLOW_MATCH_CRITERIA_SIP) {
        ...
        rv = _bcm_flow_usr_logical_field_get(unit,
                             mem,
                             SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY,
        ...
        if (SOC_MEM_FIELD_VALID(unit,mem,IPV6__SIPf)) { 
            ...
            _bcm_flow_frag_field_info_t f_info[2];
            uint32 ip6_frag[_BCM_FLOW_IPV6_ADDR_WORD_LEN];
            uint32 ip6_frag1[_BCM_FLOW_IPV6_ADDR_WORD_LEN];
            uint32 ip6_field[_BCM_FLOW_IPV6_ADDR_WORD_LEN];

            /* check fragment fields */
            rv = _bcm_flow_fragment_field_info_get(unit, mem, "IPV6__SIP", f_info);
            if (rv != SOC_E_NONE && rv != SOC_E_NOT_FOUND) {
                return rv;
            }
            if (rv == SOC_E_NONE) {
                ...
                soc_mem_field_get(unit, mem, entry, f_info[0].id, ip6_frag);
                soc_mem_field_get(unit, mem, entry, f_info[1].id, ip6_frag1);
                sal_memset(ip6_field,0,sizeof(ip6_field));
                BCM_IF_ERROR_RETURN(_bcm_flow_frag_to_usr_field_convert(ip6_field,
                                f_info[0].pos_min,f_info[0].pos_max, ip6_frag));
                BCM_IF_ERROR_RETURN(_bcm_flow_frag_to_usr_field_convert(ip6_field,
                                f_info[1].pos_min,f_info[1].pos_max, ip6_frag1));
                _bcm_flow_mem_field_2_ip6_addr_t(info->sip6, ip6_field);
            } 
            info->valid_elements |= BCM_FLOW_MATCH_SIP_V6_VALID;
        }
