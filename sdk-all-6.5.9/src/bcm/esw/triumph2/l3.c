                                ...
                                soc_tunnel_term_t *entry)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, L3_TUNNELm, MODEf)) {
        ...
        if ((tnl_info->type == bcmTunnelTypeWlanWtpToAc) ||
            ...
            /* Ignore UDP checksum */
            if (soc_mem_field_valid(unit, L3_TUNNELm, IGNORE_UDP_CHECKSUMf)) {
                soc_L3_TUNNELm_field32_set(unit, entry_ptr, IGNORE_UDP_CHECKSUMf, 0x1);
            }
            ...
            if (soc_mem_field_valid(unit, L3_TUNNELm, CTRL_PKTS_TO_CPUf)) {
...
bcm_tr2_l3_ecmp_defragment_no_lock(int unit, int ecmp_level)
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        /* To be fixed by module owner - skip for swtich control bring up for now */
        return BCM_E_NONE;
    }
    if (!soc_mem_field_valid(unit, L3_ECMP_COUNTm, BASE_PTRf) &&
        !soc_mem_field_valid(unit, L3_ECMP_COUNTm, BASE_PTR_0f)) {
