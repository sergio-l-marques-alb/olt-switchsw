 ...
 * 1bit (reserved) + 3-bit bcm_tsn_pri_map_type + 12-bit (reserved)
 * + 16-bit sw_idx
 */
...
#define SW_IDX_TO_MAP_ID(_id_, _map_type_) \
        ((_id_) | (_map_type_) << BCMI_TSN_PRI_MAP_TYPE_SHIFT)

#define PRI_MAP_TYPE_GET(_id_) \
        ((_id_) >> BCMI_TSN_PRI_MAP_TYPE_SHIFT)
...
bcmi_esw_tsn_sr_mac_proxy_reload(int unit)
{
    ...
    /* Refresh ref_count for auto learn group usage (TBD) */
...
bcmi_esw_tsn_egress_mtu_reload(int unit)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        for (idx = 0; idx < ctrl_info->logical_port_num; idx++) {
            ...
            bcmi_esw_tsn_pri_map_mtu_stu_wb_set(unit,
                bk_info->tsn_mtu_bk_info.pri_map_port[idx], &config);
        }
...
bcmi_esw_tsn_egress_stu_reload(int unit)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        for (idx = 0; idx < ctrl_info->logical_port_num; idx++) {
            ...
            bcmi_esw_tsn_pri_map_mtu_stu_wb_set(unit,
                bk_info->tsn_stu_bk_info.pri_map_port[idx], &config);
        }
    ...
    int unit)
{
    ...
    return sr_port_config->sr_port_prp_forwarding_wb_reload(unit, scache_handle);
}
...
STATIC int bcmi_esw_tsn_sr_auto_learn_init(int unit);
    ...
    for (flow_type = bcmi_sr_auto_learn_flow_tx;
        ...
        for (i = 0; i < flowset_cnt[flow_type]; i++) {
            if (0 != flowset[flow_type][i]) {
                (void)bcm_esw_tsn_sr_flowset_destroy(unit, flowset[flow_type][i]);
            }
        }
    ...
    bcm_tsn_sr_auto_learn_config_t *config)
{
    ...
    if (TRUE == enable) {
        ...
        bcmi_esw_tsn_sr_auto_learn_free_flowset(
                unit, bk_data->flowsets, bk_data->flowsets_cnt);
...
bcmi_esw_tsn_sr_auto_learn_sync(int unit)
{
    ...
    if (FALSE == al_bk_info->initialized) {
        /* 1st initialization hasn't been trigger */
        return BCM_E_NONE;
    }
    ...
    /* Size of bcmi_esw_tsn_sr_auto_learn_bk_t.group_data to be stored */
    group_data_size =
        sizeof(bcmi_esw_tsn_sr_auto_learn_group_bk_data_t) * table_size;
    data_size = group_data_size;

    /* Size of bcmi_esw_tsn_sr_auto_learn_bk_t.data to be stored */
    for (flow_type = bcmi_sr_auto_learn_flow_tx;
         flow_type < bcmi_sr_auto_learn_flow_count;
         flow_type++) {
        data_size +=
            bk_data->flowsets_cnt[flow_type] * sizeof(bcm_tsn_sr_flowset_t);
    }
    ...
    SOC_IF_ERROR_RETURN(
        _bcm_esw_scache_ptr_get(unit,
                                scache_handle,
                                FALSE,
                                data_size,
                                &al_state,
                                BCM_TSN_WB_DEFAULT_VERSION,
                                NULL));
    ...
    /* Update SW bookkeeping database */
    AUTO_LEARN_BK_LOCK(unit);
    /* Store bcmi_esw_tsn_sr_auto_learn_bk_t.group_data */
    sal_memcpy(al_state, bk_group_data, data_size);

    /* Store bcmi_esw_tsn_sr_auto_learn_bk_t.data */
    for (flow_type = bcmi_sr_auto_learn_flow_tx;
         flow_type < bcmi_sr_auto_learn_flow_count;
         flow_type++) {
        sal_memcpy(al_state, bk_data->flowsets[flow_type],
            bk_data->flowsets_cnt[flow_type] * sizeof(bcm_tsn_sr_flowset_t));
    }
    AUTO_LEARN_BK_UNLOCK(unit);
...
/*
 * Function:
 *   bcmi_esw_tsn_sr_auto_learn_reinit
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_tsn_sr_auto_learn_reinit(int unit)
{
    bcm_error_t rv = BCM_E_UNAVAIL;
    int enabled;
    bcmi_esw_tsn_bk_info_t *bk_info = NULL;
    ...
    bcmi_esw_tsn_sr_auto_learn_bk_data_t *bk_data = NULL;
    bcmi_tsn_sr_auto_learn_flow_t flow_type;
    uint32 ctrl_current_fid, ctrl_last_fid;
    uint32 ctrl_id_size[bcmi_sr_auto_learn_flow_count];
    bcm_tsn_sr_tx_flow_config_t tx_flow_config;
    bcm_tsn_sr_rx_flow_config_t rx_flow_config;
    bcmi_esw_tsn_sr_auto_learn_group_bk_data_t *bk_group_data;
    uint32 table_size;
    int data_size = 0;
    uint8 *al_state = NULL;
    ...
    rv = _bcm_esw_scache_ptr_get(unit, scache_handle, FALSE, 0,
            &al_state, BCM_TSN_WB_DEFAULT_VERSION, NULL);
    ...
    sal_memcpy(bk_group_data, al_state, data_size);
    ...
    for (flow_type = bcmi_sr_auto_learn_flow_tx;
         ...
         flow_type++) {
        rv = al_dev_info->flow_id_ctrl_get(
                 unit,
                 flow_type,
                 &ctrl_current_fid,
                 &ctrl_last_fid,
        ...
        if (bcmi_sr_auto_learn_flow_tx == flow_type) {
            bk_data->config.num_tx_flows =
                ctrl_id_size[bcmi_sr_auto_learn_flow_tx];

            /* Convert hardware SR flow ID to software SR flow ID */
            rv = bcmi_esw_tsn_sr_sw_flow_id_get(unit,
                    ctrl_current_fid, &ctrl_current_fid);
            if (BCM_FAILURE(rv)) {
                AUTO_LEARN_UNLOCK(unit);
            }
            ...
            rv = bcm_esw_tsn_sr_tx_flow_config_get(unit,
                    ctrl_current_fid, &tx_flow_config);
            if (BCM_FAILURE(rv)) {
                AUTO_LEARN_UNLOCK(unit);
            }

            sal_memcpy(&(bk_data->config.tx_flow_config),
                       &tx_flow_config,
                       sizeof(bcm_tsn_sr_tx_flow_config_t));
        } else if (bcmi_sr_auto_learn_flow_rx == flow_type) {
            bk_data->config.num_rx_flows =
                ctrl_id_size[bcmi_sr_auto_learn_flow_rx];
            ...
            rv = bcm_esw_tsn_sr_rx_flow_config_get(unit,
                    ctrl_current_fid, &rx_flow_config);
            if (BCM_FAILURE(rv)) {
                AUTO_LEARN_UNLOCK(unit);
            }

            sal_memcpy(&(bk_data->config.rx_flow_config),
                       &rx_flow_config,
                       sizeof(bcm_tsn_sr_rx_flow_config_t));
        }
    }
    ...
    for (flow_type = bcmi_sr_auto_learn_flow_tx;
        ...
        TSN_ALLOC(unit, bk_data->flowsets[flow_type],
                  ...
                  sizeof(bcm_tsn_sr_flowset_t) * ctrl_id_size[flow_type],
                  "bk record Rx/Tx Flowset", 0, rv);
        sal_memcpy(&(bk_data->flowsets[flow_type]), al_state,
            bk_data->flowsets_cnt[flow_type] * sizeof(bcm_tsn_sr_flowset_t));
    }
...
bcmi_esw_tsn_pri_map_cmp(
    ...
    uint8 *equal)
{
    int compare_flow, compare_prof, compare_intp;
    ...
    bcm_tsn_pri_map_t *map_id)
{
    ...
    if (pm_bk_info->valid_count[map_type] == pm_dev_info->entry_size) {
        PRI_MAP_BK_UNLOCK(unit);
        return BCM_E_RESOURCE;
    }
 ...
 *      map_id - (IN) Map ID
 * Returns:
 *      BCM_E_NONE - Success on destorying the given map_id
 */
STATIC int
bcmi_esw_tsn_pri_map_destroy_map_id(
    int unit,
...
/*
 * Function:
 *   bcmi_esw_tsn_pri_map_tsn_sr_reinit
 * Purpose:
 *   Restore bookkeeping information of TSN & SR priority map resource.
 * Parameters:
 *   unit - (IN) unit number
 ...
 */
STATIC int
bcmi_esw_tsn_pri_map_tsn_sr_reinit(int unit)
{
    bcm_error_t rv = BCM_E_UNAVAIL;
    uint32 sw_idx;
    bcmi_esw_tsn_bk_info_t *bk_info = NULL;
    ...
    /* only recovery TSN & SR since MTU/STU wb is done by MTU/STU module */
    for (map_type = bcmTsnPriMapTypeTsnFlow;
         map_type <= bcmTsnPriMapTypeSrFlow;
         map_type++) {
...
bcmi_esw_tsn_reinit(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_tsn_sr)) {
         ...
         * into software data structures.
         */
        rv = bcmi_esw_tsn_sr_auto_learn_reinit(unit);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_TSN,
                      (BSL_META_U(unit,
    ...
    rv = bcmi_esw_tsn_pri_map_tsn_sr_reinit(unit);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TSN,
                  (BSL_META_U(unit,
    ...
    uint32 arg)
{
    if (type == bcmTsnControlEgressMtuPriorityMap) {
        if (soc_feature(unit, soc_feature_tsn_mtu_stu)) {
            (void)bcmi_esw_tsn_mtu_profile_ref_inc(unit, arg);
        ...
        if (soc_feature(unit, soc_feature_tsn_mtu_stu)) {
            (void)bcmi_esw_tsn_stu_profile_ref_inc(unit, arg);
 ...
 */
STATIC int
bcmi_esw_tsn_event_rx_flow_add(int unit, int idx)
{
    if (TSN_EVENT_DEVINFO(unit)->update_rx_flow) {
        BCM_IF_ERROR_RETURN(
            TSN_EVENT_DEVINFO(unit)->update_rx_flow(unit, -1, idx, 1));
    }
    return BCM_E_UNAVAIL;
}

STATIC int
bcmi_esw_tsn_event_rx_flow_delete(int unit, int idx)
{
    if (TSN_EVENT_DEVINFO(unit)->update_rx_flow) {
        BCM_IF_ERROR_RETURN(
            TSN_EVENT_DEVINFO(unit)->update_rx_flow(unit, -1, idx, 0));
    }
    return BCM_E_UNAVAIL;
}
    ...
    int hw_flow_id)
{
    ...
    if ((event == bcmTsnEventTypeSrWrongLanA) ||
        ...
        if (multiple) {
            /* If multiple error, scan the active rx flow and get the stat */
    ...
    if (th_src == bcmTsnStatThresholdSourcePort) {
        ...
        BCM_PBMP_ITER(pbmp, p) {
            ...
            if (COMPILER_64_GT(hw_value, th_value) &&
                COMPILER_64_NE(hw_value, sw_value)) {
