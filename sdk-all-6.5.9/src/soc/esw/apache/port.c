...
#define AP_PORTS_PER_TSC        4
#define AP_NUM_PGWS             2
#define AP_TSCS_PER_PGW         9
...
typedef struct ap_phy_port_s {
    ...
    int                  pipe;         /* unused */
    int                  sisters_ports[AP_PORTS_PER_TSC];
...
typedef struct soc_info_misc_s {
    ...
    int              port_num_lanes[SOC_MAX_NUM_PORTS];   /* Current speed */
                                            ...
                                            portmod_pbmp_t phy_pbmp)
{
    soc_info_t *si = &SOC_INFO(unit);
    soc_port_resource_t *pr;
    int xlport_mask[AP_NUM_PGWS][AP_TSCS_PER_PGW];
    int xlport_validated[AP_NUM_PGWS][AP_TSCS_PER_PGW];
    int pgw;
    int tsc;
    int port_index;
    int phy_port;
    ...
    /* Clear data */
    for (pgw = 0; pgw < AP_NUM_PGWS; pgw++) {
        for (tsc = 0; tsc < AP_TSCS_PER_PGW; tsc++) {
                                ...
                                soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if ((pr->speed >= 100000) && IS_CXX_PORT(unit, pr->logical_port)) {
            num_lanes = 12;
        }
...
soc_apache_port_mode_get(int unit, int logical_port, int *mode)
{
    ...
    int num_lanes[AP_PORTS_PER_TSC];

    si = &SOC_INFO(unit);
    first_phyport = si->port_l2p_mapping[logical_port];

    for (lane = 0; lane < AP_PORTS_PER_TSC; lane++) {
                                             ...
                                             soc_info_misc_t *post_si)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_PORT,
     ...
     for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
         ...
         */
        SOC_IF_ERROR_RETURN(
            soc_esw_portctrl_pll_div_get(unit, phy_port, speed, num_lanes,
                                         encap, &pll_div));

        for (j = 0; j < AP_PORTS_PER_TSC; j++) {
            ...
            SOC_IF_ERROR_RETURN(
                soc_esw_portctrl_pll_div_get(unit, sister_port, sister_speed, num_lanes,
        ...
        for (j = 0; j < AP_PORTS_PER_TSC; j++) {
            ...
            if (((speed == 10000) && (post_si->port_speed[logic_port] == 1000)) ||
                ((pr->speed == 1000) && (sister_speed == 10000))) {
                if (!PORT_IS_FALCON(sister_port)) {
                    continue;
                }
            }

            /* For MV, even though <=10G speeds belong in same speed class only 1g & 10g mixed speed
             * is supported in TSCe PM4x10
             */
            if ((spd_cls != sister_spd_cls) ||
                ((pr->speed == 10000) && (post_si->port_speed[logic_port] <= 5000)) ||
                ((pr->speed <= 5000) && (post_si->port_speed[logic_port] == 10000)))
            {
                LOG_ERROR(BSL_LS_SOC_PORT,
                            (BSL_META_U(unit,
...
soc_ap_port_mixed_sister_speed_validate(int unit)
{
    ...
    for (phy_port = 1; phy_port < SOC_MAX_NUM_PORTS; phy_port += AP_PORTS_PER_TSC) {
        ...
        for (i = 1; i < AP_PORTS_PER_TSC; i++) {
            ...
            if (((speed == 10000) && (si->port_speed_max[sister_lport] == 1000)) ||
                ((si->port_speed_max[lport] == 1000) && (sister_speed == 10000))) {
                if (!PORT_IS_FALCON(sister_port)) {
                    continue;
                }
            }
             ...
             */
            if ((spd_cls != sister_spd_cls) ||
                ((si->port_speed_max[lport] == 10000) && (si->port_speed_max[sister_lport] <= 5000)) ||
                ((si->port_speed_max[lport] <= 5000) && (si->port_speed_max[sister_lport] == 10000)))
            {
                LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit,
...
soc_ap_port_resource_validate(int unit, int nport, soc_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (!(pr->flags & SOC_PORT_RESOURCE_SPEED)) {
            ...
            SOC_IF_ERROR_RETURN(_soc_ap_phy_port_flex_valid(unit, pr->physical_port));
...
#define RECONFIGURE_PORT_TYPE_INFO(ptype)                   \
    ...
    sal_memset(si->ptype.port, 0, COUNTOF(si->ptype.port)); \
                             ...
                             soc_port_resource_t *post_resource)
{
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        si->port_serdes[logic_port] = (phy_port - 1) / AP_PORTS_PER_TSC;
        if (pr->oversub) {
        ...
        if (port_is_hsp == -1) {
            port_is_hsp = soc_apache_mmu_hsp_port_reserve(unit, logic_port, pr->speed);
        }
                                ...
                                soc_ap_info_t *pre_soc_info)
{
     ...
     * */
    SOC_IF_ERROR_RETURN(soc_apache_port_config_bandwidth_check(unit));
    ...
    LOG_VERBOSE(BSL_LS_SOC_PORT,
                (BSL_META_U(unit,
                            "--- LED MicroProcessor Reconfigure\n")));
