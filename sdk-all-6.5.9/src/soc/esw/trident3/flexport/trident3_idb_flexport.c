    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
     ...
     * "TD3_uA_flexport.pdf", Section 6.6.1 */
 ...
 *         Follows the sequence specified in document "TD3_uA_flexport.pdf", Section
 *         6.6.1.1.
 *         Two main parts:
                               ...
                               soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            subp = soc_trident3_get_subp_from_phy_pnum(phy_port);
            reset_buffer = 1;
            if (TRIDENT3_PHY_IS_FRONT_PANEL_PORT(phy_port)) {
                SOC_IF_ERROR_RETURN(soc_trident3_idb_obm_reset_buffer(unit,
                                                                pipe_num,
                                                                pm_num, subp,
                                                                reset_buffer));
            ...
            if (TRIDENT3_PHY_IS_FRONT_PANEL_PORT(phy_port)) {
                ...
                SOC_IF_ERROR_RETURN(soc_trident3_idb_lpbk_ca_reset_buffer(unit,
                                                               pipe_num,
                                                               reset_buffer));
            }else if((phy_port==TRIDENT3_PHY_PORT_CPU)||
                     (phy_port==TRIDENT3_PHY_PORT_MNG0)){
                SOC_IF_ERROR_RETURN(soc_trident3_idb_cpu_ca_reset_buffer(unit,
                                                               pipe_num,
                                                               reset_buffer));
            }
        }
