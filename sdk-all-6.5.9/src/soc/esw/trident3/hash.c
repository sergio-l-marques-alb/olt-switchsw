...
int
soc_td3_shared_hash_mem_bucket_read(int unit, int ent, int key_type, int *width,
                                    ...
                                    void *bkt_entry, void *entry)
{
    ...
    case EXACT_MATCH_QUADm:
        ...
        default:
            *width = 4;
            *mem = EXACT_MATCH_QUADm;
        }
        entry_bits = (soc_mem_entry_bits(unit, EXACT_MATCH_DOUBLEm) - 1) * ((*width)/2);
        meminfo.bytes = SOC_MEM_WORDS(unit, EXACT_MATCH_QUADm) * 4;
        break;
...
soc_trident3_hash_bank_count_get(int unit, soc_mem_t mem, int *num_banks)
{
    ...
    case EXACT_MATCH_DOUBLEm:
    case EXACT_MATCH_ONLY_DOUBLEm:
    case EXACT_MATCH_QUADm:
    case EXACT_MATCH_ONLY_QUADm:
        /*
         * 16k double wide entries in each shared bank
         */
        count = soc_mem_index_count(unit, EXACT_MATCH_DOUBLEm);
                        ...
                        int use_lsb)
{
    ...
    soc_mem_field32_set(unit, mem, hash_control_entry, HASH_TABLE_TEST_MODEf,
                        use_lsb);
    if (is_shared == 0) {
        soc_reg_field_set(unit, mem, hash_control_entry, field, hash_offset);
    }
...
soc_td3_hash_index_get(int unit, int mem, int bank, int bucket)
{
    ...
    case L3_ENTRY_DOUBLEm:
    case L3_ENTRY_QUADm:
        rv = soc_td3_hash_bank_info_get(unit, L3_ENTRY_ONLY_SINGLEm, bank, NULL,
    ...
    case EXACT_MATCH_DOUBLEm:
    case EXACT_MATCH_QUADm:
        rv = soc_td3_hash_bank_info_get(unit, EXACT_MATCH_DOUBLEm, bank, NULL,
                                        &entries_per_row, NULL,
                                        &bank_base, &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
            if (mem == EXACT_MATCH_QUADm) {
                return index / 2;
            }
            return index;
        }
    ...
    case VLAN_XLATE_1_SINGLEm:
    case VLAN_XLATE_1_DOUBLEm:
            rv = soc_td3_hash_bank_info_get(unit, VLAN_XLATE_1_SINGLEm, bank, NULL,
                                        &entries_per_row, NULL, &bank_base,
                                        &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
    ...
    case VLAN_XLATE_2_SINGLEm:
    case VLAN_XLATE_2_DOUBLEm:
            rv = soc_td3_hash_bank_info_get(unit, VLAN_XLATE_2_SINGLEm, bank, NULL,
                                        &entries_per_row, NULL, &bank_base,
                                        &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
    ...
    case MPLS_ENTRY_SINGLEm:
    case MPLS_ENTRYm:
        rv = soc_td3_hash_bank_info_get(unit, MPLS_ENTRY_SINGLEm, bank, NULL,
                                        &entries_per_row, NULL, &bank_base,
                                        &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
    ...
    case EGR_VLAN_XLATE_1_SINGLEm:
    case EGR_VLAN_XLATE_1_DOUBLEm:
            rv = soc_td3_hash_bank_info_get(unit, EGR_VLAN_XLATE_1_SINGLEm, bank, NULL,
                                        &entries_per_row, NULL, &bank_base,
                                        &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
    ...
    case EGR_VLAN_XLATE_2_SINGLEm:
    case EGR_VLAN_XLATE_2_DOUBLEm:
            rv = soc_td3_hash_bank_info_get(unit, EGR_VLAN_XLATE_2_SINGLEm, bank, NULL,
                                        &entries_per_row, NULL, &bank_base,
                                        &bucket_offset);
        if (SOC_SUCCESS(rv)) {
            index = bank_base + bucket * entries_per_row + bucket_offset;
...
soc_td3_l2x_base_entry_to_key(int unit, int bank, uint32 *entry, uint8 *key)
{
    ...
    return _soc_td3_hash_entry_to_key(unit, bank, entry, key, L2Xm, field_list);
}
...
soc_td3_l3x_base_entry_to_key(int unit, int bank, uint32 *entry, uint8 *key)
{
    ...
    return _soc_td3_hash_entry_to_key(unit, bank, entry, key, mem, field_list);
}
                         ...
                         uint8 *key2)
{
    ...
    if (use_lsb && (hash_offset + hash_bits > 32)) {
        switch (soc_mem_field32_get(unit, EXACT_MATCH_DOUBLEm, base_entry,
                                    KEY_TYPEf)) {
        case TD3_FPEM_HASH_KEY_TYPE_128B:
            lsb_val = soc_mem_field32_get(unit, EXACT_MATCH_DOUBLEm, base_entry,
                                          MODE128__HASH_LSBf);
            break;
        case TD3_FPEM_HASH_KEY_TYPE_160B:
            lsb_val = soc_mem_field32_get(unit, EXACT_MATCH_DOUBLEm, base_entry,
            ...
            break;
        case TD3_FPEM_HASH_KEY_TYPE_320B:
            lsb_val = soc_mem_field32_get(unit, EXACT_MATCH_QUADm, base_entry,
                                          MODE320__HASH_LSBf);
            break;
        default:
            lsb_val = 0;
...
soc_td3_exact_match_base_entry_to_key(int unit, int bank, uint32 *entry,
    ...
    switch (soc_mem_field32_get(unit, EXACT_MATCH_DOUBLEm, entry, KEY_TYPE_0f)) {
    case TD3_FPEM_HASH_KEY_TYPE_128B:
        mem = EXACT_MATCH_DOUBLEm;
...
soc_td3_exact_match_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    ...
    rv = soc_td3_hash_offset_get(unit, EXACT_MATCH_DOUBLEm, bank, &hash_offset,
                                 &use_lsb);
    if (SOC_FAILURE(rv)) {
        return 0;
    }
...
uint32
soc_td3_mpls_hash(int unit, int bank, int hash_offset, int use_lsb,
                  int key_nbits, void *base_entry, uint8 *key1, uint8 *key2)
{
    uint32 lsb_val;
    uint32 hash_mask;
...
int
soc_td3_mpls_base_entry_to_key(int unit, int bank, void *entry, uint8 *key) {
...
uint32
soc_td3_mpls_entry_hash(int unit, int bank, int hash_offset,
                        int use_lsb, uint32 *entry)
{
    int             key_nbits;
    uint8           key1[SOC_MAX_MEM_WORDS * 4] = {0};
    ...
    /* Robust Hashed Key using Remap and Action Tables A */
    key_nbits = soc_td3_mpls_base_entry_to_key(unit, 0, entry, key1);
    /* Robust Hashed Key using Remap and Action Tables B */
    key_nbits = soc_td3_mpls_base_entry_to_key(unit, 1, entry, key2);

    return soc_td3_mpls_hash(unit, bank, hash_offset, use_lsb,
                             key_nbits, entry, key1, key2);
}

uint32
soc_td3_mpls_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    int rv, hash_offset, use_lsb;

    rv = soc_td3_hash_offset_get(unit, MPLS_ENTRYm, bank, &hash_offset,
                                 &use_lsb);
    if (SOC_FAILURE(rv)) {
        return 0;
    }

    return soc_td3_mpls_entry_hash(unit, bank, hash_offset, use_lsb, entry);
}
                                                ...
                                                uint8 *key)
{
    ...
    return _soc_td3_hash_entry_to_key(unit, bank, entry, key,
                                      ING_DNAT_ADDRESS_TYPEm, field_list);
}
                                                 ...
                                                 void *entry, uint8 *key)
{
    ...
    return _soc_td3_hash_entry_to_key(unit, bank, entry, key,
                                      SUBPORT_ID_TO_SGPP_MAPm, field_list);
}
                        ...
                        uint32 *bucket)
{
    ...
    case MPLS_ENTRY_SINGLEm:
    case MPLS_ENTRYm:
        *bucket = soc_td3_mpls_bank_entry_hash(unit, bank, entry);
        break;
    case VLAN_XLATE_1_SINGLEm:
    case VLAN_XLATE_1_DOUBLEm:
    case VLAN_XLATE_2_SINGLEm:
