...
tdm_td3_chk_get_pipe_token_cnt(tdm_mod_t *_tdm, int cal_id, int port_token)
{
    ...
    param_cal_len = tdm_td3_cmn_get_pipe_cal_len(cal_id, _tdm);
...
/**
@name: tdm_td3_chk_struct_os_ratio
@param:

Check oversub ratio for each pipe and/or half-pipe
**/
int 
tdm_td3_chk_struct_os_ratio(tdm_mod_t *_tdm, int cal_id, int os_ratio_limit)
{
        ...
        for (port_phy=phy_lo; port_phy<=phy_hi; port_phy++) {
            ...
            if (port_state==PORT_STATE__OVERSUB    ||
                ...
                if (port_phy>port_phy_mid) {
                    hp0_os_bw_req += tdm_td3_cmn_get_speed_slots(port_speed);
                } else {
                    hp1_os_bw_req += tdm_td3_cmn_get_speed_slots(port_speed);
                }
            }
        }
        /* os_bw_max */
...
tdm_td3_chk_struct(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        ...
        cal_len_act = tdm_td3_cmn_get_pipe_cal_len_max(cal_id, _tdm);
        cal_len_limit = tdm_td3_cmn_get_pipe_cal_len(cal_id, _tdm);
        TDM_SEL_CAL(cal_id, param_cal_main);
    ...
    if (PASS!=tdm_td3_chk_struct_os_ratio(_tdm, TD3_IDB_PIPE_0_CAL_ID, 2) ||   /* IDB pipe 0 */
        PASS!=tdm_td3_chk_struct_os_ratio(_tdm, TD3_IDB_PIPE_1_CAL_ID, 2) ||   /* IDB pipe 1 */
        PASS!=tdm_td3_chk_struct_os_ratio(_tdm, TD3_MMU_PIPE_0_CAL_ID, 2) ||   /* MMU pipe 0 */
        PASS!=tdm_td3_chk_struct_os_ratio(_tdm, TD3_MMU_PIPE_1_CAL_ID, 2)) {   /* MMU pipe 1 */
        result = FAIL;
    }
    ...
    for (port_phy=param_phy_lo; port_phy<=param_phy_hi; port_phy++) {
        ...
        if ((port_speed > SPEED_0) && 
            (port_state == PORT_STATE__LINERATE    || 
             ...
             port_state == PORT_STATE__OVERSUB_HG  ||
             port_state == PORT_STATE__MANAGEMENT ) ) {
            port_tsc = tdm_td3_cmn_get_port_pm(port_phy, _tdm);
    ...
    if (hp0_port_cnt==0 && hp1_port_cnt==0) {
        ...
        for (i=0; i<param_cal_grp_num; i++) {
            ...
            for (j=0; j<param_cal_grp_len; j++) {
                port_phy = param_cal_grp[i][j];
                if (port_phy>=param_phy_lo && port_phy<=param_phy_hi) {
                    port_pm = tdm_td3_cmn_get_port_pm(port_phy, _tdm);
...
tdm_td3_chk_pkt_sched_print(tdm_mod_t *_tdm, int cal_id, int hpipe_id)
{
    int i, j, k, x, y, port_phy, port_space, slot_cnt, 
        hp_id, result=PASS;
    int dist_max[TD3_NUM_PHY_PORTS_PER_HPIPE],
        dist_min[TD3_NUM_PHY_PORTS_PER_HPIPE],
        dist_port[TD3_NUM_PHY_PORTS_PER_HPIPE][TD3_NUM_PKT_SLOTS_PER_PM];
    int port_cnt = 0, port_buff[TD3_NUM_PHY_PORTS_PER_HPIPE];
    int max_tmp, min_tmp,
    ...
    for (i = 0; i < TD3_NUM_PHY_PORTS_PER_HPIPE; i++) {
    ...
    for (i=0; i<param_cal_grp_num; i++) {
        ...
        for (j=0; j<param_cal_grp_len; j++){
            port_phy = param_cal_grp[i][j];
            if (port_phy >= param_phy_lo && port_phy <= param_phy_hi) {
                if (port_cnt < TD3_NUM_PHY_PORTS_PER_HPIPE) {
