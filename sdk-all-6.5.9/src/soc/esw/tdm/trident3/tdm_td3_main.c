...
tdm_td3_main_ingress_pipe( tdm_mod_t *_tdm )
{
    ...
    param_cal_len     = tdm_td3_cmn_get_pipe_cal_len(param_pipe_id, _tdm);
...
tdm_td3_main_init( tdm_mod_t *_tdm )
{
    ...
    for (index=0; index<TD3_NUM_PM_MOD; index++) {
        ...
        /* encap */
        if (_tdm->_chip_data.soc_pkg.state[index*TD3_NUM_PM_LNS] == PORT_STATE__LINERATE_HG ||
            _tdm->_chip_data.soc_pkg.state[index*TD3_NUM_PM_LNS] == PORT_STATE__OVERSUB_HG) {
            _tdm->_chip_data.soc_pkg.soc_vars.td3.pm_encap_type[index] = PM_ENCAP__HIGIG2;
        } else {
            _tdm->_chip_data.soc_pkg.soc_vars.td3.pm_encap_type[index] = PM_ENCAP__ETHRNT;
        }
    }
