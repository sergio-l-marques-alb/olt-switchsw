                                                                ...
                                                                char* msg)
{
    ...
    rc = sal_dpc(soc_ppd_oam_dma_event_handler, INT_TO_PTR(unit), INT_TO_PTR(SOC_PPC_OAM_DMA_EVENT_TYPE_EVENT),0,0,0 );
    if (rc)
    {
        LOG_INFO(BSL_LS_BCM_INTR,
            (BSL_META_U(unit,
                                                                     ...
                                                                     char* msg)
{
    ...
    rc = sal_dpc(soc_ppd_oam_dma_event_handler, INT_TO_PTR(unit), INT_TO_PTR(SOC_PPC_OAM_DMA_EVENT_TYPE_STAT_EVENT),0,0,0 );
    if (rc)
    {
        LOG_INFO(BSL_LS_BCM_INTR,
            (BSL_META_U(unit,
    ...
    void *param2)
{
    ...
    case DCMN_INT_CORR_ACT_IPS_QSZ_CORRECT:
        ...
        break;
    case DCMN_INT_CORR_ACT_XOR_FIX:
        rc = dcmn_interrupt_handles_corrective_action_for_xor(unit, block_instance, interrupt_id,(dcmn_interrupt_mem_err_info*)param1, msg, 0);
        SOCDNX_IF_ERR_EXIT(rc);
        break;
    case DCMN_INT_CORR_ACT_TCAM_SHADOW_FROM_SW_DB:
