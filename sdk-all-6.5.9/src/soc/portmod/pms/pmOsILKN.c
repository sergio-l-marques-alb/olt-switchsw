...
#define OS_ILKN_QMX_FIRST_LANE_OFFSET_IN_ILKN_O_FABRIC   8   /* In QMX, ILKN over fabric starts at offset 8, while others start at lane 0 */
...
int pmOsILKN_pm_init(int unit, const portmod_pm_create_info_internal_t* pm_add_info, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    /* In QMX, ILKN4/5 have 6 PMs but in JER, the lower 3 PMs of ILKN4/5 doesn't exist. */
...
int pmOsILKN_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if (pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en) /* ILKN over Fabric */
    {
        PORTMOD_PBMP_ITER(ilkn_data->phys, port_i)
        {
            ...
            lane = is_port_qmx_and_over_fabric ? lane + OS_ILKN_QMX_FIRST_LANE_OFFSET_IN_ILKN_O_FABRIC : lane; /*Align to QMX over fabric*/
...
pmOsILKN_port_logical_lane_order_set(int unit, int port, pm_info_t pm_info, const int* lane_order, int lane_order_size)
{
    ...
    PORTMOD_PBMP_ITER(temp_phys_aligned, port_i){
        ...
        if (is_port_qmx_and_over_fabric) {
            lane_index += OS_ILKN_QMX_FIRST_LANE_OFFSET_IN_ILKN_O_FABRIC;
        }
