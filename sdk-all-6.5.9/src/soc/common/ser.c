                    ...
                    case EXACT_MATCH_4_PIPE3m:
                        if (!reported_mem_is_fpem_lp) {
                            ...
                            if (SOC_IS_TOMAHAWKX(unit)) {
                                ...
                                if (index % iratio[m] == 1) {
                                    ecc_data_field = ECC_DATA1f;
                                } else {
                                    ecc_data_field = ECC_DATA0f;
                                }
                            }
                            soc_mem_field_get(unit, mem,
                                              cache + ((vindex+i)*entry_dw),
...
#define _SOC_IF_ERR_L2X_THAW(rv, mem)                                          \
    ...
    sal_memset(&sram_info, 0x00, sizeof(sram_info));
    /* Determine RAM count and indexes for all RAMs */
        ...
        if (SOC_IS_TRIDENT3X(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_trident3_mem_sram_info_get(unit, mem, index, &sram_info));
        } else
#endif
    {
        return SOC_E_UNAVAIL;
    ...
    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit) || SOC_IS_TD2P_TT2P(unit) || SOC_IS_APACHE(unit)) {
        ...
        if (SOC_IS_APACHE(unit)) {
            /* use_recovery_hw_cache is to be enabled for multi-pipe devices */
            use_recovery_hw_cache = 0;
            if ((mem == L2_ENTRY_ISS_LPm) || (mem == L3_ENTRY_ISS_LPm)) {
                mem_to_restore = sram_info.view[0];
                mem_for_cache_read = sram_info.view[0];
                mem_to_lock= sram_info.view[0];
            }
        }
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
...
#endif
#if defined(BCM_APACHE_SUPPORT)
    /* Temporarily freeze L2 activity during SER of L2 LP memories */
    if (((mem == L2_ENTRY_LPm) || (mem == L2_ENTRY_ISS_LPm)) &&
        soc_feature(unit, soc_feature_l2_activity_freeze_ser)) {
        rv = soc_l2x_freeze(unit);
        if (SOC_FAILURE(rv)) {
            LOG_WARN(BSL_LS_SOC_SER,
                     (BSL_META_U(unit,
                                 "L2 freeze failed in %s correction\n"),
                     SOC_MEM_NAME(unit, mem)));
            if (mem_to_lock != INVALIDm) {
                MEM_UNLOCK(unit, mem_to_lock);
            }
...
#if defined(BCM_APACHE_SUPPORT)
            /* For memories that cannot be recovered using HW cache */
            if ((mem == L3_ENTRY_ISS_LPm) && !use_recovery_hw_cache) {
                /*
                 * To recover errors in L3 ENTRY LP memory, the main L3_ENTRY
                 * needs to be rewritten at the corresponding index with the
                 * view used for writing the entry initially.
                 *
                 * The sram_info will have different view names for different
                 * entries that need to be corrected.
                 */
                if ((i > 0) && (sram_info.view[i] != sram_info.view[i-1])) {
                    if (mem_to_lock != INVALIDm) {
                        MEM_UNLOCK(unit, mem_to_lock);
                    }
                    mem_to_restore = sram_info.view[i];
                    mem_for_cache_read = sram_info.view[i];
                    mem_to_lock= sram_info.view[i];
                    if (mem_to_lock != INVALIDm) {
                        MEM_LOCK(unit, mem_to_lock);
                    }
                    cache = SOC_MEM_STATE(unit, mem_for_cache_read).cache[copyno];
...
#if defined(BCM_APACHE_SUPPORT)
            if (((mem == L2_ENTRY_LPm) || (mem == L2_ENTRY_ISS_LPm)) &&
                !use_recovery_hw_cache) { /* Correct by HW entry rewrite */
                /*
                 * For single pipe devices with errors in the LP memory
                 * corresponding to the banks mapped to L2, the correction cannot be
                 * performed from software memory cache.
                 * By design, the L2X memory is not cached because the HW can learn
                 * the entries with traffic.
                 *
                 * To correct these errors, the learning and aging is frozen and
                 * the main L2X entry is read from the hardware and written back.
                 */
                rv = soc_mem_read(unit, mem_to_restore, copyno, start_index,
                                  entry);
                _SOC_IF_ERR_L2X_THAW(rv, mem);
                rv = soc_mem_write(unit, mem_to_restore, copyno,
                                   start_index, entry);
                _SOC_IF_ERR_L2X_THAW(rv, mem);
            } else
#endif /* BCM_APACHE_SUPPORT */
            { /* Fall back to clear (write null-entry) */
...
#endif
#if defined(BCM_APACHE_SUPPORT)
    /* Enabled learning and aging after SER of LP L2 memories */
    if (((mem == L2_ENTRY_LPm) || (mem == L2_ENTRY_ISS_LPm)) &&
        soc_feature(unit, soc_feature_l2_activity_freeze_ser)) {
        rv = soc_l2x_thaw(unit);
        if (SOC_FAILURE(rv)) {
...
#ifdef BCM_CMICX_SUPPORT
void
soc_cmicx_parity_intr(int unit, void *val)
{
#if defined(BCM_XGS_SUPPORT)
    int index;

    index = PTR_TO_INT(val) / (sizeof(uint32) * 8);
    (void)soc_cmic_intr_disable(unit, PTR_TO_INT(val));

#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        /* SER interrupts */
        if (PTR_TO_INT(val) >= MMU_TO_CMIC_MEMFAIL_INTR &&
            PTR_TO_INT(val) <= PM32_INTR) {
            LOG_INFO(BSL_LS_SOC_INTR,
                     (BSL_META_U(unit,
                                 "soc_cmicm_intr type 3 unit %d: dispatch\n"), unit));
            if (soc_ser_parity_error_cmicm_intr(INT_TO_PTR(unit), 0, 0,
                                                INT_TO_PTR(index - 4),
                                                val)) {
            } else {
        ...
        } else if (PTR_TO_INT(val) == L2_MGMT_TO_CMIC_INTR) { /* L2 MGMT */
            /* dispatch interrupt */
            LOG_INFO(BSL_LS_SOC_INTR,
                     (BSL_META_U(unit,
                                 "soc_cmicm_intr type 3 unit %d: dispatch\n"),
...
STATIC int
_soc_vfp_ser_correction(int unit, soc_mem_t mem, int index, int copyno, int entry_dw)
{
    int rv = SOC_E_NONE;
    ...
    if (si->flags & SOC_SER_SRC_MEM) { /* Handle mems */
        ...
        case SOC_MEM_FLAG_SER_WRITE_CACHE_RESTORE:
            ...
            if (cache != NULL && CACHE_VMAP_TST(vmap, index)) {
                ...
                if (soc_feature(unit, soc_feature_unique_acc_type_access) &&
                    ...
                    mem_for_write = SOC_MEM_UNIQUE_ACC(unit,mem)[si->pipe_num];
                } else
#endif
                {
                    mem_for_write = mem;
                }
                if ((rv = soc_mem_write(unit, mem_for_write, copyno, index, null_entry)) < 0) {
                    LOG_ERROR(BSL_LS_SOC_SER,
                              (BSL_META_U(unit,
                                          "CLEAR_RESTORE %s.%s[%d] failed: %s\n"),
                               SOC_MEM_NAME(unit, mem),
        ...
        case SOC_MEM_FLAG_SER_SPECIAL:
            ...
            case RMEPm:
                ...
                return rv;
            case VFP_TCAMm:
                rv = _soc_vfp_ser_correction(unit, mem, index, copyno, entry_dw);
                if (rv != SOC_E_NONE) {
                    LOG_ERROR(BSL_LS_SOC_SER,
                              (BSL_META_U(unit,
                                          "Error in SER Handling rv=%d\n"),
