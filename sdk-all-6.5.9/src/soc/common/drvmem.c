...
typedef struct dest_type0_s {
#if defined(LE_HOST)
    uint32      nh_dgpp_dvp:16; /* Next-Hop|DGPP|DVP */
    uint32      dtype:2;
    uint32      allzero:14;
#else
    uint32      allzero:14;
    uint32      dtype:2;
    uint32      nh_dgpp_dvp:16;
#endif
} dest_type0_t;

typedef struct dest_type1_s {
#if defined(LE_HOST)
    uint32      mc_ecmp:14;     /* L2MC|IPMC|ECMP */
    uint32      dtype:2;
    uint32      allzero:16;
#else
    uint32      allzero:16;
...
typedef struct dest_type2_s {
#if defined(LE_HOST)
    uint32      lag_rt_tgid:13; /* LAG|REMOTE_TRUNK|TGID */
    uint32      dtype:1;
    uint32      allzero:18;
#else
    uint32      allzero:18;
    uint32      dtype:1;
    uint32      lag_rt_tgid:13;
#endif
} dest_type2_t;

typedef struct dest_type3_s {
#if defined(LE_HOST)
    uint32      my_sta_prof:11;
    uint32      dtype:2;
    uint32      allzero:19;
#else
    uint32      allzero:19;
...
typedef struct dest_type5_s {
#if defined(LE_HOST)
    uint32      disc:3;         /* Disc Copy2CPU | Disc */
    uint32      dtype:2;
    uint32      allzero:27;
#else
    uint32      allzero:27;
 ...
 */
uint32 _soc_mem_dest_value_construct(uint32 dest_type, uint32 value)
{
    uint32 dest_value = 0;
    dest_fmt_t *pdest = (void *)&dest_value;

    switch (dest_type) {
        case SOC_MEM_FIF_DEST_NEXTHOP:
            pdest->type0.dtype              = 3;
            pdest->type0.nh_dgpp_dvp        = value;
            break;
        ...
        case SOC_MEM_FIF_DEST_LAG:
            pdest->type2.dtype              = 1;
            pdest->type2.lag_rt_tgid        = value;
            break;
        case SOC_MEM_FIF_DEST_MYSTA:
            pdest->type3.dtype              = 3;
            pdest->type3.my_sta_prof        = value;
            break;
        case SOC_MEM_FIF_DEST_DISC_CP2CPU:
            pdest->type5.dtype              = 3;
            pdest->type5.disc               = value;
            break;
 ...
 * value     - OUT
 */
void _soc_mem_dest_value_resolve(uint32 dest_value, uint32 *dest_type,
                                 uint32 *value)
{
    dest_fmt_t *pdest = (void *)&dest_value;

    *dest_type = SOC_MEM_FIF_DEST_INVALID;
    if (pdest->type0.dtype != 0) {
        switch (pdest->type0.dtype) {
            case 3:
            ...
            case 2:
                *dest_type = SOC_MEM_FIF_DEST_DGPP;
                break;
            case 1:
                *dest_type = SOC_MEM_FIF_DEST_DVP;
                break;
            default:
                break;
        }
        *value = pdest->type0.nh_dgpp_dvp;
    } else if (pdest->type1.dtype != 0) {
        switch (pdest->type1.dtype) {
            case 3:
                *dest_type = SOC_MEM_FIF_DEST_IPMC;
                break;
            case 2:
                *dest_type = SOC_MEM_FIF_DEST_L2MC;
                break;
            case 1:
        ...
        *value = pdest->type1.mc_ecmp;
    } else if (pdest->type2.dtype != 0) {
        switch (pdest->type2.dtype) {
            case 1:
        ...
        *value = pdest->type2.lag_rt_tgid;
    } else if (pdest->type3.dtype != 0) {
        switch (pdest->type3.dtype) {
            case 3:
                *dest_type = SOC_MEM_FIF_DEST_MYSTA;
                break;
            default:
                break;
        }
        *value = pdest->type3.my_sta_prof;
    } else if (pdest->type5.dtype != 0) {
        switch (pdest->type5.dtype) {
            case 3:
                *dest_type = SOC_MEM_FIF_DEST_DISC_CP2CPU;
                break;
            case 2:
                *dest_type = SOC_MEM_FIF_DEST_DISC;
                break;
            default:
                break;
        }
        *value = pdest->type5.disc;
    } else {
        *value = 0;
    }
...
{
    uint32 dest_value = 0;
    dest_value = _soc_mem_dest_value_construct(dest_type, value);
...
{
    uint32 value = 0, dest_value;

    dest_value = soc_mem_field32_get(unit, mem, entbuf, fld);
    _soc_mem_dest_value_resolve(dest_value, dest_type, &value);
