...
void cmicx_pktdma_desc_endian_set(int unit, int cmc, int channel) {
    uint32          val = 0;
...
void cmicx_pktdma_ep_to_cpu_hdr_endian_set(int unit, int cmc, int channel) {
    uint32          val = 0;

    val = soc_pci_read(unit, CMIC_CMCx_PKTDMA_CHy_CTRL_OFFSET(cmc, channel));
    val |= PKTMDA_EP_TO_CPU_HEADER_ENDIAN;
    soc_pci_write(unit, CMIC_CMCx_PKTDMA_CHy_CTRL_OFFSET(cmc, channel), val);
}

void cmicx_pktdma_data_endian_set(int unit, int cmc, int channel) {
    uint32          val = 0;
...
uint32 cmicx_pktdma_cos_ctrl_rx0_get(int unit, int cmc, int channel) {
    return soc_pci_read(unit, CMIC_CMCx_PKTDMA_CHy_COS_CTRL_RX_0_OFFSET(cmc, channel));
}

uint32 cmicx_pktdma_cos_ctrl_rx1_get(int unit, int cmc, int channel) {
    return soc_pci_read(unit, CMIC_CMCx_PKTDMA_CHy_COS_CTRL_RX_1_OFFSET(cmc, channel));
}

uint32 cmicx_pktdma_cos_ctrl_rx0_reg_addr_get(int unit, int cmc, int channel) {
    return CMIC_CMCx_PKTDMA_CHy_COS_CTRL_RX_0_OFFSET(cmc, channel);
}

uint32 cmicx_pktdma_cos_ctrl_rx1_reg_addr_get(int unit, int cmc, int channel) {
    return CMIC_CMCx_PKTDMA_CHy_COS_CTRL_RX_1_OFFSET(cmc, channel);
}

uint32 cmicx_pktdma_cos_ctrl_queue_id_get(int unit, int cmc, int channel, int queue) {
    uint32 reg_addr;
    reg_addr = (queue < 32) ?
...
sal_vaddr_t cmicx_pktdma_desc_addr_get(int unit, int cmc, int channel) {
    uint32      val1;
    sal_paddr_t paddr;
...
uint32 cmicx_pktdma_header_size_get(int unit) {
    uint32 hdr_size = 0;
...
static uint32 cmicx_dma_get_intr_base(int cmc) {
    uint32 intr_base = 0;
...
cmicx_dma_ctrl_init(int unit)
{
    ...
    for(vchan=0; vchan < SOC_DCHAN_NUM(unit); vchan++) {
        ...
        val &= (PKTDMA_BIG_ENDIAN | PKTDMA_DESC_BIG_ENDIAN | PKTMDA_EP_TO_CPU_HEADER_ENDIAN);
...
cmicx_dma_chan_config(int unit, int vchan, dvt_t type, uint32 flags)
{
    ...
    cr &= (PKTDMA_BIG_ENDIAN | PKTDMA_DESC_BIG_ENDIAN | PKTMDA_EP_TO_CPU_HEADER_ENDIAN);
    cr |= bits;
...
cmicx_dma_chan_dv_start(int unit, sdc_t *sc, dv_t *dv_chain)
{
    ...
    if (sc->sc_q_cnt != 0) {
        ...
        if (soc_property_get(unit,
            spn_PDMA_CONTINUOUS_MODE_ENABLE, 0)) {
...
cmicx_dma_masks_get(int unit, uint32 *pkt_endian, uint32 *desc_endian, uint32 *header_endian)
{
    ...
    *header_endian = PKTMDA_EP_TO_CPU_HEADER_ENDIAN;
