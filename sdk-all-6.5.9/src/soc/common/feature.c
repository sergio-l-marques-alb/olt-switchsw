...
soc_features_bcm56960_a0(int unit, soc_feature_t feature)
{
    ...
    if ((dev_id == BCM56168_DEVICE_ID) || (dev_id == BCM56169_DEVICE_ID)) {
        if ((feature == soc_feature_l3_no_ecmp) ||
            (feature == soc_feature_l3_1k_defip_table) ||
        ...
        if ((feature == soc_feature_fcoe) ||
            ...
            (feature == soc_feature_ecmp_dlb) ||
            (feature == soc_feature_ecmp_resilient_hash) ||
            (feature == soc_feature_ecmp_resilient_hash_optimized) ||
            (feature == soc_feature_ecmp_round_robin) ||
            (feature == soc_feature_ecmp_random) ||
            (feature == soc_feature_vp_lag)) {
            return FALSE;
        }
    }
...
soc_features_bcm56970_a0(int unit, soc_feature_t feature)
{
    ...
    if (dev_id == BCM56971_DEVICE_ID) {
        if ((feature == soc_feature_l2gre) ||
            (feature == soc_feature_oob_fc) ||
    ...
    case soc_feature_sw_autoneg:
       soc_cm_get_id(unit, &dev_id, &rev_id);
        if((dev_id == BCM56970_DEVICE_ID) && (rev_id == BCM56970_A0_REV_ID)) {
            return TRUE;
        } else {
            return FALSE;
        }
