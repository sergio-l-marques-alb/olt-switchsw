...
int arad_kbp_op_recover_txpcs_soft_reset(int unit, int mdio_id)
{
    uint32_t flags = SOC_PHY_NOMAP | SOC_PHY_CLAUSE45;
    uint32_t data;
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_get(unit, mdio_id, flags, 0x02F03E, &data));
    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_set(unit, mdio_id, flags, 0x02F03E, 0));
    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_set(unit, mdio_id, flags, 0x02F03E, data));
    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_get(unit, mdio_id, flags, 0x02F00F, &data));
    SOCDNX_IF_ERR_EXIT(arad_port_phy_reg_set(unit, mdio_id, flags, 0x02F00F, 0));
...
/* Recover the KBP  */
int arad_kbp_recover_run_recovery_sequence(int unit, uint32 core,
                        uint32 kbp_mdio_id[MAX_NUM_OF_CORES], uint32 retries, void *buf, int option)
{
    ...
    for (i = 0; i < retries; ++i) {
        for (k = 0; k < core_iter; k ++) {
            mdio_id = is_broadcast ? kbp_mdio_id[k] : kbp_mdio_id[core];
            if (SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
                SOCDNX_IF_ERR_EXIT(arad_kbp_op_recover_txpcs_soft_reset(unit, mdio_id));
                sal_usleep(100000);
            } else {
