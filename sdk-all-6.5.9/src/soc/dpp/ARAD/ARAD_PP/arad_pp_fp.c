    ...
    for (round = 0; round < 2 ; round++)
    {
        ...
        for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
        {
            ...
            /* Do not allocate action and key for large DE DBs, inserted on entry insertion */
                    ...
                    (is_large_direct_extraction && (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_ALLOCATE_FES)) )
            {
                /* Allocate the Key */
  ...
  )
{
  ...
  else { /* Large Direct extraction / single mapping  - use FES*/
      *success = SOC_SAND_SUCCESS;

      /*extractions verification*/
      /*Loop on all extractions and check there are no "holes" or lost-bits*/
      if (database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_ALLOCATE_FES) {  
          for (extr_ndx = 0 ; (extr_ndx < info->actions[0].nof_fields ) && (info->actions[0].fld_ext[extr_ndx].type != BCM_FIELD_ENTRY_INVALID)  ; extr_ndx ++)
          {
                res = arad_pp_fp_qual_lsb_and_length_get(
                   unit,
                   db_id_ndx,
                   info->actions[0].fld_ext[extr_ndx].type,
                   &key_lsb,
                   &qual_length_no_padding
                );
                ...
                {
                    tmp_lsb = key_lsb;
                    tmp_len = qual_length_no_padding;
                }
                else 
                {
                    if (tmp_lsb + tmp_len < key_lsb )
                    {
                       LOG_ERROR(BSL_LS_SOC_FP,
                          (BSL_META_U(unit,
                              "   Error in DE entry add: db-id: %d, stage %s, Extractions not consecutive in Key or Lost bits in non-first extraction\n\r"),
                       db_id_ndx, SOC_PPC_FP_DATABASE_STAGE_to_string(stage)));
                      *success = SOC_SAND_FAILURE_INTERNAL_ERR;
                        ARAD_DO_NOTHING_AND_EXIT;
                    }
      ...
      /* In case we want to allocate FES but LARGE flag is not set */
      ...
      for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
      {
          ...
          if ( is_large_direct_extraction && !(database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_ALLOCATE_FES)) {
          ...
          { /*In case CREATE_LARGE flag is set */
              res = sw_state_access[unit].dpp.soc.arad.tm.pmf.db_info.get(
                      unit,
                      stage,
                      db_id_ndx,
              ...
              action_const.tcam_res_id[1] += (is_for_tm) ? info->actions[0].fld_ext[0].fld_lsb : key_lsb;  /*CE contstraint lost bits*/
          }
  ...
  )
{
  ...
  if (!is_large_direct_extraction) {
      ...
      if (is_found == TRUE)
      {
        ...
        /*Think about if different from other*/
  }
    ...
    for(qual_index = 0; qual_index < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX ; ++qual_index) {
      ...
      for (action_ndx = 0; action_ndx < SOC_DPP_DEFS_GET(unit, nof_egress_pmf_actions); action_ndx++) {
          ...
          case SOC_PPC_FP_ACTION_TYPE_EGR_OFP:
              if(SOC_IS_JERICHO(unit)) {
                  dpp_dsig_read(unit, core_id, "ERPP", "PMF", "TM Action", "System_Headers_Record", val, 1);
