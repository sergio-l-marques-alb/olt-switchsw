    ...
    )
{
    ...
    if (use_tcam) {
        ...
        */
        ...
        res = jer_pp_kaps_db_enabled(unit, vrf_ndx);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        res = arad_pp_dbal_entry_get(unit, table_id, qual_vals, &payload, 0/*priority*/, hit_bit, found);
    ...
    )
{
    ...
    if(!ARAD_KBP_ENABLE_IPV6_EXTENDED && SOC_IS_JERICHO(unit)) {
        ...
        if(route_key->prefix_len == 0) {
            /* send KBP default route to KAPS */
            uint32 kaps_payload;
            ...
            res = arad_pp_dbal_entry_add(unit, table_id, qual_vals, 0/*priority*/,  &kaps_payload, success);
            SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    )
{
    ...
    if(!ARAD_KBP_ENABLE_IPV6_EXTENDED && SOC_IS_JERICHO(unit)) {
        ...
        if(route_key->prefix_len == 0) {
            /* remove KBP default route from KAPS */
            table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_KBP_DEFAULT_ROUTE_KAPS;
    ...
    )
{
    ...
    if(!ARAD_KBP_ENABLE_IPV6_EXTENDED && SOC_IS_JERICHO(unit)) {
        ...
        if(route_key->prefix_len == 0) {
            /* get KBP default route from KAPS */
            table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_KBP_DEFAULT_ROUTE_KAPS;
            ...
            res = arad_pp_dbal_entry_get(unit, table_id, qual_vals, &kaps_payload, 0/*priority*/, NULL/*hit_bit*/, found);
            SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    ...
    if(*found) 
    {
        if(!ARAD_KBP_ENABLE_IPV6_EXTENDED && SOC_IS_JERICHO(unit) && (route_key->prefix_len == 0)) {
            /* expecting default route result from KAPS */
            if(route_info->flags & _BCM_L3_FLAGS2_RAW_ENTRY) {
                /* Do not decode if raw entry */
                route_info->dest_id = kaps_payload;
            } else {
                route_info->dest_id = JER_PP_KAPS_DEFAULT_DECODE(JER_PP_KAPS_FEC_DECODE(kaps_payload));
            }
        } else if(!ARAD_KBP_ENABLE_IPV6_EXTENDED && SOC_IS_JERICHO(unit) && (route_info->flags & _BCM_L3_FLAGS2_RAW_ENTRY)) {
            /* Do not decode if raw entry */
    ...
    )
{
    ...
    if(*found) {
        ...
        } else if(SOC_IS_JERICHO(unit) && (route_info->flags & _BCM_IPMC_FLAGS_RAW_ENTRY)) {
            /* Do not decode if raw entry */
    ...
    )
{
    ...
    if (ARAD_KBP_ENABLE_IPV6_EXTENDED) { /* both tables should be cleared */
        ...
        if(SOC_IS_JERICHO(unit)) {
            /* also clear KBP default route tables in KAPS */
