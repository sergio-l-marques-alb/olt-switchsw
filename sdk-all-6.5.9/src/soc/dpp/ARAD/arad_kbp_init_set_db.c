    ...
    )
{
    ...
    if (elk->tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
        is_12k = 0;
    } else {
        is_12k = 1;
    }
    ...
    if(elk->kbp_connect_mode == ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL) {
        arad_kbp_device = AradAppData[unit]->thread_p[core];
    } else {
        arad_kbp_device = AradAppData[unit]->device_p[core];
    }

    if (!SOC_WARM_BOOT(unit) && elk->is_master) {
        ...
        if (is_12k || (db_type == KBP_DB_ACL) || (!is_12k && (db_type == KBP_DB_LPM) && (AradAppData[unit]->g_gtmInfo[table_id].tblInfo.tbl_width > NLM_TBL_WIDTH_160))) {
    ...
    else if (!SOC_WARM_BOOT(unit) && !(elk->is_master)) {
        sal_memcpy(AradAppData[unit], AradAppData[unit-1], sizeof(genericTblMgrAradAppData));
    }
    else {
    ...
    arad_kbp_result_sizes_configurations_init(int unit, ARAD_KBP_FRWRD_IP_DB_TYPE db_type, int results[ARAD_PP_FLP_KBP_MAX_NUMBER_OF_RESULTS])
{
	...
	res = KBP_INFO.Arad_kbp_ltr_config.parallel_srches_bmp.get(unit,db_type,&parallel_bitmap);
	SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   int core)
{
    ...
    if (elk->is_master) {
        ...
        /* looping until the master unit assignes a thread to this unit */
    ...
    if (ARAD_KBP_IS_CR_MODE(unit) == 1) {
        /* Get allocation HA */
        if(core == 0) {
            kbp_nv_mem_ptr = ha_mem_alloc(unit, HA_KBP_Mem_Pool, HA_KBP_SUB_ID_0, KBP_VERSION_1_0, KBP_STRUCT_SIG, &nv_mem_size);
    ...
    if (is_12k) {
        ...
        if (elk->is_master) {
            ...
            if (status != KBP_OK) {
                LOG_ERROR(BSL_LS_SOC_TCAM,
                        (BSL_META_U(unit,
                                "Error in %s(): kbp_pcie_init failed: %s\n"),
                                FUNCTION_NAME(), kbp_get_status_string(status)));
            }
            if (elk->kbp_connect_mode == ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL) {
            ...
            if (1) {
                ...
                status = kbp_pcie_set_property(AradAppData[unit]->alg_kbp_xpt_p[core], KBP_PCIE_REPLACE_SEARCH, handle, arad_op_search);
                if (status != KBP_OK) {
                    LOG_ERROR(BSL_LS_SOC_TCAM,
                            (BSL_META_U(unit,
                                    "Error in %s(): arad_kbp_op_xpt_init failed: %s\n"),
                                    FUNCTION_NAME(), kbp_get_status_string(status)));
                }
            }
        } else {
        ...
        if (status != KBP_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                    (BSL_META_U(unit,
                            "Error in %s(): kbp_pcie_set_property() KBP_PCIE_VERBOSE_LEVEL 2 failed: %s\n"),
                            FUNCTION_NAME(), kbp_get_status_string(status)));
        }
#endif
       ...
       int no_sync_flag)
{
   ...
   /* destroy kbp_dm , was created in arad_kbp_deinit_nlm_dev_mgr() */
    if (elk->is_master) {
        ...
        {
            res = default_allocator_destroy(AradAppData[unit]->dalloc_p[0]);
            res |= default_allocator_destroy(AradAppData[unit]->dalloc_p[1]);
            res |= default_allocator_create(&AradAppData[unit]->dalloc_p[0]);
    ...
    if (is_12k){
        ...
        if(second_kbp_supported) {
            arad_kbp_xpt_destroy(AradAppData[unit]->alg_kbp_xpt_p[1]);
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
                    ...
                    if(ltr_config_info.ltr_search[srch_ndx].key_segment[key_ndx].nof_bytes){
                        if(!(AradAppData[unit]->g_gtmInfo[table_idx].tblInfo.is_key_adde_to_db)){
                            ...
                            if(SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
                                type = sal_strcmp("VRF", ltr_config_info.ltr_search[srch_ndx].key_segment[key_ndx].name) ? KBP_KEY_FIELD_EM : ltr_config_info.ltr_search[srch_ndx].key_segment[key_ndx].type;
                            } else {
                            ...
                            if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                                if(res != KBP_DUPLICATE_KEY_FIELD) {
                                    LOG_ERROR(BSL_LS_SOC_TCAM,
                                            (BSL_META_U(unit,
                                                    "Error in %s(): DB Key : kbp_key_add_field with failed: %s, LTR ID %d, search %d\n"), 
             ...
             key_ndx++)
        {
            ...
            if(ltr_config_info.master_key_fields.key_segment[key_ndx].nof_bytes){
                int type;
                if(SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
                    type = sal_strcmp("VRF", ltr_config_info.master_key_fields.key_segment[key_ndx].name) ? KBP_KEY_FIELD_EM : ltr_config_info.master_key_fields.key_segment[key_ndx].type;
                } else {
    ...
    uint32 second_kbp_supported)
{
    ...
    res = KBP_INFO.Arad_kbp_gtm_lut_info.get(unit, gtm_table_id, &lut_data); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    res = KBP_INFO.Arad_kbp_gtm_ltr_info.get(unit, gtm_table_id, &Arad_kbp_gtm_ltr_info); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    ...
    uint32 gtm_table_id)
{
    ...
    res = KBP_INFO.Arad_kbp_gtm_opcode_config_info.get(unit, gtm_table_id, &Arad_kbp_gtm_opcode_config_info); 
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    ARAD_INIT_ELK *elk)
{
    ...
    uint32 max_available_capacity = 1 << 20; /* 1M of 80b entries */
#endif
    ...
    for (tbl_idx = 0; tbl_idx < ARAD_KBP_MAX_NUM_OF_TABLES; tbl_idx++) 
    {
        if (SOC_WARM_BOOT(unit)) {
            ...
            SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);
            res = KBP_INFO.Arad_kbp_table_config_info.tbl_id.get(unit, tbl_idx, &tbl_id); 
            SOC_SAND_CHECK_FUNC_RESULT(res, 110, exit);
            res = KBP_INFO.Arad_kbp_table_config_info.tbl_size.get(unit, tbl_idx, &tbl_size); 
            SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);    
            res = KBP_INFO.Arad_kbp_table_config_info.tbl_width.get(unit, tbl_idx, &tbl_width);
            SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);   
            res = KBP_INFO.Arad_kbp_table_config_info.clone_of_tbl_id.get(unit, tbl_idx, &clone_of_tbl_id);
            SOC_SAND_CHECK_FUNC_RESULT(res, 120, exit);            
        }
        ...
        if (is_valid) 
        {
            ...
            if (ARAD_KBP_IS_CR_MODE(unit) == 1 && clone_of_tbl_id == ARAD_KBP_FRWRD_IP_NOF_TABLES) {
                /* CR is enabled and the table is not a clone */
    ...
    if (!SOC_WARM_BOOT(unit) && (elk->tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311)) {
        for(db_type = 0; db_type < ARAD_KBP_MAX_NUM_OF_FRWRD_DBS; db_type++) {
            uint32 srch_ndx;
            arad_kbp_lut_data_t kbp_gtm_lut_info = {0};
            int num_of_kbp_searches = ARAD_KBP_NUM_OF_KBP_SEARCHES(unit);
    ...
    for (core = 0; core < iter_count; core++) {
        ...
        if(!SAL_BOOT_PLISIM){
            ...
            if (res != 0 && elk->kbp_recover_enable) {
                res = arad_kbp_recover_run_recovery_sequence(unit, core, elk->kbp_mdio_id, elk->kbp_recover_iter, NULL, 1);
            }
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        }    
        ...
        if (!SOC_WARM_BOOT(unit)) {
           ...
           if (res != 0 && elk->kbp_recover_enable) {
               res = arad_kbp_recover_run_recovery_sequence(unit, core, elk->kbp_mdio_id, elk->kbp_recover_iter, NULL, 2);
           }
...
#ifdef CRASH_RECOVERY_SUPPORT
        if (ARAD_KBP_IS_CR_MODE(unit) == 1) {
            res = ha_mem_free(unit, kbp_nv_mem_ptr);
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        }
#endif
        if(elk->is_master) {
...
/* Do not perform sync in CR mode */
...
#endif
    {
        ...
        res = kbp_device_save_state(arad_kbp_device, warmboot_data->kbp_file_read , warmboot_data->kbp_file_write, warmboot_data->kbp_file_fp);
        if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK){
            LOG_ERROR(BSL_LS_SOC_TCAM,
                    (BSL_META_U(unit,
                            "Error in %s(): kbp_device_save_state failed: %s\n"),
                            FUNCTION_NAME(), kbp_get_status_string(res)));
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 100, exit);
        }
    }
