...
int soc_jer_trunk_init_tables (int unit)
{
    ...
    /* IRR_LAG_NEXT_MEMBERm/TAR_LAG_NEXT_MEMBERm is dynamic, so we enable dynamic access. this should be removed if table become static */
    ...
    /* disable back dynamic access. Do this for QAX, QUX and JERICHO PLUS. for Jericho devices we don't want this change inorder not casue bugs due to writes to dynamic memory */
