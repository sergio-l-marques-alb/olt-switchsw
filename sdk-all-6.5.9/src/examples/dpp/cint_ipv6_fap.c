...
int bcm_l3_ip6_uc_priority(int unit, int port0, int port1, int port2, int port3) {
    ...
    bcm_ip6_t mask   = {0,0,0,0,0,0,0,0,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff};
...
int bcm_l3_basic_ip6_mc(int unit, int port0, int port1, int port2, int port3) {
    ...
    for (i=0; i < NOF_BASIC_MC_GROUP; i++) {
        ...
        for(j=0; j<16; j++) { data.s_ip6_addr[j] = 0xA;}
...
int ipv6_mc_prefix(int unit, int port0, int port1, int port2, int port3, int table_id) {
    ...
    int ipmc_group[NOF_IPV6_MC_PREFIX_INTERFACES];
    ...
    for (i=0; i < NOF_IPV6_MC_PREFIX_INTERFACES; i++) {
        ...
        /* Associate each ipmc_group to i%4 + 1 ports */
        for (j=0; j < (i % 4) + 1; j++) {
            rc = bcm_multicast_ingress_add(unit, ipmc_group[i], ports[j], l3if.l3a_intf_id);
              if (rc != BCM_E_NONE) {
                  printf("Error, bcm_multicast_ingress_add");
                  return rc;
              }
    ...
    for (prefix_length=128; prefix_length >= 0; prefix_length-=4) {
        ...
        l3rts_ipv6_mc_traverse[interface_counter].group = ipmc_group[interface_counter];
        l3rts_ipv6_mc_traverse[interface_counter].flags = BCM_IPMC_SOURCE_PORT_NOCHECK | BCM_IPMC_IP6;
...
int ipv6_mc_default_route(int unit, int out_port, int out_port2) {
    ...
    bcm_ip6_t ip6_addr = { 0xff, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xff, 0xed, 0xcb, 0xa9, 0x87, 0x65, 0x43, 0x21 };
    int prefix_length[3] = { 128, 64, 0 };

    for(i = 0; i < 3; i++) {
        vlan = 100 + i * 100;
        ipmc_group = 0x2000001 + vlan + i;
        ...
        rv = bcm_multicast_ingress_add(unit, ipmc_group, out_port, l3_intf.l3a_intf_id);
        if(rv) {
            printf("Error, bcm_multicast_ingress_add out_port index %d\n", i);
            return rv;
        }
        rv = bcm_multicast_ingress_add(unit, ipmc_group, out_port2, l3_intf.l3a_intf_id);
        if(rv) {
            printf("Error, bcm_multicast_ingress_add out_port2 index %d\n", i);
            return rv;
        }
...
int ipv6_route_prefix_sem_traverse_cb(int unit, int index, bcm_l3_route_t *info, void *data){

    bcm_l3_route_t l3route = l3routelist[index];
    bcm_ip6_t address;
...
int ipv6_route_prefix_sem(int unit) {
...
int ipv6_route_prefix(int unit, int out_port) {
    ...
    for(prefix = 124; prefix >= 0; prefix -= 4) {
        ipv6_create_mask(l3route.l3a_ip6_mask, prefix);
        frwd_uc_config_intf_example(unit, out_port, vrf, &(l3route.l3a_intf), NULL, NULL, NULL);
...
int ipv6_route_prefix_entry_delete(int unit, int prefix) {
