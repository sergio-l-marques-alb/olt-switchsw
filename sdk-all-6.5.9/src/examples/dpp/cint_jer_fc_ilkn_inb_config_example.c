...
int cint_jer_test_fc_ilkn_inb_src_vsq_gen_set_example(int unit, int is_pfc, int ilkn_port, int inb_calendar_idx, int shaped_port)
{
    ...
    /* Trigger global fc : Setting FC threshold and Shaping local port*/
    rv = cint_jer_test_fc_src_vsq_trigger_set_example(unit, is_pfc, ilkn_port, shaped_port, &vsq_gport_global);
    if (rv != BCM_E_NONE) {
        printf("Trigger src vsq fc failed(%d) in cint_jer_test_fc_src_vsq_trigger_set_example.\n", rv);
        return rv;
    }

    fc_trigger.port = vsq_gport_global;
    /* setting flags */
    fc_trigger.flags = 0;
    /* setting cosq */
    fc_trigger.cosq = 0;
...
int cint_jer_test_fc_ilkn_inb_src_vsq_gen_unset_example(int unit, int ilkn_port, int inb_calendar_idx, int shaped_port)
{
    int rv = BCM_E_NONE;
    bcm_cosq_fc_endpoint_t fc_trigger;
    bcm_cosq_fc_endpoint_t fc_handler;
    ...
    fc_handler.flags = 0;
    fc_handler.cosq = -1; /* cosq can be 0 or -1, but -1 is recommended */
    fc_handler.calender_index = inb_calendar_idx; /* setting calender index */

    rv = bcm_cosq_fc_path_delete(unit, bcmCosqFlowControlGeneration, &fc_trigger, &fc_handler);
    if (rv != BCM_E_NONE) {
        printf("fc generation delete failed(%d) in cint_jer_test_fc_ilkn_inb_src_vsq_gen_unset_example.\n", rv);
        return rv;
    }

    /* Trigger global fc : Setting FC threshold and Shaping local port*/
    rv = cint_jer_test_fc_src_vsq_trigger_unset_example(unit, ilkn_port, shaped_port, vsq_gport_global);
    if (rv != BCM_E_NONE) {
