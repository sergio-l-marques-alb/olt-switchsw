...
#define PRT_FREE                     if(prt_ctr != NULL)                                                               \
                                     {                                                                                 \
                                         ...
                                         if(prt_ctr->info_list != NULL)                                                \
                                             utilex_rhlist_free_all(prt_ctr->info_list);                               \
                                         sal_free(prt_ctr);                                                            \
                                         prt_ctr = NULL;                                                               \
                                     }
 ...
 * \par INDIRECT INPUT: prt_ctr      - control structure that provides all the info on the table
 *                      sand_control - control structure that provides all command related info
 * \par INDIRECT OUTPUT: printing or formatted output to the standard output
 * \remark automatically frees the list
 */
#define PRT_COMMITX { \
                      diag_sand_prt(prt_ctr, sand_control);                                                            \
                      PRT_FREE;                                                                                        \
                    }

#define PRT_COMMIT  { \
                      diag_sand_prt(prt_ctr, NULL);                                                                    \
                      PRT_FREE;                                                                                        \
                    }

#define PRT_CELL_VERIFY \
            if(prt_ctr->row_cur == NULL)                                                                               \
            {                                                                                                          \
...
#define _PRT_INFO_ADD(...) \
        {                                                                                                              \
            ...
            if(prt_ctr->info_list == NULL)                                                                             \
            {                                                                                                          \
                if((prt_ctr->info_list = utilex_rhlist_create("prt_print", sizeof(rhentry_t), 0)) == NULL)             \
                {                                                                                                      \
                    SHR_CLI_EXIT(_SHR_E_MEMORY,"info_list create %s", "failed\n");                                     \
                }                                                                                                      \
            }                                                                                                          \
            if(utilex_rhlist_entry_add_tail(prt_ctr->info_list, NULL, RHID_TO_BE_GENERATED, &entry) == _SHR_E_MEMORY)  \
            {                                                                                                          \
                 SHR_CLI_EXIT(_SHR_E_MEMORY,"add entry to info_list  %s", "failed\n");                                 \
            }                                                                                                          \
            PRT_FORMAT(RHNAME(entry), RHNAME_MAX_SIZE - 1, __VA_ARGS__);                                               \
        }
 ...
 * \par DIRECT INPUT variadic variables
 * \par INDIRECT INPUT prt_ctr - Control structure containing information on table to be printed
 * \remark MACRO limits the size to RHNAME_MAX_SIZE - 1
 */
#define PRT_INFO_ADD(...) _PRT_INFO_ADD(__VA_ARGS__)
