...
bool cmicx_update() {
    ...
    /*
    cmicx_schan_update(&cmicx->cmc0.schan_data);
    cmicx_schan_update(&cmicx->cmc1.schan_data);
    cmicx_schan_update(&cmicx->common.schan_data);
    */

    /* update the state of each outstanding sbus dma unit */
    /*
    cmicx_sbus_dma_update(&cmicx->cmc0.sbus_dma_ch0);
    cmicx_sbus_dma_update(&cmicx->cmc0.sbus_dma_ch1);
    cmicx_sbus_dma_update(&cmicx->cmc0.sbus_dma_ch2);
    cmicx_sbus_dma_update(&cmicx->cmc1.sbus_dma_ch0);
    cmicx_sbus_dma_update(&cmicx->cmc1.sbus_dma_ch1);
    cmicx_sbus_dma_update(&cmicx->cmc1.sbus_dma_ch2);
    */
    ...
    /*
    for (i=0; i<CMICX_NUM_PACKET_DMA_PER_CMC; i++) {
        cmicx_packet_dma_update(&cmicx->cmc0.packet_dma[i]);
        cmicx_packet_dma_update(&cmicx->cmc1.packet_dma[i]);
    }
    */
    ...
    for (i=0; i<CMICX_NUM_SCHAN_IN_COMMON; i++) {
        ...
        for (j=0; j<cmicx->sbus_ring_arb_ctrl_schan_common_ch_weight[i]; j++) {
            if (cmicx_schan_update(&cmicx->common.schan[i])==false) {
    ...
    */
    ...
    for (i=0; i<CMICX_NUM_SBUS_DMA_PER_CMC; i++) {
        for (j=0; j<cmicx->sbus_ring_arb_ctrl_sbusdma_cmc0_ch_weight[i]; j++) {
            if (cmicx_sbus_dma_update(&cmicx->cmc0.sbus_dma[i])==false) {
    ...
    for (i=0; i<CMICX_NUM_SBUS_DMA_PER_CMC; i++) {
        for (j=0; j<cmicx->sbus_ring_arb_ctrl_sbusdma_cmc1_ch_weight[i]; j++) {
            if (cmicx_sbus_dma_update(&cmicx->cmc1.sbus_dma[i])==false) {
                cmicx_log("ERROR: failure to update sbus dma \"");
...
bool cmicx_pcid_interrupt_send() {
    int rv;
...
bool cmicx_pcid_mem_write(addr_t addr,void *mem,int amount) {
    ...
    /* sdk and standalone sim case: write each word to the internal cmicx sim memory */
    for (i=0; i<amount; i++) {
...
bool cmicx_switch_reg_read(addr_t addr,reg_t *val) {
    ...
    /* write the value 0xBEEF to the register, and then return that value */
    if (cmicx->last_switch_register!=(CMICX_MAX_SWITCH_REGISTERS-1)) {
        ...
        cmicx->switch_registers[         cmicx->last_switch_register] = 0xBEEF;
...
bool cmicx_switch_mem_read(addr_t addr,reg_t *words,int req,int *resp) {
    ...
    /* create the memory with the request size, fill it with beef, and then return this memory */
    if (cmicx->last_switch_memory!=(CMICX_MAX_SWITCH_MEMORIES-1)) {
        ...
        for (j=1; j<=req; j++) {
            cmicx->switch_memories[cmicx->last_switch_memory][j] = 0xBEEF;
        }
