...
bool cmic_schan_sbus_command_complete(reg_t *command,reg_t *ack_command,int expected_resp_words,int *num_words_in_command) {
    ...
    /* variables */
    schan_command_opcode opcode = CMIC_SCHAN_COMMAND_STRIP_OPCODE(command[0]);
    int                  dlen   = CMIC_SCHAN_COMMAND_STRIP_DLEN(  command[0]);
    char                 opcode_id[16];
    schan_command_opcode ack_opcode;
    int                  ack_dlen;
    int                  switch_mem_read_num_words;

    /* determine which ack we need to send and the ack dlen, based on the command opcode */
    switch (opcode)
    {
        case SCHAN_MEM_RD:
            cmicx_switch_mem_read(command[1],&(ack_command[1]),expected_resp_words,&switch_mem_read_num_words);
            ack_opcode = SCHAN_MEM_RD_ACK;
            ack_dlen   = (switch_mem_read_num_words*CMIC_SBUS_DMA_BYTES_PER_WORD);
            strcpy(opcode_id, "MEM_RD");
            *num_words_in_command = 2;
            break;
        case SCHAN_MEM_WR:
            cmicx_switch_mem_write(command[1],&command[2],(dlen/CMIC_SBUS_DMA_BYTES_PER_WORD));
