...
#else
int interactive_loop(int fd_in, int fd_out, char *file1, char *file2, int cmdflag);
#endif /* L7_SSHD */
...
#else
    if (do_download(conn, abs_src, dst, pflag) == -1)
        err = -1;
#endif /* L7_SSHD */
...
#else
    if (do_upload(conn, src, tmp_dst, pflag) == -1)
        err = -1;
...
#else
        if (!osapiStrCaseCmp((char *)cp, (char *) cmds[i].c, cmdlen) &&
#endif /* L7_SSHD */
...
#else
    char *path1, *path2;
    int pflag=0, lflag, iflag, cmdnum;
#endif /* L7_SSHD */
    unsigned long n_arg;
#ifndef L7_SSHD
    Attrib a, *aa;
...
#else
interactive_loop(int fd_in, int fd_out, char *file1, char *file2, int cmdflag)
#endif /* L7_SSHD */
{
    char *pwd;
#ifndef L7_SSHD
    ...
    else
    {
      if (cmdflag == I_GET)
      {
        osapiSnprintf(cmd, sizeof cmd, "get %s %s", file1, file2);
      }
      else /* PUT */
      {
        osapiSnprintf(cmd, sizeof cmd, "put %s %s", file1, file2);
      }
...
#else
sftpc_main(char *hostname, int address_family, char *username, char *password,
...
#endif /* L7_SSHD */
{
    ...
    /*log_init("L7_SFTPC", SYSLOG_LEVEL_INFO, SYSLOG_FACILITY_NOT_SET, 1);*/
    in = ssh_client_main(hostname, address_family, username, password, "sftp", cmd, &cn);
    out = in;
    if (in != -1)
    {
      if (cmd == L7_TRANSFER_DIRECTION_DOWN)
      {
        err = interactive_loop(in, out, remote_file, local_file, I_GET);
      }
      else
      {
        err = interactive_loop(in, out, local_file, remote_file, I_PUT);
      }

      logit("SFTP command completed");
    }
    else
    {
      return -1;
    ...
    exit(err == 0 ? 0 : 1);
#else
    sshc_quit_pending(cn, in);
    shutdown(in, SHUT_RDWR);
    close(in);
    /*osapiTaskDelete(sshpid);*/
    return err;
#endif /* L7_SSHD */
