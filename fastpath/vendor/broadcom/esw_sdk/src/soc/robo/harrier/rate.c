    ...
    uint32 burst_size)
{
    ...
    if ((limit > HARRIER_RATE_METER_MAX) ||
        (limit < HARRIER_RATE_METER_MIN)) {
        soc_cm_debug(DK_ERR, 
            "_drv_harrier_port_erc_set : rate unsupported.\n");
        return  SOC_E_PARAM;
    }
    ...
    if ((burst_size > HARRIER_RATE_BURST_MAX) ||
        (burst_size < HARRIER_RATE_BURST_MIN)) {
        soc_cm_debug(DK_ERR, 
            "_drv_harrier_port_erc_set : burst size unsupported.\n");
        return SOC_E_PARAM;
    }
    ...
    uint32 limit, uint32 burst_size)
{
    ...
    if ((limit > HARRIER_RATE_METER_MAX) ||
        (limit < HARRIER_RATE_METER_MIN)) {
        soc_cm_debug(DK_ERR, 
            "_drv_harrier_port_queue_erc_set : rate unsupported.\n");
        return  SOC_E_PARAM;
    }
    ...
    if ((burst_size > HARRIER_RATE_BURST_MAX) ||
        (burst_size < HARRIER_RATE_BURST_MIN)) {
        soc_cm_debug(DK_ERR, 
            "_drv_harrier_port_queue_erc_set : burst size unsupported.\n");
        return SOC_E_PARAM;
    }
    ...
    uint32 value)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_rate_config_set: \
    ...
    uint32 *value)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_rate_config_get: \
    ...
    uint32 kbits_sec_max, uint32 burst_size)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_rate_set: unit = %d, bmp = 0x%x, %s, flags = 0x%x, \
    ...
    uint32 *kbits_sec_max, uint32 *burst_size)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_rate_get: unit = %d, port = %d, %s, flags = 0x%x, \
        kbits_sec_min = %dK, kbits_sec_max = %dK, burst size = %dKB\n", 
