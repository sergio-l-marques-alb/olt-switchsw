            ...
            uint32 port, drv_mcrep_control_flag_t op, int *param)
{
    ...
    assert(param);
    if (mc_group >= DRV_MAX_MCREP_MCAST_GROUP_NUM){
        ...
        drv_mcrep_control_flag_t op, uint32 vport, uint32 vid)
{
    ...
    /* valid section : assume the port id is verified already */
    if (vport >= DRV_MAX_MCREP_VPORT_NUM){
    ...
    switch(op){
        ...
        case DRV_MCREP_VPORT_OP_UNTAG_VP:
            ...
            if (fld_val32 == 0){
                 ...
                 */
                *vport = DRV_VPORT_NONE;
            } else {
                for (i = 0; i < DRV_MAX_MCREP_VPORT_NUM; i++){
                    if (fld_val32 & (0x1 << i)){
                        break;
                    }
                }
                assert(i < DRV_MAX_MCREP_VPORT_NUM);
                *vport = i;
            }
            break;
        ...
        uint32 *vport, int *param)
{
    ...
    if(i < DRV_MAX_MCREP_VPORT_NUM){    /* found */
