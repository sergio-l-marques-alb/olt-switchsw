...
/*
 *  Function : _drv_bcm53280_port_irc_set
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_port_irc_set(int unit, uint32 port, uint32 limit,
    uint32 burst_size)
{
    ...
    if ((limit > TB_RATE_METER_MAX(unit)) ||
        ...
        soc_cm_debug(DK_ERR, 
            "drv_bcm53280_port_irc_set : rate unsupported. \n");
        return SOC_E_PARAM;
    }
    ...
    if ((burst_size > TB_RATE_BURST_MAX(unit)) ||
        ...
        soc_cm_debug(DK_ERR, 
            "drv_bcm53280_port_irc_set : burst size unsupported. \n");
        return SOC_E_PARAM;
    }
...
/*
 *  Function : _drv_bcm53280_port_irc_get
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_port_irc_get(uint32 unit, uint32 port, uint32 *limit, 
    uint32 *burst_size)
{
    irc_port_entry_t  irc_entry;
    uint32  index, field_val32;
    ...
    if (robo_ingress_rate_init_flag) {
         ...
         */ 
        robo_ingress_rate_init_flag = (field_val32 == 0) ? 0 : 1;
    }
    ...
    if (field_val32 == 0) {
        ...
        *limit = field_val32 * TB_RATE_REF_COUNT_GRANULARITY(unit);
        ...
        rv = soc_IRC_PORTm_field_get(unit,  (uint32 *)&irc_entry, 
            BKT0_BKT_SIZEf, &field_val32);
        SOC_IF_ERROR_RETURN(rv);
        *burst_size = (field_val32 * TB_RATE_BUCKET_UNIT_SIZE(unit) * 8) 
            / 1000;
    }
...
/*
 *  Function : _drv_bcm53280_port_erc_check
 *
 *  Purpose :
 ...
 */
static int
_drv_bcm53280_port_erc_check(uint32 unit, uint32 port, uint32 ref_cnt)
{
...
/*
 *  Function : _drv_bcm53280_port_erc_set
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_port_erc_set(uint32 unit, uint32 port, uint32 limit, 
    uint32 burst_size)
{
    erc_port_entry_t  erc_entry;
    ...
    if (rate_type == TB_RATE_TYPE_PPS) {
        ...
        if ((limit > TB_RATE_METER_PKT_MAX(unit)) ||
            (limit < TB_RATE_METER_MIN(unit))) {
            soc_cm_debug(DK_ERR, 
                "drv_bcm53280_port_erc_set : rate unsupported. \n");
            return  SOC_E_PARAM;
        }        
    ...
    if ((burst_size > TB_RATE_BURST_MAX(unit)) ||
        (burst_size < TB_RATE_BURST_MIN(unit))) {
        soc_cm_debug(DK_ERR, 
            "drv_bcm53280_port_erc_set : burst size unsupported. \n");
        return SOC_E_PARAM;
    }
    ...
    if (limit == 0) { /* Disable egress rate control for Total */
        ...
        /* Software protection : Max rate of total bucket > Min rate of all queues. */
        rv = _drv_bcm53280_port_erc_check(unit, port, ref_cnt);
        if (rv < 0) {
            soc_cm_debug(DK_ERR, 
                "_drv_bcm53280_port_erc_set : Max rate of total < Min rate of all queues.\n");
            return SOC_E_PARAM;
        }
        ...
        if (field_val32 < TB_RATE_ERC_MIN_AVAIL_BUCKET_SIZE(unit)) {
            soc_cm_debug(DK_ERR, 
                "Bucket size < ERC Minimum Available Bucket Size.\n");
            return SOC_E_PARAM;
        }
...
/*
 *  Function : _drv_bcm53280_port_erc_get
 *
 *  Purpose :
 ...
 */
int
_drv_bcm53280_port_erc_get(uint32 unit, uint32 port, uint32 *limit, 
    uint32 *burst_size)
{
    erc_port_entry_t  erc_entry;
...
/*
 *  Function : _drv_bcm53280_port_queue_erc_check
 *
 *  Purpose :
 ...
 */
static int
_drv_bcm53280_port_queue_erc_check(uint32 unit, uint32 port, uint8 queue_n,
    uint32 ref_cnt)
{
    erc_port_entry_t  erc_entry;
    uint32  index, field_val32 = 0, valid = 0;
...
/*
 *  Function : _drv_bcm53280_port_queue_erc_set
 *
 *  Purpose :
 *     Set the burst size and rate limit value of the selected port egress rate.
 *
 ...
 *      port  :  port id.
 *      queue_n  :  COSQ id.
 *      kbits_sec_min  :  minimum bandwidth, kbits/sec (Kbits : kilobits (1000 bits)).
 *      kbits_sec_max  :  maximum bandwidth, kbits/sec (Kbits : kilobits (1000 bits)).
 *      burst_size  :  max burst size (Kbits : kilobits (1000 bits)).
 *
 *  Return :
 *      SOC_E_XXX
 ...
 */
int
_drv_bcm53280_port_queue_erc_set(uint32 unit, uint32 port, uint8 queue_n, 
    uint32 kbits_sec_min, uint32 kbits_sec_max, uint32 burst_size)
{
    ...
    if (kbits_sec_max == 0) { /* Disable egress rate control for Q-n */
        ...
        if (rate_type == TB_RATE_TYPE_PPS){
            /* pps */
            ref_cnt_min = kbits_sec_min  / 
                TB_RATE_REF_COUNT_PKT_GRANULARITY(unit);
        } else {
            /* kbps*/
            ref_cnt_min = kbits_sec_min  / 
                TB_RATE_REF_COUNT_GRANULARITY(unit);
        }
        ...
        /* Software protection : Min rate of all queues < Max rate of total bucket. */
        if (field_val32) {
            rv = _drv_bcm53280_port_queue_erc_check(unit, port, queue_n, ref_cnt_min);
            if (rv < 0) {
                soc_cm_debug(DK_ERR, 
                    "_drv_bcm53280_port_queue_erc_set : Min rate of all queues > Max rate of total.\n");
                return SOC_E_PARAM;
            }
        }
...
/*
 *  Function : _drv_bcm53280_queue_port_erc_get
 *
 *  Purpose :
 *     Get the burst size and rate limit value of the selected port.
 *
 ...
 *      port  :  port id.
 *      queue_n  :  COSQ id.
 *      kbits_sec_min  :  (OUT) minimum bandwidth, kbits/sec (Kbits : kilobits (1000 bits)).
 *      kbits_sec_max  :  (OUT) maximum bandwidth, kbits/sec (Kbits : kilobits (1000 bits)).
 *      burst_size  :  (OUT)max burst size (Kbits : kilobits (1000 bits)).
 *
 *  Return :
 *      SOC_E_XXX
 ...
 */
int
_drv_bcm53280_port_queue_erc_get(uint32 unit, uint32 port, uint8 queue_n,
    uint32 *kbits_sec_min, uint32 *kbits_sec_max, uint32 *burst_size)
{
    erc_port_entry_t  erc_entry;
    uint32  index, field_val32;
...
/*
 *  Function : drv_bcm53280_rate_config_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_rate_config_set(int unit, soc_pbmp_t pbmp, uint32 config_type, 
    uint32 value)
{
    irc_port_entry_t  irc_entry;
    erc_port_entry_t  erc_entry;
...
/*
 *  Function : drv_bcm53280_rate_config_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_rate_config_get(int unit, uint32 port, uint32 config_type, 
    uint32 *value)
{
    irc_port_entry_t  irc_entry;
    erc_port_entry_t  erc_entry;
...
/*
 *  Function : drv_bcm53280_rate_set
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_rate_set(int unit, soc_pbmp_t bmp, uint8 queue_n, int direction, 
    uint32 kbits_sec_min, uint32 kbits_sec_max, uint32 burst_size)
{
    uint32  port;
    int  rv = SOC_E_NONE;
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm53280_port_irc_set(unit, port, kbits_sec_max, burst_size));
            }
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm53280_port_erc_set(unit, port, kbits_sec_max, burst_size));
            }                
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(
                    _drv_bcm53280_port_queue_erc_set
                        (unit, port, queue_n, kbits_sec_min, kbits_sec_max, burst_size));
            }                
            break;
        default:
            return SOC_E_PARAM;
...
/*
 *  Function : drv_bcm53280_rate_get
 *
 *  Purpose :
 ...
 */
int 
drv_bcm53280_rate_get(int unit, uint32 port, uint8 queue_n, int direction, 
    uint32 *kbits_sec_min, uint32 *kbits_sec_max, uint32 *burst_size)
{
    int  rv = SOC_E_NONE;
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            SOC_IF_ERROR_RETURN(
                _drv_bcm53280_port_irc_get(unit, port, kbits_sec_max, burst_size));
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
            SOC_IF_ERROR_RETURN(
                _drv_bcm53280_port_erc_get(unit, port, kbits_sec_max, burst_size));
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
            SOC_IF_ERROR_RETURN(
                _drv_bcm53280_port_queue_erc_get
                    (unit, port, queue_n, kbits_sec_min, kbits_sec_max, burst_size));
