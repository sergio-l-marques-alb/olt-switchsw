...
#include <soc/robo_stat.h>
    ...
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
             ...
             case DRV_SNMP_ETHER_STATS_BROADCAST_PKTS:
                 if (SOC_IS_TBX(unit) || SOC_IS_POLAR(unit) || 
                     SOC_IS_NORTHSTAR(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_POLAR_SUPPORT) || \
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS)
                     soc_robo_counter_set(unit, port, RXBROADCASTPKTSr, val);
#endif
                 } else {
                     soc_robo_counter_set(unit, port, RXBROADCASTPKTr, val);
     ...
     int sync_hw, uint64 *val)
 {
     ...
     switch (counter_type) 
     {
         ...
         case DRV_SNMP_IF_IN_OCTETS:         
             soc_robo_counter_get(unit, port, RXOCTETSr, sync_hw, &count);
             break;
 
         case DRV_SNMP_IF_IN_UCAST_PKTS:     
             /* Total non-error frames minus broadcast/multicast frames */
             soc_robo_counter_get(unit, port, RXUNICASTPKTSr, sync_hw, &count);   
             break;
 
         case DRV_SNMP_IF_IN_N_UCAST_PKTS:    
             /* Multicast frames plus broadcast frames */
             soc_robo_counter_get(unit, port, RXMULTICASTPKTSr, 
                 sync_hw, &count_tmp);
             count=count_tmp;
             if (SOC_IS_TBX(unit) || SOC_IS_POLAR(unit) || 
                 SOC_IS_NORTHSTAR(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_POLAR_SUPPORT) || \
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXBROADCASTPKTSr, 
                     sync_hw, &count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, RXBROADCASTPKTr, 
                     sync_hw, &count_tmp);
             }
             COMPILER_64_ADD_64(count, count_tmp); 
             soc_robo_counter_get(unit, port, RXPAUSEPKTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp); 
             break;
         ...
         case DRV_SNMP_IF_IN_DISCARDS:
             if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                 soc_robo_counter_get(unit, port, RXFWDDISCPKTSr, 
                     sync_hw, &count);
#endif
             } else if (SOC_IS_HARRIER(unit)){
#ifdef BCM_HARRIER_SUPPORT            
                 soc_robo_counter_get(unit, port, RXDROPPKTSr, 
                     sync_hw, &count);
                 soc_robo_counter_get(unit, port, RXFWDDISCPKTSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
                 ...
                 soc_robo_counter_get(unit, port, RXDROPPKTSr, 
                     sync_hw, &count);
                 soc_robo_counter_get(unit, port, RXDISCARDr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else if (SOC_IS_DINO8(unit)) {
         ...
         case DRV_SNMP_IF_IN_ERRORS: 
             soc_robo_counter_get(unit, port, RXALIGNMENTERRORSr, 
                 sync_hw, &count_tmp);
             count=count_tmp;
             soc_robo_counter_get(unit, port, RXFCSERRORSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             soc_robo_counter_get(unit, port, RXFRAGMENTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             soc_robo_counter_get(unit, port, RXOVERSIZEPKTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             soc_robo_counter_get(unit, port, RXUNDERSIZEPKTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                 soc_robo_counter_get(unit, port, RXJABBERPKTSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
                 soc_robo_counter_get(unit, port, RXSYMBOLERRr, 
                     sync_hw, &count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, RXJABBERSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
                 soc_robo_counter_get(unit, port, RXSYMBLERRr, 
                     sync_hw, &count_tmp);
             }
             COMPILER_64_ADD_64(count, count_tmp);
             break;
...
#ifdef BCM_HARRIER_SUPPORT
                 soc_robo_counter_get(unit, port, TXQOS0OCTETSr, 
                     sync_hw, &count_tmp);
                 count=count_tmp;
                 soc_robo_counter_get(unit, port, TXQOS1OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
                 soc_robo_counter_get(unit, port, TXQOS2OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
                 soc_robo_counter_get(unit, port, TXQOS3OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, TXOCTETSr, sync_hw, &count);
             }
             break;
 
         case DRV_SNMP_IF_OUT_UCAST_PKTS:  /* ALL - mcast - bcast */
             soc_robo_counter_get(unit, port, TXUNICASTPKTSr, 
                 sync_hw, &count);
             break;
 
         case DRV_SNMP_IF_OUT_N_UCAST_PKTS:  
             /* broadcast frames plus multicast frames */
             soc_robo_counter_get(unit, port, TXBROADCASTPKTSr, 
                 sync_hw, &count_tmp);
             count=count_tmp;
             soc_robo_counter_get(unit, port, TXMULTICASTPKTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp); 
             soc_robo_counter_get(unit, port, TXPAUSEPKTSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             break;
         ...
         case DRV_SNMP_IF_OUT_DISCARDS:
             if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                 soc_robo_counter_get(unit, port, TXPORTCONGESTIONDROPr, 
                     sync_hw, &count);
#endif
             } else {
                 soc_robo_counter_get(unit, port, TXDROPPKTSr, 
                     sync_hw, &count_tmp);
                 count=count_tmp;
                 soc_robo_counter_get(unit, port, TXFRAMEINDISCr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
             }
             break;
         ...
         case DRV_SNMP_IF_OUT_ERRORS:
             soc_robo_counter_get(unit, port, TXEXCESSIVECOLLISIONr, 
                 sync_hw, &count_tmp);
             count=count_tmp;
             soc_robo_counter_get(unit, port, TXLATECOLLISIONr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             break;
         ...
         case DRV_SNMP_DOT1D_TP_PORT_IN_FRAMES:
             soc_robo_counter_get(unit, port, RXUNICASTPKTSr, 
                 sync_hw, &count_tmp);
             count=count_tmp;
             soc_robo_counter_get(unit, port, RXMULTICASTPKTSr, 
                 sync_hw, &count_tmp);
    ...
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
         ...
         case DRV_SNMP_ETHER_STATS_PKTS64_OCTETS:
             if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXPKTS64OCTETSr, 
                     sync_hw, &count_tmp);
                 count = count_tmp;
                 soc_robo_counter_get(unit, port, TXPKTS64OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, PKTS64OCTETSr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_ETHER_STATS_PKTS65TO127_OCTETS:
             if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXPKTS65TO127OCTETSr, 
                     sync_hw, &count_tmp);
                 count = count_tmp;
                 soc_robo_counter_get(unit, port, TXPKTS65TO127OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, PKTS65TO127OCTETSr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_ETHER_STATS_PKTS128TO255_OCTETS:
             if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXPKTS128TO255OCTETSr, 
                     sync_hw, &count_tmp);
                 count = count_tmp;
                 soc_robo_counter_get(unit, port, TXPKTS128TO255OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, PKTS128TO255OCTETSr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_ETHER_STATS_PKTS256TO511_OCTETS:
             if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXPKTS256TO511OCTETSr, 
                     sync_hw, &count_tmp);
                 count = count_tmp;
                 soc_robo_counter_get(unit, port, TXPKTS256TO511OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, PKTS256TO511OCTETSr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_ETHER_STATS_PKTS512TO1023_OCTETS:
             if (SOC_IS_TBX(unit) || SOC_IS_NORTHSTARPLUS(unit)) {
#if defined(BCM_TB_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
                 soc_robo_counter_get(unit, port, RXPKTS512TO1023OCTETSr, 
                     sync_hw, &count_tmp);
                 count = count_tmp;
                 soc_robo_counter_get(unit, port, TXPKTS512TO1023OCTETSr, 
                     sync_hw, &count_tmp);
                 COMPILER_64_ADD_64(count, count_tmp);
#endif
             } else {
                 soc_robo_counter_get(unit, port, PKTS512TO1023OCTETSr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_ETHER_STATS_OCTETS:
             soc_robo_counter_get(unit, port, RXOCTETSr, 
                 sync_hw, &count_tmp);
             count = count_tmp;
             soc_robo_counter_get(unit, port, TXOCTETSr, 
                 sync_hw, &count_tmp);
             COMPILER_64_ADD_64(count, count_tmp);
             break;
    ...
    defined(BCM_NORTHSTARPLUS_SUPPORT)
         ...
         case DRV_SNMP_ETHER_STATS_COLLISIONS:
             soc_robo_counter_get(unit, port, TXCOLLISIONSr, sync_hw, &count); 
             break;
    ...
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
         ...
         case DRV_SNMP_DOT3_STATS_SINGLE_COLLISION_FRAMES:   
             ...
             if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                 soc_robo_counter_get(unit, port, TXSINGLECOLLISIONSr, 
                     sync_hw, &count);
#endif
             } else {
                 soc_robo_counter_get(unit, port, TXSINGLECOLLISIONr, 
                     sync_hw, &count);
             }
             break;
         ...
         case DRV_SNMP_DOT3_STATS_MULTIPLE_COLLISION_FRAMES: 
             ...
             if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
                 soc_robo_counter_get(unit, port, TXMULTIPLECOLLISIONSr, 
                     sync_hw, &count);
#endif
             } else {
                 soc_robo_counter_get(unit, port, TXMULTIPLECOLLISIONr, 
                     sync_hw, &count);
             }
             break;
 
         case DRV_SNMP_DOT3_STATS_DEFERRED_TRANSMISSIONS:    
             soc_robo_counter_get(unit, port, TXDEFERREDTRANSMITr, 
                 sync_hw, &count);
             break;  
         ...
         case DRV_SNMP_DOT3_IN_PAUSE_FRAMES:  
             soc_robo_counter_get(unit, port, RXPAUSEPKTSr, sync_hw, &count);
             break;  
 
         case DRV_SNMP_DOT3_OUT_PAUSE_FRAMES:  
             soc_robo_counter_get(unit, port, TXPAUSEPKTSr, sync_hw, &count);
             break;  
    ...
    defined(BCM_NORTHSTAR_SUPPORT) || defined(BCM_NORTHSTARPLUS_SUPPORT)
         ...
         case DRV_SNMP_BCM_ETHER_STATS_PKTS2048TO4095_OCTETS:
             if (SOC_IS_DINO8(unit)) {
#ifdef BCM_DINO8_SUPPORT
                 soc_robo_counter_get(unit, port, PKTS2048TO4095r, 
                     sync_hw, &count);    
                 break;
#endif /* BCM_DINO8_SUPPORT */
             } else {
