/*
 * $Id: mem.c 1.34.56.9 Broadcom SDK $
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0;i < count; i++ ) {
        ...
        if ((index % 2) == 0) {
            ...
            if ((rv = REG_READ_MEM_DEBUG_DATA_0_0r(unit, (uint32 *)&value64)) < 0) {
                goto mem_read_exit;
            }
            ...
            if (++i < count) {
                entry32 = (uint32 *)gmem_entry;
                COMPILER_64_ZERO(value64);
                if ((rv = REG_READ_MEM_DEBUG_DATA_0_1r(unit, (uint32 *)&value64)) < 0) {
                    goto mem_read_exit;
                }
                entry32[0] = COMPILER_64_LO(value64);
                entry32[1] = COMPILER_64_HI(value64);
    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((index % 2) == 0) {
            ...
            if ((rv = REG_WRITE_MEM_DEBUG_DATA_0_0r(unit, (uint32 *)&value64)) < 0) {
                goto mem_write_exit;
            }
            if (++i < count) {
                gmem_entry++;
            ...
            entry32 = (uint32 *)gmem_entry;
            COMPILER_64_SET(value64, entry32[1], entry32[0]);
            if ((rv = REG_WRITE_MEM_DEBUG_DATA_0_1r(unit, (uint32 *)&value64)) < 0) {
                goto mem_write_exit;
            }
        }
