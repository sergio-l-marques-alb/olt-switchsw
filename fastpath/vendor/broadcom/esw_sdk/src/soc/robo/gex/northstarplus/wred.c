            ...
            uint32 *cell)
{
    ...
    SOC_IF_ERROR_RETURN(
        REG_READ_FC_LAN_TOTAL_THD_HYST_QNr(unit, q_id, &reg_val));
    soc_FC_LAN_TOTAL_THD_HYST_QNr_field_get(unit, &reg_val, 
        TOTAL_HYST_THDf, &fld_val);
    *cell = fld_val;
...
_drv_nsp_wred_config_set(int unit, int index, drv_wred_config_t *config)
{
    ...
    SOC_IF_ERROR_RETURN(
        _drv_nsp_queue_max_cell_get(unit, drv_map->port, drv_map->cosq, &cell)); 
...
{
    drv_wred_config_t       *drv_config;
    drv_wred_map_info_t     *drv_map, *pre, *cur;
    int                     rv = SOC_E_NONE;
    ...
    drv_config = &drv_wred[wred_id];
    drv_map = drv_config->map_list;
    pre = drv_map;
    cur = drv_map;

    /* Check if the port/cosq is already attached */
    while (cur) {
        if ((cur->port == map->port) &&
            (cur->cosq== map->cosq) &&
            (cur->flags == map->flags)) {
            /* matched */ 
            return SOC_E_EXISTS;
    ...
    cur = sal_alloc(sizeof(drv_wred_map_info_t), "wred_map");
    if (cur == NULL) {
        return SOC_E_MEMORY;
    }
    sal_memcpy(cur, map, sizeof(drv_wred_map_info_t));
    cur->next = NULL;
    if (NULL == drv_map) {
        drv_config->map_list = cur;
    } else {
        pre->next = cur;
    }
    ...
    if (drv_config->refer_count == 0) {
        ...
        rv = _drv_nsp_wred_config_set(unit, wred_id, drv_config); 
        if (rv != SOC_E_NONE) {
            sal_free(cur);
            drv_config->map_list = NULL;
            return rv;
    ...
    drv_config->refer_count++;

    /* Configure the WRED mapping table */
    rv = _drv_nsp_wred_map_set(unit, drv_config->hw_index, map); 
...
{
    drv_wred_config_t       *drv_config;
    drv_wred_map_info_t     *drv_map, *cur;
    int i;
