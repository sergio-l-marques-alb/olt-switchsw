    ...
    uint32 pause_on, uint32 pause_off)
{
    ...
    if (pause_on == 0) {  /* Disable ingress rate control */
        /* Disable shaper mode */
        temp = 1;
        SOC_IF_ERROR_RETURN(soc_BC_SUP_RATECTRL_Pr_field_set
        ...
        /* Pause of is fixed at 12K bytes of NorthstarPlus */
    ...
    switch (config_type) {
        ...
        case DRV_RATE_CONFIG_INGRESS_SHAPER_PAUSE_OFF:
            ...
            *value = temp;
            break;
        case DRV_RATE_CONFIG_INGRESS_SHAPER_PAUSE_ON:
            *value = NSP_INGRESS_RATE_PAUSE_ON_THRESHOLD_KBYTES * 8;
            break;
        default:
            return SOC_E_PARAM;
    }
        ...
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(_drv_nsp_port_erc_per_queue_set
                    (unit, port, queue_n, flags, kbits_sec_max, burst_size));
            }
            break;
        default:
            return SOC_E_PARAM;
