...
#else
            if ((SOC_IS_VULCAN(unit) || SOC_IS_LOTUS(unit) ||
                SOC_IS_STARFIGHTER(unit) || SOC_IS_NORTHSTARPLUS(unit)) && 
#endif          
                (port == 5)){
...

{
    ...
    if (PHY_ADDR_ROBO_CPU_MDIOBUS_CHK(phy_id)) {
        ...
        addr = SOC_EXTERNAL_PHY_BUS_CPUMDIO | phy_id;
        addr |= phy_reg_addr;
        SPI_LOCK;
                ...
                uint32 phy_reg_addr, uint16 *phy_rd_data)
{
    ...
    soc_cm_debug(DK_MIIM, "%s:id=0x%02x addr=0x%02x\n",
...
#if BCM_NSP_MACSEC_DEVICE_READY
    rv = soc_spi_read(unit, addr, (uint8*)phy_rd_data, len);
#else   /* BCM_NSP_MACSEC_DEVICE_READY */
    soc_cm_debug(DK_WARN, "%s:dummy read(addr=0x%04x)\n", 
            FUNCTION_NAME(), addr);
#endif  /* BCM_NSP_MACSEC_DEVICE_READY */
    ...
    *phy_rd_data = (*phy_rd_data>>8) | (*phy_rd_data<<8);
    ...
    soc_cm_debug(DK_MIIM, "%s:read data=0x%04x\n", 
                ...
                uint32 phy_reg_addr, uint16 phy_wr_data)
{
    ...
    soc_cm_debug(DK_MIIM, "%s: id=0x%02x addr=0x%02x data=0x%04x\n",
    ...
    rv = soc_spi_write(unit, addr, (uint8*)&phy_wr_data, len);
#else   /* BCM_NSP_MACSEC_DEVICE_READY */
    soc_cm_debug(DK_WARN, "%s:dummy write(addr=0x%04x)\n", 
            FUNCTION_NAME(), addr);
#endif  /* BCM_NSP_MACSEC_DEVICE_READY */
