...
#define TRUNK53242_HASH_FIELD_MACDA_VALUE  0x1
#define TRUNK53242_HASH_FIELD_MACSA_VALUE  0x2
#define TRUNK53242_HASH_FIELD_IP_MACDA_VALUE  0x4
#define TRUNK53242_HASH_FIELD_IP_MACSA_VALUE  0x8

#define TRUNK53242_HASH_FIELD_VALID_VALUE  (DRV_TRUNK_HASH_FIELD_MACDA | \
                                                                                 DRV_TRUNK_HASH_FIELD_MACSA | \
                                                                                 DRV_TRUNK_HASH_FIELD_IP_MACDA | \
                                                                                 DRV_TRUNK_HASH_FIELD_IP_MACSA)

static uint32   default_trunk_seed  = (TRUNK53242_HASH_FIELD_MACDA_VALUE |\
                                                          TRUNK53242_HASH_FIELD_MACSA_VALUE);
/*
...
drv_bcm53242_trunk_set(int unit, int tid, soc_pbmp_t bmp, uint32 flag, uint32 hash_op)
{
    ...
    soc_cm_debug(DK_PORT, 
        "drv_bcm53242_trunk_set: unit = %d, trunk id = %d, bmp = %x %x, flag = %x\n",
...
drv_bcm53242_trunk_get(int unit, int tid, soc_pbmp_t *bmp, uint32 flag, uint32 *hash_op)
{
    ...
    if (flag & DRV_TRUNK_FLAG_HASH_DEFAULT) {
        *hash_op = 0;
        if (default_trunk_seed & TRUNK53242_HASH_FIELD_MACDA_VALUE) {  /* DA */
            *hash_op |= DRV_TRUNK_HASH_FIELD_MACDA;
        }
        if (default_trunk_seed & TRUNK53242_HASH_FIELD_MACSA_VALUE) {  /* SA */
            *hash_op |= DRV_TRUNK_HASH_FIELD_MACSA;
        }
        if (default_trunk_seed & TRUNK53242_HASH_FIELD_IP_MACDA_VALUE) {  /* IP DA */
            *hash_op |= DRV_TRUNK_HASH_FIELD_IP_MACDA;
        }
        if (default_trunk_seed & TRUNK53242_HASH_FIELD_IP_MACSA_VALUE) {  /* IP SA */
        ...
        soc_cm_debug(DK_PORT, 
            "drv_bcm53242_trunk_get: unit = %d, trunk id = %d, value = 0x%x, flag = %x\n",
    ...
    soc_cm_debug(DK_PORT, 
        "drv_bcm53242_trunk_get: unit = %d, trunk id = %d, value = 0x%x 0x%x, flag = %x\n",
        unit, tid, SOC_PBMP_WORD_GET(*bmp, 0), SOC_PBMP_WORD_GET(*bmp, 1), flag);
...
drv_bcm53242_trunk_hash_field_add(int unit, uint32 field_type)
{
    ...
    soc_cm_debug(DK_PORT, 
        "drv_bcm53242_trunk_hash_field_add: unit = %d, field type = %d\n",
        unit, field_type);

    /* check the valid trunk hash field types */
    if (field_type & ~TRUNK53242_HASH_FIELD_VALID_VALUE) {
        soc_cm_debug(DK_WARN, 
            "drv_bcm53242_trunk_hash_field_add:hash type=%d, is invalid!\n",
            field_type);
...
drv_bcm53242_trunk_hash_field_remove(int unit, uint32 field_type)
{
    ...
    soc_cm_debug(DK_PORT, 
        "drv_bcm53242_trunk_hash_field_remove: unit = %d, field type = %d\n",
        unit, field_type);
