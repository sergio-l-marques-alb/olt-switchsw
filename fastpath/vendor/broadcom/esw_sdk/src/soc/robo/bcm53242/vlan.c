/*
 * $Id: vlan.c 1.21.122.2 Broadcom SDK $
 *
...
_drv_v2v_reset(int unit, int reset_type)
{
    ...
    if (reset_type == V2V_SW_RESET) {
        ...
        SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->mem_length_get)
        ...
        for (i = 0; i < men_ent_count; i++){
            ...
            SOC_IF_ERROR_RETURN((DRV_SERVICES(unit)->mem_write)
                            (unit, DRV_MEM_VLANVLAN,(uint32)i, 1, 
                            (uint32 *)&vt_entry));
        }
 ...
 */
int 
drv_bcm53242_vlan_vt_add(int unit, uint32 vt_type, uint32 port,  uint32 cvid, 
                uint32 sp_vid, uint32 pri, uint32 mode)
{
    ...
    case DRV_VLAN_XLAT_INGRESS :
        ...
        if (!v2v_reset_done) {
            ...
            if (rv){
                soc_cm_debug(DK_VLAN, "%s,can't reset V2V table\n", FUNCTION_NAME());
            }
        }
    ...
    case DRV_VLAN_XLAT_EGRESS:
         ...
         *      - the egress basis VLAN translation mechanism is working by 
         *          CFP.
         */
        rv = SOC_E_UNAVAIL;
        break;
 ...
 *          VLAN2VLAN entry.
 *      2. bcm53242 V2V table entry have no valid bit
 ...
 */
int 
drv_bcm53242_vlan_vt_delete(int unit, uint32 vt_type, uint32 port, uint32 vid)
{
    int rv = SOC_E_NONE;
    vlan2vlan_entry_t vt_entry;
 ...
 *
 *  Note :
 *      1. bcm53242 V2V table is always existed without no valid bit
 *
 */
int 
drv_bcm53242_vlan_vt_delete_all(int unit, uint32 vt_type)
{
    ...
    case DRV_VLAN_XLAT_INGRESS_TRAN :
        ...
        tmp = 0;
        tmp_rv = (DRV_SERVICES(unit)->vlan_prop_get)
                    (unit, DRV_VLAN_PROP_PER_PORT_TRANSLATION, &tmp);
        if ((tmp_rv == SOC_E_NONE) && tmp) {
            for (i = 0; i < men_ent_count; i++){
                drv_bcm53242_vlan_vt_delete(unit, 
                        DRV_VLAN_XLAT_INGRESS_PER_PORT, (uint32)-1, i);
            }
        } else {
            for (i = 0; i < men_ent_count; i++){
                /* entry valid or mode will be detected in delete routine */
                drv_bcm53242_vlan_vt_delete(unit, vt_type, (uint32)-1, i);
            }
        }
 ...
 */
int 
drv_bcm53242_vlan_vt_set(int unit, uint32 prop_type, uint32 vid, 
 ...
 */
int 
drv_bcm53242_vlan_vt_get(int unit, uint32 prop_type, uint32 vid, 
                                uint32 port, uint32 *prop_val)
{
    uint32  reg_value;
 ...
 */
int 
drv_bcm53242_vlan_prop_set(int unit, uint32 prop_type, uint32 prop_val)
{
    ...
    case DRV_VLAN_PROP_V2V:
        ...
        if (igr_vt_db.vt_en != prop_val){
            ...
            SOC_IF_ERROR_RETURN(
                        ...
                        (unit, DRV_VLAN_PROP_V2V_PORT, set_bmp, prop_val));
 ...
 */
int 
drv_bcm53242_vlan_prop_get(int unit, uint32 prop_type, uint32 *prop_val)
{
    ...
    case DRV_VLAN_PROP_V2V:
        ...
        if (v2v_reset_done) { 
            *prop_val = (igr_vt_db.vt_en) ? TRUE : FALSE;
            ...
            soc_cm_debug(DK_VLAN,"%s, V2V table not reset yet!\n", FUNCTION_NAME());
            *prop_val = FALSE;
        }
        break;
 ...
 */
int 
drv_bcm53242_vlan_prop_port_enable_set(int unit, uint32 prop_type, 
 ...
 */
int 
drv_bcm53242_vlan_prop_port_enable_get(int unit, uint32 prop_type, 
