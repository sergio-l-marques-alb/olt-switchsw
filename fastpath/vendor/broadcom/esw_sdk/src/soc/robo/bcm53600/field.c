/*
 * $Id: field.c 1.2.2.6 Broadcom SDK $
...
int
_vo_cfp_qualify_conflict_check(int unit, drv_field_qualify_t qual, 
            drv_cfp_entry_t *drv_entry, uint32* p_data, 
            uint32* p_mask);
int
_vo_ingress_qualify_support(int unit, drv_field_qualify_t qual);
...
int
_vo_cfp_field_control(int unit, drv_cfp_entry_t *drv_entry, int mem, int field,
...
int 
_vo_cfp_qual_value_set(int unit, drv_field_qualify_t qual, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    rv =_vo_ingress_qualify_support(unit, qual);
    SOC_IF_ERROR_RETURN(rv);
    
    rv = _vo_cfp_qualify_conflict_check(unit, qual, drv_entry, p_data, p_mask);
    SOC_IF_ERROR_RETURN(rv);
    ...
    if ((-1 == field) ||(0 == mem)){
        DRV_FP_VERB(("indirect qual field setting.\n"));
        return SOC_E_NONE;
    }    
    ...
    if (qual == drvFieldQualifyEtherType){
        ...
        rv = _vo_cfp_field_control(unit, drv_entry, mem,  INDEX(IP_PROTOf), &temp_data, &temp_mask, 
            _cfp_field_data_mask_set);
        *p_data = (*p_data & 0xff00) >> 8;
        *p_mask = (*p_mask & 0xff00) >> 8;
    }
    rv = _vo_cfp_field_control(unit, drv_entry, mem, field, p_data, p_mask, 
            _cfp_field_data_mask_set);
...
int 
_vo_cfp_qual_value_get(int unit, drv_field_qualify_t qual, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int rv = SOC_E_UNAVAIL;
    int i;
    ...
    if (qual == drvFieldQualifyL4Ports) {
        rv = _vo_cfp_field_control(unit, drv_entry, INDEX(CFP_TCAM_SCm), 
...
int
_vo_cfp_udf_value_set(int unit, uint32 udf_idx, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int rv = SOC_E_UNAVAIL;
    int i;
            ...
            if (drv_entry->slice_id == CFP_53600_SLICE_ID_CHAIN_SLICE) {
                if (drv_entry->flags & _DRV_CFP_SLICE_CHAIN){
                    if ((_vo_udf_mapping_tbl[i].slice_id & 0x3) 
                        ...
                        DRV_FP_WARN(("udf idex %d is not supported for current slice id %d\n",
    ...
    if (-1 == field) {
        DRV_FP_WARN(("no matched udf idex\n"));
        return SOC_E_INTERNAL;
    }
    ...
    rv = _vo_cfp_field_control(unit, drv_entry, mem, field, 
        p_data, p_mask, _cfp_field_data_mask_set);
    SOC_IF_ERROR_RETURN(rv);
    
    rv = _vo_cfp_field_control(unit, drv_entry, mem, valid_field, 
...
int
_vo_cfp_udf_value_get(int unit, uint32 udf_idx, drv_cfp_entry_t *drv_entry,
                      uint32 *p_data, uint32 *p_mask)
{
    int rv = SOC_E_UNAVAIL;
    int i;
...

{
    ...
    DRV_FP_VERB(("(1)flags l2 %x l3 %x l4 %x\n",l2_flags, l3_flags, l4_flags));
    switch (qual) {
        /* Non-IP */            
    ...
    DRV_FP_VERB(("%s: conflict %d qual %d, l2_flags %x l3_flags %x l4_flags %x\n", 
...
int
_vo_fp_cfp_qualify_support(int unit, drv_field_qset_t qset)
{
    int rv = SOC_E_NONE;
    int i;
    ...
    for (i = 0; i < drvFieldQualifyCount; i++) {
        if (DRV_FIELD_QSET_TEST(qset, i)) {
            rv = _vo_ingress_qualify_support(unit, i);
            if (SOC_FAILURE(rv)){
                return rv;
            }
        }            
...
int
_vo_qset_to_cfp(int unit, drv_field_qset_t qset, drv_cfp_entry_t * drv_entry, int mode)
{
    int     idx, i, slice_id;
                ...
                if (_vo_udf_mapping_tbl[i].udf_id== idx) {
                    ...
                    if (mode == DRV_FIELD_GROUP_MODE_DOUBLE) {
                        if ((_vo_udf_mapping_tbl[i].slice_id & 0x3) != CFP_53600_SLICE_ID_BASE) {
                            DRV_FP_WARN(("udf conflict for double-wide mode!\n"));
                            return SOC_E_UNAVAIL;
                        } 
                        ...
                        if ((_vo_udf_mapping_tbl[i].slice_id & 0x3) != (slice_id & 0x3)) {
                            DRV_FP_WARN(("udf conflict for auto mode!\n"));
                            return SOC_E_UNAVAIL;
                        } 
    ...
    DRV_FP_VERB(("after udf sets slice id %d bmp %x\n",
    ...
    for (idx = 0; idx < drvFieldQualifyCount; idx++) {
        ...
        retval = _vo_ingress_qualify_support(unit, idx);
        SOC_IF_ERROR_RETURN(retval);
                ...
                if(slice_id != -1){
                    if (_vo_qual_slice_mapping_tbl[i].conflict_qid[0] != -1){
                        if (1 == DRV_FIELD_QSET_TEST((qset), 
                            ...
                            DRV_FP_WARN(("qset (EtherType/LLC ) conflict with (DSCP/IpProtocol) \n"));
                            return SOC_E_UNAVAIL; 
                        }
                        if (1 == DRV_FIELD_QSET_TEST((qset), 
                        ...
                        if (_shr_popcount(slice_bmp) == 1) {   
                            ...
                            if (slice_id == CFP_53600_SLICE_ID_CHAIN_SLICE) {
                                ...
                                if (!(_vo_qual_slice_mapping_tbl[i].slice_bmp & (1<< slice_id))) {
                                    DRV_FP_WARN(("qset conflict for AUTO mode!\n"));
                                    return SOC_E_UNAVAIL; 
                                }
                            }
                        } else {
                    ...
                    if (mode == DRV_FIELD_GROUP_MODE_SINGLE) {                        
                        if (!(_vo_qual_slice_mapping_tbl[i].slice_bmp & (1<< slice_id))) {
                            DRV_FP_WARN(("qset conflict for SINGLE mode!\n"));
                            return SOC_E_UNAVAIL; 
                        }
                    }
                } else {
    ...
    DRV_FP_VERB(("after qual sets slice id %d bmp %x\n",
            drv_entry->slice_id, drv_entry->slice_bmp));
...
int
_vo_fp_entry_tcam_chain_mode_get(int unit, int stage_id, void *drv_entry, int sliceId, void *mode)    
{
...
int
_vo_fp_entry_tcam_slice_id_set(int unit, int stage_id, void *entry, int sliceId, void *slice_map)
{
    uint32 temp;
    uint32 id;
...
int
_vo_cfp_framing_configure(int unit, drv_cfp_entry_t *drv_entry, int flag)    
{
    int l2_flag;
        ...
        if (l2_flag == VO_CFP_FRAME_L2_DIXv2) {
            if (l2_frame_mask) {
                if ((l2_frame & l2_frame_mask) != 0){
                    DRV_FP_WARN(("%s l2 framing conflict (VO_CFP_FRAME_L2_DIXv2)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l2_frame_mask) {
                if ((l2_frame & l2_frame_mask) != 0x1){
                    DRV_FP_WARN(("%s l2 framing conflict (VO_CFP_FRAME_L2_SNAP_PUBLIC)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l2_frame_mask) {
                if ((l2_frame & l2_frame_mask) != 0x2){
                    DRV_FP_WARN(("%s l2 framing conflict (VO_CFP_FRAME_L2_LLC)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l2_frame_mask) {
                if ((l2_frame & l2_frame_mask) != 0x3){
                    DRV_FP_WARN(("%s l2 framing conflict (VO_CFP_FRAME_L2_SNAP_PRIVATE)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            DRV_FP_WARN(("%s l2 framing conflict (non-handle case )!\n",
                FUNCTION_NAME()));  
            rv = SOC_E_UNAVAIL;
            goto framing_err; 
        }
        DRV_FP_VERB(("%s l2 frame %x %x\n",FUNCTION_NAME(), 
        ...
        if (l3_flag == _DRV_CFP_FRAME_IP4) {
            if (l3_frame_mask) {
                if ((l3_frame & l3_frame_mask) != 0x0 ){
                    DRV_FP_WARN(("%s l3 framing conflict (_DRV_CFP_FRAME_IP4)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l3_frame_mask) {
                if ((l3_frame & l3_frame_mask) != 0x1 ){
                    DRV_FP_WARN(("%s l3 framing conflict (_DRV_CFP_FRAME_IP6)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l3_frame_mask) {
                if ((l3_frame & l3_frame_mask) != 0x3 ){
                    DRV_FP_WARN(("%s l3 framing conflict (_DRV_CFP_FRAME_NONIP)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l3_frame_mask) {
                if ((l3_frame == 0x3 )|| (l3_frame_mask != 0x2)) {
                    DRV_FP_WARN(("%s l3 framing conflict (_DRV_CFP_FRAME_IPANY)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l3_frame_mask) {
                if ((l3_frame & l3_frame_mask) == 0x3 ){
                    DRV_FP_WARN(("%s l3 framing conflict (_DRV_CFP_FRAME_IP)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            DRV_FP_WARN(("%s l3 framing conflict (non-handle case )!\n",
                FUNCTION_NAME()));  
            rv = SOC_E_UNAVAIL;
            goto framing_err; 
        }
        DRV_FP_VERB(("%s l3 frame %x %x\n",FUNCTION_NAME(), 
    ...
    if ((l4_flag != 0) && (l4_flag != _DRV_CFP_FRAME_L4_ALL)){
        ...
        if (l4_flag == VO_CFP_FRAME_L4_TCP) {
            if (l4_frame_mask) {
                if ((l4_frame & l4_frame_mask) != 0){
                    DRV_FP_WARN(("%s l4 framing conflict (VO_CFP_FRAME_L4_TCP)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l4_frame_mask) {
                if ((l4_frame & l4_frame_mask) != 1){
                    DRV_FP_WARN(("%s l4 framing conflict (VO_CFP_FRAME_L4_UDP)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l4_frame_mask) {
                if ((l4_frame & l4_frame_mask) != 2){
                    DRV_FP_WARN(("%s l4 framing conflict (VO_CFP_FRAME_L4_ICMPIGMP)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l4_frame_mask) {
                if ((l4_frame & l4_frame_mask) != 3){
                    DRV_FP_WARN(("%s l4 framing conflict (VO_CFP_FRAME_L4_OTHER)!\n",
                        FUNCTION_NAME()));
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            if (l4_frame_mask) {
                if (((l4_frame & l4_frame_mask) != 3) || 
                    ...
                    DRV_FP_WARN(("%s l4 framing conflict (VO_CFP_FRAME_L4_TCPUDP)!\n",
                        FUNCTION_NAME()));                
                    rv = SOC_E_UNAVAIL;
                    goto framing_err; 
                }
            ...
            DRV_FP_WARN(("%s l4 framing conflict (non-handle case )!\n",
                FUNCTION_NAME()));  
            rv = SOC_E_UNAVAIL;
            goto framing_err; 
        }
        DRV_FP_VERB(("%s l4 frame %x %x\n",FUNCTION_NAME(), 
        ...
        int tcam_chain_idx)
{
    ...
    SOC_IF_ERROR_RETURN(
     ...
     if (drv_entry->slice_id != CFP_53600_SLICE_ID_WITH_CHAIN) {
        SOC_IF_ERROR_RETURN(
            _vo_cfp_framing_configure(unit, drv_entry, config_main_entry));
     }
    ...
    /* entry with chain {slice0,slice3}*/        
        ...
        SOC_IF_ERROR_RETURN(
        _vo_cfp_framing_configure(unit, drv_entry, config_chain_main));
        drv_entry->flags |= _DRV_CFP_SLICE_CONFIG_SLICE_MAIN;
        ...
        drv_entry->flags |= _DRV_CFP_SLICE_CONFIG_SLICE_CHAIN;
        temp = 0xfff;
        rv = _vo_cfp_qual_value_set(unit, drvFieldQualifyClassId, 
...
int
drv_bcm53600_cfp_slice_id_select(int unit, drv_cfp_entry_t *entry, uint32 *slice_id, uint32 flags)
{
    int qual_num, i;
    uint32 slice_bmp;
    ...
    if (entry->slice_id != -1){
        ...
        DRV_FP_VERB(("%s  %d slice_id %d entry slice id %d bmp %x \n",
...
/*
 * Function: drv_53600_cfp_udf_get
 *
 * Purpose:
 ...
 */
int 
drv_bcm53600_cfp_udf_get(int unit, uint32 port, uint32 udf_index, 
...
int 
drv_bcm53600_cfp_udf_set(int unit, uint32 port, uint32 udf_index, 
    uint32 offset, uint32 base)
{
...
int 
drv_bcm53600_cfp_range_set(int unit, uint32 type, uint32 id, 
...
int 
drv_bcm53600_cfp_range_get(int unit, uint32 type, uint32 id, 
        uint32 *param1, uint32 *param2) {
...
int
_vo_cfp_data_mask_read(int unit, uint32 ram_type, 
                         uint32 index, drv_cfp_entry_t *cfp_entry)
{
    ...
    if (1 == read_data) {
        ...
        rv = _vo_cfp_mem_hw2sw(unit, mem_entry, tcam_data);
        if (SOC_FAILURE(rv)) {
            sal_free(mem_entry);
            return rv;
        }
    ...
    if (1 == read_mask) {
        ...
        rv = _vo_cfp_mem_hw2sw(unit, mem_entry, tcam_mask);
        if (SOC_FAILURE(rv)) {
            sal_free(mem_entry);
            return rv;
        }        
...
int
_vo_cfp_data_mask_write(int unit, uint32 ram_type, 
                              uint32 index, drv_cfp_entry_t *cfp_entry)
{
    int rv = SOC_E_NONE;
	...
	if (ram_type == DRV_MEM_CFP_DATA_MASK) {	
        sal_memset(mem_entry, 0, size);
        rv = _vo_cfp_mem_sw2hw(unit, tcam_data, mem_entry);
        if (SOC_FAILURE(rv)) {
            sal_free(mem_entry);
            return rv;
        }
        ...
        rv = _vo_cfp_mem_sw2hw(unit, tcam_mask, &mem_entry[mask_offset]);
        if (SOC_FAILURE(rv)) {
            sal_free(mem_entry);
            return rv;
        }
	    ...
	    if (1 == write_mask) {
	        sal_memset(mem_entry, 0, size);
	        rv = _vo_cfp_mem_sw2hw(unit, tcam_mask, mem_entry);
	        if (SOC_FAILURE(rv)) {
	            sal_free(mem_entry);
	            return rv;
	        }
...
int
_vo_cfp_field_set(int unit, uint32 mem_type, uint32 field_type, 
            drv_cfp_entry_t* entry, uint32* fld_val)
{
    ...
    if (fld == -1) {
        DRV_FP_WARN(("_vo_cfp_field_set DRV_CFP_FIELD_XXX %d \
    ...
    if (mem_type == DRV_CFP_RAM_TCAM) {
        rv = _vo_cfp_field_control(unit, entry, INDEX(CFP_TCAM_SCm), 
            fld, fld_val, NULL, 
            _cfp_field_data_set);
    }
...
int
_vo_cfp_mem_hw2sw(int unit, uint32 *hw_entry, uint32 *sw_entry){
...
int
_vo_cfp_mem_sw2hw(int unit, uint32 *sw_entry, uint32 *hw_entry){
    int len, wp, bp,i;
    uint32 mask, mask_lo, mask_hi;
