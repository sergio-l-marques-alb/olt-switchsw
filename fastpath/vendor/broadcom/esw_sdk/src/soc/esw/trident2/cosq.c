/*
 * $Id: cosq.c 1.62.4.1 Broadcom SDK $
...
/*
 * Low-level debugging may be enabled from the CLI
...
_soc_trident2_port_sched_type_get(int unit, soc_port_t port)
{
    ...
    if (si->port_speed_max[port] >= 100000 ||
        (si->port_speed_max[port] >= 40000 && si->bandwidth < 960000)) {
                         ...
                         int hw_index, int weight)
{
    ...
    _SOC_NORM(("sched_weight_set L%d csch_index=%d wt=%d\n", 
                              ...
                              int first_sp_mc_child, uint32 ucmap)
{
    ...
    _SOC_NORM(("Port:%d L%s%d config : index=%d FC=%d FMC=%d UMAP=0x%x\n", 
                            ...
                            soc_td2_sched_mode_e mode, int weight)
{
    ...
    _SOC_NORM(("Port:%d L%s%d config : index=%d MODE=%d WT=%d\n", 
               port, (level == 0) ? "r" : "", level - 1, 
               index, mode, weight));
...
soc_td2_setup_hsp_port(int unit, int port)
{
    ...
    soc_field_t flds[2] = {IS_MC_T2OQ_PORT0f, IS_MC_T2OQ_PORT1f};

    SOC_IF_ERROR_RETURN(
        soc_td2_port_common_attributes_get(unit, port, &pipe, &mmu_port, NULL));

    rval = 0;
    SOC_IF_ERROR_RETURN(WRITE_HSP_SCHED_PORT_CONFIGr(unit, port, 2));
    ...
    SOC_IF_ERROR_RETURN(READ_TOQ_MC_CFG1r(unit, &rval));
    fval = soc_reg_field_get(unit, TOQ_MC_CFG1r, rval, flds[pipe]);
    fval |= 1 << (mmu_port % 64);
    soc_reg_field_set(unit, TOQ_MC_CFG1r, &rval, flds[pipe], fval);
    SOC_IF_ERROR_RETURN(WRITE_TOQ_MC_CFG1r(unit, rval));
...
{
    int port, rv = SOC_E_NONE, reserve_queues;
    soc_trident2_sched_type_t sched_type;
    uint32 entry[SOC_MAX_MEM_WORDS];

    reserve_queues = soc_feature(unit, soc_feature_td2_cmic_reserved_queues);
    ...
    soc_td2_init_invalid_pointers(unit);
                            ...
                            int *hw_index)
{
    ...
    _SOC_NORM(("Alloced : port=%d lvl=%d ofst=%d Index=%d\n",
...
soc_td2_get_def_qbase(int unit, soc_port_t inport, int qtype, 
                       int *pbase, int *pnumq)
{
    int base = 0, numq = 0, in_pipe, port_in_pipe;
    ...
    SOC_IF_ERROR_RETURN(
        soc_td2_port_common_attributes_get(unit, inport, &in_pipe, NULL, NULL));
    ...
    if (qtype == _SOC_TD2_INDEX_STYLE_UCAST_QUEUE) {
        ...
        SOC_IF_ERROR_RETURN(
                soc_td2_port_common_attributes_get(unit, inport, 
                                                   NULL, &mmu_port, NULL));
        if (mmu_port == MMU_CMIC_PORT) {
            numq = 48;
        } else if (mmu_port == MMU_LB_PORT) {
...
_soc_td2_dump_sched_at(int unit, int port, int level, int offset, int hw_index)
{
    ...
    if ((level > SOC_TD2_NODE_LVL_L0) && 
         (hw_index == INVALID_PARENT(unit, level))) {
        return SOC_E_NONE;
    }
    ...
    if (level == SOC_TD2_NODE_LVL_L1) {
        soc_cm_print("  %s.%d : INDEX=%d NUM_SP=%d FC=%d FMC=%d UCMAP=0x%08x MODE=%s WT=%d\n",
                            lvl_name[level], offset, hw_index, num_spri, 
                            first_child, first_mc_child, ucmap, 
                            sched_modes[sched_mode], wt);
    } else {
        soc_cm_print("  %s.%d : INDEX=%d NUM_SPRI=%d FC=%d MODE=%s Wt=%d\n",
