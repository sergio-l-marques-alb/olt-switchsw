/* $Id: lpm.c 1.36.20.4 Broadcom SDK $
...
uint32 kt_dip128_start_offset;
uint32 kt_sip128_start_offset;
uint32 kt_depth128;
uint32 kt_dip64_start_offset;
uint32 kt_sip64_start_offset;
uint32 kt_depth64;
...
#define SOC_MEM_OPT_F32_GET(m_unit, m_mem, m_entry_data, m_field) \
        soc_meminfo_fieldinfo_field32_get((&SOC_MEM_INFO(m_unit, m_mem)), (m_entry_data), (soc_lpm_field_cache_state[(m_unit)]->m_mem##m_field), m_field)
...
soc_fb_lpm_init(int u)
{
    ...
    int i;
#if defined(BCM_KATANA_SUPPORT)
    uint32 l3_defip_pair128_tbl_size;
    ...
    if (SOC_IS_KATANA(u)) {
        ...
        l3_defip_pair128_tbl_size = soc_mem_index_count(u, L3_DEFIP_PAIR_128m);
         ...
         * 128b pairing mode */
        kt_dip128_start_offset = 0;
        kt_sip128_start_offset = 0;
        kt_depth128            = 0;
        kt_dip64_start_offset  = 0;
        kt_sip64_start_offset  = 0;
        kt_depth64             = 0;
        if (SOC_URPF_STATUS_GET(u)) {
            /* urpf check enabled */
            if (ipv6_lpm_128b_enable == 1) {
                ...
                kt_depth128 = L3_DEFIP_TCAM_SIZE;
                /* TCAM 4 is used for DIP IPv6 LPM 64b
                 * TCAM 5 is used for SIP IPv6 LPM 64b */
                kt_dip64_start_offset = L3_DEFIP_TCAM_SIZE * 4;
                kt_depth64            = L3_DEFIP_TCAM_SIZE;
                ...
                kt_depth64 = l3_defip_tbl_size / 2;
            }            
            kt_sip128_start_offset = kt_dip128_start_offset + kt_depth128;
            kt_sip64_start_offset  = kt_dip64_start_offset + kt_depth64;
        } else {
            /* urpf check disabled */
            if (ipv6_lpm_128b_enable == 1) {
                /* C3:
                ...
                kt_depth128 = L3_DEFIP_TCAM_SIZE * 2;
                /* TCAMs 4-5 is used for DIP IPv6 LPM 64b */
                kt_dip64_start_offset = L3_DEFIP_TCAM_SIZE * 4;
                kt_depth64            = L3_DEFIP_TCAM_SIZE * 2;
            } else {
                /* C1:
                ...
                kt_depth64            = l3_defip_tbl_size;
            };
        }
        SOC_LPM_STATE_FENT(u, (MAX_PFX_INDEX)) = kt_depth64;
        SOC_LPM_STATE_END(u, (MAX_PFX_INDEX)) = kt_dip64_start_offset - 1;
    }
    #endif
...
soc_fb_lpm_reinit_done(int unit)
{
    ...
    if (SOC_IS_KATANA(unit)) {
        SOC_LPM_STATE_FENT(unit, MAX_PFX_INDEX) = kt_depth64;
    }
#endif
