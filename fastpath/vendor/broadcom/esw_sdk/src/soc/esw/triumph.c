/*
 * $Id: triumph.c 1.248.6.5 Broadcom SDK $
...
STATIC _soc_parity_info_t _soc_tr_mmu_parity_info[] = {
    ...
    { TOQ0_IPMC_TBL_PAR_ERR_ENf, TOQ0_IPMC_TBL_PAR_ERRf, NULL,
      INVALIDm, _SOC_PARITY_INFO_TYPE_SINGLE_TABLE1,
      INVALIDr,
...
{
    int block, table, index, size;
    uint32 addr, cmic_status, block_status, status, block_enable;
    uint32 enable_addr, entry_idx, bitmap0, bitmap1;
    _soc_parity_info_t *info;
    soc_reg_t block_reg, enable_reg, reg;
    char *msg;
    ...
    for (block = 0; _soc_tr_parity_block_info[block].cpi_bit; block++) {
        ...
        for (table = 0; info[table].error_field != INVALIDf; table++) {
            ...
            /* Disable error reporting on this memory to avoid continuous interrupts
               untill the code for fixing the error at source is put in. */
               soc_reg_field_set(unit, enable_reg, &block_enable,
                                 info[table].enable_field, 0);
        } /* loop through each table in the block */
...
_soc_triumph_esm_init_select_ad_mode(int unit, int *ad_mode)
{
    ...
    if (cfg_ad_mode < 0 || cfg_ad_mode > 12) {
        soc_cm_debug(DK_ERR, "ESM init: unit %d bad %s %d\n",
...
_soc_triumph_esm_init_set_sram_tuning_result(int unit, int intf_num)
{
    ...
    if (tune_data) {
        ...
        if (freq != tcam_info->sram_freq) {
            tune_data = 0;
            soc_cm_print("SRAM%d: Ignore %s, config was tuned at %d MHz, "
...
_soc_triumph_esm_init_set_tcam_tuning_result(int unit)
{
    ...
    if (tune_data[1]) {
        ...
        if (freq != tcam_info->tcam_freq) {
            ...
            soc_cm_print("TCAM: Ignore %s0 and %s1, config was tuned at %d "
        ...
        if (use_midl != cur_use_midl) {
            tune_data[0] = tune_data[1] = 0;
            soc_cm_print("TCAM: Ignore %s0 and %s1, config was tuned using %s "
...
soc_triumph_esm_init_pvt_comp(int unit, int intf_num)
{
    ...
    if (!soc_reg_field_get(unit, status_reg, rval, ODT_PVT_DONEf) &&
        odt != 0 && odt != 0xf) {
        soc_cm_print("%s ODT PVT compensation fail\n",
    ...
    if (!soc_reg_field_get(unit, status_reg, rval, NDRIVER_PVT_DONEf) &&
        ndrive != 0 && ndrive != 0xf) {
        soc_cm_print("%s NDRIVE PVT compensation fail\n",
                     ddr_reg[intf_num].intf_name);
    }
    ...
    if (!soc_reg_field_get(unit, status_reg, rval, PDRIVER_PVT_DONEf) &&
        pdrive != 0 && pdrive != 0xf) {
        soc_cm_print("%s PDRIVE PVT compensation fail\n",
                     ddr_reg[intf_num].intf_name);
    }
...
_soc_triumph_esm_init_set_esm_mode_per_port(int unit)
{
    ...
    if (fwd_ip6_key < 0 || fwd_ip6_key > 1 ||
        ...
        soc_cm_debug(DK_ERR, "ESM init: unit %d incorrect key selection\n",
                     unit);
        return SOC_E_PARAM;
    }
...
soc_triumph_esm_init(int unit)
{
    ...
    if (tcam_info->subtype_override &&
        ...
        soc_cm_debug(DK_ERR, "ESM init: unit %d invalid device type %d\n",
    ...
    for (intf_num = 0; intf_num < 2; intf_num++) {
        ...
        if (rv == SOC_E_NOT_FOUND) {
            soc_cm_print("SRAM%d TX DLL not locked\n", intf_num);
    ...
    for (intf_num = 0; intf_num < 2; intf_num++) {
        ...
        if (!rx_locked[intf_num]) {
            soc_cm_print("SRAM%d RX DLL not locked\n", intf_num);
        }
    }
    } else { /* Triumph */
    /* Bring SRAM and TCAM out of reset */
...
soc_triumph_mmu_init(int unit)
{
    ...
    out_reserved_cells =
        NUM_ALL_PORT(unit) * TR_MMU_OUT_PORT_MIN_CELLS * TR_MMU_NUM_COS;
    out_reserved_pkts =
        NUM_ALL_PORT(unit) * TR_MMU_OUT_PORT_MIN_PKTS * TR_MMU_NUM_COS;
                        ...
                        int num_post_nop, uint32 *dbus, int *ibus)
{
    ...
    for (i = 0; i < num_inst;) {
        ...
        if (i + 1 < num_inst) {
            if (ibus[i] <= 0xff) {
                ...
                soc_mem_field32_set(unit, ET_UINST_MEMm, &uinst_entry, IBUS0f,
                                    ibus[i]);
                if (SOC_DEBUG_CHECK(SOC_DBG_TCAM | SOC_DBG_VERBOSE)) {
                    soc_cm_print("IBUS0: %08x DBUS0: %08x %08x %08x\n",
                                 ibus[i],
        ...
        if (ibus[i] <= 0xff) {
            ...
            soc_mem_field32_set(unit, ET_UINST_MEMm, &uinst_entry, IBUS1f,
                                ibus[i]);
            if (SOC_DEBUG_CHECK(SOC_DBG_TCAM | SOC_DBG_VERBOSE)) {
                soc_cm_print("IBUS1: %08x DBUS1: %08x %08x %08x\n",
                             ibus[i],
                             dbus[i * TCAM_TR_WORDS_PER_ENTRY],
                             dbus[i * TCAM_TR_WORDS_PER_ENTRY + 1],
                             dbus[i * TCAM_TR_WORDS_PER_ENTRY + 2]);
        ...
        if (SOC_DEBUG_CHECK(SOC_DBG_TCAM | SOC_DBG_VERBOSE)) {
            soc_cm_print("UINST %d: %08x %08x %08x %08x %08x %08x\n",
                         (i - 1)/ 2,
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        if (!retry) {
            if (SOC_DEBUG_CHECK(SOC_DBG_TCAM)) {
                soc_cm_print("soc_triumph_tcam_access: Instruction timeout\n");
            }
            rv = SOC_E_TIMEOUT;
        }
    }
    ...
    case TCAM_TR_OP_READ:
        if (!soc_reg_field_get(unit, ETU_ET_INST_STATUSr, status, RDACKf)) {
            if (SOC_DEBUG_CHECK(SOC_DBG_TCAM)) {
                soc_cm_print("soc_triumph_tcam_access: No rdack\n");
            }
            return SOC_E_NOT_FOUND;
        }
        ...
        dbus[2] = rval2 & 0xff;
        if (SOC_DEBUG_CHECK(SOC_DBG_TCAM)) {
            soc_cm_print("READ: RSLT_DAT: %08x %08x %08x\n",
                         rval0, rval1, rval2);
        }
        break;
    ...
    case TCAM_TR_OP_PARALLEL_SEARCH:
        ...
        dbus[1] = rval1;
        if (SOC_DEBUG_CHECK(SOC_DBG_TCAM)) {
            soc_cm_print("SEARCH: RESULT_RESULT: %08x %08x\n",
                         rval0, rval1);
        }
        break;
...
_soc_tr_l2e_ppa_update(int unit, int index, ext_l2_entry_entry_t *entry)
{
    ...
    soc_cm_debug(DK_VERBOSE, "tr_l2e_ppa: index=%d oldvlan=%d min:max %d:%d\n",
                 index, oldvlan,
                 ppa_vlan->vlan_min[oldvlan],
                 ppa_vlan->vlan_max[oldvlan]);
    soc_cm_debug(DK_VERBOSE, "tr_l2e_ppa: ent=0x%x mac=%x:%x:%x:%x:%x:%x vlan=%d min:max %d:%d\n",
                 ppa_info[index].data,
...
_soc_triumph_tx(int unit, int port, int pkt_len)
{
    ...
    if (dcb == NULL) {
        soc_dma_rom_init(unit, 256, TRUE, FALSE);
        dcb = soc_dma_rom_dcb_alloc(unit, 192);
        if (dcb == NULL) {
            return SOC_E_MEMORY;
        }
    }
    ...
    SOC_DCB_STAT_SET(unit, dcb, 1);
    SOC_DCB_HG_SET(unit, dcb, 1);
    SOC_DCB_REQCOUNT_SET(unit, dcb, pkt_len);
    dcb->mh0 = soc_ntohl(((uint32 *)&pbsmh)[0]);
    dcb->mh1 = soc_ntohl(((uint32 *)&pbsmh)[1]);
    dcb->mh2 = soc_ntohl(((uint32 *)&pbsmh)[2]);
    dcb->mh3 = 0;
...
soc_triumph_xgport_mode_change(int unit, soc_port_t port, soc_mac_mode_t mode)
{
    ...
    if (old_bits == 2) { /* it was xport-mode */
        _soc_triumph_tx(unit, port, 192);
    } else if (old_bits == 1) { /* it was gport-mode */
        _soc_triumph_tx(unit, port, 64);
    }
