/*
 * $Id: firebolt.c 1.101 Broadcom SDK $
...
*/
    ...
    do {
        ...
        if (soc_timeout_check(&to)) {
            soc_cm_debug(DK_WARN, "unit %d : DEFIP_CAM_BIST timeout\n", unit);
            rv = SOC_E_INTERNAL;
            goto done;
        }
    } while (TRUE);
...
soc_firebolt_misc_init(int unit)
{
    ...
    if(SOC_IS_HAWKEYE(unit)) {
        ...
        PBMP_ALL_ITER(unit, port) {
            SOC_IF_ERROR_RETURN(READ_EGR_ENABLEr(unit, port, &reg64));
    ...
    /* Fixup the reset value and make it compatible with other devices */
    ...
    if (soc_feature(unit, soc_feature_lpm_tcam)) {
        ...
        if (rv < 0) {
            soc_cm_print("soc_mem_clear: LPM flush failed.\n");
            return(rv);
        }
    }
#ifdef BCM_FIREBOLT2_SUPPORT
    ...
    if(!SOC_IS_HAWKEYE(unit)) {
        ...
        if (soc_feature(unit, soc_feature_post)) {
            ...
            if ((rv = soc_post_init(unit)) < 0) {
                soc_cm_print("soc_post_init failed on unit %d\n", unit);
            }
            soc_cm_debug(DK_VERBOSE, "soc_post_init: took %d usec\n",
...
int _soc_fb_mmu_parity_error(int unit)
{
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr, istat, CFAPFAILERRORf)) {
        soc_cm_debug(DK_ERR, "unit = %d, CFAPFAILERROR\n", unit);
    }
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr, istat, SOFTRESETERRORf)) {
        soc->stat.err_sr++;
        soc_cm_debug(DK_ERR, "unit = %d, MMU Soft Reset Error\n", unit);
    }

    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr, istat, CELLNOTIPERRORf)) {
        soc_cm_debug(DK_ERR, "unit = %d, MMU Cell not in progress\n", unit);
    }
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr, istat, CELLCRCERRORf)) {
        ...
        soc_cm_debug(DK_ERR, "unit = %d, CBPCELLCRCERRPTR 0x%08x\n", unit, val);
    }
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr,
        ...
        soc_cm_debug(DK_ERR, "unit = %d, CBPPKTHDRPARITYERRPTR 0x%08x\n",
                     unit, val);
    }
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr,
        ...
        soc_cm_debug(DK_ERR, "unit = %d, CBPCELLHDRPARITYERRPTR 0x%08x\n",
                     unit, val);
    }
    ...
    if (soc_reg_field_get(unit, MEMFAILINTSTATUSr, istat, XQPARITYERRORf)) {
        ...
        soc_cm_debug(DK_ERR, "unit = %d, XQPARITYERRORPBM 0x%08x\n",
                     unit, val);
    }
    ...
    if (soc_feature(unit, soc_feature_l3defip_parity)) {
        ...
        if (soc_reg_field_get(unit, L3_DEFIP_PARITY_STATUSr,
            ...
            mem_idx = soc_reg_field_get(unit, L3_DEFIP_PARITY_STATUSr,
                                        istat, MEMORY_IDXf);
            if (SOC_IS_FIREBOLT2(unit)) {
                uint32 mem_idx_min, mem_idx_max;
...
soc_xgs3_higig_bitmap_to_bitmap(int unit, pbmp_t hg_pbmp, pbmp_t *pbmp)
{
    ...
    if (SOC_IS_RAVEN(unit) || SOC_IS_TR_VL(unit)) {
        soc_port_t hgp;
        int *hg_list, len;
