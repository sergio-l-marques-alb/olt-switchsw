/*
 * $Id: cosq.c 1.89.2.1 Broadcom SDK $
                               ...
                               int parent_hw_idx)
{
    ...
    _SOC_NORM(("Port:%d L%d : %d parent:%d\n", 
...
soc_tr3_sched_weight_set(int unit, int level, int index, int weight)
{
    ...
    _SOC_NORM(("sched_weight_set L%d index=%d wt=%d\n", level, index, weight));
    return SOC_E_NONE;
}
...
soc_tr3_sched_weight_get(int unit, int level, int index, int *weight)
{
    ...
    *weight = soc_mem_field32_get(unit, mem_weight, &entry, C_WEIGHTf);
    ...
    _SOC_NORM(("sched_weight_get L%d index=%d wt=%d\n", level, index, *weight));
    return SOC_E_NONE;
}
                                       ...
                                       uint32 spmap, int child_index)
{
    ...
    _SOC_NORM(("Port:%d L%s%d config : index=%d FC=%d FMC=%d UMAP=0x%x NUMSP=%d\n", 
                      ...
                      int setup, _soc_tr3_lls_traverse_cb cb, void *cookie)
{
    ...
    int  l2_lvl_offsets[2], rc, numq, lvl_offsets[4];
...
int soc_tr3_lls_init(int unit)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        if (_SOC_TR3_IS_HSP_PORT(unit, port)) {
            ...
            dyn_ports += 1;
        } else if (soc_property_port_get(unit, port,
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        dyn_mode = soc_property_port_get(unit, port, spn_PORT_SCHED_DYNAMIC, 0);
        if (_SOC_TR3_IS_HSP_PORT(unit, port)) {
            rv = soc_tr3_setup_hsp_port(unit, port);
        } else if (IS_CPU_PORT(unit, port)) {
            if (soc_feature(unit, soc_feature_tr3_cmic_reserved_queues)) {
    ...
    if (qtype == _SOC_TR3_INDEX_STYLE_UCAST_QUEUE) {
        PBMP_ALL_ITER(unit, port) {
            ...
            if (_SOC_TR3_IS_HSP_PORT(unit, port)) {
                ...
                numq = soc_property_port_get(unit, port, spn_LLS_NUM_L2UC, dflt_cnt);
        ...
        if (_SOC_TR3_IS_HSP_PORT(unit, inport)) {
            ...
            numq = soc_feature(unit, soc_feature_tr3_cmic_reserved_queues) ? 45 : 48;
        } else if (mmu_port == MMU_SM_PORT) {
            numq = 1;
...
_soc_tr3_dump_sched_at(int unit, int port, int level, int offset, int hw_index)
{
    ...
    if (level == SOC_TR3_NODE_LVL_L1) {
        soc_cm_print("  %s.%d : INDEX=%d NUM_SP=%d FC=%d FMC=%d UCMAP=0x%08x SPMAP=0x%08x MODE=%s WT=%d\n",
                            lvl_name[level], offset, hw_index, num_spri, 
                            first_child, first_mc_child, ucmap, spmap,
                            sched_modes[sched_mode], wt);
    } else {
        soc_cm_print("  %s.%d : INDEX=%d NUM_SPRI=%d FC=%d SPMAP=0x%08x MODE=%s Wt=%d\n",
    ...
    for (ii = 0, ci = 0; ii <= index_max; ii++) {
        ...
        if (rv) {
            soc_cm_print("Failed to read memory at index: %d\n", ii);
            break;
        }
        ...
        if (cindex == hw_index) {
            if (child_level == SOC_TR3_NODE_LVL_L2) {
                ...
                soc_cm_print("     L2.%s INDEX=%d Mode=%s WEIGHT=%d\n", 
                                    ((ii < 1024) ? "uc" : "mc"),
