/*
 * $Id: esw_ddr40.c 1.9.2.33 Broadcom SDK $
...
_soc_ddr40_katana_phy_init_mem_set_wrapper(uint32 unit, KATANAfreq_grade_mem_set_set_t *fgmssPtr, int freq_loc, int grade_loc)
{
    ...
    fgmssPtr->refrate = KATANAfgmss[freq_loc][grade_loc].refrate;
    ...
    if (!stat) {
        switch(phyType) {
            ...
            case DDR_PHYTYPE_AND:
                ...
                SOC_DEBUG(SOC_DBG_DDR,("D04. Calibrate ZQ (ddr40_phy_calib_zq) during Shmoo\n"));
            ...
            /*    SOC_IF_ERROR_RETURN(READ_CI_PHY_CONTROLr(unit,0,&data));
                ...
                if (!SAL_BOOT_QUICKTURN) {
                    ...
                    SOC_DEBUG(SOC_DBG_DDR,("D07. VDL Calibration during Shmoo\n"));
                    rval = 0;
                    ...
                    SOC_DEBUG(SOC_DBG_DDR,("D07. VDL Calibration SKIPPED during Shmoo\n"));
                }/* Not QUICKTURN */
                break;
            case DDR_PHYTYPE_CE:
                break;
...
int _shmoo_init_vdl_result(int unit, int ci, int wl, int self_test, uint32 * result)
{
    ...
    if (wl == 0) {
        data = SET_OVR_STEP(setting + SHMOO_CI02_WL0_OFFSET_RD_EN);
    ...
    data =
        (wl ==
          0) ? SET_OVR_STEP(setting + SHMOO_CI02_WL0_OFFSET_RD_DQ)
    ...
    data =
        (wl ==
		  0) ? SET_OVR_STEP(setting + SHMOO_CI02_WL0_OFFSET_RD_DQS)
    ...
    data =
        (wl ==
          0) ? SET_OVR_STEP(setting + SHMOO_CI02_WL0_OFFSET_WR_DQ)
...
int soc_ddr40_shmoo_savecfg(int unit, int ci)
{
    ...
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DATA_DLY, ci);
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DQ_WL0_RP, ci);
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DQ_WL1_RP, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x,0x%08x,0x%08x", value[0], value[1], value[2], value[3]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DQ_WL0_RN, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x,0x%08x,0x%08x", value[0], value[1], value[2], value[3]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DQ_WL1_RN, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x,0x%08x,0x%08x", value[0], value[1], value[2], value[3]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    value[1] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_RD_DQS, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x", value[0], value[1]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_VREF, ci);
    ...
    value[0] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_WR_DQ, ci);
    sal_sprintf(val_str, "0x%08x", value[0]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_WR_DQ_WL0, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x,0x%08x,0x%08x", value[0], value[1], value[2], value[3]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
    ...
    value[3] = OVR_COMPRESS(valarr[0]) |  (OVR_COMPRESS(valarr[1]) << 8) |
               (OVR_COMPRESS(valarr[2]) << 16) | (OVR_COMPRESS(valarr[3]) << 24);
    sal_sprintf(name_str, "%s_ci%d", spn_DDR3_TUNE_WR_DQ_WL1, ci);
    sal_sprintf(val_str, "0x%08x,0x%08x,0x%08x,0x%08x", value[0], value[1], value[2], value[3]);
    if (soc_mem_config_set(name_str, val_str) < 0) {
         return SOC_E_MEMORY;
    }
