/*
 * $Id: xlmac.c 1.19.2.3 Broadcom SDK $
...
_mac_xl_drain_cells(int unit, soc_port_t port)
{
    ...
    for (;;) {
        ...
        if (soc_reg64_field32_get(unit, XLMAC_TXFIFO_CELL_CNTr, rval64,
                                  CELL_CNTf) == 0) {
            break;
        }
...
mac_xl_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    values[0] = values[1] = speed < 10000 ? 1 : 0;
    SOC_IF_ERROR_RETURN
                  ...
                  int value)
{
    ...
    case SOC_MAC_CONTROL_E2E_ENABLE:
        SOC_IF_ERROR_RETURN(READ_XLMAC_E2E_CTRLr(unit, port, &rval));
        soc_reg64_field32_set(unit, XLMAC_E2E_CTRLr, &rval, E2E_ENABLEf, value);
        SOC_IF_ERROR_RETURN(WRITE_XLMAC_E2E_CTRLr(unit, port, rval));
        break;
                  ...
                  int *value)
{
    ...
    case SOC_MAC_CONTROL_E2E_ENABLE:
        SOC_IF_ERROR_RETURN(READ_XLMAC_E2E_CTRLr(unit, port, &rval));
        *value = soc_reg64_field32_get(unit, XLMAC_E2E_CTRLr, rval,
                                       E2E_ENABLEf);
        break;
                          ...
                          soc_port_ability_t *ability)
{
    ...
    int phy_port, port1, port2, port3;
    ...
    port1 = si->port_p2l_mapping[phy_port - bindex + 1];
    port2 = si->port_p2l_mapping[phy_port - bindex + 2];
    port3 = si->port_p2l_mapping[phy_port - bindex + 3];
    if ((port1 != -1 &&
         !SOC_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), port1)) ||
        (port3 != -1 &&
         !SOC_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), port3))) {
        port_speed_max = port_speed_max < 10000 ? port_speed_max : 10000;
    } else if (port2 != -1 &&
               !SOC_PBMP_MEMBER(SOC_PORT_DISABLED_BITMAP(unit, all), port2)) {
        port_speed_max = port_speed_max < 20000 ? port_speed_max : 20000;
    }
