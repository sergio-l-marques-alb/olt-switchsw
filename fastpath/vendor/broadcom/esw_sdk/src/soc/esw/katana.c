/*
 * $Id: katana.c 1.137.2.1 Broadcom SDK $
...
_soc_katana_parity_enable_mmu(int unit, soc_reg_t mmu_en_reg, int enable)
{
    ...
    for (sub_block_index = 0; ; sub_block_index++) {
        ...
        /* Access to CI_ERROR_MASK register is resulting is S-Chan timeout on some
    ...
    for (sub_block_index = 0; ; sub_block_index++) {
        ...
        switch (sub_block_type) {
        /* Access to EMC_ERROR_X_MASK register is resulting is S-Chan timeout on some
           KATANA devices which do not have ddr3 memory  */
...
STATIC int
_soc_katana_mem_nack_process_info(int unit, int pipe,
...
STATIC int
_soc_katana_mem_nack_error_process(int unit, soc_mem_t mem, int copyno, int pipe)
{
    const _soc_katana_parity_route_block_t *route_block;
    int route_block_index;
    uint32 cmic_bit;
                     ...
                     void *d3, void *d4)
{
    ...
    if ((rv = _soc_katana_mem_nack_error_process(unit, mem, block, pipe)) < 0) {
        soc_cm_debug(DK_ERR,
...
soc_katana_num_cosq_init(int unit)
{
    ...
    si = &SOC_INFO(unit);
    prev_port = 0;
    /* For KT: PBMP_ALL_ITER(unit, port) { */      /* } */ 
    for (port=0;port < 42;port++) {  
        if (SOC_IS_KATANA(unit) && (!SOC_PBMP_MEMBER((PBMP_ALL(unit)), port))) {
            continue;
        }
        ...
        if (IS_CPU_PORT(unit, port)) {
            ...
            si->port_num_uc_cosq[port] = SOC_IS_KATANA2(unit) ? 64 : 48;
            si->port_num_ext_cosq[port] = 0;
        } else {
...
#define SB_MMU_INT_BUF_SIZE    512/*c34*/
#define KA_MMU_INT_BUF_SIZE    2048/*c34*/
#define KA_MMU_EXT_BUF_SIZE    192/*c38*/
#define SB_MMU_EGRESS_QUEUE_ENTRIES    128/*c39*/
...
_soc_katana_mmu_init_helper(int unit)
{
    ...
    ka_mmu_params.num_ge_ports = si->ge.num; /* c18*/
    ...
    max_intbuf_size = ((soc_feature(unit,
                       soc_feature_mmu_reduced_internal_buffer) ?
                       SB_MMU_INT_BUF_SIZE : KA_MMU_INT_BUF_SIZE)
                       * 1024) / KA_MMU_INT_BUF_CELL_SIZE; /*c78*/
    ...
    opthd_total_shared_queue_eqe = (soc_feature(unit,
                                    ...
                                    1024 - opthd_total_min_grntee_eqe;/*192*/
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        soc_reg_field_set(unit, THDIEMA_PG_SHARED_LIMIT_CELLr, &rval,
                          PG_SHARED_DYNAMICf, lossless ? 1 : 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIEMA_PG_SHARED_LIMIT_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIEMA_PG_RESET_OFFSET_CELLr, &rval,
                          PG_RESET_OFFSETf, 2 * ethernet_mtu_cells_intbuf);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIEMA_PG_RESET_OFFSET_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIEMA_PG_RESET_FLOOR_CELLr, &rval,
                          PG_RESET_FLOORf, 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIEMA_PG_RESET_FLOOR_CELLr (unit, port, pg_num, rval));
        rval = 0;
        val = (IS_CPU_PORT (unit, port)) ? 0 :
                  ((IS_HG_PORT (unit, port)) ?
                      ka_mmu_params.hg_pg_hdrm_intbuf :
        ...
        soc_reg_field_set(unit, THDIRQE_PG_SHARED_LIMIT_CELLr, &rval,
                          PG_SHARED_DYNAMICf, lossless ? 1 : 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIRQE_PG_SHARED_LIMIT_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIRQE_PG_RESET_OFFSET_CELLr, &rval,
                          PG_RESET_OFFSETf, 2);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIRQE_PG_RESET_OFFSET_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIRQE_PG_RESET_FLOOR_CELLr, &rval,
                          PG_RESET_FLOORf, 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIRQE_PG_RESET_FLOOR_CELLr (unit, port, pg_num, rval));
        rval = 0;
        val = (IS_CPU_PORT (unit, port)) ? 0 :
                  ((IS_HG_PORT (unit, port)) ?
                      ka_mmu_params.hg_pg_hdrm_rewqe :
        ...
        soc_reg_field_set(unit, THDIRQE_PG_HDRM_LIMIT_CELLr, &rval,
                          PG_HDRM_LIMITf, val);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIRQE_PG_HDRM_LIMIT_CELLr (unit, port, pg_num, rval));
        ...
        soc_reg_field_set(unit, THDIQEN_PG_SHARED_LIMIT_CELLr, &rval,
                          PG_SHARED_DYNAMICf, lossless ? 1 : 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIQEN_PG_SHARED_LIMIT_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIQEN_PG_RESET_OFFSET_CELLr, &rval,
                   PG_RESET_OFFSETf, 2 * ka_mmu_params.num_replication_per_pkt);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIQEN_PG_RESET_OFFSET_CELLr (unit, port, pg_num, rval));
        rval = 0;
        soc_reg_field_set(unit, THDIQEN_PG_RESET_FLOOR_CELLr, &rval,
                          PG_RESET_FLOORf, 0);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIQEN_PG_RESET_FLOOR_CELLr (unit, port, pg_num, rval));
        rval = 0;
        val = (IS_CPU_PORT (unit, port)) ? 0 :
                  ((IS_HG_PORT (unit, port)) ?
        ...
        soc_reg_field_set(unit, THDIQEN_PG_HDRM_LIMIT_CELLr, &rval,
                          PG_HDRM_LIMITf, val);
        SOC_IF_ERROR_RETURN(
                     WRITE_THDIQEN_PG_HDRM_LIMIT_CELLr (unit, port, pg_num, rval));
    }
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        if (IS_CPU_PORT(unit, port) || 
            ...
            for (i = si->port_cosq_base[port];
                 ...
                 soc_mem_field32_set(unit, MMU_THDO_QOFFSET_CELLm, &entry,
                                    LIMIT_RED_CELLf, lossless ? (val/8) : val);
                 ...
                 soc_mem_field32_set(unit, MMU_THDO_QCONFIG_CELLm, &entry,
                                 Q_COLOR_LIMIT_DYNAMIC_CELLf, lossless ? 0 : 1);
                 soc_mem_field32_set(unit, MMU_THDO_QCONFIG_CELLm, &entry,
                                 LIMIT_YELLOW_CELLf, lossless ? (val / 8) : val);
    ...
    if (!all_int_mem_port) { /* not applicable for all internal mem port */
        ...
        for(i=0; i < SOC_REG_NUMELS(unit,
            ...
            soc_reg_field_set(unit, OP_QUEUE_LIMIT_YELLOW_THDOEMAr, &rval,
                              Q_LIMIT_YELLOWf, lossless ? (val / 8) : val);
            SOC_IF_ERROR_RETURN(
        ...
        for(i=0; i < SOC_REG_NUMELS(unit,
            ...
            soc_reg_field_set(unit, OP_QUEUE_LIMIT_RED_THDOEMAr, &rval,
                              Q_LIMIT_REDf, lossless ? (val / 8) : val);
            SOC_IF_ERROR_RETURN(
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (i = si->port_cosq_base[port];
            ...
            soc_mem_field32_set(unit, MMU_THDO_QCONFIG_QENTRYm, &entry,
                               Q_COLOR_LIMIT_DYNAMIC_QENTRYf, lossless ? 0 : 1);
            soc_mem_field32_set(unit, MMU_THDO_QCONFIG_QENTRYm, &entry,
                               LIMIT_YELLOW_QENTRYf, lossless ? (val/8) : val);
