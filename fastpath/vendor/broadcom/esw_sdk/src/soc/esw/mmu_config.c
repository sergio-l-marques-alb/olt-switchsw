/*
 * $Id: mmu_config.c 1.11 Broadcom SDK $
                           ...
                           _soc_mmu_device_info_t *devcfg)
{
    ...
    for (idx = 0; idx < SOC_MMU_CFG_QGROUP_MAX; idx++) {
        if (qgpid[idx] & (qgpid[idx] - 1)) {
            soc_cm_print("Queue belonging to same group use different Pools !!");
            sal_free(qgpid);
            return SOC_E_PARAM;
        }
    }
    ...
    if (soc_cm_debug_check(DK_VERBOSE)) {
        ...
        for (idx = 0; idx < _SOC_MMU_CFG_DEV_POOL_NUM(devcfg); idx++) {
            ...
            soc_cm_print("  Pool %d total mcq entry reserved: %d\n",
                        ...
                        _soc_mmu_device_info_t *devcfg)
{
    ...
    for (idx = 0; idx < _SOC_MMU_CFG_DEV_POOL_NUM(devcfg); idx++) {
        ...
        if (buf_pool->total <= 0) {
            soc_cm_print("Pool %d has no shared space after deducting guaranteed !!", idx);
            return SOC_E_FAIL;
        }
