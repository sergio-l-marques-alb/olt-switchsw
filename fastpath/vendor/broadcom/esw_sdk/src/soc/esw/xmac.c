/*
 * $Id: xmac.c 1.112.2.19 Broadcom SDK $
...
struct {
    ...
    { 13000, SOC_PA_SPEED_13GB }, /* Added 13GB For Katana: 12GB ??? */
...
mac_x_init(int unit, soc_port_t port)
{
	...
	if (SOC_IS_KATANA(unit)) {
        if (IS_GE_PORT(unit,port)) {
			...
			soc_reg64_field32_set(unit, XMAC_RX_CTRLr, &rval, STRICT_PREAMBLEf, 0);
			soc_reg64_field32_set(unit, XMAC_RX_CTRLr, &rval, PROCESS_VARIABLE_PREAMBLEf, 1);
