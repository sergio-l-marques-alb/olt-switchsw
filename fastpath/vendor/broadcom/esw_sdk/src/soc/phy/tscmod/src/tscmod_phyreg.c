/*----------------------------------------------------------------------
 * $Id: tscmod_phyreg.c 1.25.2.1 Broadcom SDK $
  ...
  *data = (0xffff & tmpData);
  ...
  if (pc->verbosity & TSCMOD_DBG_MDIO)
    printf("%-22s: unit:%d port:%d phy_ad:%d addr:0x%08x data:0x%04x\n",
                                  __func__,pc->unit,pc->port,phy_ad,addr,*data);
  return rv;
}
...
int tscmod_totsc_read(tscmod_st *pc,uint32 addr,uint16 *data)
{
   ...
   if (pc->verbosity & TSCMOD_DBG_MDIO)
      printf("%-22s: unit:%d port:%d phy_ad:%d addr:0x%08x data:0x%04x radd=%x\n",
             __func__,pc->unit,pc->port, pc->phy_ad,addr,*data, raddr);
   rv = SOC_E_NONE;
   return rv;  
}
  ...
  do {
     ...
     if (pc->verbosity & TSCMOD_DBG_TO)
        printf ("%-22s: p=%0d reg:0x%08x bit_num:0x%08x bitset:%d d=%x\n",
               __func__, pc->port, reg, bit_num, bitset, data16);
...
int tscmod_reg_domain_test(tscmod_st *pc, uint32 a, int access_mode, int rpt_mode) 
{
   ...
   if((pc->ctrl_type & TSCMOD_CTRL_TYPE_FW_LOADED) && (!(pc->ctrl_type &TSCMOD_CTRL_TYPE_UC_STALL))) {
      ...
      if((addr>=0xc010) && (addr< 0xc030)) viol = 2 ; 
      if((addr>=0x9010) && (addr< 0x9020)) viol = 3 ; 

      if(rpt_mode && viol) {
         switch(access_mode) {
         case TSCMOD_REG_WR: printf("ERROR %-22s: p=%0d wr addr=%0x violation=%0d\n", 
                        __func__, pc->port, addr, viol); break ; 
...
{
   int rv ;  uint16 rdata0, rdata1, rdata2, rdata3 ; int i, tmp_sel, tmp_lane ; 
   tmp_sel = pc->lane_select ; 
   tmp_lane= pc->this_lane ;
   printf("Diagnosis: sync_cmd=") ; 
   /* TSCMOD_UC_SYNC_CMDr has 4 copies per core. TSC623_SDK_42585_A1 */
   for(i=0; i<4; i++) {
      pc->this_lane   = i ; 
...
{
   int rv, loop, cnt ;  uint16 cmd, add, rdata ;  
   uint32            start_time, diff_time ;

   if ((pc->verbosity & TSCMOD_DBG_REG)&&(access_mode==TSCMOD_REG_WR))
      printf("%-22s: u=%0d p=%0d addr=%x WR rd=%x wd=%x mask=%x\n", 
             __func__, unit, pc->port, a, *phy_data, d, m) ;
   if(pc->tsc_touched) 
      printf("%-22s: Semaphore error u=%0d p=%0d addr=%x rd=%x wd=%x mask=%x\n", 
             __func__, unit, pc->port, a, *phy_data, d, m) ;
   pc->tsc_touched = 1; 
   rv = SOC_E_NONE ;  cmd = 0 ;
      ...
      if(loop++ >= 1000) {  
         ...
         if (pc->verbosity & TSCMOD_DBG_REG) {
            ...
            printf("%-22s: u=%0d p=%0d addr=%x access=%0d wait done (%0d usc) pending\n", 
                   __func__, pc->unit, pc->port, a, access_mode, diff_time) ;
         }
      }
      if(cnt >10) {  
         diff_time = sal_time_usecs() - start_time  ; 
         printf("ERROR: %-22s: u=%0d p=%0d addr=%x access=%0d d=%x m=%x sel=0x%x l=%0d usec=%0d wait aborted\n", 
                __func__, pc->unit, pc->port, a, access_mode, d, m, pc->lane_select, pc->this_lane, diff_time) ;
   ...
   if(pc->verbosity & TSCMOD_DBG_REG) {
      ...
      if(access_mode !=TSCMOD_REG_RD)
         printf("%-22s: u=%0d p=%0d addr=%x access=%0d wait usec=%0d\n", 
                __func__, unit, pc->port, a, access_mode, diff_time) ;
   }
   if(access_mode ==TSCMOD_REG_RD) {
   ...
   if ((pc->verbosity & TSCMOD_DBG_REG) && (access_mode ==TSCMOD_REG_RD))
      printf("%-22s: u=%0d p=%0d addr=%x RD access=%0d rd=%x wait usec=%0d\n", 
             __func__, unit, pc->port, a, access_mode, rdata, diff_time) ;
   return rv ;  
}
...
{
   int rpt_mode, access_mode ; 
   uint16 d, m ; 

   rpt_mode = 0 ; access_mode =TSCMOD_REG_RD ;  d=0 ; m =0 ;  
   
   if( tscmod_reg_domain_test(pc, a, access_mode, rpt_mode) ) 
      return tscmod_reg_uc_sync_cmd(unit, pc, access_mode, a, phy_data, d, m) ; 
   else 
      return tscmod_reg_aer_read_core(unit, pc, a, phy_data) ; 
}
...
int tscmod_reg_aer_write_core(uint32 unit, tscmod_st *pc, uint32 a, uint16 d)
{
  ...
  if (pc->verbosity & TSCMOD_DBG_REG)
    printf("%-22s: u:%0d p:%0d phy_ad=%x addr:0x%x(%x) data:0x%x\n",
           __func__,pc->unit,pc->port, pc->phy_ad, a, addr, d);
...
{     
   ...
   if( tscmod_reg_domain_test(pc, a, access_mode, rpt_mode) ) 
      return tscmod_reg_uc_sync_cmd(unit, pc, access_mode, a, &rdata, d, m) ; 
   else 
      return tscmod_reg_aer_write_core(unit, pc, a, d) ; 
...
int tscmod_reg_aer_modify_core(uint32 unit, tscmod_st *pc,uint32 a,uint16 d,uint16 m)
{
  ...
  if (pc->verbosity & TSCMOD_DBG_REG)
    printf("%-22s: u:%0d p:%0d Pre Mod:addr:0x%08x->(w/r:%08x/%08x) data:0x%04x mask:0x%x\n",
           __func__, pc->unit, pc->port, a, waddr,raddr, data, mask);
  ...
  if (pc->verbosity & TSCMOD_DBG_REG )
    printf("%-22s: u:%0d p:%0d Post Mod: phy_ad:%x addr:0x%08x rdb:0x%04x data:0x%04x mask:0x%x\n",
           __func__, pc->unit, pc->port, pc->phy_ad, waddr, otmp, tmp_data, mask);
  return rv;
}
...
{  
   ...
   if( tscmod_reg_domain_test(pc, a, access_mode, rpt_mode) ) 
      ...
      return tscmod_reg_aer_modify_core(unit, pc, a, d, m) ; 
