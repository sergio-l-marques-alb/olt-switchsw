/*
 * $Id: phy84728.c 1.35.30.1 Broadcom SDK $
 *
...
phy_84728_init(int unit, soc_port_t port)
{
    ...
    */
    ...
    if (BSDK_PHY84728_IS_KR_VARIANT(devid)) {
    ...
    soc_cm_debug(DK_PHY, "84728: u=%d port=%d mode=%s : init.\n", 
...
_phy_54942_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /* disable the Rx */
    SOC_IF_ERROR_RETURN
        (BSDK_MOD_PHY84728_XGXS_CTRLr(pc, enable? (1<<13) : 0, (1<<13)));
...
_phy_84728_mode_set(int unit, soc_port_t port, uint32 mode) {
    ...
    switch(mode) {
        case WAN_MODE:
            ...
            mask = PMD_MISC_CTRLr_WAN_MODE;
            SOC_IF_ERROR_RETURN
                (BSDK_MOD_PHY84728_PMD_MISC_CTRLr(pc, PMD_MISC_CTRLr_WAN_MODE, mask));
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_CLOCK_ENABLE:
            SOC_IF_ERROR_RETURN
                (BSDK_MOD_PHY84728_PMD_MISC2_CTRLr(pc, value ? (0x1 << 15) : 0, (0x1 << 15)));
            rv = SOC_E_NONE;
            break;
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_CLOCK_ENABLE:
            SOC_IF_ERROR_RETURN
                (BSDK_RD_PHY84728_PMD_MISC2_CTRLr(pc, &temp16));
                              ...
                              bsdk_phy84728_intf_t mode)
{
    ...
    case BSDK_PHY84728_INTF_SGMII :
        ...
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_WR(pc, 0x00, 0x0007, 0x8300, 0x0184));
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_WR(pc, 0x00, 0x0007, 0x8308, 0x6000));
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_MOD(pc, 0x00, 0x0007, 0x8309, 0x2000, 0x2020));
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_WR(pc, 0x00, 0x0007, 0x835c, 0x0801));
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_WR(pc, 0x00, 0x0007, 0xffe0, 0x1140));
#if 0
        SOC_IF_ERROR_RETURN(
            BSDK_MOD_PHY84728_LN_DEV7_AN_BASE1000X_CTRL1r(pc, 0,
...
soc_cm_print("phy_sys_xaui_tx_lane_map: 0x%x\n", phy_sys_xaui_tx_lane_map);
...
soc_cm_print("phy_sys_xaui_rx_lane_map: 0x%x\n", phy_sys_xaui_rx_lane_map);
...
{
        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_MOD(pc, 0x00, 0x0004, 0x8000, 0x0000, 0x2000));

        SOC_IF_ERROR_RETURN(
            BSDK_WR_PHY84728_SYS_DEV4_XGXS_BASE1000X_CTRL1r(pc, 0x01a0));

        SOC_IF_ERROR_RETURN(
            BSDK_WR_PHY84728_SYS_DEV4_XGXS_MISC1r(pc, 0x6000));

        SOC_IF_ERROR_RETURN(
            BSDK_MOD_PHY84728_SYS_DEV4_XGXS_MISC2r(pc, 0x2000, 0x2020));

        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_WR(pc, 0x00, 0x0004, 0x835c, 0x0001));

        SOC_IF_ERROR_RETURN(
            BSDK_WR_PHY84728_SYS_DEV4_XGXS_MII_CTRLr(pc, 0x0140));

        SOC_IF_ERROR_RETURN(
            BSDK_PHY84728_REG_MOD(pc, 0x00, 0x0004, 0x8000, 0x2000, 0x2000));
...
phy_84728_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    uint16 rx_control_reg = 0, tx_control_reg = 0, rx_tx_control_reg = 0,
           ...
           interval, pulse_1_length, pulse_2_length;
    int i;
    phy_ctrl_t *pc;
