/*
 * $Id: phy_xmac.h 1.4.2.2 Broadcom SDK $
...
#define XMAC_REG_MAC_CTRLr(p) \
		(0x01100000 + (((p) & 0x7) << 12))

#define XMAC_REG_TX_CTRLr(p) \
		(0x01100004 + (((p) & 0x7) << 12))

#define XMAC_REG_RX_CTRLr(p)  \
        (0x01100006 + (((p) & 0x7) << 12))

#define XMAC_REG_RX_MAXSZr(p)  \
        (0x01100008 + (((p) & 0x7) << 12))

#define XMAC_REG_RX_LSS_CTRLr(p)  \
        (0x0110000a + (((p) & 0x7) << 12))

#define XMAC_REG_PAUSE_CTRLr(p)  \
        (0x0110000d + (((p) & 0x7) << 12))

#define XMAC_REG_MAC_MACSEC_CTRLr(p)  \
        (0x01100024 + (((p) & 0x7) << 12))
...
#define XMAC_READ_MAC_MACSEC_CTRL_REG(_mmc, _dev_port, _data)          \
            XMAC_MMI_READ_REG((_mmc), (_dev_port),                     \
                                 XMAC_REG_MAC_MACSEC_CTRLr(_dev_port), \
                                 (_data))
...
#define XMAC_READ_PAUSE_CTRL_REG(_mmc, _dev_port, _data)              \
            XMAC_MMI_READ_REG((_mmc), (_dev_port),                    \
                                 XMAC_REG_PAUSE_CTRLr(_dev_port),     \
                                 (_data))
...
#define XMAC_READ_RX_LSS_CTRL_REG(_mmc, _dev_port, _data)             \
            XMAC_MMI_READ_REG((_mmc), (_dev_port),                    \
                                 XMAC_REG_RX_LSS_CTRLr(_dev_port),    \
                                 (_data))
