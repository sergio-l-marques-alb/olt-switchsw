/*
 * $Id: phy54680.c 1.151.16.1 Broadcom SDK $
...
STATIC int
_phy_northstar_eee_war(int unit, soc_port_t port);
...
_phy_northstar_eee_war(int unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x17, 0x0ffb));
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x15, 0x7c00));

    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x17, 0x0fff));
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x15, 0x5f0f));

    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x17, 0x0fff));
    SOC_IF_ERROR_RETURN(
        WRITE_PHY_REG(unit, pc, 0x15, 0x500f));
    ...
    soc_cm_debug(DK_PORT,"_phy_northstar_eee_war port %d\n", port);
...
_phy_54680_reset_setup(int unit, soc_port_t port)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        SOC_IF_ERROR_RETURN
            (phy_54680_control_set( unit, port, SOC_PHY_CONTROL_PORT_PRIMARY, primary_port));
        SOC_IF_ERROR_RETURN
...
#if defined(BCM_NORTHSTAR_SUPPORT)
    if (PHY_IS_BCM53010_A1(pc)) {
        /* Apply the EEE workaround */
        SOC_IF_ERROR_RETURN(
            _phy_northstar_eee_war(unit, port));
    }
#endif /* BCM_NORTHSTAR_SUPPORT */ 

#if defined(BCM_53128) || defined(BCM_NORTHSTAR_SUPPORT)
    /* BCM53125 internal GPHY is EEE capable */
    if (PHY_IS_BCM53128(pc) || PHY_IS_BCM53010(pc)) {
         PHY_FLAGS_SET(unit, port, PHY_FLAGS_EEE_CAPABLE);
    }
#endif /* BCM_53128 || BCM_NORTHSTAR_SUPPORT */    
    ...
    if (!enable) {
        if ((PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_CAPABLE)) &&
           (((pc->phy_rev & 0x4) == 0x0) || /* A0,A1,B0,B1 */
            ...
            (PHY_IS_BCM53010(pc) && pc->phy_rev == 0x0))) {
...
phy_54680_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || PHY_IS_BCM53324(pc) || 
        PHY_IS_BCM52681E(pc) || PHY_IS_BCM53010(pc)) {
        /* No SGMII interface */
...
phy_54680_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    if (PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || PHY_IS_BCM53010(pc)) {
        /* No SGMII interface */
        *pif = pc->interface;
        return (SOC_E_NONE);
    }
...
_phy_54680_eee_enable(int unit, soc_port_t port, int enable)
{
    ...
    if (enable == 1) {
        ...
        } else if(PHY_IS_BCM53125(pc) || 
            PHY_IS_BCM53128(pc) || PHY_IS_BCM53010(pc)) {
            /* Skip to configure the 7.803d register */
        } else {
            SOC_IF_ERROR_RETURN
        ...
        if(!PHY_IS_BCM53125(pc) && !PHY_IS_BCM53128(pc) &&
           !PHY_IS_BCM53010(pc)) { /* no support  of AutogrEEEn */
            SOC_IF_ERROR_RETURN
        ...
        } else if(PHY_IS_BCM53125(pc) || PHY_IS_BCM53128(pc) || 
            PHY_IS_BCM53010(pc)) {
            /* Skip to configure the 7.803d register */
        } else {
            SOC_IF_ERROR_RETURN
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_EEE:
        ...
        if (value == 1) {
            ...
            if ((!PHY_IS_BCM53125(pc)) && (!PHY_IS_BCM53128(pc)) 
                && (!PHY_IS_BCM53324(pc)) && (!PHY_IS_BCM53010(pc))) { 
                /* Disable Auto EEE */
                SOC_IF_ERROR_RETURN
...
phy_54680_probe(int unit, phy_ctrl_t *pc)
{
    ...
    case PHY_BCM53010_MODEL:
    case PHY_BCM53018_MODEL:
    case PHY_BCM53324_MODEL:
    case PHY_BCM53020_MODEL:
        return SOC_E_NONE;
    break;
...
phy_54680_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x52, conf->otpid2));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4b, conf->ts_divider & 0xfff));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc, 0, 0x6f, conf->rx_link_delay & 0xffff));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc, 0, 0x70, (conf->rx_link_delay >> 16) & 0xffff));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
        ...
        mask |= (0xf << 2);
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x51, conf->framesync.length_threshold));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x50, conf->framesync.event_offset));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
        ...
        mask |= (0x3 << 0);
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x48, conf->syncout.interval & 0xffff));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x49, 
                ((conf->syncout.pulse_1_length & 0x3) << 14) | ((conf->syncout.interval >> 16) & 0x3fff)));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4a, 
                ((conf->syncout.pulse_2_length & 0x1ff) << 7) | ((conf->syncout.pulse_1_length >> 2) & 0x7f)));

        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4e, 
                (uint16)(COMPILER_64_LO(conf->syncout.syncout_ts) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4d, 
                (uint16)((COMPILER_64_LO(conf->syncout.syncout_ts) >> 16) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4c, 
                (uint16)(COMPILER_64_HI(conf->syncout.syncout_ts) & 0xffff)));
    }

    SOC_IF_ERROR_RETURN
        (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x4f, (gmode << 14) | (framesync_mode << 2) | (syncout_mode << 0),
                                       mask ));

    /* tx_timestamp_offset */
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x15 + offset, (uint16)(conf->tx_timestamp_offset & 0xffff)));

        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x63, 
                    ((conf->tx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)));
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x64, 
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
        ...
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x65, 
                    ((conf->rx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)));
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x66, 
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54680e_blk_top_lvl_reg_modify(unit, pc, 0, 0x11, value, mask)); 
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54680e_blk_top_lvl_reg_modify(unit, pc, 0, 0x12, value << 8, mask << 8)); 

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
        /* Initial ref phase [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x38, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) & 0xffff)));

        /* Initial ref phase [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x39, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) >> 16) & 0xffff)));

        /*  Initial ref phase [47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3a, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_ref_phase) & 0xffff)));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
        /* Ref phase delta [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3b, (uint16)(conf->phy_1588_dpll_ref_phase_delta & 0xffff)));

        /* Ref phase delta [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3c, (uint16)((conf->phy_1588_dpll_ref_phase_delta >> 16) & 0xffff)));
    }
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54680e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x3d, (conf->phy_1588_dpll_k1 & 0xff)<<8 | (conf->phy_1588_dpll_k2 & 0xff), mask ));

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
        /* DPLL K3 */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3e, conf->phy_1588_dpll_k3 & 0xff));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3f, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) & 0xffff)));

        /* Initial loop filter[31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x40, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));

        /*  Initial loop filter[47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x41, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) & 0xffff)));

        /* Initial loop filter[63:48]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x42, (uint16)((COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));
    }
...
phy_54680_timesync_config_get(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x51, &value));
        conf->framesync.length_threshold = value;
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x50, &value));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x48, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x49, &temp2));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4a, &temp3));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4e, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4d, &temp2));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4c, &temp3));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x52, &conf->otpid2));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4b, &conf->ts_divider));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc, 0, 0x6f, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc, 0, 0x70, &temp2));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
        ...
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x63, &temp2));
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x64, &temp2));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x1d + offset, &temp1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x65, &temp2));
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x66, &temp2));
            temp32 = (temp2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
        }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
        /* Initial ref phase [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x38, &temp1));

        /* Initial ref phase [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x39, &temp2));

        /*  Initial ref phase [47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3a, &temp3));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
        /* Ref phase delta [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3b, &temp1));

        /* Ref phase delta [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3c, &temp2));
    ...
    /* DPLL K1 & K2 */
    SOC_IF_ERROR_RETURN
        (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3d, &temp1 ));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3e, &temp1));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        /* Initial loop filter[15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3f, &temp1));

        /* Initial loop filter[31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x40, &temp2));

        /*  Initial loop filter[47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x41, &temp3));

        /* Initial loop filter[63:48]  */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x42, &temp4));
...
phy_54680_timesync_control_set(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_FRAMESYNC:
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4f, &temp1));
        temp2 = ((temp1 | (0x3 << 14) | (0x1 << 12)) & ~(0xf << 2)) | (0x1 << 5);
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp2));
        sal_udelay(1);
        temp2 &= ~((0x1 << 5) | (0x1 << 12));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp2));
        sal_udelay(1);
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp1));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x2e, temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x2f, temp2));
        break;
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x30, temp1));
        break;
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_TX_TIMESTAMP_OFFSET:
        ...
        /* tx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x15 + offset, (uint16)(value0 & 0xffff)));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_RX_TIMESTAMP_OFFSET:
        ...
        /* rx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x1d + offset, (uint16)(value0 & 0xffff)));
...
phy_54680_timesync_control_get(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 *value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_HEARTBEAT_TIMESTAMP:
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x1));

        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x56, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x57, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x58, &value2));

        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x2));
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x0));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_CAPTURE_TIMESTAMP:
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x4));

        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x53, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x54, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x55, &value2));

        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x8));
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54680_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x0));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOCAL_TIME:
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x45, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x46, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x47, &value2));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x2e, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x2f, &value2));
        value0 = 0;
        if (value1 & (1U << 11)) {
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT:
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x31, &value1));
        value0 = 0;
        if (value1 & (1U << 1)) {
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x30, &value1));
        value0 = 0;
        if (value1 & (1U << 1)) {
    ...
    /* tx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x15 + offset, &value1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x63, &value2));
            temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x64, &value2));
    ...
    /* rx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x1d + offset, &value1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x65, &value2));
            temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54680e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x66, &value2));
            temp32 = (value2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
        }
        /* *value = temp32 | value1; */
        COMPILER_64_SET((*value), 0, (uint32)(temp32 | value1));
