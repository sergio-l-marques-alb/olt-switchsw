/*
 * $Id: wcmod.c 1.338.2.2 Broadcom SDK $ 
...
_phy_wcmod_config_init(int unit, soc_port_t port)
{
   ...
   /* for CD3 QSGMII port, init the warpcore staructure */
    if (((serdes_id0 & SERDESID_SERDESID0_MODEL_NUMBER_MASK) == MODEL_QSGMII) && SOC_IS_CALADAN3(unit)) {
        wcmod_st *temp_ws;
        temp_ws = (wcmod_st *)(ws + 1);
            WCMOD_MEM_SET(temp_ws, 0, sizeof(*temp_ws));
            SOC_IF_ERROR_RETURN
                    (_init_wcmod_st(temp_ws));
    }
   ...
   /* default */
    ...
    } else if (SOC_PBMP_MEMBER(PBMP_XL_ALL(unit), port)) {
    ...
    if (SOC_IS_TRIUMPH3(unit) || SOC_IS_ARAD(unit) || SOC_IS_CALADAN3(unit) || SOC_IS_HURRICANE2(unit)) {
        soc_info_t *si;
    ...
    if (PHY_INDEPENDENT_LANE_MODE(unit, port)) {
        ...
        if (pc->phy_mode == PHYCTRL_QSGMII_CORE_PORT) {
    ...
    switch (FORCE_CL72_STATE(pc)) {
        case WCMOD_CL72_STATE_WAIT_FOR_LINK:
            if (link == FALSE) {
                ...
                /* reset the status */
                SOC_IF_ERROR_RETURN
                    (MODIFY_WC40_UC_INFO_B1_GPIO_STS_LN0r(unit, ws, 0, 1));
                SOC_IF_ERROR_RETURN
                    (MODIFY_WC40_UC_INFO_B1_GPIO_STS_LN1r(unit, ws, 0, 1));
                SOC_IF_ERROR_RETURN
                    (MODIFY_WC40_UC_INFO_B1_GPIO_STS_LN2r(unit, ws, 0, 1));
                SOC_IF_ERROR_RETURN
                    (MODIFY_WC40_UC_INFO_B1_GPIO_STS_LN3r(unit, ws, 0, 1));
            }
        ...
        case WCMOD_CL72_STATE_LINK_GOOD:
            if (link == FALSE) {
                /*first check the pmd started bit */
                SOC_IF_ERROR_RETURN
                    (READ_WC40_UC_INFO_B1_GPIO_STS_LN0r(unit, ws, &data16));
                /*if the bit is set , then go to wait for link */
                data16 &= 0x1;
...
_phy_wcmod_tx_control_set(int unit, phy_ctrl_t *pc, WCMOD_TX_DRIVE_t *tx_drv)
{
    ...
    uint32          pdriver;
    uint32          post2;    
    int i, lane, num_core, core_num, tempLane, temp_id[6], size;
    wcmod_st    *ws_warpcore[6];
    WCMOD_DEV_DESC_t     *pDesc;
    ...
    for (i = 0 ; i < num_core; i++) {
        ...
        temp_id[i] = ws_warpcore[i]->phy_ad;
        SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_warpcore[i]));
        ws_warpcore[i]->phy_ad = temp_id[i];
    }
    ...
    for(i=0; i<size; i++) {
        ...
        if (ws->verbosity) 
            printf ("%s: port=%0d pc=%p ixd=%0d lane_num=%0d sel=%0x size=%0d\n", 
                    __func__, port, (void *)pc, i, pc->lane_num, ws->lane_select, size);
...
_phy_wcmod_combo_core_init(int unit, soc_port_t port)
{
    ...
    /*for C3 only, do the slave wake up */
    /* set the 0x8197 for the slave wake up for CD3 */
    if (SOC_IS_CALADAN3(unit) && ((ws->model_type) == WCMOD_WC_C0)) {
         SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(unit, ws, 0x8000));
    }
    ...
    if (((ws->model_type) == WCMOD_WC_C0) && SOC_IS_TRIUMPH3(unit)) {
        SOC_IF_ERROR_RETURN
...
_phy_wcmod_qsgmii_core_init(int unit, soc_port_t port)
{
    ...
    /* get the mdio address of qsgmii, and warpcore only for CD3*/
    if (SOC_IS_CALADAN3(unit)) {
        SOC_IF_ERROR_RETURN
            (soc_phy_addr_multi_get(unit, port, array_max, &array_size, &mdio_addr[0]));
        ...
        if (SOC_IS_CALADAN3(unit)) {
            qsgmii_ws->phy_ad = mdio_addr[0];
        }
        SOC_IF_ERROR_RETURN (_wcmod_getRevDetails(qsgmii_ws));

        if (SOC_IS_CALADAN3(unit)) {
            ws = (wcmod_st *)(qsgmii_ws + 1);
            SOC_IF_ERROR_RETURN
                (_wcmod_phy_parameter_copy(pc, ws));
            ws->phy_ad = mdio_addr[1];
            SOC_IF_ERROR_RETURN (_wcmod_getRevDetails(ws));
        }
        ...
        /* set speed and interface   */
        qsgmii_ws->spd_intf = WCMOD_SPD_1G_QSGMII;
        SOC_IF_ERROR_RETURN
           (wcmod_tier1_selector("SET_SPD_INTF", qsgmii_ws, &rv));
        qsgmii_ws->lane_select = temp_lane_select;
        ...
        /* first check if the warpcore has been programmed or not */
        SOC_IF_ERROR_RETURN(READ_WC40_UC_INFO_B1_VERSIONr(unit, ws, &data));
        if(data == 0) {
            /*program the warpcore into 6.25G pcs by pass */
            ...
            /* set the default polarity setting */
            ws->lane_select = WCMOD_LANE_1_1_1_1;
            ws->per_lane_control = (pCfg->txpol <<2) ;
            ws->per_lane_control |=  0x1;  
            ws->per_lane_control |=  0x2; /*set TX*/
            SOC_IF_ERROR_RETURN
                (wcmod_tier1_selector("SET_POLARITY", ws, &rv));
            ws->per_lane_control = (pCfg->rxpol <<2) ;
            ws->per_lane_control |=  0x1;  
            ...
            /* set speed and interface   */
            ws->spd_intf = WCMOD_SPD_PCSBYP_5G;
            SOC_IF_ERROR_RETURN
               (wcmod_tier1_selector("SET_SPD_INTF", ws, &rv));
            ws->lane_select = temp_lane_select;
...
_phy_wcmod_multi_core_init(int unit, soc_port_t port)
{
    ...
    /* set the mld mdio address */
        ...
        ws->phy_ad = mdio_addr[num_core];
    } else {
        ws   =  (wcmod_st *)( temp_ws + num_core);
        ws->phy_ad = mdio_addr[num_core];
        ws   =  (wcmod_st *)( temp_ws + num_core + 1);
        ws->phy_ad = mdio_addr[num_core + 1];
    }
...
_phy_wcmod_ind_init_common(wcmod_st *ws, int fw_load_method) {
    ...
    /*for C3 only, do the slave wake up */
    /* set the 0x8197 for the slave wake up for CD3 */
    if (SOC_IS_CALADAN3(ws->unit) && ((ws->model_type) == WCMOD_WC_C0)) {
         SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(ws->unit, ws, 0x8000));
    }
    ...
    /* tr3 WC pll lock workaround */
    if (((ws->model_type) == WCMOD_WC_C0) && SOC_IS_TRIUMPH3(ws->unit)) {
        SOC_IF_ERROR_RETURN
...
phy_wcmod_link_get(int unit, soc_port_t port, int *link)
{
    ...
    if (!SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port)) {
        if (SOC_INFO(unit).port_num_lanes[pc->port] >= 10) {
            ...
            /*add the link recovery here for 100G plus port */
            if (*link == 0) {
                /*toggle the rx sequencer */
                int i;
                wcmod_st  *temp_ws;
                int tmp_lane_select;
                for (i = 0 ; i < 3; i++) {
                    ...
                    SOC_IF_ERROR_RETURN(MODIFY_WC40_DSC2B0_DSC_MISC_CTRL0r(unit, temp_ws, DSC2B0_DSC_MISC_CTRL0_RXSEQSTART_MASK,
                                        DSC2B0_DSC_MISC_CTRL0_RXSEQSTART_MASK));
                    SOC_IF_ERROR_RETURN(MODIFY_WC40_DSC2B0_DSC_MISC_CTRL0r(unit, temp_ws, 0,
                                        DSC2B0_DSC_MISC_CTRL0_RXSEQSTART_MASK));
...
_phy_wcmod_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if ((SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port)) && (SOC_IS_ARAD(unit) || SOC_IS_CALADAN3(unit))) {
        SOC_IF_ERROR_RETURN
...
_phy_wcmod_ind_speed_get(int unit, soc_port_t port, int *speed, int *intf, int *scr)
{
    ...
    if((*speed==20000)||(*speed==10000)) {
        ...
        vco_freq = DEV_CFG_PTR(pc)->refclk * divider;
...
phy_wcmod_xgxs16g1l_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    /* Hold rxSeqStart */
    SOC_IF_ERROR_RETURN
       (MODIFY_WC40_RX0_ANARXCONTROLr(unit, ws, 0x8000, 0x8000)) ;
    ...
    /* release rxSeqStart */
    SOC_IF_ERROR_RETURN
       (MODIFY_WC40_RX0_ANARXCONTROLr(unit, ws,
                                      0,  0x8000)) ;
    /* DSC_2_0_DSC_CTRL0_RXSEQSTART_MASK */
 ...
 */                                                                                
    ...
    if (an) {                                                                         
        ...
        SOC_IF_ERROR_RETURN(MODIFY_WC40_SERDESDIGITAL_MISC1r(unit, ws, 0x0,
                     SERDESDIGITAL_MISC1_FORCE_PLL_MODE_AFE_SEL_MASK| 0x0010));   
        /* SERDESDIGITAL_MISC1_FORCE_SPEED_SEL_MASK 0x0010 */
...
phy_wcmod_an_set2(int unit, soc_port_t port, int an)
{
    ...
    */
    ...
    if (an) {
        ...
        /* do we need software AN start time ?? */
        SOC_IF_ERROR_RETURN
            (WRITE_WC40_UC_INFO_B1_FIRMWARE_MODEr(unit, ws, 0x0));   
        ...
        an_enable = USE_CLAUSE_37_AN_WITH_BAM ;  /* default setting when an is */
...
int phy_wcmod_per_lane_control_prbs_polynomial_get(int unit, soc_port_t port, int lane, uint32 *value)
{
   ...
   SOC_IF_ERROR_RETURN
            (_phy_wcmod_control_prbs_polynomial_get(ws, value));
...
int phy_wcmod_per_lane_control_prbs_tx_invert_data_get(int unit, soc_port_t port, int lane, uint32 *value)
{
   ...
   SOC_IF_ERROR_RETURN
            (_phy_wcmod_control_prbs_tx_invert_data_get(ws, value));
...
int phy_wcmod_per_lane_control_prbs_enable_get(int unit, soc_port_t port, int lane, uint32 *value)
{
   ...
   temp_ws->this_lane = lane_num ;
   SOC_IF_ERROR_RETURN
            (_phy_wcmod_control_prbs_enable_get(ws, value));
...
int phy_wcmod_per_lane_control_prbs_rx_status_get(int unit, soc_port_t port, int lane, uint32 *value)
{
   ...
   temp_ws->this_lane = lane_num ;
   SOC_IF_ERROR_RETURN
            (_phy_wcmod_control_prbs_rx_status_get(ws, value));
...
_phy_wcmod_control_preemphasis_set(int unit, phy_ctrl_t *pc,
                                soc_phy_control_t type, uint32 value)
{
    int tmpLane, preTap, mainTap, postTap, regTap, tmpSelect;
    int rv;
...
{
    int tmpLane, preTap, mainTap, postTap, regTap, tmpSelect, temp_port_type, temp_dxgxs = 0;
    int rv;
    int lane_num, core_num = 0, max_core_num, temp_id;
    wcmod_st    *ws, *temp_ws;
    WCMOD_DEV_DESC_t     *pDesc;
    phy_ctrl_t *pc;
...
{
    int rv;
    int tmpLane, tmpSelect, tmpValue=0 ;
    wcmod_st    *ws;
    WCMOD_DEV_DESC_t     *pDesc;
    int     lane_ctrl;
    int     shifter;
    ...
    if(ws->verbosity>0) printf("%s p=%0d pc=%p per_lane_control=%x lane_select=%x type=%0x\n", 
                               __func__, pc->port, (void *)pc, ws->per_lane_control, ws->lane_select, type) ; 
    SOC_IF_ERROR_RETURN
    ...
    if(ws->verbosity>0) 
        printf("%s p=%0d pc=%p per_lane_control=%x lane_select=%x tmp_sel=%0x type=%0x\n", 
               __func__, pc->port, (void *)pc, ws->per_lane_control, ws->lane_select, tmpSelect, type) ;
    ...
    if(ws->verbosity>0) 
        printf("%s per_lane_control=%x lane_select=%x tmp_sel=%0x type=%0x\n", __func__, ws->per_lane_control, ws->lane_select, tmpSelect, type) ;
    SOC_IF_ERROR_RETURN (wcmod_tier1_selector("TX_AMP_CONTROL", ws, &rv));
    /* next we need to check if we need to write multiple core */
    if (pc->phy_mode == PHYCTRL_MULTI_CORE_PORT) {
        int num_core;
        wcmod_st    *ws_temp;
        int i = 0, temp_id;
        /* next check how many core we need to write */
        num_core = (SOC_INFO(unit).port_num_lanes[pc->port] + 3) / 4 - 1;
        for (i = 0; i < num_core; i++) {
             ws_temp   =  (wcmod_st *)( ws + 1 + i);
            temp_id = ws_temp->phy_ad;
            SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_temp));
            ws_temp->phy_ad = temp_id;
            ...
            SOC_IF_ERROR_RETURN
                (WRITE_WC40_AERBLK_AERr(unit,ws_temp,0));
        }
    }
                                ...
                                soc_phy_control_t type, uint32 value)
{
    ...
    wcmod_st    *ws, *temp_ws;
    WCMOD_DEV_DESC_t     *pDesc;
    int     lane_ctrl, core_num, max_core_num, lane_num, temp_id;
    int     shifter;
    uint16  mask;
    phy_ctrl_t *pc;
    ...
    SOC_IF_ERROR_RETURN (_wcmod_phy_parameter_copy(pc, temp_ws));  /* this call resets lane_select */
    temp_ws->phy_ad = temp_id;
    tmpLane  = temp_ws->this_lane;
    tmpSelect = temp_ws->lane_select;
    if( IS_DUAL_LANE_PORT(pc)) { 
    ...
    if(temp_ws->verbosity>0) 
        printf("%s p=%0d pc=%p per_lane_control=%x lane_select=%x tmp_sel=%0x type=%0x\n", 
               __func__, port, (void *)pc, temp_ws->per_lane_control, temp_ws->lane_select, tmpSelect, type) ;
    ...
    tmpLane = temp_ws->this_lane;
    temp_ws->this_lane = lane_num;
    SOC_IF_ERROR_RETURN (wcmod_tier1_selector("TX_AMP_CONTROL", temp_ws, &rv));
    SOC_IF_ERROR_RETURN
        (WRITE_WC40_AERBLK_AERr(unit,temp_ws,0));
...
_phy_wcmod_control_tx_lane_squelch_set(int unit, phy_ctrl_t *pc, uint32 value)
{
   int rv;
    ...
    SOC_IF_ERROR_RETURN (_wcmod_phy_parameter_copy(pc, ws));
    if (value) {
      ws->per_lane_control = (0x1 << (8 * ws->this_lane));
    } else {
      ws->per_lane_control = 0;
    }

    SOC_IF_ERROR_RETURN (wcmod_tier1_selector("TX_LANE_DISABLE", ws, &rv));
    return SOC_E_NONE;
}
...
_phy_wcmod_rx_DFE_tap1_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : DFE tap1 overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
    ...
    if ( enable == 0x8000) {
        ...
        ws->per_lane_control = enable | RX_UC_CONTROL;
    }
    SOC_IF_ERROR_RETURN
...
_phy_wcmod_rx_DFE_tap2_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : DFE tap2 overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
...
_phy_wcmod_rx_DFE_tap3_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : DFE tap3 overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
...
_phy_wcmod_rx_DFE_tap4_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
   ...
   SOC_IF_ERROR_RETURN
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : DFE tap4 overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
...
_phy_wcmod_rx_DFE_tap5_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 enable)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : DFE tap5 overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
...
_phy_wcmod_rx_vga_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 value)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : VGA overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
    ...
    if ( value == 0x8000) {
        ...
        ws->per_lane_control = value | RX_UC_CONTROL;
    }
    SOC_IF_ERROR_RETURN
...
_phy_wcmod_rx_pf_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 value)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : peak filter overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
    ...
    if ( value == 0x8000) {
        ...
        ws->per_lane_control = value | CHANGE_RX_PF_VALUES | RX_UC_CONTROL;
    }
    SOC_IF_ERROR_RETURN
...
_phy_wcmod_rx_low_freq_pf_ovr_control_set(int unit, phy_ctrl_t *pc, uint32 value)
{
    ...
    if (link == 0) {
        SOC_DEBUG_PRINT((DK_ERR,
                       "WCMOD : low_freq peak filter overwrite fails due to link down: u=%d p=%d\n",
                        unit, pc->port));
        return (SOC_E_FAIL);
    }
    ...
    if ( value == 0x8000) {
        ...
        ws->per_lane_control = value | CHANGE_RX_LOW_FREQ_PF_VALUES | RX_UC_CONTROL;
    }
    SOC_IF_ERROR_RETURN
...
#if 0
STATIC int 
_phy_wcmod_tx_pi_control_set(int unit, phy_ctrl_t *pc,
                                           soc_phy_control_t type,uint32 enable)
{
    int rv;
    wcmod_st    *ws;
...
_phy_wcmod_control_firmware_mode_set(int unit, phy_ctrl_t *pc, uint32 value)
{
    wcmod_st    *ws;
    WCMOD_DEV_DESC_t     *pDesc;
    int rv, per_lane_control = 0, size = 0, i = 0;
    int link = 0;
   ...
   void *arg)     /* command argument based on op_type/op_cmd */
{
    ...
    switch(op_cmd) {
        ...
        case PHY_DIAG_CTRL_EYE_MARGIN_HEYE_RIGHT:
            ...
            ws->ber       = *pData++;
            ws->tol       = *pData;
            ws->per_lane_control = 3 << (pc->lane_num * 8);
        ...
        case PHY_DIAG_CTRL_EYE_MARGIN_HEYE_LEFT:
            ...
            ws->ber       = *pData++;
            ws->tol       = *pData;
            ws->per_lane_control = 4 << (pc->lane_num * 8);
...
phy_wcmod_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if ((serdes_id0 & SERDESID_SERDESID0_MODEL_NUMBER_MASK) == MODEL_QSGMII) { 
        if (!SOC_IS_HELIX4(unit) && !SOC_IS_CALADAN3(unit) && !SOC_IS_HURRICANE2(unit)) {
            return SOC_E_NOT_FOUND;
        }
    }
    ...
    if (((serdes_id0 & SERDESID_SERDESID0_MODEL_NUMBER_MASK) == MODEL_QSGMII) && SOC_IS_CALADAN3(unit))  {
        /* one for QSGMII and the other is for warpCore */
        pc->size = sizeof(WCMOD_DEV_DESC_t) + sizeof(wcmod_st) * 2;
...
{
    int i, num_core, temp_id, rv = SOC_E_NONE;
    phy_ctrl_t *pc;
    wcmod_st   *ws, *temp_ws;
    WCMOD_DEV_DESC_t     *pDesc;
    WCMOD_DEV_CFG_t *pCfg;
...
{
    int i,num_core, myReturnValue, returnValue = 0;
    ...
    for (i = 0 ; i < num_core; i++) {
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_WC40_DIGITAL5_MISC6r(unit,ws_warpcore[i],
                  DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK, 
                  DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK));
        ...
        /* set the 0x8197 for the slave wake up for CD3 */
        if (SOC_IS_CALADAN3(unit)) {
            SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(unit, ws_warpcore[i], 0x8000));
        }
    }
    ...
    if (num_core > 3) {
        ...
        SOC_IF_ERROR_RETURN (WC40_REG_WRITE(unit, ws_mld[1], 0x0, 0x8011, 0x670));
    } else {
        ws_mld[0] =  (wcmod_st *)( ws_warpcore[0] + num_core);
        temp_id[num_core] =  ws_mld[0]->phy_ad;
        SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_mld[0]));
...
phy_wcmod_100g_plus_config(int unit, soc_port_t port, int speed)
{
    ...
    temp_id[0] = ws_master->phy_ad;
     ws_slave0   =  (wcmod_st *)( ws_master + 1);
    temp_id[1] = ws_slave0->phy_ad;
     ws_slave1   =  (wcmod_st *)( ws_slave0 + 1);
    temp_id[2] = ws_slave1->phy_ad;
     ws_mld      =  (wcmod_st *)( ws_slave1 + 1);
    temp_id[3] = ws_mld->phy_ad;

    /* set the 100G plus flag */
    ws_master->s100g_plus = 1;

    SOC_IF_ERROR_RETURN(_init_wcmod_st(ws_master));

    /* first init the ws structure */
    SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_master));

    SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_slave0));
    ws_slave0->phy_ad = temp_id[1];

    SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_slave1));
    ws_slave1->phy_ad = temp_id[2];

    SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws_mld));
    ws_mld->phy_ad = temp_id[3];
    ...
    /* set the 0x8197 for the slave wake up for CD3 */
    if (SOC_IS_CALADAN3(unit)) {
         SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(unit, ws_master, 0x8000));
    }
    ...
    /* set the 0x8197 for the slave wake up for CD3 */
    if (SOC_IS_CALADAN3(unit)) {
         SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(unit, ws_slave0, 0x8000));
    }
    ...
    /* set the 0x8197 for the slave wake up for CD3 */
    if (SOC_IS_CALADAN3(unit)) {
         SOC_IF_ERROR_RETURN(WRITE_WC40_XGXSBLK10_MISC_CONTROLr(unit, ws_slave1, 0x8000));
    }
    ...
    tmp_lane_select     = ws_master->lane_select;        /* save this */
    ws_master->lane_select = WCMOD_LANE_BCST; /* go to broadcast mode */
    SOC_IF_ERROR_RETURN(MODIFY_WC40_DIGITAL5_MISC6r(unit, ws_master, 0,
              DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK));
    ws_master->lane_select = tmp_lane_select;

    tmp_lane_select     = ws_slave0->lane_select;        /* save this */
    ws_slave0->lane_select = WCMOD_LANE_BCST; /* go to broadcast mode */
    SOC_IF_ERROR_RETURN(MODIFY_WC40_DIGITAL5_MISC6r(unit, ws_slave0, 0,
              DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK));
    ws_slave0->lane_select = tmp_lane_select;

    tmp_lane_select     = ws_slave1->lane_select;        /* save this */
    ws_slave1->lane_select = WCMOD_LANE_BCST; /* go to broadcast mode */
    SOC_IF_ERROR_RETURN(MODIFY_WC40_DIGITAL5_MISC6r(unit, ws_slave1, 0,
              DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK));
    ws_slave1->lane_select = tmp_lane_select;
...
_phy_wcmod_cfg_dump(int unit, soc_port_t port)
{
    ...
    soc_cm_print("speedMax     0x%04d\n", pc->speed_max);
    soc_cm_print("pc->flags    0x%04x\n", pc->flags);
    soc_cm_print("pc->stop     0x%04x\n", pc->stop);
    soc_cm_print("pi->phy_flags   0x%04x\n", pi->phy_flags);
    soc_cm_print("lane_select  0x%04x\n", ws->lane_select);
    soc_cm_print("spd_intf     0x%04x(%s)\n", ws->spd_intf,e2s_wcmod_spd_intfc_set[ws->spd_intf]); 
    soc_cm_print("spd_mode     0x%04x\n", speed_mode);
    soc_cm_print("higi scrm    0x%02x 0x%02x\n", pCfg->hg_mode, pCfg->scrambler_en) ;  
    soc_cm_print("phy_id       0x%04x\n", pc->phy_id);
    return SOC_E_NONE;
}
...
wcmod_uc_status_dump(int unit, soc_port_t port, soc_phy_wcmod_uc_desc_t* uc_desc /* size of array MAX_NUM_LANES,  may be NULL */)
{
    ...
    soc_phy_wcmod_uc_desc_t * pDesc;
    wcmod_st  *ws[6];      /* wcmod structure    */
    int i, num_core, temp_id[6], temp_monitor[6], core_num = 0, size;   
    int tmp_lane_select = 0 ;
    ...
    for (i = 0 ; i < num_core; i++) {
        ...
        temp_id[i] = ws[i]->phy_ad;
        SOC_IF_ERROR_RETURN(_wcmod_phy_parameter_copy(pc, ws[i]));
        ws[i]->phy_ad = temp_id[i];
    }
...
*/
    ...
    for (i = 0; i < size; i++) {
        ...
        if(ws[core_num]->verbosity>0)printf("%s this_lane=%0d\n", __func__, ws[core_num]->this_lane) ;
        ws[core_num]->diag_type = WCMOD_DIAG_TX_TAPS;
        SOC_IF_ERROR_RETURN
          (wcmod_tier1_selector("WCMOD_DIAG", ws[core_num], &rv));
        data16 = ws[core_num]->accData;
    ...
    if(uc_desc == NULL) {
        ...
        soc_cm_print("junction temperature: ");
        for (i = 0; i < size / 4 ; i++) {
           soc_cm_print("core%d %dc ",i, temp_monitor[i]);
        }
        soc_cm_print(" \n\n");

        soc_cm_print("PLL Range: %d\n\n",data16);

        soc_cm_print("LN SD PMD(FW_MODE) PPM  clk90_ofs |clkp1_of p1_lvl m1_lvl PF lowp |VGA BIAS DFE1 DFE2 "
                     "DFE3 |DFE4 DFE5 TX_DRVR |PREC MAIN POSTC MTRC PE   |ZE   ME  PO  ZO  MO |DSC_SM\n");
    
        for (i = 0; i < size; i++) {
            pDesc = &uc_desc_s[i];
            soc_cm_print("%02d %02d  %8s %04d %07d |%07d %04d %04d  %04d %04d |%04d %4s %04d %04d %04d |%04d "
                         "%04d 0x%04x |%04d %04d %04d %04d %04d |%04d %2d %3d %3d %2d |0x%4x\n",
               i,pDesc->sd,  pDesc->pmd_mode_s, pDesc->integ_reg,pDesc->clk90_offset, pDesc->clkp1_offset, pDesc->p1_lvl,
               pDesc->m1_lvl,pDesc->pf_ctrl,pDesc->lopf, pDesc->vga_sum,pDesc->vga_bias_reduced,
               pDesc->dfe1_bin,pDesc->dfe2_bin,pDesc->dfe3_bin,pDesc->dfe4_bin,
               pDesc->dfe5_bin,pDesc->tx_drvr, pDesc->tx_pre_cursor,pDesc->tx_main,pDesc->tx_post_cursor,
               pDesc->postc_metric,pDesc->offset_pe,pDesc->offset_ze,pDesc->offset_me,
               pDesc->offset_po,pDesc->offset_zo,pDesc->offset_mo, pDesc->dsc_sm);
        }
    }
...
_phy_wcmod_diag_dump(int unit, soc_port_t port, int lane, int diag_val)
{
    ...
    soc_cm_print("\n_phy_wcmod_diag_dump call lane=%0d diag_type=%x per_lane_ctl=%x\n\n", lane, diag_val, ws->per_lane_control) ;
    SOC_IF_ERROR_RETURN
      (wcmod_tier1_selector("WCMOD_DIAG", ws, &rv));
...
int
wcmod_eye_margin(int unit, soc_port_t port)
{
    phy_ctrl_t      *pc;
    wcmod_st            *ws;      /*wcmod structure */
    WCMOD_DEV_DESC_t    *pDesc;
