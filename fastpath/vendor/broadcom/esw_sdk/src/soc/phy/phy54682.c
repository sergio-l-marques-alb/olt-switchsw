/*
 * $Id: phy54682.c 1.78.16.1 Broadcom SDK $
...
_phy_54682_reset_setup(int unit, soc_port_t port)
{
    ...
    if (soc_phy_primary_and_offset_get(unit, port, &primary_port, &index) == SOC_E_NONE) {
        SOC_IF_ERROR_RETURN
            (phy_54682_control_set( unit, port, SOC_PHY_CONTROL_PORT_PRIMARY, primary_port));
        SOC_IF_ERROR_RETURN
            (phy_54682_control_set( unit, port, SOC_PHY_CONTROL_PORT_OFFSET, index ));
        pc->lane_num = index;
    } else {
        phy_addr = pc->phy_id;
        index = phy_addr & 7;
...
_phy_54682_no_reset_setup(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_phy_54682_reset_setup: u=%d p=%d medium=%s\n",
                     unit, port,
   ...
   /* SGMII interface to MAC */
     ...
     */
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54682_1000X_MII_CTRLr(unit, pc, 0, MII_CTRL_PD));
    ...
    /* Power down SerDes */
    SOC_IF_ERROR_RETURN
        (MODIFY_PHY54682_1000X_MII_CTRLr(unit, pc, MII_CTRL_PD, 
                                               MII_CTRL_PD));
        ...
        if (SOC_IS_CALADAN3(unit)) {
            ...
            SOC_IF_ERROR_RETURN   
               (PHY54682_REG_WRITE(unit, pc, SOC_PHY_REG_RESERVE_ACCESS,   
                                     0x0000, 0x1f, 0xffd0));   
            SOC_IF_ERROR_RETURN   
               (PHY54682_REG_WRITE(unit, pc, 0x0, 0x0000, 0x1e, 0x1f));   
            SOC_IF_ERROR_RETURN   
               (PHY54682_REG_WRITE(unit, pc, SOC_PHY_REG_RESERVE_ACCESS,   
                                     0x0000, 0x1f, 0x0));   
            SOC_IF_ERROR_RETURN
               (PHY54682_REG_WRITE(unit, pc, 0x0,   
                                     0x0000, 0x0, 0x0140));

            SOC_IF_ERROR_RETURN
              (PHY54682_REG_READ(unit, pc, 0x0, 0x0000, 0x1, &status));
            SOC_IF_ERROR_RETURN
              (PHY54682_REG_READ(unit, pc, 0x0, 0x0000, 0x1, &status)); /* latched */
        }
#endif     
...
phy_54682_init(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_54682_init: u=%d p=%d\n",
                     unit, port));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_54682_init: "
...
phy_54682_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (pc->copper.enable && (pc->automedium || PHY_COPPER_MODE(unit, port))) {
        ...
        SOC_DEBUG_PRINT((DK_PHY, "phy_54682_enable_set: "
    ...
    if (pc->fiber.enable && (pc->automedium || PHY_FIBER_MODE(unit, port))) {
        phy_ctrl_t  *int_pc;
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54682_1000X_MII_CTRLr(unit, pc, power, 
                                                   MII_CTRL_PD));
...
phy_54682_ability_remote_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if (PHY_COPPER_MODE(unit, port)) {
        ...
        if (data & MII_ANP_SGMII_MODE) { /* Link partner is SGMII master */
            if (data & MII_ANP_SGMII_FD) { /* Full Duplex */
...
phy_54682_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY_COPPER_MODE(unit, port)) {
        ...
        rv   = MODIFY_PHY54682_1000X_MII_CTRLr
...
{
    int rv;
    phy_ctrl_t *pc, *pc_port2, *pc_port6;
    uint16 data, freq;
    soc_port_t primary;
    ...
    case SOC_PHY_CONTROL_CLOCK_FREQUENCY:
        ...
        if ((primary == -1) || (SOC_PHY_INFO(unit, primary).chip_info == NULL)) {
            return SOC_E_FAIL;
        }
        pc_port2 = EXT_PHY_SW_STATE(unit, 
            SOC_PHY_INFO(unit, primary).chip_info->offset_to_port[1]);
        pc_port6 = EXT_PHY_SW_STATE(unit, 
            SOC_PHY_INFO(unit, primary).chip_info->offset_to_port[5]);
        if (!(pc_port2 && pc_port6)) {
            return SOC_E_FAIL;
        }
        switch (value) {
        case 125000:
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54682_EXP_SERDES_MULTIPLIER_CTRLr(unit, pc_port2, (freq << 9),
                (0x1 << 9)));
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY54682_EXP_SERDES_MULTIPLIER_CTRLr(unit, pc_port6, (freq << 9),
                (0x1 << 9)));
        break;
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_CLOCK_FREQUENCY:
        ...
        SOC_IF_ERROR_RETURN
            (READ_PHY54682_EXP_SERDES_MULTIPLIER_CTRLr(unit, pc_port2, &data));
        if ((data >> 9) & 0x1) {
            *value = 156250;
        } else {
            *value = 125000;
...
phy_54682_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FLAGS) {
        ...
        if (conf->flags & SOC_PORT_PHY_TIMESYNC_ENABLE) {
            ...
            SOC_IF_ERROR_RETURN
                (MODIFY_PHY54682_SGMII_SLAVEr(unit, pc, 0x02, 0x02));

            SOC_IF_ERROR_RETURN
                (MODIFY_PHY54682_EXT_SERDES_CTRLr(unit, pc, 0x00, 0x01));

            SOC_IF_ERROR_RETURN
                (PHY54682_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 1U<<0, 1U<<0)); /* exp reg f5 */
        } else {

            SOC_IF_ERROR_RETURN
                (PHY54682_REG_MODIFY(unit, pc, 0x00, 0x0FF5,0x15, 0, 1U<<0)); /* exp reg f5 */

        }

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x10, en_control_reg, 
                                       ((1U << (offset + 8)) | (1U << offset))));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x10, &en_control_reg));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x12, tx_capture_en_reg, 
                                       (1U << offset)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x13, rx_capture_en_reg, 
                                       (1U << (offset + 8))));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x4f, nse_nco_6, 
                                       (1U << 13)));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x34, rx_crc_control_reg));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x32, tx_control_reg));
                                          
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x33, rx_control_reg));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x47, nse_nco_2c, 
                                           (1U << 14)));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ITPID) {
        /* VLAN TAG register */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x35, conf->itpid));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID) {
        /* OUTER VLAN TAG register */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x36, conf->otpid));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
        /* INNER VLAN TAG register */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x52, conf->otpid2));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4b, conf->ts_divider & 0xfff));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc, 0, 0x6f, conf->rx_link_delay & 0xffff));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc, 0, 0x70, (conf->rx_link_delay >> 16) & 0xffff));
    }


    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ORIGINAL_TIMECODE) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x25, (uint16)(conf->original_timecode.nanoseconds & 0xffff)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x26, (uint16)((conf->original_timecode.nanoseconds >> 16) & 0xffff)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x27, (uint16)(COMPILER_64_LO(conf->original_timecode.seconds) & 0xffff)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x28, (uint16)((COMPILER_64_LO(conf->original_timecode.seconds) >> 16) & 0xffff)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x29, (uint16)(COMPILER_64_HI(conf->original_timecode.seconds) & 0xffff)));
    }
    mask = 0;
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
        ...
        mask |= (0xf << 2);
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x51, conf->framesync.length_threshold));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x50, conf->framesync.event_offset));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x48, conf->syncout.interval & 0xffff));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x49, 
                ((conf->syncout.pulse_1_length & 0x3) << 14) | ((conf->syncout.interval >> 16) & 0x3fff)));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4a, 
                ((conf->syncout.pulse_2_length & 0x1ff) << 7) | ((conf->syncout.pulse_1_length >> 2) & 0x7f)));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4e, 
                (uint16)(COMPILER_64_LO(conf->syncout.syncout_ts) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4d, 
                (uint16)((COMPILER_64_LO(conf->syncout.syncout_ts) >> 16) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4c, 
                (uint16)(COMPILER_64_HI(conf->syncout.syncout_ts) & 0xffff)));
    }

    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x4f, (gmode << 14) | (framesync_mode << 2) | (syncout_mode << 0),
                                       mask ));

    /* tx_timestamp_offset */
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x15 + offset, (uint16)(conf->tx_timestamp_offset & 0xffff)));

        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x63, 
                    ((conf->tx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)));
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x64, 
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
        /* rx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x1d + offset, (uint16)(conf->rx_timestamp_offset & 0xffff)));

        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x65, 
                    ((conf->rx_timestamp_offset & 0xf0000) >> ((4 - offset) << 2)), 0xf << (offset << 2)));
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x66, 
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_modify(unit, pc, 0, 0x11, value, mask)); 
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_modify(unit, pc, 0, 0x12, value << 8, mask << 8)); 

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
        /* Initial ref phase [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x38, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) & 0xffff)));

        /* Initial ref phase [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x39, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_ref_phase) >> 16) & 0xffff)));

        /*  Initial ref phase [47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3a, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_ref_phase) & 0xffff)));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
        /* Ref phase delta [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3b, (uint16)(conf->phy_1588_dpll_ref_phase_delta & 0xffff)));

        /* Ref phase delta [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3c, (uint16)((conf->phy_1588_dpll_ref_phase_delta >> 16) & 0xffff)));
    }
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x3d, (conf->phy_1588_dpll_k1 & 0xff)<<8 | (conf->phy_1588_dpll_k2 & 0xff), mask ));

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
        /* DPLL K3 */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3e, conf->phy_1588_dpll_k3 & 0xff));
    }
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x3f, (uint16)(COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) & 0xffff)));

        /* Initial loop filter[31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x40, (uint16)((COMPILER_64_LO(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));

        /*  Initial loop filter[47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x41, (uint16)(COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) & 0xffff)));

        /* Initial loop filter[63:48]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x42, (uint16)((COMPILER_64_HI(conf->phy_1588_dpll_loop_filter) >> 16) & 0xffff)));
    }
...
phy_54682_timesync_config_get(int unit, soc_port_t port, soc_port_phy_timesync_config_t *conf)
{
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4f, &nse_nco_6));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FLAGS) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x12, &tx_capture_en_reg)); 
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x34, &rx_crc_control_reg));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x32, &tx_control_reg));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x47, &nse_nco_2c));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_FRAMESYNC_MODE) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x51, &value));
        conf->framesync.length_threshold = value;
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x50, &value));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_SYNCOUT_MODE) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x48, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x49, &temp2));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4a, &temp3));
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4e, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4d, &temp2));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4c, &temp3));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ITPID) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x35, &conf->itpid));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x36, &conf->otpid));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_OTPID2) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x52, &conf->otpid2));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TS_DIVIDER) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4b, &conf->ts_divider));
    }

    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_LINK_DELAY) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc, 0, 0x6f, &temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc, 0, 0x70, &temp2));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_ORIGINAL_TIMECODE) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x25, &temp1));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x26, &temp2));

        conf->original_timecode.nanoseconds = ((uint32)temp2 << 16) | temp1; 

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x27, &temp1));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x28, &temp2));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x29, &temp3));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_TX_TIMESTAMP_OFFSET) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x15 + offset, &temp1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x63, &temp2));
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x64, &temp2));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_RX_TIMESTAMP_OFFSET) {
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x1d + offset, &temp1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x65, &temp2));
            temp32 = (temp2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x66, &temp2));
            temp32 = (temp2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
        }
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_read(unit, pc, 0, 0x11, &value)); 
    ...
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_read(unit, pc, 0, 0x12, &value)); 
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE) {
        /* Initial ref phase [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x38, &temp1));

        /* Initial ref phase [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x39, &temp2));

        /*  Initial ref phase [47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3a, &temp3));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_REF_PHASE_DELTA) {
        /* Ref phase delta [15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3b, &temp1));

        /* Ref phase delta [31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3c, &temp2));
    ...
    /* DPLL K1 & K2 */
    SOC_IF_ERROR_RETURN
        (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3d, &temp1 ));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_K3) {
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3e, &temp1));
    ...
    if (conf->validity_mask & SOC_PORT_PHY_TIMESYNC_VALID_PHY_1588_DPLL_LOOP_FILTER) {
        /* Initial loop filter[15:0] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x3f, &temp1));

        /* Initial loop filter[31:16]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x40, &temp2));

        /*  Initial loop filter[47:32] */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x41, &temp3));

        /* Initial loop filter[63:48]  */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x42, &temp4));
...
phy_54682_timesync_control_set(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_NCOADDEND:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x44, (uint16)COMPILER_64_LO(value)));
        break;

    case SOC_PORT_CONTROL_PHY_TIMESYNC_FRAMESYNC:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x4f, &temp1));
        temp2 = ((temp1 | (0x3 << 14) | (0x1 << 12)) & ~(0xf << 2)) | (0x1 << 5);
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp2));
        sal_udelay(1);
        temp2 &= ~((0x1 << 5) | (0x1 << 12));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp2));
        sal_udelay(1);
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x4f, temp1));

        break;

    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOCAL_TIME:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x45, (uint16)(COMPILER_64_LO(value) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x46, (uint16)((COMPILER_64_LO(value) >> 16) & 0xffff)));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_modify(unit, pc_port1, 0, 0x47, (uint16)(COMPILER_64_HI(value) & 0xfff), 0xfff));
        break;
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x2e, temp1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x2f, temp2));
        break;
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
        ...
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x30, temp1));
        break;
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_TX_TIMESTAMP_OFFSET:
        ...
        /* tx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x15 + offset, (uint16)(value0 & 0xffff)));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_RX_TIMESTAMP_OFFSET:
        ...
        /* rx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_write(unit, pc_port1, 0, 0x1d + offset, (uint16)(value0 & 0xffff)));
...
phy_54682_timesync_control_get(int unit, soc_port_t port, soc_port_control_phy_timesync_t type, uint64 *value)
{
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_HEARTBEAT_TIMESTAMP:
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x1));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x56, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x57, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x58, &value2));

        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x2));
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x0));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_CAPTURE_TIMESTAMP:
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x4));

        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x53, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x54, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x55, &value2));

        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x8));
        SOC_IF_ERROR_RETURN
            (WRITE_PHY54682_EXP_READ_START_END_CTRLr(unit, pc_port1, 0x0));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_NCOADDEND:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x44, &value0));
        /* *value = value0; */
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOCAL_TIME:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x45, &value0));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x46, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x47, &value2));
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_LOAD_CONTROL:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x2e, &value1));
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x2f, &value2));
        value0 = 0;
        if (value1 & (1U << 11)) {
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x31, &value1));
        value0 = 0;
        if (value1 & (1U << 1)) {
    ...
    case SOC_PORT_CONTROL_PHY_TIMESYNC_INTERRUPT_MASK:
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x30, &value1));
        value0 = 0;
        if (value1 & (1U << 1)) {
    ...
    /* tx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x15 + offset, &value1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x63, &value2));
            temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x64, &value2));
    ...
    /* rx_timestamp_offset */
        SOC_IF_ERROR_RETURN
            (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x1d + offset, &value1));
        if (offset < 4 ) {
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x65, &value2));
            temp32 = (value2 & (0xf << (offset << 2))) << ((4 - offset) << 2);
        } else {
            uint16 offset1 = offset - 4;
            SOC_IF_ERROR_RETURN
                (_phy_54682e_blk_top_lvl_reg_read(unit, pc_port1, 0, 0x66, &value2));
            temp32 = (value2 & (0xf << (offset1 << 2))) << ((4 - offset1) << 2);
        }
        /* *value = temp32 | value1; */
        COMPILER_64_SET((*value), 0, (uint32)(temp32 | value1));
                                 ...
                                 soc_phy_control_t type, uint32 value)
{
    ...
    soc_port_t primary_port;
    uint16 data, mask;
    uint16 phy_addr, qsgmii_phy_addr;
    ...
    phy_addr = PORT_TO_PHY_ADDR(unit, primary_port);
    qsgmii_phy_addr = phy_addr + 8;
    qsgmii_port = ((port - primary_port) < 4) ? 0 : 1;
    ...
    switch(type) {
        case SOC_PHY_CONTROL_PREEMPHASIS:
            pc->phy_id = qsgmii_phy_addr ;
            data = data << 12;
            mask = 0xf000;

            SOC_IF_ERROR_RETURN   
               (phy_reg_aer_modify(unit, pc, qsgmii_aer_reg, data, mask));
