/*
 * $Id: phy84740.c 1.83 Broadcom SDK $
...
phy_84740_link_get(int unit, soc_port_t port, int *link)
{
    ...
    if (!an) {
        ...
        if (PHY84740_IS_QUADRA(devid)) {
            SOC_IF_ERROR_RETURN
                (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc, PHY84740_SPEED_LINK_DETECT_STAT_REG ,
                                               &speed_val));
	    ...
	    if(PHY84740_IS_QUADRA(devid)) {
		   ...
		   (PHY84740_IS_QUADRA_BACKPLANE(devid))) {
	            post_tap2 = 0;
                    ...
                    SOC_IF_ERROR_RETURN
                    (READ_PHY84740_MMF_PMA_PMD_REG(unit, pc, 0xca08, &post_tap2));
		}
	        *value |= ((post_tap2 & PHY84740_PREEMPH_REG_POST_TAP2_MASK) >>
		            PHY84740_PREEMPH_REG_POST_TAP2_SHFT) <<
		          PHY84740_PREEMPH_CTRL_POST_TAP2_SHFT;
                SOC_IF_ERROR_RETURN
                    (READ_PHY84740_MMF_PMA_PMD_REG(unit, pc, 0xca06, &pre_tap));
         ...
         int tx)   /* tx or rx */
{
    ...
    if (PHY84740_SINGLE_PORT_MODE(pc)) {
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_PHY84740_MMF_AN_REG(unit, pc, 0x8019, poly_ctrl, 0x0003)); 
	} else {
            /* 001 : prbs7 
            010 : prbs9
            011 : prbs11
...
_phy_84740_control_prbs_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (PHY84740_SINGLE_PORT_MODE(pc)) {
        ...
        if(PHY84740_IS_QUADRA(devid) && (speed_val & (1 << 2))) {
            ...
            if(enable) {
                SOC_IF_ERROR_RETURN
                (MODIFY_PHY84740_MMF_AN_REG(unit, pc, 0x8019, 0x0008, 0x0008));
            } else {
                SOC_IF_ERROR_RETURN
                (MODIFY_PHY84740_MMF_AN_REG(unit, pc, 0x8019, 0x0000, 0x0008));
            }
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_TX_LANE_SQUELCH: 
        if (PHY84740_IS_QUADRA(devid)) { /* Quadra */
            rv = _phy_84740_squelch_enable(unit,port,value);
	} else {
	    rv = SOC_E_UNAVAIL;
	}
    break;
...
STATIC int 
phy_84740_tx_polarity_flip_set(int unit,int port,uint32 value)
{
    int            lane;
    phy_ctrl_t     *pc;
    uint16 tx_pol;
    ...
    if ((PHY84740_SINGLE_PORT_MODE(pc))) {
        ...
        SOC_IF_ERROR_RETURN(
            MODIFY_PHY84740_MMF_PMA_PMD_REG(unit,pc, PHY84740_DIG_CTRL_REG,
                                 data16,mask16));
        SOC_IF_ERROR_RETURN
           (READ_PHY84740_MMF_PMA_PMD_REG(unit,pc,PHY84740_DIG_CTRL_REG,&data16));
     }
...
STATIC int 
phy_84740_rx_polarity_flip_set(int unit,int port,uint32 value)
{
    int            lane;
    phy_ctrl_t     *pc;
    uint16 rx_pol;
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PRE_DRIVER_CURRENT_LANE2:
        /* fall through */
    case SOC_PHY_CONTROL_PRE_DRIVER_CURRENT_LANE3:
        rv = _phy_84740_control_tx_driver_get(unit, port, intf, type, value);
        break;
    case SOC_PHY_CONTROL_EDC_MODE:
