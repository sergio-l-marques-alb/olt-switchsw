...
/*
 * $Id: phy8072.c 1.29.6.2 Broadcom SDK $
...
_8073_A0_xaui_check(int unit, int port)
{
    ...
    soc_cm_debug(DK_PHY, "xaui w/a in progress \n");
...
phy_8072_init(int unit, soc_port_t port)
{
    ...
    if (phy_ext_rom_boot) {
        ...
        if (PMDA_ROM_LOAD_8072ID == (data16 & 0xFFF0) || 
            ...
            soc_cm_print("8072: u=%d p=%d external rom boot complete f/w ver. 0x%x: init.\n",
                unit, port,data16);
    ...
    soc_cm_debug(DK_PHY, "8072: u=%d port%d: init.\n", unit, port);
...
phy_8072_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    soc_cm_debug(DK_PHY,"phy_8072_speed_set: u=%d p=%d speed=%d\n",
                 unit, port,speed);
...
phy_8072_rom_wait(int unit, int port)
{
    ...
    if (!(rd_data & 0x0100)) {
        soc_cm_debug(DK_ERR, "phy 8072 : u = %d p = %d timeout 1\n",
                     unit, port);
    ...
    while (SPI_READY == 1) {
        ...
        if (!(rd_data & 0x0100)) {
            soc_cm_debug(DK_ERR, "phy 8072 : u = %d p = %d timeout 2\n",
                         unit, port);
            SOC_IF_ERROR_RETURN(SOC_E_TIMEOUT);
        }
        /*
...
phy_8072_firmware_set(int unit, int port, int offset, uint8 *array,int datalen)
{
    ...
    soc_cm_debug(DK_PHY, "phy_8072_rom_program: u=%d p=%d done\n", unit, port);
