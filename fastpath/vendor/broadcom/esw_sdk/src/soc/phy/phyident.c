/*
 * $Id: phyident.c 1.448.8.1 Broadcom SDK $
...
static soc_phy_table_t _standard_phy_table[] = {
    ...
    {_chk_phy, _phy_id_BCM53020, "BCM53020", &phy_54680drv_ge, NULL},
                          ...
                          uint16 *phy_addr, uint16 *phy_addr_int)
{
    ...
    static const uint16 _soc_hurricane2_gphy_addr[] = {
        0x00, /* Port  0 (cmic)         N/A */
        0x00, /* Port  1 (N/A)                */
        0x89, /* Port  2 (QGPHY_0)    IntBus=0 Addr=0x09 */
        0x88, /* Port  3 (QGPHY_0)    IntBus=0 Addr=0x08 */
        0x87, /* Port  4 (QGPHY_0)    IntBus=0 Addr=0x07 */
        0x86, /* Port  5 (QGPHY_0)    IntBus=0 Addr=0x06 */
        0x84, /* Port  6 (QGPHY_1)    IntBus=0 Addr=0x04 */
        0x83, /* Port  7 (QGPHY_1)    IntBus=0 Addr=0x03 */
        0x82, /* Port  8 (QGPHY_1)    IntBus=0 Addr=0x02 */
        0x81, /* Port  9 (QGPHY_1)    IntBus=0 Addr=0x01 */
    ...
    *phy_addr_int = _soc_hurricane2_int_phy_addr[port];
    switch (dev_id) {
        ...
        *phy_addr = _soc_phy_addr_bcm56150[port];
        break;
    }
    if ( soc_feature(unit, soc_feature_gphy) && (port < 18)) {
        *phy_addr_int = 0;
        *phy_addr = _soc_hurricane2_gphy_addr[port];
    }
}
...
_dpp_phy_addr_multi_get(int unit, soc_port_t port, int is_logical, int array_max,
                       int *array_size, uint16 *mdio_addr)
{
    ...
    soc_info_t *si;
    int phy_port, required_mdio_addr;

    *array_size = 0;

    si = &SOC_INFO(unit);
    if (soc_feature(unit, soc_feature_logical_port_num) && is_logical) {
    ...
    * 0-32 NIF serdes lanes (WC0-7) 
    * 40-75 Fabric serdes (FMAC) 
    */ 
    if (SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), port)) {

        /*verify*/
        required_mdio_addr = si->port_num_lanes[port] / 4; /*amount of wcores to be used*/
        required_mdio_addr = (required_mdio_addr<2 ? 2 : required_mdio_addr); /*minimum wcores is 2*/
        required_mdio_addr++; /*add MLD*/
        if(si->port_num_lanes[port]>12) {
            required_mdio_addr++; /*add second MLD*/
        }
        if(array_max < required_mdio_addr) {
            if(1 == array_max) {
                mdio_addr[0] = _soc_phy_addr_arad[phy_port];
                *array_size = 1;
                return SOC_E_NONE;
            } else {
                return SOC_E_PARAM;
        ...
        *array_size = 1;
        ...
        /*Add wcores according to port_num_lanes*/
        if(si->port_num_lanes[port]>4) {
            mdio_addr[*array_size] = _soc_phy_addr_arad[phy_port+4];
            (*array_size)++;
        }
        if(si->port_num_lanes[port]>8) {
            mdio_addr[*array_size] = _soc_phy_addr_arad[phy_port+8];
            (*array_size)++;
        }
        if(si->port_num_lanes[port]>12) {
            mdio_addr[*array_size] = _soc_phy_addr_arad[25];
            (*array_size)++;
            if(si->port_num_lanes[port]>16) {
                mdio_addr[*array_size] = _soc_phy_addr_arad[21];
                (*array_size)++;
                if(si->port_num_lanes[port]>20) {
                    mdio_addr[*array_size] = _soc_phy_addr_arad[17];
                    (*array_size)++;
                }
            }
        }
        ...
        /*ILKN with two MLD*/
        if(si->port_num_lanes[port]>12) {
            mdio_addr[*array_size] = _soc_phy_addr_arad[33];
            (*array_size)++;
            mdio_addr[*array_size] = _soc_phy_addr_arad[34];
            (*array_size)++;
        } else {
            if(phy_port<=12) {
                mdio_addr[*array_size] = _soc_phy_addr_arad[33];
        ...
        /* Internal MDIO address add bit 7 */
        if(array_max >= 1) {
           mdio_addr[0] = _soc_phy_addr_arad[phy_port];
           *array_size = 1;
        }
        if(array_max >= 2) {
           mdio_addr[1] = _soc_phy_addr_arad[phy_port+4];
           *array_size = 2;
        }
        if(array_max >= 3) {
           mdio_addr[2] = _soc_phy_addr_arad[phy_port+8];
           *array_size = 3;
        }
        if(array_max >= 4) {
           mdio_addr[3] =
                (0 == SOC_BLOCK_NUMBER(unit,
                                       SOC_PORT_BLOCK(unit, phy_port))) ?
                _soc_phy_addr_arad[33] :  _soc_phy_addr_arad[34];
           *array_size = 4;
        }   
    }
                          ...
                          uint16 *phy_addr, uint16 *phy_addr_int)
{
    ...
    *phy_addr = 0;
    rv = _dpp_phy_addr_multi_get(unit, port, 1, 1, &array_size, phy_addr_int);
                         ...
                         uint16 *phy_addr, uint16 *phy_addr_int)
{
    static const uint16 _soc_bcm88030_int_phy_addr[] = {
        ...
        0x86, /*    6 (QSGMII0_1) IntBus=0 Addr=0x06 */
        0x87, /*    7 (QSGMII0_2) IntBus=0 Addr=0x07 */
        0x88, /*    8 (QSGMII0_3) IntBus=0 Addr=0x08 */
        0x89, /*    9 (QSGMII0_4) IntBus=0 Addr=0x09 */
        0x8A, /*   10 (QSGMII0_5) IntBus=0 Addr=0x0A */
        0x8B, /*   11 (QSGMII0_6) IntBus=0 Addr=0x0B */
        0x8C, /*   12 (QSGMII0_7) IntBus=0 Addr=0x0C */
        0x8D, /*   13 (QSGMII1_0) IntBus=0 Addr=0x0D */
        0x8E, /*   14 (QSGMII1_1) IntBus=0 Addr=0x0E */
        0x8F, /*   15 (QSGMII1_2) IntBus=0 Addr=0x0F */
        0x90, /*   16 (QSGMII1_3) IntBus=0 Addr=0x10 */
        0x91, /*   17 (QSGMII1_4) IntBus=0 Addr=0x11 */
        0x92, /*   18 (QSGMII1_5) IntBus=0 Addr=0x12 */
        0x93, /*   19 (QSGMII1_6) IntBus=0 Addr=0x13 */
        0x94, /*   20 (QSGMII1_7) IntBus=0 Addr=0x14 */
        0x95, /*   21 (WC3) IntBus=0 Addr=0x15 */
        0x95, /*   22 (WC3) IntBus=0 Addr=0x16 */
        0x95, /*   23 (WC3) IntBus=0 Addr=0x17 */
        0x95, /*   24 (WC3) IntBus=0 Addr=0x18 */
        ...
        0xA6, /*   34 (QSGMII2_1) IntBus=1 Addr=0x06 */
        0xA7, /*   35 (QSGMII2_2) IntBus=1 Addr=0x07 */
        0xA8, /*   36 (QSGMII2_3) IntBus=1 Addr=0x08 */
        0xA9, /*   37 (QSGMII2_4) IntBus=1 Addr=0x09 */
        0xAA, /*   38 (QSGMII2_5) IntBus=1 Addr=0x0A */
        0xAB, /*   39 (QSGMII2_6) IntBus=1 Addr=0x0B */
        0xAC, /*   40 (QSGMII2_7) IntBus=1 Addr=0x0C */
        0xAD, /*   41 (QSGMII3_0) IntBus=1 Addr=0x0D */
        0xAE, /*   42 (QSGMII3_1) IntBus=1 Addr=0x0E */
        0xAF, /*   43 (QSGMII3_2) IntBus=1 Addr=0x0F */
        0xB0, /*   44 (QSGMII3_3) IntBus=1 Addr=0x10 */
        0xB1, /*   45 (QSGMII3_4) IntBus=1 Addr=0x11 */
        0xB2, /*   46 (QSGMII3_5) IntBus=1 Addr=0x12 */
        0xB3, /*   47 (QSGMII3_6) IntBus=1 Addr=0x13 */
        0xB4, /*   48 (QSGMII3_7) IntBus=1 Addr=0x14 */
        0xB5, /*   49 (WC5) IntBus=1 Addr=0x15 */
        0xB5, /*   50 (WC5) IntBus=1 Addr=0x16 */
        0xB5, /*   51 (WC5) IntBus=1 Addr=0x17 */
        0xB5, /*   52 (WC5) IntBus=1 Addr=0x18 */
        ...
        0xC6, /*   58 (QSGMII4_1) IntBus=2 Addr=0x06 */
        0xC7, /*   59 (QSGMII4_2) IntBus=2 Addr=0x07 */
        0xC8, /*   60 (QSGMII4_3) IntBus=2 Addr=0x08 */
        0xC9, /*   61 (QSGMII4_4) IntBus=2 Addr=0x09 */
        0xCA, /*   62 (QSGMII4_5) IntBus=2 Addr=0x0A */
        0xCB, /*   63 (QSGMII4_6) IntBus=2 Addr=0x0B */
        0xCC, /*   64 (QSGMII4_7) IntBus=2 Addr=0x0C */
        0xCD, /*   65 (QSGMII5_0) IntBus=2 Addr=0x0D */
        0xCE, /*   66 (QSGMII5_1) IntBus=2 Addr=0x0E */
        0xCF, /*   67 (QSGMII5_2) IntBus=2 Addr=0x0F */
        0xD0, /*   68 (QSGMII5_3) IntBus=2 Addr=0x10 */
        0xD1, /*   69 (QSGMII5_4) IntBus=2 Addr=0x11 */
        0xD2, /*   70 (QSGMII5_5) IntBus=2 Addr=0x12 */
        0xD3, /*   71 (QSGMII5_6) IntBus=2 Addr=0x13 */
        0xD4, /*   72 (QSGMII5_7) IntBus=2 Addr=0x14 */
        0xD5, /*   73 (MLD0) IntBus=2 Addr=0x15 */
        0xD6, /*   74 (MLD1) IntBus=2 Addr=0x16 */
        0xD7, /*   75 (XC0) IntBus=2 Addr=0x17 */
        0xD7, /*   76 (XC0) IntBus=2 Addr=0x18 */
...
_katana2_phy_addr_default(int unit, int port,
                          uint16 *phy_addr, uint16 *phy_addr_int)
{
    static const uint16 _katana2_phy_int_addr_wc[] = { /* BUS-1 */
        0x21, 0x22, 0x23, 0x27, 0x28,
        0x29, 0x2a, 0x24, 0x25, 0x26,
        0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30 /* Ports 35-40 is Unconfirmed */
    };
    ...
    if(port < 25) {
        ...
        *phy_addr_int = (0x80 | _katana2_phy_int_addr_wc[port-25]);
        *phy_addr = _katana2_phy_int_addr_wc[port-25];
    }
}
               ...
               soc_phy_info_t *pi, phy_ctrl_t *ext_pc)
{
    ...
    if (SOC_IS_ARAD(unit) && !cl45_override) {
                       ...
                       int *array_size, uint16 *mdio_addr)
{
    ...
    if ((si->port_num_lanes[port] >= 10) || (si->port_type[port] == SOC_BLK_IL)) {
        ...
        mdio_addr[idx++] = port_phy_addr[unit][port].int_addr;
        _bcm88030_wc_phy_addr(unit, port, &mdio_addr[idx++]);
        mdio_addr[idx++] = (port % 16) >> 3;
    } else {
        mdio_addr[idx++] = port_phy_addr[unit][port].int_addr;
    }
    *array_size = idx;
                       ...
                       int *array_size, uint16 *mdio_addr)
{
    ...
    if ((0 >= array_max) || (NULL == array_size) || (NULL == mdio_addr)) {
        return SOC_E_PARAM;
    }

#ifdef BCM_PETRA_SUPPORT
    if(SOC_IS_DPP(unit)) {
        return _dpp_phy_addr_multi_get(unit, port, 1, array_max, array_size, mdio_addr);
    }
#endif
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit)) {
        return soc_sbx_caladan3_phy_addr_multi_get(unit, port, array_max, array_size, mdio_addr);
    }
#endif
    ...
    if (IS_CL_PORT(unit, port)) {
        if (SOC_IS_TRIUMPH3(unit)) {
            if (array_max > 1) {
                mdio_addr[addr_num] = port_phy_addr[unit][port].int_addr;
                addr_num++;
            }
            if (array_max > 2) {
                mdio_addr[addr_num] = mdio_addr[addr_num - 1] + 4;
                addr_num++;
            }
            if (array_max > 3) {
                mdio_addr[addr_num] = mdio_addr[addr_num - 1] + 4;
            ...
            if (array_max > 4) {
                mdio_addr[addr_num] =
                    (0 == SOC_BLOCK_NUMBER(unit, SOC_PORT_BLOCK(unit, 
                          SOC_INFO(unit).port_l2p_mapping[port]))) ? 0xdd : 0xde;
                addr_num++;
            }
            *array_size = addr_num;
        } else {
