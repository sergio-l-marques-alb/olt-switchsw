/*
 * $Id: drv.c 1.68.2.10 Broadcom SDK $
...
soc_wc_xgxs_reset(int unit, soc_port_t port, int reg_idx)
{
    ...
    lcpll = soc_property_port_get(unit, port, spn_XGXS_LCPLL,
                                  SAL_BOOT_QUICKTURN ? 0 : 1);
    SOC_IF_ERROR_RETURN(READ_XLPORT_XGXS_CTRL_REGr(unit, port, &rval));
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, LCREF_ENf,
                      lcpll ? 1 : 0);
    ...
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, IDDQf, 0);
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, PWRDWNf, 0);
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, PWRDWN_PLLf, 0);
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, RSTB_HWf, 1);
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_XGXS_CTRL_REGr(unit, port, rval));
    sal_usleep(reset_sleep_usec);
    ...
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, TXD1G_FIFO_RSTBf, 0);
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, TXD10G_FIFO_RSTBf,
                      0);
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_XGXS_CTRL_REGr(unit, port, rval));
    sal_usleep(reset_sleep_usec);
    ...
    /* Bring MDIO registers out of reset */
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, RSTB_MDIOREGSf, 1);
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_XGXS_CTRL_REGr(unit, port, rval));

    /* Activate all clocks */
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, RSTB_PLLf, 1);
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_XGXS_CTRL_REGr(unit, port, rval));

    /* Bring Tx FIFO out of reset */
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, TXD1G_FIFO_RSTBf,
                      0xf);
    soc_reg_field_set(unit, XLPORT_XGXS_CTRL_REGr, &rval, TXD10G_FIFO_RSTBf,
                      1);
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_XGXS_CTRL_REGr(unit, port, rval));
...
soc_wc_xgxs_pll_check(int unit, soc_port_t port, int reg_idx)
{
    ...
    while (retry > 0) {
        SOC_IF_ERROR_RETURN
            (READ_XLPORT_XGXS_STATUS_GEN_REGr(unit, port, &rval));
        if (soc_reg_field_get(unit, XLPORT_XGXS_STATUS_GEN_REGr, rval,
                              TXPLL_LOCKf)) {
            return SOC_E_NONE;
        }
