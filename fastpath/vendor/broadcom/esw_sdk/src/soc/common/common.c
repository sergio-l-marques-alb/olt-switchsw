/*
 * $Id: common.c 1.60.6.1 Broadcom SDK $
    ...
    case soc_genreg:
        if (soc_feature(unit, soc_feature_new_sbus_format)) {
            if (ainfo->valid && (int)ainfo->reg >= 0 &&
                ...
                rv = soc_reg_get(unit, ainfo->reg, port, 
                                 ainfo->idx, data);
            } else {
                rv = soc_reg32_get(unit, ainfo->reg, port,
                                   ainfo->idx, &tmpdata);
                COMPILER_64_SET(*data, 0, tmpdata);
            }
            break;
        ...
        case soc_portreg:
            PBMP_ALL_ITER(unit, port) {
                if (SOC_BLOCK_IN_LIST(regblktype, SOC_BLK_PORT) &&
                    !SOC_BLOCK_IS(regblktype, SOC_PORT_TYPE(unit, port))) {
                    continue;
                }
