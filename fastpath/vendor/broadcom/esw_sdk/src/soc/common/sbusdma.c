/*
 * $Id: sbusdma.c 1.97 Broadcom SDK $
...
{
    return _soc_mem_array_sbusdma_read(unit, mem, 0, copyno, index_min, index_max, buffer);
}
                            ...
                            int copyno, int index_min, int index_max, void *buffer)
{
    ...
    soc_cm_debug(DK_SOCMEM | DK_DMA, "_soc_mem_sbusdma_read: unit %d"
                 " mem %s.%s[%u] index %d-%d buffer %p\n",
                 unit, SOC_MEM_UFNAME(unit, mem), SOC_BLOCK_NAME(unit, copyno),
                 array_index, index_min, index_max, buffer);
    ...
    if (SOC_IS_CALADAN3(unit)) {
        soc_sbx_caladan3_sbusdma_cmc_ch_map(unit, mem, SOC_SBUSDMA_TYPE_TDMA, &cmc, &ch);
    }
#endif
    ...
    if (soc->tableDmaIntrEnb) {
        ...
        if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
            ...
            if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                                  rval, ERRORf)) {
                rv = SOC_E_FAIL;
                soc_cm_debug(DK_ERR+DK_VERBOSE, "STATUS: 0x%08x\n", rval);
                rval = soc_pci_read(unit, CMIC_CMCx_SBUSDMA_CHy_CUR_SBUSDMA_CONFIG_OPCODE(cmc, ch));
                soc_cm_debug(DK_ERR+DK_VERBOSE, "OPCODE: 0x%08x\n", rval);
                rval = soc_pci_read(unit, CMIC_CMCx_SBUSDMA_CHy_CUR_SBUSDMA_CONFIG_ADDR(cmc, ch));
                soc_cm_debug(DK_ERR+DK_VERBOSE, "START ADDR: 0x%08x\n", rval);
                rval = soc_pci_read(unit, CMIC_CMCx_SBUSDMA_CHy_CUR_SBUSADDR(cmc, ch));
                soc_cm_debug(DK_ERR+DK_VERBOSE, "CUR ADDR: 0x%08x\n", rval);
            }
        }
    } else {
        soc_timeout_t to;
                             ...
                             int clear_buf_ent)
{
    ...
    for (array_index = array_index_start; array_index <= array_index_end; ++array_index) {
        ...
        if (rv < 0) {
            if (rv != SOC_E_TIMEOUT) {
                soc_cm_debug(DK_ERR, "%s: %s[%u].%s failed(NAK)\n",
...
_soc_sbusdma_desc(void *unit_vp)
{
    ...
    while((interval = SOC_SBUSDMA_DM_TO(unit)) != 0) {
        ...
        if (rv != SOC_E_NONE) {
            if (rv != SOC_E_TIMEOUT) {
                soc_cm_debug(DK_ERR, "%s: %s failed(NAK)\n",
...
soc_sbusdma_desc_detach(int unit)
{
    ...
    for (i=1; i<SOC_SBUSDMA_MAX_DESC; i++) {
        if (SOC_SBUSDMA_DM_HANDLES(unit)[i]) {
            swd = (_soc_sbusdma_state_t *)INT_TO_PTR(SOC_SBUSDMA_DM_HANDLES(unit)[i]);
            sal_free(swd->cfg);
                        ...
                        sbusdma_desc_handle_t *desc_handle)
{
    ...
    *desc_handle = i; 
    ...
    SOC_SBUSDMA_DM_HANDLES(unit)[i] = PTR_TO_INT(swd);
    SOC_SBUSDMA_DM_COUNT(unit)++;
    SOC_SBUSDMA_DM_UNLOCK(unit);
    if (ctrl->cfg_count == 1) {
...
soc_sbusdma_desc_get_state(int unit, sbusdma_desc_handle_t desc_handle, uint8 *state)
{
    ...
    if ((desc_handle > 0) && (desc_handle <= SOC_SBUSDMA_MAX_DESC) 
        ...
        swd = (_soc_sbusdma_state_t*)INT_TO_PTR(SOC_SBUSDMA_DM_HANDLES(unit)[desc_handle]);
    } else {
        SOC_SBUSDMA_DM_UNLOCK(unit);
...
soc_sbusdma_desc_delete(int unit, sbusdma_desc_handle_t handle)
{
    ...
    if ((handle > 0) && (handle <= SOC_SBUSDMA_MAX_DESC) 
        ...
        swd = (_soc_sbusdma_state_t*)INT_TO_PTR(SOC_SBUSDMA_DM_HANDLES(unit)[handle]);
