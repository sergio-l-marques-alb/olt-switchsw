/*
 * $Id: ism.c 1.66.2.4 Broadcom SDK $
...
soc_ism_log_to_phy_set(int unit)
{
    ...
    for (tab = 0; tab < _SOC_ISM_MAX_TABLES; tab++) {
        soc_cm_debug(DK_SOCMEM, "Table: %d\n", tab);
        for (bank = 0; bank < SOC_ISM_INFO(unit)->total_banks; bank++) {
            soc_cm_debug(DK_SOCMEM, "[%d]-%d ", bank, 
...
soc_ism_table_bank_set(int unit)
{
    ...
    for (tab = 0; tab < _SOC_ISM_MAX_TABLES; tab++) {
        ...
        for (stg = 0; stg < _SOC_ISM_MAX_STAGES; stg++) {
            ...
            soc_cm_debug(DK_SOCMEM, "stage: %d - bmask: %x ", stg, val);
...
soc_ism_mem_config(int unit, soc_ism_mem_size_config_t *mem_cfg, int count)
{
    ...
    for (tab = 0; tab < count && tab < SOC_ISM_MEM_TOTAL; tab++) {
        soc_cm_debug(DK_VERBOSE, "ISM mem: [%s], size: %d\n", 
    ...
    for (tab = 0; tab < count && tab < SOC_ISM_MEM_TOTAL; tab++) {
        ...
        if (rem) {
            ...
            soc_cm_debug(DK_VERBOSE, "Updated size %s: %d\n", 
        ...
        do {
            ...
            if (inc) {
                inc = inc / mem_info_entry[midx].epb;
                _ism_total += inc * _SOC_ISM_ENTRIES_PER_BKT;
                inc = 0;
            } else {
                _ism_total += size * _SOC_ISM_ENTRIES_PER_BKT;
            }
            avail = 0;
            for (idx = 0; idx < SOC_ISM_INFO(unit)->max_banks; idx++) {
                ...
                if (size >= SOC_ISM_INFO(unit)->bank_raw_sizes[bank]) {
                    ...
                    soc_cm_debug(DK_VERBOSE, "bank: %d\n", bank);
            ...
            if (size) {
                ...
                inc = mem_info_entry[midx].epb * _SOC_ISM_ENTRY_SIZE_QUANTA;
        ...
        if (size) {
            soc_cm_debug(DK_ERR, "Could not allocate banks for mem: %s\n",
                         soc_ism_table_to_name(mem));
            return SOC_E_PARAM;
        }
next_mem:
        size = ism_hash_tables[tab].size;
        soc_cm_debug(DK_VERBOSE, "Final size %s: %d\n", 
                     soc_ism_table_to_name(mem), size);
        switch (mem) {
    ...
    soc_cm_debug(DK_VERBOSE, "ISM used total: %d k of %d k\n", 
                 _ism_total/_SOC_ISM_ENTRY_SIZE_QUANTA, 
                 SOC_ISM_INFO(unit)->total_entries/_SOC_ISM_ENTRY_SIZE_QUANTA);
    soc_cm_debug(DK_VERBOSE, "ISM remaining : %d k\n", 
                 SOC_ISM_INFO(unit)->total_entries/_SOC_ISM_ENTRY_SIZE_QUANTA - 
                 (_ism_total/_SOC_ISM_ENTRY_SIZE_QUANTA));

    if (_ism_total/_SOC_ISM_ENTRY_SIZE_QUANTA > 
        SOC_ISM_INFO(unit)->total_entries/_SOC_ISM_ENTRY_SIZE_QUANTA) {
        soc_cm_debug(DK_ERR, "Over-allocation of ISM resources !!\n");
        return SOC_E_PARAM;
    }
    ...
    soc_cm_debug(DK_VERBOSE, "ISM configured.\n");
    return SOC_E_NONE;
}
...
soc_ism_hash_init(int unit)
{
    ...
    soc_cm_debug(DK_SOCMEM, "Used banks: %d\n", count);
    for (i = 0; i < count; i++) {
        ...
        soc_cm_debug(DK_SOCMEM, "Bank: %d, base: %d, mem: [%s], buckets: %d, entries: %d "
                     "hash offset: %d\n", banks[i], 
                     _SOC_ISM_BANKS(unit)[i].base_entry, 
    ...
    /* Just for sanity */
    for (i = 0; i < SOC_MEM_SET_MAX; i++) {
        soc_cm_debug(DK_SOCMEM, "Set: %s, num mems: %d, banks: %d, zero_lsb: %d, \n", 
...
{
    int i, r, f, stg, bank, len, rv = SOC_E_NONE;
    uint32 pd_val, rval;
    _soc_ism_pd_t *pd;
    ...
    if ((SOC_ISM_INFO(unit)->ism_mode == _ISM_SIZE_MODE_96) ||
        (SOC_ISM_INFO(unit)->ism_mode == _ISM_SIZE_MODE_80) ||
        (SOC_ISM_INFO(unit)->ism_mode == _ISM_SIZE_MODE_64)) {
        return SOC_E_NONE;
    }
    /* Power down unused banks */
    for (i = 0; i < SOC_ISM_INFO(unit)->max_banks; i++) {
        if (!_soc_ism_bank_avail[unit][i]) {
            stg = i / SOC_ISM_INFO(unit)->banks_per_stage;
            bank = i % SOC_ISM_INFO(unit)->banks_per_stage;
            soc_cm_debug(DK_VERBOSE, "Powering down stage[%d] bank[%d]\n",
                         stg, bank);
            ...
            stg = i / SOC_ISM_INFO(unit)->banks_per_stage;
            bank = i % SOC_ISM_INFO(unit)->banks_per_stage;
            soc_cm_debug(DK_VERBOSE, "Powering down hit-bit rams for stage[%d] bank[%d]\n",
                         stg, bank);
            pd = &_soc_ism_pd[stg][bank];
            r = 0;
...
soc_ism_hash_offset_config(int unit, uint8 bank, uint8 offset)
{
    ...
    b = bank%SOC_ISM_INFO(unit)->banks_per_stage;
    ...
    if (!found) {
        soc_cm_debug(DK_ERR, "No memory mapped to bank: %d\n", bank);
                         ...
                         soc_field_t *keyf, soc_field_t *lsbf, uint8 *num_flds)
{
    ...
    if (i < 0) {
        soc_cm_debug(DK_ERR, "Invalid hash memory !!\n");
        return SOC_E_PARAM;
    }
    found = 0;
    for (j = 0; j < _SOC_ISM_MEMS(unit)[i].shms->num_keys; j++) {
        if (key_type == _SOC_ISM_MEMS(unit)[i].shms->shk[j].key_type) {
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Retreived key_type: %d for mem: %s\n", 
                     ...
                     uint32 offset, uint32 mask, uint8 *key, uint16 lsb)
{
    ...
    uint16 crc_hi;
    int32 i, j = 0;
    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Num bits: %d, zero_lsb: %d, lsb: %x, offset: %d, "
                 "mask: %x\n", num_bits, zero_lsb, lsb, offset, mask);
    /* mask bit 0 of key */
    key[0] &= 0xfe;
    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Key: [");
    for (i = num_bits; i > 0; i-=8) {
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "%0x", key[j++]);
    }
    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "]\n");
    if (offset >= 48) {
        if (!zero_lsb) {
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Hash(zero)\n");
            return 0;
        } else {
            if (offset > 48) {
                lsb = lsb >> (offset-48);
            ...
            lsb &= mask;
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Hash(lsb): %d\n", lsb);
            return lsb & mask;
        }
    } else {
        crc_lo = _soc_crc32b(key, num_bits);
        crc_hi = _soc_crc16b(key, num_bits);
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "crc32: %x\n", crc_lo);
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "crc16: %x\n", crc_hi);
        ...
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Hash(crc): %d\n", crc_lo);
        return crc_lo & mask;
    }
    return SOC_E_NONE;
}
                              ...
                              uint32 *fldbuf, uint16 size, uint8 lendian)
{
    ...
    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "offset: %d, size: %d\n", offset, size);
    if (lendian) {
        wp = offset / 32;
        bp = offset & (32 - 1);
        i = 0;
                               ...
                               soc_field_t *keyflds, uint8 *key, uint8 num_flds)
{
    ...
    for (i = 0; i < num_flds; i++) {
        SOC_FIND_FIELD(keyflds[i], meminfo->fields, meminfo->nFields,
                       fieldinfo);
        if (NULL == fieldinfo) {
            soc_cm_print("mem %s field %s is invalid\n",
    ...
    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Combined Key: ");
...
soc_gen_entry_from_key(int unit, soc_mem_t mem, uint8 *key, void *entry)
{
    ...
    for (j = 0; j < _SOC_ISM_MEMS(unit)[i].shms->num_keys; j++) {
        if (_SOC_ISM_MEMS(unit)[i].shms->shk[j].hmv->shm->mem == mem) {
            if (key_type == _SOC_ISM_MEMS(unit)[i].shms->shk[j].key_type) {
                found = 1;
                soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Input key_type: %d found for mem: %s\n", 
                             key_type, SOC_MEM_NAME(unit, mem));
                break;
            }
        }  
    ...
    if (!found) {
        soc_cm_print("Key type not found for this memory !!\n");
        return SOC_E_INTERNAL;
    }
    while (_SOC_ISM_MEMS(unit)[i].shms->shk[j].hmv->key_fields[f] != -1) { 
        keyf[f] = _SOC_ISM_MEMS(unit)[i].shms->shk[j].hmv->key_fields[f];
...
soc_gen_key_from_entry(int unit, soc_mem_t mem, void *entry, void *key)
{
    ...
    if (memidx < 0) {
        soc_cm_debug(DK_ERR, "Invalid hash memory: %s !!\n", 
    ...
    if (soc_generic_get_hash_key(unit, mem, entry, keyflds, &lsbfld, 
        ...
        for (i = 0; i < num_flds; i++) {
            SOC_FIND_FIELD(keyflds[i], meminfo->fields, meminfo->nFields,
                           fieldinfo);
            if (NULL == fieldinfo) {
                soc_cm_print("Unit %d: mem %s field %s is invalid\n", unit,
                       SOC_MEM_NAME(unit, mem), SOC_FIELD_NAME(unit, keyflds[i]));
                assert(fieldinfo);
            }
            soc_mem_field_get(unit, mem, entry, keyflds[i], val);
                 ...
                 uint8 *num_entries)
{
    ...
    if (soc_generic_get_hash_key(unit, mem, entry, keyflds, &lsbfld, 
                                 &num_flds) == SOC_E_NONE) {
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Key field(s): ");
        for (idx = 0; idx < num_flds; idx++) {
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "%d, ", keyflds[idx]);
        }
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "\nLsb field: %d\n", lsbfld);
        ...
        soc_cm_print("Key field not found !!\n");
        return -1;
    } 
    ...
    for (idx = 0; idx < _SOC_ISM_MEMS(unit)[memidx].shms->num_banks; idx++) {
        ...
        soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Bank[%d]: bucket:%d\n", bidx, bucket);
        if (base_idx) {
        ...
        for (midx = 0; midx < shbank[idx].bkt_size/idxinc;) {
            ...
            if (soc_mem_field32_get(unit, mem, tmp_hs, vf)) {
                ...
                if (!sal_memcmp(key, tmp_key, sizeof(key))) {
                    ...
                    sbo[bidx].mode = 1;
                    soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Existing mem index: %d\n", 
                ...
                sbo[bidx].bank = shbank[idx].my_id % SOC_ISM_INFO(unit)->banks_per_stage;
                sbo[bidx].mode = 0;
                soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "New mem index: %d\n", 
                             sbo[bidx].index+sbo[bidx].entry);
                break;
            }
    ...
    for (k = 0; k < _SOC_ISM_MEMS(unit)[s].shms->num_keys; k++) {
        if (key_type == _SOC_ISM_MEMS(unit)[s].shms->shk[k].key_type) {
            *norm_mem = _SOC_ISM_MEMS(unit)[s].shms->shk[k].hmv->shm->mem;
            soc_cm_debug(DK_VERBOSE+DK_SOCMEM, "Normalized for key_type: %d mem: %s\n", 
