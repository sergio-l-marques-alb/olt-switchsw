/*
 * $Id: schan.c 1.38.6.2 Broadcom SDK $
...
soc_schan_dump(int unit, schan_msg_t *msg, int dwc)
{
    ...
    if (soc_feature(unit, soc_feature_generic_table_ops) &&
        ...
        if (soc_feature(unit, soc_feature_new_sbus_format) &&
            ...
            soc_cm_print("  RSP[TYPE=%d=%s ERR_INFO=%d=%s "
                         "INDEX=0x%05x]\n",
             ...
             int intr)
{
     ...
     */
#if 0
/* put these under #if 0 because of mutex deadlock error */

    if (SOC_IS_ARAD(unit) && SOC_HW_ACCESS_DISABLE(unit) && (0 == (SOC_CONTROL(unit)->schan_override))) {
        soc_cm_print("soc_schan_op: ERROR, trying to access HW during deinit or warm reboot\n");
        return SOC_E_FAIL;
    }
#endif
#endif
    ...
    do {
            ...
            if (sal_sem_take(SOC_CONTROL(unit)->schanIntr,
                ...
                if(soc_feature(unit, soc_feature_cmicm)) {
                    ...
                    if(soc_feature(unit, soc_feature_schan_err_check)) {
                        ...
                        if(err_bit) {
                            rv = SOC_E_FAIL;
                            soc_cm_debug(DK_ERR, "  ERRBIT received in CMIC_SCHAN_ERR.\n");
                        }
                    }
                } else
#endif
            ...
            if(soc_feature(unit, soc_feature_cmicm)) {
                ...
                if (schanCtrl & SC_CMCx_MSG_NAK) {
                    ...
                    if (SOC_IS_KATANA(unit)) {
                        if (msg->header.opcode == READ_MEMORY_CMD_MSG) {
                                sal_dpc(soc_katana_mem_nack, INT_TO_PTR(unit),
                                        INT_TO_PTR(msg->gencmd.address), 
                                        INT_TO_PTR(msg->gencmd.header.dstblk), 0, 0);
                        }
                    }
#endif
                }
                if ((schanCtrl & SC_CMCx_MSG_SER_CHECK_FAIL) &&
                    ...
                    rv = SOC_E_FAIL;
                    soc_cm_debug(DK_ERR, "  SER Parity Check Error.\n");
                    ...
                    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                        ...
                        case READ_REGISTER_CMD_MSG:
                            ...
                            if (SOC_IS_TRIDENT(unit)) {
                                sal_dpc(soc_trident_mem_nack, INT_TO_PTR(unit),
                                        INT_TO_PTR(msg->gencmd.address), 
                                        INT_TO_PTR(msg->gencmd.header.dstblk), 
                                        msg->header.opcode == READ_REGISTER_CMD_MSG ? INT_TO_PTR(1) : 0, 0);
                            } else 
#endif
                            {
        ...
        if (rv == SOC_E_TIMEOUT) {
            if (SOC_IS_TRIUMPH3(unit)) {
                if (msg->header.opcode == WRITE_REGISTER_CMD_MSG) {
                    if ((msg->writecmd.address == 0x02029700) && 
                        (msg->writecmd.header.dstblk >= 0x8  && msg->writecmd.header.dstblk <= 0xf)) {
                        rv = SOC_E_NONE;
                    }
                } else {
                    soc_cm_debug(DK_VERBOSE,
