/*
 * $Id: intr_cmicm.c 1.96.12.1 Broadcom SDK $
...
#define HOST_IRQ_MASK_SET(_unit, _offset, _mask)                               \
    do {                                                                       \
        if (soc_cm_get_bus_type(unit) & SOC_AXI_DEV_TYPE) {                    \
            soc_pci_write(_unit, (_offset + HOST_IRQ_MASK_OFFSET_DIFF), _mask);\
        }                                                                      \
        else {                                                                 \
            soc_pci_write(_unit, _offset, _mask);                              \
        }                                                                      \
    } while(0)
...
#else
#define IRQ_MASK_SET(_u,_a,_m) HOST_IRQ_MASK_SET(_u,_a,_m)
#endif
...
soc_cmicm_parity_intr(int unit, uint32 val)
{
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        ...
        soc_cm_debug(DK_INTR,
                     ...
                     unit);
        sal_dpc(soc_tr3_process_func_intr, INT_TO_PTR(unit), 0,
    ...
    if (SOC_IS_TD2_TT2(unit)) {
        ...
        soc_cm_debug(DK_INTR,
                     "soc_cmicm_intr type 2 unit %d: dispatch\n",
                     unit);
        sal_dpc(soc_trident2_ser_error, INT_TO_PTR(unit), 0, INT_TO_PTR(oldmask), 
                0, 0);
    } else
#endif /* BCM_TRIDENT2_SUPPORT */
#ifdef BCM_HURRICANE2_SUPPORT
    if (SOC_IS_HURRICANE2(unit)) {
        ...
        soc_cm_debug(DK_INTR,
                     "soc_cmicm_intr type 2 unit %d: dispatch\n",
                     unit);
        sal_dpc(soc_hurricane2_parity_error, INT_TO_PTR(unit), 0, INT_TO_PTR(oldmask), 
                0, 0);
    } else 
#endif /* BCM_HURRICANE2_SUPPORT */
    {
...
soc_cmicm_block_lo_intr(int unit, uint32 val)
{
    ...
    if (SOC_IS_CALADAN3(unit)) {
        ...
        soc_cmicm_intr3_disable(unit, irqMask);
...
soc_cmicm_cmcx_intr0_enable(int unit, int cmc, uint32 mask)
{
    ...
    IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK0_OFFSET(cmc), newMask);
...
soc_cmicm_cmcx_intr1_enable(int unit, int cmc, uint32 mask)
{
    ...
    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK1_OFFSET(cmc), newMask);
    sal_spl(s);
...
soc_cmicm_cmcx_intr2_enable(int unit, int cmc, uint32 mask)
{
    ...
    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK2_OFFSET(cmc), newMask);
    sal_spl(s);
...
soc_cmicm_cmcx_intr3_enable(int unit, int cmc, uint32 mask)
{
    ...
    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK3_OFFSET(cmc), newMask);
    sal_spl(s);
...
soc_cmicm_cmcx_intr4_enable(int unit, int cmc, uint32 mask)
{
    ...
    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK4_OFFSET(cmc), newMask);
    sal_spl(s);
...
check_arm_type0:
    ...
    IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK0_OFFSET(cmc), SOC_CMCx_IRQ0_MASK(unit, cmc));
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_DPP_SUPPORT)

    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK1_OFFSET(cmc), SOC_CMCx_IRQ1_MASK(unit, cmc));
    HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK2_OFFSET(cmc), SOC_CMCx_IRQ2_MASK(unit, cmc));
    if (soc_feature(unit, soc_feature_extended_cmic_error)) {
        HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK3_OFFSET(cmc), SOC_CMCx_IRQ3_MASK(unit, cmc));
        HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK4_OFFSET(cmc), SOC_CMCx_IRQ4_MASK(unit, cmc));
    }
    if (soc_feature(unit, soc_feature_short_cmic_error)) {
         ...
         */
        HOST_IRQ_MASK_SET(unit, CMIC_CMCx_PCIE_IRQ_MASK2_OFFSET(cmc), SOC_CMCx_IRQ2_MASK(unit, cmc) & 0x1);
    }
