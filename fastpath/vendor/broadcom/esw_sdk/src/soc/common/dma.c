/*
 * $Id: dma.c 1.60.26.1 Broadcom SDK $
...
#ifdef BCM_CMICM_SUPPORT
        if (soc_feature(unit, soc_feature_cmicm)) {
            uint32 ch, numq, reg_addr, reg_val;
            int cmc = SOC_PCI_CMC(unit);
            ...
            reg_addr = CMIC_CMCx_CHy_COS_CTRL_RX_0_OFFSET(cmc, ch);
            soc_pci_write(unit, reg_addr, reg_val);
            reg_val = 0;
            if (numq > 32 && numq < 64) {
                reg_val = (1 << (numq - 32)) - 1;
            }
            reg_addr = CMIC_CMCx_CHy_COS_CTRL_RX_1_OFFSET(cmc, ch);
            soc_pci_write(unit, reg_addr, reg_val);
        } else 
#endif
