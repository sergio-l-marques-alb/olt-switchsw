/*
 * $Id: uc_msg.c 1.14.2.33 Broadcom SDK $
    ...
    while (1) {
        while (soc_cmic_uc_msg_receive(unit, uC, MOS_MSG_CLASS_SYSTEM,
            ...
            if (soc_cmic_uc_msg_active_wait(unit, uC) != SOC_E_NONE) {
                ...
                soc_cm_print("System thread exitting\n");
                return;
            }
        } 
        ...
        if (rcv.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_LOG) {
            ...
            send.s.data = _shr_swap32(0x0);
           soc_cm_print("KEYSTONE: uc_msg swap=%d\n",send.s.data);
#else
...
int soc_cmic_uc_msg_receive_cancel(int unit, int uC, int msg_class)
{
    ...
    if (soc->uc_msg_rcv_sems[uC][msg_class] != NULL) {
        /* Kick the waiter */
        rcv_sem = soc->uc_msg_rcv_sems[uC][msg_class];
        soc->uc_msg_rcv_sems[uC][msg_class] = NULL;
        sal_sem_give(rcv_sem);
    }
