/*
 * $Id: system.c 1.167.6.19 Broadcom SDK $
...
#if VX_VERSION == 62 || VX_VERSION == 64 || VX_VERSION == 65 || VX_VERSION == 66
#ifndef MAX_GRPS
#define MAX_GRPS                20
#endif
       ...
       char *msg)
{
    ...
    if (!strcmp(sysBootParams.usr, user) &&
    !strcmp(sysBootParams.passwd, pass)) {
...
diag_init(void)
{
    ...
    telnet_initted++;
    printk("TELNET service started on TCP port 23.\n");
    }
#endif /* INCLUDE_TELNET */
...
cmd_tcl(int unit, args_t *a)
{
    ...
    bootdir_asst = sal_alloc(strlen(bootdir) + 10, "tcl_bootdir");
...
{
#if defined(BCM_ROBO_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    soc_reg_t reg;
    char **av;
#endif /* BCM_ROBO_SUPPORT || BCM_ESW_SUPPORT  || defined(BCM_SIRIUS_SUPPORT) */
    int ac = 0;
    int unit = diag_list_possib_unit;
#if defined(BCM_ESW_SUPPORT) || defined(BCM_ROBO_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
#if defined(BCM_ROBO_SUPPORT) || !defined(SOC_NO_NAMES)
    soc_field_t fld;
#endif /* BCM_ROBO_SUPPORT !SOC_NO_NAMES */
#if defined(BCM_ROBO_SUPPORT) || !defined(SOC_NO_NAMES) ||\
    !defined(SOC_NO_ALIAS)  || defined(BCM_SIRIUS_SUPPORT)
    int len = strlen(regpref);
...
#endif /* BCM_ESW_SUPPORT || BCM_ROBO_SUPPORT || BCM_SIRIUS_SUPPORT */
...
diag_user_var_get(char *varname)
{
    ...
    if (strncmp(varname, "feature_", 8) == 0) {
        ...
        for (f = 0; f < soc_feature_count; f++) {
            if (strcmp(varname + 8, soc_feature_name[f]) == 0) {
...
#ifndef VXWORKS_NETWORK_STACK_6_5
    if (pingLibInit() != OK) {
    printk("ERROR: pingLib not initialized.\n");
    parse_arg_eq_done(&pt);
...
if_host(int unit, args_t *a)
{
    ...
    if (sethostname(hostbuf, strlen(hostbuf) + 1) != OK) {
        printk("sethostname failed\n");
        return CMD_FAIL;
    }
...
diag_shell(void)
{
    ...
    if (!(flags & BOOT_F_NO_PROBE)) {
        if ((sysconf_probe()) < 0) {
            printk("ERROR: PCI SOC device probe failed\n");
        }
        ...
        if (!(flags & BOOT_F_NO_ATTACH)) {
            for (i = 0; i < soc_all_ndev; i++) {
                 if (sysconf_attach(i) < 0) {
                     printk("ERROR: SOC unit %d attach failed\n", i);
                 }
#if defined(BCM_SBX_SUPPORT)
	    ...
	    printk("Boot flags: Attach NOT performed\n");
	}
    } else {
        printk("Boot flags: Probe NOT performed\n");
    }
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (soc_property_get_str(i, "scache_nh") != NULL) {
            ...
            stable_flags = 0;
            stable_size = 1000 * 1024;      /* 1000 Kbytes */
        }
        ...
        if (BCM_SWITCH_STABLE_NONE != stable_location) {
            ...
            if ((BCM_SWITCH_STABLE_APPLICATION == stable_location) &&
                ...
                if (appl_scache_file_open(i, (flags & BOOT_F_WARM_BOOT) ?
                                          TRUE : FALSE,
                                          stable_filename) < 0) {
                    printk("Unit %d: stable cache file not %s\n", i,
                ...
                printk("Build-in stable cache file not supported in this configuration\n");
            ...
            if (soc_stable_set(i, stable_location, stable_flags) < 0) {
                printk("Unit %d: soc_stable_set failure\n", i);
            } else if (soc_stable_size_set(i, stable_size) < 0) {
                printk("Unit %d: soc_stable_size_set failure\n", i);
            }
            ...
            if (1 == soc_property_get(i,
                ...
                if (soc_event_register(i,
                    ...
                    printk("Unit %d: soc_event_register failure\n", i);
                }
            }
        } else {
    ...
    if ((script = sal_boot_script()) != NULL) {
        if (sh_rcload_file(-1, NULL, script, FALSE) != CMD_OK) {
            printk("ERROR loading boot init script: %s\n", script);
        }
        ...
        for (i = 0; i < soc_ndev; i++) {
            if (soc_attached(i)) {
                ...
                if (diag_rc_load(i) != CMD_OK) {
                    printk("ERROR loading rc script on unit %d\n", i);
                }
            }
        }
    } else if (no_rc_warning) {
        printk("Boot flags: initialization scripts NOT loaded\n");
    }
...
system_init(int unit)
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        soc_cm_debug(DK_ARL, "Init ARL Thread on unit %d\n", unit);

        if (soc_property_get(unit, "reg_write_log", FALSE)) {
            debug_flag = DK_REG+DK_VERBOSE;
        } else {
            debug_flag = DK_ERR | DK_WARN;
        }
        debugk_select(debug_flag);
#endif 
    } 
    else {
#if defined(BCM_ESW_SUPPORT)
    ...
    if (SOC_IS_ROBO5324(unit)) {
        /* 
         * Register trunk callback function for BCM5324 errata workaround. 
         */
        BCM_IF_ERROR_RETURN(bcm_linkscan_register(unit, bcm5324_trunk_patch_linkscan));                               
    }    
#endif
...
uint32 sal_portable_debugk_select(uint32 dk)
{
...
uint32 sal_portable_debugk_enable(uint32 dk)
{
    uint32 old_dk = current_dk;
    current_dk |= dk;
    return old_dk;
...
uint32 sal_portable_debugk_disable(uint32 dk)
{
    uint32 old_dk = current_dk;
    current_dk &= ~dk;
    return old_dk;
...
int sal_portable_debugk_check(uint32 dk)
{
    return ((current_dk & dk) == dk);
}

int sal_portable_vdebugk(uint32 dk, const char *fmt, va_list varg)
{
    int rc = 0; 

    if(sal_portable_debugk_check(dk)) {
        rc = sal_portable_vprintf(fmt, varg); 
    }

    return rc;
}

int sal_portable_debugk(uint32 dk, const char* fmt, ...)
{
    int rc; 
    va_list args; 
    
    va_start(args, fmt); 
    rc = sal_portable_vdebugk(dk, fmt, args); 
    va_end(args); 
