/*
 * $Id: system.c 1.223 Broadcom SDK $
...
cmd_tcl(int unit, args_t *a)
{
    ...
    bootFlags = sal_alloc(strlen(bootTxt) + 12, "tcl_bootflags");
    c3SimPort = sal_alloc(strlen(c3SimTxt) + 8, "tcl_simPort");
    c3SimRunLongSystemTests  = sal_alloc(strlen(c3SimRunLongSystemTestsTxt) + 8,
        "tcl_runLongSysTests");
...
diag_shell(void)
{
    ...
    if (flags & BOOT_F_RELOAD) {
      ...
      printk("Boot flags: regular load\n");
      for (i = 0; i < soc_ndev; i++) {
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (BCM_SWITCH_STABLE_NONE != stable_location) {
            ...
            if (soc_switch_stable_register(i,
                                           ...
                                           NULL, NULL) < 0) {
                printk("Unit %d: soc_switch_stable_register failure\n", i);
            }
