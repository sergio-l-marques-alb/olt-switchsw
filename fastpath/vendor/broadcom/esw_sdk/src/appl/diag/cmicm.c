...
mspi_cmd(int unit, args_t *a)
{
    ...
    if ((dev != -1) || (cpol != -1) || (cpha != -1)) {
        if (soc_mspi_config(unit, dev, cpol, cpha) != SOC_E_NONE){
            soc_cm_print("MSPI: Failure Configuring\n");
            return CMD_FAIL;
        }
    }
    ...
    if (sal_strcasecmp(c, "write") &&
        ...
        soc_cm_print("Neither Write Nor Read\n");
        return CMD_USAGE;
    }
    ...
    if ((wbytes > 0) && (rbytes > 0)) { /* Write and read */
        if ((wbytes + rbytes) > CMICM_MSPI_BLOCK_SIZE) {
            soc_cm_print("MSPI: Too many bytes for single Write-read operation\n");
            return(CMD_FAIL);
        }
        soc_cm_debug(DK_VERBOSE, "MSPI: Writing %d bytes and Reading %d bytes\n", wbytes, rbytes);
...
dpll_cmd(int unit, args_t *a)
{
    ...
    if (cfg > 0) {
         soc_cm_debug(DK_VERBOSE, "Using Preset %d\n", cfg);
    ...
    if (!sal_strcasecmp(c, "write")) {
        rd_op = 0;
        soc_cm_debug(DK_VERBOSE, "Write DPLL\n");
    } else if (!sal_strcasecmp(c, "read")) {
        rd_op = 1;
        soc_cm_debug(DK_VERBOSE, "Read DPLL\n");
    } else {
        return CMD_USAGE;
    }
    ...
    if(addr_width == 1) {
        ...
        soc_cm_print("Only 1 or 2 byte addresses supported\n");
        return CMD_FAIL;
    }
    ...
    if(rd_op == 0) { /* Write */
        ...
        data = rdata[0];
        soc_cm_debug(DK_VERBOSE, "Read %02x\n", data);
