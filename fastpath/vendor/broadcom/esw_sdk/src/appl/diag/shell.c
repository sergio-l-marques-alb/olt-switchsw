/*
 * $Id: shell.c 1.219.4.1 Broadcom SDK $
...
char sh_warmboot_usage[] =
    ...
    "Parameters: [on | off | show | storage | usage | test_mode <on|off>]\n"
...
cmd_sh_restest(int unit, args_t *a){
    ...
    } else if (!sal_strncasecmp(c, "t6", resTestMax(strlen(c), 2))) {
        if (localRes) {
            result = shr_mres_get(localRes, &rescount, NULL);
            if (BCM_E_NONE == result) {
                if (rescount > SHR_RES_ALLOCATOR_COUNT << 1) {
    ...
    } else if (!sal_strncasecmp(c, "t7", resTestMax(strlen(c), 2))) {
        if (localRes) {
    ...
    } else if (!sal_strncasecmp(c, "t8", resTestMax(strlen(c), 2))) {
        if (localRes) {
    ...
    } else if (!sal_strncasecmp(c, "t9", resTestMax(strlen(c), 2))) {
        if (localRes) {
