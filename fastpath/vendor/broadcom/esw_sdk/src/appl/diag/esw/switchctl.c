/*
 * $Id: switchctl.c 1.5.166.1 Broadcom SDK $
...
cmd_switch_control(int unit, args_t *a)
{
	...
	printk("%s: Error: NAMES ARRAY OUT OF SYNC\n", ARG_CMD(a));
	return CMD_FAIL;
    }
	...
	if (!arg_pbmp_given) {
		...
		printk("%-40s", switch_control_names[type]);
	...
	if (type == bcmSwitch__Count) {
	    printk("Unknown switch control: %s\n", name);
	    return CMD_FAIL;
	}
	...
	if (value == NULL) {
		...
		r = bcm_switch_control_get(unit, type, &param);
		if (r < 0) {
		    printk("bcm_switch_control_get ERROR: %s\n",
			   bcm_errmsg(r));
		    retCode = CMD_FAIL;
		} else {
		    printk("0x%x\n", (uint32)param);
                ...
                DPORT_BCM_PBMP_ITER(unit, arg_pbmp, dport, port) {
		    printk("%s: ", BCM_PORT_NAME(unit, port));
		    r = bcm_switch_control_port_get(unit, port, type, &param);
		    if (r < 0) {
			printk("bcm_switch_control_port_get ERROR: %s\n",
			       bcm_errmsg(r));
		    } else {
			printk("0x%x\n", (uint32)param);
		    }
		...
		if (r < 0) {
		    printk("bcm_switch_control_set ERROR: %s\n",
			   bcm_errmsg(r));
		    retCode = CMD_FAIL;
		}
	    } else {
		    ...
		    if (r < 0) {
			printk("bcm_switch_control_port_set ERROR: "
			       "port=%d: %s\n",
			       port, bcm_errmsg(r));
			retCode = CMD_FAIL;
		    }
