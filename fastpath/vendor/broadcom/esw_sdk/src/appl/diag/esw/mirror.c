/*
 * $Id: mirror.c 1.14.6.3 Broadcom SDK $
                           ...
                           void *user_data)
{
    ...
    (*count)++;
    printk("Mirror Dest %2d: ", *count);
    printk("Mirror ID: %2d; ", BCM_GPORT_MIRROR_GET(mirror_dest->mirror_dest_id));
    if (mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_L2) {
        printk("%s ", "L2 Tunnel to");
    }
    if (mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_IP_GRE) {
        printk("IPv%d GRE Tunnel\n", mirror_dest->version);
    ...
    if (BCM_GPORT_IS_LOCAL(mirror_dest->gport)) {
        ...
        if (bcm_port_local_get(unit, mirror_dest->gport, &local_port) != BCM_E_NONE) {
            printk("modport %d/%d", BCM_GPORT_MODPORT_MODID_GET(mirror_dest->gport),
                   BCM_GPORT_MODPORT_PORT_GET(mirror_dest->gport));
        } else {
            printk("local port %s", BCM_PORT_NAME(unit, local_port));
        }
    } else if (BCM_GPORT_IS_TRUNK(mirror_dest->gport)) {
        printk("trunk group %d", BCM_GPORT_TRUNK_GET(mirror_dest->gport));
    } else {
        printk("GPORT 0x%08X", mirror_dest->gport);
    }
    printk("\n");
    return BCM_E_NONE;
}
...
_if_esw_mirror_dest(int unit, args_t *a)
{
    ...
    if (0 == sal_strcasecmp("show", cmd)) {
        ...
        rv = bcm_mirror_destination_traverse(unit, show_mirror_destination_cb,
                                             &md_count);
        if (BCM_FAILURE(rv)) {
            printk("%s bcm_mirror_destination_traverse() failed  %s\n",
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
	if (md_count == 0) {
	    printk("No mirror destinations currently defined\n");
	}
        return CMD_OK;
    }
    ...
    switch (mode) {
        case 1:
            mirror_dest.flags |= BCM_MIRROR_PORT_INGRESS;
            break;
        case 2:
            mirror_dest.flags |= BCM_MIRROR_PORT_EGRESS;
            break;
        case 3:
            mirror_dest.flags |= BCM_MIRROR_PORT_EGRESS_TRUE;
            break;
        case 4:
            mirror_dest.flags |= (BCM_MIRROR_PORT_INGRESS | BCM_MIRROR_PORT_EGRESS);
            break;
       default:
            break;
    }
    ...
    if (0 == sal_strcasecmp("create", cmd)) {
        ...
        if (BCM_FAILURE(rv)) {
            printk("%s bcm_mirror_destination_create() failed  %s\n", 
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        printk(" Mirror Id = 0x%x\n", mirror_dest.mirror_dest_id);
        return CMD_OK;
    } else if (0 == sal_strcasecmp("add", cmd)) {
        rv = bcm_mirror_port_dest_add(unit, src_port, mirror_dest.flags, mirror_dest_id);
        if (BCM_FAILURE(rv)) {
            printk("%s bcm_mirror_port_dest_add() failed  %s\n", 
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        return CMD_OK;
        ...
        if (0 == mirror_dest_id) {
            printk("%s mirror destination id is missing\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        rv = bcm_mirror_destination_destroy(unit, mirror_dest_id);
        if (BCM_FAILURE(rv)) {
            printk("%s bcm_mirror_destination_destroy failed()  %s\n", 
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        return CMD_OK;
    } else if (0 == sal_strcasecmp("delete", cmd)) {
        rv = bcm_mirror_port_dest_delete(unit, src_port, mirror_dest.flags, mirror_dest_id);
        if (BCM_FAILURE(rv)) {
            printk("%s bcm_mirror_port_dest_delete failed %s\n", 
                   ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        }
        return CMD_OK;
...
_mirror_show(int unit)
{
    ...
    if (bcm_port_config_get(unit, &pcfg) != BCM_E_NONE) {
        printk("Error: bcm ports not initialized\n");
        return CMD_FAIL;
    }
        ...
        for (fc = 0; fc < sizeof(flags) / sizeof(uint32); fc++) {
            rv =
              bcm_mirror_port_dest_get(unit,
                                       ...
                                       &mirror_dest_count);
            if (rv < 0) {
                printk("bcm_mirror_port_dest_get() FAILED: %s\n", bcm_errmsg(rv));
                return CMD_FAIL;
            }
                ...
                for (i = 0; i < mirror_dest_count; i++) {
                    ...
                    rv =
                      bcm_mirror_destination_get(unit, mirror_dest[i],
                                                 &mirror_dest_desc);
                    if (rv < 0) {
                        printk("bcm_mirror_destination_get() FAILED: %s\n",
                               bcm_errmsg(rv));
                        return CMD_FAIL;
                    }
    ...
    if (count == 0) {
        printk("No mirror ports configured\n");
    }
    return CMD_OK;
}
...
_dmirror_show(int unit, int port, int dstmod, int dstport, uint32 flags)
{
    ...
    if (show_dest) {
        ...
        if (BCM_GPORT_IS_TRUNK(dstport)) {
            printk(" to trunk group %d", BCM_GPORT_TRUNK_GET(dstport));
        } else if (flags & BCM_MIRROR_PORT_DEST_TRUNK) {
            printk(" to trunk group %d", dstport);
        ...
        if (local_port >= 0) {
            ...
            if (rv == BCM_E_NONE && vlan > 0) {
                printk(" (TPID=0x%04X(%d), VLAN=0x%04X(%d))", tpid, tpid, vlan, vlan);
            }
        }
    }
    printk("\n");
...
if_dmirror(int unit, args_t *a)
{
    ...
    if (SOC_IS_XGS12_FABRIC(unit) ) {
        printk("%s: command not supported for switch fabric chips\n", ARG_CMD(a));
        return CMD_FAIL;
    }
    ...
    if ((ARG_CNT(a) == 0) && parse_cmp("SHow", mstr, '\0')) {
        ...
        if (count == 0) {
            printk("%s: No mirror ports configured\n", ARG_CMD(a));
        }
        return CMD_OK;
    }
    ...
    if (argMode < 0) {
        printk("%s: must specify a mode\n", ARG_CMD(a));
        return CMD_USAGE;
    }

    if ((argMode > 0) && (argDestPort < 0) && (argDestTrunk < 0)) {
        printk("%s: missing DestPort or DestTrunk\n", ARG_CMD(a));
        return CMD_USAGE;
    }
    if ((argDestPort >= 0) && (argDestTrunk >= 0)) {
        printk("%s: cannot specify both DestPort and DestTrunk\n", ARG_CMD(a));
        return CMD_USAGE;
    }
