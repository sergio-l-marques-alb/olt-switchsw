...
#define MIM_ROE(op, arg)     \
    do {               \
        ...
        if (BCM_FAILURE(__rv__)) { \
            printk("MIM_CLI: Error: " #op " failed, %s\n", \
                           bcm_errmsg(__rv__)); \
            return CMD_FAIL; \
        } \
    } while(0)
#define MIM_CMD(_s)                         \
    if ((cmd = ARG_CUR(a)) == NULL) {        \
        ARG_PREV(a);  \
        printk("MIM_CLI: Error: Missing arg after %s\n", ARG_CUR(a)); \
        return CMD_USAGE;                    \
...
#define PT_PARSE(_a, _pt) \
    do {     \
        if (parse_arg_eq(_a, &_pt) < 0) { \
            printk("MIM_CLI: Error: Invalid option or expression: %s\n", \
                    ARG_CUR(_a)); \
            PT_DONE(_pt); \
            return CMD_USAGE; \
        } \
...
if_tr2_mim(int unit, args_t * a)
{
    ...
    switch (action) {
        ...
        case MIM_OPTIONS:
            ...
            break;
        default:
            printk("Error:Unknown MIM command %d\n", action);
            return CMD_USAGE;
    }
...
_bcm_tr2_mim_cli_vpn_create(int unit, args_t * a)
{
    ...
    ARG_NEXT(a);
    if (ARG_CNT(a) < 5) {
        printk("MIM_CLI: Missing arguments\n");
        return CMD_USAGE;
    }
    PT_INIT(unit, pt);
    ...
    printk("env var added : $bcast_type_id=0x%08x\n", bcast_group);
    var_set_integer(ENV_MCAST_TYPE_ID, umc_group, TRUE, FALSE);
    printk("env var added : $umc_type_id=0x%08x\n", umc_group);
    var_set_integer(ENV_MCAST_TYPE_ID, uuc_group, TRUE, FALSE);
    printk("env var added : $uuc_type_id=0x%08x\n", uuc_group);
    return CMD_OK;
}
...
_bcm_tr2_mim_cli_vpn_port_add(int unit, args_t * a)
{
    ...
    if (ifl & (BCM_MIM_PORT_REPLACE | BCM_MIM_PORT_WITH_ID)) {
        ...
        soc_cm_debug(DK_TESTS, "mim port=%08x\n", port);
    }
    MIM_ROE(bcm_port_gport_get, (unit, phys_port, &gp));
    mport.port = gp;
    soc_cm_debug(DK_TESTS, "gport_get gp=%08x\n", gp);
    mport.criteria = pm;
    mport.flags = ifl;
    mport.if_class = icl;
    ...
    printk("env var added : $mim_port_id=0x%08x\n", mport.mim_port_id);
...
_bcm_tr2_mim_cli_mcast_group_create(int unit, args_t * a)
{
    ...
    PT_PARSE(a, pt);
    soc_cm_debug(DK_TESTS, "mcast group %d\n", mcast_group);
...
_bcm_tr2_mim_cli_mcast_group_del(int unit, args_t * a)
{
    ...
    soc_cm_debug(DK_TESTS, "mcast group %d destroyed\n", mcast_group);
...
_bcm_tr2_mim_cli_mcast_group_addr(int unit, args_t * a)
{
    ...
    printk("MIM_CLI: mc_group 0x%08x vpn 0x%08x\n", mcast_group, vpn);
    PT_DONE(pt);
...
_bcm_tr2_mim_cli_parse_flags(char *input_s, int type)
{
    ...
    if(!flags) {
        printk("MIM CLI: Error: Discarded unrecognized Flags\n\t %s\n", 
                        input_s);
        return 0;
    }
    ...
    while (!done) {
        ...
        if (p) {
            ...
            len = (strlen(input_s) >= MAX_FLAG_LENGTH) ? 
                   (MAX_FLAG_LENGTH - 1) : strlen(input_s);
        ...
        if (!found) {
            printk("MIM_CLI: flag %s not recognized, discarded\n", f_name);
        }
    }
    return result;
