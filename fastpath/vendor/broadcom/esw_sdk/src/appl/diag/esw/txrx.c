/*
 * $Id: txrx.c 1.25.6.3 Broadcom SDK $
...
_xd_init(int unit)
{
    ...
    if (bcm_port_config_get(unit, &pcfg) != BCM_E_NONE) {
        printk("WARNING: bcm ports not initialized\n");
        return;
    }
    ...
    if (xd == NULL) {
        ...
        if (xd == NULL) {
            printk("WARNING: xd memory allocation failed\n");
            return;
        }
    ...
    if (bcm_stk_my_modid_get(unit, (int *)&xd->xd_hg_smod) < 0 ) {
        printk("WARNING: getting my_modid failed\n");
        return;
    }
 ...
 */
{
    ...
    if (xd->hdr_mode == ENCAP_IEEE) {
        ...
        parse_table_add(&pt, "Word2",   PQ_DFL|PQ_INT,0,
                        &xd->w2, NULL);
#ifdef BCM_HIGIG2_SUPPORT
        if (soc_feature(u, soc_feature_higig2)) {
    ...
    if (!XD_FILE(xd) && !tagged) {
        debugk(DK_WARN,
               "%s: Warning: Sending untagged packets from CPU "
               "not recommended\n",
               ARG_CMD(a));
    }
    ...
    if (xd->xd_hg_cos > NUM_COS(xd->xd_unit)-1 && 
        ...
        printk("%s: Error: Invalid COS queue %d, must be < %d\n", ARG_CMD(a),
...
add_higig_hdr(xd_t *xd, bcm_pkt_t *pkt_info)
{
    ...
    case ENCAP_HIGIG2:      /* HiGig2 header */
        if (xd->hdr_mode == ENCAP_HIGIG2 &&
            ...
            _tx_hghdr_setup(pkt_info, SOC_HIGIG2_START);
        } else {
            printk("Warning:  Higig2 not supported by device\n");
        }
        break;
#endif /* BCM_HIGIG2_SUPPORT */
    ...
    case ENCAP_RAW:        /* Raw HIGIG */
        ...
        memcpy(ptr,   (void*)&temp_raw, sizeof(uint32));
        temp_raw = soc_htonl(xd->w1);
        memcpy(ptr+4, (void*)&temp_raw, sizeof(uint32));
        temp_raw = soc_htonl(xd->w2);
        memcpy(ptr+8, (void*)&temp_raw, sizeof(uint32));
#ifdef BCM_HIGIG2_SUPPORT
        if (soc_feature(xd->xd_unit, soc_feature_higig2)) {
            temp_raw = soc_htonl(xd->w3);
            memcpy(ptr+12, (void*)&temp_raw, sizeof(uint32));
        }
#endif /* BCM_HIGIG2_SUPPORT */
        break;

    default:          /* Error */
        printk("Warning:  Unknown HG header format: %d\n", xd->hdr_mode);
        break;
    }
}
 ...
 */
{
    ...
    for (xd->xd_cur_cnt = 0;
        ...
        /* Store pattern */
        if (!XD_FILE(xd) && xd->xd_pat_random) {
        ...
        if ((rv = bcm_tx(xd->xd_tx_unit, pkt_info, NULL)) != BCM_E_NONE) {
            debugk(DK_ERR, "bcm_tx failed: TX Unit %d: %s\n",
 ...
 */
{
    ...
    if ((1 != ARG_CNT(a)) || !(isint(ARG_CUR(a)))) {
        ...
        printk("%s: Error: Packetwatcher running\n", ARG_CMD(a));
        return(CMD_FAIL);
    }
    ...
    if (!(dv = soc_dma_dv_alloc(u, DV_RX, 1))) {
        printk("%s: failed to allocate DV\n", ARG_CMD(a));
        return(CMD_FAIL);
    }
    if (!(p = soc_cm_salloc(u, 10240, "RX"))) {
        printk("%s: failed to allocate packet memory\n", ARG_CMD(a));
...
cmd_drt(int unit, args_t *args)
{
    ...
    if(!strcmp(arg, "init")) {
        if((rv = soc_dma_rom_init(unit, 1600, 1, 1)) < 0) {
            printk("soc_dma_rom_init() returned %d: %s\n",
                   rv, soc_errmsg(rv)); 
            return CMD_FAIL; 
        }
    }
        ...
        if((rv = soc_dma_rom_detach(unit)) < 0) {
            printk("soc_dma_rom_detach() returnd %d: %s\n",
                   rv, soc_errmsg(rv)); 
            return CMD_FAIL;
        }
    }
        ...
        if((rv=soc_dma_rom_rx_poll(unit, &dcb)) < 0) {
            printk("soc_dma_rom_rx_poll() returned %d: %s\n",
                   rv, soc_errmsg(rv)); 
            return CMD_FAIL; 
        }
        ...
        if ((rv = soc_dma_rom_tx_start(unit, dcb)) < 0) {
            printk("soc_dma_rom_tx_start() returned %d: %s\n",
                   rv, soc_errmsg(rv));         
            soc_dma_rom_dcb_free(unit, dcb); 
            return CMD_FAIL; 
        }
