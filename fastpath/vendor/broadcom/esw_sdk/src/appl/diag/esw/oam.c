/*
 * $Id: oam.c 1.30.2.9 Broadcom SDK $
 * 
...
#define ENDPOINT_LIST_HEADER \
    "ID   Grp Name L Period CLDTLO VLAN Mod Port Tr MAC address       PktPri IntPri\n" \
    "---- --- ---- - ------ ------ ---- --- ---- -- ----------------- ------ ------\n"
    ...
    bcm_oam_endpoint_info_t *endpoint_info_p, void *user_data)
{
    ...
    if (endpoint_info_p->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT) {
        printk("    Priority mapping: ");
        for (i = 0; i < BCM_OAM_INTPRI_MAX; i++) {
             printk(" %d ", endpoint_info_p->pri_map[i]);
        }
        printk("\n");
    }
    ...
    {    
        ...
        if (endpoint_info.flags & BCM_OAM_ENDPOINT_UP_FACING) {
            if (port != -1) {
                ...
                printk("Destination port is required for UP MEP.\n");
                CLEAN_UP_AND_RETURN(CMD_FAIL);
            }
            ...
            if (port != -1) {
                ...
                printk("Destination port is required for down MEP created on TRUNK.\n");
                CLEAN_UP_AND_RETURN(CMD_FAIL);
            }
        }
        ...
        else
        {
            ...
            else if (trunk_id != BCM_TRUNK_INVALID)
            {
                ...
                if ((1 == local_tx) && (-1 == trunk_index)) {
                    ...
                    printk("A TrunkIndex value is required.\n");
            ...
            if (local_tx)
            {
                ...
                {
                    printk("Source MAC address is required for CCM TX.\n");
                ...
                if (SOC_IS_ENDURO(unit)) {
                    if (BCM_MAC_IS_ZERO(endpoint_info.dst_mac_address)) {
                        printk("Destination MAC address is required for CCM TX.\n");
                ...
                if (cpu_pri < 0)
                {
                    printk("A cpu queue is required.\n");
    ...
    else if (sal_strcasecmp(arg_string_p, "events") == 0)
    {
        ...
        if (arg_string_p != NULL)
        {
            if (sal_strcasecmp(arg_string_p, "on") == 0)
            {
                ...
                {
                    printk("Failed to enable OAM events.\n");
            ...
            else if (sal_strcasecmp(arg_string_p, "off") == 0)
            {
                ...
                {
                    printk("Failed to disable OAM events.\n");
