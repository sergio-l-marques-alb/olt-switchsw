...
_mcs_dump_region(int unit, FILE *fp, uint32 *buffer, mos_coredump_region_t *reg) {
    ...
    while (len > 0) {
        ...
        if (++buf_idx >= (DUMP_BUFFER_SIZE / sizeof(uint32)) || (len == 0)) {
            if (fwrite(buffer, 4, buf_idx, fp) != buf_idx) {
                return(CMD_FAIL);
            }
            buf_idx = 0;
        }
...
mcs_dump(int unit, FILE *fp) {
    ...
    if (fwrite(buffer, sizeof(mos_coredump_region_t), 6, fp) != 6) {
 ...
 */
{
    ...
    cpsr = soc_pci_mcs_read(unit, 0x100060);
    printk("Core 0 status: %s\n", (cpsr) ? "Fault" : "Ok");

    cpsr = soc_pci_mcs_read(unit, 0x200060);
    printk("Core 1 status: %s\n", (cpsr) ? "Fault" : "Ok");
