                           ...
                           int *found)
{
    ...
    for (tid = first; tid <= last; tid++) {
        ...
        printk("trunk %d: (%s, %d ports)", tid,
               ...
               t_add_info.num_ports);

        if (t_add_info.num_ports > 0) {
#ifdef BCM_TRX_SUPPORT
            if (SOC_IS_TRX(unit)) {
                for (i = 0; i < t_add_info.num_ports; i++) {
                    if (BCM_GPORT_IS_SET(t_add_info.tp[i])) {
                        int tgid, id;
                        rv = _bcm_esw_gport_resolve(unit, t_add_info.tp[i],
                                                    &(t_add_info.tm[i]),
                                                    &(t_add_info.tp[i]),
                                                    &tgid, &id);
                        if ((rv < 0) || (tgid != -1) || (id != -1)) {
                    ...
                    rv = bcm_stk_modmap_map(unit, BCM_STK_MODMAP_SET,
                                            t_add_info.tm[i], t_add_info.tp[i],
                                            &(t_add_info.tm[i]),
                                            &(t_add_info.tp[i]));
                    if (rv < 0) {
                        return CMD_FAIL;
            ...
            for (i = 0; i < t_add_info.num_ports; i++) {
                printk("%s%s",
                       i == 0 ? "=" : ",",
                       mod_port_name(unit, t_add_info.tm[i], t_add_info.tp[i]));
            }

            s = t_add_info.dlf_index < 0 ? "any" :
                mod_port_name(unit,
                              t_add_info.tm[t_add_info.dlf_index],
                              t_add_info.tp[t_add_info.dlf_index]);
            printk(" dlf=%s", s);
            s = t_add_info.mc_index < 0 ? "any" :
                mod_port_name(unit,
                              t_add_info.tm[t_add_info.mc_index],
                              t_add_info.tp[t_add_info.mc_index]);
            printk(" mc=%s", s);
            s = t_add_info.ipmc_index < 0 ? "any" :
                mod_port_name(unit,
                              t_add_info.tm[t_add_info.ipmc_index],
                              t_add_info.tp[t_add_info.ipmc_index]);
            printk(" ipmc=%s", s);
