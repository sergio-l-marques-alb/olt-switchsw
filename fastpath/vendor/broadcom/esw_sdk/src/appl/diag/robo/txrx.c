/*
 * $Id: txrx.c 1.26.2.2 Broadcom SDK $
...
{
    printk("TX callback function: TX pkt = %p done.\n", (void *)pkt);
...
_robo_xd_init(int unit, xd_t *xd)
{
    ...
    if ((xd->pkt_info.alloc_ptr = (uint8 *)soc_cm_salloc(unit,
        ...
        printk("WARNING:  Could not allocate tx buffer.  Memory error.\n");
        xd->xd_init     = FALSE;
        xd->pkt_info.pkt_data = NULL;
 ...
 */
{
    ...
    parse_table_t   pt;
    int         min_len, tagged, a_argc, a_arg;
    char        *xfile;
#ifdef BCM_TB_SUPPORT
    ...
    /* Save args state before sub-parsing ... */
    a_argc = a->a_argc;
    a_arg = a->a_arg;
...
_robo_tx_first_pbm(int unit, bcm_pbmp_t pbm)
{
    ...
    BCM_PBMP_CLEAR(rpbm);
    PBMP_ITER(pbm, p) {
        debugk(DK_TX, "First to port %d\n", p);
        BCM_PBMP_PORT_ADD(rpbm, p);
        return rpbm;
    }

    debugk(DK_TX, "Warning: first pbm null\n");
    BCM_PBMP_CLEAR(rpbm);
...
_robo_tx_next_port(int unit, pbmp_t *newpbm, pbmp_t allpbm)
{
    ...
    *newpbm = _robo_tx_first_pbm(unit, allpbm);
    debugk(DK_TX, "Resetting to pbm %s\n", SOC_PBMP_FMT(*newpbm, pfmt));
    return 1;
}
...
robo_check_pkt_fields(xd_t *xd)
{
    ...
    if (XD_FILE(xd)) {          /* Loaded from file */
        ...
        if (!ENET_TAGGED(ep)) {
            printk("Warning:  Untagged packet read from file for tx.\n");
        ...
        if (xd->xd_vlan) {          /* Tagged format */
            ...
            printk("Warning:  Sending untagged packet.\n");
 ...
 */
{
    ...
    if (pkt_info->pkt_data[0].len != xd->xd_pkt_len) {
        ...
        if ((pkt_info->pkt_data[0].data = (uint8 *)
            ...
            debugk(DK_ERR, "Unable to allocate packet memory\n");
            return(CMD_FAIL);
        }
    ...
    if (xd->xd_ppsm) { /* save base info. setup first port. */
        debugk(DK_TX,"Per port source is active\n");
 ...
 */
{
    ...
    printk("TX Completed %s: TX Requested(%d) Tx Sent(%d)\n",
...
cmd_robo_tx_count(int u, args_t *a)
{
    ...
    printk("TX (%sRunning) Transmit Req(%d) Sent(%d)\n",
...
cmd_robo_tx_start(int u, args_t *a)
{
    ...
    if (!sh_check_attached(ARG_CMD(a), u)) {
        ...
        printk("%s: Error: tx command running\n", ARG_CMD(a));
        return(CMD_FAIL);
    }
    ...
    if (SAL_THREAD_ERROR == sal_thread_create("TX", SAL_THREAD_STKSZ, 100,
        ...
        printk("%s: Error: Failed to create background thread\n",ARG_CMD(a));
        xd->xd_state = XD_IDLE;
        rv = CMD_FAIL;
    }
    /* UN-BLOCK_CNTL_C */
...
cmd_robo_tx_stop(int u, args_t *a)
{
    ...
    if (XD_RUNNING != xd->xd_state) {
        printk("%s: TX not currently running\n", ARG_CMD(a));
        return(CMD_FAIL);
    } else {
        xd->xd_state = XD_STOP;
        printk("%s: TX termination requested\n", ARG_CMD(a));
        return(CMD_OK);
    }
}
...
robo_rx_cb_handler(int unit, bcm_pkt_t *info, void *cookie)
{
    ...
    if (SOC_IS_TBX(unit)) {
#ifdef BCM_TB_SUPPORT
        printk("RX packet %d: unit=%d len=%d rx_port=%d rx_vport=%d cos=%d\n",
           count, unit, info->tot_len, info->rx_port,
           BCM_GPORT_SUBPORT_PORT_GET(info->src_gport),info->cos);
#endif
    } else {
        printk("RX packet %d: unit=%d len=%d rx_port=%d cos=%d\n",
    ...
    if (SOC_IS_TBX(unit)) {
        ...
        if (info->flags & BCM_RX_MIRRORED) {
            printk("Mirrored packet, ");
            rx_flag_print_cnt++;
        }
        if (info->flags & BCM_RX_LEARN_DISABLED) {
            printk("Learning disabled packet");
            rx_flag_print_cnt++;
        }
        ...
        if (info->multicast_group < 0) {
            ...
            printk("RX Multicast id: %d\n", info->multicast_group);
        }
        printk("RX Traffic Class: %d\n", info->prio_int);
        if (info->color == bcmColorPreserve) {
            printk("RX Color: Preserve\n");
        } else if (info->color == bcmColorGreen) {
            printk("RX Color: Green\n");
        } else if (info->color == bcmColorYellow) {
            printk("RX Color: Yellow\n");
        } else {
            printk("RX Color: Red\n");
        }
#endif
    }
    printk("Parsed packet info:\n");
    printk("    src mod=%d. src port=%d. op=%d.\n", info->src_mod,
           info->src_port, info->opcode);
    printk("    dest mod=%d. dest port=%d. chan=%d.\n", info->dest_mod,
 ...
 */
{
    ...
    if (!ARG_CUR(args)) {
        ...
        printk("    Pkt Size %d. Pkts/chain %d. Global PPS %d. Burst %d\n",
               rx_cfg.pkt_size, rx_cfg.pkts_per_chain,
               rx_cfg.global_pps, rx_cfg.max_burst);
         /*printk("    Weights cos 0 to 7:      %d %d %d %d    %d %d %d %d\n",
        ...
        for (chan = 0; chan < BCM_ROBO_RX_CHANNELS; chan++) {
            printk("    Channel %d:  Chains %d. PPS %d.\n",
        ...
        printk("(with mapped CosQ number)\n");
        for (i = 0; i < bcmRxReasonCount; i++) {
            if (BCM_RX_REASON_GET(reasons, i)) {
                ...
                if (rv < 0) {
                    printk("ERROR:  Get Rx mapped cosq number failed: %s\n", bcm_errmsg(rv));
                    return CMD_FAIL;
                }
                printk("%s(%d), ", _reason_names[i], mapped_q);
            }
        }
    ...
    if (sal_strcasecmp(subcmd, "reason") == 0) {
        ...
        for (i = 0; i < bcmRxReasonCount; i++) {
            if (subcmd == NULL ||sal_strcasecmp(subcmd, _reason_names[i]) == 0) {
                matched += 1;
                BCM_RX_REASON_CLEAR_ALL(reasons_q);
                BCM_RX_REASON_SET(reasons_q, i);
                if (value == NULL) {
                    rv = bcm_rx_cosq_mapping_get
                        ...
                        &pkt_type, &pkt_type, &mapped_q);
                    if (rv < 0) {
                        printk("Get Reason code %s mapped CosQ failed: %s\n", 
                            _reason_names[i], bcm_errmsg(rv));
                        return CMD_FAIL;
                    }
                    printk("Get Reason code %s mapped CosQ %d\n", 
                    ...
                    rv = bcm_rx_cosq_mapping_set
                        ...
                        pkt_type, pkt_type, mapped_q);
                    if (rv < 0) {
                        printk("Set CosQ %d mapped Reason code %s failed: %s\n", 
        ...
        if (pkt_type != 0) {
            ...
            if (value == NULL) {
                rv = bcm_rx_cosq_mapping_get
                    ...
                    &pkt_type, &pkt_type, &mapped_q);
                if (rv < 0) {
                    printk("Get Packet type %s mapped CosQ failed: %s\n", 
                        pkt_type_name[pkt_type_idx], bcm_errmsg(rv));
                    return CMD_FAIL;
                }
                printk("Get Packet type %s mapped CosQ %d\n", 
                ...
                rv = bcm_rx_cosq_mapping_set
                    ...
                    pkt_type, pkt_type, mapped_q);
                if (rv < 0) {
                    printk("Set CosQ %d mapped Packet type %s failed: %s\n", 
        ...
        if (matched == 0) {
            printk("%s: ERROR: Unknown reason code or packet type\n", subcmd);
            return CMD_FAIL;
        }
        return CMD_OK;
    }
...
cmd_robo_rx_init(int unit, args_t *args)
{
    ...
    if ((ch = ARG_GET(args)) == NULL) {
        printk("RXINIT requires unit specification\n");
        return CMD_USAGE;
    }
 ...
 */
{
    ...
    if (xd->hdr_mode < 0 || xd->hdr_mode > 0) {
        printk("Current header mode for unit %d is invalid (%d).\n",
               unit, xd->hdr_mode);
    } else {
        printk("Current header mode for unit %d is%s %s (%d).\n",
               unit, (i != -1) ? " now" : "",
               hdr_modes[xd->hdr_mode], xd->hdr_mode);
    }

    return (i < 4) ? CMD_OK : CMD_USAGE;
}
