 ...
 */
{
	...
	ARG_NEXT(a);
	printk("%s: Refreshing configuration database\n", ARG_CMD(a));
	if (sal_config_refresh()) {
	    printk("%s: Failed to refresh configuration database\n",
		   ARG_CMD(a));
	    return CMD_FAIL;
	}
	return CMD_OK;
	...
	if (parsed_value == NULL) {
	    printk("%s: cannot allocate memory for config values\n",
		   ARG_CMD(a));
	    return CMD_FAIL;
	}
	...
	while (sal_config_get_next(&name, &value) >= 0) {
		...
		printk("Internal error in parsing\n");
		return CMD_FAIL;
	    }
	    i += 1;
	}
