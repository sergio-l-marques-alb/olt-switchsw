...
cmd_tcam(int u, args_t * a)
{
	...
	rv = soc_tcam_part_index_to_mem_index(u, part, index, mem, &mem_index);
        ...
        if((cmd == _PHY_PRBS_SET_CMD) || 
            ...
            for(lane=0; lane <= max_lane; ++lane) {
                if (poly <= 3) {
                    rv = wcmod_esm_serdes_control_set(u, 
                            ...
                            &poly);
                    if( rv != BCM_E_NONE) {
                        printk("Setting prbs polynomial failed on Warpcore\n");
                        return CMD_FAIL;
                    }
                    /* Tx enable */
                    rv = wcmod_esm_serdes_control_set(u, 
            ...
            for(lane=12; lane <= max_lane; ++lane) {
                if (poly <= 3) {
                    rv = wcmod_esm_serdes_control_set(u, 
                            ...
                            &poly);
                    if( rv != BCM_E_NONE) {
                        printk("Enabling prbs checker on WC failed\n");
                        return CMD_FAIL;
                    }
                    /* Tx enable */
                    rv = wcmod_esm_serdes_control_set(u, 
            ...
            printk("CHECKING PRBS: TR3 Warpcore Serdes\n");
            max_lane = 23;
            for(lane=12; lane <= max_lane; ++lane) {
                rv = wcmod_esm_serdes_control_get(u, 
                        ...
                        &status);
                if( rv != BCM_E_NONE) {
                    printk("WC: Getting prbs status failed (lane:%d) \n", lane);
                    return CMD_FAIL;
                }
                switch (status) {
                    case 0:
                        ...
                        break;
                    case -1:
                        printk("\t->WC PRBS Failed! (%d)\n", lane);
                        prbs_pass = 0;
                        break;
                    default:
                        printk("\t->WC PRBS has %d errors! (%d)\n", lane, status);
                        prbs_pass = 0;
                        break;
                }
            }         
