/*
 * $Id: memory.c 1.70.2.8 Broadcom SDK $
...
#define _ERR_MSG_MODULE_NAME SOC_DBG_NORMAL
...
#endif /* BCM_POLAR_SUPPORT */
    {
        ...
        if (soc_mem_array_write(parm->unit, parm->mem, array_index,
            ...
            if (parm->array_index_start != 0 || parm->array_index_end != parm->array_index_start) {
                    printk("Write ERROR: table %s[%u].%s[%d]\n",
                           SOC_MEM_UFNAME(parm->unit, parm->mem), array_index,
       ...
       int copyno, int index, uint32 *entry_data)
{
...
#endif /* BCM_POLAR_SUPPORT */
    {
        ...
        if (soc_mem_array_read(parm->unit, parm->mem, array_index,
                 copyno, index, entry_data) < 0) {
            if (parm->array_index_start != 0 || parm->array_index_end != parm->array_index_start) {
                    printk("Read ERROR: table %s[%u].%s[%d]\n",
                           SOC_MEM_UFNAME(parm->unit, parm->mem), array_index,
                           SOC_BLOCK_NAME(parm->unit, copyno), index);
            } else {
...
mem_test_default_init(int u, soc_mem_t mem, void **p)
{
    ...
    if (mw == NULL) {
        ...
        if (mw == NULL) {
            printk("cannot allocate memory test data\n");
            return -1;
        }
    ...
    if (parse_defualt_fill(&pt) < 0) {
      printk("Invalid defulat fill:\n");
      goto done;
    }
...
#ifdef BCM_ARAD_SUPPORT
        || 
        (SOC_IS_ARAD(u) && (soc_mem_is_writeonly(u, parm->mem) || soc_mem_is_signal(u, parm->mem)))
#endif
        ) {
#ifdef BCM_POLAR_SUPPORT
        if (SOC_IS_POLAR(u)) {
...
memories_rw_test(int unit, args_t *a, void *pa)
{
        ...
        debugk(DK_TESTS, "Memories read/write test\n");
        ...
        if (SOC_IS_ARAD(unit))
        {
          ...
          if ((rv = soc_dpp_device_reset(unit, SOC_DPP_RESET_MODE_REG_ACCESS,SOC_DPP_RESET_ACTION_INOUT_RESET)) < 0) {
              debugk(DK_ERR, "ERROR: Unable to reinit unit %d \n", unit);
...
enable_dynamic_memories_access(int unit)
{
  ...
  int i;
    ...
    _SOC_IF_ERR_EXIT(WRITE_EGQ_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_EPNI_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_IHB_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_IPS_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_IPT_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    _SOC_IF_ERR_EXIT(WRITE_IQM_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
    ...
    _SOC_IF_ERR_EXIT(WRITE_IHP_INDIRECT_WR_MASKr(unit,write_val));/* 32 */
    _SOC_IF_ERR_EXIT(WRITE_IPT_INDIRECT_WR_MASKr(unit,write_val));/* 32 */
    _SOC_IF_ERR_EXIT(WRITE_EPNI_INDIRECT_WR_MASKr(unit,write_val));/* 16 */
    _SOC_IF_ERR_EXIT(WRITE_IHB_INDIRECT_WR_MASKr(unit,write_val));/* 16 */
    ...
    _SOC_IF_ERR_EXIT(WRITE_EGQ_INDIRECT_WR_MASKr(unit,write_val_above64));/*64 */
    ...
    for (i=0; i<4; i++) {
        write_val = 0;
        _SOC_IF_ERR_EXIT(READ_CRPS_CRPS_PREFETCH_TIMERS_CFGr(unit, i, &write_val));
        ...
        _SOC_IF_ERR_EXIT(WRITE_CRPS_CRPS_PREFETCH_TIMERS_CFGr(unit, i, write_val));
    }
 ...
 */
int 
disable_ecc_disable_dynamic_mechanism(int device_id)
{
#ifdef BCM_ARAD_SUPPORT
  uint32 res;
  uint32 write_val = 1;
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, WRITE_CRPS_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_EGQ_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_EPNI_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, WRITE_IDR_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 50, exit, WRITE_IHB_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 60, exit, WRITE_IPS_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, WRITE_IPT_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 80, exit, WRITE_IQM_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 90, exit, WRITE_IRE_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit, WRITE_IRR_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 105, exit, WRITE_OLP_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit,write_val));

  ARAD_FIELD_SET(NBI_STATISTICSr,STAT_USE_GLOBAL_INDIRECTf, 1 ,110, exit);
  ARAD_FIELD_SET(CFC_REG_00CFr,FIELD_16_16f, 1 ,120, exit);
  ARAD_FIELD_SET(CRPS_REG_00CEr,FIELD_16_16f, 1 ,130, exit);
  ARAD_FIELD_SET(ECI_GLOBALFr,CPU_BYPASS_ECC_PARf, 1 ,140, exit);
  ARAD_FIELD_SET(EGQ_REG_00CFr,FIELD_16_16f, 1 ,150, exit);
  ARAD_FIELD_SET(EPNI_REG_00CFr,FIELD_16_16f, 1 ,160, exit);
  ARAD_FIELD_SET(FCR_REG_00CFr,FIELD_16_16f, 1 ,170, exit);
  ARAD_FIELD_SET(FDT_REG_00CFr,FIELD_16_16f, 1 ,180, exit);
  ARAD_FIELD_SET(IDR_REG_00CFr,FIELD_16_16f, 1 ,190, exit);
/* WRITE_BIT_DISABLE_DYNAMIC(unit , IDR_REG_00CFr , FIELD_16_16f);*/
  ARAD_FIELD_SET(IHB_REG_008Ar,FIELD_16_16f, 1 ,200, exit);
  ARAD_FIELD_SET(IHP_REG_00CFr,FIELD_16_16f, 1 ,210, exit);
  ARAD_FIELD_SET(IPS_REG_00C6r,FIELD_16_16f, 1 ,220, exit);
  ARAD_FIELD_SET(IPT_REG_00CFr,FIELD_16_16f, 1 ,230, exit);
  ARAD_FIELD_SET(IQM_REG_00CDr,FIELD_16_16f, 1 ,240, exit);
  ARAD_FIELD_SET(IRE_REG_00CFr,FIELD_16_16f, 1 ,250, exit);
  ARAD_FIELD_SET(IRR_REG_00CFr,FIELD_16_16f, 1 ,260, exit);
  ARAD_FIELD_SET(MMU_REG_00CFr,FIELD_16_16f, 1 ,270, exit);
  ARAD_FIELD_SET(NBI_REG_00CFr,FIELD_16_16f, 1 ,280, exit);
  ARAD_FIELD_SET(OAMP_REG_00CFr,FIELD_16_16f, 1 ,290, exit);
  ARAD_FIELD_SET(OLP_REG_00F0r,FIELD_16_16f, 1 ,300, exit);
  ARAD_FIELD_SET(CFC_REG_00CFr,FIELD_15_15f, 1 ,310, exit);
  ARAD_FIELD_SET(CRPS_REG_00CEr,FIELD_15_15f, 1 ,320, exit);
  ARAD_FIELD_SET(ECI_GLOBALFr, DIS_ECCf, 1 ,330, exit);
  ARAD_FIELD_SET(EGQ_REG_00CFr, FIELD_15_15f, 1 ,340, exit);
  ARAD_FIELD_SET(EPNI_REG_00CFr, FIELD_15_15f, 1 ,350, exit);
  ARAD_FIELD_SET(FCR_REG_00CFr, FIELD_15_15f, 1 ,360, exit);
  ARAD_FIELD_SET(FDT_REG_00CFr, FIELD_15_15f, 1 ,370, exit);
  ARAD_FIELD_SET(IDR_REG_00CFr, FIELD_15_15f, 1 ,380, exit);
  ARAD_FIELD_SET(IHB_REG_008Ar, FIELD_15_15f, 1 ,390, exit);
  ARAD_FIELD_SET(IHP_REG_00CFr, FIELD_15_15f, 1 ,400, exit);
  ARAD_FIELD_SET(IPS_REG_00C6r, FIELD_15_15f, 1 ,410, exit);
  ARAD_FIELD_SET(IPT_REG_00CFr, FIELD_15_15f, 1 ,420, exit);
  ARAD_FIELD_SET(IQM_REG_00CDr, FIELD_15_15f, 1 ,430, exit);
  ARAD_FIELD_SET(IRE_REG_00CFr, FIELD_15_15f, 1 ,440, exit);
  ARAD_FIELD_SET(IRR_REG_00CFr, FIELD_15_15f, 1 ,450, exit);
  ARAD_FIELD_SET(MMU_REG_00CFr, FIELD_15_15f, 1 ,460, exit);
  ARAD_FIELD_SET(NBI_REG_00CFr, FIELD_15_15f, 1 ,470, exit);
  ARAD_FIELD_SET(OAMP_REG_00CFr, FIELD_15_15f, 1 ,480, exit);
  ARAD_FIELD_SET(OLP_REG_00F0r, FIELD_15_15f, 1 ,490, exit);
  ARAD_FIELD_SET(IPS_IPS_GENERAL_CONFIGURATIONSr, DISCARD_ALL_IQM_MSGf, 1 ,500, exit);
  ARAD_FIELD_SET(IPS_IPS_GENERAL_CONFIGURATIONSr, DISCARD_ALL_CRDTf, 1 ,510, exit);
  ARAD_FIELD_SET(IPS_IPS_GENERAL_CONFIGURATIONSr, DISABLE_STATUS_MSG_GENf, 1 ,520, exit);
  ARAD_FIELD_SET(IPS_CREDIT_WATCHDOG_CONFIGURATIONr, CR_WD_MIN_SCAN_CYCLE_PERIODf, 0 ,530, exit);
  ARAD_FIELD_SET(IDR_GLBL_CFGr, MCDA_INITf, 1 ,540, exit);
  ARAD_FIELD_SET(IDR_GLBL_CFGr, MCDB_INITf, 1 ,550, exit);
  ARAD_FIELD_SET(SCH_SYSTEM_RED_CONFIGURATIONr, AGING_TIMER_CFGf, 0 ,560, exit);
  ARAD_FIELD_SET(SCH_DVS_CONFIG_0r, NIF_FORCE_PAUSEf, 0xffffffff ,570, exit);
  ARAD_FIELD_SET(SCH_SCHEDULER_CONFIGURATION_REGISTERr, DISABLE_FABRIC_MSGSf, 1 ,580, exit);
  ARAD_FIELD_SET(MMU_REG_00C1r, FIELD_2_2f, 0 ,590, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_6_6f, 1 ,600, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_5_5f, 1 ,610, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_4_4f, 1 ,620, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_3_3f, 1 ,630, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_2_2f, 1 ,640, exit);
  ARAD_FIELD_SET(MMU_REG_0102r, FIELD_1_1f, 1 ,650, exit);
  ARAD_FIELD_ASET(CRPS_CRPS_CNT_SRC_CFGr, 0, CRPS_N_CNT_SRCf, 0x12, 660, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_CNT_SRC_CFGr, 1, CRPS_N_CNT_SRCf, 0x12, 670, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_CNT_SRC_CFGr, 2, CRPS_N_CNT_SRCf, 0x12, 680, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_CNT_SRC_CFGr, 3, CRPS_N_CNT_SRCf, 0x12, 690, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 0, CRPS_N_CPU_CLEAR_READ_ENf, 0, 700, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 0, CRPS_N_BUBBLE_ENf, 0, 710, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 1, CRPS_N_CPU_CLEAR_READ_ENf, 0, 720, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 1, CRPS_N_BUBBLE_ENf, 0, 730, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 2, CRPS_N_CPU_CLEAR_READ_ENf, 0, 740, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 2, CRPS_N_BUBBLE_ENf, 0, 750, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 3, CRPS_N_CPU_CLEAR_READ_ENf, 0, 760, exit );
  ARAD_FIELD_ASET(CRPS_CRPS_GENERAL_CFGr, 3, CRPS_N_BUBBLE_ENf, 0, 770, exit );
	
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 780, exit, WRITE_CMIC_SBUS_TIMEOUTr(unit, 0x30000));
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in disable_ecc_disable_dynamic_mechanism()", 0, 0);
#else
  return 0;
#endif
}
...
memories_rw_first_last_test(int unit, args_t *a, void *pa)
{
   ...
   COMPILER_REFERENCE(pa);
        ...
        debugk(DK_TESTS, "Memories read/write first and last test\n");
        ...
        if (SOC_IS_ARAD(unit))
        {
...
#endif
          {
              ...
              debugk(DK_TESTS, "Skip device reset\n");
          }
   ...
   help_status = 0;
        ...
        if (xd_usefile > 0) {
            ...
            if (tr8_open_dump_file(unit,file_name ) == -1) {
                printk("Error open dump file %s, dump file disabled\n", file_name);
            }
        } 
        ...
        /* COMPILER_REFERENCE(pa); */
        debugk(DK_TESTS, "Memories read/write first and last test, %s\n",ctime((const time_t *)&current_time));
        ...
        if (SOC_IS_ARAD(unit))
        {
            if (tr8_data.skip_reset_and_write  != 1) {     
              ...
              if (BCM_E_NONE == result) {
                  printk("unit %d counter processor background accesses suspended\n", unit);
              } else {
                  printk("unit %d counter processor background access suspend failed: %d (%s)\n",
                         unit,
                         result,
                         _SHR_ERRMSG(result));
              }
...
int ser_test(int unit, args_t *arg, void *p) {
	...
	uint32 my_debug_level = DK_ERR;
	_soc_ser_test_t test_type = SER_SINGLE_INDEX;
	for ( ; ; ) {
		parameter = ARG_GET(arg);
		if (parameter == NULL) {
			break;
		} else if (!sal_strcasecmp(parameter, "single")) {
			test_type = SER_SINGLE_INDEX;
		} else if (!sal_strcasecmp(parameter, "spread")) {
			test_type = SER_FIRST_MID_LAST_INDEX;
		} else if (!sal_strcasecmp(parameter, "all")) {
			test_type = SER_ALL_INDEXES;
		} else if (!sal_strcasecmp(parameter, "verbose")) {
			debugk_enable(DK_ERR);
			my_debug_level = 0;
		} else if (!(parse_memory_name(unit,&mem,parameter,&copyno,0) >=0)) {
			soc_cm_print("Invalid parameters.\n");
			soc_cm_print("Valid usage is: 'tr <testNum> [verbose] [Memory Name] [Index options]'\n");
			soc_cm_print("Valid index options are: single, spread, all.\n");
			return SOC_E_FAIL;
		}
	}
	(void)test_type; /*Currently required to avoid iproc build errors.*/
	/*Find the current memscan properties*/
#ifdef INCLUDE_MEM_SCAN
...
#endif /*INCLUDE_MEM_SCAN*/
	debugk_disable(my_debug_level);
	if (mem != INVALIDm) {
#ifdef BCM_TRIUMPH2_SUPPORT
		rv = soc_tr2_ser_mem_test(unit,mem, test_type);
#endif/*BCM_TRIUMPH2_SUPPORT*/
	} else {
#ifdef BCM_TRIUMPH2_SUPPORT
		rv = soc_tr2_ser_test(unit,test_type);
#endif/*BCM_TRIUMPH2_SUPPORT*/
	}
	debugk_enable(my_debug_level);
#ifdef INCLUDE_MEM_SCAN
	if (memscan_running) {
...
#endif /*defined(BCM_TRIUMPH2_SUPPORT)*/
