/* 
 * $Id: brd_chassis.c 1.5 Broadcom SDK $
                         ...
                         cpudb_base_t  *cpudb_base_ptr, bcm_ip_t *ip_addr)
{
    ...
    if ((slot < 0) || (slot >= max_slots )) {
        TOPO_ERR(("ATP-Socket ERROR: Invalid destination slot %d\n", slot));
        return BCM_E_FAIL;
    }
...
bcm_board_chassis_atptrans_socket_update(cpudb_ref_t new_db, cpudb_ref_t old_db)
{
    ...
    TOPO_VERB(("TKS ATP-Socket Update: new_db %p. old_db %p\n",
             new_db, old_db));
    ...
    if (new_db != NULL) {
        ...
        CPUDB_FOREACH_ENTRY(new_db, new_cpu_ptr) {
            ...
            if (BCM_SUCCESS(rv)) {
                ...
                TOPO_ERR(("Invalid IP address.  Cannot install ATP over "
...
int
bcm_board_topo_lm_xgs3_12x(topo_cpu_t *tp_cpu, cpudb_ref_t db_ref)
{
    return chassis_run(tp_cpu, db_ref);
}
