/*
 * $Id: idxres_mdb.c 1.8 Broadcom SDK $
...
#define _MDB_MESSAGE_ENABLE FALSE
#define _MDB_EXCESS_VERBOSITY TRUE
#if 0 
#undef BCM_DEBUG
#define BCM_DEBUG(flags, stuff)   soc_cm_print stuff
#endif 
#if _MDB_MESSAGE_ENABLE
#define MDB_WARN(stuff)        BCM_DEBUG(BCM_DBG_WARN     , stuff)
#define MDB_ERR(stuff)         BCM_DEBUG(BCM_DBG_ERR      , stuff)
#define MDB_VERB(stuff)        BCM_DEBUG(BCM_DBG_VERBOSE  , stuff)
#define MDB_VVERB(stuff)       BCM_DEBUG(BCM_DBG_VVERBOSE , stuff)
#if _MDB_EXCESS_VERBOSITY
#define MDB_EVERB(stuff)       BCM_DEBUG(BCM_DBG_VVERBOSE , stuff)
#else /* _MDB_EXCESS_VERBOSITY */
#define MDB_EVERB(stuff)
#endif /* _MDB_EXCESS_VERBOSITY */
#else /* _MDB_MESSAGE_ENABLE */
#define MDB_WARN(stuff)
#define MDB_ERR(stuff)
#define MDB_VERB(stuff)
#define MDB_VVERB(stuff)
#define MDB_EVERB(stuff)
#endif /* _MDB_MESSAGE_ENABLE */
#if defined(__GNUC__) && !defined(__PEDANTIC__)
#define MDB_MSG(string) "%s[%d]%s" string, __FILE__, __LINE__, __FUNCTION__
#define MDB_MSG1(string) "%s[%d]%s: " string, __FILE__, __LINE__, __FUNCTION__
#else /* defined(__GNUC__) && !defined(__PEDANTIC__) */
#define MDB_MSG(string) "%s[%d]" string, __FILE__, __LINE__
#define MDB_MSG1(string) "%s[%d]: " string, __FILE__, __LINE__
...
#define _MDB_VALID_CHECK(_mdb) \
    if (!(_mdb)) { \
        MDB_ERR((MDB_MSG1("NULL is not a valid handle\n"))); \
        return BCM_E_PARAM; \
    }
#define _MDB_LOCK_TAKE(_mdb) \
    if ((_mdb)->lock) { \
        if (sal_mutex_take((_mdb)->lock,sal_mutex_FOREVER)) { \
            MDB_ERR((MDB_MSG1("unable to take mdb %08X lock\n"), \
                     (uint32)_mdb)); \
...
#define _MDB_LOCK_GIVE(_mdb) \
    if ((_mdb)->lock) { \
        if (sal_mutex_give((_mdb)->lock)) { \
            MDB_ERR((MDB_MSG1("unable to release mdb %08X lock\n"), \
                     (uint32)_mdb)); \
            return BCM_E_INTERNAL; \
        } \
    }
 ...
 *    (in) int prepAll = TRUE to prep all elements, FALSE for just head
 *  Return
 *    void
 *  Notes
                ...
                int prepAll)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,%d,%s)\n"),
               (uint32)res,
               head,
               count,
               prepAll?"PrepAll":"PrepHead"));
                      ...
                      shr_mdb_elem_index_t *extra)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,%d,%s,*,*) enter\n"),
               (uint32)res,
               head,
               count,
               high?"High":"Low"));
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,%d,%s,&(%08X),&(%08X)) leave\n"),
               (uint32)res,
               head,
               count,
               high?"High":"Low",
                       ...
                       shr_mdb_elem_index_t second)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,%08X) enter\n"),
               (uint32)res,
               head,
               second));
                         ...
                         shr_mdb_elem_index_t head)
{
    ...
    while (count) {
        ...
        MDB_EVERB((MDB_MSG1("mdb %08X: block at %08X has %d elements;"
                            " freelist = %d\n"),
                   (uint32)res,
                   oldHead,
                   count,
                  ...
                  shr_mdb_alloc_pref_t override)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,&(%08X)) enter\n"),
               (uint32)res,
               *head));
    ...
    if (pref & shr_mdb_join_low) {
        ...
        while (offs) {
            ...
            MDB_EVERB((MDB_MSG1("join downward to %08X (%d elements)\n"),
                       nextBlock,
                       count));
            offs -= count;
    ...
    if ((pref & shr_mdb_join_high) ||
        ...
        MDB_EVERB((MDB_MSG1("current %08X count %d next %08X mask %08X\n"),
                   *head,
                   count,
                   *head + count,
                   res->bankMask));
        while (((offs + count) <= (~(res->bankMask))) &&
            ...
            MDB_EVERB((MDB_MSG1("join upward to %08X (%d elements)\n"),
                       nextBlock,
    ...
    MDB_VVERB((MDB_MSG("(%08X,&(%08X)) leave\n"),
               (uint32)res,
               *head));
}
                    ...
                    shr_mdb_elem_index_t *head)
{
    ...
    if (_MDB_BLOCK_NOT_HEAD != res->elem[*head].list) {
        ...
        MDB_VVERB((MDB_MSG("(%08X,&(%08X)) early leave (head, %d)\n"),
                   (uint32)res,
                   *head,
                   res->elem[*head].count));
        return;
    ...
    if (0 != res->elem[*head].count) {
        ...
        *head -= (res->elem[*head].count - 1);
        MDB_VVERB((MDB_MSG("(%08X,&(%08X)) early leave (tail, %d)\n"),
                   (uint32)res,
                   *head,
                   res->elem[*head].count));
        return;
    ...
    for (currBlock = ((*head) & (res->bankMask));
        ...
        MDB_EVERB((MDB_MSG1("mdb %08X: block at %08X length %d\n"),
                   (uint32)res,
                   currBlock,
                     ...
                     shr_mdb_elem_bank_index_t count)
{
    ...
    int result = BCM_E_RESOURCE;

    MDB_VVERB((MDB_MSG("(%08X,*,%d) enter\n"),
               (uint32)res,
               count));
    ...
    if (count > res->list[res->freeLists - 1].elemsBlock) {
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: count %d exceeds largest block %d\n"),
                 (uint32)res,
                 count,
    ...
    MDB_EVERB((MDB_MSG1("mdb %08X: looking for free block >= %d elements\n"),
               (uint32)res,
    ...
    case shr_mdb_alloc_bank_first:
        ...
        MDB_EVERB((MDB_MSG1("mdb %08X: using '%s' alloc mode\n"),
                   (uint32)res,
                   "first"));
    ...
    default:
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: unknown alloc mode: %d\n"),
                 (uint32)res,
    ...
    if (BCM_E_NONE == result) {
        ...
        MDB_EVERB((MDB_MSG1("mdb %08X: found suitable block at %08X (%d elements)\n"),
                   (uint32)res,
                   block + res->low,
    ...
    MDB_VVERB((MDB_MSG("(%08X,%d,&(%08X)) return %d (%s)\n"),
               (uint32)res,
               count,
               *head,
               result,
                    ...
                    shr_mdb_elem_index_t head)
{
    ...
    if ((head < res->low) || (block >= res->count)) {
        MDB_ERR((MDB_MSG1("mdb %08X: invalid block at %08X can not be freed\n"),
                 (uint32)res,
                 head));
    ...
    if (res->elem[block].list < res->freeLists) {
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: block at %08X is already free\n"),
                 (uint32)res,
                 head));
        return BCM_E_NOT_FOUND;
    }
    if (_MDB_BLOCK_NOT_IN_LIST != res->elem[block].list) {
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: block at %08X is in a list\n"),
                 (uint32)res,
                 head));
        return BCM_E_BUSY;
    }
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X) return %d (%s)\n"),
               (uint32)res,
               head,
               BCM_E_NONE,
                    ...
                    shr_mdb_elem_bank_index_t *count)
{
    ...
    if ((head < res->low) || (block >= res->count)) {
        MDB_ERR((MDB_MSG1("mdb %08X: element %08X is not valid\n"),
                 (uint32)res,
                 head));
        /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
        return BCM_E_NOT_FOUND;
    ...
    if (res->freeLists > res->elem[block].list) {
        MDB_ERR((MDB_MSG1("mdb %08X: block at %08X..%08X is free\n"),
                 (uint32)res,
                 block + res->low,
    ...
    *count = res->elem[block].count;
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,&(%d)) return %d (%s)\n"),
               (uint32)res,
               head,
               *count,
               BCM_E_NONE,
                       ...
                       shr_mdb_elem_index_t count)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,%d) enter\n"),
               (uint32)res,
               head,
               count));
    ...
    if ((head < res->low) ||
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: can not reserve %d elements at %08X since"
    ...
    if (!count) {
        /* do nothing? */
        MDB_ERR((MDB_MSG1("mdb %08X: can not reserve zero element range\n"),
                 (uint32)res));
        return BCM_E_PARAM;
    }
    ...
    while (currElem < block + count) {
        if (res->elem[currElem].list >= res->freeLists) {
            ...
            MDB_ERR((MDB_MSG1("mdb %08X: can't reserve in-use block"
                              " at %08X\n"),
                     (uint32)res,
                     currElem));
    ...
    while (currElem < (block + count)) {
        ...
        if (block > currElem) {
            MDB_EVERB((MDB_MSG1("mdb %08X: break off low elements"
                                " %08X..%08X\n"),
                       (uint32)res,
                       currElem + res->low,
                       head - 1));
        ...
        if ((currElem + currCount) > (block + count)) {
            MDB_EVERB((MDB_MSG1("mdb %08X: break off high elements"
                                " %08X..%08X\n"),
                       (uint32)res,
                       head + count,
                       ...
                       shr_mdb_elem_bank_index_t count)
{
    ...
    if ((head < res->low) || ((head + count - res->low) > res->count)) {
        /* head or count places part or all of the block outside range */
        MDB_ERR((MDB_MSG1("mdb %08X: can not collect %d elements at %08X since"
                          " the range contains invalid elements\n"),
                 (uint32)res,
                 count,
                 head));
    ...
    if ((_MDB_OFFS_FROM_ELEM(*res, block) + count) > ((~(res->bankMask)) + 1)) {
        MDB_ERR((MDB_MSG1("mdb %08X: %d element block at %08X"
                          " would span banks\n"),
                 (uint32)res,
                 count,
                 head));
    ...
    for (currElem = block;
        ...
        if (res->elem[currElem].list == _MDB_BLOCK_NOT_HEAD) {
            ...
            MDB_ERR((MDB_MSG1("mdb %08X: can't collect non-single-element"
                              " block at %08X\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_PARAM;
        ...
        if (res->elem[currElem].list < res->freeLists) {
            /* this block is already free */
            MDB_ERR((MDB_MSG1("mdb %08X: can't collect free"
                              " block at %08X\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_NOT_FOUND;
        ...
        if (res->elem[currElem].list != _MDB_BLOCK_NOT_IN_LIST) {
            ...
            MDB_ERR((MDB_MSG1("mdb %08X: can't collect block at %08X since"
                              " it is in a list\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_BUSY;
        ...
        if (res->elem[currElem].count != 1) {
            ...
            MDB_ERR((MDB_MSG1("mdb %08X: can't collect %d-element"
                              " block at %08X\n"),
                     (uint32)res,
    ...
    _mdb_block_prep(res, block, count, TRUE);
                         ...
                         shr_mdb_elem_index_t count)
{
    ...
    if ((head < res->low) || ((head + count - res->low) > res->count)) {
        /* head or count places part or all of the block outside range */
        MDB_ERR((MDB_MSG1("mdb %08X: can not unreserve %d elements at"
    ...
    for (currElem = block;
        ...
        if (res->elem[currElem].list == _MDB_BLOCK_NOT_HEAD) {
            /* non-head elements do not exist in one-element blocks */
            MDB_ERR((MDB_MSG1("mdb %08X: can't unreserve non-single-element"
                              " block at %08X\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_PARAM;
        ...
        if (res->elem[currElem].list < res->freeLists) {
            /* this block is already free */
            MDB_ERR((MDB_MSG1("mdb %08X: can't unreserve free"
                              " block at %08X\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_NOT_FOUND;
        ...
        if (res->elem[currElem].list != _MDB_BLOCK_NOT_IN_LIST) {
            /* the block is in a list */
            MDB_ERR((MDB_MSG1("mdb %08X: can't unreserve block at %08X since"
                              " it is in a list\n"),
                     (uint32)res,
                     currElem + res->low));
            return BCM_E_BUSY;
        ...
        if (res->elem[currElem].count != 1) {
            /* the block size is not one */
            MDB_ERR((MDB_MSG1("mdb %08X: can't unreserve %d-element"
                              " block at %08X\n"),
                     (uint32)res,
                     res->elem[currElem].count,
                     currElem + res->low));
    ...
    while (count > 0) {
        ...
        /* prepare the newly gathered block */
        _mdb_block_prep(res, currElem, currCount, TRUE);
                        ...
                        shr_mdb_elem_bank_index_t count)
{
    ...
    if ((head < res->low) ||
        ((head + count - res->low) > res->count)) {
        /* head or count places part or all of the block outside range */
        MDB_ERR((MDB_MSG1("mdb %08X: can not alloc %d elements at %08X since"
                          " the range contains invalid elements\n"),
                 (uint32)res,
                 count,
                 head));
    ...
    if (res->freeLists <= res->elem[currElem].list) {
        MDB_ERR((MDB_MSG1("mdb %08X: can not allocate %d elements at %08X since"
    ...
    if ((currElem > block) ||
        ...
        _mdb_block_frag_and_free(res, currElem);
        MDB_ERR((MDB_MSG1("mdb %08X: unable to alloc %d elements at %08X"
                      ...
                      shr_mdb_elem_index_t head)
{
    ...
    MDB_VVERB((MDB_MSG("(%08X,%d,%08X) enter\n"),
               (uint32)res,
               list,
               head));
    ...
    if (list >= res->userLists) {
        MDB_ERR((MDB_MSG1("mdb %08X: list %d is not valid\n"),
                 (uint32)res,
                 list));
        /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
        return BCM_E_NOT_FOUND;
    ...
    if (_MDB_BLOCK_NOT_IN_LIST != res->elem[block].list) {
        MDB_ERR((MDB_MSG1("mdb %08X: block at %08X..%08X in list %d\n"),
                 (uint32)res,
                 block + res->low,
                 block + res->elem[block].count + res->low - 1,
                      ...
                      shr_mdb_elem_index_t head)
{
    ...
    if (_MDB_BLOCK_NOT_IN_LIST == res->elem[block].list) {
        MDB_ERR((MDB_MSG1("mdb %08X: block at %08X..%08X not in a list\n"),
                 (uint32)res,
                 block + res->low,
                 block + res->elem[block].count + res->low - 1));
        return BCM_E_EMPTY;
                    ...
                    shr_mdb_elem_index_t *head)
{
    ...
    if (list >= res->userLists) {
        MDB_ERR((MDB_MSG1("mdb %08X: there is no list %d\n"),
                 (uint32)res,
                 list));
        /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
        return BCM_E_NOT_FOUND;
    ...
    if (_MDB_BANK_END == bank) {
        ...
        MDB_ERR((MDB_MSG1("mdb %08X: list %d has no member blocks\n"),
                 (uint32)res,
                 list));
        return BCM_E_EMPTY;
    }
               ...
               shr_mdb_elem_index_t *pred)
{
    ...
    if (_MDB_BLOCK_END == offs) {
        ...
        if (_MDB_BANK_END == bank) {
            ...
            MDB_ERR((MDB_MSG1("mdb %08X: block at %08X is head of list %d\n"),
                     (uint32)res,
                     current,
                     thisElem->list));
            return BCM_E_NOT_FOUND;
    ...
    *pred = _MDB_ELEM_FROM_BANK_OFFS(*res, bank, offs);
    ...
    MDB_VVERB((MDB_MSG("(%08X,%08X,&(%08X)) return %d (%s)\n"),
               (uint32)res,
               current,
               *pred,
               BCM_E_NONE,
                     ...
                     shr_mdb_elem_bank_index_t list)
{
    ...
    if (_MDB_BANK_END == nextBank) {
        ...
        MDB_VVERB((MDB_MSG1("mdb %08X: user list %d has no member blocks\n"),
                 (uint32)res,
                 list));
        return BCM_E_NONE;
    }
    ...
    while (_MDB_BANK_END != nextBank) {
        ...
        MDB_EVERB((MDB_MSG1("mdb %08X: user list %d has blocks in bank %04X,"
                            " head %04X\n"),
                   (uint32)res,
                   list,
                   bank,
        ...
        while (_MDB_BLOCK_END != nextOffs) {
            ...
            MDB_EVERB((MDB_MSG1("mdb %08X: list %d has block to free at"
    ...
    MDB_VVERB((MDB_MSG("(%08X,%d) return %d (%s)\n"),
               (uint32)res,
               list,
               BCM_E_NONE,
               _SHR_ERRMSG(BCM_E_NONE)));
               ...
               int lock)
{
    ...
    MDB_VERB((MDB_MSG("(*,%d,%d,*,%d,%d,%d,%s) enter\n"),
              bankSize,
              freeLists,
              userLists,
              first,
    ...
    if (!size || (32768 < size)) {
        ...
        MDB_ERR((MDB_MSG1("requested bank size %d is too large\n"),
                 bankSize));
        return BCM_E_PARAM;
    }
    if (last <= first) {
        /* there are too few elements (one, zero, or negative) to bother */
        MDB_ERR((MDB_MSG1("range has negative, zero, or one element\n")));
        return BCM_E_PARAM;
    }
    if ((freeLists + userLists) > 255) {
        /* there are too many lists */
        MDB_ERR((MDB_MSG1("there are too many lists (free+user = %d, which is"
                          " > 255)\n"),
                 freeLists + userLists));
        return BCM_E_PARAM;
    }
    if (!handle) {
        ...
        MDB_ERR((MDB_MSG1("NULL pointer for out argument\n")));
        return BCM_E_PARAM;
    }
    /* keep the decided bank size */
    bankSize = size;
    /* determine how many banks are needed */
    banks = ((last - first) + bankSize) / bankSize;
    if (banks > 0xFFF0) {
        /* there are too many banks */
        MDB_ERR((MDB_MSG1("there are too many banks (%d)\n"),
                 banks));
        return BCM_E_PARAM;
    }
    ...
    /* try to allocate it */
    MDB_VVERB((MDB_MSG1("allocate %d bytes for descriptor\n"),
               size));
    tempList = sal_alloc(size, "mdb_management");
    if (!tempList) {
        MDB_ERR((MDB_MSG1("unable to allocate %d bytes for descriptor\n"),
                 size));
        return BCM_E_MEMORY;
    }
    /* looks good so far */
    ...
    /* prepare it */
    if (lock) {
        MDB_VVERB((MDB_MSG1("create lock\n")));
        tempList->lock = sal_mutex_create("mdb_management_lock");
        if (!(tempList->lock)) {
            MDB_ERR((MDB_MSG1("unable to create lock\n")));
            result = BCM_E_RESOURCE;
        }
    } else {
        tempList->lock = NULL;
    }
    MDB_VVERB((MDB_MSG1("prepare top-level descriptor\n")));
    tempList->low = first;
    ...
    tempList->lastBankSize = (banks * bankSize) - elems;
    if (0 == tempList->lastBankSize) {
        tempList->lastBankSize = bankSize;
    }
    ...
    if (BCM_E_NONE == result) {
        ...
        MDB_VVERB((MDB_MSG1("prepare list descriptors\n")));
        bankList.blocks = 0;
        bankList.elems = 0;
        ...
        for (list = 0; list < lists; list++) {
            ...
            if (0 == list) {
                MDB_VVERB((MDB_MSG1("obligatory free list %d:"
                                    " blocks of 1 element\n"),
                           list));
                tempList->list[list].elemsBlock = 1;
            } else if (list <= freeLists) {
                MDB_VVERB((MDB_MSG1("additional free list %d:"
                ...
                if (tempList->list[list].elemsBlock <=
                    ...
                    MDB_ERR((MDB_MSG1("block size in user specified free lists"
                                      " is not strictly increasing\n")));
                    result = BCM_E_PARAM;
                    break;
                }
                if (tempList->list[list].elemsBlock > bankSize) {
                    MDB_ERR((MDB_MSG1("block size %d exceeds bank size %d\n"),
                ...
                MDB_VVERB((MDB_MSG1("user list %d\n"),
                           list - (freeLists + 1)));
    ...
    if (BCM_E_NONE == result) {
        MDB_VVERB((MDB_MSG1("prepare element descriptors (%d banks)\n"),
                   tempList->banks));
        element.count = 0;
        element.list = _MDB_BLOCK_NOT_HEAD;
        element.next = _MDB_BLOCK_END;
        element.prev = _MDB_BLOCK_END;
        ...
        MDB_VVERB((MDB_MSG1("assign elements to appropriate free lists\n")));
        for (bank = 0; bank < banks; bank++) {
    ...
    MDB_VERB((MDB_MSG("(&(%08X),%d,%d,*,%d,%d,%d,%s) return %d (%s)\n"),
              (uint32)(*handle),
              bankSize,
              freeLists,
              userLists,
...
{
    MDB_VERB((MDB_MSG("(%08X) enter\n"),
              (uint32)handle));

    if (!handle) {
        MDB_ERR((MDB_MSG1("NULL handle is not acceptable\n")));
        return BCM_E_PARAM;
    }
    if (handle->lock) {
    ...
    MDB_VERB((MDB_MSG("(%08X) return %d (%s)\n"),
              (uint32)handle,
              BCM_E_NONE,
              _SHR_ERRMSG(BCM_E_NONE)));
    return BCM_E_NONE;
                ...
                shr_mdb_elem_index_t last)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%08X,%08X) enter\n"),
              (uint32)handle,
              first,
              last));
    ...
    if (first > last) {
        MDB_ERR((MDB_MSG1("first %08X is greater than last %08X\n"),
                 first,
                 last));
        return BCM_E_PARAM;
    }
    ...
    MDB_VERB((MDB_MSG("(%08X,%08X,%08X) return %d (%s)\n"),
              (uint32)handle,
              first,
              last,
              result,
              ...
              shr_mdb_elem_bank_index_t count)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,*,%d) enter\n"),
              (uint32)handle,
              count));

    _MDB_VALID_CHECK(handle);
    if (!block) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as block pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
    ...
    MDB_VERB((MDB_MSG("(%08X,&(%08X),%d) return %d (%s)\n"),
              (uint32)handle,
              *block,
              count,
              result,
                 ...
                 shr_mdb_elem_bank_index_t count)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%08X,%d) enter\n"),
              (uint32)handle,
              block,
              count));
             ...
             shr_mdb_elem_index_t block)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%08X) enter\n"),
              (uint32)handle,
              block));
    ...
    MDB_VERB((MDB_MSG("(%08X,%08X) return %d (%s)\n"),
              (uint32)handle,
              block,
              result,
              _SHR_ERRMSG(result)));
                       ...
                       shr_mdb_elem_bank_index_t *count)
{
    ...
    _MDB_VALID_CHECK(handle);
    if (!count) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as count pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
                    ...
                    shr_mdb_elem_index_t block)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%d,%08X) enter\n"),
              (uint32)handle,
              list,
              block));
    ...
    MDB_VERB((MDB_MSG("(%08X,%d,%08X) return %d (%s)\n"),
              (uint32)handle,
              list,
              block,
              result,
                 ...
                 shr_mdb_elem_bank_index_t *list)
{
    ...
    _MDB_VALID_CHECK(handle);
    if (!list) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as list pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
                  ...
                  shr_mdb_elem_index_t *head)
{
    ...
    _MDB_VALID_CHECK(handle);
    if (!head) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as head pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
                  ...
                  shr_mdb_elem_index_t *tail)
{
    ...
    _MDB_VALID_CHECK(handle);
    if (!tail) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as tail pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
                  ...
                  shr_mdb_elem_index_t *pred)
{
    ...
    _MDB_VALID_CHECK(handle);
    if (!pred) {
        MDB_ERR((MDB_MSG1("NULL is unacceptable as predecessor pointer\n")));
        return BCM_E_PARAM;
    }
    _MDB_LOCK_TAKE(handle);
                   ...
                   shr_mdb_elem_bank_index_t list)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%d) return %d (%s)\n"),
              (uint32)handle,
              list,
              result,
              _SHR_ERRMSG(result)));
                   ...
                   shr_mdb_block_info_t *blockInfo)
{
    ...
    if (!blockInfo) {
        MDB_ERR((MDB_MSG1("NULL pointer unacceptable for"
                          " outbound argument\n")));
        return BCM_E_PARAM;
    }
    _MDB_VALID_CHECK(handle);
    ...
    if ((block < handle->low) ||
        (head >= handle->count)) {
        MDB_ERR((MDB_MSG1("element %08X is not valid\n"), block));
        /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
    ...
    if (BCM_E_NONE == result) {
        ...
        if (handle->elem[head].list < handle->freeLists) {
            ...
            MDB_ERR((MDB_MSG1("element %08X is free\n"), block));
            result = BCM_E_NOT_FOUND;
        }
    }
    if (BCM_E_NONE == result) {
    ...
    MDB_VERB((MDB_MSG("(%08X,%d,&{%d,%d,%d}) return %d (%s)\n"),
              (uint32)handle,
              block,
              blockInfo->head,
              blockInfo->list,
                  ...
                  shr_mdb_list_info_t *listInfo)
{
    ...
    MDB_VERB((MDB_MSG("(%08X,%d,%s,*) enter\n"),
              (uint32)handle,
              list,
              free?"Free":"User"));
    ...
    if (free) {
        if (list >= handle->freeLists) {
            MDB_ERR((MDB_MSG1("there are not %d free lists\n"), list));
            /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
            result = BCM_E_NOT_FOUND;
        }
    } else {
        if (list >= handle->userLists) {
            MDB_ERR((MDB_MSG1("there are not %d user lists\n"), list));
            /* don't ask me why, but BCM likes NOT_FOUND for INVALID_ID */
            result = BCM_E_NOT_FOUND;
        }
    ...
    MDB_VERB((MDB_MSG("(%08X,%d,%s,*) return %d (%s)\n"),
              (uint32)handle,
              list,
              free?"Free":"User",
              result,
                      ...
                      shr_mdb_alloc_pref_t allocmode)
{
    ...
    case shr_mdb_alloc_bank_low:
        ...
        break;
    default:
        MDB_ERR((MDB_MSG1("invalid alloc mechanism %d\n"),
                 allocmode & shr_mdb_alloc_bank_mask));
        return BCM_E_PARAM;
    }
    if (!(allocmode & shr_mdb_join_alloc_and_free)) {
        MDB_ERR((MDB_MSG1("must join on free, alloc, or both\n")));
        return BCM_E_PARAM;
    }
    if (!(allocmode & shr_mdb_join_high_and_low)) {
        MDB_ERR((MDB_MSG1("must join high, low, or both\n")));
        return BCM_E_PARAM;
    }
    if (allocmode & (~(shr_mdb_alloc_bank_mask |
                       ...
                       shr_mdb_join_alloc_and_free |
                       shr_mdb_join_high_and_low))) {
        MDB_ERR((MDB_MSG1("invalid bits are set in allocmode (%08X)\n"),
                 allocmode & (~(shr_mdb_alloc_bank_mask |
                                shr_mdb_alloc_block_high |
                                shr_mdb_free_block_high |
                                shr_mdb_join_alloc_and_free |
                              ...
                              shr_mdb_elem_bank_index_t userList)
{
    ...
    if (userList < handle->userLists) {
        ...
        MDB_ERR((MDB_MSG1("user list %d invalid for mdb %08X\n"),
                 userList,
                 (uint32)handle));
        result = BCM_E_PARAM;
    }
