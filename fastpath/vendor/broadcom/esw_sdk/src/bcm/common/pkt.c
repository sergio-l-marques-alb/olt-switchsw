              ...
              uint32 flags, bcm_pkt_t **pkt_buf)
{
    ...
    if (bcm_pkt_flags_len_setup(unit, pkt, bytes, -1, flags) < 0) {
	if (local_alloc) { /* Packet was locally allocated */
	    sal_free(pkt);
	}
	pkt = NULL;
    }
...
bcm_pkt_flags_init(int unit, bcm_pkt_t *pkt, uint32 flags)
{
    ...
    if (BCM_IS_LOCAL(unit)) {
        ...
        if (bcm_stk_mode_get(unit, &stkflags) >= 0) {
            if (stkflags & BCM_STK_SL) {
