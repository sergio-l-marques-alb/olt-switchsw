...
static int _tk371x_clear_all_staticmacs(int unit){
    ...
    PBMP_E_ITER(unit, port) {
        ...
        soc_cm_debug(DK_L2TABLE,"_tk371x_clear_all_staticmacs: port=%d, entryCount=%d\n", port, entryCount);
        if (rv == OK) {
    ...
    PBMP_E_ITER(unit, port) {
        sal_memset(macs, 0, sizeof(macs));
        _soc_ea_static_mac_entries_get(unit, 0, port, &entryCount, macs); 
        soc_cm_debug(DK_L2TABLE, "port=%d, entryCount=%d\n", port, entryCount);
        if (entryCount > 0) {
        	return BCM_E_FAIL;
        }
    }
