/*
 * $Id: port.c 1.348.2.1 Broadcom SDK $
...
_bcm_robo_port_cfg_init(int unit)
{
    ...
    BCM_PBMP_ITER(PBMP_ALL(unit), port){
        ...
        if (BCM_FAILURE(rv)){
            soc_cm_debug(DK_WARN,"fail to set pvid in %s\n",FUNCTION_NAME());            
            return;
        }
    }
...
_bcm_robo_port_update(int unit, bcm_port_t port, int link)
{
     ...
     *          - bcm53242/53262/53115/53118/53101/53125/53128
    ...
    if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit) || 
        SOC_IS_ROBO_ARCH_VULCAN(unit) || SOC_IS_DINO8(unit)) {
    ...
    if (temp == MLF_FORWARD_FLOOD) {
        *mode = BCM_PORT_MCAST_FLOOD_UNKNOWN;
        ...
        if (NUM_ALL_PORT(unit) > 32) {
            ...
            BCM_IF_ERROR_RETURN((DRV_SERVICES(unit)->reg_field_get)
                (unit, MLF_DROP_MAPr, (void *)&reg_val64, 
                MLF_FWD_MAPf, (void *)&fld_val64));
                             ...
                             uint32 burst)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_rate_egress_pps_set()..\n");
    BCM_IF_ERROR_RETURN(
        _bcm_robo_port_gport_validate(unit, port, &port));
                             ...
                             uint32 *burst)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_rate_egress_pps_get()..\n");
                ...
                uint32 kbits_resume)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_rate_pause_set()..\n");
    BCM_IF_ERROR_RETURN(
        _bcm_robo_port_gport_validate(unit, port, &port));
                ...
                uint32 *kbits_resume)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_robo_port_rate_pause_get()..\n");
    BCM_IF_ERROR_RETURN(
        _bcm_robo_port_gport_validate(unit, port, &port));
