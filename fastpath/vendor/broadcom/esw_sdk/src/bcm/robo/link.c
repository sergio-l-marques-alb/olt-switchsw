/*
 * $Id: link.c 1.63.2.4 Broadcom SDK $
...
_bcm_robo_linkscan_update_port(int unit, int port)
{
    ...
    /*
     * Place code here since WAN port is not valid in port check.
     */
    if (SOC_IS_ROBO5397(unit)) {
#ifdef WAN_PORT_SUPPORT
        if (port == WAN_PORT_NUMBER) {
            /* Force link down at init time */
            if (-1 == wan_port_cur_link) {
                wan_port_cur_link = FALSE;
                soc_robo_wan_port_link_sw_update(unit,FALSE);
            }
            cur_link = wan_port_cur_link;
            BCM_IF_ERROR_RETURN(soc_robo_wan_port_link_get(unit, &new_link));

            if (cur_link == new_link){
                return BCM_E_NONE;
            }

            wan_port_cur_link = new_link;
            rv = soc_robo_wan_port_link_sw_update(unit,new_link);
            return rv;
        }
#endif
    }
    ...
    if (!new_link) {
        ...
        if ((SOC_IS_ROBO5395(unit)) || (SOC_IS_ROBO5398(unit)) ||
                (SOC_IS_ROBO5397(unit)) || (SOC_IS_ROBO53115(unit)) || 
    ...
    if (new_link) {
        ...
        if ((SOC_IS_ROBO5395(unit)) || (SOC_IS_ROBO5398(unit)) ||
            (SOC_IS_ROBO5397(unit)) || (SOC_IS_ROBO53115(unit)) ||
            (SOC_IS_ROBO53118(unit)) || (SOC_IS_ROBO53125(unit))) {        
            /* check if bcm53118 need iop workaround verification also */
            if (!(SOC_IS_ROBO53115(unit) || SOC_IS_ROBO53118(unit) ||
                SOC_IS_ROBO53125(unit))) {
                /* phy53115 a0/a1/b0 has applied IOP(interoperation) WAR and 
                 *  the statement in following might causes link problem.
                 *  - link-up port will run cable diag, and problem occurred 
                 *      if the link partner is not broadcom PHY.
                 *      >> AN off and link down
                 */
                bcm_robo_port_phy_power_mode_select(unit, port);
            }
        }                   
    }
    /*
...
_bcm_robo_mac_low_power_update(int unit) 
{
    ...
    if (!SOC_MAC_LOW_POWER_ENABLED(unit)) {
        ...
        /* Reset the timestamp if low power is not enabled. */
        if (SOC_CONTROL(unit)->all_link_down_detected) {
            MAC_LOW_POWER_LOCK(unit);
            SOC_CONTROL(unit)->all_link_down_detected = 0;
            MAC_LOW_POWER_UNLOCK(unit);
...
_bcm_robo_linkscan_update(int unit, pbmp_t pbm)
{
    ...
    if (SOC_IS_ROBO5397(unit)) {
#ifdef WAN_PORT_SUPPORT
        _bcm_robo_linkscan_update_port(unit, WAN_PORT_NUMBER);
#endif
    }
...
#if defined(BCM_5396_A0) || defined(BCM_5389_A0) || defined(BCM_5398_A0) \
    || defined(BCM_5397_A0) || defined(BCM_5395_A0) || defined(BCM_53115_A0) \
    || defined(BCM_53118_A0) || defined(BCM_53101_A0) || defined(BCM_53125_A0) \
