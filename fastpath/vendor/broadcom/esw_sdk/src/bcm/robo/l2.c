                ...
                bcm_l2_addr_t *l2addr, l2_arl_sw_entry_t *arl_entry)
{
    ...
    if (l2addr->mac[0] & 0x01) { /* mcast address */
        ...
        if (BCM_FAILURE(rv)) {
            soc_cm_debug(DK_WARN,"fail to get bcmSwitchUseGport\n");
            return;
        }
        /* set Port */
    ...
    uint32    flags)
{
    ...
    soc_cm_debug(DK_ARL, "BCM API:%s,flags=0x%x\n", FUNCTION_NAME(), flags);
    assert(mac);
    if (!BCM_MAC_IS_ZERO(mac)){
        ...
        if (BCM_SUCCESS(rv)){
            soc_cm_debug(DK_L2TABLE, "%s, %d L2 entries were removed!\n",
                    FUNCTION_NAME(), rv);
            rv = BCM_E_NONE;
        }
    }
    ...
    uint32    flags)
{
    ...
    assert(mac);
    if (!BCM_MAC_IS_ZERO(mac)){
        soc_cm_debug(DK_L2TABLE, "%s,mac=%08x-%08x,port=0x%x\n", 
...
bcm_robo_l2_learn_limit_set(int unit, bcm_l2_learn_limit_t *limit)
{
    ...
    if (type & BCM_L2_LEARN_LIMIT_SYSTEM) {
        ...
        if (SOC_IS_NORTHSTARPLUS(unit)){
            /* NSP HW Spec. for systme based learn limit, the action is still 
...
_bcm_robo_l2_match_op(int unit, _bcm_robo_l2_match_t *match_addr, bcm_l2_addr_t *l2_addr)
{
    ...
    soc_cm_debug(DK_L2TABLE | DK_ARL, "%s,%d,op_flags=0x%x!!\n", 
    ...
    if (match_addr->op_flags & _BCM_ROBO_L2_MATCH_OP_DEST){
        if (BCM_MAC_IS_MCAST(l2_addr->mac)){
             ...
             *  l2addr->l2mc_index with two differnet format per ROBO Arch.
             *  1. GEX devices (no individual MCAST group table)
             *      - l2addr->l2mc_index is PBMP
             *  2. Harrier/TBX devices(within individual MCAST group table)
             *      - l2addr->l2mc_index is MCAST group index.
             */
            if (l2_addr->l2mc_index != match_addr->mc_group) {
    ...
    soc_cm_debug(DK_L2TABLE | DK_ARL, "%s,%d,diff_flags=0x%x, rv=%d\n", 
        ...
        _bcm_robo_l2_traverse_t *trav_st, uint32 response_type) 
{
    ...
    soc_cm_debug(DK_L2TABLE, "%s, Starting L2(HW) traverse!\n",
            FUNCTION_NAME());
        ...
        if (rv != SOC_E_EXISTS){
            ...
            if (valid) {
                ...
                if (match_rv < BCM_E_NONE){
                    ...
                    soc_cm_debug(DK_ARL | DK_L2TABLE, 
        ...
        int sync_detect, uint32 response_type)
{
    ...
    if (sync_detect_op) {
        ...
        soc_cm_debug(DK_L2TABLE, "%s, is_sync=%d, sync_detect count=%d!\n", 
    ...
    for (chnk_idx = soc_robo_mem_index_min(unit, mem); 
        ...
        if (chnk_idx < mem_idx_max) {
            soc_cm_debug(DK_L2TABLE, "%s,%d,Chunk SLEEP...\n",__func__,__LINE__);
            sal_usleep(100000);
        }
    } 
    ...
    soc_cm_debug(DK_L2TABLE, "%s, this_trv_cnt=%d, rv=%d\n", 
        ...
        bcm_l2_traverse_cb trav_fn, void *user_data)
{
    ...
    if (trvrs_flags & BCM_L2_TRAVERSE_MATCH_DEST) {
        ...
        if (BCM_MAC_IS_MCAST(match_addr->mac)) {
            int_match_addr.mc_group = match_addr->l2mc_index;
        } else {
