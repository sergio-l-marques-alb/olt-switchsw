/*
 * $Id: multicast.c 1.53 Broadcom SDK $
                ...
                bcm_gport_t vlan_port_id, bcm_if_t *encap_id)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s,%d,vlan_port_id=0x%x\n",
                             ...
                             bcm_if_t encap_id)
{
    ...
    soc_cm_debug(DK_VERBOSE,  "%s,gid=0x%x,port=%d,encap_id=0x%x\n", 
    ...
    if (op_type == BCM_MULTICAST_TYPE_L2){
        ...
        if (SOC_IS_TBX(unit)) {
            ...
            soc_cm_debug(DK_VERBOSE,  "%s,pp=%d,ut=%d,vid=%d\n", 
            ...
            if (robo_mcast_repl_config[unit]->mcast_repl_enable == FALSE) {
                soc_cm_debug(DK_VERBOSE, "%s, Enable MCast replication!!\n", 
                        FUNCTION_NAME());
...
bcm_robo_multicast_egress_delete_all(int unit, bcm_multicast_t group)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s,group=0x%x\n", FUNCTION_NAME(), group);
    type = _BCM_MULTICAST_TYPE_GET(group);
                             ...
                             bcm_if_t *encap_id_array)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s,gid=%d,port_count=%d\n", 
    ...
    if (op_type == BCM_MULTICAST_TYPE_L2){
        ...
        if (SOC_IS_TBX(unit)) {
            ...
            for (i = 0; i < port_count; i++){
                ...
                is_untag = _BCM_MULTICAST_ENCAP_FLAGS_IS_UNTAG(encap_id);
                vid_in = _BCM_MULTICAST_ENCAP_VLAN_GET(encap_id);
                soc_cm_debug(DK_VERBOSE, "%s,encap[%d]:pp=%d,ut=%d,vid=%d\n", 
                             ...
                             int *port_count)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s,gid=0x%x,port_max=%d\n", 
    ...
    if (op_type == BCM_MULTICAST_TYPE_L2){
        ...
        *port_count = i;
        ...
        if (encap_id_array != NULL) {
            sal_memset(encap_id_array, 0, sizeof(encap_id_array));
        }
    } else if (op_type == BCM_MULTICAST_TYPE_VLAN) {
#ifdef  BCM_TB_SUPPORT
        if (SOC_IS_TBX(unit)) {
                                       ...
                                       bcm_multicast_t *group_max)
{
    ...
    if (type_flag == BCM_MULTICAST_TYPE_L2) {
        if (SOC_MEM_IS_VALID(unit, MARL_PBMPm)) {
