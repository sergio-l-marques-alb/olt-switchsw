/*
 * $Id: trunk.c 1.28.2.20 Broadcom SDK $
        ...
        if (!((i < region_size) && (flags & BCM_TRUNK_FLAG_IPMC_CLEAVE))) {
            ...
            if (localcount > 0) {
                if (all_local || (member_count > BCM_SWITCH_TRUNK_MAX_PORTCNT)) {
                     ...
                     * We need to be conservatively configure
        ...
        trunk_private_t *t_info)
{
    ...
    if (num_uc_egr_ports > 0) {
         ...
         * If RTAG is 1-6, Trident A0 ignores the TG_SIZE field and assumes 
    ...
    /* Call Trill module to clear states for local leaving members */
#ifdef INCLUDE_L3
    if (soc_feature(unit, soc_feature_trill)) {
        bcm_port_t local_member;
        bcm_port_t local_leaving_members[SOC_MAX_NUM_PORTS];
        int num_local_leaving_members = 0;

        for (i = 0; i < num_leaving_members; i++) {
            if (BCM_SUCCESS(bcm_esw_port_local_get(unit, leaving_members[i],
                            &local_member))) {
                local_leaving_members[num_local_leaving_members++] = local_member;
            }
        }
        if (num_local_leaving_members > 0) {
            rv = _bcm_esw_trill_trunk_member_delete(unit, tid,
                    num_local_leaving_members, local_leaving_members);
            if (BCM_FAILURE(rv)) {
                _BCM_TRIDENT_TRUNK_FP_SET_FREE_MEM_RETURN(rv);
            }
        }
    ...
    if (dlb_enable) {
       dlb_id = soc_HGT_DLB_CONTROLm_field32_get(unit,
        ...
        if (_bcm_trident_hg_tg_size_field_forced(unit)) {
            tg_size = MEMBER_INFO(unit, (hgtid + _BCM_FP_TRUNK_NUM_MAX)).num_ports;
        }
                          ...
                          trunk_private_t *trunk_info)
{
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
        ...
        if (dlb_enable) {
            ...
            /* Recover Higig trunk group ports */
            hwport = sal_alloc(sizeof(bcm_port_t) * tg_size, "hwport");
            if (hwport == NULL) {
                return BCM_E_MEMORY;
            }
