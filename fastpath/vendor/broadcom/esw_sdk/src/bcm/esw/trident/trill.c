/*
 * $Id: trill.c 1.46.6.55 Broadcom SDK $
...
void
bcm_td_trill_root_name_get (int unit, uint8 trill_tree_profile_idx, uint16 *trill_name)
{
    _bcm_td_trill_bookkeeping_t *trill_info = TRILL_INFO(unit);
...
_bcm_td_trill_next_hop_set(int unit, int nh_index, bcm_trill_port_t *trill_port)
{
    ...
    replace = trill_port->flags & BCM_TRILL_PORT_REPLACE;
...
bcm_td_trill_multicast_transit_entry_set(int unit, int flags,
                   uint16 root_name, int mc_index, uint8  trill_tree_id)
{
...
int
bcm_td_trill_multicast_enable(int unit, int flags, uint16 root_name)
{
    mpls_entry_entry_t  tr_key, tr_entry;
    int rv = BCM_E_UNAVAIL;
    int entry_index=0;
...
int
bcm_td_trill_multicast_disable(int unit, int flags, uint16 root_name)
{
    mpls_entry_entry_t  tr_ent;
    int rv = BCM_E_UNAVAIL;
    int index;
       ...
       BCM_TRILL_L3MC_CHECK (unit, l3mc_index);
        ...
        if (vid != BCM_VLAN_NONE){
             ...
             BCM_IF_ERROR_RETURN(
              ...
              BCM_IF_ERROR_RETURN(
                        bcm_td_l2_trill_multicast_entry_update(unit, TR_L2_HASH_KEY_TYPE_BRIDGE, 
                                               vid, c_dmac, trill_tree_id, group));
 ...
 */
int
bcm_td_trill_unpruned_entry_traverse (int unit, _bcm_td_trill_multicast_traverse_t *trav_st)
{
...
bcm_td_trill_multicast_traverse(int unit, bcm_trill_multicast_traverse_cb  cb,
                                                        void *user_data)
{
    _bcm_td_trill_multicast_traverse_t  trav_st;
    int rv = BCM_E_NONE;
    ...
    sal_memset(&trav_st, 0, sizeof(_bcm_td_trill_multicast_traverse_t));
 ...
 */
int
bcm_td_trill_vlan_multicast_group_add(int unit, bcm_vlan_t vlan)
{
 ...
 */
int
bcm_td_trill_vlan_multicast_group_delete(int unit, bcm_vlan_t vlan)
{
...
bcm_td_trill_egress_add(int unit, bcm_gport_t trill_port, 
                                                      bcm_multicast_t l3mc_group, bcm_if_t encap_id)
{
    _bcm_td_trill_bookkeeping_t *trill_info;
    int vp=0;
    int network_port=0;
                                                                    ...
                                                                    bcm_gport_t *gport)
{
    ...
    if (soc_L3_ENTRY_IPV4_UNICASTm_field32_get(unit, &l3_entry, TRILL__EXPECTED_Tf)) {
                                                        ...
                                                        void *user_data)
{
    ...
    for (idx = idx_min; idx <= idx_max; idx++) {
        ...
        if (soc_mem_field32_get(unit, mem, l3_entry_p, TRILL__EXPECTED_Tf)) {
             ...
             rv = _bcm_gport_modport_hw2api_map(unit, module_id, port_num, 
                                                &mod_out, &port_out);
             if (BCM_FAILURE(rv)) {
                 break; 
             }
