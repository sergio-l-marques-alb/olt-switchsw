/*
 * $Id: txbeacon.c 1.8.2.12 Broadcom SDK $
 *
...
bcm_esw_txbeacon_init(int unit, int uC)
{
    ...
    if (soc_cmic_uc_appl_init(unit, uC, MOS_MSG_CLASS_PKT_BCN,
                              ...
                              TXBEACON_UK_MIN_VERSION) != SOC_E_NONE) {
        return (BCM_E_UNAVAIL);
    }
    ...
    bcm_txbeacon_t *txbeacon)
{
    ...
    /* Note that this is different in 5.10 and 6.0 */
    send.s.len = soc_htons((8 * txbeacon->port) + 40);
