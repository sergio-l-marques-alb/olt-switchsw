...
/* Function:
 *      _bcm_esw_vxlan_multicast_leaf_entry_check
 * Purpose:
 *      Get  VXLAN Tunnel terminator Info
 * Parameters:
 *      unit     - Device Number
...
int
_bcm_esw_vxlan_multicast_leaf_entry_check(int unit,  bcm_ip_t mc_ip_addr, int flag)
{
    int rv = BCM_E_NONE;
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_TRIDENT2(unit) && soc_feature(unit, soc_feature_vxlan)) {
         rv = bcm_td2_vxlan_lock(unit);
         if ( rv == BCM_E_NONE ) {
              rv = bcm_td2_vxlan_multicast_leaf_entry_check(unit, mc_ip_addr, flag);
              bcm_td2_vxlan_unlock (unit);
         }
    }
#endif
            ...
            bcm_stat_flex_table_info_t *table_info)
{
    ...
    if ( !(_BCM_VXLAN_VPN_INVALID == vpn) &&
        ...
        #if 0 /* EGR_VFI is unused in vxlan */
        /* Egress Vxlan VPN */
                            ...
                            bcm_vpn_t           vpn)
{
    ...
    if (bcm_esw_vxlan_stat_id_get(unit, port, vpn,
                   bcmVxlanOutPackets, &stat_counter_id) == BCM_E_NOT_FOUND) {
        BCM_IF_ERROR_RETURN(bcm_esw_vxlan_stat_id_get(unit, port, vpn,
                                        bcmVxlanInPackets, &stat_counter_id));
