/*
 * $Id: ipmc.c 1.29.6.14 Broadcom SDK $
...
_rep_regs_t ka_rep_regs[] = { {INVALIDf,         INVALIDf,       INVALIDf,    INVALIDm},
                  ...
                  bcm_port_t port, int *vptr, int last, int set)
{
    ...
    if (set) {
        ...
        if (INVALIDf != ptr_type) {
            soc_mem_field32_set(unit, mem, &gentry, ptr_type, 0);
        }
                        ...
                        bcm_vlan_vector_t vlan_vec)
{
    ...
    int                 vlan_ptr, last_vlan_ptr, partition;
    uint32              vlan_count;
    int                 ls_pos, nh_idx;
    int                 rvth = BCM_E_NONE;
    int                 intf_base, nh_offset;
                            ...
                            bcm_port_t port, int count)
{
    ...
    soc_mem_field32_set(unit, L3_IPMCm, entry, EXT_Q_NUM_COPIESf, 
                                    count);
    soc_mem_field32_set(unit, L3_IPMCm, entry, VALIDf, 
                                    1);
    ...
    for (i = 0; i < 2; i++) {     
        if ((queue_id = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, 
            ...
            ptr = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist0_entry, 
                                                           repl_ptr[i]);
            type = soc_MMU_EXT_MC_QUEUE_LIST0m_field32_get(unit, &qlist0_entry, 
                                                           ptr_type[i]);
            ...
            ptr |= ((type << 14) | (copy << 15) | (buf_type << 16) | (vlan_last << 17));
                        ...
                        int *extq_index, int *count)
{
    ...
    *count = 0;
    ...
    if (q_count != 0) {
        ...
        /* set  L2==0 AND L3_REPL_TYPE==1 AND L3_REPL_PTR==0 to disable */
        switch (q_count) {
        case 1:        
            soc_MMU_EXT_MC_QUEUE_LIST0m_field32_set(unit, &qlist0_entry, 
                                                    ptr_type[1], 1);
                                        ...
                                        int is_l3, int *queue_count)
{
    ...
    int     ipmc_ptr, i;
    int     new_ipmc_ptr, last_ipmc_flag = 0;
    uint32     *q_repl_ptr = NULL;
    SHR_BITDCL *q_vec = NULL;
    ...
    queue_id = BCM_GPORT_UCAST_SUBSCRIBER_QUEUE_GROUP_QID_GET
                                            (subscriber_queue);
    ...
    if (q_found) {
        ...
        if (ptr_type == 0) {
            ...
            if (if_count == 1) {
                _bcm_kt_ipmc_egress_intf_add(unit, ipmc_id, port, ipmc_ptr, 
                                             *id, &new_ipmc_ptr, 
                                             &last_ipmc_flag);    
            } else {
                _bcm_kt_ipmc_egress_intf_set(unit, ipmc_id, port, ipmc_ptr, 
                                             if_count, id, &new_ipmc_ptr, 
                                        ...
                                        int *count)
{
       ...
       for (i = 0; i < q_count; i++) {
            if ((repl_ptr_array[i] >> 14) & 1) { /* if its a nh or l3 intf */
                if_array[vlan_count++] = repl_ptr_array[i] & ~(1 << 14);
                subs_array[vlan_count++] = baseq_array[i]; /* TODO subscriber gport */
...
_bcm_tr2_ipmc_repl_sw_dump(int unit)
{
    ...
    if (repl_info != NULL) {
        soc_cm_print("    IPMC Size    : %d\n", repl_info->ipmc_size);
        soc_cm_print("    Intf Size    : %d\n", repl_info->intf_num);
        soc_cm_print("    Vlan total   : %d\n", repl_info->ipmc_vlan_total);

        soc_cm_print("    Bitmap (index:value-hex) :");
        ...
        soc_cm_print("    port (index:vlan-count) :\n");
        for (port = 0; port < SOC_MAX_NUM_PORTS; port++) {
            ...
            for (i = 0, j = 0; i < repl_info->ipmc_size; i++) {
                ...
                soc_cm_print(" %5d:%-4d", i, port_info->vlan_count[i]);
                j++;
            }
            soc_cm_print("\n");
        }
    ...
    if (SOC_IS_KATANA(unit)) {
        ...
        soc_cm_print("  IPMC EXTQ REPL Info -\n");
        extq_info = _kt_extq_repl_info[unit];
        if (repl_info != NULL) {
            soc_cm_print("    IPMC Size    : %d\n", extq_info->ipmc_size);
            soc_cm_print("    Queue Num    : %d\n", extq_info->queue_num);
            soc_cm_print("    Extq total   : %d\n", extq_info->extq_list_total);
            ...
            for (i = 0, j = 0; i < extq_info->ipmc_size; i++) {
                ...
                soc_cm_print(" %5d:%-4d", i, group_info->queue_count[i]);
                j++;
            }
            soc_cm_print("\n");
