/*
 * $Id: qos.c 1.48.2.8 Broadcom SDK $
    ...
    if (BCM_GPORT_IS_SET(port) && !BCM_GPORT_IS_MODPORT(port) && 
        ...
        if (BCM_GPORT_IS_MIM_PORT(port) || BCM_GPORT_IS_MPLS_PORT(port)) {
            ...
            if (ing_map != -1) { /* -1 means no change */
                ...
                if (ing_map == 0) {
                    ...
                    case _BCM_QOS_MAP_TYPE_DSCP_TABLE:      
                        ...
                        /*Right now there is no way to tell whether the user want TRUST_DSCP for v4 or v6
                         * So set both pointing to same profile index. But there could be requirement to 
                         * to set v6 for a different profile(not possible as there is only 1 profile ptr) or 
                         * dont want TRUST_DSCP for V4 or V6 alone.    
                         */   
                        if (soc_mem_field_valid(unit,SOURCE_VPm,TRUST_DSCP_V6f)) {
            ...
            if (ing_map != -1) { /* -1 means no change */
                ...
                int index_min = soc_mem_index_min(unit, L3_TUNNELm);
                int index_max = soc_mem_index_max(unit, L3_TUNNELm);

                alloc_size = (index_max + 1) * sizeof(tnl_entry);
