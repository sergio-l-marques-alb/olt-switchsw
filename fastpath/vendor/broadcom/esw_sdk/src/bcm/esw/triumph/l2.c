/*
 * $Id: l2.c 1.272.2.3 Broadcom SDK $
...
_bcm_kt2_l2_station_non_tcam_reload(int unit, _bcm_l2_station_control_t *sc) 
{
    ...
    for (index = 0; index < sc->port_entries_total; index++) {
        ...
        soc_mem_field_get(unit, PORT_TABm, (uint32 *)ptab, MAC_ADDRf, mac_field);
        ...
        if ((mac_field[0] > 0) || (mac_field[1] > 0)) {
            ...
            sc->entry_arr[_BCM_L2_STATION_ID_KT2_PORT_MAC_MIN + index] =
                                                                     s_ent_p;
    ...
    for (index = 0; index < sc->olp_entries_total; index++) {
        ...
        if ((mac_field[0] > 0) || (mac_field[1] > 0)) {
            ...
            sc->entry_arr[_BCM_L2_STATION_ID_KT2_OLP_MAC_MIN + index] = s_ent_p;
            sc->port_entries_free--;
        }
    }
    ...
    if ((mac_field[0] > 0) || (mac_field[1] > 0)) {
        ...
        s_ent_p->flags |= _BCM_L2_STATION_ENTRY_INSTALLED;
        s_ent_p->flags |= _BCM_L2_STATION_ENTRY_TYPE_NON_TCAM;
        sc->entry_arr[_BCM_L2_STATION_ID_KT2_XGS_MAC_MIN + index] = s_ent_p;
    }
    return (BCM_E_NONE);
}
#endif /* BCM_KATANA2_SUPPORT */
                          ...
                          _bcm_l2_station_entry_t *ent_p)
{
    ...
    if (SOC_IS_KATANAX(unit)
        ...
        soc_mem_field32_set(unit, tcam_mem, ent_p->tcam_ent,
                            ING_PORT_NUMf, station->src_port);

        soc_mem_field32_set(unit, tcam_mem, ent_p->tcam_ent,
                            ING_PORT_NUM_MASKf, station->src_port_mask);
...
_bcm_kt2_l2_port_mac_set(int unit, int index, bcm_mac_t port_mac) 
{
    ...
    soc_mem_mac_addr_set(unit, PORT_TABm, &ptab, MAC_ADDRf, port_mac);
    ...
    SOC_IF_ERROR_RETURN(
                    READ_LPORT_TABm(unit, MEM_BLOCK_ANY, index, &pent));
    soc_mem_mac_addr_set(unit, LPORT_TABm, &pent, 
                                   MAC_ADDRf, port_mac);
    SOC_IF_ERROR_RETURN(
                WRITE_LPORT_TABm(unit, MEM_BLOCK_ALL, index, &pent));
    return BCM_E_NONE; 

}

STATIC int
_kt_olp_mac_addr_get(int unit, uint32 *new_mac, uint32 flag)
{
    ...
    for (; index < max_entries; index++) {
        ...
        if ((mac_addr[0] == 0) && (mac_addr[1] == 0)) {
          /* skip the entry */ 
            ...
            if (flag == OLP_HIGHEST_MAC) {
                if(mac_addr[0] > new_mac[0]) {
                    new_mac[0] = mac_addr[0];
                    new_mac[1] = mac_addr[1];
                } 
            } else if (flag == OLP_LOWEST_MAC) {
               if(mac_addr[0] < new_mac[0]) {
                   new_mac[0] = mac_addr[0];
                   new_mac[1] = mac_addr[1];
               } 
...
STATIC int
_bcm_kt2_l2_olp_mac_set(int unit, int index, bcm_port_t port, bcm_mac_t olp_mac)
{
    egr_olp_dgpp_config_entry_t eolp_dgpp_cfg;
    ...
    if (port == 0) {
        ...
        rv = bcm_kt2_subport_port_resolve(unit, port, &module_id, 
    ...
    soc_mem_mac_addr_set(unit, EGR_OLP_DGPP_CONFIGm, &eolp_dgpp_cfg, 
                         MAC_ADDRf, (olp_enable ? olp_mac : zero_mac));
    ...
    /*  */
    SOC_IF_ERROR_RETURN(READ_ING_OLP_CONFIG_0_64r(unit, &reg_val));
    /* Get the configured value of  data and mask used in MAC_DA_PREFIX */
    mac_addr[0] = COMPILER_64_LO(reg_val);
    mac_addr[1] = COMPILER_64_HI(reg_val);
    SAL_MAC_ADDR_TO_UINT32(olp_mac, new_mac);

    SOC_IF_ERROR_RETURN(READ_ING_OLP_CONFIG_1_64r(unit, &reg_val));
    mac_mask[0] = COMPILER_64_LO(reg_val);
    mac_mask[1] = COMPILER_64_HI(reg_val);

    /* get number of OLP entries configured so far */
    no_entries = _BCM_OLP_MAC_MAX - sc->olp_entries_free;
    
    /* If first olp entry, then set olp mac into ING_OLP_CONFIG_0_64 and
       set ING_OLP_CONFIG_1_64 to 0xffffffffffff */
    if (no_entries == 1) {
        if (olp_enable) {
            COMPILER_64_SET(reg_val, new_mac[1], new_mac[0]);
            ...
            /* only OLP entry is being deleted */
            COMPILER_64_ZERO(reg_val); 
            SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_0_64r(unit, reg_val));
            SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_1_64r(unit, reg_val));
        }
        ...
        if (olp_enable) {
            if (new_mac[1] != mac_addr[1]) {
                COMPILER_64_ZERO(reg_val); 
                SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_1_64r(unit, reg_val));
            } else {
                if (new_mac[0] >= mac_addr[0]) {
                    /* New olp mac is greater than the DA_PREFIX_CHECK data.
                       So, no need to chage the data. Only, mask might have
                       to be changed */
                    addr_diff = new_mac[0] - mac_addr[0];
                    if (mac_mask[0] >= (~addr_diff)) {
                        /* do nothing */
                    } else {
                        if (addr_diff > OLP_MAX_MAC_DIFF) {
                            COMPILER_64_SET(reg_val, 0, 0);
                        } else {
                            mac_mask[0] = ~addr_diff;
                            COMPILER_64_SET(reg_val, mac_mask[1], mac_mask[0]);
                        }
                    }
                    SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_0_64r(unit, reg_val));
                } else {
                    /* Both data and Mask needs to change */
                    COMPILER_64_SET(reg_val, new_mac[1], new_mac[0]);
                    SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_0_64r(unit, reg_val));
                    addr_diff = mac_addr[0] - new_mac[0];
                    if (addr_diff > OLP_MAX_MAC_DIFF) {
                        COMPILER_64_SET(reg_val, 0, 0);
                    } else {
                        mac_mask[0] = ~addr_diff;
          ...
          /* delete case */
            if ((mac_addr[0] == new_mac[0]) && (mac_addr[1] == new_mac[1])) {
               /* Get the next lowest mac addr and use it as data */
                new_mac[0] = MAC_MASK_LOW;
                new_mac[1] = 0;
                BCM_IF_ERROR_RETURN(_kt_olp_mac_addr_get(unit, new_mac, 
                                                         OLP_LOWEST_MAC));
                COMPILER_64_SET(reg_val, new_mac[1], new_mac[0]);
                SOC_IF_ERROR_RETURN(WRITE_ING_OLP_CONFIG_0_64r(unit, reg_val));
                addr_diff = ((~mac_mask[0]) - (new_mac[0] - mac_addr[0]));
            } else if(((mac_addr[0] + (~mac_mask[0])) == new_mac[0]) &&
                      (mac_addr[1] == new_mac[1]))  {
               /* Get the next highest mac addr and use it to compute mask  */
                new_mac[0] = 0;
                new_mac[1] = 0;
                BCM_IF_ERROR_RETURN(_kt_olp_mac_addr_get(unit, new_mac,
                                    OLP_HIGHEST_MAC));
                             ...
                             _bcm_l2_station_entry_t **ent_pp)
{
    ...
    index = BCM_GPORT_LOCAL_GET(station->src_port);
    ...
    if (station->flags & BCM_L2_STATION_OAM) {
        if (NULL == sc->entry_arr[_BCM_L2_STATION_ID_KT2_PORT_MAC_MIN + \
                                                               index]) {
        ...
        if (NULL == sc->entry_arr[sid -_BCM_L2_STATION_ID_KT2_OLP_MAC_MIN]) {
            ent_p->hw_index = sid - _BCM_L2_STATION_ID_KT2_OLP_MAC_MIN;
        }
        ent_p->flags |= _BCM_L2_STATION_ENTRY_TYPE_NON_TCAM;
        sc->olp_entries_free--;
        /* Set MAC address and DGLP in EGR_OLP_DGLP_CONFIG */
        rv = _bcm_kt2_l2_olp_mac_set(unit, ent_p->hw_index,
                                     station->src_port, station->dst_mac);
        if (BCM_FAILURE(rv)) {
            sal_free(ent_p);
            return (rv);
        }
    } else if (station->flags & BCM_L2_STATION_XGS_MAC) {
        if (NULL == sc->entry_arr[sid -_BCM_L2_STATION_ID_KT2_XGS_MAC_MIN]) {
            ent_p->hw_index = sid -_BCM_L2_STATION_ID_KT2_XGS_MAC_MIN;
        }
        ent_p->flags |= _BCM_L2_STATION_ENTRY_TYPE_NON_TCAM;
        ...
        _bcm_l2_station_entry_set(unit, tcam_mem, station, ent_p);
        sc->entries_free--;
        sc->entry_count++;
    }
                                 ...
                                 int station_id) 
{
    ...
    if ((s_ent->sid >= _BCM_L2_STATION_ID_KT2_PORT_MAC_MIN) &&
        ...
        rv = _bcm_kt2_l2_olp_mac_set(unit, s_ent->hw_index, 0, mac_addr);
        sc->olp_entries_free++;
...
_bcm_l2_station_param_check(int unit, bcm_l2_station_t *station)
{
   ...
   if ((SOC_IS_KATANA2(unit))  && 
       ...
       if (SOC_IS_KATANAX(unit)
            ...
            || SOC_IS_TD2_TT2(unit)
            || SOC_IS_TRIUMPH3(unit)) {
            ParamCheck(unit, tcam_mem, ING_PORT_NUMf, station->src_port);
            ParamCheck(unit, tcam_mem, ING_PORT_NUM_MASKf, 
