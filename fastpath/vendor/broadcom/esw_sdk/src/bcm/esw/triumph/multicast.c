/*
 * $Id: multicast.c 1.78.44.6 Broadcom SDK $
                                      ...
                                      bcm_multicast_t *group)
{
    if (SOC_IS_XGS3_FABRIC(unit)) {
        /* _multicast_ipmc_group_types is not initialized for fabric chips. */
        *group = 0;
        return BCM_E_NOT_FOUND;
    }

    if (0 != _multicast_ipmc_group_types[unit][ipmc_id]) {
...
{
    int mem_size, mc_index, is_set, rv = BCM_E_NONE;
    int alloc_size;
    soc_scache_handle_t scache_handle;
    uint32 group_flags;
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        for (mc_index = 0; mc_index < mem_size; mc_index++) {
                                 ...
                                 uint32 flags, void *user_data)
{
    ...
    if (flags & BCM_MULTICAST_TYPE_L2) {
        mem_size = soc_mem_index_count(unit, L2MCm);
