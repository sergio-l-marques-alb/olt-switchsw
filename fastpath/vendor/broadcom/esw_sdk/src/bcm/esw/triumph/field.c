/* $Id: field.c 1.239.2.15 Broadcom SDK $
            ...
            if(_bcm_field_stat_get(unit,f_ent_st->sid,&f_st) == BCM_E_NONE) {
               if(f_st->flex_mode != 0) {
                  BCM_IF_ERROR_RETURN(_field_stage_control_get(
                                      unit, f_ent->group->stage_id,
                                      &stage_fc));
                  if (f_st->flex_mode != 0) {
                      stat_counter_id = f_st->flex_mode;
                      _bcm_esw_stat_get_counter_id_info(stat_counter_id,
                                      &group_mode,&object,&offset_mode,
                                      &pool_number,&base_index);
                      FP_VERB(("\t## Attaching flex counter resources \n"));
                      rv = _bcm_esw_stat_flex_attach_ingress_table_counters1(
                                   unit, VFP_POLICY_TABLEm,
                                   tcam_idx,
                                   offset_mode, base_index, pool_number,e);
                      /* Ignoring BCM_E_EXISTS case althouth not possible */
                      if (!((rv == BCM_E_EXISTS) || (rv == BCM_E_NONE))) {
                           return rv;
                      }
                  }
               }
