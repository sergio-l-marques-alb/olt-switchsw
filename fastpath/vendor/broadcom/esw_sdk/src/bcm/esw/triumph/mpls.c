/*
 * $Id: mpls.c 1.362.2.98 Broadcom SDK $
...
_bcm_tr_mpls_extended_reinit(int unit)
{
    ...
    int       egr_nh_idx, idx, idx_min, idx_max;
    int       entry_type, entry_intf_num, intf_num;
    ...
    /* Warm Boot */
        ...
        if (stable_size > alloc_sz) {
            ...
            while (nh_if != 0xdeadbeef) {
                egr_nh_idx = nh_if - BCM_XGS3_EGRESS_IDX_MIN;
                egr_nh_entry = soc_mem_table_idx_to_pointer(unit, EGR_L3_NEXT_HOPm,
                                                     egr_l3_next_hop_entry_t *,
                                                     egr_nh_tbl_buf, egr_nh_idx);
                intf_num = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, egr_nh_entry, INTF_NUMf);
                for (idx = idx_min; idx < idx_max; idx++) {
                    ...
                    if ((entry_type != 0x1) && (entry_type != 0x2)) {
                        continue;
                    } 
                    entry_intf_num = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm,
                                                               egr_nh_entry, INTF_NUMf);
                    if ((entry_intf_num == intf_num) && (idx != egr_nh_idx)) {
                        BCM_IF_ERROR_RETURN(_bcm_tr_mpls_add_egrNhop(unit, nh_if));
                        BCM_IF_ERROR_RETURN(_bcm_tr_mpls_add_vp_nh(unit, nh_if, idx));
                    }
                }
                mpls_state += sizeof(int);
                sal_memcpy(&nh_if, mpls_state, sizeof(int));
            } 
            mpls_state = mpls_state_marker + (sizeof(int) * num_nh);
        ...
        if (_bcm_tr_mpls_nhop_headPtr[unit][i] != NULL) {
            for (nhop_ptr = _bcm_tr_mpls_nhop_headPtr[unit][i];
                ...
                for (vp_ptr = nhop_ptr->vp_head_ptr;
                     vp_ptr != NULL;
                     vp_ptr = vp_ptr->link) {
                    sal_memcpy(mpls_state, &nhop_ptr->egr_if, sizeof(int));
                    mpls_state += sizeof(int);
                } /* for each vp on nh */
...
_egr_qos_id2hw_idx (int unit,int qos_id,int *hw_inx)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        if ((qos_id & _BCM_TR_MPLS_EXP_MAP_TABLE_TYPE_MASK) ==
    ...
    if (ing_nh_info.trunk != -1) {
        ...
        for (idx = 0; idx < tinfo.num_ports; idx++) {
            ...
            if (mod_local) {
                ...
                info->vp_count++;
                /* Update the physical port's attributes */
                rv = bcm_esw_port_vlan_member_get(unit, port_out, &port_flags);
                _BCM_MPLS_CLEANUP(rv);

                rv = bcm_esw_port_vlan_member_set(unit, port_out, port_flags);
                _BCM_MPLS_CLEANUP(rv);
  ...
  cleanup:
    ...
    if (egr_nh_info.vc_swap_index != -1) {
        _bcm_tr_mpls_vc_and_swap_ref_count_adjust (unit, 
                             egr_nh_info.vc_swap_index, -1);
        /* Free VC_AND_SWAP_LABEL entry */
        rv = _bcm_tr_mpls_vc_and_swap_table_index_reset (unit, 
                             egr_nh_info.vc_swap_index);
    }
    return rv;
}
