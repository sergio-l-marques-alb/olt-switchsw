/*
 * $Id: stack.c 1.342.2.1 Broadcom SDK $
...
bcm_esw_stk_my_modid_set(int unit, int my_modid)
{
	...
	PBMP_ST_ITER(unit, port) {
        ...
        if (soc_feature(unit, soc_feature_src_modid_base_index)) {
            int old_my_modid;
            int old_base_index, base_index;
            int old_num_ports, num_ports;
            int i;
...
_src_modid_base_index_bk_init(int unit)
{
    ...
    _BCM_SRC_MOD_PORT_TABLE_USED_SET_RANGE(unit, 0, SOC_PORT_ADDR_MAX(unit) + 1);
