/*
 * $Id: init.c 1.153.2.18 Broadcom SDK $
...
_bcm_esw_modules_deinit(int unit)
{
    ...
    _DEINIT_INFO_VERB("dmux");
    rv = bcm_esw_dmux_detach(unit);
    _DEINIT_CHECK_ERR(rv, "dmux");
#endif
...
#if defined(BCM_FILTER_SUPPORT) || defined(BCM_FIELD_SUPPORT)
    _DEINIT_INFO_VERB("diffserv");
    rv = _bcm_esw_ds_deinit(unit);
    _DEINIT_CHECK_ERR(rv, "diffserv");
    ...
    _DEINIT_INFO_VERB("filter");
    rv = _bcm_esw_filter_detach(unit);
    _DEINIT_CHECK_ERR(rv, "filter");
