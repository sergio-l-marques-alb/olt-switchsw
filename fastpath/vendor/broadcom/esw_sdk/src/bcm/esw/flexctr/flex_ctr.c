/*
 * $Id: flex_ctr.c 1.21.2.2 Broadcom SDK $
            ...
            uint32 *mode_id)
{
    ...
    if (total_bits <= 8) {
        soc_cm_print("INFO: UnCompressedMode Will be used \n");
        if (direction == bcmStatFlexDirectionIngress) {
            ing_attr->packet_attr_type=bcmStatFlexPacketAttrTypeUncompressed;
            if (flex_attribute->pre_ifp_color != 0) {
                ing_attr->uncmprsd_attr_selectors.uncmprsd_attr_bits_selector |=
       ...
       for (counter=0;counter < flex_attribute->total_counters ; counter ++ ) {
        ...
        soc_cm_print("INFO: TotalBits=%d Trying out CompressedMode\n",
                      total_bits);
        if (direction == bcmStatFlexDirectionIngress) {
            ing_attr->packet_attr_type=bcmStatFlexPacketAttrTypeCompressed;
        } else {
        ...
        if ((total_bits = 
             ...
             dvp_cmprsd_max_bits + drop_cmprsd_max_bits) > 8)  {
             soc_cm_print("TotalBits=%d Even Compressed mode cannot be used\n",
                           total_bits);
        ...
        case bcmStatFlexDirectionIngress:
            if ((flex_attribute->pre_ifp_color != 0) ||
                ...
                (flex_attribute->int_pri != 0)) {
                 ...
                 mapped_value2 = 0;
                 max_bits=0;
                 soc_cm_print("Ing: PRI_CNG_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrColor] != NULL) {
            ...
            if ((flex_attribute->vlan_format != 0) ||
                ...
                (flex_attribute->inner_dot1p != 0)) {
                 ...
                 mapped_value2 = 0;
                 max_bits=0;
                 soc_cm_print("Ing: PKT_PRI_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrVlan] != NULL) {
             ...
             if (flex_attribute->port != 0) {
                 mapped_value0 = 0;
                 max_bits=0;
                 soc_cm_print("Ing: PORT_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrPort] != NULL) {
             ...
             if ((flex_attribute->tos_dscp != 0) ||
                 ...
                 mapped_value1 = 0;
                 max_bits=0;
                 soc_cm_print("Ing: TOS_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrTosDscp] != NULL) {
            ...
            if ((flex_attribute->pkt_resolution != 0) ||
                ...
                (flex_attribute->drop != 0)) {
                 ...
                 mapped_value2 = 0;
                 max_bits=0;
                 soc_cm_print("Ing: PKT_RES_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrPktType] != NULL) {
        ...
        case bcmStatFlexDirectionEgress:
            ...
            if ((flex_attribute->pre_ifp_color != 0) ||
                (flex_attribute->int_pri != 0)) {
                 soc_cm_print("Egr: PRI_CNG_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrColor] != NULL) {
                     max_bits =  pre_ifp_cng_cmprsd_max_bits;
             ...
             if ((flex_attribute->vlan_format != 0) ||
                 ...
                 mapped_value2 = 0;
                 max_bits=0;
                 soc_cm_print("Egr: PKT_PRI_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrVlan] != NULL) {
                     max_bits =  vlan_format_cmprsd_max_bits;
             ...
             if (flex_attribute->port != 0) {
                 mapped_value0 = 0;
                 max_bits=0;
                 soc_cm_print("Egr: PORT_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrPort] != NULL) {
                     max_bits =  port_cmprsd_max_bits;
             ...
             if ((flex_attribute->tos_dscp != 0) ||
                 ...
                 mapped_value1 = 0;
                 max_bits=0;
                 soc_cm_print("Egr: TOS_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrTosDscp] != NULL) {
                     max_bits =  tos_dscp_cmprsd_max_bits;
            ...
            if ((flex_attribute->pkt_resolution != 0) ||
                ...
                (flex_attribute->drop != 0)) {
                 ...
                 mapped_value3 = 0;
                 max_bits=0;
                 soc_cm_print("Egr: PKT_RES_MAP Creation \n");
                 if (map_array[bcmStatGroupModeAttrPktType] != NULL) {
                     max_bits =  pkt_resolution_cmprsd_max_bits;
    ...
    for (sel = 0; sel < num_selectors ; sel++) {
         ...
         case bcmStatGroupModeAttrFieldIngressColor:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                   ...
                   break;
              default: 
                   soc_cm_print("BAD PARAM(FieldIngressColor)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrVlan:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                   ...
                   break;
              default:
                   soc_cm_print("BAD PARAM(VlanFormat)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrOuterPri:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                     ...
                     break;
              default:
                   soc_cm_print("BAD PARAM(OuterPri)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrInnerPri:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                     ...
                     break;
              default:
                   soc_cm_print("BAD PARAM(InnerPri)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrPktType:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                   ...
                   break;
              default:
                   soc_cm_print("BAD PARAM(PktType)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrEgrNetworkGroup:
              ...
              if (flags & BCM_STAT_GROUP_MODE_INGRESS) {
                  soc_cm_print("BAD PARAM(DVP) for Ingress Direction\n");
                  return BCM_E_PARAM;
              } else {
         ...
         case bcmStatGroupModeAttrDrop:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                     ...
                     break;     
              default:
                   soc_cm_print("BAD PARAM(DropType)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
         ...
         case bcmStatGroupModeAttrPacketTypeIp:
              ...
              case BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES:
                     flex_attribute->combine_attr_counter[counter_offset].
                         drop_flags |= (BCM_STAT_FLEX_IP_PKT_ENABLE |
                                        BCM_STAT_FLEX_IP_PKT_DISABLE);
                     break;
              default:
                   soc_cm_print("BAD PARAM(Ip)"
                                ":sel=%d offset=%d attr=%d value=%d\n",
                                sel, counter_offset , attr, attr_value );
                   return BCM_E_PARAM;
              }
    ...
    for (counter_offset = 0; 
         ...
         if (flex_attribute->pre_ifp_color) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 ...
                 soc_cm_print("PreIfpColor:Combination issue..Check parameters"
         ...
         if (flex_attribute->ifp_color) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 ifp_color_flags == 0) {
                 soc_cm_print("IfpColor:Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->int_pri) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 int_pri_flags == 0) {
                 soc_cm_print("IntPri:Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->vlan_format) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 vlan_format_flags == 0) {
                 soc_cm_print("Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->outer_dot1p) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 outer_dot1p_flags == 0) {
                 soc_cm_print("OuterPri: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->inner_dot1p) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 inner_dot1p_flags == 0) {
                 soc_cm_print("InnerPri: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->port) {
             ...
             if (array_index == BCM_STAT_FLEX_BIT_ARRAY_SIZE ) {
                 soc_cm_print("Port: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->tos_dscp) {
             ...
             if (array_index == BCM_STAT_FLEX_BIT_ARRAY_SIZE ) {
                 soc_cm_print("Tos: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         ...
         if (flex_attribute->svp) {
             ...
             if (array_index == BCM_STAT_FLEX_BIT_ARRAY_SIZE ) {
                 soc_cm_print("SVP: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         if (flex_attribute->dvp) {
             ...
             if (array_index == BCM_STAT_FLEX_BIT_ARRAY_SIZE ) {
                 soc_cm_print("DVP: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
         ...
         if (flex_attribute->ip_pkt) {
             if (flex_attribute->combine_attr_counter[counter_offset].
                 ip_pkt_flags == 0) {
                 soc_cm_print("DROP: Combination issue..Check parameters"
                              "And Assign dummy attributes\n"); 
                 return BCM_E_PARAM; 
             }
         }
