/*
 * $Id: clock_init.c 1.1.2.17 Broadcom SDK $
 * 
    ...
    bcm_ptp_clock_info_t *clock_info)
{
    ...
    _bcm_ptp_telecom_network_option_t telecom_network_option = _bcm_ptp_telecom_disable;
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        ...
        _bcm_ptp_uint16_write(payload+i, PTP_MAX_UNICAST_MASTER_TABLE_ENTRIES);
        i += sizeof(uint16);

        _bcm_ptp_uint16_write(payload+i, PTP_MAX_ACCEPTABLE_MASTER_TABLE_ENTRIES);
        i += sizeof(uint16);

        _bcm_ptp_uint16_write(payload+i, PTP_MAX_UNICAST_SLAVE_TABLE_ENTRIES);
        i += sizeof(uint16);

        _bcm_ptp_uint16_write(payload+i, PTP_MAX_MULTICAST_SLAVE_STATS_ENTRIES);
        i += sizeof(uint16);
        ...
        if (SHR_BITGET(&(clock_info->flags), PTP_CLOCK_FLAGS_TELECOM_PROFILE_BIT)) {
            ...
            telecom_network_option = _bcm_ptp_telecom_disable;
        }

        if (BCM_FAILURE(rv = _bcm_ptp_telecom_network_option_set(unit, ptp_id,
        ...
        if (SOC_IS_KATANA(unit)) {
            ...
            if (BCM_FAILURE(rv = bcm_esw_ptp_signal_output_set(unit, ptp_id,
