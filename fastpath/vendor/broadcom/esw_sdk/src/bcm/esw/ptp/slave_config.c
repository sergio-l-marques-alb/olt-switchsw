/*
 * $Id: slave_config.c 1.12 Broadcom SDK $
 *
    ...
    int clock_num)
{
    ...
    unit_ucs_array[unit].stack_array[ptp_id].table[clock_num].entry =
        sal_alloc(sizeof(bcm_ptp_clock_peer_t) * PTP_MAX_UNICAST_SLAVE_TABLE_ENTRIES,
                  "PTP Slave Table");
    ...
    bcm_ptp_clock_peer_t *slave_info)
{
    ...
    while (i < unicast_slave_table->num_entries) {
        if ((port_num == unicast_slave_table->entry[i].local_port_number) &&
    ...
    bcm_ptp_clock_peer_t *slave_info)
{
    ...
    *num_slaves = 0;
    ...
    for (i = 0; i < unicast_slave_table->num_entries; ++i) {
        if (unicast_slave_table->entry[i].local_port_number == port_num) {
            if (*num_slaves == max_num_slaves) {
                /* No more room */
                *num_slaves = 0;
                return BCM_E_RESOURCE;
            }
            slave_info[*num_slaves] = unicast_slave_table->entry[i];
            ++(*num_slaves);
        }
    }
    ...
    int port_num)
{
     ...
     */
    if (BCM_FAILURE(rv = bcm_esw_ptp_static_unicast_slave_remove(unit, ptp_id, clock_num,
 ...
 *      BCM_E_XXX
 * Notes:
 *      bcm_esw_ptp_static_unicast_slave_clear() unregisters unicast services on PTP
    ...
    while (i < unicast_slave_table->num_entries) {
        if (port_num == unicast_slave_table->entry[i].local_port_number ||
            ...
            if (BCM_FAILURE(rv = _bcm_ptp_add_unicast_slave_announce(
                    ...
                    (uint32)unicast_slave_table->entry[i].local_port_number,
                    &unicast_slave_table->entry[i]))) {
                return rv;
            }
