/*
 * $Id: signaling.c 1.1.2.27 Broadcom SDK $
 *
...
#define BCM_PTP_SIGNAL_SLAVE_SEM_TIMEOUT_USEC        (50000)
...
typedef struct unit_sig_info_s {
    _bcm_ptp_signaling_arbiter_t arbiter;
...
static int send_unicast_request(int unit, bcm_ptp_stack_id_t ptp_id, int clock_num,
    int num_tlvs, int *type, int8 *log_inter_message_period, bcm_ptp_clock_peer_t *master);
...
static int _bcm_ptp_slave_signaling_send(int unit, bcm_ptp_stack_id_t ptp_id,
    int clock_num, _bcmPTPtlvType_t tlv_type, _bcmPTPmessageType_t msg_type,
    int8 log_intermessage_period, uint32 duration, bcm_ptp_clock_peer_t *slave);
static int _bcm_ptp_slave_signaling_request_deny(int unit, bcm_ptp_stack_id_t ptp_id,
    int clock_num, _bcmPTPmessageType_t msg_type, int8 log_intermessage_period,
    bcm_ptp_clock_peer_t *slave);
...
/*
 * Function:
 *      _bcm_ptp_signaled_unicast_master_add
 * Purpose:
 ...
 *      log_annc_interval  - (IN) logAnnounceInterval.
 *      log_query_interval - (IN) Unicast master table logQueryInterval 
 *                                See IEEE Std. 1588-2008, Sect. 17.5.2.
    ...
    uint32 flags)
{
    ...
    if (BCM_FAILURE(rv = bcm_ptp_unicast_master_table_add(unit, ptp_id, clock_num,
    ...
    signaled_master[idx].log_announce_interval = log_annc_interval;
    ...
    if (log_query_interval < 0) {
        signaled_master[idx].query_interval_sec = 1;
    } else {
        signaled_master[idx].query_interval_sec = (1 << log_query_interval);
    }
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_telecom_packet_master_add(unit, ptp_id,
            clock_num, &address))) {
        return rv;
    }
...
/*
 * Function:
 *      _bcm_ptp_signaled_unicast_master_remove
 * Purpose:
    ...
    bcm_ptp_clock_unicast_master_t *master_info)
{
    ...
    status = bcm_ptp_unicast_master_table_remove(unit, ptp_id, clock_num, port_num, &master_info->address);
     ...
     */
    _bcm_ptp_peer_address_convert(&master_info->address, &address);
    if (BCM_FAILURE(rv = _bcm_ptp_telecom_packet_master_remove(unit, ptp_id,
            clock_num, &address))) {
        return rv;
    }
...
/*
 * Function:
 *      _bcm_ptp_signaled_unicast_slave_list
 * Purpose:
    ...
    bcm_ptp_clock_peer_t *slave_table)
{
    ...
    if ((rv = _bcm_ptp_mutex_take(slaveTable_mutex, BCM_PTP_SIGNAL_SLAVE_SEM_TIMEOUT_USEC))) {
    ...
    bcm_ptp_clock_peer_t *master)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_construct_signaling_msg(&local_port,
            master->clock_identity, master->remote_port_number, domain, 0, 0,
            udp, &udpLen))) {
    ...
    for (i = 0; i < num_tlvs; ++i) {
        if (BCM_FAILURE(rv = _bcm_ptp_construct_signaling_request_tlv(type[i],
                log_inter_message_period[i], request_uc_durationField, tlv, &tlvLen))) {
    ...
    bcm_ptp_clock_peer_t *slave)
{
    ...
    if (BCM_FAILURE(bcm_esw_ptp_clock_port_identity_get(unit, ptp_id, clock_num,
                                                        slave->local_port_number, &local_port_id))) {
        return BCM_E_PARAM;
    }

    local_port_addr = (_bcm_esw_ptp_unit_array[unit]
                       .stack_array[ptp_id]
    ...
    switch (tlv_type) {
    case _bcmPTPtlvType_grant_unicast_transmission:
    ...
    case _bcmPTPtlvType_ack_cancel_unicast_transmission:
    ...
    _bcm_ptp_construct_signaling_msg(&local_port_id, slave->clock_identity,
        slave->remote_port_number, domain, tlv, tlvLen, udp, &udpLen);
    ...
    _bcm_ptp_tunnel_message_to_world(unit, ptp_id, packet_len, packet);
    ...
    void *unused)
{
    ...
    for (i = 0; i < num_signaled_masters; ++i) {
        ...
        if (signaled_master[i].service_enable.announce &&
            ...
            messageType[num_tlvs] = _bcmPTPmessageType_announce;
        ...
        if (signaled_master[i].service_grants.announce &&
            ...
            messageType[num_tlvs] = _bcmPTPmessageType_sync;
        ...
        if (signaled_master[i].service_grants.announce &&
            ...
            messageType[num_tlvs] = _bcmPTPmessageType_delay_resp;
...
process_tlvs:
        if (num_tlvs) {
            ...
            send_unicast_request(unit, ptp_id, clock_num, num_tlvs, messageType,
                                 logInterMessagePeriod, &signaled_master[i].master);
    ...
    bcm_ptp_clock_peer_t *slave)
{
     ...
     */
    if (BCM_FAILURE(rv = _bcm_ptp_slave_signaling_send(unit, ptp_id,
            clock_num, _bcmPTPtlvType_grant_unicast_transmission, msg_type,
    ...
    if (BCM_FAILURE(_bcm_ptp_signaled_unicast_slave_exists(unit, ptp_id,
            slave, &i))) {
        _bcm_ptp_mutex_give(slaveTable_mutex);
    ...
    void *arg_clock_num)
{
    ...
    int i;
    int isqueued = 0;
    _bcmPTPtlvType_t tlvType = 0;
    _bcmPTPtlvType_t tlvTypeResp = 0;
    _bcmPTPmessageType_t messageType = 0;
    ...
    for (i = 0; i < signaled_slave_table.num_entries; ++i)
    {
        ...
        if (signaled_slave_table.entry[i].queue_announce_grant) {
            ...
            if ((signaled_slave_table.entry[i].announce_request_time <=
                ...
                signaled_slave_table.entry[i].announce_grant_time = sal_time();

                /* Select message to service. */
                tlvType = _bcmPTPtlvType_request_unicast_transmission;
                tlvTypeResp = _bcmPTPtlvType_grant_unicast_transmission;
                messageType = _bcmPTPmessageType_announce;
        ...
        if (signaled_slave_table.entry[i].queue_sync_grant) {
            ...
            if ((signaled_slave_table.entry[i].sync_request_time <=
                ...
                signaled_slave_table.entry[i].sync_grant_time = sal_time();

                /* Select message to service. */
                tlvType = _bcmPTPtlvType_request_unicast_transmission;
                tlvTypeResp = _bcmPTPtlvType_grant_unicast_transmission;
                messageType = _bcmPTPmessageType_sync;
        ...
        if (signaled_slave_table.entry[i].queue_delay_grant) {
            ...
            if ((signaled_slave_table.entry[i].delay_request_time <=
                ...
                signaled_slave_table.entry[i].delay_grant_time = sal_time();

                /* Select message to service. */
                tlvType = _bcmPTPtlvType_request_unicast_transmission;
                tlvTypeResp = _bcmPTPtlvType_grant_unicast_transmission;
                messageType = _bcmPTPmessageType_delay_resp;
...
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].announce_ack_time = sal_time();
#endif

            top_needs_updating = 1;

            /* Select message to service. */
            tlvType = _bcmPTPtlvType_cancel_unicast_transmission;
            tlvTypeResp = _bcmPTPtlvType_ack_cancel_unicast_transmission;
            messageType = _bcmPTPmessageType_announce;
            logInterMessagePeriod = signaled_slave_table.entry[i].slave.log_announce_interval;
            durationField = signaled_slave_table.entry[i].announce_duration;
            slave = signaled_slave_table.entry[i].slave;
...
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].sync_ack_time = sal_time();
#endif
...
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].delay_ack_time = sal_time();
#endif
        ...
        if (signaled_slave_table.entry[i].queue_announce_expire) {
            ...
            signaled_slave_table.entry[i].slave.log_announce_interval = 99;
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].announce_expire_time = sal_time();
#endif
            ...
            /* Select message to service. */
            tlvType = _bcmPTPtlvType_cancel_unicast_transmission;
            tlvTypeResp = _bcmPTPtlvType_cancel_unicast_transmission;
            messageType = _bcmPTPmessageType_announce;
            logInterMessagePeriod = signaled_slave_table.entry[i].slave.log_announce_interval;
            durationField = signaled_slave_table.entry[i].announce_duration;
            slave = signaled_slave_table.entry[i].slave;
        ...
        if (signaled_slave_table.entry[i].queue_sync_expire) {
            ...
            signaled_slave_table.entry[i].slave.log_sync_interval = 99;
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].sync_expire_time = sal_time();
#endif
        ...
        if (signaled_slave_table.entry[i].queue_delay_expire) {
            ...
            signaled_slave_table.entry[i].slave.log_delay_request_interval = 99;
#if defined(BCM_PTP_SIGNAL_TIMESTAMP)
            signaled_slave_table.entry[i].delay_expire_time = sal_time();
#endif
    ...
    if (isqueued) {
        ...
        rv = _bcm_ptp_slave_signaling_send(unit, ptp_id, clock_num, tlvTypeResp,
                                           messageType, logInterMessagePeriod, durationField, &slave);
        if (BCM_FAILURE(rv)) {
        ...
        if (top_needs_updating) {
            /* Subscribe slave. */
            rv = _bcm_ptp_unicast_slave_subscribe(unit, ptp_id, clock_num, BCM_PTP_ANY_PORT, &slave,
                                                  messageType, tlvType, logInterMessagePeriod);
            if (BCM_FAILURE(rv)) {
...
/*
 * Function:
 *      _bcm_ptp_signaled_unicast_slave_clear()
 * Purpose:
    ...
    for (i = 0; i < signaled_slave_table.num_entries; ++i)
    {
       ...
       if (port_num == slave.local_port_number ||
           ...
           if (signaled_slave_table.entry[i].announce_service_active) {
                ...
                /* Cancel message. */
                rv = _bcm_ptp_slave_signaling_send(unit, ptp_id, clock_num, tlvType, messageType,
                                                   logInterMessagePeriod, durationField, &slave);
                if (BCM_FAILURE(rv)) {
                    PTP_ERROR_FUNC("_bcm_ptp_slave_signaling_send()");
                }
    ...
    uint8 *data)
{
    ...
    _bcmPTPtlvType_t tlvType;
    _bcmPTPtlvType_t tlvTypes[BCM_PTP_SIGNAL_MAX_TLV_PER_MESSAGE];

    _bcmPTPmessageType_t messageType;
    _bcmPTPmessageType_t messageTypes[BCM_PTP_SIGNAL_MAX_TLV_PER_MESSAGE];
    ...
    _bcm_ptp_signaling_arbiter_t arbiter = unit_sig_array[unit].arbiter;
    ...
    for (tlv_num = 0; tlv_num < num_tlvs; ++tlv_num) {
        ...
        if (tlvType == _bcmPTPtlvType_request_unicast_transmission) {
            if (!arbiter) {
                return PTP_CALLBACK_REJECT;
            }
            /*
             ...
             */
            clock = &_bcm_esw_ptp_unit_array[unit].stack_array[ptp_id].clock_array[clock_num];

            rvcb = arbiter(unit, ptp_id, clock_num, port_num, data_length, data,
                           protocol, src_addr_offset, ptp_msg_offset, &clock->clock_info,
                           &tlvType, &messageType, &logInterMessagePeriod, &durationField,
                           &peer, user_data);

            if (PTP_CALLBACK_REJECT == rvcb) {
                /*
        ...
        case _bcmPTPtlvType_grant_unicast_transmission:
            {
                ...
                for (idx = 0; idx < num_signaled_masters; ++idx) {
                    if (_bcm_ptp_peer_address_raw_compare(&signaled_master[idx].master.peer_address,
                        ...
                        case _bcmPTPmessageType_announce:
                            if (durationField) {
                                signaled_master[idx].announce_refresh_time = sal_time() + durationField / 2 + 1;
                        ...
                        case _bcmPTPmessageType_sync:
                            if (durationField) {
                                signaled_master[idx].sync_refresh_time = sal_time() + durationField / 2 + 1;
                        ...
                        case _bcmPTPmessageType_delay_resp:
                            if (durationField) {
                                signaled_master[idx].delay_refresh_time = sal_time() + durationField / 2 + 1;
        ...
        case _bcmPTPtlvType_cancel_unicast_transmission:
            {
                    ...
                    if (_bcm_ptp_peer_address_raw_compare(&signaled_master[idx].master.peer_address,
                        ...
                        case _bcmPTPmessageType_announce:
                            signaled_master[idx].announce_refresh_time =
                                sal_time() + signaled_master[idx].query_interval_sec;
                            signaled_master[idx].service_grants.announce = 0;
                            goto release_return;

                        case _bcmPTPmessageType_sync:
                            signaled_master[idx].sync_refresh_time = sal_time() +
                                signaled_master[idx].query_interval_sec;
                            signaled_master[idx].service_grants.sync = 0;
                            goto release_return;

                        case _bcmPTPmessageType_delay_resp:
                            signaled_master[idx].delay_refresh_time = sal_time() +
                                signaled_master[idx].query_interval_sec;
                            signaled_master[idx].service_grants.delayresp = 0;
                            goto release_return;
                ...
                switch (messageType) {
                case _bcmPTPmessageType_announce:
                    signaled_slave_table.entry[i].announce_cancel_time = sal_time();
                    signaled_slave_table.entry[i].queue_announce_cancel = 1;
                    goto release_return;

                case _bcmPTPmessageType_sync:
                    signaled_slave_table.entry[i].sync_cancel_time = sal_time();
                    signaled_slave_table.entry[i].queue_sync_cancel = 1;
                    goto release_return;

                case _bcmPTPmessageType_delay_resp:
                    signaled_slave_table.entry[i].delay_cancel_time = sal_time();
                    signaled_slave_table.entry[i].queue_delay_cancel = 1;
                    goto release_return;
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_get
 * Purpose:
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_set
 * Purpose:
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_min_get
 * Purpose:
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_min_set
 * Purpose:
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_max_get
 * Purpose:
...
/*
 * Function:
 *      _bcm_ptp_unicast_request_duration_max_set
 * Purpose:
    ...
    void *arg_clock_num)
{
    ...
    bcm_ptp_unicast_master_table_add(unit, ptp_id, clock_num,
