/*
 * $Id: metrics.c 1.9.16.1 Broadcom SDK $
 *
    ...
    uint16 alpha_denominator)
{
    ...
    soc_cm_print("bcm_esw_ptp_ctdev_alpha_set(): ALPHA = %u/%u\n",
    ...
    uint8 *ev_data)
{
    ...
    if (0 == ctdev_options.enable) {
        ...
        if (ctdev_options.verbose) {
            soc_cm_print("_bcm_ptp_ctdev_gateway(): C-TDEV disabled.\n");
        }
        return BCM_E_NONE;
    }
    ...
    while (numel--) {
        freq_corr_ppt = (int64)_bcm_ptp_uint64_read(ev_data+cursor);
        cursor += sizeof(int64);
        ...
        if (ctdev_options.verbose) {
             ...
             */
            /*
            soc_cm_print("%u,%lld,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu\n",
    ...
    const int64 freq_corr_ppt)
{
    ...
    if (0 == ctdev_options.enable) {
        /* C-TDEV disabled. */
        if (ctdev_options.verbose) {
            soc_cm_print("_bcm_ptp_ctdev_phase_accumulator(): C-TDEV disabled.\n");
        }
        return BCM_E_NONE;
    }
    ...
    int clock_num)
{
    ...
    if (0 == ctdev_options.enable) {
        /* C-TDEV disabled. */
        if (ctdev_options.verbose) {
            soc_cm_print("_bcm_ptp_ctdev_calculator(): C-TDEV disabled.\n");
        }
        return BCM_E_NONE;
    }
    ...
    uint64 *tdev_psec)
{
    ...
    if (0 == ctdev_options.enable) {
        /* C-TDEV disabled. */
        if (ctdev_options.verbose) {
            soc_cm_print("_bcm_ptp_ctdev_get(): C-TDEV disabled.\n");
        }
