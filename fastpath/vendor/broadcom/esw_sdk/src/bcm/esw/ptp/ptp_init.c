/*
 * $Id: ptp_init.c 1.1.2.33 Broadcom SDK $
 * 
...
int bcm_esw_ptp_lock(int unit)
{
    ...
    return _bcm_ptp_mutex_take(_bcm_esw_ptp_info[unit].mutex, 100);
}
...
{
    if (_bcm_ptp_mutex_give(_bcm_esw_ptp_info[unit].mutex) != 0) {
    ...
    int unit)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        if (_bcm_esw_ptp_info[unit].mutex == NULL) {
            _bcm_esw_ptp_info[unit].mutex = _bcm_ptp_mutex_create("ptp.mutex");
            if (_bcm_esw_ptp_info[unit].mutex == NULL) {
                rv = BCM_E_MEMORY;
            }
        }
    ...
    if (BCM_FAILURE(rv)) {
       _bcm_ptp_mutex_destroy(_bcm_esw_ptp_info[unit].mutex);
    ...
    bcm_ptp_stack_info_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        ...
        if (BCM_FAILURE(rv = _bcm_ptp_telecom_init(
                unit, stack_p->stack_id, PTP_CLOCK_NUMBER_DEFAULT))) {
            PTP_ERROR_FUNC("_bcm_ptp_telecom_init()");
            goto done;
        }
    ...
    bcm_ptp_cb cb)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        ...
        if (cb_types.w[bcmPTPCallbackTypeCheckPeers]) {
    ...
    bcm_ptp_tod_input_t *tod_input)
{
    ...
    *tod_input = _bcm_esw_ptp_unit_array[unit].stack_array[ptp_id]
    ...
    bcm_ptp_tod_output_t *tod_output)
{
    ...
    *tod_output = _bcm_esw_ptp_unit_array[unit].stack_array[ptp_id]
                                               .clock_array[clock_num]
