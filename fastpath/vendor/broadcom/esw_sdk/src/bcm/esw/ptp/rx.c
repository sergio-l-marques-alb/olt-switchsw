...
void (*_bcm_ptp_arp_callback)(int unit, bcm_ptp_stack_id_t ptp_id, int protocol, int src_addr_offset, int payload_offset, int msg_len, uint8 *msg);
    ...
    while (!finished) {
        ...
        for (mbox = 0; mbox < BCM_PTP_MAX_BUFFERS; ++mbox) {
            ...
            case MBOX_STATUS_ATTN_HOST_TUNNEL: {
                ...
                /* ptp_printf("Got Tunnel: %d, %d (%d)\n", message_type, protocol, (int)this_time - (int)last_time); */
                ...
                if (message_type == _bcmPTPmessageType_signaling ||
                    message_type == _bcmPTPmessageType_management) {
            ...
            case MBOX_STATUS_ATTN_HOST_RESP: {
                ...
                /* ptp_printf("Got HOST_RESP at %d (%d)\n", (int)this_time, (int)this_time - (int)last_time); */
    ...
    void *cookie)
{
    ...
    int wrapClockNumber;
    int wrapPortNumber;
    _bcmPTPmessageType_t wrapMessageType;
    ...
    case (0x0141):
         ...
         * identity.
         */
        if (wrapMessageType == _bcmPTPmessageType_signaling ||
            wrapMessageType == _bcmPTPmessageType_management) {
        ...
        case _bcmPTPmessageType_signaling:
            ...
            if (unit_rx_array[unit].signal_cb) {
                unit_rx_array[unit].signal_cb(unit, cb_ptp_id, cb_flags,
                    bcmPTPCallbackTypeSignal,
        ...
        case _bcmPTPmessageType_management:
            ...
            if (unit_rx_array[unit].management_cb) {
                if (unit_rx_array[unit].management_cb(unit, cb_ptp_id, cb_flags,
                        bcmPTPCallbackTypeManagement,
                        message_len, BCM_PKT_IEEE(pkt) + i,
                        unit_rx_array[unit].management_user_data) == PTP_CALLBACK_ACCEPT) {
    ...
    case (0x0142):
        ...
        if (BCM_FAILURE(rv = _bcm_ptp_event_message_monitor(unit, cb_ptp_id,
                message_len, BCM_PKT_IEEE(pkt) + PTP_PTPHDR_START_IDX))) {
                PTP_ERROR_FUNC("_bcm_ptp_event_message_monitor()");
        }
        ...
        if (unit_rx_array[unit].event_cb) {
            ...
            unit_rx_array[unit].event_cb(unit, cb_ptp_id, cb_flags,
                bcmPTPCallbackTypeEvent,
                message_len, BCM_PKT_IEEE(pkt) + i,
                unit_rx_array[unit].event_user_data);
        }
    ...
    void *user_data)
{
    ...
    int clock_num = _bcm_ptp_most_recent_clock_num;
    ...
    _bcm_ptp_dump_hex(data, length,0);
#endif

    return _bcm_ptp_process_incoming_signaling_msg(unit, ptp_id, clock_num, port_num, protocol, src_addr_offset, msg_offset, length, data);
}
