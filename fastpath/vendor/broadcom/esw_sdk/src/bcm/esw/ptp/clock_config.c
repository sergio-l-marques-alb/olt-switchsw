/*
 * $Id: clock_config.c 1.19 Broadcom SDK $
 *
    ...
    _bcm_ptp_port_state_t *portState)
{
    ...
    *portState = _bcm_esw_ptp_unit_array[unit].stack_array[ptp_id]
    ...
    bcm_ptp_foreign_master_dataset_t *data_set)
{
    ...
    BCM_IF_ERROR_RETURN(
        bcm_esw_ptp_clock_port_identity_get(unit, ptp_id, clock_num,
                                            port_num, &portid));
    ...
    uint32 domain)
{
    ...
    if (BCM_FAILURE(rv = bcm_esw_ptp_signaled_unicast_slave_table_clear(unit, ptp_id,
