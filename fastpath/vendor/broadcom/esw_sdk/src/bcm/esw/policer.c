/*
 * $Id: policer.c 1.30 Broadcom SDK $
...
int _bcm_global_meter_min_burst_size_set(bcm_policer_config_t *pol_cfg)
{
    ...
    min_burst = ((pol_cfg->ckbits_sec * METER_REFRESH_INTERVAL) /
                                    CONVERT_SECOND_TO_MICROSECOND);
    if (min_burst > pol_cfg->ckbits_burst) {
        pol_cfg->ckbits_burst = min_burst;
        POLICER_WARN(("%s : Commited burst is less than the minimum required \
                            value. Hence changing it to minimum required value \
                            \n", FUNCTION_NAME()));
    } 
    min_burst = ((pol_cfg->pkbits_sec * METER_REFRESH_INTERVAL) /
                                    CONVERT_SECOND_TO_MICROSECOND);
    if (min_burst > pol_cfg->pkbits_burst) {
        pol_cfg->pkbits_burst = min_burst;
        POLICER_WARN(("%s : Peak burst is less than the minimum required \
                            value. Hence changing it to minimum required value \
                            \n", FUNCTION_NAME()));
    } 
    return (BCM_E_NONE); 
    ...
    switch (mode) {
        ...
        case bcmPolicerGroupModeDlfAll:
        {
            *npolicers = 2;
            ...
            BCM_IF_ERROR_RETURN(rv);
#if 0
            table_offset[0] = pkt_res[L2DLF_PKT];
#endif
...
#endif
                pkt_res_map[(pkt_res[UNKNOWN_L2UC_PKT] << 2) + offset] = 1 << 2;
            }
            /* Reset pri_cnf map */
        ...
        case bcmPolicerGroupModeTypedIntPri:
        {
            *npolicers = 20;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(pkt_res[UNKNOWN_L3UC_PKT] << 2) + offset] = 1 << 2;
                pkt_res_map[(pkt_res[KNOWN_L2UC_PKT] << 2) + offset] = 2 << 2;
                pkt_res_map[(pkt_res[KNOWN_L3UC_PKT] << 2) + offset] = 2 << 2;
                pkt_res_map[(pkt_res[KNOWN_L2MC_PKT] << 2) + offset] = 3 << 2;
                pkt_res_map[(pkt_res[UNKNOWN_L2MC_PKT] << 2) + offset] = 3 << 2;
                pkt_res_map[(pkt_res[L2BC_PKT] << 2) + offset] = 4 << 2;
            }
            /* Reset pri_cng map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
        ...
        case bcmPolicerGroupModeDlfIntPriWithControl:
        {
            *npolicers = 18;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(pkt_res[CONTROL_PKT] << 2) + offset] = 1 << 2;
                pkt_res_map[(pkt_res[BPDU_PKT] << 2) + offset] = 1 << 2;
                pkt_res_map[(pkt_res[UNKNOWN_L2UC_PKT] << 2) + offset] = 2 << 2;
            }
            /* Reset pri_cng map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
        ...
        case bcmPolicerGroupModeTypedIntPriWithControl:
        {
            *npolicers = 21;
            ...
            for (offset = 0; offset < 4; offset++) { 
                pkt_res_map[(pkt_res[CONTROL_PKT] << 2) + offset] = 1 << 2;
                pkt_res_map[(pkt_res[BPDU_PKT] << 2) + offset] = 1 << 2;
                pkt_res_map[(pkt_res[UNKNOWN_L3UC_PKT] << 2)+ offset] = 2 << 2;
                pkt_res_map[(pkt_res[KNOWN_L2UC_PKT] << 2) + offset] = 3 << 2;
                pkt_res_map[(pkt_res[KNOWN_L3UC_PKT] << 2) + offset] = 3 << 2;
                pkt_res_map[(pkt_res[KNOWN_L2MC_PKT] << 2) + offset] = 4 << 2;
                pkt_res_map[(pkt_res[UNKNOWN_L2MC_PKT] << 2) + offset] = 4 << 2;
                pkt_res_map[(pkt_res[L2BC_PKT] << 2) + offset] = 5 << 2;
            }
            /* Reset pri_cnf map */
            pri_cnf_map = &mode_attr->compressed_attr_selectors_v.\
                                             compressed_pri_cnf_attr_map_v[0];
