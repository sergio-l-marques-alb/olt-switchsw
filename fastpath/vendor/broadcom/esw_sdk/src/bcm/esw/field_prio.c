/* $Id: field_prio.c 1.15.230.2 Broadcom SDK $
...
_bcm_field_prio_mgmt_init(int unit, _field_stage_t *stage_fc)
{
    ...
    for (i = 0; i < stage_fc->tcam_slices; i++) {
        ...
        prio_temp->end_index = stage_fc->slices[i].entry_count - 1;
        prio_temp->num_free_entries = stage_fc->slices[i].entry_count;
        prio_temp->prev = NULL;
        prio_temp->next = NULL;

        stage_fc->slices[i].prio_mgmt = prio_temp;
    }
...
_bcm_field_prio_mgmt_deinit(int unit, _field_stage_t *stage_fc)
{
    ...
    for (i = 0; i < stage_fc->tcam_slices; i++) {
        prio1 = stage_fc->slices[i].prio_mgmt;
        while (prio1 != NULL) {
            prio2 = prio1->next;
            sal_free(prio1);
            prio1 = prio2;
        }
        stage_fc->slices[i].prio_mgmt = NULL;
    }
                            ...
                            _field_prio_mgmt_t *target_node)
{
    ...
    while (node1 != node2) {
        _field_prio_mgmt_move(unit, 
            stage_fc->slices[slice_numb].entries[node1->end_index], 
                            ...
                            _field_prio_mgmt_t *next_free_node)
{
    ...
    while (node2 != node1) {
        if ((node2->end_index - node2->start_index + 1) >
            ...
            _field_prio_mgmt_move(unit, 
                stage_fc->slices[slice_numb].entries[node2->start_index], 
                node2->start_index, 
                                 ...
                                 uint32 *new_location)
{
    ...
    list = stage_fc->slices[slice_numb].prio_mgmt;
    ...
    if (target_node->num_free_entries > 0) {
         ...
         */
            ...
            *new_location = node->start_index;
            ...
            if (node->prev == NULL) {
                stage_fc->slices[slice_numb].prio_mgmt = node;
            } else {
                node->prev->next = node;
            }
                                  ...
                                  int flag, uint32 old_location)
{
    ...
    if (flag == 1) {
        ...
        if (old_location != (list->end_index - list->num_free_entries)) {
            _field_prio_mgmt_move(unit, 
                                  stage_fc->slices[f_ent->fs->slice_number].
