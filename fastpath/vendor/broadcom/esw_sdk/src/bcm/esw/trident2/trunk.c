...
bcm_td2_hg_rh_status_get(int unit, int *rh_status)
{
    ...
    *rh_status = FALSE;
    ...
    for (i = chip_info.trunk_fabric_id_min;
        ...
        BCM_IF_ERROR_RETURN(bcm_esw_trunk_psc_get(unit, i, &psc));
...
void bcm_td2_hg_rh_sw_dump(int unit)
{
    ...
    soc_cm_print("Higig Resilient Hashing Info -\n");

    /* Print Higig RH flowset table usage */
    soc_cm_print("    Higig RH Flowset Table Blocks Used:");
    j = 0;
...
void bcm_td2_lag_rh_sw_dump(int unit)
{
    ...
    soc_cm_print("LAG Resilient Hashing Info -\n");
    soc_cm_print("    LAG RH Flowset Table Blocks Total: %d\n",
            _td2_lag_rh_info[unit]->num_lag_rh_flowset_blocks);

    /* Print LAG RH flowset table usage */
    soc_cm_print("    LAG RH Flowset Table Blocks Used:");
    j = 0;
...
bcm_td2_vp_lag_sw_dump(int unit)
{
    ...
    soc_cm_print("VP LAG Information:\n");
    soc_cm_print("  Max number of VP LAGs: %d\n", MAX_VP_LAGS(unit));

    (void)bcm_esw_trunk_chip_info_get(unit, &trunk_chip_info);
    soc_cm_print("  VP LAG trunk ID min:   %d\n", trunk_chip_info.vp_id_min);
    soc_cm_print("  VP LAG trunk ID max:   %d\n", trunk_chip_info.vp_id_max);
    soc_cm_print("  VP LAG max members:    %d\n", trunk_chip_info.vp_ports_max);

    for (i = 0; i < MAX_VP_LAGS(unit); i++) {
        if (VP_LAG_USED_GET(unit, i)) {
            soc_cm_print("  VP LAG %d: vp_id = %d, ", i,
                    VP_LAG_GROUP_INFO(unit, i).vp_id);
            soc_cm_print("has_member = %d, ",
                    VP_LAG_GROUP_INFO(unit, i).has_member);
            soc_cm_print("non_uc_index = %d\n",
