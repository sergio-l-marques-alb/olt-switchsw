/*
 * $Id: vxlan.c 1.60.2.6 Broadcom SDK $
 *
...
int _bcm_td2_vxlan_port_get(int unit, bcm_vpn_t vpn, int vp, bcm_vxlan_port_t  *vxlan_port);
STATIC int _bcm_td2_vxlan_match_vnid_entry_set(int unit, mpls_entry_entry_t *ment);
STATIC int _bcm_td2_vxlan_match_vnid_entry_reset(int unit, mpls_entry_entry_t *ment);
STATIC int _bcm_td2_vxlan_egr_xlate_entry_set(int unit, egr_vlan_xlate_entry_t  *key_ent);
 ...
 */
int
bcm_td2_vxlan_udpDestPort_set(int unit, uint16 udpDestPort)
{
    int rv = BCM_E_NONE;
    uint64 reg64;
...
_bcm_td2_vxlan_hw_clear(int unit)
{
    ...
    rv_error = _bcm_td2_vxlan_bud_loopback_disable(unit);
...
_bcm_td2_vxlan_reinit(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (_BCM_VXLAN_KEY_TYPE_TUNNEL == key_type) {
            if (_bcm_vp_used_get(unit, vp, _bcmVpTypeVxlan)) {
                vxlan_info->match_key[vp].match_tunnel_sip = 
                  soc_MPLS_ENTRYm_field32_get(unit, &ment, VXLAN_SIP__SIPf);
            }
        } else if (_BCM_VXLAN_KEY_TYPE_TUNNEL_VNID_VFI == key_type) {
            vxlan_info->match_key[vp].match_tunnel_sip = 
              soc_MPLS_ENTRYm_field32_get(unit, &ment, VXLAN_VN_ID__SIPf);
            vxlan_info->match_key[vp].flags =
                                     _BCM_VXLAN_PORT_MATCH_TYPE_TUNNEL_VNID;
        } else if (_BCM_VXLAN_KEY_TYPE_VNID_VFI == key_type) {
            vxlan_info->match_key[vp].flags =
...
bcm_td2_vxlan_init(int unit)
{
    ...
    if (!L3_INFO(unit)->l3_initialized) {
        soc_cm_debug(DK_ERR, "L3 module must be initialized prior to VXLAN Init\n");
        return BCM_E_CONFIG;
    }
...
_bcm_td2_vxlan_bud_loopback_enable(int unit)
{
   ...
   rv = soc_mem_read(unit, LPORT_TABm, MEM_BLOCK_ALL, 0, &lport_profile);
   if (rv < 0) {
      return rv;
   }

   soc_LPORT_TABm_field32_set(unit, &lport_profile, PORT_TYPEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_TERMINATION_ALLOWEDf, 0x1);   
   soc_LPORT_TABm_field32_set(unit, &lport_profile, V4IPMC_ENABLEf, 0x1);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_VN_ID_LOOKUP_KEY_TYPEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_DEFAULT_SVP_ENABLEf, 0x1);
   rv = soc_mem_write(unit, LPORT_TABm,
                        MEM_BLOCK_ALL, 0, &lport_profile);
   return rv;
}
...
_bcm_td2_vxlan_bud_loopback_disable(int unit)
{
   ...
   soc_LPORT_TABm_field32_set(unit, &lport_profile, PORT_TYPEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_TERMINATION_ALLOWEDf, 0x0);   
   soc_LPORT_TABm_field32_set(unit, &lport_profile, V4IPMC_ENABLEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_VN_ID_LOOKUP_KEY_TYPEf, 0x0);
   soc_LPORT_TABm_field32_set(unit, &lport_profile, VXLAN_DEFAULT_SVP_ENABLEf, 0x0);
   rv = soc_mem_write(unit, LPORT_TABm,
                        MEM_BLOCK_ALL, 0, &lport_profile);
   return rv;
}
...
_bcm_td2_vxlan_vpn_is_eline( int unit, bcm_vpn_t l2vpn, uint8 *isEline)
{
    ...
    /* Check for Valid vpn */
     _BCM_VXLAN_VPN_SET(vxlan_vpn_min, _BCM_VXLAN_VPN_TYPE_ELINE, 0);
...
bcm_td2_vxlan_vpn_create(int unit, bcm_vxlan_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_VXLAN_VPN_ELINE) {
        ...
        if (info->flags & BCM_VXLAN_VPN_WITH_ID) {
            _BCM_VXLAN_VPN_SET(vxlan_vpn_min, _BCM_VXLAN_VPN_TYPE_ELINE, 0);
            if ( info->vpn < vxlan_vpn_min || info->vpn > (vxlan_vpn_min+num_vfi-1) ) {
                return BCM_E_PARAM;
            }

            _BCM_VXLAN_VPN_GET(vfi_index, _BCM_VXLAN_VPN_TYPE_ELINE,  info->vpn);
            if (_bcm_vfi_used_get(unit, vfi_index, _bcmVfiTypeVxlan)) {
                return BCM_E_EXISTS;
            } else {
                rv = _bcm_vfi_alloc_with_id(unit, VFIm, _bcmVfiTypeVxlan, vfi_index);
                if (rv < 0) {
                    return rv;
                }
            }
        } else {
            rv = _bcm_vfi_alloc(unit, VFIm, _bcmVfiTypeVxlan, &vfi_index);
            if (rv < 0) {
                return rv;
            }
        }
        ...
        _BCM_VXLAN_VPN_SET(info->vpn, _BCM_VXLAN_VPN_TYPE_ELINE, vfi_index);
        ...
        if (info->flags & BCM_VXLAN_VPN_WITH_ID) {
            _BCM_VXLAN_VPN_SET(vxlan_vpn_min, _BCM_VXLAN_VPN_TYPE_ELAN, 0);
            if ( info->vpn < vxlan_vpn_min || info->vpn > (vxlan_vpn_min+num_vfi-1) ) {
                return BCM_E_PARAM;
            }

            _BCM_VXLAN_VPN_GET(vfi_index, _BCM_VXLAN_VPN_TYPE_ELAN,  info->vpn);
            if (_bcm_vfi_used_get(unit, vfi_index, _bcmVfiTypeVxlan)) {
                return BCM_E_EXISTS;
            } else {
                rv = _bcm_vfi_alloc_with_id(unit, VFIm, _bcmVfiTypeVxlan, vfi_index);
        ...
        _BCM_VXLAN_VPN_SET(info->vpn, _BCM_VXLAN_VPN_TYPE_ELAN, vfi_index);
    }
    ...
    if (info->flags & BCM_VXLAN_VPN_WITH_VPNID) {
        ...
        soc_MPLS_ENTRYm_field32_set(unit, &ment, VXLAN_VN_ID__VN_IDf, 
                                    info->vnid);
        ...
        rv = _bcm_td2_vxlan_match_vnid_entry_set(unit, &ment);
        BCM_IF_ERROR_RETURN(rv);

        /* Configure Egress VNID */
        sal_memset(&vxlate_entry, 0, sizeof(egr_vlan_xlate_entry_t));
        soc_EGR_VLAN_XLATEm_field32_set(unit, &vxlate_entry, ENTRY_TYPEf, 
                      _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI);
        ...
        rv = _bcm_td2_vxlan_egr_xlate_entry_set(unit, &vxlate_entry);
        return rv;
    }
...
STATIC int 
_bcm_td2_vxlan_vpn_vnid_get( int unit, int vfi, uint32 *vnid)
{
   /* Indexes to iterate over memories, chunks and entries */
    ...
    vxlan_vnid_chunk = soc_cm_salloc(unit, buf_size, "Vxlan vpn vnid get");
    if (NULL == vxlan_vnid_chunk) {
        return BCM_E_MEMORY;
    }
    ...
    for (chnk_idx = soc_mem_index_min(unit, mem); 
        ...
        sal_memset((void *)vxlan_vnid_chunk, 0, buf_size);
        ...
        rv = soc_mem_read_range(unit, mem, MEM_BLOCK_ANY,
                                chnk_idx, chnk_idx_max, vxlan_vnid_chunk);
        if (SOC_FAILURE(rv)) {
            break;
        }
        ...
        for (ent_idx = 0 ; ent_idx <= chnk_end; ent_idx ++) {
            ment = soc_mem_table_idx_to_pointer(unit, mem, mpls_entry_entry_t *, 
                                             vxlan_vnid_chunk, ent_idx);

            if (!soc_mem_field32_get(unit, mem, ment, VALIDf)) {
                continue;
            }
          
            /* Match the Key Type to VXLAN */
            key_type = soc_mem_field32_get(unit, mem, ment, KEY_TYPEf);

            if (key_type != _BCM_VXLAN_KEY_TYPE_VNID_VFI) { 
                continue;
            }

            /* Check for Valid VFI */
            if (vfi != soc_mem_field32_get(unit, mem, ment, VXLAN_VN_ID__VFIf) ) {
                continue;
            }

            *vnid = soc_mem_field32_get(unit, mem, ment, VXLAN_VN_ID__VN_IDf);
        }
    }
    soc_cm_sfree(unit, vxlan_vnid_chunk);
    return rv;
}
...
bcm_td2_vxlan_vpn_destroy(int unit, bcm_vpn_t l2vpn)
{
    ...
    rv = _bcm_td2_vxlan_vpn_vnid_get(unit, vfi,  &vnid);
    BCM_IF_ERROR_RETURN(rv);
    ...
    rv = _bcm_td2_vxlan_match_vnid_entry_reset(unit, &ment);
    BCM_IF_ERROR_RETURN(rv);
...
{
    int num_vfi=0, idx=0, rv = BCM_E_NONE;
    bcm_vpn_t l2vpn=0;
    vfi_entry_t vfi_entry;
    ...
    for (idx = 0; idx < num_vfi; idx++) {
         if (_bcm_vfi_used_get(unit, idx, _bcmVfiTypeVxlan)) {
              ...
              if (soc_VFIm_field32_get(unit, &vfi_entry, PT2PT_ENf)) {
                   _BCM_VXLAN_VPN_SET(l2vpn, _BCM_VXLAN_VPN_TYPE_ELINE, idx);
              } else {
                   _BCM_VXLAN_VPN_SET(l2vpn, _BCM_VXLAN_VPN_TYPE_ELAN, idx);
              }
...
bcm_td2_vxlan_vpn_get( int unit, bcm_vpn_t l2vpn, bcm_vxlan_vpn_config_t *info)
{
    ...
    /* Get VNID */
    info->vnid = 0;
    rv = _bcm_td2_vxlan_vpn_vnid_get(unit, vfi_index,  &info->vnid);
    if (info->vnid != 0) {
                             ...
                             void *user_data)
{
    ...
    for (idx = 0; idx < num_vfi; idx++) {
         if (_bcm_vfi_used_get(unit, idx, _bcmVfiTypeVxlan)) {
              _BCM_VXLAN_VPN_SET(vpn, _BCM_VXLAN_VPN_TYPE_ELAN, idx);
              bcm_vxlan_vpn_config_t_init(&info);
              rv = bcm_td2_vxlan_vpn_get(unit, vpn, &info);
              if (BCM_FAILURE(rv)) {
                   return rv;
              }
...
/*
 * Function:
 *      bcm_td2_vxlan_multicast_transit_entry_check
 * Purpose:
...
STATIC int
_bcm_td2_vxlan_multicast_transit_entry_check (int unit, 
                               bcm_tunnel_terminator_t *tnl_info, uint8 *out_mode)
{
   ...
   if (BCM_SUCCESS(rv)) {
      *out_mode = _BCM_VXLAN_MULTICAST_BUD;
   } else {
       *out_mode = _BCM_VXLAN_MULTICAST_LEAF;
   }
   return BCM_E_NONE;
}
...
int
bcm_td2_vxlan_multicast_leaf_entry_check(int unit, bcm_ip_t mc_ip_addr, int enable_flag)
{
...
bcm_td2_vxlan_tunnel_terminator_create(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    BCM_IF_ERROR_RETURN (_bcm_td2_vxlan_multicast_transit_entry_check(unit, tnl_info, &out_mode));
    (void) _bcm_td2_vxlan_tunnel_terminate_entry_key_set(unit, tnl_info, &vxlate_entry, out_mode, 1);

    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ANY, &index,
                        &vxlate_entry, &return_entry, 0);

    if (rv == SOC_E_NONE) {
         (void) _bcm_td2_vxlan_tunnel_terminate_entry_key_set(unit, tnl_info, &vxlate_entry, out_mode, 0);
         rv = soc_mem_write(unit, VLAN_XLATEm,
                                           MEM_BLOCK_ALL, index,
                                           &return_entry);
...
bcm_td2_vxlan_tunnel_terminator_destroy(int unit, bcm_gport_t vxlan_tunnel_id)
{
    ...
    if (BCM_SUCCESS(rv) && (tunnel_idx != -1)) {
       vxlan_info->vxlan_tunnel_term[tunnel_idx].sip = 0;
...
{
    int rv = BCM_E_UNAVAIL, i, num_entries;
    vlan_xlate_entry_t  vxlate_entry;
 ...
 */
STATIC int
_bcm_td2_vxlan_tunnel_init_get(int unit, int idx, bcm_tunnel_initiator_t *info)
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info;
    ...
    if (idx > num_tnl) { /* Software State */
        ...
        /* Find matching entry within Hardware-Map */
       for (tnl_idx=0; tnl_idx < num_tnl; tnl_idx++) {
            if (vxlan_info->vxlan_tunnel_init[tnl_idx].sip == sip) {
...
{
    _bcm_td2_vxlan_bookkeeping_t *vxlan_info;
    int tnl_idx=-1, rv = BCM_E_NONE, ref_count=0, idx=0;
    uint32 flags = 0;
    bcm_tunnel_initiator_t info;           /* Tunnel initiator structure */
    int num_tnl=0, num_vp=0;
    bcm_ip_t sip=0;                       /* Tunnel header SIP (IPv4). */
    int hw_match_entry_to_be_deleted = 0;
    ...
    for (idx=0; idx < num_vp; idx++) {
        if (vxlan_info->vxlan_tunnel_init[idx].sip == sip) {
            ref_count++;
            if (idx < num_tnl) {
               hw_match_entry_to_be_deleted = 1;
            }
        }
    }

    if (hw_match_entry_to_be_deleted && (ref_count == 1)) {
       /* Get the entry first */
    ...
    if (!hw_match_entry_to_be_deleted ) {
       vxlan_info->vxlan_tunnel_init[tnl_idx].sip = 0;
       vxlan_info->vxlan_tunnel_init[tnl_idx].dip = 0;
    }
    return rv;
                                    ...
                                    int vp_nh_ecmp_index, bcm_vxlan_port_t *vxlan_port)
{
    ...
    int rv = BCM_E_NONE;
         ...
         if ((mpath_flag) && (vxlan_port->flags & BCM_VXLAN_MULTIPATH)) {
        ...
        if (vxlan_port->flags & BCM_VXLAN_PORT_EGRESS_TUNNEL) {
             ...
             soc_ING_DVP_TABLEm_field32_set(unit, &dvp, 
                           VP_TYPEf, _BCM_VXLAN_DEST_VP_TYPE_NETWORK);
        } else {
...
_bcm_td2_vxlan_egress_dvp_set(int unit, int vp, int drop, bcm_vxlan_port_t *vxlan_port)
{
    ...
    soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTEm, &egr_dvp_attribute,
                                            VP_TYPEf, _BCM_VXLAN_DEST_VP_ATTRIBUTE_TYPE);
    soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTEm, &egr_dvp_attribute,
    ...
    if (vxlan_port->flags & BCM_VXLAN_PORT_MULTICAST) {
        soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTE_1m, &egr_dvp_attribute_1,
                                            VXLAN__UUC_DROPf, drop ? 1 : 0);
        soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTE_1m, &egr_dvp_attribute_1,
                                            VXLAN__UMC_DROPf, drop ? 1 : 0);
        soc_mem_field32_set(unit, EGR_DVP_ATTRIBUTE_1m, &egr_dvp_attribute_1,
                                            VXLAN__BC_DROPf, drop ? 1 : 0);
    }
...
/*
 * Function:
 *		_bcm_td2_vxlan_learn_entry_key_set
 * Purpose:
 *		Set VXLAN egr_xlate key
 * Parameters:
 *		IN :  Unit
 *  IN :  vxlan_port
 *  IN :  VP
...
STATIC void
 _bcm_td2_vxlan_egr_xlate_entry_key_set(int unit, int vp, bcm_vpn_t vpn, bcm_vxlan_port_match_t criteria,
                                  egr_vlan_xlate_entry_t   *tr_ent, int clean_flag)
 {
    ...
    if (criteria == BCM_VXLAN_PORT_MATCH_VN_ID) {
         soc_EGR_VLAN_XLATEm_field32_set(unit, tr_ent, ENTRY_TYPEf, 
                      _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI);
    } else if (criteria == BCM_VXLAN_PORT_MATCH_TUNNEL_VN_ID) {
         soc_EGR_VLAN_XLATEm_field32_set(unit, tr_ent, ENTRY_TYPEf, 
    ...
    soc_EGR_VLAN_XLATEm_field32_set(unit, tr_ent, VXLAN_VFI__VFIf, vfi);
 }

/*
 * Function:
 *		_bcm_td_vxlan_learn_entry_set
 * Purpose:
 *		Set VXLAN  egr_xlate Entry 
 * Parameters:
 *		IN :  Unit
 *  IN :  vlan_xlate_entry
 * Returns:
...
STATIC int
_bcm_td2_vxlan_egr_xlate_entry_set(int unit, egr_vlan_xlate_entry_t  *key_ent)
{
    egr_vlan_xlate_entry_t  return_ent;
    int rv = BCM_E_UNAVAIL;
    int index=-1;

    sal_memset(&return_ent, 0, sizeof(egr_vlan_xlate_entry_t));
...
STATIC int
_bcm_td2_vxlan_egr_xlate_entry_reset(int unit, bcm_vpn_t vpn, int vp, bcm_vxlan_port_match_t criteria)
{
    int rv = BCM_E_UNAVAIL;
    int index=-1;
    ...
    _BCM_VXLAN_VPN_GET(vfi, _BCM_VXLAN_VPN_TYPE_ELAN,  vpn);
    if (criteria == BCM_VXLAN_PORT_MATCH_VN_ID) {
         soc_EGR_VLAN_XLATEm_field32_set(unit, &key_ent, ENTRY_TYPEf, 
                      _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI);
    } else if (criteria == BCM_VXLAN_PORT_MATCH_TUNNEL_VN_ID) {
         soc_EGR_VLAN_XLATEm_field32_set(unit, &key_ent, ENTRY_TYPEf, 
                      _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP);
...
STATIC int
_bcm_td2_vxlan_sd_tag_set( int unit, bcm_vxlan_port_t *vxlan_port, 
...
STATIC void
_bcm_td2_vxlan_sd_tag_get( int unit, bcm_vxlan_port_t *vxlan_port, 
                         egr_l3_next_hop_entry_t *egr_nh, 
                         egr_vlan_xlate_entry_t   *vxlate_entry,
...
/*
 * Function:
 *           bcm_td2_vxlan_egr_nexthop_mapping_set
 * Purpose:
 *           Set VXLAN Egress NextHop Mapping
 * Parameters:
 *           IN :  Unit
 *           IN :  nh_index
 * Returns:
...
int
bcm_td2_vxlan_egr_nexthop_mapping_set(int unit, int nh_index, bcm_vxlan_port_t *vxlan_port)
{
    ing_l3_next_hop_entry_t ing_nh;
    int rv=BCM_E_NONE;
    bcm_port_t      port=0;
    ...
    replace = vxlan_port->flags & BCM_VXLAN_PORT_REPLACE;
...
/*
 * Function:
 *           bcm_td2_vxlan_egress_nexthop_reset
 * Purpose:
...
int
bcm_td2_vxlan_egress_nexthop_reset(int unit, int nh_index, int vp_type, int vp, bcm_vpn_t vpn)
{
    soc_mem_t      hw_mem;
    egr_l3_next_hop_entry_t egr_nh;
    ...
    if (vp_type == _BCM_VXLAN_INGRESS_DEST_VP_TYPE) {
         /* Egressing into VXLAN tunnel */
         /* Find existing entry */
         (void) _bcm_td2_vxlan_egr_xlate_entry_key_set(unit, vp, vpn, vxlan_port.criteria, &vxlate_entry, 1);
         ...
         if (BCM_SUCCESS(rv)) {
            ...
            if ((action_not_present == 0x1) || (action_present == 0x1)) {
                old_tpid_idx = 
                    soc_EGR_VLAN_XLATEm_field32_get(unit, 
                                        &vxlate_entry, VXLAN_VFI__SD_TAG_TPID_INDEXf);
            }
         }

         /* Delete egr_vxlate entry */
         rv = _bcm_td2_vxlan_egr_xlate_entry_reset(unit, vpn, vp, vxlan_port.criteria);
         BCM_IF_ERROR_RETURN(rv);
    ...
    } else if (vp_type== _BCM_VXLAN_DEST_VP_TYPE_ACCESS) {
...
STATIC int
bcm_td2_vxlan_nexthop_reset(int unit, int nh_index, int vp_type,  int vp, bcm_vpn_t vpn)
{
    int rv=BCM_E_NONE;
    ing_l3_next_hop_entry_t ing_nh;
    bcm_port_t      port=0;
    ...
    bcm_td2_vxlan_egress_nexthop_reset(unit, nh_index, vp_type, vp, vpn);
...
/*
 * Function:
 *     _bcm_td2_vxlan_ingress_multipath_set
 * Purpose:
...
STATIC int
_bcm_td2_vxlan_ingress_multipath_set(int unit, int vp_nh_ecmp_index, bcm_vxlan_port_t *vxlan_port)
{
    uint32 hw_buf[SOC_MAX_MEM_FIELD_WORDS]; /* Buffer to read hw entry. */
    int  idx=0, max_ent_count=0, base_idx=0, nh_index=-1;
      ...
      for (idx = 0; idx < max_ent_count; idx++) {
           ...
           nh_index = soc_mem_field32_get(unit, L3_ECMPm, 
                     hw_buf, NEXT_HOP_INDEXf);
           BCM_IF_ERROR_RETURN(
                    bcm_td2_vxlan_egr_nexthop_mapping_set(unit, nh_index, vxlan_port));
      }
      return BCM_E_NONE;
}
...
/*
 * Function:
 *     _bcm_td2_vxlan_ingress_multipath_reset
 * Purpose:
...
STATIC int
_bcm_td2_vxlan_ingress_multipath_reset(int unit, int vp_nh_ecmp_index, 
    ...
    int  idx=0, max_ent_count=0, base_idx=0, nh_index=-1;
        ...
        BCM_IF_ERROR_RETURN(
         ...
         for (idx = 0; idx < max_ent_count; idx++) {
              ...
              if (ref_count == 1) {
                  BCM_IF_ERROR_RETURN(
                        bcm_td2_vxlan_nexthop_reset(unit, nh_index, vp_type, vp, vpn));
              }
         }
         return BCM_E_NONE;
}
...
int
bcm_td2_vxlan_egress_set(int unit, int nh_index)
{
    ing_l3_next_hop_entry_t ing_nh;
    soc_mem_t      hw_mem;
    ...
    if (vxlan_port->flags & BCM_VXLAN_PORT_EGRESS_TUNNEL) {
        ...
        if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
            egr_nh_info.entry_type = 
                soc_EGR_L3_NEXT_HOPm_field32_get(unit, &egr_nh, ENTRY_TYPEf);
            if (vxlan_port->flags & BCM_VXLAN_PORT_MULTICAST)  {
                if (egr_nh_info.entry_type != _BCM_VXLAN_EGR_NEXT_HOP_L3MC_VIEW) {
                    rv = BCM_E_PARAM;
                    goto cleanup;
                }
            } else {
                if (egr_nh_info.entry_type != _BCM_VXLAN_EGR_NEXT_HOP_L3_VIEW) {
                    rv = BCM_E_PARAM;
                    goto cleanup;
                }
            }
        ...
        if (vxlan_port->flags & BCM_VXLAN_PORT_SERVICE_TAGGED) {
            ...
            (void) _bcm_td2_vxlan_egr_xlate_entry_key_set(unit, vp, vpn, vxlan_port->criteria, &vxlate_entry, 1);
            rv = _bcm_td2_vxlan_sd_tag_set(unit, vxlan_port, &egr_nh_info, &vxlate_entry, &tpid_index);
        ...
        if (vxlan_port->flags & BCM_VXLAN_PORT_MULTICAST) {
            ...
            if ((mpath_flag) && (vxlan_port->flags & BCM_VXLAN_MULTIPATH)) {
                rv = _bcm_td2_vxlan_ingress_multipath_set(unit, vp_nh_ecmp_index, vxlan_port);
                _BCM_VXLAN_CLEANUP(rv);
            } else {
                rv = bcm_td2_vxlan_egr_nexthop_mapping_set(unit, vp_nh_ecmp_index, vxlan_port);
                _BCM_VXLAN_CLEANUP(rv);
            }
        }
    } else {
        ...
        if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
            ...
            if (vxlan_port->flags & BCM_VXLAN_PORT_MULTICAST)  {
                ...
                if (egr_nh_info.entry_type != _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) {
                    rv = BCM_E_PARAM;
                    goto cleanup;
                }
            }
...
_bcm_td2_vxlan_port_nh_delete(int unit, bcm_vpn_t vpn, int vp)
{
    ...
    if (ecmp) {
        ...
        BCM_IF_ERROR_RETURN(
             _bcm_td2_vxlan_ingress_multipath_reset(unit, nh_ecmp_index, ref_count, vp_type, vp, vpn));
    } else {
        ...
        BCM_IF_ERROR_RETURN(
                 bcm_td2_vxlan_nexthop_reset(unit, nh_ecmp_index, vp_type, vp, vpn));
    }
    return rv;
}
...
_bcm_td2_vxlan_port_nh_get(int unit, bcm_vpn_t vpn,  int vp, bcm_vxlan_port_t *vxlan_port)
{
    ...
    if (soc_ING_DVP_TABLEm_field32_get(unit, &dvp, VP_TYPEf) == 
        ...
        if (ecmp) {
            ...
            vxlan_port->flags |=  BCM_VXLAN_MULTIPATH;
        } else {
        ...
        rv = soc_mem_search(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ANY, &vxlate_index,
                    &vxlate_entry, &vxlate_entry, 0);
        if (BCM_SUCCESS(rv)) {
              (void)  _bcm_td2_vxlan_sd_tag_get( unit, vxlan_port, NULL,
                            &vxlate_entry, 1);
        }
        ...
        BCM_IF_ERROR_RETURN (soc_mem_read(unit, EGR_L3_NEXT_HOPm, MEM_BLOCK_ANY, 
         ...
         if (BCM_SUCCESS(rv)) {
              (void) _bcm_td2_vxlan_sd_tag_get( unit, vxlan_port, &egr_nh,
                         NULL, 0);
         }
...
/*
 * Function:
 *		_bcm_td2_vxlan_match_vnid_entry_update
 * Purpose:
 *		Update Match VXLAN VNID Entry 
 * Parameters:
 *  IN :  Unit
 *  IN :  vxlan_entry
 *  OUT :  vxlan_entry
...
STATIC int
_bcm_td2_vxlan_match_vnid_entry_update(int unit,
                                         mpls_entry_entry_t *ment, 
                                         mpls_entry_entry_t *return_ment)
{
    ...
    value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__SIPf);
    soc_MPLS_ENTRYm_field32_set(unit, return_ment, VXLAN_VN_ID__SIPf, value);
    value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__VFIf);
    soc_MPLS_ENTRYm_field32_set(unit, return_ment, VXLAN_VN_ID__VFIf, value);
    value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__VN_IDf);
    soc_MPLS_ENTRYm_field32_set(unit, return_ment, VXLAN_VN_ID__VN_IDf, value);
...
/*
 * Function:
 *		_bcm_td2_vxlan_match_vnid_entry_set
 * Purpose:
 *		Set VXLAN Match VNID Entry 
 * Parameters:
 *  IN :  Unit
 *  IN :  mpls_entry
 * Returns:
...
STATIC int
_bcm_td2_vxlan_match_vnid_entry_set(int unit, mpls_entry_entry_t *ment)
{
    mpls_entry_entry_t return_ment;
    int rv = BCM_E_UNAVAIL;
    int index=0;
    ...
    if (rv == SOC_E_NONE) {
         BCM_IF_ERROR_RETURN(
              _bcm_td2_vxlan_match_vnid_entry_update (unit, ment, &return_ment));
...
STATIC int
_bcm_td2_vxlan_match_vnid_entry_reset(int unit, mpls_entry_entry_t *ment)
{
    mpls_entry_entry_t return_ment;
    int rv = BCM_E_UNAVAIL;
    int index=0;
...
bcm_td2_vxlan_match_clear (int unit, int vp)
{
    ...
    vxlan_info->match_key[vp].match_tunnel_sip = 0;
...
/*
 * Function:
 *      _bcm_td2_vxlan_match_trunk_idx_get
 * Purpose:
 *      Obtain Trunk_Idx
 * Parameters:
 *      unit    - (IN) Device Number
...
STATIC int
_bcm_td2_vxlan_match_trunk_idx_get(int unit,
                                      bcm_module_t mod_id,
                                      bcm_port_t port_id,
                                      int *src_trk_idx)
{
 ...
 bcm_td2_vxlan_match_trunk_add(int unit, bcm_trunk_t tgid, int vp)
{
    ...
    for (port_idx = 0; port_idx < local_member_count; port_idx++) {
        BCM_IF_ERROR_RETURN(_bcm_td2_vxlan_match_trunk_idx_get(unit,
...
_bcm_td2_vxlan_match_add(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_VLAN ) {
    ...
    } else if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_TUNNEL_VN_ID) {
        ...
        sal_memset(&ment, 0, sizeof(mpls_entry_entry_t));
        vxlan_info->match_key[vp].flags = _BCM_VXLAN_PORT_MATCH_TYPE_TUNNEL_VNID;
        ...
        vxlan_info->match_key[vp].match_tunnel_sip = tunnel_sip;
    ...
    if  (vxlan_info->match_key[vp].flags &  _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        ...
        rv = _bcm_esw_stk_modmap_map(unit, BCM_STK_MODMAP_GET,
         ...
         BCM_GPORT_TRUNK_SET(vxlan_port->match_port, trunk_id);
                          ...
                          _BCM_VXLAN_PORT_MATCH_TYPE_TUNNEL_VNID) {
        vxlan_port->criteria = BCM_VXLAN_PORT_MATCH_TUNNEL_VN_ID;
      
        tunnel_sip = vxlan_info->match_key[vp].match_tunnel_sip;
        for (idx = 0; idx < _BCM_MAX_NUM_VXLAN_TUNNEL; idx++) {
           if (tunnel_sip == vxlan_info->vxlan_tunnel_term[idx].sip) {
                tunnel_idx = idx;
                break;
           }
        }
...
_bcm_td2_vxlan_sw_dump(int unit)
{
    ...
    for (i = 0; i < num_vp; i++) {
        ...
        soc_cm_print("Match tunnel sip = %x\n", 
                     vxlan_info->match_key[i].match_tunnel_sip);
    }
