/*
 * $Id: multicast.c 1.1 Broadcom SDK $
    ...
    if (_td2_multicast_l3_vp_info[unit]) {
        if (_td2_multicast_l3_vp_info[unit]->list_array) {
            for (i = 0; i < _td2_multicast_l3_vp_info[unit]->list_array_size;
                ...
                while (NULL != curr_ptr) {
                    BCM_IF_ERROR_RETURN(bcm_xgs3_nh_del(unit,
                            _BCM_L3_SHR_WRITE_DISABLE, curr_ptr->nh_index));
...
bcm_td2_multicast_l3_vp_next_hop_free(int unit, int intf)
{
    ...
    curr_ptr = _td2_multicast_l3_vp_info[unit]->list_array[intf];
    while (NULL != curr_ptr) {
        BCM_IF_ERROR_RETURN(bcm_xgs3_nh_del(unit, _BCM_L3_SHR_WRITE_DISABLE,
                    curr_ptr->nh_index));
        next_ptr = curr_ptr->next;
        sal_free(curr_ptr);
        curr_ptr = next_ptr;
...
bcm_td2_multicast_l3_vp_sw_dump(int unit)
{
    ...
    soc_cm_print("L3 Interface-on-Virtual Port Encapsulation ID Information:\n");
    ...
    for (i = 0; i < _td2_multicast_l3_vp_info[unit]->list_array_size; i++) {
        if (_td2_multicast_l3_vp_info[unit]->list_array[i] != NULL) {
            soc_cm_print("  L3 interface %4d: virtual_port:next_hop_index", i);
            curr_ptr = _td2_multicast_l3_vp_info[unit]->list_array[i];
            j = 0;
            while (curr_ptr != NULL) {
                ...
                soc_cm_print(" %5d:%-5d", curr_ptr->vp, curr_ptr->nh_index);
                j++;
                curr_ptr = curr_ptr->next;
            }
            soc_cm_print("\n");
