/*
 * $Id: oam.c 1.66.2.19 Broadcom SDK $
 * 
    ...
    int *num_lm, int *mdl_index)
{
    ...
    *created_endpoint_p = endpoint_p;
    ...
    if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
        ...
        BCM_IF_ERROR_RETURN
            (bcm_esw_trunk_get(unit, trunk_id, &t_info));

        /* Get the Modid and Port value using Trunk Index value. */
        module_id = t_info.tm[endpoint_info->trunk_index];
        port_id = t_info.tp[endpoint_info->trunk_index];
