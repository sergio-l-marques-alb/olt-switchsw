...
_bcm_tr3_wlan_reinit(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        rv = _bcm_vp_used_set(unit, vp, _bcmVpTypeWlan);
        if (BCM_FAILURE(rv)) {
            goto cleanup;
        }
        switch (entry_type) {
        ...
        _BCM_WLAN_TNL_USED_SET(unit, WLAN_TNL_INDEX_TO_ID(tnl_idx));


        TUNNEL_VLAN(unit, WLAN_TNL_INDEX_TO_ID(tnl_idx)) = 
            soc_AXP_WTX_DVP_PROFILEm_field32_get(unit, &axp_wtx_dvp, TUNNEL_VLANf);
        TUNNEL_PRI(unit, WLAN_TNL_INDEX_TO_ID(tnl_idx)) = 
            soc_AXP_WTX_DVP_PROFILEm_field32_get(unit, &axp_wtx_dvp, TUNNEL_PRIf);
        TUNNEL_CFI(unit, WLAN_TNL_INDEX_TO_ID(tnl_idx)) = 
        ...
        if (WLAN_INFO(unit)->port_info[vp].flags &
            ...
            for (j = BCM_VLAN_MIN; j < BCM_VLAN_MAX; j++) {
                ...
                soc_mem_field_get(unit, VLAN_TABm, (uint32*)vtab, VP_GROUP_BITMAPf, grp_vec);
...
bcm_tr3_wlan_init(int unit)
{
    ...
    for (i = 0; i < BCM_VLAN_COUNT; i++) {
        int bitmap_size = soc_mem_field_length(unit, VLAN_TABm, VP_GROUP_BITMAPf);
...
bcm_tr3_wlan_port_learn_get(int unit, bcm_gport_t wlan_port_id, uint32 *flags)
{
    ...
    lport_ptr = soc_SOURCE_VP_ATTRIBUTES_2m_field32_get(unit, &svp,
                                                LPORT_PROFILE_INDEXf);
    rv = READ_LPORT_TABm(unit, MEM_BLOCK_ANY, lport_ptr, &lport_profile);
    BCM_IF_ERROR_RETURN(rv);
...
{
    int vp, cml_old = 0, cml = 0, rv = BCM_E_NONE;
    ...
    old_lport_ptr = soc_SOURCE_VP_ATTRIBUTES_2m_field32_get(unit, &svp,
                                                    LPORT_PROFILE_INDEXf);
    rv = READ_LPORT_TABm(unit, MEM_BLOCK_ANY, old_lport_ptr, &lport_profile);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
...
bcm_tr3_wlan_port_add(int unit, bcm_wlan_port_t *wlan_port)
{
    ...
    void                            *entries[1];    /* For lport profile mem  */
    ...
    if (wlan_port->flags & BCM_WLAN_PORT_REPLACE) {
        ...
        soc_AXP_WTX_DVP_PROFILEm_field32_set(unit, &axp_wtx_dvp, 
                WLAN_TUNNEL_INDEXf, WLAN_TNL_ID_TO_INDEX(tunnel));
        /*
...
_bcm_tr3_wlan_port_delete(int unit, int vp)
{
    ...
    rv = READ_SOURCE_VP_ATTRIBUTES_2m(unit, MEM_BLOCK_ANY, vp, &svp);
    BCM_IF_ERROR_RETURN(rv);
    lport_ptr = soc_SOURCE_VP_ATTRIBUTES_2m_field32_get(unit, &svp, LPORT_PROFILE_INDEXf);
    rv = READ_LPORT_TABm(unit, MEM_BLOCK_ANY, lport_ptr, &lport_profile);
    BCM_IF_ERROR_RETURN(rv);
...
_bcm_tr3_wlan_port_get(int unit, int vp, bcm_wlan_port_t *wlan_port)
{
    ...
    wlan_port->encap_id = nh_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
    ...
    egress_tunnel =  WLAN_TNL_INDEX_TO_ID(
                        soc_AXP_WTX_DVP_PROFILEm_field32_get(unit, &axp_wtx_dvp, 
                                                         WLAN_TUNNEL_INDEXf));
...
_bcm_tr3_wlan_tunnel_init_add(int unit, bcm_tunnel_initiator_t *info, int *tnl_id_p)
{
    ...
    if (ipv6) {
        ...
        soc_mem_field32_set(unit, mem, &tnl_entry[TNL_ENTRY_0], DIP_LOWf, 
                            ip6_fields[0]);
        soc_mem_field32_set(unit, mem, &tnl_entry[TNL_ENTRY_2], DIP_HIf, 
                            ip6_fields[1]);
        SAL_IP6_ADDR_TO_UINT32(info->sip6, ip6_fields);
        soc_mem_field32_set(unit, mem, &tnl_entry[TNL_ENTRY_0], SIP_LOWf, 
                            ip6_fields[0]);
        soc_mem_field32_set(unit, mem, &tnl_entry[TNL_ENTRY_3], SIP_HIf, 
                            ip6_fields[1]);
    } else { /* ipv4 */
    ...
    if (info->flags & BCM_TUNNEL_INIT_WLAN_MTU && info->mtu > 0) {
        ...
        uint32 first = (((info->mtu + 7) / 8) > 240) ? 240 :
                        ((info->mtu + 7) / 8);
        ...
        if (info->flags & BCM_TUNNEL_INIT_WLAN_FRAG_ENABLE) {
    ...
    for (idx_cnt =0; idx_cnt < _BCM_WLAN_TNL_ENTRY_WIDTH; idx_cnt++) {
        /* Add the tunnel info in wlan tunnel table */
        rv = soc_mem_write(unit, mem, MEM_BLOCK_ALL, (*tnl_id_p + idx_cnt),
                           (void *)&tnl_entry[idx_cnt]);
        if (BCM_FAILURE(rv)) {
            goto tnl_cleanup;
        }
    }
                                   ...
                                   bcm_vlan_vector_t *vlan_vec)
{
    ...
    for (i = BCM_VLAN_MIN; i < BCM_VLAN_MAX; i++) {
        ...
        grp_vec[0] = grp_vec[1] = 0;
        soc_mem_field_get(unit, VLAN_TABm, (uint32*)vtab,
                                   ...
                                   bcm_vlan_vector_t vlan_vec)
{
    ...
    for (i = BCM_VLAN_MIN; i < BCM_VLAN_MAX; i++) {
        ...
        soc_VLAN_TABm_field_set(unit, vtab, VP_GROUP_BITMAPf, grp_vec);
    }
...
int
_bcm_tr3_lport_field_set(int unit, bcm_gport_t wlan_port_id, soc_field_t field,
                         int value)
{
    int vp, value_old = 0, rv = BCM_E_NONE;
    source_vp_attributes_2_entry_t svp;
    lport_tab_entry_t lport_profile;
    int lport_ptr, old_lport_ptr = -1;
    void *entries[1];
    ...
    old_lport_ptr = soc_SOURCE_VP_ATTRIBUTES_2m_field32_get(unit, &svp, 
                                                    LPORT_PROFILE_INDEXf);
    BCM_IF_ERROR_RETURN
        (READ_LPORT_TABm(unit, MEM_BLOCK_ANY, old_lport_ptr, &lport_profile));
...
int
_bcm_tr3_lport_field_get(int unit, bcm_gport_t wlan_port_id, 
                         soc_field_t field, int *value)
{
    ...
    lport_ptr = soc_SOURCE_VP_ATTRIBUTES_2m_field32_get(unit, &svp, LPORT_PROFILE_INDEXf);
    BCM_IF_ERROR_RETURN
        (READ_LPORT_TABm(unit, MEM_BLOCK_ANY, lport_ptr, &lport_profile));
                       ...
                       uint32 value)
{
    ...
    rv = _bcm_tr3_lport_field_set(unit, wlan_port_id, field, value);
...
bcm_tr3_wlan_port_untagged_vlan_set(int unit, bcm_gport_t port, bcm_vlan_t vid)
{
    ...
    rv = _bcm_tr3_lport_field_get(unit, port, TAG_ACTION_PROFILE_PTRf, 
                                  &old_profile);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    ...
    rv = _bcm_tr3_lport_field_get(unit, port, PORT_PRIf, &(action.priority));
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
    ...
    rv = _bcm_tr3_lport_field_set(unit, port, PORT_VIDf, vid);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
    /* Point to the new profile */
    rv = _bcm_tr3_lport_field_set(unit, port, TAG_ACTION_PROFILE_PTRf, profile);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
                                    ...
                                    bcm_vlan_t *vid_ptr)
{
    ...
    rv = _bcm_tr3_lport_field_get(unit, port, PORT_VIDf, &value);
...
bcm_tr3_wlan_port_untagged_prio_get(int unit, bcm_gport_t port, int *prio_ptr)
{
    ...
    rv = _bcm_tr3_lport_field_get(unit, port, PORT_PRIf, &value);
    *prio_ptr = value;
                                 ...
                                 int pri_index)
{
    ...
    int vp, wlan_dvp_index, egress_tunnel;
