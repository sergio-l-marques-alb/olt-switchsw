...
STATIC int _bcm_tr3_flex_ctr_pool_map(int unit, _bcm_tr3_ctr_mode_t mode)
{
    ...
    */
    for (dir = bcmStatFlexDirectionIngress; dir <=
        ...
        device->flex_pools[dir].pool_size = SOC_INFO(unit).size_flex_ingress_pool;
        device->flex_pools[dir].flags |= BCM_FLEX_INGRESS_POOL;
        device->flex_pools[dir].pool_id = dir;
        rv = _bcm_esw_stat_flex_pool_operation(unit, &device->flex_pools[dir]);

        if (rv != 0) {
            BCM_DEBUG(BCM_DBG_REGEX, ("map: _bcm_esw_stat_flex_pool_operation failed: dir = %d, id = %d, flags = 0x%04x, size = %d\n",
        ...
        {
            BCM_DEBUG(BCM_DBG_REGEX, ("map: _bcm_esw_stat_flex_pool_operation succeeded: dir = %d, id = %d, flags = 0x%04x, size = %d\n",
                         dir, device->flex_pools[dir].pool_id,
                         device->flex_pools[dir].flags,
                         device->flex_pools[dir].pool_size));
        }
        /*
        if (SOC_IS_HELIX4(unit)) {
            soc_reg64_field_set(unit, FT_CTR_POOLr, &regval, poolfH4[(dir * (poolf_size/2))], dir);
        }
        else {
            soc_reg64_field_set(unit, FT_CTR_POOLr, &regval, poolfTR3[(dir * (poolf_size/2))], dir);
        }
        */
    }
    ...
    */
    ...
    for (rgn = 0; rgn < poolf_size; rgn++) {
        ...
        if (SOC_IS_HELIX4(unit)) {
            soc_reg64_field_set(unit, FT_CTR_POOLr, &regval, poolfH4[rgn], fval);
        }
        else {
...
STATIC int _bcm_tr3_flex_ctr_pool_unmap(int unit)
{
    ...
    for (i = bcmStatFlexDirectionEgress; i >= bcmStatFlexDirectionIngress; i--) {
        ...
        if (rv != 0) {
            BCM_DEBUG(BCM_DBG_REGEX, ("unmap: _bcm_esw_stat_flex_pool_operation failed: i = %d, id = %d, flags = 0x%04x, size = %d\n",
        ...
        {
            BCM_DEBUG(BCM_DBG_REGEX, ("unmap: _bcm_esw_stat_flex_pool_operation succeeded: i = %d, id = %d, flags = 0x%04x, size = %d\n",
                         i, device->flex_pools[i].pool_id,
                         device->flex_pools[i].flags,
                         device->flex_pools[i].pool_size));
        }
...
_bcm_tr3_regex_dump_axp(int unit)
{
    ...
    soc_cm_print("Regex AXP status register values:\n");
    SOC_IF_ERROR_RETURN(READ_AXP_SM_SIGNATURE_MATCH_ACTIVE_STATUS1r(unit, &regval));
    soc_cm_print(" AXP_SM_SIGNATURE_MATCH_ACTIVE_STATUS1 = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_SIGNATURE_MATCH_ACTIVE_STATUS0r(unit, &regval));
    soc_cm_print(" AXP_SM_SIGNATURE_MATCH_ACTIVE_STATUS0 = 0x%08x\n\n", regval);
    soc_cm_print("Regex AXP counter register values:\n");
    SOC_IF_ERROR_RETURN(READ_AXP_SM_PACKETS_RECEIVED_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_PACKETS_RECEIVED_COUNTER       = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_PACKETS_SENT_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_PACKETS_SENT_COUNTER           = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_PACKETS_DROPPED_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_PACKETS_DROPPED_COUNTER        = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_BYTES_MATCHED_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_BYTES_MATCHED_COUNTER          = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_MATCHED_FLOWS_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_MATCHED_FLOWS_COUNTER          = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_UNMATCHED_FLOWS_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_UNMATCHED_FLOWS_COUNTER        = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_TOTAL_MATCH_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_TOTAL_MATCH_COUNTER            = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_TOTAL_CROSS_SIG_FLAGS_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_TOTAL_CROSS_SIG_FLAGS_COUNTER  = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_FRAGMENTS_RECEIVED_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_FRAGMENTS_RECEIVED_COUNTER     = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_IN_PACKET_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_IN_PACKET_ERROR_COUNTER        = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_FLOW_TRACKER_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_FLOW_TRACKER_ERROR_COUNTER     = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_PACKET_LENGTH_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_PACKET_LENGTH_ERROR_COUNTER    = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_L4_CHECKSUM_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_L4_CHECKSUM_ERROR_COUNTER      = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_FLOW_DONE_PACKET_DROP_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_FLOW_DONE_PACKET_DROP_COUNTER  = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_FLOW_TIMESTAMP_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_FLOW_TIMESTAMP_ERROR_COUNTER   = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_FLOW_PACKET_NUM_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_FLOW_PACKET_NUM_ERROR_COUNTER  = 0x%08x\n", regval);
    SOC_IF_ERROR_RETURN(READ_AXP_SM_ECC_ERROR_COUNTERr(unit, &regval));
    soc_cm_print(" AXP_SM_ECC_ERROR_COUNTER              = 0x%08x\n", regval);
...
_bcm_tr3_regex_config_set(int unit, bcm_regex_config_t *config)
{
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("Counter mode = %d, config flags = 0x%x\n", ctr_mode, config->flags));
    if (config->flags & (BCM_REGEX_COUNTER_ENABLE | 
        ...
        if ((ctr_mode != _CTR_MODE_NONE) &&
            ...
            */
        if (ctr_mode != _CTR_MODE_NONE) {
            BCM_DEBUG(BCM_DBG_REGEX, ("Setting counter mode to none and unmapping\n"));
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("Counter mode = %d, setting and mapping flex counters\n", ctr_mode));
        _bcm_tr3_flex_ctr_pool_map(unit, ctr_mode);
        _bcm_tr3_ctr_mode_set(unit, ctr_mode);
    } else {
        /* free up the pools */
        BCM_DEBUG(BCM_DBG_REGEX, ("No counters enabled, setting counter mode to none and unmapping\n"));
        _bcm_tr3_ctr_mode_set(unit, _CTR_MODE_NONE);
        _bcm_tr3_flex_ctr_pool_unmap(unit);
    }
                           ...
                           int num_dfa_state, void *user_data)
{
    ...
    if (cd->last_state != in_state) {
        /*
        BCM_DEBUG(BCM_DBG_REGEX, ("State %s %d:\n", (match_idx>=0)? "[FINAL]" : "", in_state));
        */
        /*
    ...
    if (to_state >= 0)
    {
        BCM_DEBUG(BCM_DBG_REGEX, ("\t %d -> %d [%d-%d];\n", in_state, to_state, from_c, to_c));
    }
    */
    cd->compile_size++;
                           ...
                           axp_sm_state_table_mem0_entry_t *pmem)
{
...
#if 0
    /*
     * soc_mem_read_range does not seem to work on Helix4.
     */
    ...
    for (i = pengine->from_line; i < pengine->from_line + pengine->req_lsz; i++) {
        ...
        if (rv < 0) {
            REGEX_ERR(("Read ERROR: table %s[%d]: %s\n",
                         ...
                         int *hw_states, void **ppdfa)
{
    ...
    if (rv < 0) {
        switch (rv) {
            case REGEX_ERROR_INVALID_CLASS:
                REGEX_WARN(("Regex: Invalid class.\n"));
                break;
            case REGEX_ERROR_EXPANSION_FAIL:
                REGEX_WARN(("Regex: Expansion failed.\n"));
                break;
            case REGEX_ERROR_NO_POST:
                REGEX_WARN(("Regex: No post.\n"));
                break;
            case REGEX_ERROR_NO_DFA:
                REGEX_WARN(("Regex: No DFA.\n"));
                break;
            case REGEX_ERROR_NO_NFA:
                REGEX_WARN(("Regex: No NFA.\n"));
                break;
            case REGEX_ERROR_NO_MEMORY:
                REGEX_WARN(("Regex: No memory.\n"));
                break;
            default:
                REGEX_WARN(("Regex: Error.\n"));
                break;
        }
        return BCM_E_FAIL;
    }
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("%s: line %d, DFA num hw_states/transitions = %d, %d lines\n",
                                  ...
                                  bcm_regex_engine_info_t *engine_info)
{
    ...
    if (engine_id == BCM_REGEX_ENGINE_ALL) {
        ...
        for (i = 0; i < BCM_TR3_NUM_ENGINE(device); i++) {
            ...
            if (ltmp == NULL) {
                BCM_DEBUG(BCM_DBG_REGEX, ("i = %d, logical engine NULL\n", i));
                continue;
            }
            ...
            if (ptmp == NULL) {
                BCM_DEBUG(BCM_DBG_REGEX, ("i = %d, Engine id %d : physical engine NULL\n", i, ltmp->id));
                continue;
            }
            in_use += ptmp->req_lsz;
        }
    ...
    else
    {
        if (engine_id < 0 || engine_id >= BCM_TR3_MAX_ENGINE) {
            BCM_DEBUG(BCM_DBG_REGEX, ("engine_id = %d, INVALID\n", engine_id));
            return BCM_E_PARAM;
        }

        if (_bcm_tr3_find_engine(unit, engine_id, &ltmp) ||
            ((ptmp = ltmp->physical_engine) == NULL)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("engine_id = %d, Engine logical id %d : physical engine NULL\n", engine_id, ltmp->id));
            ...
            */
            ...
            for (i = 0; i < BCM_TR3_NUM_ENGINE(device); i++) {
                ...
                ptmp = ltmp->physical_engine;
                if (ptmp == NULL) {
                    BCM_DEBUG(BCM_DBG_REGEX, ("i = %d, Engine id %d, pid NULL\n", i, ltmp->id));
                    continue;
                }
                tile = BCM_TR3_ENGINE_TILE(device, ptmp->hw_idx);
                tile_free_size[tile] -= ptmp->req_lsz;
            }
            ...
            soc_cm_print("Engine %d not allocated, max size in tile would be:\n", engine_id);
            return BCM_E_NONE;
        }
        ...
        for (i = 0; i < BCM_TR3_NUM_ENGINE(device); i++) {
            ...
            BCM_DEBUG(BCM_DBG_REGEX, ("i = %d, Engine id %d, pid %d : tile %d, size %d\n", i,
                          ...
                          int count)
{
    ...
    if ((config.flags & BCM_REGEX_ENGINE_CONFIG_MULTI_PACKET) && 
        ...
        BCM_DEBUG(BCM_DBG_REGEX,("Can't use multi-packet with chaining\n"));
        return BCM_E_CONFIG;
    }
    ...
    for(pengine=NULL, try=0; (try<2) && !pengine; try++) {
        ...
        if (rv && (try == 0)) {
             ...
             * fit things.
             */
            BCM_DEBUG(BCM_DBG_REGEX,("Alloc of regex engine failed.. rearrange engines\n"));
    ...
    if (match_set_ids) {
        ...
        if (rv) {
            BCM_DEBUG(BCM_DBG_REGEX,("Alloc of set_ids failed\n"));
            goto fail;
        }
    }
    ...
    if (match_check_ids) {
        ...
        if (rv) {
            BCM_DEBUG(BCM_DBG_REGEX,("Alloc of check_ids failed\n"));
            goto fail;
        }
    }
                         ...
                         bcm_regex_match_t *matches, int count)
{
    ...
    if (!(device = REGEX_INFO(unit))) {
        return BCM_E_INIT;
        BCM_DEBUG(BCM_DBG_REGEX,("unit %d, device not initialized\n", unit));
    }
    ...
    rv = _bcm_tr3_regex_hw_engine_detach(unit, pengine);
    if (rv) {
        BCM_DEBUG(BCM_DBG_REGEX,("can't detach hw_engine %d, rv %d\n", engid, rv));
        goto fail;
    }

    if (count <= 0) {
        BCM_REGEX_UNLOCK(unit);
        BCM_DEBUG(BCM_DBG_REGEX,("no patterns to install\n"));
        return BCM_E_NONE;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (sal_strlen(matches[i].pattern) >= sizeof(matches[i].pattern)) {
            BCM_DEBUG(BCM_DBG_REGEX,("regex %d too long\n", i));
            rv = BCM_E_PARAM;
            goto fail;
        }
        if (device->info->flags & BCM_REGEX_USE_POLICY_ID) {
            if (matches[i].action_flags & BCM_REGEX_MATCH_ACTION_NON_POLICY_ACTIONS) {
                BCM_DEBUG(BCM_DBG_REGEX,("non-policy action flags set with policy-ID flag\n"));
                rv = BCM_E_PARAM;
                goto fail;
            }
            ...
            */
            ...
            if (matches[i].policy_id != -1) {
                BCM_DEBUG(BCM_DBG_REGEX,("attempting to use policy_id with no policy-ID flag\n"));
                rv = BCM_E_PARAM;
                goto fail;
            }
        }
    ...
    if (req_lsz > _bcm_tr3_get_tile_line_size(unit)) {
        BCM_DEBUG(BCM_DBG_REGEX,("required lines %d too large to install\n", req_lsz));
        rv = BCM_E_RESOURCE;
        goto fail;
    }
    ...
    if (need_set_ids) {
        ...
        if (!cps_match_set_ids) {
            BCM_DEBUG(BCM_DBG_REGEX,("no memory for match set IDs\n"));
            rv = BCM_E_MEMORY;
            goto fail;
        }
        for (i = 0; i < count; i++) {
    ...
    if (need_check_ids) {
        ...
        if (!cps_match_check_ids) {
            BCM_DEBUG(BCM_DBG_REGEX,("no memory for match check IDs\n"));
            rv = BCM_E_MEMORY;
            goto fail;
        }
        for (i = 0; i < count; i++) {
    ...
    if (rv) {
        BCM_DEBUG(BCM_DBG_REGEX,("could not attach hw_engine, rv %d\n", rv));
        goto fail;
    }
    ...
    if (rv) {
        BCM_DEBUG(BCM_DBG_REGEX,("could not install DFA, rv %d\n", rv));
        goto fail;
    }
    ...
    */
    ...
    rv = READ_AXP_SM_REGEX_CONTROL0r(unit, 
                            lengine->physical_engine->hw_idx, &regval);
    if (SOC_FAILURE(rv)) {
        BCM_DEBUG(BCM_DBG_REGEX,("could not REGEX_CONTROL0 register, rv %d\n", rv));
        goto fail;
    }
    fval = 0;
                          ...
                          bcm_regex_session_t *session)
{
    ...
    if (session != NULL) {
        ...
        */
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("ticks = %d (0x%x), age_profile = 0x%x\n",
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("flow_state: 0x%x\n", flow_state));
    }
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("ip1 = 0x%08x\n", ip1));
    BCM_DEBUG(BCM_DBG_REGEX, ("ip2 = 0x%08x\n", ip2));
    BCM_DEBUG(BCM_DBG_REGEX, ("port1 = 0x%04x\n", port1));
    BCM_DEBUG(BCM_DBG_REGEX, ("port2 = 0x%04x\n", port2));
    BCM_DEBUG(BCM_DBG_REGEX, ("protocol = 0x%02x\n", protocol));
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("direction = 0x%x\n", direction));
    soc_FT_SESSIONm_field32_set(unit, hw_entry, KEY_DIRECTIONf, direction);
    p8 = (uint8*) key_field;
    BCM_DEBUG(BCM_DBG_REGEX, ("key_field = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
                               ...
                               bcm_regex_session_t *session)
{
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("set mem %s field %s\n",
                            ...
                            ft_session_entry_t *ft_hw_entry)
{
    ...
    */
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("age_profile = 0x%x\n", age_profile));
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("ticks = 0x%x, timeout = %d\n", ticks, session->inactivity_timeout));
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("flow_state = 0x%08x\n", flow_state));

    BCM_DEBUG(BCM_DBG_REGEX, ("session->flags = 0x%x\n", session->flags));
    ...
    if (key->flags & BCM_REGEX_SESSION_KEY_IPV6) {
        ...
        if (BCM_FAILURE(rv) && (rv != BCM_E_NOT_FOUND)) {
            ...
            if ((key->flags & BCM_REGEX_SESSION_KEY_REPLACE) != 0) {
                BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_insert ipv6 rv: %d, can't replace non-existent entry\n", rv));
                goto end;
            }
            BCM_DEBUG(BCM_DBG_REGEX, ("session_add ipv6\n"));
            ...
            if ((key->flags & BCM_REGEX_SESSION_KEY_REPLACE) == 0) {
                ...
                BCM_DEBUG(BCM_DBG_REGEX, ("session_modify ipv6\n"));
                rv = soc_mem_insert(unit, mem, MEM_BLOCK_ALL, (void *)&session_entry_lookup6);
                BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_insert ipv6 rv: %d, OK for modify\n", rv));
                if (rv == BCM_E_EXISTS) {
        ...
        if (BCM_FAILURE(rv) && (rv != BCM_E_NOT_FOUND)) {
            ...
            if ((key->flags & BCM_REGEX_SESSION_KEY_REPLACE) != 0) {
                BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_insert ipv4 rv: %d, can't replace non-existent entry\n", rv));
                goto end;
            }
            BCM_DEBUG(BCM_DBG_REGEX, ("session_add ipv4\n"));
            ...
            if ((key->flags & BCM_REGEX_SESSION_KEY_REPLACE) == 0) {
                ...
                BCM_DEBUG(BCM_DBG_REGEX, ("session_modify ipv4\n"));
                rv = soc_mem_insert(unit, mem, MEM_BLOCK_ALL, (void *)&session_entry_lookup);
                BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_insert ipv4 rv: %d, OK for modify\n", rv));
                if (rv == BCM_E_EXISTS) {
                    rv = BCM_E_NONE; /* OK to get BCM_E_EXISTS in update case, insert still happens */
                }
            }
                                         ...
                                         bcm_regex_policy_t policy)
{
    ...
    if (flags & BCM_REGEX_SESSION_KEY_IPV6) {
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_read ipv6 rv: %d, can't update non-existent entry\n", rv));
            goto end;
        }
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_write ipv6 rv: %d, can't write entry\n", rv));
            goto end;
        }
    } else {
        sal_memset(&session_entry, 0, sizeof(ft_session_entry_t));
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_read ipv4 rv: %d, can't update non-existent entry\n", rv));
            goto end;
        }
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_write ipv4 rv: %d, can't write entry\n", rv));
            goto end;
        }
    }
                                        ...
                                        bcm_regex_session_t *session)
{
    ...
    if (flags & _REGEX_ENTRY_TYPE_IS_IPV6) {
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_read ipv6 rv: %d, can't get non-existent entry\n", rv));
            goto end;
        } else if (rv == BCM_E_NONE) {
            return _bcm_tr3_ft_session_ipv6_from_hw(unit, flags, session, key,
                                                    &session_entry6);
        ...
        if (BCM_FAILURE(rv)) {
            BCM_DEBUG(BCM_DBG_REGEX, ("soc_mem_read ipv4 rv: %d, can't get non-existent entry\n", rv));
            goto end;
        } else if (rv == BCM_E_NONE) {
            return _bcm_tr3_ft_session_from_hw(unit, flags, session, key,
                                                    &session_entry);
                                    ...
                                    void *user_data)
{
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("index_min4: %d, index_max4: %d, index_min6: %d, index_max6: %d\n",
    ...
    for (i = index_min4; i <= index_max4; i++) {
        ...
        */
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("entry: %d, key_type = %d\n", i, type));
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("flow_state = 0x%08x, static_bit = 0x%x\n", flow_state, static_bit));
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("rv = %d (0x%08x)\n", rv, rv));
                                        ...
                                        uint32 granularity, soc_mem_t meter_table)
{
    ...
    meter_hw_idx = (f_pl->pool_index * stage_fc->meter_pool[0]->pool_size) + (2 * f_pl->hw_index) + meter_offset;
...
STATIC int _regex_tr3_policy_set_l3_info(int unit, soc_mem_t mem, int value, uint32 *buf)
{
    ...
    if (flags & BCM_L3_MULTIPATH) {
        REGEX_VVERB(("FT(unit %d) vverb: Install mpath L3 policy (Ecmp_group: %d)))",
                     unit, nh_ecmp_id));
        RegexPolicySet(unit, mem, buf, ECMPf, 1);
        RegexPolicySet(unit, mem, buf, ECMP_PTRf, nh_ecmp_id);
    } else {
        REGEX_VVERB(("FT(unit %d) vverb: Install unipath L3 policy(Next hop id: %d)))",
                     unit, nh_ecmp_id));
                                                  ...
                                                  int value, uint32 *buf)
{
    ...
    if (flags & BCM_L3_MULTIPATH) {
        ...
        REGEX_VVERB(("FT(unit %d) vverb: Set ECMP (Group id: %d\n)))",
                     unit, nh_ecmp_id));
    } else {
        /* Regular next hop */
        redirect_to_nhi = (0x1 << 14) | nh_ecmp_id;
        REGEX_VVERB(("FT(unit %d) vverb: Set unipath (Nexthop index: %d\n)))",
                     unit, nh_ecmp_id));
    }
                                        ...
                                        _regex_policy_action_t *pa, uint32 *buf)
{
    ...
    REGEX_VVERB(("FT(unit %d) vverb: BEGIN %s(eid=%d)\n",
    ...
    REGEX_VVERB(("FT(unit %d) vverb: END %s()\n", unit, FUNCTION_NAME()));
    return (BCM_E_NONE);
        }
...
void _bcm_regex_dump_dfa(int unit, bcm_regex_engine_t engid)
{
    ...
    if (NULL == state_storage) {
        soc_cm_print("Unable to allocate memory storage.\n");
        return;
    }
    ...
    jump_table = sal_alloc(sizeof(jump_table[0]) * jump_table_size,
                     "re_jump_table");
    if (NULL == jump_table) {
        soc_cm_print("Unable to allocate jump table storage.\n");
        return;
    }
        ...
        for (j = 0; j < BCM_TR3_NUM_RV_PER_LINE; j++) {
            ...
            if ((match_index < 0) && (jump_offset != 0xffff)) {
                if ((jump_offset - base) >= jump_table_size) {
                    soc_cm_print("Jump offset %d (0x%x) out of range.\n", jump_offset, jump_offset);
    ...
    soc_cm_print("\n------------------DFA-(Tile:%d)---------------\n\n", pengine->tile);
    soc_cm_print("Number of lines = %d\nNumber of states = %d\n", pengine->req_lsz, num_states);
    this_state = 0;
    print_id = 0;
    num_final = 0;
    for (i = 0; i < pengine->req_lsz; i++) {
...
#else
                soc_cm_print("\nState %d %s @line=%d, line offset=%d full offset (0x%x)\n",
                             this_state, (match_index >= 0) ? "[FINAL]" : "",
                             pengine->from_line + i, j, (pengine->from_line + i) * BCM_TR3_NUM_RV_PER_LINE + j);
#endif
                if (match_index >= 0) {
                    num_final++;
                    soc_cm_print("\tMatch ID = %d, %s\n", match_index, ((tc == 2) ? "IDLE STOP" : ((tc == 1) ? "IDLE CONT" : "")));
                }
                print_id = 0;
            }
            if (jump_offset != 0xffff) {
                soc_cm_print("\t %4d -> %4d : ", this_state, jump_table[jump_offset - base]);
                if ((tc == 0) && (fc == 255)) {
                    soc_cm_print("[\\%-3d-\\%-3d], ", fc, tc);
                    soc_cm_print("Same data offset;\n");
                } else {
                    soc_cm_print("[\\%-3d-\\%-3d]", fc, tc);
                    if (isprint(fc) && isprint(tc)) {
                        soc_cm_print(" (%2c-%-2c)", fc, tc);
                    } else if (isprint(fc)) {
                        soc_cm_print(" (%2c-'')", fc);
                    } else if (isprint(tc)) {
                        soc_cm_print(" (''-%-2c)", tc);
                ...
                soc_cm_print("\t %4d -> 0xffff (IDLE) : ", this_state);
                soc_cm_print("[\\%-3d-\\%-3d]", fc, tc);
                if ((fc == 255) && ((tc == 1) || (tc == 2))) {
                    soc_cm_print(" Match Id %d", match_index);
                }
                soc_cm_print(";\n");
            }
        }
                                ...
                                uint32 flags)
{
    ...
    /* dump engine info */
    soc_cm_print("Regex Tile %d, Engine %d : Start offset:%04d Size (lines):%04d/%04d\n",
                 pengine->tile, pengine->hw_idx, pengine->from_line, pengine->req_lsz, pengine->total_lsz);
    soc_cm_print("          CPS   :%s\n", 
