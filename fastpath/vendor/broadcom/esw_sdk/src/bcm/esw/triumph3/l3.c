/*
 * $Id: l3.c 1.112.2.1 Broadcom SDK $
...
#define _TR3_ESM_HOST_TBL_FREE_ENTRY(_u_, _tbl_) \
        (*_bcm_tr3_esm_host_tbl_freeidx_ptr[(_tbl_)][(_u_)])
_bcm_tr3_l3_defip_pair128_table_t *l3_tr3_defip_pair128[BCM_MAX_NUM_UNITS];
 ...
 */
STATIC int
_tr3_defip_pair128_hash(int unit, _bcm_defip_cfg_t *lpm_cfg, uint8 *hash) 
{
    uint32 key[BCM_TR3_DEFIP_PAIR128_HASH_SZ];
    ...
    *hash = (_shr_crc16b(0, (void *)key,
                         WORDS2BITS((BCM_TR3_DEFIP_PAIR128_HASH_SZ))) & 0xff);
    return (BCM_E_NONE);
}
 ...
 */
STATIC int 
_tr3_defip_pair128_reinit(int unit, int idx, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    BCM_IF_ERROR_RETURN(_tr3_defip_pair128_hash(unit, lpm_cfg, &hash));

    /* Set software structure info. */
    BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, idx, lkup_plen, hash);

    /* Update software usage counter. */
    BCM_TR3_DEFIP_PAIR128_USED_COUNT(unit)++;
 ...
 */
STATIC int 
_bcm_tr3_defip_pair128_deinit(int unit)
{
    if (BCM_TR3_DEFIP_PAIR128(unit) == NULL) {
        return (BCM_E_NONE);
    }

    /* De-allocate any existing structures. */
    if (BCM_TR3_DEFIP_PAIR128_ARR(unit) != NULL) {
        sal_free(BCM_TR3_DEFIP_PAIR128_ARR(unit));
        BCM_TR3_DEFIP_PAIR128_ARR(unit) = NULL;
    }
    sal_free(BCM_TR3_DEFIP_PAIR128(unit));
    BCM_TR3_DEFIP_PAIR128(unit) = NULL;
...
_bcm_tr3_defip_pair128_init(int unit)
{
    ...
    /* De-allocate any existing structures. */
    if (BCM_TR3_DEFIP_PAIR128(unit) != NULL) {
        rv = _bcm_tr3_defip_pair128_deinit(unit);
        BCM_IF_ERROR_RETURN(rv);
    }

    /* Allocate cam control structure. */
    mem_sz = sizeof(_bcm_tr3_l3_defip_pair128_table_t);
    BCM_TR3_DEFIP_PAIR128(unit) = sal_alloc(mem_sz, "tr3_l3_defip_pair128");
    if (BCM_TR3_DEFIP_PAIR128(unit) == NULL) {
        return (BCM_E_MEMORY);
    }

    /* Reset cam control structure. */
    sal_memset(BCM_TR3_DEFIP_PAIR128(unit), 0, mem_sz);
    ...
    BCM_TR3_DEFIP_PAIR128_TOTAL(unit) = ipv6_128_depth;
    BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit) = 0;
    BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit) = ipv6_128_depth - 1;
    mem_sz = ipv6_128_depth * sizeof(_bcm_tr3_l3_defip_pair128_entry_t);
    BCM_TR3_DEFIP_PAIR128_ARR(unit) = 
                            sal_alloc(mem_sz, "tr3_l3_defip_pair128_entry_array");
    if (BCM_TR3_DEFIP_PAIR128_ARR(unit) == NULL) {
        BCM_IF_ERROR_RETURN(_bcm_tr3_defip_pair128_deinit(unit));
        return (BCM_E_MEMORY);
    }
    sal_memset(BCM_TR3_DEFIP_PAIR128_ARR(unit), 0, mem_sz);
...
/*
 * Function:    
 *     _tr3_defip_pair128_ip6_mask_set
 * Purpose:  
 ...
 */
STATIC void
_tr3_defip_pair128_ip6_mask_set(int unit, soc_mem_t mem, uint32 *entry, 
...
/*
 * Function:    
 *     _tr3_defip_pair128_ip6_mask_get
 * Purpose:  
 ...
 */
STATIC void
_tr3_defip_pair128_ip6_mask_get(int unit, soc_mem_t mem, const void *entry, 
                                ip6_addr_t ip6)
{
    uint32              ip6_field[4];
...
/*
 * Function:    
 *     _tr3_defip_pair128_ip6_addr_set
 * Purpose:  
 ...
 */
STATIC void
_tr3_defip_pair128_ip6_addr_set(int unit, soc_mem_t mem, uint32 *entry, 
                                const ip6_addr_t ip6)
{
    uint32              ip6_field[4];
    ip6_field[3] = ((ip6[12] << 24)| (ip6[13] << 16) |
...
/*
 * Function:    
 *     _tr3_defip_pair128_ip6_addr_get
 * Purpose:  
 ...
 */
STATIC void
_tr3_defip_pair128_ip6_addr_get(int unit, soc_mem_t mem, const void *entry, 
                                ip6_addr_t ip6)
{
    uint32              ip6_field[4];
 ...
 */
STATIC int 
_tr3_defip_pair128_get_key(int unit, uint32 *hw_entry, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    /* Extract ip6 address. */
    _tr3_defip_pair128_ip6_addr_get(unit,mem, hw_entry, lpm_cfg->defip_ip6_addr);

    /* Extract subnet mask. */
    _tr3_defip_pair128_ip6_mask_get(unit, mem, hw_entry, mask);
 ...
 */
STATIC int 
_tr3_defip_pair128_match(int unit, _bcm_defip_cfg_t *lpm_cfg, 
                         uint32 *hw_entry, int *hw_index)
{
    ...
    for (index = 0; index <= BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit); index++) {

        /* Route prefix length comparison. */
        if (lkup_plen != BCM_TR3_DEFIP_PAIR128_ARR(unit)[index].prefix_len) {
            continue;
        }

        /* Route lookup key hash comparison. */
        if (hash != BCM_TR3_DEFIP_PAIR128_ARR(unit)[index].entry_hash) {
            continue;
        }
        ...
        rv =  _tr3_defip_pair128_get_key(unit, hw_entry, &candidate);
        if (BCM_FAILURE(rv)) {
            break;
        }
    ...
    if (index > BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit))  {
        return (BCM_E_NOT_FOUND);
    }
...
/*
 * Function:
 *      _tr3_defip_pair128_entry_clear
 * Purpose:
 ...
 */
STATIC int 
_tr3_defip_pair128_entry_clear(int unit, int hw_index)  
{
    ...
    /* Reset sw entry. */
    BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, hw_index, 0, 0);

    /* Reset urpf source lookup entry. */
    if (SOC_URPF_STATUS_GET(unit)) {
        BCM_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                            (hw_index + BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit)),
                            &soc_mem_entry_null(unit, L3_DEFIP_PAIR_128m)));
        BCM_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                 (hw_index+ BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit)), hw_entry));
    }
    return (BCM_E_NONE);
}
 ...
 */
STATIC INLINE int 
_tr3_defip_pair128_parse(int unit, uint32 *hw_entry,
 ...
 */
STATIC int 
_tr3_defip_pair128_shift(int unit, int idx_src, int idx_dest)  
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS]; /* HW entry buffer. */

    /* Don't copy uninstalled entries. */
    if (0 != BCM_TR3_DEFIP_PAIR128_ARR(unit)[idx_src].prefix_len) {
        ...
        if (SOC_URPF_STATUS_GET(unit)) {
            ...
            BCM_IF_ERROR_RETURN
                (BCM_XGS3_MEM_READ(unit, L3_DEFIP_PAIR_128m, 
                              idx_src + BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit),
                              hw_entry));

            /* Write entry to the destination index + half tcam. */
            BCM_IF_ERROR_RETURN
                (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                            idx_dest +  BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit),
                            hw_entry));
        }
    }

    /* Copy software portion of the entry. */ 
    sal_memcpy (&BCM_TR3_DEFIP_PAIR128_ARR(unit)[idx_dest],
                &BCM_TR3_DEFIP_PAIR128_ARR(unit)[idx_src], 
                sizeof(_bcm_tr3_l3_defip_pair128_entry_t));
...
/*
 * Function:
 *      _tr3_defip_pair128_shift_range
 * Purpose:
 *      Shift entries in the tcam. 
 * Parameters:
 *      unit        - (IN)SOC unit number.
 ...
 */
STATIC int 
_tr3_defip_pair128_shift_range(int unit, int idx_start, int idx_end)  
{
    int idx;     /* Iteration index. */
    ...
    if (idx_end > idx_start) {
        for (idx = idx_end - 1; idx >= idx_start; idx--) {
            rv = _tr3_defip_pair128_shift(unit, idx, idx + 1);  
            if (BCM_FAILURE(rv)) { 
                break;
            }
        }
        /* Delete last entry from hardware & software. */
        rv = _tr3_defip_pair128_entry_clear(unit, idx_start);
    }
 ...
 */
STATIC int 
_tr3_defip_pair128_idx_alloc(int unit, _bcm_defip_cfg_t *lpm_cfg, int *hw_index,
                             int nh_ecmp_idx)
{
    int  lkup_plen;                   /* Vrf weighted lookup prefix.    */
    uint8 hash;                       /* Entry hash value.              */
    ...
    int  range_end = BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit);
    ...
    if (BCM_TR3_DEFIP_PAIR128_USED_COUNT(unit) ==
                      BCM_TR3_DEFIP_PAIR128_TOTAL(unit)) {
        return (BCM_E_FULL);
    }
     ...
     */ 
    for (idx = 0; idx <= BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit); idx++) {
        /* Remember free entry before installation location. */
        if (0 == BCM_TR3_DEFIP_PAIR128_ARR(unit)[idx].prefix_len) {
            free_idx_before = idx;
            continue;
        }
        /* Route prefix length comparison. */
        if (lkup_plen < BCM_TR3_DEFIP_PAIR128_ARR(unit)[idx].prefix_len) {
            range_start = idx;
            continue;
        }
        break;
     ...
     */ 
    for (idx = BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit); idx >= 0; idx--) {
    ...
    if (range_start == BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit)) {
        /* Shift all entries from free_idx_before till the end 1 up. */   
        rv = _tr3_defip_pair128_shift_range(unit, BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit), free_idx_before);

        if (BCM_SUCCESS(rv)) { 
            /* Allocated index is last TCAM entry. */
            *hw_index = BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit);
            BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, *hw_index, lkup_plen, hash);
        }
        return (rv);
    }
    ...
    if (range_end < 0) {
        ...
        rv = _tr3_defip_pair128_shift_range(unit, 0, free_idx_after);

        if (BCM_SUCCESS(rv)) {
            /* Allocated index is first TCAM entry. */
            *hw_index = 0;
            BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, 0, lkup_plen, hash);
        }
        return (rv);
    }
    ...
    if (shift_min <= 0) {
        rv = _tr3_defip_pair128_shift_range(unit, range_start, free_idx_before);
        if (BCM_SUCCESS(rv)) {
            *hw_index= range_start;
            BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, range_start, lkup_plen, hash);
        }
    } else {
        rv = _tr3_defip_pair128_shift_range(unit, range_end, free_idx_after);
        if (BCM_SUCCESS(rv)) {
            *hw_index = range_end;
            BCM_TR3_DEFIP_PAIR128_ENTRY_SET(unit, range_end, lkup_plen, hash);
        }
    }
    return (rv);
}
 ...
 */
STATIC int 
_bcm_tr3_l3_defip_pair128_get(int unit, _bcm_defip_cfg_t *lpm_cfg, 
                              int *nh_ecmp_idx) 
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS];/* Hw entry buffer.          */
    bcm_ip6_t mask;                          /* Subnet mask.              */
    ...
    rv =  _tr3_defip_pair128_match(unit, lpm_cfg, hw_entry, &hw_index);
    BCM_IF_ERROR_RETURN(rv);

    lpm_cfg->defip_index = hw_index;

    /* Parse matched entry. */
    rv = _tr3_defip_pair128_parse(unit, hw_entry, lpm_cfg, nh_ecmp_idx);
    BCM_IF_ERROR_RETURN(rv);
 ...
 */
STATIC int
_bcm_tr3_l3_defip_pair128_add(int unit, _bcm_defip_cfg_t *lpm_cfg,
                              int nh_ecmp_idx) 
{
    ...
    if (BCM_XGS3_L3_INVALID_INDEX == lpm_cfg->defip_index) {
        rv = _tr3_defip_pair128_idx_alloc(unit, lpm_cfg, &hw_index, nh_ecmp_idx);
        BCM_IF_ERROR_RETURN(rv);
    } else {
    ...
    _tr3_defip_pair128_ip6_addr_set(unit, mem, hw_entry, lpm_cfg->defip_ip6_addr);

    /* Set ip mask. */
    _tr3_defip_pair128_ip6_mask_set(unit, mem, hw_entry, mask);
    ...
    if (SOC_URPF_STATUS_GET(unit)) {
        ...
        rv = BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                                ...
                                hw_entry);
        if (BCM_FAILURE(rv)) {
            _tr3_defip_pair128_entry_clear(unit, hw_index);
            return (rv);
        }
    }
 ...
 */
STATIC int 
_bcm_tr3_l3_defip_pair128_del(int unit, _bcm_defip_cfg_t *lpm_cfg) 
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS];/* Hw entry buffer.          */
    bcm_ip6_t mask;                          /* Subnet mask.              */
    int hw_index;                            /* Entry offset in the TCAM. */ 
    ...
    rv = _tr3_defip_pair128_entry_clear(unit, hw_index);
    if (BCM_SUCCESS(rv)) {
...
/*
 * Function:
 *      _bcm_tr3_l3_defip_pair128_update_match
 * Purpose:
 ...
 */
STATIC int
_bcm_tr3_l3_defip_pair128_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    int idx;                                 /* Iteration index.           */
    int rv;                                  /* Operation return status.   */
    ...
    for (idx = 0; idx < defip_pair128_tbl_size; idx++) {
        ...
        /* Parse  the entry. */
        _tr3_defip_pair128_parse(unit, hw_entry, &lpm_cfg, &nh_ecmp_idx);
        lpm_cfg.defip_index = idx;

        /* Fill entry ip address &  subnet mask. */
        _tr3_defip_pair128_get_key(unit, hw_entry, &lpm_cfg);
        ...
        if (SOC_WARM_BOOT(unit)) {
            rv = _tr3_defip_pair128_reinit(unit, idx, &lpm_cfg);
            if (BCM_FAILURE(rv)) {
                soc_cm_sfree(unit, lpm_tbl_ptr);
                return rv;
            }
                      ...
                      bcm_l3_host_traverse_cb cb, void *user_data)
{
    ...
    for(tbl = 0; tbl < num_tables; tbl++) {
        ...
        for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
            ...
            memset(l3_tbl_buf, 0, (table_ent_size * chunk_size));
...
_bcm_tr3_l3_defip_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l3_expanded_defip_table)) {/* HX4 */
        return _bcm_hx4_l3_defip_init(unit);
    }
...
_bcm_tr3_l3_lpm_get(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    switch (mem) {
      ...
      case L3_DEFIP_PAIR_128m:
        return _bcm_tr3_l3_defip_pair128_get(unit, lpm_cfg, nh_ecmp_idx);
      default:
...
_bcm_tr3_l3_lpm_add(int unit, _bcm_defip_cfg_t *lpm_cfg, int nh_ecmp_idx)
{
    ...
    switch (mem) {
      ...
      case L3_DEFIP_PAIR_128m:
        return _bcm_tr3_l3_defip_pair128_add(unit, lpm_cfg, nh_ecmp_idx);
      default:
...
_bcm_tr3_l3_lpm_del(int unit, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    switch (mem) {
      ...
      case L3_DEFIP_PAIR_128m:
        return _bcm_tr3_l3_defip_pair128_del(unit, lpm_cfg);
      default: 
...
_bcm_tr3_l3_lpm_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    switch (mem) {
      ...
      case L3_DEFIP_PAIR_128m:
        return _bcm_tr3_l3_defip_pair128_update_match(unit, trv_data);
      default: 
...
_bcm_hx4_l3_defip_urpf_enable(int unit, int enable)
{
    ...
    SOC_LPM_STATE_START(unit, (MAX_PFX_INDEX)) = - 1;
    SOC_LPM_STATE_END(unit, (MAX_PFX_INDEX))   = - 1;
    BCM_TR3_DEFIP_PAIR128_URPF_OFFSET(unit)    = ipv6_128_dip_offset;
    BCM_TR3_DEFIP_PAIR128_IDX_MAX(unit)        = ipv6_128_depth - 1;
    BCM_TR3_DEFIP_PAIR128_TOTAL(unit)          = ipv6_128_depth;
    BCM_IF_ERROR_RETURN(WRITE_L3_DEFIP_KEY_SEL_0r(unit, defip_key_sel_0));
    BCM_IF_ERROR_RETURN(WRITE_L3_DEFIP_KEY_SEL_1r(unit, defip_key_sel_1));
...
_bcm_tr3_l3_ecmp_grp_add(int unit, int ecmp_grp, void *buf, int max_paths)
{
    ...
    if (BCM_XGS3_L3_ENT_REF_CNT(BCM_XGS3_L3_TBL_PTR(unit,
        ...
        BCM_XGS3_L3_ENT_REF_CNT_INC(data.tbl_ptr, data.entry_index, max_paths);
    }
...
_bcm_tr3_nh_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
        ...
        /* Reset allocated buffer. */
        memset(egr_tbl_buf, 0, (table_ent_size * chunk_size));
