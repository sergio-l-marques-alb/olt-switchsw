/* $Id: field.c 1.105.8.1 Broadcom SDK $
...
#define NUM_METERS_PER_POOL_IFP  512
...
_field_tr3_lookup_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_EXT_ADD(unit, stage_fc, bcmFieldQualifySrcMplsGport,
                     ...
                     _bcmFieldSliceSrcEntitySelect, _bcmFieldFwdEntityMplsGport,
                     0,
                     f1_ofs + 36, 16, /* S_FIELD */
                     f1_ofs + 55, 1,  /* SVP_VALID */
                     0, 0,
                     0
                     );
...
_field_tr3_external_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_EXT_ADD(unit, stage_fc, bcmFieldQualifyInPort,
                     _bcmFieldDevSelDisable, 0,
                     _bcmFieldSliceSelExternal, _FP_EXT_ACL_144_L2,
                     _bcmFieldSliceSelDisable, 0,
                     0,
                     8, 7,      /* port number of SGLP */
                     23, 1,     /* trunk/port indicator of SGLP */
                     0, 0,
                     0
                     );
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyDstMac,
    ...
    if (soc_property_get(unit, spn_EXT_TCAM_MODE, 0) == 0) {
        ...
        _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyRangeCheck,
                     _bcmFieldSliceSelExternal, _FP_EXT_ACL_L2, 40, 16);
        _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifySrcMac,
                     _bcmFieldSliceSelExternal, _FP_EXT_ACL_L2, 176, 48);
        _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyEtherType,
                     _bcmFieldSliceSelExternal, _FP_EXT_ACL_L2, 224, 16);
    }
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlan,
                 _bcmFieldSliceSelExternal, _FP_EXT_ACL_IPV6_SHORT, 256, 16);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlanId,
                 _bcmFieldSliceSelExternal, _FP_EXT_ACL_IPV6_SHORT, 256, 12);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlanCfi,
                 _bcmFieldSliceSelExternal, _FP_EXT_ACL_IPV6_SHORT, 268, 1);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOuterVlanPri,
                 _bcmFieldSliceSelExternal, _FP_EXT_ACL_IPV6_SHORT, 269, 3);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifySrcIp6,
                 _bcmFieldSliceSelExternal, _FP_EXT_ACL_IPV6_SHORT, 272, 128);
                                      ...
                                      )
{
    ...
    case bcmFieldActionGpPortPrioIntCosQNew:
        *result = (soc_feature(unit, soc_feature_field_ingress_cosq_override)
...
{
    _field_slice_t * const fs = f_ent->fs;
    _field_stage_t     *stage_fc;
    _field_cntr_pool_t *p = NULL;
    unsigned           n, hw_idx, i;
    int                pool_idx, free_pool_idx;
    ...
    for (pool_idx = 0; n != 0; --n, ++pool_idx) {
        ...
        if (p->slice_id == -1) {
            free_pool_idx = pool_idx;
            continue;
        }

        if (p->slice_id == fs->slice_number) {
            break;
        }
    }
    ...
    if (n == 0) {
        ...
        (p = stage_fc->cntr_pool[pool_idx = free_pool_idx])->slice_id
            = fs->slice_number;
    }
                               ...
                               )
{
    ...
    case _BCM_FIELD_STAGE_INGRESS:
        ...
        *pairs_per_pool = NUM_METERS_PER_POOL_IFP;

        if (soc_feature(unit, soc_feature_field_stage_half_slice)) {
            *pairs_per_pool = NUM_METERS_PER_POOL_IFP / 2;
        }
        if (SOC_IS_HELIX4(unit)) {
...
{
    /* Initialization for EXTFP is handled by separate code */

    return (stage_fc->stage_id == _BCM_FIELD_STAGE_EXTERNAL
            ? BCM_E_NONE
            : _bcm_field_tr_hw_clear(unit, stage_fc)
            );
}
                             ...
                             )
{
    ...
    if (((stage_id != _BCM_FIELD_STAGE_EGRESS)
         && _FP_METER_BMP_TEST(f_mp->meter_bmp, pool_index))
        ...
        if (stage_id == _BCM_FIELD_STAGE_EGRESS) {
            ...
            _FP_METER_BMP_ADD(f_mp->meter_bmp, pool_index);
            f_mp->free_meters--;
        }
                                    ...
                                    )
{
    ...
    uint8 *buf = fc->scache_ptr;          /* Secondary storage base ptr.    */
    ...
    _field_slice_t *fs;                   /* Pointer to Slice info.         */
    _field_group_t *fg;                   /* Pointer to group info.         */
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        for (idx = 0; idx < slice_ent_cnt; idx++) {
            ...
            sid = pid = -1;
            if (fc->l2warm) {
                rv = _field_tr2_entry_info_retrieve
                        (unit,
                         &eid,
                         &prio,
                         fc,
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        /* If Level 2, retrieve the GIDs in this slice */
        if (fc->l2warm) {
            rv = _field_tr2_scache_slice_group_recover
                    (unit,
                     fc,
                     slice_idx,
                     NULL,
