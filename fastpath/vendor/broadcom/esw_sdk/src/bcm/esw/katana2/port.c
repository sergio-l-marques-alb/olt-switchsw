/*
 * $Id: port.c 1.15.2.2 Broadcom SDK $
 * 
...
_bcm_kt2_port_cfg_init(int unit, bcm_port_t port, bcm_vlan_data_t *vlan_data)
{
    ...
    uint32 field_values[SOC_MAX_MEM_FIELD_WORDS], port_type = 0;
    int field_count = 0;
    int key_type_value;
    int dual_modid = 0;

    dual_modid = dual_modid; /* To Avoid unused warning message with GCC 4.6.x*/
    if (SOC_MEM_FIELD_VALID(unit, EGR_PHYSICAL_PORTm, PORT_TYPEf)) {
        if (IS_ST_PORT(unit, port)) {
            port_type = KT2_PORT_TYPE_HIGIG;
        } else if (IS_LB_PORT(unit, port)) {
            port_type = KT2_PORT_TYPE_LOOPBACK;
        } else if (IS_LP_PORT(unit, port)) { /* LinkPHY port */
        ...
        BCM_IF_ERROR_RETURN(soc_mem_field32_modify(
                   unit, ING_PHYSICAL_PORT_TABLEm, port,PORT_TYPEf,port_type));
        BCM_IF_ERROR_RETURN(soc_mem_field32_modify(
                       unit, EGR_PHYSICAL_PORTm, port,PORT_TYPEf,port_type));
    ...
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0_VALIDf,1));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0f,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0_BASE_PP_PORTf,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0_VALIDf,1));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0f,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0_BASE_PP_PORTf,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_3m, 0, MODID_0_VALIDf,1));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_3m, 0, MODID_0f,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                PP_PORT_GPP_TRANSLATION_3m, 0, MODID_0_BASE_PP_PORTf,0));

    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0_VALIDf,1));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0f,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_1m, 0, MODID_0_BASE_PP_PORTf,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0_VALIDf,1));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0f,0));
    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit,
                EGR_PP_PORT_GPP_TRANSLATION_2m, 0, MODID_0_BASE_PP_PORTf,0));
    ...
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
         ...
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), mxqblock_port)) {
             ...
             rv = bcm_esw_port_speed_set(unit, mxqblock_port ,
                                 mxqspeeds[mxqblock][new_lanes-1]);
...
int _bcm_kt2_port_lanes_set(int unit, bcm_port_t port, int lanes)
{
    ...
    switch(mxqspeeds[mxqblock][new_lanes-1]) {
    ...
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
         ...
         si->port_speed_max[mxqblock_port]=mxqspeeds[mxqblock][new_lanes-1];
    ...
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
         ...
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), mxqblock_port)) {
             ...
             if (si->port_speed_max[mxqblock_port] <= 1000) {
                 ...
                 egr_fifo_depth = 16;
             } else if (si->port_speed_max[mxqblock_port] < 20000) {
                 egr_fifo_depth = 53;
             } else {
                 egr_fifo_depth = 90;
             }
    ...
    if ((mxqblock >= 8) && (mxqblock <= 9)) {
        WRITE_TXLP_PORT_CREDIT_RESETr(unit, mxqblock_port,0xf);
        for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
             ...
             BCM_IF_ERROR_RETURN(WRITE_TXLP_PORT_ADDR_MAP_TABLEm(
                                 unit,MEM_BLOCK_ALL,mxqblock_port,
    ...
    if ((mxqblock >= 8) && (mxqblock <= 9)) {
        WRITE_TXLP_PORT_CREDIT_RESETr(unit, mxqblock_port,0);
    }
    /* 2 */
    BCM_IF_ERROR_RETURN(READ_EPC_LINK_BMAPm(
                        unit, MEM_BLOCK_ANY, 0, &epc_link_bmap_entry));
    ...
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
         ...
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), mxqblock_port)) {
             ...
             if ((si->port_speed_max[mxqblock_port]) >= 13000) {
                  ...
                  soc_reg_field_set(unit, XPORT_CONFIGr, &rval, HIGIG2_MODEf,1);
             } else {
    ...
    SOC_IF_ERROR_RETURN(WRITE_XPORT_XMAC_CONTROLr(unit, mxqblock_port, 0));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XPORT_MIB_RESETr(unit,mxqblock_port, 0xf));
    SOC_IF_ERROR_RETURN(WRITE_XPORT_MIB_RESETr(unit,mxqblock_port, 0x0));
    ...
    SOC_IF_ERROR_RETURN(WRITE_XPORT_ECC_CONTROLr(unit,mxqblock_port, 0xf));

    SOC_IF_ERROR_RETURN(READ_XPORT_PORT_ENABLEr(unit,mxqblock_port, &rval));
    for (loop=0;loop<KT2_MAX_MXQPORTS_PER_BLOCK;loop++) {
         mxqblock_port=kt2_mxqblock_ports[mxqblock][loop];
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), mxqblock_port)) {
             soc_reg_field_set(unit, XPORT_PORT_ENABLEr, &rval, 
             port_enable_field[kt2_port_to_mxqblock_subports[mxqblock_port]],1);
         }
    }
    SOC_IF_ERROR_RETURN(WRITE_XPORT_PORT_ENABLEr(unit,mxqblock_port, rval));
    ...
    SOC_IF_ERROR_RETURN(READ_XPORT_XGXS_CTRLr(unit,mxqblock_port,&rval));
    soc_reg_field_set(unit,XPORT_XGXS_CTRLr,&rval,PWRDWNf,0);
    SOC_IF_ERROR_RETURN(WRITE_XPORT_XGXS_CTRLr(unit,mxqblock_port,rval));
    ...
    SOC_IF_ERROR_RETURN(READ_XPORT_XGXS_CTRLr(unit,mxqblock_port,&rval));
    soc_reg_field_set(unit,XPORT_XGXS_CTRLr,&rval,REFSELf,1);
    SOC_IF_ERROR_RETURN(WRITE_XPORT_XGXS_CTRLr(unit,mxqblock_port,rval));
         ...
         if (SOC_PBMP_MEMBER((PBMP_ALL(unit)), mxqblock_port)) {
             if (mxqblock <= 7 ) {
                 ...
                 port_enable = soc_reg_field_get(unit, TXLP_PORT_ENABLEr,
                                     rval, PORT_ENABLEf);
                 port_enable |= (1<< 
                                 kt2_port_to_mxqblock_subports[mxqblock_port]);
