/*
 * $Id: subport.c 1.28.2.10 Broadcom SDK $
...
/* Index resolution handle to manage PP_PORT indices for subport port addition*/
static shr_idxres_list_handle_t 
         _bcm_subport_port_pp_port_idxres_list[BCM_UNITS_MAX] = { 0 };

/* Check if subport group is created on trunk or port*/
#define _BCM_KT2_SUBPORT_GROUP_IS_TRUNK(_subport_group)    \
        (((_subport_group >> _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_SHIFT) & \
            _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_MASK))
...
#define _BCM_KT2_SUBPORT_GROUP_SPGID_GET(_subport_group) \
        (((_subport_group) >> _BCM_KT2_SUBPORT_GROUP_SPGID_SHIFT) & \
            _BCM_KT2_SUBPORT_GROUP_SPGID_MASK)

/* Get subport_group id associated with subport_port gport */
#define _BCM_KT2_SUBPORT_PORT_SPGID_GET(_subport_port) \
    (((_subport_port) >> _BCM_KT2_SUBPORT_PORT_SPGID_SHIFT) & \
        _BCM_KT2_SUBPORT_PORT_SPGID_MASK)

/* Get subport_port id associated with subport_port gport */
#define _BCM_KT2_SUBPORT_PORT_SPPID_GET(_subport_port) \
    (((_subport_port) >> _BCM_KT2_SUBPORT_PORT_SPPID_SHIFT) & \
        _BCM_KT2_SUBPORT_PORT_SPPID_MASK)
...
#define _BCM_KT2_LINKPHY_SUBPORT_GROUP_SET(_subport_group, \
                                           _is_trunk, _port, _gid) \
    ((_subport_group) = ((_BCM_KT2_SUBPORT_TYPE_LINKPHY & \
                          _BCM_KT2_SUBPORT_GROUP_TYPE_MASK) \
                              << _BCM_KT2_SUBPORT_GROUP_TYPE_SHIFT) | \
                         ((_is_trunk & _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_MASK) \
                              << _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_SHIFT) | \
...
/* Set the LinkPHY subport port gport */
#define _BCM_KT2_LINKPHY_SUBPORT_PORT_SET(_subport_port, _spg_id, _spp_id)   \
    ((_subport_port) = ((_BCM_KT2_SUBPORT_TYPE_LINKPHY & \
                          _BCM_KT2_SUBPORT_PORT_TYPE_MASK) \
                              << _BCM_KT2_SUBPORT_PORT_TYPE_SHIFT) | \
                        ((_spg_id & _BCM_KT2_SUBPORT_PORT_SPGID_MASK) \
                              << _BCM_KT2_SUBPORT_PORT_SPGID_SHIFT) | \
                        ((_spp_id & _BCM_KT2_SUBPORT_PORT_SPPID_MASK) \
                               << _BCM_KT2_SUBPORT_PORT_SPPID_SHIFT))
...
/* Per LinkPHY subport group, subport port array */
_bcm_linkphy_subport_port_info_t
           *(_bcm_linkphy_subport_port_info[BCM_MAX_NUM_UNITS]) = { 0 };


#define _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_SET(unit, sp_group_id, sp_port_id, \
    sp_group, sp_num_streams, pp_port, is_valid) \
{ \
    _bcm_linkphy_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].group = sp_group; \
     _bcm_linkphy_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].num_streams = sp_num_streams; \
    _bcm_linkphy_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].pp_port_idx = pp_port; \
    _bcm_linkphy_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].valid = is_valid; \
}
...
static uint32 *linkphy_frag_byte_ctr
               [BCM_UNITS_MAX][_BCM_KT2_LINKPHY_DIRECTION_MAX]
               [_BCM_KT2_LINKPHY_BLOCK_MAX] = {{{NULL}}};
static uint32 *linkphy_frag_ctr
               [BCM_UNITS_MAX][_BCM_KT2_LINKPHY_DIRECTION_MAX]
               [_BCM_KT2_LINKPHY_BLOCK_MAX] = {{{NULL}}};
static uint32 *linkphy_frame_byte_ctr
               [BCM_UNITS_MAX][_BCM_KT2_LINKPHY_DIRECTION_MAX]
               [_BCM_KT2_LINKPHY_BLOCK_MAX] = {{{NULL}}};
static uint32 *linkphy_frame_ctr
               [BCM_UNITS_MAX][_BCM_KT2_LINKPHY_DIRECTION_MAX]
               [_BCM_KT2_LINKPHY_BLOCK_MAX] = {{{NULL}}};
...
#define _BCM_KT2_SUBTAG_SUBPORT_GROUP_SET(_subport_group, \
                                          _is_trunk, _port, _gid) \
    ((_subport_group) = ((_BCM_KT2_SUBPORT_TYPE_SUBTAG & \
                          _BCM_KT2_SUBPORT_GROUP_TYPE_MASK) \
                              << _BCM_KT2_SUBPORT_GROUP_TYPE_SHIFT) | \
                         ((_is_trunk & _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_MASK) \
                              << _BCM_KT2_SUBPORT_GROUP_IS_TRUNK_SHIFT) | \
...
/* Set the SubTag subport port gport */
#define _BCM_KT2_SUBTAG_SUBPORT_PORT_SET(_subport_port, _spg_id, _spp_id)   \
    ((_subport_port) = ((_BCM_KT2_SUBPORT_TYPE_SUBTAG & \
                          _BCM_KT2_SUBPORT_PORT_TYPE_MASK) \
                              << _BCM_KT2_SUBPORT_PORT_TYPE_SHIFT) | \
                        ((_spg_id & _BCM_KT2_SUBPORT_PORT_SPGID_MASK) \
                              << _BCM_KT2_SUBPORT_PORT_SPGID_SHIFT) | \
...
/* Per SubTag subport group, subport port array */
_bcm_subtag_subport_port_info_t
           *(_bcm_subtag_subport_port_info[BCM_MAX_NUM_UNITS]) = { 0 };


#define _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_SET(unit, sp_group_id, sp_port_id, \
    sp_group, pp_port, vlan_id, tcam_hw_id, is_valid) \
{ \
    ...
    _bcm_subtag_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].vlan = vlan_id; \
    _bcm_subtag_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
        + sp_port_id].subtag_tcam_hw_idx = tcam_hw_id; \
    _bcm_subtag_subport_port_info[unit] \
        [sp_group_id * _BCM_KT2_SUBPORT_PORT_MAX \
            + sp_port_id].valid = is_valid; \
}
...
/*
 * Function:
 *      _bcm_linkphy_port_subport_cleanup
 * Purpose:
 *      Clean all subport groups and subport ports (streams)
...
int _bcm_linkphy_port_subport_cleanup(int unit, bcm_port_t port)
{
    int group_id = 0, rv = BCM_E_NONE;
    int linkphy_group_count = 0, linkphy_subport_count = 0;
    int group_traversed_count = 0, linkphy_port = 0;
    ...
    if (!_BCM_KT2_PORT_SUPPORTS_LINKPHY(unit, port)) {
        SUBPORT_ERR(
            ("ERROR: Invalid port %d for LinkPHY subport cleanup\n", port));
        return BCM_E_PORT;
    }

    linkphy_group_count = _bcm_linkphy_subport_group_count[unit];

    for (group_id = 0; group_id < _BCM_KT2_SUBPORT_GROUP_MAX; group_id++) {

        if (linkphy_group_count == group_traversed_count) {
            break;
        }
        ...
        subport_group = 
            _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_GROUP_GET(unit, group_id, 0);

        linkphy_port = _BCM_KT2_SUBPORT_GROUP_PORT_GET(subport_group);

        if (linkphy_port != port) {
            continue;
        }

        rv = _bcm_linkphy_subport_group_destroy(unit, subport_group);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: subport cleanup failed to delete"
                " LinkPHY subport group 0x%x.\n group_id=%d, port=%d\n",
                subport_group, group_id, port));
            return rv;
        }

        group_traversed_count++;
    }
    ...
    for (direction = 0; direction < _BCM_KT2_LINKPHY_DIRECTION_MAX; direction++) {
        for (block_id = 0; block_id < _BCM_KT2_LINKPHY_BLOCK_MAX; block_id++) {
            for (pool_id = 0; pool_id < _BCM_KT2_LINKPHY_POOL_MAX; pool_id++) {
                ...
                for (index = 0;
                index <= (_BCM_KT2_LINKPHY_COUNTER_MAX - 1); index++) {
                    ...
                    switch (pool_id) {
                        case 0:
                            if (linkphy_frag_byte_ctr[unit][direction]
                                [block_id][index] != count) {
                                SUBPORT_VERB(
                                    ("%s_DEBUG_COUNTER8.lp%d index:%d "
                                    "frag byte : old count:%d  new_count:%d\n",
                                    direction ? "TXLP" : "RXLP",
                                    block_id, index,
                                    linkphy_frag_byte_ctr[unit][direction]
                                    [block_id][index],
                                    count));
                                linkphy_frag_byte_ctr[unit][direction]
                                    [block_id][index] = count;
                            ...
                            break;
                        case 1:
                            if(linkphy_frag_ctr[unit][direction]
                                [block_id][index] != count) {
                                SUBPORT_VERB(
                                    ("%s_DEBUG_COUNTER9.lp%d index:%d "
                                    "fragment : old count:%d  new_count:%d\n",
                                    direction ? "TXLP" : "RXLP",
                                    block_id, index,
                            ...
                            break;
                        case 2:
                            if(linkphy_frame_byte_ctr[unit][direction]
                                [block_id][index] != count) {
                                SUBPORT_VERB(
                                    ("%s_DEBUG_COUNTER10.lp%d index:%d "
                                    "frame byte : old count:%d  new_count:%d\n",
                                    direction ? "TXLP" : "RXLP",
                                    block_id, index,
                                    linkphy_frame_byte_ctr[unit][direction]
                                    [block_id][index],
                                    count));
                                linkphy_frame_byte_ctr[unit][direction]
                                    [block_id][index] = count;
                            ...
                            break;
                        case 3:
                            if(linkphy_frame_ctr[unit][direction]
                                [block_id][index] != count) {
                                SUBPORT_VERB(
                                    ("%s_DEBUG_COUNTER11.lp%d index:%d "
                                    "frame : old count:%d  new_count:%d\n",
                                    direction ? "TXLP" : "RXLP",
                                    block_id, index,
                                    linkphy_frame_ctr[unit][direction]
                                    [block_id][index],
                                    count));
                                linkphy_frame_ctr[unit][direction]
                                    [block_id][index] = count;
...
{
    int direction = 0, block_id = 0, pool_id = 0, copyno = 0;
    ...
    block_alloc_size = sizeof(uint32) * _BCM_KT2_LINKPHY_COUNTER_MAX;
    ...
    uint64 val)
{
    ...
    for (i =0; i < config.num_streams; i++) {
        ...
        if ((stat_type == bcmSubportLinkPhyStatRxFragmentBytes) ||
            ...
            linkphy_frag_byte_ctr[unit][direction]
                               [block][counter_id] = COMPILER_64_LO(val);
    ...
    uint64 *val)
{
    ...
    for (i =0; i < config.num_streams; i++) {
        ...
        if ((stat_type == bcmSubportLinkPhyStatRxFragmentBytes) ||
            (stat_type ==  bcmSubportLinkPhyStatTxFragmentBytes)) {
            COMPILER_64_ADD_32(*val, linkphy_frag_byte_ctr[unit][direction]
                                                         [block][counter_id]);
        } else if ((stat_type == bcmSubportLinkPhyStatRxFragments) ||
                   (stat_type == bcmSubportLinkPhyStatTxFragments)) {
            COMPILER_64_ADD_32(*val, linkphy_frag_ctr[unit][direction]
                                                    [block][counter_id]);
        } else if ((stat_type == bcmSubportLinkPhyStatRxFrameBytes) ||
                  (stat_type == bcmSubportLinkPhyStatTxFrameBytes)) {
            COMPILER_64_ADD_32(*val, linkphy_frame_byte_ctr[unit][direction]
                                                          [block][counter_id]);
        } else {
            /* if ((stat_type == bcmSubportLinkPhyStatRxFrames) ||
                  (stat_type == bcmSubportLinkPhyStatTxFrames)) { */
            COMPILER_64_ADD_32(*val, linkphy_frame_ctr[unit][direction]
                                                     [block][counter_id]);
        }
    }
    BCM_LINKPHY_COUNTER_UNLOCK(unit);
...
_bcm_linkphy_subport_init(int unit)
{
   ...
   */
    ...
    /* LinkPHY subport port info per subport group per subport port */
    if (_bcm_linkphy_subport_port_info[unit] == NULL) {
        _bcm_linkphy_subport_port_info[unit] =
            sal_alloc((_BCM_KT2_SUBPORT_GROUP_MAX *
    ...
    sal_memset(_bcm_linkphy_subport_port_info[unit], 0,
        (_BCM_KT2_SUBPORT_GROUP_MAX * _BCM_KT2_SUBPORT_PORT_MAX *
         sizeof(_bcm_linkphy_subport_port_info_t)));
...
{
    int linkphy_subport_group = -1;

    _BCM_KT2_SUBPORT_CHECK_INIT(unit);

    _BCM_KT2_LINKPHY_SUBPORT_GROUP_SET(linkphy_subport_group,
        is_trunk, port, group_id);
    BCM_GPORT_SUBPORT_GROUP_SET(*group, linkphy_subport_group);
...
_bcm_linkphy_subport_group_destroy(int unit,
                                   bcm_gport_t group)
{
    int subport_group = 0, group_id = -1, i = 0, rv = BCM_E_NONE;
    int linkphy_subport_count = 0;
    bcm_gport_t subport_port_gport;

    _BCM_KT2_SUBPORT_CHECK_INIT(unit);

    subport_group = BCM_GPORT_SUBPORT_GROUP_GET(group);

    if (_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_GROUP(subport_group)) {

        group_id = _BCM_KT2_SUBPORT_GROUP_SPGID_GET(subport_group);
        ...
        /*
        * If the given group was created after deleting some other old
        * group then the group_id of both will be same.
        * We need to verify that the subport ports under this group_id 
        * indeed belongs to this group*/
        if (linkphy_subport_count) {
            /* read the first subport_port and get the group gport */
            if (group != _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_GROUP_GET(unit,
                group_id, 0)) {
                SUBPORT_ERR(
                    ("ERROR: LinkPHY subport group 0x%x not found.\n", group));
                return BCM_E_NOT_FOUND;
        ...
        for (i = 0; i < linkphy_subport_count; i++) {

            _BCM_KT2_LINKPHY_SUBPORT_PORT_SET(subport_port_gport, group_id, i);
...
{
    int subport_group = 0, group_id = -1;
    ...
    if (_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_GROUP(subport_group)) {
        ...
        config->port = _BCM_KT2_SUBPORT_GROUP_PORT_GET(subport_group);
    } else {
        return BCM_E_PARAM;
    }
...
_bcm_linkphy_subport_group_traverse(int unit,
                                    ...
                                    void *user_data)
{
    int rv = BCM_E_NONE, subport_group = -1, group_id = 0;
    int port = 0, spp_idx = 0, subport_port;
    int linkphy_subport_count = 0;
    bcm_subport_config_t config;
    ...
    if (_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_GROUP(subport_group)) {
        group_id =
            _BCM_KT2_SUBPORT_GROUP_SPGID_GET(subport_group);
        ...
        /*
        * If the given group was created after deleting some other old
        * group then the group_id of both group gports will be same.
        * We need to verify that the subport ports under this group_id 
        * indeed belongs to the given group.*/
        if (linkphy_subport_count) {
            /* read the first subport_port and get the group gport */
            if (group != _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_GROUP_GET(unit,
                group_id, 0)) {
        ...
        for (spp_idx = 0; spp_idx < linkphy_subport_count; spp_idx++) {

            _BCM_KT2_LINKPHY_SUBPORT_PORT_SET(subport_port, group_id, spp_idx);

            rv = _bcm_linkphy_subport_port_get(unit, subport_port, &config);
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(
                    ("ERROR: LinkPHY subport group 0x%x traverse failed \n"
                    "to get subport port (group id %d, subport_port id %d)\n",
                    group, group_id, spp_idx));
                return rv;
            }

            rv = cb(unit, port, &config, user_data);
                              ...
                              bcm_gport_t *gport)
{
    ...
    int subport_group = -1, linkphy_subport_group = -1;
    int sp_group_idx = -1, sp_port_idx = 0, i = 0, is_trunk = 0;
    int linkphy_subport_port = -1, start_addr = 0, end_addr = 0;
    uint16 int_stream_id_base = 0, ext_stream_id_base = 0;
    uint16 dev_int_stream_id = 0, int_stream_id = 0;
    uint32 pp_port_index = 0, free_count = 0;
    soc_mem_t mem;
    soc_info_t *si = &SOC_INFO(unit);
    ...
    *******************************/
    ...
    subport_group = BCM_GPORT_SUBPORT_GROUP_GET(config->group);
    ...
    linkphy_subport_group = subport_group;
    sp_group_idx =
        _BCM_KT2_SUBPORT_GROUP_SPGID_GET(linkphy_subport_group);
    if (!_BCM_KT2_LINKPHY_SUBPORT_GROUP_USED_GET(unit, sp_group_idx)) {
        SUBPORT_ERR(("ERROR: subport group 0x%x does not exist. group id =%d\n",
            subport_group, sp_group_idx));
        return BCM_E_NOT_FOUND;
    }

    port = _BCM_KT2_SUBPORT_GROUP_PORT_GET(linkphy_subport_group);
    ...
    is_trunk = _BCM_KT2_SUBPORT_GROUP_IS_TRUNK(config->group);

    /* B. PP_PORT INDEX RESERVATION
    *************************************/
    /* Reserve free PP_PORT index */
    shr_idxres_list_state(_bcm_subport_port_pp_port_idxres_list[unit],
                           NULL,NULL,NULL,NULL, &free_count,NULL);
    if (free_count == 0) {
        SUBPORT_ERR(("ERROR: No more PP_PORTs available\n"));
        return BCM_E_RESOURCE;
    }

    rv = shr_idxres_list_alloc(
        _bcm_subport_port_pp_port_idxres_list[unit], &pp_port_index);
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: PP_PORT allocation failed.\n"));
        return rv;
    }
    ...
    *********************************/
    for (i = 0; i < config->num_streams; i++) {
        ...
        * reserve 3 cells for each stream by default (3 * 128 = 384).
        * Remainig cells can be used when stream speed is
        * set to 1G or more as per following scheme.
        * Stream-Speed(Gbps) |  Min Num.Cells
        *    < 1G            |     3
        *      1G            |     6
        *      10G           |     50 
        ...
        * so 12 lines of buffer are needed.
        */
        start_addr = 12 * (pp_port_index - _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN);
        end_addr = start_addr + 11;
        ...
        sp_port_idx =
            *(_bcm_subport_group_subport_port_count[unit] + sp_group_idx);

        _bcm_linkphy_subport_port_info[unit]
            [sp_group_idx * _BCM_KT2_SUBPORT_PORT_MAX +
            sp_port_idx].ext_stream_idx[i] = config->stream_id_array[i];
        _bcm_linkphy_subport_port_info[unit]
            [sp_group_idx * _BCM_KT2_SUBPORT_PORT_MAX +
            sp_port_idx].int_stream_idx[i] = int_stream_id;
    }
    ...
    */
    ...
    _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_SET(unit, sp_group_idx, sp_port_idx,
        config->group, config->num_streams, pp_port_index, 1);

    _BCM_KT2_LINKPHY_SUBPORT_PORT_SET(linkphy_subport_port,
                                      sp_group_idx, sp_port_idx);

    BCM_GPORT_SUBPORT_PORT_SET(*gport, linkphy_subport_port);
...
_bcm_linkphy_subport_port_delete(int unit,
                                 bcm_gport_t subport_port_gport)
{
    int rv = BCM_E_NONE, group_id = 0, subport_port_id = 0;
    int port = 0, subport_group = -1, disable = 0, poll_active = 0;
    int dev_int_stream_id = 0, num_streams = 0, i = 0;
    uint16 int_stream_id_base = 0;
    uint16 int_stream_id = 0, pp_port_index = 0, ext_stream_id = 0;
    uint32 timeoutval = 0, regval = 0;
    soc_timeout_t timeout;
    soc_reg_t reg;
    soc_mem_t mem;

    if (!_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_PORT(subport_port_gport)) {
        return BCM_E_PARAM;
    }

    group_id = _BCM_KT2_SUBPORT_PORT_SPGID_GET(subport_port_gport);

    subport_port_id =
        _BCM_KT2_SUBPORT_PORT_SPPID_GET(subport_port_gport);

    if (!_BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_VALID_GET(unit,
                  group_id, subport_port_id)) {
    ...
    pp_port_index = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_PP_PORT_ID_GET(unit,
                        group_id, subport_port_id);

    if ((pp_port_index < _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN) ||
        (pp_port_index > _BCM_KT2_SUBPORT_PP_PORT_INDEX_MAX)) {
        SUBPORT_ERR(("ERROR: LinkPHY subport port gport 0x%x is not found.\n"
            "pp_port index = %d\n", subport_port_gport, pp_port_index));
        return BCM_E_NOT_FOUND;
    }
    ...
    */
    ...
    for (i = 0; i < num_streams; i++) {
        ...
        int_stream_id = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_INT_STREAM_ID_GET(unit,
                            group_id, subport_port_id, i);
        ...
        ext_stream_id = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_EXT_STREAM_ID_GET(unit,
                            group_id, subport_port_id, i);
        ...
        if (SOC_REG_PORT_VALID(unit, reg, port)) {
            ...
            timeoutval = _BCM_STREAM_ID_FLUSH_TIMEOUT;
                ...
                while (poll_active) {
                    ...
                    if (soc_timeout_check(&timeout)) {
                        SUBPORT_ERR(("ERROR: RX stream id %d flush timeout :"
                            ...
                            "external stream_id %d, pp_port %d\n",
                            int_stream_id, subport_port_gport,
                            port, ext_stream_id, pp_port_index));
        ...
        _bcm_linkphy_subport_port_info[unit]
            [group_id * _BCM_KT2_SUBPORT_PORT_MAX +
            subport_port_id].ext_stream_idx[i] = 0;
        _bcm_linkphy_subport_port_info[unit]
            [group_id * _BCM_KT2_SUBPORT_PORT_MAX +
            subport_port_id].int_stream_idx[i] = 0;
    }
    ...
    *(_bcm_subport_group_subport_port_count[unit] + group_id) -= 1;
    ...
    _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_SET(unit, group_id, subport_port_id,
                                           0, 0, 0, 0);
...
_bcm_linkphy_subport_port_get(int unit,
                              ...
                              bcm_subport_config_t *config)
{
    int i = 0, group_idx = -1, subport_port_id = -1, port = 0, pp_port = 0;
    int rv = BCM_E_NONE;
    bcm_gport_t subport_group;
    ...
    if (_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_PORT(subport_port_gport)) {
        group_idx =
            _BCM_KT2_SUBPORT_PORT_SPGID_GET(subport_port_gport);
        subport_port_id =
            _BCM_KT2_SUBPORT_PORT_SPPID_GET(subport_port_gport);

        if (!_BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_VALID_GET(unit,
                      group_idx, subport_port_id)) {
            SUBPORT_ERR(("ERROR: LinkPHY subport port gport 0x%x is invalid\n",
                subport_port_gport));
            return BCM_E_NOT_FOUND;
        }

        pp_port = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_PP_PORT_ID_GET(unit,
                      group_idx, subport_port_id);
        if (pp_port < _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN) {
            SUBPORT_ERR(("ERROR: LinkPHY subport port gport 0x%x is not found\n"
                "pp_port index = %d \n", subport_port_gport, pp_port));
            return BCM_E_NOT_FOUND;
        }
        ...
        config->num_streams =
            _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_NUM_STREAMS_GET(unit,
                group_idx, subport_port_id);

        for (i = 0; i < config->num_streams; i++) {
            int_stream_id =
                _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_INT_STREAM_ID_GET(
                    unit, group_idx, subport_port_id, i);
            dev_int_stream_id = int_stream_id + int_stream_id_base;
            if (dev_int_stream_id > soc_mem_index_max(unit,
                                        DEVICE_STREAM_ID_TO_PP_PORT_MAPm)) {
                SUBPORT_ERR(
...
_bcm_linkphy_subport_port_traverse(int unit,
                                   ...
                                   void *user_data)
{
    int group_id = 0, port = 0, subport_port_id = 0, rv = BCM_E_NONE;
    int linkphy_group_count = 0, linkphy_subport_count = 0;
    int group_traversed_count = 0;
    bcm_subport_config_t config;
    ...
    for (group_id = 0; group_id < _BCM_KT2_SUBPORT_GROUP_MAX; group_id++) {
        ...
        for (subport_port_id = 0; subport_port_id < linkphy_subport_count;
        subport_port_id++) {
            ...
            rv = _bcm_linkphy_subport_port_get(unit,
                                               linkphy_subport_port, &config);
            if (BCM_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: subport port traverse failed \n"
                    "to get LinkPHY subport port (subport_group id %d,"
                    " subport_port id %d)\n", group_id, subport_port_id));
                return rv;
            }
            ...
            if (BCM_FAILURE(rv) && SOC_CB_ABORT_ON_ERR(unit)) {
                SUBPORT_ERR(("ERROR: subport port traverse failed \n"
                    "to callback for subport port (subport_group id %d,"
                    " subport_port id %d)\n", group_id, subport_port_id));
                return rv;
            }
#endif
...
{
    int group_id = 0, rv = BCM_E_NONE;
    int group_traversed_count = 0, linkphy_group_count = 0;
    int linkphy_subport_count = 0;
    bcm_gport_t subport_group;
    ...
    for (group_id = 0; group_id < _BCM_KT2_SUBPORT_GROUP_MAX; group_id++) {
        ...
        if (linkphy_subport_count == 0 ) {
            ...
            rv = _bcm_linkphy_subport_group_destroy(unit, subport_group);
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: LinkPHY cleanup failed to delete"
                    " LinkPHY subport group 0x%x\n", subport_group));
                return rv;
            }
        }
...
_bcm_subtag_subport_init(int unit)
{
    ...
    SOC_PBMP_ITER(si->subtag_pbm, port) {
        ...
        port_type = _BCM_KT2_PORT_TYPE_CASCADED;
        cascaded_port_type = _BCM_KT2_PORT_TYPE_CASCADED_SUBTAG;
...
_bcm_subtag_subport_group_create(int unit,
                                 ...
                                 int group_id, bcm_gport_t *group)
{
    int subtag_subport_group = -1;

    _BCM_KT2_SUBPORT_CHECK_INIT(unit);

    _BCM_KT2_SUBTAG_SUBPORT_GROUP_SET(subtag_subport_group,
        is_trunk, port, group_id);
    BCM_GPORT_SUBPORT_GROUP_SET(*group, subtag_subport_group);
                                  ...
                                  bcm_gport_t group)
{
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(subport_group)) {
        ...
        * We need to verify that the subport ports under this group_id 
        * indeed belongs to this group*/
        if (subtag_subport_count) {
            /* read the first subport_port and get the group gport */
            if (group != _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_GROUP_GET(unit,
                group_id, 0)) {
                SUBPORT_ERR(
                    ("ERROR: SubTag subport group 0x%x not found.\n", group));
                return BCM_E_NOT_FOUND;
        ...
        /* delete all subport ports attached to the subport group */
        for (i = 0; i < subtag_subport_count; i++) {

            _BCM_KT2_SUBTAG_SUBPORT_PORT_SET(subport_port_gport, group_id, i);
                                   ...
                                   void *user_data)
{
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(subport_group)) {
        ...
        * indeed belongs to the given group.*/
        ...
        for (spp_idx = 0; spp_idx < subtag_subport_count; spp_idx++) {

            _BCM_KT2_SUBTAG_SUBPORT_PORT_SET(subport_port, group_id, spp_idx);
                             ...
                             bcm_gport_t *gport)
{
    ...
    int subport_group = -1, subtag_subport_port = -1, subtag_index = -1;
    int sp_group_idx = -1, sp_port_idx = 0, is_trunk = 0;
    uint16 vlan_id = 0;
    uint32 pp_port_index = 0, free_count = 0;
    soc_mem_t mem;
    soc_info_t *si = &SOC_INFO(unit);
    ...
    *******************************/
    ...
    if (!_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(subport_group)) {
        SUBPORT_ERR(("ERROR: config->group 0x%x is not SubTag group\n",
            config->group));
        return BCM_E_PARAM;
    }

    sp_group_idx = _BCM_KT2_SUBPORT_GROUP_SPGID_GET(subport_group);

    if (!_BCM_KT2_SUBTAG_SUBPORT_GROUP_USED_GET(unit, sp_group_idx)) {
        SUBPORT_ERR(("ERROR: config->group 0x%x does not exist. group id =%d\n",
    ...
    if (!SOC_PBMP_MEMBER(si->subtag_pbm, port)) {
        SUBPORT_ERR(("ERROR: Port %d is not member of pbmp_subtag\n", port));
        return BCM_E_PORT;
    }

    if (!BCM_VLAN_VALID(config->pkt_vlan)) {
        SUBPORT_ERR(
            ("ERROR: config->pkt_vlan %d is invalid\n", config->pkt_vlan));
        return BCM_E_PARAM;
    }
    ...
    vlan_id = config->pkt_vlan;

    if (_BCM_KT2_SUBTAG_VLAN_ID_USED_GET(unit, port, vlan_id)) {
        SUBPORT_ERR(("ERROR: VLAN id %d already used.port port %d\n",
            vlan_id, port));
        return BCM_E_EXISTS;
    }
    ...
    ***********************************************/
    ...
    soc_SUBPORT_TAG_TO_PP_PORT_MAPm_field32_set(unit,
        (uint32 *)&subtag_to_pp_entry, SUBPORT_TAGf, vlan_id);
    soc_SUBPORT_TAG_TO_PP_PORT_MAPm_field32_set(unit,
    ...
    soc_SUBPORT_TAG_TO_PP_PORT_MAPm_field32_set(unit,
        (uint32 *)&subtag_to_pp_entry, SUBPORT_TAG_MASKf,
        ((1 << soc_mem_field_length(unit, mem, SUBPORT_TAG_MASKf)) - 1));
    soc_SUBPORT_TAG_TO_PP_PORT_MAPm_field32_set(unit,
    ...
    *****************************************************/
    ...
    _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_SET(unit, sp_group_idx, sp_port_idx,
        config->group, pp_port_index, config->pkt_vlan, subtag_index, 1);

    _BCM_KT2_SUBTAG_SUBPORT_PORT_SET(subtag_subport_port,
                                      sp_group_idx, sp_port_idx);

    BCM_GPORT_SUBPORT_PORT_SET(*gport, subtag_subport_port);
                                ...
                                bcm_gport_t subport_port_gport)
{
    ...
    uint16 pp_port_index = 0, subtag_to_pp_port_tab_idx = 0;
    bcm_vlan_t vlan_id;
    soc_mem_t mem;
    subport_tag_to_pp_port_map_entry_t subport_tag_to_pp_port_map_entry;

    if (!_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_PORT(subport_port_gport)) {
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VALID_GET(unit,
                  group_id, subport_port_id)) {
    ...
    subport_group = _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_GROUP_GET(unit,
                        group_id, subport_port_id);

    port = _BCM_KT2_SUBPORT_GROUP_PORT_GET(subport_group);

    pp_port_index = _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_PP_PORT_ID_GET(unit,
                        group_id, subport_port_id);
    ...
    subtag_to_pp_port_tab_idx =
                        _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_TCAM_HW_ID_GET(unit,
                            group_id, subport_port_id);

    mem = SUBPORT_TAG_TO_PP_PORT_MAPm;
    if (subtag_to_pp_port_tab_idx > soc_mem_index_max(unit,mem)) {
        SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x is not found.\n"
            "subtag_to_pp_port_tab_idx = %d\n", subport_port_gport,
            subtag_to_pp_port_tab_idx));
        return BCM_E_NOT_FOUND;
    }

    vlan_id = _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VLAN_GET(unit,
                                    group_id, subport_port_id);
    ...
    *(_bcm_subport_group_subport_port_count[unit] + group_id) -= 1;
    ...
    _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_SET(unit, group_id, subport_port_id,
                                          0, 0, 0, 0, 0);
...
_bcm_subtag_subport_port_get(int unit,
                             ...
                             bcm_subport_config_t *config)
{
    int group_idx = -1, subport_port_id = -1, pp_port = 0;
    int rv = BCM_E_NONE;
    bcm_gport_t subport_group;
    uint16 subtag_to_pp_hw_id = 0;
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_PORT(subport_port_gport)) {
        ...
        if (!_BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VALID_GET(unit,
                      group_idx, subport_port_id)) {
            SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x not found\n",
                subport_port_gport));
            return BCM_E_NOT_FOUND;
        }

        pp_port = _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_PP_PORT_ID_GET(unit,
                      group_idx, subport_port_id);
        if (pp_port < _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN) {
            SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x is invalid\n"
                "pp_port index = %d \n", subport_port_gport, pp_port));
        ...
        subport_group = _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_GROUP_GET(unit,
                group_idx, subport_port_id);
        if (!BCM_GPORT_IS_SUBPORT_GROUP(subport_group)) {
            SUBPORT_ERR(("ERROR:Invalid SubTag subport port gport 0x%x\n",
                subport_port_gport));
            return BCM_E_PARAM;
        }
        ...
        config->pkt_vlan =
            _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VLAN_GET(unit,
                group_idx, subport_port_id);
        ...
        mem = SUBPORT_TAG_TO_PP_PORT_MAPm;
        if (subtag_to_pp_hw_id > soc_mem_index_max(unit, mem)) {
            SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x is not found.\n"
                "subtag_to_pp_hw_id = %d\n", subport_port_gport,
                subtag_to_pp_hw_id));
            return BCM_E_NOT_FOUND;
        }
...
{
    int group_id = 0, port = 0, subport_port_id = 0, rv = BCM_E_NONE;
    int subtag_group_count = 0, subtag_subport_count = 0;
    int group_traversed_count = 0;
    ...
    subtag_group_count = _bcm_subtag_subport_group_count[unit];

    for (group_id = 0; group_id < _BCM_KT2_SUBPORT_GROUP_MAX; group_id++) {

        if (subtag_group_count == group_traversed_count) {
            break;
        }
        ...
        for (subport_port_id = 0; subport_port_id < subtag_subport_count;
        subport_port_id++) {
            ...
            rv = _bcm_subtag_subport_port_get(unit,
                                              subtag_subport_port, &config);
            if (BCM_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: subport port traverse failed \n"
                    "to get SubTag subport port (subport_group id %d,"
                    " subport_port id %d)\n", group_id, subport_port_id));
                return rv;
            }
...
{
    int group_id = 0, rv = BCM_E_NONE;
    int group_traversed_count = 0, subtag_group_count = 0;
    int subtag_subport_count = 0;
    bcm_gport_t subport_group;
    bcm_port_t port;
    soc_mem_t mem;
    ...
    for (group_id = 0; group_id < _BCM_KT2_SUBPORT_GROUP_MAX; group_id++) {
        ...
        if (subtag_subport_count == 0 ) {
            ...
            subport_group =
                _BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_GROUP_GET(unit, group_id, 0);

            rv = _bcm_subtag_subport_group_destroy(unit, subport_group);
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: SubTag cleanup failed to delete"
                    " SubTag subport group 0x%x\n", subport_group));
                return rv;
            }
        }
...
bcm_kt2_subport_init(int unit)
{
    ...
    /* Destroy if any pp_port index reservation list handle exist */
    if (_bcm_subport_port_pp_port_idxres_list[unit]) {

        rv = shr_idxres_list_destroy(
                _bcm_subport_port_pp_port_idxres_list[unit]);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: pp_port index reservation list "
                "destroy failed.\n"));
        ...
        _bcm_subport_port_pp_port_idxres_list[unit] = NULL;
    }

    /* Create pp_port index reservation list handle */
    if (shr_idxres_list_create(
        &_bcm_subport_port_pp_port_idxres_list[unit],
        _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN, _BCM_KT2_SUBPORT_PP_PORT_INDEX_MAX,
        _BCM_KT2_SUBPORT_PP_PORT_INDEX_MIN, _BCM_KT2_SUBPORT_PP_PORT_INDEX_MAX,
        "subport-pp_port") != BCM_E_NONE) {
        SUBPORT_ERR(("ERROR: pp_port index reservation list create failed\n"));
                             ...
                             bcm_gport_t *group)
{
    ...
    if ((config->flags & BCM_SUBPORT_GROUP_TYPE_LINKPHY) &&
        !BCM_PBMP_MEMBER(si->linkphy_pbm, config->port)) {
        SUBPORT_ERR(("ERROR: config->port %d is not member of pbmp_linkphy\n",
                    config->port));
        return BCM_E_PORT;
    }

    if ((config->flags & BCM_SUBPORT_GROUP_TYPE_SUBPORT_TAG) &&
        !BCM_PBMP_MEMBER(si->subtag_pbm, config->port)) {
        SUBPORT_ERR(("ERROR: config->port %d should be member of pbmp_subport "
                    "and should not be member of pbmp_linkphy\n", config->port));
        return BCM_E_PORT;
    }

    if (!BCM_GPORT_IS_SET(config->port)) {
        port = config->port;
        is_trunk = 0;
    } else {
        if (BCM_GPORT_IS_TRUNK(config->port)) {
            is_trunk = 1;
            port = SOC_GPORT_TRUNK_GET(config->port);
        } else if (BCM_GPORT_IS_LOCAL(config->port)) {
            is_trunk = 0;
            port = SOC_GPORT_LOCAL_GET(config->port);
        } else {
            SUBPORT_ERR(("ERROR: Invalid config->port gport.\n"));
            return BCM_E_PORT;
        }
    }
    ...
    if (config->flags & BCM_SUBPORT_GROUP_WITH_ID) {
        ...
        /* Group id should be within 0 to 511.
        * BCM_GPORT_SUBPORT_GROUP_MASK is 0xffffff (for all chips)
        * subtag/linkphy group id is 9 bits. 
        * Check that only subtag/linkphy group id bits are configured*/
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) && 
        (config->flags & BCM_SUBPORT_GROUP_TYPE_LINKPHY)) {
        /* Check if config->port is a member of pbmp_linkphy.*/
        if (!SOC_PBMP_MEMBER(si->linkphy_pbm, config->port)) {
            SUBPORT_ERR(
                ("ERROR: Port %d should be LinkPHY enabled.\n", config->port));
        ...
        (config->flags & BCM_SUBPORT_GROUP_TYPE_SUBPORT_TAG)) {
        /* check if config->port is member of pbmp_subtag*/
        if (!SOC_PBMP_MEMBER(si->subtag_pbm, config->port)) {
            SUBPORT_ERR(
                ("ERROR: Port %d should be SubTag enabled.\n", config->port));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return BCM_E_PARAM;
        }
...

{
    ...
    *trunk_id = BCM_TRUNK_INVALID;
    ...
    if ((soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        group_id = _BCM_KT2_SUBPORT_GROUP_SPGID_GET(subport_group_gport);
        if(!_BCM_KT2_SUBPORT_GROUP_USED_GET(unit, group_id)) {
        ...
        if (_BCM_KT2_SUBPORT_GROUP_IS_TRUNK(subport_group_gport)) {
            *trunk_id =_BCM_KT2_SUBPORT_GROUP_PORT_GET(subport_group_gport);
        } else {
                            ...
                            bcm_gport_t subport_port_gport)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        _BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_PORT(subport_port_gport)) {
        if (BCM_PBMP_IS_NULL(si->linkphy_pbm)) {
            SUBPORT_ERR(("ERROR: LinkPHY ports not configured\n"));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return BCM_E_PARAM;
        ...
        _BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_PORT(subport_port_gport)) {
        if (BCM_PBMP_IS_NULL(si->subtag_pbm)) {
            SUBPORT_ERR(("ERROR: SubTag ports not configured\n"));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return BCM_E_PARAM;
                         ...
                         bcm_subport_config_t *config)
{
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Subport port 0x%x get failed. rv=%d(%s)\n",
                 subport_port_gport, rv, soc_errmsg(rv)));
        _BCM_KT2_SUBPORT_UNLOCK(unit);
        return rv;
    }
...
bcm_kt2_subport_port_resolve(int unit,
                              ...
                              bcm_trunk_t *trunk_id, int *id)
{
    int group_id = 0, subport_id = 0, pp_port_index = -1;
    ...
    *trunk_id = BCM_TRUNK_INVALID;
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        subport_id = _BCM_KT2_SUBPORT_PORT_SPPID_GET(subport_port_gport);
...
bcm_kt2_subport_cleanup(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_subtag_coe)) {
        if (BCM_PBMP_NOT_NULL(si->subtag_pbm)) {
                                     ...
                                     uint16 tpid)
{
    ...
    uint16 subport_group;
    bcm_port_t port;
    soc_mem_t mem;
    ing_physical_port_table_entry_t ing_physical_port_entry;

    subport_group = BCM_GPORT_SUBPORT_GROUP_GET(gport);
