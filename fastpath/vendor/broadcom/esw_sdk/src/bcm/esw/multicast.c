/*
 * $Id: multicast.c 1.73.2.23 Broadcom SDK $
...
#define _BCM_MULTICAST_MTU_TABLE_OFFSET(unit) (SOC_IS_HURRICANE(unit) ? 512 : 8192)
                             ...
                             bcm_pbmp_t *l3_pbmp)
{
    ...
    if (SOC_IS_EASYRIDER(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_ipmc_valid_as_hit_get(unit, &valid_as_hit));
    }
#endif /* BCM_EASYRIDER_SUPPORT */

    /* If entry is invalid - clear L2 & L3 bitmaps. */
    if (!valid_as_hit &&
         (0  == soc_mem_field32_get(unit, L3_IPMCm, entry, VALIDf))) {
        BCM_PBMP_CLEAR(*l2_pbmp);
        BCM_PBMP_CLEAR(*l3_pbmp);
        return (BCM_E_NONE);
    }
    /* If valid_as_hit, then invalid entry _is_ empty bitmaps */
                              ...
                              bcm_pbmp_t l3_pbmp, int valid)
{
    ...
    valid_as_hit = FALSE;   /* Need for Easyrider support */ 
    ...
    if (BCM_SUCCESS(rv)) {
        if (valid_as_hit) {
            /* No way to tell invalid entry, only flush if
             * we want to force it invalid */
            if (!valid) {
                sal_memset(&entry, 0, sizeof(ipmc_entry_t));
#if defined(BCM_EASYRIDER_SUPPORT) && defined(INCLUDE_L3)
                if (SOC_IS_EASYRIDER(unit)) {
                    uint32  min_mtu_l3;
                    rv = _bcm_er_ipmc_min_mtu_get(unit, l3_pbmp,
                                                  &min_mtu_l3);
                    if (BCM_FAILURE(rv)) {
                        soc_mem_unlock(unit, L3_IPMCm);
                        return rv;
                    }
                    soc_L3_IPMCm_field32_set(unit, entry, MIN_MTUf,
                                             min_mtu_l3);
                }
#endif /* BCM_EASYRIDER_SUPPORT */
            }
...
#if defined(BCM_EASYRIDER_SUPPORT)
    if (SOC_IS_EASYRIDER(unit)) {
        return _bcm_er_ipmc_egress_intf_list_add(unit, ipmc_id, port,
                                                 nh_index);
    } 
#endif
...
#if defined(BCM_EASYRIDER_SUPPORT)
    if (SOC_IS_EASYRIDER(unit)) {
        return _bcm_er_ipmc_egress_intf_list_delete(unit, ipmc_id, port, 
                                                    if_max, nh_index);
    } 
#endif
#if defined(BCM_FIREBOLT_SUPPORT)
...
/*
 * Function:
 *      _bcm_esw_multicast_local_trunk_members_setup
 * Purpose:
 *      Helper function to setup local trunk ports that are members of 
 *      a multicast group
 * Parameters:
 *      unit       - (IN) Device Number
 ...
 */
STATIC int 
_bcm_esw_multicast_local_trunk_members_setup(int unit, bcm_trunk_t trunk_id, 
                                       ...
                                       int *local_member_count)
{
    ...
    if (tinfo.flags & BCM_TRUNK_FLAG_MORE_MEMBERS) {
        /* There are more trunk members than can be fitted in
         * bcm_trunk_add_info_t. Use bcm_trunk_member_get API
         * to get all the trunk members.
         */
        ...
        for (i = 0; i < tinfo.num_ports; i++) {
            if (BCM_GPORT_IS_SET(tinfo.tp[i])) {
                member_array[i].gport = tinfo.tp[i];
            } else {
                dest.gport_type = _SHR_GPORT_TYPE_MODPORT;
                dest.modid = tinfo.tm[i];
                dest.port = tinfo.tp[i];
                          ...
                          bcm_port_t port, bcm_if_t encap_id)
{
    ...
    soc_mem_t       l2mem;
    bcm_pbmp_t      l2_pbmp;
    int             mc_index, gport_id, modid_local, idx, trunk_local_ports;
    bcm_module_t    mod_out;
    bcm_port_t      port_out;
    bcm_port_t      local_trunk_member_ports[SOC_MAX_NUM_PORTS];	
    bcm_trunk_t     trunk_id;
   
    modid_local = 0;

    l2mem  = SOC_IS_EASYRIDER(unit) ? L2MC_TABLEm : L2MCm;
...
#endif	/* BCM_XGS3_FABRIC_SUPPORT */
    if ((mc_index < 0) ||
        (mc_index >= soc_mem_index_count(unit, l2mem))) {
        return BCM_E_PARAM;
    }
    ...
    if (BCM_TRUNK_INVALID != trunk_id) {
        BCM_IF_ERROR_RETURN(
            _bcm_esw_multicast_local_trunk_members_setup(unit, trunk_id, 
                                                   SOC_MAX_NUM_PORTS,
    ...
    BCM_IF_ERROR_RETURN
        (soc_mem_read(unit, l2mem, MEM_BLOCK_ANY, mc_index, &l2mc_entry));

    soc_mem_pbmp_field_get(unit, l2mem, &l2mc_entry, PORT_BITMAPf,
                           &l2_pbmp);
    ...
    soc_mem_pbmp_field_set(unit, l2mem, &l2mc_entry, PORT_BITMAPf, &l2_pbmp);

    return soc_mem_write(unit, l2mem, MEM_BLOCK_ALL, mc_index, &l2mc_entry);
}
                             ...
                             int *port_count)
{
    ...
    rv = soc_mem_read(unit, l2mem, MEM_BLOCK_ANY, mc_index, &l2mc_entry);
    soc_mem_unlock(unit, l2mem);
    BCM_IF_ERROR_RETURN(rv);

    soc_mem_pbmp_field_get(unit, l2mem, &l2mc_entry, PORT_BITMAPf, &l2_pbmp);
...
_bcm_esw_multicast_l2_create(int unit, uint32 flags, bcm_multicast_t *group)
{
    ...
    soc_mem_field32_set(unit, l2mem, &l2mc_entry, VALIDf, 1);
    BCM_IF_ERROR_RETURN(
        soc_mem_write(unit, l2mem, MEM_BLOCK_ALL, mc_index, &l2mc_entry));
...
_bcm_esw_multicast_l2_destroy(int unit, bcm_multicast_t group)
{
    ...
    BCM_IF_ERROR_RETURN(
        soc_mem_write(unit, L2MCm, MEM_BLOCK_ALL, 
                      mc_index, soc_mem_entry_null(unit, l2mem)));
        ...
        bcm_gport_t *trunk_member)
{
    ...
    *trunk_member = port;
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_trunk_local_members_get(unit, trunk_id,
                SOC_MAX_NUM_PORTS, local_trunk_port_array,
 ...
 *      bcm_trunk_add_info_t. The hash fields used for trunk group member
 *      selection are specified by ipmc_psc field in bcm_trunk_add_info_t.
...
{
    int mc_index, is_l3, gport_id, idx, trunk_local_ports; 
    bcm_module_t mod_out;
    bcm_port_t port_out;
    bcm_trunk_t trunk_id;
    ...
    if (soc_feature(unit, soc_feature_trill)) {
        if (BCM_GPORT_IS_TRILL_PORT(port)) {
             BCM_IF_ERROR_RETURN
                (_bcm_td_trill_port_resolve(unit, port, encap_id,
                                &mod_out, &port_out, &trunk_id, &gport_id));
             BCM_IF_ERROR_RETURN
                (bcm_td_trill_egress_add(unit, port, group, encap_id));
            }
        }
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
             for (idx = 0; idx < trunk_local_ports; idx++) {
                 (void) _bcm_esw_ipmc_egress_intf_delete(unit,
                             ...
                             BCM_MULTICAST_PORT_MAX, encap_id, is_l3);
             }
        } else {
...
{
    int mc_index, is_l3, gport_id, i, idx, trunk_local_ports; 
    bcm_module_t mod_out;
    bcm_port_t port_out;
    bcm_trunk_t trunk_id;
    int modid_local, local_count, port_count, rv = BCM_E_NONE; 
    bcm_port_t    temp_port, local_trunk_member_ports[SOC_MAX_NUM_PORTS];	
    ...
    if (soc_feature(unit, soc_feature_trill)) {
        if (BCM_GPORT_IS_TRILL_PORT(port)) {
             BCM_IF_ERROR_RETURN
                (bcm_td_trill_egress_delete(unit, port, group, encap_id));
            }
        }
#endif /* BCM_TRIDENT_SUPPORT && INCLUDE_L3 */
     ...
     */
    port_array = sal_alloc(BCM_MULTICAST_PORT_MAX * sizeof(bcm_gport_t),
                           "mcast port array");
    if (port_array == NULL) {
        return BCM_E_MEMORY;
    }
    encap_array = sal_alloc(BCM_MULTICAST_PORT_MAX * sizeof(bcm_if_t),
                            "mcast encap array");
    if (encap_array == NULL) {
        sal_free (port_array);
        return BCM_E_MEMORY;
    ...
    rv = bcm_esw_multicast_egress_get(unit, group, BCM_MULTICAST_PORT_MAX, 
    ...
    if (BCM_TRUNK_INVALID != trunk_id) {
        for (idx = 0; idx < trunk_local_ports; idx++) {
            rv =  _bcm_esw_ipmc_egress_intf_delete(unit, mc_index,
                                local_trunk_member_ports[idx], BCM_MULTICAST_PORT_MAX, 
                                               encap_id, is_l3);
            if (rv < 0) {
                while (idx--) {
                            ...
                            bcm_gport_t *port_array, bcm_if_t *encap_id_array)
{
    ...
    mc_index = _BCM_MULTICAST_ID_GET(group);
    l2mem = SOC_IS_EASYRIDER(unit) ? L2MC_TABLEm : L2MCm;
    mem = (_BCM_MULTICAST_L2_OR_FABRIC(unit, group)) ? l2mem : L3_IPMCm; 
    /* Table index sanity check. */
#ifdef BCM_XGS3_FABRIC_SUPPORT
    if (SOC_IS_XGS3_FABRIC(unit)) {
        BCM_IF_ERROR_RETURN
    ...
    if (port_count > 0) {
        ...
        for (i = 0; i < port_count; i++) {
            local_trunk_port_array[i] =
                sal_alloc(sizeof(bcm_port_t) * SOC_MAX_NUM_PORTS,
                          "local_trunk_port_array");
        ...
        for (i = 0; i < port_count; i++) {
            ...
            for (idx = 0; idx < SOC_MAX_NUM_PORTS; idx++) {
    ...
    for (i = 0; i < port_count ; i++) {
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            rv = _bcm_esw_multicast_local_trunk_members_setup(unit, trunk_id,
                    SOC_MAX_NUM_PORTS, local_trunk_port_array[i],
                    &trunk_local_ports[i]);
            if (BCM_FAILURE(rv)) {
                BCM_MULTICAST_MEM_FREE_RETURN(rv);
            }
        ...
        if (0 == port_count) {
            ...
            PBMP_PORT_ITER(unit, port_iter) {
                ...
                for (i = 0; i < port_count ; i++) {
                    ...
                    } else if (0 == IS_E_PORT(unit, port_iter)) {
                                 ...
                                 bcm_gport_t mpls_port_id, bcm_if_t *encap_id)
{
    ...
    if (SOC_IS_TRIDENT(unit) || SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                                ...
                                bcm_gport_t mim_port, bcm_if_t *encap_id)
{
    ...
    if (SOC_IS_TRIDENT(unit) || SOC_IS_TRIUMPH2(unit) ||
        SOC_IS_APOLLO(unit) || SOC_IS_KATANA(unit) ||
...
#if defined(BCM_TRIDENT_SUPPORT) && defined(INCLUDE_L3)
    if (soc_feature(unit, soc_feature_mpls) && soc_feature(unit, soc_feature_l3) ) {
        /* Verify egress object validity */ 
        if (!BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf) && 
            !BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, intf)) {
             return (BCM_E_PARAM);
        }
        /* Next-hop index is used for multicast replication */
        *encap_id = (intf - BCM_XGS3_EGRESS_IDX_MIN) + BCM_XGS3_DVP_EGRESS_IDX_MIN;
        return BCM_E_NONE;
    }
#endif /* BCM_TRIDENT_SUPPORT && INCLUDE_L3 */
        ...
        PBMP_PORT_ITER(unit, port_iter) {
            ...
            for (i = 0; i < port_count ; i++) {
                ...
                if (list_count) {
                    ...
                    rv = _bcm_esw_ipmc_subscriber_egress_intf_set(unit, remap_index, 
                                              port_iter, list_count, temp_encap_list, 
                                              q_id, TRUE, &q_count);
                    if (BCM_FAILURE(rv)) {
                        BCM_SUBSCRIBER_MULTICAST_MEM_FREE_RETURN(rv);
                    } else {
    ...
    for (i = 0; i < count; i++) {
        if (NULL != port_array && NULL != queue_array) {
           /* Convert to GPORT values */
             ...
             BCM_GPORT_UCAST_SUBSCRIBER_QUEUE_GROUP_QID_SET(port_array[i],
                                                 queue_array[i]);
             port_array[i] |= (port << 16);
        }
        if (i == port_max) {
            *port_count = port_max;
            break;
...
_bcm_esw_multicast_egress_subscriber_add(int unit, bcm_multicast_t group,
                                         ...
                                         bcm_gport_t subscriber_queue)
{
    int mc_index, is_l3, gport_id, trunk_local_ports; 
    bcm_module_t mod_out;
    bcm_port_t port_out;
    bcm_trunk_t trunk_id;
    int modid_local, rv = BCM_E_NONE; 
    ...
    if (BCM_IF_INVALID != encap_id) {
       ...
       BCM_IF_ERROR_RETURN
        (_bcm_esw_ipmc_mmu_mc_remap_ptr(unit, mc_index, &remap_index, FALSE));
       BCM_IF_ERROR_RETURN(
       _bcm_esw_ipmc_subscriber_egress_intf_add(unit, remap_index, port_out, 
                                          encap_id, subscriber_queue, is_l3,  
                                          &q_count));
	   new_pbmp = &l3_pbmp;
    } else {
        if (!_BCM_MULTICAST_IS_VPLS(group)) {
    ...
    if (BCM_FAILURE(rv)) {
        (void) _bcm_esw_ipmc_subscriber_egress_intf_delete(unit, mc_index, 
                                   port_out, BCM_MULTICAST_PORT_MAX, 
...
_bcm_esw_multicast_egress_subscriber_delete(int unit, bcm_multicast_t group,
                                            ...
                                            bcm_gport_t subscriber_queue) 
{
    int mc_index, is_l3, gport_id, i, trunk_local_ports; 
    bcm_module_t mod_out;
    bcm_port_t port_out;
    bcm_trunk_t trunk_id;
    int modid_local, local_count, port_count, rv = BCM_E_NONE; 
    ...
    q_array = sal_alloc(BCM_MULTICAST_PORT_MAX * sizeof(bcm_if_t),
                            "mcast q array");
    if (q_array == NULL) {
        sal_free (q_array);
        return BCM_E_MEMORY;
    ...
    rv =  _bcm_esw_ipmc_subscriber_egress_intf_delete(unit, remap_index, port_out,
                                        encap_id, subscriber_queue, is_l3, &q_count);
    if (BCM_SUCCESS(rv)) {
...
_bcm_esw_multicast_sw_dump_cb(int unit, bcm_multicast_t group, 
                              uint32 flags, void *user_data)
{
    soc_cm_print("    0x%08x    0x%08x     ", group, flags);
    if (flags & BCM_MULTICAST_TYPE_L2) {
        soc_cm_print(" L2");
    }
...
_bcm_multicast_sw_dump(int unit)
{
    ...
    soc_cm_print("    Groups:       Flag value:     Flags:\n");
    ...
    if (SOC_REG_IS_VALID(unit, IPMC_L3_MTUr) && (NULL != _bcm_mtu_profile)) {
        ...
        soc_cm_print("  IPMC_L3_MTU\n");
        soc_cm_print("    Number of entries: %d\n", num_entries);
        soc_cm_print("    Index RefCount -  IPMC_L3_MTU\n");
