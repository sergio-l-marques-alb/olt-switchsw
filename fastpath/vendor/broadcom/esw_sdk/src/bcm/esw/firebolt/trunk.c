/*
 * $Id: trunk.c 1.150.2.8 Broadcom SDK $
                           ...
                           trunk_private_t *t_info)
{
    ...
    if (SOC_IS_HBX(unit) || SOC_IS_TRX(unit)) {
        uint64        val64;
        int        i=0, port_limit,val=0,temp=0;
        ...
        if (!SOC_IS_HURRICANE(unit)) { /* PR 34459 */
           ...
           for (i = 0; i < 4; i++) {
              val=soc_reg64_field32_get(unit, HG_TRUNK_GROUPr, val64,
                                        _hg_trunk[i]);
              if (i==0) {
                 temp = val;
                          ...
                          trunk_private_t *trunk_info)
{
    ...
    for (tid = min_tid; tid <= max_tid; tid++, trunk_info++) {
        ...
        for (i = 0; i < t_data->num_ports; i++) {
            MEMBER_INFO(unit, (hgtid + _BCM_FP_TRUNK_NUM_MAX)).modport[i] = t_data->tp[i];
        }
        MEMBER_INFO(unit, (hgtid + _BCM_FP_TRUNK_NUM_MAX)).num_ports = t_data->num_ports;
