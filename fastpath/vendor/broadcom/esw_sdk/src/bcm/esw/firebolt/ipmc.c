/*
 * $Id: ipmc.c 1.162.2.9 Broadcom SDK $
...
_xgs3_ipmc_write(int unit, int ipmc_id, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (SOC_IS_HBX(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_ipmc_bitmap_set(unit, ipmc_id, l2_pbmp, FALSE));
    }
#endif /* BCM_BRADLEY_SUPPORT */
...
bcm_fb_er_ipmc_put(int unit, int index, bcm_ipmc_addr_t *ipmc)
{
    ...
    /* Need to write to L3_IPMC table before replacing
     * the IPMC address entry in L3 table, for the following reason:
     * If an IPMC address entry's IPMC index field is replaced before
     * the source port information is written into L3_IPMC table,
     * there would be a temporary loss of traffic due to source port
     * mismatch.
     */ 
