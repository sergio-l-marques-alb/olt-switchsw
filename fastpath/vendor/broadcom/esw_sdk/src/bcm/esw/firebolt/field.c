/* $Id: field.c 1.496.2.32 Broadcom SDK $
...
_field_physical_to_virtual(int unit, int slice_idx, _field_stage_t *stage_fc)
{
    ...
    slice_num = SOC_IS_HAWKEYE(unit) ? 4 : (SOC_IS_KATANA(unit) ? 12 : 16);
    for (vslice = 0; vslice < slice_num; vslice++) {
                         ...
                         int *tcam_idx_old, int *tcam_idx_new)
{
    ...
    for (idx = 0; idx < parts_count; idx++) {
        ...
        if (SOC_IS_KATANA(unit) && (stage_id == _BCM_FIELD_STAGE_INGRESS)) {
    ...
    if (SOC_IS_KATANA(unit)) {
        if (f_ent->actions != NULL) {   
           if ((f_ent->actions->action == bcmFieldActionFabricQueue) &&
               ...
               if (param0 & BCM_FABRIC_QUEUE_DEST_OFFSET) {
                           ...
                           int tbl_idx, int offset)
{
    ...
    for (idx = 0; idx <= _FP_DATA_DATA1_WORD_MAX; idx++) {
        ...
        if(SOC_IS_ENDURO(unit) || SOC_IS_HURRICANE(unit)) {
            if ((0 == idx) && !((*usage_bmap) & (1 << 0x1)) &&
                !((*usage_bmap) & (1 << 0x3))) {
                /* 
                 * If UDF1_31_0 (FPF1=9) is used for UDF only, The field 
                 * UDF1_OFFSET3f is required for UDF1_31_0 on Enduro. 
                 */
                offset_idx = _FP_DATA_DATA0_WORD_MAX;
            } else if((2 == idx) && !((*usage_bmap) & (1 << 0x1)) &&
                      !((*usage_bmap) & (1 << 0x3))) {
                /* 
                 * If UDF1_31_0 (FPF1=9) and UDF1_95_64 (FPF3=9) are used for 
                 *      UDF only, The field UDF1_OFFSET1f is required for 
                 *      UDF1_95_64 on Enduro. 
                 */
                offset_idx = _FP_DATA_DATA0_WORD_MAX - idx;
            } else if ((1 == idx) && !(f_dq->hw_bmap & 0x1)) {
                /* 
                 * If SDK is requred to add one more chunk besides UDF1_31_0
                 *      and UDF1_95_64, SDK needs to set the value of  
                 *      UDF1_OFFSET0 and UDF1_OFFSET2 for UDF1_31_0 and 
                 *      UDF1_95_64 separetly.
                 * Because Enduro chip uses different UDF_OFFSET for UDF1_31_0 
                 *      and UDF1_95_64 when chip is using UDF1 instead of 
                 *      single chunk.
                 */

                value = soc_FP_UDF_OFFSETm_field32_get(
                                       unit, &tbl_entry, UDF1_OFFSET3f);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_OFFSET0f,value);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_OFFSET3f,0x0);
                value = soc_FP_UDF_OFFSETm_field32_get(
                            unit, &tbl_entry, UDF1_ADD_IPV4_OPTIONS3f);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_ADD_IPV4_OPTIONS0f, value);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_ADD_IPV4_OPTIONS3f, 0x0);
                if (SOC_MEM_FIELD_VALID(unit, FP_UDF_OFFSETm, 
                                        UDF1_ADD_GRE_OPTIONS0f)) {
                    value = soc_FP_UDF_OFFSETm_field32_get(
                                unit, &tbl_entry, UDF1_ADD_GRE_OPTIONS3f);
                    soc_FP_UDF_OFFSETm_field32_set(
                        unit, &tbl_entry, UDF1_ADD_GRE_OPTIONS0f, value);
                    soc_FP_UDF_OFFSETm_field32_set(
                        unit, &tbl_entry, UDF1_ADD_GRE_OPTIONS3f, 0x0);
                }
                
                value = soc_FP_UDF_OFFSETm_field32_get(
                                       unit, &tbl_entry, UDF1_OFFSET1f);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_OFFSET2f,value);
                value = soc_FP_UDF_OFFSETm_field32_get(
                            unit, &tbl_entry, UDF1_ADD_IPV4_OPTIONS1f);
                soc_FP_UDF_OFFSETm_field32_set(
                    unit, &tbl_entry, UDF1_ADD_IPV4_OPTIONS2f, value);
                if (SOC_MEM_FIELD_VALID(unit, FP_UDF_OFFSETm, 
                                        UDF1_ADD_GRE_OPTIONS0f)) {
                    value = soc_FP_UDF_OFFSETm_field32_get(
                                unit, &tbl_entry, UDF1_ADD_GRE_OPTIONS1f);
                    soc_FP_UDF_OFFSETm_field32_set(
                        unit, &tbl_entry, UDF1_ADD_GRE_OPTIONS2f, value);
                }
                offset_idx = idx;
            } else {
                offset_idx = idx;
            ...
            if ((etype->l2 == etype_ptr->l2) &&
                ...
                for (tbl_idx = soc_mem_index_min(unit, FP_UDF_OFFSETm); 
                    ...
                    if (soc_FP_UDF_OFFSETm_field32_get(
                        unit, &tbl_entry, UDF1_OFFSET3f) == 0) {
                        continue;
                    }
