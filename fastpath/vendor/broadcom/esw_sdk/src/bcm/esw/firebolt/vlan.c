/*
 * $Id: vlan.c 1.177.2.28 Broadcom SDK $
                                ...
                                bcm_vlan_control_vlan_t *control)
{
	...
	if (_BCM_MULTICAST_IS_VLAN(control->broadcast_group) ||
	    _BCM_MULTICAST_IS_VLAN(control->unknown_multicast_group) ||
	    _BCM_MULTICAST_IS_VLAN(control->unknown_unicast_group))
        {
          if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
             /* Turn on VIRTUAL_PORT_ENf when the group is being set */
              soc_mem_field32_set(unit, VLAN_TABm, &vt, VIRTUAL_PORT_ENf, 1);
          }
        }
        else if ((control->broadcast_group == 0) &&							                                      (control->unknown_multicast_group == 0) &&							                           (control->unknown_unicast_group == 0))
        {
	  /* When the group is cleared, the VIRTUAL_PORT_ENf must be	
	   * disabled, so that the UUC_IDXf, UMC_IDXf, BC_IDXf fields
	   * are not used.
	   */
	  if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
