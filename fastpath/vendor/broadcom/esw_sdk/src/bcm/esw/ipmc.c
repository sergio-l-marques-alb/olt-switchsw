        ...
        if (SOC_MEM_IS_VALID(unit, MMU_REPL_LIST_TBLm)) {
            ...
            SOC_IF_ERROR_RETURN
                (READ_MMU_REPL_LIST_TBLm(unit, MEM_BLOCK_ANY, 0,
                                         &repl_list_entry));
            /* Temporarily Avoiding SOC_IS_KATANA2(unit) */
...
_bcm_esw_ipmc_required_scache_size_get(int unit, uint32 *size)
{
    *size = 0;
    ...
    if (soc_feature(unit, soc_feature_pim_bidir)) {
        ...
        BCM_IF_ERROR_RETURN
            (bcm_td2_ipmc_required_scache_size_get(unit, &temp_size));
        *size += temp_size;
    }
#endif /* BCM_TRIDENT2_SUPPORT */
...
#ifdef BCM_TRIDENT2_SUPPORT
    if (soc_feature(unit, soc_feature_pim_bidir)) {
        BCM_IF_ERROR_RETURN(bcm_td2_ipmc_sync(unit, &ipmc_scache_ptr));
    }
#endif /* BCM_TRIDENT2_SUPPORT */
        ...
        if (required_scache_size > 0) {
            ...
            if (BCM_E_NOT_FOUND == rv) {
                ...
                if (soc_feature(unit, soc_feature_pim_bidir)) {
                     ...
                     */
                    BCM_IF_ERROR_RETURN
                        (bcm_td2_ipmc_scache_recover(unit, &ipmc_scache_ptr));
                }
#endif /* BCM_TRIDENT2_SUPPORT */
            }
        }
...
bcm_esw_ipmc_add(int unit, bcm_ipmc_addr_t *data)
{
    ...
    /* Call VXLAN module to enable BUD functionality */
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_esw_vxlan_multicast_leaf_entry_check(unit, data->mc_ip_addr, 0x1);
    }
...
#if defined(BCM_TRIDENT2_SUPPORT) && defined(INCLUDE_L3)
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_esw_vxlan_multicast_leaf_entry_check(unit, data->mc_ip_addr, 0x0);
    }
#endif /* BCM_TRIDENT2_SUPPORT && INCLUDE_L3 */
