/*
 * $Id: port.c 1.1012.2.86 Broadcom SDK $
...
_bcm_port_update(int unit, bcm_port_t port, int link)
{
    ...
    if ((SOC_REG_IS_VALID(unit, GXPORT_LAG_FAILOVER_CONFIGr) &&
        ...
        reg = SOC_REG_IS_VALID(unit, LAG_FAILOVER_CONFIGr) ?
              LAG_FAILOVER_CONFIGr : GXPORT_LAG_FAILOVER_CONFIGr;
...
int
bcm_esw_port_dscp_map_mode_set(int unit, bcm_gport_t port, int mode)
{
    int rv;
                         ...
                         bcm_port_control_t type, int value)
{
    ...
    case bcmPortControlCustomerQueuing:
      ...
      case bcmPortControlOamLoopback:
          ...
          if (SOC_IS_TRX(unit) && !SOC_IS_HURRICANE(unit) && !SOC_IS_SHADOW(unit))
          {
                         ...
                         bcm_port_control_t type, int *value)
{
    ...
    case bcmPortControlCustomerQueuing:
        ...
        if (SOC_IS_KATANA(unit)) {
            ...
            if (SOC_REG_FIELD_VALID(unit, ING_COS_MODEr, COS_MODEf)) {
                *value = soc_reg_field_get(unit, ING_COS_MODEr, rval, COS_MODEf);  
                break;
            }     
        }     
#endif /* BCM_KATANA_SUPPORT */
