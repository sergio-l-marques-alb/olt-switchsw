/*
 * $Id: port.c 1.1321.4.8 Broadcom SDK $
...
int _bcm_tr3_port_lanes_set(int unit, bcm_port_t port, int value);
#endif /* BCM_TRIUMPH3_SUPPORT */
...

{
    ...
    if (BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        if (SOC_IS_TRIUMPH3(unit)) {
            rv = _bcm_tr3_wlan_port_set(unit, port, field, value);
        } else 
#endif /* BCM_TRIUMPH3_SUPPORT */
        {
...
#if defined(BCM_TRIUMPH3_SUPPORT)
        if (SOC_IS_TRIUMPH3(unit)) {
            rv = _bcm_tr3_lport_field_get(unit, port, field, value);
        } else
#endif /* BCM_TRIUMPH3_SUPPORT */
        {
...
bcm_esw_port_enable_set(int unit, bcm_port_t port, int enable)
{
    ...
    if (loopback) {
        BCM_IF_ERROR_RETURN(_bcm_esw_link_force(unit, port, TRUE, enable));
    }
...
_bcm_esw_port_wb_recover(int unit)
{
    ...
    if (SOC_IS_TRX(unit)) {
        ...
        soc_info_t *si = &SOC_INFO(unit);
#ifdef BCM_TRIUMPH3_SUPPORT
        int _port_num_lanes[SOC_MAX_NUM_PORTS];
...
#ifdef BCM_TRIUMPH3_SUPPORT
        if (SOC_IS_TRIUMPH3(unit) && soc_feature(unit, soc_feature_flex_port) &&
            _port_lanes_valid) {
            ...
            PBMP_ALL_ITER(unit, port) {
                if (soc_tr3_port_is_flex_port(unit, port) && 
                    ...
                    soc_cm_debug(DK_VERBOSE, "Port %d needs flex transform !!\n", port);
            ...
            /* Call PHY probe for config to take effect */
            BCM_PBMP_ASSIGN(enabled_ports, PBMP_PORT_ALL(unit));
            BCM_PBMP_REMOVE(enabled_ports, SOC_PORT_DISABLED_BITMAP(unit, port));

            if ((rv = bcm_esw_port_probe(unit, enabled_ports,
                                         &port_bitmap)) != BCM_E_NONE) {
                soc_cm_debug(DK_ERR, "Error unit %d:\
                                      Failed port probe: %s\n",
                                      unit, bcm_errmsg(rv));
...
_bcm_esw_port_higig2_mode_set(int unit, bcm_port_t port, int higig2_mode)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_NONE,
                HIGIG2f, higig2_mode));
    ...
    if (SOC_REG_FIELD_VALID(unit, XLPORT_CONFIGr, HIGIG2_MODEf)) {
    ...
    if (SOC_MEM_FIELD_VALID(unit, EGR_ING_PORTm, HIGIG2f)) {
        if (IS_CPU_PORT(unit, port)) {
            BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit, EGR_ING_PORTm,
...
bcm_esw_port_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_action)) {
        ...
        SOC_PBMP_CLEAR(bmp);
        if (SOC_IS_KATANA2(unit)) {
            for (p=0; p<(SOC_INFO(unit).lb_port)+1; p++) {
                 SOC_PBMP_PORT_ADD(bmp, p);
            }
        } else {
    ...
    PBMP_ITER(okay_ports, p) {
         ...
         * XL, XT, CL or MXQ blocks */
        if (IS_XL_PORT(unit, p) || IS_XT_PORT(unit, p) ||
            IS_CL_PORT(unit, p) || IS_MXQ_PORT(unit,p)) {
            uint32 rval;
    ...
    if (SOC_IS_SHADOW(unit))  {
        ...
        for (p = 0; p <= 16; p++) {
            ...
            SOC_IF_ERROR_RETURN(
                    READ_PORT_TABm(unit, MEM_BLOCK_ANY, p, &ptab));
            soc_PORT_TABm_field32_set(unit, &ptab, IPV4_ENABLEf, 1);
            soc_PORT_TABm_field32_set(unit, &ptab, IPV6_ENABLEf, 1);
            SOC_IF_ERROR_RETURN(
                WRITE_PORT_TABm(unit,MEM_BLOCK_ALL, p, &ptab));
        }
    ...
    if (SOC_IS_TD_TT(unit) || SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit)) {
        ...
        PBMP_ALL_ITER(unit, p) {
            ...
            if (SOC_IS_TRIDENT2(unit) && blk_num >= 7) {
                continue;
            }
            if (SOC_IS_KATANA2(unit)) {
    ...
    if (SOC_IS_HURRICANE (unit) || SOC_IS_TD_TT (unit) ||
        ...
        if (soc_feature (unit, soc_feature_eee)) {
            ...
            PBMP_ALL_ITER(unit, p) {
                if (!IS_LB_PORT(unit, p) && !IS_CPU_PORT (unit, p) &&
                    !IS_TDM_PORT(unit, p)) {
                    /* EEE standard compliance Work Around:
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (soc_feature(unit, soc_feature_higig2)) {
            ...
            PBMP_HG_ITER(unit, hg_port) {
                if (soc_property_port_get(unit, hg_port, spn_HIGIG2_HDR_MODE, 0)) {
                    BCM_IF_ERROR_RETURN
    ...
    if (SOC_IS_KATANA2(unit)) {
        ...
        SOC_IF_ERROR_RETURN(
            WRITE_EGR_ENABLEm(unit, MEM_BLOCK_ALL, 0, entry));
    }
#endif /* BCM_KATANA_SUPPORT */
...
bcm_esw_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp)
{
    ...
    if (SOC_REG_IS_VALID(unit, XLPORT_MAC_CONTROLr)) {
        int blk, port;
    ...
    if (SOC_REG_IS_VALID(unit, CPORT_MAC_CONTROLr)) {
        int blk, port;
                                        ...
                                        int enable)
{
    ...
    if (soc_feature(unit, soc_feature_ep_redirect)) {
        soc_field_t egr_port_fields[2] =
            { REDIRECTION_TYPEf, REDIRECTION_DESTINATIONf };
...
bcm_esw_port_loopback_set(int unit, bcm_port_t port, int loopback)
{
    ...
    if ((loopback == BCM_PORT_LOOPBACK_NONE) || !BCM_SUCCESS(rv)) {
        ...
        if (BCM_SUCCESS(rv)) {
             ...
             */
            rv = _bcm_esw_link_force(unit, port, TRUE, link);
        }
        if (BCM_FAILURE(rv)) {
            return (rv);
        }
...
#if defined(BCM_TRIUMPH3_SUPPORT)
                if (SOC_IS_TRIUMPH3(unit)) {
                    rv = _bcm_tr3_lport_field_get(unit, port, EN_IFILTERf, &value);
                } else
#endif /* BCM_TRIUMPH3_SUPPORT */
                {
...
_bcm_port_encap_xport_set(int unit, bcm_port_t port, int mode)
{
    ...
    port_type_field = (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                       ...
                       SOC_IS_TRIUMPH3(unit) || SOC_IS_KATANAX(unit)) ? PORT_TYPEf : HIGIG_PACKETf;
    egr_port_reg = (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
                    SOC_IS_VALKYRIE2(unit)) ? EGR_PORT_64r : EGR_PORTr;
    ...
    if (SOC_MEM_IS_VALID(unit, EGR_PORTm)) {
        if (soc_mem_field_valid(unit, EGR_PORTm, port_type_field)) {
            SOC_IF_ERROR_RETURN(soc_mem_field32_modify(
                            unit, EGR_PORTm, port, port_type_field,port_type));
        }
    } else {
        SOC_IF_ERROR_RETURN
...
_bcm_port_encap_stport_set(int unit, bcm_port_t port, int mode)
{
    ...
    port_type = (SOC_IS_ENDURO(unit) || SOC_IS_TRIUMPH2(unit) || 
                 ...
                 SOC_IS_HURRICANEX(unit) || SOC_IS_TRIUMPH3(unit)) ? PORT_TYPEf : HIGIG_PACKETf;
    ...
    /* Reset the vlan default action */
    if ((SOC_IS_ENDURO(unit) || SOC_IS_HURRICANEX(unit)) && soc_feature(unit, soc_feature_vlan_action)) {
        bcm_vlan_action_set_t action;
...
bcm_esw_port_encap_set(int unit, bcm_port_t port, int mode)
{
    ...
    if (SOC_IS_TRIUMPH3(unit) && (mode != BCM_PORT_ENCAP_IEEE) &&
        ...
        if (hgc+1 > _SOC_TR3_MAX_HG_PORT_COUNT) {
            soc_cm_debug(DK_ERR, "Error: Unit: %d Port %d config error "\
    ...
    if (IS_GX_PORT(unit, port) || IS_XG_PORT(unit,port) ||
        ...
        if (SOC_IS_TRIUMPH3(unit) && (mode == BCM_PORT_ENCAP_IEEE)) {
            ...
            if (IS_CL_PORT(unit, port) || (blk == 13)) {
                ...
                _BCM_PORT_IF_ERROR_RETURN_WITH_UNLOCK
                    (READ_E2ECC_TX_ENABLE_BMPr(unit, blk_num, &regval));
                if (regval & (1 << bindex)) {
...
bcm_esw_port_inner_tpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    ...
    if (SOC_IS_FBX(unit)) {
        ...
        BCM_IF_ERROR_RETURN(soc_reg_field32_modify(unit, EGR_CONFIGr,
                                                   ...
                                                   tpid));
#if defined (BCM_HURRICANE2_SUPPORT) || defined (BCM_KATANA2_SUPPORT) 
        if (SOC_IS_HURRICANE2(unit) || SOC_IS_KATANA2(unit)) {
           uint32 enable = 1;
           BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit, PORT_TABm, port,                                                          INNER_TPID_ENABLEf, enable));
        }
#endif
                                ...
                                int udp, int tcp, int enable)
{
    ...
    if (vp >= 0) {
        ...
        BCM_IF_ERROR_RETURN(
            WRITE_SOURCE_TRUNK_MAP_TABLEm(unit, MEM_BLOCK_ALL, 
                                            local_port, &stm_entry));
...
_bcm_tr3_port_lanes_set(int unit, bcm_port_t port, int value)
{
    ...
    for (i = 0; i < ports; i+=incr) {
        ...
        soc_cm_debug(DK_VERBOSE, "Port %d probe: %d\n", port + i, okay);
        if (!okay) {
            return BCM_E_INTERNAL;
        } 
...
_bcm_td2_port_lanes_set(int unit, bcm_port_t port_base, int lanes)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_trident2_port_lanes_set(unit, port_base, lanes, &cur_lanes,
                         ...
                         bcm_port_control_t type, int value)
{
...
#if defined(BCM_TRIUMPH3_SUPPORT)
            if (SOC_IS_TRIUMPH3(unit)) {
                rv = _bcm_tr3_wlan_port_set(unit, port, PORT_BRIDGEf, value);
            } else
#endif /* BCM_TRIUMPH3_SUPPORT */
            {
                ...
                if (BCM_SUCCESS(rv)) {
                    ...
                    if (value == 1) { /* Probe other ports */
                        for (i = port; i < port+4; i++) {
                            SOC_IF_ERROR_RETURN(soc_phyctrl_detach(unit, i));
                        }
                        for (i = port; i < port+4; i++) {
    ...
    case bcmPortControlPFCTransmit:
        ...
        if (soc_feature(unit, soc_feature_priority_flow_control)) {
            ...
            if (SOC_IS_TD2_TT2(unit)) {
                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, THDI_INPUT_PORT_XON_ENABLESr,
                                                    port, 0, &rval));
                soc_reg_field_set(unit, THDI_INPUT_PORT_XON_ENABLESr, &rval, 
                                PORT_PRI_XON_ENABLEf, pfc_enable ? 0xffff : 0);
                
                soc_reg_field_set(unit, THDI_INPUT_PORT_XON_ENABLESr, &rval,
                                      PORT_PAUSE_ENABLEf, pfc_enable ? 0 : 1);
                SOC_IF_ERROR_RETURN(soc_reg32_set(unit, THDI_INPUT_PORT_XON_ENABLESr, port, 0, rval));
    ...
    case bcmPortControlL2Learn:
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            BCM_IF_ERROR_RETURN(bcm_xgs3_port_cfg_get(unit, port, &port_cfg));
            port_cfg.pc_cml = hw_val;
            rv = bcm_xgs3_port_cfg_set(unit, port, &port_cfg);
        } 
#endif /* BCM_TRX_SUPPORT */
          break;
    ...
    case bcmPortControlPrbsTxInvertData:
        ...
        PORT_LOCK(unit);
        rv = soc_phyctrl_control_set(unit, port, 
                                     SOC_PHY_CONTROL_PRBS_TX_INVERT_DATA, value);
        PORT_UNLOCK(unit);
        break;
        ...
        if (soc_feature(unit, soc_feature_eee)) {
            ...
            if ((MAC_CONTROL_GET(PORT(unit, port).p_mac, unit, port,
                ...
                if (SOC_IS_TD2_TT2(unit)) {
                    ...
                    for (idx = 0; idx < sizeof(regs) / sizeof(regs[0]);
                         idx++) {
                        rv = soc_counter_set(unit, port, regs[idx], 0, val64);
                        if (SOC_FAILURE(rv)) {
                            break;
                        }
                    }
 ...
 */
    ...
    case bcmPortControlRemoteCpuTcMapping: 
        ...
        if (soc_feature(unit, soc_feature_rcpu_tc_mapping)) {
            ...
            if (BCM_SUCCESS(rv) && SOC_MEM_IS_VALID(unit, LPORT_TABm)) {
                rv = _bcm_esw_lport_tab_set(unit, port, REMOTE_CPU_ENf,
                                            (value) ? 1 : 0);
            }
        } else {
            rv = BCM_E_UNAVAIL;
    ...
    case bcmPortControlNonTrillAllow:
        if (BCM_GPORT_IS_WLAN_PORT(port)) {
         ...
         if (soc_feature(unit, soc_feature_trill)) {
              ...
              if (SOC_MEM_FIELD_VALID(unit, EGR_PORTm, ALLOW_NON_TRILL_FRAMESf) &&
                  BCM_SUCCESS(rv)) {
                   rv = soc_mem_field32_modify(unit, EGR_PORTm, port, 
                                       ALLOW_NON_TRILL_FRAMESf, (value) ? 1 : 0);
              }
    ...
    case bcmPortControlL2GreEnable:
        ...
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_TERMINATION_ALLOWEDf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                       L2GRE_TERMINATION_ALLOWEDf, (value) ? 1 : 0);
              }  
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
       break;
...
#if defined(BCM_TRIUMPH3_SUPPORT)
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_VPNID_LOOKUP_KEY_TYPEf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                      L2GRE_VPNID_LOOKUP_KEY_TYPEf, (value) ? 1 : 0);
              }
              if (SOC_MEM_FIELD_VALID(unit, EGR_PORTm, L2GRE_VFI_LOOKUP_KEY_TYPEf) &&
                  BCM_SUCCESS(rv)) {
                   rv = soc_mem_field32_modify(unit, EGR_PORTm, port, 
                                       L2GRE_VFI_LOOKUP_KEY_TYPEf, (value) ? 1 : 0);
              }
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
       break;
...
#if defined(BCM_TRIUMPH3_SUPPORT)
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_DEFAULT_SVP_ENABLEf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                      L2GRE_DEFAULT_SVP_ENABLEf, (value) ? 1 : 0);
              }  
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
       break;
    ...
    case bcmPortControlVxlanEnable:
        ...
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_TERMINATION_ALLOWEDf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                       VXLAN_TERMINATION_ALLOWEDf, (value) ? 1 : 0);
              }  
        }
#endif /* BCM_TRIDENT2_SUPPORT */
       break;
...
#if defined(BCM_TRIDENT2_SUPPORT)
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_VN_ID_LOOKUP_KEY_TYPEf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                      VXLAN_VN_ID_LOOKUP_KEY_TYPEf, (value) ? 1 : 0);
              }
              if (SOC_MEM_FIELD_VALID(unit, EGR_PORTm, VXLAN_VFI_LOOKUP_KEY_TYPEf) &&
                  BCM_SUCCESS(rv)) {
                   rv = soc_mem_field32_modify(unit, EGR_PORTm, port, 
                                       VXLAN_VFI_LOOKUP_KEY_TYPEf, (value) ? 1 : 0);
              }
        }
#endif /* BCM_TRIDENT2_SUPPORT */
    break;
...
#if defined(BCM_TRIDENT2_SUPPORT)
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_DEFAULT_SVP_ENABLEf)) {
                   rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_ETHER,
                                      VXLAN_DEFAULT_SVP_ENABLEf, (value) ? 1 : 0);
              }  
        }
#endif /* BCM_TRIDENT2_SUPPORT */
    break;
    ...
    case bcmPortControlSubportTagEnable:
        if (soc_feature(unit, soc_feature_subtag_coe) && SOC_IS_KATANA2(unit)) {
                         ...
                         bcm_port_control_t type, int *value)
{
...
#if defined(BCM_TRIUMPH3_SUPPORT)
            if (SOC_IS_TRIUMPH3(unit)) {
                rv = _bcm_tr3_lport_field_get(unit, port, PORT_BRIDGEf, value);
            } else
#endif /* BCM_TRIUMPH3_SUPPORT */
            {
    ...
    case bcmPortControlPreservePacketPriority:
        ...
        if (SOC_IS_TRIUMPH3(unit)) {
            /* Triumph3 has unity mapping to ING_OUTER_DOT1P_MAPPING_TABLE */
            rv = _bcm_esw_port_tab_get(unit, port, DOT1P_REMAP_POINTERf, value);
            if (BCM_SUCCESS(rv)) {
    ...
    case bcmPortControlCustomerQueuing:
        ...
        if (SOC_IS_KATANA(unit)) {
            ...
            if (SOC_REG_FIELD_VALID(unit, ING_COS_MODEr, COS_MODEf)) {
                *value = soc_reg_field_get(unit, ING_COS_MODEr, rval, 
                                           COS_MODEf);
                break;
            }    
        }    
    ...
    case bcmPortControlL2Learn:
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            rv = bcm_xgs3_port_cfg_get(unit, port, &port_cfg);              
            if (BCM_SUCCESS(rv)) {
 ...
 */
    ...
    case bcmPortControlNonEtagAdd:
         ...
         if (soc_feature(unit, soc_feature_port_extension)) {
             int etag_action;
             rv = _bcm_esw_port_tab_get(unit, port, 
                     VNTAG_ACTIONS_IF_NOT_PRESENTf, &etag_action);
             if (BCM_SUCCESS(rv)) {
    ...
    case bcmPortControlPrbsPolynomial:
        rv = soc_phyctrl_control_get(unit, port, 
                                     SOC_PHY_CONTROL_PRBS_POLYNOMIAL, (uint32 *)value);
        break;
    case bcmPortControlPrbsTxEnable:
        rv = soc_phyctrl_control_get(unit, port, 
                                     SOC_PHY_CONTROL_PRBS_TX_ENABLE, (uint32 *)value);
        break;

    case bcmPortControlPrbsRxEnable:
        rv = soc_phyctrl_control_get(unit, port, 
                                     SOC_PHY_CONTROL_PRBS_RX_ENABLE, (uint32 *)value);
        break;

    case bcmPortControlPrbsTxInvertData:
        rv = soc_phyctrl_control_get(unit, port, 
                                     SOC_PHY_CONTROL_PRBS_TX_INVERT_DATA, (uint32 *)value);
        break;
    ...
    case bcmPortControlL2GreEnable:
        ...
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_TERMINATION_ALLOWEDf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, L2GRE_TERMINATION_ALLOWEDf, value);
              }  
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
       break;
    ...
    case bcmPortControlL2GreTunnelbasedVpnId:
        ...
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_VPNID_LOOKUP_KEY_TYPEf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, L2GRE_VPNID_LOOKUP_KEY_TYPEf, value);
              }  
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
        break;
    ...
    case bcmPortControlL2GreDefaultTunnelEnable:
        ...
        if (soc_feature(unit, soc_feature_l2gre)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, L2GRE_DEFAULT_SVP_ENABLEf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, L2GRE_DEFAULT_SVP_ENABLEf, value);
              }  
        }
#endif /* BCM_TRIUMPH3_SUPPORT */
    ...
    case bcmPortControlVxlanEnable:
        ...
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_TERMINATION_ALLOWEDf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, VXLAN_TERMINATION_ALLOWEDf, value);
              }  
        }
#endif /* BCM_TRIDENT2_SUPPORT */
       break;
    ...
    case bcmPortControlVxlanTunnelbasedVnId:
        ...
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_VN_ID_LOOKUP_KEY_TYPEf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, VXLAN_VN_ID_LOOKUP_KEY_TYPEf, value);
              }
        }
#endif /* BCM_TRIDENT2_SUPPORT */
       break;
    ...
    case bcmPortControlVxlanDefaultTunnelEnable:
        ...
        if (soc_feature(unit, soc_feature_vxlan)) {
              if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VXLAN_DEFAULT_SVP_ENABLEf)) {
                   rv = _bcm_esw_port_tab_get(unit, port, VXLAN_DEFAULT_SVP_ENABLEf, value);
              }  
        }
#endif /* BCM_TRIDENT2_SUPPORT */
       break;
    ...
    case bcmPortControlMplsEntropyHashSet:
...
#endif /* BCM_TRIUMPH3_SUPPORT */
        {
             ...
             if (soc_feature(unit, soc_feature_mpls)) {
                rv = _bcm_esw_port_tab_get(unit, port, 
                                RTAG7_HASH_CFG_SEL_ENTROPY_LABELf, value);
             }
#endif /* BCM_KATANA_SUPPORT */
        }
         break;
    ...
    case bcmPortControlManagementPacketIp6Reserved:
        ...
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, MGMT_FRAME_ENABLEr, val, IPV6_RESVf);
        }
        PORT_UNLOCK(unit); 
        break;
    case bcmPortControlManagementPacketIp4Reserved:
        ...
        rv = READ_MGMT_FRAME_ENABLEr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, MGMT_FRAME_ENABLEr, val, IPV4_RESVf);
        }
        PORT_UNLOCK(unit); 
        break;
    case bcmPortControlManagementPacketLinkConstrained:
        ...
        rv = READ_MGMT_FRAME_ENABLEr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, MGMT_FRAME_ENABLEr, val, LINK_CONSTf);
        }
        PORT_UNLOCK(unit); 
        break;
    case bcmPortControlManagementPacketGarp:
        ...
        rv = READ_MGMT_FRAME_ENABLEr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, MGMT_FRAME_ENABLEr, val, DOT1D_GARPf);
        }
        PORT_UNLOCK(unit); 
        break;
    case bcmPortControlManagementPacketCos:
        ...
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, COSDP_REMAP_CONTROLr, val, MGMT_FRAME_COSf);
        }
        PORT_UNLOCK(unit); 
        break;
    case bcmPortControlManagementPacketColor:
    ...
    case bcmPortControlTaggedL3PacketPktPri:
        ...
        rv = READ_COSDP_REMAP_CONTROLr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, COSDP_REMAP_CONTROLr, val, SELECT5f);
            switch (*value) {
            case 0:
    ...
    case bcmPortControlTaggedMPLSPacketPktPri:
        ...
        rv = READ_COSDP_REMAP_CONTROLr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, COSDP_REMAP_CONTROLr, val, SELECT3f);
            switch (*value) {
            case 0:
                *value = BCM_PORT_COS_REMAP_DSCP;
            break;
    ...
    case bcmPortControlTaggedMPLSL3PacketPktPri:
        ...
        rv = READ_COSDP_REMAP_CONTROLr(unit, port, &val);
        if (BCM_SUCCESS(rv)) {
            *value = soc_reg_field_get(unit, COSDP_REMAP_CONTROLr, val, SELECT7f);
            switch (*value) {
            case 0:
                *value = BCM_PORT_COS_REMAP_DSCP;
            break;
...
#if defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    case bcmPortControlFieldEgressClassSelect:
        rv = _bcm_trx_port_control_egress_class_select_get(unit, port, (uint32 *)value);
        break;
#endif
#if defined(BCM_KATANA2_SUPPORT)
    case bcmPortControlOamLookupWithDvp:
        if (SOC_IS_KATANA2(unit)) {
            rv = _bcm_kt2_port_control_oam_loopkup_with_dvp_get(unit, port, 
                                                                value); 
        }
        break;
    case bcmPortControlSubportTagEnable:
                         ...
                         _bcm_port_config_t type, int value)
{
    ...
    case _bcmPortVlanTranslate:
        rv = _bcm_esw_port_tab_set(unit, port, _BCM_CPU_TABS_BOTH,
                                   VT_ENABLEf, value);
#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT) || defined(BCM_KATANA2_SUPPORT)
                             ...
                             bcm_port_phy_control_t type, uint32 value)
{
    ...
    PORT_LOCK(unit);
    rv = soc_phyctrl_redirect_control_set(unit, local_port, phyn, phy_lane, sys_side, type,  value);
    PORT_UNLOCK(unit);
                             ...
                             bcm_port_phy_control_t type, uint32 *value)
{
    ...
    PORT_LOCK(unit);
    rv = soc_phyctrl_redirect_control_get(unit, local_port, phyn, phy_lane, sys_side, type,  value);
    PORT_UNLOCK(unit);
...
bcm_esw_port_gport_is_free(int unit, bcm_gport_t gport)
{
    ...
    if (SOC_GPORT_IS_MPLS_PORT(gport) &&
 ...
 *      
 */
bcm_error_t _bcm_esw_port_stat_attach(
            int	        unit,
            bcm_gport_t	port,
            uint32      stat_counter_id)
{
 ...
 *
 */
bcm_error_t _bcm_esw_port_stat_detach(
            int	        unit,
            bcm_gport_t	port)
{
    uint32                     count=0;
 ...
 *
 */
bcm_error_t _bcm_esw_port_stat_counter_get( 
            int	             unit, 
            bcm_gport_t      port, 
...
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    if (soc_feature(unit,soc_feature_advanced_flex_counter)) {
        return _bcm_esw_port_stat_counter_get(unit,port,stat,
 ...
 *
 */
bcm_error_t _bcm_esw_port_stat_counter_set( 
            int	             unit, 
            bcm_gport_t      port, 
            bcm_port_stat_t  stat,
            uint32           num_entries,
     ...
     && defined(INCLUDE_L3))
        ...
        if (soc_feature(unit, soc_feature_extended_queueing)) {
            ...
            rv = _bcm_kt_cosq_port_resolve(unit, gport, &local_modid,
                                       &local_port, &extended_queue_id);
#else
            local_modid = 0;
            local_port = ((gport >> 16) & 0xFF);
            extended_queue_id = (gport & 0xFFFF);
#endif
...
#endif /* BCM_TRIDENT_SUPPORT && INCLUDE_L3 */
#if defined(BCM_TRIUMPH3_SUPPORT) && defined(INCLUDE_L3)
    else if (SOC_GPORT_IS_EXTENDER_PORT(gport)) {
        if (soc_feature(unit, soc_feature_port_extension)) {
...
#if defined(BCM_TRIUMPH3_SUPPORT)
        if (SOC_IS_TRIUMPH3(unit) && !soc_tr3_port_is_flex_port(unit, port)) {
            return BCM_E_PORT;
        } else 
#endif
        if (!_bcm_esw_valid_flex_port_controlling_port(unit, port)) {
    ...
    int *count)
{
    ...
    soc_mem_t mem;
#if defined(BCM_TRIUMPH3_SUPPORT)
    uint8 *vlan_xlate_extd_buf = NULL; /* VLAN_XLATE_EXTD DMA buffer */
    vlan_xlate_extd_entry_t *vlan_xlate_extd;
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        ...
        vlan_xlate_extd_buf = soc_cm_salloc(unit, 
                                  (soc_mem_index_count(unit, VLAN_XLATE_EXTDm) *
                                   sizeof(vlan_xlate_extd_entry_t)),
                                  "VLAN_XLATE_EXTD buffer");
        if (NULL == vlan_xlate_extd_buf) {
            rv = BCM_E_MEMORY;
            goto cleanup;
        }
        vt_buf = vlan_xlate_extd_buf;
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (SOC_IS_TRIUMPH3(unit)) {
            ...
            vlan_xlate_extd = soc_mem_table_idx_to_pointer
                         (unit, VLAN_XLATE_EXTDm, vlan_xlate_extd_entry_t *, 
                         vlan_xlate_extd_buf, i);

            /* double wide entry should always start at double wide boundary */
            key_type = soc_VLAN_XLATE_EXTDm_field32_get(unit, vlan_xlate_extd,KEY_TYPE_0f);
            ...
            mpls_action = soc_VLAN_XLATE_EXTDm_field32_get(unit, vlan_xlate_extd, 
                                   MPLS_ACTIONf);
            vp = soc_VLAN_XLATE_EXTDm_field32_get(unit, vlan_xlate_extd, 
                                   SOURCE_VPf);
            sal_memset(&vent, 0, sizeof(vent));
            rv = _bcm_tr3_vxlate_extd2vxlate(unit,vlan_xlate_extd, &vent,0);
            if (rv != BCM_E_NONE) {
                goto cleanup;
            }
            vlan_xlate = &vent;
...
#endif  /* BCM_TRIUMPH3_SUPPORT */
        {
            vlan_xlate = soc_mem_table_idx_to_pointer
                         ...
                         vlan_xlate_buf, i);
            mpls_action = soc_VLAN_XLATEm_field32_get(unit, vlan_xlate, 
                             MPLS_ACTIONf);
        ...
        key_type_value = (int)soc_VLAN_XLATEm_field32_get(unit, vlan_xlate, KEY_TYPEf);
    ...
    if (info != NULL) {
        ...
        if (soc_feature(unit, soc_feature_src_modid_blk_profile)) {
            ...
            if (PORT_SRC_MOD_EGR_PROFILE(unit) != NULL) {
                ...
                soc_cm_print("  Source Modid Egress Profile\n");
...
int bcm_esw_port_tdm_init(int unit)
{
     ...
     */
    *tdm_ctrl = (bcm_tdm_port_record_t*)sal_alloc(sizeof(bcm_tdm_port_record_t) * count,
                                                  "TDM port record");
    if ( *tdm_ctrl == NULL)
	return BCM_E_MEMORY;
...
void bcm_esw_port_tdm_cas_replacement_set(int unit, bcm_tdm_port_record_t *record) {
    ...
    (void)ag_nd_device_write(ctrl->ndHandle, AG_ND_OPCODE_CONFIG_CAS_DATA_REPLACE, 
                             &ndCasDataReplace);
}
    ...
    uint32 *cas_changed)
{
    ...
    n_ret = ag_nd_device_read(ctrl->ndHandle, AG_ND_OPCODE_STATUS_CAS_CHANGE, 
                              &ndStatusCasChange);
    ...
    uint8 *cas_abcd)
{
    ...
    n_ret = ag_nd_device_read(ctrl->ndHandle, AG_ND_OPCODE_STATUS_CAS_DATA, 
                              &ndStatusCasData);
    if (!AG_SUCCEEDED(n_ret))
    {
	return BCM_E_INTERNAL;
    ...
    int deactivation_code)
{
    ...
    n_ret = ag_nd_device_write(ctrl->ndHandle, AG_ND_OPCODE_FRAMER_PORT_LOOPBACK_CONTROL, 
                               &ndLoopbackControl);
    if (!AG_SUCCEEDED(n_ret))
    {
	return BCM_E_INTERNAL;
				      ...
				      AgFramerPortStatus *ndFramerPortStatus)
{
    ...
    n_ret = ag_nd_device_read(ctrl->ndHandle, AG_ND_OPCODE_FRMER_PORT_STATUS, 
                              ndFramerPortStatus);
    if (!AG_SUCCEEDED(n_ret))
    {
	return BCM_E_INTERNAL;
				      ...
				      AgFramerPortPm *ndFramerPortPm)
{
    ...
    n_ret = ag_nd_device_read(ctrl->ndHandle, AG_ND_OPCODE_FRAMER_PORT_PM_READ, 
                              ndFramerPortPm);
    if (!AG_SUCCEEDED(n_ret))
    {
	return BCM_E_INTERNAL;
