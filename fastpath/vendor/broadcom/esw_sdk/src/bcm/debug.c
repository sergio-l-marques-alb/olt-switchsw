...
uint32		bcm_debug_level = BCM_DBG_DEFAULT;
	       ...
	       int arg1, int arg2, int arg3, int rv)
{
    ...
    switch (ninargs) {
    case 0:
	BCM_DEBUG(BCM_DBG_API, ("API: %s(%s) -> %d %s\n",
				api,
				nargs > 0 ? "..." : "",
				rv, rvstr));
	break;
    case 1:
	BCM_DEBUG(BCM_DBG_API, ("API: %s(%d%s) -> %d %s\n",
				api, arg1,
				nargs > 1 ? ",..." : "",
				rv, rvstr));
	break;
    case 2:
	BCM_DEBUG(BCM_DBG_API, ("API: %s(%d,%d%s) -> %d %s\n",
				api, arg1, arg2,
				nargs > 2 ? ",..." : "",
				rv, rvstr));
	break;
    default:
	BCM_DEBUG(BCM_DBG_API, ("API: %s(%d,%d,%d%s) -> %d %s\n",
				api, arg1, arg2, arg3,
				nargs > 3 ? ",..." : "",
				rv, rvstr));
	break;
