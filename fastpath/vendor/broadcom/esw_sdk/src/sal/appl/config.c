/*
 * $Id: config.c 1.23.2.1 Broadcom SDK $
...
sal_config_refresh(void)
{
    ...
    while (fgets(str, sizeof(str), fp)) {
        ...
        *c = '\0';
        ...
        while (found_sc != NULL) {
            if (strcmp(found_sc->sc_name, sc->sc_name) == 0) {
                break;
            }
...
int
sal_config_save(char *fname, char *pat)
{
    int   rv = 0;
#ifndef SAL_CONFIG_FILE_DISABLE
    FILE  *new_fp;
    ...
    if ((new_fp = sal_fopen(fname, "w")) == NULL) {
        rv = -1;
        goto done;
    }
        ...
        if (wildcard != NULL) {
            ...
            *(sc_wildcard+length) = 0;
            for (i = 0; i < MAX_CONFIG_HASH_COUNT; i++) {
                ...
                psc = NULL;
                while (sc != NULL){
                    sc_wildcard = strncpy(sc_wildcard, sc->sc_name, length);
                    if (strcmp(sc_wildcard, wildcard) == 0) {
