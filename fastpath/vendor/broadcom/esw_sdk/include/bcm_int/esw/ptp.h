/*
 * $Id: ptp.h 1.1.2.40 Broadcom SDK $
 * 
...
#define PTP_ERROR(errmsg)      soc_cm_print("%s() FAILED %s\n", \
                               __func__, errmsg)
...
#define PTP_WARN(warnmsg)      soc_cm_print("%s() failed %s\n", \
                               __func__, warnmsg)
                               
#define PTP_WARN_FUNC(func)    soc_cm_print("%s() failed " \
                               func " returned %d : %s\n", \
                               __func__, rv, bcm_errmsg(rv))
...
/* Servo Redundancy Support bit. */
#define PTP_CLOCK_FLAGS_REDUNDANT_BIT                                  (5u)
/* Multi-TLV signaling message support bit. */
#define PTP_CLOCK_FLAGS_SIGNALING_MULTI_TLV_BIT                        (6u)
...
/* Smaller limits on peers & slaves for Keystone stack */
 ...
 /* Non-Keystone stack */
 #ifndef PTP_MAX_UNICAST_SLAVE_TABLE_ENTRIES
  #define PTP_MAX_UNICAST_SLAVE_TABLE_ENTRIES       (300)
 #endif /* PTP_MAX_UNICAST_SLAVE_TABLE_ENTRIES */
...
#define PTP_SERVO_OCXO_FREQ_OFFSET_HOURLY_FIXEDPOINT_SCALEFACTOR     (1000)
#define PTP_SERVO_OCXO_FREQ_OFFSET_FACTORY_FIXEDPOINT_SCALEFACTOR    (1000)
...
#else
/* Non-Keystone stack */
#define _PTP_MAX_PEER_DATASET_ENTRIES           150
#define _PTP_MAX_PEER_DATASET_CHUNK_SIZE        10
#define _PTP_MAX_FOREIGN_MASTER_DATASET_ENTRIES 10
#endif  /* PTP_KEYSTONE_STACK */
/* Peer Dataset */
typedef struct _bcm_ptp_peer_entry_s {
    uint16 localPortNum;
    ...
    uint64 tx_delayreqs;
    uint64 rx_delayreqs;
    uint64 tx_delayresps;
    uint64 rx_delayresps;
    uint64 tx_mgmts;
    uint64 rx_mgmts;
    uint64 tx_signals;
    uint64 rx_signals;
    uint64 rejected;
} _bcm_ptp_peer_entry_t;

typedef struct bcm_ptp_peer_dataset_s {
    uint16 num_peers;
    _bcm_ptp_peer_entry_t peer[_PTP_MAX_PEER_DATASET_ENTRIES];
} bcm_ptp_peer_dataset_t;

/* Query window statitics. */
typedef struct _bcm_ptp_window_stats_s {
    uint8 min;
    uint8 max;
    uint8 fallEvents;
    uint8 riseEvents;
} _bcm_ptp_window_stats_t;

/* Foreign Master Dataset */
typedef struct _bcm_ptp_foreign_master_entry_s {
    bcm_ptp_port_identity_t port_identity;
    bcm_ptp_clock_port_address_t address;
    uint8 clockClass;
    uint8 grandmasterPriority1;
    uint8 grandmasterPriority2;
    _bcm_ptp_window_stats_t clockClass_window;
    uint32 ms_since_sync;
    uint32 ms_since_sync_ts;  /* time since sync, if 1step, or followup, if 2step */
    uint32 ms_since_del_resp;
    uint16 announce_messages;
    uint64 pdv_scaled_allan_var;
} _bcm_ptp_foreign_master_entry_t;

typedef struct _bcm_ptp_foreign_master_dataset_s {
    uint8 num_foreign_masters;
    uint8 current_best_master;
    _bcm_ptp_foreign_master_entry_t foreign_master[_PTP_MAX_FOREIGN_MASTER_DATASET_ENTRIES];
} _bcm_ptp_foreign_master_dataset_t;

int _bcm_esw_ptp_foreign_master_dataset_get(int unit, bcm_ptp_stack_id_t ptp_id,
                                            int clock_num, int port_num,
                                            _bcm_ptp_foreign_master_dataset_t *data_set);
...
/* PTP signaling arbiter callback, for here now until signal API is approved */
typedef _bcm_ptp_callback_t (*_bcm_ptp_signaling_arbiter_t) (
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    int port_num,
...
/* PTP servo data types. */
typedef enum _bcm_ptp_osc_type_e {
    _bcm_ptp_osc_type_RB        = 0,   /* Rubidium Oscillator. */ 
    _bcm_ptp_osc_type_DOCXO     = 1,   /* Double oven OCXO.    */      
    _bcm_ptp_osc_type_OCXO      = 2,   /* OCXO.                */    
    _bcm_ptp_osc_type_mini_OCXO = 3,   /* Mini OCXO.           */    
    _bcm_ptp_osc_type_TCXO      = 4,   /* TCXO.                */
    
    /* 
     * Add to oscillator type value to indicate that Enduro clock is governed 
     * by the servo output, i.e., the frequency multiplier of the ToP/Enduro 
     * timestampers should be fixed at 1.0.
     */
    _bcm_ptp_flag_lock_enduro_freq_mult = 64,  
    
    /* 
     * Add to oscillator type value to have the system ignore any servo outputs.
     * NOTE : Debug only.
     */
    _bcm_ptp_flag_ignore_servo_output = 128    
} _bcm_ptp_osc_type_t;

typedef enum _bcm_ptp_transport_mode_e {
    _bcm_ptp_transport_mode_ethernet      = 0,   /* PTP Ethernet mode.      */
    _bcm_ptp_transport_mode_DSL           = 1,   /* PTP DSL mode.           */
    _bcm_ptp_transport_mode_microwave     = 2,   /* PTP microwave mode.     */
    _bcm_ptp_transport_mode_SONET         = 3,   /* PTP SONET mode.         */
    _bcm_ptp_transport_mode_slow_ethernet = 4,   /* PTP Slow Ethernet mode. */
    _bcm_ptp_transport_mode_high_jitter   = 5    /* PTP High Jitter.        */
} _bcm_ptp_transport_mode_t;
  
typedef struct _bcm_ptp_servo_config_s {
    /* Local oscillator type. */
    _bcm_ptp_osc_type_t osc_type;           
    /* PTP transport mode. */
    _bcm_ptp_transport_mode_t transport_mode;    
    /* Bit-mapped phase mode of servo loop. */
    uint32 ptp_phase_mode;       
    /* OCXO frequency offset sampled hourly. */
    int32 freq_corr_hourly;   
    /* Timestamp for hourly frequency offset.*/
    bcm_ptp_timestamp_t hourly_corr_time;    
    /* OCXO frequency offset at factory. */
    int32 freq_calib_factory; 
    /* Timestamp for factory frequency offset. */
    bcm_ptp_timestamp_t factory_calib_time;   
    /* Bridge time in nanoseconds. */
    uint32 bridge_time;          
} _bcm_ptp_servo_config_t;
...
typedef struct _bcm_ptp_servo_status_s {
    _bcm_ptp_fll_state_t fll_state; /* FLL state enumeration.    */
    uint32 fll_state_dur;           /* FLL state duration (sec). */
} _bcm_ptp_servo_status_t;
...
typedef struct _bcm_ptp_servo_performance_s {
    /* FLL state data. */
    _bcm_ptp_servo_status_t status;
    /* Forward flow weight. */
    uint64 fwd_weight;         
...
#define PTP_TELECOM_NETWORKING_OPTION_DEFAULT (_bcm_ptp_telecom_network_option_II)
...
#define PTP_TELECOM_PDV_SCALED_ALLAN_VARIANCE_THRESHOLD_NSECSQ_DEFAULT (1000000)
#define PTP_TELECOM_NON_REVERSION_MODE_DEFAULT                               (0)
#define PTP_TELECOM_WAIT_TO_RESTORE_SEC_DEFAULT                            (300)

/* Packet master state. */
typedef enum _bcm_ptp_telecom_pktmaster_state_e {
    _bcm_ptp_telecom_pktmaster_state_UNUSED = 0x00,
    _bcm_ptp_telecom_pktmaster_state_INIT   = 0x01,
    _bcm_ptp_telecom_pktmaster_state_VALID  = 0x02
} _bcm_ptp_telecom_pktmaster_state_t;

/* ITU-T G.781 synchronization networking options. */
typedef enum _bcm_ptp_telecom_network_option_e {
    _bcm_ptp_telecom_disable            = 0x00,
    _bcm_ptp_telecom_network_option_I   = 0x01,
    _bcm_ptp_telecom_network_option_II  = 0x02,
    _bcm_ptp_telecom_network_option_III = 0x03
} _bcm_ptp_telecom_network_option_t;

/* ITU-T G.781 quality level (QL). */
typedef enum _bcm_ptp_telecom_QL_e {
    _bcm_ptp_telecom_QL_INVALID = 0x00,
    /* ITU-T G.781 Option I synchronization networking */
    _bcm_ptp_telecom_QL_I_PRC   = 0x10,
    _bcm_ptp_telecom_QL_I_SSUA  = 0x11,
    _bcm_ptp_telecom_QL_I_SSUB  = 0x12,
    _bcm_ptp_telecom_QL_I_SEC   = 0x13,
    _bcm_ptp_telecom_QL_I_DNU   = 0x14,
    /* ITU-T G.781 Option II synchronization networking */
    _bcm_ptp_telecom_QL_II_PRS  = 0x20,
    _bcm_ptp_telecom_QL_II_STU  = 0x21,
    _bcm_ptp_telecom_QL_II_ST2  = 0x22,
    _bcm_ptp_telecom_QL_II_TNC  = 0x23,
    _bcm_ptp_telecom_QL_II_ST3E = 0x24,
    _bcm_ptp_telecom_QL_II_ST3  = 0x25,
    _bcm_ptp_telecom_QL_II_SMC  = 0x26,
    _bcm_ptp_telecom_QL_II_ST4  = 0x27,
    _bcm_ptp_telecom_QL_II_PROV = 0x28,
    _bcm_ptp_telecom_QL_II_DUS  = 0x29,
    /* ITU-T G.781 Option III synchronization networking */
    _bcm_ptp_telecom_QL_III_UNK = 0x30,
    _bcm_ptp_telecom_QL_III_SEC = 0x31,
    /* Not applicable (packet slave). */
    _bcm_ptp_telecom_QL_NA_SLV  = 0xff
} _bcm_ptp_telecom_QL_t;

/* Packet timing signal fail (PTSF) data. */
typedef struct _bcm_ptp_telecom_PTSF_s {
    uint8 lossAnnounce;
    uint8 lossSync;
    uint8 unusable;

    uint8 lossTimingSync;
    uint8 lossTimingSyncTS;
    uint8 lossTimingDelay;

    uint32 counter;
    sal_time_t timestamp;
} _bcm_ptp_telecom_PTSF_t;

/* PTP message receipt timeouts (msec). */
typedef struct _bcm_ptp_telecom_receipt_timeouts_s {
    uint32 announce;
    uint32 sync;
    uint32 delayResp;
} _bcm_ptp_telecom_receipt_timeouts_t;

/* PTP message elapsed times (msec). */
typedef struct _bcm_ptp_telecom_elapsed_times_s {
    uint32 announce;
    uint32 sync;
    uint32 syncTS;
    uint32 delayResp;
} _bcm_ptp_telecom_elapsed_times_t;

/* Packet timing and PDV statistics. */
typedef struct _bcm_ptp_telecom_pktstats_s {
    uint64 pdv_scaled_allan_var;
} _bcm_ptp_telecom_pktstats_t;

/* Packet master historical information and QL degradation data. */
typedef struct _bcm_ptp_telecom_hQL_s {
    _bcm_ptp_window_stats_t clockClass_window;
    uint8 degradeQL;
    uint32 counter;
    sal_time_t timestamp;
} _bcm_ptp_telecom_hQL_t;

/* Packet master priority. */
typedef struct _bcm_ptp_telecom_priority_s {
    uint8 override;
    uint16 value;
} _bcm_ptp_telecom_priority_t;

/* Packet master selector data. */
typedef struct _bcm_ptp_telecom_selector_s {
    uint8 lockout;
    uint8 non_reversion;
    uint8 wait_to_restore;

    sal_time_t wait_sec;
} _bcm_ptp_telecom_selector_t;

/* Packet master data and metadata. */
typedef struct _bcm_ptp_telecom_pktmaster_s {
    bcm_ptp_port_identity_t portIdentity;
    bcm_ptp_clock_port_address_t address;
    ...
    uint8 grandmasterPriority1;
    uint8 grandmasterPriority2;
    _bcm_ptp_telecom_priority_t priority;

    _bcm_ptp_telecom_selector_t selector;

    _bcm_ptp_telecom_PTSF_t PTSF;
    _bcm_ptp_telecom_elapsed_times_t elapsed_times;
    _bcm_ptp_telecom_pktstats_t stats;

    _bcm_ptp_telecom_hQL_t hQL;

    _bcm_ptp_telecom_pktmaster_state_t state;
    uint32 fmds_counter;
} _bcm_ptp_telecom_pktmaster_t;

/* Telecom profile data and metadata. */
typedef struct _bcm_ptp_telecom_profile_s {
    _bcm_ptp_telecom_network_option_t network_option;
    _bcm_ptp_telecom_receipt_timeouts_t receipt_timeouts;
    _bcm_ptp_telecom_pktstats_t thresholds;

    int previous_GM;
    int selected_GM;
    _bcm_ptp_telecom_pktmaster_t GM[PTP_TELECOM_MAX_NUMBER_PKTMASTERS];
} _bcm_ptp_telecom_profile_t;

/* Packet statistics data types. */
typedef struct _bcm_ptp_packet_counters_s {
    uint32 packets_transmitted;
    uint32 packets_received;
    uint32 packets_discarded;
    uint32 rcpu_encap_packets_received;
    uint32 ipv4_packets_received;
    uint32 ipv6_packets_received;
    uint32 l2_ptp_packets_received;
    uint32 udp_ptp_packets_received;
    uint32 enduro_sync_packets_transmitted;
    uint32 enduro_sync_packets_received;
    uint32 rx_queue_overflows;
    uint32 port_packets_transmitted[PTP_MAX_CLOCK_INSTANCE_PORTS];
    uint32 port_packets_received[PTP_MAX_CLOCK_INSTANCE_PORTS];
    uint32 port_packets_discarded[PTP_MAX_CLOCK_INSTANCE_PORTS];
} _bcm_ptp_packet_counters_t;
...
extern int _bcm_ptp_servo_configuration_get(
    int unit, 
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_servo_config_t *config);

extern int _bcm_ptp_servo_configuration_set(
    int unit, 
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    const _bcm_ptp_servo_config_t config);
...
extern int _bcm_ptp_servo_status_get(
    int unit, 
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_servo_status_t *status);

extern int _bcm_ptp_telecom_init(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num);

extern int _bcm_ptp_telecom_shutdown(
    void);

extern int _bcm_ptp_telecom_network_option_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_network_option_t *network_option);

extern int _bcm_ptp_telecom_network_option_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_network_option_t network_option);

extern int _bcm_ptp_telecom_quality_level_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_QL_t QL);

extern int _bcm_ptp_telecom_receipt_timeout_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
...
extern int _bcm_ptp_telecom_pktstats_thresholds_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_pktstats_t thresholds);

extern int _bcm_ptp_telecom_packet_master_list(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    int *num_masters,
...
extern int _bcm_ptp_telecom_packet_master_best_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_telecom_pktmaster_t *pktmaster);

extern int _bcm_ptp_telecom_packet_master_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    bcm_ptp_clock_port_address_t *address,
...
extern int _bcm_ptp_telecom_packet_master_add(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    bcm_ptp_clock_port_address_t *address);

extern int _bcm_ptp_telecom_packet_master_remove(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    bcm_ptp_clock_port_address_t *address);

extern int _bcm_ptp_telecom_packet_master_lockout_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    uint8 lockout,
...
extern int _bcm_ptp_telecom_packet_master_non_reversion_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    uint8 nonrev,
...
extern int _bcm_ptp_telecom_packet_master_wait_duration_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    sal_time_t wait_sec,
...
extern int _bcm_ptp_telecom_packet_master_priority_override(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    uint8 override,
...
extern int _bcm_ptp_telecom_packet_master_priority_set(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    uint16 priority,
...
extern int _bcm_ptp_telecom_map_QL_clockClass(
    const _bcm_ptp_telecom_QL_t QL,
    uint8 *clockClass);

extern int _bcm_ptp_telecom_map_clockClass_QL(
    const uint8 clockClass,
    _bcm_ptp_telecom_QL_t *QL);

extern int _bcm_ptp_packet_counters_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    _bcm_ptp_packet_counters_t *counters);
...
extern int _bcm_ptp_register_signaling_arbiter(
    int unit, 
    _bcm_ptp_signaling_arbiter_t arb,
    void *user_data);
...
_bcm_ptp_callback_t _bcm_ptp_default_signaling_arbiter(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    int port_num,
...
extern int _bcm_ptp_signaled_unicast_master_add(
    int unit, bcm_ptp_stack_id_t ptp_id, int clock_num, int port_num,
    bcm_ptp_clock_unicast_master_t *master_info, int log_annc_interval,
    int log_query_interval, uint32 flags);
extern int _bcm_ptp_signaled_unicast_master_remove( int unit, bcm_ptp_stack_id_t ptp_id, int clock_num, int port_num,
...
extern int _bcm_ptp_signaled_unicast_slave_list ( int unit, bcm_ptp_stack_id_t ptp_id, int *num_slaves, bcm_ptp_clock_peer_t *slave_table);
extern int _bcm_ptp_signaled_unicast_slave_clear( int unit, bcm_ptp_stack_id_t ptp_id, int clock_num, int port_num, int callstack);
...
extern int _bcm_ptp_construct_signaling_msg(bcm_ptp_port_identity_t *from_port, bcm_ptp_clock_identity_t to_clock,
                                            uint16 to_port, int domain, uint8* tlv_payload, int tlv_payload_len,
...
extern _bcm_ptp_callback_t _bcm_ptp_process_incoming_signaling_msg(
...
extern int _bcm_ptp_unicast_request_duration_get(uint32 *duration);
extern int _bcm_ptp_unicast_request_duration_set(uint32 duration);
extern int _bcm_ptp_unicast_request_duration_min_get(uint32 *duration_min);
extern int _bcm_ptp_unicast_request_duration_min_set(uint32 duration_min);
extern int _bcm_ptp_unicast_request_duration_max_get(uint32 *duration_max);
extern int _bcm_ptp_unicast_request_duration_max_set(uint32 duration_max);
...
/* For use by diag shell */
extern int _bcm_esw_ptp_input_channels_status_get(int unit,
...
extern int _bcm_ptp_update_peer_counts(uint16 localPortNum, bcm_ptp_clock_identity_t *clockID,
...
extern int _bcm_ptp_peer_dataset_get(int unit, 
                                bcm_ptp_stack_id_t ptp_id, 
                                int clock_num, 
                                int port_num,
...
extern int ptp_printf(const char *, ...);
