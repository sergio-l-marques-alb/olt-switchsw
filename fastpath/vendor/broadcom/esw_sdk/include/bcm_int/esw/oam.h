/*
 * $Id: oam.h 1.50.14.1 Broadcom SDK $
 * 
...
#define _BCM_OAM_KATANA2_ENDPOINT_MAX      16384
#endif
 ...
 */
#define OAM_DEBUG(flags, stuff)     BCM_DEBUG((flags) | BCM_DBG_OAM, stuff)
#define OAM_OUT(stuff)              BCM_DEBUG(BCM_DBG_OAM, stuff)
#define OAM_WARN(stuff)             OAM_DEBUG(BCM_DBG_WARN, stuff)
#define OAM_ERR(stuff)              OAM_DEBUG(BCM_DBG_ERR, stuff)
#define OAM_VERB(stuff)             OAM_DEBUG(BCM_DBG_VERBOSE, stuff)
#define OAM_VVERB(stuff)            OAM_DEBUG(BCM_DBG_VVERBOSE, stuff)
#define OAM_SHOW(stuff)             ((*soc_cm_print) stuff)
...
/* Group Name + Group ID + Endpoint Name + Level + Direction + Gport + 
                                                     VID/Label(inner and outer) */
#define _OAM_HASH_KEY_SIZE (BCM_OAM_GROUP_NAME_LENGTH  + 4 + 4 + 4 + 4 + 4 + 4 + 4)
...
struct _bcm_oam_hash_data_s {
    ...
    uint32              sw_flags;           /* Endpoint software flags.      */
    ...
    int                 lmep_tx_ctr1;       /* LMEP Tx counter id            */ 
    int                 lmep_tx_ctr2;
};
...
struct _bcm_oam_control_s {
    ...
    shr_aidxres_list_handle_t   ing_lm_ctr_pool[2];/* Ingress LM counter indices
                                                                       pool */
    shr_aidxres_list_handle_t   egr_lm_ctr_pool[2];/* Egress LM counter indices 
                                                                       pool */
