...
typedef enum bcm_fabric_link_threshold_type_e {
    bcmFabricLinkRxFifoLLFC,        /* the threshold that initiates link level
                                       flow control when the receiving FIFO is
                                       overloaded */
    bcmFabricLinkFE1TxBypassLLFC,   /* Only relevant for FE1 links. When the
                                       transmitting threshold is overloaded; the
                                       receiving FIFOs of all the FE1 links
                                       start generating link level flow control */
    bcmFabricLinkRciFC,             /* The TX FIFO threshold; that initiates the
                                       Route Congestion Indication flow control */
    bcmFabricLinkTxGciLvl1FC,       /* The TX FIFO threshold that initiates the
                                       Global Congestion Indication level 1 Flow
                                       Vontrol. */
    bcmFabricLinkTxGciLvl2FC,       /* The TX FIFO threshold that initiates the
                                       Global Congestion Indication level 2 Flow
                                       Vontrol. */
    bcmFabricLinkTxGciLvl3FC,       /* The TX FIFO threshold that initiates the
                                       Global Congestion Indication level 3 Flow
                                       Vontrol. */
    bcmFabricLinkRxGciLvl1FC,       /* The RX FIFO threshold that initiates the
                                       Global Congestion Indication level 1 Flow
                                       Control. */
    bcmFabricLinkRxGciLvl2FC,       /* The RX FIFO threshold that initiates the
                                       Global Congestion Indication level 2 Flow
                                       Control. */
    bcmFabricLinkRxGciLvl3FC,       /* The RX FIFO threshold that initiates the
                                       Global Congestion Indication level 3 Flow
                                       Control. */
    bcmFabricLinkTxPrio0Drop,       /* The TX FIFO threshold; that initiate the
                                       priority 0 packet dropping. */
    bcmFabricLinkTxPrio1Drop,       /* The TX FIFO threshold; that initiate the
                                       priority 1 packet dropping. */
    bcmFabricLinkTxPrio2Drop,       /* The TX FIFO threshold; that initiate the
                                       priority 2 packet dropping. */
    bcmFabricLinkTxPrio3Drop,       /* The TX FIFO threshold; that initiate the
                                       priority 3 packet dropping. */
    bcmFabricLinkRxPrio0Drop,       /*  The RX FIFO threshold that initiate the
                                       priority 0 packet dropping. */
    bcmFabricLinkRxPrio1Drop,       /*  The RX FIFO threshold that initiate the
                                       priority 1 packet dropping. */
    bcmFabricLinkRxPrio2Drop,       /*  The RX FIFO threshold that initiate the
                                       priority 2 packet dropping. */
    bcmFabricLinkRxPrio3Drop        /*  The RX FIFO threshold that initiate the
                                       priority 3 packet dropping. */
...
#define BCM_FABRIC_WATCHDOG_QUEUE_ENABLE_MAX BCM_FABRIC_WATCHDOG_QUEUE_ENABLE_FAST_STATUS_MESSAGE 
