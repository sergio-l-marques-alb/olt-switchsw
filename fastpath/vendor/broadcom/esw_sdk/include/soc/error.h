...
#define SOC_INIT_FUNC_DEFS \
    ...
    (void)_lock_taken; \
    _SOC_VVERB((_SOC_MSG("enter"))); 

#define SOC_FUNC_ERROR \
    SOC_FAILURE(_rv)

#define SOC_FUNC_RETURN \
    _SOC_VVERB((_SOC_MSG("exit"))); \
    return _rv;
...
#define _SOC_IF_ERR_EXIT(_rc) \
      do { \
        ...
        if(__err__rc != SOC_E_NONE) { \
            _SOC_ERR((_SOC_MSG("%s"),soc_errmsg(__err__rc))); \
            _rv = __err__rc; \
            SOC_EXIT; \
        } \
      } while(0)
...
#define SOC_NULL_CHECK(arg) \
    do {   \
        if ((arg) == NULL) \
        { _SOC_EXIT_WITH_ERR(SOC_E_PARAM,(_SOC_MSG("null parameter" ))); } \
    } while (0)
