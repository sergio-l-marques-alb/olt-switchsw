/*
 * $Id: drvtypes.h 1.58 Broadcom SDK $
...
typedef struct drv_wred_map_info_s drv_wred_map_info_t;

struct drv_wred_map_info_s {
    soc_port_t              port; /* port number */
    int                     cosq;
    uint32                  flags;
    drv_wred_map_info_t     *next;
};

#define DRV_WRED_MAP_FLAGS_FLOW_MARK        (0x1)
...
typedef struct drv_wred_config_s {
    ...
    int                     refer_count;
    drv_wred_map_info_t     *map_list;
} drv_wred_config_t;

/* Reserved for default WRED configuration */
#define DRV_WRED_CONFIG_FLAGS_DEFAULT_PROFILE   (0x1) 
