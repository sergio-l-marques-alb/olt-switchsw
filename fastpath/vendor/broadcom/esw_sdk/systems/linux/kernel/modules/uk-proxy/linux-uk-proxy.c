...
/*
 * $Id: linux-uk-proxy.c 1.15.44.1 Broadcom SDK $
...
static int
_list_op(luk_proxy_list_t *source_list, luk_proxy_list_t *dest_list, void *data, int *len, int flags)
{
    luk_proxy_data_t *dp; 
    int rv = 0;
    int err;
...
#define LDBG(l, str) gprintk("%d:%s:%s - %s\n", current->pid, l->service, l->name, str)
    ...
    if (flags & _LIST_OP_F_PUT) {
        ...
        if (flags & _LIST_OP_F_USER) {
            ...
            rv = copy_from_user(dp->data, data, *len);
	} else {
	...
	if (flags & _LIST_OP_F_USER) {
            ...
            rv = copy_to_user(data, dp->data, dp->len);
        } else {
 ...
 */
static int
_linux_uk_proxy_recv_from_user(const char *service, void *data, unsigned int len)
{
    int rc;
    ...
    rc = _list_op(&s->recv_q.free, &s->recv_q.data, data, (int*)&len, 
    ...
    if (service1[0]) {
        ...
        int len = LUK_MAX_DATA_SIZE;

        /* Get the packet from the incoming service */
        if (linux_uk_proxy_recv(service0, d, (unsigned int *)&len) < 0) {
            return 0;
        }
 ...
 */
static int
_linux_uk_proxy_send_to_user(const char *service, void *data, int *len)
{
    luk_proxy_service_t *s; 
    int rc;
...
_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    rv = copy_from_user(&io, (void*)arg, sizeof(io));
    ...
    rv = copy_to_user((void*)arg, &io, sizeof(io));
...
linux_uk_proxy_recv(const char *service, void *data, unsigned int *len)
{
    ...
    return _list_op(&s->recv_q.data, &s->recv_q.free, data, (int *)len, _LIST_OP_F_GET | _LIST_OP_F_KERN);
}
...
{      
    ...
    return _list_op(&s->send_q.free, &s->send_q.data, data, (int *)&len, _LIST_OP_F_PUT | _LIST_OP_F_KERN);
}
