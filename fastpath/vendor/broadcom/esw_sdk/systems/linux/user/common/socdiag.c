/*
 * $Id: socdiag.c 1.17.6.1 Broadcom SDK $
 ...
 */
int pci_dma_putw(pci_dev_t *dev, sal_vaddr_t addr, uint32 data)
{
    *(uint32 *) INT_TO_PTR(addr) = data;
    return 0;
}

int pci_dma_puth(pci_dev_t *dev, sal_vaddr_t addr, uint16 data)
{
    *(uint16 *) INT_TO_PTR(addr) = data;
    return 0;
}

int pci_dma_putb(pci_dev_t *dev, sal_vaddr_t addr, uint8 data)
{
    *(uint8 *) INT_TO_PTR(addr) = data;
    return 0;
}

uint32 pci_dma_getw(pci_dev_t *dev, sal_vaddr_t addr)
{
  uint32 data;
...
uint16 pci_dma_geth(pci_dev_t *dev, sal_vaddr_t addr)
{
  uint16 data;
...
uint8 pci_dma_getb(pci_dev_t *dev, sal_vaddr_t addr)
{
  uint8 data;
