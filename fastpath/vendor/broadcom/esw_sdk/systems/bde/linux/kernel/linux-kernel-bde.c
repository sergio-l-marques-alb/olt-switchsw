/*
 * $Id: linux-kernel-bde.c 1.370.2.3 Broadcom SDK $
...
#ifdef IPROC_CMICD
#define BCM_IPROC_CHIPCOMMONA_BASE 0x18000000
...
static int
_iproc_device_create (void) {

    bde_ctrl_t *ctrl;
    uint32 dev_rev_id;
    resource_size_t paddr = BCM_IPROC_CMICD_BASE;
    uint32 size = 0x40000;
    ...
    iproc_cca_base = (sal_vaddr_t)IOREMAP(BCM_IPROC_CHIPCOMMONA_BASE, 0x3000);
    if (!iproc_cca_base) {
        gprintk("_iproc_device_create: ioremap of ChipcommonA registers failed"); 
        return -1;
    }
    cca_cid = readl((uint32 *)(iproc_cca_base + 0x0000));
    cca_cid &= 0xffff;
    iounmap((void *)iproc_cca_base);
    ...
    if (!request_mem_region(paddr, size, "cmicd")) {
        gprintk("_iproc_device_create: Can't get cmicd memory region");
        return -1;
    }
    else {
        /* Map CMIC block in the AXI memory space into CPU address space */
        ctrl->bde_dev.base_address = (sal_vaddr_t)IOREMAP(paddr, size);
        if (!ctrl->bde_dev.base_address) {
            gprintk("_iproc_device_create: ioremap of cmic registers failed"); 
    ...
    /* Probe the AXI bus: There is one Switch device off of IPROC AXI bus */
    ...
    ctrl->isr = NULL;
    ctrl ->isr_data = NULL;
    gprintk("Created a iProc BCM switch device BA=%x IRQ=%d device=0x%x\n",
            paddr, ctrl->iLine, ctrl->bde_dev.device);
...
_pci_remove(struct pci_dev* dev)
{
    ...
    if (ctrl->isr != NULL) {
         ...
         free_irq(ctrl->pci_device->irq, ctrl);
...
static struct bde_spi_device_id _spi_id_table[] = {
    ...
    { BCM53020_PHYID_HIGH, BCM53020_PHYID_LOW ,0x240230, 0x110240, 0},
#ifdef NSP_FPGA_PLATFORM
    { BCM53020_FPGA_PHYID_HIGH, BCM53020_FPGA_PHYID_LOW ,0x0, 0x0, DEFAULT_FREQ},
#endif /* NSP_FPGA_PLATFORM */
    { 0, 0, 0, 0, 0 },
};
#endif
...
_dma_segment_alloc(size_t size, size_t blk_size)
{
    ...
    if ((dseg = kmalloc(sizeof(dma_segment_t), mem_flags)) == NULL) {
        return NULL;
    }
    ...
    dseg->blk_ptr = KMALLOC(blk_ptr_size, mem_flags);
...
probe_robo_switch(void)
{
    ...
    for (dev = 0; dev < max_devices; dev++) {
...
#ifdef NSP_FPGA_PLATFORM                
        if (phyidh == BCM53020_FPGA_PHYID_HIGH && phyidl_nr == BCM53020_FPGA_PHYID_LOW) {
            
            rev_id = BCM53020_A0_REV_ID;
            model_id = BCM53020_DEVICE_ID;
        }
#endif /* NSP_FPGA_PLATFORM */		
        ...
        if (NULL == (ctrl->spi_device = (struct spi_dev *)
                     KMALLOC(sizeof(struct spi_dev), mem_flags))) {
...
#if _SIMPLE_MEMORY_ALLOCATION_
static int lkbde_remap_mmap(struct file *filp, struct vm_area_struct *vma)
{
    unsigned long phys_addr = vma->vm_pgoff << PAGE_SHIFT;
    unsigned long size = vma->vm_end - vma->vm_start;
    unsigned long pool_end = _dma_pbase + (unsigned long)_dma_mem_size;
    if (phys_addr < (unsigned long)_dma_pbase || phys_addr + size > pool_end) {
        gprintk(LINUX_KERNEL_BDE_NAME " : requested physical address range 0x%lx-0x%lx not inside pool 0x%lx-0x%lx\n",
          phys_addr, phys_addr + size, (unsigned long)_dma_pbase, (unsigned long)_dma_pbase + _dma_mem_size);
        return -EINVAL;
    }
    if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff, size, vma->vm_page_prot)) {
        gprintk(LINUX_KERNEL_BDE_NAME " : failed to map phys range 0x%lx-0x%lx to 0x%lx-0x%lx\n", phys_addr, phys_addr + size, vma->vm_start,vma->vm_end);
        return -EAGAIN;
    }
    return 0;
}
...
static uint32_t
_iproc_offset(uint32_t map_no, uint32_t addr)
{
    _iproc_map_t *map;
...
_iproc_read(int d, uint32_t addr)
{
    ...
    if ((offset = _iproc_offset(0, addr)) == 0) {
        return -1;
    }
    addr -= offset;
