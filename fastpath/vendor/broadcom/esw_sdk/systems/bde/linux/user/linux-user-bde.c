/*
 * $Id: linux-user-bde.c 1.95.4.2 Broadcom SDK $
...
_read(int d, unsigned int addr)
{
    ...
    if (_devices[d]->dev_type & BDE_I2C_DEV_TYPE) {
        ...
        if (cpu_i2c_read(_devices[d]->i2c_addr, addr,
                         CPU_I2C_ALEN_LONG_DLEN_LONG, &data) != 0) {
            printf("linux-user-bde: warning: "
                   "cpu_i2c_read failed unit=%d addr=%x",
                   d, addr);
            return -1;
        }
        return data;
...
static uint32
_iproc_offset(uint32 map_no, uint32 addr)
{
    _iproc_map_t *map;
...
_bus_features(int unit, int *be_pio, int *be_packet, int *be_other)
{
    ...
    if ((_devices[unit]->bde_dev.device & 0xFF00) != 0x5600 &&
        ...
        (_devices[unit]->bde_dev.device != BCM88030_DEVICE_ID) &&
