/*
 * $Id: tigon3.c 1.3.348.1 Broadcom SDK $
...
PLM_DEVICE_BLOCK pDevice) {
    ...
    while(pPacket) {
        ...
        atomic_inc(&pDevice->RxPacketQueuedCnt);
        pPacket = (PLM_PACKET) QQ_PopHead(&pDevice->RxPacketFreeQ.Container);
    } /* while */
...
PLM_DEVICE_BLOCK pDevice)
{
    ...
    if(pDevice->ChipRevId != T3_CHIP_ID_5703_A1 &&
        pDevice->ChipRevId != T3_CHIP_ID_5703_B0 &&
...
/* No CPU Swap modes for PCI IO */    
    ...
    */
    ...
    if (pDevice->BondId != GRC_MISC_BD_ID_5700 &&
        ...
        pDevice->BondId != GRC_MISC_BD_ID_5703B0 &&
...
PLM_DEVICE_BLOCK pDevice)
{
    ...
    atomic_set(&pDevice->RxPacketQueuedCnt, 0);
...
PLM_DEVICE_BLOCK pDevice) {
    ...
    while(SwRcvRetConIdx != HwRcvRetProdIdx)
    {
        ...
        atomic_dec(&pDevice->RxPacketQueuedCnt);
...
#else
        /* 
        * jmb: it actually takes a couple of seconds for link pass,
        * so wait for it, but let someone else run while we wait if
        * we're in task context.  If in interrupt context, do it like
        * Linux.
        */
        if (INT_CONTEXT ())
            MM_Wait (40);
