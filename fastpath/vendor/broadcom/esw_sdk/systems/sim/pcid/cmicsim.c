/*
 * $Id: cmicsim.c 1.183.24.1 Broadcom SDK $
...
/*        PCIM(pcid_info, CMIC_SCHAN_CTRL) &= ~SC_MSG_NAK_TST; */
    ...
    case WRITE_REGISTER_CMD_MSG:
       if (pcid_info->opt_pli_verbose)
        ...
        rv = soc_internal_extended_write_reg(pcid_info, msg->header.dstblk,
                                             msg->header.srcblk,
                                             msg->writecmd.address,
                                             msg->writecmd.data); 
        break;
    case READ_REGISTER_CMD_MSG:
       if (pcid_info->opt_pli_verbose)
        ...
        rv = soc_internal_extended_read_reg(pcid_info, msg->header.dstblk,
                                            msg->header.srcblk,
                                            msg->readcmd.address,
                                            msg->readresp.data);
        break;
    case WRITE_MEMORY_CMD_MSG:
       if (pcid_info->opt_pli_verbose)
        ...
        rv = soc_internal_extended_write_mem(pcid_info, msg->header.dstblk,
                                             msg->header.srcblk,
                                             msg->writecmd.address,
                                             msg->writecmd.data); 
        break;
    case READ_MEMORY_CMD_MSG:
       if (pcid_info->opt_pli_verbose)
        ...
        rv = soc_internal_extended_read_mem(pcid_info, msg->header.dstblk,
                                            msg->header.srcblk,
                                            msg->readcmd.address,
                                            msg->readresp.data);
        break;
...
#endif    /* BCM_FIREBOLT_SUPPORT */
        debugk(DK_WARN, "Bad call for Non-XGS3: L3_INSERT_CMD_MSG\n");
        rv = -1;
        break;
...
#endif    /* BCM_FIREBOLT_SUPPORT */
        debugk(DK_WARN, "Bad call for Non-XGS3: L3_DELETE_CMD_MSG\n");
        rv = -1;
        break;
    ...
    case TABLE_INSERT_CMD_MSG:
        if (soc_feature(unit, soc_feature_generic_table_ops)) {
            ...
            if (SBUS_ADDR_VERSION_2 == pcid_info->sbus_addr_version) {
                ...
                mem = soc_addr_to_mem_extended(pcid_info->unit, msg->header.dstblk,
                                               msg->header.srcblk, address);
            } else {
...
soc_internal_xgs3_cmicm_table_dma(pcid_info_t *pcid_info)
{
    ...
    debugk(DK_VERBOSE,
           "Table DMA: schan_addr=0x%x block: %d acc_type=%d "
...
soc_internal_cmicm_sbusdma(pcid_info_t *pcid_info, uint32 reg)
{
    ...
    block = msg.header.dstblk;
    acc_type = msg.header.srcblk;
    ...
    mem = soc_addr_to_mem_extended(pcid_info->unit, block, acc_type, sbus_addr);
    assert(mem != INVALIDm);
    ...
    debugk(DK_VERBOSE,
           "SBUS DMA: mem=%d index=0x%x sbus_addr=0x%x block=%d acc_type=%d"
           "dma_cout=0x%x beats=%d sbus_incr=%d host_addr=0x%x host_incr=%d\n",
           mem, index, sbus_addr, block, acc_type, dma_count, beats, sbus_incr,
           host_addr, host_incr);

    if (msg.header.opcode == WRITE_MEMORY_CMD_MSG) {
        while (dma_count > 0) {
            /* Adjust the burst size at the end of the table */
                                ...
                                uint32 address, uint32 *data)
{
    ...
    if (SBUS_ADDR_VERSION_2 == pcid_info->sbus_addr_version) {
        ...
        if (!ainfo.valid || ainfo.reg < 0) {
            debugk(DK_WARN,
                   "Attempt to write to unknown S-channel "
                               ...
                               uint32 address, uint32 *data)
{
    ...
    if (SBUS_ADDR_VERSION_2 == pcid_info->sbus_addr_version) {
        ...
        if (!ainfo.valid || ainfo.reg < 0) {
            debugk(DK_ERR, 
                   "Attempt to read from unknown S-channel "
                   "block %d acc_type %d address 0x%x\n",
                   block, acc_type, address);
        } else if (SOC_REG_IS_VALID(pcid_info->unit, ainfo.reg) &&
            SOC_REG_INFO(pcid_info->unit, ainfo.reg).flags & SOC_REG_FLAG_WO) {
