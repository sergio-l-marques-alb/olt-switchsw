...
#if defined(L7_IPV6_PACKAGE) || defined(L7_IPV6_MGMT_PACKAGE)
        memset(&ipTrapTI6, 0, sizeof(ipTrapTI6));
        ipTrapTI6.type = SR_IP6_TRANSPORT;
        if (OpenTransport(&ipTrapTI6)) {
            DPRINTF((APWARN,
                  "%s: cannot open IPv6 transport traps.\n", Fname));
            initialized = -1;
            return -1;
        }
#endif
    ...
    const OctetString *contextName)
{
    ...
    case SR_IP6_TRANSPORT:
        DPRINTF((APTRAP, "%s: using IPv6 transport to %s\n", Fname,
                 FormatTransportString(tstr, sizeof(tstr), &ipTrapTI6)));

        memcpy(ipTrapTI6.t_ip6Addr, dstTI->t_ip6Addr, sizeof(ipTrapTI6.t_ip6Addr));
        if (dstTI->t_ipPort == 0) {
          ipTrapTI6.t_ip6Port = htons((short)GetSNMPTrapPort());
        } else {
