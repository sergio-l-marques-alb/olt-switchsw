    ...
    int             payload;
{
    ...
    char            LogBuffer[LOG_MSG_MAX_MSG_SIZE];
	...
	if ((i = FindObjectClass(curVb->name, EXACT, payload)) == -1) {
	    ...
	    osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, "Can't find object");
	    usmDbCmdLoggerSnmpEntryAdd(LogBuffer);
	    goto fail;
	}
	curObj = GetOidElem(i);
	...
	if (curObj->oidtype != curVb->value.type &&
	    ...
	    osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, " Value wrong type for object %s is being set ", curObj->oidname);
	    usmDbCmdLoggerSnmpEntryAdd(LogBuffer);
	    goto fail;
	}
...
#endif				/* SR_SNMPv2_PDU */
	    osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, " Attempt to set read-only object %s", curObj->oidname);
	    usmDbCmdLoggerSnmpEntryAdd(LogBuffer);
	    goto fail;
	}
	...
	if (cc != NO_ERROR) {
	    DPRINTF((APTRACE, "%s: test method returned error\n", Fname));
            error_status = cc;
	    osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, " Test method returned error for the object %s", curObj->oidname);
	    usmDbCmdLoggerSnmpEntryAdd(LogBuffer);
	    goto fail;
	}
    ...
    for (doListCur = doListHead; doListCur != NULL; doListCur = doListCur->next) {
        if (doListCur->state != SR_UNUSED && doListCur->state !=SR_ADD_MODIFY &&
	        ...
	        osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, " All related objects not set");
	        ...
	        osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, "Inconsistent name in value");
            } else if (doListCur->state == INCONSISTENT_VALUE_STATE) {
	        DPRINTF((APTRACE, "%s: Inconsistent value\n", Fname));
	        error_status = INCONSISTENT_VALUE_ERROR;
	        osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, "%s: Inconsistent value", Fname);
            } else {
	        DPRINTF((APTRACE, "%s: Unknown state\n", Fname));
	        error_status = INCONSISTENT_VALUE_ERROR;
	        osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE, "Unknown state");
            }
...
undo:
...
#else
        /*
         * LVL700130168:SNMP: undo functionality in snmp is broken
         * Changed the above for loop to a while loop.
         */

        temp = doListHead;
        while(temp != doListCur)  {
            if (temp->state == SR_NO_OP) {
                temp =  temp->next;
                continue;
            }
    ...
    osapiSnprintf(LogBuffer, LOG_MSG_MAX_MSG_SIZE," Set operation is performed on the object %s ", curObj->oidname);
    usmDbCmdLoggerSnmpEntryAdd(LogBuffer);
