    ...
    const OID *oidb;
{
  ...
  */
OID            *
CloneOID_track(oid_ptr, file, line)
    const OID *oid_ptr;
    char *file;
    unsigned int line;
{
    ...
    return MakeOID_track(oid_ptr->oid_ptr, oid_ptr->length, file, line);
}
  ...
  */
OID            *
ClonePartialOID_track(oid_ptr, start, len, file, line)
    const OID       *oid_ptr;
    int             start;
    int             len;
    char *file;
    unsigned int line;
{
    ...
    return MakeOID_track(&(oid_ptr->oid_ptr[start]),
		   MAX(0, MIN(len, oid_ptr->length - start)), file, line);
}
