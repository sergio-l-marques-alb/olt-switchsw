  ...
  if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
  {
    ...
    for (bcm_unit=0;bcm_unit < soc_ndev;bcm_unit++)
    {
      if (!BCM_IS_FABRIC(bcm_unit))
      {
        ...
        PT_LOG_TRACE(LOG_CTX_HAPI,"Group id %d will %s created (flags=0x%08x)",group,((create_group) ? "BE" : "NOT be"),flags);
        ...
        if (create_group)
        {
          ...
          PT_LOG_TRACE(LOG_CTX_HAPI,"Group id %d reated (with flags=0x%08x)",group,flags);
        }
        ...
        PT_LOG_TRACE(LOG_CTX_HAPI,"IPMC added");
      }
      else
      {
...
int usl_bcm_ipmc_delete (usl_bcm_ipmc_addr_t *ipmc, L7_uint32 keep)
{
  ...
  if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
  {
    for (bcm_unit=0;bcm_unit < soc_ndev;bcm_unit++)
    {
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"IPMC removed");
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"Group id %d destroyed",ipmc->ipmc_index);
...
L7_RC_t usl_bcm_ipmc_set_l2_ports (usl_bcm_ipmc_addr_t *ipmc_addr)
{
  ...
  if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
  {
    for (bcm_unit=0; bcm_unit < soc_ndev; bcm_unit++)
    {
      ...
      #if (SDK_VERSION_IS >= SDK_VERSION(6,0,0,0))
      PT_LOG_TRACE(LOG_CTX_HAPI,"Adding L2 ports");
      /* Add L2 ports */
      BCM_PBMP_ITER(ipmc_addr->l2_pbmp[modid], port)
      {
        ...
        PT_LOG_TRACE(LOG_CTX_HAPI,"Added bcm_port %d (gport=0x%08x)",port,gport);
      }
  ...
  for (i = 0; i < (sizeof(ipmc_cmd->ipmc_index_mask) / sizeof(ipmc_cmd->ipmc_index_mask[0])); i++)
  {
    ...
    for (index = (i * 32); index < ((i + 1) * 32); index++)
    {
      ...
      if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
      {
        ...
        PT_LOG_TRACE(LOG_CTX_HAPI,"Removed bcm_port %d (gport=0x%08x)",port,gport);
        #else
        rv = bcm_ipmc_get_by_index(unit, index, &ipmc);
        if (rv != BCM_E_NONE)
        {
                                  ...
                                  usl_bcmx_port_ipmc_cmd_t *ipmc_cmd)
{
  ...
  if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
  {
    for (i = 0; i < num_ipmc_groups; i++)
    {
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"Added egress port %d (gport=0x%08x)",port,gport);
                                     ...
                                     usl_bcmx_port_ipmc_cmd_t *ipmc_cmd)
{
  ...
  if (USL_BCM_CONFIGURE_HW(USL_IPMC_ROUTE_DB_ID) == L7_TRUE)
  {
    for (i = 0; i < num_ipmc_groups; i++)
    {
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"Removed egress port %d (gport=0x%08x)",port,gport);
      #else
      rv = bcm_ipmc_get_by_index(unit, index, &ipmc);
      if (rv != BCM_E_NONE)
      {
