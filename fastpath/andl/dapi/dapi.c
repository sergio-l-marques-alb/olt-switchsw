...
//  case DAPI_CMD_SYSTEM_DHCP_CONFIG:         /* For dealing with physical interfaces (not CPU)*/
    ...
    /* PTin end */
      ...
      {
        PT_LOG_TRACE(LOG_CTX_MISC, "Slot not valid: dapiUsp={%d,%d,%d}", dapiUsp.unit, dapiUsp.slot, dapiUsp.port);
      ...
      else if (dapi_g->unit[dapiUsp.unit]->slot[dapiUsp.slot]->cardPresent == L7_FALSE)
      {
        ...
        PT_LOG_TRACE(LOG_CTX_MISC, "Card not present: dapiUsp={%d,%d,%d}", dapiUsp.unit, dapiUsp.slot, dapiUsp.port);
      ...
      {
        PT_LOG_TRACE(LOG_CTX_MISC, "USP {%d,%d,%d} not valid", dapiUsp.unit, dapiUsp.slot, dapiUsp.port);
    ...
    case DAPI_CMD_CARD_INSERT:
      ...
      PT_LOG_TRACE(LOG_CTX_STARTUP,"DAPI_CMD_CARD_INSERT event");

      if (dapi_g->unit[dapiUsp.unit]->slot[dapiUsp.slot]->cardPresent == L7_FALSE)
      {
        PT_LOG_TRACE(LOG_CTX_STARTUP,"Going to insert card...");
        if (dapiCardInsert(&dapiUsp, cmd, data) == L7_FAILURE) {
          PT_LOG_ERR(LOG_CTX_STARTUP,"Error inserting card!");
          result = L7_FAILURE;
        }
        else {
          PT_LOG_NOTICE(LOG_CTX_STARTUP,"Success inserting card!");
...
L7_RC_t dapiCardInsert(DAPI_USP_t *dapiUsp, DAPI_CMD_t cmd, void *data)
{
  ...
  PT_LOG_TRACE(LOG_CTX_STARTUP,"Starting dapiCardInsert...");
...
/* Added to initialize all commands in cmdTable to L7_FAILURE */
  ...
  if ((dapiCardInfoPtr->hapiCardInsert)(&usp, cmd, data, dapi_g) == L7_FAILURE)
  {
    ...
    */
    ...
    PT_LOG_ERR(LOG_CTX_STARTUP,"Error inserting card: result=%d",result);
    return result;
  }

  PT_LOG_INFO(LOG_CTX_STARTUP,"dapiCardInsert: result=%d",result);
  return result;
}
...
L7_RC_t dapiCpuCardInsert(DAPI_USP_t *dapiUsp, DAPI_CMD_t cmd, void *data)
{
  ...
  PT_LOG_TRACE(LOG_CTX_STARTUP,"dapiCpuCardInsert starting...");
  ...
  PT_LOG_INFO(LOG_CTX_STARTUP,"dapiCpuCardInsert: result=%d",result);
...
/* PTin added: debug */
void familyCallbackListDebugShow(void)
{
  L7_int i;

  extern L7_RC_t dtlLinkChangeCallback(DAPI_USP_t *ddusp, DAPI_FAMILY_t family, DAPI_CMD_t cmd, DAPI_EVENT_t event, void *dapiEventInfo);
  extern L7_RC_t dtlPduReceiveCallback(DAPI_USP_t *ddusp, DAPI_FAMILY_t family, DAPI_CMD_t cmd, DAPI_EVENT_t event, void *dapiEventInfo);
  extern L7_RC_t dtlGenericCallback   (DAPI_USP_t *ddusp, DAPI_FAMILY_t family, DAPI_CMD_t cmd, DAPI_EVENT_t event, void *dapiEventInfo);
  extern L7_RC_t dtlFdbReceiveCallback(DAPI_USP_t *ddusp, DAPI_FAMILY_t family, DAPI_CMD_t cmd, DAPI_EVENT_t event, void *dapiEventInfo);
