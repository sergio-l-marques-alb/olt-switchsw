...
L7_RC_t dtlMulticastAddrAdd(L7_multicastForwList_t *pMcastForwEntry)
{
  ...
  nimGetIntfName(pMcastForwEntry->inIntIfNum, L7_SYSNAME, ifName);

  DTL_IP_TRACE("%s %d: %s : Dest IP Addr = %x  Source IP addr = %x OutIntIfMask = ??, inIntf = %u, %s\n",
                 __FILE__, __LINE__, routine_name,
...
/*                 pMcastForwEntry->outIntIfMask,    */
                 pMcastForwEntry->inIntIfNum, ifName);
