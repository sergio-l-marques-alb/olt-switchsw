                   ...
                   L7_uint32 prefixLen)
{
  ...
  DTL_IP_TRACE("%s %d: %s : IP Addr = %s, prefixLen = %d, intf = %u, %s\n",
                 __FILE__, __LINE__, routine_name,
                 addr_str, prefixLen, intIfNum, ifName);
...
EXT_API L7_RC_t dtlIpv6NeighEntryQuery(L7_ndpQuery_t *pNdpQuery)
{
  ...
  L7_uint32 lhs,lhd;
  L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
  nimGetIntfName(pNdpQuery->intIfNum, L7_SYSNAME, ifName);
  ...
  DTL_IP_TRACE("%s %d: %s : IP Addr =  %s, Intf %u, %s \n",
                 __FILE__, __LINE__, routine_name,
                 addr_str, pNdpQuery->intIfNum, ifName);
