...
L7_char8 *pStrErr_ipv6_AddrConflict = "Address conflict between specified IPv6 address and current configuration.";
...
L7_char8 *pStrErr_ipv6_Ipv6Ping_1 = "Use 'ping ipv6 {{<ipv6-address> [size <datagram-size>]} | {{{interface <%s>} | {tunnel <tunnel-id>} | {loopback <loopback-id>} } <link-local-address> [size <datagram-size>]}'.";
