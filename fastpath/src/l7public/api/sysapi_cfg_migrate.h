...
do{\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "DUMMY Version %d %d Size %d\n", (L7_int32)oldVer, (L7_int32)ver, (L7_int32)newSize);\
}while(0);

#define MIGRATE_FUNCTION(oldVer,ver,newSize)\
do{\
  /* L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "IMPLEMENTED Version %d %d Size %d\n",oldVer, ver, newSize); */\
}while(0);

#if 1
#define MIGRATE_INVALID_SLOT_PORT(slot,port)\
do{\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "Invalid slot %d or port %d or both\n",(L7_int32)slot, (L7_int32)port);\
}while(0);
...
do{\
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "Failed to get interface info for index %d, %s\n", (L7_int32)index, ifName);\
}while(0);
#else
...
do{\
  L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];\
  nimGetIntfName(index, L7_SYSNAME, ifName);\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "intf %d, %s is out of bound\n", (L7_int32)index, ifName);\
}while(0);

#define MIGRATE_INCOMPLETE()\
do{\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "This is incomplete: Check Back\n");\
}while(0);

#define MIGRATE_ONLY_VERSION(oldVer,ver,newSize)\
do{\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "IGNORE Version only changed: Version %d %d Size %d\n", (L7_int32)oldVer, (L7_int32)ver, (L7_int32)newSize);\
}while(0);

#define MIGRATE_LOG_INTERFACE_TRUNCATE(file, intfCfgIndex)\
do{\
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "File %s: Forced truncation of interface configuration migration at %d entries\n", file, (L7_int32)intfCfgIndex);\
}while(0);

#define MIGRATE_LOG_UNEXPECTED_VERSION(cur,exp)\
do{\
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "Unexpected Header Version= %d expecting %d\n", (L7_int32)cur, (L7_int32)exp);\
}while(0);
...
do{\
  ...
  {\
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "[%s: %d]: Configuration Migration Assertion\n", _filename, __LINE__);\
  }\
}while(0);
...
do{\
  ...
  {\
    L7_char8 * _filename = __FILE__; \
    l7utilsFilenameStrip(&_filename); \
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "[%s: %d]: Data Sizes (%d-%d) are DIFFERENT\n", _filename, __LINE__, \
