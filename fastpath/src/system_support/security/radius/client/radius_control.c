...
L7_RC_t radiusStartTasks()
{
  ...
  if( clusterSupportGet() == L7_SUCCESS)
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS_CLUSTER: Cluster component present.\n");
...
void radiusDispatchCmd(radiusMsg_t *msg)
{
  ...
  switch (msg->event)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: Unknown message event %u\n", msg->event);
  }
                                     ...
                                     L7_RADIUS_SERVER_CONFIGPARAMS_FLAG_t paramStatus)
{
  ...
  else /* server specific setting*/
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventMaxNoOfRetransSet(): server entry not found \n");
      return L7_FAILURE;
    }
                                      ...
                                      L7_RADIUS_SERVER_CONFIGPARAMS_FLAG_t paramStatus)
{
  ...
  else /* server specific setting*/
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventTimeOutDurationSet(): server entry not found \n");
      return L7_FAILURE;
    }
    ...
    L7_uint32 serverType, L7_char8 *serverName)
{
  ...
  if (serverType == RADIUS_SERVER_TYPE_AUTH)
  {
    ...
    {
       L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
               "RADIUS: radiusEventServerHostNameAdd(): Failed to set the name \
                     to the server\n");
       return L7_FAILURE;
    }
  ...
  else
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: %s() Failed to select the new Acct. current server\n",__FUNCTION__);
      return L7_FAILURE;
    }
  }
  radiusCfg.hdr.dataChanged = L7_TRUE;
    ...
    L7_uint32 type)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusEventServerHostNameRemove(): Failed to delete \
                     the server entry \n");
    return L7_FAILURE;
  }
...
L7_RC_t radiusEventAllServerRemove(void)
{
  ...
  while((serverEntry = radiusServerEntries) != L7_NULL)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventAllServerRemove(): Failed to delete the server \
                     entry \n");
      return L7_FAILURE;
    }
                                         ...
                                         L7_RADIUS_SERVER_CONFIGPARAMS_FLAG_t paramStatus)
{
  ...
  { /* global parameter */
          ...
          serverEntry = serverEntry->nextEntry)
    {
      if ((serverEntry->serverConfig.localConfigMask & L7_RADIUS_SERVER_CONFIG_SECRET) == 0)
      {
        ...
        if (strlen(serverEntry->serverConfig.radiusServerConfigSecret) > 0)
        {
          serverEntry->serverConfig.radiusServerIsSecretConfigured = L7_TRUE;
        }
        else
  ...
  else
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventServerSharedSecretSet(): server entry not found \n");
      return L7_FAILURE;
    }
    ...
    L7_uint32 entryType)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS(TypeSet): %s : Invalid inputs\n",__FUNCTION__);
    return L7_FAILURE;
  }
  ...
  if ( L7_NULLPTR == serverEntry )
  {
    /* No entry with serverIPAddr was found */
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS(TypeSet): %s : Specified Server Not Found\n",__FUNCTION__);
    return L7_FAILURE;
  }
  ...
  if(L7_RADIUS_SERVER_ENTRY_TYPE_PRIMARY == entryType)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: %s() : Failed to fetch Existing Primary SErver\n",
                     __FUNCTION__);
      return rc;
    }
    ...
    radiusPacket_t *packet)
{
  ...
  RADIUS_DLOG(RD_LEVEL_INFO,"%s(): Notifying registered component, ID: %d, %s status: %d correlator: %d",
                     __FUNCTION__,requestInfo->componentID,name,requestInfo->status,
 ...
 *          preferably the PRIMARY server in all the named servers group
 *          starting from given serverNameIndex in an increasing order
 *          fashion.
 *
 ...
 *
 *********************************************************************/
static radiusServerEntry_t *radiusAllServerSelect(L7_uint32 serverType, L7_RADIUS_SERVER_USAGE_TYPE_t usageType,
                                                    L7_uint32 serverNameIndex)
{
  radiusServerEntry_t *nextServerEntry = L7_NULL, *rvEntry = L7_NULLPTR, *currentEntry = L7_NULLPTR;
  L7_char8 serverName[L7_RADIUS_SERVER_NAME_LENGTH + 1];
  L7_RC_t rc = L7_FAILURE;

  if (serverType == RADIUS_SERVER_TYPE_AUTH)
  {
    if (radiusServerNameEntryGet(serverNameIndex, serverName) != L7_SUCCESS)
    {
      return L7_NULLPTR;
    }
    if (osapiStrncmp(serverName, L7_RADIUS_SERVER_DEFAULT_NAME_AUTH,
                      L7_RADIUS_SERVER_NAME_LENGTH) == 0)
    {
      memset(serverName, L7_NULL, sizeof(serverName));
    }
    rc = radiusAuthServerNameEntryOrderlyGetNext(serverName, serverName, &nextServerEntry);
  }
  else
  {
    if (radiusAcctServerNameEntryGet(serverNameIndex, serverName) != L7_SUCCESS)
    {
      return L7_NULLPTR;
    }

    if (osapiStrncmp(serverName, L7_RADIUS_SERVER_DEFAULT_NAME_ACCT,
                      L7_RADIUS_SERVER_NAME_LENGTH) == 0)
    {
      memset(serverName, L7_NULL, sizeof(serverName));
    }
    rc = radiusAcctServerNameEntryOrderlyGetNext(serverName, serverName, &nextServerEntry);
  }

  while (rc == L7_SUCCESS)
  {
    /* Skip radius servers with Default server name */
    if ((serverType == RADIUS_SERVER_TYPE_AUTH &&
         osapiStrncmp(serverName, L7_RADIUS_SERVER_DEFAULT_NAME_AUTH,
                      L7_RADIUS_SERVER_NAME_LENGTH) != 0)||
        (serverType == RADIUS_SERVER_TYPE_ACCT &&
         osapiStrncmp(serverName, L7_RADIUS_SERVER_DEFAULT_NAME_ACCT,
                      L7_RADIUS_SERVER_NAME_LENGTH) != 0))
    {
      serverNameIndex = nextServerEntry->serverNameIndex;
      radiusServerAuthCurrentEntryGet(serverNameIndex, usageType, &currentEntry);
      if (currentEntry != L7_NULLPTR)
      {
        return currentEntry;
      }
      if ((rvEntry = radiusServerSelect(serverType, usageType, serverNameIndex)))
      {
        return rvEntry;
      }
    }
  ...
  if (rc != L7_SUCCESS)
  {
    nextServerEntry = L7_NULLPTR;
  }

  return nextServerEntry;
}
                     ...
                     L7_uint32 serverNameIndex)
{
  ...
  *selectedEntry = L7_NULL;
  ...
  RADIUS_DLOG(RD_LEVEL_INFO,"%s(): entered into the function..index %d",__FUNCTION__, serverNameIndex);

  if (serverNameIndex < L7_RADIUS_AUTH_SERVER_NAME_INDEX_MIN ||
      serverNameIndex  > L7_RADIUS_MAX_AUTH_SERVERS )
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusServerSelect(): Inavalid servernameIndex: %d",
            serverNameIndex);
    return L7_NULL;
  }
      ...
      serverEntry = serverEntry->nextEntry)
  {
                     ...
                     RADIUS_SERVER_DESTROY)
    {
                     ...
                     RADIUS_SERVER_TYPE_AUTH)
      {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
                    "RADIUS: radiusServerEntryDelete(): Failed to delete server\n");
            break;
          }
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
                    "RADIUS: %s(): Failed remove the server \
      ...
      else
      {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
                    "RADIUS: radiusServerEntryDelete(): Failed remove the \
...
L7_RC_t radiusConfigChangePropagate(L7_uint32 eventType, L7_uint32 serverNameIndex)
{
  ...
  if (eventType == RADIUS_CONFIG_EVENT_TYPE_MODIFY_AUTH)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: %s(): Failed to get current \
...
L7_RC_t radiusResponsePacketProcess(L7_uint32 serverAddress, radiusPacket_t *packet)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusResponsePacketProcess: Server not found \
  ...
  if (request == L7_NULL)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "%s(): Request not found for the response! Dropped the packet with ID:%d.",
...
L7_RC_t radiusClientShutdown(void)
{
  ...
  for (serverEntry = radiusServerEntries; serverEntry != L7_NULL; serverEntry = nextEntry)
  {
    ...
    {
       L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
               "RADIUS: radiusClientShutdown(): Failed remove the server \
                         name from the server entry\n");
    }
        ...
        */
        if (request->retries++ < serverEntry->serverConfig.maxNumRetrans) /* regardless of global/local setting, this is the value in use */
        {
          ...
          continue;
                 ...
                 (request->tryAnotherServer == L7_NULL && request->requestInfo->tryAllServers == L7_TRUE))
        {
          ...
          if (request->tryAnotherServer == L7_NULL)
          {
            nextServerEntry = radiusAllServerSelect(serverEntry->serverConfig.radiusServerConfigServerType,
                                                    usageType, serverEntry->serverNameIndex);
          }
          else
          {
            /*
            ...
            */
            ...
            nextServerEntry = radiusServerSelect(serverEntry->serverConfig.radiusServerConfigServerType,
                                                  usageType, serverEntry->serverNameIndex);
          }
          ...
          if (nextServerEntry)
          {
            ...
            */
            ...
            else
            {
              if (serverEntry->serverConfig.radiusServerConfigServerType == RADIUS_SERVER_TYPE_AUTH)
              {
                request->tryAnotherServer = radiusAuthActiveServerCountGet(nextServerEntry->serverNameIndex, usageType);
              }
              else
              {
               /* There are no fallback servers for accounting */
                request->tryAnotherServer = 1;
              }
            }
...
{
  if( index > L7_RADIUS_MAX_AUTH_SERVERS)
  {
    RADIUS_DLOG(RD_LEVEL_INFO,"RADIUS: %s() Invalid Parameter: \
                index=%d ",__FUNCTION__, index);
    return L7_FAILURE;
  }
...
L7_RC_t radiusAcctServerNameEntryGet(L7_uint32 index,L7_char8 *serverName)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusAcctServerNameEntryGet(): No RADIUS Acct. server \
...
L7_RC_t radiusServerAuthCurrentEntrySet(L7_uint32 index, L7_RADIUS_SERVER_USAGE_TYPE_t usageType, radiusServerEntry_t *serverEntry)
{
 ...
 /* serverEntry can also be NULL since there are situations
  ...
  if(radiusServerNameEntryGet(index,serverName) == L7_SUCCESS )
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusServerAuthCurrentEntrySet(): Invalid usageType\n");
      return L7_FAILURE;
    }
    return L7_SUCCESS;
  }
...
L7_RC_t radiusAcctServerNameAdd(radiusServerEntry_t *serverEntry, L7_char8 *serverName)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusAcctServerNameAdd(): All of the name entries\
                               ...
                               L7_RADIUS_SERVER_CONFIGPARAMS_FLAG_t paramStatus)
{
  ...
  else /* server specific setting*/
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventDeadtimeSet(): server entry not found \n");
      return L7_FAILURE;
    }
                               ...
                               L7_uint32  paramStatus)
{
  ...
  else /* server specific setting*/
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusEventSourceIPSet(): server entry not found \n");
      return L7_FAILURE;
    }
                     ...
                     L7_char8 *serverName)
{
  ...
  if(radiusServerNameEntryGet(serverEntry->serverNameIndex,sName)== L7_SUCCESS )
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusAuthServerNameSet(): Trying to set the same \
                     name to server\n");
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
              "RADIUS: radiusAuthServerNameSet(): Failed to detach the server from\
                     current group");
      return L7_FAILURE;
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusAuthServerNameSet(): Failed to set the name to the server\n");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RADIUS_COMPONENT_ID,
            "RADIUS: radiusAuthServerNameSet(): Failed to select the new current Auth. server\n");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
