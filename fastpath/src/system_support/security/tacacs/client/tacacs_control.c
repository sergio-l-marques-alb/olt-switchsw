...
static void tacacsTimerCallback()
{
  ...
  if (!tacacsQueue)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "tacacsTimerCallback(): tacacsQueue has not been created!\n");
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: timer event send failed.\n");
  }
...
static void tacacsTimeoutProcess(void)
{
  ...
  for (i = 0; i < L7_TACACS_MAX_AUTH_SESSIONS; i++)
  {
    ...
    if ((session->sessionId != 0) &&
        ((session->state == TACACS_AUTH_STATE_CONNECT_WAIT)  || (session->state == TACACS_AUTH_STATE_RESPONSE_WAIT) ||
        (session->state == TACACS_AUTH_STATE_READY_TO_SEND)) )
    {
      if (session->connectTimer != 0)
      {
        ...
        if (session->connectTimer == 0)
        {
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: connection timeout to server %s.", osapiInet_ntoa(session->server));
          tacacsTimerDelete();
...
void tacacsConnectTask(L7_uint32 argc, void *argv)
{
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: Failed to send server connect message.\n");
  }
  return;
}
