...
sysapiCfgFileRel4_0CfgEntryAdd (L7_char8 * oldCfgFile, L7_uchar8 * pCfgData, L7_uint32 cfgLen)
{
  ...
  if (pCfgData == L7_NULL || cfgLen == 0)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "Failed to get sizeof Config file %s.\n", oldCfgFile);
    return L7_NULL;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "Config file %s size %u is too small.\n", oldCfgFile, cfgLen);
    return L7_NULL;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "sysapiCfgFileIndexGet (%s) failed\n", oldCfgFile);
    return L7_NULL;
  }
                               ...
                               L7_uint32 * pCfgLen)
{
  ...
  *pCfgLen = 0;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "Failed to read config file %s data %u\n", cfgFile, length);
    return L7_ERROR;
  }
...
sysapiCfgFileUncombinedToCombinedCfgFileConvert (void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "sysapiCfgFileMapInterfaces failed\n");
    (void) osapiSemaGive (sysapiCfgSema);
    return L7_ERROR;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "cfgFileCount > SYSAPI_MAX_FILE_ENTRIES\n");
    (void) osapiSemaGive (sysapiCfgSema);
    return L7_ERROR;
  }
                                       ...
                                       L7_uint32 * configIdOffset, L7_INTF_TYPES_t * intfType)
{
  ...
  /* L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID, "unknown configId for index %u\n", index); */
  return L7_FAILURE;
}
...
sysapiCfgFileRel4_0Separate (L7_char8 * bigCfn)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
          "sysapiCfgFileRel4_0Separate: Allocating %u\n", bufsz);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "CRC check failed. 0x%x read and 0x%x calculated\n", read_checksum, checksum);
    osapiFree (L7_SIM_COMPONENT_ID, buf);
    return L7_ERROR;
  }
  ...
  for (i = 0; i < num_cfg_files; i++)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
              "Config %s is separated from %s length = %u\n", filename, bigCfn, file_size);
      if (L7_NULL == sysapiCfgFileRel4_0CfgEntryAdd (filename, buf_ptr, file_size))
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "sysapiCfgFileRel4_0CfgEntryAdd(%s) Failed.\n", filename);
      }
    }
    else
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
              "Config %s is not separated from %s as we don't need\n", filename, bigCfn);
    }
    buf_ptr += file_size;
  }
  osapiFree (L7_SIM_COMPONENT_ID, buf);
