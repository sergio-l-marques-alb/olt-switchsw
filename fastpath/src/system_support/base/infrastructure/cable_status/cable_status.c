                       ...
                       L7_clocktime           *timestamp)
{
  ...
  /* If the port has SFP capability, it may support a copper SFP. */
  if ((nimGetIntfPhyCapability(intIfNum, &phyCapability) != L7_SUCCESS) || 
      (nimGetIntfConnectorType(intIfNum, &connType) != L7_SUCCESS))
  {
    return L7_ERROR;
  }
  if ((connType != L7_RJ45) && ((phyCapability & L7_PHY_CAP_PORTSPEED_SFP) == 0))
  {
  ...
  if ( L7_SUCCESS != rc )
  {
    /* Mark result specifying test was not performed */
    if (rc == L7_NOT_SUPPORTED)
    {
      cableStatusIntfData[intIfNum].tested = L7_FALSE;
    }
    return( rc );
  }

  /*osapiUTCTimeGet( &cableStatusIntfData[intIfNum].copper.timestamp );*/
  /* Setting local time instead of UTC. */
  cableStatusIntfData[intIfNum].copper.timestamp.seconds = simAdjustedTimeGet();
