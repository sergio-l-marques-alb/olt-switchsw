  ...
  do
  {
    ...
    switch ( request )
    {
      ...
      case L7_CNFGR_RQST_I_PHASE2_START:
        sysapiHpcStackEventRegisterCallback(L7_CARDMGR_COMPONENT_ID,
                                            HPC_EVENT_CARD_INSERT_FAILURE,
                                            cmgrCardInsertFailureCallback);
...

{
  ...
  do
  {
    ...
    if (cmgrRC != L7_SUCCESS)
    {
      *pReason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CARDMGR_COMPONENT_ID,
              "cmgrCnfgrInitPhase2Process(): Could not register with HPC events callback \n");
      break;
    }
    ...
    if (cmgrRC != L7_SUCCESS)
    {
      *pReason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CARDMGR_COMPONENT_ID,
              "cmgrCnfgrInitPhase2Process(): Could not register with HPC receive callback \n");
      break;
    }
    ...
    if (cmgrRC != L7_SUCCESS)
    {
      *pReason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CARDMGR_COMPONENT_ID,
              "cmgrCnfgrInitPhase2Process(): Could not register with nvStore \n");
      break;
    }
    ...
    L7_CNFGR_ERR_RC_t *pReason )
{
  ...
  *pResponse = L7_CNFGR_CMD_COMPLETE;
  ...
  cmgrAsyncCardStart(L7_CMGR_STARTUP_CREATE);
  ...
  cmgrAsyncCardStart(L7_CMGR_STARTUP_ACTIVATE);
...
/*********************************************************************
 *
 * @purpose  Send Startup notifications
 *
 * @param    L7_uint32 startAction  startup command type
 *
 * @returns
 *
 * @notes    This API is called when a startup event needs to be sent
 *           to the insert/remove task
 *
 * @end
 *********************************************************************/
void cmgrAsyncCardStart(L7_uint32 startAction)
{
  cmgrWorkerMsg_t cmd;
  L7_RC_t rc;

  cmd.event = startAction;
