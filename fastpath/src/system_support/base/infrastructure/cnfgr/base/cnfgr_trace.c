...

{
                       ...
                       &cnfgrTraceHandle) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CNFGR_COMPONENT_ID,
            "CNFGR: unable to create the trace buffer\n");
    cnfgrTraceHandle = ~0;
  }
  else
  {
...
void cnfgrTraceFini(void)
{
  ...
  if (cnfgrTraceHandle != ~0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CNFGR_COMPONENT_ID,
              "CNFGR: unable to delete the trace buffer\n");
    }
    else
    {
      cnfgrTraceHandle = ~0;
...
void cnfgrTraceModeApply(L7_BOOL enable)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CNFGR_COMPONENT_ID,
            "cnfgrTraceModeApply: Unable to apply CNFGR trace mode (%u), "
...
L7_RC_t cnfgrTraceEventCompFormat(L7_ushort16 traceId, L7_uchar8 *pDataStart)
{
  ...
  if (traceId == CNFGR_EVENT_PER_COMP_START)
  {
    ...
    if (cnfgrApiComponentNameGet(p->targetCompId, tmpStr) != L7_SUCCESS)
    {
...
void cnfgrProfileInfoShow()
{
  ...
  if (cnfgrProfileEnable)
  {
    ...
    {
      memset(tmpStr, 0, sizeof(tmpStr));
      if (cnfgrApiComponentNameGet(i, tmpStr) != L7_SUCCESS)
      {
        osapiStrncpySafe (tmpStr, "Unknown", sizeof (tmpStr));
      };
      SYSAPI_PRINTF(SYSAPI_APPLICATION_LOGGING_ALWAYS,
