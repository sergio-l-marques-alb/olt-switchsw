                                     ...
                                     dnsClientLookupStatus_t status)
{
  ...
  } else
  {
    ...
    if ((rc = cnfgrApiComponentNameGet(request->componentId, name)) != L7_SUCCESS)
    {
      osapiStrncpySafe(name, "Unknown", 8);
    }

    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "DNS: Failed to send response to component %i, %s, not registered",
                                         ...
                                         L7_inet_addr_t         *inetAddr)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "dnsNameLookupResponseMessageSend(): message send failed!\n");
  }
  return L7_SUCCESS;
}
...
static void dnsRequestPacketWrite(dnsRequestEntry_t *request)
{
  ...
  switch (request->requestType)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "DNS Client: dnsRequestPacketWrite() - invalid request type - %d\n", request->requestType);
      break;
  }
  /* QCLASS = INTERNET */
  temp = 0;
                                     ...
                                     L7_inet_addr_t    *inetAddr)
{
  ...
  inetAddressReset(&inetAddrNetwork);
#endif
  for (addrIndex = 0; addrIndex < DNS_CACHE_ENTRY_IP_ADDRESSES; addrIndex++)
  {
    inetAddressReset(&inetAddrList[addrIndex]);
  }
         ...
         (cnameRefs < DNS_CNAME_REFERENCES_MAX))
  {
    ...
    if (type == DNS_RR_TYPE_CNAME)
    {
      ...
      if (ttl != 0)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
                  "DNS Client: failed to cache alias for request id %u\n", request->id);
        } else
        {
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "DNS Client: no ipv4 or ipv6 address found in response for request id %u\n", request->id);
    return L7_FAILURE;
  }
  ...
  if (ttl != 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "DNS Client: failed to add cache entry for request id %u\n", request->id);
    } else
    {
...
void dnsResponseProcess(dnsRequestEntry_t *request)
{
  ...
  rcode = header->rcode & DNS_MSG_HDR_RCODE_BIT_MASK;
  ...
  switch (rcode)
  {
    ...
    case DNS_RCODE_REFUSED:
      dnsOprData->counters.rcodes[rcode]++;
      dnsRequestQueryNextServer(request, DNS_LOOKUP_STATUS_RESPONSE_ERROR);
      return;
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "DNS Client: Received bad RCODE value\n");
  ...
  switch (request->requestType)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "DNS Client: dnsResponseProcess() - invalid request type - %d\n", request->requestType);
      break;
  }
...
void dnsTimerCallback()
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "dnsTimerCallback(): dnsQueue has not been created!\n");
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "dnsTimerCallback(): timer send failed\n");
  }
  osapiTimerAdd((void *)dnsTimerCallback, L7_NULL, L7_NULL,
                DNS_TIMER_INTERVAL, &dnsTimer);
}
