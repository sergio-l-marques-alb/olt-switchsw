...
extern L7_BOOL dnsAdminModeReady;
extern L7_BOOL dnsInitAdminMode;
...
L7_RC_t dnsClientNameServerEntryAdd(L7_inet_addr_t *serverAddr)
{
  ...
  if ( (inetIsAddressZero(serverAddr) == L7_TRUE) || (dnsCfgData == L7_NULLPTR) )
  {
    /* not a valid address */
    return L7_FAILURE;
  }
  ...
  if (found == L7_TRUE)
  {
                         ...
                         L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "DNS Client: failed to send server remove message\n");
    }
                                          ...
                                          L7_inet_addr_t     inetIp)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "[DNS client] %s (%d) invalid component ID %d \n",__FUNCTION__, __LINE__, componentId);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "[DNS client] %s (%d) component is disabled \n",__FUNCTION__, __LINE__);
    osapiSemaGive(dnsSemaphore);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "[DNS client] %s (%d) failed to generate request \n",__FUNCTION__, __LINE__);
    osapiSemaGive(dnsSemaphore);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "[DNS client] %s (%d) failed to send request \n",__FUNCTION__, __LINE__);
    osapiSemaGive(dnsSemaphore);
    return L7_FAILURE;
  }
...
L7_RC_t dnsClientDomainNameListAdd(L7_char8 *domainname)
{
  ...
  if( (domainname == L7_NULLPTR) || (dnsCfgData == NULL) )
  {
    return L7_FAILURE;
  }
  osapiSemaTake(dnsSemaphore, L7_WAIT_FOREVER);
  ...
  {
    osapiStrncpy(dnsCfgData->domainList[i], domainname, DNS_DOMAIN_NAME_SIZE_MAX+1);
...
L7_RC_t dnsClientDomainNameListEntryGetNext(L7_char8 *domainName)
{
  L7_BOOL found = L7_FALSE;
  L7_uint32 i = 0;
  L7_char8 prev_domain[DNS_DISPLAY_DOMAIN_NAME_SIZE_MAX];

  memset(prev_domain, '\0', sizeof(prev_domain));
