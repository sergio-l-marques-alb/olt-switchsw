  ...
  } else
  {
                         ...
                         L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
              "dnsApiCnfgrCommand(): message send failed\n");
    }
  }
}
...
L7_RC_t dnsInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DNS_CLIENT_COMPONENT_ID,
            "dnsInit: msgQueue creation error.\n");
                                  ...
                                  L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  *pReason    = 0;
  ...
  if (txtCfgApplyCompletionNotifyRegister(L7_DNS_CLIENT_COMPONENT_ID,
                            dnsApplyConfigCompleteCallback) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DHCP_SNOOPING_COMPONENT_ID,
            "DNS Client failed to register for apply config completions.");
    *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
    return L7_ERROR;
  }
