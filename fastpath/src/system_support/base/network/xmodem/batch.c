...
char *name;
    ...
    for (ptr=name; *ptr; ++ptr)
        if (*ptr == '/')
            slash = ptr + 1;
    (void)strncpy (xfilename, slash, sizeof (xfilename) - 1);

    /* change ':' to '/' and covert to all lower case */

    for (ptr=xfilename; *ptr; ++ptr)
        {
        if (*ptr == ':')
            *ptr = '/';
        if (isupper ((int)*ptr))
...
char *cpm_unix (char *string)
{
    ...
    if (*string == '\0') {
        xmodem_error("Null file name in MODEM7 batch receive", TRUE);
        return ((char *)-1);
        };

    for (iptr=(unsigned char *)string; (temp = *iptr) ; ) {
...
char *unix_cpm(char *string)
{
    ...
    xfilename[NAMSIZ] = '\000';

    /* Fuss with name */
    for (iptr=xfilename; (temp = *iptr) ;) {
    ...
    if (xdebugflag == TRUE)
        fprintf (xlogfp, "DEBUG: File %s sent as %s\n\r", string, xfilename);
