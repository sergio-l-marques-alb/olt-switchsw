                               ...
                               L7_uchar8 *data)
{
  ...
  if (pdu_process_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Searching for subType=%u",sub_type);
  ...
  for (i = 0; i < FD_CNFGR_SYSNET_MAX_REGISTRATIONS; i++)
  {
    if (sysnetNotifyList.sysnetNotifyEntries[i].type == SYSNET_SUBTYPE_ENTRY)
    {
      ...
      {
        if (pdu_process_debug)
          PT_LOG_TRACE(LOG_CTX_DTL,"Calling callback for subType=%u",sub_type);
                               ...
                               L7_char8 *data)
{
  ...
  if (pdu_process_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Searching for macAddr=%02x:%02x:%02x:%02x:%02x:%02x",macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5]);
  ...
  for (i = 0; i < FD_CNFGR_SYSNET_MAX_REGISTRATIONS; i++)
  {
    if (sysnetNotifyList.sysnetNotifyEntries[i].type == SYSNET_MAC_ENTRY)
    {
      ...
      {
        if (pdu_process_debug)
          PT_LOG_TRACE(LOG_CTX_DTL,"Calling callback for macAddr=%02x:%02x:%02x:%02x:%02x:%02x",macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5]);
                                    ...
                                    L7_char8 *data)
{
  ...
  if (pdu_process_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Searching for protocol_type=0x%04X",protocol_type);
  ...
  for (i = 0; i < FD_CNFGR_SYSNET_MAX_REGISTRATIONS; i++)
  {
    if (sysnetNotifyList.sysnetNotifyEntries[i].type == SYSNET_ETHERTYPE_ENTRY)
    {
      if (protocol_type ==  sysnetNotifyList.sysnetNotifyEntries[i].u.protocol_type)
      {
        ...
        if (pdu_process_debug)
          PT_LOG_TRACE(LOG_CTX_DTL,"Calling callback for protocol_type=0x%04X",protocol_type);
...
L7_RC_t sysNetNotifyPduReceive (L7_netBufHandle bufHandle, sysnet_pdu_info_t *pduInfo)
{
  ...
  if (pdu_process_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"I have received a packet here");
