...
L7_RC_t getParamsFromBootpStruct(L7_bootp_t* bootpParams, L7_bootp_dhcp_t* networkParams)
{
  ...
  if(L7_FALSE== L7_bootp_option_get(bootpParams, L7_TAG_SUBNET_MASK,&netMask , sizeof(netMask)))
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_BOOTP_COMPONENT_ID,
            "getParamsFromBootpStruct():L7_bootp_option_get() returned failure for netmask");
    return L7_FAILURE;
  }
  ...
  networkParams->optDnsServerIpAddr[0]  = osapiNtohl(dnsServerIpAddr[0]);
...
L7_int32 bootpDebugPortParamsPrint(struct bootp* bootpParams, L7_uint8 portType)
{
  ...
  sysapiPrintf("BOOTP offered DNS server address ");
  ...
  sysapiPrintf("\nBOOTP offered TFTP server address %s \n", ipString);
  ...
  while (L7_TRUE)
  {
        ...
        (servPortConfigStatus != L7_SYSCONFIG_MODE_COMPLETE))
    {
      ...
      if ( rc == L7_TRUE )
      {
          ...
          */
          ...
          if (defaultServiceIpConfigExists == L7_FALSE)
          {
                                       ...
                                       (L7_uint32 *)&errorMsg) == L7_FALSE)
            {
              ...
              simSetServPortAddrWithMask(params.ip, params.netMask);
              simSetServPortIPGateway(params.gateway);
              servPortConfigStatus = L7_SYSCONFIG_MODE_COMPLETE;
            ...
            {
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_BOOTP_COMPONENT_ID,
                      "L7_bootp():Conflict w/offered IP Addr, error=%u\r\n", errorMsg);
            }
          }
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_BOOTP_COMPONENT_ID,
                  "L7_bootp():getParamsFromBootpStruct()returned failure for service port");
        }
      }
    }
          ...
          */
          ...
          if (defaultServiceIpConfigExists == L7_FALSE)
          {
                                       ...
                                       (L7_uint32 *)&errorMsg) == L7_FALSE)
            {
              /* No conflict */
              /*Set IP Addresss, Default Gateway, Netmask */
              simSetSystemAddrWithMask(params.ip, params.netMask);
              simSetSystemIPGateway(params.gateway);
              networkConfigStatus = L7_SYSCONFIG_MODE_COMPLETE;
