...
static void l7_md5_encode(L7_uchar8 *output, L7_uint32 *input, L7_uint32 len);
static void l7_md5_transform(L7_uint32 *state, L7_uchar8 *block);
static void l7_md5_decode(L7_uint32 *output, L7_uchar8 *input, L7_uint32 len);

static L7_uchar8 l7_md5_padding_g[64] = {
...
{ \
  (a) += F((b), (c), (d)) + (x) + (L7_uint32)(ac); \
  (a) = ROTATE_LEFT ((a), (s)); \
  (a) += (b); \
}
#define GG(a, b, c, d, x, s, ac) \
{ \
  (a) += G((b), (c), (d)) + (x) + (L7_uint32)(ac); \
  (a) = ROTATE_LEFT ((a), (s)); \
  (a) += (b); \
}
#define HH(a, b, c, d, x, s, ac) \
{ \
  (a) += H((b), (c), (d)) + (x) + (L7_uint32)(ac); \
  (a) = ROTATE_LEFT ((a), (s)); \
  (a) += (b); \
}
#define II(a, b, c, d, x, s, ac) \
{ \
  (a) += I ((b), (c), (d)) + (x) + (L7_uint32)(ac); \
  (a) = ROTATE_LEFT ((a), (s)); \
  (a) += (b); \
}
...
void l7_md5_update(L7_MD5_CTX_t *context, L7_uchar8 *input, L7_uint32 inputLen)
{
  ...
  if (inputLen >= partLen)
  {
    ...
    l7_md5_transform(context->state, context->buffer);

    for (i = partLen; i + 63 < inputLen; i += 64)
      l7_md5_transform(context->state, &input[i]);
...
{
  L7_uchar8     bits[8];
  L7_uint32     index, padLen;

  /* Save number of bits */
  l7_md5_encode(bits, context->count, 8);

  /* Pad out to 56 mod 64 */
  index = (L7_uint32)((context->count[0] >> 3) & 0x3f);
  padLen = (index < 56) ? (56 - index) : (120 - index);
  l7_md5_update(context, l7_md5_padding_g, padLen);
   ...
   */
  l7_md5_update(context, bits, 8);
  
  /* Store state in digest */
  l7_md5_encode(digest, context->state, 16);

  /* Zeroize sensitive information */
  memset((L7_uchar8 *)context, 0, sizeof(*context));
}
...
static void l7_md5_transform(L7_uint32 *state, L7_uchar8 *block)
{
  register L7_uint32    a, b, c, d;
  L7_uint32             x[16];

  a = state[0], b = state[1], c = state[2], d = state[3];

  l7_md5_decode(x, block, 64);
...
static void l7_md5_encode(L7_uchar8 *output, L7_uint32 *input, L7_uint32 len)
{
  ...
  {
    output[j]   = (L7_uchar8)(input[i] & 0xff);
    output[j+1] = (L7_uchar8)((input[i] >> 8) & 0xff);
    output[j+2] = (L7_uchar8)((input[i] >> 16) & 0xff);
    output[j+3] = (L7_uchar8)((input[i] >> 24) & 0xff);
  }
}
...
static void l7_md5_decode(L7_uint32 *output, L7_uchar8 *input, L7_uint32 len)
{
  ...
  for (i = 0, j = 0; j < len; i++, j += 4)
    output[i] = ((L7_uint32)input[j]) | (((L7_uint32)input[j+1]) << 8) |
                (((L7_uint32)input[j+2]) << 16) | (((L7_uint32)input[j+3]) << 24);
}
...
void MD5TimeTrial(L7_uint32 blockCount);
void MD5TestSuite(void);
void MD5String_v(L7_uchar8 *string, L7_uchar8 answer[16]);
void MD5Print(L7_uchar8 digest[16]);
...
#define TEST_BLOCK_COUNT_MAX  10000
  ...
  L7_uchar8     block[TEST_BLOCK_LEN], digest[16];
  L7_uint32     i;
  time_t        deltaTime;
  ...
  if (blockCount > TEST_BLOCK_COUNT_MAX)
  {
    printf("Block count too large.  Must be between 0 and %u\n", 
           (L7_uint32)TEST_BLOCK_COUNT_MAX);
    return;
  }
  ...
  for (i = 0; i < blockCount; i++)
    l7_md5_update(&context, block, TEST_BLOCK_LEN);
  ...
  MD5Print(digest);
  printf("\nTime = %u seconds\n", (L7_uint32)deltaTime);
  if ((L7_uint32)deltaTime > 0)
    printf("Speed = %u bytes/second\n",
           (L7_uint32)(TEST_BLOCK_LEN * blockCount/deltaTime));
}
...
void MD5String_v(L7_uchar8 *string, L7_uchar8 answer[16])
{
  L7_MD5_CTX_t context;
  L7_uchar8 digest[16];
  L7_uint32 len = strlen(string);

  l7_md5_init(&context);
  l7_md5_update(&context, string, len);
  l7_md5_final(digest, &context);
