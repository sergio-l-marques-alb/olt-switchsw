...
xLibRC_t fpObjSet_routingvrrpOperationEntry_ConfigPriority (void *wap, void *bufp)
{
  ...
  /* set the value in application */
  owa.l7rc = usmDbVrrpConfigPrioritySet (objVrIdValue, keyifIndexValue, priority);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
...
/**************************************************************************************************
* @function fpObjGet_routingvrrpOperationEntry_RoutingPortListFirstIPAddress
*
* @purpose Get 'RoutingPortListFirstIPAddress'
*
* @description [RoutingPortListFirstIPAddress]: The IP Address of the first routing interface.
*                                               Used specifically in a scenario where the ip address
*                                               of the first routing interface should be fecthed 
*                                               regardless of whether the interface is configured
*                                               for vrrp operation entry or not.   
*                                
*
* @return
***************************************************************************************************/
xLibRC_t fpObjGet_routingvrrpOperationEntry_RoutingPortListFirstIPAddress (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objIfIndexValue;
  xLibU32_t rtr_intf_mode;
  L7_IP_ADDR_t objipAddressValue;
  L7_IP_MASK_t objMaskValue;
  xLibBool_t entryFound = XLIB_FALSE;
  ...
  memset(&objMaskValue,0x0,sizeof(objMaskValue));

  owa.l7rc = usmDbIpRtrIntfIpAddressGet (L7_UNIT_CURRENT, objIfIndexValue,
  ...
  /* return the object value: RoutingPortListFirstIPAddress */
...
/*******************************************************************************
 * @function fpObjGet_routingvrrpOperationEntry_AcceptMode
 *
 * @purpose Get 'AcceptMode'
 * @description [AcceptMode] Allow/Disallow VRRP master from accepting packets
 * destined to VRIP address(es).   
 * @notes       
 *
 * @return
 *******************************************************************************/
xLibRC_t fpObjGet_routingvrrpOperationEntry_AcceptMode (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2();
  xLibU32_t objAcceptModeValue;
  xLibU32_t keyifIndexValue;
  xLibU32_t keyVrIdValue;
  ...
  FPOBJ_TRACE_CURRENT_KEY(bufp, &keyVrIdValue, owa.len);

  /* get the value from application */
  owa.l7rc = usmDbVrrpOperAcceptModeGet (L7_UNIT_CURRENT, 
                                         keyVrIdValue,
                                         keyifIndexValue,
                                         &objAcceptModeValue);
  if (owa.l7rc != L7_SUCCESS)
  ...
  FPOBJ_TRACE_VALUE (bufp, &objAcceptModeValue, sizeof(objAcceptModeValue));

  /* return the object value: AcceptMode */
  owa.rc = xLibBufDataSet(bufp, (xLibU8_t *)&objAcceptModeValue,
                          sizeof(objAcceptModeValue));
  FPOBJ_TRACE_EXIT(bufp, owa);
  return owa.rc;
}

/*******************************************************************************
 * @function fpObjSet_routingvrrpOperationEntry_AcceptMode
 *
 * @purpose Set 'AcceptMode'
 * @description [AcceptMode] Allow/Disallow VRRP master from accepting packets
 * destined to VRIP address(es).   
 * @notes       
 *
 * @return
 *******************************************************************************/
xLibRC_t fpObjSet_routingvrrpOperationEntry_AcceptMode (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2();
  xLibU32_t objAcceptModeValue;
  xLibU32_t keyifIndexValue;
  ...
  /* retrieve object: AcceptMode */
  owa.len = sizeof(objAcceptModeValue);
  owa.rc = xLibBufDataGet(bufp,
                          (xLibU8_t *)&objAcceptModeValue,
                          &owa.len);
  if(owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_EXIT(bufp, owa);
  ...
  FPOBJ_TRACE_VALUE (bufp, &objAcceptModeValue, owa.len);
  ...
  /* set the value in application */
  owa.l7rc = usmDbVrrpOperAcceptModeSet (L7_UNIT_CURRENT, 
                                         keyVrIdValue,
                                         keyifIndexValue,
                                         objAcceptModeValue);
