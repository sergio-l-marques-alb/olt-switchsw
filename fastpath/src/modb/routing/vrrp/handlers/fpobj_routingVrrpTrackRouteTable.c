...
#define VRRP_TRACK_ROUTE_PFX_LEN_MIN 0
...
xLibRC_t fpObjGet_routingVrrpTrackRouteTable_VrrpTrackRtPfx (void *wap, void *bufp)
{
  ...
  if (owa.rc != XLIBRC_SUCCESS)
  {
    ...
    /* Get first tracked route for this VR on this interface */
    owa.l7rc = usmDbVrrpVrTrackedRouteGetFirst(objVrrpOperVrIdValue, 
                                               objifIndexValue,
                                               &nextObjVrrpTrackRtPfxValue,
                                               &nextObjVrrpTrackRtPfxLenValue);
  }
...
xLibRC_t fpObjGet_routingVrrpTrackRouteTable_VrrpTrackRtPfxLen (void *wap, void *bufp)
{
  ...
  if (owa.rc != XLIBRC_SUCCESS)
  {
    /* We get here both on the first row of the table, and also on getting
     * the first prefix length for each prefix. */
    FPOBJ_TRACE_CURRENT_KEY (bufp, NULL, 0);

    nextObjVrrpTrackRtPfxLenValue = objVrrpTrackRtPfxLenValue = 0;
    if (objVrrpTrackRtPfxValue == 0)
    {
      /* If prefix is 0, then only valid mask value is 0. 
       * If the default route is tracked, it must be the first 
       * tracked route. So get first tracked route for this VR on this interface */
      owa.l7rc = usmDbVrrpVrTrackedRouteGetFirst(objVrrpOperVrIdValue, 
                                               objifIndexValue,
                                               &nextObjVrrpTrackRtPfxValue,
                                               &nextObjVrrpTrackRtPfxLenValue);
