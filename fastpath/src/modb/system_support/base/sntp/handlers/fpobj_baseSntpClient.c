...
xLibRC_t fpObjSet_baseSntpClient_UcastServerRowStatus (void *wap, void *bufp)
{
  ...
  if(objUcastServerRowStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
    ...
    if( usmDbIPHostAddressValidate(objcastServerAddressValue, 
                                     &ipAddr, &addrType) != L7_SUCCESS)
    {
     /*Validation of Host address */
      ...
      if((usmDbIPv6HostAddressValidate(objcastServerAddressValue, &serverAddr,&addrType) != L7_SUCCESS) ||
          (addrType != objcastServerAddressTypeValue ))
          
      {
         owa.rc = XLIBRC_INVALID_IPV4_DNS_ADDRESS; /* TODO: Look for appopriate error(if exists)*/
         FPOBJ_TRACE_EXIT (bufp, owa);
         return owa.rc;
      }  
      #else
    ...
    if (addrType != objcastServerAddressTypeValue )
    {
      owa.rc = XLIBRC_INVALID_IPV4_DNS_ADDRESS;  /* TODO: add error string */
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
...
xLibRC_t fpObjGet_baseSntpClient_sntpAuthType (void *wap, void *bufp)
{
  ...
  OSAPI_STRNCAT(objsntpAuthTypeValue,"MD5");
  owa.l7rc = L7_SUCCESS;
  /* get the value from application */
