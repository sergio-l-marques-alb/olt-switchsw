...
xLibRC_t fpObjSet_baseSystemInfo_sysName (void *wap, void *bufp)
{
  ...
  usmDbCommandPromptSet(L7_UNIT_CURRENT, objsysNameValue);
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
}
...
/*******************************************************************************
* @function fpObjGet_baseSystemInfo_banner
*
* @purpose Get 'banner'
 *@description  [banner] Capable of configuring the CLI / Web attention banner.
* The banner should support 1500 characters.
* @notes
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseSystemInfo_banner (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2 ();
  L7_char8 objbannerValue[2048];
  L7_char8 objbannerValue1[1024];

  FPOBJ_TRACE_ENTER (bufp);

  memset(objbannerValue, 0x0,sizeof(objbannerValue));
  memset(objbannerValue1, 0x0,sizeof(objbannerValue1));

#if L7_FEAT_BANNER_MOTD
  /* get the value from application */
  owa.l7rc = usmDbBannerGet(L7_UNIT_CURRENT, objbannerValue);
  osapiStrncpySafe(objbannerValue1, objbannerValue, sizeof(objbannerValue1));
#else 
  ...
  FPOBJ_TRACE_VALUE (bufp, objbannerValue, strlen (objbannerValue));

  /* return the object value: banner */
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) objbannerValue1, strlen (objbannerValue1));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
...
/*******************************************************************************
* @function fpObjSet_baseSystemInfo_banner
*
* @purpose Set 'banner'
 *@description  [banner] Capable of configuring the CLI / Web attention banner.
* The banner should support 1500 characters.
...
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseSystemInfo_banner (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2 ();
  L7_char8 objbannerValue[1024];

  FPOBJ_TRACE_ENTER (bufp);

  memset(objbannerValue, 0x0,sizeof(objbannerValue));
  /* retrieve object: banner */
  owa.len = sizeof (objbannerValue);
  owa.rc = xLibBufDataGet (bufp, (xLibU8_t *) objbannerValue, &owa.len);
  ...
  FPOBJ_TRACE_VALUE (bufp, objbannerValue, owa.len);

  /* if row status object is specified and eual to delete return success */

#if L7_FEAT_BANNER_MOTD
  /* set the value in application */
  owa.l7rc = usmDbBannerSet(L7_UNIT_CURRENT, objbannerValue);
#else
  owa.l7rc = L7_NOT_SUPPORTED;
#endif
...
/*******************************************************************************
* @function fpObjGet_baseSystemInfo_bannerAckFlag
*
* @purpose Get 'bannerAckFlag'
 *@description  [bannerAckFlag] The user will be required to acknowledge the
* banner displayed on the console if banner motd acknowledge flag is
* set to true. The user would have to type y or n to continue to the
* login prompt.  If n is typed, the session is terminated and no
* further communication is allowed on that session. However, serial
* connection will not get terminated if user does not enter y.
* @notes
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseSystemInfo_bannerAckFlag (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2 ();
  xLibU32_t objbannerAckFlagValue;
...
#if L7_FEAT_BANNER_MOTD
  /* get the value from application */
  owa.l7rc = usmDbBannerAckGet(L7_UNIT_CURRENT, &flag);
#else
  owa.l7rc = L7_NOT_SUPPORTED;
#endif
  ...
  objbannerAckFlagValue = flag;
  FPOBJ_TRACE_VALUE (bufp, &objbannerAckFlagValue, sizeof (objbannerAckFlagValue));

  /* return the object value: bannerAckFlag */
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) & objbannerAckFlagValue,
                           sizeof (objbannerAckFlagValue));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
...
/*******************************************************************************
* @function fpObjSet_baseSystemInfo_bannerAckFlag
*
* @purpose Set 'bannerAckFlag'
 *@description  [bannerAckFlag] The user will be required to acknowledge the
* banner displayed on the console if banner motd acknowledge flag is
* set to true. The user would have to type y or n to continue to the
* login prompt.  If n is typed, the session is terminated and no
...
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseSystemInfo_bannerAckFlag (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2 ();
  xLibU32_t objbannerAckFlagValue;
  ...
  /* retrieve object: bannerAckFlag */
  owa.len = sizeof (objbannerAckFlagValue);
  owa.rc = xLibBufDataGet (bufp, (xLibU8_t *) & objbannerAckFlagValue, &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
  }
  FPOBJ_TRACE_VALUE (bufp, &objbannerAckFlagValue, owa.len);

  /* if row status object is specified and eual to delete return success */

#if L7_FEAT_BANNER_MOTD
  /* set the value in application */
  owa.l7rc = usmDbBannerAckSet(L7_UNIT_CURRENT, objbannerAckFlagValue);
#else
  owa.l7rc = L7_NOT_SUPPORTED;
#endif
...
/*******************************************************************************
* @function fpObjSet_baseSystemInfo
*
* @purpose Set to 'renewDHCP'
*
* @description [renewDHCP]: To renew IPaddress configured through DHCP.
*
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseSystemInfo_renewDHCP(void *wap, 
                                           void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objIfval;
  ...
  FPOBJ_TRACE_VALUE (bufp, &objIfval, owa.len);
#if 0
  /* retrieve key: IfIndex */
  kwa.rc = xLibFilterGet (wap, XOBJ_baseSystemInfo_sysMgmtPort,
...
#endif
  /* set the value in application */
  owa.l7rc = usmDbIntfIpAddressMethodSet (objIfval, L7_INTF_IP_ADDR_RENEW,
