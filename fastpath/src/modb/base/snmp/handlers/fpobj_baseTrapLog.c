...
xLibRC_t fpObjGet_baseTrapLog_TrapLogIndex (void *wap, void *bufp)
{
  ...
  max = (objTrapLogTotalSinceLastResetValue > TRAPMGR_TABLE_SIZE) ? TRAPMGR_TABLE_SIZE : objTrapLogTotalSinceLastResetValue;
  if(nextObjTrapLogIndexValue >= max)
  {
    owa.rc = XLIBRC_ENDOF_TABLE;
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
...
static xLibU32_t wrapTrapLogIndex(xLibU32_t keyTrapLogIndexValue)
{
  xLibU32_t retval;
  xLibU32_t keyTrapLogEntryCurrentCount = usmDbTrapLogEntryLastReceivedGet();
  if(keyTrapLogEntryCurrentCount < keyTrapLogIndexValue)
  {
    retval = TRAPMGR_TABLE_SIZE + keyTrapLogEntryCurrentCount - keyTrapLogIndexValue;
  }
  else
  {
    retval = keyTrapLogEntryCurrentCount - keyTrapLogIndexValue;
  }
  return retval;
}
...
xLibRC_t fpObjGet_baseTrapLog_TrapLogSystemUptime (void *wap, void *bufp)
{
  ...
  keyTrapLogIndexValue = wrapTrapLogIndex(keyTrapLogIndexValue);
