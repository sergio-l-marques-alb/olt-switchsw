...
xLibRC_t ObjUtilDiffServPolicyClsDefStrToIndexValGet(xLibS8_t *str, xLibU32_t *val)
{
  ...
  xLibU8_t *matchTypeList[] = {"None","Assign Queue","Drop","Mark CoS","Mark CoS As Secondary CoS","Mark IP DSCP",
                               "Mark IP Precedence","Mirror Interface","Police Simple","Single Rate","Police Two Rate","Redirect Interface"};
                                                                     ...
                                                                     void *bufp)
{
  ...
  if (objDiffServPolicyAttrRowStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
    ...
    {
      owa.l7rc = usmDbDiffServPolicyAttrStmtEntryTypeGet(L7_UNIT_CURRENT, keyPolicyIndexValue, keyPolicyInstIndexValue, tempPolicyAttrIndxValue,  
          &EntryType);
      if (owa.l7rc != L7_SUCCESS)
      {
        owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
