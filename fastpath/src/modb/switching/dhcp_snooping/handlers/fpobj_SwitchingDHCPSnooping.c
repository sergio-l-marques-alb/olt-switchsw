...
/*******************************************************************************
* @function fpObjGet_SwitchingDHCPSnooping_EnabledInterface
*
* @purpose Get 'EnabledInterface'
*
* @description [EnabledInterface] Get the DHCP Snooping Enabled Interface
*
* @notes  
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_SwitchingDHCPSnooping_EnabledInterface (void *wap, void *bufp)
{
  ...
  xLibU32_t nextObjInterfaceValue;  
  xLibU32_t objtrust;
  xLibU32_t objInterfaceTrustValue; 
  FPOBJ_TRACE_ENTER (bufp);

  /* retrieve key: Interface */
  owa.rc = xLibFilterGet (wap, XOBJ_SwitchingDHCPSnooping_EnabledInterface,
                          (xLibU8_t *) & objInterfaceValue, &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_CURRENT_KEY (bufp, NULL, 0);
  ...
  {
      if ((usmDbDsIntfIsValid(nextObjInterfaceValue) == L7_TRUE)
        && (usmDbDsPortEnabledGet(nextObjInterfaceValue, &objtrust) == L7_SUCCESS)
        && (objtrust == L7_TRUE) 
        && (usmDbDsIntfTrustGet(nextObjInterfaceValue, &objInterfaceTrustValue) == L7_SUCCESS)
        && (objInterfaceTrustValue == L7_FALSE))
      {
        owa.l7rc = L7_SUCCESS;
        break;
      }     
  ...
  if (objRowStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
	  ...
	  if (kwaintIfNumber.rc != XLIBRC_SUCCESS)
	  {
	    ...
	    return kwaintIfNumber.rc;
         ...
         owa.l7rc = usmDbDsStaticBindingAdd(&macAddr, keyIPAddressValue, keyVlan, 0 /* PTin modified: DHCP */, keyintIfNumber);
    if (owa.l7rc != L7_SUCCESS)
    {
...
/*******************************************************************************
* @function fpObjSet_SwitchingDHCPSnooping_ClearStats
*
* @purpose Set 'ClearStats'
*
* @description [ClearStatss] Clear DHCP snooping interface Statistics.
*
* @notes
*
* @return
*******************************************************************************/
xLibRC_t fpObjSet_SwitchingDHCPSnooping_ClearStats (void *wap, void *bufp)
{
  ...
  /* set the value in application */
  if(objClearStatsValue == L7_TRUE)
  {
    owa.l7rc = usmDbDsIntfStatsClear();
  }
  else
  {
    /*Do nothing*/
