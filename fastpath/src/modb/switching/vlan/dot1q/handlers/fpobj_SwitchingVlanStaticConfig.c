...
xLibRC_t fpObjSet_SwitchingVlanStaticConfig_Name (void *wap, void *bufp)
{
  ...
  memset(objNameValue, 0x00, sizeof(objNameValue));
  memset(&filterMask,0x0,sizeof(filterMask));
  ...
  while(1)
  {
    ...
    if(keyVlanIndexValue != 1)
    {
      ...
      {
        owa.rc = XLIBRC_DYNAMIC_VLAN;    /* Name cannot be set for a Dynamic VLAN */
        FPOBJ_TRACE_EXIT (bufp, owa);
        return owa.rc;
      } 
                                                           ...
                                                           void *bufp)
{
  ...
  do
  {
    ...
    else
    {
      ...
      else if((objPortVlanMode == DOT1Q_SWPORT_MODE_ACCESS) && (objParticipationValue == L7_DOT1Q_NORMAL_REGISTRATION))
      {
        owa.l7rc = usmDbDot1qSwPortAccessVlanSet(L7_UNIT_CURRENT, keyInterfaceValue, FD_DOT1Q_DEFAULT_VLAN);
      }
      else 
      {
                                                           ...
                                                           void *bufp)
{
  ...
  if ( owa.l7rc == L7_SUCCESS )
  {
      if(vlanType == L7_DOT1Q_DYNAMIC || vlanType == L7_DOT1Q_WS_DYNAMIC  || vlanType == L7_DOT1Q_IP_INTERNAL_VLAN)
      {
