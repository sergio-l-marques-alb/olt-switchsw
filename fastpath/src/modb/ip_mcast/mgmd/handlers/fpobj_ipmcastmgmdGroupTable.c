    ...
    while(owa.l7rc == L7_SUCCESS)
    {
                              ...
                              keymgmdGroupIfIndexValue, &val) == L7_SUCCESS)
      {
        if(val == MGMD_FILTER_MODE_INCLUDE)
        {
           ...
           {
             osapiStrncpy(tempObjmgmdGroupSrcListExpiryTimeValue, pStrErr_common_Err_Temp,sizeof(tempObjmgmdGroupSrcListExpiryTimeValue)); 
           }
        }
        else if(val == MGMD_FILTER_MODE_EXCLUDE)
             {
                                         ...
                                         keymgmdGroupIfIndexValue, &groupHostAddressValue, &val) == L7_SUCCESS)
               {
                 if(val == 0)
                 {
                   osapiStrncpy(tempObjmgmdGroupSrcListExpiryTimeValue, pStrInfo_common_Line_Temp,sizeof(tempObjmgmdGroupSrcListExpiryTimeValue)); 
                 }
                 else
                 {
                   memset(&timeSpec, 0, sizeof(usmDbTimeSpec_t));
