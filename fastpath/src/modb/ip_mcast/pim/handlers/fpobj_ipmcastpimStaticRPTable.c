...
xLibRC_t fpObjSet_ipmcastpimStaticRPTable_pimStaticRPRowStatus (void *wap, void *bufp)
{
  ...
  /* Validate NetMask */
  if (keypimStaticRPAddressTypeValue == L7_AF_INET)
  {
    L7_inet_addr_t mask;

    inetAddressZeroSet(L7_AF_INET,&mask);
    inetMaskLenToMask(L7_AF_INET,keypimStaticRPGrpPrefixLengthValue.addr,&mask);
    if (usmDbNetmaskIsContiguous (mask.addr.ipv4.s_addr) != L7_TRUE)
    {
