...
xLibRC_t fpObjSet_baseSlotInfo_SlotPowerMode (void *wap, void *bufp)
{
  ...
  {
    if (objSlotPowerModeValue == L7_ENABLE)
    {
      owa.rc = XLIBRC_SLOT_POWER_SET_ENABLE_UNSUPPORTED;
    }
    else if (objSlotPowerModeValue == L7_DISABLE)
    {
      owa.rc = XLIBRC_SLOT_POWER_SET_DISABLE_UNSUPPORTED;
    }
  }
  else if (owa.l7rc == L7_ERROR || owa.l7rc == L7_FAILURE)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
  }
  FPOBJ_TRACE_EXIT (bufp, owa);
...
xLibRC_t fpObjSet_baseSlotInfo_SlotAdminMode (void *wap, void *bufp)
{
  ...
  if (owa.l7rc == L7_NOT_SUPPORTED)
  {
    if (objSlotAdminModeValue == L7_ENABLE)
    {
      owa.rc = XLIBRC_SLOT_ADMIN_SET_ENABLE_UNSUPPORTED;
    }
    else if (objSlotAdminModeValue == L7_DISABLE)
    {
      owa.rc = XLIBRC_SLOT_ADMIN_SET_DISABLE_UNSUPPORTED;
    }
  }
  else if (owa.l7rc == L7_ERROR || owa.l7rc == L7_FAILURE)
  {
...
/*******************************************************************************
* @function fpObjSet_baseStacking_RowStatus
*
* @purpose Set 'RowStatus'
*
* @description Create / Delete Slot. 
*              
* @note  This is Row-Status Object
*
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseSlotInfo_RowStatus(void *wap, void *bufp)
{

  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objBaseSlotCardStatusValue;
  ...
  xLibU32_t objUnitIdValue;
  xLibU32_t ux,sx;
  xLibU32_t slot_number;
  xLibBool_t foundSlotIndex = XLIB_FALSE;

  FPOBJ_TRACE_ENTER (bufp);

  /* retrieve object: objBaseSlotCardStatusValue*/
  owa.rc = xLibBufDataGet (bufp, (xLibU8_t *) & objBaseSlotCardStatusValue, &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
  }
  FPOBJ_TRACE_VALUE (bufp, objSupportedCardValue, owa.len);
  ...
  if(objBaseSlotCardStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
    /* check if specified slot is supported on the specified unit */
    owa.l7rc = usmDbUnitMgrUnitTypeGet(keyUnitNumberValue,&objUnitIdValue);
    if(owa.l7rc != L7_SUCCESS)
    {
      /* TODO: Change if required */
       owa.rc= XLIBRC_UNIT_NO_EXISTS;
       FPOBJ_TRACE_EXIT (bufp, owa);
       return owa.rc;
    }
    if (usmDbUnitIndexFromIDGet(objUnitIdValue, &ux) != L7_SUCCESS)
    {
       /* TODO: Change if required */
       owa.rc= XLIBRC_SWITCH_TYPE_NOT_SUPPORTED;
       FPOBJ_TRACE_EXIT (bufp, owa);
       return owa.rc;
    }
    sx =1;
    ...
    {
      if (slot_number == keySlotIndexValue)
      {
    ...
    {
       /* TODO: Change if required */
       owa.rc= XLIBRC_SLOT_DOES_NOT_EXISTS;
       FPOBJ_TRACE_EXIT (bufp, owa);
       return owa.rc;
    }
    /* retrieve key: CardIndex */
    kwaSlotIndex.rc = xLibFilterGet (wap, XOBJ_baseCardTypeDetails_CardIndex,
    ...
    owa.l7rc = usmDbCardInSlotSupportedGet(ux, sx, keyCardIndexValue); 
    if(owa.l7rc != L7_SUCCESS)
    {
      /* TODO: Change if required */
      owa.rc= XLIBRC_SLOT_CARD_UNSUPPORTED;
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
    } 

    if (usmDbCardSupportedTypeIdGet(keyCardIndexValue, &objCardTypeValue) != L7_SUCCESS)    
    {
       /* TODO: Change if required */
       owa.rc= XLIBRC_CARD_UNSUPPORTED;
       FPOBJ_TRACE_EXIT (bufp, owa);
       return owa.rc;
    }

    /* configure specified slot */
    owa.l7rc = usmDbSlotCardConfigSet(keyUnitNumberValue, keySlotIndexValue, objCardTypeValue);
    if (owa.l7rc != L7_SUCCESS)
    {
       if(owa.l7rc == L7_ALREADY_CONFIGURED)
       {
         /* TODO: Change if required */
         owa.rc= XLIBRC_SLOT_CARD_EXISTS;
         FPOBJ_TRACE_EXIT (bufp, owa);
         return owa.rc;
       }
       else
       {
         /* TODO: Change if required */
         owa.rc= XLIBRC_SLOT_CARD_CONFIGURE_ERROR;
         FPOBJ_TRACE_EXIT (bufp, owa);
         return owa.rc;
       }
    }
  }
  else if (objBaseSlotCardStatusValue == L7_ROW_STATUS_DESTROY)
  { 
     owa.l7rc = usmDbSlotCardConfigClear(keyUnitNumberValue, keySlotIndexValue);
     if(owa.l7rc != L7_SUCCESS)
     {
        if(owa.l7rc == L7_NOT_SUPPORTED)
        {
           /* TODO: Change if required */
          owa.rc= XLIBRC_SLOT_CARD_CLEAR_ERROR;
          FPOBJ_TRACE_EXIT (bufp, owa);
          return owa.rc;
        }
        else
        {
          /* TODO: Change if required */
          owa.rc= XLIBRC_SLOT_CLEAR_ERROR;
          FPOBJ_TRACE_EXIT (bufp, owa);
          return owa.rc;
        }
     }
...
/*******************************************************************************
* @function fpObjGet_baseStacking_RowStatus
*
...
xLibRC_t fpObjGet_baseSlotInfo_RowStatus(void *wap, void *bufp)
{
  return XLIBRC_SUCCESS;
}
