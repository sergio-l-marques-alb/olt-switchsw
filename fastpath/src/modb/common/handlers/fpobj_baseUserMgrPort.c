...
#define REM_STR_LEN(a) (sizeof(a)-strlen(a))
  ...
  {
    FPOBJ_TRACE_CURRENT_KEY (bufp, NULL, 0);
    owa.l7rc=usmDbPortUserGetFirst (L7_UNIT_CURRENT, keyifIndexValue, &usrIndex, nextObjportUserNameValue, sizeof(nextObjportUserNameValue));
    while(owa.l7rc == L7_SUCCESS) 
    {
       owa1.l7rc = usmDbPortUserAccessGet(L7_UNIT_CURRENT, keyifIndexValue, nextObjportUserNameValue, &flag );
       if(owa1.l7rc == L7_SUCCESS && flag == L7_TRUE)
       {
       ...
       owa.l7rc = usmDbPortUserGetNext (L7_UNIT_CURRENT,keyifIndexValue,
                     &usrIndex, nextObjportUserNameValue, sizeof(nextObjportUserNameValue));
    }
  }
  else
  {
