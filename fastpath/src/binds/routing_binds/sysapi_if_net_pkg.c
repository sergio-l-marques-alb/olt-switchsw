...
*
*           The address 0/0 is not considered to conflict with any other address. 
*           (0/0 gets passed when clearing the address on an interface.)
*       
* @end
*********************************************************************/
L7_BOOL sysapiIPConfigConflict(L7_uint32 intIfNum, L7_uint32 ifIpAddress, 
                               L7_uint32 ifNetMask, L7_uint32 *errorMsg)
{
  ...
  *errorMsg = 0;
  ...
  if ((rc != L7_TRUE) && 
      ((intIfNum < FD_CNFGR_NIM_MIN_SERV_PORT_INTF_NUM) || 
       (intIfNum > FD_CNFGR_NIM_MAX_SERV_PORT_INTF_NUM)))
  {
