                    ...
                    L7_inet_addr_t *groupAddr, L7_inet_addr_t *groupMask)
{
  ...
  L7_uint32 defSSMGrpMaskLen = L7_NULL;
  ...
  if (ssmMode == L7_ENABLE)
  {
    for (index = 0; index < L7_MAX_SSM_RANGE; index++)
    {
      ...
      { 
        ...
        /* to check for input is "default" and if any value  with in default ssm Range 
           is already configured for IPV6*/ 
        if(familyType == L7_AF_INET6)
        {
          if(((((L7_in6_addr_t *)(&(groupAddr->addr.ipv6)))->in6.addr16[0]) == 0xff30))
          {
            defSSMGrpMaskLen = 12;/*for IPV6 mask length for ff3x::(default) */ 
            if(inetAddrCompareAddrWithMask (&ssmRange->groupAddr, defSSMGrpMaskLen,
                                     ...
                                     L7_uint32 dataThresholdRate)
{
   ...
   {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES, "Non-Zero SPT Threshold - %d "
                      "is not Supported.", dataThresholdRate);
      L7_LOGF (L7_LOG_SEVERITY_NOTICE,L7_FLEX_PIMSM_MAP_COMPONENT_ID,
               "Non-Zero SPT/Data Threshold Rate - %d is currently Not Supported "
               "on this platform.", dataThresholdRate);
      return L7_NOT_SUPPORTED;
   }
                                         ...
                                         L7_uint32 regThresholdRate)
{
   ...
   if (pimsmMapCbPtr->pPimsmInfo->pimsmOperational != L7_TRUE)
   {
     ...
     "operational config paramter stored but not applied\n"); 
     PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                       "PIM-SM not operational.config parameter stored but not applied\n"); 
   }
    ...
    L7_inet_addr_t *rpGrpAddr, L7_uchar8 prefixLen, L7_BOOL conflict)
{
   ...
   /* range checking */
   if (inetIpAddressValidityCheck(familyType, rpIpAddr) != L7_SUCCESS)
   {
   ...
   {
     L7_uint32  rpIPAddr;
     rpIPAddr = rpIpAddr->addr.ipv4.s_addr;
     if (rlimIsAddressLoopback(rpIPAddr) != L7_SUCCESS)
     {
       PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"RP Address(%s) is loopback interface address", 
                       inetAddrPrint(rpIpAddr,rpAddr));
       return L7_NOT_SUPPORTED;
     }
   }
    ...
    L7_inet_addr_t *rpGrpAddr, L7_uchar8 prefixLen)
{
   ...
   PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                   "Failed to remove static RP(%s) for Grp Address(%s)",
                   inetAddrPrint(rpIpAddr,rpAddr), 
                   inetAddrPrint(rpGrpAddr,grpAddr));
   return L7_FAILURE;
}
                                 ...
                                 L7_uchar8 prefixLen)
{
    ...
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Failed to get static RP entry for Grp Address(%s)",
                    inetAddrPrint(rpGrpAddr,grpAddr));
    return L7_FAILURE;
}
                                           ...
                                           L7_uint32 *overRide)
{
    ...
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Failed to find next Static RP entry for  Grp Address(%s)",
                    inetAddrPrint(rpGrpAddr,grpAddr));
    return L7_FAILURE;
}
                                           ...
                                           L7_uint32 overRide)
{
    ...
    {
       PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                       "Invalid rpGrpAddr(%s) or invalid rpGrpMask (%s)",
                       inetAddrPrint(rpGrpAddr,grpAddr),
                                ...
                                L7_inet_addr_t *staticRpAddr)
{
    ...
    L7_uchar8          rpBuf[PIMSM_MAX_DBG_ADDR_SIZE];
    ...
    if (pimsmMapStaticRpEntryGet(familyType, rpGrpAddr, 
             prefixLength, &rpIndex, &staticRpEntry) == L7_SUCCESS)
    {
      /* Do basic validation on the RP Address */
      L7_uchar8 rpAddrBuf[IPV6_DISP_ADDR_LEN];
      if (inetIpAddressValidityCheck(familyType, staticRpAddr) != L7_SUCCESS)
      {
        PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"Invalid RP Address(%s)", 
                        inetAddrPrint(staticRpAddr,rpAddrBuf));
        return L7_FAILURE; 
      }
#ifdef L7_RLIM_PACKAGE
      /* loopback interface check */
      ...
      {
        L7_uint32  rpIPAddr;
        rpIPAddr = staticRpAddr->addr.ipv4.s_addr;
        if (rlimIsAddressLoopback(rpIPAddr) != L7_SUCCESS)
        {
          PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"RP Address(%s) is loopback interface address", 
                          inetAddrPrint(staticRpAddr,rpAddrBuf));
...
#endif
      if (inetIsAddressZero(staticRpAddr) == L7_TRUE)
      {
        PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"RpAddress is 0.It is an invalid address");
        return L7_FAILURE;
      }

      /* Found existing entry. To modify the existing entry,
       * Remove it and re-add it.
       */
      conflict = staticRpEntry->overRide;
      if ((retCode = pimsmMapStaticRPRemoveApply(pimsmMapCbPtr,  
                &staticRpEntry->rpIpAddr, &staticRpEntry->rpGrpAddr, 
                &staticRpEntry->rpGrpMask, rpIndex))
             != L7_SUCCESS)
      {
        PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,"Failed to remove the existing "
                         "RP address - %s from PIMSM application",
                         inetAddrPrint(&staticRpEntry->rpIpAddr,rpBuf));
      }
      inetCopy(&staticRpEntry->rpIpAddr, staticRpAddr);
      staticRpEntry->overRide = conflict;
      if ((retCode = pimsmMapStaticRPApply(pimsmMapCbPtr,  
                &staticRpEntry->rpIpAddr, &staticRpEntry->rpGrpAddr, 
                &staticRpEntry->rpGrpMask, staticRpEntry->overRide, rpIndex))
             != L7_SUCCESS)
      {
        PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,"Failed to modify existing "
                         "RP configuration - %s in PIMSM application",
                         inetAddrPrint(&staticRpEntry->rpIpAddr,rpBuf));
      }
    }
    else
                                 ...
                                 L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  {
       PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                       "intIfNum(%d) not valid for PIM-SM configuration",
                       intIfNum);
       return L7_FAILURE;
  }
                                  ...
                                  L7_uint32 intIfNum)
{
    ...
    {
       PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                       "PIM-SM component was not started on intIfNum(%d)",intIfNum);
    }
    else
    {
       if(pimsmMapCbPtr->pPimsmIntfInfo[intIfNum].pimsmOperational == L7_TRUE)
       ...
       {
         PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                         "PIM-SM component not started on intIfNum(%d)",intIfNum);
         return L7_FALSE;
       }
    }
                                          ...
                                          L7_uint32 helloIntvl)
{
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "intIfNum(%d) not valid for PIM-SM configuration",intIfNum);
    return L7_FAILURE;
  }
  /* range checking */
...
L7_RC_t pimsmMapPimTrapNeighborLoss(L7_uchar8 familyType, L7_uint32 rtrIntNum, L7_uint32 neighborUpTime)
{
    ...
    {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                      "Failed to convert rtrIfNum(%d) to intIfNum",rtrIntNum);
      return L7_FAILURE;
    }
                                  ...
                                  L7_uint32 intIfNum)
{
    ...
    PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,
                     "PIMSM not enabled on intIfNum(%d) for familyType %d",intIfNum,familyType);
    return L7_FAILURE;
                                      ...
                                      L7_uint32 *pIntIfNum)
{
  ...
  PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,
                   "Failed to get the next PIMSM enabled interface after intfIfNum(%d) for familyType %d\n",
    ...
    L7_inet_addr_t *rpGroupMask, L7_inet_addr_t *rpAddress)
{
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Invalid args:rpGroupAddress(%p) or rpGroupMask(%p) or rpAddress(%p) passed",
        ...
        L7_uint32 intIfNum, L7_uint32 *drPriority)
{
   ...
   if (pimsmMapInterfaceIsConfigurable(pimsmMapCbPtr, intIfNum, &pCfg) != L7_TRUE)
   {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"intIfNum(%d) not valid for PIM-SM configuration",intIfNum);
      return L7_FAILURE;
   }
                                ...
                                L7_uint32 *pIndex)
{
  ...
  *pIndex = L7_MAX_CAND_RP_NUM;
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Invalid Rp Address(%s) or Grp Address(%s) or prefix Len(%d) input",
                    inetAddrPrint(candRPAddr,rpAddr),
  ...
  PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                  "Failed to find candidate RP Entry for RP(%s) and GrpAddr(%s)",
                  inetAddrPrint(candRPAddr,rpAddr),
                                ...
                                pimsmCfgCandRPdata_t **candRpEntryGet)
{
  ...
  if ((inetIsAddressZero(candRPAddr) == L7_TRUE) ||
     ...
     (prefixLen == L7_NULL))
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"Invalid Rp Address(%s) or Grp Address(%s) or prefix Len(%d) input",
                    inetAddrPrint(candRPAddr,rpAddr),
                    inetAddrPrint(candRPGrpAddr,grpAddr),prefixLen);
    return L7_FAILURE;
  }
    ...
    L7_inet_addr_t *rpGrpMask, L7_uint32 mode)
{
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Group Address(%s) not in multicast range", inetAddrPrint(rpGrpAddr,grpAddr));
    return L7_FAILURE;
  }
  ...
  if (mode == L7_ENABLE)
  {
    for (index = 0; index < L7_MAX_CAND_RP_NUM; index++)
    {
      ...
      if(pimsmCrpEntry->intIfNum > 0)
      {
        ...
        {
           PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                           "This candidate RP group combination is already set for GrpAddress(%s) and prefixLen(%d)",
                           inetAddrPrint(rpGrpAddr,grpAddr),
                           paramPrefixLen);
           return L7_ALREADY_CONFIGURED;
        }
  ...
  else if (mode == L7_DISABLE)
  {
    ...
    {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                      "GrpAddress(%s) and prefixLen(%d) not found in RP table",
                      inetAddrPrint(rpGrpAddr,grpAddr), paramPrefixLen);
      return L7_FAILURE;
    }
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_APIS,
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,"Failed to apply the RP candidate (ENABLE/DISABLE)");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
    ...
    L7_uchar8 *candPrefixLen)
{
    ...
    {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                      "Invalid input parameters: candRpAddress(%p) or cRpGroupAddress(%p) or candPrefixLen(%p)",
                      (void *)candRpAddress,(void *)cRpGroupAddress, (void *)candPrefixLen);
      return L7_FAILURE;
    }
                              ...
                              L7_uint32 *sgPimMode)
{
  ...
  return pimsmMapExtenSGPimModeGet (pimsmMapCbPtr, pimSGGrpAddr, pimSGSrcAddr,
                                    sgPimMode);
}
     ...
     if (found !=L7_TRUE)
      {
         ...
         PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                         "Failed to find next Static RP entry for Grp Address(%s)",inetAddrPrint(rpGrpAddr,groupAddr));
        return L7_FAILURE;
       }
 ...
 L7_RC_t pimsmMapCliCandidateRPEntryNextGet(L7_uchar8 familyType,L7_uint32 *intIfNum,L7_inet_addr_t *cRPGroupAddress,
...

  {
   ...
   if ((intIfNum == L7_NULLPTR) || (cRPGroupAddress == L7_NULLPTR) ||
    ...
    {
      PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                      "Invalid input parameters: intIfNum(%p) or cRpGroupAddress(%p) or  candPrefixLen(%p)",
                      intIfNum,cRPGroupAddress, cRPprefixLen);
      return L7_FAILURE;
    }
     ...
     if (found !=L7_TRUE)
      {
         ...
         PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                         "Failed to find next Static RP entry for Grp Address(%s)",
                         inetAddrPrint(cRPGroupAddress,groupAddr));
        return L7_FAILURE;
       }
