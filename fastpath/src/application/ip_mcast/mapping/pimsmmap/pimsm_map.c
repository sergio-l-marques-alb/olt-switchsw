...
void pimsmResetDataChanged(void)
{
  ...
  if (pimsmMapCtrlBlockGet(L7_AF_INET, &pimsmMapCbPtr) == L7_SUCCESS)
    pimsmMapCbPtr->pPimsmMapCfgData->cfgHdr.dataChanged = L7_FALSE;
  if (pimsmMapCtrlBlockGet(L7_AF_INET6, &pimsmMapCbPtr) == L7_SUCCESS)
    pimsmMapCbPtr->pPimsmMapCfgData->cfgHdr.dataChanged = L7_FALSE;
  return;
...
L7_RC_t pimsmApplyConfigData(pimsmMapCB_t *pimsmMapCbPtr)
{
  ...
  for (index=0;index<L7_MAX_CAND_RP_NUM;index++)
  {
    ...
    if ((pCandRp->intIfNum != 0) && (nimCheckIfNumber(pCandRp->intIfNum) == L7_SUCCESS))
    {
      ...
      {
        PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                        "mcastIpMapRtrIntfGlobalIpAddressGet() failed! intIfNum = %d",pCandRp->intIfNum);
        return L7_FAILURE;
      }
      ...
      {
        PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, 
                         "Failed to apply Candidate RP(Enable/Disable) for CandRP intfNum(%d) ",pCandRp->intIfNum);
      }
    }
  ...
  if ((pCandBsr->intIfNum != 0) && (nimCheckIfNumber(pCandBsr->intIfNum) == L7_SUCCESS))
  {
    ...
    {
      PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, 
                       "Failed to apply BSR (Enable/Disable) for BSR intfNum(%d) ",
                       pCandBsr->intIfNum);
    }
  }
                                        ...
                                        ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  {
    PIMSM_MAP_DEBUG(PIMSM_MAP_DEBUG_FAILURES,
                    "Invalid eventType(%d) for intfIfNum(%d)",
                      event,intIfNum);
                                   ...
                                   ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  if (pimsmMapPimsmAdminModeGet(pimsmMapCbPtr->familyType, &glblAdminMode) != L7_SUCCESS) 
  {
      ...
      ((intIfNum != 0) && (pimsmIntfIsValid(intIfNum) == L7_TRUE)))
  {
    ...
    case L7_RTR_INTF_DISABLE_PENDING:
      if (glblAdminMode == L7_ENABLE)
      {
        if ((pimsmMapInterfaceModeGet(pimsmMapCbPtr->familyType, intIfNum, &mode) == L7_SUCCESS) && 
            (mode == L7_ENABLE))
        {
    ...
    case L7_RTR_STARTUP_DONE:
      ...
      /* 
         ...
         - all physical ports are created
         - config has been applied
         - all router interfaces we are going to get are created and in their final state
       */
      PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_EVENTS, "pimsm received L7_RTR_STARTUP_DONE event");

      pimsmMapRtrStartupDoneProcess(pimsmMapCbPtr);
      break;
                                  ...
                                  ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  if (pimsmMapPimsmAdminModeGet(pimsm6MapCbPtr->familyType, &glblAdminMode) != L7_SUCCESS)
  {
    glblAdminMode = L7_DISABLE;
  }
      ...
      ((intIfNum != 0) && (pimsmIntfIsValid(intIfNum) == L7_TRUE)))
  {
      ...
      case RTR6_EVENT_INTF_CREATE:
        rc = pimsmIntfCreate(intIfNum, pimsm6MapCbPtr);
        break;

      case RTR6_EVENT_INTF_DELETE:
        if (pimsmMapInterfaceIsConfigurable(pimsm6MapCbPtr, intIfNum, &pCfg) == L7_TRUE)
        {
          pimsmIntfBuildDefaultConfigData(pCfg);
          memset((void *)&(pCfg->configId), 0, sizeof(nimConfigID_t));
          pimsm6MapCbPtr->pPimsmMapCfgMapTbl[intIfNum] = 0;
        }
        break;
      ...
      case RTR6_EVENT_STARTUP_DONE:
        /* signal that all router events for the startup phase have been issued */
        PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_EVENTS, "pimsm received RTR6_EVENT_STARTUP_DONE event");

        pimsmMapRtrStartupDoneProcess(pimsm6MapCbPtr);
        break;
                                              ...
                                              void *eventMsg)
{
  ...
  {
    PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,"Failed to post event(%d) to the PIMSM Map Message Queue ",eventType);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
...
/*********************************************************************
* @purpose  Do common processing in response to the L7_RTRn_STARTUP_DONE
*           events.
*
* @param    pimsmMapCbPtr   @b{(input)} mapping layer control block
*
*
* @end
*********************************************************************/
static void pimsmMapRtrStartupDoneProcess(pimsmMapCB_t *pimsmMapCbPtr)
{
  ...
  if (pimsmMapCbPtr->warmRestartInProgress == L7_TRUE)
  {
    if ((pimsmMapPimsmAdminModeGet(pimsmMapCbPtr->familyType, &mode) == L7_SUCCESS) && (mode == L7_ENABLE))
    {
      osapiTimerAdd(pimsmMapStartupTimerExpireCallback, pimsmMapCbPtr->familyType, 0, 
                    PIMSM_WARM_RESTART_INTERVAL_MSEC, 
                    &(pimsmMapCbPtr->pimsmMapStartupTimer));

      if (pimsmMapCbPtr->pimsmMapStartupTimer == L7_NULLPTR)
      {
        PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES,
        ...
        pimsmMapStartupTimerExpireProcess(pimsmMapCbPtr);
      }
    }
    else
    {
      /* component is disabled, invoke timer expiration processing immediately */
      PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, "Component disabled. pimsmMapCbPtr->familyType = %d",
                    ...
                    L7_uint32 event, pimsmMapCfgSetEventInfo_t *pData)
{
  ...
  switch (event)
  {
    ...
    case PIMSMMAP_RESTART_MODE_EXIT_EVENT:
      pimsmMapStartupTimerExpireProcess(pimsmMapCbPtr);      
      break;
...
void pimsmMapStartupTimerExpireProcess(pimsmMapCB_t *pimsmMapCbPtr)
{
  L7_uint32    cbIndex;
  L7_uint32    protoMax;
  L7_BOOL      restartInProgress;

  PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_EVENTS, "Processing timer expired for familyType = %d", 
                   pimsmMapCbPtr->familyType);

  pimsmMapCbPtr->warmRestartInProgress = L7_FALSE;
  pimsmMapExtenRestartFlagClear(pimsmMapCbPtr);
  ...
  for (cbIndex = L7_NULL, pimsmMapCbPtr = &pimsmMapCB[0]; 
        ((cbIndex < protoMax) && (restartInProgress == L7_FALSE)); 
        cbIndex++, pimsmMapCbPtr++)
  {
    if (L7_TRUE == pimsmMapCbPtr->warmRestartInProgress)
    {
...
void pimsmMapStartupTimerExpireCallback(L7_uint32 familyType, L7_uint32 arg2)
{
  ...
  if (pimsmMapCbPtr->pimsmMapStartupTimer != L7_NULLPTR)
  {
    osapiTimerFree(pimsmMapCbPtr->pimsmMapStartupTimer);
    pimsmMapCbPtr->pimsmMapStartupTimer = L7_NULLPTR;
  }

  /* enqueue this event to the pimsmMapTask */
  if (pimsmMapCfgSetEventQueue(pimsmMapCbPtr, PIMSMMAP_RESTART_MODE_EXIT_EVENT, L7_NULLPTR, 0) != L7_SUCCESS)
  {
    PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, "Error returned from pimsmMapCfgSetEventQueue()");
    return;
  }
}
...
L7_RC_t pimsmMessageSend(PIMSM_QUEUE_ID_t QIndex,void *msg)
{
  ...
  if (osapiMessageSend (pimsmGblVariables_g.pimsmQueue[QIndex].QPointer, (L7_VOIDPTR) msg,
                     ...
                     != L7_SUCCESS)
  {
    PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, "PIMSM Map Queue of queueIndex:%d  Failed while sending the message :.\n", QIndex);
    if (osapiSemaTake (pimsmGblVariables_g.queueCountersSemId, L7_WAIT_FOREVER)
                    == L7_SUCCESS)
    {
...
L7_RC_t pimsmApplyDebugConfigData(void)
{
       ...
       familyIndex++)
  {
    ...
    { 
       ...
       {
         rc = pimsmDebugTraceFlagSet(familyIndex+1, flagIndex, L7_TRUE);
       }
       else
       {
         rc = pimsmDebugTraceFlagSet(familyIndex+1, flagIndex, L7_FALSE);
       }
    }
  }
  return rc;
