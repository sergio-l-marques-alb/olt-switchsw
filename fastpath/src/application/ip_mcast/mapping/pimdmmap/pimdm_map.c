...
void pimdmResetDataChanged(void)
{
  ...
  if (pimdmMapCtrlBlockGet(L7_AF_INET, &pimdmMapCbPtr) == L7_SUCCESS)
    pimdmMapCbPtr->pPimdmMapCfgData->cfgHdr.dataChanged = L7_FALSE;
  if (pimdmMapCtrlBlockGet(L7_AF_INET6, &pimdmMapCbPtr) == L7_SUCCESS)
    pimdmMapCbPtr->pPimdmMapCfgData->cfgHdr.dataChanged = L7_FALSE;
  return;
}
/*********************************************************************
                                  ...
                                  ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  if (pimdmMapPimdmAdminModeGet(L7_AF_INET, &glblAdminMode) != L7_SUCCESS)
  {
      ...
      (pimdmIntfIsValid (intIfNum) == L7_TRUE)))
  {
    ...
    case L7_RTR_INTF_CREATE:
      rc = pimdmIntfCreate(intIfNum, pimdmMapCbPtr);
      break;
    ...
    case L7_RTR_STARTUP_DONE:
      ...
      PIMDM_MAP_DEBUG (PIMDM_MAP_DEBUG_EVENTS, "pimdm received L7_RTR_STARTUP_DONE event");
      pimdmMapRtrStartupDoneProcess(pimdmMapCbPtr);
                                  ...
                                  ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  if (pimdmMapPimdmAdminModeGet(L7_AF_INET6, &glblAdminMode) != L7_SUCCESS)
  {
    glblAdminMode = L7_DISABLE;
  }
      ...
      (pimdmIntfIsValid (intIfNum) == L7_TRUE)))
  {
    ...
    case RTR6_EVENT_INTF_CREATE:
      rc = pimdmIntfCreate(intIfNum, pimdm6MapCbPtr);
      break;

    case RTR6_EVENT_INTF_DELETE:
      if (pimdmMapIntfIsConfigurable(pimdm6MapCbPtr, intIfNum, &pCfg) == L7_TRUE)
      {
        pimdmIntfBuildDefaultConfigData(pCfg);
        memset((void *)&(pCfg->configId), 0, sizeof(nimConfigID_t));
        pimdm6MapCbPtr->pPimdmMapCfgMapTbl[intIfNum] = 0;
      }
      break;
    ...
    case RTR6_EVENT_STARTUP_DONE:
      /* signal that all router events for the startup phase have been issued */
      PIMDM_MAP_DEBUG (PIMDM_MAP_DEBUG_EVENTS, "pimdm received RTR6_EVENT_STARTUP_DONE event");

      pimdmMapRtrStartupDoneProcess(pimdm6MapCbPtr);
      break;
...
void pimdmMapStartupTimerExpireCallback(L7_uint32 familyType, L7_uint32 arg2)
{
  ...
  pimdmMapCbPtr->warmRestartInProgress = L7_FALSE;
  if (pimdmMapCbPtr->pimdmMapStartupTimer != L7_NULLPTR)
  {
    /* free the osapiTimer used to trigger this callback */
    osapiTimerFree(pimdmMapCbPtr->pimdmMapStartupTimer);
    pimdmMapCbPtr->pimdmMapStartupTimer = L7_NULLPTR;
  }
  ...
  for (cbIndex = L7_NULL, pimdmMapCbPtr = &pimdmMapCB_g[0]; 
        ((cbIndex < protoMax) && (restartInProgress == L7_FALSE)); 
        cbIndex++, pimdmMapCbPtr++)
  {
    if (L7_TRUE == pimdmMapCbPtr->warmRestartInProgress)
    {
  ...
  {
    (void)cnfgrApiComponentHwUpdateDone(L7_FLEX_PIMDM_MAP_COMPONENT_ID, L7_CNFGR_HW_APPLY_IPMCAST);
  }
}
...
static void pimdmMapRtrStartupDoneProcess(pimdmMapCB_t *pimdmMapCbPtr)
{
  L7_uint32 glblAdminMode;

  if (pimdmMapPimdmAdminModeGet(pimdmMapCbPtr->familyType, &glblAdminMode) != L7_SUCCESS)
  {
    glblAdminMode = L7_DISABLE;
  }

  if (pimdmMapCbPtr->warmRestartInProgress == L7_TRUE)
  {
    if (glblAdminMode == L7_ENABLE)
    {
      osapiTimerAdd(pimdmMapStartupTimerExpireCallback, pimdmMapCbPtr->familyType, 0, 
                    PIMDM_WARM_RESTART_INTERVAL_MSEC, 
                    &(pimdmMapCbPtr->pimdmMapStartupTimer));
      ...
      if (pimdmMapCbPtr->pimdmMapStartupTimer == L7_NULLPTR)
      {
        ...
        pimdmMapStartupTimerExpireCallback(pimdmMapCbPtr->familyType, 0);
      }
    }
    else
    {
      /* since we are disabled, declare warm restart interval complete now */
      pimdmMapStartupTimerExpireCallback(pimdmMapCbPtr->familyType, 0);
    }
  }
}
...
L7_RC_t pimdmApplyDebugConfigData(void)
{
       ...
       familyIndex++)
  {
    ...
    { 
       ...
       {
         rc = pimdmDebugTraceFlagSet(familyIndex+1, flagIndex, L7_TRUE);
       }
       else
       {
         rc = pimdmDebugTraceFlagSet(familyIndex+1, flagIndex, L7_FALSE);
       }
    }
  }
  return rc;
