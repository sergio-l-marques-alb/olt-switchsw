...
{                          
  ...
  if (((event != L7_RTR_ENABLE) &&
       ...
       (event != L7_RTR_INTF_CREATE) &&
       (event != L7_RTR_INTF_CREATE)) ||
      ((intIfNum != 0) && (mcastIntfIsValid(intIfNum) != L7_TRUE)))
  {
                                  ...
                                  ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  (void)mcastMapMcastAdminModeGet(&mode);
  ...
  case L7_RTR_DISABLE_PENDING:
    if (mode == L7_ENABLE)
    {
      ...
      /* Do Notify here */
      if (mcastMapMcastIsOperational(L7_AF_INET)== L7_FALSE)
      {
  ...
  case L7_RTR_STARTUP_DONE:
    /* if warm restart and component enabled, start warm restart processing */
    if ((mcastGblVariables_g.warmRestart == L7_TRUE) &&
        (mode == L7_ENABLE))
    {
      /* system restart was Warm and MCAST is enabled so start
       * warm restart interval timer 
       */
      rc = mfcWarmRestartTimerStart();
    }
    else
    {
      /* MCAST is disabled or restart is COLD so signal restart period complete immediately */
                                  ...
                                  ASYNC_EVENT_NOTIFY_INFO_t *pEventInfo)
{
  ...
  switch (event)
  {
    case RTR6_EVENT_ENABLE:
      ...
      /* Do Notify here */
      if (mcastMapMcastIsOperational(L7_AF_INET6)== L7_TRUE)
      {
        mcastMapAdminModeEventChangeNotify(MCAST_RTR_ADMIN_MODE_ENABLED,
                                           L7_NULLPTR);
      }
    ...
    case RTR6_EVENT_DISABLE_PENDING:
      ...
      /* Do Notify here */
      if (mcastMapMcastIsOperational(L7_AF_INET6)== L7_FALSE)
      {
        mcastMapAdminModeEventChangeNotify(MCAST_RTR_ADMIN_MODE_DISABLE_PENDING,
                                           L7_NULLPTR);
      }
...
* @end
*********************************************************************/
void mcastMapWarmRestartPeriodComplete(void)
{   
  if (mcastGblVariables_g.warmRestartInProgress == L7_TRUE)
  {
    mcastGblVariables_g.warmRestartInProgress = L7_FALSE;
  }
  (void)cnfgrApiComponentHwUpdateDone(L7_FLEX_MCAST_MAP_COMPONENT_ID, L7_CNFGR_HW_APPLY_IPMCAST);
}
...
* @end
*********************************************************************/
L7_RC_t mcastMapMRPEventMsgSend(mcastEventTypes_t eventType,
                               L7_VOIDPTR pMsg,
                               L7_uint32 msgLen)
{
  mcastMapMsg_t msg;
...
L7_RC_t mcastApplyDebugConfigData(void)
{
       ...
       familyIndex++)
  {
    ...
    { 
       ...
       {
         rc = mcastDebugTraceFlagSet(familyIndex+1, flagIndex, L7_TRUE);
       }
       else
       {
         rc = mcastDebugTraceFlagSet(familyIndex+1, flagIndex, L7_FALSE);
       }
    }
  }
  return rc;
