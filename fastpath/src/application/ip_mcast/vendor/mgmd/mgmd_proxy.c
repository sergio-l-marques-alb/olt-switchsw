                                                  ...
                                                  L7_uint32  rtrIfNum)
{
  ...
  if ((srcsCount == 0)  && ((mgmdCB->mgmd_host_info->hostCompatibilityMode == L7_MGMD_VERSION_1)
                              || (mgmdCB->mgmd_host_info->hostCompatibilityMode == L7_MGMD_VERSION_2))) 
  {
    /* rx'ed V2 join downstream");
    so send a IS_EXC(NULL) report upstream */

    MGMD_PROXY_DEBUG(MGMD_PROXY_DEBUG_APIS," Rx'ed join dnstream ");
                                                ...
                                                L7_uint32          rtrIfNum)
{
  ...
  L7_uint32 srcExpiryTime = 0;

  MGMD_PROXY_DEBUG(MGMD_PROXY_DEBUG_APIS,"\n  Entered ");

  if(mgmdMapCtrlBlockGet(mgmd_host_group->group.family, &mgmdMapCbPtr) != L7_SUCCESS)
  {
    MGMD_PROXY_DEBUG(MGMD_PROXY_DEBUG_APIS, "Failed to get control block\n");
    return ; 
  }
  ...
  for (index=0; index < numSrcs; index++)
  {
    ...
    if ((srcRec = (mgmd_host_source_record_t *) SLLFind(&(mgmd_host_group->sourceRecords),(L7_sll_member_t *)&srcRecord)) != L7_NULL)
    {
     if((mgmdSrcExpiryTimeGet(mgmdMapCbPtr->cbHandle, &(mgmd_host_group->group), 
                                rtrIfNum, &(srcRecord.sourceAddress), &srcExpiryTime)) == L7_SUCCESS)
     {
       srcRec->sourceInterval = srcExpiryTime;
     }
     else
     {
  ...
  case L7_IGMP_MODE_IS_INCLUDE:            
    {
      if (mgmd_host_group->filterMode == MGMD_FILTER_MODE_INCLUDE)
      {
        ...
        mgmd_proxy_src_tmrs_grp_rec_update (mgmd_host_group, numSrcs,
                                            sourceList, IGMP_PROXY_MALI, ifNum);
      }
        else
