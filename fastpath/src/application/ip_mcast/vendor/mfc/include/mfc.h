...
typedef struct mfcTimers_s
{
  ...
  osapiTimerDescr_t     *pWarmRestartTimer;  /* controls length of warm restart period */
  L7_uint32              warmRestartRepeatCount; /* tracks how many times the warm restart interval action
                                                    has repeated */
}mfcTimers_t;
...
/* in milli-seconds - time values used to control processing during a system warm restart */
#define MFC_WARM_RESTART_TIMER_INITIAL_DELAY_INTERVAL_MSECS  (40 * 1000)
#define MFC_WARM_RESTART_TIMER_PERIODIC_INTERVAL_MSECS  (1 * 1000)
#define MFC_WARM_RESTART_TIMER_REPEAT_COUNT_LIMIT   (20)
