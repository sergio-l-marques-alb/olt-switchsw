...
static L7_RC_t filterIntfListGet(L7_INTF_MASK_t* mask, L7_BOOL checkNimState, L7_uint32* numIntf, L7_uint32 intfList[])
{
  ...
  L7_INTF_FLMASKBIT(tempMask, index);
  numIntfTemp = 0;
  while (index != 0)
  {
                                                  ...
                                                  L7_uint32 vlanId)
{
  ...
  if (dot1qVlanQueueSyncCheckValid(vlanId) != L7_SUCCESS)
  {
    /* vlan does not exist */
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Vlan %d does not exist . Cannot create static filter.",vlanId);
    return entry;
  }
...
* @end
*********************************************************************/
L7_RC_t filterSrcIntfMaskModify(L7_uchar8* macAddr,
                                L7_uint32 vlanId,
                                L7_INTF_MASK_t *srcIntfMask)
{
  ...
  L7_BOOL srcPresent;
  L7_BOOL entryMulticast;
  L7_uint32 srcIntf, srcIntfCount = 0;
  ...
  for (srcIntf = 1; srcIntf < L7_MAX_INTERFACE_COUNT;  srcIntf++)
  {
    if (L7_INTF_ISMASKBITSET(*srcIntfMask, srcIntf))
    {
      if (filterIntfCheck(srcIntf) == L7_SUCCESS)
      {
        srcIntfCount++;
      }
      else
      {
        L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
        ifName[0] = L7_NULL;
        nimGetIntfName(srcIntf, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FILTER_COMPONENT_ID,
                "Invalid interface %s used to configure source port of mac filter"
                "%02X:%02X:%02X:%02X:%02X:%02X vlan %d", ifName,
                macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], 
                macAddr[5], vlanId);
        L7_INTF_CLRMASKBIT(*srcIntfMask, srcIntf);
      }
    }
  }
  ...
  {
    /*entry not found*/
    L7_LOGF(L7_LOG_SEVERITY_DEBUG, L7_FILTER_COMPONENT_ID,
            "MAC Filter %02X:%02X:%02X:%02X:%02X:%02X vlan %d does not exists.",
            macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], 
            macAddr[5], vlanId);
    rc = osapiSemaGive(filterAvlTree.semId);
    return L7_FAILURE;
  }
  /* this filter was created .
     Check if this filter exceeds the maximum number of static
    multicast entries allowed */
  if ((entryMulticast == L7_TRUE) &&
      (srcPresent==L7_TRUE) &&
      (filterEntry->numSrc==0))
  {
    if (numNonMfdbEntries <= L7_MAX_FDB_STATIC_FILTER_ENTRIES)
    {
      /*call dtl routine to create a filter*/
      ...
      else
      {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_FILTER_COMPONENT_ID,
                "Failed to add MAC Filter %02X:%02X:%02X:%02X:%02X:%02X vlan %d to hardware", 
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FILTER_COMPONENT_ID,
              "Maximum number of static multicast MAC Filters configured. "
              "Maximum allowed are %d",
  ...
  if (srcIntfCount > L7_FILTER_MAX_INTF)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FILTER_COMPONENT_ID,
            "Maximum number of interfaces configured as source interfaces. Maximum allowed %d, requested %d",
            L7_FILTER_MAX_INTF, srcIntfCount);
    rc = osapiSemaGive(filterAvlTree.semId);
    return L7_TABLE_IS_FULL;
  }
  ...
  if (memcmp(srcIntfMask, &backupEntry.srcIntfMask, sizeof(backupEntry.srcIntfMask)) == 0)
  {
    rc = osapiSemaGive(filterAvlTree.semId);
    return rc;
  }
  memcpy(&backupEntry.srcIntfMask, srcIntfMask, sizeof(backupEntry.srcIntfMask));
  backupEntry.numSrc = srcIntfCount;
                         ...
                         L7_BOOL   adminAction)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FILTER_COMPONENT_ID,
            "Maximum number of interfaces configured as source interfaces. Maximum allowed %d",
            L7_FILTER_MAX_INTF);
    rc = osapiSemaGive(filterAvlTree.semId);
    return L7_FAILURE;
  }
                               ...
                               L7_uint32 dstIntf)
{
  ...
  if (L7_INTF_ISMASKBITSET(backupEntry.dstIntfFilterMask, dstIntf) == L7_FALSE)
  {
    /* Only update the filter entry if this is unique. Allowing duplicate 
     * ports to go through so that they can be added in mfdb if they come from intfApply
     * code path
     */
    L7_INTF_SETMASKBIT(backupEntry.dstIntfFilterMask,dstIntf);
  }
  ...
  if (filterCfgData->filteringEnabled == L7_TRUE)
  {
    ...
    if (entryMulticast == L7_TRUE)
    {
          ...
          (nimQueryData.data.state == L7_INTF_ATTACHED))
      {
        memset(&tmp, 0, sizeof(L7_INTF_MASK_t));
        L7_INTF_SETMASKBIT(tmp, dstIntf);
        rc = filterMfdbEntryAdd(vlanId,macAddr,L7_NULL, &tmp);
      }
                                  ...
                                  L7_uint32 dstIntf)
{
  ...
  /*entry is found proceed to delete*/
  if (L7_INTF_ISMASKBITSET(backupEntry.dstIntfFilterMask, dstIntf))
  {
    L7_INTF_CLRMASKBIT(backupEntry.dstIntfFilterMask,dstIntf);
      ...
      if (entryMulticast == L7_TRUE)
      {
        ...
        */
            ...
            (nimQueryData.data.state == L7_INTF_DETACHING))
        {
          memset(&tmp, 0, sizeof(L7_INTF_MASK_t));
          L7_INTF_SETMASKBIT(tmp, dstIntf);
          rc = filterMfdbEntryPortsDelete(vlanId,macAddr,L7_NULLPTR,&tmp);
                              ...
                              L7_INTF_MASK_t filterMask)
{
  ...
  L7_uint32 indx;
  L7_NIM_QUERY_DATA_t nimQueryData;
  L7_INTF_MASK_t intfNotPresentMask;

  memset(&tempMask, 0, sizeof(tempMask));
  memset(&intfNotPresentMask, 0, sizeof(intfNotPresentMask));
  L7_INTF_MASKOREQ(tempMask,fwdMask);
  L7_INTF_MASKOREQ(tempMask,filterMask);


  /*check if we have an acceptable dstIntf
    clean out the interfaces that are not valid.
  */
  nimQueryData.request = L7_NIM_QRY_RQST_STATE;
  ...
  while (indx != 0 )
  {
    ...
    if (filterIntfCheck(indx) != L7_SUCCESS)
    {
      L7_INTF_CLRMASKBIT(fwdMask, indx);
      L7_INTF_CLRMASKBIT(filterMask, indx);
    }
    else if ((nimIntfQuery(&nimQueryData) != L7_SUCCESS) ||
             ((nimQueryData.data.state != L7_INTF_ATTACHING) &&
              (nimQueryData.data.state != L7_INTF_ATTACHED)))
    {
      L7_INTF_SETMASKBIT(intfNotPresentMask, indx);
    }
  ...
  */
  ...
  if (fwdMaskNonZero)
  {
    ...
    /* Do not add the intfs not present to h/w*/
    L7_INTF_MASKANDEQINV(fwdMask, intfNotPresentMask);
  }

  if (filterMaskNonZero)
  {
    L7_INTF_MASKOREQ(filterEntry->dstIntfFilterMask, filterMask);
    L7_INTF_MASKANDEQINV(filterMask, intfNotPresentMask);
  }
...
L7_uint32 filterRemove(L7_uchar8* macAddr, L7_uint32 vlanId)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FILTER: dtl filter delete failure for vlanid %u "
...
L7_RC_t filterMacAddrCheck(L7_uchar8* macAddr)
{
  ...
  if (macAddr[0] == 0x01 && macAddr[1] == 0x80 && macAddr[2] == 0xC2)
  {
    return L7_FAILURE;
  }
  return L7_SUCCESS;
...
void filterDebugShow()
{
  printf("Num Filter Entries = %d\n", numFilterEntries);
  printf("Num MfdbEntries = %d\n", numMfdbEntries);
  printf("Num Non MfdbEntries = %d\n", numNonMfdbEntries);
  printf("L7_MAX_GROUP_REGISTRATION_ENTRIES = %d\n", L7_MAX_GROUP_REGISTRATION_ENTRIES);
  printf("L7_MAX_STATIC_FILTER_ENTRIES = %d\n", L7_MAX_STATIC_FILTER_ENTRIES);
  printf("L7_MAX_FDB_STATIC_FILTER_ENTRIES = %d and L7_MFDB_MAX_MAC_ENTRIES = %d\n", L7_MAX_FDB_STATIC_FILTER_ENTRIES,L7_MFDB_MAX_MAC_ENTRIES);
