...
L7_RC_t filterAdminModeApply(L7_BOOL enable)
{
   ...
   L7_INTF_MASK_t filterMask, fwdMask, tempMask;
   L7_uint32 indx;
   L7_NIM_QUERY_DATA_t nimQueryData;
   filterVlanCfgData_t *vCfg;
   L7_uint32 type,i;
   L7_uchar8 mode;
   ...
   while (filterEntry != L7_NULLPTR)
   {
     ...
     */
     ...
     if (filterIsMacAddrTypeMulticast(macAddr) == L7_TRUE)
     {
       ...
       if (enable == L7_TRUE)
       {
         /* Add only the valid interfaces*/
         nimQueryData.request = L7_NIM_QRY_RQST_STATE;

         memset(&tempMask, 0, sizeof(tempMask));
         memcpy(&fwdMask, &filterEntry->dstIntfMask, sizeof(fwdMask));
         memcpy(&filterMask, &filterEntry->dstIntfFilterMask, 
                sizeof(filterMask));
         L7_INTF_MASKOREQ(tempMask,fwdMask);
         L7_INTF_MASKOREQ(tempMask,filterMask);

         L7_INTF_FHMASKBIT(tempMask, indx);
         while (indx != 0 )
         {
           nimQueryData.intIfNum = indx;
           if ((nimIntfQuery(&nimQueryData) != L7_SUCCESS) ||
               ((nimQueryData.data.state != L7_INTF_ATTACHING) &&
                (nimQueryData.data.state != L7_INTF_ATTACHED)))
           {
             L7_INTF_CLRMASKBIT(fwdMask, indx);
             L7_INTF_CLRMASKBIT(filterMask, indx);
           }

           L7_INTF_CLRMASKBIT(tempMask, indx);
           L7_INTF_FHMASKBIT(tempMask, indx);
         }

         rc = filterMfdbEntryAdd(vid,macAddr, &fwdMask, &filterMask);
       }
       else
       {
   ...
   {
     vCfg =&filterCfgData->filteringVlanCfgData[i]; 
     ...
     if (dot1qVlanStatusGet(vCfg->vlanId, &type) == L7_SUCCESS)
     {	
       if(enable == L7_TRUE)
       {
         mode = vCfg->mode;
       ...
       {
         mode = FD_VLAN_FILTERING_MODE;
       }

       if (dtlL2McastVlanFloodModeSet(vCfg->vlanId, mode) 
           != L7_SUCCESS)
       {    
         L7_LOGF(L7_LOG_SEVERITY_ERROR,L7_FILTER_COMPONENT_ID,
                 "mfdbVlanAdd: failed to apply VLAN %u PFM default\n", 
                 vCfg->vlanId);
       }
     }
   }
