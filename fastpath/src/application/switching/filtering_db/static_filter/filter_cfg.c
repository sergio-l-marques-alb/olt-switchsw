...
PORTEVENT_MASK_t filterPortEventMask_g;
...
* @end
*********************************************************************/
L7_BOOL filterIntfIsAttached(L7_uint32 intIfNum)
{
  ...
  *pReason   = 0;
      ...
      (filterTreeHeap == L7_NULLPTR) || (filterDataHeap == L7_NULLPTR))
  {
    ...
    *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Filter: Unable to allocate resources\n");
    return filterRC;
  }
  ...
  *pReason   = 0;
  ...
  if ( filterRC != L7_SUCCESS )
  {
    ...
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Filter: Unable to register with nvStore\n");
    return filterRC;
  }
  ...
  if ((filterRC = nimRegisterIntfChange(L7_FILTER_COMPONENT_ID,
                                        filterIntfChangeCallback,
                                        filterStartupNotifyCallback,
                                        NIM_STARTUP_PRIO_FILTER)) != L7_SUCCESS)
  {
    *pResponse = 0;
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    filterRC   = L7_ERROR;
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Filter: Unable to register with NIM\n");
    return filterRC;
  }
                                      ...
                                      L7_FILTER_COMPONENT_ID, eventMask)) != L7_SUCCESS)
  {
    ...
    *pReason   = L7_CNFGR_ERR_RC_FATAL;
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Filter unable to register for Vlan Change callback");
                                      ...
                                      L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason   = 0;
  ...
  memset(&filterPortEventMask_g, 0, sizeof(filterPortEventMask_g));
  nimRegisterIntfEvents(L7_FILTER_COMPONENT_ID, filterPortEventMask_g);
...
void filterCnfgrFiniPhase1Process()
{
  ...
  if (filterAvlTree.semId != L7_NULLPTR)
    if (osapiSemaDelete((void *)filterAvlTree.semId) != L7_SUCCESS)
        L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
                "filterCnfgrFiniPhase1Process:  could not delete avl semaphore\n");
...
L7_RC_t filterSave(void)
{
  ...
  if (filterCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
    ...
    if (sysapiCfgFileWrite(L7_FILTER_COMPONENT_ID, FILTER_CFG_FILENAME,
        ...
        L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
                "filterSave: Error on call to sysapiCfgFileWrite routine on file %s\n", FILTER_CFG_FILENAME);
  }
  ...
  while (rc == L7_SUCCESS)
  {
    ...
    if (filterEntry != L7_NULLPTR)
    {
      for (i = 0; i < L7_MAX_FDB_STATIC_FILTER_ENTRIES; i++)
      {
        ...
        {
          if (filterMacVlanIdExtract(filterEntry->vlanidmacAddr,
                                     cfgMacAddr,&cfgVlanId) == L7_SUCCESS)
          {
            if (memcmp(filterCfgData->filterData[i].macAddr,
                       cfgMacAddr,L7_MAC_ADDR_LEN) == 0 &&
              filterCfgData->filterData[i].vlanId == cfgVlanId)
            {
              memcpy(&filterCfg, &filterCfgData->filterData[i],
                     sizeof(filterData_t));
              break;
            }
          }
    ...
    {
      rc = filterSrcIntfAdd(macAddr, vlanId, intIfNum, L7_FALSE);
    }

    if (filterEntry != L7_NULLPTR &&
          (L7_INTF_ISMASKBITSET(filterEntry->dstIntfFilterMask, intIfNum)))
    {
      rc = filterDstIntfFilterAdd(macAddr, vlanId, intIfNum);
    }
...
L7_BOOL filterMapIntfConfigEntryGet(L7_uint32 intIfNum, filterIntfCfgData_t **pCfg)
{
  ...
  if ((FILTER_IS_READY != L7_TRUE) ||
      (nimConfigIdGet(intIfNum, &configId) != L7_SUCCESS))
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Error on call to filterMapIntfIsConfigEntryGet routine outside the EXECUTE state\n");
    return L7_FALSE;
  }

  /* Avoid N^2 processing when interfaces created at startup */
  if ((nextIndex < L7_FILTER_INTF_MAX_COUNT) &&
      (NIM_CONFIG_ID_IS_EQUAL(&filterCfgData->filterIntfCfgData[nextIndex].configId, &configIdNull)))
  {
...
L7_BOOL filterMapIntfIsConfigurable(L7_uint32 intIfNum, filterIntfCfgData_t **pCfg)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "Error on call to filterMapIntfIsConfigurable routine outside the EXECUTE state\n");
    return L7_FALSE;
  }
...
L7_RC_t filterIntfDetach(L7_uint32 intIfNum)
{
 ...
 /* filterIntfCfgData_t *pCfg;*/
  ...
  while (rc == L7_SUCCESS)
  {
      ...
      if (filterEntry != L7_NULLPTR && L7_INTF_ISMASKBITSET(filterEntry->dstIntfFilterMask, intIfNum))
                rc = filterDstIntfFilterDelete(macAddr, vlanId, intIfNum);
...
void filterStartupNotifyCallback(NIM_STARTUP_PHASE_t startupPhase)
{
  ...
  while (rc == L7_SUCCESS)
  {
    if (filterIntfCheck(intIfNum) == L7_SUCCESS)
    {
      ...
      case NIM_INTERFACE_ACTIVATE_STARTUP:
        ...
        if (filterIntfIsAttached(intIfNum))
        {
          rc = filterApplyIntfConfigData(intIfNum);
        }
        break;
  ...
  {
  case NIM_INTERFACE_CREATE_STARTUP:
    PORTEVENT_SETMASKBIT(filterPortEventMask_g, L7_CREATE);
    PORTEVENT_SETMASKBIT(filterPortEventMask_g, L7_DELETE);
    nimRegisterIntfEvents(L7_FILTER_COMPONENT_ID, filterPortEventMask_g);
    break;

  case NIM_INTERFACE_ACTIVATE_STARTUP:
    /* Add ATTACH/DETACH events to our NIM registration */
    PORTEVENT_SETMASKBIT(filterPortEventMask_g, L7_ATTACH);
    PORTEVENT_SETMASKBIT(filterPortEventMask_g, L7_DETACH);
    nimRegisterIntfEvents(L7_FILTER_COMPONENT_ID, filterPortEventMask_g);
    break;
  ...
  nimStartupEventDone(L7_FILTER_COMPONENT_ID);

  /* Inform cnfgr that static filter has completed it HW updates */
  if (startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    cnfgrApiComponentHwUpdateDone(L7_FILTER_COMPONENT_ID,
...
static void filterVlanAdd(L7_uint32 vlanId)
{
  ...
  for (i = 0; i < L7_MAX_VLANS; i++)
  {
    if (filterCfgData->filteringVlanCfgData[i].vlanId == vlanId)
    {
      if (filterCfgData->filteringEnabled)
      {
        ...
        if (dtlL2McastVlanFloodModeSet(vlanId, filterCfgData->filteringVlanCfgData[i].mode) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
                  "mfdbVlanAdd: failed to apply VLAN %u PFM configuration\n", vlanId);
        }
      } else
      {
        /* apply default vlan mode to the HW */
        if (dtlL2McastVlanFloodModeSet(vlanId, FD_VLAN_FILTERING_MODE) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
  ...
  filterCfgData->filteringVlanCfgData[index].mode = FD_VLAN_FILTERING_MODE;
  if (dtlL2McastVlanFloodModeSet(vlanId, FD_VLAN_FILTERING_MODE) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "mfdbVlanAdd: failed to apply VLAN %u PFM default\n", vlanId);
...
L7_RC_t filterVlanChangeCallback(dot1qNotifyData_t *vlanData,L7_uint32 intIfNum,L7_uint32 event)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "filterVlanChangeCallback is deregistered\n");
    return L7_FAILURE;
  }
  if (!(FILTER_IS_READY))
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            ...
            filterCnfgrState, vlanId, event, ifName);
    return L7_FAILURE;
  }
              ...
              if (filterCfgData->filterData[i].vlanId == vlanId)
              {
                /* check to see if the filter entry already present */
                  ...
                  if (filterEntry ==L7_NULLPTR)
                  {
                    ...
                    {
                      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
                              "filterVlanCallback:Could not create filter for macaddr %02x:%02x:%02x:%02x:%02x:%02x , vlan %d",
