...
* @returns  L7_TABLE_IS_FULL, if mfdb reaches its limit
* @returns  L7_REQUEST_DENIED, if mac address is in the restricted range
*
...
L7_RC_t mfdbEntryAdd(mfdbMemberInfo_t *mfdbMemberInfo)
{
  ...
  /* Invalid MAC check 0180C2XXXXXX are invalid */
  if (mfdbMemberInfo->macAddr[0] == 0x01 &&
      mfdbMemberInfo->macAddr[1] == 0x80 &&
      mfdbMemberInfo->macAddr[2] == 0xC2)
  {
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    {
      ptin_timer_start(17,"mfdbEntryAdd-mfdbDtlAdd");
    ...
    if (rc == L7_SUCCESS)
    {
      ptin_timer_start(18,"mfdbEntryAdd-mfdbUserComponentsNotify");
