...
L7_RC_t mfdbTreeEntryDelete(mfdbData_t *pData)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
          "mfdbTreeEntryDelete: entry does not exist\n");
  return L7_NOT_EXIST;
}
...
L7_RC_t mfdbDtlAdd(mfdbData_t *pData)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
            "mfdbDtlAdd: dtlL2McastEntryAdd failed\n");
  }
...
if (rc != L7_SUCCESS)
   {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
            "mfdbDtlRemove: dtlL2McastEntryDelete failed\n");
   }
...
void mfdbCnfgrFiniPhase1Process()
{
  ...
  if (mfdbTree.semId != L7_NULLPTR)
    if (osapiSemaDelete((void *)mfdbTree.semId) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
              "mfdbCnfgrFiniPhase1Process:  could not delete avl semaphore\n");
...
void mfdbUserComponentsNotify(mfdbData_t *pData, L7_uint32 index, L7_uint32 event)
{
  ...
  if ((rc2 = cnfgrApiComponentNameGet(pData->mfdbUser[index].componentId, name)) != L7_SUCCESS)
  {
  ...
  for (i = 1; i < L7_LAST_COMPONENT_ID; i++)
  {
    if (mfdbNotifyRequest[i].registrarID != L7_NULL)
    {
      ...
      if (rc != L7_SUCCESS)
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
                "Bad rc %d in mfdbUserComponentsNotify for component %d, %s\n", rc,
                (L7_int32)pData->mfdbUser[index].componentId, name);
    }
  }
}
                                                    ...
                                                    L7_COMPONENT_IDS_t componentID)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_MFDB_COMPONENT_ID,
            "MFDB: component ID %d greater than L7_LAST_COMPONENT_ID\n", (L7_int32)componentID);
    return L7_FAILURE;
  }
