                              ...
                              L7_inet_addr_t *leave_grp_addr)
{
  ...
  */
  ...
  {    
    ...
    PT_LOG_TRACE(LOG_CTX_IGMP, "Robustness Variable %u",pSnoopOperEntry->snoopQuerierInfo.sFlagQRV);
  }
...
void snoopQuerierPeriodicQuerySend(snoopOperData_t *pSnoopOperEntry)
{
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Robustness Variable:%u",pSnoopOperEntry->snoopQuerierInfo.sFlagQRV);
  PT_LOG_TRACE(LOG_CTX_IGMP, "Querier's Query Interval Code:%u",pSnoopOperEntry->snoopQuerierInfo.qqic);    
  PT_LOG_TRACE(LOG_CTX_IGMP, "Max Response Code:%u",pSnoopOperEntry->snoopQuerierInfo.maxResponseCode);    
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Sending periodic query to client interfaces (vlanId=%u)",vlanId);
