...
typedef struct snoopPTinProxySourcetimerParams_s
{
  L7_uint8 eventIdx;
} snoopPTinProxySourcetimerParams_t;


#define PTIN_IGMP_TIMER_MSG_SIZE  sizeof(snoopPTinProxySourcetimerParams_t)
...
{
  snoopPTinProxySourcetimerParams_t msg = {0};
  L7_RC_t                     rc;

  rc = osapiMessageSend(cbEventQueue, &msg, PTIN_IGMP_TIMER_MSG_SIZE, L7_NO_WAIT,L7_MSG_PRIORITY_NORM);
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_PTIN_IGMP,"Interfacetimer timer tick send failed");
    return;
  }
}
...
{
  L7_uint32                   status;
  snoopPTinProxySourcetimerParams_t msg;
...
L7_inet_addr_t psourceAddr,qsourceAddr;
L7_uint32      pVlanId,qVlanId;
L7_uint16      pInterfaceIdx,qInterfaceIdx;

  pgroupAddr = ((snoopPTinProxySourcetimer_t *) p)->groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr;
  psourceAddr = ((snoopPTinProxySourcetimer_t *) p)->groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr;
  pVlanId         = ((snoopPTinProxySourcetimer_t *) p)->groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId;
  pInterfaceIdx   = ((snoopPTinProxySourcetimer_t *) p)->groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx;

  qgroupAddr = ((snoopPTinProxySourcetimer_t *) q)->groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr;
  qsourceAddr = ((snoopPTinProxySourcetimer_t *) q)->groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr;
  qVlanId         = ((snoopPTinProxySourcetimer_t *) q)->groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId;
  qInterfaceIdx   = ((snoopPTinProxySourcetimer_t *) q)->groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx;

  if ( pgroupAddr.family == qgroupAddr.family && psourceAddr.family == qsourceAddr.family &&
       pVlanId         == qVlanId         &&
       pInterfaceIdx   == qInterfaceIdx )
  {
    if ((pgroupAddr.family==L7_AF_INET && pgroupAddr.addr.ipv4.s_addr==qgroupAddr.addr.ipv4.s_addr) || 
        (pgroupAddr.family==L7_AF_INET6 && pgroupAddr.addr.ipv6.in6.addr32==qgroupAddr.addr.ipv6.in6.addr32))
      if((psourceAddr.family==L7_AF_INET && psourceAddr.addr.ipv4.s_addr==qsourceAddr.addr.ipv4.s_addr) || 
        (psourceAddr.family==L7_AF_INET6 && psourceAddr.addr.ipv6.in6.addr32==qsourceAddr.addr.ipv6.in6.addr32))
        return 0;
  }
...
{
  snoopPTinProxySourcetimer_t *pTimerData;
  ...
  pTimerData = (snoopPTinProxySourcetimer_t *)ll_member;
...
void timerCallback(void *param)
{
  ...
  snoopPTinProxySourcetimerInfoData_t *groupData;
  ...
  /* Get timer handler */
  pTimerData = (snoopPTinProxySourcetimer_t*) handleListNodeRetrieve(timerHandle);
  ...
  LOG_TRACE(LOG_CTX_PTIN_IGMP,"Triggering Group & Source Specific Message Report Message (ifId:%u vlan:%u group:%s source:%s)",
            pTimerData->groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx, pTimerData->groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId, 
            snoopPTinIPAddrPrint(pTimerData->groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr, address),
            snoopPTinIPAddrPrint(pTimerData->groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr, address));
...
//Schedule Report
    snoop_ptin_proxy_Sourcetimer_start(pTimerData, pTimerData->selectedDelay, pTimerData->mgmdsQRV,pTimerData->groupData);
    return;
  }
  else if (pTimerData->mrmCounter>=pTimerData->mgmdsQRV)
  {
//  continue;
  }

  //Save Sourcetimer's internal data
...
///* Interfacetimer triggers only affect groups in filter-mode EXCLUDE */
...
L7_RC_t snoop_ptin_proxy_Sourcetimer_start(snoopPTinProxySourcetimer_t* pTimer, L7_uint32 timeout, L7_uchar8 mgmdsQRV, snoopPTinProxySourcetimerInfoData_t* groupData)
{
  ...
  pTimerData.selectedDelay = timeout;
  pTimerData.mgmdsQRV = mgmdsQRV;
  ...
  if (SLLFind(&timerLinkedList, (void *)&pTimerData) != L7_NULL)
  {
    ...
    {
      osapiSemaGive(timerSem);
      LOG_ERR(LOG_CTX_PTIN_IGMP,"Failed to Reschedule Group & Source Specific Membership Report Message(ifIdx:%u vlan:%u group:%s source:%s responsetime:%u)",
            pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx, pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId,
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr, address), 
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr, address),timeout);
      return L7_FAILURE;
    }
  ...
  else
  {
    ...
    pTimer->selectedDelay = timeout;
    pTimer->mgmdsQRV = mgmdsQRV;
  }

  LOG_TRACE(LOG_CTX_PTIN_IGMP,"Scheduling Group Specific Membership Report Message(ifIdx:%u vlan:%u group:%s source:%s responsetime:%u)",
            pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx, pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId, 
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr, address), 
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr, address),timeout);
  ...
  if (pTimer->timer == NULL)
  {
    ...
    pTimer->timerHandle = 0;
    osapiSemaGive(timerSem);
    LOG_ERR(LOG_CTX_PTIN_IGMP,"Could Not Schedule Group & Source Specific Membership Report Message(ifIdx:%u vlan:%u group:%s  source:%s responsetime:%u)",
            pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx, pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId, 
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr, address),
            snoopPTinIPAddrPrint(pTimerData.groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr, address),timeout);
    return L7_FAILURE;
...
L7_RC_t snoop_ptin_proxy_Sourcetimer_stop(snoopPTinProxySourcetimer_t *pTimer)
{
  ...
  LOG_TRACE(LOG_CTX_PTIN_IGMP,"Unscheduling Group & Source Specific Membership Report Message (ifIdx=%u vlan=%u group:%s source:%s)",
            pTimer->groupData->snoopPTinProxySourcetimerInfoDataKey.interfaceIdx, pTimer->groupData->snoopPTinProxySourcetimerInfoDataKey.vlanId, 
            snoopPTinIPAddrPrint(pTimer->groupData->snoopPTinProxySourcetimerInfoDataKey.groupAddr, address),
            snoopPTinIPAddrPrint(pTimer->groupData->snoopPTinProxySourcetimerInfoDataKey.sourceAddr, address));
