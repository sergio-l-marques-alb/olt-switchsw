...
static L7_RC_t pbvlanMigrateConfigV4V5Convert (pbVlanCfgDataV4_t * pCfgV4,
                                               pbVlanCfgDataV5_t * pCfgV5);
...
static void pbVlanBuildDefaultConfigDataV5 (pbVlanCfgDataV5_t * pCfgV4);
...
pbvlanMigrateConfigData (L7_uint32 oldVer, L7_uint32 ver, L7_char8 * pCfgBuffer)
{
  ...
  pbVlanCfgDataV5_t *pCfgV5 = L7_NULLPTR;
  ...
  case PBVLAN_CFG_VER_5:
    pCfgV5 = (pbVlanCfgDataV5_t *) osapiMalloc (L7_PBVLAN_COMPONENT_ID,
                                                (L7_uint32) sizeof (pbVlanCfgDataV5_t));
    ...
    oldCfgSize = (L7_uint32) sizeof (pbVlanCfgDataV5_t);
    pOldCfgCksum = &pCfgV5->checkSum;
    oldCfgVer = PBVLAN_CFG_VER_5;
    break;
  ...
  if (buildDefault != L7_TRUE)
  {
    ...
    case PBVLAN_CFG_VER_4:
      ...
      pCfgV5 = (pbVlanCfgDataV5_t *) osapiMalloc (L7_PBVLAN_COMPONENT_ID,
                                                  sizeof (pbVlanCfgDataV5_t));
      if ((L7_NULLPTR == pCfgV5) || (L7_NULLPTR == pCfgV4) || (L7_SUCCESS != pbvlanMigrateConfigV4V5Convert (pCfgV4, pCfgV5)))
      {
        buildDefault = L7_TRUE;
        break;
      }
    ...
    case PBVLAN_CFG_VER_5:
	  if (pCfgV5 != L7_NULLPTR)
	    ...
	    osapiFree (L7_PBVLAN_COMPONENT_ID, pCfgV5);
      pCfgV5 = L7_NULLPTR;
...
*********************************************************************/
static L7_RC_t
pbvlanMigrateConfigV4V5Convert (pbVlanCfgDataV4_t * pCfgV4, pbVlanCfgDataV5_t * pCfgV5)
{
  L7_uint32 i, count;

  /* verify correct version of old config file */
  if (pCfgV4->cfgHdr.version != PBVLAN_CFG_VER_4 &&
      pCfgV4->cfgHdr.version != PBVLAN_CFG_VER_3 &&
      pCfgV4->cfgHdr.version != PBVLAN_CFG_VER_2 &&
      pCfgV4->cfgHdr.version != PBVLAN_CFG_VER_1)
  {
    MIGRATE_LOG_UNEXPECTED_VERSION (pCfgV4->cfgHdr.version, PBVLAN_CFG_VER_4);
    return L7_FAILURE;
  }

  pbVlanBuildDefaultConfigDataV5 (pCfgV5);

  /* Map the fields in the older structure to the appropriate fields in the newer one */
  count = min (L7_PBVLAN_MAX_NUM_GROUPS_REL_4_4, L7_PBVLAN_MAX_NUM_GROUPS_REL_4_3);
  for (i = 0; i <= count; i++)
  {
    osapiStrncpy (pCfgV5->group[i].name, pCfgV4->group[i].name, L7_PBVLAN_MAX_GROUP_NAME - 1);
    pCfgV5->group[i].inUse = pCfgV4->group[i].inUse;
    pCfgV5->group[i].groupID = pCfgV4->group[i].groupID;
    pCfgV5->group[i].vlanid = pCfgV4->group[i].vlanid;
    MIGRATE_COPY_STRUCT (pCfgV5->group[i].protocol, pCfgV4->group[i].protocol);
    MIGRATE_COPY_NIM_INTF_MASK(pCfgV5->group[i].ports, pCfgV4->group[i].ports);
  }
...
{
  bzero((char*)pCfgV4, sizeof(*pCfgV4));

  /* Build header */
  strcpy((char *)&pCfgV4->cfgHdr.filename, PBVLAN_CFG_FILENAME);
  pCfgV4->cfgHdr.version = PBVLAN_CFG_VER_4;
  pCfgV4->cfgHdr.componentID = L7_PBVLAN_COMPONENT_ID;
...
*********************************************************************/
static void
pbVlanBuildDefaultConfigDataV5 (pbVlanCfgDataV5_t * pCfgV5)
{
  memset ((L7_uchar8 *) pCfgCur, 0, sizeof (pbVlanCfgDataV5_t));
  pbVlanBuildDefaultConfigData (PBVLAN_CFG_VER_5);
