  ...
  if (dot1qIntfIsSettable(intIfNum) == L7_TRUE)
  {
    if (dtlDot1dDefaultPortPrioritySet(intIfNum,priority) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "Unable to set dot1d default priority (%u) on intf %s\n",
              priority, ifName);
    }
  }
...
L7_RC_t dot1dPortDefaultUserPriorityGlobalProcess(L7_uint32 priority)
{
  ...
  for (i = 1; i < DOT1Q_INTF_MAX_COUNT; i++)
  {
    ...
    if (dot1dPortDefaultUserPriorityProcess(intIfNum, priority) != L7_SUCCESS)
    {
      ...
      /* log a message to indicate an apply failure, but keep going */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "Unable to set global dot1d default priority (%u) on intf %s\n",
              priority, ifName);
    }
  }
   ...
   L7_RC_t rc;
    ...
    for (i = 1; i < DOT1Q_INTF_MAX_COUNT; i++)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        /* log a message to indicate an apply failure, but keep going */
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                "Unable to set global dot1d number of traffic classes (%u) on intf %s\n",
                numTrafficClasses, ifName);
      }
    }
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    */
    ...
    if (applyFailed == L7_TRUE)
    {
      /* log a message to indicate an apply failure, but keep going */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "Unable to set one or more dot1d port priority mappings on intIfNum %u\n",
              intIfNum);
    }
    ...
    if (dot1qIntfIsSettable(intIfNum) == L7_TRUE)
    {
      if (dot1qDot1pMappingIsActive(intIfNum) == L7_TRUE)
      {
        if (dtlDot1dTrafficClassSet(intIfNum, priority, trafficClass) != L7_SUCCESS)
        {
          /* log a message to indicate an apply failure, but keep going */
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                  "Unable to set dot1d priority %u to traffic class %u on intIfNum %u\n",
...
L7_RC_t dot1dPortTrafficClassGlobalProcess(L7_uint32 priority, L7_uint32 trafficClass)
{
  ...
  for (i = 1; i < DOT1Q_INTF_MAX_COUNT; i++)
  {
    ...
    if (dot1dPortTrafficClassProcess(intIfNum, priority, trafficClass) != L7_SUCCESS)
    {
      ...
      /* log a message to indicate an apply failure, but keep going */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "Unable to set global dot1d priority %u to traffic class %u on intf %s\n",
              priority, trafficClass, ifName);
    }
  }
...
L7_RC_t   dot1qQportsIngressFilterProcess(L7_uint32 intIfNum, L7_uint32 status)
{
  ...
  DOT1Q_TRACE("%s(%d) intf %d, %s  value %d \n", __FUNCTION__, __LINE__, intIfNum, ifName, status);
  for (i = start; i < end; i++)
  {
      if (dot1qMapIntfIsConfigurable(i, &pCfg) != L7_TRUE)
      {
          ...
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DOT1Q_COMPONENT_ID,
                  "failure getting pCfg for intf %s\n", ifName);
          continue;

      }
      else if (dot1qIntfIsSettable(i) == L7_FALSE)
      {
        /* interface config is handle at a later stage
           when L7_ATTACH event is received by dot1q for
           this interface */
        continue;
      }
           ...
           (currSwPortMode == DOT1Q_SWPORT_MODE_TRUNK))
      {
          ...
          L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID,
                 "Vlan acceptable fram type can only be configured for General port mode %s",
                 ifName);
          continue;
      }
      ...
      if (rc != L7_SUCCESS)
      {
          nimGetIntfName(i, L7_SYSNAME, ifName);
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                  "dtl failure while modifying ingress filter value to %d for intf %s\n",
                  status, ifName);
      }
  }
                                ...
                                L7_uint32 vid_type,L7_uint32 requestor)
{
  ...
  DOT1Q_TRACE("%s(%d) intf %d, %s vid %d \n", __FUNCTION__, __LINE__, intIfNum, ifName, vid);
  for (i=start;i<end;i++)
  {
      if (dot1qMapIntfIsConfigurable(i, &pCfg) != L7_TRUE)
      {
           ...
           (currSwPortMode == DOT1Q_SWPORT_MODE_TRUNK))
      {
          nimGetIntfName(i, L7_SYSNAME, ifName);
          L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1Q_COMPONENT_ID,
                 "Vlan PVID can only be configured for General port mode %s",
                 ifName);
          continue;
      }
           ...
           ((COMPONENT_ISMASKBITSET(dot1qQports[i].acquiredList,L7_DOT1X_COMPONENT_ID)!=0) && (requestor == DOT1Q_DOT1X )&& (result == 1)))
      {
          ...
          if (rc != L7_SUCCESS)
          {
              nimGetIntfName(i, L7_SYSNAME, ifName);
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                      "dtl failure while modifying pvid configuration to %d for intf %s and vid_type %d\n",
                      vid, ifName, vid_type);
          }
...
L7_RC_t dot1qQportsAcceptFrameTypeProcess(L7_uint32 intIfNum, L7_uint32 type)
{
  ...
  DOT1Q_TRACE("%s(%d) intf %d, %s frameType %d \n", __FUNCTION__, __LINE__, intIfNum, ifName, type);
  for (i = start; i < end; i++)
  {
      if (dot1qMapIntfIsConfigurable(i, &pCfg) != L7_TRUE)
      {
      ...
      if (rc != L7_SUCCESS)
      {
          nimGetIntfName(i, L7_SYSNAME, ifName);
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                  "dtl failure while modifying accept frame type to %d for intf %s\n",
                  type, ifName);
      }
  }
                                ...
                                L7_uchar8 ingressFiltering)
{
    ...
    {
       L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
               "dtl failed with err(%d) applying port cfg: intf %s:"
               "  pvid = %d,acceptFrameType = %d, ingressFiltering = %d\n",
               rc, ifName, vid, acceptFrameType, ingressFiltering);
    }
