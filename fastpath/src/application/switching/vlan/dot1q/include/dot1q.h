...
typedef struct dot1qNimStatus_s
{
    /* L7_TRUE after NIM create startup is done */
  L7_BOOL               createStartupDone;

  /* Bit mask of NIM events that dot1q is registered to receive */
...
typedef struct dot1qInfo_s
{
  ...
  dot1qNimStatus_t  nimStatus;
} dot1qInfo_t;


/* VLANs used for internal usage, not for network operations */
typedef struct dot1qInternalVlanInfo_s
{
  L7_uint32 vlanId;
  dot1qVlanTypeVal_t  requestor;    /* owning entity */
  L7_uchar8 description[L7_DOT1Q_MAX_INTERNAL_VLANS_DESCR];
}dot1qInternalVlanInfo_t;
...
typedef struct dot1q_vlan_oper_s
{
  ...
  dynamicVlanAcquiredMask dynamicVlanAcquiredList;
...
typedef struct dot1qGlobCfgData_s
{
  ...
  L7_uint32 baseInternalVlan;                       /* First VLAN ID assigned for internal usage. */

  DOT1Q_INTERNAL_VLAN_POLICY internalVlanPolicy;   /* Whether to move up or down in the VLAN ID space
                                                     from the base internal VLAN ID when allocating
                                                     internal VLAN IDs. */
...
#define DOT1Q_LOG_MSG(format, args...)  {\
        if ( (dot1qInfo->log_enabled) == L7_TRUE) {\
             L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID, format, ##args);  \
                }\
        }
