...
L7_RC_t dot1qInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
	...
	memset(dot1qVlanPendingCreate,0x0,sizeof(dot1qVlanPendingCreate_t)*(DOT1Q_MAX_PENDING_CREATES+1));
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Unable to initialize dot1s_task()\n");
    ...
    if (dot1qIssueCmd(&msg) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Unable to issue cnfgr event\n");
    }
...
void dot1qApiCnfgrCommand(L7_CNFGR_CMD_DATA_t *pCmdData)
{
   ...
   */
    ...
    } else
    {
      ...
      if(rc!=L7_SUCCESS)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"DOT1Q: Failed to put comamnd : %d configurator message in queue.",pCmdData->command);
      }
    }
    ...
    case L7_CNFGR_CMD_TERMINATE:
      switch (pCmdData->u.rqstData.request)
      {
        case L7_CNFGR_RQST_T_START:
          dot1qCnfgrTerminateProcess(pCmdData);
          break;
...
L7_RC_t dot1qCnfgrInitPhase1Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Error creating Dot1q Semaphore \n");
      return L7_FAILURE;
  }
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Error creating dot1qVlanCount Semaphore \n");
      return L7_FAILURE;
  }
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Error creating dot1qCfgRWlock semaphore \n");
      return L7_FAILURE;
  }


  /* controls access to internal vlan database */
  dot1qInternalVlanSem = osapiSemaBCreate (OSAPI_SEM_Q_FIFO, OSAPI_SEM_FULL);
  if (dot1qInternalVlanSem == L7_NULLPTR)
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Error creating Dot1q Internal Vlan Semaphore \n");
      return L7_FAILURE;
  }
  ...
  dot1qInternalVlanInfo   = (dot1qInternalVlanInfo_t*)osapiMalloc( L7_DOT1Q_COMPONENT_ID, sizeof(dot1qInternalVlanInfo_t) * (L7_DOT1Q_MAX_INTERNAL_VLANS+1) );
  ...
  if ( (pVlanTreeHeap == L7_NULLPTR) || 
       ...
       (dot1qInternalVlanInfo == L7_NULLPTR) ||
  ...
  bzero((L7_uchar8 *)dot1qInternalVlanInfo,  sizeof(dot1qInternalVlanInfo_t)*(L7_DOT1Q_MAX_INTERNAL_VLANS+1));
...
void dot1qStartupNotify(NIM_STARTUP_PHASE_t startupPhase)
{
...
L7_RC_t dot1qCnfgrInitPhase2Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if (nimRegisterIntfChange(L7_DOT1Q_COMPONENT_ID, dot1qIntfChangeCallback,
                            dot1qIntfStartupCallback, NIM_STARTUP_PRIO_DOT1Q) != L7_SUCCESS)
  {
    L7_assert(1);
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"DOT1Q: Unable to register with NIM\n");
    return L7_FAILURE;
  }
  ...
  {
    L7_assert(1);
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"DOT1Q: Unable to register with DTL\n");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,"Unable to register for VLAN change callback!\n");
    rc     = L7_ERROR;
...
L7_RC_t dot1qCnfgrInitPhase3Process(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  memset(&dot1qInfo->nimStatus.nimEventMask, 0, sizeof(dot1qInfo->nimStatus.nimEventMask));
  ...
  memset((L7_uchar8 *)dot1qInternalVlanInfo, 0, sizeof(dot1qInternalVlanInfo_t)*(L7_DOT1Q_MAX_INTERNAL_VLANS+1));
...
*/
  ...
  if (dot1qInternalVlanInfo != L7_NULLPTR)
    osapiFree(L7_DOT1Q_COMPONENT_ID, dot1qMapTbl);
  ...
  if (dot1qInternalVlanSem != L7_NULLPTR)
    osapiSemaDelete(dot1qInternalVlanSem);
...
* @end
***************************************************************************/
void dot1qCnfgrTerminateProcess(L7_CNFGR_CMD_DATA_t *pCmdData)
{
