...
L7_BOOL dsVlanIntfIsSnooping(L7_uint16 vlanId, L7_uint32 intIfNum)
{
  ...
  if ( (ptin_evc_get_fromIntVlan(vlanId, &evcConf)==L7_SUCCESS) &&
       (evcConf.flags & PTIN_EVC_MASK_DHCP_PROTOCOL) )
  {
    for (port=0; port<evcConf.n_intf; port++)
    {
      if ( evcConf.intf[port].intf_type == ptin_intf.intf_type &&
           evcConf.intf[port].intf_id   == ptin_intf.intf_id )
      {
        return L7_TRUE;
      }
    }
...
{
  return _dsVlanIntfTrustGet(vlanId, intIfNum);
}
...
L7_BOOL _dsVlanL2RelayGet(L7_uint32 vlanNum)
{
  /* PTin added: This function should be updated */
  #if 1
  return _dsVlanEnableGet(vlanNum);
  #else
...
L7_BOOL _dsVlanIntfTrustGet(L7_uint16 vlanId, L7_uint32 intIfNum)
{
       ...
       (evcConf.flags & PTIN_EVC_MASK_DHCP_PROTOCOL) )
  {
    for (port=0; port<evcConf.n_intf; port++)
    {
      if (evcConf.intf[port].intf_type == ptin_intf.intf_type &&
          evcConf.intf[port].intf_id   == ptin_intf.intf_id   &&
          evcConf.intf[port].mef_type  == PTIN_EVC_INTF_ROOT)
      {
        return L7_TRUE;
      }
    }
...
L7_RC_t dsRestoreProcess(void)
{
  ...
  /* This gets restarted in p3 init */
  dsTimerStop();
...
* @returns  L7_SUCCESS or L7_FALIURE
*
* @notes    We trigger the remote ftp or local db
*           restore in case of cold restart in the function
*           dsNimActivateStartup().
*           dsNimActivateStartup would be called after configuration is
*           pushed to us.
*           Do nothing here - may fill it up in future.
* @end
*********************************************************************/
