...
L7_RC_t dsDHCPv6ClientFrameProcess(L7_uint32 intIfNum, L7_ushort16 vlanId, L7_uchar8 *frame, L7_ushort16 innerVlanId, L7_uint client_idx)
{
   ...
   while (frame_len > 0)
   {
      ...
      switch (dhcp_op_header->option_code)
      {
         ...
         {
            LOG_TRACE(LOG_CTX_PTIN_DHCP, "Discarding client DHCP request: interface-id was already present");
            return L7_SUCCESS;
         }
         default:
   ...
   memcpy(relay_agent_header.peer_address, ipv6_header->src, 16 * sizeof(L7_uchar8));
...
L7_RC_t dsDHCPv6ServerFrameProcess(L7_uint32 intIfNum, L7_ushort16 vlanId, L7_uchar8 *frame, L7_ushort16 innerVlanId, L7_uint client_idx)
{
   ...
   if(!op_relaymsg_ptr || !op_interfaceid_ptr)
   {
   ...
   if(!inetIsInAddressAny(&link_addr) || 0 != memcmp(ipv6_header->dst, relay_agent_header->peer_address, 16 * sizeof(L7_uchar8)))
   {
...
L7_RC_t dsv6AddOption18or37(L7_uint32 intIfNum, L7_uchar8 *frame, L7_uint32 *frameLen, L7_dhcp6_opttype_t dhcpOp)
{
   ...
   if (ptin_dhcp_strings_def_get(&ptin_intf, macAddr, circuit_id, remote_id) != L7_SUCCESS)
   {
      if (ptin_debug_dhcp_snooping)
