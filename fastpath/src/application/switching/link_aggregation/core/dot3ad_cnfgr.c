...
extern L7_RC_t helperTimer_task();
/*
 * Local prototypes
 */
...
L7_RC_t dot3adCnfgrInitPhase2Process( L7_CNFGR_RESPONSE_t *pResponse,
                                      L7_CNFGR_ERR_RC_t   *pReason );
L7_RC_t dot3adCnfgrInitPhase3Process( L7_BOOL warmRestart,
...
L7_uint32 dot3adSwitchoverInProgress = L7_FALSE;

/*********************************************************************
* @purpose  This function returns the restart type notified by the configurator.
*
* @param
*
* @returns  L7_BOOL
*
* @notes    Returns L7_TRUE if the restart type is warm
*           L7_FALSE otherwise.
*
*
* @end
*********************************************************************/
L7_BOOL dot3adCnfgrIsRestartTypeWarm()
{
  return globalWarmRestart;
}
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                globalWarmRestart = (pCmdData->u.rqstData.data & L7_CNFGR_RESTART_WARM);

                if ((rc = dot3adCnfgrInitPhase3Process( globalWarmRestart, &response, &reason )) != L7_SUCCESS)
                {
...
{
  L7_RC_t rc ;
  dot3ad_helper_msgType_t helperMsgType = DOT3AD_HELPER_PURGE;

  if (warmRestart == L7_TRUE)
  {
    dot3adSwitchoverInProgress = L7_TRUE;
    helperMsgType = DOT3AD_HELPER_SWITCHOVER_START;
  ...
  else
  {
    ...
    {
    dot3adNsfCheckPtDataReset();
  }
  }
#endif

  dot3adTransportBcastHelperCtlMsg(helperMsgType);
...
/*********************************************************************
* @purpose  Initialize the resources needed for core helper support
*
* @param    void
*
* @returns  L7_SUCCESS
...
* @end
*********************************************************************/
L7_RC_t dot3adCoreHelperInit()
{
  L7_RC_t rc = L7_FAILURE;
   /* Tasks introduced by NSF *********************************************************/

  if ((rc = dot3adTransportInit()) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
           "Initialization of dot3ad transport subsystem failed %d", rc);
    return rc;
  }

  if ((rc = dot3adHelperInit()) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
           "Initialization of dot3ad helper subsystem failed %d", rc);
    return rc;
  }
...
L7_RC_t   dot3adPhaseOneInit(void)
{
#if defined (L7_NSF_PACKAGE) || defined (L7_STACKING_PACKAGE)
  L7_RC_t rc ;
#endif
  ...
  memset(&dot3adNsfFuncTable, 0, sizeof(dot3adNsfFuncTable));
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "dot3adPhaseOneInit: Error allocating buffers\n");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "Unable to create dot3ad task semaphore()\n");
    return(L7_FAILURE);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "Unable to create dot3ad timer semaphore()\n");
    return(L7_FAILURE);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "Unable to create dot3ad Intf create semaphore()\n");
    return(L7_FAILURE);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "Unable to create dot3adQueueSync semaphore()\n");
    return(L7_FAILURE);
  }
  ...
  if (dot3ad_core_task_id == L7_ERROR ||
      dot3ad_timer_task_id == L7_ERROR
     )
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
            "Failed to create dot3ad tasks.\n");
    return(L7_FAILURE);
  }
  ...
  dot3adTraceInit(L7_NULL, L7_NULL);

#ifdef L7_NSF_PACKAGE
  if (cnfgrIsFeaturePresent(L7_FLEX_STACKING_COMPONENT_ID,
                            L7_STACKING_NSF_FEATURE_ID) == L7_TRUE)
  {
    if ((rc = dot3adNsfInit()) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
              "DOT3AD: trace block not enabled\n");
      return(rc);
    }
  }
#endif
  ...
  if ((rc = dot3adCoreHelperInit()) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
           "Cannot Initialize Core Helpers \n");
    return rc;
  }
#endif
...
void dot3adPhaseOneFini(void)
{
  ...
  if (dot3ad_core_task_id != L7_ERROR)
    osapiTaskDelete(dot3ad_core_task_id);
...
L7_RC_t   dot3adPhaseTwoInit(void)
{
  ...
  do
  {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                 "DOT3AD: Failed to register NV routines\n");
      }
      /* register callback with NIM for L7_UPs and L7_DOWNs */
      if ((rc = nimRegisterIntfChange(L7_DOT3AD_COMPONENT_ID,
                                           dot3adIntfChangeCallBack,
                                           dot3adIntfStartupCallback, NIM_STARTUP_PRIO_DOT3AD)) != L7_SUCCESS)

      {
        L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                 "DOT3AD: failed to register to NIM\n");
        break;
      }
 #ifdef L7_NSF_PACKAGE
      if ((rc =ckptMgrCheckpointCallbackRegister(L7_DOT3AD_COMPONENT_ID,
                                                       dot3adManagerCheckpoint)) != L7_SUCCESS )
      {
          L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                 "DOT3AD: failed to register to check point service\n");
          break;
      }
      if ((rc =ckptBackupMgrCheckpointCallbackRegister(L7_DOT3AD_COMPONENT_ID,
                                                             dot3adBackupUnitCheckpoint)) != L7_SUCCESS )
      {
          L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                 "DOT3AD: failed to register with check point service\n");
          break;
      }
 #endif

 #ifdef L7_STACKING_PACKAGE
      if ((rc = simPtsRegister(L7_DOT3AD_COMPONENT_ID, 0, dot3adHelperTimerCallback, &dot3adHelperUsrHandle)) != L7_SUCCESS)
      {
         L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                "DOT3AD: failed to with register helper Timer  service\n");
         printf("DOT3AD: failed to with register helper Timer  service(%d)\n",rc);
         break;
      }

      if ((rc = msMsgRegister(L7_MS_DOT3AD_REGISTRAR_ID, dot3adTransportCoreCallback, MS_TARGET_CORE)) != L7_SUCCESS)
      {
         L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                "DOT3AD: failed to with register helper Timer  service\n");
         printf("DOT3AD: failed to with register helper Timer  service(%d)\n",rc);
      ...
      if ((rc = msMsgRegister(L7_MS_DOT3AD_REGISTRAR_ID, dot3adTransportHelperCallback, MS_TARGET_HELPER)) != L7_SUCCESS)
      {
         L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
                "DOT3AD: failed to with message service for helper callback\n");
         printf("DOT3AD: failed to with message service for helper callback(%d)\n",rc);
         break;
      }
     ...
     /* register for PDUs */
     if ((rc = dot3adLacpmInit()) != L7_SUCCESS)
     {
       L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT3AD_COMPONENT_ID,
               "DOT3AD: failed to register for PDUs\n");
       break;
     }
...
L7_RC_t   dot3adPhaseExecuteInit(void)
{
  ...
  simMacAddrGet((L7_uchar8 *)&(dot3adSystem.actorSys));
...
void dot3adNimUnconfigEventCompletionCallback(NIM_NOTIFY_CB_INFO_t retVal)
{
  ...
  if (retVal.response.rc != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_DOT3AD_COMPONENT_ID,
            "DOT3AD: notification failed for event(%d), intf(%s), reason(%d)\n",
...
/*********************************************************************
* @purpose  return the presence of helpers
*
* @param
*
* @returns  L7_TRUE if helpers are present
*           L7_FALSE otherwise
*
* @notes    none
*
