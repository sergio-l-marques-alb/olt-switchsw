...
* @end
*********************************************************************/
L7_RC_t q931ConnInfoEqual(connInfo_t *ct_info, const connPair_t *info)
{
...
*********************************************************************/
L7_RC_t
voipH323CallInsert(const connPair_t *info, voipH323Call_t **ins)
{
  ...
  if ((entry = osapiMalloc(L7_FLEX_QOS_VOIP_COMPONENT_ID,sizeof(voipH323Call_t))) == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
            "voipH323CallInsert memory allocation failed\n");
    return L7_FAILURE;
  }
  ...
  entry->next      = voipH323CallList;
  voipH323CallList = entry;
  *ins             = entry;
...
*********************************************************************/
L7_RC_t
voipH323CallRemove(connInfo_t *del)
{
  voipH323Call_t *entry;
  voipH323Call_t *prevEntry = L7_NULL;
  L7_char8  strSrcIPaddr[L7_CLI_MAX_STRING_LENGTH];
  L7_char8  strDstIPaddr[L7_CLI_MAX_STRING_LENGTH];
  ...
  entry = voipH323CallList;

  while (entry != L7_NULLPTR)
  {
    if (memcmp(&entry->ct_info, del, sizeof(connInfo_t)) == 0)
    {
      if (prevEntry == L7_NULL)
      {
        voipH323CallList = entry->next;
      }
      else
      {
...
voipSetupH245Session(connInfo_t *info,L7_uint32 enable)
{
   ...
   rc = dtlVoipProfileParamSet(enable,
                               ...
                               srcL4port,
                               dstL4port);
   if ((rc == L7_SUCCESS) && (enable == L7_ENABLE))
   {
    /* Log the event */
...
#ifdef L7_NSF_PACKAGE
  voipCkptH323CallSync(entry, L7_TRUE);
#endif
  return L7_SUCCESS;
}
/*********************************************************************
...
L7_RC_t voipQ931MessageParse(const L7_uchar8 *packetBuf, L7_uint32 length)
{
  ...
  if (ret == L7_FAILURE)
  {
     ret = voipH323CallInsert(&pair_info, &entry);
     if(ret != L7_SUCCESS)
     {
  ...
  if (ret == L7_SUCCESS)
  {
    /* If the call hasn't been removed as a result of the above processing,
       go ahead and sync it. */
    voipCkptH323CallSync(entry, L7_TRUE);
  }
#endif
  ...
  while(entry!=L7_NULLPTR)
  {
    ...
    if (entry->ct_info.rtp_SessionStatus == L7_TRUE)
    {
        if (voipSetupRTPSession(&entry->ct_info, L7_DISABLE) == L7_SUCCESS)
           voipH323CallRemove(&entry->ct_info);
    }
    entry = entry->next;
  }
  return L7_SUCCESS;
  ...
  while(entry!=L7_NULLPTR)
  {
    ...
    if (entry->ct_info.rtp_SessionStatus == L7_TRUE)
    {
                                    ...
                                    &counter) == L7_SUCCESS)
      {
        ...
        {
          entry->prev_counter.high = counter.high;
          entry->prev_counter.low  = counter.low;
        }
      }
    }
...
void voipH323CkptRebuild()
{
  ...
  while (entry != L7_NULL)
  {
    voipCkptH323CallSync(entry, L7_FALSE);
...
L7_RC_t voipDebugH323CallAdd(L7_uint32 src_ip, L7_uint32 dst_ip, L7_ushort16 l4_src_port, L7_ushort16 l4_dst_port)
{
  L7_RC_t         rc;
  connPair_t      pair_info;
  voipH323Call_t *entry;

  pair_info.dst_ip   = dst_ip;
  pair_info.src_ip   = src_ip;
  pair_info.src_port = l4_src_port;
  pair_info.dst_port = l4_dst_port;

  rc = voipH323CallLookup(&voipH323CallList, &pair_info, &entry);
  if (rc != L7_SUCCESS)
  {
     rc = voipH323CallInsert(&pair_info, &entry);
     if(rc != L7_SUCCESS)
       return rc;
  }

  entry->ct_info.media_ip[0]       = src_ip;
  entry->ct_info.media_ip[1]       = dst_ip;
  entry->ct_info.media_port[0]     = l4_src_port;
  entry->ct_info.media_port[1]     = l4_dst_port;
  entry->ct_info.rtp_SessionStatus = L7_TRUE;

#ifdef L7_NSF_PACKAGE
  voipCkptH323CallSync(entry, L7_TRUE);
#endif

  rc = voipSetupRTPSession(&entry->ct_info, L7_ENABLE);
  if (rc != L7_SUCCESS)
  {
    voipH323CallRemove(&entry->ct_info);
  }
...
L7_RC_t voipDebugH323CallDelete(L7_uint32 src_ip, L7_uint32 dst_ip, L7_ushort16 l4_src_port, L7_ushort16 l4_dst_port)
{
  L7_RC_t         rc;
  connPair_t      pair_info;
  voipH323Call_t *entry;
  ...
  rc = voipSetupRTPSession(&entry->ct_info, L7_DISABLE);
  voipH323CallRemove(&entry->ct_info);
