...
* @end
*********************************************************************/
L7_RC_t voipSipCallInsert(const L7_uchar8 *id,
                          voipSipCall_t **ins)
{
    voipSipCall_t   *entry;
    ...
    if(entry == L7_NULLPTR)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
              "voipSipCallInsert could not allocate memory\n");
      return L7_FAILURE;
    }
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
              "voipSipCallInsert (entry->call_id) could not allocate memory\n");
    ...
    entry->next     = voipSipCallList;
    voipSipCallList = entry;
    *ins            = entry;
    return L7_SUCCESS;
}
...
voipSipCallEnd(L7_uchar8 *callId)
{
   ...
   rc = voipSipCallRemove(callId, &entry);
...
*********************************************************************/
L7_RC_t
voipSipCallRemove(const L7_uchar8 *id, voipSipCall_t **del)
{
  voipSipCall_t   *entry;
  voipSipCall_t   *prevEntry = L7_NULL;
  L7_char8  strSrcIPaddr[L7_CLI_MAX_STRING_LENGTH];
  L7_char8  strDstIPaddr[L7_CLI_MAX_STRING_LENGTH];

  entry = voipSipCallList;

  while (entry != L7_NULLPTR)
  {
    if (strcmp(entry->call_id, id) == 0)
    {
      *del  = entry;

      if (prevEntry != L7_NULL)
      {
        /* The entry removed is not the first entry, so update the previous entry. */
        prevEntry->next = entry->next;
      }
      else
      {
        /* The entry removed is the first entry, so update the list pointer. */
        voipSipCallList = entry->next;
      }
      /* Log the event */
                   ...
                   const L7_uchar8 **body)
{
  ...
  for (;;)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
              "SIP:voipSipHeaderParse could not allocate memory\n");
      return L7_FAILURE;
    }
    ...
    if (MSG_IS_INVITE(sip))
    {
      ...
      if (rc == L7_SUCCESS)
      {
        if (entry->valid)
        {
...
#ifdef L7_NSF_PACKAGE
            voipCkptSipCallSync(entry, L7_TRUE);
#endif
          }
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
                    "SIP:voipSipMessageParseSdp should not happen!!!\n");
          }
        }
        else
        {
      ...
      else
      {
        ...
        rc = voipSipCallInsert(sip->call_id, &entry);
        if(rc != L7_SUCCESS)
        {
          return rc;
        }
        ...
        if (rc == L7_SUCCESS)
        {
          if (entry->valid == L7_FALSE)
          {
            ...
            if (rc != L7_SUCCESS)
            {
              ...
              rc = voipSipCallRemove(sip->call_id, &entry);
              if (rc == L7_SUCCESS)
              {
                osapiFree(L7_FLEX_QOS_VOIP_COMPONENT_ID,entry->call_id);
                osapiFree(L7_FLEX_QOS_VOIP_COMPONENT_ID,entry);
...
void voipSipCkptRebuild()
{
  ...
  while (entry != L7_NULL)
  {
    voipCkptSipCallSync(entry, L7_FALSE);
...
L7_RC_t voipDebugSipCallAdd(L7_uchar8 *call_id, L7_uint32 src_ip, L7_uint32 dst_ip, L7_ushort16 l4_src_port, L7_ushort16 l4_dst_port)
{
  L7_RC_t        rc;
  voipSipCall_t *entry;

  rc = voipSipCallLookup(&voipSipCallList, call_id, &entry);
  if (rc != L7_SUCCESS)
  {
    rc = voipSipCallInsert(call_id, &entry);
    if (rc != L7_SUCCESS)
      return rc;
  }

  entry->valid          = L7_TRUE;
  entry->db.src_ip      = src_ip;
  entry->db.dst_ip      = dst_ip;
  entry->db.l4_src_port = l4_src_port;
  entry->db.l4_dst_port = l4_dst_port;
  ...
  {
    VOIP_TRACE(VOIP_DEBUG_INFO, VOIP_PROTO_SIP, "Failed to install policy");
    rc = voipSipCallRemove(call_id, &entry);
    if (rc == L7_SUCCESS)
    {
      osapiFree(L7_FLEX_QOS_VOIP_COMPONENT_ID,entry->call_id);
      osapiFree(L7_FLEX_QOS_VOIP_COMPONENT_ID,entry);
...
L7_RC_t voipDebugSipCallDelete(L7_uchar8 *call_id)
{
  return voipSipCallEnd(call_id);
}
