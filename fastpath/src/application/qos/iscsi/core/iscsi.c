...
/*********************************************************************
* @purpose  Create the semaphore for the ISCSI operational data
*
* @param    void
*
* @returns  void
...
L7_RC_t iscsiSemaCreate(void)
{
  ...
  iscsiSema = osapiSemaMCreate(OSAPI_SEM_Q_FIFO);
  if (iscsiSema == L7_NULL)
  {
    rc = L7_FAILURE;
  }
  return(rc);
...
/*********************************************************************
* @purpose  Delete the semaphore for the ISCSI operational data
*
* @param    void
*
* @returns  void
...
void iscsiSemaDelete(void)
{
  if (iscsiSema != L7_NULL)
  {
    osapiSemaDelete(iscsiSema);
  }
}

/*********************************************************************
* @purpose  Take the semaphore for the ISCSI operational data
*
* @param    void
*
* @returns  void
...
void iscsiSemaTake()
{
  (void)osapiSemaTake(iscsiSema, L7_WAIT_FOREVER);
}

/*********************************************************************
* @purpose  Give the semaphore for the ISCSI operational data
*
* @param    void
*
* @returns  void
...
L7_RC_t iscsiImpTargetTableLookUp(L7_uint32 port, L7_uint32 ip_addr, L7_uint32 *index)
{
  ...
  for (i=0; i<L7_ISCSI_CFG_TARGET_TABLE_ENTRY_COUNT; i++)
  {
    ...
    {
      /* if TCP port matches see if we care about IP address (0 means don't care) */
      if ((iscsiCfgData->iscsiTargetCfgData[i].ipAddress == 0) ||
...
* @end
*********************************************************************/
L7_RC_t iscsiImpConnectionCreate(L7_uchar8 *targetName,
                                 ...
                                 L7_uint32 targetTcpPortTableId)
{
                                    ...
                                    initiatorIpAddress, initiatorTcpPort, cid, &connectionId, &sessionId, targetTcpPortTableId)) == L7_SUCCESS)
  {
    ...
    if ((rc = iscsiImpCosDataGet(&cosQueue, &tagMode, &tagData)) == L7_SUCCESS)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_FLEX_QOS_ISCSI_COMPONENT_ID,
...
#ifdef L7_NSF_PACKAGE
  /* if everything went well setting up the connection and there is a backup manager, checkpoint the connection data */
  if (L7_SUCCESS == rc)
  {
    if (iscsiCkptConnectionSync(connectionId, L7_TRUE) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FLEX_QOS_ISCSI_COMPONENT_ID, "Failed to sync connection record.");
    }
  }
#endif
  return(rc);
                              ...
                              L7_uint32 targetTcpPortTableId)
{
  ...
  iscsiSemaTake();
  rc = iscsiImpConnectionCreate(targetName, initiatorName, isid, targetIpAddress, targetTcpPort,
                                initiatorIpAddress, initiatorTcpPort, cid, targetTcpPortTableId);
  iscsiSemaGive();
...
#ifdef L7_NSF_PACKAGE
  if (iscsiCkptConnectionDelete(connectionId) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_FLEX_QOS_ISCSI_COMPONENT_ID, "Failed to delete connection record.");
  }
#endif
