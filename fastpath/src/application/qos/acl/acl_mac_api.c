...
L7_RC_t aclMacNumRulesGet(L7_uint32 aclIndex, L7_uint32 *ruleCount)
{
  ...
  rc = aclMacImpAclRuleCountGet(aclIndex, ruleCount);
...
L7_RC_t aclMacDelete(L7_uint32 aclIndex, L7_BOOL force)
{
  ...
  if ((force != L7_TRUE) &&
      (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_LIST_DELETE,
                                 aclIndex, L7_NULLPTR) != L7_SUCCESS))
  {
    (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
    return L7_REQUEST_DENIED;
  }
...
L7_RC_t aclMacRuleActionAdd(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uint32 action)
{
  ...
  else
  {
     ...
     */
    if (aclMacImpAclRuleCountGet(aclIndex, &ruleCount) != L7_SUCCESS)
    {
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_FAILURE;
    }
    if (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_RULE_CREATE,
                                  aclIndex, &ruleCount) != L7_SUCCESS)
    {
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_REQUEST_DENIED;
    }
...
L7_RC_t aclMacRuleTimeRangeNameAdd(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uchar8 *timeRangeName)
{
  ...
  if (ACL_RULE_FIELD_IS_SET(p->configMask, ACL_MAC_TIME_RANGE_NAME) == L7_TRUE)
  {
  ...
  if (modify !=L7_TRUE)
  {
    if (aclMacTimeRangeEntryAdd(acl_ptr, rulenum, aclTRCorrEntryPtr) != L7_SUCCESS)
    {
