...
L7_RC_t aclMacRemoveRule(void *acl_ptr, L7_uint32 rulenum)
{
  ...
  while (curr->last == L7_FALSE)
  {
    if (rulenum == curr->ruleNum)
    {
      ...
      if (ACL_RULE_FIELD_IS_SET(curr->configMask, ACL_MAC_TIME_RANGE_NAME) == L7_TRUE)
      {
...
*
* @returns  L7_SUCCESS  if time range is added for ACL rule
* @returns  L7_ERROR    access list or rulenum does not exist
* @returns  L7_FAILURE  invalid time range or other failure
*
* @comments
*
* @end
...
L7_RC_t aclMacTimeRangeEntryAdd(void *acl_ptr, L7_uint32 rulenum, aclTRCorrEntry_t *timeRangeEntry)
{
  ...
  if (found == L7_TRUE)
  {
    ...
    curr->configMask |= (1 << ACL_MAC_TIME_RANGE_NAME);
                              ...
                              L7_uint8 *actionFlag)
{
  ...
  *ruleCount = 0;
  ...
  while ((rc == L7_SUCCESS) && (needDenyAll == L7_TRUE))
  {
    ...
    /* set a flag to bypass processing this rule if a Non Time Based 'match every' IP 
    ...
    *actionFlag |= rule.denyFlag;
    ...
    if ((cnfgrIsFeaturePresent(L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_PER_RULE_ACTIVATION_ID) == L7_TRUE)
        && (ACL_RULE_FIELD_IS_SET(p->configMask, ACL_MAC_TIME_RANGE_NAME) == L7_TRUE))
    {    
        ...
        if(loggingValid == L7_TRUE)
           ...
           val1 = aclCorrEncode(L7_ACL_TYPE_MAC, aclIndex, rulenum, p->action);
    }
    else
    {
        ...
        (skipRule == L7_FALSE))
    {
      ...
      {
        /* get the rule status from the aclTimeRangeNameTableEntry */
      ...
      if (((p->every == L7_TRUE) || (aclMacIsTlvMaskedRuleValid(aclIndex, rulenum, vlan) != L7_TRUE)) && 
          (timeBasedRule != L7_TRUE))
      { 
      ...
      if ((rc == L7_SUCCESS) && (ruleIntIfNum == intIfNum))
      {
        ...
        if (aclMacUpdateExceptIntf(aclIndex, ACL_MODIFY, intIfNum) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                  "MAC ACL %s: Error updating device with latest configuration\n",
                  pAcl->aclName);
        }
                                   ...
                                   L7_uint32 seqNum )
{
  ...
  rc = aclImpVlanDirAdd( vlanNum, aclDir, aclType, aclIndex, seqNum );
  if (rc != L7_SUCCESS)
  {
