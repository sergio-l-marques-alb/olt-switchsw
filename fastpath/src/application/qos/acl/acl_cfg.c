...
aclIntfTRInfo_t   *aclIntfTrMem     = L7_NULLPTR;
aclIntfTRInfo_t   *aclVlanTrMem     = L7_NULLPTR;
PORTEVENT_MASK_t  aclPortEventMask_g;
    ...
    if (aclImpIsInterfaceInUse(intIfNum, d) == L7_TRUE)
    {
      if (aclIntfUpdate(intIfNum, d, ACL_INTF_ADD) != L7_SUCCESS)
      {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                "aclApplyIntfConfigData: Error activating saved config for "
                "intf %s %s\n",
                ifName, acl_direction_str[d]);
      }
    }
  } /* endfor */
  ...
  for (d=0; d<ACL_INTF_DIR_MAX; d++)
  {
    if (aclImpIsInterfaceInUse(intIfNum, d) == L7_TRUE)
    {
      if (aclIntfUpdate(intIfNum, d, ACL_INTF_REMOVE) != L7_SUCCESS)
      {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                "aclUnApplyIntfConfigData: unable to remove ACL config from intf %s %s\n",
                ifName, acl_direction_str[d]);
        return L7_FAILURE;
      }
    }
...
L7_RC_t aclApplyVlanConfigData(L7_uint32 vlanNum)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "Data inconsistency found in config table for vlanNum %u\n", vlanNum);
    return L7_FAILURE;
  }
  else
  {
    for (d=0; d<ACL_INTF_DIR_MAX; d++)
    {
      ...
      if (aclIsVlanInUse(vlanNum, direction) == L7_TRUE)
      {
        if (aclVlanUpdate(vlanNum, d, ACL_VLAN_ADD) != L7_SUCCESS)
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "Error activating saved config for vlanNum %u %s -- using factory defaults instead\n",
                    vlanNum, acl_direction_str[d]);
          }
          else
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID, "Error activating saved config for "
...
L7_RC_t aclSave(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID, "ACL: Error writing config file %s\n", ACL_CFG_FILENAME);
  }
    ...
    for (dir = L7_INBOUND_ACL; dir < L7_ACL_DIRECTION_TOTAL; dir++)
    {
      if (aclVlanDirAclListGet(vlanId, dir, &listInfo) == L7_SUCCESS)
      {
        for (i=0; i<listInfo.count; i++)
        {
          ...
          default:
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "Unknown aclType encountered in table (aclType = %d, vlanId=%d, dir=%d, aclId=%d).\n",
                    listInfo.listEntry[i].aclType, vlanId, dir, listInfo.listEntry[i].aclId);
          }
          if (rc  != L7_SUCCESS)
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "Error returned from acl(Mac)VlanDirectionRemove(rc=%d, vlanId=%d, dir=%d, aclId=%d, aclType=%d).\n",
...
L7_BOOL aclMapIntfIsConfigurable(L7_uint32 intIfNum, aclIntfCfgData_t **pCfg)
{
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(aclCfgFileData->cfgParms.aclIntfCfgData[index].configId)) == L7_FALSE)
    {
      ...
      */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
              "Error accessing ACL config data for interface %s in aclMapIntfIsConfigurable.\n", ifName);
      return L7_FALSE;
    }
  }
...
L7_BOOL aclMapIntfConfigEntryGet(L7_uint32 intIfNum, aclIntfCfgData_t **pCfg)
{
  ...
  if ((nextIndex < L7_ACL_INTF_MAX_COUNT) &&
      (NIM_CONFIG_ID_IS_EQUAL(&aclCfgFileData->cfgParms.aclIntfCfgData[nextIndex].configId, &configIdNull)))
  {
...
L7_RC_t aclIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event, NIM_CORRELATOR_t correlator)
{
  ...
  if (aclDeregister.aclIntfChange == L7_TRUE)
  {
    rc = L7_SUCCESS;
  }
  ...
  else if (ACL_IS_READY)
  {
    switch (event)
    {
      ...
      case L7_LAG_ACQUIRE:
      case L7_PROBE_SETUP:
        if (aclIsIntfAcquired(intIfNum) == L7_TRUE)
        {
      ...
      case L7_PROBE_TEARDOWN:
         ...
         */
        if (aclIsIntfAcquired(intIfNum) == L7_FALSE)
        {
          if (osapiWriteLockTake(aclRwLock, L7_WAIT_FOREVER) == L7_SUCCESS)
          {
            rc = aclApplyIntfConfigData(intIfNum);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "aclIntfChangeCallback called outside permitted phase\n");
    rc = L7_FAILURE;
...
void aclStartupNotifyCallback(NIM_STARTUP_PHASE_t startupPhase)
{
  ...
  while (rc == L7_SUCCESS)
  {
    ...
    case NIM_INTERFACE_CREATE_STARTUP:
      ACL_PRT(ACL_MSGLVL_LO_2, "ACL: Create Startup %3u\n", intIfNum);
      (void)aclIntfCreate(intIfNum);
      break;

    case NIM_INTERFACE_ACTIVATE_STARTUP:
      ACL_PRT(ACL_MSGLVL_LO_2, "ACL: Activate Startup %3u\n", intIfNum);
      if (osapiWriteLockTake(aclRwLock, L7_WAIT_FOREVER) == L7_SUCCESS)
      {
        rc = aclApplyIntfConfigData(intIfNum);
        if (rc == L7_SUCCESS)
        {
          (void)aclCollateralConfigApply(intIfNum);
        }
        (void)osapiWriteLockGive(aclRwLock);
      }
      break;
  ...
  case NIM_INTERFACE_CREATE_STARTUP:
    ...
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_CREATE);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_DELETE);
  ...
  case NIM_INTERFACE_ACTIVATE_STARTUP:
    ...
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_ATTACH);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_DETACH);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_LAG_ACQUIRE);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_LAG_RELEASE);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_PROBE_SETUP);
    PORTEVENT_SETMASKBIT(aclPortEventMask_g, L7_PROBE_TEARDOWN);
    nimRegisterIntfEvents(L7_FLEX_QOS_ACL_COMPONENT_ID, aclPortEventMask_g);
    break;
  ...
  nimStartupEventDone(L7_FLEX_QOS_ACL_COMPONENT_ID);

  /* Inform cnfgr that protected port has completed it HW updates */
  if (startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    cnfgrApiComponentHwUpdateDone(L7_FLEX_QOS_ACL_COMPONENT_ID, L7_CNFGR_HW_APPLY_CONFIG);
  }
}
...
void aclMapVlanConfigEntryRelease(L7_uint32 vlanNum)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "aclMapVlanConfigEntryRelease called to release config entry for unconfigurable vlanNum (==%u)\n", vlanNum);
    return;
  }
