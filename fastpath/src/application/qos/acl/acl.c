    ...
    if ( (p->configMask & cfgMask) != 0 )
    {
      ...
      else
      {
             ...
             (ACL_RULE_FIELD_IS_SET(p->configMask, ACL_DSTENDPORT) == L7_TRUE) )
        {
          ...
          PT_LOG_DEBUG(LOG_CTX_MSG, "DSTL4 Range: [%d-%d] -> [%d->%d]", p->dstStartPort, p->dstEndPort, new_start, new_end);
             ...
             (ACL_RULE_FIELD_IS_SET(p->configMask, ACL_SRCENDPORT) == L7_TRUE) )
        {
          ...
          PT_LOG_DEBUG(LOG_CTX_MSG, "SRCL4 Range: [%d-%d] -> [%d->%d]", p->srcStartPort, p->srcEndPort, new_start, new_end);
