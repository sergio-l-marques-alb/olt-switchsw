...
extern PORTEVENT_MASK_t   cosPortEventMask_g;
  ...
  *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
  ...
  if ((rc != L7_SUCCESS) && (errLine != 0))
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_COS_COMPONENT_ID,
            "%s %u: %s: Error initializing COS component\n",
  ...
  *pReason   = L7_CNFGR_ERR_RC_FATAL;
  ...
  do
  {
    ...
    if (nimRegisterIntfChange(L7_FLEX_QOS_COS_COMPONENT_ID,
                              cosIntfChangeCallback,
                              cosStartupNotifyCallback,
                              NIM_STARTUP_PRIO_COS) != L7_SUCCESS)
    {
      errLine = __LINE__;
      break;
    }
    cosDeregister_g.cosIntfChange = L7_FALSE;
                                  ...
                                  L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  *pReason   = L7_CNFGR_ERR_RC_FATAL;
  ...
  memset(&cosPortEventMask_g, 0, sizeof(cosPortEventMask_g));
...
void cosCnfgrFiniPhase2Process(void)
{
  ...
  cosDeregister_g.cosIntfChange = L7_TRUE;
...
void cosCnfgrFiniPhase3Process(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_COS_COMPONENT_ID,
            "%s %u: %s: Error unconfiguring COS phase 2\n",
