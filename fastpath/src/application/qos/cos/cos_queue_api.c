...
L7_RC_t cosQueueDefaultsRestore(L7_uint32 intIfNum)
{
  ...
  if (cosConfigIntfQueueCfgDataApply(intIfNum, pCfg) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_COS_COMPONENT_ID,
            "Unable to apply COS queue defaults on intf %s\n", ifName);
  }
                                   ...
                                   L7_QOS_COS_QUEUE_MGMT_TYPE_t val)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_COS_COMPONENT_ID,
            "Unable to obtain COS drop parms for intf %s\n", ifName);
    return L7_FAILURE;
  }
                                    ...
                                    L7_qosCosQueueBwList_t *pVal)
{
  ...
  {
    pQ = &pCfg->queue[i];
    pQ->minBwPercent = /*(L7_uchar8)*/ qParms.minBwList.bandwidth[i];   /* PTin modified: COS */
    pQ->maxBwPercent = /*(L7_uchar8)*/ qParms.maxBwList.bandwidth[i];   /* PTin modified: COS */
  }
  pCosCfgData_g->cfgHdr.dataChanged = L7_TRUE;
                                    ...
                                    L7_qosCosQueueBwList_t *pVal)
{
  ...
  /* update the COS queue config from the temporary local list info */
  for (i = 0; i < L7_MAX_CFG_QUEUES_PER_PORT; i++)
    pCfg->queue[i].maxBwPercent = /*(L7_uchar8)*/ qParms.maxBwList.bandwidth[i];  /* PTin modified: COS */
