...
void cmdLoggerEntryAdd(L7_char8 *strHistory)
{
  ...
  {
    L7_LOGF(cmdLoggerCfgData.severity, L7_CMD_LOGGER_COMPONENT_ID, strHistory);
  }
}
...
void cmdLoggerWebEntryAdd(L7_char8 *strHistory)
{
  ...
  {
    L7_LOGF(cmdLoggerCfgData.webSeverity, L7_CLI_WEB_COMPONENT_ID, strHistory);
  }
}
...
void cmdLoggerSnmpEntryAdd(L7_char8 *strHistory)
{
  ...
  {
    L7_LOGF(cmdLoggerCfgData.snmpSeverity, L7_SNMP_COMPONENT_ID, strHistory);
  }
}

/*********************************************************************
* @purpose  Returns the Auditing Admin Mode 
*
* @param    void  
*
* @returns  mode  Auditing admin mode
*
* @notes
*
* @end
**********************************************************************/
L7_uint32 cmdLoggerAuditAdminModeGet(void)
{
  return cmdLoggerCfgData.auditAdminMode;
}

/*********************************************************************
* @purpose  Sets the Auditing Admin Mode
*
* @param    L7_uint32 mode - New auditing admin mode
*
* @returns nothing
*
* @notes
...
* @end
*********************************************************************/
void cmdLoggerAuditAdminModeSet(L7_uint32 mode)
{
  if (cmdLoggerCfgData.auditAdminMode != mode)
  {
    cmdLoggerCfgData.auditAdminMode = mode;
    cmdLoggerCfgData.cfgHdr.dataChanged = L7_TRUE;
  }
}
...
* @end
*********************************************************************/
void cmdLoggerAuditEntryAdd(L7_uint32 component, L7_char8 *strEntry)
{
  if (strEntry != L7_NULLPTR)
  {
    /* check if the command to be log doesn't contain any password settings */
    /* do not log commands specifying passwords and hence compromise security */
    if (strstr(strEntry, "password") == L7_NULLPTR)
    {
      if (cmdLoggerCfgData.auditAdminMode == L7_ENABLE)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, component, strEntry);
      }
    }
  }
}
