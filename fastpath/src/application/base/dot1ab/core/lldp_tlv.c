...
/*********************************************************************
*
* @purpose  Write 8023 Power MDI TLV
*
* @param    L7_uint32       index @((input)) index to the port
* @param    L7_uint32       intIfNum @((input)) Internal Interface Number
* @param    lldpPDUHandle_t *pduHdl @b((input)) pointer to LLDP PDU type.
...
*
*********************************************************************/
L7_RC_t lldp8023PowerMDITLVWrite(L7_uint32 index, L7_uint32 intIfNum, lldpPDUHandle_t* pduHdl)
{
#ifdef L7_POE_AT_PACKAGE
  lldp8023PowerMdiTLV_t data;
  L7_RC_t rc = L7_SUCCESS;

  L7_BOOL portClass,mdiSupported,mdiEnabled,pairControlable;
  L7_uint32 portPairs, powerClass;
  L7_uchar8 powerType,powerSource,powerPriority;
  L7_uint32 PDRequestedPower;
  L7_uint32 PSEAllocatedPower;
  L7_ushort16 tempShort;
  L7_uchar8   tempByte;

  memset(&data,  0x00,  sizeof(lldp8023PowerMdiTLV_t));

  do
  {
    rc = lldpXdot3LocPowerPortClassGet(intIfNum, &portClass);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }
    rc = lldpXdot3LocPowerMDISupportedGet(intIfNum, &mdiSupported);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }
    if(mdiSupported) /* If PSE ONLY GET OTHER VALUES */
    {
      rc = lldpXdot3LocPowerMDIEnabledGet(intIfNum, &mdiEnabled);
      if (rc != L7_SUCCESS)
      {
        return rc;
      }
      rc = lldpXdot3LocPowerPairControlableGet(intIfNum, &pairControlable);
      if (rc != L7_SUCCESS)
      {
        return rc;
      }
      rc = lldpXdot3LocPowerPairsGet(intIfNum, &portPairs);
      if (rc != L7_SUCCESS)
      {
        return rc;
      }
      rc = lldpXdot3LocPowerClassGet(intIfNum, &powerClass);
      if (rc != L7_SUCCESS)
      {
        return rc;
      }
    ...
    else
    {
     /* If Device is PD */
      ...
      pairControlable = 0;
      portPairs = 0;
      /* If we came Here that means POE_AT is enabled hence it should be Class 4 device */
      powerClass = L7_POE_PORT_POWER_CLASS4;
    }

    rc = poePethPortLldpPowerTypeGet(intIfNum,&powerType);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }

    rc = poePethPortLldpPowerSourceGet(intIfNum,&powerSource);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }
    rc = poePethPortLldpPowerPriorityGet(intIfNum,&powerPriority);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }
    poePethPortLldpPDRequestedPowerValueGet(intIfNum,&PDRequestedPower);
    poePethPortLldpPSEAllocatedPowerValueGet(intIfNum,&PSEAllocatedPower);

    /* Fill the Data from the extracted API */
    tempByte = (L7_uchar8)((pairControlable << 3)|(mdiEnabled<<2)|(mdiSupported<<1)|(portClass));
    data.mdiSupport[0] = tempByte;
    data.psePair[0] = (L7_uchar8)portPairs;
    data.powerClass[0] =(L7_uchar8)powerClass;
    data.type[0] = (L7_uchar8)((powerSource << LLDP_MED_EXT_POWER_SOURCE_SHIFT) | 
                               (powerType<<LLDP_MED_EXT_POWER_TYPE_SHIFT) | 
                               (powerPriority<<LLDP_MED_EXT_POWER_PRIORITY_SHIFT));

    tempShort = osapiHtons(PDRequestedPower);
    memcpy(&data.pdReqValue[0],&tempShort,sizeof(data.pdReqValue));
    tempShort = osapiHtons(PSEAllocatedPower);
    memcpy(&data.pseAllocValue[0],&tempShort,sizeof(data.pseAllocValue));
    rc = lldp8023TLVWrite(LLDP_TLV_TYPE_ORG_DEF,
          LLDP_8023_SUBTYPE_POWER_MDI,
          (char*)&data,
          sizeof(lldp8023PowerMdiTLV_t),
          pduHdl);
...
L7_RC_t lldpMedNetPolicyTLVWrite(L7_uint32 index, L7_uint32 intIfNum, lldpPDUHandle_t* pduHdl)
{
  ...
  if (lldpXMedLocMediaPolicyAppSupportedGet(intIfNum, voice_policyapptype) == L7_TRUE)
  {
    do
    {
      ...
      else if ((pri < VOICE_VLAN_MIN_DOT1P_PRIORITY) ||
               (pri > VOICE_VLAN_MAX_DOT1P_PRIORITY))
      {
        pri = 0;                        /* Default priority is invalid */
      }
...
L7_RC_t lldpMedPDTLVWrite(L7_uint32 index, L7_uint32 intIfNum, lldpPDUHandle_t* pduHdl)
{
  ...
  do
  {
   ...
   rc = lldpXMedLocXPoePDPowerSourceGet(intIfNum,&powerSrc);
   if (rc != L7_SUCCESS)
   {
     return rc;
   }
