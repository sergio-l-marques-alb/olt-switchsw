...
typedef struct
{
  ...
  L7_uint32               remIndex;
  L7_uchar8               medDevice;    /* Set this flag to true if this entry is MED */
...
typedef struct
{
  ...
  L7_uchar8 rxActive;
  L7_uchar8 txActive;
  L7_uchar8 activeWarmRestart;
  L7_uchar8 rxFrame;       /* indicates if we registered to receive PDUs */
  L7_uchar8 medTransmitEnabled;         /* This is a counter of the number of remote MED devices */
  L7_uchar8 pdEndpointDevice;           /* This is true if the port is a PoE PD (set in lldpIntfCreate) */
  L7_uchar8 localDbChanged;
  AcquiredMask   acquiredList;  /* Keep track acquired ports */
} lldpIntfOprData_t;
...
typedef enum
{
  ...
  lldpMsgNewBackupManager,
  lldpMsgIntfModeApply,
  lldpMsgRemDbClear
} lldpMgmtMessages_t;
