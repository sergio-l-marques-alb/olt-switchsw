...
L7_BOOL                lldpWarmRestart = L7_FALSE;
L7_BOOL                lldpActivateStartupDone = L7_FALSE;
L7_BOOL                lldpBackupElected = L7_FALSE;
void                  *lldpCkptReqQ = L7_NULLPTR;
avlTree_t              lldpCkptInfoAvlTree;
avlTreeTables_t       *lldpCkptInfoTreeHeap = L7_NULLPTR;
lldpCkptInfo_t        *lldpCkptInfoDataHeap = L7_NULLPTR;
  ...
  if (lldpCfgData->cfgHdr.dataChanged == L7_TRUE)
  {
                           ...
                           (L7_char8 *)lldpCfgData, sizeof(lldpCfgData_t)) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LLDP_COMPONENT_ID,
              "lldpSave: Error on call to sysapiCfgFileWrite file %s\n", LLDP_CFG_FILENAME);
    }
  }
