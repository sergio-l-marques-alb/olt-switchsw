...
L7_RC_t lldpXMedPolicyEventCallback(L7_uint32 intIfNum, lldpXMedPolicyInfoParms_t *policyInfo)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LLDP_COMPONENT_ID,
            "lldpXMedPolicyEventCallback(): message send failed\n");
  }
...
L7_RC_t lldpXMedPortAdminModeSet(L7_uint32 intIfNum, L7_uint32 value)
{
  ...
  lldpIntfModeApplyPostMsg(intIfNum, L7_TRUE, L7_TRUE); /* Rx/Tx apply */
...
L7_RC_t lldpXMedPortTransmitEnabledGet(L7_uint32 intIfNum, L7_uint32 *status)
{
  ...
  *status = (lldpIntfTbl[index].medTransmitEnabled > 0) ? L7_ENABLE : L7_DISABLE;
                                         ...
                                         L7_BOOL *unknown )
{
  ...
  switch (appType)
  {
    case voice_policyapptype:
      ...
      {
      L7_BOOL noneMode;
      if(voiceVlanPortNoneGet(intIfNum,&noneMode) == L7_SUCCESS && noneMode == L7_FALSE)
      {
        *unknown = L7_FALSE; /* Should be transmitted as a zero indicating that the network policy is defined*/
      }
      else
      {
        *unknown = L7_TRUE; /* Should be transmitted as a one indicating that the network policy is not defined*/
      }
        rc = L7_SUCCESS;
      }
      else
...
#ifdef L7_POE_PACKAGE
  rc = poeIsValidIntf(intIfNum);
  if(rc == L7_SUCCESS)
  {
    rc = poeIsValidPSEIntf(intIfNum);
    if(rc == L7_SUCCESS)
    {
      *mdiSupported = L7_TRUE;
    }
    else
    {
      rc = L7_SUCCESS;
      *mdiSupported = L7_FALSE;
    }
  }
#endif
  return rc;
...
#ifdef L7_POE_PACKAGE
  L7_uint32  val = 0;
  rc = poePethPsePortAdminEnableStateGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *mdiEnabled = val;
  }
...
#ifdef L7_POE_PACKAGE
  L7_uint32  val = 0;
  rc = poePethPsePortPowerPairsControlAbilityGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *pairControlable = val;
  }
...
#ifdef L7_POE_PACKAGE
  L7_uint32  val = 0;
  rc = poePethPsePortPowerPairsGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *pairs = val;
  }
...
#ifdef L7_POE_PACKAGE
  L7_uint32  val = 0;
  rc = poePethPsePortPowerClassificationsGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerClass = val;
  }
...
L7_RC_t lldpXdot3LocPowerTypeGet(L7_uint32 intIfNum, L7_uint32 *powerType)
{
  ...
  L7_uchar8  val = 0;
  rc = poePethPortLldpPowerTypeGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerType = val;
  }
...
#ifdef L7_POE_AT_PACKAGE
  L7_uchar8  val = 0;
  rc = poePethPortLldpPowerSourceGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerSource = val;
  }
...
/*********************************************************************
*
* @purpose  Gets Local Device Power Priroity value
*
* @param    L7_uint32                intIfNum  @b((input)) Internal Interface Number
...
#ifdef L7_POE_AT_PACKAGE
  L7_uchar8  val = 0;
  rc = poePethPortLldpPowerPriorityGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerPriority = val;
  }
...
L7_RC_t lldpXdot3LocPDRequestedPowerValueGet(L7_uint32 intIfNum, L7_uint32 *powerValue)
{
  ...
  if(rc == L7_SUCCESS)
  {
    rc = poePethPortLldpPDRequestedPowerValueGet(intIfNum,&val);
    if(rc == L7_SUCCESS)
    {
      *powerValue = val;
    }
...
L7_RC_t lldpXdot3LocPSEAllocatedPowerValueGet(L7_uint32 intIfNum, L7_uint32 *powerValue)
{
  ...
  if(rc == L7_SUCCESS)
  {
    rc = poePethPortLldpPSEAllocatedPowerValueGet(intIfNum,&val);
    if(rc == L7_SUCCESS)
    {
      *powerValue = val;
    }
...
#ifdef L7_POE_AT_PACKAGE
  L7_uint32  val = 0;
  rc = poePethPortLldpResponseTimeGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *responseTime = val;
  }
...
#ifdef L7_POE_AT_PACKAGE
  L7_BOOL  val = 0;
  rc = poePethPortLldpReadyGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *ready = val;
  }
...
* @returns  L7_FAILURE
*
* @noteshardwareRev should be a minimum of 32 bytes
*
* @end
*********************************************************************/
...
* @end
******************************************************************************/
L7_RC_t lldpMedProcessPoeDevicePdu(lldpRemDataEntry_t *remEntry,L7_BOOL poeDevice,L7_uchar8 flag)
{
  lldp8023PowerMdiTLV_t powerMdiInfo;
  lldpXMedNotify_t   notifyMsg;
  lldpXMedNotify_t*   notify = &notifyMsg;
  L7_uchar8 workByte;
  L7_ushort16 workShort;

  memset(&powerMdiInfo, 0x00, sizeof(lldp8023PowerMdiTLV_t));
  ...
  if(lldpOrgDefTLVGet(remEntry,
              ...
              (L7_uchar8*)&powerMdiInfo,
              sizeof(lldp8023PowerMdiTLV_t)) == L7_SUCCESS)
  {
    /* Power Support Byte*/
    workByte = (L7_uchar8)(powerMdiInfo.mdiSupport[0]);
       /* Port Class */
    if(workByte & LLDP_8023_POWER_MDI_PORT_CLASS)
    {
       notify->evData.poe.remLocPortClass = port_class_pse;
    }
    else
    {
       notify->evData.poe.remLocPortClass = port_class_pd;
    }
       /* MDI Support */
    if(workByte & LLDP_8023_POWER_MDI_POWER_SUPPORT)
    {
       notify->evData.poe.remLocPSEMdiSupport = L7_TRUE;
    }
    else
    {
       notify->evData.poe.remLocPSEMdiSupport = L7_FALSE;
    }
       /* MDI State */
    if(workByte & LLDP_8023_POWER_MDI_POWER_STATE)
    {
       notify->evData.poe.remLocPSEMdiPowerState = L7_TRUE;
    }
    else
    {
       notify->evData.poe.remLocPSEMdiPowerState = L7_FALSE;
    }

       /* MDI Port Pair Control Ability */
    if(workByte & LLDP_8023_POWER_MDI_PORT_PAIR_CNTRL)
    {
       notify->evData.poe.remLocPSEPairControlAbility = L7_TRUE;
    }
    else
    {
       notify->evData.poe.remLocPSEPairControlAbility = L7_FALSE;
    }

    /* PSE Power Pair Byte*/
    notify->evData.poe.remLocPowerPair = (L7_uchar8)(powerMdiInfo.psePair[0]);

    /* PSE Power Class Byte*/
    notify->evData.poe.remLocPowerClass = (L7_uchar8)(powerMdiInfo.powerClass[0]);

    /* Type/Source/Priority Byte */
    workByte = (L7_uchar8)(powerMdiInfo.type[0]);
    /* Power Device Type */
    switch(workByte & LLDP_MED_EXT_POWER_TYPE)
    {
      case POWER_DEVICE_TYPE2_PSE:
        notify->evData.poe.remLocPowerType = power_type_pse_type2;
        break;
      case POWER_DEVICE_TYPE2_PD:
        notify->evData.poe.remLocPowerType = power_type_pd_type2;
      break;
      case POWER_DEVICE_TYPE1_PSE:
        notify->evData.poe.remLocPowerType = power_type_pse_type1;
      break;
      case POWER_DEVICE_TYPE1_PD:
        notify->evData.poe.remLocPowerType = power_type_pd_type1;
      break;
    }
    /* Power Source */
    if(notify->evData.poe.remLocPowerType&1)
    {
      switch(workByte & LLDP_MED_EXT_POWER_SOURCE)
      {
        case POWER_SOURCE_PSE_AND_LOCAL:
          notify->evData.poe.remLocPowerSource = localAndPSE_pdsrc;
          break;
      case POWER_SOURCE_LOCAL:
    ...
    else
    {
      ...
      {
        case POWER_SOURCE_BACKUP:
          notify->evData.poe.remLocPowerSource = backup_psesrc;
          break;
        case POWER_SOURCE_PRIMARY:
          notify->evData.poe.remLocPowerSource = primary_psesrc;
          break;
        case POWER_SOURCE_UNKNOWN:
          notify->evData.poe.remLocPowerSource = unknown_psesrc;
          break;
      }
    }
    /* Power Priority */
    ...
    memcpy(&workShort, &powerMdiInfo.pdReqValue[0], sizeof(workShort));
    workShort = osapiNtohs(workShort);
    notify->evData.poe.remLocPDReqPowerValue = workShort;

    /* PSE Allocated Power Value */
    memcpy(&workShort, &powerMdiInfo.pseAllocValue[0], sizeof(workShort));
    workShort = osapiNtohs(workShort);
    notify->evData.poe.remLocPSEAllocPowerValue = workShort;
    if(flag)
    {
    ...
    {
      notify->event = LLDP_POE_MOD_EVENT;
    }
...
{
  L7_RC_t rc = L7_SUCCESS;
  lldp8023PowerMdiTLV_t info;
  L7_uint8 workByte;

  lldpRemDataEntry_t *remEntry;

  if (lldpPoeIsValidIntf(intIfNum) != L7_SUCCESS)
  {
  return L7_FAILURE;
  }
  ...
  workByte = (L7_uchar8)(info.mdiSupport[0]);
       /* Port Class */
   if(workByte & LLDP_8023_POWER_MDI_PORT_CLASS)
   {
      *portClass = L7_TRUE;
                           ...
                           L7_uint32 timestamp, L7_BOOL *pairControlable)
{
  ...
  workByte = (L7_uchar8)(info.mdiSupport[0]);
       /* Port Pair Control Ability*/
   if(workByte & LLDP_8023_POWER_MDI_PORT_PAIR_CNTRL)
   {
      *pairControlable = L7_TRUE;
   }
   else
   {
      *pairControlable = L7_FALSE;
   }
                           ...
                           L7_uint32 timestamp, L7_uint32 *pairs)
{
  ...
  *pairs = (L7_uint32)(info.psePair[0]);
                           ...
                           L7_uint32 timestamp, L7_uint32 *powerClass)
{
  ...
  *powerClass = (L7_uint32)(info.powerClass[0]);
                           ...
                           L7_uint32 timestamp, L7_uint32 *powerType)
{
  ...
  L7_uchar8  val = 0;
  osapiSemaTake(lldpSemaphore, L7_WAIT_FOREVER);
  rc = poePethPortLldpMirroredPowerTypeGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerType = val;
  }
                           ...
                           L7_uint32 timestamp,  L7_uint32 *powerSource)
{
  ...
  L7_uchar8  val = 0;
  osapiSemaTake(lldpSemaphore, L7_WAIT_FOREVER);
  rc = poePethPortLldpMirroredPowerSourceGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerSource = val;
  }
                           ...
                           L7_uint32 timestamp,  L7_uint32 *powerPriority)
{
  ...
  L7_uchar8  val = 0;
  osapiSemaTake(lldpSemaphore, L7_WAIT_FOREVER);
  rc = poePethPortLldpMirroredPowerPriorityGet(intIfNum, &val);
  if(rc == L7_SUCCESS)
  {
    *powerPriority = val;
  }
                           ...
                           L7_uint32 timestamp,  L7_uint32 *powerValue)
{
  ...
  if(rc == L7_SUCCESS)
  {
    rc = poePethPortLldpMirroredPDRequestedPowerValueGet(intIfNum,&val);
    if(rc == L7_SUCCESS)
    {
      *powerValue = val;
    }
                           ...
                           L7_uint32 timestamp,  L7_uint32 *powerValue)
{
  ...
  if(rc == L7_SUCCESS)
  {
    rc = poePethPortLldpMirroredPSEAllocatedPowerValueGet(intIfNum,&val);
    if(rc == L7_SUCCESS)
    {
      *powerValue = val;
    }
...
* @returns  L7_FAILURE
*
* @notesThe info should be supplied with sufficient space
*
* @end
*********************************************************************/
...
L7_RC_t lldpMedProcessVoiceDevicePdu(lldpRemDataEntry_t *remEntry,L7_BOOL voiceDevice,L7_uchar8 deviceType)
{
  ...
  L7_RC_t            result;
  L7_BOOL            remVoiceDevice = L7_FALSE;
  L7_uchar8          remDeviceType;
  ...
  result = lldpMedGetRemEntryVoiceInfo(remEntry, &remVoiceDevice, &remDeviceType);

  medData.evData.med.deviceType = remDeviceType;

  if ((voiceDevice    == L7_FALSE) &&
      (remVoiceDevice == L7_TRUE))
  {
    /* We see a new voice device endpoint class III
     * send out a ADD notification*/
    LLDP_TRACE(LLDP_DEBUG_MED, "Detected MED device type: %d, send Add", remDeviceType);
    medData.event = LLDP_MED_ADD_EVENT;
    lldpXMedNotifyRegisteredUsers(remEntry->intIntfNum, &medData);
  }
  else if ((voiceDevice    == L7_TRUE) &&
           (remVoiceDevice == L7_FALSE))
  {
    /* We do not see a previously detected voice device endpoint class III
     * send out a REMOVE notification */
    LLDP_TRACE(LLDP_DEBUG_MED, "Detected MED non-endpointClass3 device type: %d, send Remove", remDeviceType);
           ...
           (result != L7_SUCCESS))
  {
    /* We do not see a previously detected voice device endpoint class III
     * send out a REMOVE notification  */
    LLDP_TRACE(LLDP_DEBUG_MED, "Detected MED device not type: %d, send Remove", deviceType);
    medData.event = LLDP_MED_REMOVE_EVENT;
  ...
  {
    LLDP_TRACE(LLDP_DEBUG_MED, "Detected non-voice MED device not type: %d, return Success", deviceType);
  }
  return L7_SUCCESS;
}
...
{
  lldpMedCapTLV_t       info;
  lldpMedNetPolicyTLV_t netPolicy;

  memset(&info,      0x00, sizeof(info));
  memset(&netPolicy, 0x00, sizeof(netPolicy));

  if (lldpOrgDefTLVGet(remEntry,
                       (L7_uchar8*)LLDP_MED_TIA_OUI_STRING,
                       LLDP_MED_SUBTYPE_CAP,
                       (L7_uchar8*)&info,
                       sizeof(info)) == L7_SUCCESS)
  {
    if (((L7_uint32)info.deviceType[0] == endpointClass3) &&
        (lldpOrgDefMedXTLVGet(remEntry,
                              (L7_uchar8*)LLDP_MED_TIA_OUI_STRING,
                              LLDP_MED_SUBTYPE_NET_POLICY,
                              voice_policyapptype,
                              (L7_uchar8*)&netPolicy,
                              sizeof(netPolicy)) == L7_SUCCESS))
    {
      *voiceDevice = L7_TRUE;
    }
