       ...
       else
       {
         if (osapiSemaGive(agent.sFlowMsgQSema) != L7_SUCCESS)
         {
           L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                   "sFlowPduReceive: Failed to give msgQueue semaphore\n");
         }
         if (congestionCount)
         {
           congestionCount--;
                               ...
                               SFLOW_sample_data_t *pSample)
{
      ...
      pReceiver->rcvrCfg->sFlowRcvrMaxDatagramSize)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowReceiverFlowSampleAdd: Failed to send out datagram");
    }
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowReceiverFlowSampleAdd: sample length is overshooting max datagram size");
    return L7_FAILURE;
  }
  ...
  switch (pSample->data_format)
  {
    ...
    default:
         L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                 "sFlowReceiverSampleAdd: data_format %u not supported ", pSample->data_format);
         return L7_FAILURE;
         break;
  }
                                         ...
                                         SFLOW_flow_sample_t *pFlowSample)
{
   ...
   for (idx = 0 ; idx < pFlowSample->num_records; idx++)
   {
     ...
     switch(pFlowSampleRecord->tag)
     {
        ...
        default:
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                    "sFlowReceiverFlowSampleWrite: flow record type %u not supported ", pFlowSampleRecord->tag);
            break;
     }
   }
}
                                            ...
                                            SFLOW_counters_sample_t *pCounterSample)
{
   ...
   for (idx = 0; idx < pCounterSample->num_records; idx++)
   {
     ...
     switch (pCounterSampleRecord->tag)
     {
       ...
       default:
           L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                   "sFlowReceiverCounterSampleWrite: flow record type %u not supported ", pCounterSampleRecord->tag);
          break;
     }
   }
}
...
static void sFlowReceiverSampleDatagramHeaderWrite(SFLOW_receiver_t *pReceiver)
{
  ...
  if (pReceiver->rcvrCfg->sFlowRcvrAddressType == L7_AF_INET)
  {
    ...
    if (sFlowAgentAddrGet(&ipAddr) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowReceiverSampleDatagramHeaderWrite: IPv4 agent address get failed");
    }
  ...
  else if (pReceiver->rcvrCfg->sFlowRcvrAddressType == L7_AF_INET6)
  {
    if (sFlowAgentAddrGet(&ipAddr) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowReceiverSampleDatagramHeaderWrite: IPv6 agent address get failed");
    }
    pReceiver->sFlowDatagram.agent_address = ipAddr;
  }
#endif
