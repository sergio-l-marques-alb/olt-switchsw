    ...
    */
    ...
    if (osapiRWLockCreate(&rtrDiscIntfRWLock, OSAPI_RWLOCK_Q_PRIORITY) == L7_FAILURE)
    {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RTR_DISC_COMPONENT_ID,
                "%s %d: %s: Unable to create semaphore for router discovery interface linked list\n",__FILE__, __LINE__,routine_name);
        *pResponse = 0;
    ...
    } else
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RTR_DISC_COMPONENT_ID,
                    "%s: %d: %s: Failed to create Router Discovery processing task\n", __FILE__, __LINE__, routine_name);
            return L7_FAILURE;
        }
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RTR_DISC_COMPONENT_ID,
                    "%s: %d: %s: Unable to synchronize Router Discovery processing task\n", __FILE__, __LINE__, routine_name);
            return L7_FAILURE;
        }
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RTR_DISC_COMPONENT_ID,
                    "%s: %d: %s: Unable to create router discovery processing message queue \n", __FILE__, __LINE__, routine_name);
            return L7_FAILURE;
        } else
        {
                 ...
                 request < L7_CNFGR_RQST_LAST )
            {
                ...
                case L7_CNFGR_CMD_INITIALIZE:
                    ...
                    default:
                        ...
                        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RTR_DISC_COMPONENT_ID,
                                "%s %d: %s: Invalid command/request pair\n",__FILE__, __LINE__,routine_name);
                        break;
