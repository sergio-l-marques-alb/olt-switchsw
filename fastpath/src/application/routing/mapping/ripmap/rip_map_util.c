...
L7_RC_t ripMapAdminModeEnable(void)
{
      ...
      if (ripMapIntfMayParticipate(intIfNum) == L7_TRUE)
      {
        ...
        if (ripMapIntfAdminModeEnable(intIfNum) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
                  "ripMapAdminModeEnable: State change callback failed " \
                  "for router intf %d\n", i);
        }
      }
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
            "ripMapAdminModeEnable: Could not apply split horizon mode.\n");
  }
  if (ripMapAutoSummarizationModeApply(pRipMapCfgData->ripAutoSummarization) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
            "ripMapAdminModeEnable: Could not apply auto-summarization mode.\n");
  }
  if (ripMapHostRoutesAcceptModeApply(pRipMapCfgData->ripHostRoutesAccept) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
            "ripMapAdminModeEnable: Could not apply host routes accept mode.\n");
  }
  ...
  */
  ...
  if (pRipMapCfgData->ripDefRtAdv.defRtAdv == L7_ENABLE)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapAdminModeEnable: Could not apply Default Route Advertise mode.\n");
    }
  }
  ...
  for (index = REDIST_RT_FIRST+1; index < REDIST_RT_LAST; index ++)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapAdminModeEnable: Could not apply Route Redist for routes from -- protocol.\n");
    }
  }
...
L7_RC_t ripMapAdminModeDisable(void)
{
  ...
  for (i = RIP_MAP_RTR_INTF_FIRST; i <= max; i++)
  {
    if (ipMapRtrIntfToIntIfNum(i, &intIfNum) == L7_SUCCESS)
    {
          ...
          (pCfg->adminMode == L7_ENABLE))
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
                  "ripMapAdminModeDisable: State change callback failed " \
                  "for router intf %d\n", i);
        }
      }
    }
  ...
  */
  ...
  if (pRipInfo->ripDefRtAdv == L7_ENABLE)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapAdminModeDisable: Could not apply Default Route Advertise mode.\n");
    }
  }
  ...
  for (index = REDIST_RT_FIRST+1; index < REDIST_RT_LAST; index ++)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapAdminModeDisable: Could not disable Route Redist for routes from -- protocol %d\n", index);
    }
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
           "ripMapAdminModeDisable: RIP route table flush failed \n");
  }
  */
  return L7_SUCCESS;
}
...
L7_RC_t ripMapIntfAdminModeApply(L7_uint32 intIfNum, L7_uint32 mode)
{
  ...
  if (mode == L7_ENABLE)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapIntfAdminModeApply: Could not add RIP multicast addr\n");
  ...
  if (mode == L7_DISABLE)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "ripMapIntfAdminModeApply: Could not delete RIP multicast addr\n");
...
L7_BOOL ripMapIsInitialized(void)
{
  if (pRipInfo == L7_NULL)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
            "WARNING:  Invalid build - RIP component was not started\n");
    L7_assert(1);
    return L7_FALSE;
  }
...
*           - RIP is enabled
*           - Routing is operational on the interface (implies intf not acquired)
...
L7_BOOL ripMapIntfIsConfigurable(L7_uint32 intIfNum, ripMapCfgIntf_t **pCfg)
{
  ...
  if (nimConfigIdGet(intIfNum, &configId) == L7_SUCCESS)
  {
    if (NIM_CONFIG_ID_IS_EQUAL(&configId, &(pRipMapCfgData->intf[index].configId)) == L7_FALSE)
    {
      ...
      */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RIP_MAP_COMPONENT_ID,
              "Error accessing RIPMAP config data for interface %s in rippMapIntfIsConfigurable.\n", ifName);
      return L7_FALSE;
    }
  }
...
L7_BOOL ripMapIntfConfigEntryGet(L7_uint32 intIfNum, ripMapCfgIntf_t **pCfg)
{
  ...
  if ((nextIndex < L7_IPMAP_INTF_MAX_COUNT) &&
      (NIM_CONFIG_ID_IS_EQUAL(&pRipMapCfgData->intf[nextIndex].configId, &configIdNull)))
  {
    i = nextIndex;
  }
  else
  ...
  if ((i != 0) && (i < L7_IPMAP_INTF_MAX_COUNT))
  {
