...
typedef struct dhcpRelayInfo_s
{
   ...
   * binary semaphore and avoid the overhead of a read write lock. */
    ...
    /* Bit mask of NIM events that IP Helper is registered to receive */
    PORTEVENT_MASK_t ihNimEventMask;
...
typedef struct dhcpRelayNimStartup_s
{
  NIM_STARTUP_PHASE_t startupPhase;
} dhcpRelayNimStartup_t;
...
typedef struct udpRelayMsg_s
{
    ...
    union
    {
        ...
        dhcpRelayNimStartup_t nimStartupEvent;
