    ...
    do
    {
        ...
        if (dhcpRelayQueue == L7_NULLPTR)
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DHCP_RELAY_COMPONENT_ID,
                    "Unable to create IP helper message queue.\n");
            L7_assert(1);
        }
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DHCP_RELAY_COMPONENT_ID,
                    "Failed to create IP helper task\n");
            return L7_FAILURE;
        }
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DHCP_RELAY_COMPONENT_ID,
                    "Unable to synchronize with IP helper task\n");
            return L7_FAILURE;
        }
                                         ...
                                         L7_CNFGR_ERR_RC_t   *pReason )
{
    ...
    else if (nimRegisterIntfChange(L7_DHCP_RELAY_COMPONENT_ID,
                                   dhcpRelayIntfChangeCallback,
                                   dhcpRelayIntfStartupCallback, 500) != L7_SUCCESS)
    {
        *pResponse = 0;
        *pReason   = L7_CNFGR_ERR_RC_FATAL;
        dhcpRelayRC = L7_ERROR;
...
L7_RC_t dhcpRelayCnfgrInitPhase3Process( L7_CNFGR_RESPONSE_t *pResponse,
                                         L7_CNFGR_ERR_RC_t   *pReason )
{
  /* Zero NIM event mask before startup callback */
  memset(&ihInfo->ihNimEventMask, 0, sizeof(ihInfo->ihNimEventMask));
