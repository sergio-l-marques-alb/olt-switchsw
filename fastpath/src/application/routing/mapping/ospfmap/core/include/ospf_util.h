...
{
    /* The OSPF protocol engine has been initialized (i.e., RTO_Init() is done) */
    ...
    /* system up time when OSPF last entered p3 init. Used by stub 
     * router feature when router configured as stub router at
     * startup. */
    L7_uint32 p3InitTime;

    /* L7_TRUE if the checkpoint service has notified OSPF that a backup
     * management unit has been elected. OSPF checkpoints neighbors and
     * DR status when a backup exists. */
    L7_BOOL  backupElected;

    /* L7_TRUE if the last system restart was a warm restart. That is, whether
     * Configurator indicated a warm restart in its p3 init notification. This
     * does not necessarily mean OSPF does a graceful restart, since GR could
     * be disabled or OSPF could even be disabled at the time of the restart. */
    L7_BOOL lastSystemRestartWarm;

    /* L7_TRUE if IP MAP has informed OSPFv2 that it has issued all system startup
     * router events. When this is true, OSPF is guaranteed to have its configuration
     * and its router interface state. */
    L7_BOOL startupDone;
...
{                                                           \
   ...
   {                                                        \
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID, format, ##args);                               \
   }                                                        \
}
...
L7_BOOL ospfMayEnableOspf(void);
L7_RC_t ospfMapOspfEnable(L7_BOOL gracefulRestart);
...
L7_RC_t ospfMapStubRtrModeApply(OSPF_STUB_ROUTER_CFG_t stubRtrMode, 
                                L7_uint32 startupDuration);
L7_RC_t ospfMapStubRtrSummaryMetricApply(L7_uint32 overrideSummaryMetric, 
...
/* ospf_gr.c */
L7_RC_t o2TerminateProcess(void);
