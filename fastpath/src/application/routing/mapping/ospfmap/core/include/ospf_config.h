...
typedef struct ospfCfgData_s
{
  ...
  L7_uint32  opaqueLSAMode;            /* Enable opaque LSA support. L7_ENABLE or L7_DISABLE */
  ...
  /* Whether this router is willing to act as a helpful neighbor for 
   * a neighbor doing graceful restart */
  OSPF_HELPER_SUPPORT_t helperSupport;

  /* When acting as a helpful neighbor, whether to terminate GR on a 
   * topology change. */
  L7_BOOL strictLsaChecking;

  /* Whether to attempt graceful restart for unplanned, planned, or both types
   * of restarts. */
  OSPF_NSF_SUPPORT_t nsfSupport;

  /* Length of graceful restart interval. seconds. */
  L7_uint32 restartInterval;

  /* Whether OSPF is configured to be a stub router, either always or 
   * at startup. */
  OSPF_STUB_ROUTER_CFG_t stubRtrMode;

  /* Number of seconds that OSPF stays in stub router mode after reboot, if 
   * configured to start as stub router. */
  L7_uint32 stubRtrStartupTime;

  /* L7_ENABLE or L7_DISABLE. If enabled, when in stub router mode, override the
   * metric in T3 and T4 summary LSAs with the value in stubRtrSummaryMetric. */
  L7_uint32 overrideSummaryMetric;

  /* Metric to set in summary LSAs when in stub router mode and overriding 
   * summary LSA metrics. L7_OSPF_STUB_ROUTER_SUMLSA_MET_MIN to 
   * L7_OSPF_STUB_ROUTER_SUMLSA_MET_MAX. */
