...
L7_BOOL ospfMapRouteFromOspf(L7_routeEntry_t *routeEntry);
...
extern e_Err RTO_OpaqueCapability_Set(t_Handle RTO_Id, Bool opaqueEnabled);
extern e_Err ARO_OpaqueLsdbEntry(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id,
                                 L7_ospfOpaqueLsdbEntry_t *p_Lsa);
extern e_Err ARO_OpaqueLsdbNextEntry(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id,
                                     L7_ospfOpaqueLsdbEntry_t *p_Lsa);
extern e_Err ARO_OpaqueAdvertisement(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id,
                                     L7_ospfOpaqueLsdbEntry_t *p_Lsa,
                                     L7_char8 **p_LsdbAdvertisement);
...
static e_Err ospfStubRouterClear_Unpack(void *p_Info);
static e_Err ospfBeginGr_Unpack(void *p_Info);
static e_Err ospfStartupDone_Unpack(void *p_Info);
static e_Err ospfHelpfulNbrStatus_Unpack(void *p_Info);
...
/*********************************************************************
* @purpose  Set the configured stub router mode.
*
                                   ...
                                   L7_uint32 startupDuration)
{
  ...
  memcpy(&rtrCfg, &p_RTO->Cfg, sizeof(t_S_RouterCfg));
  rtrCfg.StubRtrMode = stubRtrMode;
  rtrCfg.StubRtrStartupTime = startupDuration;

  return ospfMapExtenRTOConfigSet(&rtrCfg);
}

/*********************************************************************
* @purpose  Set the configured stub router summary LSA metric.
*
* @param    overrideSummaryMetric   @b{(input)} the configured stub router mode
* @param    stubRtrSummaryMetric    @b{(input)} If stubRtrMode is startup, how long
*                                               OSPF stays in stub router mode on 
*                                               startup (seconds) 
*
* @returns  L7_SUCCESS  if success
                                            ...
                                            L7_uint32 stubRtrSummaryMetric)
{
  ...
  if (overrideSummaryMetric == L7_ENABLE)
    rtrCfg.StubRtrSummaryMetricOveride = TRUE;
  else
    rtrCfg.StubRtrSummaryMetricOveride = FALSE;

  rtrCfg.StubRtrSummaryMetric = stubRtrSummaryMetric;
...
static e_Err ospfMapExtenIntfAreaIdSet(void *p_Info)
{
  ...
  if (e != E_OK || p_ARO == NULL)
  {
    if (ARO_Init(p_IFO, (void *)&p_ARO) != E_OK)
    {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                "Failed to initialize area %s while setting the OSPF area ID on interface %s.",
                areaIdStr, ifName);
  ...
  else
  {
      ...
      if (HL_FindFirst(p_ARO->IfoHl, (byte*)&p_IFO->IfoId, NULL) == E_OK)
      {
          ...
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                  "Interface %s already in list of interfaces for area %s",
                  ifName, areaIdStr);
      }
  }
...
L7_RC_t ospfMapExtenIntfAreaIdPack(L7_uint32 intIfNum, L7_uint32 area)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfMapExtenIntfAreadIdSet on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
/*********************************************************************
* @purpose  Get the OSPF options for a neighbor with a given
*           router ID on a given interface
*
* @param    intIfNum @b{(input)}  Internal Interface Number
* @param    routerId @b{(input)}  the neighbor's router ID
* @param    *val     @b{(output)} options
*
* @returns  L7_SUCCESS if neighbor found and options returned
* @returns  L7_FAILURE otherwise
*
* @notes    none
*
...
{
  t_Handle ospfNbrTablePtr = ospfNbrTable_Lookup(ospfMapMibCtrl_g.p_ospfMibObj,
                                                 ipAddress, intIfNum);
  L7_uint32 nbrOptions = 0;

  *val = 0;
  if (ospfNbrTablePtr != L7_NULLPTR)
  {
    nbrOptions = OSPFNBR_OPTIONS(ospfNbrTablePtr);
    if ((nbrOptions & OSPF_OPT_Q_BIT) == OSPF_OPT_Q_BIT)
    {
      *val |= L7_OSPF_OPT_Q_BIT;
    }
    if ((nbrOptions & OSPF_OPT_E_BIT) == OSPF_OPT_E_BIT)
    {
      *val |= L7_OSPF_OPT_E_BIT;
    }
    if ((nbrOptions & OSPF_OPT_N_BIT) == OSPF_OPT_N_BIT)
    {
      *val |= L7_OSPF_OPT_NP_BIT;
    }
    if ((nbrOptions & OSPF_OPT_O_BIT) == OSPF_OPT_O_BIT)
    {
      *val |= L7_OSPF_OPT_O_BIT;
    }
    /* don't support VPN or MC options */
    return L7_SUCCESS;
  }
  return L7_FAILURE;
}
                               ...
                               L7_RtbEntryInfo_t *p_rtbEntry)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAbrEntry_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                                ...
                                L7_RtbEntryInfo_t *p_rtbEntry)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAsbrEntry_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                            ...
                            L7_RtbEntryInfo_t *p_rtbEntry)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAbrEntryNext_UnPack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                             ...
                             L7_RtbEntryInfo_t *p_rtbEntry)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAsbrEntryNext_UnPack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                                       ...
                                       L7_uint32 *val)
{
  ...
  memset(&Lsa, 0, sizeof(L7_ospfLsdbEntry_t));
  Lsa.intIfNum = intIfNum;
...
* @end
*********************************************************************/
L7_RC_t ospfMapExtenAreaLSADbLSACksumGet(L7_uint32 areaId, L7_uint32 intIfNum,
                                         L7_int32 Type,
                                         L7_uint32 Lsid, L7_uint32 RouterId,
                                         L7_uint32 *val)
{
                              ...
                              L7_ospfLsdbEntry_t *p_Lsa)
{
  ...
  if((e = ARO_LsdbNextEntry(p_ARO, p_Lsa)) != E_OK)
  {
    ...
    {
      /* Get the Next Area */
      e = HL_GetNext(p_RTO->AroHl, (void **)&p_ARO, (void*)p_ARO);
      if ((e != E_OK) || (p_ARO == NULL))
      {
        return L7_FAILURE;
      }
...
L7_RC_t ospfMapExtenNSSAConfigurationGet(L7_uint32 areaId, t_S_NssaCfg *cfg)
{
  ...
  cfg->NSSADefMetric   = (ulng)((metric == L7_OSPF_NOT_CONFIGURED)? 10: metric);
                                            ...
                                            L7_uint32 *nssaTRState)
{
  ...
  */
  ...
  if(NSSA_Config_Pack(areaId, &nssaCfg) == E_OK)
  {
...
L7_RC_t ospfMapExtenRTOConfigSet(t_S_RouterCfg *p_rtoCfg)
{
  ...
  if ((erc = RTO_Config_Pack(ospfMapCtrl_g.RTO_Id, p_rtoCfg)) != E_OK)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_OSPF_MAP_COMPONENT_ID,
            "Failed to configure OSPF. Failure code %u.", erc);
    return L7_FAILURE;
  }
...
ospfMapRedistMetricType(L7_routeEntry_t *routeEntry)
{
    ...
    if (redistCfg) {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                "SOFTWARE ERROR: Unable to get redistribution configuration\nin ospfMapRedistMetricType().\n");
...
/*********************************************************************
* @purpose  Determine whether a given route is an OSPF route.
*
* @param    routeEntry @b{(input)} Route in question
*
* @returns  L7_SUCCESS
*
* @notes
...
L7_BOOL ospfMapRouteFromOspf(L7_routeEntry_t *routeEntry)
{
  return ((routeEntry->protocol == RTO_OSPF_INTRA_AREA) ||
          (routeEntry->protocol == RTO_OSPF_INTER_AREA) ||
          (routeEntry->protocol == RTO_OSPF_TYPE1_EXT) ||
          (routeEntry->protocol == RTO_OSPF_TYPE2_EXT) ||
          (routeEntry->protocol == RTO_OSPF_NSSA_TYPE1_EXT) ||
          (routeEntry->protocol == RTO_OSPF_NSSA_TYPE2_EXT));
}
                                  ...
                                  RTO_ROUTE_EVENT_t route_status)
{
    ...
    default:
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                "ERROR:  Invalid route_status %d.\n", route_status);
    }

  if ((routeEntry->ipAddr == 0) && (routeEntry->subnetMask == 0))
  {
    if ((route_status == RTO_ADD_ROUTE) || (route_status == RTO_CHANGE_ROUTE))
    {
      /* Update default route status */
      if (ospfMapRouteFromOspf(routeEntry))
      {
        /* RTO's best default route is an OSPF route. Don't use this to
         * trigger orig of an OSPF default route. */
    ...
    else if (route_status == RTO_DELETE_ROUTE)
    {
...
e_Err RTO_Config_Pack(t_Handle RTO_Id, t_S_RouterCfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call RTO_Config_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
      ...
      (p_Cfg->RouterStatus == ROW_DESTROY))
  {
    ...
    {
      if (osapiMsgQueueGetNumMsgs((void *)(pThreadGlobal->QueueID[q]), &numOfMsgs) == L7_SUCCESS)
      {
...
e_Err IFO_Config_Pack(t_Handle IFO_Id, t_IFO_Cfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_Config_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err ARO_Config_Pack(t_Handle ARO_Id, t_S_AreaCfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ARO_Config_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                        ...
                                        L7_char8 **p_LsdbAdvertisement)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfLsdbAdvertisement_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                ...
                                L7_ospfLsdbEntry_t *p_Lsa)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfLsdbTable_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                 ...
                                 L7_ospfLsdbEntry_t *p_Lsa)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfLsdbTable_GetNext_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                           ...
                                           L7_char8 **p_LsdbAdvertisement)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfExtLsdbAdvertisement_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                   ...
                                   L7_ospfLsdbEntry_t *p_Lsa)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfExtLsdbTable_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
                                    ...
                                    L7_ospfLsdbEntry_t *p_Lsa)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ospfExtLsdbTable_GetNext_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err NBO_Config_Pack(t_Handle NBO_Id, t_S_NeighborCfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call NBO_Config_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
  ...
  */
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    if (e != E_OK)
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call RTO_AsExternalCfg_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err RTO_PurgeExternalLsas_Pack(t_Handle RTO_Id)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call RTO_PurgeExternalLsas_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err ARO_StubMetric_Pack(t_Handle ARO_Id, t_S_StubAreaEntry *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ARO_StubMetric_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err ARO_AddressRange_Pack(t_S_AreaAggregateEntry *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ARO_AddressRange_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err ARO_HostConfig_Pack(t_Handle ARO_Id, t_S_HostRouteCfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ARO_HostConfig_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err ARO_Delete_Pack(t_Handle ARO_Id, ulng flag)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call ARO_Delete_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_Delete_Pack(t_Handle IFO_Id, ulng flag, L7_uint32 intIfNum)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_Delete_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_Up_Pack(t_Handle IFO_Id)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_Up_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_Down_Pack(t_Handle IFO_Id)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_Down_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_MetricConfig_Pack(t_Handle IFO_Id, t_S_IfMetric *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_MetricConfig_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_AuthKeyConfig_Pack(t_Handle IFO_Id, t_OspfAuthKey *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call IFO_AuthKeyConfig_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err NSSA_Config_Pack(L7_uint32 areaId, t_S_NssaCfg *p_Cfg)
{
    ...
    if (currThread != (L7_int32) pThreadGlobal->TaskID)
    {
        ...
        */
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Failed to call NSSA_Config_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
            return E_FAILED;
        }
...
e_Err IFO_Stats_Pack(t_IFO *p_IFO, t_PCK_Stats *intfStats)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
            "Failed to call IFO_Stats_Unpack() on OSPF thread\n");
    osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
    return E_FAILED;
  }
...
e_Err RTO_OpaqueCapability_Get_Pack(t_Handle RTO_Id, L7_uint32 *opaqueEnabled)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call RTO_OpaqueCapability_Get_UnPack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
static e_Err RTO_OpaqueCapability_Set_UnPack(void *p_Info)
{
  t_XXCallInfo  *xxCallInfo = (t_XXCallInfo *)p_Info;
  t_Handle      RTO_Id = (t_Handle)PACKET_GET(xxCallInfo, 0);
  Bool     opaqueEnabled = (Bool) PACKET_GET(xxCallInfo, 1);
  e_Err         e;
 ...
 *
 * @param    RTO_Id     RTO object Id
 * @param    opaqueEnabled   L7_TRUE if opaque LSA support is enabled
 *
 * @returns  E_OK           config successfuly setup or read
 *           E_FAILED       couldn't setup or read
 *           E_BADPARM      bad configuration parameters
...
e_Err RTO_OpaqueCapability_Set_Pack(t_Handle RTO_Id, L7_BOOL opaqueEnabled)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call RTO_OpaqueCapability_Set_UnPack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
 ...
 *
 * @param    opaqueEnabled   @b{(input)}   L7_ENABLE or L7_DISABLE
 *
 * @returns  L7_SUCCESS
 * @returns  L7_FAILURE
 *
...
{
  e_Err erc;
  L7_BOOL on = (opaqueEnabled == L7_ENABLE);
  ...
  if ((erc = RTO_OpaqueCapability_Set_Pack(ospfMapCtrl_g.RTO_Id, on)) != E_OK)
  {
    OSPFMAP_ERROR("%s %d: %s: ERROR: OSPF_MAP: Unable to get opaque support for OSPF, rc=%d\n",
                  __FILE__, __LINE__, __FUNCTION__, erc);
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  /* First find the ARO object corresponding to the LsdbAreaId */
  e = HL_FindFirst(p_RTO->AroHl, (byte*)&lsdbAreaId, (void**)&p_ARO);
  if ((e != E_OK) || (p_ARO == NULL))
  {
    return L7_FAILURE;
  }
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if ((e = ARO_OpaqueLsdbEntry(p_RTO, p_ARO, NULL, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfaAreaOpaqueLsdbTable_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueLsdbNextEntry(p_RTO, p_ARO, NULL, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAreaOpaqueLsdbTable_NextLookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueAdvertisement(p_RTO, p_ARO, NULL, p_Lsa, p_LsdbAdvertisement)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAreaOpaqueAdvertisement_Lookup_Pack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                                       ...
                                       L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  /* Use the MIB table to get the interface object corresponding to
   * interface address or interface index */
  p_IFO = ospfIfTable_GetOwner(ospfMapMibCtrl_g.p_ospfMibObj, ipAddr, ifIndex);
  if (!p_IFO)
    return L7_FAILURE;
  ...
  /* Find the LSA corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueLsdbEntry(p_RTO, p_ARO, p_IFO, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfLinkOpaqueLsdbTable_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
/*********************************************************************
 * @purpose  Given the attributes of a link opaque LSA, find the next
 *           link opaque LSA.
 *
 * @param    ipAddr      primary IP address on interface associated with link scope LSA.
 *                       Only set for numbered interfaces.
 * @param    ifIndex     interface index of interface associated with link scope LSA.
 *                       Not guaranteed to be non-zero for numbered interfaces.
 * @param    Type        Lsdb Type
 * @param    Lsid        Link State id
 * @param    RouterId    Database entry
 * @param    p_Lsa       Lsdb Entry given to the external world
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  e_Err         e = E_FAILED;
  L7_ospfOpaqueLsdbEntry_t temp_Lsa;
  ulng ipAddrLong = (ulng) *ipAddr;
  ulng ifIndexLong = (ulng) *ifIndex;
  ...
  if ((*ipAddr == 0) && (*ifIndex == 0))
  {
    /* get first interface */
    if (ospfIfTable_GetFirst(ospfMapMibCtrl_g.p_ospfMibObj, &ipAddrLong,
                             &ifIndexLong) == NULL)
    {
      return L7_FAILURE;
    }
  ...
  /* Use the MIB table to get the interface object corresponding to
   * interface address or interface index */
  p_IFO = (t_IFO*) ospfIfTable_GetOwner(ospfMapMibCtrl_g.p_ospfMibObj,
                                        ipAddrLong, ifIndexLong);
  if (!p_IFO)
  {
    return L7_FAILURE;
  }
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueLsdbNextEntry(p_RTO, p_ARO, p_IFO, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
  p_Lsa->IpAdr = p_IFO->Cfg.IpAdr;
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfLinkOpaqueLsdbTable_NextLookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
/*********************************************************************
 * @purpose  Given a set of attributes uniquely identifying a link opaque
 *           LSA, find the database entry and copy the LSA itself to an
 *           output buffer.
 *
 * @param    ipAddr      primary IP address on interface associated with link scope LSA
 * @param    ifIndex     Interface Index of interface associated with link scope LSA
 * @param    Type        Lsdb Type, must be S_LINK_OPAQUE_LSA.
 * @param    Lsid        Link State id
 * @param    RouterId    Database entry
 * @param    p_Lsa       Lsdb Entry given to the external world
 * @param    p_LsdbAdvertisement  Lsdb Advertisement
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueAdvertisement(p_RTO, p_ARO, p_IFO, p_Lsa, p_LsdbAdvertisement)) != E_OK)
  {
    return L7_FAILURE;
  }
  p_Lsa->IpAdr = p_IFO->Cfg.IpAdr;
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfLinkOpaqueAdvertisement_Lookup_Pack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
/*********************************************************************
 * @purpose  Given a set of attributes identifying a link opaque LSA, copy
 *           the LSA into a given output buffer.
 *
 * @param  ipAddr     @b{(input)}  primary IP address on interface associated with link scope LSA
 * @param  ifIndex    @b{(input)}  Interface Index of interface associated with link scope LSA
 * @param  Type       @b{(input)}  LS type. Must be L7_S_LINK_OPAQUE_LSA.
 * @param  Lsid       @b{(input)}  Link State ID
 * @param  RouterId   @b{(input)}  advertising router
 * @param  buf        @b{(output)} output location for copy of LSA itself
 * @param  len        @b{(output)} length of LSA, incl. LSA header
 *
 * @returns  L7_SUCCESS  if success
 * @returns  L7_FAILURE  if failure
 *
                                     ...
                                     L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueLsdbEntry(p_RTO, NULL, NULL, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAsOpaqueLsdbTable_Lookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if((e = ARO_OpaqueLsdbNextEntry(p_RTO, NULL, NULL, p_Lsa)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAsOpaqueLsdbTable_NextLookup_Unpack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
                                       ...
                                       L7_char8 **p_LsdbAdvertisement)
{
  ...
  /* Find the Lsa corresponding to LsdbType, LsdbId, LsdbRouterId */
  if ((e = ARO_OpaqueAdvertisement(p_RTO, NULL, NULL, p_Lsa, p_LsdbAdvertisement)) != E_OK)
  {
    return L7_FAILURE;
  }
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfAsOpaqueAdvertisement_Lookup_Pack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
e_Err ospfASOpaqueCksumSum_Get_Pack(t_Handle RTO_Id, L7_uint32 *opaqueCksumSum)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call RTO_ASOpaqueCksumSum_Get_UnPack on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
e_Err ospfIsStubRtr_Get_Pack(t_Handle RTO_Id, L7_BOOL *isStubRtr)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfIsStubRtr_Get_Unpack() on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
e_Err ospfExtLsdbOverflow_Get_Pack(t_Handle RTO_Id, L7_BOOL *extLsdbOverflow)
{
  ...
  if (currThread != (L7_int32) pThreadGlobal->TaskID)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to call ospfExtLsdbOverflow_Get_Unpack() on OSPF thread\n");
      osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
      return E_FAILED;
    }
...
/*********************************************************************
* @purpose  Tell OSPF to exit stub router mode.
*
* @param    RTO_Id - OSPF instance
*
* @returns  E_OK           config successfuly setup or read
...
/*********************************************************************
* @purpose  Callback function to be called by the ospf thread to 
*           exit stub router mode.
*
* @param    p_Info         pointer to the message containing
*                          the required parameters
*
...
L7_RC_t ospfMapExtenStubRouterClear(void)
{
  ...
  if (ospfStubRouterClear_Pack(RTO_Id) == E_OK)
    return L7_SUCCESS;
...
/*********************************************************************
* @purpose  Tell OSPF that a graceful restart is beginning
*
* @param    RTO_Id - OSPF instance
*
* @returns  E_OK           config successfuly setup or read
...
/*********************************************************************
* @purpose  Callback function to be called by the OSPF protocol thread
*           when starting a graceful restart
*
* @param    p_Info         pointer to the message containing
*                          the required parameters
*
...
static e_Err ospfBeginGr_Unpack(void *p_Info)
{
  ...
  e = o2BeginGracefulRestart(RTO_Id);
  osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
  return e;
}

/*********************************************************************
* @purpose  Notify OSPF protocol that a graceful restart is afoot.
*
* @param    void
*
* @returns  L7_SUCCESS
...
L7_RC_t ospfMapExtenBeginGracefulRestart(void)
{
  ...
  if (ospfBeginGr_Pack(RTO_Id) == E_OK)
    return L7_SUCCESS;
...
/*********************************************************************
* @purpose  Tell OSPF that all interfaces have been enabled
*
* @param    RTO_Id - OSPF instance
*
* @returns  E_OK           config successfuly setup or read
...
*           E_BADPARM      bad configuration parameters
*
* @notes    Useful when doing graceful restart. OSPF can then see if any
*           checkpointed neighbors are on interfaces that have not come back.
*
* @end
*********************************************************************/
e_Err ospfStartupDone_Pack(t_Handle RTO_Id)
...
static e_Err ospfStartupDone_Unpack(void *p_Info)
{
  ...
  e = o2StartupDone(RTO_Id);
  osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
  return e;
}

/*********************************************************************
* @purpose  Notify OSPF protocol when all OSPF interfaces are up.
*
* @param    void
*
* @returns  L7_SUCCESS
...
L7_RC_t ospfMapExtenStartupDone(void)
{
  ...
  if (ospfStartupDone_Pack(RTO_Id) == E_OK)
    return L7_SUCCESS;
...
/*********************************************************************
* @purpose  Communicate the helper configuration to the protocol layer
*
* @param    helperSupport - whether to serve as helpful neighbor
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
L7_RC_t ospfMapExtenHelperSupportSet(OSPF_HELPER_SUPPORT_t helperSupport)
{
  ...
  memcpy(&rtrCfg, &p_RTO->Cfg, sizeof(t_S_RouterCfg));
  rtrCfg.HelperSupport = helperSupport;

  return ospfMapExtenRTOConfigSet(&rtrCfg);
}

/*********************************************************************
* @purpose  Communicate the strict LSA checking configuration to the protocol layer
*
* @param    stricLsaChecking - whether to perform strict LSA checking
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
L7_RC_t ospfMapExtenStrictLsaCheckingSet(L7_BOOL strictLsaChecking)
{
  ...
  memcpy(&rtrCfg, &p_RTO->Cfg, sizeof(t_S_RouterCfg));
  rtrCfg.StrictLsaChecking = strictLsaChecking;
...
static e_Err ospfHelpfulNbrStatus_Unpack(void *p_Info)
{
  t_XXCallInfo  *xxCallInfo = (t_XXCallInfo *)p_Info;
  t_Handle  RTO_Id = (t_Handle)PACKET_GET(xxCallInfo, 0);
  L7_uint32  intIfNum = PACKET_GET(xxCallInfo, 1);
  L7_uint32  nbrIpAddr = PACKET_GET(xxCallInfo, 2);
  L7_ospfHelpfulNbrStatus_t *nbrStatus =
    (L7_ospfHelpfulNbrStatus_t*) PACKET_GET(xxCallInfo, 3);
  e_Err e;

  e = o2HelpfulNbrStatusGet(RTO_Id, intIfNum, nbrIpAddr, nbrStatus);
  osapiSemaGive(ospfMapCtrl_g.ospfSyncSema);
  return e;
}

/*********************************************************************
* @purpose  Request helpful neighbor status for specific neighbor
*
* @param    RTO_Id     OSPF instance
* @param    intIfNum   Internal interface number of neighbor
* @param    nbrIpAddr  IPv4 address of neighbor's interface address
* @param    status     output structure ptr
*
* @returns  E_OK           config successfuly setup or read
*           E_FAILED       couldn't setup or read
                                ...
                                L7_ospfHelpfulNbrStatus_t *status)
{
  ...
  PACKET_INIT(xxci, ospfHelpfulNbrStatus_Unpack, 0, 0, 4, (ulng)RTO_Id);
  PACKET_PUT(xxci, intIfNum);
  PACKET_PUT(xxci, nbrIpAddr);
  PACKET_PUT(xxci, (ulng) status);
  if (osapiSemaTake(ospfMapCtrl_g.ospfSyncSema, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
                                        ...
                                        L7_ospfHelpfulNbrStatus_t *nbrStatus)
{
  ...
  if (ospfHelpfulNbrStatus_Pack(RTO_Id, intIfNum, nbrIpAddr, nbrStatus) == E_OK)
    return L7_SUCCESS;
