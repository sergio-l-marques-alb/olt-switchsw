          ...
          else
          {
              ...
              if (vrrpRC != L7_SUCCESS)
              {
                  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_VRRP_MAP_COMPONENT_ID,
                          "%s: %d: vrrpApiCnfgrCommand: osapiMessageSend failed\n",__FILE__, __LINE__);
              }
...
static void vrrpFuncTableInit(void)
{
    ...
    vrrpFuncTable.L7_vrrpMasterGratARPSend     = L7_vrrpMasterGratARPSend;
    vrrpFuncTable.L7_vrrpStateGet              = L7_vrrpStateGet;
}
