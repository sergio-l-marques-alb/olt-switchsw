                            ...
                            L7_uint32 *pIntIfNum)
{
  ...
  if ((rc = nimIntfCreate(&nimRequest, &nimOutput)) != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "RLIM: failed to create interface %s\n", namebuf);
    return L7_FAILURE;
  }
  ...
  if (detach == L7_TRUE)
  {
        ...
        (linkState == L7_UP) && (iftype == L7_LOOPBACK_INTF))
    {
      ...
      if (nimEventIntfNotify(eventInfo,&handle) != L7_SUCCESS)
      {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                "down notification failed for intf %s\n", ifName);
      }
    }
    ...
    if (nimEventIntfNotify(eventInfo, &handle) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "detach notification failed for intf %s\n", ifName);
      return L7_FAILURE;
    }
  }                             /* detach == L7_TRUE */
  else
  {
    ...
    while (1)
    {
      ...
      if (nimIntfQuery(&nimQueryData) != L7_SUCCESS)
      {
        /* should never get here */
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                "nimIntfQuery failed for intf %s in delete attempt\n",
                ifName);
        return L7_FAILURE;
      }
      ...
      switch (nimQueryData.data.state)
      {
        ...
        default:
          /* detach the interface before deleting */
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                  "Interface %s should not be in state %d\n",
                  ifName, nimQueryData.data.state);
          wait_for_transition = L7_FALSE;
          return L7_FAILURE;
          break;
      }                         /* switch */
    ...
    if (nimEventIntfNotify(eventInfo, &handle) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
              "delete notification failed for intf %s\n", ifName);
      return L7_FAILURE;
    }
  }                             /* detach != L7_TRUE */
...
void rlimNimEventCompletionCallback(NIM_NOTIFY_CB_INFO_t retVal)
{
  ...
  {
    L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
    nimGetIntfName(retVal.intIfNum, L7_SYSNAME, ifName);

    /* Failed to complete the request */
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
            "RLIM: notification failed for event(%d), intf(%s), reason(%d)\n",
      ...
      case L7_ATTACH:
            ...
            itype == L7_LOOPBACK_INTF)
        {
          ...
          if (rc != L7_SUCCESS)
          {
            ...
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                    "failed to enable routing on intf %s", ifName);
          }
          ...
          if (rc != L7_SUCCESS)
          {
            ...
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_RLIM_COMPONENT_ID,
                    "failed to enable v6 routing on intf %s\n",
                    ifName);
          }
#endif
        }
