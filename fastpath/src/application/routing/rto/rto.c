...
static rtoNextHop_t      *rtoNextHopHeap = NULL;
...
static rtoRouteInfo_t *rtoFreeRouteInfoList = NULL;

/* Head of the list of free next hop objects. */
static rtoNextHop_t *rtoFreeNextHopList = NULL;
...
L7_uint32 rtoTracePfx = RTO_TRACE_PFX_NONE;
...
rtoCnfgrInitPhase1Process(L7_uint32 max_routes, L7_uint32 max_nhres)
{
  ...
  L7_uint32 nextHopHeapSize;  /* memory for rtoNextHop_t structures that describe
                               * the next hops of a route. Each routeInfo object
                               * has a linked list of next hops, up to the max
                               * number of next hops allowed by the active SDM 
                               * template. */

  L7_uint32 maxAllRoutes;  /* max routes (best + non-best) */
  L7_uint32 maxNextHops;   /* Number of next hops if all routes have max next hops. */
  ...
  memset((void *) CallbackList, 0, sizeof (CallbackList));
  ...
  maxNextHops = maxAllRoutes * platRtrRouteMaxEqualCostEntriesGet();

  /* calculate buffer sizes. */
  treeHeapSize = RADIX_TREE_HEAP_SIZE(RtoMaxRoutes, sizeof(rtoTreeKey_t));
  dataHeapSize = RADIX_DATA_HEAP_SIZE(RtoMaxRoutes, sizeof(rtoRouteData_t));
  routeInfoSize = maxAllRoutes * sizeof(rtoRouteInfo_t);
  nextHopHeapSize = maxNextHops * sizeof(rtoNextHop_t);

  if (rtoTraceFlags & RTO_TRACE_ALLOC) 
  {
    L7_uchar8 traceBuf[RTO_TRACE_LEN_MAX];
    osapiSnprintf(traceBuf, RTO_TRACE_LEN_MAX, 
                  "RTO allocating memory for radix tree supporting %u routes. "
                  "\ntree heap: %u\ndata heap: %u\nroute info: %u\nnext hop heap: %u",
                  max_routes, treeHeapSize, dataHeapSize, routeInfoSize, nextHopHeapSize);
  ...
  rtoNextHopHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, nextHopHeapSize);
  if (rtoNextHopHeap == L7_NULLPTR)
  {
    return L7_FAILURE;
  }

  /* Cleanup data structures before proceeding */
  memset((char *)rtoRouteTreeHeap, 0, treeHeapSize);
  memset((char *)rtoRouteDataHeap, 0, dataHeapSize);
  memset((char *)rtoRouteInfo, 0, routeInfoSize);
  memset((char *)rtoNextHopHeap, 0, nextHopHeapSize);
  memset((char *) &rtoStats, 0, sizeof(rto_stats_t));

  rtoEcmpRouteCount = 0;
  rtoEcmpRouteMax = platRtrRouteMaxEntriesGet();
  ...
  radixCreateTree(&rtoRouteTreeData, rtoRouteDataHeap, rtoRouteTreeHeap, 
                  RtoMaxRoutes, sizeof(rtoRouteData_t), sizeof(rtoTreeKey_t));
  ...
  ** Here we create the free list. */
  ...
  /* Create free list of next hops. */
  for (i = 0; i < maxNextHops; i++)
  {
    rtoNextHopHeap[i].nextNh = rtoFreeNextHopList;
    rtoFreeNextHopList = &rtoNextHopHeap[i];
  }

  RtoMaxNHRes  = max_nhres;
  /* space for next hop resolution tree */
  rtoNHResTreeHeap = osapiMalloc(L7_IP_MAP_COMPONENT_ID, 
                                 RADIX_TREE_HEAP_SIZE(RtoMaxNHRes, sizeof(rtoTreeKey_t)));
...
rtoCnfgrFiniPhase1Process(L7_uint32 max_routes)
{
  ...
  if (rtoRouteTreeHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoRouteTreeHeap);
    rtoRouteTreeHeap = L7_NULLPTR;
  }

  if (rtoRouteDataHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoRouteDataHeap);
    rtoRouteDataHeap = L7_NULLPTR;
  }

  if (rtoRouteInfo != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoRouteInfo);
    rtoRouteInfo = L7_NULLPTR;
  }

  if (rtoNextHopHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoNextHopHeap);
    rtoNextHopHeap = L7_NULLPTR;
  }
  ...
  if (rtoNHResTreeHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoNHResTreeHeap);
    rtoNHResTreeHeap = L7_NULLPTR;
  }

  if (rtoNHResDataHeap != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoNHResDataHeap);
    rtoNHResDataHeap = L7_NULLPTR;
  }

  if (rtoProtoInfo != L7_NULLPTR)
  {
    osapiFree(L7_IP_MAP_COMPONENT_ID, rtoProtoInfo);
    rtoProtoInfo = L7_NULLPTR;
  }
...
/*********************************************************************
* @purpose  Return a route info structure to the free list.
*
* @param   routeInfo @b{(input)}  route info now free
*
* @returns  void
*
* @notes
...
* @end
*********************************************************************/
void rtoRouteInfoFree(rtoRouteInfo_t *routeInfo)
{
  /* Free each of the route's next hops */
  while (routeInfo->nextHops)
  {
    rtoRouteInfoNextHopDelete(routeInfo, routeInfo->nextHops);
  }
...
/*********************************************************************
* @purpose  Get a next hop structure from the free list.
*
* @param    none
*
* @returns  pointer to an available next hop structure. All fields
*           in the structure are initialized to 0.
*
* @notes
*
* @end
*********************************************************************/
rtoNextHop_t *rtoFreeNextHopGet(void)
{
  rtoNextHop_t *nextHop = rtoFreeNextHopList;
  if (nextHop == L7_NULLPTR)
  {
  ...
  if (rtoStats.nextHopCount > rtoStats.nextHopHighWater)
  {
    rtoStats.nextHopHighWater = rtoStats.nextHopCount;
  }
  rtoFreeNextHopList = rtoFreeNextHopList->nextNh;
  memset((char *) nextHop, 0, sizeof(rtoNextHop_t));
  return nextHop;
}

/*********************************************************************
* @purpose  Return a next hop structure to the free list.
*
* @param   nextHop @b{(input)}  next hop now free
*
* @returns  
*
* @notes
...
* @end
*********************************************************************/
void rtoNextHopFree(rtoNextHop_t *nextHop)
{
  nextHop->nextNh = rtoFreeNextHopList;
  rtoFreeNextHopList = nextHop;
  rtoStats.nextHopCount--;
}
...
L7_RC_t rtoRouterPreferenceApply(L7_uint32 index, L7_uint32 pref)
{
    ...
    rtoRouteInfo_t  *oldBestRouteInfo = L7_NULLPTR;
    ...
    if (pData == L7_NULLPTR)
    {
        ...
        if (osapiWriteLockGive(rtoRwLock) != L7_SUCCESS)
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_IP_MAP_COMPONENT_ID,
                    "Error: rto.c osapiWriteLockGive\n");
    ...
    while (pData != L7_NULLPTR)
    {
        ...
        while (routeInfo != L7_NULLPTR)
        {
            if (routeInfo->protocol == index)
            {
                ...
                /* Only identify current route as former best route if best */
                if (pData->nextRouteInfo->flags & RTO_BEST_ROUTE_NODE)
                  oldBestRouteInfo = pData->nextRouteInfo;
                else
                  oldBestRouteInfo = NULL;
                rtoSelectNewBestRoute(pData, oldBestRouteInfo, L7_FALSE);
...
L7_RC_t rtoRouteDelete (L7_routeEntry_t *routeEntry)
{
  ...
  if ((rtoTraceFlags & RTO_TRACE_ROUTE_DEL) &&
      ((rtoTracePfx == RTO_TRACE_PFX_NONE) || 
       (rtoTracePfx == (routeEntry->ipAddr & routeEntry->subnetMask))))
  {
      L7_uchar8 traceBuf[RTO_TRACE_LEN_MAX];
      L7_char8 destAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
      L7_char8 destMaskStr[OSAPI_INET_NTOA_BUF_SIZE];
  ...
  */
  ...
  /* If route is not found then update an error counter and return failure 
   * indication. */
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_IP_MAP_COMPONENT_ID, 
            "RTO is no longer full. Routing table contains %u best routes, "
...
L7_RC_t rtoRouteAdd (L7_routeEntry_t *routeEntry)
{
       ...
       (rtoTracePfx == (routeEntry->ipAddr & routeEntry->subnetMask))))
  {
      ...
      for (i = 0; i < routeEntry->ecmpRoutes.numOfRoutes; i++)
      {
          ...
          osapiSnprintf(nhStr, OSAPI_INET_NTOA_BUF_SIZE, " intf %d", 
  ...
  */
  ...
  while (routeInfo != L7_NULLPTR)
  {
    if (rtoRouteInfoMatch(routeInfo, routeEntry) == L7_TRUE)
    {
       if ((routeInfo->flags & RTO_TENTATIVE_ROUTE) &&
           ((routeEntry->flags & L7_RTF_CHECKPOINT) == 0))
       {
         /* checkpoint route confirmed by post-failover route */
         routeInfo->flags &= ~RTO_TENTATIVE_ROUTE;
         rtoStats.tentative_routes--;
         rtoStats.ckpt_conf++;
       }
       else
       {
  ...
  memset((char *)&allRouteEntry, 0, sizeof(L7_routeEntry_t));
  ...
  for (i = 0; i < numNextHops; i++)
  {
    ...
    if (intIfNum)
    {
      ...
      rtoRouteInfoNextHopAdd(routeInfo, intIfNum, 
  ...
  if (!isRejectRoute && (activeNextHops == 0))
  {
    L7_char8 destAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
    L7_uint32 maskLen;
  ...
  if (rtoIsFull() == L7_TRUE)
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_IP_MAP_COMPONENT_ID,
            ...
            " The routing table manager stores a limited number"
            " of best routes. The count of total routes includes alternate"
...
L7_RC_t rtoRouteModify (L7_routeEntry_t *routeEntry)
{
  ...
  rtoRouteInfo_t *routeInfo;
  rtoNextHop_t *nextHop;
  rtoRouteInfo_t *updRouteInfo = NULL;
       ...
       (rtoTracePfx == (routeEntry->ipAddr & routeEntry->subnetMask))))
  {
    #define RTOROUTEADD_TRACE_BUFF_LEN (50 + (OSAPI_INET_NTOA_BUF_SIZE + 2) * platRtrRouteMaxEqualCostEntriesGet())
      ...
      osapiInetNtoa(routeEntry->subnetMask, destMaskStr);
      osapiSnprintf(traceBuf, RTOROUTEADD_TRACE_BUFF_LEN, 
          "RTO modifying %s route to %s %s via ", 
              rtoProtoNames[routeEntry->protocol], destAddrStr, destMaskStr);
      for (i = 0; i < routeEntry->ecmpRoutes.numOfRoutes; i++)
      {
          osapiInetNtoa(routeEntry->ecmpRoutes.equalCostPath[i].arpEntry.ipAddr, nhStr);
  ...
  while ((routeInfo != L7_NULLPTR) && (route_found == L7_FALSE))
  {
      if (rtoRouteInfoMatch(routeInfo, routeEntry) == L7_TRUE) 
      {
        ...
        if ((routeInfo->flags & RTO_TENTATIVE_ROUTE) &&
            ((routeEntry->flags & L7_RTF_CHECKPOINT) == 0))
        {
          /* Was a checkpointed route. New route is not; so clear tentative flag. */
          routeInfo->flags &= ~RTO_TENTATIVE_ROUTE;
          rtoStats.tentative_routes--;
        }
      }
  ...
  */
  ...
  updRouteInfo = rtoNewRouteInfoGet();
  /* check each next hop in the changed route against installed next hops */
  for (i = 0; i < routeEntry->ecmpRoutes.numOfRoutes; i++)
  {
    nextHop = routeInfo->nextHops;
    while (nextHop)
    {
      if ((nextHop->nextHopIP == routeEntry->ecmpRoutes.equalCostPath[i].arpEntry.ipAddr) &&
          (nextHop->intIfNum == routeEntry->ecmpRoutes.equalCostPath[i].arpEntry.intIfNum))
      {
        break;
      }
      nextHop = nextHop->nextNh;
    ...
    if (nextHop)
    {
      /* Next hop in old and new routes. Retain it. */
      rtoRouteInfoNextHopAdd(updRouteInfo, nextHop->intIfNum, nextHop->nextHopIP);
      activeNextHops++;
    }
    else 
    {
      /* Did not find an exact match for this next hop. So add it as new. 
                            ...
                            &intIfNum) == L7_SUCCESS)
    {
      /* add to updated routeInfo */
        rtoRouteInfoNextHopAdd(updRouteInfo, intIfNum, 
                               routeEntry->ecmpRoutes.equalCostPath[i].arpEntry.ipAddr);    
  ...
  *
  *********************************************************************/
  memset((char *)&delRouteEntry, 0, sizeof(L7_routeEntry_t));
  delRouteEntry.ipAddr      = osapiNtohl(pData->network.addr);
  delRouteEntry.subnetMask  = osapiNtohl(pData->netmask.addr);
  delRouteEntry.protocol    = routeInfo->protocol;
  delRouteEntry.metric      = routeInfo->metric;
  ...
  while (nextHop)
  {
    ...
    {
      if ((routeEntry->ecmpRoutes.equalCostPath[j].arpEntry.ipAddr == nextHop->nextHopIP) &&
          (routeEntry->ecmpRoutes.equalCostPath[j].arpEntry.intIfNum == nextHop->intIfNum))
      {
        break;
      }
    }
   ...
   */
  if ((deletedNextHops > 0) && 
      ((activeNextHops > 0) || (routeEntry->flags & L7_RTF_REJECT)))
  {
    isChanged = L7_TRUE;
  }
  ...
  *********************************************************************/
  if(isChanged == L7_TRUE)
  {
     ...
     * because if there were any we would not need to do this (delete).
     */
    if((activeNextHops == 0) && !(routeEntry->flags & L7_RTF_REJECT))
    {
      memset((char *)&inactiveRouteEntry, 0, sizeof(L7_routeEntry_t));
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_IP_MAP_COMPONENT_ID,
                "Error: rto.c unable to delete route on modify\n");
      }
    ...
    rtoRouteInfoNextHopsReplace(routeInfo, updRouteInfo);
  ...
  /* return update route info to free list */
  rtoRouteInfoFree(updRouteInfo);
  if (osapiWriteLockGive(rtoRwLock) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_IP_MAP_COMPONENT_ID,
                           ...
                           L7_BOOL forceNotify)
{
  ...
  if ((rtoTraceFlags & RTO_TRACE_BEST) &&
      ((rtoTracePfx == RTO_TRACE_PFX_NONE) || 
       (rtoTracePfx == osapiNtohl(pData->network.addr))))
  {
    L7_uchar8 traceBuf[RTO_TRACE_LEN_MAX];
...
/*********************************************************************
* @purpose  Add a next hop to a route.
*
* @param    routeInfo    @b{(input)}  Route info object to which next hop is added
* @param    intIfNum     @b{(input)}  outgoing interface
* @param    nextHopAddr  @b{(input)}  next hop IPv4 address
* 
* @returns  void
*
* @notes    Next hops are sorted by next hop address, from lowest to highest. 
*
* @end
**********************************************************************/
void rtoRouteInfoNextHopAdd(rtoRouteInfo_t *routeInfo, L7_uint32 intIfNum,
                            L7_uint32 nextHopAddr)
{
  rtoNextHop_t *nextHop = rtoFreeNextHopGet();   /* guaranteed to succeed or LOG_ERROR */
  rtoNextHop_t *nh;                              /* existing next hop in route */

  nextHop->intIfNum = intIfNum;
  nextHop->nextHopIP = nextHopAddr;

  nh = routeInfo->nextHops;
  if ((nh == NULL) || (rtoCompareNextHops(nextHop, nh) == -1))
  {
    /* insert at head */
    nextHop->nextNh = routeInfo->nextHops;
    routeInfo->nextHops = nextHop;
  }
  else
  {
    while (nh->nextNh && (rtoCompareNextHops(nextHop, nh) == 1))
    {
      nh = nh->nextNh;
    }
    nextHop->nextNh = nh->nextNh;
    nh->nextNh = nextHop;
  }
...
/*********************************************************************
* @purpose  Delete a next hop from a route.
*
* @param    routeInfo  @b{(input)}  Route info object from which next hop is deleted
* @param    nextHop    @b{(input)}  next hop to be removed
* 
* @returns  L7_SUCCESS if next hop deleted
*           L7_ERROR if inputs invalid
*           L7_FAILURE if next hop not found
*
* @notes    Returns next hop to free list.
*
* @end
**********************************************************************/
L7_RC_t rtoRouteInfoNextHopDelete(rtoRouteInfo_t *routeInfo, 
                                  rtoNextHop_t *nextHop)
{
  rtoNextHop_t *nh = routeInfo->nextHops;

  if (nextHop == NULL)
    return L7_ERROR;

  /* remove nextHop from route's next hop list */
  if (nh == nextHop)
  {
    /* Deleting first in list */
    routeInfo->nextHops = nextHop->nextNh;
  }
  else
  {
    ...
    while (nh)
    {
      if (nh->nextNh == nextHop)
      {
        nh->nextNh = nh->nextNh->nextNh;
        break;
      }
      nh = nh->nextNh;
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,
            "Failed to find next hop to delete from route.");
    return L7_FAILURE;
  }

  routeInfo->numNextHops--;

  /* return next hop structure to free list */
  rtoNextHopFree(nextHop);
  return L7_SUCCESS;
}

/*********************************************************************
* @purpose  Replace the set of next hops in a route.
*
* @param    routeInfo     @b{(input)}  Route info object whose next hops are replaced.
* @param    updRouteInfo  @b{(input)}  Route info with updated next hops
* 
* @returns  void
*
* @notes    Logically a static function.
*           Next hops are sorted by next hop address. 
*
* @end
**********************************************************************/
void rtoRouteInfoNextHopsReplace(rtoRouteInfo_t *routeInfo, 
                                 rtoRouteInfo_t *updRouteInfo)
{
  ...
  /* In with the new */
  nextHop = updRouteInfo->nextHops;
  while (nextHop)
  {
    rtoRouteInfoNextHopAdd(routeInfo, nextHop->intIfNum, nextHop->nextHopIP);
    nextHop = nextHop->nextNh;
  }
}
...
*
* @notes  Sort numerically on the next hop IP address.
*
*********************************************************************/
                            ...
                            L7_routeEntry_t *routeEntry)
{
  ...
  if (routeInfo->numNextHops > platRtrRouteMaxEqualCostEntriesGet())
    {
  ...
  if (routeInfo->flags & RTO_REJECT_ROUTE)
    ...
    if (routeInfo->flags & RTO_TENTATIVE_ROUTE)
      routeEntry->flags |= L7_RTF_CHECKPOINT;
  ...
  while (nextHop)
        {
            routeEntry->ecmpRoutes.equalCostPath[index].flags = 0;
...
*
*           Note that deleted nodes can remain in the tree until best route
*           clients receive the delete notification. These count toward 
*           the rto full count. So in rare cases, it's possible RTO could 
*           be considered full with fewer than the max active routes. 
*
* @end
*********************************************************************/
...
L7_RC_t rtoBestRouteLookupExact(L7_uint32 ipAddr, L7_uint32 netMask, L7_uint32 *gateway)
{
  ...
  */
  ...
  if ((gateway != L7_NULLPTR) && 
      (pData->nextRouteInfo->nextHops))
  {
    *gateway = pData->nextRouteInfo->nextHops->nextHopIP;
  }
                                   ...
                                   void * passthru)
{
  ...
  */
  if ((pData == L7_NULLPTR) && (rtoNHResIsFull() == L7_TRUE))
  {
      ...
      osapiInetNtoa(ipAddr, destAddrStr);
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_IP_MAP_COMPONENT_ID,
              "Failed to add NHRes to %s to the routing table. \n", destAddrStr);
...
/*********************************************************************
* @purpose  Checks if a Default route learned via DHCP is already
*           added to the RTO/IP Stack and returns it, if found
*
* @param    defGwAddr  @b{(output)}  Default Gateway value
*
* @returns  L7_TRUE   If a default route exists
* @returns  L7_FALSE  If a default route doesn't exist
*
* @notes    None
*
* @end
**********************************************************************/
L7_BOOL
rtoIsDhcpDefGatewayPresent (L7_uint32 *defGwAddr)
{
  L7_routeEntry_t routeEntry;
  ...
  while (rtoNextRouteGet (&routeEntry, L7_FALSE) == L7_SUCCESS)
  {
    ...
    {
      if (routeEntry.pref == FD_RTR_RTO_PREF_DEFAULT_DHCP)
      {
        *defGwAddr = routeEntry.ecmpRoutes.equalCostPath[0].arpEntry.ipAddr;
        return L7_TRUE;
      }
    }
  }
...
/*********************************************************************
* @purpose  Checks if a Global Default Gateway route is added to the RTO
*           and returns it, if found
*
* @param    globalDefGwAddr  @b{(output)}  Global Default Gateway value
*                                          if found
* @returns  L7_TRUE   If route for Global Default Gateway exists
* @returns  L7_FALSE  If route for Global Default Gateway doesn't exist
*
* @notes    None
*
* @end
**********************************************************************/
L7_BOOL
rtoIsGlobalDefGatewayRoutePresent (L7_uint32 *globalDefGwAddr)
{
  L7_routeEntry_t routeEntry;
  L7_BOOL found = L7_FALSE;

  if (rtoPrefixFind (0, 0, &routeEntry) == L7_SUCCESS)
  {
    if (routeEntry.pref == ipMapRouterPreferenceGet(ROUTE_PREF_GLOBAL_DEFAULT_GATEWAY))
    {
      found = L7_TRUE;
      *globalDefGwAddr = routeEntry.ecmpRoutes.equalCostPath[0].arpEntry.ipAddr;
    }
  }
