...
/* Allow db size to be 3 * max routes plus a few miscellaneous router and 
   network LSAs */
#define OSPF_MAX_LSAS		((3 * platRtrRouteMaxEntriesGet()) + 200)
...
#define OSPF_MAX_ROUTES  (platRtrRouteMaxEntriesGet())
...
#define OSPF_MAX_RTB_UPDATE_MSECS 1000

/* Maximum number of neighbors simultaneously in Exchange or Loading 
 * state for a given area. */
#define OSPF_MAX_NBRS_EXCH_LOAD 5

/* Number of seconds in the stats interval */
#define O2_STATS_INTERVAL 5

/* Number of O2_STATS_INTERVALS intervals in 30 minutes (the LS Refresh time) */
#define O2_STATS_SIZE (LSRefreshTime / O2_STATS_INTERVAL)
...
#define HELLO_INTERVAL_DFLT            FD_OSPF_INTF_DEFAULT_HELLO_INTERVAL    /* sec */
#define ROUTER_DEAD_INTERVAL_DFLT      FD_OSPF_INTF_DEFAULT_DEAD_INTERVAL    /* sec */
#define POLL_INTERVAL_DFLT              120   /* sec */
#define INF_TRANS_DELAY_DFLT           FD_OSPF_INTF_DEFAULT_IF_TRANSIT_DELAY     /* sec */
#define RXMT_INTERVAL_DFLT             FD_OSPF_INTF_DEFAULT_RXMT_INTERVAL     /* sec */
#define DEF_AGING_INTERVAL                1     /* sec */
#define DEF_EXTERNAL_ROUTING_CAPABILITY   1
#define SILENCE_INTERVAL_DFLT          FD_OSPF_DEFAULT_SPF_DELAY   /* sec */
#define HOLDTIME_INTERVAL_DFLT         FD_OSPF_DEFAULT_SPF_HOLDTIME   /* sec */
#define RTO_UPDATE_BACKOFF                1   /* sec */
#define ITEMS_IN_TURN_DEF               100
#define DEFAULT_ROUTE_METRIC           FD_OSPF_INTF_DEFAULT_METRIC
#define DEF_OSPF_TICK_INTERVAL            1     /* sec */

#define DEFAULT_MAX_PATHS            FD_OSPF_MAXIMUM_PATHS   
#define DEFAULT_HELPER_SUPPORT       FD_OSPF_HELPER_SUPPORT
#define DEFAULT_STRICT_LSA_CHECKING  FD_OSPF_STRICT_LSA_CHECKING
#define DEFAULT_NSF_SUPPORT          FD_OSPF_NSF_SUPPORT
#define DEFAULT_RESTART_INTERVAL     FD_OSPF_RESTART_INTERVAL
...
/*        DEFVAL { 40}                                        */
   ...
   /* Whether OSPF is configured to be in stub router mode or to start up in 
    * stub router mode. If OSPF starts in stub router mode, how long it 
    * remains in stub router mode before advertising a normal router LSA. */
   OSPF_STUB_ROUTER_CFG_t StubRtrMode;
   L7_uint32 StubRtrStartupTime;

   /* TRUE if OSPF overrides the metric in T3 and T4 summary LSAs when in 
    * stub router mode. */
   Bool StubRtrSummaryMetricOveride;

   /* Metric to use in summary LSAs when in stub router mode and 
    * StubRtrSummaryMetricOveride is TRUE. */
...
/*        DEFVAL {FALSE}                                      */
   ...
   /* Whether OSPF is willing to act as a helpful neighbor to a neighbor
    * doing graceful restart. */
   OSPF_HELPER_SUPPORT_t HelperSupport;

   /* Whether OSPF terminates GR on a topology change */
   Bool StrictLsaChecking;

   /* Whether OSPF will do a graceful restart */
   OSPF_NSF_SUPPORT_t NsfSupport;

   /* Graceful restart interval. How long we ask helpful neighbors to
    * ignore our insanity. */
   ulng RestartInterval;
