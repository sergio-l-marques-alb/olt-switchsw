...
static char __fileid[] = "$Header: lvl7dev\src\application\protocol\ospf\mibospf\ospfmib.c 1.1.2.6 2002/09/11 14:47:12EDT anayar Exp  $";
#endif
...
e_Err ospfAreaTable_DeleteInternal(t_Handle ospfMibObj, ulng areaId)
{
   ...
   p_AreaAggr = OLL_GetNext(_ospfAreaAggrTable_, NULL);
   while (p_AreaAggr)
   {
      p_AggrNext = OLL_GetNext(_ospfAreaAggrTable_, p_AreaAggr);

      if (p_AreaAggr->ospfAreaAggrAreaId == areaId)
      {
...
ulng OSPFIF_BACKUPDESRTR(t_Handle p)
{
   ...
   if ((p_Entry ->info.rowStatus == active) && p_Entry->owner)
      if (IFO_Config(p_Entry->owner, &ifCfg) == E_OK)
         return (ulng)ifCfg.BackupIpAddr; 
...
{
   ospfNbrEntry *p_Entry = (ospfNbrEntry *)p;
   t_NBO *p_NBO = (t_NBO*) p_Entry->owner;

   if ((p_Entry->info.rowStatus == active) && p_Entry->owner)
   {
     return p_NBO->Options; 
   }
