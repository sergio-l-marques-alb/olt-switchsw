...
static char __fileid[] = "$Header: lvl7dev\src\application\protocol\ospf\spnbo.c 1.1.3.9 2003/02/18 12:45:13EST anayar Exp  $";
#endif
...
static void o2QueueDirectAck(t_NBO *p_NBO, t_S_LsaHeader *p_LsaHdr);
...
e_Err o2DatabaseEntryCreate(t_RTO *p_RTO, t_ARO *p_ARO, t_NBO *p_NBO, t_IFO *p_IFO, 
                            t_S_LsaHeader *p_LsaHdr, t_A_DbEntry *p_Former, 
                            t_A_DbEntry **p_DbEntry);
void o2SelfOrigLsaSeqNoIncr(t_RTO *p_RTO, t_A_DbEntry *p_DbEntry, 
                            t_ARO *p_ARO, t_NBO *p_NBO, L7_uint32 newSeqNo);
static e_Err o2SendDirectAcks(void *p_Info);
                ...
                t_IFO_Cfg *p_Cfg, t_Handle *p_Object)
{
   ...
   p_NBO->helperExitReason = OSPF_HELPER_EXIT_NONE;   
   ...
   /* grace interval timer */
   e |= TIMER_InitSec(1, p_NBO, &p_NBO->gracePeriodTimer);
   ...
   e |= HL_SortedInsert(p_IFO->NboHl, (byte*) &p_NBO->RouterId, p_NBO);
   
   /* Init Link State Acknowledgment list */
   p_NBO->DirectLsAckList = LL_New(0);  
   if (p_NBO->DirectLsAckList == NULL)
   {
     e = E_FAILED;
   }
...
/*********************************************************************
* @purpose  Determine whether topology changes are pending for a given neighbor.
*
* @param    p_NBO - neighbor
*
* @returns  E_OK
*
* @notes    A topology change is pending if the neighbor's retx list
*           includes any type 1 - 5, 7 LSAs that are not periodic refreshes. 
*
* @end
*********************************************************************/
L7_BOOL o2TopologyChangePending(t_NBO *p_NBO)
{
  e_Err e;
  t_RetxEntry *p_RetxEntry;
  t_A_DbEntry *dbEntry;

  e = HL_GetFirst(p_NBO->LsReTx, (void**) &p_RetxEntry);
  while (e == E_OK)
  {
    dbEntry = p_RetxEntry->DbEntry;
    if (((dbEntry->Lsa.LsType <= S_AS_EXTERNAL_LSA) || (dbEntry->Lsa.LsType = S_NSSA_LSA)) &&
        (!L7_BIT_ISSET(dbEntry->dbFlags, O2_DB_ENTRY_PERIODIC_UPDATE)))
    {
      return L7_TRUE;
    }

    e = HL_GetNext(p_NBO->LsReTx, (void**) &p_RetxEntry, (void*) p_RetxEntry);
  }
  return L7_FALSE;
}

/*********************************************************************
* @purpose  Determine whether a grace LSA is on the retx list for a given neighbor
*
* @param    p_NBO - neighbor
*
* @returns  E_OK
...
* @end
*********************************************************************/
L7_BOOL o2GraceLsaOnRetxList(t_NBO *p_NBO)
{
  e_Err e;
  t_RetxEntry *p_RetxEntry;
  t_A_DbEntry *dbEntry;
  ...
  {
    dbEntry = p_RetxEntry->DbEntry;
    if ((dbEntry->Lsa.LsType == S_LINK_OPAQUE_LSA) &&
        (dbEntry->Lsa.LsId[0] == OPAQUE_LSA_TYPE_GRACE))
    {
      return L7_TRUE;
    }
...
Bool NBO_Delete( t_Handle Id, ulng flag)
{
   ...
   /* Delete direct ACK list */
   o2ClearDirectAckList(p_NBO);
   LL_Delete(p_NBO->DirectLsAckList);
   ...
   TIMER_Delete(p_NBO->gracePeriodTimer);
  ...
  if (p_IFO->Cfg.MtuIgnore == FALSE)
   ...
   while(entries_num--)
   {
      if((p_LsaHdr->LsType < S_ROUTER_LSA) ||
         ...
         (p_LsaHdr->LsType == S_GROUP_MEMBER_LSA) ||
      ...
      {
        if ((p_LsaHdr->LsType == S_ROUTER_LSA) &&
            (A_GET_4B(p_LsaHdr->AdvertisingRouter) == p_RTO->Cfg.RouterId))
        {
          p_NBO->hasMyPrerestartRtrLsa = TRUE;
        }
      }
      
      /* Find corresponding database entry */
      er = AVLH_Find(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_LsaHdr->LsType), 
...
void LsReqReceived(t_NBO *p_NBO, t_S_PckHeader *p_Hdr)
{
   ...
   while(entries_num--)
   {
      ...
      if (o2LsaTypeValid(p_LsReq->LsType[3]) &&
          AVLH_Find(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_LsReq->LsType[3]), 
                      (byte*)&p_LsReq->LsId, (void *)&p_DbEntry,0) == E_OK)      
      {
         ...
         ASSERT(lsaLength);
         ASSERT(L7_BIT_ISSET(p_DbEntry->dbFlags, O2_DB_ENTRY_IS_CUR));
...
/*********************************************************************
 * @purpose   Create a new database entry.
 *
 * @param   p_RTO      @b{(input)}  OSPF instance
 * @param   p_ARO      @b{(input)}  area to which LSA belongs
 * @param   p_NBO      @b{(input)}  neighbor who sent LSA to us
 * @param   p_IFO      @b{(input)}  interface where neighbor is connected
 * @param   p_LsaHdr   @b{(input)}  received LSA
 * @param   p_Former   @b{(input)}  former instance
 * @param   p_DbEntry  @b{(output)} new database instance. If NULL on return, 
 *                                  caller should simply process the next 
 *                                  received LSA. 
 *
 * @returns E_OK if successful. 
 *          E_TOOMANY if LSDB is full
 *          E_DISCARD if db entry not installed and remaining received LSAs
 *                    should be discarded
 *
 * @notes
 *
                            ...
                            t_A_DbEntry **p_DbEntry)
{
  void  *p_F;
  void  *p_B;
  e_OSPF_RES_CHECK resCheck = RTO_LsaResourceCheck(p_RTO, p_Former ? TRUE : FALSE);
  ...
  *p_DbEntry = NULL;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
           "Unable to allocate buffer for received OSPF LSA");
    return E_DISCARD;
  }
  if ((p_F = F_NewEx(p_B,0,0,0)) == NULL)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
           "Unable to allocate frame for received OSPF update");
    B_DeleteEx(p_B, TRUE, NULL);  
    return E_DISCARD;
  }
...
/*********************************************************************
 * @purpose          Increment the sequence number of a received, self-originated
 *                   LSA before either reoriginating or flushing. 
 *
 * @param   p_RTO      @b{(input)}  OSPF instance
 * @param   p_DbEntry  @b{(input)}  database entry whose seqno is to be incremented
 * @param   p_ARO      @b{(input)}  area to which LSA belongs
 * @param   p_NBO      @b{(input)}  neighbor who sent LSA to us
 * @param   newSeqNo   @b{(input)}  sequence number of received LSA
 *
 * @returns    void
 *
 * @notes
                            ...
                            t_ARO *p_ARO, t_NBO *p_NBO, L7_uint32 newSeqNo)
{
  ...
  if (p_RTO->ospfTraceFlags & OSPF_TRACE_LSA_ORIG)
  {
    ...
    snprintf(traceBuf, OSPF_MAX_TRACE_STR_LEN,
             "Bumping sequence number of self-originated %s on area %s. LSID: %s, "
             "Adv Router: %s, Age: %d, Old SeqNo: %#lx, New SeqNo: %#x, csum: %#x",
             ...
             newSeqNo, A_GET_2B(p_DbEntry->Lsa.CheckSum));
    RTO_TraceWrite(traceBuf);
  }

  A_SET_4B(newSeqNo, p_DbEntry->Lsa.SeqNum);
   ...
   * than MinLSInterval. Since the seqno is not bumped until we originate, 
...
/*********************************************************************
 * @purpose          Processing of received Link State Update packet.
 *
 *
 * @param p_NBO      @b{(input)}  neighbor that sent the LS Update
 * @param p_Hdr      @b{(input)}  received LS update
 *
 * @returns          void
 *
 * @notes
...
void LsUpdateReceived(t_NBO *p_NBO, t_S_PckHeader *p_Hdr)
{
  ...
  while (lsanum-- && (((byte*)p_LsaHdr) < pktEnd))
  {
    ...
    if (FletcherCheckSum((byte *)p_LsaHdr + 2, A_GET_2B(p_LsaHdr->Length) - 2, 0) != 0)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_OSPF_MAP_COMPONENT_ID,
              "LSA checksum error. Dropping LSA with LSID %s checksum 0x%x.", 
    ...
    if (p_RTO->ospfTraceFlags & OSPF_TRACE_LS_UPDATE)
    {
      ...
      osapiInetNtoa(p_ARO->AreaId, areaIdStr);
      snprintf(traceBuf, OSPF_MAX_TRACE_STR_LEN,
               "OSPF %s received via area %s from neighbor at %s. "
    ...
    if (((e = AVLH_Find(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_LsaHdr->LsType), 
        ...
        (p_ARO->ExchOrLoadState == 0))
    {
      ...
      o2QueueDirectAck(p_NBO, p_LsaHdr);
      goto next_lsa;      
    }

    /* If instance of the received LSA found in DB, update LSA age */
    /* in the database entry before comparing                      */
    if (e == E_OK)
    {
      /*--------------------------------------------------------------------*/
      ...
      /*--------------------------------------------------------------------*/
      if (A_GET_4B(p_LsaHdr->AdvertisingRouter) == 
        ...
        ((compres = LsaCompare(p_LsaHdr, &p_DbEntry->Lsa)) == LSA_MORE_RECENT))
    {
      ...
      {
        p_IFO->MinLsArrivalDrops++;
        goto next_lsa;   
      }

      /* If doing GR, accept our pre-restart LSAs. Don't modify or flush them. 
       * RFC 3623 section 2. */
      if (selfOriginatedLsa && !o2GracefulRestartInProgress(p_RTO))
      {
        if ((p_DbEntry == NULL) || (A_GET_2B(p_DbEntry->Lsa.LsAge) == MaxAge))
        {
          ...
          if (p_DbEntry == NULL)
          {
            /* Have to install the LSA in order to flush it. */
            ec = o2DatabaseEntryCreate(p_RTO, p_ARO, p_NBO, p_IFO, p_LsaHdr,
                                       NULL, &newDbEntry);
            if (ec == E_TOOMANY)
            {
            ...
            else if (ec == E_DISCARD)
            {
              /* Don't ack, because we want the neighbor to retransmit. */
              if (p_RTO->BundleLsas)
              {
                LsUpdatesSend(p_RTO);
              }
            ...
            p_DbEntry = newDbEntry;
          }
          else
          {
            /* Received self-originated LSA is more recent than max age db copy. 
             * Need to bump the sequence number before flushing again. */
            o2SelfOrigLsaSeqNoIncr(p_RTO, p_DbEntry, p_ARO, p_NBO, 
                                   A_GET_4B(p_LsaHdr->SeqNum));
          }

          /* Don't check that AdvertisingRouter matches our current router
           * ID. We have already verified this LSA is self originated. */
          FlushLsaNoCheck(p_RTO, p_DbEntry, p_ARO);
          goto next_lsa;  
        }
        else
        {
      ...
      /* (c-d) Install this LSA in link state database, replacing former instance */

      /* Check if OSPF is in external LSA overflow state (RFC 1765). If so, don't 
       * accept a T5 LSA for which there is no former instance. */
      ...
      ec = o2DatabaseEntryCreate(p_RTO, p_ARO, p_NBO, p_IFO, p_LsaHdr, p_DbEntry, 
                                 &newDbEntry);
      if (ec == E_TOOMANY)
      {
        /* LSDB is full. ACK anyway so that neighbor doesn't send it again. 
      ...
      /* Mark pre-restart self-originated LSA. We can't get to this point for
       * a real self-originated LSA. */
      if (selfOriginatedLsa)
      {
        L7_BIT_SET(p_DbEntry->dbFlags, O2_DB_ENTRY_PRE_RESTART);
      }

      /* Only type of opaque LSA we interpret is a grace LSA (link opaque) */
      if (p_DbEntry->Lsa.LsType == S_LINK_OPAQUE_LSA)
      {
        OpaqueLsaProcess(p_NBO, p_DbEntry);
      }
    ...
    if (compres == LSA_EQUAL)
    {
      ...
      {
        /* Send delayed ACK if we are the BDR and LSA from DR */
        if ((p_IFO->BackupId == router_id) &&
            (p_NBO->RouterId == p_IFO->DrId))
        {
          SendDelayedAck(p_NBO, p_LsaHdr);
   ...
   while(entries_num--)
   {
      ...
      if(HL_FindFirst(p_NBO->LsReTx, &p_LsaHdr->LsType, (void **)&p_RetxEntry) == E_OK)
      {
            ...
            ((res = LsaCompare(p_LsaHdr, &p_RetxEntry->DbEntry->Lsa)) == LSA_EQUAL))
         {
            ...
            /* If we are in an unplanned graceful restart and neighbor has 
             * acknowledged our grace LSA, delete the temporary neighbor. */
            if (p_NBO->ckptNeighbor && 
                (p_RTO->restartStatus == OSPF_GR_UNPLANNED_RESTART) &&
                (p_LsaHdr->LsType == S_LINK_OPAQUE_LSA) && 
                (p_LsaHdr->LsId[0] == OPAQUE_LSA_TYPE_GRACE))
            {
              o2GrAckReceived(p_NBO);    /* deletes p_NBO */
              entries_num = 0;   /* force while loop exit so we don't use p_NBO again */
            }
         }
         else
         {
...
void DoNboTransition(t_NBO *p_NBO, e_NBO_Events Event, void *p_Prm)
{
  ...
  else
  {
    ...
    if (nbrState != PreviousState)
    {
      ...
      /* if the NBO changed its state to/from FULL */
      if ((nbrState == NBO_FULL) || (PreviousState == NBO_FULL))
      {
        ...
        /* If neighbor is pending exit from graceful restart, exit. */
        if (o2NeighborIsRestarting(p_NBO) && 
            (nbrState == NBO_FULL) && 
            p_NBO->helperExitPending)
        {
        ...
        if (o2GracefulRestartInProgress(p_RTO))
        {
          /* Report positive transtion. See if all pre-restart neighbors 
           * have been reacquired. */
          if ((nbrState == NBO_FULL) && (PreviousState != NBO_FULL))
          {
            o2GrNeighborFull(p_NBO);
            /* In GR, we won't originate a new router LSA. So transition to FULL
             * may not trigger new SPF. Need to trigger SPF to bring up virtual 
             * links. So trigger SPF here. */
            RTO_RecomputeAllRoutes(p_RTO);
          }
          else if ((nbrState != NBO_FULL) && (PreviousState == NBO_FULL))
          {
            /* Negative transition while still in GR. Consider this a topology
             * change and exit GR. */
            ...
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                    "Lost full adjacency with neighbor %s at %s while in graceful restart. "
...
static void Nbo3(t_NBO *p_NBO, void *p_Prm)
{
   ...
   {
      if ((LsaType == S_TMP2_LSA) || (LsaType == S_GROUP_MEMBER_LSA))
         continue;
      ...
      e = AVLH_GetFirst(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, LsaType), (void *)&p_DbEntry);
      while(e == E_OK)
      {
         if(!(
         ...
         e = AVLH_GetNext(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, LsaType), (void *)&p_DbEntry, p_DbEntry);
      }
   }
   ...
   {
      (void) HL_GetFirst(p_NBO->DbSum, &p_NBO->p_OutDdEntry);
   }
}
...
static void Nbo4(t_NBO *p_NBO, void *p_Prm)
{
  t_RTO *p_RTO = (t_RTO*) p_NBO->RTO_Id;
   ...
   if (o2GracefulRestartInProgress(p_RTO))
   {
     /* If neighbor did not announce this router's pre-restart router LSA
      * during db exchange, terminate graceful restart. */
     if (!p_NBO->hasMyPrerestartRtrLsa)
     {
       L7_uchar8 nbrIdStr[OSAPI_INET_NTOA_BUF_SIZE];
       L7_uchar8 nbrAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
       osapiInetNtoa(p_NBO->RouterId, nbrIdStr);
       osapiInetNtoa(p_NBO->IpAdr, nbrAddrStr);
       L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
               "Neighbor %s at %s did not announce this router's pre-restart "
               "router LSA during database exchange. Exiting graceful restart.",
               nbrIdStr, nbrAddrStr);
       o2GracefulRestartExit(p_RTO, OSPF_GR_TOPO_CHANGE);
     }
   }
...
/* We can only be in graceful restart helpful neighbor mode for this neighbor
 * if the event was NBO_INACT_TIMER. NBO_KILL terminates helper mode (and we 
 * don't use NBO_LL_DOWN). So if we are in helper mode, don't delete the
 * neighbor, but reset its state. */
static void Nbo8(t_NBO *p_NBO, void *p_Prm)
{
   ...
   if (p_IFO->Cfg.Type != IFO_NBMA)
   {
     if (o2NeighborIsRestarting(p_NBO))
     {
       /* reset neighbor state to init and do other cleanup */
       Nbo11(p_NBO, p_Prm);
     }
     else
     {
       if ((p_NBO->State == NBO_FULL) && p_RTO->Clbk.f_NeighborFullInd)
       {
...
static void Nbo9(t_NBO *p_NBO, void *p_Prm)
{
   ...
   if (!o2NeighborIsRestarting(p_NBO))
   {
       /* If the neighbor is restarting, don't reset its
        * earlier values of DR and BDR. */
       p_NBO->DrId = 0;
       p_NBO->BackupId = 0;
   }
...
/*********************************************************************
 * @purpose   Queue a direct acknowledgement for a received LSA.
 *
 * @param     p_NBO  @b{(input)}  neighbor that sent the LSA to be ACK'd
 * @param p_Hdr      @b{(input)}  LSA header pointer
 *
 * @returns   void
 *
 * @notes     RFC 2328 section 13.5 says,
 *
 ...
 * 
 *            Sending a single ACK in an LS ACK packet can lead to ACK 
 *            storms, particularly in networks with many parallel adjacencies
 *            between routers. So instead we put the ACK on the interface's 
 *            direct ACK queue (like a delayed ACK). But instead of waiting 
 *            until the ack timer fires, we queue an event to the OSPF thread 
 *            to send all direct ACKs for this neighbor. This design means that
 *            OSPF can process multiple packets and events before sending the
 *            direct ACKs. It also means the LS ACK packet can include ACKs
 *            for LSAs received in multiple LS Updates. 
 *
 * @end
 * ********************************************************************/
static void o2QueueDirectAck(t_NBO *p_NBO, t_S_LsaHeader *p_Hdr)
{
  t_AckElement *p_Ack;
  ...
  /* If this is the first element in the list, put an event on the 
   * OSPF event queue to send direct ACKs for this neighbor. */

  if ((LL_HowMany(p_NBO->DirectLsAckList) == 0))
  {
    t_RTO *p_RTO = (t_RTO*) p_NBO->RTO_Id;
    t_XXCallInfo *callData = 0;
    t_IFO *p_IFO = p_NBO->IFO_Id;

    /* Avoid queueing a pointer to a neighbor object, which could 
     * get deleted before OSPF processes this event. So instead of 
     * putting p_NBO in the message, put the interface index and 
     * neighbor's router ID. From these, we can find the p_NBO 
     * after we dequeue the msg. */
    PACKET_INIT_MQUEUE(callData, o2SendDirectAcks, 0, 0, 
                       OSPF_EVENT_QUEUE, 2, p_IFO->Cfg.IfIndex);
    ASSERT(callData);
    PACKET_PUT (callData, p_NBO->RouterId)
    if (XX_Call(p_RTO->OspfRtbThread.threadHndle, callData) != E_OK)
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
              "Failed to enqueue event to send direct acknowledgements.");
    }
    if (p_RTO->ospfTraceFlags & OSPF_TRACE_DIRACK)
    {
       char traceBuf[OSPF_MAX_TRACE_STR_LEN];
       L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
       L7_uchar8 nbrRouterIdStr[OSAPI_INET_NTOA_BUF_SIZE];
       RTO_InterfaceName(p_IFO, ifName);
       osapiInetNtoa((L7_uint32) p_NBO->RouterId, nbrRouterIdStr);
       sprintf(traceBuf, "Queued event to send direct ACK on interface %s to router %s.",
               ifName, nbrRouterIdStr);
       RTO_TraceWrite(traceBuf);
    }
  }

  /* Put into neighbor's direct ACK list */               
  LL_Put(p_NBO->DirectLsAckList, p_Ack);   
}

/*********************************************************************
 * @purpose   Delete all direct acks queued for a given neighbor.
 *
 * @param     p_NBO  @b{(input)}  neighbor whose ACKs are to be cleared
 *
 * @returns   void
 *
 * @notes     
 ...
 * @end
 * ********************************************************************/
void o2ClearDirectAckList(t_NBO *p_NBO)
{
  t_AckElement *p_Ack;
  while ((p_Ack = LL_Get(p_NBO->DirectLsAckList)) != NULL)
  {
    XX_Free(p_Ack);
  }
}
...
/*********************************************************************
 * @purpose   Send all direct acks queued for a given neighbor.
 *
 * @param     p_Info  @b{(input)}  message data
 *
 * @returns   void
 *
 * @notes     When we receive an LSA that requires a direct ACK, we put
 *            the LSA header on an ACK queue. Here is where we build
 *            LS ACK packets containing the ACKs. 
 *
 * @end
 * ********************************************************************/
static e_Err o2SendDirectAcks(void *p_Info)
{
   ...
   ulng ifIndex = PACKET_GET(callInfo, 0);
   ulng nbrRouterId = PACKET_GET(callInfo, 1);
   t_RTO *p_RTO = (t_RTO*) RTO_List;         /* assume single ospf instance */
   t_NBO *p_NBO;
   t_IFO *p_IFO;
   t_AckElement *p_Ack;
   void *p_F = NULL;
   ...
   /* Find neighbor from interface index and neighbor router ID. */
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_DIRACK)
   {
     ...
     RTO_InterfaceName(p_IFO, ifName);
     osapiInetNtoa((L7_uint32) p_NBO->RouterId, nbrRouterIdStr);
     sprintf(traceBuf, "Sending direct ACK on interface %s to router %s.",
             ifName, nbrRouterIdStr);
     RTO_TraceWrite(traceBuf);
   }
   ...
   /* If a partial packet is left, send it now. */
   if (p_F)
   {
      if (F_GetLength(p_F))
         IFO_SendPacket(p_IFO, p_F, S_LS_ACK, p_NBO, FALSE);
...
static void SendDelayedAck(t_NBO *p_NBO, t_S_LsaHeader *p_Hdr)
{
   ...
   while((p_El = LL_Read(p_IFO->LsAck)) != NULL)
   {
...
static void SendDd(t_NBO *p_NBO, Bool IsInit)
{
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_DD)
   {
       ...
       sprintf(traceBuf, "OSPF DD sent on interface %s to router %s. MTU: %d Options: %s Flags: %s SeqNo: %lu Length: %u ",
               ifName, destRouterIdStr, A_GET_2B(p_DbDscr->InterfaceMtu), optStr, flagStr, 
               A_GET_4B(p_DbDscr->SeqNum), (L7_uint32) (sizeof(t_S_PckHeader) + F_GetLength(p_F)));
       RTO_TraceWrite(traceBuf);
   }
   ...
   if(p_NBO->p_LastDd)
   {
     if (p_RTO->ospfTraceFlags & OSPF_TRACE_DD)
     {
         ...
         sprintf(traceBuf, "Retransmitting OSPF DD on interface %s to router %s."
                 " MTU: %d Options: %s Flags: %s SeqNo: %lu Length: %u ",
                 ifName, destRouterIdStr, A_GET_2B(p_DbDscr->InterfaceMtu), 
                 optStr, flagStr, A_GET_4B(p_DbDscr->SeqNum), 
                 (L7_uint32) (sizeof(t_S_PckHeader) + F_GetLength(p_NBO->p_LastDd)));
         RTO_TraceWrite(traceBuf);
     }
 ...
 * @end
 * ********************************************************************/
e_Err ospfDebugMinLsArrivalDrops(void)
{
  ...
  while(e == E_OK && p_ARO)
  {
    ...
    while(e == E_OK && p_IFO)
    {
      if (p_IFO->MinLsArrivalDrops)
      {
        L7_uchar8 ifName[L7_NIM_IFNAME_SIZE + 1];
        RTO_InterfaceName(p_IFO, ifName);
        printf("\nNum rx LSAs dropped on %s because of MinLSArrival: %lu.",
                ifName, p_IFO->MinLsArrivalDrops);
      }
...
* @returns  L7_FAILURE
*
* @notes    If we are in helpful neighbor mode for any neighbor cleared, we
*           exit from helper mode.
*
* @end
*********************************************************************/
      ...
      if (!nbrIfo || (nbrIfo == p_IFO))
      {
        ...
        while (en == E_OK)
        {
          ...
          if ((routerId == 0) || (routerId == p_NBO->RouterId))
          {
            if (o2NeighborIsRestarting(p_NBO))
            {
              o2HelperModeExit(p_NBO, OSPF_HELPER_MANUAL_CLEAR);
            } 
...
/*********************************************************************
* @purpose  Process a received opaque LSA. The only opaque LSA we process
*           is a grace LSA. 
*
* @param    p_NBO - neighbor that sent the opaque LSA
* @param    p_DbEntry - database entry for the opaque LSA
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
* @end
*********************************************************************/
e_Err OpaqueLsaProcess(t_NBO *p_NBO, t_A_DbEntry *p_DbEntry)
{
  if ((p_DbEntry->Lsa.LsId[0] == OPAQUE_LSA_TYPE_GRACE) && 
      (p_DbEntry->Lsa.LsId[1] == 0) &&
      (p_DbEntry->Lsa.LsId[2] == 0) &&
      (p_DbEntry->Lsa.LsId[3] == 0))
  {
    return o2GraceLsaProcess(p_NBO, p_DbEntry);
  }
  return E_OK;
}
