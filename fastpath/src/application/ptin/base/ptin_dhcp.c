...
inline L7_int dhcp_clientIndex_allocate(L7_uint8 dhcp_idx, ptinDhcpClientInfoData_t *infoData)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"No free clients available");
    return -1;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"No free clients available in queue");
    return -1;
  }
  ...
  if (rc != NOERR) {
    PT_LOG_ERR(LOG_CTX_DHCP, "There are no free clients available! rc=%d", rc);
    return -1;
  }

  client_idx = clientIdx_pool_entry->client_id;

  PT_LOG_DEBUG(LOG_CTX_DHCP, "Selected index=%u, Free clients pool: %u of %u entries",
  ...
  if (infoData != L7_NULLPTR)
  {
    ...
    if (rc != NOERR) {
      ...
      PT_LOG_ERR(LOG_CTX_DHCP, "Error adding element to queue! rc=%d", rc);
      return -1;
    }
  }
...
inline void dhcp_clientIndex_release(L7_uint8 dhcp_idx, L7_uint32 client_idx)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "Invalid DHCP instance %u, or client index %u", dhcp_idx, client_idx);
    return;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "There are no busy clients in queue!");
    return;
  }
  ...
  if (rc != NOERR) {
    PT_LOG_ERR(LOG_CTX_DHCP, "Error removing element from queue! rc=%d", rc);
    return;
  }
  ...
  if (rc != NOERR) {
    ...
    PT_LOG_ERR(LOG_CTX_DHCP, "Error adding client to free queue! rc=%d", rc);
    return;
  }
  ...
  PT_LOG_DEBUG(LOG_CTX_DHCP, "Free client pool: %u of %u entries",
...
L7_RC_t ptin_dhcp_init(void)
{
  ...
  if (dhcp_sem == NULL)
  {
    PT_LOG_TRACE(LOG_CTX_DHCP, "Error creating a mutex for DHCP module");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error allocating data for DHCP AVL Trees\n");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_dhcp_stats_sem semaphore!");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_enable(L7_BOOL enable)
{
  ...
  PT_LOG_TRACE(LOG_CTX_DHCP,"Success setting DHCP global enable to %u",enable);
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error configuring packet trapping for QUATTRO VLANs (enable=%u)", enable);
  ...
  PT_LOG_TRACE(LOG_CTX_DHCP,"Packet trapping for QUATTRO VLANs configured (enable=%u)", enable);
#endif
...
L7_RC_t ptin_dhcp_is_evc_used(L7_uint32 evc_idx)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid eEVC id: evc_idx=%u",evc_idx);
    return L7_FALSE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"EVC id is not active: evc_idx=%u",evc_idx);
    return L7_FALSE;
  }
...
L7_RC_t ptin_dhcp_instance_add(L7_uint32 evc_idx)
{
  ...
  if (evc_idx>=PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid eEVC id: ucEvcId=%u",evc_idx);
    return L7_FAILURE;
  }
  ...
  if (!ptin_evc_is_in_use(evc_idx))
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"eEVC id is not active: ucEvcId%u",evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error getting eEVC %u type and flags", evc_idx);
    return L7_FAILURE;
  }
  ...
  if (ptin_dhcp_instance_find(evc_idx, &dhcp_idx) != L7_SUCCESS)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"DHCP flag is not present for eEVC %u", evc_idx);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"There is no free instances to be used");
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Error setting DHCP instance to ext evc id %u", evc_idx);
      return L7_FAILURE;
    }
  ...
  {
    PT_LOG_WARN(LOG_CTX_DHCP,"There is already an instance with eEvcId%u", evc_idx);
  }
  ...
  #endif
  {
    ...
    if (rc != L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Error configuring DHCP snooping for dhcp_idx=%u",dhcp_idx);
...
L7_RC_t ptin_dhcp_instance_remove(L7_uint32 evc_idx)
{
  ...
  if (evc_idx>=PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid eEVC ids: ucEvcId=%u",evc_idx);
    return L7_FAILURE;
  }
  ...
  if (ptin_dhcp_instance_find(evc_idx, &dhcp_idx)!=L7_SUCCESS)
  {
    PT_LOG_WARN(LOG_CTX_DHCP,"There is no instance with ucEvcId=%u",evc_idx);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error removing all clients from dhcp_idx %u (evc_idx=%u)",dhcp_idx,evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error resetting DHCP instance to ext evc id %u", evc_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_evc_add(L7_uint32 evc_idx, L7_uint16 nni_ovlan)
{
  ...
  if (ptin_evc_check_evctype(evc_idx, &evc_type) != L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error getting eEVC %u type", evc_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_reconf_evc(L7_uint32 evc_idx, L7_uint8 dhcp_flag, L7_uint32 options)
{
   ...
   if (ptin_dhcp_instance_find(evc_idx, &dhcp_idx) != L7_SUCCESS)
   {
    PT_LOG_ERR(LOG_CTX_DHCP, "There is no DHCP instance with EVC id %u", evc_idx);
    return L7_NOT_EXIST;
   }
...
L7_RC_t ptin_dhcp_reconf_rootVid(L7_uint16 rootVid, L7_uint8 dhcp_flag, L7_uint32 options)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "There is no DHCP instance for root Vid %u", rootVid);
    return L7_NOT_EXIST;
  }
                                ...
                                L7_uint8 *rack, L7_uint8 *frame, L7_uint8 *ethernet_priority, L7_uint16 *s_vid)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "DHCP instance %u is not in use", dhcp_idx);
    return L7_FAILURE;
  }
                                       ...
                                       L7_uint8 rack, L7_uint8 frame, L7_uint8 ethernet_priority, L7_uint16 s_vid)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "There is no DHCP instance for NNI Vid %u", nni_outerVid);
    return L7_NOT_EXIST;
  }
                                                ...
                                                L7_uint8 rack, L7_uint8 frame, L7_uint8 ethernet_priority, L7_uint16 s_vid)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "Template string length is invalid!");
    return L7_FAILURE;
  }
  if ( strnlen(access_node_id, FD_DS_MAX_REMOTE_ID_STRING) >= FD_DS_MAX_REMOTE_ID_STRING )
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "Access node identifier length is invalid!");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_evc_flags_get(L7_uint32 evc_idx, L7_uchar8 *dhcp_mask, L7_char8 *dhcp_flags)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP, "Error getting DHCP flags for eEVC %u / dhcp_idx=%u", evc_idx, dhcp_idx);
    return L7_FAILURE;
  }
                             ...
                             ptin_clientCircuitId_t *circuitId_data, L7_char8 *circuitId, L7_char8 *remoteId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Non existent client in DHCP instance %u (EVC id %u)",dhcp_idx,evc_idx);
    return L7_FAILURE;
  }
  ...
  if (circuitId!=L7_NULLPTR)
  {
    ...
    PT_LOG_TRACE(LOG_CTX_DHCP, "%s", circuitId);
  }
  if (remoteId!=L7_NULLPTR)
  {
    ...
    PT_LOG_TRACE(LOG_CTX_DHCP, "%s", remoteId);
  }
                             ...
                             L7_uint16 options, ptin_clientCircuitId_t *circuitId, L7_char8 *remoteId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid client id");
    return L7_FAILURE;
  }
  ...
  if (client.mask & PTIN_CLIENT_MASK_FIELD_INTF)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Error acquiring intf configuration for intf %u/%u, evc=%u",client.ptin_intf.intf_type,client.ptin_intf.intf_id,evc_idx);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"intf %u/%u is not in use or is not a leaf in evc %u",client.ptin_intf.intf_type,client.ptin_intf.intf_id,evc_idx);
      return L7_FAILURE;
    }
       ...
       (client.mask & PTIN_CLIENT_MASK_FIELD_INNERVLAN) )
  {
    ...
    if (ptin_intf_ptintf2intIfNum(&client.ptin_intf, &intIfNum)==L7_SUCCESS)
    {
      ...
      {
        uni_ovid = uni_ivid = 0;
        PT_LOG_ERR(LOG_CTX_DHCP,"Cannot get ext vlans for ptin_intf %u/%u, cvlan %u",
                client.ptin_intf.intf_type, client.ptin_intf.intf_id, client.innerVlan);
      }
    }
    else
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Invalid ptin_intf %u/%u", client.ptin_intf.intf_type, client.ptin_intf.intf_id);
    }
  }
  ...
  if ((avl_infoData=(ptinDhcpClientInfoData_t *) avlSearchLVL7( &(avl_tree->dhcpClientsAvlTree), (void *)&avl_key, AVL_EXACT))!=L7_NULLPTR)
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_WARN(LOG_CTX_DHCP,"This key {"
                #if (DHCP_CLIENT_INTERF_SUPPORTED)
                                  "port=%u,"
                #endif
                #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  else
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"There is no more free clients to be allocated for dhcp_idx=%u (evc=%u)", dhcp_idx, evc_idx);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Error inserting key {"
              #if (DHCP_CLIENT_INTERF_SUPPORTED)
                                "port=%u,"
              #endif
              #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    if ((avl_infoData=(ptinDhcpClientInfoData_t *) avlSearchLVL7(&(avl_tree->dhcpClientsAvlTree),(void *)&avl_key, AVL_EXACT))==L7_NULLPTR)
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Cannot find key {"
              #if (DHCP_CLIENT_INTERF_SUPPORTED)
                                "port=%u,"
              #endif
              #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    if (client_idx < 0 || client_idx >= PTIN_SYSTEM_DHCP_MAXCLIENTS)
    {
      ...
      PT_LOG_ERR(LOG_CTX_DHCP,"Error obtaining new client index for dhcp_idx=%u (evc=%u)",dhcp_idx, evc_idx);
      return L7_FAILURE;
    }
  ...
  PT_LOG_TRACE(LOG_CTX_DHCP,"Success inserting Key {"
            #if (DHCP_CLIENT_INTERF_SUPPORTED)
                              "port=%u,"
            #endif
            #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
...
L7_RC_t ptin_dhcp_client_delete(L7_uint32 evc_idx, const ptin_client_id_t *client_id)
{
  ...
  if (ptin_dhcp_instance_find(evc_idx, &dhcp_idx)!=L7_SUCCESS)
  {
    PT_LOG_WARN(LOG_CTX_DHCP,"There is no DHCP instance with EVC id %u",evc_idx);
    return L7_NOT_EXIST;
  }
  ...
  #if (DHCP_CLIENT_DEBUG)
  PT_LOG_TRACE(LOG_CTX_DHCP,"Key to search {"
            #if (DHCP_CLIENT_INTERF_SUPPORTED)
                              "port=%u,"
            #endif
            #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error removing key {"
            #if (DHCP_CLIENT_INTERF_SUPPORTED)
                              "port=%u,"
            #endif
            #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  PT_LOG_TRACE(LOG_CTX_DHCP,"Success removing Key {"
            #if (DHCP_CLIENT_INTERF_SUPPORTED)
                              "port=%u,"
            #endif
            #if (DHCP_CLIENT_OUTERVLAN_SUPPORTED)
...
{
  PT_LOG_DEBUG(LOG_CTX_DHCP, "Removing all DHCP leases associated with external service ID %u", evc_ext_id);
...
L7_BOOL ptin_dhcp_vlan_validate(L7_uint16 intVlanId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid arguments: intVlan=%u",intVlanId);
    return L7_FALSE;
  }
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"No DHCP instance associated to intVlan %u",intVlanId);
    return L7_FALSE;
  }
...
L7_BOOL ptin_dhcp_intfVlan_validate(L7_uint32 intIfNum, L7_uint16 intVlanId /*, L7_uint16 innerVlanId*/)
{
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"No EVC id associated to intVlan %u", intVlanId);
    return L7_FALSE;
  }
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Inconsistency: eEVCid=%u (Vlan %u) not in use", evc_id_ext, intVlanId);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Error acquiring interface %u/%u configuarion from eEVC id %u",
...
L7_BOOL ptin_dhcp_intfTrusted_getList(L7_uint16 intVlanId, NIM_INTF_MASK_t *intfList)
{
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Error acquiring eEVC %u configuration", evc_id_ext);
    return L7_FALSE;
  }
                               ...
                               L7_int client_idx, L7_uint16 *uni_ovid, L7_uint16 *uni_ivid)
{
  ...
  if (ptin_debug_dhcp_snooping)
    PT_LOG_TRACE(LOG_CTX_DHCP, "ovid=%u ivid=%u", ovid, ivid); 
                                 ...
                                 ptin_client_id_t *client)
{
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Provided client_idx (%u) does not exist",client_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_flags_get(L7_uint16 intVlan, L7_uint8 *dhcp_flags, L7_uint32 *evc_flags)
{
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Internal vlan %u does not correspond to any DHCP instance",intVlan);
    return L7_FAILURE;
  }
  ...
  if (dhcp_flags != L7_NULLPTR)
  {
    ...
    {
      if (ptin_debug_dhcp_snooping)
        PT_LOG_ERR(LOG_CTX_DHCP,"Error acquiring DHCP flags for internal vlan %u / dhcp_idx=%u", intVlan, dhcp_idx);
      return L7_FAILURE;
    }
  }
  ...
  if (evc_flags != L7_NULLPTR)
  {
    ...
    {
      if (ptin_debug_dhcp_snooping)
        PT_LOG_ERR(LOG_CTX_DHCP,"Error acquiring EVC flags for internal vlan %u", intVlan);
      return L7_FAILURE;
    }
  }
                                ...
                                L7_char8 *circuitId, L7_char8 *remoteId)
{
...
#endif
  {
    ...
    {
      if (ptin_debug_dhcp_snooping)
        PT_LOG_ERR(LOG_CTX_DHCP,"Non existent client in DHCP instance %u (EVC id %u)",dhcp_idx,dhcpInstances[dhcp_idx].evc_idx);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP, "circuitId is NULL");
    }
    if (remoteId!=L7_NULLPTR)
    {
      strncpy(remoteId ,client_info->client_data.remoteId_str ,FD_DS_MAX_REMOTE_ID_STRING);
  ...
  else
  {
    ...
    {
      if (ptin_debug_dhcp_snooping)
        PT_LOG_ERR(LOG_CTX_DHCP,"Error getting default strings");
      return L7_FAILURE;
    }
    #else
    PT_LOG_ERR(LOG_CTX_DHCP,"No client defined!");
    return L7_FAILURE;
    #endif
  }
                                     ...
                                     L7_BOOL *isActiveOp37, L7_BOOL *isActiveOp18)
{
  ...
  if (L7_NULLPTR != isActiveOp18)
  {
...
#endif
   {
      ...
      {
         if (ptin_debug_dhcp_snooping)
            PT_LOG_ERR(LOG_CTX_DHCP, "Non existent client in DHCP instance %u (EVC id %u)",
...
static L7_RC_t ptin_dhcp_client_find(L7_uint dhcp_idx, ptin_client_id_t *client_ref, ptinDhcpClientInfoData_t **client_info)
{
  ...
  if (client_ref->mask & PTIN_CLIENT_MASK_FIELD_INTF)
  {
    ...
    {
      if (ptin_debug_dhcp_snooping)
        PT_LOG_ERR(LOG_CTX_DHCP,"Cannot convert client_ref intf %u/%u to ptin_port format",client_ref->ptin_intf.intf_type,client_ref->ptin_intf.intf_id);
      return L7_FAILURE;
    }
  }
  #endif
...
static L7_RC_t ptin_dhcp_instance_deleteAll_clients(L7_uint dhcp_idx)
{
  ...
  if (dhcp_idx>=PTIN_SYSTEM_N_DHCP_INSTANCES)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid dhcp instance index (%u)",dhcp_idx);
    return L7_FAILURE;
  }
  /* DHCP instance must be in use */
  if (!dhcpInstances[dhcp_idx].inUse)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"dhcp instance index %u is not in use",dhcp_idx);
    return L7_FAILURE;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_DHCP,"Success removing all clients from dhcp_idx=%u",dhcp_idx);
...
static L7_RC_t ptin_dhcp_inst_get_fromIntVlan(L7_uint16 intVlan, st_DhcpInstCfg_t **dhcpInst, L7_uint *dhcpInst_idx)
{
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"No EVC associated to internal vlan %u",intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_dhcp_snooping)
      PT_LOG_ERR(LOG_CTX_DHCP,"Inconsistency: DHCP index %u (EVCid=%u, Vlan %u) is not in use",dhcp_idx,evc_idx,intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_evc_trap_configure(L7_uint32 evc_idx, L7_BOOL enable, L7_uint8 family)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Can't get UC root vlan for evc id %u",evc_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp_reconf_instance(L7_uint32 dhcp_instance_idx, L7_uint8 dhcp_flag, L7_uint32 options)
{
   ...
   {
    PT_LOG_ERR(LOG_CTX_DHCP, "DHCP instance %u is not in use", dhcp_instance_idx);
    return L7_FAILURE;
   }
                        ...
                        L7_char8 *circuitId, L7_char8 *remoteId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error getting intIfNum or internal Vlan");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error reading circuitId+remoteId entry");
    return L7_FAILURE;
  }
                           ...
                           L7_char8 *circuitId, L7_char8 *remoteId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error adding circuitId+remoteId entry");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_dhcp82_unconfig(L7_uint32 evc_idx, ptin_intf_t *ptin_intf, L7_uint16 innerVlanId, L7_uint8 *macAddr)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error removing circuitId+remoteId entry");
    return L7_FAILURE;
  }
                                 ...
                                 L7_char8 *circuitId, L7_char8 *remoteId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"No valid input fields provided");
    return L7_FAILURE;
  }
  ...
  if (i>=DHCP_RELAY_DATABASE_MAX_ENTRIES)
  {
    ...
    PT_LOG_WARN(LOG_CTX_DHCP,"Not found entry with the provided inputs (intIfNum=%u, intVlanId=%u, innerVlanId=%u)",intIfNum,intVlanId,innerVlanId);
    return L7_FAILURE;
  }
...
static L7_RC_t ptin_dhcp82_database_remove(L7_uint32 intIfNum, L7_uint16 intVlanId, L7_uint16 innerVlanId, L7_uint8 *macAddr)
{
  ...
  if (i>=DHCP_RELAY_DATABASE_MAX_ENTRIES)
  {
    PT_LOG_WARN(LOG_CTX_DHCP,"Not found entry");
    return L7_SUCCESS;
  }
...
static L7_RC_t ptin_dhcp82_get_intfData(L7_uint32 evc_idx, ptin_intf_t *ptin_intf, L7_uint32 *intIfNum, L7_uint16 *intVlanId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Error getting interface configuration for evc_idx=%u, intf=%u/%u",evc_idx,ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  if (!intf_cfg.in_use)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Interface %u/%u is not in use in evc_idx=%u",ptin_intf->intf_type,ptin_intf->intf_id,evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Caanot calculate intIfNum (evc_idx=%u, intf=%u/%u)",evc_idx,ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"intVlan %u is not valid (evc_idx=%u, intf=%u/%u)",intf_cfg.int_vlan,evc_idx,ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  /* Get internal vlan */
...
static L7_RC_t ptin_dhcp_clientId_convert(L7_uint32 evc_idx, ptin_client_id_t *client)
{
  ...
  if (!ptin_evc_is_in_use(evc_idx))
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"eEVC id is not active: evc_idx=%u",evc_idx);
    return L7_FAILURE;
  }
  /* Validate client */
  if (client==L7_NULLPTR)
  {
    PT_LOG_ERR(LOG_CTX_DHCP,"Invalid client pointer");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_WARN(LOG_CTX_DHCP,"Client mask is null");
    return L7_FAILURE;
  }
  ...
  if (client->mask & PTIN_CLIENT_MASK_FIELD_INNERVLAN)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Invalid inner vlan (%u)",client->innerVlan);
      return L7_FAILURE;
    }
      ...
      client->mask & PTIN_CLIENT_MASK_FIELD_OUTERVLAN)
  {
    ...
    if (client->outerVlan<PTIN_VLAN_MIN || client->outerVlan>PTIN_VLAN_MAX)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_DHCP,"Error obtaining internal vlan for evc_idx=%u, ptin_intf=%u/%u",
    ...
    else
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_DHCP,"Error obtaining internal vlan for OVid=%u, IVid=%u, ptin_intf=%u/%u",
...
static L7_RC_t ptin_dhcp_clientId_restore(ptin_client_id_t *client)
{
      ...
      client->mask & PTIN_CLIENT_MASK_FIELD_OUTERVLAN)
  {
    ...
    if (ptin_intf_ptintf2intIfNum(&client->ptin_intf, &intIfNum)!=L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Cannot convert client intf %u/%u to intIfNum format",
              client->ptin_intf.intf_type,client->ptin_intf.intf_id);
      return L7_FAILURE;
    }
    ...
    if (client->outerVlan<PTIN_VLAN_MIN || client->outerVlan>PTIN_VLAN_MAX)
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Invalid outer vlan (%u)",client->outerVlan);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_DHCP,"Could not obtain external vlan for intVlan %u, ptin_intf %u/%u",
