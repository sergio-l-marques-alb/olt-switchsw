...
captivePortalCnfgrState_t captivePortalCnfgrState   = CAPTIVE_PORTAL_PHASE_INIT_0;
...
static L7_RC_t
captivePortalInitPhase3Func(L7_BOOL warmRestart, L7_CNFGR_CMD_DATA_t *,
                            L7_CNFGR_RESPONSE_t *, L7_CNFGR_ERR_RC_t *);
static L7_RC_t
captivePortalTerminate(L7_CNFGR_RESPONSE_t *pResp, L7_CNFGR_ERR_RC_t *pReason);
...
void  captivePortalApiCnfgrCommand(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  switch (command) 
  {
    case L7_CNFGR_CMD_INITIALIZE:
      ...
      switch (request) 
      {
        case L7_CNFGR_RQST_I_PHASE1_START:
		  ...
		  /* Enable default debug messages
		  */
          (void) captivePortalDebugReset();
		  captivePortalDebugLevelSet (CPD_LEVEL_DEFAULT);
    ...
    case L7_CNFGR_CMD_TERMINATE:
      cmd = "L7_CNFGR_CMD_TERMINATE";
      switch (request)
      {
        case L7_CNFGR_RQST_T_START:
          req = "L7_CNFGR_RQST_T_START";
          break;  
        default:
          break;
      }
...
#if 0
  CP_DLOG(CPD_LEVEL_LOG, "%s: cmd = %s, req = %s\n", __FUNCTION__, cmd, req);
...
void captivePortalCnfgrParse(void)
{
  ...
  L7_BOOL               warmRestart=L7_FALSE, phase3=L7_FALSE;
  ...
  do 
  {
    ...
    switch (command) 
    {
      ...
      case L7_CNFGR_CMD_TERMINATE:
        ...
        {
          case L7_CNFGR_RQST_T_START:
            rc = captivePortalTerminate(&response, &reason);
            break;
          default:
            break;
        }
    ...
    if ((L7_NULLPTR == configFunc) && (phase3 == L7_FALSE))
    {
      rc = L7_SUCCESS;
    }
    else
    {
      if (phase3 == L7_TRUE)
      {
        rc = captivePortalInitPhase3Func(warmRestart, pCmdData, &response, &reason);
        phase3 = L7_FALSE;
      }
      else
      {
...
/*********************************************************************
* @purpose  Process a terminate event from the configurator
*
* @param    L7_CNFGR_RESPONSE_t pResp   @b{(output)} command complete
* @param    L7_CNFGR_ERR_RC_t   pReason @b{(output)} always zero
*
* @returns  L7_SUCCESS
*
* @comments This event indicates failover to the backup is initiated.
*           Assuming a mgmt unit reboot, semaphore is never given.
*
* @end
*********************************************************************/
L7_RC_t captivePortalTerminate(L7_CNFGR_RESPONSE_t *pResp, L7_CNFGR_ERR_RC_t *pReason)
{
  (void)osapiSemaTake(cpdmOprData->cpCkptPendingSema, 20000);
...
*
*********************************************************************/
L7_RC_t captivePortalInitPhase3Func(L7_BOOL warmRestart, L7_CNFGR_CMD_DATA_t * pCmd, 
                                    L7_CNFGR_RESPONSE_t * pResp, 
                                    L7_CNFGR_ERR_RC_t * pReason)
{
  L7_RC_t rc;

  cpdmOprData->cpWarmRestart = warmRestart;

  /* clear NIM port event mask until ready to receive events */
  memset(&cpdmOprData->nimEventMask, 0, sizeof(cpdmOprData->nimEventMask));
...
L7_RC_t cpCopyImageFsToData(L7_uchar8 *images)
{
  ...
  if ((dir = opendir(RAM_CP_PATH RAM_CP_NAME)) == NULL)
  {
    CP_DLOG(CPD_LEVEL_LOG, "%s: Failed to open directory:%s.",__FUNCTION__,filePath);
    SEMA_GIVE(WRITE);
    return L7_FAILURE;
  }
  ...
  while ((entry = readdir(dir)) != NULL)
  {
	    ...
	    (idx+sizeof(L7_uint32)+fileNameLen+sizeof(L7_uint32)+fileSize))
	{
	  ...
	  if (osapiFsFileSizeGet(fqf,&fileSize) != L7_SUCCESS)
	  {
            closedir(dir);
            SEMA_GIVE(WRITE);
            CP_DLOG(CPD_LEVEL_LOG, "%s: Unable to read file size for:%s.",__FUNCTION__,fileName);
            return L7_FAILURE;
	  }
	  ...
	  if (L7_SUCCESS != osapiFsRead(fqf,&images[idx+=4],fileSize))
	  {
            closedir(dir);
            SEMA_GIVE(WRITE);
            CP_DLOG(CPD_LEVEL_LOG, "%s: Unable to read file:%s.",__FUNCTION__,fileName);
            return L7_FAILURE;
	  }
