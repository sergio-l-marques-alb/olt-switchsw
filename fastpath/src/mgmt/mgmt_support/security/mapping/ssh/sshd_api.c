...
#endif
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_FLEX_SSHD_COMPONENT_ID, "Failed to find SSHD key files");
      osapiSemaGive(sshdGlobalSema);
      return L7_FAILURE;
    }
    if (sshdIssueCmd(SSHD_ADMIN_MODE_SET, &mode) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_FLEX_SSHD_COMPONENT_ID, "Failed to set admin mode %d", mode);
      osapiSemaGive(sshdGlobalSema);
      return L7_FAILURE;
    }
...
/*********************************************************************
*
* @purpose Validate SSH keys when downloaded
*
* @param L7_FILE_TYPES_t     type  @b{(input)} Key file type.
*
* @returns L7_SUCCESS key is valid
* @returns L7_FAILURE key is not valid
*
* @comments
*
* @end
...
L7_RC_t sshdKeyValidate(L7_FILE_TYPES_t type)
{
    Key         *tempKey = NULL;
    ...
    switch (type)
    {
        case L7_FILE_TYPE_SSHKEY_RSA1:
            if (sshdKeyRSA1Get(&tempKey) != L7_SUCCESS)
            {
                return L7_FAILURE;
            }
            break;
        ...
        case L7_FILE_TYPE_SSHKEY_RSA2:
            if (sshdPubKeyGet(SSHD_KEY_TYPE_RSA, &tempKey) != L7_SUCCESS)
            {
                return L7_FAILURE;
            }
            break;
        ...
        case L7_FILE_TYPE_SSHKEY_DSA:
            if (sshdPubKeyGet(SSHD_KEY_TYPE_DSA, &tempKey) != L7_SUCCESS)
            {
                return L7_FAILURE;
            }
            break;
        ...
        default:
            /*
            Invalid file type; only key files valid
            */
            return L7_FAILURE;
            break;
    }
...
/*********************************************************************
*
* @purpose Delete SSH keys by file type
*
* @param L7_FILE_TYPES_t     type  @b{(input)} Key file type.
*
* @returns void
...
void sshdKeyDelete(L7_FILE_TYPES_t type)
{
    switch (type)
    {
        case L7_FILE_TYPE_SSHKEY_RSA1:
            (void)osapiFsDeleteFile(L7_SSHD_PATH_SERVER_PRIVKEY);
            break;

        case L7_FILE_TYPE_SSHKEY_RSA2:
            (void)osapiFsDeleteFile(L7_SSHD_PATH_SERVER_PRIVKEY_RSA);
            break;

        case L7_FILE_TYPE_SSHKEY_DSA:
            (void)osapiFsDeleteFile(L7_SSHD_PATH_SERVER_PRIVKEY_DSA);
            break;
