...
601715975189520189103sldkjf0932urnds0923072680137298017313444030079107190kkj3fde381257";
  ...
  if (randomFile == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
            "OPENSSL: random file is null, exiting\n");
    return;
  }
  ...
  if (RAND_load_file(randomFile, -1) == 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
              "OPENSSL: Error loading random file, %s\n", randomFile);
    }
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
            "OPENSSL: Error creating NEW random file %s\n", randomFile);
  }
...
void opensslSecureBlockingSet(SSL *ssl)
{
  ...
  if ((ssl_rfd = SSL_get_rfd(ssl)) > 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
              "OPENSSL: failed to set read socket %d to blocking\n", ssl_rfd);
    }
  }
  ...
  if ((ssl_wfd = SSL_get_wfd(ssl)) > 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
              "OPENSSL: failed to set write socket %d to blocking\n", ssl_wfd);
    }
  }
...
void opensslSecureNonBlockingSet(SSL *ssl)
{
  ...
  if ((ssl_rfd = SSL_get_rfd(ssl)) > 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
              "OPENSSL: failed to set read socket %d to non-blocking\n", ssl_rfd);
    }
  }
  ...
  if ((ssl_wfd = SSL_get_wfd(ssl)) > 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
              "OPENSSL: failed to set write socket %d to non-blocking\n", ssl_wfd);
    }
  }
...
void opensslPemPassPhrasePrompt(void)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
          "OPENSSL: The Server SSL PEM file should not be pass phrase protected\n");
...
opensslServerKeyRead(L7_char8 *keyFile, EVP_PKEY **pkey)
{
  ...
  *pkey = PEM_read_PrivateKey(fp, L7_NULLPTR, ((void *)opensslPemPassPhrasePrompt), L7_NULLPTR);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
            "OPENSSL_SSL: Error loading private key from keyFile\n");
    return L7_FAILURE;
  }
...
opensslServerCertWrite(L7_char8 *certFile, X509 *cert)
{
  ...
  if (osapiFsOpen(certFile, &fd) == L7_ERROR)
  {
    return L7_FAILURE;
  }
...
opensslDiffieHellmanParamsWrite(L7_char8 *file, DH *dh)
{
    ...
    {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                "OPENSSL: Could not get file pointer for %s\n", file);
        osapiFsClose(fd);
        return L7_FAILURE;
    }
    ...
    {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                "OPENSSL: PEM DH params could not be written\n");
        fclose(fp);
        return L7_FAILURE;
    }
...
void opensslDiffieHellmanParamsCreate(void **semaP, DH **dh512P, DH **dh1024P)
{
    ...
    */
    ...
    do
    {
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                    "DH Weak generate bogus\n");
            break;
        }

        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                "OPENSSL: Diffie Hellman weak parameters created.\n");
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                    "DH Strong generate bogus\n");
            break;
        }

        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
                "OPENSSL: Diffie Hellman strong parameters created.\n");
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_OPENSSL_COMPONENT_ID,
            "OPENSSL: Failed to create Diffie Hellman parameters.\n");
