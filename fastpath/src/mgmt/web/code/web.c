...
L7_BOOL usmWebComponentPresent(L7_char8 * compName)
{
 ...
 /*
  ...
  else if (strcmp(compName, pStrInfo_base_Dot1agComponent) == 0)
  {
    componentId =   L7_DOT1AG_COMPONENT_ID;
    runCheck = L7_TRUE;
  }
  ...
  else if (strcmp(compName, pStrInfo_base_pfc) == 0)
  {
    componentId =  L7_PFC_COMPONENT_ID;
    runCheck = L7_TRUE;
  }
  else if (strcmp(compName, pStrInfo_base_llpf) == 0)
  {
    componentId =  L7_LLPF_COMPONENT_ID;
    runCheck = L7_TRUE;
  }
...
L7_BOOL usmWebFeaturePresent(L7_char8 * featureDescription)
{
  ...
  else if (strcmp(featureDescription, pStrInfo_base_AclTimeRangeFeat) == 0)
  {
    componentId = L7_FLEX_QOS_ACL_COMPONENT_ID;
    featureId = L7_ACL_PER_RULE_ACTIVATION_ID;
    runCheck = L7_TRUE;
  }
  ...
  else if (strcmp(featureDescription, pStrInfo_base_Dot1adIntfProtocolTunnelFeat) == 0)
  {
    componentId = L7_FLEX_METRO_DOT1AD_COMPONENT_ID;
    featureId = L7_FLEX_METRO_DOT1AD_INTF_BASED_PROTOCOL_TUNNEL_FEATURE_ID;
    runCheck = L7_TRUE;
  }
  else if (strcmp(featureDescription, pStrInfo_base_Dot1adVlanProtocolTunnelFeat) == 0)
  {
    componentId = L7_FLEX_METRO_DOT1AD_COMPONENT_ID;
    featureId = L7_FLEX_METRO_DOT1AD_VLAN_BASED_PROTOCOL_TUNNEL_FEATURE_ID;
    runCheck = L7_TRUE;
  }
  ...
  else if ( strcmp(featureDescription,pStrInfo_base_Nsf) == 0 )
  {
    componentId = L7_FLEX_STACKING_COMPONENT_ID;
    featureId= L7_STACKING_NSF_FEATURE_ID;
    runCheck = L7_TRUE;
  }
	else if (strcmp(featureDescription,pStrInfo_base_Sfs_feature_id) == 0 )
	{
    componentId = L7_FLEX_STACKING_COMPONENT_ID;
    featureId= L7_STACKING_SFS_FEATURE_ID;
    runCheck = L7_TRUE;
	}
...
L7_char8 *usmWebPortDisplayHelp(L7_uint32 intIfNum)
{
  ...
  {
    osapiStrncpy(buf, strUtilUspGet (unit, u, s, p, L7_NULLPTR, 0),sizeof(buf));
  }
    else
  {
...
/*********************************************************************
* @purpose  Convert incoming Ascii code stream into Unicode.
*
* @param    L7_char8 *in  @b{(input)}  String of virgin ascii chars
* @param    L7_char8 *out @b{(output)} Converted unicode string
*
* @returns  L7_RC_t
*
* @notes    Caller should make sure that sizeof(out) >= 6 * sizeof(in)
*
* @end
*********************************************************************/
L7_RC_t usmWebConvertAsciiToUnicode(L7_char8 *in, L7_char8 *out, L7_uint32 inLen, L7_uint32 outLen)
{
  ...
  while (in[idx] != '\0')
  {
		osapiSnprintf(buf, sizeof(buf), pStrInfo_common_Ucode, in[idx++]);
		osapiStrncat(out, buf, strlen(buf));
  }
  return L7_SUCCESS;
}
...
/*********************************************************************
*
* @purpose To get the descrition for Submit button used in help pages
*
*
* @returns L7_char8 help description for submit button 
*
* @end
*
*********************************************************************/
...
{
  static L7_char8 buf[150];
  osapiSnprintf(buf, sizeof(buf), "%s",pStrInfo_submit_description_help);
  return buf;
}

/*********************************************************************
*
* @purpose To get the descrition for refresh button used in help pages
*
*
* @returns L7_char8 help description for refresh button
*
* @end
*
*********************************************************************/
...
{
  static L7_char8 buf[80];
  osapiSnprintf(buf, sizeof(buf), "%s",pStrInfo_refresh_description_help);
  return buf;
}

/*********************************************************************
*
* @purpose To get the descrition for cancel button used in help pages
*
*
* @returns L7_char8 help description for cancel button
*
* @end
*
*********************************************************************/
...
{
  static L7_char8 buf[50];
  osapiSnprintf(buf, sizeof(buf), "%s",pStrInfo_cancel_description_help);
  return buf;
}
