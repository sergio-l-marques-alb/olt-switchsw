...
function isSelected ( tabInfo )
{
   ...
   {
     if( (b_version.search(/MSIE 8.0/)) != -1 )
     {
       check = ((tabInfo[1][0] != undefined) && (tabInfo[1][0][1] != undefined));
     }
     else
   ...
   if ( typeof(glbCheckVariable) != "undefined" )
   {
      if ( glbCheckVariable != "" )
      {
        if( cmpVar[0] == truncate(glbCheckVariable,9))
        {
          return true;
        }
        else
   ...
   {
     /*Start - Fix for Issue#LVL700108147*/
     if( (pathInfo[i][0] == "Valid AP Configuration") && (cmpVar[0] == "Valid AP") )
     {
       return true;
     }
     /*End - Fix for Issue#LVL700108147*/
...
function loadme()
{
  ...
  for(var i=0; i < nPath.length; i++)
  {
     ...
     blankTD1.setAttribute('noWrap','true');
     ...
     if ( (isSelected(nPath[i])) && (check) ) 
     {
       ...
       for(var j=1; j < nPath[i].length; j++)
       {
         ...
         innerblankTD3.setAttribute('noWrap','true');
   ...
   blankTD2.setAttribute('noWrap','true');
   ...
   blankTD3.setAttribute('noWrap','true');
