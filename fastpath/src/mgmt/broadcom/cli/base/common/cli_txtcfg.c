...
{
  L7_char8 one = 1;
  if(0 > write (cliTxtCfgWap->emwebPipe, &one, 1)){}
}
...
static void cliTxtCfgCardConfigCallback (L7_uint32 event)
{
  ...
  if (cliGlobalConfigurationSkipGet () == L7_TRUE)
  {
    ...
    /* If not pushing the text config, still need to tell everyone that
     * that registered that text config apply is done. */
    if (event == L7_CMGR_CONFIG_EVENT_CONFIG_SLOTS)
    {
    ...
    /* Reset the global config skip flag */
    if (event == L7_CMGR_CONFIG_EVENT_CONFIG_PORTS)
    {
      cliGlobalConfigurationSkipSet(L7_FALSE);
    }
  ...
  else if (event == L7_CMGR_CONFIG_EVENT_CONFIG_PORTS)
  {
    cliTxtCfgWap->intfCfgDone = L7_FALSE;
  }

  /* wake up emweb */
  OSAPI_TRACE_EVENT(L7_TRACE_EVENT_EMWEB_CONFIG_APPLY_START,
                    &event, sizeof(event));
  cliTxtCfgWakeEmweb ();

  /* Wait for emweb to apply the config */
  osapiSemaTake (cliTxtCfgWap->syncSema, L7_WAIT_FOREVER);
  OSAPI_TRACE_EVENT(L7_TRACE_EVENT_EMWEB_CONFIG_APPLY_END,
                    &event, sizeof(event));
}
                                   ...
                                   L7_BOOL useComp)
{
  ...
  for (cp = cmdString, cmdp = pCfgScr->cfgData; cmdp; cmdp = cmdp->next)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
              "Failed to write command block\n");
    }
    scrLenComp += length;
  }
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
          "Text Configuration of length %d (%d CMDS) compressed to save %d%% Flash\n",
...
L7_RC_t cliTxtCfgGenerateFile (L7_char8 * filename, L7_BOOL useComp)
{
  ...
  context->showRunningSaveFileFlag = L7_SAVE;
  ...
  /* Don't write this message when the running config is saved */
  if (strcmp(filename, sysapiTxtRunCfgFileGet()) != 0)
  {
...
L7_int32 cliTxtCfgPoll (L7_int32 kick)
{
  ...
  /* Wait for configurator to be in execute state. We want to guarantee that all
   * components are in EXECUTE state when the configuration is pushed. */
  cnfgrApiSystemStateGet (&systemState);
  if (L7_CNFGR_STATE_E != systemState)
  {
    /* poke emweb again so we don't have to wait for 10-second select timeout
     * to try again. */
    cliTxtCfgWakeEmweb();
    return cliTxtCfgWap->emwebPipe;
  }
                             ...
                             L7_uint32 (*notifyFunction)(L7_uint32 event))
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
            "txtConfig  registrar_ID %d greater then the LAST_COMPONENT_ID\n",(L7_int32)registrar_ID);
    return(L7_FAILURE);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
            "txtConfig Register:  registrar_ID %d, %s already registered\n", (L7_int32)registrar_ID, name);
    return(L7_FAILURE);
  }
...
L7_RC_t txtCfgApplyCompletionNotifyDeregister(L7_uint32 registrar_ID)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
            "txtConfig unregister:  registrar_ID %d greater then the LAST_COMPONENT_ID\n",(L7_int32)registrar_ID);
    return(L7_FAILURE);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_CLI_WEB_COMPONENT_ID,
            "txtConfig unregister: registrar_ID %d, %s already unregistered\n",(L7_int32)registrar_ID, name);
    return(L7_FAILURE);
  }
...
/*********************************************************************
*
* @purpose  Validates and applies a downloaded IAS user database file
*
*
...
L7_RC_t IASUsersScriptValidateAndApply(L7_char8 * scriptFileName, L7_char8 * newFilename)
{
  ...
  if(L7_SUCCESS == (rc = readDownloadedConfigScript(scriptFileName, newFilename, &configScriptData)))
  {
    rc = configScriptValidate(context, &configScriptData);

    if (rc == L7_SUCCESS)
    {
      /* Clear IAS user database */
      for (userCount = 0; userCount < L7_MAX_IAS_USERS; userCount++)
      {
        usmDbUserMgrIASUserDBUserDelete(userCount);
      }
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_AUTH_SERV_COMPONENT_ID,
              "Internal users database file validation failed.\n");
    }
    /*release buffer*/
    releaseScriptBuffer(&configScriptData) ;
  }
