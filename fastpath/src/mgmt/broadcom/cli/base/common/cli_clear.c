...
/*********************************************************************
*
* @purpose  clear the  email alert stats
*
*
* @param EwsContext ewsContext
* @param L7_uint32 argc
...
*
*********************************************************************/
const L7_char8 *commandClearEmailAlertStats(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  L7_uint32  numArgs;
  ...
  if ( numArgs != 2 )
  {
    return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput,  ewsContext, pStrErr_base_ClrEmailStatistics_1);
  }


  usmDbLogEmailAlertStatsClear(USMDB_UNIT_CURRENT);
  ...
  if ( cliGetCharInputID() != CLI_INPUT_EMPTY || ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING)
  {
    if ( tolower(cliGetCharInput()) == 'y' || ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING)
    {
      ...
      if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
      {
        ...
        rc = usmDbDynamicVlanEntriesGet(unit, &dynamicVlanCount);
      
        /* This command deltes only static VLANs created. So After deletion the remaining VLANs
         * are equal total - dynamicVlanCount.
         */  
        while((usmDbDot1qNumVlansGet(unit, &val) == L7_SUCCESS) && ((val-dynamicVlanCount) != 1))
        {
    ...
    if ( tolower(cliGetCharInput()) == 'y' || ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING)
    {
      ...
      if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
      {
                                     ...
                                     L7_STACKING_SFS_FEATURE_ID) == L7_TRUE)
        {
          ...
          {
		        return cliSyntaxReturnPrompt (ewsContext, pStrErr_stack_autocopyinProgress);
          }
        }
  ...
  else if(numArgs == 0)
  {
    ...
    else if ( cliGetCharInputID() == 1 || ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING)
    {
      if ( tolower(cliGetCharInput()) == 'y' || ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING)
      {
        ...
        if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
        {
          if (usmDbMgtSwitchintIfNumGet(unit, &interface) == L7_SUCCESS)
          { 
            usmDbIntfStatReset(unit, interface);
...
const L7_char8 *commandClearHostname(EwsContext ewsContext, L7_uint32 argc, const L7_char8 **argv, L7_uint32 index)
{
  ...
  if (usmDbHostNameValidateWithSpace((L7_uchar8 *)argv[index + 1]) != L7_SUCCESS)
  {
                                      ...
                                      const L7_char8 **argv, L7_uint32 index)
{
  ...
  {
    return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput,  ewsContext, pStrErr_common_ClrArpSwitch);
    ;
  }
  unit = cliGetUnitId();
  ...
  /*******Check if the Flag is Set for Execution*************/
  if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  if (usmDbArpSwitchClear(unit) != L7_SUCCESS)
  {
...
/*********************************************************************
*
* @purpose  To clear the last detected ip address conflict status
*
* @param EwsContext ewsContext
* @param L7_uint32 argc
* @param const L7_char8 **argv
...
* @notes none
*
* @cmdsyntax for normal command:  clear ip address-conflict-detect
*
* @cmdhelp
*
* @cmddescript
...
*
*********************************************************************/
const L7_char8 *commandClearIpAddrConflict(EwsContext ewsContext, L7_uint32 argc,
                                           const L7_char8 **argv, L7_uint32 index)
{
  /*************Set Flag for Script Failed******/
  ewsContext->actionFunctionStatusFlag=L7_ACTION_FUNC_FAILED;
  ...
  {
    return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput,
                                           ewsContext, pStrErr_base_ClearIpAddrConflict);
  }

  /*******Check if the Flag is Set for Execution*************/
  if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    if (usmDbIPAddrConflictDetectStatusClear() != L7_SUCCESS)
    {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext,
                                               pStrErr_base_ClrIpAddrConflict);
    }
  }
...
/*********************************************************************
*
* @purpose  clear dot1x user database entries
*
*
* @param EwsContext ewsContext
* @param L7_uint32 argc
...
* @notes none
*
* @cmdsyntax [Privileged EXEC]> clear dot1x-users
*
* @cmdhelp Clear all IAS user entries.
*
* @cmddescript
*   This command causes the all dot1x user entries to be cleared.
*
* @end
*
*********************************************************************/
const L7_char8 *commandClearIntAuthServUsers(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  {
    cliSetCharInputID(1, ewsContext, argv);
    cliAlternateCommandSet(pStrInfo_base_ClrIASUsers);
    return pStrInfo_base_VerifyClrIASUsersYesNo; 
  }
  else if ((tolower(cliGetCharInput()) == 'y') ||
           (ewsContext->configScriptStateFlag == L7_CONFIG_SCRIPT_RUNNING))
  {
    /* Check if flag is set for execution */
    if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
    {
      for (userCount = 0; userCount < L7_MAX_IAS_USERS; userCount++ )
      {
         ...
         {
           return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error,
                  ewsContext, pStrErr_base_UnableClrIASUsers);
         }
       }
      ewsTelnetWrite(ewsContext, pStrInfo_base_IASUsersDBClred);
    }
  }
  else
  {
    ...
    {
      ewsTelnetWrite(ewsContext, pStrErr_base_UnableClrIASUsers);
    }
  }
