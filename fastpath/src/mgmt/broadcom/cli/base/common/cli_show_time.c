                                 ...
                                 const L7_char8 **argv, L7_uint32 index)
{
  ...
  (void)usmDbTimeZoneOffsetGet(unit, &minutes);
                                 ...
                                 const L7_char8 **argv, L7_uint32 index)
{
  ...
  L7_char8  zoneName [L7_TIMEZONE_ACRONYM_LEN+1];
  L7_char8  timezoneName[L7_TIMEZONE_ACRONYM_LEN+1];
  L7_char8  systemzoneName[L7_TIMEZONE_ACRONYM_LEN+1];
  ...
  memset (systemzoneName, 0, sizeof(systemzoneName));
  zoneName [0] = '\0';
  /* validate arguments */
  cliExamine(ewsContext, argc, argv, index);
  cliSyntaxTop(ewsContext);
  ...
  currentTime = usmDbAdjustedTimeGet();
  (void)usmDbTimeZoneOffsetGet(unit, &tzMinutes);
  usmDbSystemTimeZoneAcronymGet(unit, systemzoneName);
  ...
  sprintf(buffer, "\r\n%2.2d:%2.2d:%2.2d %s(UTC%+d:%02d) %s %d %d \r\n",
          LocalTime->tm_hour, LocalTime->tm_min, LocalTime->tm_sec, systemzoneName,
          hours, minutes, monthsOfYear[LocalTime->tm_mon+1], LocalTime->tm_mday,
          (LocalTime->tm_year + 1900));
...
L7_RC_t cliRunningConfigSummerTime(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if (usmDbStModeGet(USMDB_UNIT_CURRENT, &mode) == L7_SUCCESS && mode == FD_TIMEZONE_SUMMERTIME)
  {
    return L7_SUCCESS;
  }
  ...
  if (mode == L7_SUMMER_TIME_NON_RECURRING)
  {
