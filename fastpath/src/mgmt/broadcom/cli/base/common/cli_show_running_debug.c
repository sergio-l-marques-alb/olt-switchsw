...
L7_RC_t cliRunningConfigDebuggingInfo(EwsContext ewsContext)
{
 ...
 /*---------------------------------------------*/
  ...
  if (usmDbIpDhcpClientDebugTraceFlagGet (&rxFlag, &txFlag) == L7_SUCCESS)
  {
    if ((txFlag == L7_TRUE) && (rxFlag == L7_TRUE))
    {
      EWSWRITEBUFFER_ADD_BLANKS (1, 0, 0, 0, L7_NULLPTR, ewsContext,
                                 pStrInfo_base_DebugDhcpPktShow);
    }
    else if(rxFlag == L7_TRUE)
    {
      EWSWRITEBUFFER_ADD_BLANKS (1, 0, 0, 0, L7_NULLPTR, ewsContext,
                                 pStrInfo_base_DebugDhcpPktRx);
    }
    else if(txFlag == L7_TRUE)
    {
      EWSWRITEBUFFER_ADD_BLANKS (1, 0, 0, 0, L7_NULLPTR, ewsContext,
                                 pStrInfo_base_DebugDhcpPktTx);
    }
    else
    {
      /* Do Nothing */
