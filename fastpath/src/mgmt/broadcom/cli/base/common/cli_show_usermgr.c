                                                  ...
                                                  uintf index)
{
  ...
  /* display information about password strength check */
  cliFormat(ewsContext, pStrInfo_base_PasswdStrengthCheck);
  if (usmDbUserMgrPasswordStrengthCheckConfigGet(&enable) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%s", (enable == L7_ENABLE)?"Enable":"Disable");
  }

  /* display information about password minimum uppercase check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMinUppercaseLetters);
  if (usmDbUserMgrPasswdMinUppercaseCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum lowercase check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMinLowercaseLetters);
  if (usmDbUserMgrPasswdMinLowercaseCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum numeric check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMinNumericCharacters);
  if (usmDbUserMgrPasswdMinNumericCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum special check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMinSpecialCharacters);
  if (usmDbUserMgrPasswdMinSpecialCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum repeated check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMaxRepeatedCharacters);
  if (usmDbUserMgrPasswdMaxRepeatedCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum consecutive check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMaxConsecutiveCharacters);
  if (usmDbUserMgrPasswdMaxConsecutiveCharsLengthGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password minimum character classes check */
  cliFormat(ewsContext, pStrInfo_base_PasswdMinCharacterClasses);
  if (usmDbUserMgrPasswdMinCharacterClassesGet(&val8) == L7_SUCCESS)
  {
    ewsTelnetPrintf (ewsContext, "%d", val8);
  }

  /* display information about password exclude keyworkds */
  cliFormat(ewsContext, pStrInfo_base_PasswdExculdeKeywords);
  memset(keyword, 0x00, sizeof(keyword));
  do
  {
    if ((rc = usmDbUserMgrPasswdExcludeKeywordNextGet(keyword)) == L7_SUCCESS)
    {
      if(more == L7_TRUE)
      {
        ewsTelnetPrintf (ewsContext, ",\n\r                                                %s", keyword);
      ...
      {
        ewsTelnetPrintf (ewsContext, "%s", keyword);
      }
      more = L7_TRUE;
    }
  }while(rc == L7_SUCCESS);

  if(rc != L7_SUCCESS && more == L7_FALSE)
  {
    ewsTelnetPrintf (ewsContext, "%s", "<none>");
  }
                                         ...
                                         uintf index)
{
  ...
  L7_char8  strUserName[L7_LOGIN_SIZE];
  L7_char8  strOverrideCheck[L7_CLI_MAX_STRING_LENGTH + 1];
  L7_char8  strPasswdStrength[L7_CLI_MAX_STRING_LENGTH + 1];
  L7_char8  passwd[L7_CLI_MAX_STRING_LENGTH + 1];
  L7_uint32 unit;
  ...
  L7_BOOL val;
  L7_ushort16 val1;
  static L7_BOOL   detailFlg=L7_FALSE;
  ...
  for (; userCount < L7_MAX_LOGINS; userCount++)
  {
    L7_BOOL firstRow = L7_TRUE;
    L7_char8 splitLabel[L7_CLI_MAX_STRING_LENGTH];
    L7_uchar8 *hostAddrTraverse = strUserName;
    ...
    while (*hostAddrTraverse  != L7_EOS)
    {
       if (firstRow == L7_TRUE)
       {
         ...
         memset (strAccessLevel, 0, sizeof(strAccessLevel));
         ...
         memset (strPasswordAging, 0, sizeof(strPasswordAging));
         memset (strPasswordExpiry, 0, sizeof(strPasswordExpiry));
         ...
         if(usmDbUserMgrLoginPasswdStrengthOverrideCheckGet(userCount, &val) == L7_SUCCESS)
         {
           if (val == L7_ENABLE)
           {
             osapiSnprintf(strOverrideCheck, sizeof(strOverrideCheck), "%s", "Enable");
           }
           else
           {
             osapiSnprintf(strOverrideCheck, sizeof(strOverrideCheck), "%s", "Disable");
           }
         }
         else
         {
           osapiSnprintf(strOverrideCheck, sizeof(strOverrideCheck), "---");
         }

         if((cliWebLoginUserPasswordEncryptedGet(userCount, passwd) == L7_SUCCESS) &&
            (passwd[0] != 0))
         {
           if((usmDbUserMgrPasswordStrengthCheckConfigGet(&val) == L7_SUCCESS) &&
              (val == L7_ENABLE))
           {
             if(usmDbUserMgrPasswordIsStrong(strUserName, passwd, L7_TRUE) == L7_SUCCESS)
             {
               osapiSnprintf(strPasswdStrength, sizeof(strPasswdStrength), "%s", "Strong");
             }
             else
             {
               osapiSnprintf(strPasswdStrength, sizeof(strPasswdStrength), "%s", "Weak");
             }
           }
           else
           {
             osapiSnprintf(strPasswdStrength, sizeof(strPasswdStrength), "---");
           }
         }
         else
         {
         ...
         {
           cliFormat(ewsContext, "UserName");
           ewsTelnetWrite(ewsContext, strUserName);

           cliFormat(ewsContext, "Privilege");
           ewsTelnetWrite(ewsContext, strAccessLevel);
          
           cliFormat(ewsContext, "Password Aging");
           ewsTelnetWrite(ewsContext, strPasswordAging);
          
           cliFormat(ewsContext, "Password Expiry");
           ewsTelnetWrite(ewsContext, strPasswordExpiry);

           cliFormat(ewsContext, "Lockout");
           ewsTelnetWrite(ewsContext, strLockout);

           cliFormat(ewsContext, "Override Complexity Check");
           ewsTelnetWrite(ewsContext, strOverrideCheck);

           cliFormat(ewsContext, "Password Strength");
           ewsTelnetWrite(ewsContext, strPasswdStrength);
...
const L7_char8 *commandShowUsers(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  L7_char8 columnNames[140];
  L7_char8 columnNamesTop[140];
  L7_char8 columnUnderscore[140];
  ...
  L7_char8  strAccessLevel[L7_CLI_STRING_LENGTH + 1];
  L7_char8  strSnmpAccessLevel[L7_CLI_STRING_LENGTH + 1];
  L7_char8  strAuthentication[L7_CLI_STRING_LENGTH + 1];
  L7_char8  strEncryption[L7_CLI_MAX_STRING_LENGTH + 1];
  ...
  OSAPI_STRNCAT_ADD_BLANKS (0, 0, 3, 6, L7_NULLPTR,  columnNamesTop,  "User");
  ...
  for (; tempInt < L7_MAX_LOGINS; tempInt++)
  {
    ...
    if (strcmp(buf, pStrInfo_common_EmptyString) != 0)
    {
      L7_BOOL firstRow = L7_TRUE;
      L7_char8 splitLabel[L7_CLI_MAX_STRING_LENGTH];
      L7_uchar8 *hostAddrTraverse = buf;
        ...
        if (firstRow == L7_TRUE)
        {
          ...
          switch (val)                                                       /* val =  invalid, read only, read/write*/
          {
            case L7_LOGIN_ACCESS_READ_ONLY:
              osapiSnprintf(strAccessLevel,sizeof(strAccessLevel),"%s",pStrInfo_base_ReadOnly);
              break;
            case L7_LOGIN_ACCESS_READ_WRITE:
              osapiSnprintf(strAccessLevel,sizeof(strAccessLevel),"%s",pStrInfo_base_ReadWrite);
              break;
            default:
              osapiSnprintf(strAccessLevel,sizeof(strAccessLevel),"%s",pStrInfo_base_ReadOnly);
          }
          ...
          if (snmpUserSupported == L7_TRUE)
          {
            ...
            /*access mode*/
            memset (strSnmpAccessLevel, 0,sizeof(strSnmpAccessLevel));
            switch (accessLevel)                                             /*val =  invalid, read only, read/write*/
            {
              case L7_AGENT_COMM_ACCESS_LEVEL_READ_ONLY:
                osapiSnprintf(strSnmpAccessLevel,sizeof(strSnmpAccessLevel),"%s",pStrInfo_base_ReadOnly);
                break;
              case L7_AGENT_COMM_ACCESS_LEVEL_READ_WRITE:
                osapiSnprintf(strSnmpAccessLevel,sizeof(strSnmpAccessLevel),"%s",pStrInfo_base_ReadWrite);
                break;
              default:
                osapiSnprintf(strSnmpAccessLevel,sizeof(strSnmpAccessLevel),"%s",pStrInfo_base_ReadOnly);
            }
          ...
          if (snmpAuthEncryptSupported == L7_TRUE)
          {
            ...
            /*authentication type*/
             memset (strAuthentication, 0,sizeof(strAuthentication));
             switch (auth)
             {
               case L7_SNMP_USER_AUTH_PROTO_HMACMD5:
                 osapiSnprintf(strAuthentication,sizeof(strAuthentication),"%s",pStrInfo_common_Md5);    
                 break;
               case L7_SNMP_USER_AUTH_PROTO_HMACSHA:
                 osapiSnprintf(strAuthentication,sizeof(strAuthentication),"%s", pStrInfo_base_Sha);
                 break;
               case L7_SNMP_USER_AUTH_PROTO_NONE:
                 osapiSnprintf(strAuthentication,sizeof(strAuthentication),"%s", pStrInfo_common_None_1);
                 break;
               default:
                 osapiSnprintf(strAuthentication,sizeof(strAuthentication),"%s", pStrInfo_base_NotApplicable2); 
                 break;
             }
             ...
             switch (encrypt)
             {
               case L7_SNMP_USER_PRIV_PROTO_DES:
                 osapiSnprintf(strEncryption,sizeof(strEncryption),"%s",pStrInfo_base_Des);
                 break;
               case L7_SNMP_USER_PRIV_PROTO_NONE:
                 osapiSnprintf(strEncryption,sizeof(strEncryption),"%s", pStrInfo_common_None_1);
                 break;
               default:
                 osapiSnprintf(strEncryption,sizeof(strEncryption),"%s", pStrInfo_base_NotApplicable2); 
                 break;
             }
          ...
          memset(stat, L7_NULL, sizeof(stat));
          osapiSnprintf(stat, sizeof(stat),"\r\n%-26s%-13s%-13s%-16s%-10s", 
                        splitLabel, strAccessLevel, strSnmpAccessLevel, strAuthentication, strEncryption);
          ewsTelnetWrite(ewsContext, stat);
          firstRow = L7_FALSE;
        }
        else
...
/*********************************************************************
*
* @purpose   show passwords last configured result information
*
*
* @param EwsContext ewsContext
* @param uintf argc
...
* @notes
*
* @cmdsyntax  show passwords result
* @cmdhelp
*
* @cmddescript
*
                                           ...
                                           uintf index)
{
  ...
  {
    case L7_AUTH_METHOD_LOCAL:
      if(cliWebLoginUserNameGet(id, userName) == L7_SUCCESS)
      {
        ewsTelnetPrintf(ewsContext, "\n\rLast User whose password is set .................. %s\n\r", userName);
      }
      break;
    case L7_AUTH_METHOD_LINE:
      ewsTelnetPrintf(ewsContext, "\n\rLast Line Access Level whose password is set ..... %u\n\r", id);
      break;
    case L7_AUTH_METHOD_ENABLE:
      ewsTelnetPrintf(ewsContext, "\n\rLast Enable Access Level whose password is set ... %u\n\r", id);
      break;
    default:
      ewsTelnetPrintf(ewsContext,"\n\rUnknown Index .................................... %u\n\r", id);
      break;
  ...
  ewsTelnetPrintf(ewsContext, "Password strength check .......................... %s\n\r", (strengthCheck == L7_ENABLE)?"Enable":"Disable");

  ewsTelnetPrintf(ewsContext, "Last Password Set Result:\n\r");
  ewsTelnetPrintf(ewsContext, "========================");
  switch (result)
  {
    case L7_SUCCESS:
      ewsTelnetWrite(ewsContext, "\n\rSuccessfully Configured");
      break;
    case L7_ERROR:
...
* @notes
*
* @cmdsyntax    show aaa ias-users
*
* @cmdhelp
*
* @cmddescript
*    This commands displays all configured dot1x users.
*
* @end
*
*********************************************************************/
const L7_char8 *commandShowIntAuthServUsers(EwsContext ewsContext,
                                             L7_uint32 argc,
                                             const L7_char8 * * argv,
                                             L7_uint32 index)
{
/*  L7_char8  strPassword[L7_PASSWORD_SIZE + 1];*/
  ...
  {
    count = 0;
    for (; userCount < L7_MAX_IAS_USERS; )
    {
      bzero(strUserName, sizeof(strUserName));
    ...
    if(count++ >= CLI_MAX_SCROLL_LINES-6)
      {
        cliSetCharInputID(CLI_INPUT_NOECHO, ewsContext, argv);
        cliSyntaxBottom(ewsContext);
        cliAlternateCommandSet(pStrInfo_ShowAaaUser_1);
  ...
  else if(numArg == 1)
  {
    ...
    if (usmDbUserMgrIASUserDBUserIndexGet(strUserName, &userIndex) != L7_SUCCESS)
    {
      ewsTelnetWrite(ewsContext, CLIERROR_SHOWAUTHENTICATIONNOUSERERROR);
      cliSyntaxBottom(ewsContext);
      ewsCliDepth(ewsContext, cliPrevDepthGet(), argv);
