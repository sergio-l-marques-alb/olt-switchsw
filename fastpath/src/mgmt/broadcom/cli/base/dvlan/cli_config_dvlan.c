    ...
    if (usmDbFeaturePresentCheck(unit, L7_DVLANTAG_COMPONENT_ID, L7_DVLANTAG_ETHERTYPE_FEATURE_ID) == L7_TRUE)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        if (usmDbDvlantagIntfValidCheck(interface) != L7_SUCCESS)
        {
          ...
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrErr_switching_DvlanIntfError);
          continue;
        }
        ...
        switch (rc)
        {
          case L7_NOT_EXIST:
            status = L7_FAILURE;
            ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
            ewsTelnetWriteAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanTPIDNotCnfgrdGlobally);
            break;
          case L7_ALREADY_CONFIGURED:
            status = L7_FAILURE;
            ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
            ewsTelnetWriteAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanTPIDAlreadyConfigured);
            break;
          case L7_NOT_SUPPORTED:
            status = L7_FAILURE;
            ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
            ewsTelnetWriteAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanNotEnbldOnIntf);
            break;
          case L7_FAILURE:
            status = L7_FAILURE;
            ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
            osapiSnprintf(buf, sizeof(buf), pStrErr_switching_DvlanEtherType, L7_DVLANTAG_MIN_ETHERTYPE, L7_DVLANTAG_MAX_ETHERTYPE);
            ewsTelnetWriteAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, buf);
            break;
          default:
            break;
        }
...
* @notes
*
* @cmdsyntax  dvlan-tunnel ethertype {<802-1q|vman> | <custom><%d-%d>} [primary-tpid]
*
* @cmdhelp Configure DVLAN EtherType
*
* @cmddescript
*   Determines the ethertype for a double tagged interface in a VLAN
*
...
const L7_char8 *commandDVlanEtherType(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  L7_uint32   unit;
  L7_uint32   numArg;
  L7_BOOL    primaryTpid = L7_FALSE, setEtherType = L7_FALSE;
  ...
  if(strcmp(argv[index+argDefaultTpidVal], pStrInfo_common_PrimaryTpid) == 0 || 
  ...
  if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    if (usmDbFeaturePresentCheck(unit, L7_DVLANTAG_COMPONENT_ID, L7_DVLANTAG_ETHERTYPE_FEATURE_ID) == L7_TRUE)
    {
      rc = usmDbDvlantagEthertypeSet(unit, ether_val, primaryTpid, setEtherType);
      switch (rc)
      {
        case L7_NOT_EXIST:
          return cliSyntaxReturnPromptAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanInvalidTpid);
          break;
        case L7_ALREADY_CONFIGURED:
          return cliSyntaxReturnPromptAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanTpidExists);
          break;
        case L7_TABLE_IS_FULL:
          return cliSyntaxReturnPromptAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanTpidxFull);
          break;
        case L7_HARDWARE_ERROR:
          return cliSyntaxReturnPromptAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanHwError);
          break;
        case L7_FAILURE:
          return cliSyntaxReturnPromptAddBlanks (1,1, 0, 0, pStrErr_common_IncorrectInput,ewsContext, pStrErr_switching_DvlanEtherType, L7_DVLANTAG_MIN_ETHERTYPE, L7_DVLANTAG_MAX_ETHERTYPE );
          break;
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          status = L7_FAILURE;
          ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_FailedToSet,  ewsContext, pStrInfo_switching_DvlanModeSetFail);
        }
      }
    }
  }
  ...
  if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        status = L7_FAILURE;
        ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_FailedToSet,  ewsContext, pStrInfo_switching_DvlanCustidSetFail);
      }
    }
  }
