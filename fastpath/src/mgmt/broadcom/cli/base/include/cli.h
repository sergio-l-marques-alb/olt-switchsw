...
#define MAX_COMMAND_LENGTH              256 /*find emweb #defines */
...
typedef enum
{
  ...
  L7_WIRELESS_WDS_APGROUP_CONFIG_MODE,
  L7_MAINTENANCE_MODE,
  ...
  L7_WIRELESS_BUILDING_CONFIG_MODE,
  L7_WIRELESS_BUILDING_FLR_CONFIG_MODE,
  L7_INT_AUTH_SERV_USER_CONFIG_MODE,
  L7_TIMERANGE_CONFIG_MODE,
  L7_LAST_MODE
} cliModeID_t;
...
typedef struct L7_techSupportCurrentData_s
{
  L7_uint32 entryIndex;
  L7_uint32 numEntries;
  L7_uint32 startingEntryIndex;
  L7_BOOL flag;
  localOptions_t currentOption;
  L7_uchar8 lastDeviceId[65];
}L7_techSupportCurrentData_t;
...
#define CLI_FAILURE_DATA_MAX_LENGTH max(L7_PLATFORM_MAX_VLAN_ID, L7_MAX_INTERFACE_COUNT)
#define CLI_FAILURE_DATA_CHECK(data)  ((data.Count < CLI_FAILURE_DATA_MAX_LENGTH) ? L7_TRUE : L7_FALSE)

/* Use this to fill the failure errors during the command execution */
#define CLI_FAILURE_DATA_FILL(d, i, s) if(CLI_FAILURE_DATA_CHECK(d)) {\
d.Interface[d.Count] = i;\
d.String[d.Count] = s;\
d.Count++;\
}

extern void cliDisplayFailureInfo(EwsContext ewsContext, cliFailureData_t *failureData);
extern void cliPrintErrorMessage(cliFailureData_t *data, EwsContext ewsContext);
