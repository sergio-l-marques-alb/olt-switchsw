...
const L7_char8 *commandIpPimsmSptThreshold(EwsContext ewsContext, L7_uint32 argc,const L7_char8 * * argv, L7_uint32 index)
{
  ...
  else if((ewsContext->commType == CLI_NO_CMD) && ( numArgs == 0 ))
  {
        ...
        (thresholdRate <= L7_PIMSM_DATATHRESHOLD_RATE_MAX) )
   {
        ...
        if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
        {
          retVal = usmDbPimsmDataThresholdRateSet(unit, L7_AF_INET, thresholdRate);
          if (retVal == L7_NOT_SUPPORTED)
          {
            return cliSyntaxReturnPrompt (ewsContext,pStrErr_common_PimSmNonZeroSPTThreshold);
          }
          else if (retVal == L7_SUCCESS)
          {
            /* Do Nothing */
          }
          else
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (iface=1; iface < L7_MAX_INTERFACE_COUNT; iface++)
      {
                                                    ...
                                                    joinPruneIntvl) != L7_SUCCESS)
        {
          status = L7_FAILURE;
          ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_common_SetJoinPruneIntvl);
        }
      }
    }
  }
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (iface=1; iface < L7_MAX_INTERFACE_COUNT; iface++)
      {
                                          ...
                                          helloInterval) != L7_SUCCESS)
        {
          status = L7_FAILURE;
          ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_common_SetHelloIntvl);
        }
      }
    }
  }
  ...
  if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    ...
    if ( rc != L7_SUCCESS)
    {
      ...
      else
      {
        ...
        {
          return cliSyntaxReturnPrompt (ewsContext, pStrErr_common_PimSmStaticRpLoopback);
        }
        else
        {
