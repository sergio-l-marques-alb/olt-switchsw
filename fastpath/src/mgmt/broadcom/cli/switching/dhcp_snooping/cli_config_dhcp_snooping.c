                                             ...
                                             const L7_char8 **argv, L7_uint32 index)
{
  ...
  if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
  {
    if (ewsContext->commType == CLI_NORMAL_CMD) 
    {
      rc = usmDbDsStaticBindingAdd(&macAddr, ipAddr, vlanId, 0 /* PTin modified: DHCP */, intIfNum);
      if (rc == L7_ERROR)
      {
        return cliSyntaxReturnPromptAddBlanks(1, 1, 0, 0, L7_NULLPTR,  
                                              ewsContext, 
  ...
  if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
  {
    for (intIfNum=1; intIfNum < L7_MAX_INTERFACE_COUNT; intIfNum++)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        {
          ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, pStrErr_base_IpDhcpSnoopingIntfBad);
        }
        else 
        {
          ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, pStrErr_base_IpDhcpSnoopingTrustFail);
        }
      }
    }
  }
    ...
    for (intIfNum = 1; intIfNum < L7_MAX_INTERFACE_COUNT; intIfNum++)
    {
         ...
         (usmDbDsIntfBurstIntervalSet(intIfNum, FD_DHCP_SNOOPING_BURST_INTERVAL) != L7_SUCCESS))
      {
        snprintf(buf, L7_CLI_MAX_STRING_LENGTH, 
                 "Failed to reset the DHCP snooping rate limit on interface %s.",
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        else if (rc == L7_REQUEST_DENIED)
        {
          ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                  "IPSG port security cannot be disabled without disabling IPSG.");
        }
        else if (rc == L7_DEPENDENCY_NOT_MET)
        {
          ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                  "IPSG port security cannot be enabled without first enabling IPSG.");
        }
        else 
        {
          ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR,  
