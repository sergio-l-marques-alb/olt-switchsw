    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        if (usmDbDot3adIntfIsMemberGet(unit, interface, &portChannelIf) == L7_SUCCESS)
        {
          ...
          ewsTelnetPrintf (ewsContext, " %s %s", cliDisplayInterfaceHelp(unit, s, p),
        ...
        if(rc == L7_FAILURE)
        {
          ewsTelnetWriteAddBlanks (0, 2, 0, 1, pStrErr_common_UserInput,  ewsContext,pStrErr_common_UsrInputInvalidMcastMacAddr);
          continue;
        }        
        ...
        if (rc== L7_NOT_IMPLEMENTED_YET)
        {
          ...
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, L7_NULLPTR,  ewsContext, pStrInfo_common_CfguringDstPortsWithUcastMacAddrNotSupportedOnPlatform);
          continue;
        ...
        {
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_base_PortNotAdded);
          continue;
        }
      }
    }
  ...
  else if (ewsContext->commType == CLI_NO_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        {
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_base_PortNotDeld);
          continue;
        }
      }
    }
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        {
          ewsTelnetWriteAddBlanks (1, 0, 0, 0, pStrErr_common_CouldNot, ewsContext, pStrErr_base_AddPort);
          osapiSnprintf(buf, sizeof(buf)," %s %s", cliDisplayInterfaceHelp(unit, s, p),
                        pStrInfo_base_SlotPortWithoutUnitIsPartOfALag);
        ...
        if (rc == L7_NOT_SUPPORTED)
        {
          ...
          return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, 
                                                 pStrInfo_common_CfguringSrcPortsMacFilterNotSupportedOnPlatform);
        }
        else if (rc != L7_SUCCESS)
        {
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_base_SrcPortNotAdded);
        }
      }
    }
  }
  else if (ewsContext->commType == CLI_NO_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
      {
        ...
        {
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_base_SrcPortNotDeld);
        }
      }
    }
  }
...
const L7_char8 *commandMacFilterAddSrcAll(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    {
      interface = nextInterface;
      if (usmDbFilterPortParmCanSet(unit, interface) != L7_SUCCESS ||
          usmDbDot3adIntfIsMemberGet(unit, interface, &portChannelIf) == L7_SUCCESS)
      {
        continue;
      }
      L7_INTF_SETMASKBIT(allPortsMask, interface);
    }

    rc = usmdbFilterSrcIntfMaskModify(strMacAddr, vlanID, &allPortsMask);
    if (rc == L7_NOT_SUPPORTED)
      {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrInfo_common_CfguringSrcPortsMacFilterNotSupportedOnPlatform);
      }
    else if (rc == L7_TABLE_IS_FULL)
    {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_Error, ewsContext, pStrInfo_base_MacFilterFull);
    }
    else if (rc != L7_SUCCESS)
    {
  ...
  else if (ewsContext->commType == CLI_NO_CMD)
  {
    ...
    if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      {
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_base_PortNotRemovedFrmMask);
        return cliPrompt(ewsContext);
      }
    }
  }
