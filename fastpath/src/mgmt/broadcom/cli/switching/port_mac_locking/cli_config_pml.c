  ...
  for (interface=interface_start; interface < interface_end; interface++)
  {
    ...
    if (ewsContext->commType ==  CLI_NORMAL_CMD)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot, ewsContext, pStrErr_switching_CantEnblPortSecurity);
      }
    }
    else if (ewsContext->commType ==  CLI_NO_CMD)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        status = L7_FAILURE;
        ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot, ewsContext, pStrErr_switching_CantDsblPortSecurity);
      }
    }
  }
        ...
        if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
        {
          for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
          {
            ...
            if (rc != L7_SUCCESS)
            {
              status = L7_FAILURE;
              ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
              ewsTelnetWriteAddBlanks (1, 1, 1, 0, pStrErr_common_CouldNot,  ewsContext, pStrErr_switching_CfgPortSecurityIfMacAddAddErr);
