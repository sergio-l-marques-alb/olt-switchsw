  ...
  if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
  {
    for (intIfNum=1; intIfNum < L7_MAX_INTERFACE_COUNT; intIfNum++)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        ...
        ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, pStrErr_base_DaiIfTrustFail);
      }
    }
  }
    ...
    if(ewsContext->commType == CLI_NORMAL_CMD)
    {
      if (ewsContext->scriptActionFlag == L7_EXECUTE_SCRIPT)
      {
        if(strcmp(argv[index+1], pStrInfo_base_LimitNone) == L7_SUCCESS)
        {
          ...
          {
            ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
            ewsTelnetWriteAddBlanks(1, 1, 0, 0, L7_NULLPTR,  
        ...
        else if(cliConvertTo32BitUnsignedInteger(argv[index+2], &rate) == L7_SUCCESS)
        {
          ...
          { 
            ...
            {
              ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
              ewsTelnetWriteAddBlanks(1, 1, 0, 0, pStrErr_common_IncorrectInput,  
                                                    ewsContext, pStrErr_base_CfgDaiIfRateLimit);
              status = L7_FAILURE;
            }
          }
                                        ...
                                        const L7_char8 **argv, L7_uint32 index)
{
  ...
  OSAPI_STRNCPY_SAFE(macAddrStr, argv[index + argMacAddr]);
  ...
  if (ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    if (ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
    {
      ...
      if (usmDbArpAclRuleGet(arpAclName, hostIPAddr, &macAddr) == L7_SUCCESS)
      {
        ewsContext->actionFunctionStatusFlag=L7_ACTION_FUNC_SUCCESS;
        return cliPrompt(ewsContext);
      }

      rc = usmDbArpAclRuleAdd(arpAclName, hostIPAddr, &macAddr);
      if (rc != L7_SUCCESS)
      {
  ...
  else if(ewsContext->commType == CLI_NO_CMD)
  {
    ...
    rc = usmDbArpAclRuleDelete(arpAclName, hostIPAddr, &macAddr);
    if (rc != L7_SUCCESS)
    {
      return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext,
