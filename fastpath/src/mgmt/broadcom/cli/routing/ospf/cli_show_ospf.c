...

{
  ...
  {
    osapiStrncpySafe(stat, strUtilEnableDisableGet(val, pStrInfo_common_Blank), sizeof(stat));
  }
  else
  {
    osapiStrncpySafe(stat, pStrErr_common_Err, sizeof(stat));
  }
  ewsTelnetWrite(ewsContext, stat);
  ...
  {
    osapiStrncpySafe(stat, strUtilEnabledDisabledGet(passiveMode,pStrInfo_common_Dsbld), sizeof(stat));
  }
  else
  {
    osapiStrncpySafe(stat, pStrErr_common_Err, sizeof(stat));
  ...
  {
    memset(stat, 0, sizeof(stat));
    cliFormat(ewsContext, "Stub Router Configuration");
    if (stubRtrMode == OSPF_STUB_RTR_CFG_DISABLED)
      osapiSnprintf(stat, sizeof(stat), "None");
    else if (stubRtrMode == OSPF_STUB_RTR_CFG_ALWAYS)
      osapiSnprintf(stat, sizeof(stat), "Always");
    else if (stubRtrMode == OSPF_STUB_RTR_CFG_STARTUP)
      osapiSnprintf(stat, sizeof(stat), "Startup");
    ewsTelnetWrite(ewsContext, stat);

    if (stubRtrMode == OSPF_STUB_RTR_CFG_STARTUP)
    {
      memset(stat, 0, sizeof(stat));
      cliFormat(ewsContext, "Stub Router Startup Time");
      osapiSnprintf(stat, sizeof(stat), "%u seconds", startupDuration);
      ewsTelnetWrite(ewsContext, stat);
    }

    memset(stat, 0, sizeof(stat));
    cliFormat(ewsContext, "Summary LSA Metric Override");
    if (overrideSummaryMetric == L7_ENABLE)
    {
      osapiSnprintf(stat, sizeof(stat), "Enabled (%u)", summaryMetric);
    }
    else
    {
      osapiSnprintf(stat, sizeof(stat), "Disabled");
    }
    ewsTelnetWrite(ewsContext, stat);
  }
#endif
  ...
  status = usmDbOspfStatusGet(&ospfStatus);
  if (status == L7_SUCCESS)
  {
    ...
    /* Stub Router */
    memset (stat, 0, sizeof(stat));
    cliFormat(ewsContext, "Stub Router Status");                           
    if (ospfStatus.stubRouter == L7_TRUE)
    {
      osapiSnprintf(stat, sizeof(stat), "Active");
    }
    else 
    {
      osapiSnprintf(stat, sizeof(stat), "Inactive");
    }
    ewsTelnetWrite(ewsContext, stat);

    if (ospfStatus.stubRouter)
    {
      memset (stat, 0, sizeof(stat));
      cliFormat(ewsContext, "Stub Router Reason");                           
      if (ospfStatus.stubReason == OSPF_STUB_RTR_CONFIGURED)
        osapiSnprintf(stat, sizeof(stat), "Configured");
      else if (ospfStatus.stubReason == OSPF_STUB_RTR_STARTUP)
        osapiSnprintf(stat, sizeof(stat), "Startup");
      else if (ospfStatus.stubReason == OSPF_STUB_ROUTER_RESOURCE)
        osapiSnprintf(stat, sizeof(stat), "Resource Limitation");
      ewsTelnetWrite(ewsContext, stat);

      if (ospfStatus.stubReason == OSPF_STUB_RTR_STARTUP)
      {
        memset (stat, 0, sizeof(stat));
        cliFormat(ewsContext, "Stub Router Time Remaining");                           
        osapiSnprintf(stat, sizeof(stat), "%u seconds", ospfStatus.stubRemainingTime);
        ewsTelnetWrite(ewsContext, stat);
      }
    }
  ...
  cliSyntaxTop(ewsContext);
  memset (stat, 0, sizeof(stat));
  cliFormat(ewsContext, "NSF Support");
  if (usmDbOspfNsfSupportGet(&nsfSupport) == L7_SUCCESS)
  {
    osapiSnprintf(stat, sizeof(stat), 
                  (nsfSupport == OSPF_NSF_NONE) ? "Disabled" : 
                  (nsfSupport == OSPF_NSF_PLANNED_ONLY) ? "Planned" :
                  (nsfSupport == OSPF_NSF_ALWAYS) ? "Always" : "Error");
  }
  else
  {
    osapiSnprintf(stat, sizeof(stat), pStrErr_common_Err);
  ...
  memset (stat, 0, sizeof(stat));
  cliFormat(ewsContext, "NSF Restart Interval");
  if (usmDbOspfRestartIntervalGet(&value) == L7_SUCCESS)
  {
    osapiSnprintf(stat, sizeof(stat), "%u seconds", value);
  }
  else
  {
    osapiSnprintf(stat, sizeof(stat), pStrErr_common_Err);
  ...
  {
    memset(stat, 0, sizeof(stat));
    cliFormat(ewsContext, "NSF Restart Status");
    osapiSnprintf(stat, sizeof(stat), 
                  (ospfStatus.restartStatus == OSPF_GR_NOT_RESTARTING) ? "Not Restarting" : 
                  (ospfStatus.restartStatus == OSPF_GR_PLANNED_RESTART) ? "Planned restart" :
                  (ospfStatus.restartStatus == OSPF_GR_UNPLANNED_RESTART) ? "Unplanned restart" : "Error");
    ewsTelnetWrite(ewsContext, stat);

    memset (stat, 0, sizeof(stat));
    cliFormat(ewsContext, "NSF Restart Age");
    osapiSnprintf(stat, sizeof(stat), "%u seconds", ospfStatus.restartAge);
    ewsTelnetWrite(ewsContext, stat);

    memset(stat, 0, sizeof(stat));
    cliFormat(ewsContext, "NSF Restart Exit Reason");
    osapiSnprintf(stat, sizeof(stat), 
                  (ospfStatus.grExitReason == OSPF_GR_EXIT_NONE) ? "Not attempted" : 
                  (ospfStatus.grExitReason == OSPF_GR_IN_PROGRESS) ? "In progress" :
                  (ospfStatus.grExitReason == OSPF_GR_COMPLETED) ? "Completed" : 
                  (ospfStatus.grExitReason == OSPF_GR_TIMED_OUT) ? "Timed out" : 
                  (ospfStatus.grExitReason == OSPF_GR_TOPO_CHANGE) ? "Topology change" : 
                  (ospfStatus.grExitReason == OSPF_GR_MANUAL_CLEAR) ? "Manual clear" : "Error");
    ewsTelnetWrite(ewsContext, stat);
  }
#endif

  /* NSF helper support can be included even if NSF is not. */
  memset(stat, 0, sizeof(stat));
  cliFormat(ewsContext, "NSF Helper Support");
  if (usmDbOspfHelperSupportGet(&helperSupport) == L7_SUCCESS)
  {
    osapiSnprintf(stat, sizeof(stat), 
                  (helperSupport == OSPF_HELPER_NONE) ? "Disabled" : 
                  (helperSupport == OSPF_HELPER_PLANNED_ONLY) ? "Planned" :
                  (helperSupport == OSPF_HELPER_ALWAYS) ? "Always" : "Error");
  }
  else
  {
    osapiSnprintf(stat, sizeof(stat), pStrErr_common_Err);
  ...
  memset(stat, 0, sizeof(stat));
  cliFormat(ewsContext, "NSF Helper Strict LSA Checking");
  if (usmDbOspfStrictLsaCheckingGet(&strictLsaChecking) == L7_SUCCESS)
  {
    if (strictLsaChecking)
      osapiSnprintf(stat, sizeof(stat), "Enabled");
    else
      osapiSnprintf(stat, sizeof(stat), "Disabled");
  }
  else
        ...
        (adminMode == L7_ENABLE))
    {
          ...
          (usmDbInetNtoa(intAreaId, stat) == L7_SUCCESS))
      {
        memset(attrib[2].remainStr, 0, sizeof(attrib[2].remainStr));
         ...
         {
           osapiStrncpySafe(attrib[2].remainStr, &stat[attrib[2].precision], 
                            (strlen(stat) - attrib[2].precision));
...
const char *commandShowIpOspfNeighbor(EwsContext ewsContext, uintf argc, const char * * argv, uintf index)
{
...
#ifdef L7_NSF_PACKAGE
    /* Only show helper status if helping is allowed */
    if ((usmDbOspfHelperSupportGet(&helperSupport) == L7_SUCCESS) &&
        (helperSupport != OSPF_HELPER_NONE))
    {
      L7_ospfHelpfulNbrStatus_t nbrStatus;
      if (usmDbOspfHelpfulNbrStatusGet(intIface, nbrIpAddress, &nbrStatus) == L7_SUCCESS)
      {
        /* helping or not */
        cliFormat(ewsContext, "Restart Helper Status");
        if (nbrStatus.exitReason == OSPF_HELPER_IN_PROGRESS)
        {
          ewsTelnetWrite(ewsContext, "Helping");
        }
        else
        {
          ewsTelnetWrite(ewsContext, "Not helping");
        }

        /* restart reason. Don't print if restart never tried */
        if (nbrStatus.restartCode < 100)
        {
          memset (stat, 0, sizeof(stat));
          cliFormat(ewsContext, "Restart Reason");
          osapiSnprintf(stat, sizeof(stat), "%s (%u)", 
                        nbrStatus.restartReason, nbrStatus.restartCode);
          ewsTelnetWrite(ewsContext, stat);
        }
        ...
        if (nbrStatus.exitReason == OSPF_HELPER_IN_PROGRESS)
        {
          ...
          memset (stat, 0, sizeof(stat));
          cliFormat(ewsContext, "Remaining Grace Time");
          osapiSnprintf(stat, sizeof(stat), "%u sec", nbrStatus.restartAge);
          ewsTelnetWrite(ewsContext, stat);
        }

        cliFormat(ewsContext, "Restart Helper Exit Reason");
        ewsTelnetWrite(ewsContext, nbrStatus.exitReasonStr);
      }
    }
#endif
  ...
  if(nbrIpAddress != 0)
  {
    ...
    for (i = 1; i < argc; i++)
    {
      ...
      strncat(cmdBuf, argv[i], sizeof(cmdBuf));
    }
    cliAlternateCommandSet(cmdBuf);
    return pStrInfo_common_Name_2;
  }
 ...
 * @returns
 *
 * @notes   User not able to specify an interface. So he can't limit the 
 *          output to link scope LSAs for a specific interface. Best he can
 *          do is limit output to an area and we'll list all link scope LSAs
 *          within that area. 
 *
 * @end
 *
    ...
    L7_ospfLsdbEntry_t * p_Lsa)
{
  ...
  switch (intType)
  {
    ...
    case L7_S_AS_OPAQUE_LSA:
      ...
      break;
    default:
      rc = usmDbOspfLsdbAdvertisementGet(unit, areaId, p_Lsa->intIfNum, intType, intLsId,
                                         intRouterId, &p_LsaInfo, &lsaLen);
    break;
  }
...
const char *commandShowIpOspfDatabase(EwsContext ewsContext, uintf argc, const char * * argv, uintf index)
{
  ...
  else
  {
    ...
    memset(&opaqueLsa, 0, sizeof(L7_ospfOpaqueLsdbEntry_t));
    ...
    if (callExtFunc == L7_FALSE)
    {
      ...
      if(foundEntry == L7_SUCCESS)
      {
        if(isUserLsIdCfg && (intLsId < userLsId))
        {
          ...
          intRouterId = (isUserAdvRouterCfg) ? userAdvRouter : 0;
          foundEntry = usmDbOspfLsdbEntryNext(unit, &intAreaId, &intType, &intLsId, &intRouterId, &Lsa);
        }
      }
      else
