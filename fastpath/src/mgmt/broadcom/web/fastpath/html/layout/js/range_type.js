...
function fp_EncodeRange(xid,list_values,conf_values)
{
...
function fp_DecodeRange(xid,list_values,rangeStr){
  var range = new Array();
  ...
  for(var i=0;i<length;i++){
   var upperLimit = fp_parseAndGetUpperLimit(range[i]);
    ...
    }while(j != fp_nullValue  && 
           fp_getDiff(list_values,j,upperLimit) <= 0) ;
...
function range_type_SubmitStart(submitFlag){

}
function range_type_SubmitEnd(submitFlag){
}
function range_type_LoadEnd(arg){
}
...
function range_type_LoadStart(arg){
  xuiTypeRegister("string", fp_EncodeStringRange, fp_DecodeStringRange);
  xuiTypeRegister("uspRange_t", fp_EncodeRange, fp_DecodeRange);
  xuiTypeRegister("vlanRange_t", fp_EncodeRange, fp_DecodeRange);
}
