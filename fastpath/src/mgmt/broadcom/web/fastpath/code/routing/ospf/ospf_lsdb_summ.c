...
L7_char8 *usmWebLSDBTableInfoGet(L7_uint32 unit, usmWeb_LsdbInfo_t lsdb_entry)
{
  ...
  strcat(usmWebBigBuf, buf);
    ...
    memset(buf, 0, sizeof(buf));
    memset(tempBuff,0x00,sizeof(tempBuff));
    rc = usmDbOspfLsdbAgeGet(unit, lsdb_entry.areaId, 0, lsdb_entry.type, lsdb_entry.Lsid, lsdb_entry.routerId, &val);
    osapiSnprintf(tempBuff, sizeof(tempBuff),  "%u", val);
     osapiSnprintf(buf, sizeof(buf),  " <td class=\"tabledata\"> %s </td>",tempBuff);
     strcat(usmWebBigBuf, buf);
    ...
    memset(buf, 0, sizeof(buf));
    memset(tempBuff,0x00,sizeof(tempBuff));
    rc = usmDbOspfLsdbSequenceGet(unit, lsdb_entry.areaId, 0, lsdb_entry.type, lsdb_entry.Lsid, lsdb_entry.routerId, &val);
    osapiSnprintf(tempBuff, sizeof(tempBuff), "%#x", val);
    osapiSnprintf(buf, sizeof(buf),  " <td class=\"tabledata\"> %s </td>",tempBuff);
    strcat(usmWebBigBuf, buf);
    ...
    memset(buf, 0, sizeof(buf));
    memset(tempBuff,0x00,sizeof(tempBuff));
    rc = usmDbOspfLsdbChecksumGet(unit, lsdb_entry.areaId, 0, lsdb_entry.type, lsdb_entry.Lsid, lsdb_entry.routerId, &val);
    osapiSnprintf(tempBuff, sizeof(tempBuff),  "%#x", val);
    osapiSnprintf(buf, sizeof(buf),  " <td class=\"tabledata\"> %s </td>",tempBuff);
    strcat(usmWebBigBuf, buf);

    memset(buf, 0, sizeof(buf));
    if (usmDbOspfAreaLSADbLSAOptionsGet(unit, lsdb_entry.areaId, 0, lsdb_entry.type, lsdb_entry.Lsid,
                                        lsdb_entry.routerId, &byteOptions) == L7_SUCCESS)
    {
...
char *ewaFormSubmit_ospf_lsdb_summ(EwsContext context, EwaForm_ospf_lsdb_summP form)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_CLI_WEB_COMPONENT_ID, "# (ospf_lsdb_summ.html) Form Submission Failed. "
