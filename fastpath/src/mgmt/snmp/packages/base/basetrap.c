...
int
snmp_agentSwitchIpAddressConflictTrapSend(L7_uint32 conflictingIp,
                                          L7_uchar8 *conflictingMac)
{
  return send_agentSwitchIpAddressConflictTrap(conflictingIp, conflictingMac);
}
...
/*********************************************************************
*
* @purpose  Sends when an ip address conflict is reported on any
*           IP interface in the switch
*
* @param    conflictingIp  @b{(input)} Conflicting IP address
* @param    conflictingMac @b{(input)} MAC of the conflicting host
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
* @end
*********************************************************************/
L7_RC_t send_agentSwitchIpAddressConflictTrap(L7_uint32 conflictingIp,
                                              L7_uchar8 *conflictingMac)
{
  L7_uint32 status, tempIp;
  VarBind *temp_vb = NULL;
  OID *snmpTrapOID;
  OID *oid_conflictIp, *oid_conflictMac;
  VarBind *var_conflictIp = L7_NULLPTR, *var_conflictMac = L7_NULLPTR;
  OctetString *os_conflictIp = L7_NULLPTR, *os_conflictMac = L7_NULLPTR;

  /* Making OIDs for agentSwitchIpAddressConflictTrap, agentSwitchConflictIPAddr,
   * agentSwitchConflictMacAddr */
  snmpTrapOID     = MakeOIDFromDot("agentSwitchIpAddressConflictTrap");

  oid_conflictIp  = MakeOIDFromDot("agentSwitchConflictIPAddr");
  oid_conflictMac = MakeOIDFromDot("agentSwitchConflictMacAddr");

  tempIp          = osapiHtonl(conflictingIp);
  os_conflictIp   = MakeOctetString((char*) &tempIp, L7_IP_ADDR_LEN);
  os_conflictMac  = MakeOctetString((char*) conflictingMac, L7_ENET_MAC_ADDR_LEN);

  /* Creating Var Binds */
  var_conflictIp  = MakeVarBindWithValue(oid_conflictIp, NULL, IP_ADDR_PRIM_TYPE, os_conflictIp);
  var_conflictMac = MakeVarBindWithValue(oid_conflictMac, NULL, OCTET_PRIM_TYPE, os_conflictMac);

  FreeOID(oid_conflictIp);
  FreeOID(oid_conflictMac);

  if (snmpTrapOID == NULL || var_conflictIp == NULL || var_conflictMac == NULL)
  {
    FreeOID(snmpTrapOID);

    if(var_conflictIp == NULL)
      FreeOctetString(os_conflictIp);
    else
      FreeVarBind(var_conflictIp);

    if(var_conflictMac == NULL)
      FreeOctetString(os_conflictMac);
    else
      FreeVarBind(var_conflictMac);

    return L7_FAILURE;
  }

  temp_vb                   = var_conflictIp;
  var_conflictIp->next_var  = var_conflictMac;
  var_conflictMac->next_var = NULL;
  status = SendNotificationsSMIv2Params(snmpTrapOID, temp_vb, NULL);
