...
L7_RC_t l7_snmpTargetAddrTAddressGet(L7_char8* snmpTargetAddrName, L7_char8 *val)
{
  ...
  {
    if (snmpTargetAddrEntryData->snmpTargetAddrTAddress->length == 6)
    {
      ...
      debug_snmp_sprintHexString(snmpTargetAddrEntryData->snmpTargetAddrTAddress->octet_ptr, 
                                 snmpTargetAddrEntryData->snmpTargetAddrTAddress->length, 
                                 val);
    }
...
debug_snmpTargetAddrTable_walk()
{
  ...
  while(l7_snmpTargetAddrTableNextGet(snmpTargetAddrName) == L7_SUCCESS)
  {
    ...
    {
      l7_ObjectFromOID(temp_string, temp_string2);
      sysapiPrintf("snmpTargetAddrTDomain.\"%s\" = \"%s\" (%s)\n", snmpTargetAddrName, temp_string, temp_string2);
    }
    else
    {
