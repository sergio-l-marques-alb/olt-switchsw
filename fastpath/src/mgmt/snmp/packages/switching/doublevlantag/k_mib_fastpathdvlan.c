...
#ifdef I_agentSwitchDVlanTagPrimaryTPid
   case I_agentSwitchDVlanTagPrimaryTPid:
     if (snmpAgentSwitchDvlanTagIsPrimaryTPid(USMDB_UNIT_CURRENT,
                 (L7_uint32)agentSwitchDVlanTagEntryData.agentSwitchDVlanTagTPid) == L7_SUCCESS)
     {
       agentSwitchDVlanTagEntryData.agentSwitchDVlanTagPrimaryTPid = 
                            D_agentSwitchDVlanTagPrimaryTPid_true; /* SNMP TRUE */
     }
     else
     {
       agentSwitchDVlanTagEntryData.agentSwitchDVlanTagPrimaryTPid = 
                            D_agentSwitchDVlanTagPrimaryTPid_false; /* SNMP FALSE */
     }
     SET_VALID(I_agentSwitchDVlanTagPrimaryTPid, agentSwitchDVlanTagEntryData.valid);     
     if (nominator != -1) break;
     /* else pass through */
#endif
                               ...
                               ContextInfo *contextInfo, int function)
{
  ...
  if (VALID(I_agentSwitchDVlanTagRowStatus, data->valid) &&
    data->agentSwitchDVlanTagRowStatus == D_agentSwitchDVlanTagRowStatus_destroy)
  {
    ...
    if (usmDbDvlantagEthertypeSet(USMDB_UNIT_CURRENT,
                               ...
                               data->agentSwitchDVlanTagPrimaryTPid,
                               L7_FALSE) != L7_SUCCESS)
#else
    if (usmDbDvlantagEthertypeSet(USMDB_UNIT_CURRENT,
                               (L7_uint32)data->agentSwitchDVlanTagTPid,
  ...
  else if (VALID(I_agentSwitchDVlanTagRowStatus, data->valid) &&
...
{
  if (usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_DVLANTAG_COMPONENT_ID,
                               L7_DVLANTAG_FEATURE_SUPPORTED) == L7_TRUE )
  {
  ...
  if ((usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_DVLANTAG_COMPONENT_ID, L7_DVLANTAG_CUSTID_FEATURE_ID) == L7_TRUE)
          || (usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_DVLANTAG_COMPONENT_ID, L7_DVLANTAG_ETHERTYPE_PER_INTERFACE_FEATURE_ID) == L7_TRUE))
  {
    if (usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_DVLANTAG_COMPONENT_ID,
                                 L7_DVLANTAG_CUSTID_FEATURE_ID) == L7_TRUE)
    {
      #ifdef I_agentSwitchPortDVlanTagCustomerId
                                 ...
                                 == L7_TRUE)
    {
      ...
      if (VALID(I_agentSwitchPortDVlanTagRowStatus, data->valid) &&
      ...
      else if (VALID(I_agentSwitchPortDVlanTagRowStatus, data->valid) &&
                               ...
                               int nominator)
{
   ...
   case I_agentSwitchDVlanTagEthertype:
     if (snmpAgentSwitchDvlanTagEthertypeCurrentValueGet(USMDB_UNIT_CURRENT,
                               ...
                               ContextInfo *contextInfo, int function)
{
  ...
  {
    if (usmDbDvlantagEthertypeSet(USMDB_UNIT_CURRENT,
                               data->agentSwitchDVlanTagEthertype, L7_TRUE, L7_TRUE) != L7_SUCCESS)
    {
      return COMMIT_FAILED_ERROR;
    }
  }
