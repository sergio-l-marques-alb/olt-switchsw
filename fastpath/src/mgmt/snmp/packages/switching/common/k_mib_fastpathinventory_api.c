...
L7_RC_t snmpAgentInventoryStackSTKnameGet(L7_uint32 *val)
{
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    if (strcmp(localName, image1FileName) == 0)
    {
      *val = D_agentInventoryStackSTKname_image1;
    }
    else if (strcmp(localName, image2FileName) == 0)
    {
...
L7_RC_t snmpAgentInventoryStackSTKnameSet(L7_uint32 val)
{
  L7_RC_t rc = L7_SUCCESS;
  L7_uint32 curVal;
  L7_char8 imageFileName[L7_CLI_MAX_STRING_LENGTH];
  ...
  case D_agentInventoryStackSTKname_image1:
    usmDbImageFileNameGet(0, imageFileName);
    rc = usmDbTransferFileNameLocalSet(USMDB_UNIT_CURRENT, imageFileName);
    break;

  case D_agentInventoryStackSTKname_image2:
    usmDbImageFileNameGet(1, imageFileName);
    rc = usmDbTransferFileNameLocalSet(USMDB_UNIT_CURRENT, imageFileName);
    break;
...
L7_RC_t snmpAgentInventoryStackActivateSTKSet(L7_uint32 val)
{
  ...
  case D_agentInventoryStackActivateSTK_enable:
    rc = usmDbTransferFileNameLocalGet(USMDB_UNIT_CURRENT, snmp_buffer);
    rc = usmDbImageActivate(USMDB_UNIT_CURRENT, snmp_buffer, L7_FALSE);
    break;
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    case L7_USMDB_UNITMGR_CFG_MISMATCH:
      *val = D_agentInventoryUnitStatus_configMismatch;
      break;
    case L7_USMDB_UNITMGR_SDM_MISMATCH:
      *val = D_agentInventoryUnitStatus_sdmMismatch;
      break;
    case L7_USMDB_UNITMGR_UNIT_NOT_PRESENT:
      *val = D_agentInventoryUnitStatus_notPresent;
      break;
    case L7_USMDB_UNITMGR_CODE_UPDATE:
      *val = D_agentInventoryUnitStatus_codeUpdate;
      break;
    default:
      rc = L7_FAILURE;
      break;
...
L7_RC_t snmpAgentInventoryStackFirmwareSyncModeSet(L7_uint32 val)
{
  ...
  /* succeed if set to current value */
  rc = usmDbUnitMgrStackFirmwareSyncModeGet(&curVal);
  if (rc == L7_SUCCESS && curVal == val)
  {
    return L7_SUCCESS;
  }
  ...
  switch (val)
  {
    case D_agentInventorySFS_enable:
      curVal = L7_ENABLE;
      break;

    case D_agentInventorySFS_disable:
      curVal = L7_DISABLE;
      break;
  ...
  if (rc != L7_FAILURE)
  {
    return usmDbUnitMgrStackFirmwareSyncModeSet(curVal);
  }  
  return rc;
}
...
L7_RC_t snmpAgentInventoryStackFirmwareSyncAllowDowngradeModeSet(L7_uint32 val)
{
  ...
  /* succeed if set to current value */
  rc = usmDbUnitMgrStackFirmwareSyncAllowDowngradeModeGet(&curVal);
  if (rc == L7_SUCCESS && curVal == val)
  {
    return L7_SUCCESS;
  }
  ...
  switch (val)
  {
    case D_agentInventorySFSAllowDowngrade_enable:
      curVal = L7_ENABLE;
      break;

    case D_agentInventorySFSAllowDowngrade_disable:
      curVal = L7_DISABLE;
      break;
  ...
  if (rc != L7_FAILURE)
  {
    return usmDbUnitMgrStackFirmwareSyncAllowDowngradeModeSet(curVal);
  }  
  return rc;
}
...
L7_RC_t snmpAgentInventoryStackFirmwareSyncTrapModeSet(L7_uint32 val)
{
  ...
  /* succeed if set to current value */
  rc = usmDbUnitMgrStackFirmwareSyncTrapModeGet(&curVal);
  if (rc == L7_SUCCESS && curVal == val)
  {
    return L7_SUCCESS;
  }
  ...
  switch (val)
  {
    case D_agentInventorySFSTrap_enable:
      curVal = L7_ENABLE;
      break;

    case D_agentInventorySFSTrap_disable:
      curVal = L7_DISABLE;
      break;
  ...
  if (rc != L7_FAILURE)
  {
    return usmDbUnitMgrStackFirmwareSyncTrapModeSet(curVal);
  } 
  return rc;
}
...
L7_RC_t snmpAgentInventoryUnitSFSTransferStatusGet(L7_uint32 unit,L7_uint32* status)
{
  ...
  rc = usmDbUnitMgrStackFirmwareSyncMemberStatusGet(unit,status); 
  if (L7_SUCCESS == rc)
  {
    if (*status == L7_SFS_SWITCH_STATUS_NO_ACTION)
    {
      *status = D_agentInventoryUnitSFSTransferStatus_noAction;
    }
    else if (*status == L7_SFS_SWITCH_STATUS_IN_PROGRESS)
    {
      *status = D_agentInventoryUnitSFSTransferStatus_inProgress;
    }  
  }
  return rc;
}
...
L7_RC_t snmpAgentInventoryUnitSFSLastAttemptStatusGet(L7_uint32 unit,L7_uint32* status)
{
  ...
  rc = usmDbUnitMgrStackFirmwareSyncMemberLastAttemptStatusGet(unit,status);  
  if (L7_SUCCESS == rc)
  {
    if (*status == L7_SFS_NONE)
    {
      *status = D_agentInventoryUnitSFSLastAttemptStatus_none;
    }
    else if (*status == L7_SFS_SUCCESS)
    {
      *status = D_agentInventoryUnitSFSLastAttemptStatus_success;
    }
    else if (*status == L7_SFS_FAIL)
    {
      *status = D_agentInventoryUnitSFSLastAttemptStatus_failure;  
    }
    else if (*status == L7_SFS_MIN_BOOTCODE_VERSION_NOT_PRESENT)
    {
      *status = D_agentInventoryUnitSFSLastAttemptStatus_min_bootcode_version_not_present;
    }
  }
  return rc;
}
