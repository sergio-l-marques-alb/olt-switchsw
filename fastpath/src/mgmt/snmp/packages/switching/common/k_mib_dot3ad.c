...
void snmpReverseBitOrderChar(L7_uchar8 prevState, L7_uchar8* newState);
lagMIBObjects_t *
 ...
 */ 
   ...
   case I_dot3adAggPortActorAdminState:
     ...
     {
       snmpReverseBitOrderChar(snmp_state_buffer, &newState);
       if (SafeMakeOctetString(&dot3adAggPortEntryData.dot3adAggPortActorAdminState, &newState, 1) == L7_TRUE)
...
/*************************************************************
* This function will reverse the bit order for 8 bits (one byte)
* This will not work if more than 8 bits (one byte) is given input.
* input   prevState -  byte to be reversed
* output  newState - byte after reversing the order
****************************************************************/
void snmpReverseBitOrderChar(L7_uchar8 prevState, L7_uchar8* newState)
{
   L7_uint32 i=0;
   ...
   for(i=0; i<8;i++)
   {
     new= new|(((prevState)>>i)&1);
     
     if(i!=7)
       new = new<<1;
   }
   memcpy(newState, &new, sizeof(L7_uchar8));
}
                         ...
                         ContextInfo *contextInfo, int function)
{
  L7_uint32 intIfIndex;
  L7_uchar8 snmp_buffer[SNMP_BUFFER_LEN];
  L7_uchar8 newState[SNMP_BUFFER_LEN];
  ...
  if ( VALID(I_dot3adAggPortActorAdminState, data->valid) )
  {
   ...
   snmpReverseBitOrderChar(*data->dot3adAggPortActorAdminState->octet_ptr, newState);

   memcpy(snmp_buffer, newState, data->dot3adAggPortActorAdminState->length);
