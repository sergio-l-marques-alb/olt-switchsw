                    ...
                    SR_INT32 vrrpOperVrId)
{
  ...
  case I_vrrpOperAcceptMode:
    if (snmpVrrpOperAcceptModeGet(USMDB_UNIT_CURRENT, vrrpOperEntryData.vrrpOperVrId,
                                   intIfNum, &vrrpOperEntryData.vrrpOperAcceptMode) == L7_SUCCESS)
      SET_VALID(I_vrrpOperAcceptMode, vrrpOperEntryData.valid);
    break;
...
k_vrrpOperEntry_set_defaults(doList_t *dp)
{
  ...
  data->vrrpOperAcceptMode = D_vrrpOperAcceptMode_false;
                    ...
                    ContextInfo *contextInfo, int function)
{
  ...
  if (VALID(I_vrrpOperAcceptMode, data->valid) &&
      (snmpVrrpOperAcceptModeSet(USMDB_UNIT_CURRENT, data->vrrpOperVrId,
                                  intIfNum,data->vrrpOperAcceptMode) != L7_SUCCESS))
      return(COMMIT_FAILED_ERROR);
                              ...
                              SR_INT32 vrrpOperVrId)
{
...
#ifdef I_agentRouterVrrpVridDescription
  memcpy(snmp_buffer, SNMP_BUFFER_LEN);
  if (firstTime == L7_TRUE)
  {
   firstTime = L7_FALSE;
   agentRouterVrrpConfiguredEntryData.agentRouterVrrpVridDescription = MakeOctetString(NULL, 0);
  }
#endif
  ...
  switch (nominator)
  {
    ...
    case I_agentRouterVrrpConfiguredPreemptDelay:
      if (snmpVrrpConfigPreemptDelayGet(agentRouterVrrpConfiguredEntryData.vrrpOperVrId,
                                   intIfNum, &agentRouterVrrpConfiguredEntryData.agentRouterVrrpConfiguredPreemptDelay)
                                   == L7_SUCCESS)
        SET_VALID(I_agentRouterVrrpConfiguredPreemptDelay, agentRouterVrrpConfiguredEntryData.valid);
      if(nominator ! = -1)
      break;
#endif
    ...
    case I_agentRouterVrrpTimerLearn:
      if (snmpVrrpConfigTimerLearnGet(agentRouterVrrpConfiguredEntryData.vrrpOperVrId,
                                   intIfNum, &agentRouterVrrpConfiguredEntryData.agentRouterVrrpTimerLearn)
                            == L7_SUCCESS)
        SET_VALID(I_agentRouterVrrpTimerLearn, agentRouterVrrpConfiguredEntryData.valid);
      if(nominator != -1)
      break;
#endif

#ifdef I_agentRouterVrrpVridDescription
    case I_agentRouterVrrpVridDescription:
      if ((usmDbVrrpOperDescriptionGet(agentRouterVrrpConfiguredEntryData.vrrpOperVrId,
                                   intIfNum, snmp_buffer) == L7_SUCCESS) && 
        (SafeMakeOctetStringFromTextExact(&agentRouterVrrpConfiguredEntryData.agentRouterVrrpVridDescription, snmp_buffer) == L7_TRUE))
        SET_VALID(I_agentRouterVrrpVridDescription, agentRouterVrrpConfiguredEntryData.valid);
      break;
#endif
...
int k_agentRouterVrrpConfiguredEntry_test(ObjectInfo *object, ObjectSyntax *value,
                                          doList_t *dp, ContextInfo *contextInfo)
{
  return NO_ERROR;
}

int k_agentRouterVrrpConfiguredEntry_ready(ObjectInfo *object, ObjectSyntax *value,
                                           doList_t *doHead, doList_t *dp)
{
   dp->state = SR_ADD_MODIFY;
   return NO_ERROR;
...
int k_agentRouterVrrpConfiguredEntry_set_defaults(doList_t *dp)
{
  agentRouterVrrpConfiguredEntry_t *data = (agentRouterVrrpConfiguredEntry_t *) (dp->data);

#ifdef I_agentRouterVrrpConfiguredPreemptDelay
  data->agentRouterVrrpConfiguredPreemptDelay = FD_VRRP_DEFAULT_PREEMPT_DELAY;
#endif
#ifdef I_agentRouterVrrpTimerLearn
  data->agentRouterVrrpTimerLearn = D_agentRouterVrrpTimerLearn_disable;   
#endif
#ifdef I_agentRouterVrrpVridDescription  
  if ((data->agentRouterVrrpVridDescription = MakeOctetStringFromText("")) == 0)
  {
    return RESOURCE_UNAVAILABLE_ERROR;
  }
#endif
...
int k_agentRouterVrrpConfiguredEntry_set(agentRouterVrrpConfiguredEntry_t *data,
                                         ContextInfo *contextInfo,
                                         int function)
{
  L7_uint32 intIfNum;
...
#ifdef I_agentRouterVrrpConfiguredPreemptDelay
  if (VALID(I_agentRouterVrrpConfiguredPreemptDelay, data->valid) &&
      (snmpVrrpConfigPreemptDelaySet(data->vrrpOperVrId, intIfNum,
                                     data->agentRouterVrrpConfiguredPreemptDelay) != L7_SUCCESS))
  return(COMMIT_FAILED_ERROR);
#endif

#ifdef I_agentRouterVrrpTimerLearn
  if(VALID(I_agentRouterVrrpTimerLearn, data->valid) &&
      (snmpVrrpConfigTimerLearnset(data->vrrpOperVrId, intIfNum,
                                   data->agentRouterVrrpTimerLearn) != L7_SUCCESS))
  return(COMMIT_FAILED_ERROR);
#endif

#ifdef I_agentRouterVrrpVridDescription  
  if(VALID(I_agentRouterVrrpVridDescription, data->valid)) 
  {
   memcpy(snmp_buffer, SNMP_BUFFER_LEN);
   memcpy(snmp_buffer, data->agentRouterVrrpVridDescription->octet_ptr,
                       data->agentRouterVrrpVridDescription->length);

   if(snmpVrrpConfigDescriptionSet(data->vrrpOperVrId, intIfNum, snmp_buffer) != L7_SUCCESS)
      return(COMMIT_FAILED_ERROR);
  }
#endif 
 return (NO_ERROR); 
                                     ...
                                     SR_INT32        agentRouterVrrpTrackRtPfxLen)
{
  ...
  /* help the agent a little */
  if ((agentRouterVrrpTrackRtPfxLen > 32) &&
      (searchType != EXACT))
  {
    agentRouterVrrpTrackRtPfx++;
    agentRouterVrrpTrackRtPfxLen = 0;
  }
  ...
  switch (nominator)
  {
    ...
    case I_agentRouterVrrpTrackRtPrioDec:
      if (snmpVrrpOperTrackRoutePrioGet(USMDB_UNIT_CURRENT,
            ...
            agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtPfx,
            agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtPfxLen,
            &agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtPrioDec) == L7_SUCCESS)
    ...
    case I_agentRouterVrrpTrackRtReachable:
       if (snmpVrrpOperTrackRoutReachabilityGet(USMDB_UNIT_CURRENT,
             ...
             agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtPfx,
             agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtPfxLen,
             &agentRouterVrrpTrackRouteEntryData.agentRouterVrrpTrackRtReachable) == L7_SUCCESS)
