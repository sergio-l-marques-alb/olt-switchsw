...
SR_UINT32 ipAdEntAddr;
{
  ...
  if ((searchType == EXACT) ? 
      usmDbIpAdEntAddrGet(USMDB_UNIT_CURRENT, ipAddrEntryData.ipAdEntAddr, L7_INVALID_INTF) != L7_SUCCESS :
    (usmDbIpAdEntAddrGet(USMDB_UNIT_CURRENT, ipAddrEntryData.ipAdEntAddr, L7_INVALID_INTF) != L7_SUCCESS &&
...
k_ipRouteEntry_set(ipRouteEntry_t *data, ContextInfo *contextInfo, int function)
{
  ...
  L7_uint32 pref = FD_RTR_RTO_PREF_STATIC, routeFlags = 0;
  ...
  if (usmDbIpRouteEntryGet(USMDB_UNIT_CURRENT, data->ipRouteDest) != L7_SUCCESS)
  {
    ...
    if (data->ipRouteDest == 0)
    {
      if ( !(VALID(I_ipRouteNextHop, data->valid)) )
        return COMMIT_FAILED_ERROR;
      rc = usmDbIpStaticRouteAdd(USMDB_UNIT_CURRENT, 0, 0, data->ipRouteNextHop, pref, 0, L7_RTF_DEFAULT_ROUTE);
    }
    else
    {
      ...
      rc = usmDbIpStaticRouteAdd(USMDB_UNIT_CURRENT, data->ipRouteDest, 
                                 subnetMask, nextHopRtr, cost, 0, 0);
    }
  ...
  */
  if ( (VALID(I_ipRouteType, data->valid)) && (data->ipRouteType == D_ipRouteType_invalid) )
  {
    ...
    if (data->ipRouteDest == 0)
      rc = usmDbIpStaticRouteDelete(USMDB_UNIT_CURRENT, 0, 0, 0, 0, 0);
    else
    {
      ...
      rc = usmDbIpStaticRouteDelete(USMDB_UNIT_CURRENT, data->ipRouteDest,
                                    data->ipRouteMask, data->ipRouteNextHop, 0,
                                    routeFlags);
    }
...
SR_UINT32 ipNetToMediaNetAddress;
{
  ...
  case I_ipNetToMediaPhysAddress:
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    if (usmDbIpNetToMediaPhysAddressGet(USMDB_UNIT_CURRENT, intIfNum, ipNetToMediaEntryData.ipNetToMediaNetAddress,
  ...
  case I_ipNetToMediaType:
    if (usmDbIpNetToMediaTypeGet(USMDB_UNIT_CURRENT, intIfNum, ipNetToMediaEntryData.ipNetToMediaNetAddress,
...
int function;
{
  ...
  if (usmDbIntIfNumFromExtIfNum(data->ipNetToMediaIfIndex, &intIfNum) != L7_SUCCESS)
  {
    return(COMMIT_FAILED_ERROR);
  }

  if (usmDbIpNetToMediaEntryGet(USMDB_UNIT_CURRENT, intIfNum, 
  ...
  if (VALID(I_ipNetToMediaType, data->valid))
  {
    ...
    if (usmDbIpNetToMediaTypeSet(USMDB_UNIT_CURRENT, intIfNum,
  ...
  if (VALID(I_ipNetToMediaPhysAddress, data->valid) &&
      ...
      usmDbIpNetToMediaPhysAddressSet(USMDB_UNIT_CURRENT, intIfNum,
                                      data->ipNetToMediaNetAddress,
...
int function;
{
  ...
  {
    tcpEntryToSet.tcpConnState = (long) IPSTK_SNMP_TCPCONN_DELETE_TCB;
