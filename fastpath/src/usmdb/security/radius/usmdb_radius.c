...
*
* @param   *serverName @b((input)) Name of the RADIUS server.
* @param    radiusServerType_t type @b((input)) Auth/Acct.
*
* @returns L7_SUCCESS - Valid server name
* @returns L7_FAILURE - Invalid server name
*
* @comments None
*
* @end
*
*********************************************************************/
L7_RC_t usmDbRadiusNamedAcctServerGet(L7_char8 *serverName, radiusServerType_t type)
{
  if (type == RADIUS_SERVER_TYPE_AUTH)
  {
    return radiusNamedServerGet(serverName);
  }
  else if (type == RADIUS_SERVER_TYPE_ACCT)
  {
    return radiusNamedAcctServerGet(serverName);
  }
  return L7_FAILURE;
}
