...
/* router-alert header */
    ...
    to4.sin_port = tsa->sin_port;
        ...
        memcpy(&(pktinfo_in->ipi_spec_dst.s_addr), &((L7_sockaddr_in_t *)from)->sin_addr.s_addr, L7_IP_ADDR_LEN);
    ...
    if ((count = sendmsg (fd, &msg, flags)) < 0)
    {
        ...
        L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID,
                "OSAPI: sendmsg() failed for fd=%d, intf=%d, %s."
                " A component has attempted to send a UDP packet with"
                " special options (hop count, etc.) set and failed.", fd, intIfNum, ifName);
        return L7_ERROR;
    }
    *bytesSent = count;
    return(L7_SUCCESS);
...
*/
  ...
  else if(level == IPPROTO_IPV6)
  {
     ...
     case L7_IPV6_ADD_MEMBERSHIP:
          ...
          {
            /* If interface number is not given as input, check if the interface name
             * is given. If so, get the stack interface index by querying the stack
             * before passing it on in setSocketOpt */
            if (0 != l7mreq6->imr6_ifName[0])
            {
              osapiStrncpySafe(ifr.ifr_name, l7mreq6->imr6_ifName, sizeof(ifr.ifr_name));
              sd = socket(AF_INET, SOCK_DGRAM, 0);
              if (sd < 0)
              {
                L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID,
                        "Failed to create socket on %s. Error = %s\n", l7mreq6->imr6_ifName, strerror(errno));
                return L7_FAILURE;
              }
              ...
              {
                L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_OSAPI_COMPONENT_ID,
                        "Failed to get interface index on %s. Error = %s", l7mreq6->imr6_ifName, strerror(errno));
                close(sd);
                return L7_FAILURE;
              }
...
L7_RC_t osapiIfNameStringGet(L7_uint32 intIfNum, L7_uchar8 *buf, L7_uint32 buflen)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSAPI_COMPONENT_ID,
          "intf %d, %s: Could not get type, rc = %d\n", intIfNum, ifName, rc);
  return L7_FAILURE;
  }
  ...
  if (itype == L7_LOOPBACK_INTF)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSAPI_COMPONENT_ID,
              "intf %d, %s: Could not get configId, rc = %d\n", intIfNum, ifName, rc);
      return L7_FAILURE;
    }
  ...
  else
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSAPI_COMPONENT_ID,
              "intf %d, %s: Could not get USP, rc = %d\n", intIfNum, ifName, rc);
      return L7_FAILURE;
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSAPI_COMPONENT_ID,
            "intf %d, %s: Interface name %s invalid\n", intIfNum, ifName, tmpBuf);
    return L7_FAILURE;
  }
