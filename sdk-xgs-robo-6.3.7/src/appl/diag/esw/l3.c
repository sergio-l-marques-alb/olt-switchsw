/*
 * $Id: l3.c 1.105.6.1 Broadcom SDK $
...
_l3_cmd_route_print(int unit, int index, bcm_l3_route_t *info, void *cookie)
{
    ...
    if (info->l3a_flags & BCM_L3_ROUTE_LABEL) {
        printk(" (MPLS)%d", info->l3a_mpls_label);
    } else if (info->l3a_tunnel_option) {
        printk(" (UDP tunnel) %d", info->l3a_tunnel_option);
    } 
    printk("\n");
    ...
    case L3_SHOW:
        ...
        if ((SOC_IS_TRIDENT2(unit) || SOC_IS_TRIUMPH3(unit)) 
            ...
            if ((r = bcm_switch_object_count_get(unit, 
                     ...
                     &defip_64_max)) < 0) { 
                printk("%s: Error getting max number of 64bv6: %s\n", 
                       ARG_CMD(a), bcm_errmsg(r));
                return CMD_FAIL;         
            }
            if ((r = bcm_switch_object_count_get(unit, 
                     ...
                     &defip_128_max)) < 0) {
                printk("%s: Error getting max number of 128bv6: %s\n", 
                       ARG_CMD(a), bcm_errmsg(r));
                return CMD_FAIL;         
            }
            ...
            if ((r = bcm_switch_object_count_get(unit, 
                     ...
                     &defip_64_free)) < 0) {
                printk("%s: Error getting free number of 64bv6: %s\n", 
                       ARG_CMD(a), bcm_errmsg(r));
                return CMD_FAIL;         
            }
            if ((r = bcm_switch_object_count_get(unit, 
                     ...
                     &defip_128_free)) < 0) {
                printk("%s: Error getting free number of 128bv6: %s\n", 
                       ARG_CMD(a), bcm_errmsg(r));
                return CMD_FAIL;         
            }                
