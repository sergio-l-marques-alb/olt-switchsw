...
/* Static functions. */
static void _bcm_ptp_modular_supervisor(
    ...
    uint32 flags)
{
    ...
    /* Cancel prior modular system supervisor DPC process (if any). */
    if (BCM_FAILURE(rv = _bcm_ptp_modular_shutdown(unit, ptp_id, clock_num))) {
        PTP_ERROR_FUNC("_bcm_ptp_modular_shutdown()");
        return rv;
    }
    return BCM_E_NONE;
}
...
_bcm_ptp_modular_shutdown(
    ...
    int clock_num)
{
    /* Cancel modular system supervisor delayed procedure call. */
    sal_dpc_cancel(INT_TO_PTR(&_bcm_ptp_modular_supervisor));
    return BCM_E_NONE;
    ...
    uint32 flags)
{
     ...
     * NB: Cancel prior modular system supervisor DPC process (if any).
     */
    sal_dpc_cancel(INT_TO_PTR(&_bcm_ptp_modular_supervisor));
    if (enable) {
        sal_dpc_time(PTP_MODULAR_SUPERVISOR_DPC_TIME_USEC_DEFAULT,
                     &_bcm_ptp_modular_supervisor, INT_TO_PTR(&_bcm_ptp_modular_supervisor),
 ...
 *      _bcm_ptp_modular_supervisor
 * Purpose:
 *      Run modular system supervisor (as DPC).
 * Parameters:
 *      owner         - (IN) DPC owner.
    ...
    void *unused)
{
    ...
    sal_usecs_t dpc_recall_usec = PTP_MODULAR_SUPERVISOR_DPC_TIME_USEC_DEFAULT;

    if (0 == modular_options.phyts) {
        /* IDLE. PHY timestamping option is not currently set. */
        dpc_recall_usec = PTP_MODULAR_SUPERVISOR_DPC_TIME_USEC_IDLE;
        goto modular_dpc_recall;
    }

    if (BCM_FAILURE(rv = _bcm_ptp_modular_physync_synchronize(unit, ptp_id, clock_num))) {
...
modular_dpc_recall:
    sal_dpc_time(dpc_recall_usec, &_bcm_ptp_modular_supervisor,
                 INT_TO_PTR(&_bcm_ptp_modular_supervisor),
