...
_bcm_xgs3_tx_pipe_bypass_header_setup(int unit, bcm_pkt_t *pkt)
{
    ...
    if ((SOC_DCB_TYPE(unit) == 23) || (SOC_DCB_TYPE(unit) == 26) || (SOC_DCB_TYPE(unit) == 29) ||(SOC_DCB_TYPE(unit) == 30) )  {
...
_bcm_tx(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    if (soc_cm_debug_check(DK_TX)) {
        ...
        {
            uint16 i;
            soc_cm_debug(DK_TX, "bcm_tx: packet: ");
            for (i=0; i<pkt->pkt_data[0].len ; i++) {
                soc_cm_debug(DK_TX, "%.2x",pkt->pkt_data[0].data[i]);
            }
            soc_cm_debug(DK_TX, "\n");
        }
    }
    ...
    if (!BCM_IS_LOCAL(unit)) { /* Tunnel the packet to the remote CPU */
        ...
        if (pkt->call_back != NULL && cookie != NULL) {
            soc_cm_debug(DK_ERR, "bcm_tx ERROR:  "
	...
	if ((pkt->flags & BCM_TX_NO_PAD) && (dv->dv_vcnt > 0)) {
            ...
            if (SOC_IS_XGS3_SWITCH(unit)) {
                if ((BCM_PKT_NO_VLAN_TAG(pkt) && bytes < 60) ||
                    ...
                    soc_cm_debug(DK_ERR, "bcm_tx: Discarding %s runt packet %s higig header %d\n", 
             ...
             void *cookie)
{
    ...
    if (!BCM_IS_LOCAL(unit)) { /* Tunnel the packet to the remote CPU */
        soc_cm_debug(DK_ERR, "bcm_tx_array ERROR:  Cannot tunnel\n");
        return BCM_E_PARAM;
    }
...
bcm_common_tx_list(int unit, bcm_pkt_t *pkt, bcm_pkt_cb_f all_done_cb, void *cookie)
{
    ...
    if (!BCM_IS_LOCAL(unit)) { /* Tunnel the packet to the remote CPU */
        soc_cm_debug(DK_ERR, "bcm_tx_list ERROR:  Cannot tunnel\n");
        return BCM_E_PARAM;
    }
    ...
    if (dv->dv_vcnt > 0) {
        ...
        if (all_done_cb != NULL) {
            pkt_ptr = TX_INFO(dv)->pkt[0];
            if (pkt_ptr == NULL) {
                soc_cm_debug(DK_VERBOSE, "%s: NULL pkt\n", FUNCTION_NAME());
                pkt_ptr = pkt;
            } 
            (all_done_cb)(unit, (bcm_pkt_t *)pkt_ptr, cookie);
        }
...
_bcm_tx_chain_send(int unit, dv_t *dv, int async)
{
    ...
    if (!soc_cm_debug_check(DK_DMA) &&
            soc_cm_debug_check(DK_VERBOSE | DK_TX | DK_PACKET)) {
    ...
    if (async) { /* Send packet(s) asynchronously */
        ...
        soc_cm_debug(DK_TX, "bcm_tx: Sent synchronously.\n");
             ...
             bcm_pkt_cb_f chain_done_cb, void *cookie, int per_pkt_cb)
{
    ...
    dv_info->pkt = soc_at_alloc(unit, (sizeof(bcm_pkt_t *) * pkt_count), "tx_dv pkt");
    if (dv_info->pkt == NULL) {
        soc_dma_dv_free(unit, dv);
        soc_at_free(unit, dv_info);
        return NULL;
    }
    sal_memset(dv_info->pkt, 0, (sizeof(bcm_pkt_t *) * pkt_count));
...
_tx_dv_free(int unit, dv_t *dv)
{
    if (dv) {
        if (TX_INFO(dv)) {
            if (TX_INFO(dv)->pkt) {
                soc_at_free(unit, TX_INFO(dv)->pkt);
            }
            soc_at_free(unit, TX_INFO(dv));
        }
        soc_dma_dv_free(unit, dv);
    }
}
...
_soc_tx_pkt_setup(int unit, bcm_pkt_t *pkt, soc_tx_param_t *tx_param)
{
    ...
    if ((tx_param->pfm & (~PFM_MASK)) != 0) {
        soc_cm_debug(DK_WARN, "bcm_tx: PFM too big, truncating\n");
...
_tx_pkt_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    if (pkt->pkt_data[0].len < sizeof(bcm_mac_t)) {
         soc_cm_debug(DK_TX, "_tx_pkt_desc_add: pkt->pkt_data[0].len < sizeof(bcm_mac_t) exit ");
        return BCM_E_PARAM;
    }
    ...
    if (block_offset >= pkt->blk_count) {
        soc_cm_debug(DK_TX, "_tx_pkt_desc_add: block_offset >= pkt->blk_count exit ");
        return BCM_E_PARAM;
    }
 ...
 *   _bcm_tx_chain_done:      Handles the dv_done in non-interrupt
...
bcm_common_tx_dv_dump(int unit, void *dv_p)
{
    ...
    if (dv_info != NULL) {
        ...
        soc_cm_print("    cookie %p. cb pointer unprintable\n",
                     (void *)dv_info->cookie);
