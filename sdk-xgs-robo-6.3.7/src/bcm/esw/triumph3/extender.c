/*
 * $Id: extender.c 1.10 Broadcom SDK $
        ...
        int vp, int drop, int *nh_index)
{
    ...
    if (BCM_SUCCESS(_bcm_esw_failover_extender_check(unit, extender_port))) {
        ...
        rv = _bcm_esw_failover_prot_nhi_create(unit, 
                                               protection_flags, 
                                               *nh_index, 
                                               failover_nh_index, 
...
bcm_tr3_extender_sw_dump(int unit)
{
    ...
    for (i = 0; i < num_vp; i++) {
        ...
        soc_cm_print("QOS Map Id = 0x%x\n", 
                EXTENDER_PORT_INFO(unit, i)->qos_map_id);
