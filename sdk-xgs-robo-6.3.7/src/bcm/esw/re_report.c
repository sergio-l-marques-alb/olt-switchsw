/*
 * $Id: re_report.c 1.25 Broadcom SDK $
                         ...
                         int *pending, uint32 *pdir)
{
    ...
    *pending = 0;
    ...
    if ((type == _REGEX_SESSION_KEY_TYPE_IPV4) ||
        ...
        *pdir = dir;
        BCM_DEBUG(BCM_DBG_REGEX, (" KEY_DIRECTION = 0x%x\n", dir));
        BCM_DEBUG(BCM_DBG_REGEX, (" KEY_TYPE = 0x%x\n", type));
        ...
        BCM_DEBUG(BCM_DBG_REGEX, (" FLOW_STATE = 0x%03x\n", val));

        soc_mem_field_get(unit, mem, entry, KEYf, pk);
        p8 = (uint8*) pk;
        BCM_DEBUG(BCM_DBG_REGEX, ("pk = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
        ...
        if (type == _REGEX_SESSION_KEY_TYPE_IPV4) {
            ...
            BCM_DEBUG(BCM_DBG_REGEX, (" Get upper address, direction = %d...\n", dir));
            *pending = 1;
            data->dip = 0;
            data->sip = 0;
            data->src_port = 0;
        ...
        */
        ...
        type = soc_mem_field32_get(unit, mem, entry, KEY_TYPEf);
        BCM_DEBUG(BCM_DBG_REGEX, (" KEY_TYPE = 0x%x\n", type));
        BCM_DEBUG(BCM_DBG_REGEX, (" Get lower address, direction = %d, ports, proto...\n", dir));

        soc_mem_field_get(unit, mem, entry, IPV6_LOWER_KEYf, pkl);
        p8 = (uint8*) pkl;
        BCM_DEBUG(BCM_DBG_REGEX, ("pkl = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
                     *p8, *(p8+1), *(p8+2), *(p8+3), *(p8+4), *(p8+5), *(p8+6), *(p8+7), *(p8+8), *(p8+9), *(p8+10), *(p8+11), *(p8+12),
        ...
        BCM_DEBUG(BCM_DBG_REGEX, ("p8 = %p, p8+port/proto = %p\n", p8, p8+_REGEX_PROTOCOL_PORT_LENGTH_V6/8));
        p8 += _REGEX_PROTOCOL_PORT_LENGTH_V6/8; /* skip past ports/proto */

        BCM_DEBUG(BCM_DBG_REGEX, ("pkl+port/proto = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
                     *p8, *(p8+1), *(p8+2), *(p8+3), *(p8+4), *(p8+5), *(p8+6), *(p8+7), *(p8+8), *(p8+9), *(p8+10), *(p8+11), *(p8+12),
    ...
    BCM_DEBUG(BCM_DBG_REGEX, ("Flags :0x%x\n",data->flags));
    BCM_DEBUG(BCM_DBG_REGEX, ("Match Flags :0x%x\n",data->match_flags));
