...
#define BCM_INNER_TPID_ENABLE   0x1
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_RAVEN_SUPPORT) || defined(BCM_HURRICANE_SUPPORT)
    if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || SOC_IS_HAWKEYE(unit) || SOC_IS_HURRICANEX(unit)) {
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_RAPTOR_SUPPORT) || defined(BCM_SCORPION_SUPPORT) || defined(BCM_HURRICANE_SUPPORT)
        if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || 
            SOC_IS_HAWKEYE(unit) || SOC_IS_SC_CQ(unit) || SOC_IS_HURRICANEX(unit)) {
            uint32 miscval;
...
_bcm_fb2_port_tpid_set(int unit, bcm_port_t port, uint16 tpid)
{
    ...
    if (SOC_IS_HURRICANE2(unit)) {
        soc_PORT_TABm_field32_set(unit, &ptab, INNER_TPID_ENABLEf, BCM_INNER_TPID_ENABLE);
        soc_reg_field32_modify(unit, EGR_VLAN_CONTROL_1r, port,
                       INNER_TPID_ENABLEf, BCM_INNER_TPID_ENABLE);
