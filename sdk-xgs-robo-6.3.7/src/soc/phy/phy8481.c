/*
 * $Id: phy8481.c 1.252 Broadcom SDK $
...
#define PHY_IS_BCM8483_PLUS(_pc)\
                                  (PHY_IS_BCM8483X((_pc)) ||\
                                   PHY_IS_BCM8484X((_pc)) || PHY_IS_BCM8485X((_pc)) )
...
phy_8481_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) ||
        ...
        if (SOC_WARM_BOOT(unit) || SOC_IS_RELOADING(unit)){
            ...
            if (PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_CAPABLE) && PHY_IS_BCM8483_PLUS(pc)) {
                SOC_IF_ERROR_RETURN
        ...
        if (SOC_WARM_BOOT(unit) || SOC_IS_RELOADING(unit)){
            if (PHY_FLAGS_TST(unit, port, PHY_FLAGS_EEE_CAPABLE) && PHY_IS_BCM8483_PLUS(pc)) {
                ...
                if (!args[0]) {
                    ...
                    SOC_DEBUG_PRINT((DK_PHY,"u=%d p=%d recovered EEE None\n", unit, port));
                } else if (args[0] == 1) {
                    PHY_FLAGS_SET(unit, pc->port, PHY_FLAGS_EEE_ENABLED);
                    PHY_FLAGS_CLR(unit, pc->port, PHY_FLAGS_EEE_MODE);
                    SOC_DEBUG_PRINT((DK_PHY,"u=%d p=%d recovered EEE Native\n", unit, port));
                } else if (args[0] == 2) {
                    PHY_FLAGS_SET(unit, pc->port, PHY_FLAGS_EEE_ENABLED);
                    PHY_FLAGS_SET(unit, pc->port, PHY_FLAGS_EEE_MODE);
                    SOC_DEBUG_PRINT((DK_PHY,"u=%d p=%d recovered EEE AutogrEEEn\n", unit, port));
                }
            }
            PHYCTRL_INIT_STATE_SET(pc,PHYCTRL_INIT_STATE_DEFAULT);
...
_phy84834_top_level_cmd_get_dispatch_v2(int unit,phy_ctrl_t *pc, uint16 cmd)
{
    ...
    if ((status & PHY84834_CMD_OPEN_FOR_CMDS) == 0) {
        soc_cm_print(
                         "PHY84834_TOP_LEVEL_CMD_GET (dispatch) failed:<1> u=%d p=%d cmd=%04x status=%04x\n", unit, pc->port, cmd, status);
        return SOC_E_FAIL;
    }
    SOC_IF_ERROR_RETURN
        (WRITE_PHY84834_TOP_CONFIG_SCRATCH_0r(unit, pc, cmd));
                                ...
                                soc_phy_control_t type, uint32 *value)
{
    phy_ctrl_t    *pc;       /* PHY software state */
    uint16 temp, temp1;
    uint16 digital_temp = 0, analog_temp = 0;
    int rv = SOC_E_NONE;
...
#ifdef PHY8481_MEASSURE_DOWNLOAD_TIME
    soc_cm_print("u=%d p=%d raw download took %u us\n", unit, pc->port, SAL_USECS_SUB(sal_time_usecs(), t));
#endif
        ...
        case PHYCTRL_UCODE_BCST_LOAD:
            ...
            if(temp1 != 0xffff) {
                if((temp1 != pc->phy_id0) || (temp2 != pc->phy_id1)) {
                    SOC_DEBUG_PRINT((DK_PHY,"Another PHY already exist on same boradcast MDIO address.\n")) ;
                }
            }
