/*
 * $Id: phy5482robo.c 1.29 Broadcom SDK $
...
_robo_phy_5482_no_reset_setup(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_robo_phy_5482_no_reset_setup: u=%d p=%d medium=%s\n",
                     unit, port,
    ...
    SOC_DEBUG_PRINT((DK_PHY, "%s: u=%d p=%d Pri-SerDes PowerDown=%d!\n",
    ...
    if (PHY_SECONDARY_SERDES_MODE(unit, port)){
        ...
        SOC_DEBUG_PRINT((DK_PHY, "%s: u=%d p=%d Sec-SerDes PowerDown=%d!\n",
                        FUNCTION_NAME(), unit, port, 1));
        SOC_IF_ERROR_RETURN
...
robo_phy_5482_init(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "robo_phy_5482_init: u=%d p=%d\n",
                     unit, port));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_5482_init: "
...
robo_phy_5482_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (pc->automedium || PHY_COPPER_MODE(unit, port)) {
        if (pc->copper.enable){
            ...
            SOC_DEBUG_PRINT((DK_PHY, "phy_5482_enable_set: "
    ...
    if (pc->automedium || PHY_FIBER_MODE(unit, port)) {
        ...
        if (pc->fiber.enable) {        
            ...
            SOC_DEBUG_PRINT((DK_PHY, "robo_phy_5482_enable_set: "
