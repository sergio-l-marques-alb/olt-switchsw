...
/*
 * $Id: 4acab927f421202dad5a427c67957a62ada2d8f1 $
 *
...
_phy84756_init_ucode_bcst(int unit, int port, uint32 cmd)
{
    ...
    if (cmd == PHYCTRL_UCODE_BCST_SETUP) {
        ...
        if (bcst_fc_fw) 
        {
            SOC_DEBUG_PRINT((DK_PHY, "uc_firmware_bcst,device name %s: u=%d p=%d\n",
                             pc->dev_name? pc->dev_name: "NULL", unit, port));
        ...
        /* read Rev-ID */
                ...
                SOC_DEBUG_PRINT((DK_PHY, "BCM84756 u=%d p=%d: MDIO broadcast download, EDC firmware version=%x\n",
                             unit, port, data16));
            } 
        }
        /* restore the original phy_id */
        ...
        if (bcst_fc_fw) 
        {
            SOC_DEBUG_PRINT((DK_PHY,"BCM84756 u=%d p=%d: FC firmware MDIO broadcast download\n", 
                         unit, port));
            if (fc_extra_delay) 
            {
