                      ...
                      soc_field_t field, uint32 *fldbuf)
{
    ...
    if (!_soc_field_value_fit(fieldinfo, fldbuf)) {
#if defined(BCM_ESW_SUPPORT) && !defined(SOC_NO_NAMES)
        soc_cm_print("mem %s field %s value does not fit\n",
                     soc_mem_name[mem], soc_fieldnames[field]);
#endif
        assert(_soc_field_value_fit(fieldinfo, fldbuf)); 
    }
                  ...
                  soc_field_t field, uint32 *fldbuf)
{
    ...
    return soc_meminfo_field_get(mem, meminfo, entbuf, field, fldbuf);
}
                       ...
                       soc_field_t field, uint32 *fldbuf)
{
    ...
    rfldbuf = soc_meminfo_field_get(mem, meminfo, entbuf, field, fldbuf);
                                ...
                                uint32 *dest, uint32 *src[4], uint8 conv_type)
{
    ...
    case TYPE_1_TO_TYPE_4:
        ...
        return;
    default:
        soc_cm_print("Unimplemented convertion type: %d\n", conv_type);
        assert(0);
    }
}
#endif
...
soc_mem_addr(int unit, soc_mem_t mem, unsigned array_index, int blk, int index)
{
...
#endif
    if (((blk>=32)?(mip->blocks_hi & (1 << (blk&0x1F))):(mip->blocks & (1 << blk))) && !(mip->flags & SOC_MEM_FLAG_MONOLITH)) {
    ...
    if (array_index) {
        ...
        soc_cm_debug(DK_MEM, "addr: %x, mip->base: %x, blkoff: %x, "
                 ...
                 int index, uint8 *acc_type)
{
    ...
    *acc_type = (soc_mem_flags(unit, mem) >> SOC_MEM_FLAG_ACCSHIFT) & 7;
#if defined(BCM_TRIUMPH_SUPPORT)
    if (soc_feature(unit, soc_feature_esm_support)) {
    ...
    if (array_index) {
        ...
        assert(maip);                       /* verify this is memory array information exists */
        assert(array_index < maip->numels); /* verify that the array index is in range */
        soc_cm_debug(DK_MEM, "addr: %x, mip->base: %x, block: %x, "
                     "index = %d, mip->gran: %d, * = %x, arr_in = %u, skip = %u\n", 
...
soc_addr_to_mem(int unit, uint32 address, uint32 *mem_block)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (soc_mem_is_valid(unit, mem) &&
            ...
            if (soc_feature(unit, soc_feature_two_ingress_pipes)) {
                min_addr = max_addr = max_addr & ~ACC_TYPE_MASK;
            }
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (soc_mem_is_valid(unit, mem) &&
            ...
            if (SOC_IS_XGS3_SWITCH(unit) || SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit)) {
                ...
                if ((acc_type >= 0) && 
                    (acc_type != ((SOC_MEM_INFO(unit, mem).flags >>
    ...
    if (soc_mem_is_valid(unit, mem) &&
        ...
        if (!SOC_MEM_IS_ARRAY(unit, mem)) {
            ...
            if((address < (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) || 
                ...
                *numels = (address - (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) / \
                          (SOC_MEM_INFO(unit, mem).index_max - SOC_MEM_INFO(unit, mem).index_min);
                *index  = (address - (SOC_MEM_INFO(unit, mem).base + SOC_MEM_INFO(unit, mem).index_min)) % \
                          (SOC_MEM_INFO(unit, mem).index_max - SOC_MEM_INFO(unit, mem).index_min);
                return SOC_E_NONE;
            }
        }
