/*
 * $Id: dma.c 1.64.6.1 Broadcom SDK $
...
soc_dma_abort_channel(int unit, dma_chan_t channel)
{
    ...
    SOC_DEBUG_PRINT((DK_DMA, "soc_dma_abort_channel: c=%d\n", channel));
...
soc_dma_abort_dv(int unit, dv_t *dv_chain)
{
    ...
    /*
     * Check if active operation. _soc_dma_start_channel will update the
    ...
    if (sc->sc_q == NULL) {
        ...
        _soc_dma_start_channel(unit, sc); /* Try to start next operation */
    } else {
        dv_t    *dv;
        /*
                   ...
                   int *ether_offset)
{
    ...
    if (soc_higig_field_get(unit, xgh, HG_hgi) == SOC_HIGIG_HGI) {
        ...
        /* BCM5632 format */
        soc_cm_print("%sBCM5632 Frame:"
                     " len=%d (header=%d payload=%d)\n",
                     pfx, len, SOC_BCM5632_HDR_SIZE,
                     len - SOC_BCM5632_HDR_SIZE);
        soc_cm_print("%s0x%02x%02x%02x%02x <D_PORTID=%d>\n",
                     pfx,
        ...
        soc_cm_print("%s0x%02x%02x%02x%02x <S_PORTID=%d> "
                     "<LEN=%d> START=<0x%x>\n",
                     pfx,
...
soc_dma_dump_dv(int unit, char *pfx, dv_t *dv_chain)
{
    ...
    for (dv = dv_chain; dv; dv = dv->dv_chain) {
        ...
        if (dv_chain->dv_flags & DV_F_NOTIFY_DSC)
            sal_strcpy(tmps + sal_strlen(tmps), "notify-dsc ");
        if (dv_chain->dv_flags & DV_F_NOTIFY_CHN)
            sal_strcpy(tmps + sal_strlen(tmps), "notify-chn ");
        if (dv_chain->dv_done_packet)
            sal_strcpy(tmps + sal_strlen(tmps), "notify-pkt ");
        if (dv_chain->dv_flags & DV_F_COMBINE_DCB)
            sal_strcpy(tmps + sal_strlen(tmps), "combine-dcb ");

        soc_cm_print("%sdv@%p unit %d dcbtype-%d op=%s vcnt=%d dcnt=%d "
                     "cnt=%d\n",
        ...
        soc_cm_print("%s    chan=%s chain=%p flags=0x%x-->%s\n",
                     pfx, chan_str,
                     (void *)dv->dv_chain,
                     dv->dv_flags, tmps);
        ...
        if (dv->tx_param.flags != 0) {
            ...
            soc_cm_print("%s    tx-param flags 0x%x cos %d sp.sm %d.%d\n",
...
soc_dma_chan_config(int unit, dma_chan_t c, dvt_t type, uint32 flags)
{
    ...
    SOC_DEBUG_PRINT((DK_DMA, "soc_dma_chan_config: c=%d type=%d\n", c, type));
...
_soc_dma_free_list(int unit)
{
    ...
    SOC_DMA_LOCK(soc_dma_lock);
    soc->soc_dv_free_cnt = 0;           /* Mark no free DVs on list */
    dv = soc->soc_dv_free;              /* Pick up free list */
    soc->soc_dv_free = NULL;            /* NULL list off soc structure */
    SOC_DMA_UNLOCK(soc_dma_lock);
...
{
    _soc_dma_free_list(unit);           /* Free current DV list */
...
static struct {
    ...
    int psize;
} _soc_dma_rom_control[SOC_MAX_NUM_DEVICES];
...
static int
_soc_dma_rom_dv_start_polled(int unit, int channel,
...
static int
_soc_dma_rom_rx_start(int unit)
{
    dv_t                *dv;
    int                 rv;
    ...
    if ((dv = _soc_dma_rom_control[unit].rx.dv) == NULL) {
        return SOC_E_INIT;
    }
    ...
    dv->dv_dcb = soc_dma_rom_dcb_alloc(unit,
                                       _soc_dma_rom_control[unit].psize);

    /* Bring it on */

    if ((rv = _soc_dma_rom_dv_start_polled(unit, SOC_DMA_ROM_RX_CHANNEL, dv,
                           &_soc_dma_rom_control[unit].rx.done)) < 0) {
        /* D'oh */
...
soc_dma_rom_init(int unit, int max_packet_rx, int tx, int rx)
{
    ...
    sal_memset(&_soc_dma_rom_control[unit], 0,
               sizeof(_soc_dma_rom_control[unit]));
    ...
    _soc_dma_rom_control[unit].psize = max_packet_rx;
    ...
    _soc_dma_rom_control[unit].rx.dv = dv_rx;
    _soc_dma_rom_control[unit].tx.dv = dv_tx;

    /* Startup an RX */

    if (rx) {
        _soc_dma_rom_rx_start(unit);
    }

    /* No TX's outstanding */

    _soc_dma_rom_control[unit].tx.done = 1;
...
soc_dma_rom_detach(int unit)
{
    ...
    dv = _soc_dma_rom_control[unit].rx.dv;
    ...
    dv = _soc_dma_rom_control[unit].tx.dv;
...
soc_dma_rom_tx_start(int unit, dcb_t *dcb)
{
    ...
    if ((dv = _soc_dma_rom_control[unit].tx.dv) == NULL) {
        return SOC_E_INIT;
    }
    ...
    _soc_dma_rom_control[unit].tx.done = 0;

    return _soc_dma_rom_dv_start_polled(unit, SOC_DMA_ROM_TX_CHANNEL,
                                        dv,
                                        &_soc_dma_rom_control[unit].tx.done);
}
...
soc_dma_rom_tx_poll(int unit, int *done)
{
    ...
    *done = _soc_dma_rom_control[unit].tx.done;
...
{
    if (_soc_dma_rom_control[unit].tx.done == 0) {
        _soc_dma_rom_control[unit].tx.done = 1;
...
soc_dma_rom_rx_poll(int unit, dcb_t **dcb)
{
    ...
    if (_soc_dma_rom_control[unit].rx.done) {
        /* Packet received.  Pull out the DCB */

        *dcb = _soc_dma_rom_control[unit].rx.dv->dv_dcb;
        _soc_dma_rom_control[unit].rx.dv->dv_dcb = NULL;
        _soc_dma_rom_control[unit].rx.done = 0;
