/*
 * $Id: 894683766d2fedced5f102dd9558560775fe5555 $
...
soc_ser_log_print_tlv(void *buffer)
{
    ...
    soc_cm_debug(DK_ERR, "Tlv Header:\n");
    soc_cm_debug(DK_ERR, "\ttype: %d\n", (int)(tlv_hdr->type));
    soc_cm_debug(DK_ERR, "\tlength: %d\n", (int)(tlv_hdr->length));
    soc_cm_debug(DK_ERR, "\tvalue: \n");
    soc_cm_debug(DK_ERR, "Tlv Data:\n");
    switch (tlv_hdr->type) {
    case SOC_SER_LOG_TLV_MEMORY:
        soc_cm_debug(DK_ERR, "\tSOC_SER_LOG_TLV_MEMORY\n");
        soc_cm_debug(DK_ERR, "\t\tmemory: %d\n",
            (int)(((soc_ser_log_tlv_memory_t*)(data))->memory));
        soc_cm_debug(DK_ERR, "\t\tindex: %d\n",
            (int)(((soc_ser_log_tlv_memory_t*)(data))->index));
        break;
    case SOC_SER_LOG_TLV_REGISTER:
        soc_cm_debug(DK_ERR, "\tSOC_SER_LOG_TLV_REGISTER\n");
        soc_cm_debug(DK_ERR, "\t\tregister: %d\n",
            (int)(((soc_ser_log_tlv_register_t*)(data))->reg));
        soc_cm_debug(DK_ERR, "\t\tindex: %d\n",
            (int)(((soc_ser_log_tlv_register_t*)(data))->index));
        soc_cm_debug(DK_ERR, "\t\tport: %d\n",
            (int)(((soc_ser_log_tlv_register_t*)(data))->port));
        break;
    case SOC_SER_LOG_TLV_CONTENTS:
        soc_cm_debug(DK_ERR, "\tSOC_SER_LOG_TLV_CONTENTS\n\t\t");
        for (index = 0; index < tlv_hdr->length; index++) {
            soc_cm_debug(DK_ERR, "%02x ", *((char*)(data + index)));
            if ((index%16)==15) {
                soc_cm_debug(DK_ERR, "\n\t\t");
            }
        }
    ...
    case SOC_SER_LOG_TLV_SER_FIFO:
        soc_cm_debug(DK_ERR, "\tSOC_SER_LOG_TLV_SER_FIFO\n\t\t");
        for (index = 0; index < tlv_hdr->length; index++) {
            soc_cm_debug(DK_ERR, "%02x ", *((char*)(data + index)));
            if ((index%16)==15) {
                soc_cm_debug(DK_ERR, "\n\t\t");
    ...
    case SOC_SER_LOG_TLV_CACHE:
        soc_cm_debug(DK_ERR, "\tSOC_SER_LOG_TLV_CACHE\n\t\t");
        for (index = 0; index < tlv_hdr->length; index++) {
            soc_cm_debug(DK_ERR, "%02x ", *((char*)(data + index)));
            if ((index%16)==15) {
                soc_cm_debug(DK_ERR, "\n\t\t");
...
soc_ser_log_print_all(int unit)
{
    ...
    do {
        ...
        soc_cm_debug(DK_ERR, "Log Entry ID:%d\n", id);
...
soc_ser_log_init(int unit, void *location, int size) 
{
    ...
    if (mut == NULL) {
        soc_cm_debug(DK_ERR, "SER Logging failed to create mutex\n");
        return SOC_E_RESOURCE;
    }
