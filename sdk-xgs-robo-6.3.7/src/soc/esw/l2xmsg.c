/*
 * $Id: l2xmsg.c 1.49.6.2 Broadcom SDK $
...
{                                                           \
    ...
    soc_cm_debug(DK_ARL | DK_VERBOSE, "set_entry_callback: u:%d i=%d\n", \
                _u_, _index_);                             \
}
...
{                                                           \
    ...
    soc_cm_debug(DK_ARL | DK_VERBOSE, "set_entry_deleted: u:%d i=%d\n",  \
                _u_, _index_);                             \
}
...
_soc_l2x_thread(void *unit_vp)
{
    ...
    if (shadow_tab == NULL || chunk_buf == NULL || 
        ...
        soc_cm_debug(DK_ERR, "AbnormalThreadExit:soc_l2x_thread: not enough memory \n");
        soc_event_generate(unit, SOC_SWITCH_EVENT_THREAD_ERROR, 
