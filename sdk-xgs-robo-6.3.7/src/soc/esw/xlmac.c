/*
 * $Id: xlmac.c 1.34.2.1 Broadcom SDK $
...
STATIC int
_mac_xl_drain_cells(int unit, soc_port_t port, int notify_phy)
{
    int         rv;
...
mac_xl_egress_queue_drain(int unit, soc_port_t port)
{
    ...
    DBG_10G_VERB(("mac_xl_egress_queue_drain: unit %d port %s \n",
                  unit, SOC_PORT_NAME(unit, port)));
    ...
    if (SOC_PBMP_MEMBER(mask, port)) {
        ...
        /* Drain cells */
    SOC_IF_ERROR_RETURN(_mac_xl_drain_cells(unit, port, 0));
...
mac_xl_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (enable) {
        ...
        /* Drain cells */
        SOC_IF_ERROR_RETURN(_mac_xl_drain_cells(unit, port, 1));
