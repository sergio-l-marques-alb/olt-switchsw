/*
 * $Id: bcm-robo-knet.c 1.14 Broadcom SDK $
...
LKM_MOD_PARAM(use_tx_skb, "i", int, 1);
 ...
 */
static int 
bkn_proc_dma_read(char *page, char **start, off_t off,
                    int count, int *eof, void *data)
{
    ...
    *eof = 1;
    ...
    list_for_each(list, &_minfo_list) {
        ...
        gmodule_pprintf(&ptr, "DMA Configuration:\n");
        dma_dumptx(minfo->di[0],buf,FALSE);
        gmodule_pprintf(&ptr, buf);
                    ...
                    int count, int *eof, void *data)
{
    ...
    *eof = 1;
    ...
    gmodule_pprintf(&ptr, "  use_rx_skb_chan:     0x%x\n", use_rx_skb_chan);
    gmodule_pprintf(&ptr, "  use_tx_skb:          0x%x\n", use_tx_skb);
                    ...
                    int count, int *eof, void *data)
{
    ...
    *eof = 1;
    ...
    list_for_each(list, &_minfo_list) {
        ...
        gmodule_pprintf(&ptr,"tx active chan %x\n", minfo->tx_active_chan);
        gmodule_pprintf(&ptr,"rx active chan %x\n", minfo->rx_active_chan);

        for (chan = 0; chan < NUMTXQ; chan++) {
            gmodule_pprintf(&ptr, "Device stats:\n");
            gmodule_pprintf(&ptr, "  Tx%d packets  %10lu\n", 
                        chan, minfo->tx[chan].pkts);
            gmodule_pprintf(&ptr, "  Rx%d packets %10lu\n",
                            chan, minfo->rx[chan].pkts);
            gmodule_pprintf(&ptr, "  Tx%d suspends %10lu\n", 
                            chan, minfo->tx[chan].suspends);
            gmodule_pprintf(&ptr, "DMA event :\n");
            gmodule_pprintf(&ptr, "  Tx%d api_tx status %d dcb_list empty %d \n",
                chan,
                minfo->tx[chan].api_active,
                list_empty(&minfo->tx[chan].api_dcb_list));
            gmodule_pprintf(&ptr, "  Rx%d dcb_list empty %d dma active %d\n",
                chan,               
                list_empty(&minfo->rx[chan].api_dcb_list),
                dma_rxactive(minfo->di[chan]));
            gmodule_pprintf(&ptr, "  rx_dcb_seqno[%d] :%d\n",
                chan, minfo->rx[chan].rx_dcb_seqno);
            gmodule_pprintf(&ptr, "  tx_desc_seqno[%d] :%d\n",
...
bkn_proc_init(void)
{
    create_proc_read_entry("dma", 0, bkn_proc_root, bkn_proc_dma_read, NULL);
...
bkn_tx(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (down_trylock(&minfo->tx[use_tx_skb].sem) != 0) {
    ...
    if (minfo->di[use_tx_skb]->txavail > 0 ){
    ...
    dma_txfast(minfo->di[use_tx_skb], new_skb, 1);

    priv->stats.tx_packets++;
    priv->stats.tx_bytes += pktlen;
    minfo->tx[use_tx_skb].pkts++;
    } else {
        priv->stats.tx_dropped++;
        dev_kfree_skb(skb);
    ...
    if (minfo->di[use_tx_skb]->txavail < 1) {
        bkn_suspend_tx(minfo, use_tx_skb);
    }
