                            ...
                            mpls_entry_entry_t *entry, uint32 *result)
{
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        if (soc_mem_field32_get(unit, MPLS_ENTRYm, tmp, VALIDf)) {
            if (soc_mem_compare_key(unit, MPLS_ENTRYm, entry, tmp) == 0) {
                debugk(DK_VERBOSE, "write sblk %d acc_type %d bucket %d, "
                       "slot %d, index %d\n",
    ...
    if (free_index != -1) {
        ...
        debugk(DK_VERBOSE, "write sblk %d acc_type %d bucket %d, slot %d, "
               "index %d\n",
               sblk, acc_type, index / 8, index % 8, index);
                            ...
                            mpls_entry_entry_t *entry, uint32 *result)
{
    ...
    for (slot = slot_min; slot <= slot_max; slot++) {
        ...
        if (soc_mem_field32_get(unit, MPLS_ENTRYm, tmp, VALIDf)) {
            if (soc_mem_compare_key(unit, MPLS_ENTRYm, entry, tmp) == 0) {
                debugk(DK_VERBOSE, "found sblk %d acc_type %d bucket %d, "
                       "slot %d, index %d\n",
    ...
    debugk(DK_VERBOSE, "soc_internal_mpls_entry_del: Not found\n");
                             ...
                             mpls_entry_entry_t *entry, uint32 *result)
{
    ...
    debugk(DK_VERBOSE, "soc_internal_mpls_entry_lkup: Not found\n");
