...
static int __devinit fsl_srio_port_probe(struct srio_dev *srio_dev,
					struct device_node *node)
{
	...
	if (!cell) {
		dev_err(srio_dev->dev, "Can't get %s property"
			" '#address-cells'\n", srio_node->full_name);
		return -ENODEV;
	}
	aw = *cell;
...
static int __devinit fsl_srio_uio_probe(struct platform_device *dev)
{
...
static void __exit fsl_srio_uio_exit(void)
{
	...
	pr_warning("fsl srio uio driver removed\n");
}
