...
*            PR710_SDH_RA                                               *
*                                                                       *
* Descrição: Este módulo possui as funções necessárias para calculo de  *
*            CRC de um conjunto de bytes contiguos na memoria           *
*                                                                       *
*                                                                       *
* Histórico:                                                            *
...
/************************************************************************
* Tabela auxiliar que permite o calculo do crc de uma forma mais rapida *
************************************************************************/
...
*         9 Julho de 2002                                                        *
*                                                                                *
* Autor: Celso / Nuno Pires                                                      *
*                                                                                *
*********************************************************************************/
uint16 getCRC16(uint8 *startaddr,uint32 size)
{
  uint16 ret_crc = 0xFFFF;  
  uint32 ctr;
  uint8 *pflash;

  pflash = startaddr;

  //printf("pflash = 0x%.2x ret_crc=0x%.4x\n\r",*pflash,ret_crc);

  for (ctr=0; ctr < size ; ctr++)
  {
    ret_crc = CRC_TABLE[(uint8) (*pflash) ^ (uint8) ret_crc] ^ (uint8) (ret_crc >> 8);
    pflash++;
  }
...
uint16 getCRC16_from_file(FILE *fp, uint32 size)
{
  ...
  for (ctr=0; ctr < size ; ctr++)
  {
    ret_crc = CRC_TABLE[pflash ^ (uint8) ret_crc] ^ (uint8) (ret_crc >> 8);
    fread(&pflash,1,1,fp);
  }
