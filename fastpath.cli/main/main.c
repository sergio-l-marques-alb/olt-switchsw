...
//      "m 1412 vlan1[2-4093] vlan2 ... - IGMP snooping querier: remove vlans\r\n"*/
        ...
        "m 1601 EVC#[0-64] Type[0:PMP/1:P2P/2:Q-PMP/3:Q-PP] Stacked[0/1] MacLearn[0/1] Mask[0x010:CPUtrap;0x100:DHCP] MCFlood[0-All;1-Unknown;2-None]\r\n"
...
//      break;
      ...
      case 1607:
        if (resposta.flags == (FLAG_RESPOSTA | FLAG_ACK))
          printf("EVC flow successfully created\n\r");
        else
          printf("Failed to create EVC flow - error %08X\r\n", *(unsigned int*)resposta.info);
        break;

      case 1608:
        if (resposta.flags == (FLAG_RESPOSTA | FLAG_ACK))
          printf("EVC flow successfully deleted\n\r");
        else
          printf("Failed to delete EVC flow - error %08X\r\n", *(unsigned int*)resposta.info);
        break;
