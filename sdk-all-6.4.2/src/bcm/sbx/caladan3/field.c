...
static _sbx_caladan3_field_unit_info_t _sbx_caladan3_field[BCM_MAX_NUM_UNITS];
...
/*
 *  Function
 *     _bcm_caladan3_field_unit_data_size_get
 *  Purpose
 ...
 */
int
_bcm_caladan3_field_unit_data_size_get(int           unit, 
                                       unsigned int *unit_data_size)
{
    _CALADAN3_FIELD_COMMON_LOCALS;
    result = _bcm_caladan3_field_unit_intro(unit, &lock, &microcode, &unitData);
    if (BCM_E_NONE == result) {
        bcm_caladan3_g3p1_field_unit_data_size_get(unitData, unit_data_size);
    }
    result = _bcm_caladan3_field_unit_outro(result, unit, lock);
    return result;
}

void * _bcm_caladan3_field_glob_data_ptr_get(int unit)
{
    return (void *) (_sbx_caladan3_field[unit].data);
}
