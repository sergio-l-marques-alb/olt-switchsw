...
bcm_dfe_port_loopback_set(int unit, bcm_port_t port, int loopback)
{
    ...
    case BCM_PORT_LOOPBACK_MAC:
        dfe_loopback = soc_dcmn_loopback_mode_mac_outer;
        break;
    ...
    BCMDNX_IF_ERR_EXIT(bcm_dfe_port_loopback_get(unit, port, &lb_start));
    if (loopback != 0) {
        if (lb_start == 0) {
            BCMDNX_IF_ERR_EXIT(bcm_dfe_port_phy_control_get(unit, port, BCM_PORT_PHY_CONTROL_CL72, &cl72_start));
            rc = SOC_DFE_WARM_BOOT_ARR_VAR_SET(unit, PORT_CL72_CONF, port, &cl72_start);
            SOCDNX_IF_ERR_EXIT(rc);
            if (cl72_start == 1) {
                BCMDNX_IF_ERR_EXIT(bcm_dfe_port_phy_control_set(unit, port, BCM_PORT_PHY_CONTROL_CL72, 0)); 
            }
        }
    }
    
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_loopback_set,(unit, port, dfe_loopback));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (loopback == 0) {
        if (lb_start != 0) {
            ...
            if (cl72_start == 1) {
                BCMDNX_IF_ERR_EXIT(bcm_dfe_port_phy_control_set(unit, port, BCM_PORT_PHY_CONTROL_CL72, 1));
            }
        }
    }

    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_burst_control_set,(unit, port, dfe_loopback));
    BCMDNX_IF_ERR_EXIT(rc);
...
{
    int rc;
    soc_dcmn_loopback_mode_t lb_type = soc_dcmn_loopback_mode_none;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_loopback_get,(unit, port, &lb_type));
    BCMDNX_IF_ERR_EXIT(rc);
