...
STATIC uint32 _bcm_petra_foce_route_key_to_ppd(
    ...
    if (SOC_DPP_CONFIG(unit)->arad->enable_lpm_custom_lookup) { /* custom LPM lookup, we are done */
...
STATIC uint32 _bcm_petra_foce_route_info_to_ppd(
    ...
    if (SOC_DPP_CONFIG(unit)->arad->enable_lpm_custom_lookup) { /* custom LPM lookup */
...
/* Delete all zone check entries */
int bcm_petra_fcoe_zone_entry_delete(
    int unit, 
    bcm_fcoe_zone_entry_t *zone)
{ 
    SOC_PPD_FRWRD_FCF_ZONING_KEY 
