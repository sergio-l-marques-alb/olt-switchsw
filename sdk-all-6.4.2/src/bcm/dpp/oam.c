    ...
    if ((endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) == 0 || _BCM_OAM_IS_SERVER_SERVER(endpoint_info)  ) {
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) &&  PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info)  && (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING)) {
                /* In this special case local port serves two purposes: 1) pointer to local_port_2_sys_port which will serve as the SSP on the ITMH
                   which should be the remote device recycle port and should be retreived from the remote_gport.
                   2) The local port itself will be the SSP on the inner PTCH, which is retreived from the tx_gport.
...
int _bcm_oam_classifier_default_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
	...
	if (endpoint_info->lm_counter_base_id == 0) {
		...
		_BCM_PETRA_PROC_CNTR_FROM_STAT(proc_dummy, classifier_mep_entry->counter, endpoint_info->lm_counter_base_id);
        if ((endpoint_info->lm_flags & BCM_OAM_LM_PCP) != 0) {
			classifier_mep_entry->flags |= SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP;
        }
	}
...
int _bcm_oam_classifier_mep_entry_struct_get(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
	...
	if (classifier_mep_entry->counter == 0) {
		endpoint_info->lm_counter_base_id = 0;
	}
	else {
		_BCM_PETRA_STAT_FROM_PROC_CNTR(endpoint_info->lm_counter_base_id, 0/*counter_proc (irrelevant)*/, classifier_mep_entry->counter);
		endpoint_info->lm_flags |= ((classifier_mep_entry->flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP) != 0) ? BCM_OAM_LM_PCP : 0;
	}
        ...
        *meter_disable_lcl = profile_data->mep_profile_data.meter_disable;
		...
		if (classifier_mep_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) { 
			 if ((opcode == SOC_PPD_OAM_ETHERNET_PDU_OPCODE_CCM) || (opcode == SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LTM) ||
				 ...
				 (opcode == SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LTR)) { /* These opcodes trap in multicast */
                 ...
                 if (opcode == SOC_PPD_OAM_ETHERNET_PDU_OPCODE_LBM) {
					 if (is_upmep) {
						 ...
						 if (classifier_mep_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) { /* recycle */
							 profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode] = trap_id_recycle;
						 }
						 else { /* trap */
			...
			if (no_traps) {
				...
				if (trap_config.flags & BCM_RX_TRAP_UPDATE_DEST) {
                    ...
                    BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {
                    	soc_sand_rv = soc_ppd_oam_pp_pct_profile_set(unit, local_port, 0);
	                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
				}
...
int _bcm_dpp_endpoint_counters_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_LIF_ID lif) {
    ...
    if (endpoint_info->lm_counter_base_id > 0) {
        ...
        for (processor_ndx=0; processor_ndx<SOC_TMC_CNT_NOF_PROCESSOR_IDS_ARAD; processor_ndx++) {
            BCM_SAND_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_cnt_counters_get,(unit, processor_ndx, &processor_info)));
            if ((processor_info.src_type==SOC_TMC_CNT_SRC_TYPE_OAM_ING_A) ||
                (processor_info.src_type==SOC_TMC_CNT_SRC_TYPE_OAM_ING_C)) {
                if ((endpoint_info->lm_flags & BCM_OAM_LM_PCP)!=0) {
                    for ( i = 0; i < 8; i++) {
...
int _bcm_dpp_oam_bfd_trap_code_from_remote_gport(int unit, 
			...
			if (trap_config.flags & BCM_RX_TRAP_UPDATE_DEST) {
				...
				if (is_equal) {
					...
					if (is_upmep) {
						...
						/* If that's a MIP we search for 2 consecutive trap codes for up and down meps */
						if (!is_mep) {
							soc_sand_rv = soc_ppd_eg_mirror_recycle_command_trap_get(unit, wb_mirror_profile, &trap_code_dummy, &snoop_strength);
							BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
							if (trap_code_dummy == trap_code_i+1) {
								is_destination_new = FALSE;
								break;
							}
						}
	...
	if (is_destination_new) {
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {
           	soc_sand_rv = soc_ppd_oam_pp_pct_profile_set(unit, local_port, 1);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
...
int bcm_petra_oam_init(int unit) {
...
#endif /*BCM_WARM_BOOT_SUPPORT*/
	{
		...
		/*get CPU port*/
		BCM_PBMP_ITER(port_config.cpu, cpu_port) {
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, cpu_port, &soc_ppd_port, &core)));
        ...
        soc_sand_rv = soc_ppd_oam_pp_pct_profile_set(unit, cpu_local_port, 1);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        oamp_local_port = gport_info.local_port;
           BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, oamp_local_port, &soc_ppd_port, &core)));

		soc_sand_rv = soc_ppd_oam_pp_pct_profile_set(unit, oamp_local_port, 1);
		BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
	...
	#ifdef BCM_ARAD_SUPPORT
		if ((SOC_IS_ARAD(unit))) {
            BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, recycle_port, &soc_ppd_port, &core)));
...
int bcm_petra_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (is_default) {
			...
			if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
				...
				if (is_accelerated) {
                    ...
                    if (is_upmep) {
                        ...
                        if (SOC_IS_ARADPLUS_AND_BELOW(unit) && (endpoint_info->gport!=BCM_GPORT_INVALID || 
                            ...
                            _bcm_dpp_gport_info_t gport_info;
							/* For the server there is no reason to expect that the gport is local.*/
							uint32 operaation =  _BCM_OAM_IS_SERVER_SERVER(endpoint_info)? 0 : _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY;
							bcm_gport_t gport_to_verify = _BCM_OAM_IS_SERVER_SERVER(endpoint_info)? endpoint_info->tx_gport : endpoint_info->gport;
                            rv = _bcm_dpp_gport_to_phy_port(unit, gport_to_verify, operaation, &gport_info);
                            BCMDNX_IF_ERR_EXIT(rv);
                    ...
                    } else if (endpoint_info->tx_gport != BCM_GPORT_INVALID  && SOC_IS_ARAD_A0(unit)) {
				...
				if ( (endpoint_info->lm_counter_base_id >= 0x4000) || (endpoint_info->lm_counter_base_id < 0) ) {
					/*value must be  must be in the range 0 - (16K-1)*/
						BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
						 (_BSL_BCM_MSG("Error: invalid lm_counter_base_id (must be in range 0 to 16K-1).\n")));
		...
		/* Port/Interface status. May be used for accelerated EPs or remote EPs*/
		if (((endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) || is_accelerated)
			&& SOC_IS_JERICHO(unit)) {
			if ((endpoint_info->flags & BCM_OAM_ENDPOINT_PORT_STATE_UPDATE) ?
				(endpoint_info->port_state > BCM_OAM_PORT_TLV_UP || endpoint_info->port_state < BCM_OAM_PORT_TLV_BLOCKED) :
	...
	if ((endpoint_info->type != bcmOAMEndpointTypeEthernet) && 
		(endpoint_info->type != bcmOAMEndpointTypeBHHMPLS)) {
		BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
			 (_BSL_BCM_MSG("Error: Supporting only Ethernet and BHH-MPLS type.\n")));
	}
                ...
                if (_BCM_OAM_IS_SERVER_CLIENT(endpoint_info)) {
                    ...
                    for (is_passive = 0; is_passive <= 1; is_passive++) {
                        ...
                        if (found_profile) {
                            ...
                            SOC_PPD_OAM_LIF_PROFILE_DATA_clear(profile_data_pointer);
                            rv = _bcm_oam_default_profile_get(unit, profile_data_pointer, &classifier_mep_entry, 0 /* non accelerated */, 0 /* is default */, is_passive ^ is_upmep/*is_up*/);
                            BCMDNX_IF_ERR_EXIT(rv);
                        }
...
int bcm_petra_oam_endpoint_action_set_internal(
    ...
    profile_data.is_piggybacked = profile_data_passive.is_piggybacked = is_piggy_backed;

    if (((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit) && !is_mip ) || is_accelerated) && !is_default) {
    ...
    for (is_passive=0; is_passive<=is_mip; is_passive++) {
        ...
        if (profile_info.is_last) { 
            ...
            if (SOC_IS_ARADPLUS(unit) && profile_change) {
                soc_sand_rv = soc_ppd_oam_classifier_counter_disable_map_set(unit, 0/*packet_is_oam*/, old_profile, 0); /*!!!!!!!!!!!! where to get the old profile and profile from???*/
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
        }
