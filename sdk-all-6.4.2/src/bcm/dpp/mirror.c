...
{   
    ...
    if (mirror_enable) {    
        ...
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_to_interface_map_set,(unit, core, recycle_port, SOC_TMC_PORT_DIRECTION_INCOMING, &port2if_mapping_info));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));
            
        /* Step 3: port header type set */
        result = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_header_type_set,(unit, core, recycle_port, SOC_TMC_PORT_DIRECTION_INCOMING, SOC_TMC_PORT_HEADER_TYPE_RAW));
        BCMDNX_IF_ERR_EXIT(soc_sand_get_error_code_from_error_word(result));
