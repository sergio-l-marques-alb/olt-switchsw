        ...
        if (is_egress_protection) {
            ...
            /* In Decoupled mode, a logic should be applied in order to lookup for an unused (but allocated)
               EEDB half entry according to the supplied failover ID */
                ...
                lif_flags = ((vlan_port->egress_failover_id) % 2) ?
          ...
          /* protected case, where need to configure only LIF,
             in this case failover-id is invalid, but failover_port_id includes the FEC id to learn */
              ...
              if (SOC_DPP_ENCAP_ID_IS_FORWARD_GROUP(failover_encap_id)) {
                  /* This fec id i used for the learning information */
