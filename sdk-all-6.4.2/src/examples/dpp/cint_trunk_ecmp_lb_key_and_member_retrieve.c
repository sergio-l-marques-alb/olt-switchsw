...
static uint8 is_arad_p;
/*
 * Function:
 *    get_device_type
 * Purpose:
 *    Returns the device type (0x8660 for Arad+, 0x8650 for Arad)
 * Parameters: 
 *    unit    -- unit id.
 */
int get_device_type(int unit) {
   bcm_info_t info;
...
int is_arad(int unit) {
   return get_device_type(unit) == 0x8650;
}

int is_arad_p(int unit) {
   return get_device_type(unit) == 0x8660;
}
...
uint32 fer_lb_key(
   ...
   if ((!is_arad(unit)) && (!is_arad_p(unit))) {
