    ...
    uint32 flags)
{
    ...
    /*get info*/
    nof_fmacs = SOC_DFE_DEFS_GET(unit, nof_instances_mac);
    ...
    if (!(flags & SOC_DFE_ENABLE_MAC_LOOPBACK))
    { 
        port_power_flags = (flags & SOC_DFE_ENABLE_EXTERNAL_LOOPBACK) ? SOC_DFE_PORT_CONTROL_FLAGS_RX_SERDES_IGNORE : 0; 
        PBMP_SFI_ITER(unit, port) 
        {
            SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,  mbcm_dfe_port_control_power_set, (unit, port, port_power_flags, soc_dfe_port_power_off)));
        }
    }
    ...
    if (!(flags & SOC_DFE_ENABLE_MAC_LOOPBACK))
    {
        ...
        PBMP_SFI_ITER(unit, port)
        {
            SOCDNX_IF_ERR_EXIT(MBCM_DFE_DRIVER_CALL(unit,  mbcm_dfe_port_control_power_set, (unit, port, port_power_flags, soc_dfe_port_power_on)));
        }
    }
    ...
    *field_val = soc_reg_field_get(unit, OCCG_TEST_MODE_CMD_REGr, reg_val, FIELD_2_10f); 

    
    /*Data fifo*/
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_DATA_FIFO_POS 0
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_DATA_FIFO_LOAD_LOW 0x0
    SHR_BITCLR_RANGE(field_val, _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_DATA_FIFO_POS, 2);
    *field_val |= (_SOC_FE3200_FABRIC_CELL_SNAKE_TEST_DATA_FIFO_LOAD_LOW << _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_DATA_FIFO_POS);
   
    /*Control fifo*/
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_CONTROL_FIFO_POS 2
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_CONTROL_FIFO_LOAD_LOW 0x0
    SHR_BITCLR_RANGE(field_val, _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_CONTROL_FIFO_POS, 2);
    *field_val |= (_SOC_FE3200_FABRIC_CELL_SNAKE_TEST_CONTROL_FIFO_LOAD_LOW << _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_CONTROL_FIFO_POS);
    
    SHR_BITCLR_RANGE(field_val, 4, 1);
    *field_val |= (0x1 << 4);

    /* Reserved bits - must be 2'b00*/
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_RESERVED_POS 5
    SHR_BITCLR_RANGE(field_val, _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_RESERVED_POS, 2);
    *field_val |= (0x0 << _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_RESERVED_POS);

    /*Filter*/
    #define _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_FILTER_POS 7
    SHR_BITCLR_RANGE(field_val, _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_FILTER_POS, 2);
    *field_val |= (0x3 << _SOC_FE3200_FABRIC_CELL_SNAKE_TEST_FILTER_POS);

    soc_reg_field_set(unit, OCCG_TEST_MODE_CMD_REGr, &reg_val, FIELD_2_10f, *field_val); 
    ...
    *field_val = ((!(flags & SOC_DFE_ENABLE_MAC_LOOPBACK)) ? 0x1:0x0);
    ...
    soc_reg_field_set(unit, OCCG_TEST_MODE_CMD_REGr, &reg_val, FIELD_12f, 0); 
    ...
    /*Clear out of frame interrupt*/
    for (blk_id = 0; blk_id < nof_fmacs; blk_id++)
    {
        SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAC_INTERRUPT_REGISTER_4r(unit, blk_id, &reg_val));
        soc_reg_field_set(unit, FMAC_FMAC_INTERRUPT_REGISTER_4r, &reg_val, OOF_INTf, 0xf);
    ...
    /* 
     *link load load-balancing levels ignore 
     */
    SOCDNX_IF_ERR_EXIT(READ_RTP_DRH_LOAD_BALANCING_LEVEL_CONFIGr(unit, &reg64_val));
    soc_reg64_field32_set(unit, RTP_DRH_LOAD_BALANCING_LEVEL_CONFIGr, &reg64_val, LOAD_BALANCE_LEVELS_IGNOREf, 0);
    SOCDNX_IF_ERR_EXIT(WRITE_RTP_DRH_LOAD_BALANCING_LEVEL_CONFIGr(unit, reg64_val));
...
exit:
    ...
    {
      /*interrupts enable*/
        rv = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_interrupt_all_enable_set, (unit, 1));
        SOCDNX_IF_ERR_EXIT(rv);
