...
soc_fe3200_cell_filter_count_get(int unit, int *count)
{
    ...
    *count = 0;
    ...
    for(i=0 ; i<SOC_DFE_DEFS_GET(unit, nof_instances_dch) ; i++) {  
        ...
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_Af);
        *count += temp_count;
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_Bf);
        *count += temp_count;

        /*check overflow*/
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_AOf);
        if(temp_count) {
            *count = -1;
            continue;
        }
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_BOf);
        if(temp_count) {
            *count = -1;
            continue;
        }
