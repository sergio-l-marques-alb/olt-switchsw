...
soc_fe1600_sr_cell_format(int unit, const vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf) 
{
    ...
    {   
        ...
        soc_vsc256_build_header(unit, cell, buf_size_bytes - (FE1600_SR_DATA_CELL_PAYLOAD_LENGTH/8), (uint32*)(((uint8*)buf) + (FE1600_SR_DATA_CELL_PAYLOAD_LENGTH/8)));             
        /*
        * bits 511:0 - payload
        */
...
soc_fe1600_sr_cell_send(int unit, const vsc256_sr_cell_t* cell)
{
    ...
    blk_id = INT_DEVIDE(first_link,32);
    inner_link = first_link%32;
    ...
    SOC_REG_ABOVE_64_CLEAR(reg_val);
    SHR_BITCOPY_RANGE(reg_val, 0, (uint32*)buf, 4*128, 88);
