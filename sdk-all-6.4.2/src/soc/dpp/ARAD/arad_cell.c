...
int soc_fabric_inband_route_t_2reg_32(int unit, const soc_fabric_inband_route_t* route, uint32 *reg){
  ...
  *reg = 0;
  ...
  rv = soc_dpp_fabric_route2sr_link_list(unit, route, &sr_link_list);
  if(rv) {
    return rv;
  }
...
soc_arad_sr_cell_format(int unit, const vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf) 
{
    ...
    if(VSC256_CELL_FORMAT_FE600 == cell->payload.inband.cell_format) {
  ...
  )
{
  ...
  arad_sw_db_cell_cell_ident_get(unit, &cell_id);
  ++cell_id;
  arad_sw_db_cell_cell_ident_set(unit, cell_id);
