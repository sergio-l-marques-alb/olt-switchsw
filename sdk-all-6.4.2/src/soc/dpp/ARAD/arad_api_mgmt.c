  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_CREDIT_WORTH_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  res = arad_mgmt_credit_worth_verify(
    unit,
    credit_worth
  );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_mgmt_credit_worth_set_unsafe(
    unit,
    credit_worth
  );
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mgmt_credit_worth_set()",0,0);
}
  ...
  )
{
    ...
    if (credit_value_type > ARAD_PLUS_FAP_CREDIT_VALUE_MAX) {
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MGMT_SYSTEM_FAP_ID_GET);
 SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
 SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mgmt_system_fap_id_get()",0,0);
}
  ...
  )
{
  ...
  SOC_TMC_STAT_IF_REPORT_INFO_clear(&(info->stat_if_info));
