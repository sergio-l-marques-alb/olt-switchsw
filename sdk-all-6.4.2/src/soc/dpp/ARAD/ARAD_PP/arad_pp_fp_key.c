       ...
       )
{
    ...
    const ARAD_SW_DB_PMF_CE ce_array_kbp_ipv6_match[] = {
    ...
    const ARAD_SW_DB_PMF_CE ce_array_kbp_match[] = {
    ...
    const ARAD_SW_DB_PMF_CE ce_array_kbp_p2p_match[] = {
                {1,   0, 0, 15, 0, 0, 0, SOC_PPC_FP_QUAL_IRPP_IN_LIF}
               };    

    if (flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) {
        (*nof_ces) = sizeof(ce_array_kbp_ipv6_match)/sizeof(ARAD_SW_DB_PMF_CE);
        sal_memcpy(ce_array,&ce_array_kbp_ipv6_match,sizeof(ARAD_SW_DB_PMF_CE)* (*nof_ces) );        
    } else if(flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB){
        (*nof_ces) = sizeof(ce_array_kbp_match)/sizeof(ARAD_SW_DB_PMF_CE);
        sal_memcpy(ce_array,&ce_array_kbp_match,sizeof(ARAD_SW_DB_PMF_CE)* (*nof_ces) );
    } else /*SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB*/{
        (*nof_ces) = sizeof(ce_array_kbp_p2p_match)/sizeof(ARAD_SW_DB_PMF_CE);
        sal_memcpy(ce_array,&ce_array_kbp_p2p_match,sizeof(ARAD_SW_DB_PMF_CE)* (*nof_ces) );
    }
  ...
  )
{
    ...
    /* Get the database Info */
     ...
     if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) || (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) || (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB))
     {
    ...
    if ( is_vt_match  ) /*Vlan translation classification*/
    {
        ...
        /*Build the CEs array*/
        res = arad_pp_fp_kbp_match_key_ces_get(unit, fp_database_info.flags, ce_array, &nof_ces_to_handle);
        SOC_SAND_CHECK_FUNC_RESULT(res, 8, exit);
        is_key_320b = 0;
    }
    else
            ...
            if(buf_msb != ARAD_PP_FP_TCAM_ENTRY_SIZE*32) {
                ...
                if(nof_lsb[key_ndx] > 0) 
                {
                    ...
                    if (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) {
                        ...
                        } else if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB){
                            frwrd_table_id = ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6;
                        } else if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB){
