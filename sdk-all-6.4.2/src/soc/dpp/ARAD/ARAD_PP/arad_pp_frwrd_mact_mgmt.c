  ...
  )
{
   ...
   */
  res = arad_mgmt_system_fap_id_get(
          unit,
          &fap_id
        );
  ...
  )
{
  ...
  last_age_key_default_ndx =  age_key_ndx + ARAD_PP_FRWRD_MACT_NOF_VSI_AGING_DEFAULT_PROFILES;
  ...
  for (; age_key_ndx < last_age_key_ndx; ++age_key_ndx)
  {
    ...
    /* cfg-ptr:0-1, sys-phy:2-2, sys-lag: 3-3*/
    /* update Aging Configuration Pointer only for default profiles*/
    if (age_key_ndx < last_age_key_default_ndx) {
  ...
  )
{
  ...
  /* Set aging pointer to age_aging_mode register. Only dynamic entry */
  pointer_offset = ARAD_PP_FRWRD_MACT_AGE_AGING_DYNAMIC_CONF_OFF + fid_aging_profile*ARAD_PP_FRWRD_MACT_AGE_AGING_CONF_SIZE; 

  /* cfg-ptr:0-1*/
  SHR_BITCOPY_RANGE(reg_above_64_val,pointer_offset, &fid_aging_profile, 0, 2);
    ...
    )
{
 ...
 res = arad_pp_frwrd_mact_is_ingress_learning_get(
  ...
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_EVENT_HANDLE_KEY_IS_LAG_OUT_OF_RANGE_ERR, 40, exit);
  }
    ...
    )
{
 ...
 /*
  ...
  case ARAD_PP_FRWRD_MACT_MSG_HDR_TYPE_RAW:
    header_size = ARAD_PP_FRWRD_MACT_ETHERNET_TYPE_SIZE;
    header_type_lsb = 0;
    to_skip = 0;
    break;
