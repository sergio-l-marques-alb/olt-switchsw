  ...
  )
{
  ...
  res = arad_pp_sw_db_ilm_key_mask_inrif_set(unit, glbl_info->key_info.mask_inrif);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  res = arad_pp_sw_db_ilm_key_mask_port_set(unit, glbl_info->key_info.mask_port);
  ...
  )
{
  ...
  res = arad_pp_sw_db_ilm_key_mask_inrif_get(unit, &glbl_info->key_info.mask_inrif);
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  res = arad_pp_sw_db_ilm_key_mask_port_get(unit, &glbl_info->key_info.mask_port);
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
  ...
  )
{
  ...
  *nof_entries = 0;
  ...
  if (SAL_BOOT_PLISIM) {
      ...
      read_vals = (ARAD_PP_LEM_ACCESS_PAYLOAD*)sal_alloc(sizeof(ARAD_PP_LEM_ACCESS_PAYLOAD) * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
      if (read_vals == NULL)
      {
          SOC_SAND_SET_ERROR_CODE(SOC_SAND_MALLOC_FAIL, 10, exit);
      }
      read_keys = (ARAD_PP_LEM_ACCESS_KEY*)sal_alloc(sizeof(ARAD_PP_LEM_ACCESS_KEY) * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
      if (read_keys == NULL)
      {
          SOC_SAND_SET_ERROR_CODE(SOC_SAND_MALLOC_FAIL, 15, exit);
      }

      arad_pp_lem_block_get(unit, NULL, NULL, block_range, read_keys, read_vals, &nof_total_entries);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->inrif, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 13, exit);
