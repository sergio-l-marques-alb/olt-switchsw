  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_MPLS_TERM_LABEL_RANGE_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(label_range_info);

  ARAD_PP_MPLS_TERM_LABEL_RANGE_INFO_clear(label_range_info);

  res = READ_IHP_MPLS_LABEL_RANGE_LOWr(unit, REG_PORT_ANY, range_ndx, &label_range_info->range.first_label);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 10, exit);
  res = READ_IHP_MPLS_LABEL_RANGE_HIGHr(unit, REG_PORT_ANY, range_ndx, &label_range_info->range.last_label);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

  
  label_range_info->term_info.processing_type = (temp) ? ARAD_PP_MPLS_TERM_MODEL_PIPE : ARAD_PP_MPLS_TERM_MODEL_UNIFORM;
  
  
  label_range_info->term_info.cos_profile = temp;

  
  /* if rif not valid then set it to ARAD_PP_RIF_NULL */
  if (temp == 0)
  {
    label_range_info->term_info.rif= ARAD_PP_RIF_NULL;
  }

  label_range_info->term_info.next_prtcl = ARAD_PP_L3_NEXT_PRTCL_TYPE_MPLS;

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_mpls_term_label_range_get_unsafe()", range_ndx, 0);
}
  ...
  )
{
	...
	term_profile_tbl_data.labels_to_terminate = term_profile_info->nof_headers-1;
  ...
  )
{
	...
	term_profile_info->nof_headers = term_profile_tbl_data.labels_to_terminate+1;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->default_rif, SOC_DPP_DEFS_GET(unit, nof_rifs), ARAD_PP_RIF_ID_OUT_OF_RANGE_ERR, 11, exit);
