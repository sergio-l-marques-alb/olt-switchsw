...
soc_error_t
soc_port_sw_db_port_add(int unit, soc_port_t port, int core, uint32 channel, uint32 flags, soc_port_if_t interface, soc_pbmp_t phy_ports) 
{
...
soc_error_t
soc_port_sw_db_port_validate_and_add(int unit, soc_port_t port, int core, uint32 channel, uint32 flags, soc_port_if_t interface, soc_pbmp_t phy_ports) 
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    /*add port to DB*/
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_port_add(unit, port, core, channel, flags, interface, phy_ports));
...
soc_port_sw_db_protocol_offset_get(int unit, soc_port_t port, uint32 *offset) 
{
    ...
    if (SOC_PORT_IF_ILKN != interface_type && SOC_PORT_IF_CAUI != interface_type) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PORT, (_BSL_SOCDNX_MSG("Port %d isn't ILKN port"),port));
    }
...
soc_error_t
soc_port_sw_db_tm_to_local_port_get(int unit, uint32 tm_port, int core, soc_port_t *port) {
...
soc_error_t
soc_port_sw_db_pp_to_local_port_get(int unit, uint32 pp_port, int core, soc_port_t *port) 
{
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_port_sw_db_pp_is_valid_get(int unit, uint32 pp_port, int core, uint32 *is_valid)
{
    soc_port_t port;
    SOCDNX_INIT_FUNC_DEFS;
...
int
soc_port_sw_db_tm_port_to_out_port_priority_get(int unit, uint32 tm_port, int core, uint32* nof_priorities)
{
    soc_port_t port;
    soc_error_t rv;
    SOCDNX_INIT_FUNC_DEFS;
...
int
soc_port_sw_db_pp_port_to_out_port_priority_get(int unit, uint32 pp_port, int core, uint32* nof_priorities)
{
    soc_port_t port;
    soc_error_t rv;
    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_pp_to_local_port_get(unit, pp_port, core, &port));
...
soc_error_t
soc_port_sw_db_pp_port_to_base_q_pair_get(int unit, uint32 pp_port, int core, uint32* base_q_pair)
{
    soc_port_t port;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_error_t
soc_port_sw_db_tm_port_to_base_q_pair_get(int unit, uint32 tm_port,  int core, uint32* base_q_pair)
{
    soc_port_t port;
    SOCDNX_INIT_FUNC_DEFS;
...
soc_port_sw_db_snapshot_restore(int unit) {
    ...
    for (port = 0; port < SOC_MAX_NUM_PORTS; port++) {
        if (PORT_INFO_SS.valid) {
            ...
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_port_add(unit, port,
