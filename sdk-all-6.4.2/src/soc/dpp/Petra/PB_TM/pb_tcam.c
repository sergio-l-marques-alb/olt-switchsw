    ...
    )
{
  ...
  previous = SOC_SAND_SORTED_LIST_ITER_BEGIN(list);
  current  = SOC_SAND_SORTED_LIST_ITER_BEGIN(list);
  ...
  while (!priority_found && (current != SOC_SAND_SORTED_LIST_ITER_END(list)))
  {
  ...
  if (previous != SOC_SAND_SORTED_LIST_ITER_BEGIN(list))
  {
  ...
  if (current != SOC_SAND_SORTED_LIST_ITER_END(list))
  {
    soc_pb_tcam_db_location_decode(
      key_buffer,
      &range->max
    ...
    )
{
  ...
  while (
    (direction == SOC_PB_TCAM_DIRECTION_BACKWARD) ?
      ((next != SOC_SAND_SORTED_LIST_ITER_END(prio_list)) && (next_priority < target_priority))
        : ((next != SOC_SAND_SORTED_LIST_ITER_BEGIN(prio_list)) && (next_priority > target_priority)))
  {
    /*
    ...
    while ((direction == SOC_PB_TCAM_DIRECTION_BACKWARD ? 
              next != SOC_SAND_SORTED_LIST_ITER_END(prio_list)
                : next != SOC_SAND_SORTED_LIST_ITER_BEGIN(prio_list))
  ...
  )
{
  ...
  else
  {
    ...
    if (entry_size == old_entry_size)
    {
       ...
       */
      soc_sand_occ_bm_clear(occ_bm);
    }
    else
    {
      soc_sand_occ_bm_destroy(occ_bm);
      create_bitmap = TRUE;
    }
  }
  ...
  )
{
   ...
   */
  priorities = soc_pb_sw_db_tcam_db_priorities_get(
                 unit,
                 tcam_db_id
               );
  soc_sand_SAND_SORTED_LIST_INFO_clear(priorities);

  priorities->init_info.prime_handle  = unit;
  priorities->init_info.sec_handle    = 0;
  priorities->init_info.list_size     = max_nof_entries;
  priorities->init_info.key_size      = SOC_PB_TCAM_DB_LIST_KEY_SIZE * sizeof(uint8);
  priorities->init_info.data_size     = SOC_PB_TCAM_DB_LIST_DATA_SIZE * sizeof(uint8);
  priorities->init_info.get_entry_fun = soc_pb_tcam_db_data_structure_entry_get;
  priorities->init_info.set_entry_fun = soc_pb_tcam_db_data_structure_entry_set;
  priorities->init_info.cmp_key_fun   = (prio_mode == SOC_PB_TCAM_DB_PRIO_MODE_BANK) ?
                                          soc_pb_tcam_db_priority_list_cmp_bank :
                                          soc_pb_tcam_db_priority_list_cmp_interlaced;
  ...
  entry_id_to_location->init_info.table_size    = max_nof_entries;
  entry_id_to_location->init_info.table_width   = max_nof_entries;
  entry_id_to_location->init_info.key_size      = SOC_PB_TCAM_DB_HASH_TBL_KEY_SIZE * sizeof(uint8);
  entry_id_to_location->init_info.data_size     = SOC_PB_TCAM_DB_HASH_TBL_KEY_SIZE * sizeof(uint8);
  entry_id_to_location->init_info.get_entry_fun = soc_pb_tcam_db_data_structure_entry_get;
  entry_id_to_location->init_info.set_entry_fun = soc_pb_tcam_db_data_structure_entry_set;
