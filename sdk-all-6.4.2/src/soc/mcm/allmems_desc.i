 ...
 */
    ...
    /* IPS_QUEUE_PRIORITY_TABLE  */ "The values in this table Determines if the Queue is High / Low priority and if it is OCB-Only (low latency).\n                   This affects the DQCQ context selection for this Q-type. Access to this table is according to Queue Type.",
    ...
    /* IPS_QUEUE_SIZE_TABLE      */ "Q size variable of the Q descriptor. The value is given in mantissa and exponent format, where M=q_size9:4 and E=q_size3:0. The value is (M x 2^E). The queue size is given in 16B resolution.\n                   Caution: This table is for debug purposes and should be used with user discretion as it can effect the IPS operation. Special consideration should be given when writing to this table.",
    ...
    /* IPS_SYSTEM_RED_MAX_QUEUE_SIZE_TABLE */ "Maximum queue size for system RED feature- One entry per system physical port reads the value received from credits.",
    ...
    /* IRE_CTXT_MAP              */ "Egress Recycling channel to incoming  Context mapping.\n                   Write 8'd255 to an invalid Recycling channel.",
    ...
    /* IRR_SNOOP_MIRROR_TM_TABLE */ "Snoop and Mirror table. This memory maps the incoming snoop command or mirror command to a replication. First half is for mirror, second half is for snoop.\n                   This table is split into two parts.\n                   First line of this table must be set to all 0's.",
    ...
    /* SCH_CH_NIF_CALENDAR_CONFIGURATION_CNCC */ "Holds the length and weight of the active calendar. Notce that the maximum valid value for calendars 31:4 is 255 (not 1023)",
    /* SCH_CH_NIF_CALENDAR_CONFIGURATION__CNCC */ "",
    /* SCH_CH_NIF_RATES_CONFIGURATION_CNRC */ "(Cnrc)",
    /* SCH_CH_NIF_RATES_CONFIGURATION__CNRC */ "",
    /* SCH_CIR_SHAPERS_DYNAMIC_TABEL_CSDT */ "INTERNAL",
    /* SCH_CIR_SHAPERS_STATIC_TABEL_CSST */ "(Csst)- Shaper define per each PG. holds Quanta to add and max-burst per each PG",
    /* SCH_CIR_SHAPERS_STATIC_TABEL__CSST */ "Shaper define per each PG. holds Quanta to add and max-burst per each PG",
    /* SCH_CIR_SHAPER_CALENDAR_CSC */ "(Csc) - Holds pointer to CSST",
    /* SCH_CIR_SHAPER_CALENDAR__CSC */ "Holds pointer to CSST",
    /* SCH_CL_SCHEDULERS_CONFIGURATION_SCC */ "This memory allocates to each of the 16k class schedulers one of 256 types. (See memory SCT).\n                   Entry i relates to class scheduler i.",
    ...
    /* SCH_CL_SCHEDULERS_TYPE_SCT */ "This memory defines for each class type exactly how the class scheduler will be configured. In the table below, n relates to set of values, while X relates to a range, as it is configured by the WFQ mode.",
    ...
    /* SCH_DUAL_SHAPER_MEMORY_DSM */ "Each entry contains data for 16 dual bucket schedulers.",
    /* SCH_DUAL_SHAPER_MEMORY__DSM */ "The DSM is a 1K-entry memory. Each entry contains data for 16 dual bucket schedulers.",
    /* SCH_FC_MAP_FCM            */ "(FCM) - Maps each of the 128 EGQ interface FC to each of 128 Scheduler interface FC, If more than one EGQ interface FC is mapped to one scheduer interface FC then they are bit-wise ORed",
    ...
    /* SCH_FLOW_DESCRIPTOR_MEMORY_STATIC_FDMS */ "Each entry contains the Flow-Attributes. There is one entry per sub-flow. Both Schedulers and flows share this memory context.\n                   The Flow-Attributes are set by the user. The Flow-Attributes depend on the type of scheduler this flow is attached to.",
    ...
    /* SCH_FLOW_TO_QUEUE_MAPPING_FQM */ "Each entry relates Four flows to one Base Queue, Based on SubFlowMode. If SubFlowMode is set, then four queues are mapped to four even flows. Base_flow15:2 is the address to the table.\n                   The mapping between the flow and the base queue is done\n                   Per four flows.",
    ...
    /* SCH_HR_SCHEDULER_CONFIGURATION_SHC */ "This memory describes an HR scheduler operating in single, dual, or Enhanced mode. It also holds the High/Low flow-control mode to be applied for the HR.",
    ...
    /* SCH_ONE_PORT_NIF_CONFIGURATION_OPNC */ "(Opnc)",
    /* SCH_ONE_PORT_NIF_CONFIGURATION__OPNC */ "",
    /* SCH_PIR_SHAPERS_DYNAMIC_TABEL_PSDT */ "INTERNAL",
    /* SCH_PIR_SHAPERS_STATIC_TABEL_PSST */ "(Psst) - Shaper define per each TC (HR). holds Quanta to add and max-burst per each HR",
    /* SCH_PIR_SHAPERS_STATIC_TABEL__PSST */ "Shaper define per each TC (HR). holds Quanta to add and max-burst per each HR",
    /* SCH_PIR_SHAPER_CALENDAR_PSC */ "(Psc) - Holds pointer to PSST",
    /* SCH_PIR_SHAPER_CALENDAR__PSC */ "Holds pointer to PSST",
    /* SCH_PORT_ENABLE_PORTEN    */ "(PORTEN) - Enables HR scheduler to function as Scheduler-Port.",
    ...
    /* SCH_PORT_SCHEDULER_WEIGHTS_PSW */ "(Psw) - Holds the weight of each PG, the weight is valid only if the relevant bit in the field PSM.PgWfqValid is set.\n                   In case of Strict Priority then PG with higher index will have higher priority (PG7 is highest SP)",
    ...
    /* SCH_SHAPER_DESCRIPTOR_MEMORY_STATIC_SHDS */ "This memory contains the Shaper-Flow-Attributes. Each entry contains the data for two flows. For each flow, this memory holds the Shaper-Flow-Attributes. The Flow-Attributes are set by the user and are never modified by the logic.\n                   The Shaper receives a credit every four clocks. The bandwidth given to a flow is the Rate (see calculation below) multiplied by the shaper bandwidth.",
    ...
    /* SCH_SHARED_DEVICE_RATE_SHARED_DRM */ "This momory defines the total BW of the two schedulers in case shared-DRM is used.\n                   RateGbps = (Credit_Size*8*128)/(clk_cycle_in_nano*SharedDrm)",
