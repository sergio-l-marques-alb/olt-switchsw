...
soc_pci_ep_config(int unit, int pcie)
{
    ...
    if (soc_feature(unit, soc_feature_iproc) &&
        ...
        if (pci_num == 0) {
            ...
            addr = soc_reg_addr_get(unit, PAXB_0_OARR_FUNC0_MSI_PAGEr,
                                    REG_PORT_ANY, 0, TRUE, &blk, &acc_type);
            rval = soc_cm_iproc_read(unit, addr);
            soc_cm_iproc_write(unit, addr, rval | 0x1);
        } else {
            ...
            addr = soc_reg_addr_get(unit, PAXB_1_OARR_FUNC0_MSI_PAGEr,
                                    REG_PORT_ANY, 0, TRUE, &blk, &acc_type);
            rval = soc_cm_iproc_read(unit, addr);
            soc_cm_iproc_write(unit, addr, rval | 0x1);
