/*
 * $Id: counter.c,v 1.239 Broadcom SDK $
                            ...
                            int *base_index, int *num_entries, char **cname)
{
    ...
    if ((IS_IL_PORT(unit, port)) && SOC_IS_CALADAN3(unit)) {
        /* IL counters support up to 64 channels + aggregate, total 65 counters per ctr_reg */
        *base_index = COUNTER_IDX_PORTBASE(unit, port) + soc->controlled_counters[ctr_reg].counter_idx * \
            (SOC_SBX_CALADAN3_IL_NUM_CHANNEL_STATS);
        *num_entries = SOC_SBX_CALADAN3_IL_NUM_CHANNEL_STATS;
    }
    else
...
#ifdef BCM_CALADAN3_SUPPORT
STATIC int
_soc_counter_caladan3_get_info(int unit, soc_port_t port, soc_reg_t ctr_reg,
                               int *base_index, int *num_entries, char **cname)
{
    int rv = SOC_E_UNAVAIL;
    soc_control_t   *soc = SOC_CONTROL(unit);

    if (IS_IL_PORT(unit, port) || IS_CPU_PORT(unit, port)) {
        if (ctr_reg < soc->soc_controlled_counter_all_num) {
        ...
        *base_index = COUNTER_IDX_PORTBASE(unit, port) + SOC_REG_CTR_IDX(unit, ctr_reg);
                      ...
                      int *base_index, int *num_entries, char **cname)
{
    ...
    if (SOC_IS_CALADAN3(unit)) {
        rv = _soc_counter_caladan3_get_info(unit, port, ctr_reg, base_index,
                                            num_entries, cname);
        return rv;
    }
#endif /* BCM_CALADAN3_SUPPORT */
        ...
        int non_dma_start_index, int *non_dma_entries)
{
    ...
    *non_dma_entries += non_dma0->num_entries;
    ...
    non_dma0 = &soc->counter_non_dma[SOC_COUNTER_NON_DMA_COSQ_PG_CELL_CNT -
                                     SOC_COUNTER_NON_DMA_START];
    non_dma0->flags = _SOC_COUNTER_NON_DMA_VALID;
    non_dma0->pbmp = PBMP_ALL(unit);
    non_dma0->base_index = non_dma_start_index + *non_dma_entries;
    ...
    non_dma0->num_entries = total_num_cosq;
    non_dma0->mem = INVALIDm;
    non_dma0->reg = PGCELLCNTr;
    non_dma0->field = CELLCNTf;
    non_dma0->cname = "PG_CELL_CNT";
    *non_dma_entries += non_dma0->num_entries;
...
soc_counter_attach(int unit)
{
    ...
    if (SOC_IS_HB_GW(unit)) {
        ...
        /* increase to support 1K 64bits counter per port,
         * so that IL port could have per channel controlled counters
         */
        portsize = 8192;
        ports64 = nports;
#endif
...
static sal_spinlock_t sl;
#define COUNTER_ATOMIC_DEF              sal_spinlock_t
#define COUNTER_ATOMIC_BEGIN(s)         do { \
                                            s = sl; \
                                            if (s) { \
                                                sal_spinlock_lock(s); \
                                            } \
                                        } while (0)
#define COUNTER_ATOMIC_END(s)           do { \
                                            s = sl; \
                                            if (s) { \
                                                sal_spinlock_unlock(s); \
                                            } \
                                        } while (0)
...
_soc_counter_fifo_process(int unit, central_ctr_eviction_fifo_entry_t *entry)
{
    ...
    non_dma = &soc->counter_non_dma[pkt_ctr_id - SOC_COUNTER_NON_DMA_START];
    fval[0] = soc_mem_field32_get(unit, mem, entry, PKT_CNTf);
    COMPILER_64_SET(ctr_diff, 0, fval[0]);
    base_index = non_dma->base_index + offset;

    COUNTER_ATOMIC_BEGIN(s);
    COMPILER_64_ADD_64(soc->counter_sw_val[base_index + index], ctr_diff);
    soc->counter_delta[base_index + index] = ctr_diff;
    COUNTER_ATOMIC_END(s);

    non_dma = &soc->counter_non_dma[byte_ctr_id - SOC_COUNTER_NON_DMA_START];
...
_soc_ctr_evict_fifo_dma_thread(void *unit_vp)
{
    ...
    soc_mem_t mem;
    soc_reg_t stat_reg, stat_clr_reg;
    uint32 stat_addr, stat_clr_addr;
    int i, count, adv_threshold;
    uint32 rval, stat_fail_mask, stat_clr_mask;
    ...
    intr_mask = IRQ_CMCx_FIFO_CH_DMA(chan);
    ...
    host_buff = soc_cm_salloc(unit, entries_per_buf * entry_words *
    ...
    rv = _soc_mem_sbus_fifo_dma_start(unit, chan, mem, MEM_BLOCK_ANY,
    ...
    stat_reg = CMIC_CMC0_FIFO_CH1_RD_DMA_STATr;
    stat_addr = CMIC_CMCx_FIFO_CHy_RD_DMA_STAT_OFFSET(cmc, chan);
    stat_fail_mask = 0;
    soc_reg_field_set(unit, stat_reg, &stat_fail_mask, HOSTMEM_TIMEOUTf, 1);
    soc_reg_field_set(unit, stat_reg, &stat_fail_mask, HOSTMEM_OVERFLOWf, 1);
    ...
    soc_reg_field_set(unit, stat_clr_reg, &stat_clr_mask, HOSTMEM_OVERFLOWf,
                      1);
    ...
    while ((interval = soc->ctr_evict_interval)) {
        ...
        do {
            ...
            rv = _soc_mem_sbus_fifo_dma_get_num_entries(unit, chan, &count);
            if (SOC_SUCCESS(rv)) {
                non_empty = TRUE;
                ...
                for (i = 0; i < count; i++, yield_num--) {
                    ...
                    _soc_counter_fifo_process(unit, host_entry);
...
cleanup_exit:
    (void)_soc_mem_sbus_fifo_dma_stop(unit, chan);
...
soc_ctr_evict_start(int unit, uint32 flags, sal_usecs_t interval)
{
    ...
    soc_counter_tomahawk_eviction_flags_update(unit, flag, interval ? 1 : 0);
...
soc_counter_eviction_sync(int unit)
{
     ...
     */
    (void)soc_cmicm_intr0_disable(unit, IRQ_CMCx_FIFO_CH_DMA(chan));
    sal_sem_give(soc->ctrEvictIntr);
...
#ifdef BCM_TOMAHAWK_SUPPORT
STATIC int
_soc_th_xpe_counter_get(int unit, soc_port_t port, soc_reg_t ctr_reg, int ar_idx,
                    int zero, int sync_mode, uint64 *val)
{
    ...
    /* coverity fix : execution does not go inside if statement
    so changing from && to || */
    if ((ctr_reg < SOC_COUNTER_NON_DMA_START) ||
        (ctr_reg >= SOC_COUNTER_NON_DMA_END)) {
        /* only for Non-stat Dma style counters */
        LOG_WARN(BSL_LS_SOC_COUNTER,
                 (BSL_META_U(unit,
                             "ERR %s(%d): Invalid Param. Id %d\n"),
                  __FUNCTION__, __LINE__, ctr_reg));
        return SOC_E_PARAM;
    }

    mem = soc->counter_non_dma[ctr_reg - SOC_COUNTER_NON_DMA_START].mem;
    pipe = si->port_pipe[port];
    COMPILER_64_ZERO(*val);

    for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
        if (SOC_MEM_UNIQUE_ACC_XPE_PIPE(unit, mem, xpe, pipe) == INVALIDm) {
            /* XPE/PIPE combination is NOT present. */
            continue;
        }

        /* Encoding offset into ctr_reg. */
        ctr_reg_offset |= xpe << 24;

        SOC_IF_ERROR_RETURN(_soc_counter_get_info(unit, port, ctr_reg_offset,
...
#ifdef BCM_TOMAHAWK_SUPPORT
    /* Feature check for XPE needs to be added */
    if (((ctr_reg >= SOC_COUNTER_NON_DMA_START) &&
         (ctr_reg < SOC_COUNTER_NON_DMA_END)) &&
        (soc_tomahawk_mem_is_xpe(unit,
                soc->counter_non_dma[ctr_reg - SOC_COUNTER_NON_DMA_START].mem))) {
        return _soc_th_xpe_counter_get(unit, port, ctr_reg, ar_idx,
                                   zero, sync_mode, val);
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
 ...
 * Purpose:
 *      Given an input array of heterogenous soc_ctr_control_info_t 
 *      datastructures, encode them into one single output
 *      soc_ctr_control_info_t datastructure.
 * Parameters:
 *      input_ctrl_info  - (IN) Input array of ctrl_info variables to be 
 *                          encoded.
 *      count            - (IN) Number of ctrl_info variables to be encoded.
 *      output_ctrl_info - (OUT) Encoded output ctrl_info datastructure.
 * Returns:
 *      SOC_E_XXX.
 * Notes:
                         ...
                         int ar_idx, int zero, int sync_mode, uint64 *val)
{
    ...
    SOC_IF_ERROR_RETURN(soc_counter_generic_get_info(unit, ctrl_info,ctr_reg,
                                                     &base_index,&num_entries));
    if ((ar_idx >= num_entries) || (ar_idx < 0)) {
        return SOC_E_PARAM;
    }
    ...
    if ((ctr_reg >= SOC_COUNTER_NON_DMA_START) && 
        ...
        if (ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_XPE) {
            ...
            for (xpe = 0; xpe < NUM_XPE(unit) ; xpe++) {
                ...
                rv = _soc_counter_generic_get(unit, output_ctrl_info, ctr_reg,
                        ar_idx, FALSE, sync_mode, &value);
                if (rv != SOC_E_NONE) {
                    return rv;
                }
    ...
    if (!SOC_IS_PETRAB(unit) && !_SOC_CONTROLLED_COUNTER_USE(unit, port)) {
        ...
        if (ctr_reg >= NUM_SOC_REG) {
            ...
            if (non_dma->mem != INVALIDm) {
                ...
                for (i = 1; i < SOC_INFO(unit).num_pipe; i++) {
        ...
        int ar_idx, uint64 val)
{
    ...
    uint32              val32;
    soc_port_t port = -1 ;
    int subset_ct = 0, pipe_iter = 0, pipe_ct = 0 ;

    if (ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_PORT) {
        port = ctrl_info.instance;
    }

    SOC_IF_ERROR_RETURN(soc_counter_generic_get_info(unit, ctrl_info, id,
                &base_index, &num_entries));
        ...
        if (id >= NUM_SOC_REG) {
            ...
            if (!(non_dma->flags & _SOC_COUNTER_NON_DMA_VALID)) {
                ...
                if ((ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_POOL)
                     ...
                     * parent-child model */
                    entries_per_pipe = num_entries/(NUM_PIPE(unit));
                    if (ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_POOL) {
            ...
            if (non_dma->mem != INVALIDm) {
                ...
                if (pipe_ct) {
                    ...
                    for (pipe_iter = 0; pipe_iter <= pipe_ct; pipe_iter++,
                            base_index += entries_per_pipe) {
                        dma_mem = non_dma->dma_mem[pipe_iter];
                        ...
                        fval[0] = COMPILER_64_LO(val);
                        fval[1] = COMPILER_64_HI(val);
                        soc_mem_field_set(unit, dma_mem, entry,
                                non_dma->field, fval);
...
#ifdef BCM_CALADAN3_SUPPORT
        if ((SOC_IS_CALADAN3(unit)) &&
                (IS_IL_PORT(unit, port) || IS_CPU_PORT(unit, port))) {
            continue;
        }
#endif
 ...
 */
STATIC int
soc_petra_port_counter_get(int unit, soc_port_t port, int ctr_type, uint64 *value)
{
    int                         rv, soc_sand_rv;
    ...
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port_to_interface_map_get, (unit, core, port, &if_id, NULL));
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    int index_min = 0, index_max = 0, pipe, entries_per_pipe;
            ...
            do {
                ...
                if (ctr_reg == INVALIDr) {
                    ...
                    if (non_dma->mem == MMU_CTR_COLOR_DROP_MEMm) {
                        entries_per_pipe = 68;
                        index_min = non_dma->dma_index_min[0];
                        index_max = non_dma->dma_index_max[0] + 1;
                        num_non_dma_entries = index_max - index_min;
                    }
                ...
                if ((non_dma->flags & _SOC_COUNTER_NON_DMA_FLEX_MAPPING)
                    ...
                    for (i = 0 ; i < num_non_dma_entries; i++) {
                        ...
                        if (!non_dma->dma_buf[pipe]) {
                             ...
                             * is NULL.
                             * Also -1 because for-loop increases index by 1 after continue (-1).
                             */
                ...
                count++;
                non_dma++;
            } while ((count < (subset_ct - 1)) && non_dma);
        } else if (non_dma->reg != INVALIDr) {
            pbmp_t counter_pbmp;
...
_soc_counter_sbusdma_setup(int unit)
{
    ...
    ing_addr = soc_reg_addr_get(unit, ing_reg, REG_PORT_ANY, 0, FALSE, &ing_blkoff,
                                &ing_acc_type);
...
#define _EP_COUNTERS "EP COUNTERS"
        ...
        if (SOC_BLOCK_IS_CMP(unit, SOC_PORT_BLOCK(unit, phy_port), SOC_BLK_GPORT)||
            ...
            mac_addr = soc_reg_addr_get(unit, gmac_reg, port, 0, FALSE, &mac_blkoff,
                                        &mac_acc_type);
            cfg.count = gmac_c_cnt;
            cfg.width = 1;
        } else {
            mac_addr = soc_reg_addr_get(unit, mac_reg, port, 0, FALSE, &mac_blkoff,
                                        &mac_acc_type);
            cfg.count = mac_c_cnt;
            cfg.width = 2;
        }
...
#define _BLK_CTRS "BLK CTRS"
                ...
                for (i=0; i<cnt; i++) {
                    cfg[i].addr = soc_reg_addr_get(unit,
                                                   ...
                                                   REG_PORT_ANY, 0, FALSE, &ctr_blk, &at);
                    cfg[i].blk = ctr_blk;
...
soc_counter_thread(void *unit_vp)
{
    ...
    sl = sal_spinlock_create("counter spinlock");
                                            ...
                                            soc_reg_t ctr_reg)    
{
    ...
    int non_dma_counter_end = SOC_COUNTER_NON_DMA_END - 
                              SOC_COUNTER_NON_DMA_START;
    static int ctr_dma_ct = 0;
    int index_max = 0, pipe = 0, entries_per_pipe = 0;
    uint32 subset_ct = 0; /* For Parent-Child Model */
                ...
                if ((non_dma->flags & _SOC_COUNTER_NON_DMA_CTR_EVICT) &&
                    ...
                    LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                             (BSL_META_U(unit,
                              ...
                              id, SOC_COUNTER_NON_DMA_START, count,
                              NUM_PIPE(unit), non_dma->mem,
                              non_dma->dma_mem[0], non_dma->dma_mem[1]));count++;
                    non_dma++;
                    continue;
                }
                    LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                ...
                if (ctr_reg == INVALIDr) {
                    ...
                    if (ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_PORT) {
                        ...
                        rv = soc_mem_read(unit,
                                          non_dma->dma_mem[ctrl_info.instance],
                                          MEM_BLOCK_ANY, ar_idx,
                                          non_dma->dma_buf[ctrl_info.instance]);
                        num_non_dma_entries = 1;
