...
typedef struct soc_sbx_caladan3_sws_dbase_s {
    ...
    int icc_reserved_rules[SOC_SBX_CALADAN3_SWS_MAX_PR_INSTANCE];
...
int
sws_pr_icc_reserved_rules_get(int unit, int instance) 
{
    if ((unit < 0) || (unit >= SOC_MAX_NUM_DEVICES)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "sws_pr_icc_reserved_rules_get: Invalid unit\n")));
        return 0;
    }
    instance &= 1;
    return (sws_dbase[unit]->icc_reserved_rules[instance]);
}

void
sws_pr_icc_reserved_rules_set(int unit, int instance, int idx) 
{
    if ((unit < 0) || (unit >= SOC_MAX_NUM_DEVICES)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "sws_pr_icc_reserved_rules_set: Invalid unit\n")));
        return;
    }
    instance &= 1;
    sws_dbase[unit]->icc_reserved_rules[instance] = idx;
}
...
soc_sbx_caladan3_sws_find_queue_range(int unit, int range, int type, int *start_queue) 
{
    ...
    if ((range < SOC_SBX_CALADAN3_SWS_LINE_SQUEUE_BASE) ||
          (range > SOC_SBX_CALADAN3_SWS_QUEUE_PER_REGION)) {
         LOG_ERROR(BSL_LS_SOC_COMMON,
                   (BSL_META_U(unit,
...
{
    int idx, num_cos COMPILER_ATTRIBUTE((unused));
...
soc_sbx_caladan3_sws_config_dump(int unit) 
{
    ...
    LOG_CLI((BSL_META_U(unit,
                        "      Reserved rules  : %d \n"), sws_dbase[unit]->icc_reserved_rules[0]));
    LOG_CLI((BSL_META_U(unit,
                        "  PR1 ICC: \n")));
    LOG_CLI((BSL_META_U(unit,
                        "      State           : %s \n"), (sws_dbase[unit]->icc_bypass[1]) ? "Bypassed" : "Enabled"));
    LOG_CLI((BSL_META_U(unit,
                        "      Reserved rules  : %d \n"), sws_dbase[unit]->icc_reserved_rules[1]));
    LOG_CLI((BSL_META_U(unit,
                        "  Reserved Queues:\n")));
    LOG_CLI((BSL_META_U(unit,
                        "      Redirect Queues:\n")));
