...
/*
 * $Id: interrupt_handler_corr_act_func.c,v 1.38 Broadcom SDK $
 ...
 */
int
arad_interrupt_handles_corrective_action_do_nothing (
  int unit,
  int block_instance,
...
int arad_interrupt_handles_corrective_action_hard_reset(
    int unit,
    int block_instance,
    arad_interrupt_type interrupt_id,
    char *msg)
                                                                       ...
                                                                       char* msg)
{
    ...
    rc = _bcm_dpp_oam_fifo_interrupt_handler(unit);
    BCMDNX_IF_ERR_EXIT(rc);
        ...
        if(tmp_fap_id == fap_id) {
            ...
            for(n_flow=0; n_flow < ARAD_INTERRUPTS_NOF_FLOWS_PER_FFM_ENTRY ; n_flow++) {
                ...
                flow = ndx * ARAD_INTERRUPTS_NOF_FLOWS_PER_FFM_ENTRY + n_flow;
                    ...
                    rc = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_sch_flow_status_set, (unit, flow, ARAD_SCH_FLOW_OFF));
    ...
    char* special_msg)
{
    ...
    rc = arad_port_control_pcs_get( unit, link, p_pcs);
    SOCDNX_IF_ERR_EXIT(rc);

    if(ARAD_PORT_PCS_8_10!= (*p_pcs))
    {
...
int arad_interrupt_data_collection_for_recurring_mac_oof_int( 
    ...
    arad_interrupt_type en_arad_interrupt,
    ARAD_PORT_PCS* p_pcs,
    arad_int_corr_act_type* p_corrective_action,
    char* special_msg,
    uint8*  array_links)
{
    int rc;
    ...
    *p_corrective_action = ARAD_INT_CORR_ACT_NONE ;
    ...
    rc = arad_port_control_pcs_get( unit, block_instance ,p_pcs);
    SOCDNX_IF_ERR_EXIT(rc);
    ...
    if(ARAD_PORT_PCS_8_10!= (*p_pcs))
    {
        sal_sprintf(special_msg, "The receiver frame decoder lost of frame alignment for link %d", link);
        array_links[link] = ARAD_INT_LINK_SHUTDOWN;
        *p_corrective_action = ARAD_INT_CORR_ACT_SHUTDOWN_FBR_LINKS;
    }
    else{
        sal_sprintf(special_msg, "Out of frame interrupt received for link %d for for encoding type 8/10, for this mode interrupt should be masked ",link);
    }
    ...
    uint8* array_links)
{
    ...
    *p_corrective_action = ARAD_INT_CORR_ACT_NONE;
    ...
    rc = arad_port_control_pcs_get( unit, block_instance ,&pcs);
    SOCDNX_IF_ERR_EXIT(rc);
...
arad_interrupt_data_collection_for_shadowing(
    ...
    char* special_msg, 
    arad_int_corr_act_type* p_corrective_action,
    arad_interrupt_mem_err_info* shadow_correct_info)
{
    int rc;
    uint64 counter;
    uint32 counter32;
    ...
    if (addr_validf != 0) {
        ...
        */
        if((  (en_arad_interrupt == ARAD_INT_EGQ_PARITYERRINT)  || (en_arad_interrupt == ARAD_INT_EGQ_ECC_2BERRINT) ) 
            ...
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x",
        ...
        */
         if ((en_arad_interrupt == ARAD_INT_IRE_PARITYERRINT) && 
            ...
            *p_corrective_action = ARAD_INT_CORR_ACT_INGRESS_HARD_RESET;
            rc = SOC_E_NONE;
            SOC_EXIT;       
        }
        ...
        switch(mem) {
        case INVALIDm:
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory is not accessible",
                    cntf, cnt_overflowf, addrf);
            break;     
        ...
        case IPS_QDESCm:
            ...
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory=%s, array element=%d, index=%d",
                        cntf, cnt_overflowf, addrf, memory_name, numels, index);
            /* set corrective action */
            *p_corrective_action = ARAD_INT_CORR_ACT_IPS_QDESC;
        ...
        default:
            ...
            rc = interrupt_memory_cached(unit, mem, COPYNO_ALL,  &cached_flag);
            if(rc != SOC_E_NONE) {
                sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory not cached",
                            cntf, cnt_overflowf, addrf);
            }
                ...
                sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory=%s, array element=%d, index=%d",
                            cntf, cnt_overflowf, addrf, memory_name, numels, index);
                /* set corrective action */
                *p_corrective_action = ARAD_INT_CORR_ACT_SHADOW;
                shadow_correct_info->array_index = numels;
                shadow_correct_info->copyno = COPYNO_ALL;
                shadow_correct_info->min_index = index;
                shadow_correct_info->max_index = index;
                ...
                sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory=%s",
                            cntf, cnt_overflowf, addrf, memory_name);
            }
        }
    
     } else {
         sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x address is not valid",
                     cntf, cnt_overflowf, addrf);
     }
...
arad_interrupt_data_collection_for_ecc_1b(
    ...
    char* special_msg, 
    arad_int_corr_act_type* p_corrective_action,
    arad_interrupt_mem_err_info* ecc_1b_correct_info)
{
    int rc;
    uint64 counter;
    uint32 counter32;
    ...
    *p_corrective_action = ARAD_INT_CORR_ACT_NONE;
    ...
    if (addr_validf != 0) {
        ...
        switch (mem) {
        case INVALIDm:
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x memory not accessible",
                        cntf, cnt_overflowf, addrf);
        break;
        ...
        case IPS_QDESCm:
            ...
            *p_corrective_action = ARAD_INT_CORR_ACT_IPS_QDESC;
            ...
            ecc_1b_correct_info->copyno = COPYNO_ALL;
        ...
        default:    
            ...
            SOCDNX_IF_ERR_EXIT(rc);
             ...
             ecc_1b_correct_info->max_index = index;
             ecc_1b_correct_info->mem = mem;
             *p_corrective_action = ARAD_INT_CORR_ACT_ECC_1B_FIX;
        }
     } else {
         sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, memory address=0x%08x address is not valid",
                     cntf, cnt_overflowf, addrf);
...
arad_interrupt_data_collection_for_nbi_link_status_change(
    ...
    int en_arad_interrupt,
    char *special_msg,
    arad_int_corr_act_type *corrective_action,
    soc_port_t *port)
{
    uint32 phy_port;
    soc_port_t port_i;
    ...
    *corrective_action = ARAD_INT_CORR_ACT_NONE;
    ...
    *port = -1;
    ...
    PBMP_ITER(ports_bm, port_i) {
        ...
        if (phy_port - 1 /*1 base to 0 base*/ == lane) {
            *port = port_i;
            *corrective_action = ARAD_INT_CORR_ACT_RX_LOS_HANDLE;
            break;  
        }
    }
...
{    
   ...
   rc = soc_reg32_get(unit, MMU_INTERRUPT_REGISTERr, block_instance, 0, &err_cnt);
    ...
    if(err_field_cnt) {
        ...
        *p_corrective_action = ARAD_INT_CORR_ACT_CONFIG_DRAM;
    ...
    *cell_type = 0x0;
    ...
    if( SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        ...
        if ( soc_reg_field_get(unit, FCT_UNREACHABLE_DESTINATIONr, unr_dst, UNRCH_DEST_EVTf) && *cell_type ==  ARAD_INTERRUPTS_FCT_UNREACHABLE_DEST_CELL_TYPE_CREDIT) {
            *corrective_action = ARAD_INT_CORR_ACT_SHUTDOWN_UNREACH_DESTINATION;
        }
    }
...
int arad_interrupt_data_collection_for_ihbtcamprotectionerror(int unit,
                                                              int block_instance,
                                                              arad_interrupt_type interrupt_id,
                                                              ...
                                                              arad_int_corr_act_type* corrective_action)
{
     ...
     */
    rc = soc_reg32_get(unit, IHB_TCAM_PROTECTION_ERRORr, block_instance, 0, &reg_value);
    SOCDNX_IF_ERR_EXIT(rc);

    fld_val_bank = soc_reg_field_get(unit, IHB_TCAM_PROTECTION_ERRORr, reg_value, TCAM_PROTECTION_ERROR_TCAM_BANKf);
    fld_val_entry = soc_reg_field_get(unit, IHB_TCAM_PROTECTION_ERRORr, reg_value, TCAM_PROTECTION_ERROR_TCAM_ENTRY_ADDf);

    /* first get bank entry size to adjust error address */
    rc = arad_sw_db_tcam_bank_entry_size_get(unit, fld_val_bank, &entry_size);
    SOCDNX_IF_ERR_EXIT(rc);
    ...
    *corrective_action = ARAD_INT_CORR_ACT_TCAM_SHADOW_FROM_SW_DB;
...
int
arad_interrupt_handles_corrective_action_print(
    int unit,
    int block_instance,
    arad_interrupt_type interrupt_id,
    char* msg_print,
    char* msg)
{
    ...
    char print_msg[ARAD_INTERRUPT_PRINT_MSG_SIZE];

    SOCDNX_INIT_FUNC_DEFS;

    if (!SOC_UNIT_VALID(unit)) {
        BCMDNX_ERR_EXIT_MSG(SOC_E_UNIT, (_BSL_BCM_MSG("invalid unit")));
    }
...
arad_interrupt_handles_corrective_action_ips_qdesc(
    ...
    int block_instance,
    arad_interrupt_type interrupt_id,
    arad_interrupt_mem_err_info* shadow_correct_info_p,
    char* msg)
{
    int rc;
    uint32* data_entry=0;
    ...
    for (current_index = shadow_correct_info_p->min_index ; current_index <= shadow_correct_info_p->max_index ; current_index++) {
        ...
        soc_sand_rc = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ipq_queue_to_flow_mapping_get,(unit, SOC_TMC_IPQ_Q_TO_QRTT_ID(current_index), &queue2flow));
        ...
        SOCDNX_IF_ERR_EXIT(WRITE_IPS_ENABLE_DYNAMIC_MEMORY_ACCESSr(unit, REG_PORT_ANY, 0));
        SOCDNX_IF_ERR_EXIT(rc);
    }
...
int 
arad_interrupt_handles_corrective_action_shadow(
    int unit,
    int block_instance,
    arad_interrupt_type interrupt_id,
    arad_interrupt_mem_err_info* shadow_correct_info_p,
    char* msg)
{
    ...
    if (arad_tbl_is_dynamic(shadow_correct_info_p->mem)) {
        SOC_EXIT;
    }
...
int 
arad_interrupt_handles_corrective_action_for_ecc_1b(
    int unit,
    int block_instance,
    arad_interrupt_type interrupt_id,
    arad_interrupt_mem_err_info* ecc_1b_correct_info_p,
    char* msg)
{
    int rc;
    uint32* data_entry=0;
    ...
    if (arad_tbl_is_dynamic(ecc_1b_correct_info_p->mem)) {
        SOC_EXIT;
    }

    entry_dw = soc_mem_entry_words(unit, ecc_1b_correct_info_p->mem);
    data_entry = sal_alloc((entry_dw * 4), "ARAD_INTERRUPT ecc 1 data entry allocation");
...
/* when a link changes its status from up to down, the RTP_DRMA/CRMA/RRMA tables needs to be updated */
...
int arad_interrupt_handles_corrective_action_rtp_link_mask_change( 
   int unit){  
    ...
    SOCDNX_IF_ERR_EXIT(READ_RTP_LINK_ACTIVE_MASKr(unit,&rtp_link_active_mask_val));
    ...
    if (SOC_IS_ARADPLUS(unit))
    {
        ...
        SHR_BITCOPY_RANGE(rtp_mem_mask,BYTES2BITS(sizeof(uint32)),&mask_hi,BYTES2BITS(sizeof(uint32)),SOC_DPP_DEFS_GET(unit, nof_fabric_links)-BYTES2BITS(sizeof(uint32)));
        SHR_BITCOPY_RANGE(rtp_mem_mask,SOC_DPP_DEFS_GET(unit, nof_fabric_links),&mask_lo,0,BYTES2BITS(sizeof(uint32))); 
        SHR_BITCOPY_RANGE(rtp_mem_mask,BYTES2BITS(sizeof(uint32))+SOC_DPP_DEFS_GET(unit, nof_fabric_links),&mask_hi,BYTES2BITS(sizeof(uint32)),SOC_DPP_DEFS_GET(unit, nof_fabric_links)-BYTES2BITS(sizeof(uint32)));

        for (i=0; i<soc_mem_index_count(unit,RTP_DRMAm);i++)
        {
            
            SOCDNX_IF_ERR_EXIT(READ_RTP_DRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val)); 
            SOC_REG_ABOVE_64_AND(rtp_mem_val, rtp_mem_mask);
            SOCDNX_IF_ERR_EXIT(WRITE_RTP_DRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val));
        }
        for (i=0; i<soc_mem_index_count(unit,RTP_CRMAm);i++)
        {
            SOCDNX_IF_ERR_EXIT(READ_RTP_CRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val)); 
            SOC_REG_ABOVE_64_AND(rtp_mem_val, rtp_mem_mask);
            SOCDNX_IF_ERR_EXIT(WRITE_RTP_CRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val));

        }
        for (i=0; i<soc_mem_index_count(unit,RTP_RRMAm);i++)
        {
            SOCDNX_IF_ERR_EXIT(READ_RTP_RRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val)); 
            SOC_REG_ABOVE_64_AND(rtp_mem_val, rtp_mem_mask);
            SOCDNX_IF_ERR_EXIT(WRITE_RTP_RRMAm(unit,MEM_BLOCK_ANY,i,rtp_mem_val));
        }
    }
...
int arad_interrupt_handles_corrective_action_tcam_shadow_from_sw_db(
    int unit,
    int block_instance,
    arad_interrupt_type interrupt_id,
    ARAD_TCAM_LOCATION* location,
    char* msg)
{
    ...
    rc = arad_sw_db_tcam_bank_valid_get(unit, location->bank_id, &bank_valid);
    SOCDNX_SAND_IF_ERR_EXIT(rc);

    if (bank_valid)
    {
        /* Validate this entry is in Occupation Bitmap */
        occ_bm = arad_sw_db_tcam_bank_entries_used_get(unit, location->bank_id, 0);
        if (occ_bm == NULL) {
    ...
    rc = arad_sw_db_tcam_bank_entry_size_get(unit, location->bank_id, &entry_size);
    SOCDNX_SAND_IF_ERR_EXIT(rc);
    ...
    for (entry_ndx = 0; entry_ndx < nof_entries; entry_ndx++)
    {
        ...
        rc = soc_sand_occ_bm_is_occupied(occ_bm, location->entry, &found);
        SOCDNX_SAND_IF_ERR_EXIT(rc);
        ...
        if (found)
        {
            ...
            rc = arad_sw_db_tcam_global_location_tbl_get(unit, global_location_id, &global_location);
            SOCDNX_SAND_IF_ERR_EXIT(rc);
        }
        ...
        case ARAD_TCAM_BANK_OWNER_TT:
             ...
             * VTT tcam management is done using hash table (like in FLP), however,
             * in VTT the entry values are saved in the hash table without the mask.
             * This issue should be fixed, and when it is, the support to the
             * TCAM error protection for VTT will be added as well.
             */
...
arad_interrupt_handles_corrective_action(
    ...
    arad_interrupt_type interrupt_id,
    char *msg,
    arad_int_corr_act_type corr_act,
    void *param1, 
    void *param2)
{
    int rc;
    ...
    switch(corr_act) {
    case ARAD_INT_CORR_ACT_NONE:
        rc = arad_interrupt_handles_corrective_action_do_nothing(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;

    case ARAD_INT_CORR_ACT_FORCE:
        rc = arad_interrupt_handles_corrective_action_force(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;

    case ARAD_INT_CORR_ACT_HARD_RESET:
        rc = arad_interrupt_handles_corrective_action_hard_reset(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;

    case ARAD_INT_CORR_ACT_INGRESS_HARD_RESET:
        rc = arad_interrupt_handles_corrective_action_ingress_hard_reset(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
        
    case ARAD_INT_CORR_ACT_SHUTDOWN_FBR_LINKS:
        rc = arad_interrupt_handles_corrective_action_shutdown_fbr_link(unit, block_instance, interrupt_id,(uint8*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_SHADOW:
        rc = arad_interrupt_handles_corrective_action_shadow(unit, block_instance, interrupt_id,(arad_interrupt_mem_err_info*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_PRINT:
        rc = arad_interrupt_handles_corrective_action_print(unit, block_instance, interrupt_id,(char*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_REPROGRAM_RESOURCE:
        rc = arad_interrupt_handles_corrective_action_reprogram_resource(unit, block_instance, interrupt_id,(void*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_CONFIG_DRAM:
        rc = arad_interrupt_handles_corrective_action_reprogram_resource(unit, block_instance, interrupt_id,(void*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_ECC_1B_FIX:
        rc = arad_interrupt_handles_corrective_action_for_ecc_1b(unit, block_instance, interrupt_id, (arad_interrupt_mem_err_info * )param1, msg); 
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_HANDLE_CRC_DEL_BUF_FIFO:
        rc = arad_interrupt_handles_corrective_action_handle_crc_del_buf_fifo(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_HANDLE_OAMP_EVENT_FIFO:
        rc = arad_interrupt_handles_corrective_action_handle_oamp_event_fifo(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_CLEAR_CHECK:
        rc = arad_interrupt_handles_corrective_action_clear_check(unit, block_instance, interrupt_id,(int*)param1, (int*)param2);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_RX_LOS_HANDLE:
        /* get link number */
        ...
        break;
    case ARAD_INT_CORR_ACT_SHUTDOWN_UNREACH_DESTINATION:
        rc = arad_interrupt_handles_corrective_action_shutdown_unreach_destination(unit, block_instance, interrupt_id, *((uint32*)param1));
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_HANDLE_MACT_EVENT_FIFO:
        rc = arad_interrupt_handles_corrective_action_handle_mact_event_fifo(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break; 

    case ARAD_INT_CORR_ACT_HARD_RESET_WITHOUT_FABRIC:
        rc = arad_interrupt_handles_corrective_action_hard_reset_without_fabric(unit, block_instance, interrupt_id, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_RTP_LINK_MASK_CHANGE:
        rc=arad_interrupt_handles_corrective_action_rtp_link_mask_change(unit);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_TCAM_SHADOW_FROM_SW_DB:
        rc=arad_interrupt_handles_corrective_action_tcam_shadow_from_sw_db(unit, block_instance, interrupt_id,(ARAD_TCAM_LOCATION*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    case ARAD_INT_CORR_ACT_IPS_QDESC:
        rc = arad_interrupt_handles_corrective_action_ips_qdesc(unit, block_instance, interrupt_id,(arad_interrupt_mem_err_info*)param1, msg);
        BCMDNX_IF_ERR_EXIT(rc);
        break;
    default:
    ...
    arad_int_corr_act_type* p_corrective_action)
{
    ...
    if(multiple_errf || double_bit_errf) {
        *p_corrective_action = ARAD_INT_CORR_ACT_HARD_RESET;
...
/*
 *  Common utilities for the same interrupts (data collection...)
 */


/* Standard utilities */

int arad_interrupt_print_info(
      int unit,
    int block_instance,
    arad_interrupt_type en_arad_interrupt,
    ...
    char *special_msg)
{
    ...
    char cur_special_msg[ARAD_INTERRUPT_SPECIAL_MSG_SIZE];
    char cur_corr_act_msg[ARAD_INTERRUPT_COR_ACT_MSDG_SIZE];
    char print_msg[ARAD_INTERRUPT_PRINT_MSG_SIZE];
    ...
    interrupt = &(SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[en_arad_interrupt]);

    if (special_msg == NULL) {
        sprintf(cur_special_msg, "None");
    } else {
        if(sal_strlen(special_msg) >= ARAD_INTERRUPT_SPECIAL_MSG_SIZE) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("Too long special message"))); 
        }
        sal_strcpy(cur_special_msg, special_msg);
    }

    rc = soc_interrupt_flags_get(unit, en_arad_interrupt, &flags);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    switch(corr_act) {
        ...
        case ARAD_INT_CORR_ACT_HARD_RESET:
            ...
            break;
        case ARAD_INT_CORR_ACT_INGRESS_HARD_RESET:
            sal_sprintf(cur_corr_act_msg, "Ingress Hard Reset");
            break;
        case ARAD_INT_CORR_ACT_SHUTDOWN_FBR_LINKS:
        ...
        case ARAD_INT_CORR_ACT_SHADOW:
            ...
            break;
        case ARAD_INT_CORR_ACT_REPROGRAM_RESOURCE:
            sal_sprintf(cur_corr_act_msg, "Reprogram Resource");
            break;
        case ARAD_INT_CORR_ACT_CONFIG_DRAM:
            sal_sprintf(cur_corr_act_msg, "Config Dram");
            break;
        case ARAD_INT_CORR_ACT_ECC_1B_FIX:
            sal_sprintf(cur_corr_act_msg, "ECC 1B Correct");
            break; 
        case ARAD_INT_CORR_ACT_HANDLE_CRC_DEL_BUF_FIFO:
            sal_sprintf(cur_corr_act_msg, "Handle CRC deleted buffers FIFO");
            break;
        case ARAD_INT_CORR_ACT_HANDLE_OAMP_EVENT_FIFO:
            sal_sprintf(cur_corr_act_msg, "Handle OAMP Event Fifo");
            break;
        case ARAD_INT_CORR_ACT_RX_LOS_HANDLE:
            sal_sprintf(cur_corr_act_msg, "RX los handle");
            break;
        case ARAD_INT_CORR_ACT_SHUTDOWN_UNREACH_DESTINATION:
            sal_sprintf(cur_corr_act_msg, "Shutdown unreach destination handle");
            break;
        case ARAD_INT_CORR_ACT_HANDLE_MACT_EVENT_FIFO:
            sal_sprintf(cur_corr_act_msg, "Handle MACT Event Fifo");
            break;
        case ARAD_INT_CORR_ACT_HARD_RESET_WITHOUT_FABRIC:
            sal_sprintf(cur_corr_act_msg, "Hard Reset Without Fabric");
            break;
        case ARAD_INT_CORR_ACT_CLEAR_CHECK:
            sal_sprintf(cur_corr_act_msg, "Clear & Wait");
            break;
        case ARAD_INT_CORR_ACT_TCAM_SHADOW_FROM_SW_DB:
            sal_sprintf(cur_corr_act_msg, "TCAM Shadow from SW DB");
            break;
        case ARAD_INT_CORR_ACT_IPS_QDESC:
            sal_sprintf(cur_corr_act_msg, "IPS QDESC clear unused");
            break;
        default:
    ...
    sal_sprintf(print_msg,"id=%d, index=%d, block=%d, unit=%d, recurring_action=%d | %s | %s\n ", 
                en_arad_interrupt, interrupt->reg_index, block_instance, unit, recurring_action, cur_special_msg, cur_corr_act_msg);
#else
    sal_sprintf(print_msg,"name=%s, id=%d, index=%d, block=%d, unit=%d, recurring_action=%d | %s | %s\n ", 
                interrupt->name, en_arad_interrupt, interrupt->reg_index, block_instance, unit, recurring_action, cur_special_msg, cur_corr_act_msg);
#endif
