        ...
        if (!sal_strcasecmp(_ARG_CUR(a), "detach")) {
            ...
            if ((c = ARG_CUR(a)) != NULL) {
                ...
                DPORT_BCM_PBMP_ITER(u, pbm, dport, p) {
                    ...
                    cli_out("%5s(%3d) %9s %14d %14d %12d %14d %12d %14d\n",
                            SOC_PORT_NAME(u, p), p,
        ...
        for (vt_i = vt_max; vt_i >= vt_min; vt_i--) {
            ...
            for (hz_i = hz_min; hz_i <= hz_max; hz_i++) {
                ...
                if (time_tot == 0 && error_cnt == 0) {
                     ...
                     * 'A' BER ~10-12
                     */
                    if (pc->phy_id0 == 0x600d && pc->phy_id1 == 0x8500) {
                        if (time_tot == 0) {
                            legend_ndx = 0;
                        } else {
                            time_n = 1;
...
{
    uint8 delimit = 0;
    cli_out("InBand TimeStamp MATCH (ip, mac, pnum, vlid) - ");
...
phymod_sym_access(int u, args_t *a, int intermediate, soc_pbmp_t *pbm)
{
    ...
    DPORT_SOC_PBMP_ITER(u, *pbm, dport, p) {
        ...
        rv = phymod_symop_exec(&iter, pc->phymod_ctrl.symbols,
                               &pc->phymod_ctrl.phy[0]->pm_phy, hdr);
        if (rv != CMD_OK) {
            return rv;
        }
    }
...
if_esw_phy(int u, args_t *a)
{
    ...
    if (c != NULL && sal_strcasecmp(c, "info") == 0) {
        ...
        cli_out("%10s %5s %5s %5s %5s %23s %10s\n",
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if (flags & 0x2) {
                    ...
                    rv = bcm_port_phy_control_get(u, p,
                                                  ...
                                                  &latency);
                    if (rv == BCM_E_NONE) {
                        if (latency == 0) {     /* AutogrEEEn Ctrl Reg. */
                            str = "fixed";      /*  bit_2 == 0  */
                        } else {
