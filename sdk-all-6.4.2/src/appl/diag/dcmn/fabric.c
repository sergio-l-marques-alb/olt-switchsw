...
diag_dnx_fabric_connectivity(int unit, args_t *a) {
   ...
   for (i = 0; i < array_size; i++) {
      if (BCM_FABRIC_LINK_NO_CONNECTIVITY != links_connectivity_array[i].link_id) {
...
#ifdef BCM_DPP_SUPPORT
         if (SOC_IS_DPP(unit)) {
            logical = SOC_DPP_FABRIC_LINK_TO_PORT(unit, i);
         } else
#endif
         {
            logical = i;
         ...
         cli_out(" %03d  |     %03d      |     %04d      |     %03d     |     %s\n", i, logical, links_connectivity_array[i].module_id, links_connectivity_array[i].link_id, dev_type);
      }
   }
...
#ifdef BCM_ARAD_SUPPORT
         if (SOC_IS_ARAD(unit)) {
            cli_out(" %3d(%3d) |", (SOC_DPP_FABRIC_LINK_TO_PORT(unit, i)), i);
         } else
#endif
         {
            cli_out(" %8d |", i);
                                          ...
                                          int nof_types)
{
    ...
    if (pipe_id == bcmFabricPipeAll)
    {
        pipe_id_start = bcmFabricPipe0;
        pipe_id_end = bcmFabricPipe0;
#ifdef BCM_DFE_SUPPORT
        if (SOC_IS_DFE(unit))
        {
            pipe_id_end += SOC_DFE_FABRIC_PIPES_CONFIG(unit).nof_pipes - 1;
        }
#endif
    } else {
...
diag_dnx_fabric_thresholds(int unit, args_t* a)
{
    ...
    if (direction_s != NULL && *direction_s != '\0') {
...
diag_dnx_fabric_link_config(int unit, args_t *a) 
{
    ...
    char pcs_str[20], pipe_mapping_buf[20];
...
diag_dnx_fabric_traffic_profile_usage(int unit)
{
    ...
    cli_out("               can be dispalyed for a specific source_id and/or dest_is\n");
}
