...
#ifndef __KERNEL__
static int arad_card_power_monitor_arr[14] = {
    0x0,
    0x00,
    0x11,
    0x02,
...
uint32 arad_card_power_monitor(int eq_sel)
{
#ifndef __KERNEL__
    unsigned
        err = 0;
    ...
    UTILS_ARAD_INIT_ERR_DEFS("arad_card_power_monitor");
    ...
    for (i = 1; i <= ARAD_CARD_POWER_MONITOR_CH_NUM; i++) {
        cli_out("%s(): Setting %40s channel %02d (write 0x%02x in reg 0x17).\n", FUNCTION_NAME(), arad_card_power_monitor_string_arr[i], i, arad_card_power_monitor_arr[i]);
        err = cpu_i2c_write(0x17, 0x9, CPU_I2C_ALEN_BYTE_DLEN_BYTE, arad_card_power_monitor_arr[i]);
        UTILS_ARAD_EXIT_IF_ERR(err, 10);
        
        sal_usleep(1000);

        /* Average Power  */
        for (j = 0; j < ARAD_CARD_POWER_MONITOR_LOOP_AVG ; j++) {
            ...
            act_value_arr[i] += adc_code * 2;
            /* cli_out("j=%d, adc_val_high=0x%x, adc_val_low=0x%x, adc_code=0x%x, act_value_arr[%d]=0x%x,%d\n", j, adc_val_high, adc_val_low, adc_code, i, act_value_arr[i], act_value_arr[i]); */
        }

        /* Average Power */
        act_value_arr[i] = act_value_arr[i] / ARAD_CARD_POWER_MONITOR_LOOP_AVG;
        
        if (i == 6) {
            cli_out("Actual Current = %dmA\n", ((((act_value_arr[i] * 1000) - (606160))*10)/40719)*100);
        } else if ((i == 7) || (i == 8)) {
            cli_out("Actual Current = %dmA\n", ((act_value_arr[i] * 1000)/200));
        } else if ((i == 9) || (i == 10)) {
            cli_out("Actual Current = %dmA\n", ((act_value_arr[i] * 1000)/40));
        } else {
