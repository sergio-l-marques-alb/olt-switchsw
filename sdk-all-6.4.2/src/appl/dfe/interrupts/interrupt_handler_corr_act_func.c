    ...
    *p_corrective_action = FE1600_INT_CORR_ACT_NONE;
    ...
    if (addr_validf != 0) {
        ...
        case RTP_MCTm:
            ...
            if(!is_free && tables_num > FE1600_INTERRUPT_MAX_TABLES_FOR_RTP_TABLE_NON_REPAIR) {
                ...
                if (FE1600_INT_MULTICAST_TABLES_SHOULD_BE_CORRECTED == multicast_tables_info.multicast_tables_status) {
                    ...
                    /* prepare message */
                    sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x, memory=%s, multicast group=%d, reference table id=0%01d",
                    ...
                    *p_corrective_action = FE1600_INT_CORR_ACT_MC_RTP_CORRECT;
                    ...
                    sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x, memory=%s, index=%u, multicast group=0x%05x, Inconsistency in more than one entry",
                                cntf, cnt_overflowf, addrf, memory_name, index, multicast_tables_info.valid_table_id);

                } else {

                    sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x, memory=%s, index=%u, multicast group=0x%05x",
                                cntf, cnt_overflowf, addrf, memory_name, index, multicast_tables_info.valid_table_id);
      
                }
            } else {
                /* multicast id is free */
                sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x, memory=%s, multicast group=0x%05x, is free ",
        ...
        case RTP_CUCTm:
            ...
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x memory=%s, array element=%d, index=%d",
        ...
        case INVALIDm:
            sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x memory does not exist",
        ...
        default:
            ...
            if(rc != SOC_E_NONE)
            {
                ...
                if((RTP_SLSCTm == mem) && (soc_dfe_load_balancing_mode_destination_unreachable == SOC_DFE_CONFIG(unit).fabric_load_balancing_mode)) {
                    sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x memory=%s",
      ...
      /* for invalid received address */
        ...
        sal_sprintf(special_msg, "cntf=0x%04x, cnt_overflowf=0x%01x, block RTP memory address=0x%08x address is not valid",
                    cntf, cnt_overflowf, addrf);
    }
