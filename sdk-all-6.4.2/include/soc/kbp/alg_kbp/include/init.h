...
struct kbp_device_config
{
    ...
    } port_map[2];             /**< Flags to API must be set to KBP_DEVICE_DUAL_PORT, to honor second port info. */
     ...
     * translate it to actual MDIO ID
     */
     ...
     *
     * @param handle Opaque user provided handle passed to the callback.
     * @param chip_no Virtaul chip number to address the device on cascade. The user is responsible to translate to the real MPID select.
     * @param dev The MDIO device ID.
     * @param reg The MDIO register to be read.
     * @param value Returned by the call. Valid if function resturns zero
     *
