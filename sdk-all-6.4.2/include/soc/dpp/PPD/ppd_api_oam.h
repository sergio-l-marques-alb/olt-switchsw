 ...
 *   Tx period (same "units" as in CCMs). 0 indicates one shot.
 *     SOC_SAND_IN  SOC_SAND_PP_MAC_ADDRESS                mac_address,
 * DA MAC address of ioutgoing LBMs
 *      SOC_SAND_OUT uint8                                                   *is_added
 *  returns 1 if the loopback session was added, 0 if loopback session is used by other mep.
 ...
 *   PROC
 * FUNCTION:
 *    Return info from LB session.
 * INPUT:
 *   SOC_SAND_IN  int                   unit -
 *     Identifier of the device to access.
       SOC_SAND_OUT uint32                                              *rx_packet_count - recieved LBRs
       SOC_SAND_OUT uint32                                              *tx_packet_count - Transmitted LBMs
