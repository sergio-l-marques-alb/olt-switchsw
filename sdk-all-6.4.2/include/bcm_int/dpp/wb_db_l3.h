 ...
 *
 * File:        wb_db_l3.h
 * Purpose:     WarmBoot - Level 2 support (L3 Module)
 */

#ifndef _BCM_DPP_WB_DB_L3_H_
#define _BCM_DPP_WB_DB_L3_H_
 ...
 *
 * Currently there is a single handle for L3 module.
...
#define BCM_DPP_WB_L3_VERSION_1_0            SOC_SCACHE_VERSION(1,0)
#define BCM_DPP_WB_L3_CURRENT_VERSION        BCM_DPP_WB_L3_VERSION_1_0


#define BCM_DPP_WB_L3_INFO(unit)             (_dpp_wb_l3_info_p[unit])
#define BCM_DPP_WB_L3_INFO_INIT(unit)        ( (_dpp_wb_l3_info_p[unit] != NULL) &&          \
                                                 (_dpp_wb_l3_info_p[unit]->init_done == TRUE) )
...
#define BCM_DPP_WB_DEV_L3_DIRTY_BIT_SET(unit)                                       \
                        SOC_CONTROL_LOCK(unit);                                       \
                        SOC_CONTROL(unit)->scache_dirty = 1;                          \
                        BCM_DPP_WB_L3_INFO(unit)->is_dirty = 1;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_L3_DIRTY_BIT_CLEAR(unit)                                         \
                        SOC_CONTROL_LOCK(unit);                                       \
                        BCM_DPP_WB_L3_INFO(unit)->is_dirty = 0;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_L3_IS_DIRTY(unit)                                                \
                        (BCM_DPP_WB_L3_INFO(unit)->is_dirty == 1)
 ...
 * runtime information
 */
typedef struct bcm_dpp_wb_l3_info_s {
    int            init_done;
    int            is_dirty;
    ...
    int            tnl_to_eep_off;
    int            l3_info_off;
} bcm_dpp_wb_l3_info_t;


extern bcm_dpp_wb_l3_info_t   *_dpp_wb_l3_info_p[BCM_MAX_NUM_UNITS];
...
extern int
_bcm_dpp_wb_l3_update_tnl_to_eep(int unit, int eep);
