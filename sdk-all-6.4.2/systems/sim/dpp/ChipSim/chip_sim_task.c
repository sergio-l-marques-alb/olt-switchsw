...
extern void init_simulate_pb_device(unsigned int chip_ver);
extern void init_simulate_pcp_device(unsigned int chip_ver);
    ...
    )
{
  ...
  for (i=0; i<CHIP_SIM_NOF_CHIPS; i++)
  {
    ...
    {
      soc_petra_initialize_device_values(INT_TO_PTR(start_address[i]));
    }
#endif    	
#if LINK_T20E_LIBRARIES
    ...
    {
      soc_pb_initialize_device_values(INT_TO_PTR(start_address[i]));
    }
#endif    	
#endif
  }
  ...
  case SOC_SAND_DEV_PETRA:
    init_simulate_petra_device(chip_ver);
    break;
#endif
#if LINK_T20E_LIBRARIES
  ...
  case SOC_SAND_DEV_PB:
    init_simulate_pb_device(chip_ver);
    break;
#endif
#if LINK_PCP_LIBRARIES
  case SOC_SAND_DEV_PCP:
#if 0
    init_simulate_pcp_device(chip_ver);
#else
    soc_sand_os_printf("*** init_simulate_pcp_device was not ported !!!\n\r");
#endif
    break;
#endif
  default:
