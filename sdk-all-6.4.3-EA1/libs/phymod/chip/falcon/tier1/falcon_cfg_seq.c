...
int falcon_prbs_tx_enable_get(const phymod_access_t *pa, uint32_t *enable)
{
  ...
  __err=ERR_CODE_NONE;
  __err = falcon_get_tx_prbs_en(pa, &val); 
  if(__err) return(__err);
  *enable = val;
...
int falcon_prbs_rx_enable_get(const phymod_access_t *pa, uint32_t *enable)
{
  ...
  __err=ERR_CODE_NONE;
  __err = falcon_get_rx_prbs_en(pa, &val); 
  if(__err) return(__err);
  *enable = val;
...
int falcon_pcs_lane_swap_tx(const phymod_access_t *pa, uint32_t tx_lane_map) {
  err_code_t __err;
...
int falcon_pmd_cl72_enable_get(const phymod_access_t *pa, uint32_t *enable)   
{
  err_code_t __err;
  __err=ERR_CODE_NONE;
...
int falcon_pram_firmware_enable(const phymod_access_t *pa, int enable, int wait)   /* release the pmd core soft reset */
{
  err_code_t __err;
...
int falcon_lane_hard_soft_reset_release(const phymod_access_t *pa, uint32_t enable)   /* release the pmd core soft reset */
{
  err_code_t __err;
  __err=ERR_CODE_NONE;
