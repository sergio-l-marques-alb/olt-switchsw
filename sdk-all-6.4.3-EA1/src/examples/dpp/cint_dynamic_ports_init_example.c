...
int main(int unit)
{
    ...
    BCM_PBMP_ITER(config.cpu, port) {
        ...
        interface_info.phy_port = 0;
        interface_info.interface = BCM_PORT_IF_CPU;
        if (port == 0) {
            header_type = BCM_SWITCH_PORT_HEADER_TYPE_TM;
        } else {
            header_type = BCM_SWITCH_PORT_HEADER_TYPE_ETH;
        }

        rv = add_port_full_example(unit, port , mapping_info, interface_info, 0, header_type ,FALSE);
        if (rv != BCM_E_NONE) {
