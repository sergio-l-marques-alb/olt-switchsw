...
int C5_NUM_COS = 4;
int C5_DEFAULT_E2E_SCHED_FLAGS = (BCM_COSQ_GPORT_SCHEDULER | BCM_COSQ_GPORT_SCHEDULER_HR_DUAL_WFQ);
...
*/
    ...
    port = idx + 1;
    system_port = idx + 1;
    BCM_GPORT_MODPORT_SET(dest_gport[idx], c5_my_modid, port);
    BCM_GPORT_MODPORT_SET(src_gport[idx], c5_my_modid, 0);
    ...
    flags = C5_CONN_FLAGS | default_flags;
    rv = bcm_cosq_gport_add(c5_unit, src_gport[idx], C5_NUM_COS, 
                flags, &c5_gport_ucast_voq_connector_group0[idx]);
    ...
    BCM_COSQ_GPORT_COMPOSITE_SF2_SET(c5_gport_ucast_voq_connector_group0_sf2[idx], c5_gport_ucast_voq_connector_group0[idx]);
