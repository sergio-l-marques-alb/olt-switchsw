    ...
    )
{
    ...
    rv = create_l3_intf(unit, 0, port_id, vlan, my_mac, &ingress_intf);    
    if (rv != BCM_E_NONE) {
...
/*
 *  bcm_mpls_port_create: for protected gport that allocate only FEC.
 ...
 */
int
system_mpls_port_fec_only_create(
    int unit,
    bcm_port_t  port_id,
    ...
    )
{
    ...
    mp->criteria = BCM_MPLS_PORT_MATCH_NONE; /* no VC key */
    mp->port = port_id; /* physical port */
    mp->encap_id = encap_id;
    BCM_ENCAP_REMOTE_SET(&(mp->encap_id));
    mp->flags |= BCM_MPLS_PORT_ENCAP_WITH_ID; /* Must be used, specify LIF ID is remote */
    mp->flags |= BCM_MPLS_PORT_EGRESS_TUNNEL; 
    mp->failover_port_id = failover_port_id;
    mp->failover_id = failover_id; /* no FEC allocated */
    if (*gport!=0) {
        mp->flags |= BCM_MPLS_PORT_WITH_ID;
        BCM_GPORT_MPLS_PORT_ID_SET(mp->mpls_port_id,*gport); /* this is the LIF id */
    }
    rv = bcm_mpls_port_add(unit, 0, mp);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_mpls_port_add\n"); 
        return rv;
    }
    if(verbose >= 1) {
        printf("unit (%d): created mpls_port (FEC) :0x%08x point to remote LIF:0x%08x\n",unit,mp->mpls_port_id,encap_id);
    }
    *gport = mp->mpls_port_id;
...
int system_mpls_port_mp_example(int unit1, int port1, int second_port1, int unit2, int port2){
    ...
    rv = mpls_pipe_mode_exp_set(unit1);
    if (rv != BCM_E_NONE) {
        printf("Error, in mpls_pipe_mode_exp_set\n");
        return rv;
    }

    rv = mpls_pipe_mode_exp_set(unit2);
    if (rv != BCM_E_NONE) {
        printf("Error, in mpls_pipe_mode_exp_set\n");
        return rv;
    }
    ...
    vswitch_vpls_vpn_create(unit1, vsi);
    vswitch_vpls_vpn_create(unit2, vsi);
...
int system_multicast_mpls_port_add(int local_unit, int remote_unit, bcm_port_t port, bcm_gport_t gport, int mc_group_id) {
    ...
    rv = bcm_multicast_ingress_add(local_unit, mc_group_id, modport, encap_id);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_multicast_ingress_add local_unit: %d mc_group_id:  0x%08x  modport:  0x%08x  encap_id:  0x%08x \n", local_unit, mc_group_id, modport, encap_id);
        return rv;
    }

    rv = bcm_multicast_ingress_add(remote_unit, mc_group_id, modport, encap_id);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_multicast_ingress_add remote_unit: %d mc_group_id:  0x%08x  modport:  0x%08x  encap_id:  0x%08x \n", remote_unit, mc_group_id, modport, encap_id);
        return rv;
    }
...
int system_protected_mpls_port_example(int unit1, int port1, int unit2, int port2,int port3,int is_mp){
    ...
    bcm_gport_t gport2_fec_a =3000+base;
    bcm_mpls_port_t mp2_fec_b;
    bcm_gport_t gport2_fec_b =3001+base;
    uint32 v2 = 20;
    uint32 v3 = 30;
    bcm_mpls_label_t l1 = 100;
    bcm_mpls_label_t l2 = 101;
    ...
    rv = system_mpls_port_fec_only_create(unit1, port2, failover_id,0,gport2_lif_b,&gport2_fec_b,&mp2_fec_b);
    if (rv != BCM_E_NONE) {
        printf("Error, system_mpls_port_fec_only_create 1\n"); 
        return rv;
    }

    rv = system_mpls_port_fec_only_create(unit1, port3, failover_id,gport2_fec_b,gport2_lif_a,&gport2_fec_a,&mp2_fec_a);
    if (rv != BCM_E_NONE) {
        printf("Error, system_mpls_port_fec_only_create 1\n"); 
        return rv;
    }
