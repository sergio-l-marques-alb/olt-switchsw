                                           ...
                                           _bcm_dpp_field_ent_idx_t entry)
{
    ...
    if (offset > stageData->devInfo->entryMaxQuals) {
        ...
        if (SOC_IS_ARAD(unit)) {
            ...
            if (result == BCM_E_NONE) {
                ...
                if (isExternalTcam) {
                    ...
                    soc_sand_occ_bm_occup_status_set(unitData->extTcamBmp, unitData->entryExtTc[entry].ext_info_idx, FALSE);
                }
            }
        }
        else if (SOC_IS_PETRAB(unit)) {
...
/*
 *   Function
 *      _bcm_dpp_field_tcam_entry_action_find_bcm
 *   Purpose
 *      Find a single BCM layer action out of an action array
 *   Parameters
 *      (in) type = action type to find
 *      (in) actions = pointer to array of actions to search
 *   Returns
 *      offset into array or _BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX if not found
 */
STATIC unsigned int
_bcm_dpp_field_tcam_entry_action_find_bcm(bcm_field_action_t type,
                                          const _bcm_dpp_field_tc_b_act_t *actions)
{
    unsigned int index;

    for (index = 0; index < _BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX; index++) {
        if (actions[index].bcmType == type) {
            break;
        }
    }
    return index;
 ...
 *   Notes
 *      This does not actually commit the entry to hardawre, but other
 ...
 *      released while cleaning up disused resources.
 *
    ...
    for (bcmIndex = 0, ppdIndex = 0;
        ...
        if (SOC_PPD_FP_DATABASE_STAGE_INGRESS_PMF ==
            ...
            case bcmFieldActionPolicerLevel1:
                ...
                if (_BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX <=
                    _bcm_dpp_field_tcam_entry_action_find_bcm(bcmFieldActionUsePolicerResult,
                                                              propActs)) {
                    /* no explicit bcmFieldActionUsePolicerResult, assume it */
                    for (index = 0;
                         index < _BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX;
                         index++) {
                        if (bcmFieldActionCount == propActs[index].bcmType) {
                            propActs[index].bcmType = bcmFieldActionUsePolicerResult;
                            propActs[index].bcmParam0 =  (BCM_FIELD_USE_POLICER_RESULT_INGRESS | BCM_FIELD_USE_POLICER_RESULT_EGRESS);
                            propActs[index].bcmParam1 = 0;
                            break;
                        }
                    }
                    if (_BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX <= index) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL,
                                         (_BSL_BCM_MSG_NO_UNIT("unit %d entry %u"
                                                           " has no room for"
                                                           " implied %s"
                                          ...
                                          unit,
                                          _BCM_DPP_FIELD_TCAM_GLOBAL_ID(unit, entry, isExternalTcam),
                                          _bcm_dpp_field_action_name[bcmFieldActionUsePolicerResult]));
                    }
                    /* do not include the implied PPD action */
                    actCount = 1;
#ifdef BCM_PETRAB_SUPPORT
                    if (SOC_IS_PETRAB(unit)) {
                        hwAct[ppdIndex + 1].hwType = SOC_PPD_FP_ACTION_TYPE_DP;
                        hwAct[ppdIndex + 1].hwParam = 0x8 + SOC_PPC_MTR_RES_USE_GET_BY_TYPE(TRUE /*ingress*/, TRUE /*egress*/); /* policer, I+E */
                        /* include the implied PPD action */
                        actCount = 2;
                    }
#endif /* def BCM_PETRAB_SUPPORT */
                    /* we have already considered the extra BCM layer action */
                    SHR_BITSET(actHit, index);
                } /* if (not able to find UsePolicerResult action) */
                break;
            ...
            case bcmFieldActionRedirectIpmc:
                ...
                if (BCM_GPORT_IS_SET(propActs[bcmIndex].bcmParam0)) {
                    ...
                    if (16383 < propActs[bcmIndex].bcmParam0) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
...
/* coverity[uninit_use_in_call] */
            ...
            case bcmFieldActionIngressGportSet:
                ...
                if (BCM_GPORT_IS_SET(propActs[bcmIndex].bcmParam0)) {
                    ...
                    if ((BCM_GPORT_IS_VLAN_PORT(propActs[bcmIndex].bcmParam0)) &&  (0 == BCM_GPORT_VLAN_PORT_ID_GET(propActs[bcmIndex].bcmParam0))) {
                      ...
                      result = _bcm_dpp_gport_to_lif(unit,
                                                     propActs[bcmIndex].bcmParam0,
                                                     &inLif,
                                                     NULL,
                                                     &fec,
            ...
            for (index = 0; index < actCount; index++) {
                ...
                if (SOC_IS_ARAD(unit) &&
                    ...
                    rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unitData->unitHandle, pp_port, 0, &hwAct[ppdIndex + index].hwParam);
                    if (BCM_FAILURE(rv)) { /* function returns valid port number or error ID */
                        BCMDNX_ERR_EXIT_MSG(hwAct[ppdIndex + index].hwParam,
                                         (_BSL_BCM_MSG_NO_UNIT("unable to map port"
                                                           "%d (%08X) to base queue"
    ...
    /* Everthing went well to this point, commit the changes. */
    commit = TRUE;
exit:
                                  ...
                                  _bcm_dpp_field_ent_idx_t entry)
{
    ...
    if (_BCM_DPP_FIELD_TCAM_ENTRY_FLAGS(isExternalTcam, entry) & _BCM_DPP_FIELD_ENTRY_IN_USE) {
        ...
        if (BCM_E_NONE == result) {
            ...
            if (isExternalTcam) {
                soc_sand_occ_bm_is_occupied(unitData->extTcamBmp, unitData->entryExtTc[entry].ext_info_idx, &isExtInfoUsed);
                if (isExtInfoUsed) {
                    _bcm_dpp_field_ext_info_entry_clear(&(unitData->extTcamInfo[unitData->entryExtTc[entry].ext_info_idx]));
                    soc_sand_occ_bm_occup_status_set(unitData->extTcamBmp, unitData->entryExtTc[entry].ext_info_idx, FALSE);
                }
