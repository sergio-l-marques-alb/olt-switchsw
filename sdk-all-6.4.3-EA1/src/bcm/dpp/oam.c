...
#define _BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(lif, is_upmep, found)		               		                        \
  do {  														               		                                    \
	  uint8 mdl, found_profile_dummy, is_mp_type_flexible_dummy;               		                                    \
...
do {\
	int nic_profile, oui_profile, is_last;\
		rv = _bcm_dpp_am_template_oam_lmm_nic_tables_free(unit, endpoint_id, &nic_profile, &is_last);\
		BCMDNX_IF_ERR_EXIT(rv);\
		rv = _bcm_dpp_am_template_oam_lmm_oui_tables_free(unit, endpoint_id, &oui_profile, &is_last);\
		BCMDNX_IF_ERR_EXIT(rv);\
...
int _bcm_oam_mep_db_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry, int icc_ndx) {
	...
	case bcmOAMEndpointTypeEthernet:
		mep_db_entry->mep_type = SOC_PPD_OAM_MEP_TYPE_ETH_OAM;
		break;
    ...
    if ((endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING) == 0) {
        ...
        ret = _bcm_dpp_gport_to_phy_port(unit, endpoint_info->tx_gport, _BCM_DPP_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info);
        BCMDNX_IF_ERR_EXIT(ret); 
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) &&  PPC_API_OAM_STORE_LOCAL_PORT_IN_MEP_DB(mep_db_entry->mep_type)) {
            /* Allocating for the LOCAL_PORT_2_SYS_PORT table, used only in down-MEPS, Arad B0 and above*/
            ...
            ret = _bcm_dpp_am_template_oam_local_port_2_sys_port_alloc(unit,alloc_flags,&gport_info.sys_port,&ignored,&local_port);
            BCMDNX_IF_ERR_EXIT(ret);
...
int _bcm_oam_classifier_mep_entry_struct_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
	...
	switch (endpoint_info->type) {
	case bcmOAMEndpointTypeEthernet:
		classifier_mep_entry->mep_type = SOC_PPD_OAM_MEP_TYPE_ETH_OAM;
		break;
	case bcmOAMEndpointTypeBHHMPLS:
		classifier_mep_entry->mep_type = SOC_PPD_OAM_MEP_TYPE_Y1731_O_MPLSTP ;
		break;
	case bcmOAMEndpointTypeBHHPwe:
		classifier_mep_entry->mep_type = SOC_PPD_OAM_MEP_TYPE_Y1731_O_PWE;
		break;
	default:
		BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Unsupported endpoint type.")));
	}
   ...
   void *user_data)
{
	...
	if (lif == profile_info->lif) {
		rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_exchange(unit, 0/*flags*/,  endpoint_info->id, profile_info->profile_data,
								 &old_profile, &is_last, &profile, &is_allocated);
		BCMDNX_IF_ERR_EXIT(rv);
		if ((profile != profile_info->profile) || is_allocated) {
			BCMDNX_ERR_EXIT_MSG(BCM_E_INIT, (_BSL_BCM_MSG("Error: _bcm_oam_endpoint_exchange_profile_cb internal error.\n")));
		}
		if (is_last && (profile != old_profile)) {
            rv = _bcm_dpp_oam_mep_profile_trap_data_free(unit, &(profile_info->profile_data->mep_profile_data), 1/*is_mep*/);
			BCMDNX_IF_ERR_EXIT(rv);
            rv = _bcm_dpp_oam_mep_profile_trap_data_free(unit, &(profile_info->profile_data->mip_profile_data), 0/*is_mep*/);
			BCMDNX_IF_ERR_EXIT(rv);
		}
	}
    BCM_EXIT;   
...
int _bcm_dpp_oam_error_trap_set(int unit, uint32 oamp_error_trap_id, bcm_gport_t dest_port)
{
    ...
    for (trap_type=0; trap_type<SOC_PPD_OAM_OAMP_TRAP_TYPE_COUNT; trap_type++) {
        ...
        if (trap_id == oamp_error_trap_id) {
            /* Set destination */
            soc_sand_rv = soc_ppd_oam_oamp_error_trap_id_and_destination_set(unit, trap_type, trap_id, gport_info.local_port);  
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
...
int bcm_petra_oam_init(int unit) {
...
#endif /*BCM_WARM_BOOT_SUPPORT*/
	{
	...
	#ifdef BCM_ARAD_SUPPORT
		if ((SOC_IS_ARAD(unit))) {
			soc_sand_rv = arad_port_header_type_get(unit, recycle_port, &header_type_incoming, &header_type_outgoing); 
			BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
		...
		BCM_PBMP_ITER(port_config.e, ethernet_port) {
		  if (!BCM_PBMP_MEMBER(port_config.rcy, ethernet_port)) {
				bcm_petra_mirror_port_dest_add(unit, ethernet_port, BCM_MIRROR_PORT_EGRESS_ACL, 0);
			}
		}
...
int bcm_petra_oam_endpoint_create(int unit, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (endpoint_info->id == -1) {
		/* In this case all fields except id should be zero*/
        if ( (endpoint_info->flags !=BCM_OAM_ENDPOINT_WITH_ID)  || endpoint_info->ccm_period || endpoint_info->int_pri || endpoint_info->vlan || endpoint_info->inner_pkt_pri || 
											endpoint_info->inner_tpid || endpoint_info->inner_vlan || endpoint_info->outer_tpid ||   !BCM_MAC_IS_ZERO(endpoint_info->src_mac_address) 
                                             ||  !BCM_MAC_IS_ZERO(endpoint_info->dst_mac_address) ||  endpoint_info->level ||  endpoint_info->name || endpoint_info->type || endpoint_info->group
		...
		if ( endpoint_info->type == bcmOAMEndpointTypeEthernet ) {
			...
			if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) {
				...
				/* Arad+ may does not support level 0. Arad does.*/
				if ((endpoint_info->level >7)  || ( (!SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) &&  (endpoint_info->level <0))  ||
                                                     (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) &&  (endpoint_info->level <=0))   ) ) {
					BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
		...
		if (((endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) || (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW))
			&& SOC_IS_JERICHO(unit)) {
			if ((endpoint_info->flags & BCM_OAM_ENDPOINT_PORT_STATE_UPDATE) ?
	...
	if (endpoint_info->flags & BCM_OAM_ENDPOINT_REPLACE) {
		...
		if (existing_endpoint_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
            ...
            if (endpoint_info->gport != BCM_GPORT_INVALID || endpoint_info->gport != BCM_GPORT_INVALID) {
			...
			if (endpoint_info->level != classifier_mep_entry.md_level) {
			...
			if (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) { /* TX - OAMP */
				LOG_VERBOSE(BSL_LS_BCM_OAM,
                                            (BSL_META_U(unit,
			...
			if (endpoint_info->gport != BCM_GPORT_INVALID) {
				...
				if (SOC_IS_ARADPLUS(unit)) {
					/* Set the counter*/
					mep_db_entry.counter_pointer = new_classifier_mep_entry.counter; 
				}
		...
		/* Default OAM trap */
		if ((endpoint_info->flags & BCM_OAM_ENDPOINT_WITH_ID) && (endpoint_info->id == -1)) {
			if (SOC_IS_ARADPLUS(unit)) {
				BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Default profile is not supported in ARAD PLUS.\n")));
			}
			...
			rv = _bcm_oam_default_profile_get(unit, &profile_data, &classifier_mep_entry, 0 /* non accelerated */,1 /* is default */);
			BCMDNX_IF_ERR_EXIT(rv);
			...
			soc_sand_rv = soc_ppd_oam_classifier_oam1_entries_insert_default_profile(unit, &profile_data, _bcm_oam_cpu_trap_code_to_mirror_profile_map[unit], 0);
			BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
		...
		if (endpoint_info->flags & BCM_OAM_ENDPOINT_REMOTE) { /* Remote endpoint */
			...
			if ((endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) && (endpoint_info->flags & BCM_OAM_ENDPOINT_INTERMEDIATE)) {
				BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: Intermediate endpoint can not be accelerated in HW.\n")));
			}

			if (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) { /* TX - OAMP */
				if (endpoint_info->tx_gport == BCM_GPORT_INVALID && (endpoint_info->flags & BCM_OAM_ENDPOINT_UP_FACING)==0) {
			...
			if (endpoint_info->gport == BCM_GPORT_INVALID) {
				...
				if (found && (profile != _BCM_OAM_PASSIVE_PROFILE)) {
					BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS,
				...
				if (!(endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW)) { /* non-accelerated - allocate non-accelerated ID */
				...
				if (found_profile) {
					if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)) {
						...
						BCMDNX_IF_ERR_EXIT(rv);
						if (profile_data.mp_type_passive_active_mix && ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT)== 0)) {
							/* Previous type was up/down, now trying to add mip */
							if (is_mp_type_flexible) {
								profile_data.mp_type_passive_active_mix = FALSE;
								updated_mp_type = 1;
							}
							else {
								BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
							...
							_BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(classifier_mep_entry.lif, 
																	((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0), found);
							if (!profile_data.mp_type_passive_active_mix && ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT)!= 0) && found) {
								/* Previous type was mep/mip, now trying to add mep on different direction than already existing one */
								if (is_mp_type_flexible) {
									profile_data.mp_type_passive_active_mix = TRUE;
									updated_mp_type = 1;
								}
								else {
									BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
						...
						if ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ENDPOINT)!= 0) {
							if (profile == _BCM_OAM_PASSIVE_PROFILE) {
								/* Get a new profile */
								SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data);
								rv = _bcm_oam_default_profile_get(unit, &profile_data, &classifier_mep_entry, 0 /* non accelerated */, 0 /* is default */);
								BCMDNX_IF_ERR_EXIT(rv);
							}
							else {
								/* get profile data existing on this lif */
							...
							if (profile == _BCM_OAM_PASSIVE_PROFILE) {
								...
								soc_sand_rv = soc_ppd_oam_classifier_find_mep_and_profile_by_lif_and_mdlevel(
								   ...
								   ((classifier_mep_entry.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) != 0), 
								   &found, &profile_temp, &found_profile, &is_mp_type_flexible);
								BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
								profile = (int)profile_temp;
							}
							/* get profile data existing on this lif */
				...
				if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && (updated_mp_type || !found_profile)) {
					soc_sand_rv = soc_ppd_oam_mep_passive_active_enable_set(unit, profile, profile_data.mp_type_passive_active_mix);
					BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
				}

				SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data_acc);
				if (endpoint_info->opcode_flags & BCM_OAM_OPCODE_CCM_IN_HW) {
					rv = _bcm_oam_default_profile_get(unit, &profile_data_acc, &classifier_mep_entry, 1 /* accelerated */, 0 /* is default */);
					BCMDNX_IF_ERR_EXIT(rv);
				...
				if (endpoint_info->lm_counter_base_id > 0) {
					rv = _bcm_dpp_oam_set_counter(unit, endpoint_info->lm_counter_base_id, ((endpoint_info->lm_flags & BCM_OAM_LM_PCP)!=0), classifier_mep_entry.lif);
					BCMDNX_IF_ERR_EXIT(rv);
					...
					for (processor_ndx=0; processor_ndx<SOC_TMC_CNT_NOF_PROCESSOR_IDS_ARAD; processor_ndx++) {
						...
						if ((processor_info.src_type==SOC_TMC_CNT_SRC_TYPE_OAM_ING_A) ||
							...
							if ((endpoint_info->lm_flags & BCM_OAM_LM_PCP)!=0) {
								for (i=endpoint_info->lm_counter_base_id; i<endpoint_info->lm_counter_base_id+8; i++) {
									rv = bcm_dpp_counter_set(unit, processor_ndx, i, bcm_dpp_counter_pkts, stat);
								}
							}
							else {
								rv = bcm_dpp_counter_set(unit, processor_ndx, endpoint_info->lm_counter_base_id, bcm_dpp_counter_pkts, stat);
							}
						}
					}
				}
				...
				unit, &classifier_mep_entry, &profile_data, &profile_data_acc, _bcm_oam_cpu_trap_code_to_mirror_profile_map[unit]);
				BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
								...
								bcm_oam_endpoint_info_t *endpoint_info)
{
	...
	if (endpoint < -1) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                         (_BSL_BCM_MSG("Error: Endpoint id must be positive or -1. Given id: %d\n"), endpoint));
	}
	...
	/* Default OAM trap */
	if (endpoint == -1) {
		rv = _bcm_dpp_oam_bfd_mep_info_db_get(unit, -1, &classifier_mep_entry, &found);
		BCMDNX_IF_ERR_EXIT(rv);
	...
	/* local endpoint */
		...
		if ((mep_info.flags & SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ACCELERATED)!=0 ) {
			...
			if (mep_info.lif != _BCM_OAM_INVALID_LIF) {
				/* Free acc profile  - Only if RX is set*/
            LOG_VERBOSE(BSL_LS_BCM_OAM,
                        (BSL_META_U(unit,
		...
		if (mep_info.lif != _BCM_OAM_INVALID_LIF) {
			/* Free profile */
			rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(unit, endpoint, &profile_data);
		BCMDNX_IF_ERR_EXIT(rv);
		...
		BCMDNX_IF_ERR_EXIT(rv);
			...
			_BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(mep_info.lif, TRUE /*is_upmep*/, found);

			if (!found) { /* Serch meps on the other directions */
				_BCM_OAM_MEP_EXIST_ON_LIF_AND_DIRECTION(mep_info.lif, FALSE/*is_upmep*/, found);
			}
...
int bcm_petra_oam_endpoint_action_set_internal(
	...
	SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data);
	rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(unit, id, &profile_data);
    BCMDNX_IF_ERR_EXIT(rv);
	SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&old_profile_data);
	rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_data_get(unit, id, &old_profile_data);
    BCMDNX_IF_ERR_EXIT(rv);
	SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data_acc);
	if (is_accelerated) {
	...
	rv = _bcm_dpp_am_template_oam_mep_profile_non_accelerated_exchange(unit, 0/*flags*/, id, &profile_data,
                                             &old_profile, &is_last, &profile, &is_allocated);
    BCMDNX_IF_ERR_EXIT(rv);
	...
	if (is_last) { 
		rv = _bcm_dpp_oam_mep_profile_trap_data_free(unit, &old_profile_data.mep_profile_data, 1/*is_mep*/);
		BCMDNX_IF_ERR_EXIT(rv);
		rv = _bcm_dpp_oam_mep_profile_trap_data_free(unit, &old_profile_data.mip_profile_data, 0/*is_mep*/);
		BCMDNX_IF_ERR_EXIT(rv);
	...
	if (is_accelerated) {
		...
		if (is_last && (profile != old_profile_acc)) { 
    ...
    profile_data.is_piggybacked = profile_data_acc.is_piggybacked = is_piggy_backed;
    ...
    if (id == -1) {
		...
		if ((old_profile != classifier_mep_entry.non_acc_profile) || 
			(is_accelerated && (old_profile_acc != classifier_mep_entry.acc_profile))) {
...
int _manage_DA_with_existing_entries_internal(int unit , int mep_id ,SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb, 
    ...
    if (is_dm && (classifier_mep->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_ONLY_ON_DEMAND_DMM_ACTIVE ) &&
