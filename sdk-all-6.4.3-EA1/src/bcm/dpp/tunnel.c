    ...
    int *tunnel_eep)
{
    ...
    if(!eep_alloced) {/* need to allocate IP tunnel */
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                rv = soc_ppd_eg_encap_entry_type_get(unit, tunnel->l3_intf_id, &entry_type);
                BCM_IF_ERROR_RETURN(rv);

                if (entry_type == SOC_PPD_EG_ENCAP_ENTRY_TYPE_DATA && SOC_IS_ROO_ENABLE(unit)) {
    ...
    if (SOC_IS_PETRAB(unit)) {
        ...
        /* IP tunnel points to outRIF/Link-layer */
        next_eep[0] = tunnel->l3_intf_id;
