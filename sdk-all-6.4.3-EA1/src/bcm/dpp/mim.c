...
{
    return SOC_DPP_CONFIG(unit)->pp.mim_lif_ndx;
}

SOC_PPD_AC_ID
  __dpp_mim_out_ac_get(int unit)
{
    /* No default out ac in AVT mode*/
    return ((SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit)) ? 0 : SOC_DPP_CONFIG(unit)->pp.mim_out_ac);
}
...
bcm_petra_mim_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (!SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit)) { /* Standard vlan translate mode has a default ac for MIM, while AVT mode doesn't */
            ...
            rv = bcm_dpp_am_l2_ac_alloc(unit, _BCM_DPP_AM_L2_AC_TYPE_VLAN_EDITING, SHR_RES_ALLOC_WITH_ID, &SOC_DPP_CONFIG(unit)->pp.mim_lif_ndx);
            BCMDNX_IF_ERR_EXIT(rv);

            /* alloc a special out-ac, used only fof MiM */
            if (SOC_IS_PETRAB(unit)) {
                rv = bcm_dpp_am_l2_ac_alloc(unit, 0, SHR_RES_ALLOC_WITH_ID, &SOC_DPP_CONFIG(unit)->pp.mim_out_ac);
            } else {
                rv = bcm_dpp_am_out_ac_alloc(unit, _BCM_DPP_AM_OUT_AC_TYPE_DEFAULT, SHR_RES_ALLOC_WITH_ID, &SOC_DPP_CONFIG(unit)->pp.mim_out_ac);
            }
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (SOC_IS_ARAD(unit)) {
                ...
                soc_sand_rv = soc_ppd_eg_ac_info_set(soc_sand_dev_id, SOC_DPP_CONFIG(unit)->pp.mim_out_ac, &ac_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
        }
        ...
        /* write mim_out_ac to default_sem_index reg */
        if (SOC_IS_PETRAB(unit)) {
            soc_sand_rv = soc_pb_pp_frwrd_bmact_default_sem_index_set(soc_sand_dev_id, SOC_DPP_CONFIG(unit)->pp.mim_out_ac);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
#endif
    ...
    bcm_mim_vpn_config_t *info)
{
    ...
    SOC_PPD_LIF_ID tmp_lif, isid_lif_id = 0;
    ...
    if (info->flags & BCM_MIM_VPN_BVLAN) { /* B-VID */
        ...
        if (!update) {
            ...
            soc_sand_rv = soc_ppd_l2_lif_isid_get(soc_sand_dev_id, &isid_key, &isid_lif_id, &vsi_id, &isid_info, &found);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            if (!found) {
...
/* coverity[uninit_use_in_call] */
        if(info->flags & BCM_MIM_VPN_SERVICE_ENCAP_WITH_ID) {
            ...
            isid_lif_id = BCM_GPORT_TUNNEL_ID_GET(info->service_encap_id);
        }

        if (!update) {
            rv = bcm_dpp_am_l2_ac_alloc(unit, _BCM_DPP_AM_L2_AC_TYPE_ISID, l2_ac_alloc_flags, &isid_lif_id);
            BCMDNX_IF_ERR_EXIT(rv);
        } 

        BCM_GPORT_TUNNEL_ID_SET(info->service_encap_id, isid_lif_id);
        ...
        if (!update) {
            soc_sand_rv = soc_ppd_l2_lif_isid_add(soc_sand_dev_id, vsi_id, &isid_key, isid_lif_id, &isid_info, &success);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        } else {
            if (old_lookup_id != info->lookup_id) {                
                ...
                /* remove the isid */            
                soc_sand_rv = soc_ppd_l2_lif_isid_remove(soc_sand_dev_id, &isid_key, &isid_lif_id);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
        }                
        ...
        if (SOC_IS_ARAD(unit)) {
            if (soc_property_get(unit, spn_MIM_NUM_VSIS, 0) == 32768) {
                arad_sw_db_vsi_isid_set(soc_sand_dev_id,vsi_id,info->lookup_id);
            }
        }
        #endif
         ...
         */
        rv = _bcm_petra_mim_tpid_profile_update(unit, isid_lif_id, info->match_service_tpid);
        BCMDNX_IF_ERR_EXIT(rv);    

        /* save type and LIF-ID data - for restoration */
        bcm_mim_port_t_init(&mim_port);
        mim_port.mim_port_id = info->service_encap_id;
        rv = _bcm_dpp_in_lif_mim_match_add(unit, &mim_port, isid_lif_id);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mim_vpn_config_t *info)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(vpn)) { /* B-VID */
        ...
        soc_sand_rv = soc_ppd_l2_lif_isid_get(soc_sand_dev_id, &isid_key, &lif_index, &vsi_id, &isid_info, &found);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        BCM_GPORT_TUNNEL_ID_SET(info->service_encap_id, lif_index);
    }
    ...
    bcm_mim_vpn_t vpn)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(vpn)) { /* B-VID */
        ...
        if(SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit)) {
            BCM_GPORT_TUNNEL_ID_SET(service_encap_id, isid_lif_id);
            rv = bcm_petra_port_tpid_delete_all(unit, service_encap_id);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else {
            /* isid_lif_id should no longer point at TPID index for B-tag TPID */
            rv = _bcm_petra_mim_tpid_profile_update(unit, isid_lif_id, 0);
            BCMDNX_IF_ERR_EXIT(rv);  
        }

        /* Deallocate isid_lif_id */
        rv = bcm_dpp_am_l2_ac_dealloc(unit, 0, isid_lif_id);
        BCMDNX_IF_ERR_EXIT(rv);

        /* remove mim lif match SW */
        rv = _bcm_dpp_in_lif_match_delete(unit, isid_lif_id);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    if (!update) {
        ...
        rv = bcm_dpp_am_l3_eep_alloc(unit, _BCM_DPP_AM_L3_EEP_TYPE_DEFAULT, eep_flags, &mim_port->encap_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (update) {
        ...
        /* Check that mim_port->encap_id is an index of a MIM OutLIF (in SW DB) */
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, mim_port->encap_id, NULL, &usage) );
        ...
        soc_sand_rv = soc_ppd_eg_encap_entry_get(soc_sand_dev_id, SOC_PPD_EG_ENCAP_EEP_TYPE_LL, mim_port->encap_id, 1, encap_entry_info, 
    ...
    soc_sand_rv = soc_ppd_eg_encap_ll_entry_add(soc_sand_dev_id, mim_port->encap_id, ll_encap_info_ptr);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    /* Add outlif to SW DB */
    rv = _bcm_dpp_out_lif_mim_match_add(unit,mim_port,mim_port->encap_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (vpn == BCM_PETRA_MIM_VPN_INVALID) { /* P2P */
        ...
        soc_sand_rv = soc_ppd_l2_lif_isid_get(soc_sand_dev_id, &isid_key, &lif_id, &vsi_index, &isid_info, &found);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        if (found && !update) {
        ...
        if (!update) {
            if(mim_port->flags & BCM_MIM_PORT_SERVICE_ENCAP_WITH_ID) {
                ...
                lif_id = BCM_GPORT_TUNNEL_ID_GET(mim_port->service_encap_id);
            }
            /* allocate LIF */
            rv = bcm_dpp_am_l2_ac_alloc(unit, _BCM_DPP_AM_L2_AC_TYPE_ISID, lif_flags, &lif_id);
            BCMDNX_IF_ERR_EXIT(rv);

            BCM_GPORT_TUNNEL_ID_SET(mim_port->service_encap_id, lif_id);
        ...
        soc_sand_rv = soc_ppd_l2_lif_isid_add(soc_sand_dev_id, vsi_index, &isid_key, lif_id, &isid_info, &success);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        SOC_SAND_IF_FAIL_RETURN(success);
        ...
        phy_port->type = _BCM_DPP_GPORT_IN_TYPE_MIM_P2P;
        phy_port->phy_gport = lif_id; /* LIF */
        ...
        rv = _bcm_dpp_in_lif_mim_match_add(unit, mim_port, lif_id);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else { /* MP */
        ...
        /* update previous entry / fill new entry */
        phy_port->type = _BCM_DPP_GPORT_IN_TYPE_MIM_MP;
    ...
    bcm_mim_port_t * mim_port)
{
    ...
    if (phy_port->type != _BCM_DPP_GPORT_IN_TYPE_MIM_MP && phy_port->type != _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) {
    ...
    if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) { /* P2P */
        ...
        soc_sand_rv = soc_ppd_l2_lif_isid_get(soc_sand_dev_id, &isid_key, &lif, &vsi_index, &isid_info, &found);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

        BCM_GPORT_TUNNEL_ID_SET(mim_port->service_encap_id, lif);
    }
    else {
    ...
    /* Verify LIF is MIM */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(soc_sand_dev_id, -1, mim_port->encap_id, NULL, &lif_usage));
    ...
    bcm_gport_t mim_port_id)
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_usage_get(unit, -1, mim_port.encap_id, NULL, &lif_usage));
    if (lif_usage != _bcmDppLifTypeMim) {
    ...
    /* remove EEP entry */
    soc_sand_rv = soc_ppd_eg_encap_entry_remove(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_LL, mim_port.encap_id);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    if (phy_port->type == _BCM_DPP_GPORT_IN_TYPE_MIM_P2P) { /* P2P */
        ...
        /* delete lif from HW */
        soc_sand_rv = soc_ppd_l2_lif_isid_remove(unit, &isid_key, &isid_lif_id);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        /* de-allocate LIF */
        rv = bcm_dpp_am_l2_ac_dealloc(unit, 0, lif);
        BCMDNX_IF_ERR_EXIT(rv);

        /* remove mim lif match SW */
        rv = _bcm_dpp_in_lif_match_delete(unit, lif);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* de-allocate EEP */
    rv = bcm_dpp_am_l3_eep_dealloc(unit, mim_port.encap_id);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Remove entry from SW DB */
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_out_lif_match_delete(unit,mim_port.encap_id));
