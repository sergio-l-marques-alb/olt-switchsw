...
STATIC
int th_flex_counter_set(int unit, bcm_stat_flex_direction_t direction,
    ...
    bcm_stat_flex_direction_t       direction)
{
    ...
    *flex_entries = total_entries;
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        return th_flex_counter_set(unit, direction, pool_number,
         ...
         for (pool_id=0;pool_id<num_pools[direction];pool_id++) {
              ...
              if (SOC_IS_TOMAHAWK(unit)) {
                  if (residual_byte_counter[unit][direction][pool_id] != NULL) {
                      soc_cm_sfree(unit,
                            residual_byte_counter[unit][direction][pool_id]);
                      residual_byte_counter[unit][direction][pool_id] = NULL;
                  }
                  if (residual_packet_counter[unit][direction][pool_id] !=
                      NULL) {
                      soc_cm_sfree(unit,
                           residual_packet_counter[unit][direction][pool_id]);
                      residual_packet_counter[unit][direction][pool_id] = NULL;
                  }
              }
#endif
    ...
    for (direction=bcmStatFlexDirectionIngress;direction<2;direction++) {
         ...
         for (pool_id=0;pool_id<num_pools[direction];pool_id++) {
              ...
              if (SOC_IS_TOMAHAWK(unit)) {
                  ...
                  sal_memset(residual_packet_counter[unit][direction][pool_id],
                  ...
                  sal_memset(residual_byte_counter[unit][direction][pool_id], 0,
                         sizeof(uint64)*size_pool[direction]);
              }
#endif
                      ...
                      int num_entries,  int byte_flag)
{
    ...
    for (cnt = 0; cnt < num_entries; cnt++) {
        ...
        if (1 == byte_flag || -1 == byte_flag) {
            ...
            COMPILER_64_SET(flex_byte_counter[unit][direction][pool_id]
                                             [index + cnt],
                            COMPILER_64_HI(value.bytes),
                            COMPILER_64_LO(value.bytes));
            COMPILER_64_ZERO(residual_byte_counter[unit][direction][pool_id]
                                                  [index + cnt]);
        }
        ...
        if (0 == byte_flag || -1 == byte_flag) {
            ...
            COMPILER_64_SET(flex_packet64_counter[unit][direction][pool_id]
                                                 [index + cnt],0,
                            value.packets);
            COMPILER_64_ZERO(residual_packet_counter[unit][direction][pool_id]
                                                    [index + cnt]);
        }
    }
         ...
         for (pool_id = 0; pool_id < num_pools[direction]; pool_id++) {
             ...
             for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
              ...
              for (index = soc_mem_index_min(unit,mem);
                   ...
                   index++) {
                  COMPILER_64_ZERO(pkt_ctr_new);
                  COMPILER_64_ZERO(byte_ctr_new);
                  ...
                  if (rv != BCM_E_NONE) {
                   ...
                   for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                       ...
                       COMPILER_64_SET(byte_ctr_hw, byte_count_read[1],
                                       byte_count_read[0]);
                       COMPILER_64_ADD_64(byte_count,byte_ctr_hw);
                   }
                   ...
                   if (0 == soc->ctr_evict_interval) {
                       ...
                       COMPILER_64_SUB_64(temp_count, pkt_ctr_new);
                       COMPILER_64_SUB_64(temp_count, residual_packet_counter[unit]
                                          [direction][pool_id][index]);
                       old_pkt_count = COMPILER_64_LO(temp_count);
                       if (old_pkt_count > packet_count) {
                           packet_count += (max_packet_count - old_pkt_count);
                       }

                       COMPILER_64_ZERO(temp_count);
                       COMPILER_64_ADD_64(temp_count, flex_byte_counter[unit]
                                         [direction][pool_id][index]);
                       COMPILER_64_SUB_64(temp_count, byte_ctr_new);
