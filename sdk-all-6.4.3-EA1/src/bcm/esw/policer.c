    ...
    uint32                     *total_udf_bits)
{
    ...
    if (*total_udf_bits > BCM_POLICER_SVC_METER_OFFSET_TABLE_KEY_SIZE) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Invalid selection of UDF bits - exceeds max allowed\n")));
        return BCM_E_PARAM;
    }
    ...
    uint8       *current_bit_selector_position)
{
    ...
    if ((*current_bit_selector_position) + 
        ...
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Number of slector bits exceeds max allowed \n"),
                   FUNCTION_NAME()));
    ...
    bcm_policer_svc_meter_mode_t svc_meter_mode)
{
    ...
    if (!((svc_meter_mode >= 1) && (svc_meter_mode <= 3))) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META("%s: Wrong offset mode specified \n"),
                   FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
    ...
    bcm_policer_svc_meter_mode_t            svc_meter_mode)
{
    ...
    if ( global_meter_offset_mode[unit][svc_meter_mode].used == 0) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META("%s: Wrong offset mode: Mode is not in use\n"), 
                   FUNCTION_NAME()));
        return BCM_E_NOT_FOUND;
    }
    if ( global_meter_offset_mode[unit][svc_meter_mode].reference_count != 0) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META("%s: Mode is still in use\n"),
                   FUNCTION_NAME()));
        return BCM_E_INTERNAL;
    }
    ...
    switch (mode_type_v) {
       ...
       default:
           LOG_ERROR(BSL_LS_BCM_POLICER,
                     (BSL_META("%s :Invalid offset mode\n"),
                      FUNCTION_NAME()));
           return BCM_E_PARAM;
    }
    ...
    pkt_attr_bits_t    pkt_attr_bits_v)
{
    ...
    if (!((pkt_attr_selector_key >= ING_SVM_PKT_ATTR_SELECTOR_KEY_0r) && 
          (pkt_attr_selector_key <= ING_SVM_PKT_ATTR_SELECTOR_KEY_3r))) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Invalid Key for packet attribute selector\n"),
                       FUNCTION_NAME()));
            return BCM_E_PARAM;
    }
    ...
    bcm_policer_svc_meter_mode_t *svc_meter_mode)
{
    ...
    if (total_bits_used > BCM_POLICER_SVC_METER_OFFSET_TABLE_KEY_SIZE) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Key size exceeds max allowed size \n"), 
                   FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
    ...
    bcm_policer_svc_meter_mode_t *svc_meter_mode)
{
    ...
    if ( mode_info == NULL) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Failed to allocate memory for mode bookkeep "
                                "info\n"), FUNCTION_NAME()));
        return BCM_E_MEMORY;
    }
            ...
            if ((mode_info->meter_attr.mode_type_v != compressed_mode) ||
               (group_mode != mode_info->group_mode )) {
                if (mode_info->meter_attr.mode_type_v == compressed_mode) {
                    LOG_ERROR(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Some other policer group is already "
                                          "using this resource\n"), FUNCTION_NAME()));
                    sal_free(mode_info);
                    return BCM_E_PARAM;
                } 
    ...
    if (config_pkt_attr->cng != 0) {
        if (config_pkt_attr->cng > BCM_POLICER_SVC_METER_CNG_ATTR_SIZE) { 
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s :CNG attribute size exceeds max allowed size\n"),
                       FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->int_pri != 0) {
        if (config_pkt_attr->int_pri > BCM_POLICER_SVC_METER_INT_PRI_ATTR_SIZE) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : int_pri attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->vlan_format != 0) {
        if (config_pkt_attr->vlan_format > 
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Vlan attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->outer_dot1p != 0) {
        if (config_pkt_attr->outer_dot1p > 
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Outer DOT1P attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->inner_dot1p != 0) {
        if (config_pkt_attr->inner_dot1p >
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Inner DOT1P attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->ing_port != 0) {
        if (config_pkt_attr->ing_port > 
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : port attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->tos != 0) {
        if (config_pkt_attr->tos > BCM_POLICER_SVC_METER_TOS_ATTR_SIZE) { 
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : TOS attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->pkt_resolution != 0) {
        if (config_pkt_attr->pkt_resolution > 
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Pkt resolution attribute size exceeds max "
                                  "allowed size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->svp_type != 0) {
        if (SOC_IS_KATANA2(unit)) {
            if (config_pkt_attr->svp_type > 
                ...
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s : SVP type attribute size exceeds max allowed "
                                      "size\n"), FUNCTION_NAME()));
                return BCM_E_PARAM;
            }
        } else { 
    ...
    if (config_pkt_attr->drop != 0) {
        if (config_pkt_attr->drop > BCM_POLICER_SVC_METER_DROP_ATTR_SIZE) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : DROP attribute size exceeds max allowed  "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (config_pkt_attr->ip_pkt != 0) {
        if (config_pkt_attr->ip_pkt > BCM_POLICER_SVC_METER_IP_PKT_ATTR_SIZE) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : IP pkt attribute size exceeds max allowed "
                                  "size\n"), FUNCTION_NAME()));
            return BCM_E_PARAM;
        }
    ...
    if (total_bits_used > BCM_POLICER_SVC_METER_OFFSET_TABLE_KEY_SIZE) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Key size exceeds max allowed  "
                              "size\n"), FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
...
_bcm_esw_global_meter_cleanup(int unit)
{
    ...
    if (0 == global_meter_status[unit].initialised) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Global meter feature not initialized for "
                                "unit %d\n"),FUNCTION_NAME(), unit));
        return rv;
    }
    /* mutex destroy */
    ...
    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "%s : Clening up global meter config\n"),
                 FUNCTION_NAME()));
    return rv;
}
...
bcm_esw_global_meter_init(int unit)
{
    ...
    if ( BCM_POLICER_GLOBAL_METER_DISABLE == service_meter_control) {
        ...
        LOG_WARN(BSL_LS_BCM_POLICER,
                 (BSL_META_U(unit,
                             "%s : GLOBAL METER FEATURE disabled "
                             "at HW for unit %d\n"), FUNCTION_NAME(), unit));
        return rv;
    }
    num_pools =  SOC_INFO(unit).global_meter_pools;
    ...
    if ((total_size) > (soc_mem_index_max(unit, SVM_METER_TABLEm) + 1))
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Number of global meters exceeding "
    ...
    if ((action_size) > (soc_mem_index_max(unit, SVM_POLICY_TABLEm) + 1))
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Global meter actions exceeding "
                              "its max value for unit %d \n"), FUNCTION_NAME(), unit));
        return BCM_E_INTERNAL;
    } 
    ...
    if (NULL == global_meter_policer_bookkeep[unit]) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate memory for policer bookkeep "
    ...
    if (NULL == global_meter_hz_alloc_bookkeep[unit]) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate memory for hz alloc bookkeep "
                              "data structure for unit %d \n"), FUNCTION_NAME(), unit));
        return (BCM_E_MEMORY);
    }
    ...
    if (NULL == global_meter_action_bookkeep[unit]) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate memory for meter action bookkeep "
                              "data structure for unit %d \n"), FUNCTION_NAME(), unit));
        return (BCM_E_MEMORY);
    }
    if (global_meter_mutex[unit] == NULL) {
        ...
        if (global_meter_mutex[unit] == NULL) {
            ...
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to create mutex for unit %d \n"),
                       FUNCTION_NAME(), unit));
            return BCM_E_MEMORY;
        }
    }
    ...
    if (shr_aidxres_list_create(
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Couldn't create alligned list "
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Not able to reserve first entry "
    ...
    for (pool_id = 0; pool_id < num_pools; pool_id++) {
                      ...
                      8, "global_meter_policer") != BCM_E_NONE) 
        {
            ...
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Couldn't create alligned list for "
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (!BCM_SUCCESS(rv)) {
            ...
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Warm boot init failed for unit %d \n"),
                       FUNCTION_NAME(), unit));
            return BCM_E_INTERNAL;
        }
    } else { /* cold boot */
    ...
    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "%s :  GLOBAL METER FEATURE initialised "
                            "for unit %d \n"), FUNCTION_NAME(), unit));
...
int   _bcm_esw_policer_validate(int unit, bcm_policer_t *policer) 
{
    ...
    if (index > (soc_mem_index_max(unit, SVM_METER_TABLEm)))
    {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Invalid table index for SVM_POLICY_TABLE\n"),
                   FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
...
int  _bcm_esw_policer_increment_ref_count(int unit, bcm_policer_t policer) 
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to get policer control for policer "
                                "id %d \n"), FUNCTION_NAME(), policer));
        return (rv);
    }
        ...
        switch (table) {
            ...
            case VLAN_XLATE_EXTDm:
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                                (BSL_META_U(unit,
                                            "%s : Unable to read the table entry %d at "
                ...
                if (index >= soc_mem_index_count(unit, SVM_METER_TABLEm))  {
                    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                                (BSL_META_U(unit,
                                            "%s : Invalid table index passed for "
                                            "SVM_METER_TABLE\n"), FUNCTION_NAME()));
                    return BCM_E_PARAM;
                }
                                      ...
                                      int table, int index, void *data) 
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to delete policer from table %d at "
                              "index %d \n"),FUNCTION_NAME(), table, index));
        return (rv);
    }
        ...
        switch (table) {
            ...
            case VLAN_XLATE_EXTDm:
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Unable to read the policer from table "
                                          "%d at index %d\n"), FUNCTION_NAME(), table, index));
                    return (rv);
                }
                if (current_policer != policer) {
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Policer Id passed is different from the "
    ...
    uint32 *action_id)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate a handle for new "
                              "action\n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    uint32 param0)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to read SVM_POLICY_TABLE for given "
                              "action id \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    switch (action) { 
       ...
       default:
          ...
          LOG_ERROR(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unsupported Action specified\n"),
                     FUNCTION_NAME()));
          return BCM_E_PARAM;
    }
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to write to SVM_POLICY_TABLE at location "
    ...
    uint32 action_id)
{
    ...
    if (global_meter_action_bookkeep[unit][action_id].used != 1)
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Action Id specified doesn't exist\n"),
                   FUNCTION_NAME()));
        return BCM_E_PARAM;
    }
                                  ...
                                  [action_id].reference_count != 0)
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Action Id specified still being used \n"),
                   FUNCTION_NAME()));
        return BCM_E_BUSY;
    }
    GLOBAL_METER_LOCK(unit);
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to read SVM_POLICY_TABLE at location "
                              "specified by action_id \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to free action handle \n"),
                     FUNCTION_NAME()));
        return (rv);
    }
    GLOBAL_METER_UNLOCK(unit);
                                                  ...
                                                  uint32 policer_index) 
{
    ...
    for (index = 0;index < numbers; index++) {
        ...
        if (!BCM_SUCCESS(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to free policer handle \n"),
                       FUNCTION_NAME()));
            return BCM_E_INTERNAL;
        }
                                         ...
                                         uint8 *offset)
{
        ...
        /* check if we have enough free policers to allocate */
            ...
            if (allocated_policer != *numbers) {
                ...
                *pid = index; 
                ...
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s : Allocated base policer with index %x \n"),
                           FUNCTION_NAME(), index));
                return BCM_E_NONE;
            }
        }
    ...
    LOG_DEBUG(BSL_LS_BCM_POLICER,
              (BSL_META_U(unit,
                          "%s : Unable to allocate policer as table is full  \n"),
               FUNCTION_NAME()));
    return BCM_E_FULL;
}
        ...
        if (alloc_control[pid].no_of_groups_allocated == 1) {
            if (pool_id + offset[index - 1] == 
                ...
                if (!BCM_SUCCESS(rv)) {
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Unable to get policer control for pid "
                                          "%x\n"), FUNCTION_NAME(), pid));
                    return rv;
                }
                alloc_control[pid].first_bit_to_use = 
                ...
                if (num_pol > 0 ) {
                    ...
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Number of policers in policer control "
                                          "structure is 0\n"), FUNCTION_NAME()));
                    return BCM_E_INTERNAL;
                } 
                                             ...
                                             uint8 *pid_offset) 
{
    ...
    if (direction == GLOBAL_METER_ALLOC_VERTICAL) {
        ...
        if (!BCM_SUCCESS(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to reserve policer in shared index "
                                  "management\n"), FUNCTION_NAME()));
            return BCM_E_INTERNAL;
        }
        for (index = 0; index < numbers; index++) {
            ...
            if (!BCM_SUCCESS(rv)) {
                ...
                if (!BCM_SUCCESS(rv)) {
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Unable to free policer in shared "
                                          "index management\n"), FUNCTION_NAME()));
                    return BCM_E_INTERNAL;
                }
                for (free_index = 0; free_index < index; free_index++) {
                    ...
                    if (!BCM_SUCCESS(rv)) {
                        LOG_DEBUG(BSL_LS_BCM_POLICER,
                                  (BSL_META_U(unit,
                                              "%s : Unable to free policer in hz "
                                              "index management\n"), FUNCTION_NAME()));
                        return BCM_E_INTERNAL;
                    }
                }
        ...
        for (index = 0; index < numbers; index++) {
            ...
            if (!BCM_SUCCESS(rv)) {
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s : Unable to reserve policer in shared "
                                      "index management\n"), FUNCTION_NAME()));
                return BCM_E_INTERNAL;
            }
            rv = _bcm_global_meter_reserve_bloc_horizontally(unit,
        ...
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Invalid direction for policer allocation \n"),
                   FUNCTION_NAME()));
        return BCM_E_INTERNAL;
    }
    return rv;
        ...
        for ( ;pool < num_pools; pool++) {
            ...
            if (BCM_SUCCESS(rv)) {
                ...
                *pid = *pid | (pool << _shr_popcount(size_pool - 1));
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s : allocated policer with pid %x \n"),
                           FUNCTION_NAME(), *pid));
                return rv;
            }
            no_of_pools_checked++;
                                          ...
                                          bcm_policer_config_t *pol_cfg) 
{
    ...
    if (!BCM_SUCCESS(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to read SVM METER TABLE at index %d "
                              "\n"), FUNCTION_NAME(), index));
        return rv;
    }
    ...
    if (pol_cfg->mode == bcmPolicerModeCoupledCascade) {
        ...
        if (!BCM_SUCCESS(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to translate rate in kbps to bucket size "
                                  "and granularity \n"), FUNCTION_NAME()));
            return rv;
        }
    ...
    if (pol_cfg->mode == bcmPolicerModeCoupledCascade) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to get policer control for the policer Id "
                                  "passed  \n"), FUNCTION_NAME()));
            return (rv);
        }
                                ...
                                uint32 action_id)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to read SVM_METER_TABLE entry \n"),
                   FUNCTION_NAME()));
        return (rv);
    }
    /* set action to 0 */
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to write SVM_METER_TABLE entry \n"),
                   FUNCTION_NAME()));
        return (rv);
    }
                  ...
                  _global_meter_policer_control_t *policer_control)
{
    ...
    if ((coupling == 1) && (mode == GLOBAL_METER_MODE_CASCADE)) {
        ...
        if (!BCM_SUCCESS(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to configure the cascaded pair of "
                                  "policers \n"), FUNCTION_NAME()));
            return rv;
        }
    } 
    if (pol_cfg->flags & BCM_POLICER_MIXED_MICRO_MACRO) {
        ...
        if (macro_flow_meter_index > 0) {
            ...
            if (!(BCM_SUCCESS(rv))) {
                LOG_ERROR(BSL_LS_BCM_COMMON, \
                          (BSL_META_U(unit,
                                      "%s : Unable to write to macro flow table at index \
                                      provided\n"), FUNCTION_NAME()));
                return rv;
            }
        }
   ...
   /* we come here from bcm_esw_policer_get,
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to read policer config from hw %x\n"),
                       FUNCTION_NAME(), policer_id));
            GLOBAL_METER_UNLOCK(unit);
            return (rv);
        }
...
int _bcm_global_meter_min_burst_size_set(bcm_policer_config_t *pol_cfg)
{
    ...
    if (min_burst > pol_cfg->ckbits_burst) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META("%s : Commited burst is less than the minimum required "
                            "value.  \n"), FUNCTION_NAME()));
        return (BCM_E_PARAM); 
    } 
    min_burst = (MIN_BURST_MULTIPLE * 
    ...
    if (min_burst > pol_cfg->pkbits_burst) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META("%s : Peak burst is less than the minimum required "
                            "value.  \n"), FUNCTION_NAME()));
        return (BCM_E_PARAM); 
    } 
    return (BCM_E_NONE); 
    ...
    if (BCM_SUCCESS(rv)) {
       ...
       /* Add code to write this to HW */
        ...
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to write policer config to hw %x\n"),
                       FUNCTION_NAME(), policer_id));
            GLOBAL_METER_UNLOCK(unit);
            return (rv);
        }
...
int _bcm_esw_global_meter_policer_destroy(int unit, bcm_policer_t policer_id)
{
    ...
    if (BCM_FAILURE(rv)) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to get policer control for policer id %x\n"),
                     FUNCTION_NAME(), policer_id));
        return (rv);
    }
    rv = _bcm_esw_global_meter_policer_destroy2(unit, policer_control);
               ...
               _global_meter_policer_control_t *policer_control) 
{
    ...
    if (policer_control->direction == GLOBAL_METER_ALLOC_VERTICAL) {
        ...
        for (index = 0; index < numbers; index++) {
            ...
            if (!BCM_SUCCESS(rv)) {
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s : Unable to free policer handle in hz "
                                      "index management\n"), FUNCTION_NAME()));
                return BCM_E_INTERNAL;
            }
        }
   ...
   /* Delete policer from policers hash. */
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to Get policer index for "
                              "policer \n"), FUNCTION_NAME()));
        return rv;
    }
        ...
        for (entry_index = 0; entry_index < numbers; entry_index++) {
            ...
            if (macro_flow_meter_index > 0) {
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                                (BSL_META_U(unit,
                                            "%s : Unable to decrement ref count "
        ...
        if (entry_modified) { 
            if ((rv = soc_mem_write_range(unit, SVM_MACROFLOW_INDEX_TABLEm,
                ...
                if (BCM_FAILURE(rv)) {
                         LOG_VERBOSE(BSL_LS_BCM_POLICER,
                                     (BSL_META_U(unit,
                                                 "%s : Unable to write to macro flow "
                          ...
                          bcm_policer_group_mode_t mode, uint8 *pid_offset) 
{
    ...
    for (index=0; index < numbers; index++) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to read SVC METER TABLE at "
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to write SVC METER TABLE at "
                                  "offset %d\n"), FUNCTION_NAME(), table_offset));
            return (rv);
        }
    }
                               ...
                               offset_table_entry_t *offset_map)
{
    ...
    if (num_offsets >= BCM_SVC_METER_MAP_SIZE_256) {  
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META("%s : Number of offsets passed is more than map table "
                               ...
                               offset_table_entry_t *offset_map) 
{
    ...
    for(i = 0; i < num_offsets; i++) {
        if ((table_offset[i] >= BCM_SVC_METER_MAP_SIZE_256) || 
            ...
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META("%s : Offset/value passed is more than map table "
                                   ...
                                   int *npolicers)
{
    ...
    if ( mode_attr == NULL) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Failed to allocate memory for svc meter attr \n"),
                     FUNCTION_NAME()));
        return BCM_E_MEMORY;
    }
        ...
        if (BCM_FAILURE(rv) && (rv != BCM_E_EXISTS)) {
            ...
            /* set group mode and number of policers in offset table at
             ...
             if (BCM_FAILURE(rv)) {
                 ...
                 LOG_VERBOSE(BSL_LS_BCM_POLICER,
                             (BSL_META_U(unit,
                                         "%s : Unable to write to offset table  \n"),
                              FUNCTION_NAME()));
                 return rv;
             } 
        }
    ...
    if (NULL == policer_control) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to allocate memeory for policer control \n"),
                     FUNCTION_NAME()));
        return (BCM_E_MEMORY);
    }
                                   ...
                                   bcm_policer_t *policer_id)
{
    ...
    if (flags == BCM_POLICER_GLOBAL_METER_ENVELOP_MACRO_FLOW) {
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to create macro flow policer\n"),
                       FUNCTION_NAME()));
            return rv;
        } 
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to Get policer index for macro flow "
                                  "policer \n"), FUNCTION_NAME()));
            return rv;
        } 
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to create micro flow policer\n"),
                       FUNCTION_NAME()));
            return rv;
        } 
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to increment ref count for  micro flow "
                                  "policer\n"), FUNCTION_NAME()));
            return rv;
        } 
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to Get policer index for micro flow "
                                  "policer \n"), FUNCTION_NAME()));
            return rv;
        } 
        /* Write the index of the macro-meter at "micro_flow_index" location 
        ...
        if (!(BCM_SUCCESS(rv))) {
            LOG_VERBOSE(BSL_LS_BCM_POLICER,
                        (BSL_META_U(unit,
                                    "%s : Unable to access macro flow table at the index "
                                    "provided\n"), FUNCTION_NAME()));
            return rv;
        } 
                                   ...
                                   bcm_policer_t *policer_id, int *npolicers)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to create policer for given mode %d \n"),
                     FUNCTION_NAME(),mode));
        return (rv);
    }
    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "%s : Created policer group of %d with base policer %x \n"),
                                          ...
                                          int *num_policers)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "%s : Created policer group of %d with base policer %x \n"),
                 FUNCTION_NAME(), *num_policers, *policer_id));
    return rv;
}
                                     ...
                                     int *npolicers)
{
    ...
    if (flag == BCM_POLICER_GLOBAL_METER_ENVELOP_MACRO_FLOW) {
        ...
        if ((mode == bcmPolicerGroupModeCascade) ||
            ...
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to create micro flow policers "
                                ...
                                uint32 action_id)
{
    ...
    if (policer_control->action_id == action_id) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Action Id passed is different from the one in "
                           ...
                           _global_meter_policer_control_t *policer_control) 
{
   ...
   if (NULL == policer_control) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate memory for Policer control data "
                              "structure\n"), FUNCTION_NAME()));
       return (BCM_E_MEMORY);
   }
   ...
   _bcm_esw_get_policer_table_index(unit, policer, &policer_index); 
    if(mode == GLOBAL_METER_MODE_CASCADE) {
       ...
       for (index = pool + 1; index < num_pools; index++) {
           ...
           if (!BCM_SUCCESS(rv)) {
               ...
               LOG_DEBUG(BSL_LS_BCM_POLICER,
                         (BSL_META_U(unit,
                                     "%s : Unable to read SVM meter table at "
    ...
    if (!BCM_SUCCESS(rv)) {
        ...
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to allocate policers with base id %x\n"),
                   FUNCTION_NAME(), policer));
        return BCM_E_INTERNAL;
    }
    /* Insert policer into policers hash. */
...
int _bcm_policer_config_reinit_from_table(int unit, soc_mem_t table,void *data)
{
    ...
    for ( index = 0; index <= max_index; index++) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to read policer entry from table %d at"
                                  "%d \n"), FUNCTION_NAME(), table, index));
            return (rv);
        }
        /* get policer config */
        if ((policer & BCM_POLICER_GLOBAL_METER_INDEX_MASK) > 0) {
            ...
            if (table == SVM_MACROFLOW_INDEX_TABLEm) {
                ...
                if (BCM_FAILURE(rv)) {
                    LOG_DEBUG(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "%s : Unable to increment the policer usage "
...
int _bcm_policer_config_reinit(int unit) 
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from Port "
                              "table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from vlan "
                              "table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    if (SOC_IS_KATANAX(unit)) {
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from svp "
                              "table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from vfi "
                              "table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from VFP "
                              "policy table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to re-init policer config from Macro flow "
                              "index table  \n"), FUNCTION_NAME()));
        return (rv);
    }
    return rv;
                                                              ...
                                                              void *data)
{
    ...
    for (index = 0; index <= max_index; index++) {
        ...
        if (BCM_FAILURE(rv)) {
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "%s : Unable to get policer usage info from table "
                                  "%d at index %d \n"), FUNCTION_NAME(), table, index));
            return (rv);
        }
        if (policer > 0) {
...
int _bcm_policer_ref_count_reinit(int unit) 
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to get policer usage info from port "
                              "table \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to get policer usage info from Vlan "
                              "table \n"), FUNCTION_NAME()));
        return (rv);
    }
    if (SOC_IS_KATANAX(unit)) {
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to get policer usage info from SVP "
                              "table \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to get policer usage info from VFI "
                              "table \n"), FUNCTION_NAME()));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "%s : Unable to get policer usage info from VFP "
                              "policy table \n"), FUNCTION_NAME()));
        return (rv);
    }
    return rv;
        ...
        if (!COMPILER_64_IS_ZERO(selector_key_value))
        {
                                      ...
                                      selector_key_value, USE_UDF_KEYf)) 
            {
                ...
                if (BCM_FAILURE(rv)) {
                   LOG_DEBUG(BSL_LS_BCM_POLICER,
                             (BSL_META_U(unit,
                                         "%s : Unable to re-init UDF offset mode\n"),
                              FUNCTION_NAME()));
                   return (rv);
                }
            ...
            {            
                ...
                if (BCM_FAILURE(rv)) {
                   LOG_DEBUG(BSL_LS_BCM_POLICER,
                             (BSL_META_U(unit,
                                         "%s : Unable to re-init compressed offset "
                                         "mode\n"), FUNCTION_NAME()));
                   return (rv);
                }
            } else {
                ...
                if (BCM_FAILURE(rv)) {
                   LOG_VERBOSE(BSL_LS_BCM_POLICER,
                               (BSL_META_U(unit,
                                           "%s : Unable to re-init uncompressed offset "
                                           "mode\n"), FUNCTION_NAME()));
                   return (rv);
                }
            }
            ...
            if (BCM_FAILURE(rv)) {
               LOG_DEBUG(BSL_LS_BCM_POLICER,
                         (BSL_META_U(unit,
                                     "%s : Unable to re-init number of policers "
...
int _bcm_esw_global_meter_action_reinit(unit)
{
    ...
    for (index = 0; index <= total_size; index++) {
        ...
        if (BCM_FAILURE(rv)) {
            GLOBAL_METER_UNLOCK(unit);
            LOG_DEBUG(BSL_LS_BCM_POLICER,
                      (BSL_META("%s : Unable to read SVM_POLICY_TABLE at "
                                "index %d \n"), FUNCTION_NAME(), index));
            return (rv);
        }
                                                      ...
                                                      (red_action[0] > 0)) 
        {
            ...
            if (BCM_FAILURE(rv)) {
                GLOBAL_METER_UNLOCK(unit);
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META("%s : Unable to reserve action id %d in "
...
int _bcm_esw_global_meter_reinit(int unit)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to reinit offset modes for  "
                                "unit %d\n"), FUNCTION_NAME(), unit));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to reinit policer configuration for  "
                                "unit %d\n"), FUNCTION_NAME(), unit));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to reinit policer and offset mode ref count "
                                "for unit %d\n"), FUNCTION_NAME(), unit));
        return (rv);
    }
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "%s : Unable to reinit meter action configuration for  "
                                "unit %d\n"), FUNCTION_NAME(), unit));
        return (rv);
    }
    return BCM_E_NONE; 
