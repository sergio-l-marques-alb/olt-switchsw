    ...
    for (i = 0; i < port_count ; i++) {
        ...
        if (BCM_TRUNK_INVALID != trunk_id) {
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                 ...
                 *
                 * For trunk port, only first memebr port is picked
                 * and programmed into REPL_GROUP table and hence there
                 * will be single entry in REPL_HEAD table representing 
                 * all trunk memebers. 
                 *
                 * REPL_PORT_AGG_MAP table will be programmed 
                 * with same L3MC-port-Agg-ID (first member port of trunk)
                 * for all trunk members.
                 */
                uint32 regval = 0;
                int mmu_port;   /* MMU port number        */
                int port;       /* Local trunk port       */
                int ltp_idx;    /* Local trunk port index */
                mmu_port = local_trunk_port_array[i][0];
                mmu_port = SOC_INFO(unit).port_l2p_mapping[mmu_port];
                mmu_port = SOC_INFO(unit).port_p2m_mapping[mmu_port];
                for (ltp_idx = 0; ltp_idx < trunk_local_ports[i]; ltp_idx++) {
                    port = local_trunk_port_array[i][ltp_idx];
                    soc_reg_field_set(unit, MMU_DQS_REPL_PORT_AGG_MAPr, &regval,
                                      L3MC_PORT_AGG_IDf, (mmu_port & 0x3f));
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                uint32 regval = 0;
                soc_reg_field_set(unit, MMU_DQS_REPL_PORT_AGG_MAPr, &regval,
                                  L3MC_PORT_AGG_IDf, (port_out & 0x3f));
                SOC_IF_ERROR_RETURN(soc_reg32_set(unit, MMU_DQS_REPL_PORT_AGG_MAPr,
                                    port_out, 0, regval));
            }
        }
            ...
            BCM_PBMP_ITER(mc_eligible_pbmp, port_iter) {
                ...
                for (i = 0; i < port_count ; i++) {
                    if (BCM_GPORT_IS_TRUNK(local_port_array[i])) {
                         ...
                         * Tomahawk introduced REPL_PORT_AGG_MAP table.
                         *
                         * For trunk port, only first memebr port is programmed
                         * into REPL_GROUP table and hence there will be single
                         * entry in REPL_HEAD table representing all trunk 
                         * memebers. 
                         *
                         * However bits corrosponding to ALL trunk members need 
                         * to be set in L3_IPMC.l3_pbmp 
                         * 
                         * Similarly, REPL_PORT_AGG_MAP table will be programmed 
                         * with same L3MC-port-Agg-ID for all trunk members.
                         * ( grep SOC_IS_TOMAHAWK for code above) 
                         */
                        if (SOC_IS_TOMAHAWK(unit)) {
                        ...
                        for (idx = 0; idx < idx_max; idx++) {
                            if (local_trunk_port_array[i][idx] == port_iter) {
                                ...
                                if (SOC_IS_TOMAHAWK(unit)) {
                                    ...
                                    int ltp_idx; /* Local trunk port index   */
                                    for(ltp_idx = 0; ltp_idx < trunk_local_ports[i]; ltp_idx++) {
                                         BCM_PBMP_PORT_ADD(l3_pbmp, local_trunk_port_array[i][ltp_idx]);
                                    } /* end of ltp_idx for loop */
                                } /* SOC_IS_TOMAHAWK */
                                break;
                            }
                        }
