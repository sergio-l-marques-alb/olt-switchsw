...
int _bcm_ptp_stack_destroy(int unit, bcm_ptp_stack_id_t ptp_id)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_signaling_teardown(unit, ptp_id))) {
        PTP_ERROR_FUNC("_bcm_ptp_signaling_teardown()");
        return rv;
    }

    
    /*
    if (BCM_FAILURE(rv = bcm_tdpll_esmc_shutdown(unit, ptp_id))) {
        PTP_ERROR_FUNC("bcm_tdpll_esmc_shutdown()");
        return rv;
    }

    if (BCM_FAILURE(rv = bcm_esmc_shutdown(unit, ptp_id))) {
        PTP_ERROR_FUNC("bcm_esmc_shutdown()");
        return rv;
    }

    if (BCM_FAILURE(rv = bcm_tdpll_output_clock_shutdown(unit, ptp_id))) {
        PTP_ERROR_FUNC("bcm_tdpll_output_clock_shutdown()");
        return rv;
    }
    */

    if (BCM_FAILURE(rv = bcm_tdpll_input_clock_shutdown(unit))) {
        PTP_ERROR_FUNC("bcm_tdpll_input_clock_shutdown()");
        return rv;
    }

    
    /*
    if (BCM_FAILURE(rv = bcm_tdpll_dpll_instance_shutdown(unit, ptp_id))) {
        PTP_ERROR_FUNC("bcm_tdpll_instance_shutdown()");
        return rv;
    }
    */
