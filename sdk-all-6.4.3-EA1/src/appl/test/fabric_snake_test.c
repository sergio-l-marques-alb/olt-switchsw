...
fabric_snake_test_init(int unit, args_t *a, void **p)
{
   ...
   /*fill fabric_snake_test_params structure */
    ...
    {
        test_params[unit].loopback_flag = SOC_DFE_ENABLE_PHY_LOOPBACK;
    } else if (! sal_strncasecmp(loopback_str, "MAC", sal_strlen(loopback_str)) ) 
    {
        test_params[unit].loopback_flag = SOC_DFE_ENABLE_MAC_LOOPBACK;
    } else if (! sal_strncasecmp(loopback_str, "EXTERNAL", sal_strlen(loopback_str)) )
    {
        test_params[unit].loopback_flag = SOC_DFE_ENABLE_EXTERNAL_LOOPBACK;
    } else {
    ...
    {
        test_params[unit].action_flags = FABRIC_SNAKE_TEST_ALL;
    } else if (! sal_strncasecmp(action_str, "PREPARE_ONLY", sal_strlen(action_str)) ) 
    {
        test_params[unit].action_flags = FABRIC_SNAKE_TEST_PREPARE_ONLY;
    } else if (! sal_strncasecmp(action_str, "RUN_ONLY", sal_strlen(action_str)) )
    {
        test_params[unit].action_flags = FABRIC_SNAKE_TEST_RUN_ONLY;
    } else {
    ...
    {
        test_params[unit].snake_test_run_flags = 0x0;
    } else if (! sal_strncasecmp(run_mode_str, "INFINITE", sal_strlen(run_mode_str)) ) 
    {
        test_params[unit].snake_test_run_flags = SOC_DFE_SNAKE_INFINITE_RUN;
    } else if (! sal_strncasecmp(run_mode_str, "STOP", sal_strlen(run_mode_str)) )
    {
        test_params[unit].snake_test_run_flags = SOC_DFE_SNAKE_STOP_TEST;
    } else {
