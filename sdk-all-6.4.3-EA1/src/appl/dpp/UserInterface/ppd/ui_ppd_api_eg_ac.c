                          ...
                          )
{
  ...
  SOC_PPD_EG_AC_INFO_clear(&prm_ac_info);
  SOC_PPD_EG_AC_INFO_clear(&tmp_ac_info);
                          ...
                          )
{
  ...
  SOC_PPD_EG_AC_INFO_print(&prm_ac_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_EG_AC_MP_INFO_clear(&prm_info);
...
{   
  ...
  SOC_PPD_EG_AC_MP_INFO_print(&prm_info);
...
*/
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_AC_VBP_KEY_clear(&prm_vbp_key);
  SOC_PPD_EG_AC_INFO_clear(&prm_ac_info);
  prm_ac_info.edit_info.nof_tags = 2;
                                    ...
                                    )
{
  ...
  unit = soc_ppd_get_default_unit();
  SOC_PPD_EG_AC_CEP_PORT_KEY_clear(&prm_cep_key);
  SOC_PPD_EG_AC_INFO_clear(&prm_ac_info);
...
/******************************************************************** 
*  Function handler: port_cvid_map (section eg_ac)
********************************************************************/
int 
ui_ppd_api_eg_ac_port_cvid_map(
                               CURRENT_LINE *current_line 
                               ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_eg_ac"); 
  soc_sand_proc_name = "soc_ppd_eg_ac_port_cvid_map"; 

  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_EG_AC_CEP_PORT_KEY_clear(&prm_cep_key);

  /* Get parameters */ 
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_PEP_EDIT_PROFILE_ID,1))          
  {  
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_PEP_EDIT_PROFILE_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_cep_key.pep_edit_profile = (uint32)param_val->value.ulong_value;
  } 
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_CVID_ID,1))          
  {  
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_CVID_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_cep_key.cvid = (uint32)param_val->value.ulong_value;
  } 
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_OUT_PORT_ID,1))          
  {  
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_CEP_KEY_OUT_PORT_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_cep_key.vlan_domain = (uint32)param_val->value.ulong_value;
  } 
  else 
  {    
    send_string_to_screen(" *** SW error - expecting parameter cep_key after port_cvid_map***", TRUE); 
    goto exit; 
  } 

  /* Call function */  
  ret = soc_ppd_eg_ac_port_cvid_map(
    unit,
    &prm_cep_key,
    &prm_out_ac_id
    );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_eg_ac_port_cvid_map - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_eg_ac_port_cvid_map");   
    goto exit; 
  } 
...
/******************************************************************** 
*  Function handler: port_vsi_map (section eg_ac)
********************************************************************/
int 
ui_ppd_api_eg_ac_port_vsi_map(
                              CURRENT_LINE *current_line 
                              ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_eg_ac"); 
  soc_sand_proc_name = "soc_ppd_eg_ac_port_vsi_map"; 

  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_EG_AC_VBP_KEY_clear(&prm_vbp_key);

  /* Get parameters */ 
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_VSI_MAP_PORT_VSI_MAP_VBP_KEY_VSI_ID,1))          
  {  
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_AC_PORT_VSI_MAP_PORT_VSI_MAP_VBP_KEY_VSI_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_vbp_key.vsi = (uint32)param_val->value.ulong_value;
  } 
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_VSI_MAP_PORT_VSI_MAP_VBP_KEY_OUT_PORT_ID,1))          
  {  
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_EG_AC_PORT_VSI_MAP_PORT_VSI_MAP_VBP_KEY_OUT_PORT_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_vbp_key.vlan_domain = (uint32)param_val->value.ulong_value;
  } 
  else 
  {    
    send_string_to_screen(" *** SW error - expecting parameter vbp_key after port_vsi_map***", TRUE); 
    goto exit; 
  } 

  /* Call function */  
  ret = soc_ppd_eg_ac_port_vsi_map(
    unit,
    &prm_vbp_key,
    &prm_out_ac_id
    );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_eg_ac_port_vsi_map - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_eg_ac_port_vsi_map");   
    goto exit; 
  } 
                 ...
                 )
{
  ...
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_CVID_MAP_PORT_CVID_MAP_ID,1)) 
  { 
    ui_ret = ui_ppd_api_eg_ac_port_cvid_map(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_EG_AC_PORT_VSI_MAP_PORT_VSI_MAP_ID,1)) 
  { 
    ui_ret = ui_ppd_api_eg_ac_port_vsi_map(current_line); 
  } 
  else
  {
