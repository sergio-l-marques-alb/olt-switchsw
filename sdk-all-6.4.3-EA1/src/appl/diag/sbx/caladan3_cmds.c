...
_cmd_sbx_g3_reload(int unit, args_t *args)
{
    ...
    if(NULL == ucodeFile){
        ...
        ucodeFile = malloc(strlen(ucodestr) + strlen(ucodesuffix) + 1);
        if (ucodeFile == NULL) {
...
extern int bcm_stg_init(int unit);
...
cmd_sbx_caladan3_tcam_access(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "init") == 0) {
        ...
        if (!val) {
            ...
            rv =  cmd_parse_hex_string(val, value, 80);
            if (rv == CMD_OK) {
		...
		return (CMD_OK);
            ...
            rv =  cmd_parse_hex_string(data_str, data, soc_sbx_caladan3_tcam_diag_entry_width);
            if (rv != CMD_OK) {
            ...
            mask_str = ARG_GET(a);
            if (mask_str) {
                rv =  cmd_parse_hex_string(mask_str, mask, soc_sbx_caladan3_tcam_diag_entry_width);
                if (rv != CMD_OK) {
