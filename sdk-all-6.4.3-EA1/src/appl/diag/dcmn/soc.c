...
cmd_dpp_show(int unit, args_t *a)
{
   ...
   if (!sal_strcasecmp(c, "unit") || !sal_strcasecmp(c, "units")) {
      ...
      return CMD_OK;
        ...
        int temp_pvt,temp_int, temp_remainder, rc = BCM_E_UNAVAIL;
        if (SOC_IS_ARADPLUS_AND_BELOW(unit))
        {
#ifdef BCM_DPP_SUPPORT
            {
                rc = soc_dpp_temp_pvt_get(unit, &temp_pvt);
            }
#endif
            ...
            /*convert to deg*/
            temp_int = 428 - 566*temp_pvt/1000;
            temp_remainder = (428390-566*temp_pvt)%1000;
            cli_out("temp pvt avg: %d.%dC\n", temp_int, temp_remainder);
        } else {
