/* 
 * $Id: diag_egq.c,v 1.3 Broadcom SDK $
 *
...
dpp_diag_egq_port2scheduler_map_print(int unit) {
   ...
   cli_out("\n EGQ MAPPING:\n**************\n");
   cli_out(" Port #  |  Priorities  |  Base Q-Pair  |   PS #  |\n"
           "---------|--------------|---------------|---------|\n");
   ...
   BCM_PBMP_ITER(bmp,port_i) {
      ...
      rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, port_fap_index, core, &port_base_q_pair);
      if(BCM_FAILURE(rv)) {
          return CMD_FAIL;
      }
      rv = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, port_fap_index, core, &port_nof_q_pairs);
      if(BCM_FAILURE(rv)) {
          return CMD_FAIL;
      }
      port_to_ps_num = ARAD_BASE_PORT_TC2PS(port_base_q_pair);
      cli_out("  %3u    |       %1u      |      %3u      |    %2u   |\n", port_fap_index, port_nof_q_pairs, port_base_q_pair, port_to_ps_num);
   }
   cli_out("For a graphic display of port number <i> enter 'diag egq port=<i>'.\n\n");
   return CMD_OK;
}

STATIC
int dpp_diag_port_num_valid(int unit, int port_num){
   int port_i, rv, core;
   soc_pbmp_t  bmp;
   uint32      tm_port;
   ...
   BCM_PBMP_ITER(bmp,port_i) {
       rv = soc_port_sw_db_local_to_tm_port_get(unit,port_i, &tm_port, &core);
       if(BCM_FAILURE(rv)) {
           return CMD_FAIL;
       }
...
dpp_diag_egq_graphic(int unit, int port_num) {
   ...
   uint32 pps_port_index, port_nof_q_pairs, pps_base_q_pair;
   int ps_num, rv;
   soc_port_t local_port_gport;

   sal_memset(TCG_to_TCs, 0, 8 * sizeof(uint32));

   if (dpp_diag_port_num_valid(unit,port_num)) {
      pps_port_index = port_num;

      rv = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, pps_port_index, 0, &port_nof_q_pairs);
      if(BCM_FAILURE(rv)) {
          return CMD_FAIL;
      }
      rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, pps_port_index, 0, &pps_base_q_pair);
      if(BCM_FAILURE(rv)) {
          return CMD_FAIL;
      }
      ...
      if (soc_port_sw_db_tm_to_local_port_get(unit, pps_port_index, 0, &local_port_gport)<0) return CMD_FAIL; /*map tm port(pps_port_index) to local port*/
      ...
      for (i = 0; i < port_nof_q_pairs; i++) {
        cli_out("  Port Index %03d   ", pps_port_index);
      }
      cli_out(" \n  ");
