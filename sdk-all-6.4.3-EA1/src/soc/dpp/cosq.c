  ...
  )
{
  ...
  if (flow_and_up_info->credit_sources_nof == 0)
  {
      ...
      if (is_flow) {
         ...
         ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_destination_id_packets_base_queue_id_get,(unit, dest_id, &valid, &(flow_and_up_info->base_queue)));
         ...
         ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_mgmt_system_fap_id_get,(unit, &local_fap_id));
         ...
         ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ipq_queue_to_flow_mapping_get,(unit, queue_quartet_ndx, &(flow_and_up_info->qrtt_map_info)));
  ...
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_flow_and_up_info_get,(unit, flow_id, reterive_status, flow_and_up_info));
  SOCDNX_SAND_IF_ERR_RETURN(ret);
  ...
  )
{
  ...
  /* get port_id and tc */
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_sch_se2port_tc_id, (unit, se_id, &port_id, &tc));
  ...
  /* get scheduler info */
  ret = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_sch_aggregate_get, (unit, se_id, &(hr2ps_info->se_info), &(hr2ps_info->flow_info))); 
  if (SOC_SAND_FAILURE(ret)) {
    LOG_ERROR(BSL_LS_BCM_COSQ,
              (BSL_META_U(unit,
  ...
  /* get rate info*/
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_port_priority_rate_get,(unit, port_id , tc, &(hr2ps_info->kbits_sec_max)));
  if (SOC_SAND_FAILURE(ret)) {
    LOG_ERROR(BSL_LS_BCM_COSQ,
              (BSL_META_U(unit,
  ...
  /* get burst info*/
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_port_priority_max_burst_get,(unit, port_id , tc, &(hr2ps_info->max_burst)));
  if (SOC_SAND_FAILURE(ret)) {
    LOG_ERROR(BSL_LS_BCM_COSQ,
              (BSL_META_U(unit,
  ...
  /* get sched info*/
  ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_sch_port_sched_get,(unit, port_id, &sch_port_info));
  if (SOC_SAND_FAILURE(ret)) {
    LOG_ERROR(BSL_LS_BCM_COSQ,
              (BSL_META_U(unit,
  ...
  )
{
  ...
  SOCDNX_INIT_FUNC_DEFS; 
    ...
    ret = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ips_non_empty_queues_info_get,(unit, queue_to_read_from,
  ...
  )
{
    ...
    ret = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_itm_queue_info_get,(unit, queue_ndx, info)));
    SOCDNX_SAND_IF_ERR_RETURN(ret);
  ...
  )
{
    ...
    ret = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_itm_category_rngs_get,(unit, &info)));
    SOCDNX_SAND_IF_ERR_RETURN(ret);
  ...
  )
{
    ...
    ret = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_ipq_queue_to_flow_mapping_get,
            (unit, SOC_TMC_IPQ_Q_TO_QRTT_ID(queue_ndx), queue_map_info));
	SOCDNX_SAND_IF_ERR_RETURN(ret);
  ...
  )
{
    ...
    if (SOC_SAND_DEVICE_TYPE_GET(unit) == SOC_SAND_DEV_ARAD) {
        ret = arad_itm_queue_dyn_info_get(unit, queue_ndx, info);
        SOCDNX_SAND_IF_ERR_RETURN(ret);
    }
    else 
#endif
  ...
  )
{
    ...
    ret = (MBCM_DPP_DRIVER_CALL_WITHOUT_DEV_ID(unit, mbcm_dpp_itm_vsq_index_global2group,(vsq_id, soc_vsq_group_type, vsq_index)));
    SOCDNX_SAND_IF_ERR_RETURN(ret);
...
soc_dpp_voq_max_size_drop(int unit, uint32 *is_max_size) 
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        for (core=0; core<SOC_DPP_DEFS_GET(unit, nof_cores) ; core++) {
            ...
            (*is_max_size) = soc_reg64_field32_get(unit, IQM_REJECT_STATUS_BMPr, val, VOQ_MX_QSZ_BUFF_STATUSf);
            if (*is_max_size != 0){
                break;
            }
        }
    } else {
        SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_BSL_SOCDNX_MSG("Function is unavail for the device")));
    }
