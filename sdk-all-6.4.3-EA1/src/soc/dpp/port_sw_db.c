...
soc_port_sw_db_tm_to_local_port_get(int unit, uint32 tm_port, int core, soc_port_t *port) {
    ...
    *port = core_info[unit][core].tm_to_local_port_map[tm_port];
    ...
    if(*port == SOC_MAX_NUM_PORTS) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("tm_port %d invalid mapping"), tm_port));
    }
...
soc_port_sw_db_pp_to_local_port_get(int unit, uint32 pp_port, int core, soc_port_t *port) 
{
    ...
    *port = core_info[unit][core].pp_to_local_port_map[pp_port];
    ...
    if(*port == SOC_MAX_NUM_PORTS) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("pp_port %d invalid mapping"), pp_port));
    }
...
typedef struct soc_phy_to_logical_ports_str {
    char ports[SOC_MAX_NUM_PORTS][13];
