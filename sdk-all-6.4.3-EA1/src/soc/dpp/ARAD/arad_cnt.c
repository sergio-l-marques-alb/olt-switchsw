...
/*configurable mode handling*/
  ...
  if((info->src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP) || (info->src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP_MSB))
  {
    ...
    for(pp_port_ndx = 0;  pp_port_ndx < ARAD_PORT_NOF_PP_PORTS;  pp_port_ndx++)
    {
		SOC_SAND_SOC_IF_ERROR_RETURN(res, 1000, exit, READ_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm(unit, MEM_BLOCK_ANY, pp_port_ndx, data));
		soc_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm_field32_set(unit, data, COUNTER_PROFILEf, fld_val);
		SOC_SAND_SOC_IF_ERROR_RETURN(res, 1010, exit, WRITE_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm(unit, MEM_BLOCK_ANY, pp_port_ndx, data));
    }
  }
  ...
  )
{
  ...
  if (direction == SOC_TMC_PORT_DIRECTION_INCOMING) {
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1000, exit, READ_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm(unit, MEM_BLOCK_ANY, port_ndx, data));

      soc_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm_field32_set(unit, data, COUNTER_COMPENSATIONf, fld_val);

      SOC_SAND_SOC_IF_ERROR_RETURN(res, 1010, exit, WRITE_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm(unit, MEM_BLOCK_ANY, port_ndx, data));
  ...
  )
{
 ...
 if (direction == SOC_TMC_PORT_DIRECTION_INCOMING) {
     ...
     fld_val = soc_EGQ_PACKET_PROCESSING_PER_PORT_CONFIGURATION_TABLE_PP_PPCTm_field32_get(unit, data, COUNTER_COMPENSATIONf);
