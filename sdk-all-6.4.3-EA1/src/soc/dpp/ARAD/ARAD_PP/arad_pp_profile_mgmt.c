...
/* TBD - OAM Default Profile
                if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_oam, 0)) {
                    soc_sand_rv = soc_sand_occ_bm_alloc_next(inlif_bit_map, &bit_index, &found);
                    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);

                    soc_sand_rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_INLIF_PROFILE_MASK, &inlif_type, bit_index);
                    SOCDNX_IF_ERR_EXIT(soc_sand_rv);
                }
*/
            }
