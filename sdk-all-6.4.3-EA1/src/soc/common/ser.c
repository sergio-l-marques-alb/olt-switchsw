...
#define _ALPM_LOCK(unit) \
    MEM_LOCK(unit, L3_DEFIP_ALPM_IPV4m); \
    MEM_LOCK(unit, L3_DEFIP_ALPM_IPV4_1m); \
    MEM_LOCK(unit, L3_DEFIP_ALPM_IPV6_64m); \
    MEM_LOCK(unit, L3_DEFIP_ALPM_IPV6_64_1m); \
    MEM_LOCK(unit, L3_DEFIP_ALPM_IPV6_128m); \
    MEM_LOCK(unit, L3_DEFIP_ALPM_RAWm)

#define _ALPM_UNLOCK(unit, lock) \
    if ((lock)) { \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_IPV4m); \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_IPV4_1m); \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_IPV6_64m); \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_IPV6_64_1m); \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_IPV6_128m); \
        MEM_UNLOCK(unit, L3_DEFIP_ALPM_RAWm); \
    }
...
#define _SOC_FPEM_QUERY_MEM_MODE 1
...
soc_generic_ser_mem_update(int unit, soc_mem_t mem, int mode)
{
    ...
    if (soc_property_get(unit, spn_MEM_SCAN_ENABLE, SAL_BOOT_SIMULATION ? 0 : 1)) {
        ...
        while (ser_info[index].mem != INVALIDm) {
            /* Note: This will be the global view index */
            if (ser_info[index].mem == mem) {
                ...
                if (mode == _SOC_SER_MEM_MODE_PIPE_UNIQUE) {
                     ...
                     * transaction errors 
                     */
                    range_enable &= ~(1 << index);
                    range_enable |= (1 << (index+1));
                } else {
                    range_enable &= ~(1 << (index+1));
                    range_enable |= (1 << index);
                }
    ...
    if (SOC_IS_TD2_TT2(unit) || SOC_IS_TD_TT(unit)) {
        ...
        case L3_DEFIP_DATA_ONLYm:
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "Unit %d: RESTORE[from pipe %0d]: %s[%d] "
                                      "blk: %s index: %d\n"),
                           unit, restore_pipe, SOC_MEM_NAME(unit, mem),
                           mem, SOC_BLOCK_NAME(unit, copyno), index));
            } else {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "Unit %d: RESTORE[from %c pipe]: %s[%d] "
                                      "blk: %s index: %d\n"),
                           unit, pipe ? 'X':'Y', SOC_MEM_NAME(unit, mem),
                           mem, SOC_BLOCK_NAME(unit, copyno), index));
            }
            stat->ser_err_restor++;
...
#define _SOC_SER_MEM_OVERLAY_SET 6
    ...
    case L3_ENTRY_ONLY_ECCm:
        ...
        break;
    case FPEM_ECCm:
        fpem_mem_mode = _SOC_FPEM_QUERY_MEM_MODE; 
        num_mems = 2;
            ...
            if (cache != NULL && CACHE_VMAP_TST(vmap, vindex+i)) {
                ...
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                           ...
                           unit, SOC_MEM_NAME(unit, mem_for_write),
...
#define _SOC_SER_SRAM_CHK_RETURN(str) \
    ...
    void *null_entry;
    uint8 *vmap, at = 0, _l3_lock=0, _alpm_lock=0, _l2_lock=0;
                ...
                if (sram_info.view[i] == FPEM_LPm) {
                    ...
                    LOG_ERROR(BSL_LS_SOC_COMMON,
                              (BSL_META_U(unit,
                               ...
                               unit, SOC_MEM_NAME(unit, FPEM_LPm), FPEM_LPm,
                               start_index));
                } else {
            ...
            if (cache != NULL && CACHE_VMAP_TST(vmap, start_index)) {
                ...
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                           ...
                           unit, SOC_MEM_NAME(unit, mem_to_restore),
    ...
    if (si->flags & SOC_SER_SRC_MEM) { /* Handle mems */
        ...
        if (SOC_IS_TD2_TT2(unit)) {
            if (mem == L3_DEFIP_ALPM_IPV4m || mem == L3_DEFIP_ALPM_IPV4_1m ||
                ...
                /* xsm: ??? different chips have different remap functions */
#endif /* BCM_TRIDENT2_SUPPORT */
            }
        }
