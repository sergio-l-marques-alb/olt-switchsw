        ...
        if (soc_feature(dev, soc_feature_extended_cmic_error) ||
            ...
            if (soc_feature(dev, soc_feature_cmicm_extended_interrupts)) {
                if ((CMCx_IRQ5_STAT(dev, cmc) & SOC_CMCx_IRQ5_MASK(dev,cmc)) ||
                    ...
                    if (soc_feature(dev, soc_feature_cmicm_extended_interrupts)) {
                        if ((soc_pci_read(dev, CMIC_CMCx_IRQ_STAT3_OFFSET(cmc)) & SOC_CMCx_IRQ3_MASK(dev,cmc)) ||
                            (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT4_OFFSET(cmc)) & SOC_CMCx_IRQ4_MASK(dev,cmc)) ||
                            (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT5_OFFSET(cmc)) & SOC_CMCx_IRQ5_MASK(dev,cmc))) {
                            _ictrl[dev].handler(_ictrl[dev].data);
                        }
                    }
