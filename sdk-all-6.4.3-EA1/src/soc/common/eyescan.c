    ...
    for(j = 0 ; j < nof_ports ; j++) {
        ...
        if(soc_feature(unit, soc_feature_portmod)) {
           ...
           intf = PHY_DIAG_INTF_DFLT;
            ...
            rv = portmod_port_phy_lane_access_get(unit, ports[j], &params, PHYMOD_CONFIG_MAX_CORES_PER_PORT, access, &nof_phys);
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    /*enable eyescan*/
    rv = phymod_diag_eyescan_run_lowber(phy_acc, line_rates, phy_acc_length, &eyescan_options);
    ...
    soc_port_phy_eyescan_results_t* results)    /*array of result per port*/
{
    ...
    /* Run eyescan */
    rv = phymod_diag_eyescan_run_uc(phy_acc, phy_acc_length);
