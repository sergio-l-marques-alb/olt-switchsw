...
soc_fe3200_fabric_cell_get(int unit, soc_dcmn_fabric_cell_entry_t entry)
{
    ...
    for(blk_idx=0 ; blk_idx<SOC_DFE_DEFS_GET(unit, nof_instances_dcl) ; blk_idx++) 
    {
        ...
        if(nof_cell) {
            SOCDNX_IF_ERR_EXIT(READ_DCL_CPU_Hm(unit, blk_idx, 0, entry));
            found_cell = 1;
            break;
        }
    }
...
soc_error_t
soc_fe3200_fabric_cell_parse_table_get(int unit, soc_dcmn_fabric_cell_type_t cell_type, uint32 max_nof_lines, soc_dcmn_fabric_cell_parse_table_t *parse_table, uint32 *nof_lines)
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    {
        soc_dcmn_fabric_cell_parse_table_t temp_table[] = SOC_FE3200_FABRIC_CELL_CAPTURED_SRC_FORMAT;
        ...
        sal_memcpy(parse_table, temp_table, sizeof(soc_dcmn_fabric_cell_parse_table_t) * SOC_FE3200_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES);
