...
soc_sbx_caladan3_sws_tdm_lookup(int unit, int *tdmid, sws_config_t **sws_cfg)
{
    ...
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(unit,
                          "Unit %d, Detected %d Line Ports and %d Fabric Ports\n"), 
               unit, tdm_data.num_line_ports, tdm_data.num_fabric_ports));
    if (tdm_data.num_line_ports > 0) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Unit %d, Detected "), unit));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " 100G(%d)"), tdm_data.num_100g));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " ILKN100(%d)"), tdm_data.num_ilkn_100));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " ILKN50(%d)"), tdm_data.num_ilkn_50));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " HG127(%d)"), tdm_data.num_hg127));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " HG42(%d)"), tdm_data.num_hg42));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " HG25(%d)"), tdm_data.num_hg25));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " HG10(%d)"), tdm_data.num_hg10));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " 40G(%d)"), tdm_data.num_40g));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " 10G(%d)"), tdm_data.num_10g));
 LOG_ERROR(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       " 1G(%d)"), tdm_data.num_1g));
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "\n")));
    }
