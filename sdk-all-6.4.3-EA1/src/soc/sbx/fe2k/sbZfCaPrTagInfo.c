/*
 * $Id: sbZfCaPrTagInfo.c,v 1.2 Broadcom SDK $
...
sbZfCaPrTagInfo_Unpack(sbZfCaPrTagInfo_t *pToStruct,
  ...
  (pToStruct)->m_bActive =  (uint32)  ((pFromData)[5] >> 7) & 0x01;

  /* Unpack Member: m_bTestPkt */
  (pToStruct)->m_bTestPkt =  (uint32)  ((pFromData)[5] >> 6) & 0x01;

  /* Unpack Member: m_nPort */
  (pToStruct)->m_nPort =  (uint32)  ((pFromData)[5] ) & 0x3f;

  /* Unpack Member: m_bEndOfPkt */
  (pToStruct)->m_bEndOfPkt =  (uint32)  ((pFromData)[4] >> 7) & 0x01;

  /* Unpack Member: m_nEnqueueCount */
  (pToStruct)->m_nEnqueueCount =  (uint32)  ((pFromData)[4] >> 5) & 0x03;

  /* Unpack Member: m_nLineCount */
  (pToStruct)->m_nLineCount =  (uint32)  ((pFromData)[4] >> 2) & 0x07;

  /* Unpack Member: m_nNumPages */
  (pToStruct)->m_nNumPages =  (uint32)  ((pFromData)[3] >> 3) & 0x1f;
  (pToStruct)->m_nNumPages |=  (uint32)  ((pFromData)[4] & 0x03) << 5;

  /* Unpack Member: m_nBuffer */
  (pToStruct)->m_nBuffer =  (uint32)  ((pFromData)[1] >> 7) & 0x01;
  (pToStruct)->m_nBuffer |=  (uint32)  (pFromData)[2] << 1;
  (pToStruct)->m_nBuffer |=  (uint32)  ((pFromData)[3] & 0x07) << 9;

  /* Unpack Member: m_nLength */
  (pToStruct)->m_nLength =  (uint32)  ((pFromData)[0] >> 1) & 0x7f;
  (pToStruct)->m_nLength |=  (uint32)  ((pFromData)[1] & 0x7f) << 7;
