 ...
 *
 *-----------------------------------------------------------------------------*/
#ifndef _SBX_CALADN3_TRIE_H_
#define _SBX_CALADN3_TRIE_H_

#include <soc/sbx/caladan3/hpcm.h>

extern uint8 mem_pool_enable;
extern soc_heap_sl_mem_chunk_t *g_trie_node_hpcm;
...
typedef enum _trie_type_t{
    TCAM_PIVOT_TRIE,
    SRAM_PIVOT_TRIE,
    DRAM_PREFIX_TRIE
}trie_type_t;

typedef struct _trie_traverse_data_s{
    trie_type_t trie_type;
    uint16 expected_node_idx;
    uint16 max_node_num;
    trie_node_t *next_node;
}trie_traverse_data_s;
 ...
 */
extern int trie_node_mem_alloc(trie_node_t **node, void **hpcm_elem_handle);
 ...
 */
extern int trie_node_mem_free(trie_node_t *node);
 ...
 */
extern int trie_split_len_get(trie_t *trie,
 ...
 */
extern int trie_traverse_find(trie_node_t *trie, trie_callback_f cb, 
                  void *user_data);
...
extern int trie_internal_node_free(trie_node_t *trie);
