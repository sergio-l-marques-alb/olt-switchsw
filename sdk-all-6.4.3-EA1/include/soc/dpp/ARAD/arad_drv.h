...
typedef struct soc_dpp_config_arad_s {
    ...
    int8 credit_watchdog_mode; /* supported values are: CREDIT_WATCHDOG_AGGRESSIVE_STATUS_MSG_MODE, CREDIT_WATCHDOG_NORMAL,
                                  CREDIT_WATCHDOG_COMMON_STATUS_MESSAGE_MODE, CREDIT_WATCHDOG_COMMON_STATUS_MESSAGE_MODE+1
                                  The last value means generate a message after two scans */
    uint32 exact_credit_watchdog_scan_time_nano; /* exact credit watchdog scan time in nano seconds */
    ...
    soc_pbmp_t reserved_ports; /* for dynamic nif */
    soc_pbmp_t reserved_isq_base_q_pair; /* for dynamic nif */
    soc_pbmp_t reserved_fmq_base_q_pair; /* for dynamic nif */
...
int soc_arad_allocate_tm_port_in_range_and_recycle_channel(int unit, int max_port, int *tm_port, uint32 *channel);
int soc_arad_free_tm_port_and_recycle_channel(int unit, int port);
int soc_arad_isq_hr_get(int unit, int *hr_isq);
int soc_arad_fmq_base_hr_get(int unit, int** base_hr_fmq);
