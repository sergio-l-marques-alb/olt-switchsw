...
#define  SOC_PPC_FP_NOF_INGRESS_FLP_ACTIONS (4)
...
/* Number of PFGs */
#define SOC_PPC_FP_NOF_PFGS_ARAD            64

/* Number of Program selection lines in longs */
#define SOC_PPC_FP_NOF_PFGS_IN_LONGS_ARAD             2
...
#define SOC_PPC_FP_DATABASE_INFO_FLAGS_STAGE_VT     (1 << 5)
#define SOC_PPC_FP_DATABASE_INFO_FLAGS_HEADER_SELECTION  (1 << 6)
#define SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB  (1 << 7)
#define SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB (1 << 8)
...
typedef enum
{
   ...
   *  constructed using the macro SOC_PPD_FP_QUAL_VAL_ENCODE
   *  _MAC_ADDRESS (48b).
   */
  SOC_PPC_FP_QUAL_HDR_FWD_SA = 1,
  /*
   *  Default Destination MAC address. The Qualifier value can
   *  be constructed using the macro SOC_PPD_FP_QUAL_VAL_ENCODE
   *  _MAC_ADDRESS (48b).
   */
   ...
   *  SOC_PPD_FP_CONTROL_TYPE_ETHERTYPE (16b) and
   *  SOC_PPD_FP_CONTROL_TYPE_EGR_L2_ETHERTYPES for egress (16b).
   ...
   *  SOC_PPD_FP_CONTROL_TYPE_INNER_ETH_NOF_VLAN_TAGS
   */
   ...
   *  SOC_PPD_FP_CONTROL_TYPE_L2_L3_KEY_IN_LIF_ENABLE
   */
   ...
   *  soc_ppd_fp_control_set API using the type
   *  SOC_PPD_FP_CONTROL_TYPE_NEXT_PROTOCOL_IP (8b) for ingress
   *  parsing and SOC_PPD_FP_CONTROL_TYPE_EGR_IPV4_NEXT_PROTOCOL
   ...
   *  the macro SOC_PPD_FP_QUAL_VAL_ENCODE _IPV4_SUBNET (32b).
   */
  SOC_PPC_FP_QUAL_HDR_FWD_IPV4_SIP = 19,
  /*
   *  Destination-IP. The Qualifier value can be constructed
   *  using the macro SOC_PPD_FP_QUAL_VAL_ENCODE _IPV4_SUBNET
   *  (32b).
   */
   ...
   *  L4-Src-Port (16b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_SRC_PORT = 41,
  /*
   *  L4-Dest-Port (16b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_DEST_PORT = 42,
  /*
   *  Type of Service (8b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_TOS = 43,
  /*
   *  TCP-Control field (Bits URG to FIN of the TCP header)
   *  (6b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_TCP_CTL = 44,
  /*
   ...
   *  with the type SOC_PPD_FP_CONTROL_TYPE_L4OPS_RANGE (8b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_L4OPS_HI = 45,
  /*
   *  Layer-4 Ops Low value (L4Ops[15:0]). Its value is defined according to the L4Ops
   *  ranges which can be set via the soc_ppd_fp_control_set API
   *  with the type SOC_PPD_FP_CONTROL_TYPE_L4OPS_RANGE (16b).
   *  Note: the L4Ops low is not part of the predefined qualifiers if using the macro.
   */
  SOC_PPC_FP_QUAL_HDR_IPV4_L4OPS_LOW = 46,
  /*
   *  Source-IP MSBs. The Qualifier value can be constructed
   *  using the macro SOC_PPD_FP_QUAL_VAL_ENCODE _IPV6_SUBNET
   *  (64b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_SIP_HIGH = 47,
  /*
   *  Source-IP LSBs. The Qualifier value can be constructed
   *  using the macro SOC_PPD_FP_QUAL_VAL_ENCODE _IPV6_SUBNET
   *  (64b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_SIP_LOW = 48,
  /*
   *  Destination-IP MSBs. The Qualifier value can be
   *  constructed using the macro SOC_PPD_FP_QUAL_VAL_ENCODE
   *  _IPV6_SUBNET (64b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_DIP_HIGH = 49,
  /*
   *  Destination-IP LSBs. The Qualifier value can be
   *  constructed using the macro SOC_PPD_FP_QUAL_VAL_ENCODE
   *  _IPV6_SUBNET (64b).
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_DIP_LOW = 50,
  /*
   *  Next-Protocol (8b). Similar of use as IPv4
   *  Next-Protocol (see above).
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_NEXT_PRTCL = 51,
  /*
   *  TCP-Control (6b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_TCP_CTL = 52,
  /*
   *  L4-Ops. Its value is defined according to the L4Ops
   *  ranges which can be set via the soc_ppd_fp_control_set API
   *  with the type SOC_PPD_FP_CONTROL_TYPE_L4OPS_RANGE (24b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_L4OPS = 53,
  /*
   *  IPv6 Traffic Class (8b)
   */
  SOC_PPC_FP_QUAL_HDR_IPV6_TC = 54,
  /*
   *  Forwarding MPLS Label (32b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_LABEL_FWD = 55,
  /*
   *  Forwarding MPLS Experimental Use (3b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_EXP_FWD = 56,
  /*
   *  Forwarding MPLS Time-To-Live (8b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_TTL_FWD = 57,
  /*
   *  Forwarding MPLS Bottom-of-Stack (32b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_BOS_FWD = 58,
  /*
   *  MPLS Label 1 (20b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_LABEL1 = 59,
  /*
   *  MPLS Experimental Use 1 (3b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_EXP1 = 60,
  /*
   *  MPLS Time-To-Live 1 (8b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_TTL1 = 61,
  /*
   *  MPLS Bottom-of-Stack 1 (32b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_BOS1 = 62,
  /*
   *  MPLS Label 2 (20b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_LABEL2 = 63,
  /*
   *  MPLS Experimental Use 2 (3b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_EXP2 = 64,
  /*
   *  MPLS Time-To-Live 2 (8b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_TTL2 = 65,
  /*
   *  MPLS Bottom-of-Stack 2 (32b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_BOS2 = 66,
  /*
   *  MPLS Label 3 (20b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_LABEL3 = 67,
  /*
   *  MPLS Experimental Use 3 (3b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_EXP3 = 68,
  /*
   *  MPLS Time-To-Live 3 (8b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_TTL3 = 69,
  /*
   *  MPLS Bottom-of-Stack 3 (32b)
   */
  SOC_PPC_FP_QUAL_HDR_MPLS_BOS3 = 70,
  /*
   *  Local incoming TM port. Range: 0 - 79.
   */
  SOC_PPC_FP_QUAL_IRPP_SRC_TM_PORT = 71,
  /*
   *  Source System-Port ID. Range: 0 - 4K-1.
   */
  SOC_PPC_FP_QUAL_IRPP_SRC_SYST_PORT = 72,
  /*
   *  Local incoming PP port. Range: 0 - 63.
   */
  SOC_PPC_FP_QUAL_IRPP_SRC_PP_PORT = 73,
  /*
   *  Processing type set by the parser for this Packet. The
   *  value is set according to the SOC_PPD_FP_PROCESSING_TYPE
   *  enum
   */
  SOC_PPC_FP_QUAL_IRPP_PROCESSING_TYPE = 74,
  /*
   ...
   */
  SOC_PPC_FP_QUAL_IRPP_PKT_HDR_TYPE = 75,
  /*
   ...
   *  SOC_PPD_FP_QUAL_VAL_ENCODE_ETH_TAG_FORMAT.
   */
  SOC_PPC_FP_QUAL_IRPP_ETH_TAG_FORMAT = 76,
  /*
   *  The Forwarding Decision taken for the packet. The
   *  Qualifier value can be constructed using the macro
   *  SOC_PPD_FP_QUAL_VAL_ENCODE_FWD_DECISION (16b).
   ...
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_DEC_DEST = 77,
  /*
   ...
   *  qualifier per action type).
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_DEC_TC = 78,
  /*
   ...
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_DEC_DP = 79,
  /*
   *  CPU Trap code of the Forwarding Decision (8b)
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_DEC_CPU_TRAP_CODE = 80,
  /*
   *  CPU Trap qualifier of the Forwarding Decision (14b)
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_DEC_CPU_TRAP_QUAL = 81,
  /*
   *  User-Priority of the Packet (3b)
   */
  SOC_PPC_FP_QUAL_IRPP_UP = 82,
  /*
   *  Snoop code (8b)
   */
  SOC_PPC_FP_QUAL_IRPP_SNOOP_CODE = 83,
  /*
   *  Learn Decision Destination. The Qualifier value can be
   *  constructed using the macro
   *  SOC_PPD_FP_QUAL_VAL_ENCODE_FWD_DECISION (16b).
   */
  SOC_PPC_FP_QUAL_IRPP_LEARN_DECISION_DEST = 84,
  /*
   *  Learn Decision Addition Information. The Qualifier value
   *  can be constructed using the macro
   *  SOC_PPD_FP_QUAL_VAL_ENCODE_FWD_DECISION (24b).
   */
  SOC_PPC_FP_QUAL_IRPP_LEARN_ADD_INFO = 85,
  /*
   ...
   *  SOC_PPD_FP_CONTROL_TYPE_L2_L3_KEY_IN_LIF_ENABLE
   */
  SOC_PPC_FP_QUAL_IRPP_IN_LIF = 86,
  /*
   *  LL Mirror Command (4b)
   */
  SOC_PPC_FP_QUAL_IRPP_LL_MIRROR_CMD = 87,
  /*
   *  System VSI (16b)
   */
  SOC_PPC_FP_QUAL_IRPP_SYSTEM_VSI = 88,
  /*
   *  Orientation=Is-Hub (1b)
   */
  SOC_PPC_FP_QUAL_IRPP_ORIENTATION_IS_HUB = 89,
  /*
   *  VLAN-ID result of the VLAN Ingress Editing (12b)
   */
  SOC_PPC_FP_QUAL_IRPP_VLAN_ID = 90,
  /*
   *  PCP result of the VLAN Ingress Editing  (3b)
   */
  SOC_PPC_FP_QUAL_IRPP_VLAN_PCP = 91,
  /*
   *  DEI result of the VLAN Ingress Editing  (1b)
   */
  SOC_PPC_FP_QUAL_IRPP_VLAN_DEI = 92,
  /*
   *  State in the Spanning Tree Protocol (2b)
   */
  SOC_PPC_FP_QUAL_IRPP_STP_STATE = 93,
  /*
   *  Type of forwarding processed on the packet. The field
   *  value is set according to the SOC_PPD_FP_FWD_TYPE enum.
   */
  SOC_PPC_FP_QUAL_IRPP_FWD_TYPE = 94,
  /*
   *  Indicate on which header the forwarding occurs (3b)
   */
  SOC_PPC_FP_QUAL_IRPP_SUB_HEADER_NDX = 95,
  /*
   ...
   *  an Action type SOC_PPD_FP_ACTION_TYPE_CHANGE_KEY (12b).
   ...
   */
   SOC_PPC_FP_QUAL_IRPP_KEY_CHANGED = 96,
  /*
   *  In-RIF (12b)
   */
  SOC_PPC_FP_QUAL_IRPP_IN_RIF = 97,
  /*
   *  VRF (8b)
   */
  SOC_PPC_FP_QUAL_IRPP_VRF = 98,
  /*
   *  Packet-Is-Compatible-Multicast (1b)
   */
  SOC_PPC_FP_QUAL_IRPP_PCKT_IS_COMP_MC = 99,
  /*
   *  My-Backbone-MAC (1b). Arad only
   */
  SOC_PPC_FP_QUAL_IRPP_MY_BMAC = 100,
  /*
   *  In-TTL (8b)
   */
  SOC_PPC_FP_QUAL_IRPP_IN_TTL = 101,
  /*
   *  In-DSCP-EXP (8b)
   */
  SOC_PPC_FP_QUAL_IRPP_IN_DSCP_EXP = 102,
  /*
   *  Packet-Size-Range (2b). The soc_ppd_fp_control_set API can
   *  be called to set the packet size ranges.
   */
  SOC_PPC_FP_QUAL_IRPP_PACKET_SIZE_RANGE = 103,
  /*
   *  Packet termination type (4b) indicating the number
   *  of headers that have been terminated.
   *  Its encoding is based on the enum SOC_PPD_PKT_TERM_TYPE.
   */
  SOC_PPC_FP_QUAL_IRPP_TERM_TYPE = 104,
  /*
   *  Out-PP-Port data part of the predefined Egress ACL Keys.
   *  The value of each PP-Port can be defined via the
   *  soc_ppd_fp_control_set API with type
   *  SOC_PPD_FP_CONTROL_TYPE_EGR_PP_PORT_DATA (6b).
   */
  SOC_PPC_FP_QUAL_ERPP_PP_PORT_DATA = 105,
  /*
   *  IPv4 Next-Protocol matches. The IPv4 Next-Protocol is
   *  compared to values which can be set via the
   *  soc_ppd_fp_control_set API with type
   *  SOC_PPD_FP_CONTROL_TYPE_EGR_IPV4_NEXT_PROTOCOL (4b).
   */
  SOC_PPC_FP_QUAL_ERPP_IPV4_NEXT_PROTOCOL = 106,
  /*
   ...
   *  type SOC_PPD_FP_CONTROL_TYPE_HDR_USER_DEF.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_0 = 109,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_1 = 110,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_2 = 111,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_3 = 112,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_4 = 113,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_5 = 114,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_6 = 115,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_7 = 116,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_8 = 117,
  /*
   *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
   */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_9 = 118,
 /*
  *  See SOC_PPD_FP_QUAL_HDR_USER_DEF_0.
  */
  SOC_PPC_FP_QUAL_HDR_USER_DEF_LAST = SOC_PPC_FP_QUAL_HDR_USER_DEF_0 + 63,  
    /*
     *  TTL (8b)
     */
...
typedef enum
{
   ...
   *  17 bits, use the macro SOC_PPD_FP_QUAL_VAL_ENCODE
   ...
   *  SOC_PPD_FP_ACTION_TYPE_VSQ_PTR as action type for the
   *  same supported PFGs.
   */
   ...
   *  the Qualifier SOC_PPD_FP_QUAL_IRPP_KEY_CHANGED).
   ...
   *  soc_ppd_api_control_set with SOC_PPD_FP_CONTROL_TYPE_EGRESS_DP type.
   */
   ...
   */
    ...
    /* FLP Actions */
    SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_0,
...
typedef enum
{
  ...
  SOC_PPC_NOF_FP_DATABASE_STAGES_PETRA_B,
    /*
     *  Ingress FLP block (used also for External lookup).
     */
  SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP = SOC_PPC_NOF_FP_DATABASE_STAGES_PETRA_B,
    /*
...
typedef enum
{
   ...
   *  the Base-Header type (via the SOC_PPD_FP_BASE_HEADER_TYPE
...
typedef struct
{
   ...
   *  action value, according to the qualifier value.
   */
   ...
   *  IPv4 or IPv6 or Egress), use the SOC_PPD_FP_QUAL_TYPE_PRESET
...
typedef struct
{
  ...
  /*
   *  Packet header format as parsed
   */
  SOC_PPC_FP_PKT_HDR_TYPE hdr_format;
  /*
...
typedef struct
{
   ...
   */
  SOC_PPC_FP_PACKET_DIAG_ACTION_ELK elk_action[SOC_PPC_FP_NOF_INGRESS_FLP_ACTIONS];
...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  Parameter for this PP-Port. Only one Header type must be
   *  set in the 'header_type' bitmap.
   */
  SOC_TMC_PORT_PP_PORT_INFO pp_port;
  /*
   *  Packet-Format-Group (relevant if Header type is
   *  Ethernet) given by the FP.
   */
  SOC_TMC_PMF_PFG_INFO pfg_info;

} SOC_TMC_PMF_PGM_MGMT_PARAMS;

typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   * source local port (between 0-255)
   */
  uint8 local_port_src; 
  /*
...
void
  SOC_TMC_PMF_PFG_INFO_clear(
    SOC_SAND_OUT SOC_TMC_PMF_PFG_INFO *info
  );

void
  SOC_TMC_PMF_PGM_MGMT_PARAMS_clear(
    SOC_SAND_OUT SOC_TMC_PMF_PGM_MGMT_PARAMS *info
  );
...
void
  SOC_TMC_PMF_PGM_MGMT_PARAMS_print(
    SOC_SAND_IN  SOC_TMC_PMF_PGM_MGMT_PARAMS *info
  );
...
void
  SOC_TMC_PMF_PFG_INFO_print(
    SOC_SAND_IN  SOC_TMC_PMF_PFG_INFO *info
  );
