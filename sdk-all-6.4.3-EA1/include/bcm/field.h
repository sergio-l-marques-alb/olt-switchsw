...
typedef enum bcm_field_qualify_e {
    ...
    bcmFieldQualifyVxlanHeaderBits56_63 = 410, /* Qualify Reserved_2 field in Vxlan
                                           Header */
    bcmFieldQualifyCount = 411          /* Always Last. Not a usable value. */
} bcm_field_qualify_t;
...
typedef enum bcm_field_action_e {
    ...
    bcmFieldActionCount = 358           /* Always Last. Not a usable value. */
} bcm_field_action_t;
...
#define BCM_FIELD_CSET_INIT(c_set)  \
    sal_memset(&(c_set), 0, sizeof(bcm_field_compress_set_t)) 

#define BCM_FIELD_CSET_ADD(c_set, c_id)  SHR_BITSET(((c_set).w), (c_id)) 

#define BCM_FIELD_CSET_REMOVE(c_set, c_id)  SHR_BITCLR(((c_set).w), (c_id)) 

#define BCM_FIELD_CSET_TEST(c_set, c_id)  SHR_BITGET(((c_set).w), (c_id)) 
...
#define BCM_FIELD_GROUP_CREATE_SPARSE_ENTRY_PRIORITIES (1 << 9)   /* If set, the TCAM
                                                          ...
                                                          the Field group
                                                          entries */
#define BCM_FIELD_GROUP_CREATE_WITH_CSET    (1 << 10)  /* If set, Groups lookup
                                                          key includes
                                                          compressed KEY fields */
...
#define BCM_FIELD_COMPRESS_MAX  512        /* Highest possible compressio
                                              identifiers. */

/* Bitmap of all possible field compression IDs. */
typedef struct bcm_field_compress_set_s {
    SHR_BITDCL w[_SHR_BITDCLSIZE(BCM_FIELD_COMPRESS_MAX)]; 
} bcm_field_compress_set_t;
...
typedef struct bcm_field_group_config_s {
    ...
    bcm_field_compress_set_t cset;      /* Bitmap of compression identifiers
                                           used in Field Group. */
...
/* Opaque handle to a field compress. */
typedef int bcm_field_compress_t;

/* 
 * Options to create compression identifier using
 * bcm_field_compress_create API.
 */
typedef enum bcm_field_compression_e {
    bcmFieldCompressionWithId,  /* Create field compression Identifier with the
                                   given compress_id of type
                                   bcm_field_compress_t . */
    bcmFieldCompressionCount    /* Always last. Not a usable value. */
} bcm_field_compression_t;

#define BCM_FIELD_COMPRESSION_STRINGS \
{ \
    "WithId"  \
}

/* 
 * Field compression attributre object used for creation of compression
 * identifier.
 */
typedef struct bcm_field_compress_attr_s {
    bcm_field_qualify_t qual;           /* Field qualifier to indicate the match
                                           field that is being compressed. */
    int priority;                       /* Entry priority, applicable to TCAM
                                           entries. */
    int size;                           /* Number of entries that must be
                                           programmed for this compress_id. */
    bcm_field_compress_t compress_id;   /* Compression identifier. */
} bcm_field_compress_attr_t;

/* Initializes a bcm_field_compress_attr structure to all zeros. */
extern void bcm_field_compress_attr_t_init(
    bcm_field_compress_attr_t *c_attr);

#ifndef BCM_HIDE_DISPATCHABLE

/* 
 * Returns default maximum number of compression table entries that must
 * be set up for a compression qualifier.
 */
extern int bcm_field_compress_size_get(
    int unit, 
    bcm_field_qualify_t qual, 
    int *max_size);

/* 
 * Create a compression identifier for a given qualifier and reserve size
 * (if supplied) number of entries in hardware for this qualifier.  By
 * default, this API reserves max_size entries for the qualifier.
 */
extern int bcm_field_compress_create(
    int unit, 
...
/* 
 * Populates the compression table with the user supplied compression
 * value/s.
 */
extern int bcm_field_compress_set(
    int unit, 
    int compress_id, 
    bcm_field_qualify_t qual, 
    int alloc_size, 
 ...
 * value/s.
 */
extern int bcm_field_compress_multi_set(
    int unit, 
    int compress_id, 
    int alloc_size, 
    uint32 *orig_data, 
...
/* 
 * Retrieves the compression table entry data, mask and compressed values
 * for a given compression identifier and qualifier.
 */
extern int bcm_field_compress_get(
    int unit, 
    int compress_id, 
    bcm_field_qualify_t qual, 
    uint64 *comp_data, 
...
/* 
 * For a given compression identifier, this API returns the qualifier
 * name, size of the compression array, pointer to the actual packet data
 * array and a pointer to compressed data array.
 */
extern int bcm_field_compress_multi_get(
    int unit, 
    int compress_id, 
    uint16 alloc_size, 
    uint32 *orig_data, 
...
/* 
 * Destroys a field compression identifier and its associated hardware
 * table entry or entries.
 */
extern int bcm_field_compress_destroy(
    int unit, 
    int compress_id);

/* 
 * For a given qualifier, this API returns the maximum MASK length value
 * supported in hardware for a compressed qualifier.
 */
extern int bcm_field_compressed_mask_length_get(
    int unit, 
    bcm_field_qualify_t qual, 
    int *length);
