 ...
 *
 * File:        wb_db_policer.h
 * Purpose:     WarmBoot - Level 2 support (POLICER Module)
 */

#ifndef _BCM_DPP_WB_DB_POLICER_H_
#define _BCM_DPP_WB_DB_POLICER_H_
 ...
 *
 * Currently there is a single handle for POLICER module.
...
#define BCM_DPP_WB_POLICER_VERSION_1_0            SOC_SCACHE_VERSION(1,0)
#define BCM_DPP_WB_POLICER_CURRENT_VERSION        BCM_DPP_WB_POLICER_VERSION_1_0


#define BCM_DPP_WB_POLICER_INFO(unit)             (_dpp_wb_policer_info_p[unit])
#define BCM_DPP_WB_POLICER_INFO_INIT(unit)        ( (_dpp_wb_policer_info_p[unit] != NULL) &&          \
                                                 (_dpp_wb_policer_info_p[unit]->init_done == TRUE) )
...
#define BCM_DPP_WB_DEV_POLICER_DIRTY_BIT_SET(unit)                                       \
                        SOC_CONTROL_LOCK(unit);                                       \
                        SOC_CONTROL(unit)->scache_dirty = 1;                          \
                        BCM_DPP_WB_POLICER_INFO(unit)->is_dirty = 1;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_POLICER_DIRTY_BIT_CLEAR(unit)                                         \
                        SOC_CONTROL_LOCK(unit);                                       \
                        BCM_DPP_WB_POLICER_INFO(unit)->is_dirty = 0;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_POLICER_IS_DIRTY(unit)                                                \
                        (BCM_DPP_WB_POLICER_INFO(unit)->is_dirty == 1)
 ...
 * runtime information
 */
typedef struct bcm_dpp_wb_policer_info_s {
    int            init_done;
    int            is_dirty;
...
} bcm_dpp_wb_policer_info_t;

extern bcm_dpp_wb_policer_info_t   *_dpp_wb_policer_info_p[BCM_MAX_NUM_UNITS] ;
/*
 * Functions
 */
extern int
_bcm_dpp_wb_policer_state_init(int unit);

extern int
_bcm_dpp_wb_policer_state_deinit(int unit);

extern int
_bcm_dpp_wb_policer_sync(int unit);

extern int
_bcm_dpp_wb_policer_update_group_mode_state(int unit, int policer);

#endif /* BCM_WARM_BOOT_SUPPORT */

#endif /* _BCM_DPP_WB_DB_POLICE_H_ */
