 ...
 *
 * File:        wb_db_multicast.h
 * Purpose:     WarmBoot - Level 2 support (MULTICAST Module)
 */

#ifndef _BCM_DPP_WB_DB_MULTICAST_H_
#define _BCM_DPP_WB_DB_MULTICAST_H_
 ...
 *
 * Currently there is a single handle for MULTICAST module.
...
#define BCM_DPP_WB_MULTICAST_VERSION_1_0            SOC_SCACHE_VERSION(1,0)
#define BCM_DPP_WB_MULTICAST_CURRENT_VERSION        BCM_DPP_WB_MULTICAST_VERSION_1_0


#define BCM_DPP_WB_MULTICAST_INFO(unit)             (_dpp_wb_multicast_info_p[unit])
#define BCM_DPP_WB_MULTICAST_INFO_INIT(unit)        ( (_dpp_wb_multicast_info_p[unit] != NULL) &&         \
                                                 (_dpp_wb_multicast_info_p[unit]->init_done == TRUE) )
 ...
 */
#define BCM_DPP_WB_MULTICAST_MAX_MC_GROUPS_EGREES(unit)                                          \
                        (SOC_DPP_CONFIG(unit)->tm.nof_mc_ids)
#define BCM_DPP_WB_MULTICAST_MAX_MC_GROUPS_INGREES(unit)                                         \
                        (SOC_DPP_CONFIG(unit)->tm.nof_mc_ids)

#define BCM_DPP_WB_DEV_MULTICAST_DIRTY_BIT_SET(unit)                                  \
                        SOC_CONTROL_LOCK(unit);                                       \
                        SOC_CONTROL(unit)->scache_dirty = 1;                          \
                        BCM_DPP_WB_MULTICAST_INFO(unit)->is_dirty = 1;                \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_MULTICAST_DIRTY_BIT_CLEAR(unit)                                         \
                        SOC_CONTROL_LOCK(unit);                                       \
                        BCM_DPP_WB_MULTICAST_INFO(unit)->is_dirty = 0;                     \
                        SOC_CONTROL_UNLOCK(unit);

#define BCM_DPP_WB_MULTICAST_IS_DIRTY(unit)                                                \
                        (BCM_DPP_WB_MULTICAST_INFO(unit)->is_dirty == 1)
 ...
 * runtime information
 */
typedef struct bcm_dpp_wb_multicast_info_s {
    int                      init_done;
    /*bcm_dpp_multicast_config_t   *multicast_config;*/
    ...
    uint32                   max_ingress_mc;
    uint32                   ingress_mc_off;

    uint32                   max_egress_mc;
    uint32                   egress_mc_off;

    uint32                   gbl_data_off;
} bcm_dpp_wb_multicast_info_t;

/*
 * ingress multicast data structure - save if mc id is alloced or not
 *  
 */
typedef struct bcm_dpp_wb_multicast_ingress_mc_config_s {
    uint8    is_alloc;
}  __ATTRIBUTE_PACKED__ bcm_dpp_wb_multicast_ingress_mc_config_t;


/*
 * egress multicast data structure - save if mc id is alloced or not
 *  
 */
typedef struct bcm_dpp_wb_multicast_egress_mc_config_s {
    uint8    is_alloc;
}  __ATTRIBUTE_PACKED__ bcm_dpp_wb_multicast_egress_mc_config_t;


extern bcm_dpp_wb_multicast_info_t   *_dpp_wb_multicast_info_p[BCM_MAX_NUM_UNITS] ;
...
#endif /* _BCM_DPP_WB_DB_MULTICAST_H_ */
