...
xLibRC_t fpObjGet_baseDVLANTunneling_EtherType (void *wap, void *bufp)
{
  ...
  for(i=0; i<L7_DVLANTAG_MAX_TPIDS; i++)
  {
    ...
    osapiStrncat(obj_ether_type_value , tmpbuf, sizeof(tmpbuf));
  }
...
xLibRC_t fpObjGet_baseDVLANTunneling_GlobalEtherType(void *wap, void *bufp)
{
 ...
 /* retrieve object: Global Ether Type */
  ...
  if (usmDbFeaturePresentCheck(L7_UNIT_CURRENT, L7_DVLANTAG_COMPONENT_ID, L7_DVLANTAG_MULTI_TPIDS_FEATURE_ID)
                                 == L7_TRUE)
  {
    ...
    {
      if (usmDbDvlantagEthertypeGet(L7_UNIT_CURRENT, &tpidValue, i) == L7_SUCCESS)
      {
        if(tpidValue == 0)
          continue;
        osapiSnprintf(tmpbuf, sizeof(tmpbuf), "0x%x ", tpidValue);
        osapiStrncat(objEtherTypeValue , tmpbuf, sizeof(tmpbuf));
      }
    }
  }
  else
  {
    for(i=0; i<L7_DVLANTAG_MAX_TPIDS; i++)
    {
      if (usmDbDvlantagEthertypeGet(L7_UNIT_CURRENT, &tpidValue, i) == L7_SUCCESS)
      {
        if (tpidValue != 0x00 && i == 0)
        {
          osapiSnprintf(objEtherTypeValue, sizeof(objEtherTypeValue), "%d", tpidValue); 
        }
      }
    }
  }
...
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseDVLANTunneling_GlobalPrimaryTpid (void *wap, void *bufp)
{
  fpObjWa_t  owa = FPOBJ_INIT_WA2 ();
  xLibStr256_t objEtherTypeValue = {0};
  L7_uint32  tpidValue;
  ...
  /* get the value from application */
  if (usmDbDvlantagEthertypeGet(L7_UNIT_CURRENT, &tpidValue, 0) == L7_SUCCESS)
  {
...
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseDVLANTunneling_GlobalPrimaryTpid (void *wap, void *bufp)
{
  fpObjWa_t     owa = FPOBJ_INIT_WA2 ();
  xLibStr256_t  objGlobalEtherTypeValue_str = {0};
  xLibU32_t     objGlobalEtherTypeValue_uint; 
...
xLibRC_t fpObjGet_baseDVLANTunneling_GlobalDVlanTagRowStatus(void *wap, void *bufp)
{
     ...
     for(i=0; i<L7_DVLANTAG_MAX_TPIDS; i++)
     {
          if (usmDbDvlantagEthertypeGet(L7_UNIT_CURRENT, &tpid_value, i) == L7_SUCCESS)
          {
...
xLibRC_t fpObjSet_baseDVLANTunneling_GlobalDVlanTagRowStatus(void *wap, void *bufp)
{
     ...
     xLibU32_t     primaryTpid = L7_FALSE;
...
/*******************************************************************************
* @function fpObjGet_baseDVLANTunneling_GlobalEtherTypeEnum
*
* @purpose Get 'GlobalEtherTypeEnum'
* @description  [GlobalEtherTypeEnum] Get the Ethertype Enum value
* @notes
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseDVLANTunneling_GlobalEtherTypeEnum(void *wap, void *bufp)
{
  fpObjWa_t  owa = FPOBJ_INIT_WA2 ();
  xLibU32_t objGlobalEtherTypeValue;
  xLibU32_t objGlobalEtherTypeEnumValue;
  xLibU32_t tpIdx = 0;
  FPOBJ_TRACE_ENTER (bufp);
  ...
  {
    if((owa.l7rc = usmDbDvlantagEthertypeGet(L7_UNIT_CURRENT, &objGlobalEtherTypeValue, tpIdx))
         == L7_SUCCESS )
    {
      if (objGlobalEtherTypeValue != 0x00 &&  tpIdx == 0)
      {
       switch(objGlobalEtherTypeValue)
       {
         case 33024:
           objGlobalEtherTypeEnumValue = L7_XUI_DVLANTAG802Q_ETHERTYPE;
           break;
         case 34984:
           objGlobalEtherTypeEnumValue = L7_XUI_DVLANTAGVMAN_ETHERTYPE;
           break;
         default:
           objGlobalEtherTypeEnumValue = L7_XUI_DVLANTAGCUSTOM_ETHERTYPE;
           break;
       }
      }
    }
  ...
  /* return the object value: GlobalEtherTypeEnum */
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) & objGlobalEtherTypeEnumValue,
                           sizeof (objGlobalEtherTypeEnumValue));
...
/*******************************************************************************
* @function fpObjSet_baseDVLANTunneling_GlobalEtherTypeEnum
*
* @purpose Set 'GlobalEtherTypeEnum'
*
* @description [GlobalEtherTypeEnum] ToDO: Add Help
*
* @notes
*
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseDVLANTunneling_GlobalEtherTypeEnum (void *wap, void *bufp)
{
  fpObjWa_t  owa = FPOBJ_INIT_WA2 ();
  FPOBJ_TRACE_ENTER (bufp);
