...
L7_RC_t sysapiConfigServicePortIp (L7_uint32 ipAddr, L7_uint32 ipNetMask)
{
  ...
  sprintf(buf,"%s%d", bspapiServicePortNameGet(), bspapiServicePortUnitGet());
                                 ...
                                 L7_in6_addr_t *newGateway)
{
  ...
  if (newGateway != L7_NULLPTR)
  {
    ...
    if (strcmp(ifName, bspapiServicePortNameGet()) == 0)
      sprintf(nameBuf,"%s%d", ifName, bspapiServicePortUnitGet());
    else if (strcmp(ifName, L7_DTL_PORT_IF) == 0)
