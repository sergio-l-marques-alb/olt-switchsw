...
if [ "$1" == "cxo2t4" ]; then
  ...
  export PTIN_MGMD_PLATFORM_MAX_CLIENTS=128
  export PTIN_MGMD_PLATFORM_MAX_PORTS=12
  export PTIN_MGMD_PLATFORM_MAX_PORT_ID=16
  export PTIN_MGMD_PLATFORM_MAX_SERVICES=40
  export PTIN_MGMD_PLATFORM_MAX_SERVICE_ID=255
  export PTIN_MGMD_PLATFORM_ADMISSION_CONTROL_SUPPORT=1
  export PTIN_MGMD_PLATFORM_ROOT_PORT_IS_ON_MAX_PORT_ID=1
...
echo "[MGMD] Compiling..."
make -j$NUM_CPUS -C $MGMD_OUTPUT_PATH install
...
#Create buildir directory to store the generated  .h and .so files  
mkdir -pv $BUILDIR/$BOARD/usr/local/lib/mgmd
mkdir -pv $BUILDIR/$BOARD/usr/local/include/mgmd
...
cp -uv $MGMD_OUTPUT_PATH/src/ptin_mgmd_defs.h $BUILDIR/$BOARD/usr/local/include/mgmd | awk -F'/' '{if ($NF != "") print $NF " updated!"}' | sed "s/'//"
