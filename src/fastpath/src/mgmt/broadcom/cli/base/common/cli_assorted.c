...
const L7_char8 *commandDevShell(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  {                                                                       /*check to for invalid # of arguments */
   ...
   if (cliTelnetConnectionGet() == L7_TRUE)
   {
       ...
       {
         L7_uint32 j, num_cmds;
         static const char* prohibited_cmds[] = {"i", "osapiGetChoice",
                                                 "osapiFsUtils", "osapiDebugRegisteredSysInfoDump",
                                                 "layer3DebugMenu", "ipMapCfgDataShow",
                                                 "ripMapCfgDataShow", "ospfMapCfgDataShow",
                                                 "ip6MapCfgDataShow",
                                                 "v3layer3DebugMenu", "ospfv3MapCfgDataShow"};
         num_cmds = sizeof(prohibited_cmds) / sizeof(char*);
         for (j=0; j<num_cmds; j++)
         {
           if (strcmp(argv[index+1], prohibited_cmds[j]) == 0)
           {
             runDevShell = L7_FALSE;
             ewsTelnetWriteAddBlanks(1, 0, 0, 0, L7_NULLPTR, ewsContext, pStrErr_base_ProhibitedCmdNotAuthorized);
   ...
   if (runDevShell == L7_TRUE)
   {
     ...
     if ((usmDbOperSysGet(0, stat) == L7_SUCCESS) &&
         (strncmp(stat, "Linux", 5) == 0) &&
         (strlen(argv[index+1]) >= strlen("binsh")) && 
         (strncmp(argv[index+1], "binsh", 5)) == 0)
     {
       sysapiPrintf("\n");
       sysapiPrintf("Please note: All other UI sessions will be suspended until you exit the shell.");
     }
       sysapiPrintf("\n");
...
*                                    [interval <interval>][size <size>] [source { <ip-address> | <if-name> } ]
...
const L7_char8 *commandPing(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  L7_char8 sourceStr[L7_CLI_MAX_STRING_LENGTH];
  L7_uint32 srcMask = 0;
  L7_uint32 u, s, p;
  ...
  L7_uint32 unit, numArgs;
  L7_uchar8 *syntax = "Use 'ping {<ipaddr> [count <count>][interval <interval>][size <size>][source {<src-address> | <if-name>}]'";
  ...
  if (numArgs > 9)
  {
    return cliSyntaxReturnPromptAddBlanks(1, 1, 0, 0, pStrErr_common_IncorrectInput, ewsContext, syntax);
  }
...
#ifdef L7_ROUTING_PACKAGE
    if (strcmp(buf, L7_PING_SOURCE) == 0)
    {
      if (strlen(argv[index + 2]) >= sizeof(sourceStr))
      {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                               pStrErr_common_CfgBgpRtrPolicyIpAddrInvalid);
      }
      OSAPI_STRNCPY_SAFE(sourceStr, argv[index + 2]);
      /* see if user specified source as an IP address */
      if (usmDbInetAton(sourceStr, &srcIpAddr) == L7_SUCCESS)
      {
        /* Verify that source IP address is configured on a local interface. */
        if (usmDbIPAddrToIntIf(unit, srcIpAddr, &intIfNum) != L7_SUCCESS)
        {
          return cliSyntaxReturnPromptAddBlanks(1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                "The source IP address is not an address on a local interface");
        }
        numArgs -= 2;
        index += 2;
      }
      ...
      {
        /* See if user entered an interface name */
        if ((strcmp(argv[index + 2], RLIM_INTF_LOOPBACK_NAME_PREFIX) == 0))
        {
#ifdef L7_RLIM_PACKAGE
          if (cliValidateLoopbackId(ewsContext, (L7_char8 *)argv[index + 3],
                                    &loopbackID) != L7_SUCCESS)
          {
            return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput,
                                                   ewsContext, pStrErr_common_LoopBackIdOutOfRange, 
                                                   L7_CLIMIN_LOOPBACKID, L7_CLIMAX_LOOPBACKID);
          }
          if (usmDbRlimLoopbackIntIfNumGet(loopbackID, &intIfNum) != L7_SUCCESS)
          {
            return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                   "The source loopback interface does not exist.");
          }
          else
          {
            ...
            {
              return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                     "Source loopback interface not found.");
            }
          }
          numArgs -= 3;
          index += 3;
        ...
        {
          /* source specified as u/s/p */
          if (cliIsStackingSupported())
          {
            if (cliValidSpecificUSPCheck(argv[index + 2], &u, &s, &p) != L7_SUCCESS)
            {
              return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                     "Source interface is invalid.");
            }
          ...
          else
          {
            /* non-stacking */
            u = cliGetUnitId();
            if (cliSlotPortToIntNum(ewsContext, argv[index + 2], &s, &p, &intIfNum) != L7_SUCCESS)
            {
              return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, "Source interface not found.");
            }
          }
          numArgs -= 2;
          index += 2;
        ...
        /* Verify that source interface is a valid IP interface */
        if ((intIfNum == 0) || (cliValidateRtrIntf(ewsContext, intIfNum) != L7_SUCCESS))
        {
          return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                 "The ping source is invalid.");
        }

        /* Verify that the source interface is up */
        if ((usmDbIpRtrIntfOperModeGet(intIfNum, &ifState) != L7_SUCCESS) ||
            (ifState != L7_ENABLE))
        {
          return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                 "The source interface is not up.");
        }

        /* Use the primary IP address on the source interface as the source 
         * address for the Echo Request. */
        if (usmDbIpRtrIntfIpAddressGet(unit, intIfNum, &srcIpAddr, &srcMask) != L7_SUCCESS)
        {
          return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                                 "Failed to get the IP address on the source interface.");
        }
      }
      ...
      {
        return cliSyntaxReturnPromptAddBlanks (1, 1, 0, 0, L7_NULLPTR, ewsContext, 
                                               "Failed to set the ping source.");
      }
      
    }
    else   /* not source option */
    {
  ...
  if ( usmDbPingStart( "", "", L7_TRUE, vrfId, ipAddr_check, probeCount, probeSize, probeInterval, 
                       srcIpAddr, pingCallback, NULL, &ewsContext, &handle ) != L7_SUCCESS )
  {
