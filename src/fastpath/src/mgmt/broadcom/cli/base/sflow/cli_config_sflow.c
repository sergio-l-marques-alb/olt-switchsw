...
const L7_char8 *commandInterfacesFlowSampler(EwsContext ewsContext, L7_uint32 argc, const L7_char8 * * argv, L7_uint32 index)
{
  ...
  if (numArg ==1 && ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    for (intInterface=1; intInterface < L7_MAX_INTERFACE_COUNT; intInterface++)
    {
      if (!L7_INTF_ISMASKBITSET(EWSINTFMASK(ewsContext), intInterface) ||
          usmDbUnitSlotPortGet(intInterface, &unit, &s, &p) != L7_SUCCESS)
      {
        continue;
      }
  ...
  if (numArg ==1 && ewsContext->commType == CLI_NORMAL_CMD)
  {
    ...
    for (intInterface=1; intInterface < L7_MAX_INTERFACE_COUNT; intInterface++)
    {
      ...
      else if (rc == L7_FAILURE)
      {
        status = L7_FAILURE;
        ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
        ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_IncorrectInput, ewsContext,
                                                    pStrErr_common_InvalidsFlowRcvrIndex);
      }
    }
  }
