                                         ...
                                         L7_uint32 index)
{
  ...
  if (numArg == 1)
  {
    ...
    if (usmDbInetAton(ipAddr, &intIpAddress) == L7_SUCCESS)
    {
      ...
      {
        osapiSnprintfAddBlanks (0, 0, 5, 0, L7_NULLPTR, buffer,sizeof(buffer),pStrInfo_common_Manual);
      }
      else if (intPoolType == L7_DHCPS_DYNAMIC_POOL)
      {
        osapiSnprintfAddBlanks (0, 0, 5, 0, L7_NULLPTR, buffer,sizeof(buffer),pStrInfo_base_Auto_1);
      }
      else
      {
        osapiSnprintfAddBlanks (0, 0, 5, 0, L7_NULLPTR, buffer,sizeof(buffer),pStrInfo_common_Unknown_1);
      }
