    ...
    const L7_char8 * cmdParm )
{
  ...
  L7_int32 retVal;
  static L7_BOOL all;
  static L7_BOOL active = L7_FALSE;
  L7_BOOL boolVal;
  ...
  else
  {
    ...
    else if ( strcmp(strSlotPort, pStrInfo_common_Active   /* "active" */)==0 )
    {
      active = L7_TRUE;
      all = L7_FALSE;
      interface = 1;
  ...
  while ( interface != 0 )
  {
    ...
    {
      if ((active == L7_TRUE) &&
          cliIsSpanningTreePortActive(unit, instance, interface) != L7_TRUE)
      {
        /* Do not show this interface*/
      }
      else
      {
          memset (stat, 0, sizeof(stat));
    ...
    L7_uint32 index)
{
  ...
  (void)usmDbDot1sVlanToMstiConfiguredGet(unit, vid, &mstid);
