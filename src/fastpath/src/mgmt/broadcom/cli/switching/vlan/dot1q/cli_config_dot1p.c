    ...
    if(ewsContext->commType == CLI_NORMAL_CMD)
    {
      ...
      if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          ...
          ewsTelnetWriteAddBlanks (1, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_switching_Set8021pMapping);
          continue;
        }
      }
        ...
        if(ewsContext->scriptActionFlag==L7_EXECUTE_SCRIPT)
        {
          for (interface=1; interface < L7_MAX_INTERFACE_COUNT; interface++)
          {
            ...
            if (rc == L7_NOT_SUPPORTED)
            {
              ...
              status = L7_FAILURE;
              ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
              ewsTelnetWriteAddBlanks (1, 1, 0, 0, L7_NULLPTR,  ewsContext, pStrInfo_switching_AnInvalidSlotPortHasBeenUsedForFunc);
            ...
            else if (rc != L7_SUCCESS)
            {
              status = L7_FAILURE;
              ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR, ewsContext, cliDisplayInterfaceHelp(unit, s, p));
              ewsTelnetWriteAddBlanks (0, 1, 0, 0, pStrErr_common_CouldNot,  ewsContext, pStrInfo_switching_CfgurePortPri);
            }
          }
        }
      }
