    ...
    L7_uint32 intfType)
{
  ...
  /* find the first interface selected */
  L7_INTF_FHMASKBIT(EWSINTFMASK(ewsContext),intIfNum);

  if (intIfNum >= 0 && usmDbUnitSlotPortGet(intIfNum, unit, slot, port) != L7_SUCCESS)
  {
    ewsTelnetWriteAddBlanks (1, 0, 0, 0, L7_NULLPTR,  ewsContext, pStrErr_common_InvalidSlotPort_1);
    cliSyntaxBottom(ewsContext);
    return L7_FAILURE;
