...
L7_RC_t cliRunningConfigGlobalCosQueue(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_MAX_BW_FEATURE_ID) == L7_TRUE)
  {
    ...
    if (usmDbQosCosQueueMaxBandwidthListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &queueBwList) == L7_SUCCESS)
    {
  ...
  {
    memset(&queueBwList, 0, sizeof(queueBwList));
    if (usmDbQosCosQueueMinBandwidthListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &queueBwList) == L7_SUCCESS)
    {
      memset (buf, 0,sizeof(buf));
      for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
      {
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_MGMT_INTF_ONLY_FEATURE_ID) == L7_FALSE)
  {
    ...
    if (usmDbQosCosQueueMgmtTypeListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &queueMgmtTypeList) == L7_SUCCESS)
    {
      memset (buf, 0,sizeof(buf));
      for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
      {
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_SCHED_STRICT_ONLY_FEATURE_ID) == L7_FALSE)
  {
    ...
    if (usmDbQosCosQueueSchedulerTypeListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &queueSchedTypeList) == L7_SUCCESS)
    {
      memset (buf, 0,sizeof(buf));
  ...
  {
    if (usmDbQosCosQueueIntfShapingRateGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &val, L7_NULLPTR) == L7_SUCCESS)
    {
...
L7_RC_t cliRunningConfigInterfaceCosQueue(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
      ...
      usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_CFG_PER_INTF_FEATURE_ID) == L7_TRUE)
  {
    ...
    if(usmDbQosCosQueueMinBandwidthListGet(unit, interface, L7_QOS_QSET_PORT, &interfaceQueueBwMinList) == L7_SUCCESS)
    {
      if(usmDbQosCosQueueMinBandwidthListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalQueueBwMinList) == L7_SUCCESS)
      {
        memset (buf, 0,sizeof(buf));
        for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
        {
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_SCHED_STRICT_ONLY_FEATURE_ID) == L7_FALSE)
  {
    ...
    if (usmDbQosCosQueueSchedulerTypeListGet(unit, interface, L7_QOS_QSET_PORT, &interfaceQueueSchedTypeList) == L7_SUCCESS  &&
        usmDbQosCosQueueSchedulerTypeListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalQueueSchedTypeList) == L7_SUCCESS)
    {
      memset (buf, 0,sizeof(buf));
      for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
      {
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_INTF_SHAPING_FEATURE_ID) == L7_TRUE)
  {
    if (usmDbQosCosQueueIntfShapingRateGet(unit, interface, L7_QOS_QSET_PORT, &interfaceRate, L7_NULLPTR) == L7_SUCCESS  &&
        usmDbQosCosQueueIntfShapingRateGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalRate, L7_NULLPTR) == L7_SUCCESS)
    {
      ...
      usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_CFG_PER_INTF_FEATURE_ID) == L7_TRUE)
  {
    ...
    if (usmDbQosCosQueueMaxBandwidthListGet(unit, interface, L7_QOS_QSET_PORT, &interfaceQueueBwMaxList) == L7_SUCCESS &&
        usmDbQosCosQueueMaxBandwidthListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalQueueBwMaxList) == L7_SUCCESS)
    {
      memset (buf, 0,sizeof(buf));
      for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
      {
      ...
      usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_WRED_SUPPORT_FEATURE_ID) == L7_TRUE) 
  {
    ...
    if (usmDbQosCosQueueMgmtTypeListGet(unit, interface, L7_QOS_QSET_PORT, &interfaceQueueMgmtTypeList) == L7_SUCCESS  &&
        usmDbQosCosQueueMgmtTypeListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalQueueMgmtTypeList) == L7_SUCCESS)
    {
      buf[0] = '\0';
      buf2[0] = '\0';
      for (queueNum = 0 ; queueNum < L7_MAX_CFG_QUEUES_PER_PORT; queueNum++)
...
L7_RC_t cliRunningConfigGlobalClassofService(EwsContext ewsContext, L7_uint32 unit)
{
  ...
  {
    if (usmDbQosCosMapTrustModeGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalVal) == L7_SUCCESS &&
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_MAP_IPDSCP_FEATURE_ID) == L7_TRUE)
  {
    ...
    {
      if ((usmDbQosCosMapIpDscpTrafficClassGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, dscp, &globalVal) == L7_SUCCESS &&
           usmDbQosCosMapIpDscpDefaultTrafficClassGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, dscp, &defaultTrafficClass) == L7_SUCCESS) && (globalVal != defaultTrafficClass || EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
  ...
  if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_MAP_IPPREC_FEATURE_ID) == L7_TRUE)
  {
    ...
    {
      if ((usmDbQosCosMapIpPrecTrafficClassGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, precedence, &globalVal) == L7_SUCCESS &&
           usmDbQosCosMapIpPrecDefaultTrafficClassGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, precedence, &defaultTrafficClass) == L7_SUCCESS) && ( globalVal != defaultTrafficClass || EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
...
L7_RC_t cliRunningConfigInterfaceClassofService(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
  ...
  {
    if ((usmDbQosCosMapTrustModeGet (unit, interface, L7_QOS_QSET_PORT, &val) == L7_SUCCESS &&
         usmDbQosCosMapTrustModeGet (unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalVal) == L7_SUCCESS) &&
      ...
      (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_MAP_IPDSCP_PER_INTF_FEATURE_ID) == L7_TRUE))
  {
    for (dscp = 0; dscp < L7_QOS_COS_MAP_NUM_IPDSCP; dscp++)
    {
      if ((usmDbQosCosMapIpDscpTrafficClassGet(unit, interface, L7_QOS_QSET_PORT, dscp, &val) == L7_SUCCESS &&
           usmDbQosCosMapIpDscpTrafficClassGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, dscp, &globalVal) == L7_SUCCESS) &&
          (val != globalVal || EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
      ...
      (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_MAP_IPPREC_PER_INTF_FEATURE_ID) == L7_TRUE))
  {
    for (precedence = 0; precedence < L7_QOS_COS_MAP_NUM_IPPREC; precedence++)
    {
      if ((usmDbQosCosMapIpPrecTrafficClassGet(unit, interface, L7_QOS_QSET_PORT, precedence, &val) == L7_SUCCESS &&
           usmDbQosCosMapIpPrecTrafficClassGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, precedence, &globalVal) == L7_SUCCESS)&&
          (val != globalVal || EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))
      {
...
L7_RC_t cliRunningConfigInterfaceRandomDetect(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
    ...
    if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_MGMT_INTF_ONLY_FEATURE_ID) == L7_TRUE &&
        usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_COS_COMPONENT_ID, L7_COS_QUEUE_WRED_SUPPORT_FEATURE_ID) == L7_TRUE)
    {
      if ((usmDbQosCosQueueMgmtTypePerIntfGet(unit, interface, L7_QOS_QSET_PORT, &val) == L7_SUCCESS) &&
          (((val != FD_QOS_COS_QCFG_MGMT_TYPE) || (EWSSHOWALL(ewsContext) == L7_SHOW_DEFAULT))) && 
           ((interface == L7_ALL_INTERFACES) ||
            (usmDbQosCosQueueMgmtTypePerIntfGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalVal) != L7_SUCCESS) ||
            (val != globalVal)))
      {
        memset (buf, 0,sizeof(buf));
        switch (val)
    ...
    if ((usmDbQosCosQueueWredDecayExponentGet(unit, interface, L7_QOS_QSET_PORT, &val) == L7_SUCCESS) &&
        ...
        ((interface == L7_ALL_INTERFACES) || 
        (usmDbQosCosQueueWredDecayExponentGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalVal) != L7_SUCCESS) || 
        (val != globalVal)))
    {
    ...
    {
      if ((interface != L7_ALL_INTERFACES) || (usmDbQosCosQueueDefaultConfigGet(unit, interface, L7_QOS_QSET_PORT, &defCfg) == L7_SUCCESS))
      {
          if (usmDbQosCosQueueDropParmsListGet(unit, interface, L7_QOS_QSET_PORT, &dList) == L7_SUCCESS)
          {
            if ((interface == L7_ALL_INTERFACES) || 
                (usmDbQosCosQueueDropParmsListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalDList) == L7_SUCCESS)) 
            {
...
L7_RC_t cliRunningConfigInterfaceTailDrop(EwsContext ewsContext, L7_uint32 unit, L7_uint32 interface)
{
  ...
  if ((interface != L7_ALL_INTERFACES) || (usmDbQosCosQueueDefaultConfigGet(unit, interface, L7_QOS_QSET_PORT, &defCfg) == L7_SUCCESS))
  {
    if (usmDbQosCosQueueDropParmsListGet(unit, interface, L7_QOS_QSET_PORT, &dropList) == L7_SUCCESS)
    {
      if ((interface == L7_ALL_INTERFACES) || 
          (usmDbQosCosQueueDropParmsListGet(unit, L7_ALL_INTERFACES, L7_QOS_QSET_PORT, &globalDropList) == L7_SUCCESS)) 
      {
          for (queueId = 0; queueId < L7_MAX_CFG_QUEUES_PER_PORT; queueId++)
          {
            this_queueset[0] = '\0';
