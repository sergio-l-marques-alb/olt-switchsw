...
static struct EwsArchive_s* ew_archive_p = NULL;
static uint8 *ew_archive_data_p = NULL;
...
EwsAuthHandle         auth_handle[2];                       /* returned handle */
appAuthHandle         appAuthHndle[2];
#endif
...
char emweb_hostname[EMWEB_MAX_NAME_LEN];
char *emweb_argv[2];
...
int ewaTaskInit(void)
{
#if defined(L7_WEB_PACKAGE) || defined(L7_XWEB_PACKAGE)
  usmWebGetArchive((void **)&ew_archive_data_p, (void **)&ew_archive_p);
#endif

#if defined(L7_XWEB_PACKAGE) || defined(L7_XCLI_PACKAGE)
  /* Initialise XLIB */
  xLibInit ();
#endif
  return 0;
...
/*
 * Launch the tEmWeb task
 */
int ewaTaskStart(void) {

    L7_uint32 val;

   usmDbServicePortIPAddrGet(1, &val);
   usmDbInetNtoa(val, emweb_hostname);
   emweb_hostname[(EMWEB_MAX_NAME_LEN - 1)] = '\0';

   emweb_argv[0] = "emweb_main"; /* doesn't matter */
   emweb_argv[1] = emweb_hostname;
  ...
  if ( argc >= 1 )
    {
      ...
      strcpy(local_host_name, argv[0]);
      /*
      printf("ewTask: local_host_name: %s\n", local_host_name);
      */
#endif
...
# endif
   ...
   */
  if(ew_archive_p == NULL || ew_archive_data_p == NULL)
  {
    status = EWA_STATUS_OK;
  }
  else
  {
    status = ewsDocumentInstallArchive( ew_archive_p ,ew_archive_data_p);
  }
...
#     ifdef EW_CONFIG_OPTION_AUTH_BASIC
       ...
       */
#     ifdef COMMENTED_OUT  /* LVL7_P0006 Start */
...
#     endif /* COMMENTED_OUT */  /* LVL7_P0006 End */

  /* lvl7_07258 Start */

  auth.scheme = ewsAuthSchemeManualBasic;
  strcpy(appAuthHndle[0].uname, "READWRITE");
  strcpy(appAuthHndle[0].access, "READWRITE");
  auth.handle = (void *) &appAuthHndle[0];
  auth_handle[0] = ewsAuthRegister("READWRITE", &auth);

  if (auth_handle[0] == EWS_AUTH_HANDLE_NULL)
    EMWEB_ERROR(( "ewsAuthRegister failed\n" ));

  auth.scheme = ewsAuthSchemeManualBasic;
  strcpy(appAuthHndle[1].uname, "READONLY");
  strcpy(appAuthHndle[1].access, "READONLY");
  auth.handle = (void *) &appAuthHndle[1];
  auth_handle[1] = ewsAuthRegister("READONLY", &auth);

  if (auth_handle[1] == EWS_AUTH_HANDLE_NULL)
    EMWEB_ERROR(( "ewsAuthRegister failed\n" ));
...
static int xLibWaDelayedCreate = 0;
...
{
#if defined(L7_XWEB_PACKAGE)
  if(connection != NULL && connection->workarea != NULL)
  {
...
# else
  /* Begin LVL7 2478 */
  /*
  Get localName from current context's NetHandle, just
  like VxWorks does it, otherwise multi-homed machines
  will not work properly
  */
  EMWEB_TRACE(("ewaNetLocalHostName: context = %p net handle = %p local host name = '%s'\n",
               context,
               ewsContextNetHandle( context ),
               ewsContextNetHandle( context )->localName));
