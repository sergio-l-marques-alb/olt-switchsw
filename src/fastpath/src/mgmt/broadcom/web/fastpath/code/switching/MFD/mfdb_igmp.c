...
L7_char8 *usmWebMfdbIgmpTableInfoGet(L7_uint32 unit, usmWeb_MfdbEntry_t *entry)
{
  ...
  case L7_MFDB_TYPE_STATIC:
    osapiStrncpy(type, pStrInfo_common_PimSmGrpRpMapStatic,sizeof(type));
    break;
  case L7_MFDB_TYPE_DYNAMIC:
    osapiStrncpy(type, pStrInfo_common_Dyn_1,sizeof(type));
    break;
  default:
    osapiStrncpy(type, pStrInfo_common_Unknown_1,sizeof(type));
    break;
  }
  ...
  for (i=1; i <= numFwd; i++)
  {
    ...
    {
      osapiStrncpy(stat, strUtilUspGet (unit, u, s, p, L7_NULLPTR, 0),sizeof(stat));
