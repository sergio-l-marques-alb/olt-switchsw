...
typedef enum
{
  ...
  L7_SNMP_TRAP_SWITCHING_agentSwitchIpAddressConflictTrap
...
typedef struct
{
  L7_uint32 agentInventoryStackRestartUnit;
  L7_uint32 agentInventoryStackRestartReason;
} L7_SNMP_TRAP_SWITCHING_AgentInventoryStackRestart_t;
...
typedef struct
{
  L7_uint32 conflictingIp;
  L7_uchar8 conflictingMac[L7_ENET_MAC_ADDR_LEN];
} L7_SNMP_TRAP_SWITCHING_IpAddrConflictNotify_t;
...
void SnmpTrapSendCallback_switching(L7_uint32 trap_id, void *data)
{
 ...
 case L7_SNMP_TRAP_BASE_noStartupConfig: 
  ...
  case L7_SNMP_TRAP_SWITCHING_agentSwitchIpAddressConflictTrap:
    (void)snmp_agentSwitchIpAddressConflictTrapSend
          (((L7_SNMP_TRAP_SWITCHING_IpAddrConflictNotify_t*)data)->conflictingIp,
           ((L7_SNMP_TRAP_SWITCHING_IpAddrConflictNotify_t*)data)->conflictingMac);
    break;
...
L7_RC_t SnmpAgentInventoryStackPortLinkUpTrapSend(L7_uint32 agentInventoryStackPortIndex, L7_uint32 agentInventoryStackPortUnit, L7_char8 *agentInventoryStackPortTag)
{
  ...
  memcpy(trap->agentInventoryStackPortTag, agentInventoryStackPortTag, sizeof(trap->agentInventoryStackPortTag));
...
{
  L7_SNMP_TRAP_SWITCHING_IpAddrConflictNotify_t *trap = L7_NULLPTR;

  trap = osapiMalloc(L7_SNMP_COMPONENT_ID,
                     sizeof(L7_SNMP_TRAP_SWITCHING_IpAddrConflictNotify_t));

  if (trap == L7_NULLPTR)
    return L7_ERROR;

  trap->conflictingIp = conflictingIp;
  memcpy(trap->conflictingMac, conflictingMac, L7_ENET_MAC_ADDR_LEN);

  return snmpTrapSend(L7_SNMP_TRAP_SWITCHING_agentSwitchIpAddressConflictTrap,
                      (void*)trap, &SnmpTrapSendCallback_switching);
}
...
/*********************************************************************
* @purpose  Receives a trap request for switching traps
*
...
* @end
*********************************************************************/
L7_RC_t SwitchingTrapSend(L7_TRAP_t *trap)
{
  switch (trap->trapId)
  {
  case TRAP_CPU_RISING_THRESHOLD:
    return snmpSwitchCpuRisingThresholdTrapSend(trap->u.cpuUtil.threshold, trap->u.cpuUtil.buf);
    break;

  case TRAP_CPU_FALLING_THRESHOLD:
    return snmpSwitchCpuFallingThresholdTrapSend(trap->u.cpuUtil.threshold);
    break;
...
void SnmpTrapRegistrationSwitching(void)
{
  ...
  trapMgrFunctionList.sendTrap = SwitchingTrapSend;

  trapMgrFunctionList.registrar_ID = TRAP_CPU_RISING_THRESHOLD;
  trapMgrRegister(&trapMgrFunctionList);

  trapMgrFunctionList.registrar_ID = TRAP_CPU_FALLING_THRESHOLD;
  trapMgrRegister(&trapMgrFunctionList);
}
...
* @param    buf              @b{(input)} buf containing top three tasks
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
L7_RC_t snmpSwitchCpuRisingThresholdTrapSend(L7_uint32 risingThreshold, L7_char8* buf)
{
  ...
  {
    FreeOID(snmpTrapOID);
    if (var_risingThreshold != NULL)
    {
      FreeVarBind(var_risingThreshold);
    }

    if (var_buf == NULL)
    {
      FreeOctetString(os_buf);
    }
    else
    {
      FreeVarBind(var_buf);
  ...
  temp_vb = var_risingThreshold;
  var_risingThreshold->next_var = var_buf;
  var_buf->next_var = NULL;
...
L7_RC_t snmpSwitchCpuFallingThresholdTrapSend(L7_uint32 fallingThreshold)
{
  ...
  OID *snmpTrapOID = MakeOIDFromDot("agentSwitchCpuFallingThresholdTrap");
  OID *oid_fallingThreshold = MakeOIDFromDot("agentSwitchCpuProcessFallingThreshold");
  
  VarBind *var_fallingThreshold = L7_NULLPTR;
  var_fallingThreshold = MakeVarBindWithValue(oid_fallingThreshold, NULL, INTEGER_TYPE, &fallingThreshold);
  
  FreeOID(oid_fallingThreshold);
  
  if (snmpTrapOID == NULL || var_fallingThreshold == NULL)
  {
    FreeOID(snmpTrapOID);
    if (var_fallingThreshold != NULL)
    {
      FreeVarBind(var_fallingThreshold);
    }
    return L7_FAILURE;
  }
  
  temp_vb = var_fallingThreshold;
  var_fallingThreshold->next_var = NULL;
...
L7_RC_t snmpSwitchCpuFreeMemBelowThresholdTrapSend(L7_uint32 freeMemThreshold)
{
  ...
  OID *snmpTrapOID = MakeOIDFromDot("agentSwitchCpuFreeMemBelowThresholdTrap");
  OID *oid_freeMemThreshold = MakeOIDFromDot("agentSwitchCpuProcessFreeMemoryThreshold");
  
  VarBind *var_freeMemThreshold = L7_NULLPTR;
  var_freeMemThreshold = MakeVarBindWithValue(oid_freeMemThreshold, NULL, INTEGER_TYPE, &freeMemThreshold);
  
  FreeOID(oid_freeMemThreshold);
  
  if (snmpTrapOID == NULL || var_freeMemThreshold == NULL)
  {
    FreeOID(snmpTrapOID);
    if (var_freeMemThreshold != NULL)
    {
      FreeVarBind(var_freeMemThreshold);
    }
    return L7_FAILURE;
  }
  
  temp_vb = var_freeMemThreshold;
  var_freeMemThreshold->next_var = NULL;
...
L7_RC_t snmpSwitchCpuFreeMemAboveThresholdTrapSend(L7_uint32 freeMemThreshold)
{
  ...
  OID *snmpTrapOID = MakeOIDFromDot("agentSwitchCpuFreeMemAboveThresholdTrap");
  OID *oid_freeMemThreshold = MakeOIDFromDot("agentSwitchCpuProcessFreeMemoryThreshold");
...
L7_RC_t trapMgrLogemailAlertSendEmailFailureTrapSend(L7_uint32  noEmailFailures)
{
  L7_uint32 status;
  VarBind *temp_vb = NULL;
  OID *snmpTrapOID = MakeOIDFromDot("agentLogEmailSendFailed");
  OID *oid_noOfEmailFailures = MakeOIDFromDot("agentLogEmailStatsemailsFailureCount");
  
  VarBind *var_noOfEmailFailures = L7_NULLPTR;
  var_noOfEmailFailures  = MakeVarBindWithValue(oid_noOfEmailFailures, NULL, INTEGER_TYPE, &noEmailFailures);
  
  FreeOID(oid_noOfEmailFailures);
  
  if (snmpTrapOID == NULL || var_noOfEmailFailures== NULL)
  {
    FreeOID(snmpTrapOID);
    if (var_noOfEmailFailures != NULL)
    {
      FreeVarBind(var_noOfEmailFailures);
    }
    return L7_FAILURE;
  }
  
  temp_vb = var_noOfEmailFailures;
  var_noOfEmailFailures->next_var = NULL;
