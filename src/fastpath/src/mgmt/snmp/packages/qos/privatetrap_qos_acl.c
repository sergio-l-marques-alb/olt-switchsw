...
L7_RC_t snmp_qosAclRuleTimeRangeEventTrapSend(SNMP_QOS_ACL_RULE_TIMERANGE_EVENT_TRAP_t *pTrapData)
{
  ...
  L7_uint32 snmp_aclType;
  L7_uint32 snmp_action;
  L7_uint32 snmp_status;
  L7_uint32 snmp_notification;
  VarBind   *temp_vb = NULL;

  OID *snmpTrapOID = MakeOIDFromDot("aclNotifications.2");

  OID *oid_aclIfAclType = MakeOIDFromDot("aclIfAclType");
  OID *oid_aclIfAclId = MakeOIDFromDot("aclIfAclId");
  OID *oid_aclTrapRuleIndex = MakeOIDFromDot("aclTrapRuleIndex");
  OID *oid_aclTrapRuleAction = MakeOIDFromDot("aclTrapRuleAction");
  OID *oid_aclTrapRuleTimeRangeName = MakeOIDFromDot("aclTrapRuleTimeRangeName");
  OID *oid_aclTrapRuleTimeRangeNotification = MakeOIDFromDot("aclTrapRuleTimeRangeNotification");
  OID *oid_aclTrapRuleInstallationStatus = MakeOIDFromDot("aclTrapRuleInstallationStatus");
 
    /* Freed with the varbind */
  OctetString *timeRangeName = MakeOctetStringFromText(pTrapData->timeRangeName);

  VarBind *var_aclIfAclType;
  VarBind *var_aclIfAclId;
  VarBind *var_aclTrapRuleIndex;
  VarBind *var_aclTrapRuleAction;
  VarBind *var_aclTrapRuleTimeRangeName;
  VarBind *var_aclTrapRuleTimeRangeNotification;
  VarBind *var_aclTrapRuleInstallationStatus;
  ...
  else if(pTrapData->aclType == L7_ACL_TYPE_IPV6) 
  {
    snmp_aclType = D_aclIfAclType_ipv6;
  }
  else
  { 
    snmp_aclType = D_aclIfAclType_mac;
  ...
  {
    snmp_notification = D_aclTrapRuleTimeRangeNotification_activate;
  }
  else if (pTrapData->notification == L7_TIMERANGE_EVENT_DEACTIVATE)
  {
    snmp_notification = D_aclTrapRuleTimeRangeNotification_deactivate;
  }
  else 
  {
    snmp_notification = D_aclTrapRuleTimeRangeNotification_delete;
  }
 
  
   if (pTrapData->status == L7_SUCCESS)
   {
     snmp_status = D_aclTrapRuleInstallationStatus_success;
   }
   else
   {
     snmp_status = D_aclTrapRuleInstallationStatus_failure;
   }
  ...
  var_aclTrapRuleIndex = MakeVarBindWithValue(oid_aclTrapRuleIndex, NULL, INTEGER_32_TYPE, &pTrapData->ruleNum);
  var_aclTrapRuleAction = MakeVarBindWithValue(oid_aclTrapRuleAction, NULL, INTEGER_TYPE, &snmp_action);
  var_aclTrapRuleTimeRangeName = MakeVarBindWithValue(oid_aclTrapRuleTimeRangeName, NULL,  OCTET_PRIM_TYPE, timeRangeName);
  var_aclTrapRuleTimeRangeNotification = MakeVarBindWithValue(oid_aclTrapRuleTimeRangeNotification, NULL, INTEGER_32_TYPE, &snmp_notification);
  var_aclTrapRuleInstallationStatus = MakeVarBindWithValue(oid_aclTrapRuleInstallationStatus, NULL, INTEGER_32_TYPE, &snmp_status);

  FreeOID(oid_aclIfAclType);
  FreeOID(oid_aclIfAclId);
  FreeOID(oid_aclTrapRuleIndex);
  FreeOID(oid_aclTrapRuleAction);
  FreeOID(oid_aclTrapRuleTimeRangeName);
  FreeOID(oid_aclTrapRuleTimeRangeNotification);
  FreeOID(oid_aclTrapRuleInstallationStatus);
    ...
    if (snmpTrapOID == NULL ||
      ...
      var_aclTrapRuleIndex == NULL ||
      var_aclTrapRuleAction == NULL ||
      var_aclTrapRuleTimeRangeName == NULL ||
      var_aclTrapRuleTimeRangeNotification == NULL ||
      var_aclTrapRuleInstallationStatus == NULL)
  {
    FreeOID(snmpTrapOID);
    ...
    FreeVarBind(var_aclTrapRuleIndex);
    FreeVarBind(var_aclTrapRuleAction);
    if(var_aclTrapRuleTimeRangeName  == NULL)
    {
        FreeOctetString(timeRangeName);
    }
    else
    {
        FreeVarBind(var_aclTrapRuleTimeRangeName);
    }
    FreeVarBind(var_aclTrapRuleTimeRangeNotification);
    FreeVarBind(var_aclTrapRuleInstallationStatus);
  ...
  var_aclIfAclId->next_var = var_aclTrapRuleIndex;
  var_aclTrapRuleIndex->next_var = var_aclTrapRuleAction;
  var_aclTrapRuleAction->next_var = var_aclTrapRuleTimeRangeName;  
  var_aclTrapRuleTimeRangeName->next_var = var_aclTrapRuleTimeRangeNotification;  
  var_aclTrapRuleTimeRangeNotification->next_var = var_aclTrapRuleInstallationStatus;
  var_aclTrapRuleInstallationStatus->next_var = NULL;
