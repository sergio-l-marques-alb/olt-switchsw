                   ...
                   SR_INT32 aclRuleIndex)
{
  ...
  if (firstTime == L7_TRUE)
  {
    ...
    aclRuleEntryData.aclRuleTimeRangeName = MakeOctetString(NULL, 0);
#endif
    firstTime = L7_FALSE;
  }
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if ((data->aclRuleTimeRangeName = MakeOctetStringFromText("")) == 0) {
      return RESOURCE_UNAVAILABLE_ERROR;
  }
#endif
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if (VALID(I_aclRuleTimeRangeName, data->valid))
  {
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    memcpy(snmp_buffer, data->aclRuleTimeRangeName->octet_ptr, data->aclRuleTimeRangeName->length);
   
  if ((usmDbComponentPresentCheck(USMDB_UNIT_CURRENT,L7_TIMERANGES_COMPONENT_ID) == L7_TRUE ) &&(usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_PER_RULE_ACTIVATION_ID) == L7_TRUE))
    {
     if((usmDbTimeRangeNameStringCheck(USMDB_UNIT_CURRENT, snmp_buffer) !=  L7_SUCCESS) ||         
     (usmDbQosAclRuleTimeRangeNameAdd(USMDB_UNIT_CURRENT,
                                    data->aclIndex,
                                    data->aclRuleIndex,
     ...
     {
        SET_VALID(I_aclRuleTimeRangeName, tempValid);
     }
    }
  } 
#endif
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
    aclMacRuleEntryData.aclMacRuleTimeRangeName = MakeOctetString(NULL, 0);
#endif
  }
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if ((data->aclMacRuleTimeRangeName = MakeOctetStringFromText("")) == 0) {
      return RESOURCE_UNAVAILABLE_ERROR;
  }
#endif
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if (VALID(I_aclMacRuleTimeRangeName, data->valid))
  {
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    memcpy(snmp_buffer, data->aclMacRuleTimeRangeName->octet_ptr, data->aclMacRuleTimeRangeName->length);

    if ((usmDbComponentPresentCheck(USMDB_UNIT_CURRENT,L7_TIMERANGES_COMPONENT_ID) == L7_TRUE ) &&(usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_PER_RULE_ACTIVATION_ID) == L7_TRUE))
    {
     if((usmDbTimeRangeNameStringCheck(USMDB_UNIT_CURRENT, snmp_buffer)  !=  L7_SUCCESS) ||
     (usmDbQosAclMacRuleTimeRangeNameAdd(USMDB_UNIT_CURRENT,
                                    data->aclMacIndex,
                                    data->aclMacRuleIndex,
                                    snmp_buffer) != L7_SUCCESS))
     {
     ...
     {
        SET_VALID(I_aclMacRuleTimeRangeName, tempValid);
     }
    }
  }
#endif
                      ...
                      int nominator)
{
  ...
  if ((usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_TYPE_MAC_FEATURE_ID) != L7_TRUE) ||
     ...
     ((usmDbComponentPresentCheck(USMDB_UNIT_CURRENT,L7_TIMERANGES_COMPONENT_ID) == L7_TRUE ) 
       &&(usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_PER_RULE_ACTIVATION_ID) == L7_TRUE)))
  {
    return(NULL);
  }
  ...
  switch (nominator)
  {
    ...
    #if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
    case I_aclTrapRuleTimeRangeName:
    case I_aclTrapRuleTimeRangeNotification:
    case I_aclTrapRuleInstallationStatus:
    #endif
      if (nominator != -1) break;
      /* else pass through */
                       ...
                       SR_INT32 aclIpv6RuleIndex)
{
  ...
  if (firstTime == L7_TRUE)
  {
   ...
   #if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
    aclIpv6RuleEntryData.aclIpv6RuleTimeRangeName = MakeOctetString(NULL, 0);
   #endif
    firstTime = L7_FALSE;
  }
...
k_aclIpv6RuleEntry_set_defaults(doList_t *dp)
{
  ...
  data->aclRuleDstIpv6PrefixLength = 0;
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if((data->aclIpv6RuleTimeRangeName = MakeOctetStringFromText("")) == 0) {
        return RESOURCE_UNAVAILABLE_ERROR;
  }
#endif
...
#if defined (L7_TIMERANGES_PACKAGE) && (L7_FEAT_ACL_RULE_TIME_RANGE)
  if (VALID(I_aclIpv6RuleTimeRangeName, data->valid))
  {
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    memcpy(snmp_buffer, data->aclIpv6RuleTimeRangeName->octet_ptr, data->aclIpv6RuleTimeRangeName->length);
    ...
    if ((usmDbComponentPresentCheck(USMDB_UNIT_CURRENT,L7_TIMERANGES_COMPONENT_ID) == L7_TRUE ) &&(usmDbFeaturePresentCheck(USMDB_UNIT_CURRENT, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_PER_RULE_ACTIVATION_ID) == L7_TRUE))
    {
     ...
     {
        SET_VALID(I_aclIpv6RuleTimeRangeName, tempValid);
     }
    }
  }
#endif
