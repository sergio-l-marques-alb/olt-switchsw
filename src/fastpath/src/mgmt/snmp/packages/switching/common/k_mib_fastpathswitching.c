                                 ...
                                 int nominator)
{
  ...
  case I_agentSwitchCpuProcessRisingThreshold:
    if (usmdbCpuUtilMonitorParamGet(USMDB_UNIT_CURRENT, SIM_CPU_UTIL_MONITOR_RISING_THRESHOLD_PARAM,
                                    &agentSwitchCpuProcessGroupData.agentSwitchCpuProcessRisingThreshold) == L7_SUCCESS)
    {
      SET_VALID(I_agentSwitchCpuProcessRisingThreshold, agentSwitchCpuProcessGroupData.valid);
      break;
    }
  case I_agentSwitchCpuProcessRisingThresholdInterval:
    if (usmdbCpuUtilMonitorParamGet(USMDB_UNIT_CURRENT, SIM_CPU_UTIL_MONITOR_RISING_PERIOD_PARAM,
                                    &agentSwitchCpuProcessGroupData.agentSwitchCpuProcessRisingThresholdInterval) == L7_SUCCESS)
    {
      SET_VALID(I_agentSwitchCpuProcessRisingThresholdInterval, agentSwitchCpuProcessGroupData.valid);
      break;
    }
  case I_agentSwitchCpuProcessFallingThreshold:
    if (usmdbCpuUtilMonitorParamGet(USMDB_UNIT_CURRENT, SIM_CPU_UTIL_MONITOR_FALLING_THRESHOLD_PARAM,
                                    &agentSwitchCpuProcessGroupData.agentSwitchCpuProcessFallingThreshold) == L7_SUCCESS)
    {
      SET_VALID(I_agentSwitchCpuProcessFallingThreshold, agentSwitchCpuProcessGroupData.valid);
      break;
    }
  case I_agentSwitchCpuProcessFallingThresholdInterval:
    if (usmdbCpuUtilMonitorParamGet(USMDB_UNIT_CURRENT, SIM_CPU_UTIL_MONITOR_FALLING_PERIOD_PARAM,
                                    &agentSwitchCpuProcessGroupData.agentSwitchCpuProcessFallingThresholdInterval) == L7_SUCCESS)
    {
      SET_VALID(I_agentSwitchCpuProcessFallingThresholdInterval, agentSwitchCpuProcessGroupData.valid);
      break;
    }
  case I_agentSwitchCpuProcessFreeMemoryThreshold:
    if (usmdbCpuFreeMemoryThresholdGet(USMDB_UNIT_CURRENT,
                                       &agentSwitchCpuProcessGroupData.agentSwitchCpuProcessFreeMemoryThreshold) == L7_SUCCESS)
    {
      SET_VALID(I_agentSwitchCpuProcessFreeMemoryThreshold, agentSwitchCpuProcessGroupData.valid);
      break;
    }

#ifdef I_agentSwitchCpuProcessTotalUtilization
  case I_agentSwitchCpuProcessTotalUtilization:
       CLR_VALID(I_agentSwitchCpuProcessTotalUtilization, agentSwitchCpuProcessGroupData.valid);
       if (usmDbTotalCpuUtilGet(USMDB_UNIT_CURRENT,
           snmp_buffer, sizeof(snmp_buffer)) == L7_SUCCESS)
       {
         if (SafeMakeOctetStringFromTextExact(&agentSwitchCpuProcessGroupData.agentSwitchCpuProcessTotalUtilization, snmp_buffer) == L7_TRUE)
         {  
           SET_VALID(I_agentSwitchCpuProcessTotalUtilization, agentSwitchCpuProcessGroupData.valid);
         }
       }
       break;
#endif /* I_agentSwitchCpuProcessTotalUtilization */
...
#ifdef SETS
int
k_agentSwitchCpuProcessGroup_test(ObjectInfo *object, ObjectSyntax *value,
...
int
k_agentSwitchCpuProcessGroup_ready(ObjectInfo *object, ObjectSyntax *value, 
...
int
k_agentSwitchCpuProcessGroup_set(agentSwitchCpuProcessGroup_t *data,
                                 ContextInfo *contextInfo, int function)
{
  ...
  if (VALID(I_agentSwitchCpuProcessRisingThreshold, data->valid))
  {
    if (snmpCpuProcessRisingThresholdSet(USMDB_UNIT_CURRENT, data->agentSwitchCpuProcessRisingThreshold) != L7_SUCCESS)
    {
    ...
    {
      SET_VALID(I_agentSwitchCpuProcessRisingThreshold, tempValid);
    }
  }

  if (VALID(I_agentSwitchCpuProcessRisingThresholdInterval, data->valid))
  {
    if (snmpCpuProcessRisingThresholdIntervalSet(USMDB_UNIT_CURRENT, data->agentSwitchCpuProcessRisingThresholdInterval) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    ...
    {
      SET_VALID(I_agentSwitchCpuProcessRisingThresholdInterval, tempValid);
    }
  }

  if (VALID(I_agentSwitchCpuProcessFallingThreshold, data->valid))
  {
    if (snmpCpuProcessFallingThresholdSet(USMDB_UNIT_CURRENT, data->agentSwitchCpuProcessFallingThreshold) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    ...
    {
      SET_VALID(I_agentSwitchCpuProcessFallingThreshold, tempValid);
    }
  }

  if (VALID(I_agentSwitchCpuProcessFallingThresholdInterval, data->valid))
  {
    if (snmpCpuProcessFallingThresholdIntervalSet(USMDB_UNIT_CURRENT, data->agentSwitchCpuProcessFallingThresholdInterval) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    ...
    {
      SET_VALID(I_agentSwitchCpuProcessFallingThresholdInterval, tempValid);
    }
  }

  if (VALID(I_agentSwitchCpuProcessFreeMemoryThreshold, data->valid))
  {
    if ((data->agentSwitchCpuProcessFreeMemoryThreshold < 0) ||
        (usmdbCpuFreeMemoryThresholdSet(USMDB_UNIT_CURRENT, data->agentSwitchCpuProcessFreeMemoryThreshold) != L7_SUCCESS))
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    ...
    {
      SET_VALID(I_agentSwitchCpuProcessFreeMemoryThreshold, tempValid);
    }
  }
...
#ifdef SR_agentSwitchCpuProcessGroup_UNDO
/* add #define SR_agentSwitchCpuProcessGroup_UNDO in sitedefs.h to
 * include the undo routine for the agentSwitchCpuProcessGroup family.
 */
int
agentSwitchCpuProcessGroup_undo(doList_t *doHead, doList_t *doCur,
                                ContextInfo *contextInfo)
{
  agentSwitchCpuProcessGroup_t *data = (agentSwitchCpuProcessGroup_t *) doCur->data;
  agentSwitchCpuProcessGroup_t *undodata = (agentSwitchCpuProcessGroup_t *) doCur->undodata;
  agentSwitchCpuProcessGroup_t *setdata = NULL;
  ...
  if ((setdata != NULL) && (k_agentSwitchCpuProcessGroup_set(setdata, contextInfo, function) == NO_ERROR))
    return NO_ERROR;

   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentSwitchCpuProcessGroup_UNDO */
#endif /* SETS */
                                 ...
                                 SR_INT32 agentSwitchCpuProcessIndex)
{
  ...
  if ( (searchType == EXACT) )
  {
      if ( snmpAgentSwitchCpuProcessEntryGet(USMDB_UNIT_CURRENT, agentSwitchCpuProcessEntryData.agentSwitchCpuProcessIndex, 
           snmp_buffer, sizeof(snmp_buffer)) != L7_SUCCESS )
      {
          ZERO_VALID(agentSwitchCpuProcessEntryData.valid);
          return(NULL);
  ...
  else
  {
      if ( snmpAgentSwitchCpuProcessEntryGet(USMDB_UNIT_CURRENT, agentSwitchCpuProcessEntryData.agentSwitchCpuProcessIndex, 
           snmp_buffer, sizeof(snmp_buffer)) != L7_SUCCESS )
      {
          if(snmpAgentSwitchCpuProcessEntryNextGet(USMDB_UNIT_CURRENT, &agentSwitchCpuProcessEntryData.agentSwitchCpuProcessIndex, 
             snmp_buffer, sizeof(snmp_buffer)) != L7_SUCCESS )
          {
              ZERO_VALID(agentSwitchCpuProcessEntryData.valid);
              return(NULL);
...
#ifdef I_agentSwitchCpuProcessId
  case I_agentSwitchCpuProcessId:
      if (snmpAgentSwitchCpuProcessEntryGet(USMDB_UNIT_CURRENT, agentSwitchCpuProcessEntryData.agentSwitchCpuProcessIndex,
           snmp_buffer, sizeof(snmp_buffer)) == L7_SUCCESS )
      {
        if (SafeMakeOctetStringFromTextExact(&agentSwitchCpuProcessEntryData.agentSwitchCpuProcessId, snmp_buffer) == L7_TRUE)
        SET_VALID(I_agentSwitchCpuProcessId, agentSwitchCpuProcessEntryData.valid);
      }
    break;
#endif /* I_agentSwitchCpuProcessIn */
  ...
  case I_agentLoginSessionInetAddress :
                                         ...
                                         &gaddr) == L7_SUCCESS)
    {
      if(gaddr.family == L7_AF_INET)
      {
        ...
        if (SafeMakeOctetString(&agentLoginSessionEntryData.agentLoginSessionInetAddress,
                                (L7_uchar8*)&gaddr.addr.ipv4.s_addr, sizeof(gaddr.addr.ipv4.s_addr)) == L7_TRUE)
      ...
      else if (gaddr.family == L7_AF_INET6)
      {
        ...
        if (SafeMakeOctetString(&agentLoginSessionEntryData.agentLoginSessionInetAddress, 
                                gaddr.addr.ipv6.in6.addr8, sizeof(gaddr.addr.ipv6.in6.addr8)) == L7_TRUE)
         SET_VALID(I_agentLoginSessionInetAddress, agentLoginSessionEntryData.valid);
      }
      else
      {
        agentLoginSessionEntryData.agentLoginSessionInetAddressType = D_agentLoginSessionInetAddressType_unknown;
        if (SafeMakeOctetString(&agentLoginSessionEntryData.agentLoginSessionInetAddress, NULL, 0) == L7_TRUE)
         SET_VALID(I_agentLoginSessionInetAddress, agentLoginSessionEntryData.valid);
      }
    }
    else
                             ...
                             int nominator)
{
  ...
  L7_uint8 val8 = 0;
  L7_BOOL  enable;
  L7_char8 result[L7_CLI_MAX_STRING_LENGTH]={0};
  ...
  case I_agentPasswordManagementPasswordStrengthCheck:
    CLR_VALID(I_agentPasswordManagementPasswordStrengthCheck, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswordStrengthCheckConfigGet(&enable) == L7_SUCCESS)
    {
      ...
      agentPasswordManagementConfigGroupData.agentPasswordManagementPasswordStrengthCheck = enable;
      SET_VALID(I_agentPasswordManagementPasswordStrengthCheck, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMinUpperCase:
    CLR_VALID(I_agentPasswordManagementStrengthMinUpperCase, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMinUppercaseCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMinUpperCase = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMinUpperCase, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMinLowerCase:
    CLR_VALID(I_agentPasswordManagementStrengthMinLowerCase, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMinLowercaseCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMinLowerCase = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMinLowerCase, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMinNumericNumbers:
    CLR_VALID(I_agentPasswordManagementStrengthMinNumericNumbers, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMinNumericCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMinNumericNumbers = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMinNumericNumbers, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMinSpecialCharacters:
    CLR_VALID(I_agentPasswordManagementStrengthMinSpecialCharacters, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMinSpecialCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMinSpecialCharacters = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMinSpecialCharacters, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMaxConsecutiveCharacters:
    CLR_VALID(I_agentPasswordManagementStrengthMaxConsecutiveCharacters, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMaxConsecutiveCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMaxConsecutiveCharacters = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMaxConsecutiveCharacters, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMaxRepeatedCharacters:
    CLR_VALID(I_agentPasswordManagementStrengthMaxRepeatedCharacters, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMaxRepeatedCharsLengthGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMaxRepeatedCharacters = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMaxRepeatedCharacters, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthMinCharacterClasses:
    CLR_VALID(I_agentPasswordManagementStrengthMinCharacterClasses, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdMinCharacterClassesGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthMinCharacterClasses = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthMinCharacterClasses, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordManagementStrengthScore:
    CLR_VALID(I_agentPasswordManagementStrengthScore, agentPasswordManagementConfigGroupData.valid);
    if (usmDbUserMgrPasswdStrengthScoreGet(&val8) == L7_SUCCESS)
    {
      agentPasswordManagementConfigGroupData.agentPasswordManagementStrengthScore = (L7_uint32)val8;
      SET_VALID(I_agentPasswordManagementStrengthScore, agentPasswordManagementConfigGroupData.valid);
    }
    break;

  case I_agentPasswordMgmtLastPasswordSetResult:
    CLR_VALID(I_agentPasswordMgmtLastPasswordSetResult, agentPasswordManagementConfigGroupData.valid);
    if ((usmDbUserMgrPasswordLastSetResultStringGet(result) == L7_SUCCESS) &&
        (SafeMakeOctetStringFromTextExact(&agentPasswordManagementConfigGroupData.agentPasswordMgmtLastPasswordSetResult, result) == L7_TRUE))
    {
      SET_VALID(I_agentPasswordMgmtLastPasswordSetResult, agentPasswordManagementConfigGroupData.valid);
    }
    break;
                             ...
                             ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if (VALID(I_agentPasswordManagementPasswordStrengthCheck, data->valid))
  {
    enable = data->agentPasswordManagementPasswordStrengthCheck;
    if(enable == L7_DIAG_DISABLE)
    {
      enable = L7_DISABLE;
    }
    else if(enable != L7_ENABLE)
    {
      return COMMIT_FAILED_ERROR;  
    }
    ...
    SET_VALID(I_agentPasswordManagementPasswordStrengthCheck, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMinUpperCase, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMinUpperCase;
    if(usmDbUserMgrPasswdMinUppercaseCharsLengthSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMinUpperCase, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMinLowerCase, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMinLowerCase;
    if(usmDbUserMgrPasswdMinLowercaseCharsLengthSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMinLowerCase, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMinNumericNumbers, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMinNumericNumbers;
    if(usmDbUserMgrPasswdMinNumericCharsLengthSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMinNumericNumbers, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMinSpecialCharacters, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMinSpecialCharacters;
  ...
  if (VALID(I_agentPasswordManagementStrengthMinCharacterClasses, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMinCharacterClasses;
    if(usmDbUserMgrPasswdMinCharacterClassesSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMinCharacterClasses, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMaxConsecutiveCharacters, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMaxConsecutiveCharacters;
    if(usmDbUserMgrPasswdMaxConsecutiveCharsLengthSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMaxConsecutiveCharacters, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthMaxRepeatedCharacters, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthMaxRepeatedCharacters;
    if(usmDbUserMgrPasswdMaxRepeatedCharsLengthSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthMaxRepeatedCharacters, tempValid);
  }
  if (VALID(I_agentPasswordManagementStrengthScore, data->valid))
  {
    val8 = (L7_uint8)data->agentPasswordManagementStrengthScore;
    if(usmDbUserMgrPasswdStrengthScoreSet(val8) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
    SET_VALID(I_agentPasswordManagementStrengthScore, tempValid);
  }

  return NO_ERROR;
}

/*** Password Management Exclude Keywords Start ***/

agentPasswordManagementStrengthExcludeKeywordEntry_t *
k_agentPasswordManagementStrengthExcludeKeywordEntry_get(int serialNum, ContextInfo *contextInfo,
                                                         int nominator,
                                                         int searchType,
                                                         OctetString * agentPasswordMgmtStrengthExcludeKeyword)
{
  static agentPasswordManagementStrengthExcludeKeywordEntry_t agentPasswordManagementStrengthExcludeKeywordEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 keyword[L7_PASSWD_EXCLUDE_KEYWORD_MAX_LEN]={0};
  L7_uint32 keywordStatus = D_agentPasswordMgmtStrengthExcludeKeywordStatus_notInService;
  L7_uint32 len;

  if (firstTime == L7_TRUE)
  {
    firstTime = L7_FALSE;
    agentPasswordManagementStrengthExcludeKeywordEntryData.agentPasswordMgmtStrengthExcludeKeyword = MakeOctetString(NULL, 0);
    agentPasswordManagementStrengthExcludeKeywordEntryData.agentPasswordMgmtStrengthExcludeKeywordStatus = L7_NULL;
  }

  ZERO_VALID(agentPasswordManagementStrengthExcludeKeywordEntryData.valid);

  memset(keyword, 0x00, sizeof(keyword));
  memcpy(keyword, agentPasswordMgmtStrengthExcludeKeyword->octet_ptr, agentPasswordMgmtStrengthExcludeKeyword->length);

  if(searchType == EXACT)
  {
    if(agentPasswordMgmtStrengthExcludeKeyword->length > sizeof(keyword))
    {
      ZERO_VALID(agentPasswordManagementStrengthExcludeKeywordEntryData.valid);
      return(NULL);
    }
  ...
  {
    if(snmpAgentUserMgrPasswdExcludeKeywordNextGet(keyword, L7_PASSWD_EXCLUDE_KEYWORD_MAX_LEN) != L7_SUCCESS)
    {
      ZERO_VALID(agentPasswordManagementStrengthExcludeKeywordEntryData.valid);
      return(NULL);
    }
  }
  keywordStatus = D_agentPasswordMgmtStrengthExcludeKeywordStatus_active;
  
  len = osapiStrnlen(keyword, sizeof(keyword));
  if(SafeMakeOctetString(&agentPasswordManagementStrengthExcludeKeywordEntryData.agentPasswordMgmtStrengthExcludeKeyword, keyword, len) != L7_TRUE)
  {
    ZERO_VALID(agentPasswordManagementStrengthExcludeKeywordEntryData.valid);
    return(NULL);
  }

  SET_VALID(I_agentPasswordMgmtStrengthExcludeKeyword, agentPasswordManagementStrengthExcludeKeywordEntryData.valid);
  ...
  {
    case -1:
    case I_agentPasswordMgmtStrengthExcludeKeyword:
       if ( nominator != -1 ) break;
      /* else pass through */
    case I_agentPasswordMgmtStrengthExcludeKeywordStatus:
        agentPasswordManagementStrengthExcludeKeywordEntryData.agentPasswordMgmtStrengthExcludeKeywordStatus = keywordStatus;
        SET_VALID(I_agentPasswordMgmtStrengthExcludeKeywordStatus, agentPasswordManagementStrengthExcludeKeywordEntryData.valid);
        break;
  ...
  if ( nominator >= 0 && !VALID(nominator, agentPasswordManagementStrengthExcludeKeywordEntryData.valid) )
    return(NULL);

  return(&agentPasswordManagementStrengthExcludeKeywordEntryData);
}

int
k_agentPasswordManagementStrengthExcludeKeywordEntry_set(agentPasswordManagementStrengthExcludeKeywordEntry_t *data,
                                                         ContextInfo *contextInfo, int function)
{
   L7_uchar8 snmp_buffer[SNMP_BUFFER_LEN]={0};
   
   if( VALID(I_agentPasswordMgmtStrengthExcludeKeywordStatus, data->valid))
   {
     if(data->agentPasswordMgmtStrengthExcludeKeywordStatus == D_agentPasswordMgmtStrengthExcludeKeywordStatus_createAndGo)
     {
       if(VALID(I_agentPasswordMgmtStrengthExcludeKeyword, data->valid)) 
       {
         bzero(snmp_buffer, SNMP_BUFFER_LEN);
         memcpy(snmp_buffer, data->agentPasswordMgmtStrengthExcludeKeyword->octet_ptr, 
                                       data->agentPasswordMgmtStrengthExcludeKeyword->length);
         /* set the value in application */
         if(usmDbUserMgrPasswdExcludeKeywordSet(snmp_buffer) != L7_SUCCESS)
         {
           return(COMMIT_FAILED_ERROR);
         }
       }
     }
     else if(data->agentPasswordMgmtStrengthExcludeKeywordStatus == D_agentPasswordMgmtStrengthExcludeKeywordStatus_destroy)
     {
       bzero(snmp_buffer, SNMP_BUFFER_LEN);
       memcpy(snmp_buffer, data->agentPasswordMgmtStrengthExcludeKeyword->octet_ptr, 
                               data->agentPasswordMgmtStrengthExcludeKeyword->length);
       if(usmDbUserMgrPasswdExcludeKeywordDelete(snmp_buffer)!=L7_SUCCESS)
         return COMMIT_FAILED_ERROR;
     }
     else if(data->agentPasswordMgmtStrengthExcludeKeywordStatus == D_agentPasswordMgmtStrengthExcludeKeywordStatus_active)
     {
       return COMMIT_FAILED_ERROR;
     }
   }
...
#ifdef SETS
int
k_agentPasswordManagementStrengthExcludeKeywordEntry_test(ObjectInfo *object, ObjectSyntax *value,
                                                          doList_t *dp, ContextInfo *contextInfo)
{
#ifdef NOT_YET
    agentPasswordManagementStrengthExcludeKeywordEntry_t *data = (agentPasswordManagementStrengthExcludeKeywordEntry_t *) dp->data;
    ...
    if (object->nominator == I_agentPasswordMgmtStrengthExcludeKeywordStatus) {
        /*
         * Examine value->sl_value.  In case of error,
         * return from the function here with an error code.
         */
        SET_VALID(I_agentPasswordMgmtStrengthExcludeKeywordStatus, data->mod);
    }
#endif /* NOT_YET */
    return NO_ERROR;
}
...
int
k_agentPasswordManagementStrengthExcludeKeywordEntry_ready(ObjectInfo *object, ObjectSyntax *value, 
                                                           doList_t *doHead, doList_t *dp)
{
#ifdef NOT_YET
    agentPasswordManagementStrengthExcludeKeywordEntry_t *data = (agentPasswordManagementStrengthExcludeKeywordEntry_t *) dp->data;
    ...
    if (VALID(I_agentPasswordMgmtStrengthExcludeKeywordStatus, data->valid)) {
        /*
         * Examine data->agentPasswordMgmtStrengthExcludeKeywordStatus in relation to other
...
int
k_agentPasswordManagementStrengthExcludeKeywordEntry_set_defaults(doList_t *dp)
{
    agentPasswordManagementStrengthExcludeKeywordEntry_t *data = (agentPasswordManagementStrengthExcludeKeywordEntry_t *) (dp->data);
...
#ifdef SR_agentPasswordManagementStrengthExcludeKeywordEntry_UNDO
/* add #define SR_agentPasswordManagementStrengthExcludeKeywordEntry_UNDO in sitedefs.h to
 * include the undo routine for the agentPasswordManagementStrengthExcludeKeywordEntry family.
 */
int
agentPasswordManagementStrengthExcludeKeywordEntry_undo(doList_t *doHead, doList_t *doCur,
                                                        ContextInfo *contextInfo)
{
   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentPasswordManagementStrengthExcludeKeywordEntry_UNDO */

#endif /* SETS */



/*** Password Management Exclude Keywords End ***/
                           ...
                           ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if (VALID(I_agentUserConfigCreate, data->valid))
  {
    ...
    if (strlen(snmp_buffer) != L7_NULL)
    {
      osapiSnprintf(auditMsg, sizeof(auditMsg), pStrInfo_base_Auditing_NewUser, snmp_buffer);
                           ...
                           ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if ( VALID(I_agentUserStatus, data->valid))
  {
    ...
    {
      if (usmDbLoginsGet(USMDB_UNIT_CURRENT, 
                         data->agentUserIndex, snmp_buffer) != L7_SUCCESS)
      {
        osapiSnprintf(snmp_buffer, sizeof(snmp_buffer), "<<UNKNOWN>>");
      }

      osapiSnprintf(auditMsg, sizeof(auditMsg), 
                    pStrInfo_base_Auditing_RemoveUser, snmp_buffer);
      SnmpCmdLoggerAuditEntryAdd(contextInfo, auditMsg);
    }
  }
                                 ...
                                 SR_INT32 agentLagSummaryLagIndex)
{
  ...
  if ( usmDbIntIfNumFromExtIfNum(agentLagSummaryConfigEntryData.agentLagSummaryLagIndex, &intIfNum)!= L7_SUCCESS)
  {
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SNMP_COMPONENT_ID,"Failed to convert internal interface number");
     return (NULL);
  }
...
#ifdef I_agentLagSummaryAccessVlanID
  case I_agentLagSummaryAccessVlanID :
   {
    L7_uint32 tempVal;

    if (usmDbDot1qSwPortModeGet(USMDB_UNIT_CURRENT, intIfNum, &tempVal) == L7_SUCCESS)   
    {
      if (tempVal != DOT1Q_SWPORT_MODE_ACCESS)
      {
        agentLagSummaryConfigEntryData.agentLagSummaryAccessVlanID = L7_NULL;
        SET_VALID(I_agentLagSummaryAccessVlanID, agentLagSummaryConfigEntryData.valid);
      }
      else if (usmDbDot1qSwPortAccessVlanGet(USMDB_UNIT_CURRENT, intIfNum,
                                             &agentLagSummaryConfigEntryData.agentLagSummaryAccessVlanID) == L7_SUCCESS)
      {
        SET_VALID(I_agentLagSummaryAccessVlanID, agentLagSummaryConfigEntryData.valid);
      }
    }
                                 ...
                                 ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  /* Conversion to internal interface number for usmDb calls */
 if ( usmDbIntIfNumFromExtIfNum(data->agentLagSummaryLagIndex, &intIfNum) != L7_SUCCESS)
 {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SNMP_COMPONENT_ID,"Failed to convert internal interface number");
    return COMMIT_FAILED_ERROR;
 }
...
#ifdef I_agentLagSummaryAccessVlanID
  if (VALID(I_agentLagSummaryAccessVlanID, data->valid))
  {
    if(usmDbDot1qSwPortAccessVlanSet(USMDB_UNIT_CURRENT, intIfNum,
                                              data->agentLagSummaryAccessVlanID) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
...
/* lvl7_@p1261 start */
  ...
  case I_agentNetworkConfigProtocolDhcpRenew :
    agentNetworkConfigGroupData.agentNetworkConfigProtocolDhcpRenew = D_agentNetworkConfigProtocolDhcpRenew_renew;
      SET_VALID(I_agentNetworkConfigProtocolDhcpRenew, agentNetworkConfigGroupData.valid);
    if (nominator != -1) break;
                              ...
                              ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if ( VALID(I_agentNetworkConfigProtocolDhcpRenew, data->valid))
  {
    if(usmDbAgentBasicConfigNetworkConfigProtocolDesiredGet(USMDB_UNIT_CURRENT,
                                                            &oldval)
                                                            == L7_SUCCESS )
    {
      if(oldval == L7_SYSCONFIG_MODE_DHCP)
      {
        if(usmDbIntfIpAddressMethodSet (0, L7_INTF_IP_ADDR_RENEW,
                                        L7_MGMT_NETWORKPORT, L7_FALSE)
                                        != L7_SUCCESS )
        {
          CLR_VALID(I_agentNetworkConfigProtocolDhcpRenew, data->valid);
          return(COMMIT_FAILED_ERROR);
        }
      }
    }
...
/* lvl7_@p1261 start */
  ...
  case I_agentServicePortProtocolDhcpRenew :
    agentServicePortConfigGroupData.agentServicePortProtocolDhcpRenew = D_agentServicePortProtocolDhcpRenew_renew;
      SET_VALID(I_agentServicePortProtocolDhcpRenew, agentServicePortConfigGroupData.valid);
    if (nominator != -1)
    break;
  ...
  */
  ...
  if ( VALID(I_agentServicePortConfigProtocol, data->valid))
  {
    if(snmpAgentServicePortConfigProtocolSet(USMDB_UNIT_CURRENT,
                                             data->agentServicePortConfigProtocol)
                                             != L7_SUCCESS )
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return(COMMIT_FAILED_ERROR);
  ...
  if ( VALID(I_agentServicePortProtocolDhcpRenew, data->valid))
  {
    if(usmDbAgentBasicConfigServPortConfigProtocolDesiredGet(USMDB_UNIT_CURRENT,
                                                             &oldval) == L7_SUCCESS )
    {
      if(oldval == L7_SYSCONFIG_MODE_DHCP)
      {
                                       ...
                                       SR_INT32 agentSnmpTrapReceiverIndex)
{
  ...
  case I_agentSnmpTrapReceiverAddressType :
     if (snmpAgentSnmpTrapReceiverAddressTypeGet(USMDB_UNIT_CURRENT, agentSnmpTrapReceiverConfigEntryData.agentSnmpTrapReceiverIndex,
                                           &agentSnmpTrapReceiverConfigEntryData.agentSnmpTrapReceiverAddressType) == L7_SUCCESS)
      SET_VALID(I_agentSnmpTrapReceiverAddressType, agentSnmpTrapReceiverConfigEntryData.valid);
    if ( nominator != -1 ) break;
    /* else pass through */

  case I_agentSnmpTrapReceiverAddress :
     bzero(snmp_buffer, SNMP_BUFFER_LEN);
     if (snmpAgentSnmpTrapReceiverAddressGet(USMDB_UNIT_CURRENT, agentSnmpTrapReceiverConfigEntryData.agentSnmpTrapReceiverIndex, snmp_buffer) == L7_SUCCESS &&
        (SafeMakeOctetStringFromTextExact(&agentSnmpTrapReceiverConfigEntryData.agentSnmpTrapReceiverAddress, snmp_buffer) == L7_TRUE))
      SET_VALID(I_agentSnmpTrapReceiverAddress, agentSnmpTrapReceiverConfigEntryData.valid);
     if ( nominator != -1 ) break;
     /* else pass through */
                                       ...
                                       ContextInfo *contextInfo, int function)
{
 ...
 if ( VALID(I_agentSnmpTrapReceiverAddressType, data->valid))
  {
    if( snmpAgentSnmpTrapReceiverAddressTypeSet(USMDB_UNIT_CURRENT,
                                    data->agentSnmpTrapReceiverIndex,
                                    data->agentSnmpTrapReceiverAddressType) != L7_SUCCESS )
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return(COMMIT_FAILED_ERROR);
    }
    ...
    {
      SET_VALID(I_agentSnmpTrapReceiverAddressType, tempValid);
    }
  }
 
  if ( VALID(I_agentSnmpTrapReceiverAddress, data->valid))
  {
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    memcpy(snmp_buffer, data->agentSnmpTrapReceiverAddress->octet_ptr, data->agentSnmpTrapReceiverAddress->length);
    if( snmpAgentSnmpTrapReceiverAddressSet(USMDB_UNIT_CURRENT,
                                    data->agentSnmpTrapReceiverIndex,
                                    snmp_buffer) != L7_SUCCESS )
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
    ...
    {
      SET_VALID(I_agentSnmpTrapReceiverAddress, tempValid);
    }
  }
                                   ...
                                   int nominator)
{
  ...
  case I_agentSpanningTreeMode:
    if (snmpAgentSpanningTreeModeGet(USMDB_UNIT_CURRENT, &agentSpanningTreeConfigGroupData.agentSpanningTreeMode) == L7_SUCCESS)
    {
      SET_VALID(I_agentSpanningTreeMode, agentSpanningTreeConfigGroupData.valid);
    }
    break;
...
k_agentSpanningTreeConfigGroup_set(agentSpanningTreeConfigGroup_t *data,
                                   ContextInfo *contextInfo, int function)
{
  if (VALID(I_agentSpanningTreeMode, data->valid))
  {
    if( snmpAgentSpanningTreeModeSet(USMDB_UNIT_CURRENT, data->agentSpanningTreeMode) != L7_SUCCESS)
    {
    ...
    {
      SET_VALID(I_agentSpanningTreeMode, data->valid);
    }
  }
                      ...
                      ContextInfo *contextInfo)
{
  ...
  if (data == NULL)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SNMP_COMPONENT_ID,"Failed to Undo data was NULL");
    return UNDO_FAILED_ERROR;
  }
                           ...
                           SR_UINT32 dot1qVlanIndex)
{
  ...
  if ((searchType == EXACT) ?
      usmDbDot1sInstanceVlanGet(USMDB_UNIT_CURRENT, agentStpMstVlanEntryData.agentStpMstId, agentStpMstVlanEntryData.dot1qVlanIndex) != L7_SUCCESS :
      (usmDbDot1sInstanceVlanGet(USMDB_UNIT_CURRENT, agentStpMstVlanEntryData.agentStpMstId, agentStpMstVlanEntryData.dot1qVlanIndex) != L7_SUCCESS &&
       snmpDot1sInstanceVlanNextGet(USMDB_UNIT_CURRENT, agentStpMstVlanEntryData.agentStpMstId, agentStpMstVlanEntryData.dot1qVlanIndex,
                                    &agentStpMstVlanEntryData.agentStpMstId, &agentStpMstVlanEntryData.dot1qVlanIndex) != L7_SUCCESS))
  {
    ZERO_VALID(agentStpMstVlanEntryData.valid);
    return(NULL);
  }
                           ...
                           ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  L7_VLAN_SETMASKBIT(vlan_mask, data->dot1qVlanIndex);
  ...
  if (usmDbDot1sMstiVIDGet(USMDB_UNIT_CURRENT, data->agentStpMstId, data->dot1qVlanIndex) != L7_SUCCESS)
  {
    ...
    { 
      if( data->agentStpMstVlanRowStatus == D_agentStpMstVlanRowStatus_createAndGo &&
        usmDbDot1sMstiVlanAdd(USMDB_UNIT_CURRENT, data->agentStpMstId, &vlan_mask) != L7_SUCCESS)
      {
        /* don't set the status */
        memcpy(data->valid, tempValid, sizeof(data->valid));
        return COMMIT_FAILED_ERROR;
  ...
  if ((data->agentStpMstVlanRowStatus == D_agentStpMstVlanRowStatus_destroy) &&
      (VALID(I_agentStpMstVlanRowStatus, data->valid)))
  {
    if (usmDbDot1sInstanceVlanGet(USMDB_UNIT_CURRENT, data->agentStpMstId, data->dot1qVlanIndex) == L7_SUCCESS)
    {
      if ((data->agentStpMstVlanRowStatus != D_agentStpMstVlanRowStatus_active) &&
          (usmDbDot1sMstiVlanRemove(USMDB_UNIT_CURRENT, data->agentStpMstId, &vlan_mask) != L7_SUCCESS))
      {
        memcpy(data->valid, tempValid, sizeof(data->valid));
        return COMMIT_FAILED_ERROR;
      }
                                 ...
                                 SR_INT32 family)
{
  ...
  case I_agentSwitchSnoopingPortMask:
    ...
    if (usmDbSnoopIntfEnabledMaskGet(USMDB_UNIT_CURRENT, snmp_buffer, &snmp_buffer_len,
        ...
        SafeMakeOctetString(&agentSwitchSnoopingGroupData.agentSwitchSnoopingPortMask, snmp_buffer, snmp_buffer_len) != L7_FALSE)
                                       ...
                                       SR_INT32 family)
{
  ...
  if ( usmDbIntIfNumFromExtIfNum(agentSwitchSnoopingIntfEntryData.ifIndex, &intIfNum) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SNMP_COMPONENT_ID,"Failed to convert to internal interface");
    return (NULL);
  }
                                            ...
                                            ContextInfo *contextInfo, int function)
{
  ...
  if (VALID(I_agentSwitchVlanStaticMrouterAdminMode, data->valid) && 
      VALID(I_agentSwitchSnoopingProtocol, data->valid))
  {
    if(snmpAgentSwitchVlanStaticMrouterAdminModeSet(USMDB_UNIT_CURRENT, intIfIndex, data->dot1qVlanIndex,
                       data->agentSwitchVlanStaticMrouterAdminMode,
                       data->agentSwitchSnoopingProtocol) != L7_SUCCESS)
      {
                                          ...
                                          SR_INT32 agentSwitchSnoopingProtocol)
{
  ...
  switch (nominator)
  {
    ...
    case I_agentSwitchSnoopingQuerierOperMaxResponseTime:
      if (snmpAgentSwitchSnoopingQuerierOperMaxResponseTimeGet(vlanId,
    ...
    case I_agentSwitchSnoopingQuerierLastQuerierAddress:
      ...
      if ((snmpAgentSwitchSnoopingQuerierLastQuerierAddressGet(vlanId, snmp_buffer, family) == L7_SUCCESS)
          && (SafeMakeOctetString(&agentSwitchSnoopingQuerierVlanEntryData.agentSwitchSnoopingQuerierLastQuerierAddress,
                                  snmp_buffer, (family == L7_AF_INET)? L7_IP_ADDR_LEN : L7_IP6_ADDR_LEN) == L7_TRUE))
      {
    ...
    case I_agentSwitchSnoopingQuerierLastQuerierVersion:
      if (snmpAgentSwitchSnoopingQuerierLastQuerierVersionGet(vlanId,
                                      ...
                                      OctetString * agentSwitchVoiceVlanDeviceMacAddress)
{
  ...
  {
    firstCall = L7_FALSE;
    if (SafeMakeOctetString(&agentSwitchVoiceVlanDeviceEntryData.agentSwitchVoiceVlanDeviceMacAddress, NULL, 0) != L7_TRUE)
    {
                           ...
                           OctetString * agentArpAclRuleMatchSenderMacAddr)
{
  ...
  if((searchType == EXACT) ?
     (usmDbArpAclRuleGet(name_buffer, agentArpAclRuleMatchSenderIpAddr, &macAddr) != L7_SUCCESS) :
     ((usmDbArpAclRuleGet(name_buffer, agentArpAclRuleMatchSenderIpAddr, &macAddr) != L7_SUCCESS) &&
      (usmDbArpAclRuleNextGet(name_buffer, agentArpAclRuleMatchSenderIpAddr, &macAddr,
                              name_buffer,
...
/* IP address conflict detection feature APIs - begin */
agentSwitchAddressConflictGroup_t *
k_agentSwitchAddressConflictGroup_get(int serialNum, ContextInfo *contextInfo,
                                      int nominator)
{
  static agentSwitchAddressConflictGroup_t agentSwitchAddressConflictGroupData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 snmp_buffer[SNMP_BUFFER_LEN];
  ...
  if (firstTime == L7_TRUE)
  {
    ...
    agentSwitchAddressConflictGroupData.agentSwitchLastConflictingMacAddr =
                                 MakeOctetString(NULL, 0);
  }

  ZERO_VALID(agentSwitchAddressConflictGroupData.valid);
  ...
  case -1:
    /* pass through*/
  case I_agentSwitchAddressConflictDetectionStatus :
    if (snmpSwitchAddrConflictDetectStatusGet
        (&agentSwitchAddressConflictGroupData.agentSwitchAddressConflictDetectionStatus) == L7_SUCCESS)
      SET_VALID(I_agentSwitchAddressConflictDetectionStatus,
                agentSwitchAddressConflictGroupData.valid);
    if (nominator != -1) break;
    /* else pass through */

  case I_agentSwitchLastConflictingIPAddr :
    if (usmDbIPAddrConflictLastDetectIPGet
        (&agentSwitchAddressConflictGroupData.agentSwitchLastConflictingIPAddr) == L7_SUCCESS)
      SET_VALID(I_agentSwitchLastConflictingIPAddr,
                agentSwitchAddressConflictGroupData.valid);
    if (nominator != -1) break;
    /* else pass through */

  case I_agentSwitchLastConflictingMacAddr :
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    if ((usmDbIPAddrConflictLastDetectMACGet(snmp_buffer) == L7_SUCCESS) &&
        (SafeMakeOctetString(&agentSwitchAddressConflictGroupData.agentSwitchLastConflictingMacAddr,
                             snmp_buffer, L7_MAC_ADDR_LEN) == L7_TRUE))
      SET_VALID(I_agentSwitchLastConflictingMacAddr, agentSwitchAddressConflictGroupData.valid);
    if (nominator != -1) break;
    /* else pass through */

  case I_agentSwitchAddressConflictDetectionStatusReset :
    agentSwitchAddressConflictGroupData.agentSwitchAddressConflictDetectionStatusReset =
                              D_agentSwitchAddressConflictDetectionStatusReset_none;
    SET_VALID(I_agentSwitchAddressConflictDetectionStatusReset,
              agentSwitchAddressConflictGroupData.valid);
    break;

  case I_agentSwitchAddressConflictDetectionRun :
    agentSwitchAddressConflictGroupData.agentSwitchAddressConflictDetectionRun =
                              D_agentSwitchAddressConflictDetectionRun_none;
    SET_VALID(I_agentSwitchAddressConflictDetectionRun,
              agentSwitchAddressConflictGroupData.valid);
    break;

  case I_agentSwitchLastConflictReportedTime :
    if (snmpSwitchLastConflictReportedTimeGet
        (&agentSwitchAddressConflictGroupData.agentSwitchLastConflictReportedTime) == L7_SUCCESS)
      SET_VALID(I_agentSwitchLastConflictReportedTime,
                agentSwitchAddressConflictGroupData.valid);
    if (nominator != -1) break;
    /* else pass through */
  ...
  if ( nominator >= 0 && !VALID(nominator, agentSwitchAddressConflictGroupData.valid) )
    return(NULL);

  return(&agentSwitchAddressConflictGroupData);
}

#ifdef SETS
int
k_agentSwitchAddressConflictGroup_test(ObjectInfo *object, ObjectSyntax *value,
                                       doList_t *dp, ContextInfo *contextInfo)
{
    return NO_ERROR;
}
...
int
k_agentSwitchAddressConflictGroup_ready(ObjectInfo *object, ObjectSyntax *value, 
                                        doList_t *doHead, doList_t *dp)
{
    dp->state = SR_ADD_MODIFY;
    return NO_ERROR;
...
int
k_agentSwitchAddressConflictGroup_set(agentSwitchAddressConflictGroup_t *data,
                                      ContextInfo *contextInfo, int function)
{
  if (VALID(I_agentSwitchAddressConflictDetectionRun, data->valid) &&
      (data->agentSwitchAddressConflictDetectionRun ==
                   D_agentSwitchAddressConflictDetectionRun_run) &&
      usmDbIPAddrConflictDetectRun() != L7_SUCCESS)
  {
      CLR_VALID(I_agentSwitchAddressConflictDetectionRun, data->valid);
      return COMMIT_FAILED_ERROR;
  }

  if (VALID(I_agentSwitchAddressConflictDetectionStatusReset, data->valid) &&
      (data->agentSwitchAddressConflictDetectionStatusReset ==
                   D_agentSwitchAddressConflictDetectionStatusReset_reset) &&
      usmDbIPAddrConflictDetectStatusClear() != L7_SUCCESS)
  {
      CLR_VALID(I_agentSwitchAddressConflictDetectionStatusReset, data->valid);
      return COMMIT_FAILED_ERROR;
  }
...
#ifdef SR_agentSwitchAddressConflictGroup_UNDO
/* add #define SR_agentSwitchAddressConflictGroup_UNDO in sitedefs.h to
 * include the undo routine for the agentSwitchAddressConflictGroup family.
 */
int
agentSwitchAddressConflictGroup_undo(doList_t *doHead, doList_t *doCur,
                                     ContextInfo *contextInfo)
{
   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentSwitchAddressConflictGroup_UNDO */

#endif /* SETS */

/* IP address conflict detection feature APIs - end */
...
/* lvl7_@p0772 start */
    ...
    {
      if (usmDbImageFileNameValid(snmp_buffer, &imageId) == L7_TRUE)
      {
        agentTransferUploadGroupData.agentTransferUploadImagename = D_agentTransferUploadImagename_image1 + imageId;
      }
  ...
  */
  ...
  if (VALID(I_agentTransferUploadImagename, data->valid))
  {
    ...
    if(data->agentTransferUploadImagename == D_agentTransferUploadImagename_image1)
    {
      ...
      usmDbTransferFileNameLocalSet(USMDB_UNIT_CURRENT, snmp_buffer);
    }
  ...
  if ( VALID(I_agentTransferUploadStart, data->valid))
  {
    ...
    if (data->agentTransferUploadStart == D_agentClearConfig_enable)
    {
      if (snmpTransferInfoGet(L7_TRUE, auditMsg, sizeof(auditMsg)) == L7_SUCCESS)
      {
        SnmpCmdLoggerAuditEntryAdd(contextInfo, auditMsg);
      }
    }
...
/* lvl7_@p0772 start */
    if (usmDbTransferFileNameLocalGet(USMDB_UNIT_CURRENT, snmp_buffer) == L7_SUCCESS)
    {
      if(usmDbImageFileNameValid(snmp_buffer, &imageId) == L7_TRUE)
      {
        agentTransferDownloadGroupData.agentTransferDownloadImagename = D_agentTransferDownloadImagename_image1 + imageId;
      }
                                 ...
                                 ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if ( VALID(I_agentTransferDownloadStart, data->valid))
  {
    ...
    if (data->agentTransferDownloadStart == D_agentClearConfig_enable)
    {
      if (snmpTransferInfoGet(L7_TRUE, auditMsg, sizeof(auditMsg)) == L7_SUCCESS)
      {
        SnmpCmdLoggerAuditEntryAdd(contextInfo, auditMsg);
...
static L7_BOOL firstTime = L7_TRUE;
L7_char8 snmp_buffer[SNMP_BUFFER_LEN];
L7_char8 imageName[SNMP_BUFFER_LEN];
L7_uint32 unitId;
  ...
  case I_agentImage1 :
    ...
    bzero(imageName, SNMP_BUFFER_LEN);
    usmDbImageFileNameGet(0, imageName);
    if (usmDbImageVersionGet(unitId, imageName, snmp_buffer) == L7_SUCCESS &&
  ...
  case I_agentImage2 :
    ...
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    bzero(imageName, SNMP_BUFFER_LEN);
    usmDbImageFileNameGet(1, imageName);
    if (usmDbImageVersionGet(unitId, imageName, snmp_buffer) == L7_SUCCESS &&
                            ...
                            ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if (VALID(I_agentNextActiveImage, data->valid) && data->agentNextActiveImage != NULL)
  {
    ...
    if (usmDbImageActivate(USMDB_UNIT_CURRENT, snmp_buffer, L7_FALSE) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return(COMMIT_FAILED_ERROR);
    }
                           ...
                           SR_INT32 agentPortDot1dBasePort)
{
  ...
  if (firstTime == L7_TRUE)
  {
    ...
    #endif
    #ifdef I_agentPortSpeedDuplexStatus
    agentPortConfigEntryData.agentPortSpeedDuplexStatus = MakeOctetString(NULL,0);
    #endif
...
#ifdef I_agentPortAccessVlanID
  case I_agentPortAccessVlanID :
   {
    L7_uint32 tempVal;

    if (usmDbDot1qSwPortModeGet(USMDB_UNIT_CURRENT, agentPortConfigEntryData.agentPortDot1dBasePort, 
                                &tempVal) == L7_SUCCESS)
    {
      if (tempVal != DOT1Q_SWPORT_MODE_ACCESS)
      {
        agentPortConfigEntryData.agentPortAccessVlanID = L7_NULL;
        SET_VALID(I_agentPortAccessVlanID, agentPortConfigEntryData.valid);
      }
      else if (usmDbDot1qSwPortAccessVlanGet(USMDB_UNIT_CURRENT, agentPortConfigEntryData.agentPortDot1dBasePort,
                                             &agentPortConfigEntryData.agentPortAccessVlanID) == L7_SUCCESS)
      {
        SET_VALID(I_agentPortAccessVlanID, agentPortConfigEntryData.valid);
      }
    }
...
#ifdef I_agentPortSpeedDuplexStatus 
  case I_agentPortSpeedDuplexStatus :
    if ((snmpAgentPortSpeedDuplexGet(USMDB_UNIT_CURRENT, (L7_uint32)agentPortConfigEntryData.agentPortDot1dBasePort, &intfStatus) == L7_SUCCESS) && 
        (SafeMakeOctetString(&agentPortConfigEntryData.agentPortSpeedDuplexStatus, &intfStatus, sizeof(intfStatus)) == L7_TRUE))
    {  
      SET_VALID(I_agentPortSpeedDuplexStatus, agentPortConfigEntryData.valid);
    }
    else
    {
      CLR_VALID(I_agentPortSpeedDuplexStatus, agentPortConfigEntryData.valid);
    }
    if ( nominator != -1)
    break;
#endif
...
#ifdef I_agentPortAccessVlanID
   if (VALID(I_agentPortAccessVlanID,data->valid))
   {
      if (usmDbDot1qSwPortAccessVlanSet(USMDB_UNIT_CURRENT,data->agentPortDot1dBasePort,
                                        data->agentPortAccessVlanID) == L7_SUCCESS)
      {
          SET_VALID(I_agentPortAccessVlanID, tempValid);
      }
      else
      {
        memcpy (data->valid,tempValid,sizeof(data->valid));
...
#ifdef I_agentPortSpeedDuplexStatus
  if (VALID(I_agentPortSpeedDuplexStatus, data->valid))
  {
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    memcpy(snmp_buffer, data->agentPortSpeedDuplexStatus->octet_ptr, data->agentPortSpeedDuplexStatus->length);
    if (snmpAgentPortSpeedDuplexSet(USMDB_UNIT_CURRENT, data->agentPortDot1dBasePort, 
                                    snmp_buffer) == L7_SUCCESS)
    {
      SET_VALID(I_agentPortSpeedDuplexStatus, data->valid);
    }
    else 
    {
       memcpy (data->valid, tempValid, sizeof(data->valid));
...
#ifdef I_agentAuthenticationListMethod5
  case I_agentAuthenticationListMethod5:
    if (snmpAgentAuthenticationListMethod5Get(USMDB_UNIT_CURRENT,
                                              agentAuthenticationListEntryData.agentAuthenticationListIndex,
                                              &agentAuthenticationListEntryData.agentAuthenticationListMethod5) == L7_SUCCESS)
      SET_VALID(nominator, agentAuthenticationListEntryData.valid);
    break;
#endif
...
#ifdef I_agentAuthenticationListMethod5
  if (VALID(I_agentAuthenticationListMethod5, data->valid))
  {
    if( snmpAgentAuthenticationListMethod5Set(USMDB_UNIT_CURRENT,
                                            data->agentAuthenticationListIndex,
                                            data->agentAuthenticationListMethod5) != L7_SUCCESS)
    {
      memcpy(data->valid, tempValid, sizeof(data->valid));
      return COMMIT_FAILED_ERROR;
    }
                       ...
                       ContextInfo *contextInfo, int function)
{
  ...
  */
  ...
  if ( VALID(I_agentSaveConfig, data->valid))
  {
    ...
    if (data->agentSaveConfig == D_agentSaveConfig_enable)
    {
      SnmpCmdLoggerAuditEntryAdd(contextInfo, pStrInfo_base_Auditing_SaveConfig);
    }
  }
  ...
  if ( VALID(I_agentClearConfig, data->valid))
  {
    ...
    if (data->agentClearConfig == D_agentClearConfig_enable)
    {
      SnmpCmdLoggerAuditEntryAdd(contextInfo, pStrInfo_base_Auditing_ClearConfig);
    }
  }
  ...
  if (rc == L7_SUCCESS)
  {
    if (VALID(I_agentIpsgIfVerifySource, data->valid))
    {
      if (snmpIpsgIfVerifySourceSet(intIfNum,
                                    data->agentIpsgIfVerifySource) != L7_SUCCESS)
      {
        CLR_VALID(I_agentIpsgIfVerifySource, data->valid);
        return COMMIT_FAILED_ERROR;
      }
    }
    if (VALID(I_agentIpsgIfPortSecurity, data->valid))
    {
      if (snmpIpsgIfPortSecuritySet(intIfNum, 
...
agentStaticIpsgBindingEntry_t *
k_agentStaticIpsgBindingEntry_get(int serialNum, ContextInfo *contextInfo,
                             int nominator,
                             int searchType,
                             ...
                             SR_UINT32 agentStaticIpsgBindingIpAddr)
{
  ...
  static agentStaticIpsgBindingEntry_t agentStaticIpsgBindingEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 mac_buffer[SNMP_BUFFER_LEN];
  L7_char8 name_buffer[SNMP_BUFFER_LEN];
  L7_enetMacAddr_t macAddr;
  ...
  {
    firstTime = L7_FALSE;
    agentStaticIpsgBindingEntryData.agentStaticIpsgBindingMacAddr = MakeOctetString(NULL, 0);
  }

  ZERO_VALID(agentStaticIpsgBindingEntryData.valid);
  bzero(name_buffer, SNMP_BUFFER_LEN);
  bzero(mac_buffer, SNMP_BUFFER_LEN);
  ...
  agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIfIndex = agentStaticIpsgBindingIfIndex;
  SET_VALID(I_agentStaticIpsgBindingIfIndex, agentStaticIpsgBindingEntryData.valid);

  agentStaticIpsgBindingEntryData.agentStaticIpsgBindingVlanId = agentStaticIpsgBindingVlanId;
  SET_VALID(I_agentStaticIpsgBindingVlanId , agentStaticIpsgBindingEntryData.valid);

  agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIpAddr = agentStaticIpsgBindingIpAddr;
  ...
  if((searchType == EXACT) ?
     (usmDbIpsgBindingGetByType(&agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIfIndex, 
                          &vlanIndex,
                          &agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS) :
     (( usmDbIpsgBindingGetByType(&agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIfIndex, 
                          &vlanIndex,
                          &agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS) &&
      (usmDbIpsgBindingGetNextByType(&agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIfIndex, 
                          &vlanIndex,
                          &agentStaticIpsgBindingEntryData.agentStaticIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS)))
  {
  ...
  else
  {
   ...
   agentStaticIpsgBindingEntryData.agentStaticIpsgBindingVlanId = vlanIndex;
    ...
    if(SafeMakeOctetString(&( agentStaticIpsgBindingEntryData.agentStaticIpsgBindingMacAddr),
                            name_buffer, L7_ENET_MAC_ADDR_LEN) == L7_TRUE)
    {
      SET_VALID(I_agentStaticIpsgBindingMacAddr, agentStaticIpsgBindingEntryData.valid);
    }
    else
    {
      ZERO_VALID(agentStaticIpsgBindingEntryData.valid);
  ...
  case I_agentStaticIpsgBindingRowStatus:
    agentStaticIpsgBindingEntryData.agentStaticIpsgBindingRowStatus = D_agentStaticIpsgBindingRowStatus_active;
    SET_VALID(I_agentStaticIpsgBindingRowStatus, agentStaticIpsgBindingEntryData.valid);
    break;
  ...
  if ( nominator >= 0 && !VALID(nominator, agentStaticIpsgBindingEntryData.valid) )
    return(NULL);
  return(&agentStaticIpsgBindingEntryData);

}

#ifdef SETS
int
k_agentStaticIpsgBindingEntry_test(ObjectInfo *object, ObjectSyntax *value,
                              doList_t *dp, ContextInfo *contextInfo)
{
...
int
k_agentStaticIpsgBindingEntry_ready(ObjectInfo *object, ObjectSyntax *value,
                               doList_t *doHead, doList_t *dp)
{
...
int
k_agentStaticIpsgBindingEntry_set_defaults(doList_t *dp)
{
    agentStaticIpsgBindingEntry_t *data = (agentStaticIpsgBindingEntry_t *) (dp->data);
...
#ifdef SR_agentStaticIpsgBindingEntry_UNDO
/* add #define SR_agentStaticIpsgBindingEntry_UNDO in sitedefs.h to
 * include the undo routine for the agentStaticIpsgBinding family.
 */
int
agentStaticIpsgBindingEntry_undo(doList_t *doHead, doList_t *doCur,
                            ContextInfo *contextInfo)
{
   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentStaticIpsgBindingEntry_UNDO */

#endif /* SETS */
#endif /* I_agentStaticIpsgBindingIfIndex */

#ifdef I_agentDynamicIpsgBindingIfIndex
agentDynamicIpsgBindingEntry_t *
k_agentDynamicIpsgBindingEntry_get(int serialNum, ContextInfo *contextInfo,
                              int nominator,
                              int searchType,
                              ...
                              SR_UINT32 agentDynamicIpsgBindingIpAddr)
{
  ...
  static agentDynamicIpsgBindingEntry_t agentDynamicIpsgBindingEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 mac_buffer[SNMP_BUFFER_LEN];
  L7_char8 name_buffer[SNMP_BUFFER_LEN];
  L7_enetMacAddr_t macAddr;
  ...
  {
    firstTime = L7_FALSE;
    agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingMacAddr = MakeOctetString(NULL, 0);
  }

  ZERO_VALID(agentDynamicIpsgBindingEntryData.valid);
  bzero(name_buffer, SNMP_BUFFER_LEN);
  bzero(mac_buffer, SNMP_BUFFER_LEN);
  ...
  agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIfIndex = agentDynamicIpsgBindingIfIndex;
  SET_VALID(I_agentDynamicIpsgBindingIfIndex, agentDynamicIpsgBindingEntryData.valid);

  agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingVlanId = agentDynamicIpsgBindingVlanId;
  SET_VALID(I_agentDynamicIpsgBindingVlanId , agentDynamicIpsgBindingEntryData.valid);

  agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIpAddr = agentDynamicIpsgBindingIpAddr;
  ...
  if((searchType == EXACT) ?
     (usmDbIpsgBindingGetByType(&agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIfIndex,
                          &vlanIndex,
                          &agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS) :
     (( usmDbIpsgBindingGetByType(&agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIfIndex,
                          &vlanIndex,
                          &agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS) &&
      (usmDbIpsgBindingGetNextByType(&agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIfIndex,
                          &vlanIndex,
                          &agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingIpAddr,
                          &macAddr, entryType) != L7_SUCCESS)))
  {
  ...
  else
  {
   ...
   agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingVlanId = vlanIndex;
    ...
    if(SafeMakeOctetString(&( agentDynamicIpsgBindingEntryData.agentDynamicIpsgBindingMacAddr),
                            name_buffer, L7_ENET_MAC_ADDR_LEN) == L7_TRUE)
    {
      SET_VALID(I_agentDynamicIpsgBindingMacAddr, agentDynamicIpsgBindingEntryData.valid);
    }
    else
    {
      ZERO_VALID(agentDynamicIpsgBindingEntryData.valid);
  ...
  if ( nominator >= 0 && !VALID(nominator, agentDynamicIpsgBindingEntryData.valid) )
    return(NULL);
  return(&agentDynamicIpsgBindingEntryData);
...
agentStaticDsBindingEntry_t *
k_agentStaticDsBindingEntry_get(int serialNum, ContextInfo *contextInfo,
                           int nominator,
                           int searchType,
                           OctetString * agentStaticDsBindingMacAddr)
{
  static agentStaticDsBindingEntry_t agentStaticDsBindingEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 mac_buffer[SNMP_BUFFER_LEN];
  L7_char8 name_buffer[SNMP_BUFFER_LEN];
  L7_enetMacAddr_t macAddr;
  ...
  {
    firstTime = L7_FALSE;
    agentStaticDsBindingEntryData.agentStaticDsBindingMacAddr = MakeOctetString(NULL, 0);
  }

  ZERO_VALID(agentStaticDsBindingEntryData.valid);
  bzero(name_buffer, SNMP_BUFFER_LEN);
  bzero(mac_buffer, SNMP_BUFFER_LEN);
  ...
  else
  {
   ...
   SET_VALID(I_agentStaticDsBindingVlanId , agentStaticDsBindingEntryData.valid);
   SET_VALID(I_agentStaticDsBindingIpAddr, agentStaticDsBindingEntryData.valid);
    ...
    if(SafeMakeOctetString(&( agentStaticDsBindingEntryData.agentStaticDsBindingMacAddr),
                            name_buffer, L7_ENET_MAC_ADDR_LEN) == L7_TRUE)
    {
      SET_VALID(I_agentStaticDsBindingMacAddr, agentStaticDsBindingEntryData.valid);
    }
    else
    {
      ZERO_VALID(agentStaticDsBindingEntryData.valid);
  ...
  case I_agentStaticDsBindingRowStatus:
    agentStaticDsBindingEntryData.agentStaticDsBindingRowStatus = D_agentStaticDsBindingRowStatus_active;
    SET_VALID(I_agentStaticDsBindingRowStatus, agentStaticDsBindingEntryData.valid);
    break;
  ...
  if ( nominator >= 0 && !VALID(nominator, agentStaticDsBindingEntryData.valid) )
    return(NULL);
  return(&agentStaticDsBindingEntryData);

}

#ifdef SETS
int
k_agentStaticDsBindingEntry_test(ObjectInfo *object, ObjectSyntax *value,
                            doList_t *dp, ContextInfo *contextInfo)
{
...
int
k_agentStaticDsBindingEntry_ready(ObjectInfo *object, ObjectSyntax *value,
                             doList_t *doHead, doList_t *dp)
{
...
int
k_agentStaticDsBindingEntry_set_defaults(doList_t *dp)
{
    agentStaticDsBindingEntry_t *data = (agentStaticDsBindingEntry_t *) (dp->data);
...
#ifdef SR_agentStaticDsBindingEntry_UNDO
/* add #define SR_agentStaticDsBindingEntry_UNDO in sitedefs.h to
 * include the undo routine for the agentStaticDsBinding family.
 */
int
agentStaticDsBindingEntry_undo(doList_t *doHead, doList_t *doCur,
                          ContextInfo *contextInfo)
{
   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentStaticDsBindingEntry_UNDO */

#endif /* SETS */
#endif /* I_agentStaticDsBindingMacAddr */

#ifdef I_agentDynamicDsBindingMacAddr
agentDynamicDsBindingEntry_t *
k_agentDynamicDsBindingEntry_get(int serialNum, ContextInfo *contextInfo,
                           int nominator,
                           int searchType,
                           OctetString * agentDynamicDsBindingMacAddr)
{
  static agentDynamicDsBindingEntry_t agentDynamicDsBindingEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 mac_buffer[SNMP_BUFFER_LEN];
  L7_char8 name_buffer[SNMP_BUFFER_LEN];
  L7_enetMacAddr_t macAddr;
  ...
  {
    firstTime = L7_FALSE;
    agentDynamicDsBindingEntryData.agentDynamicDsBindingMacAddr = MakeOctetString(NULL, 0);
  }

  ZERO_VALID(agentDynamicDsBindingEntryData.valid);
  bzero(name_buffer, SNMP_BUFFER_LEN);
  bzero(mac_buffer, SNMP_BUFFER_LEN);
 ...
 /*  agentDynamicDsBindingEntryData.agentDynamicDsBindingIfIndex = agentDynamicDsBindingIfIndex;
  SET_VALID(I_agentDynamicDsBindingIfIndex, agentDynamicDsBindingEntryData.valid);

  agentDynamicDsBindingEntryData.agentDynamicDsBindingVlanId = agentDynamicDsBindingVlanId;
  SET_VALID(I_agentDynamicDsBindingVlanId , agentDynamicDsBindingEntryData.valid);

  agentDynamicDsBindingEntryData.agentDynamicDsBindingIpAddr = agentDynamicDsBindingIpAddr;
  SET_VALID(I_agentDynamicDsBindingIpAddr, agentDynamicDsBindingEntryData.valid); */
  ...
  else
  {
   ...
   SET_VALID(I_agentDynamicDsBindingVlanId , agentDynamicDsBindingEntryData.valid);
   SET_VALID(I_agentDynamicDsBindingIpAddr, agentDynamicDsBindingEntryData.valid); 
   SET_VALID(I_agentDynamicDsBindingLeaseRemainingTime, agentDynamicDsBindingEntryData.valid); 
    ...
    if(SafeMakeOctetString(&( agentDynamicDsBindingEntryData.agentDynamicDsBindingMacAddr),
                            name_buffer, L7_ENET_MAC_ADDR_LEN) == L7_TRUE)
    {
      SET_VALID(I_agentDynamicDsBindingMacAddr, agentDynamicDsBindingEntryData.valid);
    }
    else
    {
      ZERO_VALID(agentDynamicDsBindingEntryData.valid);
  ...
  if ( nominator >= 0 && !VALID(nominator, agentDynamicDsBindingEntryData.valid) )
    return(NULL);
  return(&agentDynamicDsBindingEntryData);
                                  ...
                                  SR_INT32 ifIndex)
{
  ...
  case I_agentDhcpL2RelayTrustedClntMsgsWithoutOptn82:
    agentDhcpL2RelayStatsEntryData.agentDhcpL2RelayTrustedClntMsgsWithoutOptn82 =
                                           stats.trustedCliMsgsWithoutOpt82;
                                                                      ...
                                                                      SR_INT32 agentLinkDependencyGroupId )
{
  ...
  L7_RC_t        rc;
  link_dependency_action_constants_t link_action = L7_LINK_DEPENDENCY_ACTION_LINK_DOWN;
  ...
  rc = usmDbLinkDependencyGroupDependencyActionGet( agentLinkDependencyGroupId, &link_action );
  if ( L7_SUCCESS != rc )
  {
    ZERO_VALID( agentLinkDependencyGroupEntryData.valid );
    return( NULL );
  ...
  switch (nominator)
  {
    ...
    case I_agentLinkDependencyGroupAction:
      L7_INTF_MASKREV( members );
      L7_INTF_MASKREV( dependencies );

      agentLinkDependencyGroupEntryData.agentLinkDependencyGroupAction = link_action;
      SET_VALID( I_agentLinkDependencyGroupAction,                 agentLinkDependencyGroupEntryData.valid);
...
#ifdef I_agentLinkDependencyGroupAction
    case I_agentLinkDependencyGroupAction:
      break;
#endif /* I_agentLinkDependencyGroupDependsOnPortMask */
                                    ...
                                    ContextInfo *contextInfo, int function)
{
  ...
  if ( ( !VALID( I_agentLinkDependencyGroupMemberPortMask,    data->valid ) ) &&
       ...
       ( !VALID( I_agentLinkDependencyGroupAction, data->valid ) ) )
  {
    ZERO_VALID(data->valid);
    return INCONSISTENT_NAME_ERROR;
  }

  if ( !VALID( I_agentLinkDependencyGroupAction, data->valid ) )
  {
      /* Put back to current */
      usmDbLinkDependencyGroupDependencyActionGet( data->agentLinkDependencyGroupId, &link_action );
  }
  else
  {
      link_action = data->agentLinkDependencyGroupAction;
  }

  if ( L7_SUCCESS != usmDbLinkDependencyGroupDependencyActionSet( data->agentLinkDependencyGroupId,  link_action) )
  {
    ZERO_VALID(data->valid);
    return COMMIT_FAILED_ERROR;
  }
...
L7_BOOL creationErrorDot1xUserConfigGroup = L7_FALSE;

agentIASUserConfigGroup_t *
k_agentIASUserConfigGroup_get(int serialNum, ContextInfo *contextInfo,
                                        int nominator)
{
static agentIASUserConfigGroup_t agentIASUserConfigGroupData;
static L7_BOOL firstTime = L7_TRUE;
...
if (firstTime == L7_TRUE)
{
  ...
  agentIASUserConfigGroupData.agentIASUserConfigCreate = MakeOctetString(NULL, 0);
}

ZERO_VALID(agentIASUserConfigGroupData.valid);
SET_VALID(I_agentIASUserConfigCreate, agentIASUserConfigGroupData.valid);

return(&agentIASUserConfigGroupData);
...
#ifdef SETS
int
k_agentIASUserConfigGroup_test(ObjectInfo *object, ObjectSyntax *value,
                                         doList_t *dp, ContextInfo *contextInfo)
{
  return NO_ERROR;
}
...
int
k_agentIASUserConfigGroup_ready(ObjectInfo *object, ObjectSyntax *value, 
                                          doList_t *doHead, doList_t *dp)
{
  dp->state = SR_ADD_MODIFY;
  return NO_ERROR;
...
int
k_agentIASUserConfigGroup_set(agentIASUserConfigGroup_t *data,
                                        ContextInfo *contextInfo, int function)
{
   L7_char8 snmp_buffer[SNMP_BUFFER_LEN];
   ...
   */
   ...
   if (VALID(I_agentIASUserConfigCreate, data->valid))
   {
     bzero(snmp_buffer, SNMP_BUFFER_LEN);
     memcpy(snmp_buffer, data->agentIASUserConfigCreate->octet_ptr, data->agentIASUserConfigCreate->length);
     if (snmpAgentIASUserConfigCreateSet(snmp_buffer) != L7_SUCCESS)
     {
       memcpy(data->valid, tempValid, sizeof(data->valid));
       creationErrorDot1xUserConfigGroup = L7_TRUE;
       return COMMIT_FAILED_ERROR;
     }
     else
     {
       SET_VALID(I_agentIASUserConfigCreate, tempValid);
     }
   }
...
#ifdef SR_agentIASUserConfigGroup_UNDO
/* add #define SR_agentIASUserConfigGroup_UNDO in sitedefs.h to
 * include the undo routine for the agentIASUserConfigGroup family.
 */
int
agentIASUserConfigGroup_undo(doList_t *doHead, doList_t *doCur,
                                       ContextInfo *contextInfo)
{
  agentIASUserConfigGroup_t *data = (agentIASUserConfigGroup_t *) doCur->data;
  ...
  /* Define a table entry for destroying the created row */
  agentIASUserConfigEntry_t agentIASUserConfigEntryData;
  agentIASUserConfigEntry_t *entrysetdata = &agentIASUserConfigEntryData;
  
  /* Initialize this table entry */
  agentIASUserConfigEntryData.agentIASUserName = MakeOctetString(NULL, 0);
  agentIASUserConfigEntryData.agentIASUserPassword = MakeOctetString(NULL, 0);

  ZERO_VALID(agentIASUserConfigEntryData.valid);
  
  if ( creationErrorDot1xUserConfigGroup == L7_TRUE )
  {
    creationErrorDot1xUserConfigGroup = L7_FALSE;
    return NO_ERROR;
  }
  ...
  bzero(snmp_buffer, SNMP_BUFFER_LEN);
  strncpy(snmp_buffer, data->agentIASUserConfigCreate->octet_ptr, data->agentIASUserConfigCreate->length);
  
  /* Get the table index for the specific user name */
  if ( (rc = usmDbUserMgrIASUserDBUserIndexGet(snmp_buffer, &agentUserTempIndex)) == L7_SUCCESS)
  {
    /* we are trying to undo an add to the Table
     * undoing an add, so delete
    */
    agentIASUserConfigEntryData.agentIASUserStatus = D_agentIASUserStatus_destroy;
    agentIASUserConfigEntryData.agentIASUserIndex = agentUserTempIndex;
    SET_VALID(I_agentIASUserStatus, agentIASUserConfigEntryData.valid);
    function = SR_DELETE;
  
    /* use the set method for the undo */
    if ((entrysetdata != NULL) && (k_agentIASUserConfigEntry_set(entrysetdata, contextInfo, function) == NO_ERROR))
      return NO_ERROR;
  }
...
#endif /* SR_agentIASUserConfigGroup_UNDO */

#endif /* SETS */

agentIASUserConfigEntry_t *
k_agentIASUserConfigEntry_get(int serialNum, ContextInfo *contextInfo,
                                        int nominator,
                                        int searchType,
                                        SR_INT32 agentIASUserIndex)
{
  static agentIASUserConfigEntry_t agentIASUserConfigEntryData;
  static L7_BOOL firstTime = L7_TRUE;
  L7_char8 snmp_buffer[SNMP_BUFFER_LEN];
  ...
  ZERO_VALID(agentIASUserConfigEntryData.valid);
  agentIASUserConfigEntryData.agentIASUserIndex = agentIASUserIndex;
  SET_VALID(I_agentIASUserIndex, agentIASUserConfigEntryData.valid);

  if ( (searchType == EXACT) ?
       (snmpAgentIASUserConfigEntryGet(agentIASUserConfigEntryData.agentIASUserIndex) != L7_SUCCESS) :
       ( (snmpAgentIASUserConfigEntryGet(agentIASUserConfigEntryData.agentIASUserIndex) != L7_SUCCESS) &&
         (snmpAgentIASUserConfigEntryNextGet(&agentIASUserConfigEntryData.agentIASUserIndex) != L7_SUCCESS) ) )
  {
    ZERO_VALID(agentIASUserConfigEntryData.valid);
    return(NULL);
  }
  ...
  {
  case -1:
  case I_agentIASUserIndex:
    if ( nominator != -1 ) break;
    /* else pass through */

  case I_agentIASUserName :
    bzero(snmp_buffer, SNMP_BUFFER_LEN);
    if (usmDbUserMgrIASUserDBUserNameGet(agentIASUserConfigEntryData.agentIASUserIndex, snmp_buffer, sizeof(snmp_buffer)) == L7_SUCCESS &&
        (SafeMakeOctetStringFromTextExact(&agentIASUserConfigEntryData.agentIASUserName, snmp_buffer) == L7_TRUE))
      SET_VALID(I_agentIASUserName, agentIASUserConfigEntryData.valid);
    if ( nominator != -1 ) break;
    /* else pass through */

  case I_agentIASUserPassword :
    SET_VALID(I_agentIASUserPassword, agentIASUserConfigEntryData.valid);
    if ( nominator != -1 ) break;
    /* else pass through */

    case I_agentIASUserStatus  :
      if (snmpAgentIASUserStatusGet(agentIASUserConfigEntryData.agentIASUserIndex,
                                 &agentIASUserConfigEntryData.agentIASUserStatus ) == L7_SUCCESS)
        SET_VALID(I_agentIASUserStatus, agentIASUserConfigEntryData.valid);
      break;
  ...
  if ( nominator >= 0 && !VALID(nominator, agentIASUserConfigEntryData.valid) )
    return(NULL);

  return(&agentIASUserConfigEntryData);
}
#ifdef SETS
int
k_agentIASUserConfigEntry_test(ObjectInfo *object, ObjectSyntax *value,
                                         doList_t *dp, ContextInfo *contextInfo)
{
  return NO_ERROR;
}
...
int
k_agentIASUserConfigEntry_ready(ObjectInfo *object, ObjectSyntax *value, 
                                          doList_t *doHead, doList_t *dp)
{
  dp->state = SR_ADD_MODIFY;
  return NO_ERROR;
...
int
k_agentIASUserConfigEntry_set_defaults(doList_t *dp)
{
  agentIASUserConfigEntry_t *data = (agentIASUserConfigEntry_t *) (dp->data);
  
  if ((data->agentIASUserName = MakeOctetStringFromText("")) == 0) {
      return RESOURCE_UNAVAILABLE_ERROR;
  }
  if ((data->agentIASUserPassword = MakeOctetStringFromText("")) == 0) {
      return RESOURCE_UNAVAILABLE_ERROR;
  }
...
int
k_agentIASUserConfigEntry_set(agentIASUserConfigEntry_t *data,
                                        ContextInfo *contextInfo, int function)
  {
    L7_char8 snmp_buffer[SNMP_BUFFER_LEN];
    ...
    */
    L7_char8 tempValid[sizeof(data->valid)];
    bzero(tempValid, sizeof(tempValid));
    if ( snmpAgentIASUserConfigEntryGet(data->agentIASUserIndex) != L7_SUCCESS)
    {
      ZERO_VALID(data->valid);
      return(COMMIT_FAILED_ERROR);
    }
    if (data->agentIASUserName != NULL)
    {
      bzero(snmp_buffer, SNMP_BUFFER_LEN);
      memcpy(snmp_buffer, data->agentIASUserName->octet_ptr, data->agentIASUserName->length);
      if ( VALID(I_agentIASUserName, data->valid))
      {
        if(snmpAgentIASUserNameSet(data->agentIASUserIndex, snmp_buffer) != L7_SUCCESS )
        {
          memcpy(data->valid, tempValid, sizeof(data->valid));
          return(COMMIT_FAILED_ERROR);
        }
        ...
        {
          SET_VALID(I_agentIASUserName, tempValid);
        }
      }
    }
    if (data->agentIASUserPassword != NULL)
    {
      bzero(snmp_buffer, SNMP_BUFFER_LEN);
      memcpy(snmp_buffer, data->agentIASUserPassword->octet_ptr, data->agentIASUserPassword->length);
      if ( VALID(I_agentIASUserPassword, data->valid))
      {
        if(strlen(snmp_buffer) <= L7_MAX_PASSWORD_SIZE)
        {
          if(usmDbUserMgrIASUserDBUserPasswordSet(data->agentIASUserIndex, snmp_buffer, L7_FALSE) != L7_SUCCESS )
          {
            memcpy(data->valid, tempValid, sizeof(data->valid));
            return(COMMIT_FAILED_ERROR);
          }
          ...
          {
            SET_VALID(I_agentIASUserPassword, tempValid);
          }
        }
        else
        {
    ...
    if ( VALID(I_agentIASUserStatus, data->valid))
    {
      if(snmpAgentIASUserStatusSet(data->agentIASUserIndex,
                                data->agentIASUserStatus) != L7_SUCCESS )
      {
        memcpy(data->valid, tempValid, sizeof(data->valid));
        return(COMMIT_FAILED_ERROR);
      }
      ...
      {
        SET_VALID(I_agentIASUserStatus, tempValid);
      }
    }
    return NO_ERROR;
  }

#ifdef SR_agentIASUserConfigEntry_UNDO
/* add #define SR_agentIASUserConfigEntry_UNDO in sitedefs.h to
 * include the undo routine for the agentIASUserConfigEntry family.
 */
int
agentIASUserConfigEntry_undo(doList_t *doHead, doList_t *doCur,
                                       ContextInfo *contextInfo)
  {
  agentIASUserConfigEntry_t *data = (agentIASUserConfigEntry_t *) doCur->data;
  agentIASUserConfigEntry_t *undodata = (agentIASUserConfigEntry_t *) doCur->undodata;
  agentIASUserConfigEntry_t *entrysetdata = NULL;
  ...
  /* Define a group instance so as to create the deleted row */
  agentIASUserConfigGroup_t agentIASUserConfigGroupTempdata;
  agentIASUserConfigGroup_t *setdata = &agentIASUserConfigGroupTempdata;
  /* Initialize this group instance */
  agentIASUserConfigGroupTempdata.agentIASUserConfigCreate = CloneOctetString(undodata->agentIASUserName);
  ZERO_VALID(agentIASUserConfigGroupTempdata.valid);
   ...
   * undoing a delete or modify, replace the original data
   */
  if(data->agentIASUserStatus ==  D_agentIASUserStatus_destroy)
  {
    /* check that the entry was actually deleted */
    if (snmpAgentIASUserConfigEntryGet(undodata->agentIASUserIndex) == L7_SUCCESS)
      return NO_ERROR;
  
    /* Use the set method for the creation of the row with default values */
    SET_VALID(I_agentIASUserConfigCreate, agentIASUserConfigGroupTempdata.valid);
    function = SR_ADD_MODIFY;
    if ((setdata != NULL) && k_agentIASUserConfigGroup_set(setdata, contextInfo, function) == NO_ERROR)
    {
      bzero(snmp_buffer, SNMP_BUFFER_LEN);
      strncpy(snmp_buffer, undodata->agentIASUserName->octet_ptr, undodata->agentIASUserName->length);
  
      /* Get the table index for the specific user name */
      if ( (rc = usmDbUserMgrIASUserDBUserIndexGet(snmp_buffer, &agentUserTempIndex)) == L7_SUCCESS)
      {
        undodata->agentIASUserIndex = agentUserTempIndex;
  
        /* set all valid for the setting the previous values to the created row */
        memset(undodata->valid, 0xffff, sizeof(undodata->valid));
  
        /* Status should not be set again */
        CLR_VALID(I_agentIASUserStatus, undodata->valid);
        ...
        if ((entrysetdata != NULL) && (k_agentIASUserConfigEntry_set(entrysetdata, contextInfo, function) == NO_ERROR))
        {
          free(agentIASUserConfigGroupTempdata.agentIASUserConfigCreate);
          return NO_ERROR;
        }
      }
    }
  ...
  {
    /* we are trying to undo a modify to the Table */
    if(undodata->agentIASUserStatus == D_agentIASUserStatus_notReady
    || undodata->agentIASUserStatus == D_agentIASUserStatus_notInService)
    {
      undodata->agentIASUserStatus = D_agentIASUserStatus_createAndWait;
    }
    else
    {
      if(undodata->agentIASUserStatus == D_agentIASUserStatus_active)
      {
        undodata->agentIASUserStatus = D_agentIASUserStatus_createAndGo;
      }
    }
...
#endif /* SR_agentIASUserConfigEntry_UNDO */

#endif /* SETS */
#endif /* I_agentIASUserIndex */

#ifdef L7_ROUTING_PACKAGE

agentSdmPreferConfigEntry_t *
k_agentSdmPreferConfigEntry_get(int serialNum, ContextInfo *contextInfo,
                                int nominator)
{
  static agentSdmPreferConfigEntry_t agentSdmPreferConfigEntryData;

  ZERO_VALID(agentSdmPreferConfigEntryData.valid);

  switch (nominator)
  {
  case -1:
    /* Pass through*/
  case I_agentSdmPreferCurrentTemplate:
    agentSdmPreferConfigEntryData.agentSdmPreferCurrentTemplate = usmDbSdmActiveTemplateGet();
      SET_VALID(I_agentSdmPreferCurrentTemplate, agentSdmPreferConfigEntryData.valid);
    break;

  case I_agentSdmPreferNextTemplate:
    agentSdmPreferConfigEntryData.agentSdmPreferNextTemplate = usmDbSdmNextActiveTemplateGet();
      SET_VALID(I_agentSdmPreferNextTemplate, agentSdmPreferConfigEntryData.valid);
    break;
  ...
  if (nominator >= 0 && !VALID(nominator, agentSdmPreferConfigEntryData.valid))
    return(NULL);

  return(&agentSdmPreferConfigEntryData);
}

#ifdef SETS
int
k_agentSdmPreferConfigEntry_test(ObjectInfo *object, ObjectSyntax *value,
                            doList_t *dp, ContextInfo *contextInfo)
{
#ifdef NOT_YET
    agentSdmPreferConfigEntry_t *data = (agentSdmPreferConfigEntry_t *) dp->data;
    ...
    if (object->nominator == I_agentSdmPreferNextTemplate) {
        /*
         * Examine value->sl_value.  In case of error,
         * return from the function here with an error code.
         */
        SET_VALID(I_agentSdmPreferNextTemplate, data->mod);
    }
#endif /* NOT_YET */
    return NO_ERROR;
}
...
int
k_agentSdmPreferConfigEntry_ready(ObjectInfo *object, ObjectSyntax *value,
                             doList_t *doHead, doList_t *dp)
{
#ifdef NOT_YET
    agentSdmPreferConfigEntry_t *data = (agentSdmPreferConfigEntry_t *) dp->data;
    ...
    if (VALID(I_agentSdmPreferNextTemplate, data->valid)) {
        /*
         * Examine data->agentSdmPreferNextTemplate in relation to other
         * MIB objects.  If a relationship is not invalid, set
         * dp->state to SR_UNKNOWN and return with NO_ERROR.
         */
    }
...
int
k_agentSdmPreferConfigEntry_set(agentSdmPreferConfigEntry_t *data,
                           ContextInfo *contextInfo, int function)
{
  if (VALID(I_agentSdmPreferNextTemplate, data->valid) &&      
      (snmpSdmNextActiveTemplateSet(data->agentSdmPreferNextTemplate) != L7_SUCCESS))
    return COMMIT_FAILED_ERROR;

  return NO_ERROR;
}

#ifdef SR_agentSdmPreferConfigEntry_UNDO
/* add #define SR_agentSdmPreferConfigEntry_UNDO in sitedefs.h to
 * include the undo routine for the agentSdmPreferConfigEntry family.
 */
int
agentSdmPreferConfigEntry_undo(doList_t *doHead, doList_t *doCur,
                          ContextInfo *contextInfo)
{
   return UNDO_FAILED_ERROR;
}
#endif /* SR_agentSdmPreferConfigEntry_UNDO */

#endif /* SETS */

agentSdmTemplateEntry_t *
k_agentSdmTemplateEntry_get(int serialNum, ContextInfo *contextInfo,
                            int nominator,
                            int searchType,
                            SR_INT32 agentSdmTemplateId)
{
   static agentSdmTemplateEntry_t agentSdmTemplateEntryData;
   L7_uint32 templateId;

   ZERO_VALID(agentSdmTemplateEntryData.valid);
   agentSdmTemplateEntryData.agentSdmTemplateId = agentSdmTemplateId;
   SET_VALID(I_agentSdmTemplateId, agentSdmTemplateEntryData.valid);

   if(searchType == EXACT ?
      (snmpSdmTemplateIdGet (agentSdmTemplateEntryData.agentSdmTemplateId) != L7_TRUE) :
      ((snmpSdmTemplateIdGet (agentSdmTemplateEntryData.agentSdmTemplateId) != L7_TRUE) &&
      (usmDbSdmTemplateIdNextGet (&agentSdmTemplateEntryData.agentSdmTemplateId) != L7_SUCCESS)))
   {
     ZERO_VALID(agentSdmTemplateEntryData.valid);
     return(NULL);
   }

   templateId = agentSdmTemplateEntryData.agentSdmTemplateId;
   switch (nominator)
   {
   case -1:
   case I_agentSdmTemplateId:
     break;

   case I_agentArpEntries:
     agentSdmTemplateEntryData.agentArpEntries = usmDbSdmTemplateMaxArpEntriesGet(templateId);
       SET_VALID(I_agentArpEntries, agentSdmTemplateEntryData.valid);
     break;

   case I_agentIPv4UnicastRoutes:
     agentSdmTemplateEntryData.agentIPv4UnicastRoutes = usmDbSdmTemplateMaxIpv4RoutesGet(templateId);
       SET_VALID(I_agentIPv4UnicastRoutes, agentSdmTemplateEntryData.valid);
     break;

   case I_agentIPv6NdpEntries:
     agentSdmTemplateEntryData.agentIPv6NdpEntries = usmDbSdmTemplateMaxNdpEntriesGet(templateId);
       SET_VALID(I_agentIPv6NdpEntries, agentSdmTemplateEntryData.valid);
     break;

   case I_agentIPv6UnicastRoutes:
     agentSdmTemplateEntryData.agentIPv6UnicastRoutes = usmDbSdmTemplateMaxIpv6RoutesGet(templateId);
       SET_VALID(I_agentIPv6UnicastRoutes, agentSdmTemplateEntryData.valid);
     break;

   case I_agentEcmpNextHops:
     agentSdmTemplateEntryData.agentEcmpNextHops = usmDbSdmTemplateMaxEcmpNextHopsGet(templateId);
       SET_VALID(I_agentEcmpNextHops, agentSdmTemplateEntryData.valid);
     break;

   case I_agentIPv4MulticastRoutes:
     agentSdmTemplateEntryData.agentIPv4MulticastRoutes = usmDbSdmTemplateMaxIpv4McastRoutesGet(templateId);
       SET_VALID(I_agentIPv4MulticastRoutes, agentSdmTemplateEntryData.valid);
     break;

   case I_agentIPv6MulticastRoutes:
     agentSdmTemplateEntryData.agentIPv6MulticastRoutes = usmDbSdmTemplateMaxIpv6McastRoutesGet(templateId);
       SET_VALID(I_agentIPv6MulticastRoutes, agentSdmTemplateEntryData.valid);
     break;
   ...
   if (nominator >= 0 && !VALID(nominator, agentSdmTemplateEntryData.valid))
    return(NULL);

   return(&agentSdmTemplateEntryData);
}
#endif
