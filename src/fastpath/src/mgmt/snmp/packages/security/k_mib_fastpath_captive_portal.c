                                     ...
                                     SR_INT32 cpCaptivePortalIntfIfIndex)
{
  ...
  */
  ...
  case I_cpCaptivePortalIntfActivationStatus:
    if (snmpCpdmCPIntfStatusGet(cpCaptivePortalIntfStatusEntry.cpCaptivePortalIntfIfIndex, 
                                &cpCaptivePortalIntfStatusEntry.cpCaptivePortalIntfActivationStatus) == L7_SUCCESS)
      SET_VALID(I_cpCaptivePortalIntfActivationStatus, cpCaptivePortalIntfStatusEntry.valid);
    if ( nominator != -1 ) break;
    /* else pass through */

  case I_cpCaptivePortalIntfActivationDisabledReason:
    if (snmpCpdmCPIntfStatusGetWhy(cpCaptivePortalIntfStatusEntry.cpCaptivePortalIntfIfIndex,
                                   &cpCaptivePortalIntfStatusEntry.cpCaptivePortalIntfActivationDisabledReason) == L7_SUCCESS)
      SET_VALID(I_cpCaptivePortalIntfActivationDisabledReason, cpCaptivePortalIntfStatusEntry.valid);
    if ( nominator != -1 ) break;
    /* else pass through */
