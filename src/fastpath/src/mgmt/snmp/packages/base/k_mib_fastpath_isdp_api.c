...
L7_RC_t snmpIsdpDeviceIdFormatCapabilityGet(L7_uchar8 *buf)
{
  ...
  if(rc == L7_SUCCESS)
  {
    ...
    if (ch & L7_ISDP_DEVICE_ID_FORMAT_CAPABILITY_HOSTNAME_BITMASK)
    {
      buf[0] |= (0x80 >> D_agentIsdpGlobalDeviceIdFormatCpb_hostName);
    }
...
{
  L7_uint32 ipAddressIndex, ipAddress = 0;

  if((usmdbIsdpIntfNeighborAddressGetNext(intIfNum, deviceId, 0, &ipAddressIndex)) == L7_SUCCESS)
  {
    if((usmdbIsdpIntfNeighborIpAddressGet(intIfNum, deviceId, ipAddressIndex, &ipAddress)) == L7_SUCCESS)
    {
