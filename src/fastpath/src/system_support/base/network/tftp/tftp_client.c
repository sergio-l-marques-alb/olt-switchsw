...
/* Select TFTP private port to avoid well known ports and FASTPATH Wireless/Clustering
** ports. 
** Tests under VxWorks show that if a random port is selected that conflicts 
** with existing an port then the TFTP transfer fails, but there is no error from the
** osapiSocketBind() function. 
** Since the OS doesn't return an error, the user will just have to retry 
** the TFTP operation. 
*/ 
#define TFTP_PORT_RANGE_LOW  50000
#define TFTP_PORT_RANGE_HIGH 51000

/*********************************************************************
* @purpose  Compute a random port number for TFTP session.
* 
* @returns  Port Number
*
* @end
*********************************************************************/
static L7_ushort16 tftpRandomPortGet (void)
{
  L7_ushort16 port_num;

  port_num = (L7_ushort16) (L7_Random() * 
                            (TFTP_PORT_RANGE_HIGH - TFTP_PORT_RANGE_LOW));
  port_num += TFTP_PORT_RANGE_LOW;
...
L7_TRANSFER_STATUS_t L7_TFTP_COPY (L7_inet_addr_t *inet_address,L7_char8 *pFilename,
                                   ...
                                   L7_uint8 requestDelay, L7_TFTP_CALLBACK_t callback, void *cbarg, L7_BOOL val)
{
      ...
      &&(pCommand[2]=='t'||pCommand[2] =='T'))
  {
    if ((rc = L7_TFTP_GET(inet_address,pFilename,pMode,fd,
                          requestDelay, callback, cbarg, val)) != L7_TRANSFER_SUCCESS)
    {
           ...
           (mode[4]=='t'||mode[4] =='T'))
  {
    ...
    if (count <0)
    {
      ...
      return L7_FAILURE;
            ...
            &&(mode[4]=='i'||mode[4] =='I')) ||
            ((mode[0]=='n'||mode[0] =='N')&&(mode[1]=='e'||mode[1] =='E')
            &&(mode[2]=='T'||mode[2] =='t')&&(mode[3]=='a'||mode[3] =='A')
            &&(mode[4]=='s'||mode[4] =='S')&&(mode[5]=='c'||mode[5] =='C')
            &&(mode[6]=='i'||mode[6] =='I')&&(mode[7]=='i'||mode[7] =='I')))
  {                                           /*file is in ascii/netascii mode*/
    ...
    for (j=0;j<datalength;j++)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "Error while putting data into the file");
        return L7_FAILURE;
      }
    }/* end of for*/
  }/* end of else*/
...
L7_int32 tftpGetSocketPrepare(L7_inet_addr_t *inet_address)
{
  ...
  L7_ushort16       get_port  = tftpRandomPortGet();
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpSocketPrepare(): Invalid Address family");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpSocketPrepare(): Failed to create socket.");
    return L7_NULL;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpSocketPrepare(): osapiSetsockopt SO_REUSEADDR failed");
  ...
  if (inet_address->family == L7_AF_INET)
  {
    ...
    if( L7_FILE_TYPE_AUTO_INSTALL_SCRIPT == simGetTransferDownloadFileType())
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "tftpSocketPrepare(): Set socket SO_BINDTODEVICE option error");
        osapiSocketClose(clientSocket);
        return L7_NULL;
      }
    } /*  if( L7_FILE_TYPE_AUTO_INSTALL_SCRIPT == simGetTransferDownloadFileType()) */
    ...
    if (broadcastModeValue == L7_NULL)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "tftpSocketPrepare(): Set socket L7_IP_TTL option error");
        osapiSocketClose (clientSocket);
        return L7_NULL;
      }
    }
    else
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "tftpSocketPrepare(): Set socket IP_MULTICAST_TTL option error");
        osapiSocketClose(clientSocket);
        return L7_NULL;
      }
    }
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
              "tftpSocketPrepare(): Set socket SO_BROADCAST option error");
      osapiSocketClose(clientSocket);
      return L7_NULL;
    }
  }
                           ...
                           L7_char8 *fileName)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpRequestSend(): Send to error. Errno(%d)", errno);
    return L7_FAILURE;
  }
					 ...
					 L7_ushort16 *source_port)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpGetReplyProcess(): Invalid Address family");
    return L7_TRANSFER_FAILED;
  }   
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpGetReplyProcess(): Recv from error");
    return L7_TRANSFER_FAILED;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
            "tftpGetReplyProcess(): TFTP client has received non TFTP reply");
    return L7_TRANSFER_FAILED;
  }
                                  ...
                                  L7_TFTP_CALLBACK_t callback, void *cbarg, L7_BOOL val)
{
  ...
  L7_ushort16    source_port    = TFTP_PORT;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_get(): Invalid Address family");
    return L7_TRANSFER_FAILED;
  }

  if ((clientSocket = tftpGetSocketPrepare(inet_address)) == L7_NULL)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_get(): Failed to create socket");  
    return L7_TRANSFER_FAILED;
  }
  ...
  {
    osapiSocketClose(clientSocket);  
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_get(): Send to error");
    return L7_TRANSFER_FAILED;
  }
  ...
  while (totalTimeouts++ <= TFTP_TIMEOUT_RETRY) /* do while loop */
  {
    ...
    /* NOTE: For AutoInstall TFTP read requests, force connection to time out
     *       by setting totalTimeouts to TFTP_TIMEOUT_RETRY+1 and exit.
     *       Since AutoInstall retry count is 3, three requests will go out anyway.
     *       No additional requests are required.                                                                     .
     */
    if (val == L7_TRUE)
    {
      totalTimeouts = TFTP_TIMEOUT_RETRY + 1;
    }
  ...
  *count=0;
      ...
      &&(mode[4]=='r'||mode[4] =='R')&&(mode[5]=='Y'||mode[5] =='y') )
  {
    ...
    *count=tftpFileRead(fileDesc, callback, cbarg, fileBuffer, TFTP_SEGSIZE);
    if (*count <0)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"Error while reading file");
      return L7_FAILURE;
    }
    return(L7_SUCCESS);
  }/* end of binary*/
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"Wrong value of mode entered");
    return L7_FAILURE;
  }
                    ...
                    L7_TFTP_CALLBACK_t callback, void *cbarg)
{
  ...
  L7_int32 data_len        = 0;
  L7_ushort16 put_port        = tftpRandomPortGet();
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_put(): Failed to create socket");
    return L7_FAILURE;
  }

  if (osapiSocketBind(Clientsocket, baddr, baddr_len) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_put(): Bind error");
    osapiSocketClose(Clientsocket);
    return L7_FAILURE;
  }
  while (1)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_put(): Send to error");
      osapiSocketClose(Clientsocket);
      return L7_FAILURE;
    }
    ...
    /*no reply from server*/
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"TFTP Timeout no reply from TFTP server");
        osapiSocketClose(Clientsocket);
        return L7_FAILURE;
      }
    }
    else
    {
      ...
      if (osapiSocketRecvfrom(Clientsocket, tftpBuffer, sizeof(L7_TFTP_PACKET_t),
                              0,raddr, &from_len, &data_len) != L7_SUCCESS)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"tftp_put(): recv from error");
        osapiSocketClose(Clientsocket);
        return L7_FAILURE;
      }
      if(inet_address->family == L7_AF_INET)
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"Error is: %s\n with Error code:: %d\n",
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"Wrong protocol used");
        osapiSocketClose(Clientsocket);
        return L7_FAILURE;
      }
  ...
  do
  {
    ...
    if (osapiSelect (Clientsocket+1, &readFds, NULL, NULL, TFTP_TIMEOUT,0) <= 0)
    {
      if (++total_timeouts > TFTP_TIMEOUT_RETRY)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"TFTP Timeout no ACK from TFTP server... aborting transfer");
        osapiSocketClose(Clientsocket);
        return L7_FAILURE;
      }
      resendFlag = L7_TRUE;
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"TFTP Timeout %u no ACK from TFTP server... resending block", total_timeouts);
    ...
    else
    {
      ...
      if (osapiNtohs(tftpContent->th_opcode) == TFTP_ACK)
      {
        ...
        else
        {
          ...
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,"Send block %d\n, got ACK for #%d\n",
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID," Server error message is: %s\n with error code:: %d\n",
                (tftpContent->th_errMsg), osapiNtohs(tftpContent->th_error));
        /*print error string*/
        osapiSocketClose(Clientsocket);
        return L7_FAILURE;
