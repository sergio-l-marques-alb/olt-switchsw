...
void sntpApiInit(struct sntpCfgData_s * cfg, struct sntpServerStats_s * srv)
{
  ...
  if (cfg == L7_NULL)
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SNTP_COMPONENT_ID,
            "sntpApiInit: cannot initialize API with cfg data\n");
  if (srv == L7_NULL)
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SNTP_COMPONENT_ID,
            "sntpApiInit: cannot initialize API with server stats data\n");
  sntpCfgData = cfg;
  serverStats = srv;
}
                            ...
                            L7_uint32 * ndx)
{
  ...
  if (firstEmptySlot >= 0)
  {
    ...
    memset(sntpCfgData->server[firstEmptySlot].address, 0, sizeof(sntpCfgData->server[firstEmptySlot].address));
    osapiStrncpy(sntpCfgData->server[firstEmptySlot].address, address, L7_SNTP_MAX_ADDRESS_LEN);
                                    ...
                                    L7_SNTP_ADDRESS_TYPE_t type)
{
  ...
  {
    memset(sntpCfgData->server[ndx-1].address, 0, sizeof(sntpCfgData->server[ndx-1].address));
    osapiStrncpy(sntpCfgData->server[ndx-1].address, address, L7_SNTP_MAX_ADDRESS_LEN);
