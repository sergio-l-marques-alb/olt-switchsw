...
/* Auto Install Gbl ErrorCode, default value L7_NO_VALUE */
static L7_RC_t autoInstallGblErrorCode = L7_NO_VALUE;
...
L7_RC_t autoInstallGblErrorCodeGet(void);

/* Get auto install global error code value */
L7_RC_t autoInstallGblErrorCodeGet(void)
{
  return autoInstallGblErrorCode;
}
...
void autoInstallTask()
{
  ...
  while(1)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "autoInstallTask: message recv failed event %d\n", msg.event);
    } 
...
void autoInstallDispatchCmd(autoInstallMsg_t *msg)
{
  switch (msg->event)
  {
    ...
    default:
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : Unrecognized event %d\n", msg->event);
      break;
...
void autoInstallStateMachineEventGenerate (autoInstalStateMachineEvent_t event)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "autoInstallStateMachineEventGenerate(): message send failed!\n");
  }
...
void autoInstallStateMachineEventProcess (autoInstalStateMachineEvent_t event)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : No event handler for state (%s)", autoInstallStateStrs[autoInstallRunTimeData->currentState]);
    return;
  }
...
void autoInstallStateMachineTransit (autoInstalState_t state)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : No state handler for state (%d)\n", state);
    return;
  }
...
static void autoInstallHostFileStateEventProcess (autoInstalStateMachineEvent_t event)
{
  ...
  {
    /* NOTE: Control reaches here in two cases:
     *
     * 1) if <hostname>.cfg is not found in 6 tries - in this case, seek host.cfg.
     * 2) if host.cfg is not found in 6 tries - in this case, wait for a restart.
     */
    if (osapiStrncmp(autoInstallRunTimeData->hostFileName, AUTO_INSTALL_DEFAULT_CONFIG_FILE, 
                     AUTO_INSTALL_HOST_FILENAME_LENGTH) != L7_NULL)
    {
      autoInstallStateMachineTransit(AUTO_INSTALL_GET_HOST_FILE_STATE);
    }
    else
...
static void autoInstallDhcpStateProcess (void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall process started...\n");  
  }
  ...
  if (autoInstallStartupConfigIsExist() == L7_TRUE)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : startup configuration exists. AutoInstall is stopped.\n");
...
void autoInstallSpecificFileStateComplete (void)
{
  ...
  else if (autoInstallRunTimeData->transferStatus == L7_TRANSFER_FILE_NOT_FOUND)
  {
  ...
  {    
    ...
    if (autoInstallRegisteredComponentsNotify(AUTO_INSTALL_SPECIFIED_FILE_FAILED) != L7_SUCCESS)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : Failed to set the port to blink.\n");
      }
    }
  }

  if (autoInstallRunTimeData->specificFileRetryCounter < (autoInstallRunTimeData->unicastRetryCnt * 2))
  {
...
void autoInstallPredefinedFileStateComplete1 (void)
{
  ...
  {
    autoInstallPredefinedFileStateComplete2();
    return;
  }
  ...
  {
    /* set 'predefinedFileRetryCounter' to 6, for AutoInstall to seek host.cfg */
...
void autoInstallPredefinedFileStateComplete2 (void)
{
  ...
  if (autoInstallRunTimeData->predefinedFileRetryCounter < (autoInstallRunTimeData->unicastRetryCnt * 2))
  {
...
static void autoInstallHostFileStateProcess(void)
{
  ...
  /* NOTE: Control reaches here in two cases:
   *
   * 1) to process <hostname>.cfg states
   * 2) to process host.cfg states -
   *   a) if host.cfg is sought after 6 tries of <hostname>.cfg,
   *      the 'predefinedFileRetryCounter' is less than 6, and 
   *      the 'hostFileRetryCounter' is 6. So, reset it as it is reused during host.cfg state processing.
   *   b) if host.cfg is sought after fp-net.cfg with <hostname> unresolved,
   *      the 'predefinedFileRetryCounter' is already initialized to 6, and 
   *      the 'hostFileRetryCounter' is already reset.
   */

  if ((autoInstallRunTimeData->predefinedFileRetryCounter == AUTO_INSTALL_TFTP_TOTAL_REQUESTS_NUMBER) ||
      (autoInstallRunTimeData->hostFileRetryCounter == AUTO_INSTALL_TFTP_TOTAL_REQUESTS_NUMBER))
  {
    memset(hostFileName, 0, AUTO_INSTALL_HOST_FILENAME_LENGTH);
    osapiStrncpy(hostFileName, AUTO_INSTALL_DEFAULT_CONFIG_FILE, AUTO_INSTALL_HOST_FILENAME_LENGTH-1);
    hostFileName[AUTO_INSTALL_HOST_FILENAME_LENGTH-1] = '\0';

    /* also copy file name in the run time config data */
    memset(autoInstallRunTimeData->hostFileName, 0 , sizeof(autoInstallRunTimeData->hostFileName));
    osapiStrncpySafe(autoInstallRunTimeData->hostFileName, AUTO_INSTALL_DEFAULT_CONFIG_FILE, AUTO_INSTALL_HOST_FILENAME_LENGTH);
...
void autoInstallHostFileStateComplete(void)
{
  ...
  if (autoInstallRunTimeData->hostFileRetryCounter < (autoInstallRunTimeData->unicastRetryCnt * 2))
  {
...
static void autoInstallAutoSaveStateProcess (void)
{
  ...
  if (autoInstallRunTimeData->autoSave == L7_TRUE)
  {
    if (nvStoreSaveCompleteRegister(L7_AUTO_INSTALL_COMPONENT_ID, autoInstallSaveConfigCompleteCallback)
        == L7_SUCCESS)
    {
    ...
    {
      autoInstallDebugTrace(AUTO_INSTALL_DBG_FLAG_ControlGroup,
                            "Registration for configuration save completion failed.\n");
      return;
    }
  }
  else 
...
static void autoInstallStoppedStateProcess (void)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall : process is stopped\n");
...
L7_RC_t autoInstallNotificationCallback(L7_bootp_dhcp_t *pBootConfigParams)
{
  ...
  if (osapiMessageSend(autoInstallMsgQueue, 
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "autoInstallNotificationCallback(): message send failed!\n");
    return L7_ERROR;
  }
  return L7_SUCCESS;
}
...
/*********************************************************************
* @purpose  Callback from nvStore when config save is completed.
*
* @param    event      event @b{(input)} success status of configuration save. 
*
* @returns  L7_RC_t  
*
* @notes   we register this notify function when saving configuration, using
*          the L7_AUTO_INSTALL_COMPONENT_ID. 
*       
* @end
*********************************************************************/
...
{
  if (event == REMOTE_SAVE_CFG_SUCCESS)
  {
    ...
    if (nvStoreSaveCompleteDeRegister(L7_AUTO_INSTALL_COMPONENT_ID) != L7_SUCCESS)
    {
      autoInstallDebugTrace(AUTO_INSTALL_DBG_FLAG_ControlGroup,
                          "Deregistration for configuration save completion failed.\n");
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_AUTO_INSTALL_COMPONENT_ID, "Failed to save autoInstalled configuration.\n");
  }
...
*/
  ...
  else
  {
    ...
    else if ( autoInstallRunTimeData->transferStatus == L7_FLASH_FAILED )
    {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall<->TFTP : Error writing file to memory: tftp://%s/%s (via %s) (No such file or directory)\n",
                ipCompleteString, completeFileName, bindIntfName);  
    }
    
    else if (autoInstallRunTimeData->transferStatus == L7_TRANSFER_FAILED)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "AutoInstall<->TFTP : Error opening tftp://%s/%s (via %s) (Timed out)\n",
              ipCompleteString, completeFileName, bindIntfName);  
    }
  }
...
L7_int32 autoInstallTmpScriptFileCreate(L7_uchar8 *pFileName)
{
  ...
  if (osapiFsOpen (pFileName, &cfgFileDesc) == L7_ERROR)
  {
    return L7_NULL;
  }
...
L7_RC_t autoInstallBootFileNameResolve(L7_char8 *configFileName)
{
  ...
  if (dnsClientReverseNameLookup(inetIp, hostname, &status, hostFQDN) == L7_SUCCESS )
  {
    ...
    /* NOTE: AUTO_INSTALL_HOST_FILENAME_LENGTH = 32 (filename) + 4 (.cfg) + 1 (null) */
    osapiStrncpy(configFileName, hostname, AUTO_INSTALL_HOST_FILENAME_LENGTH-5);
    osapiStrncat(configFileName, ".cfg", 4);
    configFileName[AUTO_INSTALL_HOST_FILENAME_LENGTH-1] = '\0';
  }
...
void autoInstallRestartTimerCallback()
{
  ...
  if (osapiMessageSend(autoInstallMsgQueue,
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "autoInstallRestartTimerCallback(): message send failed!\n");
  }
...
void autoInstallTransferStatusCallback(L7_TRANSFER_STATUS_t status)
{
  ...
  if (osapiMessageSend(autoInstallMsgQueue,
                       ...
                       L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_AUTO_INSTALL_COMPONENT_ID, "autoInstallTransferStatusCallback(): message send failed!\n");
    return;
  }
  return;
}
...
L7_RC_t autoInstallTftpFileNameParse(L7_char8 *incomingStr, L7_char8 *remoteFilePath, L7_char8 *remoteFileName)
{
  ...
  if ((pos = strrchr(pos, '/')) == L7_NULLPTR)
  {
      pos = incomingStr;
      pos = strrchr(pos, '\\');
  }

  /* if pos is still null then there's no directory in the name */
  if (pos != L7_NULLPTR)
  {
    slash = pos;
