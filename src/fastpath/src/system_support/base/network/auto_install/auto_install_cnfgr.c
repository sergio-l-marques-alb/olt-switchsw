...
void  *autoTxtCfgSyncSemaphore = L7_NULLPTR;
...
void autoInstallApiCnfgrCommand( L7_CNFGR_CMD_DATA_t *pCmdData )
{
  ...
  */
  ...
  else
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
              "autoInstallApiCnfgrCommand(): message send failed: event = %d\n", msg.event);
    }
  }
}
...
L7_RC_t autoInstallInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  /* create a task sync lock between emweb and auto-install while configuration 
     is being applied in emweb context */
  autoTxtCfgSyncSemaphore = osapiSemaBCreate(OSAPI_SEM_Q_FIFO, OSAPI_SEM_EMPTY);

  if ( (autoInstallCfgSemaphore == L7_NULLPTR) || (autoTxtCfgSyncSemaphore == L7_NULLPTR))
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallInit: autoInstallCfgSemaphore creation error.\n");
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallInit: autoInstallMsgQueue creation error.\n");
    cbData.correlator             = pCmdData->correlator;
    cbData.asyncResponse.rc       = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallInit: task creation error.\n");
    cbData.correlator             = pCmdData->correlator;
    cbData.asyncResponse.rc       = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallInit: Unable to initialize autoInstallTask()\n");
    cbData.correlator             = pCmdData->correlator;
    cbData.asyncResponse.rc       = L7_FAILURE;
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallInit: message send failed: event = %d\n", msg.event);
    return L7_FAILURE;
  }
                                           ...
                                           L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  */
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallCnfgrInitPhase2Process: Unable to register for dhcp callback\n");
    *pResponse  = 0;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallCnfgrInitPhase2Process: Unable to register for bootp callback\n");
    *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_AUTO_INSTALL_COMPONENT_ID,
            "autoInstallCnfgrInitPhase2Process: Unable to register for TFTP callback\n");
    *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
