...
L7_RC_t asyncEventHandlerInit()
{
    ...
    if (pAsyncHandlerDescr == L7_NULL)
    {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SIM_COMPONENT_ID,
                "Unable to initialize async event handler utility\n");
        L7_assert(1);
        return L7_FAILURE;
    }
...
L7_RC_t asyncEventCompleteTally(ASYNC_EVENT_COMPLETE_INFO_t *pComplete)
{
  ...
  if (pCorrelator == L7_NULLPTR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_ERROR, pHandlerEntry->handler.componentId,
              "Client %d, %s of async event handler %s responded to an invalid event "
              "with event sequence number %u.",
              pComplete->componentId, name, pHandlerEntry->handler.name, pComplete->correlator);
    return L7_FAILURE;
  }
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, pHandlerEntry->handler.componentId,
              "Found an idle correlator when handler %s processed event %u from client %u, %s.",
              pHandlerEntry->handler.name, pComplete->correlator, pComplete->componentId, name);
      osapiSemaGive(pHandlerEntry->correlatorLock);
      return L7_FAILURE;
  }
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, pHandlerEntry->handler.componentId,
                "Async event client %d, %s of handler %s unexpectedly signalled event %u completion.",
                pComplete->componentId, name, pHandlerEntry->handler.name, pComplete->correlator);
      osapiSemaGive(pHandlerEntry->correlatorLock);
      return L7_FAILURE;
  }
