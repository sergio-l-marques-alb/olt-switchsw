                       ...
                       handle_member_t    *handleListMemHndl)
{
  ...
  if(list == L7_NULL)
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, compId,
  ...
  *list = (handle_list_t*)osapiMalloc(compId, sizeof(handle_list_t));
  if ((*list) == L7_NULLPTR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, compId,
            "\n HANDLE LIST : Failed to allocate handle list memory for Component= %d, %s",
            compId, name);
    return L7_FAILURE;
  }
  ...
  if ((*list)->list == L7_NULLPTR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, compId,
            "\n HANDLE LIST : Component (%d, %s) returned invalid memory", compId, name);
    /* Return error as memory was not allocated */
    return L7_FAILURE;
  }
