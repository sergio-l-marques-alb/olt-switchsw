...
L7_BOOL cnfgrIsVlanIpSubnetPresent()
{
  ...
  switch (techType)
  {
    ...
    case L7_BASE_TECHNOLOGY_TYPE_BROADCOM_DNX:    /* PTin added: DNX family supported */
      retVal = L7_TRUE;
      break;
...
L7_BOOL cnfgrIsCliWebCustomizationFeaturePresent(L7_uint32 featureId)
{
  ...
  switch(featureId)
  {
    ...
    case L7_TXT_RUN_CFG_FEATURE_ID:
     /* This feature controls whether the management unit copies the
      * running configuration to the backup unit. FD_TXT_RUN_CFG_SUPPORTED 
      * is set on platforms that support the fast initialization feature. 
      * We also want to copy the running config file on stacks supporting 
      * non-stop forwarding. */
      result = FD_TXT_RUN_CFG_SUPPORTED || 
        cnfgrIsFeaturePresent(L7_FLEX_STACKING_COMPONENT_ID, L7_STACKING_NSF_FEATURE_ID);
      break;
  ...
  switch (featureId)
  {
    ...
    case L7_MLD_SNOOPING_FEATURE_SUPPORTED:
       featureSupported = (L7_FEAT_MLD_SNOOPING && ((techType == L7_BASE_TECHNOLOGY_TYPE_BROADCOM_XGS3) || 
                                                    ...
                                                    (techType == L7_BASE_TECHNOLOGY_TYPE_BROADCOM_DNX)));   /* PTin added: DNX family supported */
       break;
...
L7_BOOL cnfgrIsComponentPresent(L7_COMPONENT_IDS_t  componentId)
{
  ...
  switch (componentId)
  {
...
#endif
    case L7_DHCP_CLIENT_COMPONENT_ID:
      return L7_TRUE;
      break;
    ...
    case L7_DOT1X_AUTH_SERV_COMPONENT_ID:
      return L7_TRUE;
      break;
...
case L7_VOICE_VLAN_COMPONENT_ID:
  ...
  case L7_TR069_COMPONENT_ID:    
    ...
    case L7_PFC_COMPONENT_ID:
      return L7_TRUE;
      break;
#endif
#ifdef L7_LLPF_PACKAGE
    case L7_LLPF_COMPONENT_ID:
      return L7_TRUE;
      break;
#endif
 
#ifdef L7_TIMERANGES_PACKAGE
    case L7_TIMERANGES_COMPONENT_ID:
      return L7_TRUE;
      break;
#endif
...
static L7_BOOL cnfgrIsPoeFeaturePresent(L7_uint32 featureId)
{
  ...
  switch (featureId)
  {
    ...
    case L7_POE_FEATURE_PORT_DISCONNECT_TYPE:
      rc = L7_FEAT_POE_BRCM;
      break;
    case L7_POE_FEATURE_SYSTEM_POE_AT_SUPPORT:
...
static L7_BOOL cnfgrDot1xIsFeaturePresent(L7_uint32 featureId)
{
  ...
  switch (featureId)
  {
    ...
    break;
    case L7_DOT1X_DYNAMIC_VLANASSIGN_FEATURE_ID:
       featurePresent = L7_FEAT_DOT1X_DYNAMIC_VLANASSIGN;
    break;
...
/*********************************************************************
* @purpose  Check to see if a feature is present for PFC component
*
* @param    L7_uint32   featureId
*
* @returns  L7_TRUE
...
* @end
*********************************************************************/
static L7_BOOL cnfgrPfcIsFeaturePresent(L7_uint32 featureId)
{
  L7_BOOL rc = L7_FALSE;
  ...
  if (L7_BASE_TECHNOLOGY_SUBTYPE_BROADCOM_XGS4_5682x  != techSubType)
  {
    rc = L7_FALSE; 
  }
  else if (L7_PFC_FEATURE_ID_TOTAL > featureId )
  {
    switch (featureId)
    {
      case L7_PFC_PORT_PRIORITY_FLOW_CONTROL_FEATURE_ID:
        rc = L7_TRUE;
        break; 
      default:
        rc = L7_FALSE;
...
#ifdef L7_LLPF_PACKAGE
/*********************************************************************
* @purpose  Check to see if a feature is present for LLPF component
*
* @param    L7_uint32   featureId
*
* @returns  L7_TRUE
...
* @end
*********************************************************************/
static L7_BOOL cnfgrLlpfIsFeaturePresent(L7_uint32 featureId)
{
  L7_BOOL rc = L7_FALSE;
  ...
  switch (featureId)
  {
  case  L7_LLPF_BLOCK_ISDP_FEATURE_ID:                 /* ISDP */
  case  L7_LLPF_BLOCK_VTP_FEATURE_ID:                  /* VTP */
  case  L7_LLPF_BLOCK_DTP_FEATURE_ID:                  /* DTP */
  case  L7_LLPF_BLOCK_UDLD_FEATURE_ID:                 /* UDLD */
  case  L7_LLPF_BLOCK_PAGP_FEATURE_ID:                 /* PAGP */
  case  L7_LLPF_BLOCK_SSTP_FEATURE_ID:                 /* SSTP */
  case  L7_LLPF_BLOCK_ALL_FEATURE_ID:                  /* ALL */
    techType = cnfgrBaseTechnologyTypeGet();
    ...
    case L7_BASE_TECHNOLOGY_TYPE_BROADCOM_ROBO:
      ...
      /* For Broadcom silicon, only devices w/ a VFP may support this feature. 
         Additionally, the VFP must have the capability of setting the 
         priority COS for packets copied to the CPU (This means that FB2
  ...
  switch (featureId)
  {
    ...
    case L7_FDB_AGETIME_PER_VLAN_FEATURE_ID:
      switch (techType)
      {
        ...
        case L7_BASE_TECHNOLOGY_TYPE_BROADCOM_DNX : /* fallthrough */     /* PTin added: DNX  family supported */
          return L7_FALSE;
          break;
...
static L7_BOOL cnfgrNimIsFeaturePresent(L7_uint32 featureId)
{
  ...
  else if(featureId == L7_NIM_AUTONEG_WITH_SPEED_DUPLEX_FEATURE_ID)
  { 
    return L7_FEAT_ETHERNET_SPEED_DUPLEX_ENHANCEMENTS;
  }
  return L7_FALSE;
}
...
static L7_BOOL cnfgrSimIsFeaturePresent(L7_uint32 featureId)
{
  ...
  else if (featureId == L7_SIM_NETWORK_PORT_FEATURE_ID)
  {
...
/*********************************************************************
* @purpose  Check to see if a feature is present for Dhcp relay component
*
* @param    L7_uint32   featureId
*
* @returns  L7_TRUE
...
* @end
*********************************************************************/
static L7_BOOL cnfgrIsDhcpRelayFeaturePresent(L7_uint32 featureId)
{
  if (featureId > L7_RELAY_FEATURE_ID_TOTAL)
     return L7_FALSE;

  if (featureId == L7_RELAY_OPTION82_INTF_FEATURE_ID)
     return L7_FEAT_DHCP_RELAY_OPTION82_ENHANCEMENTS; 
...
L7_BOOL cnfgrIsFeaturePresent(L7_COMPONENT_IDS_t  componentId, L7_uint32 featureId)
{
  ...
  case L7_DHCP_RELAY_COMPONENT_ID:
       retVal = cnfgrIsDhcpRelayFeaturePresent(featureId);
       break;
  case L7_PFC_COMPONENT_ID:
      retVal = cnfgrPfcIsFeaturePresent(featureId);
      break;

#ifdef L7_LLPF_PACKAGE
  case L7_LLPF_COMPONENT_ID:
      retVal = cnfgrLlpfIsFeaturePresent(featureId);
      break;
#endif
    default:
      retVal = L7_FALSE;
...
L7_BASE_TECHNOLOGY_TYPES_t cnfgrBaseTechnologyTypeGet(void)
{
  ...
  switch (npdType)
  {
    ...
    case __BROADCOM_88650_ID: /* PTin added: new switch 88650 (ARAD) */
      tType = L7_BASE_TECHNOLOGY_TYPE_BROADCOM_DNX;      /* PTin added: DNX family supported */
      break;
