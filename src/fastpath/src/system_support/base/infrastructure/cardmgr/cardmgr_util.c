                                ...
                                L7_enetMacAddr_t         lastMgrKey)
{
  ...
  LOG_PT_TRACE(LOG_CTX_STARTUP, "Initializing slots: maxSlots=%u", maxSlots);

  /* Insert only logical cards and local cards to the driver */
  for (slot = 0 ; slot <= maxSlots; slot++ )
  {
    LOG_PT_TRACE(LOG_CTX_STARTUP, "Initializing slots: slot=%u", slot);
    ...
    LOG_PT_TRACE(LOG_CTX_STARTUP, "slot=%u", slot);
    ...
    {
      LOG_PT_TRACE(LOG_CTX_STARTUP, "Inserting card of slot=%u", slot);
...
L7_RC_t cmgrInitSlotsOnUnit(L7_uint32 unit)
{
  ...
  LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrInitSlotsOnUnit: init");

  for (slot = 0; slot <= cmgrCMDBUnitMaxSlotsGet(unit); slot++)
  {
    LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrInitSlotsOnUnit: slot=%d", slot);
    ...
    LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrInitSlotsOnUnit: pluggable=%d", unit_entry.physSlot[sx].pluggable);

    if (unit_entry.physSlot[sx].pluggable != L7_TRUE)
    {
      for (i = 0; i < unit_entry.physSlot[sx].num_supported_card_types; i++)
      {
        LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrInitSlotsOnUnit: supported_card[%d] = %d", i, unit_entry.physSlot[sx].supported_cards[i]);
                                     ...
                                     SYSAPI_HPC_UNIT_DESCRIPTOR_t *unit_entry)
{
  ...
  LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrUnitEntryAndSlotIndexGet: unit=%d slot=%d", unit, slot);
  ...
  {
    LOG_PT_TRACE(LOG_CTX_STARTUP, "cmgrUnitEntryAndSlotIndexGet: slot_index=%d, u_entry->physSlot[slot_index].slot_number=%d",
