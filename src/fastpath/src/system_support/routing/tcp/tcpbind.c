...
static char __fileid[] = "$Header: lvl7dev\src\system_support\routing\tcp\tcpbind.c 1.2.3.9.1.5 2002/12/11 16:14:31EST dcbii Exp  $";
#endif
...
e_Err TcpUdpBind_AddMulticastAddr(IN t_Handle connId, ulng intIfNum, ulng addr)
{
   ...
   if(conn->localAddress != INADDR_ANY)
   {
     ...
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
             "Failure adding OSPF to multicast group %s on interface %s."
             " Socket not bound to INADDR_ANY.", groupStr, ifName2);
     return E_BADPARM;
   }
    ...
    if (osapiIfNameStringGet(intIfNum, ifName, IFNAMSIZ) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
              "Failure adding OSPF to multicast group %s on interface %s."
              " Failed to get interface name.", groupStr, ifName2);
    ...
    if (ipstkStackIfIndexGet(ifName, &ifIndex) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
              "Failure adding OSPF to multicast group %s on interface %s."
              " Failed to get interface index.", groupStr, ifName2);
      return E_FAILED;
    }
...
e_Err TcpUdpBind_DropMulticastAddr(IN t_Handle connId, ulng intIfNum, ulng addr)
{
  ...
  nimGetIntfName(intIfNum, L7_SYSNAME, ifName2);
   ...
   if(conn->localAddress != INADDR_ANY)
   {
     ...
     osapiInetNtoa(addr, groupStr);
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
             "Failure removing OSPF from multicast group %s on interface %s."
             " Socket not bound to INADDR_ANY.", groupStr, ifName2);
     return E_BADPARM;
   }
