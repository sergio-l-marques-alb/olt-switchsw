...
static char* restartReasonStr(int restartReason)
{
...
ospfLsaOptionsDecode(L7_char8 * options, outputFuncPtr output, void * context)
{
  ...
  if (opt == 0)
  {
    ...
    do {
      ...
      {
        osapiStrncat(outBuf, v2lsaOptOBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2lsaOptDCBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2lsaOptEABitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2lsaOptNPBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2lsaOptMCBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2lsaOptEBitStr, sizeof(outBuf));
...
ospfRouterLsaBitsDecode(L7_char8 * rtrLsa, outputFuncPtr output, void * context)
{
  ...
  int bits = *rtrLsa & L7_OSPF_LSA_ROUTER_KNOWNBITS_MASK;
    ...
    do {
      ...
      {
        osapiStrncat(outBuf, v2rtrLsaNtBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2rtrLsaWBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2rtrLsaVBitStr, sizeof(outBuf));
      ...
      {
        osapiStrncat(outBuf, v2rtrLsaEBitStr, sizeof(outBuf));
      ...
      {
         osapiStrncat(outBuf, v2rtrLsaBBitStr, sizeof(outBuf));
...
ospfASExternalLsaDecode(L7_char8 *aseLsa, outputFuncPtr output, void * context, L7_uint32 lsaLength)
{
  ...
  if(flags)
  {
    ...
    {
      osapiStrncat(optBuf, v2aseLsaOptionsEBitStr, sizeof(optBuf));
...
L7_RC_t
ospfLinkOpaqueLsaDecode(L7_char8 *linkOpaqueLsa, outputFuncPtr output, void * context, L7_uint32 lsaLength)
{
  L7_uchar8 opaqueType = *(linkOpaqueLsa + L7_OSPF_LSA_HDR_LSID_OFFSETOF);
  L7_uchar8 opaqueId = A_GET_3B(linkOpaqueLsa + L7_OSPF_LSA_HDR_LSID_OFFSETOF + 1);
  char outBuf[255];


  if ((opaqueType == L7_OSPF_OPAQUE_LSA_TYPE_GRACE) && (opaqueId == 0))
  {
    /* Looks like a grace LSA */
    L7_uchar8 *firstTlv = (L7_uchar8*) linkOpaqueLsa + L7_OSPF_LSA_HDR_LENGTH;
    L7_uchar8 *pos = firstTlv;
    L7_uint32 lsaLength = A_GET_2B(linkOpaqueLsa + L7_OSPF_LSA_HDR_LENGTH_OFFSETOF);
    L7_ushort16 tlvType;
    L7_ushort16 tlvLength;        /* does not include type and length fields */
    L7_ushort16 padLen;
    L7_uint32 gracePeriod;
    L7_uchar8 restartReason;
    L7_uint32 ipAddr;
    L7_uchar8 ipAddrStr[OSAPI_INET_NTOA_BUF_SIZE];

    while ((pos - firstTlv) < (lsaLength - L7_OSPF_LSA_HDR_LENGTH))
    {
      ...
      switch (tlvType)
      {
        case L7_OSPF_GR_TLV_GRACE_PERIOD:
          gracePeriod = A_GET_4B(pos);
          sprintf(outBuf, " Grace Period: %u seconds\r\n", gracePeriod);
          output(context, outBuf);
          break;

        case L7_OSPF_GR_TLV_RESTART_REASON:
          restartReason = *pos;
          sprintf(outBuf, " Restart Reason: %s\r\n", restartReasonStr(restartReason));
          output(context, outBuf);
          break;

        case L7_OSPF_GR_TLV_IP_ADDRESS:
          ipAddr = A_GET_4B(pos);
          osapiInetNtoa(ipAddr, ipAddrStr);
          sprintf(outBuf, " IP Address: %s\r\n", ipAddrStr);
          output(context, outBuf);
          break;
