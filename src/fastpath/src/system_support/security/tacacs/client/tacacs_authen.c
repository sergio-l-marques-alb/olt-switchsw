...
static L7_BOOL tacacsDebugFlag = L7_FALSE;

#define TACACS_DEBUG(__fmt__, __args__...)            \
  if (tacacsDebugFlag == L7_TRUE)                     \
  {                                                   \
    sysapiPrintf((L7_char8 *)__fmt__, ## __args__);   \
  }
                             ...
                             tacacsAuthSession_t **session)
{
  ...
  if (sess == L7_NULLPTR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: failed to create authentication session, no buffers available.");
    return L7_FAILURE;
  }
                           ...
                           tacacsAuthStatus_t   status)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: Failed to send failure response to component %i, not registered",
            session->componentId);
  }
...
static void tacacsAuthSuccessSend(tacacsAuthSession_t *session)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: Failed to send success response to component %i, not registered",
            session->componentId);
  }
...
static void tacacsAuthChallengeSend(tacacsAuthSession_t *session, L7_uchar8 *pkt)
{
  ...
  *((void **)session->pState) = session;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: Failed to send challenge response to component %i, not registered",
            session->componentId);
    tacacsAuthSessionDelete(session);
  }
...
void tacacsContactNextServer(tacacsAuthSession_t *session)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING,  L7_TACACS_COMPONENT_ID, "TACACS+: authentication error, no "
                                      ...
                                      L7_uint32 socket)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "failed to find session for server connection.");
  return;
}
                                 ...
                                 L7_uint32            serverIdx)
{
  ...
  if (taskId == L7_ERROR)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: authentication error, failed to create connection task.");
...
void tacacsAuthenticationStart(tacacsAuthSession_t *session)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: failed to create authentication session, no server to contact.");
    tacacsAuthFailureSend(session, TACACS_STATUS_SERVER_FAILURE);
    return;
  }
  session->server = tacacsCfg->servers[serverIdx].ip;
...
static void tacacsAuthStateConnectedProcess(tacacsAuthSession_t *session)
{
  ...
  if (tacacsOprData->unencrypted != L7_TRUE)
  {
    ...
    if (strlen(key) == 0)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: no key configured to encrypt packet for server %s.",
              osapiInet_ntoa(tacacsCfg->servers[serverIdx].ip));
      tacacsContactNextServer(session);
      return;
    }
                                           ...
                                           L7_uchar8 *pkt)
{
  ...
  if (tacacsOprData->unencrypted != L7_TRUE)
  {
    ...
    {
      /* no key to use, try another server */
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: no key configured to encrypt CONTINUE packet for server %s.",
              osapiInet_ntoa(tacacsCfg->servers[serverIdx].ip));
      tacacsContactNextServer(session);
      return;
    }
...
void tacacsAuthResponseProcess(tacacsRxPacket_t *rxPacket)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: received response packet for unknown session.");
    return;
  }
  TACACS_DEBUG("\nTACACS+: TACACS AuthResponse for session %d\n", session->sessionId);

  /* once we have a session, all errors should be handled with session */

  /* verify packet type */
  type = (*(pkt + L7_TACACS_PKT_TYPE_OFFSET));
  TACACS_DEBUG("TACACS+: type = 0x%x\n", type);

  /* get corresponding server configuration */
  if (tacacsServerIndexGet(session->server, &serverIdx) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: failed to find server to process response.");
    tacacsContactNextServer(session);
    return;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: received invalid unencrypted packet.");
    tacacsContactNextServer(session);
    return;
  }
  ...
  if (unencrypt == 0)
  {
    ...
    {
      /* no key to use, try another server */
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: no key configured to unencrypt REPLY packet for server %s.",
              osapiInet_ntoa(tacacsCfg->servers[serverIdx].ip));
      tacacsContactNextServer(session);
      return;
    }
    tacacsEncryptDecrypt(pkt, rxPacket->length, key);
    TACACS_DEBUG("TACACS+: Decrypted packet.\n");
  }
  ...
  {
	L7_LOGF(L7_LOG_SEVERITY_NOTICE,  L7_TACACS_COMPONENT_ID, "TACACS+: invalid major version in"
												    " received packet. Major version mismatch.");
    tacacsContactNextServer(session);
    return;
  }
  TACACS_DEBUG("TACACS+: MAJOR_VER = 0x%x\n", (*(pkt + L7_TACACS_PKT_VERSION_OFFSET)) >> L7_TACACS_MAJOR_VER_SHIFT);

  if (((*(pkt + L7_TACACS_PKT_VERSION_OFFSET))
       & L7_TACACS_MINOR_VER_MASK) != L7_TACACS_MINOR_VER_DEFAULT)
  {
	L7_LOGF(L7_LOG_SEVERITY_NOTICE,  L7_TACACS_COMPONENT_ID, "TACACS+: invalid minor version in"
												    " received packet. Minor version mismatch.");
    tacacsContactNextServer(session);
    return;
  }
  TACACS_DEBUG("TACACS+: MINOR_VER = 0x%x\n", (*(pkt + L7_TACACS_PKT_VERSION_OFFSET)) & L7_TACACS_MINOR_VER_MASK);

  if ((*(pkt + L7_TACACS_PKT_SEQ_NO_OFFSET)) != (session->sequenceNo + 1))
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: invalid sequece number in received packet.");
    tacacsContactNextServer(session);
    return;
  }
  TACACS_DEBUG("TACACS+: PKT_SEQ_NO = %d\n", (*(pkt + L7_TACACS_PKT_SEQ_NO_OFFSET)));

  status = (*(pkt + L7_TACACS_REPLY_STATUS_OFFSET));
  msgLen = osapiNtohs(*((L7_ushort16 *)(pkt + L7_TACACS_REPLY_SRV_MSG_LEN_OFFSET)));
  dataLen = osapiNtohs(*((L7_ushort16 *)(pkt + L7_TACACS_REPLY_DATA_LEN_OFFSET)));

  TACACS_DEBUG("TACACS+: rxPacket->length = %d\n", rxPacket->length);
  TACACS_DEBUG("TACACS+: status = 0x%x\n", status);
  TACACS_DEBUG("TACACS+: msgLen = %d\n", msgLen);
  TACACS_DEBUG("TACACS+: dataLen = %d\n", dataLen);

  if (rxPacket->length != (L7_TACACS_REPLY_FIXED_FIELDS + msgLen + dataLen) && type == L7_TACACS_AUTHEN_LOGIN)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: length fields (%d) and packet length (%d) do not match.\n", rxPacket->length, (L7_TACACS_REPLY_FIXED_FIELDS + msgLen + dataLen));
    tacacsContactNextServer(session);
    return;
  }
  ...
  {
    TACACS_DEBUG("TACACS+: type = L7_TACACS_AUTHEN_LOGIN\n");
    switch (status)
    {
      case L7_TACACS_AUTHEN_STATUS_PASS:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHEN_STATUS_PASS\n");
        tacacsAuthSuccessSend(session);
        tacacsTimerDelete(); 
        break;
      case L7_TACACS_AUTHEN_STATUS_FAIL:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHEN_STATUS_FAIL\n");
        /* just send failure response back to requesting component */
        tacacsAuthFailureSend(session, TACACS_STATUS_AUTH_FAILURE);
        tacacsTimerDelete(); 
        break;
      case L7_TACACS_AUTHEN_STATUS_GETPASS:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHEN_STATUS_GETPASS\n");
        tacacsAuthStatusGetPassProcess(session,
                                       pkt);
        break;
      case L7_TACACS_AUTHEN_STATUS_GETUSER:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHEN_STATUS_GETUSER\n");
      ...
      default:
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: received unsupported authentication reply status %i.\n",
                status);
        tacacsContactNextServer(session);
        break;
    }
  ...
  {
    if (type == L7_TACACS_AUTHEN_SENDPASS) {
      TACACS_DEBUG("TACACS+: type = L7_TACACS_AUTHEN_SENDPASS\n");
    }
    else if (type == L7_TACACS_AUTHEN_TERMINATE) {
      TACACS_DEBUG("TACACS+: type = L7_TACACS_AUTHEN_TERMINATE\n");
    }

    switch (status)
    {
      case L7_TACACS_AUTHOR_STATUS_PASS_ADD:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHOR_STATUS_PASS_ADD\n");
        tacacsAuthSuccessSend(session);
        break;
      ...
      case L7_TACACS_AUTHOR_STATUS_FAIL:
        TACACS_DEBUG("TACACS+: status = L7_TACACS_AUTHOR_STATUS_FAIL\n");
        tacacsAuthFailureSend(session, TACACS_STATUS_AUTH_FAILURE);
        break;
      case L7_TACACS_AUTHOR_STATUS_ERROR:
      case L7_TACACS_AUTHOR_STATUS_FOLLOW:
      default:
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_TACACS_COMPONENT_ID, "TACACS+: received unsupported authorization reply status %i.\n",
                status);
        tacacsContactNextServer(session);
        break;
    }
...
/*********************************************************************
*
* @purpose To enable debug of processed authentication response packets
*
* @param   L7_BOOL flag  @b{(input)}  debug flag
*
* @returns void
*
* @comments
...
*
*********************************************************************/
void tacacsEnableDebug(L7_BOOL flag)
{
  tacacsDebugFlag = flag;
}

void tacacshelp()
{
  sysapiPrintf("TACACS+ debug help:\n\n");
  sysapiPrintf("Turn on TACACS+ Debug mode:  devshell tacacsEnableDebug(1) (0 to disable)\n");
  sysapiPrintf("Dump User Manager config:    devshell userMgrConfigDump\n");
}
