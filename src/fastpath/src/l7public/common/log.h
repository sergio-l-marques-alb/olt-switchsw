...
#define LOG_USER_TRACE(__comp__,__fmt__, __args__... )                      \
  if (logCnfgrStateCheck() == L7_TRUE)                                      \
  {                                                                         \
    if (logger_check(fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(L7_LOG_SEVERITY_DEBUG))) \
        logger_print(fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(L7_LOG_SEVERITY_DEBUG) , NULL, __FUNCTION__, __LINE__, "\r\n", __fmt__, ##__args__ );\
  }
#endif
...
#else
#define L7_LOGF(__sev__, __comp__, __fmt__,__args__...)                         \
        {if (logger_check(fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(__sev__))) \
            logger_print( fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(__sev__) , NULL, __FUNCTION__, __LINE__, "\r\n", __fmt__, ##__args__ );}
#endif
