...
#define VALG_LOG_PATH                           "/usr/local/ptin/log/fastpath/bin"      /* PTin modified: paths */
...
typedef enum
{
    L7_LOG_EMAIL_ALERT_TLSV1=1,
    L7_LOG_EMAIL_ALERT_NONE
} L7_LOG_EMAIL_ALERT_SECURITY_t;

typedef enum
{
    L7_LOG_EMAIL_ALERT_PORT_NONE = 25,
    L7_LOG_EMAIL_ALERT_PORT_TLSV1 = 465,
} L7_LOG_EMAIL_ALERT_SERVER_PORT_t;

typedef enum
{
  L7_LOG_EMAIL_ALERT_URGENT=1, /*  Urgent  messages    */
  L7_LOG_EMAIL_ALERT_NON_URGENT, /* Non Urgent Messages */
  L7_LOG_EMAIL_ALERT_MAX_MSG_TYPES /* Max no of Msg Types*/  
} L7_LOG_EMAIL_ALERT_MSGTYPE_t;

#define   L7_LOG_EMAIL_ALERT_BOTH_MSGTYPES  4 /* both Message types */


typedef enum
{
  L7_LOG_EMAIL_ALERT_EMAIL_ADDRESS_INVALID=1, /*  Invalid email Address */
  L7_LOG_EMAIL_ALERT_MAX_EMAIL_ADDRESSES_CONFIGURED, /* This error condition is to show that max email addresses are configured. */
  L7_LOG_EMAIL_ALERT_EMAIL_ADDRESS_INVALID_LENGTH,
  L7_LOG_EMAIL_ALERT_SUBJECT_INVALID
...
#define LOG_USER_TRACE(__comp__,__fmt__, __args__... )                      \
  if (logCnfgrStateCheck() == L7_TRUE)                                      \
  {                                                                         \
    if (logger_check(fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(L7_LOG_SEVERITY_DEBUG))) logger_print( fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(L7_LOG_SEVERITY_DEBUG) , NULL, __FUNCTION__, __LINE__, __fmt__, ##__args__ );\
  }
#endif
...
#else
#define L7_LOGF(__sev__, __comp__, __fmt__,__args__...)                         \
        {if (logger_check(fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(__sev__))) logger_print( fp_to_ptin_component(__comp__), fp_to_ptin_logger_verbosity(__sev__) , NULL, __FUNCTION__, __LINE__, __fmt__, ##__args__ );}
#endif
...
#define L7_assert(test)  \
        if ((test) !=0)    \
        { L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_LOG_COMPONENT_DEFAULT, "Assertion failed"); }
#endif
