...
#else
  #define L7_ACL_MAX_LISTS_PER_VLAN_DIR     min(L7_ACL_MAX_LISTS, L7_ACL_MAX_RULES_PER_MULTILIST)
#endif
#if (L7_ACL_MAX_RULES_PER_MULTILIST == 0)
  #define L7_ACL_MAX_LISTS_PER_INTF_DIR     L7_ACL_MAX_LISTS
#else
  #define L7_ACL_MAX_LISTS_PER_INTF_DIR     min(L7_ACL_MAX_LISTS, L7_ACL_MAX_RULES_PER_MULTILIST)
#endif  
...
/*Maximum number of applied ACL rules that can be time range */
#ifndef L7_ACL_TIME_RANGE_RULE_PER_INTF_DIR_LIMIT
  #define L7_ACL_TIME_RANGE_RULE_PER_INTF_DIR_LIMIT  L7_ACL_MAX_RULES_PER_LIST
#endif
...
/* Approximately 136 bytes per rule to define the TLVs. */
#define L7_QOS_ACL_TLV_MAX_SIZE                 (136 * L7_ACL_MAX_RULES_PER_MULTILIST)
...
/*Time Based ACLs*/
#define L7_QOS_ACL_TLV_RULE_STATUS              0x0623
...
/* Time Based ACLs Time Range attribute in TLV */
typedef struct
{
  L7_ushort16   status; 
  L7_uint32     timeBasedCorrelator;            /* time based ACL rule correlator value*/
  L7_uchar8     rsvd1[3];                       /* reserved -- for alignment  */
}aclTlvRuleStatus_t;

#define L7_QOS_ACL_TLV_RULE_STATUS_LEN ((L7_uint32)sizeof(aclTlvRuleStatus_t))

typedef enum
{
  L7_TIMERANGE_EVENT_ACTIVATE = 1,
  L7_TIMERANGE_EVENT_DEACTIVATE,
  L7_TIMERANGE_EVENT_DELETE
} aclTimeRangeEvents_t;
...
typedef enum
{
  ...
  ACL_IPDSCP,
  ACL_FLOWLBLV6,
  ACL_TIME_RANGE_NAME,                 /* Time Range Name for ACL Rule */
...
typedef enum
{
  ...
  ACL_MAC_VLANID2_END,
  ACL_MAC_TIME_RANGE_NAME,             /* Time range name for an MAC ACL Rule */
...
/* ACL change approval types */
typedef enum
{
  L7_ACL_CHANGE_APPROVAL_LIST_CREATE = 1,
  L7_ACL_CHANGE_APPROVAL_LIST_DELETE,
  L7_ACL_CHANGE_APPROVAL_RULE_CREATE,
  L7_ACL_CHANGE_APPROVAL_RULE_DELETE
...
/*********************************************************************
* @purpose  Callback function signature to approve an ACL change request.
*
* @param    aclChange   @b{(input)} ACL change approval ID
* @param    aclType     @b{(input)} Type of ACL
* @param    aclIndex    @b{(input)} ACL index value (can be ACL number or index)
* @param    aclName     @b{(input)} Name of this ACL
* @param    pData       @b{(input)} Ptr to request-specific data, or L7_NULLPTR
*
* @returns  L7_SUCCESS  Allow change
* @returns  L7_FAILURE  Do not allow change
*
* @comments This function is invoked by the QOS ACL component based on component
*           callback registration.
*
...
* @end
*********************************************************************/
typedef L7_RC_t (*ACL_CHANGE_APPROVAL_FUNCPTR_t) (aclChangeApproval_t aclChange,
                                                  L7_ACL_TYPE_t aclType,
                                                  L7_uint32 aclIndex, 
                                                  L7_uchar8 *aclName,
                                                  void *pData);
...
*
*********************************************************************/
extern L7_RC_t aclNumRulesGet(L7_uint32 aclnum, L7_uint32 *ruleCount);
...
* @returns  L7_TABLE_IS_FULL, if the maximum number of ACLs already created
*
...
*
*********************************************************************/
extern L7_RC_t aclCommonNumRulesGet(L7_ACL_TYPE_t aclType, L7_uint32 aclnum,
...
* @returns  L7_REQUEST_DENIED access list config change is not approved
...
* @returns  L7_REQUEST_DENIED access list config change is not approved
* @returns  L7_TABLE_IS_FULL  storage unavailable for rule
*
* @comments An L7_REQUEST_DENIED response means another application   
*           has denied approval of this configuration change.
*
...
*
* @param    aclIndex    @b{(input)}  access list index to begin search
* @param    *ruleCount  @b{(output)} pointer to rule count output location
*
* @returns  L7_SUCCESS
* @returns  L7_ERROR    access list does not exist
* @returns  L7_FAILURE
...
*
*********************************************************************/
L7_RC_t aclMacNumRulesGet(L7_uint32 aclIndex, L7_uint32 *ruleCount);
...
* @param    timeRangeId  @b{(output)}   the time-range name is a string
...
* @param    aclnum          @b{(input)}   the access-list identifying number
* @param    rulenum        @b{(input)}    the acl rule number
* @param    timeRangeName  @b{(input)}   the time-range name is a string
* identifying the  time range associated with the  ACL Rule..
...
/*********************************************************************
*
* @purpose  To get the time range name assigned to access list rule.
*
* @param    aclIndex        @b{(input)}  access list index
* @param    rulenum         @b{(input)}  ACL rule number
* @param    *timeRangeName  @b{(output)} the time-range name is a string
*                           identifying the  time range associated with the  ACL Rule
*
* @returns  L7_SUCCESS  if the timerange name is retrieved
* @returns  L7_ERROR    access list or rulenum does not exist
...
*
*********************************************************************/
L7_RC_t aclMacRuleTimeRangeNameGet(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uchar8 *timeRangeName);
/*********************************************************************
*
* @purpose  To add the TimeRange Name for an access list rule.
*
* @param    aclIndex       @b{(input)}   the access-list identifying number
* @param    rulenum        @b{(input)}   the acl rule number
* @param    timeRangeName  @b{(input)}   the time-range   the time-range name
* is a string identifying the  time range associated with the  ACL Rule.
* @returns  L7_SUCCESS, if timerange name is added
* @returns  L7_FAILURE, if invalid queue id or other failure
* @returns  L7_ERROR, if the access-list or rule does not exist
* @returns  L7_REQUEST_DENIED, if hardware update failed
...
*
*********************************************************************/
L7_RC_t aclMacRuleTimeRangeNameAdd(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uchar8 *timeRangeName);
/*********************************************************************
*
...
*
*********************************************************************/
L7_RC_t aclMacRuleStatusGet(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uint32 *status);
...
/* ACL Rule FLAGS for correlator table */
#define ACL_RULE_NONE 0x0
#define ACL_RULE_LOG 0x1
#define ACL_RULE_TIME_RANGE 0x2
