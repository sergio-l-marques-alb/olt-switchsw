...
* @function fpObjGet_SwitchingStpMstConfig_MstIdWithOutCST

*

* @purpose Get 'MstIdWithOutCST'

*

* @description [MstIdWithOutCST]: The MSTP instance ID
...
xLibRC_t fpObjGet_SwitchingStpMstConfig_MstIdWithOutCST (void *wap, void *bufp)

{

  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));

  xLibU32_t objMstIdWithOutCSTValue;

  xLibU32_t nextObjMstIdWithOutCSTValue;

  FPOBJ_TRACE_ENTER (bufp);



  /* retrieve key: MstIdWithOutCST */

  owa.rc = xLibFilterGet (wap, XOBJ_SwitchingStpMstConfig_MstIdWithOutCST,

                          (xLibU8_t *) & objMstIdWithOutCSTValue, &owa.len);
...

  {
    ...
    owa.l7rc = usmDbDot1sInstanceFirstGet(L7_UNIT_CURRENT, &nextObjMstIdWithOutCSTValue);

    if (nextObjMstIdWithOutCSTValue == 0)
      
     owa.l7rc = usmDbDot1sInstanceNextGet (L7_UNIT_CURRENT, objMstIdWithOutCSTValue,

                                          &nextObjMstIdWithOutCSTValue);
  ...
  else
 {
    ...
    FPOBJ_TRACE_CURRENT_KEY (bufp, &objMstIdWithOutCSTValue, owa.len);
  ...
  FPOBJ_TRACE_NEW_KEY (bufp, &nextObjMstIdWithOutCSTValue, owa.len);



  /* return the object value: MstIdWithOutCST */

  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) & nextObjMstIdWithOutCSTValue,

                           sizeof (objMstIdWithOutCSTValue));
...
/*******************************************************************************
* @function fpObjList_SwitchingStpMstConfig_MstIdWithOutCST
*
* @purpose Get 'MstIdWithOutCST'
*
* @description [MstIdWithOutCST]: MST Instance
*
* @return
*******************************************************************************/
xLibRC_t fpObjList_SwitchingStpMstConfig_MstIdWithOutCST(void *wap,void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objMstIdWithOutCSTValue;
  xLibU32_t nextObjMstIdWithOutCSTValue;
...

{
  ...
  /* retrieve key: MstId */
  kwaMstId.rc = xLibFilterGet (wap, XOBJ_SwitchingStpMstConfig_MstId, (xLibU8_t *)&mstId, &kwaMstId.len);
  ...
  FPOBJ_TRACE_CURRENT_KEY (bufp, &mstId, kwaMstId.len);
...

  {
    ...
    owa.l7rc = usmDbDot1sMstiVIDNextGet(L7_UNIT_CURRENT, mstId, objIDValue, &nextObjIDValue);
  }
...

{
  ...
  xLibU32_t objMstVlanRowStatusValue;
  fpObjWa_t kwaMstId = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t keyMstIdValue,tempMstIdValue,keyVlanIndex=0;
  fpObjWa_t kwaVlanIndex = FPOBJ_INIT_WA (sizeof (L7_VLAN_MASK_t));
  L7_VLAN_MASK_t keyVlanMask;
  L7_VLAN_MASK_t vlan_mask;
  L7_BOOL flag = L7_FALSE;
  L7_BOOL error = L7_FALSE, failure = L7_FALSE;
  ...
  if (objMstVlanRowStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
    ...
    {
      if (usmDbDot1sVlanToMstiConfiguredGet(L7_UNIT_CURRENT, keyVlanIndex,
      ...
      {
        L7_VLAN_SETMASKBIT(vlan_mask, keyVlanIndex);
    ...
    owa.l7rc = usmDbDot1sMstiVlanAdd(L7_UNIT_CURRENT, keyMstIdValue, &vlan_mask);
    if (owa.l7rc != L7_SUCCESS)
    {
      owa.rc = XLIBRC_MST_VLAN_ADD_FAILURE;    /* TODO: Change if required */
      FPOBJ_TRACE_EXIT (bufp, owa);
  ...
  else if (objMstVlanRowStatusValue == L7_ROW_STATUS_DESTROY)
  {
    ...
    while (kwa.l7rc == L7_SUCCESS)
    {
      if (usmDbDot1sInstanceVlanGet(L7_UNIT_CURRENT, keyMstIdValue, keyVlanIndex) != L7_SUCCESS)
      {
        error = L7_TRUE;
      }
      else 
    ...
    if (usmDbDot1sMstiVlanRemove(L7_UNIT_CURRENT, keyMstIdValue,&vlan_mask) != L7_SUCCESS)
    {
      failure = L7_TRUE;
    }

    if ((error == L7_TRUE) && (failure != L7_TRUE))
    {
      if (usmDbDot1sInstanceGet(L7_UNIT_CURRENT, keyMstIdValue) == L7_SUCCESS)
      {
        owa.rc = XLIBRC_MST_VLAN_DELETE_INVALID;
      }
    }
    else if ((error != L7_TRUE) && (failure == L7_TRUE))
    {
      owa.rc = XLIBRC_MST_VLAN_DEL_FAILURE;
    }
    else if ((error == L7_TRUE) && (failure == L7_TRUE))
    {
      owa.rc = XLIBRC_MST_VLAN_DELETE_ERROR;
    }
  }
...
xLibRC_t fpObjSet_SwitchingStpMstConfig_VlanParticipation (void *wap, void *bufp)
{
  ...
  memset(&vlan_mask, 0, sizeof(L7_VLAN_MASK_t));
  oid = XOBJ_SwitchingStpMstConfig_MstId;
  len = sizeof (mstId);
  if (XLIBRC_SUCCESS != xLibFilterGet (wap, oid, (xLibU8_t *) & mstId, &len))
  {
  ...
  rc = xLibBufDataGet (bufp, (xLibU8_t *) & val, &len);
  ...
  if (val == L7_ENABLE)
  {
    rv = usmDbDot1sMstiVlanAdd (L7_USMDB_UNIT_ZERO, mstId, &vlan_mask);
  }
  else
  {
    rv = usmDbDot1sMstiVlanRemove (L7_USMDB_UNIT_ZERO, mstId, &vlan_mask);
  }
