...
xLibRC_t fpObjGet_baseCableTest_Status (void *wap, void *bufp)
{
  ...
  L7_uint32 shortestLength,connType;
  ...
  if( ( usmDbIntfConnectorTypeGet(keyInterfaceValue, &connType) != L7_SUCCESS ) ||
      ( connType != L7_RJ45 ) )
  {
      objStatusValue = L7_CABLE_INVALIDCABLETYPE; 
      owa.l7rc = L7_SUCCESS;
  }
  else
  {
    ...
    if ((owa.l7rc == L7_FAILURE) || (objStatusValue == L7_CABLE_TEST_FAIL) || (owa.l7rc == L7_ERROR ))
    {
...
xLibRC_t fpObjGet_baseCableTest_Length (void *wap, void *bufp)
{
  ...
  L7_uint32 cableStatus,connType;
