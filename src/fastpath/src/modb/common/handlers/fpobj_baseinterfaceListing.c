...
/*******************************************************************************
* @function     fpObjGet_baseinterfaceListing_Intf
*
* @purpose      Get 'Intf'
 *@description  [Intf] Intf number in u/s/p(s/p) format
* @notes        This object is key. This is used as a dummy object in the web xml   
*               to perform unit/slot/port number based actions
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseinterfaceListing_Intf (void *wap, void *bufp)
{
  fpObjWa_t owa = FPOBJ_INIT_WA2 ();
  xLibU32_t objUnitIndexValue;
  xLibU32_t nextObjUnitIndexValue;
  ...
  /* retrieve key: UnitIndex */
  owa.len = sizeof (objUnitIndexValue);
  owa.rc = xLibFilterGet (wap, XOBJ_baseinterfaceListing_Intf,
                          (xLibU8_t *) & objUnitIndexValue, &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_CURRENT_KEY (bufp, NULL, 0);
