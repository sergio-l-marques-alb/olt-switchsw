...
xLibRC_t fpObjSet_baseStacking_UnitMgrAdminMgmtPrefValue (void *wap, void *bufp)
{
  ...
  case 0:
    ...
    { 
      owa.rc = XLIBRC_PRIORITY_ZERO_STNDBY;
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
    }
    temp_val = L7_USMDB_UNITMGR_MGMTPREF_DISABLED;
...
xLibRC_t fpObjGet_baseStacking_Slots (void *wap, void *bufp)
{
  ...
  xLibU32_t tempValue[L7_MAX_SLOTS_PER_UNIT];
  FPOBJ_TRACE_ENTER (bufp);
...
xLibRC_t fpObjSet_baseStacking_UnitMgrReplicateSTK (void *wap, void *bufp)
{
  ...
  case L7_ENABLE:
    ...
    if (usmDbUnitMgrMgrNumberGet(&mgrNum) != L7_SUCCESS)
    {
      owa.l7rc = L7_FAILURE;
      break; 
    } 
    if ((mgrNum == keyStackMemberValue) && 
        (osapiStrncmp(srcimage,destimage,strlen(srcimage))==0))
    {
      owa.rc = XLIBRC_SRC_IMAGE_DST_IMAGE_SAME;
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
    }
    owa.l7rc = usmDbFileCopy(keyStackMemberValue, srcimage, destimage,L7_TRUE);
    break;
...
xLibRC_t fpObjGet_baseStacking_UnitMgrImage1Version (void *wap, void *bufp)
{
  ...
  owa.l7rc = usmDbImageFileNameGet ( 0, image1name);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
  ...
  /* get the value from application */
  owa.l7rc = usmDbImageVersionGet ( keyStackMemberValue, image1name,
...
xLibRC_t fpObjGet_baseStacking_UnitMgrImage2Version (void *wap, void *bufp)
{
  ...
  /* get the name from application */
  owa.l7rc = usmDbImageFileNameGet ( 1, image2name);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
  ...
  /* get the value from application */
  owa.l7rc = usmDbImageVersionGet (keyStackMemberValue,image2name,
...
xLibRC_t fpObjGet_baseStacking_UnitMgrSTKName (void *wap, void *bufp)
{
  ...
  {
      strcpy(imageNameStr, image1NameStr);
  }

  if(strcmp(imageNameStr, image2NameStr) == 0)
  {
...
xLibRC_t fpObjSet_baseStacking_UnitMgrSTKName (void *wap, void *bufp)
{
  ...
  /* get the name from application */
  owa.l7rc = usmDbImageFileNameGet(objUnitMgrSTKNameValue-L7_XUI_IMGTYPE_IMAGE1,
                               imageNameStr);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
...
xLibRC_t fpObjSet_baseStacking_UnitMgrActivateSTK (void *wap, void *bufp)
{
  ...
  {
    /* set the value in application */
    owa.l7rc = usmDbImageActivate (keyStackMemberValue,imageNameStr, L7_FALSE);
  }
...
xLibRC_t fpObjSet_baseStacking_UnitMgrDeleteSTK (void *wap, void *bufp)
{
  ...
  xLibU32_t keyStackMemberValue;
  xLibStr256_t imageNameStr;
  L7_uchar8 auditMsg[L7_CLI_MAX_STRING_LENGTH];
  ...
  if (owa.l7rc != L7_SUCCESS)
  {
    osapiSnprintf(auditMsg, sizeof(auditMsg), 
                  pStrInfo_base_Auditing_FileDelete, imageNameStr);
...
xLibRC_t fpObjSet_baseStacking_CopyMasterImageToStackMember(void *wap, void *bufp)
{
 ...
 L7_char8  auditMsg[L7_CLI_STRING_LENGTH];
 masterUnitNo = 0;
 ...
 {
  osapiSnprintf(auditMsg, sizeof(auditMsg), pStrInfo_base_Auditing_FileCopy, 
                localFileName, "backup");
  WEB_CMD_LOGGER_AUDIT_ENTRY_ADD_EXT(wap, auditMsg);
...
xLibRC_t fpObjGet_baseStacking_UnitMgrManagementStandByStatus(void *wap, void *bufp)
{
  ...
  owa.l7rc = usmDbUnitMgrStandbyGet(&operStandby,&mode);
  if( owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_ERROR_STANDBY_UNIT;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
  }
  owa.l7rc = usmDbUnitMgrCfgStandbyGet(&cfgStandby);
  if(owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_ERROR_USER_CFG_STANDBY_UNIT;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
  }
