...
/*******************************************************************************
* @function fpObjUtil_baseUserMgrPasswordXLIBErrorGet
*
* @purpose  Returns User Password XLIB error
*
* @description  
*
* @return   User Mgr XLIB error
*******************************************************************************/
static L7_RC_t fpObjUtil_baseUserMgrPasswordXLIBErrorGet(L7_RC_t errorType)
{
  switch (errorType)
  {
          case L7_USER_MGR_PASSWD_INVALID_PRINTABLE_CHARACTERS:
             return XLIBRC_USERMGR_PASSWD_INVALID_PRINTABLE_CHARACTERS;
             break;
          case L7_USER_MGR_PASSWD_INVALID_MIN_LENGTH:
             return XLIBRC_USERMGR_PASSWD_INVALID_MIN_LENGTH;
             break;
          case L7_USER_MGR_PASSWD_INVALID_MAX_LENGTH:
             return XLIBRC_USERMGR_PASSWD_INVALID_MAX_LENGTH;
             break;
          case L7_USER_MGR_PASSWD_EXCLUDE_LOGIN_NAME_ERROR:
             return XLIBRC_USERMGR_PASSWD_EXCLUDE_LOGIN_NAME_ERROR;
             break;
          case L7_USER_MGR_PASSWD_EXCLUDE_KEYWORDS_ERROR:
             return XLIBRC_USERMGR_PASSWD_EXCLUDE_KEYWORDS_ERROR;
             break;
          case L7_USER_MGR_PASSWD_INVALID_REPEATED_CHAR_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_REPEATED_CHAR_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_CONSECUTIVE_CHAR_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_CONSECUTIVE_CHAR_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_CHARACTER_CLASSES:
             return XLIBRC_USERMGR_PASSWD_INVALID_CHARACTER_CLASSES;
             break;
          case L7_USER_MGR_PASSWD_INVALID_UPPERCASE_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_UPPERCASE_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_LOWERCASE_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_LOWERCASE_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_NUMERIC_CHAR_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_NUMERIC_CHAR_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_SPECIAL_CHAR_LEN:
             return XLIBRC_USERMGR_PASSWD_INVALID_SPECIAL_CHAR_LEN;
             break;
          case L7_USER_MGR_PASSWD_INVALID_QUOTATION_CHAR:
             return XLIBRC_USERMGR_PASSWD_INVALID_QUOTATION_SPECIAL_CHAR;
             break;
          case L7_USER_MGR_PASSWD_INVALID_QUESTIONMARK_CHAR:
             return XLIBRC_USERMGR_PASSWD_INVALID_QUESTIONMARK_SPECIAL_CHAR;
             break;
          case L7_USER_MGR_PASSWD_UPPERCASE_CHARS_EXCEEDS_MAX_LIMIT:
             return XLIBRC_USERMGR_PASSWD_EXCEEDS_UPPERCASE_CHAR_MAX_LIMIT;
             break;
          case L7_USER_MGR_PASSWD_LOWERCASE_CHARS_EXCEEDS_MAX_LIMIT:
             return XLIBRC_USERMGR_PASSWD_EXCEEDS_LOWERCASE_CHAR_MAX_LIMIT;
             break;
          case L7_USER_MGR_PASSWD_NUMERIC_CHARS_EXCEEDS_MAX_LIMIT:
             return XLIBRC_USERMGR_PASSWD_EXCEEDS_NUMERIC_CHAR_MAX_LIMIT;
             break;
          case L7_USER_MGR_PASSWD_SPECIAL_CHARS_EXCEEDS_MAX_LIMIT:
             return XLIBRC_USERMGR_PASSWD_EXCEEDS_SPECIAL_CHAR_MAX_LIMIT;
             break;
          default:
...
xLibRC_t fpObjSet_baseUsrMgrUserConfig_UserPassword (void *wap, void *bufp)
{
 ...
 if (userMgrLoginUserBlockStatusSet (keyUserIndexValue, L7_FALSE) == L7_FAILURE)
 {
  ...
  {
    owa.rc = fpObjUtil_baseUserMgrPasswordXLIBErrorGet(owa.l7rc);
    FPOBJ_TRACE_EXIT (bufp, owa);
    return owa.rc;
  }
  owa.l7rc = L7_SUCCESS;    
...
xLibRC_t fpObjSet_baseUsrMgrUserConfig_UserStatus (void *wap, void *bufp)
{
  ...
  xLibU32_t objUserPasswdOverideComplexityValue;
  ...
  if ( objUserStatusValue == L7_ROW_STATUS_CREATE_AND_GO ) 
  {
      ...
      */
			...
			for (tempInt = 0; tempInt < L7_MAX_LOGINS; tempInt++)
			{
		        ...
		        /* retrieve object: UserAuthType */
                        owa.len = sizeof(objUserPasswdOverideComplexityValue);
			owa.rc = xLibFilterGet (wap, XOBJ_baseUsrMgrUserConfig_UserPasswordOverrideComplexityCheck,
                                                (xLibU8_t *) & objUserPasswdOverideComplexityValue, 
			                        &owa.len);
			if (owa.rc != XLIBRC_SUCCESS)
			{
			  owa.rc = XLIBRC_FAILURE;
			...
			if(owa.l7rc == L7_SUCCESS)
			{
             /* set the value in application */
             owa.l7rc =
                usmDbUserMgrLoginPasswdStrengthOverrideCheckSet(indx, objUserPasswdOverideComplexityValue);
             if (owa.l7rc != L7_SUCCESS)
             {
               owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
               FPOBJ_TRACE_EXIT (bufp, owa);
			...
			if(owa.l7rc == L7_SUCCESS)
			{
	           ...
	           if(owa.l7rc == L7_SUCCESS)
	           {
                  ...
                  osapiSnprintf(auditMsg, sizeof(auditMsg), 
                                pStrInfo_base_Auditing_NewUser, objUserNameValue);
                  WEB_CMD_LOGGER_AUDIT_ENTRY_ADD_EXT(wap, auditMsg);
	           }  
			}
		}
       return owa.rc;
  ...
  else if (objUserStatusValue == L7_ROW_STATUS_DESTROY)
  {
    ...
    /*Retrieve username*/
    memset(temp_buf, 0x00, sizeof(temp_buf));
    if (usmDbLoginsGet(L7_UNIT_CURRENT, keyUserIndexValue, temp_buf) != L7_SUCCESS)
    {
      osapiSnprintf(temp_buf, sizeof(temp_buf), "<<UNKNOWN>>");
    }
    ...
    osapiSnprintf(auditMsg, sizeof(auditMsg), pStrInfo_base_Auditing_RemoveUser, temp_buf);
    WEB_CMD_LOGGER_AUDIT_ENTRY_ADD_EXT(wap, auditMsg);
...
xLibRC_t fpObjSet_baseUsrMgrUserConfig_UserAccessLevel (void *wap, void *bufp)
{
  ...
  fpObjWa_t kwaUserIndex = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t keyUserIndexValue;
  L7_char8  userName[L7_SYS_SIZE];
  L7_char8  auditMsg[L7_SYS_SIZE];

  memset(userName, L7_NULL, sizeof(userName));
  memset(auditMsg, L7_NULL, sizeof(auditMsg));
  ...
  usmDbLoginsGet(L7_UNIT_CURRENT, keyUserIndexValue, userName);
  osapiSnprintf(auditMsg, sizeof(auditMsg), 
                pStrInfo_base_Auditing_AccessLevel, userName, objUserAccessLevelValue);
  WEB_CMD_LOGGER_AUDIT_ENTRY_ADD_EXT(wap, auditMsg);
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
}
...
xLibRC_t fpObjSet_baseUsrMgrUserConfig_ResetPassword (void *wap, void *bufp)
{
  ...
  (void)usmDbMinPassLengthGet(&minLen);

  owa.l7rc = L7_SUCCESS;
  if(minLen != 0)
  {
    owa.l7rc = usmDbMinPasswdSet(L7_UNIT_CURRENT, 0);
  }
    ...
    while(index < L7_MAX_LOGINS )
    {
         ...
         (userNameValue[0] != '\0'))
      {
        ...
        if((owa.l7rc != L7_ERROR)  && (owa.l7rc != L7_SUCCESS))
        {
          ...
          if(minLen != 0)
          {
            (void)usmDbMinPasswdSet(L7_UNIT_CURRENT, (L7_uint32)minLen);
          }
          return owa.rc;
        }
      }
...
/*******************************************************************************
* @function fpObjGet_baseUsrMgrUserConfig_UserPasswordOverrideComplexityCheck
*
* @purpose Get 'UserPasswordOverrideComplexityCheck'
*
* @description Password Override Strength Check feature
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_baseUsrMgrUserConfig_UserPasswordOverrideComplexityCheck (void *wap,
                                                                        void *bufp)
{
  fpObjWa_t kwaUserIndex = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t keyUserIndexValue;
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU16_t));
  xLibU32_t objUserPasswordOverrideComplexityCheckValue;
  FPOBJ_TRACE_ENTER (bufp);
  ...
  /* get the value from application */
  owa.l7rc =
    usmDbUserMgrLoginPasswdStrengthOverrideCheckGet (keyUserIndexValue, 
                                                     &objUserPasswordOverrideComplexityCheckValue);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    
    FPOBJ_TRACE_EXIT (bufp, owa);
  ...
  FPOBJ_TRACE_VALUE (bufp, &objUserPasswordOverrideComplexityCheckValue,
                     sizeof (objUserPasswordOverrideComplexityCheckValue));

  /* return the object value: UserPasswordOverrideComplexityCheck */
  owa.rc =
    xLibBufDataSet (bufp, (xLibU8_t *) & objUserPasswordOverrideComplexityCheckValue,
                    sizeof (objUserPasswordOverrideComplexityCheckValue));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
}
...
/*******************************************************************************
* @function fpObjSet_baseUsrMgrUserConfig_UserPasswordOverrideComplexityCheck
*
* @purpose Set 'UserPasswordOverrideComplexityCheck'
*
* @description [UserPasswordOverrideComplexityCheck]: password override strength 
*               feature 
*
* @return
*******************************************************************************/
xLibRC_t fpObjSet_baseUsrMgrUserConfig_UserPasswordOverrideComplexityCheck(void *wap,
                                                                           void *bufp)
{
  fpObjWa_t kwaUserIndex = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t keyUserIndexValue;
  ...
  /* retrieve object: UserPasswordOverrideComplexityCheck */
  owa.rc = xLibBufDataGet (bufp,
                           (xLibU8_t *) & objUserPasswordOverrideComplexityCheckValue,
                           &owa.len);
  if (owa.rc != XLIBRC_SUCCESS)
  {
    FPOBJ_TRACE_EXIT (bufp, owa);
  ...
  FPOBJ_TRACE_VALUE (bufp, &objUserPasswordOverrideComplexityCheckValue, owa.len);
  ...
  /* set the value in application */
  owa.l7rc =
    usmDbUserMgrLoginPasswdStrengthOverrideCheckSet(keyUserIndexValue,
                                                    objUserPasswordOverrideComplexityCheckValue);
  if (owa.l7rc != L7_SUCCESS)
  {
    owa.rc = XLIBRC_FAILURE;    
