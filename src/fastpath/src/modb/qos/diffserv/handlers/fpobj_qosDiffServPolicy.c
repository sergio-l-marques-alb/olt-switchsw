...
xLibRC_t fpObjSet_qosDiffServPolicy_PolicyRowStatus (void *wap, void *bufp)
{
      ...
      (objPolicyRowStatusValue == L7_ROW_STATUS_CREATE_AND_WAIT) )
  {
    ...
    owa.l7rc = usmDbDiffServPolicyCreate(L7_UNIT_CURRENT, keyPolicyIndexValue, 
          ((objPolicyRowStatusValue == L7_ROW_STATUS_CREATE_AND_GO) ? L7_TRUE : L7_FALSE));
    if (owa.l7rc != L7_SUCCESS)
    {
  ...
  else if (objPolicyRowStatusValue == L7_ROW_STATUS_DESTROY)
  {
	  ...
	  if (kwa.rc != XLIBRC_SUCCESS)
	  {
		...
		owa.l7rc = usmDbDiffServPolicyDelete(L7_UNIT_CURRENT, keyPolicyIndexValue);
    if (owa.l7rc != L7_SUCCESS)
    {
      if (owa.l7rc == L7_REQUEST_DENIED)
      {
        owa.rc = XLIBRC_DIFFSERV_CHANGE_REQ_DENY_APP_USING_POLICY;
      }
      else
      {
...
, &policyInstId) != L7_SUCCESS)
    ...
    owa.l7rc = usmDbDiffServPolicyInstCreate(L7_UNIT_CURRENT, keyPolicyIndexValue, 
policyInstId, L7_TRUE);
    if (owa.l7rc != L7_SUCCESS)
    {
      if (owa.l7rc == L7_REQUEST_DENIED)
      {
...
keyPolicyIndexValue, 
  ...
  owa.l7rc = usmDbDiffServPolicyInstDelete(L7_UNIT_CURRENT, keyPolicyIndexValue, 
policyInstId);
  if (owa.l7rc != L7_SUCCESS)
  {
    if (owa.l7rc == L7_REQUEST_DENIED)
