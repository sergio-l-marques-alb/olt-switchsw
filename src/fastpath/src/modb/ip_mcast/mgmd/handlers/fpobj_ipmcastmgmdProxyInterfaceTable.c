...
/*******************************************************************************
* @function fpObjGet_ipmcastmgmdProxyInterfaceTable_mgmdProxyIntfStatus
*
* @purpose Get 'mgmdProxyIntfStatus'
*
* @description [mgmdProxyIntfStatus] Status of MGMD Proxy on the selected interface
*
* @notes  
*
* @return
*******************************************************************************/
xLibRC_t fpObjGet_ipmcastmgmdProxyInterfaceTable_mgmdProxyIntfStatus (void *wap, void *bufp)
{
  ...
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objmgmdProxyIntfStatusValue;
  FPOBJ_TRACE_ENTER (bufp);
  ...
  /* get the value from application */
  owa.l7rc = usmDbMgmdProxyInterfaceGet(L7_UNIT_CURRENT, keymgmdProxyIntfTypeValue, 
                                        &currentProxyInterface);
  
  if (owa.l7rc == L7_SUCCESS && currentProxyInterface != L7_NULL)
  {
    owa.l7rc = usmDbMgmdProxyInterfaceModeGet (L7_UNIT_CURRENT, keymgmdProxyIntfTypeValue,
                              keymgmdProxyIntfIndexValue, &objmgmdProxyIntfModeValue);
    if ((owa.l7rc == L7_SUCCESS) && (objmgmdProxyIntfModeValue == L7_DISABLE))
    {
      objmgmdProxyIntfStatusValue = L7_TRUE1FALSE2_FALSE;
    }
    else
    {
      objmgmdProxyIntfStatusValue = L7_TRUE1FALSE2_TRUE;
    }
  }
  else
  {
  ...
  /* return the object value: mgmdProxyIntfMode */
  owa.rc = xLibBufDataSet (bufp, (xLibU8_t *) & objmgmdProxyIntfStatusValue,
                           sizeof (objmgmdProxyIntfStatusValue));
  FPOBJ_TRACE_EXIT (bufp, owa);
  return owa.rc;
