...
xLibRC_t fpObjGet_baseDhcpServerPoolConfig_PoolName (void *wap, void *bufp)
{
 fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibStr256_t));
   ...
   if (owa.rc != XLIBRC_SUCCESS)
  {
   ...
   owa.rc = usmDbDhcpsNumberToPoolNameGet(L7_UNIT_CURRENT, objPoolIndexValue, poolNameTemp);
     if(owa.l7rc != L7_SUCCESS)
     {
                                                                   ...
                                                                   void *bufp)
{
  ...
  xLibS8_t objManualPoolClientHWAddrValue[L7_DHCPS_HARDWARE_ADDR_MAXLEN];
...
{
  fpObjWa_t owa = FPOBJ_INIT_WA (sizeof (xLibU32_t));
  xLibU32_t objStatusValue,keyPoolIndexValue,tempPoolIndexValue;

  xLibStr256_t objPoolName;
  fpObjWa_t kwa = FPOBJ_INIT_WA (sizeof (xLibStr256_t));
  ...
  /* retrieve key: PoolIndex */
  owa.rc = xLibFilterGet (wap, XOBJ_baseDhcpServerPoolConfig_PoolIndex,
                          (xLibU8_t *) & keyPoolIndexValue, &owa.len);

  /* call the usmdb only for add and delete */
  if ((owa.rc != XLIBRC_SUCCESS)&&(objStatusValue != L7_ROW_STATUS_CREATE_AND_GO))
  {
    owa.rc = XLIBRC_FAILURE;    /* TODO: Change if required */
    FPOBJ_TRACE_EXIT (bufp, kwa);
    return owa.rc;
  }
  FPOBJ_TRACE_CURRENT_KEY (bufp, &keyPoolIndexValue, owa.len);

  memset(objPoolName,0x0,sizeof(objPoolName));
  kwa.rc = xLibFilterGet(wap,XOBJ_baseDhcpServerPoolConfig_PoolName,(xLibU8_t *) & objPoolName, &kwa.len);
  if( objStatusValue == L7_ROW_STATUS_CREATE_AND_GO)
  {
      /* set the value in application */
      owa.l7rc = usmDbDhcpsPoolCreate (L7_UNIT_CURRENT,objPoolName);
      if (owa.l7rc != L7_SUCCESS)
      {
         ...
         {
            owa.rc = XLIBRC_DHCPS_POOLNAME_EXISTS;
            FPOBJ_TRACE_EXIT (bufp, owa);
            return owa.rc;
         }
      }  
      owa.rc = usmDbDhcpsPoolEntryFirst(L7_UNIT_CURRENT,poolNameTemp, &tempPoolIndexValue);
      if (owa.l7rc != L7_SUCCESS)
      {
        owa.rc = XLIBRC_FAILURE;  /* TODO: Change if required */
        FPOBJ_TRACE_EXIT (bufp, owa);
      ...
      {
         if(strcmp(poolNameTemp,objPoolName) == 0)  
         {
            break;
         }
         else
         {
             usmDbDhcpsPoolEntryNext(L7_UNIT_CURRENT,poolNameTemp, &tempPoolIndexValue);
         }
      }
      tempPoolIndexValue = tempPoolIndexValue +1; 
      owa.rc = xLibFilterSet(wap,XOBJ_baseDhcpServerPoolConfig_PoolIndex,0, (xLibU8_t *)&tempPoolIndexValue,sizeof(tempPoolIndexValue));
      if (owa.rc != XLIBRC_SUCCESS)
      {
         FPOBJ_TRACE_EXIT (bufp, owa);
         return owa.rc;
