                                                               ...
                                                               void *bufp)
{
  ...
  {		
    ...
    if (owa.l7rc != L7_SUCCESS)
    {
      ...
      {
        case L7_NOT_EXIST:
          owa.rc = XLIBRC_PORTMIRROR_INVALID_SESSION_ID;
          break;
        case L7_ALREADY_CONFIGURED:  /* this case is valid only for destination interface */
          owa.rc = XLIBRC_PORTMIRROR_PROBE_IS_ALREADY_CONFIGURED;
          break;
        case L7_REQUEST_DENIED:     /* valid only for source interface */
          owa.rc = XLIBRC_PORTMIRROR_MAX_SRC_PORT_LIMIT_REACHED;
          break;
        default:
          owa.rc = XLIBRC_PORTMIRROR_INVALID_INTERFACE;
          break;
      } 
      FPOBJ_TRACE_EXIT (bufp, owa);
      return owa.rc;
