...
xLibRC_t fpObjGet_baseSummerTimeSupport_Status (void *wap, void *bufp)
{
  ...
  L7_uint32 stMonth, stDay, stHour, stMin, endMonth, endDay, endHour, endMin, offset = 0;
  L7_uint32 stWeek, endWeek;
  ...
  switch(objTempStatusValue)
  {
      ...
      /* check for USA or EU summertime */
      if ( (usmDbStRecurringGet(0, L7_START_MONTH,    &stMonth)  == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_START_DAY,      &stDay)    == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_START_WEEK,     &stWeek)   == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_START_HOUR,     &stHour)   == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_START_MINUTE,   &stMin)    == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_END_MONTH,      &endMonth) == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_END_DAY,        &endDay)   == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_END_WEEK,       &endWeek)  == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_END_HOUR,       &endHour)  == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_END_MINUTE,     &endMin)   == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_OFFSET_MINUTES, &offset)   == L7_SUCCESS) &&
           (usmDbStRecurringGet(0, L7_ZONE_ACRONYM,   zoneName)  == L7_SUCCESS) )
      {
        /* check for USA summertime */
        if ( (stMonth  == L7_SUMMER_TIME_USA_START_MONTH) &&
             ...
             (endMin   == 0) &&
             (offset   == L7_DEFAULT_SUMMERTIME_OFFSET) )
        {
          objStatusValue = L7_SUMMER_TIME_RECURRING_USA;
        }
        /* check for EU summertime */
        else if ( (stMonth  == L7_SUMMER_TIME_EU_START_MONTH) &&
                  (stDay    == L7_SUMMER_TIME_DAY) &&
