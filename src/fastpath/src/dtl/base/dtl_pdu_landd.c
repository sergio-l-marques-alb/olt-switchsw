                                    ...
                                    void *dapiEventInfo)
{
  ...
  if (pdu_receive_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Packet received here");
  ...
  if (dtlMsgQueue && ((dtlMsgTail+1)%DTL_MSG_COUNT != dtlMsgHead))
  {
    ...
    if (pdu_receive_debug)
      PT_LOG_TRACE(LOG_CTX_DTL,"Sending packet to dtlMsgQueue queue");
                      ...
                      void *dapiEventInfo)
{
  ...
  {
    if (pdu_receive_debug)
      PT_LOG_TRACE(LOG_CTX_DTL,"Going to call sysNetNotifyPduReceive");
                        ...
                        DTL_CMD_TX_INFO_t *dtlCmdInfo )
{
  ...
  {
    if (ptin_debug_dtl)
      PT_LOG_NOTICE(LOG_CTX_DTL,"Silently ignoring packet transmission [intfNum:%u]. I'm a Stanby Matrix",dtlCmdInfo->intfNum);
...
L7_RC_t dtlPduLoopback(L7_uint32 destIntf, L7_netBufHandle bufHandle)
{
  ...
  if (pdu_receive_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Received packet here!");
  ...
  if (pdu_receive_debug)
    PT_LOG_TRACE(LOG_CTX_DTL,"Calling dtlPduReceiveCallback function");
