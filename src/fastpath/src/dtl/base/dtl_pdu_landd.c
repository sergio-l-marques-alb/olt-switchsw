                                    ...
                                    void *dapiEventInfo)
{
  ...
  if (pdu_receive_debug)
    LOG_PT_TRACE(LOG_CTX_DTL,"Packet received here");
  ...
  if (dtlMsgQueue && ((dtlMsgTail+1)%DTL_MSG_COUNT != dtlMsgHead))
  {
    ...
    if (pdu_receive_debug)
      LOG_PT_TRACE(LOG_CTX_DTL,"Sending packet to dtlMsgQueue queue");
                      ...
                      void *dapiEventInfo)
{
  ...
  {
    if (pdu_receive_debug)
      LOG_PT_TRACE(LOG_CTX_DTL,"Going to call sysNetNotifyPduReceive");
                        ...
                        DTL_CMD_TX_INFO_t *dtlCmdInfo )
{
  ...
  {
    if (ptin_debug_dtl)
      LOG_PT_NOTICE(LOG_CTX_DTL,"Silently ignoring packet transmission [intfNum:%u]. I'm a Stanby Matrix",dtlCmdInfo->intfNum);
  ...
  {
    if (ptin_debug_dtl)
      LOG_PT_NOTICE(LOG_CTX_DTL,"Silently ignoring packet transmission. Outgoing interface [intIfNum=%u] is down!",dtlCmdInfo->intfNum);    
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  if (ptin_debug_dtl)
  {
    LOG_PT_DEBUG(LOG_CTX_MISC,"TX PDU: dtlCmd=%u intIfNum=%u prio=%u typeToSend=%u L2.domainId=%u L2.vlanId=%u L2.flags=0x%08x",
...
L7_RC_t dtlPduLoopback(L7_uint32 destIntf, L7_netBufHandle bufHandle)
{
  ...
  if (pdu_receive_debug)
    LOG_PT_TRACE(LOG_CTX_DTL,"Received packet here!");
  ...
  if (pdu_receive_debug)
    LOG_PT_TRACE(LOG_CTX_DTL,"Calling dtlPduReceiveCallback function");
