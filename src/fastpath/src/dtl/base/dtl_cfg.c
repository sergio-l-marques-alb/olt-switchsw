...
L7_RC_t dtlTask()
{
  ...
  if (dtlMsgQueue == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DTL_COMPONENT_ID,
            "DTL: Unable to create msg queue for dtlTask\n");
    return(L7_FAILURE);
  }
  ...
  do
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DTL_COMPONENT_ID,
              "DTL: dtlTask msg receive error, status = %x\n",mrc);
      continue;
    }
...
L7_RC_t dtlStartTask(L7_uint32 *taskId)
{
  ...
  *taskId = osapiTaskCreate("dtlTask",dtlTask,0,L7_NULLPTR,
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DTL_COMPONENT_ID,
            "DTL: Could not create dtlInfo.routeEntrySem\n");
    return(L7_ERROR);
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DTL_COMPONENT_ID,
            "Unable to create dtlL2McastSema\n");
    return L7_ERROR;
  }
