...
L7_RC_t dtlPtinGeneric(L7_uint32 intIfNum, L7_uint16 msgId, DAPI_CMD_GET_SET_t operation, L7_uint32 dataSize, void *data)
{
  ...
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_DTL, "Invalid intIfNum %u!", intIfNum);
      return L7_FAILURE;
    }
  ...
  {
    LOG_ERR(LOG_CTX_DTL, "Invalid dataSize (%u > %u bytes)!", dataSize, PTIN_DTL_GENERICMSG_MAX_DATASIZE);
    return L7_FAILURE;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_DTL, "Error rc=%u", rc);
    return rc;
  }
  else
  {
