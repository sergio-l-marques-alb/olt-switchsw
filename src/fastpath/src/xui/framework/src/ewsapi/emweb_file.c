...
extern L7_uint32 L7_http_dl_data_last_seen;
extern int osapiWebFileDescriptorGet (void *appWap);
#endif
...
EwaFileHandle ewaFilePost (EwsContext context, const EwsFileParams * params)
{
  ...
  if (strstr(context->url, "nv_image_mgmt.html") != NULL)
  {
    fp = (EwaFileHandle)ewaAlloc(sizeof( EwaFileHandle_t )
                                      + strlen(path)
                                      + 1 /* path delimiter */
                                      + strlen(filename)
                                      + 1 /* EOL */);
  ...
  if (0 > osapiWebFileHandler ('n', filename, &fp->appWap, NULL, 0))
  {
...
# ifdef FILE_MAX_LENGTH
  ...
  if ((strstr(context->url, "cp_web_custom.html") != NULL) ||
      (strstr(context->url, "cp_web_custom_global.html") != NULL))
  {
  ...
  if (strstr(context->url, "http_file_download.html") != NULL)
    {
      ...
      {
        if(usmDbTransferFileNameLocalSet(unit, "") != L7_SUCCESS)
        {
         /* Unable to set the file name to empty string, nothing to be done here, as in any case memory will be freed in the subsequent lines of code*/
        }
        osapiFree(L7_CLI_WEB_COMPONENT_ID, L7_http_dl_ram_buffer);
        L7_http_dl_ram_buffer = NULL;
      }
...
sintf ewaFileWrite (EwsContext context, EwaFileHandle handle, const uint8 * datap, uintf length)
{
  ...
  if (0 > osapiWebFileHandler ('w', handle->localPath, &handle->appWap, (char *) datap, length))
  {
...
sintf ewaFileRead (EwsContext context, EwaFileHandle handle, uint8 * datap, uintf length)
{
  ...
  retval = osapiWebFileHandler ('r', handle->localPath, &handle->appWap, datap, length);
  if (0 > retval)
  {
...
void emwebFileClose( EwsContext context)
{
#ifdef _L7_OS_LINUX_ 
  EwaFileHandle fp = NULL;
  int fd = 0;

  if ((context != NULL) && (context->inputFile != NULL) && (context->inputFile->formFile != NULL))
  {
    fp = context->inputFile->formFile;
    if (fp->appWap != NULL)
    {
      fd = osapiWebFileDescriptorGet(fp->appWap);
    }
    if (fd != 0)
    {
      if (fp->appFunc == NULL)
...
EwaFileStatus ewaFileClose (EwaFileHandle handle, EwsStatus status)
{
  ...
  osapiWebFileHandler ('c', handle->localPath, &handle->appWap, NULL, 0);
...
EwaFileHandle ewaFileGet (EwsContext context, const char *url, const EwsFileParams * params)
{
  ...
  if (0 > osapiWebFileHandler ('o', (char *) params->fileInfo.fileName, &efa->appWap, NULL, 0))
  {
    EMWEB_FILE_TRACE ("ewaFileGet: unable to open file '%s'\n", (char *) params->fileInfo.fileName);
    return EWA_FILE_HANDLE_NULL;
  }
...
EwaFileHandle fp;
  ...
  if ((fp = (EwaFileHandle)ewaAlloc(  sizeof( EwaFileHandle_t )))
      != EWA_FILE_HANDLE_NULL)
    {
		...
		osapiStrncpySafe(lbuf, (char *)params->fileInfo.fileName,sizeof(lbuf));

        if(osapiStrncmp(params->fileInfo.fileName,"/filesystem/image2.stk",22) == 0 )
        {
           memset(lbuf,0x0,sizeof(lbuf));
           osapiStrncpy(lbuf,"/filesystem/image2",18);
        }

        if(osapiStrncmp(params->fileInfo.fileName,"/filesystem/image1.stk",22) == 0 )
        {
           memset(lbuf,0x0,sizeof(lbuf));
        ...
        if(osapiStrncmp(params->fileInfo.fileName,"image2.stk",10) == 0 )
        {
           memset(lbuf,0x0,sizeof(lbuf));
           osapiStrncpy(lbuf,"image2\0",7);
        }

        if(osapiStrncmp(params->fileInfo.fileName,"image1.stk",10) == 0 )
        {
           memset(lbuf,0x0,sizeof(lbuf));
      ...
      if (osapiFsOpen(lbuf, &fp->fd) == L7_ERROR)
        {
