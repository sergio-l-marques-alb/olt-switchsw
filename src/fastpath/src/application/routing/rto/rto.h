...
/* special value to indicate a trace prefix is not set */
#define RTO_TRACE_PFX_NONE  0xffffffff
...
typedef struct rto_stats_s
{
  ...
  L7_uint32 tot_mods;      /* Total number of route mods                    */
  ...
  /* Number of checkpoint routes confirmed by a post-failover route */
  L7_uint32 ckpt_conf;

  /* Number of tentative routes */
  L7_uint32 tentative_routes;

  L7_uint32 nhres_radix_entries;

  /* Number of next hop structures currently in use. */
  L7_uint32 nextHopCount;

  /* High water mark of nextHopCount */
  L7_uint32 nextHopHighWater;
...
typedef struct rtoNextHop_s
{
  ...
  /* next in next hop linked list. Used for both free list and list of 
   * next hops in a route. */
  struct rtoNextHop_s *nextNh;   
...
#define RTO_REJECT_ROUTE        0x04
  ...
  /* checkpointed routes are marked tentative. If a duplicate route is added after
   * a warm restart, the tentative flag is cleared. */
#define RTO_TENTATIVE_ROUTE     0x08

#define RTO_DHCP_ROUTE          0x16
  ...
  /* Linked list of next hops to the destination. */
  rtoNextHop_t *nextHops;  

  /* Next route info in route info linked list. Could be route info free list 
   * or list of routes to a given destination in radix tree. */
