...
/*********************************************************************
* @purpose  Determine whether OSPF may be enabled
*
* @param    void
*
* @returns  L7_TRUE if all conditions are met to enable OSPF
*           L7_FALSE otherwise
*
* @notes    These conditions must be met for OSPF to be enabled:
*            - OSPF is administratively enabled
*            - routing is globally enabled
*            - a non-zero router ID is configured
*            - initial startup is done (interfaces up, config pushed)
*
* @end
*********************************************************************/
L7_BOOL ospfMayEnableOspf(void)
{
  ...
  if (pOspfMapCfgData->rtr.routerID == 0)
    return L7_FALSE;

  if (!pOspfInfo->startupDone)
    return L7_FALSE;
...
/*********************************************************************
* @purpose  Operationally enable OSPF
*
* @param    gracefulRestart  @b{(input)} L7_TRUE if OSPF is doing a graceful restart
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @notes    Callers should use ospfMayEnableOspf() to verify that all
*           conditions are met to enable OSPF.
*
*           This function should only be called if OSPF is currently
...
/*------------------------------------------------------*/
  ...
  if (gracefulRestart)
  {
    ospfMapExtenBeginGracefulRestart();
  }
  else
  {
#ifdef L7_NSF_PACKAGE
...
/*------------------------------------------------------*/
  ...
  /* Don't care about bad rc here -- can happen if this function
  ...
  if (ospfMapAclCallbackIsRegistered(ospfMapAclCallback) == L7_FALSE)
  {
    if (ospfMapAclCallbackRegister(ospfMapAclCallback, "ospfMapAclCallback") == L7_FAILURE)
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
              "OSPFv2 failed to register for ACL changes.");
          }
      }
  ...
  if (ospfMapIsAsbr())
  {
        ...
        {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
              "OSPFv2 failed to register for best route changes.");
        }
      }
  ...
  /* Tell OSPF that it is now fully configured and all interfaces have
   * been created. */
  if (ospfMapExtenOspfAdminModeSet(L7_ENABLE) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
            "Failed to enable OSPFv2");
    return L7_FAILURE;
  }

  /* Interface can't come up until OSPF is globally operationally enabled.
   * So send the UP notification again. */
  ...
  /* Tell protocol code that all interfaces have now been enabled */
  if (ospfMapExtenStartupDone() != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_OSPF_MAP_COMPONENT_ID,
            "OSPFv2 failed to handle startup done event.");
    }
...
#ifdef L7_NSF_PACKAGE
    /* Clear checkpoint table on management unit and backup */
    o2CkptTableClear();    /* clear any data waiting to be checkpointed */
    o2CkptSendFlush();     /* tell backup unit to clear its checkpoint table */
#endif
...
/*-----------------------------------------*/
    ...
    /* Disable OSPF globally. This triggers premature aging of self-originated
     * LSAs. Important to do this before disabling interfaces. */
...
* @returns  L7_FAILURE
*
* @notes    This only gets called when OSPF is operationally disabled
*
* @end
*********************************************************************/
L7_RC_t ospfMapRouterIDApply(L7_uint32 routerID)
{
  (void) ospfMapExtenRouterIDSet(routerID);

  if (ospfMayEnableOspf())
  {
    (void) ospfMapOspfEnable(L7_FALSE);
  }
...
/*********************************************************************
* @purpose  Apply the stub router configuration.
*
                                ...
                                L7_uint32 startupDuration)
{
  ...
  if (ospfMapExtenStubRtrModeSet(stubRtrMode, startupDuration) != L7_SUCCESS)
  {
    /* Log failure and return success */
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
            "Failed to set OSPFv2 stub router mode.");
  }
...
/*********************************************************************
* @purpose  Apply the stub router summary LSA metric configuration.
*
                                         ...
                                         L7_uint32 stubRtrSummaryMetric)
{
  ...
  if (ospfMapExtenStubRtrSummaryMetricSet(overrideSummaryMetric, 
                                          stubRtrSummaryMetric) != L7_SUCCESS)
  {
    /* Log failure and return success */
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_OSPF_MAP_COMPONENT_ID,
            "Failed to set OSPFv2 stub router summary metric.");
  }
...
* @returns  L7_FAILURE
*
* @notes    Assumes caller has already verified that all conditions are
*           met to enable OSPF on this interface. That is, the caller has
*           gotten the thumbs up from ospfMayEnableInterface(). So no need
*           to check preconditions here.
*
* @end
*********************************************************************/
L7_RC_t ospfMapIntfAdminModeEnable(L7_uint32 intIfNum)
{
  ...
  L7_RC_t rc = L7_FAILURE;
    ...
    if (ospfMapExtenIntfAdminModeSet(intIfNum, L7_ENABLE) == L7_SUCCESS)
        ...
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_OSPF_MAP_COMPONENT_ID,
                "Failed to set secondary IP addresses on OSPFv2 interface %s.",
                ifName);
          }
      ...
      if (ospfMapLocalRedistSuppress(intIfNum) != L7_SUCCESS)
      {
        ...
        nimGetIntfName(intIfNum, L7_SYSNAME, ifName);
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_OSPF_MAP_COMPONENT_ID,
                "Failed to suppress redistribution of local route on OSPFv2 interface %s.",
                ifName);
      }
...
L7_RC_t ospfMapIntfAreaIdApply(L7_uint32 intIfNum, L7_uint32 area)
{
  ...
  OSPFMAP_TRACE("%s %d: %s : intf = %d, %s , area = %x\n", __FILE__, __LINE__, routine_name,
              intIfNum, ifName, area);
...
L7_RC_t ospfMapIntfSecondariesFlagApply(L7_uint32 intIfNum, L7_uint32 secondaries)
{
  ...
  OSPFMAP_TRACE("%s %d: %s : intf = %d, %s , secondaries = %x\n", __FILE__, __LINE__, routine_name,
              intIfNum, ifName, secondaries);
...
L7_BOOL ospfMapOspfInitialized(void)
{
  if (pOspfInfo == L7_NULL)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
            "WARNING:  Invalid build - OSPF component was not initialized\n");
    L7_assert(1);
    return L7_FALSE;
  }
...
L7_BOOL ospfMapIsSubnet(L7_uint32 prefix, L7_uint32 mask)
{
    ...
    prefix.  */
    {
        ...
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
                "ERROR:  OSPF attempt to redistribute invalid prefix %s\n",
                destAddrStr);
    }
    return L7_TRUE;
}
