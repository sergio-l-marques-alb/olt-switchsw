...
extern L7_uchar8 *ipMapCnfgrStateNames[];
...
typedef enum {
    ...
    /* Trace checkpoint events */
    IPMAP_TRACE_CKPT = 0x200,

    /* Trace nonstop forwarding */
    IPMAP_TRACE_NSF = 0x400,

    /* Trace ARP checkpoint events */
    IPMAP_TRACE_ARP_CKPT = 0x800,

    /* Detailed tracing of IP MAP checkpointing */
    IPMAP_TRACE_CKPT_DETAIL = 0x1000,

    /* Trace route update */
    IPMAP_TRACE_ROUTE_UPDATE = 0x2000,

    /* Trace DHCP Updates */
    IPMAP_TRACE_DHCP_UPDATE = 0x4000,

    /* Trace Interface State Changes */
    IPMAP_TRACE_INTF_STATE_CHANGE = 0x8000
...
typedef struct ipMapNimStartup_s
{
  NIM_STARTUP_PHASE_t startupPhase;
} ipMapNimStartup_t;

typedef struct ipMapCfgComplete_s
{
   L7_uint32 event;
} ipMapCfgComplete_t;
...
typedef struct ipMapMsg_s
{
    ...
    union
    {
      ...
      ipMapNimStartup_t         nimStartupEvent; 
      ipMapCfgComplete_t        cfgCompleteEvent;
...
/* Routing Interface Enable Method.
 * Enable for Routing or Enable for Host or Do not Enable.
 */
typedef enum
{
  IPMAP_RTR_INTF_NO_ENABLE = 1,
  IPMAP_RTR_INTF_ENABLE_ROUTING,
  IPMAP_RTR_INTF_ENABLE_HOST,
...
/* Estimate the number of events of each type that can be in 
 * the IP MAP processing thread's message queue at any given time:
 *  
 *  - incoming PDUs:           0     (these go the the forwarding thread Qs)
 *  - NIM events:              1
 *  - NIM startup events:      1 
 *  - config complete events:  1
 *  - RTO best route change:   1
 *  - router event complete:   number of interfaces (not just routing interfaces)
 *  - configurator commands:   1
 */
#define IPMAP_PROCESS_QUEUE "ipMap_Process_Queue"
#define IPMAP_PROC_MSG_COUNT (platIntfTotalMaxCountGet())
...
/*
 * Maximum number of registrations for the IP Address Conflict
 */
#define IPMAP_IP_ADDR_CONFLICT_CB_MAX 10

typedef void (* ipAddrConflictCB_t) (L7_uint32, L7_uint32, L7_uchar8 *);
...
/* IP dhcp checkpoint table */
typedef struct ipMapDhcpCkptNode_s
{
    L7_uint32       intIfNum;     /* search key */
    L7_uint32       ipAddr;
    L7_uint32       ipMask;
    L7_uint32       ipGateway;
    L7_BOOL         isAdd;        /* Add or Delete */
    void            *next;        /* avl req */

}ipMapDhcpCkptNode_t;

typedef struct ipMapDhcpCkptTbl_s {
...
typedef struct ipMapInfo_s
{
    ...
    ipMapOperRtrIntf_t      operIntf[L7_MAX_INTERFACE_COUNT];
    ...
    /* L7_TRUE if warm restart has completed */
    L7_BOOL warmRestartCompleted;

  /* L7_TRUE when the checkpoint service has notified IP MAP that a 
   * backup unit has been elected. IP MAP checkpoints the route table
   * when there is a backup. */
  L7_BOOL backupElected;

  /* L7_TRUE when IP MAP has told Configurator that the L3 hardware table
   * update is complete. */
  L7_BOOL l3HwUpdateComplete;
  ...
  /* IP MAP holds this semaphore when there are routes waiting to 
   * be checkpointed. Not held if only internal VLANs are waiting. */
  void *ckptPendingSema;

  /* bit mask corresponding to L7_NSF_PROTOCOL_t. Bit is one if 
   * NSF application expects to send post-failover routes to RTO, but 
   * hasn't yet. */
  L7_uint32 nsfRoutesPending;

  /* Timer started if any protocol indicates it will provide NSF routes
   * after a warm restart. */
  osapiTimerDescr_t *staleRouteTimer;

  /* IP Address Conflict registrations */
  ipAddrConflictCB_t       ipAddrConflictCallback[IPMAP_IP_ADDR_CONFLICT_CB_MAX];

#ifdef L7_NSF_PACKAGE
   /* DHCP Address Checkpoint table */
  ipMapDhcpCkptTbl_t    dhcpCkptTbl;
#endif /* L7_NSF_PACKAGE */
...
{                                                           \
   ...
   {                                                        \
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_IP_MAP_COMPONENT_ID,format,##args);  \
   }                                                        \
}

#define IPMAP_ERROR_SEVERE(format,args...)                  \
{                                                           \
      L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_IP_MAP_COMPONENT_ID,format,##args);  \
}
...
{                                                           \
   ...
   {                                                        \
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_IP_MAP_COMPONENT_ID,format,##args);  \
   }                                                        \
}
...
/* APIs for address conflict detection feature */
L7_RC_t ipMapAddrConflictDetectModeSet(L7_uint32 conflictDetectMode);
L7_RC_t ipMapAddrConflictDetectRun(void);
...
IPMAP_RTR_INTF_ENABLE_MODE_t ipMapMayEnableInterface (L7_uint32 intIfNum);
...
L7_RC_t ipMapSrAdd (L7_uint32 ipAddr,
                    L7_uint32 subnetMask, 
                    L7_uint32 nextHop,
