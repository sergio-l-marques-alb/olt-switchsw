    ...
    L7_arpEntry_t *pArp)
{
  ...
  if (pIp->key != ipAddr)               /* these should match */
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ARP_MAP_COMPONENT_ID,
            "%s: found entry, but lookup of %s got %s instead\n",
...
L7_RC_t ipMapArpExtenCacheSizeSet(L7_uint32 maxEnt)
{
  ...
  if ((maxEnt < (L7_uint32)L7_IP_ARP_CACHE_SIZE_MIN) ||
      (maxEnt > platRtrArpMaxEntriesGet()))
    return L7_FAILURE;
                               ...
                               t_ADR **ppArpEntry)
{
  ...
  *ppArpEntry = L7_NULLPTR;
  ...
  if((pArpInfo->entryType == ARP_ENTRY_TYPE_DYNAMIC) ||
     (pArpInfo->entryType == ARP_ENTRY_TYPE_GATEWAY))
  {
    /* Calling the checkpointing code for NSF */
    arpMapCheckpointCallback(ARP_ENTRY_ADD, pIp->key, pIp->intIfNum,
                             pIp->arpEntry.macAddr);
  }
#endif
                               ...
                               Bool macHasChanged)
{
  ...
  if (macHasChanged == TRUE)
  {
    ...
    {
      /* Calling the checkpointing code for NSF */
      arpMapCheckpointCallback(ARP_ENTRY_MODIFY, pIp->key, pIp->intIfNum,
                               pIp->arpEntry.macAddr);
    }
#endif
  }
...
e_Err ipMapArpExtenCacheRemove(t_Handle arpCacheId, t_ADR *pArpInfo)
{
  ...
  {
    /* Calling the checkpointing code for NSF */
    arpMapCheckpointCallback(ARP_ENTRY_DELETE, pIp->key, pIp->intIfNum,
                             pIp->arpEntry.macAddr);
  }
#endif
...
* @end
*********************************************************************/
void ipMapArpExtenAddrConflictStatsShow()
{
  t_ARP *p_A = (t_ARP *)ipMapArpCtx_g.arpHandle;
  t_ADDREnt *addrEnt;
