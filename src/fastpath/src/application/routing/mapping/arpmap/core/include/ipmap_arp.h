...
typedef enum ARP_EVENT_TYPE_s
{
  ARP_ENTRY_ADD = 0,
  ARP_ENTRY_MODIFY,
  ARP_ENTRY_DELETE,
  ARP_ENTRY_FLUSH_ALL,
  ARP_ENTRY_EVENT_LAST
} ARP_EVENT_TYPE_t;
...
/* ARP Mapping layer definition for checkpointed arp data */
typedef struct
{
  L7_uint32       ipAddr;                   /* SEARCH KEY: ARP Entry's IP address */
  L7_uint32       intIfNum;                 /* SEARCH KEY: Internal interface number */
  L7_uchar8       macAddr[L7_MAC_ADDR_LEN]; /* ARP entry MAC addr */
  L7_BOOL         deleteFlag;               /* Whether this arp entry is marked for deletion */
...
typedef void (* arpEventCallbackFn_t)(ARP_EVENT_TYPE_t, L7_uint32, L7_uint32, L7_uchar8 *);
...
typedef struct
{
  ...
  /* ARP Checkpoint table */
  struct
  {
    L7_uint32           treeEntryMax;
    avlTreeTables_t     *treeHeap;
    L7_uint32           treeHeapSize;
    ipMapArpCkptNode_t  *dataHeap;
    L7_uint32           dataHeapSize;
    avlTree_t           treeData;
  } ckptTbl;

  /* ARP MAP holds this semaphore when a checkpoint is pending */
  void *ckptPendingSema;

  /* would be set in ARP control block of the active manager
