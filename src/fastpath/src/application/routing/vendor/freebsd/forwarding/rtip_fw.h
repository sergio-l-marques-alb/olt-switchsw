...
* @filename  rtip_fw.h
*
* @purpose   Header File for Firewall related stuff
*
* @component 
*
* @comments  
...
#define IP_FW_ICMPTYPES_DIM	(IP_FW_ICMPTYPES_MAX / (sizeof(unsigned) * 8))
    ...
    u_long fw_loghighest;		/* highest number packet to log */
};
...
struct ipfw_flow_id {
    ...
    u_char proto ;    
    u_char flags ;    /* protocol-specific flags */
} ;
...
struct ipfw_dyn_rule {
    ...
    u_int type ;			/* rule type			*/
    u_int expire ;			/* expire time			*/
    u_long pcnt, bcnt;		/* match counters		*/
    u_int bucket ;			/* which bucket in hash table	*/
    u_int state ;			/* state of this rule (typ. a   */
...
#endif
struct sockopt;
typedef	int ip_fw_chk_t (struct ip **, int, struct rtm_ifnet *, u_short *,
	     struct rtmbuf **, struct ip_fw_chain **, struct sockaddr_in **);
