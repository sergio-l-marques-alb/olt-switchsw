...
static char __fileid[] = "$Header: lvl7dev\src\application\protocol\ospf\sparo.c 1.2.2.7 2003/02/20 16:48:44EST anayar Exp  $";
#endif
...
static e_Err LsaOriginate(t_RTO *p_RTO, t_ARO *p_ARO, t_IFO *p_IFO, void *p_F, e_S_LScodes LsaType,
...
void ospfDebugAgingTableShow(L7_uint32 top)
{
  ...
  {
    printf("\nOSPFv2 aging table not running");
    return;
  }
...
Bool ARO_Delete(t_Handle ARO_Id, ulng flag)
{
   ...
   while(er == E_OK)
   {
     ...
     if(HL_FindFirst(p_RTO->RtbHl, (byte *)&VpnCos, (void *)&p_RTB) == E_OK)
     {
        ...
        p_RtEntry = o2RouteFind(p_RTB, p_RTB->RtbNtBt, p_Entry->NetIpAdr, p_Entry->SubnetMask);
        if (p_RtEntry)
        {
           if(p_RTO->Clbk.p_RoutingTableChanged && (p_RtEntry->isRejectRoute == TRUE))
           {
              ...
              o2RouteDelete(p_RTB, p_RTB->RtbNtBt, p_RtEntry->DestinationId, 
                            p_RtEntry->IpMask, TRUE);
           }
        }
     }
   ...
   while(e == E_OK)
   {
      o2RoutesBrowse(p_RTB, p_RTB->RtbRtBt, RteOfAreaDeleteAndNotify, (ulng)p_ARO);
      o2RoutesBrowse(p_RTB, p_RTB->RtbNtBt, RteOfAreaDeleteAndNotify, (ulng)p_ARO);
 ...
 e_Err ARO_Config(t_Handle ARO_Id, t_S_AreaCfg *p_Cfg)
  {
   ...
   switch(p_Cfg->AreaStatus)
   {
      ...
      case  ROW_CHANGE:
         ...
         else if (p_Cfg->ExternalRoutingCapability == AREA_IMPORT_EXTERNAL)
         {
           p_ARO->ImportSummaries = TRUE;
         }
         /* else if area currently configured as an NSSA, don't set the area's
          * import summaries option based on stub area configuration. */
...
/*********************************************************************
 * @purpose  Validate an LSA before installing in the LSDB.
 *
 * @param    lsaHeader  @b{(input)}  Pointer to start of LSA header
 *
 * @returns  L7_TRUE if LSA passes validation checks
 *           L7_FALSE if considered invalid
 *
 * @notes    
 *
 * @end
 * ********************************************************************/
L7_BOOL o2LsaValid(t_RTO *p_RTO, t_S_LsaHeader *lsaHeader)
{
  t_S_LinkDscr *p_Link;
  word linkNum;
  t_S_RouterLsa *p_Lsa;
  L7_ushort16 lsaLength = A_GET_2B(lsaHeader->Length);
  L7_uchar8 *pktEnd = ((L7_uchar8*)(lsaHeader)) + lsaLength;
  ...
  switch (lsaHeader->LsType)
  {
    case S_ROUTER_LSA:
      ...
      while (linkNum--)
      {
        if ((L7_uchar8*) p_Link > pktEnd)
        {
          if (p_RTO->ospfTraceFlags & OSPF_TRACE_LS_UPDATE)
          {
            ...
            L7_uchar8 lsIdStr[OSAPI_INET_NTOA_BUF_SIZE];
            L7_uchar8 advRouterStr[OSAPI_INET_NTOA_BUF_SIZE];
            osapiInetNtoa(A_GET_4B(lsaHeader->LsId), lsIdStr);
            osapiInetNtoa(A_GET_4B(lsaHeader->AdvertisingRouter), advRouterStr);
            sprintf(traceBuf, "OSPF %s with LSID: %s from advertising router %s is invalid.",
                    lsaTypeNames[lsaHeader->LsType], lsIdStr, advRouterStr);
      ...
      break;
    case S_NETWORK_LSA:
      /* LSID of network LSA is IP address of DR. Can't be 0.0.0.0. */
      if (A_GET_4B(lsaHeader->LsId) == 0)
      {
        if (p_RTO->ospfTraceFlags & OSPF_TRACE_LS_UPDATE)
        {
          char traceBuf[OSPF_MAX_TRACE_STR_LEN];
          ...
          osapiInetNtoa(A_GET_4B(lsaHeader->LsId), lsIdStr);
          osapiInetNtoa(A_GET_4B(lsaHeader->AdvertisingRouter), advRouterStr);
          sprintf(traceBuf, "OSPF %s from advertising router %s has invalid LSID %s.",
                  lsaTypeNames[lsaHeader->LsType], advRouterStr, lsIdStr);
          RTO_TraceWrite(traceBuf);    
        } 
        return L7_FALSE;
      }
    ...
    default:
     /* No need to throw an error if default case invoked. LS Type already validated above. */
      ;
  }
 ...
 * @end
 * ********************************************************************/
t_Handle GET_OWNER_HL(t_RTO *p_RTO, t_ARO *p_ARO, t_IFO *p_IFO, e_S_LScodes lsaType)
{
  if (!o2LsaTypeValid(lsaType))
    return NULL;

  if (lsaType == S_AS_EXTERNAL_LSA) 
    return p_RTO->AsExternalLsaHl;
  if (lsaType == S_AS_OPAQUE_LSA) 
    return p_RTO->AsOpaqueLsaHl;
  if (lsaType == S_LINK_OPAQUE_LSA)
  {
    if (p_IFO)
      return p_IFO->LinkOpaqueLsaHl;
    else
      return NULL;
  }
   ...
   switch(p_AdrRange->AggregateStatus)
   {
      ...
      case  ROW_CREATE_AND_WAIT:
         ...
         if(HL_Init(OWN_LINK, 2*sizeof(SP_IPADR), offsetof(t_RoutingTableEntry, DestinationId),
                      HL_LARGE_TABLE_SIZE, NULL, NULL, &p_Entry->AgrRteHl) != E_OK)
         {
            XX_Free(p_Entry);
            return E_FAILED;
         }
...
/*********************************************************************
 * @purpose         Copy an LS database entry to a structure that can be 
 *                  given to the UI. 
 *
 *
...
void updateLsdbEntry(L7_ospfLsdbEntry_t *p_Lsa, t_A_DbEntry *p_Entry)
{
  ...
  t_IFO *p_IFO = (t_IFO*) p_Entry->IFO_Id;
  ...
  {
    p_Lsa->intIfNum = p_IFO->Cfg.IfIndex;
  }
...
/*********************************************************************
 * @purpose   Given a set of parameters, p_Lsa, that uniquely identify an LSA
 *            and given the area that the LSA belongs to (ARO_Id), find the
 *            LSA in the link state database and make a copy of the entire
 *            LSA.
 *
 * @param  ARO_Id              @b{(input)}  Area Object handle. Always non-NULL, 
 *                                          even if LSA does not have area flooding
 *                                          scope. In that case, it's area 0.
 * @param  p_Lsa               @b{(input)}  parameters identifying an LSA
 * @param  p_LsdbAdvertisement @b{(output)} LSDB Advertisement
 *
 * @return    E_OK       LSA found and copied
 * @return    E_FAILED   LSA not found
 *
 * @notes
 *
 * @end
                            ...
                            L7_char8 **p_LsdbAdvertisement)
{
  ...
  if((p_Lsa->LsdbType >= S_LAST_LSA_TYPE) ||
     ...
     (p_Lsa->LsdbType == S_GROUP_MEMBER_LSA) ||
  ...
  if (p_Lsa->LsdbType == S_LINK_OPAQUE_LSA)
  {
    ...
    if ((HL_FindFirst(p_RTO->IfoIndexHl, (byte*) &p_Lsa->intIfNum, (void**) &p_IFO) != E_OK) ||
        (p_IFO == NULL))  /* help coverity out here */
    {
      /* previous LSA has gone away */
      return E_FAILED;
    }
  }
  ...
  if((AVLH_Find(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_Lsa->LsdbType),
                (byte *)&dbKey,
                (void **)&p_Entry,0)) != E_OK)
  {
    return E_FAILED;
 ...
 *
 *
 * @param ARO_Id    @b{(input)}  Area Object handle. Always non-NULL regardless
 *                               of LSA type. If LSA type does not have area scope
 *                               then this is the area 0 object.
 * @param p_Lsa     @b{(input)}  LSA Entry
 *
 * @return              E_OK       success
 * @return              E_FAILED   failed
...
/*********************************************************************
 * @purpose         Copy a database entry for an opaque LSa to a structure
 *                  that can be given to the UI.
 *
 * @param p_Lsa     @b{(output)}  output structure
 * @param p_Entry   @b{(input)}  Database Entry
 *
 * @notes      The LS type of the db entry is always the same as the 
 *             LS type of the previous LSA. 
 *
 * @end
 *--------------------------------------------------------------------*/
void updateOpaqueLsdbEntry(L7_ospfOpaqueLsdbEntry_t *p_Lsa, t_A_DbEntry *p_Entry)
{
  ...
  /* Not really necessary, but left for completeness */
  p_Lsa->LsdbType = p_Entry->Lsa.LsType;

  p_ARO = (p_Lsa->LsdbType == S_AS_OPAQUE_LSA) ? L7_NULLPTR : (t_ARO *)p_Entry->ARO_Id;
  p_Lsa->AreaId = (p_Lsa->LsdbType == S_AS_OPAQUE_LSA) ? 0 : p_ARO->AreaId;
...
/*********************************************************************
 * @purpose         Copy an opaque LSA for presentation in the UI.
 *
 * @param RTO_Id    @b{(input)}  OSPF instance
 * @param ARO_Id    @b{(input)}  Area Object handle
 * @param IFO_Id    @b{(input)}  interface. Only required for link opaque LSAs.
 * @param p_Lsa     @b{(input)}  LSA Entry
 * @param p_LsdbAdvertisement @b{(output)} LSDB Advertisement
 *
 * @return              E_OK       success
 ...
 * @end
 *--------------------------------------------------------------------*/
e_Err ARO_OpaqueAdvertisement(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id,
    ...
    L7_char8 **p_LsdbAdvertisement)
{
  ...
  if ((p_Lsa->LsdbType == S_LINK_OPAQUE_LSA) && !p_IFO)
    return E_FAILED;
  ...
  OpaqueLsaHl = GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_Lsa->LsdbType);
  if ((AVLH_Find(OpaqueLsaHl, (byte *)&dbKey, (void **)&p_Entry, 0)) != E_OK)
  {
    return E_FAILED;
  }

  updateOpaqueLsdbEntry(p_Lsa, p_Entry);  /* necessary? */
...
/*********************************************************************
 * @purpose    Gives the next opaque LSA of a single type entry to external world
 *
 * @param RTO_Id    @b{(input)}  OSPF instance
 * @param ARO_Id    @b{(input)}  Area Object handle
 * @param IFO_Id    @b{(input)}  interface
 * @param p_Lsa     @b{(input/output)}  previous/next LSA Entry
 *
 * @return     E_OK       next LSA found
 * @return     E_FAILED   no next LSA found
 *
 * @notes      This API only iterates through LSAs of a single type
 *             (9, 10, or 11). This API does not start on type 10 LSAs
 *             after the last type 9 is returned. To walk all opaque  
 *             types, use ARO_LsdbNextEntry().
 *
 * @end
 *--------------------------------------------------------------------*/
e_Err ARO_OpaqueLsdbNextEntry(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id,
                              L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  e_Err          e;
  t_A_DbEntry    *p_Entry = NULL;
  ...
  if (e != E_OK)
  {
    if (p_Lsa->LsdbType == S_LINK_OPAQUE_LSA)
    {
      ...
      nextIpAddr = p_Lsa->IpAdr;
      nextIfIndex = p_Lsa->IfIndex;
      while (ospfIfTable_GetNext(p_RTO->ospfMibHandle, nextIpAddr,
                                 nextIfIndex, &nextIpAddr, &nextIfIndex) != NULL)
      {
        p_IFO = ospfIfTable_GetOwner(p_RTO->ospfMibHandle, nextIpAddr, nextIfIndex);  
        if (p_IFO)
        {
          OpaqueLsaHl = GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_Lsa->LsdbType);
          if (AVLH_FindNext(OpaqueLsaHl, (byte *)&currentKey, (void **)&p_Entry) == E_OK)
          {
            /* copy db entry to output */
    ...
    /* Either not link opaque, or no link opaque found on any interface */
    return E_FAILED;
  }

  /* Copy database entry to output arg */
  updateOpaqueLsdbEntry(p_Lsa, p_Entry);
...
/*********************************************************************
 * @purpose         Given an LSID and advertising router for an opaque LSA,
 *                  find the corresponding database entry and return all the
 *                  attributes of the LSA.
 *
 * @param RTO_Id    @b{(input)}  OSPF instance
 * @param ARO_Id    @b{(input)}  Area Object handle
 * @param IFO_Id    @b{(input)}  interface
 ...
 *
 * @return          E_OK       success
 * @return          E_FAILED   failed to find the matching database entry
 *
 * @notes
 *
 * @end
 *--------------------------------------------------------------------*/
e_Err ARO_OpaqueLsdbEntry(t_Handle RTO_Id, t_Handle ARO_Id, t_Handle IFO_Id, 
                          L7_ospfOpaqueLsdbEntry_t *p_Lsa)
{
  t_A_DbEntry   *p_Entry = NULL;
  t_A_DbKey      dbKey;
...
/*********************************************************************
 * @purpose        Given the attributes of an LSA in a given area, find 
 *                 the next LSA in the same area. Report the attributes of
 *                 the next LSA. 
 *
 *
 * @param ARO_Id    @b{(input)}  Area Object handle. Required.
 * @param p_Lsa     @b{(input)}  LSA Entry
 *
 * @return              E_OK       success
 * @return              E_FAILED   failed
 *
 * @notes          Includes link opaque LSAs for interfaces in the area.
 *
 *                 SNMP uses this function to populate ospfLsdbTable
 *                 for RFC 1850; however, RFC 1850 doesn't include opaque
 *                 LSAs; so the order of link opaque LSAs, and specifically
 *                 the order in which we traverse the interfaces within an
 *                 area, really isn't defined. In RFC 4750, opaque link 
 *                 LSAs are included in ospfLsdbTable, but no order is defined
 *                 for them (the index to the table doesn't include interface). 
 *                 RFC 4750 also reports opaque link LSAs in their own table, 
 *                 ospfLocalLsdbTable, which does have a defined interface order.
 *                 So we report link opaque LSAs within the area in a random order here.
 *
 * @end
 *--------------------------------------------------------------------*/
e_Err ARO_LsdbNextEntry(t_Handle ARO_Id, L7_ospfLsdbEntry_t *p_Lsa)
{
  ...
  e = AVLH_FindNext(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_Lsa->LsdbType),
                    (byte *)&currentKey, (void **)&p_Entry);

  if(e != E_OK)
  {
    /* No more LSAs of previous type in this area. If link opaque, no more 
     * on this interface. If we are at the end of all LSAs in this Area, 
     * return failure */
    if(p_Lsa->LsdbType == (LastType))
      return E_FAILED;

    /* If iterating through link opaque LSAs, try next interface */
    if (p_Lsa->LsdbType == S_LINK_OPAQUE_LSA)
    {
      e = HL_GetNext(p_ARO->IfoHl, (void**) &p_IFO, p_IFO);
      while (e == E_OK)
      {
        if (AVLH_GetFirst(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, S_LINK_OPAQUE_LSA), 
                          (void **)&p_Entry) == E_OK)
        {
          updateLsdbEntry(p_Lsa, p_Entry);
          return E_OK;
    ...
    for(LsaType = p_Lsa->LsdbType+1; LsaType < LastType; LsaType++)
    {
      ...
      if (LsaType == S_LINK_OPAQUE_LSA)
      {
        /* Get first interface in area */
        if (HL_GetFirst(p_ARO->IfoHl, (void**) &p_IFO) != E_OK)
        {
          return E_FAILED;
        }
      }
      if (AVLH_GetFirst(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, LsaType), (void **)&p_Entry) == E_OK)
        break;
    }
    if(LsaType == LastType)
    {
...
/*********************************************************************
 * @purpose    Enter the LSDB overload state defined by RFC 3137.  
 *
 * @param  p_RTO    @b{(input)}  OSPF instance pointer
 * @param  reason   @b{(input)}  The cause of the transition to stub router mode
 *
 * @returns      E_OK
 *
 * @notes   We can get here through administrative action (max-metric router-lsa)
 *          or because of an out-of-resource condition (memory, LSDB space, 
 *          routing table space). 
 *
 * @end
 * ********************************************************************/
e_Err EnterLsdbOverload(t_Handle RTO_Id, OSPF_STUB_ROUTER_REASON_t reason)
{
    t_RTO *p_RTO = (t_RTO*) RTO_Id;
    t_ARO *p_ARO = NULL;
    t_IFO *p_IFO = NULL;
    t_NBO *p_NBO = NULL;
    e_Err err;
    L7_uchar8 logMsg[256];

    /* If in startup mode and configuration changed to always, 
     * cancel exit timer and update reason. */
    if ((p_RTO->LsdbOverload == TRUE) &&
        (p_RTO->LsdbOverloadReason == OSPF_STUB_RTR_STARTUP) &&
        ((reason == OSPF_STUB_RTR_CONFIGURED) ||
         (reason == OSPF_STUB_ROUTER_RESOURCE)))
    {
      if (TIMER_Active(p_RTO->LsdbOverloadTimer))
      {
        TIMER_Stop(p_RTO->LsdbOverloadTimer);
      }
      p_RTO->LsdbOverloadReason = reason;
      return E_OK;
    }

    if ((p_RTO->LsdbOverload == TRUE) &&
        (p_RTO->LsdbOverloadReason == OSPF_STUB_RTR_CONFIGURED) &&
        (reason == OSPF_STUB_RTR_STARTUP))
    {
      L7_uint32 duration;
      L7_uint32 exitTime = (p_RTO->p3InitTime + p_RTO->Cfg.StubRtrStartupTime);

      if (exitTime > osapiUpTimeRaw())
      {
        /* Should remain in overload */
        duration = exitTime - osapiUpTimeRaw();
        TIMER_StartSec(p_RTO->LsdbOverloadTimer, duration, FALSE,
                       o2StubRouterTimerExp, p_RTO->OspfSysLabel.threadHndle);
        L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_OSPF_MAP_COMPONENT_ID, 
                "OSPFv2 will exit stub router mode (RFC 3137) in %u seconds.",
                duration);
      }
      p_RTO->LsdbOverloadReason = reason;
      return E_OK;
    ...
    /* Configuration takes precedence over automatic. So if in stub router mode 
     * because of a resource shortage, and the user puts the router in stub router 
     * mode by configuration, the configuration action becomes the current reason
     * for being in stub router mode. */
    if ((p_RTO->LsdbOverload == FALSE) ||
        ((p_RTO->LsdbOverloadReason == OSPF_STUB_ROUTER_RESOURCE) &&
         (reason != OSPF_STUB_ROUTER_RESOURCE)))
    {
      p_RTO->LsdbOverloadReason = reason;

      if (reason == OSPF_STUB_RTR_CONFIGURED)
      {
        osapiSnprintf(logMsg, 256,
                     "OSPFv2 entering stub router mode (RFC 3137) by administrative action."); 
      }
      else if (reason == OSPF_STUB_RTR_STARTUP)
      {
        L7_uint32 duration;
        L7_uint32 exitTime = (p_RTO->p3InitTime + p_RTO->Cfg.StubRtrStartupTime);
        ...
        if (exitTime > osapiUpTimeRaw())
        {
          ...
          TIMER_StartSec(p_RTO->LsdbOverloadTimer, duration, FALSE,
                         o2StubRouterTimerExp, p_RTO->OspfSysLabel.threadHndle);
          osapiSnprintf(logMsg, 256,
                        "OSPFv2 entering stub router mode (RFC 3137) for %u seconds at startup.",
                        duration); 
        }
      }
      else if (reason == OSPF_STUB_ROUTER_RESOURCE)
      {
        osapiSnprintf(logMsg, 256,
                      "OSPFv2 entering stub router mode (RFC 3137) because "
                      "of a resource limitation. Use 'clear ip ospf stub-router' "
                      "to exit stub router mode after resolving the cause of the overload.");
      }
      L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_OSPF_MAP_COMPONENT_ID, logMsg);
    }
    ...
    while(err == E_OK)
    {
      ...
      while(err == E_OK)
      {
        ...
        /* If overriding summary metrics, also reoriginate summary LSAs. */
        if (p_RTO->Cfg.StubRtrSummaryMetricOveride)
        {
          LsaOrgSumBulk(p_ARO);
        }
...
/*********************************************************************
 * @purpose    Exit the LSDB overload state (aka stub router mode) defined by RFC 3137.  
 *
 * @param  p_RTO    @b{(input)}  OSPF instance pointer
 *
 * @returns      E_OK
 ...
 * @end
 * ********************************************************************/
e_Err o2StubRouterModeExit(t_RTO *p_RTO)
{
    t_ARO *p_ARO = NULL;
    t_IFO *p_IFO = NULL;
    t_NBO *p_NBO = NULL;
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID, 
            "OSPFv2 exiting stub router mode.");

    p_RTO->LsdbOverload = FALSE;
  
    /* For each area, reissue the router LSA with normal metrics. */
    err = HL_GetFirst(p_RTO->AroHl, (void *)&p_ARO);
    while (err == E_OK)
    {
      /* clear NBO LSA request lists for nbr's this area */
      ...
      LsaReOriginate[S_ROUTER_LSA](p_ARO, 0);
      /* don't condition this on config since config change may have caused 
       * the exit. Note that summary LSAs are based on the routing table. So
       * even though we have just updated the area's router LSA, the summaries
       * originated here will be based on routes computed with the previous
       * router LSA. Installation of the router LSA will trigger SPF. When it
       * runs, the updated routes will trigger new summaries. So why originate
       * summaries here? Some summaries, like summary defaults for stub and 
       * NSSA, are not triggered by routes. */
      LsaOrgSumBulk(p_ARO);    
...
e_Err RemoveFromRetxLists(t_ARO *p_ARO, t_A_DbEntry *p_DbEntry)
{
  ...
  while (er == E_OK)
  {
    ...
    while (e == E_OK)
    {
      ...
      {
        if ((! (L7_BIT_ISSET(p_RetxEntry->DbEntry->dbFlags, O2_DB_ENTRY_IS_CUR))) ||
 ...
 *                                AS flooding scope.
 * @param  p_IFO     @b{(input)}  interface. non-NULL only for link scope LSAs.
 ...
 * @end
 * ********************************************************************/
t_A_DbEntry *LsaInstall(t_RTO *p_RTO, t_ARO *p_ARO, t_IFO *p_IFO, 
                        void *p_Lsa, t_A_DbEntry *p_former)
{
   t_A_DbEntry   *p_DbEntry;
   ...
   if(!lsaChanged)
   {
                ...
                p_former->InitAge, &p_former->AgeIndex) == E_OK))
      {
         ...
         L7_BIT_SET(p_former->dbFlags, O2_DB_ENTRY_PERIODIC_UPDATE);
         return p_former;
      }
      else
      {
   ...
   L7_BIT_CLR(p_DbEntry->dbFlags, O2_DB_ENTRY_PERIODIC_UPDATE);

   /* if this LSA is a self-originated LSA from a neighbor during GR, 
    * flag will be set later in LsUpdateReceived(). */
   L7_BIT_CLR(p_DbEntry->dbFlags, O2_DB_ENTRY_PRE_RESTART);
   ...
   p_DbEntry->IFO_Id = (t_Handle) p_IFO;   /* non-NULL only for link scope LSAs */
   p_DbEntry->OrgTime = TIMER_SysTime();  /* installation time stamp     */
   p_DbEntry->InitAge = (word)A_GET_2B(p_DbEntry->Lsa.LsAge); /* init age */
   L7_BIT_SET(p_DbEntry->dbFlags, O2_DB_ENTRY_IS_CUR);    /* last installed instance flag */

   /* Insert the new LSA into LSA database if this is a new db entry */
   if(!p_former)
     AVLH_Insert(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, p_DbEntry->Lsa.LsType),
   ...
   if(p_DbEntry->Lsa.LsType == S_ROUTER_LSA)
   {
     if (A_GET_2B(p_DbEntry->Lsa.LsAge) < MaxAge)
     {
      RtrLsaPreParsing(p_ARO, p_DbEntry, NULL);
       ...
       {
         o2GrRouterLsaProcess(p_RTO, p_DbEntry);
       }
     }
     else
     {
         ...
         (p_RTO->NonDefExtLsaCount >= (ulng)((9*p_RTO->Cfg.ExtLsdbLimit)/10)))
      {
         ...
         if(p_RTO->NonDefExtLsaCount >= (ulng)p_RTO->Cfg.ExtLsdbLimit)
         {
            ...
            while(err == E_OK)
            {
              ...
              e = HL_GetFirst(p_tmpARO->IfoHl, (void *)&ifo);
              while(e == E_OK)
              {
                 er = HL_GetFirst(ifo->NboHl, (void *)&p_NBO);
                 while(er == E_OK)
                 {
                    HL_Browse(p_NBO->LsReq, AsExtAndNssaRqFree, 0);
                    er = HL_GetNext(ifo->NboHl, (void *)&p_NBO, p_NBO);
                 }
                 e = HL_GetNext(p_tmpARO->IfoHl, (void *)&ifo, ifo);
              }
...
Bool LsaRelease(t_RTO *p_RTO, t_Handle db, ulng flag)
{
   ...
   if(flag==1)
   {
      e = AVLH_Delete(GET_OWNER_HL(p_RTO, p_DbEntry->ARO_Id, p_DbEntry->IFO_Id, 
   ...
   L7_BIT_CLR(p_DbEntry->dbFlags, O2_DB_ENTRY_IS_CUR);    /* reset installed instance flag */
...
/*********************************************************************
 * @purpose         Flush Self-Originated LSA w/o checking if LSID 
 *                  matches ours.
 *
 * @param  db       @b{(input)}  LSA DB entry pointer
 ...
 *
 * @notes           Router should only prematurely age self-originated
 *                  LSAs. Call FlushLsa() to enforce. If caller is sure 
 *                  LSA is self-originated even if router ID doesn't match
 *                  our current router ID, then this may be called directly.
 *
 * @end
 * ********************************************************************/
Bool FlushLsaNoCheck(t_RTO *p_RTO, t_A_DbEntry *p_DbEntry, t_ARO *p_ARO)
{
   t_DelayedLsa *p_DlyLsa = NULLP;
   t_DlyLsaKey   dlykey;
   t_Handle delayList = NULL;

   /* if in graceful restart, don't flush */
   if (o2GracefulRestartInProgress(p_RTO))
   {
     return TRUE;
   }
 ...
 * @end
 * ********************************************************************/
Bool FlushLsa(t_Handle db, ulng aroId, Bool unused)
{
   t_A_DbEntry *p_DbEntry = (t_A_DbEntry *) db;
   ...
   return FlushLsaNoCheck(p_RTO, p_DbEntry, p_ARO);
}
                  ...
                  t_A_DbEntry *p_DbEntry, Bool *p_FloodedBack)
{
   ...
   while(e == E_OK)
   {
      ...
      while(er == E_OK)
      {
         ...
         /* If neighbor is in the middle of a graceful restart, and strict LSA 
          * checking is enabled, terminate GR if LSA indicates a topology change. */
         if (o2NeighborIsRestarting(p_NBO) && p_RTO->Cfg.StrictLsaChecking)
         {
           if (!L7_BIT_ISSET(p_DbEntry->dbFlags, O2_DB_ENTRY_PERIODIC_UPDATE))
           {
             ...
             osapiInetNtoa(A_GET_4B(p_DbEntry->Lsa.LsId), lsidStr);
             osapiInetNtoa(A_GET_4B(p_DbEntry->Lsa.AdvertisingRouter), advRtrStr);
             osapiInetNtoa(p_ARO->AreaId, areaIdStr);
             L7_LOGF(L7_LOG_SEVERITY_CRITICAL, L7_OSPF_MAP_COMPONENT_ID, 
              "Exiting helper mode with neighbor at %s. Flooding area %s %s with LSID %s from "
              "advertising router %s to restarting router.",
              nbrAddrStr, areaIdStr, lsaTypeNames[p_DbEntry->Lsa.LsType], lsidStr, advRtrStr);
             o2HelperModeExit(p_NBO, OSPF_HELPER_TOPO_CHANGE);
           }
         }
 ...
 * @param LsType    @b{(input)}  9/10/11 Opaque LSA type
 ...
 * @end
 * ********************************************************************/
e_Err OpaqueGenerate(t_Handle Id, byte LsType, byte OpqType,
                      ulng OpqId, byte *p_Data, word DataLen)
{
  ...
  if ((LsType != S_AS_OPAQUE_LSA) && (!p_ARO || !p_ARO->OperationState))
    return E_FAILED;
  ...
  /* Set LSA length */
  B_SetLength(F_GetFirstBuf(p_F), DataLen);
  F_GetLength(p_F) = DataLen;

  /* Go to the common part of all LSA types origination procedure */
  LsaOriginate(p_RTO, p_ARO, p_IFO, p_F, LsType,
               ((ulng)OpqType << 24) | (OpqId & 0x00FFFFFFL), FALSE, FALSE,
               OSPF_OPT_O_BIT);
                            ...
                            t_A_DbEntry **p_foundLsa, SP_IPADR *p_LsId)
{
   ...
   t_A_DbKey key;
   e_Err    err;
   t_Handle lsaHashList = GET_OWNER_HL(p_RTO, p_ARO, NULL, LsType);
   ...
   if(er != E_OK)
   {
      if(p_DbEntry && (A_GET_2B(p_DbEntry->Lsa.LsAge) < MaxAge))
      {
         ...
         else if(NetMask < found_mask)
         {
            ...
            LsaOriginate(p_RTO, p_ARO, NULL, p_NewF, LsType, 
                         DestAddr |~found_mask, 0, FALSE, Options);
         }
      }
      else
         LsId = DestAddr;
...
{
  int j;
  word metric = InterfaceMetric(p_IFO);
  ...
  AddLinkToRtrLsa(*p_LinkDscr, p_IFO->Cfg.IpAdr, p_IFO->Cfg.IpMask, S_STUB_NET, metric);
  (*p_LinkDscr)++;
  (*links)++;
                                ...
                                byte *pktEnd, word *links)
{
  ...
  if (p_IFO->Cfg.State > IFO_WAIT)
  {
    ...
    if (p_IFO->DrId == router_id)
    {
      ...
      while (e == E_OK)
      {
        if ((p_NBO->State == NBO_FULL) || o2NeighborIsRestarting(p_NBO))
        {
 ...
 * @param prm       @b{(input)}  force origination flag
 *
...
static e_Err LsaOrgRouter(t_Handle Id, ulng prm)
{
  ...
  if (lsaFull)
  {
    ...
    LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_ROUTER_LSA, router_id, 0, forceOrig, Option);
...
static e_Err LsaOrgNetwork(t_Handle Id, ulng prm)
{
   ...
   * data in case any interface in the area runs MD5. */
   ...
   if (nr != 0)
   {
      ...
      /* Go to the common part of all LSA types origination procedure */
      LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_NETWORK_LSA, p_IFO->Cfg.IpAdr, FALSE, FALSE, 0);
   }
   else
   {
      ...
      if((AVLH_Find(p_ARO->NetworkLsaHl,
         ...
         (L7_BIT_ISSET(former->dbFlags, O2_DB_ENTRY_IS_CUR)) && (A_GET_2B(former->Lsa.LsAge) < MaxAge))
      {
...

/*********************************************************************
 * @purpose     For a given target area, originate all summary LSAs 
 *              that should be originated into the area. 
 *
 * @param Id    @b{(input)}  target area pointer
 *
 * @returns     E_OK or E_FAILED
 *
 * @notes       A T3 summary is originated for each non-default network route,
 *              taking into consideration area ranges, transit area status, etc.
 *              A T4 summary is originated for each ASBR route. 
 *              Summary defaults are originated into stub and NSSAs.
 *
 * @end
 * ********************************************************************/
e_Err LsaOrgSumBulk(t_Handle Id)
{
   ...
   t_RTO         *p_RTO = (t_RTO *) p_ARO->RTO_Id;
   t_RTB         *p_RTB;
   Bool           ntLookup = TRUE;     /* True when processing the network route table */
   t_Handle       routetbl = NULLP;    /* Tree of network or router routes */
   e_Err          er       = E_FAILED;
   t_RoutingTableEntry *p_Rte;
   t_AgrEntry *p_AreaRange;
...
getfirst:
   er = o2RouteGetFirst(p_RTB, routetbl, &p_Rte);
   while (er == E_OK)
   {
...
nextEntry:
      er = o2RouteGetNext(p_RTB, routetbl, &p_Rte);
   }
   ...
   /* Originate summaries for active area ranges. */
   for (er = HL_GetFirst(p_ARO->AdrRangeHl, (void *)&p_AreaRange);
        er == E_OK;
        er = HL_GetNext(p_ARO->AdrRangeHl, (void *)&p_AreaRange, p_AreaRange))
   {
     if ((p_AreaRange->LsdbType == AGGREGATE_SUMMARY_LINK) &&
         (p_AreaRange->AggregateEffect == AGGREGATE_ADVERTISE))
     {
       OrigAggregateSummary(p_ARO, p_AreaRange);
     }
   }

   /* If target area is a stub area, originate a T3 default. */
   p_ARO->StubDefLSAOriginated = FALSE;
   StubDefaultLsaOriginate(p_ARO);

   /* If target area is an NSSA, originate the appropriate defaults */
   NssaDefaultLsaOriginate(p_ARO);
...
static e_Err LsaOrgSummary(t_Handle Id, ulng prm)
{
   ...
   if(p_Rte->DestinationType & DEST_TYPE_IS_AS_BOUNDARY_ROUTER)
   {
      ...
      if (p_RTO->LsdbOverload && p_RTO->Cfg.StubRtrSummaryMetricOveride)
      {
        A_SET_4B((p_RTO->Cfg.StubRtrSummaryMetric & 0x00ffffffL), p_Lsa);
      }
      else
      {
      ...
      LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_ASBR_SUMMARY_LSA, 
   ...
   if(!LsaSummaryCondense(p_ARO, p_Rte))
   {
      ...
      /* Go to the common part of all LSA types origination procedure */
      LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_IPNET_SUMMARY_LSA, LsId, 
                   0, FALSE, Options);
...
/*********************************************************************
 * @purpose  Determine if OSPF is originating a T3 summary LSA into 
 *           a given area for a given prefix. 
 *
 * @param p_RTO   @b{(input)}   OSPF instance
 * @param p_ARO   @b{(input)}   target area
 * @param prefix  @b{(input)}   destination prefix
 * @param netmask @b{(input)}   network mask
 *
 * @returns    TRUE - OSPF is originating a T3 summary 
 *
 * @notes      A T3 summary is originated in a given area if there is 
 *             an active address range in another area and the range
 *             address exactly matches prefix/netmask
 *
 * @end
 * ********************************************************************/
Bool o2OriginatingRangeSummary(t_RTO *p_RTO, t_ARO *p_ARO, 
                               L7_uint32 prefix, L7_uint32 netmask)
{
  e_Err e;
  t_ARO *otherArea = NULL;
  t_RoutingTableKey destKey;
  t_AgrEntry *p_AdrRange = NULL;

  destKey.DstAdr = prefix;
  ...
  while (e == E_OK)
  {
    if (otherArea != p_ARO)
    {
      if ((HL_FindFirst(otherArea->AdrRangeHl, (byte*)&destKey.DstAdr, 
...
static Bool LsaSummaryCondense(t_ARO *p_ARO, t_RoutingTableEntry *p_Rte)
{
  ...
  while (er == E_OK)
  {
    ...
    /* aggregation summary route itself.                            */
        ...
        (p_Rte == &p_AdrRange->AggregateRte))
    {
      if ((p_AdrRange->AggregateEffect == AGGREGATE_ADVERTISE) &&
          p_AdrRange->alive)
      {
        return FALSE;      /* advertise the summary */
      }
      else
      {
        return TRUE;       /* don't advertise the summary */
      }
    }
         ...
         (p_AdrRange->NetIpAdr & p_AdrRange->SubnetMask)))
    {
      ...
      /* If summary LSA for this dest is being originated, flush it */
      ...
      /* Check if the "aggregation summary-LSA" has to be reoriginated */
      if ((p_AdrRange->AggregateEffect == AGGREGATE_ADVERTISE) &&
          (o2AggrSummaryLsaCostCorrect(p_RTO, p_ARO, p_AdrRange) == FALSE))
      {
        o2ReoriginateAggrSummary(p_RTO, p_ARO, p_AdrRange);
        return TRUE;
      }
      break;
...
/*********************************************************************
 * @purpose       Determine whether the cost advertised in the most recent instance
 *                of a T3 LSA for an aggregate matches the largest cost route within
 *                the aggregate.
 *
 * @param p_RTO        @b{(input)}  OSPF instance
 * @param p_ARO        @b{(input)}  area where summary would be advertised
 * @param p_AdrRange   @b{(input)}  route range
 *
 * @returns TRUE  if LSA cost matches aggregate cost
 *          FALSE if no LSA is currently origated for the aggregate, or the costs
 *                do not match
 *
 * @notes
 *
 * @end
 * ********************************************************************/
Bool o2AggrSummaryLsaCostCorrect(t_RTO *p_RTO, t_ARO *p_ARO, t_AgrEntry *p_AdrRange)
{
  t_A_DbEntry *p_AggrDbEntry;
  SP_IPADR LsId = 0;

  /* If cost for the range is out of date, recompute it. */
  ...
  else
  {
     ...
     return (lsaMetric == (p_AdrRange->LargestCost & 0x00ffffffL));
  }
}

/*********************************************************************
 * @purpose       Reoriginate the T3 LSA for a route range. 
 *
 * @param p_RTO        @b{(input)}  OSPF instance
 * @param p_ARO        @b{(input)}  area where summary is to be advertised
 * @param p_AdrRange   @b{(input)}  route range
 *
 * @returns E_OK      if successfully originated 
 *
 * @notes
 *
 * @end
 * ********************************************************************/
e_Err o2ReoriginateAggrSummary(t_RTO *p_RTO, t_ARO *p_ARO, t_AgrEntry *p_AdrRange)
{
  byte Options = 0;
  ulng cost = p_AdrRange->LargestCost;
  byte *p_Lsa, *p_start;
  void *p_F;
  t_A_DbEntry *formerInstance;
  SP_IPADR LsId;

  /* If already originating an LSA for the aggregate, be sure to use the same 
   * LSID as the former instance. */
  if (FindSummaryOrASExtLsa(p_RTO, p_ARO, S_IPNET_SUMMARY_LSA,
                            p_AdrRange->NetIpAdr, p_AdrRange->SubnetMask,
                            &formerInstance, &LsId) != E_OK)
  {
    /* No former instance. Get new LSID. */
    if (GetLsIdForSummOrASExtLsa(p_RTO, p_ARO, S_IPNET_SUMMARY_LSA, 
                                 p_AdrRange->NetIpAdr, 
  ...
  /* Allocate frame for LSA with sizeof(t_S_LsaHeader) offset */
  if ((p_F = LSA_Alloc(p_ARO, S_IPNET_SUMMARY_LSA)) == NULL)
    return E_FAILED;
  ...
  return LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_IPNET_SUMMARY_LSA, LsId, 0, FALSE, Options);
}
...
e_Err StubDefaultLsaOriginate(t_ARO *p_ARO)
{
   ...
   /* Suppress if in graceful restart */
   if (o2GracefulRestartInProgress(p_RTO))
   {
     return E_OK;
   }
      ...
      case  RTE_ADDED:
        ...
        */
                                    ...
                                    p_AsExt->DestNetIpMask, &p_AsExt->LsId) != E_OK)
        {
         ...
         /* Go to the common part of all LSA types origination procedure */
         LsaOriginate(p_RTO, NULL, NULL, p_F, S_AS_EXTERNAL_LSA, 
                      p_AsExt->LsId, FALSE, FALSE, 0);
...
static Bool OrigT5ToNssaDest(t_RTO *p_RTO, t_S_AsExternalCfg *p_AsExt)
{
    ...
    if ((AVLH_Find(p_RTO->AsExternalLsaHl, (byte *)&key, 
        ...
        (L7_BIT_ISSET(p_AsExtDbEntry->dbFlags, O2_DB_ENTRY_IS_CUR)) && 
  ...
  */
  if(p_AsExt->ForwardingAdr != DefaultDestination)
  {
    if(HL_FindFirst(p_RTO->RtbHl, (byte*)&CoS, (void*)&p_RTB) == E_OK)
    {
      ...
      p_Rte = o2LongestPrefixMatch(p_RTB, p_RTB->RtbNtBt, p_AsExt->ForwardingAdr);
      if (p_Rte)
      {
  ...
  */
  ...
  case RTE_ADDED:
    ...
    */
    ...
    while(e == E_OK)
    {
      ...
      if((A_GET_4B(p_DbEntry->Lsa.LsId) == p_AsExt->LsId) &&
         ((isMetric2 == p_AsExt->IsMetricType2) && 
          (A_GET_3B(p_NssaLsa->Metric) == p_AsExt->MetricValue)) &&
    ...
    */
    ...
    Option = pFlag ? OSPF_OPT_P_BIT : 0;

    /* Go to the common part of all LSA types origination procedure */
    LsaOriginate(p_RTO, p_ARO, NULL, p_F, S_NSSA_LSA, p_AsExt->LsId, 
                 0, FALSE, Option);
 ...
 * @param p_IFO       @b{(input)}  interface. NULL for LSAs with area or AS scope
                          ...
                          SP_IPADR LinkStateId, Bool ReOrig, Bool forceOrig, byte Option)
{
   ...
   if ((LsaType == S_LINK_OPAQUE_LSA) && !p_IFO)
   {
     F_Delete(p_F);
     return E_FAILED;
   }
   ...
   /* if in graceful restart, don't originate */
   if (o2GracefulRestartInProgress(p_RTO) && (LsaType < S_LINK_OPAQUE_LSA))
   {
     F_Delete(p_F);
     return E_OK;
   }
   ...
   if (p_RTO->ospfTraceFlags & OSPF_TRACE_LSA_ORIG)
   {
     ...
     sprintf(traceBuf, 
             "Originating area %s interface %s %s %s. Reorig: %s. Option: %#x",
             areaStr, ifName, lsaTypeNames[LsaType], lsidStr, ReOrig ? "TRUE" : "FALSE",
             Option);
     RTO_TraceWrite(traceBuf);    
   }
   ...
   p_former = NULL;
   AVLH_Find(GET_OWNER_HL(p_RTO, p_ARO, p_IFO, LsaType), (byte *)&key, (void *)&p_former, 0);
   ...
   if ((LsaType == S_AS_EXTERNAL_LSA) || (LsaType == S_AS_OPAQUE_LSA))
   {
     delayList = p_RTO->DelayedLsa;
   ...
   /* Don't enforce the delay interval for grace LSAs (the only 
    * link opaque LSAs we originate). If we need to eventually enforce
    * the MinLSInterval for link opaque LSAs, we need to create a 
    * new delay list on t_IFO. */ 
   if ((LsaType != S_LINK_OPAQUE_LSA) &&
       !ReOrig && p_former && (GET_INTERVAL(p_former->OrgTime) < MinLSInterval))
   {
   ...
   if(p_former)
   {
     ...
     if (!forceOrig && !LsaContentDiff(p_F, p_former))
     {
       ...
       if (L7_BIT_ISSET(p_former->dbFlags, O2_DB_ENTRY_PRE_RESTART))
       {
         /* We have originated an LSA identical to a pre-restart LSA. 
          * Claim it as our own. */
         L7_BIT_CLR(p_former->dbFlags, O2_DB_ENTRY_PRE_RESTART);
       }
       F_Delete(p_F);
       return E_OK;
     }
      ...
      if(A_GET_4B(p_former->Lsa.SeqNum) == MaxSequenceNumber)
      {
        ...
        */
        L7_BIT_SET(p_former->dbFlags, O2_DB_ENTRY_SEQNO_OVER);
   ...
   p_DbEntry = LsaInstall(p_RTO, p_ARO, p_IFO, p_F, p_former);
...
static void *LSA_Alloc(t_ARO *p_ARO, e_S_LScodes lsaType)
{
   ...
   {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Max allowed OSPF buf len is zero");
      return NULL;
   }
   ...
   {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Unable to allocate buffer for OSPF LSA.");
      return NULL;
   }
   ...
   {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
              "Unable to allocate frame for OSPF LSA.");
      return NULL;
   }
...
e_Err LsaRefresh (t_RTO *p_RTO, t_ARO *p_ARO, t_A_DbEntry *p_DbEntry)
{
  ...
  LsaOriginate(p_RTO, p_ARO, p_DbEntry->IFO_Id, p_NewF, p_DbEntry->Lsa.LsType, 
...
static Bool AgingCheckLsaCheckSum (t_Handle user, byte *value, ulng param)
{
   ...
   {
     L7_LOGF(L7_LOG_SEVERITY_INFO, L7_OSPF_MAP_COMPONENT_ID,
             "LSA Checksum error detected for LSID %s checksum 0x%x. OSPF Database may be corrupted.",
...
static Bool AgingFlushLsa(t_Handle user, byte *value, ulng param)
{
   ...
   if(p_RTO->retxEntries >= p_RTO->Cfg.MaxLsaRetxEntries)
   {
     ...
     L7_BIT_SET(p_DbEntry->dbFlags, O2_DB_ENTRY_FLUSHING_HELD);
     return TRUE;
   }
   else
   {
     /* Ready to flush and delete this overaged lsa */
     L7_BIT_CLR(p_DbEntry->dbFlags, O2_DB_ENTRY_FLUSHING_HELD);
   }
...
static Bool AgingRemoveOverAgedLsa(t_Handle user, byte *value, ulng param)
{
  ...
  if(L7_BIT_ISSET(p_DbEntry->dbFlags, O2_DB_ENTRY_FLUSHING_HELD))
  {
  ...
  if (!foundInRetxList)
  {
    ...
    if(L7_BIT_ISSET(p_DbEntry->dbFlags, O2_DB_ENTRY_SEQNO_OVER))
    {
      if (p_RTO->ospfTraceFlags & OSPF_TRACE_AGING)
      {
         char traceBuf[OSPF_MAX_TRACE_STR_LEN];
      ...
      /* increment it                                            */
      L7_BIT_CLR(p_DbEntry->dbFlags, O2_DB_ENTRY_SEQNO_OVER);
  ...
  /* Check Delayed LSA list */
   ...
   while((err == E_OK) && p_DlyLsa)
   {
      ...
      if(GET_INTERVAL(p_DlyLsa->OrgTime) >= MinLSInterval)
      {
         ...
         LsaOriginate(p_RTO, tmp_DlyLsa.ARO_Id, NULL, tmp_DlyLsa.p_F, tmp_DlyLsa.LsaType, 
...
e_Err AsLsaDelayedTimerExp(t_Handle Id, t_Handle TimerId, word Flag)
{
   ...
   /* Possible that OSPF got disabled since timer was started. If 
    * so, just clear the delay list. */
   if (!p_RTO->Cfg.AdminStat)
   {
...
static e_Err HideAllSummaryOfRange(t_ARO *p_ARO, t_AgrEntry *p_AggrEntry)
{
  ...
  /* that belong to the address aggregation. */
   error = o2RouteGetFirst(p_RTB, p_RTB->RtbNtBt, &p_Rte);
   p_RTO->BundleLsas = TRUE;
...
next_dbEntry:
      ...
      error = o2RouteGetNext(p_RTB, p_RTB->RtbNtBt, &p_Rte);
...
static e_Err DeliverAllSummaryOfRange(t_ARO *p_ARO, t_AgrEntry *p_AggrEntry)
{
   ...
   /* Remove reject routes of active address ranges from the rto */
   p_RtEntry = o2RouteFind(p_RTB, p_RTB->RtbNtBt, p_AggrEntry->NetIpAdr, p_AggrEntry->SubnetMask);
   if (p_RtEntry)
   {
      if(p_RTO->Clbk.p_RoutingTableChanged && (p_RtEntry->isRejectRoute == TRUE))
      {
         ...
         o2RouteDelete(p_RTB, p_RTB->RtbNtBt, p_RtEntry->DestinationId, p_RtEntry->IpMask, TRUE);
      }
   }
   ...
   */
   ...
   o2AggregateCostReset(p_AggrEntry);
   ...
   while(link)
   {
      ...
      if((p_Link->Type == S_POINT_TO_POINT) || (p_Link->Type == S_VIRTUAL_LINK))
      {
         ...
         if ((AVLH_Find(p_ARO->RouterLsaHl, p_Link->LinkId, (void**) 
             ...
             L7_BIT_ISSET(p_NextDbEntry->dbFlags, O2_DB_ENTRY_IS_CUR) && 
...
/*********************************************************************
 * @purpose      Determine whether a given router LSA includes a link
 *               that matches the input parameters.        
 *
 * @param   p_DbEntry   @b{(input)}  router LSA database entry 
 * @param   linkType    @b{(input)}  router LSA link type. Host byte order.
 * @param   linkId      @b{(input)}  router LSA link ID. Host byte order.
 * @param   linkData    @b{(input)}  (optional) router LSA link data. Host byte order.                                   
 *
 * @returns  void
 *
 * @notes
 ...
 * @end
 * ********************************************************************/
Bool o2RtrLsaHasLink(t_A_DbEntry *p_DbEntry, L7_uint32 linkType, 
                     L7_uint32 linkId, L7_uint32 linkData)
{
  t_S_RouterLsa *p_RouterLsa;
  t_S_LinkDscr *p_Link;
  L7_uint32 numLinks;       /* number of links left to process in router LSA */

  if (p_DbEntry->Lsa.LsType != S_ROUTER_LSA)
    return FALSE;

  p_RouterLsa = (t_S_RouterLsa *) p_DbEntry->p_Lsa;
  numLinks = A_GET_2B(p_RouterLsa->LinkNum);
  p_Link = (t_S_LinkDscr *) ((byte*)p_RouterLsa + sizeof(t_S_RouterLsa));
  while (numLinks)
  {
    if ((linkType == p_Link->Type) &&
        (linkId == A_GET_4B(p_Link->LinkId)) &&
        (!linkData || (linkData == A_GET_4B(p_Link->LinkData))))
    {
      return TRUE;
    }
...
static Bool CheckNssaLsaOrg(t_A_DbEntry *p_DbEntry, t_ARO *p_ARO)
{
  ...
  */
  p_AsBr = o2RouteFind(p_RTB, p_RTB->RtbRtBt, A_GET_4B(p_DbEntry->Lsa.AdvertisingRouter), SPF_HOST_ROUTE);
  if (p_AsBr == NULL)
    return FALSE;
  ...
  */
  ...
  while(e == E_OK)
  {
    ...
    if(CheckLsaEquivalent(p_DbEntry, p_AsExtEntry) == TRUE)
    {
      ...
      */
      p_AsBr = o2RouteFind(p_RTB, p_RTB->RtbRtBt, A_GET_4B(p_AsExtEntry->Lsa.AdvertisingRouter), SPF_HOST_ROUTE);
      if (p_AsBr == NULL)
        goto nextAsExtLsa;
...
static void NssaTranslatorEnable(t_ARO *p_ARO)
{
  ...
  */
  p_RTO->BundleLsas = TRUE;
  o2RoutesBrowse(p_RTB, p_RTB->RtbNtBt, NssaTranslate, (ulng)NULLP);
  if (p_RTO->BundleLsas)
    LsUpdatesSend(p_RTO);
}
                         ...
                         Bool EntryIsAdded)
{
   ...
   else if ((p_DbEntry->Lsa.LsType == S_LINK_OPAQUE_LSA) && p_DbEntry->IFO_Id)
   {
     ...
     if (p_ARO)   /* guaranteed by type check above, but coverity prefers this */
     {
