...
typedef struct t_IFO
{
   ...
   /* These are router IDs, not interface addresses. */
   ...
   /* Linked list of delayed ACKs to be sent on this interface, and timer
    * that triggers the transmission of delayed ACKs. */
   t_Handle       LsAck;              
   t_Handle       AckTimer;           

   t_Handle       LinkOpaqueLsaHl;    /* set of opaque LSAs with link flooding scope */
   ...
   ulng           MinLsArrivalDrops;  /* number of LSAs received on this interface
                                       * dropped because of MinLSArrival violation. */
#ifdef L7_OSPF_TE
   ...
   /* L7_TRUE before grace LSAs are ack'd on by all neighbors on this interface
    * on a unplanned GR. When L7_TRUE, no hellos are sent or received on this
    * interface. */
   L7_BOOL        grSuppressHellos;

   /* L7_TRUE if a grace LSA has been sent on this interface for an unplanned
    * restart. */
   L7_BOOL        graceLsaSent;

   /* On an unplanned GR, we would like to ensure that all neighbors get our 
    * grace LSA before we send a 1-way hello on the interface. So we might 
    * suppress hellos on an interface until all checkpointed neighbors have
    * ACK'd the grace LSA. But some neighbors may never ack our grace LSA. 
    * If we continue to suppress hellos on
    * the interface until the grace period expires, then we won't send hellos
    * to other neighbors on the interface who are willing to be helpful. 
    * Those adjacencies will not come up until after the grace period expires. 
    * In order to establish adjacencies more quickly with those neighbors 
    * who are willing to be helpful, only suppress hellos for 3 LSA retx
    * intervals. If a neighbor has not ack'd our grace LSA within 3 LSA retx
    * intervals, delete the checkpoint neighbor and start sending hellos
    * on the interface. */
   t_Handle       grSuppressHelloTimer; 
...
typedef struct t_NBO
{
   ...
   /* Linked list of direct ACKs to be sent to this neighbor. These are kept 
    * in a separate list from delayed ACKs queued on an interface. There is no 
    * timer for direct ACKs. An event is queued to OSPF whenever a direct 
    * ACK is enqueued on this list. */
   t_Handle DirectLsAckList;    
   ...
   /* Only set if a grace LSA received from neighbor. Then set to the value 
    * specified in the grace period TLV. Seconds. */
   L7_uint32 gracePeriod;

   t_Handle gracePeriodTimer;   /* Timer for end of grace period for this neighbor */

   /* If O2_GR_REASON_NOT_RESTARTING, neighbor is not currently restarting. Otherwise, 
    * neighbor is in the process of restarting and restartReason is the restart reason 
    * code in the last grace LSA received from the neighbor. */
   e_o2RestartReason restartReason;

   /* Reason this router last exited helpful neighbor mode for this neighbor. */
   OSPF_HELPER_EXIT_REASON_t helperExitReason;

   /* Set this flag if we received the flushed Grace LSA from restarting neighbor
    * before we reached FULL adjacency with that neighbor */
   Bool helperExitPending;

   /* When this router is restarting, set to TRUE if neighbor lists our 
    * pre-restart router LSA during db exchange. */
   Bool hasMyPrerestartRtrLsa;

   /* TRUE if this NBO was created from a checkpoint neighbor in order to send
    * and retx grace LSAs on an unplanned restart. Such neighbors only exist during
    * an unplanned graceful restart and are deleted before we exit GR. */
   Bool ckptNeighbor;
...
/* Used to index dbFlags in t_A_DbEntry */
typedef enum
{
  O2_DB_ENTRY_IS_CUR = 0x1,
  O2_DB_ENTRY_SEQNO_OVER = 0x2,
  O2_DB_ENTRY_FLUSHING_HELD = 0x4,

  /* last update of this LSA was a periodic update with no content change */
  O2_DB_ENTRY_PERIODIC_UPDATE = 0x8,   

  /* A self-originated LSA we received from a neighbor during graceful restart. */
  O2_DB_ENTRY_PRE_RESTART = 0x10
...
typedef struct t_A_DbEntry
{
   ...
   word     dbFlags;   /* Indexed by e_o2DbEntryFlags */
...
/* For a given LSA type, the combination of LSID and advertising router 
 * uniquely identify an LSA within its flooding scope. */
...
typedef struct t_RetxEntry
{
   ...
   byte   pad[3];    /* force alignment of LsId and OrigId and keep all fields in key together */
   byte	  LsType;
   byte   LsId[4];
   byte   OrigId[4];
   ulng   SentTime;
...
typedef struct t_ARO
{
   ...
   t_Handle  GroupMembLsaHl;  /* group-membership-LSAs. MOSPF not supported. */
   t_Handle  NssaLsaHl;        /* NSSA LSAs DB list */
   t_Handle  Tmp2;             /* not implemented LSAs */
   t_Handle  Tmp3;             /* unused. link opaque LSAs maintained on interface */
...
/* Routing table debug stats */
typedef struct o2RouteDebugStats_s
{
  L7_uint32 o2RdxBadClear;
  L7_uint32 o2RdxBadAddr;
  L7_uint32 o2RdxMallocFail;
  L7_uint32 o2RdxTreeFull;
  L7_uint32 o2RdxAddFail;
  L7_uint32 o2RdxBadDel;
  L7_uint32 o2RdxBadChange;
} o2RouteDebugStats_t;
...
typedef struct t_RTB
{
   ...
   /* Radix trees of routes to routers (Rt) and routes to networks (Nt).
    ...
    * Radix trees support LPM lookups.
    */
   t_Handle       RtbRtBt;         
   t_Handle       RtbNtBt;         
   t_Handle       RtbRtBtOld;
   ...
   /* Heap of memory for data structures internal to the radix tree, 
    * for new and old networks trees. */
   L7_uchar8  *o2NtRouteTreeHeap;
   L7_uchar8  *o2OldNtRouteTreeHeap;

   /* Same, for routers trees */
   L7_uchar8  *o2RtRouteTreeHeap;
   L7_uchar8  *o2OldRtRouteTreeHeap;

   o2RouteDebugStats_t rtDbgStats;
   ...
   /* TRUE after OSPF has done its first RTO update */
   Bool            FirstRtoUpdateDone;
...
typedef struct t_AgrEntry
{
   ...
   /* A value of O2_AGGR_COST_UNKNOWN indicates that the cost has not been set */
   long  LargestCost;
...
void RteRelease(t_RoutingTableEntry *rte, Bool freeRoute);
...
typedef enum e_OSPF_TRACE_FLAGS
{
   ...
   OSPF_TRACE_GR = 0x20000,
   OSPF_TRACE_CKPT = 0x40000,
   OSPF_TRACE_RETX = 0x80000,
   OSPF_TRACE_ROUTE = 0x100000,
   OSPF_TRACE_DIRACK = 0x200000
} e_OSPF_TRACE_FLAGS;
...
/* Don't change upper fields without t_GeneralObject (ipms.h) changing) */
   ...
   /* Time when OSPF last went through phase 3 init. Used as the base time
    * when determining if OSPF should be in stub router mode when it starts. 
    * Seconds since boot. */
   ...
   /* LsdbOverload is TRUE when the router is in stub router mode as defined
    * by RFC 3137.  IsOverflowed above considers only external LSAs. */
   Bool      LsdbOverload;

   /* Reason router is in stub router mode. */
   OSPF_STUB_ROUTER_REASON_t LsdbOverloadReason;

   /* Stub router mode exit timer. Only active when LsdbOverloadReason 
    * is OSPF_STUB_RTR_STARTUP. */
   t_Handle  LsdbOverloadTimer;  
   ...
   /* IFO objects indexed by Layer 2 index hash list. Includes
    * virtual interfaces. */
   t_Handle  IfoIndexHl;    
   ...
   /* Number of LS Update packets received and transmitted */
   L7_uint32 RxLsUpdate;
   L7_uint32 TxLsUpdate;

   /* Maximum number of LS Update packets sent and received in any 5-second interval */
   L7_uint32 RxLsUpdateMax;
   L7_uint32 TxLsUpdateMax;

   /* Number of LS update packets received and transmitted in each 5 second 
    * interval in the last 30 minutes. */
   L7_uint32 statsIndex;    /* next index to write */
   L7_uint32 lsuRxStats[O2_STATS_SIZE];
   L7_uint32 lsuTxStats[O2_STATS_SIZE];

   /* Whether this router is currently doing a graceful restart */
   OSPF_GR_RESTART_STATUS_t restartStatus;

   /* Reason last graceful restart ended */
   OSPF_GR_EXIT_REASON_t grExitReason;

   /* Timer that expires when the graceful restart grace interval expires */
   t_Handle gracePeriodTimer;

   /* Statistics related to graceful restart */
   ospf_gr_stats_t grStats;
...
t_Handle GET_OWNER_HL(t_RTO *p_RTO, t_ARO *p_ARO, t_IFO *p_IFO, e_S_LScodes type);
...
t_A_DbEntry *LsaInstall(t_RTO *p_RTO, t_ARO *p_ARO, t_IFO *p_IFO, void *p_Lsa, t_A_DbEntry *p_former);
...
e_Err o2GrRouterLinkProcess(t_RTO *p_RTO, L7_uint32 routerId, L7_uint32 linkType, 
                            L7_uint32 linkId, L7_uint32 linkData);
