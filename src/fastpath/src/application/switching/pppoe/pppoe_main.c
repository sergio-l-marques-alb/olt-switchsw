...
L7_RC_t pppoePduReceive(L7_netBufHandle bufHandle, sysnet_pdu_info_t *pduInfo)
{
  ...
  if (ptin_debug_pppoe_snooping)
    LOG_PT_DEBUG(LOG_CTX_PPPOE, "Packet intercepted: intIfNum=%u, vlanId=%u, innerVlanId=%u",
...
//}
  ...
  {
     if (ptin_debug_pppoe_snooping)
       LOG_PT_ERR(LOG_CTX_PPPOE,"Root vlan not found! (intIfNum=%u, vlanId=%u)", pduInfo->intIfNum, pduInfo->vlanId);
  }
  else
  {
  ...
  {
     if (ptin_debug_pppoe_snooping)
       LOG_PT_NOTICE(LOG_CTX_PPPOE,"No PPPoE instance found for intVlanId %u. Ignored", vlanId);
     return L7_FAILURE;
  }
...
//  if (!_pppoeVlanIntfTrustGet(pduInfo->vlanId,pduInfo->intIfNum))
  {
    ...
    {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE,"Client not referenced! (intIfNum=%u, innerVlanId=%u, intVlanId=%u)",
    ...
    #endif
    {
      ...
      {
        if (ptin_debug_pppoe_snooping)
          LOG_PT_WARN(LOG_CTX_PPPOE,"Client not found! (intIfNum=%u, ptin_intf=%u/%u, innerVlanId=%u, intVlanId=%u)",
...
//  if (ptin_debug_pppoe_snooping)
//    LOG_PT_TRACE(LOG_CTX_PPPOE,"Incremented DHCP_STAT_FIELD_RX_FILTERED");
                         ...
                         L7_ushort16 innerVlanId, L7_uint *client_idx)
{
  ...
  {
    if (ptin_debug_pppoe_snooping)
      LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE malformed packet: invalid frame length (pktLen=%u ethHdrLen=%u pppoeHdrLen=%u pppoeDataLen=%u",
              dataLen, ethHdrLen, sizeof(L7_pppoe_header_t), osapiNtohs(pppoeHeader->length));
    return L7_FAILURE;
  }

  if (ptin_debug_pppoe_snooping)
    LOG_PT_DEBUG(LOG_CTX_PPPOE, "Packet PPPoE received at intIfNum=%u, oVlan=%u, iVlan=%u, MAC=%02x:%02x:%02x:%02x:%02x:%02x",
...
{
   if (ptin_debug_pppoe_snooping)
     LOG_PT_DEBUG(LOG_CTX_PPPOE, "PPPoE: Received new message");
...
//{
//  if (ptin_debug_pppoe_snooping)
//    LOG_PT_ERR(LOG_CTX_PPPOE,"Packet dropped here: server filter");
//  return L7_TRUE;
//}
//
...
//{
//  if (ptin_debug_pppoe_snooping)
//    LOG_PT_ERR(LOG_CTX_PPPOE,"Packet dropped here: client filter");
//   return L7_TRUE;
//}
//
...
//{
//  if (ptin_debug_pppoe_snooping)
//    LOG_PT_ERR(LOG_CTX_PPPOE,"Packet dropped here: verify MAC");
//  return L7_TRUE;
//}
...
L7_RC_t pppoeServerInterfaceGet(L7_uint32 *intIfNum, L7_ushort16 vlanId)
{
  ...
  LOG_PT_ERR(LOG_CTX_PPPOE, "Unable to find a valid ROOT interface for vlan %u", vlanId);
  return L7_FAILURE;
}
...
L7_RC_t pppoeClientFrameSend(L7_uint32 intIfNum, L7_uchar8* frame, L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint client_idx)
{
  ...
  if (dtlIpBufSend(intIfNum, vlanId, bufHandle) != L7_SUCCESS)
  {
    ...
    if (ptin_debug_pppoe_snooping)
      LOG_PT_ERR(LOG_CTX_PPPOE, " Failed to send PPPoE pkt on intf (%s) and vlanID (%d)", ifName, vlanId);
    return L7_FAILURE;
  }

  if (ptin_debug_pppoe_snooping)
    LOG_PT_TRACE(LOG_CTX_PPPOE,"Packet transmited to intIfNum=%u, with oVlan=%u (intVlan=%u), iVlan=%u", intIfNum, extOVlan, vlanId, extIVlan);
...
L7_RC_t pppoeServerFrameSend(L7_uchar8* frame, L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint client_idx)
{
  ...
  {
     if (ptin_debug_pppoe_snooping)
       LOG_PT_ERR(LOG_CTX_PPPOE, "Unable to determine server interface for vlanId %u", vlanId);
     return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_pppoe_snooping)
      LOG_PT_ERR(LOG_CTX_PPPOE, "Invalid outgoing interface intIfNum %u with type %u", intIfNum, sysIntfType);
    return L7_SUCCESS;
  }
  ...
  {
    if (ptin_debug_pppoe_snooping)
      LOG_PT_ERR(LOG_CTX_PPPOE, "Unable to get new bufHanddle");
    return L7_FAILURE;
  }
  ...
  if (ptin_pppoe_extVlans_get(intIfNum, vlanId, innerVlanId, client_idx, &extOVlan, &extIVlan) == L7_SUCCESS)
  {
    ...
    {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE,"Error checking if vlan %u belongs to a stacked EVC",vlanId);
...
void pppoeProcessClientFrame(L7_uchar8* frame, L7_uint32 intIfNum, L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint client_idx)
{
   ...
   if (ptin_debug_pppoe_snooping)
     LOG_PT_DEBUG(LOG_CTX_PPPOE, "PPPoE: Received new client message");
   ...
   if (ptin_debug_pppoe_snooping)
     LOG_PT_INFO(LOG_CTX_PPPOE, "PPPoE: verType=%02x code=%02x sessionId=%04x length=%04x",
   ...
   {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: Received invalid message on an untrusted port. Dropped.");
      return;
   }
   ...
   {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: Unable to register client (macAddr: %02X:%02X:%02X:%02X:%02X:%02X, rootVlan:%u).",
   ...
   else if (result == L7_NULLPTR )
   {
      ...
      {
         if (ptin_debug_pppoe_snooping)
           LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: Unable to find inserted client (macAddr: %02X:%02X:%02X:%02X:%02X:%02X, rootVlan:%u).",
                    binding_table_key.macAddr[0],binding_table_key.macAddr[1],binding_table_key.macAddr[2],binding_table_key.macAddr[3],binding_table_key.macAddr[4],binding_table_key.macAddr[5],
                    binding_table_key.rootVlan);
         return;
      }
   ...
   while(frame_len > 0)
   {
      ...
      switch(osapiNtohs(tlv_header->type))
      {
         case L7_TLV_TAGTYPE_VENDOR_ID:
         {
            ...
            if (ptin_debug_pppoe_snooping)
              LOG_PT_DEBUG(LOG_CTX_PPPOE, "Found a Vendor ID Tag Type. Replacing.");
            ...
            {
               if (ptin_debug_pppoe_snooping)
                 LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: New frame exceeds MTU. Dropping and returning a PADO/PADS with code 0x0203.");
   ...
   if(L7_TRUE != tlv_vendor_id_found)
   {
      ...
      if (ptin_debug_pppoe_snooping)
        LOG_PT_DEBUG(LOG_CTX_PPPOE, "Adding vendor ID TLV.");
      pppoeAddVendorIdTlv(tlv_header_copy_ptr, intIfNum, vlanId, innerVlanId);
...
void pppoeProcessServerFrame(L7_uchar8* frame, L7_uint32 intIfNum, L7_ushort16 vlanId, L7_ushort16 innerVlanId, L7_uint client_idx)
{
   ...
   if (ptin_debug_pppoe_snooping)
     LOG_PT_DEBUG(LOG_CTX_PPPOE, "PPPoE: Received new server message");
   ...
   {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: Received invalid message on a trusted port. Dropped.");
      return;
   }
   ...
   {
      if (ptin_debug_pppoe_snooping)
        LOG_PT_ERR(LOG_CTX_PPPOE, "PPPoE: Message received for an unknown client (macAddr: %02X:%02X:%02X:%02X:%02X:%02X, rootVlan:%u).",
                   binding_table_key.macAddr[0],binding_table_key.macAddr[1],binding_table_key.macAddr[2],binding_table_key.macAddr[3],binding_table_key.macAddr[4],binding_table_key.macAddr[5],
                   binding_table_key.rootVlan);
      return;
   }
   ...
   while(frame_len > 0)
   {
      ...
      switch(osapiNtohs(tlv_header->type))
      {
         case L7_TLV_TAGTYPE_VENDOR_ID:
         {
            ...
            if (ptin_debug_pppoe_snooping)
              LOG_PT_DEBUG(LOG_CTX_PPPOE, "Found a Vendor ID Tag Type. Removing it.");
