...
PORTEVENT_MASK_t fdbPortEventMask_g;
...
* @end
*********************************************************************/
L7_BOOL fdbIntfIsAttached(L7_uint32 intIfNum)
{
...
/*********************************************************************
* @purpose  Checks if the intf is up.
*
* @param    intIfNum     @b{(input)}    internal interface number
*
* @returns  L7_TRUE
...
* @end
*********************************************************************/
L7_BOOL fdbIntfIsUp(L7_uint32 intIfNum)
{
  L7_uint32 linkState;
  ...
  return (linkState == L7_UP) ? L7_TRUE : L7_FALSE;
}
                           ...
                           L7_BOOL     insert)
{
  ...
  if( (entryType == L7_FDB_ADDR_FLAG_LEARNED) || (entryType == L7_FDB_ADDR_FLAG_DOT1X_STATIC))
  {
    if (insert == L7_TRUE)
    {
  ...
  else if (entryType == L7_FDB_ADDR_FLAG_STATIC)
  {
    ...
    } else
    {
      ...
      fdb_stats.vlanStats[vlanId].static_entries--;
           ...
           entryType == L7_FDB_ADDR_FLAG_L3_MANAGEMENT ||
...
L7_RC_t fdbDelEntry(fdbMeberInfo_t *fdbMemberInfo)
{
  ...
  LOG_PT_TRACE(LOG_CTX_L2, "Going to remove MacAddr=%02x:%02x:%02x:%02x:%02x:%02x, VLAN=%u (intIfNum=%u): entryType=%u",
  ...
  {
    LOG_PT_TRACE(LOG_CTX_L2, "Sending message to remove SW table...");
    if (osapiMessageSend(fdbQueue, &fdbMsg, sizeof(fdbLearnMsg_t), L7_NO_WAIT, L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
      rc = L7_FAILURE;
  }
  ...
  if (rc == L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_L2, "Removing MAC at HW...");
  ...
  LOG_PT_TRACE(LOG_CTX_L2, "result = %d", rc);
...
void fdbTask( void )
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "fdb Task: Failed to create, exiting....\n");
  }
  else
  {
    while (1)
    ...
    for (i = 1; i<=L7_VLAN_MAX_MASK_BIT; i++)
    {
      ...
      switch (event)
      {
          ...
          case VLAN_DELETE_NOTIFY:
            if (fdbPhaseStatusCheck() == L7_FALSE) 
            {
              /* fdb databse is cleared by now, no need to
...
L7_RC_t fdbPhaseOneInit(void)
{
  ...
  do
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
              "FDB: Unable to allocate resources\n");
      break; /* goto while */
    }
    else
    {
...
L7_RC_t fdbPhaseTwoInit(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB: Unable to register to DTL for address changes\n");
    rc = L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB: Unable to register for VLAN change callback\n");
    rc = L7_FAILURE;
  }
  else if ((rc = nimRegisterIntfChange(L7_FDB_COMPONENT_ID,
                                       fdbIntfChangeCallback,
                                       fdbStartupNotifyCallback,
                                       NIM_STARTUP_PRIO_FDB)) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB: Unable to register to NIM for port create\n");
    rc = L7_FAILURE;
  }
  else
  {
...
L7_RC_t fdbPhaseThreeInit()
{
  ...
  memset(&fdbPortEventMask_g, 0, sizeof(fdbPortEventMask_g));
  nimRegisterIntfEvents(L7_FDB_COMPONENT_ID, fdbPortEventMask_g);
...
/*********************************************************************
* @purpose  Do the FDB processing for an interface that is attached.
*
...
* @end
*********************************************************************/
L7_RC_t fdbIntfAttachProcess(L7_uint32 intIfNum)
{
  L7_RC_t           rc = L7_SUCCESS;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB: failed to get interface type for intf %s on L7_ATTACH\n", ifName);
  }
...
/*********************************************************************
* @purpose  Do the FDB processing for an interface that is 'up'.
*
* @param    intIfnum    @b{(input)} internal interface
*
* @returns  L7_RC_t
...
* @end
*********************************************************************/
L7_RC_t fdbIntfUpProcess(L7_uint32 intIfNum)
{
  L7_RC_t           rc = L7_SUCCESS;
  L7_INTF_TYPES_t   intfType;
  L7_char8          mac[L7_MAC_ADDR_LEN];
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB:interface %s has no mac address\n", ifName);
  }
  else if (fdbGetTypeOfVL(&type) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
            "FDB: Could not get the VL Type\n");
  }
  else
  {
    if (type == L7_SVL)
...
L7_RC_t fdbIntfChangeCallback(L7_uint32 intIfNum, L7_uint32 event,NIM_CORRELATOR_t correlator)
{
  ...
  {
  case L7_ATTACH:
    rc = fdbIntfAttachProcess(intIfNum);
    break;
  ...
  case L7_DETACH:
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
              "Mirror: failed to get interface type for intf %s on L7_ATTACH\n", ifName);
    }
    else if (intfType == L7_CPU_INTF)
    {
      /* we have the cpu interface, add the mac address */
    ...
    else
    {
      if (type == L7_SVL)
      {
        if ((nimGetIntfType(intIfNum,&intfType) == L7_SUCCESS) && (intfType != L7_CPU_INTF))
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_FDB_COMPONENT_ID,
                    "FDB: Failed to delete fdb entry\n");
          }
        }
      }
    }
...
/*********************************************************************
* @purpose  Propogate Startup notifications to FDB component
*
...
void fdbStartupNotifyCallback(NIM_STARTUP_PHASE_t startupPhase)
{
  ...
  case NIM_INTERFACE_CREATE_STARTUP:
    /* We're not interested in port CREATE/DELETE events */
    break;
  ...
  case NIM_INTERFACE_ACTIVATE_STARTUP:
    ...
    while (rc == L7_SUCCESS)
    {
      ...
      if (fdbIntfIsAttached(intIfNum))
      {
        (void)fdbIntfAttachProcess(intIfNum);

        /* If the interface is up, do the 'up' processing. */
        if (fdbIntfIsUp(intIfNum))
        {
          (void)fdbIntfUpProcess(intIfNum);
        }
      }
    ...
    if (fdbWarmStart)
    {
      /* If this is a warm start, then we need to indicate to the driver that it
         should begin the FDB sync w/ the application. */
      (void)dtlFdbSync();
    }

    /* Add ATTACH/DETACH events to our NIM registration */
    PORTEVENT_SETMASKBIT(fdbPortEventMask_g, L7_ATTACH);
    PORTEVENT_SETMASKBIT(fdbPortEventMask_g, L7_DETACH);
    PORTEVENT_SETMASKBIT(fdbPortEventMask_g, L7_UP);
    PORTEVENT_SETMASKBIT(fdbPortEventMask_g, L7_DOWN);
    nimRegisterIntfEvents(L7_FDB_COMPONENT_ID, fdbPortEventMask_g);
    break;
  ...
  nimStartupEventDone(L7_FDB_COMPONENT_ID);

  /* Inform cnfgr that fdb has completed it HW updates */
  if (startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    cnfgrApiComponentHwUpdateDone(L7_FDB_COMPONENT_ID,
