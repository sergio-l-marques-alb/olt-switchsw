...
L7_RC_t dot1qVlanIntfCreate(L7_uint32 vlanId, L7_uint32 intfId, L7_uint32 *intIfNum)
{
  ...
  if ( nimIntIfFromConfigIDGet(&configId, &nimCfgIntIfNum) == L7_SUCCESS)
  {
          ...
          (dot1qVlanIntf[index].vlanId != vlanId) )
     {
         L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                 "vlan id %d cannot use index %d - already in use for vlan %d\n",
  ...
  if (dot1qVlanIntfCreateInNim(vlanId, &tmpIntIfNum) == L7_SUCCESS)
  {
      ...
      *intIfNum = tmpIntIfNum;
      ...
      {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                  "DOT1Q: dtl failed to create an interface for vlan %d\n", vlanId);
          return L7_FAILURE;
      }
  ...
  if (nimGetIntfLinkState(intIfNum,&linkState) == L7_SUCCESS)
  {
    if (linkState != L7_DOWN)
    {
        if (nimEventIntfNotify(eventInfo,&handle) != L7_SUCCESS)
        {
            ...
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                    "link down notification failed for vlan %d, intf %s\n", vlanId, ifName);
        }
...
L7_RC_t dot1qVlanIntfDeleteInNim(L7_uint32 intIfNum)
{
  ...
  else
  {
      ...
      {
         L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                 "DOT1Q: dtl failed to delete an interface for intf %s\n", ifName);
      }
...
L7_RC_t dot1qVlanIntfCreateInNim(L7_uint32 vlanId, L7_uint32 *pIntIfNum)
{
  ...
  do
  {
    ...
    if ((rc = cmgrIfaceMacGet(intfIdInfo.type, L7_LOGICAL_UNIT, platSlotVlanSlotNumGet(),
                              vlanIfIndex, intfDescr.macAddr.addr,
                              intfDescr.l3MacAddr.addr)) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "DOT1Q: failed to get the MAC address of vlan %d\n",vlanId);
      break; /* goto while (0) */
    }
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
              "DOT1Q: failed to create the interface for vlanId %d\n", vlanId);
      break; /* goto while (0) */
    }
...
void dot1qNimEventCompletionCallback(NIM_NOTIFY_CB_INFO_t retVal)
{
  ...
  if (retVal.response.rc != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
            "DOT1Q: notification failed for event(%d), intf(%s), reason(%d)\n",
    ...
    */
    ...
    while (1)
    {
        ...
        if (dot1qVlanIntfIntIfNumIsValid(intIfNum, &index) != L7_SUCCESS)
        {
            ...
            {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                        "dot1qNimEventCompletionCallback: intf %s not valid\n", ifName);
                return;
            }
    ...
    case L7_CREATE:
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                    "dot1q vlan intIf state is not in CREATE_PENDING for intf %s/vlanid %d\n",
                    ifName, vlanId);
            break;
        }
        ...
        if (dot1qVlanCheckValid(dot1qVlanIntf[index].vlanId) == L7_SUCCESS)
        {
            ...
            {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                        "dot1qNimEventCompletionCallback: intf = %s, nimEventIntfNotify failed\n",
                        ifName);
            }
        }
        else
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                    "dot1qNimEventCompletionCallback: intf = %s, vlanId %d check invalid\n",
                   ifName, vlanId);
        }
    ...
    case L7_ATTACH:
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                    "dot1q vlan intIf state is not VALID for intf %s/ vlanId %d\n",
                    ifName, vlanId);
            break;
        }
        ...
        else
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                    "dot1q vlan intIf link state cannot be determined "
                    "for intf %s/ vlanId %d\n",
                    ifName, vlanId);
            break;
          }
    ...
    case L7_DETACH:
        ...
        if (dot1qVlanIntf[index].intfState == DOT1Q_VLAN_INTF_DELETE_PENDING)
        {
            ...
            {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1Q_COMPONENT_ID,
                        "DOT1Q: dtl failed to delete an interface for intf %s\vlan %d\n",
                         ifName, dot1qVlanIntf[index].vlanId);
            }
