...
L7_INTF_MASK_t portCommonChangeMask;
L7_INTF_MASK_t *portInstInfoChangeMask;

dot1s_HelperFuncTable_t dot1sHelperFuncTable;
dot1s_nsfFuncTable_t dot1sNsfFuncTable;
dot1s_startup_times_t dot1sStartupTime;
...
L7_BOOL *syncInProgress = L7_NULLPTR;
static L7_BOOL warmRestart;
L7_BOOL dot1sSwitchoverInProgress ;
dot1s_nsf_recon_states_t dot1sAppState = DOT1S_RECON_NA;
...
{
  return (dot1sAppState == DOT1S_READY);
}
/*********************************************************************
*
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  dot1sCB          = (dot1sStateCirBuf_t *)osapiMalloc(L7_DOT1S_COMPONENT_ID,
      ...
      portInstInfoChangeMask = (L7_INTF_MASK_t *)osapiMalloc(L7_DOT1S_COMPONENT_ID,
                                           (L7_uint32)sizeof(L7_INTF_MASK_t)
                                           * (L7_MAX_MULTIPLE_STP_INSTANCES+1));
    syncInProgress   = (L7_BOOL *)osapiMalloc(L7_DOT1S_COMPONENT_ID,
                                            (L7_uint32)sizeof(L7_BOOL)*(L7_MAX_MULTIPLE_STP_INSTANCES+1));
  ...
  if ((dot1sBridge      == L7_NULLPTR) ||
      ...
      (dot1sPortStats   == L7_NULLPTR) ||
          ...
          (dot1sCB          == L7_NULLPTR) ||
          (portInstInfoChangeMask == L7_NULLPTR)  ||
      (syncInProgress   == L7_NULLPTR)
         )
      {
        rc   = L7_NO_MEMORY;
        break;
  ...
  if (bufferPoolInit(DOT1S_MAX_BPDU_BUF_COUNT, sizeof(DOT1S_MSTP_ENCAPS_t), "dot1s BPDU Bufs",
                     &dot1sBPDUBufferPoolId) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "%s: Error allocating large buffers\n",__FUNCTION__);
        rc   = L7_NO_MEMORY;
        break;
  }
  ...
  memset((void *)dot1sNotifyList, 0x00, ((L7_uint32)sizeof(dot1sNotifyList_t) * L7_LAST_COMPONENT_ID));
      memset((void *)portInstInfoChangeMask, 0, sizeof(L7_INTF_MASK_t) * (L7_MAX_MULTIPLE_STP_INSTANCES+1));
      memset((void *)&dot1sHelperFuncTable , 0, sizeof(dot1sHelperFuncTable));
      memset((void *)&dot1sNsfFuncTable , 0, sizeof(dot1sNsfFuncTable));
      memset((void *)&dot1sStartupTime , 0, sizeof(dot1sStartupTime));
  memset((void *)syncInProgress, 0x00, ((L7_uint32)sizeof(L7_BOOL) * (L7_MAX_MULTIPLE_STP_INSTANCES+1)));
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "%s: Error creating dot1sStateSetSema\n",__FUNCTION__);
        rc   = L7_NO_MEMORY;
        break;
      }

#ifdef L7_STACKING_PACKAGE
      if (dot1sCnfgrIsHelperPresent() == L7_TRUE)
      {
        if ((rc = dot1sTransportInit()) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT1S_COMPONENT_ID,
                  "Cannot init dot1s transport \n");
          rc   = L7_NO_MEMORY;
          break;
        }
        if ((rc = dot1sHelperInit()) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT1S_COMPONENT_ID,
                  "Cannot init dot1s Helper \n");
          rc   = L7_NO_MEMORY;
          break;
        }
      }
      ...
      {
        if ((rc = dot1sNsfInit()) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT1S_COMPONENT_ID,
                "Cannot init dot1s NSF \n");
          rc   = L7_NO_MEMORY;
          break;
        }
      }
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  if (nimRegisterIntfChange(L7_DOT1S_COMPONENT_ID, dot1sIntfChangeCallBack,
                            dot1sIntfStartupCallBack,
                            NIM_STARTUP_PRIO_DOT1S) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "Unable to register for NIM Interface change callback!\n");
    *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
    dot1sRC     = L7_ERROR;
  ...
  if (vlanRegisterForChange(dot1sVlanChangeCallback,
                            L7_DOT1S_COMPONENT_ID, eventMask) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "Unable to register for dot1s state callback with DTL!\n");
        *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
    dot1sRC     = L7_ERROR;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "Unable to register for dot1s state callback with Sysnet!\n");
        *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
    dot1sRC     = L7_ERROR;
...
#ifdef L7_NSF_PACKAGE
  if (dot1sNsfCallbacksRegister() != L7_SUCCESS)
  {
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
    dot1sRC     = L7_ERROR;
  ...
  if (dot1sCnfgrIsHelperPresent() == L7_TRUE)
  {
    if (dot1sHelperCallbacksRegister() != L7_SUCCESS)
    {
      *pReason    = L7_CNFGR_ERR_RC_FATAL;
      dot1sRC     = L7_ERROR;
    ...
    if (dot1sTransportCallbacksRegister() != L7_SUCCESS)
    {
      *pReason    = L7_CNFGR_ERR_RC_FATAL;
      dot1sRC     = L7_ERROR;
...
void dot1sCnfgrFiniPhase1Process()
{
  ...
  if (syncInProgress != L7_NULLPTR)
    osapiFree(L7_DOT1S_COMPONENT_ID, (void *)syncInProgress);
                                 ...
                                 L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1S_COMPONENT_ID,
            "Unable to delete MSTP Instance %u in the hardware", i);
        *pResponse  = 0;
    *pReason    = L7_CNFGR_ERR_RC_FATAL;
        dot1sRC = L7_ERROR;
  }
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                if ((dot1sRC = dot1sCnfgrInitPhase3Process(warmRestart,
                                              &response, &reason )) != L7_SUCCESS)
                {
...
{
  L7_BOOL helperPresent = L7_FALSE;
#ifdef L7_STACKING_PACKAGE
  helperPresent = L7_TRUE;
#endif
  return helperPresent;
}
/*********************************************************************
*
* @purpose  To parse the configurator commands send to dot1sTask
...
void dot1sCnfgrInstApply()
{
  ...
  syncInProgress[DOT1S_CIST_INDEX] = L7_FALSE;
