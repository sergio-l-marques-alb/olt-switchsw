...
//*
//* @returns  L7_SUCCESS change is successful
...
{
  L7_RC_t test1 = L7_FAILURE;
  L7_RC_t test2 = L7_FAILURE;
  L7_RC_t test3 = L7_FAILURE;
...
L7_uint32 dot1sMstiPortStateGet(L7_uint32 mstID, L7_uint32 intIfNum)
{
  ...
  if (dot1sInstIndexFind(mstID, &index) == L7_SUCCESS)
  {
    ...
    if (pPort != L7_NULLPTR)
    {
      ...
      /* If reconciliation has not finished
         if intf not reconciled ; get ckpted data.
      */
      ...
      {
        dot1sNsfFuncTable.dot1sCkptStateGet(intIfNum,index, &state);
        return state;
      }
      else
    {
...
* @end
*********************************************************************/
L7_uint32 dot1sVlanToMstiConfiguredGet(L7_uint32 VID)
{
  return(dot1sCfg->cfg.dot1sInstVlanMap[VID].instNumber);
}
...
* @end
*********************************************************************/
L7_uint32 dot1sDefaultPortPriorityGet(L7_uint32 intIfNum)
{
  ...
  {
    return FD_DOT1S_LAG_PORT_PRIORITY;
  }
  return(FD_DOT1S_PORT_PRIORITY);
}
...
/*********************************************************************
* @purpose  Add an association between a VLAN(s) and an MSTP instance
*
* @param    mstID     @b{(input)} the MSTID for the desired MTSI
* @param    vlan_mask @b{(input)} the VLAN mask of the all the vlans
*                                 to be added to the instance
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
* @comments The VLAN(s) will no longer be associated with the CIST.
*
*
* @end
*********************************************************************/
L7_RC_t dot1sMstiVlanAdd(L7_uint32 mstID, L7_VLAN_MASK_t *pvlan_mask)
{
  L7_BOOL instInUse = L7_FALSE;
  L7_RC_t rc = L7_FAILURE;
  L7_uint32 vlanID;
  L7_BOOL is_modified = L7_TRUE;
  ...
  if (instInUse == L7_TRUE)
  {
    for (vlanID = L7_DOT1Q_MIN_VLAN_ID; vlanID <= L7_DOT1Q_MAX_VLAN_ID; vlanID++)
    {
      ...
      {
        dot1sCfg->cfg.dot1sInstVlanMap[vlanID].instNumber = (L7_ushort16)mstID;
        is_modified = L7_TRUE; 

        if (dot1qVlanCheckValid(vlanID) != L7_SUCCESS)
        {
          L7_VLAN_CLRMASKBIT(vlan_mask,vlanID);
        }
      }  
    }
    ...
    {
      rc = dot1sIssueCmd(instanceVlanAdd, L7_NULL, mstID, &vlan_mask);
...
/*********************************************************************
* @purpose  Remove an association between a VLAN(s) and an MSTP instance
*
...
* @end
*********************************************************************/
L7_RC_t dot1sMstiVlanRemove(L7_uint32 mstID, L7_VLAN_MASK_t *pvlan_mask)
{
  L7_RC_t rc = L7_FAILURE;
  L7_uint32 vlanID;
  ...
  if (dot1sInstCheckInUse(mstID) == L7_SUCCESS)
  {
    for (vlanID = L7_DOT1Q_MIN_VLAN_ID; vlanID <= L7_DOT1Q_MAX_VLAN_ID; vlanID++)
    {
      if (L7_VLAN_ISMASKBITSET(vlan_mask,vlanID))
      {
        if (dot1sCfg->cfg.dot1sInstVlanMap[vlanID].instNumber == mstID)
        {
    ...
    if (is_modified == L7_TRUE)
    {
      rc = dot1sIssueCmd(instanceVlanRemove, L7_NULL, mstID, &vlan_mask);
      rc = dot1sMstConfigIdCompute();
      dot1sCfg->hdr.dataChanged = L7_TRUE;
    }
  }
...
L7_RC_t dot1sInstanceGet(L7_uint32 mstID)
{
  ...
  if (((mstID < L7_DOT1S_MSTID_MIN) || (mstID > L7_DOT1S_MSTID_MAX)) && mstID != L7_DOT1S_CIST_INSTANCE)
  {
    return L7_FAILURE;
  }
...
L7_uint32 dot1sMstiFIDFirstGet(L7_uint32 mstID)
{
  ...
  for (id = 1; id <= L7_DOT1Q_MAX_VLAN_ID; id++)
  {
     ...
     */ 
    if (((L7_uint32)dot1sCfg->cfg.dot1sInstVlanMap[id].instNumber == mstID) &&
...
L7_uint32 dot1sPortStatsSTPBPDUsReceivedGet(L7_uint32 intIfNum)
{
  ...
  return(dot1sPortStats[portIndex].stpRx);
}
...
{
    L7_uint32 portIndex = dot1sPortIndexFromIntfNumGet(intIfNum);
  return(dot1sPortStats[portIndex].rstpRx);
}
...
{
    L7_uint32 portIndex = dot1sPortIndexFromIntfNumGet(intIfNum);
  return(dot1sPortStats[portIndex].mstpRx);
}
...
{
    L7_uint32 portIndex = dot1sPortIndexFromIntfNumGet(intIfNum);
  return(dot1sPortStats[portIndex].stpTx);
}
...
{
    L7_uint32 portIndex = dot1sPortIndexFromIntfNumGet(intIfNum);
  return(dot1sPortStats[portIndex].rstpTx);
}
...
{
    L7_uint32 portIndex = dot1sPortIndexFromIntfNumGet(intIfNum);
  return(dot1sPortStats[portIndex].mstpTx);
}
...
L7_uint32 dot1sPortStatsTransitionsIntoLoopInconsistentStateGet(L7_uint32 mstID, L7_uint32 intIfNum)
{
  ...
  if (dot1sInstIndexFind(mstID, &index) == L7_SUCCESS)
  {
    return(dot1sPortStats[portIndex].transitionsIntoLoopInconsistentState);
  }
  else
  {
    return(L7_NULL);
...
L7_uint32 dot1sPortStatsTransitionsOutOfLoopInconsistentStateGet(L7_uint32 mstID, L7_uint32 intIfNum)
{
  ...
  if (dot1sInstIndexFind(mstID, &index) == L7_SUCCESS)
  {
    return(dot1sPortStats[portIndex].transitionsOutOfLoopInconsistentState);
  }
  else
  {
    return(L7_NULL);
...
* @end
*********************************************************************/
L7_RC_t	dot1sInstCheckInUseConfirm(L7_uint32 mstID)
{
  L7_uint32 index;
  L7_RC_t rc;

  rc = dot1sInstIndexFind(mstID, &index);
  if (rc != L7_SUCCESS)
  {
    dot1sApiQueueSynchronize();
    rc = dot1sInstIndexFind(mstID, &index);
									...
									L7_BOOL *stateQuery)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DOT1S_COMPONENT_ID,
            "dot1sInstStateQueryCallback is deregistered\n");
	return L7_FAILURE;
  }
  if (!(DOT1S_IS_READY))
  {
...
L7_RC_t dot1sPortStatsClear(L7_uint32 intIfNum)
{
  ...
  dot1sPortStats[portIndex].stpRx = L7_NULL;
  dot1sPortStats[portIndex].rstpRx = L7_NULL;
  dot1sPortStats[portIndex].mstpRx = L7_NULL;
  dot1sPortStats[portIndex].stpTx = L7_NULL;
  dot1sPortStats[portIndex].rstpTx = L7_NULL;
  dot1sPortStats[portIndex].mstpTx = L7_NULL;
  dot1sPortStats[portIndex].rxDiscards = L7_NULL;
  dot1sPortStats[portIndex].txDiscards = L7_NULL;
...
L7_RC_t dot1sRestrictedRoleSet(L7_uint32 intIfNum, L7_BOOL val)
{
  ...
  if (pPortCfg != L7_NULLPTR)
  {
    if ( (val == L7_TRUE) && (pPortCfg->loopGuard == L7_TRUE) )
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DOT1S_COMPONENT_ID,
              "Root Guard cannot be enabled on interface %s with enabled Loop Guard\n", ifName);
      return L7_ERROR;
    }   
...
L7_RC_t dot1sLoopGuardSet(L7_uint32 intIfNum, L7_BOOL val)
{
  ...
  if (pPortCfg != L7_NULLPTR)
  {
    if ( (val == L7_TRUE) && (pPortCfg->restrictedRole == L7_TRUE) )
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DOT1S_COMPONENT_ID,
              "Loop Guard  cannot be enabled on interface %s with enabled Root Guard\n", ifName);
      return L7_ERROR;
    }   
...
* @end
*********************************************************************/
L7_RC_t dot1sIntfBpduFloodGet(L7_uint32 intIfNum, L7_uint32 *val)
{
  DOT1S_PORT_COMMON_CFG_t *pPortCfg = L7_NULLPTR;
