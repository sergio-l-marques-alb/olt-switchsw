...
L7_RC_t __matrix_slotid_get(L7_uint8 matrixType, L7_uint8 *slotId)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_PROTB, "Invalid context [slotId:%p]", slotId);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PROTB, "Invalid matrix type [%u]", matrixType);
    return L7_FAILURE;
  }
...
L7_RC_t __matrix_ipaddr_get(L7_uint8 matrixType, L7_uint32 *ipAddr)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_PROTB, "Invalid context [ipAddr:%p]", ipAddr);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PROTB, "Unable to get matrix slot id");
    return L7_FAILURE;
  }
...
L7_RC_t __remoteslot_mfdbport_sync(L7_uint8 slotId, L7_uint8 admin, L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_uint8 groupType)
{
  ...
  PT_LOG_TRACE(LOG_CTX_PROTB, "Sending message to card %08X(%u) to set port %u admin to %u for group %08X/%08X", protectionSlotIp, slotId, portId, admin, groupAddr, sourceAddr);
...
L7_RC_t __matrix_mfdbport_sync(L7_uint8 admin, L7_uint8 matrixType, L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_uint8 groupType)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_PROTB, "Unable to get matrix IP address");
    return L7_FAILURE;
  }
...
unsigned int snooping_portType_get(unsigned int serviceId, unsigned int portId, ptin_mgmd_port_type_t *portType)
{
  ...
  {
    #if 0    
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Port is Unknown");
    *portType = PTIN_MGMD_PORT_TYPE_LEAF;
    #else
    PT_LOG_ERR(LOG_CTX_IGMP,"Unknown port type");
    return FAILURE;
...
#if (!PTIN_BOARD_IS_MATRIX) //Conversion from IGMP service to the Multicast service is only performed in the linecards
{
  ...
  if (ptin_igmp_McastRootVlan_get(intVlan, portId, isLeafPort, clientId, &groupInetAddr, &sourceInetAddr, &mcastRootVlan)==L7_SUCCESS)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP,"This is not a leaf Port (portId=%u, mcastRootVlan=%u)", portId, mcastRootVlan);    
      return L7_FAILURE;
    }

    if (ptin_evc_get_evcIdfromIntVlan(mcastRootVlan, serviceId)!=L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "No EVC associated to mcastRootVlan %u", mcastRootVlan);
      return L7_FAILURE;
    }
...
unsigned int snooping_port_open(unsigned int serviceId, unsigned int portId, unsigned int groupAddr, unsigned int sourceAddr, unsigned char isStatic)
{
  ...
  PT_LOG_DEBUG(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X isStatic:%u]", serviceId, portId, groupAddr, sourceAddr, isStatic);
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to open a port on the switch");
  if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Unable to unlock snooping's queue semaphore");
      return L7_FAILURE;
    }
  }
  ...
  if(isStatic != L7_TRUE)
  {
    ...
    if(protTypebIntfConfig.status == L7_ENABLE)
    {
      ...
      /* Send a Port_Open event to the FP */
      PT_LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to open a protection port on the switch");
      if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
      {
        if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
        {
...
unsigned int snooping_port_close(unsigned int serviceId, unsigned int portId, unsigned int groupAddr, unsigned int sourceAddr)
{
  ...
  PT_LOG_DEBUG(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X]", serviceId, portId, groupAddr, sourceAddr);
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_NOTICE(LOG_CTX_IGMP, "Ignoring Port Close Request!");
    return rc;
  }
  #endif
  ...
  {
//  if (ptin_debug_igmp_snooping)
      PT_LOG_NOTICE(LOG_CTX_IGMP, "Ignoring Port Close. This port is inactive [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X]", serviceId, portId, groupAddr, sourceAddr);
    return rc;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to close a port");
  if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
  {
    if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
    {
  ...
  if(protTypebIntfConfig.status == L7_ENABLE)
  {
    ...
    /* Send a Port_Close event to the FP */
    PT_LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to close a protection port on the switch");
    if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
    {
      if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
      {
...
L7_RC_t ptin_snoop_sync_mx_process_request(L7_uint16 vlanId, L7_uint32 groupAddr)
{
  ...
  { 
    PT_LOG_NOTICE(LOG_CTX_PROTB, "Silently Ignoring Snoop Sync Request. I'm a protection Matrix!");
    return L7_SUCCESS;
  }
  else
  {
  ...
  if (numberOfSnoopEntries>0)
  {    
    PT_LOG_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Reply Message to ipAddr:%08X with %u snoop Entries  to sync the protection matrix",ipAddr, numberOfSnoopEntries);
    if(vlanId==0 && groupAddr==0)
    {
