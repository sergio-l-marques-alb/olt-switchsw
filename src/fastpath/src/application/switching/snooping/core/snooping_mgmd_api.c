...
L7_RC_t __matrix_slotid_get(L7_uint8 matrixType, L7_uint8 *slotId)
{
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Invalid context [slotId:%p]", slotId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Invalid matrix type [%u]", matrixType);
    return L7_FAILURE;
  }
...
L7_RC_t __matrix_ipaddr_get(L7_uint8 matrixType, L7_uint32 *ipAddr)
{
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Invalid context [ipAddr:%p]", ipAddr);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Unable to get matrix slot id");
    return L7_FAILURE;
  }
...
L7_RC_t __remoteslot_mfdbport_sync(L7_uint8 slotId, L7_uint8 admin, L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_uint8 groupType)
{
  ...
  LOG_TRACE(LOG_CTX_PROTB, "Sending message to card %08X(%u) to set port %u admin to %u for group %08X/%08X", protectionSlotIp, slotId, portId, admin, groupAddr, sourceAddr);
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Failed to sync MGMD between active and protection interface");
    return L7_FAILURE;
  }
...
L7_RC_t __matrix_mfdbport_sync(L7_uint8 admin, L7_uint8 matrixType, L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_uint8 groupType)
{
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Unable to get matrix IP address");
    return L7_FAILURE;
  }
  ...
  LOG_TRACE(LOG_CTX_PROTB, "Sending message to matrix %08X(%u) to set port %u admin to %u for group %08X/%08X", matrixIpAddr, matrixSlotId, portId, admin, groupAddr, sourceAddr);
...
{
  LOG_TRACE(LOG_CTX_IGMP, "Context [admin:%u]", admin);

  // Snooping global activation
  if (L7_SUCCESS != usmDbSnoopAdminModeSet(1, admin, L7_AF_INET))  {
    LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopAdminModeSet");
    return FAILURE;
  }
...
{
  LOG_TRACE(LOG_CTX_IGMP, "Context [cos:%u]", cos);

  // Attrib IGMP packets priority
  if (L7_SUCCESS != usmDbSnoopPrioModeSet(1, cos, L7_AF_INET))
  {
    LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopPrioModeSet");
    return FAILURE;
  }
...
unsigned int snooping_portList_get(unsigned int serviceId, ptin_mgmd_port_type_t portType, PTIN_MGMD_PORT_MASK_t *portList, unsigned int *noOfPorts)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Context [serviceId:%u portType:%u portList:%p noOfPorts:%p]", serviceId, portType, portList, noOfPorts);

  if ( portList == L7_NULLPTR || noOfPorts == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP, "Context [serviceId:%u portType:%u portList:%p noOfPorts:%p]", serviceId, portType, portList, noOfPorts);
    return FAILURE;
  }
  ...
  *noOfPorts = 0;
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Unable to get mcastRootVlan from serviceId");
  ...
  {   
    LOG_ERR(LOG_CTX_IGMP,"Unknown port type");
    memcpy(portList, &interfaceBitmap, sizeof(*portList));
  ...
  if(SUCCESS != res)
  {
    LOG_ERR(LOG_CTX_IGMP,"Unable to get port information");
    return FAILURE;
  }
...
unsigned int snooping_portType_get(unsigned int serviceId, unsigned int portId, ptin_mgmd_port_type_t *portType)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u portType:%u]", serviceId, portId, *portType);
...
{
  LOG_TRACE(LOG_CTX_IGMP, "Context [groupAddr:%08X sourceAddr:%08X serviceId:%p]", groupAddr, sourceAddr, serviceId);

  if(serviceId == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP, "Abnormal context [serviceId:%p]", serviceId);
    return FAILURE;
  }
...
#if (!PTIN_BOARD_IS_MATRIX) //Conversion from IGMP service to the Multicast service is only performed in the linecards
{
  ...
  if (ptin_igmp_McastRootVlan_get(intVlan, portId, isLeafPort, clientId, &groupInetAddr, &sourceInetAddr, &mcastRootVlan)==L7_SUCCESS)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"This is not a leaf Port (portId=%u, mcastRootVlan=%u)", portId, mcastRootVlan);    
      return L7_FAILURE;
    }

    if (ptin_evc_get_evcIdfromIntVlan(mcastRootVlan, serviceId)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP, "No EVC associated to mcastRootVlan %u", mcastRootVlan);
      return L7_FAILURE;
    }
    LOG_TRACE(LOG_CTX_IGMP,"Found serviceID %u associated to the pair {groupAddr,sourceAddr}={%08X,%08X}", *serviceId, groupAddr, sourceAddr);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_IGMP,"Unable to determine serviceID associated to the pair {groupAddr,sourceAddr}={%08X,%08X}", groupAddr, sourceAddr);
    return L7_FAILURE;
  }
}
...
{
  LOG_TRACE(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u clientList:%p noOfClients:%p]", serviceId, portId, clientList, noOfClients);
  ...
  {
    ptin_timer_stop(72);
    LOG_ERR(LOG_CTX_IGMP,"Failed to obtain client bitmap [serviceId:%u portId:%u clientList:%p]", serviceId, portId, clientList);
    return SUCCESS;
  }
  ptin_timer_stop(72);
#else
...
unsigned int snooping_port_resources_available(unsigned int serviceId, unsigned int portId, unsigned int groupAddr, unsigned int sourceAddr)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u groupAddr:0x%08x sourceAddr:0x%08x]", serviceId, portId, groupAddr, sourceAddr);
 
  if (L7_FALSE == ptin_igmp_proxy_admission_control_get())
  {
    LOG_NOTICE(LOG_CTX_IGMP, "Admission Control Feature is Disabled!");
    return TRUE;
  }

  if (portId == 0)
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Ignoring Request [serviceId:%u portId:%u groupAddr:0x%08x sourceAddr:0x%08x]", serviceId, portId, groupAddr, sourceAddr);
    return L7_TRUE;
  }

  if (L7_SUCCESS != ptin_intf_intIfNum2port(portId, &ptin_port))
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid Port Id [serviceId:%u portId:%u groupAddr:0x%08x sourceAddr:0x%08x]", serviceId, portId, groupAddr, sourceAddr);
    return L7_FALSE;
  }
  ...
  if (L7_TRUE == ptin_igmp_proxy_bandwidth_control_get())
  {
    ...
    {
      ptin_timer_stop(60);
      LOG_ERR(LOG_CTX_IGMP, "Failed to obtain channel bandwidth [serviceId:%u portId:%u groupAddr:0x%08x sourceAddr:0x%08x]", serviceId, portId, groupAddr, sourceAddr);
      return L7_FALSE;
    }
    ptin_timer_stop(60);
    ...
    {   
      LOG_TRACE(LOG_CTX_IGMP,"Channel Bandwidth:%u kbps", channelBandwidth); 
    }
  }
  else
  {
...
{ 
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u clientId:%u groupAddr:0x%08x sourceAddr:0x%08x noOfClients:%u]", serviceId, portId, clientId, groupAddr, sourceAddr, noOfClients);
...
unsigned int snooping_port_open(unsigned int serviceId, unsigned int portId, unsigned int groupAddr, unsigned int sourceAddr, unsigned char isStatic)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X isStatic:%u]", serviceId, portId, groupAddr, sourceAddr, isStatic);
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to open a port on the switch");
  if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to unlock snooping's queue semaphore");
      return L7_FAILURE;
    }
  }
  ...
  if(isStatic != L7_TRUE)
  {
    ...
    if(protTypebIntfConfig.status == L7_ENABLE)
    {
      ...
      /* Send a Port_Open event to the FP */
      LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to open a protection port on the switch");
      if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
      {
        if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
        {
...
unsigned int snooping_port_close(unsigned int serviceId, unsigned int portId, unsigned int groupAddr, unsigned int sourceAddr)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Context [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X]", serviceId, portId, groupAddr, sourceAddr);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Ignoring Port Close Request!");
    return rc;
  }
  #endif
  ...
  {
//  if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Ignoring Port Close. This port is inactive [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X]", serviceId, portId, groupAddr, sourceAddr);
    return rc;
  }
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to close a port");
  if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
  {
    if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
    {
  ...
  if(protTypebIntfConfig.status == L7_ENABLE)
  {
    ...
    /* Send a Port_Close event to the FP */
    LOG_TRACE(LOG_CTX_IGMP, "Sending request to FP to close a protection port on the switch");
    if(L7_SUCCESS == (rc = osapiMessageSend(pSnoopCB->snoopExec->snoopIGMPQueue, &msg, SNOOP_PDU_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM)))
    {
      if (osapiSemaGive(pSnoopEB->snoopMsgQSema) != L7_SUCCESS)
      {
...
unsigned int snooping_tx_packet(unsigned char *payload, unsigned int payloadLength, unsigned int serviceId, unsigned int portId, unsigned int clientId, unsigned char family)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Context [payLoad:%p payloadLength:%u serviceId:%u portId:%u clientId:%u family:%u]", payload, payloadLength, serviceId, portId, clientId, family);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Silently ignoring packet transmission. I'm a Slave Matrix [portId=%u serviceId=%u]",portId, serviceId );
    return SUCCESS;
  }
#elif (PTIN_BOARD_IS_LINECARD || PTIN_BOARD_IS_STANDALONE)
  ptin_prottypeb_intf_config_t protTypebIntfConfig = {0};
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Silently ignoring packet transmission. I'm a Protection Port [portId=%u serviceId=%u]",portId, serviceId );
    return SUCCESS;
  }
#else
  #error "Not Implemented Yet"
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Silently ignoring packet transmission. Outgoing interface [portId=%u serviceId=%u] is down!",portId,serviceId );    
    return SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Unable to get port type from int_ovlan [%u] portId [%u]",serviceId,portId);
    return FAILURE;
  }
  //Get Group Address
  ...
  else
  {
    if (clientId != (unsigned int) -1)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Cannot convert intIfNum %u to ptin_port format", portId);
        return L7_FAILURE;
      }
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Cannot obtain int_ovlan (%u) from client id:%u", int_ovlan, clientId);              
        return L7_FAILURE;
      }
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Obtained int_ovlan (%u) from client id:%u", int_ovlan, clientId);  
    }     
  }
  #endif
  ...
  else //To support sending one Membership Query Message per ONU 
  {
    ...
    { 
      ...
      {        
        ...
        {
          LOG_WARNING(LOG_CTX_IGMP,"Either mgmdNumberOfQueryInstances [%u] >= PTIN_SYSTEM_N_EVCS [%u] or mgmdQueryInstances=%p",mgmdNumberOfQueryInstances,PTIN_SYSTEM_N_EVCS,mgmdQueryInstancesPtr);
          mgmdNumberOfQueryInstances=0;
          return SUCCESS;
        }

        if (ptin_debug_igmp_snooping)
          LOG_TRACE(LOG_CTX_IGMP,"Going to send %u Group Specific Queries",mgmdNumberOfQueryInstances);
...
L7_RC_t ptin_mgmd_send_leaf_packet(uint32 portId, L7_uint16 int_ovlan, L7_uint16 int_ivlan, L7_uchar8 *payload, L7_uint32 payloadLength,uchar8 family, L7_uint client_idx)
{
  ...
  do
  {
    ...
    #if (defined IGMP_QUERIER_IN_UC_EVC)
    {
      ...
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"rc=%d", rc);
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        if (clientFlow.int_ivid != 0)
        {
          ...
          if (ptin_debug_igmp_snooping)
          {
            LOG_TRACE(LOG_CTX_IGMP,"Packet will be transmited for client cvlan=%u (client_idx=%u) in intIfNum=%u (intVlan=%u)",
      ...
      else if ( rc == L7_NOT_SUPPORTED )
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_TRACE(LOG_CTX_IGMP,"Packet will be transmited for intIfNum=%u (intVlan=%u)", portId, int_ovlan);
        }
      }
      else
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_TRACE(LOG_CTX_IGMP,"No more transmissions for intIfNum=%u (intVlan=%u), rc=%u", portId, int_ovlan, rc);
        }
        break;
      }
    }
...
L7_RC_t ptin_snoop_sync_mx_process_request(L7_uint16 vlanId, L7_uint32 groupAddr)
{
  ...
  { 
    LOG_NOTICE(LOG_CTX_PROTB, "Silently Ignoring Snoop Sync Request. I'm a protection Matrix!");
    return L7_SUCCESS;
  }
  else
  {
  ...
  {
    LOG_NOTICE(LOG_CTX_PROTB, "Silently Ignoring Snoop Sync Request. Snoop Table is Empty");
    return L7_SUCCESS;
  }
  else
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_PROTB,"Max Number of MAC MFDB Entries:%u",L7_MFDB_MAX_MAC_ENTRIES);
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_PROTB,"Number of Existing Snoop Entries (%u) | Maximum number of snoop entries (%u)",numberOfSnoopEntries,maxNumberOfSnoopEntries);
          ...
          numberOfSnoopEntries<maxNumberOfSnoopEntries)
  {
    ...
    if( internalRootVlan != internalRootVlanTmp)
    {
      ...
      {
        LOG_WARNING(LOG_CTX_IGMP,"  Failed to obtain extended service Id from internal root vlan [internalRootVlan=%u]",internalRootVlan);
        continue;
      }
    }
    ...
    {
       /* Global information */    
      LOG_TRACE(LOG_CTX_IGMP,"  Family=%s     Vlan=%-4u MAC=%02x:%02x:%02x:%02x:%02x:%02x   %s",
    ...
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"  Channels information:");
    for (channel=0; channel<SNOOP_MAX_CHANNELS_PER_SNOOP_ENTRY; channel++)
    {
      ...
      if (ptin_debug_igmp_snooping)
      {
        LOG_TRACE(LOG_CTX_IGMP,"    Channel#%-2u:       IpAddr=%u.%u.%u.%u",channel,
  ...
  if (numberOfSnoopEntries>0)
  {    
    LOG_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Reply Message to ipAddr:%08X with %u snoop Entries  to sync the protection matrix",ipAddr, numberOfSnoopEntries);
    if(vlanId==0 && groupAddr==0)
    {
      LOG_DEBUG(LOG_CTX_MSG, "Remaining Snoop Entries to be Sync:%u",avlTreeCount(&(snoopEBGet()->snoopAvlTree))-numberOfSnoopEntries);     
    }
    ...
    {
      LOG_ERR(LOG_CTX_PROTB, "Failed to send Snoop Sync Reply Message");
      return L7_FAILURE;
    }
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "No more Snoop Entries Remaining to be Sync");    
  }
...
L7_RC_t ptin_snoop_sync_port_process_request(L7_uint16 vlanId, L7_uint32 groupAddr, L7_uint32 portId)
{
  ...
  { //I'm Protection
    LOG_NOTICE(LOG_CTX_PROTB, "Silently Ignoring Snoop Sync Request. I'm a Protection Port");
    return L7_SUCCESS;
  }
          ...
          numberOfSnoopEntries<maxNumberOfSnoopEntries)
  {
    ...
    if (L7_INTF_ISMASKBITSET(snoopInfoData->snoopGrpMemberList,portId))
    {
      ...
      /* Global information */    
      LOG_TRACE(LOG_CTX_PROTB,"  Family=%s     Vlan=%-4u MAC=%02x:%02x:%02x:%02x:%02x:%02x   %s",
             ((snoopInfoDataKeyPtr->family==L7_AF_INET) ? "IPv4" : "IPv6"),
             internalRootVlan,
             snoopInfoDataKeyPtr->vlanIdMacAddr[L7_FDB_IVL_ID_LEN+0],
             snoopInfoDataKeyPtr->vlanIdMacAddr[L7_FDB_IVL_ID_LEN+1],
      ...
      /* Channels information */
      LOG_TRACE(LOG_CTX_PROTB,"  Channels information:");
      for (channel=0; channel<SNOOP_MAX_CHANNELS_PER_SNOOP_ENTRY; channel++)
      {
        //Only Sync Active Channels
        if (!snoopInfoData->channel_list[channel].active)  continue;
        ...
        LOG_TRACE(LOG_CTX_PROTB,"    Channel#%-2u:       IpAddr=%u.%u.%u.%u",channel,
               (snoopSyncReply[numberOfSnoopEntries].groupAddr>>24) & 0xff,
               (snoopSyncReply[numberOfSnoopEntries].groupAddr>>16) & 0xff,
               (snoopSyncReply[numberOfSnoopEntries].groupAddr>>8) & 0xff,
               snoopSyncReply[numberOfSnoopEntries].groupAddr & 0xff);            
  ...
  if (numberOfSnoopEntries>0)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Reply Message ipAddr:%08X with %u Snoop Entries  to sync slot/port:%u/%u",ipAddr, numberOfSnoopEntries, protTypebIntfConfig.pairSlotId, protTypebIntfConfig.pairIntfNum);
    /*Send the snoop sync request to the protection matrix */  
    if (send_ipc_message(IPC_HW_FASTPATH_PORT, ipAddr, CCMSG_MGMD_SNOOP_SYNC_REPLY, (char *)(&snoopSyncReply), NULL, numberOfSnoopEntries*sizeof(msg_SnoopSyncReply_t), NULL) < 0)
    {
      LOG_ERR(LOG_CTX_PROTB, "Failed to send Snoop Sync Reply Message");
...
L7_RC_t ptin_igmp_mgmd_status_get(void)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "MGMD replied");
  ptin_mgmd_event_ctrl_parse(&outEventMsg, &ctrlResMsg);
  LOG_DEBUG(LOG_CTX_IGMP,  "  CTRL Msg Code: %08X", ctrlResMsg.msgCode);
  LOG_DEBUG(LOG_CTX_IGMP,  "  CTRL Msg Id  : %08X", ctrlResMsg.msgId);
  LOG_DEBUG(LOG_CTX_IGMP,  "  CTRL Res     : %u",   ctrlResMsg.res);
 
  /* Copy the response contents to igmpProxy */
  if(sizeof(PTIN_MGMD_CTRL_MGMD_STATUS_t) != ctrlResMsg.dataLength)
  {
    LOG_ERR(LOG_CTX_IGMP,"Unexpected size in the MGMD response [dataLength:%u/%u]", ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_MGMD_STATUS_t));
    return L7_FAILURE;
  }
  else
  {
    memcpy(&mgmdStatus, ctrlResMsg.data, ctrlResMsg.dataLength);
    LOG_DEBUG(LOG_CTX_IGMP,  "  Mgmd Status  : %s",   mgmdStatus.mgmdStatus == PTIN_MGMD_STATUS_WORKING ? "Alive":"Dead");
  }
