...
L7_RC_t snoop_ptin_sourcetimer_init(void)
{
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Could not create task ptin_igmp_sourcetimer_task");
    return L7_FAILURE;
  }
  if (osapiWaitForTaskInit (L7_PTIN_IGMP_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR,"Unable to initialize ptin_igmp_sourcetimer_task");
    return(L7_FAILURE);
  }
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"Initializations for IGMPv3 source timers finished");
...
void cbtimerCallback(L7_APP_TMR_CTRL_BLK_t timerCtrlBlk, void* ptrData)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Sourcetimer timer tick send failed");
    return;
  }
}
...
void timerCallback(void *param)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Sourcetimer expired (group:%s vlan:%u ifId:%u sourceAddr)", 
...
#endif        
        LOG_DEBUG(LOG_CTX_IGMP, "Root Interface - Removing source %s", inetAddrPrint(&(sourcePtr->sourceAddr), debug_buf));        
      }
      else
      {

        LOG_DEBUG(LOG_CTX_IGMP, "Leaf Interface - Removing source %s", inetAddrPrint(&(sourcePtr->sourceAddr), debug_buf));        
      }
      /* Remove source */
...
#endif
      LOG_DEBUG(LOG_CTX_IGMP, "Group has no more sources, thus it is being removed.");
...
L7_RC_t snoop_ptin_sourcetimer_start(snoopPTinL3Sourcetimer_t *pTimer, L7_uint32 timeout, snoopPTinL3InfoData_t *groupData, L7_uint32 interfaceIdx, L7_uint32 sourceIdx)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP,"Starting sourcetimer (timeout:%u group:%s vlan:%u ifIdx:%u srcAddr:%s)",
  ...
  if (pTimer->timer == NULL)
  {
    ...
    pTimer->timerHandle = 0;
    osapiSemaGive(timerSem);
    LOG_ERR(LOG_CTX_IGMP,"Could not start the sourcetimer.");
    return L7_FAILURE;
  }
...
L7_RC_t snoop_ptin_sourcetimer_stop(snoopPTinL3Sourcetimer_t *pTimer)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Removing sourcetimer (group:%s vlan:%u ifIdx:%u srcAddr:%s)",
