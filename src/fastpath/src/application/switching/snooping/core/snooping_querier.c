                              ...
                              L7_inet_addr_t *leave_grp_addr)
{
  ...
  */
  ...
  {    
    ...
    LOG_PT_TRACE(LOG_CTX_IGMP, "Robustness Variable %u",pSnoopOperEntry->snoopQuerierInfo.sFlagQRV);
  }
...
void snoopQuerierPeriodicQuerySend(snoopOperData_t *pSnoopOperEntry)
{
  ...
  LOG_PT_TRACE(LOG_CTX_IGMP, "Robustness Variable:%u",pSnoopOperEntry->snoopQuerierInfo.sFlagQRV);
  LOG_PT_TRACE(LOG_CTX_IGMP, "Querier's Query Interval Code:%u",pSnoopOperEntry->snoopQuerierInfo.qqic);    
  LOG_PT_TRACE(LOG_CTX_IGMP, "Max Response Code:%u",pSnoopOperEntry->snoopQuerierInfo.maxResponseCode);    
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    LOG_PT_DEBUG(LOG_CTX_IGMP,"Sending periodic query to client interfaces (vlanId=%u)",vlanId);
