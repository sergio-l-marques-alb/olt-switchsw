 ...
 */
static void snoopMgmdSwitchPortOpenProcess(L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_BOOL isStatic, L7_BOOL isProtection);
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
static void snoopMgmdSwitchPortCloseProcess(L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_BOOL isProtection);
...
//             "snoopTask: Queues out of sync");
    ...
    case snoopMgmdSwitchPortOpen:
        snoopMgmdSwitchPortOpenProcess(pduMsg.vlanId, pduMsg.intIfNum, pduMsg.groupAddress, pduMsg.sourceAddress, pduMsg.isStatic, pduMsg.isProtection);
        break;

    case snoopMgmdSwitchPortClose:
        snoopMgmdSwitchPortCloseProcess(pduMsg.vlanId, pduMsg.intIfNum, pduMsg.groupAddress, pduMsg.sourceAddress, pduMsg.isProtection);
        break;
                                       ...
                                       snoop_cb_t *pSnoopCB)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_IGMP,"Error configuring to %u vlan %u trapping",mode,vlanId);
  }
  else
  {
    LOG_PT_TRACE(LOG_CTX_IGMP,"Success configuring to %u vlan %u trapping",mode,vlanId);
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_IGMP,"Error configuring to %u vlan %u querier",mode,vlanId);
  }
  else
  {
    LOG_PT_TRACE(LOG_CTX_IGMP,"Success configuring to %u vlan %u querier",mode,vlanId);
  }
...
static void snoopMgmdSwitchPortOpenProcess(L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_BOOL isStatic, L7_BOOL isProtection)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_IGMP, "Received request to open a new port on the switch [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X isStatic:%u isProtection:%s]", serviceId, portId, groupAddr, sourceAddr, isStatic, isProtection?"Yes":"No");
  ...
  if( L7_SUCCESS != (rc=ptin_evc_intRootVlan_get(serviceId, &mcastRootVlan)))
  {
    if( rc != L7_NOT_EXIST)
    {
      if (ptin_debug_igmp_snooping)
        LOG_PT_ERR(LOG_CTX_IGMP, "Unable to get mcastRootVlan from serviceId:%u",serviceId);
      return;
    }
    if (ptin_debug_igmp_snooping)
      LOG_PT_NOTICE(LOG_CTX_IGMP, "Evc Id is not yet created. Silently Ignoring Port Open Request! [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X]", serviceId, portId, groupAddr, sourceAddr);
    return;
  }

  inetAddressSet(L7_AF_INET, &groupAddr, &groupIp);
  inetAddressSet(L7_AF_INET, &sourceAddr, &sourceIp);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_PT_ERR(LOG_CTX_IGMP, "Unable to open port on switch for intVlan:%u groupAddr:%08X intfNum:%u", mcastRootVlan, groupIp.addr.ipv4.s_addr, portId);
  }
  ptin_timer_stop(88);
}
...
static void snoopMgmdSwitchPortCloseProcess(L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_BOOL isProtection)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_IGMP, "Received request to close an existing port on the switch [serviceId:%u portId:%u groupAddr:%08X sourceAddr:%08X isProtection:%s]", serviceId, portId, groupAddr, sourceAddr, isProtection?"Yes":"No");
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_PT_ERR(LOG_CTX_IGMP, "Unable to close port on switch for intVlan:%u groupAddr:%08X intfNum:%u", mcastRootVlan, groupIp.addr.ipv4.s_addr, portId);
  }
  ptin_timer_stop(89);
}
