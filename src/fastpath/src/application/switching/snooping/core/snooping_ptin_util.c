...
{
  PT_LOG_DEBUG(LOG_CTX_IGMP, "Adding client to L2");
  return L7_SUCCESS;
}
...
{
  PT_LOG_WARN(LOG_CTX_IGMP, "Removing client from L2");
  return L7_SUCCESS;
}
...
//  {
//    PT_LOG_ERR(LOG_CTX_IGMP, "Query buffer is full");
//    return L7_FAILURE;
//  }
//
...
//void* newPtr=L7_NULLPTR;
  ...
  else
  {
    ...
    {
      PT_LOG_WARN(LOG_CTX_IGMP, "Using default value for Unsolicited Report Interval, %u",PTIN_IGMP_DEFAULT_UNSOLICITEDREPORTINTERVAL);  
      igmpCfg.host.unsolicited_report_interval=PTIN_IGMP_DEFAULT_UNSOLICITEDREPORTINTERVAL;    
    }
    if (igmpCfg.host.max_records_per_report==0)
    {
      PT_LOG_WARN(LOG_CTX_IGMP, "Using default value for Max Records per Report, %u",PTIN_IGMP_DEFAULT_MAX_RECORDS_PER_REPORT);  
      igmpCfg.host.max_records_per_report=PTIN_IGMP_DEFAULT_MAX_RECORDS_PER_REPORT;
    }
    if (igmpCfg.igmp_cos==0x0)
    {
      PT_LOG_WARN(LOG_CTX_IGMP, "Using default value for IGMPv3 CoS, %u",PTIN_IGMP_DEFAULT_COS);  
      igmpCfg.igmp_cos=PTIN_IGMP_DEFAULT_COS;
    }
  }
    ...
    if (proxyTimer->isFirstTransmission==L7_TRUE)
    {
      if (isInterface==L7_TRUE)
      {
        if ((groupPtr=snoopPTinBuildCSR(interfacePtr,&noOfRecordsAux,robustnessVariable))==L7_NULLPTR)
        {
          if (noOfRecordsAux>0)
          {
            PT_LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinBuildCSR()");
            return L7_ERROR;
          }
          else
          {
            PT_LOG_TRACE(LOG_CTX_IGMP, "Membership Response to General Query silenty discarded, once we do not have any active groups");
            return L7_SUCCESS;
          }
        }
      }
      else
      {
        ...
        if (reportType==L7_IGMP_MEMBERSHIP_GROUP_SPECIFIC_QUERY || reportType==L7_IGMP_MEMBERSHIP_GROUP_AND_SOURCE_SCPECIFC_QUERY)
        {
          ...
          {
            PT_LOG_TRACE(LOG_CTX_IGMP, "Membership Response to Group Query silenty discarded, once this group is no longer active");
            return L7_SUCCESS;          
          }
          if (reportType==L7_IGMP_MEMBERSHIP_GROUP_AND_SOURCE_SCPECIFC_QUERY)
          {
            ...
            {
              PT_LOG_ERR(LOG_CTX_IGMP, "Report Type=SNOOP_PTIN_GROUP_AND_SOURCE_SPECIFIC_QUERY, while numberOfSources=0");
              return L7_ERROR;
            }
            ...
            for (sourceIdx=0;sourceIdx<groupPtr->numberOfSources && sourcePtr!=L7_NULLPTR;sourceIdx++)
            {
              ...
              else if (rc==L7_SUCCESS && snoopPTinZeroClients(avlTreeEntry->interfaces[SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM].sources[foundIdx].clients)==L7_SUCCESS)
              {
                ...
                {
                  PT_LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinGroupRecordSourceRemove()");
                  return L7_ERROR;
                }
                PT_LOG_TRACE(LOG_CTX_IGMP, "Membership Response to Group & Source Specific Query, source address silently discarded, once this source is no longer active");
              }
              sourcePtr++;
            }
          }
...
//      newgroupPtr=newgroupPtr+igmpCfg.host.max_records_per_report;       
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordIncrementTransmissions()");
      return L7_FAILURE;
    }

    if (newNoOfRecords>0)
    {
      PT_LOG_DEBUG(LOG_CTX_IGMP, "Re-Scheduling Membership Report Message with timeout: %u ",igmpCfg.host.unsolicited_report_interval);  
...
//We need to check if the MTU is reached or not!
  ...
  PT_LOG_DEBUG(LOG_CTX_IGMP, "Number of Group Records :%u",(L7_ushort16) noOfRecords);
  groupPtrAux=groupPtr;
  for (i=0;i<noOfRecords && groupPtrAux != L7_NULLPTR; i++)
  {
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Group Record (groupAddr: %s  recordType: %u)", inetAddrPrint(&groupPtrAux->key.groupAddr, debug_buf),groupPtrAux->recordType);
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "snoopPTinGroupRecordV3Build()");
      return L7_FAILURE;
    }
    if (groupRecordLength==0)
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "groupRecordLength :%u",groupRecordLength);
      return L7_FAILURE;
    }
    PT_LOG_DEBUG(LOG_CTX_IGMP, "groupRecordLength :%u",groupRecordLength);
  ...
  if (i!=noOfRecords)
  {
    ...
    PT_LOG_WARN(LOG_CTX_IGMP, "Problems with groupPrt %u<%u",i,noOfRecords);
  }

  /* Update frame length */
  *length = SNOOP_IGMPv1v2_HEADER_LENGTH +totalGroupRecordLength;
  PT_LOG_DEBUG(LOG_CTX_IGMP, "IGMP Frame Size  :%u",*length);
...
static L7_uchar8* snoopPTinGroupRecordV3Build(L7_uint32 vlanId, L7_inet_addr_t* groupAddr,L7_uint8 recordType,L7_uint16 numberOfSources,snoopPTinProxySource_t* source, L7_uchar8 *buffer, L7_uint32 *length)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "He have a null pointer, while having the number of sources different of 0: %u",numberOfSources);
    return L7_NULLPTR;
  }
  ...
  {
    PT_LOG_WARN(LOG_CTX_IGMP, "Invalid IP Family");
    return L7_NULLPTR;
  }
  ...
  PT_LOG_DEBUG(LOG_CTX_IGMP, "Number of Sources :%u", numberOfSources);
  sourcePtr=source;  
  for (i=0;i<numberOfSources && sourcePtr != L7_NULLPTR;i++)
  {
    ...
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Source (sourceAddr: %s)", inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf));
  ...
  if (i!=numberOfSources)
  {
    ...
    PT_LOG_WARN(LOG_CTX_IGMP, "Problems with groupRecord %u<%u",i,numberOfSources);
  }
...
void snoopPTinQuerySend(L7_uint32 arg1)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Error getting pSnoopCB");
    return;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Not IPv4 packet");
    return;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Error building IGMPv3 Query frame");
    return;
  }
  ...
  if (rc == L7_SUCCESS)
  {
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Packet transmitted to client interfaces. (queue idx %d)", queryData->queuePos);
  }
  else
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Error transmitting to client interfaces");
  }
...
//snoopOperData_t       *pSnoopOperEntry;
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Preparing to send a Membership Report Message with %u group records",noOfGroupRecords);
...
//pSnoopOperEntry = snoopOperEntryGet(vlanId, pSnoopCB, L7_MATCH_EXACT);
  ...
  if (rc != L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Error building Membership Report Frame (IGMPv3)");
    return L7_FAILURE;
  }
  ...
  if (rc == L7_SUCCESS)
  {
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Packet transmitted to root interfaces.");
  }
  else
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Error transmitting to root interfaces");
    return L7_FAILURE;
  }  
...
{  
  ...
  for (i=0;i<noOfRecords&&groupPtrAux!=L7_NULLPTR;i++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinGroupRecordSourceIncrementTransmissions()");
//    return L7_NULLPTR;
    }
    if (++groupPtrAux->retransmissions>=groupPtrAux->robustnessVariable)
    {
      PT_LOG_TRACE(LOG_CTX_IGMP, "Adding Group Record %s with recorType %u  (retransmissions:%u) to Removal List", inetAddrPrint(&groupPtrAux->key.groupAddr, debug_buf),groupPtrAux->recordType,groupPtrAux->retransmissions);      
  ...
  if (i!=noOfRecords)
  {
    PT_LOG_WARN(LOG_CTX_IGMP, "Problems with groupRecordPtr %u<%u",i,noOfRecords);
  }


  if (((*newNoOfRecords)=noOfRecords-noOfGroupRecord2remove)==0)
  {
    PT_LOG_TRACE(LOG_CTX_IGMP, "Going to remove all Group Records");
    newgroupPtr=L7_NULLPTR;    
  }
  else
  {
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Number of Group Records pending to be Retransmitted %u",*newNoOfRecords);
    if (newgroupPtr==L7_NULLPTR)
    {
      PT_LOG_WARN(LOG_CTX_IGMP,"Invalid Group Record Pointer");
    }
  }

  for (i=0;i<noOfGroupRecord2remove;i++)
  {
    PT_LOG_DEBUG(LOG_CTX_IGMP, "Going to Remove  Group Record:  vlanId:%u GroupAdd=%s, RecordType=%u",interfacePtr->key.vlanId, inetAddrPrint(groupAddr[i], debug_buf),recordType[i]);     
    if ((rc=snoopPTinGroupRecordRemove(interfacePtr,groupAddr[i], recordType[i]))!=L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordSourceDelete()");        
    }
  }
...
{  
  ...
  if (groupPtr->numberOfSources==0)
  {
    PT_LOG_TRACE(LOG_CTX_IGMP, "This Group record was no sources");
    return L7_SUCCESS;
  }
  ...
  for (i=0;i<groupPtr->numberOfSources && sourcePtrTmp!=L7_NULLPTR;i++)
  {
    ...
    {
      PT_LOG_TRACE(LOG_CTX_IGMP, "Adding Source Address %s from the group record (retransmissions:%u) to Removal List", inetAddrPrint(&sourcePtrTmp->key.sourceAddr, debug_buf),sourcePtrTmp->retransmissions); 
  ...
  {
    PT_LOG_WARN(LOG_CTX_IGMP, "Problems with sourcePtr %u<%u",i,groupPtr->numberOfSources);
  }

  if ( groupPtr->numberOfSources==noOfSources2Remove)/*We should remove all sources within this group record*/
  {
    PT_LOG_TRACE(LOG_CTX_IGMP, "Going to Remove all Sources from Group Record  (groupAddr:%s recordType:%u)", inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->recordType);  
  ...
  else
  {
    ...
    {
      PT_LOG_DEBUG(LOG_CTX_IGMP, "Going to remove Source from Group Record  %s", inetAddrPrint(sourceAddr[i], debug_buf));     
...
static snoopPTinProxyGroup_t* snoopPTinBuildCSR(snoopPTinProxyInterface_t *interfacePtr, L7_uint32 *noOfRecordsPtr,L7_uint8 robustnessVariable)
{
  ...
  PT_LOG_NOTICE(LOG_CTX_IGMP, "Building Current State Records of vlanId:%u",interfacePtr->key.vlanId);
/* Run all cells in AVL tree */    
  ...
  while ( ( avlTreeEntry = avlSearchLVL7(&pSnoopEB->snoopPTinL3AvlTree, &avlTreeKey, L7_MATCH_GETNEXT) ) != L7_NULLPTR )
  {
    ...
    {
      PT_LOG_NOTICE(LOG_CTX_IGMP, "Group Address Number %u",++groupIdx);
      ...
      PT_LOG_DEBUG(LOG_CTX_IGMP, "Group Address: %s,  recordType:%u", inetAddrPrint(&avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr, debug_buf),recordType);    
      ...
      {
        PT_LOG_NOTICE(LOG_CTX_IGMP, "First Group Ptr Added");
  ...
  *noOfRecordsPtr=noOfRecords;
  PT_LOG_NOTICE(LOG_CTX_IGMP, "Number of Group Records to be sent :%u",*noOfRecordsPtr);
...
/* Run all cells in AVL tree */    
  memset(&avlTreeKey,0x00,sizeof(snoopPTinL3InfoDataKey_t));
  PT_LOG_NOTICE(LOG_CTX_IGMP, "snoopPTinDumpL3AvlTree");
  while ( ( avlTreeEntry = avlSearchLVL7(&pSnoopEB->snoopPTinL3AvlTree, &avlTreeKey, L7_MATCH_GETNEXT) ) != L7_NULLPTR )
  {    
    /* Prepare next key */
    memcpy(&avlTreeKey, &avlTreeEntry->snoopPTinL3InfoDataKey, sizeof(snoopPTinL3InfoDataKey_t));
...
void snoopPTinDumpGroupRecordAvlTree(void)
{
  ...
  PT_LOG_NOTICE(LOG_CTX_IGMP, "snoopPTinDumpGroupRecordAvlTree");
/* Run all cells in AVL tree */    
  memset(&avlTreeKey,0x00,sizeof(snoopPTinL3InfoDataKey_t));
...
//  PT_LOG_NOTICE(LOG_CTX_IGMP, "vlanId:%u  groupAddr  %s recordType:%u",avlTreeEntry->key.vlanId,inetAddrPrint(&avlTreeEntry->key.groupAddr, debug_buf),avlTreeEntry->key.recordType);
