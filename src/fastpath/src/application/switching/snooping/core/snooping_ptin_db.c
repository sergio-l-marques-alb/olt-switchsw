...
L7_RC_t snoopPTinClientFind(L7_uint32 *clientList, L7_uint32 clientIdx)
{
  ...
  for (i=0;i<PTIN_SYSTEM_IGMP_CLIENT_BITMAP_SIZE && clientsPtr!=L7_NULLPTR; i++)
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u found within this Client List",clientIdx);
      return L7_SUCCESS;
    }
    clientsPtr++; 
  }  

  LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u not found within this Client List",clientIdx);
  return L7_FAILURE;
}
...
L7_RC_t snoopPTinClientAdd(snoopPTinL3Interface_t *interfacePtr, snoopPTinL3Source_t *sourcePtr, L7_uint32 clientIdx)
{
  ...
  if (L7_SUCCESS != snoopPTinClientFind(sourcePtr->clients, clientIdx))
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u added to this Source Addr %s, number of Clients: %u",clientIdx,inetAddrPrint(&sourcePtr->sourceAddr, debug_buf), sourcePtr->numberOfClients);
  }
  else
  {
    LOG_TRACE(LOG_CTX_IGMP, "Existing ClientIdx:%u in this Source Addr %s, number of Clients: %u",clientIdx,inetAddrPrint(&sourcePtr->sourceAddr, debug_buf), sourcePtr->numberOfClients);
...
L7_RC_t snoopPTinClientInterfaceAdd(snoopPTinL3Interface_t *interfacePtr, L7_uint32 clientIdx)
{
  ...
  if (L7_SUCCESS != snoopPTinClientFind(interfacePtr->clients, clientIdx))
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u added to this Interface, number of Clients: %u",clientIdx,interfacePtr->numberOfClients);
    return L7_SUCCESS;
  }
  else
  {
    LOG_TRACE(LOG_CTX_IGMP, "Existing ClientIdx:%u in this Interface, number of Clients: %u",clientIdx,interfacePtr->numberOfClients);
    return L7_ALREADY_CONFIGURED;
  }   
}
...
//L7_uint32 i, clientSourceCnt;
  ...
  if (L7_SUCCESS == snoopPTinClientFind(sourcePtr->clients, clientIdx))
  {
    ...
    --sourcePtr->numberOfClients;
    LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u removed from this Source Addr %s, number of Clients: %u",clientIdx,inetAddrPrint(&sourcePtr->sourceAddr, debug_buf), sourcePtr->numberOfClients);
    return L7_SUCCESS;
  }
  else
  {
    LOG_TRACE(LOG_CTX_IGMP, "Non existing ClientIdx:%u in this Source Addr %s, number of Clients: %u",clientIdx,inetAddrPrint(&sourcePtr->sourceAddr, debug_buf), sourcePtr->numberOfClients);
  }
  return L7_NOT_EXIST;
}
...
L7_RC_t snoopPTinClientInterfaceRemove(snoopPTinL3Interface_t *interfacePtr, L7_uint32 clientIdx)
{
  ...
  if (L7_SUCCESS == snoopPTinClientFind(interfacePtr->clients, clientIdx))
  {
    ...
    --interfacePtr->numberOfClients;
    LOG_DEBUG(LOG_CTX_IGMP, "ClientIdx:%u removed from this Interface, number of Clients left: %u",clientIdx,interfacePtr->numberOfClients);
    return L7_SUCCESS;
  }
...
L7_RC_t snoopPTinZeroInterfaceClients(snoopPTinL3InfoData_t* avlTreeEntry)
{
  ...
  for (interfaceIdx=1;interfaceIdx<PTIN_SYSTEM_MAXINTERFACES_PER_GROUP;interfaceIdx++)
  {
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "Existing Clients interfaceIdx:%u",interfaceIdx);
      return L7_ALREADY_CONFIGURED;  

    }
  }
  LOG_DEBUG(LOG_CTX_IGMP, "Interface bitmap without clients");
  return L7_SUCCESS;
}
  ...
  for (interfaceIdx=1;interfaceIdx<PTIN_SYSTEM_MAXINTERFACES_PER_GROUP;interfaceIdx++)
  {
    if (avlTreeEntry->interfaces[interfaceIdx].active==L7_TRUE)
    {
      for (sourceIdx=0;sourceIdx<avlTreeEntry->interfaces[interfaceIdx].numberOfSources;sourceIdx++)
      {
        ...
        {
          LOG_NOTICE(LOG_CTX_IGMP, "Existing Client @ interfaceIdx:%u, sourceIdx:%u",interfaceIdx,sourceIdx);
          return L7_ALREADY_CONFIGURED;            
        }
      }
    }
  }
  LOG_DEBUG(LOG_CTX_IGMP, "All source client bitmaps are without clients");
  return L7_SUCCESS;
}
#endif
...
L7_RC_t snoopPTinZeroClients(L7_uint32* clients)
{
  ...
  for (i=0;i<PTIN_SYSTEM_IGMP_CLIENT_BITMAP_SIZE && clientsPtr!=L7_NULLPTR; i++)
  {
    if (*clientsPtr!=0)
    {
      LOG_NOTICE(LOG_CTX_IGMP, "Bitmap with clients");
      return L7_ALREADY_CONFIGURED; 
    }
    clientsPtr++; 
  }  

  LOG_DEBUG(LOG_CTX_IGMP, "Bitmap without Clients");
  return L7_SUCCESS;  
...
L7_RC_t snoopPTinProxySourceAdd(snoopPTinL3InfoData_t* avlPtr,L7_uint32 clientIdx, L7_inet_addr_t* sourceAddr)
{
  ...
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Existing source %s on idx %d", inetAddrPrint(sourceAddr, debug_buf), sourceIdx);
    rc=L7_FAILURE;                
  }
  else
  {
    LOG_DEBUG(LOG_CTX_IGMP, "New source %s with clientIdx %u", inetAddrPrint(sourceAddr, debug_buf), clientIdx);
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Source list for this multicast group is full");
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_DEBUG(LOG_CTX_IGMP, "New clientIdx %u", clientIdx);
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Client list for this multicast group is full");
      return L7_FAILURE;
    }
  }
...
//  {
//    LOG_DEBUG(LOG_CTX_IGMP, "Removing clientIdx %u", clientIdx);
//    if (L7_SUCCESS != snoopPTinClientRemove(&avlTreeEntry->interfaces[intIfNum].sources[sourceIdx], clientIdx))
//    {
//      LOG_ERR(LOG_CTX_IGMP, "Unable to remove clientIdx %u", clientIdx);
//      return L7_FAILURE;
//    }
//  }
//}
...
L7_RC_t snoopPTinSourceFind(snoopPTinL3Source_t *sourceList, L7_inet_addr_t* sourceAddr, L7_uint16* foundIdx)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Input Source Address: %s", inetAddrPrint(sourceAddr, debug_buf));
  for (sourceIdx = 0; sourceIdx < PTIN_SYSTEM_IGMP_MAXSOURCES_PER_GROUP; ++sourceIdx)
  {
    ...
    {
      LOG_TRACE(LOG_CTX_IGMP, "Existing Source Address: %s", inetAddrPrint(&sourceList[sourceIdx].sourceAddr, debug_buf));
    }
    ...
    {
      LOG_TRACE(LOG_CTX_IGMP, "Source Found: Source Address: %s", inetAddrPrint(&sourceList[sourceIdx].sourceAddr, debug_buf));
      *foundIdx = sourceIdx;
      return L7_SUCCESS;
    }
  }
...
L7_RC_t snoopPTinInterfaceRemove(snoopPTinL3Interface_t *interfacePtr,L7_uint32 vlanId, L7_inet_addr_t *groupAddr,L7_uint16 interfaceIdx)
{
  ...
  {
    LOG_NOTICE(LOG_CTX_IGMP,"Going to close this L2 Port (interfaceIdx:%u vlanId:%u groupAddr:%s)",interfaceIdx,vlanId,inetAddrPrint(groupAddr, debug_buf));
    /*Close L2 Port on Switch*/
    if (snoopL2GroupIntfRemove(vlanId,groupAddr,interfaceIdx, L7_FALSE)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to snoopGroupIntfRemove()");
      return L7_FAILURE;
    }
  }
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Failed to snoopPTinL3EntryDelete()");
      return L7_FAILURE;
    }
  }
  memset(interfacePtr, 0x00, sizeof(*interfacePtr));
...
L7_RC_t snoopPTinMembershipReportIsIncludeProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Group Address: %s,  IntIfnum:%d, VLANID=%d", inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf),intIfNum, avlTreeEntry->snoopPTinL3InfoDataKey.vlanId);
  ...
  if (noOfSources > 0)
  {
    ...
    while (noOfSources > 0 && sourceAddr !=L7_NULLPTR)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinSourceFind()");
        return L7_FAILURE;
      }
      else if (rc==L7_SUCCESS)
      {
        ...
        {
          LOG_WARNING(LOG_CTX_IGMP, "Source list for this multicast group is full");
          return L7_FAILURE;
        }
        if (L7_SUCCESS == snoopPTinProxySourceAdd(avlTreeEntry,intIfNum, sourceAddr))
        {
          ...
          {
            LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordSourcedAdd()");       
            return L7_FAILURE;
          }
          LOG_TRACE(LOG_CTX_IGMP, "Source Added to Group Record (vlanId:%u, groupAddr:%s recordType:%u sourceAddr:%s)", groupPtr->interfacePtr->key.vlanId,inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->recordType,inetAddrPrint(sourceAddr, debug_buf2));   
          if (noOfRecords==0)
            noOfRecords=1;
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Unable to start sourcetimer");
        return L7_FAILURE;
      }
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinClientAdd()");
        return L7_FAILURE;
      }
  ...
  if (snoopPTinClientFind(avlTreeEntry->interfaces[intIfNum].clients,clientIdx)==L7_SUCCESS)
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP, "Going to remove ClientIdx %u from this interface :%u", clientIdx,intIfNum );
      if (L7_ERROR == snoopPTinClientInterfaceRemove(&avlTreeEntry->interfaces[intIfNum], clientIdx))
      {
        LOG_WARNING(LOG_CTX_IGMP, "Failed to remove Client from Interface bitmap");
        return L7_FAILURE;
      }
      /*Add group Record */
      if ((SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM==intIfNum) && (L7_SUCCESS==snoopPTinZeroClients(avlTreeEntry->interfaces[intIfNum].clients)))
      {
        ...
        LOG_DEBUG(LOG_CTX_IGMP, "Change group record type to ToIn{}");
      }
    }
  }
  else if (flagnoOfSources==L7_TRUE)
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "Going to add clientIdx %u to this interface :%u", clientIdx,intIfNum );
    if (L7_ERROR == snoopPTinClientInterfaceAdd(&avlTreeEntry->interfaces[intIfNum],clientIdx))
    {
      LOG_ERR(LOG_CTX_IGMP, "Interface bitmap is full");
      return L7_FAILURE;
    }
  }
...
L7_RC_t snoopPTinMembershipReportIsExcludeProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Group Address: %s,  IntIfnum:%d, VLANID=%d noOfSources=%u", inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf),intIfNum, avlTreeEntry->snoopPTinL3InfoDataKey.vlanId,noOfSources);
  ...
  if (noOfSources>0)
  {
    ...
    while (noOfSources > 0 && sourceAddr !=L7_NULLPTR)
    {
      ...
      else if (rc==L7_SUCCESS)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP, "Failed to remove Client clientIdx:%u",clientIdx);
          return L7_FAILURE;
        }
      ...
      {
        LOG_DEBUG(LOG_CTX_IGMP, "New source %s with clientIdx %d", inetAddrPrint(sourceAddr, debug_buf), clientIdx);
  ...
  else
  {
    ...
    if ((SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM==intIfNum) && (L7_SUCCESS==snoopPTinZeroClients(avlTreeEntry->interfaces[intIfNum].clients)))
    {
      ...
      LOG_DEBUG(LOG_CTX_IGMP, "Change group record type to ToEx{}");
    }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Unable to start grouptimer");
    return L7_FAILURE;
  }
...
L7_RC_t snoopPTinMembershipReportToIncludeProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  if (noOfSources > 0)
  {
    ...
    for (sourceIdx = 0; avlTreeEntry->interfaces[intIfNum].numberOfSources>0 && sourceIdx < sizeof(avlTreeEntry->interfaces[intIfNum].sources); ++sourceIdx)
    {
      ...
      {
        LOG_TRACE(LOG_CTX_IGMP, "Adding source %s to Q(G,S)", inetAddrPrint(&(avlTreeEntry->interfaces[intIfNum].sources[sourceIdx].sourceAddr), debug_buf));
    ...
    if (SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM != intIfNum)
    {
      ...
      {
        /* send Q(G,A-B) */
        LOG_TRACE(LOG_CTX_IGMP, "Schedule Group & Source Specific Query G=%s",inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf));
      ...
      {
        LOG_TRACE(LOG_CTX_IGMP, "Schedule Group Specific Query G=%s",inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf));
...
#endif
      LOG_NOTICE(LOG_CTX_IGMP,"Going to open this L2 Port (interfaceIdx:%u vlanId:%u groupAddr:%s)",intIfNum,avlTreeEntry->snoopPTinL3InfoDataKey.vlanId,inetAddrPrint(&avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr, debug_buf));
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoopGroupIntfAdd()");       
        return L7_FAILURE;
      }
    }
    else
  ...
  else /*To_In{}=IGMPv2 Leave Group*/
  {
    ...
    if (snoopPTinClientFind(avlTreeEntry->interfaces[intIfNum].clients,clientIdx)==L7_SUCCESS)
    {
      for (sourceIdx = 0; avlTreeEntry->interfaces[intIfNum].numberOfSources>0 && sourceIdx < sizeof(avlTreeEntry->interfaces[intIfNum].sources); ++sourceIdx)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinClientRemove()");
          return L7_FAILURE;
        }
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to remove Client from Interface bitmap");
        return L7_FAILURE;
      }
          ...
          L7_SUCCESS==snoopPTinZeroClients(avlTreeEntry->interfaces[intIfNum].clients))
      {
        ...
        {
          LOG_NOTICE(LOG_CTX_IGMP,"Going to close this L2 Port (interfaceIdx:%u vlanId:%u groupAddr:%s)",intIfNum,avlTreeEntry->snoopPTinL3InfoDataKey.vlanId,inetAddrPrint(&avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr, debug_buf));
...
L7_RC_t snoopPTinMembershipReportToExcludeProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  if (noOfSources > 0)
  {
        ...
        else if (rc==L7_SUCCESS)
        {
          ...
          if (intIfNum!=SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM || snoopPTinZeroSourceClients(avlTreeEntry, sourceAddr)==L7_SUCCESS)
          {
            ...
            else if (rc==L7_SUCCESS)
            {
              ...
              {
#if SNOOP_PTIN_GROUP_AND_SOURCE_SPECIFC_QUERY_SUPPORT
                LOG_TRACE(LOG_CTX_IGMP, "Adding source %s to Q(G,S)", inetAddrPrint(sourceAddr, debug_buf));
                sources2Query[sources2QueryCnt++] = avlTreeEntry->interfaces[intIfNum].sources[sourceIdx].sourceAddr;          
#endif
              }
            }
    ...
    for (sourceIdx = 0; avlTreeEntry->interfaces[intIfNum].numberOfSources>0 && sourceIdx < sizeof(avlTreeEntry->interfaces[intIfNum].sources); ++sourceIdx)
    {
      if (avlTreeEntry->interfaces[intIfNum].sources[sourceIdx].status == PTIN_SNOOP_SOURCESTATE_TOREMOVE)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinSourceRemove()");       
          return L7_FAILURE;
        }
      }
    }
  ...
  if (avlTreeEntry->interfaces[intIfNum].filtermode == PTIN_SNOOP_FILTERMODE_INCLUDE)
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Going to change filter-mode to Exclude");
...
L7_RC_t snoopPTinMembershipReportAllowProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  while (noOfSources > 0 && sourceAddr !=L7_NULLPTR)
  {
    ...
    else
    {
      ...
      LOG_DEBUG(LOG_CTX_IGMP, "New source %s on idx %u", inetAddrPrint(sourceAddr, debug_buf), sourceIdx);                      
    }       
...
L7_RC_t snoopPTinMembershipReportBlockProcess(snoopPTinL3InfoData_t* avlTreeEntry, L7_uint32 intIfNum, L7_uint32 clientIdx, L7_ushort16 noOfSources, L7_inet_addr_t* sourceList,L7_uint32 *noOfRecordsPtr, snoopPTinProxyGroup_t* groupPtr)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Group Address: %s,  IntIfnum:%d, clientIdx:%u, VLANID=%d noOfSources=%u", inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf),intIfNum,clientIdx, avlTreeEntry->snoopPTinL3InfoDataKey.vlanId,noOfSources);
...
snoopPTinProxyInterface_t* snoopPTinProxyInterfaceAdd(L7_uint32 vlanId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxyInterfaceEntryAdd()");
    return L7_NULLPTR;
  }

//LOG_DEBUG(LOG_CTX_IGMP, "Proxy Interface Added (vlanId:%u)",interfacePtr->key.vlanId);
  return interfacePtr;
}
...
snoopPTinProxyGroup_t* snoopPTinGroupRecordAdd(snoopPTinProxyInterface_t* interfacePtr,L7_uint8 recordType, L7_inet_addr_t* groupAddr,L7_BOOL* newEntryFlag,L7_uint8 robustnessVariable)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxyGroupEntryAdd()");
    return L7_NULLPTR;
  }
  ...
  if (*newEntryFlag==L7_TRUE)
  {
    if ((groupPtrAux=interfacePtr->groupRecord)==L7_NULLPTR || interfacePtr->numberOfGroupRecords==0)
    {
      ...
      {
        LOG_WARNING(LOG_CTX_IGMP, "Problems with Pointer or number of Group Records. Resetting both");
      }
      LOG_DEBUG(LOG_CTX_IGMP, "This is the first group record added to this interface");
    ...
    else
    {
      for (i=0;i<interfacePtr->numberOfGroupRecords && groupPtrAux!=L7_NULLPTR;i++)
      {
        if (groupPtrAux->nextGroupRecord==L7_NULLPTR)
        {
          ...
          {
            LOG_TRACE(LOG_CTX_IGMP, "There are %u group records in this interface (previous groupAddr:%s recordType:%u)",i+1,  inetAddrPrint(&groupPtrAux->key.groupAddr, debug_buf),groupPtrAux->recordType);
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "Group Record (vlanId: %u groupAddr: %s recordType: %u  noOfGroupRecords: %u)", groupPtr->interfacePtr->key.vlanId, inetAddrPrint(&groupPtr->key.groupAddr, debug_buf), groupPtr->recordType, interfacePtr->numberOfGroupRecords );
  }
  return groupPtr;
}
...
L7_RC_t snoopPTinGroupRecordSourcedAdd(snoopPTinProxyGroup_t* groupPtr,L7_inet_addr_t* sourceAddr,L7_uint8 robustnessVariable)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Existing Source :%s, restoring retransmission variable",inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf));    

    if (sourcePtr->key.sourceAddr.addr.ipv4.s_addr!=sourcePtr->key.sourceAddr.addr.ipv4.s_addr)
    {
      LOG_WARNING(LOG_CTX_IGMP, "The Source Addr %s stored in the AVL Tree is different from Input Source Address %s ",inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf),inetAddrPrint(sourceAddr, debug_buf2));
    }
    sourcePtr->retransmissions=PTIN_IGMP_DEFAULT_ROBUSTNESS;    
    sourcePtr->retransmissions=PTIN_IGMP_DEFAULT_ROBUSTNESS;
    if (sourcePtr->groupPtr != groupPtr)
    {
      LOG_WARNING(LOG_CTX_IGMP, "Fixing groupPtr");
  ...
  else if ((sourcePtr=snoopPTinProxySourceEntryFind(&groupAddrAux,sourceAddr,L7_MATCH_EXACT))!=L7_NULLPTR)
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP, "Resetting SourcePtr->NextSrc & SourcePtr->PreviousSrc");
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxySourceEntryFind()");
    return L7_FAILURE;
  }

  if (newEntry==L7_FALSE)
  {
    LOG_NOTICE(LOG_CTX_IGMP, "We are not adding this source to the group record");
    return L7_SUCCESS;   
  }
//else
//{
//  LOG_DEBUG(LOG_CTX_IGMP, "New Source Address added :%s)", inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf));
//}

  if ((sourcePtrAux=groupPtr->source)==L7_NULLPTR)
  {
    LOG_TRACE(LOG_CTX_IGMP, "This is the first source address in this group record");
  ...
  else
  {
    for (i=0;i<groupPtr->numberOfSources && sourcePtrAux!=L7_NULLPTR;i++)
    {
      if (sourcePtrAux->nextSource==L7_NULLPTR)
      {
        ...
        {
          LOG_TRACE(LOG_CTX_IGMP, "There are %u sources in this group record (previous sourceAddr:%s)",i+1,  inetAddrPrint(&sourcePtrAux->key.sourceAddr, debug_buf));
    ...
    {
      LOG_WARNING(LOG_CTX_IGMP, "Pointers are not properly configured %u<%u",i+1,groupPtr->numberOfSources-flagsourceAdded);
    }
  } 

  LOG_DEBUG(LOG_CTX_IGMP, "Group Record (vlanId: %u groupAddr: %s recordType: %u, sourceAddr: %s noOfSources: %u)", groupPtr->interfacePtr->key.vlanId, inetAddrPrint(&sourcePtr->key.groupPtr->key.groupAddr, debug_buf), groupPtr->recordType,  inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf2),groupPtr->numberOfSources);
...
L7_RC_t snoopPTinGroupRecordAddSourceList( L7_uint32  vlanId, L7_inet_addr_t *groupAddr, L7_uint8 recordType, L7_inet_addr_t   *sourceAddr, L7_uint32 sourceCnt,snoopPTinProxyGroup_t *groupPtr)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordInterfaceAdd()");
    return L7_FAILURE;
  }

  if ((groupPtr=snoopPTinGroupRecordAdd( interfacePtr,recordType,groupAddr))==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordGroupAdd()");
    return L7_FAILURE;
  }
...
L7_RC_t snoopPTinGroupRecordRemoveAll(snoopPTinProxyInterface_t* interfacePtr,L7_inet_addr_t* groupAddr)
{
  ...
  for (i=0;i<MGMD_GROUP_REPORT_TYPE_MAX;i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordDelete()");
      return L7_FAILURE;
    }
  } 
  return L7_SUCCESS;    
...
L7_RC_t snoopPTinProxyInterfaceRemove(snoopPTinProxyInterface_t* interfacePtr)
{
  ...
  if (snoop_ptin_proxytimer_isRunning(&interfacePtr->timer))
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to snoop_ptin_proxytimer_stop()");
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxyInterfaceEntryDelete()");      
    return L7_FAILURE;
  }
  LOG_DEBUG(LOG_CTX_IGMP, "Root vlan removed");
  return L7_SUCCESS;
}
...
L7_RC_t snoopPTinGroupRecordRemove(snoopPTinProxyInterface_t* interfacePtr, L7_inet_addr_t* groupAddr,L7_uint8 recordType)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Failed to snoopPTinProxyGroupEntryFind()");
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordSourceDeleteAll()");
//  return L7_FAILURE;
  }
...
//    return L7_FAILURE;
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxyGroupEntryDelete()");      
    return L7_FAILURE;
  }
  LOG_DEBUG(LOG_CTX_IGMP, "Group Record Removed");
...
//  return(snoopPTinProxyInterfaceRemove(interfacePtr));
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Number of Group Records was already equal to zero");   
    interfacePtr->groupRecord=L7_NULLPTR;
  }
...
//char                debug_buf[IPV6_DISP_ADDR_LEN]={},debug_buf2[IPV6_DISP_ADDR_LEN]={};
  ...
  {
    LOG_NOTICE(LOG_CTX_IGMP, "Group Record Empty");
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Problems with sourcePrt %u<%u",i+1,groupPtr->numberOfSources);
  }
...
//  sourcePtrAux->groupPtr=L7_NULLPTR;
    ...
    if (snoopPTinProxySourceEntryFind(&groupAddrAux,&sourcePtrAux->key.sourceAddr,L7_MATCH_EXACT)==L7_NULLPTR)
    {
      ...
      LOG_NOTICE(LOG_CTX_IGMP, "Source Address Saved on Group Record (vlanId: %u groupAddr:%s sourceAddr:%s)",groupPtr->key.vlanId, inetAddrPrint(&sourcePtrAux->key.groupAddr, debug_buf), inetAddrPrint(&sourcePtrAux->key.sourceAddr, debug_buf2));    
    ...
    else
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinProxySourceEntryDelete()");      
        return L7_FAILURE;
      }
      LOG_DEBUG(LOG_CTX_IGMP, "Source Address Removed from Group Record (vlanId: %u groupAddr:%s recordType:%u sourceAddr:%s)",groupPtr->key.vlanId, inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->key.recordType, inetAddrPrint(&sourcePtrAux->key.sourceAddr, debug_buf2));    
      flagSourceRemoved=L7_TRUE;
    }
#else
    if (snoopPTinProxySourceEntryDelete(groupPtr,&sourcePtrAux->key.sourceAddr)!=L7_SUCCESS)
...
//LOG_DEBUG(LOG_CTX_IGMP, "Source Address Removed from Group Record (vlanId: %u groupAddr:%s recordType:%u sourceAddr:%s)",groupPtr->key.vlanId, inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->key.recordType, inetAddrPrint(&sourcePtrAux->key.sourceAddr, debug_buf2));
    LOG_DEBUG(LOG_CTX_IGMP, "Source Address Removed");
    flagSourceRemoved=L7_TRUE;
#endif
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Problems with groupPtr %u<%u",i,groupPtr->numberOfSources);
  }
...
L7_RC_t snoopPTinGroupRecordSourceRemove(snoopPTinProxyGroup_t*   groupPtr, L7_inet_addr_t *sourceAddr)
{
  ...
  {
    LOG_NOTICE(LOG_CTX_IGMP, "Failed to snoopPTinProxySourceEntryFind()");
    return L7_SUCCESS;
  }
  ...
  for (i=0;i<groupPtr->numberOfSources && sourcePtrTmp !=L7_NULLPTR;i++)
  {
    if (sourcePtrTmp ==sourcePtr)
    {
      ...
      {
        LOG_NOTICE(LOG_CTX_IGMP, "First source address of this group record");
        groupPtr->source=sourcePtrTmp->nextSource;
      }
      else
      {
        LOG_NOTICE(LOG_CTX_IGMP, "This is not the first source address of this group record");                
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "Unable to find Source Address within Group Record (vlanId: %u groupAddr:%s recordType:%u, sourceAddr:%s)",groupPtr->interfacePtr->key.vlanId, inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->recordType,  inetAddrPrint(sourceAddr, debug_buf2));
    return L7_SUCCESS;
  }
...
//Should we change also the VLAN ID to MAX VLANs?
//  groupPtr->key.vlanId=L7_MAX_VLANS;
    LOG_DEBUG(LOG_CTX_IGMP, "Source Address Saved on Group Record (vlanId: %u groupAddr:%s sourceAddr:%s)",groupPtr->interfacePtr->key.vlanId,inetAddrPrint(&sourcePtr->key.groupAddr, debug_buf), inetAddrPrint(&sourcePtr->key.sourceAddr, debug_buf2));    
  ...
  else
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "Source Address Removed from Group Record (vlanId: %u groupAddr:%s recordType:%u, sourceAddr:%s)",groupPtr->key.vlanId, inetAddrPrint(&groupPtr->key.groupAddr, debug_buf),groupPtr->key.recordType,  inetAddrPrint(sourceAddr, debug_buf2));    
  }
#else
  if (snoopPTinProxySourceEntryDelete(groupPtr,sourceAddr)!=L7_SUCCESS)
  {
...
//ptin_IgmpProxyCfg_t igmpCfg;
  ...
  *timeout=selectedDelay;
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinPendingReport2GeneralQuery()");
    return L7_NULLPTR;
  }
  ...
  if (pendingReport==L7_TRUE)
  {
    if (timeLeft<selectedDelay)
    {
      ...
      LOG_DEBUG(LOG_CTX_IGMP,"There is a Pending Response to a General Query General with timeleft smaller then selected delay  %u<%u: Packet silently ignored",timeLeft,selectedDelay);               
      *sendReport=L7_FALSE; 
      return L7_NULLPTR;
    }
    else
    {
      LOG_TRACE(LOG_CTX_IGMP,"There is a Pending Response to a General Query General with timeleft higher then selected delay  %u>%u",timeLeft,selectedDelay);
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoop_ptin_proxytimer_start()");
        *sendReport=L7_FALSE;
        return L7_NULLPTR;
      }
      *sendReport=L7_FALSE;
 ...
 selected delay. Any previously pending response to a General*/
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid snoopPTinActiveGroups()");
    return L7_NULLPTR;
  }
...
needs to be scheduled*/
  if (pendingReport==L7_TRUE && timeLeft<selectedDelay)
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP,"There is a Pending Response to a General Query General with timeleft smaller then selected delay  %d<%d: Packet silently ignored",timeLeft,selectedDelay);                  
    return L7_NULLPTR;
  }
  else if (pendingReport==L7_TRUE)
  {
    LOG_TRACE(LOG_CTX_IGMP,"There is a Pending Response to a General Query General with timeleft higher then selected delay  %d>%d",timeLeft,selectedDelay);           
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinPendingReport2GroupQuery()");
    return L7_NULLPTR;
  }
...
pending report and the selected delay.*/
  if (pendingReport==L7_TRUE && timeLeft<selectedDelay)
  {
    ...
    {
      LOG_TRACE(LOG_CTX_IGMP,"There is a Pending Response to a Group Specific Query with timeleft smaller then selected delay  %d<%d: Packet silently ignored",timeLeft,selectedDelay);     
      *sendReport=L7_FALSE;
//    groupPtr=L7_NULLPTR;
      return L7_NULLPTR;
    }
    else
    {
      ...
      *sendReport=L7_TRUE;
      if (snoopPTinGroupRecordSourceRemoveAll(groupPtr)!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinGroupRecordSourceRemoveAll()");
        return L7_NULLPTR;
      }
    }
  }
...
//{
  ...
  while (noOfSources > 0 && sourceAddr !=L7_NULLPTR)
  {
    ...
    else if (rc==L7_SUCCESS && snoopPTinZeroClients(avlTreeEntry->interfaces[intIfNum].sources[sourceIdx].clients)==L7_ALREADY_CONFIGURED)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed snoopPTinGroupRecordSourcedAdd()");
        return L7_NULLPTR;        
      }
      //    else
      //    {
    ...
    {
      LOG_WARNING(LOG_CTX_IGMP, "Inexisting Source %s on idx %u", inetAddrPrint(sourceAddr, debug_buf), sourceIdx);           
    }
    --noOfSources;
    ++sourceAddr;
  }
...
//*pendingReport=L7_FALSE;
  ...
  if ((interfacePtr=snoopPTinProxyInterfaceEntryAdd(vlanId,&newEntry))==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP,"Failed to snoopPTinProxyInterfaceEntryFind()");
    return L7_NULLPTR;
  }
  ...
  if ((*pendingReport=snoop_ptin_proxytimer_isRunning(&interfacePtr->timer))==L7_TRUE && interfacePtr->timer.reportType==L7_IGMP_MEMBERSHIP_QUERY)
  {
    *timeout=snoop_ptin_proxytimer_timeleft(&interfacePtr->timer);    
    LOG_NOTICE(LOG_CTX_IGMP,"Pending report to general query exists (timeout %u)",*timeout);     
  }
  else
  {
    LOG_TRACE(LOG_CTX_IGMP,"No pending report to general query exists");     
    *pendingReport=L7_FALSE;
  }
...
//Initialize Output Variables
  ...
  *pendingReport=L7_FALSE;  
  ...
  if ((groupPtr=snoopPTinProxyGroupEntryFind(interfacePtr->key.vlanId,&avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr,recordType, L7_MATCH_EXACT))==L7_NULLPTR)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Failed snoopPTinGroupRecordGroupAdd()");
      return L7_NULLPTR;
    }
  }
  else
  {
        ...
        (groupPtr->timer.reportType==L7_IGMP_MEMBERSHIP_GROUP_SPECIFIC_QUERY || groupPtr->timer.reportType==L7_IGMP_MEMBERSHIP_GROUP_AND_SOURCE_SCPECIFC_QUERY))
    {
      *timeout=snoop_ptin_proxytimer_timeleft(&groupPtr->timer);            
      LOG_NOTICE(LOG_CTX_IGMP,"Pending report to group query exists (timeout %u)",*timeout);   
    }
    else
    {
      *pendingReport=L7_FALSE;
      LOG_TRACE(LOG_CTX_IGMP,"No Pending report to group query exists");   
    }
  }
  return groupPtr;
}
...
//char                debug_buf[IPV6_DISP_ADDR_LEN]={};
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid VLAN Id");
    return L7_ERROR;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to snoopEBGet()");
    return L7_ERROR;
  }

  *activeGroups=L7_FALSE;
  LOG_NOTICE(LOG_CTX_IGMP, "Verifying if there is any active group on vlanId:%u",vlanId);
...
/* Run all cells in AVL tree */    
  ...
  while ( ( avlTreeEntry = avlSearchLVL7(&pSnoopEB->snoopPTinL3AvlTree, &avlTreeKey, L7_MATCH_GETNEXT) ) != L7_NULLPTR )
  {
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "We have at least one active group within this vlan Id:%u",vlanId);
...
L7_RC_t snoopPTinAddStaticGroup(L7_uint32 vlanId, L7_uint32 intIfNum,L7_inet_addr_t *groupAddr,L7_uint32 noOfSources,L7_inet_addr_t* sourceAddr)
{
  ...
  if (L7_NULLPTR == (snoopEntry = snoopPTinL3EntryFind(vlanId, groupAddr, L7_MATCH_EXACT)))
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Add L3 Entry");
      return L7_FAILURE;
    }
    else
    {
      LOG_TRACE(LOG_CTX_IGMP, "snoopPTinL3EntryAdd(%u,%u)",groupAddr,vlanId);
    }
    if (L7_NULLPTR == (snoopEntry = snoopPTinL3EntryFind(vlanId, groupAddr, L7_MATCH_EXACT)))
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Add&Find L3 Entry");
      return L7_FAILURE;
    }
  }
  else
  ...
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Initializing interface idx: %u", intIfNum);
  ...
  if (interfaceProvided==L7_TRUE)
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP, "Initializing root interface idx: %u", SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP, "Leaf interface: Failed to  snoopPTinAddStaticSource()");
    return L7_FAILURE;
  }
  ...
  if (interfaceProvided==L7_TRUE)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP, "Root Interface: Failed to snoopPTinAddStaticSource()");
      return L7_FAILURE;
    }
  ...
  if (noOfSources==0 || noOfRecords>0)
  {
    ...
    noOfSources=1;
    LOG_TRACE(LOG_CTX_IGMP, "Schedule Membership Report Message");
  ...
  else
  {
    if (newEntry==L7_TRUE)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordRemove()");
        return L7_ERROR;
      }
    }
  }
...
/* Get proxy configurations */
  ...
  if (intIfNum==SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM)
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Proxy - Group Address: %s,  IntIfnum:%d, VLANID=%d", inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf),intIfNum, avlTreeEntry->snoopPTinL3InfoDataKey.vlanId);    
  }
  else
    LOG_DEBUG(LOG_CTX_IGMP, "Group Address: %s,  IntIfnum:%d, VLANID=%d", inetAddrPrint(&(avlTreeEntry->snoopPTinL3InfoDataKey.mcastGroupAddr), debug_buf),intIfNum, avlTreeEntry->snoopPTinL3InfoDataKey.vlanId);
...
/* Add new source */
      ...
      if (L7_SUCCESS == snoopPTinProxySourceAdd(avlTreeEntry,intIfNum, sourceAddr))
      {
        if (L7_SUCCESS != snoopPTinGroupRecordSourcedAdd(groupPtr,sourceAddr,igmpCfg.host.robustness))
        {
          LOG_ERR(LOG_CTX_IGMP, "Failed to snoopPTinGroupRecordSourceListAdd()");
          return L7_FAILURE;
        }
        if (noOfRecords==0)
          noOfRecords=1;
...
/* Create new entry in AVL tree for VLAN+IP if necessary */
  if (L7_NULLPTR == (snoopEntry = snoopPTinL3EntryFind(vlanId, groupAddr, L7_MATCH_EXACT)))
  {
    LOG_WARNING(LOG_CTX_IGMP, "Failed to snoopPTinL3EntryFind()");
    return L7_SUCCESS;       
  }
...
/* If root interface is  used*/
  if (snoopEntry->interfaces[SNOOP_PTIN_PROXY_ROOT_INTERFACE_NUM].active == L7_TRUE)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP, "Leaf interface: Failed to  snoopPTinRemoveStaticSource()");
      return L7_FAILURE;
    }
...
/* If no sources remain and no clients remain, remove group*/
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP,"Removing interface");
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_IGMP, "Root Interface: Failed to snoopPTinInterfaceRemove()");
        return L7_FAILURE;
      }
    }
    else
...
/* If no sources remain and no clients remain, remove group*/
    if (snoopEntry->interfaces[intIfNum].numberOfClients==0 /*&& snoopEntry->interfaces[intIfNum].numberOfSources == 0*/)
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_IGMP, "Leaf Interface: Failed to snoopPTinInterfaceRemove()");
        return L7_FAILURE;
      }
    }
    else
