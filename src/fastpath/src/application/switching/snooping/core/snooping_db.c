...
L7_RC_t snoopL3GroupIntfAdd(L7_uint32 evcId, L7_uint16 vlanId, L7_inet_addr_t* groupAddr, L7_inet_addr_t* sourceAddr, L7_uint32 intIfNum, L7_BOOL isStatic, L7_BOOL isProtection)
{
  ...
  if (evcId == 0 || vlanId < PTIN_VLAN_MIN || vlanId > PTIN_VLAN_MAX || groupAddr==L7_NULLPTR || groupAddr->family!=L7_AF_INET || sourceAddr == L7_NULLPTR || sourceAddr->family != L7_AF_INET
      || intIfNum==0 || intIfNum >= PTIN_SYSTEM_MAXINTERFACES_PER_GROUP)
  {   
    PT_LOG_ERR(LOG_CTX_IGMP,"Invalid arguments [evcId:%u vlanId:%u groupAddr:%p sourceAddr:%p intIfNum:%u isStatic:%u isProtection:%u", evcId, vlanId, groupAddr, sourceAddr, intIfNum, isStatic, isProtection);
    return L7_FAILURE;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Input Arguments [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isStatic:%u isProtection:%u", evcId, vlanId, intIfNum, 
  ...
  else
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Invalid Interface Mask Pointer [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isStatic:%u isProtection:%u pChannelIntfMask:%p", evcId, vlanId, intIfNum, 
    ...
    else 
    {
      ...
      {        
        ...
        PT_LOG_WARN(LOG_CTX_IGMP, "Failed to Add Channel Table is Full [rc:%u evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isStatic:%u isProtection:%u]", 
      ...
      {
        //Error
        PT_LOG_WARN(LOG_CTX_IGMP, "Failed to Add Channel [rc:%u evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isStatic:%u isProtection:%u]", 
                    rc, evcId, vlanId, intIfNum, groupAddrStr, sourceAddrStr, isStatic, isProtection);
        return rc;
      }
    }
  ...
  if (rc != L7_SUCCESS)
  {    
    PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Add Channel to HW IPMC Table [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s multicast_group:0x%08x", evcId, vlanId, intIfNum, 
...
{  
  ...
  if (evcId == 0 || vlanId < PTIN_VLAN_MIN || vlanId > PTIN_VLAN_MAX || groupAddr==L7_NULLPTR || groupAddr->family!=L7_AF_INET || sourceAddr == L7_NULLPTR || sourceAddr->family != L7_AF_INET
      || intIfNum==0 || intIfNum >= PTIN_SYSTEM_MAXINTERFACES_PER_GROUP)
  {   
    PT_LOG_ERR(LOG_CTX_IGMP,"Invalid arguments [evcId:%u vlanId:%u groupAddr:%p sourceAddr:%p intIfNum:%u isProtection:%u", evcId, vlanId, groupAddr, sourceAddr, intIfNum, isProtection);
    return L7_FAILURE;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "Input Arguments [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isProtection:%u", evcId, vlanId, intIfNum, 
  ...
  {
    PT_LOG_WARN(LOG_CTX_IGMP, "This channel does not exist [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s isProtection:%u multicastGroup:0x%08x]", evcId, vlanId, intIfNum, 
...
//else
  ...
  if ( --pChannelEntry->noOfInterfaces == 0 )
  {
    ...
    if (rc != L7_SUCCESS)
    {    
      PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Remove Channel from HW IPMC Table [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s multicast_group:0x%08x", evcId, vlanId, intIfNum, 
          inetAddrPrint(&dtl_ipmc.mc_ip_addr, groupAddrStr), inetAddrPrint(&dtl_ipmc.s_ip_addr, sourceAddrStr), dtl_ipmc.group_index);      
      return rc;
    }
  ...
  if (rc != L7_SUCCESS)
  {    
    PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Update Channel on the HW IPMC Table [evcId:%u vlanId:%u intIfNum:%u groupAddr:%s sourceAddr:%s multicast_group:0x%08x", evcId, vlanId, intIfNum, 
...
{ 
  ...
  {
    /*some error in avl tree addition*/
    PT_LOG_ERR(LOG_CTX_IGMP, "Failed to add entry [vlanId:%u groupAddr:%s sourceAddr:%s]",
                  (*snoopEntry)->snoopChannelInfoDataKey.vlanId, inetAddrPrint(&(*snoopEntry)->snoopChannelInfoDataKey.groupAddr, groupAddrStr), inetAddrPrint(&(*snoopEntry)->snoopChannelInfoDataKey.sourceAddr, sourceAddrStr));              
    return L7_TABLE_IS_FULL;
  }
