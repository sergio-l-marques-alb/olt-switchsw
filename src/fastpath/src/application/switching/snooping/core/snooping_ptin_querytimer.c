...
L7_RC_t snoop_ptin_querytimer_init(void)
{
   ...
   {
     LOG_FATAL(LOG_CTX_CNFGR, "Could not create task ptin_igmp_querytimer_task");
     return L7_FAILURE;
   }
   if (osapiWaitForTaskInit (L7_PTIN_IGMP_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
   {
     LOG_FATAL(LOG_CTX_CNFGR,"Unable to initialize ptin_igmp_querytimer_task");
     return(L7_FAILURE);
   }
...
void cbtimerCallback(L7_APP_TMR_CTRL_BLK_t timerCtrlBlk, void* ptrData)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Querytimer timer tick send failed");
    return;
  }
}
...
void timerCallback(void *param)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Querytimer expired (group:%s vlan:%u ifId:%u)", 
...
L7_RC_t snoop_ptin_querytimer_start(snoopPTinL3Querytimer_t *pTimer, L7_uint16 timeout, snoopPTinL3InfoData_t* groupData, L7_uint32 interfaceIdx)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Starting querytimer (group:%s vlan:%u ifIdx:%u)",
  ...
  if (pTimer->timer == NULL)
  {
    ...
    pTimer->timerHandle = 0;
    osapiSemaGive(timerSem);
    LOG_ERR(LOG_CTX_IGMP,"Could not start the querytimer.");
    return L7_FAILURE;
  }
...
L7_RC_t snoop_ptin_querytimer_stop(snoopPTinL3Querytimer_t *pTimer)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Stopping querytimer (group:%s vlan:%u ifIdx:%u)",
