...
L7_RC_t ptin_snoop_activeChannels_get(L7_uint16 vlanId,L7_uint32 intIfNum, L7_uint16 client_index, ptin_igmpChannelInfo_t *channels, L7_uint16 *nChannels)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid IGMP version [clientVersion=%u]", igmpCfg.clientVersion);
    return L7_FAILURE;
  }
  LOG_NOTICE(LOG_CTX_IGMP, "We've read num_channels:%u",*nChannels);
  return L7_SUCCESS;
}
...
{  
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Parameters");
    return L7_FAILURE;
  }

  if (groupAddr->family!=L7_AF_INET)
  {
    LOG_ERR(LOG_CTX_IGMP,"Only IPv4 is supported!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_TRACE(LOG_CTX_IGMP,"IGMPv3 Clients List Get!");
    ...
    {
      LOG_WARNING(LOG_CTX_IGMP,"Channel %u.%u.%u.%u does not exist for vlan %u",
    ...
    else
    {
      ...
      *number_of_clients=0;
        ...
        {    
          for (idx=0;idx<PTIN_SYSTEM_IGMP_CLIENT_BITMAP_SIZE ; idx++)
          {
            if (snoopEntryV3->interfaces[intIfNum].clients[idx]!=0)
            {
              ...
              LOG_NOTICE(LOG_CTX_IGMP,"At least one client!");
            }            
          } 
          *number_of_clients+=snoopEntryV3->interfaces[intIfNum].numberOfClients;          
        }
      }
      LOG_NOTICE(LOG_CTX_IGMP,"We have read number_of_clients:%u",*number_of_clients);
      ...
      {
        LOG_WARNING(LOG_CTX_IGMP,"Something must had went wrong, since the variable number of clients is different from what we have read on the client bitmap!");
      }
    }
  }
  else
...
L7_RC_t ptin_snoop_client_remove(L7_uint16 sVlanId, L7_uint16 client_index, L7_uint32 intIfNum)
{
  ...
  while ((snoopEntry=snoopEntryFind(macAddr,vlanId,L7_AF_INET,L7_MATCH_GETNEXT))!=L7_NULLPTR)
  {
    ...
    for (channel_index=0; channel_index<SNOOP_MAX_CHANNELS_PER_SNOOP_ENTRY; channel_index++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Error with snoop_client_remove_procedure");
        rc = L7_FAILURE;
      }
    }
  }
...
{  
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid channel");
    return L7_FAILURE;
  }
  ...
  if (igmp_network_version==3)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Error getting client interfaces of vlan %u",vlanId);
      return L7_SUCCESS;
    }  
    ...
    for (intIfNum = 1; intIfNum <= L7_MAX_INTERFACE_COUNT; intIfNum++)
    {
      if ( (L7_INTF_ISMASKBITSET(mcastClientAttached,intIfNum)) )
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Error adding static channel");
          return L7_FAILURE;
        }
      }
    }    
...
{  
  ...
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error removing channel");
      return L7_FAILURE;
    }
  }
...
L7_RC_t ptin_snoop_channel_removeAll(L7_uint16 sVlanId)
{
  ...
  while ((snoopEntry=snoopEntryFind(macAddr,vlanId,L7_AF_INET,L7_MATCH_GETNEXT))!=L7_NULLPTR)
  {
    ...
    for (channel_idx=0; channel_idx<SNOOP_MAX_CHANNELS_PER_SNOOP_ENTRY; channel_idx++)
    {
      ...
      {
        LOG_TRACE(LOG_CTX_IGMP,"Channel removed from vlan %u",vlanId);
      }
      else
      {
        LOG_ERR(LOG_CTX_IGMP,"Error removing channel from vlan %u",vlanId);
        rc = L7_FAILURE;
      }
    }
  }
