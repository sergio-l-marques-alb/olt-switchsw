...
extern void garpIntfEventActiveProcess(L7_uint32 intIfNum);
...
void garpIntfStartupCallback(NIM_STARTUP_PHASE_t startupPhase)
{
  ...
  if (garpQueue != L7_NULL)
  {
    ...
    msg.msgId = G_NIM_STARTUP;
    msg.msgData.startup.startupPhase = startupPhase;

    garpTraceQueueMsgSend(&msg);

    rc = osapiMessageSend(garpQueue, &msg, sizeof(GarpPacket),
                          L7_WAIT_FOREVER, L7_MSG_PRIORITY_NORM);

    if (rc != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_GARP_COMPONENT_ID,
              "Failed to send NIM startup phase %d to Garp thread",
              startupPhase);
    }
    else
    {
...
* @returns  L7_SUCCESS
*
* @notes    Garp has no configuration at this point
*
* @end
*********************************************************************/
L7_RC_t garpNimCreateStartup(void)
{
  L7_RC_t          rc;
  L7_uint32        intIfNum;
  ...
  while (rc == L7_SUCCESS)
  {
        ...
        (garpIsValidIntfType(sysIntfType) == L7_TRUE))
    {
      ...
      if (garpIntfCreate(intIfNum) == L7_SUCCESS)
      {
  ...
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_CREATE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_DELETE);
  nimRegisterIntfEvents(L7_GARP_COMPONENT_ID, GARPCB->nimEventMask);

  nimStartupEventDone(L7_GARP_COMPONENT_ID);

  GARPCB->createStartupDone = L7_TRUE;

  L7_LOGF(L7_LOG_SEVERITY_DEBUG, L7_GARP_COMPONENT_ID,
          "Garp Create Startup done");
...
* @end
*********************************************************************/
L7_RC_t garpNimActivateStartup(void)
{
  L7_RC_t          rc;
  L7_uint32        intIfNum, activeState;
  L7_INTF_STATES_t intIfState;
  L7_INTF_TYPES_t  sysIntfType;
  L7_uint32        routingEnabled;
  L7_BOOL          lagAcquired, probeSetup;
  ...
  while (rc == L7_SUCCESS)
  {
    ...
    switch (intIfState)
    {
      ...
      case L7_INTF_ATTACHED:
        ...
        probeSetup = L7_FALSE;
        routingEnabled = L7_DISABLE;

        garpApplyIntfConfigData(intIfNum, GARP_APPLY_CONFIG_NSF);
        ...
        */
        ...
        {
          /*
          Unlike with the normal ATTACH event, we don't need to worry about
          the port state having been previously acquired, so no need to slow
          down startup by disabling GARP
          */

          /* update acquired list */
          if (probeSetup == L7_TRUE)
          {
              COMPONENT_ACQ_SETMASKBIT(garpInfo.acquiredList[intIfNum], L7_PORT_MIRROR_COMPONENT_ID);
          }

          if (lagAcquired == L7_TRUE)
          {
              COMPONENT_ACQ_SETMASKBIT(garpInfo.acquiredList[intIfNum], L7_DOT3AD_COMPONENT_ID);
          }

          if (routingEnabled == L7_ENABLE)
          {
              COMPONENT_ACQ_SETMASKBIT(garpInfo.acquiredList[intIfNum], L7_IP_MAP_COMPONENT_ID);
          }
        }
        else
        {
          ...
          COMPONENT_ACQ_CLRMASKBIT(garpInfo.acquiredList[intIfNum], L7_PORT_MIRROR_COMPONENT_ID);
          COMPONENT_ACQ_CLRMASKBIT(garpInfo.acquiredList[intIfNum], L7_DOT3AD_COMPONENT_ID);
          COMPONENT_ACQ_CLRMASKBIT(garpInfo.acquiredList[intIfNum], L7_IP_MAP_COMPONENT_ID);
  ...
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_INACTIVE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_ACTIVE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_CREATE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_ATTACH);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_DELETE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_DETACH);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_LAG_ACQUIRE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_LAG_RELEASE);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_PROBE_SETUP);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_PROBE_TEARDOWN);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_PORT_ROUTING_ENABLED);
  PORTEVENT_SETMASKBIT(GARPCB->nimEventMask, L7_PORT_ROUTING_DISABLED);
...
L7_RC_t garpVlanChangeCallback(dot1qNotifyData_t *vlanDataCopy, L7_uint32 intIfNum, L7_uint32 event)
{
  ...
  if (event == VLAN_DELETE_PENDING_NOTIFY || event == VLAN_ADD_NOTIFY)
  {
      if (vlanData.numVlans == 1)
      {
        ...
        if (rc1 == L7_SUCCESS && (vlanType == L7_DOT1Q_ADMIN || vlanType == L7_DOT1Q_DOT1X_REGISTERED))
        {
      ...
      case VLAN_RESTORE_NOTIFY:
          ...
          if (osapiMessageSend(garpQueue, (void*)&msg, sizeof(GarpPacket), L7_WAIT_FOREVER, L7_MSG_PRIORITY_NORM) != L7_SUCCESS)
          {
            L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_GARP_COMPONENT_ID,
                    "GARP VLAN callback: Failed to send message for intf %s, event %d\n",
                    ifName, event);
            return L7_FAILURE;
          }
          else
...
L7_uint32 garpDot1sChangeCallBack(L7_uint32 mstID, L7_uint32 intIfNum, L7_uint32 event)
{
  ...
  switch (event)
  {
    ...
    case L7_DOT1S_EVENT_INSTANCE_DELETE:
        ...
        {
            L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_GARP_COMPONENT_ID,
                    "garpDot1sChangeCallBack: QUEUE SEND FAILURE: %s: mstID = %d, intf = %s, event = %d."
                    " The garpQueue is full, logs specifics of the message content like"
                    " internal interface number, type of message etc.",
                    "G_DOT1S_NOTIFY", mstID, ifName, event);
        }
        else
        {
            osapiSemaGive(garpInfo.msgQSema);
