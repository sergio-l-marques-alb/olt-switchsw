...
void daiDebugStats(void)
{
  ...
  printf("\n%-50s  %12u", 
         "Number of ARP messages dropped as control plane is not ready", 
...
L7_RC_t daiTraceWrite(L7_uchar8 *daiTrace)
{
  ...
  osapiSnprintf(debugMsg, DAI_MAX_TRACE_LEN, "\n%d.%03d (%d,%d) %s",
                secs, msecs, eventQLen, pktQLen, daiTrace);
  printf("%s", debugMsg);
  return L7_SUCCESS;
}
...
void daiTraceFlagsDump(void)
{
  ...
  printf("Trace NSF events      (0x80) : %s\n\n",
         (((daiCfgData->daiTraceFlags) & DAI_TRACE_NSF) ? "Enabled" : "Disabled"));
}
