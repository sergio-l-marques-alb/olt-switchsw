...
L7_RC_t daiVlanArpAclClear(L7_uint32 vlanId)
{
  ...
  /* If an ACL name is associated to this VLAN */
  if (_daiVlanArpAclGet(vlanId, aclTemp))
  {
    _daiVlanArpAclClear(vlanId);
    daiConfigDataChange();
  }
...
* @end
*********************************************************************/
L7_RC_t arpAclRuleAdd(L7_uchar8 *aclName, L7_uint32 ipAddr, L7_enetMacAddr_t *macAddr)
{
  ...
  if((macAddr->addr[0] & 1) == 1)
    return L7_ERROR;

  if (osapiWriteLockTake(daiCfgRWLock, L7_WAIT_FOREVER) != L7_SUCCESS)
    return L7_FAILURE;

  if((rc = _arpAclRuleAdd(aclName, ipAddr, macAddr->addr)) == L7_SUCCESS)
  {
    daiConfigDataChange();
  }
  osapiWriteLockGive(daiCfgRWLock);
...
* @end
*********************************************************************/
L7_RC_t arpAclRuleDelete(L7_uchar8 *aclName, L7_uint32 ipAddr, L7_enetMacAddr_t *macAddr)
{
  L7_RC_t rc = L7_FAILURE;
  ...
  if((rc = _arpAclRuleDelete(aclName, ipAddr, macAddr->addr)) == L7_SUCCESS)
  {
    daiConfigDataChange();
  }
...
* @end
*********************************************************************/
L7_RC_t arpAclRuleGet(L7_uchar8 *aclName, L7_uint32 ipAddr, L7_enetMacAddr_t *macAddr)
{
  L7_RC_t rc = L7_FAILURE;
  ...
  rc = _arpAclRuleGet(aclName, ipAddr, macAddr->addr);
...
* @end
*********************************************************************/
L7_RC_t arpAclRuleNextGet(L7_uchar8 *aclNameIn, L7_uint32 ipAddrIn, L7_enetMacAddr_t *macAddrIn,
                          L7_uchar8 *aclNameOut, L7_uint32 *ipAddrOut, L7_enetMacAddr_t *macAddrOut)
{
  ...
  rc = _arpAclRuleNextGet(aclNameIn, ipAddrIn, macAddrIn->addr,
                          aclNameOut, ipAddrOut, macAddrOut->addr);
                               ...
                               L7_uint32 *ipAddrOut, L7_enetMacAddr_t *macAddrOut)
{
  ...
  rc = _arpAclRuleInAclNextGet(aclName, ipAddrIn, macAddrIn->addr,
