...
L7_RC_t pmlInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if (pmlQueue == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_PORT_MACLOCK_COMPONENT_ID,
            "pmlInit: msgQueue creation error.\n");
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  if (nimRegisterIntfChange(L7_PORT_MACLOCK_COMPONENT_ID,
                            pmlIntfChangeCallback,
                            pmlStartupNotifyCallback,
                            NIM_STARTUP_PRIO_PML) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_PORT_MACLOCK_COMPONENT_ID,
            "Port MAC Locking: Unable to register with NIM\n");
                                   ...
                                   L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  pmlWarmStart = warmRestart;

  /* Clear the NIM port event mask until ready to receive events */
  memset(&pmlPortEventMask_g, 0, sizeof(pmlPortEventMask_g));
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                if ((pmlRC = pmlCnfgrInitPhase3Process( warmRestart, &response, &reason )) != L7_SUCCESS)
                {
