...
L7_uint32 dot3adIntfChangeCallBackProcess(NIM_EVENT_COMPLETE_INFO_t eventInfo)
{
  ...
  LOG_PT_TRACE(LOG_CTX_EVENTS, "dot3adIntfChangeCallBackProcess: event=%u, intIfNum=%u", event, intIfNum);
  ...
  if (dot3adIsValidIntf(intIfNum) == L7_FALSE)
  {
     ...
     */
    rc = L7_SUCCESS;
    LOG_PT_TRACE(LOG_CTX_EVENTS, "Not Valid interface: intIfNum=%u", intIfNum);
  }
  else
  {
    ...
    {
	 rc = L7_SUCCESS;
     LOG_PT_TRACE(LOG_CTX_EVENTS, "Error: intIfNum=%u", intIfNum);
    }
    ...
    if (tempRc == L7_SUCCESS)
    {
    LOG_PT_TRACE(LOG_CTX_EVENTS, "switch: event=%u, intIfNum=%u", event, intIfNum);
  ...
  LOG_PT_TRACE(LOG_CTX_EVENTS, "finishing: rc=%u, performNimCallback=%u", rc, performNimCallback);

  if (performNimCallback == L7_TRUE)
  {
    LOG_PT_TRACE(LOG_CTX_EVENTS, "Going to call nimEventStatusCallback: intIfNum=%u", intIfNum);
    ...
    LOG_PT_TRACE(LOG_CTX_EVENTS, "nimEventStatusCallback called: intIfNum=%u", intIfNum);
  }
  else
  {
    LOG_PT_TRACE(LOG_CTX_EVENTS, "nimEventStatusCallback NOT called: intIfNum=%u", intIfNum);
  }
...
L7_RC_t dot3adIntfChangeCallBack(L7_uint32 intIfNum, L7_uint32 event,NIM_CORRELATOR_t correlator)
{
  ...
  LOG_PT_TRACE(LOG_CTX_EVENTS, "dot3adIntfChangeCallBack: event=%u, intIfNum=%u", event, intIfNum);
      ...
      event != L7_SPEED_CHANGE)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_EVENTS, "Error: event=%u, intIfNum=%u", event, intIfNum);
  ...
  LOG_PT_TRACE(LOG_CTX_EVENTS, "Going to send message: event=%u, intIfNum=%u, msg_count=%d", event, intIfNum, numMsg);

  rc = osapiMessageSend(dot3ad_queue, &msg, (L7_uint32)DOT3AD_MSG_SIZE, L7_NO_WAIT, L7_MSG_PRIORITY_NORM);

  numMsg = -1;
  osapiMsgQueueGetNumMsgs(dot3ad_queue, &numMsg);
  LOG_PT_TRACE(LOG_CTX_EVENTS, "Message sent: msg_count=%d, rc=%d", numMsg, rc);
...
void dot3ad_lac_task()
{
  ...
  for (;;)
  {
    ...
    {
      LOG_PT_TRACE(LOG_CTX_EVENTS, "Message received: event=%u, intIfNum=%u", msg.event, msg.intfData.nimInfo.intIfNum);
    }
...
****/
  ...
  case lacpDetached:
    ...
    case lacNimIntfChange:
      LOG_PT_TRACE(LOG_CTX_EVENTS, "Dispatching message-dot3adIntfChangeCallBackProcess: event=%u, intIfNum=%u", msg.intfData.nimInfo.event, msg.intfData.nimInfo.intIfNum);
