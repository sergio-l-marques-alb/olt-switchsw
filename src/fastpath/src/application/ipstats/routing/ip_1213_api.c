...
{
  *val = 0;
  if (osapiIpReasmTimeoutGet(val) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
...
{
  *val = 0;
  if (osapiIpReasmReqdsGet(val) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
...
{
  *val = 0;
  if (osapiIpReasmOKsGet(val) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
...
{
  *val = 0;
  if (osapiIpReasmFailsGet(val) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
...
{
  L7_ulong64 ctr;
  L7_uint32 stackFails = 0;

  *val = 0;
  if (dtlStatsGet(FD_CNFGR_NIM_MIN_CPU_INTF_NUM, L7_PLATFORM_CTR_RX_IP_FRAG_FAILS, 
                  C32_BITS, &ctr) == L7_SUCCESS)
  {
    *val += ctr.low;
  }
  
  if (osapiIpFragFailsGet(&stackFails) == L7_SUCCESS)
  {
    *val += stackFails;
  }
  return L7_SUCCESS;
