...
/*********************************************************************
*
* @purpose  Get the current number of rules defined for the specified ACL
*
* @param    L7_uint32   aclnum     the access-list identifying number
* @param    L7_uint32   *ruleCount pointer to rule count output location
*
* @returns  L7_SUCCESS
...
*
*********************************************************************/
L7_RC_t aclNumRulesGet(L7_uint32 aclnum, L7_uint32 *ruleCount)
{
  L7_RC_t rc;
  ...
  rc = aclImpAclRuleCountGet(aclnum, ruleCount);
...
L7_RC_t aclCreate(L7_uint32 aclnum)
{
  ...
  /* get current number of ACLs configured in the system
   *
   * NOTE: Uses aclRwLock, so call it before taking the lock
   */
  if (aclCurrNumGet(&aclCount) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
  ...
  /* check with callback registrants to make sure ACL can be created */
  if (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_LIST_CREATE,
                                aclnum, &aclCount) != L7_SUCCESS)
  {
    (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
...
L7_RC_t aclDelete(L7_uint32 aclnum, L7_BOOL force)
{
  ...
  if ((force != L7_TRUE) &&
      (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_LIST_DELETE,
                                 aclnum, L7_NULLPTR) != L7_SUCCESS))
  {
    (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
    return L7_REQUEST_DENIED;
  }
...
/*********************************************************************
*
* @purpose  Get the current number of rules defined for the specified access list,
*           given its ACL type and identifier.
*
* @param    aclType     @b{(input)}  access list type
* @param    aclnum      @b{(input)}  access list identifying number
* @param    *pRuleCount @b{(output)} pointer to rule count output location
*
* @returns  L7_SUCCESS
* @returns  L7_ERROR    ACL does not exist
* @returns  L7_FAILURE
...
*
*********************************************************************/
L7_RC_t aclCommonNumRulesGet(L7_ACL_TYPE_t aclType, L7_uint32 aclnum,
                             L7_uint32 *pRuleCount)
{
  L7_RC_t       rc = L7_FAILURE;
  ...
  switch (aclType)
  {
    ...
    case L7_ACL_TYPE_IPV6:
      /* IPV4 or IPv6 named ACL */
      rc = aclNumRulesGet(aclnum, pRuleCount);
      break;

    case L7_ACL_TYPE_MAC:
      /* MAC named ACL */
      rc = aclMacNumRulesGet(aclnum, pRuleCount);
      break;
...
L7_RC_t aclRuleActionAdd(L7_uint32 aclnum, L7_uint32 rulenum, L7_uint32 action)
{
  ...
  if (rc == L7_SUCCESS)
  {
    ...
    if ((ACL_RULE_FIELD_IS_SET(p->configMask, ACL_ACTION) == L7_TRUE) &&
        (p->action == action))
    {
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_SUCCESS;
  ...
  {
    /* rule does not exist -- check with callback registrants
     * to make sure a new rule can be created
     */
    if (aclImpAclRuleCountGet(aclnum, &ruleCount) != L7_SUCCESS)
    {
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_FAILURE;
    }
    if (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_RULE_CREATE,
                                  aclnum, &ruleCount) != L7_SUCCESS)
    {
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_REQUEST_DENIED;
    }
...
L7_RC_t aclRuleRemove(L7_uint32 aclnum, L7_uint32 rulenum)
{
  ...
  /* make sure rule removal is approved */
  if (aclImpChangeApprovalQuery(L7_ACL_CHANGE_APPROVAL_RULE_DELETE,
                                aclnum, L7_NULLPTR) != L7_SUCCESS)
  {
    (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
    return L7_REQUEST_DENIED;
                                         ...
                                         L7_uchar8 *displayStr)
{
  ...
  if ((rc = cnfgrApiComponentNameGet(compId, name)) != L7_SUCCESS)
  {
      ...
      (tableId >= ACL_CALLBACK_TABLE_ID_TOTAL))
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "Invalid ACL callback registration table ID %d for component %u, %s\n",
            tableId, compId, name);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "Invalid ACL callback registration for component %u, %s\n", compId, name);
    return L7_FAILURE;
  }
...
* @end
*********************************************************************/
L7_RC_t aclChangeApprovalCallbackRegister(ACL_CHANGE_APPROVAL_FUNCPTR_t funcPtr,
                                          L7_COMPONENT_IDS_t compId,
                                          L7_uchar8 *displayStr)
{
  return aclCommonCallbackRegister(ACL_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                   (L7_VOIDFUNCPTR_t)funcPtr, compId, displayStr);
}
...
* @end
*********************************************************************/
L7_RC_t aclChangeApprovalCallbackUnregister(ACL_CHANGE_APPROVAL_FUNCPTR_t funcPtr)
{
  return aclCommonCallbackUnregister(ACL_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                     (L7_VOIDFUNCPTR_t)funcPtr);
}
...
* @end
*********************************************************************/
L7_RC_t aclChangeApprovalCallbackIsRegistered(ACL_CHANGE_APPROVAL_FUNCPTR_t funcPtr)
{
  return aclCommonCallbackIsRegistered(ACL_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                       (L7_VOIDFUNCPTR_t)funcPtr);
}
...
* @end
*********************************************************************/
L7_RC_t aclChangeApprovalCallbackTableShow(void)
{
  return aclCommonCallbackTableShow(ACL_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
...
L7_RC_t aclCallbackTableShow(void)
{
  ...
  if (aclChangeApprovalCallbackTableShow() != L7_SUCCESS)
    rc = L7_FAILURE;
...
/*********************************************************************
*
* @purpose  To get the TimeRange Name for an access list rule.
*
* @param    aclnum         @b{(input)}   the access-list identifying number
* @param    rulenum        @b{(input)}   the acl rule number
* @param    timeRangeName  @b{(output)}  the time range name
*
* @returns  L7_SUCCESS, if the timerange Name is retrieved
* @returns  L7_FAILURE, if invalid list number or rulenum
* @returns  L7_ERROR, if the access-list or rulenum does not exist
*
* @comments None.
...
*
*********************************************************************/
L7_RC_t aclRuleTimeRangeNameGet(L7_uint32 aclnum, L7_uint32 rulenum, L7_uchar8 *timeRangeName)
{
  L7_RC_t rc;
  aclRuleParms_t *acl_rp;
  aclTRCorrEntry_t *aclTRCorrEntryPtr;
  ...
  if(acl_rp->timeRangeEntry == L7_NULLPTR)
  {
    (void)ACL_READ_LOCK_GIVE(aclRwLock);
    return L7_FAILURE;
  }

  aclTRCorrEntryPtr = (acl_rp->timeRangeEntry);

  if (aclTimeRangeTableEntryTRNameGet(aclTRCorrEntryPtr->trId, timeRangeName)!= L7_SUCCESS)
  {
    (void)ACL_READ_LOCK_GIVE(aclRwLock);
    return L7_FAILURE;
  }
...
/*********************************************************************
*
* @purpose  To add the TimeRange name for an access list rule.
*
* @param    aclnum         @b{(input)}   the access-list identifying number
* @param    rulenum        @b{(input)}   the acl rule number
* @param    timeRangeName  @b{(input)}   the time range name
*
* @returns  L7_SUCCESS, if timerange name is added
* @returns  L7_FAILURE, if invalid queue id or other failure
* @returns  L7_ERROR, if the access-list or rule does not exist
* @returns  L7_REQUEST_DENIED, if hardware update failed
*
...
*
*********************************************************************/
L7_RC_t aclRuleTimeRangeNameAdd(L7_uint32 aclnum, L7_uint32 rulenum, L7_uchar8 *timeRangeName)
{
  L7_RC_t rc;
  void *acl_ptr;
  aclRuleParms_t  *p;
  aclTRCorrEntry_t *aclTRCorrEntryPtr = L7_NULLPTR;
  L7_uint32 saved_config =0;
  aclTRCorrEntry_t *saved_entry = L7_NULLPTR;
  L7_uchar8 prevTRName[L7_TIMERANGE_NAME_LEN_MAX+1];
  L7_uint32 prevTRId = -1;
  L7_uint32 currTRId = -1;
  L7_uint32 timeRangeIndex;
  L7_uint32 trStatus;
  L7_uint32 status = L7_ACL_RULE_STATUS_NONE;
  L7_BOOL modify = L7_FALSE;

  memset(prevTRName, 0, sizeof(prevTRName));
  ...
  /* validate the time range name */
  if (timeRangeName == L7_NULLPTR)
    return L7_FAILURE;

  if (aclTimeRangeNameStringCheck(timeRangeName) != L7_SUCCESS)
     return L7_FAILURE;
      ...
      (ACL_RULE_FIELD_IS_SET(p->configMask, ACL_TIME_RANGE_NAME) == L7_TRUE))
  {
    ...
    {
      if (aclTimeRangeTableEntryTRNameGet(p->timeRangeEntry->trId, prevTRName) !=L7_SUCCESS)
      {
        (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
        return L7_FAILURE;
      }
      /* if ACL Rule is configured with same time range name */
      if (osapiStrncmp(prevTRName, timeRangeName, sizeof(prevTRName)) == 0)
      {
        (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
        return L7_SUCCESS;
      }
      /* restore trId and corrId */
      prevTRId = p->timeRangeEntry->trId;
      modify = L7_TRUE;
    }
  }

  saved_config = p->configMask;
  saved_entry = p->timeRangeEntry;

  rc = aclTimeRangeNameToIndex(timeRangeName, &timeRangeIndex);
  if (rc == L7_SUCCESS)
  {
    rc = aclTimeRangeStatusGet(timeRangeIndex, &trStatus);
    if (rc == L7_SUCCESS)
    {
      if ((trStatus ==  TIMERANGE_STATUS_ACTIVE) || (trStatus ==  TIMERANGE_STATUS_NON_EXISTENT))
      {
        status = L7_ACL_RULE_STATUS_ACTIVE;
      }
      else if (trStatus ==  TIMERANGE_STATUS_INACTIVE)
      {
        status = L7_ACL_RULE_STATUS_INACTIVE;
      }
      else
      {
        (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
  ...
  /* Add the entry to Time Range Name table */
  if (aclTimeRangeTableEntryUpdate(timeRangeName, status, &currTRId) != L7_SUCCESS)
  {
    (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
    return L7_FAILURE;
  }

  /* Create the correlator node only when rule is created and not for modifications */
  if (p->timeRangeEntry == L7_NULLPTR)
  {
    if (aclTimeTangeTableCorrEntryCreate(currTRId, L7_QOS_ACL_TLV_RULE_CORR_NULL, &aclTRCorrEntryPtr) != L7_SUCCESS)
    {
      aclTimeRangeTableEntryDelete(currTRId);
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_FAILURE;
    }
  }
  ...
  {
    aclTRCorrEntryPtr = p->timeRangeEntry;       
    aclTRCorrEntryPtr->trId = currTRId;        
    /* Relocate the correlator node to current trId */
    aclTimeRangeTableCorrEntryRelocate(prevTRId, currTRId, aclTRCorrEntryPtr);      
  }     
    /* update App */ 
  if (modify !=L7_TRUE)
  {
    if (aclTimeRangeEntryAdd(acl_ptr, rulenum, aclTRCorrEntryPtr) != L7_SUCCESS)
    {
      aclTRCorrEntryFree(aclTRCorrEntryPtr);
      aclTimeRangeTableEntryDelete(currTRId);
      (void)ACL_WRITE_LOCK_GIVE(aclRwLock);
      return L7_FAILURE;
    }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    if(modify == L7_TRUE)
    {
      p->timeRangeEntry->trId = prevTRId;
      /*relocate the node to prevTRId */
      aclTimeRangeTableCorrEntryRelocate(currTRId, prevTRId, p->timeRangeEntry);
    }
    else
    {
      p->timeRangeEntry->refCount=0;
      aclTimeRangeTableCorrEntryDelete(p->timeRangeEntry->trId, p->timeRangeEntry);
      aclTRCorrEntryFree(p->timeRangeEntry);
      p->timeRangeEntry = saved_entry;
    }
   (void)aclUpdate(aclnum, ACL_MODIFY);        /* ignore rc here */
    /* delete the currTRId*/
    aclTimeRangeTableEntryDelete(currTRId);
  ...
  {
    /* delete the previous trid from aclTimeRangeTable*/
    if (modify == L7_TRUE)
    {
      aclTimeRangeTableEntryDelete(prevTRId);
    }
  }
  if (ACL_WRITE_LOCK_GIVE(aclRwLock) != L7_SUCCESS)
  {
...
*
*********************************************************************/
L7_RC_t aclRuleStatusGet(L7_uint32 aclIndex, L7_uint32 rulenum, L7_uint32 *status)
{
  L7_RC_t rc;
  aclRuleParms_t *acl_rp;
  aclTRCorrEntry_t *aclTRCorrEntryPtr;
  ...
  /* check inputs */
  if (aclImpNumRangeCheck(aclIndex) != L7_SUCCESS)
    return L7_FAILURE;

  if (aclImpRuleNumRangeCheck(aclIndex, rulenum) != L7_SUCCESS)
    return L7_FAILURE;
  ...
  rc = aclRuleGet(aclIndex, rulenum, &acl_rp);
  if (rc == L7_ERROR)
  {
    (void)ACL_READ_LOCK_GIVE(aclRwLock);
    return L7_ERROR;
  ...
  rc = aclTimeRangeTableEntryStatusGet(aclTRCorrEntryPtr->trId, status);
  if(rc != L7_SUCCESS)
  {
    (void)ACL_READ_LOCK_GIVE(aclRwLock);
    return L7_FAILURE;
