  ...
  while (offset < msg->length)
  {
    ...
    switch (element.elementId)
    {
      ...
      case ACL_CLUSTER_MSG_IE_ACL_DEF_END:
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "%s: invalid ACL configuration, received wrong index in ACL_CLUSTER_MSG_IE_ACL_DEF_END IE.\n",
                    __FUNCTION__);
            aclRxError = L7_TRUE;
          }
        }
      ...
      case ACL_CLUSTER_MSG_IE_RULE_GROUP_START:
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                  "%s: invalid configuration for ACL index=%u, max rules per list is %u.\n",
      ...
      case ACL_CLUSTER_MSG_IE_RULE_GROUP_END:
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                  "%s: invalid ACL configuration, invalid rule count for ACL index=%u, received:%u, expected:%u.\n",
    ...
    if (aclRxError == L7_TRUE)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
              "%s: invalid ACL configuration, error receiving IE:%u.\n",
              __FUNCTION__, element.elementId);
      break;
    }
  } /* endwhile */
      ...
      for (dir=0; dir < ACL_INTF_DIR_MAX; dir++)
      {
                                          ...
                                          dir, &intfList) == L7_SUCCESS))
        {
          ...
          for (j=0; j < intfList.count; j++)
          {
            ...
            {
              nimGetIntfName(intfList.intIfNum[j], L7_SYSNAME, ifName);
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                      "%s: aclIntfDirListRemove failed for intf=%s, dir=%u, acl=%u\n",
                      __FUNCTION__, ifName, dir, pAclNotifyList[i].currIndex);
            }
            else
            {
              nimGetIntfName(intfList.intIfNum[j], L7_SYSNAME, ifName);
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                      "%s: removed acl:%u from intf:%s, dir:%u, ACL does not exist in received configuration.\n",
                      __FUNCTION__, pAclNotifyList[i].currIndex, ifName, dir);
            }
          }
        }
        if (((aclType == L7_ACL_TYPE_MAC) &&
                                          ...
                                          dir, &vlanList) == L7_SUCCESS))
        {
          for (j = 0; j < vlanList.count; j++)
          {
            ...
            {
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                      "%s: aclVlanDirListRemove failed for vlanNum=%u, dir=%u, acl=%u\n",
                      __FUNCTION__, vlanList.vlanNum[j], dir, pAclNotifyList[i].currIndex);
            }
            else
            {
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                      "%s: removed acl:%u from vlan:%u, dir:%u, ACL does not exist in received configuration.\n",
  ...
  for (i = 1; i < L7_ACL_INTF_MAX_COUNT; i++)
  {
    ...
    for (dir=0; dir < ACL_INTF_DIR_MAX; dir++)
    {
      ...
      for (j = 0; j < L7_ACL_MAX_LISTS_PER_INTF_DIR; j++)
      {
        ...
        for (k=0; k < notifyIndex; k++)
        {
              ...
              (pIntfList->assignList[j].id.aclId == pAclNotifyList[k].currIndex))
          {
            ...
            {
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                      "%s: aclRuleCountGet failed for aclId:%u, aclType:%u\n",
                      __FUNCTION__,
            ...
            if (intfDirRuleCount > L7_ACL_MAX_RULES_PER_MULTILIST)
            {
              ...
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                        "%s: removed aclId:%u, aclType:%u from intf:%s, dir:%u, L7_ACL_MAX_RULES_PER_MULTILIST exceeded.\n",
                        __FUNCTION__, pIntfList->assignList[j].id.aclId, pIntfList->assignList[j].id.aclType, ifName, dir);
              }
              else
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                        "%s: failed to remove aclId:%u, aclType:%u from intf:%s, dir:%u, L7_ACL_MAX_RULES_PER_MULTILIST exceeded.\n",
                        __FUNCTION__, pIntfList->assignList[j].id.aclId, pIntfList->assignList[j].id.aclType, ifName, dir);
              }
            }
            else
      ...
      if (L7_INTF_ISMASKBITSET(intfMask[dir], intIfNum))
      {
        ...
        ACL_CLUSTER_DLOG("%s: calling aclIntfUpdate with intf=%s, dir=%u, ACL_INTF_REMOVE.\n",
                         __FUNCTION__, ifName, dir);
        (void)aclIntfUpdate(intIfNum, dir, ACL_INTF_REMOVE);
        if (inUse == L7_TRUE)
        {
          ACL_CLUSTER_DLOG("%s: calling aclIntfUpdate with intf=%s, dir=%u, ACL_INTF_ADD.\n",
                           __FUNCTION__, ifName, dir);
          if (aclIntfUpdate(intIfNum, dir, ACL_INTF_ADD) != L7_SUCCESS)
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "%s: unable to assign ACL rules to intf:%s, dir:%u.\n",
                    __FUNCTION__, ifName, dir);
          }
        }
      }
    ...
    for (dir=0; dir < ACL_INTF_DIR_MAX; dir++)
    {
        ...
        for (k=0; k < notifyIndex; k++)
        {
              ...
              (pVlanList->assignList[j].id.aclId == pAclNotifyList[k].currIndex))
          {
            ...
            if (vlanDirRuleCount > L7_ACL_MAX_RULES_PER_MULTILIST)
            {
              ...
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                        "%s: removed aclId:%u, aclType:%u from vlanId:%u, dir:%u, L7_ACL_MAX_RULES_PER_MULTILIST exceeded.\n",
                        __FUNCTION__, pVlanList->assignList[j].id.aclId, pVlanList->assignList[j].id.aclType, vlanId, dir);
              }
              else
              {
                L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                        "%s: failed to remove aclId:%u, aclType:%u from vlanId:%u, dir:%u, L7_ACL_MAX_RULES_PER_MULTILIST exceeded.\n",
                        __FUNCTION__, pVlanList->assignList[j].id.aclId, pVlanList->assignList[j].id.aclType, vlanId, dir);
              }
            }
            else
      ...
      if (L7_VLAN_ISMASKBITSET(vlanMask[dir], vlanId))
      {
        ...
        if (inUse == L7_TRUE)
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                    "%s: unable to assign ACL rules to vlanId:%u, dir:%u.\n",
                    __FUNCTION__, vlanId, dir);
          }
        }
      }
...
void aclClusterConfigAclDelete(aclStructure_t *pAcl)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "%s: avlDeleteEntry failed for aclNum=%u\n",
...
void aclClusterConfigMacAclDelete(aclMacStructure_t *pMacAcl)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
            "%s: avlDeleteEntry failed for aclIndex=%u\n",
...
void aclClusterConfigApply(void)
{
  ...
  for (i=0; i < aclRxCount; i++)
  {
    ...
    else
    {
      ...
      aclClusterConfigAclRulesFree(&pAclRxList[i]);
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
              "%s: unexpected error, failed to add aclNum=%u\n",
  ...
  for (i=0; i < aclMacRxCount; i++)
  {
    ...
    else
    {
      ...
      aclClusterConfigMacAclRulesFree(&pAclMacRxList[i]);
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
              "%s: unexpected error, failed to add aclIndex=%u\n",
    ...
    case CLUSTER_CONFIG_CMD_END_TRANSFER:
      ...
      else
      {
        ...
        if (rc != L7_SUCCESS)
        {
          ...
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_ACL_COMPONENT_ID,
                  "%s: ACL received config verification failed.\n", __FUNCTION__);
        }
      }
...
/*********************************************************************
*
* @purpose  Get the index number of a config push access list, given its ACL type and name.
*
...
*
*********************************************************************/
L7_RC_t aclClusterCommonNameToIndex(L7_ACL_TYPE_t aclType, L7_uchar8 *pName,
                                    L7_uint32 *pAclNum)
{
  L7_RC_t       rc = L7_FAILURE;
  L7_uint32     aclNum = 0;
  ...
  else
  {
    switch (aclType)
    {
      ...
      case L7_ACL_TYPE_IPV6:
        ...
        rc = aclClusterNameToIndex(pName, &aclNum);
        break;

      case L7_ACL_TYPE_MAC:
        /* MAC named ACL */
        rc = aclClusterMacNameToIndex(pName, &aclNum);
        break;
...
*
*********************************************************************/
L7_RC_t aclClusterCommonNumRulesGet(L7_ACL_TYPE_t aclType, L7_uint32 aclnum,
                                    L7_uint32 *pRuleCount)
{
  ...
  switch (aclType)
  {
    ...
    case L7_ACL_TYPE_IPV6:
      /* IPV4 or IPv6 named ACL */
      rc = aclClusterNumRulesGet(aclnum, pRuleCount);
      break;

    case L7_ACL_TYPE_MAC:
      /* MAC named ACL */
      rc = aclClusterMacNumRulesGet(aclnum, pRuleCount);
      break;
...
*
*********************************************************************/
L7_RC_t aclClusterNumRulesGet(L7_uint32 aclnum, L7_uint32 *ruleCount)
{
  L7_RC_t           rc = L7_FAILURE;
  L7_uint32         i;
  aclStructure_t    *pAcl;
...
/*********************************************************************
*
* @purpose  Get the current number of rules defined for the specified MAC ACL
*
* @param    L7_uint32   aclIndex   access list index
* @param    L7_uint32   *ruleCount pointer to rule count output location
*
* @returns  L7_SUCCESS
* @returns  L7_ERROR    access list does not exist
...
*
*********************************************************************/
L7_RC_t aclClusterMacNumRulesGet(L7_uint32 aclIndex, L7_uint32 *ruleCount)
{
  L7_RC_t           rc = L7_FAILURE;
  L7_uint32         i;
  aclMacStructure_t *pAcl;
