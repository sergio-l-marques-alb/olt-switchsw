...
voipSccpCmsLookup (const L7_uchar8 *callManager, voipSccpCms_t **entry)
{
  ...
  while (ptr != L7_NULLPTR)
  {
    if (strcmp(ptr->callManager, callManager) == 0)
    {
       *entry = ptr;
       return L7_SUCCESS;
    }
...
voipSccpCmsInsert(const L7_uchar8 *callManager, voipSccpCms_t **ins)
{
  ...
  if ((rv = voipSccpCmsLookup(callManager, &entry)) == L7_SUCCESS)
  {
      *ins = entry;
  ...
  entry->next     = voipSccpCmsList;
  voipSccpCmsList = entry;
  *ins            = entry;
...
voipSccpCallInsert(voipSccpCms_t *cm,voipOperSccp_t m,voipSccpCall_t **ins)
{
       ...
       sizeof(voipSccpCall_t))) == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
            "voipSccpCallInsert Could not allocate memory\n");
    return L7_FAILURE;
  }

  memset(entry, 0x00, sizeof(voipSccpCall_t));
  entry->conferenceId = m.lel_conferenceId;
...
voipSccpCallRemove(voipSccpCms_t **cm,voipSccpCall_t *del)
{
  ...
  while (entry != L7_NULLPTR)
  {
    ...
    {
      /* Log the event */
      usmDbInetNtoa(entry->remoteIP, strRemoteIPaddr);

      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
              "Removing SCCP call. Call Manager IP = %s\r\n ConferenceId = %d\r\n"
               "Remote IP = %s, Remote Port = %d\r\n",
               (*cm)->callManager, entry->conferenceId,
               strRemoteIPaddr, entry->remotePort);
...
#ifdef L7_NSF_PACKAGE
      voipCkptSccpCallDelete(*cm, entry);
#endif
...
L7_RC_t voipSetupSccpSession(voipSccpCall_t *t, L7_BOOL callStatus)
{
  ...
  if (rc == L7_SUCCESS)
  {
    if (callStatus == L7_ENABLE)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_VOIP_COMPONENT_ID,
              "Established new SCCP call. ConferenceId = %d\r\n"
              "Remote IP = %s, Remote Port = %d\r\n",
  ...
  if (tcpdatalen > 4)
  {
    ...
    */
    ...
    if (callInfo.lel_messageId == StartMediaTransmission)
    {
      ...
      rv = voipSccpCmsInsert(callInfo.callManager, &cmentry);
      if (rv != L7_SUCCESS)
      {
        return L7_FAILURE;
      }
...
#ifdef L7_NSF_PACKAGE
      voipCkptSccpCallSync(cmentry, callentry, L7_TRUE);
#endif
    ...
    else if (callInfo.lel_messageId == StopMediaTransmission)
    {
      ...
      rv = voipSccpCmsLookup(callInfo.callManager, &cmentry);
      if (rv == L7_FAILURE)
      {
         return L7_FAILURE;
      }
...
{
  voipSccpCms_t *cms= voipSccpCmsList;
  voipSccpCms_t *nextCms;
  voipSccpCall_t *entry=L7_NULLPTR;
  voipSccpCall_t *nextEntry;
  L7_ulong64     counter;
...
{
  voipSccpCms_t  *cmsEntry;
  voipSccpCall_t *callEntry;

  cmsEntry = voipSccpCmsList;

  while (cmsEntry != L7_NULL)
  {
    callEntry = cmsEntry->calls;

    while (callEntry != L7_NULL)
    {
      voipCkptSccpCallSync(cmsEntry, callEntry, L7_FALSE);

      callEntry = callEntry->next;
    }

    cmsEntry = cmsEntry->next;
  }
}
#endif
...
L7_RC_t voipDebugSccpCallAdd(L7_uchar8  *call_mgr,
                             L7_uint32   conference_id,
                             L7_uint32   remote_ip,
                             L7_ushort16 remote_port)
{
  L7_RC_t         rc;
  voipSccpCms_t  *cmsEntry;
  voipSccpCall_t *callEntry;
  ...
  rc = voipSccpCmsLookup(call_mgr, &cmsEntry);
  if (rc != L7_SUCCESS)
  {
    rc = voipSccpCmsInsert(call_mgr, &cmsEntry);
    if (rc != L7_SUCCESS)
      return rc;
  }

  m.lel_conferenceId     = conference_id;
  m.bel_remoteIpAddr     = remote_ip;
  m.lel_remotePortNumber = remote_port;
  rc = voipSccpCallInsert(cmsEntry, m, &callEntry);
  if (rc != L7_SUCCESS)
    return rc;

#ifdef L7_NSF_PACKAGE
  voipCkptSccpCallSync(cmsEntry, callEntry, L7_TRUE);
#endif
  rc = voipSetupSccpSession(callEntry, L7_ENABLE);
  if (rc != L7_SUCCESS)
  {
    voipSccpCallRemove(&cmsEntry, callEntry);
    VOIP_TRACE(VOIP_DEBUG_INFO, VOIP_PROTO_SCCP, "voipSetupSccpSession(enable) failed\n");
  }
  return rc;
}

L7_RC_t voipDebugSccpCallDelete(L7_uchar8  *call_mgr, L7_uint32 conference_id)
{
  L7_RC_t         rc;
  voipSccpCms_t  *cmsEntry;
  voipSccpCall_t *callEntry;
  ...
  rc = voipSccpCallLookup(cmsEntry, conference_id, &callEntry);
  if (rc != L7_SUCCESS)
    return rc;

  rc = voipSetupSccpSession(callEntry, L7_DISABLE);
  voipSccpCallRemove(&cmsEntry, callEntry);
