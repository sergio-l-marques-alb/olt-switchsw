  ...
  do
  {
    ...
    else
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsmibPolicyPerfInDelete: couldn't delete row AVL node for "
              "Policy Perf In Table indexes (%u,%u), interface %s\n",
              policyIndex, policyInstIndex, ifName);
                                 ...
                                 L7_uint32 intIfNum)
{
  ...
  do
  {
    ...
    else
    {
      /* something's wrong */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsmibPolicyPerfOutDelete: couldn't delete row AVL node for "
              "Policy Perf In Table indexes (%u,%u), interface %s\n",
              policyIndex, policyInstIndex, ifName);
                              ...
                              L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_t ifDirection)
{
         ...
         (nextPolicyIndex == policyIndex))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsmibPolicyPerfAttach: couldn't create Perf %s row for "
              "indexes %u,%u on intf %s\n", pStr, policyIndex,
              policyInstIndex, ifName);
      return L7_FAILURE;
    }
                              ...
                              L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_t ifDirection)
{
         ...
         (nextPolicyIndex == policyIndex))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsmibPolicyPerfDetach: couldn't delete Perf %s row for "
              "indexes %u,%u on intf %s\n", pStr, policyIndex,
              policyInstIndex, ifName);
      return L7_FAILURE;
    }
