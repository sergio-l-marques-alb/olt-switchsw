        ...
        if (rc != L7_SUCCESS)
        {
          ...
          else
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                    "diffServClassRuleCreate: Error deleting row after create "
...
L7_RC_t diffServClassRuleDeleteAll(L7_uint32 classIndex)
{
         ...
         (nextClassIndex == classIndex))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "%s: could not delete rule %u from class index %u\n",
                             ...
                             L7_BOOL   bypassEval)
{
  ...
  do
  {
    ...
    else
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServClassRuleDelete: couldn't delete row AVL node for rule "
                "%u in class \'%s\'\n", classRuleIndex, (char *)className);
      }
      else
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServClassRuleDelete: couldn't delete row AVL node for "
                "indexes %u,%u\n", classIndex, classRuleIndex);
      }
      rc = L7_FAILURE;
    }
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
    {
      ...
      if (diffServClassRuleDelete(classIndex, classRuleIndex) != L7_SUCCESS)
      {
        pRow->mib.rowStatus = restoreStatus;
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServClassRuleObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
    }
    else
                                               ...
                                               L7_uint32 *pClassRuleIndex)
{
  ...
  while ((diffServClassRuleGetNext(classIndex, classRuleIndex,
         ...
         (nextClassIndex == classIndex))
  {
    if ((pRow = dsmibClassRuleRowFind(classIndex, classRuleIndex)) == L7_NULLPTR)
      return L7_ERROR;
...
void dsmibClassRuleIndexNextUpdate(L7_uint32 classIndex)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsmibClassRuleIndexNextUpdate: ERROR -- could not find "
...
void dsmibClassRuleIndexNextUndo(L7_uint32 classIndex)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsmibClassRuleIndexNextUndo: ERROR -- could not find "
            "class row for index %u\n", classIndex);
    return;                                     /* ERROR -- should not occur */
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsmibClassRuleIndexNextUndo: WARNING -- attempting to decrement "
...
L7_RC_t dsmibClassRuleAclDelete(L7_uint32 classIndex)
{
         ...
         (nextClass == classIndex))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsmibClassRuleAclDelete: Could not delete rule %u from "
                                    ...
                                    L7_uint32 classRuleIndex)
{
  ...
  for (rd = 0; rd < relevantClassCount; rd++)
  {
                                         ...
                                         &classIdx, &ruleIdx) == L7_SUCCESS)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "%s: row find error for indexes %u,%u\n",
                                     ...
                                     L7_uint32 classRuleIndex)
{
  ...
  for (rd = 0; rd < relevantClassCount; rd++)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "%s: relevant class row find error for index %u\n",
           ...
           (nextClass == classIdx) && (ruleIdx != 0))
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "%s: relevant class rule row find error for indexes %u,%u\n",
                routine_name, classIdx, ruleIdx);
        return L7_FALSE;
      }
      ...
      while (refClassIdx != DSMIB_CLASS_REF_INDEX_NONE)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                  "%s: refclass row find error for index %u\n",
               ...
               (refNextClass == refClassIdx) && (refRuleIdx != 0))
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                    "%s: refclass rule row find error for indexes %u,%u\n",
