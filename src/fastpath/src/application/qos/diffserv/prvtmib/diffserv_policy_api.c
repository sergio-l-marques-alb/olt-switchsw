                             ...
                             L7_BOOL activateRow)
{
  ...
  do
  {
    ...
    /* check with callback registrants to make sure policy can be created */
    currCount = genStatus.tableSize[tableId];
    if (diffServChangeApprovalQuery(L7_DIFFSERV_CHANGE_APPROVAL_POLICY_CREATE,
...
L7_RC_t diffServPolicyDelete(L7_uint32 policyIndex)
{
  ...
  do
  {
    ...
    if (diffServChangeApprovalQuery(L7_DIFFSERV_CHANGE_APPROVAL_POLICY_DELETE,
    ...
    else
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "diffServPolicyDelete: couldn't delete row AVL node for policy "
              "\'%s\'\n", (char *)policyName);
      rc = L7_FAILURE;
    }
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
      if (diffServPolicyDelete(policyIndex) != L7_SUCCESS)
      {
        pRow->mib.rowStatus = restoreStatus;
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServPolicyObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
  }
