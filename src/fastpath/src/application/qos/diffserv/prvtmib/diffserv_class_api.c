                            ...
                            L7_BOOL activateRow)
{
  ...
  do
  {
    ...
    /* check with callback registrants to make sure class can be created */
    currCount = genStatus.tableSize[tableId];
    if (diffServChangeApprovalQuery(L7_DIFFSERV_CHANGE_APPROVAL_CLASS_CREATE,
                                    classIndex, &currCount) != L7_SUCCESS)
    {
...
L7_RC_t diffServClassDelete(L7_uint32 classIndex)
{
  ...
  do
  {
    ...
    /* check with callback registrants to make sure class is not currently in use */
    if (diffServChangeApprovalQuery(L7_DIFFSERV_CHANGE_APPROVAL_CLASS_DELETE,
                                    classIndex, L7_NULLPTR) != L7_SUCCESS)
    {
      rc = L7_REQUEST_DENIED;
      break;
    }
    ...
    else
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "diffServClassDelete: couldn't delete row AVL node for class "
              "\'%s\'\n", (L7_char8 *)className);
      rc = L7_FAILURE;
    }
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
      if (diffServClassDelete(classIndex) != L7_SUCCESS)
      {
        pRow->mib.rowStatus = restoreStatus;
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServClassObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
  }
