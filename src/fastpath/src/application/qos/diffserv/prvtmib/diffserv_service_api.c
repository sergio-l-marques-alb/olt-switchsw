                              ...
                              L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_t ifDirection)
{
  ...
  do
  {
    ...
    if (pRow->mib.policyIndex != 0)
    {
            ...
            != L7_SUCCESS)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServServiceDelete:  unable to delete Policy Perf Table row "
                "for intf %s, dir \'%s\', policy index %u\n", ifName,
    ...
    else
    {
      /* something's wrong */
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "diffServServiceDelete:  unable to delete row AVL node for "
              "service intf %s, dir \'%s\'\n", ifName,
              (ifDirection == L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_IN) ? "in" : "out");
      rc = L7_FAILURE;
    }
  ...
  case L7_USMDB_MIB_DIFFSERV_SERVICE_SETTEST_OID_POLICY_INDEX:
    {
      ...
      *pValueLen = sizeof(policyIndex);
      ...
      if (prevPolicyIndex != 0)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                  "diffServServiceObjectSet:  unable to delete Policy Perf Table row "
                  "for intf %s, dir \'%s\', policy index %u\n", ifName,
                  (ifDirection == L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_IN) ? "in" : "out",
                  prevPolicyIndex);
          return L7_FAILURE;
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServServiceObjectSet:  unable to create Policy Perf Table row "
                "for intf %s, dir \'%s\', policy index %u\n", ifName,
                (ifDirection == L7_USMDB_MIB_DIFFSERV_IF_DIRECTION_IN) ? "in" : "out",
                policyIndex);
        return L7_FAILURE;
  ...
  if (newStatus != oldStatus)
  {
    ...
    if (pRow->mib.rowStatus == L7_USMDB_MIB_DIFFSERV_ROW_STATUS_DESTROY)
      if (diffServServiceDelete(intIfNum, ifDirection) != L7_SUCCESS)
      {
        pRow->mib.rowStatus = restoreStatus;
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "diffServServiceObjectSet: attempt to destroy the row failed\n");
        return L7_FAILURE;
      }
  }
