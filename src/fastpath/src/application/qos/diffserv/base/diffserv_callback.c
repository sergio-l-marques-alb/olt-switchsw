...
L7_RC_t diffServCallbackInit(void)
{
  ...
  if (pDiffServCallbackInfo_g == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "%s: Unable to allocate DiffServ callback info structure\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "%s: Unable to create callback table semaphore\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "%s: Unable to allocate DiffServ Notify Event callback table\n", __FUNCTION__);
    return L7_FAILURE;
  }
  ...
  /* malloc space for the DiffServ policy Change Approval callback registration table */
  maxEnt = DIFFSERV_CALLBACK_REGISTRANTS_MAX_CHANGE_APPROVAL;
  memSize = maxEnt * sizeof(diffServCallbackEntry_t);
  pMem = osapiMalloc(L7_FLEX_QOS_DIFFSERV_COMPONENT_ID, memSize);
  if (pMem == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "%s: Unable to allocate DiffServ Change Approval callback table\n", __FUNCTION__);
    return L7_FAILURE;
  }
  memset(pMem, 0, memSize);
  pTable = &pDiffServCallbackInfo_g->table[DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL];
  pTable->pStart = (diffServCallbackEntry_t *)pMem;
  pTable->maxEntries = maxEnt;
                                        ...
                                        L7_uchar8 *displayStr)
{
  ...
  if ((rc2 = cnfgrApiComponentNameGet(compId, name)) != L7_SUCCESS)
  {
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "Invalid DiffServ callback registration table ID %d for component %u, %s\n",
            tableId, compId, name);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "Invalid DiffServ callback registration for component %u, %s\n", compId, name);
    return L7_FAILURE;
  }
                                          ...
                                          L7_VOIDFUNCPTR_t funcPtr)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "Invalid DiffServ callback de-registration for table ID %d\n", tableId);
    return L7_FAILURE;
  }
...
L7_RC_t diffServChangeApprovalCallbackRegister(L7_DIFFSERV_CHANGE_APPROVAL_FUNCPTR_t funcPtr,
                                               ...
                                               L7_uchar8 *displayStr)
{
  return dsCommonCallbackRegister(DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                  (L7_VOIDFUNCPTR_t)funcPtr, compId, displayStr);
}
...
{
  return dsCommonCallbackUnregister(DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                    (L7_VOIDFUNCPTR_t)funcPtr);
}
...
{
  return dsCommonCallbackIsRegistered(DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                      (L7_VOIDFUNCPTR_t)funcPtr);
}
...
{
  return dsCommonCallbackTableShow(DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL,
                                   "Change Approval Callback");
}
...
L7_RC_t diffServCallbackTableShow(void)
{
  ...
  if (diffServChangeApprovalCallbackTableShow() != L7_SUCCESS)
    rc = L7_FAILURE;
...
void diffServChangeNotify(L7_uint32 policyIndex, diffServEvent_t event, L7_uchar8 *policyOldName)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "%s: Improper usage for DiffServ policy delete event\n", __FUNCTION__);
    return;
  }
...
*
* @param    policyChange  @b{(input)} Policy change approval ID
* @param    policyIndex   @b{(input)} Policy index
* @param    pData         @b{(input_} Ptr to request-specific data, or L7_NULLPTR
*
...
*
*********************************************************************/
L7_RC_t diffServChangeApprovalQuery(L7_DIFFSERV_CHANGE_APPROVAL_t policyChange,
                                    L7_uint32 policyIndex, void *pData)
{
   ...
   *       strange about this policy, so allow it to be changed without
  ...
  /* initiate DiffServ policy change approval callbacks */
  pTableStart = pDiffServCallbackInfo_g->table[DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL].pStart;
  maxEnt      = pDiffServCallbackInfo_g->table[DIFFSERV_CALLBACK_TABLE_ID_CHANGE_APPROVAL].maxEntries;
  ...
  for (i = 0, pCbEntry = pTableStart; i < maxEnt; i++, pCbEntry++)
  {
    if (pCbEntry->funcPtr != L7_NULLPTR)
    {
      ...
      rc = (funcPtr)(policyChange, policyType, policyIndex, policyName, pData);
      if (rc != L7_SUCCESS)
      {
        return L7_FAILURE;
      }
