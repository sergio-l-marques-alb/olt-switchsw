...
L7_RC_t dsDstlTlvInit(void)
{
  ...
  if (rc != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsDstlTlvInit:  Unable to register with the TLV utility, rc=%d\n",
            rc);
    tlvHandle = 0;
  }
...
void dsDstlTlvFini(void)
{
  ...
  if (pDsDstlCtrl->tlvHandle != 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsDstlTlvFini:  Unable to Unregister with the TLV utility, rc=%d\n", rc);
    }
    tlvHandle = 0;
  }
                                   ...
                                   dsmibServiceIfDir_t ifDirection)
{
         ...
         (nextPolicy == policyIndex))
  {
    ...
    do
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                "dsDstlTlvPolicyAddSeparate: TLV issued, but could not update "
                "TLV issued flags for policy %u, intf %s, ifIndex %u\n", policyIndex,
                ifName, ifIndex);
        rc = L7_FAILURE;
      }
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsDstlTlvPolicyAddSeparate: Could not delete TLV for handle %u\n",
              (L7_uint32)tlvHandle);
      rc = L7_ERROR;
    }
                                   ...
                                   dsmibServiceIfDir_t ifDirection)
{
  ...
  do
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsDstlTlvPolicyAddCombined: TLVs issued, but could not update "
              "TLV issued flags for policy %u, intf %s, ifIndex %u\n", policyIndex,
              ifName, ifIndex);
      rc = L7_FAILURE;
    }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsDstlTlvPolicyAddCombined: Could not delete TLV for handle %u\n",
            (L7_uint32)tlvHandle);
    rc = L7_ERROR;
  }
  ...
  do
  {
    ...
    if (count != 0)
    {
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                  "dsDstlTlvPolicyDel: DiffServ deletion TLV failure for "
                  "interface %s %s, key count %u\n", ifName,
                  (outboundFlag != L7_TRUE) ? "inbound" : "outbound",
                  count);
        }

        DS_TRACE_PT_POL_TLV_DEL_DTL(policyIndex, intIfNum, ifDirection,
                                    tlvHandle, tempRc, osapiNtohl(pTlv->type), 
                                    tlvLen);
        ...
        if (pNode == pNodeCheck)                /* intf node still same */
        {
          for (i = 0; i < count; i++)
          {
            ...
            {
              L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                      "dsDstlTlvPolicyDel:  Key to Inst translation got "
       ...
       */
       ...
       {
         L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
                 "dsDstlTlvPolicyDel: Could not delete TLV for handle %u\n",
                 (L7_uint32)tlvHandle);
         rc = L7_ERROR;
       }
                                   ...
                                   dsDstlTlvWorkInfo_t *pTlvWorkInfo)
{
  ...
  case L7_USMDB_MIB_DIFFSERV_CLASS_RULE_TYPE_REFCLASS:
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsDstlTlvClassMatchInfoGet: Rule order list contains ref "
...
void dsTlvDebugGetTlv(L7_uint32 policyIndex, L7_uint32 intIfNum, dsmibServiceIfDir_t ifDirection)
{
  if (dsTlvDebugTlvHandle != 0)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
              "dsTlvDebugGetTlv: Could not delete TLV for handle %u\n",
...
L7_RC_t dsTlvDebugGetTlvInit(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_QOS_DIFFSERV_COMPONENT_ID,
            "dsTlvDebugInit:  Unable to register with the TLV utility, rc=%d\n",
            rc);
    tlvHandle = 0;
  }
