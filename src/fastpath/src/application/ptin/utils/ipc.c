...
//if (!global_var_system_ready)  return S_OK;
  ...
  PT_LOG_TRACE(LOG_CTX_IPC,"%d",intfType);
  PT_LOG_TRACE(LOG_CTX_IPC,"%d",interface);
...
int send_TxTsRecord(ptin_PptTsRecord_t *ptpTs)
{
  ...
  PT_LOG_TRACE(LOG_CTX_IPC,"send_message (Canal=%d) 0x%04X. PTP info: ToD %lld.%d, Encap %ld, messageType %d, sequenceId %d, domainNumber %d\n", 
