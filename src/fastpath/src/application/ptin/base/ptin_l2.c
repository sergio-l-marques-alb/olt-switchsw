                            ...
                            L7_uint32 vlanId, L7_uchar8 msgsType)
{
  ...
  LOG_TRACE(LOG_CTX_L2, "Learning event received: %u", msgsType);
  ...
  LOG_TRACE(LOG_CTX_L2, "intIfNum %u, vport 0x%x", intIfNum, virtual_port);

  if (nimGetIntfType(intIfNum, &intf_type) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_L2, "Unknown intIfNum type: %u", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_TRACE(LOG_CTX_L2, "Not a virtual port: intIfNum %u, vport 0x%x", intIfNum, virtual_port);
    return L7_SUCCESS;
  }

  LOG_TRACE(LOG_CTX_L2, "Processing vlan %u, MAC=%02x:%02x:%02x:%02x:%02x:%02x, intIfNum %u, vport 0x%x",
  ...
  {
    LOG_WARNING(LOG_CTX_L2, "Virtual port 0x%08x does not exist!", virtual_port);
    return L7_FAILURE;
  }
  ...
  if (msgsType == FDB_ADD)
  {
    LOG_TRACE(LOG_CTX_L2, "ADD event: MAC=%02x:%02x:%02x:%02x:%02x:%02x VLAN=%u intIfNum=%u virtual_port=0x%08x",
  ...
  {
    LOG_TRACE(LOG_CTX_L2, "DEL event: MAC=%02x:%02x:%02x:%02x:%02x:%02x VLAN=%u intIfNum=%u virtual_port=0x%08x",
              macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5], vlanId, intIfNum, virtual_port);
  ...
  {
    LOG_ERR(LOG_CTX_L2, "Unknown operation type %u", msgsType);
    rc = L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_L2, "Going to configure policer");

  /* Apply policer */
  if (ptin_bwPolicer_set(&profile, &meter, vp_entry.policer.policer_id) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_L2, "Error applying profile");
    rc = L7_FAILURE;
  }

  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_L2, "Error occurred");
    return rc;
  }

  LOG_TRACE(LOG_CTX_L2, "Policer configured successfully");
#endif
...
L7_RC_t ptin_l2_mac_aging_get(L7_uint32 *age)
{
  ...
  if (age==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_L2,"Null pointer");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Error reading MAC aging");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_l2_mac_aging_set(L7_uint32 age)
{
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Error applying MAC aging of %u seconds",age);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_L2,"Success applying MAC aging of %u seconds",age);
...
L7_RC_t ptin_l2_mac_table_load(void)
{
  ...
  LOG_INFO(LOG_CTX_L2, "Loading MAC table...");
  ...
  for (i=0, index=0; i<PLAT_MAX_FDB_MAC_ENTRIES && fdbFind(keyNext, L7_MATCH_GETNEXT, &fdbEntry)==L7_SUCCESS; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_L2,"Error getting interface type of intIfNum=%u",fdbEntry.dot1dTpFdbPort);
      rc = L7_FAILURE;
      continue;
    }
    ...
    {
      LOG_WARNING(LOG_CTX_L2,"Invalid interface type (%u) of intIfNum=%u",intfType,fdbEntry.dot1dTpFdbPort);
      continue;
    }
    ...
    if (intfType==L7_VLAN_PORT_INTF) {
        ...
        if (intf_vp_DB(3, &e)) {
            LOG_WARNING(LOG_CTX_L2,"PON&GEMid for intIfNum %lu / vport %lu not found",fdbEntry.dot1dTpFdbPort,fdbEntry.dot1dTpFdbVirtualPort);
            continue;
        }
        else LOG_TRACE(LOG_CTX_L2,"intIfNum %lu / vport %u, PON=%u/%u GEMid=%u",
  ...
  #endif
    {
      ...
      {
        LOG_WARNING(LOG_CTX_L2,"Invalid intIfNum=%u",fdbEntry.dot1dTpFdbPort);
        continue;
      }
    }
    ...
    if (vlan>4095) {
      LOG_ERR(LOG_CTX_L2, "Invalid vlanid (%u) on index %u",vlan,index);
      rc = L7_FAILURE;
      continue;
    }
    ...
    LOG_TRACE(LOG_CTX_L2, "index=%u evcId=%u vlan=%u intf=%u/%u gem=%u static=%u mac=%02u:%02u:%02u:%02u:%02u:%02u",
  ...
  LOG_INFO(LOG_CTX_L2, "%u MAC entries loaded.",index);
...
L7_RC_t ptin_l2_mac_table_get( L7_uint32 startId, L7_uint32 *numEntries, ptin_switch_mac_entry **entries )
{
  ...
  {
    LOG_ERR(LOG_CTX_L2,"startId %u out of range [0,%u[",startId,PLAT_MAX_FDB_MAC_ENTRIES);
    return L7_FAILURE;
  }
  ...
  {
    LOG_WARNING(LOG_CTX_L2,"startId %u higher than numElems=%u",startId,mac_table_entries);
    *numEntries = 0;
    *entries    = L7_NULLPTR;
    return L7_SUCCESS;
  }
  ...
  *entries = &mac_table[startId];
  ...
  LOG_TRACE(LOG_CTX_L2, "%u MAC entries retrieved (starting from startId=%u)",*numEntries,startId);
...
L7_RC_t ptin_l2_mac_table_flush( void )
{
  ...
  LOG_WARNING(LOG_CTX_L2,"MAC table will be cleared!");

  /* Reload MAC table */
  if (ptin_l2_mac_table_load()==L7_SUCCESS)
  {
    LOG_TRACE(LOG_CTX_L2,"MAC table reloaded");
    ...
    for (i=0; i<mac_table_entries; i++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_L2,"Error clearing MAC entry of index %u",i);
        rc = L7_FAILURE;
      }
    }
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_L2,"An error ocurred while clearing MAC entries");
    }
    else
    {
      LOG_TRACE(LOG_CTX_L2,"Success clearing all MAC entries");
    }
  }
  else
  {
    LOG_ERR(LOG_CTX_L2,"An error ocurred while reloading MAC table");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_L2,"An error ocurred while flushing all dynamic MAC entries");
    rc = L7_FAILURE;
  }
  else
  {
    LOG_TRACE(LOG_CTX_L2,"Success flushing all dynamic MAC entries");
  }
  ...
  {
    LOG_TRACE(LOG_CTX_L2,"Success reloading MAC table");
  }
...
L7_RC_t ptin_l2_mac_table_entry_remove( ptin_switch_mac_entry *entry )
{
  ...
  if ( entry==L7_NULLPTR )
  {
    LOG_WARNING(LOG_CTX_L2,"No items to be removed");
    return L7_SUCCESS;
  }

  LOG_TRACE(LOG_CTX_L2,"Entry to be removed:");
  LOG_TRACE(LOG_CTX_L2," EVCid  = %u",entry->evcId);
  LOG_TRACE(LOG_CTX_L2," Vlan   = %u",entry->vlanId);
  LOG_TRACE(LOG_CTX_L2," MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",
            entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
  LOG_TRACE(LOG_CTX_L2," Intf   = %u/%u",entry->intf.intf_type,entry->intf.intf_id);
  LOG_TRACE(LOG_CTX_L2," Static = %u",entry->static_entry);
...
//  {
//    LOG_ERR(LOG_CTX_L2,"Vlan %u and evcId %u are invalid",entry->vlanId,entry->evcId);
//    return L7_FAILURE;
//  }
    ...
    {
      LOG_ERR(LOG_CTX_L2,"Can't get root vlan for evcId %u",entry->evcId);
      return L7_FAILURE;
    }
    ...
    {
      LOG_ERR(LOG_CTX_L2,"Vlan %u (extracted from evcId %u) is invalid",vlanId,entry->evcId);
      return L7_FAILURE;
    }
  }
  else
  {
    vlanId = entry->vlanId;
    LOG_TRACE(LOG_CTX_L2,"Vlan to be used will be the provided one: %u",vlanId);
  }

  LOG_TRACE(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x will be removed",
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x not found",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Error removing entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_l2_mac_table_entry_add( ptin_switch_mac_entry *entry )
{
  ...
  LOG_TRACE(LOG_CTX_L2,"Entry to be added:");
  LOG_TRACE(LOG_CTX_L2," EVCid  = %u",entry->evcId);
  LOG_TRACE(LOG_CTX_L2," Vlan   = %u",entry->vlanId);
  LOG_TRACE(LOG_CTX_L2," MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",
            entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
...
//  }
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Interface %u/%u is invalid",entry->intf.intf_type,entry->intf.intf_id);
    return L7_FAILURE;
  }
  ...
  if (fdbFind(keyToFind, L7_MATCH_EXACT, &fdbEntry)==L7_SUCCESS)
  {
    LOG_WARNING(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x exists (attached to intIfNum=%u)",
    ...
    {
      LOG_WARNING(LOG_CTX_L2,"Found entry matches the one to be added... nothing to be done.");
      return L7_SUCCESS;
    }
  ...
  LOG_TRACE(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x (%s) will be added (attached to interface %u/%u)",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5],
  ...
  {
    LOG_ERR(LOG_CTX_L2,"Error adding entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x to intf=%u/%u",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5],
            entry->intf.intf_type,entry->intf.intf_id);
    return L7_FAILURE;
  }
