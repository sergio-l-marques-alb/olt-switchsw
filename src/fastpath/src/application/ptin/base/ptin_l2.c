...
L7_RC_t ptin_l2_mac_table_load(void)
{
  ...
  for (i=0, index=0; i<PLAT_MAX_FDB_MAC_ENTRIES && fdbFind(keyNext, L7_MATCH_GETNEXT, &fdbEntry)==L7_SUCCESS; i++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_L2,"Error getting interface type of intIfNum=%u",fdbEntry.dot1dTpFdbPort);
      rc = L7_FAILURE;
      continue;
    }
    ...
    PT_LOG_TRACE(LOG_CTX_L2, "index=%u evcId=%u vlan=%u intf=%u/%u gem=%u static=%u mac=%02u:%02u:%02u:%02u:%02u:%02u",
