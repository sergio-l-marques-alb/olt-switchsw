...
L7_RC_t ptin_l2_mac_aging_get(L7_uint32 *age)
{
  ...
  if (age==L7_NULLPTR)
  {
    PT_LOG_ERR(LOG_CTX_L2,"Null pointer");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Error reading MAC aging");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_l2_mac_aging_set(L7_uint32 age)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Error applying MAC aging of %u seconds",age);
    return L7_FAILURE;
  }

  PT_LOG_TRACE(LOG_CTX_L2,"Success applying MAC aging of %u seconds",age);
...
L7_RC_t ptin_l2_mac_table_load(void)
{
  ...
  for (i=0, index=0; i<PLAT_MAX_FDB_MAC_ENTRIES && fdbFind(keyNext, L7_MATCH_GETNEXT, &fdbEntry)==L7_SUCCESS; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_L2,"Error getting interface type of intIfNum=%u (rc=%d)",fdbEntry.dot1dTpFdbPort, rc);
      //rc = L7_FAILURE;
      continue;
    }
    ...
    {
      PT_LOG_WARN(LOG_CTX_L2,"Invalid interface type (%u) of intIfNum=%u",intfType,fdbEntry.dot1dTpFdbPort);
      continue;
    }
    ...
    if ((vlan>4095) || (vlan==PTIN_ASPEN2CPU_A_VLAN) || (vlan==PTIN_ASPEN2CPU_B_VLAN)) 
    {
    ...
    PT_LOG_TRACE(LOG_CTX_L2, "index=%u evcId=%u vlan=%u intf=%u/%u gem=%u static=%u mac=%02x:%02x:%02x:%02x:%02x:%02x",
  ...
  PT_LOG_INFO(LOG_CTX_L2, "%u MAC entries loaded.",index);
...
L7_RC_t ptin_l2_mac_table_get( L7_uint32 startId, L7_uint32 *numEntries, ptin_switch_mac_entry **entries )
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"startId %u out of range [0,%u[",startId,PLAT_MAX_FDB_MAC_ENTRIES);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_WARN(LOG_CTX_L2,"startId %u higher than numElems=%u",startId,mac_table_entries);
    *numEntries = 0;
    *entries    = L7_NULLPTR;
    return L7_SUCCESS;
  }
  ...
  *entries = &mac_table[startId];
  ...
  PT_LOG_TRACE(LOG_CTX_L2, "%u MAC entries retrieved (starting from startId=%u)",*numEntries,startId);
...
L7_RC_t ptin_l2_mac_table_flush( void )
{
  ...
  PT_LOG_WARN(LOG_CTX_L2,"MAC table will be cleared!");

  /* Reload MAC table */
  if (ptin_l2_mac_table_load()==L7_SUCCESS)
  {
    PT_LOG_TRACE(LOG_CTX_L2,"MAC table reloaded");
    ...
    for (i=0; i<mac_table_entries; i++)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_L2,"Error clearing MAC entry of index %u",i);
        rc = L7_FAILURE;
      }
    }
    if (rc!=L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_L2,"An error ocurred while clearing MAC entries");
    }
    else
    {
      PT_LOG_TRACE(LOG_CTX_L2,"Success clearing all MAC entries");
    }
  }
  else
  {
    PT_LOG_ERR(LOG_CTX_L2,"An error ocurred while reloading MAC table");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"An error ocurred while flushing all dynamic MAC entries");
    rc = L7_FAILURE;
  }
  else
  {
    PT_LOG_TRACE(LOG_CTX_L2,"Success flushing all dynamic MAC entries");
  }
  ...
  {
    PT_LOG_TRACE(LOG_CTX_L2,"Success reloading MAC table");
  }
...
L7_RC_t ptin_l2_mac_table_entry_remove( ptin_switch_mac_entry *entry )
{
  ...
  if ( entry==L7_NULLPTR )
  {
    PT_LOG_WARN(LOG_CTX_L2,"No items to be removed");
    return L7_SUCCESS;
  }

  PT_LOG_TRACE(LOG_CTX_L2,"Entry to be removed:");
  PT_LOG_TRACE(LOG_CTX_L2," EVCid  = %u",entry->evcId);
  PT_LOG_TRACE(LOG_CTX_L2," Vlan   = %u",entry->vlanId);
  PT_LOG_TRACE(LOG_CTX_L2," MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",
            entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
  PT_LOG_TRACE(LOG_CTX_L2," Intf   = %u/%u",entry->intf.intf_type,entry->intf.intf_id);
  PT_LOG_TRACE(LOG_CTX_L2," Static = %u",entry->static_entry);
...
//  {
//    PT_LOG_ERR(LOG_CTX_L2,"Vlan %u and evcId %u are invalid",entry->vlanId,entry->evcId);
//    return L7_FAILURE;
//  }
    ...
    {
      PT_LOG_ERR(LOG_CTX_L2,"Can't get root vlan for evcId %u",entry->evcId);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_L2,"Vlan %u (extracted from evcId %u) is invalid",vlanId,entry->evcId);
      return L7_FAILURE;
    }
  }
  else
  ...
  PT_LOG_TRACE(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x will be removed",
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x not found",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Error removing entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_l2_mac_table_entry_add( ptin_switch_mac_entry *entry )
{
  ...
  PT_LOG_TRACE(LOG_CTX_L2,"Entry to be added:");
  PT_LOG_TRACE(LOG_CTX_L2," EVCid  = %u",entry->evcId);
  PT_LOG_TRACE(LOG_CTX_L2," Vlan   = %u",entry->vlanId);
  PT_LOG_TRACE(LOG_CTX_L2," MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",
            entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5]);
...
//  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Interface %u/%u is invalid",entry->intf.intf_type,entry->intf.intf_id);
    return L7_FAILURE;
  }
  ...
  if (fdbFind(keyToFind, L7_MATCH_EXACT, &fdbEntry)==L7_SUCCESS)
  {
    PT_LOG_WARN(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x exists (attached to intIfNum=%u)",
    ...
    {
      PT_LOG_WARN(LOG_CTX_L2,"Found entry matches the one to be added... nothing to be done.");
      return L7_SUCCESS;
    }
  ...
  PT_LOG_TRACE(LOG_CTX_L2,"Entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x (%s) will be added (attached to interface %u/%u)",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5],
  ...
  {
    PT_LOG_ERR(LOG_CTX_L2,"Error adding entry of Vlan=%u and MAC=%02x:%02x:%02x:%02x:%02x:%02x to intf=%u/%u",
            vlanId,entry->addr[0],entry->addr[1],entry->addr[2],entry->addr[3],entry->addr[4],entry->addr[5],
            entry->intf.intf_type,entry->intf.intf_id);
    return L7_FAILURE;
  }
