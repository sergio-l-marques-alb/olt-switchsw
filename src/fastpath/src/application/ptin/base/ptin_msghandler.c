...
int CHMessageHandler (ipc_msg *inbuffer, ipc_msg *outbuffer)
{
  ...
  switch (inbuffer->msgId)
  {
    ...
    case CHMSG_ETH_UPLINK_COMMAND:
    {
      ...
      SETIPCACKOK(outbuffer);
      PT_LOG_INFO(LOG_CTX_MSGHANDLER, "Message processed: no response necessary");
    }
    break;
    ...
    case CCMSG_ETH_EVC_ADD:
    {
      ...
      if (L7_SUCCESS != rc)
      {
        PT_LOG_ERR(LOG_CTX_MSGHANDLER, "Error while creating EVC# %u", evcConf->id);
        res = SIR_ERROR(ERROR_FAMILY_HARDWARE, ERROR_SEVERITY_ERROR, SIRerror_get(rc));
        SetIPCNACK(outbuffer, res);
        break;
      }
    ...
    case CCMSG_ETH_EVC_OPTIONS_SET:
    {
      ...
      if (L7_SUCCESS != rc)
      {
        PT_LOG_ERR(LOG_CTX_MSGHANDLER, "Error while reconfiguring EVC# %u", evcOptions->id);
        res = SIR_ERROR(ERROR_FAMILY_HARDWARE, ERROR_SEVERITY_ERROR, SIRerror_get(rc));
        SetIPCNACK(outbuffer, res);
        break;
      }
    ...
    **************************************************************************/
    ...
    {
      PT_LOG_TRACE(LOG_CTX_MSGHANDLER, "Message received: CCMSG_MGMD_PORT_SYNC (0x%04X)", inbuffer->msgId);
    ...
    {      
      ...
      PT_LOG_INFO(LOG_CTX_MSGHANDLER, "Message received: CCMSG_MGMD_SNOOP_SYNC (0x%04X)", inbuffer->msgId);
