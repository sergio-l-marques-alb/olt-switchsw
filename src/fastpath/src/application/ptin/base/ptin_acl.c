...
void  ptin_aclIpClean(L7_BOOL isAclAdded, L7_uint32 mngAclId, L7_uint32 mngRuleId)
{
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing Rule ID %d on ACL ID %d", ptin_aclIpDb[mngAclId].aclRuleNum[mngRuleId], ptin_aclIpDb[mngAclId].aclId);
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing ACL ID %d", ptin_aclIpDb[mngAclId].aclId);
...
void  ptin_aclIpv6Clean(L7_BOOL isAclAdded, L7_uint32 mngAclId, L7_uint32 mngRuleId)
{
  ...
  if( isAclAdded == L7_FALSE)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing Rule ID %d on ACL ID %d", ptin_aclIpv6Db[mngAclId].aclRuleNum[mngRuleId], ptin_aclIpv6Db[mngAclId].aclId);
  ...
  if( isAclAdded == L7_TRUE)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing ACL ID %d", ptin_aclIpv6Db[mngAclId].aclId);
...
void ptin_aclMacClean(L7_BOOL isAclAdded, L7_uint32 mngAclId, L7_uint32 mngRuleId)
{
  ...
  if (isAclAdded == L7_FALSE)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing Rule ID %d on ACL ID %d", ptin_aclMacDb[mngAclId].aclRuleNum[mngRuleId], ptin_aclMacDb[mngAclId].aclId);
  ...
  if (isAclAdded == L7_TRUE)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing ACL ID %d", ptin_aclMacDb[mngAclId].aclId);
...
L7_RC_t ptin_aclIpRuleConfig(msg_ip_acl_t *msgAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Removing MNG Rule ID %d on MNG ACL ID %d", msgAcl->aclRuleId, msgAcl->aclId);
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Entry is empty");
    }
  ...
  LOG_PT_DEBUG(LOG_CTX_MSG, "Applying Rule...");
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid Action (%d)", msgAcl->action);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid ACL Type (%d)", msgAcl->aclType);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Every");
    matchEvery = L7_TRUE;
  }
  ...
  if (matchEvery == L7_FALSE)
  {
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Assuming protocol is IP");
    ...
    if (aclType != ACL_STANDARD)
    {
      if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_PROTOCOL_FEATURE_ID) == L7_TRUE)
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid Protocol Value [0..255] (%d)", protVal);
          return L7_FAILURE;
        }
    ...
    if ( (aclType != ACL_STANDARD) && !(msgAcl->aclRuleMask & ACL_IP_RULE_MASK_srcIpAddr) )
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: L7_ACL_RULE_MATCH_DSTIP_FEATURE_ID");
        return L7_FAILURE;
      }
      matchSrc = L7_TRUE;
    }
    else if ( (aclType == ACL_STANDARD) && !(msgAcl->aclRuleMask & ACL_IP_RULE_MASK_srcIpAddr) )
    {
      ...
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: In Standard ACL Type, Src IP must be specified");
      return L7_FAILURE;
    }
    ...
    if (matchSrc == L7_FALSE && usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_SRCIP_FEATURE_ID) == L7_TRUE)
    {
      if ((aclType != ACL_STANDARD) && (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_DSTIP_FEATURE_ID) == L7_TRUE))
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: L7_ACL_RULE_MATCH_SUPPORTS_MASKING_FEATURE_ID");
          return L7_FAILURE;
        }        
      }

      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Src IP");
  ...
  if (aclType == ACL_EXTENDED)
  {
    ...
    if (msgAcl->aclRuleMask & ACL_IP_RULE_MASK_srcStartPort)
    {
      ...
      {
        LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Src L4 Port");
        ...
        if(srcPortValue < L7_ACL_MIN_L4PORT_NUM || srcPortValue > L7_ACL_MAX_L4PORT_NUM)
        {
          ...
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid start SRCL4 port. <select SRCL4 port between 0 to 65535>");
          return L7_FAILURE;
        }
      }
      else
      {
        LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Src L4 Port Range");
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Dst IP");
    ...
    if (msgAcl->aclRuleMask & ACL_IP_RULE_MASK_dstStartPort)
    {
      ...
      {
        LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Dst L4 Port");
        ...
        if(dstPortValue < L7_ACL_MIN_L4PORT_NUM || dstPortValue > L7_ACL_MAX_L4PORT_NUM)
        {
          ...
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid start DSTL4 port. <select DSTL4 port between 0 to 65535>");
          return L7_FAILURE;
        }
      }
      else
      {
        LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Dst L4 Port Range");
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching PREC");
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching TOS");
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching DSCP");
      ...
      {
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: DSCP value must be between 0 to 63");
          return L7_FAILURE;
      }
      else
      {
  ...
  if (configuringNamedIpAcl == L7_FALSE)
  {
    if (ptin_aclIpDb[msgAcl->aclId].aclId == 0) /* ACL does not exists */
    {
      ...
      switch (rc)
      {
        ...
        case L7_FAILURE:
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclCreate return L7_FAILURE");
          return L7_FAILURE;

        case L7_TABLE_IS_FULL:
          usmDbQosAclMaxNumGet(unit, &maxAcls);           /* This function always returns L7_SUCCESS */
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclCreate return L7_TABLE_IS_FULL (maxAcls=%d)", maxAcls);
          return L7_FAILURE;

        case L7_ERROR:
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclCreate return L7_ERROR");
          return L7_FAILURE;

        case L7_REQUEST_DENIED:
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclCreate return L7_REQUEST_DENIED");
          return L7_FAILURE;

        default:
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclCreate return (rc=%d)", rc);
          return L7_FAILURE;
      }

      /* keep the association between MNG and usmDb ACL IDs */
      ptin_aclIpDb[msgAcl->aclId].aclId = aclId;
      LOG_PT_DEBUG(LOG_CTX_MSG, "Created New ACL: ID %d (MNG ID %d)", aclId, msgAcl->aclId);
      isAclAdded = L7_TRUE;
    }
    else  /* ACL already exists */
    {
      aclId = ptin_aclIpDb[msgAcl->aclId].aclId;
      LOG_PT_DEBUG(LOG_CTX_MSG, "Configuring existing ACL: ID %d (MNG ID %d)", aclId, msgAcl->aclId);
    }
  }
  else
  {
    ...
    if (usmDbQosAclNameToIndex(unit, accessListName, &aclId) != L7_SUCCESS)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        if (rc == L7_TABLE_IS_FULL)
        {
          ...
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclNamedIndexNextFree return L7_TABLE_IS_FULL (maxAcls=%d)", maxAcls);
          return L7_FAILURE;
        }
        else
        {
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclNamedIndexNextFree return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
      ...
      if (usmDbQosAclNameAdd(unit, aclId, accessListName) != L7_SUCCESS)
      {
        ...
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclNameAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
  }
  ...
  if (ptin_aclIpDb[msgAcl->aclId].aclRuleNum[msgAcl->aclRuleId] != 0)
  {
    ...
    LOG_PT_DEBUG(LOG_CTX_MSG, "Configuring existing Rule ID %d (MNG ID %d) on ACL ID %d (MNG ID %d)", aclRuleNum, msgAcl->aclRuleId, aclId, msgAcl->aclId);
  }
  ...
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: aclRuleNum > L7_ACL_MAX_RULE_NUM");
      return L7_FAILURE;
    }
    ...
    LOG_PT_DEBUG(LOG_CTX_MSG, "Created New Rule ID %d (MNG ID %d) on ACL ID %d (MNG ID %d)", aclRuleNum, msgAcl->aclRuleId, aclId, msgAcl->aclId);
  }
  ...
  switch (rc)
  {
    ...
    case L7_FAILURE:
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleActionAdd return L7_FAILURE");
      return L7_FAILURE;

    case L7_ERROR:
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleActionAdd return L7_ERROR");
      return L7_FAILURE;

    case L7_TABLE_IS_FULL:
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleActionAdd return L7_TABLE_IS_FULL");
      return L7_FAILURE;

    case L7_REQUEST_DENIED:
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleActionAdd return L7_REQUEST_DENIED");
      return L7_FAILURE;

    default:
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleActionAdd return (rc=%d)", rc);
      return L7_FAILURE;
  }
      ...
      (aclType != ACL_STANDARD && matchSrc == L7_TRUE && matchDst == L7_TRUE && matchOther == L7_FALSE))
  {
    ...
    {
      ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleEveryAdd return L7_FAILURE");
      return L7_FAILURE;
    }
    ...
    {
      ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleEveryAdd return L7_ERROR");
      return L7_FAILURE;
    }
    ...
    {
      ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleEveryAdd return L7_REQUEST_DENIED");
      return L7_FAILURE;
    }
  }
  else
  {
    if (matchSrc == L7_FALSE)
    {
      if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_SRCIP_FEATURE_ID) == L7_TRUE)
      {
        ...
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleSrcIpMaskAdd return L7_FAILURE");
          return L7_FAILURE;
        }
        else if (rc == L7_REQUEST_DENIED)
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleSrcIpMaskAdd return L7_REQUEST_DENIED");
          return L7_FAILURE;
        }
      }
    }
    ...
    if (aclType != ACL_STANDARD)
    {
      ...
      if (protVal != 0)
      {
        ...
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleProtocolAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
      if (srcPortValue > -1)
      ...
      {
        ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleSrcL4Port(Range)Add return (rc=%d)", rc);
        return L7_FAILURE;
      }
      ...
      if (matchDst == L7_FALSE)
      {
        if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_DSTIP_FEATURE_ID) == L7_TRUE)
        {
          ...
          {
            ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
            LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleDstIpMaskAdd return L7_FAILURE");
            return L7_FAILURE;
          }
          else if (rc == L7_REQUEST_DENIED)
          {
            ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
            LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleDstIpMaskAdd return L7_REQUEST_DENIED");
            return L7_FAILURE;
          }
        }
      }
      ...
      {
        ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleDstL4PortRangeAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
      ...
      if (precVal > -1)
      {
        ...
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleIPPrecedenceAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
      ...
      if (tosVal > -1)
      {
        ...
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleIPTosAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
      ...
      if (dscpVal > -1)
      {
        ...
        {
          ptin_aclIpClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleIPDscpAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
    }
  ...
  LOG_PT_DEBUG(LOG_CTX_MSG, "Rule Applied!");
...
L7_RC_t ptin_aclIpApply(ptin_acl_apply_t *aclApply, ACL_OPERATION_t operation)
{
      ...
      aclApply->vlanId[0] != L7_ACL_INVALID_VLAN_ID)
  {
    ...
    LOG_PT_DEBUG(LOG_CTX_MSG, "Applying ACL to VLAN ID %d", vlanId);

    /* verify if the specified value is in valid range */
    if (usmDbQosAclIsValidVlan(unit, vlanId) == L7_FALSE)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: VLAN ID is invalid");
      return L7_FAILURE;
    }
  }
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Cannot convert port %u to intIfNum", aclApply->interface);
      return L7_FAILURE;
    }
    if (ptin_intf_port2ptintf(aclApply->interface, &ptin_intf) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Cannot convert port %u to ptin_intf", aclApply->interface);
      return L7_FAILURE;
    }
    LOG_PT_DEBUG(LOG_CTX_MSG, "Applying ACL to Interface %d", aclApply->interface);
  }
  ...
  if (aclApply->aclType == ACL_TYPE_IP_NAMED)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: ACL Name NOT found");
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: ACD ID out of range");
    return L7_FAILURE;
  }
  ...
  if (rc == L7_FAILURE)
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclNumCheckValid return (%d)", rc);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid direction (%d)", aclApply->direction);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: L7_FLEX_QOS_ACL_COMPONENT_ID");
    return L7_FAILURE;
  }

  if (aclVlanIdGiven == L7_FALSE)
  {
    LOG_PT_TRACE(LOG_CTX_MSG, "operation=%d intIfNum=%d direction=%d aclId=%u sequence=%d", operation, intIfNum, direction, aclId, sequence);
    ...
    else /* ACL_OPERATION_REMOVE */
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_DEBUG(LOG_CTX_MSG, "No ACL to be Unapplied");
        return L7_SUCCESS;
      }
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclInterfaceDirectionRemove return (rc=%d)", rc);
      status = L7_FAILURE;
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_PT_TRACE(LOG_CTX_MSG, "operation=%d vlanId=%d direction=%d aclId=%u sequence=%d", operation, vlanId, direction, aclId, sequence);
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE (rc=%d)", rc);
      return L7_FAILURE;
    }

    LOG_PT_DEBUG(LOG_CTX_MSG, "Translating VLAN ID %d to %d (rc=%d)", oldVlan, (L7_uint32) newVlan, rc);
    ...
    if (operation == ACL_OPERATION_CREATE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclVlanDirectionAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
    else  /* ACL_OPERATION_REMOVE */
...
L7_RC_t ptin_aclIpv6RuleConfig(msg_ipv6_acl_t *msgAcl, ACL_OPERATION_t operation)
{
  ...
  if (matchEvery == L7_FALSE)
  {
    ...
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Assuming protocol is IPv6");
      msgAcl->protocol = L7_ACL_PROTOCOL_IP;
    }
    ...
    if (matchSrc == L7_FALSE && usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_SRCIP_FEATURE_ID) == L7_TRUE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: L7_ACL_RULE_MATCH_IPV6_DSTIP_FEATURE_ID");
        return L7_FAILURE;
      }
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Flow-label");
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: flow-label value must be between 0 to 1048575");
      return L7_FAILURE;
    }
    else
    {
      ...
      (matchSrc == L7_TRUE && matchDst == L7_TRUE && matchOther == L7_FALSE))
  {
    ...
    {
      ptin_aclIpv6Clean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleEveryAdd return (rc=%d)", rc);
      return L7_FAILURE;
    }
  }
  else
  {
    if (matchSrc == L7_FALSE)
    {
      if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_IPV6_SRCIP_FEATURE_ID) == L7_TRUE)
      {
        ...
        {
          ptin_aclIpv6Clean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleSrcIpv6AddrAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
    }
    ...
    if (matchDst == L7_FALSE)
    {
      if (usmDbFeaturePresentCheck(unit, L7_FLEX_QOS_ACL_COMPONENT_ID, L7_ACL_RULE_MATCH_IPV6_DSTIP_FEATURE_ID) == L7_TRUE)
      {
        ...
        {
          ptin_aclIpv6Clean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
          LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleDstIpv6AddrAdd return (rc=%d)", rc);
          return L7_FAILURE;
        }
      }
    }
    ...
    {
      ptin_aclIpv6Clean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleDstL4Port(Range)Add return (rc=%d)", rc);
      return L7_FAILURE;
    }
    ...
    if (flowLabelVal > -1)
    {
      ...
      {
        ptin_aclIpv6Clean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclRuleIpv6FlowLabelAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
  }
...
L7_RC_t ptin_aclIpv6Apply(ptin_acl_apply_t *aclApply, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid ACL Type");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Ivalid direction (%d)", aclApply->direction);
    return L7_FAILURE;
  }
  ...
  if (aclVlanIdGiven == L7_FALSE)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclInterfaceDirectionAdd return (rc=%d)", rc);
      status = L7_FAILURE;
      return L7_FAILURE;
    }
  }
...
L7_RC_t ptin_aclMacRuleConfig(msg_mac_acl_t *msgAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Src MAC");
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Dst MAC");
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching Ethertype");
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching VLAN");
    if (msgAcl->startVlan == msgAcl->endVlan)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid VLAN ID [1..4094] (%d)", vlanVal);
        return L7_FAILURE;
      }
    }
    else
    {
      LOG_PT_DEBUG(LOG_CTX_MSG, "Matching VLAN Range");
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid VLAN ID [1..4094] (%d:%d)", startVlanVal, endVlanVal);
        return L7_FAILURE;
      }
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Matching COS");
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: Invalid COS Value [0..7] (%d)", cosVal);
      return L7_FAILURE;
    }
    matchOther = L7_TRUE;
  }
  ...
  if (usmDbQosAclMacNameToIndex(unit, macAccessListName, &aclId) != L7_SUCCESS)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      if (rc == L7_TABLE_IS_FULL)
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacIndexNext return L7_TABLE_IS_FULL");
        return L7_FAILURE;
      }
      else
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacIndexNext return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
    ...
    switch (rc)
    {
      ...
      case L7_TABLE_IS_FULL:
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacCreate return L7_TABLE_IS_FULL");
        return L7_FAILURE;

      case L7_REQUEST_DENIED:
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacCreate return L7_REQUEST_DENIED");
        return L7_FAILURE;

      default:
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacCreate return (rc=%d)", rc);
        return L7_FAILURE;
    }
    ...
    if (usmDbQosAclMacNameAdd(unit, aclId, macAccessListName) != L7_SUCCESS)
    {
      ...
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacNameAdd return (rc=%d)", rc);
      return L7_FAILURE;
    }
  }
  ...
  default:
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleActionAdd return (rc=%d)", rc);
    return L7_FAILURE;
  }
  ...
  if (matchOther == L7_FALSE && matchSrc == L7_TRUE && matchDst == L7_TRUE)
  {
    ...
    {
      ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleEveryAdd return L7_FAILURE");
      return L7_FAILURE;
    }
    ...
    {
      ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleEveryAdd return L7_ERROR");
      return L7_FAILURE;
    }
    ...
    {
      ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleEveryAdd return L7_REQUEST_DENIED");
      return L7_FAILURE;
    }
  }
  else
  {
    if (matchSrc == L7_FALSE)
    {
      ...
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleSrcMacAdd return L7_FAILURE");
        return L7_FAILURE;
      }
      else if (rc == L7_REQUEST_DENIED)
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleSrcMacAdd return L7_REQUEST_DENIED");
        return L7_FAILURE;
      }
    }
    ...
    if (matchDst == L7_FALSE)
    {
      ...
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleDstMacAdd return L7_FAILURE");
        return L7_FAILURE;
      }
      else if (rc == L7_REQUEST_DENIED)
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleDstMacAdd return L7_REQUEST_DENIED");
        return L7_FAILURE;
      }
    }
    ...
    if (eTypeKeyVal > -1 && eTypeKeyVal != 0)
    {
      ...
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleEtypeKeyAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
    ...
    {
      ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleVlanIdRangeAdd return (rc=%d)", rc);
      return L7_FAILURE;
    }
    ...
    if (cosVal > -1)
    {
      ...
      {
        ptin_aclMacClean(isAclAdded, msgAcl->aclId, msgAcl->aclRuleId);
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacRuleCosAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
...
L7_RC_t ptin_aclMacApply(ptin_acl_apply_t *aclApply, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: ACD ID Invalid");
    return L7_FAILURE;
  }
  ...
  if (aclVlanIdGiven == L7_FALSE)
  {
      ...
      for (i=0; i<L7_MAX_NUM_RULES_PER_ACL; i++)
      {
        ...
        if ( (ptin_aclMacDb[aclApply->aclId].aclRuleNum[i] != 0) && !(ptin_aclMacDb[aclApply->aclId].aclRuleXlated[i]))
        {
          ...
          else if (oldVlan != 0)
          {
            ...
            if (rc == L7_SUCCESS)
            {
              LOG_PT_DEBUG(LOG_CTX_MSG, "Rule %d: Translating VLAN ID %d to %d (rc=%d)", i, oldVlan, (L7_uint32) newVlan, rc);
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacInterfaceDirectionAdd return (rc=%d)", rc);
      status = L7_FAILURE;
  ...
  else
  {
    ...
    if (operation == ACL_OPERATION_CREATE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL FAILURE: usmDbQosAclMacVlanDirectionAdd return (rc=%d)", rc);
        return L7_FAILURE;
      }
    }
    else  /* ACL_OPERATION_REMOVE */
...
L7_RC_t ptin_aclArpRuleConfig(msg_arp_acl_t *msgAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "Invalid ACL_TYPE (%u)!", msgAcl->aclType);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "Only IPv4 is supported!");
    return L7_FAILURE;
  }
  ...
  if (msgAcl->name[0] == '\0')
  {
    LOG_PT_ERR(LOG_CTX_MSG, "ACL name is empty!");
    return L7_FAILURE;
  }
  ...
  if (operation == ACL_OPERATION_REMOVE)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "ACL group \"%s\" does not exist", msgAcl->name);
      return L7_NOT_EXIST;
    }
    /* Check if entry does not exist */
    if (arpAclRuleGet(msgAcl->name, msgAcl->srcIpAddr.addr.ipv4, &srcMacAddr) != L7_SUCCESS)
    {
      LOG_PT_WARN(LOG_CTX_MSG, "ACL group \"%s\" does not exist", msgAcl->name);
      return L7_NOT_EXIST;
    }
    /* Remove entry */
    if (arpAclRuleDelete(msgAcl->name, msgAcl->srcIpAddr.addr.ipv4, &srcMacAddr) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "Error removing ACL rule %02x:%02x:%02x:%02x:%02x:%02x / %03u.%03u.%03u.%03u from \"%s\" group",
    ...
    if (arpAclRuleInAclNextGet(msgAcl->name, ipAddr, &srcMacAddr, &ipAddr, &srcMacAddr) != L7_SUCCESS)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Error deleting ACL group \"%s\"", msgAcl->name);
        return L7_FAILURE;
      }
    }
    return L7_SUCCESS;
  ...
  else if (operation == ACL_OPERATION_CREATE)
  {
    ...
    if (msgAcl->action != ACL_ACTION_PERMIT)
    {
      LOG_PT_ERR(LOG_CTX_MSG, "Only PERMIT action is allowed");
      return L7_FAILURE;
    }
    ...
    if (arpAclGet(msgAcl->name) != L7_SUCCESS)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Error creating ACL group \"%s\"", msgAcl->name);
        return L7_FAILURE;
      }
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "Error adding ACL rule %02x:%02x:%02x:%02x:%02x:%02x / %03u.%03u.%03u.%03u to \"%s\" group",
              msgAcl->srcMacAddr[0],msgAcl->srcMacAddr[1],msgAcl->srcMacAddr[2],msgAcl->srcMacAddr[3],msgAcl->srcMacAddr[4],msgAcl->srcMacAddr[5],
              (msgAcl->srcIpAddr.addr.ipv4>>24) & 0xff, (msgAcl->srcIpAddr.addr.ipv4>>16) & 0xff, (msgAcl->srcIpAddr.addr.ipv4>>8) & 0xff, msgAcl->srcIpAddr.addr.ipv4 & 0xff,
              msgAcl->name);
      return L7_FAILURE;
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "Invalid operation %u", operation);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_aclArpApply(ptin_acl_apply_t *aclApply, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "Invalid ACL_TYPE (%u)!", aclApply->aclType);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_MSG, "Only ACL_DIRECTION_IN is allowed (%u)!", aclApply->direction);
    return L7_FAILURE;
  }
  ...
  for (i = 0; i < aclApply->number_of_vlans; i++)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "Invalid VLAN %u at index %u", vlanId, i);
      rc = L7_FAILURE;
      continue;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG, "VLAN %u is not valid!", vlanId);
      rc = L7_NOT_EXIST;
      continue;
    }
    ...
    {
      LOG_PT_TRACE(LOG_CTX_MSG, "ACL name is empty... changing operation to REMOVE");
    ...
    if (operation == ACL_OPERATION_CREATE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "ACL group \"%s\" does not exist", aclApply->name);
        rc = L7_NOT_EXIST;
        continue;
      }
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Error assigning ACL name to VLAN %u", vlanId);
        rc = L7_FAILURE;
        continue;
      }
    }
    ...
    else if (operation == ACL_OPERATION_REMOVE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Error removing ACL name from VLAN %u", vlanId);
        rc = L7_FAILURE;
        continue;
      }
    }
...
L7_RC_t ptin_aclIpDbDump(L7_uint32 mngAclId)
{
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "Invalid ACL ID %d", mngAclId);
    return L7_FAILURE;
  }

  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "IP ACL Info: MNG ID %d -> ACL ID %d",        mngAclId, ptin_aclIpDb[mngAclId].aclId);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "  Number of Rules: %d",                      ptin_aclIpDb[mngAclId].aclRuleCount);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");

  for (i=1; i<=L7_MAX_NUM_RULES_PER_ACL; i++)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "  Rule Info: MNG ID %2d -> Rule ID %2d",   i, ptin_aclIpDb[mngAclId].aclRuleNum[i]);
  }
...
L7_RC_t ptin_aclIpv6DbDump(L7_uint32 mngAclId)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "IPv6 ACL Info: MNG ID %d -> ACL ID %d",      mngAclId, ptin_aclIpv6Db[mngAclId].aclId);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "  Number of Rules: %d",                      ptin_aclIpv6Db[mngAclId].aclRuleCount);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");

  for (i=1; i<=L7_MAX_NUM_RULES_PER_ACL; i++)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "  Rule Info: MNG ID %2d -> Rule ID %2d",   i, ptin_aclIpv6Db[mngAclId].aclRuleNum[i]);
  }
...
L7_RC_t ptin_aclMacDbDump(L7_uint32 mngAclId)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "MAC ACL Info: MNG ID %d -> ACL ID %d",       mngAclId, ptin_aclMacDb[mngAclId].aclId);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_PT_DEBUG(LOG_CTX_MSG, "  Number of Rules: %d",                      ptin_aclMacDb[mngAclId].aclRuleCount);
  LOG_PT_DEBUG(LOG_CTX_MSG, "-------------------------------------------");

  for (i=1; i<=L7_MAX_NUM_RULES_PER_ACL; i++)
  {
    LOG_PT_DEBUG(LOG_CTX_MSG, "  Rule Info: MNG ID %2d -> Rule ID %2d",   i, ptin_aclMacDb[mngAclId].aclRuleNum[i]);
  }
