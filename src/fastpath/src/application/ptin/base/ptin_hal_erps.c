...
L7_RC_t ptin_hal_erps_init(void)
{
  ...
  {
    PT_LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_hal_erps_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_ERPS,"Task ptin_hal_apsPacketTx_task created");

  if (osapiWaitForTaskInit (L7_PTIN_APS_PACKET_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS) {
    PT_LOG_FATAL(LOG_CTX_ERPS,"Unable to initialize ptin_hal_apsPacketTx_task()\n");
    return(L7_FAILURE);
  }
  PT_LOG_TRACE(LOG_CTX_ERPS,"Task ptin_hal_apsPacketTx_task initialized");
...
L7_RC_t ptin_hal_erps_counters_tx(L7_uint8 erps_idx, L7_uint8 port, L7_uint8 req, L7_uint8 units)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_ERPS,"ERPS#%d: port %d not valid", erps_idx, port);
    return PROT_ERPS_INDEX_VIOLATION;
  }
  ...
  {
    PT_LOG_TRACE(LOG_CTX_ERPS,"Unknown Req: 0x%02X, erps_idx %d, port %d", req, erps_idx, port);
  }
  ...
  for (byte=0; byte<(sizeof(tbl_erps[erps_idx].protParam.vid_bmp)); byte++) {
    for (bit=0; bit<8; bit++) {
      ...
      if ((tbl_erps[erps_idx].protParam.vid_bmp[byte] >> bit) & 1) {       
        PT_LOG_DEBUG(LOG_CTX_ERPS, "ERPS#%d: VLAN ID %d Added", erps_idx, vid);
      ...
      else if (erpsIdx_from_serviceVid[vid] == erps_idx)
      {
        ...
        PT_LOG_DEBUG(LOG_CTX_ERPS, "ERPS#%d: VLAN ID %d Removed", erps_idx, vid);
      }
    } // for(bit...)
  } // for(byte...)
...
L7_RC_t ptin_hal_erps_deinit(void)
{
  ...
  {
    PT_LOG_FATAL(LOG_CTX_CNFGR, "Failed to delete ptin_hal_erps_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  switch (sig) 
    {
      case PTIN_ERPS_WAKE_UP_SIGNAL:
        //PT_LOG_ERR(LOG_CTX_ERPS, "ERPS: Tx R-APS WAKE UP SIGNAL Handler!");
        break;
      default:
        break;
    }
...
L7_RC_t ptin_hal_erps_sendaps(L7_uint8 erps_idx, L7_uint8 req, L7_uint8 status)
{
  ...
  //PT_LOG_ERR(LOG_CTX_ERPS, "ERPS#%d: Tx R-APS WAKE UP SIGNAL Sent!", erps_idx);
...
void ptin_hal_apsPacketTx_task(void)
{
  ...
  PT_LOG_INFO(LOG_CTX_ERPS,"PTin APS packet task ready to process events");
  ...
  while (1) {
    ...
    for (erps_idx=0; erps_idx<MAX_PROT_PROT_ERPS; erps_idx++)
    {
      ...
      if (req != RReq_NONE)
      {
        ...
        if (counter > 0)
        {
          PT_LOG_NOTICE(LOG_CTX_ERPS, "ERPS#%d: Tx R-APS Req %d (reqStatus 0x%02X)", erps_idx, req, reqStatus);
          osapiSemaTake(ptin_hal_erps_sem, L7_WAIT_FOREVER);
...
static L7_RC_t ptin_hal_erps_queue_vlans_used_init(void)
{
  ...
  for (erps_idx=0; erps_idx<MAX_PROT_PROT_ERPS; erps_idx++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_ERPS, "Unable to create new dl_queue %d for VLANs in use", erps_idx);
      return L7_FAILURE;
    }
  }
...
struct vlan_entry_s *ptin_hal_erps_queue_vlan_used_find(int erps_idx, L7_uint16 internalVlan)
{
  ...
  {
    PT_LOG_DEBUG(LOG_CTX_ERPS, "VLANs queue %d is empty", erps_idx);
    return NULL;
  }
...
static L7_RC_t ptin_hal_erps_queue_vlan_used_add(int erps_idx, L7_uint16 internalVlan)
{
  ...
  {
    PT_LOG_TRACE(LOG_CTX_ERPS, "VLAN %d already exists on queue %d", internalVlan, erps_idx);
    return L7_SUCCESS;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_ERPS, "VLAN %d added to queue %d", internalVlan, erps_idx);
...
static L7_RC_t ptin_hal_erps_queue_vlan_used_remove(int erps_idx, L7_uint16 internalVlan)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_ERPS, "VLANs %d not found on queue %d", internalVlan, erps_idx);
    return L7_SUCCESS;
  }

  PT_LOG_TRACE(LOG_CTX_ERPS, "VLANs %d added to queue %d", internalVlan, erps_idx);
...
L7_RC_t ptin_hal_erps_internal_vlans_used_sync(L7_uint8 erps_idx)
{
  ...
  PT_LOG_TRACE(LOG_CTX_ERPS, "Adding VLANs to queue %d", erps_idx);
...
L7_RC_t ptin_hal_erps_hwSync(L7_uint8 erps_idx)
{
  ...
  {
    PT_LOG_DEBUG(LOG_CTX_ERPS,"ERPS#%d: HW Sync in progress...", erps_idx);
    ...
    if(NOERR != dl_queue_get_head(&queue_vlans_used[erps_idx], (dl_queue_elem_t**)&vlan_entry))
    {
      tbl_halErps[erps_idx].hwSync = 0;
      osapiSemaGive(ptin_hal_erps_sem);
      PT_LOG_DEBUG(LOG_CTX_ERPS, "VLANs queue is empty");
      return L7_FAILURE;
    }
    ...
    PT_LOG_DEBUG(LOG_CTX_ERPS,"ERPS#%d: HW Sync done!", erps_idx);
...
L7_RC_t ptin_hal_erps_hwFdbFlush(L7_uint8 erps_idx)
{
  ...
  {
    PT_LOG_DEBUG(LOG_CTX_ERPS,"ERPS#%d: HW FdbFlush in progress...", erps_idx);
    ...
    while (vlan_entry != NULL)
    {
      ...
      if (internalVlan != 0)
      {
        ...
        {
          //PT_LOG_DEBUG(LOG_CTX_ERPS,"ERPS#%d: Flushing VLAN ID %d", erps_idx, internalVlan);
    ...
    PT_LOG_DEBUG(LOG_CTX_ERPS,"ERPS#%d: HW Flush done!", erps_idx);
...
L7_BOOL ptin_hal_erps_isPortBlocked(L7_uint root_intf, L7_uint16 vlan, L7_uint16 internalVlan)
{
  ...
  if (vlan >= 4096)
  {
    PT_LOG_ERR(LOG_CTX_ERPS,"VLAN %u not valid", vlan);
    return PROT_ERPS_INDEX_VIOLATION;
  }
  ...
  if (erps_idx != PROT_ERPS_UNUSEDIDX)
  {
    ...
    if ( (root_intf == tbl_erps[erps_idx].protParam.port0.idx) || (root_intf == tbl_erps[erps_idx].protParam.port1.idx) )
    {
      if ( (vlan < 1<<12) && (tbl_erps[erps_idx].protParam.vid_bmp[vlan/8] & 1<<(vlan%8)) ) //ERP protected VID
      {
        ...
        if ( (root_intf == tbl_erps[erps_idx].protParam.port0.idx) && (tbl_erps[erps_idx].portState[PROT_ERPS_PORT0] == ERPS_PORT_BLOCKING) )
        {
          ...
          PT_LOG_TRACE(LOG_CTX_ERPS, "root intf %u with Int.VLAN %u is blocked by ERPS#%d", root_intf, vlan, erps_idx);      
          return L7_TRUE;
        }
        ...
        PT_LOG_TRACE(LOG_CTX_ERPS, "root intf %u with Int.VLAN %u is NOT blocked by ERPS#%d", root_intf, vlan, erps_idx);      
        return L7_FALSE;
      }
    }
    ...
    PT_LOG_TRACE(LOG_CTX_ERPS, "EVC with root intf %u and Int.VLAN %u is NOT protected by ERPS#%d", root_intf, vlan, erps_idx);
    return L7_FALSE;
  }

  PT_LOG_TRACE(LOG_CTX_ERPS, "EVC with root intf %u and Int.VLAN %u is NOT protected by any ERPS", root_intf, vlan);
...
L7_BOOL ptin_hal_erps_evcProtectedRemove(L7_uint root_intf, L7_uint16 vlan, L7_uint16 internalVlan)
{
  ...
  if (erps_idx != PROT_ERPS_UNUSEDIDX)
  {
    ...
    if ( (root_intf == tbl_erps[erps_idx].protParam.port0.idx) || (root_intf == tbl_erps[erps_idx].protParam.port1.idx) )
    {
      ...
      PT_LOG_TRACE(LOG_CTX_ERPS, "EVC with root intf %u and Int.VLAN %u is protected by ERPS#%d", root_intf, vlan, erps_idx);      
