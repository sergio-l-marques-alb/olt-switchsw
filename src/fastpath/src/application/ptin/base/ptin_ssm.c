...
L7_RC_t ssm_init(void)
{
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR,"Error allocating buffers!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Unable to create ssmTimersSyncSema semaphore()");
    return(L7_FAILURE);
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Unable to create ssm rx task semaphore()");
    return(L7_FAILURE);
  }
  ssmTaskTxSyncSema = osapiSemaBCreate( OSAPI_SEM_Q_FIFO, OSAPI_SEM_FULL);
  if (ssmTaskTxSyncSema == L7_NULLPTR)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Unable to create ssm tx task semaphore()");
    return(L7_FAILURE);
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create SSM RX task!");
    return L7_FAILURE;
  }
  /* Wait for task to be launched */
  if (osapiWaitForTaskInit (L7_SSM_RX_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to start SSM RX task!");
    return L7_FAILURE;
  }
  LOG_INFO(LOG_CTX_CNFGR, "SSM RX task launch OK");
  ...
  if ( (ssm_task_tx_id = osapiTaskCreate("SSM tx task", ssm_task_pdu_transmit, 0, 0,
                                         ...
                                         L7_DEFAULT_TASK_SLICE)) == L7_ERROR)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create SSM TX task!");
    return L7_FAILURE;
  }
  /* Wait for task to be launched */
  if (osapiWaitForTaskInit (L7_SSM_TX_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to start SSM TX task!");
    return L7_FAILURE;
  }
  LOG_INFO(LOG_CTX_CNFGR, "SSM TX task launch OK");
...
L7_RC_t ssmPDUReceive(L7_netBufHandle bufHandle, sysnet_pdu_info_t *pduInfo)
{
  ...
  if (ssm_debug_enable)
  {
    LOG_TRACE(LOG_CTX_SSM,"Packet from intIfNum %u, rxPort %u", pduInfo->intIfNum, pduInfo->rxPort);
  }

  /* Remove tags */
  if (ssmPduHeaderTagRemove(bufHandle)!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_SSM,"Error extracting vlans");
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
      ...
      (dataLength > SSM_BUFFER_SIZE))
  {
    ...
    LOG_ERR(LOG_CTX_SSM,"Error allocating buffer");
  ...
  if ( rc != L7_SUCCESS )
  {
    ...
    /* By returning a success we ensure that this mbuf is not freed 
       again by sysnet*/
    LOG_ERR(LOG_CTX_SSM,"Error in processing pdu (%d)", rc);
    return L7_SUCCESS;
  }
...
void ssm_task_pdu_receive()
{
  ...
  if (ssm_queue == L7_NULLPTR)
  {
    LOG_FATAL(LOG_CTX_SSM, "Unable to create msg queue for ssm_task!");
    return;
  }
...
void ssm_task_pdu_transmit( void )
{
  ...
  {
    LOG_FATAL(LOG_CTX_SSM, "Error registering period timer! CRASH!");
    PTIN_CRASH();
  }
...
L7_RC_t ssmPDUProcess(L7_uint32 intf, void *buffer)
{
  ...
  if (ssm_debug_enable)
  {
    LOG_TRACE(LOG_CTX_SSM,"Packet received on interface %u:",intf);
    LOG_TRACE(LOG_CTX_SSM," DMAC=%02x:%02x:%02x:%02x:%02x:%02x",pdu->dmac[0],pdu->dmac[1],pdu->dmac[2],pdu->dmac[3],pdu->dmac[4],pdu->dmac[5]);
    LOG_TRACE(LOG_CTX_SSM," SMAC=%02x:%02x:%02x:%02x:%02x:%02x",pdu->smac[0],pdu->smac[1],pdu->smac[2],pdu->smac[3],pdu->smac[4],pdu->smac[5]);
    LOG_TRACE(LOG_CTX_SSM," EtherType     = 0x%04x",osapiNtohs(pdu->etherType));
    LOG_TRACE(LOG_CTX_SSM," SubType       = 0x%02x",pdu->subType);
    LOG_TRACE(LOG_CTX_SSM," ITU-OUI       = 0x%02x%02x%02x",pdu->itu_oui[0],pdu->itu_oui[1],pdu->itu_oui[2]);
    LOG_TRACE(LOG_CTX_SSM," ITU-T Subtype = 0x%04x",osapiNtohs(pdu->itut_subtype));
    LOG_TRACE(LOG_CTX_SSM," Version       = 0x%02x",pdu->version);
    LOG_TRACE(LOG_CTX_SSM," SSM Type      = 0x%02x",pdu->ssm_type);
    LOG_TRACE(LOG_CTX_SSM," SSM Length    = 0x%04x",osapiNtohs(pdu->ssm_length));
    LOG_TRACE(LOG_CTX_SSM," SSM Code      = 0x%02x",pdu->ssm_code);
    LOG_TRACE(LOG_CTX_SSM,"End of Packet processing");
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"ITU-T field is not valid: received=0x%x, expected=0x%x",itu_oui,SSM_ITU_OUI);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  /* Protocol version */
  if ((pdu->version >> 4) != 1)
  {
    LOG_ERR(LOG_CTX_SSM,"Protocol version is not the expected: received=%u, expected=1",pdu->version >> 4);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  /* SSM type */
  if (pdu->ssm_type!=1)
  {
    LOG_ERR(LOG_CTX_SSM,"SSM type is not the expected: received=%u, expected=1",pdu->ssm_type);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"SSM length is less than %u bytes (%u)",SSM_L4_LENGTH, osapiNtohs(pdu->ssm_length));
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
...
void ssmPDUSend(void)
{
  ...
  //LOG_INFO(LOG_CTX_SSM,"This is the place where i will transmit SSM messages!");
  ...
  for (slot=0; slot<SSM_N_SLOTS; slot++)
  {
    for (intf=0; intf<SSM_N_INTFS_IN_USE; intf++)
    {
      ...
      {
        //if (ssm_debug_enable)
        //  LOG_ERR(LOG_CTX_SSM,"Invalid slot %u / intf %u... no tranmission executed",slot,intf);
      }
    }
  }
}
...
L7_RC_t ssmPDUTransmit(L7_uint32 intIfNum)
{
  ...
  {
    //if (ssm_debug_enable)
    //  LOG_ERR(LOG_CTX_SSM,"Cannot convert intIfNum %u to slot/intf",intIfNum);
    return L7_FAILURE;
  }

  if (slot >= SSM_N_SLOTS || intf >= SSM_N_INTFS_IN_USE)
  {
    LOG_ERR(LOG_CTX_SSM,"Invalid slot/intf %u/%u", slot, intf);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"Error initializing bufHandle");
    return L7_FAILURE;
  }
  ...
  {
    if (ssm_debug_enable)
      LOG_ERR(LOG_CTX_SSM,"Error getting MAC address of intIfNum %u",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"Error transmiting packet");
    return L7_FAILURE;
  }

  if (ssm_debug_enable)
    LOG_TRACE(LOG_CTX_SSM,"SSM transmitted to intIfNum %u (slot=%u/intf=%u) with SSM_code=%x",intIfNum,slot,intf,SHMEM(slot,intf).ssm_tx);
...
L7_RC_t ssmTransmit(L7_uint32 intIfNum, L7_uint32 bufHandle)
{
  ...
  if (nimIntfQuery(&nimQueryData) != L7_SUCCESS)
  {
    ...
    //LOG_ERR(LOG_CTX_SSM,"nimIntfQuery error (intIfNum=%u)",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_FAILURE;
  }
           ...
           (nimQueryData.data.state != L7_INTF_ATTACHED))
  {
    ...
    //if (ssm_debug_enable)
    //  LOG_ERR(LOG_CTX_SSM,"intIfNum %u is not yet attached!",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"Error getting intIfNum %u status!",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"intIfNum %u is not physical",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    //if (ssm_debug_enable)
    //  LOG_ERR(LOG_CTX_SSM,"intIfNum %u not ready",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_SSM,"Error transmiting packet to intIfNum %u",intIfNum);
  }

  if (ssm_debug_enable)
    LOG_TRACE(LOG_CTX_SSM,"Packet transmitted successfully to intIfNum %u",intIfNum);
  ...
  for (slot=0; slot<SSM_N_SLOTS; slot++)
  {
    for (intf=0; intf<SSM_N_INTFS_IN_USE; intf++)
    {
      ...
      if (ssm_timer[slot][intf]>5)
      {
        ...
        {
          if (ssm_timer[slot][intf]==6)
            LOG_TRACE(LOG_CTX_SSM,"slot %u / intf %u timed out... ssm code is 0xf",slot,intf);
        }
      }
    }
  }
...
L7_RC_t ssmCodeUpdate(L7_uint32 intIfNum, L7_uint16 ssm_code)
{
  ...
  if (ssm_debug_enable)
    LOG_TRACE(LOG_CTX_SSM,"SSM code (0x%02x) updated for slot=%u, intf=%u (intIfNum=%u)",ssm_code,slot,intf,intIfNum);
