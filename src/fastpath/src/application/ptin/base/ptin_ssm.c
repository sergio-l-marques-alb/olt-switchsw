...
L7_RC_t ssmPDUReceive(L7_netBufHandle bufHandle, sysnet_pdu_info_t *pduInfo)
{
  ...
  if (ssm_debug_enable)
  {
    PT_LOG_TRACE(LOG_CTX_PACKET,"Packet from intIfNum %u, rxPort %u", pduInfo->intIfNum, pduInfo->rxPort);
  }

  /* Remove tags */
  if (ssmPduHeaderTagRemove(bufHandle)!=L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Error extracting vlans");
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
      ...
      (dataLength > SSM_BUFFER_SIZE))
  {
    ...
    PT_LOG_ERR(LOG_CTX_PACKET,"Error allocating buffer");
  ...
  if ( rc != L7_SUCCESS )
  {
    ...
    /* By returning a success we ensure that this mbuf is not freed 
       again by sysnet*/
    PT_LOG_ERR(LOG_CTX_PACKET,"Error in processing pdu (%d)", rc);
    return L7_SUCCESS;
  }
...
void ssm_task_pdu_receive()
{
  ...
  if (ssm_queue == L7_NULLPTR)
  {
    PT_LOG_FATAL(LOG_CTX_PACKET, "Unable to create msg queue for ssm_task!");
    return;
  }
...
void ssm_task_pdu_transmit( void )
{
  ...
  {
    PT_LOG_FATAL(LOG_CTX_PACKET, "Error registering period timer! CRASH!");
    PTIN_CRASH();
  }
...
L7_RC_t ssmPDUProcess(L7_uint32 intf, void *buffer)
{
  ...
  if (ssm_debug_enable)
  {
    PT_LOG_TRACE(LOG_CTX_PACKET,"Packet received on interface %u:",intf);
    PT_LOG_TRACE(LOG_CTX_PACKET," DMAC=%02x:%02x:%02x:%02x:%02x:%02x",pdu->dmac[0],pdu->dmac[1],pdu->dmac[2],pdu->dmac[3],pdu->dmac[4],pdu->dmac[5]);
    PT_LOG_TRACE(LOG_CTX_PACKET," SMAC=%02x:%02x:%02x:%02x:%02x:%02x",pdu->smac[0],pdu->smac[1],pdu->smac[2],pdu->smac[3],pdu->smac[4],pdu->smac[5]);
    PT_LOG_TRACE(LOG_CTX_PACKET," EtherType     = 0x%04x",osapiNtohs(pdu->etherType));
    PT_LOG_TRACE(LOG_CTX_PACKET," SubType       = 0x%02x",pdu->subType);
    PT_LOG_TRACE(LOG_CTX_PACKET," ITU-OUI       = 0x%02x%02x%02x",pdu->itu_oui[0],pdu->itu_oui[1],pdu->itu_oui[2]);
    PT_LOG_TRACE(LOG_CTX_PACKET," ITU-T Subtype = 0x%04x",osapiNtohs(pdu->itut_subtype));
    PT_LOG_TRACE(LOG_CTX_PACKET," Version       = 0x%02x",pdu->version);
    PT_LOG_TRACE(LOG_CTX_PACKET," SSM Type      = 0x%02x",pdu->ssm_type);
    PT_LOG_TRACE(LOG_CTX_PACKET," SSM Length    = 0x%04x",osapiNtohs(pdu->ssm_length));
    PT_LOG_TRACE(LOG_CTX_PACKET," SSM Code      = 0x%02x",pdu->ssm_code);
    PT_LOG_TRACE(LOG_CTX_PACKET,"End of Packet processing");
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"ITU-T field is not valid: received=0x%x, expected=0x%lx",itu_oui,SSM_ITU_OUI);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  /* Protocol version */
  if ((pdu->version >> 4) != 1)
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Protocol version is not the expected: received=%u, expected=1",pdu->version >> 4);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  /* SSM type */
  if (pdu->ssm_type!=1)
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"SSM type is not the expected: received=%u, expected=1",pdu->ssm_type);
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"SSM length is less than %u bytes (%u)",SSM_L4_LENGTH, osapiNtohs(pdu->ssm_length));
    bufferPoolFree(ssmBufferPoolId,  buffer);
    return L7_FAILURE;
  }
...
void ssmPDUSend(void)
{
  ...
  //PT_LOG_INFO(LOG_CTX_PACKET,"This is the place where i will transmit SSM messages!");
  ...
  for (slot=0; slot<SSM_N_SLOTS; slot++)
  {
    for (intf=0; intf<SSM_N_INTFS_IN_USE; intf++)
    {
      ...
      {
        //if (ssm_debug_enable)
        //  PT_LOG_ERR(LOG_CTX_PACKET,"Invalid slot %u / intf %u... no tranmission executed",slot,intf);
      }
    }
  }
}
...
L7_RC_t ssmPDUTransmit(L7_uint32 intIfNum)
{
  ...
  {
    //if (ssm_debug_enable)
    //  PT_LOG_ERR(LOG_CTX_PACKET,"Cannot convert intIfNum %u to slot/intf",intIfNum);
    return L7_FAILURE;
  }

  if (slot >= SSM_N_SLOTS || intf >= SSM_N_INTFS_IN_USE)
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Invalid slot/intf %u/%u", slot, intf);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Error initializing bufHandle");
    return L7_FAILURE;
  }
  ...
  {
    if (ssm_debug_enable)
      PT_LOG_ERR(LOG_CTX_PACKET,"Error getting MAC address of intIfNum %u",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Error transmiting packet");
    return L7_FAILURE;
  }

  if (ssm_debug_enable)
    PT_LOG_TRACE(LOG_CTX_PACKET,"SSM transmitted to intIfNum %u (slot=%u/intf=%u) with SSM_code=%x (length=%u)",
...
L7_RC_t ssmTransmit(L7_uint32 intIfNum, L7_uint32 bufHandle)
{
  ...
  if (nimIntfQuery(&nimQueryData) != L7_SUCCESS)
  {
    ...
    //PT_LOG_ERR(LOG_CTX_PACKET,"nimIntfQuery error (intIfNum=%u)",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_FAILURE;
  }
           ...
           (nimQueryData.data.state != L7_INTF_ATTACHED))
  {
    ...
    //if (ssm_debug_enable)
    //  PT_LOG_ERR(LOG_CTX_PACKET,"intIfNum %u is not yet attached!",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Error getting intIfNum %u status!",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"intIfNum %u is not physical",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    //if (ssm_debug_enable)
    //  PT_LOG_ERR(LOG_CTX_PACKET,"intIfNum %u not ready",intIfNum);
    SYSAPI_NET_MBUF_FREE(bufHandle);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_PACKET,"Error transmiting packet to intIfNum %u",intIfNum);
  }

  if (ssm_debug_enable)
    PT_LOG_TRACE(LOG_CTX_PACKET,"Packet transmitted successfully to intIfNum %u",intIfNum);
  ...
  for (slot=0; slot<SSM_N_SLOTS; slot++)
  {
    for (intf=0; intf<SSM_N_INTFS_IN_USE; intf++)
    {
      ...
      if (ssm_timer[slot][intf]>5)
      {
        ...
        {
          if (ssm_timer[slot][intf]==6)
            PT_LOG_TRACE(LOG_CTX_PACKET,"slot %u / intf %u timed out... ssm code is 0xf",slot,intf);
        }
      }
    }
  }
...
L7_RC_t ssmCodeUpdate(L7_uint32 intIfNum, L7_uint16 ssm_code)
{
  ...
  if (ssm_debug_enable)
    PT_LOG_TRACE(LOG_CTX_PACKET,"SSM code (0x%02x) updated for slot=%u, intf=%u (intIfNum=%u)",ssm_code,slot,intf,intIfNum);
