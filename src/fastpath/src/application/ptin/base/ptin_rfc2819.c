...
L7_RC_t ptin_rfc2819_config_probe(L7_int Port, L7_uint8 Admin)
{
  ...
  { 
    if (ptin_rfc2819_get_admin(&RFC2819_probes_Rx[Port]) || ptin_rfc2819_get_admin(&RFC2819_probes_Tx[Port])) {
      PT_LOG_INFO(LOG_CTX_RFC2819, "RFC2819 Probe (Port: %d) already enabled",Port);
    }
    else {
      PT_LOG_INFO(LOG_CTX_RFC2819, "Enable RFC2819 Probe (Port: %d)",Port);
  ...
  {
      PT_LOG_INFO(LOG_CTX_RFC2819, "Disable RFC2819 Probe (Port: %d)",Port);
...
void ptin_rfc2819_init_buffers(void)
{
  ...
  PT_LOG_INFO(LOG_CTX_RFC2819, "Initializing RFC2819 buffers");
...
void ptin_rfc2819_clear_buffers(void)
{
  ...
  for (i=0;i<MAX_QUAL_RFC2819_BUFFERS;i++) {
    PT_LOG_INFO(LOG_CTX_RFC2819, "Clearing RFC2819 buffers");
...
void ptin_rfc2819_init_all_probes(void)
{
  ...
  PT_LOG_INFO(LOG_CTX_STARTUP, "Initializing RFC2819 all Probes");
...
L7_RC_t ptin_rfc2819_init(void)
{
  ...
  if (ptin_rfc2819_TaskId == L7_ERROR) {
    PT_LOG_FATAL(LOG_CTX_RFC2819, "Could not create task ptin_rfc2819_task");
    return L7_FAILURE;
  }
  PT_LOG_TRACE(LOG_CTX_RFC2819,"Task ptin_rfc2819_task created");

  if (osapiWaitForTaskInit (L7_PTIN_RFC2819_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS) {
    PT_LOG_FATAL(LOG_CTX_RFC2819,"Unable to initialize ptin_rfc2819_task()\n");
    return(L7_FAILURE);
  }
  PT_LOG_TRACE(LOG_CTX_RFC2819,"Task ptin_rfc2819_task initialized");
