...
L7_RC_t ptin_intf_init(void)
{
  ...
  LOG_PT_TRACE(LOG_CTX_INTF, "Port <=> intIfNum lookup tables init:");
  for (i=0; i<ptin_sys_number_of_ports; i++)
  {
    /* Get interface ID */
    if (usmDbIntIfNumFromUSPGet(1, 0, i+1, &intIfNum) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to get interface of port# %u", i);
      return L7_FAILURE;
    }

    UPDATE_PORT_MAP(i, intIfNum);

    LOG_PT_TRACE(LOG_CTX_INTF, " Port# %02u => intIfNum# %02u", i, intIfNum);
  }

  LOG_PT_INFO(LOG_CTX_INTF, "Waiting for interfaces to be attached...");
  for (i=0; i<ptin_sys_number_of_ports; i++)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, "Port %u attached!",i);
  }
  LOG_PT_INFO(LOG_CTX_INTF, "All interfaces attached!");
  ...
  for (i=0; i<ptin_sys_number_of_ports; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to remove port# %u from vlan 1", i);
      return L7_FAILURE;
    }
  }
...
//    phyExt_data[i].inner_tpid = PTIN_TPID_INNER_DEFAULT;
    ...
    if ((PTIN_SYSTEM_ETH_PORTS_MASK >> i) & 1)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Failed to disable port# %u", i);
        return L7_FAILURE;
      }
    }
    ...
    if ((PTIN_SYSTEM_10G_PORTS_MASK >> i) & 1)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_CRITIC(LOG_CTX_INTF, "Failed to set DAI-trust mode for port# %u", i);
        return L7_FAILURE;
      }
    }
  #endif
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_CRITIC(LOG_CTX_INTF, "Failed to enable DVLAN mode on port# %u", i);
      return L7_FAILURE;
    }
    ...
    {
      LOG_PT_CRITIC(LOG_CTX_INTF, "Failed to configure default TPID 0x%04X on port# %u (rc = %d)", PTIN_TPID_OUTER_DEFAULT, i, rc);
      return L7_FAILURE;
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to set max frame on port# %u", i);
      return L7_FAILURE;
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to set ovsersized frame size on port# %u", i);
      return L7_FAILURE;
    }
    #endif
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Phy# %u: Error initializing QoS definitions",i);
      return L7_FAILURE;
    }
...
//      if (rc != L7_SUCCESS)
//      {
//        LOG_PT_ERR(LOG_CTX_INTF, "Failed to set Mac Learn priority on port# %u", i);
//        return L7_FAILURE;
//      }
//    }
//#endif
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed initializing MEF Ext parameters");
    return L7_FAILURE;
  }
  LOG_PT_NOTICE(LOG_CTX_INTF, "MEF Ext defaults applied");
...
L7_RC_t ptin_intf_portExt_init(void)
{
  ...
  for (port=0; port<ptin_sys_number_of_ports; port++)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed initializing MEF Ext parameters for interface %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
      rc = L7_FAILURE;
    }
  }
...
L7_RC_t ptin_intf_portExt_set(const ptin_intf_t *ptin_intf, ptin_HWPortExt_t *mefExt)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Invalid argument");
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_INTF,"MefExt parameters:");
  LOG_PT_TRACE(LOG_CTX_INTF," Port = %u/%u"                     , ptin_intf->intf_type,ptin_intf->intf_id);
  LOG_PT_TRACE(LOG_CTX_INTF," Mask = 0x%08x"                    , mefExt->Mask);
  LOG_PT_TRACE(LOG_CTX_INTF," defVid = %u"                      , mefExt->defVid);
  LOG_PT_TRACE(LOG_CTX_INTF," defPrio = %u"                     , mefExt->defPrio);
  LOG_PT_TRACE(LOG_CTX_INTF," acceptable_frame_types = %u"      , mefExt->acceptable_frame_types);
  LOG_PT_TRACE(LOG_CTX_INTF," ingress_filter = %u"              , mefExt->ingress_filter);
  LOG_PT_TRACE(LOG_CTX_INTF," restricted_vlan_reg = %u"         , mefExt->restricted_vlan_reg);
  LOG_PT_TRACE(LOG_CTX_INTF," vlan_aware = %u"                  , mefExt->vlan_aware);
  LOG_PT_TRACE(LOG_CTX_INTF," type       = %u"                  , mefExt->type);
  LOG_PT_TRACE(LOG_CTX_INTF," doubletag  = %u"                  , mefExt->doubletag);
  LOG_PT_TRACE(LOG_CTX_INTF," outer_tpid = 0x%04X"              , mefExt->outer_tpid);
  LOG_PT_TRACE(LOG_CTX_INTF," inner_tpid = 0x%04X"              , mefExt->inner_tpid);
  LOG_PT_TRACE(LOG_CTX_INTF," egress_type = %u"                 , mefExt->egress_type);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_enable = %u"             , mefExt->macLearn_enable);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_stationMove_enable  = %u", mefExt->macLearn_stationMove_enable);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_stationMove_prio    = %u", mefExt->macLearn_stationMove_prio);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_stationMove_samePrio= %u", mefExt->macLearn_stationMove_samePrio);
  LOG_PT_TRACE(LOG_CTX_INTF," MaxChannels  = %u"                , mefExt->maxChannels);
  LOG_PT_TRACE(LOG_CTX_INTF," MaxBandwidth = %llu bits/s", mefExt->maxBandwidth);
  LOG_PT_TRACE(LOG_CTX_INTF," dhcp_trusted = %u"                , mefExt->dhcp_trusted);
  ...
  {    
    ...
    if (ptin_intf->intf_type == PTIN_EVC_INTF_PHYSICAL)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG,"Failed to obtain ptin_port from ptin_intf [ptin_intf.intf_type:%u ptin_intf:%u]",ptin_intf->intf_type, ptin_intf->intf_id);
        return L7_FAILURE;
      }
      ...
      if (ptin_port < PTIN_SYSTEM_N_UPLINK_INTERF)
      {
        ...
        if (mask != 0x00)
        {
          ...
          {
            LOG_PT_ERR(LOG_CTX_MSG, "Invalid Admission Control Parameters [ptin_port:%u mask:0x%04x maxBandwidth:%llu maxChannels:%hu", ptin_port, mefExt->Mask, mefExt->maxBandwidth, mefExt->maxChannels);
            return L7_FAILURE;
          }

          if (ptin_igmp_admission_control_port_set(ptin_port, mask, mefExt->maxChannels, mefExt->maxBandwidth) != L7_SUCCESS)
          {
            LOG_PT_ERR(LOG_CTX_MSG,"Failed to set port admission control parameters");
            return L7_FAILURE;
          }      
        }
      }
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Invalid Admission Control Port = %u/%u (ptin_port=%u)", ptin_intf->intf_type, ptin_intf->intf_id, ptin_port);
        return L7_FAILURE;
      }
#endif
    }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error converting port %u/%u to intIfNum", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_INTF, "Port# %u/%u: intIfNum# %2u", ptin_intf->intf_type,ptin_intf->intf_id, intIfNum);
  ...
  if (mefExt->Mask & PTIN_HWPORTEXT_MASK_DEFVID)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error converting VID %u", mefExt->defVid);
      return L7_FAILURE;
    }
  }
  ...
  if (mefExt->Mask & PTIN_HWPORTEXT_MASK_DEFPRIO)
  {
    ...
    if (mefExt->defPrio > 7)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Invalid Priority %u", mefExt->defPrio);
      return L7_FAILURE;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error applying Priority %u", mefExt->defPrio);
      return L7_FAILURE;
    }
  }
  ...
  if (mefExt->Mask & PTIN_HWPORTEXT_MASK_ACCEPTABLE_FRAME_TYPES)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Invalid Acceptable Frame Type %d", mefExt->acceptable_frame_types);
      return L7_FAILURE;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error applying Ingress Filtering %d", mefExt->acceptable_frame_types);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error SETTING MEF Ext of port %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_INTF, "Success setting MEF Ext of port %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
  return L7_SUCCESS;
}
...
L7_RC_t ptin_intf_portExt_get(const ptin_intf_t *ptin_intf, ptin_HWPortExt_t *mefExt)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting MEF Ext of port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_INTF," type       = %u"                  , mefExt->type);
  LOG_PT_TRACE(LOG_CTX_INTF," doubletag  = %u"                  , mefExt->doubletag);
  LOG_PT_TRACE(LOG_CTX_INTF," outer_tpid = %u"                  , mefExt->outer_tpid);
  LOG_PT_TRACE(LOG_CTX_INTF," inner_tpid = %u"                  , mefExt->inner_tpid);
  LOG_PT_TRACE(LOG_CTX_INTF," egress_type = %u"                 , mefExt->egress_type);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_enable = %u"             , mefExt->macLearn_enable);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_stationMove_enable = %u" , mefExt->macLearn_stationMove_enable);
  LOG_PT_TRACE(LOG_CTX_INTF," macLearn_stationMove_prio   = %u" , mefExt->macLearn_stationMove_prio);
  LOG_PT_TRACE(LOG_CTX_INTF," Max Channels      = %u"           , mefExt->maxChannels);
  LOG_PT_TRACE(LOG_CTX_INTF," Max Bandwidth     = %u"           , mefExt->maxBandwidth);
  LOG_PT_TRACE(LOG_CTX_INTF," Interface trusted = %u"           , mefExt->dhcp_trusted);

  LOG_PT_TRACE(LOG_CTX_INTF, "Success getting MEF Ext of port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
...
L7_RC_t ptin_intf_portMAC_set(const ptin_intf_t *ptin_intf, ptin_HWPortMac_t *portMac)
{
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"MAC address parameters:");
  LOG_PT_TRACE(LOG_CTX_INTF," Port = %u/%u"                       , ptin_intf->intf_type,ptin_intf->intf_id);
  LOG_PT_TRACE(LOG_CTX_INTF," Mask = 0x%04x"                      , portMac->Mask);
  LOG_PT_TRACE(LOG_CTX_INTF," MAC = %02x:%02x:%02x:%02x:%02x:%02x",
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error SETTING MAC address to port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_INTF, "Success setting MAC address to port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
...
L7_RC_t ptin_intf_portMAC_get(const ptin_intf_t *ptin_intf, ptin_HWPortMac_t *portMac)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting MAC address of port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_INTF, "Success getting MAC address of port %u/%u", ptin_intf->intf_type,ptin_intf->intf_id);
...
L7_RC_t ptin_intf_PhyConfig_set(ptin_HWEthPhyConf_t *phyConf)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u is out of range [0..%u]", port, ptin_sys_number_of_ports-1);
    return L7_FAILURE;
  }

  /* Get intIfNum */
  ptin_intf_port2intIfNum(port, &intIfNum);
  LOG_PT_TRACE(LOG_CTX_INTF, "Port# %2u:     intIfNum# %2u", port, intIfNum);
  ...
  if ( (phyConf->Mask & PTIN_PHYCONF_MASK_PORTEN) )     /* Enable mask bit */
  {
    ...
    {
      LOG_PT_INFO(LOG_CTX_INTF, "Port %u in forced link state... nothing to be done!");
    }
    else
  #endif
    {
          ...
          (value != phyConf->PortEnable))
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Failed to set enable state on port# %u", port);
          return L7_FAILURE;
        }
        ...
        LOG_PT_TRACE(LOG_CTX_INTF, " State:       %s", phyConf->PortEnable ? "Enabled":"Disabled");
      }
    }
      ...
      (value != phyConf->MaxFrame))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to set max frame on port# %u", port);
      return L7_FAILURE;
    }

    phyConf_data[port].MaxFrame = phyConf->MaxFrame; /* update buffered conf data */
    LOG_PT_TRACE(LOG_CTX_INTF, " MaxFrame:    %u", phyConf->MaxFrame);
  }
  #else
  if ((phyConf->Mask & PTIN_PHYCONF_MASK_MAXFRAME) &&
      ...
      (value != (phyConf->LoopBack != 0)))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Failed to set loopback state on port# %u", port);
      return L7_FAILURE;
    }

    phyConf_data[port].LoopBack = phyConf->LoopBack; /* update buffered conf data */
    LOG_PT_TRACE(LOG_CTX_INTF, " Loopback:    %s", phyConf->LoopBack ? "Enabled":"Disabled");
  }
  ...
  if (phyConf->Mask & PTIN_PHYCONF_MASK_SPEED)
  {
        ...
        (value != speed_mode))
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Failed to set port speed on port# %u", port);
        return L7_FAILURE;
      }

      phyConf_data[port].Speed = phyConf->Speed; /* update buffered conf data */
      LOG_PT_TRACE(LOG_CTX_INTF, " Speed:       %s", speedstr);
    }
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed to clear counters on port# %u", port);
    return L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_INTF, " Counters:    Cleared!");
...
L7_RC_t ptin_intf_PhyState_read(ptin_HWEthPhyState_t *phyState)
{
  ...
  phyState->Duplex = 1;
  LOG_PT_TRACE(LOG_CTX_INTF, " Full-Duplex: %s", phyState->Duplex?"Enabled":"Disabled");
  ...
  else
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, " Link State:  %s", phyState->LinkUp ? "Up":"Down");
  }
      ...
      phyConf_data[port].Speed == PHY_PORT_1000AN_GBPS)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, " AutoNeg End: %s", phyState->AutoNegComplete?"Yes":"No");
  }
...
L7_RC_t ptin_intf_counters_clear(ptin_HWEthRFC2819_PortStatistics_t *portStats)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u is out of range [0..%u]", portStats->Port, ptin_sys_number_of_ports-1);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_boardid_get(L7_int ptin_port, L7_uint16 *board_id)
{
  ...
  {
    //LOG_PT_ERR(LOG_CTX_INTF,"Invalid inputs: ptin_port=%d", ptin_port);
    return L7_FAILURE;
  }
  /* Validate input params */
  if ((slot_id < PTIN_SYS_LC_SLOT_MIN || slot_id > PTIN_SYS_LC_SLOT_MAX) && (slot_id != 0))
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid slot_id=%d", slot_id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_boardid_set(L7_int ptin_port, L7_uint16 board_id)
{
  ...
  if ((slot_id < PTIN_SYS_LC_SLOT_MIN || slot_id > PTIN_SYS_LC_SLOT_MAX) && (slot_id != 0))
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid inputs: slot_id=%d", slot_id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_slotPort2port(L7_uint16 slot, L7_uint16 port, L7_uint32 *ptin_port_ret)
{
 ...
 #endif
  {
    ...
    {
      //LOG_PT_ERR(LOG_CTX_INTF,"slot %u / port %u is out of range",slot,port);
      return L7_FAILURE;
    }
  ...
  if (slot < 2)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"slot %u is invalid", slot);
    return L7_FAILURE;
  }
...
#else
  LOG_PT_ERR(LOG_CTX_INTF, "Not in a matrix board");
  return L7_FAILURE;
#endif
  ...
  {
    //LOG_PT_ERR(LOG_CTX_INTF,"Invalid ptin_port %u derived from slot %u / port %u", ptin_port, slot, port);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_port2SlotPort(L7_uint32 ptin_port, L7_uint16 *slot_ret, L7_uint16 *port_ret, L7_uint16 *board_type)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid port id (%u)", ptin_port);
    return L7_FAILURE;
  }
  ...
  #endif
  {
    ...
    {
      //LOG_PT_ERR(LOG_CTX_INTF,"ptin_intf=%u/%u is not mapped!", ptin_intf->intf_type, ptin_intf->intf_id);
      return L7_FAILURE;
    }
  ...
  if (slot <= 1)
  {
    if (port >= ptin_sys_number_of_ports)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Invalid slot (%u) or port (%u) for ptin_port=%u", slot, port, ptin_port);
    }
  }
  else
 #endif
...
L7_RC_t ptin_intf_slotPort2ptintf(L7_uint16 slot, L7_uint16 port, ptin_intf_t *ptin_intf)
{
  ...
  {
    //LOG_PT_ERR(LOG_CTX_INTF,"Error converting slot %u / port %u to ptin_port",slot,port);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_ptintf2SlotPort(const ptin_intf_t *ptin_intf, L7_uint16 *slot_ret, L7_uint16 *port_ret, L7_uint16 *board_type)
{
  ...
  if (ptin_intf == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid arguments");
    return L7_FAILURE;
  }
                                  ...
                                  L7_uint16 *slot_ret, L7_uint16 *port_ret, L7_uint16 *board_type)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid interface (%u/%u)", intf_type, intf_id);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid interface id (%u/%u)", intf_type, intf_id);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_port2intIfNum(L7_uint32 ptin_port, L7_uint32 *intIfNum)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u is out of range [0..%u]", ptin_port, PTIN_SYSTEM_N_INTERF-1);
    return L7_FAILURE;
  }
  ...
  {
    //LOG_PT_WARN(LOG_CTX_INTF, "ptin_port# %u is not assigned to any interface",ptin_port);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_intIfNum2port(L7_uint32 intIfNum, L7_uint32 *ptin_port)
{
  ...
  {
    //LOG_PT_ERR(LOG_CTX_INTF, "intIfNum# %u is out of range [1..%u]", intIfNum, L7_MAX_INTERFACE_COUNT);
    return L7_FAILURE;
  }
  ...
  {
    //LOG_PT_WARN(LOG_CTX_INTF, "intIfNum# %u is not assigned!", intIfNum);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_port2ptintf(L7_uint32 ptin_port, ptin_intf_t *ptin_intf)
{
  ...
  if (ptin_port >= PTIN_SYSTEM_N_INTERF ||
      (ptin_port >= ptin_sys_number_of_ports && ptin_port < PTIN_SYSTEM_N_PORTS))
  {
    LOG_PT_ERR(LOG_CTX_INTF, "PTin port is out of range: %u", ptin_port);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_ptintf2port(const ptin_intf_t *ptin_intf, L7_uint32 *ptin_port)
{
  ...
  if (ptin_intf == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"ptin_intf is a null pointer");
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_typeId2port(L7_uint8 intf_type, L7_uint8 intf_id, L7_uint32 *ptin_port)
{
  ...
  if (intf_type == PTIN_EVC_INTF_PHYSICAL)
  {
    if (intf_id >= ptin_sys_number_of_ports)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Physical port id is out of range: %u", intf_id);
      return L7_FAILURE;
    }
    p_port = intf_id;
  }
  else if (intf_type == PTIN_EVC_INTF_LOGICAL)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Lag id is out of range: %u", intf_id);
      return L7_FAILURE;
    }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port type is not valid");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port id is out of range: %u", p_port);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_intIfNum2ptintf(L7_uint32 intIfNum, ptin_intf_t *ptin_intf)
{
  ...
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "Unable to get interface type for intfNum %u", intIfNum);
    return L7_FAILURE;
  }
  ...
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error converting ptin_port %u to type+id format", ptin_port);
      return L7_FAILURE;
    }
  }
...
inline L7_RC_t ptin_intf_typeId2intIfNum(L7_uint8 intf_type, L7_uint8 intf_id, L7_uint32 *intIfNum)
{
  ...
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error converting ptin_intf %u/%u to ptin_port", intf_type, intf_id);
      return L7_FAILURE;
    }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "IntIfNum is out of range (%u)", intIfNum0);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_lag2intIfNum(L7_uint32 lag_idx, L7_uint32 *intIfNum)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u is out of range [0..%u]", lag_idx, PTIN_SYSTEM_N_LAGS-1);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_intIfNum2lag(L7_uint32 intIfNum, L7_uint32 *lag_idx)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Invalid intIfNum %u", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Can't get intIfNum %u type", intIfNum);
    return L7_FAILURE;
  }
  /* This should be a LAG port */
  if (sysIntfType != L7_LAG_INTF)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "intIfNum %u is not a lag (type=%u)", intIfNum, sysIntfType);
    return L7_FAILURE;
  }
  #endif
  ...
  if (ptin_port >= PTIN_SYSTEM_N_INTERF)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "intIfNum %u / ptin_port %u is invalid", intIfNum, ptin_port);
    return L7_FAILURE;
  }
  /* Check again for LAG type */
  if (ptin_port < PTIN_SYSTEM_N_PORTS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "intIfNum %u / ptin_port %u is physical type", intIfNum, ptin_port);
    return L7_FAILURE;
  }
...
inline L7_RC_t ptin_intf_slot2lagIdx(L7_uint16 slot, L7_uint32 *lag_idx)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Slot %u is out of range [%u..%u]", slot, PTIN_SYS_LC_SLOT_MIN, PTIN_SYS_LC_SLOT_MAX);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u is out of range [0..%u]", aux, PTIN_SYSTEM_N_LAGS-1);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_LagConfig_get(ptin_LACPLagConfig_t *lagInfo)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u does not exist", lag_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_Lag_create(ptin_LACPLagConfig_t *lagInfo)
{
  ...
  if (lag_idx >= PTIN_SYSTEM_PROTECTION_LAGID_BASE)
  {
    ...
    for (port = 0; port < ptin_sys_number_of_ports; port++, members_pbmp>>=1)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF,"Error converting port %u to intIfNum", port);
        continue;
      }
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF,"Port %u does not belong to an uplink board (board id %u)", port, board_id);
        continue;
      }
      #endif
      ...
      if ((uplink_protection_ports_bmp >> port) & 1)
      {
        if ((lag_uplink_protection_ports_bmp[lag_idx - PTIN_SYSTEM_PROTECTION_LAGID_BASE] >> port) & 1)
        {
          LOG_PT_WARN(LOG_CTX_INTF,"Port %u already belongs to this protection lag", port);
        }
        else
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Port %u already belongs to another protection lag", port);
        }
        continue;
      }
      ...
      LOG_PT_INFO(LOG_CTX_INTF,"Port %u removed from all vlans", port);
      ...
      if (!phyConf_data[port].PortEnable)
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Error enabling port %u", port);
          ptin_vlan_port_add(port, 0);
          continue;
        }
        phyConf_data[port].PortEnable = L7_TRUE;
        LOG_PT_INFO(LOG_CTX_INTF,"Port %u enabled", port);
      }
      else
      {
        LOG_PT_INFO(LOG_CTX_INTF,"Port %u is already enabled", port);
      }
      ...
      if (ptin_fgpa_mx_is_matrixactive())
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Error disablink linkscan for intIfNum %u", intIfNum);
        }
        else
        {
          LOG_PT_INFO(LOG_CTX_INTF,"Linkscan successfully enabled for intIfNum %u (port %u)", intIfNum, port);
        }
      }
      #endif
    #endif
      ...
      LOG_PT_INFO(LOG_CTX_INTF,"Port %u is a protection port", port);
    }
    ...
    LOG_PT_WARN(LOG_CTX_INTF, "Protection LAG %u configured", lag_idx);
    return L7_SUCCESS;
  }
#endif
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: members list is empty", lag_idx);
    return L7_FAILURE;
  }
  ...
  for (port = PTIN_SYSTEM_N_PONS; port < ptin_sys_number_of_ports; port++, members_pbmp>>=1)
  {
    if (members_pbmp & 1)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading ifSpeed from port# %u", lag_idx, port);
        return L7_FAILURE;
      }
      ...
      if (value != ifSpeed)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: there are members with different if speed", lag_idx);
        return L7_FAILURE;
      }
      #endif
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: there are members with different EtherType (0x%04X != 0x%04X)",
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: the following members belong to other LAGs: 0x%010llX", lag_idx, lagInfo->members_pbmp64 & members_pbmp_all);
    return L7_FAILURE;
  }
  ...
  for (i = 0; i < ptin_sys_number_of_ports; i++, members_pbmp>>=1)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: member %u is being used in EVCs! Lag cannot be created/updated", lag_idx, i);
      return L7_FAILURE;
    }
  }
  ...
  if (!ptin_intf_lag_exists(lag_idx))
  {
    ...
    if (res != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error on usmDbDot3adCreateSet()", lag_idx);
      return L7_FAILURE;
    }
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %02u created with empty members (interface# %02u)", lag_idx, lag_intf);
        ...
        && (value != maxFrame))
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: failed setting max frame (%u)", lag_idx, maxFrame);
        rc = L7_FAILURE;
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_INTF, " MaxFrameSize set to %u", maxFrame);
      }
    }
    ...
    if (ptin_intf_QoS_init(&ptin_intf)!=L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: Error initializing QoS definitions", lag_idx);
      rc = L7_FAILURE;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %u: Success initializing QoS definitions", lag_idx);
    }
    ...
    if (lag_idx < PTIN_SYSTEM_N_LAGS_EXTERNAL)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: Error setting MAC learning attributes", lag_idx);
      }
    }
    #else
    ...
    #endif
    {
      ...
      #endif
      LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %u is trusted", lag_idx);
     #endif
    }
    #endif
  }
  else
  {
    ...
    if (usmDbDot3adIsConfigured(1, lag_intf) == L7_FALSE)
    {
      ...
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: no FP config found! Clearing PTin structs and aborting operation", lag_idx);
      return L7_FAILURE;
    }

    LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %02u already exists (interface# %02u)", lag_idx, lag_intf);
  }
  ...
  do
  {
    ...
    if (lagConf_data[lag_idx].admin != lagInfo->admin)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error setting Admin mode to %u", lag_idx, lagInfo->admin);
        rc = L7_FAILURE;
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_INTF, " .admin            = %u", lagInfo->admin);
    ...
    if (lagConf_data[lag_idx].stp_enable != lagInfo->stp_enable)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error setting STP mode to %u", lag_idx, lagInfo->stp_enable);
        rc = L7_FAILURE;
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_INTF, " .stp_enable       = %u", lagInfo->stp_enable);
    ...
    if (lagConf_data[lag_idx].static_enable != lagInfo->static_enable)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error setting Static mode to %u", lag_idx, lagInfo->static_enable);
        rc = L7_FAILURE;
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_INTF, " .static_enable    = %u", lagInfo->static_enable);
    ...
    if (lagConf_data[lag_idx].loadBalance_mode != lagInfo->loadBalance_mode)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error setting Balance mode to %u", lag_idx, lagInfo->loadBalance_mode);
        rc = L7_FAILURE;
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_INTF, " .loadBalance_mode = %u", lagInfo->loadBalance_mode);
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    if (newLag)
    {
      if (usmDbDot3adRemoveSet(1, lag_intf) != L7_SUCCESS)
        LOG_PT_CRITIC(LOG_CTX_INTF, "LAG# %u: failed to undo LAG creation", lag_idx);
      ...
      #endif
      {
        ...
        LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %u goes back to untrusted", lag_idx);
        #endif
      }
      ...
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u was removed (undo creation)", lag_idx);
      return rc;
    }
  ...
  if (lagConf_data[lag_idx].members_pbmp64 != lagInfo->members_pbmp64)
    LOG_PT_TRACE(LOG_CTX_INTF, " .members_pbmp     = 0x%010llX", lagInfo->members_pbmp64);
  ...
  for (port = 0; port < ptin_sys_number_of_ports; port++, members_pbmp>>=1)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Error getting MEF Ext. attributes of interface %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
      rc = L7_FAILURE;
      continue;
    }
    #endif
    ...
    if (members_pbmp & 1)
    {
          ...
          && (value != L7_TRUE))
      {
        ...
        if ((usmDbDot3adAggPortLacpModeGet(1, intIfNum, &value) != L7_SUCCESS) || (value != L7_ENABLE)) {
          LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: port# %u does not have LACP enabled", lag_idx, port);
          rc = L7_FAILURE;
          continue;
        }
      }
      ...
      if (dot3adAggGet(intIfNum, &value) == L7_SUCCESS)
      {
        ...
        if (value != lag_intf)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: port# %u already belongs to another LAG", lag_idx, port);
          rc = L7_FAILURE;
        }
        continue;
      }
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: Error getting admin state of port# %u", lag_idx, port);
        rc = L7_FAILURE;
        continue;
      }
      #endif
      ...
      {
        LOG_PT_WARN(LOG_CTX_INTF, "LAG# %u: could not configure Auto-Neg off on port# %u", lag_idx, port);
        //rc = L7_FAILURE;
        //continue;
      }
      #endif
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: could not add member port# %u", lag_idx, port);
        rc = L7_FAILURE;
        continue;
      }
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: Error setting admin state to port# %u", lag_idx, port);
        //rc = L7_FAILURE;
        //continue;
      }
      #endif
      ...
      LOG_PT_TRACE(LOG_CTX_INTF, " Port# %02u added", port);
      ...
      if (newLag && lag_idx >= PTIN_SYSTEM_PROTECTION_LAGID_BASE)
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Error forcing link-up for intIfNum %u", intIfNum);
        }
        else
        {
          LOG_PT_INFO(LOG_CTX_INTF,"Linkscan successfully disabled for intIfNum %u (port %u)", intIfNum, port);
        }
      }
      #endif
      #endif
    ...
    else
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: could not remove member port# %u", lag_idx, port);
        rc = L7_FAILURE;
        continue;
      }
      ...
      {
        LOG_PT_CRITIC(LOG_CTX_INTF, "LAG# %u: could not update PortGroup for member port# %u", lag_idx, port);
        rc = L7_FAILURE;
      }

      lagConf_data[lag_idx].members_pbmp64 &= ~((L7_uint64)1 << port);
      LOG_PT_TRACE(LOG_CTX_INTF, " Port# %02u removed", port);
    }
  }
  ...
  if (ptin_xlate_portgroup_set(lag_intf, PTIN_XLATE_PORTGROUP_INTERFACE) != L7_SUCCESS)
  {
    ...
    LOG_PT_CRITIC(LOG_CTX_INTF, "LAG# %u: could not update PortGroup for this LAG", lag_idx);
    rc = L7_FAILURE;
  }
  ...
  if (rc == L7_SUCCESS && newLag)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: could not remove interface# %u from VLAN 1", lag_idx, lag_intf);
      rc = L7_FAILURE;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF, " Intf# %02u removed from VLAN# 1", lag_intf);
    }

    /* Enable DVLAN mode */
    if (usmDbDvlantagIntfModeSet(0, lag_intf, L7_ENABLE) != L7_SUCCESS)
    {
      LOG_PT_CRITIC(LOG_CTX_INTF, "Failed to enable DVLAN mode on LAG# %u", lag_idx);
      rc = L7_FAILURE;
    }
    ...
    {
      LOG_PT_CRITIC(LOG_CTX_INTF, "Failed to configure TPID 0x%04X on LAG# %u (rc = %d)", lagEtherType, lag_idx, rc);
      rc = L7_FAILURE;
    }
  }
  ...
  if ( rc == L7_SUCCESS )
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Error getting ptin_intf from LAG intIfNum %u",lag_intf);
      rc = L7_FAILURE;
    }
    else
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_INTF,"Error applying MEF Ext attributes to ptin_intf %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
        rc = L7_FAILURE;
      }
    }
  }
...
L7_RC_t ptin_intf_Lag_delete(ptin_LACPLagConfig_t *lagInfo)
{
  ...
  if (lag_idx >= PTIN_SYSTEM_PROTECTION_LAGID_BASE)
  {
    ...
    for (port = 0; port < ptin_sys_number_of_ports; port++, ptin_pbmp>>=1)
    {
      ...
      if (ptin_fgpa_mx_is_matrixactive())
      {
        ...
        if (ptin_intf_linkscan_set(intIfNum, L7_ENABLE) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Error enabling linkscan for intIfNum %u", intIfNum);
        }
        else
        {
          LOG_PT_INFO(LOG_CTX_INTF,"Linkscan successfully enabled for intIfNum %u (port %u)", intIfNum, port);
    ...
    LOG_PT_WARN(LOG_CTX_INTF, "Protection lag %u removed", lag_idx);
    return L7_SUCCESS;
  }
#endif
  ...
  if (!ptin_intf_lag_exists(lag_idx))
  {
    LOG_PT_WARN(LOG_CTX_INTF, "LAG# %u does not exist", lag_idx);
    return L7_SUCCESS;
  }

  if (ptin_intf_lag2intIfNum(lag_idx, &lag_intIfNum) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error acquiring intIfNum of lag_idx %u", lag_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u is being used in EVCs! Cannot be removed", lag_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: failed to remove this LAG", lag_idx);
    return L7_FAILURE;
  }
  ...
  #if (PTIN_BOARD_IS_LINECARD || PTIN_BOARD_IS_STANDALONE)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %u returns to untrusted", lag_idx);
  }
  #endif
  ...
  for (i=0; i<ptin_sys_number_of_ports; i++, ptin_pbmp >>= 1)
  {
    if (ptin_pbmp & 1)
    {
      ...
      if (ptin_xlate_portgroup_set(intIfNum, PTIN_XLATE_PORTGROUP_INTERFACE) != L7_SUCCESS)
      {
         ...
         * However, any error should occur ONLY during debug or validation process, and
         * not during normal operation (under production) */ 
        LOG_PT_CRITIC(LOG_CTX_INTF, "LAG# %u: could not update PortGroup for member port# %u", lag_idx, i);
      }
      ...
      if (lag_idx >= PTIN_SYSTEM_PROTECTION_LAGID_BASE)
      {
        ...
        if (ptin_intf_linkscan_set(intIfNum, L7_ENABLE) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF,"Error reenabling linkscan for intIfNum %u", intIfNum);
        }
      }
      #endif
      #endif
...
// TODO
  ...
  LOG_PT_TRACE(LOG_CTX_INTF, "LAG# %u: successfully deleted", lag_idx);
...
L7_RC_t ptin_intf_LagStatus_get(ptin_LACPLagStatus_t *lagStatus)
{
  ...
  /* Normal LAGs */
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading Link State", lag_idx);
      return L7_FAILURE;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading Active Members List", lag_idx);
      return L7_FAILURE;
    }
    ...
    for (i=0; i<nElems; i++)
    {
      ...
      {
        LOG_PT_CRITIC(LOG_CTX_INTF, "LAG# %u: invalid active members found (port# %u; intf# %u)", lag_idx, value, members_list[i]);
        continue;
      }
...
L7_RC_t ptin_intf_LACPAdminState_set(ptin_LACPAdminState_t *adminState)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u is out of range [0..%u]", port_idx, ptin_sys_number_of_ports-1);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u: failed to read actor/partner states (intf=%u)", port_idx, port_intf);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: failed to set actor/partner states", port_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_LACPAdminState_get(ptin_LACPAdminState_t *adminState)
{
  ...
  if (usmDbDot3adAggPortActorAdminStateGet(1, port_intf, &actor_state) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u: failed to read actor state (intf=%u)", port_idx, port_intf);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_LACPStats_get(ptin_LACPStats_t *lagStats)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting intIfNum value from Port# %u", port_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u: error reading LACPDUs Rx stats", port_idx);
    return L7_FAILURE;
  }

  if (usmDbDot3adAggPortStatsLACPDUsTxGet(1, port_intf, &txStats) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u: error reading LACPDUs Tx stats", port_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_LACPStats_clear(ptin_LACPStats_t *lagStats)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Port# %u: error clearing LACP stats", port_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_intf_bcast_stormControl_get(const ptin_intf_t *ptin_intf, L7_BOOL *enable, L7_uint32 *rate_limit, L7_uint32 *burst_size, L7_uint8 *rate_units)
{
  ...
  if (ptin_intf == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Null pointer");
    return L7_FAILURE;
  }
  ...
  if (ptin_intf_ptintf2intIfNum(ptin_intf, &intIfNum) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Invalid ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_MSG, "Reading Broadcast stormcontrol data...");

  /* Read current enable status */
  rc = usmDbSwDevCtrlBcastStormModeIntfGet(intIfNum, &enable_status);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Broadcast enable status from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Broadcast threshold data from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
...
L7_RC_t ptin_intf_mcast_stormControl_get(const ptin_intf_t *ptin_intf, L7_BOOL *enable, L7_uint32 *rate_limit, L7_uint32 *burst_size, L7_uint8 *rate_units)
{
  ...
  LOG_PT_TRACE(LOG_CTX_MSG, "Reading Multicast stormcontrol data...");

  /* Read current enable status */
  rc = usmDbSwDevCtrlMcastStormModeIntfGet(intIfNum, &enable_status);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Multicast enable status from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
  /* Read current threshold data */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Multicast threshold data from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
...
L7_RC_t ptin_intf_ucast_stormControl_get(const ptin_intf_t *ptin_intf, L7_BOOL *enable, L7_uint32 *rate_limit, L7_uint32 *burst_size, L7_uint8 *rate_units)
{
  ...
  LOG_PT_TRACE(LOG_CTX_MSG, "Reading Unicast stormcontrol data...");

  /* Read current enable status */
  rc = usmDbSwDevCtrlUcastStormModeIntfGet(intIfNum, &enable_status);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Unicast enable status from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
  /* Read current threshold data */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"Error reading Unicast threshold data from ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return rc;
  }
...
static L7_RC_t ptin_intf_stormcontrol_reset(L7_uint32 intIfNum, L7_uint8 rate_units)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error disabling Broadcast StormControl - rc=%d", intIfNum, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Broadcast StormControl disabled", intIfNum);

  rc = usmDbSwDevCtrlMcastStormModeIntfSet(intIfNum, L7_DISABLE);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error disabling Multicast StormControl - rc=%d", intIfNum, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Multicast StormControl disabled", intIfNum);

  rc = usmDbSwDevCtrlUcastStormModeIntfSet(intIfNum, L7_DISABLE);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error disabling Unicast StormControl - rc=%d", intIfNum, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Unicast StormControl disabled", intIfNum);

  /* Check result */
  if (rc_global != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error disabling StormControl - rc_global=%d", intIfNum, rc_global);
    return rc_global;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error setting Broadcast threashold units to %u - rc=%d", intIfNum, rate_units, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Broadcast threashold units defined to %u", intIfNum, rate_units);

  rc = usmDbSwDevCtrlMcastStormThresholdIntfSet(intIfNum, 0, 0, rate_units);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error setting Multicast threashold units to %u - rc=%d", intIfNum, rate_units, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Multicast threashold units defined to %u", intIfNum, rate_units);

  rc = usmDbSwDevCtrlUcastStormThresholdIntfSet(intIfNum, 0, 0, rate_units);
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error setting Unicast threashold units to %u - rc=%d", intIfNum, rate_units, rc);
    rc_global = rc;
  }
  LOG_PT_TRACE(LOG_CTX_MSG,"intIfNum=%u: Unicast threashold units defined to %u", intIfNum, rate_units);

  /* Check results */
  if (rc_global != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_MSG,"intIfNum=%u: Error redefining rate units - rc_global=%d", intIfNum, rc_global);
    return rc_global;
  }
...
L7_RC_t ptin_intf_bcast_stormControl_set(const ptin_intf_t *ptin_intf, L7_BOOL enable, L7_uint32 rate_value, L7_uint32 rate_burst, L7_uint8 rate_units)
{
  ...
  LOG_PT_TRACE(LOG_CTX_MSG, "Configuring Broadcast stormcontrol...");
  ...
  if (enable)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Unknown units (%u)", rate_units);
      return L7_FAILURE;
    }
    ...
    if (rate_units != rate_units_curr)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_MSG,"ptin_intf %u/%u: Error resetting StormControl to units %u - rc=%d",
                ptin_intf->intf_type,ptin_intf->intf_id, rate_units, rc);
        return rc;
      }
      LOG_PT_TRACE(LOG_CTX_MSG,"ptin_intf %u/%u: StormControl resetted to units %u",
                ptin_intf->intf_type,ptin_intf->intf_id, rate_units);
    }

    LOG_PT_TRACE(LOG_CTX_MSG,"Going to apply BC threshold of %u (units=%u)", rate_value, rate_units);

    /* Redefine threshold/units */
    rc = usmDbSwDevCtrlBcastStormThresholdIntfSet(intIfNum, rate_value, rate_burst, rate_units);
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Broadcast threashold to %u for ptin_intf %u/%u", rate_value,
  ...
  if (enable != enable_curr)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Broadcast enable to %u for ptin_intf %u/%u", enable, ptin_intf->intf_type, ptin_intf->intf_id);
      return rc;
    }
  }

  LOG_PT_TRACE(LOG_CTX_MSG, "Success applying Broadcast stormcontrol to ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
...
L7_RC_t ptin_intf_mcast_stormControl_set(const ptin_intf_t *ptin_intf, L7_BOOL enable, L7_uint32 rate_value, L7_uint32 rate_burst, L7_uint8 rate_units)
{
  ...
  LOG_PT_TRACE(LOG_CTX_MSG, "Configuring Multicast stormcontrol for ptin_intf %u/%u, intIfNum %u...", ptin_intf->intf_type, ptin_intf->intf_id, intIfNum);
  ...
  if (enable)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Multicast threashold to %u for ptin_intf %u/%u", rate_value,
              ptin_intf->intf_type, ptin_intf->intf_id);
      return rc;
    }
  }
  ...
  if (enable != enable_curr)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Multicast enable to %u for ptin_intf %u/%u", enable, ptin_intf->intf_type, ptin_intf->intf_id);
      return rc;
    }
  }

  LOG_PT_TRACE(LOG_CTX_MSG, "Success applying Multicast stormcontrol to ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
...
L7_RC_t ptin_intf_ucast_stormControl_set(const ptin_intf_t *ptin_intf, L7_BOOL enable, L7_uint32 rate_value, L7_uint32 rate_burst, L7_uint8 rate_units)
{
  ...
  LOG_PT_TRACE(LOG_CTX_MSG, "Configuring Unicast stormcontrol...");
  ...
  if (enable)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Unicast threashold to %u for ptin_intf %u/%u", rate_value,
              ptin_intf->intf_type, ptin_intf->intf_id);
      return rc;
    }
  }
  ...
  if (enable != enable_curr)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_MSG,"Error setting Unicast enable to %u for ptin_intf %u/%u", enable, ptin_intf->intf_type, ptin_intf->intf_id);
      return rc;
    }
  }

  LOG_PT_TRACE(LOG_CTX_MSG, "Success applying Unicast stormcontrol to ptin_intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
...
L7_RC_t ptin_QoS_intf_cos_policer_set(const ptin_intf_t *ptin_intf, L7_uint8 cos, ptin_bw_meter_t *meter)
{
  ...
  if (ptin_intf == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Null pointer");
    return L7_FAILURE;
  }
  if (ptin_intf_ptintf2port(ptin_intf, &ptin_port) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Invalid ptin_intf=%u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
  if (cos >= L7_COS_INTF_QUEUE_MAX_COUNT)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Invalid COS %u", cos);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_EVC,"Error applying policer");
    return rc;
  }
...
L7_RC_t ptin_QoS_intf_config_set(const ptin_intf_t *ptin_intf, ptin_QoS_intf_t *intfQos)
{
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"Interface = %u/%u",ptin_intf->intf_type,ptin_intf->intf_id);
  LOG_PT_TRACE(LOG_CTX_INTF,"Mask         = 0x%02x",intfQos->mask);
  LOG_PT_TRACE(LOG_CTX_INTF,"TrustMode    = %u",intfQos->trust_mode);
  LOG_PT_TRACE(LOG_CTX_INTF,"BWunits      = %u",intfQos->bandwidth_unit);
  LOG_PT_TRACE(LOG_CTX_INTF,"ShapingRate  = %u",intfQos->shaping_rate);
  LOG_PT_TRACE(LOG_CTX_INTF,"WREDDecayExp = %u",intfQos->wred_decay_exponent);
  LOG_PT_TRACE(LOG_CTX_INTF,"PrioMap.mask   =0x%02x",intfQos->pktprio.mask);
  LOG_PT_TRACE(LOG_CTX_INTF,"PrioMap.prio[8]={0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x}",
  ...
  if (ptin_intf_ptintf2intIfNum(ptin_intf,&intIfNum)!=L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Interface %u/%u invalid",ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_INTF, "Interface %u/%u is intIfNum=%u",ptin_intf->intf_type,ptin_intf->intf_id,intIfNum);

  /* Is there any configuration to be applied? */
  if (intfQos->mask==0x00)
  {
    LOG_PT_WARN(LOG_CTX_INTF, "Empty mask: no configuration to be applied");
    return L7_SUCCESS;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid interface for QoS operation (intIfNum=%u, ptin_intf=%u/%u)",intIfNum,ptin_intf->intf_type,ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error with usmDbQosCosMapTrustModeGet (rc=%d)", rc);
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid trust mode (%u)",trust_mode);
    trust_mode = L7_QOS_COS_MAP_INTF_MODE_UNTRUSTED;
    rc_global = L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_INTF, "Current trust mode is %u",trust_mode);
  ...
  if (intfQos->mask & PTIN_QOS_INTF_TRUSTMODE_MASK)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosMapTrustModeSet (rc=%d)", rc);
      rc_global = rc;
    }
    else
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF, "New trust mode is %u",trust_mode);
    }
  }
  /* Bandwidth units */
  ...
  {
    /* Do nothing */
    LOG_PT_WARN(LOG_CTX_INTF, "Bandwidth units were not changed");
  }
  /* Shaping rate */
  if (intfQos->mask & PTIN_QOS_INTF_SHAPINGRATE_MASK)
  {
    ...
    if (rc != L7_SUCCESS)
    {  
      LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosQueueIntfShapingRateSet (rc=%d)", rc);
      rc_global = rc;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF, "New shaping rate is %u",intfQos->shaping_rate);
    }
  }
  /* WRED decay exponent */
  if (intfQos->mask & PTIN_QOS_INTF_WRED_DECAY_EXP_MASK)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosQueueWredDecayExponentSet (rc=%d)", rc);
      rc_global = rc;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF, "New WRED Decay exponent is %u",intfQos->wred_decay_exponent);
    }
  }
      ...
      trust_mode!=L7_QOS_COS_MAP_INTF_MODE_UNTRUSTED)
  {
    ...
    for (prio=0; prio<8; prio++)
    {
      ...
      if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_DOT1P)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbDot1dTrafficClassSet (prio=%u => cos=%u): rc=%d",prio,cos, rc);
          rc_global = rc;
        }
        else
        {
          LOG_PT_TRACE(LOG_CTX_INTF, "Pbit %u => CoS=%u",prio,cos);
        }
      }
      ...
      else if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_IPPREC)
      {
        ...
        if (rc != L7_SUCCESS)
        { 
          LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosMapIpPrecTrafficClassSet (IPprec=%u => cos=%u): rc=%d",prio,cos, rc); 
          rc_global = rc;
        }
        else
        {
          LOG_PT_TRACE(LOG_CTX_INTF, "IPprec %u => CoS=%u",prio,cos);
        }
      }
      // DSCP trust mode
      else if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_IPDSCP)
      {
        ...
        for (prio2=0; prio2<8; prio2++)
        {
          ...
          if (rc != L7_SUCCESS)
          {
            LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosMapIpDscpTrafficClassSet (DscpPrio=%u => CoS=%u): rc=%d",prio*8+prio2,cos, rc);
            rc_global = rc;
          }
          else
          {
            LOG_PT_TRACE(LOG_CTX_INTF, "DscpPrio %u => CoS=%u",prio*8+prio2,cos);
          }
        }
      }
      else
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Unknown trust mode for prio=%u (%u)",prio,trust_mode);
        rc_global = L7_FAILURE;
      }
    }
  }
  ...
  if (rc_global==L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "QoS configuration successfully applied to ptin_intf=%u/%u",ptin_intf->intf_type,ptin_intf->intf_id);
  }
  else
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error applying QoS configuration to ptin_intf=%u/%u (rc_global=%d)",ptin_intf->intf_type,ptin_intf->intf_id, rc_global);
  }
...
L7_RC_t ptin_QoS_intf_config_get(const ptin_intf_t *ptin_intf, ptin_QoS_intf_t *intfQos)
{
  ...
  if (rc != L7_SUCCESS)
  {  
    LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosQueueIntfShapingRateGet (rc=%d)", rc);
    rc_global = rc;
  }
  else
  {
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosQueueWredDecayExponentGet (rc=%d)", rc);
    rc_global = rc;
  }
  else
  {
  ...
  if (trust_mode!=L7_QOS_COS_MAP_INTF_MODE_UNTRUSTED)
  {
    ...
    for (prio=0; prio<8; prio++)
    {
      ...
      if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_DOT1P)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbDot1dTrafficClassGet (prio=%u) (rc=%d)",prio, rc);
          rc_global = rc;
        }
        else
        {
      ...
      else if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_IPPREC)
      {
        ...
        if (rc != L7_SUCCESS)
        { 
          LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosMapIpPrecTrafficClassGet (IPprec=%u) (rc=%d)",prio, rc);
          rc_global = rc;
        }
        else
        {
      ...
      else if (trust_mode==L7_QOS_COS_MAP_INTF_MODE_TRUST_IPDSCP)
      {
        ...
        for (prio2=0; prio2<8; prio2++)
        {
          ...
          if (rc != L7_SUCCESS)
          {
            LOG_PT_ERR(LOG_CTX_INTF, "Error with usmDbQosCosMapIpDscpTrafficClassGet (DscpPrio=%u) (rc=%d)",prio*8+prio2, rc);
            rc_global = rc;
            break;
          }
          else
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"ShapingRate  = %u",intfQos->shaping_rate);
  LOG_PT_TRACE(LOG_CTX_INTF,"WREDDecayExp = %u",intfQos->wred_decay_exponent);
  LOG_PT_TRACE(LOG_CTX_INTF,"PrioMap.mask   ={0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x}",
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "QoS configuration successfully read from ptin_intf=%u/%u",ptin_intf->intf_type,ptin_intf->intf_id);
  }
  else
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error reading QoS configuration from ptin_intf=%u/%u (rc_global=%d)",ptin_intf->intf_type,ptin_intf->intf_id, rc_global);
  }
...
L7_RC_t ptin_QoS_cos_config_set(const ptin_intf_t *ptin_intf, L7_uint8 cos, ptin_QoS_cos_t *qosConf)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading scheduler type (rc=%d)", rc);
    return rc;
  }
  /* Minimum bandwidth */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading minimum bandwith (rc=%d)", rc);
    return rc;
  }
  /* Maximum bandwidth */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading maximum bandwith (rc=%d)", rc);
    return rc;
  }
  /* Weights list */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading weights list (rc=%d)", rc);
    return rc;
  }
  ...
  for (i=0; i<8; i++)
  {
    ...
    if (qosConf[conf_index].mask & PTIN_QOS_COS_SCHEDULER_MASK)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"Scheduler type in cos=%u, will be updated to %u",i,qosConf[conf_index].scheduler_type);
    }
    /* Minimum bandwidth */
    if (qosConf[conf_index].mask & PTIN_QOS_COS_BW_MIN_MASK)
    {
      minBw_list.bandwidth[i] = qosConf[conf_index].min_bandwidth;
      LOG_PT_TRACE(LOG_CTX_INTF,"Minimum bandwidth in cos=%u, will be updated to %u",i,qosConf[conf_index].min_bandwidth);
    }
    /* Maximum bandwidth */
    if (qosConf[conf_index].mask & PTIN_QOS_COS_BW_MAX_MASK)
    {
      maxBw_list.bandwidth[i] = qosConf[conf_index].max_bandwidth;
      LOG_PT_TRACE(LOG_CTX_INTF,"Maximum bandwidth in cos=%u, will be updated to %u",i,qosConf[conf_index].max_bandwidth);
    }
    /* Scheduler type */
    if (qosConf[conf_index].mask & PTIN_QOS_COS_WRR_WEIGHT_MASK)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"WRR weight for cos=%u, will be updated to %u",i,qosConf[conf_index].wrrSched_weight);
    }
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying scheduler type (rc=%d)", rc);
    rc_global = rc;
  }
  /* Minimum bandwidth */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying minimum bandwith (rc=%d)", rc);
    rc_global = rc;
  }
  /* Maximum bandwidth */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying maximum bandwith (rc=%d)", rc);
    rc_global = rc;
  }
  /* WRR weights */
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying WRR weights (rc=%d)", rc);
    rc_global = rc;
  }
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "QoS configuration successfully applied to ptin_intf=%u/%u, cos=%u",ptin_intf->intf_type,ptin_intf->intf_id,cos);
  }
  else
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error applying QoS configuration to ptin_intf=%u/%u, cos=%u (rc_global=%d)",ptin_intf->intf_type,ptin_intf->intf_id,cos, rc_global);
  }
...
L7_RC_t ptin_QoS_cos_config_get(const ptin_intf_t *ptin_intf, L7_uint8 cos, ptin_QoS_cos_t *qosConf)
{
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"Interface=%u/%u, cos=%u",ptin_intf->intf_type,ptin_intf->intf_id,cos);
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading WRR weights (rc=%d)", rc);
    return rc;
  }
  ...
  for (i=0; i<8; i++)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF,"Scheduler type for cos=%u is %u",i,qosConf[conf_index].scheduler_type);

    qosConf[conf_index].min_bandwidth = minBw_list.bandwidth[i];
    qosConf[conf_index].mask |= PTIN_QOS_COS_BW_MIN_MASK;
    LOG_PT_TRACE(LOG_CTX_INTF,"Minimum bandwith for cos=%u is %u",i,qosConf[conf_index].min_bandwidth);

    qosConf[conf_index].max_bandwidth = maxBw_list.bandwidth[i];
    qosConf[conf_index].mask |= PTIN_QOS_COS_BW_MAX_MASK;
    LOG_PT_TRACE(LOG_CTX_INTF,"Maximum bandwith for cos=%u is %u",i,qosConf[conf_index].max_bandwidth);

    qosConf[conf_index].wrrSched_weight = schedWeight_list.queue_weight[i];
    qosConf[conf_index].mask |= PTIN_QOS_COS_WRR_WEIGHT_MASK;
    LOG_PT_TRACE(LOG_CTX_INTF,"WRR weight for cos=%u is %u",i,qosConf[conf_index].wrrSched_weight);
  }

  LOG_PT_TRACE(LOG_CTX_INTF, "QoS drop configuration successfully read from ptin_intf=%u/%u, cos=%u",ptin_intf->intf_type,ptin_intf->intf_id,cos);
...
L7_RC_t ptin_QoS_drop_config_set(const ptin_intf_t *ptin_intf, L7_uint8 cos, ptin_QoS_drop_t *qosConf)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading mgmtType type (rc=%d)", rc);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading dropParams list (rc=%d)", rc);
    return rc;
  }
  ...
  for (i=0; i<8; i++)
  {
    ...
    if (qosConf[conf_index].mask & PTIN_QOS_COS_QUEUE_MANGM_MASK)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"Mgmt type in cos=%u, will be updated to %u",i,qosConf[conf_index].queue_management_type);
    }
    /* WRED decay exponent */
    if (qosConf[conf_index].mask & PTIN_QOS_COS_WRED_DECAY_EXP_MASK)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"WRED decay Exp. in cos=%u, will be updated to %u",i,qosConf[conf_index].wred_decayExp);
    }
    ...
    if (qosConf[conf_index].mask & PTIN_QOS_COS_WRED_THRESHOLDS_MASK)
    {
      ...
      for (j = 0; j < 4; j++)
      {
        ...
        if (qosConf[conf_index].dp[j].local_mask & PTIN_QOS_COS_DP_TAILDROP_THRESH_MASK)
        {
          ...
          LOG_PT_TRACE(LOG_CTX_INTF,"Taildrop threshold for cos=%u/dp=%u, will be updated to %u",i,j,qosConf[conf_index].dp[j].taildrop_threshold);
        }
        /* WRED min threshold */
        if (qosConf[conf_index].dp[j].local_mask & PTIN_QOS_COS_DP_WRED_THRESH_MIN_MASK)
        {
          ...
          LOG_PT_TRACE(LOG_CTX_INTF,"WRED min threshold for cos=%u/dp=%u, will be updated to %u",i,j,qosConf[conf_index].dp[j].wred_min_threshold);
        }
        /* WRED max threshold */
        if (qosConf[conf_index].dp[j].local_mask & PTIN_QOS_COS_DP_WRED_THRESH_MAX_MASK)
        {
          ...
          LOG_PT_TRACE(LOG_CTX_INTF,"WRED max threshold for cos=%u/dp=%u, will be updated to %u",i,j,qosConf[conf_index].dp[j].wred_max_threshold);
        }
        ...
        if (qosConf[conf_index].dp[j].local_mask & PTIN_QOS_COS_DP_WRED_DROP_PROB_MASK)
        {
          ...
          LOG_PT_TRACE(LOG_CTX_INTF,"WRED drop probability for cos=%u/dp=%u, will be updated to %u",i,j,qosConf[conf_index].dp[j].wred_drop_prob);
        }
      }
    }
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error setting new mgmtType list (rc=%d)", rc);
    rc_global = rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error setting new dropParams list (rc=%d)", rc);
    rc_global = rc;
  }
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "QoS drop configuration successfully applied to ptin_intf=%u/%u, cos=%u",ptin_intf->intf_type,ptin_intf->intf_id,cos);
  }
  else
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error applying QoS drop configuration to ptin_intf=%u/%u, cos=%u (rc_global=%d)",ptin_intf->intf_type,ptin_intf->intf_id,cos, rc_global);
  }
...
L7_RC_t ptin_QoS_drop_config_get(const ptin_intf_t *ptin_intf, L7_uint8 cos, ptin_QoS_drop_t *qosConf)
{
  ...
  for (i=0; i<8; i++)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF,"Mgmt type at cos=%u is %u",i,qosConf[conf_index].queue_management_type);
    ...
    LOG_PT_TRACE(LOG_CTX_INTF,"WRED decay Exp. at cos=%u is %u",i,qosConf[conf_index].wred_decayExp);
    ...
    for (j = 0; j < 4; j++)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"Taildrop threshold for cos=%u/dp=%u is %u",i,j,qosConf[conf_index].dp[j].taildrop_threshold);
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"WRED min threshold for cos=%u/dp=%u is %u",i,j,qosConf[conf_index].dp[j].wred_min_threshold);
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"WRED max threshold for cos=%u/dp=%u is %u",i,j,qosConf[conf_index].dp[j].wred_max_threshold);
      ...
      LOG_PT_TRACE(LOG_CTX_INTF,"WRED drop probability for cos=%u/dp=%u is %u",i,j,qosConf[conf_index].dp[j].wred_drop_prob);
    }
  }
...
static L7_RC_t ptin_intf_PhyConfig_read(ptin_HWEthPhyConf_t *phyConf)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed to get enable state of port# %d", port);
    return L7_FAILURE;
  }
  else
  {
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed to get max frame value of port# %u", port);
    return L7_FAILURE;
  }
  else
  {
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed to get loopback state of port# %u", port);
    return L7_FAILURE;
  }
  else
  {
  ...
  if (usmDbIfDefaultSpeedGet(1, intIfNum, &speed_mode))
  {
    ...
    switch (speed_mode)
    {
      case L7_PORTCTRL_PORTSPEED_AUTO_NEG:
        ...
        LOG_PT_TRACE(LOG_CTX_INTF, " Speed:       Auto-Neg");
        break;
  ...
  phyConf->Mask |= 0x0004;
  phyConf->Duplex = 1;
  LOG_PT_TRACE(LOG_CTX_INTF, " Full-Duplex: %s", phyConf->Duplex?"Enabled":"Disabled");
  ...
  if ( port < PTIN_SYSTEM_N_PONS || port >= PTIN_SYSTEM_N_ETH )
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF, " Media:       Internal");
  }
  else
  {
    phyConf->Media = PHY_PORT_MEDIA_OPTICAL;
    LOG_PT_TRACE(LOG_CTX_INTF, " Media:       Optical");
  }
...
static L7_RC_t ptin_intf_QoS_init(ptin_intf_t *ptin_intf)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Intf %u/%u: failed QoS initialization of interface", ptin_intf->intf_type,ptin_intf->intf_id);
    rc = L7_FAILURE;
  }
  ...
  for (i=0; i<8; i++)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Intf %u/%u: failed QoS initialization of CoS=%u", ptin_intf->intf_type,ptin_intf->intf_id, i);
      rc = L7_FAILURE;
    }
  }
...
L7_RC_t ptin_pcs_prbs_enable(L7_uint32 intIfNum, L7_BOOL enable)
{
  ...
  if (rc!=L7_SUCCESS)  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error setting PRBS enable of intIfNum %u to %u",intIfNum, enable);
    return rc;
  }

  LOG_PT_TRACE(LOG_CTX_INTF,"Success applying global enable of intIfNum %u to %u",intIfNum,enable);
...
L7_RC_t ptin_pcs_prbs_errors_get(L7_uint32 intIfNum, L7_uint32 *counter)
{
  ...
  rc=dtlPtinPcsPrbs(intIfNum, &dapiCmd);
  if (rc!=L7_SUCCESS)  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error getting PRBS errors of intIfNum %u",intIfNum);
    return rc;
  }
...
L7_RC_t ptin_intf_linkscan_control(L7_uint port, L7_BOOL enable)
{
  ...
  if (ptin_intf_port2intIfNum(port, &intIfNum) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting intIfNum from port %u", port);
    return L7_FAILURE;
  }
  ...
  do
  {
    ...
    if (enable) 
    {
      ...
      if (ptin_fgpa_mx_is_matrixactive() && PTIN_BOARD_IS_UPLINK(board_id))
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Uplink port %u: Error disabling force link-up", port);
          break;
        }
        /* Force link-down */
        if ((rc=ptin_intf_link_force(intIfNum, L7_FALSE, 0)) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Uplink port %u: Error forcing link-down", port);
          break;
        }
        /* Enable linkscan */
        if ((rc=ptin_intf_linkscan_set(intIfNum, L7_ENABLE)) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Uplink port %u: Error enabling linkscan", port);
          break;
        }
      }
      ...
      else
      {
        ...
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Board port %u: Error disabling linkscan", port);
          break;
        }
        /* Disable force link-up */
        if ((rc=ptin_intf_link_force(intIfNum, L7_TRUE, L7_DISABLE)) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Board port %u: Error disabling force link-up", port);
          break;
        }
        /* Force link-down */
        if ((rc=ptin_intf_link_force(intIfNum, L7_FALSE, 0)) != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Board port %u: Error forcing link-down", port);
          break;
        }
        ...
        if (PTIN_BOARD_IS_PRESENT(board_id))
        {
          ...
          {
            LOG_PT_ERR(LOG_CTX_INTF, "Board port %u: Error enabling force link-up", port);
            break;
          }
        }
      }
    ...
    else
    {
      ...
      if ((rc=ptin_intf_link_force(intIfNum, L7_TRUE, L7_DISABLE)) != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Port %u: Error disabling link-up force", port);
        break;
      }
      /* Force link-down */
      if ((rc=ptin_intf_link_force(intIfNum, L7_FALSE, 0)) != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Port %u: Error forcing link-down force", port);
        break;
      }
      /* Enable linkscan */
      if ((rc=ptin_intf_linkscan_set(intIfNum, L7_ENABLE)) != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Port %u: Error enabling linkscan", port);
        break;
      }
    }
  } while (0);
  ...
  if (rc == L7_SUCCESS)
    LOG_PT_INFO(LOG_CTX_INTF, "Success setting linkscan to %u of port %u", enable, port); 
  else
    LOG_PT_ERR(LOG_CTX_INTF, "Error setting linkscan to %u of port %u", enable, port);
  #endif
  #endif
  #endif
...
L7_RC_t ptin_intf_slot_reset(L7_int slot_id)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid slot id %d", slot_id);
    return L7_FAILURE;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying HW procedure to slot %d", slot_id);
    return rc;
  }

  LOG_PT_TRACE(LOG_CTX_INTF,"HW procedure applied to slot %d", slot_id);
...
L7_RC_t ptin_intf_linkscan_get(L7_uint32 intIfNum, L7_uint8 *enable)
{
  ...
  if (rc != L7_SUCCESS)
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying HW procedure to intIfNum=%u", intIfNum);
  else
  {
    ...
    LOG_PT_TRACE(LOG_CTX_INTF,"HW linkscan get from intIfNum=%u (%u)", intIfNum, hw_proc.param1);
  }
...
L7_RC_t ptin_intf_linkscan_set(L7_uint32 intIfNum, L7_uint8 enable)
{
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"HW procedure applied to intIfNum=%u", intIfNum);
...
L7_RC_t ptin_intf_link_force(L7_uint32 intIfNum, L7_uint8 link, L7_uint8 enable)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid intIfNum %u -> no ptin_port correspondence", intIfNum);
    return L7_FAILURE;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error applying link force to %u for intIfNum=%u", enable, intIfNum);
    return rc;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_INTF,"Force link to %u, applied to intIfNum=%u", enable, intIfNum);
...
L7_RC_t ptin_slot_linkscan_set(L7_int slot_id, L7_int slot_port, L7_uint8 enable)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid inputs: slot_id=%d, slot_port=%d", slot_id, slot_port);
    return L7_FAILURE;
  }
  ...
  if ( slot_port >= 0)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Invalid reference slot_id=%d, slot_port=%d -> port=%d", slot_id, port_idx, ptin_port);
      return L7_FAILURE;
    }
         ...
         ptin_intf_is_uplinkProtectionActive(ptin_port))
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_INTF,"Error applying LS procedure to slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", slot_id, port_idx, ptin_port, intIfNum);
      else
        LOG_PT_TRACE(LOG_CTX_INTF,"LS procedure applied to slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", slot_id, port_idx, port_idx, intIfNum);
    }
  }
  ...
  else
  {
    ...
    for (port_idx = 0; port_idx < PTIN_SYS_INTFS_PER_SLOT_MAX; port_idx++)
    {
      ...
      {
        LOG_PT_TRACE(LOG_CTX_INTF,"LS procedure applied to slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", slot_id, port_idx, ptin_port, intIfNum);
        /* Next port */
      }
    }
  }
  ...
  if (rc != L7_SUCCESS)
    LOG_PT_ERR(LOG_CTX_INTF,"Terminated with error %d", rc);
  else
    LOG_PT_TRACE(LOG_CTX_INTF,"Finished successfully");
...
L7_RC_t ptin_slot_link_force(L7_int slot_id, L7_int slot_port, L7_uint8 link, L7_uint8 enable)
{
  ...
  if ( slot_port >= 0)
  {
         ...
         ptin_intf_is_uplinkProtectionActive(ptin_port))
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_INTF,"Error forcing link to slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", slot_id, port_idx, ptin_port, intIfNum);
      else
        LOG_PT_TRACE(LOG_CTX_INTF,"Link forced to %u to slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", enable, slot_id, port_idx, port_idx, intIfNum);
    }
  }
  /* Apply to all slot ports */
  else
  {
    ...
    for (port_idx = 0; port_idx < PTIN_SYS_INTFS_PER_SLOT_MAX; port_idx++)
    {
      ...
      {
        LOG_PT_TRACE(LOG_CTX_INTF,"Link forced to %u in slot_id=%d, slot_port=%d -> port=%d / intIfNum=%u", enable, slot_id, port_idx, ptin_port, intIfNum);
        /* Next port */
      }
    }
  }
...
/* Only applied to CXO640G boards */
  ...
  LOG_PT_DEBUG(LOG_CTX_INTF,"Inserting board %u at slot %u", board_id, slot_id);
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid board_id %d", board_id);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting board id for slot %u (rc=%d)", slot_id, rc);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_WARN(LOG_CTX_INTF, "Card already present at slot %u (board id is %u)", slot_id, board_id_current);
    return L7_SUCCESS;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_ERR(LOG_CTX_INTF, "Error inserting card %u at slot %u (%d)", board_id, slot_id, rc);
    return L7_FAILURE;
  }
  ...
  if (slots_to_be_reseted[slot_id])
  {
    ...
    {
      LOG_PT_INFO(LOG_CTX_INTF, "Going to reset warpcore of slot %u", slot_id);
      rc = ptin_intf_slot_reset(slot_id);
      if (rc == L7_SUCCESS)
      {
        LOG_PT_INFO(LOG_CTX_INTF, "Slot %d reseted", slot_id);
      }
      else if (rc == L7_NOT_EXIST)
      {
        LOG_PT_TRACE(LOG_CTX_INTF, "Nothing done to slot %u", slot_id);
      }
      else
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Error reseting slot %u", slot_id);
      }
    }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_ERR(LOG_CTX_MSG, "I am not active matrix");
    return L7_SUCCESS;
  }
  #endif
  ...
  for (port_idx = 0; port_idx < PTIN_SYS_INTFS_PER_SLOT_MAX; port_idx++)
  {
        ...
        ptin_intf_port2intIfNum(ptin_port, &intIfNum) != L7_SUCCESS)
    {
      ...
      LOG_PT_ERR(LOG_CTX_INTF,"Invalid ptin_port %d", ptin_port);
      continue;
    }
    ...
    if (linkscan_update_control && PTIN_BOARD_LS_CTRL(board_id))
    {
      ...
      if (PTIN_BOARD_IS_DOWNLINK(board_id) || ptin_intf_is_uplinkProtection(ptin_port))
      {
        ...
        if (rc != L7_SUCCESS)
        {
          ...
          LOG_PT_ERR(LOG_CTX_INTF, "Error enabling force linkup for port %u (%d)", ptin_port, rc);
        }
        ...
        LOG_PT_INFO(LOG_CTX_INTF, "Forced linkup for port %u", ptin_port);
      }
      ...
      else if (PTIN_BOARD_IS_UPLINK(board_id))
      {
        ...
        {
          rc_global = max(rc, rc_global);
          LOG_PT_ERR(LOG_CTX_INTF, "Error enabling linkscan for port %u (%d)", ptin_port, rc);
        }
        LOG_PT_INFO(LOG_CTX_INTF, "Linkscan enabled for port %u", ptin_port);
      }
    }
    #endif
  }
...
/* Only applied to CXO640G boards */
  ...
  LOG_PT_DEBUG(LOG_CTX_INTF,"Removing board from slot %u", slot_id);
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_ERR(LOG_CTX_INTF, "Error getting board type for slot %u (rc=%d)", slot_id, rc);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_WARN(LOG_CTX_INTF, "No card present at slot %u (board id is %u)", slot_id, board_id);
    return L7_SUCCESS;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_PT_ERR(LOG_CTX_INTF, "Error removing card %u from slot %u (%d)", board_id, slot_id, rc);
    return L7_FAILURE;
  }
  ...
  for (port_idx = 0; port_idx < PTIN_SYS_INTFS_PER_SLOT_MAX; port_idx++)
  {
    ...
    if (linkscan_update_control && PTIN_BOARD_LS_CTRL(board_id))
    {
      ...
      if (PTIN_BOARD_IS_DOWNLINK(board_id) || ptin_intf_is_uplinkProtection(ptin_port))
      {
        ...
        {
          rc_global = max(rc, rc_global);
          LOG_PT_ERR(LOG_CTX_INTF, "Error disabling force linkup for port %u (%d)", ptin_port, rc);
        }
        /* Cause link-down */
        ...
        LOG_PT_INFO(LOG_CTX_INTF, "Force link-up disabled for port %u", ptin_port);
      }
      /* Enable linkscan for uplink boards */
      else if (PTIN_BOARD_IS_UPLINK(board_id))
      {
        ...
        {
          rc_global = max(rc, rc_global);
          LOG_PT_ERR(LOG_CTX_INTF, "Error disabling linkscan (%d)", rc);
        }
        LOG_PT_INFO(LOG_CTX_INTF, "Linkscan disabled for port %u", ptin_port);
      }
    }
    #endif
  }
  ...
  if (rc_global == L7_SUCCESS)
  {
    ...
    LOG_PT_INFO(LOG_CTX_INTF, "Slot %u marked to be reseted ", slot_id);
  }
 #endif
#endif
...
L7_RC_t ptin_intf_info_get(const ptin_intf_t *ptin_intf, L7_uint16 *enable, L7_uint16 *link, L7_uint16 *board_type)
{
  ...
  if (ptin_intf_ptintf2intIfNum(ptin_intf, &intIfNum) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Intf %u/%u does not exist!", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  if (ptin_intf_ptintf2SlotPort(ptin_intf, &slot_id, &slot_port, &board_id) != L7_SUCCESS)
  {
    ...
    slot_port = 0;
    board_id  = 0;
    //LOG_PT_WARN(LOG_CTX_INTF,"Error getting slot information for intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
  }

  /* Admin state */
  if (nimGetIntfAdminState(intIfNum, &adminState) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error getting admin state for intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error getting link state for intf %u/%u", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
...
L7_BOOL ptin_intf_link_get(L7_uint32 ptin_port)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error getting admin state for ptin_port %d", ptin_port);
    return link;
  }
  ...
  if (nimGetIntfLinkState(intIfNum, &linkState) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error getting link state for ptin_port %d", ptin_port);
    return link;
  }
...
L7_RC_t ptin_intf_slotMode_get(L7_uint32 *slotmodes)
{
  ...
  if (rc!=L7_SUCCESS)  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error reading slot mode list");
    return rc;
  }
...
L7_RC_t ptin_intf_slotMode_validate(L7_uint32 *slotmodes)
{
  ...
  rc=dtlPtinSlotMode(&slot_mode);
  if (rc!=L7_SUCCESS)  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error validating slot mode list");
    return rc;
  }
...
L7_BOOL ptin_intf_is_internal_lag_member(L7_uint32 intIfNum)
{
  ...
  for (i=0; i<4; i++)
  {
    ...
    {
      LOG_PT_WARN(LOG_CTX_INTF, "Error getting intIfNum from port %u", port);
      return L7_FALSE;
    }
...
L7_RC_t ptin_intf_protection_cmd(L7_uint slot, L7_uint port, L7_uint cmd)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Cannot find a valid intIfNum from slot/port=%u/%u", slot, port);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "intIfNum %u does not belong to any lag", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "intIfNum %u / ptin_port %u is not a lag", intIfNum, ptin_port);
    return L7_FAILURE;
  }
  ...
  if (lag_idx < PTIN_SYSTEM_PROTECTION_LAGID_BASE)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Protection not applicable to intIfNum %u / ptin_port %u (lag_idx=%u)", intIfNum, ptin_port, lag_idx);
    return L7_FAILURE;
  }
  ...
  if (!ptin_intf_is_uplinkProtection(ptin_port))
  {
    LOG_PT_ERR(LOG_CTX_INTF, "ptin_port %u is not a protection port", ptin_port);
    return L7_FAILURE;
  }
  ...
  if (cmd & 1)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error adding intIfNum %u (ptin_port %u) to lag_intIfNum %u (lag_idx=%u)", intIfNum, ptin_port, lag_intIfNum, lag_idx);
      return L7_FAILURE;
    }
    /* Port is active */
    uplink_protection_ports_active_bmp |= ((L7_uint64) 1 << ptin_port);
    LOG_PT_TRACE(LOG_CTX_INTF, "intIfNum %u (ptin_port %u) added to lag_intIfNum %u (lag_idx=%u)", intIfNum, ptin_port, lag_intIfNum, lag_idx);
  }
  ...
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error removing intIfNum %u (ptin_port %u) from lag_intIfNum %u (lag_idx=%u)", intIfNum, ptin_port, lag_intIfNum, lag_idx);
      return L7_FAILURE;
    }
    /* Port inactive */
    uplink_protection_ports_active_bmp &= ~((L7_uint64) 1 << ptin_port);
    LOG_PT_TRACE(LOG_CTX_INTF, "intIfNum %u (ptin_port %u) removed from lag_intIfNum %u (lag_idx=%u)", intIfNum, ptin_port, lag_intIfNum, lag_idx);
  }
#endif
...
L7_RC_t ptin_intf_protection_cmd_planC(L7_uint slot, L7_uint port, L7_uint cmd)
{
  ...
  if (ptin_intf_slotPort2port(slot, port, &ptin_port) != L7_SUCCESS ||
      ptin_intf_port2intIfNum(ptin_port, &intIfNum) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Slot/port=%u/%u is not valid", slot, port);
    return L7_FAILURE;
  }
  ...
  if (cmd & 1)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error adding port %u to all vlans", ptin_port);
      return L7_FAILURE;
    }
    /* Port is active */
    uplink_protection_ports_active_bmp |= ((L7_uint64) 1 << ptin_port);
    LOG_PT_TRACE(LOG_CTX_INTF, "ptin_port %u added to all vlans", ptin_port);
    ...
    if (ptin_fgpa_mx_is_matrixactive())
    {
      ...
      LOG_PT_TRACE(LOG_CTX_INTF, "Linkscan enabled for port %u", ptin_port);
    }
    #endif
  #else
  ...
  else
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error removing port %u from all vlans", ptin_port);
      return L7_FAILURE;
    }
    //fdbFlushByPort(intIfNum);
    /* Port inactive */
    uplink_protection_ports_active_bmp &= ~((L7_uint64) 1 << ptin_port);
    LOG_PT_TRACE(LOG_CTX_INTF, "ptin_port %u removed from all vlans", ptin_port);
    ...
    if (ptin_fgpa_mx_is_matrixactive())
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Error disabling linkscan for port %u (%d)", ptin_port, rc);
      }
      LOG_PT_TRACE(LOG_CTX_INTF, "Linkscan enabled for port %u", ptin_port);
      if (rc == L7_SUCCESS)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_INTF, "Error forcing linkup for port %u (%d)", ptin_port, rc);
        }
        LOG_PT_TRACE(LOG_CTX_INTF, "Forced link-up for port %u", ptin_port);
      }
    }
    #endif
  #endif
...
L7_RC_t ptin_intf_protection_cmd_planD(L7_uint slot_old, L7_uint port_old, L7_uint slot_new, L7_uint port_new)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Slot/port=%u/%u is not valid", slot_old, port_old);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Slot/port=%u/%u is not valid", slot_new, port_new);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "ptin_port %u or ptin_port %u is not a protection port", ptin_port_old, ptin_port_new);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_WARN(LOG_CTX_INTF, "ptin_port %u is already the active port", ptin_port_new);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error switching port %u to port %u", ptin_port_old, ptin_port_new);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_INTF, "port %u successfully switched to port %u", ptin_port_old, ptin_port_new);
  ...
  if (ptin_fgpa_mx_is_matrixactive())
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error disabling linkscan for port %u (%d)", ptin_port_old, rc);
    }
    LOG_PT_TRACE(LOG_CTX_INTF, "Linkscan enabled for port %u", ptin_port_old);
    if (rc == L7_SUCCESS)
    {
      rc = ptin_intf_link_force(intIfNum_old, L7_TRUE, L7_ENABLE); 
      if (rc != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_INTF, "Error forcing linkup for port %u (%d)", ptin_port_old, rc);
      }
      LOG_PT_TRACE(LOG_CTX_INTF, "Forced link-up for port %u", ptin_port_old);
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error enabling linkscan for port %u (%d)", ptin_port_new, rc);
    }
    LOG_PT_TRACE(LOG_CTX_INTF, "Linkscan enabled for port %u", ptin_port_new);
  }
  #endif
#else
  rc = L7_SUCCESS; /* avoid warning */
...
L7_RC_t ptin_intf_vcap_defvid(L7_uint32 intIfNum, L7_uint16 outerVlan, L7_uint16 innerVlan)
{
  ...
  if (intIfNum == 0 || intIfNum >= L7_ALL_INTERFACES)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid intIfNum %d", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Unable to get intfType from intIfNum %d", intIfNum);
    return L7_FAILURE;
  }
  ...
  else if (intf_type == L7_LAG_INTF)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Unable to get LAG members from intIfNum %d", intIfNum);
      return L7_FAILURE;
    }
  }
  else
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Not supported type (%u) for intIfNum %d", intf_type, intIfNum);
    return L7_FAILURE;
  }
  ...
  for (i = 0; i < intIfNum_list_size; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Error configuring defVid %u+%u for intIfNum %u", outerVlan, innerVlan, intIfNum_list[i]);
      rc_global = rc;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF,"defVid %u+%u for intIfNum %u configured", outerVlan, innerVlan, intIfNum_list[i]);
    }
  }
...
L7_RC_t ptin_intf_clock_recover_set(L7_int ptin_port_main, L7_int ptin_port_bckp)
{
  ...
  {
    LOG_PT_WARN(LOG_CTX_INTF,"Invalid ptin_port %d", ptin_port_main);
    ptin_port_main = -1;
  }
  if (ptin_port_bckp < 0 || ptin_port_bckp >= ptin_sys_number_of_ports)
  {
    LOG_PT_WARN(LOG_CTX_INTF,"Invalid ptin_port %d", ptin_port_bckp);
    ptin_port_bckp = -1;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Error configuring recover clocks (main port %d and backup port %d)", ptin_port_main, ptin_port_bckp);
    return rc;
  }

  LOG_PT_TRACE(LOG_CTX_INTF,"Recover clocks configured (main port %d and backup port %d)", ptin_port_main, ptin_port_bckp);
...
L7_RC_t ptin_intf_frame_oversize_set(L7_uint32 intIfNum, L7_uint32 frame_size)
{
  ...
  for (i = 0; i < intIfNum_list_size; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Error configuring frame_size %u for intIfNum %u", frame_size, intIfNum_list[i]);
      rc_global = rc;
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF,"Max Frame size (%u) configured for intIfNum %u configured", frame_size, intIfNum_list[i]);
    }
  }
...
L7_RC_t ptin_intf_frame_oversize_get(L7_uint32 intIfNum, L7_uint32 *frame_size)
{
  ...
  for (i = 0; i < intIfNum_list_size; i++)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF,"Error reading frame_size for intIfNum %u", intIfNum_list[i]);
      rc_global = rc;
    }
    else 
    {
      LOG_PT_TRACE(LOG_CTX_INTF,"Oversize frame_size for intIfNum %u is %u bytes", intIfNum_list[i], hw_proc.param1);
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF,"Invalid frame_size (%u) for intIfNum %u", fsize, intIfNum_list[i]);
    rc_global = L7_FAILURE;
  }
  ...
  if (rc_global == L7_SUCCESS)
  {
    ...
    {
      *frame_size = fsize;
      LOG_PT_TRACE(LOG_CTX_INTF,"Oversize frame_size for intIfNum %u is %u bytes", intIfNum_list[i], *frame_size);
    }
  }
...
int dapi_usp_is_internal_lag_member(DAPI_USP_t *dusp)
{
  ...
  /* Only applicable to TA48GE boards */
   ...
   {
     LOG_PT_ERR(LOG_CTX_INTF, "usp {%d,%d,%d} is not a LAG", dusp->unit, dusp->slot, dusp->port);
     return L7_FALSE;
   }
   ...
   {
     LOG_PT_ERR(LOG_CTX_INTF, "Error getting intIfNum from usp {%d,%d,%d}", usp.unit, usp.slot, usp.port);
     return L7_FALSE;
   }
   ...
   if (lag_idx >= PTIN_SYSTEM_PROTECTION_LAGID_BASE)
   {
     LOG_PT_TRACE(LOG_CTX_INTF, "intIfNum %u is a special port (lagIdx=%u)", intIfNum, lag_idx);
     return L7_TRUE;
   }
   LOG_PT_WARN(LOG_CTX_INTF, "intIfNum %u is a regular LAG (lagIdx=%u)", intIfNum, lag_idx);
   #endif
...
static L7_RC_t ptin_intf_LagConfig_read(ptin_LACPLagConfig_t *lagInfo)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u is out of range [1..%u]", lag_idx, PTIN_SYSTEM_N_LAGS);
    return L7_FAILURE;
  }
  ...
  if (usmDbDot3adAdminModeGet(1, intIfNum, &value) != L7_SUCCESS) {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading admin field", lag_idx);
    return L7_FAILURE;
  }
  lagInfo->admin = value;
  ...
  if (usmDbDot1sPortStateGet(1, intIfNum, &value) != L7_SUCCESS) {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading Dot1sPortState field", lag_idx);
    return L7_FAILURE;
  }
  ...
  if (usmDbDot3adIsStaticLag(1, intIfNum, &value) != L7_SUCCESS) {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading static mode field", lag_idx);
    return L7_FAILURE;
  }
  ...
  if (usmDbDot3adLagHashModeGet(1, intIfNum, &value) != L7_SUCCESS) {
    LOG_PT_ERR(LOG_CTX_INTF, "LAG# %u: error reading balance mode field", lag_idx);
    return L7_FAILURE;
  }
