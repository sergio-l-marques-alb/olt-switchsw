...
{
  PT_LOG_INFO(LOG_CTX_MSG, "Resetting to default configuration (mode %x)", mode);
...
L7_RC_t ptin_msg_CoS3_get(msg_QoSConfiguration3_t *qos_msg)
{
  ...
  {
    PT_LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.dscp_map.prio_mask = 0x%016llx",qos_msg->ingress.cos_classif.dscp_map.prio_mask);
    for (i = 0; i < 64; i+=8)
    {
...
L7_RC_t ptin_msg_CoS3_set(msg_QoSConfiguration3_t *qos_msg)
{
  ...
  if (rc != L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_MSG,"Error readin interface configuration (rc=%d)", rc);
    return L7_FAILURE;
  }
    ...
    if (qos_msg->ingress.ingress_mask & MSG_QOS3_INGRESS_COS_POLICER_MASK)
    {
      ...
      for (i=0; i<8; i++)
      {
        if (qos_msg->ingress.cos_policer[i].local_mask != 0)
        {
          ...
          PT_LOG_WARN(LOG_CTX_MSG,"Missing policer implementation for CoS %u", i);
        }
      }
    }
  }
...
L7_RC_t ptin_msg_dai_vlan_config(msg_dai_vlan_settings_t *config, L7_uint nElems)
{
  ...
  for (i = 0; i < nElems; i++)
  {
    ...
    else if (item->service.id_type == MSG_ID_NNIVID_TYPE)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_MSG, "NNI VLAN %u is invalid, or don't belong to any EVC!", item->service.id_val.nni_vid);
        rc_global = L7_NOT_EXIST;
        continue;
      }
    }
...
//  PT_LOG_DEBUG(LOG_CTX_MSG," service_id   = %u", item->service.id_val.evc_id);
    ...
    else /*if (item->service.id_type == MSG_ID_NNIVID_TYPE)*/
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_MSG, "NNI VLAN %u is invalid, or don't belong to any EVC!", item->vlan_id /*item->service.id_val.nni_vid*/);
        rc_global = L7_NOT_EXIST;
        continue;
      }
    }
...
L7_RC_t ptin_msg_evc_create(ipc_msg *inbuff, ipc_msg *outbuff)
{
  ...
  for (i=0; i < ptinEvcConf.n_intf; i++)
  {
    ...
    ptinEvcConf.intf[i].new_vid       = msgEvcConf->intf[i].new_vid;
    ptinEvcConf.intf[i].new_inner_vid = msgEvcConf->intf[i].new_inner_vid;
    ptinEvcConf.intf[i].xlate_action_ingress_outerVid = msgEvcConf->intf[i].xlate_action_ingress_outerVid;
    ptinEvcConf.intf[i].xlate_action_ingress_innerVid = msgEvcConf->intf[i].xlate_action_ingress_innerVid;
    ptinEvcConf.intf[i].xlate_action_egress_outerVid  = msgEvcConf->intf[i].xlate_action_egress_outerVid;
    ptinEvcConf.intf[i].xlate_action_egress_innerVid  = msgEvcConf->intf[i].xlate_action_egress_innerVid;
    ...
    PT_LOG_DEBUG(LOG_CTX_MSG, "   .Xlate_new_vlans       = %u + %u", ptinEvcConf.intf[i].new_vid, ptinEvcConf.intf[i].new_inner_vid);
    PT_LOG_DEBUG(LOG_CTX_MSG, "   .Xlate_actions_ingress = %u + %u", ptinEvcConf.intf[i].xlate_action_ingress_outerVid, ptinEvcConf.intf[i].xlate_action_ingress_innerVid);
    PT_LOG_DEBUG(LOG_CTX_MSG, "   .Xlate_actions_egress  = %u + %u", ptinEvcConf.intf[i].xlate_action_egress_outerVid , ptinEvcConf.intf[i].xlate_action_egress_innerVid );
  }
...
L7_RC_t ptin_msg_evc_port_add(ipc_msg *inbuff, ipc_msg *outbuff)
{
  ...
  for (i=0; i<n_size; i++)
  {
    ...
    ptinEvcPort.new_vid       = msgEvcPort[i].intf.new_vid;
    ptinEvcPort.new_inner_vid = msgEvcPort[i].intf.new_inner_vid;
    ptinEvcPort.xlate_action_ingress_outerVid = msgEvcPort[i].intf.xlate_action_ingress_outerVid;
    ptinEvcPort.xlate_action_ingress_innerVid = msgEvcPort[i].intf.xlate_action_ingress_innerVid;
    ptinEvcPort.xlate_action_egress_outerVid  = msgEvcPort[i].intf.xlate_action_egress_outerVid;
    ptinEvcPort.xlate_action_egress_innerVid  = msgEvcPort[i].intf.xlate_action_egress_innerVid;
    ...
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_new_vlans       = %u + %u", ptinEvcPort.new_vid, ptinEvcPort.new_inner_vid);
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_actions_ingress = %u + %u", ptinEvcPort.xlate_action_ingress_outerVid, ptinEvcPort.xlate_action_ingress_innerVid);
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_actions_egress  = %u + %u", ptinEvcPort.xlate_action_egress_outerVid , ptinEvcPort.xlate_action_egress_innerVid );
...
L7_RC_t ptin_msg_evc_config(msg_HwEthMef10EvcOptions_t *msgEvcOptions, L7_uint16 n_size)
{
  ...
  for (i=0; i<n_size; i++)
  {
    PT_LOG_DEBUG(LOG_CTX_MSG, "EVC# %u",     msgEvcOptions[i].id);
    ...
    {
      PT_LOG_ERR(LOG_CTX_MSG, "EVC# %u is out of range [0..%u]", msgEvcOptions[i].id, PTIN_SYSTEM_N_EXTENDED_EVCS-1);
      return L7_FAILURE;
    }
    ...
    {
      PT_LOG_ERR(LOG_CTX_MSG, "Error configuring EVC# %u", msgEvcOptions[i].id);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
        rc_global_failure = rc;
    }
    ...
    {
      PT_LOG_TRACE(LOG_CTX_MSG, "EVC# %u configured successfully", msgEvcOptions[i].id);
    }
  }
...
L7_RC_t ptin_msg_EVCFlow_add(ipc_msg *inbuff, ipc_msg *outbuff)
{
  ...
  for (i=0; i<n_size; i++)
  {
    ...
    /* Copy data */
    evc_ext_id = msgEvcFlow[i].evcId;
    intf_vlan.intf_type = msgEvcFlow[i].intf.intf_type;
    intf_vlan.intf_id   = msgEvcFlow[i].intf.intf_id;
    intf_vlan.vid       = msgEvcFlow[i].intf.vid;
    intf_vlan.vid_inner = msgEvcFlow[i].intf.inner_vid;
    intf_vlan.pcp       = msgEvcFlow[i].intf.pcp;
    intf_vlan.ethertype = msgEvcFlow[i].intf.ethertype;
    intf_vlan.new_vid       = msgEvcFlow[i].intf.new_vid;
    intf_vlan.new_inner_vid = msgEvcFlow[i].intf.new_inner_vid;
    intf_vlan.xlate_action_ingress_outerVid = msgEvcFlow[i].xlate_action_ingress_outerVid;
    intf_vlan.xlate_action_ingress_innerVid = msgEvcFlow[i].xlate_action_ingress_innerVid;
    intf_vlan.xlate_action_ingress_outerVid = msgEvcFlow[i].xlate_action_egress_outerVid;
    intf_vlan.xlate_action_ingress_innerVid = msgEvcFlow[i].xlate_action_egress_innerVid;

    PT_LOG_DEBUG(LOG_CTX_MSG, "EVC# %u Flow",      evc_ext_id);
    PT_LOG_DEBUG(LOG_CTX_MSG, " Port %u/%u",       intf_vlan.intf_type, intf_vlan.intf_id);
    PT_LOG_DEBUG(LOG_CTX_MSG, " UNI-OVID    = %u", intf_vlan.vid);
    PT_LOG_DEBUG(LOG_CTX_MSG, " UNI-IVID    = %u", intf_vlan.vid_inner);
    PT_LOG_DEBUG(LOG_CTX_MSG, " UNI-PCP     = %u", intf_vlan.pcp);
    PT_LOG_DEBUG(LOG_CTX_MSG, " EtherType   = %u", intf_vlan.ethertype);
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_new_vlans       = %u + %u", intf_vlan.new_vid, intf_vlan.new_inner_vid);
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_actions_ingress = %u + %u", intf_vlan.xlate_action_ingress_outerVid, intf_vlan.xlate_action_ingress_innerVid);
    PT_LOG_DEBUG(LOG_CTX_MSG, " .Xlate_actions_egress  = %u + %u", intf_vlan.xlate_action_egress_outerVid , intf_vlan.xlate_action_egress_innerVid );

    if ((rc=ptin_evc_flow_add(evc_ext_id, &intf_vlan, L7_NULLPTR)) != L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_MSG, "Error adding EVC# %u flow", evc_ext_id);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
      {
        rc_global_failure = rc;
...
L7_RC_t ptin_msg_EVCFlow_remove(ipc_msg *inbuff, ipc_msg *outbuff)
{
  ...
  for (i=0; i<n_size; i++)
  {
    ...
    if ((rc=ptin_evc_flow_remove(evc_ext_id, &intf_vlan)) != L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_MSG, "Error removing EVC# %u flow", evc_ext_id);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
      {
        rc_global_failure = rc;
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_add(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels, L7_uint8 isStatic)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_MSG,"Group channel = 0x%08x (ipv6=%u) / %u is not a multicast address",channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstIp.family, channel_list[i].channel_dstmask);
      return L7_FAILURE;
    }
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_remove(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels, L7_uint8 isStatic)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_MSG, "Error (%d) removing group address 0x%08x/%u, source address 0x%08x/%u to MC EVC %u",
              rc,
              channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstmask,
              channel_list[i].channel_srcIp.addr.ipv4, channel_list[i].channel_srcmask,
              channel_list[i].evcid_mc);
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_remove_all(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_MSG, "Error (%d) removing groups to MC EVC %u", rc, channel_list[i].evcid_mc);
      rc_global = rc;
      continue;
    }
...
L7_RC_t ptin_msg_snoop_sync_reply(msg_SnoopSyncReply_t *snoopSyncReply, L7_uint32 numberOfSnoopEntries)
{
    ...
    for(iterator=0;iterator < numberOfSnoopEntries; iterator++)
    {
      ...
      if(snoopSyncReply[iterator].numberOfActivePorts>0)
      {
        ...
        {   
          ...
          {
            PT_LOG_DEBUG(LOG_CTX_PROTB, "Snoop Port Open :%u", intIfNum);
            ...
            {
              PT_LOG_ERR(LOG_CTX_PROTB, "Failed to open port");
              return L7_FAILURE;
            }
  ...
  {
    PT_LOG_NOTICE(LOG_CTX_MSG, "Not sending Another Snoop Sync Request Message to Sync the Remaining Snoop Entries. I'm a Working Matrix on slotId:%u",ptin_fgpa_board_slot());
    return SUCCESS;
  }
  ...
  PT_LOG_DEBUG(LOG_CTX_MSG, "Sending Snoop Sync Request Message [groupAddr:%08X | serviceId:%u] to ipAddr:%08X to Sync the Remaining Snoop Entries", snoopSyncRequest.groupAddr, snoopSyncRequest.serviceId, ipAddr);         
#else
  ...
  {
    PT_LOG_NOTICE(LOG_CTX_MSG, "Not sending Another Snoop Sync Request Message to Sync the Remaining Snoop Entries. I'm a Working slotId/intfNum:%u/%u",protTypebIntfConfig.pairSlotId, protTypebIntfConfig.intfNum);
    return SUCCESS;
  }
...
void ptin_msg_protection_matrix_configuration_flush_end(void)
{
  ...
  { /*Trigger the Sync of the Snooping Table*/   
    ...
    if(!ptin_fgpa_mx_is_matrixactive())//If I'm a Protection Matrix
    {
      ...
      PT_LOG_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Request Message to ipAddr:%08X", ipAddr);
...
//      return;
    ...
    {
      PT_LOG_NOTICE(LOG_CTX_MSG, "Not sending Snoop Sync Request Message. Since, I'm not a protection matrix");      
//    return
    }
  #endif
  }
