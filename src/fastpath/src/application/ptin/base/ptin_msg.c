...
L7_RC_t fp_to_ptin_ip_notation(L7_inet_addr_t *fpIpAddr, chmessage_ip_addr_t *ptinIpAddr)
{
   ...
   {
     LOG_ERR(LOG_CTX_MSG, "IP Family Address of FP not Supported:%u",fpIpAddr->family);
...
L7_RC_t ptin_to_fp_ip_notation(chmessage_ip_addr_t *ptinIpAddr, L7_inet_addr_t *fpIpAddr)
{
   ...
   {
     LOG_ERR(LOG_CTX_MSG, "IP Family Address of PTIN not Supported:%u",fpIpAddr->family);
     return L7_NOT_SUPPORTED;
   }   
}
...
{
  LOG_INFO(LOG_CTX_MSG, "Resetting to default configuration (mode %x)", mode);
...
L7_RC_t ptin_msg_typeBprotIntfSwitchNotify(msg_HwTypeBProtSwitchNotify_t *msg)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Type-B Protection switch notification");
  LOG_DEBUG(LOG_CTX_MSG, " slotId = %u"   , msg->slotId);
  LOG_DEBUG(LOG_CTX_MSG, " portId = %u", msg->portId); //ptin_port format
  LOG_DEBUG(LOG_CTX_MSG, " cmd    = %08X" , msg->cmd);

  /* Convert portId to intfNum */
  if (ptin_intf_port2intIfNum(msg->portId, &intIfNum)!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Non existent port");
    return L7_FAILURE;
  }
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to set interface's type-b protection configurations");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_typeBprotIntfConfig(msg_HwTypeBProtIntfConfig_t *msg)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Configurations");
  LOG_DEBUG(LOG_CTX_MSG, " slotId     = %u"   , msg->slotId);
  LOG_DEBUG(LOG_CTX_MSG, " intfId     = %u/%u", msg->intfId.intf_type, msg->intfId.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, " pairSlotId = %u"   , msg->pairSlotId);
  LOG_DEBUG(LOG_CTX_MSG, " pairIntfId = %u/%u", msg->pairIntfId.intf_type, msg->pairIntfId.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, " intfRole   = %u"   , msg->intfRole);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid port");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_typeBprotSwitch(msg_HwTypeBprot_t *msg)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "ptin_msg_typeBprotSwitch(slot %d, port %d)", msg->slot, msg->port);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to reset MGMD General Queriers");
  }
#endif

  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "return %d", rc);
  }
...
L7_RC_t ptin_msg_board_action(msg_HwGenReq_t *msg)
{
  ...
  LOG_INFO(LOG_CTX_MSG, "ptin_msg_board_action");
  LOG_DEBUG(LOG_CTX_MSG," slot       = %u", msg->slot_id);
  LOG_DEBUG(LOG_CTX_MSG," generic_id = %u", msg->generic_id);
  LOG_DEBUG(LOG_CTX_MSG," type       = 0x%02x", msg->type);
  LOG_DEBUG(LOG_CTX_MSG," param      = 0x%02x", msg->param);
  ...
  if (msg->type == 0x03)
  {
    LOG_INFO(LOG_CTX_MSG,"Insertion detected (slot %u, board_id=%u)", msg->generic_id, msg->param);

    rc = ptin_slot_action_insert(msg->generic_id, msg->param);
    if ( rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error inserting card (%d)", rc);
    }
    else
    {
      LOG_INFO(LOG_CTX_MSG, "Card inserted successfully");
    }
  }
  /* Board removed */
  else if (msg->type == 0x00)
  {
    LOG_INFO(LOG_CTX_MSG,"Remotion detected (slot %u)", msg->generic_id);

    rc = ptin_slot_action_remove(msg->generic_id);
    if ( rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error removing card (%d)", rc);
    }
    else
    {
      LOG_INFO(LOG_CTX_MSG, "Card removed successfully");
    }
  }
  #endif
...
L7_RC_t ptin_msg_link_action(msg_HwGenReq_t *msg)
{
  ...
  LOG_INFO(LOG_CTX_MSG, "ptin_msg_link_action");
  LOG_DEBUG(LOG_CTX_MSG," slot       = %u", msg->slot_id);
  LOG_DEBUG(LOG_CTX_MSG," generic_id = %u", msg->generic_id);
  LOG_DEBUG(LOG_CTX_MSG," type       = 0x%02x", msg->type);
  LOG_DEBUG(LOG_CTX_MSG," param      = 0x%02x", msg->param);
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_ERR(LOG_CTX_MSG, "Error getting board_id for slot id %u (rc=%d)", msg->generic_id, rc);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_ERR(LOG_CTX_MSG, "Not an uplink board (board_id=%u)", board_type);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_ERR(LOG_CTX_MSG, "No ptin_port related to slot/port %u/%u", msg->generic_id, msg->param);
    return L7_FAILURE;
  }
  if (ptin_intf_port2intIfNum(ptin_port, &intIfNum) != L7_SUCCESS)
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_ERR(LOG_CTX_MSG, "No intIfNum related to ptin_port %u", ptin_port);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_boardaction_sem);
    LOG_ERR(LOG_CTX_MSG, "ptin_port %u is not a protection port", ptin_port);
    return L7_FAILURE;
  }
  ...
  if (msg->type == 0x01)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Link-up detected at ptin_port %u", ptin_port);
    ...
    if (linkscan_update_control)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_API, "Error enabling force linkup for port %u (%d)", ptin_port, rc);
      }
      ...
      LOG_DEBUG(LOG_CTX_API, "Forced linkup for port %u", ptin_port);
    }
    #endif
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG,"Link-down detected at ptin_port %u", ptin_port);
    ...
    if (linkscan_update_control)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_API, "Error disabling force linkup for port %u (%d)", ptin_port, rc);
      }
      ...
      LOG_DEBUG(LOG_CTX_API, "Force link-up disabled for port %u", ptin_port);
    }
    #endif
  }
...
{
  LOG_INFO(LOG_CTX_MSG, "Resetting alarms states");
...
L7_RC_t ptin_msg_hw_resources_get(msg_ptin_policy_resources *msgResources)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error consulting hardware resources");
    return rc;
  }
...
L7_RC_t ptin_msg_PhyConfig_set(msg_HWEthPhyConf_t *msgPhyConf)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Port # %u",           msgPhyConf->Port);
  LOG_DEBUG(LOG_CTX_MSG, " Enable        = %u", msgPhyConf->PortEnable );
  LOG_DEBUG(LOG_CTX_MSG, " Speed         = %u", msgPhyConf->Speed );
  LOG_DEBUG(LOG_CTX_MSG, " Duplex        = %u", msgPhyConf->Duplex );
  LOG_DEBUG(LOG_CTX_MSG, " Media         = %u", msgPhyConf->Media );
  LOG_DEBUG(LOG_CTX_MSG, " MaxFrame      = %u", msgPhyConf->MaxFrame );
  LOG_DEBUG(LOG_CTX_MSG, " Loopback      = %u", msgPhyConf->LoopBack );
  LOG_DEBUG(LOG_CTX_MSG, " MACLearn Prio = %u", msgPhyConf->MacLearning );
  LOG_DEBUG(LOG_CTX_MSG, " Mask          = 0x%04X", msgPhyConf->Mask );
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error applying configurations on port# %u", phyConf.Port);
...
L7_RC_t ptin_msg_PhyConfig_get(msg_HWEthPhyConf_t *msgPhyConf)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting configurations of port# %u", phyConf.Port);
    memset(msgPhyConf, 0x00, sizeof(msg_HWEthPhyConf_t));
...
L7_RC_t ptin_msg_PhyState_get(msg_HWEthPhyState_t *msgPhyState)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting state of port# %u", phyState.Port);
    memset(msgPhyState, 0x00, sizeof(msg_HWEthPhyState_t));
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting statistics of port# %u", portStats.Port);
    memset(msgPhyState, 0x00, sizeof(msg_HWEthPhyState_t));
...
//msgPhyState->LOS                = 0;    /* Always FALSE */
  ...
  /* Output info read */
  LOG_DEBUG(LOG_CTX_MSG, "Port # %u",                   msgPhyState->Port);
  LOG_DEBUG(LOG_CTX_MSG, " Mask             = 0x%04X",  msgPhyState->Mask );
  LOG_DEBUG(LOG_CTX_MSG, " Speed            = %u",      msgPhyState->Speed );
  LOG_DEBUG(LOG_CTX_MSG, " Duplex           = %u",      msgPhyState->Duplex );
  LOG_DEBUG(LOG_CTX_MSG, " LinkUp           = %s",      msgPhyState->LinkUp?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " AutoNegComplete  = %s",      msgPhyState->AutoNegComplete?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " Collisions       = %s",      msgPhyState->Collisions?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " RxActivity       = %s",      msgPhyState->RxActivity?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " TxActivity       = %s",      msgPhyState->TxActivity?"Yes":"No" );
//LOG_DEBUG(LOG_CTX_MSG, " TxFault          = %s",      msgPhyState->TxFault?"Yes":"No" );
//LOG_DEBUG(LOG_CTX_MSG, " RemoteFault      = %s",      msgPhyState->RemoteFault?"Yes":"No" );
//LOG_DEBUG(LOG_CTX_MSG, " LOS              = %s",      msgPhyState->LOS?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " Media            = %u",      msgPhyState->Media );
  LOG_DEBUG(LOG_CTX_MSG, " MTU_mismatch     = %s",      msgPhyState->MTU_mismatch?"Yes":"No" );
  LOG_DEBUG(LOG_CTX_MSG, " Support.MaxFrame = %u",      msgPhyState->Supported_MaxFrame );
...
L7_RC_t ptin_msg_PhyActivity_get(msg_HWEthPhyActivity_t *msgPhyAct)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unknown interface (slot=%u/%u)", msgPhyAct->intf.slot, msgPhyAct->intf.port);
    return L7_FAILURE;
  }
  ...
  /* Output info read */
  LOG_TRACE(LOG_CTX_MSG, "Slot/Port # %u/%u",           msgPhyAct->intf.slot, msgPhyAct->intf.port);
  LOG_TRACE(LOG_CTX_MSG, " Mask             = 0x%02x",  msgPhyAct->Mask );
  LOG_TRACE(LOG_CTX_MSG, " RX Activity      = %u",      msgPhyAct->RxActivity );
  LOG_TRACE(LOG_CTX_MSG, " TX Activity      = %u",      msgPhyAct->TxActivity );
...
L7_RC_t ptin_msg_PhyStatus_get(msg_HWEthPhyStatus_t *msgPhyStatus)
{
  ...
  /* Output info read */
  LOG_DEBUG(LOG_CTX_MSG, "Port # %u",                   msgPhyStatus->Port);
  LOG_DEBUG(LOG_CTX_MSG, " alarmes          = 0x%04X",  msgPhyStatus->phy.alarmes );
  LOG_DEBUG(LOG_CTX_MSG, " alarmes_mask     = 0x%04X",  msgPhyStatus->phy.alarmes_mask );
...
L7_RC_t ptin_msg_PhyCounters_read(msg_HwGenReq_t *msgRequest, msg_HWEthRFC2819_PortStatistics_t *msgPortStats, L7_uint nElems)
{
  ...
  for (i=0; i<nElems; i++)
  {
    ...
    /* Output info read */
    LOG_TRACE(LOG_CTX_MSG, "Slotid=%u, Port # %2u", p_msgPortStats->SlotId, p_msgPortStats->Port);
    LOG_TRACE(LOG_CTX_MSG, " Rx.DropEvents           = %15llu  | Tx.DropEvents           = %15llu", p_msgPortStats->Rx.etherStatsDropEvents,           msgPortStats->Tx.etherStatsDropEvents);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Octets               = %15llu  | Tx.Octets               = %15llu", p_msgPortStats->Rx.etherStatsOctets,               msgPortStats->Tx.etherStatsOctets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts                 = %15llu  | Tx.Pkts                 = %15llu", p_msgPortStats->Rx.etherStatsPkts,                 msgPortStats->Tx.etherStatsPkts);
    LOG_TRACE(LOG_CTX_MSG, " Rx.BroadcastPkts        = %15llu  | Tx.BroadcastPkts        = %15llu", p_msgPortStats->Rx.etherStatsBroadcastPkts,        msgPortStats->Tx.etherStatsBroadcastPkts);
    LOG_TRACE(LOG_CTX_MSG, " Rx.MulticastPkts        = %15llu  | Tx.MulticastPkts        = %15llu", p_msgPortStats->Rx.etherStatsMulticastPkts,        msgPortStats->Tx.etherStatsMulticastPkts);
    LOG_TRACE(LOG_CTX_MSG, " Rx.CRCAlignErrors       = %15llu  | Tx.CRCAlignErrors       = %15llu", p_msgPortStats->Rx.etherStatsCRCAlignErrors,       msgPortStats->Tx.etherStatsCRCAlignErrors);
    LOG_TRACE(LOG_CTX_MSG, " Rx.UndersizePkts        = %15llu  | Tx.OversizePkts         = %15llu", p_msgPortStats->Rx.etherStatsUndersizePkts,        msgPortStats->Tx.etherStatsOversizePkts);
    LOG_TRACE(LOG_CTX_MSG, " Rx.OversizePkts         = %15llu  | Tx.Fragments            = %15llu", p_msgPortStats->Rx.etherStatsOversizePkts,         msgPortStats->Tx.etherStatsFragments);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Fragments            = %15llu  | Tx.Jabbers              = %15llu", p_msgPortStats->Rx.etherStatsFragments,            msgPortStats->Tx.etherStatsJabbers);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Jabbers              = %15llu  | Tx.Collisions           = %15llu", p_msgPortStats->Rx.etherStatsJabbers,              msgPortStats->Tx.etherStatsCollisions);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts64Octets         = %15llu  | Tx.Pkts64Octets         = %15llu", p_msgPortStats->Rx.etherStatsPkts64Octets,         msgPortStats->Tx.etherStatsPkts64Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts65to127Octets    = %15llu  | Tx.Pkts65to127Octets    = %15llu", p_msgPortStats->Rx.etherStatsPkts65to127Octets,    msgPortStats->Tx.etherStatsPkts65to127Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts128to255Octets   = %15llu  | Tx.Pkts128to255Octets   = %15llu", p_msgPortStats->Rx.etherStatsPkts128to255Octets,   msgPortStats->Tx.etherStatsPkts128to255Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts256to511Octets   = %15llu  | Tx.Pkts256to511Octets   = %15llu", p_msgPortStats->Rx.etherStatsPkts256to511Octets,   msgPortStats->Tx.etherStatsPkts256to511Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts512to1023Octets  = %15llu  | Tx.Pkts512to1023Octets  = %15llu", p_msgPortStats->Rx.etherStatsPkts512to1023Octets,  msgPortStats->Tx.etherStatsPkts512to1023Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts1024to1518Octets = %15llu  | Tx.Pkts1024to1518Octets = %15llu", p_msgPortStats->Rx.etherStatsPkts1024to1518Octets, msgPortStats->Tx.etherStatsPkts1024to1518Octets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Pkts1519toMaxOctets  = %15llu  | Tx.Pkts1519toMaxOctets  = %15llu", p_msgPortStats->Rx.etherStatsPkts1519toMaxOctets,  msgPortStats->Tx.etherStatsPkts1519toMaxOctets);
    LOG_TRACE(LOG_CTX_MSG, " Rx.Throughput (bps)     = %15llu  | Tx.Throughput (bps)     = %15llu", p_msgPortStats->Rx.Throughput,                     msgPortStats->Tx.Throughput);
  }
...
L7_RC_t ptin_msg_PhyCounters_clear(msg_HWEthRFC2819_PortStatistics_t *msgPortStats)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of port# %u", portStats.Port);
    memset(msgPortStats, 0x00, sizeof(msg_HWEthRFC2819_PortStatistics_t));

    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG, "Port# %u counters cleared", portStats.Port);
...
L7_RC_t ptin_msg_intfInfo_get(msg_HwIntfInfo_t *intf_info)
{
  ...
  for (ptin_port = 0; ptin_port < ptin_sys_number_of_ports; ptin_port++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error reading info for ptin_port %d", ptin_port);
      return L7_FAILURE;
    }
    ...
    LOG_TRACE(LOG_CTX_MSG, "port=%u: boardId=%u admin=%u link=%u", ptin_port, board_id, admin, link);
  }
  /* Number of ports */
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "I am inactive matrix");
    return L7_FAILURE;
  }
  #endif
  #endif
...
L7_RC_t ptin_msg_slotMode_get(msg_slotModeCfg_t *slotMode)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error obtaining list of slot modes");
    return L7_FAILURE;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Slot mode list:");
  for (i=0; i<PTIN_SYS_SLOTS_MAX; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Slot %02u: Mode=%u", i+1, slot_list[i]);
  }
  LOG_INFO(LOG_CTX_MSG,"Success");
...
L7_RC_t ptin_msg_slotMode_validate(msg_slotModeCfg_t *slotMode)
{
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG,"Idx%02u: Slot %02u Active=%u Mode=%u", i,
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Slot mode list to be validated:");
  for (i=0; i<PTIN_SYS_SLOTS_MAX; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Slot %02u: Mode=%u", i+1, slot_list[i]);
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Slot mode list is not valid! (rc=%d)",rc);
    return rc;
  }
  LOG_INFO(LOG_CTX_MSG, "Slot mode list is valid!");
...
L7_RC_t ptin_msg_portExt_set(msg_HWPortExt_t *portExt, L7_uint nElems)
{
  ...
  for (i=0; i<nElems; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error setting MEF EXT configurations");
      return L7_FAILURE;
    }
  }

  LOG_DEBUG(LOG_CTX_MSG, "Success setting MEF EXT configurations",__FUNCTION__);
...
L7_RC_t ptin_msg_portExt_get(msg_HWPortExt_t *portExt, L7_uint *nElems)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading interface %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
    return L7_FAILURE;
  }
  ...
  for (port=port_start; port<=port_end; port++)
  {
    ...
    {
      LOG_WARNING(LOG_CTX_MSG,"Error reading port %u",port);
      continue;
    }
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error getting MEF EXT parameters for interface=%u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
      return L7_FAILURE;
    }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Success reading MEF EXT parameters for %u interfaces",index);
...
L7_RC_t ptin_msg_portMAC_set(msg_HWPortMac_t *portMac, L7_uint nElems)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Processing %u structures",nElems);

  for (i=0; i<nElems; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Structure %u",i);
    LOG_DEBUG(LOG_CTX_MSG," SlotId = %u",portMac[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Intf   = %u/%u",portMac[i].intf.intf_type,portMac[i].intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG," Mask   = 0x%04x",portMac[i].Mask);
    LOG_DEBUG(LOG_CTX_MSG," MAC    = %02x:%02x:%02x:%02x:%02x:%02x",
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error setting MAC address");
      return L7_FAILURE;
    }
  }

  LOG_DEBUG(LOG_CTX_MSG, "Success setting MAC address",__FUNCTION__);
...
L7_RC_t ptin_msg_portMAC_get(msg_HWPortMac_t *portMac, L7_uint *nElems)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Going to read %u structures",port_end-port_start+1);
  ...
  for (port=port_start; port<=port_end; port++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error getting MAC address for interface=%u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG,"Structure %u",index);
    LOG_DEBUG(LOG_CTX_MSG," SlotId = %u",portMac[index].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Intf   = %u/%u",portMac[index].intf.intf_type,portMac[index].intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG," Mask   = 0x%04x",portMac[index].Mask);
    LOG_DEBUG(LOG_CTX_MSG," MAC    = %02x:%02x:%02x:%02x:%02x:%02x",
...
L7_RC_t ptin_msg_CoS_get(msg_QoSConfiguration_t *qos_msg)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading interface QoS configuration");
    return rc;
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading QoS configuration for all 8 CoS");
    return rc;
  }

  LOG_DEBUG(LOG_CTX_MSG, "Slotid         = %u",qos_msg->SlotId);
  LOG_DEBUG(LOG_CTX_MSG, "Interface      = %u/%u",qos_msg->intf.intf_type,qos_msg->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "General Mask   = 0x%02X",qos_msg->mask);
  LOG_DEBUG(LOG_CTX_MSG, "Trust mode     = %u",qos_msg->trust_mode);
  LOG_DEBUG(LOG_CTX_MSG, "Bandwidth unit = %u",qos_msg->bandwidth_unit);
  LOG_DEBUG(LOG_CTX_MSG, "Shaping rate   = %u",qos_msg->shaping_rate);
  LOG_DEBUG(LOG_CTX_MSG, "pktprio.cos    = [ %Xh %Xh %Xh %Xh %Xh %Xh %Xh %Xh ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Priority Mask  = 0x%02X",qos_msg->pktprio.mask);
  LOG_DEBUG(LOG_CTX_MSG, "CoS Mask       = 0x%02X",qos_msg->cos_config.mask);
  LOG_DEBUG(LOG_CTX_MSG, "CoS Spec. Mask = [ 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Queue_scheduler = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Queue_min_bw    = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Queue_max_bw    = [ %u %u %u %u %u %u %u %u ]",
...
L7_RC_t ptin_msg_CoS_set(msg_QoSConfiguration_t *qos_msg)
{
  ...
  if (qos_intf.mask)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error configuring priority map (rc=%d)", rc);
      rc_global = rc;
    }
  }
  ...
  if (qos_msg->mask & MSG_QOS_CONFIGURATION_QOSCONF_MASK)
  {
    ...
    if (qos_msg->cos_config.mask)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error configuring QoS (rc=%d)",rc);
        rc_global = rc;
      }
    }
  }
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG,"Success applying QoS configurations to all CoS");
  }
  else
  {
    LOG_ERR(LOG_CTX_MSG,"Error applying QoS configurations to all CoS (rc_global=%d)", rc_global);
  }
...
L7_RC_t ptin_msg_CoS2_get(msg_QoSConfiguration2_t *qos_msg)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading QoS drop configuration for all 8 CoS");
    return rc;
  }

  LOG_DEBUG(LOG_CTX_MSG, "Slotid            = %u",qos_msg->SlotId);
  LOG_DEBUG(LOG_CTX_MSG, "Interface         = %u/%u",qos_msg->intf.intf_type,qos_msg->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "General Mask      = 0x%02X",qos_msg->generic_mask);
  LOG_DEBUG(LOG_CTX_MSG, "Trust mode        = %u",qos_msg->trust_mode);
  LOG_DEBUG(LOG_CTX_MSG, "Bandwidth unit    = %u",qos_msg->bandwidth_unit);
  LOG_DEBUG(LOG_CTX_MSG, "Shaping rate      = %u",qos_msg->shaping_rate);
  LOG_DEBUG(LOG_CTX_MSG, "pktprio.prio_mask = [ 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "pktprio.cos       = [ %Xh %Xh %Xh %Xh %Xh %Xh %Xh %Xh ]",
            qos_msg->pktprio.cos[0],qos_msg->pktprio.cos[1],qos_msg->pktprio.cos[2],qos_msg->pktprio.cos[3],qos_msg->pktprio.cos[4],qos_msg->pktprio.cos[5],qos_msg->pktprio.cos[6],qos_msg->pktprio.cos[7]);
  LOG_DEBUG(LOG_CTX_MSG, "CoS Mask       = 0x%02X",qos_msg->cos_config.cos_mask);
  LOG_DEBUG(LOG_CTX_MSG, "CoS local Mask = [ 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Queue_max_bw    = [ %u %u %u %u %u %u %u %u ]",
            qos_msg->cos_config.cos[0].max_bandwidth, qos_msg->cos_config.cos[1].max_bandwidth, qos_msg->cos_config.cos[2].max_bandwidth, qos_msg->cos_config.cos[3].max_bandwidth, qos_msg->cos_config.cos[4].max_bandwidth, qos_msg->cos_config.cos[5].max_bandwidth, qos_msg->cos_config.cos[6].max_bandwidth, qos_msg->cos_config.cos[7].max_bandwidth);
  LOG_DEBUG(LOG_CTX_MSG, "wrrSched_weight = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "dropMgmttype    = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "wred_decayExp   = [ %u %u %u %u %u %u %u %u ]",
  ...
  for (i=0; i<4; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG, "DP%u: local_mask = [ 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "DP%u: Taildrop threshold = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "DP%u: WRED min.threshold = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "DP%u: WRED drop prob.    = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
...
L7_RC_t ptin_msg_CoS2_set(msg_QoSConfiguration2_t *qos_msg)
{
  ...
  if (qos_msg->generic_mask & MSG_QOS_CONFIGURATION_QOSCONF_MASK)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error configuring QoS Drop management (rc=%d)",rc);
      rc_global = rc;
    }
  }
...
L7_RC_t ptin_msg_CoS3_get(msg_QoSConfiguration3_t *qos_msg)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading interface configuration (rc=%d)", rc);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading QoS configurations (rc=%d)",rc);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading QoS Drop management configurations (rc=%d)",rc);
    return rc;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Slotid         = %u",qos_msg->SlotId);
  LOG_DEBUG(LOG_CTX_MSG, "Interface      = %u/%u",qos_msg->intf.intf_type,qos_msg->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "Main Mask      = 0x%02x",qos_msg->main_mask);
  LOG_DEBUG(LOG_CTX_MSG, "Bandwidth unit = %u",qos_msg->bandwidth_unit);
  LOG_DEBUG(LOG_CTX_MSG, "Ingress:");
  LOG_DEBUG(LOG_CTX_MSG, "  Ingress Mask = 0x%02x",qos_msg->ingress.ingress_mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Trust mode   = %u",qos_msg->ingress.trust_mode);
  if (qos_msg->ingress.trust_mode == L7_QOS_COS_MAP_INTF_MODE_TRUST_DOT1P)
  {
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.pcp_map.prio_mask = 0x%02x",qos_msg->ingress.cos_classif.pcp_map.prio_mask);
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.pcp_map.cos = { %u %u %u %u %u %u %u %u } ",
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.ipprec_map.prio_mask = 0x%02x",qos_msg->ingress.cos_classif.ipprec_map.prio_mask);
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.ipprec_map.cos = { %u %u %u %u %u %u %u %u } ",
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.dscp_map.prio_mask = 0x%016llx",qos_msg->ingress.cos_classif.dscp_map.prio_mask);
    for (i = 0; i < 64; i+=8)
    {
      LOG_DEBUG(LOG_CTX_MSG, "  cos_classif.dscp_map.cos[%2u-%2u] = { %u %u %u %u %u %u %u %u } ", i, i+7,
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG, "  cos_classif does not have usable information.");
  }
  for (i = 0; i < 64; i+=8)
  {
    LOG_DEBUG(LOG_CTX_MSG, "  Policer[%u]: mask=0x%02x - cir=%u eir=%u cbs=%u ebs=%u",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Egress:");
  LOG_DEBUG(LOG_CTX_MSG, "  Egress Mask  = 0x%02x", qos_msg->egress.egress_mask); 
  LOG_DEBUG(LOG_CTX_MSG, "  Shaping rate = %u %%", qos_msg->egress.shaping_rate);

  LOG_DEBUG(LOG_CTX_MSG, "  cos_scheduler->local_mask      = [ 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_scheduler->schedulerType   = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_scheduler->wrrSched_weight = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_shaper->local_mask         = [ 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_shaper->min_bandwidth      = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_shaper->max_bandwidth      = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->local_mask       = [ 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->dropMgmttype     = [ %u %u %u %u %u %u %u %u ]",
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->wred_decayExp    = [ %u %u %u %u %u %u %u %u ]",
  ...
  for (i=0; i<6; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->dp_threshold[%u]: local_mask = [ 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->dp_threshold[%u]: Taildrop threshold = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->dp_threshold[%u]: WRED min.threshold = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
    ...
    LOG_DEBUG(LOG_CTX_MSG, "  cos_dropmgmt->dp_threshold[%u]: WRED drop prob.    = [ %3u %3u %3u %3u %3u %3u %3u %3u ]", i+1,
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Dump finished!");
...
L7_RC_t ptin_msg_CoS3_set(msg_QoSConfiguration3_t *qos_msg)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG,"Error readin interface configuration (rc=%d)", rc);
    return L7_FAILURE;
  }
    ...
    if (qos_msg->ingress.ingress_mask & MSG_QOS3_INGRESS_COS_POLICER_MASK)
    {
      ...
      for (i=0; i<8; i++)
      {
        if (qos_msg->ingress.cos_policer[i].local_mask != 0)
        {
          ...
          LOG_WARNING(LOG_CTX_MSG,"Missing policer implementation for CoS %u", i);
        }
      }
    }
  }
  ...
  if (apply_conf)
  {
    LOG_TRACE(LOG_CTX_MSG,"Applying Interface configurations...");
    /* Execute priority map configuration */
    rc = ptin_QoS_intf_config_set(&ptin_intf, &qos_intf);
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error applying interface configuration (rc=%d)", rc);
      rc_global = rc;
    }
    else
    {
      LOG_TRACE(LOG_CTX_MSG,"Interface successfully configured");
    }
  }
  ...
  if (apply_conf)
  {
    LOG_TRACE(LOG_CTX_MSG,"Applying QoS configurations...");
    rc = ptin_QoS_cos_config_set(&ptin_intf, (L7_uint8)-1, qos_cos); 
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error configuring QoS (rc=%d)",rc);
    ...
    {
      LOG_TRACE(LOG_CTX_MSG,"QoS successfully configured");
    }
  }
  ...
  if (apply_conf)
  {
    LOG_TRACE(LOG_CTX_MSG,"Applying Drop Management configurations...");
    rc = ptin_QoS_drop_config_set(&ptin_intf, (L7_uint8)-1, qos_drop);
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error configuring QoS Drop management (rc=%d)",rc);
    ...
    {
      LOG_TRACE(LOG_CTX_MSG,"QoS Drop management successfully configurted");
    }
  }
...
L7_RC_t ptin_msg_Lag_get(msg_LACPLagInfo_t *lagInfo, L7_uint *nElems)
{
  ...
  *nElems = 0;
  ...
  for (lag_idx=start, array_idx=0; lag_idx<=end; lag_idx++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error reading LAG# %u", lag_idx);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "LAG# %2u", (L7_uint32)lagInfo[array_idx].id);
    LOG_DEBUG(LOG_CTX_MSG, " .admin            = %u", lagInfo[array_idx].admin);
    LOG_DEBUG(LOG_CTX_MSG, " .static_enable    = %u", lagInfo[array_idx].static_enable);
    LOG_DEBUG(LOG_CTX_MSG, " .stp_enable       = %u", lagInfo[array_idx].stp_enable);
    LOG_DEBUG(LOG_CTX_MSG, " .loadBalance_mode = %u", lagInfo[array_idx].loadBalance_mode);
    LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[0]  = 0x%08X", lagInfo[array_idx].members_pbmp32[0]);
    LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[1]  = 0x%08X", lagInfo[array_idx].members_pbmp32[1]);
...
L7_RC_t ptin_msg_Lag_create(msg_LACPLagInfo_t *lagInfo)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "LAG# %2u",                    (L7_uint32)lagInfo->id);
  LOG_DEBUG(LOG_CTX_MSG, " .admin            = %u",     lagInfo->admin);
  LOG_DEBUG(LOG_CTX_MSG, " .static_enable    = %u",     lagInfo->static_enable);
  LOG_DEBUG(LOG_CTX_MSG, " .stp_enable       = %u",     lagInfo->stp_enable);
  LOG_DEBUG(LOG_CTX_MSG, " .loadBalance_mode = %u",     lagInfo->loadBalance_mode);
  LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[0]  = 0x%08X", lagInfo->members_pbmp32[0]);
  LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[1]  = 0x%08X", lagInfo->members_pbmp32[1]);

  if (ptin_intf_Lag_create(&ptinLagConf) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Failed to create LAG# %u", (L7_uint32)lagInfo->id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_Lag_destroy(msg_LACPLagInfo_t *lagInfo)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Failed to delete LAG# %u", (L7_uint32)lagInfo->id);
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG, "LAG# %u: successfully deleted", (L7_uint32)lagInfo->id);
...
L7_RC_t ptin_msg_LagStatus_get(msg_LACPLagStatus_t *lagStatus, L7_uint *nElems)
{
  ...
  *nElems = 0;
  ...
  for (lag_idx=start, array_idx=0; lag_idx<=end; lag_idx++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error getting status of LAG# %u", lag_idx);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "LAG# %2u", (L7_uint32)lagStatus[array_idx].id);
    LOG_DEBUG(LOG_CTX_MSG, " .admin                  = %s",     lagStatus[array_idx].admin?"ON":"OFF");
    LOG_DEBUG(LOG_CTX_MSG, " .link_status            = %s",     lagStatus[array_idx].link_status?"UP":"DOWN");
    LOG_DEBUG(LOG_CTX_MSG, " .port_channel_type      = %s",     lagStatus[array_idx].port_channel_type?"STATIC":"DYNAMIC");
    LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[0]        = 0x%08X", lagStatus[array_idx].members_pbmp32[0]);
    LOG_DEBUG(LOG_CTX_MSG, " .members_pbmp[1]        = 0x%08X", lagStatus[array_idx].members_pbmp32[1]);
    LOG_DEBUG(LOG_CTX_MSG, " .active_members_pbmp[0] = 0x%08X", lagStatus[array_idx].active_members_pbmp32[0]);
    LOG_DEBUG(LOG_CTX_MSG, " .active_members_pbmp[1] = 0x%08X", lagStatus[array_idx].active_members_pbmp32[1]);
...
L7_RC_t ptin_msg_LACPAdminState_set(msg_LACPAdminState_t *adminState, L7_uint nElems)
{
  ...
  for (i=0; i<nElems; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting Port# %u LACP Admin State", ptinAdminState.port);
      return L7_FAILURE;
    }

    LOG_DEBUG(LOG_CTX_MSG, "Port# %2u LACP Admin State configured", ptinAdminState.port);
    LOG_DEBUG(LOG_CTX_MSG, " .state_aggregation = %s", ptinAdminState.state_aggregation?"Enabled":"Disabled");
    LOG_DEBUG(LOG_CTX_MSG, " .lacp_activity     = %s", ptinAdminState.lacp_activity?"True":"False");
    LOG_DEBUG(LOG_CTX_MSG, " .lacp_timeout      = %s", ptinAdminState.lacp_timeout?"Short":"Long");
  }
...
L7_RC_t ptin_msg_LACPAdminState_get(msg_LACPAdminState_t *adminState, L7_uint *nElems)
{
  ...
  *nElems = 0;
  ...
  for (port_idx=start, array_idx=0; port_idx<=end; port_idx++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error getting admin status of Port# %u", port_idx);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "Port# %2u", (L7_uint32)adminState[array_idx].id);
    LOG_DEBUG(LOG_CTX_MSG, " .state_aggregation = %s", adminState[array_idx].state_aggregation?"Enabled":"Disabled");
    LOG_DEBUG(LOG_CTX_MSG, " .lacp_activity     = %s", adminState[array_idx].lacp_activity?"True":"False");
    LOG_DEBUG(LOG_CTX_MSG, " .lacp_timeout      = %s", adminState[array_idx].lacp_timeout?"Short":"Long");
...
L7_RC_t ptin_msg_LACPStats_get(msg_LACPStats_t *lagStats, L7_uint *nElems)
{
  ...
  *nElems = 0;
  ...
  for (port_idx=start, array_idx=0; port_idx<=end; port_idx++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error getting status of Port# %u", port_idx);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "Port# %2u", (L7_uint32)lagStats[array_idx].id);
    LOG_DEBUG(LOG_CTX_MSG, " .LACPdus_rx = %u", lagStats[array_idx].LACPdus_rx);
    LOG_DEBUG(LOG_CTX_MSG, " .LACPdus_tx = %u", lagStats[array_idx].LACPdus_tx);
...
L7_RC_t ptin_msg_LACPStats_clear(msg_LACPStats_t *lagStats)
{
  ...
  for (port_idx=start, array_idx=0; port_idx<=end; port_idx++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error clearing LACP statistics of Port# %u", port_idx);
      return L7_FAILURE;
    }

    LOG_DEBUG(LOG_CTX_MSG, "Port# %2u: LACP statistics successfully cleared", port_idx);
  }
...
{
  LOG_DEBUG(LOG_CTX_MSG, "Message function '%s' is being executed",__FUNCTION__);
  ...
  if (switch_config->mask & 0x0001)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting aging time to %u seconds",switch_config->aging_time);
      return L7_FAILURE;
    }
    else
    {
      LOG_DEBUG(LOG_CTX_MSG, "Success setting aging time to %u seconds",switch_config->aging_time);
    }

    /* Set ARP timeout to be the same as MAC age time */
    if(L7_SUCCESS != usmDbIpArpAgeTimeSet(1, switch_config->aging_time))
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting ARP timeout to %u seconds", switch_config->aging_time);
      return L7_FAILURE;
    }
    else
    {
      LOG_DEBUG(LOG_CTX_MSG, "Success setting ARP timeout to %u seconds", switch_config->aging_time);
    }
  }
...
L7_RC_t ptin_msg_l2_switch_config_get(msg_switch_config_t *switch_config)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting aging time");
    return L7_FAILURE;
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Success getting aging time (%u seconds)",switch_config->aging_time);
...
L7_RC_t ptin_msg_l2_macTable_get(msg_switch_mac_table_t *mac_table, int struct1or2)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Invalid argument");
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG," SlotId       = %u",mac_table->intro.slotId);
  LOG_DEBUG(LOG_CTX_MSG," StartEntryId = %u",mac_table->intro.startEntryId);
  LOG_DEBUG(LOG_CTX_MSG," NumEntries   = %u",mac_table->intro.numEntries);
  ...
  if (startId>=maxSize)
  {
    ...
    LOG_WARNING(LOG_CTX_MSG," StartEntryId is higher than 999");
    return L7_SUCCESS;
  }
  ...
  if (startId+numEntries>=maxSize)
  {
    ...
    LOG_WARNING(LOG_CTX_MSG," First majoration of Number of entries to %u",numEntries);
  }
  if (numEntries>MSG_CMDGET_MAC_TABLE_MAXENTRIES)
  {
    numEntries = MSG_CMDGET_MAC_TABLE_MAXENTRIES;
    LOG_WARNING(LOG_CTX_MSG," Second majoration of entries limited to %u",numEntries);
  }
  ...
  if (mac_table->intro.startEntryId==0)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error loading table");
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error getting MAC list");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_l2_macTable_remove(msg_switch_mac_table_entry_t *mac_table, L7_uint16 numEntries)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Message function '%s' being executed",__FUNCTION__);
  ...
  if (numEntries > MSG_CMDGET_MAC_TABLE_MAXENTRIES)
  {
    LOG_WARNING(LOG_CTX_MSG,"numEntries limited from %u to %u", numEntries, MSG_CMDGET_MAC_TABLE_MAXENTRIES);
    numEntries = MSG_CMDGET_MAC_TABLE_MAXENTRIES;
  }

  LOG_DEBUG(LOG_CTX_MSG," NumEntries   = %u", numEntries);
  ...
  if (numEntries == 0)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error flushing MAC table");
      return L7_FAILURE;
    }
    else
    {
      LOG_WARNING(LOG_CTX_MSG,"MAC table will flushed");
      return L7_SUCCESS;
    }
  }
  ...
  for (i = 0; i < numEntries; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error removing index entry %u",i);
      rc = L7_FAILURE;
    }
    else
    {
      LOG_DEBUG(LOG_CTX_MSG,"Success adding index entry %u",i);
    }
  }
...
L7_RC_t ptin_msg_l2_macTable_add(msg_switch_mac_table_entry_t *mac_table, L7_uint16 numEntries)
{
  ...
  for (i = 0; i < numEntries; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Error adding index entry %u",i);
      rc = L7_FAILURE;
    }
    else
    { 
...
L7_RC_t ptin_msg_l2_maclimit_config(msg_l2_maclimit_config_t *maclimit)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," slotId       = %u",      maclimit->slotId);
  LOG_DEBUG(LOG_CTX_MSG," interface    = %u/%u",   maclimit->intf.intf_type, maclimit->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," mask         = 0x%.8X",  maclimit->mask);
  LOG_DEBUG(LOG_CTX_MSG," vid          = %u",      maclimit->vid);
  LOG_DEBUG(LOG_CTX_MSG," system       = %u",      maclimit->system);
  LOG_DEBUG(LOG_CTX_MSG," limit        = %u",      maclimit->limit);
  LOG_DEBUG(LOG_CTX_MSG," action       = %u",      maclimit->action);
  LOG_DEBUG(LOG_CTX_MSG," trap         = %u",      maclimit->send_trap);
  ...
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_EVC,"Invalid ptin_intf");
      return L7_FAILURE;
    }
...
L7_RC_t ptin_msg_l2_maclimit_status(msg_l2_maclimit_status_t *maclimit_status)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," slotId       = %u",      maclimit_status->slotId);
  LOG_DEBUG(LOG_CTX_MSG," interface    = %u/%u",   maclimit_status->intf.intf_type, maclimit_status->intf.intf_id);
  ...
  LOG_DEBUG(LOG_CTX_MSG," Status Response");
  LOG_DEBUG(LOG_CTX_MSG," slotId       = %u",      maclimit_status->slotId);
  LOG_DEBUG(LOG_CTX_MSG," interface    = %u/%u",   maclimit_status->intf.intf_type, maclimit_status->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," MacLearned   = %u",      maclimit_status->number_mac_learned);
  LOG_DEBUG(LOG_CTX_MSG," Status       = %u",      maclimit_status->status);
  LOG_DEBUG(LOG_CTX_MSG," Mask         = %u",      maclimit_status->mask);
...
L7_RC_t ptin_msg_dai_global_config(msg_dai_global_settings_t *config)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," SlotId = %u", config->slotId);
  LOG_DEBUG(LOG_CTX_MSG," Mask   = 0x%02x", config->mask);
  LOG_DEBUG(LOG_CTX_MSG," Enable = %u", config->global_enable);
  LOG_DEBUG(LOG_CTX_MSG," SrcMAC Validate = %u", config->validate_smac);
  LOG_DEBUG(LOG_CTX_MSG," DstMAC Validate = %u", config->validate_dmac);
  LOG_DEBUG(LOG_CTX_MSG," IPAddr Validate = %u", config->validate_ipAddr);
  ...
  if (config->mask & 0x02)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting SMAC validation (%u)", config->validate_smac);
      rc_global = rc;
    ...
    {
      LOG_TRACE(LOG_CTX_MSG, "SMAC validation set to %u", config->validate_smac);
    }
  }
  ...
  if (config->mask & 0x04)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting DMAC validation (%u)", config->validate_smac);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
        rc_global_failure = rc;
    }
    ...
    {
      LOG_TRACE(LOG_CTX_MSG, "DMAC validation set to %u", config->validate_dmac);
    }
  }
  ...
  if (config->mask & 0x08)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error setting IP validation (%u)", config->validate_ipAddr);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
        rc_global_failure = rc;
    }
    ...
    {
      LOG_TRACE(LOG_CTX_MSG, "IPAddr validation set to %u", config->validate_ipAddr);
    }
  }
...
L7_RC_t ptin_msg_dai_intf_config(msg_dai_intf_settings_t *config, L7_uint nElems)
{
  ...
  for (i = 0; i < nElems; i++)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," SlotId = %u", item->slotId);
    LOG_DEBUG(LOG_CTX_MSG," Mask   = 0x%02x", item->mask);
    LOG_DEBUG(LOG_CTX_MSG," Interface      = %u/%u", item->intf.intf_type, item->intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG," Rate Limit     = %u", item->rateLimit);
    LOG_DEBUG(LOG_CTX_MSG," Burst Interval = %u", item->burstInterval);
    ...
    if (item->mask & 0x01)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error setting trust mode (%u) for ptin_intf %u/%u", item->trust, ptin_intf.intf_type, ptin_intf.intf_id);
        rc_global = rc;
        if (IS_FAILURE_ERROR(rc))
          rc_global_failure = rc;
      }
      ...
      {
        LOG_TRACE(LOG_CTX_MSG, "Trust mode of intf %u/%u set to %u", item->intf.intf_type, item->intf.intf_id, item->trust);
      }
    }
    /* Rate Limit */
    if (item->mask & 0x02)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error setting rate limit (%u) for ptin_intf %u/%u", item->rateLimit, ptin_intf.intf_type, ptin_intf.intf_id);
        rc_global = rc;
        if (IS_FAILURE_ERROR(rc))
          rc_global_failure = rc;
      }
      ...
      {
        LOG_TRACE(LOG_CTX_MSG, "Rate Limit of intf %u/%u set to %u", item->intf.intf_type, item->intf.intf_id, item->rateLimit);
      }
    }
    /* Burst interval */
    if (item->mask & 0x04)
    {
      if (usmDbDaiIntfRateLimitGet(intIfNum, &rate) == L7_SUCCESS && rate != -1)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_ERR(LOG_CTX_MSG, "Error setting burst interval (%u) for ptin_intf %u/%u", item->burstInterval, ptin_intf.intf_type, ptin_intf.intf_id);
          rc_global = rc;
          if (IS_FAILURE_ERROR(rc))
            rc_global_failure = rc;
        }
        ...
        {
          LOG_TRACE(LOG_CTX_MSG, "Burst interval of intf %u/%u set to %u", item->intf.intf_type, item->intf.intf_id, item->burstInterval);
        }
      }
      else
      {
        LOG_TRACE(LOG_CTX_MSG, "Burst interval of intf %u/%u ignored", item->intf.intf_type, item->intf.intf_id);
      }
    }
  }
...
L7_RC_t ptin_msg_dai_vlan_config(msg_dai_vlan_settings_t *config, L7_uint nElems)
{
  ...
  for (i = 0; i < nElems; i++)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," SlotId = %u", item->slotId);
    LOG_DEBUG(LOG_CTX_MSG," service_type = %u", item->service.id_type);
    LOG_DEBUG(LOG_CTX_MSG," service_id   = %u", item->service.id_val.evc_id);
    LOG_DEBUG(LOG_CTX_MSG," Mask         = 0x%02x", item->mask);
    LOG_DEBUG(LOG_CTX_MSG," DAI enable   = %u", item->dai_enable);
    LOG_DEBUG(LOG_CTX_MSG," Static Flag  = %u", item->staticFlag);
    ...
    if (item->service.id_type == MSG_ID_DEF_TYPE || item->service.id_type == MSG_ID_EVC_TYPE)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "eEVC#%u is out of range!", item->service.id_val.evc_id);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
      /* EVC must be active */
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "eEVC#%u is not in use!", item->service.id_val.evc_id);
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "Cannot get intVlan from eEVC#%u!", item->service.id_val.evc_id, dai_intVid_list[0]);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
    }
    ...
    else if (item->service.id_type == MSG_ID_NNIVID_TYPE)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "NNI VLAN %u is out of range!", item->service.id_val.nni_vid);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "NNI VLAN %u is invalid, or don't belong to any EVC!", item->service.id_val.nni_vid);
        rc_global = L7_NOT_EXIST;
        continue;
      }
    }
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid service type %u", item->service.id_type);
      rc_global = L7_NOT_SUPPORTED;
      continue;
    }

    LOG_TRACE(LOG_CTX_MSG, "Going to process %u VLANs", dai_maxVlans);
    ...
    for (vlan_index = 0; vlan_index < dai_maxVlans; vlan_index++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "Int. VLAN %u is out of range!", vlanId);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "VLAN %u is invalid, or don't belong to any EVC!", vlanId);
        rc_global = L7_NOT_EXIST;
        continue;
      }
      ...
      if (item->mask & 0x01)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_ERR(LOG_CTX_MSG, "Error setting enable state (%u) for VLAN %u", item->dai_enable, vlanId);
          rc_global = rc;
          if (IS_FAILURE_ERROR(rc))
            rc_global_failure = rc;
        }
        ...
        {
          LOG_TRACE(LOG_CTX_MSG, "DAI enable for VLAN %u set to %u", vlanId, item->dai_enable);
        }
      }
      /* Static flag */
      if (item->mask & 0x02)
      {
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_ERR(LOG_CTX_MSG, "Error setting static flag (%u) for VLAN %u", item->staticFlag, vlanId);
          rc_global = rc;
          if (IS_FAILURE_ERROR(rc))
            rc_global_failure = rc;
        }
        ...
        {
          LOG_TRACE(LOG_CTX_MSG, "Static flag for VLAN %u set to %u", vlanId, item->staticFlag);
        }
      }
    }
  }
...
L7_RC_t ptin_msg_dai_stats_get(msg_dai_statistics_t *msg_stats, L7_uint nElems)
{
  ...
  for (i = 0; i < nElems; i++)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG,"Stats index %u:", i);
    LOG_DEBUG(LOG_CTX_MSG," SlotId       = %u", item->slotId);
//  LOG_DEBUG(LOG_CTX_MSG," service_type = %u", item->service.id_type);
//  LOG_DEBUG(LOG_CTX_MSG," service_id   = %u", item->service.id_val.evc_id);
    LOG_DEBUG(LOG_CTX_MSG," EVC id       = %u", item->evc_idx);
    LOG_DEBUG(LOG_CTX_MSG," VLAN id      = %u", item->vlan_id);
    LOG_DEBUG(LOG_CTX_MSG," Intf         = %u", item->intf.intf_type, item->intf.intf_id);
    ...
    if (item->evc_idx != (L7_uint32) -1)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "eEVC#%u is out of range!", item->evc_idx /*item->service.id_val.evc_id*/);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
      /* EVC is active? */
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "eEVC#%u is not in use!", item->evc_idx /*item->service.id_val.evc_id*/);
        rc_global = L7_NOT_EXIST;
        continue;
      }
      /* Get internal VLAN from eEVC# */
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "Cannot get intVlan from eEVC#%u!", item->evc_idx /*item->service.id_val.evc_id*/);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
    }
    ...
    else /*if (item->service.id_type == MSG_ID_NNIVID_TYPE)*/
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "VLAN %u is out of valid range!", item->vlan_id /*item->service.id_val.nni_vid*/);
        rc_global = rc_global_failure = L7_FAILURE;
        continue;
      }
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "NNI VLAN %u is invalid, or don't belong to any EVC!", item->vlan_id /*item->service.id_val.nni_vid*/);
        rc_global = L7_NOT_EXIST;
        continue;
      }
    }
    ...
    for (vlan_index = 0; vlan_index < dai_maxVlans; vlan_index++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "VLAN %u is not used by DAI!", vlanId);
        rc_global = rc;
        if (IS_FAILURE_ERROR(rc))
          rc_global_failure = rc;
        continue;
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error getting Stats from VLAN %u!", vlanId);
        rc_global = rc;
        if (IS_FAILURE_ERROR(rc))
          rc_global_failure = rc;
        continue;
...
L7_RC_t ptin_msg_EVC_get(msg_HwEthMef10Evc_t *msgEvcConf)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "EVC# %u is out of range [0..%u]", msgEvcConf->id, PTIN_SYSTEM_N_EXTENDED_EVCS-1);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting EVC# %u configuration", ptinEvcConf.index);
    return L7_FAILURE;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "EVC# %u",              msgEvcConf->id);
  LOG_DEBUG(LOG_CTX_MSG, " .Flags    = 0x%08X",  msgEvcConf->flags);
  LOG_DEBUG(LOG_CTX_MSG, " .Type     = %u",      msgEvcConf->type);
  LOG_DEBUG(LOG_CTX_MSG, " .MC Flood = %u (%s)", msgEvcConf->mc_flood, msgEvcConf->mc_flood==0?"All":msgEvcConf->mc_flood==1?"Unknown":"None");
  LOG_DEBUG(LOG_CTX_MSG, " .Nr.Intf  = %u",      msgEvcConf->n_intf);
  ...
  for (i=0; i < ptinEvcConf.n_intf; i++)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG, "   %s# %02u %s VID=%04u",
...
L7_RC_t ptin_msg_EVC_create(msg_HwEthMef10Evc_t *msgEvcConf)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "EVC# %u",              ptinEvcConf.index);
  LOG_DEBUG(LOG_CTX_MSG, " .Flags    = 0x%08X",  ptinEvcConf.flags);
  LOG_DEBUG(LOG_CTX_MSG, " .Type     = %u",      ptinEvcConf.type);
  LOG_DEBUG(LOG_CTX_MSG, " .MC Flood = %u (%s)", ptinEvcConf.mc_flood, ptinEvcConf.mc_flood==0?"All":ptinEvcConf.mc_flood==1?"Unknown":"None");
  LOG_DEBUG(LOG_CTX_MSG, " .Nr.Intf  = %u",      ptinEvcConf.n_intf);
  ...
  for (i=0; i < ptinEvcConf.n_intf; i++)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG, "ptin_sys_number_of_ports (%d)", ptin_sys_number_of_ports);
    ...
    if ( (msgEvcConf->intf[i].intf_id == ptin_sys_number_of_ports) && (msgEvcConf->intf[i].intf_type == PTIN_EVC_INTF_PHYSICAL))
    {
      ...
      LOG_DEBUG(LOG_CTX_MSG, "PTP EVC (%u)",              ptinEvcConf.index);
      break;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "   %s# %02u %s VID=%04u/%-04u",
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error creating/reconfiguring EVC# %u", ptinEvcConf.index);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_EVC_delete(msg_HwEthMef10EvcRemove_t *msgEvcConf, L7_uint16 n_structs)
{
  ...
  for (i=0; i<n_structs; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "EVC# %u is out of range [0..%u]", msgEvcConf[i].id, PTIN_SYSTEM_N_EXTENDED_EVCS-1);
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error deleting EVC# %u", msgEvcConf[i].id);
      rc_global = L7_FAILURE;
      continue;
    }

    LOG_DEBUG(LOG_CTX_MSG, "EVC# %u successfully deleted", msgEvcConf[i].id);
  }
...
L7_RC_t ptin_msg_evc_port(msg_HWevcPort_t *msgEvcPort, L7_uint16 n_size, ptin_msg_oper_t oper)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "No data provided");
    return L7_FAILURE;
  }
  ...
  for (i=0; i<n_size; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "EVC# %u is out of range [0..%u]", msgEvcPort[i].evcId, PTIN_SYSTEM_N_EXTENDED_EVCS-1);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "EVC# %u - oper %s",     msgEvcPort[i].evcId,
              ((oper==PTIN_MSG_OPER_ADD) ? "ADD" : ((oper==PTIN_MSG_OPER_REMOVE) ? "REMOVE" : "UNKNOWN")));
    LOG_DEBUG(LOG_CTX_MSG, " .Intf      = %u/%u",   ptinEvcPort.intf_type, ptinEvcPort.intf_id);
    LOG_DEBUG(LOG_CTX_MSG, " .IntfType  = %s",     (ptinEvcPort.mef_type == PTIN_EVC_INTF_LEAF) ? "LEAF" : "ROOT");
    LOG_DEBUG(LOG_CTX_MSG, " .OuterVlan = %u",      ptinEvcPort.vid);
    LOG_DEBUG(LOG_CTX_MSG, " .InnerVlan = %u",      ptinEvcPort.vid_inner);
    ...
    case PTIN_MSG_OPER_ADD:
      ...
      {        
        ...
        if (IS_FAILURE_ERROR(rc))
        {
          LOG_ERR(LOG_CTX_MSG, "Error adding port %u/%u to EVC# %u (rc:%u)", ptinEvcPort.intf_type, ptinEvcPort.intf_id, msgEvcPort[i].evcId, rc);
          rc_global_failure = rc;
        }
        else
        {
      ...
      {
        LOG_TRACE(LOG_CTX_MSG, "Added port %u/%u to EVC# %u", ptinEvcPort.intf_type, ptinEvcPort.intf_id, msgEvcPort[i].evcId);
      }
      break;
    case PTIN_MSG_OPER_REMOVE:
      if ((rc=ptin_evc_port_remove(msgEvcPort[i].evcId, &ptinEvcPort)) != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error removing port %u/%u to EVC# %u", ptinEvcPort.intf_type, ptinEvcPort.intf_id, msgEvcPort[i].evcId);
        rc_global = rc;
        if (IS_FAILURE_ERROR(rc))
          rc_global_failure = rc;
      }
      ...
      {
        LOG_TRACE(LOG_CTX_MSG, "Removed port %u/%u from EVC# %u", ptinEvcPort.intf_type, ptinEvcPort.intf_id, msgEvcPort[i].evcId);
      }
      break;
    default:
      LOG_ERR(LOG_CTX_MSG, "Unknown operation %u", oper);
      rc_global = L7_FAILURE;
    }
  }
...
L7_RC_t ptin_msg_evc_config(msg_HwEthMef10EvcOptions_t *msgEvcOptions, L7_uint16 n_size)
{
  ...
  for (i=0; i<n_size; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG, "EVC# %u",     msgEvcOptions[i].id);
    LOG_DEBUG(LOG_CTX_MSG, " .Mask      = 0x%04x", msgEvcOptions[i].mask);
    LOG_DEBUG(LOG_CTX_MSG, " .Flags     = 0x%08x/0x%08x", msgEvcOptions[i].flags.value, msgEvcOptions[i].flags.mask);
    LOG_DEBUG(LOG_CTX_MSG, " .Type      = %u", msgEvcOptions[i].type);
    LOG_DEBUG(LOG_CTX_MSG, " .MC_flood  = %u", msgEvcOptions[i].mc_flood);

    /* Validate EVC# range (EVC index [0..PTIN_SYSTEM_N_EXTENDED_EVCS[) */
    if (msgEvcOptions[i].id >= PTIN_SYSTEM_N_EXTENDED_EVCS)
    {
      LOG_ERR(LOG_CTX_MSG, "EVC# %u is out of range [0..%u]", msgEvcOptions[i].id, PTIN_SYSTEM_N_EXTENDED_EVCS-1);
      return L7_FAILURE;
    }
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error configuring EVC# %u", msgEvcOptions[i].id);
      rc_global = rc;
      if (IS_FAILURE_ERROR(rc))
        rc_global_failure = rc;
    }
    ...
    {
      LOG_TRACE(LOG_CTX_MSG, "EVC# %u configured successfully", msgEvcOptions[i].id);
    }
  }
...
L7_RC_t ptin_msg_EVCBridge_add(msg_HwEthEvcBridge_t *msgEvcBridge)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "EVC# %u Bridge",         ptinEvcBridge.index);
  LOG_DEBUG(LOG_CTX_MSG, " %s# %u",                ptinEvcBridge.intf.intf_type == PTIN_EVC_INTF_PHYSICAL ? "PHY":"LAG",
            ptinEvcBridge.intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, " .Inner VID       = %u", ptinEvcBridge.inn_vlan);
  LOG_DEBUG(LOG_CTX_MSG, " .Outer VID [NEW] = %u", ptinEvcBridge.intf.vid);

  if (ptin_evc_p2p_bridge_add(&ptinEvcBridge) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error adding EVC# %u bridge", ptinEvcBridge.index);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_EVCBridge_remove(msg_HwEthEvcBridge_t *msgEvcBridge)
{
  ...
  if ( rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error removing EVC# %u bridge", ptinEvcBridge.index);
    return rc;
  }
...
L7_RC_t ptin_msg_EVCFlow_add(msg_HwEthEvcFlow_t *msgEvcFlow)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "EVC# %u Flow",     ptinEvcFlow.evc_idx);
  LOG_DEBUG(LOG_CTX_MSG, " Flags = 0x%08x",  ptinEvcFlow.flags);
  LOG_DEBUG(LOG_CTX_MSG, " %s# %u",          ptinEvcFlow.ptin_intf.intf_type == PTIN_EVC_INTF_PHYSICAL ? "PHY":"LAG",
                                                  ptinEvcFlow.ptin_intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, " Int.IVID    = %u", ptinEvcFlow.int_ivid);
  LOG_DEBUG(LOG_CTX_MSG, " UNI-OVID    = %u", ptinEvcFlow.uni_ovid);
  LOG_DEBUG(LOG_CTX_MSG, " UNI-IVID    = %u", ptinEvcFlow.uni_ivid);
  LOG_DEBUG(LOG_CTX_MSG, " macLearnMax = %u", ptinEvcFlow.macLearnMax);  

  if (ptinEvcFlow.flags & PTIN_EVC_MASK_IGMP_PROTOCOL)
  {
    if  (msgEvcFlow->mask > PTIN_MSG_EVC_FLOW_MASK_VALID)
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid Mask [mask:0x%02x",msgEvcFlow->mask);
      return L7_FAILURE;
    }
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid Admission Control Parameters [mask:0x%02x maxBandwidth:%llu bits/s maxChannels:%hu]",msgEvcFlow->mask, msgEvcFlow->maxBandwidth, msgEvcFlow->maxChannels);
      return L7_FAILURE;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, " onuId       = %u", ptinEvcFlow.onuId);
    LOG_DEBUG(LOG_CTX_MSG, " mask        = 0x%x", ptinEvcFlow.mask);
    LOG_DEBUG(LOG_CTX_MSG, " maxChannels = %u", ptinEvcFlow.maxChannels);
    LOG_DEBUG(LOG_CTX_MSG, " maxBandwidth= %llu bit/s", ptinEvcFlow.maxBandwidth);
#endif
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error adding EVC# %u flow", ptinEvcFlow.evc_idx);
    return rc;
  }
...
L7_RC_t ptin_msg_EVCFlow_remove(msg_HwEthEvcFlow_t *msgEvcFlow)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error removing EVC# %u flow", ptinEvcFlow.evc_idx);
    return rc;
  }
...
L7_RC_t ptin_msg_EvcFloodVlan_add(msg_HwEthEvcFloodVlan_t *msgEvcFlood, L7_uint n_clients)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid params");
    return L7_FAILURE;
  }
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG,"EVC flood vlan %u:",i);
    LOG_DEBUG(LOG_CTX_MSG," Slot    = %u",    msgEvcFlood[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," EVC_idx = %u",    msgEvcFlood[i].evcId);
    LOG_DEBUG(LOG_CTX_MSG," Mask    = 0x%02x",msgEvcFlood[i].mask);
    LOG_DEBUG(LOG_CTX_MSG," Intf    = %u/%u", msgEvcFlood[i].intf.intf_type, msgEvcFlood[i].intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG," CVlan   = %u",    msgEvcFlood[i].client_vlan);
    LOG_DEBUG(LOG_CTX_MSG," Outer Vlan : %u", msgEvcFlood[i].oVlanId);
    LOG_DEBUG(LOG_CTX_MSG," Inner Vlan : %u", msgEvcFlood[i].iVlanId);
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error adding EVC# %u flooding vlan", msgEvcFlood[i].evcId);
      rc = L7_FAILURE;
    }
  }
...
L7_RC_t ptin_msg_EvcFloodVlan_remove(msg_HwEthEvcFloodVlan_t *msgEvcFlood, L7_uint n_clients)
{
  ...
  for ( i=0; i<n_clients; i++)
  {
    ...
    if (ptin_evc_flood_vlan_remove( msgEvcFlood[i].evcId,
                                    ...
                                    msgEvcFlood[i].iVlanId ) != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error removing EVC# %u flooding vlan", msgEvcFlood[i].evcId);
      rc = L7_FAILURE;
    }
  }
...
L7_RC_t ptin_msg_bwProfile_get(msg_HwEthBwProfile_t *msgBwProfile)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Starting message processing...");

  /* Validate arguments */
  if (msgBwProfile==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_MSG,"Invalid Message Pointer");
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG," evcId  = %u",msgBwProfile->evcId);
  LOG_DEBUG(LOG_CTX_MSG," mask   = 0x%02x",msgBwProfile->mask);
  LOG_DEBUG(LOG_CTX_MSG," SVID   = %u",msgBwProfile->service_vlan);
  LOG_DEBUG(LOG_CTX_MSG," CVID   = %u",msgBwProfile->client_vlan);
  LOG_DEBUG(LOG_CTX_MSG," SrcIntf= %u/%u",msgBwProfile->intf_src.intf_type,msgBwProfile->intf_src.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," DstIntf= %u/%u",msgBwProfile->intf_dst.intf_type,msgBwProfile->intf_dst.intf_id);
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error with ptin_msg_bwProfileStruct_fill");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading profile!");
    return rc;
  }

  LOG_DEBUG(LOG_CTX_MSG,"Message processing finished!");
  return L7_SUCCESS;
}
...
L7_RC_t ptin_msg_bwProfile_set(msg_HwEthBwProfile_t *msgBwProfile, unsigned int msgId)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," SrcIntf= %u/%u",msgBwProfile->intf_src.intf_type,msgBwProfile->intf_src.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," DstIntf= %u/%u",msgBwProfile->intf_dst.intf_type,msgBwProfile->intf_dst.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," Meter {CIR,CBS}={%llu,%llu}",msgBwProfile->profile.cir,msgBwProfile->profile.cbs);
  LOG_DEBUG(LOG_CTX_MSG," Meter {EIR,EBS}={%llu,%llu}",msgBwProfile->profile.eir,msgBwProfile->profile.ebs);
  ...
  case CCMSG_ETH_BW_PROFILE_SET:
      ...
      {
        LOG_ERR(LOG_CTX_MSG,"Error applying profile!");
        return rc;
      }
      break;
...
L7_RC_t ptin_msg_bwProfile_delete(msg_HwEthBwProfile_t *msgBwProfile, unsigned int msgId)
{
  ...
  case CCMSG_ETH_BW_PROFILE_DELETE:
      ...
      {
        LOG_ERR(LOG_CTX_MSG,"Error removing profile!");
        return rc;
      }
      break;
...
L7_RC_t ptin_msg_stormControl2_get(msg_HwEthStormControl2_t *msgStormControl)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," slotId = %u", msgStormControl->SlotId);
  LOG_DEBUG(LOG_CTX_MSG," intf   = %u/%u",  msgStormControl->intf.intf_type, msgStormControl->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," mask   = 0x%02x", msgStormControl->mask);
  LOG_DEBUG(LOG_CTX_MSG," Broadcast = %ld (%u)", msgStormControl->broadcast.rate_value, msgStormControl->broadcast.rate_units);
  LOG_DEBUG(LOG_CTX_MSG," Multicast = %ld (%u)", msgStormControl->multicast.rate_value, msgStormControl->multicast.rate_units);
  LOG_DEBUG(LOG_CTX_MSG," UnknownUC = %ld (%u)", msgStormControl->unknown_uc.rate_value, msgStormControl->unknown_uc.rate_units);
  LOG_DEBUG(LOG_CTX_MSG," Block UC = %u", msgStormControl->block_unicast);
  LOG_DEBUG(LOG_CTX_MSG," Block MC = %u", msgStormControl->block_multicast);
...
L7_RC_t ptin_msg_stormControl2_set(msg_HwEthStormControl2_t *msgStormControl)
{
  ...
  {
    LOG_TRACE(LOG_CTX_MSG, "Processing Broadcast stormcontrol...");
    do
    {
      /* Data units */
      ...
      {
        LOG_ERR(LOG_CTX_MSG,"Unknown units (%u)", msgStormControl->broadcast.rate_units);
        rc_global = L7_FAILURE;
        break;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error configuring Broadcast stormcontrol for ptin_intf %u/%u", ptin_intf.intf_type, ptin_intf.intf_id);
        rc_global = rc;
        break;
      }
    } while (0);
  ...
  {
    LOG_TRACE(LOG_CTX_MSG, "Processing Multicast stormcontrol...");
    do
    {
      /* Data units */
      ...
      if (rate_units == L7_RATE_UNIT_NONE)
      {
        LOG_ERR(LOG_CTX_MSG,"Unknown units (%u)", msgStormControl->multicast.rate_units);
        rc_global = L7_FAILURE;
        break;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error configuring Multicast stormcontrol for ptin_intf %u/%u", ptin_intf.intf_type, ptin_intf.intf_id);
        rc_global = rc;
        break;
      }
    } while (0);
  ...
  {
    LOG_TRACE(LOG_CTX_MSG, "Processing Unknown Unicast stormcontrol...");
    do
    {
      /* Data units */
      ...
      if (rate_units == L7_RATE_UNIT_NONE)
      {
        LOG_ERR(LOG_CTX_MSG,"Unknown units (%u)", msgStormControl->unknown_uc.rate_units);
        rc_global = L7_FAILURE;
        break;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error configuring Unicast stormcontrol for ptin_intf %u/%u", ptin_intf.intf_type, ptin_intf.intf_id);
        rc_global = rc;
        break;
      }
    } while (0);
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_TRACE(LOG_CTX_MSG, "Success applying stormcontrol to ptin_intf %u/%u", ptin_intf.intf_type, ptin_intf.intf_id);
  }
  else
  {
    LOG_ERR(LOG_CTX_MSG, "Error applying stormcontrol to ptin_intf %u/%u", ptin_intf.intf_type, ptin_intf.intf_id);
  }
...
L7_RC_t ptin_msg_stormControl_get(msg_HwEthStormControl_t *msgStormControl)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," slotId = %u", msgStormControl->SlotId);
  LOG_DEBUG(LOG_CTX_MSG," id     = %u", msgStormControl->id);
  LOG_DEBUG(LOG_CTX_MSG," flags  = 0x%04x", msgStormControl->flags);
  LOG_DEBUG(LOG_CTX_MSG," mask   = 0x%04x", msgStormControl->mask);

  LOG_WARNING(LOG_CTX_MSG, "Obsolete Feature: not supported anymore!");
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error reading storm control data!");
    return rc;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Message processing finished! (rc=%d)", rc);
  return rc;
#endif
}
...
L7_RC_t ptin_msg_stormControl_set(msg_HwEthStormControl_t *msgStormControl)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," flags  = 0x%04x", msgStormControl->flags);
  LOG_DEBUG(LOG_CTX_MSG," mask   = 0x%04x", msgStormControl->mask);
  LOG_DEBUG(LOG_CTX_MSG," bcast_rate = %u bps", msgStormControl->bcast_rate);
  LOG_DEBUG(LOG_CTX_MSG," mcast_rate = %u bps", msgStormControl->mcast_rate);
  LOG_DEBUG(LOG_CTX_MSG," ucunk_rate = %u bps", msgStormControl->ucast_unknown_rate);
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error applying storm control profile!");
    return rc;
  }
...
L7_RC_t ptin_msg_stormControl_clear(msg_HwEthStormControl_t *msgStormControl)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error disabling storm control profile!");
    return rc;
  }
...
L7_RC_t ptin_msg_stormControl_reset(msg_HwEthStormControl_t *msgStormControl)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error clearing storm control profile!");
    return rc;
  }
...
L7_RC_t ptin_msg_evcStats_get(msg_evcStats_t *msg_evcStats)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," evcId    = %u",    msg_evcStats->evc_id);
  LOG_DEBUG(LOG_CTX_MSG," mask     = 0x%02x",msg_evcStats->mask);
  LOG_DEBUG(LOG_CTX_MSG," Intf     = %u/%u", msg_evcStats->intf.intf_type,msg_evcStats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," SVID     = %u",    msg_evcStats->service_vlan);
  LOG_DEBUG(LOG_CTX_MSG," CVID     = %u",    msg_evcStats->client_vlan);
  LOG_DEBUG(LOG_CTX_MSG," ChannelIP= %u",    msg_evcStats->channel_ip);
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error filling structure");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"No policy defined to read stats!");
  ...
  LOG_DEBUG(LOG_CTX_MSG," CVID     = %u",    msg_evcStats->client_vlan);
  LOG_DEBUG(LOG_CTX_MSG," ChannelIP= %u",    msg_evcStats->channel_ip);
  LOG_DEBUG(LOG_CTX_MSG," Stat_mask  = 0x%02x",msg_evcStats->stats.mask_stat);
  LOG_DEBUG(LOG_CTX_MSG," Stats_RX:");
  LOG_DEBUG(LOG_CTX_MSG,"   Total    =%10u", msg_evcStats->stats.rx.pktTotal);
  LOG_DEBUG(LOG_CTX_MSG,"   Unicast  =%10u", msg_evcStats->stats.rx.pktUnicast);
  LOG_DEBUG(LOG_CTX_MSG,"   Multicast=%10u", msg_evcStats->stats.rx.pktMulticast);
  LOG_DEBUG(LOG_CTX_MSG,"   Broadcast=%10u", msg_evcStats->stats.rx.pktBroadcast);
  LOG_DEBUG(LOG_CTX_MSG,"   Dropped  =%10u", msg_evcStats->stats.rx.pktDropped);
  LOG_DEBUG(LOG_CTX_MSG," Stats_TX:");
  LOG_DEBUG(LOG_CTX_MSG,"   Total    =%10u", msg_evcStats->stats.tx.pktTotal);
  LOG_DEBUG(LOG_CTX_MSG,"   Unicast  =%10u", msg_evcStats->stats.tx.pktUnicast);
  LOG_DEBUG(LOG_CTX_MSG,"   Multicast=%10u", msg_evcStats->stats.tx.pktMulticast);
  LOG_DEBUG(LOG_CTX_MSG,"   Broadcast=%10u", msg_evcStats->stats.tx.pktBroadcast);
  LOG_DEBUG(LOG_CTX_MSG,"   Dropped  =%10u", msg_evcStats->stats.tx.pktDropped);
...
L7_RC_t ptin_msg_evcStats_set(msg_evcStats_t *msg_evcStats)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG," SVID     = %u",    msg_evcStats->service_vlan);
  LOG_DEBUG(LOG_CTX_MSG," CVID     = %u",    msg_evcStats->client_vlan);
  LOG_DEBUG(LOG_CTX_MSG," ChannelIP= 0x%08x",    msg_evcStats->channel_ip);
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error allocating statistics!");
    return rc;
  }
...
L7_RC_t ptin_msg_evcStats_delete(msg_evcStats_t *msg_evcStats)
{
  ...
  if ( rc != L7_SUCCESS )
  {
    LOG_ERR(LOG_CTX_MSG,"Error deallocating statistics!");
    return rc;
  }
...
L7_RC_t ptin_msg_ntw_connectivity_get(msg_NtwConnectivity_t *msgNtwConn)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting inband management config");
    return L7_FAILURE;
  }

  /* Output data */
  LOG_INFO(LOG_CTX_MSG, "Network Connectivity (mask=0x%08X)", ptinNtwConn.mask);
  LOG_INFO(LOG_CTX_MSG, "  IP Addr         = %u.%u.%u.%u",    (ptinNtwConn.ipaddr  >> 24) & 0xFF, (ptinNtwConn.ipaddr  >> 16) & 0xFF,
           (ptinNtwConn.ipaddr  >>  8) & 0xFF,  ptinNtwConn.ipaddr         & 0xFF);
  LOG_INFO(LOG_CTX_MSG, "  Mask            = %u.%u.%u.%u",    (ptinNtwConn.netmask >> 24) & 0xFF, (ptinNtwConn.netmask >> 16) & 0xFF,
           (ptinNtwConn.netmask >>  8) & 0xFF,  ptinNtwConn.netmask        & 0xFF);
  LOG_INFO(LOG_CTX_MSG, "  Gateway         = %u.%u.%u.%u",    (ptinNtwConn.gateway >> 24) & 0xFF, (ptinNtwConn.gateway >> 16) & 0xFF,
           (ptinNtwConn.gateway >>  8) & 0xFF,  ptinNtwConn.gateway        & 0xFF);
  LOG_INFO(LOG_CTX_MSG, "  Mgmt VLAN ID    = %u",             ptinNtwConn.mgmtVlanId);
  LOG_INFO(LOG_CTX_MSG, "  Interfaces (%u)", ptinNtwConn.n_intf);
  for (i=0; i<ptinNtwConn.n_intf; i++)
    LOG_INFO(LOG_CTX_MSG, "    %s Intf #    = %u", ptinNtwConn.intf[i].intf_type == PTIN_EVC_INTF_PHYSICAL ? "PHY":"LAG", ptinNtwConn.intf[i].intf_id);
...
L7_RC_t ptin_msg_ntw_connectivity_set(msg_NtwConnectivity_t *msgNtwConn)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error setting inband management config");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_DHCP_evc_reconf(msg_DhcpEvcReconf_t *dhcpEvcInfo)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Processing message");

  /* Validate input parameters */
  if (dhcpEvcInfo==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid parameters");
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG, "  ID Type    = %u",      dhcpEvcInfo->idType);
  LOG_DEBUG(LOG_CTX_MSG, "  ID         = %u",      dhcpEvcInfo->id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask       = 0x%04X",  dhcpEvcInfo->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  DHCP Flag  = %u",      dhcpEvcInfo->dhcp_flag);
  LOG_DEBUG(LOG_CTX_MSG, "  Options    = 0x%04X",  dhcpEvcInfo->options);
      ...
      dhcpEvcInfo->idType == MSG_ID_EVC_TYPE)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error reconfiguring global DHCP EVC");
      return rc;
    }
  ...
  else if (dhcpEvcInfo->idType == MSG_ID_NNIVID_TYPE)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error reconfiguring global DHCP Instance");
      return rc;
    }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid id %u", dhcpEvcInfo->idType);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_DHCP_circuitid_set(msg_AccessNodeCircuitId_t *circuitid)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  Id_type            = %u",      circuitid->id_ref.id_type);
  LOG_DEBUG(LOG_CTX_MSG, "  Id value           = %u",      circuitid->id_ref.id_val.evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Template           = %s",      circuitid->template_str);
  LOG_DEBUG(LOG_CTX_MSG, "  CircuitId Mask     = 0x%04X",  circuitid->mask_circuitid);
  LOG_DEBUG(LOG_CTX_MSG, "  AccessNode ID      = %s",      circuitid->access_node_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Chassis            = %u",      circuitid->chassis);
  LOG_DEBUG(LOG_CTX_MSG, "  Rack               = %u",      circuitid->rack);
  LOG_DEBUG(LOG_CTX_MSG, "  Frame              = %u",      circuitid->frame);
  LOG_DEBUG(LOG_CTX_MSG, "  Ethernet Priority  = %u",      circuitid->ethernet_priority);
  LOG_DEBUG(LOG_CTX_MSG, "  S-VID              = %u",      circuitid->s_vid);
  LOG_DEBUG(LOG_CTX_MSG, "  Flags Mask         = 0x%02X",  circuitid->mask_flags);
  LOG_DEBUG(LOG_CTX_MSG, "  Flags              = 0x%02X",  circuitid->broadcast_flag);
      ...
      circuitid->id_ref.id_type == MSG_ID_EVC_TYPE)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error configuring DHCP circuit-id");
      return rc;
    }
    /* Flags */
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error configuring DHCP flags");
      return rc;
    }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid id %u", circuitid->id_ref.id_type);
    return L7_FAILURE;
  }
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error configuring circuit-id global data");
    return rc;
  }
#endif
...
L7_RC_t ptin_msg_DHCP_circuitid_get(msg_AccessNodeCircuitId_t *circuitid)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Not supported yet");
    return L7_FAILURE;
  }
  else
  {
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  ID type            = %u",      circuitid->id_ref.id_type);
  LOG_DEBUG(LOG_CTX_MSG, "  ID value           = %u",      circuitid->id_ref.id_val);
  LOG_DEBUG(LOG_CTX_MSG, "  Template           = %s",      circuitid->template_str);
  LOG_DEBUG(LOG_CTX_MSG, "  CircuitId Mask     = 0x%04X",  circuitid->mask_circuitid);
  LOG_DEBUG(LOG_CTX_MSG, "  AccessNode ID      = %s",      circuitid->access_node_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Chassis            = %u",      circuitid->chassis);
...
L7_RC_t ptin_msg_DHCP_profile_get(msg_HwEthernetDhcpOpt82Profile_t *profile)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "  Slot Id      = %u",     profile->SlotId);
  LOG_DEBUG(LOG_CTX_MSG, "  evc_idx      = %u",     profile->evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", profile->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u",  profile->intf.intf_type,profile->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", profile->client.mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.OVlan = %u",     profile->client.outer_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.IVlan = %u",     profile->client.inner_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Intf  = %u/%u",  profile->client.intf.intf_type, profile->client.intf.intf_id);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error obtaining circuit and remote ids");
    return rc;
  }

  LOG_DEBUG(LOG_CTX_MSG,"Options                      = %02x",  profile->options);
  LOG_DEBUG(LOG_CTX_MSG,"CircuitId.onuid              = %u",    profile->circuitId.onuid);
  LOG_DEBUG(LOG_CTX_MSG,"CircuitId.slot               = %u",    profile->circuitId.slot);
  LOG_DEBUG(LOG_CTX_MSG,"CircuitId.port               = %u",    profile->circuitId.port);
  LOG_DEBUG(LOG_CTX_MSG,"CircuitId.q_vid              = %u",    profile->circuitId.q_vid);
  LOG_DEBUG(LOG_CTX_MSG,"CircuitId.c_vid              = %u",    profile->circuitId.c_vid);
  LOG_DEBUG(LOG_CTX_MSG,"RemoteId                     = \"%s\"",profile->remoteId);
...
L7_RC_t ptin_msg_DHCP_profile_add(msg_HwEthernetDhcpOpt82Profile_t *profile, L7_uint n_clients)
{
  ...
  for (i=0; i<n_clients; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Processing message %u",i);

    LOG_DEBUG(LOG_CTX_MSG, "  Slot Id                      = %u",     profile[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG, "  evc_idx                      = %u",     profile[i].evc_id);
    LOG_DEBUG(LOG_CTX_MSG, "  Mask                         = 0x%02x", profile[i].mask);
    LOG_DEBUG(LOG_CTX_MSG, "  Interface                    = %u/%u",  profile[i].intf.intf_type,profile[i].intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask                  = 0x%02x", profile[i].client.mask);
    LOG_DEBUG(LOG_CTX_MSG, "  Client.OVlan                 = %u",     profile[i].client.outer_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "  Client.IVlan                 = %u",     profile[i].client.inner_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "  Client.Intf                  = %u/%u",  profile[i].client.intf.intf_type, profile[i].client.intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG, "  Options                      = %04x",   profile[i].options);
    LOG_DEBUG(LOG_CTX_MSG, "  CircuitId.onuid              = %u",     profile[i].circuitId.onuid);
    LOG_DEBUG(LOG_CTX_MSG, "  CircuitId.slot               = %u",     profile[i].circuitId.slot);
    LOG_DEBUG(LOG_CTX_MSG, "  CircuitId.port               = %u",     profile[i].circuitId.port);
    LOG_DEBUG(LOG_CTX_MSG, "  CircuitId.q_vid              = %u",     profile[i].circuitId.q_vid);
    LOG_DEBUG(LOG_CTX_MSG, "  Remote Id                    = \"%s\"", profile[i].remoteId);

    /* Check if all UseGlobal_DHCP_options match */
    if ( ((profile[i].options & 0x02) >> 1) != ((profile[i].options & 0x08) >> 3) )
    {
      LOG_ERR(LOG_CTX_MSG, "Error: UseGlobal_DHCP_options do not match");
      return L7_FAILURE;
    }
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error adding DHCP circuitId+remoteId entry");
      return rc;
    }
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error adding PPPoE circuitId+remoteId entry");
      return rc;
    }
#endif
  }
...
L7_RC_t ptin_msg_DHCP_profile_remove(msg_HwEthernetDhcpOpt82Profile_t *profile, L7_uint n_clients)
{
  ...
  for (i=0; i<n_clients; i++)
  {
    ...
    if ( rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error removing DHCP circuitId+remoteId entry");
      return rc;
    }
    ...
    if ( rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error removing PPPoE circuitId+remoteId entry");
      return rc;
    }
#endif
  }
...
L7_RC_t ptin_msg_DHCP_clientStats_get(msg_DhcpClientStatistics_t *dhcp_stats)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid arguments");
    return L7_FAILURE;
  }

  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Reading client DHCP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  evc_idx      = %u",     dhcp_stats->evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", dhcp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u",  dhcp_stats->intf.intf_type,dhcp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", dhcp_stats->client.mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.OVlan = %u",     dhcp_stats->client.outer_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.IVlan = %u",     dhcp_stats->client.inner_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Intf  = %u/%u",  dhcp_stats->client.intf.intf_type, dhcp_stats->client.intf.intf_id);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "MC EVC and client reference must be provided");
    return L7_FAILURE;
  }
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting client statistics");
    return rc;
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Success getting client statistics");
  }
...
L7_RC_t ptin_msg_DHCP_clientStats_clear(msg_DhcpClientStatistics_t *dhcp_stats)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Clearing client DHCP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  evc_idx      = %u",     dhcp_stats->evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", dhcp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u",  dhcp_stats->intf.intf_type,dhcp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", dhcp_stats->client.mask);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error clearing client statistics");
    return rc;
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Success clearing client statistics");
  }
...
L7_RC_t ptin_msg_DHCP_intfStats_get(msg_DhcpClientStatistics_t *dhcp_stats)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Reading interface DHCP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  evc_idx      = %u",     dhcp_stats->evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", dhcp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u",  dhcp_stats->intf.intf_type,dhcp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", dhcp_stats->client.mask);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "At least, interface must be provided");
    return L7_FAILURE;
  }
  ...
  if (dhcp_stats->evc_id==(L7_uint16)-1)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error getting global interface statistics");
      return rc;
    }
    else
    {
      LOG_DEBUG(LOG_CTX_MSG, "Success getting global interface statistics");
    }
  }
  else
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error getting statistics struct");
      return rc;
    }
    else
    {
      LOG_DEBUG(LOG_CTX_MSG, "Success getting interface statistics of one DHCP instance");
    }
  }
...
L7_RC_t ptin_msg_DHCP_intfStats_clear(msg_DhcpClientStatistics_t *dhcp_stats)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Clearing interface DHCP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  evc_idx      = %u",     dhcp_stats->evc_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", dhcp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u",  dhcp_stats->intf.intf_type,dhcp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", dhcp_stats->client.mask);
  ...
  if (dhcp_stats->evc_id==(L7_uint16)-1)
  {
    ...
    if ( !(dhcp_stats->mask & MSG_INTERFACE_MASK) )
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing all statistics data");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing all statistics data");
      }
    }
    /* Interface provided */
    else
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of one complete interface");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing statistics of one complete interface");
      }
    }
  }
  /* MC EVC provided */
  else
  {
    ...
    if ( !(dhcp_stats->mask & MSG_INTERFACE_MASK) )
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of one DHCP instance");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing statistics of one DHCP instance");
      }
    }
    /* Interface provided */
    else
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of one DHCP instance + interface");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing statistics of one DHCP instance + interface");
      }
    }
  }
...
L7_RC_t ptin_msg_DHCPv4v6_bindTable_get(msg_DHCP_bind_table_request_t *input, msg_DHCPv4v6_bind_table_t *output)
{
  ...
  /* Debug */
  LOG_DEBUG(LOG_CTX_MSG, "Binding table get:");
  LOG_DEBUG(LOG_CTX_MSG, "  SlotId = %u",   input->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "  Page   = %u",   input->page);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask   = %02X", input->mask);
  ...
  if (page==0)
  {
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error reading binding table");
      return rc;
    }
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Requested page exceeds binding table size (table size=%u, page=%u)",dhcp_bindtable_entries,page);
    return L7_FAILURE;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "There at least %u entries left", entries);
...
//  memset(&output->bind_table[i],0x00,sizeof(msg_DHCP_bind_entry));
    ...
    LOG_TRACE(LOG_CTX_MSG, "Entry %u:", first+i);
    LOG_TRACE(LOG_CTX_MSG, "  entry_index = %u",    output->bind_table[i].entry_index);
    LOG_TRACE(LOG_CTX_MSG, "  evc_idx     = %u",    output->bind_table[i].evc_idx);
    LOG_TRACE(LOG_CTX_MSG, "  intf        = %u/%u", output->bind_table[i].intf.intf_type, output->bind_table[i].intf.intf_id);
    LOG_TRACE(LOG_CTX_MSG, "  outer_vlan  = %u",    output->bind_table[i].outer_vlan);
    LOG_TRACE(LOG_CTX_MSG, "  inner_vlan  = %u",    output->bind_table[i].inner_vlan);
    LOG_TRACE(LOG_CTX_MSG, "  macAddr     = %02X:%02X:%02X:%02X:%02X:%02X", output->bind_table[i].macAddr[0], output->bind_table[i].macAddr[1], 
              output->bind_table[i].macAddr[2], output->bind_table[i].macAddr[3], output->bind_table[i].macAddr[4], output->bind_table[i].macAddr[5]);
    LOG_TRACE(LOG_CTX_MSG, "  ipAddr      = %08X",  output->bind_table[i].ipAddr);
    LOG_TRACE(LOG_CTX_MSG, "  remLeave    = %u",    output->bind_table[i].remLeave);
    LOG_TRACE(LOG_CTX_MSG, "  bindingType = %u",    output->bind_table[i].bindingType);
  }
...
L7_RC_t ptin_msg_DHCP_bindTable_remove(msg_DHCP_bind_table_entry_t *table, L7_uint16 numEntries)
{
  ...
  for (i=0; i<numEntries ; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Evc_idx=%u",    table[i].bind_entry.evc_idx);
    LOG_DEBUG(LOG_CTX_MSG,"Port   = %u/%u",table[i].bind_entry.intf.intf_type, table[i].bind_entry.intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG,"OVlan  = %u",   table[i].bind_entry.outer_vlan);
    LOG_DEBUG(LOG_CTX_MSG,"IVlan  = %u",   table[i].bind_entry.inner_vlan);
    LOG_DEBUG(LOG_CTX_MSG,"MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",
    ...
    LOG_DEBUG(LOG_CTX_MSG,"family = %u",table[i].bind_entry.ipAddr.family);
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error removing entry");
      return rc;
    }
    LOG_DEBUG(LOG_CTX_MSG,"Success removing entry");
  }
...
L7_RC_t ptin_msg_ipsg_verify_source_set(msg_IPSG_set_t* msgIpsgVerifySource)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "slotId         = %u"   , msgIpsgVerifySource->slotId);  
  LOG_DEBUG(LOG_CTX_MSG, "ptinPort       = %u/%u",msgIpsgVerifySource->intf.intf_type,msgIpsgVerifySource->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "verifySource   = %s"   ,msgIpsgVerifySource->enable==L7_FALSE?"No":"Yes");
  ...
  if (ptin_intf_ptintf2intIfNum(&ptin_intf, &intIfNum)!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error converting port %u/%u to intIfNum",ptin_intf.intf_type, ptin_intf.intf_id);
    return L7_FAILURE;
  }
  LOG_TRACE(LOG_CTX_MSG, "Port# %u/%u: intIfNum# %2u", ptin_intf.intf_type, ptin_intf.intf_id, intIfNum);
  ...
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid value for enable:%u", msgIpsgVerifySource->enable);
      return L7_FAILURE;
    }
  }  
#else
  LOG_ERR(LOG_CTX_MSG, "IP Source Guard not Supported!");
  return L7_FAILURE;
#endif
}
...
{  
  ...
  for (i = 0; i < n_msg; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG, "slotId        = %u"   , msgIpsgStaticEntry[i].slotId);  
    LOG_DEBUG(LOG_CTX_MSG, "iDType        = %u"   , msgIpsgStaticEntry[i].idType);  
    LOG_DEBUG(LOG_CTX_MSG, "iD            = %u"   , msgIpsgStaticEntry[i].id);  
    LOG_DEBUG(LOG_CTX_MSG, "ptinP         = %u/%u", msgIpsgStaticEntry[i].intf.intf_type,msgIpsgStaticEntry[i].intf.intf_id);  
    LOG_DEBUG(LOG_CTX_MSG, "action        = %s"   , msgIpsgStaticEntry[i].action==L7_FALSE?"Remove":"Add");  
    LOG_DEBUG(LOG_CTX_MSG, "MAC Addr      = %02X:%02X:%02X:%02X:%02X:%02X",msgIpsgStaticEntry[i].macAddr[0],msgIpsgStaticEntry[i].macAddr[1],
    ...
    LOG_DEBUG(LOG_CTX_MSG, "IP Address    = %s",ipAddrStr);
    ...
    if( (msgIpsgStaticEntry[i].idType & IPSG_ID_ALL) == IPSG_EVC_ID)
    {
      ...
      {
        rc_global = L7_NOT_EXIST;
        LOG_ERR(LOG_CTX_MSG,"Error getting internal root vlan for eEVCId=%u", msgIpsgStaticEntry[i].id);
        continue;
      }
      LOG_TRACE(LOG_CTX_MSG, "EVCidx# %u: internalRootVlan# %u",msgIpsgStaticEntry[i].id,vlanId);
    }
    else
    {
      if ( (msgIpsgStaticEntry[i].idType & IPSG_ID_ALL) == IPSG_ROOT_VLAN )
      {
        ...
        {
          rc_global = L7_FAILURE;
          LOG_ERR(LOG_CTX_MSG,"Invalid root vlan given:%u", msgIpsgStaticEntry[i].id);
          continue;
        }
  /*Disabled this verification to support MAC Bridge Services*/
        ...
        {
          rc_global = L7_NOT_EXIST;
          LOG_ERR(LOG_CTX_IGMP,"Invalid root VLAN:%u", vlanId);
          continue;
        }
        LOG_TRACE(LOG_CTX_MSG, "EVCidx# %u: internalRootVlan# %u",eEVCId, vlanId);
  #endif
      }
      else
      {
        rc_global = L7_FAILURE;
        LOG_ERR(LOG_CTX_MSG,"Invalid IdType:%u", msgIpsgStaticEntry[i].idType);
        continue;
      }
    }
...
{  
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid Input Parameters: igmpAdmissionControl=%p", msgAdmissionControl);
    return L7_FAILURE;
  }
   /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "SlotId       = %u"        , msgAdmissionControl->SlotId);
  LOG_DEBUG(LOG_CTX_MSG, "mask         = 0x%02X"    , msgAdmissionControl->mask);
  LOG_DEBUG(LOG_CTX_MSG, "evcId        = %u"        , msgAdmissionControl->evcId);
  LOG_DEBUG(LOG_CTX_MSG, "intf         = %u/%u"     , msgAdmissionControl->intf.intf_type, msgAdmissionControl->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "onuId        = %u"        , msgAdmissionControl->onuId);
  LOG_DEBUG(LOG_CTX_MSG, "outer_vlan   = %u"        , msgAdmissionControl->outer_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "inner_vlan   = %u"        , msgAdmissionControl->inner_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "maxChannels  = %hu"       , msgAdmissionControl->maxChannels);
  LOG_DEBUG(LOG_CTX_MSG, "maxBandwidth = %llu bit/s", msgAdmissionControl->maxBandwidth);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid Admission Control Parameters [mask:0x%02x maxChannels:%hu maxBandwidth:%llu bits/s", msgAdmissionControl->mask, msgAdmissionControl->maxChannels, msgAdmissionControl->maxBandwidth);
    return L7_FAILURE;
  }
  ...
  if (igmpAdmissionControl.mask != 0x00)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Failed to obtain ptin_port from ptin_intf [ptin_intf.intf_type:%u ptin_intf:%u]",intf.intf_type, intf.intf_id);
      return L7_FAILURE;
    }
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Failed to add multicast service");
      return L7_FAILURE;
    }

    if (ptin_igmp_admission_control_multicast_service_set(&igmpAdmissionControl) != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Failed to set multicast admission control parameters");
      return L7_FAILURE;
    }     
  }
  else
  {
    LOG_NOTICE(LOG_CTX_MSG,"Ignoring Request - Admission Control Mask is 0x00!");    
  }
#endif 
  return L7_SUCCESS;  
...
L7_RC_t ptin_msg_igmp_proxy_set(msg_IgmpProxyCfg_t *msgIgmpProxy)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "IGMP Proxy (mask=0x%08X)", ptinMgmdProxy.mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Admin #                          = %u", ptinMgmdProxy.admin);
  LOG_DEBUG(LOG_CTX_MSG, "  Network Version                  = %u", ptinMgmdProxy.networkVersion);
  LOG_DEBUG(LOG_CTX_MSG, "  Client Version                   = %u", ptinMgmdProxy.clientVersion);
  LOG_DEBUG(LOG_CTX_MSG, "  IP Addr                          = %u.%u.%u.%u", (ptinMgmdProxy.ipv4Addr>>24)&0xFF, (ptinMgmdProxy.ipv4Addr>>16)&0xFF, 
                                                                                  (ptinMgmdProxy.ipv4Addr>>8)&0xFF, ptinMgmdProxy.ipv4Addr&0xFF);
  LOG_DEBUG(LOG_CTX_MSG, "  COS                              = %u", ptinMgmdProxy.igmpCos);
  LOG_DEBUG(LOG_CTX_MSG, "  FastLeave                        = %s", ptinMgmdProxy.fastLeave != 0 ? "ON":"OFF");  
  LOG_DEBUG(LOG_CTX_MSG, "  Querier (mask=0x%08X)", ptinMgmdProxy.querier.mask);
  LOG_DEBUG(LOG_CTX_MSG, "    Flags                          = 0x%04X", ptinMgmdProxy.querier.flags);
  LOG_DEBUG(LOG_CTX_MSG, "    Robustness                     = %u", ptinMgmdProxy.querier.robustness);
  LOG_DEBUG(LOG_CTX_MSG, "    Query Interval                 = %u", ptinMgmdProxy.querier.queryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Query Response Interval        = %u", ptinMgmdProxy.querier.queryResponseInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Group Membership Interval      = %u", ptinMgmdProxy.querier.groupMembershipInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Other Querier Present Interval = %u", ptinMgmdProxy.querier.otherQuerierPresentInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Startup Query Interval         = %u", ptinMgmdProxy.querier.startupQueryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Startup Query Count            = %u", ptinMgmdProxy.querier.startupQueryCount);
  LOG_DEBUG(LOG_CTX_MSG, "    Last Member Query Interval     = %u", ptinMgmdProxy.querier.lastMemberQueryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Last Member Query Count        = %u", ptinMgmdProxy.querier.lastMemberQueryCount);
  LOG_DEBUG(LOG_CTX_MSG, "    Older Host Present Timeout     = %u", ptinMgmdProxy.querier.olderHostPresentTimeout);
  LOG_DEBUG(LOG_CTX_MSG, "  Host (mask=0x%08X)", ptinMgmdProxy.host.mask);
  LOG_DEBUG(LOG_CTX_MSG, "    Flags                          = 0x%02X", ptinMgmdProxy.host.flags);
  LOG_DEBUG(LOG_CTX_MSG, "    Robustness                     = %u", ptinMgmdProxy.host.robustness);
  LOG_DEBUG(LOG_CTX_MSG, "    Unsolicited Report Interval    = %u", ptinMgmdProxy.host.unsolicitedReportInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Older Querier Present Timeout  = %u", ptinMgmdProxy.host.olderQuerierPresentTimeout);
  LOG_DEBUG(LOG_CTX_MSG, "    Max Group Records per Packet   = %u", ptinMgmdProxy.host.maxRecordsPerReport);
  LOG_DEBUG(LOG_CTX_MSG, "  Bandwidth Control                = %s", ptinMgmdProxy.bandwidthControl != 0 ? "ON":"OFF");
  LOG_DEBUG(LOG_CTX_MSG, "  Channels Control                 = %s", ptinMgmdProxy.channelsControl != 0 ? "ON":"OFF");
  LOG_DEBUG(LOG_CTX_MSG, "  WhiteList                        = %s", ptinMgmdProxy.whiteList != 0 ? "ON":"OFF");
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error setting IGMP Proxy config");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_igmp_proxy_get(msg_IgmpProxyCfg_t *msgIgmpProxy)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error getting IGMP Proxy config");
    return L7_FAILURE;
  }

  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "IGMP Proxy (mask=0x%08X)", ptinIgmpProxy.mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Admin #                          = %u", ptinIgmpProxy.admin);
  LOG_DEBUG(LOG_CTX_MSG, "  Network Version                  = %u", ptinIgmpProxy.networkVersion);
  LOG_DEBUG(LOG_CTX_MSG, "  Client Version                   = %u", ptinIgmpProxy.clientVersion);  
  LOG_DEBUG(LOG_CTX_MSG, "  IP Addr                          = %u.%u.%u.%u", (ptinIgmpProxy.ipv4Addr >> 24) & 0xFF, (ptinIgmpProxy.ipv4Addr >> 16) & 0xFF,
                                                                                  (ptinIgmpProxy.ipv4Addr >>  8) & 0xFF,  ptinIgmpProxy.ipv4Addr        & 0xFF);
  LOG_DEBUG(LOG_CTX_MSG, "  COS                              = %u", ptinIgmpProxy.igmpCos);
  LOG_DEBUG(LOG_CTX_MSG, "  FastLeave                        = %s", ptinIgmpProxy.fastLeave != 0 ? "ON":"OFF");
  LOG_DEBUG(LOG_CTX_MSG, "  Querier (mask=0x%08X)", ptinIgmpProxy.querier.mask);
  LOG_DEBUG(LOG_CTX_MSG, "    Flags                          = 0x%08X", ptinIgmpProxy.querier.flags);  
  LOG_DEBUG(LOG_CTX_MSG, "    Robustness                     = %u", ptinIgmpProxy.querier.robustness);
  LOG_DEBUG(LOG_CTX_MSG, "    Query Interval                 = %u", ptinIgmpProxy.querier.queryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Query Response Interval        = %u", ptinIgmpProxy.querier.queryResponseInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Group Membership Interval      = %u", ptinIgmpProxy.querier.groupMembershipInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Other Querier Present Interval = %u", ptinIgmpProxy.querier.otherQuerierPresentInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Startup Query Interval         = %u", ptinIgmpProxy.querier.startupQueryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Startup Query Count            = %u", ptinIgmpProxy.querier.startupQueryCount);
  LOG_DEBUG(LOG_CTX_MSG, "    Last Member Query Interval     = %u", ptinIgmpProxy.querier.lastMemberQueryInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Last Member Query Count        = %u", ptinIgmpProxy.querier.lastMemberQueryCount);
  LOG_DEBUG(LOG_CTX_MSG, "    Older Host Present Timeout     = %u", ptinIgmpProxy.querier.olderHostPresentTimeout);
  LOG_DEBUG(LOG_CTX_MSG, "  Host (mask=0x%08X)", ptinIgmpProxy.host.mask);
  LOG_DEBUG(LOG_CTX_MSG, "    Flags                          = 0x%02X", ptinIgmpProxy.host.flags);  
  LOG_DEBUG(LOG_CTX_MSG, "    Robustness                     = %u", ptinIgmpProxy.host.robustness);
  LOG_DEBUG(LOG_CTX_MSG, "    Unsolicited Report Interval    = %u", ptinIgmpProxy.host.unsolicitedReportInterval);
  LOG_DEBUG(LOG_CTX_MSG, "    Older Querier Present  Timeout = %u", ptinIgmpProxy.host.olderQuerierPresentTimeout);
  LOG_DEBUG(LOG_CTX_MSG, "  Bandwidth Control                = %s", ptinIgmpProxy.bandwidthControl != 0 ? "ON":"OFF");
  LOG_DEBUG(LOG_CTX_MSG, "  Channels Control                 = %s", ptinIgmpProxy.channelsControl != 0 ? "ON":"OFF");
  LOG_DEBUG(LOG_CTX_MSG, "  WhiteList                        = %s", ptinIgmpProxy.whiteList != 0 ? "ON":"OFF");
...
L7_RC_t ptin_msg_igmp_instance_add(msg_IgmpMultcastUnicastLink_t *msgIgmpInst)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Going to add IGMP Instance:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx = %u", msgIgmpInst->multicastEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  UC evc_idx = %u", msgIgmpInst->unicastEvcId);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error creating/updating IGMP instance");
    return rc;
  }
...
L7_RC_t ptin_msg_igmp_instance_remove(msg_IgmpMultcastUnicastLink_t *msgIgmpInst)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Going to remove IGMP Instance:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx = %u", msgIgmpInst->multicastEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  UC evc_idx = %u", msgIgmpInst->unicastEvcId);
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error removing IGMP instance");
    return rc;
  }
...
L7_RC_t ptin_msg_igmp_client_add(msg_IgmpClient_t *McastClient, L7_uint16 n_clients)
{
  ...
  {
    /* Output data */
    LOG_DEBUG(LOG_CTX_MSG, "Going to add MC client");
    LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx = %u", McastClient[i].mcEvcId);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.Mask         = 0x%02x", McastClient[i].client.mask);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.OVlan        = %u", McastClient[i].client.outer_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.IVlan        = %u", McastClient[i].client.inner_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.Intf         = %u/%u", McastClient[i].client.intf.intf_type,McastClient[i].client.intf.intf_id);

    if (McastClient[i].mask > PTIN_MSG_IGMP_CLIENT_MASK_VALID)
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid Mask [mask:0x%02x]",McastClient[i].mask, McastClient[i].maxBandwidth, McastClient[i].maxChannels);
      return L7_FAILURE;
    }
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "Invalid Admission Control Parameters [mask:0x%02x maxBandwidth:%llu bits/s maxChannels:%hu",McastClient[i].mask, McastClient[i].maxBandwidth, McastClient[i].maxChannels);
        return L7_FAILURE;
      }

      LOG_DEBUG(LOG_CTX_MSG, "   onuId        = %u", McastClient[i].onuId);
      LOG_DEBUG(LOG_CTX_MSG, "   mask         = %u", McastClient[i].mask);
      LOG_DEBUG(LOG_CTX_MSG, "   maxChannels  = %u", McastClient[i].maxChannels);
      LOG_DEBUG(LOG_CTX_MSG, "   maxBandwidth = %llu bit/s ", McastClient[i].maxBandwidth);
#endif
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error adding MC client");
      return rc;
    }
  }
...
L7_RC_t ptin_msg_igmp_client_delete(msg_IgmpClient_t *McastClient, L7_uint16 n_clients)
{
  ...
  {
    /* Output data */
    LOG_DEBUG(LOG_CTX_MSG, "Going to remove MC client");
    LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx = %u", McastClient[i].mcEvcId);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.Mask  = 0x%02x", McastClient[i].client.mask);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.OVlan = %u", McastClient[i].client.outer_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.IVlan = %u", McastClient[i].client.inner_vlan);
    ...
    if ( rc != L7_SUCCESS )
    {
      LOG_ERR(LOG_CTX_MSG, "Error removing MC client");
      return rc;
    }
  }
...
L7_RC_t ptin_msg_IGMP_clientStats_get(msg_IgmpClientStatistics_t *igmp_stats)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Reading client IGMP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx   = %u", igmp_stats->mcEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", igmp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u", igmp_stats->intf.intf_type,igmp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", igmp_stats->client.mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.OVlan = %u", igmp_stats->client.outer_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.IVlan = %u", igmp_stats->client.inner_vlan);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Intf  = %u/%u", igmp_stats->client.intf.intf_type, igmp_stats->client.intf.intf_id);
  ...
  #if PTIN_BOARD_IS_LINECARD
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG,"Converted [client.Mask:%u Client.OVlan:%u]",igmp_stats->client.mask,igmp_stats->client.outer_vlan);
  }  
  #endif
...
L7_RC_t ptin_msg_IGMP_clientStats_clear(msg_IgmpClientStatistics_t *igmp_stats, uint16 n_clients)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Clearing client IGMP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx   = %u", igmp_stats->mcEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", igmp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u", igmp_stats->intf.intf_type,igmp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", igmp_stats->client.mask);
...
L7_RC_t ptin_msg_IGMP_intfStats_get(msg_IgmpClientStatistics_t *igmp_stats)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Reading interface IGMP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx   = %u", igmp_stats->mcEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", igmp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u", igmp_stats->intf.intf_type,igmp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", igmp_stats->client.mask);
  ...
  else
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_MSG, "Success getting interface statistics of one IGMP instance");
    }
  }
...
L7_RC_t ptin_msg_IGMP_intfStats_clear(msg_IgmpClientStatistics_t *igmp_stats, uint16 n_clients)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Clearing interface IGMP stats:");
  LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx   = %u", igmp_stats->mcEvcId);
  LOG_DEBUG(LOG_CTX_MSG, "  Mask         = 0x%02x", igmp_stats->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Interface    = %u/%u", igmp_stats->intf.intf_type,igmp_stats->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  Client.Mask  = 0x%02x", igmp_stats->client.mask);
  ...
  else
  {
    ...
    if ( !(igmp_stats->mask & MSG_INTERFACE_MASK) )
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of one IGMP instance");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing statistics of one IGMP instance");
      }
    }
    /* Interface provided */
    else
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error clearing statistics of one IGMP instance + interface");
        return rc;
      }
      else
      {
        LOG_DEBUG(LOG_CTX_MSG, "Success clearing statistics of one IGMP instance + interface");
      }
    }
  }
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_get(msg_MCAssocChannel_t *channel_list, L7_uint16 *n_channels)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Reading White list channel list:");
  LOG_DEBUG(LOG_CTX_MSG," Slot   = %d",channel_list->SlotId);
  LOG_DEBUG(LOG_CTX_MSG," EVC_MC = %d",channel_list->evcid_mc);
  LOG_DEBUG(LOG_CTX_MSG," Entry_idx = %d",channel_list->entry_idx);
  LOG_DEBUG(LOG_CTX_MSG," DstIP_Channel = 0x%08x (ipv6=%u) / %u",channel_list->channel_dstIp.addr.ipv4, channel_list->channel_dstIp.family, channel_list->channel_dstmask);
  LOG_DEBUG(LOG_CTX_MSG," SrcIP_Channel = 0x%08x (ipv6=%u) / %u",channel_list->channel_srcIp.addr.ipv4, channel_list->channel_srcIp.family, channel_list->channel_srcmask);
  ...
  if (channel_list->entry_idx==0)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error reading list of channels");
      return L7_FAILURE;
    }
  ...
  *n_channels = number_of_channels;
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Not supported!");
  return L7_NOT_SUPPORTED;
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_add(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels, L7_uint8 isStatic)
{
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG,"Adding channel index %u:",i);
    LOG_DEBUG(LOG_CTX_MSG," Slot   = %d",channel_list[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," EVC_MC = %d",channel_list[i].evcid_mc);
    LOG_DEBUG(LOG_CTX_MSG," Entry_idx = %d",channel_list[i].entry_idx);
    LOG_DEBUG(LOG_CTX_MSG," group_Channel = 0x%08x (ipv6=%u) / %u",channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstIp.family, channel_list[i].channel_dstmask);
    LOG_DEBUG(LOG_CTX_MSG," source_Channel = 0x%08x (ipv6=%u) / %u",channel_list[i].channel_srcIp.addr.ipv4, channel_list[i].channel_srcIp.family, channel_list[i].channel_srcmask);

    #if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
    LOG_DEBUG(LOG_CTX_MSG," channelBandwidth = %llu bits/s", channel_list[i].channelBandwidth);

    if (channel_list[i].channelBandwidth > PTIN_IGMP_ADMISSION_CONTROL_MAX_BANDWIDTH_IN_BPS)
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid channelBandwidth:%llu bits/s",channel_list[i].channelBandwidth);
      return L7_FAILURE;
    }
    #endif
    ...
    {
      LOG_ERR(LOG_CTX_MSG,"Group channel = 0x%08x (ipv6=%u) / %u is not a multicast address",channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstIp.family, channel_list[i].channel_dstmask);
      return L7_FAILURE;
    }
                                    ...
                                    &sourceAddr, channel_list[i].channel_srcmask, isStatic, channel_list[i].channelBandwidth )) != L7_SUCCESS)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG, "Error adding group address 0x%08x/%u, source address 0x%08x/%u to MC EVC %u",
  ...
  {    
    if (rc != L7_SUCCESS)
      rc_global = rc;
    LOG_WARNING(LOG_CTX_MSG, "One or more channels were already added! rc:%u", rc_global);
  }  
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_remove(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels, L7_uint8 isStatic)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Removing channel index %u:",i);
    LOG_DEBUG(LOG_CTX_MSG," Slot   = %d",channel_list[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," EVC_MC = %d",channel_list[i].evcid_mc);
    LOG_DEBUG(LOG_CTX_MSG," Entry_idx = %d",channel_list[i].entry_idx);
    LOG_DEBUG(LOG_CTX_MSG," isStatic = %s", isStatic?"Yes":"No");
    LOG_DEBUG(LOG_CTX_MSG," DstIP_Channel = 0x%08x (ipv6=%u) / %u",channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstIp.family, channel_list[i].channel_dstmask);
    LOG_DEBUG(LOG_CTX_MSG," SrcIP_Channel = 0x%08x (ipv6=%u) / %u",channel_list[i].channel_srcIp.addr.ipv4, channel_list[i].channel_srcIp.family, channel_list[i].channel_srcmask);
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error (%d) removing group address 0x%08x/%u, source address 0x%08x/%u to MC EVC %u",
              rc,
              channel_list[i].channel_dstIp.addr.ipv4, channel_list[i].channel_dstmask,
              channel_list[i].channel_srcIp.addr.ipv4, channel_list[i].channel_srcmask,
              channel_list[i].evcid_mc);
...
L7_RC_t ptin_msg_IGMP_ChannelAssoc_remove_all(msg_MCAssocChannel_t *channel_list, L7_uint16 n_channels)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error (%d) removing groups to MC EVC %u", rc, channel_list[i].evcid_mc);
      rc_global = rc;
      continue;
    }
...
L7_RC_t ptin_msg_IGMP_staticChannel_add(msg_MCStaticChannel_t *channel, L7_uint16 n_channels)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error (%d) adding static channel", rc);
      return rc;
    }
    #endif//End Static Channel Add

    LOG_DEBUG(LOG_CTX_MSG,"Static channel addition index %u:",i);
    LOG_DEBUG(LOG_CTX_MSG," SlotId           = %u",channel[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," EvcId            = %u",channel[i].evc_id);
    LOG_DEBUG(LOG_CTX_MSG," Channel          = %u.%u.%u.%u",
    ...
    LOG_DEBUG(LOG_CTX_MSG," SourceIP         = %u.%u.%u.%u",
    ...
    #if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
    LOG_DEBUG(LOG_CTX_MSG," channelBandwidth = %llu", channel[i].channelBandwidth);
                               
    if (channel[i].channelBandwidth > PTIN_IGMP_ADMISSION_CONTROL_MAX_BANDWIDTH_IN_BPS)
    {
      LOG_ERR(LOG_CTX_MSG,"Invalid channelBandwidth = %llu", channel[i].channelBandwidth);
      return L7_FAILURE;
    }
    #endif
...
L7_RC_t ptin_msg_IGMP_channel_remove(msg_MCStaticChannel_t *channel, L7_uint16 n_channels)
{
  ...
  for (i=0; i<n_channels; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"Channel remotion index %u:",i);
    LOG_DEBUG(LOG_CTX_MSG," SlotId =%u",channel[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," EvcId  =%u",channel[i].evc_id);
    LOG_DEBUG(LOG_CTX_MSG," Channel=%u.%u.%u.%u",
              (channel[i].channelIp.s_addr>>24) & 0xff,(channel[i].channelIp.s_addr>>16) & 0xff,(channel[i].channelIp.s_addr>>8) & 0xff,channel[i].channelIp.s_addr & 0xff);
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Error (%d) removing channel", rc);
      rc_global = rc;
      continue;
    }
...
{  
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Going to retrieve list of channels");
  LOG_DEBUG(LOG_CTX_MSG," slotId = %u",             inputPtr->SlotId);
  LOG_DEBUG(LOG_CTX_MSG," EvcId  = %u",             inputPtr->evc_id);
  LOG_DEBUG(LOG_CTX_MSG," Client.Mask  = 0x%02x",   inputPtr->client.mask);
  LOG_DEBUG(LOG_CTX_MSG," Client.OVlan = %u",       inputPtr->client.outer_vlan);
  LOG_DEBUG(LOG_CTX_MSG," Client.IVlan = %u",       inputPtr->client.inner_vlan);
  LOG_DEBUG(LOG_CTX_MSG," Client.Intf  = %u/%u",    inputPtr->client.intf.intf_type,inputPtr->client.intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG," Entry_idx=%u",            inputPtr->entryId);
  LOG_DEBUG(LOG_CTX_MSG," Max Number of Channels=%u",*numberOfChannels);
  ...
  #if (PTIN_BOARD_IS_LINECARD || PTIN_BOARD_IS_STANDALONE)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG,"Converted [client.Mask:%u Client.OVlan:%u]",inputPtr->client.mask,inputPtr->client.outer_vlan);
  }
  #endif
  ...
  if (rc==L7_SUCCESS)
  {
    ...
    {      
      ...
      LOG_TRACE(LOG_CTX_MSG,"EntryId[%u] -> Group:[%08X] Source[%08X] isStatic[%s]", outputPtr[i].entryId, outputPtr[i].chIP, outputPtr[i].srcIP, outputPtr[i].chType?"Yes":"No");
    }
     *numberOfChannels = i;
     LOG_DEBUG(LOG_CTX_MSG, "Read %u channels and retrieving %u channels.",number_of_channels, *numberOfChannels);
  }
  else if (rc==L7_NOT_EXIST)
  {
    *numberOfChannels = 0;
    LOG_NOTICE(LOG_CTX_MSG, "No channels to retrieve");
  }
  else
  {
    LOG_ERR(LOG_CTX_MSG,"Error with ptin_igmp_channelList_get");
    return rc;
  }
...
L7_RC_t ptin_msg_snoop_sync_request(msg_SnoopSyncRequest_t *snoopSyncRequest)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Invalid input parameters snoopSyncRequest=%p",snoopSyncRequest);
    return L7_FAILURE;
  }
  
  LOG_DEBUG(LOG_CTX_MSG,"Received Snoop Sync Request Message");     
  LOG_DEBUG(LOG_CTX_MSG," serviceId=%u",snoopSyncRequest->serviceId);
  LOG_DEBUG(LOG_CTX_MSG," groupAddr=%08X",snoopSyncRequest->groupAddr);
#if !PTIN_BOARD_IS_MATRIX  
  LOG_DEBUG(LOG_CTX_MSG," portId=%u",snoopSyncRequest->portId);
#endif
       ...
       && (L7_SUCCESS != (rc=ptin_evc_intRootVlan_get(snoopSyncRequest->serviceId, &mcastRootVlan))))
  {
    if( rc != L7_NOT_EXIST)
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to get mcastRootVlan from serviceId:%u",snoopSyncRequest->serviceId);
      return rc;
    }
#if PTIN_BOARD_IS_MATRIX 
    LOG_NOTICE(LOG_CTX_IGMP, "Evc Id is not yet created. Silently Ignoring Snoop Sync Request! [serviceId:%u groupAddr:%08X]", snoopSyncRequest->serviceId, snoopSyncRequest->groupAddr);
#else
    LOG_NOTICE(LOG_CTX_IGMP, "Evc Id is not yet created. Silently Ignoring Snoop Sync Request! [serviceId:%u portId:%u groupAddr:%08X]", snoopSyncRequest->serviceId, snoopSyncRequest->portId, snoopSyncRequest->groupAddr);
#endif
    return rc;
  }
...
L7_RC_t ptin_msg_snoop_sync_reply(msg_SnoopSyncReply_t *snoopSyncReply, L7_uint32 numberOfSnoopEntries)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Invalid parameters: snoopSyncReply=%p",snoopSyncReply);
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG,"Received Snoop Sync Reply Message: numberOfSnoopEntries=%u | maxNumberOfSnoopEntries:%u",numberOfSnoopEntries,maxNumberOfSnoopEntries);
  
  if(numberOfSnoopEntries==0)
  {
    LOG_NOTICE(LOG_CTX_MSG,"The number of snoop entries is equal to zero. Silently ignoring this reply message.");
    return L7_SUCCESS;
  }
  ...
  #if PTIN_BOARD_IS_MATRIX
  {
    ...
    {
      LOG_TRACE(LOG_CTX_MSG," serviceId=%u",snoopSyncReply[iterator].serviceId);
      LOG_TRACE(LOG_CTX_MSG," groupAddr=%08X",snoopSyncReply[iterator].groupAddr);
      LOG_TRACE(LOG_CTX_MSG," StaticEntry=%s",snoopSyncReply[iterator].isStatic?"Yes":"No");
      LOG_TRACE(LOG_CTX_MSG," numberOfActivePorts=%u",snoopSyncReply[iterator].numberOfActivePorts);
      ...
      if(snoopSyncReply[iterator].numberOfActivePorts>0)
      {
        ...
        {   
          ...
          {
            LOG_DEBUG(LOG_CTX_PROTB, "Snoop Port Open :%u", intIfNum);
            ...
            {
              LOG_ERR(LOG_CTX_PROTB, "Failed to open port");
              return L7_FAILURE;
            }
  ...
  #else
  {
    ...
    { 
      ...
      LOG_TRACE(LOG_CTX_MSG," groupAddr=%08X",snoopSyncReply[iterator].groupAddr);
      LOG_TRACE(LOG_CTX_MSG," StaticEntry=%s",snoopSyncReply[iterator].isStatic?"Yes":"No");   
      LOG_TRACE(LOG_CTX_MSG," portId=%u",snoopSyncReply->portId);
  ...
  {
    LOG_TRACE(LOG_CTX_MSG, "This is the Last Snoop Sync Reply Message Received");
    return L7_SUCCESS;
  }
  ...
  {
    LOG_NOTICE(LOG_CTX_MSG, "Not sending Another Snoop Sync Request Message to Sync the Remaining Snoop Entries. I'm a Working Matrix on slotId:%u",ptin_fgpa_board_slot());
    return SUCCESS;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Sending Snoop Sync Request Message [groupAddr:%08X | serviceId:%u] to ipAddr:%08X to Sync the Remaining Snoop Entries", snoopSyncRequest.groupAddr, snoopSyncRequest.serviceId, ipAddr);         
#else
  ...
  {
    LOG_NOTICE(LOG_CTX_MSG, "Not sending Another Snoop Sync Request Message to Sync the Remaining Snoop Entries. I'm a Working slotId/intfNum:%u/%u",protTypebIntfConfig.pairSlotId, protTypebIntfConfig.intfNum);
    return SUCCESS;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Sending Snoop Sync Request Message [groupAddr:%08X | serviceId:%u | portId:%u] to ipAddr:%08X to Sync the Remaining Snoop Entries", snoopSyncRequest.groupAddr, snoopSyncRequest.serviceId, snoopSyncRequest.portId, ipAddr);
#endif
  ...
  {
    LOG_ERR(LOG_CTX_PROTB, "Failed to Send Snoop Sync Request Message");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_IGMP_clientList_get(msg_MCActiveChannelClientsResponse_t *client_list)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG,"Going to retrieve list of clients");
  LOG_DEBUG(LOG_CTX_MSG," slotId     =%u",client_list->SlotId);
  LOG_DEBUG(LOG_CTX_MSG," EvcId      =%u",client_list->evc_id);
  LOG_DEBUG(LOG_CTX_MSG," groupAddr=%u.%u.%u.%u",(client_list->channelIp.s_addr>>24) & 0xff,(client_list->channelIp.s_addr>>16) & 0xff,(client_list->channelIp.s_addr>>8) & 0xff,client_list->channelIp.s_addr & 0xff);
  
  LOG_DEBUG(LOG_CTX_MSG," sourceAddr=%u.%u.%u.%u",(client_list->sourceIp.s_addr>>24) & 0xff,(client_list->sourceIp.s_addr>>16) & 0xff,(client_list->sourceIp.s_addr>>8) & 0xff,client_list->sourceIp.s_addr & 0xff);
  LOG_DEBUG(LOG_CTX_MSG," Page_idx=%u",client_list->page_index);
  ...
  LOG_DEBUG(LOG_CTX_MSG,"number_of_clients=%u total_clients=%u", number_of_clients, total_clients);
  if (rc==L7_SUCCESS)
  {
    /* Copy channels to message */
  ...
  {
    LOG_WARNING(LOG_CTX_MSG, "No channels to retrieve");
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Error with ptin_igmp_clientList_get");
    return rc;
  }
...
L7_RC_t ptin_msg_uplink_protection_cmd(msg_uplinkProtCmd *cmd, L7_int n)
{
  ...
  for (i = 0; i < n; i++) 
  {
    LOG_TRACE(LOG_CTX_MSG, "Received protection command: "); 
    LOG_TRACE(LOG_CTX_MSG, " slot = %u", cmd[i].slotId);
    LOG_TRACE(LOG_CTX_MSG, " port = %u", cmd[i].port);
    LOG_TRACE(LOG_CTX_MSG, " cmd  = %u", cmd[i].protCmd);
  ...
  if (i2add >= 0 && i2rem >= 0)
  {
    LOG_TRACE(LOG_CTX_MSG, "Applying plan D for slot/ports %d/%d -> %d/%d",
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Plan D failed for slot/ports %d/%d -> %d/%d",
              cmd[i2rem].slotId, cmd[i2rem].port, cmd[i2add].slotId, cmd[i2add].port);
      rc = L7_FAILURE;
    }
  }
  ...
  for (i = 0; i < n; i++) 
  {
    ...
    LOG_TRACE(LOG_CTX_MSG, "Applying plan C for slot/port %d/%d", cmd[i].slotId, cmd[i].port);
    /* PLAN C */
    if (ptin_intf_protection_cmd_planC(cmd[i].slotId, cmd[i].port, cmd[i].protCmd) != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Plan C failed for slot/port %d/%d", cmd[i].slotId, cmd[i].port);
      rc = L7_FAILURE;
    }
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Protection switch failed"); 
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Successfull protection switch!"); 
  }
#else
  LOG_ERR(LOG_CTX_MSG, "Uplink protection not supported for this board!");
  rc = L7_NOT_SUPPORTED;
#endif
...
{
  LOG_TRACE(LOG_CTX_MSG, "Received request to sync MGMD port: ");
  LOG_TRACE(LOG_CTX_MSG, " admin      = %u",   port_sync_data->admin);
  LOG_TRACE(LOG_CTX_MSG, " serviceId  = %u",   port_sync_data->serviceId);
  LOG_TRACE(LOG_CTX_MSG, " portId     = %u",   port_sync_data->portId);
  LOG_TRACE(LOG_CTX_MSG, " groupAddr  = %08X", port_sync_data->groupAddr);
  LOG_TRACE(LOG_CTX_MSG, " sourceAddr = %08X", port_sync_data->sourceAddr);
  LOG_TRACE(LOG_CTX_MSG, " groupType  = %u",   port_sync_data->groupType);
...
L7_RC_t ptin_msg_pcs_prbs_enable(msg_ptin_pcs_prbs *msg, L7_int n_msg)
{
  ...
  for (i=0; i<n_msg; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"PRBS configuration:");
    LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Port   = %u/%u",msg[i].intf.intf_type,msg[i].intf.intf_id);
    LOG_DEBUG(LOG_CTX_MSG," Enable = %u",msg[i].enable);
    ...
    if (ptin_intf_ptintf2intIfNum(&ptin_intf, &intIfNum)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Non existent port (%u/%u)",ptin_intf.intf_type,ptin_intf.intf_id);
      return L7_FAILURE;
    }
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error settings PRBS enable of port %u/%u to %u",ptin_intf.intf_type,ptin_intf.intf_id,msg[i].enable);
      return rc;
    }

    LOG_TRACE(LOG_CTX_MSG,"Success setting PRBS enable of port %u/%u to %u",ptin_intf.intf_type,ptin_intf.intf_id,msg[i].enable);
  }
...
L7_RC_t ptin_msg_pcs_prbs_status(msg_ptin_pcs_prbs *msg, L7_int n_msg)
{
  ...
  for (i=0; i<n_msg; i++)
  {
    LOG_DEBUG(LOG_CTX_MSG,"PRBS status:");
    LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg[i].SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Port   = %u/%u",msg[i].intf.intf_type,msg[i].intf.intf_id);
    ...
    if (rc!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG,"Error getting PRBS errors from port %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
      return rc;
    }

    LOG_TRACE(LOG_CTX_MSG,"Success getting PRBS errors from port %u/%u",ptin_intf.intf_type,ptin_intf.intf_id);
...
L7_RC_t ptin_msg_prbs_enable(msg_ptin_prbs_enable *msg, L7_int n_msg)
{
  ...
  if (msg->intf == 0xff)
  {
    LOG_DEBUG(LOG_CTX_MSG,"PRBS configuration:");
    LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg->SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Port   = %u",msg->intf);
    LOG_DEBUG(LOG_CTX_MSG," Enable = %u",msg->enable);
    ...
    for (port = 0; port < ptin_sys_number_of_ports; port++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_MSG,"Non existent port %u", port);
        rc_global = L7_FAILURE;
        continue;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error settings PRBS enable of port %u to %u", port, enable);
        rc_global = rc;
        continue;
      }

      LOG_TRACE(LOG_CTX_MSG, "Success setting PRBS enable of port %u to %u", port, enable);
    }
  }
  /* Apply to each port */
  else
  {
    for (i=0; i<n_msg; i++)
    {
      LOG_DEBUG(LOG_CTX_MSG,"PRBS configuration:");
      LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg[i].SlotId);
      LOG_DEBUG(LOG_CTX_MSG," Port   = %u",msg[i].intf);
      LOG_DEBUG(LOG_CTX_MSG," Enable = %u",msg[i].enable);
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_TRACE(LOG_CTX_MSG, "Success setting PRBS enable of all ports"); 
  }
...
L7_RC_t ptin_msg_prbs_status(msg_ptin_prbs_request *msg_in, msg_ptin_prbs_status *msg_out, L7_int *n_msg)
{
  ...
  {
    LOG_DEBUG(LOG_CTX_MSG,"PRBS status:");
    LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg_in->SlotId);
    LOG_DEBUG(LOG_CTX_MSG," Port   = %u",msg_in->intf);
    ...
    for (port = 0, i = 0; port < ptin_sys_number_of_ports; port++)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG,"Error getting PRBS errors from port %u/%u", port);
        rc_global = rc;
        continue;
      }

      LOG_TRACE(LOG_CTX_MSG,"Success getting PRBS errors from port %u", port);
  ...
  else
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_MSG,"PRBS status:");
      LOG_DEBUG(LOG_CTX_MSG," slotId = %u",msg_in[i].SlotId);
      LOG_DEBUG(LOG_CTX_MSG," Port   = %u",msg_in[i].intf);
  ...
  if (rc_global == L7_SUCCESS)
  {
    LOG_TRACE(LOG_CTX_MSG, "Success getting PRBS errors for all ports");
  }
...
static L7_RC_t ptin_msg_bwProfileStruct_fill(msg_HwEthBwProfile_t *msgBwProfile, ptin_bw_profile_t *profile, ptin_bw_meter_t *meter)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG,"Null arguments");
    return L7_FAILURE;
  }
  ...
  if (msgBwProfile->mask & MSG_HWETH_BWPROFILE_MASK_INTF_SRC)
  {
        ...
        (msgBwProfile->service_vlan>0 && msgBwProfile->service_vlan<4096))
    {
      ...
      LOG_DEBUG(LOG_CTX_MSG," SVID extracted!");
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG," SrcIntf extracted!");
  }
  ...
  if (msgBwProfile->mask & MSG_HWETH_BWPROFILE_MASK_INTF_DST)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," DstIntf extracted!");
  }
  ...
  if (msgBwProfile->mask & MSG_HWETH_BWPROFILE_MASK_PROFILE)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," Profile data extracted!");
  }
  else
  {
...
static L7_RC_t ptin_msg_evcStatsStruct_fill(msg_evcStats_t *msg_evcStats, ptin_evcStats_profile_t *evcStats_profile)
{
  ...
  if (msg_evcStats->mask & MSG_EVC_COUNTERS_MASK_INTF)
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," Intf extracted!");
  }
      ...
      (msg_evcStats->channel_ip!=0 && msg_evcStats->channel_ip!=(L7_uint32)-1))
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG," ChannelIP extracted!");
  }
...
L7_RC_t ptin_msg_wr_MEP(ipc_msg *inbuff, ipc_msg *outbuff, L7_uint32 i)
{
  ...
  L7_uint16 slot, port;
    ...
    if (slot!=pi[i].tu_slot) {
        LOG_ERR(LOG_CTX_MSG, "ptin_intf_port=%lu => (slot,port)=(%u,%u) struct_passed_slot=%u", porta, slot, port, pi[i].tu_slot);
        //return L7_FAILURE;
    }
  ...
  if (valid_mep_index(pi[i].index)) {
      ...
      if (EMPTY_T_MEP(*p)) LOG_DEBUG(LOG_CTX_MSG, "MEP EMPTY");//changing_trap=0;
  ...
  case 0:    r=S_OK;
...
#ifdef USING_SDK_OAM_FP_CREATE
    {
     ...
     if (L7_SUCCESS!=ptin_intf_port2intIfNum(porta, &intIfNum))
       LOG_DEBUG(LOG_CTX_MSG, "Insucess port2intfNum");
     else
     if (L7_SUCCESS!=ptin_xlate_ingress_get(intIfNum, pi[i].bd.vid, PTIN_XLATE_NOT_DEFINED, &vidInternal, L7_NULLPTR)) 
       LOG_DEBUG(LOG_CTX_MSG, "Insucess ingress get");
     else {
         pi[i].bd.vid=vidInternal;
         LOG_DEBUG(LOG_CTX_MSG, "Sucess ingress get");
         if (L7_SUCCESS!=dtlPtinMEPControl(intIfNum, &hm))
           LOG_DEBUG(LOG_CTX_MSG, "Insucess MEP CONTROL");
         else {
           LOG_DEBUG(LOG_CTX_MSG, "Sucess MEP CONTROL");
         }
     }
    }
#endif
    LOG_DEBUG(LOG_CTX_MSG, "i_MEP#%llu\tporta=%lu\tvid=%llu\tlevel=%lu", pi[i].index, porta, pi[i].bd.vid, pi[i].bd.level);
    break;
  ...
  if (r==S_OK)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Sucess");
    return L7_SUCCESS;
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Insucess");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_erps_set(msg_erps_t *msgErpsConf)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "ERPS#%u is out of range [0..%u]", msgErpsConf->idx, MAX_PROT_PROT_ERPS-1);
    return L7_FAILURE;
  }
  ...
  LOG_DEBUG(LOG_CTX_MSG, "ERPS#%u",                    msgErpsConf->idx);
  LOG_DEBUG(LOG_CTX_MSG, " .ringId             = %d",  ptinErpsConf.ringId);
  LOG_DEBUG(LOG_CTX_MSG, " .isOpenRing         = %d",  ptinErpsConf.isOpenRing);
  LOG_DEBUG(LOG_CTX_MSG, " .controlVid         = %d",  ptinErpsConf.controlVid);
  LOG_DEBUG(LOG_CTX_MSG, " .megLevel           = %d",  ptinErpsConf.megLevel);
  LOG_DEBUG(LOG_CTX_MSG, " .port0.slot         = %d",  ptinErpsConf.port0.slot);
  LOG_DEBUG(LOG_CTX_MSG, " .port0.type         = %d",  ptinErpsConf.port0.type);
  LOG_DEBUG(LOG_CTX_MSG, " .port0.idx          = %d",  ptinErpsConf.port0.idx);
  LOG_DEBUG(LOG_CTX_MSG, " .port1.slot         = %d",  ptinErpsConf.port1.slot);
  LOG_DEBUG(LOG_CTX_MSG, " .port1.type         = %d",  ptinErpsConf.port1.type);
  LOG_DEBUG(LOG_CTX_MSG, " .port1.idx          = %d",  ptinErpsConf.port1.idx);
  LOG_DEBUG(LOG_CTX_MSG, " .port0Role          = %d",  ptinErpsConf.port0Role);
  LOG_DEBUG(LOG_CTX_MSG, " .port1Role          = %d",  ptinErpsConf.port1Role);
  LOG_DEBUG(LOG_CTX_MSG, " .port0CfmIdx        = %d",  ptinErpsConf.port0CfmIdx);
  LOG_DEBUG(LOG_CTX_MSG, " .port1CfmIdx        = %d",  ptinErpsConf.port1CfmIdx);
  LOG_DEBUG(LOG_CTX_MSG, " .revertive          = %d",  ptinErpsConf.revertive);
  LOG_DEBUG(LOG_CTX_MSG, " .guardTimer         = %d",  ptinErpsConf.guardTimer);
  LOG_DEBUG(LOG_CTX_MSG, " .holdoffTimer       = %d",  ptinErpsConf.holdoffTimer);
  LOG_DEBUG(LOG_CTX_MSG, " .waitToRestoreTimer = %d",  ptinErpsConf.waitToRestoreTimer);

  if (ptin_erps_add_entry(msgErpsConf->idx, (erpsProtParam_t *) &ptinErpsConf) != msgErpsConf->idx)
  {
    LOG_ERR(LOG_CTX_MSG, "Error Creating ERPS#%u", msgErpsConf->idx);
    return L7_FAILURE;
  } 
...
L7_RC_t ptin_msg_erps_del(msg_erps_t *msgErpsConf)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error Removing ERPS#%u", msgErpsConf->idx);
...
L7_RC_t ptin_msg_erps_config(msg_erps_t *msgErpsConf)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "ERPS#%u",                     msgErpsConf->idx);
  LOG_DEBUG(LOG_CTX_MSG, " .mask               = 0x%x", msgErpsConf->mask);
  LOG_DEBUG(LOG_CTX_MSG, " .isOpenRing         = %d",   msgErpsConf->isOpenRing);
  LOG_DEBUG(LOG_CTX_MSG, " .port0CfmIdx        = %d",   msgErpsConf->port0CfmIdx);
  LOG_DEBUG(LOG_CTX_MSG, " .port1CfmIdx        = %d",   msgErpsConf->port1CfmIdx);
  LOG_DEBUG(LOG_CTX_MSG, " .revertive          = %d",   msgErpsConf->revertive);
  LOG_DEBUG(LOG_CTX_MSG, " .guardTimer         = %d",   msgErpsConf->guardTimer);
  LOG_DEBUG(LOG_CTX_MSG, " .holdoffTimer       = %d",   msgErpsConf->holdoffTimer);
  LOG_DEBUG(LOG_CTX_MSG, " .waitToRestoreTimer = %d",   msgErpsConf->waitToRestoreTimer);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Error creating/reconfiguring ERPS#%u", msgErpsConf->idx);
    return L7_FAILURE;
  }  
...
L7_RC_t ptin_msg_erps_status(msg_erps_status_t *msgErpsStatus)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "ERPS#%u is out of range [0..%u]", msgErpsStatus->idx, MAX_PROT_PROT_ERPS-1);
    return L7_FAILURE;
  }

  LOG_DEBUG(LOG_CTX_MSG, "ERPS#%u", msgErpsStatus->idx);

  if (ptin_erps_get_status(msgErpsStatus->idx, &status) != msgErpsStatus->idx)
  {
    LOG_ERR(LOG_CTX_MSG, "Error Retrieving Status ERPS#%u", msgErpsStatus->idx);
    return L7_FAILURE;
  }
...
int ptin_msg_erps_status_next(msg_erps_status_t *msgErpsStatus, L7_int *n)
{
  ...
  *n = 0;
  ...
  LOG_DEBUG(LOG_CTX_MSG, "ERPS Next Index %d", nextIdx);
  ...
  while ( i < CCMSG_ERPS_STATUS_PAGESIZE )
  {
    ...
    {
      LOG_DEBUG(LOG_CTX_MSG, "ERPS#%u is out of range [0..%u]", nextIdx, MAX_PROT_PROT_ERPS-1);
      break;
    }
    ...
    LOG_DEBUG(LOG_CTX_MSG, "ERPS#%d status retrieved", nextIdx);
...
L7_RC_t ptin_msg_erps_cmd(msg_erps_cmd_t *msgErpsCmd)
{
  ...
  LOG_DEBUG(LOG_CTX_MSG, "ERPS#%u: CMD %d, Port %d", msgErpsCmd->idx, msgErpsCmd->cmd, msgErpsCmd->port);

  /* Validate ERPS# range (idx [0..MAX_PROT_PROT_ERPS[) */
  if (msgErpsCmd->idx >= MAX_PROT_PROT_ERPS)
  {
    LOG_ERR(LOG_CTX_MSG, "ERPS#%u is out of range [0..%u]", msgErpsCmd->idx, MAX_PROT_PROT_ERPS-1);
    return L7_FAILURE;
  }

  if ( (msgErpsCmd->port != 0) && (msgErpsCmd->port != 1) )
  {
    LOG_ERR(LOG_CTX_MSG, "Port %d is out of range [0,1]", msgErpsCmd->port);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_arp_acl_rule_config(msg_arp_acl_t *msgArpAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclType Invalid (%d)", msgArpAcl->aclType);
    return L7_FAILURE;
  }

  if (operation == ACL_OPERATION_CREATE && msgArpAcl->action != ACL_ACTION_PERMIT)
  {
    LOG_ERR(LOG_CTX_MSG, "action Invalid (%d) for rule creation", msgArpAcl->action);
    return L7_FAILURE;
  }
  

  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Slot Id        %d",                              msgArpAcl->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Type       %s",                              "ARP");
  LOG_DEBUG(LOG_CTX_MSG, "ACL Name       %s",                              msgArpAcl->name);
  LOG_DEBUG(LOG_CTX_MSG, "Action         %s",                              "PERMIT");
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Src Mac Addr   %02x:%02x:%02x:%02x:%02x:%02x",   msgArpAcl->srcMacAddr[0], 
                                                                                msgArpAcl->srcMacAddr[1],
                                                                                msgArpAcl->srcMacAddr[2],
                                                                                msgArpAcl->srcMacAddr[3],
                                                                                msgArpAcl->srcMacAddr[4],
                                                                                msgArpAcl->srcMacAddr[5]);
  LOG_DEBUG(LOG_CTX_MSG, "SrcIPAddr      %03u.%03u.%03u.%03u (family=%u)", (msgArpAcl->srcIpAddr.addr.ipv4>>24) & 0xff,
...
L7_RC_t ptin_msg_mac_acl_rule_config(msg_mac_acl_t *msgMacAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclType Invalid (%d)", msgMacAcl->aclType);
    return L7_FAILURE;
  }

  if (msgMacAcl->aclId >= L7_MAX_ACL_LISTS)
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid ACL ID (%d)", msgMacAcl->aclId);
    return L7_FAILURE;
  }

  if (msgMacAcl->aclRuleId > L7_MAX_NUM_RULES_PER_ACL)
  {
    LOG_ERR(LOG_CTX_MSG, "aclRuleId Invalid (%d)", msgMacAcl->aclRuleId);
    return L7_FAILURE;
  }

  if (msgMacAcl->action > ACL_ACTION_PERMIT)
  {
    LOG_ERR(LOG_CTX_MSG, "action Invalid (%d)", msgMacAcl->action);
    return L7_FAILURE;
  }
  

  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Slot Id        %d",                              msgMacAcl->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Type       %s",                              aclTypeStr[msgMacAcl->aclType]);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Id         %d",                              msgMacAcl->aclId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Name       %s",                              msgMacAcl->name);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Id    %d",                              msgMacAcl->aclRuleId);
  LOG_DEBUG(LOG_CTX_MSG, "Action         %s",                              actionStr[msgMacAcl->action]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Mask  0x%x",                            msgMacAcl->aclRuleMask);
  LOG_DEBUG(LOG_CTX_MSG, "Src Mac Addr   %.2x:%.2x:%.2x:%.2x:%.2x:%.2x",   msgMacAcl->srcMacAddr[0], 
                                                                                msgMacAcl->srcMacAddr[1],
                                                                                msgMacAcl->srcMacAddr[2],
                                                                                msgMacAcl->srcMacAddr[3],
                                                                                msgMacAcl->srcMacAddr[4],
                                                                                msgMacAcl->srcMacAddr[5]);
  LOG_DEBUG(LOG_CTX_MSG, "Src Mac Mask   %.2x:%.2x:%.2x:%.2x:%.2x:%.2x",   msgMacAcl->srcMacMask[0], 
                                                                                msgMacAcl->srcMacMask[1],
                                                                                msgMacAcl->srcMacMask[2],
                                                                                msgMacAcl->srcMacMask[3],
                                                                                msgMacAcl->srcMacMask[4],
                                                                                msgMacAcl->srcMacMask[5]);
  LOG_DEBUG(LOG_CTX_MSG, "Dst Mac Addr   %.2x:%.2x:%.2x:%.2x:%.2x:%.2x",   msgMacAcl->dstMacAddr[0], 
                                                                                msgMacAcl->dstMacAddr[1],
                                                                                msgMacAcl->dstMacAddr[2],
                                                                                msgMacAcl->dstMacAddr[3],
                                                                                msgMacAcl->dstMacAddr[4],
                                                                                msgMacAcl->dstMacAddr[5]);
  LOG_DEBUG(LOG_CTX_MSG, "Dst Mac Mask   %.2x:%.2x:%.2x:%.2x:%.2x:%.2x",   msgMacAcl->dstMacMask[0], 
                                                                                msgMacAcl->dstMacMask[1],
                                                                                msgMacAcl->dstMacMask[2],
                                                                                msgMacAcl->dstMacMask[3],
                                                                                msgMacAcl->dstMacMask[4],
                                                                                msgMacAcl->dstMacMask[5]);
  LOG_DEBUG(LOG_CTX_MSG, "EtherType      0x%.4x",                          msgMacAcl->eType);

  if (msgMacAcl->startVlan == msgMacAcl->endVlan)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Vlan           %d",                            msgMacAcl->startVlan);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Vlan Range     %d-%d",                         msgMacAcl->startVlan, msgMacAcl->endVlan);
  }

  LOG_DEBUG(LOG_CTX_MSG, "COS            %d",                              msgMacAcl->cosVal);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Operation      %s",                              operationStr[operation]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
...
L7_RC_t ptin_msg_ip_acl_rule_config(msg_ip_acl_t *msgIpAcl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclType Invalid (%d)", msgIpAcl->aclType);
    return L7_FAILURE;
  }
  ...
  if (msgIpAcl->aclType == ACL_TYPE_IP_STANDARD)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid ACL ID (%d)", msgIpAcl->aclId);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclRuleId Invalid (%d)", msgIpAcl->aclRuleId);
    return L7_FAILURE;
  }

  if (msgIpAcl->action > ACL_ACTION_PERMIT)
  {
    LOG_ERR(LOG_CTX_MSG, "action Invalid (%d)", msgIpAcl->action);
    return L7_FAILURE;
  }

  
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Slot Id        %d",                              msgIpAcl->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Type       %s",                              aclTypeStr[msgIpAcl->aclType]);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Id         %d",                              msgIpAcl->aclId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Name       %s",                              msgIpAcl->name);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Id    %d",                              msgIpAcl->aclRuleId);
  LOG_DEBUG(LOG_CTX_MSG, "Action         %s",                              actionStr[msgIpAcl->action]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Mask  0x%x",                            msgIpAcl->aclRuleMask);
  LOG_DEBUG(LOG_CTX_MSG, "Protocol       %d",                              msgIpAcl->protocol);

  usmDbInetNtoa(msgIpAcl->srcIpAddr,  ipAddr);
  LOG_DEBUG(LOG_CTX_MSG, "Src IP Addr    %s",                              ipAddr);

  usmDbInetNtoa(msgIpAcl->srcIpMask,  ipAddr);
  LOG_DEBUG(LOG_CTX_MSG, "Src IP Mask    %s",                              ipAddr);
  
  usmDbInetNtoa(msgIpAcl->dstIpAddr,  ipAddr);
  LOG_DEBUG(LOG_CTX_MSG, "Dst IP Addr    %s",                              ipAddr);
  
  usmDbInetNtoa(msgIpAcl->dstIpMask,  ipAddr);
  LOG_DEBUG(LOG_CTX_MSG, "Dst IP Mask    %s",                              ipAddr);  

  if (msgIpAcl->srcStartPort == msgIpAcl->srcEndPort)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Src L4 Port    %d",                            msgIpAcl->srcStartPort);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Src L4 Port Range   %d-%d",                    msgIpAcl->srcStartPort, msgIpAcl->srcEndPort);
  }

  if (msgIpAcl->dstStartPort == msgIpAcl->dstEndPort)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Dst L4 Port    %d",                            msgIpAcl->dstStartPort);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Dst L4 Port Range   %d-%d",                    msgIpAcl->dstStartPort, msgIpAcl->dstEndPort);
  }

  LOG_DEBUG(LOG_CTX_MSG, "TOS            0x%.2x",                          msgIpAcl->tosVal);
  LOG_DEBUG(LOG_CTX_MSG, "TOS Mask       0x%.2x",                          msgIpAcl->tosMask);
  LOG_DEBUG(LOG_CTX_MSG, "DSCP           %d",                              msgIpAcl->dscpVal);
  LOG_DEBUG(LOG_CTX_MSG, "Prec           %d",                              msgIpAcl->precVal);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Operation      %s",                              operationStr[operation]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
...
L7_RC_t ptin_msg_ipv6_acl_rule_config(msg_ipv6_acl_t *msgIpv6Acl, ACL_OPERATION_t operation)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclType Invalid (%d)", msgIpv6Acl->aclType);
    return L7_FAILURE;
  }

  if (msgIpv6Acl->aclId > L7_MAX_ACL_LISTS)
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid ACL ID (%d)", msgIpv6Acl->aclId);
    return L7_FAILURE;
  }

  if (msgIpv6Acl->aclRuleId > L7_MAX_NUM_RULES_PER_ACL)
  {
    LOG_ERR(LOG_CTX_MSG, "aclRuleId Invalid (%d)", msgIpv6Acl->aclRuleId);
    return L7_FAILURE;
  }

  if (msgIpv6Acl->action > ACL_ACTION_PERMIT)
  {
    LOG_ERR(LOG_CTX_MSG, "action Invalid (%d)", msgIpv6Acl->action);
    return L7_FAILURE;
  }

  
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Slot Id        %d",                              msgIpv6Acl->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Type       %s",                              aclTypeStr[msgIpv6Acl->aclType]);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Id         %d",                              msgIpv6Acl->aclId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Name       %s",                              msgIpv6Acl->name);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Id    %d",                              msgIpv6Acl->aclRuleId);
  LOG_DEBUG(LOG_CTX_MSG, "Action         %s",                              actionStr[msgIpv6Acl->action]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "ACL Rule Mask  0x%x",                            msgIpv6Acl->aclRuleMask);
  LOG_DEBUG(LOG_CTX_MSG, "Protocol       %d",                              msgIpv6Acl->protocol);

  if (osapiInetNtop(L7_AF_INET6, (L7_uchar8 *)msgIpv6Acl->src6Addr, ipAddr, sizeof(ipAddr)) != L7_NULLPTR)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Src IP Addr    %s/%d",                         ipAddr, msgIpv6Acl->src6PrefixLen);
  }

  if (osapiInetNtop(L7_AF_INET6, (L7_uchar8 *)msgIpv6Acl->dst6Addr, ipAddr, sizeof(ipAddr)) != L7_NULLPTR)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Dst IP Addr    %s/%d",                         ipAddr, msgIpv6Acl->dst6PrefixLen);
  }

  if (msgIpv6Acl->srcStartPort == msgIpv6Acl->srcEndPort)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Src L4 Port    %d",                            msgIpv6Acl->srcStartPort);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Src L4 Port Range   %d-%d",                    msgIpv6Acl->srcStartPort, msgIpv6Acl->srcEndPort);
  }

  if (msgIpv6Acl->dstStartPort == msgIpv6Acl->dstEndPort)
  {
    LOG_DEBUG(LOG_CTX_MSG, "Dst L4 Port    %d",                            msgIpv6Acl->dstStartPort);
  }
  else
  {
    LOG_DEBUG(LOG_CTX_MSG, "Dst L4 Port Range   %d-%d",                    msgIpv6Acl->dstStartPort, msgIpv6Acl->dstEndPort);
  }

  LOG_DEBUG(LOG_CTX_MSG, "DSCP           %d",                              msgIpv6Acl->dscpVal);
  LOG_DEBUG(LOG_CTX_MSG, "Flow Label     %d",                              msgIpv6Acl->flowLabelVal);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Operation      %s",                              operationStr[operation]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
...
L7_RC_t ptin_msg_acl_rule_config(void *msgAcl, L7_uint msgId, L7_uint msgDim)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid msgId: %u", msgId);
    return L7_FAILURE;
  }
  ...
  while (((L7_uint8 *) msg) < ((L7_uint8 *) msgAcl + msgDim))
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Invalid struct type: %u", msg[1]);
      return L7_FAILURE;
    }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Message pointer is out of the expected place... returning error");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_acl_apply(msg_apply_acl_t *msgAcl, ACL_OPERATION_t operation, L7_uint8 aclType)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "aclType Invalid (%d)", msgAcl->aclType);
    return L7_FAILURE;
  }

  if (msgAcl->direction >= ACL_DIRECTION_MAX)
  {
    LOG_ERR(LOG_CTX_MSG, "direction Invalid (%d)", msgAcl->direction);
    return L7_FAILURE;
  }
  
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Slot Id        %u",                              msgAcl->slotId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Type       %s",                              aclTypeStr[msgAcl->aclType]);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Id         %u",                              msgAcl->aclId);
  LOG_DEBUG(LOG_CTX_MSG, "ACL Name       %s",                              msgAcl->name);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "interface      %u",                              msgAcl->interface);
  LOG_DEBUG(LOG_CTX_MSG, "vlanId         %u",                              msgAcl->vlanId);
  LOG_DEBUG(LOG_CTX_MSG, "evcId          %u",                              msgAcl->evcId);
  LOG_DEBUG(LOG_CTX_MSG, "direction      %s",                              directionStr[msgAcl->direction]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  LOG_DEBUG(LOG_CTX_MSG, "Operation      %s",                              operationStr[operation]);
  LOG_DEBUG(LOG_CTX_MSG, "-------------------------------------------");
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Neither interface neither evcId is valid");
    return L7_FAILURE;
  }
  ...
  if (msgAcl->vlanId > 0 && msgAcl->vlanId < 4096)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "ACL FAILURE: Unable to get extEVCid from NNI VLAN %u", msgAcl->vlanId);
      return L7_NOT_EXIST;
    }

    LOG_DEBUG(LOG_CTX_MSG, "Retrieved VLAN ID %d", (L7_uint32) aclApply.vlanId[0]);
  }
  else if (msgAcl->evcId < PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error while retrieving VLAN ID(rc=%d)", rc);
      return rc;
    }
  ...
  else
  {
    ...
    LOG_DEBUG(LOG_CTX_MSG, "Using Interface %d", aclApply.interface);
  }
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Invalid ACL type: %d", aclType);
    return L7_FAILURE;
  }

  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG, "Error occurred: rc=%d", rc);
  }

  LOG_DEBUG(LOG_CTX_MSG, "rc=%d", rc);
  return rc;
}
...
L7_RC_t ptin_msg_acl_enable(msg_apply_acl_t *msgAcl, L7_uint msgId, L7_uint n_msg)
{
  ...
  for (i = 0; i < n_msg; i++)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "invalid entry type (i=%u): %d", i, aclType);
      rc_global = L7_FAILURE;
      continue;
    }
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_MSG, "Error occurred at i=%u (rc=%d)", i, rc);
      rc_global = rc;
    }
  }
...
L7_RC_t ptin_msg_routing_intf_create(msg_RoutingIntf* data)
{
  ...
  /* Debug */
  LOG_DEBUG(LOG_CTX_MSG, "Creating new routing interface:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask          = %08X",  data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  routingIntf   = %u/%u", data->routingIntf.intf_type, data->routingIntf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  evcId         = %u",    data->evcId);
  LOG_DEBUG(LOG_CTX_MSG, "  ipAddress     = %08X",  data->ipAddress);
  LOG_DEBUG(LOG_CTX_MSG, "  subnetMask    = %08X",  data->subnetMask);
  LOG_DEBUG(LOG_CTX_MSG, "  mtu           = %u",    data->mtu);
  ...
  if(data->routingIntf.intf_type == PTIN_EVC_INTF_ROUTING)
  {
     ...
     {
       LOG_ERR(LOG_CTX_MSG, "Unable to convert evc_id to internal root vlan");
       return L7_FAILURE;
     }

     LOG_TRACE(LOG_CTX_MSG, "Creating routing interface");
     if(L7_SUCCESS != ptin_routing_intf_create(&routingIntf, internalVlan))
     {
       LOG_ERR(LOG_CTX_MSG, "Unable to create a new routing interface");
       return L7_FAILURE;
     }
  }
  else if(data->routingIntf.intf_type == PTIN_EVC_INTF_LOOPBACK)
  {
     LOG_TRACE(LOG_CTX_MSG, "Creating loopback interface");
     if(L7_SUCCESS != ptin_routing_loopback_create(&routingIntf))
     {
       LOG_ERR(LOG_CTX_MSG, "Unable to create a new loopback interface");
       return L7_FAILURE;
     }
  }

  LOG_TRACE(LOG_CTX_MSG, "Configuring interface IP Address");
  if(L7_SUCCESS != ptin_routing_intf_ipaddress_set(&routingIntf, L7_AF_INET, data->ipAddress, data->subnetMask))
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to set interface IP address");
    return L7_FAILURE;
  }
  ...
  {
    LOG_TRACE(LOG_CTX_MSG, "Configuring interface MTU");
    if(L7_SUCCESS != ptin_routing_intf_mtu_set(&routingIntf, data->mtu))
    {
      LOG_ERR(LOG_CTX_MSG, "Unable to set interface MTU");
      return L7_FAILURE;
    }
  }
...
L7_RC_t ptin_msg_routing_intf_modify(msg_RoutingIntf* data)
{
  ...
  /* Debug */
  LOG_DEBUG(LOG_CTX_MSG, "Configuring routing interface:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask          = %08X",  data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  routingIntf   = %u/%u", data->routingIntf.intf_type, data->routingIntf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  evcId         = %u",    data->evcId);
  LOG_DEBUG(LOG_CTX_MSG, "  ipAddress     = %08X",  data->ipAddress);
...
L7_RC_t ptin_msg_routing_intf_remove(msg_RoutingIntf* data)
{
  ...
  /* Debug */
  LOG_DEBUG(LOG_CTX_MSG, "Removing routing interface:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask          = %08X",  data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  routingIntf   = %u/%u", data->routingIntf.intf_type, data->routingIntf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  evcId         = %u",    data->evcId);
  LOG_DEBUG(LOG_CTX_MSG, "  ipAddress     = %08X",  data->ipAddress);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to remove the existing routing interface");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_arptable_get(msg_RoutingArpTableRequest* inBuffer, msg_RoutingArpTableResponse* outBuffer, L7_uint32* readEntries)
{
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Abnormal context [inBuffer=%p outBuffer=%p readEntries=%p]", inBuffer, outBuffer, readEntries);
    return L7_FAILURE;
  }

  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Getting ARP table:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask       = %08X",  inBuffer->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  intf       = %u/%u", inBuffer->intf.intf_type, inBuffer->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  lastIndex  = %u",    inBuffer->lastIndex);
  LOG_DEBUG(LOG_CTX_MSG, "  maxEntries = %u",    inBuffer->maxEntries);
  ...
  if(inBuffer->mask & CCMSG_ROUTING_ARPTABLE_GET_MASK_INTF)
  {
    ...
    {
      LOG_ERR(LOG_CTX_ROUTING, "Unable to to convert intf %u/%u to intfNum", intf.intf_type, intf.intf_id);
      return L7_FAILURE;
    }
  }
  else
  ...
  */
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to get the ARP table");
    return L7_FAILURE;
  }
  LOG_DEBUG(LOG_CTX_MSG, "Successfully read %u entries", *readEntries);
...
L7_RC_t ptin_msg_routing_arpentry_purge(msg_RoutingArpEntryPurge* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Removing ARP entry:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask       = %08X",  data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  Intf       = %u/%u", data->intf.intf_type, data->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  IP Address = %08X",  data->ipAddr);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to remove the existing ARP entry");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_routetable_get(msg_RoutingRouteTableRequest* inBuffer, msg_RoutingRouteTableResponse* outBuffer, L7_uint32 maxEntries, L7_uint32* readEntries)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Getting route table:");
  LOG_DEBUG(LOG_CTX_MSG, "  intf       = %u/%u", inBuffer->intf.intf_type, inBuffer->intf.intf_id);
  LOG_DEBUG(LOG_CTX_MSG, "  lastIndex  = %u",    inBuffer->lastIndex);
  LOG_DEBUG(LOG_CTX_MSG, "  maxEntries = %u",    maxEntries);
  ...
  */
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to get the route table");
    return L7_FAILURE;
  }
  LOG_DEBUG(LOG_CTX_MSG, "Successfully read %u entries", *readEntries);
...
L7_RC_t ptin_msg_routing_staticroute_add(msg_RoutingStaticRoute* data)
{
  ...
  if( (data == L7_NULLPTR) )
  {
    LOG_ERR(LOG_CTX_MSG, "Abnormal context [data=%p]", data);
    return L7_FAILURE;
  }

  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Configuring static route:");
  LOG_DEBUG(LOG_CTX_MSG, "  dstIpAddr   = %08X", data->dstIpAddr);
  LOG_DEBUG(LOG_CTX_MSG, "  subnetMask  = %08X", data->subnetMask);
  LOG_DEBUG(LOG_CTX_MSG, "  nextHopRtr  = %08X", data->nextHopRtr);
  LOG_DEBUG(LOG_CTX_MSG, "  pref        = %u",   data->pref);
  LOG_DEBUG(LOG_CTX_MSG, "  isNullRoute = %u",   data->isNullRoute);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to configure static route [rc:%u]", rc);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_staticroute_delete(msg_RoutingStaticRoute* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Removing an existing static route:");
  LOG_DEBUG(LOG_CTX_MSG, "  dstIpAddr   = %08X", data->dstIpAddr);
  LOG_DEBUG(LOG_CTX_MSG, "  subnetMask  = %08X", data->subnetMask);
  LOG_DEBUG(LOG_CTX_MSG, "  nextHopRtr  = %08X", data->nextHopRtr);
  LOG_DEBUG(LOG_CTX_MSG, "  pref        = %u",   data->pref);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to remove static route");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_pingsession_create(msg_RoutingPingSessionCreate* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Creating new ping session:");
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx    = %u",   data->sessionIdx);
  LOG_DEBUG(LOG_CTX_MSG, "  dstIpAddr     = %08X", data->dstIpAddr);
  LOG_DEBUG(LOG_CTX_MSG, "  probeCount    = %u",   data->probeCount);
  LOG_DEBUG(LOG_CTX_MSG, "  probeSize     = %u",   data->probeSize);
  LOG_DEBUG(LOG_CTX_MSG, "  probeInterval = %u",   data->probeInterval);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to create new ping session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_pingsession_query(msg_RoutingPingSessionQuery* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Querying ping session:");
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx = %u", data->sessionIdx);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to query ping session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_pingsession_free(msg_RoutingPingSessionFree* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Freeing ping session:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask       = %02X", data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx = %u",   data->sessionIdx);
  ...
  if(data->mask & CCMSG_ROUTING_PINGSESSION_MASK_SESSIONIDX)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Unable to free ping session");
      return L7_FAILURE;
    }
  }
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Unable to free ping sessions");
      return L7_FAILURE;
    }
  }
...
L7_RC_t ptin_msg_routing_tracertsession_create(msg_RoutingTracertSessionCreate* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Creating new traceroute session:");
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx    = %u",   data->sessionIdx);
  LOG_DEBUG(LOG_CTX_MSG, "  dstIpAddr     = %08X", data->dstIpAddr);
  LOG_DEBUG(LOG_CTX_MSG, "  probePerHop   = %u",   data->probePerHop);
  LOG_DEBUG(LOG_CTX_MSG, "  probeSize     = %u",   data->probeSize);
  LOG_DEBUG(LOG_CTX_MSG, "  probeInterval = %u",   data->probeInterval);
  LOG_DEBUG(LOG_CTX_MSG, "  dontFrag      = %u",   data->dontFrag);
  LOG_DEBUG(LOG_CTX_MSG, "  port          = %u",   data->port);
  LOG_DEBUG(LOG_CTX_MSG, "  maxTtl        = %u",   data->maxTtl);
  LOG_DEBUG(LOG_CTX_MSG, "  initTtl       = %u",   data->initTtl);
  LOG_DEBUG(LOG_CTX_MSG, "  maxFail       = %u",   data->maxFail);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to create new traceroute session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_tracertsession_query(msg_RoutingTracertSessionQuery* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Querying traceroute session:");
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx = %u", data->sessionIdx);
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to query traceroute session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_msg_routing_tracertsession_gethops(msg_RoutingTracertSessionHopsRequest* inBuffer, msg_RoutingTracertSessionHopsResponse* outBuffer, L7_uint32 maxEntries, L7_uint32* readEntries)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Getting traceroute hops:");
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx = %u", inBuffer->sessionIdx);
  LOG_DEBUG(LOG_CTX_MSG, "  lastIndex  = %u", inBuffer->lastIndex);
  LOG_DEBUG(LOG_CTX_MSG, "  maxEntries = %u", maxEntries);
  ...
  */
  ...
  {
    LOG_ERR(LOG_CTX_MSG, "Unable to get traceroute session hops");
    return L7_FAILURE;
  }
  LOG_DEBUG(LOG_CTX_MSG, "Successfully read %u entries", *readEntries);
...
L7_RC_t ptin_msg_routing_tracertsession_free(msg_RoutingTracertSessionFree* data)
{
  ...
  /* Output data */
  LOG_DEBUG(LOG_CTX_MSG, "Freeing traceroute session:");
  LOG_DEBUG(LOG_CTX_MSG, "  mask       = %02X", data->mask);
  LOG_DEBUG(LOG_CTX_MSG, "  sessionIdx = %u",   data->sessionIdx);
  ...
  if(data->mask & CCMSG_ROUTING_TRACEROUTESESSION_MASK_SESSIONIDX)
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Unable to free traceroute session");
      return L7_FAILURE;
    }
  }
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_MSG, "Unable to free traceroute sessions");
      return L7_FAILURE;
    }
  }
...
void ptin_msg_protection_matrix_configuration_flush_end(void)
{
  ...
  { /*Trigger the Sync of the Snooping Table*/   
    ...
    if(!ptin_fgpa_mx_is_matrixactive())//If I'm a Protection Matrix
    {
      ...
      LOG_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Request Message to ipAddr:%08X", ipAddr);

      /*Send the snoop sync request to the protection matrix */  
      if (send_ipc_message(IPC_HW_FASTPATH_PORT, ipAddr, CCMSG_MGMD_SNOOP_SYNC_REQUEST, (char *)(&snoopSyncRequest), NULL, sizeof(snoopSyncRequest), NULL) < 0)
      {
        LOG_ERR(LOG_CTX_MSG, "Failed to send Snoop Sync Request Message");
//      return;
      }         
    }
    else
    {
      LOG_NOTICE(LOG_CTX_MSG, "Not sending Snoop Sync Request Message. Since, I'm not a protection matrix");      
//    return
    }
  #endif
  }
...
{ 
  LOG_DEBUG(LOG_CTX_MSG, "Clear buffer_index: %u", buffer_index);
...
{ 
  ...
  LOG_DEBUG(LOG_CTX_MSG, "Configure monitoring: SlotId: %d, Port: %d, admin: %d", config->SlotId, config->Port, config->Admin);
...
//L7_char8 *charPtr = packageBmpStr;
  ...
  if ( msg  == L7_NULLPTR )
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [msg:%p]",msg);    
    return L7_FAILURE;
  }
#if 0      
  ...
  /*Input Parameters*/
  LOG_DEBUG(LOG_CTX_MSG, "Input Arguments [slotId:%u noOfPackages:%u packageBmpList:%s]",msg->slotId, msg->noOfPackages, packageBmpStr);
  ...
  return rc;  
#else
  LOG_ERR(LOG_CTX_IGMP, "Featured not supported in this card!");  
  return L7_NOT_SUPPORTED;  
#endif 
}
...
L7_RC_t ptin_msg_igmp_package_channels_add(msg_igmp_package_channels_t *msg, L7_uint32 noOfMessages)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [msg:%p noOfMessages:%u]",msg, noOfMessages);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  LOG_DEBUG(LOG_CTX_MSG,"Input Arguments [noOfMessages:%u]", noOfMessages);
  ...
  for (messageIterator = 0; messageIterator < noOfMessages; messageIterator++)
  {
    ...
    /*Input Parameters*/
    LOG_DEBUG(LOG_CTX_MSG, "Input Arguments [slotId:%u packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u]",
...
//L7_char8        *charPtr           = packageBmpStr;
  ...
  for (messageIterator = 0; messageIterator < noOfMessages; messageIterator++)
  {
    ...
    /* Output data */
    LOG_DEBUG(LOG_CTX_MSG, "Going to add MC client");
    LOG_DEBUG(LOG_CTX_MSG, "  MC evc_idx = %u", msg[messageIterator].evcId);
    LOG_DEBUG(LOG_CTX_MSG, "   onuId               = %u", msg[messageIterator].onuId);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.Mask         = 0x%02x", msg[messageIterator].client.mask);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.OVlan        = %u", msg[messageIterator].client.outer_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.IVlan        = %u", msg[messageIterator].client.inner_vlan);
    LOG_DEBUG(LOG_CTX_MSG, "   Client.Intf         = %u/%u", msg[messageIterator].client.intf.intf_type,msg[messageIterator].client.intf.intf_id);        
    LOG_DEBUG(LOG_CTX_MSG, "   noOfPackages        = %u ", msg[messageIterator].noOfPackages);
    LOG_DEBUG(LOG_CTX_MSG, "   PackageBmpList      = %s", packageBmpStr);

    #if PTIN_BOARD_IS_ACTIVETH   
    if (msg[messageIterator].onuId != 0)
    {
      LOG_WARNING(LOG_CTX_MSG, "   I'm an Active Ethernet Card. OnuId:%u is different from 0. Going to set it to zero", msg[messageIterator].onuId);
...
//L7_char8        *charPtr           = packageBmpStr;
  ...
  for (messageIterator = 0; messageIterator < noOfMessages; messageIterator++)
  {
    ...
    if ( msg[messageIterator].noOfPackages > 0 )
    {
      ...
      if (rc!=L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_MSG, "Error adding MC client rc:%u", rc);
        return rc;
      }
    }
  }
...
//L7_char8        *charPtr           = packageBmpStr;
  ...
  {    
    ...
    LOG_DEBUG(LOG_CTX_MSG, " UNI-IVID    = %u", ptinEvcFlow.uni_ivid);
    LOG_DEBUG(LOG_CTX_MSG, " OnuId        = %u", ptinEvcFlow.onuId);
    LOG_DEBUG(LOG_CTX_MSG, " noOfPackages       = %u", ptinEvcFlow.noOfPackages);      
    LOG_DEBUG(LOG_CTX_MSG, " packageBmpList:%s", packageBmpStr);
    ...
    if (ptinEvcFlow.noOfPackages >= 0)
    {
      ...
      {        
        LOG_ERR(LOG_CTX_MSG, "Error adding EVC# %u flow rc:%u", ptinEvcFlow.evc_idx, rc);
        return rc;
      }
    }
  }
...
L7_RC_t ptin_msg_igmp_multicast_service_add(msg_multicast_service_t *msg, L7_uint32 noOfMessages)
{
  ...
  /*Input Parameters*/
  LOG_DEBUG(LOG_CTX_MSG,"Input Arguments [msg:%p noOfMessages:%u]", msg, noOfMessages);

  for (messageIterator = 0; messageIterator < noOfMessages; messageIterator++)
  { 
    /*Input Parameters*/
    LOG_DEBUG(LOG_CTX_MSG, "Input Arguments [slotId:%u evcId:%u intf.type:%u intf.id:%u onuId:%u]",
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to convert to ptin port [slotId:%u evcId:%u intf.type:%u intf.id:%u onuId:%u]");  
      return rc;
    }
  ...
  return rc;  
#else
  LOG_ERR(LOG_CTX_IGMP, "Featured not supported on this card!");  
  return L7_NOT_SUPPORTED;  
#endif
}
