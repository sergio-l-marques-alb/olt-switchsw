...
L7_RC_t ptin_prottypeb_init(void)
{
  ...
  LOG_PT_INFO(LOG_CTX_PROTB, "Initializing interface type-b protection configurations");
...
L7_RC_t ptin_prottypeb_intf_config_set(ptin_prottypeb_intf_config_t* data)
{
  ...
  if(data==L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_PROTB, "Invalid context [data:%p]", data);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_PROTB, "Invalid intfNum[%u]", data->intfNum);
    return L7_FAILURE;
  }
  ...
  LOG_PT_DEBUG(LOG_CTX_PROTB, "Setting intfNum[%u] type-b protection configurations", intfNum);
  LOG_PT_TRACE(LOG_CTX_PROTB, "Configurations:");
  LOG_PT_TRACE(LOG_CTX_PROTB, "    slotId     : %u", data->slotId);
  LOG_PT_TRACE(LOG_CTX_PROTB, "    intfNum    : %u", data->intfNum);
  LOG_PT_TRACE(LOG_CTX_PROTB, "    intfRole   : %u", data->intfRole);
  LOG_PT_TRACE(LOG_CTX_PROTB, "    status     : %u", data->status);
  LOG_PT_TRACE(LOG_CTX_PROTB, "    pairSlotId : %u", data->pairSlotId);
  LOG_PT_TRACE(LOG_CTX_PROTB, "    pairIntfNum: %u", data->pairIntfNum);
  ...
  if (snoop_sync_flag == L7_TRUE)
  {
    ...
    {
      LOG_PT_NOTICE(LOG_CTX_MSG, "Not sending a Snoop Sync Request Message to Sync the  Snoop Entries. I'm a Working slotId/intfNum:%u/%u",prottypeb_interfaces[intfNum-1].slotId, prottypeb_interfaces[intfNum-1].intfNum);
      return L7_SUCCESS;
    }
    ...
    LOG_PT_DEBUG(LOG_CTX_MSG, "Sending a Snoop Sync Request Message to ipAddr:%08X to Sync the Snoop Entries of remote slotId/intfNum:%u/%u", ipAddr,prottypeb_interfaces[intfNum-1].pairSlotId, prottypeb_interfaces[intfNum-1].pairIntfNum);
...
L7_RC_t ptin_prottypeb_intf_config_get(L7_uint32 intfNum, ptin_prottypeb_intf_config_t* data)
{
  ...
  if(intfNum==0 || intfNum>=PTIN_SYSTEM_N_INTERF)
  {
    LOG_PT_ERR(LOG_CTX_PROTB, "Invalid intfNum[%u]", intfNum);
    return L7_FAILURE;
  }

  /* Return the configurations for the desired interface */
  LOG_PT_DEBUG(LOG_CTX_PROTB, "Getting intfNum[%u] type-b protection configurations", intfNum);
...
L7_RC_t ptin_prottypeb_intf_switch_notify(L7_uint32 intfNum, L7_uint8 status)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_PROTB, "Invalid intfNum[%u] status[%u]", intfNum, status);
    return L7_FAILURE;
  }
  ...
  if (prottypeb_interfaces[intfNum-1].intfRole != PROT_TYPEB_ROLE_NONE)
  {
    ...
    else if (status == L7_DISABLE)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_MSG, "Unable to remove protection port from MGMD Control Plane");
      }     
    }
  }
