...
L7_RC_t vlan_port_intIfNum_create(L7_uint32 intfId, L7_uint32 *pIntIfNum)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Failed to create interface %u", intfId);
    return L7_FAILURE;
  }
...
L7_RC_t vlan_port_intIfNum_delete(L7_uint32 intIfNum)
{
  ...
  while (1)
  {
    if (nimIntfQuery(&nimQueryData) != L7_SUCCESS)
    {
      ...
      LOG_PT_ERR(LOG_CTX_INTF, "nimIntfQuery failed for intIfNum %u", intIfNum);
      return L7_FAILURE;
    }
  ...
  if (detach == L7_TRUE)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "detach notification failed for intIfNum %u", intIfNum);
      return L7_FAILURE;
    }
  }                             /* detach == L7_TRUE */
  else
  {
    ...
    while (1)
    {
      ...
      {
        /* should never get here */
        LOG_PT_ERR(LOG_CTX_INTF, "nimIntfQuery failed for intIfNum %u in delete attempt", intIfNum);
        return L7_FAILURE;
      }
      ...
      switch (nimQueryData.data.state)
      {
        ...
        default:
          /* detach the interface before deleting */
          LOG_PT_ERR(LOG_CTX_INTF, "Interface should not be in state %d", nimQueryData.data.state);
          wait_for_transition = L7_FALSE;
          return L7_FAILURE;
          break;
      }                         /* switch */
    ...
    if (nimEventIntfNotify(eventInfo, &handle) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_INTF, "delete notification failed for intIfNum %u", intIfNum);
      return L7_FAILURE;
    }
  }                             /* detach != L7_TRUE */
...
void vlanportNimEventCompletionCallback(NIM_NOTIFY_CB_INFO_t retVal)
{
  ...
  if (retVal.response.rc != L7_SUCCESS)
  {
    ...
    LOG_PT_ERR(LOG_CTX_INTF, "notification failed for event(%d), intIfNum(%u), reason(%d)",
  ...
  else
  {
    ...
    switch (retVal.event)
    {
      ...
      case L7_ATTACH:
        LOG_PT_TRACE(LOG_CTX_INTF, "intIfNum %u attached!", retVal.intIfNum);
        break;
    ...
    case L7_DETACH:
      ...
      case L7_DELETE:
        LOG_PT_TRACE(LOG_CTX_INTF, "intIfNum %u deleted!", retVal.intIfNum);
        break;
