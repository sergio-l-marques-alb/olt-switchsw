...
L7_RC_t ptin_xlate_dnx_add(L7_uint32 lif, L7_uint16 newOuterVlanId, L7_uint16 newInnerVlanId, L7_int newOuterPrio, L7_int newInnerPrio, L7_BOOL is_ingress)
{
  ...
  if (ptin_debug_xlate)
    PT_LOG_TRACE(LOG_CTX_XLATE, "Finished: LIF=0x%x,  rc=%d", lif, rc);
...
L7_RC_t ptin_xlate_dnx_delete(L7_uint32  lif, L7_BOOL is_ingress)
{
  ...
  if (ptin_debug_xlate)
    PT_LOG_TRACE(LOG_CTX_XLATE, "lif=%u", lif);
...
L7_RC_t ptin_xlate_PVID_set(L7_uint32 intIfNum, L7_uint16 vlanId)
{
  ...
  if (ptin_xlate_ingress_get(intIfNum, vlanId, PTIN_XLATE_NOT_DEFINED, &int_defVid, &int_innerVid) != L7_SUCCESS)
  {
    int_defVid = 1;
    if (ptin_debug_xlate)
      PT_LOG_WARN(LOG_CTX_INTF, "Could not acquire internal vlan associated to intIfNum %u/vlan %u... Assuming 1", intIfNum, vlanId);
  }
  else
  {
    PT_LOG_TRACE(LOG_CTX_INTF, "Converted VID %u to internal %u+%u (intIfNum %u)", vlanId, int_defVid, int_innerVid, intIfNum);
  }
  ...
  if (pvid_original >= 1 && pvid_original <= 4095)
  {
    ...
    {
      int_defVid = 1;
      PT_LOG_WARN(LOG_CTX_INTF, "Could not acquire internal vlan associated to intIfNum %u/vlan %u... Assuming 1", intIfNum, pvid_original);
    }
  }
  else
  {
...
static L7_RC_t ptin_xlate_operation(L7_int operation, L7_uint32 intIfNum, ptin_vlanXlate_t *xlate)
{
  ...
  if (ptin_debug_xlate)
    PT_LOG_TRACE(LOG_CTX_XLATE, "oper=%d: intIfNum=%u/pgroup=%u/LIF=0x%x, Vid=%u+%u, newVid=%u.%u+%u.%u (action %u.%u+%u.%u), rem_VLANs=%u",
              operation, intIfNum, xlate->portgroup, xlate->lif,
