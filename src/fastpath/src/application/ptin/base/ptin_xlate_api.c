...
L7_RC_t ptin_xlate_ingress_add_2( L7_uint32 intIfNum, L7_uint16 outerVlanId, L7_uint16 innerVlanId, L7_uint16 newOuterVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u, outerVlanId=%u, innerVlanId=%u, newOuterVlanId=%u",
              intIfNum, outerVlanId, innerVlanId, newOuterVlanId);

  /* Validate arguments */
  if (outerVlanId>4095 || innerVlanId>4095 || newOuterVlanId>4095)
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid arguments");
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: rc=%d", rc);
...
L7_RC_t ptin_xlate_egress_add_2( L7_uint32 intIfNum, L7_uint16 outerVlanId, L7_uint16 newOuterVlanId, L7_uint16 newInnerVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u, outerVlanId=%u, newOuterVlanId=%u newInnerVlanId=%u",
              intIfNum, outerVlanId, newOuterVlanId, newInnerVlanId);

  /* Get class id */
  if (xlate_portgroup_from_intf(intIfNum, &portgroup)!=L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR getting class id");
    return L7_FAILURE;
  }

  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "portgroup=%u, outerVlanId=%u, newOuterVlanId=%u, newInnerVlanId=%u",
...
L7_RC_t ptin_xlate_init(void)
{
  ...
  LOG_PT_TRACE(LOG_CTX_XLATE, "Initializing xlate module...");
  ...
  LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: rc1=%d, rc2=%d, rc3=%d", rc1, rc2, rc3);  
...
L7_RC_t ptin_xlate_portgroup_get(L7_uint32 intIfNum, L7_uint32 *portgroup)
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u", intIfNum);

  /* Validate arguments */
  if ( ptin_intf_intIfNum2port(intIfNum, &ptin_port)!=L7_SUCCESS || ptin_port>=ptin_sys_number_of_ports)
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid interface");
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: rc=0");
...
L7_RC_t ptin_xlate_portgroup_set(L7_uint32 intIfNum, L7_uint32 portgroup)
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u portgroup=%u", intIfNum, portgroup);
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Unable to find interface type");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid interface type");
    return L7_FAILURE;
  }
  ...
  if (intf_members_n==0)
  {
    LOG_PT_WARN(LOG_CTX_XLATE, " WARN: No interfaces to be processed");
    return L7_SUCCESS;
  }
...
L7_RC_t ptin_xlate_portgroup_reset_all(void)
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Resetting class ids...");
                                ...
                                L7_uint16 *newOuterVlanId, L7_uint16 *newInnerVlanId)
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u, outerVlanId=%u, innerVlanId=%u",
  ...
  {
    if (ptin_debug_xlate)
      LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Entry not found");
    rc = L7_FAILURE;
  }
  #if 0
  /* Define structure */
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: newOVid=%u, newIVid=%u, rc=%d", newOuterVid, newInnerVid, rc);
                                             ...
                                             L7_uint16 newOuterVlanId, L7_uint16 newInnerVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u, newOuterVlanId=%u, newInnerVlanId=%u",
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: oldOVlan=%u, oldIVid=%u rc=%d", oldOuterVid, oldInnerVid, rc);
                                ...
                                L7_int newOuterPrio, L7_int newInnerPrio )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "intIfNum=%u, outerVlanId=%u, innerVlanId=%u, newOuterVlanId=%u, newInnerVlanId=%u, newOuterPrio=%u, newInnerPrio=%u",
  ...
  if (xlate_table_pvid[intIfNum] == outerVlanId && (innerVlanId == 0 || innerVlanId >= 4096))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_INTF, "Error applying defVID %u for intIfNum %u", newOuterVlanId, intIfNum);
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_INTF, "defVID %u for intIfNum %u applied", newOuterVlanId, intIfNum);
    }
    /* Configure defVID using a VCAP rule */
    if (ptin_intf_vcap_defvid(intIfNum, newOuterVlanId, newInnerVlanId) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_XLATE,"Error configuring VCAP defVID %u+%u for intIfNum %u", newOuterVlanId, newInnerVlanId, intIfNum);
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_XLATE,"VCAP defVID %u+%u for intIfNum %u configured", newOuterVlanId, newInnerVlanId, intIfNum);
    }
  }
...
L7_RC_t ptin_xlate_ingress_delete( L7_uint32 intIfNum, L7_uint16 outerVlanId, L7_uint16 innerVlanId )
{
  ...
  if (xlate_table_pvid[intIfNum] == outerVlanId && (innerVlanId == 0 || innerVlanId >= 4096))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_XLATE,"Error removing VCAP rule from intIfNum %u", intIfNum);
    }
    else
    {
      LOG_PT_TRACE(LOG_CTX_XLATE,"VCAP rule removed from intIfNum %u", intIfNum);
    }
  }
...
L7_RC_t ptin_xlate_ingress_delete_all( void )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Beginning...");
                               ...
                               L7_uint16 *newOuterVlanId, L7_uint16 *newInnerVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: newOVid=%um newIVid=%u, rc=%d", newOuterVid, newInnerVid, rc);
                               ...
                               L7_int newOuterPrio, L7_int newInnerPrio)
{
  ...
  if (xlate.remove_VLANs)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_EVC, "Error setting intIfNum# %u internal VLAN %u as UNtagged (rc=%d)", intIfNum, outerVlanId);
      return L7_FAILURE;
    }
  }
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "Finished: intIfNum=%u, class_id=%u,  rc=%d", intIfNum, class_id, rc);
                                         ...
                                         L7_uint16 *newOuterVlanId, L7_uint16 *newInnerVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "portgroup=%u, outerVlanId=%u, innerVlanId=%u",
                                                      ...
                                                      L7_uint16 newOuterVlanId, L7_uint16 newInnerVlanId )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "portgroup=%u, newOuterVlanId=%u, newInnerVlanId=%u",
                                         ...
                                         L7_uint16 newOuterVlanId, L7_uint16 newInnerVlanId, L7_BOOL remove_VLANs )
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "portgroup=%u, outerVlanId=%u, innerVlanId=%u, newOuterVlanId=%u, newInnerVlanId=%u",
...
L7_RC_t ptin_xlate_PVID_set(L7_uint32 intIfNum, L7_uint16 vlanId)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, "Invalid intIfNum %d", intIfNum);
    return L7_FAILURE;
  }
  if (vlanId == 0 || vlanId >= 4096)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "Invalid VID %u", vlanId);
    return L7_FAILURE;
  }
  ...
  if (ptin_intf_intIfNum2port(intIfNum, &ptin_port) != L7_SUCCESS)
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "Invalid intIfNum %u", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "Nothing to be done for intIfNum %u: VID %u is the same", intIfNum, vlanId);
    return L7_SUCCESS;
  }
  ...
  if (ptin_xlate_ingress_get(intIfNum, vlanId, PTIN_XLATE_NOT_DEFINED, &int_defVid, &int_innerVid) != L7_SUCCESS)
  {
    int_defVid = 1;
    if (ptin_debug_xlate)
      LOG_PT_WARN(LOG_CTX_INTF, "Could not acquire internal vlan associated to intIfNum %u/vlan %u... Assuming 1", intIfNum, vlanId);
  }
  else
  {
    LOG_PT_TRACE(LOG_CTX_INTF, "Converted VID %u to internal %u+%u (intIfNum %u)", vlanId, int_defVid, int_innerVid, intIfNum);
  }

  /* Configure defVID using a VCAP rule */
  if (ptin_intf_vcap_defvid(intIfNum, int_defVid, int_innerVid) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_XLATE,"Error configuring VCAP defVID %u+%u for intIfNum %u", int_defVid, int_innerVid, intIfNum);
  }
  else
  {
    LOG_PT_TRACE(LOG_CTX_XLATE,"VCAP defVID %u+%u for intIfNum %u configured", int_defVid, int_innerVid, intIfNum);
  }

  /* Set default VID */
  if (usmDbQportsPVIDSet(unit, intIfNum, int_defVid) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error applying VID %u", int_defVid);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_INTF, "Error setting intIfNum %u of VID %u as untagged", intIfNum, int_defVid);
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_XLATE, "PVID %u applied to intIfNum %u", vlanId, intIfNum);
  ...
  if (pvid_original >= 1 && pvid_original <= 4095)
  {
    ...
    if (ptin_xlate_ingress_get(intIfNum, pvid_original, PTIN_XLATE_NOT_DEFINED, &int_defVid, &int_innerVid) == L7_SUCCESS)
    {
      ...
      {
        LOG_PT_TRACE(LOG_CTX_XLATE, "Success restoring intIfNum %u of VLAN %u to tagged type", intIfNum, vlanId);
      }
      else
      {
        LOG_PT_WARN(LOG_CTX_INTF, "Error restoring intIfNum %u of VID %u to tagged type", intIfNum, int_defVid);
      }
    }
    else
    {
      int_defVid = 1;
      LOG_PT_WARN(LOG_CTX_INTF, "Could not acquire internal vlan associated to intIfNum %u/vlan %u... Assuming 1", intIfNum, pvid_original);
    }
  }
  else
  {
  ...
  if (xlate_portgroup_from_intf(intIfNum, &class_id) == L7_SUCCESS)
  {
    ...
    while ((avl_infoData = avlSearchLVL7(&database_xlate[PTIN_XLATE_STAGE_EGRESS].avlTree, (void *)&avl_key, AVL_NEXT)) != L7_NULLPTR)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_XLATE, "Error redefining egress xlate entry for pgroup=%u, OVid=%u, IVid=%u (remove_VLANs=%u)",
                xlate.portgroup, xlate.outerVlan, xlate.innerVlan, xlate.remove_VLANs);
      }
      else
      {
        LOG_PT_TRACE(LOG_CTX_XLATE, "Egress xlate entry for pgroup=%u, OVid=%u, IVid=%u redefined (remove_VLANs=%u)",
                  xlate.portgroup, xlate.outerVlan, xlate.innerVlan, xlate.remove_VLANs);
      }
    }

    LOG_PT_TRACE(LOG_CTX_XLATE, "Egress translations redefined");
  }
...
static L7_RC_t ptin_xlate_operation(L7_int operation, L7_uint32 intIfNum, ptin_vlanXlate_t *xlate)
{
  ...
  if (ptin_debug_xlate)
    LOG_PT_TRACE(LOG_CTX_XLATE, "oper=%d: intIfNum=%u/pgroup=%u, Vid=%u+%u, newVid=%u.%u+%u.%u (action %u.%u+%u.%u), rem_VLANs=%u",
...
static L7_RC_t xlate_database_init(void)
{
  ...
  for (stage=PTIN_XLATE_STAGE_INGRESS; stage<=PTIN_XLATE_STAGE_EGRESS; stage++)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_IGMP,"Error allocating data for XLATE AVL Trees");
      return L7_FAILURE;
    }
  ...
  for (stage=PTIN_XLATE_STAGE_INGRESS; stage<=PTIN_XLATE_STAGE_EGRESS; stage++)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_IGMP,"Error allocating data for Inverted XLATE AVL Trees\n");
      return L7_FAILURE;
    }
...
static L7_RC_t xlate_database_newVlan_get(L7_uint32 intIfNum, ptin_vlanXlate_t *vlanXlate_data)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid arguments (stage=%d, intIfNum=%u, ptin_port=%u, oVlan=%u)", vlanXlate_data->stage, intIfNum, ptin_port, vlanXlate_data->outerVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_xlate)
      LOG_PT_ERR(LOG_CTX_XLATE, " Entry does not exist"); 
    return L7_FAILURE;
  }
...
static L7_RC_t xlate_database_oldVlan_get(L7_uint32 intIfNum, ptin_vlanXlate_t *vlanXlate_data)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid arguments (stage=%d, intIfNum=%u, ptin_port=%u, newOVlan=%u)", vlanXlate_data->stage, intIfNum, ptin_port, vlanXlate_data->outerVlan_new);
    return L7_FAILURE;
  }
...
static L7_RC_t xlate_database_store(L7_uint32 intIfNum, const ptin_vlanXlate_t *vlanXlate_data)
{
  ...
  if ( ( vlanXlate_data->stage != PTIN_XLATE_STAGE_INGRESS && vlanXlate_data->stage != PTIN_XLATE_STAGE_EGRESS ) ||
       ...
       ( vlanXlate_data->outerVlan > 4095) )
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid arguments (stage=%d, intIfNum=%u, ptin_port=%u, oVlan=%u.%u, newOVlan=%u.%u)",
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: XLATE AVL tree is full (stage=%u, %u/%u entries)",
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR inserting new node");
    return L7_FAILURE;
  }
  ...
  if (avl_infoData == L7_NULLPTR)
  {
    ...
    if (avl_infoData == L7_NULLPTR)
    {
      LOG_PT_ERR(LOG_CTX_XLATE, " Strange... item looks to not be created");
      return L7_FAILURE;
    }
    /* One more entry */
...
static L7_RC_t xlate_database_clear(L7_uint32 intIfNum, const ptin_vlanXlate_t *vlanXlate_data)
{
  ...
  if (avl_infoData == L7_NULLPTR)
  {
    LOG_PT_WARN(LOG_CTX_XLATE, " Entry does not exist (ptin_port=%u, pgroup=%u, ovlan=%u, ivlan=%u)",
                avl_key.ptin_port, avl_key.portGroup, avl_key.outerVid, avl_key.innerVid);
    return L7_SUCCESS;
  }

  LOG_PT_TRACE(LOG_CTX_XLATE, " Found entry: ptin_port=%u, pgroup=%u, ovlan=%u, ivlan=%u -> oVlan=%u.%u (action %u.%u), iVlan=%u.%u (action %u.%u)",
            avl_key.ptin_port, avl_key.portGroup, avl_key.outerVid, avl_key.innerVid,
  ...
  if (avl_infoData == L7_NULLPTR)
  {
    LOG_PT_WARN(LOG_CTX_XLATE, " Failed Inverted Entry remotion (ptin_port=%u, pgroup=%u, ovlan=%u, ivlan=%u)",
...
static L7_RC_t xlate_database_clear_all(ptin_vlanXlate_stage_enum stage)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " ERROR: Invalid arguments (stage=%d)", stage);
    return L7_FAILURE;
  }
...
static L7_RC_t xlate_portgroup_from_intf(L7_uint32 intIfNum, L7_uint32 *portgroup)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " Error getting USP (intIfNum=%u)", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " Invalid interface type (usp={%d, %d, %d})", usp.unit, usp.slot, usp.port);
    return L7_FAILURE;
  }
  ...
  if ( class_id == 0 )
  {
    LOG_PT_ERR(LOG_CTX_XLATE, " Class id is null");
    return L7_FAILURE;
  }
