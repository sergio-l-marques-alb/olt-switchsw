...
L7_RC_t ptin_packet_init(void)
{
  ...
  {
    LOG_PT_FATAL(LOG_CTX_PACKET,"PTIN packet queue creation error.");
    return L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_PACKET,"PTIN packet queue created.");
  ...
  {
    LOG_PT_FATAL(LOG_CTX_PACKET, "Could not create task ptin_packet_task");
    return L7_FAILURE;
  }
  LOG_PT_TRACE(LOG_CTX_PACKET,"Task ptin_packet_task created");

  if (osapiWaitForTaskInit (L7_PTIN_PACKET_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_PT_FATAL(LOG_CTX_PACKET,"Unable to initialize ptin_packet_task()\n");
    return(L7_FAILURE);
  }
  LOG_PT_TRACE(LOG_CTX_PACKET,"Task ptin_packet_task initialized");
  ...
  {
    LOG_PT_ERR(LOG_CTX_PACKET, "Cannot register ptinBcastPduReceive callback!");
    return L7_FAILURE;
  }
  LOG_PT_INFO(LOG_CTX_PACKET, "ptinBcastPduReceive registered!");
...
L7_RC_t ptin_packet_deinit(void)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_PACKET, "Cannot unregister ptinBcastPduReceive callback!");
    return L7_FAILURE;
  }
  LOG_PT_INFO(LOG_CTX_PACKET, "ptinBcastPduReceive unregistered!");
  ...
  LOG_PT_INFO(LOG_CTX_PACKET, "PTin packet deinit OK");
...
L7_RC_t ptinMacBcastRecv(L7_netBufHandle bufHandle, sysnet_pdu_info_t *pduInfo)
{
  ...
  if (ptin_packet_debug_enable)
    LOG_PT_TRACE(LOG_CTX_PACKET,"Packet received at intIfNum=%u with vlanId=%u and innerVlanId=%u",
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"Invalid vlanId %u",vlanId);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"Invalid innerVlanId %u",innerVlanId);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"Packet is not broadcast");
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"intIfNum %u and vlan %u does not belong to any valid EVC/interface");
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"Failed message sending to ptin_packet queue");
    return L7_FAILURE;
  }
...
void ptin_packet_task(void)
{
  ...
  LOG_PT_INFO(LOG_CTX_PACKET,"PTin packet process task started");
  ...
  LOG_PT_INFO(LOG_CTX_PACKET,"PTin packet task ready to process events");
  ...
  while (1)
  {
    ...
    if (status == L7_SUCCESS)
    {
      if ( msg.msgId == PTIN_PACKET_MESSAGE_ID )
      {
        ...
        {
          if (ptin_packet_debug_enable)
            LOG_PT_ERR(LOG_CTX_PACKET,"Error processing message");
        }
      }
      else
      {
        if (ptin_packet_debug_enable)
          LOG_PT_ERR(LOG_CTX_PACKET,"Message id is unknown (%u)",msg.msgId);
      }
    }
    else
    {
      if (ptin_packet_debug_enable)
        LOG_PT_ERR(LOG_CTX_PACKET,"Failed packet reception from ptin_packet queue (status = %d)",status);
    }
  }
}
...
L7_RC_t ptinMacBcastProcess( ptin_PDU_Msg_t *pktMsg )
{
  ...
  {
    if (ptin_packet_debug_enable)
      LOG_PT_ERR(LOG_CTX_PACKET,"Error getting list of leaf interfaces for vlanId=%u", vlanId);
    return L7_FAILURE;
  }
  ...
  for (intf = 1; intf <= L7_MAX_INTERFACE_COUNT; intf++)
  {
    ...
    {
      if (ptin_packet_debug_enable)
        LOG_PT_ERR(LOG_CTX_PACKET,"Error getting list of vlans for vlanId=%u, innervlan=%u, intIfNum=%u", vlanId, innerVlanId, intf);
      continue;
    }

    if (ptin_packet_debug_enable)
      LOG_PT_TRACE(LOG_CTX_PACKET,"%u vlans obtained for intIfNum %u, vlanId=%u, cvlan=%u", number_of_vlans, intf, vlanId, innerVlanId);

    /* Transmit packet to all vlans of this interface */
    for (i=0; i<number_of_vlans; i++)
    {
      if (ptin_packet_debug_enable)
        LOG_PT_TRACE(LOG_CTX_PACKET,"Sending packet to intIfNum %u, with oVlan=%u", intf, flood_vlan[i]);
                      ...
                      L7_uint32 payloadLen)
{
  ...
  if (ptin_packet_debug_enable)
    LOG_PT_TRACE(LOG_CTX_PACKET,"Packet transmited to intIfNum=%u", intIfNum);
