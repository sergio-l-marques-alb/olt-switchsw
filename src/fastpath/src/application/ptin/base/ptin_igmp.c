...
static L7_RC_t ptin_igmp_multicast_package_init(void)
{
  ...
  /*Initialize Channel Pool*/
  {
    ...
    for (iterator = 0; iterator < PTIN_SYSTEM_IGMP_MAXPACKAGES; iterator++)
    {
      ...
      {
        LOG_FATAL(LOG_CTX_IGMP,"Failed to Initialize Queue on packageId:%u", iterator );
        return rc;
      }
    }
    ...
    for ( iterator = 0; iterator <PTIN_SYSTEM_IGMP_MAXPACKAGES_CHANNEL_ENTRIES; iterator++)
    {
      ...
      {
        LOG_FATAL(LOG_CTX_IGMP,"Failed to Add Queue on packageId:%u", iterator );
        return rc;
      }
    }
  }
...
//  dl_queue_init(&avl_infoData->queuePackage);
    ...
    {
      LOG_FATAL(LOG_CTX_IGMP,"Failed to Initialize Queue of packageId");
      return rc;
    }
    ...
    for ( iterator = 0; iterator <PTIN_SYSTEM_IGMP_MAXPACKAGES_CHANNEL_ENTRIES; iterator++)
    {
      ...
      {
        LOG_FATAL(LOG_CTX_IGMP,"Failed to Add Element to Queue: packageId:%u", iterator);
        return rc;
      }
    }
  }
...
L7_RC_t ptin_igmp_proxy_init(void)
{
  ...
  if (igmp_sem == NULL)
  {
    LOG_TRACE(LOG_CTX_IGMP, "Error creating a mutex for IGMP module");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error allocating data for IGMP Snapshot AVL Trees\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Unable to clear clientGroupSnapshot avlTree");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error allocating data for IGMP AVL Trees\n");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error allocating data for IGMP Pairing AVL Trees\n");
    return L7_FAILURE;
  }
  ...
  LOG_INFO(LOG_CTX_IGMP,"Going to initialize Multicast Packages.");
  if (ptin_igmp_multicast_package_init() != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to Initialize Multicast Packages!");
    return L7_FAILURE;
  }
  LOG_INFO(LOG_CTX_IGMP,"Initialized Multicast Packages.");
#endif

  ptin_igmp_stats_sem = osapiSemaBCreate(OSAPI_SEM_Q_FIFO, OSAPI_SEM_FULL);
  if (ptin_igmp_stats_sem == L7_NULLPTR)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_igmp_stats_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_igmp_clients_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_igmp_clients_snapshot_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to initialize timers!");
    return L7_FAILURE;
  }
#endif

  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInstances)             = %u", sizeof(igmpInstances));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientGroups)          = %u", sizeof(igmpGroupClients));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientGroups)*         = %u",
  ...
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpSnapshotClientGroups)  = %u", sizeof(igmpSnapshotClientGroups));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpSnapshotClientGroups)* = %u", 
  ...
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClients_unified)       = %u", sizeof(igmpDeviceClients));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClients_unified)*      = %u",
...
#ifdef IGMPASSOC_MULTI_MC_SUPPORTED
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPairDB)                = %u", sizeof(channelDB));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPairDB)*               = %u",
           sizeof(avlTree_t) + sizeof(avlTreeTables_t)*PTIN_IGMP_CHANNELS_MAX + sizeof(ptinIgmpChannelInfoData_t)*PTIN_IGMP_CHANNELS_MAX);
#endif
  LOG_INFO(LOG_CTX_IGMP,"sizeof(global_stats_intf)         = %u", sizeof(global_stats_intf));

  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpProxyCfg   )         = %u", sizeof(igmpProxyCfg));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientsIntf)         = %u", sizeof(igmpClientsIntf));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpRoutersIntf)         = %u", sizeof(igmpRoutersIntf));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInst_fromRouterVlan) = %u", sizeof(igmpInst_fromRouterVlan));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInst_fromUCVlan)     = %u", sizeof(igmpInst_fromUCVlan));

#if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
  LOG_INFO(LOG_CTX_IGMP,"sizeof(ptinIgmpChannelBandwidthCache) = %u", sizeof(ptinIgmpChannelBandwidthCache));
#if 0
  LOG_INFO(LOG_CTX_IGMP,"sizeof(clientGroupLookUpTable)        = %u", sizeof(clientGroupLookUpTable));
#endif
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPortAdmissionControl)      = %u", sizeof(igmpPortAdmissionControl));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpMulticastAdmissionControl) = %u", sizeof(igmpMulticastAdmissionControl));
#endif

  LOG_INFO(LOG_CTX_IGMP,"sizeof(pool_device_client_id)      = %u", sizeof(pool_device_client_id));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(queue_free_device_clientIdx)= %u", sizeof(queue_free_device_client_id));

  LOG_INFO(LOG_CTX_IGMP,"sizeof(pool_group_client_id)      = %u", sizeof(pool_group_client_id));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(queue_free_group_clientIdx)= %u", sizeof(queue_free_group_client_id));

  #ifdef IGMPASSOC_MULTI_MC_SUPPORTED
  LOG_INFO(LOG_CTX_IGMP,"sizeof(channelIdPoolEntry)      = %u", sizeof(channelIdPoolEntry));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(channelPoolEntry)      = %u", sizeof(channelPoolEntry));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(packageIdPoolEntry)      = %u", sizeof(packageIdPoolEntry));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(packagePoolEntry)      = %u", sizeof(packagePoolEntry));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(multicastServices)      = %u", sizeof(multicastServices));
  LOG_INFO(LOG_CTX_IGMP,"sizeof(multicastServiceId)      = %u", sizeof(multicastServiceId));
  #endif

  LOG_INFO(LOG_CTX_IGMP, "IGMP init OK");
...
L7_RC_t ptin_igmp_proxy_deinit(void)
{
  ...
  LOG_INFO(LOG_CTX_IGMP, "IGMP deinit OK");
...
L7_RC_t ptin_igmp_proxy_defaultcfg_load(void)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP, "IGMP default config failed to be load");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_proxy_config_set__snooping_old(ptin_IgmpProxyCfg_t *igmpProxy)
{
  ...
  if (rc == L7_FAILURE)
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to get IGMP mutex");
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP, "Applying new config to IGMP Proxy...");

  /*Validate Mask & Flags*/
  {
    if (igmpProxy->host.mask & PTIN_IGMP_HOST_MASK_RV && !(igmpProxy->querier.mask & PTIN_IGMP_QUERIER_MASK_RV))
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Querier Robustness Mask Not Set: 0x%08X", igmpProxy->querier.mask);
          ...
          igmpProxy->querier.mask & PTIN_IGMP_QUERIER_MASK_RV))
    {
      ...
      {
        LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto GMI Flag Not Set: 0x%08X", igmpProxy->querier.flags);
        igmpProxy->querier.flags |= PTIN_IGMP_QUERIER_MASK_AUTO_GMI;
      }

      if (!(igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_OQPI))
      {
        LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto OQPI Flag Not Set: 0x%08X", igmpProxy->querier.flags);
        igmpProxy->querier.flags |= PTIN_IGMP_QUERIER_MASK_AUTO_OQPI;
      }

      if (!(igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_OHPT))
      {
        LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto OHPT Flag Not Set: 0x%08X", igmpProxy->querier.flags);
        igmpProxy->querier.flags |= PTIN_IGMP_QUERIER_MASK_AUTO_OHPT;
      }

      if (!(igmpProxy->host.flags & PTIN_IGMP_HOST_MASK_OQPT))
      {
        LOG_NOTICE(LOG_CTX_IGMP, "  Host Auto OQPT Flag Not Set: 0x%08X", igmpProxy->host.flags);
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto SQI Mask Set: 0x%08X", igmpProxy->querier.flags); 
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto SQC Mask Set: 0x%08X", igmpProxy->querier.flags);    
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Querier Auto LMQC Mask Set: 0x%08X", igmpProxy->querier.flags);    
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Querier Mask Not Set: 0x%08X", igmpProxy->mask);
    ...
    {
      LOG_NOTICE(LOG_CTX_IGMP, "  Host Mask Not Set: 0x%08X", igmpProxy->mask);
  ...
  {
    LOG_TRACE(LOG_CTX_IGMP, "  Admin:                                   %s", igmpProxy->admin == 0 ? "OFF":"ON");
  }
  ...
  {
    igmpProxyCfg.networkVersion = igmpProxy->networkVersion;
    LOG_TRACE(LOG_CTX_IGMP, "  IGMP Network Version:                     %u", igmpProxyCfg.networkVersion);
  }
  ...
  {
    igmpProxyCfg.clientVersion = igmpProxy->clientVersion;
    LOG_TRACE(LOG_CTX_IGMP, "  IGMP Client Version:                      %u", igmpProxyCfg.clientVersion);
  }
  ...
  {
    igmpProxyCfg.ipv4_addr.s_addr = igmpProxy->ipv4_addr.s_addr;
    LOG_TRACE(LOG_CTX_IGMP, "  IPv4:                                    %u.%u.%u.%u",
  ...
  {
    igmpProxyCfg.igmp_cos = igmpProxy->igmp_cos;
    LOG_TRACE(LOG_CTX_IGMP, "  IGMP COS:                                %u", igmpProxyCfg.igmp_cos);
  }
  ...
  {
    igmpProxyCfg.fast_leave = igmpProxy->fast_leave;
    LOG_TRACE(LOG_CTX_IGMP, "  Fast-Leave mode:                         %s", igmpProxyCfg.fast_leave != 0 ? "ON":"OFF");
  }
  ...
  {
    igmpProxyCfg.bandwidthControl = igmpProxy->bandwidthControl;
    LOG_TRACE(LOG_CTX_IGMP, "  Bandwidth Control mode:                         %s", igmpProxyCfg.bandwidthControl != 0 ? "ON":"OFF");
  }
  ...
  {
    igmpProxyCfg.channelsControl = igmpProxy->channelsControl;
    LOG_TRACE(LOG_CTX_IGMP, "  Channels Control mode:                         %s", igmpProxyCfg.channelsControl != 0 ? "ON":"OFF");
  }

  /* *******************
   * IGMP Querier config
   * *******************/
  LOG_TRACE(LOG_CTX_IGMP, "  Querier config:");
  if (igmpProxy->mask & PTIN_IGMP_PROXY_MASK_QUERIER)
  {
        ...
        && igmpProxyCfg.querier.robustness != igmpProxy->querier.robustness && igmpProxy->querier.robustness>=PTIN_MIN_ROBUSTNESS_VARIABLE && igmpProxy->querier.robustness<=PTIN_MAX_ROBUSTNESS_VARIABLE)
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Robustness:                            %u", igmpProxyCfg.querier.robustness);
    }
    ...
    {
      igmpProxyCfg.querier.query_interval = igmpProxy->querier.query_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Query Interval:                        %u (s)", igmpProxyCfg.querier.query_interval);
    }
    ...
    {
      igmpProxyCfg.querier.query_response_interval = igmpProxy->querier.query_response_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Query Response Interval:               %u (1/10s)", igmpProxyCfg.querier.query_response_interval);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_GMI)
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Group Membership Interval (AUTO):      %u (s)", igmpProxyCfg.querier.group_membership_interval);
    }
    ...
    {
      igmpProxyCfg.querier.group_membership_interval = igmpProxy->querier.group_membership_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Group Membership Interval:             %u (s)", igmpProxyCfg.querier.group_membership_interval);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_OQPI)
    {
      igmpProxyCfg.querier.other_querier_present_interval = PTIN_IGMP_AUTO_OQPI(igmpProxyCfg.querier.robustness,
                                                                                igmpProxyCfg.querier.query_interval,
                                                                                igmpProxyCfg.querier.query_response_interval);
      LOG_TRACE(LOG_CTX_IGMP, "    Other Querier Present Interval (AUTO): %u (s)", igmpProxyCfg.querier.other_querier_present_interval);
    }
    ...
    {
      igmpProxyCfg.querier.other_querier_present_interval = igmpProxy->querier.other_querier_present_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Other Querier Present Interval:        %u (s)", igmpProxyCfg.querier.other_querier_present_interval);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_SQI)
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Startup Query Interval (AUTO):         %u (s)", igmpProxyCfg.querier.startup_query_interval);
    }
    ...
    {
      igmpProxyCfg.querier.startup_query_interval = igmpProxy->querier.startup_query_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Startup Query Interval:                %u (s)", igmpProxyCfg.querier.startup_query_interval);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_SQC)
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Startup Query Count (AUTO):            %u (s)", igmpProxyCfg.querier.startup_query_count);
    }
    ...
    {
      igmpProxyCfg.querier.startup_query_count = igmpProxy->querier.startup_query_count;
      LOG_TRACE(LOG_CTX_IGMP, "    Startup Query Count:                   %u (s)", igmpProxyCfg.querier.startup_query_count);
    }
    ...
    {
      igmpProxyCfg.querier.last_member_query_interval = igmpProxy->querier.last_member_query_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Last Member Query Interval:            %u (1/10s)", igmpProxyCfg.querier.last_member_query_interval);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_LMQC)
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Last Member Query Count (AUTO):        %u (s)", igmpProxyCfg.querier.last_member_query_count);
    }
    ...
    {
      igmpProxyCfg.querier.last_member_query_count = igmpProxy->querier.last_member_query_count;
      LOG_TRACE(LOG_CTX_IGMP, "    Last Member Query Count:               %u (s)", igmpProxyCfg.querier.last_member_query_count);
    }
    ...
    if (igmpProxy->querier.flags & PTIN_IGMP_QUERIER_MASK_AUTO_OHPT)
    {
      igmpProxyCfg.querier.older_host_present_timeout = PTIN_IGMP_AUTO_OHPT(igmpProxyCfg.querier.robustness,
                                                                            igmpProxyCfg.querier.query_interval,
                                                                            igmpProxyCfg.querier.query_response_interval);
      LOG_TRACE(LOG_CTX_IGMP, "    Older Host Present Timeout (AUTO):     %u (s)", igmpProxyCfg.querier.older_host_present_timeout);
    }
    else if (igmpProxy->querier.mask & PTIN_IGMP_QUERIER_MASK_LMQC
             && igmpProxyCfg.querier.older_host_present_timeout != igmpProxy->querier.older_host_present_timeout)
    {
      igmpProxyCfg.querier.older_host_present_timeout = igmpProxy->querier.older_host_present_timeout;
      LOG_TRACE(LOG_CTX_IGMP, "    Older Host Present Timeout:            %u (s)", igmpProxyCfg.querier.older_host_present_timeout);
    }
   ...
   * IGMP Host config
   * *******************/
  LOG_TRACE(LOG_CTX_IGMP, "  Host config:");
  if (igmpProxy->mask & PTIN_IGMP_PROXY_MASK_HOST)
  {
    ...
    {
      igmpProxyCfg.host.robustness = igmpProxy->host.robustness;
      LOG_TRACE(LOG_CTX_IGMP, "    Host Robustness:                            %u", igmpProxyCfg.host.robustness);
    }
    ...
    {
      igmpProxyCfg.host.unsolicited_report_interval = igmpProxy->host.unsolicited_report_interval;
      LOG_TRACE(LOG_CTX_IGMP, "    Unsolicited Report Interval:           %u (s)", igmpProxyCfg.host.unsolicited_report_interval);
    }
    ...
    if (igmpProxy->host.flags & PTIN_IGMP_HOST_MASK_OQPT)
    {
      igmpProxyCfg.host.older_querier_present_timeout = PTIN_IGMP_AUTO_OQPT(igmpProxyCfg.host.robustness,
                                                                            igmpProxyCfg.querier.query_interval,
                                                                            igmpProxyCfg.querier.query_response_interval);
      LOG_TRACE(LOG_CTX_IGMP, "    Older Querier Present Timeout (AUTO):  %u (s)", igmpProxyCfg.host.older_querier_present_timeout);
    }
    ...
    {
      igmpProxyCfg.host.older_querier_present_timeout = igmpProxy->host.older_querier_present_timeout;
      LOG_TRACE(LOG_CTX_IGMP, "    Older Querier Present Timeout:         %u (s)", igmpProxyCfg.host.older_querier_present_timeout);
    }
    ...
    if (igmpProxy->host.mask & PTIN_IGMP_HOST_MASK_MRPR && igmpProxyCfg.host.max_records_per_report != igmpProxy->host.max_records_per_report )
    {
      ...
      LOG_TRACE(LOG_CTX_IGMP, "    Max Records per Report:                %u (s)", igmpProxyCfg.host.max_records_per_report);
    }
      ...
      && igmpProxyCfg.admin != igmpProxy->admin)
  {
    ...
    {
      LOG_TRACE(LOG_CTX_IGMP, "Resetting Admission Control Allocation Parameters");
    ...
    if ( igmpProxy->admin == L7_ENABLE )
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to enable L3 IPMC Table!");
        return L7_FAILURE;
      }
      LOG_TRACE(LOG_CTX_IGMP, "Enabled L3 IPMC Table");
    }
    else
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to disable L3 IPMC Table!");
        return L7_FAILURE;
      }
      LOG_TRACE(LOG_CTX_IGMP, "Disabled L3 IPMC Table");
    }
#endif
...
void ptin_igmp_proxy_config_dump__snooping_old(void)
{
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "IGMP Proxy (mask=0x%08X)", igmpProxy.mask);
  LOG_DEBUG(LOG_CTX_IGMP, " Admin #                          = %u",          igmpProxy.admin);  
  LOG_DEBUG(LOG_CTX_IGMP, " Network Version                  = %u",          igmpProxy.networkVersion);
  LOG_DEBUG(LOG_CTX_IGMP, " Client Version                   = %u",          igmpProxy.clientVersion);
  LOG_DEBUG(LOG_CTX_IGMP, " IP Addr                          = %u.%u.%u.%u", (igmpProxy.ipv4_addr.s_addr>>24)&0xFF, (igmpProxy.ipv4_addr.s_addr>>16)&0xFF, (igmpProxy.ipv4_addr.s_addr>>8)&0xFF, igmpProxy.ipv4_addr.s_addr&0xFF);
  LOG_DEBUG(LOG_CTX_IGMP, " COS                              = %u",          igmpProxy.igmp_cos);
  LOG_DEBUG(LOG_CTX_IGMP, " FastLeave                        = %s",          igmpProxy.fast_leave ? "ON" : "OFF");
  LOG_DEBUG(LOG_CTX_IGMP, " Querier (mask=0x%08X)", igmpProxy.querier.mask);
  LOG_DEBUG(LOG_CTX_IGMP, "   Flags                          = 0x%04X",      igmpProxy.querier.flags);
  LOG_DEBUG(LOG_CTX_IGMP, "   Robustness                     = %u",          igmpProxy.querier.robustness);
  LOG_DEBUG(LOG_CTX_IGMP, "   Query Interval                 = %u (s)",          igmpProxy.querier.query_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Query Response Interval        = %u (ds)",          igmpProxy.querier.query_response_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Group Membership Interval      = %u (s)",          igmpProxy.querier.group_membership_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Other Querier Present Interval = %u (s)",          igmpProxy.querier.other_querier_present_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Startup Query Interval         = %u (s)",          igmpProxy.querier.startup_query_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Startup Query Count            = %u",          igmpProxy.querier.startup_query_count);
  LOG_DEBUG(LOG_CTX_IGMP, "   Last Member Query Interval     = %u (ds)",          igmpProxy.querier.last_member_query_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Last Member Query Count        = %u",          igmpProxy.querier.last_member_query_count);
  LOG_DEBUG(LOG_CTX_IGMP, "   Older Host Present Timeout     = %u (s)",          igmpProxy.querier.older_host_present_timeout);
  LOG_DEBUG(LOG_CTX_IGMP, " Host (mask=0x%08X)", igmpProxy.host.mask);         
  LOG_DEBUG(LOG_CTX_IGMP, "   Flags                          = 0x%02X",      igmpProxy.host.flags);
  LOG_DEBUG(LOG_CTX_IGMP, "   Robustness                     = %u",          igmpProxy.host.robustness);
  LOG_DEBUG(LOG_CTX_IGMP, "   Unsolicited Report Interval    = %u (s)",          igmpProxy.host.unsolicited_report_interval);
  LOG_DEBUG(LOG_CTX_IGMP, "   Older Querier Present Timeout  = %u (s)",          igmpProxy.host.older_querier_present_timeout);
  LOG_DEBUG(LOG_CTX_IGMP, "   Max Group Records per Packet   = %u",          igmpProxy.host.max_records_per_report);
  LOG_DEBUG(LOG_CTX_IGMP, " Bandwidth Control                = %s",          igmpProxy.bandwidthControl ? "ON" : "OFF");
  LOG_DEBUG(LOG_CTX_IGMP, " Channels Control                 = %s",          igmpProxy.channelsControl ? "ON" : "OFF");
  LOG_DEBUG(LOG_CTX_IGMP, " White-List                       = %s",          igmpProxy.whiteList  ? "ON" : "OFF");
}
...
L7_RC_t ptin_igmp_proxy_config_set(PTIN_MGMD_CTRL_MGMD_CONFIG_t *igmpProxy)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Error configuring Igmp Proxy rc:%u", ctrlResMsg.res);
  }

  /* Configure global trapping */
  if (ptin_igmp_enable(igmpProxy->admin) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP, "Error configuring VLANs trapping (enable=%u)", igmpProxy->admin);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_proxy_config_get(PTIN_MGMD_CTRL_MGMD_CONFIG_t *igmpProxy)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Unexpected size in the MGMD response [dataLength:%u/%u]", ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_MGMD_CONFIG_t));
    return L7_FAILURE;
  }
  else
  {
...
{
  LOG_INFO(LOG_CTX_IGMP,"Multicast queriers reset:");

#if PTIN_SNOOP_USE_MGMD
  if (ptin_igmp_generalquerier_reset() != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP, "Unable to reset MGMD General Queriers");
    return L7_FAILURE;
  }
#else
  L7_uint32 admin;
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error reading Querier Admin status");
    return L7_FAILURE;
  }
  /* If disabled, there is nothing to be done */
  if (!admin)
  {
    LOG_TRACE(LOG_CTX_IGMP,"Multicast querier is disabled... nothing to be done!");
    return L7_SUCCESS;
  }

  LOG_INFO(LOG_CTX_IGMP,"Going to reset Multicast queriers...");

  /* Disable, and reenable querier mechanism for all vlans */
  if (usmDbSnoopQuerierAdminModeSet(L7_DISABLE, L7_AF_INET)!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_MSG,"Failed Querier disable operation");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Failed Querier reenable operation");
    return L7_FAILURE;
  }
#endif

  LOG_INFO(LOG_CTX_IGMP,"Multicast queriers reenabled!");
...
L7_RC_t ptin_igmp_enable(L7_BOOL enable)
{
  ...
  {
    LOG_ERR(LOG_CTX_DHCP,"Error setting IGMP global enable to %u", enable);
    return L7_FAILURE;
  }
  LOG_TRACE(LOG_CTX_DHCP,"Success setting IGMP global enable to %u", enable);
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error configuring packet trapping for QUATTRO VLANs (enable=%u)", enable);
    return L7_FAILURE;
  }
  LOG_TRACE(LOG_CTX_IGMP,"Packet trapping for QUATTRO VLANs configured (enable=%u)", enable);
#endif
...
L7_RC_t ptin_igmp_is_evc_used(L7_uint32 evc_idx)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid eEVC id: evc_idx=%u",evc_idx);
    return L7_FALSE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"eEVC id is not active: evc_idx=%u",evc_idx);
    return L7_FALSE;
  }
...
L7_RC_t ptin_igmp_instance_add(L7_uint32 McastEvcId, L7_uint32 UcastEvcId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid eEVC ids: [mcEvcId,ucEvcId]=[%u,%u]",McastEvcId,UcastEvcId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"eEVC ids are not active: [mcEvcId,ucEvcId]=[%u,%u]",McastEvcId,UcastEvcId);
    return L7_FAILURE;
  }
  ...
  if (ptin_igmp_instance_find(McastEvcId,UcastEvcId,L7_NULLPTR)==L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      LOG_WARNING(LOG_CTX_IGMP,"There is already an instance with [mcEvcId,ucEvcId]=[%u,%u]",McastEvcId,UcastEvcId);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"There is conflict with these parameters: [mcEvcId,ucEvcId]=[%u,%u]",McastEvcId,UcastEvcId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"There is no free instances to be used");
    return L7_TABLE_IS_FULL;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Using free index %u",igmp_idx);

  /* Save direct referencing to igmp index from evc ids */
  if (ptin_evc_igmpInst_set(McastEvcId, igmp_idx) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error saving igmp instance to ext evc id %u", McastEvcId);
    return L7_FAILURE;
  }
#if (!defined IGMPASSOC_MULTI_MC_SUPPORTED)
  if (ptin_evc_igmpInst_set(UcastEvcId, igmp_idx) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error saving igmp instance to ext evc id %u", UcastEvcId);
  ...
  LOG_TRACE(LOG_CTX_IGMP,"IGMP index %u",igmp_idx);

  /* Configure querier for this instance */
  if (ptin_igmp_querier_configure(igmp_idx,L7_ENABLE)!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error setting querier configuration for igmp_idx=%u",igmp_idx);
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error configuring IGMP snooping for igmp_idx=%u",igmp_idx);
...
L7_RC_t ptin_igmp_instance_remove(L7_uint32 McastEvcId, L7_uint32 UcastEvcId)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP,"There is no instance with [mcEvcId,ucEvcId]=[%u,%u]",McastEvcId,UcastEvcId);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error clearing querier configuration for igmp_idx=%u",igmp_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error resetting igmp instance to ext evc id %u", McastEvcId);
    return L7_FAILURE;
  }
#if (!defined IGMPASSOC_MULTI_MC_SUPPORTED)
  if (ptin_evc_igmpInst_set(UcastEvcId, IGMP_INVALID_ENTRY) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error resetting igmp instance to ext evc id %u", UcastEvcId);
    return L7_FAILURE;
  }
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Evc index %u: Unable to remove service from MGMD",McastEvcId);
    return L7_FAILURE;
  }
#endif
...
L7_RC_t ptin_igmp_clean_all(void)
{
  ...
  for (igmp_idx=0; igmp_idx<PTIN_SYSTEM_N_IGMP_INSTANCES; igmp_idx++)
  {
    if ((rc=ptin_igmp_instance_delete(igmp_idx))!=L7_SUCCESS)
    {
      if (rc_global == L7_SUCCESS)
        rc_global = rc;
      LOG_ERR(LOG_CTX_IGMP,"Error clearing igmp_idx=%u",igmp_idx);
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error clearing igmp associations");
    rc_global = rc;
  }
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error clearing igmp clients");
    rc_global = rc;
  }
...
L7_RC_t ptin_igmp_instances_reactivate(void)
{
  ...
  for (igmp_idx=0; igmp_idx<PTIN_SYSTEM_N_IGMP_INSTANCES; igmp_idx++)
  {
    ...
    if (ptin_igmp_querier_configure(igmp_idx,L7_DISABLE)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Error disabling querier for igmp_idx=%u",igmp_idx);
      rc = L7_FAILURE;
      continue;
    }
    if (ptin_igmp_querier_configure(igmp_idx,L7_ENABLE)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Error enabling querier for igmp_idx=%u",igmp_idx);
      rc = L7_FAILURE;
      continue;
    }
  }
...
L7_RC_t ptin_igmp_instance_destroy(L7_uint32 evc_idx)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP,"EVC id %u is not used in any IGMP instance",evc_idx);
    return L7_SUCCESS;
  }
...
L7_RC_t ptin_igmp_evc_add(L7_uint32 evc_idx, L7_uint16 nni_ovlan)
{
  ...
  if (evc_idx>=PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid eEVC ids: mcEvcId=%u",evc_idx);
    return L7_FAILURE;
  }
  ...
  if (!ptin_evc_is_in_use(evc_idx))
  {
    LOG_ERR(LOG_CTX_IGMP,"eEVC ids are not active: mcEvcId=%u",evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error getting eEVC %u type",evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP,"There is already an instance with mcEvcId=%u",evc_idx);
    return L7_SUCCESS;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Going to add evc %u",evc_idx);
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error setting igmp instance to ext evc id %u", evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error setting querier configuration for evc_idx=%u (igmp_idx=%u)",evc_idx,igmp_idx);
    if (new_instance)
      memset(&igmpInstances[igmp_idx], 0x00, sizeof(st_IgmpInstCfg_t));
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to enable trap rule of igmp_idx=%u",evc_idx, igmp_idx);
...
#endif
  {
    ...
    LOG_TRACE(LOG_CTX_IGMP,"Success enabling trap rule of igmp_idx=%u",igmp_idx);
  }
...
L7_RC_t ptin_igmp_evc_remove(L7_uint32 evc_idx)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP,"There is no instance with [mcEvcId,ucEvcId]=[%u,%u]",evc_idx);
    return L7_SUCCESS;
  }
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to remove evc %u from igmp_idx=%u",evc_idx, igmp_idx);
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to disable trap rule of igmp_idx=%u", igmp_idx);
...
#endif
  {
    ...
    LOG_TRACE(LOG_CTX_IGMP,"Success disabling trap rule of igmp_idx=%u",igmp_idx);
  }

  /* Reset direct referencing to igmp index from evc ids */
  if (ptin_evc_igmpInst_set(evc_idx, IGMP_INVALID_ENTRY) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error resetting igmp instance to ext evc id %u", evc_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_client_add(L7_uint32 evc_idx, const ptin_client_id_t *client_id, L7_uint16 uni_ovid, L7_uint16 uni_ivid, L7_uint8 onuId, L7_uint8 mask, L7_uint64 maxBandwidth, L7_uint16 maxChannels, L7_BOOL addOrRemove, L7_uint32 *packagePtr, L7_uint32 noOfPackages)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid client id");
    return L7_FAILURE;
  }
       ...
       (client.mask & PTIN_CLIENT_MASK_FIELD_INNERVLAN) )
  {
    ...
    if (ptin_intf_ptintf2intIfNum(&client.ptin_intf, &intIfNum)==L7_SUCCESS)
    {
      ...
      {
        LOG_TRACE(LOG_CTX_IGMP,"Ext vlans for ptin_intf %u/%u, cvlan %u: uni_ovid=%u, uni_ivid=%u",
                  client.ptin_intf.intf_type,client.ptin_intf.intf_id, client.innerVlan, uni_ovid, uni_ivid);
      }
      else
      {
        uni_ovid = uni_ivid = 0;
        LOG_ERR(LOG_CTX_IGMP,"Cannot get ext vlans for ptin_intf %u/%u, cvlan %u",
                client.ptin_intf.intf_type,client.ptin_intf.intf_id, client.innerVlan);
      }
    }
    else
    {
      LOG_ERR(LOG_CTX_IGMP,"Invalid ptin_intf %u/%u", client.ptin_intf.intf_type, client.ptin_intf.intf_id);
    }
  }
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error creating static client");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_client_delete(L7_uint32 evc_idx, const ptin_client_id_t *client_id)
{
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error deleting static client");
  }
...
L7_RC_t ptin_igmp_all_clients_flush(void)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error flushing all clients");
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Clients flushed!");
...
L7_RC_t ptin_igmp_generalquerier_reset(void)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Response");
  LOG_TRACE(LOG_CTX_IGMP, "  CTRL Msg Code: %08X", ctrlResMsg.msgCode);
  LOG_TRACE(LOG_CTX_IGMP, "  CTRL Msg Id  : %08X", ctrlResMsg.msgId);
  LOG_TRACE(LOG_CTX_IGMP, "  CTRL Res     : %u",   ctrlResMsg.res);
                                  ...
                                  L7_uint16 *total_channels)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Null parameters");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid client pointer");
    return L7_FAILURE;
  }
  ...
  if (client.mask == 0)
  {
    ...
    do
    {
      ...
      LOG_DEBUG(LOG_CTX_IGMP, "Response");
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Msg Code: %08X",      ctrlResMsg.msgCode);
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Msg Id  : %08X",      ctrlResMsg.msgId);
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Res     : %u",        ctrlResMsg.res);
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Length  : %u (%.1f)", ctrlResMsg.dataLength, ((double)ctrlResMsg.dataLength)/sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));


      if (0 == ctrlResMsg.dataLength%sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t))
      {
        LOG_DEBUG(LOG_CTX_IGMP, "Active groups (Service:%u)", McastEvcId);
        while ((ctrlResMsg.dataLength > 0) && (numberOfChannels < *max_number_of_channels))
        {
          ...
          LOG_DEBUG(LOG_CTX_IGMP, "  Entry [%u]", mgmdGroupsRes.entryId);
          LOG_DEBUG(LOG_CTX_IGMP, "    Type:           %s",   mgmdGroupsRes.groupType==PTIN_MGMD_CTRL_GROUPTYPE_DYNAMIC? ("Dynamic"):("Static"));
          LOG_DEBUG(LOG_CTX_IGMP, "    Filter-Mode:    %s",   mgmdGroupsRes.filterMode==PTIN_MGMD_CTRL_FILTERMODE_INCLUDE? ("Include"):("Exclude"));
          LOG_DEBUG(LOG_CTX_IGMP, "    Group Timer:    %u",   mgmdGroupsRes.groupTimer);
          LOG_DEBUG(LOG_CTX_IGMP, "    Groups Address: %08X", mgmdGroupsRes.groupIP);
          LOG_DEBUG(LOG_CTX_IGMP, "    Source Timer:   %u",   mgmdGroupsRes.sourceTimer);
          LOG_DEBUG(LOG_CTX_IGMP, "    Source Address: %08X", mgmdGroupsRes.sourceIP);
      ...
      else
      {
        if (ctrlResMsg.res==NOT_EXIST)
          break;
        LOG_ERR(LOG_CTX_IGMP, "Invalid response size from MGMD [size:%u]. Expecting Multiple of :%u", ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
        return L7_FAILURE;
      }      
      entryId = mgmdGroupsRes.entryId;

      LOG_TRACE(LOG_CTX_IGMP, "channelCopied=%u channelCopied=%u sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t)=%u", channelCopied, numberOfChannels,sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
  ...
  else
  {
    ...
    if (ptin_intf_ptintf2intIfNum(&client.ptin_intf, &intIfNum)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_INTF, "Error converting port %u/%u to intIfNum",client.ptin_intf.intf_type,client.ptin_intf.intf_id);
      ...
      for (clientId = 0; clientId<PTIN_IGMP_CLIENTIDX_MAX; ++clientId)
      {
        ...
        if (IS_BITMAP_BIT_SET(clientGroup->client_bmp_list, clientId, UINT32_BITSIZE))
        {
          do
          {
            ...
            LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Msg Id  : %08X",      ctrlResMsg.msgId);
            LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Res     : %u",        ctrlResMsg.res);
            LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Length  : %u (%u)", ctrlResMsg.dataLength, (ctrlResMsg.dataLength)/sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));    
            ...
            LOG_TRACE(LOG_CTX_IGMP, "channelCopied=%u numberOfChannels=%u sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t)=%u", channelCopied, numberOfChannels, sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
          ...
          {
            LOG_TRACE(LOG_CTX_IGMP, "noOfClientsFound:%u >= noOfClients:%u", noOfClientsFound, noOfClients);
            break;
          }
    ...
    {
      LOG_DEBUG(LOG_CTX_IGMP, "This groupClient has zero devices currently attached!");
    }
                                 ...
                                 L7_uint16 *total_clients)
{
  ...
  {
    *number_of_clients=0;
    LOG_ERR(LOG_CTX_IGMP,"There is no IGMP instance with MC EVC id %u",McastEvcId);
    return L7_NOT_EXIST;
  }
  ...
  {
    *number_of_clients=0;
    LOG_ERR(LOG_CTX_IGMP,"Unable to clean the current clientGroup snapshot");
    return L7_FAILURE;
  }
  ...
  if (0 == client_index)
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "Max Entries:%u", maxResponseEntries);
    ...
    do
    {
      ...
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Msg Id              : %08X",      ctrlResMsg.msgId);
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Res                 : %u",        ctrlResMsg.res);
      LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Length              : %u", ctrlResMsg.dataLength);
      LOG_DEBUG(LOG_CTX_IGMP, "  Entry Length             : %u", sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t), ctrlResMsg.dataLength/sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t));
      LOG_DEBUG(LOG_CTX_IGMP, "  Number Of Entries Length : %u", ctrlResMsg.dataLength/sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t));

      if (0 != ctrlResMsg.dataLength%sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t))
      {
        if (ctrlResMsg.res==NOT_EXIST)
          break;
        LOG_ERR(LOG_CTX_IGMP,"Invalid dataLength:%u received. Expecting multiple of :%u", ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t));
        /*Give Semaphore*/
        osapiSemaGive(ptin_igmp_clients_sem);
      ...
      LOG_DEBUG(LOG_CTX_IGMP, "Active groups (Service:%u GroupAddr:%08X)", McastEvcId, groupAddr->s_addr);
      ...
      while (ctrlResMsg.dataLength > 0 && mgmdGroupsRes != L7_NULLPTR)
      {
        ...
        LOG_DEBUG(LOG_CTX_IGMP, "  EntryId [%u]",   mgmdGroupsRes->entryId);
        LOG_DEBUG(LOG_CTX_IGMP, "    PortId:   %u", mgmdGroupsRes->portId);
        LOG_DEBUG(LOG_CTX_IGMP, "    ClientId: %u", mgmdGroupsRes->clientId);

        if (ptin_intf_intIfNum2port(mgmdGroupsRes->portId,&ptinPort) != L7_SUCCESS)
        {
          *number_of_clients=0;
          LOG_ERR(LOG_CTX_IGMP,"Failed to convert intIfNum [%u] to ptinPort",mgmdGroupsRes->portId);
          /*Give Semaphore*/
          osapiSemaGive(ptin_igmp_clients_sem);

          return L7_FAILURE;
        }
        LOG_TRACE(LOG_CTX_IGMP, "Converted   intIfNum [%u] to ptinPort [%u]", mgmdGroupsRes->portId,ptinPort);
        ...
        if (L7_NULLPTR == (client = igmpDeviceClients.client_devices[ptinPort][mgmdGroupsRes->clientId].client))
        {
          ...
          LOG_WARNING(LOG_CTX_IGMP,"Invalid client returned from MGMD clientId:%u", mgmdGroupsRes->clientId);
          mgmdGroupsRes++;                  
          continue;
        }
        ...
        {
          *number_of_clients=0;
          LOG_ERR(LOG_CTX_IGMP,"Invalid client returned from MGMD clientId:%u", mgmdGroupsRes->clientId);
        ...
        {
          *number_of_clients=0;
          LOG_ERR(LOG_CTX_IGMP,"Unable to Convert intfNum[%u]", clientGroup->ptin_port);
        ...
        {
          *number_of_clients=0;
          LOG_ERR(LOG_CTX_IGMP,"Unable to add this clientIdx[%u] port portId[%u] to the clientGroupSnapshot avlTree", mgmdGroupsRes->clientId, mgmdGroupsRes->portId);
  ...
  while (L7_NULLPTR != (avl_infoData = (ptinIgmpClientGroupsSnapshotInfoData_t *)avlSearchLVL7(&(igmpSnapshotClientGroups.avlTree), &avl_key, L7_MATCH_GETNEXT)))
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to get external EVC Id :%u",tempKey.outerVlan);
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to convert client[mask:%02X intf:%u/%u oVlan:%u iVlan:%u]", avl_infoData->key.mask, avl_infoData->key.ptin_intf.intf_type, avl_infoData->key.ptin_intf.intf_id,
              avl_infoData->key.outerVlan, avl_infoData->key.innerVlan);
      return L7_FAILURE;
    }
    LOG_DEBUG(LOG_CTX_IGMP, "      Idx:   %u",      clientBufferIdx);
    LOG_DEBUG(LOG_CTX_IGMP, "        Mask:  0x%02X",  tempKey.mask);
    LOG_DEBUG(LOG_CTX_IGMP, "        Intf:  %u/%u", tempKey.ptin_intf.intf_type, tempKey.ptin_intf.intf_id);
    LOG_DEBUG(LOG_CTX_IGMP, "        oVlan: %u",    tempKey.outerVlan);
    LOG_DEBUG(LOG_CTX_IGMP, "        iVlan: %u",    tempKey.innerVlan);
    LOG_DEBUG(LOG_CTX_IGMP, "        eEVCId:%u",    extendedEvcId[clientBufferIdx]);
                               ...
                               L7_int client_idx, L7_uint16 *uni_ovid, L7_uint16 *uni_ivid)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error getting ptin_port for intIfNum %u", intIfNum);
    return L7_FAILURE;
  }
      ...
      (client_idx >= 0 && client_idx < PTIN_IGMP_CLIENTIDX_MAX))
  {
    ...
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP, "Caught vlans: %u+%u", ovid, ivid);
  }
  ...
  if (ovid == 0)
  {
    ...
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP, "Vlans obtained from EVC: %u+%u", ovid, ivid);
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Vlans to retrieve: %u+%u", ovid, ivid);
                              ...
                              L7_uint16 *inner_vlan_next, L7_uint16 *uni_ovid, L7_uint16 *uni_ivid)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Invalid internal vlan %u", intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client_ref intIfNum to ptin_port format", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No more clients found!");
    return L7_NO_VALUE;
  }
                                 ...
                                 ptin_client_id_t *client)
{
  ...
  if (intIfNum>0 && intIfNum<L7_ALL_INTERFACES)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error converting intIfNum %u to ptin_intf format",intIfNum);
      return L7_FAILURE;
    }
                                  ...
                                  L7_uint *client_index)
{
  ...
  if (ptin_debug_igmp_snooping)
  {
    LOG_TRACE(LOG_CTX_IGMP,"Client_idx=%u for key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
                              ...
                              L7_BOOL *isDynamic)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Invalid client_idx %u", client_idx);
    return L7_FAILURE;
  }
  /* Validate interface */
  if (ptin_intf_intIfNum2port(intIfNum, &ptin_port) != L7_SUCCESS ||
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error converting intIfNum %u to ptin_port format", intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Provided client_idx (%u) does not exist",client_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_client_timer_start(L7_uint32 intIfNum, L7_uint32 client_idx)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error (re)starting timer for this client (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  }
...
L7_RC_t ptin_igmp_group_client_add(ptin_client_id_t *client, L7_uint16 uni_ovid, L7_uint16 uni_ivid, L7_uint8 onuId, L7_uint8 mask, L7_uint64 maxAllowedBandwidth, L7_uint16 maxAllowedChannels, L7_BOOL addOrRemove, L7_uint32 *packagePtr, L7_uint32 noOfPackages)
{
  ...
  if (client->mask & PTIN_CLIENT_MASK_FIELD_INTF)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client intf %u/%u to ptin_port format",
              client->ptin_intf.intf_type, client->ptin_intf.intf_id);
      return L7_FAILURE;
    }
  }
  ...
  if ((avl_infoData=avlSearchLVL7( &(avl_tree->igmpClientsAvlTree), (void *)&avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    ...
    if (noOfPackages != 0)
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot find key for this package{"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Failed to Obtain Group Client Identifier (groupClientId:%u) {"              
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error inserting key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot find key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    if (ptin_debug_igmp_snooping)
    {
      LOG_TRACE(LOG_CTX_IGMP,"Success inserting Key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  else
  {
    if (ptin_debug_igmp_snooping)
    {
      LOG_WARNING(LOG_CTX_IGMP,"This key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                  "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  if ( mask <= PTIN_IGMP_ADMISSION_CONTROL_MASK_VALID )
  {
    ...
    if ( (mask & PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS) == PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS)
    {
      if (maxAllowedChannels == PTIN_IGMP_ADMISSION_CONTROL_MAX_CHANNELS_DISABLE)  /*Disable this Parameter*/
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_DEBUG(LOG_CTX_IGMP,"Disabling Max Channels Control [mask:0x%x allocatedChannels:%u maxAllowedChannels:%u]",avl_infoData->admissionControl.mask, avl_infoData->admissionControl.allocatedChannels, avl_infoData->admissionControl.maxAllowedChannels);
        }
      }
      else
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_TRACE(LOG_CTX_IGMP,"Max Channels Control [mask:0x%x allocatedChannels:%u maxAllowedChannels:%u]",avl_infoData->admissionControl.mask, avl_infoData->admissionControl.allocatedChannels, avl_infoData->admissionControl.maxAllowedChannels);
        }
      }    
    }
    ...
    if ( (mask & PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH) == PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH)
    {
      if (maxAllowedBandwidth == PTIN_IGMP_ADMISSION_CONTROL_MAX_BANDWIDTH_IN_BPS_DISABLE) /*Disable this Parameter*/
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_DEBUG(LOG_CTX_IGMP,"Disabling Max Channels Control [mask:0x%x allocatedBandwidth:%u maxAllowedBandwidth:%u]",avl_infoData->admissionControl.mask, avl_infoData->admissionControl.allocatedBandwidth, avl_infoData->admissionControl.maxAllowedBandwidth);
        }
      }
      else
      {
        ...
        if (ptin_debug_igmp_snooping)
        {
          LOG_TRACE(LOG_CTX_IGMP,"Max Bandwidth Control [mask:0x%x allocatedBandwidth:%u maxAllowedBandwidth:%u]",avl_infoData->admissionControl.mask, avl_infoData->admissionControl.allocatedBandwidth, avl_infoData->admissionControl.maxAllowedBandwidth);
        }
      }
    }
  }
  ...
  if (noOfPackages > 0)
  {
    if ( addOrRemove == L7_FALSE)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Failed to Add Packages {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    else
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Failed to Remove Packages {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
...
L7_RC_t ptin_igmp_group_client_remove(ptin_client_id_t *client)
{
  ...
  if (ptin_debug_igmp_snooping)
  {
    LOG_TRACE(LOG_CTX_IGMP,"Key to search {"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  {
    osapiSemaGive(ptin_igmp_clients_sem);
    LOG_ERR(LOG_CTX_IGMP,"Could not remove child clients!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error removing key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
            "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  if (ptin_debug_igmp_snooping)
  {
    LOG_TRACE(LOG_CTX_IGMP,"Success removing Key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
              "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
...
L7_RC_t ptin_igmp_group_client_clean(void)
{
  ...
  if (rc!=L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"An error ocurred during clients remotion.");
  }
  else
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Clients removed!");
  }
                                     ...
                                     L7_uint *client_idx_ret)
{
  ...
  if (intIfNum > 0 && intVlan > 0 && innerVlan > 0)
  {
    ...
    {
      LOG_TRACE(LOG_CTX_IGMP,"Ext vlans for intIfNum %u, cvlan %u: uni_ovid=%u, uni_ivid=%u",
                intIfNum, innerVlan, uni_ovid, uni_ivid);
    }
    else
    {
      uni_ovid = uni_ivid = 0;
      LOG_ERR(LOG_CTX_IGMP,"Cannot get ext vlans for intIfNum %u, cvlan %u", intIfNum, innerVlan);
    }
  }

  LOG_TRACE(LOG_CTX_IGMP,"Going to create new client: intIfNum %u, intVlan %u, innerVlan %u",
  ...
  LOG_TRACE(LOG_CTX_IGMP,"New client created: intIfNum %u, intVlan %u, innerVlan %u",
            intIfNum, intVlan, innerVlan);

  if (rc!=L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error adding dynamic client");
  }
...
//    LOG_ERR(LOG_CTX_IGMP,"Error converting intIfNum %u to ptin_port format", intIfNum);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Error flushing dynamic client (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
    osapiSemaGive(ptin_igmp_clients_sem);
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Success flushing client");
...
L7_RC_t ptin_igmp_dynamic_all_clients_flush(void)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error flushing all dynamic clients");
    return L7_FAILURE;
  }
                                 ...
                                 ptin_client_id_t *client)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Provided client (ptin_port=%u client_idx=%u) does not exist", ptin_port, client_idx);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client port %u to ptin_intf format",clientInfo->igmpClientDataKey.ptin_port);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_intfVlan_validate(L7_uint32 intIfNum, L7_uint16 intVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No IGMP instance associated to intVlan %u",intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No EVC associated to intVlan %u",intVlan);
    return L7_FAILURE;
  }
  evc_uc = evc_mc;
#endif
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting ptin_intf from intIfNum %u",intIfNum);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting intf configuration for ptin_intf=%u/%u (intIfNum=%u), mcEvcId=%u (intVlan=%u)",
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (intIfNum=%u) not in use for mcEvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,intIfNum, evc_mc, intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_vlan_validate(L7_uint16 intVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"EVC %u (intVlan=%u) is not active!",evc_idx,intVlan);
    return L7_FAILURE;
  }
#endif
...
L7_RC_t ptin_igmp_vlan_UC_is_unstacked(L7_uint16 intVlan, L7_BOOL *is_unstacked)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No EVC id associated to intVlan %u",intVlan);
    return L7_FAILURE;
  }
#endif
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting EVC configuration for ucEvcId=%u (intVlan=%u)",
              evc_id, intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_rootIntfVlan_validate(L7_uint32 intIfNum, L7_uint16 intVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_DEBUG(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (intIfNum=%u) is not root for mcEvcId=%u (intVlan=%u)",
                ptin_intf.intf_type,ptin_intf.intf_id,intIfNum, evc_id, intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_clientIntfVlan_validate(L7_uint32 intIfNum, L7_uint16 intVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting intf configuration for ptin_intf=%u/%u (intIfNum=%u), EvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,intIfNum,evc_id,intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (intIfNum=%u) not in use for EvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,intIfNum,evc_id,intVlan);
    return L7_FAILURE;
  }
  ...
  {
//  if (ptin_debug_igmp_snooping)
//    LOG_ERR(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (intIfNum=%u) is not client/leaf for EvcId=%u (intVlan=%u)",
//            ptin_intf.intf_type,ptin_intf.intf_id,intIfNum,evc_id,intVlan);
    return L7_FAILURE;
  }
...
static ptinIgmpGroupClientInfoData_t* deviceClientId2groupClientPtr(L7_uint32 ptin_port, L7_uint32 clientId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments: ptin_port:%u clientId:%u",ptin_port, clientId);
    return L7_NULLPTR;
  }
...
L7_RC_t ptin_igmp_McastRootVlan_get(L7_uint16 intVlan, L7_uint32 intIfNum, L7_BOOL isLeafPort, L7_uint32 clientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint16 *mcastRootVlan)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [isLeafPort:%u intVlan:%u intIfNum:%u client_idx:%u groupAddr:%p sourceAddr:%p McastRootVlan:%p]", isLeafPort, intVlan, intIfNum, clientId, groupAddr, sourceAddr, mcastRootVlan);    
    return L7_FAILURE;
  }
  ...
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Failed to obtain ptin_port from intIfNum:%u", intIfNum);
      return L7_FAILURE;
    }
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_ERR(LOG_CTX_IGMP,"Failed to Get Multicast Service [ptin_port:%u client_idx:%u]", ptinPort, clientId);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error getting McastRootVlan from MCEvcId=%u", mcastEvcId);      
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_McastRootVlan_get(L7_uint16 intVlan, L7_uint16 *McastRootVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting McastRootVlan for MCEvcId=%u (intVlan=%u)",igmpInst->McastEvcId,intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_rootIntfs_getList(L7_uint16 intVlan, L7_INTF_MASK_t *intfList, L7_uint32 *noOfInterfaces)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting evc %u configuration (intVlan=%u)",igmpInst->McastEvcId,intVlan);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_clientIntfs_getList(L7_uint16 intVlan, L7_INTF_MASK_t *intfList, L7_uint32 *noOfInterfaces)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting evc %u configuration (intVlan=%u)",evcCfg.index,intVlan);
    return L7_FAILURE;
  }
  ...
  *noOfInterfaces = 0;
  ...
  for (intf_idx=0; intf_idx<evcCfg.n_intf; intf_idx++)
  {
    ...
    if (evcCfg.intf[intf_idx].mef_type==PTIN_EVC_INTF_LEAF)
    {
      ...
      {
        if (ptin_debug_igmp_snooping)
          LOG_DEBUG(LOG_CTX_IGMP,"Port bitmap set intIfNum:%u",intIfNum);
        L7_INTF_SETMASKBIT(*intfList,intIfNum);
        (*noOfInterfaces)++;
      }
    }
...
L7_RC_t ptin_igmp_extMcastVlan_get(L7_uint32 intIfNum, L7_uint16 intOVlan, L7_uint16 intIVlan, L7_uint16 *extMcastVlan, L7_uint16 *extIVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No IGMP instance associated to intVlan %u",intOVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting external vlans to intIfNum=%u, intOVlan=%u, intIVlan=%u (MCEvcId=%u)",intIfNum,intOVlan,intIVlan,igmpInst->McastEvcId);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_extUcastVlan_get(L7_uint32 intIfNum, L7_uint16 intOVlan, L7_uint16 intIVlan, L7_uint16 *extUcastVlan, L7_uint16 *extIVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Error getting external vlans to intIfNum=%u, intOVlan=%u, intIVlan=%u (UCEvcId=%u)",intIfNum,intOVlan,intIVlan,igmpInst->UcastEvcId);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_timersMng_init(void)
{
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to create ptin_igmp_timers_sem semaphore!");
    return L7_FAILURE;
  }
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR,"PTIN Timer msgQueue creation error.");
    return L7_FAILURE;
  }
  LOG_TRACE(LOG_CTX_CNFGR,"PTIN Timer msgQueue created.");
  ...
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Could not create task clientsMngmt_task");
    return L7_FAILURE;
  }
  LOG_TRACE(LOG_CTX_CNFGR,"Task clientsMngmt_task created");

  if (osapiWaitForTaskInit (L7_PTIN_IGMP_TASK_SYNC, L7_WAIT_FOREVER) != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR,"Unable to initialize clientsMngmt_task()\n");
    return(L7_FAILURE);
  }
  LOG_TRACE(LOG_CTX_CNFGR,"Task clientsMngmt_task initialized");
  ...
  if (bufferPoolInit(PTIN_SYSTEM_IGMP_MAXCLIENTS,
                     ...
                     &bufferPoolId) != L7_SUCCESS)
  {
    LOG_FATAL(LOG_CTX_CNFGR, "Failed to allocate memory for IGMP client timer buffers");
    return L7_FAILURE;
  }
  igmpDeviceClients.appTimerBufferPoolId = bufferPoolId;
  LOG_TRACE(LOG_CTX_CNFGR,"Allocated buffer pools");
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"SLL list created");
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"Allocated memory for handle list");
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"Handle list created");
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"Timer initialized");

  LOG_TRACE(LOG_CTX_CNFGR,"Initializations for IGMP timers finished");
...
L7_RC_t ptin_igmp_timersMng_deinit(void)
{
  ...
  LOG_TRACE(LOG_CTX_CNFGR,"Deinitializations for IGMP timers finished");
...
L7_RC_t ptin_igmp_timer_start(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Going to start timer for ptin_port=%u client_idx=%u", ptin_port, client_idx);

  /* Validate arguments */
  if (ptin_port >= PTIN_SYSTEM_N_INTERF)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid ptin_port %u", ptin_port);
    return L7_FAILURE;
  }
  if (client_idx >= PTIN_IGMP_CLIENTIDX_MAX)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid client index %u", client_idx);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Already exists a timer running for ptin_port=%u client_idx=%u", ptin_port, client_idx);
    ...
    if (pTimerData->timer != L7_NULL)
    {
      if (appTimerDelete(igmpDeviceClients.timerCB, (void *) pTimerData->timer) != L7_SUCCESS)
      {
        ...
        if (ptin_debug_igmp_snooping)
          LOG_ERR(LOG_CTX_IGMP,"Failed removing timer");
        return L7_FAILURE;
      }
      pTimerData->timer = L7_NULLPTR;
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Timer removed!");

      /* Remove timer handle */
      handleListNodeDelete(igmpDeviceClients.appTimer_handle_list, &pTimerData->timerHandle);
      pTimerData->timerHandle = 0;
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Removed node from handle list (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
    }
#else
    if (pTimerData->timer != L7_NULL)
    {
      ...
      {
        osapiSemaGive(ptin_igmp_timers_sem);
        LOG_ERR(LOG_CTX_IGMP,"Failed to update client timer");
        return L7_FAILURE;
      }
      osapiSemaGive(ptin_igmp_timers_sem);
      LOG_TRACE(LOG_CTX_IGMP,"Timer updated for ptin_port=%u client_idx=%u", ptin_port, client_idx);
      return L7_SUCCESS;
    }
    else
    {
      ...
      //pTimerData->timerHandle = 0;
      LOG_TRACE(LOG_CTX_IGMP,"Handle node removed for ptin_port=%u client_idx=%u", ptin_port, client_idx);
    }
#endif
  }
  else
  {
    ...
    {
      osapiSemaGive(ptin_igmp_timers_sem);
      LOG_ERR(LOG_CTX_IGMP,"Could not start timer. Insufficient memory.");
      return L7_FAILURE;
    }
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Allocated one Buffer node (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Stored node in handle list (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if (pTimerData->timer == NULL)
  {
    ...
    bufferPoolFree(igmpDeviceClients.appTimerBufferPoolId, (L7_uchar8 *)pTimerData);
    osapiSemaGive(ptin_igmp_timers_sem);
    LOG_ERR(LOG_CTX_IGMP,"Could not Start the Client timer.");
    return L7_FAILURE;
  }
  ...
  if ( !timer_exists )
  {
    ...
    {
      /* Free the previously allocated bufferpool */
//    LOG_ERR(LOG_CTX_IGMP,"Could not add new timer data node");
  ...
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP,"Timer Started: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p timeout=%u (s)", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, timeout);
...
L7_RC_t ptin_igmp_timer_update(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Going to update an existent timer (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Searching for an SLL node (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if ((pTimerData = (igmpTimerData_t *)SLLFind(&igmpDeviceClients.ll_timerList, (void *)&timerData)) == L7_NULLPTR)
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Client timer not found: ptin_port=%u client_idx=%u",  ptin_port, client_idx);
    return L7_FAILURE;
  }
  ...
  if (pTimerData->timer == L7_NULL)
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Client timer not running: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle:%p", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle);
    return L7_FAILURE;
  }
  ...
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    LOG_ERR(LOG_CTX_IGMP,"Failed to update client timer: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p timeout=%u (s)", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, timeout);
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP,"Timer Updated: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p timeout=%u (s)", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, timeout);
...
L7_uint32 ptin_igmp_timer_timeout_get(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Going to get the timer timeout (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP,"Client Timer TimeOut: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p time_left=%u (s))", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, time_left);
...
L7_RC_t ptin_igmp_timer_stop(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Going to stop a timer (ptin_port=%u client_idx=%u )", ptin_port, client_idx);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Deleting SLL node (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  if (SLLDelete(&igmpDeviceClients.ll_timerList, (L7_sll_member_t *)&timerData) != L7_SUCCESS)
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Failed to delete timer node: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p", ptin_port, client_idx, igmpDeviceClients.timerCB, timerData.timer, timerData.timerHandle);
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP,"Timer stopped successfully for ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p", ptin_port, client_idx, igmpDeviceClients.timerCB, timerData.timer, timerData.timerHandle);
...
void igmp_timer_expiry(void *param)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Expiration event ocurred for timerHandle %p!",timerHandle);
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Weird situation!");
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Expiration event ocurred for ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=%p", ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Cannot delete timer (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  }
  pTimerData->timer = (L7_APP_TMR_HNDL_t) NULL;
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Timer deleted for ptin_port=%u client_idx=%u", ptin_port, client_idx);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Removing client (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Failed removing client (ptin_port=%u client_idx=%u)!", ptin_port, client_idx);
  }
  else
  {
    if (ptin_debug_igmp_snooping)
      LOG_DEBUG(LOG_CTX_IGMP,"Client removed (ptin_port=%u client_idx=%u)", ptin_port, client_idx);
  }
...
void igmp_timersMng_task(void)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Timer event process task started");
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Timer task ready to process events");

  /* Loop */
  while (1)
  {
    //LOG_TRACE(LOG_CTX_IGMP,"Timer task ready for new events");
    ...
    //LOG_TRACE(LOG_CTX_IGMP,"Timer task received an event");

    /* TODO: Process message */
    if (status == L7_SUCCESS)
    {
      //LOG_TRACE(LOG_CTX_IGMP,"Timer event for igmp_idx %u received",msg.igmp_idx);
...
void igmp_timerExpiryHdlr(L7_APP_TMR_CTRL_BLK_t timerCtrlBlk, void* ptrData)
{
  ...
  //LOG_TRACE(LOG_CTX_IGMP,"Received a timer event");

  msg.dummy = 0;

  //LOG_TRACE(LOG_CTX_IGMP,"Sending timer event to queue");

  rc = osapiMessageSend(clientsMngmt_queue, &msg, PTIN_IGMP_TIMER_MSG_SIZE, L7_NO_WAIT,L7_MSG_PRIORITY_NORM);

  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Client timer tick send failed");
    return;
  }

  //LOG_TRACE(LOG_CTX_IGMP,"Timer event sent to queue");
}
...
L7_RC_t igmp_timer_dataDestroy (L7_sll_member_t *ll_member)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Going to destroy timer");
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Ready to destroy timer for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);
  ...
  if (pTimerData->timer != L7_NULL)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_NOTICE(LOG_CTX_IGMP,"Cannot delete timer (ptin_port=%u client_idx=%u)", pTimerData->ptin_port, pTimerData->client_idx);
    }
    pTimerData->timer = (L7_APP_TMR_HNDL_t) NULL;

    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Timer deleted for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);
    ...
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Timer handle removed for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);
  }
  else
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Timer not running for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);
  }

  bufferPoolFree(igmpDeviceClients.appTimerBufferPoolId, (L7_uchar8 *)pTimerData);
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Buffer node removed for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Timer destroyed successfully for ptin_port=%u client_idx=%u", pTimerData->ptin_port, pTimerData->client_idx);
                                      ...
                                      ptinIgmpChannelInfoData_t **avlEntry )
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Invalid group address");
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Channel_group=0x%08x, group_address=0x%08x!",
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Invalid source address");
    return L7_FAILURE;
  }
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Channel_source=0x%08x, source_address=0x%08x!",
  ...
  if ((avl_infoData=(ptinIgmpChannelInfoData_t *) avlSearchLVL7( &(channelDB.channelAvlTree), (void *)&avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    ptin_timer_stop(77);
    if (ptin_debug_igmp_snooping)
      LOG_WARNING(LOG_CTX_IGMP,"Group channel 0x%08x does not exist!",group_address.addr.ipv4.s_addr);
    return L7_NOT_EXIST;      
  }  
#if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
  else
                                ...
                                L7_BOOL is_static, L7_uint64 channelBandwidth)
{
  ...
  if (inetIsAddressZero(channel_group)==L7_TRUE && channel_grpMask==0)
  {
    ...
    LOG_NOTICE(LOG_CTX_IGMP,"Added a Default MC Service Gateway [evc_mc:%u]", channelDB.default_evc_mc );
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error preparing groupAddr");
    return L7_FAILURE;    
  }
  ...
  if ( n_groups == 0 )
  {
    LOG_ERR(LOG_CTX_IGMP,"Group address is not valid!");
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Going to add group 0x%08x (%u addresses)", group.addr.ipv4.s_addr, n_groups);
  ...
  {
    n_sources = 1;
    LOG_WARNING(LOG_CTX_IGMP,"Source address is not valid!");
  }  
#else
  /* source ip */  
  ...
  n_sources = 1;
#endif
  LOG_TRACE(LOG_CTX_IGMP,"Going to add source 0x%08x (%u addresses)", source.addr.ipv4.s_addr, n_sources);

  /* Validate multicast service */
  if ( evc_mc >= PTIN_SYSTEM_N_EXTENDED_EVCS )
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid MC evc (%u)", evc_mc);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Maximum addresses to be added: %u", n_groups*n_sources);
  ...
  while ((rc==L7_SUCCESS && i<n_groups) || (rc!=L7_SUCCESS && i>=0))
  {
...
#endif
    {
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Error inserting group channel 0x%08x, source=0x%08x for UC_EVC=%u",
                    group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr, evc_uc);
        }
        else
        {
          LOG_TRACE(LOG_CTX_IGMP,"Added group 0x%08x, source 0x%08x", group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr);
        }
      }
      ...
      else
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Error removing group channel 0x%08x, source=0x%08x for UC_EVC=%u",
                  group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr, evc_uc);
        }
        else
        {
          LOG_TRACE(LOG_CTX_IGMP,"Removed group 0x%08x, source 0x%08x", group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr);
        }
      }
  ...
  if ( rc == L7_SUCCESS )
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"IPv6 not supported for MGMD [UC_EVC=%u MC_EVC]",evc_uc,evc_mc);
      return L7_FAILURE;                       
    }
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Unable to create requested entry in the whitelist");
      return L7_FAILURE;
    }
  }
#endif
                                   ...
                                   L7_inet_addr_t *channel_source, L7_uint16 channel_srcMask, L7_uint8 isStatic)
{
  ...
  if (inetIsAddressZero(channel_group)==L7_TRUE && channel_grpMask==0)
  {
    ...
    LOG_NOTICE(LOG_CTX_IGMP,"Removed Default MC Service Gateway [evc_mc:%u]", evc_mc);
    return L7_SUCCESS;
  }
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to remove group 0x%08x (%u addresses)", group.addr.ipv4.s_addr, n_groups);
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to remove source 0x%08x (%u addresses)", source.addr.ipv4.s_addr, n_sources);
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Cannot add more than %u channels", PTIN_IGMP_CHANNELS_MAX);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Maximum addresses to be removed: %u", n_groups*n_sources);
  ...
  for (i=0; i<n_groups; i++)
  {
    ...
    for (j=0; j<n_sources; j++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"No MC EVC associated with UC EVC %u",evc_uc);        
      }
      else
      {
        ...
        default:
          {
            LOG_WARNING(LOG_CTX_IGMP,"Invalid whitelist entryType:%u",avlEntry->entryType);  
            break;
          }
        }
      }
  ...
  if ( rc == L7_SUCCESS )
  {
    ...
    if (channel_group->family !=L7_AF_INET || channel_source->family!=L7_AF_INET)
    {
      LOG_ERR(LOG_CTX_IGMP,"IPv6 not supported for MGMD [UC_EVC=%u MC_EVC=%u]", evc_uc, evc_mc);
      return FAILURE;                       
    }
...
L7_RC_t igmp_assoc_channel_clear( L7_uint32 evc_uc, L7_uint32 evc_mc )
{
  ...
  if ( evc_mc >= PTIN_SYSTEM_N_EXTENDED_EVCS )
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid MC eEVC (%u)", evc_mc);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error removing all channels to MC evc %u, UC evc %u !", evc_mc, evc_uc);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_IGMP,"Removed all channels to MC evc %u, UC evc %u !", evc_mc, evc_uc);
                                             ...
                                             L7_inet_addr_t *channel_out, L7_uint32 *number_of_channels)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Null pointer of provided address");
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Not supported IPv6");
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Not valid address: 0x%08x",channel_in->addr.ipv4.s_addr);
    return L7_FAILURE;
  }
  ...
  if ( number_of_channels != L7_NULLPTR )
  {
    ...
    for (i=0; i<mask_inv; i++)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Mask too small (%u bits)",channel_mask);
        return L7_FAILURE;
      }
    }
    *number_of_channels = n;
    LOG_TRACE(LOG_CTX_IGMP,"Bit mask %u, number of Addresses %u", channel_mask, n);
  }
...
static L7_RC_t ptin_igmp_channel_add( ptinIgmpChannelInfoData_t *node )
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Group channel 0x%08x already exists",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"No more free entries! (number_of_entries:%u >= IGMPASSOC_CHANNELS_MAX:%u)", channelDB.number_of_entries, PTIN_IGMP_CHANNELS_MAX);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error inserting group channel 0x%08x",
            node->channelDataKey.channel_group.addr.ipv4.s_addr);
  ...
  if ((avl_infoData=(ptinIgmpChannelInfoData_t *) avlSearchLVL7(&(channelDB.channelAvlTree),(void *)&avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP,"Group channel 0x%08x was added, but does not exist",
            node->channelDataKey.channel_group.addr.ipv4.s_addr);
  ...
  {
    LOG_FATAL(LOG_CTX_IGMP,"Failed to Initialize Package Queue");
    return L7_FAILURE;
  }
...
static L7_RC_t ptin_igmp_channel_remove( ptinIgmpChannelDataKey_t *avl_key )
{
  ...
  if ((avl_infoData=(ptinIgmpChannelInfoData_t *) avlSearchLVL7( &(channelDB.channelAvlTree), (void *) avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    LOG_WARNING(LOG_CTX_IGMP,"Group channel 0x%08x does not exist",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Cannot remove Group channel 0x%08x, since it has packages currently attached:%u!",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Cannot remove Group channel 0x%08x, since it has ports currently attached:%u!",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error removing group channel 0x%08x",
            avl_key->channel_group.addr.ipv4.s_addr);
    return L7_FAILURE;
  }
...
static L7_RC_t ptin_igmp_channel_remove_multicast_service( L7_uint32 evc_uc, L7_uint32 evc_mc )
{
          ...
          ) != L7_NULLPTR )
  {
       ...
       )
    {
      ...
      if ( avlDeleteEntry(&(channelDB.channelAvlTree), (void *)&avl_key) == L7_NULLPTR )
      {
        LOG_ERR(LOG_CTX_IGMP,"Error removing group channel 0x%08x to EVC_MC=%u, EVC=UC=%u",
      ...
      else
      {
        ...
        LOG_TRACE(LOG_CTX_IGMP,"Removed group channel 0x%08x to EVC_MC=%u, EVC=UC=%u",
                  avl_key.channel_group.addr.ipv4.s_addr, evc_mc, evc_uc);
      }
    }
  }
...
L7_RC_t ptin_igmp_evc_configure(L7_uint32 evc_idx, L7_BOOL enable, L7_BOOL set_trap)
{
  ...
  {
    LOG_TRACE(LOG_CTX_IGMP,"Evc index %u is already being used in an IGMP instance",evc_idx);
    return L7_SUCCESS;
  }
  ...
  if (set_trap)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Evc index %u: Error configuring trap rule to %u",evc_idx,enable);
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Evc index %u: Error configuring querier to %u",evc_idx,enable);
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Evc index %u: Unable to remove service from MGMD",evc_idx);
    return L7_FAILURE;
  }
#endif
  return L7_SUCCESS;
...
static L7_RC_t ptin_igmp_instance_delete(L7_uint16 igmp_idx)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid IGMP instance %u", igmp_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"IGMP instance %u is not active", igmp_idx);
    return L7_SUCCESS;
  }
...
static L7_RC_t ptin_igmp_device_client_clean(ptinIgmpGroupClientInfoData_t *clientGroup)
{
  ...
  while ((client_device=igmp_clientDevice_next(clientGroup, client_device)) != L7_NULLPTR)
  {
    ...
    if (ptin_igmp_rm_clientIdx(ptin_port, client_idx, L7_FALSE, L7_TRUE) != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Error removing client index %u", client_idx);
      return L7_FAILURE;
    }
    LOG_TRACE(LOG_CTX_IGMP,"Client index %u removed", client_idx);
  }
...
static L7_RC_t ptin_igmp_group_client_find(ptin_client_id_t *client_ref, ptinIgmpGroupClientInfoData_t **client_info)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments client_ref:[%p] client_info[%p]", client_ref, client_info);
    return L7_FAILURE;
  }
  ...
  if (client_ref->mask & PTIN_CLIENT_MASK_FIELD_INTF)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_ERR(LOG_CTX_IGMP,"Cannot convert client_ref intf %u/%u to ptin_port format",client_ref->ptin_intf.intf_type,client_ref->ptin_intf.intf_id);
      return L7_FAILURE;
    }
  }
#endif
                                    ...
                                    L7_BOOL isDynamic, L7_uint *device_client_id_ret)
{
  ...
  if ((avl_infoData=avlSearchLVL7( &(avl_tree->igmpClientsAvlTree), (void *)&avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    ...
    if (clientGroup == L7_NULLPTR)
    {
      osapiSemaGive(ptin_igmp_clients_sem);
      if (ptin_debug_igmp_snooping)
        LOG_ERR(LOG_CTX_IGMP,"Client Group not found!");
      return L7_FAILURE;
    }
    ...
    if (igmp_clientDevice_get_devices_number(clientGroup) >= PTIN_SYSTEM_IGMP_MAXDEVICES_PER_ONU)
    {
      osapiSemaGive(ptin_igmp_clients_sem);
      if (ptin_debug_igmp_snooping)
        LOG_ERR(LOG_CTX_IGMP,"Cannot be added more than %u devices!", PTIN_SYSTEM_IGMP_MAXDEVICES_PER_ONU);
      return L7_FAILURE;
    }
#endif
    ...
    if (igmpDeviceClients.queue_free_clientDevices[PTIN_IGMP_CLIENT_PORT(ptin_port)].n_elems == 0)
    {
      osapiSemaGive(ptin_igmp_clients_sem);
      if (ptin_debug_igmp_snooping)
        LOG_ERR(LOG_CTX_IGMP,"No more free clients available!");
      return L7_FAILURE;
    }
    ...
    {
      osapiSemaGive(ptin_igmp_clients_sem);
      LOG_ERR(LOG_CTX_IGMP,"Cannot get new client index %u for ptin_port %u",device_client_id, ptin_port);
      return L7_FAILURE;
    }
  ...
  else
  {
    if (ptin_debug_igmp_snooping)
    {
      LOG_DEBUG(LOG_CTX_IGMP,"This key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    if (!isDynamic)
    {
      ...
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Entry is static!");
    }
  }
...
static L7_RC_t ptin_igmp_rm_client(L7_uint igmp_idx, ptin_client_id_t *client, L7_BOOL remove_static)
{
  ...
  if (client->mask & PTIN_CLIENT_MASK_FIELD_INTF)
  {
    if (ptin_intf_ptintf2port(&client->ptin_intf,&ptin_port)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client intf %u/%u to ptin_port format",client->ptin_intf.intf_type,client->ptin_intf.intf_id);
      return L7_FAILURE;
    }
    if (ptin_intf_ptintf2intIfNum(&client->ptin_intf,&intIfNum)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client intf %u/%u to intIfNum format",client->ptin_intf.intf_type,client->ptin_intf.intf_id);
      return L7_FAILURE;
    }
  }
#endif
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Error stoping timer for client_idx=%u)",client_idx);
    //osapiSemaGive(ptin_igmp_clients_sem);
    //return L7_FAILURE;
  }
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Error removing client from snooping entries");
    //return L7_FAILURE;
  }
  else
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Client removed from snooping entries");
  }
...
static L7_RC_t ptin_igmp_rm_all_clients(L7_BOOL isDynamic, L7_BOOL only_wo_channels)
{
  ...
  while ( (avl_infoData=(ptinIgmpClientInfoData_t *) avlSearchLVL7(&(avl_tree->igmpClientsAvlTree), &avl_key, L7_MATCH_GETNEXT))!=L7_NULLPTR )
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client port %u to intIfNum format", ptin_port);
      continue;
    }
...
static L7_RC_t ptin_igmp_rm_clientIdx(L7_uint ptin_port, L7_uint client_idx, L7_BOOL remove_static, L7_BOOL force_remove )
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"Provided ptin_port=%u client_idx=%u does not exist", ptin_port, client_idx);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_WARNING(LOG_CTX_IGMP,"Error getting intIfNum for ptin_port %u", ptin_port);
    rc = L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Stopping timer");
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Cannot stop timer for ptin_port=%u client_idx=%u", ptin_port, client_idx);
      //return L7_FAILURE;
    }
#endif
    ...
    if ( remove_static || clientInfo->isDynamic )
    {
      ...
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Going to unmark ptin_port=%u client_idx=%u", ptin_port, client_idx);
      ...
      {
        if (ptin_debug_igmp_snooping)
          LOG_ERR(LOG_CTX_IGMP,"Error flushing key {"
#if (MC_CLIENT_INTERF_SUPPORTED)
                  "port=%u,"
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP,"Going to remove channels");
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Proceeding for snoop channels remotion: client_idx=%u, intIfNum=%u (port=%u)",
...
//    }
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP,"Cannot proceed to snoop channels remotion");
    }
  }
#endif

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"Success flushing client (client_idx=%u)", client_idx);
...
static L7_RC_t ptin_igmp_inst_get_fromIntVlan(L7_uint16 intVlan, st_IgmpInstCfg_t **igmpInst, L7_uint *igmpInst_idx)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"No EVC associated to internal vlan %u",intVlan);
    return L7_FAILURE;
  }
  ...
  {
//  if (ptin_debug_igmp_snooping)
//    LOG_ERR(LOG_CTX_IGMP,"No IGMP instance associated to evc_idx=%u (intVlan=%u)",evc_idx,intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Inconsistency: IGMP index %u (EVCid=%u, Vlan %u) is not in use",igmp_idx,evc_idx,intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_ERR(LOG_CTX_IGMP,"Inconsistency: IGMP index %u (EVCid=%u, Vlan %u) has EVCs not in use (MC=%u)",igmp_idx,evc_idx,intVlan,igmpInstances[igmp_idx].McastEvcId);
    return L7_FAILURE;
  }
...
//      LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopQuerierAddressSet");
//      return L7_FAILURE;
//    }
//    // IGMP version
//    if (usmDbSnoopQuerierVersionSet( igmpProxyCfg.clientVersion, L7_AF_INET)!=L7_SUCCESS)  {
//      LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopQuerierVersionSet");
//      return L7_FAILURE;
//    }
//    // Querier interval
//    if (usmDbSnoopQuerierQueryIntervalSet( igmpProxyCfg.querier.query_interval, L7_AF_INET)!=L7_SUCCESS)  {
//      LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopQuerierQueryIntervalSet");
//      return L7_FAILURE;
//    }
//
//    LOG_TRACE(LOG_CTX_IGMP,"IPv4 address set to %u.%u.%u.%u",
...
//    LOG_TRACE(LOG_CTX_IGMP,"IGMP version set to %u",igmpProxyCfg.clientVersion);
//    LOG_TRACE(LOG_CTX_IGMP,"Querier interval set to %u",igmpProxyCfg.querier.query_interval);
//  }
//
...
//    LOG_ERR(LOG_CTX_IGMP,"Error with usmDbSnoopQuerierAdminModeSet");
//    if (igmpProxyCfg.admin)  usmDbSnoopAdminModeSet( 1, L7_DISABLE, L7_AF_INET);
//    return L7_FAILURE;
//  }
//  LOG_TRACE(LOG_CTX_IGMP,"IGMP snooping enable set to %u",igmpProxyCfg.admin);
//
...
static L7_RC_t ptin_igmp_trap_configure(L7_uint igmp_idx, L7_BOOL enable)
{
  ...
  if (igmp_idx>=PTIN_SYSTEM_N_IGMP_INSTANCES)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid igmp instance index (%u)",igmp_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_TRACE(LOG_CTX_IGMP,"IGMP index %u",igmp_idx);
    LOG_ERR(LOG_CTX_IGMP,"igmp instance index %u is not in use",igmp_idx);
    return L7_FAILURE;
  }
...
static L7_RC_t ptin_igmp_evc_trap_set(L7_uint32 evc_idx_mc, L7_uint32 evc_idx_uc, L7_BOOL enable)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Can't get MC root vlan for evc id %u", evc_idx_mc);
    return L7_FAILURE;
  }
...
#endif
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error configuring vlan %u for packet trapping (enable=%u)", mc_vlan, enable);
      return L7_FAILURE;
    }
    LOG_TRACE(LOG_CTX_IGMP,"Success configuring vlan %u for packet trapping (enable=%u)", mc_vlan, enable);
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Can't get UC root vlan for evc id %u", evc_idx_uc);
    return L7_FAILURE;
  }
...
#endif
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error configuring vlan %u for packet trapping (enable=%u)", uc_vlan, enable);
      usmDbSnoopVlanModeSet(1, mc_vlan, !enable, L7_AF_INET);
      return L7_FAILURE;
    }
    LOG_TRACE(LOG_CTX_IGMP,"Success configuring vlan %u for packet trapping (enable=%u)", uc_vlan, enable);
  }
#endif
...
static L7_RC_t ptin_igmp_evc_trap_configure(L7_uint32 evc_idx, L7_BOOL enable)
{
  ...
  if (evc_idx>=PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid evc index %u",evc_idx);
    return L7_FAILURE;
  }
  /* Check if EVC is in use */
  if ( !ptin_evc_is_in_use(evc_idx) )
  {
    LOG_ERR(LOG_CTX_IGMP,"EVC  %u is not in use",evc_idx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Can't get root vlan for evc id %u",evc_idx);
    return L7_FAILURE;
  }
...
#endif
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Error configuring vlan %u for packet trapping (enable=%u)", vlan, enable);
      return L7_FAILURE;
    }
    LOG_TRACE(LOG_CTX_IGMP,"Success configuring vlan %u for packet trapping (enable=%u)", vlan, enable);
  }
...
void ptin_igmp_mgmd_whitelist_clean(void)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Unable to sent event");
    return;
  }
}
...
static L7_RC_t ptin_igmp_clientId_convert(L7_uint32 evc_idx, ptin_client_id_t *client)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP,"Client mask is null");
    return L7_FAILURE;
  }
  ...
  if (client->mask & PTIN_CLIENT_MASK_FIELD_INNERVLAN)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Invalid inner vlan (%u)",client->innerVlan);
      return L7_FAILURE;
    }
      ...
      client->mask & PTIN_CLIENT_MASK_FIELD_OUTERVLAN)
  {
    ...
    if (client->outerVlan<PTIN_VLAN_MIN || client->outerVlan>PTIN_VLAN_MAX)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Error obtaining internal vlan for evc_idx=%u, ptin_intf=%u/%u",
    ...
    else
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP,"Error obtaining internal vlan for OVid=%u, IVid=%u, ptin_intf=%u/%u",
...
static L7_RC_t ptin_igmp_clientId_restore(ptin_client_id_t *client)
{
  ...
  if (client==L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid arguments or no parameters provided");
    return L7_FAILURE;
  }
      ...
      client->mask & PTIN_CLIENT_MASK_FIELD_OUTERVLAN)
  {
    ...
    if (ptin_intf_ptintf2intIfNum(&client->ptin_intf, &intIfNum)!=L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_IGMP,"Cannot convert client intf %u/%u to intIfNum format",
              client->ptin_intf.intf_type,client->ptin_intf.intf_id);
      return L7_FAILURE;
    }
    ...
    if (client->outerVlan<PTIN_VLAN_MIN || client->outerVlan>PTIN_VLAN_MAX)
    {
      LOG_ERR(LOG_CTX_IGMP,"Invalid outer vlan (%u)",client->outerVlan);
      return L7_FAILURE;
    }
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Could not obtain external vlan for intVlan %u, ptin_intf %u/%u",
...
L7_RC_t ptin_igmp_stat_intf_get(ptin_intf_t *ptin_intf, PTIN_MGMD_CTRL_STATS_RESPONSE_t *statistics)
{
  ...
  /*Added to Include the Stats from FP*/
  {
    ...
    {
      LOG_ERR(LOG_CTX_INTF, "Error converting port %u/%u to ptin_port",ptin_intf->intf_type,ptin_intf->intf_id);
      return L7_FAILURE;
    }
  ...
  if (noOfClients>0)
  {
    ...
    for (clientId = 0; clientId < PTIN_IGMP_CLIENTIDX_MAX; ++clientId)
    {
      ...
      if (IS_BITMAP_BIT_SET(clientGroup->client_bmp_list, clientId, UINT32_BITSIZE))
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP, "Error reading clientId[%u] statistics rc:%u", clientId, ctrlResMsg.res);
          /*Give Semaphore*/
          osapiSemaGive(ptin_igmp_clients_sem);
          return ctrlResMsg.res;
        }
...
L7_RC_t ptin_igmp_stat_instanceIntf_clear(L7_uint32 evc_idx, ptin_intf_t *ptin_intf)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"Error getting IGMP stats for EVC %u",evc_idx);
    return L7_FAILURE;
  }
#else
  ...
  {
    LOG_ERR(LOG_CTX_EVC, "ptin_intf %u/%u is invalid", ptin_intf->intf_type, ptin_intf->intf_id);
    return L7_FAILURE;
  }
  ...
  if (ptin_igmp_instance_find_fromSingleEvcId(evc_idx, &igmp_idx) != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP,"No Igmp instance found for EVC %u",evc_idx);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_stat_client_clear(L7_uint32 evc_idx, const ptin_client_id_t *client_id)
{
  ...
  for (clientId=0; clientId<PTIN_IGMP_CLIENTIDX_MAX; ++clientId)
  {
    ...
    if (IS_BITMAP_BIT_SET(clientGroup->client_bmp_list, clientId, UINT32_BITSIZE))
    {
      ...
      if (L7_SUCCESS != ctrlResMsg.res)
      {
        LOG_ERR(LOG_CTX_IGMP, "Error reading clientId[%u] statistics", clientId);
        return ctrlResMsg.res;
      }
    }
  }
...
/*End IGMPv3 Counters*/
  ...
  LOG_NOTICE(LOG_CTX_IGMP,"statistics Field:%u",field);
  LOG_NOTICE(LOG_CTX_IGMP,"statPortG:%u",statPortG);
  LOG_NOTICE(LOG_CTX_IGMP,"statPort:%u",statPort);
  LOG_NOTICE(LOG_CTX_IGMP,"statClient:%u",statClient);
...
L7_RC_t ptin_igmp_mgmd_port_sync(L7_uint8 admin, L7_uint32 serviceId, L7_uint32 portId, L7_uint32 groupAddr, L7_uint32 sourceAddr, L7_uint8 groupType)
{
  ...
  LOG_TRACE(LOG_CTX_IGMP, "Received request to sync port");
  ...
  if (ptin_fgpa_mx_is_matrixactive() == 1)
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to get lag index from slot ID [slotId:%u]", slotId);
      return L7_FAILURE;
    }
    if (L7_SUCCESS != ptin_intf_lag2intIfNum(lagId, &portId))
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to get intfnum from lag index [lagId:%u]", lagId);
      return L7_FAILURE;
    }
  }
#endif
  ...
  if (admin == L7_ENABLE)
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Going to open port [intfNum:%u]", portId);
  ...
  {
    LOG_DEBUG(LOG_CTX_IGMP, "Going to close port [intfNum:%u]", portId); 
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Unknown admin value %u", admin);
    return L7_FAILURE;
  }
...
extern L7_RC_t ptin_igmp_admission_control_verify_the_presence_of_other_clients(L7_uint32 ptin_port, L7_uint32 clientId, L7_uchar8 *clientBmpPtr)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments: ptin_port:%u clientId:%u clientBmpPtr:%p",ptin_port, clientId, clientBmpPtr);
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Arguments [ptin_port:%u clientId:%u]",ptin_port, clientId);

  osapiSemaTake(ptin_igmp_clients_sem, L7_WAIT_FOREVER);  
  if ( (ptinIgmpClientGroupInfoData = deviceClientId2groupClientPtr(ptin_port, clientId)) == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP, "Unable to find any valid clientGroup [ptin_port:%u clientId:%u]",ptin_port, clientId);    
    osapiSemaGive(ptin_igmp_clients_sem);
    return L7_FAILURE;
  }
  ...
  while ( (client_device = igmp_clientDevice_next(ptinIgmpClientGroupInfoData, client_device)) != L7_NULLPTR)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_DEBUG(LOG_CTX_IGMP, "This groupclient has more than one device watching this stream [ptin_port:%u clientId:%u clientId2:%u]",ptin_port, clientId, client_device->client->deviceClientId);
      osapiSemaGive(ptin_igmp_clients_sem);
...
extern L7_RC_t ptin_igmp_admission_control_verify_the_presence_of_other_groupclients(L7_uint32 ptin_port, L7_uint32 clientId, L7_uchar8 *clientBmpPtr)
{
  ...
  for (iterator = 0; iterator < PTIN_IGMP_CLIENTIDX_MAX; iterator++)
  {
    ...
    if ( PTIN_CLIENT_IS_MASKBITSET(clientBmpPtr, iterator) == L7_TRUE)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Unable to find any valid clientGroup [ptin_port:%u clientId:%u]",ptin_port, iterator);    
        osapiSemaGive(ptin_igmp_clients_sem);
        return L7_FAILURE;
      }
      ...
      if (ptin_debug_igmp_snooping)
        LOG_DEBUG(LOG_CTX_IGMP, "This ONU/TA48GE Port has more than one groupclient watching this stream [ptin_port:%u clientId:%u clientId2:%u]",ptin_port, clientId, iterator);
      osapiSemaGive(ptin_igmp_clients_sem);
      return L7_ALREADY_CONFIGURED;
    }
  }
...
static void ptin_igmp_channel_bandwidth_cache_set(ptinIgmpChannelInfoData_t* ptinIgmpPairInfoData)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments: ptinIgmpPairInfoData:%p",ptinIgmpPairInfoData);
    return;
  }
  ...
  if (ptin_debug_igmp_snooping)
  {
    ...
    LOG_TRACE(LOG_CTX_IGMP,"Cache Channel Bandwidth [inUse:%s evc_mc:%u groupAddr:%s sourceAddr:%s bandwidth:%u kbps]",ptinIgmpChannelBandwidthCache.inUse?"Yes":"No", 
...
static void ptin_igmp_channel_bandwidth_cache_unset(ptinIgmpChannelDataKey_t* ptinIgmpPairDataKey)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments: ptinIgmpPairDataKey:%p",ptinIgmpPairDataKey);
    return;
  }
     ...
     )
  {
    if (ptin_debug_igmp_snooping)
    {
      char  groupAddrStr[IPV6_DISP_ADDR_LEN]={};
      char  sourceAddrStr[IPV6_DISP_ADDR_LEN]={};   
      LOG_TRACE(LOG_CTX_IGMP,"Uncache Channel Bandwidth [inUse:%s evc_mc:%u groupAddr:%s sourceAddr:%s bandwidth:%u kbps]",ptinIgmpChannelBandwidthCache.inUse?"Yes":"No",
                 ptinIgmpChannelBandwidthCache.evc_mc, inetAddrPrint(&ptinIgmpChannelBandwidthCache.channel_group,groupAddrStr), 
                inetAddrPrint(&ptinIgmpChannelBandwidthCache.channel_source,sourceAddrStr), ptinIgmpChannelBandwidthCache.channelBandwidth); 
    }
...
static ptinIgmpChannelBandwidthCache_t* ptin_igmp_channel_bandwidth_cache_get(void)
{
  if (ptin_debug_igmp_snooping)
  {
    char  groupAddrStr[IPV6_DISP_ADDR_LEN]={};
    char  sourceAddrStr[IPV6_DISP_ADDR_LEN]={};
    LOG_TRACE(LOG_CTX_IGMP,"Cached Channel Bandwidth [inUse:%s evc_mc:%u groupAddr:%s sourceAddr:%s bandwidth:%u kbps]",ptinIgmpChannelBandwidthCache.inUse?"Yes":"No",
               ptinIgmpChannelBandwidthCache.evc_mc, inetAddrPrint(&ptinIgmpChannelBandwidthCache.channel_group,groupAddrStr), 
              inetAddrPrint(&ptinIgmpChannelBandwidthCache.channel_source,sourceAddrStr), ptinIgmpChannelBandwidthCache.channelBandwidth); 
  }
...
RC_t ptin_igmp_admission_control_port_set(L7_uint32 ptin_port, L7_uint8 mask, L7_uint16 maxAllowedChannels, L7_uint64 maxAllowedBandwidth)  
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%ull]",ptin_port, mask, maxAllowedChannels, maxAllowedBandwidth);    
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input arguments [ptin_port:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%ull]",ptin_port, mask, maxAllowedChannels, maxAllowedBandwidth);
...
static L7_uint8 ptin_igmp_admission_control_multicast_internal_id_set(L7_uint32 externalServiceId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [externalServiceId:%u]", externalServiceId);    
    return((L7_uint8) -1);
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input arguments [externalServiceId:%u]", externalServiceId);
  ...
  for (iterator = 0; iterator<PTIN_IGMP_MAX_MULTICAST_INTERNAL_SERVICE_ID; iterator++)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP, "Output arguments [internalServiceId:%u]", iterator);
      return(iterator);
    }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output arguments [internalServiceId:%u]", firstFree);
...
void ptin_igmp_admission_control_multicast_internal_id_unset(L7_uint32 externalServiceId)
{
  ...
  if (ptin_debug_igmp_snooping)
    LOG_NOTICE(LOG_CTX_IGMP, "Failed to find internal service id [externalServiceId:%u]", externalServiceId);
...
static L7_uint8 ptin_igmp_admission_control_multicast_internal_id_get(L7_uint32 externalServiceId)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_TRACE(LOG_CTX_IGMP, "Output arguments [internalServiceId:%u]", ptinIgmpAdmissionControlMulticastInternalServiceId[externalServiceId]);
...
RC_t ptin_igmp_admission_control_multicast_service_set(ptin_igmp_admission_control_t *igmpAdmissionControl)  
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments igmpAdmissionControl:%p", igmpAdmissionControl);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%ull]",igmpAdmissionControl->ptin_port, igmpAdmissionControl->onuId, igmpAdmissionControl->serviceId, internalServiceId, igmpAdmissionControl->mask, igmpAdmissionControl->maxAllowedChannels, igmpAdmissionControl->maxAllowedBandwidth);    
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Arguments [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%ull]",igmpAdmissionControl->ptin_port, igmpAdmissionControl->onuId, igmpAdmissionControl->serviceId, internalServiceId, igmpAdmissionControl->mask, igmpAdmissionControl->maxAllowedChannels, igmpAdmissionControl->maxAllowedBandwidth);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Arguments [mask:0x%x maxAllowedChannels:%du maxAllowedBandwidth:%u allocatedChannels:%u allocatedBandwidth:%u]",
...
static igmpMulticastAdmissionControl_t* ptin_igmp_admission_control_multicast_service_get(L7_uint32 ptin_port, L7_uint32 serviceId, L7_uint8 onuId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u]", ptin_port, onuId, serviceId, internalServiceId);    
    return L7_NULLPTR;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input arguments [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u]", ptin_port, onuId, serviceId, internalServiceId);

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Arguments [mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%u (kbps) allocatedChannels:%u allocatedBandwidth:%u (kbps)]",
...
#error "Parameter Currently Not Supported!"
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid input arguments group:%p channel_source:%p channel_bandwidth:%p", channel_group, channel_source, channel_bandwidth);
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Group 0x%08x / Source does 0x%08x not exist.",
  ...
  *channel_bandwidth = ptinIgmpPairInfoData->channelBandwidth;
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Group 0x%08x / Source 0x%08x requires  %u kbit/s of available bandwidth", channel_group->addr.ipv4.s_addr, channel_source->addr.ipv4.s_addr, *channel_bandwidth);
...
static RC_t ptin_igmp_admission_control_available(ptinIgmpAdmissionControl_t* admissionControlPtr, L7_uint32 channelBandwidth)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid Input Parameters:[admissionControlPtr:%p channelBandwidth:%u]", admissionControlPtr, channelBandwidth);
    return L7_FAILURE;
  }
  ...
  if ( globalAdmissionControl == L7_FALSE )
  {
    ...
    if (ptin_debug_igmp_snooping)
      LOG_INFO(LOG_CTX_IGMP, "Global Admission Control Feature is Disabled");
    return L7_SUCCESS;
  }
  ...
  case (PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS | PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH):          
    ...
    case (PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS | PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH):
           ...
           admissionControlPtr->allocatedBandwidth + channelBandwidth > admissionControlPtr->maxAllowedBandwidth)
      {
        ...
        if (ptin_debug_igmp_snooping)
          LOG_NOTICE(LOG_CTX_IGMP, "Not have enough resources to perform this operation"
      ...
      //Enough Resources
      if (ptin_debug_igmp_snooping)
        LOG_DEBUG(LOG_CTX_IGMP, "Enough resources to perform this operation"
                  "[allocatedChannels:%u maxChannels:%u | allocatedBandwidth:%u maxAllowedBandwidth:%u]",
                  admissionControlPtr->allocatedChannels, admissionControlPtr->maxAllowedChannels,
                  admissionControlPtr->allocatedBandwidth, admissionControlPtr->maxAllowedBandwidth);
...
//          admissionControlPtr->allocatedBandwidth += channelBandwidth;
    ...
    case PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS:           
      if (admissionControlPtr->allocatedChannels + 1 > admissionControlPtr->maxAllowedChannels)
      {
        //Not Enough Resources
        if (ptin_debug_igmp_snooping)
          LOG_NOTICE(LOG_CTX_IGMP, "Not have enough resources to perform this operation [allocatedChannels:%u maxChannels:%u]",                
                     admissionControlPtr->allocatedChannels, admissionControlPtr->maxAllowedChannels);
        return L7_FAILURE;
      }
...
//          ++admissionControlPtr->allocatedChannels;
    ...
    case PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH:
      if (admissionControlPtr->allocatedBandwidth + channelBandwidth > admissionControlPtr->maxAllowedBandwidth)
      {
        //Not Enough Resources
        if (ptin_debug_igmp_snooping)
          LOG_NOTICE(LOG_CTX_IGMP, "Not enough resources to perform this operation [allocatedBandwidth:%u maxAllowedBandwidth:%u]",                     
                     admissionControlPtr->allocatedBandwidth, admissionControlPtr->maxAllowedBandwidth);
        return L7_FAILURE;
      }
...
//          admissionControlPtr->allocatedBandwidth += channelBandwidth;
    ...
    default:
      ...
      if (ptin_debug_igmp_snooping)
        LOG_DEBUG(LOG_CTX_IGMP, "The admission control feature is disabled");
      return L7_SUCCESS;              
    }
    break;                    
  case PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS: 
  ...
  case PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH:  
    if ( (admissionControlPtr->mask & PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH) == PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH)
    {
      if (admissionControlPtr->allocatedBandwidth + channelBandwidth > admissionControlPtr->maxAllowedBandwidth)
      {
        //Not Enough Resources
        if (ptin_debug_igmp_snooping)
          LOG_NOTICE(LOG_CTX_IGMP, "Not have enough resources to perform this operation [allocatedBandwidth:%u maxAllowedBandwidth:%u]",                     
                     admissionControlPtr->allocatedBandwidth, admissionControlPtr->maxAllowedBandwidth);
        return L7_FAILURE;
      }
...
//          admissionControlPtr->allocatedBandwidth += channelBandwidth;
  ...
  //Not Enough Resources
  LOG_ERR(LOG_CTX_IGMP, "Ooops Something went...wrong!");
  return L7_FAILURE;   
}
...
static RC_t ptin_igmp_admission_control_release(ptinIgmpAdmissionControl_t* admissionControlPtr, L7_uint32 channelBandwidth)
{
  ...
  case (PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS | PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH):          
    ...
    case (PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_CHANNELS | PTIN_IGMP_ADMISSION_CONTROL_MASK_MAX_ALLOWED_BANDWIDTH):
      ...
      {
        LOG_NOTICE(LOG_CTX_IGMP, "[allocatedChannels:%u maxChannels:%u]",                     
                   admissionControlPtr->allocatedChannels, admissionControlPtr->maxAllowedChannels);
      }
      ...
      {
        if (ptin_debug_igmp_snooping)
          LOG_NOTICE(LOG_CTX_IGMP, "[allocatedBandwidth:%u maxAllowedBandwidth:%u]",                     
                     admissionControlPtr->allocatedBandwidth, admissionControlPtr->maxAllowedBandwidth);
...
L7_uint8 ptin_igmp_client_id_to_onu_id(L7_uint32 ptin_port, L7_uint32 clientId)
{
  ...
  if (ptin_port >= PTIN_SYSTEM_N_UPLINK_INTERF ||  clientId >= PTIN_IGMP_CLIENTIDX_MAX)
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u clientId:%u ]",ptin_port, clientId);    
    return((L7_uint8) -1);
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input arguments [ptin_port:%u clientId:%u]",
              ptin_port, clientId);
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output arguments [onuId:%u]",
...
RC_t ptin_igmp_multicast_service_resources_available(L7_uint32 ptin_port, L7_uint32 clientId, L7_uint32 serviceId, L7_uint32 channelBandwidth)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u kbps]",
            ptin_port, clientId, onuId, serviceId, channelBandwidth);    
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input arguments [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u kbps]",
              ptin_port, clientId, onuId, serviceId, channelBandwidth);
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid Parameters: ptinIgmpAdmissionControlPtr:%p",ptinIgmpAdmissionControlPtr);
    return L7_FAILURE;
  }
  ...
  if (L7_SUCCESS != (rc = ptin_igmp_admission_control_available(&ptinIgmpAdmissionControlPtr->admissionControl, channelBandwidth)))
  {
    //Not Enough Resources    
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Service without Resources [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u kbps]",
                 ptin_port, clientId, onuId, serviceId, channelBandwidth);
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u (kbps) mask:%u maxAllowedChannels:%u maxAllowedBandwidth:%u allocatedChannels:%u allocatedBandwidth:%u]",
              ptin_port, clientId, onuId, serviceId, channelBandwidth, 
...
RC_t ptin_igmp_multicast_service_resources_allocate(L7_uint32 ptin_port, L7_uint32 clientId, L7_uint32 serviceId, L7_uint32 channelBandwidth)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Allocate Multicast Service Resources [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u kbps]",
                 ptin_port, clientId, onuId, serviceId, channelBandwidth);
  }
...
RC_t ptin_igmp_multicast_service_resources_release(L7_uint32 ptin_port, L7_uint32 clientId, L7_uint32 serviceId, L7_uint32 channelBandwidth)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Release Multicast Service Resources [ptin_port:%u clientId:%u (onuId:%u) serviceId:%u channelBandwidth:%u kbps]",
                 ptin_port, clientId, onuId, serviceId, channelBandwidth);
  }
...
RC_t ptin_igmp_port_resources_available(L7_uint32 ptin_port, L7_uint32 channelBandwidth)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u channelBandwidth:%u kbps ptinIgmpAdmissionControlPtr:%p]",ptin_port, channelBandwidth, ptinIgmpAdmissionControlPtr);    
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [ptin_port:%u channelBandwidth:%u kbps]",ptin_port, channelBandwidth);
  ...
  if (L7_SUCCESS != (rc = ptin_igmp_admission_control_available(&ptinIgmpAdmissionControlPtr->admissionControl, channelBandwidth)))
  {
    //Not Enough Resources    
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Port without Resources [ptin_port:%u channelBandwidth:%u kbps]",ptin_port, channelBandwidth);
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptin_port:%u channelBandwidth:%u kbps mask:%u maxAllowedChannels:%u maxAllowedBandwidth:%u allocatedChannels:%u allocatedBandwidth:%u]",
...
RC_t ptin_igmp_port_resources_allocate(L7_uint32 ptin_port, L7_uint32 channelBandwidth)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Allocate Port Resources [ptin_port:%u channelBandwidth:%u kbps]",ptin_port, channelBandwidth);
  }
...
RC_t ptin_igmp_port_resources_release(L7_uint32 ptin_port, L7_uint32 channelBandwidth)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Release Port Resources [ptin_port:%u channelBandwidth:%u kbps]",ptin_port, channelBandwidth);
  }
...
#error "This Mode Is Currently Not Supported!"
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u clientId:%u channelBandwidth:%u kbps]",ptin_port, clientId, channelBandwidth);    
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [ptin_port:%u clientId:%u channelBandwidth:%u kbps]",ptin_port, clientId, channelBandwidth);
  ...
  if (L7_SUCCESS != (rc = ptin_igmp_admission_control_available(&ptinIgmpClientGroupInfoData->admissionControl, channelBandwidth)))
  {
    //Not Enough Resources
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Client without resources [ptin_port:%u clientId:%u channelBandwidth:%u kbps]",ptin_port, clientId, channelBandwidth);
  }

  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptin_port:%u clientId:%u channelBandwidth:%u kbps mask:%u maxAllowedChannels:%u maxAllowedBandwidth:%u allocatedChannels:%u allocatedBandwidth:%u]",
...
#error "This Mode Is Currently Not Supported!"
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Allocate Client Resources [ptin_port:%u clientId:%u channelBandwidth:%u kbps]",ptin_port, clientId, channelBandwidth);
  }
...
#error "This Mode Is Currently Not Supported!"
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Failed to Release Client Resources[ptin_port:%u clientId:%u channelBandwidth:%u kbps]",ptin_port, clientId, channelBandwidth);
  }
...
L7_RC_t ptin_igmp_clients_bmp_get(L7_uint32 extendedEvcId, L7_uint32 intIfNum, L7_uchar8 *clientBmpPtr, L7_uint32 *noOfClients)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Invalid input parameters: [clientBmpPtrList=%p  noOfClients=%p]",clientBmpPtr,noOfClients);
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
  {
    LOG_TRACE(LOG_CTX_IGMP,"List of clients (%u clients):",igmpDeviceClients.number_of_clients);
  }
  ...
  *noOfClients=0;
          ...
          ) != L7_NULLPTR )
  {
    ...
    if (ptin_debug_igmp_snooping)
    {
      LOG_TRACE(LOG_CTX_IGMP,"      Client#%u: "
#if (MC_CLIENT_INTERF_SUPPORTED)
                "ptin_port=%-2u "
#endif
#if (MC_CLIENT_OUTERVLAN_SUPPORTED)
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Unable to convert port2intIfNum :%u",avl_info->igmpClientDataKey.ptin_port);
      continue;
    }
...
#endif
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Unable to get external EVC Id :%u",avl_info->igmpClientDataKey.outerVlan);
...
//      continue;
...
#endif
      {
        ...
        (*noOfClients)++;
        if (ptin_debug_igmp_snooping)
          LOG_TRACE(LOG_CTX_IGMP, "Client Found [ServiceId:%u PortId:%u ClientId:%u]",clientExtendedEvcId,clientIntIfNum,avl_info->deviceClientId);
      }
    }
...
L7_RC_t ptin_igmp_groupclients_bmp_get(L7_uint32 extendedEvcId, L7_uint32 intIfNum, L7_uchar8 *clientBmpPtr, L7_uint32 *noOfClients)
{
  ...
  if (intIfNum==0 || intIfNum >= L7_MAX_INTERFACE_COUNT)
  {
    LOG_ERR(LOG_CTX_IGMP, "intIfNum# %u is out of range [1..%u]", intIfNum, L7_MAX_INTERFACE_COUNT);
    return L7_FAILURE;
  }

  if (extendedEvcId >= PTIN_SYSTEM_N_EXTENDED_EVCS)
  {
    LOG_ERR(LOG_CTX_IGMP, "extendedEvcId# %u is out of range [0..%u]", extendedEvcId, PTIN_SYSTEM_N_EXTENDED_EVCS);
    return L7_FAILURE;
  }

  if (clientBmpPtr == L7_NULLPTR || noOfClients == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid Input Parameters: clientBmpPtr=%p noOfClients=%p",clientBmpPtr, noOfClients);
  }
  ...
  *noOfClients = 0;
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Unable to convert intIfNum:%u",intIfNum);    
    return L7_FAILURE;
  }
  ...
  for ( client_idx = 0; client_idx < PTIN_IGMP_CLIENTIDX_MAX; client_idx++)
  {
    ...
    if ( (client_device=igmp_clientDevice_next(clientGroup, client_device)) != L7_NULLPTR && client_device->client != L7_NULLPTR)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Unable to get external EVC Id for outerVlan:%u", clientGroup->igmpClientDataKey.outerVlan);        
        continue;
      }
      if (clientExtendedEvcId != extendedEvcId)
      {
      ...
      if (ptin_debug_igmp_snooping)
      {
        LOG_TRACE(LOG_CTX_IGMP,"Client Found [extendedEvcId:%u ptin_port:%u clientId:%u]",extendedEvcId, ptin_port, client_device->client->deviceClientId);
      }
    }
  }
  ...
  if (ptin_debug_igmp_snooping)
  {
    LOG_TRACE(LOG_CTX_IGMP,"Number of Clients found [extendedEvcId:%u ptin_port:%u noOfClients:%u]",extendedEvcId, ptin_port, *noOfClients);
  }
  ...
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP,"List of Group clients (%u clients):\n",igmpClientGroups.number_of_clients);
          ...
          ) != L7_NULLPTR )
  {
         ...
         (client_device=igmp_clientDevice_next(clientGroup, client_device)) != L7_NULLPTR)
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Unable to convert port2intIfNum :%u",clientGroup->igmpClientDataKey.ptin_port);
        continue;
      }
      if (clientIntIfNum==intIfNum)
#endif
      {
...
#endif
        {
          ...
          (*noOfClients)++;
          if (ptin_debug_igmp_snooping)
            LOG_TRACE(LOG_CTX_IGMP, "Client Found [ServiceId:%u PortId:%u ClientId:%u]",clientExtendedEvcId, clientIntIfNum,client_device->client->client_index);
        }
      }
    }
...
RC_t ptin_igmp_multicast_package_add(L7_uint32 packageId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u]",packageId);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u]",
              packageId);
  ...
  if (multicastPackage[packageId].inUse == L7_FALSE)
  {
    ...
    LOG_DEBUG(LOG_CTX_IGMP, "Multicast Package Created [packageId:%u noOfMulticastPackages:%u]",
  ...
  {
    if (ptin_debug_igmp_snooping)  
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Package Already Created [packageId:%u noOfMulticastPackages:%u]",
               packageId, noOfMulticastPackages);
...
RC_t ptin_igmp_multicast_package_remove(L7_uint32 packageId)
{
  ...
  {
    if (ptin_debug_igmp_snooping)  
      LOG_NOTICE(LOG_CTX_IGMP, "Package does not exist [packageId:%u]",packageId);        
    #if 0//Do Not Signal this to the caller
    return L7_NOT_EXIST;
    #else
    return L7_SUCCESS;    
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Package contains channels [noOfChannels:%u]", multicastPackage[packageId].queueChannel.n_elems);    
    return L7_DEPENDENCY_NOT_MET;
  }

  if ( multicastPackage[packageId].noOfPorts != 0 )
  {
    LOG_ERR(LOG_CTX_IGMP, "Package contain clients on ports [noOfPorts:%u]", multicastPackage[packageId].noOfPorts);    
    return L7_DEPENDENCY_NOT_MET;
  }
    ...
    for (ptinPort = 0; ptinPort <= PTIN_SYSTEM_N_UPLINK_INTERF; ptinPort++)
    {
      for (groupClientId = 0; groupClientId <= PTIN_IGMP_CLIENTIDX_MAX; groupClientId++)
      {
        ...
        if (IS_BITMAP_BIT_SET(channelAvlTreeEntry->groupClientBmpPerPort, groupClientId, UINT32_BITSIZE))
        {
          ...
          {
            LOG_ERR(LOG_CTX_IGMP,"Failed to remove client from channel [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",
  ...
  LOG_DEBUG(LOG_CTX_IGMP, "Multicast Package Removed [packageId:%u noOfMulticastPackages:%u]",
            packageId, noOfMulticastPackages);
...
RC_t ptin_igmp_multicast_package_channels_add(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupNetAddr, L7_uint32 groupMask, L7_inet_addr_t *sourceNetAddr, L7_uint32 sourceMask)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u serviceId:%u groupNetAddr:%p sourceNetAddr:%p]",packageId, serviceId, groupNetAddr, sourceNetAddr);    
    return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Family not supported groupNetAddr->family:%u sourceNetAddr->family:%u", groupNetAddr->family, sourceNetAddr->family);
    return L7_NOT_SUPPORTED;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u serviceId:%u groupNetAddr:%s groupMask:%u sourceNetAddr:%s sourceMask:%u]",
  ...
  if (multicastPackage[packageId].inUse == L7_FALSE)
  {
    LOG_WARNING(LOG_CTX_IGMP, "Multicast Package Not Created [packageId:%u serviceId:%u groupNetAddr:%s groupMask:%u sourceNetAddr:%s sourceMask:%u]",
                packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_NOT_EXIST;
  ...
  {
    LOG_TRACE(LOG_CTX_IGMP, "Multicast Package Does Exist [packageId:%u serviceId:%u groupNetAddr:%s groupMask:%u sourceNetAddr:%s sourceMask:%u",
              packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
  }
  ...
  {
    if (ptin_debug_igmp_snooping)  
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Package Created with Empty Channels [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u",
               packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Failed to obtain groupAddr:%s from groupMask:%u rc:%u!", inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, rc);
    return L7_FAILURE;    
  }
  /* Validate output ip address */
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to add groupAddr:%s (%u addresses)", inetAddrPrint(&groupAddr, groupAddrStr), noOfGroups);
  ...
  {   
    ...
    for (sourceIterator = 0; sourceIterator<noOfSources; sourceIterator++, sourceAddrAux.addr.ipv4.s_addr++)
    {
      ...
      if ( rc == L7_NOT_EXIST )
      {
        LOG_WARNING(LOG_CTX_IGMP, "Channel Does Not Exist [serviceId:%u groupAddr:%s sourceAddr:%s]",
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to Search Channel Entry [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%p channelEntry:%p]", packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), inetAddrPrint(&sourceAddrAux, sourceAddrStr), channelAvlTreeEntry);    
        return rc;
      }

      if ( L7_SUCCESS != (rc = ptin_igmp_package_channel_conflict_validation(packageId, channelAvlTreeEntry) ) )
      {
        LOG_ERR(LOG_CTX_IGMP, "Channel Conflict Found [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), inetAddrPrint(&sourceAddrAux, sourceAddrStr));    
        return rc;
      }      
    }
  }

  LOG_TRACE(LOG_CTX_IGMP,"Going to add groupAddr:%s (%u addresses)", inetAddrPrint(&sourceAddr, sourceAddrStr), noOfSources);
  ...
  while ((rc==L7_SUCCESS && groupIterator<noOfGroups) || ( rc!= L7_SUCCESS && groupIterator>=0))
  {
    ...
    while ( ( rc == L7_SUCCESS && sourceIterator<noOfSources ) || ( rc != L7_SUCCESS && sourceIterator>=0 ) )
    {
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Error adding channel to multicast package [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u]",
        ...
        {
          LOG_TRACE(LOG_CTX_IGMP,"Added channel to multicast package [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u]",
                    packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), groupMask, inetAddrPrint(&sourceAddrAux, sourceAddrStr), sourceMask);
        }
      }
      /* If one error ocurred, remove previously added nodes */
      else
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Error removing channel from multicast package [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u]",
                  packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), groupMask, inetAddrPrint(&sourceAddrAux, sourceAddrStr), sourceMask);
        }
        else
        {
          LOG_TRACE(LOG_CTX_IGMP,"Removed channel from multicast package [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u]",
                    packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), groupMask, inetAddrPrint(&sourceAddrAux, sourceAddrStr), sourceMask);
        }
      }
...
RC_t ptin_igmp_multicast_package_channels_remove(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupNetAddr, L7_uint32 groupMask, L7_inet_addr_t *sourceNetAddr, L7_uint32 sourceMask)
{
  ...
  {
    if (ptin_debug_igmp_snooping)  
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Package Does Not Exists [packageId:%u serviceId:%u groupNetAddr:%s groupMask:%u sourceNetAddr:%s sourceMask:%u",
               packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_NOT_EXIST;
  ...
  {
    if (ptin_debug_igmp_snooping)  
      LOG_NOTICE(LOG_CTX_IGMP, "No Channels to Remove [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u",
               packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_SUCCESS;
  }
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to remove groupAddr:%s (%u addresses)", inetAddrPrint(&groupAddr, groupAddrStr), noOfGroups);
  ...
  LOG_TRACE(LOG_CTX_IGMP,"Going to remove groupAddr:%s (%u addresses)", inetAddrPrint(&sourceAddr, sourceAddrStr), noOfSources);
...
static RC_t ptin_igmp_multicast_package_channel_add(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u serviceId:%u groupAddr:%p sourceAddr:%p]",packageId, serviceId, groupAddr, sourceAddr);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",
              packageId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr));

  if (multicastPackage[packageId].inUse == L7_FALSE)
  {
    LOG_WARNING(LOG_CTX_IGMP, "Multicast Package Does Not Exist [packageId:%u]", packageId);
    return L7_NOT_EXIST;   
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to Search Channel Entry [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%p channelEntry:%p]", packageId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr), channelAvlTreeEntry);    
    return rc;
  }
  ...
  if ( L7_SUCCESS != (rc = queue_channel_entry_add(packageId, channelAvlTreeEntry) ) )
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Add Channel to Queue [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",packageId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr));    
      return rc;
    }
    else
    {
  ...
  {    
    for ( portIterator = 0; portIterator < PTIN_SYSTEM_N_UPLINK_INTERF && noOfPortsFound < multicastPackage[packageId].noOfPorts; portIterator++)
    {
            ...
            groupClientIterator++)
      {
        ...
        {
          LOG_ERR(LOG_CTX_IGMP,"Failed to get groupClientPtr [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",
...
static RC_t ptin_igmp_multicast_package_channel_remove(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr)
{
  ...
  if ( rc == L7_NOT_EXIST )
  {
    ...
    if (ptin_debug_igmp_snooping)
      LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Not Exist [packageId:%u  serviceId:%u groupAddr:%s sourceAddr:%s]",
                packageId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr));
    #if 0//Do not return this rc to caller
    return L7_NOT_EXIST;
    #else
  ...
  if ( L7_SUCCESS != (rc = queue_channel_entry_remove(packageId, channelAvlTreeEntry) ) )
  {
    if ( L7_NOT_EXIST != rc)
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Remove Channel from Queue [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",packageId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr));    
      return rc;
    }
    else
    {
...
static struct channelPoolEntry_s* queue_channel_entry_get_next(L7_uint32 packageId, struct channelPoolEntry_s *channelEntry)
{
  ...
  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u channelEntry:%p]", packageId, channelEntry);
...
static RC_t queue_channel_entry_find(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry, struct channelPoolEntry_s **channelEntry)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u channelAvlTreeEntry:%p channelEntry:%u]", packageId, channelAvlTreeEntry, channelEntry);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u channelAvlTreeEntry:%p channelEntry:%u]",
    ...
    *channelEntry = (struct channelPoolEntry_s*) multicastPackage[packageId].queueChannel.head;
    while ( *channelEntry != L7_NULLPTR)
    {
      if ( (*channelEntry)->channelAvlTreeEntry == channelAvlTreeEntry )
      {
        /*Output Parameters*/
        if (ptin_debug_igmp_snooping)
          LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [channelEntryId:%u]",
...
static RC_t queue_channel_entry_add(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u channelAvlTreeEntry:%p]",packageId, channelAvlTreeEntry);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u channelAvlTreeEntry:%p]",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Queue is empty!");
    return L7_NO_MEMORY;
  }
  ...
  { 
    if (ptin_debug_igmp_snooping)   
      LOG_NOTICE(LOG_CTX_IGMP, "Channel Entry Already Added");        
    return L7_ALREADY_CONFIGURED;    
  }
  else
  {
    if (rc != L7_NOT_EXIST)
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Find Channel Entry [packageId:%u channelAvlTreeEntry:%p]",packageId, channelAvlTreeEntry);    
      return L7_FAILURE;
    }
  }
#endif
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Error removing element from free quueue: rc=%u channelEntryId:%p!", rc, channelEntryId);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_ERR(LOG_CTX_IGMP, "Error adding element to queue rc=%u!", rc);
    return rc;
  }
...
static RC_t queue_channel_entry_remove(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"All elements already removed (Free Queue is full: n_elems:%u)!", queueFreeChannelId.n_elems);        
    return L7_NOT_EXIST;    
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_NOTICE(LOG_CTX_IGMP, "Channel Entry Does Not Exist [packageId:%u channelAvlTreeEntry:%p]",packageId, channelAvlTreeEntry);          
      return L7_NOT_EXIST;      
    }
    else
    {    
      LOG_ERR(LOG_CTX_IGMP, "Failed to find element in queue: rc=%u!", rc);
      return rc;
    }
  }
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_IGMP, "Error removing element from queue: rc=%u!", rc);
    return rc;
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    LOG_ERR(LOG_CTX_IGMP, "Error adding element to free queue: rc=%u!", rc);
    return rc;
  }
...
static struct packagePoolEntry_s* queue_package_entry_get_next(ptinIgmpChannelInfoData_t *channelAvlTreeEntry, struct packagePoolEntry_s  *packageEntry)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [channelAvlTreeEntry:%p]", channelAvlTreeEntry);    
    return L7_NULLPTR;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [channelAvlTreeEntry:%p packageEntry:%p]", channelAvlTreeEntry, packageEntry);
...
static RC_t queue_package_entry_find(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry, struct packagePoolEntry_s  **packageEntry )
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u channelAvlTreeEntry:%p channelEntry:%u]", packageId, channelAvlTreeEntry, packageEntry);    
    return L7_FAILURE;
  }
    ...
    *packageEntry = (struct packagePoolEntry_s*) channelAvlTreeEntry->queuePackage.head;
    while ( *packageEntry != L7_NULLPTR)
    {
      if ( (*packageEntry)->packageId == packageId )
      {
        /*Output Parameters*/
        if (ptin_debug_igmp_snooping)
          LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [packageEntryId:%u]",
...
static RC_t queue_package_entry_add(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Package Entry Already Added");        
    return L7_ALREADY_CONFIGURED;    
  }
  else
  {
    if (rc != L7_NOT_EXIST)
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Find Package Entry [packageId:%u channelAvlTreeEntry:%p]",packageId, channelAvlTreeEntry);    
      return L7_FAILURE;
    }
  }
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Error removing element from free quueue: rc=%u packageEntryId:%p!", rc, packageEntryId);
    return rc;
  }
...
static RC_t queue_package_entry_remove(L7_uint32 packageId, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP,"All elements already removed (Free Queue is full: n_elems:%u)!", queueFreePackageId.n_elems);        
    return L7_NOT_EXIST;    
  }
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    {
      if (ptin_debug_igmp_snooping)
        LOG_NOTICE(LOG_CTX_IGMP, "Package Entry Does Not Exist [packageId:%u channelAvlTreeEntry:%p]",packageId, channelAvlTreeEntry);                
      return L7_NOT_EXIST;      
    }
    else
    {    
...
void ptin_igmp_multicast_service_dump(L7_uint32 ptinPort, L7_uint32 onuId, L7_uint32 serviceId)  
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptin_port:%u onuId:%u serviceId:%u]",ptinPort, onuId, serviceId);    
    return;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [ptinPort:%u onuId:%u serviceId:%u]",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to Obtain Internal Multicast Identifier [ptinPort:%u onuId:%u serviceId:%u internalServiceId:%u]",ptinPort, onuId, serviceId, internalServiceId);    
    return;
  }
...
RC_t ptin_igmp_multicast_service_add(L7_uint32 ptinPort, L7_uint32 onuId, L7_uint32 serviceId)  
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Service Already Added [ptinPort:%u onuId:%u serviceId:%u internalServiceId:%u noOfMulticastServices:%u]",
                ptinPort, onuId, serviceId, internalServiceId, multicastServices[ptinPort][onuId].noOfMulticastServices);
    #if 0//Do Not Signal this to the Caller
    return L7_ALREADY_CONFIGURED;
    #else
  ...
  /*Output Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptinPort:%u onuId:%u serviceId:%u internalServiceId:%u noOfMulticastServices:%u]",
              ptinPort, onuId, serviceId, internalServiceId, multicastServices[ptinPort][onuId].noOfMulticastServices);
...
RC_t ptin_igmp_multicast_service_remove(L7_uint32 ptinPort, L7_uint32 onuId, L7_uint32 serviceId)  
{
  ...
  if ( ptinPort >= PTIN_SYSTEM_N_UPLINK_INTERF || onuId >= PTIN_SYSTEM_IGMP_MAXONUS_PER_INTF || serviceId >= PTIN_SYSTEM_N_EXTENDED_EVCS )
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptinPort:%u onuId:%u serviceId:%u]",ptinPort, onuId, serviceId);    
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "Multicast Service Does Not Exist [ptinPort:%u onuId:%u serviceId:%u internalServiceId:%u noOfMulticastServices:%u]",
                ptinPort, onuId, serviceId, internalServiceId, multicastServices[ptinPort][onuId].noOfMulticastServices);
    #if 0//Do Not Signal this to the Caller
    return L7_NOT_EXIST;
    #else
...
RC_t ptin_igmp_multicast_service_get_next(L7_uint32 ptinPort, L7_uint32 onuId, L7_uint32 *serviceId)  
{
  ...
  if ( ptinPort >= PTIN_SYSTEM_N_UPLINK_INTERF || onuId >= PTIN_SYSTEM_IGMP_MAXONUS_PER_INTF || serviceId == L7_NULLPTR )
  {
    if (serviceId == L7_NULLPTR)
      LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptinPort:%u onuId:%u serviceId:%p]",ptinPort, onuId, serviceId);
    else
      LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptinPort:%u onuId:%u]",ptinPort, onuId);    
    return L7_FAILURE;
  }
  ...
  if (*serviceId != (L7_uint32) -1)
  {
    if ( (internalServiceId = ptin_igmp_admission_control_multicast_internal_id_get(*serviceId)) >= PTIN_IGMP_MAX_MULTICAST_INTERNAL_SERVICE_ID)
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Obtain Internal Multicast Identifier [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u]",ptinPort, onuId, serviceId, internalServiceId);    
      return L7_FAILURE;
    }
    ...
    while (++internalServiceId < PTIN_IGMP_MAX_MULTICAST_INTERNAL_SERVICE_ID)
    {
      ...
      *serviceId = multicastServiceId[ptinPort][onuId][internalServiceId].serviceId;
      ...
      /*Output Parameters*/
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptinPort:%u onuId:%u serviceId:%u (internalServiceId:%u) ]",
  ...
  /*Output Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP, "No Active/More Multicast Services Exist [ptinPort:%u onuId:%u serviceId:%u (internalServiceId:%u) ]",
              ptinPort, onuId, *serviceId, internalServiceId);
...
RC_t ptin_igmp_multicast_channel_service_get(L7_uint32 ptinPort, L7_uint32 deviceClientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint32 *serviceId)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptinPort:%u deviceClientId:%u groupAddr:%p sourceAddr:%p serviceId:%p]",ptinPort, deviceClientId, groupAddr, sourceAddr, serviceId);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [ptinPort:%u deviceClientId:%u groupAddr:%s sourceAddr:%s serviceId:%p]",
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Failed to Obtain groupClient [ptinPort:%u deviceClientId:%u serviceId:%u groupAddr:%p sourceAddr:%p serviceId:%p groupClient:%p]",ptinPort, deviceClientId, groupAddr, sourceAddr, serviceId, groupClient);    
    return L7_FAILURE;
  }
  ...
  if (multicastServices[ptinPort][onuId].noOfMulticastServices == 0)
  {
    /*Output Parameters*/  
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "No Multicast Service Configured [ptinPort:%u deviceClientId:%u onuId:%u groupAddr:%s sourceAddr:%s noOfMulticastServices:%u]",
  ...
  while (noOfServicesFound <  multicastServices[ptinPort][onuId].noOfMulticastServices )
  {
    if ( multicastServiceId[ptinPort][onuId][internalServiceId].inUse == L7_FALSE )
    {
      if (++internalServiceId >= PTIN_IGMP_MAX_MULTICAST_INTERNAL_SERVICE_ID)
      {
        ...
        if (ptin_debug_igmp_snooping)
          LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Not Exist [ptinPort:%u deviceClientId:%u onuId:%u groupAddr:%s sourceAddr:%s internalServiceId:%u noOfMulticastServices:%u]",
    ...
    if ( rc == L7_NOT_EXIST )
    {
      if (++internalServiceId >= PTIN_IGMP_MAX_MULTICAST_INTERNAL_SERVICE_ID)
      {
        /*Exit Here No More Internal Identifers Left and Entry Not Found*/
        if (ptin_debug_igmp_snooping)
          LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Not Exist [ptinPort:%u deviceClientId:%u onuId:%u groupAddr:%s sourceAddr:%s serviceId:%u internalServiceId:%u noOfMulticastServices:%u]",
    ...
    {
      LOG_ERR(LOG_CTX_IGMP, "Failed to Search Channel Entry [ptinPort:%u deviceClientId:%u onuId:%u serviceId:%u groupAddr:%p sourceAddr:%p serviceId:%u internalServiceId:%u noOfMulticastServices:%u]", 
              ptinPort, deviceClientId, onuId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr), serviceIdAux, internalServiceId, multicastServices[ptinPort][onuId].noOfMulticastServices);    
      return L7_FAILURE;
    }
    ...
    if (IS_BITMAP_BIT_SET(channelEntry->groupClientBmpPerPort[ptinPort], groupClientId, UINT32_BITSIZE) == L7_TRUE)
    {
      *serviceId = serviceIdAux;
      ...
      if (ptin_debug_igmp_snooping)
        LOG_TRACE(LOG_CTX_IGMP, "Entry Does Exist [ptinPort:%u deviceClientId:%u groupClientId:%u onuId:%u groupAddr:%s sourceAddr:%s serviceId:%u (internalServiceId:%u)]",
  ...
  if (useDefaultMCService == L7_TRUE)
  {
    *serviceId = channelDB.default_evc_mc;
    ...
    /*Exit Here Multicast Service Found*/
    if (ptin_debug_igmp_snooping)
      LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Exist Going to Use Default MC Service [ptinPort:%u deviceClientId:%u groupClientId:%u onuId:%u groupAddr:%s sourceAddr:%s serviceId:%u]",
  ...
  /*Output Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Not Exist [ptinPort:%u deviceClientId:%u groupClientId:%u onuId:%u groupAddr:%s sourceAddr:%s internalServiceId:%u noOfMulticastServices:%u]",
...
RC_t ptin_igmp_debug_multicast_channel_client_add(L7_uint32 packageId, L7_uint32 ptinPort, L7_uint32 groupClientId, L7_uint32 serviceId, L7_uint32 inGroupAddr, L7_uint32 inSourceAddr)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP,"Failed to get channel [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",
...
RC_t ptin_igmp_multicast_channel_client_add(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u groupClientPtr:%p channelEntry:%p]",packageId, groupClient, channelAvlTreeEntry);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u ptinPort:%u groupClientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]", packageId,
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "This groupClient was already added to this channel [ptin_port:%u groupClientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]", groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId, 
               channelAvlTreeEntry->channelDataKey.evc_mc, inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_group, groupAddrStr), inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_source, sourceAddrStr));    
    #if 0//DO Not Signal this to the Caller
    return L7_ALREADY_CONFIGURED;    
    #else
  ...
  { 
    LOG_ERR(LOG_CTX_IGMP,"Channel Conflict Found [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",
          groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId, channelAvlTreeEntry->channelDataKey.evc_mc, 
          inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_group, groupAddrStr), inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_source, sourceAddrStr));    
    return L7_FAILURE;
  }    
...
RC_t ptin_igmp_multicast_channel_client_remove(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, ptinIgmpChannelInfoData_t *channelAvlTreeEntry)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "This groupClient does not exist on this channel [ptin_port:%u groupClientId:%u serviceId:%u groupAddr:%s sourceAddr:%s]", groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId, 
               channelAvlTreeEntry->channelDataKey.evc_mc, inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_group, groupAddrStr), inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_source, sourceAddrStr));    
    #if 0//DO Not Signal this to the Caller
    return L7_NOT_EXIST;    
    #else
...
RC_t ptin_igmp_multicast_channel_client_get(L7_uint32 ptinPort, L7_uint32 groupClientId, L7_uint32 serviceId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_BOOL *isClientSet)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [ptinPort:%u clientId:%u serviceId:%u groupAddr:%p sourceAddr:%p isClientSet:%p]",ptinPort, groupClientId, serviceId, groupAddr, sourceAddr, isClientSet);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s isClientSet:%p]",
              ptinPort, groupClientId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr), isClientSet);

  /* Find Channel Entry */
  if ( ptin_igmp_channel_get( serviceId, groupAddr, sourceAddr,  &channelEntry) != L7_SUCCESS || channelEntry == L7_NULLPTR)
  {
    LOG_ERR(LOG_CTX_IGMP,"Failed to get channel [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s isClientSet:%p]",
            ptinPort, groupClientId, serviceId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr), isClientSet);
    return L7_FAILURE;
  }
  ...
  *isClientSet = IS_BITMAP_BIT_SET(channelEntry->groupClientBmpPerPort[ptinPort], groupClientId, UINT32_BITSIZE);
  ...
  /*Output Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Output Parameters [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s isBitSet:%s]",
...
static RC_t ptin_igmp_multicast_client_packages_add(L7_uint32 *packagePtr, L7_uint32 noOfPackages, ptinIgmpGroupClientInfoData_t *groupClient)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packagePtr:%p groupClient:%p]",packagePtr, groupClient);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packagePtr:%p groupClient:%p]",
  ...
  for (packageId = 0; packageId<= PTIN_SYSTEM_IGMP_MAXPACKAGES; packageId++)
  {
    ...
    if (IS_BITMAP_BIT_SET(packagePtr, packageId, UINT32_BITSIZE))
    {
      ...
      {
        LOG_ERR(LOG_CTX_IGMP, "Failed to add package [packageId:%p ptin_port:%u groupClientId:%u]",packagePtr, groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId);    
        return rc;
      }
...
static RC_t ptin_igmp_multicast_client_packages_remove(L7_uint32 *packagePtr, L7_uint32 noOfPackages, ptinIgmpGroupClientInfoData_t *groupClient)
{
  ...
  {
    LOG_WARNING(LOG_CTX_IGMP, "This client does not have packages associated [packageId:%p ptin_port:%u groupClientId:%u]",packagePtr, groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId);    
    return rc;
  }
  return rc;
}
...
static RC_t ptin_igmp_multicast_client_packages_remove_all(ptinIgmpGroupClientInfoData_t *groupClient)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [groupClient:%p]", groupClient);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [groupClient:%p]",
              groupClient);
...
static RC_t ptin_igmp_multicast_client_package_add(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u groupClient:%p]",packageId, groupClient);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u groupClient:%p]",
              packageId, groupClient);

  /*Let us Check if this Package is Configured*/
  if (multicastPackage[packageId].inUse == L7_FALSE )
  {
    LOG_ERR(LOG_CTX_IGMP, "This package does not exist [packageId:%u]", packageId);    
    return L7_DEPENDENCY_NOT_MET;
  }
  ...
  {
	if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "This package was already added to this groupClient [packageId:%u ptin_port:%u groupClientId:%u]", packageId, groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId);    
    #if 0//DO Not Signal this to the Caller
    return L7_ALREADY_CONFIGURED;    
    #else
    return L7_SUCCESS;    
...
static RC_t ptin_igmp_multicast_client_package_remove(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      LOG_NOTICE(LOG_CTX_IGMP, "This package does not belong to this groupClient [packageId:%u ptin_port:%u groupClientId:%u]", packageId, groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId);    
    #if 0//DO Not Signal this to the Caller
    return L7_NOT_EXIST;    
    #else
    return L7_SUCCESS;    
  ...
  else
  {
    ...
    LOG_WARNING(LOG_CTX_IGMP,"The number of packages in this channel is already zero [packageId:%u ptinPort:%u clientId:%u]",
...
RC_t ptin_igmp_multicast_client_package_get(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, L7_BOOL *isBitSet)
{
  ...
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u groupClient:%p isBitSet:%p]",packageId, groupClient);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u groupClient:%p isBitSet:%p]",
              packageId, groupClient);
...
static RC_t ptin_igmp_client_channel_conflict_validation(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, ptinIgmpChannelInfoData_t  *channelAvlTreeEntry)
{
  ...
  if ( packageId >= PTIN_SYSTEM_IGMP_MAXPACKAGES || groupClient == L7_NULLPTR  || channelAvlTreeEntry == L7_NULLPTR )
  {
    LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u groupClient:%p channelAvlTreeEntry:%p]",packageId, groupClient, channelAvlTreeEntry);    
    return L7_FAILURE;
  }

  /*Input Parameters*/
  if (ptin_debug_igmp_snooping)
    LOG_TRACE(LOG_CTX_IGMP, "Input Parameters [packageId:%u groupClient:%p channelAvlTreeEntry:%p]",
...
static RC_t ptin_igmp_client_channel_dependency_validation(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, ptinIgmpChannelInfoData_t  *channelAvlTreeEntry)
{
           ...
           (packageIdAux = packageEntry->packageId) < PTIN_SYSTEM_IGMP_MAXPACKAGES)
  {
    ...
    LOG_WARNING(LOG_CTX_IGMP,"Dependency not met: Client set within two packages for this channel [ptinPort:%u clientId:%u serviceId:%u groupAddr:%s sourceAddr:%s][packageId1:%u packageId2:%u]",
                groupClient->igmpClientDataKey.ptin_port, groupClient->groupClientId, channelAvlTreeEntry->channelDataKey.evc_mc, 
                inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_group, groupAddrStr), inetAddrPrint(&channelAvlTreeEntry->channelDataKey.channel_source, sourceAddrStr),
...
static RC_t ptin_igmp_package_channel_conflict_validation(L7_uint32 packageId, ptinIgmpChannelInfoData_t  *channelAvlTreeEntry)
{
          ...
          L7_NULLPTR != (channelAvlTreeEntryAux = channelEntry->channelAvlTreeEntry) )
  {
    ...
    {
      LOG_ERR(LOG_CTX_IGMP,"Conflict Found: Channel configured within two services [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s][serviceIdNew:]",
