...
L7_RC_t ptin_igmp_client_add(L7_uint32 evc_idx, const ptin_client_id_t *client_id, L7_uint16 uni_ovid, L7_uint16 uni_ivid, L7_uint8 onuId, L7_uint8 mask, L7_uint64 maxBandwidth, L7_uint16 maxChannels, L7_BOOL addOrRemove, L7_uint32 *packagePtr, L7_uint32 noOfPackages)
{
  ...
  if (rc!=L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error creating static client");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_igmp_client_delete(L7_uint32 evc_idx, const ptin_client_id_t *client_id)
{
  ...
  if (rc!=L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error deleting static client");
  }
...
L7_RC_t ptin_igmp_all_clients_flush(void)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error flushing all clients");
    return L7_FAILURE;
  }
                                     ...
                                     L7_uint *client_idx_ret)
{
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP,"New client created: intIfNum %u, intVlan %u, innerVlan %u",
            intIfNum, intVlan, innerVlan);

  if (rc!=L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error adding dynamic client");
  }
                                      ...
                                      ptinIgmpChannelInfoData_t **avlEntry )
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Invalid group address");
    return L7_FAILURE;
  }

  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP,"Channel_group=0x%08x, group_address=0x%08x!",
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Invalid source address");
    return L7_FAILURE;
  }
  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP,"Channel_source=0x%08x, source_address=0x%08x!",
  ...
  if ((avl_infoData=(ptinIgmpChannelInfoData_t *) avlSearchLVL7( &(channelDB.channelAvlTree), (void *)&avl_key, AVL_EXACT)) == L7_NULLPTR)
  {
    ptin_timer_stop(77);
    if (ptin_debug_igmp_snooping)
      PT_LOG_WARN(LOG_CTX_IGMP,"Group channel 0x%08x does not exist!",group_address.addr.ipv4.s_addr);
    return L7_NOT_EXIST;      
  }  
#if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
  else
                                ...
                                L7_BOOL is_static, L7_uint64 channelBandwidth)
{
  ...
  if (inetIsAddressZero(channel_group)==L7_TRUE && channel_grpMask==0)
  {
    ...
    PT_LOG_NOTICE(LOG_CTX_IGMP,"Added a Default MC Service Gateway [evc_mc:%u]", channelDB.default_evc_mc );
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error preparing groupAddr");
    return L7_FAILURE;    
  }
  ...
  if ( n_groups == 0 )
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Group address is not valid!");
    return L7_FAILURE;
  }
  ...
  {
    n_sources = 1;
    PT_LOG_WARN(LOG_CTX_IGMP,"Source address is not valid!");
  }  
#else
  /* source ip */  
  ...
  while ((rc==L7_SUCCESS && i<n_groups) || (rc!=L7_SUCCESS && i>=0))
  {
...
#endif
    {
      ...
      if (rc == L7_SUCCESS)
      {
        ...
        {
          PT_LOG_ERR(LOG_CTX_IGMP,"Error inserting group channel 0x%08x, source=0x%08x for UC_EVC=%u",
                    group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr, evc_uc);
        }
        else
        {
          PT_LOG_TRACE(LOG_CTX_IGMP,"Added group 0x%08x, source 0x%08x", group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr);
        }
      }
      ...
      else
      {
        ...
        {
          PT_LOG_TRACE(LOG_CTX_IGMP,"Removed group 0x%08x, source 0x%08x", group.addr.ipv4.s_addr, sourceAux.addr.ipv4.s_addr);
        }
      }
                                   ...
                                   L7_inet_addr_t *channel_source, L7_uint16 channel_srcMask, L7_uint8 isStatic)
{
  ...
  if (inetIsAddressZero(channel_group)==L7_TRUE && channel_grpMask==0)
  {
    ...
    PT_LOG_NOTICE(LOG_CTX_IGMP,"Removed Default MC Service Gateway [evc_mc:%u]", evc_mc);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Cannot add more than %u channels", PTIN_IGMP_CHANNELS_MAX);
    return L7_FAILURE;
  }

  PT_LOG_TRACE(LOG_CTX_IGMP,"Maximum addresses to be removed: %u", n_groups*n_sources);
  ...
  for (i=0; i<n_groups; i++)
  {
    ...
    for (j=0; j<n_sources; j++)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_IGMP,"No MC EVC associated with UC EVC %u",evc_uc);        
      }
      else
      {
...
L7_RC_t igmp_assoc_channel_clear( L7_uint32 evc_uc, L7_uint32 evc_mc )
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error removing all channels to MC evc %u, UC evc %u !", evc_mc, evc_uc);
    return L7_FAILURE;
  }

  PT_LOG_TRACE(LOG_CTX_IGMP,"Removed all channels to MC evc %u, UC evc %u !", evc_mc, evc_uc);
                                             ...
                                             L7_inet_addr_t *channel_out, L7_uint32 *number_of_channels)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Null pointer of provided address");
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Not supported IPv6");
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Not valid address: 0x%08x",channel_in->addr.ipv4.s_addr);
    return L7_FAILURE;
  }
  ...
  if ( number_of_channels != L7_NULLPTR )
  {
    ...
    for (i=0; i<mask_inv; i++)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_IGMP,"Mask too small (%u bits)",channel_mask);
        return L7_FAILURE;
      }
    }
    *number_of_channels = n;
    PT_LOG_TRACE(LOG_CTX_IGMP,"Bit mask %u, number of Addresses %u", channel_mask, n);
  }
...
static L7_RC_t ptin_igmp_channel_add( ptinIgmpChannelInfoData_t *node )
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"No more free entries! (number_of_entries:%u >= IGMPASSOC_CHANNELS_MAX:%u)", channelDB.number_of_entries, PTIN_IGMP_CHANNELS_MAX);
    return L7_FAILURE;
  }
...
static L7_RC_t ptin_igmp_channel_remove_multicast_service( L7_uint32 evc_uc, L7_uint32 evc_mc )
{
          ...
          ) != L7_NULLPTR )
  {
       ...
       )
    {
      ...
      if ( avlDeleteEntry(&(channelDB.channelAvlTree), (void *)&avl_key) == L7_NULLPTR )
      {
        PT_LOG_ERR(LOG_CTX_IGMP,"Error removing group channel 0x%08x to EVC_MC=%u, EVC=UC=%u",
      ...
      else
      {
        ...
        PT_LOG_TRACE(LOG_CTX_IGMP,"Removed group channel 0x%08x to EVC_MC=%u, EVC=UC=%u",
                  avl_key.channel_group.addr.ipv4.s_addr, evc_mc, evc_uc);
      }
    }
  }
...
static L7_RC_t ptin_igmp_instance_delete(L7_uint16 igmp_idx)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"IGMP instance %u is not active", igmp_idx);
    return L7_SUCCESS;
  }
...
static L7_RC_t ptin_igmp_group_client_find(ptin_client_id_t *client_ref, ptinIgmpGroupClientInfoData_t **client_info)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments client_ref:[%p] client_info[%p]", client_ref, client_info);
    return L7_FAILURE;
  }
  ...
  *noOfClients = 0;
  ...
  for ( client_idx = 0; client_idx < PTIN_IGMP_CLIENTIDX_MAX; client_idx++)
  {
    ...
    if ( (client_device=igmp_clientDevice_next(clientGroup, client_device)) != L7_NULLPTR && client_device->client != L7_NULLPTR)
    {
      ...
      if (ptin_debug_igmp_snooping)
      {
        PT_LOG_TRACE(LOG_CTX_IGMP,"Client Found [extendedEvcId:%u ptin_port:%u clientId:%u]",extendedEvcId, ptin_port, client_device->client->deviceClientId);
      }
    }
  }
  ...
  if (ptin_debug_igmp_snooping)
  {
    PT_LOG_TRACE(LOG_CTX_IGMP,"Number of Clients found [extendedEvcId:%u ptin_port:%u noOfClients:%u]",extendedEvcId, ptin_port, *noOfClients);
  }
...
RC_t ptin_igmp_multicast_package_channels_add(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupNetAddr, L7_uint32 groupMask, L7_inet_addr_t *sourceNetAddr, L7_uint32 sourceMask)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u serviceId:%u groupNetAddr:%p sourceNetAddr:%p]",packageId, serviceId, groupNetAddr, sourceNetAddr);    
    return L7_FAILURE;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Family not supported groupNetAddr->family:%u sourceNetAddr->family:%u", groupNetAddr->family, sourceNetAddr->family);
  ...
  {
    if (ptin_debug_igmp_snooping)  
      PT_LOG_NOTICE(LOG_CTX_IGMP, "Multicast Package Created with Empty Channels [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u",
               packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_SUCCESS;
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Failed to obtain groupAddr:%s from groupMask:%u rc:%u!", inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, rc);
    return L7_FAILURE;    
  }
  /* Validate output ip address */
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP,"Going to add groupAddr:%s (%u addresses)", inetAddrPrint(&groupAddr, groupAddrStr), noOfGroups);
  ...
  {   
    ...
    for (sourceIterator = 0; sourceIterator<noOfSources; sourceIterator++, sourceAddrAux.addr.ipv4.s_addr++)
    {
      ...
      {
        PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Search Channel Entry [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%p channelEntry:%p]", packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), inetAddrPrint(&sourceAddrAux, sourceAddrStr), channelAvlTreeEntry);    
        return rc;
      }

      if ( L7_SUCCESS != (rc = ptin_igmp_package_channel_conflict_validation(packageId, channelAvlTreeEntry) ) )
      {
        PT_LOG_ERR(LOG_CTX_IGMP, "Channel Conflict Found [packageId:%u serviceId:%u groupAddr:%s sourceAddr:%s]",packageId, serviceId, inetAddrPrint(&groupAddrAux, groupAddrStr), inetAddrPrint(&sourceAddrAux, sourceAddrStr));    
        return rc;
      }      
    }
  }

  PT_LOG_TRACE(LOG_CTX_IGMP,"Going to add groupAddr:%s (%u addresses)", inetAddrPrint(&sourceAddr, sourceAddrStr), noOfSources);
...
RC_t ptin_igmp_multicast_package_channels_remove(L7_uint32 packageId, L7_uint32 serviceId, L7_inet_addr_t *groupNetAddr, L7_uint32 groupMask, L7_inet_addr_t *sourceNetAddr, L7_uint32 sourceMask)
{
  ...
  {
    if (ptin_debug_igmp_snooping)  
      PT_LOG_NOTICE(LOG_CTX_IGMP, "No Channels to Remove [packageId:%u serviceId:%u groupAddr:%s groupMask:%u sourceAddr:%s sourceMask:%u",
               packageId, serviceId, inetAddrPrint(groupNetAddr, groupAddrStr), groupMask, inetAddrPrint(sourceNetAddr, sourceAddrStr), sourceMask);
    return L7_SUCCESS;
  }
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP,"Going to remove groupAddr:%s (%u addresses)", inetAddrPrint(&groupAddr, groupAddrStr), noOfGroups);
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP,"Going to remove groupAddr:%s (%u addresses)", inetAddrPrint(&sourceAddr, sourceAddrStr), noOfSources);
...
RC_t ptin_igmp_multicast_channel_service_get(L7_uint32 ptinPort, L7_uint32 deviceClientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint32 *serviceId)
{
  ...
  if (useDefaultMCService == L7_TRUE)
  {
    *serviceId = channelDB.default_evc_mc;
    ...
    /*Exit Here Multicast Service Found*/
    if (ptin_debug_igmp_snooping)
      PT_LOG_DEBUG(LOG_CTX_IGMP, "Entry Does Exist Going to Use Default MC Service [ptinPort:%u deviceClientId:%u groupClientId:%u onuId:%u groupAddr:%s sourceAddr:%s serviceId:%u]",
