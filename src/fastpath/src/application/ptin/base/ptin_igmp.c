...
L7_RC_t ptin_igmp_proxy_init(void)
{
  ...
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInstances)             = %zu", sizeof(igmpInstances));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientGroups)          = %zu", sizeof(igmpGroupClients));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientGroups)*         = %zu",
  ...
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpSnapshotClientGroups)  = %zu", sizeof(igmpSnapshotClientGroups));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpSnapshotClientGroups)* = %zu", 
  ...
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClients_unified)       = %zu", sizeof(igmpDeviceClients));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClients_unified)*      = %zu",
...
#ifdef IGMPASSOC_MULTI_MC_SUPPORTED
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPairDB)                = %zu", sizeof(channelDB));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPairDB)*               = %zu",
           sizeof(avlTree_t) + sizeof(avlTreeTables_t)*PTIN_IGMP_CHANNELS_MAX + sizeof(ptinIgmpChannelInfoData_t)*PTIN_IGMP_CHANNELS_MAX);
#endif
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(global_stats_intf)         = %zu", sizeof(global_stats_intf));

  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpProxyCfg   )           = %zu", sizeof(igmpProxyCfg));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpClientsIntf)           = %zu", sizeof(igmpClientsIntf));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpRoutersIntf)           = %zu", sizeof(igmpRoutersIntf));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInst_fromRouterVlan)   = %zu", sizeof(igmpInst_fromRouterVlan));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpInst_fromUCVlan)       = %zu", sizeof(igmpInst_fromUCVlan));

#if PTIN_SYSTEM_IGMP_ADMISSION_CONTROL_SUPPORT
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(ptinIgmpChannelBandwidthCache) = %zu", sizeof(ptinIgmpChannelBandwidthCache));
#if 0
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(clientGroupLookUpTable)        = %zu", sizeof(clientGroupLookUpTable));
#endif
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpPortAdmissionControl)      = %zu", sizeof(igmpPortAdmissionControl));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(igmpMulticastAdmissionControl) = %zu", sizeof(igmpMulticastAdmissionControl));
#endif

  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(pool_device_client_id)         = %zu", sizeof(pool_device_client_id));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(queue_free_device_clientIdx)   = %zu", sizeof(queue_free_device_client_id));

  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(pool_group_client_id)          = %zu", sizeof(pool_group_client_id));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(queue_free_group_clientIdx)    = %zu", sizeof(queue_free_group_client_id));

#ifdef IGMPASSOC_MULTI_MC_SUPPORTED
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(channelIdPoolEntry) = %zu", sizeof(channelIdPoolEntry));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(channelPoolEntry)   = %zu", sizeof(channelPoolEntry));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(packageIdPoolEntry) = %zu", sizeof(packageIdPoolEntry));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(packagePoolEntry)   = %zu", sizeof(packagePoolEntry));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(multicastServices)  = %zu", sizeof(multicastServices));
  PT_LOG_INFO(LOG_CTX_IGMP,"sizeof(multicastServiceId) = %zu", sizeof(multicastServiceId));
#endif
...
L7_RC_t ptin_igmp_proxy_config_get(PTIN_MGMD_CTRL_MGMD_CONFIG_t *igmpProxy)
{
  ...
  if (sizeof(PTIN_MGMD_CTRL_MGMD_CONFIG_t) != ctrlResMsg.dataLength)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Unexpected size in the MGMD response [dataLength:%u/%zu]",
               ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_MGMD_CONFIG_t));
    return L7_FAILURE;
  }
  else
  {
...
L7_RC_t ptin_igmp_evc_add(L7_uint32 evc_idx, L7_uint16 nni_ovlan)
{
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP,"Going to enable trap rule for evc_id %u (igmp_idx=%u)",evc_idx, igmp_idx);
...
L7_RC_t ptin_igmp_evc_remove(L7_uint32 evc_idx)
{
  ...
  {
    PT_LOG_WARN(LOG_CTX_IGMP,"There is no instance for evcId %u",evc_idx);
    return L7_SUCCESS;
  }
    ...
    do
    {
      ...
      else
      {
        if (ctrlResMsg.res==NOT_EXIST)
          break;
        PT_LOG_ERR(LOG_CTX_IGMP, "Invalid response size from MGMD [size:%u]. Expecting Multiple of :%zu",
                   ctrlResMsg.dataLength, sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
        return L7_FAILURE;
      }      
      entryId = mgmdGroupsRes.entryId;

      PT_LOG_TRACE(LOG_CTX_IGMP, "channelCopied=%u channelCopied=%u sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t)=%zu",
                   channelCopied, numberOfChannels, sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
             ...
             ) != L7_NULLPTR )
      {
            ...
            device_client->igmpClientDataKey.outerVlan == clientGroup->igmpClientDataKey.outerVlan )
        {
          ...
          do
          {
            ...
            PT_LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Msg Id  : %08X",     ctrlResMsg.msgId);
            PT_LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Res     : %u",       ctrlResMsg.res);
            PT_LOG_DEBUG(LOG_CTX_IGMP, "  CTRL Length  : %u (%zu)", ctrlResMsg.dataLength, (ctrlResMsg.dataLength)/sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));    
            ...
            PT_LOG_TRACE(LOG_CTX_IGMP, "channelCopied=%u numberOfChannels=%u sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t)=%zu",
                         channelCopied, numberOfChannels, sizeof(PTIN_MGMD_CTRL_ACTIVEGROUPS_RESPONSE_t));
                                 ...
                                 L7_uint16 *total_clients)
{
  ...
  if (0 == client_index)
  {
    ...
    do
    {
      ...
      PT_LOG_DEBUG(LOG_CTX_IGMP, "  Entry Length             : %zu", sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t));
      PT_LOG_DEBUG(LOG_CTX_IGMP, "  Number Of Entries Length : %zu", ctrlResMsg.dataLength/sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t));

      if (0 != ctrlResMsg.dataLength%sizeof(PTIN_MGMD_CTRL_GROUPCLIENTS_RESPONSE_t))
      {
        if (ctrlResMsg.res==NOT_EXIST)
          break;
        PT_LOG_ERR(LOG_CTX_IGMP,"Invalid dataLength:%u received. Expecting multiple of :%zu",
 ...
 #endif // ONE_MULTICAST_VLAN_RING_SUPPORT
#endif //MC_CLIENT_INTERF_SUPPORTED
 ...
 * @return L7_RC_t 
 */
L7_RC_t ptin_igmp_extVlans_get(L7_uint32 ptin_port, L7_uint16 intOVlan, L7_uint16 intIVlan,
                               L7_int client_idx, L7_uint16 *uni_ovid, L7_uint16 *uni_ivid)
{
  ...
  if (ptin_igmp_clientIntfVlan_validate(ptin_port, intOVlan) == L7_SUCCESS &&
  ...
  if ( ivid != 0 && ptin_evc_flags_get_fromIntVlan(intOVlan, &flags, L7_NULLPTR) == L7_SUCCESS)
  {
    if ( ( (flags & PTIN_EVC_MASK_QUATTRO) && !(flags & PTIN_EVC_MASK_STACKED) &&
           (ptin_igmp_rootIntfVlan_validate(ptin_port, intOVlan)==L7_SUCCESS) ) ||
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_client_type(L7_uint32 ptin_port,
                              L7_uint client_idx,
                              L7_BOOL *isDynamic)
{
  ptinIgmpClientInfoData_t *clientInfo;
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_device_client_timer_start(L7_uint32 ptin_port, L7_uint32 client_idx)
{
#ifdef CLIENT_TIMERS_SUPPORTED
  L7_RC_t   rc;
                                     ...
                                     ptin_client_id_t *client_ret)
{
  ...
  {
    PT_LOG_TRACE(LOG_CTX_IGMP,"New client created: ptin_port %u, intVlan %u, innerVlan %u, client_idx_ret:%u",
  ...
  if (rc!=L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error starting client timer: ptin_port %u, intVlan %u, innerVlan %u client_idx_ret:%u",
               ptin_port, intVlan, innerVlan, *client_idx_ret);
    return rc;
  }
  else
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_dynamic_client_remove(L7_uint32 ptin_port, L7_uint client_idx)
{
  /* Validate arguments */
  if (client_idx >= PTIN_IGMP_CLIENTIDX_MAX)
  {
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_clientData_get(L7_uint32 ptin_port,
                                 L7_uint client_idx,
                                 ptin_client_id_t *client)
{
 ...
 */
L7_RC_t ptin_igmp_intfVlan_validate(L7_uint32 ptin_port, L7_uint16 intVlan)
{
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting ptin_intf from ptin_port %u", ptin_port);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting intf configuration for ptin_intf=%u/%u (ptin_port=%u), mcEvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,ptin_port, evc_mc, intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (ptin_port=%u) not in use for mcEvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,ptin_port, evc_mc, intVlan);
    return L7_FAILURE;
  }
 ...
 * interface. 
 */
L7_RC_t ptin_igmp_rootIntfVlan_validate(L7_uint32 ptin_port, L7_uint16 intVlan)
{
  //st_IgmpInstCfg_t *igmpInst;
  L7_uint32   evc_id;
  ptin_intf_t ptin_intf;
  ...
  if (ptin_evc_intfCfg_get(evc_id, &ptin_intf,&mc_intf_cfg)!=L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting intf configuration for ptin_intf=%u/%u (ptin_port=%u), mcEvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,ptin_port, evc_id, intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_DEBUG(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (ptin_port=%u) is not root for mcEvcId=%u (intVlan=%u)",
                ptin_intf.intf_type,ptin_intf.intf_id,ptin_port, evc_id, intVlan);
    return L7_FAILURE;
  }
 ...
 * interface. 
 */
L7_RC_t ptin_igmp_clientIntfVlan_validate(L7_uint32 ptin_port, L7_uint16 intVlan)
{
  //st_IgmpInstCfg_t *igmpInst;
  ptin_intf_t ptin_intf;
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting intf configuration for ptin_intf=%u/%u (ptin_port=%u), EvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,ptin_port,evc_id,intVlan);
    return L7_FAILURE;
  }
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Interface ptin_intf=%u/%u (ptin_port=%u) not in use for EvcId=%u (intVlan=%u)",
              ptin_intf.intf_type,ptin_intf.intf_id,ptin_port,evc_id,intVlan);
    return L7_FAILURE;
  }
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_mcast_evc_id_get(L7_uint16 intVlan, L7_uint32 ptin_port, L7_BOOL isLeafPort, L7_uint32 clientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint32 *mcastEvcId)
{
  st_IgmpInstCfg_t *igmpInst;
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [isLeafPort:%u intVlan:%u ptin_port:%u client_idx:%u groupAddr:%p sourceAddr:%p mcastEvcId:%p]",
               isLeafPort, intVlan, ptin_port, clientId, groupAddr, sourceAddr, mcastEvcId);    
    return L7_FAILURE;
  }
  ...
  {
    if ( ptin_igmp_multicast_channel_service_get(ptin_port, clientId, groupAddr, sourceAddr, mcastEvcId) != L7_SUCCESS )
    {
      if (ptin_debug_igmp_snooping)
        PT_LOG_ERR(LOG_CTX_IGMP,"Failed to Get Multicast Service [ptin_port:%u client_idx:%u]", ptin_port, clientId);
      return L7_FAILURE;
    }
  }
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_McastRootVlan_get(L7_uint16 intVlan, L7_uint32 ptin_port, L7_BOOL isLeafPort, L7_uint32 clientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint16 *mcastRootVlan)
{
  L7_uint32         mcastEvcId;
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [isLeafPort:%u intVlan:%u ptin_port:%u client_idx:%u groupAddr:%p sourceAddr:%p McastRootVlan:%p]",
               isLeafPort, intVlan, ptin_port, clientId, groupAddr, sourceAddr, mcastRootVlan);
    return L7_FAILURE;
  }
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_rootptinPort_getList(L7_uint16 intVlan, ptin_port_bmp_t *ptinPortList, L7_uint32 *noOfInterfaces)
{
  st_IgmpInstCfg_t *igmpInst;
  ptin_HwEthMef10Evc_t evcCfg;
  L7_uint ptinPort_idx;
  L7_uint32 ptin_port;
  ...
  memset(ptinPortList,0x00,sizeof(ptin_port_bmp_t));

  *noOfInterfaces = 0;

  /* Run all interfaces */
  for (ptinPort_idx=0; ptinPort_idx<evcCfg.n_intf; ptinPort_idx++)
  {
...
/**
 * Get the list of client (leaf) ptin_ports associated to an 
 * internal vlan 
 * 
 * @param intVlan      : Internal vlan
 * @param ptinPortList : List of ptin_ports
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_clientPtinport_getList(L7_uint16 intVlan, ptin_port_bmp_t *ptinPortList, L7_uint32 *noOfInterfaces)
{
  ptin_HwEthMef10Evc_t evcCfg;
  L7_uint     intf_idx;
  L7_uint32   intIfNum;
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_extMcastVlan_get(L7_uint32 ptin_port, L7_uint16 intOVlan, L7_uint16 intIVlan, L7_uint16 *extMcastVlan, L7_uint16 *extIVlan)
{
  st_IgmpInstCfg_t *igmpInst;
  ...
  /* Get external vlans */
  if (ptin_evc_extVlans_get(ptin_port,igmpInst->McastEvcId,(L7_uint32)-1,intIVlan,extMcastVlan,extIVlan)!=L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting external vlans to ptin_port=%u, intOVlan=%u, intIVlan=%u (MCEvcId=%u)",
                 ptin_port,intOVlan,intIVlan,igmpInst->McastEvcId);
    return L7_FAILURE;
  }
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_extUcastVlan_get(L7_uint32 ptin_port, L7_uint16 intOVlan, L7_uint16 intIVlan, L7_uint16 *extUcastVlan, L7_uint16 *extIVlan)
{
  st_IgmpInstCfg_t *igmpInst;
  ...
  /* Get external vlans */
  if (ptin_evc_extVlans_get(ptin_port,igmpInst->UcastEvcId,(L7_uint32)-1,intIVlan,extUcastVlan,extIVlan)!=L7_SUCCESS)
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Error getting external vlans to ptin_port=%u, intOVlan=%u, intIVlan=%u (UCEvcId=%u)",ptin_port,intOVlan,intIVlan,igmpInst->UcastEvcId);
    return L7_FAILURE;
  }
...
//    PT_LOG_ERR(LOG_CTX_IGMP,"Could not add new timer data node");
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Timer Started: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx timeout=%u (s)",
...
L7_RC_t ptin_igmp_timer_update(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (pTimerData->timer == L7_NULL)
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Client timer not running: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle:0x%llx",
  ...
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    PT_LOG_ERR(LOG_CTX_IGMP,"Failed to update client timer: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx timeout=%u (s)",
               ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, timeout);
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Timer Updated: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx timeout=%u (s)",
                 ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, timeout);
...
L7_uint32 ptin_igmp_timer_timeout_get(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Client Timer TimeOut: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx time_left=%u (s))",
                 ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle, time_left);
...
L7_RC_t ptin_igmp_timer_stop(L7_uint32 ptin_port, L7_uint32 client_idx)
{
  ...
  if (SLLDelete(&igmpDeviceClients.ll_timerList, (L7_sll_member_t *)&timerData) != L7_SUCCESS)
  {
    osapiSemaGive(ptin_igmp_timers_sem);
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP,"Failed to delete timer node: ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx",
                 ptin_port, client_idx, igmpDeviceClients.timerCB, timerData.timer, timerData.timerHandle);
    return L7_FAILURE;
  }
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_DEBUG(LOG_CTX_IGMP,"Timer stopped successfully for ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx",
                 ptin_port, client_idx, igmpDeviceClients.timerCB, timerData.timer, timerData.timerHandle);
...
void igmp_timer_expiry(void *param)
{
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP,"Expiration event ocurred for timerHandle 0x%llx!", timerHandle);
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP,"Expiration event ocurred for ptin_port=%u client_idx=%u timerCB=%p timer=%p timerHandle=0x%llx",
                 ptin_port, client_idx, igmpDeviceClients.timerCB, pTimerData->timer, pTimerData->timerHandle);
                                             ...
                                             L7_inet_addr_t *channel_out, L7_uint32 *number_of_channels)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Invalid Input Arguments");
    return L7_FAILURE;
  }
...
static L7_BOOL ptin_igmp_instance_conflictFree(L7_uint32 McastEvcId, L7_uint32 UcastEvcId)
{
  ...
  for (idx=0; idx<PTIN_SYSTEM_N_IGMP_INSTANCES; idx++)
  {
    ...
    if (igmpInstances[idx].McastEvcId==McastEvcId &&
        igmpInstances[idx].UcastEvcId==UcastEvcId)
    {
      PT_LOG_ERR(LOG_CTX_IGMP,"Instance already exists (McastEvcId=%u, UcastEvcId=%u)",
                 McastEvcId,UcastEvcId);
      return L7_FALSE;
    }
#else
    if (igmpInstances[idx].McastEvcId==McastEvcId
...
static L7_RC_t ptin_igmp_clientId_restore(ptin_client_id_t *client)
{
  ...
  PT_LOG_TRACE(LOG_CTX_IGMP, "client->mask=%d, evc_type=%d ", client->mask, evc_type);
...
#if PTIN_IGMP_STATS_IN_EVCS
  (void) ptin_evc_igmp_stats_get_fromIntVlan(vlan, ptin_port, &stat_port);
#endif
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
L7_RC_t ptin_igmp_stat_increment_field(L7_uint32 ptin_port, L7_uint16 vlan, L7_uint32 client_idx, ptin_snoop_stat_enum_t field)
{
  ptinIgmpClientInfoData_t *client;
#if (!PTIN_IGMP_STATS_IN_EVCS)
  st_IgmpInstCfg_t *igmpInst;
...
static void ptin_igmp_group_client_identifier_push(L7_uint ptin_port, L7_uint16 client_idx)
{
  ...
  if (pClientIdx->clientId != client_idx)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Group client queue of port %d: client_idx %d pClientIdx->clientId %d",
               ptin_port, client_idx, pClientIdx->clientId);
  }
  ...
  if ( rc != NOERR)
  {
    PT_LOG_ERR(LOG_CTX_IGMP,"Error return indl_queue_add_head ptin port %d client_idx %d ", ptin_port, client_idx);
    return;
  }
...
RC_t ptin_igmp_admission_control_port_set(L7_uint32 ptin_port, L7_uint8 mask, L7_uint16 maxAllowedChannels, L7_uint64 maxAllowedBandwidth)  
{
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP, "Output arguments [ptin_port:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%u]",
                 ptin_port, igmpPortAdmissionControl[ptin_port].admissionControl.mask, igmpPortAdmissionControl[ptin_port].admissionControl.maxAllowedChannels, igmpPortAdmissionControl[ptin_port].admissionControl.maxAllowedBandwidth);
...
RC_t ptin_igmp_admission_control_multicast_service_set(ptin_igmp_admission_control_t *igmpAdmissionControl)  
{
  ...
  if (ptin_debug_igmp_snooping)
    PT_LOG_TRACE(LOG_CTX_IGMP, "Input Arguments [ptin_port:%u onuId:%u serviceId:%u internalServiceId:%u mask:0x%x maxAllowedChannels:%u maxAllowedBandwidth:%llu]",
                 igmpAdmissionControl->ptin_port, igmpAdmissionControl->onuId, igmpAdmissionControl->serviceId, internalServiceId, igmpAdmissionControl->mask, igmpAdmissionControl->maxAllowedChannels, igmpAdmissionControl->maxAllowedBandwidth);
 ...
 * @return  L7_RC_t           : L7_SUCCESS/L7_FAILURE 
 */
L7_RC_t ptin_igmp_clients_bmp_get(L7_uint32 extendedEvcId, L7_uint32 ptin_port, L7_uchar8 *clientBmpPtr, L7_uint32 *noOfClients)
{
  L7_uint32 i_client;
  L7_uint32 clientPort;
...
//      continue;
...
#endif
      {
        ...
        (*noOfClients)++;
        if (ptin_debug_igmp_snooping)
          PT_LOG_TRACE(LOG_CTX_IGMP, "Client Found [ServiceId:%u PortId:%u ClientId:%u]",clientExtendedEvcId,clientPort,avl_info->deviceClientId);
      }
    }
 ...
 */
L7_RC_t ptin_igmp_groupclients_bmp_get(L7_uint32 extendedEvcId, L7_uint32 ptin_port, L7_uchar8 *clientBmpPtr, L7_uint32 *noOfClients)
{
  L7_uint32                       client_idx;
  L7_uint32                       clientExtendedEvcId;
#if PTIN_BOARD_IS_LINECARD || PTIN_BOARD_IS_STANDALONE
  ...
  {
    if (ptin_debug_igmp_snooping)
      PT_LOG_ERR(LOG_CTX_IGMP, "extendedEvcId# %u is out of range [0..%lu]", extendedEvcId, PTIN_SYSTEM_N_EXTENDED_EVCS);
    return L7_FAILURE;
  }
...
RC_t ptin_igmp_multicast_channel_service_get(L7_uint32 ptinPort, L7_uint32 deviceClientId, L7_inet_addr_t *groupAddr, L7_inet_addr_t *sourceAddr, L7_uint32 *serviceId)
{
  ...
  {
#ifdef ONE_MULTICAST_VLAN_RING_SUPPORT
    PT_LOG_TRACE(LOG_CTX_IGMP, "Failed to Obtain groupClient [ptinPort:%u deviceClientId:%u]",
                 ptinPort, deviceClientId);
#else
    PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Obtain groupClient [ptinPort:%u deviceClientId:%u]",
               ptinPort, deviceClientId);
#endif
    return L7_FAILURE;
  }
...
//  else /*inUse= L7_TRUE*/
    ...
    {
      PT_LOG_ERR(LOG_CTX_IGMP, "Failed to Search Channel Entry [ptinPort:%u deviceClientId:%u onuId:%u groupAddr:%s sourceAddr:%s serviceId:%u internalServiceId:%u noOfMulticastServices:%u]", 
              ptinPort, deviceClientId, onuId, inetAddrPrint(groupAddr, groupAddrStr), inetAddrPrint(sourceAddr, sourceAddrStr), serviceIdAux, internalServiceId, multicastServices[ptinPort][onuId].noOfMulticastServices);    
      return L7_FAILURE;
    }
...
RC_t ptin_igmp_multicast_client_package_get(L7_uint32 packageId, ptinIgmpGroupClientInfoData_t *groupClient, L7_BOOL *isBitSet)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_IGMP, "Invalid arguments [packageId:%u groupClient:%p isBitSet:%p]",packageId, groupClient, isBitSet);    
    return L7_FAILURE;
  }
