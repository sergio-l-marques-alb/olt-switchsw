...
L7_RC_t ptin_igmp_proxy_init(void)
{
  ...
  for (i=0; i<sizeof(clientIdx_pool)/sizeof(clientIdx_pool[0]); i++)
  {
    ...
    dl_queue_add_head(&queue_free_clientIdx, (dl_queue_elem_t *) &clientIdx_pool[i]);
  }
  ...
  for (i=0; i<sizeof(igmpClients_unified.client_devices)/sizeof(igmpClients_unified.client_devices[0]); i++)
  {
    ...
    dl_queue_add_head(&igmpClients_unified.queue_free_clientDevices, (dl_queue_elem_t *) &igmpClients_unified.client_devices[i]);
  }
  ...
  if (channel_index==0)
  {
    ...
    if (clientGroup != L7_NULLPTR)
    {
      ...
      while ((client_device=igmp_clientDevice_next(clientGroup, client_device)) != L7_NULLPTR)
      {
        ...
        if (ptin_snoop_activeChannels_get(McastRootVlan, client_device->client->client_index, channelList_tmp, &n_channels) != L7_SUCCESS)
        {
    ...
    else
    {
      ...
      if (ptin_snoop_activeChannels_get(McastRootVlan, (L7_uint16)-1, channelList, &n_channels) != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_IGMP,"Error getting channels list");
        return L7_FAILURE;
      }
...
static L7_RC_t ptin_igmp_clean_childClients(ptinIgmpClientGroupInfoData_t *avl_infoData_clientGroup);
...
static L7_RC_t ptin_igmp_rm_clientGroup(L7_uint igmp_idx, ptin_client_id_t *client)
{
  ...
  if (ptin_igmp_clean_childClients(avl_infoData) != L7_SUCCESS)
  {
    osapiSemaGive(ptin_igmp_clients_sem);
 ...
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE
 */
static L7_RC_t ptin_igmp_clean_childClients(ptinIgmpClientGroupInfoData_t *clientGroup)
{
  ptinIgmpClientDevice_t *client_device;
 ...
 * @param evc_idx 
 */
void ptin_igmp_clientGroups_dump(void)
{
