...
//OWN ROUTINES**************************************************************************
static int send_ccm(L7_uint16 oam_prt, T_MEP_HDR *p_mep, L7_uint8 RDI, L7_uint8 use_mcast_DMAC);
static int send_csf(L7_uint16 oam_prt, T_MEP_HDR *p_mep, L7_uint8 CSF_period, L7_uint8 CSF_flags);
static int rx_ccm(L7_uint16 oam_prt, L7_uint8 *pkt_ethtype, L7_uint32 pkt_len, L7_uint64 vid, L7_uint8 *pSMAC,
                  T_MEP *p_mep_db, T_LOOKUP_MEP *p_mep_lut);
static int rx_csf(L7_uint16 oam_prt, L7_uint8 *pkt_ethtype, L7_uint32 pkt_len, L7_uint64 vid, L7_uint8 *pSMAC,
...
static L7_uint64 hash(L7_uint64 in, short ibits, short obits) {
  L7_uint64 out;
...
/*L7_uint32 hash_vidmac_to_mepid(L7_uint64 vid, T_ETH_OAM_MAC *p) {
L7_uint64 in;
 in=    ((L7_uint64)vid)<<48 | *((L7_uint32 *)&p->byte[0])<<16 | *((L7_uint16 *)&p->byte[4]);
...
static L7_uint32 hash_vidmepid_to_lookup_index(L7_uint64 vid, L7_uint32 mep_id) {
L7_uint32 in;
 in=    mep_id<<12 | vid;
...
L7_uint32 finger_lut_index(L7_uint8 find0_add1_del2,                                                                                    //operation
  ...
  L7_uint32 h, i, j, _1st_unoccupied=N_MAX_LOOKUP_MEPs;
  L7_uint32 unx_lvl=0UL-1, msmrg=0UL-1, unx_mep=0UL-1, unx_mep_potent_loop=0UL-1, unx_period=0UL-1;
        ...
        if (p_mep_lut[j].level==level) {
          ...
          if (p_mep_lut[j].mep_id==mep_id) {
            if (p_mep_lut[j].mep0_rmep1==mep0_rmep1) {
              ...
              unx_mep=j;   unx_mep_potent_loop=j;
            }
          } else unx_mep=j;
          //}
          //else msmrg=j;
...
int wr_mep(L7_uint32 i_mep, T_MEP_HDR *p_mep, T_ETH_SRV_OAM *p_oam) {
  L7_uint32 i_look;
  T_MEP   *p_mep_db;
...
//0 - OK
//1 - Invalid index
int del_mep(L7_uint32 i_mep, T_ETH_SRV_OAM *p_oam) {
  L7_uint32 i;
  T_MEP       *_p_mep;
  T_MEP_CSF   *_p_mep_csf;
  T_ME        *_p_me;
...
int wr_rmep(L7_uint32 i_mep, L7_uint32 i_rmep, T_RMEP *p_rmep, const T_MEP_HDR *p_mep, T_ETH_SRV_OAM *p_oam) {
  L7_uint32 i_look_r;
  T_MEP   *_p_mep;
  T_ME    *_p_me;
  T_MEP   *p_mep_db;
...
//0 - OK
//1 - Invalid index
int del_rmep(L7_uint32 i_mep, L7_uint32 i_rmep, T_ETH_SRV_OAM *p_oam) {
  L7_uint32 i_look_r;
  T_MEP   *_p_mep;
  T_ME    *_p_me;
  T_MEP   *p_mep_db;
...
void proc_ethsrv_oam(T_ETH_SRV_OAM *p_oam, L7_uint32 T_ms) {
  L7_uint8 timeout, time_2_send_ccms, time_2_send_csf;
  L7_uint32 i, tmout, n_rmeps;
  T_MEP       *_p_mep;
  T_MEP       *p_mep_db;
  T_MEP_CSF   *_p_mep_csf;
  T_MEP_CSF   *p_mep_csf_db;
  L7_uint16 *proc_i_mep;
  L7_uint32 j, n, meps_procssd_per_function_call;
  ...
  for (j=meps_procssd_per_function_call; j; j--) {
    ...
    if (_p_mep->CCM_timer+T_ms/2 > tmout) {
      time_2_send_ccms=1;   _p_mep->CCM_timer=0;
    } //if (_p_mep->CCM_timer >= tmout)
    ...
    for (i=0, n_rmeps=0, timeout=0; i<N_MAX_MEs_PER_MEP; i++) {
      ...
      if (_p_mep->ME[i].LOC_timer*2 < tmout*7)   _p_mep->ME[i].LOC_timer += T_ms; //3.5*tmout
        ...
        if (_p_mep->ME[i].LOC_timer!=0L-1) {
          ethsrv_oam_register_connection_loss((L7_uint8*)&_p_mep->meg_id, _p_mep->mep_id, _p_mep->ME[i].mep_id, _p_mep->prt, _p_mep->vid);
    ...
    if (_p_mep_csf->en1dis0) {
      ...
      if (_p_mep_csf->CSF_timer+T_ms/2 > tmout) {
        time_2_send_csf=1;   _p_mep_csf->CSF_timer=0;
...
int MEP_csf_admin(L7_uint16 mep_idx, L7_uint8 en1dis0, L7_uint8 period, T_ETH_SRV_OAM *p_oam) {
  T_MEP_CSF   *_p_mep_csf;
  T_MEP_CSF   *p_mep_csf_db;
...
int MEP_enable_Tx_CSF(L7_uint16 mep_idx, L7_uint8 CSF_tx_flags, T_ETH_SRV_OAM *p_oam) {
  T_MEP_CSF   *_p_mep_csf;
  T_MEP_CSF   *p_mep_csf_db;
...
int MEP_is_CC_LOC_or_RDI(L7_uint16 mep_idx, T_ETH_SRV_OAM *p_oam) {
  T_MEP   *_p_mep;
  T_MEP   *p_mep_db;
  L7_uint32     i;
...
int MEP_is_CSF_LOS(L7_uint16 mep_idx, T_ETH_SRV_OAM *p_oam) {
  T_MEP_CSF   *_p_mep_csf;
  T_MEP_CSF   *p_mep_csf_db;
...
static int send_ccm(L7_uint16 oam_prt, T_MEP_HDR *p_mep, L7_uint8 RDI, L7_uint8 use_mcast_DMAC) {
  ...
  return send_eth_pckt(oam_prt, p_mep->up1_down0, (L7_uint8*)p_ccm, sizeof(ETH_SRV_OAM_CCM_DATAGRM), p_mep->vid, p_mep->prior, p_mep->CoS, 0, OAM_ETH_TYPE, DMAC.byte);
}//send_ccm
...
static int send_csf(L7_uint16 oam_prt, T_MEP_HDR *p_mep, L7_uint8 CSF_period, L7_uint8 CSF_flags) {
  ...
  return send_eth_pckt(oam_prt, p_mep->up1_down0, (L7_uint8*)p_csf, sizeof(ETH_SRV_OAM_CSF_DATAGRM), p_mep->vid, p_mep->prior, p_mep->CoS, 0, OAM_ETH_TYPE, DMAC.byte);
}//send_csf
...
int rx_oam_pckt(L7_uint16 oam_prt, L7_uint8 *pkt_ethtype, L7_uint32 pkt_len, L7_uint64 vid, L7_uint8 *pSMAC, T_ETH_SRV_OAM *p_oam) {
  ETH_SRV_OAM_DATAGRM *p_oam_d;
  T_MEP           *p_mep_db;
  T_MEP_CSF       *p_mep_csf_db;
  T_LOOKUP_MEP    *p_mep_lut;

  if (*((L7_uint16 *) pkt_ethtype)    !=  OAM_ETH_TYPE)   return 1;
...
static int rx_ccm(L7_uint16 oam_prt, L7_uint8 *pkt_ethtype, L7_uint32 pkt_len, L7_uint64 vid, L7_uint8 *pSMAC,
  ...
  T_MEP   *_p_mep;
  L7_uint32 i_look_r, i_mep, i_rmep;
  int i;
  L7_uint8 unxlvl0_msmrg1_unxmep2_unxmeppotentloop3_unxperiod4; L7_uint32 alrm_index;
  ...
  if (!valid_mep_index(i_mep) || !valid_rmep_index(i_rmep)) {
    ...
    if (!valid_mep_index(i_mep)) {
      ethsrv_oam_register_mismerge(&p_ccm->meg_id, p_ccm->mep_id, 0xffff, oam_prt, vid);   return 1;
    }
  }
  ...
  if (_p_mep->ME[i_rmep].LOC_timer==0L-1) {
    if (flags_TO_RDI(p_ccm->flags)) {
      ethsrv_oam_register_receiving_RDI((L7_uint8*)&_p_mep->meg_id, _p_mep->mep_id, p_ccm->mep_id, oam_prt, _p_mep->vid);
      printf("MEP %u receiving RDI from RMEP %u"NLS, _p_mep->mep_id, p_ccm->mep_id);
    } else {
      ethsrv_oam_register_connection_restored((L7_uint8*)&_p_mep->meg_id, _p_mep->mep_id, p_ccm->mep_id, oam_prt, _p_mep->vid);
