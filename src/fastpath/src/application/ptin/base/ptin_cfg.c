...
L7_RC_t ptin_cfg_ntw_connectivity_get(ptin_NtwConnectivity_t *ntwConn)
{
  ...
  if (mask & PTIN_NTWCONN_MASK_IPADDR)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error getting inBand management IPv4 Address");
      return L7_FAILURE;
    }
  ...
  if (mask & PTIN_NTWCONN_MASK_NETMASK)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error getting inBand management NetMask");
      return L7_FAILURE;
    }
  ...
  if (mask & PTIN_NTWCONN_MASK_GATEWAY)
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error getting inBand management Gateway Addr");
      return L7_FAILURE;
    }
      ...
      mask & PTIN_NTWCONN_MASK_VLAN)
  {
    ...
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error getting inBand interface and VLAN info (EVC read error)");
    }
    else
    {
      /* Interface */
...
L7_RC_t ptin_cfg_ntw_connectivity_set(ptin_NtwConnectivity_t *ntwConn)
{
  ...
  if (rc != L7_SUCCESS)
  {
    LOG_ERR(LOG_CTX_PTIN_API, "Error reading current inBand management config");
    return L7_FAILURE;
  }
...
/* Only allow inBand IP, netmask and gateway configurations on OLT7-8CH board */
    ...
    if (ntwConn->mask & PTIN_NTWCONN_MASK_IPADDR)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error setting inBand management IPv4 Address");
        break;
      }
    ...
    if (ntwConn->mask & PTIN_NTWCONN_MASK_NETMASK)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error setting inBand management NetMask");
        break;
      }
    ...
    if (ntwConn->mask & PTIN_NTWCONN_MASK_GATEWAY)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error setting inBand management Gateway Addr");
        break;
      }
...
/* Only allow inBand IP, netmask and gateway configurations on OLT7-8CH board */
        ...
        ntwConn->mask & PTIN_NTWCONN_MASK_VLAN)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error creating EVC# %u for inBand management purposes", PTIN_EVC_INBAND);
        break;
      }
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error setting inBand management VLAN ID");
        break;
      }
    ...
    else
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_ERR(LOG_CTX_PTIN_API, "Error resetting inBand management VLAN ID to ZERO");
        break;
      }
      else
      {
        LOG_TRACE(LOG_CTX_PTIN_API, "inBand VLAN cleared");
      }
    }
  } while (0);
  ...
  LOG_TRACE(LOG_CTX_PTIN_API,"MacAddr=%02X:%02X:%02X:%02X:%02X:%02X",macAddr[0],macAddr[1],macAddr[2],macAddr[3],macAddr[4],macAddr[5]);
  if (usmDbAgentIpIfAddressGet(1,&ipAddr)!=L7_SUCCESS)
  {
    LOG_WARNING(LOG_CTX_PTIN_API, "Error acquiring dtl0 ipaddress");
    ipAddr = 0;
  }
  LOG_TRACE(LOG_CTX_PTIN_API,"IP_Addr=%u.%u.%u.%u",(ipAddr>>24) & 0xFF,(ipAddr>>16) & 0xFF,(ipAddr>>8) & 0xFF,ipAddr & 0xFF);
  ...
  {
    LOG_WARNING(LOG_CTX_PTIN_API, "Error calculating intIfNum");
  ...
  if (ipAddr!=0)
  {
    ...
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error activating rule to trap inband packets");
    }
    else
    {
      LOG_TRACE(LOG_CTX_PTIN_API, "Success activating rule to trap inband packets");
    }
  }
  /* Remove rule */
  else
  {
    ...
    {
      LOG_ERR(LOG_CTX_PTIN_API, "Error deactivating rule to trap inband packets");
    }
    else
    {
      LOG_TRACE(LOG_CTX_PTIN_API, "Success deactivating rule to trap inband packets");
    }
  }
#endif
...
L7_RC_t ptin_cfg_inband_bridge_set(void)
{
  ...
  if (rc != 0)
  {
    LOG_ERR(LOG_CTX_PTIN_API, "Error executing script " PTIN_INBAND_BRIDGE_SCRIPT " (rc=%d)", rc);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_PTIN_API, "InBand bridge script successfully executed");
