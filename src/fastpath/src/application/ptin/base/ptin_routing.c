...
#define PTIN_ROUTING_MAX_LOOPBACKID         L7_MAX_NUM_LOOPBACK_INTF - 1 /* This is a copy from clicommands_loopback.h, which I'm not able to include here */

#define PTIN_ROUTING_INTF_TYPE_UNKNOWN      0
#define PTIN_ROUTING_INTF_TYPE_PHYSICAL     1
#define PTIN_ROUTING_INTF_TYPE_VLAN         2

#define PTIN_ROUTING_LOOPBACK_ENABLED       0
#define PTIN_ROUTING_LOOPBACK_DISABLED      1
...
L7_uint32                         __routing_loopbacks_max;
ptin_routing_loopback_t*          __routing_loopbacks;
 ...
 * @note This creates a routing interface named dtl0.VLANID, where VLANID is the value passed through 'vlanId' 
 */
static L7_int __ioctl_loopback_create();

/**
 * Remove an existing loopback interface.
 *  
 * @param vlanId : Vlan ID of the routing interface
 * 
 * @return L7_RC_t : L7_SUCCESS/L7_FAILURE 
 ...
 * @note This removes the routing interface named dtl0.VLANID, where VLANID is the value passed through 'vlanId' 
 */
static L7_int __ioctl_loopback_remove();
#endif
...
L7_RC_t ptin_routing_init(void)
{
  ...
  LOG_PT_INFO(LOG_CTX_ROUTING, "Starting initialization");
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to initialize arp table snapshot");
    return L7_FAILURE;
  }
  if(__routetable_snapshot_init() != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to initialize route table snapshot");
    return L7_FAILURE;
  }
  if(__ping_sessions_init() != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to initialize ping session array");
    return L7_FAILURE;
  }
  if(__traceroute_sessions_init() != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to initialize traceroute session array");
    return L7_FAILURE;
  }

  /* Create a new socket for ioctl interaction */
  LOG_PT_INFO(LOG_CTX_ROUTING, "Creating new socket to interact with ioctl");
  __ioctl_socket_fd = socket(AF_INET, SOCK_DGRAM, 0);
  if (__ioctl_socket_fd < 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to create new socket to interact with ioctl");
    return L7_FAILURE;
  }

  /* Set dtl0 MTU */
  LOG_PT_INFO(LOG_CTX_ROUTING, "Setting %s mtu to %u", PTIN_ROUTING_DTL0_INTERFACE_NAME, PTIN_ROUTING_DTL0_MTU_DEFAULT);
  if(__ioctl_dtl0_mtu_set(PTIN_ROUTING_DTL0_MTU_DEFAULT) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to set %s mtu", errno, PTIN_ROUTING_DTL0_INTERFACE_NAME);
    return L7_FAILURE;
  }

  /* Enable routing on Fastpath */
  LOG_PT_INFO(LOG_CTX_ROUTING, "Setting OLTSWITCH's routing admin mode to L7_ENABLE");
  if(usmDbIpRtrAdminModeSet(PTIN_ROUTING_USMDB_UNITINDEX, L7_ENABLE) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to set OLTSWITCH's routing admin mode to L7_ENABLE");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_intf_create(ptin_intf_t* routingIntf, L7_uint16 internalVlanId)
{
  ...
  L7_uint16            routingVlanId;
  ptin_intf_t          physicalIntf; //Only used for physical routing interfaces 

  if( (routingIntf == L7_NULLPTR) )
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [routingIntf:%p]", routingIntf);
    return L7_ERROR;
  }

  if(routingIntf->intf_id >= __routing_interfaces_max)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested routing interface ID exceeds the allowed range [id:%u max:%u]", routingIntf->intf_id, __routing_interfaces_max);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_NOTICE(LOG_CTX_ROUTING, "Received request to create an interface that already exists. Ignored. [intf_id:%u]", routingIntf->intf_id);
    return L7_SUCCESS;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to to convert internalVlanId %u to an EVC", internalVlanId);
    return L7_FAILURE;
  }
  for(i=0; i<evc.n_intf; ++i)
  {
     /* If we find at least one ROOT interface, then this is an physical routing interface. Otherwise, it is considered a vlan routing interface */
  ...
  {
     intfType = PTIN_ROUTING_INTF_TYPE_VLAN;
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to to convert routingIntf %u/%u to intfNum", routingIntf->intf_type, routingIntf->intf_id);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_INFO(LOG_CTX_ROUTING, "Enabling %s interface", PTIN_ROUTING_DTL0_INTERFACE_NAME);
    if(__ioctl_dtl0_enable() != 0)
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to enable %s interface", errno, PTIN_ROUTING_DTL0_INTERFACE_NAME);
      return L7_FAILURE;
    }
  ...
  /* Create a new routing interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Adding vlan %u to interface %s", routingVlanId, PTIN_ROUTING_DTL0_INTERFACE_NAME);
  if(__ioctl_vlanintf_add(routingVlanId) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to add a new vlan %u to the %s interface", errno, routingVlanId, PTIN_ROUTING_DTL0_INTERFACE_NAME);
    return L7_FAILURE;
  }

  /* Rename the new routing interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Renaming the new routing interface from %s.%u to %s%u", PTIN_ROUTING_DTL0_INTERFACE_NAME, routingVlanId, PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
  if(__ioctl_intf_rename_dtl2rt(routingIntf->intf_id, routingVlanId) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to rename the new routing interface from %s.%u to %s%u", errno, PTIN_ROUTING_DTL0_INTERFACE_NAME, routingVlanId, PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
    return L7_FAILURE;
  }

  /* Enable the new interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Enabling %s%u interface", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
  if(__ioctl_vlanintf_enable(routingIntf->intf_id) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to enable %s%u interface", errno, PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
    return L7_FAILURE;
  }
  
  /* Associate the new interface with the given vlanId in Fastpath's routing tables */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Associating %s%u with vlan %u on OLTSWITCH's routing tables", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id, internalVlanId);
  if(usmDbIpVlanRoutingIntfCreate(PTIN_ROUTING_USMDB_UNITINDEX, internalVlanId, routingIntf->intf_id+1) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to associate %s%u with vlan %u on OLTSWITCH's routing tables", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id, internalVlanId);
    return L7_FAILURE;
  }
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Allowing IP/ARP packets through dtl0 for vlans %u/%u on intfIfNum %u", routingVlanId, internalVlanId, routingIntfNum);
  if(L7_SUCCESS != ptin_ipdtl0_control(routingVlanId, routingVlanId, internalVlanId, routingIntfNum, PTIN_IPDTL0_ETH_IPv4, L7_TRUE))
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to allow IP/ARP packets through dtl0 for this vlan");
    return L7_FAILURE;
  }
  ...
  */
  if(intfType == PTIN_ROUTING_INTF_TYPE_PHYSICAL)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to to convert physicalIntf %u/%u to intfNum", physicalIntf.intf_type, physicalIntf.intf_id);
      return L7_FAILURE;
    }

    if (nimGetIntfAddress(physicalIntfNum, L7_NULL, &macAddr.addr[0]) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to get physical interface MAC address [physicalIntf:%u/%u]", physicalIntf.intf_type, physicalIntf.intf_id);
      return L7_FAILURE;
    }

    LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting %s%u interface MAC address", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
    if(__intf_macaddress_set(routingIntf, &macAddr) != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to set %s%u interface MAC address", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
      return L7_FAILURE;
    }
...
L7_RC_t ptin_routing_loopback_create(ptin_intf_t* routingIntf)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested routing loopback ID exceeds the allowed range [id:%u max:%u]", routingIntf->intf_id, __routing_loopbacks_max);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_NOTICE(LOG_CTX_ROUTING, "Received request to create a loopback that is already enabled. Ignored. [intf_id:%u]", routingIntf->intf_id);
    return L7_SUCCESS;
  }
  ...
  /* 
     If this is not loopback#0, then create a new Linux loopback interface.
     Loopback#0 is ignored because it is permenantly associated with the EthG1 interface. Hence, only a Fastpath loopback interface is created for it.
  */
...
//   }
  ...
  /* Create a new loopback interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Creating new loopback interface %u", routingIntf->intf_id);
  if(usmDbRlimLoopbackCreate(routingIntf->intf_id) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to create loopback %u", routingIntf->intf_id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_intf_remove(ptin_intf_t* routingIntf)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Deleting routing interface associated with internal vlan %u", internalVlanId);
  if(usmDbIpVlanRoutingIntfDelete(PTIN_ROUTING_USMDB_UNITINDEX, internalVlanId) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to delete routing interface associated with internal vlan %u", internalVlanId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to allow IP/ARP packets through dtl0 for this vlan [routingVlanId:%u intfNum:%u]", routingVlanId, intfNum);
    return L7_FAILURE;
  }
#endif /* PTIN_BOARD_IS_MATRIX */

  /* Disable the interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Disabling %s%u interface", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id);
  if(__ioctl_vlanintf_disable(routingIntf->intf_id) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to disable %s.%u interface", errno, PTIN_ROUTING_DTL0_INTERFACE_NAME, routingVlanId);
    return L7_FAILURE;
  }

  /* Change the interface name to match the vconfig standards */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Renaming the routing interface from %s%u to %s.%u", PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id, PTIN_ROUTING_DTL0_INTERFACE_NAME, routingVlanId);
  if(__ioctl_intf_rename_rt2dtl(routingIntf->intf_id, routingVlanId) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to rename the routing interface from %s%u to %s.%u", errno, PTIN_ROUTING_INTERFACE_NAME_PREFIX, routingIntf->intf_id, PTIN_ROUTING_DTL0_INTERFACE_NAME, routingVlanId);
    return L7_FAILURE;
  }

  /* Delete the vlan interface */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Removing vlan %u from interface %s", routingVlanId, PTIN_ROUTING_DTL0_INTERFACE_NAME);
  if(__ioctl_vlanintf_remove(routingVlanId) != 0)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "ioctl error (errno:%d). Unable to remove a vlan %u from the %s interface", errno, routingVlanId, PTIN_ROUTING_DTL0_INTERFACE_NAME);
    return L7_FAILURE;
  }
...
void ptin_routing_intf_remove_all(void)
{
   ...
   LOG_PT_TRACE(LOG_CTX_ROUTING, "Removing all routing interfaces");
   for(i=0; i<__routing_interfaces_max; ++i)
   {
      if(__routing_interfaces[i].type != PTIN_ROUTING_INTF_TYPE_UNKNOWN)
      {
         ...
         {
            LOG_PT_WARN(LOG_CTX_ROUTING, "Unable to remove routing interface %u. Continuing", i);
         }
      }
   }
}
...
L7_RC_t ptin_routing_intf_ipaddress_set(ptin_intf_t* routingIntf, L7_uchar8 ipFamily, L7_uint32 ipAddr, L7_uint32 subnetMask)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting routing interface %u IP address to %s/%s", intfNum, inetAddrPrint(&inetIpAddr, ipAddrStr), inetAddrPrint(&inetIpSubnet, ipSubnetStr));
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to set routing interface %u IP address to %s/%s (rc = %u)", intfNum, inetAddrPrint(&inetIpAddr, ipAddrStr), inetAddrPrint(&inetIpSubnet, ipSubnetStr), rc);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_intf_mtu_set(ptin_intf_t* routingIntf, L7_uint32 mtu)
{
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting routing interface %u MTU to %d", intfNum, mtu);
  if((rc = usmDbIntfIpMtuSet(PTIN_ROUTING_USMDB_UNITINDEX, intfNum, mtu)) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to set routing interface %u MTU to %d (rc = %u)", intfNum, mtu, rc);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_intf_physicalport_get(L7_uint16 routingIntfNum, L7_uint16 *physicalIntfNum)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [physicalIntfNum:%p]", physicalIntfNum);
    return L7_ERROR;
  }

  if(L7_SUCCESS != ptin_intf_intIfNum2ptintf(routingIntfNum, &routingIntf))
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to convert requested intIfNum [routingIntfNum:%u]", routingIntfNum);
    return L7_FAILURE;
  }
  ...
  if(__routing_interfaces[routingIntf.intf_id].type == PTIN_ROUTING_INTF_TYPE_PHYSICAL)
  {
...
L7_RC_t ptin_routing_arptable_getnext(L7_uint32 intfNum, L7_uint32 firstIdx, L7_uint32 maxEntries, L7_uint32* readEntries, msg_RoutingArpTableResponse* buffer)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [readEntries:%p buffer:%p]", readEntries, buffer);
    return L7_ERROR;
  }
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_ROUTING, "ARP table snapshot is empty");
    *readEntries = 0;
    return L7_SUCCESS;
  }
  ...
  *readEntries = 0;
  while( (*readEntries < maxEntries) && (snapshotIterator != NULL) )
  {
    ...
    {
      LOG_PT_TRACE(LOG_CTX_ROUTING, "Copying local entry [idx:%u]"            , currentIndex);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  intfNum: %u"                           , snapshotIterator->intfNum);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  type:    %u"                           , snapshotIterator->type);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  age:     %u"                           , snapshotIterator->age);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  ipAddr:  %s"                           , inetAddrPrint(&snapshotIterator->ipAddr, ipAddrStr));
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  macAddr: %02X:%02X:%02X:%02X:%02X:%02X", 
      ...
      {
        LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to to convert intfNum %u to ptin_intf_t", snapshotIterator->intfNum);
        return L7_FAILURE;
      }
...
L7_RC_t ptin_routing_routetable_get(L7_uint32 intfNum, L7_uint32 firstIdx, L7_uint32 maxEntries, L7_uint32* readEntries, msg_RoutingRouteTableResponse* buffer)
{
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_ROUTING, "Route table snapshot is empty");
    *readEntries = 0;
    return L7_SUCCESS;
  }
  ...
  while( (currentIndex < maxEntries) && (snapshotIterator != NULL) )
  {
    if(currentIndex >= firstIdx)
    {
      LOG_PT_TRACE(LOG_CTX_ROUTING, "Copying local entry [idx:%u]" , currentIndex);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  intfNum:       %u"             , snapshotIterator->intfNum);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  protocol:      %u"             , snapshotIterator->protocol);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  updateTime:    %ud %uh %um %us", snapshotIterator->updateTime.days, snapshotIterator->updateTime.hours, snapshotIterator->updateTime.minutes, snapshotIterator->updateTime.seconds);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  networkIpAddr: %s"             , inetAddrPrint(&snapshotIterator->networkIpAddr, ipAddrStr));
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  subnetMask:    %u"             , snapshotIterator->subnetMask);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  gwIpAddr:      %s"             , inetAddrPrint(&snapshotIterator->gwIpAddr, ipAddrStr));
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  preference:    %u"             , snapshotIterator->preference);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  metric:        %u"             , snapshotIterator->metric);
...
L7_RC_t ptin_routing_pingsession_create(L7_uint8 sessionIdx, L7_uint32 ipAddr, L7_uint16 probeCount, L7_uint16 probeSize, L7_uint16 probeInterval)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index[%u] is higher than the maximum allowed number of ping sessions", sessionIdx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index is already being used in an active ping session [index:%u]", sessionIdx);
    return L7_FAILURE;
  }
                                  ...
                                  0, L7_NULLPTR, __ping_session_finish_callback, (void*)&__ping_sessions[sessionIdx].index, &__ping_sessions[sessionIdx].handle))
  {
    ...
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to start a new ping session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_pingsession_query(msg_RoutingPingSessionQuery* buffer)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [buffer:%p]", buffer);
    return L7_ERROR;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index[%u] is higher than the maximum allowed number of ping sessions", index);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index is does not belong to a created session [index:%u]", index);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_pingsession_free(L7_uint8 sessionIdx)
{
  ...
  {
    LOG_PT_INFO(LOG_CTX_ROUTING, "Requested index does not belong to a created session [index:%u]", sessionIdx);
    return L7_SUCCESS;
  }
                                              ...
                                              L7_BOOL dontFrag, L7_uint16 port, L7_ushort16 maxTtl, L7_ushort16 initTtl, L7_ushort16 maxFail)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index[%u] is higher than the maximum allowed number of traceroute sessions", sessionIdx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index is already being used in an active traceroute session [index:%u]", index);
    return L7_FAILURE;
  }
                                   ...
                                   L7_NULLPTR, (void*)&__traceroute_sessions[sessionIdx].index, &__traceroute_sessions[sessionIdx].handle))
  {
    ...
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to start a new traceroute session");
    return L7_FAILURE;
  }
...
L7_RC_t ptin_routing_traceroutesession_query(msg_RoutingTracertSessionQuery* buffer)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index[%u] is higher than the maximum allowed number of ping sessions", buffer->sessionIdx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index is does not belong to a created session [index:%u]", buffer->sessionIdx);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "Query [index:%u]",  buffer->sessionIdx);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Handle:       %u", session->handle);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Status:       %u", session->isRunning);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  CurrTtl:      %u", session->currTtl);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Hop Count:    %u", session->currHopCount);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Probe Count:  %u", session->currProbeCount);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Test Attempt: %u", session->testAttempt);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  Test Success: %u", session->testSuccess);
...
L7_RC_t ptin_routing_traceroutesession_gethops(L7_uint32 sessionIdx, L7_uint16 firstIdx, L7_uint32 maxEntries, L7_uint32* readEntries, msg_RoutingTracertSessionHopsResponse* buffer)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index is does not belong to a created session [sessionIdx:%u]", sessionIdx);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_DEBUG(LOG_CTX_ROUTING, "There are not hops for this traceroute session");
    *readEntries = 0;
    return L7_SUCCESS;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Local snapshot handle does not match with the requested session [local:%u requested:%u]!", snapshotIterator->sessionHandle, __traceroute_sessions[sessionIdx].handle);
    *readEntries = 0;
    return L7_FAILURE;
  }
  ...
  while( (currentIndex < maxEntries) && (snapshotIterator != NULL) )
  {
    if(currentIndex >= firstIdx)
    {
      LOG_PT_TRACE(LOG_CTX_ROUTING, "Copying Hop[idx:%u]", currentIndex);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  TTL:        %u",    snapshotIterator->ttl);      
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  IP Address: %08X",  snapshotIterator->ipAddr);   
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  Min rtt:    %u",    snapshotIterator->minRtt);   
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  Max rtt:    %u",    snapshotIterator->maxRtt);   
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  Avg rtt:    %u",    snapshotIterator->avgRtt);   
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  Probe Sent: %u",    snapshotIterator->probeSent);
      LOG_PT_TRACE(LOG_CTX_ROUTING, "  Probe Rcvd: %u",    snapshotIterator->probeRecv);
...
L7_RC_t ptin_routing_traceroutesession_free(L7_uint8 sessionIdx)
{
  ...
  if(__traceroute_sessions[sessionIdx].handle == 0)
  {
    LOG_PT_INFO(LOG_CTX_ROUTING, "Requested index is does not belong to a created session [index:%u]", sessionIdx);
    return L7_SUCCESS;
  }
...
L7_RC_t ptin_routing_L3UcastTtl1ToCpu_set(L7_BOOL enable)
{
  ...
  if (rc != L7_SUCCESS)
    LOG_PT_ERR(LOG_CTX_API,"Error applying HW L3UcastTtl1ToCpu");
...
L7_RC_t __ptin_routing_ICMPRedirects_set(L7_uint32 routingIntfNum, L7_BOOL enable)
{
  ...
  */
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting ICMP Redirects on intfIfNum %u to value %d", routingIntfNum, enable);
  if(usmDbIpMapIfICMPRedirectsModeSet(PTIN_ROUTING_USMDB_UNITINDEX, routingIntfNum, enable) != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Error while setting ICMP Redirects on intfIfNum %u to value %d", routingIntfNum, enable);
    return L7_FAILURE;
  }

  /* Enable/Disable ICMP Redirects on this Router */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting ICMP Redirects on Router to value %d", routingIntfNum, enable);
...
static L7_int __ioctl_dtl0_enable(void)
{
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "ioctl request -> SIOCSIFFLAGS");
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_name  = %s",     request.ifr_name);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_flags = 0x%04X", request.ifr_flags);
...
static L7_int __ioctl_dtl0_mtu_set(L7_uint32 mtu)
{
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "ioctl request -> SIOCSIFMTU");
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_name = %s", request.ifr_name);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_addr = %u", request.ifr_addr.sa_family);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_mtu  = %u", request.ifr_mtu);
...
static L7_int __ioctl_vlanintf_add(L7_uint16 vlanId)
{
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "ioctl request -> SIOCGIFVLAN");
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  cmd     = %u", request.cmd);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  device1 = %s", request.device1);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  u.VID   = %u", request.u.VID);
...
static L7_int __ioctl_intf_rename_dtl2rt(L7_uint16 intfId, L7_uint16 vlanId)
{
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "ioctl request -> SIOCSIFNAME");
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_name    = %s", request.ifr_name);
  LOG_PT_TRACE(LOG_CTX_ROUTING, "  ifr_newname = %s", request.ifr_newname);
...
L7_RC_t __intf_macaddress_set(ptin_intf_t* intf, L7_enetMacAddr_t* macAddr)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [macAddr:%p]", macAddr);
    return L7_ERROR;
  }

  /* Configure the routing interface with the given MAC address */
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Setting intfnum:%u MAC address to %02X:%02X:%02X:%02X:%02X:%02X\n", 
...
static L7_RC_t __arptable_snapshot_init(void)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to get ARP table stats");
    return L7_FAILURE;
  }

  /* Create pool of free elements to use in the '__arptable_snapshot' */
  LOG_PT_INFO(LOG_CTX_ROUTING, "Creating pool of %u elements for the __arptable_snapshot", arpTableStats.cacheMax);
  if(NOERR != dl_queue_init(&__arptable_pool))
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to create new dl_queue for arp table snapshot");
    return L7_FAILURE;
  }
...
static void __arptable_snapshot_refresh(L7_uint32 intfNum)
{
  ...
  LOG_PT_TRACE(LOG_CTX_ROUTING, "Clearing current snapshot and return all elements to the pool");
  ...
  LOG_PT_DEBUG(LOG_CTX_ROUTING, "Refreshing local snapshot");
  while(L7_ERROR != usmDbIpArpEntryNext(PTIN_ROUTING_USMDB_UNITINDEX, arpTablepEntry.ipAddr, arpTablepEntry.intIfNum, &arpTablepEntry))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Not enough free space in the local snapshot to save all os usmDb contents [insertedEntries:%u __arptable_entries_max:%u]", insertedEntries, __arptable_entries_max);
      return;
    }
...
static L7_RC_t __routetable_snapshot_init(void)
{
  ...
  LOG_PT_INFO(LOG_CTX_ROUTING, "Creating pool of %u elements for the __routetable_snapshot", routeTableMaxEntries);
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to create new dl_queue for route table snapshot");
    return L7_FAILURE;
  }
...
static void __routetable_snapshot_refresh(L7_uint32 intfNum)
{
  ...
  while(L7_ERROR != usmDbNextRouteEntryGet(PTIN_ROUTING_USMDB_UNITINDEX, &routeTablepEntry, L7_FALSE))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "There has to be at least one best route in this entry!");
      return;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Not enough free space in the local snapshot to save all os usmDb contents [insertedEntries:%u __routetable_entries_max:%u]", insertedEntries, __routetable_entries_max);
      return;
    }
...
static L7_RC_t __ping_sessions_init(void)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to determine max number of ping sessions");
    return L7_FAILURE;
  }
...
static L7_RC_t __ping_session_finish_callback(void *userParam)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Abnormal context [userParam:%p]", userParam);
    return L7_FAILURE;
  }
  index = *(L7_uint8*)userParam;

  LOG_PT_TRACE(LOG_CTX_ROUTING, "Ping session %u has finished", index);
...
static L7_RC_t __traceroute_sessions_init(void)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to determine max number of traceroute sessions");
    return L7_FAILURE;
  }
  ...
  LOG_PT_INFO(LOG_CTX_ROUTING, "Creating pool of %u elements for the __traceroutehops_snapshot", __traceroute_hops_max);
  if(NOERR != dl_queue_init(&__traceroutehops_pool))
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Unable to create new dl_queue for traceroute hops snapshot");
    return L7_FAILURE;
  }
...
static void __traceroutehops_snapshot_refresh(L7_uint32 sessionIdx)
{
  ...
  while((L7_SUCCESS == usmDbTraceRouteHopGetNext(sessionHandle, &lastHopIdx, &ttl, &ipAddr,  &minRtt, &maxRtt, &avgRtt, &probeSent, &probeRecv)))
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_ROUTING, "Not enough free space in the local snapshot to save all os usmDb contents [insertedEntries:%u __traceroute_hops_max:%u]", insertedEntries, __traceroute_hops_max);
      return;
    }
...
void ptin_routing_traceroutesession_dump(L7_uint32 index)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_ROUTING, "Requested index[%u] is higher than the maximum allowed number of traceroute sessions", index);
    return;
  }
