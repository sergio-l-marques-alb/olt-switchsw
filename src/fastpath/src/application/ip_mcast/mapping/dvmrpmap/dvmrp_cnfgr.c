...
L7_RC_t dvmrpMapTaskInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  /* Create the DVMRP-MAP Queue Counters Semaphore */
  if ((dvmrpGblVar_g.queueCountersSemId = osapiSemaBCreate (OSAPI_SEM_Q_FIFO,
                                                            OSAPI_SEM_FULL))
                        == L7_NULLPTR)
  {
    for (QIndex = 0; QIndex < DVMRP_MAX_Q; QIndex++)
    ...
    if (dvmrpGblVar_g.msgQSema != L7_NULLPTR)
    {
      osapiSemaDelete (dvmrpGblVar_g.msgQSema);
    }
    ...
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
    DVMRP_MAP_DEBUG (DVMRP_MAP_DEBUG_FAILURES, "queueCountersSemId creation error");
    return L7_FAILURE;
  }

  /* Create the DVMRP-MAP Event Counters Semaphore */
  if ((dvmrpGblVar_g.eventCountersSemId = osapiSemaBCreate (OSAPI_SEM_Q_FIFO,
                                                            OSAPI_SEM_FULL))
                        == L7_NULLPTR)
  {
    for (QIndex = 0; QIndex < DVMRP_MAX_Q; QIndex++)
    ...
    if (dvmrpGblVar_g.queueCountersSemId != L7_NULLPTR)
    {
      osapiSemaDelete (dvmrpGblVar_g.queueCountersSemId);
    }
    ...
    cbData.asyncResponse.u.reason = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    cnfgrApiCallback(&cbData);
    DVMRP_MAP_DEBUG (DVMRP_MAP_DEBUG_FAILURES, "eventCountersSemId creation error");
    return L7_FAILURE;
  }
