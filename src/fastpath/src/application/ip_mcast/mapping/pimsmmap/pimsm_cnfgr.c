          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                if ((pimsmRC = pimsmCnfgrInitPhase3Process(warmRestart, &response, &reason)) != L7_SUCCESS)
                {
          ...
          case L7_CNFGR_CMD_EXECUTE:
            switch ( request )
            {
              case L7_CNFGR_RQST_E_START:
                if (pimsmGblVariables_g.warmRestart == L7_FALSE)
                {
...
L7_RC_t pimsmCnfgrInitPhase1DynamicProcess (pimsmMapCB_t* pimsmMapCb)
{
  ...
  /* don't create IPv6 control block if active SDM template does not 
   * support IPv6. */
  if ((pimsmMapCb->familyType == L7_AF_INET6) &&
      !sdmTemplateSupportsIpv6())
  {
    return L7_SUCCESS;
  }

  /* Initialize the PIM-SM Control Block */
  pimsmMapCb->cbHandle = pimsmCtrlBlockInit(pimsmMapCb->familyType, pimsmMapCb->warmRestartInProgress);
...
L7_RC_t pimsmCnfgrFiniPhase1DynamicProcess (pimsmMapCB_t* pimsmMapCb)
{
  ...
  if ((pimsmMapCb->familyType == L7_AF_INET) ||
      ((pimsmMapCb->familyType == L7_AF_INET6) && sdmTemplateSupportsIpv6()))
  {
...
/*********************************************************************
* @purpose  This function builds and applies default configuration.
*
* @param    pimsmMapCbPtr @b{(input)}   Mapping Control Block.
* @param    pResponse     @b{(output)}  Response if L7_SUCCESS.
* @param    pReason       @b{(output)}  Reason if L7_ERROR.
...
* @end
*********************************************************************/
static L7_RC_t pimsmCnfgrConfigInitialize( L7_uint32 cbIndex, pimsmMapCB_t *pimsmMapCbPtr,
                 L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason)
{
  if (pimsmConstsDefs[cbIndex].buildDefaultConfigData == L7_NULL)
  {
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_FAILURES, 
                      "Default build config function is NULL in CB");
    return L7_FAILURE;
  }

  pimsmConstsDefs[cbIndex].buildDefaultConfigData(PIMSM_DEBUG_CFG_VER_CURRENT);
...
* @end
*********************************************************************/
L7_RC_t pimsmCnfgrInitPhase3Process( L7_BOOL warmRestart, L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t *pReason )
{
  ...
  *pReason   = 0;
  ...
  PIMSM_MAP_DEBUG (PIMSM_MAP_DEBUG_NSF, "Restart is %s", warmRestart ? "WARM" : "COLD");
  pimsmGblVariables_g.warmRestart = warmRestart;

  protoMax =  pimsmMapProtocolsMaxGet();
  for (cbIndex = PIMSM_MAP_IPV4_CB; cbIndex < protoMax; cbIndex++)
  {
    pimsmMapCbPtr = &pimsmMapCB[cbIndex];
    pimsmMapCbPtr->warmRestartInProgress = warmRestart;

    rc = pimsmCnfgrConfigInitialize(cbIndex, pimsmMapCbPtr, pResponse, pReason);
    if (rc != L7_SUCCESS)
    {
      return rc;
    }
