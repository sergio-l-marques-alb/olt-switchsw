                                ...
                                L7_MGMD_INTF_MODE_t intfType)
{
      ...
      && ((mgmdMapCbPtr->familyType == L7_AF_INET6) && (intfCfg->version == L7_MGMD_VERSION_2)) )
  {
                                      ...
                                      &allRoutersAddr,mode) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_MGMD_MAP_COMPONENT_ID,
              "MGMD All Routers Address - %s Set to the DTL Mcast List Failed; "
               "Mode - %d, intf - %d, %s", inetAddrPrint(&allRoutersAddr,addr),
               mode, intIfNum, ifName);
    }
  }
