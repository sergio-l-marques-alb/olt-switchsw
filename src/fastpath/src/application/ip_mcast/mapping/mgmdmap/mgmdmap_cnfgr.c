...
static L7_RC_t mgmdCnfgrConfigInitialize(mgmdMapCB_t *mgmdMapCbPtr,
                 L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason);
/*********************************************************************
*
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                if ((mgmdRC = mgmdCnfgrInitPhase3Process(warmRestart, &response, &reason )) != L7_SUCCESS)
                {
          ...
          case L7_CNFGR_CMD_EXECUTE:
            switch ( request )
            {
              case L7_CNFGR_RQST_E_START:
                if (mgmdMapGblVariables_g.warmRestart == L7_FALSE)
                {
                  /* if cold restart send sync complete now */
...
L7_RC_t mgmdCnfgrMgmdInitPhase1DynamicProcess (mgmdMapCB_t* mgmdMapCb)
{
  ...
  if ((mgmdMapCb->familyType == L7_AF_INET) ||
      ((mgmdMapCb->familyType == L7_AF_INET6) && 
  ...
  L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason)
{
  ...
  /* Create the Multicast Heap. Always create the v4 heap. Create the IPv6
   * heap if the active SDM template supports IPv6. */
  if ((mgmdMapCbPtr->familyType == L7_AF_INET) ||
      ((mgmdMapCbPtr->familyType == L7_AF_INET6) && sdmTemplateSupportsIpv6()))
  {
...
/*---------------------------*/
                                   ...
                                   mgmdMapRoutingEventCallback) != L7_SUCCESS)
  {
    mgmdRC = L7_ERROR;
    *pReason = L7_CNFGR_ERR_RC_FATAL;
    MGMD_MAP_DEBUG (MGMD_MAP_DEBUG_FAILURES, "Error registering with ip6MapRegisterRoutingEventChange().\n");
  }
...
* @end
*********************************************************************/
L7_RC_t mgmdCnfgrInitPhase3Process( L7_BOOL warmRestart,
                                    ...
                                    L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  mgmdMapGblVariables_g.warmRestart = warmRestart;
  ...
  *pReason   = 0;
  ...
  for (cbIndex = L7_NULL, mgmdMapCbPtr = &mgmdMapCB_g[0];
                       cbIndex < protoMax; cbIndex++, mgmdMapCbPtr++)
  {
    mgmdMapCbPtr->warmRestartInProgress = warmRestart;

    /* Intialize & allocate memory. */
    mgmdRC = mgmdCnfgrConfigInitialize(mgmdMapCbPtr, pResponse, pReason);
    if (mgmdRC != L7_SUCCESS)
    {
...
* @end
*********************************************************************/
L7_RC_t mgmdCnfgrConfigInitialize(mgmdMapCB_t *mgmdMapCbPtr,
                 L7_CNFGR_RESPONSE_t *pResponse, L7_CNFGR_ERR_RC_t   *pReason)
{
  if (mgmdMapCbPtr->familyType == L7_AF_INET)
  {
  ...
  {
    mgmd6BuildDefaultConfigData(L7_MGMD_CFG_VER_CURRENT);
  }
