...
static L7_RC_t
pimdmStateRfrFsmActionReplayStateRefreshResetSRT (pimdmMrtEntry_t* mrtEntry,
                                         pimdmStrfrEventInfo_t *strfrEventInfo);
...
pimdmStateRfrFsmState[PIMDM_STATE_RFR_MAX_EVENTS][PIMDM_STATE_RFR_MAX_STATES] =
{
   ...
   */
  {{PIMDM_STATE_RFR_STATE_NO_ORIG, pimdmStateRfrFsmActionDoNothing},
   {PIMDM_STATE_RFR_STATE_ORIG, pimdmStateRfrFsmActionReplayStateRefreshResetSRT}
  }
 };
...
static L7_uchar8* pimdmStateRfrFsmEventList[] =
{
  ...
  "PIMDM_SRSM_EVT_NBR_GENID_CHANGED",
};
...
void pimdmStateRefreshTimerExpiryHandler (void *mrtEntryUpstrmTmrHndlParam)
{
  ...
  if (mrtEntry->origStateInfo.stateRfrTimer == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_PIMDM_MAP_COMPONENT_ID,
            "PIMDM stateRfrTimer (SRT) is NULL, But Still Expired");
    return;
  }
...
void pimdmSourceActiveTimerExpiryHandler (void *mrtEntryUpstrmTmrHndlParam)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_FLEX_PIMDM_MAP_COMPONENT_ID,
            "PIMDM srcActiveTimer (SAT) is NULL, But Still Expired");
    return;
  }
...
* @end
*********************************************************************/
static L7_RC_t pimdmStateRfrFsmActionReplayStateRefreshResetSRT (
                                          pimdmMrtEntry_t* mrtEntry,
                                          pimdmStrfrEventInfo_t *strfrEventInfo)
{
  pimdmCB_t *pimdmCB = mrtEntry->pimdmCB;
  ...
  L7_uint32 rtrIfNum = strfrEventInfo->rtrIfNum;

  PIMDM_TRACE (PIMDM_DEBUG_FSM_STRFR, "Entry");

  /* If Neighbor is Downstream, replay the recently sent State Refresh
   * Message on this interface.
   */
  /* If this interface is Scope Boundary, Skip it.
   */
  if (BITX_TEST (&mrtEntry->boundaryGMask, rtrIfNum) != 0)
  {
    PIMDM_TRACE (PIMDM_DEBUG_FAILURE, "Group - %s is AdminScoped on Intf - %d",
                inetAddrPrint(&mrtEntry->grpAddr,grp), rtrIfNum);
    return L7_SUCCESS;
  }

  /* Let the Assert Winner do the State Refresh.
   */
  if (BITX_TEST (&mrtEntry->lostAssertMask, rtrIfNum) != 0)
  {
    PIMDM_TRACE (PIMDM_DEBUG_FAILURE, "Intf - %d is Assert Loser", rtrIfNum);
    return L7_SUCCESS;
  }

  if ((dnstrmIntfInfo = mrtEntry->downstrmStateInfo[rtrIfNum]) == L7_NULLPTR)
  {
    PIMDM_TRACE (PIMDM_DEBUG_FAILURE, "Dnstrm Intf - %d is NULL", rtrIfNum);
    return L7_SUCCESS;
  }
  ...
  if (PIMDM_BIT_TST (stRfrMsg->reserved, PIMDM_STRFR_ASRT_OVERRIDE_BIT))
  {
    asrtOverrideFlag = L7_TRUE;
  }
  if (PIMDM_BIT_TST (stRfrMsg->reserved, PIMDM_STRFR_PRUNE_NOW_BIT))
  {
   ...
   */
  if (pimdmStateRefreshMsgSend (pimdmCB, &stRfrMsg->srcAddr,
                                &stRfrMsg->grpAddr, &stRfrMsg->origAddr,
                                rtrIfNum, stRfrMsg->stRfrTTL,
                                stRfrMsg->interval,
                                stRfrMsg->metricPref, stRfrMsg->metric,
                                stRfrMsg->maskLen, pruneIndFlag,
                                asrtOverrideFlag, pruneNowFlag)
                             != L7_SUCCESS)
  {
    PIMDM_TRACE (PIMDM_DEBUG_FAILURE, "Failed to send State Refresh Message "
                 "on Dnstrm Intf - %d  for Src - %s and Grp - %s", rtrIfNum,
                 inetAddrPrint(srcAddr,src), inetAddrPrint(grpAddr,grp));
    return L7_FAILURE;
  }
