                                   ...
                                   pimsmBsrPerScopeZone_t **ppBsrNode)
{
  ...
  if (pBsrEntry == L7_NULLPTR)
  {
    ...
    {
      bsrNode->pimsmBsrFwdOnRcvdIntf = L7_TRUE;
    }
                                          ...
                                          pimsmBsrPerScopeZone_t *pBsrNode)
{
  ...
  /* 
   * during the warm restart interval, set BSR timeout interval to minimum allowed in order
   * to speed up state convergence
   */
                           ...
                           L7_uint32                rtrIfNum)
{
  ...
  for (rtrIfNumTemp = 1; rtrIfNumTemp < MCAST_MAX_INTERFACES; rtrIfNumTemp++)
  {
    if (pimsmCb->family == L7_AF_INET)
    {
      if ((rtrIfNum == rtrIfNumTemp) &&
          (pBsrNode->pimsmBsrFwdOnRcvdIntf != L7_TRUE))
      {
        /* Do not forward the BSM on the Received Interface.
         * Please see the comments in pimsmBsrBootstrapRecv() to more details.
         */
        continue;
      }
    }
    else
    {
      if (rtrIfNum == rtrIfNumTemp)
      {
        /* Do not forward the BSM on the Received Interface for IPv6.
         */
        continue;
      }
    }
                              ...
                              L7_uint32        rtrIfNum)
{
  ...
  /* This is to avoid Looping of BSR Packets when forwarded on the receiving 
   * interface.  If the BSM is received from a Neighbor...
   * 1. Check to see if the Sender of the BSM is the BSR.
   * 2. Check to see if the Elected BSR is your Neighbor.
   * If (1) fails and (2) succeeds, then Do not Forward this BSM on the 
   * interface on which it is received.
   */
  if (pimsmCb->family == L7_AF_INET)
  {
    if ((L7_INET_ADDR_COMPARE(pSrcAddr, &bsrAddr) != 0) &&
        (pimsmNeighborEntryGet(pimsmCb, rtrIfNum, &bsrAddr) == L7_SUCCESS))
    {
      PIMSM_TRACE(PIMSM_DEBUG_BIT_BSR, PIMSM_TRACE_ERROR,"Do not Forward the BSM "
                  "on the received interface as it is not received from the "
                  "Elected BSR and the Elected BSR is your Neighbor.");
      pBsrNode->pimsmBsrFwdOnRcvdIntf = L7_FALSE;
    }
  }
  ...
  /* Reset this flag as forwarding is complete */
  if (pimsmCb->family == L7_AF_INET)
  {
    pBsrNode->pimsmBsrFwdOnRcvdIntf = L7_TRUE;
  }
