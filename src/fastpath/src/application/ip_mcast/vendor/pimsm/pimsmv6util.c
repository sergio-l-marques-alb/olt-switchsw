...
#define PIMSM_IPV6_POOL_LIST_SIZE  24
...
*
*           When a pool is added here, increment PIMSM_IPV6_POOL_LIST_SIZE.
*
* @end
*********************************************************************/
heapBuffPool_t *pimsmV6HeapPoolListGet(void)
{
  ...
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmCB_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_CB", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Join/Prune Message Buffers */
  pimsmV6HeapPoolList[i].buffSize = PIMSM_MAX_JP_MSG_SIZE;
  pimsmV6HeapPoolList[i].buffCount = PIMSM_MAX_JP_PACKETS;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_JPG_MSG_BUFFER", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Join/Prune Address List Buffers for IPv6 */
  pimsmV6HeapPoolList[i].buffSize = PIM_IPV6_MAX_JOIN_OR_PRUNE_ADDRS_IN_LIST * sizeof(pim_encod_src_addr_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_MAX_PROTOCOL_PACKETS;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_JP_ADDR_LIST_BUFFER", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Cache Entry Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof(pimsmCache_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_MAX_IPV6_CACHE_ENTRIES;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_CACHE_BUFFERS", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* AVL Tree Heap Buffers for (S,G), (*,G) and (S,G,Rpt) Databases */
  pimsmV6HeapPoolList[i].buffSize = platIpv6McastRoutesMaxGet() * sizeof (avlTreeTables_t);
  pimsmV6HeapPoolList[i].buffCount = 3;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_SG_TREE_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* AVL Data Heap Buffers for (S,G) Database */
  pimsmV6HeapPoolList[i].buffSize = platIpv6McastRoutesMaxGet() * sizeof (pimsmSGNode_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_SG_DATA_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Downstream Interface Buffers for (S,G) and (*,G) Databases */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmSGIEntry_t);
  pimsmV6HeapPoolList[i].buffCount = ((PIMSM_NUM_OPTIMAL_OUTGOING_INTERFACES * platIpv6McastRoutesMaxGet()) + 
                                      (PIMSM_NUM_OPTIMAL_OUTGOING_INTERFACES * platIpv6McastRoutesMaxGet()));
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_SGI_*GI_BUFFERS", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* AVL Data Heap Buffers for (S,G,Rpt) Database */
  pimsmV6HeapPoolList[i].buffSize = (platIpv6McastRoutesMaxGet() * sizeof (pimsmSGRptNode_t));
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_SG_RPT_DATA_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Downstream Interface Buffers for (S,G,Rpt) Database */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmSGRptIEntry_t);
  pimsmV6HeapPoolList[i].buffCount = (PIMSM_NUM_OPTIMAL_OUTGOING_INTERFACES * platIpv6McastRoutesMaxGet());
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_SGRptI_BUFFERS", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* AVL Data Heap Buffers for (*,G) Database */
  pimsmV6HeapPoolList[i].buffSize = platIpv6McastRoutesMaxGet() * sizeof (pimsmStarGNode_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_*G_DATA_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* (*,*,RP) Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmStarStarRpNode_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_STAR_STAR_RP_TBL_SIZE_TOTAL;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_**RP_BUFFERS", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Downstream Interface Buffers for (*,*,RP) Database */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmStarStarRpIEntry_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_NUM_OPTIMAL_OUTGOING_INTERFACES * PIMSM_STAR_STAR_RP_TBL_SIZE_TOTAL;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_**RPI_BUFFERS", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RP Block Buffer */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmRpBlock_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_RP_BLOCK", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RADIX Tree Heap Buffers for RP Set Database */
  pimsmV6HeapPoolList[i].buffSize = (RADIX_TREE_HEAP_SIZE(PIMSM_RP_GRP_ENTRIES_MAX,sizeof(rpSetTreeKey_t)));
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_RP_SET_TREE_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RADIX Data Heap Buffers for RP Set Database */
  pimsmV6HeapPoolList[i].buffSize = PIMSM_RP_GRP_ENTRIES_MAX * sizeof (pimsmRpSetNode_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_RP_SET_DATA_HEAP", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RP Group List Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmRpGrpNode_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_RP_GRP_ENTRIES_MAX;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_RP_GRP_LIST", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RP Group Address List Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmAddrList_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_RP_GRP_ADDR_LIST_NODES_MAX;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_MRT_RP_GRP_ADDR_LIST", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* BSR Block Buffer */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmBSRBlock_t);
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_BSR_BLOCK", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* BSR Per Scope Zone Buffer */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmBsrPerScopeZone_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_BSR_SCOPE_NODES_MAX;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_BSR_PER_SCOPE_ZONE", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* BSR Fragment Group-RP Node Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmFragGrpRpNode_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_BSR_FRAGMENT_GRP_ENTRIES_MAX;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_FRAG_GRP_RP_NODE", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* BSR Fragment RP Node Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmFragRpNode_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_BSR_FRAGMENT_RP_ENTRIES_MAX;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_FRAG_RP_NODE", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* Neighbor Database Buffers */
  pimsmV6HeapPoolList[i].buffSize = sizeof (pimsmNeighborEntry_t);
  pimsmV6HeapPoolList[i].buffCount = PIMSM_MAX_NBR;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_NBR_DATABASE", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;

  /* RTO-IPv6 Route Change Buffers */
  pimsmV6HeapPoolList[i].buffSize = (PIMSM_BEST_ROUTE_CHANGES_MAX * sizeof(rto6RouteChange_t));
  pimsmV6HeapPoolList[i].buffCount = 1;
  osapiStrncpySafe(pimsmV6HeapPoolList[i].name, "PIMSM_RTO6_RtBuf", HEAP_POOLNAME_SIZE);
  pimsmV6HeapPoolList[i].flags = HEAP_BUFFPOOL_NO_OVERFLOW;
  i++;
