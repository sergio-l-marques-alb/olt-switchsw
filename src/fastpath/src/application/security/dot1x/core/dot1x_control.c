...
static 
L7_RC_t dot1xDynamicVlanDelete(L7_uint32 vlanId);
...
L7_RC_t dot1xStartTasks()
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "Unable to initialize dot1xTimerTask()\n");
    return L7_FAILURE;
  }
...
L7_RC_t dot1xFillMsg(void *data, dot1xMsg_t *msg)
{
  ...
  case dot1xMgmtAdminMonitorModeEnable:
  case dot1xMgmtAdminMonitorModeDisable:
...
L7_RC_t dot1xIssueCmd(L7_uint32 event, L7_uint32 intIfNum, void *data)
{
  ...
  if (rc != L7_SUCCESS)
  {
    DOT1X_ERROR_SEVERE("Failed to send to dot1xQueue! Event: %u, interface: %s\n", event, ifName);
  }
...
L7_RC_t dot1xDispatchCmd(dot1xMsg_t *msg)
{
  ...
  case dot1xIntfStartup:
    rc = dot1xIhProcessIntfStartup(msg->data.startupPhase);
    break;
  ...
  case dot1xMgmtAdminMonitorModeEnable:
  case dot1xMgmtAdminMonitorModeDisable:
    rc = dot1xCtlAdminMonitorModeSet(msg->data.msgParm);
    break;
        ...
        if (logicalPortInfo->inUse == L7_TRUE)
        {
             ...
             dot1xPortInfo[phyIntf].portControlMode  == L7_DOT1X_PORT_AUTO_MAC_BASED))
          {
            ...
            if (logicalPortInfo->guestVlanTimer!= 0)
            {
              ...
              if (logicalPortInfo->guestVlanTimer == 0)
              {
                DOT1X_EVENT_TRACE(DOT1X_TRACE_TIMER,phyIntf,"\n%s(%d) :: GUEST VLAN TIMER Expires to Zero for lIntIfNum:[%d]\n\r",__FUNCTION__,__LINE__,lIntIfNum);  
                ...
                else
                {
                  ...
                  DOT1X_EVENT_TRACE(DOT1X_TRACE_TIMER,phyIntf,"\n%s:%d Client is being placed in Guest VLAN, txWhenLp Timer making to NULL %d\n",
                                     ...
                                     L7_uint32 paeCapabilities)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "DOT1x: dot1xCtlPortPaeCapabilitiesSet function failed at  --%d\r\n",__LINE__);
    return L7_FAILURE;
  }
...
L7_RC_t dot1xCtlApplyConfigData(void)
{
  ...
  if (rc != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1xApplyConfigData: Unable to %s dot1x in driver\n", (L7_char8 *)modeStr);
  }
        ...
        if (dot1xCtlApplyPortConfigData(intIfNum) != L7_SUCCESS)
         {
          ...
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                  "Failure in applying config data for interface %s\n", ifName);
         }
      }
      else
      {
...
L7_RC_t dot1xRadiusServerVlanAssignmentHandle(L7_uint32 intIfNum, L7_uint32 vlanId)
{
  ...
  L7_RC_t   rc = L7_SUCCESS;
  L7_RC_t   vlanCheck = L7_SUCCESS;

  dot1xDynamicVlanCreationModeGet(&dynamicVlanMode);
  vlanCheck = dot1qStaticVlanCheckValid(vlanId);
  ...
  if(dynamicVlanMode == L7_DISABLE)
  {
   ...
   if ((vlanCheck == L7_SUCCESS) && (((dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED) &&
  ...
  else /* Dynamic vlan mode is enabled */
  {
   ...
   {
     /* If given vlan is not there and remaining conditions met then create dynamic vlan */
     if ((((dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED) &&
       ...
       ((dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO) &&
        (curr_mode != DOT1Q_SWPORT_MODE_TRUNK))) )
     {
       rc = dot1qVlanCreate(vlanId, DOT1Q_DOT1X);
       osapiSleepMSec (100);   
       return rc;
     }
     else
     {
       /*Other conditions not met, then dont create dynamic vlan */
       return L7_FAILURE;  		
     }
   }   
  }
...
*
*********************************************************************/
L7_RC_t dot1xRadiusServerVlanConversionHandle(const L7_char8 *vlanName, L7_uint32 *vlanId)
{

  /*Check that any vlan has same name with given string*/
  if(dot1qVlanIdFromNameGet(vlanName, vlanId) == L7_SUCCESS)
  {
    return L7_SUCCESS;
  } /*check that given string is pure numeric string */
  else if (usmDbStringNumericCheck(vlanName) == L7_SUCCESS)
  {
...
* @end
**********************************************************************/
static L7_RC_t dot1xDynamicVlanDelete(L7_uint32 vlanId)
{
  ...
  if(dot1qStaticVlanCheckValid(vlanId) != L7_SUCCESS)
  {
    do
    {
      logicalPortInfo = dot1xLogicalPortInfoGetNext(lIntIfNum);
      if(logicalPortInfo != L7_NULLPTR)
      {
        if(logicalPortInfo->vlanAssigned == vlanId)
        {
          /*if one entry is found we can stop the dynamic vlan deletion */      
          DOT1X_EVENT_TRACE(DOT1X_TRACE_EXTERNAL,logicalPortInfo->physPort ,"\n%s:%d: Cannot delete VLAN %d as it is associated with lInterface:%d\n\r", 
                         __FUNCTION__,__LINE__,vlanId, lIntIfNum);
          entryFound = L7_TRUE;
          break;
        }
    ...
    }while(logicalPortInfo != L7_NULLPTR);

    if(entryFound == L7_FALSE)
    {
      dot1qVlanTypeVal_t vlanType;

      if((dot1qOperVlanTypeGet(vlanId, &vlanType) == L7_SUCCESS) &&
         (vlanType == L7_DOT1Q_TYPE_DOT1X_REGISTERED))
      {
        if(dot1qVlanDelete(vlanId, DOT1Q_DOT1X) != L7_SUCCESS)
        {
          L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID, "ERROR: Failed to delete Dot1x Dynamic VLAN %d\n\r", vlanId);
          return L7_FAILURE;
        }
      }
      else
      {
        DOT1X_EVENT_TRACE(DOT1X_TRACE_EXTERNAL,0,
             "Cannot delete Dynamic VLAN %d as the VLAN is not created by DOT1X, vlanType:%d\n\r",__FUNCTION__,__LINE__,vlanId,vlanType);
      }
    }
  }
...
L7_RC_t dot1xVlanPortDeleteProcess(L7_uint32 intIfNum,L7_uint32 vlanId)
{
  ...
  /* If the given vlan is dynamic vlan and it is not used by dot1x vlan assignment then we can place the delete request */
  (void)dot1xDynamicVlanDelete(vlanId);
...
L7_RC_t dot1xVlanAssignmentDisable(L7_uint32 intIfNum,L7_uint32 vlanId)
{
  ...
  while (nimRc == L7_SUCCESS)
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID, "ERROR: Physical Port:%d is not Dot1x Configurable!\n\r",intf);
      return L7_FAILURE;
    }
      ...
      while((logicalPortInfo=dot1xLogicalPortInfoGetNextNode(intf,&lIntIfNum))!= L7_NULLPTR)
      {
        ...
        else
        {
          if (logicalPortInfo->vlanAssigned == vlanId)
          {
            ...
            if(dot1qStaticVlanCheckValid(vlanId) != L7_SUCCESS)
            {
              /* If the given vlan is dynamic vlan and it is not used by dot1x vlan assignment
                 then we can place the delete request */
              (void)dot1xDynamicVlanDelete(vlanId);
            }
          }
          else if (logicalPortInfo->guestVlanId == vlanId)
          {
            logicalPortInfo->guestVlanId = 0;
            logicalPortInfo->guestVlanTimer = 0;
          }
          ...
          else if((logicalPortInfo->isMonitorModeClient == L7_TRUE) &&
                  (logicalPortInfo->defaultVlanId == vlanId))
          {
          ...
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
                ...
                " Dot1x assigned vlan to the logical port (client) is deleted",
                      lIntIfNum,vlanId);
...
L7_RC_t dot1xPortVlanAssignmentDisable(L7_uint32 intIfNum)
{
  ...
  while((logicalPortInfo=dot1xLogicalPortInfoGetNextNode(intIfNum,&lIntIfNum))!= L7_NULLPTR)
  {
    ...
    else
    {
      ...
      else if (logicalPortInfo->defaultVlanId != 0)
      {
...
L7_RC_t dot1xVlanChangeCallback(dot1qNotifyData_t *vlanData, L7_uint32 intIfNum, L7_uint32 event)
{
  ...
  if (!((DOT1X_IS_READY) || dot1xCnfgrState == DOT1X_PHASE_UNCONFIG_2))
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1xVlanChangeCallback: Received an vlan change callback while not ready to receive it");
    return L7_FAILURE;
  }
  ...
  if ((intIfNum!=0) && (dot1xPortInfo[intIfNum].portEnabled != L7_TRUE) && (event != VLAN_DELETE_PORT_NOTIFY))
  {
    return L7_SUCCESS;
  }
...
L7_RC_t dot1xCtlPortGuestVlanMode(L7_uint32 intIfNum,L7_uint32 VlanId)
{
  ...
  if(dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
  {
    if(dot1xPortInfo[intIfNum].portStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED)
    {
      ...
      while ((logicalPortInfo=dot1xLogicalPortInfoGetNextNode(intIfNum,&lIntIfNum))!= L7_NULLPTR)
      {
        if (logicalPortInfo->inUse == L7_TRUE && (logicalPortInfo->guestVlanId != 0) && (logicalPortInfo->guestVlanId != VlanId))
        {
          dot1xCtlPortGuestVlanRemove(logicalPortInfo->logicalPortNumber,logicalPortInfo->guestVlanId);
        }
      }
    }
    return L7_SUCCESS; /* In mac-based mode, in next cycle the guest vlan timer will start */
  }

  
  if ((dot1xPortInfo[intIfNum].portStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED) &&
      (logicalPortInfo->guestVlanId != 0) && (logicalPortInfo->guestVlanId != VlanId))
  {
    if (dot1qStaticVlanCheckValid(VlanId) == L7_SUCCESS)
    {
...
/*********************************************************************
* @purpose  Enable  dot1x Monitor Mode Default vlan to a specified interface
*
* @param    intIfNum   @b{(input)) internal interface number
* @param    vlanId @b{(input)} Default vlanId
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
* @end
*********************************************************************/
L7_RC_t dot1xCtlPortMonitorVlanMode(L7_uint32 intIfNum,L7_uint32 vlanId)
{
  dot1xLogicalPortInfo_t *logicalPortInfo = L7_NULLPTR;
  L7_uint32 lIntIfNum;
  
  if(dot1xCfg->monitorMode == L7_DISABLE)
  {
    return L7_FAILURE;
  }
      ...
      (dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED))
  {
    ...
    if (dot1qStaticVlanCheckValid(vlanId) == L7_SUCCESS)
    {
      ...
      do
      {
        ...
        if((logicalPortInfo != L7_NULLPTR) && (logicalPortInfo->defaultVlanId != 0) && 
           (logicalPortInfo->isMonitorModeClient == L7_TRUE) && 
           (logicalPortInfo->defaultVlanId != vlanId))
        {
          logicalPortInfo->defaultVlanId = 0;
          logicalPortInfo->isMonitorModeClient = L7_FALSE;
          ...
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
                "Reinitializing Monitor Mode Authenticated Port-%u as there is a change in Port PVID-%d."
                ,lIntIfNum, vlanId);
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
           "Invalid vlanId-%d recevied from Port PVID change event for Port-%u.\n\r", vlanId, intIfNum);
      return L7_FAILURE;
    }
  }
  return L7_SUCCESS;
...
L7_RC_t dot1xCtlPortGuestVlanTimerStart(L7_uint32 ifindex,L7_uint32 VlanId,dot1xPortType_t type)
{
  ...
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_uint32 defaultVlanId=L7_NULL;
  ...
  DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS,logicalPortInfo->physPort,
                    "%s:%d Called for %d with type: %d pCfg->guestVlanId:%d VlanId:%d\n",
                    __FUNCTION__,__LINE__,lIntIfNum,type,pCfg->guestVlanId, VlanId);
 

  if((dot1xIntfIsConfigurable(intIfNum, &pCfg) != L7_TRUE) ||
     (dot1xPortInfo[intIfNum].portEnabled != L7_TRUE)     || 
     (pCfg->guestVlanId == 0) || (pCfg->guestVlanId != VlanId))
  {
    DOT1X_EVENT_TRACE(DOT1X_TRACE_FAILURE,intIfNum,
                      "%s:%d Failed to start the Guest Vlan Timer for %d \n",
                      __FUNCTION__,__LINE__,intIfNum);
  ...
  if(dot1qStaticVlanCheckValid(VlanId) != L7_SUCCESS)
  {
    /* Assign Default PVID to Guest VLAN Timer only if Monitor Mode
     * is enabled and Guest VLANID is Invalid */
    if(dot1xCfg->monitorMode == L7_ENABLE)
    {
      /* Retreive Default VLAN PVID for monitoring mode interface */
      (void) dot1qQportsVIDGet(intIfNum, L7_DOT1Q_DEFAULT_VID_INDEX, &defaultVlanId);
      DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS, intIfNum,
                      "%s(): interface:[%d] MonitorVlanId:[%d]\n\r",
                      __FUNCTION__,intIfNum, defaultVlanId);
      ...
      dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO)
  {
    if (dot1xPortInfo[intIfNum].portStatus == L7_DOT1X_PORT_STATUS_UNAUTHORIZED)
    {
      ...
      DOT1X_EVENT_TRACE(DOT1X_TRACE_TIMER,intIfNum,
                        "%s:Started the GuestVlan Timer for %d seconds on interface %d \n",
...
L7_RC_t dot1xCtlPortGuestVlanOperational(L7_uint32 lIntIfNum)
{
  ...
  L7_uint32 physIntf;
  L7_RC_t rc = L7_SUCCESS;
  L7_BOOL guestVlanFlag=L7_FALSE;
  ...
  rc = dot1qStaticVlanCheckValid(pCfg->guestVlanId);

  if(rc != L7_SUCCESS)
  {
    /* Apply Monitor Mode Default VLAN */
    if(dot1xCfg->monitorMode == L7_ENABLE)
    {
      /* Retreive Default VLAN PVID for monitoring mode interface */
      (void) dot1qQportsVIDGet(physIntf, L7_DOT1Q_DEFAULT_VID_INDEX, &vlanId);
  ...
  {
    vlanId = pCfg->guestVlanId;
    guestVlanFlag = L7_TRUE;
    if (pCfg->reAuthEnabled == L7_TRUE)
    {
  ...
  if(rc == L7_SUCCESS)
  {
    if(dot1xPortInfo[physIntf].portControlMode == L7_DOT1X_PORT_AUTO)
    {
      dot1xCtlPortDot1xAcquire(logicalPortInfo->physPort, vlanId);
      rc = dot1xPortVlanMembershipSet(physIntf, vlanId, L7_TRUE);
    }
    else
    {
      /* Mac-based mode */
    ...
    if (rc == L7_SUCCESS)
    {
      if(guestVlanFlag == L7_TRUE)
      {
        logicalPortInfo->guestVlanId = vlanId;
        logicalPortInfo->defaultVlanId = L7_NULL;
        logicalPortInfo->isMonitorModeClient = L7_FALSE;
        DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS,logicalPortInfo->physPort,
                          "Authenticated the logical port %d on Guest Vlan %d\n",
                          lIntIfNum,vlanId);
      ...
      else
      {
        logicalPortInfo->defaultVlanId = vlanId;
        logicalPortInfo->isMonitorModeClient = L7_TRUE;
        logicalPortInfo->guestVlanId = L7_NULL;
        DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS,logicalPortInfo->physPort,
                          "Authenticated the logical port %d on Monitor Vlan %d\n",
                           lIntIfNum, vlanId);
        dot1xStateMachineClassifier(bamAssignMonitorModeVlan,
                                    lIntIfNum, L7_NULL, L7_NULLPTR);
        dot1xStateMachineClassifier(apmAssignMonitorModeVlan,
                                    lIntIfNum, L7_NULL, L7_NULLPTR);
      }

      /*** MONITOR THE EVENT **/
      /* If Monitor Mode is enabled, Authenticate the client on Monitor Mode VLAN */
      if((dot1xCfg->monitorMode == L7_ENABLE) && (logicalPortInfo->guestVlanId == L7_NULL) &&
               (logicalPortInfo->isMonitorModeClient == L7_TRUE))
      {
        if(dot1xPortInfo[physIntf].portControlMode == L7_DOT1X_PORT_AUTO)
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
               "Port %d gets Authenticated on Monitor Mode VLAN[%d] due to Guest VLAN Timer expiry\n\r",
                logicalPortInfo->physPort, logicalPortInfo->defaultVlanId);
          /* Log the information */
          dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_GUEST_VLAN_TIMER_EXPIRY,
                                       L7_DOT1X_PORT_STATUS_AUTHORIZED, logicalPortInfo);
        }
        else
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
               "Supplicant MacAddress [%2x:%2x:%2x:%2x:%2x:%2x] on lIntIfNum:%d gets Authenticated on Monitor Mode VLAN[%d] due to Guest VLAN Timer expiry\n\r",
                ...
                lIntIfNum, logicalPortInfo->defaultVlanId);
          /* Log the information */
          dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_GUEST_VLAN_TIMER_EXPIRY,
                                       L7_DOT1X_PORT_STATUS_AUTHORIZED, logicalPortInfo);
        }
      }
      else if(logicalPortInfo->guestVlanId != L7_NULL)
      {
        if(dot1xPortInfo[physIntf].portControlMode == L7_DOT1X_PORT_AUTO)
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
               "Port %d gets Authenticated on Guest VLAN[%d] due to Guest VLAN Timer expiry\n\r",
                logicalPortInfo->physPort, logicalPortInfo->guestVlanId);
          /* Log the information */
          dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_GUEST_VLAN_TIMER_EXPIRY,
                                       L7_DOT1X_PORT_STATUS_AUTHORIZED, logicalPortInfo);
        }
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
               "Supplicant MacAddress [%2x:%2x:%2x:%2x:%2x:%2x] on lIntIfNum:%d gets Authenticated on Guest Mode VLAN[%d] due to Guest VLAN Timer expiry\n\r",
                logicalPortInfo->suppMacAddr.addr[0],
                logicalPortInfo->suppMacAddr.addr[1],
                logicalPortInfo->suppMacAddr.addr[2],
                logicalPortInfo->suppMacAddr.addr[3],
                logicalPortInfo->suppMacAddr.addr[4],
                logicalPortInfo->suppMacAddr.addr[5],
                lIntIfNum, logicalPortInfo->guestVlanId);
          /* Log the information */
          dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_GUEST_VLAN_TIMER_EXPIRY,
                                       L7_DOT1X_PORT_STATUS_AUTHORIZED, logicalPortInfo);
        }
    ...
    if (dot1xIntfIsConfigurable(intf, &pCfg) == L7_TRUE)
    {
         ...
         (pCfg->guestVlanId == VlanId))
      {
        if((dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO) &&
           ...
           (dot1xPortInfo[intIfNum].portEnabled == L7_TRUE))
        {
          logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intf);
          if (logicalPortInfo != L7_NULLPTR)
          {
...
L7_RC_t dot1xCtlApplyLogicalPortConfigData(L7_uint32 lIntIfNum)
{
  ...
  /* Initialize state machines */
  if ( (dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED || dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_AUTO) &&
      dot1xPortInfo[physPort].portEnabled == L7_TRUE &&
      dot1xCnfgrIsRestartTypeWarm() == L7_TRUE)
  {
    (void)dot1xStateMachineClassifier(apmWarmRestart, lIntIfNum, L7_NULL, L7_NULLPTR);
  }
  ...
  {
    (void)dot1xStateMachineClassifier(bamWarmRestart, lIntIfNum, L7_NULL, L7_NULLPTR);
  }
  else
...
L7_RC_t dot1xMacBasedVlanParticipationReset(dot1xLogicalPortInfo_t *logicalPortInfo, L7_uint32 vlanId)
{
  ...
  if(delParticiaption == L7_TRUE)
  {
    if( dot1qStaticVlanCheckValid(vlanId) != L7_SUCCESS )
    {
       dot1qDynamicVlanMemberRevert(vlanId,phyPort);
    }
    else
    { 
...
L7_RC_t dot1xVlanPVIDChangeEventProcess(L7_uint32 intIfNum,L7_uint32 vlanId)
{
      ...
      while((logicalPortInfo=dot1xLogicalPortInfoGetNextNode(intIfNum,&lIntIfNum))!= L7_NULLPTR)
      {
            ...
            (logicalPortInfo->logicalPortStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED))
        {
          if (((logicalPortInfo->vlanId == vlanId) || (logicalPortInfo->vlanId == L7_MAX_VLAN_ID)) &&
              (logicalPortInfo->defaultVlanId == 0 && logicalPortInfo->isMonitorModeClient == L7_FALSE))
          {
            curr_pvid =0;
            ...
            if (curr_pvid != vlanId)
            {
              ...
              {
                if (dot1xIhPortStatusSet(logicalPortInfo->logicalPortNumber, 
                            L7_DOT1X_PORT_STATUS_UNAUTHORIZED) != L7_SUCCESS)
                { 
                   L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                                       "dot1xApmAuthenticatedAction: could not set state to unauthorized for applying changes during pvid change, logical intf %u\n",
                                         logicalPortInfo->logicalPortNumber);
                ...
                if (dot1xIhPortStatusSet(logicalPortInfo->logicalPortNumber,
                         L7_DOT1X_PORT_STATUS_AUTHORIZED) != L7_SUCCESS)
                {  
                  L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
                                    "dot1xApmAuthenticatedAction: could not set state to authorized for applying changes during pvid change, intf %u vlan:%u."
                                    " Could not reset the port to authorized state after applying PVID  change to the client details", logicalPortInfo->logicalPortNumber,curr_pvid);
                }
              }
            }/* curr_pvid != vlanId*/
    ...
    {
      logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
      /* If Monitor Mode is enabled, Update new default VLANID to the interface */
      if((logicalPortInfo != L7_NULLPTR)             &&
         (logicalPortInfo->guestVlanId == 0)         &&   
         (logicalPortInfo->unauthVlan == 0)          &&
         (logicalPortInfo->vlanAssigned == 0))
      {
        (void) dot1qQportsVIDGet(intIfNum, L7_DOT1Q_DEFAULT_VID_INDEX, &curr_pvid);
        rc = dot1xCtlPortMonitorVlanMode(intIfNum, curr_pvid);
      }
    }
    return rc;
}
...
L7_RC_t dot1xCtlPortUnauthenticatedVlanOperational(dot1xLogicalPortInfo_t *logicalPortInfo,L7_uint32 reason)
{
      ...
      curr_mode == DOT1Q_SWPORT_MODE_GENERAL)))
  {
    ...
    /* check if Monitor Mode is enabled on the port */
    if ((dot1xCfg->monitorMode == L7_ENABLE) && (logicalPortInfo->defaultVlanId != 0)
         && (logicalPortInfo->isMonitorModeClient == L7_TRUE))
    {
      logicalPortInfo->isMonitorModeClient = L7_FALSE;
    }
...
/*********************************************************************
* @purpose  Disable dot1x Monitor Mode vlan to a specified interface
*
* @param    intIfNum   @b{(input)) internal interface number
*
* @returns  L7_SUCCESS
...
* @end
*********************************************************************/
L7_RC_t dot1xCtlLogicalPortMonitorModeVlanReset(L7_uint32 lIntIfNum)
{
  L7_uint32   curr_vlanId;
  dot1xLogicalPortInfo_t *logicalPortInfo;
  L7_RC_t rc = L7_FAILURE;
  ...
  if (logicalPortInfo != L7_NULLPTR)
  {
    ...
    if (logicalPortInfo->inUse == L7_TRUE)
    {
      if (logicalPortInfo->defaultVlanId !=0)
      {
        DOT1X_EVENT_TRACE(DOT1X_TRACE_EXTERNAL, physPort,
                      "%s Resetting Monitor Mode vlanid - %d for int- %d  \n",
                          __FUNCTION__, logicalPortInfo->defaultVlanId, lIntIfNum);

        curr_vlanId = logicalPortInfo->defaultVlanId;
        logicalPortInfo->defaultVlanId = 0;
...
L7_RC_t dot1xCtlPortUnauthAddrCallbackProcess(L7_uint32 intIfNum,L7_enetMacAddr_t macAddr,L7_ushort16 vlanId)
{
  ...
  if (exists == L7_FALSE)
  {
    ...
    dtlDot1xIntfClientBlock(intIfNum,macAddr,vlanId);

    logicalPortInfo->vlanId = vlanId;
    logicalPortInfo->blockVlanId = vlanId;
...
L7_RC_t dot1xCtlLogicalPortVlanAssignmentDisable(dot1xLogicalPortInfo_t *logicalPortInfo)
{
  ...
  {
    if (dot1xPortInfo[logicalPortInfo->physPort].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
    {
      /* Need to delete this port from vlan participation */
      dot1xMacBasedVlanParticipationReset(logicalPortInfo, logicalPortInfo->defaultVlanId);
    }
    else if (dot1xPortInfo[logicalPortInfo->physPort].portControlMode == L7_DOT1X_PORT_AUTO &&
             dot1xPortInfo[logicalPortInfo->physPort].dot1xAcquire == L7_TRUE)
    {
      dot1xPortVlanMembershipSet(logicalPortInfo->physPort, logicalPortInfo->defaultVlanId,L7_FALSE);
      dot1xCtlPortDot1xRelease(logicalPortInfo->physPort, logicalPortInfo->defaultVlanId);
    }
    logicalPortInfo->defaultVlanId = 0;
  } 
  else if (logicalPortInfo->vlanId != 0)
  ...
  case EAPOL_EAPPKT:
    ...
    case EAP_REQUEST:
      ...
      case EAP_RRIDENTITY:
        ...
        {
           L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                   "Supplicant: EAP_REQUEST is received on port --%s--%s--%d with state %d\r\n",
                      ifName,__FUNCTION__,__LINE__,dot1xSupplicantPortInfo[intIfNum].spmState);
           rc = L7_FAILURE;
        }
      break;
      case EAP_RRMD5:
    ...
    case EAP_RESPONSE:
      dot1xPortStats[intIfNum].suppInvalidEapolFramesRx++;
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
              "Supplicant: EAP_RESPONSE is received on port --%s--%s--%d\r\n",
...
L7_RC_t dot1xCtlLogicalPortMABRemove(L7_uint32 lIntIfNum)
{
  ...
  L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
          "Port-%u moved from authenticated state as MAB is not Operational on the port\n",
          lIntIfNum);
...
L7_RC_t dot1xCtlLogicalPortMABGenResp(L7_uint32 lIntIfNum)
{
  ...
  userData = ((L7_uchar8 *)eapRrPkt) + sizeof(L7_eapRrPacket_t);
   ...
   {
     L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
             "dot1xCtlLogicalPortMABGenResp: couldn't allocate responseData\n");
   }
...
/*********************************************************************
* @purpose  Set dot1x Monitor mode to a specified interface
*
* @param    lIntIfNum @b{(input))  Logical Interface Number
*
* @returns  L7_SUCCESS
...
* @end
*********************************************************************/
L7_RC_t dot1xCtlPortMonitorModeOperational(L7_uint32 lIntIfNum)
{
  dot1xPortCfg_t         *pCfg;
  L7_uint32              phyIntf;
  L7_uint32              vlanId;
  ...
  phyIntf =  logicalPortInfo->physPort;
  ...
  (void) dot1qQportsVIDGet(phyIntf, L7_DOT1Q_DEFAULT_VID_INDEX, &vlanId);
  
  /* check if the default VLAN is being set as a result of a re-authentication*/
  if ((logicalPortInfo->defaultVlanId !=0) &&
      (logicalPortInfo->defaultVlanId != vlanId))
  {
    DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS, phyIntf,
                     "\n%s: Resetting Monitor Mode VLAN %u for port %u -  \n",
                      __FUNCTION__,logicalPortInfo->defaultVlanId,phyIntf);
    logicalPortInfo->reauth_auth_apply = L7_TRUE;
  ...
  {
    DOT1X_EVENT_TRACE(DOT1X_TRACE_API_CALLS, phyIntf,
                      "\n%s Setting default vlanId:%d for interface:%d  \n",
                      __FUNCTION__,vlanId,lIntIfNum);
    ...
    {
      dot1xCtlLogicalPortVlanAssignedReset(lIntIfNum);
      /*port moving from Radius assigned vlan to Monitor Mode vlan*/
      if (dot1xPortInfo[phyIntf].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
      {
        logicalPortInfo->reauth_auth_apply = L7_TRUE;
      }
...
/*********************************************************************
* @purpose  Configure administrative Monitor mode setting for dot1x
*
* @param    monitorMode (Enable or Disable)
*
* @returns  L7_SUCCESS
* @returns  L7_FAILURE
*
...
* @end
*********************************************************************/
L7_RC_t dot1xCtlAdminMonitorModeSet(L7_uint32 monitorMode)
{
  L7_RC_t                rc = L7_FAILURE, nimRc;
  L7_uint32              intIfNum, lIntIfNum, linkState, adminState;
  dot1xPortCfg_t         *pCfg;
  dot1xLogicalPortInfo_t *logicalPortInfo=L7_NULLPTR; 

  /* Have to remove it later .. Need to observe when this condition arises ?? */
  if (DOT1X_IS_READY == L7_FALSE)
  {
    return L7_SUCCESS;
  }
  ...
  switch(monitorMode)
  {
    case L7_ENABLE:
      if(dot1xCfg->dot1xAdminMode == L7_ENABLE)
      {
        /* Initialize the unauthenticated ports */
            ...
            if(pCfg->paeCapabilities == L7_DOT1X_PAE_PORT_AUTH_CAPABLE)
            {
              if(dot1xPortInfo[intIfNum].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
              {
                ...
                {   
                  ...
                  if(logicalPortInfo != L7_NULLPTR)
                  {
                    if(logicalPortInfo->apmState != APM_FORCE_UNAUTH &&
                       logicalPortInfo->logicalPortStatus == L7_DOT1X_PORT_STATUS_UNAUTHORIZED)
                    {
                      /* reset clients associated with the port */
#if 0
                      sysapiPrintf("%s(%d): Resetting logical Port:%d on Port %d\n\r", __FUNCTION__, __LINE__,lIntIfNum,intIfNum);
                      dot1xPortInfoInitialize(intIfNum,L7_FALSE);
                      dot1xCtlLogicalPortInitializeSet(lIntIfNum,L7_FALSE);
#endif
                    } 
              ...
              {
                logicalPortInfo = dot1xPhysicalPortInfoNodeGet(intIfNum);
                if(logicalPortInfo != L7_NULLPTR && logicalPortInfo->apmState != APM_FORCE_UNAUTH)
                {
                  (void)dot1xPortInfoInitialize(intIfNum,L7_TRUE);
                  (void)dot1xCtlApplyPortConfigData(intIfNum);
                  if(pCfg->guestVlanId != 0)
    ...
    case L7_DISABLE:
      rc = dot1xCtlLogicalMonitorModeRemove();
      break;
...
/*********************************************************************
 * * @purpose  Remove all logical interfaces that has been authenticated
 ...
 * * @end
 * *********************************************************************/
L7_RC_t dot1xCtlLogicalMonitorModeRemove()
{
  dot1xLogicalPortInfo_t *logicalPortInfo = L7_NULLPTR;
  dot1xPortCfg_t    *pCfg = L7_NULLPTR;
  L7_uint32 lIntIfNum, intIfNum;

  if( dot1xFirstValidIntfNumber(&intIfNum) != L7_SUCCESS)
  {
    return L7_FAILURE;
  }
  ...
  do
  {
      ...
      while(logicalPortInfo != L7_NULLPTR)
      {
        ...
        if(logicalPortInfo->defaultVlanId != 0 || logicalPortInfo->isMonitorModeClient == L7_TRUE) 
        {
          /*remove from Monitor Mode assigned VLAN*/
          dot1xCtlLogicalPortVlanAssignmentDisable(logicalPortInfo);
          ...
          {
            logicalPortInfo->guestVlanTimer = 0;
            dot1xStateMachineClassifier(apmMonitorModeDisable,lIntIfNum, L7_NULL, L7_NULLPTR);
          }
#if 0
          /* Generate the events to reset the state machines */
          dot1xStateMachineClassifier(bamInitialize, lIntIfNum, L7_NULL, L7_NULLPTR);
#endif
          dot1xStateMachineClassifier(rtmInitialize,lIntIfNum, L7_NULL,L7_NULLPTR);
...
/*********************************************************************
* @purpose  Parse the dot1x event and Log into the History Log table
*
* @param    intIfNum        @b{(input)) internal interface number
*           vlanId          @b{(input)) VlanId
*           reasonCode      @b{(input)) Reason Code
*           accessStatus    @b{(input)) Actual state of the dot1x
*           logicalPortInfo @b{(input)) Reference to the dot1x port info
*
* @returns  void
*
* @comments
...
* @end
*********************************************************************/
void dot1xAuthHistoryInfoLog(L7_uint32 intIfNum,
                             L7_uint32 vlanId,
                             L7_uint32 reasonCode,
                             L7_uint32 accessStatus,  
                             dot1xLogicalPortInfo_t *logicalPortInfo)
{
   dot1xAuthHistoryLogInfo_t entry;
   L7_uint32     ifIndex;
 
   memset(&entry, 0, sizeof(entry));

   entry.timeStamp = simAdjustedTimeGet();
   entry.accessStatus = accessStatus;
   entry.reasonCode = reasonCode;
   
   if(logicalPortInfo != L7_NULLPTR)
   {
     ifIndex = logicalPortInfo->physPort;
     memcpy(&entry.suppMacAddr, &logicalPortInfo->suppMacAddr, sizeof(L7_enetMacAddr_t));
     memcpy(entry.filterName, logicalPortInfo->filterName, strlen(logicalPortInfo->filterName));
     entry.authStatus = logicalPortInfo->logicalPortStatus;
     entry.vlanId = vlanId;
     if(logicalPortInfo->vlanAssigned != 0)
     { 
       entry.vlanAssignedType = L7_DOT1X_RADIUS_ASSIGNED_VLAN;
       entry.vlanIdAssigned = logicalPortInfo->vlanAssigned;
     }
     else if(logicalPortInfo->unauthVlan != 0)
     {
       entry.vlanAssignedType = L7_DOT1X_UNAUTHENTICATED_VLAN;
       entry.vlanIdAssigned = logicalPortInfo->unauthVlan;
     }
     else if(logicalPortInfo->guestVlanId != 0)
     {
       entry.vlanAssignedType = L7_DOT1X_GUEST_VLAN;
       entry.vlanIdAssigned = logicalPortInfo->guestVlanId;
     }
     else if(logicalPortInfo->voiceVlanId != 0)
     {
       entry.vlanAssignedType = L7_DOT1X_VOICE_VLAN;
       entry.vlanIdAssigned = logicalPortInfo->voiceVlanId;
     }
     else if((logicalPortInfo->defaultVlanId != 0) && (logicalPortInfo->isMonitorModeClient == L7_TRUE))
     {
       entry.vlanAssignedType = L7_DOT1X_MONITOR_MODE_VLAN;   
       entry.vlanIdAssigned = logicalPortInfo->defaultVlanId;
     }
     else if(logicalPortInfo->vlanId != 0)
     {
       entry.vlanAssignedType = L7_DOT1X_DEFAULT_ASSIGNED_VLAN;
       entry.vlanIdAssigned = logicalPortInfo->vlanId;
     }
     else
     {
       entry.vlanAssignedType = L7_DOT1X_NOT_ASSIGNED;
       entry.vlanIdAssigned = 0;
     }

     if(logicalPortInfo->isMABClient == L7_TRUE)
     {
       entry.isMABClient = L7_TRUE;  
     }
   }
   else
   {
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID,"Unable to log entry into History table!\n\r");
     return;
   }
 
   if(dot1xAuthHistoryLogEntryAdd(ifIndex, &entry) != L7_SUCCESS)
   {
     L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DOT1X_COMPONENT_ID,
            "Failed to Insert Dot1x Log Entry in the History Log table on Interface:[%d]\n\r",ifIndex);    
     return;
   }
}
