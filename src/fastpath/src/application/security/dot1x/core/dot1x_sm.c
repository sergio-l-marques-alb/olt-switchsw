...
/*E46*/{APM_AUTHENTICATED, APM_STATES,      APM_STATES,         APM_STATES,        APM_STATES,        APM_STATES,       APM_STATES,       APM_STATES,       APM_STATES       },
/*E47*/{APM_STATES,       APM_STATES,       APM_AUTHENTICATED,  APM_STATES,        APM_STATES,        APM_STATES,       APM_AUTHENTICATED,APM_STATES,       APM_STATES       },
/*E48*/{APM_STATES,       APM_STATES,       APM_STATES,         APM_STATES,        APM_DISCONNECTED,  APM_STATES,       APM_STATES,       APM_STATES,       APM_STATES       }
};
...
/*E46*/{BAM_STATES,     BAM_STATES,     BAM_STATES,     BAM_STATES,     BAM_STATES,     BAM_STATES,      BAM_IDLE      },
/*E47*/{BAM_SUCCESS,    BAM_SUCCESS,    BAM_STATES,     BAM_STATES,     BAM_STATES,     BAM_SUCCESS,     BAM_STATES    }
};
    ...
    else
    {
      ...
      if (dot1xIhPortStatusSet(lIntIfNum, L7_DOT1X_PORT_STATUS_UNAUTHORIZED) != L7_SUCCESS)
        L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
            "dot1xApmMachine: could not set state to unauthorized, logical intf %u."
            " DTL call failed setting authorization state of the port or client (logical port)."
               , lIntIfNum);
      /* Cleanup the H/w entry */
      if (logicalPortInfo->blockVlanId != 0 && dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
      {
...
L7_RC_t dot1xApmDisconnectedAction(dot1xLogicalPortInfo_t *logicalPortInfo)
{
       ...
       (logicalPortInfo->eapLogoff == L7_TRUE) )
  {
    ...
    if (dot1xRadiusAccountingStop(lIntIfNum, RADIUS_ACCT_TERM_CAUSE_USER_REQUEST) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
          "dot1xApmDisconnectedAction: failed sending terminate cause, logical intf %u."
  ...
  if (logicalPortInfo->reAuthenticating == L7_TRUE)
  {
      if (dot1xRadiusAccountingStop(lIntIfNum, RADIUS_ACCT_TERM_CAUSE_REAUTHENTICATION_FAILURE) != L7_SUCCESS)
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                  "dot1xApmHeldAction reAuthenticating: failed sending terminate cause, logical intf %u\n", lIntIfNum);
  ...
  if (logicalPortInfo->logicalPortStatus != L7_DOT1X_PORT_STATUS_UNAUTHORIZED)
  {
    if (dot1xIhPortStatusSet(lIntIfNum, L7_DOT1X_PORT_STATUS_UNAUTHORIZED) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
          "dot1xApmDisconnectedAction: could not set state to unauthorized, logical intf %u."
          " DTL call failed setting authorization state of the port or client (logical port)."
              , lIntIfNum);
  }
...
L7_RC_t dot1xApmConnectingAction(dot1xLogicalPortInfo_t *logicalPortInfo)
{
  ...
  DOT1X_EVENT_TRACE(DOT1X_TRACE_FSM_EVENTS,physPort,"%s:Logical port-%d logicalPortInfo->apmState:[%d]\n",
                      __FUNCTION__,lIntIfNum,logicalPortInfo->apmState);
...
L7_RC_t dot1xApmAuthenticatedAction(dot1xLogicalPortInfo_t *logicalPortInfo)
{
  ...
  if (logicalPortInfo->reauth_auth_apply == L7_TRUE)
  {
    ...
    {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                  "dot1xApmAuthenticatedAction: could not set state to unauthorized for applying changes during reauthentication, logical port %u\n",
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
        "dot1xApmAuthenticatedAction: could not set state to authorized, logical port %u."
        " DTL call failed setting authorization state of the port or client (logical port).",
            logicalPortInfo->logicalPortNumber);
  }

    if (dot1xRadiusAccountingStart(logicalPortInfo->logicalPortNumber) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
              "error calling dot1xRadiusAccountingStart, lIntIfNum=%u"
              " Failed sending accounting start to RADIUS server", logicalPortInfo->logicalPortNumber);
  ...
  if (dot1xPortInfo[physPort].portControlMode == L7_DOT1X_PORT_AUTO_MAC_BASED)
  {
    ...
    if ((logicalPortInfo->sessionTimeout == 0) &&
        ((dot1xPortInfo[physPort].reAuthEnabled == L7_FALSE) ||
         ...
         ((dot1xMonitorModeGet(&monitorMode) == L7_SUCCESS) && 
          (monitorMode == L7_ENABLE) && (logicalPortInfo->defaultVlanId != 0))))
    {
  ...
  if (logicalPortInfo->logicalPortStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED)
  {
    if (logicalPortInfo->suppRestarting == L7_TRUE)
    {
      if (dot1xRadiusAccountingStop(lIntIfNum,RADIUS_ACCT_TERM_CAUSE_SUPPLICANT_RESTART) != L7_SUCCESS)
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                "dot1xApmHeldAction suppRestarting: failed sending terminate cause, logical intf %u\n",lIntIfNum);
      logicalPortInfo->suppRestarting = L7_FALSE;
    }
    else
    {
    ...
    if (dot1xIhPortStatusSet(lIntIfNum, L7_DOT1X_PORT_STATUS_UNAUTHORIZED) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
          "dot1xApmHeldAction: could not set state to unauthorized, logical intf %u."
          " DTL call failed setting authorization state of the port or client (logical port)."
          , lIntIfNum);
  }
...
L7_RC_t dot1xApmForceAuthAction(dot1xLogicalPortInfo_t *logicalPortInfo)
{
  ...
  if (dot1xIhPortStatusSet(lIntIfNum, L7_DOT1X_PORT_STATUS_AUTHORIZED) != L7_SUCCESS)
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
        "dot1xApmForceAuthAction: could not set state to authorized, logical intf %u."
        " DTL call failed setting authorization state of the port or client (logical port)."
            , lIntIfNum);
...
L7_RC_t dot1xApmForceUnauthAction(dot1xLogicalPortInfo_t *logicalPortInfo)
{
  ...
  if (logicalPortInfo->logicalPortStatus == L7_DOT1X_PORT_STATUS_AUTHORIZED)
  {
    ...
    if (dot1xRadiusAccountingStop(lIntIfNum, RADIUS_ACCT_TERM_CAUSE_ADMIN_RESET) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
          "dot1xApmForceUnauthAction: failed sending terminate cause, logical intf %u."
          " Failed sending accounting stop to RADIUS server", lIntIfNum);
  }
...
L7_RC_t dot1xBamRequestAction(dot1xLogicalPortInfo_t *logicalPortInfo, L7_netBufHandle bufHandle)
{
  ...
  {
    /* Need to retransmit */
    SYSAPI_NET_MBUF_GET(reXmitBufHandle);
  ...
  if (logicalPortInfo->isMABClient == L7_TRUE)
  {
    /* Free bufHandle as another one will be allocated in dot1xCtlLogicalPortMABGenResp() */
    SYSAPI_NET_MBUF_FREE(bufHandle);
...
L7_RC_t dot1xBamResponseAction(dot1xLogicalPortInfo_t *logicalPortInfo, L7_netBufHandle bufHandle)
{
  ...
  if (dot1xSmAuthMethodGet(logicalPortInfo, bufHandle, &authMethod) == L7_SUCCESS)
  {
    ...
    case L7_AUTH_METHOD_REJECT:
      SYSAPI_NET_MBUF_FREE(bufHandle);
      /* Log the message in History Auth table */
      reasonCode = (authMethod == L7_AUTH_METHOD_UNDEFINED) ? 
                   DOT1X_AUTH_REASON_AUTH_METHOD_UNDEFINED:DOT1X_AUTH_REASON_AUTH_METHOD_REJECT;
    ...
    default:
      SYSAPI_NET_MBUF_FREE(bufHandle);
      dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_AUTH_METHOD_INVALID, 
  ...
  else
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID,
        "dot1xBamResponseAction: failed getting auth method, logical intf %u."
        " Could not determine the authentication method to be used . "
        "Probably because of  a mis-configuration.", lIntIfNum);
    /* Log the message in History Auth table */
    dot1xAuthHistoryInfoLog(L7_NULL, L7_NULL, DOT1X_AUTH_REASON_AUTH_METHOD_NOT_CONFIGURED, 
                            L7_DOT1X_PORT_STATUS_UNAUTHORIZED, logicalPortInfo);
    /* reset port in any dot1x assigned vlans*/
    dot1xCtlLogicalPortVlanAssignmentDisable(logicalPortInfo);
    (void) dot1xStateMachineClassifier(bamAFail, lIntIfNum, L7_NULL, L7_NULLPTR);
...
void dot1xAbortAuth(dot1xLogicalPortInfo_t *logicalPortInf)
{
  ...
  logicalPortInf->dot1xUserIndex = DOT1X_USER_INDEX_INVALID;
                             ...
                             L7_USER_MGR_AUTH_METHOD_t *authMethod)
{
      ...
      ((logicalPortInf->isMABClient == L7_FALSE )&&(eapRrPkt!=L7_NULLPTR)&& (eapRrPkt->type == EAP_RRIDENTITY)))
  {
    ...
    for (i=0; i<L7_MAX_AUTH_METHODS; i++) /* check all methods; break when we find first valid */
    {
      ...
      case L7_AUTH_METHOD_IAS:
        /* attempt internal authentication */
     ...
     if (authIntf != intIfNum)
     {
        if (dot1xIntfIsConfigurable(authIntf, &pCfg) == L7_TRUE)
        {
          ...
          while((logicalPortInfo=dot1xLogicalPortInfoGetNextNode(authIntf,&lIntIfNum))!= L7_NULLPTR)
          {
            ...
            nimGetIntfName(authIntf, L7_SYSNAME, ifName);
    ...
    case DOT1X_SPM_DISCONNECTED:
      ...
      else
      {
        ...
        if (dot1xIhPhyPortStatusSet(intIfNum, L7_DOT1X_PORT_STATUS_UNAUTHORIZED,L7_TRUE)
                                                         != L7_SUCCESS)
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
                  "dot1xSpmMachine: could not set state to unauthorized, intf %s\n", ifName);
#endif
      }
