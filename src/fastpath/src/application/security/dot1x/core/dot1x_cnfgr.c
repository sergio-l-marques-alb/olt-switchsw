...
L7_BOOL dot1xSwitchoverInProgress = L7_FALSE;

L7_BOOL dot1xCnfgrIsRestartTypeWarm()
{
  return warmRestart;
}

void dot1xCnfgrWarmRestartTypeSet( L7_BOOL warmType)
{
  warmRestart = warmType;
}
 ...
 */
  ...
  else
  {
    if (dot1xIssueCmd(dot1xCnfgr, L7_NULL, pCmdData) != L7_SUCCESS)
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID, "dot1xApiCnfgrCommand: Failed calling dot1xIssueCmd. "
...
L7_RC_t dot1xInit(L7_CNFGR_CMD_DATA_t *pCmdData)
{
  ...
  if (dot1xQueue == L7_NULLPTR)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1xInit: msgQueue creation error.\n");
  ...
  if (dot1xIssueCmd(dot1xCnfgr, L7_NULL, pCmdData) != L7_SUCCESS)
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_DOT1X_COMPONENT_ID, "dot1xInit: Failed calling dot1xIssueCmd. "
                                                            "802.1X message queue is full\n");
...
L7_uint32 dot1xMaxAllocMemGet()
{
  ...
  {
      max_node = L7_DOT1X_INTF_MAX_COUNT;
  }
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  dot1xPortInfo   = osapiMalloc(L7_DOT1X_COMPONENT_ID, sizeof(dot1xPortInfo_t) * L7_DOT1X_INTF_MAX_COUNT);
  ...
  memset((void *)dot1xPortInfo, 0, sizeof(dot1xPortInfo_t) * L7_DOT1X_INTF_MAX_COUNT);
  ...
  {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
              "Failed to initialize dot1x outcalls\n");
      return(L7_FAILURE);
  }
  ...
  {
    if ((rc = dot1xNsfInit()) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT1X_COMPONENT_ID,
            "Cannot init dot1x NSF \n");
  ...
  /* Initialize Dot1x Auth History Log Database */
  if (dot1xAuthHistoryLogDbInit() != L7_SUCCESS)
  {
    *pResponse = 0;
    *pReason   = L7_CNFGR_ERR_RC_LACK_OF_RESOURCES;
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_DOT1X_COMPONENT_ID,
               "dot1xAuthHistoryLogDbInit() failed\n\r");
    return L7_ERROR;
  }
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  if (nimRegisterIntfChange(L7_DOT1X_COMPONENT_ID, dot1xIntfChangeCallback,
                            dot1xIntfStartupCallback, NIM_STARTUP_PRIO_DOT1X) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1x: Unable to register with NIM\n");
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1x: Unable to register for RADIUS response callback\n");
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_DOT1X_COMPONENT_ID,
            "dot1x: Unable to register for Vlan callback\n");
  ...
  if (cnfgrIsFeaturePresent(L7_FLEX_STACKING_COMPONENT_ID,
                            L7_STACKING_NSF_FEATURE_ID) == L7_TRUE)
  {
    if (dot1xNsfCallbacksRegister() != L7_SUCCESS)
    {
      *pReason    = L7_CNFGR_ERR_RC_FATAL;
      dot1xRC     = L7_ERROR;
...
* @end
*********************************************************************/
L7_RC_t dot1xCnfgrInitPhase3Process( L7_BOOL warmRestart,
                                     ...
                                     L7_CNFGR_ERR_RC_t   *pReason )
{
  ...
  *pReason    = 0;
  ...
  {
    dot1xSwitchoverInProgress = L7_TRUE;
  }
...
void dot1xCnfgrFiniPhase1Process()
{
  ...
  /* Dot1x Authentication History Log Db Un-initilization */
  dot1xAuthHistoryLogDbDeInit();
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                if ((dot1xRC = dot1xCnfgrInitPhase3Process(warmRestart, &response, &reason )) != L7_SUCCESS)
                {
