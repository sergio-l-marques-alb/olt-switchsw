...
L7_uint32 cpdmCPConfigWebIdAllDelete(cpId_t cpId)
{
  ...
  if (retval > 0)
  {
	...
	/* Something bad happened. Try to recover. */
        CP_DLOG(CPD_LEVEL_LOG, "CP id %d: Axed %d locales, but only had %d!?",
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "Removed %d locales for CP id %d, which doesn't exist!?",
	      retval, cpId);
    }
  }
  SEMA_GIVE(WRITE);
...
{ 
  ...
  {
    CP_DLOG(CPD_LEVEL_LOG, "%s: Invalid webId: %d", __FUNCTION__, webId);
    return rc;
  }
  ...
  do 
  {
    ...
    if (L7_NULLPTR == pCData)
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: Invalid cpId:%d, does not exist", __FUNCTION__, cpId);
      break;
    }

    if (pCData->localeCount > FD_CP_CUSTOM_LOCALE_MAX) 
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: Invalid webId:%d, reached maximum.", __FUNCTION__, webId);
      break;
    }
