  ...
  while(L7_SUCCESS == loop_rc) 
  {
    ...
    if (CP_AUTH_ON_LOCAL_SWITCH == swType)
    {
      if (!session_still_ok(&mac))
      {
        ...
        if (L7_SUCCESS != rc)
        {
          CP_DLOG(CPD_LEVEL_LOG, "Could not deauth client @ MAC "
        ...
        {
          CP_DLOG(CPD_LEVEL_LOG, "Deauth'ed client @ MAC "
                  "%02x:%02x:%02x:%02x:%02x:%02x\n",
                  mac.addr[0], mac.addr[1], mac.addr[2], 
                  mac.addr[3], mac.addr[4], mac.addr[5]);
        }
...
L7_BOOL session_still_ok(L7_enetMacAddr_t * mac)
{
  ...
  if (L7_SUCCESS != cpdmClientConnStatusIntfIdGet(mac, &intf))
  {
    return L7_FALSE;
  }
...
L7_BOOL session_time_check(L7_enetMacAddr_t * mac)
{
  ...
  do 
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x : can't get session time; killing it",
              __FUNCTION__,
              mac->addr[0], mac->addr[1], mac->addr[2], 
              mac->addr[3], mac->addr[4], mac->addr[5]);
      break;
    ...
    if (0 == sessionTO)
    {
      ...
      {
        CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x : no CP ID for this MAC",
                __FUNCTION__,
                mac->addr[0], mac->addr[1], mac->addr[2], 
                mac->addr[3], mac->addr[4], mac->addr[5]);
        break;
      ...
      {
        CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x : no CP instance session timeout",
                __FUNCTION__,
                mac->addr[0], mac->addr[1], mac->addr[2], 
                mac->addr[3], mac->addr[4], mac->addr[5]);
        break;
...
{ 
  ...
  if (0 == idleTO)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x : "
  ...
  do 
  {
    ...
    {
#if 0
      CP_DLOG(CPD_LEVEL_LOG, "%s: Could not get connection stats",__FUNCTION__);
#endif
      break;
    }
    ...
    if (L7_SUCCESS != cpdmClientConnStatusStatisticsUpdate(mac, &txRxNew, L7_FALSE))
    {
       ...
       */
#if 0
      CP_DLOG(CPD_LEVEL_LOG, "%s: Could not get last refresh data, let session age out", __FUNCTION__);
#endif
    }

    if (L7_SUCCESS != cpdmClientConnStatusIdleTimeGet(mac, &idleTime))
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x: can't get idle time",
              __FUNCTION__,
              mac->addr[0], mac->addr[1], mac->addr[2], 
              mac->addr[3], mac->addr[4], mac->addr[5]);
      break;
...
L7_BOOL user_logout_check(L7_enetMacAddr_t * mac)
{
  ...
  do 
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: %02x:%02x:%02x:%02x:%02x:%02x : can't get session user logout flag; killing it",
              __FUNCTION__,
              mac->addr[0], mac->addr[1], mac->addr[2], 
              mac->addr[3], mac->addr[4], mac->addr[5]);
      break;
...
L7_BOOL cpdmIsClientWired(L7_uint32 intf)
{
  ...
  if ((nimGetIntfType(intf, &sysIntfType) == L7_SUCCESS) &&
