...
extern avlTree_t      aipStatusTree;
...
avlTree_t         clientConnStatusTree;
...
L7_RC_t cpdmActStatusOperEnabledCount(L7_uint32 * pCount)
{
  ...
  *pCount = 0;
  while (L7_SUCCESS==cpdmCPConfigNextGet(cpId,&cpId))
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "Failed to get operational status for cpId:%d",cpId);
      return L7_FAILURE;
    }
    if (L7_CP_INST_OPER_STATUS_ENABLED == status)
    {
      ...
      {
        CP_DLOG(CPD_LEVEL_LOG, "Failed to get IntIfNum for cpId:%d",cpId);
        return L7_FAILURE;
      }
      if (cpId == next_cpId)
      {
        if (L7_SUCCESS != cpdmIntfStatusGetIsEnabled(intIfNum, &enabled))
        {
          ...
          CP_DLOG(CPD_LEVEL_LOG, "Failed to get operational status for intIfNum:%d, %s",intIfNum,ifName);
          return L7_FAILURE;
        }
        if (enabled == L7_TRUE)
        {
...
L7_RC_t cpdmCPConfigOperStatusSet(cpId_t cpId, L7_CP_INST_OPER_STATUS_t status)
{
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmCPConfigOperStatusSet: Failed to set instance status send flag.\n");
...
L7_RC_t cpdmCPConfigBlockedStatusSet(cpId_t cpId, L7_CP_INST_BLOCK_STATUS_t blocked)
{
  ...
  do {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmCPConfigBlockedStatusSet: Failed to set instance block status send flag.\n");
      cpdmOprData->cpLocalSw.cpId = 0;
    }
...
L7_RC_t cpdmCPConfigAuthenticatedUsersSet(cpId_t cpId, L7_uint32 authUsers)
{
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
  {
    ...
    if (cpClusterFlagSet(CP_CLUSTER_INST_STATUS_SEND) != L7_SUCCESS)
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmCPConfigAuthenticatedUsersSet: Failed to set instance status send flag.\n");
      cpdmOprData->cpLocalSw.cpId = 0;
    }
  }
...
L7_RC_t cpdmIntfStatusSetIsEnabled(L7_uint32 intfId, L7_BOOL isEnabled)
{
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmIntfStatusSetIsEnabled: Failed to set instance intf status send flag.\n");
      cpdmOprData->cpLocalSw.cpId = 0;
...
L7_RC_t cpdmIntfStatusSetUsers(L7_uint32 intfId, L7_uint32  users)
{
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
  {
    ...
    if (cpClusterFlagSet(CP_CLUSTER_INST_INTF_STATUS_SEND) != L7_SUCCESS)
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmIntfStatusSetUsers: Failed to set instance intf status send flag.\n");
      cpdmOprData->cpLocalSw.cpId = 0;
      cpdmOprData->cpLocalSw.intf = 0;
    }
  }
                        ...
                        L7_IP_ADDR_t ipAddr)
{
  ...
  {
    CP_DLOG(CPD_LEVEL_LOG, "no CP ID associated with intf %d, %s?\n", intIfNum, ifName);
    return L7_FAILURE;
  }
  ...
  {
    CP_DLOG(CPD_LEVEL_LOG, "client connection: can't get stats for intf %d, %s\n",
      intIfNum, ifName);
  ...
  do
  {
    ...
    if (L7_NULLPTR != pEntry) /* if item not inserted */
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: client not added to status table", __FUNCTION__);
      break;                  /* ..then die           */
    }
    else
    {
      ...
      if (cpdmIsClientWired(intIfNum) == L7_TRUE)
      {
        #ifdef L7_NSF_PACKAGE
        ...
        {
          cpdmCkptData_t data;
          cpcmAuthInProgressStatusData_t *dataEntry = L7_NULLPTR;

          memset(&data, 0x00, sizeof(cpdmCkptData_t));
          memcpy(&data.macAddr, mac, sizeof(L7_enetMacAddr_t));
          data.ip = ipAddr;
          data.intf = intIfNum;
          osapiStrncpy(data.uName, userName, CP_USER_LOCAL_USERNAME_MAX);
          data.uName[CP_USER_LOCAL_USERNAME_MAX] = '\0';
          if ((dataEntry=avlSearchLVL7(&aipStatusTree, &ipAddr, AVL_EXACT)) != L7_NULLPTR)
          {
            data.sessionTimeout = dataEntry->limits.sessionTimeout;
          }
          if (cpdmOprData->cpWarmRestart != L7_TRUE)
          {
            data.stackUpTimeAtClientConn = simSystemUpTimeGet();
          }
          data.flag = L7_TRUE;
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: client not added to CAC status table", __FUNCTION__);
      break;                    /* ..then die           */
    }
    else
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: client not added to intf/conn table", __FUNCTION__);
      break;                     /* ..then die           */
    }
    else
...

    {
      ...
      if (0 < cpId)
      {
        ...
        if (L7_NULLPTR != pActStat)
        {
          ...
          if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
          {
            ...
            if (cpClusterFlagSet(CP_CLUSTER_INST_STATUS_SEND) != L7_SUCCESS)
            {
              CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusAdd: Failed to set instance status send flag.\n");
              cpdmOprData->cpLocalSw.cpId = 0;
            }
          }
        }
      ...
      if (L7_NULLPTR != pIntfStat)
      {
        ...
        if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
        {
          ...
          if (cpClusterFlagSet(CP_CLUSTER_INST_INTF_STATUS_SEND) != L7_SUCCESS)
          {
            CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusAdd: Failed to set instance intf status send flag.\n");
            cpdmOprData->cpLocalSw.cpId = 0;
            cpdmOprData->cpLocalSw.intf = 0;
          }
        }
                                              ...
                                              cpId_t cpId, L7_BOOL all)
{
  ...
  if (all == L7_TRUE)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusPeerDeleteFlagSet: Failed to set deauth all clients send flag.\n");
      rc = L7_FAILURE;
    }
  }
  else if ((cpId >= CP_ID_MIN) && (cpId <= CP_ID_MAX))
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusPeerDeleteFlagSet: Failed to set deauth instance clients send flag.\n");
      cpdmOprData->cpLocalSw.cpId = 0;
      rc = L7_FAILURE;
    }
  }
                                       ...
                                       L7_enetMacAddr_t swMacAddr)
{
  if (clusterSupportGet() != L7_SUCCESS)
  {
        CP_DLOG(CPD_LEVEL_LOG, "%s: NOT SUPPORTED", __FUNCTION__);
        return L7_NOT_SUPPORTED;
  }

  if (!clientConnStatusTreeReady)
  {
        CP_DLOG(CPD_LEVEL_LOG, "%s: FAIL", __FUNCTION__);
        return L7_FAILURE;
  }
...
L7_RC_t cpdmClientConnStatusDelete(L7_enetMacAddr_t * mac)
{
  ...
  if (clusterSupportGet() == L7_SUCCESS)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: Failed to get client switch type", __FUNCTION__);
      return L7_FAILURE;
    }
...
{
  {
     ...
     if (L7_NULLPTR == pEntry)
     {
        CP_DLOG(CPD_LEVEL_LOG, "%s: entry not found?", __FUNCTION__);
        return;
     }
  ...
  }
  {
     ...
     if (att != del)
     {
        CP_DLOG(CPD_LEVEL_LOG, "%s: %d of %d items removed from CAC status table",
  ...
  }
  {
     ...
     if (att != del)
     {
        CP_DLOG(CPD_LEVEL_LOG, "%s: %d of %d items removed from intf conn table",
                __FUNCTION__, del, att);
     }
  }
}
...
L7_RC_t clientConnStatusDeleteHelper(L7_enetMacAddr_t * mac, L7_BOOL doDeauth)
{
  ...
  do
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG,
              "%s: MAC address %02x:%02x:%02x:%02x:%02x:%02x not found",
    ...
    if (cpdmIsClientWired(intIfNum) == L7_TRUE)
    {
      ...
      if (cnfgrIsFeaturePresent(L7_FLEX_STACKING_COMPONENT_ID,
                                L7_STACKING_NSF_FEATURE_ID) == L7_TRUE)
      {
        cpdmCkptData_t *ptrData;

        if ((ptrData=avlSearchLVL7(&cpdmOprData->cpCkptTree,
                                   mac, AVL_EXACT)) != L7_NULLPTR)
        {
          ptrData->flag = L7_FALSE;
          if (osapiSemaTake(cpdmOprData->cpCkptPendingSema, L7_WAIT_FOREVER) == L7_SUCCESS)
          {
            cpdmOprData->cpCkptReqd = L7_TRUE;
    ...
    if (0 < cpId)
    {
      ...
      if ((L7_NULLPTR != pActStat) && (0 < pActStat->users))
      {
        ...
        if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
        {
          ...
          if (cpClusterFlagSet(CP_CLUSTER_INST_STATUS_SEND) != L7_SUCCESS)
          {
            CP_DLOG(CPD_LEVEL_LOG, "clientConnStatusDeleteHelper: Failed to set instance status send flag.\n");
            cpdmOprData->cpLocalSw.cpId = 0;
          }
        }
    ...
    if ((L7_NULLPTR != pIntfStat) && (0 < pIntfStat->users))
    {
      ...
      if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_MEMBER)
      {
        ...
        if (cpClusterFlagSet(CP_CLUSTER_INST_INTF_STATUS_SEND) != L7_SUCCESS)
        {
          CP_DLOG(CPD_LEVEL_LOG, "clientConnStatusDeleteHelper: Failed to set instance intf status send flag.\n");
          cpdmOprData->cpLocalSw.cpId = 0;
          cpdmOprData->cpLocalSw.intf = 0;
        }
      }
  ...
  if ((L7_SUCCESS == rc) && (L7_TRUE == doDeauth))
  {
    ...
    if (L7_SUCCESS != rc)
    {
      CP_DLOG(CPD_LEVEL_LOG, "%s: Failed to enq client delete info", __FUNCTION__);
    }
  }
...
L7_RC_t cpdmClientConnStatusDeleteAll(void)
{
  ...
  while(L7_SUCCESS == cpdmClientConnStatusNextGet(&mac, &mac))
  {
    if (cpdmClientConnStatusWhichSwitchGet(&mac, &swType) != L7_SUCCESS)
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusDelete: Failed to get client switch type.\n");
      return L7_FAILURE;
    }
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusDeleteAll: Failed to set deauth all clients send flag.\n");
      return L7_FAILURE;
    }
  }
...
L7_RC_t cpdmClientConnStatusByCPIdDeleteAll(cpId_t cpId)
{
  ...
  if (cpdmOprData->cpLocalSw.status == CP_CLUSTER_SWITCH_CONTROLLER)
  {
    ...
    {
      CP_DLOG(CPD_LEVEL_LOG, "cpdmClientConnStatusByCPIdDeleteAll: Failed to set deauth CP instance clients send flag.\n");
      rc = L7_FAILURE;
    }
  }
                                           ...
                                           L7_uint32 * sessionTime)
{
  ...
  do
  {
    ...
    if (cpdmIsClientWired(pEntry->intfId) == L7_TRUE)
    {
      #ifdef L7_NSF_PACKAGE
      if (cnfgrIsFeaturePresent(L7_FLEX_STACKING_COMPONENT_ID,
                                L7_STACKING_NSF_FEATURE_ID) == L7_TRUE)
      {
        ...
        if ((ptrData=avlSearchLVL7(&cpdmOprData->cpCkptTree, macAddr, AVL_EXACT)) != L7_NULLPTR)
        {
          *sessionTime = simSystemUpTimeGet() - ptrData->stackUpTimeAtClientConn;
          SEMA_GIVE(READ);
          return L7_SUCCESS;
        }
      }
...
{
  cpdmClientConnStatusData_t *pEntry = L7_NULLPTR;
  cpdmIntfStatusData_t *pIntf = L7_NULLPTR;
  L7_RC_t rc = L7_FAILURE;
  ...
  do
  {
    ...
    if (pEntry == L7_NULLPTR)
      break;
    pEntry->intfId = newIntf;

    /* decrement user count on previous intf */
    pIntf = cpdmFindIntfStatus(prevIntf);
    if ((pIntf != L7_NULLPTR) && (pIntf->users > 0))
    {
      pIntf->users--;
    }
    /* increment user count on new intf */
    pIntf = cpdmFindIntfStatus(newIntf);
    if (pIntf != L7_NULLPTR)
    {
      pIntf->users++;
    }
                              ...
                              L7_STACKING_NSF_FEATURE_ID) == L7_TRUE)
    {
      ...
      if ((ptrData=avlSearchLVL7(&cpdmOprData->cpCkptTree,
                                 macAddr, AVL_EXACT)) != L7_NULLPTR)
      {
        ptrData->flag = L7_TRUE; /* does not hurt specifying again even if already L7_TRUE */
