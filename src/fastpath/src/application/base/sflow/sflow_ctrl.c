...
static void    sFlowStartupNotifyProcess(NIM_STARTUP_PHASE_t startup_phase);
/* Timer Process event */
...
PORTEVENT_MASK_t   sFlowEventMask_g;
/*********************************************************************
  ...
  do
  {
                            ...
                            != L7_SUCCESS)
    {
                            ...
                            != L7_SUCCESS)
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                "sFlowTask(): Internal event msg receive failed");
      }
      else
      {
        msgId = pduMsg.msgId;
    ...
    switch (msgId)
    {
      ...
      /* Process interface change event notfied by NIM */
        ...
        break;
      case (SFLOW_NIM_STARTUP_EVENT):
      /* Process startup phase message from NIM */
        sFlowStartupNotifyProcess(eventMsg.u.nimStartupPhase);
        break;
      ...
      default:
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                "sFlowTask(): invalid message type:%d. %s:%d\n",
                                ...
                                NIM_CORRELATOR_t correlator)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowIntfChangeCallback: Received an interface change callback while not ready to receive it");
    rc = L7_SUCCESS;
  ...
  if (skip == L7_FALSE)
  {
                           ...
                           L7_MSG_PRIORITY_NORM) == L7_SUCCESS)
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                "sFlowIntfChangeCallback: Failed to give msgQueue semaphore\n");
      }
    }
  }
  else
...
void sFlowNimStartupNotifyCallback(NIM_STARTUP_PHASE_t startup_phase)
{
  ...
  msg.msgId = SFLOW_NIM_STARTUP_EVENT;
  msg.u.nimStartupPhase = startup_phase;

  if (osapiMessageSend(agent.sFlowEventQueue, &msg, SFLOW_MSG_SIZE,
...
void sflowTimerExpiryHdlr(L7_APP_TMR_CTRL_BLK_t timerCtrlBlk, void* ptrData)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sflowTimerExpiryHdlr(): Sflow timer tick send failed\n");
  }
  else
  {
    if (osapiSemaGive(agent.sFlowMsgQSema) != L7_SUCCESS)
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sflowTimerExpiryHdlr: MsgQSema give failed");
    }
  }
}
/*********************************************************************
                                   ...
                                   NIM_CORRELATOR_t correlator)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowIntfChangeProcess: Received an interface change callback while not ready to receive it");
    rc = L7_FAILURE;
    status.response.rc  = rc;
    nimEventStatusCallback(status);
    return;
...
* @end
*********************************************************************/
L7_BOOL sFlowIntfIsAttached(L7_uint32 intIfNum)
{
...
void sFlowStartupNotifyProcess(NIM_STARTUP_PHASE_t startup_phase)
{
  L7_RC_t rc;
  L7_uint32 intIfNum;

  if (startup_phase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    ...
    {
      if ((sFlowIsValidIntf(intIfNum) == L7_TRUE) &&
          (sFlowIntfIsAttached(intIfNum) == L7_TRUE) &&
          (dot3adIsLagActiveMember(intIfNum) == L7_FALSE))
      {
        (void)sFlowIntfAttach(intIfNum);
      }
  ...
  switch (startup_phase)
  {
    case NIM_INTERFACE_CREATE_STARTUP:
      ...
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_DELETE);
      nimRegisterIntfEvents(L7_SFLOW_COMPONENT_ID, sFlowEventMask_g);
      break;

    case NIM_INTERFACE_ACTIVATE_STARTUP:
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_ATTACH);
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_DETACH);
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_LAG_ACQUIRE);
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_LAG_RELEASE);
      PORTEVENT_SETMASKBIT(sFlowEventMask_g, L7_PORT_STATS_RESET);
      nimRegisterIntfEvents(L7_SFLOW_COMPONENT_ID, sFlowEventMask_g);
      break;
  ...
  nimStartupEventDone(L7_SFLOW_COMPONENT_ID);
}
/*********************************************************************
...
static L7_RC_t sFlowIntfAttach(L7_uint32 intIfNum)
{
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowSamplerOperModeSet(pSampler, L7_ENABLE) != L7_SUCCESS)
    {
       ...
       L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SFLOW_COMPONENT_ID,
          "sFlowIntfAttach: could not enable the sampler object for interface %s."
  ...
  if ((pPoller = sFlowPollerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowPollerOperModeSet(pPoller, L7_ENABLE) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_SFLOW_COMPONENT_ID,
          "sFlowIntfAttach: could not enable the poller object for interface %s."
          " The configuration of new stack member ports added failed.",
              ifName);
    }
  }
...
static L7_RC_t sFlowIntfDetach(L7_uint32 intIfNum)
{
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowSamplerOperModeSet(pSampler, L7_DISABLE) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowIntfDetach: could not disable the sampler object for interface %s",
              ifName);
    }
  }
  ...
  if ((pPoller = sFlowPollerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowPollerOperModeSet(pPoller, L7_DISABLE) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowIntfDetach: could not disable the poller object for interface %s",
              ifName);
    }
  }
  return L7_SUCCESS;
...
static L7_RC_t sFlowIntfDelete(L7_uint32 intIfNum)
{
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowSamplerDelete(pSampler) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowIntfDelete: could not delete the sampler object for interface %s",
              ifName);
    }
  }
  ...
  if ((pPoller = sFlowPollerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    if (sFlowPollerDelete(pPoller) != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowIntfDelete: could not delete the poller object for interface %s",
              ifName);
    }
    else
    {
...
*/
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowReceiverAddressApply: Failed to set one or more sampler objects");
    return L7_FAILURE;
  }
  if (sFlowPollerOperModeSet(L7_NULLPTR, mode) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowReceiverAddressApply: Failed to set one or more poller objects");
    return L7_FAILURE;
  }
  return L7_SUCCESS;
}
                                ...
                                L7_uint32 pollInterval)
{
  ...
  if ((pPoller = sFlowPollerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowPollerIntervalApply: Failed to enable poller for dsIndex %u instance %u",
              dsIndex, instance);
      return L7_FAILURE;
    }
  }
                              ...
                              L7_uint32 samplingRate)
{
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    ...
    {
      L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
              "sFlowSamplerRateApply: Failed to enable sampler for dsIndex %u instance %u",
              dsIndex, instance);
      return L7_FAILURE;
    }
  ...
  if (sFlowIsReady() == L7_TRUE)
  {
    ...
    for (idx = 0; idx < SFLOW_MAX_RECEIVERS; idx++)
    {
      ...
      if (agent.receiver[idx].pduLen)
      {
         ...
         {
           L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                   "sFlowTimerTickProcess: Failed to flush the receiver data");
         }
      }
                                           ...
                                           L7_MATCH_GETNEXT)))
  {
    ...
    if (pPoller->pollerCfg->sFlowCpReceiver)
    {
      if (!agent.sFlowCfg->receiverGroup[pPoller->pollerCfg->sFlowCpReceiver - 1].sFlowRcvrTimeout)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                  "sFlowSamplerPollerInstancesCleanup: could not delete poller for dsIndex %u instance %u",
                   dsIndex, instance);
        }
                                             ...
                                             L7_MATCH_GETNEXT)))
  {
    ...
    if (pSampler->samplerCfg->sFlowFsReceiver)
    {
      if (!agent.sFlowCfg->receiverGroup[pSampler->samplerCfg->sFlowFsReceiver - 1].sFlowRcvrTimeout)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                  "sFlowSamplerPollerInstancesCleanup: could not delete sampler for dsIndex %u instance %u",
                   dsIndex, instance);
        }
      } /* Check if the assigned receiver is active */
    }/* check if a valid receiver is configured */
                              ...
                              L7_uint32 rcvrIndx)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowPollerAdd: failed to add poller object for dsIndex %u instance %u",
           dsIndex, instance);
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowPollerAdd: failed to find the added poller object for dsIndex %u instance %u",
           dsIndex, instance);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowPollerAdd: failed to enable poller for dsIndex %u instance %u",
            dsIndex, instance);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
                               ...
                               L7_uint32 rcvrIndx)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowSamplerAdd: failed to add sampler object for dsIndex %u instance %u",
           dsIndex, instance);
    /* remove the entry from cfg table */
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowSamplerAdd: failed to find the added sampler object for dsIndex %u instance %u",
           dsIndex, instance);
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowSamplerAdd: failed to enable sampler for dsIndex %u instance %u",
            dsIndex, instance);
    return L7_FAILURE;
  }
  return L7_SUCCESS;
...
static L7_RC_t sFlowPollerDelete(SFLOW_poller_t *pPoller)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowPollerDelete: failed to disable poller for dsIndex %u instance %u",
...
static L7_RC_t sFlowSamplerDelete(SFLOW_sampler_t *pSampler)
{
  ...
  /* disable sampler operational functionality */
   ...
   {
     L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
             "sFlowSamplerDelete: failed to disable sampler for dsIndex %u instance %u",
        ...
        sFlowIsDataSourceReady(pTempSampler->key.dsIndex) == L7_TRUE)
    {
      if (sFlowIsReceiverReady(pReceiver) == L7_TRUE)
      {
        ...
        else
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                    "sFlowSamplerOperModeSet: Failed to read sampling rate from hardware for dsIndex %u",
  ...
  while (pTempPoller)
  {
        ...
        sFlowIsDataSourceReady(pTempPoller->key.dsIndex) == L7_TRUE)
    {
      if (sFlowIsReceiverReady(pReceiver) == L7_TRUE)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                  "sFlowPollerOperModeSet: Timer start failed for dsIndex %u instance %u",
                   pTempPoller->key.dsIndex, pTempPoller->key.sFlowInstance);
          if (rc == L7_SUCCESS)
          {
            rc = L7_FAILURE;
    ...
    else
    {
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                "sFlowPollerOperModeSet: Timer stop failed for dsIndex %u instance %u",
                 pTempPoller->key.dsIndex, pTempPoller->key.sFlowInstance);
        if (rc == L7_SUCCESS)
        {
          rc = L7_FAILURE;
...
{
  /* Seed and priority parameters never change, so
   * Driver sets these during system initialization.
   */
#if 0
  ...
  if ((pSampler = sFlowSamplerInstanceGet(dsIndex, instance, L7_MATCH_EXACT)))
  {
    ...
    if (agent.congestionAlert == 0)
    {
      ...
      if (samplerRate != pSampler->samplerCfg->sFlowFsPacketSamplingRate)
      {
        ...
        {
          L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
                  "sFlowSamplingRateThrottle: Failed to modify sampling rate for dsIndex %d instance %d",
                  dsIndex, instance);
        }
        else
        {
