...
L7_BOOL sFlowIsDataSourceReady(L7_uint32 dsIndex)
{
  ...
  if (nimIntfQuery(&nimQueryData) != L7_SUCCESS)
  {
    ...
    nimGetIntfName(nimQueryData.intIfNum, L7_SYSNAME, ifName);
    /* should never get here */
    L7_LOGF(L7_LOG_SEVERITY_INFO, L7_SFLOW_COMPONENT_ID,
            "sFlowIntfCanBeEnabled: nimIntfQuery failed for intf %s\n",
             ifName);
    return L7_FALSE;
  }
 ...
 * then check IP address for Service Port */
  ...
  if (usmDbAgentIpIfAddressGet(L7_NULL, &(ipAddr->addr.ipv4.s_addr)) != L7_SUCCESS)
  {
