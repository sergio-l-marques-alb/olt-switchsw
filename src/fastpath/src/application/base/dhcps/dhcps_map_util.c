...
/*********************************************************************
* @purpose  Find the address pool that should be used to offer an address to 
*           a given client. 
*
* @param    ipAddr         @b{(input)}   Identifies the IP subnet the host belongs to.
*                                        Either the IP address on the DHCP server's 
*                                        interface where the client request was received
*                                        or the giaddr from the DHCP relay agent. 
* @param    pClientKey     @b{(input)}   Client key. Identifies the client, either 
*                                        by hardware address or client ID (option 61).
*                                        If client specifies option 61, that is used
*                                        rather than hw address. 
* @param    ppPool         @b{(output)}  Matching pool.
*
* @returns  L7_SUCCESS if a pool is identified
...
L7_RC_t dhcpsMatchPool(L7_IP_ADDR_t ipAddr, dhcpsClientLeaseKey_t *pClientKey, dhcpsPoolNode_t **ppPool)
{
  ...
  *ppPool = L7_NULLPTR;
  ...
  while(L7_NULLPTR != pTempPool)
  {
    ...
    {
      /* If the pool is manual, see if a manual binding has been specified for 
       * this client. Compare the client key to both the pool's client ID and the pool's 
       * client hardware address. Accept a match on either. */
      getClientKey(NULL, 0, pTempPool->poolData->hostAddr,
                   pTempPool->poolData->hostAddrtype,
                   pTempPool->poolData->hostAddrLength, &poolKey);
      if (((pClientKey->keyType == DHCPS_CLIENTID_KEY_TYPE) && 
           (memcmp(pClientKey->keyData, pTempPool->poolData->hostIdentifier, 
                   L7_DHCPS_CLIENT_ID_MAXLEN) == 0)) ||
          (memcmp(pClientKey, &poolKey, sizeof(dhcpsClientLeaseKey_t)) == 0))
      {
        L7_uchar8 clientAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
        L7_uchar8 intfAddrStr[OSAPI_INET_NTOA_BUF_SIZE];
        osapiInetNtoa(pTempPool->poolData->ipAddr, clientAddrStr);
        osapiInetNtoa(ipAddr, intfAddrStr);

        *ppPool = pTempPool;
        DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED,
          "Matched manual pool with IP address %s for client on interface with IP address %s.",
          clientAddrStr, intfAddrStr);
        break;
      }
    }
    else
  ...
  dhcpsLeaseNode_t * * ppLease)
{
  ...
  *ppLease = L7_NULLPTR;
  ...
  if(index < L7_DHCPS_MAX_LEASE_NUMBER)
  {
    ...
    (void)osapiStrncpySafe(pDhcpsMapLeaseCfgData->lease[index].poolName, pPool->poolData->poolName,
                           L7_DHCPS_POOL_NAME_MAXLEN + 1);
...
/*********************************************************************
* @purpose  Given a client ID (option 61) and a client hardware address, form
*           a key that uniquely identifies the client.
*
...
* @returns  L7_FAILURE
*
* @notes    RFC 2131 section 4.2 says, 
* 
*              If the client supplies a 'client identifier', the client MUST use 
*              the same 'client identifier' in all subsequent messages, and the 
...
*
*           So here we form the key from the client ID, if specified, and from 
*           the hw addr if no client ID is specified.
*
*           Note that we allow the network admin to specify both a client ID
*           and hardware address of a client on a pool. If both are specified, the
*           DHCP server should allow a client to match on either. That is, if a 
*           client does not specify a client ID, the DHCP server should match on the
*           client's hw address, even for manual pools where a client ID is specified.
*           So this function should NOT be used to form a key from pool data, unless
*           the caller is careful to specify only the client ID or hw address from 
*           the pool and compare on both. 
*
* @end
*********************************************************************/
L7_RC_t getClientKey(L7_uchar8 *pClientIdentifier, L7_uchar8 clientIdLen, 
                     L7_uchar8 *pHwAddr, L7_uchar8 hAddrtype, L7_uchar8 
                     hwAddrLength, dhcpsClientLeaseKey_t *pLeaseKey)
{
  /* blank out key */
  ...
  if(L7_NULLPTR != pClientIdentifier)
  {
    if(clientIdLen != 0)
    {
      /* Client ID is specified. Use it to identify the client. */
  ...
  if(L7_NULLPTR != pHwAddr)
  {
    ...
    /* Use hardware address to identify client. */
...
L7_RC_t dhcpsScheduleLease(dhcpsLeaseNode_t * pLease, L7_uint32 secs)
{
  ...
  pLease->leaseData->leaseExpiration = pLease->leaseStartTime + secs;
...
L7_RC_t dhcpsUnscheduleLease(dhcpsLeaseNode_t * pLease)
{
  ...
  if(dhcpsListRemoveLink(&pLease->scheduledLeasesLink) == L7_SUCCESS)
  {
    DHCPS_MAP_PRT(DHCPS_MAP_MSGLVL_MED, "Unscheduled lease IP addr %x\n",
      pLease->leaseData->ipAddr);
    pLease->leaseData->leaseExpiration = simSystemUpTimeGet();
...
*
* @param    pPool             @b{(input)}  DHCPS pool node.
* @param    ipAddr            @b{(input)}  IP Address
...
* @end
*********************************************************************/
L7_BOOL dhcpsCheckIfExcluded(dhcpsPoolNode_t *pPool, 
                             L7_IP_ADDR_t ipAddr,
                             L7_BOOL isManualExcluded)
{
  dhcpsPoolNode_t *manualPool = L7_NULLPTR;
  L7_uint32 i;
  ...
  if(L7_TRUE == isManualExcluded)
    if(dhcpsFindPoolByIp(ipAddr, &manualPool) == L7_SUCCESS)
      return L7_TRUE;
  ...
  /* check against the configured addresses like default-router,
   * dns-server, netbios-name-server, next-server in this pool */
  if(L7_NULLPTR != pPool)
  {
    if(pPool->poolData->nextServer == ipAddr)
    {
      return L7_TRUE;
    }
    for(i=0; i<L7_DHCPS_NETBIOS_NAME_SERVER_MAX; i++)
    {
      if(pPool->poolData->netBiosNameServer[i] == ipAddr)
      {
        return L7_TRUE;
      }
    }
    ...
    {
      if(pPool->poolData->defaultrouterAddrs[i] == ipAddr)
      {
        return L7_TRUE;
      }
    }
    ...
    {
      if(pPool->poolData->DNSServerAddrs[i] == ipAddr)
      {
        return L7_TRUE;
      }
    }
