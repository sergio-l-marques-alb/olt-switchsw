...
extern PORTEVENT_MASK_t linkDependPortEventMask_g;
...
{
  LinkDependencyData_g.warmRestart = L7_FALSE;
...
L7_RC_t linkDependencyCfgSave( void )
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LINK_DEPENDENCY_COMPONENT_ID,
...
L7_RC_t linkDependencyIntfStateUpdate( L7_uint32 intIfNum )
{
  ...
  if (L7_TRUE == LinkDependencyData_g.warmRestart)
  {
    return L7_SUCCESS;
  }
...
L7_RC_t linkDependencyGroupStateUpdate( L7_uint32 groupId )
{
  ...
  else
  {
    ...
    {
      /* If there any dependent links, then either take the group members
         up or down depending upon Link-action. The decision to move a port
         state is made here. The function linkDependencyIntfStateUpdate() just
         does the leg work.
      */
      if (dependencies->value[i])
      {
        /* Depending upon link-action, chose target state of member links */
        if ( group->link_action == L7_LINK_DEPENDENCY_ACTION_LINK_UP)
        {
          /* If dependent link is DOWN, bring the group members UP */
          linksDown = (dependencies->value[i] & status->value[i]) ?L7_TRUE :L7_FALSE;
        }
        else
        {
          /* If dependent link is DOWN, take the group members DOWN */
          linksDown = (dependencies->value[i] & status->value[i]) ?L7_FALSE :L7_TRUE;
        }
        break;
      }
    }
...
/*********************************************************************
* @purpose  Propogate Startup notifications to Link Dependency Task
*
...
* @end
*********************************************************************/
void linkDependencyStartupNotifyCallBack(NIM_STARTUP_PHASE_t startupPhase)
{
  L7_RC_t   rc;
  L7_uint32 intIfNum;
  L7_uint32 linkState;
  ...
  if (startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
  {
    ...
    /* Register with NIM to receive port L7_UP/L7_DOWN events */
    PORTEVENT_SETMASKBIT(linkDependPortEventMask_g, L7_UP);
    PORTEVENT_SETMASKBIT(linkDependPortEventMask_g, L7_DOWN);
    nimRegisterIntfEvents(L7_LINK_DEPENDENCY_COMPONENT_ID,
                          linkDependPortEventMask_g);
  }
  nimStartupEventDone(L7_LINK_DEPENDENCY_COMPONENT_ID);
}
...
void linkDependencyGroupDump( L7_uint32 groupId )
{
  ...
  printf( "Group ID = %d\r\n", groupId );
  printf( "\tinUse = %d\r\n", group->inUse );
  printf( "\tAction = %s(%d)\r\n", (group->link_action?"Up":"Down"), group->link_action);
