...
PORTEVENT_MASK_t linkDependPortEventMask_g;
          ...
          case L7_CNFGR_CMD_INITIALIZE:
            switch ( request )
            {
              ...
              case L7_CNFGR_RQST_I_PHASE3_START:
                ...
                LinkDependencyData_g.warmRestart = (pCmdData->u.rqstData.data & L7_CNFGR_RESTART_WARM) ? L7_TRUE : L7_FALSE;
...
L7_RC_t linkDependencyPhaseOneInit(void)
{
  ...
  if ( L7_NULL == LinkDependencySemaphore )
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LINK_DEPENDENCY_COMPONENT_ID,
            "Unable to create link dependency semaphore()\n");
    return(L7_FAILURE);
  }
...
L7_RC_t linkDependencyPhaseTwoInit(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LINK_DEPENDENCY_COMPONENT_ID,
            "Link Dependency: Failed to register NV routines\n");
  }
  /* register callback with NIM for L7_UPs and L7_DOWNs */
  else if ((rc = nimRegisterIntfChange(L7_LINK_DEPENDENCY_COMPONENT_ID,
                                       linkDependencyIntfChangeCallBack,
                                       linkDependencyStartupNotifyCallBack,
                                       NIM_STARTUP_PRIO_LINK_DEPENDENCY)) != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LINK_DEPENDENCY_COMPONENT_ID,
            "Link Dependency: failed to register to NIM\n");
  }
#ifdef L7_CLI_PACKAGE
  /* Register with cli text config componet to get notifications after
     apply config completion */
  else if (L7_SUCCESS != (rc = txtCfgApplyCompletionNotifyRegister(L7_LINK_DEPENDENCY_COMPONENT_ID,
                                                                   linkDepTxtCfgApplyCompletionCallback)))
  {
    L7_LOGF(L7_LOG_SEVERITY_ALERT, L7_LINK_DEPENDENCY_COMPONENT_ID,
            "Link Dependency failed to register for config apply completion.");
  }
#endif
...
L7_RC_t   linkDependencyPhaseThreeInit(void)
{
  ...
  memset(&linkDependPortEventMask_g, 0, sizeof(linkDependPortEventMask_g));
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_LINK_DEPENDENCY_COMPONENT_ID,
            "linkDependencyApplyConfigData: failed \n");
  }
