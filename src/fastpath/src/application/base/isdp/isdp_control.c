  ...
  for (;;)
  {
    ...
    else
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
              "isdp message recv failed: event %d, intf %s\n",
              msg.event, ifName);
    }
  }
}
...
void isdpDispatchCmd(isdpMsg_t msg)
{
  ...
  case ISDP_INTF_STARTUP_EVENT:
    if (msg.data.nimStartupEvent.startupPhase == NIM_INTERFACE_CREATE_STARTUP)
    {
      (void)isdpNimCreateStartupProcess();
    }
    else if (msg.data.nimStartupEvent.startupPhase == NIM_INTERFACE_ACTIVATE_STARTUP)
    {
      (void)isdpNimActivateStartupProcess();
    }
    else
    {
      L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_ISDP_COMPONENT_ID,
              "%s: Unknown NIM startup event %d.",
              __FUNCTION__, msg.data.nimStartupEvent.startupPhase);
    }
    break;
...
L7_RC_t isdpProcessMsgQueue(isdpMsg_t msg)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
            "isdpProcessQueue is not initialized\n");
    return L7_FAILURE;
  }
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
            "Failed to give msgQueue semaphore\n");
  }
...
L7_RC_t isdpPduReceiveQueue( L7_uint32 intIfNum, L7_netBufHandle bufHandle)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
            "isdpPduQueue is not initialized\n");
    return L7_FAILURE;
  }
...
L7_RC_t isdpPduReceiveProcess(L7_uint32 intIfNum, L7_netBufHandle bufHandle)
{
  ...
  if(rc != L7_SUCCESS)
  {
    ...
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
            "isdpFillEntry failed: intf %s\n", ifName);
    isdpIpAddrListRelease(&isdpPacket.ipAddressList);
    return rc;
  }

  /* If a VoIP VLAN Request is received, inform Voice VLAN of the voice device */
  if (0 != isdpEntry.voipVlan)
  {
    voiceVlanPortDeviceVlanAdd(intIfNum, L7_ISDP_COMPONENT_ID, isdpPacket.ether.src.addr);
  }

  /* Can only add entry if client is authorized */
  if (L7_TRUE == dot1xPortClientAuthenticationGet(intIfNum, isdpPacket.ether.src.addr))
  {
    ...
    if (rc != L7_SUCCESS)
    {
      ...
      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
              "isdpEntryAdd failed: intIfNum %s\n", ifName);
    }
    else
    {
      /* If a VoIP VLAN Query was received, send the reply right away */
      if (0 != isdpEntry.voipVlan)
      {
        /* There is a voice VLAN, so transmit the reply immediately */
...
/*********************************************************************
* @purpose  Parse received buffer with Tlvs
*
                            ...
                            isdpPacket_t* isdpPacket, char* value)
{
  ...
  case ISDP_VOIP_VLAN_QUERY_TLV_TYPE:
    /* copy from value + 1 to skip over unknown data (one byte), VLAN ID is two bytes */
    memcpy(&vlanId, &value[1], sizeof(isdpPacket->isdp_voip_vlan));
    isdpPacket->isdp_voip_vlan = osapiNtohs(vlanId);
    break;
...
L7_RC_t isdpPduSend(void)
{
  ...
  {
    L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_ISDP_COMPONENT_ID,
            "Isdp is disabled, timer should be deactivated! \n");
    return L7_FAILURE;
  }
  ...
  while (rcNim == L7_SUCCESS)
  {
    ...
    state = L7_DOWN;
    if  (nimGetIntfLinkState(intIfNum,&state) != L7_SUCCESS ||(state != L7_UP) )
    {
        rcNim = nimNextValidIntfNumber(intIfNum, &intIfNum);
        continue;
    }

    if (isdpIntfTbl[intIfNum].intfAcquired == L7_TRUE)
    {
      rcNim = nimNextValidIntfNumber(intIfNum, &intIfNum);
      continue;
    }
