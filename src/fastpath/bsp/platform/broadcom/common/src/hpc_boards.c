...
L7_RC_t hpcConfigPhySet(void)
{
  ...
  PT_LOG_TRACE(LOG_CTX_STARTUP,"Configuring ports (%u slots)...",L7_MAX_PHYSICAL_SLOTS_PER_UNIT);
  ...
  PT_LOG_TRACE(LOG_CTX_STARTUP,"Finished bcm_ports configuration!");
...
L7_RC_t hpcConfigSet()
{
  ...
  /* Ptin removed: init */
  #if 1
  PT_LOG_NOTICE(LOG_CTX_STARTUP,"hpcXeHgSetup/hpcGeHlSetup will not be executed here!");
  #else
  ...
  /* Ptin added: init */
  #if 1
  PT_LOG_NOTICE(LOG_CTX_STARTUP,"hpcXeHgSetup/hpcGeHlSetup will be executed now!");
  /* setup the xe/hg combo ports */
  hpcXeHgSetup();
  hpcGeHlSetup();
  #endif
