...
static int           poeTxDebug = L7_FALSE;
static int           poeRxDebug = L7_FALSE;
static int           poeErrorDebug = L7_FALSE;

static L7_RC_t hpcBrcmPoeInitPhase2(L7_uint32 cardIndex);
#ifdef PLAT_POE_FW_UPDATE
#include "poe_firmware.h"
...
*
*******************************************************************************/
void hpcBrcmDebugPoePkt(int txEnable, int rxEnable,int errorEnable)
{
  poeTxDebug = txEnable;
  poeRxDebug = rxEnable;
  poeErrorDebug = errorEnable;
}
...
#if (PLAT_POE_COMM_BSC == L7_ENABLE)
...
static int hpcBrcmPoeSendMsg(L7_uint32 cardIndex, unsigned char *txBuf)
{
  ...
  do
  {
    if( (rv = poe_device_write(cardIndex, txBuf, BSC_MSG_LEN)) == BSC_MSG_LEN)
    {
      break;
    }
    retry_count++;
...
#ifdef PLAT_POE_FW_UPDATE
static int hpcBrcmPoeSendMsgRaw(L7_uint32 cardIndex, unsigned char *txBuf,int len)
{
  unsigned int retry_count = 0;
  int rv;
  ...
  do
  {
    if( (rv = poe_device_write(cardIndex, txBuf, len)) == len)
    {
      break;
    }
    retry_count++;
                           ...
                           unsigned char *txBuf, unsigned char *rxBuf)
{
  ...
  while(retry_count < POE_DRVR_COMMAND_MAX_RETRIES)
  {
    ...
    skip_send = FALSE;
    memset(rxBuf,0xff,BSC_MSG_LEN);
    osapiSleepMSec(POE_WAIT_TIME_TO_READ_RESPONSE); /* delay before reading for response */
    rc = hpcBrcmPoeReceiveMsg(cardIndex, rxBuf);

    if(rc == POE_E_NONE)
    {
       if( (rxBuf[0] != POE_DRVR_ERROR_BOOT_CODE) &&
       ...
       else
       {
          ...
          {
            if(poeErrorDebug)
              printf("Response is not ready at the PoE Chip side\n");
            skip_send = TRUE;
          }
          if(rxBuf[0] == POE_DRVR_ERROR_BAD_REQUEST_CHKSM)
          {
            if(poeErrorDebug)
              printf("Checksum Error at the PoE Chip side\n");
          }
          if(txBuf[0] != rxBuf[0])
          {
            if(poeErrorDebug)
              printf("Sent Command is 0x%x , Received Command is 0x%x\n",txBuf[0],rxBuf[0]);
          }
       }
    }
    else if((rc == POE_E_CHECKSUM)||(rc == POE_E_RECEIVE))
    {
       /*printf("Receive %s Failure\n",(rc==POE_E_CHECKSUM)?"Checksum":"");*/
        if(poeErrorDebug)
        {
/*
          printf("%s ", "Error RESP: ");
...
{
  int    rc;
  unsigned char local_tx_buf[BSC_MSG_LEN];
  unsigned char local_rx_buf[BSC_MSG_LEN];

  memcpy(local_tx_buf, data, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);

  rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);
  if (L7_POE_OK(rc) == TRUE)
  {
    if(local_rx_buf[3] == 0) /* 0 - Ack */
    {
      rc = POE_E_NONE;
    }
  }
                       ...
                       unsigned short guardBandPower)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);

  local_tx_buf[POE_CMD_FIELD] = POE_POWER_SOURCE_CONFIG_CMD;
  local_tx_buf[2] = powerBankComb;
  /* Max power allocated */
  ushortVal = osapiHtons(totalPower);
  memcpy(&local_tx_buf[3], &ushortVal, 2);
  /* Guard Band allowed */
  ushortVal = osapiHtons(guardBandPower);
  memcpy(&local_tx_buf[5], &ushortVal, 2);
                                 ...
                                 unsigned char mode)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_POWER_MGMT_MODE_CONFIG_CMD;
  local_tx_buf[2] = mode;
...
int hpcBrcmPoeMsgGlobalIlimIcutSet(L7_uint32 cardIndex, unsigned char mode)
{
  int rc;
  unsigned char local_tx_buf[BSC_MSG_LEN];
  unsigned char local_rx_buf[BSC_MSG_LEN];
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_GLOBAL_ILIM_CMD;
  local_tx_buf[2] = mode;
                                    ...
                                    unsigned char mode)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_LOGICAL_PORT_MAP_ENABLE_CMD;
  local_tx_buf[2] = mode;
                            ...
                            unsigned char psePort, unsigned char logicalPsePort)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_PORT_MAP_CONFIG_CMD;
  local_tx_buf[2] = psePort;
  local_tx_buf[3] = logicalPsePort;

  rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);
  if (L7_POE_OK(rc) == TRUE)
  {
    if(local_rx_buf[3] == 0 && local_rx_buf[2] == psePort) /* 0 - Ack */
    {
      rc = POE_E_NONE;
    }
  }
...
*
*******************************************************************************/
static int hpcBrcmPoeMsgRapidPowerDownModeSet(L7_uint32 cardIndex,
        unsigned char mode, unsigned char priority)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_RAPID_POWER_DOWN_ENABLE_CMD;
  local_tx_buf[2] = mode;
  local_tx_buf[3] = priority;
...
/*******************************************************************************
*
* @purpose To Reset the poe sub system
*
* @param   cardIndex       @b{(input)}  Broadcom Unit number
* @param   mode            @b{(input)}  enable or disable
*
* @returns POE_E_NONE, if successfull
*
...
*
*******************************************************************************/
static int hpcBrcmPoeMsgResetCard(L7_uint32 cardIndex)
{
  int    rc;
  unsigned char local_tx_buf[BSC_MSG_LEN];
  unsigned char local_rx_buf[BSC_MSG_LEN];
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_GLOBAL_RESET_CMD;
  local_tx_buf[2] = L7_TRUE;
                           ...
                           unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_PSE_ENABLE_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
  ...
  else
  {
            memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
            memset(local_rx_buf, 0, BSC_MSG_LEN);
            local_tx_buf[POE_CMD_FIELD] = POE_GLOBAL_PSE_ENABLE_CMD;
            local_tx_buf[2] = mode;
            ...
            if (L7_POE_OK(rc) == TRUE)
            {
                    if((local_rx_buf[3] == 0) && (local_rx_buf[5] == 0) && (local_rx_buf[7] == 0) && (local_rx_buf[9] == 0)) /* 0 - Ack */
                    {
                            rc = POE_E_NONE;
                    }
                    else
                              ...
                              unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_AUTO_POWER_UP_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
  ...
  else
  {
      ...
      while (channel != 0xFF)
      {
          ...
          memset(local_rx_buf, 0, BSC_MSG_LEN);
          local_tx_buf[POE_CMD_FIELD] = POE_PORT_AUTO_POWER_UP_CONFIG_CMD;
          local_tx_buf[2] = channel--;
          local_tx_buf[3] = mode;
          local_tx_buf[4] = channel--;
          local_tx_buf[5] = mode;
          local_tx_buf[6] = channel--;
          local_tx_buf[7] = mode;
          local_tx_buf[8] = channel--;
          local_tx_buf[9] = mode;
                                   ...
                                   unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_POWER_UP_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
                        ...
                        unsigned char psePort)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_RESET_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = 1;
                                   ...
                                   unsigned char psePort, unsigned char type)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_DETECTION_TYPE_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = type;
                                      ...
                                      unsigned char psePort, unsigned char type)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_CLASSIFICATION_TYPE_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = type;
                                    ...
                                    unsigned char psePort, unsigned char type)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_DISCONNECT_TYPE_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = type;
                          ...
                          unsigned char psePort, unsigned char pair)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_PAIR_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = pair;
                                 ...
                                 unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_POWER_UP_MODE_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
                              ...
                              unsigned char psePort, unsigned char priority)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_PRIORITY_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = priority;
  ...
  else
  {
      ...
      while (channel != 0xFF)
      {
          ...
          local_tx_buf[3] = priority;
          local_tx_buf[4] = channel--;
          local_tx_buf[5] = priority;
          local_tx_buf[6] = channel--;
          local_tx_buf[7] = priority;
          local_tx_buf[8] = channel--;
          local_tx_buf[9] = priority;
...
static int hpcBrcmPoeMsgPortPowerLimitSet(L7_uint32 cardIndex,
                              unsigned char psePort, unsigned short powerLimit)
{
  int    rc = POE_E_NONE;
  HPC_POE_CARD_DATA_t *cardData = hpcPoeCardDataGet(cardIndex);
  unsigned char channel = 0;
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_MAX_POWER_THRESHOLD_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      /*local_tx_buf[3] = powerLimit/2; */
      local_tx_buf[3] = powerLimit/200;  /* As the limit is sent in Milli Watts */
  ...
  else
  {
      ...
      while (channel != 0xFF)
      {
          ...
          local_tx_buf[POE_CMD_FIELD] = POE_PORT_MAX_POWER_THRESHOLD_CONFIG_CMD;
          local_tx_buf[2] = channel--;
          local_tx_buf[3] = powerLimit/200;
          local_tx_buf[4] = channel--;
          local_tx_buf[5] = powerLimit/200;
          local_tx_buf[6] = channel--;
          local_tx_buf[7] = powerLimit/200;
          local_tx_buf[8] = channel--;
          local_tx_buf[9] = powerLimit/200;
                                     ...
                                     unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_POWER_THRESHOLD_TYPE_CONFIG_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
        ...
        unsigned char *configStatus)
{
  ...
  local_tx_buf[POE_CMD_FIELD] = POE_SYSTEM_STATUS_QUERY_CMD;
  local_tx_buf[1] = 0xFF;

  rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);
  if (L7_POE_OK(rc) == TRUE)
  {
    *modePins = local_rx_buf[2];
    *poePorts = local_rx_buf[3];
    *portMap  = local_rx_buf[4];
    memcpy(hwVer, &local_rx_buf[5], 2);
    *hwVer = osapiNtohs(*hwVer);
    *swVer   = local_rx_buf[7];
    *eepromStatus = local_rx_buf[8];
    *configStatus = local_rx_buf[9];
    rc = POE_E_NONE;
  }
                                       ...
                                       unsigned char  *ilimStatus)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_TOTAL_POWER_ALLOC_QUERY_CMD;
  rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);

  if (L7_POE_OK(rc) == TRUE)
  {
    memcpy(&ushortVal, &local_rx_buf[2], 2);
    ushortVal = osapiNtohs(ushortVal);
    *powerAllocated = ushortVal;

    memcpy(&ushortVal, &local_rx_buf[4], 2);
    ushortVal = osapiNtohs(ushortVal);
    *powerAvailable = ushortVal;

    *mpsmStatus = local_rx_buf[6];      /* Multiple Power Source Status*/
    *ilimStatus = local_rx_buf[7];
    rc = POE_E_NONE;
  }
                            ...
                            unsigned char *remotePd, unsigned char *mpssMask)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_STATUS_QUERY_CMD;
      local_tx_buf[2] = psePort;
      ...
      if (L7_POE_OK(rc) == TRUE)
      {
          *pdStatus  = local_rx_buf[3]; /* Power Device detection status */
          *classInfo = local_rx_buf[4]; /* dot3af classification info */
          *detectInfo = local_rx_buf[5]; /* Pd Detection Status */
          *remotePd = local_rx_buf[6];  /* remote Power Device Type */
          *mpssMask = local_rx_buf[7];  /* Port Mask for MPSS */
          rc = POE_E_NONE;
      }
  }
                            ...
                            unsigned char psePort, unsigned char *pdStatus, unsigned char *classInfo)
{
  ...
  unsigned char local_rx_buf[BSC_MSG_LEN];
    ...
    memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
    memset(local_rx_buf, 0, BSC_MSG_LEN);
    local_tx_buf[POE_CMD_FIELD] = POE_MULTIPLE_PORT_STATUS_QUERY_CMD;
    local_tx_buf[2] = psePort;
    local_tx_buf[3] = 1;
    local_tx_buf[4] = psePort+1;
    local_tx_buf[5] = 1;
    local_tx_buf[6] = psePort+2;
    local_tx_buf[7] = 1;
    local_tx_buf[8] = psePort+3;
    local_tx_buf[9] = 1;
    rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);
    if (L7_POE_OK(rc) == TRUE)
    {
          pdStatus[0]  = local_rx_buf[3] & 0x0F; /* Power Device detection status */
          classInfo[0] = (local_rx_buf[3] & (0x70))>>4; /* dot3af classification info */

          pdStatus[1] =  local_rx_buf[5] & 0x0F;
          classInfo[1] = (local_rx_buf[5] & (0x70))>>4; /* dot3af classification info */

          pdStatus[2] =  local_rx_buf[7] & 0x0F;
          classInfo[2] = (local_rx_buf[7] & (0x70))>>4; /* dot3af classification info */

          pdStatus[3] =  local_rx_buf[9] & 0x0F;
          classInfo[3] = (local_rx_buf[9] & (0x70))>>4; /* dot3af classification info */
          rc = POE_E_NONE;
    }
                           ...
                           unsigned short *portTemp)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_MEASUREMENT_QUERY_CMD;
      local_tx_buf[2] = psePort;
      ...
      if (L7_POE_OK(rc) == TRUE)
      {
          memcpy(&ushortVal, &local_rx_buf[3], 2);  /* Voltage */
          ushortVal = osapiNtohs(ushortVal);
          *portVoltage = ushortVal;

          memcpy(&ushortVal, &local_rx_buf[5], 2);  /* Current */
          ushortVal = osapiNtohs(ushortVal);
          *portCurr = ushortVal;

          memcpy(&ushortVal, &local_rx_buf[7], 2);  /* Temperature */
          ushortVal = osapiNtohs(ushortVal);
          *portTemp = ushortVal;

          memcpy(&ushortVal, &local_rx_buf[9], 2);  /* Power */
          ushortVal = osapiNtohs(ushortVal);
          *portPower = ushortVal;
          rc = POE_E_NONE;
      }
    ...
    unsigned char *invalidSignCntr)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_PORT_STATS_QUERY_CMD;
  local_tx_buf[2] = psePort;
  ...
  if (L7_POE_OK(rc) == TRUE)
  {
    *mpsAbsentCntr     = local_rx_buf[3];   /* MPS Absent */
    *overloadCntr      = local_rx_buf[4];   /* Overload Counter */
    *shortCntr         = local_rx_buf[5];   /* Short Counter */
    *powerDeniedCntr   = local_rx_buf[6];   /* Power Denied Counter */
    *invalidSignCntr   = local_rx_buf[7];   /* Invalid Signature */
    rc = POE_E_NONE;
  }
                                  ...
                                  unsigned char psePort)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_STATS_RESET_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = 1;
    ...
    unsigned char *pairConfig)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_PORT_CONFIG_QUERY_CMD;
  local_tx_buf[2] = psePort;
  ...
  if (L7_POE_OK(rc) == TRUE)
  {
    *pseMode        = local_rx_buf[3];   /* PSE Mode */
    *autoMode       = local_rx_buf[4];   /* Auto mode */
    *detectionType  = local_rx_buf[5];   /* Detection Type */
    *classifyType   = local_rx_buf[6];   /* Classification Type */
    *disconType     = local_rx_buf[7];   /* Disconnect Type */
    *pairConfig     = local_rx_buf[8];   /* Power Pair Config */
    rc = POE_E_NONE;
  }
    ...
    unsigned char *priority, unsigned char *physicalPort)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_PORT_EXTEN_CONFIG_QUERY_CMD;
  local_tx_buf[2] = psePort;
  ...
  if (L7_POE_OK(rc) == TRUE)
  {
    *powerMode      = local_rx_buf[3];   /* High Power Mode */
    *violationType  = local_rx_buf[4];   /* violation type */
    *maxPower       = local_rx_buf[5];   /* Max Allowed Power */
    *priority       = local_rx_buf[6];   /* Port Priority */
    *physicalPort  = local_rx_buf[7];   /* Physically Mapped  Port */
    rc = POE_E_NONE;
  }
    ...
    unsigned short *guardBand)
{
  ...
  memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
  memset(local_rx_buf, 0, BSC_MSG_LEN);
  local_tx_buf[POE_CMD_FIELD] = POE_POWER_MGMT_CONFIG_QUERY_CMD;
  local_tx_buf[1] = power_bank;

  rc = hpcBrcmPoeTransReceiveMsg(cardIndex, local_tx_buf, local_rx_buf);

  if (L7_POE_OK(rc) == TRUE)
  {
    *powerMode     = local_rx_buf[2];   /* Power Management Mode */

     memcpy(&ushortVal, &local_rx_buf[3], 2);  /* Max Configured Power */
     ushortVal = osapiNtohs(ushortVal);
    *maxPower = ushortVal;

     memcpy(&ushortVal, &local_rx_buf[5], 2);  /* Guard Band */
     ushortVal = osapiNtohs(ushortVal);
    *guardBand = ushortVal;
    rc = POE_E_NONE;
  }
...
static int hpcBrcmPoeMsgClearEepromConfig(L7_uint32 cardIndex)
{
  ...
  local_tx_buf[POE_CMD_FIELD] = POE_EEPROM_CONFIG_CMD;
  local_tx_buf[POE_SUB_CMD_FIELD] = POE_EEPROM_CLEAR_CONFIG_SUB_CMD;
...
static int hpcBrcmPoeMsgClearEepromApplicationImage(L7_uint32 cardIndex)
{
  ...
  local_tx_buf[POE_CMD_FIELD] = POE_EEPROM_CONFIG_CMD;
  local_tx_buf[POE_SUB_CMD_FIELD] = POE_EEPROM_CLEAR_APPLICATION_SUB_CMD;
...
static int hpcBrcmPoeMsgSaveEepromConfig(L7_uint32 cardIndex)
{
  ...
  local_tx_buf[POE_CMD_FIELD] = POE_EEPROM_CONFIG_CMD;
  local_tx_buf[POE_SUB_CMD_FIELD] = POE_EEPROM_SAVE_CONFIG_SUB_CMD;
...
static int hpcBrcmPoeMsgSaveEepromApplicationImage(L7_uint32 cardIndex)
{
  ...
  local_tx_buf[POE_CMD_FIELD] = POE_EEPROM_CONFIG_CMD;
  local_tx_buf[POE_SUB_CMD_FIELD] = POE_EEPROM_SAVE_APPLICATION_SUB_CMD;
                                 ...
                                 unsigned char psePort, unsigned char mode)
{
  ...
  if (psePort != POE_CHANNEL_ALL)
  {
      memset(local_tx_buf, POE_NA_FIELD_VALUE, BSC_MSG_LEN);
      memset(local_rx_buf, 0, BSC_MSG_LEN);
      local_tx_buf[POE_CMD_FIELD] = POE_PORT_SWITCH_HIGH_POWER_CMD;
      local_tx_buf[2] = psePort;
      local_tx_buf[3] = mode;
...
static L7_RC_t hpcBrcmPoeSystemFwVersion(SYSAPI_POE_MSG_t *msg)
{
  ...
  osapiStrncpySafe(msg->cmdData.mainFwVersion.version, versionString,sizeof(versionString));
  return L7_SUCCESS;
}
...
/*******************************************************************************
*
* @purpose Sets the detection mode for a System
*
* @param   DAPI_USP_t *usp    - needs to be a valid usp
...
*
*******************************************************************************/
static L7_RC_t hpcBrcmPoeSystemDetectionMode(SYSAPI_POE_MSG_t *msg)
{
  L7_uchar8             mode;
  L7_uint32             cardIndex;
  L7_RC_t               result = L7_SUCCESS;
...
static L7_RC_t hpcBrcmPoeSystemUsageThreshold(SYSAPI_POE_MSG_t *msg)
{
  ...
  for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
  {
    ...
    /*
     disable delivering ports before changing the threshold
    */
    start_port = cardData->start_phy_port;
    max_port = start_port + cardData->numOfPorts;
    ...
    for (i = start_port; i < max_port; i++)
    {
      if (poeGlobalStatus_g.poePortStatus[i].detectionStatus != L7_POE_PORT_DETECTION_STATUS_DISABLED )
      {
        ...
        hpcBrcmPoeMsgPortOnOffSet(cardIndex,poe_channel,PORT_POE_OFF);
      }
    }
    for (powerBank = 0; powerBank < num_banks; powerBank++)
    {
      guard_band = (cardData->power_bank_map[powerBank].maxPower * (100 - poeGlobalStatus_g.usageThreshold)) / 10;
      result = hpcBrcmPoeMsgPowerBankConfigSet(cardIndex, powerBank, (cardData->power_bank_map[powerBank].maxPower*10), guard_band);
    ...
    /*
     enable the delivering ports again.
     poeGlobalStatus_g.poePortStatus can be modified by monitor task.
     but here we need not worry as we have the taken the semaphore.
    */
    for (i = start_port; i < max_port; i++)
    {
      if (poeGlobalStatus_g.poePortStatus[i].detectionStatus != L7_POE_PORT_DETECTION_STATUS_DISABLED )
      {
        ...
        hpcBrcmPoeMsgPortOnOffSet(cardIndex,poe_channel,PORT_POE_ON);
      }
    }
  }
  hpcBrcmPoeReleaseAccess();
...
#ifdef CODE_NOT_USED
L7_RC_t hpcBrcmPoeSystemPowerSourceUpdate(SYSAPI_POE_MSG_t *msg)
{
  L7_RC_t               result  = L7_SUCCESS;
  L7_uint32             cardIndex;
  HPC_POE_CARD_DATA_t  *cardData;
  ...
  for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
  {
    ...
    powerBank = msg->cmdData.mainPowerBankSet.powerBank;
    powerValue = msg->cmdData.mainPowerBankSet.powerValue;

    if (powerBank >= num_banks)
    {
      L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_POE_COMPONENT_ID, "Power bank to be configured 0x%x is out of range is 0x%x", num_banks,powerBank);
      return L7_FAILURE;
    }

    guard_band = (powerValue * (100 - poeGlobalStatus_g.usageThreshold)) / 10;
    if (hpcBrcmPoeMsgPowerBankConfigSet(cardIndex, powerBank,(powerValue*10), guard_band)
        != L7_SUCCESS)
    {
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_POE_COMPONENT_ID, "Unable to set POE Power bank %d", powerBank);
        continue;
    }
    cardData->power_bank_map[powerBank].maxPower = powerValue;
  }
...
static L7_RC_t hpcBrcmPoePortSwitchToATMode(SYSAPI_POE_MSG_t *msg)
{
  L7_uchar8             mode;
  L7_uint32             cardIndex;
  L7_RC_t               result = L7_SUCCESS;
  ...
  if (L7_ALL_INTERFACES == msg->usp.port)
  {
    ...
    for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
    {
      rv = hpcBrcmPoeMsgPortSwitchTo803AT(cardIndex, POE_CHANNEL_ALL, mode);
      if (L7_POE_OK(rv) != L7_TRUE)
      {
          result = L7_FAILURE;
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_POE_COMPONENT_ID,
  ...
  else
  {
    ...
    rv = hpcBrcmPoeMsgPortSwitchTo803AT(cardIndex, poe_channel, mode);
...
static L7_RC_t hpcBrcmPoePortStatus(SYSAPI_POE_MSG_t *msg)
{
  ...
  msg->cmdData.portOperationalStatuses.powerConsumed   = BCM5910X_PORT_POWER(poeGlobalStatus_g.poePortStatus[port].powerConsumed);
...
static L7_RC_t hpcBrcmPoePortPowerPairs(SYSAPI_POE_MSG_t *msg)
{
  ...
  {
    msg->cmdData.portPowerPairs.capable = L7_TRUE;
...
/*******************************************************************************
*
* @purpose Clear Counters for the POE port
*
* @param   DAPI_USP_t *usp    - needs to be a valid usp
* @param   DAPI_CMD_t  cmd    - DAPI_CMD_POE_PORT_CLEAR_COUNTERS
* @param   DAPI_t     *dapi_g - the driver object
*
* @returns L7_RC_t result
*
...
*
*******************************************************************************/
static L7_RC_t hpcBrcmPoePortClearCounters(SYSAPI_POE_MSG_t *msg)
{
  L7_uint32             cardIndex;
  HPC_POE_CARD_DATA_t *cardData;
  L7_uint32   start_port;
  ...
  else
  {
    ...
    /* Clear Counters */
    poeGlobalStatus_g.poePortStatistics[msg->usp.port].mpsAbsent         = 0;
    poeGlobalStatus_g.poePortStatistics[msg->usp.port].overLoad          = 0;
    poeGlobalStatus_g.poePortStatistics[msg->usp.port].shortCounter      = 0;
    poeGlobalStatus_g.poePortStatistics[msg->usp.port].powerDenied       = 0;
    poeGlobalStatus_g.poePortStatistics[msg->usp.port].invalidSignature  = 0;
...
/*******************************************************************************
*
* @purpose Sets the classification type for a POE port
*
* @param   DAPI_USP_t *usp    - needs to be a valid usp
* @param   DAPI_CMD_t  cmd    - DAPI_CMD_POE_PORT_CLASSIFICATION_TYPE_SET
* @param   void       *data   - DAPI_POE_CMD_t.cmdData.poePortClassificationTypeSet
* @param   DAPI_t     *dapi_g - the driver object
*
* @returns L7_RC_t result
*
...
*
*******************************************************************************/
static L7_RC_t hpcBrcmPoePortClassificationTypeSet(SYSAPI_POE_MSG_t *msg)
{
  L7_uchar8             mode;
  L7_uint32             cardIndex;
  L7_RC_t               result = L7_SUCCESS;
  ...
  mode = msg->cmdData.portClassificationTypeSet.mode;
  ...
  if (L7_ALL_INTERFACES == msg->usp.port)
  {
    ...
    for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
    {
      rv = hpcBrcmPoeMsgPortClassificationTypeSet(cardIndex, POE_CHANNEL_ALL, mode);
      if (L7_POE_OK(rv) != L7_TRUE)
      {
          result = L7_FAILURE;
          L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_POE_COMPONENT_ID,
  ...
  else
  {
    ...
    rv = hpcBrcmPoeMsgPortClassificationTypeSet(cardIndex, poe_channel, mode);
 ...
 *
 *********************************************************************/
static L7_RC_t hpcBrcmPoeSystemDownloadProgressIndicate(SYSAPI_POE_MSG_t *msg)
{
  L7_uint32                  cardIndex;
...
#define HIGH_BYTE_IN_WORD(x) (((x)>>8) & BYTE_MASK)
#define LOW_BYTE_IN_WORD(x) ((x) & BYTE_MASK)
#define POE_DOWNLOAD_INTER_PKT_DELAY   40
...
static L7_RC_t hpcBrcmPoeFirmwareUpgradeTask(L7_uint32 args, L7_uint32 *argv)
{
  ...
  L7_int32  rc;
  L7_uchar8   sw_ver, mode_pins, port_map,temp;
  L7_uchar8   eeprom, config;
  L7_ushort16 hw_ver;

  while (offset < sizeof(Brcm_PoE_firmware))
  {
    outBuf[0] = POE_EEPROM_CONFIG_CMD;
    outBuf[1] = HIGH_BYTE_IN_WORD(offset);
    outBuf[2] = LOW_BYTE_IN_WORD(offset);

    for (count = 3; count < POE_CHKSM_FIELD; count++,offset++)
    {
      outBuf[count] = Brcm_PoE_firmware[offset];
    }
    ...
    rc = hpcBrcmPoeMsgCustomDataSend(cardIndex, outBuf);
    if (L7_POE_OK(rc) != L7_TRUE)
    {
      hpcBrcmPoeReleaseAccess();
      poeFWDownloadInProgress[cardIndex] = L7_FALSE;
      return L7_FAILURE;
    ...
    osapiSleepMSec(POE_DOWNLOAD_INTER_PKT_DELAY);
  }
  ...
  rc = hpcBrcmPoeMsgResetCard(cardIndex);
    if (L7_POE_OK(rc) != L7_TRUE)
    {
        hpcBrcmPoeReleaseAccess();
        poeFWDownloadInProgress[cardIndex] = L7_FALSE;
  ...
  osapiSleepMSec(30*POE_DOWNLOAD_INTER_PKT_DELAY);
  if (hpcBrcmPoeGetAccess(POE_CONTROLLER_ACCESS_TIMEOUT) != L7_SUCCESS)
  {
    poeFWDownloadInProgress[cardIndex] = L7_FALSE;
    return L7_FAILURE;
  ...
  rc = hpcBrcmPoeMsgSystemStatusGet(cardIndex,
            &mode_pins, &temp, &port_map, &hw_ver, &sw_ver, &eeprom, &config);

  if (L7_POE_OK(rc) != L7_TRUE)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_POE_COMPONENT_ID, "Unable to Download Firmware to PoE Card %u", cardIndex);
    hpcBrcmPoeReleaseAccess();
    poeFWDownloadInProgress[cardIndex] = L7_FALSE;
    return L7_FAILURE;
  }
  ...
  {
    POE_HPC_DPRINTF("card: %d mode_pins: 0x%0x poe_ports: %d port_map: %d  \
                       hw_ver: 0x%0x sw_ver: 0x%0x eeprom: %d  config: %d", \
                       cardIndex, mode_pins, temp,  \
                       port_map, hw_ver, sw_ver, eeprom, config);
  }
  sprintf(versionString, "%d.%d",  (sw_ver>>4),(sw_ver & 0x0f));

  poeFWDownloadInProgress[cardIndex] = L7_FALSE;

  if (poeMonitorTaskId == 0)
  {
    hpcBrcmPoeInitPhase2(cardIndex);
  }
  return L7_SUCCESS;
}
...
#define POE_DOWNLOAD_HDR_BYTES_IN_PKT  (POE_DOWNLOAD_MAX_OUT_BUF_SIZE-POE_DOWNLOAD_DATA_BYTES_IN_PKT)
#define POE_DOWNLOAD_MAX_IN_BUF_SIZE   BSC_MSG_LEN
#define POE_DOWNLOAD_MAX_PKT_COUNT     504
#define POE_DOWNLOAD_RESET_TIMEOUT     4000

  L7_uint32 offset = L7_NULL;
  L7_uint32 pktCount = L7_NULL;
  L7_uchar8 outBuffer[POE_DOWNLOAD_MAX_OUT_BUF_SIZE];
  L7_uchar8 rxBuff[POE_DOWNLOAD_MAX_IN_BUF_SIZE];
  L7_uint32 cardIndex = argv[0];
  L7_int32  rc;
  L7_uchar8   sw_ver, mode_pins, port_map,temp;
  L7_uchar8   eeprom, config;
  L7_ushort16 hw_ver;
  for (pktCount=L7_NULL; pktCount<POE_DOWNLOAD_MAX_PKT_COUNT; pktCount++)
  {
    outBuffer[0] = POE_EEPROM_CONFIG_CMD;
    outBuffer[1] = POE_EEPROM_IMAGE_DOWNLOAD_SUB_CMD;

    offset = pktCount * POE_DOWNLOAD_DATA_BYTES_IN_PKT;
    memcpy(&outBuffer[POE_DOWNLOAD_HDR_BYTES_IN_PKT],&Brcm_PoE_firmware[offset],POE_DOWNLOAD_DATA_BYTES_IN_PKT);

    offset = osapiHtons(offset);
    outBuffer[2] = HIGH_BYTE_IN_WORD(offset);
    outBuffer[3] = LOW_BYTE_IN_WORD(offset);
    if (hpcBrcmPoeGetAccess(POE_CONTROLLER_ACCESS_TIMEOUT) != L7_SUCCESS)
        {
      poeFWDownloadInProgress[cardIndex] = L7_FALSE;
            return L7_FAILURE;
    ...
    rc = hpcBrcmPoeSendMsgRaw(cardIndex,(L7_uchar8 *)outBuffer,POE_DOWNLOAD_MAX_OUT_BUF_SIZE);
    if (rc != POE_E_NONE)
    {
      hpcBrcmPoeReleaseAccess();
      poeFWDownloadInProgress[cardIndex] = L7_FALSE;
    ...
    memset(&rxBuff[0],BYTE_MASK,POE_DOWNLOAD_MAX_IN_BUF_SIZE);
    hpcBrcmPoeReceiveMsg(cardIndex,rxBuff);

    hpcBrcmPoeReleaseAccess();
    osapiSleepMSec(POE_DOWNLOAD_INTER_PKT_DELAY);
  }

  memset(outBuffer,L7_NULL,POE_DOWNLOAD_MAX_OUT_BUF_SIZE);
  outBuffer[0] = POE_EEPROM_CONFIG_CMD;
  outBuffer[1] = POE_EEPROM_IMAGE_FORCE_CRC_SUB_CMD;
  ...
  rc = hpcBrcmPoeSendMsgRaw(cardIndex,&outBuffer[0],POE_DOWNLOAD_MAX_OUT_BUF_SIZE);
  if (rc != POE_E_NONE)
  {
    hpcBrcmPoeReleaseAccess();
    poeFWDownloadInProgress[cardIndex] = L7_FALSE;
  ...
  osapiSleepMSec(POE_DOWNLOAD_RESET_TIMEOUT);
...
/*********************************************************************
 *
 * @purpose Initiate a firmware upgrade via a task creation.
 *
 * @param   void
 *
 * @returns L7_RC_t result
 ...
 *
 *********************************************************************/
L7_RC_t hpcBrcmPoeFirmwareUpgradeBegin(L7_uint32 cardIndex)
{
  L7_uchar8 fw_upgrd_task_name[80];
  ...
  sprintf(fw_upgrd_task_name, "PoeFirmwareUpgrade.%d", cardIndex);
 ...
 *
 *********************************************************************/
L7_RC_t hpcBrcmPoeFirmwareDownloadBegin(L7_uint32 cardIndex)
{
  ...
  sprintf(fw_dwnld_task_name, "PoeFirmwareUpgrade.%d", cardIndex);
  card_id[cardIndex] = cardIndex;
    ...
    if (hpcPoeMonitorEnabled == L7_TRUE)
    {
      for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
      {
          ...
          result = hpcBrcmPoeMsgPowerSourceTotalPowerGet(cardIndex,
                                                &allocPower, &availPower, &mpsmStatus,&ilim_status);
          ...
          for (i = start_port; i < max_port; i++)
          {
              ...
              if (L7_POE_OK(rv) == L7_TRUE)
              {
                  ...
                  {
                    /*
                     brcm PoE controller returns 0 for class 0 and so on But for Fastpath class 0 is 1 and so on
                    */
                      poeGlobalStatus_g.poePortStatus[i].powerClass = class_info + 1;
                  }
                  else
                  {
                      poeGlobalStatus_g.poePortStatus[i].powerClass = L7_POE_PORT_POWER_INVALID;
              ...
              if (L7_POE_OK(rv) == L7_TRUE)
              {
                  ...
                  if(L7_POE_OK(rv) != L7_TRUE)
                  {
                      L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_POE_COMPONENT_ID,
                              "POE Port statistics reset failed for port %d\n", i);
                  }
              }
              else
              {
                  L7_LOGF(L7_LOG_SEVERITY_NOTICE, L7_POE_COMPONENT_ID,
                          "POE Port statistics get failed for port %d\n", i);
              }
              hpcBrcmPoeReleaseAccess();
              osapiSleepMSec(50);
...
static L7_RC_t hpcBrcmPoeInit(SYSAPI_POE_MSG_t *const msg)
{
  ...
  L7_int32     result;
  L7_ushort16 hw_ver;
  L7_uchar8   sw_ver, mode_pins, port_map,temp,eeprom, config;
  L7_uint32   cardIndex = 0;
  L7_BOOL     cardInitialized  = L7_FALSE;
  L7_BOOL     fwUpdateRequired = L7_FALSE;
  L7_BOOL     fwDownloadRequired = L7_FALSE;
  ...
  for (cardIndex = 0; cardIndex < hpcPoeCardCountGet(); cardIndex++)
  {
    ...
    if (cardData->poe_fd < 0)
    {
      sysapiPrintf("FD for CardIndex(%d) is not VALID\n",cardIndex);
      continue;
    }
    ...
    {
#ifdef PLAT_POE_FW_UPDATE
       if(result == POE_E_CODE)
       {
         fwDownloadRequired = L7_TRUE;
       }
    }
    else
    {
      if (BRCM_POE_FIRMWARE_VERSION  != sw_ver)
      {
        fwUpdateRequired = L7_TRUE;
      }
    }

    if ((L7_TRUE == fwUpdateRequired) || (L7_TRUE == fwDownloadRequired))
    {
      if (sizeof(Brcm_PoE_firmware) != 0)
      {
        if(L7_TRUE == fwUpdateRequired)
        {
           POE_HPC_DPRINTF("\n%s: PoE controller requires Upgrading - Proceeding Firmware Upgrade.", __FUNCTION__);
           hpcBrcmPoeFirmwareUpgradeBegin(cardIndex);
        }
        else
        {
           POE_HPC_DPRINTF("\n%s: PoE controller requires programming - Proceeding Firmware Download.", __FUNCTION__);
           hpcBrcmPoeFirmwareDownloadBegin(cardIndex);
        }
      }
      else
#endif
    ...
    else                              /* This will be done after firmware download. */
    {
      if (brcmPoeDebugEnabled)
      {
        POE_HPC_DPRINTF("card: %d mode_pins: 0x%0x poe_ports: %d port_map: %d  \
                           hw_ver: 0x%0x sw_ver: 0x%0x eeprom: %d  config: %d", \
                           cardIndex, mode_pins, cardData->numOfPorts,          \
                           port_map, hw_ver, sw_ver, eeprom, config);
      }
      sprintf(versionString, "%d.%d",  (sw_ver>>4),(sw_ver & 0x0f));
      ...
      {
        L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_POE_COMPONENT_ID, "hpcBrcmPoeInitPhase2 failed.");
        return L7_FAILURE;
      }
    }
  ...
  poeMsgHandlers[SYSAPI_HPC_POE_MAIN_DETECTION_MODE]            = hpcBrcmPoeSystemDetectionMode;
  ...
  poeMsgHandlers[SYSAPI_HPC_POE_PORT_SWITCH_TO_803AT_MODE]      = hpcBrcmPoePortSwitchToATMode;
#ifdef PLAT_POE_FW_UPDATE
  poeMsgHandlers[SYSAPI_HPC_POE_FIRMWARE_DOWNLOAD_PROGRESS]     = hpcBrcmPoeSystemDownloadProgressIndicate;
#endif
#if 0
  poeMsgHandlers[SYSAPI_HPC_POE_PORT_CLASSIFICATION_TYPE]       = hpcBrcmPoePortClassificationTypeSet;
#endif
  return L7_SUCCESS;
}
...
void hpcBrcmDebugPoeDriver(L7_uchar8 psePort, int cmd, L7_uchar8 val1, int val2)
{
  ...
  switch(cmd)
  {
    ...
    case 3: /* To get the system Power Parameters */
      rc = hpcBrcmPoeMsgPowerSourceTotalPowerGet(cardIndex, &sh1, &sh2, &ch1,&ilim);
      if(rc == L7_SUCCESS)
      {
        sysapiPrintf("hapiBroadDebugPoeDriver: power_consumed: 0x%0x  power_avilable: 0x%0x MPSM: 0x%x ILim=0x%x\n", sh1, sh2,ch1,ilim);
      }
      break;
  ...
  L7_uchar8   poe_channel = 0;
    ...
    for (channel = start_port; channel < max_port; channel++)
    {
          ...
          || (poe_port == POE_CHANNEL_ALL))
      {
          ...
          {
              printf("Unable to get the PoE Logical Port for port %u", channel);
          }

          if (hpcBrcmPoeGetAccess(L7_WAIT_FOREVER) != L7_SUCCESS)
          {
              printf( "Unable to gain access to the controller.");
              return L7_FAILURE;
          }
          ...
          {
              printf("Cannot get Single port status for POE port %02d", channel);
          }
      }
    }
    ...
    for (channel = start_port; channel < max_port; channel++)
    {
          ...
          || (poe_port == POE_CHANNEL_ALL))
      {
          ...
          {
              printf("Cannot get Single port statistics for POE port %02d", channel);
          }
      }
    }
    ...
    rv = hpcBrcmPoeMsgPowerSourceTotalPowerGet(cardIndex,
                                                &allocPower, &availPower, &mpsmStatus,&ilimStatus);
    ...
    for (channel = start_port; channel < max_port; channel++)
    {
          ...
          || (poe_port == POE_CHANNEL_ALL))
      {
          ...
          {
              printf( "Cannot get Single port Configuration for POE port %02d", channel);
          }
      }
    }
    ...
    for (channel = start_port; channel < max_port; channel++)
    {
          ...
          || (poe_port == POE_CHANNEL_ALL))
      {
          ...
          {
              printf( "Cannot get Single port Extended Configuration for POE port %02d", channel);
          }
      }
    }
    ...
    for (channel = start_port; channel < max_port; channel++)
    {
          ...
          || (poe_port == POE_CHANNEL_ALL))
      {
          ...
          {
              printf( "Cannot get Single port Measurements for POE port %02d", channel);
          }
      }
    }
