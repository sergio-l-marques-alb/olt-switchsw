...
L7_RC_t hapiBroadPtinMEPCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g) {
  ...
  PT_LOG_TRACE(LOG_CTX_API, "Success %d \n\r", r);
  if (!alrdy) {
      PT_LOG_TRACE(LOG_CTX_API, "r = %d empty %d %d\n\r", r, empty);
  ...
  PT_LOG_TRACE(LOG_CTX_API, "Success\n\r", r);
