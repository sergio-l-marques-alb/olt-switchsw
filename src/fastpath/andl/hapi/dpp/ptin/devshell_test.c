...
int port_match_add(int port, int vlan, int vlan_port_id)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, bcm_port_match_add: rv=%d\n", rv);
  return rv;
 }
...
int port_match_remove(int port, int vlan, int vlan_port_id)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, bcm_port_match_delete: rv=%d\n", rv);
  return rv;
 }
...
int vlan_add(int vlan, int port)
{
 ...
 if (port >= 1)
 {
  ...
  if (rv != BCM_E_NONE && rv != BCM_E_EXISTS)
  {
   printf("error: bcm_vlan_port_add failed: rv=%d\n", rv);
   return rv;
  }
  if (rv == BCM_E_NONE)
  {
...
int vlan_remove(int vlan, int port)
{
 ...
 if (port >= 1)
 {
  ...
  if (rv != BCM_E_NONE)
  {
   printf("error: bcm_vlan_port_remove failed: rv=%d\n", rv);
   return rv;
  }
 ...
 if (rv != BCM_E_NONE)
 {
  printf("error: bcm_vlan_port_get failed: rv=%d\n", rv);
  return rv;
 }
...
int vswitch_create(int vsi)
{
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS)
 {
  printf("error: bcm_vswitch_create_with_id: rv=%d\n", rv);
  return rv;
 }
...
int vswitch_destroy(int vsi)
{
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS)
 {
  printf("error: bcm_vswitch_destroy: rv=%d\n", rv);
  return rv;
 }
...
int vswitch_remove(int vsi, int vlan_port_id)
{
 ...
 if (rv != BCM_E_NONE && rv != BCM_E_EXISTS) {
  printf("Error, bcm_vswitch_port_delete: rv=%d\n", rv);
  return rv;
 }
...
int multicast_destroy(int mc_group)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, bcm_multicast_destroy: rv=%d\n", rv);
  return rv;
 }
...
int multicast_vlan_add(int mc_group, int port, int vlan_port_id)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, in bcm_multicast_vlan_encap_get mc_group_id:0x%08x  phy_port:0x%08x  gport:0x%08x : rv=%d\n", mc_group, port, vlan_port_id, rv);
  return rv;
 }

 rv = bcm_multicast_egress_add(unit, mc_group, port, encap_id);
 if (rv != BCM_E_NONE) {
  printf("Error, in bcm_multicast_egress_add mc_group_id:0x%08x  phy_port:0x%08x  encap_id:0x%08x : rv=%d\n", mc_group, port, encap_id, rv);
  return rv;
 }
...
int multicast_vlan_remove(int mc_group, int port, int vlan_port_id)
{
 ...
 if (rv != BCM_E_NONE) {
  printf("Error, in bcm_multicast_egress_delete mc_group_id:0x%08x  phy_port:0x%08x  encap_id:0x%08x : rv=%d\n", mc_group, port, encap_id, rv);
  return rv;
 }
