...
{  
  ...
  if (BCM_FAILURE(rv))
  {
    PT_LOG_ERR(LOG_CTX_HAPI,"Error adding Channel to IPMC Table: group_index:0x%x rv:%d rv=\"%s\"", bcm_ipmc.group, rv, bcm_errmsg(rv));
    return L7_FAILURE;
  }
...
//bcm_ipmc.group = 0;
  ...
  if (BCM_FAILURE(rv))
  {
    PT_LOG_ERR(LOG_CTX_HAPI,"Error removing Channel from IPMC Table: rv=\"%s\" ipmc_index:0x%x", bcm_errmsg(rv), ptin_ipmc->group_index);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_hapi_l3_intf_create (ptin_dtl_l3_intf_t *intf)
{
  ...
  if ( (intf->flags & PTIN_BCM_L3_WITH_ID) == PTIN_BCM_L3_WITH_ID)
  {
    ...
    if (rv != L7_SUCCESS)
    {
      PT_LOG_ERR(LOG_CTX_HAPI,"Failed to set L3 Interface Id %d", intf->l3_intf_id);
      return L7_FAILURE;
    }
    #endif
  }
  ...
  {    
    ...
    {
      PT_LOG_ERR(LOG_CTX_HAPI,"Failed to pop L3 Interface Id %d (rv=%d) rv=\"%s\"", intf->l3_intf_id, rv, bcm_errmsg(rv));
      return L7_FAILURE;
    }       
  }
  ...
  {
    PT_LOG_ERR(LOG_CTX_HAPI,"Error creating L3 interface: rv=\"%s\" l3_intf_id:%d", bcm_errmsg(rv), bcm_data.l3a_intf_id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_hapi_l3_intf_delete (ptin_dtl_l3_intf_t *intf)
{
  ...
  if (rv != L7_SUCCESS)
  {
    PT_LOG_ERR(LOG_CTX_HAPI,"Failed to push L3 Interface Id %d (rv=%d) rv=\"%s\"", intf->l3_intf_id, rv, bcm_errmsg(rv));
    return L7_FAILURE;
  }
