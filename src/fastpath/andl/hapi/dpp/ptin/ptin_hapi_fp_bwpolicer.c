...
L7_RC_t hapi_ptin_bwPolicer_set(DAPI_USP_t *usp, ptin_bwPolicer_t *bwPolicer, DAPI_t *dapi_g)
{
  ...
  if (stage == BROAD_POLICY_STAGE_INGRESS)
  {
    ...
    if (usp->unit >= 0 && usp->slot >= 0 && usp->port >= 0)
    {
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"trunk_id=%d bcm_port=%d efp_class_port=%d xlate_class_port=%d",
  ...
  else if (stage == BROAD_POLICY_STAGE_EGRESS)
  {
    ...
    if (usp->unit >= 0 && usp->slot >= 0 && usp->port >= 0)
    {
      ...
      if (portDescriptor.bcm_port >= 0)
      {
        ...
        {
          hapiBroadPolicyCreateCancel();
          PT_LOG_ERR(LOG_CTX_HAPI,"Error with hapiBroadPolicyRuleQualifierAdd(OUTPORT)");
          return result;
        }
        PT_LOG_TRACE(LOG_CTX_HAPI,"OutPort qualifier added");
      }
      /* Class port */
      else if (portDescriptor.efp_class_port > 0)
      {
        ...
        {
          hapiBroadPolicyCreateCancel();
          PT_LOG_ERR(LOG_CTX_HAPI,"Error with hapiBroadPolicyRuleQualifierAdd(PORTCLASS)");
          return result;
        }
        PT_LOG_TRACE(LOG_CTX_HAPI,"Port class qualifier added");
      }
    }
  ...
  if (profile->cos < L7_COS_INTF_QUEUE_MAX_COUNT)
  {
    ...
    PT_LOG_TRACE(LOG_CTX_HAPI,"OCOS (class_id=%u) qualifier added", class_id);
  }
  #else
  if (profile->cos < L7_COS_INTF_QUEUE_MAX_COUNT &&
