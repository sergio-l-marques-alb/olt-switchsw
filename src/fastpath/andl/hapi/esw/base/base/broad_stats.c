...
void hapiBroadStatCpuUpdate(DAPI_USP_t *usp, DAPI_t *dapi_g);
  ...
  else if (dapiCmd->cmdData.statistics.getOrSet == DAPI_CMD_GET)
  {
    if (dapiPortPtr->statsEnable == L7_TRUE)
    {
      ...
      else if (IS_PORT_TYPE_CPU(dapiPortPtr) == L7_TRUE)
      {
        ...
        hapiBroadStatCpuUpdate(usp, dapi_g);

        osapiSemaTake(dapiPortPtr->modeparm.cpu.statsSema, L7_WAIT_FOREVER);
        ...
        osapiSemaGive(dapiPortPtr->modeparm.cpu.statsSema);
      }
      else
      {
        result = L7_FAILURE;
...
void hapiBroadStatCpuL3Update(DAPI_USP_t *usp, DAPI_INTF_STATS_t hapiCounterId, DAPI_t *dapi_g)
{
  ...
  osapiSemaTake(cpuDapiPortPtr->modeparm.cpu.statsSema, L7_WAIT_FOREVER);
  hapiBroadStatsConvert(&stats_p[hapiCounterId], totalBcmStat);
  osapiSemaGive(cpuDapiPortPtr->modeparm.cpu.statsSema);
...
{
  int                    rc;
  HPC_HELPER_STATS_t     helperStatId;
  DAPI_PORT_t           *dapiPortPtr;
  L7_ulong64            *stats_p;
  L7_ulong64             helperStatDeltaValue;
  L7_uint32              i;
  DAPI_INTF_STATS_t      dapiStats[] = {DAPI_STATS_TRANSMITTED_BYTES,
  ...
  for (i = 0; i < (sizeof(dapiStats) / sizeof(DAPI_INTF_STATS_t)); i++)
  {
    switch (dapiStats[i])
    {
    case DAPI_STATS_TRANSMITTED_BYTES:
      helperStatId = HPC_HELPER_STATS_TX_BYTES;
      break;

    case DAPI_STATS_TRANSMITTED_64_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_64_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_65_TO_127_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_65_TO_127_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_128_TO_255_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_128_TO_255_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_256_TO_511_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_256_TO_511_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_512_TO_1023_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_512_TO_1023_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_1024_TO_1518_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_1024_TO_1518_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_1519_TO_1530_BYTE_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_1519_TO_1530_BYTE_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_UNICAST_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_UNICAST_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_MULTICAST_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_MULTICAST_FRAMES;
      break;

    case DAPI_STATS_TRANSMITTED_BROADCAST_FRAMES:
      helperStatId = HPC_HELPER_STATS_TX_BROADCAST_FRAMES;
      break;
    ...
    rc = hpcHardwareHelperStatDeltaGet(helperStatId, &helperStatDeltaValue);
    if (rc == BCM_E_NONE)
    {
      osapiSemaTake(dapiPortPtr->modeparm.cpu.statsSema, L7_WAIT_FOREVER);
      /* Check for carry. */
      if ((stats_p[dapiStats[i]].low + helperStatDeltaValue.low) < stats_p[dapiStats[i]].low)
      {
        stats_p[dapiStats[i]].high++;
      }
      stats_p[dapiStats[i]].low  += helperStatDeltaValue.low;
      stats_p[dapiStats[i]].high += helperStatDeltaValue.high;
      osapiSemaGive(dapiPortPtr->modeparm.cpu.statsSema);
    }
  }
...
void hapiBroadStatPortUpdate(DAPI_USP_t *usp, DAPI_INTF_STATS_t hapiCounterId, DAPI_t *dapi_g)
{
  ...
  case DAPI_STATS_RECEIVED_GOOD_OVERSIZED_FRAMES:
    bcmStatId = snmpEtherRxOversizePkts;
    break;
  ...
  case DAPI_STATS_TRANSMITTED_OVERSIZED_FRAMES:
    bcmStatId = snmpEtherTxOversizePkts;
    break;
