                                             ...
                                             DAPI_t *dapi_g)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI, "broadcastControl.type=%u broadcastControl.enable=%u", dapiCmd->cmdData.broadcastControl.type, dapiCmd->cmdData.broadcastControl.enable);
  LOG_TRACE(LOG_CTX_HAPI, "hapiBroadIntfBroadcastControlModeSet with limit=%llu bucket_size=%u units=%u for bcmx_lport=0x%x",
  ...
  case STORMCONTROL_BCAST:
      ...
      LOG_TRACE(LOG_CTX_HAPI, "usl_bcmx_rate_bcast_set with limit=%u bucket_size=%u flags=0x%x for bcmx_lport=0x%x",
      ...
      LOG_TRACE(LOG_CTX_HAPI, "rv=%d", rv);
      if (L7_BCMX_OK(rv) != L7_TRUE)
      {
        LOG_ERROR (rv);
      }
  ...
  case STORMCONTROL_MCAST:
      ...
      LOG_TRACE(LOG_CTX_HAPI, "usl_bcmx_rate_mcast_set with limit=%u bucket_size=%u flags=0x%x for bcmx_lport=0x%x",
                rateLimit.limit, rateLimit.bucket_size, rateLimit.flags, hapiPortPtr->bcmx_lport);
  ...
  case STORMCONTROL_UCAST:
      ...
      LOG_TRACE(LOG_CTX_HAPI, "usl_bcmx_rate_dlfbc_set with limit=%u bucket_size=%u flags=0x%x for bcmx_lport=0x%x",
                rateLimit.limit, rateLimit.bucket_size, rateLimit.flags, hapiPortPtr->bcmx_lport);
...
L7_RC_t hapiBroadIntfIsolatePhyConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (enable && hapiPortPtr->bcm_port <= 49)
  {
    ...
    if (L7_BCMX_OK(rv) != L7_TRUE)
    {
      LOG_ERR(LOG_CTX_HAPI, "Error setting 1G speed to bcm_port %u", hapiPortPtr->bcm_port);
    }
    else
    {
      LOG_TRACE(LOG_CTX_HAPI, "1G speed to bcm_port %u", hapiPortPtr->bcm_port);
    }
  }
#endif
...
L7_RC_t hapiBroadSystemPacketTrapConfig(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  switch (dapiCmd->cmdData.snoopConfig.getOrSet)  {
    ...
    case DAPI_CMD_CLEAR:
      ...
      break;
  default:
      LOG_ERR(LOG_CTX_HAPI,"Invalid command type %u", dapiCmd->cmdData.snoopConfig.getOrSet);
      return L7_FAILURE;
  }
  ...
  {
    LOG_ERR(LOG_CTX_HAPI,"Error: %d", status);
  }
...
L7_RC_t hapiBroadConfigTrap(DAPI_USP_t *usp, cmdData_snoopConfig_t *snoopConfig, L7_BOOL switchFrame, DAPI_t *dapi_g)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting igmp trapping processing");
  ...
  {
    LOG_ERR(LOG_CTX_HAPI, "Invalid Vlan (%u) or packet type (%u) or ip_version (%u)", snoopConfig->vlanId, snoopConfig->packet_type, snoopConfig->family);
    return L7_FAILURE;
  }

  LOG_TRACE(LOG_CTX_HAPI,"Original vlan = %u",snoopConfig->vlanId);
  snoopConfig->vlan_mask = PTIN_VLAN_MASK(snoopConfig->vlanId);
  snoopConfig->vlanId &= snoopConfig->vlan_mask;
  LOG_TRACE(LOG_CTX_HAPI,"vlan = %u, mask=0x%04x", snoopConfig->vlanId, snoopConfig->vlan_mask);

  /* If vlan value is valid, Find igmp index */
  if (snoopConfig->vlanId >= PTIN_VLAN_MIN && snoopConfig->vlanId <= PTIN_VLAN_MAX)
  {
    LOG_TRACE(LOG_CTX_HAPI, "Vlan provided is valid (%u). Enable=%u", snoopConfig->vlanId, snoopConfig->enable);
    ...
    {
      LOG_TRACE(LOG_CTX_HAPI, "Not found vlan %u within the configured ones", snoopConfig->vlanId);

      /* If is going to add a new vlan, use first free index */
      if (snoopConfig->enable)
      {
        LOG_TRACE(LOG_CTX_HAPI, "Going to add vlan %u to table", snoopConfig->vlanId);

        /* Check if a free index was found */
        if (search_index_free >= PTIN_TRAP_POLICY_MAX_VLANS)
        {
          LOG_ERR(LOG_CTX_HAPI, "There is no room to add vlan %u", snoopConfig->vlanId);
        ...
        LOG_TRACE(LOG_CTX_HAPI, "Vlan %u added to table in cell %u", snoopConfig->vlanId, search_index);
      }
      /* If it is to remove a vlan, and it was not found, return SUCCESS */
      else
      {
        LOG_TRACE(LOG_CTX_HAPI, "Since it is to remove vlan %u, there is nothing to be done", snoopConfig->vlanId);
        return L7_SUCCESS;
      }
    }
    else
    {
      LOG_TRACE(LOG_CTX_HAPI, "Vlan %u was found in cell %u", snoopConfig->vlanId, search_index);
      if (!snoopConfig->enable)
      {
        LOG_TRACE(LOG_CTX_HAPI, "Going to remove cell %u with VLAN %u", search_index, snoopConfig->vlanId);
      }
      else
      {
        /* This Vlan already exists... nothing to be done */
        LOG_TRACE(LOG_CTX_HAPI, "Vlan %u already exists... nothing to be done!", snoopConfig->vlanId);
        return L7_SUCCESS;
      }
    }
  }
  ...
  else
  {
    ...
    LOG_TRACE(LOG_CTX_HAPI, "Null vlan provided: Global enable is applied (enable=%u)", snoopConfig->enable);
  }
  ...
  for (index = 0; index < PTIN_TRAP_POLICY_MAX_VLANS; index++)
  {
    ...
    if (ptin_trap_policy[index].policyId != BROAD_POLICY_INVALID)
    {
      ...
      LOG_TRACE(LOG_CTX_HAPI, "Policy of cell %u deleted", index);
    }
    ...
    LOG_TRACE(LOG_CTX_HAPI, "Processing cell #%u", index);
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished igmp trapping processing (result=%d)", result);
                                ...
                                BROAD_POLICY_t *policy_id)
{
  ...
  {
    LOG_ERR(LOG_CTX_HAPI, "Invalid VLAN (%u) or policyId pointer", vlanId);
  }
  ...
  do
  {
    ...
    if (result != L7_SUCCESS)  break;
    LOG_TRACE(LOG_CTX_HAPI, "Qualifier OVID=%u/0x%04x defined",vlanId, vlan_match);
    result = hapiBroadPolicyRuleQualifierAdd(ruleId, BROAD_FIELD_ETHTYPE, (L7_uchar8 *)&ip_ethtype, exact_match);
    if (result != L7_SUCCESS)  break;
    LOG_TRACE(LOG_CTX_HAPI, "Qualifier BROAD_FIELD_ETHTYPE defined");
    result = hapiBroadPolicyRuleQualifierAdd(ruleId, BROAD_FIELD_PROTO, igmp_proto, exact_match);
    if (result != L7_SUCCESS)  break;
    LOG_TRACE(LOG_CTX_HAPI, "Qualifier BROAD_FIELD_PROTO defined");
  ...
  if (result == L7_SUCCESS)
  {
    LOG_TRACE(LOG_CTX_HAPI, "Commiting policy");

    result = hapiBroadPolicyCommit(&policyId);

    if (result == L7_SUCCESS)
      LOG_TRACE(LOG_CTX_HAPI, "policy %d commited successfully", *policy_id);
    else
      LOG_ERR(LOG_CTX_HAPI, "Error commiting policy");
  }
  ...
  if (result != L7_SUCCESS)
  {
    ...
    LOG_TRACE(LOG_CTX_HAPI, "Some error ocurred: canceling policy");
  }
  else
  {
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished igmp trapping processing");
                                ...
                                BROAD_POLICY_t *policy_id)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting dhcp trapping processing");
  ...
  do
  {
    ...
    LOG_TRACE(LOG_CTX_HAPI, "Policy of cell %u created", index);
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished dhcp trapping processing");
                                 ...
                                 BROAD_POLICY_t *policy_id)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting PPPoE trapping processing");
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished PPPoE trapping processing");
                               ...
                               BROAD_POLICY_t *policy_id)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting APS trapping processing (vlanId %d, ringId %d)", vlanId, ringId_level);
  ...
  do
  {
    ...
    LOG_TRACE(LOG_CTX_HAPI, "Policy created");
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished APS trapping processing");
...
//L7_ushort16             ipdtl0_ethtype  = L7_ETYPE_ARP;
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting dtl0 IP packets trapping processing (VLAN ID %d)", vlanId);
...
//  if (result != L7_SUCCESS)  break;
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Finished IP DTL0 trapping processing");
...
//L7_ushort16             ccm_ethtype  = L7_ETYPE_CCM;
  ...
  LOG_TRACE(LOG_CTX_HAPI, "Starting CCM trapping processing\tenable=%d\tvlanId=%u\toam_level=%u", enable, vlanId, oam_level);

  /* Initialization */
  if (first_time)
  {
    LOG_TRACE(LOG_CTX_HAPI, "First time processing... make some initializations");
    ...
    //LOG_TRACE(LOG_CTX_HAPI, "CCM trapping is already configured");
    // Since vlanId is not used in HW rule only create rule once based on DMAC Addr
    //return L7_SUCCESS;
  }

 //#if (PTIN_SYSTEM_GROUP_VLANS)
 // LOG_TRACE(LOG_CTX_HAPI,"Original vlan = %u",vlanId);
 // vlan_match = PTIN_VLAN_MASK(vlanId);
 // vlanId &= vlan_match;
 // LOG_TRACE(LOG_CTX_HAPI,"vlan = %u, mask=0x%04x",vlanId,vlan_match);
 //#endif
...

  {
    ...
    if (result == L7_SUCCESS)
    {
      LOG_TRACE(LOG_CTX_HAPI, "Commiting policy of cell %u", index);
      if ((result=hapiBroadPolicyCommit(&ccm_policy[index].policyId)) == L7_SUCCESS)
      {
        LOG_TRACE(LOG_CTX_HAPI, "policy of cell %u commited successfully", index);
      }
    }
    else
    {
      ...
      ccm_policy[index].policyId = BROAD_POLICY_INVALID;
      LOG_TRACE(LOG_CTX_HAPI, "Some error ocurred: canceling policy of cell %u", index);
    }
    ...
    if (result != L7_SUCCESS && ccm_policy[index].policyId != BROAD_POLICY_INVALID )
    {
      ...
      ccm_policy[index].policyId = BROAD_POLICY_INVALID;
      LOG_TRACE(LOG_CTX_HAPI, "Some error ocurred: deleting policy of cell %u", index);
    }
  }

  LOG_TRACE(LOG_CTX_HAPI, "Finished CCM trapping processing");
