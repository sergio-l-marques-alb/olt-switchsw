...
L7_RC_t broad_ptin_example(DAPI_USP_t *usp, DAPI_CMD_GET_SET_t operation, L7_uint32 dataSize, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "Hello World: usp={%d,%d,%d} operation=%u dataSize=%u", usp->unit, usp->slot, usp->port, operation, dataSize);

  /* Validate data pointer */
  if (example == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Structure data pointer is null");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid data size (%u VS %u)", dataSize, sizeof(ptin_dtl_example_t));
    return L7_FAILURE;
  }
  ...
  {
    case DAPI_CMD_GET:
      LOG_PT_TRACE(LOG_CTX_HAPI, "Operation is DAPI_CMD_GET");
      /* Put here function for GET procedure */
      break;

    case DAPI_CMD_SET:
      LOG_PT_TRACE(LOG_CTX_HAPI, "Operation is DAPI_CMD_SET");
      /* Put here function for SET procedure */
      break;

    case DAPI_CMD_CLEAR:
      LOG_PT_TRACE(LOG_CTX_HAPI, "Operation is DAPI_CMD_CLEAR");
      /* Put here function for CLEAR procedure */
      break;

    default:
      LOG_PT_ERR(LOG_CTX_HAPI, "Not recognized operation (%u)!", operation);
      return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "Calling ptin_hapi_example...");
...
L7_RC_t broad_ptin_l2_maclimit(DAPI_USP_t *usp, DAPI_CMD_GET_SET_t operation, L7_uint32 dataSize, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "%s: usp={%d,%d,%d} operation=%u dataSize=%u", __FUNCTION__, usp->unit, usp->slot, usp->port, operation, dataSize);
...
L7_RC_t broad_ptin_l3_intf(DAPI_USP_t *usp, DAPI_CMD_GET_SET_t operation, L7_uint32 dataSize, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "usp={%d,%d,%d} operation=%u dataSize=%u", usp->unit, usp->slot, usp->port, operation, dataSize);

  /* Validate data pointer */
  if (intf == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid Arguments: intf=%p", intf);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid data size (%u VS %u)", dataSize, sizeof(ptin_dtl_l3_intf_t));
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "Input Parameters [flags:0x08%X vid:%d mac_address:%02x:%02x:%02x:%02x:%02x:%02x l3_intf_id:%d mtu:%d]", intf->flags, intf->vid, 
...
L7_RC_t broad_ptin_l3_ipmc(DAPI_USP_t *usp, DAPI_CMD_GET_SET_t operation, L7_uint32 dataSize, void *data, DAPI_t *dapi_g)
{
  ...
  if (ipmc == L7_NULLPTR)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid Arguments: intf=%p", ipmc);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid data size (%u VS %u)", dataSize, sizeof(ptin_dtl_ipmc_addr_t));
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "Input Parameters [flags:0x08%X vid:%d group_index:0x%x sourceAddr:0x%08x groupAddr:0x%08x]", ipmc->flags, ipmc->vid, ipmc->group_index, 
...
L7_RC_t hapiBroadPtinGeneric(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_DTL, "Null pointer");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_DTL, "Invalid Message ID (%u)", ptinDtlGeneric->msgId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_DTL, "Invalid datasize (%u)", ptinDtlGeneric->dataSize);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_DTL, "Null callback for message ID %u", ptinDtlGeneric->msgId);
    return L7_FAILURE;
  }
...
L7_RC_t hapiBroadPtinDataInit(void)
{
  ...
  LOG_PT_INFO(LOG_CTX_HAPI, "PTin HAPI Applying configs...");

  /* Initialize PTIN HAPI files */
  rc = hapi_ptin_data_init();

  LOG_PT_INFO(LOG_CTX_HAPI, "PTin HAPI data structs config: %d",rc);
...
L7_RC_t hapiBroadPtinInit(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "PTin HAPI Applying configs...");
  ...
  LOG_PT_INFO(LOG_CTX_HAPI, "PTin HAPI Configuration: %d",rc);
...
L7_RC_t hapiBroadHwApply(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "PTin HAPI Configuration: procedure=%u, param1=%d, param2=%d", hwproc->procedure, hwproc->param1, hwproc->param2);
  LOG_PT_TRACE(LOG_CTX_HAPI, "usp={%d,%d,%d}",usp->unit, usp->slot, usp->port);

  /* Validate interface */
  if ( usp->unit<0 || usp->slot<0 || usp->port<0 )
  {
    LOG_PT_ERR(LOG_CTX_HAPI,"USP not provided");
    return L7_FAILURE;
  }

  if (hwproc->procedure == PTIN_HWPROC_NONE)
  {
    LOG_PT_INFO(LOG_CTX_HAPI, "Nothing to do");
    return L7_SUCCESS;
  }
  ...
  case PTIN_HWPROC_LINKSCAN:
    if (hwproc->operation == DAPI_CMD_GET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_linkscan_get: rc=%d", rc);
    ...
    else if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_linkscan_set (%u): rc=%d", hwproc->param1, rc);
    }
    else
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Operation not recognized: %u", hwproc->operation);
      rc = L7_FAILURE;
    }
    break;
  ...
  case PTIN_HWPROC_FORCE_LINK:
    ...
    if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_force_link (link=%u, enable=1): rc=%d", hwproc->param1, rc);
    }
    else if (hwproc->operation == DAPI_CMD_CLEAR)
    {
      rc = ptin_hapi_link_force(usp, dapi_g, link, L7_DISABLE);
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_force_link (link=%u, enable=0): rc=%d", hwproc->param1, rc);
    }
    else
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Operation not recognized: %u", hwproc->operation);
  ...
  case PTIN_HWPROC_CLK_RECVR:
    if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_clock_recovery_set (main_port=%d, backup_port=%d): rc=%d", main_port, bckp_port, rc);
    }
    else
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Operation not recognized: %u", hwproc->operation);
  ...
  case PTIN_HWPROC_VCAP_DEFVID:
    if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_vcap_defvid (outerVlan=%u, innerVlan=%u): rc=%d", hwproc->param1, hwproc->param2, rc);
    }
    else if (hwproc->operation == DAPI_CMD_CLEAR)
    {
      rc = ptin_hapi_vcap_defvid(usp, 0, 0, dapi_g);
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_vcap_defvid (outerVlan=0, innerVlan=0): rc=%d", rc);
    }
    else
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Operation not recognized: %u", hwproc->operation);
  ...
  case PTIN_HWPROC_UC_TTL1_CPU:
    if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_L3UcastTtl1ToCpu_set");
    }
    else if (hwproc->operation == DAPI_CMD_CLEAR)
    {
  ...
  case PTIN_HWPROC_FRAME_OVERSIZE:
    if (hwproc->operation == DAPI_CMD_GET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_frame_oversize_get: rc=%d", rc);
      else
        LOG_PT_TRACE(LOG_CTX_HAPI, "Oversize frame limite for port {%d,%d,%d} is %u bytes",
    ...
    else if (hwproc->operation == DAPI_CMD_SET)
    {
      ...
      if (rc != L7_SUCCESS)
        LOG_PT_ERR(LOG_CTX_HAPI, "Error with ptin_hapi_frame_oversize_set (frame_size=%u): rc=%d", hwproc->param1, rc);
    }
    else
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Operation not recognized: %u", hwproc->operation);
  ...
  default:
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid procedure: %u", hwproc->procedure);
    rc = L7_FAILURE;
    break;
  }
...
L7_RC_t hapiBroadPtinSlotMode(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "Slot mode operation");
...
L7_RC_t hapiBroadPtinPortExt(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (portExt->operation==DAPI_CMD_GET)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "Result for L2LearnPortSet: %d",rc);
  }
...
L7_RC_t hapiBroadPtinCountersClear(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error on hapi_ptin_counters_clear() on port# %u", port);
    return L7_FAILURE;
  }
...
L7_RC_t hapiBroadPtinVlanTranslate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "usp={%d,%d,%d}, Stage=%u, operation=%u, group=%u, oVlanId=%u, iVlanId=%u, newOVlanId=%u(%u), newIVlanId=%u(%u), rem_VLANs=%u",
  ...
  case PTIN_XLATE_STAGE_ALL:
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "All translations will be removed");
...
//      rc = L7_FAILURE;
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "Finished: rc=%d", rc);
...
L7_RC_t hapiBroadPtinVlanTranslatePortGroups(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "usp={%d,%d,%d}, Oper=%d, classId=%d",
            usp->unit, usp->slot, usp->port, classSt->oper, classSt->class_id);

  /* Validate usp */
  if (usp->unit<0 || usp->slot<0 || usp->port<0)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid usp");
    return L7_FAILURE;
  }
...
L7_RC_t hapiBroadPtinVlanModeSet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  */
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI,"Error obtaining CPU interfaces");
    return L7_FAILURE;
  }
...
L7_RC_t hapiBroadPtinBridgeVlanModeSet(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI,"vlanId=%u, mask=0x%02X, fwdVlan=%u, outerTPID=0x%04X, mac_learn=%u, crossc=%u",
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI,"Invalid vlan (%d)", mode->vlanId);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_WARN(LOG_CTX_HAPI,"Mask is empty... nothing to be done");
    return L7_SUCCESS;
  }
...
L7_RC_t hapiBroadPtinBridgeVlanPortControl(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_API, "usp_old={%d,%d,%d}, usp_new={%d,%d,%d}",
      ...
      vlan_mode->ddUsp.port >= 0 && vlan_mode->ddUsp.port < PTIN_SYSTEM_N_PORTS)
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "Previous port {%d,%d,%d}", vlan_mode->ddUsp.unit, vlan_mode->ddUsp.slot, vlan_mode->ddUsp.port);
  }
  ...
  if (vlan_mode->oper == DAPI_CMD_SET)
  {
    ...
    if (vlan_mode->vlanId == 0 || vlan_mode->vlanId > 4095)
    {
      ...
      if (hapiPortPtr_prev != L7_NULLPTR)
      {
        ...
        if (rv != BCM_E_NONE) 
        {
          LOG_PT_ERR(LOG_CTX_HAPI,"Error applying bcm_l2_replace (rv = %d)\r\n", rv);
        }
        #endif
      }
      ...
      if (hapiPortPtr_prev != L7_NULLPTR)
      {
        ...
        if (rv != BCM_E_NONE)
        {
          LOG_PT_ERR(LOG_CTX_HAPI,"Error flushing MAC entries from port {%d,%d,%d} (rv=%d)\r\n",
                  vlan_mode->ddUsp.unit, vlan_mode->ddUsp.slot, vlan_mode->ddUsp.port, rv);
        }
        #endif

        LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} switched to {%d,%d,%d}",
                  vlan_mode->ddUsp.unit, vlan_mode->ddUsp.slot, vlan_mode->ddUsp.port,
                  usp->unit, usp->slot, usp->port);
      }

      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} added to all VLANS (MACs flushed)", usp->unit, usp->slot, usp->port);
    }
    else
    {
      hapiBroadL2VlanAddPortToVlanHw(usp, vlan_mode->vlanId, L7_TRUE, dapi_g);
      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} added to VLAN %u", usp->unit, usp->slot, usp->port, vlan_mode->vlanId);
    }
  }
  else if (vlan_mode->oper == DAPI_CMD_CLEAR)
  {
    if (vlan_mode->vlanId == 0 || vlan_mode->vlanId > 4095)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} removed from all VLANS", usp->unit, usp->slot, usp->port);
    }
    else
    {
      hapiBroadL2VlanRemovePortFromVlanHw(usp, vlan_mode->vlanId, dapi_g);
      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} removed from VLAN %u", usp->unit, usp->slot, usp->port, vlan_mode->vlanId);
    }
  }
  else if (vlan_mode->oper == DAPI_CMD_CLEAR_ALL)
  {
    if (vlan_mode->vlanId == 0 || vlan_mode->vlanId > 4095)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} removed from all VLANS (MACs flushed)", usp->unit, usp->slot, usp->port);
    }
    else
    {
      hapiBroadL2VlanRemovePortFromVlanHw(usp, vlan_mode->vlanId, dapi_g);
      ...
      LOG_PT_TRACE(LOG_CTX_HAPI, "Port {%d,%d,%d} removed from VLAN %u (MACs flushed)", usp->unit, usp->slot, usp->port, vlan_mode->vlanId);
    }
  }
...
L7_RC_t hapiBroadPtinBridgeCrossconnect(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI,"SrcPort={%d,%d,%d}, DstPort={%d,%d,%d}, outerVlanId=%u, innerVlanId=%u",
            usp->unit, usp->slot, usp->port,
...
L7_RC_t hapiBroadPTinPrbsEnable(DAPI_USP_t *usp, L7_BOOL enable, DAPI_t *dapi_g)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: Invalid port reference {%d,%d,%d}",usp->unit,usp->slot,usp->port);
    return L7_FAILURE;
  }
...
#endif
  {
    ...
    if (rc != L7_SUCCESS)
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error reading preemphasys from bcm_port %u", bcm_port);
    }
    ...
    if (enable)
    {
      ...
      {
        LOG_PT_ERR(LOG_CTX_HAPI, "Error initializing bcm_port %u", bcm_port);
      }
      /* Full duplex */
      ...
      LOG_PT_INFO(LOG_CTX_HAPI, "Needed %u tries to wait for KR4 reestablishment (bcm_port %u)", bcm_port);

      /* Force previous tap settings */
      if (rc == L7_SUCCESS)
      {
        LOG_PT_INFO(LOG_CTX_HAPI, "Going to force original preemphasys (1:0x%08x, 2:0x%08x, 3:0x%08x, 4:0x%08x) to bcm_port %u",
        ...
        if (rc != L7_SUCCESS)
        {
          LOG_PT_ERR(LOG_CTX_HAPI, "Error forcing preemphasys to bcm_port %u", bcm_port);
        }
        else
        {
          LOG_PT_INFO(LOG_CTX_HAPI, "Success forcing preemphasys to bcm_port %u", bcm_port);
        }
      }
    }
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR applying bcmPortControlPrbsMode 0 to port %d, bcm_port=%d",enable, port, bcm_port);
    return L7_FAILURE;
  }
  /* PRBS polynomial 31 */
  ...
  if (dapiCardPtr->wcPortMap[port].wcSpeedG == 10)
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "ERROR applying enable state %u to port %d, bcm_port=%d",enable, port, bcm_port);
      return L7_FAILURE;
    }
    LOG_PT_TRACE(LOG_CTX_HAPI, "Success inverting PRBS sequence to port %d, bcm_port=%d", port, bcm_port);
  }
#endif
  /* PRBS enable */
...
#endif
  {
    if (!enable)
    {
      ...
      if (rc != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_HAPI, "Error restoring preemphasys to bcm_port %u", bcm_port);
      }
      else
      {
        LOG_PT_INFO(LOG_CTX_HAPI, "Success restoring preemphasys to bcm_port %u", bcm_port);
      }

      /* KR4 mode again */
      if (ptin_hapi_kr4_set(bcm_port) != L7_SUCCESS)
      {
        LOG_PT_ERR(LOG_CTX_HAPI, "Error restoring KR4 mode at bcm_port %u", bcm_port);
      }
    }
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "PRBS TX/RX %s for port {%d,%d,%d}, bcm_port=%d",
...
L7_RC_t hapiBroadPTinPrbsRxStatus(DAPI_USP_t *usp, L7_uint32 *rxErrors, DAPI_t *dapi_g)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR reading rx status from port {%d,%d,%d}, bcm_port=%d",usp->unit,usp->slot,usp->port,bcm_port);
    return L7_FAILURE;
  }
...
static L7_RC_t hapiBroadPTinPrbsPreemphasisGet(DAPI_USP_t *usp, L7_uint16 *preemphasys, L7_int number_of_lanes)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error reading linkscan mode");
    return L7_FAILURE;
  }
  if (bcm_linkscan_enable_set(0, 0) != BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error disablink linkscan to all ports");
    return L7_FAILURE;
  }
  ...
  for (lane = 0; lane < number_of_lanes; lane++)
  {
    ...
    if (rc != BCM_E_NONE) 
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error modifying register to port %u (bcm_port %u)", usp->port, bcm_port);
      rc_global = rc;
      continue;
    }
    /* AER + lane */
    ...
    if (rc != BCM_E_NONE)
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error reading register from port %u (bcm_port %u)", usp->port, bcm_port);
      rc_global = rc;
      continue;
    }
    ...
    LOG_PT_INFO(LOG_CTX_HAPI, "Port %-2u (bcm_port %-2u) lane %u: preemphasys=0x%04x", usp->port, bcm_port, lane, preemphasys[lane]);
  }
  ...
  if (rc != BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error restoring linkscan mode to all ports");
    rc_global = rc;
  }
...
static L7_RC_t hapiBroadPTinPrbsPreemphasisSet(DAPI_USP_t *usp, L7_uint16 *preemphasys, L7_int number_of_lanes, L7_BOOL force)
{
  ...
  for (lane = 0; lane < number_of_lanes; lane++)
  {
    ...
    if (rc != BCM_E_NONE)
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error setting register to port %u (bcm_port %u)", usp->port, bcm_port);
      rc_global = rc;
      continue;
    }
...
L7_RC_t hapiBroadPtinL3Manage(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  if (ptr->oper == PTIN_L3_MANAGE_HOST)
  {
    switch (ptr->cmd) 
    {
      ...
      default:
        rc = L7_FAILURE;
        LOG_PT_ERR(LOG_CTX_HAPI,"Command not handled (%u)", ptr->cmd);
    }
  }
  ...
  {
    rc = L7_FAILURE;
    LOG_PT_ERR(LOG_CTX_HAPI,"Operation not implemented (%u)", ptr->oper);
  }

  if (rc != L7_SUCCESS)
  {
    LOG_PT_ERR(LOG_CTX_HAPI,"Error: rc=%u", rc);
  }
...
L7_RC_t hapiBroadPtinMEPCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g) {
  ...
  LOG_PT_TRACE(LOG_CTX_API, "Success %d \n\r");
  ...
  LOG_PT_TRACE(LOG_CTX_API, "Success %d \n\r", r);
  if (!alrdy) {
      LOG_PT_TRACE(LOG_CTX_API, "r = %d empty %d %d\n\r", r, empty);
      ...
      LOG_PT_TRACE(LOG_CTX_API, "r = %d \n\r", r);
      if (BCM_E_NONE!=r) {
          LOG_PT_ERR(LOG_CTX_API, "bcm_oam_group_create()=%d\n\r", r);
  ...
  LOG_PT_TRACE(LOG_CTX_API, "Success\n\r", r);
   ...
   {//SRC MAC ADDRESS
     ...
     nimUSP_t  nim_usp;
         ...
         if (L7_SUCCESS!=nimGetIntIfNumFromUSP(&nim_usp, &intIfNum)
             ...
             r=L7_REQUEST_DENIED;
             LOG_PT_ERR(LOG_CTX_HAPI, "couldn't get SMAC\n\r");
  ...
  if (BCM_E_NONE!=r) LOG_PT_ERR(LOG_CTX_HAPI, "bcm_oam_endpoint_create()=%d\n\r", r);
...
_hapiBroadPtinMEPCreate_nokend:
  if (!alrdy) {
      ...
      LOG_PT_TRACE(LOG_CTX_API, "ERROR\n\r", r);
  }
...
bcm_oam_endpoint_info_t mep;
    ...
    if (BCM_E_NONE!=r) {LOG_PT_ERR(LOG_CTX_HAPI, "bcm_oam_endpoint_get()=%d\n\r", r); return L7_NOT_EXIST;}
    group_id=mep.group;

    r=bcm_oam_endpoint_destroy(0, 1+((hapi_mep_t *) data)->imep);   //1..N
    if (BCM_E_NONE!=r) {LOG_PT_ERR(LOG_CTX_HAPI, "bcm_oam_endpoint_destroy()=%d\n\r", r); return L7_FAILURE;}
