...
L7_RC_t hapiBroadRtag7SwitchControlSet(int unit)
{
  ...
  rv = bcm_switch_control_set(unit, bcmSwitchHashIP6TcpUdpField0, arg);
  if ((L7_BCMX_OK(rv) != L7_TRUE) && (rv != BCM_E_UNAVAIL))
  {
    return L7_FAILURE;
  }

  rv = bcm_switch_control_set(unit, bcmSwitchHashIP6TcpUdpField1, arg);
  if ((L7_BCMX_OK(rv) != L7_TRUE) && (rv != BCM_E_UNAVAIL))
  {
    return L7_FAILURE;
  }
...
#if (PTIN_BOARD == PTIN_BOARD_TC16SXG)
  /* TC16SXG board will look to external config.bcm and load rc.soc base script */
...
#else
  /* Other boards will follow the traditional method */
  ...
  else if (logit && hapiBroadCmPrint_debug)   /* PTin modified: Logs */
  {
    ...
    {  
      ...
      else
      {
        ...
        if (logger_check(LOG_CTX_SDK, ptin_log_sev)) logger_print(LOG_CTX_SDK, ptin_log_sev, NULL, NULL, 0, "(0x%08x) %s", flags, buf);
      }
    }
  }
