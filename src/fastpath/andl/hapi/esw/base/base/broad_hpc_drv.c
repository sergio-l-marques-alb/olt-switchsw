...
int hapiBroadCpuCosqRateSet(int unit, int cosq, int rate, BROAD_CPU_RATE_LIMIT_TYPE_t type)
{
  ...
  if (rv != BCM_E_NONE)
  {
    PT_LOG_ERR(LOG_CTX_STARTUP, "Error setting rate limit for CPU (rv=%d)",  rv);
  }
...
int hapiBroadCmPrint(bsl_meta_t *meta_data, const char *format, va_list args)
{
  ...
  log_severity_t ptin_log_sev = LOG_SEV_INFO;
  ...
  else if (logit && hapiBroadCmPrint_debug)   /* PTin modified: Logs */
  {
    ...
    {  
      ...
      {
        if (logger_check(LOG_CTX_SDK, ptin_log_sev)) logger_print(LOG_CTX_SDK, ptin_log_sev, meta_data->file, meta_data->func, meta_data->line, "\r\n", "%s", buf);    
      }
    }
  }
