...
L7_RC_t hapiBroadReconfigInbandMeter(L7_uint16 cir, L7_uint16 cbs)
{
  ...
  PT_LOG_INFO(LOG_CTX_HAPI, "Going to apply CIR=%u CBS=%u", cir, cbs);
  ...
  {
    PT_LOG_WARN(LOG_CTX_HAPI, "No inband rule is configured, but meter info was updated successfully");
    return L7_SUCCESS;
  }
...
/*PTin modified: MIPs (more precisely some OAM ETH traffic, involving MEPs or MIPs) have to be trapped 
  ...
  */
  ...
  if ((0 != new_vlan_id) && (L7_NULLPTR != new_mac_addr))
  {
    ...
    {
      PT_LOG_ERR(LOG_CTX_STARTUP, "Error adding BROAD_FIELD_OVID qualifier");
      hapiBroadPolicyCreateCancel();
      return;
    }
#endif
