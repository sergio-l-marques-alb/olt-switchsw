...
/* Calling this with 1 sends the debug output to dapiTrace. 
** Calling with 0 sends the debug output to console
*/
void hapiBroadL3DebugTraceEnable(L7_BOOL traceMode)
{
  broadL3DebugTrace = traceMode;
}
...
void hapiBroadL3DebugNhopShow(L7_uint32 arg) /* 1=failed 2=all or EgrId */
{
  ...
  if (arg == 0)
  {
    sysapiPrintf("Usage: hapiBroadL3DebugNhopShow(arg). Where arg is \n"
                 "\t 1 - show all NHOP entries\n"
                 "\t 2 - show failed NHOP entries only\n"
  ...
  sysapiPrintf ("\nHAPI Broad L3 Nexthop table: total count %d\n", 
...
void hapiBroadL3DebugHostShow(L7_uint32 arg1, L7_uint32 arg2) 
{
  ...
  if (arg1 == 0)
  {
    sysapiPrintf("Usage: hapiBroadL3DebugHostShow(arg1, arg2). Where arg1 is \n"
                 "\t 1 - show host entries(both valid/failed)\n"
                 "\t 2 - show failed Host entries only\n"
                 "\t 3 - show the entry matching IPv4\n"
                 "\t arg2 is max entries/IPv4 address to show\n");
    return;
  }
  ...
  {
    if (((arg1 == 2) && (pHostEntry->rv == 0)) ||
        ((arg1 == 3)  && (pHostEntry->key.addrUsp.addr.ipv4 != arg2)))
    {
...
void hapiBroadL3DebugRouteShow(L7_uint32 arg1, L7_uint32 arg2)
{
  L7_uchar8 count=0;
  ...
  if (arg1 == 0)
  {
    sysapiPrintf("Usage: hapiBroadL3DebugRouteShow(arg1, arg2). Where arg1 is \n"
                 "\t 1 - show route entries(both valid/failed)\n"
                 "\t 2 - show failed route entries only\n"
                 "\t 3 - show the entry matching IPv4\n"
                 "\tAnd arg2 is the max entries/IPv4 address to show\n");
    return;
  }
  ...
  sysapiPrintf ("\nHAPI Broad L3 Route table: total count %d\n",
                hapiBroadL3RouteTree.count); 

  while(pRouteEntry != L7_NULL)
  {
    if (((arg1 == 2) && (pRouteEntry->rv == 0)) ||
        ((arg1 == 3) && (pRouteEntry->key.ipAddr.ipv4 != arg2)))
    {
