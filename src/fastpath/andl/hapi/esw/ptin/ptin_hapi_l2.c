...
L7_RC_t ptin_hapi_maclimit_inc(bcmx_l2_addr_t *bcmx_l2_addr)
{
  ...
  { 
    /* Check if is valid physical port */
    ...
    {  
      ...
      if (macLearn_info_physical[physical_port].trap_sent == L7_FALSE)
      {
...
L7_RC_t ptin_hapi_maclimit_dec(bcmx_l2_addr_t *bcmx_l2_addr)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(bcmx_l2_addr->lport))
  {
    ...
    {
      LOG_NOTICE(LOG_CTX_PTIN_HAPI, "Physical port is out of range! (vport_id=%u max=%u)", vport_id, MAX_GPORTS);
      return L7_FAILURE;
    }
...
L7_RC_t ptin_hapi_physical_maclimit_reset(L7_uint port_id)
{
  ...
  /* VLAN ID is valid? */
  if (port_id > L7_MAX_PORT_COUNT)
  {
    LOG_NOTICE(LOG_CTX_PTIN_HAPI, "Physical is out of range! (VLAN ID=%u max=%u)", port_id, L7_MAX_PORT_COUNT);
    return L7_FAILURE;
  }
 ...
 * @return L7_RC_t : L7_SUCCESS / L7_FAILURE
 */
L7_RC_t ptin_hapi_maclimit_setmax(DAPI_USP_t *ddUsp, L7_uint16 vlan_id, int mac_limit, DAPI_t *dapi_g)
{
  ...
  else
  {
    ...
    /*L7_int physical_port;
    else if ((vlan_id ==0) && ((hapi_ptin_port_get(bcm_port, &physical_port)<L7_MAX_PORT_COUNT)))
    {
      LOG_TRACE(LOG_CTX_PTIN_HAPI, "Valid Physical ID=%u", physical_port);
    ...
    if (mac_limit == (L7_uint8)-1)
    {
      ...
      if (((vlan_id == 0) && (physical_port < L7_MAX_PORT_COUNT)) && (macLearn_info_physical[physical_port].mac_limit != (L7_uint8)-1))
      {
