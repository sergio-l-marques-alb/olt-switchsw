...
L7_RC_t ptin_hapi_maclimit_dec(bcmx_l2_addr_t *bcmx_l2_addr)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(bcmx_l2_addr->lport))
  {
    ...
    {
      PT_LOG_NOTICE(LOG_CTX_HAPI, "Virtual port is out of range! (vport_id=%u max=%u)", vport_id, MAX_GPORTS);
      return L7_FAILURE;
    }
...
L7_RC_t ptin_hapi_vport_maclimit_reset(bcm_gport_t gport)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    PT_LOG_NOTICE(LOG_CTX_HAPI, "%u", macLearn_info_flow[vport_id].mac_limit );
...
L7_RC_t ptin_hapi_vport_maclimit_setmax(bcm_gport_t gport, L7_uint8 mac_limit)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    {
      PT_LOG_NOTICE(LOG_CTX_HAPI, "%d %d)", mac_limit , (L7_uint8)-1);
...
L7_RC_t ptin_hapi_maclimit_setmax(DAPI_USP_t *ddUsp, L7_uint16 vlan_id, int mac_limit, L7_uint16 action, L7_uint16 send_trap, DAPI_t *dapi_g )
{
  ...
  else if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    ...
    if(action == 1) 
    {
      PT_LOG_TRACE(LOG_CTX_HAPI,"Action =1 ");
    ...
    else if (action == 0) /* no action, only limits */
    {
      ...
      PT_LOG_TRACE(LOG_CTX_HAPI,"Action =0");
    }
    else /* Use Last mask/flags */
    {
      PT_LOG_TRACE(LOG_CTX_HAPI,"Action = %d", action);
    ...
    PT_LOG_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.flags);
    PT_LOG_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", macLearn_info_physical[physical_port].mask);
    PT_LOG_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.limit);
    PT_LOG_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", macLearn_info_physical[physical_port].old_limit);
  ...
  {
    PT_LOG_TRACE(LOG_CTX_HAPI, "Valid VLAN ID=%u", vlan_id);
  ...
  //PT_LOG_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.flags);
  //PT_LOG_NOTICE(LOG_CTX_HAPI, "trunk  %u", limit.trunk);
  //PT_LOG_NOTICE(LOG_CTX_HAPI, "port   %u", limit.port);
  //PT_LOG_NOTICE(LOG_CTX_HAPI, "vlan   %u", limit.vlan);
  //PT_LOG_NOTICE(LOG_CTX_HAPI, "limit  %u", limit.limit);

  #if(PTIN_BOARD != PTIN_BOARD_CXO640G) /*Not supported in Trident(Plus). */

  PT_LOG_NOTICE(LOG_CTX_HAPI, "Error");
  ...
  if (mac_limit == -1)
  {
    ...
    {
      PT_LOG_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in VLAN %d ", vlan_id);
    ...
    {
      PT_LOG_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in Physical Port %d ", physical_port);
    ...
    {
      PT_LOG_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in LAG %d ", trunk_id);
...
L7_RC_t ptin_hapi_vport_maclimit_alarmconfig(bcm_gport_t gport, int bcm_port, L7_uint16 outer_vid)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    PT_LOG_NOTICE(LOG_CTX_HAPI, "port %u", port);
  }
  else
  {
    PT_LOG_WARN(LOG_CTX_HAPI, "GPORT is not valid! (vport_id)", vport_id);
