...
L7_RC_t ptin_hapi_maclimit_inc(bcmx_l2_addr_t *bcmx_l2_addr)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(bcmx_l2_addr->lport)) // Check if a Virtual port level
  {
    ...
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "GPORT is out of range! (vport_id=%u max=%u)", vport_id, MAX_GPORTS);
      return L7_FAILURE;
    }
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on VID %d and GPORT 0x%x rejected (flags 0x%x)",
              __FUNCTION__, 
      ...
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Disabling FWD (GPORT=0x%x)", bcmx_l2_addr->lport);
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on VID %d and GPORT 0x%x accepted (flags 0x%x)\r\n",
              __FUNCTION__, 
              bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
              bcmx_l2_addr->vid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
  ...
  else if((bcmx_l2_addr->tgid > 0) && ((bcmx_l2_addr->tgid < PTIN_SYSTEM_N_LAGS)))
  {
    ...
    {    
      LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on LAG %d and GPORT 0x%x rejected (flags 0x%x)",
                __FUNCTION__, 
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                tgid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
      LOG_PT_TRACE(LOG_CTX_HAPI, "LAG %d Over the limit", tgid);
       
      macLearn_info_lag[tgid].mac_total++;
      LOG_PT_TRACE(LOG_CTX_HAPI, "Increased total of MAC to %d in LAG %d", macLearn_info_lag[tgid].mac_total, tgid);
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on LAG %d and GPORT 0x%x accepted (flags 0x%x)\r\n",
                __FUNCTION__, 
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                tgid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);

    LOG_PT_TRACE(LOG_CTX_HAPI, "Increase MAC Learned in LAG %d", tgid);
    macLearn_info_lag[tgid].mac_counter++;
    macLearn_info_lag[tgid].mac_total++;
    ...
    {
      /* Enable the use of Pending Mechanism but disable FWD */
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Disabling FWD (Physical Port=0x%x)", bcmx_l2_addr->lport);
  ...
  else if(BCMX_LPORT_VALID(bcmx_l2_addr->lport))
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "Is a Physical port");
   
    /* Physical port ID is valid? */
    if (physical_port > L7_MAX_PORT_COUNT)
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Physical is out of range! (physical_id=%u max=%u)", physical_port, L7_MAX_PORT_COUNT);
      return L7_FAILURE;
    }
    else
    {     
      ...
      { 
        LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on Physical port %d and GPORT 0x%x rejected (flags 0x%x)",
                __FUNCTION__, 
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                physical_port, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
       
        macLearn_info_physical[physical_port].mac_total++;
        LOG_PT_TRACE(LOG_CTX_HAPI, "Increased total");
        ...
        LOG_PT_TRACE(LOG_CTX_HAPI, "Not increase the learned MAC in %d");
        return L7_FAILURE;
      }
      #endif

      LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on Physical port %d and GPORT 0x%x accepted (flags 0x%x)\r\n",
                __FUNCTION__, 
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                physical_port, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
      ...
      macLearn_info_physical[physical_port].mac_counter++;
      macLearn_info_physical[physical_port].mac_total++;
      LOG_PT_TRACE(LOG_CTX_HAPI, "Increase MAC Learned in Port %d to %d", physical_port, macLearn_info_physical[physical_port].mac_counter);
  ...
  {  
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "Check if is a VLAN port");

    /* VLAN ID is valid? */
    if (vlan_id > MAX_VLANS)
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "VLAN is out of range! (VLAN ID=%u max=%u)", vlan_id, MAX_VLANS);
      return L7_FAILURE;
    }
    /* Feature enabled? */
...
L7_RC_t ptin_hapi_maclimit_dec(bcmx_l2_addr_t *bcmx_l2_addr)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(bcmx_l2_addr->lport))
  {
    ...
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Virtual port is out of range! (vport_id=%u max=%u)", vport_id, MAX_GPORTS);
      return L7_FAILURE;
    }
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on VID %d and GPORT 0x%x cleared (flags 0x%x)",
              __FUNCTION__, 
              bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
              bcmx_l2_addr->vid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x/VID %d with PENDING flag",__FUNCTION__,
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                bcmx_l2_addr->vid);
    ...
    if ( (macLearn_info_flow[vport_id].mac_counter < macLearn_info_flow[vport_id].mac_limit) && (macLearn_info_flow[vport_id].mac_total < macLearn_info_flow[vport_id].mac_limit))
    {
      ...
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Enabling FWD (GPORT=0x%x)", bcmx_l2_addr->lport);
      bcm_port_learn_set(0, bcmx_l2_addr->lport, BCM_PORT_LEARN_FWD | /*BCM_PORT_LEARN_CPU |*/ BCM_PORT_LEARN_PENDING | BCM_PORT_LEARN_ARL );
    }
    #endif
    return L7_SUCCESS;
  ...
  else if((bcmx_l2_addr->tgid >= 0) && ((bcmx_l2_addr->tgid < PTIN_SYSTEM_N_LAGS)))
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on LAG %d and GPORT 0x%x cleared (flags 0x%x)",
              __FUNCTION__, 
              bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
              tgid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
    ...
    if ( ((bcmx_l2_addr->flags & BCM_L2_PENDING) && !(bcmx_l2_addr->flags & BCM_L2_MOVE)) )
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x/LAG %d with PENDING flag",__FUNCTION__,
                bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
                tgid);
  ...
  else if(BCMX_LPORT_VALID(bcmx_l2_addr->lport))
  {
    ...
    if ( (physical_port > L7_MAX_PORT_COUNT) )
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "Physical is out of range! (physical_id=%u max=%u)", hapi_ptin_port_get(physical_port, &physical_port), L7_MAX_PORT_COUNT);
      return L7_FAILURE;
    }
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "%s: MAC %02x:%02x:%02x:%02x:%02x:%02x on Physical Port %d and GPORT 0x%x cleared (flags 0x%x)",
              __FUNCTION__, 
              bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
              physical_port, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
     ...
     {
       /* Enable the FWD */
       LOG_PT_NOTICE(LOG_CTX_HAPI, "Enable FWD (Physical Port=0x%x)", bcmx_l2_addr->lport);
                 bcm_port_learn_set(0, bcmx_l2_addr->lport, BCM_PORT_LEARN_FWD /*BCM_PORT_LEARN_CPU | BCM_PORT_LEARN_PENDING*/ | BCM_PORT_LEARN_ARL );
     }
     #endif
...
L7_RC_t ptin_hapi_vport_maclimit_reset(bcm_gport_t gport)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI, "Disabling Pending Mechanism (GPORT=0x%x)", gport);
    ...
    LOG_PT_NOTICE(LOG_CTX_HAPI, "%u", macLearn_info_flow[vport_id].mac_limit );
  ...
  {
    LOG_PT_WARN(LOG_CTX_HAPI, "GPORT is not valid! (vport_id)", vport_id);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_hapi_vlan_maclimit_reset(bcm_vlan_t vlan_id)
{
  ...
  LOG_PT_NOTICE(LOG_CTX_HAPI, "Disabling MAC Limit Mechanism (VLAN ID=%u)", vlan_id);
...
L7_RC_t ptin_hapi_physical_maclimit_reset(L7_uint port_id)
{
  ...
  {
    LOG_PT_NOTICE(LOG_CTX_HAPI, "Physical is out of range! (Port ID=%u max=%u)", port_id, L7_MAX_PORT_COUNT);
    return L7_FAILURE;
  }

  LOG_PT_NOTICE(LOG_CTX_HAPI, "Disabling MAC Limit Mechanism (Port ID=%u)", port_id);
...
L7_RC_t ptin_hapi_lag_maclimit_reset(bcm_trunk_t trunk_id)
{
  ...
  {
    LOG_PT_NOTICE(LOG_CTX_HAPI, "LAG is out of range! (LAG ID=%u max=%u)", trunk_id, PTIN_SYSTEM_N_LAGS);
    return L7_FAILURE;
  }

  LOG_PT_NOTICE(LOG_CTX_HAPI, "Disabling MAC Limit Mechanism (LAG ID=%u)", trunk_id);
...
L7_RC_t ptin_hapi_vport_maclimit_setmax(bcm_gport_t gport, L7_uint8 mac_limit)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    {
      LOG_PT_NOTICE(LOG_CTX_HAPI, "%d %d)", mac_limit , (L7_uint8)-1);
      rc = ptin_hapi_vport_maclimit_reset(gport);

      return rc;
    }

    LOG_PT_NOTICE(LOG_CTX_HAPI, "Enabling Pending Mechanism (GPORT=0x%x) with MAC Learned limit set to %u", gport, mac_limit);
    ...
    if (mac_limit < mac_limit_old)
    {
      ...
      LOG_PT_TRACE(LOG_CTX_HAPI, "Performing fdbFlush");
...
L7_RC_t ptin_hapi_maclimit_setmax(DAPI_USP_t *ddUsp, L7_uint16 vlan_id, int mac_limit, L7_uint16 action, L7_uint16 send_trap, DAPI_t *dapi_g )
{
  ...
  {
    LOG_PT_WARN(LOG_CTX_HAPI,"No provided interface!");
    return L7_SUCCESS;
  }
  if (ddUsp->unit<0 || ddUsp->slot<0 || ddUsp->port<0)
  {
    LOG_PT_WARN(LOG_CTX_HAPI,"Invalid interface!");
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI,"Analysing interface {%d,%d,%d}: lport=0x%08x", ddUsp->unit, ddUsp->slot, ddUsp->port, lport);
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr))
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI,"Interface {%d,%d,%d} is a lag: trunk_id = %d", ddUsp->unit, ddUsp->slot, ddUsp->port, trunk_id);    
  ...
  else if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    ...
    LOG_PT_TRACE(LOG_CTX_HAPI,"Interface {%d,%d,%d} is a port: bcm_port = %d", ddUsp->unit, ddUsp->slot, ddUsp->port, bcm_port);
    limit.port = bcm_port;
    ...
    if(action == 1) 
    {
      LOG_PT_TRACE(LOG_CTX_HAPI,"Action =1 ");
    ...
    else if (action == 0) /* no action, only limits */
    {
      ...
      LOG_PT_TRACE(LOG_CTX_HAPI,"Action =0");
    }
    else /* Use Last mask/flags */
    {
      LOG_PT_TRACE(LOG_CTX_HAPI,"Action = %d", action);
    ...
    LOG_PT_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.flags);
    LOG_PT_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", macLearn_info_physical[physical_port].mask);
    LOG_PT_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.limit);
    LOG_PT_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", macLearn_info_physical[physical_port].old_limit);
  ...
  {
    LOG_PT_TRACE(LOG_CTX_HAPI, "Valid VLAN ID=%u", vlan_id);
  ...
  {
    LOG_PT_TRACE(LOG_CTX_HAPI, "Limit is system wide");
  ...
  //LOG_PT_NOTICE(LOG_CTX_HAPI, "flags  0x%.4X", limit.flags);
  //LOG_PT_NOTICE(LOG_CTX_HAPI, "trunk  %u", limit.trunk);
  //LOG_PT_NOTICE(LOG_CTX_HAPI, "port   %u", limit.port);
  //LOG_PT_NOTICE(LOG_CTX_HAPI, "vlan   %u", limit.vlan);
  //LOG_PT_NOTICE(LOG_CTX_HAPI, "limit  %u", limit.limit);

  #if(PTIN_BOARD != PTIN_BOARD_CXO640G) /*Not supported in Trident(Plus). */

  LOG_PT_NOTICE(LOG_CTX_HAPI, "Error");
  if ((rv=bcm_l2_learn_limit_set(0, &limit))!=BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error (%d) setting L2 learn limit to %u", rv, mac_limit);
    return L7_FAILURE;
  }
  #endif
  ...
  if (mac_limit == -1)
  {
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in VLAN %d ", vlan_id);
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in Physical Port %d ", physical_port);
    ...
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, "Reset the Mac learned in LAG %d ", trunk_id);
...
L7_RC_t ptin_hapi_maclimit_status(DAPI_USP_t *ddUsp, L7_uint32 *mac_learned, L7_uint32 *status, DAPI_t *dapi_g)
{
  ...
  if (IS_PORT_TYPE_LOGICAL_LAG(dapiPortPtr))
  {
    ...
    *mac_learned = macLearn_info_lag[trunk_id].mac_counter;  
    LOG_PT_TRACE(LOG_CTX_HAPI,"Number of MAC Learned is %d in LAG %d (Limit %d, Total %d)", macLearn_info_lag[trunk_id].mac_counter, trunk_id, macLearn_info_lag[trunk_id].mac_limit,macLearn_info_lag[trunk_id].mac_total);

    if (macLearn_info_lag[trunk_id].mac_counter >= macLearn_info_lag[trunk_id].mac_limit && (macLearn_info_lag[trunk_id].mac_limit!=0))
    {
      *status = 1;
      LOG_PT_TRACE(LOG_CTX_HAPI,"Status = %d (Over the limit)", *status);
    }
    else
    {
      *status = 0;
      LOG_PT_TRACE(LOG_CTX_HAPI,"Status = %d (Within the limit)", *status);
    } 
  }
  /* Extract Physical port */
  else if (IS_PORT_TYPE_PHYSICAL(dapiPortPtr))
  {
    ...
    *mac_learned = macLearn_info_physical[physical_port].mac_counter;  
    LOG_PT_TRACE(LOG_CTX_HAPI,"Number of MAC Learned is %d in physical port %d (Limit %d, Total %d)", macLearn_info_physical[physical_port].mac_counter, physical_port, macLearn_info_physical[physical_port].mac_limit,macLearn_info_physical[physical_port].mac_total);
  ...
  {
    LOG_PT_TRACE(LOG_CTX_HAPI,"Error providing the status of the interface {%d,%d,%d}", ddUsp->unit, ddUsp->slot, ddUsp->port);
...
L7_RC_t ptin_hapi_vport_maclimit_alarmconfig(bcm_gport_t gport, int bcm_port, L7_uint16 outer_vid)
{
  ...
  if (BCM_GPORT_IS_VLAN_PORT(gport))
  {
    ...
    LOG_PT_NOTICE(LOG_CTX_HAPI, "(GPORT=0x%x) MAC Learned limit information %u, bcm_port %u, outer_vid %d", gport, bcm_port, outer_vid);
    ...
    LOG_PT_NOTICE(LOG_CTX_HAPI, "port %u", port);
  }
  else
  {
    LOG_PT_WARN(LOG_CTX_HAPI, "GPORT is not valid! (vport_id)", vport_id);
