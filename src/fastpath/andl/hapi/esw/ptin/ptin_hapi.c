...
L7_RC_t ptin_hapi_phy_init(void)
{
  ...
  {
    LOG_INFO(LOG_CTX_PTIN_HAPI, "Success initializing TG4G phys");
  }
  else
  {
    LOG_ERR(LOG_CTX_PTIN_HAPI, "Error initializing TG4G phys");
  }
...
L7_RC_t ptin_hapi_phy_init_tg4g(void)
{
  ...
  for (i=0; i<PTIN_SYSTEM_N_PONS; i++)
  {
    ...
    LOG_NOTICE(LOG_CTX_PTIN_HAPI, "Success initializing port %u (bcm_port %u) at SFI mode", i, bcm_port);
  }
  #endif
