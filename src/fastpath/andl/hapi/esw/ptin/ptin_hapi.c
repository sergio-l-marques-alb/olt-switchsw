...
L7_RC_t hapi_ptin_egress_ports(L7_uint usp_port_frontier);
...
L7_RC_t ptin_hapi_phySemaTake(L7_uint16 usp_port)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_HAPI, "usp_port %u: Error blocking semaphore related to bcm_port %u (port %u)", usp_port, hapiPortPtr->bcm_port);
    return L7_FAILURE;
  }
...
L7_RC_t ptin_hapi_phySemaGive(L7_uint16 usp_port)
{
  ...
  {
    PT_LOG_ERR(LOG_CTX_HAPI, "usp_port %u: Error releasing semaphore related to bcm_port %u (port %u)", usp_port, hapiPortPtr->bcm_port);
    return L7_FAILURE;
  }
