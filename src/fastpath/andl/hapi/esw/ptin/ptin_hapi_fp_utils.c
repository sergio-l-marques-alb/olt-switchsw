...
L7_RC_t ptin_hapi_policy_resources_get(st_ptin_policy_resources *resources)
{
  ...
  for (stage_idx=0; stage_idx<PTIN_POLICY_MAX_STAGES; stage_idx++)
  {
    ...
    for (group_idx=0; group_idx<PTIN_POLICY_MAX_GROUPS; group_idx++)
    {
      ...
      {
        LOG_TRACE(LOG_CTX_HAPI,"Group index %u (stage=%d) is not in use.",group_idx,stage_idx);
        memset(&resources->cap[group_idx][stage_idx],0x00,sizeof(st_ptin_ffp_rules));
        continue;
      }

      LOG_TRACE(LOG_CTX_HAPI,"Analysing group index %u / stage=%d: gid=%d (flags=0x%08x)",group_idx,stage_idx,gtable->gid,gtable->flags);

      /* Get group details */
      error = bcm_field_group_status_get(0, gtable->gid, &group_status);
      if (error==BCM_E_NOT_FOUND)
      {
        LOG_TRACE(LOG_CTX_HAPI,"Group %u (stage=%d) does not exist",gtable->gid,stage_idx);
        memset(&resources->cap[group_idx][stage_idx],0x00,sizeof(st_ptin_ffp_rules));
        continue;
      }
      else if (error!=BCM_E_NONE)
      {
        LOG_ERR(LOG_CTX_HAPI,"Error reading status of group %u, stage=%d (error=%d \"%s\")",gtable->gid,stage_idx,error,bcm_errmsg(error));
        return L7_FAILURE;
      }
...
void ptin_hapi_policy_clear(void *elem, ptin_hapi_database_t *db)
{
  ...
  {
    LOG_ERR(LOG_CTX_HAPI,"Invalid policy");
    return;
  }
  ...
  LOG_TRACE(LOG_CTX_HAPI,"Policy cleared");
}
...
void *ptin_hapi_policy_next(void *base_ptr, ptin_hapi_database_t *db)
{
  ...
  {
    LOG_TRACE(LOG_CTX_HAPI,"Cell not found");
    return L7_NULLPTR;
  }

  LOG_TRACE(LOG_CTX_HAPI,"Cell found!");
...
void *ptin_hapi_policy_check_conflicts(DAPI_USP_t *usp, void *profile, void *base_ptr, ptin_hapi_database_t *db, int stage)
{
  ...
  LOG_TRACE(LOG_CTX_HAPI,"Cell found in conflict!");
...
void *ptin_hapi_policy_find_free(ptin_hapi_database_t *db)
{
  ...
  {
    LOG_TRACE(LOG_CTX_HAPI,"No free elements");
  ...
  LOG_TRACE(LOG_CTX_HAPI,"Found free element");
