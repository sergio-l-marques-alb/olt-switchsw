...
L7_RC_t ptin_hapi_bridge_init(void)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_switch_control_set(%d,%d,1)", bcm_unit, bcmSwitchSharedVlanEnable);
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error enabling shared vlan: error=%d (%s)", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error initializing MAC learning control: error=%d (%s)", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  #endif

  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_init returned success");
...
L7_RC_t ptin_hapi_bridge_free_resources(L7_uint16 *crossconnects)
{
  if (crossconnects != L7_NULLPTR)
  {
    *crossconnects = resources_crossconnects;
    LOG_PT_TRACE(LOG_CTX_HAPI, "Free cross-connections consulted: %u", *crossconnects);
  }
...
L7_RC_t ptin_hapi_bridge_vlan_mode_set(L7_uint16 vlanId, L7_uint16 fwdVlanId, L7_BOOL cross_connects_apply, L7_BOOL mac_learning_apply, L7_int mc_group)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error getting vlan control structure! error=%d (%s)\r\n", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_control_vlan_set: error=%d (%s)", error, bcm_errmsg(error));
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_set returned success");
...
L7_RC_t ptin_hapi_bridge_vlan_mode_fwdVlan_set(L7_uint16 vlanId, L7_uint16 fwdVlanId)
{
  ...
  if ( fwdVlanId == 0 || fwdVlanId > 4095 )
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid fwd vlan id: %u",fwdVlanId);
    return L7_FAILURE;
  }
  ...
  if ((error = bcmx_vlan_control_vlan_get(vlanId, &control))!=BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error getting vlan control structure! error=%d (%s)", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_fwdVlan_set returned success");
...
L7_RC_t ptin_hapi_bridge_vlan_mode_outerTpId_set(L7_uint16 vlanId, L7_uint16 outer_tpid)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: TPID is null");
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_outerTpId_set returned success");
...
L7_RC_t ptin_hapi_bridge_vlan_mode_macLearn_set(L7_uint16 vlanId, L7_BOOL mac_learning_apply)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "Control flags = 0x%08x",control.flags);
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_macLearn_set returned success");
...
L7_RC_t ptin_hapi_bridge_vlan_mode_crossconnect_set(L7_uint16 vlanId, L7_BOOL cross_connects_apply, L7_BOOL double_tag)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_crossconnect_set returned success (enable=%u, double=%u)", cross_connects_apply, double_tag);
...
L7_RC_t ptin_hapi_bridge_crossconnect_add(L7_uint16 outerVlanId, L7_uint16 innerVlanId, ptin_dapi_port_t *dapiPort1, ptin_dapi_port_t *dapiPort2)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "oVlanId=%u iVlanId=%u port1={%d,%d,%d} port2={%d,%d,%d}",
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: Invalid interfaces");
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: Ports ({%d,%d,%d} and {%d,%d,%d}) are not physical neither lag",
            dapiPort1->usp->unit,dapiPort1->usp->slot,dapiPort1->usp->port,
            dapiPort2->usp->unit,dapiPort2->usp->slot,dapiPort2->usp->port);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Outer vlan is invalid: %u",outerVlanId);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_vlan_cross_connect_add(%d,%u,%u,%u({%d,%d,%d}), %u({%d,%d,%d}))",
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_cross_connect_add: %d (%s)", error, bcm_errmsg(error));
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "Finished successfully");
...
L7_RC_t ptin_hapi_bridge_crossconnect_delete(L7_uint16 outerVlanId, L7_uint16 innerVlanId)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "oVlanId=%u iVlanId=%u",outerVlanId,innerVlanId);
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_vlan_cross_connect_delete(%d,%u,%u)",bcm_unit,outerVlanId,innerVlanId);

  error = bcmx_vlan_cross_connect_delete(outerVlanId, innerVlanId);
  if ( error != BCM_E_NONE && error != BCM_E_NOT_FOUND )
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_cross_connect_delete: %d (%s)", error, bcm_errmsg(error));
  }
...
L7_RC_t ptin_hapi_bridge_crossconnect_delete_all(void)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_vlan_cross_connect_delete_all(%d)",bcm_unit);
                            ...
                            L7_int *virtual_gport)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "port={%d,%d,%d}, oVlanId=%u iVlanId=%u => oVlanId=%u iVlanId=%u",
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: Port {%d,%d,%d} are not physical",
            dapiPort->usp->unit,dapiPort->usp->slot,dapiPort->usp->port);
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_port=%d vlan_port.port=%d vport=%d", hapiPortPtr->bcm_port, vlan_port.port, vlan_port.vlan_port_id);

  if ((error=bcm_vlan_port_create(0, &vlan_port)) != BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_port_create: error=%d (\"%s\")", error, bcm_errmsg(error));
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "vport=0x%x", vlan_port.vlan_port_id);
  ...
  do
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_port_learn_set: error=%d (\"%s\")", error, bcm_errmsg(error));
      break;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_port_control_set: error=%d (\"%s\")", error, bcm_errmsg(error));
      break;
    }
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_l2_learn_class_set: error=%d (\"%s\")", error, bcm_errmsg(error));
      break;
    }
    if ((error=bcm_l2_learn_port_class_set(0, vlan_port.vlan_port_id, 1)) != BCM_E_NONE)
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_l2_learn_port_class_set: error=%d (\"%s\")", error, bcm_errmsg(error));
      break;
    }
  } while (0);
  ...
  if (error != BCM_E_NONE)
  {
    ...
    LOG_PT_ERR(LOG_CTX_HAPI, "error=%d (\"%s\")", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  #endif
  ...
  {
    bcm_vlan_port_destroy(0, vlan_port.vlan_port_id);
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_translate_egress_action_add(0, %d, %d, %d, &action): error=%d (\"%s\")",
  ...
  if ( mc_group <= 0 )
  {
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI,"Error with bcm_multicast_create(0, %d, &mcast_group): error=%d (\"%s\")",
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "mc_group=%d vlan_port.port=%d vport=%d", mc_group, vlan_port.port, vlan_port.vlan_port_id);
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_vlan_encap_get: error=%d (\"%s\")",error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  if ((error=bcm_multicast_egress_add(0, mc_group, vlan_port.port, encap_id))!=BCM_E_NONE)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_egress_add: error=%d (\"%s\")",error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  LOG_PT_DEBUG(LOG_CTX_HAPI, "ptin_hapi_vp_create: vport 0x%x created", vlan_port.vlan_port_id);
                            ...
                            L7_int mcast_group)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "port={%d,%d,%d}, oVlanId=%u iVlanId=%u",
            dapiPort->usp->unit,dapiPort->usp->slot,dapiPort->usp->port,
  ...
  if ( mcast_group <= 0 )
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid MC group (%d)", mcast_group);
    return L7_FAILURE;
  }
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_port_find: error=%d (\"%s\")", error, bcm_errmsg(error));
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "gport=0x%x, bcm_port=%d => vlan_port.port=%d vport=%d",
             virtual_gport, hapiPortPtr->bcm_port, vlan_port.port, vlan_port.vlan_port_id);
  LOG_PT_TRACE(LOG_CTX_HAPI, "flags=0x%08x criteria=0x%08x match_vlan=%u match_ivlan=%u egress_vlan=%u egress_ivlan=%u",
  ...
  if (error != BCM_E_NONE && error != BCM_E_NOT_FOUND)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_egress_delete: error=%d (\"%s\")",error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  if (error != BCM_E_NONE && error != BCM_E_NOT_FOUND)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcmx_vlan_translate_egress_action_delete(%d, %d, %d, &action): error=%d (\"%s\")",
  ...
  if (error != BCM_E_NONE && error != BCM_E_NOT_FOUND)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_vlan_port_destroy: error=%d (\"%s\")", error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  if (error != BCM_E_NONE && error != BCM_E_NOT_FOUND)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error removing MAC addresses related to this vport: error=%d (\"%s\")", error, bcm_errmsg(error));
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_vp_remove: vport 0x%x removed", vlan_port.vlan_port_id);
...
L7_RC_t ptin_hapi_multicast_egress_port_add(L7_int *mcast_group, L7_uint32 multicast_flag, L7_uint32 virtual_gport, ptin_dapi_port_t *dapiPort)
{
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Invalid MC group");
    return L7_FAILURE;
  }

  LOG_PT_TRACE(LOG_CTX_HAPI, "port={%d,%d,%d}, mcast_group=%d",
  ...
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "ERROR: Port {%d,%d,%d} are not physical nor logical",
            dapiPort->usp->unit,dapiPort->usp->slot,dapiPort->usp->port);
    return L7_FAILURE;
  }
  ...
  case BCM_MULTICAST_TYPE_L2:  
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_l2_encap_get: error=%d (\"%s\")",error, bcm_errmsg(error));
      return L7_FAILURE;
    }    
    break;      
  case BCM_MULTICAST_TYPE_L3:
    ...
    {
      LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_l3_encap_get: error=%d (\"%s\")",error, bcm_errmsg(error));
      return L7_FAILURE;
    }    
    break;    
  default:
    LOG_PT_ERR(LOG_CTX_HAPI, "Multicast Flag Not Supported :0x%08X", multicast_flag);
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_multicast_port_add returned success");
...
L7_RC_t ptin_hapi_multicast_egress_clean(L7_int mcast_group, L7_BOOL destroy_mcgroup)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "mcast_group=%d", mcast_group);

  error = bcm_multicast_egress_delete_all(0, mcast_group);
  if (error != BCM_E_NONE && error != BCM_E_NOT_FOUND)
  {
    LOG_PT_ERR(LOG_CTX_HAPI, "Error with bcm_multicast_egress_delete_all: error=%d (\"%s\")",error, bcm_errmsg(error));
    return L7_FAILURE;
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "bcm_multicast_egress_delete_all returned success");
...
L7_RC_t ptin_hapi_bridgeVlan_multicast_set(L7_uint16 vlanId, L7_int *mcast_group, L7_uint32 multicast_flag)
{
  ...
  if ( mc_group <= 0 )
  {
    if ((error=bcm_multicast_create(0, multicast_flag, &mc_group)) != BCM_E_NONE)
    {
      LOG_PT_ERR(LOG_CTX_HAPI,"Error with bcm_multicast_create(0,0x%x, &mcast_group): error=%d (\"%s\")",
              multicast_flag, error, bcm_errmsg(error));
      return L7_FAILURE;
    }
    *mcast_group = mc_group;

    LOG_PT_DEBUG(LOG_CTX_HAPI, "mc_group=0x%08x created!", mc_group);
  }
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "ptin_hapi_bridge_vlan_mode_mcast_set returned success");
...
L7_RC_t ptin_hapi_bridgeVlan_multicast_reset(L7_uint16 vlanId, L7_int mcast_group, L7_uint32 multicast_flag, L7_BOOL destroy_mcgroup)
{
  ...
  if (destroy_mcgroup)
  {
    if ((error=bcm_multicast_destroy(0, mcast_group)) != BCM_E_NONE)
    {
      if (error != BCM_E_BUSY)
      {
        LOG_PT_ERR(LOG_CTX_HAPI,"Error with bcm_multicast_destroy (mcast_group=0x%08x): error=%d (\"%s\")",
                mcast_group, error, bcm_errmsg(error));
        return L7_FAILURE;
      }
      else
      {
        LOG_PT_WARN(LOG_CTX_HAPI,"bcm_multicast_destroy (mcast_group=0x%08x): rv=%d (\"%s\")",
                mcast_group, error, bcm_errmsg(error));
        return L7_SUCCESS;
      }
    }
