...
L7_RC_t hapiBroadLagAsyncDoneWait(L7_int32 timeout)
{
  ...
  if (rc != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DRIVER_COMPONENT_ID,
            "Lag async done message send failed on Driver Lag queue %p",
  ...
  if (rc != L7_SUCCESS)
  {
    L7_LOGF(L7_LOG_SEVERITY_WARNING, L7_DRIVER_COMPONENT_ID,
            "Lag Async done message wait timeout on lag queue %p, timeout %d secs",
    ...
    for (i = 0;i < L7_MAX_MEMBERS_PER_LAG;i++)
    {
      if (lagMemberSet[i].inUse == L7_TRUE)
      {
        ...
        if (numVlans) 
        {
          ...
          {
            L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DRIVER_COMPONENT_ID, "Couldn't add port 0x%x to VLANs, rv = %d", hapiPortPtr->bcm_gport, rv);
          }
        }
      }
    }
    ...
    for (i = 0;i < L7_MAX_MEMBERS_PER_LAG;i++)
    {
      if (lagMemberSet[i].inUse == L7_TRUE)
      {
        ...
        if (numVlans)
        {
          ...
          if (L7_BCMX_OK(rv) != L7_TRUE)
          {
            L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DRIVER_COMPONENT_ID, "Couldn't remove port 0x%x from VLANs, rv = %d", hapiPortPtr->bcm_gport, rv);
          } 
        }
      }
    }
...
L7_RC_t hapiBroadLagCreate(DAPI_USP_t *usp, DAPI_CMD_t cmd, void *data, DAPI_t *dapi_g)
{
  ...
  */
  ...
  BCM_GPORT_TRUNK_SET(hapiPortPtr->bcm_gport, tid);
  ...
  */
  ...
  for (searchEntry=0;searchEntry<L7_MAX_MEMBERS_PER_LAG;searchEntry++)
  {
    if (lagMemberSet[searchEntry].inUse == L7_TRUE)
    {
      ...
      bcmTrunkInfo.tp[memberCount] = BCMY_GPORT_MODPORT(hapiLagMemberPortPtr->bcm_gport);
      memberCount++;
    }
  }
  ...
  for (entry=0; entry < cmdLagPortDelete->cmdData.lagPortDelete.numOfMembers; entry++)
  {
    ...
    */
    if (L7_BRIDGE_SPEC_SUPPORTED == L7_BRIDGE_SPEC_802DOT1D)
    {
      ...
      if (L7_BCMX_OK(rc) != L7_TRUE)
      {
        L7_LOGF(L7_LOG_SEVERITY_ERROR, L7_DRIVER_COMPONENT_ID, "Couldn't set STP state for port 0x%x, rv = %d", hapiLagMemberPortPtr->bcm_gport, rc);
      }
    } else
    {
