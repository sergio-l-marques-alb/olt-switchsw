...
void hapiBroadAddrMacUpdateLearn(bcmx_l2_addr_t *bcmx_l2_addr, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "MacUpdateLearn: New MAC [%02X:%02X:%02X:%02X:%02X:%02X] with VID %d, GPORT 0x%08X, flags 0x%x",
  ...
  */
  ...
  {
    LOG_PT_WARN(LOG_CTX_HAPI, "MacUpdateLearn: VID %d, GPORT 0x%08X, flags 0x%x",
                  bcmx_l2_addr->vid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);

    #if 1
    if (bcmx_l2_addr->flags & BCM_L2_MOVE)
    {
      LOG_PT_TRACE(LOG_CTX_HAPI, " Decrease learned mac ");
    ...
    else
    {
      ...
      {
        LOG_PT_TRACE(LOG_CTX_HAPI, " Increase learned mac ");
      ...
      else
      {
        ...
        LOG_PT_TRACE(LOG_CTX_HAPI, " Warning ");
        LOG_PT_WARN(LOG_CTX_HAPI, "MAC limit has been reached for VID %d, GPORT 0x%08X",
...
void hapiBroadAddrMacUpdateAge(bcmx_l2_addr_t *bcmx_l2_addr, DAPI_t *dapi_g)
{
  ...
  LOG_PT_TRACE(LOG_CTX_HAPI, "hapiBroadAddrMacUpdateAge: Aged MAC [%02X:%02X:%02X:%02X:%02X:%02X] with VID %d, GPORT 0x%08X, flags 0x%x",
            bcmx_l2_addr->mac[0], bcmx_l2_addr->mac[1], bcmx_l2_addr->mac[2], bcmx_l2_addr->mac[3], bcmx_l2_addr->mac[4], bcmx_l2_addr->mac[5], 
            bcmx_l2_addr->vid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
  ...
  if (bcmx_l2_addr->flags & BCM_L2_MOVE)
  {
    LOG_PT_TRACE(LOG_CTX_HAPI, "hapiBroadAddrMacUpdateAge: BCM_L2_MOVE flag causes event drop");
    return;
  }
      ...
      || (BCM_GPORT_IS_VLAN_PORT(bcmx_l2_addr->lport) && ageAddr == L7_TRUE))   /* PTin added: virtual ports */
  {
    ...
    */
    ...
    if (!(bcmx_l2_addr->flags & BCM_L2_TRUNK_MEMBER))
    {
      ...
      if(hapiBroadRoboVariantCheck() != __BROADCOM_53115_ID)
      {
        LOG_PT_TRACE(LOG_CTX_HAPI, "hapiBroadAddrMacUpdateAge: VID %d, GPORT 0x%08X, flags 0x%x",
                  bcmx_l2_addr->vid, bcmx_l2_addr->lport, bcmx_l2_addr->flags);
