                         ...
                         L7_uint32 Wait, L7_uint32 Priority)
{
  ...
  if (Size <= osapiMsgQ->msg_len)
  {
    ...
    if ((osapiMsgQ->flags & MSGQ_DELETED) != 0)
    {
      PT_LOG_ERR(LOG_CTX_MISC,"Message deleted");
      rc = L7_ERROR;
                            ...
                            L7_uint32 Size, L7_uint32 Wait)
{
         ...
         && ((osapiMsgQ->flags & MSGQ_DELETED) == 0))
  {
    ...
    if (rc != L7_SUCCESS)
    {
      //PT_LOG_CRITIC(LOG_CTX_MISC,"rc=%d (osapiMsgQ->send_wait.count=%u)", rc, osapiMsgQ->send_wait.count);    /* PTin added: Debug */
      break;
    }
  }
  ...
  if ((osapiMsgQ->flags & MSGQ_DELETED) != 0)
  {
    ...
    PT_LOG_CRITIC(LOG_CTX_MISC,"rc=%d",rc);      /* PTin added: Debug */
