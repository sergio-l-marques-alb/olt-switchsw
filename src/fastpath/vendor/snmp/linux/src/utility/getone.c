...
/*
 * Getone.c - program to send get-requests and process get-responses
...
getone.c
     ...
     Getone is an SNMP application to retrieve a set of 
     ...
     , and the fully qualified variable
     name(s) expressed as either dot-notation or the variable name
     as it appears in the MIB document. It should be
     noted that since the function is Get, as opposed to a GetNext, 
     the variable MUST be fully qualified for the request
     to be successful. For instance

     getone -v[1|2c] suzzy topSecret sysDescr.0 ifNumber.0     (SNMPv1 community string)
                    or
     getone -v3 localhost Guest sysDescr.0  ifNumber.0     (SNMPv3 username)

     would return the variables sysDescr.0 and ifNumber.0

     but the call

     getone -v[1|2c] suzzy public system
                or
     getone -v3 localhost Guest system
    ...
    char           *argv[];
{
    ...
    req_pdu_ptr = MakePdu(GET_REQUEST_TYPE, global_req_id, 0L, 0L,
			   NULL, NULL, 0L, 0L, 0L);
  ...
  retry_snmp_request:
	...
	goto finished;
        ...
        if (print_packet_stats) {
            PrintResponseStats(GET_RESPONSE_TYPE, resp_pdu_ptr->var_bind_list, -1);
        }
