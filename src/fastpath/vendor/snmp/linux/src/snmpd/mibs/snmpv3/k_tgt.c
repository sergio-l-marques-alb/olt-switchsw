                            ...
                            doList_t *doHead, doList_t *dp)
{
    ...
    snmpTargetAddrEntry_t *entry = NULL;
    int index=-1;
 ...
 */
    ...
    snmpTargetAddrTable->tip[0].value.octet_val = proposed_new_state->snmpTargetAddrName;
    if ((index = SearchTable(snmpTargetAddrTable, EXACT)) != -1) {
        entry = (snmpTargetAddrEntry_t *) snmpTargetAddrTable->tp[index];
    }


    if( entry->snmpTargetAddrRowStatus ==
