...
OctetString *
MakeOctetString_track(string, length, file, line)
    ...
    unsigned int line;
{
    ...
    if ((os_ptr = (OctetString *) osapiMalloc_track(sizeof(OctetString) + length, L7_SNMP_COMPONENT_ID, file, line)) == NULL) {
 ...
 */
OctetString *
CloneOctetString_track(os_ptr, file, line)
    ...
    unsigned int line;
{
	...
	return (MakeOctetString_track(os_ptr->octet_ptr, os_ptr->length, file, line));
    }
}
...
OID            *
MakeOID_track(sid, length, file, line)
    const SR_UINT32 sid[];
    SR_INT32        length;
    char *file;
    unsigned int line;
{
    ...
    if ((oid = 
	 (OID *) osapiMalloc_track(sizeof(OID) + length * sizeof(SR_UINT32), L7_SNMP_COMPONENT_ID, file, line)) == NULL) {
...
OctetString *
MakeOctetStringFromText_track(text_str, file, line)
    const char *text_str;
    char * file;
    unsigned int line;
{
    ...
    os = MakeOctetString_track(NULL, (text_str == NULL)? 0 : strlen(text_str), file, line);
    if (os == NULL) {
        return NULL;
    }
    if (os->length == 0) {
 ...
 */
OctetString *
MakeOctetStringFromHex_track(text_str, file, line)
    const char *text_str;
    char * file;
    unsigned int line;
{
    ...
    if ((os_ptr = MakeOctetString_track(NULL, temp_len, file, line)) == NULL) {
...
UInt64 *
MakeCounter64_track(SR_UINT32 value, char *file, unsigned int line)
 {

  UInt64 *new64;

    if((new64 = (UInt64 *)osapiMalloc_track(sizeof(UInt64), L7_SNMP_COMPONENT_ID, file, line)) == NULL) {
 ...
 */
UInt64 *
MakeUInt64_track(SR_UINT32 low, SR_UINT32 high, char *file, unsigned int line)
{
    UInt64 *new64;
 ...
 */
UInt64 *
MakeUInt64FromText_track(const char *string, UInt64 *result, char *file, unsigned int line)
{
    int     i, len;
    ...
    /* --- Initialize the sum --- */
    sum = MakeUInt64_track((SR_UINT32) 0, (SR_UINT32) 0, file, line);
