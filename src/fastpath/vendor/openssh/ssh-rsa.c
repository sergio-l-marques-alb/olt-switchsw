...
#else
int
ssh_rsa_sign(int cn, const Key *key, u_char **sigp, u_int *lenp,
             const u_char *data, u_int datalen)
#endif /* L7_SSHD */
{
    const EVP_MD *evp_md;
...
#else
    nid = (datafellows[cn] & SSH_BUG_RSASIGMD5) ? NID_md5 : NID_sha1;
#endif /* L7_SSHD */
...
#else
int
ssh_rsa_verify(int cn, const Key *key, const u_char *signature, u_int signaturelen,
    const u_char *data, u_int datalen)
#endif /* L7_SSHD */
{
    Buffer b;
