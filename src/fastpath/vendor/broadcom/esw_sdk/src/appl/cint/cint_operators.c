/*
 * $Id: cint_operators.c 1.37 Broadcom SDK $
...
{    
    ...
    if(lflags && rflags) {
        ...
        switch(lflags & CINT_ATOMIC_TYPE_FLAGS_MASK)
            {
            case CINT_ATOMIC_TYPE_F_CHAR:         cint_char_set(left, cint_char_value(right)); return left; 
    ...
    if( (left->dt.desc.pcount == 0) &&
        ...
        (((ast->utype.operator.right) && 
         ...
         (right && (right->flags & CINT_VARIABLE_F_CSTRING)))) {
        /* 
        ...
        if(left->dt.cap && left->dt.cap->assign) {
            ...
            if(rc != CINT_E_NONE) {
                cint_ast_error(ast, rc, "datatype '%s' assignment error", cint_datatype_format(&left->dt,0)); 
                return NULL; 
            }
        }
        else {
            cint_ast_error(ast, CINT_E_UNSUPPORTED, "datatype '%s' from '%s' has no assignment function.", 
                           cint_datatype_format(&left->dt, 0), left->dt.type); 
            return NULL; 
        }
    ...
    if( (left->dt.flags & CINT_DATATYPE_F_FUNC_POINTER) ) {
        ...
        if(right->dt.flags & CINT_DATATYPE_F_FUNC) {
            ...
            CINT_MEMCPY(left->data, &left->dt.basetype.fpp->cb, sizeof(left->data)); 
