...
#define ATP_DEBUG(flags, stuff) TKS_DEBUG((flags) | TKS_DBG_ATP, stuff)
#define ATP_OUT(stuff) TKS_DEBUG(TKS_DBG_ATP, stuff)
#define ATP_VERB(stuff) ATP_DEBUG(TKS_DBG_VERBOSE, stuff)
#define ATP_VVERB(stuff) ATP_DEBUG(TKS_DBG_ATPV, stuff)
#define ATP_WARN(stuff) ATP_DEBUG(TKS_DBG_WARN, stuff)
#define ATP_ERR(stuff) ATP_DEBUG(TKS_DBG_ERR, stuff)
#else
#define ATP_DEBUG(flags, stuff)
#define ATP_OUT(stuff)
#define ATP_VERB(stuff)
#define ATP_VVERB(stuff)
#define ATP_WARN(stuff)
#define ATP_ERR(stuff)
...
_atp_key_add(cpudb_key_t key)
{
    ...
    for (idx = 0; idx < CPUDB_CPU_MAX; idx++) {
        if (!(_atp_cpu_info[idx].flags & _ATP_CPU_VALID)) {
            ...
            ATP_VERB(("ATP: Adding CPU %d " CPUDB_KEY_FMT_EOLN,
                      idx,
                      CPUDB_KEY_DISP(key)));
            if (idx >= atp_cpu_max) {
                atp_cpu_max = idx+1;
...
atp_tx_trans_delete(_atp_tx_trans_t *tx_trans)
{
        ...
        if (tx_trans->prev != NULL) {
            ...
            if (TX_TRANS_NO_ACK(tx_trans)) {
                ...
                if (CPU_VALID_IDX(cpu)) {
                    ...
                    ATP_VERB(("ATP: invalid cpu index %d\n", cpu));
                }
            }
        }
    ...
    if ((tx_trans->tx_sem != NULL) &&
        ...
        ATP_VVERB(("TT delete deferred %p\n", tx_trans));
	tx_trans->flags &= ~_ATP_TX_F_SEM_WAITING;
	sal_sem_give(tx_trans->tx_sem);
        return;
    }
    ATP_VVERB(("TT delete %p\n", tx_trans));
...
_atp_cpu_purge(int idx)
{
    ...
    FOREACH_CLIENT(client, bkt) {
        if (client->cpu[idx].cpu_flags & ATP_CPU_ACK_PENDING) {
            ATP_WARN(("ATP: purge cpu %d while ack pending\n", idx));
        }
        ...
        if (client->cpu[idx].rx_tail != NULL) {
            ATP_WARN(("ATP: purge cpu %d while RX trans pending\n", idx));
        }
                 ...
                 _atp_rx_trans_t *rx_trans)
{
    ...
    if (rx_trans->flags & _ATP_RX_F_LOOPBACK) {
        ATP_VVERB(("ATP RX: Loopback packet cli %d\n", client->client_id));
        ...
        if (client->flags & ATP_F_REASSEM_BUF) {
            ...
            if (pkt->alloc_ptr == NULL) {
                ATP_WARN(("ATP RX: Failed to alloc for reassem\n"));
                do_callback = FALSE;
...
_handle_rx_data(_atp_pkt_data_t *pkt_p)
{
    ...
    if (src_cpu < 0) {
        if (_atp_flags & ATP_F_LEARN_SLF) {
            ...
            if (src_cpu < 0) {
                ATP_VERB(("ATP pkt: could not add key\n"));
                ++mem_rx_drop;
                return BCM_RX_HANDLED;
            }
        } else {
            ATP_VERB(("ATP pkt: SLF drop\n"));
            ++slf_rx_drop;
            return BCM_RX_HANDLED;
        }
    }
...
atp_rx_thread(void *cookie)
{
    ...
    ATP_VERB(("ATP: RX Thread starting\n"));
    ...
    ATP_VERB(("ATP: RX Thread exiting\n"));
    sal_thread_exit(0);
}
                    ...
                    int pkt_len, _atp_hdr_t *_atp_hdr, int loopback)
{
    ...
    ATP_VVERB(("ATP new RX, cpu %d, new seq %d, cli %d, old seq %d,"
               "flags: %sseen\n",
    ...
    if (!loopback) {
        if (_atp_hdr->segment >= _atp_hdr->tot_segs) {
            ATP_WARN(("ATP RX: Bad seg num %d >= tot %d.\n",
    ...
    if (rx_trans == NULL) {
        ATP_VERB(("ATP RX freelist empty\n"));
    ...
    if (pkt == NULL) {
        ATP_ERR(("ATP RX Could not allocate packet\n"));
           ...
           uint8 *pkt_buf, int pkt_len)
{
    ...
    ATP_VVERB(("Sending ACK to %d cli %d seq %d bytes %d\n", cpu,
    ...
    if (rv != BCM_E_NONE) {
#ifdef BROADCOM_DEBUG
        {
            ...
            ATP_VERB(("ATP: Failed sending ACK to %x:%x for protocol %d (%d): %s\n",
                 ...
                 uint8 *pkt_buf, int pkt_len)
{
    ...
    if (tail_trans != NULL) {
        ...
        if (!(tail_trans->flags & _ATP_RX_F_DATA_READY)) {
            ...
            ATP_WARN(("ATP RX Dropping non-ready rx trans, cpu %d, "
...

{
    ...
    ATP_VVERB(("ATP Ongoing RX: client %d. cpu %d. seq %d\n",
...
atp_ack_handle(int src_cpu, _atp_hdr_t *_atp_hdr)
{
    ...
    ATP_VVERB(("ATP ACK from %d cli %d. seq %d. bytes %d. tot %d\n", src_cpu,
    ...
    if (client == NULL) {
        ATP_WARN(("ATP: ACK on NULL client %d\n", _atp_hdr->client_id));
        return BCM_RX_HANDLED;
    }
                ...
                _atp_hdr_t *_atp_hdr)
{
    ...
    client = client_find(_atp_hdr->client_id);
    if (client == NULL) {
        ATP_VERB(("ATP rx: Unknown client id %d\n", _atp_hdr->client_id));
        return BCM_RX_NOT_HANDLED;
    }

    if (!CPU_VALID_IDX(src_cpu)) {
        ATP_VERB(("ATP rx: invalid cpu index %d\n", src_cpu));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    if ((_atp_cpu_info[src_cpu].flags & _ATP_RX_CXN_INIT) &&
        ...
        if ((!(client->cpu[src_cpu].cpu_flags & ATP_CPU_RX_TRANS_SEEN)) ||
            ...
            ATP_VERB(("ATP: old RX data SN %d. Diff %d. cpu %d\n",
                      new_seq_num,
                      seq_num_diff,
                      src_cpu));
            ++old_rx_trans_drop;
    ...
    if (pkt != NULL) {
        if (_atp_hdr->seq_num != BET_RX_SEQ_NUM(pkt)) {
            ...
            */
            ...
            ATP_WARN(("ATP BET bad segment index\n"));
            bet_rx_pkt_drop(client, src_cpu, TRUE);
        } else {
...
bet_data_handle(int src_cpu, uint8 *pkt_buf, int len, _atp_hdr_t *_atp_hdr)
{
    ...
    client = client_find(_atp_hdr->client_id);
    if (client == NULL) {
        ATP_VERB(("BET rx: Unknown client id %d\n", _atp_hdr->client_id));
        return BCM_RX_NOT_HANDLED;
    }

    if (!CPU_VALID_IDX(src_cpu)) {
        ATP_VERB(("BET rx: invalid cpu index %d\n", src_cpu));
        return BCM_RX_NOT_HANDLED;
    }
...
enqueue_atp_data(uint8 *pkt_buf, int len, uint32 flags)
{
    ...
    default:                       /* Uh oh. */
        ATP_ERR(("ATP: Bad packet opcode: %d\n", opcode));
        return -1;
    }
...
_atp_rx_callback(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    if (!_atp_running) {
        ATP_VVERB(("ATP pkt in, not running\n"));
        return BCM_RX_NOT_HANDLED;
    }
    ...
    if (mplx_num != ATP_PKT_TYPE) {
        ATP_VVERB(("ATP pkt in, bad pkt type %d\n", mplx_num));
        return BCM_RX_NOT_HANDLED;
    }
                       ...
                       uint8* pkt_buf, int len, void *cookie)
{
    ...
    if (!_atp_running) {
        ATP_VVERB(("BETNH pkt in, not running\n"));
        return BCM_RX_NOT_HANDLED;
    }
                     ...
                     void *cookie)
{
    ...
    if (tot_bytes > ATP_MTU) {
        ATP_ERR(("ATP TX:  Packet too big (%d bytes)\n",
                   tot_bytes));
        return NULL;
    }
              ...
              uint8 *pkt_buf, int len, uint32 ct_flags)
{
    ...
    ATP_VVERB(("ATP TX: Loopback packet len %d, "
               "flgs 0x%x\n", len, ct_flags));
    ...
    if (rx_buf == NULL) {
        ...
        ATP_WARN(("ATP LB Could not alloc RX pkt\n"));
    ...
    if (new_trans == NULL) {
        ...
        ATP_WARN(("ATP LB Could not alloc RX trans %d\n", alloc_len));
        return BCM_E_RESOURCE;
    }
          ...
          uint32 rco_flags)
{
    ...
    if (!next_hop_running()) {
        ATP_WARN(("ATP Warning:  next hop is not running\n"));
    }
    rv = next_hop_register(_atp_next_hop_callback, NULL, ATP_PKT_TYPE);
    if (rv < 0) {
        ATP_WARN(("ATP Warning:  cannot register with next hop\n"));
    }
    ...
    rv = cputrans_rx_bmp_register(unit_bmp, "atp", _atp_rx_callback,
                                  ...
                                  rco_flags);
    if (rv != BCM_E_NONE) {
        ATP_WARN(("ATP Warning: Could not register RX %d: %s\n",
                  rv, bcm_errmsg(rv)));
    }
...
atp_db_update(cpudb_ref_t db_ref)
{
    ...
    if (rv == BCM_E_NONE) {
        ATP_VERB(("ATP: Updating c2c db\n"));
        rv = c2c_cpu_update(db_ref);
    } else if (rv == BCM_E_MEMORY) {
        ATP_ERR(("ATP ERROR:  Too many CPUs in DB\n"));
    } else if (rv == BCM_E_RESOURCE) {
        ATP_WARN(("ATP WARN: Failed to add CPU key\n"));
    }
    return rv;
}
...
_atp_end_threads(int retries)
{
    ...
    if (atp_tx_thread_id != SAL_THREAD_ERROR ||
        ...
        if (atp_rx_thread_id != SAL_THREAD_ERROR ||
            atp_tx_thread_id != SAL_THREAD_ERROR) {
            if (atp_tx_thread_id != SAL_THREAD_ERROR) {
                ATP_WARN(("Warning:  ATP TX thread did not exit\n"));
            }
            if (atp_rx_thread_id != SAL_THREAD_ERROR) {
                ATP_WARN(("Warning:  ATP RX thread did not exit\n"));
            }
            return BCM_E_FAIL;
        }
    }
...
atp_timeout_set(int retry_usecs, int num_retries)
{
    if (retry_usecs < ATP_RETRY_TIMEOUT_MIN) {
        ATP_WARN(("ATP Warning: changing retry timeout "
    ...
    if (num_retries < 1) {
        ATP_WARN(("ATP Warning: changing retry count from "
                  "%d to %d\n", num_retries, 1));
        atp_retry_count = 1;
    } else {
...
#define RX_ALLOC_CHECK(id, bytes)                                       \
    do {                                                                \
        (id) = sal_dma_alloc((bytes), "ATP");                           \
        if ((id) == NULL) {                                             \
            atp_cleanup();                                              \
            ATP_UNLOCK;                                                 \
            return BCM_E_MEMORY;                                        \
...
_atp_init(void)
{
    ...
    RX_ALLOC_CHECK(ack_pkt_data, bytes);
...
_atp_tx_trans_send(_atp_tx_trans_t *trans)
{
    ...
    if (!no_ack) {  /* Use ACK */
        ATP_VVERB(("TX ATP send cli %d, seq %d txcount %d lasttx %u\n",
    ...
    if (TX_TRANS_NO_ACK(trans)) { /* Best effort */
        ...
        if (callback == NULL) { /* Sync send; create semaphor */
            ...
            if (trans->tx_sem == NULL) {
                atp_tx_trans_delete(trans);
                *trans_deleted = TRUE;
                ATP_ERR(("ATP TX:  Failed to create sem\n"));
                return BCM_E_MEMORY;
            }
                 ...
                 void *cookie)
{
    ...
    ATP_DEBUG(TKS_DBG_VERBOSE + TKS_DBG_TX,
              ("ATP simple %d:  NH %d. ctf %x. cb %p\n",
...
tx_done_handle(_atp_client_t *client, int cpu, _atp_tx_trans_t *trans)
{
    ...
    if (trans->flags & _ATP_TX_F_TIMEOUT) {
        ...
        ATP_ERR(("ATP: TX timeout, seq %d. cli %d. to %d(mac %x:%x:%x:%x:%x:%x) tx cnt %d.\n",
                 ...
                 trans->tx_count));
#else
        ATP_ERR(("ATP: TX timeout, seq %d. " CPUDB_KEY_FMT 
                 " cli %d. to %d tx cnt %d.\n",
                 trans->_atp_hdr.seq_num,
    ...
    FOREACH_CLIENT(client, idx) {
        for (cpu = 0; cpu < atp_cpu_max; cpu++) {
            ...
            while (trans != NULL) {
                ...
                if (trans->flags & _ATP_TX_F_DONE &&
                        !(trans->flags & _ATP_TX_F_PENDING)) {
                    ATP_VVERB(("TX done cli %d, to %d, seq %d\n",
...
_handle_tx_data(_atp_pkt_data_t *pkt_p)
{
    ...
    src_cpu = _atp_key_lookup(src_key);
    if (src_cpu < 0) {
        ATP_VERB(("ATP ACK pkt: could not find source CPU key\n"));
        return BCM_RX_HANDLED;
    }
...
atp_tx_thread(void *cookie)
{
    ...
    ATP_VERB(("ATP: TX Thread starting\n"));
    ...
    atp_tx_thread_id = SAL_THREAD_ERROR;
    ATP_VERB(("ATP: TX Thread exiting\n"));
    sal_thread_exit(0);
}
       ...
       void *cookie)
{
    ...
    if (dest_cpu >= 0) {  /* Found dest CPU */
        if (_atp_cpu_info[dest_cpu].override_tx != NULL) { /* Override TX */
            ...
            ATP_TX_UNLOCK;
            ATP_VVERB(("ATP tx calling override %d\n", dest_cpu));
    ...
    if (client == NULL) {  /* Client must exist */
        ATP_TX_UNLOCK;
        TKS_DEBUG(TKS_DBG_VERBOSE | TKS_DBG_WARN,
                  ("ATP TX: Client not found\n"));
        return BCM_E_NOT_FOUND;
    }

    ATP_VVERB(("TX cli %d, flags 0x%x ctflags 0x%x, cb %p to "
               CPUDB_KEY_FMT_EOLN, 
    ...
    if (trans == NULL) {
        ATP_TX_UNLOCK;
        ATP_VERB(("TX unable to alloc trans/pkt, cli %d\n", client_id));
        return BCM_E_RESOURCE;
    }
...
{
    if (attach) {
        ATP_VERB(("ATP attach unit %d; key " CPUDB_KEY_FMT_EOLN,
        ...
        if (!(cpuent->flags & CPUDB_F_IS_LOCAL)) {
            ATP_VERB(("ATP detach unit %d; key " CPUDB_KEY_FMT_EOLN,
                      unit, CPUDB_KEY_DISP(cpuent->base.key)));
...
atp_rx_inject(cpudb_key_t src_key, int client_id, uint8 *pkt_buf, int len)
{
    ...
    client = client_find(client_id);
    if (client == NULL) {
        ATP_WARN(("ATP rx inject: Unknown client id %d\n", client_id));
        ATP_RX_UNLOCK;
        return BCM_RX_HANDLED;
    }
...
{
    ATP_OUT(("tx_retry_cnt              = %d\n", tx_retry_cnt));
    ATP_OUT(("tx_timeout_cnt            = %d\n", tx_timeout_cnt));
    ATP_OUT(("reassem_alloc_fail        = %d\n", reassem_alloc_fail));
    ATP_OUT(("rx_trans_fail             = %d\n", rx_trans_fail));
    ATP_OUT(("stale_rx_trans            = %d\n", stale_rx_trans));
    ATP_OUT(("rx_pkt_drops              = %d\n", rx_pkt_drops));
    ATP_OUT(("ack_pkt_drops             = %d\n", ack_pkt_drops));
    ATP_OUT(("stale_rx_trans            = %d\n", stale_rx_trans));
    ATP_OUT(("rxt_pkt_alloc_fail        = %d\n", rxt_pkt_alloc_fail));
    ATP_OUT(("rx_mseg_alloc_fail        = %d\n", rx_mseg_alloc_fail));
    ATP_OUT(("tx_trans_fail             = %d\n", tx_trans_fail));
    ATP_OUT(("txt_pkt_alloc_fail        = %d\n", txt_pkt_alloc_fail));
    ATP_OUT(("lb_buf_alloc_fail         = %d\n", lb_buf_alloc_fail));
    ATP_OUT(("gc_deferrals              = %d\n", gc_deferrals));
    ATP_OUT(("gc_blocked                = %d\n", gc_blocked));
    ATP_OUT(("cli_del_tx_busy           = %d\n", cli_del_tx_busy));
    ATP_OUT(("clients_deleted           = %d\n", clients_deleted));
}
...
atp_dump(int verbose)
{
    ...
    ATP_OUT(("Init %d. run %d.\n",
             init_done, _atp_running));
    if (verbose) {
        for (i = 0; i < CPUDB_CPU_MAX; i++) {
            if (CPU_VALID(i)) {
                ATP_OUT(("  CPU %d " CPUDB_KEY_FMT_EOLN,
                         i, CPUDB_KEY_DISP(CPU_KEY(i))));
            }
        }
    }

    ATP_OUT (("atp_tx_mutex %p\n", atp_tx_mutex));
    ATP_OUT (("atp_rx_mutex %p\n", atp_rx_mutex));
    ATP_OUT (("atp_rxq_mutex %p\n", atp_rxq_mutex));
    ATP_OUT (("atp_tx_sem: %p\n", atp_tx_sem));
    ATP_OUT (("atp_rx_sem: %p\n", atp_rx_sem));

    ATP_OUT(("tx_pending %d. sleep cnt %d.\n",
             atp_tx_pending, tx_sleep_count));
    ATP_OUT(("Cntrs: rxt_cr %d. rxt_free %d. rx raw free %d.\n",
             rxt_create, rxt_free, rxraw_free));
    ATP_OUT(("  txt_cr %d\n",
             txt_create));
    ATP_OUT(("  rx raw grab %d. tx raw grab %d.\n",
             rxraw_grab, txraw_grab));
    ATP_OUT(("Drops: bet %d. atp %d. mem %d. slf %d. "
             "old rx %d.\n",
             bet_rx_drop, atp_rx_drop, mem_rx_drop, slf_rx_drop,
             old_rx_trans_drop));

    ATP_OUT(("tx free %p. rx free %p.\n", tx_trans_freelist,
             rx_trans_freelist));
        ...
        if (verbose) {
            for (cpu = 0; cpu < CPUDB_CPU_MAX ; cpu++) {
                ...
                for (rx_trans = client->cpu[cpu].rx_trans;
                    ...
                    ATP_OUT(("    CPU %d: RX %p: flags 0x%x. cpu %d. rsegs %d. "
                             "ack %d.\n", cpu, rx_trans, rx_trans->flags,
                             rx_trans->src_cpu, rx_trans->rcv_segs,
                             rx_trans->ack_count));
                    ATP_OUT(("      time %u. cli %d. pkt %p. len %d. seq %d\n",
                ...
                for (tx_trans = client->cpu[cpu].tx_trans;
                    ...
                    ATP_OUT(("    CPU %d: TX %p: flags 0x%x. ct_flags 0x%x. "
                             "cpu %d. len %d.\n", cpu, tx_trans,
                             tx_trans->flags, tx_trans->ct_flags,
                             tx_trans->dest_cpu, tx_trans->len));
                    ATP_OUT(("      b ack %d. last tx %u. tx_rv %d. "
    ...
    ATP_OUT(("BET queue %p tail %p\n", bet_queue, bet_queue_tail));
    ATP_OUT(("atp_cpu_max: %d\n", atp_cpu_max));
