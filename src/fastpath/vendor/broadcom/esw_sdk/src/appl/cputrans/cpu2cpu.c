/*
 * $Id: cpu2cpu.c 1.35 Broadcom SDK $
...
#define C2C_OUT(stuff) TKS_DEBUG(TKS_DBG_NORMAL, stuff)
#define C2C_DEBUG(stuff) TKS_DEBUG(TKS_DBG_C2C, stuff)
#define C2C_VERY_VERBOSE(stuff)  TKS_DEBUG(TKS_DBG_C2CV, stuff)
#else
#define C2C_OUT(stuff)
#define C2C_DEBUG(stuff)
#define C2C_VERY_VERBOSE(stuff)
...
_c2c_cpu_add(const cpudb_entry_t *db_ent)
{
    ...
    C2C_DEBUG(("C2C: add key " CPUDB_KEY_FMT ", flags 0x%x (%d,%d,%d,%d)\n",
...
c2c_cpu_add(const cpudb_entry_t *db_ent)
{
    ...
    C2C_INIT;
    C2C_DEBUG(("C2C added " CPUDB_KEY_FMT_EOLN,
...
c2c_cpu_remove(const cpudb_key_t key)
{
    ...
    C2C_INIT;
    C2C_DEBUG(("C2C removed " CPUDB_KEY_FMT_EOLN, CPUDB_KEY_DISP(key)));
...
c2c_cpu_update(cpudb_ref_t db_ref)
{
    ...
    if (db_ref == CPUDB_REF_NULL) { /* Clear out DB completely */
        ...
        CPUDB_FOREACH_ENTRY(c2c_db_ref, entry) {
            ...
            if (new_ref_entry == NULL) {
                if (entry == c2c_db_ref->local_entry) {
                    C2C_DEBUG(("C2C: Update error, can't remove local\n"));
                    continue;
                }
        ...
        for (i = 0; i < count; i++) {
            ...
            if (rv < 0) {
                C2C_DEBUG(("C2C: Failed to remove entry on update\n"));
                break;
            }
        }
        ...
        CPUDB_FOREACH_ENTRY(db_ref, entry) {
            ...
            if (rv < 0) {
                C2C_DEBUG(("C2C: Failed to add entry on update\n"));
                break;
            }
        }
    }
    ...
    uint32 flags)
{
    ...
    if (local_mac != NULL) {
        C2C_DEBUG(("c2c local mac set is deprecated.\n"));
    }
    ...
    int *rvp)
{
...
#if defined(BROADCOM_DEBUG)
        C2C_DEBUG(("c2c: Could not find %s%s%s for "
                   CPUDB_KEY_FMT_EOLN,
    ...
    if (!(db_ent->flags & CPUDB_F_TX_KNOWN)) {
        C2C_UNLOCK;
        C2C_DEBUG(("c2c: Unknown transmit port (flags 0x%x) for dest key "
                   CPUDB_KEY_FMT_EOLN,
                   db_ent->flags,
                   CPUDB_KEY_DISP(dest_key)));
        *rvp = BCM_E_INTERNAL;
    ...
    pkt = cputrans_tx_pkt_list_alloc(pkt_buf, len, seg_len, ct_flags,
                                     tot_segs);
    if (pkt == NULL) {
        C2C_DEBUG(("c2c: Could not alloc cputrans pkt\n"));
        *rvp = BCM_E_MEMORY;
        return NULL;
    }
#if defined(BROADCOM_DEBUG)
    ...
    C2C_VERY_VERBOSE(("c2c: pkt from (%d.%d) to (%d.%d) "
    ...
    cpudb_key_t dest_key)
{
    ...
    if (!init_done) {
        if (c2c_init() != BCM_E_NONE) {
            C2C_DEBUG(("c2c update. init failed\n"));
            return BCM_E_INIT;
        }
    }
    ...
    if (db_ent == NULL || c2c_db_ref->local_entry == NULL) {
        C2C_UNLOCK;
        C2C_DEBUG(("c2c update: key %sfound, local_entry %sNULL, dest key "
                   CPUDB_KEY_FMT_EOLN,
                   (db_ent == NULL) ? "not " : "",
    ...
    if (!(db_ent->flags & CPUDB_F_TX_KNOWN)) {
        C2C_UNLOCK;
        C2C_DEBUG(("c2c update: Unknown tx port (flags 0x%x) for dest key "
                   CPUDB_KEY_FMT_EOLN,
                   db_ent->flags,
                   CPUDB_KEY_DISP(dest_key)));
        return BCM_E_NOT_FOUND;
    ...
    void *cookie)
{
    ...
    if (setup_fn != NULL) {
        ...
        if (rv < 0) {  /* Problem packet configuration */
            C2C_DEBUG(("c2c tx: Error setting up packet\n"));
            return rv;
        }
    }
    ...
    void *cookie)
{
    ...
    if (rv != BCM_E_NONE) {
        C2C_DEBUG(("c2c send. can't create (%s)\n",bcm_errmsg(rv)));
        return rv;
    }
                  ...
                  uint16 *mplx_num)
{
    ...
    C2C_VERY_VERBOSE(("c2c: pkt is %srecognized. type, %d\n",
             ...
             uint16 *mplx_num)
{
    ...
    if (sal_memcmp(&pkt_data[CPUTRANS_SNAP_OFS], c2c_snap_mac,
                   sizeof(bcm_mac_t))) {
        C2C_VERY_VERBOSE(("c2c: Not SNAP mac\n"));
        return 0;
    }
    ...
    if (snap_type != c2c_snap_type) {
        C2C_VERY_VERBOSE(("c2c: Wrong SNAP type 0x%04x\n",
                          snap_type));
        return 0;
    }
    ...
    if (trans_type != c2c_trans_type) {
        C2C_VERY_VERBOSE(("c2c: Wrong trans type 0x%04x\n",
                          trans_type));
        return 0;
    }
...
c2c_dump(void)
{
    ...
    if (c2c_db_ref == CPUDB_REF_NULL) {
        ...
        CPUDB_FOREACH_ENTRY(c2c_db_ref, entry) {
            C2C_OUT(("    CPU Key " CPUDB_KEY_FMT ", flags 0x%x\n",
