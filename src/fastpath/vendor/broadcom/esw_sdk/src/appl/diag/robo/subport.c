/*
 * $Id: subport.c 1.13 Broadcom SDK $ 
...
if_robo_subport(int unit, args_t *a)
{
    ...
    if (!soc_feature(unit, soc_feature_subport)) {
        printk("%s: MSG: Feature not supported!\n", ARG_CMD(a));
        return CMD_FAIL;
    }
    ...
    if (sal_strcasecmp(subcmd, "show") == 0) {
...
#ifdef  BCM_TB_SUPPORT
        if (!SOC_IS_TBX(unit)){
            printk(" 'Subport' on this chip is not designed yet!\n");
            return CMD_OK;
        }
            ...
            for (i = 0; i < _TB_SUBPORT_NUM_GROUP_PER_PORT; i++){
                ...
                if (rv != BCM_E_NONE){
                    if (rv == BCM_E_NOT_FOUND){
                        ...
                        printk(" %s,ERROR:port=0x%x,mc_group=0x%x, %s\n", 
                                ARG_CMD(a), p, i, bcm_errmsg(rv));
                        goto bcm_err;
                    }
                }
                printk("   SubportGroup ID=0x%04x(GPORT=0x%08x) : \n",
                        mcrep_id, gport);
                printk("\t Port=0x%x,VLAN(SP)=0x%x,prop_flag=0x%08x\n", 
        ...
        subcmd = ARG_GET(a);
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing group subcommand\n", ARG_CMD(a));
            return CMD_USAGE;
        }
        ...
        if (sal_strcasecmp(subcmd, "create") == 0 ) {
            ...
            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing mcrep_id!\n\n");
                return CMD_USAGE;
            }
            mcrep_id = parse_integer(c);
            
            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing port_id!\n\n");
                return CMD_USAGE;
            }
            port = parse_integer(c);
                
            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing svid!\n\n");
                return CMD_USAGE;
            }
...
#ifdef BCM_TB_SUPPORT
            if ((c = ARG_GET(a)) == NULL) {
                printk("  Default stag_type set to 0!\n\n");
                tag_type = 0;
            } else {
            ...
            if (tag_type == 0){
                ...
                printk("  Incorrect tag_type(%d) assignment!\n\n",tag_type);
                return CMD_USAGE;
            }
#endif  /* BCM_TB_SUPPORT */
...
#else  /* BCM_TB_SUPPORT */
            printk("  ERROR: Internal problem on parsing !\n\n");
#endif  /* BCM_TB_SUPPORT */
            ...
            printk("  System basis subport group id 0x%x destroied\n",
                    mcrep_id);
            return CMD_OK;
        ...
        subcmd = ARG_GET(a);
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing vport subcommand\n", ARG_CMD(a));
            return CMD_USAGE;
        }
        ...
        if (sal_strcasecmp(subcmd, "add") == 0 ) {
            ...
            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing CVID(0-4095)!\n\n");
                return CMD_USAGE;
            }
            cvid = parse_integer(c);

            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing pri(priority)!\n\n");
                return CMD_USAGE;
            }
            pri = parse_integer(c);
...
#ifdef BCM_TB_SUPPORT
            if ((c = ARG_GET(a)) == NULL) {
                printk("  Default ctag_type and pcp_remark set to 0!\n\n");
                tag_type = 0;
                pcp_mark = 0;
            } else {
                tag_type = parse_integer(c);
                ...
                if ((c = ARG_GET(a)) == NULL) {
                    printk("  Default pcp_remark set to 0!\n\n");
                    pcp_mark = 0;
                } else {
            ...
            if (pcp_mark == 0){
                ...
                printk("  Incorrect pcp_remark(%d) assignment!\n\n",
                        pcp_mark);
                return CMD_USAGE;
            }
#endif  /* BCM_TB_SUPPORT */
...
#ifdef  BCM_TB_SUPPORT
            printk("  vport add into subport group 0x%x with ID=%d\n\t"
            ...
            if ((c = ARG_GET(a)) == NULL) {
                printk("  ERROR: Missing vport_id(0-15)!\n\n");
                return CMD_USAGE;
            }
            ...
            printk("  System basis vport id 0x%x removed!!\n", vp_id);
            return CMD_OK;
        } else {
            return CMD_USAGE;
        }
