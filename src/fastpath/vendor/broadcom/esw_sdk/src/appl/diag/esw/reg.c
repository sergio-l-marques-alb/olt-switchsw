/* 
 * $Id: reg.c 1.18.18.1 Broadcom SDK $
 ...
 */
int
_reg_get_by_type(int unit, sal_vaddr_t vaddr, soc_regtype_t regtype,
 ...
 */
int
_reg_get_extended_by_type(int unit, int port, int block, uint32 regaddr, 
                          ...
                          uint64 *outval, uint32 flags)
{
    ...
    if (!soc_feature(unit, soc_feature_new_sbus_format)) {
        return _reg_get_by_type(unit, regaddr, regtype, outval, flags);
    }
 ...
 */
int
_reg_set_by_type(int unit, sal_vaddr_t vaddr, soc_regtype_t regtype,
		 uint64 regval)
{
    int             rv = CMD_OK, r;
    uint32          regaddr = (uint32)vaddr;
 ...
 */
int
_reg_set_extended_by_type(int unit, int port, int block, uint32 regaddr, 
                          soc_regtype_t regtype, uint64 regval)
{
    ...
    if (!soc_feature(unit, soc_feature_new_sbus_format)) {
        return _reg_set_by_type(unit, regaddr, regtype, regval);
    }
    COMPILER_64_TO_32_LO(val32, regval);
...
cmd_esw_reg_get(int unit, args_t *a)
{
    ...
    if (isint(name)) {		       /* Numerical address given */
        ...
        rv = _reg_get_by_type(unit, regaddr, rt->type, &regval, flags);
    } else {
        if (*name == '$') {
            name++;
        }
        ...
        if (rt->type == soc_cpureg) {
            if (parse_cmic_regname(unit, name, &regaddr) < 0) {
                ...
                rv = _reg_get_by_type(unit, regaddr, rt->type,
                                      &regval, flags);
            }
...
do_reg_set(int unit, args_t *a, int mod)
{
    ...
    if (!mod && isint(name)) {
        ...
        rv = _reg_set_by_type(unit, vaddr, rt->type, regval);
    } else {
        ...
        if (rt->type == soc_cpureg) {
            ...
            regval = parse_uint64(valstr);
            rv = _reg_set_by_type(unit, regaddr, rt->type, regval);
        } else if (parse_symbolic_reference(unit, &alist, name) < 0) {
            printk("Syntax error parsing \"%s\"\n", name);
            sal_free(valstr);
            soc_regaddrlist_free(&alist);
            ...
            for (i = 0; i < alist.count; i++) {
                ...
                if (!isint(valstr)) {  /* Must modify registers */
                     ...
                     */
                    if (mod) {
                        rv = _reg_get_extended_by_type(unit, ainfo->port, ainfo->block, 
                                 ainfo->addr, SOC_REG_INFO(unit, ainfo->reg).regtype,
                                 &regval, 0);
                    } else {
                ...
                if (rv == CMD_OK) {
                    rv = _reg_set_extended_by_type(unit, ainfo->port, ainfo->block, ainfo->addr,
