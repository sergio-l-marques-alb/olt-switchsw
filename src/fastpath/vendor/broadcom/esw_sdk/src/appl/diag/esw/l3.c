...
if_l3(int unit, args_t *a)
{
	...
	case L3_SHOW:
            ...
            if (SOC_IS_TRIDENT2(unit) || SOC_IS_TRIUMPH3(unit)) {
                defip_128_max = soc_mem_index_max(unit, L3_DEFIP_PAIR_128m);
                defip_max = soc_mem_index_max(unit, L3_DEFIPm);
                if (defip_128_max > defip_max) {
                    l3_hw_status.l3info_max_route = defip_128_max;
                } else {
                    l3_hw_status.l3info_max_route = defip_max;
                }
            }
#endif
