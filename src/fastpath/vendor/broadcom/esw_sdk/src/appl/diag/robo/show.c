...
cmd_robo_show(int unit, args_t *a)
{
    ...
    if (!sal_strcasecmp(c, "unit") || !sal_strcasecmp(c, "units")) {
        ...
        if (c != NULL) {	/* specific unit */
            ...
            printk("Unit %d chip %s%s\n",
                   ...
                   soc_dev_name(u),
                   u == unit ? " (current)" : "");
            soc_robo_chip_dump(u, SOC_DRIVER(u));
        } else {		/* all units */
    ...
    if (!sal_strcasecmp(c, "vm") || !sal_strcasecmp(c, "VlanMapping")) {
        ...
        if (SOC_IS_TBX(unit) && !SOC_IS_TB_AX(unit)) {
            ...
            printk("Vlan Mapping last hit entries:\n");
            hit = fld_ivm & 0x8000;
            if (!hit) {
                printk("IVM is not hit\n");
            } else {
                printk("IVM hit entry id #%d\n", fld_ivm & 0x7fff);
            }
            hit = fld_evm & 0x8000;
            if (!hit) {
                printk("EVM is not hit\n");
            } else {
                printk("EVM hit entry id #%d\n", fld_evm & 0x7fff);
            }
        } else {
            rv = BCM_E_UNAVAIL;
            printk("%s: ERROR: %s\n", ARG_CMD(a), bcm_errmsg(rv));
