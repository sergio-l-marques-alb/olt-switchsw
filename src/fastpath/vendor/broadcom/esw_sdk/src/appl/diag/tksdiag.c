/*
 * $Id: tksdiag.c 1.89 Broadcom SDK $
...
unit_port_pair_parse(char *str, int *unit, bcm_port_t *port)
{
    ...
    *port = strtoul(str, &next, 0);
    if (next == str) {
        printk("Error parsing unit/port in %s\n", orig_str);
        return NULL;
    }
...
/* stktask/bcmx glue */
STATIC void
ct_attach_callback(int unit, int attach, cpudb_entry_t *cpuent, int cpuunit)
{
    COMPILER_REFERENCE(cpuent);
    COMPILER_REFERENCE(cpuunit);
    BCM_DEBUG(BCM_DBG_STK, ("STACK: Attach callback, unit %d. %s\n", unit,
...
ct_cpudb(int unit, args_t *args)
{
    ...
    int i;
#if defined(INCLUDE_BCMX)
    static int bcmx_registered = FALSE;
#endif
    cpudb_key_t key;
    ...
    if (!bcmx_registered) {
        int rv;
        if ((rv = bcm_stack_attach_register(ct_attach_callback)) < 0) {
            printk("Could not register BCMX with stack attach: %s\n",
                   bcm_errmsg(rv));
        } else {
            bcmx_registered = TRUE;
        }
    }
#endif
    if ((subcmd = ARG_GET(args)) == NULL) {
        printk("Current DB is %d and is%s valid\n",
    ...
    if ((sal_strcasecmp(subcmd, "NEWDB") == 0) ||
        ...
        if ((db_refs[i] = cpudb_create()) == NULL) {
            printk("ERROR: cpudb %d create failed\n", i);
        } else {
            printk("cpudb %d created\n", i);
            num_db++;
        }
    } else if (sal_strcasecmp(subcmd, "CURRENT") == 0) {
        if ((int_arg = ARG_GET(args)) == NULL) {
            printk("Current db is %d\n", cur_db);
            return CMD_OK;
        }
        cur_db = strtoul(int_arg, NULL, 0);
        printk("Current DB set to %d%s\n", cur_db,
        ...
        if ((int_arg = ARG_GET(args)) == NULL) {
            printk("Destroy requires database refence\n");
            return CMD_USAGE;
        }
        a_db = strtoul(int_arg, NULL, 0);
        if (db_refs[a_db] != NULL) {
            printk("Destroy returns %d\n", cpudb_destroy(db_refs[a_db]));
            db_refs[a_db] = NULL;
        } else {
            printk("DB %d does not exist\n", a_db);
        }
        ...
        if (db_refs[db_idx] == CPUDB_REF_NULL) {
            printk("Invalid DB index.  Try cpudb create\n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        if (db_refs[db_idx]->local_entry == NULL) {
            printk("No local DB entry in cpudb %d\n", db_idx);
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        if (port_str_parse(port_str, db_refs[db_idx]->local_entry) < 0) {
            printk("Bad port string specified: %s\n", port_str);
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        parse_arg_eq_done(&pt);
        ...
        if (entry == NULL) {
            printk("ERROR: cpudb entry %s create failed\n", keybuf);
        } else {
            printk("cpudb entry %s created\n", keybuf);
        ...
        if (entry == NULL) {
            printk("Could not find entry for key\n");
            parse_arg_eq_done(&pt);
            return CMD_FAIL;
        }
        ...
        if ((tx_unit >= 0 && tx_port < 0) || (tx_port >= 0 && tx_unit < 0)) {
            printk("Warning:  Not properly initializing tx_unit (%d) and "
                   "tx_port (%d)\n", tx_unit, tx_port);
        }
        if ((dest_mod >= 0 && dest_port < 0) ||
            (dest_port >= 0 && dest_mod < 0)) {
            printk("Warning:  Not properly initializing dest_mode (%d) and "
                   "dest_port (%d)\n", dest_mod, dest_port);
        }
        ...
        if ((entry = cpudb_mac_lookup(db_refs[cur_db], mac)) != NULL) {
            ...
            printk("Entry not found in database %d\n", cur_db);
        }
        ...
        printk("Current DB index is %d and is%s valid\n", cur_db,
        ...
        parse_arg_eq_done(&pt);
        printk("Remove returns %d\n",
               cpudb_entry_remove(db_refs[cur_db], key));
    } else if (sal_strcasecmp(subcmd, "UNREGISTER") == 0) {
#if defined(INCLUDE_BCMX)
        if (bcmx_registered) {
            int rv;

            if ((rv = bcm_stack_attach_unregister(ct_attach_callback)) < 0) {
                printk("Could not unregister BCMX with stack attach: %s\n",
                       bcm_errmsg(rv));
            } else {
                printk("cpudb BCMX callback unregistered\n");
                bcmx_registered = FALSE;
            }
        }
#else
        printk("cpudb: BCMX is not enabled\n");
#endif
    } else {
    ...
    if (flags & BCM_STK_ENABLE) {
        if (flags & BCM_STK_SL) {
            ...
            if ((rv < 0) && (rv != BCM_E_UNAVAIL)) {
                printk("STACK: Error setting CPU protocol priority"
                       " on unit %d, port %d: %s\n", unit, port,
                       bcm_errmsg(rv));
            }
        }
...
mh_opcode0_priority(int action)
{
    ...
    case MH_OPCODE0_SELECT:
        for (i = 0; i < soc_ndev; i++) {
            ...
            rv = mh_opcode0_priority_select(i,
                                            BCM_FIELD_GROUP_PRIO_ANY,
                                            &mh_opcode0_fp[i].group,
                                            &mh_opcode0_fp[i].entry,
                                            &mh_opcode0_fp[i].obm_reg_changed);
            if (BCM_FAILURE(rv)) {
                printk("ERROR: Failed to create FP rule to use HiGig packet "
                       "priority on unit %d.\n",i);
                printk("       Stacking may not work properly.\n");
                return rv;
            }
        }
        break;
...
tks_st_thread(void *cookie)
{
    ...
    if ((rv=topo_pkt_handle_init(TOPO_ATP_FLAGS)) != BCM_E_NONE) {
        printk("WARNING: topo_pkt_handle_init returned %s\n", bcm_errmsg(rv));
    }
    ...
    printk("ST thread exited with value %d (%s)\n", rv, bcm_errmsg(rv));
    st_tid = SAL_THREAD_ERROR;

    printk("ST: Stopping RPC and rlink\n");
    
    /* Detach all remote units from BCMX */
...
#define TKS_STK_THREAD_POLL_FRACTION 100
    ...
    if (bcm_st_timeout_get(BCM_STS_ATTACH,
        ...
        if (bcm_stack_attach_running()) {
            printk("Attach thread failed to signal completion\n");
        }
        
    } else {
        printk("Could not get attach timeout\n");
    }
...
null_attach(cpudb_ref_t db_ref)
{
    ...
    if (ismaster) {
	printk("STACK: master on %s (%d cpu%s)\n",
               master,
               db_ref->num_cpus, db_ref->num_cpus == 1 ? "" : "s");
    } else {
	printk("STACK: slave on %s (%d cpus, master %s)\n",
...
evlog_show(args_t *args)
{
    ...
    printk("Stack Task State Change Event Log");
    if (summary) {
        printk(" - Summary");
    }
    printk("\n");
     ...
     */
    if (summary) {
        printk("  Entry       Elapsed Time   Stack Task State    On\n");
        printk("  Start End        (usecs)    From  ==> To       Event\n");
        printk("  ----------------------------------------------------\n");
        ...
        printk("  Entry  Time(usecs)  Description   Stack Task State    "
               "On\n");
        printk("                                     From  ==> To       "
               "Event\n");
        printk("  -------------------------------------------------------"
...
tks_list_callback(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    printk("tks_list_callback(%d,%p,%p)\n",unit,(void *)pkt,cookie);
    while(pkt) {
        next = pkt->next;
        rv = tks_pkt_free(pkt->unit, pkt);
        if (BCM_FAILURE(rv)) {
            printk("tks_list_callback: tks_pkt_free()=%d\n",rv);
        }
        pkt = next;
    }
}
...
tks_pkt_callback(int unit, bcm_pkt_t *pkt, void *cookie)
{
    ...
    if (ctl == 1) {
        ...
        printk("tks_pkt_callback(%d,%p,%p)=%d\n",unit,pktp,cookie,rv);
    } else {
        printk("tks_pkt_callback [list]\n");
    }
}
...
tks_stk_task(int unit, args_t *args)
{
    ...
    if ((subcmd = ARG_GET(args)) == NULL) {
        ...
        printk("Stack Task thread is%s running, state %s, flags 0x%x\n",
        ...
        for (i = 0; i < 32; i++) {
            if (bcm_st_flags & (1 << i)) {
                if (i < BCM_STF_MAX) {
                    ...
                    printk("    Unknown flag 0x%x\n", bcm_st_flags & (1 << i));
                }
            }
        }
        printk("ST pending ev 0x%x; blocked 0x%x; last trans %u\n",
               st_blocked_events, st_pending_events,
               st_transition_time);
        printk("Stk ports with link:\n");
        for (i = 0; i < CPUDB_CXN_MAX; i++) {
            if (st_stk_port_flags[i] != 0) {
                printk("    %d%s: Last event %u. Link %s; Last link %s\n",
        ...
        if (st_state == BCM_STS_BLOCKED || st_state == BCM_STS_READY) {
            ...
            if (!cpudb_valid((disc = bcm_st_discovery_db_get()))) {
                printk("Stack task discovery DB is not valid\n");
            } else {
                printk("Stack task discovery DB:\n");
                db_dump(disc);
            }
            if (!cpudb_valid((cur = bcm_st_current_db_get()))) {
                printk("Stack task current DB is not valid\n");
            } else {
                printk("Stack task current DB:\n");
                db_dump(cur);
            }
            (void)cpudb_destroy(disc);
            (void)cpudb_destroy(cur);
        } else {
            printk("Disc DB %p; Cur DB %p\n", 
                   (void *)bcm_st_disc_db, (void *)bcm_st_cur_db);
            printk("Not dumping DBs due to current state\n");
        }
        return CMD_OK;
    }
    ...
    if (sal_strcasecmp(subcmd, "START") == 0) {
        ...
        if (st_tid != SAL_THREAD_ERROR) {
            printk("Stack Task is already running\n");
            return CMD_OK;
        }
        ...
        if (disc_cos >= 0) {
            ...
            rv = bcm_stk_update_callback_register(0, _stk_port_update,
                                                  INT_TO_PTR(disc_cos));
            if (rv < 0) {
                printk("ST: Could not set stack callback: %s\n",
                       bcm_errmsg(rv));
            }
        }
        if (disc_vlan >= 0) {
        ...
        if (!cpudb_valid(db_refs[db_idx]) ||
            ...
            if (need_board_handler) {
                rv = topo_board_register(topo_board_default_board_program,
                                         NULL);
                if (rv < 0) {
                    printk("Could not register board programming\n");
                    return CMD_FAIL;
                }
            }
            ...
            if (board_id_str != NULL) {
                ...
                if (BCM_FAILURE(rv)) {
                    printk("ERROR: Could not find board id for '%s'\n",
                           board_id_str);
                    return CMD_FAIL;
                } else {
                    /* Initialize board */
                    ...
                    if (BCM_FAILURE(rv)) {
                        printk("ERROR: Could not initialize board '%s'\n",
                               board_id_str);
                        return CMD_FAIL;
                    } else {
                        printk("Initialized board %s\n", board_id_str);
                    }
                }
            } else {
                printk("ERROR: No board id\n");
                return CMD_FAIL;
            }
            ...
            for (i = 0; i < db_ref->local_entry->base.num_units; i++) {
                ...
                if (rv < 0 && rv != BCM_E_EXISTS) {
                    printk("ERROR: vlan %d create failed: %s\n",
                ...
                rv = bcm_vlan_port_add(i, disc_vlan,
                                       vports, vports_untag);
                if (rv < 0) {
                    printk("ERROR: vlan %d CPU port add unit %d failed: %s\n",
            ...
            for (i = 0; i < db_ref->local_entry->base.num_stk_ports; i++) {
                ...
                if (bcm_port_speed_get(sp->unit, sp->port, &speed) < 0) {
                    printk("Error getting port speed unit %d, port %d\n",
                           sp->unit, sp->port);
                    return CMD_FAIL;
                }
                if (speed > 0) {
        ...
        if (st_done_sem == NULL) {
            ...
            if (st_done_sem == NULL) {
                printk("Could not create semaphore\n");
                return CMD_FAIL;
            }
        ...
        rv = evlog_start();
        if (rv < 0) {
            printk("Cannot start event logging\n");
        } else {
        ...
        rv = bcm_st_base_update(&db_refs[db_idx]->local_entry->base,
                                restart);
        if (rv < 0) {
            printk("ST base update ERROR %d: %s\n", rv, bcm_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        if (ARG_CNT(args) == 0) {
            if ((rv = disc_version_get(&ver)) < 0) {
                printk("Failed to get current discovery version: %s\n",
                       bcm_errmsg(rv));
                return CMD_FAIL;
            } else {
                printk("Current discovery version: %d\n", ver);
            }
            if ((rv = disc_fallback_get(&disc_fallback)) < 0) {
                printk("Failed to get discovery fallback setting: %s\n",
                       bcm_errmsg(rv));
                return CMD_FAIL;
            } else {
                printk("Fallback is %s\n",
        ...
        if (ver != -1) {
            if ((rv = disc_version_set(ver)) < 0) {
                printk("Failed to set discovery version %d: %s\n", ver,
                       bcm_errmsg(rv));
                return CMD_FAIL;
            }
        }
        if (disc_fallback != -1) {
            if ((rv = disc_fallback_set(disc_fallback)) < 0) {
                printk("Failed to %s discovery version fallback: %s\n",
                       disc_fallback ? "enable" : "disable",
                       bcm_errmsg(rv));
                return CMD_FAIL;
            }
        ...
        printk("bcm_st_config_load returned %d\n", rv);
        parse_arg_eq_done(&pt);
        return CMD_OK;
        ...
        if (st_tid == SAL_THREAD_ERROR) {
            printk("Stack Task is not running\n");
            return CMD_OK;
        }
...
#define TKS_STOP_THREAD_POLL_FRACTION 10
        ...
        if (stop_arg.rv == BCM_E_INTERNAL) {
            printk("Stack stop thread failed to signal completion\n");
            return CMD_FAIL;
        }
        
        printk("bcm_st_stop returns %d: %s\n",
        ...
        if (rv < 0) {
            printk("bcm_st_send_event ERROR: %s\n", bcm_errmsg(rv));
        }
        parse_arg_eq_done(&pt);
        ...
        if (blk < 1) {
            printk("blk must be greater than 0\n");
            return CMD_USAGE;
        }
        ...
        if (array > 0 && list > 0) {
            printk("Cannot specify array and list at the same time\n");
            cmd_rv = CMD_FAIL;
            goto done;
        }
        ...
        if (BCM_FAILURE(rv)) {
            printk("bcm_stk_modport_set: %s %d\n", bcm_errmsg(rv),rv);
            return CMD_FAIL;
        }
        return CMD_OK;
        ...
        if (BCM_FAILURE(rv)) {
            printk("bcm_stk_my_modid_set: %s %d\n", bcm_errmsg(rv),rv);
            return CMD_FAIL;
        }
        return CMD_OK;
    } else {
                         ...
                         cpudb_key_t tx_cpu_key, cpudb_key_t rx_cpu_key)
{
    ...
    if (entry->base.num_stk_ports >= CPUDB_STK_PORTS_MAX) {
        printk("ERROR: failed to add stack port unit=%d port=%d to cpudb "
...
_port_string_parse(cpudb_entry_t *entry, char *port_str)
{
    ...
    do {
        ...
        if (port_str != NULL) {
            ...
            printk("Error parsing port string: %s\n", port_str);
        }
...
cmd_cpudb_parse(int ignored_unit, args_t *args)
{
    ...
    case PARSE_OBJ_STACK_INSTALL  :
        ...
        if (rv < 0) {
            printk("WARNING:  ATP DB update failed %d: %s\n",
                   rv, bcm_errmsg(rv));
        }
        rv = bcm_stack_attach_update(db_ref);
        if (rv < 0) {
            printk("ERROR:  stack attach returned %d: %s\n",
                   rv, bcm_errmsg(rv));
        }
        break;
    case PARSE_OBJ_LOCAL_ENTRY  :
        ...
        if (db_ref->local_entry == NULL) {
            printk("ERROR:  DB parse.  Local entry not set\n");
            cmd_rv = CMD_FAIL;
            break;
        }
