...
not_link_test(int unit, args_t *a, void *pa)
{
	...
	soc_cm_debug(DK_TESTS, "Setting the Forwarding Port register to %s\n",
	...
	soc_cm_debug(DK_TESTS, "Sending Linkstat with value %s\n", pfmt);
	...
	while (!((value = soc_pci_read(unit, CMIC_SCHAN_CTRL)) &
	    ...
	    soc_cm_debug(DK_TESTS, "Waiting for message...\n");
	}
...
regStatus_test(int unit, soc_reg_t reg)
{
    ...
    for (i = 0; i < npat; i++) {
	setval = SOC_PBMP_WORD_GET(bmp[0], 0);
	soc_cm_debug(DK_TESTS, "Setting register %s to 0x%x\n",
                     regname, setval);
	...
	soc_cm_debug(DK_TESTS, "Sending schan message value 0x%x\n", setval);
