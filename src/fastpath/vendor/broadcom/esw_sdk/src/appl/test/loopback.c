/*
 * $Id: loopback.c 1.204 Broadcom SDK $
 ...
 */
{
    ...
    if (SOC_IS_CALADAN3(unit)) {
        ...
        BCM_PBMP_ITER(link_pbm, port) {
            if (IS_XL_PORT(unit, port)) {continue;} /* Pending bringup */
            do {
                sal_udelay(2000);
...
#ifdef BCM_CALADAN3_SUPPORT
    if (SOC_IS_CALADAN3(unit) && (lw->lw_c3_lrp_bypass)) {
        debugk(DK_TESTS, "Skipping ARL setup for port %s\n",
               SOC_PORT_NAME(unit, src_port));
        return SOC_E_NONE;
    }
#endif
...
#ifdef BCM_CALADAN3_SUPPORT
        /* Pending XC bring up */
        if (SOC_IS_CALADAN3(unit) && IS_XL_PORT(unit, port)) {continue;}
...
#elif defined (BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
            {
