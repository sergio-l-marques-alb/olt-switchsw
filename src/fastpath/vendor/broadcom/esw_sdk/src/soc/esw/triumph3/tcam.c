...
#if 1
#define MODIFY_ESM_SEARCH_PRIORITY_RANGE(unit, idx_start, idx_end, \
                                        esm_srch_offset, esm_srch_priority, \
                                        tbl_entry_t, src_table, tbl_name, \
                                        tbl_entry_ptr) \
do {\
    int i_macro, alloc_size_macro, rv_macro; /* some name that will not clash */\
    void *buf_macro;\
\
    alloc_size_macro = ((idx_end)-(idx_start)+1)*sizeof((*(tbl_entry_ptr)));\
    buf_macro = soc_cm_salloc((unit), alloc_size_macro, #tbl_name);\
    if (buf_macro == NULL) return SOC_E_MEMORY;\
    rv_macro = soc_mem_read_range((unit), (src_table), \
                                   MEM_BLOCK_ANY, (idx_start), (idx_end), \
                                   buf_macro);\
    if (rv_macro < 0) {\
        soc_cm_sfree((unit), buf_macro);\
        return SOC_E_FAIL;\
    }\
\
    for (i_macro = 0; i_macro < (idx_end)-(idx_start)+1; i_macro++) {\
        (tbl_entry_ptr) = soc_mem_table_idx_to_pointer((unit), \
                            (src_table),  __typeof__((tbl_entry_ptr)), \
                            buf_macro, i_macro);\
        soc_mem_field32_set((unit), (src_table), (tbl_entry_ptr), \
...
\
    rv_macro = soc_mem_write_range((unit), (src_table), MEM_BLOCK_ANY, \
                             (idx_start), (idx_end), (void *) buf_macro);\
    soc_cm_sfree((unit), buf_macro);\
    if (rv_macro < 0 ) return SOC_E_FAIL;\
} while (0)
                                             ...
                                             int esm_srch_priority)
{
    ...
    MODIFY_ESM_SEARCH_PRIORITY_RANGE(unit, start_idx, end_idx, esm_srch_offset,
                                    esm_srch_priority, 
...
_soc_tr3_esm_init_set_esm_mode_per_port(int unit)
{
    ...
    if (tcam_info->mode) { /* 350 MHz */
        ...
        if (tcam_info->subtype == 3) {
            l2_fm_offset1 = KEY_GEN_OFFSET_TR2_ACL_L2_NON7K_SGLP;
            l2_fm_offset1 = KEY_GEN_OFFSET_TR2_ACL_L2_NON7K_SVP;
        }
    }
    if (fwd_ip6_key) {
    ...
    case 4: /* ACL_L2 or ACL_L2C */
        ...
        offset1 = -1; offset2 = KEY_GEN_OFFSET_TR2_ACL_L2_7K_SGLP;
        break;
    default:
        offset1 = offset2 = -1;
        break;
    ...
    if (partitions[TCAM_PARTITION_ACL_L2].num_entries) {
        if (fwd_l2_present) {
            ...
            SOC_TR3_BUILD_FM_ENTRY(key_map, 0, 
                                ...
                                F2_FLAG_MY_STATION_HIT, F2_FLAG_FORWARDING_TYPE_2,
            ...
            /* TR2 L2_ACL (7k) */
            key_map.f_selects[5] = F5_INT_RANGE_CHECK;
            SOC_IF_ERROR_RETURN(_soc_write_esm_key_id_to_field_map_entry(unit, 
                                KEY_GEN_OFFSET_TR2_ACL_L2_7K_SVP, &key_map));
        
            key_map.f_selects[7] = F7_SGLP_DGLP;
        ...
        if (!fwd_l2_present || !fwd_ipv6u_present) {
            ...
            SOC_TR3_BUILD_FM_ENTRY(key_map, 0, 
                                ...
                                F7_SGLP_DGLP, F6_CLASSID_ALL, 
                                tcam_info->mode ? F5_FLAGS_IP_1_VRF :
                                F5_INT_RANGE_CHECK,
                                F4_ITAG, 
                                F3_UDF_1_AND_2_VALID, 
                                0, 0, F2_FLAG_HG, F2_FLAG_L3_TYPE_3, 
...
int soc_triumph3_esm_init(int unit)
{
    ...
    for (i = 0; i < soc_mem_index_count(unit, EXT_L2_ENTRY_TCAM_WIDEm); i++) {
