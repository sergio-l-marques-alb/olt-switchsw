/*
 * $Id: trident2.c 1.282.2.21 Broadcom SDK $
...
static uint32 _td2_ser_delay = 0;
    ...
    do {
        ...
        if (soc_mem_field32_get(unit, mem, entry, VALIDf)) {
            ...
            if (regmem == _SOC_TD2_SER_MEM) {
                ...
                if (stage == 7 ||stage == 8 || stage == 11) {
                    /* Mems in these stages have more than 256k indexes */
                    index |= hwmbase << 18;
                    hwmbase = addrbase = 0;
                }
                if (non_sbus == 0) {
            ...
            if (SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, rb->enable_reg).block, 
                SOC_BLK_PORT) && (port == REG_PORT_ANY)) {
                    ...
                    soc_cm_debug(DK_ERR, "unit %d SER error on disabled port block %d !!\n",
                                 unit, block_info_idx);
                    sal_usleep(SAL_BOOT_QUICKTURN ? 10000000 : 1000); /* Don't reenable too soon */
                    continue;
            }
...
void
soc_trident2_ser_set_long_delay(int delay)
{
    _td2_ser_delay = delay;
    return;
}
                       ...
                       void *d4)
{
    ...
    sal_usleep(SAL_BOOT_QUICKTURN ? 10000000 : (_td2_ser_delay != 0 ? 10000000 : 100000));
    if (d2 != NULL) {
...
soc_trident2_l3_defip_index_map(int unit, soc_mem_t mem, int index)
{
    ...
    if (_soc_alpm_mode[unit] == 1) {
        uint32  incr = soc_mem_index_count(unit, mem)/4;
        uint32 entries_per_tcam = _TD2_INDEX_1K_ENTRIES + 1;
        ...
        if (SOC_CONTROL(unit)->l3_defip_urpf) {
            ...
            tcam_idx = index % incr;
            return tcam_num * entries_per_tcam + tcam_idx + 
                                SOC_CONTROL(unit)->l3_defip_index_remap/4;
        }
        if (wide) {
            ...
            return index + _TD2_INDEX_2K_ENTRIES + 1;
        }
        if (index < soc_mem_index_count(unit, mem)/2) {
            return index + SOC_CONTROL(unit)->l3_defip_index_remap;
        } else {
            /* make index relative to upper half */
            index -= (soc_mem_index_count(unit, mem)/2);
            return index + 2 * _TD2_OFFSET_2K_ENTRIES + 
                   SOC_CONTROL(unit)->l3_defip_index_remap;
       }
    }
...
soc_trident2_l3_defip_index_remap(int unit, soc_mem_t mem, int index)
{
    ...
    if (_soc_alpm_mode[unit] == 1) {
        ...
        if (SOC_CONTROL(unit)->l3_defip_urpf) {
            ...
            if (wide) {
                num_chunks = index/(_TD2_INDEX_1K_ENTRIES + 1);
                chunk_offset = index % (_TD2_INDEX_1K_ENTRIES + 1);
                return num_chunks * soc_mem_index_count(unit, mem)/4 
                       + chunk_offset;
            }
            num_chunks = index/(_TD2_INDEX_1K_ENTRIES + 1);
            chunk_offset = index % (_TD2_INDEX_1K_ENTRIES + 1);
            chunk_offset -= (SOC_CONTROL(unit)->l3_defip_index_remap/2);
        ...
        if (wide) {
            ...
            /* get index within global space, add logical offset */
            tmp = SOC_CONTROL(unit)->l3_defip_max_tcams/2 *
                    SOC_CONTROL(unit)->l3_defip_tcam_size/2;
            return index - tmp + soc_mem_index_count(unit, mem)/2;
        }
        if (index <= _TD2_INDEX_4K_ENTRIES) {
            /* remove size of v6-128 in vrf space */
            return index - soc_mem_index_count(unit, L3_DEFIP_PAIR_128m)/2;
        }
        /* get index within global space, add logical offset */
        tmp = index - SOC_CONTROL(unit)->l3_defip_max_tcams *
              SOC_CONTROL(unit)->l3_defip_tcam_size/2;
        /* remove v6-128 size from index */
        tmp -= SOC_CONTROL(unit)->l3_defip_index_remap;
        /* add in vrf-part of logical address */
        return tmp + soc_mem_index_count(unit, mem)/2;
    }
    if (SOC_CONTROL(unit)->l3_defip_urpf) { /* URPF mode */
...
_soc_trident2_alpm_bkt_view_set(int unit, int index, soc_mem_t view)
{
    ...
    soc_cm_debug(DK_VERBOSE, "Unit:%d ALPM bkt set index:%d bkt:%d view:%s\n", 
...
_soc_trident2_alpm_bkt_view_get(int unit, int index)
{
    ...
    soc_cm_debug(DK_VERBOSE, "Unit:%d ALPM bkt get index:%d bkt:%d view:%s\n", 
                 unit, index, bkt, SOC_MEM_NAME(unit, view));
    return view;
}
                                ...
                                _soc_ser_sram_info_t *sram_info)
{
    ...
    case L3_DEFIP_ALPM_RAWm:
        ...
        base = index & 0x3;
        soc_cm_debug(DK_VERBOSE, "reported bucket: 0x%08x, bank:%d\n", base_bucket, base);
        base_bucket = base_bucket % SOC_TD2_ALPM_BKT_OFFFSET;
        soc_cm_debug(DK_VERBOSE, "base bucket: 0x%08x\n", base_bucket);
...
/*
 * Function:
 *      _soc_trident2_tsc_ports_get
 * Description:
 *      Get all logical port(s) in the TSC of the given port
 * Parameters:
 *      unit      - Device number
 *      tsc       - TSC number
 *      ports_len - (OUT) number of logical port number in ports argument
 *      ports     - (OUT) array for the logical port number
 */
int
soc_trident2_tsc_ports_get(int unit, int tsc, int *ports_len, int *ports)
{
    soc_info_t *si;
    int index;
    int phy_port;
    ...
    *ports_len = 0;
    for (index = 0; index < _TD2_PORTS_PER_XLP; index++) {
        ...
        if (si->port_p2l_mapping[phy_port] != -1) {
            ports[*ports_len] = si->port_p2l_mapping[phy_port];
            (*ports_len)++;
        }
    }
...
soc_trident2_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    if (frequency != -1 && frequency < si->frequency) {
        ...
        if (index < freq_list_len) {
            ...
            soc_cm_print("*** Input core clock frequency %dMHz is not "
                         "supported!\n", frequency);
        }
    } else if (frequency > si->frequency) {
        soc_cm_print("*** Input core clock frequency %dMHz is not supported!\n",
                     frequency);
    }
...
soc_trident2_chip_reset(int unit)
{
    ...
    if (si->frequency != max_frequency) {
        ...
        if (sel < freq_list_len) {
            ...
            soc_cm_print("*** change core clock frequency to %dMHz\n",
                         si->frequency);
...
soc_td2_mmu_config_init(int unit, int test_only)
{
    ...
    if (soc_property_get(unit, "mmu_config_override", 1) == 0) {
...
soc_trident2_port_speed_update(int unit, soc_port_t port, int speed)
{
    ...
    if (rev_id >= BCM56850_A2_REV_ID && si->port_speed_max[port] > 20000) {
        ...
        if (soc_reg64_field32_get(unit, reg, rval64, FLOW_CTRL_XOFFf) != 
            ...
            soc_reg64_field32_set(unit, reg, &rval64, MIN_THRESHOLDf,
                                  xoff_threshold - 21);
            soc_reg64_field32_set(unit, reg, &rval64, FLOW_CTRL_XONf,
                                  xoff_threshold - 24);
            soc_reg64_field32_set(unit, reg, &rval64, FLOW_CTRL_XOFFf,
                                  xoff_threshold);
            SOC_IF_ERROR_RETURN
...
int
soc_trident2_port_thdo_rx_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if (enable) {
        /* Enable enqueue to the port */
        for (i = 0; i < 3; i++) {
            reg_tmp = SOC_TD2_PREG(unit, port, reg[i][0], reg[i][1]);

            rv = soc_reg_get(unit, reg_tmp, REG_PORT_ANY, 0, &rval64);
            if (rv != SOC_E_NONE) 
            {
                return rv;
            }
            ...
            rv = soc_reg_set(unit, reg_tmp, REG_PORT_ANY, 0, rval64);
            if (rv != SOC_E_NONE) 
            { 
                return rv;
            }
...
const ser_td2_skipped_mem_t skipped_mems[] = {
    ...
    {VLAN_SUBNETm, _SOC_ACC_TYPE_PIPE_X},
                                     ...
                                     int pipe)
{
    ...
    for (i = 0; skipped_mems[i].mem != INVALIDm; i++) {
        if ((skipped_mems[i].mem == mem) &&
            (skipped_mems[i].acc_type == soc_acc_type_target)) {
            return rv;
        }
    }

    /* Search memories in EP / IP */
    ...
    /* Check if memory is a SRAM which may not be listed in IP/EP info */
...
soc_td2_ser_tcam_test (int unit, _soc_ser_test_t test_type) {
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        soc_ser_create_test_data(unit, tmp_entry, fieldData, SER_RANGE_ENABLEr,
                                 i, INVALIDf, tcams[i].mem, INVALIDf,
    ...
    soc_cm_print("TCAM tests skipped:\t%d (use verbose option to see "
                 "skipped memories)\n",
                 mem_skipped);
...
soc_td2_ser_hardware_test (int unit, _soc_ser_test_t test_type) {
    ...
    for (i = 0; _soc_td2_alpm_sram_list[i] != INVALIDm; i++) {
        ...
        _soc_trident2_mem_sram_info_get(unit, _soc_td2_alpm_sram_list[i],
                                        0, &sram_info);
        if (sram_info.disable_reg != INVALIDr) {
            soc_ser_create_test_data(unit, tmp_entry, fieldData,
                                         sram_info.disable_reg,
                                         SOC_INVALID_TCAM_PARITY_BIT,
                                         sram_info.disable_field,
                                         _soc_td2_alpm_sram_list[i],
                                         EVEN_PARITYf, MEM_BLOCK_ANY,
                                         REG_PORT_ANY, _SOC_ACC_TYPE_PIPE_Y,
                                         &test_data);
        _soc_trident2_perform_ser_test(unit, &test_data, test_type,
                                       &mem_skipped, &mem_failed);
    ...
    soc_cm_print("H/W tests passed:\t%d\n",
                 mem_tests - mem_failed - mem_skipped);
    soc_cm_print("H/W tests skipped:\t%d (use verbose option to see "
                 "skipped memories)\n",
                 mem_skipped);
...
#ifdef SOC_MEM_NAME
#define TR_TEST_MEM_PRINT(_msg_, _mem_)             \
    sal_strcpy (_msg_, SOC_MEM_NAME(unit, _mem_));  \
    soc_cm_print("Memory %s is valid, but not currently testable.\n", _msg_)
#else
#define TR_TEST_MEM_PRINT(_msg_, _mem_)  \
    sprintf(_msg_, "Mem ID: %d", _mem_);  \
    soc_cm_print("Memory %s is valid, but not currently testable.\n", _msg_)
#endif
...
soc_ser_inject_or_test_mem (int unit, soc_mem_t mem, int pipe_target, int block,
                            ...
                            int inject_only, int cmd)
{
    uint32 tmp_entry[SOC_MAX_MEM_WORDS], fieldData[SOC_MAX_REG_FIELD_WORDS];
    ser_test_data_t test_data;
    _soc_td2_fifo_ser_info_t *fifo_parity = _soc_td2_ip_ser_info;
    _soc_generic_ser_info_t *tcams = _soc_td2_tcam_ser_info[unit];
    int i,j, error_count = 0, skip_count = 0, found_mem = FALSE;
    ...
    if (found_mem) {
        ...
        if (inject_only) {
            ...
            for (i = 0; skipped_mems[i].mem != INVALIDm; i++) {
                if ((skipped_mems[i].mem == mem) &&
                    (skipped_mems[i].acc_type == test_data.acc_type)) {
                    TR_TEST_MEM_PRINT(fail_message, mem);
                    return SOC_E_UNAVAIL;
                }
            }
            ...
            if (cmd) {
                ...
                if (skip_count !=0) {
                    soc_cm_print("Test skipped due to known issues with "
                                 "this memory.\n");
                }
            }
        }
