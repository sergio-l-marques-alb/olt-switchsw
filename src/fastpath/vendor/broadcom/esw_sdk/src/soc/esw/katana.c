...
_soc_katana_misc_init(int unit)
{
    ...
    soc_port_t it_port;
    uint32 mxq = 0;
    uint32 last_two_mxqblock_ports[2][4] = {{27, 32, 33, 34},{28, 29, 30, 31}};
    ...
    switch (level)  {
        case _SOC_KT_COSQ_NODE_LEVEL_ROOT:
            ...
            soc_mem_field32_set(unit, config_mem[0], &port_entry,
                                rate_exp_f[0],
                                KT_QUEUE_FLUSH_RATE_EXP);
            soc_mem_field32_set(unit, config_mem[0], &port_entry,
                                rate_mant_f[0],
                                KT_QUEUE_FLUSH_RATE_MANTISSA);
            soc_mem_field32_set(unit, config_mem[0], &port_entry,
                                burst_exp_f[0],
                                KT_QUEUE_FLUSH_BURST_EXP);
            soc_mem_field32_set(unit, config_mem[0], &port_entry,
                                burst_mant_f[0],
                                KT_QUEUE_FLUSH_BURST_MANTISSA);
            soc_mem_field32_set(unit, config_mem[0], &port_entry,
                                cycle_sel_f[0],
                                KT_QUEUE_FLUSH_CYCLE_SEL);
            SOC_IF_ERROR_RETURN
