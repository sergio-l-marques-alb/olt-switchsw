	      ...
	      uint16 addr, uint8* data, uint32 *len)
{
	...
	soc_cm_debug(DK_I2C, "eep24c64_read(%d,%d,%x,%p,%d): "
	      ...
	      uint16 addr, uint8* data, uint32 len)
{
	...
	if( (rv = soc_i2c_start(unit, bus_addr)) < 0){
	    soc_cm_debug(DK_I2C, "eep24c64_write(%d,%d,%x,%d,%d): "
			 "failed to gen start\n",
	...
	for ( b = 0; b < nbytes; b++, ptr++, caddr++ ) {
		...
		soc_cm_debug(DK_I2C, "eep24c64_write(%d,%d,%d,%d,%d): "
			     "tx data byte error\n",
...
eep24c64_get_params(uint32 u, uint32 d, eep24c64_t* c)
{
	...
	soc_cm_debug(DK_I2C, "eep24c64_get_params: %s\n", soc_errmsg(r));
	return r;
    }
...
#define GET_IO_STAT(rtn, sz, name, op) { \
	...
	soc_cm_debug(DK_VERBOSE, "%s: %s took %.2f sec %.2fKB/sec\n", \
			name, op, etime - stime, \
			sz / (etime - stime) / 1024); \
}
#else
#define GET_IO_STAT(rtn, sz, name, op) { \
	...
	soc_cm_debug(DK_VERBOSE, "%s: %s took %u usec %dKB/sec\n", \
			name, op, etime - stime, \
...
eep24c64_init(int unit, int devno, void* data, int len)
{
	...
	for(i = 0; i <= LC2464_DEVICE_RW_SIZE/8; i+= len){
	    soc_cm_debug(DK_I2C, "eep24c64_init: unit=%d bytes=%d\n", unit, i);
	    ...
	    for( j = 0; j < len; j++){
		if ( inbuf[j] != outbuf[j]){
		    soc_cm_debug(DK_VERBOSE, "%s: ERROR: EEPROM miscompare "
				 "off=%d expected=0x%x "
	...
	soc_cm_debug(DK_VERBOSE, "%s: testing data integrity, %d bytes\n",
	...
	soc_cm_debug(DK_VERBOSE, "%s: writing %d bytes pattern=0x%x\n",
		     devname, len, pattern);
	...
	if (rlen == LC2464_DEVICE_RW_SIZE )
	    ...
	    soc_cm_debug(DK_VERBOSE, "%s: ERROR: write failed: %s\n",
	...
	soc_cm_debug(DK_VERBOSE, "%s: reading %d bytes\n", devname, rlen);
	...
	for( j = 0; j < LC2464_DEVICE_RW_SIZE; j++){
	    if ( inbuf[j] != outbuf[j]){
		soc_cm_debug(DK_VERBOSE, "%s: ERROR: miscompare at offset=%d "
	...
	if(i == 0)
	    soc_cm_debug(DK_VERBOSE, "%s: test passed (%d bytes verified)\n",
			 devname, len);
	...
	config.name[LC2464_NAME_LEN - 1] = '\0';
        ...
        sal_strcpy(config.name, soc_i2c_devname(unit,devno));
 	}
