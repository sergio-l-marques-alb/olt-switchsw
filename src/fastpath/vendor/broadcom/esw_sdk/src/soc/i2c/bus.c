...
#else
    soc_cm_print("NOTICE: SOC_I2C_EVENT_LOGGING not compiled in.\n");
#endif
}
...
{
    soc_cm_print("unit %d i2c: %s:", unit, msg);
    if (flags == 0) {
	soc_cm_print(" OFFLINE");
    }
...
soc_i2c_attach(int unit, uint32 flags, int speed_khz)
{
    ...
    soc_cm_debug(DK_I2C, "soc_i2c_attach: oldspeed=%d newspeed=%d\n",
    ...
    if ( ((speed_khz == 0) || (speed_khz > (SOC_IS_XGS3_SWITCH(unit) ? 250 : 2500))) ||
	 ((speed_khz < 0) && (i2cbus->frequency <= 0)) ) {
      if (SOC_IS_XGS_FABRIC(unit) || SOC_IS_SIRIUS(unit) || 
             SOC_IS_XGS3_SWITCH(unit) || SOC_IS_CALADAN3(unit)) {
...
soc_i2c_intr(int unit)
{
	...
	soc_cm_debug(DK_VERBOSE, "i2c%d: slave transmit mode entered: %s\n",
	...
	soc_cm_debug(DK_VERBOSE, "i2c%d: slave receive mode entered: %s\n",
		     unit, soc_i2c_status_message(s));
...
soc_i2c_wait(int unit)
{
    ...
    soc_cm_debug(DK_I2C, "soc_i2c_wait: current state=0x%x:[%s]\n",
...
_i2c_start_helper(int unit, i2c_bus_addr_t bus_addr, int repeated)
{
	...
	soc_cm_debug(DK_VERBOSE, "unit %d i2c 0x%x: %sSTART- BAD STATUS: %s\n",
		     unit, bus_addr>>1,
		     (repeated ? "REP_" : ""),
...
soc_i2c_write_one_byte(int unit, uint8 data)
{
	...
	soc_cm_debug(DK_I2C, "soc_i2c_write_one_byte: u=%d data=0x%x"
		     " DEVICE TIMEOUT!\n",
		     unit, data);
	return SOC_E_BUSY;
    }
...
soc_i2c_read_bytes(int unit, uint8* data, int* len, int ack_last_byte)
{
    ...
    *len = 0;
	...
	if (SOC_E_TIMEOUT == soc_i2c_wait(unit)) {
	    soc_cm_debug(DK_I2C, "soc_i2c_read_bytes: u=%d data=0x%x"
			 " state=0x%x:[%s]\n",
...
soc_i2c_read_short(int unit, uint16* val, int ack_last_byte)
{
    ...
    if (SOC_E_TIMEOUT == soc_i2c_wait(unit)) {
        soc_cm_debug(DK_I2C, "soc_i2c_read_short: u=%d data=0x%x"
		     " state=0x%x:[%s]\n",
		     unit, 0, soc_i2c_stat(unit),
		     soc_i2c_status_message(soc_i2c_stat(unit)));
	return SOC_E_TIMEOUT;
...
soc_i2c_ack_poll(int unit, i2c_bus_addr_t bus_addr, int maxpolls)
{
    ...
    if (unit == -1) {
        /* cpumode */
        while (--i > 0 && (cpu_i2c_device_present(unit, bus_addr) < 0)) {
           /* NOP */
        }
        return (maxpolls - i);
    }
...
soc_i2c_set_freq(int unit)
{
	...
	soc_cm_print("unit %d i2c bus: attempting to set "
		     "speed=%d.%02d%sHz (%d)\n",
	...
	if (speed >= (SOC_IS_XGS3_SWITCH(unit) ? 
	    ...
	    soc_cm_debug(DK_I2C, "unit %d i2c bus: set frequency: "
			 " just set M=%d N=%d: %s\n",
...
soc_i2c_show_speeds(int unit)
{
	...
	soc_cm_print("unit %d i2c bus: speed %s (CCR M=%d,N=%d) [%d]\n",
