                                 ...
                                 uint16 *r_len, uint32 *r_data)
{
    ...
    if (MOS_MSG_DMA_MSG(s_subclass) || MOS_MSG_DMA_MSG(r_subclass)) {
        ...
        send.s.data = soc_htonl((uint32)paddr);
    } else {
...
soc_pscan_delay(int unit, int delay)
{
    ...
    if (!PSCAN_INIT(unit)) {
        soc_cm_debug(DK_WARN, "PSCAN not initialized\n");
        return SOC_E_NONE;
    }

    pscan_info = PSCAN_INFO(unit);
    if (soc_cmic_uc_msg_active_wait(unit, pscan_info->uc_num) != SOC_E_NONE) {
        soc_cm_debug(DK_WARN, "uKernel not Ready, PSCAN not started\n");
        return SOC_E_NONE;
    }
...
soc_pscan_port_enable(int unit, uint32 port, int enable)
{
    ...
    if (port > PSCAN_PORT_BITMAP_BYTE_LEN*8) {
        soc_cm_debug(DK_WARN, "PSCAN invalid port %d\n", port);
        return SOC_E_INTERNAL;
    }
...
soc_pscan_callback_thread(int unit)
{
    ...
    soc_cm_debug(DK_VERBOSE, "PSCAN callback thread starting\n");
    ...
    while (1) {
        ...
        soc_cm_print("PSCAN port %d alert 0x%08x\n", port, code);
    }

    pscan_info->event_thread_kill = 0;
    pscan_info->event_thread_id = NULL;
    soc_cm_debug(DK_VERBOSE, "PSCAN callback thread stopped\n");
    sal_thread_exit(0);
}
...
soc_pscan_init(int unit)
{
    ...
    if (uc >= CMICM_NUM_UCS) {
        ...
        soc_pscan_free_resource(unit);
        soc_cm_debug(DK_WARN, "uKernel PSCAN application not available\n");
        return SOC_E_NONE;
    }
    ...
    priority = BCM_ESW_PSCAN_THREAD_PRI_DFLT;
    ...
    if (pscan_info->event_thread_id) {
        ...
        while (pscan_info->event_thread_id != NULL) {
            if (sal_time_usecs() < timeout) {
                ...
                /*timeout*/
                soc_cm_debug(DK_ERR, "PSCAN event thread did not exit.\n");
                return SOC_E_INTERNAL;
            }
        }
    }
