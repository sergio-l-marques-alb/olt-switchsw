/*
 * $Id: counter.c 1.62.2.19 Broadcom SDK $
...
STATIC int
_soc_counter_cosq_gport_non_dma_init(int unit, int nports, int non_dma_start_index, 
                                     int *non_dma_entries)
{
    soc_control_t *soc;
    soc_counter_non_dma_t *non_dma0, *non_dma1;
                                 ...
                                 int *non_dma_entries)
{
    ...
    if (soc_feature(unit, soc_feature_cosq_gport_stat_ability)) {
        SOC_IF_ERROR_RETURN
            (_soc_counter_cosq_gport_non_dma_init(unit, nports, 
