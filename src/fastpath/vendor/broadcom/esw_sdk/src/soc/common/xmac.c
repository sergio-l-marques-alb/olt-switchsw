/*
 * $Id: xmac.c 1.23.14.1 Broadcom SDK $
...
mac_x_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (SOC_IS_KATANAX(unit) && 
        ...
        if (SOC_IS_KATANA2(unit)) {
            SOC_IF_ERROR_RETURN(soc_katana2_tdm_feasibility_check(
                                unit,port,speed));
        } 
#endif
...
#ifdef BCM_KATANA2_SUPPORT
        if (SOC_IS_KATANA2(unit)) {
            SOC_IF_ERROR_RETURN(soc_katana2_update_tdm(unit,port,speed));
        } 
#endif
    }
#endif /* BCM_KATANA_SUPPORT */
