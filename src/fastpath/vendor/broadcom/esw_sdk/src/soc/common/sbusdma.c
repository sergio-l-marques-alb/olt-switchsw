/*
 * $Id: sbusdma.c 1.106.6.1 Broadcom SDK $
...
_soc_sbusdma_error_details(int unit, uint32 rval)
{
    ...
    if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                          rval, SBUSACK_ERRORf)) {
        soc_cm_debug(DK_ERR, "H/W received sbus ack with error bit set.\n");
    }
    if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                          rval, SBUSACK_NACKf)) {
        soc_cm_debug(DK_ERR, "H/W received sbus nack with error bit set.\n");
    }
    if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                          rval, SBUSACK_WRONG_OPCODEf)) {
        soc_cm_debug(DK_ERR, "Received sbus ack has wrong opcode.\n");
    }
    if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                            ...
                            uint32 ser_flags, void *buffer)
{
    ...
    soc_cm_debug(DK_SOCMEM | DK_DMA, "_soc_mem_sbusdma_read: unit %d"
    ...
    schan_msg_clear(&msg);
    msg.readcmd.header.opcode = READ_MEMORY_CMD_MSG;
    msg.readcmd.header.srcblk =
        (soc_mem_flags(unit, mem) >> SOC_MEM_FLAG_ACCSHIFT) & 7;
    msg.readcmd.header.dstblk = SOC_BLOCK2SCH(unit, copyno);
    msg.readcmd.header.datalen = 4;
    ...
    if (0 != (ser_flags & _SOC_SER_FLAG_MULTI_PIPE)) {
        ...
        if (0 != acc_type) {
            ...
            msg.readcmd.header.srcblk = acc_type;
        }
    }
    ...
    if (soc->tableDmaIntrEnb) {
        ...
        soc_cm_debug(DK_SOCMEM | DK_DMA | DK_VERBOSE,
    ...
    if (rv != SOC_E_NONE) {
        if (rv != SOC_E_TIMEOUT) {
            soc_cm_debug(DK_ERR, "%s: %s.%s failed(ERR)\n",
...
#ifdef BCM_88650_A0
            if (SOC_IS_ARAD(unit)) {
                soc_cm_debug(DK_ERR, "%s: This DMA failure may be due to wrong PCI configuration. "
                             "Timeout configured to %dus.\n", __FUNCTION__, soc->tableDmaTimeout);
            }
#endif /* BCM_88650_A0 */
                             ...
                             int clear_buf_ent)
{
    ...
    soc_cm_debug(DK_SOCMEM | DK_DMA, "_soc_mem_sbusdma_write: unit %d"
    ...
    schan_msg_clear(&msg);
    msg.writecmd.header.opcode = WRITE_MEMORY_CMD_MSG;
    msg.readcmd.header.srcblk =
        (soc_mem_flags(unit, mem) >> SOC_MEM_FLAG_ACCSHIFT) & 7;
    msg.writecmd.header.dstblk = SOC_BLOCK2SCH(unit, copyno);
    msg.writecmd.header.datalen = data_beats * sizeof(uint32);


    if (SOC_IS_ARAD(unit) || SOC_IS_CALADAN3(unit)) {
        msg.writecmd.header.dma_bit = 1;
    }
    ...
    for (array_index = array_index_start; array_index <= array_index_end; ++array_index) {
        ...
        if (rv < 0) {
            if (rv != SOC_E_TIMEOUT) {
                soc_cm_debug(DK_ERR, "%s: %s[%u].%s failed(ERR)\n",
                             __FUNCTION__, SOC_MEM_UFNAME(unit, mem), array_index,
                             SOC_BLOCK_NAME(unit, copyno));
                _soc_sbusdma_error_details(unit,
                    soc_pci_read(unit, CMIC_CMCx_SBUSDMA_CHy_STATUS(cmc, ch)));
            } else { /* Timeout cleanup */
                ...
                soc_cm_debug(DK_ERR, "%s: %s[%u].%s %s timeout\n",
                             __FUNCTION__, SOC_MEM_UFNAME(unit, mem), array_index,
                             SOC_BLOCK_NAME(unit, copyno),
...
_soc_sbusdma_desc(void *unit_vp)
{
    ...
    while((interval = SOC_SBUSDMA_DM_TO(unit)) != 0) {
        ...
        rv = SOC_E_TIMEOUT;
        soc_cm_debug(DK_SOCMEM+DK_DMA+DK_VERBOSE, "_soc_sbusdma_desc: Process \n");
        ...
        /* Debug stuff */
        soc_cm_debug(DK_SOCMEM+DK_DMA+DK_VERBOSE, "Count: %d\n", swd->ctrl.cfg_count);
        for (i=0; i<swd->ctrl.cfg_count; i++) {
            soc_cm_debug(DK_SOCMEM+DK_DMA+DK_VERBOSE, "cntrl: %08x, req: %08x, count: %08x, "
        ...
        if (rv != SOC_E_NONE) {
            if (rv != SOC_E_TIMEOUT) {
                soc_cm_debug(DK_ERR, "%s: %s failed(ERR)\n",
                             __FUNCTION__, swd->ctrl.name);
                _soc_sbusdma_error_details(unit, 
                    soc_pci_read(unit, CMIC_CMCx_SBUSDMA_CHy_STATUS(cmc, ch)));
            } else { /* Timeout cleanup */
                soc_timeout_t to;
        
                soc_cm_debug(DK_ERR, "%s: %s %s timeout\n",
                             __FUNCTION__, swd->ctrl.name,
...
cleanup_exit:
    ...
    soc_cm_debug(DK_SOCMEM+DK_DMA, "_soc_sbusdma_desc: exiting\n");
...
soc_sbusdma_desc_abort(int unit)
{
    ...
    if (SOC_SBUSDMA_DM_PID(unit) != SAL_THREAD_ERROR) {
        ...
        while (SOC_SBUSDMA_DM_PID(unit) != SAL_THREAD_ERROR) {
            if (soc_timeout_check(&to)) {
                soc_cm_debug(DK_ERR, "soc_sbusdma_desc_detach: SBUDMA Desc Mode thread will not exit\n");
                        ...
                        sbusdma_desc_handle_t *desc_handle)
{
    ...
    if (ctrl->cfg_count == 1) { /* single mode */
        ...
        if (!(ctrl->flags & SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC) || !ctrl->hw_desc) {
            ...
            schan_msg_clear(&msg);
            msg.readcmd.header.opcode = (ctrl->flags & SOC_SBUSDMA_CFG_COUNTER_IS_MEM) ? 
                                        READ_MEMORY_CMD_MSG : READ_REGISTER_CMD_MSG;
            msg.readcmd.header.srcblk = cfg->acc_type;
            msg.readcmd.header.dstblk = cfg->blk;
            msg.readcmd.header.datalen = 4;
            /* Prepare h/w desc */
        ...
        if (!(ctrl->flags & SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC) || !ctrl->hw_desc) {
            ...
            for (count = 0; count < ctrl->cfg_count; count++) {
                ...
                msg.readcmd.header.opcode = (ctrl->flags & SOC_SBUSDMA_CFG_COUNTER_IS_MEM) ? 
                                            READ_MEMORY_CMD_MSG : READ_REGISTER_CMD_MSG;
                msg.readcmd.header.srcblk = cfg[count].acc_type;
                msg.readcmd.header.dstblk = cfg[count].blk;
                msg.readcmd.header.datalen = 4;
                /* Prepare h/w desc */
                if (append && count) {
    ...
    *desc_handle = i; 
    ...
    SOC_SBUSDMA_DM_UNLOCK(unit);
    if (ctrl->cfg_count == 1) {
        soc_cm_debug(DK_SOCMEM+DK_DMA, "Create Single:: Handle: %d, desc count: %d, addr: %x, "
                     "opcount: %d, buff: %p\n", 
                     swd->handle, swd->ctrl.cfg_count, 
                     swd->desc->addr, swd->desc->count, swd->cfg->buff);
    } else {
        soc_cm_debug(DK_SOCMEM+DK_DMA, "Create Chain:: Handle: %d, desc count: %d\n", 
                     swd->handle, swd->ctrl.cfg_count);
    }
    
    soc_cm_debug(DK_SOCMEM+DK_DMA, "SBD DM count: %d\n", SOC_SBUSDMA_DM_COUNT(unit));
    return SOC_E_NONE;
}
...
soc_sbusdma_desc_get_state(int unit, sbusdma_desc_handle_t desc_handle, uint8 *state)
{
    ...
    if ((desc_handle > 0) && (desc_handle <= SOC_SBUSDMA_MAX_DESC) 
        ...
        SOC_SBUSDMA_DM_UNLOCK(unit);
        soc_cm_debug(DK_ERR, "Get state request for invalid or non-existing descriptor handle: %d\n", 
                     desc_handle);
        return SOC_E_PARAM;
    }
    if (swd->handle != desc_handle) {
        SOC_SBUSDMA_DM_UNLOCK(unit);
        soc_cm_debug(DK_ERR, "Handle mismatch found: %d<=>%d\n", swd->handle,
                     desc_handle);
        return SOC_E_INTERNAL;
    }
    *state = swd->status;
...
soc_sbusdma_desc_run(int unit, sbusdma_desc_handle_t desc_handle)
{
    ...
    if ((desc_handle > 0) && (desc_handle <= SOC_SBUSDMA_MAX_DESC) 
        ...
        SOC_SBUSDMA_DM_UNLOCK(unit);
        soc_cm_debug(DK_ERR, "Run request for invalid or non-existing descriptor handle: %d\n", 
                     desc_handle);
        return SOC_E_PARAM;
    }
    if (swd->handle != desc_handle) {
    ...
    if (swd->ctrl.cfg_count == 1) {
        soc_cm_debug(DK_SOCMEM+DK_DMA, "Run Single:: Handle: %d, desc count: %d, addr: %x, "
                     "opcount: %d, buff: %p\n", 
                     swd->handle, swd->ctrl.cfg_count, 
                     swd->desc->addr, swd->desc->count, swd->cfg->buff);
    } else {
        soc_cm_debug(DK_SOCMEM+DK_DMA, "Run Chain:: Handle: %d, desc count: %d\n", 
                     swd->handle, swd->ctrl.cfg_count);
    }
    sal_sem_give(SOC_SBUSDMA_DM_INTR(unit));
...
soc_sbusdma_desc_delete(int unit, sbusdma_desc_handle_t handle)
{
    ...
    if ((handle > 0) && (handle <= SOC_SBUSDMA_MAX_DESC) 
        ...
        if (swd->ctrl.cfg_count == 1) {
            soc_cm_debug(DK_SOCMEM+DK_DMA, "Delete Single:: Handle: %d, desc count: %d, addr: %x, "
                         "opcount: %d, buff: %p\n", 
                         swd->handle, swd->ctrl.cfg_count, 
                         swd->desc->addr, swd->desc->count, swd->cfg->buff);
        } else {
            soc_cm_debug(DK_SOCMEM+DK_DMA, "Delete Chain:: Handle: %d, desc count: %d\n", 
                         swd->handle, swd->ctrl.cfg_count);
        }
        sal_free(swd->cfg);
        if (!(swd->ctrl.flags & SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC)) {
        ...
        SOC_SBUSDMA_DM_UNLOCK(unit);
        soc_cm_debug(DK_ERR, "Del request for invalid or non-existing descriptor handle: %d\n", 
                     handle);
        return SOC_E_PARAM;
    }
    SOC_SBUSDMA_DM_UNLOCK(unit);
                                    ...
                                    int copyno, int host_entries, void *host_buff)
{
    ...
    if ((!is_mem) && SOC_REG_IS_VALID(unit, reg)) {
        ...
        rval = soc_reg_addr_get(unit, reg, REG_PORT_ANY, 0, &blk, &at);
    } else {
        data_beats = soc_mem_entry_words(unit, mem);
...
_soc_kt2_mem_sbus_fifo_dma_stop(int unit, int ch)
{
    ...
    if (iter >= to) {
        soc_cm_debug(DK_ERR, "FIFO DMA abort failed !!\n");
        return SOC_E_INTERNAL;
    }
    return SOC_E_NONE;
}
    ...
    while ((interval = soc->l2x_interval)) {
        ...
        if (soc->l2modDmaIntrEnb) {
            ...
            if (sal_sem_take(soc->arl_notify, interval) < 0) {
                soc_cm_debug(DK_ARL | DK_VERBOSE | DK_INTR,
        ...
        do {
            ...
            if (soc_reg_field_get(unit, CMIC_CMC0_FIFO_CH0_RD_DMA_STATr, rval,
                                  DONEf)) {
                soc_cm_debug(DK_ERR, "FIFO DMA engine terminated for cmc[%d]:ch[%d]\n", 
                             cmc, ch);
                if (soc_reg_field_get(unit, CMIC_CMC0_FIFO_CH0_RD_DMA_STATr, rval,
                                  ERRORf)) {
                    soc_cm_debug(DK_ERR, "FIFO DMA engine encountered error: [0x%x]\n",
                                 rval);
                }
                goto cleanup_exit;
            }  
