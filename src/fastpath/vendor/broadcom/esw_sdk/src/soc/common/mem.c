/*
 * $Id: mem.c 1.112.2.21 Broadcom SDK $
...
#ifdef BCM_CMICM_SUPPORT
    if(soc_feature(unit, soc_feature_cmicm)) {
        soc_pci_write(unit, CMIC_CMCx_SLAM_DMA_PCIMEM_START_ADDR_OFFSET(cmc), soc_cm_l2p(unit, buffer));
...
#endif
    {
        rv = _soc_xgs3_mem_dma(unit, *aggr_mem, copyno, index, index_max,
                               shift_buf);
    }
    if (SOC_FAILURE(rv)) {
...
#endif
    {
        rv = _soc_xgs3_mem_slam(unit, *aggr_mem, copyno, index_max + 1,
                                index + 1, shift_buf);
    }
...
#endif
    {
        rv = _soc_xgs3_mem_slam(unit, mem, copyno, index + count,
                                index_max + 1, buf_ptr);
    }
 ...
 */
STATIC int
_soc_mem_dma_read(int unit, soc_mem_t mem, int copyno,
                  int index_min, int index_max, void *buffer)
{
    assert(SOC_MEM_BLOCK_VALID(unit, mem, copyno));
    ...
    if (soc_feature(unit, soc_feature_sbusdma)) {
        SOC_IF_ERROR_RETURN
            (_soc_mem_sbusdma_read(unit, mem, copyno, index_min, index_max,
                                   buffer));
    } else
#endif
    {
        SOC_IF_ERROR_RETURN
            (_soc_xgs3_mem_dma(unit, mem, copyno, index_min,
                               index_max, buffer));
    }
 ...
 */
STATIC int
_soc_mem_dma_write(int unit, soc_mem_t mem, int copyno,
                   int index_begin, int index_end, void *buffer)
{
    int           rv;
    void          *buffer_ptr;
...
#ifdef BCM_SBUSDMA_SUPPORT
    if (soc_feature(unit, soc_feature_sbusdma)) {
        rv = _soc_mem_sbusdma_write(unit, mem, copyno, index_begin, index_end,
                                    buffer_ptr, FALSE, -1);
    } else
#endif
    {
        rv = _soc_xgs3_mem_slam(unit, mem, copyno, index_begin,
...
#ifdef BCM_SBUSDMA_SUPPORT
    if (soc_feature(unit, soc_feature_sbusdma)) {
        return _soc_mem_sbusdma_clear(unit, mem, copyno, null_entry);
    }
#endif
                   ...
                   int index_min, int index_max, void *buffer)
{
    ...
    if (soc_mem_dmaable(unit, mem, copyno)) {
        ...
        if (SOC_IS_SIRIUS(unit) || SOC_IS_XGS3_SWITCH(unit)) {
            ...
            SOC_IF_ERROR_RETURN(
                _soc_mem_dma_read(unit, mem, copyno, index_min, index_max,
                                  buffer));
            if (NULL != meminfo->snoop_cb && 
                (SOC_MEM_SNOOP_READ & meminfo->snoop_flags)) {
                           ...
                           int index_min, int index_max, void *buffer)
{
	...
	MEM_LOCK(unit, mem);
	if ((rv = _soc_mem_dma_write(unit, mem, copyno,
