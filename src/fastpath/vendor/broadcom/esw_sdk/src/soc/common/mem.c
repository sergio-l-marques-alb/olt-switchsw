/*
 * $Id: mem.c 1.455.2.10 Broadcom SDK $
              ...
              int copyno, int index, void *entry_data)
{
    ...
    if (SOC_FAILURE(rv)) {
        if (SOC_SER_CORRECTION_SUPPORT(unit)) {
            if (!(flags & SOC_MEMOP_SCHAN_ERR_RETURN)) {
    ...
    if ((schan_msg.header.cpu) || (rv == SOC_E_FAIL)) {
        ...
        if ((type == SCHAN_GEN_RESP_TYPE_FULL) || 
            ...
            if (SOC_IS_TRX(unit) && (soc_mem_is_hashed(unit, mem))) {
        	  ...
        	  /* For some Hash tables on TD2, for example, MPLS_ENTRY, 
        	   * when there are parity errors in the bucket to be inserted,
        	   * the "type" field in SCHAN response message is 
        	   * SCHAN_GEN_RESP_TYPE_UNKNOWN, not SCHAN_GEN_RESP_TYPE_ERROR. 
        	   * The execution stream will arrive here, try to restore. 
        	   */
#if defined(BCM_TRX_SUPPORT)
            if  (SOC_IS_TRX(unit) && (soc_mem_is_hashed(unit, mem))) {
                /* Try to recover */
            ...
            if (oi == ni) {
                ...
                if (oi == 2) {
                    if (key_type == TD2_L3_HASH_KEY_TYPE_V4MC) {
                        ...
                        *wider_mem = L3_ENTRY_IPV6_UNICASTm;
                    }
                } else if (oi == 4) {
                          ...
                          int recurse_depth)
{
    ...
    for (bix = 0; bix < num_banks; bix++) {
        ...
        for (i = 0; i < num_ent;) {
            ...
            rv = _soc_mem_shared_hash_entry_wide_cmp(unit, mem, move_entry, 
                                                entry, &wider_mem);
            if (rv < 0) {
                /* Don't move the narrower entry */ 
                rv = SOC_E_NONE;
                break;
            }
...
soc_l3_defip_indexes_init(int unit)
{
    ...
    SOC_L3_DEFIP_MAX_128B_ENTRIES(unit) = defip_max_index;
    return SOC_E_NONE;  
}
...
soc_defip_tables_resize(int unit, int num_ipv6_128b_entries)
{
    ...
    return soc_l3_defip_indexes_init(unit);
}
