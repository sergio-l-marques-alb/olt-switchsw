/*
 * $Id: aaae8a1cd86bb493fd55ddc3b4d1f60b62263113 $
...
_soc_log_buf_print_hdr(void *location)
{
    ...
    soc_cm_debug(DK_ERR, "Log Buffer Header\n");
    soc_cm_debug(DK_ERR, "\tfirst_entry: %d\n", buf_hdr->first_entry);
    soc_cm_debug(DK_ERR, "\tlast_entry: %d\n", buf_hdr->last_entry);
    soc_cm_debug(DK_ERR, "\tbuf_size: %d\n", buf_hdr->buf_size);
    soc_cm_debug(DK_ERR, "\tmutex: %p\n", (char*)(buf_hdr->mutex));
...
_soc_log_buf_print_entry(void *location, int offset)
{
    ...
    soc_cm_debug(DK_ERR, "Log Entry Header\n");
    soc_cm_debug(DK_ERR, "\toffset: %d\n", offset);
    soc_cm_debug(DK_ERR, "\tid: %d\n", hdr.id);
    soc_cm_debug(DK_ERR, "\tsize: %d\n", hdr.size);
    soc_cm_debug(DK_ERR, "\tdata: \n\t\t");

    _soc_log_buf_read(location, 
        offset+sizeof(_soc_log_entry_hdr_t),
        &temp_buf, _SOC_LOG_BUF_PRINT_BUFFER_SIZE);
    for (index = 0; index < hdr.size; index++) {
        soc_cm_debug(DK_ERR, "%02x ",
