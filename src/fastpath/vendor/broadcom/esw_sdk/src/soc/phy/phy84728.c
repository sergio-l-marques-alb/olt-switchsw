/*
 * $Id: phy84728.c 1.49 Broadcom SDK $
 *
...
_phy_84728_nxt_dev_probe(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "_phy_84728_nxt_dev_probe: found phy device"
...
_phy_84728_init_ucode_bcst(int unit, int port, int cmd)
{
    ...
    if (cmd == PHYCTRL_UCODE_BCST_SETUP) {
        ...
        SOC_DEBUG_PRINT((DK_ERR, "u=%d p=%d MDIO Firmware broadcast download revID: 0x%x\n",
                         unit, port,data16));
                     ...
                     soc_phy_control_t type, uint32 value)
{
    ...
    switch(type) {
        ...
        case SOC_PHY_CONTROL_CLOCK_ENABLE:
            ...
            if((PHY84728_MAP(pc)->devid) == 0x84748) {
                if(value) {
                    regval = (1 << 10);
                }
                SOC_IF_ERROR_RETURN
...
bsdk_phy84728_firmware_spi_download(phy_ctrl_t *pc)
{
    ...
    soc_cm_print("BCM84728: u=%d p=%d: SPI-ROM download complete: Version: 0x%x\n", 
