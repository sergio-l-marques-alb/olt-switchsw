...
/*
 * $Id: 8c5312b6f6df6dfea304422f2ee8430330356344 $
 *
...
#define SKIP_FW_DOWNLOAD(_pc)  ((PHY84756_PHY_INFO(_pc))->skip_fw_download)
...
#if defined(INCLUDE_FCMAP)
extern int 
bfcmap84756_a0_firmware_download(bfcmap_dev_addr_t dev_addr);
#endif
...
_phy84756_init_ucode_bcst(int unit, int port, uint32 cmd)
{
    ...
    if (cmd == PHYCTRL_UCODE_BCST_SETUP) {
        ...
        if (FCMAP_ENABLE(pc)) 
        {
            ...
            SOC_IF_ERROR_RETURN
                (BFCMAP_WR_PHY84756_LN_DEV1_PMDr(pc, 0xc8f0, 0));
            sal_udelay(100); /* Wait for 100us */

            /* Send to UC Firmware Download data */
            bfcmap84756_a0_firmware_download(pc->fcmap_uc_dev_addr);
            sal_udelay(100000);
            fc_extra_delay = 1;
        }
#endif        
...
#define BFCMAP_PORT_UC_DATA_REG14(c)         (0x4e)
            bfcmap_phy84756_reg_read(pc, 0, 0x1e, BFCMAP_PORT_UC_DATA_REG14(0), &ver);
            soc_cm_print("BCM84756:FC firmware version:%4X \n", ver);
            if (fc_extra_delay) 
            {
...
phy_84756_fcmap_firmware_set(int unit, int port, int offset, uint8 *data,int len)
{
    ...
    /* Call 40G Firmware Set */
    if (PHY84756_IS_8474X(pc) &&  !(data   == NULL &&
                                   (offset  == PHYCTRL_UCODE_BCST_LOAD ||
                                     (offset  == PHYCTRL_UCODE_BCST_END  && 
                                       !PHY84756_SINGLE_PORT_MODE(pc) )))) {
        SOC_IF_ERROR_RETURN(
            PHY_FIRMWARE_SET(PHY84756_40G, unit, port,offset,data,len));
...
phy_84741_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        ...
        soc_cm_debug(DK_WARN,"%x init u=%d port=%d rom code ver. 0x%x: %s boot\n", devid,
        ...
        if (MOD_AUTO_DETECT(pc)) {
            if ((ucode_ver & 0x0fff) < 0x105) {
                ...
                SOC_DEBUG_PRINT((DK_PHY, "PHY84740 QSFP module auto detect enabled: u=%d p=%d\n", 
                                 unit, port));
                ...
                SOC_DEBUG_PRINT((DK_PHY, "PHY84740 SFP module auto detect on: u=%d p=%d\n", 
                                 unit, port));
...
phy_84741_an_set(int unit, soc_port_t port, int an)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "phy_84741_an_set: u=%d p=%d an=%d\n",
                     unit, port, an));
    if (FCMAP_PASSTHROUGH(pc)) {
        return SOC_E_NONE;
    }
        ...
        if (!(status & PHY84740_MOD_AUTO_DETECT_STS_NOT_DONE)) {
            if (MOD_AUTO_DETECT_MSG(pc)) {
                ...
                if (status & PHY84740_MOD_AUTO_DETECT_STS_NOT_RES) {
                    ...
                    SOC_DEBUG_PRINT((DK_ERR, "PHY84740 could not detect module, "
            ...
            (void) (READ_PHY84740_MMF_PMA_PMD_REG(unit, pc, 0x81f3, &mod_131));
            MOD_AUTO_DETECT_MSG(pc) = 0;
            SOC_DEBUG_PRINT((DK_ERR, "PHY84740 i2c failed while detecting module, "
                             "defaulting to SR(4): u=%d p=%d sts=0x%x (0x%x:0x%x)\n", 
                             unit, port, status, mod_130, mod_131));
        }
    } else {
    ...
    int_pc = INT_PHY_SW_STATE(unit, port);
     ...
     if (PHY84740_SINGLE_PORT_MODE(pc)) {
         ...
         soc_cm_print("   pma_link(1.0x%04x)=0x%0x(%s), serdes_link=%s\n",
                      ...
                      dbgi.ilink ? "up" : "down");
         soc_cm_print("   line polarity(1.%04x)=(ln0=%04x, ln1=%04x, ln2=%04x, ln3=%04x)\n",
                      PHY84740_DIG_CTRL_REG,
                      dbgi.tx_polarity_reg[0], dbgi.tx_polarity_reg[1], 
                      dbgi.tx_polarity_reg[2], dbgi.tx_polarity_reg[3]);
         soc_cm_print("   sys  polarity(1.%04x)=(ln0=%04x, ln1=%04x, ln2=%04x, ln3=%04x)\n",
                      PHY84740_DIG_CTRL_REG,
   ...
   void *arg)     /* command argument  */
{
    ...
    soc_cm_debug(DK_PHY,"phy_84741_diag_ctrl: u=%d p=%d ctrl=0x%x\n", 
                 unit, port,op_cmd);
...
phy_84741_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    soc_cm_debug(DK_PHY,"phy_84741_speed_set: u=%d p=%d speed=%d\n", 
                 unit, port,speed);
...
phy_84741_rom_wait(int unit, int port)
{
    ...
    if (!(rd_data & 0x0100)) {
        soc_cm_debug(DK_ERR,"phy_84741_rom_program: u = %d p = %d "
                            "timeout 1\n", unit, port);
        return SOC_E_TIMEOUT;
    }
...
_phy_84741_firmware_to_rom(int unit, int port, int offset, uint8 *array,int datalen)
{
    ...
    soc_cm_debug(DK_PHY, "phy_84741_rom_program: u=%d p=%d done\n", unit, port);
                    ...
                    int addr, int count, void *data_array,uint32 ram_start)
{
    ...
    soc_cm_debug(DK_PHY,"phy_84741_bsc_read: u=%d p=%d addr=%04x\n",
                 unit, port, addr);
