...
/*
 *  Function : drv_nsp_queue_prio_remap_set
 *
 *  Purpose :
 ...
 *                  Specical design for NorthStarPlus. Check it in Note.
 ...
 *
 *  Note :
 *  1. The PCP to TC mapping in NorthStarPlus must be set indicidully per 
 ...
 */
int
drv_nsp_queue_prio_remap_set(int unit, uint32 port, uint8 pre_prio, 
    uint8 prio)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s:port=%d,pre_prio=%d,prio=%d\n", 
    ...
    if (pre_prio & ~(DRV_NSP_TAG_PRIORITY_MASK)){
        return SOC_E_PARAM;
    }
    DRV_NSP_TAG_PRIORITY_DEI_GET(pre_prio, in_dei);
    DRV_NSP_TAG_PRIORITY_PCP_GET(pre_prio, in_pcp);
...
/*
 *  Function : drv_nsp_queue_prio_remap_get
 *
 *  Purpose :
 ...
 */
int
drv_nsp_queue_prio_remap_get(int unit, uint32 port, uint8 pre_prio, 
    uint8 *prio)
{
...
/*
 *  Function : drv_nsp_queue_port_prio_to_queue_set
 *
 *  Purpose :
 ...
 */
int
drv_nsp_queue_port_prio_to_queue_set(int unit, uint8 port, 
    uint8 prio, uint8 queue_n)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s:unit %d,port=%d,prio=%d,queue_n=%d\n", 
    ...
    if (port32 == additional_soc_port_num) {
        SOC_IF_ERROR_RETURN(REG_READ_P7_TC2COS_MAPr
            (unit, &reg_value));
    } else if (IS_CPU_PORT(unit, port32)) {
...
/*
 *  Function : drv_nsp_queue_port_prio_to_queue_get
 *
 *  Purpose :
 ...
 */
int
drv_nsp_queue_port_prio_to_queue_get(int unit, uint8 port, 
    uint8 prio, uint8 *queue_n)
{
    uint32  reg_value, temp, additional_soc_port_num;
    /* Using 32b port variable to compare against SOC_MAX_NUM_PORTS,
...
/*
 *  Function : drv_nsp_queue_prio_set
 *
 *  Purpose :
 ...
 */
int 
drv_nsp_queue_prio_set(int unit, uint32 port, uint8 prio, 
    uint8 queue_n)
{
    uint32  p;
    soc_pbmp_t  pbmp;

    soc_cm_debug(DK_VERBOSE, "%s:unit %d,port=%d,priority=%d,queue=%d\n",
        FUNCTION_NAME(), unit, port, prio, queue_n);
...
/*
 *  Function : drv_nsp_queue_prio_get
 *
 *  Purpose :
 ...
 */
int 
drv_nsp_queue_prio_get(int unit, uint32 port, uint8 prio, 
    uint8 *queue_n)
{
    uint32  p;
    uint8   new_prio = 0;
...
/*
 *  Function : drv_nsp_queue_mode_set
 *
 *  Purpose :
 ...
 */
int 
drv_nsp_queue_mode_set(int unit, soc_pbmp_t bmp, uint32 flags, 
    uint32 mode)
{
    uint32  reg_value, temp, p;
    soc_pbmp_t in_pbmp;

    soc_cm_debug(DK_VERBOSE, "%s:unit %d,bmp=0x%x,flag=0x%x,queue mode=%d\n",
...
/*
 *  Function : drv_nsp_queue_mode_get
 *
 *  Purpose :
 ...
 */  
int 
drv_nsp_queue_mode_get(int unit, uint32 port, uint32 flags, 
    uint32 *mode)
{
    ...
    soc_cm_debug(DK_VERBOSE, "%s:unit %d,port=%d,flags=0x%x,mode=%d\n",
...
/*
 *  Function : drv_nsp_queue_WRR_weight_set
 *
 *  Purpose :
 ...
 */
int 
drv_nsp_queue_WRR_weight_set(int unit, uint32 port_type, 
...
/*
 *  Function : drv_nsp_queue_WRR_weight_get
 *
 *  Purpose :
 ...
 */
int 
drv_nsp_queue_WRR_weight_get(int unit, uint32 port_type, 
...
/*
 *  Function : drv_nsp_queue_qos_control_set
 *
 *  Purpose :
 *      Set the qos control.
 *
 ...
 */
int 
drv_nsp_queue_qos_control_set(int unit, uint32 port, uint32 type, uint32 state)
{
    uint32  reg_value, temp, p;
    soc_pbmp_t pbm;
...
/*
 *  Function : drv_nsp_queue_qos_control_get
 *
 *  Purpose :
 *      Get the qos control.
 *
 ...
 */ 
int 
drv_nsp_queue_qos_control_get(int unit, uint32 port, uint32 type, uint32 *state)
{
