    ...
    uint32 entry_id, uint32 count, uint32 *entry)
{
    ...
    if (((entry_id) < index_min) || 
        ...
        if ((count < 1) || (((entry_id + count - 1) > index_max) && 
            ...
            soc_cm_debug(DK_WARN, "%s,mem_id=0x%x, invalid entry_id=0x%x\n",
        ...
        uint32 *result_entry, int  *arl_index)
{
    ...
    if (!temp){
        ...
        soc_cm_debug(DK_ARL,"%s,%d,SVL mode,vid=%d reset to vid=0\n",
    ...
    SOC_IF_ERROR_RETURN(rv);        
    soc_cm_debug(DK_ARL,"%s,%d,eid_crc16=%d,eid_crc32=%d\n",
    ...
    for (i = 0; i < ROBO_TBX_L2_BUCKET_SIZE; i++){
        ...
        *(((uint32 *)&temp_sw_arl) + 2) =  COMPILER_64_LO(temp_reg_val);
        soc_cm_debug(DK_ARL, "%s,%d,temp_sw_arl[2-0]=%08x-%08x-%08x\n",
        ...
        if (int_flag == _TB_ARL_SEARCH_MATCH){
            ...
            soc_cm_debug(DK_ARL, "%s,%d, bin%d ab_status=%d, cd_status=%d!!\n",
    ...
    if (flags & DRV_MEM_OP_SEARCH_DONE) {
        ...
        if (search_valid){
            ...
            for (retry = 0; retry < SOC_TIMEOUT_VAL; retry++) {
                ...
                if (temp) {
                    if (SOC_DEBUG_CHECK(SOC_DBG_ARL) && retry){
                        soc_cm_print("=search_vallid (%x), table address valid (%x) retry: %d\n", 
                            srch_ctrl, reg_value,retry);
                    }                
                    break;
                } else {
                    if (SOC_DEBUG_CHECK(SOC_DBG_ARL)){
                        soc_cm_print("=search_vallid (%x), but table address not valid(%x) retry: %d\n", 
                            srch_ctrl, reg_value,retry);
                    }
                }
            }
...
_drv_tbx_arl_hash_index_get(int unit, int is_crc16, uint32 *sw_arl, int *index)
{
    ...
    *index = -1;
    ...
    if (!temp){
        /* means device is at SVL mode, VID will be zero */
        soc_cm_debug(DK_ARLMON,"%s,%d,SVL mode,vid=%d reset to vid=0\n",
                FUNCTION_NAME(),__LINE__,vid_key);
        vid_key = 0;
    }
    ...
    if (fastaging_flags) {
        ...
        if (temp_flags){
            if (temp_flags == DRV_MEM_OP_DELETE_BY_STATIC_ONLY){
                ...
                soc_cm_debug(DK_WARN, " Conflict fast aging control!\n");                
                return SOC_E_CONFIG;
            }
        } else {
        ...
        if (flags & DRV_MEM_OP_DELETE_BY_PORT){
            if (single_fastaging == FALSE){
                soc_cm_debug(DK_WARN, "  Multi-FastAging requested! "
        ...
        rv = _drv_tbx_search_key_op(unit, entry, _TB_ARL_SEARCH_MATCH,
                (uint32 *)&temp_sw_arl, &table_index);
        ...
        if (rv < 0) {
            soc_cm_debug(DK_ERR,"%s,%d,SOC driver problem!\n",
        ...
        if (is_dynamic && is_ucast){
            ...
            soc_cm_debug(DK_ARL,"%s,port%d, SA_LRN_CNT decreased one!\n",
                    FUNCTION_NAME(), temp);
        }
    ...
    switch (mem)
    {
        ...
        case DRV_MEM_VPORT_VID_MAP:            
            if (field_index == DRV_MEM_FIELD_VPORT_UNTAG){
                ...
                soc_cm_debug(DK_ERR, "%s,%d,TBD!(UNAVAIL)\n", 
                        FUNCTION_NAME(), __LINE__);
                return SOC_E_UNAVAIL;
            }
            break;
    ...
    uint32 field_index, uint32 *entry, uint32 *fld_data)
{
    ...
    if (!fieldinfo){
        soc_cm_debug(DK_ERR, "%s mem %d: no matched field %d\n", 
    ...
    uint32 *key, uint32 *entry, uint32 *entry_1, uint32 flags)
{
    soc_control_t   *soc = SOC_CONTROL(unit);
    int     rv = SOC_E_NONE;
    int     index, cf_index[ROBO_TBX_L2_BUCKET_SIZE];
    ...
    /* TB have no feature on searching OP by MAC key only without VID key */
        ...
        if (flags & DRV_MEM_OP_SEARCH_CONFLICT){
            ...
            rv = _drv_tbx_search_key_op(unit, key, 
                    _TB_ARL_SEARCH_CONFLICT, entry, cf_index);
        } else {
             ...
             */
            rv = _drv_tbx_search_key_op(unit, key, 
                    _TB_ARL_SEARCH_MATCH, entry, &index);
        }
        MEM_RWCTRL_REG_UNLOCK(soc);
    } else {
        soc_cm_debug(DK_WARN, 
        ...
        uint32 flags)
{
    ...
    if (rv < 0) {
        soc_cm_debug(DK_ERR,"%s,%d,SOC problem!\n",FUNCTION_NAME(),__LINE__);
        return rv;
    }
    ...
    rv = _drv_tbx_search_key_op(unit, entry, _TB_ARL_SEARCH_MATCH, 
            (uint32 *)&temp_sw_arl, &table_index);
    soc_cm_debug(DK_ARL,"%s,%d,rv=%d, id=%d\n", 
    ...
    if (rv == SOC_E_FULL) {
         ...
         */ 
        soc_cm_debug(DK_ARL,"%s,%d, entry existed already!!!\n",
                FUNCTION_NAME(), __LINE__);
        bin_id = table_index & _TB_ARL_INDEX_BIN_ID_MASK;
        ...
        */
        ...
        soc_cm_debug(DK_ERR,"%s,%d,ARL search problem!\n", 
            FUNCTION_NAME(),__LINE__);
        goto mem_insert_exit;
    }
    ...
    if (bin_id == -1){
        ...
        soc_cm_debug(DK_ARL,"%s,%d,sw_arl[2-0]=%08x-%08x-%08x\n",
                FUNCTION_NAME(), __LINE__, *(((uint32 *)&temp_sw_arl)+2),
        ...
        soc_cm_debug(DK_ARL,"%s,%d,HW_ARL[2-0]=%08x-%08x-%08x\n",
    ...
    uint32 entry_id, uint32 count, uint32 entry_mask, uint32 *entry)
{
    ...
    soc_cm_debug(DK_MEM, "_tb_mem_modify(mem=0x%x,entry_id=0x%x,count=%d)\n",
         mem, entry_id, count);
