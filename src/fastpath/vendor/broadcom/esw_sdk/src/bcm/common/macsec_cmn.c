/*
 * $Id: macsec_cmn.c 1.28 Broadcom SDK $
...
int _bcm_common_macsec_init(int unit)
{
    ...
    if (!_bmacsec_init_done) {
        ...
        bmacsec_config_print_set(BMACSEC_PRINT_PRINTF, soc_cm_print);
...
int bcm_common_macsec_config_print(uint32 level)
{
    ...
    rv = bmacsec_config_print_set(flag, soc_cm_print);
    return BCM_ERROR(rv);
}


#else
int _macsec_cmn_not_empty;
