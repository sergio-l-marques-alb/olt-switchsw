/*
 * $Id: auth.c 1.58.2.2 Broadcom SDK $
...
bcm_robo_auth_init(int unit)
{
    ...
    */
     ...
     */
    if (SOC_IS_ROBO_ARCH_VULCAN(unit) || SOC_IS_ROBO5395(unit) || 
...
bcm_robo_auth_detach(int unit)
{
    ...
    if (SOC_IS_ROBO5324(unit)) {
        BCM_PBMP_REMOVE(t_pbm, PBMP_GE_ALL(unit));
    }
