...
{
    if ((zone->vsan.vsan > FCOE_MAX_VSAN_ID)      ||  /* 12 bits */
...
bcm_td2_fcoe_zone_add(int unit, bcm_fcoe_zone_entry_t *zone)
{
    ...
    soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry,
                        FCOE_ZONE__VSAN_IDf, zone->vsan.vsan);  /* vsan ID */
    soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry,
...
int 
bcm_td2_fcoe_vsan_create (int unit,  bcm_fcoe_vsan_t *vsan)
{
    ...
    rv = _bcm_fcoe_vsan_param_validate(vsan);
    if (rv != BCM_E_NONE) {
        return rv;
    }

    /* check if vsan id is already created */
    if (shr_idxres_list_elem_state(vsan_id_list[unit], vsan->vsan) !=
            BCM_E_NOT_FOUND) {
        /* exception : if intention is to replace existing ID, overwrite
         * existing entry */
        if ((vsan->flags & BCM_FCOE_VSAN_REPLACE_ID) == 0) {
            return BCM_E_EXISTS;
        }
    }
    ...
    if (vsan->flags & BCM_FCOE_VSAN_NORMALIZED_ZONE_CHECK) {
    ...
    if (vsan->flags & BCM_FCOE_VSAN_FCOE_ROUTE_ENABLE) {
        soc_mem_field32_set(unit, mem, (uint32 *)&vsan_entry,
    ...
    if (rv == BCM_E_NONE) {
        ...
        if (shr_idxres_list_elem_state(vsan_id_list[unit], vsan->vsan) ==
            BCM_E_NOT_FOUND) {
            return _bcm_fcoe_reserve_vsan_id(unit, vsan->vsan);
        }
    }
    return rv;
}
...
/* Destroy a VSAN */
int 
bcm_td2_fcoe_vsan_destroy (int unit,  bcm_fcoe_vsan_t *vsan)
{
    ing_vsan_entry_t  vsan_entry;
    soc_mem_t         mem = ING_VSANm;
    int               rv = BCM_E_NONE;
    ...
    if (rv == BCM_E_NONE) {
        ...
        return _bcm_fcoe_unreserve_vsan_id(unit, vsan->vsan);
    }
    return rv;
}
                       ...
                       bcm_fcoe_vsan_t *vsan)
{
    ...
    flag = soc_mem_field32_get(unit, mem, (uint32 *)&vsan_entry, 
                               ZONE_CHECK_NORMALIZEDf);
    if (flag) {
        vsan->flags |= BCM_FCOE_VSAN_NORMALIZED_ZONE_CHECK;
    }
    flag = soc_mem_field32_get(unit, mem, (uint32 *)&vsan_entry, 
                               FCOE_ROUTE_ENABLEf);
    if (flag) {
        vsan->flags |= BCM_FCOE_VSAN_FCOE_ROUTE_ENABLE;
    }
                                     ...
                                     bcm_stat_value_t      *counter_values)
{
    ...
    for (table_count = 0; table_count < num_of_tables ; table_count++) {
         if (table_info[table_count].direction == direction) {
             for (index_count = 0; index_count < num_entries ; index_count++) {
                  BCM_IF_ERROR_RETURN(
                      _bcm_esw_stat_counter_get(unit,
                                                table_info[table_count].index,
                                                table_info[table_count].table,
                                                byte_flag,
                                                counter_indexes[index_count],
                           ...
                           void *user_data)
{
    ...
    for (chunk = soc_mem_index_min(unit, mem); chunk <= mem_idx_max;
        ...
        for (ix = 0; ix <= chnk_end; ix++) {
           ...
           zone.vsan.vsan = soc_mem_field32_get(unit, mem, l2ep, 
                                                FCOE_ZONE__VSAN_IDf); 
                           ...
                           void *user_data)
{
    ...
    for (index = 0; index <= FCOE_MAX_VSAN_ID; index++) {
        ...
        /* call user callback function with a vsan entry */
        rv = (trav_fn)(unit, &vsan, user_data);
