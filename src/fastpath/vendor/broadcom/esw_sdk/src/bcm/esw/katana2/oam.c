/* $Id: oam.c 1.34.4.5 Broadcom SDK $
...
_bcm_kt2_oam_group_endpoint_count_init(int unit, _bcm_oam_control_t *oc)
{
    ...
    OAM_VERB(("OAM(unit %d) Info: Total No. endpoint Count = %d.\n",
               unit, oc->ep_count));

    /* Max number of MA Groups supported by device. */
    oc->group_count = soc_mem_index_count(unit, MA_STATEm);
    OAM_VERB(("OAM(unit %d) Info: Total No. Group Count = %d.\n",
               unit, oc->group_count));
                               ...
                               uint32 *entry, void *ma_rmep)
{
    ...
    if (clear_mask && clear_p_faults) {
        ...
        OAM_VERB(("OAM(unit %d): clear_mask %d.\n", unit, clear_mask));
                             ...
                             bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if (h_data_p->oam_domain == _BCM_OAM_DOMAIN_PORT) {
        ...
        if ((ep_p->flags & BCM_OAM_ENDPOINT_LOSS_MEASUREMENT)) {
            ...
            if (BCM_FAILURE(rv)) {
                OAM_ERR(("OAM(unit %d) Error: Counter config failed for EP=%d"\
                          "  %s.\n", unit, ep_p->id, bcm_errmsg(rv)));
                                  ...
                                  bcm_oam_endpoint_info_t *ep_info_p)
{
    ...
    if ((ep_info_p->level < 0) || (ep_info_p->level > _BCM_OAM_EP_LEVEL_MAX)) {
        OAM_ERR(("OAM(unit %d) Error: EP Level should be in the range(0-%d).\n",
    ...
    if (ctr_valid) {
        ...
        if ((ctr_type == up_mep) && (ctr_mdl == h_data_p->level)) {
            ...
            *map = soc_mem_field32_get(unit, mem, (uint32 *)entry, 
            ...
            if (BCM_FAILURE(rv)) {
                OAM_ERR(("OAM(unit %d) Error: LM counter profile ref count  \
                         ...
                         port_tab_entry_t *port_entry)
{
    ...
    for (mdl = 0; mdl < _BCM_OAM_EP_LEVEL_COUNT; mdl++) {
        ...
        if (mdl_bitmap & (1 << mdl)) {
            ...
            if (BCM_FAILURE(rv)) {
                OAM_ERR(("OAM(unit %d) Error: Profile entry recover failed \
                          (EP=%d) - %s.\n", unit, ep_id, bcm_errmsg(rv)));
                goto cleanup;
            }
                         ...
                         egr_mp_group_entry_t *egr_mp_grp_entry)
{
    ...
    for (mdl = 0; mdl < _BCM_OAM_EP_LEVEL_COUNT; mdl++) {
        ...
        if (mdl_bitmap & (1 << mdl)) {
            ...
            if (BCM_FAILURE(rv)) {
                OAM_ERR(("OAM(unit %d) Error: DGLP profile ref count  \
                      increment failed (EP=%d) - %s.\n", unit,
                      h_data_p->ep_id, bcm_errmsg(rv)));
                goto cleanup;
            }
...
_bcm_kt2_oam_lmep_tx_config_recover(int unit)
{
    ...
    for (index = 0; index < oc->lmep_count; index++) {
        ...
        if (BCM_FAILURE(rv)) {
            OAM_ERR(("OAM(unit %d) Error: LMEP_1 table read (index=%d) failed "\
                    "- %s.\n", unit, index, bcm_errmsg(rv)));
            goto cleanup;
        }
        ...
        if (soc_MAID_REDUCTIONm_field32_get(unit, &maid_red_ent, VALIDf)) {
            ...
            if (h_data_p->oam_domain != _BCM_OAM_DOMAIN_PORT) {
                ...
                if (BCM_FAILURE(rv)) {
                    OAM_ERR(("OAM(unit %d) Error: Unable to increment key ref count "\
                           " - %s.\n", unit, bcm_errmsg(rv)));
                    goto cleanup;
                } 
                                ...
                                _bcm_oam_hash_key_t  *hash_key)
{
    ...
    if((is_remote) && (endpoint_info->local_id < _BCM_OAM_BHH_KT2_ENDPOINT_OFFSET)) {
        return (BCM_E_PARAM);
    }
    ...
    if (endpoint_info->flags & BCM_OAM_ENDPOINT_WITH_ID) {
        ...
        if((ep_req_index < _BCM_OAM_BHH_KT2_ENDPOINT_OFFSET) ||
           (ep_req_index >= (_BCM_OAM_BHH_KT2_ENDPOINT_OFFSET + oc->bhh_endpoint_count))) {
            rv = BCM_E_PARAM;
        }
...
_bcm_kt2_oam_bhh_hw_init(int unit)
{
    ...
    /* Katana2 registers are similar to Katana */
     ...
     */
     ...
     if(max_cosq < min_cosq) {
        BCM_ERR(("BHH(unit %d) Error: No BHH COS Queue available from uC%d - %s\n",
