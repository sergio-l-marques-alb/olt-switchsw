/*
 * $Id: vlan.c 1.250.2.1 Broadcom SDK $
                      ...
                      uint32 *buf, bcm_vlan_t vlan, int profile_index)
{
  ...
  /*
  * Function:
  *      _vlan_protocol_pkt_ctrl_get
  * Purpose:
  *      function to get protocol packet action for given
  *      VLAN profile index.
  * Parameters:
  ...
  */
STATIC int
_vlan_protocol_pkt_ctrl_get(int unit, int proto_index,
                                bcm_vlan_protocol_packet_ctrl_t *protocol_pkt)
{
    soc_reg_t  reg;
    uint64     regval;
 ...
 /*
 * Function:
 *      _vlan_protocol_pkt_ctrl_set
 * Purpose:
 *      function to set protocol packet action for given
 *      VLAN profile index.
 * Parameters:
 *      unit - (IN) StrataSwitch PCI device unit number (driver internal).
 ...
 */
STATIC int
_vlan_protocol_pkt_ctrl_set(int unit, int vlan_profile_index,
                                bcm_vlan_protocol_packet_ctrl_t *protocol_pkt,
                                int *protocol_pkt_index)
{
    ...
    soc_mem_t mem = VLAN_PROFILE_TABm;
    soc_reg_t reg;
    ...
    if (SOC_REG_FIELD_VALID(unit, reg, ARP_REPLY_TO_CPUf)) {
        ...
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, ARP_REPLY_TO_CPUf, value);
    }
    if (SOC_REG_FIELD_VALID(unit, reg, ARP_REPLY_DROPf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_DROP_ENABLE ? 1 : 0;
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, ARP_REPLY_DROPf, value);
    }
    ...
    if (SOC_REG_FIELD_VALID(unit, reg, ARP_REQUEST_DROPf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_DROP_ENABLE ? 1 : 0;
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, ARP_REQUEST_DROPf, value);
    }
    ...
    if (SOC_REG_FIELD_VALID(unit, reg, DHCP_PKT_TO_CPUf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_TOCPU_ENABLE ? 1 : 0;
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, DHCP_PKT_TO_CPUf, value);
    }
    if (SOC_REG_FIELD_VALID(unit, reg, DHCP_PKT_DROPf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_DROP_ENABLE ? 1 : 0;
    ...
    if (SOC_REG_FIELD_VALID(unit, reg, MMRP_PKT_TO_CPUf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_TOCPU_ENABLE ? 1 : 0;
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, MMRP_PKT_TO_CPUf, value);
    }
    if (SOC_REG_FIELD_VALID(unit, reg, MMRP_FWD_ACTIONf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE ? 2 :
               (pkt_action & BCM_VLAN_PROTO_PKT_DROP_ENABLE ? 1 : 0);
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, MMRP_FWD_ACTIONf, value);
    }
    pkt_action = protocol_pkt->srp_action;
    if (SOC_REG_FIELD_VALID(unit, reg, SRP_PKT_TO_CPUf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_TOCPU_ENABLE ? 1 : 0;
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, SRP_PKT_TO_CPUf, value);
    }
    if (SOC_REG_FIELD_VALID(unit, reg, SRP_FWD_ACTIONf)) {
        value = pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE ? 2 :
               (pkt_action & BCM_VLAN_PROTO_PKT_DROP_ENABLE ? 1 : 0);
        soc_reg_field_set(unit, reg, &prot_pkt_ctrl, SRP_FWD_ACTIONf, value);
    }
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_prot_pkt_ctrl_add(unit, prot_pkt_ctrl, igmp_mld_pkt_ctrl, &index));
    if ((index != old_index) && (ref_count > 0)) {
...
_vlan_profile_read(int unit, int index, _vlan_profile_t *entry)
{
    ...
    if (soc_feature (unit, soc_feature_vlan_protocol_pkt_ctrl)
        && SOC_MEM_FIELD_VALID (unit,mem, PROTOCOL_PKT_INDEXf)) {
        proto_index = soc_mem_field32_get (unit, mem, buf, PROTOCOL_PKT_INDEXf);
        rv = _vlan_protocol_pkt_ctrl_get(unit,
...
_vlan_profile_write(int unit, int index, _vlan_profile_t *entry)
{
    ...
    if (soc_feature (unit, soc_feature_vlan_protocol_pkt_ctrl)
        ...
        rv = _vlan_protocol_pkt_ctrl_set(unit, index,
        ...
        if (soc_feature(unit, soc_feature_vlan_ctrl)) {
            ...
            if (soc_feature (unit, soc_feature_vlan_protocol_pkt_ctrl)
                && SOC_MEM_FIELD_VALID (unit,
                                VLAN_PROFILE_TABm, PROTOCOL_PKT_INDEXf)) {
                uint32 proto_profile_index;
                uint32  buf[SOC_MAX_MEM_FIELD_WORDS];
                BCM_IF_ERROR_RETURN
                            (_bcm_prot_pkt_ctrl_add(unit, 0, 0,
                                                    &proto_profile_index));
                rv = soc_mem_read(unit,
                        VLAN_PROFILE_TABm, MEM_BLOCK_ANY, profile_index, buf);
                BCM_IF_ERROR_RETURN(rv);
                soc_mem_field32_set(unit, VLAN_PROFILE_TABm,
                               buf, PROTOCOL_PKT_INDEXf, proto_profile_index);
                rv = soc_mem_write(unit, VLAN_PROFILE_TABm, MEM_BLOCK_ALL,
                                   profile_index, buf);
                BCM_IF_ERROR_RETURN(rv);
            }
#endif /* BCM_TRX_SUPPORT */
        ...
        if (soc_feature(unit, soc_feature_vlan_ctrl)) {
            ...
            if (use_default) {
                ...
                if (soc_feature (unit, soc_feature_vlan_protocol_pkt_ctrl)
                    ...
                    uint32 proto_profile_index;
                    uint32  buf[SOC_MAX_MEM_FIELD_WORDS];
                    rv = _bcm_prot_pkt_ctrl_add(unit, 0, 0,
                                                        &proto_profile_index);
                    if (BCM_FAILURE(rv)) {
                        soc_mem_unlock(unit, table);
                        return rv;
                          ...
                          bcm_vlan_control_vlan_t *control)
{
 ...
 || defined(BCM_RAVEN_SUPPORT)
    ...
    if (soc_feature(unit, soc_feature_l3_iif_profile) && (!ingress_map_mode)) {
        BCM_IF_ERROR_RETURN(
            _bcm_td2_l3_intf_iif_profile_get(unit, vid, control));        
    }
...
#if defined(BCM_TRIUMPH_SUPPORT) && defined(INCLUDE_L3) && defined(BCM_TRIDENT2_SUPPORT)
        if (soc_feature(unit, soc_feature_nat) && 
            SOC_MEM_IS_VALID(unit, L3_IIFm) && 
            SOC_MEM_FIELD_VALID(unit, L3_IIFm, SRC_REALM_IDf)) {
            BCM_IF_ERROR_RETURN(_bcm_tr_l3_intf_nat_realm_id_get(unit,
                                ingress_map_mode ? control->ingress_if : vid,
                                &control->nat_realm_id));
        }
#endif /* BCM_TRIUMPH_SUPPORT && INCLUDE_L3  && BCM_TRIDENT2_SUPPORT */
        soc_mem_unlock(unit, VLAN_TABm);
    }
#endif /* BCM_FIREBOLT2_SUPPORT || BCM_TRX_SUPPORT || BCM_RAVEN_SUPPORT */
    return rv;
...
int _vlan_protocol_pkt_action_validate(int pkt_action)
{
                                ...
                                bcm_vlan_control_vlan_t *control)
{
    ...
    if (soc_feature (unit, soc_feature_vlan_protocol_pkt_ctrl)
        && SOC_MEM_FIELD_VALID(unit, VLAN_PROFILE_TABm, PROTOCOL_PKT_INDEXf)) {
        pkt_action = control->protocol_pkt.mmrp_action;
        if (pkt_action) {
        ...
        pkt_action = control->protocol_pkt.srp_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.arp_reply_action;
        if ((pkt_action & BCM_VLAN_PROTO_PKT_FORWARD_ENABLE) ||
            (pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE)) {
            return BCM_E_PARAM;
        }
        pkt_action = control->protocol_pkt.arp_request_action;
        if ((pkt_action & BCM_VLAN_PROTO_PKT_FORWARD_ENABLE) ||
            (pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE)) {
            return BCM_E_PARAM;
        }
        pkt_action = control->protocol_pkt.nd_action;
        if ((pkt_action & BCM_VLAN_PROTO_PKT_FORWARD_ENABLE) ||
            (pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE)) {
            return BCM_E_PARAM;
        }
        pkt_action = control->protocol_pkt.dhcp_action;
        if ((pkt_action & BCM_VLAN_PROTO_PKT_FORWARD_ENABLE) ||
            (pkt_action & BCM_VLAN_PROTO_PKT_FLOOD_ENABLE)) {
            return BCM_E_PARAM;
        }
        
        pkt_action = control->protocol_pkt.igmp_report_leave_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.igmp_query_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.igmp_unknown_msg_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.mld_report_done_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.mld_query_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.ip4_rsvd_mc_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.ip6_rsvd_mc_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.ip4_mc_router_adv_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
        ...
        pkt_action = control->protocol_pkt.ip6_mc_router_adv_action;
        if (pkt_action) {
            if (_vlan_protocol_pkt_action_validate(pkt_action)
                    == BCM_E_PARAM) {
                return BCM_E_PARAM;
