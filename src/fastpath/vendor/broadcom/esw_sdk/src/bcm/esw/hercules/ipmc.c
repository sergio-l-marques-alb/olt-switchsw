/*
 * $Id: ipmc.c 1.13.310.1 Broadcom SDK $
...
bcm_hercules_ipmc_init(int unit)
{
	...
	imin = soc_mem_index_min(unit, MEM_MCm);
	imax = soc_mem_index_max(unit, MEM_MCm);

	SOC_MEM_BLOCK_ITER(unit, MEM_MCm, blk) {
	    for (i = imin; i <= imax; i++) {
		SOC_IF_ERROR_RETURN(READ_MEM_MCm(unit, blk, i, &ent));
		soc_MEM_MCm_field32_set(unit, &ent, IPMCBITMAPf, 0);
		SOC_IF_ERROR_RETURN(WRITE_MEM_MCm(unit, blk, i, &ent));
	    }
	}
	return BCM_E_NONE;
    }
