/*
 * $Id: rate.c 1.53.6.10 Broadcom SDK $
                   ...
                   uint32 fmask, uint32 *pRps, uint32 *pBrs)
{
    ...
    if (byte_mode == mode) {
        ...
        if (!SOC_IS_SHADOW(unit) && (fmask & BCM_RATE_KNOWN_MCAST)) {
            if (soc_reg_field_get(unit, STORM_CONTROL_METER_CONFIGr, ctrlMeter,
                                  KNOWN_L2MC_ENABLEf) ||
                soc_reg_field_get(unit, STORM_CONTROL_METER_CONFIGr, ctrlMeter,
                                  KNOWN_IPMC_ENABLEf)) {
        ...
        if (!SOC_IS_SHADOW(unit) && (fmask & BCM_RATE_UNKNOWN_MCAST)) {
            if (soc_reg_field_get(unit, STORM_CONTROL_METER_CONFIGr, ctrlMeter,
                                  UNKNOWN_L2MC_ENABLEf) ||
                soc_reg_field_get(unit, STORM_CONTROL_METER_CONFIGr, ctrlMeter,
                                  UNKNOWN_IPMC_ENABLEf)) {
        ...
        if (!SOC_IS_SHADOW(unit) &&(fmask & BCM_RATE_DLF)) {
            if (soc_reg_field_get(unit, STORM_CONTROL_METER_CONFIGr, ctrlMeter,
                                  DLFBC_ENABLEf)) {
