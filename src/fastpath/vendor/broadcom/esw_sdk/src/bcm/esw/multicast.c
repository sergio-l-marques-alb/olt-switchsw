/*
 * $Id: multicast.c 1.138.2.1 Broadcom SDK $
...
#define _BCM_MULTICAST_MTU_TABLE_OFFSET(unit) (SOC_IS_HURRICANEX(unit) ? 512 : 8192)
...
{
    if (_BCM_MULTICAST_IS_L3(group) || _BCM_MULTICAST_IS_TRILL(group)) {
...

{
    ...
    if (SOC_IS_HURRICANE2(unit)) {
        /* max replication interface is limited by number of EGR_L3_INTFm */
