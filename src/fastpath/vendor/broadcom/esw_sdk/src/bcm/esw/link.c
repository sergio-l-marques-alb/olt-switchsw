/*
 * $Id: link.c 1.193.14.1 Broadcom SDK $
 ...
 *
 */
#define BCM_LINK_PBMP_SCACHE_NUM        12
...
_bcm_esw_link_failover_port_disable(int unit, int port)
{
    if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port)) {
        ...
        if (SOC_REG_IS_VALID(unit, MAC_CTRLr)) {
            BCM_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, MAC_CTRLr, port, RXENf, 0));
        } else if (SOC_REG_IS_VALID(unit, XMAC_CTRLr)) {
            BCM_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XMAC_CTRLr, port, RX_ENf, 0));
        } else {
            BCM_IF_ERROR_RETURN
                (soc_reg_field32_modify(unit, XLMAC_CTRLr, port, RX_ENf, 0));
        }
...
_bcm_esw_linkscan_update_port(int unit, int port)
{
    ...
    defined(BCM_VALKYRIE2_SUPPORT)
        ...
        if(SOC_IS_HAWKEYE(unit) && new_link) {
            ...
            BCM_IF_ERROR_RETURN(
                   bcm_esw_port_medium_get(unit, port, &medium));
            ...
            if(soc_feature(unit, soc_feature_eee)) {
                ...
                if ((need_pau_eee_war) || (need_uni_sw_reset)) {
                     _bcm_port_info_access(unit, port, &port_info); 
                    SOC_IF_ERROR_RETURN(MAC_CONTROL_SET(port_info->p_mac, 
        ...
        if (SOC_IS_HURRICANE (unit) || SOC_IS_TD_TT (unit) ||
            ...
            if(soc_feature(unit, soc_feature_eee)) {
                ...
                if (MAC_CONTROL_GET(port_info->p_mac, unit, port,
                    SOC_MAC_CONTROL_EEE_ENABLE, &mac_val) == BCM_E_NONE) {
                    if (new_link && (soc_phyctrl_ability_remote_get(unit, port, &remote_advert) == SOC_E_NONE)) {
