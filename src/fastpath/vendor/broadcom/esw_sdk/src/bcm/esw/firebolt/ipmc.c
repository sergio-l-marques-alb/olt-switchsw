...
bcm_fb_er_ipmc_add(int unit, bcm_ipmc_addr_t *ipmc)
{
    ...
    if (new_entry) {
        ...
        rv = bcm_fb_er_ipmc_put(unit, ipmc->group, ipmc);
        if (BCM_FAILURE(rv)) {
            if (SOC_IS_FBX(unit)) {
#if defined(BCM_FIREBOLT_SUPPORT)
                            ...
                            int check_port)
{
    ...
    if (!SOC_IPMCREPLSHR_GET(unit) && !SOC_IS_HURRICANEX(unit) && !soc_feature(unit, soc_feature_ipmc_repl_penultimate)) {
        return BCM_E_CONFIG;
    }
