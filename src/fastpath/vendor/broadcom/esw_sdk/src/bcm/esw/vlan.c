/*
 * $Id: vlan.c 1.464.2.8 Broadcom SDK $
...
#endif /* BCM_ENDURO_SUPPORT */
            {
                BCM_IF_ERROR_RETURN(bcm_tr2_vlan_virtual_detach(unit));
            } 
        }
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            if (_BCM_MULTICAST_IS_NIV(mc_group) ||
                ...
                rv = bcm_tr2_update_vlan_pbmp(unit, vid, &l2_l3_pbmp); 
                if (rv < 0) {
                    BCM_UNLOCK(unit);
                    return rv;
                }
 ...
 *
 */
int bcm_esw_vlan_stat_counter_get(
            int              unit,
            bcm_vlan_t       vlan,
            bcm_vlan_stat_t  stat,
            uint32           num_entries,
...
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    if (soc_feature(unit,soc_feature_advanced_flex_counter)) {
        return _bcm_esw_vlan_stat_counter_get(unit,vlan,stat,
...
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    if (soc_feature(unit,soc_feature_advanced_flex_counter)) {
        return _bcm_esw_vlan_translate_stat_counter_get(unit,port,key_type,
    ...
    if (soc_feature(unit,soc_feature_gport_service_counters)) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_esw_vlan_translate_stat_param_valid(unit, port, key_type,
                                                  outer_vlan, inner_vlan,
                                                  &vxlt_fsh));
        rv =  _bcm_esw_flex_stat_ext_get(unit, _bcmFlexStatTypeVxlt,
 ...
 *
 */
int bcm_esw_vlan_translate_egress_stat_counter_get(
            int              unit,
            int              port_class,
            bcm_vlan_t       outer_vlan,
            bcm_vlan_t       inner_vlan,
            ...
            bcm_stat_value_t *counter_values)
{
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT)
    if (soc_feature(unit,soc_feature_advanced_flex_counter)) {
        return _bcm_esw_vlan_translate_egress_stat_counter_get(unit,
    ...
    if (soc_feature(unit,soc_feature_gport_service_counters)) {
        ...
        rv =  _bcm_esw_flex_stat_ext_get(unit,_bcmFlexStatTypeEgrVxlt,
                vxlt_fsh,
