/*
 * $Id: stat_xe.c 1.69.2.1 Broadcom SDK $
 ...
 */
int
_bcm_stat_xe_get(int unit, bcm_port_t port, int sync_mode,
                 bcm_stat_val_t type, uint64 *val)
{
    ...
    case snmpIfOutUcastPkts:    /* ALL - mcast - bcast */
        ...
        if (soc_feature(unit, soc_feature_hw_stats_calc)) {
            ...
            REG_ADD(unit, port, sync_mode, ITUCr, count);  /* unicast pkts sent */
            REG_ADD(unit, port, sync_mode, ITUCAr, count);  /* unicast pkts sent */
        } else 
    ...
    case snmpEtherStatsFragments:
        ...
        if (SOC_REG_IS_VALID(unit, ITFRGr)) {
           REG_ADD(unit, port, sync_mode, ITFRGr, count);
        }
        break;
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_TRX_SUPPORT)
        if (soc_feature(unit, soc_feature_hw_stats_calc)) {
            REG_ADD(unit, port, sync_mode, ITPOKr, count); /* All good packets */
        } else 
#endif /* BCM_FIREBOLT2_SUPPORT || BCM_TRX_SUPPORT */
        {
            REG_ADD(unit, port, sync_mode, ITPKTr, count);
...
#if defined(BCM_FIREBOLT2_SUPPORT) || defined(BCM_TRX_SUPPORT)
        if (soc_feature(unit, soc_feature_hw_stats_calc)) {
            REG_ADD(unit, port, sync_mode, IRPOKr, count);
        } else 
#endif /* BCM_FIREBOLT2_SUPPORT || BCM_TRX_SUPPORT */
        {
            /*
    ...
    case snmpIfHCOutUcastPkts:
        ...
        if (soc_feature(unit, soc_feature_hw_stats_calc)) {
            /* This register has a different name on some devices.
             * Only one value is added here. */
            REG_ADD(unit, port, sync_mode, ITUCr, count); /* All good packets */
            REG_ADD(unit, port, sync_mode, ITUCAr, count);  /* unicast pkts sent */
        } else 
#endif /* BCM_FIREBOLT2_SUPPORT || BCM_TRX_SUPPORT */
        {
