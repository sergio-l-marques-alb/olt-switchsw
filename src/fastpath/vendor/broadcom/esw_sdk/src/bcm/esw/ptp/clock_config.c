/*
 * $Id: clock_config.c 1.1.2.16 Broadcom SDK $
 *
...
/*
 * Function:
 *      _bcm_esw_ptp_foreign_master_dataset
 * Purpose:
 ...
 *      ptp_id    - (IN)  PTP stack ID.
 *      clock_num - (IN)  PTP clock number.
 *      data_set  - (OUT) PTP clock priority1.
 * Returns:
 *      BCM_E_XXX
 * Notes:
 */
int
_bcm_esw_ptp_foreign_master_dataset_get(
    int unit,
    bcm_ptp_stack_id_t ptp_id,
    int clock_num,
    int port_num,
    _bcm_ptp_foreign_master_dataset_t *data_set)
{
    ...
    while (index < num_masters) {
        _bcm_ptp_foreign_master_entry_t *fm = &data_set->foreign_master[index];
        ...
        fm->clockClass_window.fallEvents = *(resp + offset++);
        fm->clockClass_window.riseEvents = *(resp + offset++);
    ...
    uint32 domain)
{
    ...
    if (BCM_FAILURE(rv = _bcm_ptp_signaled_unicast_slave_clear(unit, ptp_id,
