/*
 * $Id: trunk.c 1.198.2.23 Broadcom SDK $
 ...
 * Notes:
 *      For fabric trunk groups, the following combinations of module ID
 *      and port numbers are accepted: 
 *      - If tp[i] contains a GPORT ID, tm[i] is ignored. 
 *      - If tm[i] is BCM_MODID_INVALID or 0, tp[i] is interpreted as a
 *        local port.
 *      - If tm[i] is greater than 0, tm[i] is checked to make sure it's
 *        a local module ID by the _bcm_esw_trunk_gport_array_resolve
 *        function, and tp[i] is interpreted as a local port.
 */
int
                             ...
                             bcm_trunk_add_info_t *t_data)
{
    ...
    bcm_gport_t *gport_list = NULL;

    if (t_data->num_ports <= 0) {
        return BCM_E_NONE;
    }
       
    gport_list = sal_alloc(sizeof(bcm_gport_t) * t_data->num_ports,
                    "gport_list");
    if (gport_list == NULL) {
        return BCM_E_MEMORY;
    }
    sal_memset(gport_list, 0, sizeof(bcm_gport_t) * t_data->num_ports);
    ...
    for (i = 0; i < t_data->num_ports; i++) {
        if (BCM_GPORT_IS_SET(t_data->tp[i])) {
            ...
            if (TRUNK_FABRIC_TID(unit, tid)) {
                if ((BCM_MODID_INVALID == t_data->tm[i]) ||
                    (0 == t_data->tm[i])) {
                    gp_dest.gport_type = _SHR_GPORT_TYPE_LOCAL;
            ...
            rv = _bcm_esw_gport_construct(unit, &gp_dest, &(gport_list[i]));
            if (rv < 0) {
                sal_free(gport_list);
                return rv;
            }
