/*
 * $Id: l2.c 1.334.6.2 Broadcom SDK $
...
typedef struct _bcm_l2_vlan_port_pair_s {
    bcm_pbmp_t        pbm[BCM_VLAN_MAX+2]; /* Array of PBMP with max VLAN size, the first 0-4095 PBMP entries 
...
typedef struct _bcm_l2_del_vlan_port_pair_s {
    ...
    bcm_vlan_vector_t         del_by_vlan_vec; /* Vlan vector represents delete by vlan */
    _bcm_l2_vlan_port_pair_t  *mod_pair_arr;   /* Reference to Array of (vlan,port) for max modules */
...
_bcm_esw_l2_match_sw(int unit, bcm_l2_addr_t *info, void *user_data)
{
...

    {
        if (BCM_MAC_IS_MCAST(info->mac)) {
            dest_match = ((info->l2mc_index & l2match->addr_mask->l2mc_index) ==
                          (l2match->match_addr->l2mc_index & l2match->addr_mask->l2mc_index));
                             ...
                             bcm_l2_traverse_cb trav_fn, void *user_data)
{
    ...
    if (flags & BCM_L2_TRAVERSE_MATCH_DEST) {
        addr_mask.port = ~0;
        addr_mask.modid = ~0;
        addr_mask.l2mc_index = ~0;
    } else {
        addr_mask.port = 0x0;
        addr_mask.modid = 0x0;
        addr_mask.l2mc_index = 0x0;
    }
...
_soc_l2_sync_mem_cache(int unit, _bcm_l2_replace_t *rep_st)
{
    ...
    for (idx = 0; idx <= index_max; idx++) {
        ...
        soc_cm_debug(DK_VERBOSE, "Match found in L2 bulk cache op: %d\n", idx);
        if (rep_st->flags & BCM_L2_REPLACE_DELETE) {
        ...
        if (BCM_SUCCESS(rv) && (int_flags & _BCM_L2_REPLACE_INT_NO_ACTION)) {
            ...
            while (1) {
                if (entry_rd_idx == match_ctrl->entry_wr_idx) {
                    (void)sal_sem_take(_bcm_l2_match_ctrl[unit]->sem, 5000000);
                    continue;
                }
                                      ...
                                      void *user_data)
{
   ...
   if(BCM_VLAN_VEC_GET(l2_pair->del_by_vlan_vec, info->vid)) {
      ...
      entry_found = TRUE;
               ...
               (l2_pair->mod_pair_arr[info->modid].pbm[BCM_VLAN_MAX+1], 
                info->port)) {
      /* Delete by (mod, port) operation */
      entry_found = TRUE;
   } else if (BCM_PBMP_MEMBER
              (l2_pair->mod_pair_arr[info->modid].pbm[info->vid], info->port)) {
                                           ...
                                           bcm_gport_t *gport)
{
   ...
   mod_l2_pair_arr = sal_alloc(sizeof(_bcm_l2_vlan_port_pair_t) * max_modid,
                        "l2_addr_vlan_gport_multi_delete");
   if (mod_l2_pair_arr == NULL) {
        return BCM_E_MEMORY;
   }

   memset(&l2_pair, 0x0, sizeof(l2_pair));
   memset(mod_l2_pair_arr, 0x0, sizeof(_bcm_l2_vlan_port_pair_t) * max_modid);
   l2_pair.mod_pair_arr = mod_l2_pair_arr;

   for(ct = 0; ct < num_pairs; ct++) {
       if (vlan[ct] == BCM_VLAN_INVALID && 
           gport[ct] == BCM_VLAN_INVALID) {
             sal_free(mod_l2_pair_arr);
             return BCM_E_PARAM;
       }
       ...
       if (vlan[ct] == BCM_VLAN_INVALID) {
          BCM_PBMP_PORT_ADD(l2_pair.mod_pair_arr[modid].pbm[BCM_VLAN_MAX+1], port);
       } else if (gport[ct] == BCM_GPORT_INVALID) {
          BCM_VLAN_VEC_SET(l2_pair.del_by_vlan_vec, vlan[ct]);
       } else {
          BCM_PBMP_PORT_ADD(l2_pair.mod_pair_arr[modid].pbm[vlan[ct]], port);
       }
   }
   ...
   if (flags & BCM_L2_REPLACE_DELETE) {
       ...
       if (flags & BCM_L2_REPLACE_NO_CALLBACKS) {
           ...
           soc_cm_debug(DK_ARL, "L2 Freeze.. Stopped L2X Mode\n");
       }
       ...
       if (flags & BCM_L2_REPLACE_NO_CALLBACKS) {
           soc_cm_debug(DK_ARL, "Restarting L2X Mode and L2 Thaw\n\r");
