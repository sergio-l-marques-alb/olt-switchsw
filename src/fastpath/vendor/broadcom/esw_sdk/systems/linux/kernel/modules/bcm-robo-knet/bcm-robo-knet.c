/*
 * $Id: bcm-robo-knet.c 1.1.2.5 Broadcom SDK $
...
void *osl_pktnext(void *osh, int chan, void *skb, bool send)
{
    ...
    if (use_api) {
        next = kernel_bde->p2l(
                     minfo->dev_no,
                (sal_paddr_t)(((eth_dcb_t *)skb)->next_paddr));
    ...
    *eof = 1;
    ...
    list_for_each(list, &_minfo_list) {
        ...
        for (chan = 0; chan < NUMTXQ; chan++) {
            ...
            gmodule_pprintf(&ptr, "  tx_desc_seqno :%d\n",
                    ...
                    unsigned long val)
{
    ...
    DBG_CMD(("bkn_dma_reset chan %d flags %x \n",chan, flags));
            ...
            unsigned long val)
{
    ...
    DBG_CMD(("bkn_dma_init chan %d flags %x \n",chan, flags));
...
bkn_create_minfo(int dev_no)
{
    ...
    DBG_VERB(("base_addr %x pdev %p dev_no %d\n",
        minfo->base_addr, minfo->pdev, minfo->dev_no));
...
bkn_knet_eth_hw_config(kcom_msg_eth_hw_config_t *kmsg, int len)
{
    ...
    DBG_CMD(("bkn_knet_eth_hw_config chan %d type %d flags %x val %x\n",
    ...
    while (1) {
        ...
        if (minfo && minfo->dma_events) {
            ...
            if (minfo->dma_events & KCOM_DMA_INFO_F_RX_DONE){
                ...
                kmsg->dma_info.rx_event = minfo->rx_desc_seqno;
                minfo->rx_active_chan = 0;
            } 
            if (minfo->dma_events & KCOM_DMA_INFO_F_TX_DONE){
                kmsg->dma_info.tx_event = minfo->tx_desc_seqno;
            }
