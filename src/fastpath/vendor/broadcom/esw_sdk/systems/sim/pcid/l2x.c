...
soc_internal_l2_tr_bucket_get(pcid_info_t *pcid_info, int bank, void *entry)
{
    ...
    if (SOC_IS_TD2_TT2(unit)) {
        ...
        addr = soc_reg_addr_get(unit, reg, REG_PORT_ANY, 0, &blk, &acc_type);
        ...
        if (bank > 1) {
            ...
            addr = soc_reg_addr_get(unit, reg, REG_PORT_ANY, 0, &blk,
                                    &acc_type);
...
#endif /* BCM_TRIDENT2_SUPPORT */
    {
        ...
        if (hash_sel == -1) {
            addr = soc_reg_addr_get(unit, HASH_CONTROLr, REG_PORT_ANY, 0, &blk,
                                    &acc_type);
            soc_internal_extended_read_reg(pcid_info, blk, acc_type, addr,
                                           rval);
    ...
    debugk(DK_VERBOSE, "bank %d bucket %d\n", bank, bucket);
                             ...
                             void *entry, uint32 *result)
{
    ...
    for (bank = bank_start; bank_map != 0; bank += bank_inc) {
        ...
        for (slot = 0; slot < entries_per_bucket; slot++) {
            ...
            if (soc_mem_field32_get(unit, L2Xm, tmp, VALIDf)) {
                if (soc_mem_compare_key(unit, L2Xm, entry, tmp) == 0) {
                    debugk(DK_VERBOSE, "write sblk %d acc_type %d bank %d "
    ...
    if (free_index != -1) {
        ...
        debugk(DK_VERBOSE, "write sblk %d acc_type %d bank %d bucket %d, "
               "slot %d, index %d\n",
                             ...
                             void *entry, uint32 *result)
{
    ...
    for (bank = bank_start; bank_map != 0; bank += bank_inc) {
        ...
        for (slot = 0; slot < entries_per_bucket; slot++) {
            ...
            if (soc_mem_field32_get(unit, L2Xm, tmp, VALIDf)) {
                if (soc_mem_compare_key(unit, L2Xm, entry, tmp) == 0) {
                    debugk(DK_VERBOSE, "delete sblk %d acc_type %d bank %d "
                           "bucket %d, slot %d, index %d\n",
                           sblk, acc_type, bank, bucket, slot, index);
    ...
    debugk(DK_VERBOSE, "soc_internal_l2_tr_entry_del: Not found\n");
                              ...
                              void *entry, uint32 *result)
{
    ...
    for (bank = bank_start; bank_map != 0; bank += bank_inc) {
        ...
        for (slot = 0; slot < entries_per_bucket; slot++) {
            ...
            if (soc_mem_field32_get(unit, L2Xm, tmp, VALIDf)) {
                if (soc_mem_compare_key(unit, L2Xm, entry, tmp) == 0) {
                    debugk(DK_VERBOSE, "found sblk %d acc_type %d bank %d "
                           "bucket %d, slot %d, index %d\n",
                           sblk, acc_type, bank, bucket, slot, index);
    ...
    debugk(DK_VERBOSE, "soc_internal_l2_tr_entry_lkup: Not found\n");
