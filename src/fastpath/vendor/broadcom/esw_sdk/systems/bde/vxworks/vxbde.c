/*
 * $Id: vxbde.c 1.154.6.1 Broadcom SDK $
...
_iproc_offset(int d, uint32 addr)
{
    ...
    /* Not found. Allocate next available window */
    for (idx = 0; idx < IPROC_SUBWIN_MAX; idx++) {
        raddr = IPROC_PAXB_IMAP0_ADDR + (idx*4);
        if ((_iproc_read(d, raddr) & 0x1) == 0) {
            _iproc_write(d, raddr, (addr_page | 0x1));
            iproc_map[_d][idx] = addr_page;
            /* Bits 31..16 = BAR, 15..12 = SubPage, 11..0 = offset */
            return (idx * 0x1000) + (addr & 0xfff);
        }
    }
    ...
    return (IPROC_SUBWIN_MAX * 0x1000); /* Return Non-Existent Window */
}
...
_iproc_read(int d, uint32 addr)
{
    ...
    if ((offset = _iproc_offset(d, addr)) >= (IPROC_SUBWIN_MAX * 0x1000)) {
        return -1;
    }
       ...
       uint8 pciRevID)
{
    ...
    if (iproc) {
        vxd->bde_dev.base_address1 = (sal_vaddr_t) 
            ...
            iproc_map[_n_devices-1][0] = IPROC_CCA_PAGE;
            iproc_map[_n_devices-1][1] = IPROC_CCB_PAGE;
            iproc_map[_n_devices-1][2] = IPROC_PAXB_PAGE;
    }
