/*
 * $Id: linux-kernel-bde.c 1.237.2.33 Broadcom SDK $
...
_alloc_mpool(size_t size)
{
    ...
    if (_use_himem) {
        ...
        if ((pbase >> 16) > DMA_BIT_MASK(16)) {
            gprintk("DMA in high memory beyond 4G not supported.\n");
            return;
        }
        _dma_pbase = pbase;
        ...
        if ((pbase >> 16) > DMA_BIT_MASK(16)) {
            _dma_vbase = NULL;
            gprintk("DMA memory beyond 4G limit not supported.\n");
            return;
        }
        _dma_pbase = pbase;
