              ...
              int enable_mem_mode_check)
{
    ...
    if (soc_feature(unit, soc_feature_dump_socmem_uses_dma) &&
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            dual_mode = soc_th_mem_is_dual_mode(unit, mem, &temp_mem, &temp_pipe);
            if (dual_mode) {
                do_dma = FALSE;
            }
        }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
        if (SOC_IS_TOMAHAWK(unit) && dual_mode) {
            if (enable_mem_mode_check) {
                if (soc_th_mem_index_invalid
                        (unit, mem, k, dual_mode, temp_mem, temp_pipe)) {
                    LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit,
                        "Dump IGNORED for mem %s, index %d\n"),
                        SOC_MEM_NAME(unit, mem), k));
                    continue;
                }
            }
        }
