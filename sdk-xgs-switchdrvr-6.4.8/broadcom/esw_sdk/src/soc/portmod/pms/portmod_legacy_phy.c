...
int portmod_port_legacy_loopback_get(int unit, int port, phymod_loopback_mode_t lb, uint32_t *enable) {
    ...
    control = _portmod_phymod_to_soc_lb_control(lb, &control);
    if (rv != SOC_E_PARAM) {
        rv = soc_phyctrl_control_get(unit, port, control, enable);  
        if (rv == SOC_E_UNAVAIL) {
            /* The phy does not support requested loopback mode
              fall back to pd_lb_get  */
...
int portmod_port_legacy_phy_control_set(int unit, int port, soc_phy_control_t control, uint32_t value) {
    return soc_phyctrl_control_set(unit, port, control, value);
}

int portmod_port_legacy_phy_control_get(int unit, int port, soc_phy_control_t control, uint32_t *value) {
...
int portmod_port_legacy_advert_set(int unit, int port, const phymod_autoneg_ability_t* an_ability) {
    int rv = SOC_E_NONE;
    ...
    /* convert phymod ability to portmod ability */
    portmod_common_phy_to_port_ability((phymod_autoneg_ability_t *)an_ability, &portmod_ability);
    /* convert portmod ability to soc ability */
    _portmod_port_legacy_portmod_to_soc_ability(&portmod_ability, &port_ability);
    rv = soc_phyctrl_ability_advert_set(unit, port, &port_ability);

    return rv;
}

int portmod_port_legacy_advert_get(int unit, int port, phymod_autoneg_ability_t* an_ability) {
    int rv = SOC_E_NONE;
    soc_port_ability_t port_ability;
    portmod_port_ability_t portmod_ability;
    ...
    sal_memset(&interface_config, 0, sizeof(portmod_port_interface_config_t));
    sal_memset(&port_ability, 0, sizeof(soc_port_ability_t));
    sal_memset(&portmod_ability, 0, sizeof(portmod_port_ability_t));
    ...
    /* convert portmod ability to phymod ability */
    portmod_port_interface_config_get(unit, port, &interface_config);
    port_num_lanes = interface_config.port_num_lanes;
    line_interface = interface_config.interface;
    /* values below are not needed for soc_port_ability */
    cx4_10g        = 0;
    an_cl72        = 0;
    an_fec         = 0;
    hg_mode        = PHYMOD_INTF_MODES_HIGIG_GET(&interface_config); 

    portmod_common_port_to_phy_ability(&portmod_ability, an_ability,
...
int portmod_legacy_ext_phy_init(int unit, pbmp_t pbmp)
{
   ...
   pbmp_t legacy_pbmp, okay_pbmp;
    ...
    return (soc_phyctrl_pbm_probe_init(unit, legacy_pbmp, &okay_pbmp) >= 0) ? SOC_E_NONE: SOC_E_FAIL;
}
                                             ...
                                             portmod_phy_timesync_config_t* config)                                           
{
    ...
    rv = soc_port_phy_timesync_config_get(unit, port, &soc_config);
    sal_memcpy(config, &soc_config, sizeof(portmod_phy_timesync_config_t));
    return (rv);
}
