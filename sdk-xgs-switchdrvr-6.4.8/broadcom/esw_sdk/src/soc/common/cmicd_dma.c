 ...
 *      cmicd_dma_ctrl_init
 * Purpose:
 *      Initialize the CMICD DMA Control to a known good state for
 ...
 */
static int
cmicd_dma_chan_config(int unit, int chan, dvt_t type, uint32 flags)
{
    int             rv = SOC_E_NONE;
    ...
    if (f_interrupt) {
        (void)soc_cmicm_intr0_enable(unit, IRQ_CMCx_CNTLD_DESC_DONE(chan));
    }
...
cmicd_dma_chan_start(int unit, sdc_t *sc)
{
    ...
    if (!(sc->sc_flags & SOC_DMA_F_POLL)) {
        (void)soc_cmicm_intr0_enable(unit,
                                     IRQ_CMCx_CNTLD_DESC_DONE(sc->sc_channel));
    }
...
cmicd_dma_chan_poll(int unit,
                    ...
                    int * detected)
{
    return (soc_cmicm_dma_drv[unit].chan_poll(unit, chan, type, detected));
}
 ...
 */
static int
cmicd_dma_chan_abort(int unit, int chan)
{
    int             rv = SOC_E_NONE;
    int             cmc = SOC_PCI_CMC(unit);
    int		        to;
 ...
 */
static int
cmicd_dma_chan_chain_done(int unit, int chan, int mitigation)
{
    return (soc_cmicm_dma_drv[unit].chan_chain_done(unit, chan, mitigation));
}
 ...
 */
static int
cmicd_dma_chan_desc_done(int unit, int chan)
{
    int             rv = SOC_E_NONE;
    int             cmc = SOC_PCI_CMC(unit);
 ...
 */
static int
cmicd_dma_chan_desc_done_intr_enable(int unit, int chan)
{
    return (soc_cmicm_dma_drv[unit].chan_desc_done_intr_enable(unit, chan));
}
 ...
 */
static int
cmicd_dma_chan_reload_done(int unit, int chan, dcb_t * dcb, int *rld_done)
{
    int             rv = SOC_E_NONE;
    uint32          *rld_dcb;
 ...
 */
static int
cmicd_dma_chan_tx_purge(int unit, int chan, dv_t *dv)
{
    return (soc_cmicm_dma_drv[unit].chan_tx_purge(unit, chan, dv));
}
