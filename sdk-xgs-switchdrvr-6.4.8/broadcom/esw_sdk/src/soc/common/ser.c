...
#define SOC_SER_REG_CACHE_MAX 27
...
soc_generic_sram_mem_scan_start(int unit)
{
    ...
    if (soc_property_get(unit, spn_SRAM_SCAN_ENABLE, SAL_BOOT_SIMULATION ? 0 : 
    	                   (SOC_IS_TOMAHAWK(unit) || SOC_IS_TRIDENT2PLUS(unit)))) {
        sal_usecs_t interval = 0;
        int rate = 0;
...
_soc_ser_check_hard_fault(int unit, soc_mem_t mem, int at, int copyno,
                          int index, uint32 *cptr, int at_other, int pipe)
{
    int rv;
    uint8 hf = FALSE;
...
#endif
#if defined(BCM_TRIDENT2_SUPPORT)
        if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2(unit)) {
    ...
    if (SOC_IS_TD2_TT2(unit) || SOC_IS_TD_TT(unit)) {
        ...
        case L3_DEFIP_DATA_ONLYm:
            ...
            return _soc_ser_check_hard_fault(unit, mem, acc_type, copyno,
                                             index, entry, cache_acc_type, pipe);
        default:
            return SOC_E_UNAVAIL;
        }
    }
...
soc_ser_correction(int unit, _soc_ser_correct_info_t *si)
{
    ...
    if (si->flags & SOC_SER_SRC_MEM) { /* Handle mems */
        ...
        case SOC_MEM_FLAG_SER_WRITE_CACHE_RESTORE:
            ...
            if (cache == NULL) {
                rv = _soc_ser_recovery_hw_cache(unit, si->pipe_num, mem, copyno, index, si);
                if (rv == SOC_E_NONE || rv != SOC_E_UNAVAIL) {
            ...
            if (cache != NULL) {
                return _soc_ser_check_hard_fault(unit, mem, si->acc_type, copyno,
                                                 index,
                                                 null_entry == NULL ?
                                                 entry : null_entry, 0, si->pipe_num);
            }
            return SOC_E_NONE;
