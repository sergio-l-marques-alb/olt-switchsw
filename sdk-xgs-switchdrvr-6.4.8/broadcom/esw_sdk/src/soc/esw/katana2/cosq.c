...
#if defined(BCM_SABER2_SUPPORT)
    if(SOC_IS_SABER2(unit)) {
        min_index = SB2_MIN_SUBPORT_INDEX;
    } else
#endif
    if (SOC_IS_KATANA2(unit)) {
        min_index = KT2_MIN_SUBPORT_INDEX;
    }
...
#if defined(BCM_SABER2_SUPPORT)
    if(SOC_IS_SABER2(unit)) {
        if (port >= SB2_MIN_SUBPORT_INDEX && level == SOC_KT2_NODE_LVL_ROOT) {
            mem = LLS_S1_CONFIGm ;
        }
    } else 
#endif
    if (port >= KT2_MIN_SUBPORT_INDEX && level == SOC_KT2_NODE_LVL_ROOT) {
        mem = LLS_S1_CONFIGm ;
    }
...
#if defined(BCM_SABER2_SUPPORT)
       if(SOC_IS_SABER2(unit)) {
           if (port >= SB2_MIN_SUBPORT_INDEX){
               mem = LLS_S1_CONFIGm ;
           }
       } else
#endif
       if (port >= KT2_MIN_SUBPORT_INDEX){
           mem = LLS_S1_CONFIGm;
       }
