        ...
        if(BCM_GPORT_IS_SET(*gport) &&
            ...
            *gport = gport_out;
               ...
               soc_feature(unit, soc_feature_subtag_coe)) {
                BCM_IF_ERROR_RETURN(
                            _bcm_esw_local_modid_get(unit, &mymodid));
...
_bcm_esw_mirror_ingress_get(int unit, bcm_port_t port, int *enable)
{
    ...
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
        ...
        BCM_IF_ERROR_RETURN(
            ...
            unit, port, LPORT_PROFILE_LPORT_TAB, MIRRORf, (uint32*)enable));
...
_bcm_esw_mirror_ingress_set(int unit, bcm_port_t port, int enable)
{
    ...
    if (soc_feature(unit, soc_feature_hgproxy_subtag_coe) &&
        ...
        BCM_IF_ERROR_RETURN(
            ...
            unit, port, LPORT_PROFILE_LPORT_TAB, MIRRORf, enable));
 ...
 */
STATIC int
_bcm_tomahawk_mirror_sflow_tunnel_set(int unit, int index, int flags,
                                        void **entries)
{
    bcm_mirror_destination_t *mirror_dest;/* Destination & encapsulation. */
    /*SW tunnel encap buffers.*/
 ...
 */
STATIC int
_bcm_mirror_etag_tunnel_set(int unit, int index, int flags,
                                   void **entries)
{
    _bcm_mtp_config_p   mtp_cfg;       /* Mtp configuration.           . */
    bcm_mirror_destination_t *mirror_dest;/* Destination & encapsulation. */
                           ...
                           bcm_gport_t *trunk_arr, int flags)
{
...
#if defined(BCM_TOMAHAWK_SUPPORT)
        rv = _bcm_tomahawk_mirror_sflow_tunnel_set(unit, index,flags, entries);
        update_eme = TRUE;
#else
        rv = BCM_E_UNAVAIL;
#endif
    ...
    if (BCM_SUCCESS(rv) &&
        ...
        if(soc_feature(unit, soc_feature_port_extension)) {
            rv = _bcm_mirror_etag_tunnel_set(unit, index,
                                                    flags, entries);
            update_eme = TRUE;
        }
    }
...
#endif /* TRIDENT */
        int offset = index * BCM_SWITCH_TRUNK_MAX_PORTCNT;
...
_bcm_trident_mtp_init(int unit, int index, bcm_gport_t *trunk_arr, int flags)
{
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS_TRUE) {
        ...
        for (idx = 0; idx < COUNTOF(module_field); idx++) {  
            soc_EP_REDIRECT_EM_MTP_INDEXm_field32_set(unit, &ep_mtp_entry,
               module_field[idx],
                                     ...
                                     int *index_used)
{
    ...
    if (soc_feature(unit, soc_feature_mirror_encap_profile)) {
        ...
        flags = mdest.flags & BCM_MIRROR_DEST_REPLACE;
    }
#endif /* BCM_TRIDENT_SUPPORT */

    /* Look for existing MTP in use */
    rv = _bcm_tr2_mirror_shared_mtp_match(unit, dest_id, FALSE, &idx);
    if (BCM_SUCCESS(rv) && !(flags & BCM_MIRROR_DEST_REPLACE)) {
        MIRROR_CONFIG_SHARED_MTP_REF_COUNT(unit, idx)++;
        *index_used = idx;
        return (rv);
    }
    ...
    if (idx < BCM_MIRROR_MTP_COUNT) {
        if (((flags & BCM_MIRROR_DEST_REPLACE) == 0) || BCM_FAILURE(rv)) {
           /* Mark mtp as used. */
                                   ...
                                   int is_port, int *index_used)
{
    ...
    if (BCM_SUCCESS(rv) && ((flags & BCM_MIRROR_DEST_REPLACE) == 0)) {
        if (is_port) {
            MIRROR_CONFIG_SHARED_MTP_REF_COUNT(unit, idx)++;
        } else {
            MIRROR_CONFIG_SHARED_MTP_REF_COUNT(unit, idx) +=
...
/*
 * Function:
 *     _bcm_esw_mirror_mtp_ref_count_update
 * Purpose:
 *     update MTP reference count.
 * Parameters:
 *      unit         -  (IN) BCM device number.
...
STATIC int
_bcm_esw_mirror_mtp_ref_count_update(int unit, bcm_port_t port,
                                  bcm_gport_t mirror_dest, int mtp_index, uint32 flags)
{
...
#ifdef BCM_TRIUMPH2_SUPPORT
        if (soc_feature(unit, soc_feature_egr_mirror_true) && (flags & BCM_MIRROR_PORT_EGRESS_TRUE)) {
            /* Read mtp egress true mtp enable bitmap for source port. */
            BCM_IF_ERROR_RETURN(_bcm_port_mirror_egress_true_enable_get(unit, port, &enable));
            if (!enable) {
                MIRROR_CONFIG_SHARED_MTP_REF_COUNT(unit, mtp_index)++;
            }
        }
                                                     ...
                                                     bcm_gport_t mirror_dest)
{
    ...
    if ((-1 != port) && 
        ...
        if (SOC_IS_XGS3_SWITCH(unit)) {
            BCM_IF_ERROR_RETURN
            (_bcm_esw_mirror_mtp_ref_count_update(unit, port,
                            mirror_dest, mtp_index, BCM_MIRROR_PORT_INGRESS));
                                     ...
                                     bcm_gport_t mirror_dest)
{
    ...
    if ((-1 != port) && 
        ...
        if (SOC_IS_XGS3_SWITCH(unit)) {
            BCM_IF_ERROR_RETURN
            (_bcm_esw_mirror_mtp_ref_count_update(unit, port,
                            mirror_dest, mtp_index, BCM_MIRROR_PORT_EGRESS));
            /* Enable MTP index on mirror source port */
                                          ...
                                          bcm_gport_t mirror_dest)
{
    ...
    if ((-1 != port) && SOC_PORT_VALID(unit, port)) {
        BCM_IF_ERROR_RETURN
            (_bcm_esw_mirror_mtp_ref_count_update(unit, port,
                            mirror_dest, mtp_index, BCM_MIRROR_PORT_EGRESS_TRUE));
        /* Enable MTP index on mirror source port */
...
bcm_esw_mirror_port_dest_delete_all(int unit, bcm_port_t port, uint32 flags) 
{
    ...
    if (flags & BCM_MIRROR_PORT_INGRESS) {
        if (-1 != port) {
            ...
            if(!BCM_GPORT_IS_SET(port) && !SOC_PORT_VALID(unit, port)) {
                MIRROR_UNLOCK(unit);
                return (BCM_E_PORT);
            }
