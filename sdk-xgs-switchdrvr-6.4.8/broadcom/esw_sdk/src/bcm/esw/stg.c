...
_bcm_stg_reload(int unit)
{
    ...
    if (si->stg_mem == INVALIDm) {	/* Strata1 */
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            vlan_vpn_num += soc_mem_index_count(unit, VFIm);
        }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
        vlan_vpn_num += soc_mem_index_count(unit, vlan_mem);
        for (vid = 1; vid < vlan_vpn_num; vid++) {
            vlan_vpn = _bcm_stg_vlan_vpn_get(vid);
...
bcm_esw_stg_init(int unit)
{
    ...
    if (si->vlan_next == NULL) {
        si->vlan_next = sal_alloc(
            (BCM_VLAN_COUNT + num_vfi) * sizeof (bcm_vlan_t), "STG-vnext");
    }
    ...
    for (idx = 0; idx < (BCM_VLAN_COUNT + num_vfi); idx++) {
...
bcm_esw_stg_stp_set(int unit, bcm_stg_t stg, bcm_port_t port, int stp_state)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        if (port_type == _bcmVpGrpPortLocal) {
            ...
            if (BCM_GPORT_IS_VP_GROUP(port_in)) {
                rv = bcm_td2p_vp_group_stg_set(unit, stg,
                        port_in, BCMI_VP_GROUP_INGRESS, hw_stp_state);
            } else {
                rv = bcm_td2p_vp_group_stg_move(unit, stg,
                        port_in, hw_stp_state, BCMI_VP_GROUP_INGRESS);
            }
            ...
            if (rv == BCM_E_NONE) {
                ...
                if (BCM_GPORT_IS_VP_GROUP(port_in)) {
                    rv = bcm_td2p_vp_group_stg_set(unit, stg,
                        port_in, BCMI_VP_GROUP_EGRESS, hw_stp_state);
                } else {
                    rv = bcm_td2p_vp_group_stg_move(unit, stg,
                            port_in, hw_stp_state, BCMI_VP_GROUP_EGRESS);
                }
	...
	*stp_state = BCM_STG_STP_FORWARD;
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        if (port_type == _bcmVpGrpPortLocal) {
            ...
            soc_mem_lock(unit, STG_TABm);
            rv = bcm_td2p_vp_group_stg_get(unit, stg, port_in,
                                           BCMI_VP_GROUP_INGRESS, &hw_stp_state);
            if(rv == BCM_E_NONE) {
                /* Translate from hardware PVP_xxx state to BCM_STG_STP_xxx state. */
                BCM_IF_ERROR_RETURN
                   (_bcm_stg_pvp_translate(unit, hw_stp_state, stp_state));
            }
            soc_mem_unlock(unit, STG_TABm);
        }
    } else 
