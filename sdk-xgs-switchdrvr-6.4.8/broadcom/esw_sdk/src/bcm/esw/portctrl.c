 ...
 */
STATIC int
_bcm_esw_portctrl_enable_set1(int unit, bcm_port_t port,
                              portctrl_pport_t pport,
 ...
 */
STATIC int
_bcm_esw_portctrl_enable_set2(int unit, bcm_port_t port,
                              portctrl_pport_t pport,
                              int flags, int enable)
{
    pbmp_t mask;
 ...
 *      up or down a port.
 *
 *      Depending on the property, either the new or legacy
 *      port up/down sequence is called.
 *
 *      This is a temporary phase while the new sequence is being
 *      debugged.  Once the new sequence has been verified,
 *      the legacy routine can be removed.
 *
 * Parameters:
 *      unit    - (IN) Unit number.
 *      port    - (IN) Logical BCM Port number (non-GPORT).
                             ...
                             int flags, int enable)
{
    ...
    /*
     * NOTE:  This SOC property is TEMPORARY, to be used
     * only during the debugging phase of the new port sequence.
     * This should eventually be removed and only call only the new
     * sequence.
     */

    /* Default:  use new sequence */
    new_sequence = soc_property_get(unit, "new_up_down", 1);
    if (new_sequence) {
        rv = _bcm_esw_portctrl_enable_set2(unit, port, pport, flags, enable);
        LOG_INFO(BSL_LS_BCM_PORT,
                 (BSL_META_UP(unit, port,
                              "New Port Up/Down sequence u=%d p=%d\n"),
                  unit, port));
    } else {
        rv = _bcm_esw_portctrl_enable_set1(unit, port, pport, flags, enable);
        LOG_INFO(BSL_LS_BCM_PORT,
                 (BSL_META_UP(unit, port,
                              "Legacy Port Up/Down sequence u=%d p=%d\n"),
                  unit, port));
    }
                                       ...
                                       portmod_port_interface_config_t *if_cfg)
{
    ...
    if (if_cfg->flags & PHYMOD_INTF_F_UPDATE_INTERNAL_SERDES_ONLY) {
        phy_init_flag = PORTMOD_INIT_F_INTERNAL_SERDES_ONLY;

        /* we can clear the internal flag here - dont need to propogate down */
        if_cfg->flags ^= PHYMOD_INTF_F_UPDATE_INTERNAL_SERDES_ONLY;
    }
...
bcmi_esw_portctrl_probe(int unit, bcm_gport_t port, int init_flag, int *okay)
{
    ...
    /* check if port is disabled. If so return */
    ...
    *okay = FALSE;
    ...
    if (valid) {
        if ((!init_flag) || (init_flag == PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD)) {
    ...
    if (SOC_SUCCESS(rv)) {
        *okay = TRUE;
        ...
        if (init_flag == PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD) return (rv);
...
bcmi_esw_portctrl_probe_pbmp(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp)
{
        ...
        if (!SOC_WARM_BOOT(unit)) {
            BCM_IF_ERROR_RETURN
                (bcmi_esw_portctrl_probe(unit, port, 
                        PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD, &okay));
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
         ...
         BCM_IF_ERROR_RETURN(portmod_common_ext_phy_fw_bcst(unit));
    }
    ...
    PBMP_ITER(pbmp, port) {
        ...
        if (!SOC_WARM_BOOT(unit)) {
            BCM_IF_ERROR_RETURN
                (bcmi_esw_portctrl_probe(unit, port, 
                      PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD, &okay));
        }
        if (okay) {
    ...
    if (!IS_HG_PORT(unit, port) || IS_GX_PORT(unit, port)) {
        ...
        if (an_status.enabled) {
            ...
            if (SOC_IS_TRIDENT2PLUS(unit) &&
                (IS_GE_PORT(unit, port) || IS_XE_PORT(unit, port)
                 || IS_HG_PORT(unit, port))) 
#endif
            if (SOC_IS_TRIDENT2PLUS(unit)) {
                /* If AN is enabled, get both "line side speed" and
                   "MAC speed(multiply number of lanes", then compare them.
                   Only if MAC is enabled and speed are same, just return this function.
                   Otherwise, set LINKUP flag and continue. */
                int line_speed, mac_speed;
                int num_of_lane = SOC_INFO(unit).port_num_lanes[port];
                int enable, rv;

                PORT_LOCK(unit);
                rv = portmod_port_enable_get(unit, pport, PORTMOD_PORT_ENABLE_MAC, &enable);
                PORT_UNLOCK(unit);
                if(PORTMOD_FAILURE(rv)) {
                    return rv;
                }
                ...
                PORTMOD_IF_ERROR_RETURN(portmod_port_speed_get(
                                        unit, port, &mac_speed));
                if (enable && (line_speed == mac_speed * num_of_lane)) {
                   return BCM_E_NONE;
                }
...
bcmi_esw_portctrl_speed_set(int unit, bcm_gport_t port, int speed)
{
    ...
    if (speed == 0) {
        ...
        /* max speed 0 : port is inactive port */
        if (speed == 0) {
            LOG_VERBOSE(BSL_LS_BCM_PORT,
                        (BSL_META_UP(unit, port,
    ...
    if (sys_interface != SOC_PORT_IF_COUNT) {
        BCM_PHY_GPORT_PHYN_SYS_SIDE_PORT_SET(gport, 1, port);
    ...
    if (!soc_feature(unit, soc_feature_hg_no_speed_change)) {
        ...
        if (to_higig) {
            ...
            if (IS_GE_PORT(unit, port)) {
                ...
                force_speed = (speed_max >=120000) ? 120000 : 100000;
            } else {
                force_speed = (speed_max >=40000) ? 40000 :
                              (speed_max >=20000) ? 20000 : 10000;
            }
        }
