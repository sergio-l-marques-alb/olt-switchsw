...
{   
    ...
    if (forwarding_field_type != _BCM_MPLS_FORWARD_MULTICAST_GROUP) {
        if (action == 0x3) { /* SWAP_NHI */
            ...
            if (rv == BCM_E_NONE) {
                rv = bcm_xgs3_get_ref_count_from_nhi(unit, BCM_L3_MULTIPATH,
            ...
            bcm_stat_flex_table_info_t *table_info)
{
    ...
    table_info[*num_of_tables].table=MPLS_ENTRY_EXTDm;
 ...
 */
int
_bcm_tr3_mpls_tunnel_switch_frr_get(int unit, bcm_mpls_tunnel_switch_t *tnl_info)
{
    soc_tunnel_term_t tnl_entry;
    ...
    BCM_IF_ERROR_RETURN(soc_tunnel_term_match(unit, &tnl_key, &tnl_entry));
...
bcm_tr3_mpls_tunnel_switch_add(int unit, bcm_mpls_tunnel_switch_t *info)
{
    ...
    if (info->flags & BCM_MPLS_SWITCH_P2MP) {
         ...
         if (rv == SOC_E_NONE) {
             /* Entry exists */
             old_action = 
                   soc_MPLS_ENTRYm_field32_get(unit, &ment, MPLS__MPLS_ACTION_IF_BOSf);
             ...
             if ((soc_MPLS_ENTRYm_field32_get(unit, &ment, MPLS__P2MP_LOCAL_RECEIVERS_PRESENTf)) && 
                        (info->action == BCM_MPLS_SWITCH_ACTION_POP)) {
    ...
    if (tunnel_switch_update == 1) { /* Update existing entry */
        ...
        if (forwarding_field_type == _BCM_MPLS_FORWARD_NEXT_HOP) {
             old_nh_index = 
                   soc_MPLS_ENTRYm_field32_get(unit, &ment, MPLS__NEXT_HOP_INDEXf);
        } else if (forwarding_field_type == _BCM_MPLS_FORWARD_ECMP_GROUP) {
             old_ecmp_index = 
                   soc_MPLS_ENTRYm_field32_get(unit, &ment, MPLS__ECMP_PTRf);
    ...
    (void) bcm_tr3_mpls_entry_internal_qos_set(unit, NULL, info, &ment);

    if ((!tunnel_switch_update) && (!bud_node_trigger)) {
        rv = soc_mem_insert(unit, MPLS_ENTRYm, MEM_BLOCK_ALL, &ment);
    } else {
        rv = soc_mem_write(unit, MPLS_ENTRYm,
                           MEM_BLOCK_ALL, index, &ment);
    }
...
bcm_tr3_mpls_tunnel_switch_get(int unit, bcm_mpls_tunnel_switch_t *info)
{
    ...
    if (info->flags & BCM_MPLS_SWITCH_FRR) {
         rv = _bcm_tr3_mpls_tunnel_switch_frr_get(unit, info);
    } else {
        /* Search for MPLS entry */
        rv = _bcm_tr3_mpls_entry_find(unit, info, hw_buf, 
                               &hw_mem, &index);
...
{
    int nhidx, key_type, i, vp, port_type;
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        if (soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry,
                        MPLS__MPLS_ACTION_IF_BOSf) == 0x3) {
            nhidx = soc_MPLS_ENTRYm_field32_get(unit, &mpls_entry,
                                                    MPLS__NEXT_HOP_INDEXf);
...
int bcm_esw_tr3_mpls_not_empty;
