 ...
 *      vlan_vfi        - (IN) vlan/vfi mem entry index
...
bcm_td2p_vlan_vp_group_set(
      ...
      int vlan_vfi,       /* vlan/vfi mem entry index */
      int egress,         /* TURE for egress, FALSE for ingress */
...
)
{
    ...
    ing_vlan_vfi_membership_entry_t ing_mbrship_entry;
    egr_vlan_vfi_membership_entry_t egr_mbrship_entry;

    sal_memset(&vlan_mpls_entry, 0, sizeof(vlan_mpls_entry_t));
    sal_memset(&egr_vlan_entry, 0, sizeof(egr_vlan_entry_t));
    sal_memset(&vfi_entry, 0, sizeof(vfi_entry_t));
    sal_memset(&egr_vfi_entry, 0, sizeof(egr_vfi_entry_t));
    sal_memset(&ing_mbrship_entry, 0, sizeof(ing_vlan_vfi_membership_entry_t));
    sal_memset(&egr_mbrship_entry, 0, sizeof(egr_vlan_vfi_membership_entry_t));

    if (_BCM_VPN_VFI_IS_SET(vlan_vfi)) {
        _BCM_VPN_GET(index, _BCM_VPN_TYPE_VFI, vlan_vfi);

        if (egress) {
            vlan_vfi_mem = EGR_VFIm;
            vlan_vfi_entry = &egr_vfi_entry;
            mbrship_mem = EGR_VLAN_VFI_MEMBERSHIPm;
            mbrship_entry = &egr_mbrship_entry;
        } else {
            vlan_vfi_mem = VFIm;
            vlan_vfi_entry = &vfi_entry;
            mbrship_mem = ING_VLAN_VFI_MEMBERSHIPm;
            mbrship_entry = &ing_mbrship_entry;
        }
    } else {
        index = vlan_vfi;

        if (egress) {
            vlan_vfi_mem = EGR_VLANm;
            vlan_vfi_entry = &egr_vlan_entry;
            mbrship_mem = EGR_VLAN_VFI_MEMBERSHIPm;
            mbrship_entry = &egr_mbrship_entry;
        } else {
            vlan_vfi_mem = VLAN_MPLSm;
            vlan_vfi_entry = &vlan_mpls_entry;
            mbrship_mem = ING_VLAN_VFI_MEMBERSHIPm;
            mbrship_entry = &ing_mbrship_entry;
        }
    }
    ...
    if ((rv = soc_mem_read(unit, vlan_vfi_mem, MEM_BLOCK_ANY, index,
    ...
    /* safety check */
    field_len = soc_mem_field_length(unit, mbrship_mem,
               VP_GROUP_BITMAPf);
    if (vp_group >= 0) {
    ...
    if (vp_group >= 0) {
        soc_mem_field_get(unit, mbrship_mem, (uint32 *)mbrship_entry,
    ...
    soc_mem_field_set(unit, mbrship_mem, (uint32 *)mbrship_entry,
                    VP_GROUP_BITMAPf, vp_group_bitmap);

    rv = _bcm_td2p_vp_group_vlan_vfi_profile_entry_set(unit,
                                vlan_vfi, egress, mbrship_entry);
...
)
{
    ...
    BCM_IF_ERROR_RETURN(soc_mem_read(unit,
        vlan_vfi_mem, MEM_BLOCK_ANY, index, vlan_vfi_entry));
    soc_mem_field_get(unit, vlan_vfi_mem,
        (uint32 *)vlan_vfi_entry, MEMBERSHIP_PROFILE_PTRf, &profile_idx);
      ...
      int *port_cnt)      /* point to the first available array entry */
{
    ...
    for (vpg = 0; vpg < num_vp_groups; vpg++) {
        ...
        if (soc_feature(unit,
                  ...
                  bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
            rv = bcm_td2p_vlan_vp_group_get(unit,vlan,
                    BCMI_VP_GROUP_INGRESS, vpg, &ing_enable);
        ...
        if (soc_feature(unit,
                 ...
                 bcm_td2p_egr_vp_group_unmanaged_get(unit)) {
            rv = bcm_td2p_vlan_vp_group_get(unit,vlan,
                    BCMI_VP_GROUP_EGRESS, vpg, &egr_enable);
            BCM_IF_ERROR_RETURN(rv);
            if (egr_enable) {
                if (gport_array) {
                    BCM_GPORT_VP_GROUP_SET(gport_array[*port_cnt],vpg);
        ...
        SHR_BITDCL *vlan_vfi_bitmap)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vp_gport)) {
        ...
        BCM_IF_ERROR_RETURN(
            _bcm_esw_trunk_tid_to_vp_lag_vp(unit, BCM_GPORT_TRUNK_GET(vp_gport),
                                            &vp));
    ...
    if (BCM_GPORT_IS_TRUNK(vp_gport)) {
        ...
        for (i = 0; i < member_count; i++) {
            ...
            SHR_BITOR_RANGE(
                    vlan_bitmap_tmp, vlan_vfi_bitmap, 0, num_vlan,
                    vlan_vfi_bitmap);
        }
                            ...
                            int add, int egress, int flags)
{
    ...
    int ifilter_en = 0, vlan_vfi_bitchanged = 0, vlan_vfi_bit;
    ...
    vlan_vfi_bit = 
        SHR_BITGET(new_vlan_vfi_bitmap,
                   (!is_vfi ? index : index + (BCM_VLAN_MAX + 1)));
    if ((vlan_vfi_bit && add) || (!vlan_vfi_bit && !add)) {
        vlan_vfi_bitchanged = 0;
    } else {
        vlan_vfi_bitchanged = 1;
    }
    ...
    /* Try to use current vp group if it is using by only one gpp/vp now. */
    if ((cur_vp_grp->gpp_vp_count <= 1) &&
        (cur_vp_grp->stg_count == 0) &&
        (cur_vp_grp->untagged_count == 0)) {

        /* Current vp group can be used directly*/
        vp_grp_num = old_vp_group;

        _bcm_td2p_vp_group_update_group_for_vlan_vfi(unit, gport,
                                                     vlan_vfi,
                                                     vp_grp_num,
                                                     egress,
                                                     !add);
         ...
         */
        if ((egress) && (flags & BCM_VLAN_GPORT_ADD_UNTAGGED)
           && (vlan_vfi <= BCM_VLAN_MAX )) {
            _bcm_td2p_vp_group_ut_state_set(unit, vlan_vfi,
                        vp_grp_num, (flags & BCM_VLAN_GPORT_ADD_UNTAGGED));
        }

        /* Update vlan_vfi bitmap */
        _bcm_td2p_vp_group_set_sw_vlan_vfi(unit, vlan_vfi, old_vp_group,
                                           vp_grp_num, NULL, egress);
        /* No need to leave, No need to update vp-group index. */
        goto cleanup;
    }
    ...
    if(vp_grp_num != _BCM_INVALID_VP_GROUP) {
...
#if !defined(SOC_NO_NAMES) && defined(BROADCOM_DEBUG)
        LOG_INFO(BSL_LS_BCM_VLAN,
...
_bcm_td2p_vp_group_leave_all_stg(int unit, int vp_group, int egress)
{
    ...
    for (stg = 1; stg <= stg_max; stg++) {
        SOC_IF_ERROR_RETURN(_bcm_td2p_vp_group_stp_state_set(unit, 
               stg, vp_group, egress, _BCM_DEFAULT_STP_STATE));
    }
 ...
 */
int
bcm_td2p_vp_group_state_get(int unit, bcm_port_t gport, bcm_vlan_t vlan_vfi,
                            int egress, int *flags)
{
    ...
    if(((port_type ==  _bcmVpGrpPortGlp) &&
        (ifilter != GPP_FILTER_VP_GRP_BMAP)) ||
       ((port_type ==  _bcmVpGrpPortVp) &&
        (ifilter != VP_FILTER_VP_GRP_BMAP))) {
        return BCM_E_PARAM;
    }

    /*vlan_vfi bit status should be got here*/

    /* Now, get the ut_state associated with the port */
    if(egress && (!_BCM_VPN_VFI_IS_SET(vlan_vfi))) {
        int ut_state = 0;
        SOC_IF_ERROR_RETURN(_bcm_td2p_vp_group_ut_state_get(unit, vlan_vfi,
                                                            vp_group,
                                                            &ut_state));
        if (ut_state) {
            *flags |= BCM_VLAN_GPORT_ADD_UNTAGGED;
...
/*
 * Function:
 *      bcm_td2p_vp_group_vlan_vfi_stg_set
 * Purpose:
 *      Attach a STG group to vlan/vfi.
 * Parameters:
 *      unit - (IN) BCM device number
 ...
 */
int
bcm_td2p_vp_group_vlan_vfi_stg_set(int unit, int vlan_vfi, int egress, int stg)
{
    soc_mem_t          mem;
    uint32             index = 0;
    ...
    if (_BCM_VPN_VFI_IS_SET(vlan_vfi)) {
        ...
        index = vlan_vfi;
        if (vi->init && !SHR_BITGET(vi->bmp.w, vlan_vfi)) {
            return BCM_E_NOT_FOUND;
        }
...
/*
 * Function:
 *      bcm_td2p_vp_group_vlan_vfi_stg_get
 * Purpose:
 ...
 */
int
bcm_td2p_vp_group_vlan_vfi_stg_get(int unit, int vlan_vfi, int egress, int *stg)
{
    soc_mem_t          mem;
    uint32             index = 0;
                ...
                if (SHR_BITEQ_RANGE(vlan_vfi_bmap, vp_grp_array->vlan_vfi_bitmap,
                    ...
                    for (vlan_vfi_ctr = 0; vlan_vfi_ctr < (num_vlan + num_vfi); vlan_vfi_ctr++) {
                            ...
                            if (SHR_BITGET((vp_grp_array->vlan_vfi_bitmap), vlan_vfi_ctr)) {
                                ...
                                SOC_IF_ERROR_RETURN(
                                    bcm_td2p_vp_group_vlan_vfi_stg_get(unit,
 ...
 */
int
bcm_td2p_vp_group_stg_set(int unit, int stg, int gport, int egress, int stp_state)
{
    int vp_group = 0;
    ...
    BCM_IF_ERROR_RETURN(_bcm_td2p_vp_group_stp_state_set(
                    unit, stg, vp_group, egress, stp_state));
