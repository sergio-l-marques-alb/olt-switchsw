...
bkn_do_api_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcb_chain->dcb_cur < dcb_chain->dcb_cnt) {
        ...
        } else if ((dcb[sinfo->dcb_wsize-1] & 0x40000) == 0x40000) {
            /* Special case: customer want to process error packet. First find out 
             * whether the customer created a filter. If created, then check the mask to 
             * make sure customer really care about the error bit in dcb. If don't care, 
             * kent will not process the error packet
             */
            filter = bkn_match_rx_pkt(sinfo, dcb, pkt, chan, &cbf);
            if ((filter != NULL) &&
                ((((uint32 *)&filter->kf.mask)[sinfo->dcb_wsize-1] & 0x40000) == 0)) {
                filter = NULL;
            }
        } else {
            /* Fragment*/
