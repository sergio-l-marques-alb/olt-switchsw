                     ...
                     const phymod_core_init_config_t* init_config)
{
    ...
    /* This will get called only when fresh firmware download */
    if (ret_val != SESTO_FW_ALREADY_DOWNLOADED) {
                           ...
                           const phymod_phy_inf_config_t *config)
{
    ...
    if (core == SESTO_MERLIN_CORE) {
        ...
        if (cfg.data_rate == SESTO_SPD_10G || cfg.data_rate == SESTO_SPD_11G) {
...
int _sesto_config_dut_mode_reg(const phymod_phy_access_t *phy, const phymod_phy_inf_config_t* config)
{
    ...
    if (config->data_rate != SESTO_SPD_100G && config->data_rate != SESTO_SPD_106G) {
        ...
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
            if(config->data_rate == SESTO_SPD_10G || config->data_rate == SESTO_SPD_11G) {
           ...
           if (!is_lane_used) {
            ...
            if (((config->data_rate == SESTO_SPD_40G || config->data_rate == SESTO_SPD_20G ||
                 ...
                 ((config->data_rate == SESTO_SPD_10G || config->data_rate == SESTO_SPD_11G) && (port_idx < 2))) {
    ...
    if (aux_mode->pass_thru) {
        ...
        /*MUXX mode */   
        mode_ctrl0.fields.mode_falcon_line = SESTO_MERLIN_CORE;
    }
    if ((!aux_mode->gearbox_100g_inverse_mode)  && (config->data_rate == SESTO_SPD_100G || config->data_rate == SESTO_SPD_106G)) {
        mode_ctrl0.fields.mode_falcon_line = SESTO_FALCON_CORE;
    }
        ...
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
           ...
           if (!is_lane_used) {
            ...
            if (((config->data_rate == SESTO_SPD_40G || config->data_rate == SESTO_SPD_20G ||
                ...
                } else if(config->data_rate == SESTO_SPD_10G || config->data_rate == SESTO_SPD_11G) {
                    if (phy->access.lane_mask & 0x1) {
                        mode_ctrl1.fields.mode_dmept1_en = (mode == SESTO_AN_PROP) ? 1 : 0;
...
int _sesto_interface_set(const phymod_phy_access_t *phy, const phymod_phy_inf_config_t* config)
{
    ...
    if (no_of_ports == 1) { /* 100G/40GPT */
        if (ip == SESTO_MERLIN_CORE) {
            ...
            intf_sel_p0_p1.fields.p0_scrambling_dis = 0; 
            WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_5_ADR, intf_sel_p0_p1.data);
            /* 100G/40GPT has only one port clearing the MICRO_SYNC_6 */
            WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_6_ADR, 0x0);
        }
        ...
        if (port == 0) {
            ...
            if (ip == SESTO_MERLIN_CORE) {
                ...
                intf_sel_p2_p3.fields.p2_scrambling_dis = 0;
                WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_6_ADR, intf_sel_p2_p3.data);
            }
        }
    } else { /* 10G */
        if (port == 0) {
            ...
            if (ip == SESTO_MERLIN_CORE) {
                ...
                intf_sel_p0_p1.fields.p1_scrambling_dis = 0;
                WRITE_SESTO_PMA_PMD_REG(pa, SES_GEN_CNTRLS_MICRO_SYNC_5_ADR, intf_sel_p0_p1.data);
            }
        } else if (port == 2) {
            if (ip == SESTO_MERLIN_CORE) {
...
int _sesto_pll_config_state_get(const phymod_phy_access_t *phy, const phymod_phy_inf_config_t* config, uint16_t *pll_cfg_state)
{
    ...
    if (((fal_pll_lock != 1) || (mer_pll_lock != 1)) || (config->ref_clock != cfg.ref_clock) ||
        ...
        (config->data_rate != cfg.data_rate)) || ((no_of_ports > 1) &&
        ((cfg.data_rate == SESTO_SPD_100G || cfg.data_rate == SESTO_SPD_106G) &&
         (config->data_rate != SESTO_SPD_100G || config->data_rate != SESTO_SPD_106G)))) {
        *pll_cfg_state = 1;
    }
...
int _sesto_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    PHYMOD_MEMSET(&cfg, 0, sizeof(phymod_phy_inf_config_t));
    PHYMOD_MEMSET(&cpy_config, 0, sizeof(phymod_phy_inf_config_t));
    SESTO_IF_ERR_RETURN(
          _sesto_pll_config_state_get(phy, config, &pll_cfg_state));

    PHYMOD_MEMCPY(&cpy_config, config, sizeof(phymod_phy_inf_config_t));
    if (cpy_config.device_aux_modes == NULL) {
        cpy_config.device_aux_modes = PHYMOD_MALLOC(sizeof(SESTO_DEVICE_AUX_MODE_T), "sesto_device_aux_mode");
        clear_flag = 1;
        PHYMOD_MEMSET(cpy_config.device_aux_modes, 0, sizeof(SESTO_DEVICE_AUX_MODE_T));
    }
    aux_mode = (SESTO_DEVICE_AUX_MODE_T*) cpy_config.device_aux_modes;
    SESTO_IF_ERR_RETURN(sesto_get_chipid(&phy->access, &chip_id, &rev));
    /* CHIP ID 82792/82790/82796 merlin core on system side with TD2+ TBD*/
    if ((!aux_mode->gearbox_100g_inverse_mode)  && (chip_id != SESTO_CHIP_ID_82764)) {
        aux_mode->passthru_sys_side_core = SESTO_MERLIN_CORE;
        /* 82792/82790/82796 40G PT with BCM84793 capablity */
        if ((config->data_rate == SESTO_SPD_10G) || (config->data_rate == SESTO_SPD_40G)) {
    ...
    /* Configure DUT MODE */
    SESTO_IF_ERR_RETURN(
        _sesto_config_dut_mode_reg(phy, &cpy_config));
    
    /* Enable UDMS for non-an MODE */
    PHYMOD_DEBUG_VERBOSE(("Configuring UDMS\n"));
    SESTO_IF_ERR_RETURN(
        _sesto_udms_config(phy, &cpy_config, SESTO_AN_NONE));

    /* Configure PLL Divider*/
    if (pll_cfg_state) {
        PHYMOD_DEBUG_VERBOSE(("Configuring PLL divider\n"));
        SESTO_IF_ERR_RETURN(
            _sesto_config_pll_div(&phy->access, cpy_config));
    }

    /* Configure Interface */
    SESTO_IF_ERR_RETURN(
        _sesto_interface_set(phy, &cpy_config));

ERR:
    if (cfg.device_aux_modes != NULL) {
        PHYMOD_FREE(cfg.device_aux_modes);
    }
    if (clear_flag) {
                           ...
                           phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    switch (loopback) {
        case phymodLoopbackRemotePMD:  /* Merlin remote loopback set */
            ...
            /* WAR for Remote loopback disable causing port down PHY-2001 */
            if(!enable) {
                SESTO_IF_ERR_RETURN(
                    _sesto_set_slice_reg (pa, SESTO_SLICE_UNICAST,
                        SESTO_FALCON_CORE, SESTO_DEV_PMA_PMD,
                            0, lane));
                SESTO_IF_ERR_RETURN(wrc_falcon_furia_sesto_core_dp_s_rstb(0));
                PHYMOD_USLEEP(10);
                SESTO_IF_ERR_RETURN(wrc_falcon_furia_sesto_core_dp_s_rstb(1));
            }
        break;
        case phymodLoopbackRemotePCS:
            return PHYMOD_E_UNAVAIL;
    ...
    for (lane = 0; lane < max_lane; lane ++) {
        if (lane_mask & (1 << lane)) {
            ...
            if (ip == SESTO_FALCON_CORE) {
                ...
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_PRE, tx->pre));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_MAIN, tx->main));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_POST1, tx->post));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_POST2, tx->post2));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_POST3, tx->post3));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_write_tx_afe(pa, TX_AFE_AMP, tx->amp));
            } else {
                /* MERLIN TXFIR */
                PHYMOD_DEBUG_VERBOSE(("Merlin TXFIR set\n"));
                SESTO_IF_ERR_RETURN
                    (merlin_sesto_apply_txfir_cfg(pa, tx->pre, tx->main, tx->post, tx->post2));
            }
        }
    }
    ...
    for (lane = 0; lane < max_lane; lane ++) {
        if (lane_mask & (1 << lane)) {
            ...
            if (ip == SESTO_FALCON_CORE) {
                ...
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_PRE, &tx->pre));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_MAIN, &tx->main));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_POST1, &tx->post));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_POST2, &tx->post2));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_POST3, &tx->post3));
                SESTO_IF_ERR_RETURN
                    (falcon_furia_sesto_read_tx_afe(pa, TX_AFE_AMP, &tx->amp));
                break;
            } else {
                /* MERLIN TXFIR */
                PHYMOD_DEBUG_VERBOSE(("Merlin TXFIR get\n"));
                SESTO_IF_ERR_RETURN
                    (merlin_sesto_read_tx_afe(pa, TX_AFE_PRE, &tx->pre));
                SESTO_IF_ERR_RETURN
                    (merlin_sesto_read_tx_afe(pa, TX_AFE_MAIN, &tx->main));
                SESTO_IF_ERR_RETURN
                    (merlin_sesto_read_tx_afe(pa, TX_AFE_POST1, &tx->post));
                SESTO_IF_ERR_RETURN
                    (merlin_sesto_read_tx_afe(pa, TX_AFE_POST2, &tx->post2));
                break;
            }
        }
    }
...
int  _sesto_get_intr_reg (SESTO_INTR_TYPE_E intr_type, uint16_t *bit_pos, uint16_t *int_grp)
{
...
int _sesto_ext_intr_enable_set(const phymod_access_t *pa, uint32_t intr_type, uint32_t enable)
{
    ...
    /* Get the interrupt bit postion in the reg and interrupt group */
    _sesto_get_intr_reg(intr_type, &bit_pos, &intr_grp);
...
int _sesto_autoneg_ability_set(const phymod_phy_access_t* phy, sesto_an_ability_t *an_ability)
{
    ...
    SESTO_RESET_SLICE(pa, SESTO_DEV_PMA_PMD);
    SESTO_IF_ERR_RETURN
    (_sesto_config_hcd_link_sts (pa, config, ip));
