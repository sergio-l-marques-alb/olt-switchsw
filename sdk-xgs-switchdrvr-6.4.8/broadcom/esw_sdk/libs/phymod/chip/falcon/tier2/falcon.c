...
int _falcon_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
...
{
  PHYMOD_IF_ERR_RETURN
            (falcon_lane_soft_reset_release(&phy->access, 0));

  PHYMOD_IF_ERR_RETURN
            (_falcon_phy_firmware_lane_config_set(phy, fw_config));

  PHYMOD_IF_ERR_RETURN
            (falcon_lane_soft_reset_release(&phy->access, 1));
  return PHYMOD_E_NONE;
}
