...
#define BCM_UDC_IRQ            IPROC_UDC_IRQ
#else
...
#define IPROC_WRAP_IPROC_XGPLL_CTRL_0_VA	HW_IO_PHYS_TO_VIRT(IPROC_WRAP_IPROC_XGPLL_CTRL_0)
	...
	ndiv = __raw_readl(IPROC_WRAP_IPROC_XGPLL_CTRL_4_VA) & 0xff;
	BCM_DEBUG_DEV("ctrl4 = %x\n", __raw_readl(IPROC_WRAP_IPROC_XGPLL_CTRL_4_VA));
	BCM_DEBUG_DEV("ctrl0 = %x\n", __raw_readl(IPROC_WRAP_IPROC_XGPLL_CTRL_0_VA));
	mdiv = (__raw_readl(IPROC_WRAP_IPROC_XGPLL_CTRL_0_VA) >> IPROC_WRAP_IPROC_XGPLL_CTRL_0__CH3_MDIV_R) & 0xff; /* Ch3 MDIV */
...
#define IPROC_DDR_PLL_CTRL_REGISTER_5_VA HW_IO_PHYS_TO_VIRT(IPROC_DDR_PLL_CTRL_REGISTER_5)
	...
	ndiv = (__raw_readl(IPROC_DDR_PLL_CTRL_REGISTER_3_VA) >> 
		    IPROC_DDR_PLL_CTRL_REGISTER_3__NDIV_INT_R) & 
			((1 << IPROC_DDR_PLL_CTRL_REGISTER_3__NDIV_INT_WIDTH) - 1);
	
	/* read channel 1 mdiv */
	mdiv = (__raw_readl(IPROC_DDR_PLL_CTRL_REGISTER_5_VA) >> 
...
#define MII_READ  0x2
	...
	__raw_writel(START_SEQ, ChipcommonB_MII_Management_Control_VA);
	mdelay(10);
	...
	miidata = 1 << 12 | ndiv;
	/* 0x53721040 */
	__raw_writel(miicmd | miidata, ChipcommonB_MII_Management_Command_Data_VA);
	mdelay(10);
	...
	__raw_writel(miicmd | miidata, ChipcommonB_MII_Management_Command_Data_VA);
	mdelay(10);
	miidata = __raw_readl(ChipcommonB_MII_Management_Command_Data_VA);
	miicmd = precmd | MII_WRITE << 28 | 0x1D << 18;
	miidata |= 1 << 12;
	/* 0x5376333a  */
	__raw_writel(miicmd | miidata, ChipcommonB_MII_Management_Command_Data_VA);
...
static int __init bcm_udc_ModuleInit( void )
{
	...
	regval = __raw_readl(ChipcommonA_GPIOOutEn_VA);
	regval &= ~(0x01);
	__raw_writel(regval, ChipcommonA_GPIOOutEn_VA);
	if(__raw_readl(ChipcommonA_GPIOInput_VA) & 0x01 == 0) {
	...
	regval = __raw_readl(USB2D_IDM_IDM_RESET_CONTROL_VA);
	regval |= (1 << USB2D_IDM_IDM_RESET_CONTROL__RESET);
	__raw_writel(regval, USB2D_IDM_IDM_RESET_CONTROL_VA);

	regval = __raw_readl(USB2D_IDM_IDM_IO_CONTROL_DIRECT_VA);
	regval &= ~(1 << USB2D_IDM_IDM_IO_CONTROL_DIRECT__clk_enable);
	__raw_writel(regval, USB2D_IDM_IDM_IO_CONTROL_DIRECT_VA);
...
#ifdef CONFIG_ARCH_IPROC
    /* On the IPROC, endpoints 1-4 are bi-directional bulk endpoints. */
    UdcEpInit(udcP, 1, "ep1in-bulk",  USB_DIR_IN);
    UdcEpInit(udcP, 2, "ep2in-bulk",  USB_DIR_IN);
    UdcEpInit(udcP, 3, "ep3out-bulk", USB_DIR_OUT);
    UdcEpInit(udcP, 4, "ep4out-bulk", USB_DIR_OUT);

    /* On the IPROC, endpoints 5-8 are Interrupt-IN endpoints */
    UdcEpInit(udcP, 5, "ep5in-intr", USB_DIR_IN);
    UdcEpInit(udcP, 6, "ep6in-intr", USB_DIR_IN);
    UdcEpInit(udcP, 7, "ep7in-intr", USB_DIR_IN);
    UdcEpInit(udcP, 8, "ep8in-intr", USB_DIR_IN);
	UdcEpInit(udcP, 9, "ep9in-intr", USB_DIR_IN);
#else
