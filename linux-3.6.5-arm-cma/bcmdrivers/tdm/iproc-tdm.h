...
#define DMABUF_MAX_SIZE		64 * 1024		// Size of the full DMA buffer
#define DMABUF_SIZE			32 * 1024		// Size of TX and RX buffers
...
#define TDMDRV_MAJOR	250
#define TDMDRV_NAME		"tdm"

#define TDM_DIAL 0
#define TDM_RING_TONE 1
#define TDM_DTMF_DET 2

#define PROSLIC_ONHOOK 	0
#define PROSLIC_OFFHOOK	1

#define SCNT_NUM	127
#define PCLK	8192		// 8.1Mhz clock
...
#define TDM_RDY_CNT					(1 << 1)		/* Shorten TXREADY requesting time */
#define TDM_MODESEL_PCM				((0x00) << 2)	/* PCM Mode */
#define TDM_MODESEL_LIN				((0x01) << 2)	/* Linear Mode */
#define TDM_MODESEL_PCM16			((0x10) << 2)	/* PCM16 Mode */
#define TDM_MODESEL_LIN16			((0x11) << 2)	/* Linear 16 Mode */
#define TDM_FRAMESYNDU				(1 << 4)		/* 8 or 16 clock cycle period */
...
typedef struct iproc_tdm_info iproc_tdm_info_t;
typedef struct iproc_tdm_runtime_data iproc_tdm_runtime_data_t;
...
struct iproc_tdm_runtime_data {
	spinlock_t lock;
	iproc_tdm_info_t *tdm_info;
	unsigned int txdma_loaded;
	unsigned int rxdma_loaded;
	unsigned int dma_limit;
	unsigned int dma_period;
	dma_addr_t dma_txstart;
	dma_addr_t dma_txpos;
	dma_addr_t dma_txend;
...
// Functions
extern int slic_init(void);
extern int slic_ringStart(unsigned char channel);
extern int slic_ringStop(unsigned char channel);
extern int slic_toneSetup(unsigned char channel, unsigned char tone);
extern int slic_toneStart(unsigned char channel, unsigned char timerEnb);
extern int slic_toneStop(unsigned char channel);
extern int slic_readHookStatus(unsigned char channel);
extern int slic_PCMSetup(unsigned char channel, unsigned char pcm);
extern int slic_PCMStart(unsigned char channel);
extern int slic_PCMStop(unsigned char channel);
extern int slic_test(void);
