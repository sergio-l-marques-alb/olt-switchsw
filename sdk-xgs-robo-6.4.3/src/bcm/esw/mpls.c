    ...
    if (SOC_IS_TR_VL(unit) && soc_feature(unit, soc_feature_mpls)) {
           ...
           if ( rv == BCM_E_NONE ) {
              if (soc_feature(unit, soc_feature_mpls_lsr_ecmp)) {
                  ...
                  rv = bcm_th_mpls_tunnel_switch_add(unit, info);
#endif
...
int rv = BCM_E_UNAVAIL;
    ...
    if (SOC_IS_TR_VL(unit) && soc_feature(unit, soc_feature_mpls)) {
           ...
           if ( rv == BCM_E_NONE ) {
              if (soc_feature(unit, soc_feature_mpls_lsr_ecmp)) {
#if defined(BCM_TOMAHAWK_SUPPORT) && defined(BCM_MPLS_SUPPORT)
                  rv = bcm_th_mpls_tunnel_switch_delete(unit, info);
#endif
              } else {
...
int rv = BCM_E_UNAVAIL;
    ...
    if (SOC_IS_TR_VL(unit) && soc_feature(unit, soc_feature_mpls)) {
              ...
              if ( rv == BCM_E_NONE ) {
                  if (soc_feature(unit, soc_feature_mpls_lsr_ecmp)) {
#if defined(BCM_TOMAHAWK_SUPPORT) && defined(BCM_MPLS_SUPPORT)
                      rv = bcm_th_mpls_tunnel_switch_delete_all(unit);
#endif
                  } else {
    ...
    if (SOC_IS_TR_VL(unit) && soc_feature(unit, soc_feature_mpls)) {
              ...
              if ( rv == BCM_E_NONE ) {
                   if (soc_feature(unit, soc_feature_mpls_lsr_ecmp)) {
                       ...
                       rv = bcm_th_mpls_tunnel_switch_get(unit, info);
#endif
                   } else {
