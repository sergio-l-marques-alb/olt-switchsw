...
_bcm_mirror_sflow_dest_delete(int unit, uint32 flags, bcm_gport_t mirror_dest) 
{
    ...
    for (i = 0; i < BCM_MIRROR_MTP_COUNT; i++) {
        if (mtp_index == soc_reg_field_get(unit, SFLOW_ING_MIRROR_CONFIGr,
                                      reg_val, mtp_idxf[i])) {
            mc_enable &= ~(1 << i);
                 ...
                 bcm_gport_t *mirror_dest, int *mirror_dest_count) 
{
    ...
    *mirror_dest_count = 0;
    for (i = 0; i < BCM_MIRROR_MTP_COUNT; i++) {
        if ((mc_enable & (1 << i)) && 
            (mirror_dest_size > *mirror_dest_count)) {
            mtp_index = soc_reg_field_get(unit, SFLOW_ING_MIRROR_CONFIGr,
                                          reg_val, mtp_idxf[i]);
            *mirror_dest++ = MIRROR_CONFIG_ING_MTP_DEST(unit, mtp_index);
            *mirror_dest_count += 1;
        }
        ...
        for (i = 0; i < BCM_MIRROR_MTP_COUNT; i++) {
            if (!(mc_enable & (1 << i))) {
                ...
                soc_reg_field_set(unit, SFLOW_ING_MIRROR_CONFIGr, &reg_val, 
                    MIRROR_ENABLEf, mc_enable);
                soc_reg_field_set(unit, SFLOW_ING_MIRROR_CONFIGr, &reg_val, 
                    mtp_idxf[i], mtp_index);
                rv = WRITE_SFLOW_ING_MIRROR_CONFIGr(unit, reg_val);
                break;
            }
        }
        if (i == BCM_MIRROR_MTP_COUNT) {
            rv = BCM_E_UNAVAIL;
        }
    }
    if (BCM_FAILURE(rv)) {
