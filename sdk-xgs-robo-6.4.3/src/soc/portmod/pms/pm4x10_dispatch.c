/*
 *         
 * $Id:$
 * 
 * $Copyright: Copyright 2012 Broadcom Corporation.
 * This program is the proprietary software of Broadcom Corporation
 * and/or its licensors, and may only be used, duplicated, modified
 * or distributed pursuant to the terms and conditions of a separate,
 * written license agreement executed between you and Broadcom
 * (an "Authorized License").  Except as set forth in an Authorized
 * License, Broadcom grants no license (express or implied), right
 * to use, or waiver of any kind with respect to the Software, and
 * Broadcom expressly reserves all rights in and to the Software
 * and all intellectual property rights therein.  IF YOU HAVE
 * NO AUTHORIZED LICENSE, THEN YOU HAVE NO RIGHT TO USE THIS SOFTWARE
 * IN ANY WAY, AND SHOULD IMMEDIATELY NOTIFY BROADCOM AND DISCONTINUE
 * ALL USE OF THE SOFTWARE.  
 *  
 * Except as expressly set forth in the Authorized License,
 *  
 * 1.     This program, including its structure, sequence and organization,
 * constitutes the valuable trade secrets of Broadcom, and you shall use
 * all reasonable efforts to protect the confidentiality thereof,
 * and to use this information only in connection with your use of
 * Broadcom integrated circuit products.
 *  
 * 2.     TO THE MAXIMUM EXTENT PERMITTED BY LAW, THE SOFTWARE IS
 * PROVIDED "AS IS" AND WITH ALL FAULTS AND BROADCOM MAKES NO PROMISES,
 * REPRESENTATIONS OR WARRANTIES, EITHER EXPRESS, IMPLIED, STATUTORY,
 * OR OTHERWISE, WITH RESPECT TO THE SOFTWARE.  BROADCOM SPECIFICALLY
 * DISCLAIMS ANY AND ALL IMPLIED WARRANTIES OF TITLE, MERCHANTABILITY,
 * NONINFRINGEMENT, FITNESS FOR A PARTICULAR PURPOSE, LACK OF VIRUSES,
 * ACCURACY OR COMPLETENESS, QUIET ENJOYMENT, QUIET POSSESSION OR
 * CORRESPONDENCE TO DESCRIPTION. YOU ASSUME THE ENTIRE RISK ARISING
 * OUT OF USE OR PERFORMANCE OF THE SOFTWARE.
 * 
 * 3.     TO THE MAXIMUM EXTENT PERMITTED BY LAW, IN NO EVENT SHALL
 * BROADCOM OR ITS LICENSORS BE LIABLE FOR (i) CONSEQUENTIAL,
 * INCIDENTAL, SPECIAL, INDIRECT, OR EXEMPLARY DAMAGES WHATSOEVER
 * ARISING OUT OF OR IN ANY WAY RELATING TO YOUR USE OF OR INABILITY
 * TO USE THE SOFTWARE EVEN IF BROADCOM HAS BEEN ADVISED OF THE
 * POSSIBILITY OF SUCH DAMAGES; OR (ii) ANY AMOUNT IN EXCESS OF
 * THE AMOUNT ACTUALLY PAID FOR THE SOFTWARE ITSELF OR USD 1.00,
 * WHICHEVER IS GREATER. THESE LIMITATIONS SHALL APPLY NOTWITHSTANDING
 * ANY FAILURE OF ESSENTIAL PURPOSE OF ANY LIMITED REMEDY.$
 *         
 *     
 * DO NOT EDIT THIS FILE!
 *
 */

#include <soc/types.h>
#include <soc/error.h>
#include <soc/portmod/portmod_internal.h>
#include <soc/portmod/portmod.h>
#include <soc/portmod/portmod_dispatch.h>

#ifdef PORTMOD_PM4X10_SUPPORT

#include <soc/portmod/pm4x10.h>


__portmod__dispatch__t__ portmod_pm4x10_driver = {

    pm4x10_pm_interface_type_is_supported,
    pm4x10_pm_init,
    pm4x10_pm_destroy,
    pm4x10_port_attach,
    pm4x10_pm_bypass_set,
    pm4x10_port_detach,
    pm4x10_port_enable_set,
    pm4x10_port_enable_get,
    pm4x10_port_interface_config_set,
    pm4x10_port_interface_config_get,
    NULL, /* portmod_port_encoding_set */
    NULL, /* portmod_port_encoding_get */
    pm4x10_port_loopback_set,
    pm4x10_port_loopback_get,
    pm4x10_port_ability_local_get,
    NULL, /* portmod_port_autoneg_set */
    NULL, /* portmod_port_autoneg_get */
    NULL, /* portmod_port_autoneg_status_get */
    pm4x10_port_link_get,
    pm4x10_port_prbs_config_set,
    pm4x10_port_prbs_config_get,
    pm4x10_port_prbs_enable_set,
    pm4x10_port_prbs_enable_get,
    pm4x10_port_prbs_status_get,
    NULL, /* portmod_port_nof_lanes_set */
    NULL, /* portmod_port_nof_lanes_get */
    NULL, /* portmod_ilkn_retranstmit_config_set */
    NULL, /* portmod_ilkn_retranstmit_config_get */
    NULL, /* portmod_port_lanes_assign */
    NULL, /* portmod_port_lanes_retrieve */
    pm4x10_port_firmware_mode_set,
    pm4x10_port_firmware_mode_get,
    pm4x10_port_runt_threshold_set,
    pm4x10_port_runt_threshold_get,
    pm4x10_port_max_packet_size_set,
    pm4x10_port_max_packet_size_get,
    pm4x10_port_pad_size_set,
    pm4x10_port_pad_size_get,
    NULL, /* portmod_port_tx_mac_sa_set */
    NULL, /* portmod_port_tx_mac_sa_get */
    NULL, /* portmod_port_rx_mac_sa_set */
    NULL, /* portmod_port_rx_mac_sa_get */
    NULL, /* portmod_port_tx_average_ipg_set */
    NULL, /* portmod_port_tx_average_ipg_get */
    NULL, /* portmod_port_tx_preamble_length_set */
    NULL, /* portmod_port_tx_preamble_length_get */
    NULL, /* portmod_port_local_fault_control_set */
    NULL, /* portmod_port_local_fault_control_get */
    NULL, /* portmod_port_remote_fault_control_set */
    NULL, /* portmod_port_remote_fault_control_get */
    pm4x10_port_local_fault_status_get,
    pm4x10_port_remote_fault_status_get,
    NULL, /* portmod_port_local_fault_status_clear */
    NULL, /* portmod_port_remote_fault_status_clear */
    NULL, /* portmod_port_pause_control_set */
    NULL, /* portmod_port_pause_control_get */
    NULL, /* portmod_port_pfc_control_set */
    NULL, /* portmod_port_pfc_control_get */
    NULL, /* portmod_port_llfc_control_set */
    NULL, /* portmod_port_llfc_control_get */
    NULL, /* portmod_port_update */
    pm4x10_port_core_access_get,
    pm4x10_port_phy_lane_access_get,
    NULL, /* portmod_port_duplex_set */
    NULL, /* portmod_port_duplex_get */
    NULL, /* portmod_port_phy_reg_read */
    NULL, /* portmod_port_phy_reg_write */
    NULL, /* portmod_port_reset_set */
    NULL, /* portmod_port_reset_get */
    NULL, /* portmod_port_adv_local_set */
    NULL, /* portmod_port_adv_local_get */
    NULL, /* portmod_port_adv_remote_get */
    NULL, /* portmod_port_drv_name */
    NULL, /* portmod_port_drv_name_get */
    NULL, /* portmod_port_ability_adv_set */
    NULL, /* portmod_port_ability_adv_get */
    NULL, /* portmod_port_ability_adv_remote_get */
    NULL, /* portmod_port_frame_spacing_stretch_set */
    NULL, /* portmod_port_frame_spacing_stretch_get */
    NULL, /* portmod_port_diag_fifo_status_get */
    NULL, /* portmod_port_rx_control_set */
    NULL, /* portmod_port_pfc_config_set */
    NULL, /* portmod_port_pfc_config_get */
    NULL, /* portmod_port_eee_set */
    NULL, /* portmod_port_eee_get */
    NULL, /* portmod_port_vlan_tag_set */
    NULL, /* portmod_port_vlan_tag_get */
    pm4x10_ext_phy_attach_to_pm,
    pm4x10_ext_phy_detach_from_pm,
};

#endif /* PORTMOD_PM4X10_SUPPORT */
