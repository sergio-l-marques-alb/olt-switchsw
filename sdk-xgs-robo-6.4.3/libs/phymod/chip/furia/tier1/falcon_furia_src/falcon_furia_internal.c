 ...
 *  Revision      :  $Id: falcon_furia_internal.c 714 2014-09-18 19:54:56Z kirand $ *
 *                                                                                 *
...
#if 0
void set_lms_hoffset (char delta) {
...
err_code_t falcon_furia_log_full_pmd_state (struct falcon_furia_detailed_lane_status_st *lane_st) {
    ...
    if (!is_micro_stopped) {
        EFUN(falcon_furia_stop_rx_adaptation(1));
    }
    ...
    EFUN(falcon_furia_read_tx_afe(TX_AFE_AMP, &tmp)); lane_st->amp_ctrl = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe(TX_AFE_PRE, &tmp)); lane_st->pre_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe(TX_AFE_MAIN, &tmp)); lane_st->main_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe(TX_AFE_POST1, &tmp)); lane_st->post1_tap = (uint8_t) tmp;
    EFUN(falcon_furia_read_tx_afe(TX_AFE_POST2, &lane_st->post2_tap));
    EFUN(falcon_furia_read_tx_afe(TX_AFE_POST3, &lane_st->post3_tap));
    ...
    EFUN(falcon_furia_pmd_rdt_reg(DSC_E_RX_PI_CNT_BIN_D,&reg_data));
    lane_st->ddq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(DSC_E_RX_PI_CNT_BIN_P,&reg_data));
    lane_st->ppq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(DSC_E_RX_PI_CNT_BIN_L,&reg_data));
    lane_st->llq_hoffset = (uint8_t)dist_ccw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(DSC_E_RX_PI_CNT_BIN_PD,&reg_data));
    lane_st->dp_hoffset = (uint8_t)dist_cw(((reg_data>>8)&0xFF),(reg_data&0xFF));
    EFUN(falcon_furia_pmd_rdt_reg(DSC_E_RX_PI_CNT_BIN_LD,&reg_data));
    ...
    lane_st->prbs_chk_order = (reg_data==0)?7:
                              ...
                              (reg_data==5)?31:
                              (reg_data==6)?58:0;
    EFUN(falcon_furia_prbs_chk_lock_state(&lane_st->prbs_chk_lock));
    EFUN(falcon_furia_prbs_err_count_ll(&lane_st->prbs_chk_errcnt));

    if (!is_micro_stopped) {
        EFUN(falcon_furia_stop_rx_adaptation(0));
    }
  
    return(err_code);
}

err_code_t falcon_furia_disp_full_pmd_state (struct falcon_furia_detailed_lane_status_st *lane_st, uint8_t num_lanes) {
    uint8_t i;
    err_code_t err_code = 0;
    USR_PRINTF(("\n\n"));
    USR_PRINTF(("------------------------------------------------------------------------\n"));
    ...
    USR_PRINTF(("------------------------------------------------------------------------\n"));
    DISP_LN_VARS ("HEYE Left",heye_left,"%12d");
    DISP_LN_VARS ("HEYE Rght",heye_right,"%12d");
    DISP_LN_VARS ("VEYE Top ",veye_upper,"%12d");
    DISP_LN_VARS ("VEYE Bott",veye_lower,"%12d");
    USR_PRINTF(("------------------------------------------------------------------------\n"));
...
err_code_t _ull_mode_setup(void) {
    ...
    err_code = falcon_furia_poll_st_afe_tx_fifo_resetb_equals_1(1);  /* Check if FIFO reset is reported to be released within 1ms time interval */
    if (err_code) {
      return (err_code);
    }
...
err_code_t _display_event(uint8_t event_id,
	                uint8_t entry_len,
				    uint8_t prev_cursor,
				    uint8_t curr_cursor,
				    uint8_t post_cursor,
...
uint32_t _float12_to_uint32(uint8_t byte, uint8_t multi) {
...
err_code_t _set_rx_vga(uint8_t val) {
    ...
    ESTM(_check_uc_lane_stopped());                     /* make sure uC is stopped to avoid race conditions */
...
err_code_t _get_tx_pre(int8_t *val) {
	ESTM(*val = rd_txfir_pre_after_ovr());
 return (ERR_CODE_NONE);
}

err_code_t _get_tx_main(int8_t *val) {
	ESTM(*val = rd_txfir_main_after_ovr());
 return (ERR_CODE_NONE);
}

err_code_t _get_tx_post1(int8_t *val) {
	ESTM(*val = rd_txfir_post_after_ovr());
 return (ERR_CODE_NONE);
}

err_code_t _get_tx_post2(int8_t *val) {
...
err_code_t _falcon_furia_get_osr_mode(falcon_furia_osr_mode_st *imode) {
...
err_code_t _falcon_furia_read_lane_state(falcon_furia_lane_state_st *istate) {  
  ...
  EFUN(_falcon_furia_get_osr_mode(&osr_mode));
  ...
  state.pf_hiz = 0;
  state.pf_low = 0;
  EFUN(_get_rx_pf2(&state.pf2_ctrl)); 
...
err_code_t _falcon_furia_display_lane_state_no_newline(void) {     
  ...
  EFUN(_falcon_furia_read_lane_state(&state));
 
  lane_idx = falcon_furia_get_lane(); 
...
err_code_t _falcon_furia_display_core_state_no_newline(void) {
  ...
  EFUN(_falcon_furia_read_core_state(&state));
  /* USR_PRINTF(("%02d  ",falcon_furia_get_core())); */
  ...
  USR_PRINTF(("    %02d   ",state.afe_hardware_version));
