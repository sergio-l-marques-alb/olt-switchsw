...
#define _DNX_SW_STATE_PRINT(unit, ...) \
        do { \
            char str[DNX_SW_STATE_STRIDE_DUMP_MAX_STRING_LEN] = {0}; \
            sal_sprintf(str, __VA_ARGS__); \
            if (fix_all_the_same_flag[unit] == 1 && all_the_same_bitmap[unit]) { \
                dnx_sw_state_dump_fix_all_the_same(unit, str); \
            } \
            fix_all_the_same_flag[unit] = 0; \
...
#define DNX_SW_STATE_PRINT(unit, p, ...) \
    do { \
        DNX_SW_STATE_UPDATE_STRIDE(unit, p); \
        if (dnx_sw_state_stride_dump[unit].is_stride == FALSE) { \
            _DNX_SW_STATE_PRINT(unit, __VA_ARGS__); \
        } \
    } while (0)

#define DNX_SW_STATE_PRINT_OPAQUE(unit, p, ...) \
    do { \
        DNX_SW_STATE_UPDATE_STRIDE(unit, p); \
        if (dnx_sw_state_stride_dump[unit].is_stride == FALSE) { \
            _DNX_SW_STATE_PRINT(unit, __VA_ARGS__); \
...
#define _DNX_SW_STATE_PRINT_MONITOR(unit, ...) \
    do { \
        if (sal_strcmp(dnx_sw_state_dump_directory_get(unit),"") == 0) { \
            fix_all_the_same_flag[unit] = 1; \
            _DNX_SW_STATE_PRINT(unit, __VA_ARGS__); \
        } \
    } while (0)

#define _DNX_SW_STATE_PRINT_FILE(unit, ...) \
    do { \
        if (sal_strcmp(dnx_sw_state_dump_directory_get(unit),"") != 0) { \
            fix_all_the_same_flag[unit] = 1; \
            _DNX_SW_STATE_PRINT(unit, __VA_ARGS__); \
...
#define DNX_SW_STATE_PRINT_OPAQUE_MONITOR(unit, p, ...) \
    do { \
        if (sal_strcmp(dnx_sw_state_dump_directory_get(unit),"") == 0) \
            DNX_SW_STATE_PRINT_OPAQUE(unit, p, __VA_ARGS__); \
    } while (0)
...
int dnx_sw_state_get_nof_elements(
    int unit,
    uint32 module_id,
    void *ptr,
    uint32 size,
...
void dnx_sw_state_dump_fix_all_the_same(
    int unit,
    char *str);
...
#else
#define _DNX_SW_STATE_PRINT(unit, ...) cli_out(__VA_ARGS__)
