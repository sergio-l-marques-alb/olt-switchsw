...
#define SW_STATE_OCC_BM_CREATE(module_id, occ_bm, init_info) \
    SHR_IF_ERR_EXIT(sw_state_occ_bm_create(unit, module_id, init_info, &occ_bm));
 ...
 * structure which is assumed to be occupied.
 * \par DIRECT INPUT
 *   \param [in] _bit_map - \n
 *     bit map (starting from zero) to extract info from.
 ...
 *     which is stored on SWSTATE per bit map (See SW_STATE_OCC_BM_T): \n
 ...
 */
#define SW_STATE_OCC_BM_ACTIVE_INST_SET(_bit_map, _level_indx, _char_indx, _cur_val) \
  { \
    uint8 cache_enabled ; \
    uint32 buffer_offset ; \
    cache_enabled = _bit_map->cache_enabled; \
    ...
    { \
       ...
       */ \
      buffer_offset = _bit_map->levels_cache[_level_indx]; \
      _bit_map->levels_cache_buffer[buffer_offset + _char_indx] = _cur_val; \
    } \
    else \
    { \
      /* \
       ...
       */ \
      buffer_offset = _bit_map->levels[_level_indx]; \
      _bit_map->levels_buffer[buffer_offset + _char_indx] = _cur_val; \
    } \
  }
 ...
 *     creation of the bitmap. See \ref SW_STATE_OCC_BM_INIT_INFO
 * \par INDIRECT INPUT
 *   SWSTATE system
 * \par DIRECT OUTPUT
 ...
 *   root pointer at SW_STATE_OCC_BITMAP_ACCESS.dss_array is cleared.
 * \see
 ...
 *   of SW_STATE_OCC_BM_INIT_INFO (which is input to sw_state_occ_bm_create()).
