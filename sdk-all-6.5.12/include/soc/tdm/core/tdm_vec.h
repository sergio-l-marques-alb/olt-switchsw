 ...
 * All Rights Reserved.$
 *
 * TDM vector library for core scheduler
 */

#ifndef TDM_VECTOR_LIBRARY_H
#define TDM_VECTOR_LIBRARY_H
/**
@name: tdm_core_vec_load
@param:

Generate vector at index in TDM vector map
**/
int
tdm_core_vec_load(unsigned short **map, int bw, int port_speed, int yy, int len, int num_ext_ports)
{
	int i, j=0, n=0;
		...
		case 645: /* TH */
			switch (port_speed)
			{
				...
				case 106:
					...
					break;
				case 53:
					for (i=0; i<len && (i+31)<len; i+=38) {
						map[yy][i]    = TOKEN_50G;
						map[yy][i+8]  = TOKEN_50G;
						map[yy][i+16] = TOKEN_50G;
						map[yy][i+23] = TOKEN_50G;
						map[yy][i+31] = TOKEN_50G;
						n+=5;
						if (n==20) {break;}
					}
					break;
				case 42:
					for (i=0; i<len && (i+9)<len; i+=19) {
						map[yy][i]    = TOKEN_40G;
					...
					break;
				case 27:
					for (i=0; i<len && (i+61)<len; i+=76) {
						map[yy][i]    = TOKEN_25G;
						map[yy][i+16] = TOKEN_25G;
						map[yy][i+31] = TOKEN_25G;
						map[yy][i+46] = TOKEN_25G;
						map[yy][i+61] = TOKEN_25G;
						n+=5;
						if (n==10) {break;}
					}
					break;
...
/* ################################################################################################################## */
		case 933: /* AP */
			switch (port_speed)
			{
				...
				case 25:
				case 27:
					for (i=0; i<len && (i+92)<len; i+=116) {
						map[yy][i]    = TOKEN_25G;
						map[yy][i+23] = TOKEN_25G;
						map[yy][i+46] = TOKEN_25G;
						map[yy][i+69] = TOKEN_25G;
						map[yy][i+92] = TOKEN_25G;
						n+=5; if (n==10) {break;}
					}
					break;
				case 40:
				case 42:
					for (i=0; i<len && (i+14)<len; i+=29) {
						map[yy][i]    = TOKEN_40G;
				...
				case 50:
				case 53:
					for (i=0; i<len && (i+46)<len; i+=58) {
						map[yy][i]    = TOKEN_50G;
						map[yy][i+11] = TOKEN_50G;
						map[yy][i+23] = TOKEN_50G;
						map[yy][i+34] = TOKEN_50G;
						map[yy][i+46] = TOKEN_50G;
						n+=5; if (n==20) {break;}
					}
					break;
				case 100:
				case 106:
					for (i=0; i<len && (i+23)<len; i+=29) {
						map[yy][i]    = TOKEN_100G;
						map[yy][i+5]  = TOKEN_100G;
						map[yy][i+11] = TOKEN_100G;
						map[yy][i+17] = TOKEN_100G;
						map[yy][i+23] = TOKEN_100G;
						n+=5; if (n==40) {break;}
					}
					break;
				default:
		...
		case 510: /* AP */
			switch (port_speed)
			{
				...
				case 40:
				case 42:
					for (i=0; i<len && (i+7)<len; i+=16) {
						map[yy][i]    = TOKEN_40G;
						map[yy][i+7]  = TOKEN_40G;
						n+=2; if (n==16) {break;}
					}
					break;
				default:
