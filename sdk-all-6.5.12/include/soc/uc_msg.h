...
#define CMICX_SW_INTR_SET(unit, target) do { \
    if (target == CMICM_SW_INTR_UC0) { \
        soc_cmic_mcs_write(unit, soc_reg_addr(unit, ICFG_PCIE_SW_PROG_INTRr, REG_PORT_ANY, 0), (1<<1)); \
    } else { \
        soc_cmic_mcs_write(unit, soc_reg_addr(unit, ICFG_CMIC_RCPU_SW_PROG_INTRr, REG_PORT_ANY, 0), (1<<2)); \
    } \
} while (0)
#endif
