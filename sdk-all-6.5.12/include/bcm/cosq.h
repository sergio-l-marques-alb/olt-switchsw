...
/* Set the multiple mapping from internal priority to COS queue. */
...
/* Get the multiple mapping from internal priority to COS queue. */
...
/* Initialize a COSQ congestion mapping structure. */
extern void bcm_cosq_congestion_mapping_info_t_init(
    bcm_cosq_congestion_mapping_info_t *config);

#ifndef BCM_HIDE_DISPATCHABLE

/* Set congestion state to desired congestion state for a fabric device . */
...
/* 
 * Get congestion state to configured congestion state for a fabric
 * device.
 */
...
typedef enum bcm_cosq_control_e {
    ...
    bcmCosqControlEgressPriorityOverCast = 155, /* To allow activating SP before WFQ
                                           ...
                                           1 / 0); */
    bcmCosqControlIsHighPriority = 156, /* Flow can be high or low Priority,
                                           used for Rci Throttling. */
    bcmCosqControlIsFabric = 157,       /* Flow can be fabric(remote) or local
                                           flow, used for Rci Throttling */
    bcmCosqControlEgressPoolHighCongestionLimitBytes = 158, /* Egress pool high congestion
                                           threshold, for WRED */
    bcmCosqControlEgressPoolLowCongestionLimitBytes = 159, /* Egress pool low congestion threshold,
                                           for WRED */
...
typedef struct bcm_cosq_delay_tolerance_s {
    ...
    int queue_read_weight;              /* In extreme cases (i.e. device with
                                           many low bandwidth queues and a few
                                           high bandwidth queues), set this
                                           weight to give a priority to high
                                           bandwidth  queues while dequeuing
                                           from a group of eligible queues */
...
typedef enum bcm_cosq_stat_e {
    ...
    bcmCosqStatOBMLossyLowPriEnqueuedPackets = 72, /* OBM low priority enqueued packets
                                           count. */
    bcmCosqStatOBMLossyLowPriEnqueudBytes = 73, /* OBM low priority enqueued bytes
                                           count. */
    ...
    bcmCosqStatMaxCount = 78            /* this should be the last one. */
} bcm_cosq_stat_t;
...
typedef enum bcm_bst_stat_id_e {
    ...
    bcmBstStatIdEgrPortPoolSharedMcast = 11, /* Per Port Pool multicast BST Tracing
                                           resource */
    bcmBstStatIdMaxCount = 12           /* Must be the last. Not a usable value. */
} bcm_bst_stat_id_t;
...
/* Set the profiles on multiple buffers for tracking. */
...
/* Get the profile on multiple buffers for tracking. */
...
/* Set the resource allocation for a target/entity. */
...
/* Get the resource allocation for a target/entity. */
...
/* 
 * Setup mapping between PFC class and COSqs and/or scheduling nodes for
 * a port.
 */
...
/* Get profile information as it was set. */
...
/* Initialize a COSq SAFC class mapping structure. */
...
/* 
 * Setup mapping between SAFC class and COSqs and/or scheduling nodes for
 * a port.
 */
...
/* Configure three counters (types) for cosq stats */
extern int bcm_cosq_stat_config_set(
    int unit, 
    int stat_count, 
    bcm_cosq_stat_t *stat_array);

/* Get the configured counter types for cosq stats */
...
/* Get associated buffer_id to a specified port or a UCQ. */
...
#if defined(INCLUDE_TCB)
/* Initialize tcb config structure. */
...
#if defined(INCLUDE_TCB)
/* Initialize tcb threshold profile structure. */
...
#if defined(INCLUDE_TCB)
/* Register tcb event callback function. */
...
#if defined(INCLUDE_TCB)
/* Unregister tcb event callback function. */
...
/* get cosq ingress resource statistics value */
...
/* enable/disable cosq ingress port drop */
...
/* get enable/disable cosq ingress port drop */
...
/* set cosq ingress port drop thresholds */
...
/* get cosq ingress port drop thresholds */
...
/* set cosq ingress port drop map value */
...
/* get cosq ingress port drop map value */
...
/* 
 * This API will be used to get the highest latency values that was
 * measured for packets in the ingress between IRPP and ITPP.
 */
...
/* Initialize a CoSQ ingress drop control frames configuration structure. */
...
/* set cosq ingress port drop configurable Ether type to Ether type code. */
...
/* 
 * get cosq ingress port drop configurable Ether type from Ether type
 * code.
 */
...
/* set cosq ingress port drop default priority. */
...
/* get cosq ingress port drop default priority. */
...
/* set cosq ingress port drop flex key construct properties. */
...
/* get cosq ingress port drop flex key construct properties. */
...
/* set cosq ingress port drop flex key entry. */
...
/* get cosq ingress port drop flex key entry. */
...
/* Initialize bcm_cosq_fadt_info_t struct. */
...
/* set various TAS configurations at the gport level. */
...
/* Get various TAS configurations at the gport level. */
...
/* Get the TAS status at the gport/cosq level. */
...
/* Register a callback for handling COSQ events */
...
/* Unregister a callback for handling COSQ events */
...
/* 
 * Initialize the bcm_cosq_tas_profile_t structure. Clear the content to
 * zero.
 */
...
/* Traverse configured TAS profile */
...
/* 
 * Initialize the bcm_cosq_tas_profile_status_t structure. Clear the
 * content to zero
 */
 ...
 * buffer in a circu
 *             lar fashion and expect customer application to read
 * periodically.
 */
...
/* 
 * Customer provided array gports that are being monitored by the QCM
 * App.
 */
...
/* 
 * Customer provided array gports that are to be monitored by the QCM
 * App.
 */
