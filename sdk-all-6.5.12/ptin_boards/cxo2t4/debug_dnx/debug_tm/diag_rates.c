...
void jericho_ire_rate (uint32 unit) {
  ...
  printf("\t| Counter             |               Core 0               |               Core 1               |               Total                |\n");
  printf("\t+---------------------+------------------------------------+------------------------------------+------------------------------------+\n");
  if(show_zero_values || nif_pkt_rate_0 + nif_pkt_rate_1 > 0)		printf("\t| NIF       Rate      |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",nif_pkt_rate_0, nif_pkt_rate_1, nif_pkt_rate_0 + nif_pkt_rate_1);
  if(show_zero_values || rcy_pkt_rate_0 + rcy_pkt_rate_1 > 0)		printf("\t| RCY Core0 Rate      |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",rcy_pkt_rate_0,  rcy_pkt_rate_1, rcy_pkt_rate_0 + rcy_pkt_rate_1);
  if(show_zero_values || rcy_pkt_rate_10 + rcy_pkt_rate_11 > 0)		printf("\t| RCY Core1 Rate      |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",rcy_pkt_rate_10, rcy_pkt_rate_11, rcy_pkt_rate_10 + rcy_pkt_rate_11);
  if(show_zero_values || cpu_pkt_rate)		printf("\t| CPU       Rate      |                                    |                                    |                    %8.2f Mpps   |\n",cpu_pkt_rate);
  if(show_zero_values || oamp_pkt_rate)		printf("\t| OAMP      Rate      |                                    |                                    |                    %8.2f Mpps   |\n",oamp_pkt_rate);
  if(show_zero_values || olp_pkt_rate)		printf("\t| OLP       Rate      |                                    |                                    |                    %8.2f Mpps   |\n",olp_pkt_rate);
  printf("\t+---------------------+------------------------------------+------------------------------------+------------------------------------+\n");
...
void jericho_mmu_rate (uint32 unit) {
  ...
  uint32    counter_result[2];
  double    rate;
  double    dram_pkt_rate, dram_read_pkt_rate, ocb_total_read_rate, ocb_total_write_rate;
  ...
  dram_read_pkt_rate   	= rate_by_field_post_gtimer   	(unit,"MMU_IPT_PACKET_COUNTER"            , 0, clock_ratio);
  ...
  printf("\t+---------------------+------------------------------------+\n");
  printf("\t| DRAM Write Rate     |                   %8.2f Mpps    |\n",dram_pkt_rate);
  printf("\t| DRAM Read  Rate     |                   %8.2f Mpps    |\n",dram_read_pkt_rate);
...
void jericho_fda_rate (uint32 unit) {
  ...
  printf("\t+---------------------+------------------------------------+------------------------------------+------------------------------------+\n");
  if(show_zero_values || egq_cell_in_ipt_rate[0] + egq_cell_in_ipt_rate[1] > 0) 			printf("\t| In  IPT Rate        |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_in_ipt_rate[0], egq_cell_in_ipt_rate[1], egq_cell_in_ipt_rate[0] + egq_cell_in_ipt_rate[1]);
  if(show_zero_values || egq_cell_out_ipt_rate[0] + egq_cell_out_ipt_rate[1] > 0) 			printf("\t| Out IPT Rate        |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_out_ipt_rate[0], egq_cell_out_ipt_rate[1], egq_cell_out_ipt_rate[0] + egq_cell_out_ipt_rate[1]);
  if(show_zero_values || egq_cell_in_meshmc_rate[0] + egq_cell_in_meshmc_rate[1] > 0) 		printf("\t| In  Meshmc Rate     |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_in_meshmc_rate[0], egq_cell_in_meshmc_rate[1], egq_cell_in_meshmc_rate[0] + egq_cell_in_meshmc_rate[1]); 
  if(show_zero_values || egq_cell_out_meshmc_rate[0] + egq_cell_out_meshmc_rate[1] > 0) 	printf("\t| Out Meshmc Rate     |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_out_meshmc_rate[0], egq_cell_out_meshmc_rate[1], egq_cell_out_meshmc_rate[0] + egq_cell_out_meshmc_rate[1]);
  if(show_zero_values || egq_cell_ovf_meshmc_drop_rate[0] + egq_cell_ovf_meshmc_drop_rate[1] > 0) 			printf("\t| Meshmc ovf drop Rate|                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_ovf_meshmc_drop_rate[0], egq_cell_ovf_meshmc_drop_rate[1], egq_cell_ovf_meshmc_drop_rate[0]+egq_cell_ovf_meshmc_drop_rate[1]);
  if(show_zero_values || egq_cell_in_tdm_rate[0] + egq_cell_in_tdm_rate[1] > 0) 			printf("\t| In  TDM Rate        |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_in_tdm_rate[0], egq_cell_in_tdm_rate[1], egq_cell_in_tdm_rate[0]+egq_cell_in_tdm_rate[1]);
  if(show_zero_values || egq_cell_out_tdm_rate[0] + egq_cell_out_tdm_rate[1] > 0) 			printf("\t| Out TDM Rate        |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n",egq_cell_out_tdm_rate[0], egq_cell_out_tdm_rate[1], egq_cell_out_tdm_rate[0] + egq_cell_out_tdm_rate[1]);
  if(show_zero_values || egq_cell_out_rate_fab_fifo[0][0] + egq_cell_out_rate_fab_fifo[1][0] > 0) 			printf("\t| Out FAB FIFO0       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_fifo[0][0], egq_cell_out_rate_fab_fifo[1][0], egq_cell_out_rate_fab_fifo[0][0] + egq_cell_out_rate_fab_fifo[1][0]);
  if(show_zero_values || egq_cell_out_rate_fab_fifo[0][1] + egq_cell_out_rate_fab_fifo[1][1] > 0) 			printf("\t| Out FAB FIFO1       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_fifo[0][1], egq_cell_out_rate_fab_fifo[1][1], egq_cell_out_rate_fab_fifo[0][1] + egq_cell_out_rate_fab_fifo[1][1]);
  if(show_zero_values || egq_cell_out_rate_fab_fifo[0][2] + egq_cell_out_rate_fab_fifo[1][2] > 0) 			printf("\t| Out FAB FIFO2       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_fifo[0][2], egq_cell_out_rate_fab_fifo[1][2], egq_cell_out_rate_fab_fifo[0][2] + egq_cell_out_rate_fab_fifo[1][2]);
  if(show_zero_values || egq_cell_out_rate_fab_pipe[0][0] + egq_cell_out_rate_fab_pipe[1][0] > 0) 			printf("\t| Out FAB PIPE0       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_pipe[0][0], egq_cell_out_rate_fab_pipe[1][0], egq_cell_out_rate_fab_pipe[0][0] + egq_cell_out_rate_fab_pipe[1][0]);
  if(show_zero_values || egq_cell_out_rate_fab_pipe[0][1] + egq_cell_out_rate_fab_pipe[1][1] > 0) 			printf("\t| Out FAB PIPE1       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_pipe[0][1], egq_cell_out_rate_fab_pipe[1][1], egq_cell_out_rate_fab_pipe[0][1] + egq_cell_out_rate_fab_pipe[1][1]);
  if(show_zero_values || egq_cell_out_rate_fab_pipe[0][2] + egq_cell_out_rate_fab_pipe[1][2] > 0) 			printf("\t| Out FAB PIPE2       |                    %8.2f Mcps   |                    %8.2f Mcps   |                    %8.2f Mcps   |\n", egq_cell_out_rate_fab_pipe[0][2], egq_cell_out_rate_fab_pipe[1][2], egq_cell_out_rate_fab_pipe[0][2] + egq_cell_out_rate_fab_pipe[1][2]);
  if(show_zero_values || egq_cell_in_rate_p[0] > 0) 			printf("\t| In  FAB P1          |                                    |                                    |                    %8.2f Mcps   |\n", egq_cell_in_rate_p[0]);
  if(show_zero_values || egq_cell_in_rate_p[1] > 0) 			printf("\t| In  FAB P2          |                                    |                                    |                    %8.2f Mcps   |\n", egq_cell_in_rate_p[1]);
  if(show_zero_values || egq_cell_in_rate_p[2] > 0) 			printf("\t| In  FAB P3          |                                    |                                    |                    %8.2f Mcps   |\n", egq_cell_in_rate_p[2]);
  printf("\t+---------------------+--------------------------------------------------------------------------------------------------------------+\n");
...
void jericho_egq_rate (int unit) {
  ...
  printf("\t| Counter             |               Core 0               |               Core 1               |               Total                |\n");
  printf("\t+---------------------+------------------------------------+------------------------------------+------------------------------------+\n");
  if(show_zero_values || EHP_UC_Rate_0 + EHP_UC_Rate_1 > 0)				printf("\t| EHP UC         Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_UC_Rate_0,EHP_UC_Rate_1,EHP_UC_Rate_0+EHP_UC_Rate_1);
  if(show_zero_values || EHP_MC_H_Rate_0+EHP_MC_H_Rate_1 > 0)			printf("\t| EHP MC High    Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_MC_H_Rate_0,EHP_MC_H_Rate_1,EHP_MC_H_Rate_0+EHP_MC_H_Rate_1);
  if(show_zero_values || EHP_MC_L_Rate_0+EHP_MC_L_Rate_1 > 0)			printf("\t| EHP MC Low     Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_MC_L_Rate_0,EHP_MC_L_Rate_1,EHP_MC_L_Rate_0+EHP_MC_L_Rate_1);
  if(show_zero_values || EHP_DIS_Rate_0+EHP_DIS_Rate_1 > 0)				printf("\t| EHP Discard    Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_DIS_Rate_0,EHP_DIS_Rate_1,EHP_DIS_Rate_0+EHP_DIS_Rate_1);
  if(show_zero_values || EHP_DIS_MCH_Rate_0+EHP_DIS_MCH_Rate_1 > 0)		printf("\t| EHP MCH Discrd Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_DIS_MCH_Rate_0,EHP_DIS_MCH_Rate_1,EHP_DIS_MCH_Rate_0+EHP_DIS_MCH_Rate_1);
  if(show_zero_values || EHP_DIS_MCL_Rate_0+EHP_DIS_MCL_Rate_1 > 0)		printf("\t| EHP MCL Discrd Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",EHP_DIS_MCL_Rate_0,EHP_DIS_MCL_Rate_1,EHP_DIS_MCL_Rate_0+EHP_DIS_MCL_Rate_1);
  if(show_zero_values || PQP_UC_Rate_0+PQP_UC_Rate_1 > 0)				printf("\t| PQP UC         Rate | %8.2f Gbps      %8.2f Mpps   | %8.2f Gbps      %8.2f Mpps   |                    %8.2f Mpps   |\n", PQP_UC_Byte_Rate_0, PQP_UC_Rate_0, PQP_UC_Byte_Rate_1, PQP_UC_Rate_1,PQP_UC_Rate_0+PQP_UC_Rate_1);
  if(show_zero_values || PQP_DIS_UC_Rate_0+PQP_DIS_UC_Rate_1 > 0)		printf("\t| PQP Discard UC Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",PQP_DIS_UC_Rate_0,PQP_DIS_UC_Rate_1,PQP_DIS_UC_Rate_0+PQP_DIS_UC_Rate_1);
  if(show_zero_values || PQP_MC_Rate_1,PQP_MC_Rate_0+PQP_MC_Rate_1 > 0)	printf("\t| PQP MC         Rate | %8.2f Gbps      %8.2f Mpps   | %8.2f Gbps      %8.2f Mpps   |                    %8.2f Mpps   |\n", PQP_MC_Byte_Rate_0, PQP_MC_Rate_0, PQP_MC_Byte_Rate_1, PQP_MC_Rate_1,PQP_MC_Rate_0+PQP_MC_Rate_1);
  if(show_zero_values || PQP_DIS_MC_Rate_0+PQP_DIS_MC_Rate_1 > 0)		printf("\t| PQP Discard MC Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",PQP_DIS_MC_Rate_0,PQP_DIS_MC_Rate_1,PQP_DIS_MC_Rate_0+PQP_DIS_MC_Rate_1);
  if(show_zero_values || FQP_Rate_0+FQP_Rate_1 > 0)						printf("\t| FQP            Rate |                    %8.2f Mpps   |                    %8.2f Mpps   |                    %8.2f Mpps   |\n",FQP_Rate_0,FQP_Rate_1,FQP_Rate_0+FQP_Rate_1);
  printf("\t+---------------------+------------------------------------+------------------------------------+------------------------------------+\n");
...
void print_all_rates(int unit) {
	...
	if(show_zero_values == 0) 
		printf("Hiding Zero Values!\n");
