...
def add_header():
   ...
   file_content.append('%module(package="bcm") ' + module + "\n\n")
...
def handle_defines(entry):
    ...
    for define in defines:
        ...
        for pid in range(int(param_id)):
            ...
            if btype != None or ('BASETYPE' in param_list[pid - 1] and param_list[pid - 1]['BASETYPE'] in enum_list):
            ...
            elif "bcm_pbmp_t" in param_list[pid - 1]['BASETYPE']:
                ...
                typemap_info += "\t$" + str(pid)+ "_ltype *parg" + str(pid)+ " = &$1;\n"
                typemap_info += "\tPyObject *out" + str(pid)+ " =  SWIG_NewPointerObj(SWIG_as_voidptr(parg" + str(pid)+ "), $&" + str(pid)+ "_descriptor, 0);\n"
...
def handle_funcs(entry):
    ...
    if 'DOC' in entry:
        docstring = entry['DOC'].replace('"', '').replace("'", "")
    ...
    if 'PARAMETERS' in entry:
        for param in entry['PARAMETERS']:
            if 'PTR' in param:
                ...
                py_arg_prefix = arg_symbol.get(param['BASETYPE'], "PyInt_FromLong")
