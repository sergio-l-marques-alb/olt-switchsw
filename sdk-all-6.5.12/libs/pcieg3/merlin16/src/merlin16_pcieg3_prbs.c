/**************************************************************************************
 **************************************************************************************
 *  File Name     :  merlin16_pcieg3_prbs.c                                                *
...
err_code_t merlin16_pcieg3_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by merlin16_pcieg3_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by merlin16_pcieg3_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(merlin16_pcieg3_INTERNAL_calc_patt_gen_mode_sel(&mode_sel,&zero_pad_len,patt_length));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(merlin16_pcieg3_INTERNAL_compute_bin(pattern[i],bin));
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(merlin16_pcieg3_INTERNAL_compute_hex(bin, &hex));
...
err_code_t merlin16_pcieg3_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(merlin16_pcieg3_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t merlin16_pcieg3_display_detailed_prbs_state_hdr(void) {
  ...
  const char* merlin16_pcieg3_e2s_prbs_mode_enum[8] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t merlin16_pcieg3_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",merlin16_pcieg3_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_pcieg3_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        prbs_string = (char *)merlin16_pcieg3_e2s_prbs_mode_enum[prbs_poly_mode];
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_pcieg3_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        prbs_string = (char *)merlin16_pcieg3_e2s_prbs_mode_enum[prbs_poly_mode];
        ESTM_PRINTF((" %s",prbs_string));
        ESTM_PRINTF(("      %1d     ",prbs_inv));
    } else {
    ...
    EFUN(merlin16_pcieg3_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
    EFUN_PRINTF(("  %d  %010d ",lock_lost,err_cnt));
    ESTM_PRINTF(("    %4d  ",rd_prbs_chk_burst_err_cnt()));
    EFUN(merlin16_pcieg3_INTERNAL_display_BER(sa__,100));
    EFUN_PRINTF(("\n"));
