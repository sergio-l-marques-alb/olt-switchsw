/*
 *         
 * $Id: qtce.c,v 1.2.2.26 Broadcom SDK $
 * 
...
int qtce_core_info_get(const phymod_core_access_t* phy, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == QTCE4_MODEL) {
        ...
        PHYMOD_STRNCPY(core_name, "QTCe4", PHYMOD_STRLEN("QTCe4")+1);
    } else {
...
int qtce_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_PRE, tx->pre));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_MAIN, tx->main));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_POST1, tx->post));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_POST2, tx->post2));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_POST3, tx->post3));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_write_tx_afe(&phy_copy.access, TX_AFE_AMP,  tx->amp));
...
int qtce_phy_tx_get(const phymod_phy_access_t* phy, phymod_tx_t* tx)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_PRE, &tx->pre));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_MAIN, &tx->main));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_POST1, &tx->post));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_POST2, &tx->post2));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_POST3, &tx->post3));
    PHYMOD_IF_ERR_RETURN
        (eagle_tsc_read_tx_afe(&phy_copy.access, TX_AFE_AMP, &tx->amp));
    return PHYMOD_E_NONE;
}
...
{   
    ...
    PHYMOD_IF_ERR_RETURN(qmod_tx_squelch_get(&pm_phy_copy.access, &enable));
...
{   
    ...
    switch (rx_control) {
    case phymodRxReset:
        PHYMOD_IF_ERR_RETURN(qmod_rx_lane_control_set(&pm_phy_copy.access, 1));
        break;
...
int qtce_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (qtce_phy_tx_set(phy, &init_config->tx[start_lane]));
...
int qtce_phy_eee_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    ...
    PHYMOD_IF_ERR_RETURN(qmod_eee_control_get(&phy->access, &enable_local));
    *enable = enable_local;
    return PHYMOD_E_NONE;
}
