...
{
    PHYMOD_IF_ERR_RETURN
        (merlin16_read_tx_afe(&phy->access, TX_AFE_PRE, &tx->pre));
    PHYMOD_IF_ERR_RETURN
        (merlin16_read_tx_afe(&phy->access, TX_AFE_MAIN, &tx->main));
    PHYMOD_IF_ERR_RETURN
        (merlin16_read_tx_afe(&phy->access, TX_AFE_POST1, &tx->post));
    PHYMOD_IF_ERR_RETURN
        (merlin16_read_tx_afe(&phy->access, TX_AFE_POST2, &tx->post2));
    return PHYMOD_E_NONE;
}
