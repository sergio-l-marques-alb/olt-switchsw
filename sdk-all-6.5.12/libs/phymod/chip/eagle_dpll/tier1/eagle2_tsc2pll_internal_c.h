/***********************************************************************************
***********************************************************************************
*  File Name     :  eagle2_tsc2pll_internal.c                                         *
*  Created On    :  13/02/2014                                                    *
*  Created By    :  Justin Gaither                                                *
*  Description   :  APIs for Serdes IPs                                           *
*  Revision      :  $Id: eagle2_tsc2pll_internal.c 1490 2016-08-11 22:26:06Z cvazquez $ *
*                                                                                 *
...
/** @file eagle2_tsc2pll_internal.c
 * Implementation of API functions
 */

#include "eagle2_tsc2pll_internal.h"
...
static uint8_t _eagle2_tsc2pll_rdb_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;

	if (!err_code_p)
		return 0;

	EPSTM(rddata = eagle2_tsc2pll_rdb_uc_ram (pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Word Read */
static uint16_t _eagle2_tsc2pll_rdw_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
...
#endif
	EPSTM(rddata = eagle2_tsc2pll_rdw_uc_ram (pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro Global RAM Byte Write  */
static err_code_t _eagle2_tsc2pll_wrb_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val){

	return eagle2_tsc2pll_wrb_uc_ram (pa, addr, wr_val);                              /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
static err_code_t _eagle2_tsc2pll_wrw_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){
	if (addr % 2 != 0)                                                              /* Validate even address */
		return _error(ERR_CODE_INVALID_RAM_ADDR);
	return eagle2_tsc2pll_wrw_uc_ram (pa, addr, wr_val);                                 /* Use Micro register interface for writing RAM */
}
...
static err_code_t _get_tx_driver_mode (const phymod_access_t *pa, int8_t *val){
	ESTM(*val = rd_ams_tx_drivermode());
	return ERR_CODE_NONE;
}


static err_code_t _eagle2_tsc2pll_core_clkgate (const phymod_access_t *pa, uint8_t enable){
...
static err_code_t _eagle2_tsc2pll_lane_clkgate (const phymod_access_t *pa, uint8_t enable){
...
SDK_STATIC err_code_t _eagle2_tsc2pll_get_osr_mode (const phymod_access_t *pa, eagle2_tsc2pll_osr_mode_st *imode){
	eagle2_tsc2pll_osr_mode_st mode;

	ENULL_MEMSET(&mode, 0, sizeof(eagle2_tsc2pll_osr_mode_st));
...
SDK_STATIC err_code_t _eagle2_tsc2pll_read_lane_state (const phymod_access_t *pa, eagle2_tsc2pll_lane_state_st *istate){

	eagle2_tsc2pll_lane_state_st state;
	uint8_t ladder_range = 0;

	ENULL_MEMSET(&state, 0, sizeof(eagle2_tsc2pll_lane_state_st));

	if (!istate)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);

	EFUN(_eagle2_tsc2pll_pmd_lock_status (pa, &state.rx_lock, &state.rx_lock_chg));
	...
	ESTM(state.tx_pll_select = rd_pll_select());
	ESTM(state.rx_pll_select = rd_pll_select());
	EFUN(_eagle2_tsc2pll_sigdet_status (pa, &state.sig_det, &state.sig_det_chg));
	ESTM(state.rx_ppm = rd_cdr_integ_reg() / 84);
	EFUN(eagle2_tsc2pll_get_clk90_offset (pa, &state.clk90));
...
static err_code_t _eagle2_tsc2pll_display_lane_state_no_newline (const phymod_access_t *pa){
	uint16_t lane_idx;
	eagle2_tsc2pll_lane_state_st state;

	const char* e2s_osr_mode_enum[15] = {
		"x1       ",
		"x2       ",
		"x3       ",
		"x3.3     ",
	...
	const char* e2s_tx_osr_mode_enum[15] = {
		"x1  ",
		"x2  ",
		"x3  ",
		"x3p ",
	...
	EFUN(_eagle2_tsc2pll_read_lane_state (pa, &state));

	lane_idx = eagle2_tsc2pll_get_lane(pa);
...
SDK_STATIC err_code_t _eagle2_tsc2pll_read_core_state (const phymod_access_t *pa, eagle2_tsc2pll_core_state_st *istate){
	eagle2_tsc2pll_core_state_st state;
	struct eagle2_tsc2pll_uc_core_config_st core_cfg;

	ENULL_MEMSET(&state, 0, sizeof(eagle2_tsc2pll_core_state_st));
	ENULL_MEMSET(&core_cfg, 0, sizeof(struct eagle2_tsc2pll_uc_core_config_st));
	...
	{  int16_t die_temp = 0;
	   EFUN(eagle2_tsc2pll_read_die_temperature (pa, &die_temp));
	...
	EFUN(_eagle2_tsc2pll_pll_lock_status (pa, &state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _eagle2_tsc2pll_display_core_state_no_newline (const phymod_access_t *phy_pa){
	eagle2_tsc2pll_core_state_st state;
	eagle2_tsc2pll_core_state_st state_pll1;
	uint8_t pll_orig;
    phymod_access_t *pa;
	...
	ENULL_MEMSET(&state, 0, sizeof(state     ));
	{
		...
		EFUN(_eagle2_tsc2pll_read_core_state (pa, &state));
		EFUN(eagle2_tsc2pll_set_pll(pa,1));
		EFUN(_eagle2_tsc2pll_read_core_state (pa, &state_pll1));
		EFUN(eagle2_tsc2pll_set_pll(pa,pll_orig));
	}
	EFUN_PRINTF(("%02d  ",  eagle2_tsc2pll_get_core()));
	...
	EFUN_PRINTF((" (%02d,%02d) ",  state.pll_div,
		     state_pll1.pll_div));
	EFUN(eagle2_tsc2pll_set_pll(pa,0));
	EFUN(_eagle2_tsc2pll_display_pll_to_divider (pa,   state.pll_div));
	EFUN_PRINTF((","));
	EFUN(eagle2_tsc2pll_set_pll(pa,1));
	EFUN(_eagle2_tsc2pll_display_pll_to_divider (pa,   state_pll1.pll_div));
...
SDK_STATIC err_code_t _eagle2_tsc2pll_pll_lock_status (const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg){

	uint16_t rddata;

	EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _eagle2_tsc2pll_pmd_lock_status (const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg){

	uint16_t rddata;

	EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _eagle2_tsc2pll_sigdet_status (const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg){
	uint16_t rddata;

	EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _update_uc_lane_config_st (const phymod_access_t *pa, struct  eagle2_tsc2pll_uc_lane_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_functions_st (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st *st){
	uint8_t in = st->byte;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_st (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = st->byte;
...
static err_code_t _update_uc_core_config_st (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_core_config_st *st){
	uint16_t in = st->word;
...
static err_code_t _update_uc_core_config_word (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_core_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_uc_lane_config_word (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_lane_config_st *st){
	uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_functions_byte (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st *st){
	uint8_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_byte (const phymod_access_t *pa, struct  eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st *st){
	uint8_t in = 0;
...
static err_code_t _eagle2_tsc2pll_display_pll_to_divider (const phymod_access_t *pa, uint8_t val){
	...
	{ uint8_t mmd_en;
	  ...
	  if (mmd_en) {
		  ...
		  switch (val) {
		  case 0x0:  EFUN_PRINTF((" 46  "));
			  break;
		  case 0x1:  EFUN_PRINTF((" 72  "));
			  break;
		  case 0x2:  EFUN_PRINTF((" 40  "));
			  break;
		  case 0x3:  EFUN_PRINTF((" 42  "));
			  break;
		  case 0x4:  EFUN_PRINTF((" 20  "));
			  break;
		  case 0x5:  EFUN_PRINTF((" 50  "));
			  break;
		  case 0x6:  EFUN_PRINTF((" 52  "));
			  break;
		  case 0x7:  EFUN_PRINTF((" 32  "));
			  break;
		  case 0x8:  EFUN_PRINTF((" 60  "));
			  break;
		  case 0x9:  EFUN_PRINTF((" 64  "));
			  break;
		  case 0xa:  EFUN_PRINTF((" 66  "));
			  break;
		  case 0xb:  EFUN_PRINTF((" 68  "));
			  break;
		  case 0xc:  EFUN_PRINTF((" 70  "));
			  break;
		  case 0xd:  EFUN_PRINTF((" 80  "));
			  break;
		  case 0xe:  EFUN_PRINTF((" 92  "));
			  break;
		  case 0xf:  EFUN_PRINTF(("100  "));
			  break;
...
static err_code_t _eagle2_tsc2pll_display_ber_scan_data (const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	uint8_t range250;
	eagle2_tsc2pll_osr_mode_st osr_mode;
	struct eagle2_tsc2pll_uc_core_config_st core_config;

	ENULL_MEMSET(&core_config, 0, sizeof(core_config));
	ENULL_MEMSET(&osr_mode, 0, sizeof(osr_mode));

	EFUN(eagle2_tsc2pll_get_uc_core_config (pa, &core_config));
	EFUN(_eagle2_tsc2pll_get_osr_mode (pa, &osr_mode));
	...
	for (i = 0; i < cnt; i++) {
		...
		EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
		...
		EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
	...
	EFUN_PRINTF(("\n"));
	EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END, 0x00, 2000));
