...
err_code_t blackhawk_tsc_display_eye_scan(srds_access_t *sa__) {
    ...
    y_step = EYE_SCAN_NRZ_VERTICAL_STEP;
    {
        ...
        if (pam_mode == PAM4_NS) {
            ...
            EFUN_PRINTF(("\n!!!2-D EYE SCAN IS NOT SUPPORTED FOR PAM4 ES MODE!!!\n\n"));
...
static err_code_t blackhawk_tsc_poll_diag_data(srds_access_t *sa__, const blackhawk_tsc_info_t *blackhawk_tsc_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    const uint32_t lane_diag_size = blackhawk_tsc_info_ptr->diag_mem_ram_size;
    uint16_t loop;
    ...
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : blackhawk_tsc_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
err_code_t blackhawk_tsc_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    EFUN(blackhawk_tsc_INTERNAL_verify_blackhawk_tsc_info(blackhawk_tsc_info_ptr));
        ...
        lane_diag_base = blackhawk_tsc_info_ptr->diag_mem_ram_base + ((lane%blackhawk_tsc_info_ptr->lane_count) * blackhawk_tsc_info_ptr->diag_mem_ram_size) + 
            blackhawk_tsc_info_ptr->grp_ram_size*blackhawk_tsc_INTERNAL_grp_idx_from_lane(blackhawk_tsc_get_lane(sa__));

    EFUN(blackhawk_tsc_poll_diag_data(sa__, blackhawk_tsc_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
        
    arg.buffer_ptr = buffer;
    ESTM(blackhawk_tsc_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
...
#endif
  EFUN_PRINTF(("FFE taps  : Desc. TODO\n")); 
  EFUN_PRINTF(("DCO       : DC offset DAC control value\n"));
  EFUN_PRINTF(("P1mV      : Vertical threshold voltage of p1 slicer\n"));
  EFUN_PRINTF(("TP        : Tuning Parameter 0 - [indicates 'Channel_Loss_hint' in PAM4 mode ; 'M1 vertical threshold voltage (mV)' in NRZ mode]\n"));
  EFUN_PRINTF(("          : Tuning Parameter 1 - Loss estimate during initial startup tuning (min 0 - max 15)\n"));
...
err_code_t blackhawk_tsc_display_core_state_hdr(void) {
  ...
  EFUN_PRINTF(("CORE RST_ST  PLL_PWDN  UC_ATV   COM_CLK   UCODE_VER  AFE_VER   LIVE_TEMP   AVG_TMON   RESCAL     VCO_RATE     ANA_VCO_RANGE  PLL0_DIV PLL1_DIV PLL_LOCK"));
...
err_code_t blackhawk_tsc_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(blackhawk_tsc_version(&api_version));
    ...
    if(diag_level & SRDS_DIAG_LANE) {
        ...
        EFUN(blackhawk_tsc_disp_full_pmd_state(&lane_st, 1));
    } else {
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(blackhawk_tsc_reg_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(blackhawk_tsc_uc_core_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(blackhawk_tsc_uc_lane_var_dump(sa__));
    }
...
err_code_t blackhawk_tsc_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    blackhawk_tsc_info_t * const blackhawk_tsc_info_ptr = blackhawk_tsc_INTERNAL_get_blackhawk_tsc_info_ptr();
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(blackhawk_tsc_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + blackhawk_tsc_info_ptr->lane_var_ram_base + (blackhawk_tsc_get_lane(sa__) * blackhawk_tsc_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
