...
#ifdef EMULATION_EN
#include "blackhawk_tsc_access.h"
extern struct core_var_struct core;
extern struct lane_var_struct ln;
#endif
...
uint16_t blackhawk_tsc_INTERNAL_get_lane_addr(uint16_t addr, uint8_t lane) {
...
uint8_t blackhawk_tsc_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    ...
    if (is_diag_emu_ready(sa__->diag_em_sts)) {
        /* local var access in API emulation mode */
        uint8_t ln_var_base,ret_val;
        ln_var_base = (uint8_t)&ln;
        ret_val = ((uint8_t)(*(uint8_t*)((uint8_t*)(&ln)+addr)));
        ...
        lane_addr_offset = blackhawk_tsc_INTERNAL_get_lane_addr(addr,lane);
...
uint16_t blackhawk_tsc_INTERNAL_get_uc_address(uint16_t addr, uint8_t lane) {
...
uint8_t blackhawk_tsc_rdbuc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  ...
  lane = blackhawk_tsc_get_lane(sa__);
  uc_addr_offset = blackhawk_tsc_INTERNAL_get_uc_address(addr,lane);
...
uint16_t blackhawk_tsc_INTERNAL_get_core_address(uint16_t addr, uint8_t lane) {
...
#endif
        lane = blackhawk_tsc_get_lane(sa__);
        core_addr_offset = blackhawk_tsc_INTERNAL_get_core_address(addr,lane);
