...
{        
#if 0
    /* NEED TO REVISIT: need to get txfix config from serdes team */
    int rv = PHYMOD_E_NONE;

    rv = falcon2_monterey_validate_txfir_cfg(&phy->access, tx->pre, tx->main, tx->post, tx->post2, tx->post3);
    if (rv != PHYMOD_E_NONE) {
        PHYMOD_DEBUG_ERROR(("TXFIR Validate error: pre=%d main=%d post1=%d post2=%d post3=%d \n", tx->pre, tx->main, tx->post, tx->post2, tx->post3));
        PHYMOD_IF_ERR_RETURN(rv);
    }
#endif
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_PRE, tx->pre));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_MAIN, tx->main));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_POST1, tx->post));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_POST2, tx->post2));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_POST3, tx->post3));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_AMP,  tx->amp));
    if((tx->drivermode != -1) &&
       (phy->device_op_mode & PHYMOD_INTF_CONFIG_TX_FIR_DRIVERMODE_ENABLE)) {
        PHYMOD_IF_ERR_RETURN
            (falcon2_monterey_write_tx_afe(&phy->access, TX_AFE_DRIVERMODE,  tx->drivermode));
    }
...
{        
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_PRE, &tx->pre));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_MAIN, &tx->main));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_POST1, &tx->post));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_POST2, &tx->post2));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_POST3, &tx->post3));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_AMP, &tx->amp));
    PHYMOD_IF_ERR_RETURN
        (falcon2_monterey_read_tx_afe(&phy->access, TX_AFE_DRIVERMODE, &tx->drivermode));
        ...
        if ((new_pll_div == phymod_TSCF_PLL_DIV132) || (new_pll_div == phymod_TSCF_PLL_DIV165)){
            if (new_pll_div != cur_pll1_div) {
                if (!(flags & PHYMOD_INTF_F_DONT_TURN_OFF_PLL)){
                    PHYMOD_IF_ERR_RETURN
                        (falcon2_monterey_pll_idx_set(&pm_phy_copy.access, 1));
                    PHYMOD_IF_ERR_RETURN
            ...
            PHYMOD_IF_ERR_RETURN
                (falcon2_monterey_pll_select_set(&pm_phy_copy.access, 1));
        } else {
            PHYMOD_DEBUG_ERROR(("Pll %d One PLL mode doesn't support \n", new_pll_div));
            return PHYMOD_E_CONFIG;
        }
    } else { /* HIGIG MODE */
        if (PHYMOD_INTF_MODES_HIGIG_GET(config) && !hg_mode_ieee_speed){
            if ((new_pll_div == phymod_TSCF_PLL_DIV140) || (new_pll_div == phymod_TSCF_PLL_DIV175)) {
                if (new_pll_div != cur_pll0_div){
                    if (!(flags & PHYMOD_INTF_F_DONT_TURN_OFF_PLL)){
                        PHYMOD_IF_ERR_RETURN
                            (falcon2_monterey_pll_idx_set(&pm_phy_copy.access, 0));
                        PHYMOD_IF_ERR_RETURN
                            (tefmod_gen3_configure_pll(&pm_phy_copy.access, new_pll_div, config->ref_clock));
                    } else {
                        PHYMOD_DEBUG_ERROR(("DONT TURN OFF PLL flag is set\n" ));
                ...
                PHYMOD_IF_ERR_RETURN
                    (falcon2_monterey_pll_select_set(&pm_phy_copy.access, 0));
            } else {
                PHYMOD_DEBUG_ERROR(("Pll_div %d doesn't support HIGIG mode\n", new_pll_div));
                return PHYMOD_E_CONFIG; 
            }
        } else {  /* IEEE MODE */
            if ((new_pll_div == phymod_TSCF_PLL_DIV132) || (new_pll_div == phymod_TSCF_PLL_DIV165)){
                ...
                PHYMOD_DEBUG_ERROR(("Pll_div %d doesn't support IEEE mode\n", new_pll_div));
                return PHYMOD_E_CONFIG;
            }
        }
    }
...
int _tscf_gen3_speed_id_interface_config_get(const phymod_phy_access_t* phy, int speed_id, phymod_phy_inf_config_t* config)
{
    ...
    if(ilkn_set) {
        ...
        case 0x45:
            ...
            break;
        case 0x46:
            if (pll_div == phymod_TSCF_PLL_DIV175) {
                config->data_rate = 106000;
            } else if (pll_div == phymod_TSCF_PLL_DIV165) {
                config->data_rate = 100000;
            } else {
                /*
                 * before SDK 6.5.6, pll_div = 6 when the
...
{        
    ...
    PHYMOD_IF_ERR_RETURN
        (phymod_phy_autoneg_get(phy, &an_control, (uint32_t *) &an_done));
...
int _tscf_gen3_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /* Set the heart beat, default is for 156.25M */
    if (init_config->interface.ref_clock == phymodRefClk125Mhz) {
        PHYMOD_IF_ERR_RETURN
            (tefmod_gen3_refclk_set(&core_copy.access, phymodRefClk125Mhz));
    }

    /* Configure lane mapping */
    PHYMOD_IF_ERR_RETURN
        (tscf_gen3_core_lane_map_set(&core_copy, &init_config->lane_map));
