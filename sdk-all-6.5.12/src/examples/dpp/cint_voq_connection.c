 ...
 */
int create_voq(int unit,  int remote_base_modid, int port, int numq, uint32 additional_flags, int voq_id, bcm_gport_t* voq_gport)
{
    int core_id;
    int rv;
    int nof_cores;
    ...
    flags = BCM_COSQ_GPORT_UCAST_QUEUE_GROUP | BCM_COSQ_GPORT_WITH_ID | additional_flags; 
 ...
 *      id of voq connector to connect to
 */
int create_and_connect_voq(int unit, int remote_base_modid, int port, int numq, int voq_id, int voq_connector_id)
{
    int rv;
    uint32                    dummy_flags;
    bcm_port_interface_info_t interface_info;
    ...
    rv = create_voq(unit, remote_base_modid, port, numq, 0 /* additional_flags*/, voq_id, &voq_gport);
    if (rv != BCM_E_NONE) {
 ...
 *      id of voq connector to be created
 */
int create_and_connect_local_voq_and_connector_example(int unit, int port, int numq, int voq_id, int voq_connector_id) 
{

    int rv = BCM_E_NONE;
    int my_modid, nof_cores;
    ...
    nof_remote_connections = nof_cores;

    rv = create_and_connect_voq(unit, my_modid, port, numq, voq_id, voq_connector_id);
    if (rv != BCM_E_NONE) {
