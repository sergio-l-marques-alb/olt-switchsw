 ...
 * - set ip tunnel type
 */
void ipv6_tunnel_type_set(int unit, int type_1, int type_2){

    ip_tunnel_encap_ipv6_glbl_info.type_1 = type_1;
    ip_tunnel_encap_ipv6_glbl_info.type_2 = type_2;
}
...
int ipv6_tunnel_example(int unit, int in_port, int out_port){
    ...
    ipv6_tunnel_type_set(unit, bcmTunnelTypeIpAnyIn6, bcmTunnelTypeIpAnyIn6);
	
    /* build IP tunnels, and get back interfaces */
    rv = ipv6_tunnel_build_tunnels(unit, out_port, g_ipv6_tunnel.intf_ids, tunnel_gport_ids, &ll_encap_id);
    if (rv != BCM_E_NONE) {
