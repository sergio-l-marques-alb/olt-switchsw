    ...
    if (soc_feature(unit, soc_feature_riot) ||
        ...
        if (l3_ecmp_level > BCMI_L3_ECMP_SINGLE_LEVEL) {
            ...
            if (SOC_IS_TRIDENT3X(unit)) {
                ...
                num_banks = l3_ecmp_group_first_lkup_mem_size / BCMI_L3_ECMP_GROUP_PER_BANK +
                                         ((l3_ecmp_group_first_lkup_mem_size % BCMI_L3_ECMP_GROUP_PER_BANK)?1:0);

                _td2_vp_lag_info[unit]->base_ecmp_idx = num_banks * BCMI_L3_ECMP_GROUP_PER_BANK;
            }
    ...
    for (i = 0; i < vp_count; i++) {
        ...
        if (i == 0 && (_bcm_vp_used_get(unit, vp_array[i], _bcmVpTypeVxlan) ||
            ...
            soc_ING_DVP_TABLEm_field32_set(unit, &lag_ing_dvp_entry,
                                           VP_TYPEf, vp_type);
            SOC_IF_ERROR_RETURN(WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL,
                                lag_vp, &lag_ing_dvp_entry));
            SOC_IF_ERROR_RETURN(READ_ING_DVP_2_TABLEm(unit, MEM_BLOCK_ANY,
                                lag_vp, &lag_ing_dvp_2_entry));
            soc_ING_DVP_2_TABLEm_field32_set(unit, &lag_ing_dvp_2_entry,
                                             VP_TYPEf, vp_type);
            SOC_IF_ERROR_RETURN(WRITE_ING_DVP_2_TABLEm(unit, MEM_BLOCK_ALL,
                                lag_vp, &lag_ing_dvp_2_entry));
        }
        ...
        bcm_trunk_member_t *member_array)
{
    ...
    for (i = 0, j = 0; i < member_count; i++) {
        ...
        if (BCM_GPORT_IS_VLAN_PORT(gport)) {
            ...
            rv = _bcm_esw_l2gre_port_source_vp_lag_set(
