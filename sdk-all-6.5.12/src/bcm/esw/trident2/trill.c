 ...
 */
int 
bcm_td2_trill_l2_multicast_entry_add(int unit, uint32 flags, int key, 
                                    ...
                                    uint8 trill_tree_id, bcm_multicast_t group)
{
    ...
    if (_BCM_MULTICAST_IS_SET(group)) {
        ...
        case TR_L2_HASH_KEY_TYPE_BRIDGE:
            field_key = TD2_L2_HASH_KEY_TYPE_BRIDGE;      
        ...
        case TR_L2_HASH_KEY_TYPE_TRILL_NONUC_ACCESS:
                field_key = TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_ACCESS;
                ...
                soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry,
                                    L2__DEST_TYPEf, 0x3);
                soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry,
                                    L2__L3MC_PTRf, mc_index);
                soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry,
                                    VALIDf, 1);
            break;

        case TR_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_SHORT:
                field_key = TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_SHORT;
                if (flags & BCM_TRILL_MULTICAST_STATIC) { 
                    soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry, 
                                  TRILL_NONUC_NETWORK_SHORT__STATIC_BITf, 0x1);
                } else {
                    soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry, 
                                  TRILL_NONUC_NETWORK_SHORT__STATIC_BITf, 0x0);
                }           
            soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry, 
                                 KEY_TYPEf, field_key);
        ...
        case TR_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_LONG:
                field_key = TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_LONG;
                if (flags & BCM_TRILL_MULTICAST_STATIC) { 
                    soc_mem_field32_set(unit, mem, (uint32 *)&l2_entry, 
 ...
 */
int 
bcm_td2_trill_l2_multicast_entry_update(int unit, int key, 
                                    bcm_vlan_t vid, bcm_mac_t mac, 
                                    uint8 trill_tree_id, bcm_multicast_t group)
{
    L2_ENTRY_T  l2_entry, l2_lookup;
 ...
 */
int 
bcm_td2_trill_l2_multicast_entry_delete(int unit, int key, bcm_vlan_t vid, 
 ...
 */
int
bcm_td2_trill_l2_multicast_entry_delete_all (int unit, uint8 tree_id)
{
    ...
    for (chunk = soc_mem_index_min(unit, mem); chunk <= mem_idx_max;
        ...
        for (ix = 0; ix <= chnk_end; ix++) {
            ...
            key_type = soc_mem_field32_get(unit, mem, l2ep, KEY_TYPEf);
                if (key_type != TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_ACCESS) {
                    if (key_type != TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_SHORT) {
                        if (key_type != TD2_L2_HASH_KEY_TYPE_TRILL_NONUC_NETWORK_LONG) {
                            continue;
                        }
                    }
                }
 ...
 */
int 
bcm_td2_trill_l2_multicast_entry_get(int unit, int key, bcm_vlan_t vid, 
 ...
 */
int
bcm_td2_trill_l2_network_multicast_entry_traverse (int unit, _bcm_td_trill_multicast_entry_traverse_t *trav_st)
{
 ...
 */
int
bcm_td2_trill_l2_access_multicast_entry_traverse (int unit, _bcm_td_trill_multicast_entry_traverse_t *trav_st)
{
   /* Indexes to iterate over memories, chunks and entries */
    int        chnk_idx, ent_idx, chnk_idx_max, mem_idx_max;
    int        buf_size, chunksize, chnk_end;
 ...
 */
int
bcm_td2_trill_vlan_multicast_entry_modify (int unit, bcm_vlan_t vlan, int set_bit)
{
   /* Indexes to iterate over memories, chunks and entries */
    int        chnk_idx, ent_idx, chnk_idx_max, mem_idx_max;
    int        buf_size, chunksize, chnk_end;
