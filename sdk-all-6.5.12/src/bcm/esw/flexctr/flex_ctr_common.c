...
#define _CTR_COUNTER_TABLE_IFP(_unit, _dir, _pipe_num, _pool, _mem) {        \
    if (!(soc_feature(unit,soc_feature_stat_multi_pipe_support)) ||    \
            _bcm_esw_get_fp_mode_global(_unit, bcmFieldQualifyStageIngress) ||   \
...
#define _CTR_COUNTER_TABLE_VFP(_unit, _dir, _pipe_num, _pool, _mem) {        \
    if (!(soc_feature(unit,soc_feature_stat_multi_pipe_support)) ||      \
            _bcm_esw_get_fp_mode_global(_unit, bcmFieldQualifyStageLookup) ||    \
            (bcmStatFlexDirectionIngress != _dir) ||                         \
...
#define _CTR_COUNTER_TABLE_EM(_unit, _dir, _pipe_num, _pool, _mem) {         \
    if (!(soc_feature(unit,soc_feature_stat_multi_pipe_support)) ||    \
            _bcm_esw_get_fp_mode_global(_unit,         \
...
static soc_reg_t _pkt_selector_key_reg[BCM_STAT_FLEX_COUNTER_MAX_DIRECTION]
                                  ...
                                  [BCM_STAT_AP_FLEX_COUNTER_MAX_PKT_ATTR_SEL] = {
#else
#ifndef BCM_TOMAHAWK_SUPPORT
                                  [BCM_STAT_FLEX_COUNTER_MAX_POOL] = {
...
#endif
#if (defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
static soc_mem_t _efp_mem_arr[] = {
                                EFP_POLICY_TABLEm,
...
#if defined(BCM_TRIDENT3_SUPPORT)
static bcm_stat_flex_custom_egress_mode_t *flex_custom_egress_modes[SOC_MAX_NUM_DEVICES] = {NULL};
#endif
...
static flex_object_config_t ing_flex_object_config[]={
    ...
    {VLAN_XLATE_1_DOUBLEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {VLAN_XLATE_2_DOUBLEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {LPORT_TABm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {L3_ENTRY_QUADm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {L3_ENTRY_DOUBLEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {VRF_ATTRS_2m, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG}
};
...
static flex_object_config_t egr_flex_object_config[]={
    ...
    {EGR_DVP_ATTRIBUTEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {EGR_VLAN_XLATE_1_DOUBLEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {EGR_VLAN_XLATE_2_DOUBLEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG},
    {EGR_LPORT_PROFILEm, INVALID_CONFIG, INVALID_CONFIG, INVALID_CONFIG}
};
...
#define BCM_STAT_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_8
...
static flex_counter_fields_t egr_l3_next_hop_flex_counter_fields[]={
                             ...
                             /* L2_OTAG */
                             {L2_OTAG__FLEX_CTR_OFFSET_MODEf,
                              L2_OTAG__FLEX_CTR_POOL_NUMBERf,
                              L2_OTAG__FLEX_CTR_BASE_COUNTER_IDXf}};
...
static uint32 _bcm_stat_num_rsvd = 0;
static uint32 _bcm_stat_ing_pools_rsvd = 0;
static uint32 _bcm_stat_mode_rsvd = -1;
static bcm_error_t
_bcm_esw_stat_flex_flowtracker_cleanup(int unit);
#endif
...
extern compaction_status_t compaction_info;
/* Cache used for counter movement */
static uint64 *flex_byte_counter_compaction
               [SOC_MAX_NUM_DEVICES]
               [BCM_STAT_FLEX_COUNTER_MAX_DIRECTION]
               [SOC_MAX_NUM_PIPES]
               [BCM_STAT_FLEX_COUNTER_MAX_POOL]
               = {{{{NULL}}}};
static uint32 *flex_packet_counter_compaction
               [SOC_MAX_NUM_DEVICES]
               [BCM_STAT_FLEX_COUNTER_MAX_DIRECTION]
               [SOC_MAX_NUM_PIPES]
               [BCM_STAT_FLEX_COUNTER_MAX_POOL]
                = {{{{NULL}}}};
static uint64 *flex_packet64_counter_compaction
               [SOC_MAX_NUM_DEVICES]
               [BCM_STAT_FLEX_COUNTER_MAX_DIRECTION]
               [SOC_MAX_NUM_PIPES]
               [BCM_STAT_FLEX_COUNTER_MAX_POOL]
...
_bcm_esw_get_fp_mode_global(int unit, bcm_field_qualify_t stage) {
    ...
    if ((stage != bcmFieldQualifyStageIngress) &&
         ...
         * Please note soc_feature_field_multi_stage is used to
         * pass latency mode info ONLY!!! .
         */
        return 1;
    }
...
_bcm_esw_get_max_pool_groups(int unit) {
    ...
    return ((SOC_IS_TOMAHAWKX(unit) ||SOC_IS_TRIDENT3X(unit)) ?
                                       BCM_STAT_TH_FLEX_COUNTER_MAX_GROUPS:
...
_bcm_esw_get_fp_pipe_num(int unit, soc_mem_t ingress_table)
{
    ...
    switch (ingress_table) {
       ...
       case EXACT_MATCH_4_PIPE0m:
#if defined(BCM_TRIDENT3_SUPPORT)
       case IFP_POLICY_TABLE_WIDE_PIPE0m:
       case EFP_POLICY_TABLE_PIPE0m:
#endif
            pipe_num = 0;
         break;
       ...
       case EXACT_MATCH_4_PIPE1m:
#if defined(BCM_TRIDENT3_SUPPORT)
       case IFP_POLICY_TABLE_WIDE_PIPE1m:
       case EFP_POLICY_TABLE_PIPE1m:
#endif
            pipe_num = 1;
         break;
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if ((offset_mode >= BCM_CUSTOM_EGRESS_MODE_START) &&
            (offset_mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
        offset_mode -= BCM_CUSTOM_EGRESS_MODE_START;
        if (flex_custom_egress_modes[unit][offset_mode].hint.type
                                == bcmStatGroupAllocHintEgressFieldGroup) {
            if (!_bcm_esw_get_fp_mode_global(unit, stage)) {
                hint_val =
                    flex_custom_egress_modes[unit][offset_mode].hint.value;
                rv = _bcm_field_group_flex_ctr_info_get(unit, hint_val,
                                                pipe, &pool_bmp);
                *pool = flex_custom_egress_modes[unit][offset_mode].AllocPool;
            }
        }
    }
#endif
...
/*
 * Return 1 if the ingress_table is one of the
 * ingress policy tables
 */
static int
_bcm_esw_is_ifp_table(soc_mem_t ingress_table) {
    switch(ingress_table) {
        case IFP_POLICY_TABLEm:
        case IFP_POLICY_TABLE_PIPE0m:
        case IFP_POLICY_TABLE_PIPE1m:
...
#if defined(BCM_TRIDENT3_SUPPORT)
static int _bcm_esw_stat_table_is_profile(int unit, soc_mem_t table) {
    switch(table) {
        case LPORT_TABm:
...
STATIC
bcm_error_t  _bcm_esw_get_flex_counter_view_fields_values(
    int       unit,
    soc_mem_t view,
    void      *data,
...

{
    ...
    if ((rv =_bcm_esw_stat_validate_object(unit, object, &direction))
            != BCM_E_NONE) {
         LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                (BSL_META_U(unit,
                            "Flex view invalid stat object %d")
                        ,object));
         return rv;
    ...
    if (direction == bcmStatFlexDirectionIngress) {
        /* ingress */
        if (SOC_MEM_FIELD_VALID(unit, view, FLEX_CTR_ACTION_SETf)) {
            action_set = soc_mem_field32_get(unit, view, data,
                         FLEX_CTR_ACTION_SETf);
            *base_idx = soc_format_field32_get(unit, FLEX_CTR_ACTION_SETfmt,
               &action_set, FLEX_CTR_BASE_COUNTER_IDXf);
            *pool_number = soc_format_field32_get(unit, FLEX_CTR_ACTION_SETfmt,
               &action_set, FLEX_CTR_POOL_NUMBERf);
            *offset_mode =  soc_format_field32_get(unit, FLEX_CTR_ACTION_SETfmt,
               &action_set, FLEX_CTR_OFFSET_MODEf);
            rv = BCM_E_NONE;
        } else {
            rv = BCM_E_INTERNAL;
        }
        ...
        /* Egress */
        for (i = 0; i < 2; i++) {
            if (SOC_MEM_FIELD_VALID(unit, view, egr_field[i])) {
                action_set = soc_mem_field32_get(unit, view, data,
                             egr_field[i]);
                *base_idx = soc_format_field32_get(unit,
                   EGR_FLEX_CTR_ACTION_SETfmt,
                   &action_set, FLEX_CTR_BASE_COUNTER_IDXf);
                *pool_number = soc_format_field32_get(unit,
                   EGR_FLEX_CTR_ACTION_SETfmt,
                   &action_set, FLEX_CTR_POOL_NUMBERf);
                *offset_mode =  soc_format_field32_get(unit,
                   EGR_FLEX_CTR_ACTION_SETfmt,
                   &action_set, FLEX_CTR_OFFSET_MODEf);
                rv = BCM_E_NONE;
                break;
...
STATIC bcm_error_t
_bcm_esw_set_flex_counter_view_fields_values(
    int       unit,
    soc_mem_t view,
    void      *data,
    bcm_stat_object_t object,
    ...
    uint32    base_idx)
{
    ...
    if (direction == bcmStatFlexDirectionIngress) {
        /* ingress */
        if (SOC_MEM_FIELD_VALID(unit, view, FLEX_CTR_ACTION_SETf)) {
            soc_format_field32_set(unit, FLEX_CTR_ACTION_SETfmt,
                                 &action_set,
                                 FLEX_CTR_BASE_COUNTER_IDXf,
                                 base_idx);
            soc_format_field32_set(unit, FLEX_CTR_ACTION_SETfmt,
            ...
            soc_format_field32_set(unit, FLEX_CTR_ACTION_SETfmt,
                                 ...
                                 FLEX_CTR_OFFSET_MODEf,
                                 offset_mode);
            soc_mem_field32_set(unit, view, data,
                        FLEX_CTR_ACTION_SETf, action_set);
            rv = BCM_E_NONE;
        } else {
            rv = BCM_E_INTERNAL;
        }
...
#ifdef BCM_WARM_BOOT_SUPPORT
static int _bcm_esw_is_egr_obj(bcm_stat_object_t object) {
    switch (object) {
    ...
    soc_field_t *base_idx_field)
{
    ...
    *base_idx_field=FLEX_CTR_BASE_COUNTER_IDXf;
    ...
    case EGR_L3_NEXT_HOPm:
...
#if defined (BCM_TRIDENT3_SUPPORT)
         else if (soc_mem_field_valid(unit,table,DATA_TYPEf)) {
         ...
         key_type=soc_mem_field32_get(unit,table,data,view_field);
#if defined(BCM_TRIDENT3_SUPPORT)
         if (key_type == _BCM_VXLAN_EGR_NEXT_HOP_L2OTAG_VIEW) {
             key_type_index = 7;
         } else
#endif
    ...
    case EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm:
        LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                (BSL_META_U(unit,
    ...
    case MPLS_ENTRY_SINGLEm:
        *offset_mode_field = VXLAN_FLEX_IPV4_DIP__FLEX_CTR_OFFSET_MODEf;
        *pool_number_field = VXLAN_FLEX_IPV4_DIP__FLEX_CTR_POOL_NUMBERf;
        *base_idx_field = VXLAN_FLEX_IPV4_DIP__FLEX_CTR_BASE_COUNTER_IDXf;
        break;
#endif
    default:
         break;
    ...
    if ((soc_mem_field_valid(unit,table,*offset_mode_field) == FALSE) ||
        ...
        LOG_WARN(BSL_LS_BCM_FLEXCTR,
                 (BSL_META_U(unit,
                             "INTERNAL Error i.e. "
                              "required offset,pool,base_idx fields are not valid in %s \n"
    ...
    uint32    *base_idx)
{
    ...
    *base_idx =  0;
    ...
    if ((soc_feature(unit,soc_feature_flex_flow)) &&
        (soc_flow_db_mem_view_is_valid(unit, table))) {

        LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
                                "Flex view stat counters\n")));
        return _bcm_esw_get_flex_counter_view_fields_values(unit,
                           table, data, object,
...
#endif /* BCM_TRIDENT3_SUPPORT */
    {
        ...
        if (soc_feature(unit, soc_feature_alpm2) &&
            table == L3_DEFIP_ALPM_RAWm) {
            _alpm_bkt_node_t *bkt_node;
            bkt_node = (_alpm_bkt_node_t *)alpm_bkt_node_ptr_arr_get(index);
            if (bkt_node != NULL) {
                *base_idx = bkt_node->adata.defip_flex_ctr_base_id;
                *offset_mode = bkt_node->adata.defip_flex_ctr_mode;
                *pool_number = bkt_node->adata.defip_flex_ctr_pool;
                return BCM_E_NONE;
            } else {
                return BCM_E_FAIL;
            }
        ...
        BCM_IF_ERROR_RETURN(_bcm_esw_get_flex_counter_fields(
                        unit, index, object, table, data,
                        &fields[0], &fields[1], &fields[2]));
    }
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit) &&
        ((table == LPORT_TABm) || (table == EGR_LPORT_PROFILEm) ||
            (table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm))) {
        uint32 values[3];
        if (table == LPORT_TABm) {
            BCM_IF_ERROR_RETURN(
                bcm_esw_port_lport_fields_get(unit, index, LPORT_PROFILE_LPORT_TAB,
                                              3, fields, values));
        } else if (table == EGR_LPORT_PROFILEm) {
            BCM_IF_ERROR_RETURN(
                bcm_esw_port_egr_lport_fields_get(unit, index,
                EGR_LPORT_PROFILEm, 3, fields, values));
        }
#if (defined(INCLUDE_L3) && defined(BCM_MPLS_SUPPORT))
        else if (table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm) {
            int offset = 0;
            uint32 iptun_mpls_entry_format[3] = {0};

            offset = index & _BCM_MPLS_EGR_L3_INTF_MPLS_INDEX_OFFSET_MASK(unit);

            BCM_IF_ERROR_RETURN(bcmi_egress_tnl_label_entry_get(unit, data,
                offset, iptun_mpls_entry_format));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_get(unit,
                data, offset, iptun_mpls_entry_format,
                bcmiFlexCtrBaseCounterIdx, &values[2]));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_get(unit,
                data, offset, iptun_mpls_entry_format, bcmiFlexCtrPoolNumber,
                &values[1]));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_get(unit,
                data, offset, iptun_mpls_entry_format, bcmiFlexCtrOffsetMode,
                &values[0]));
        }
#endif
        *offset_mode = values[0];
        *pool_number = values[1];
        *base_idx    = values[2];
    } else
#endif
    {
        *offset_mode =  soc_mem_field32_get(unit, table, data, fields[0]);
        *pool_number =  soc_mem_field32_get(unit, table, data, fields[1]);
        *base_idx =  soc_mem_field32_get(unit, table, data, fields[2]);
    }
    return BCM_E_NONE;
}
/*
    ...
    uint32    base_idx)
{
    ...
    if ((soc_feature(unit,soc_feature_flex_flow)) &&
       (soc_flow_db_mem_view_is_valid(unit, table))) {
        LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                   (BSL_META_U(unit,
                               "Flex view stat counters\n")));
        return _bcm_esw_set_flex_counter_view_fields_values(unit,
                           table, data, object,
                           offset_mode, pool_number, base_idx);
    } else
#endif
    {
        ...
        if (soc_feature(unit, soc_feature_alpm2) &&
            ...
            if (bkt_node != NULL) {
                ...
                bkt_node->adata.defip_flex_ctr_mode = offset_mode;
                bkt_node->adata.defip_flex_ctr_pool = pool_number;
                return BCM_E_NONE;
            } else {
                return BCM_E_FAIL;
            }
    ...
    if (SOC_IS_TRIDENT3X(unit) &&
        ...
        (table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm))) {
        uint32 values[3];
        values[0] = offset_mode;
        values[1] = pool_number;
        values[2] = base_idx;

        if (table == LPORT_TABm) {
            BCM_IF_ERROR_RETURN(
                bcm_esw_port_lport_fields_set(unit, index, LPORT_PROFILE_LPORT_TAB,
                                              3, fields, values));
        } else if (table == EGR_LPORT_PROFILEm) {
            BCM_IF_ERROR_RETURN(
                bcm_esw_port_egr_lport_fields_set(unit, index,
                EGR_LPORT_PROFILEm, 3, fields, values));
        }
#if (defined(INCLUDE_L3) && defined(BCM_MPLS_SUPPORT))
        else if (table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm) {
            ...
            BCM_IF_ERROR_RETURN(bcmi_egress_tnl_label_entry_get(unit, data,
                offset, iptun_mpls_entry_format));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_set(unit,
                bcmiFlexCtrBaseCounterIdx, data, offset,
                iptun_mpls_entry_format, values[2]));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_set(unit,
                bcmiFlexCtrPoolNumber, data, offset, iptun_mpls_entry_format,
                values[1]));
            BCM_IF_ERROR_RETURN(bcmi_egr_ip_tnl_mpls_entry_format_set(unit,
                bcmiFlexCtrOffsetMode, data, offset, iptun_mpls_entry_format,
                values[0]));

            BCM_IF_ERROR_RETURN(bcmi_egress_tnl_label_entry_set(unit, data,
                    offset, iptun_mpls_entry_format));
        }
#endif
    } else
#endif
    {
        soc_mem_field32_set(unit, table, data, fields[0], offset_mode);
        soc_mem_field32_set(unit, table, data, fields[1], pool_number);
        soc_mem_field32_set(unit, table, data, fields[2], base_idx);
    }
    return BCM_E_NONE;
}
/*
 ...
 */
static
bcm_error_t _bcm_esw_stat_flex_install_stat_id(int unit,
                uint32  *scache_ptr, uint32 *per_pipe_mode_ifp,
                uint32 *per_pipe_mode_vfp, uint32 *per_pipe_mode_efp)
{
    uint32                    index=0;
    ...
    *per_pipe_mode_vfp = 0;
    ...
    if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_3)) {
        ...
        attr = sal_alloc(
            sizeof(bcm_stat_flex_attr_t), "attr tmp memory");
        if (NULL == attr) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
                             if (_bcm_esw_is_egr_obj(flex_stat_ctr_related_info->object)) {
                                mode_idx = flex_stat_ctr_related_info->mode - BCM_CUSTOM_EGRESS_MODE_START;
                                flex_custom_egress_modes[unit][mode_idx].total_counters = flex_stat_ctr_related_info->used & 0x3f;
                             } else
#endif
                             {
                     ...
                     if (flex_stat_ctr_related_info->mode >= BCM_CUSTOM_INGRESS_MODE_START) {
                        ...
                        total_counters = flex_custom_ingress_modes[unit][mode_idx].total_counters;
                         ...
                         if (direction == bcmStatFlexDirectionIngress) {
                            ...
                            /* EGRESS SIDE */
                            total_counters = flex_egress_modes[unit][mode_idx].total_counters;
                         }
                     }
                     if (total_counters) {
                         shr_aidxres_list_reserve_block(
                                 flex_aidxres_list_handle[unit][direction][pipe_num]
                                                         [pool_number],
                                 flex_stat_ctr_related_info->base_idx,
                                 total_counters);
                     }
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
        (_bcm_esw_stat_sync_version_above_equal(unit, BCM_WB_VERSION_1_7))
        && SOC_IS_TRIDENT3(unit)) {

        bcm_stat_flex_custom_egress_mode_t flex_custom_egress_mode;
        bcm_stat_flex_custom_mode_info_t *flex_egr_custom_mode_info =
                                     NULL;
        bcm_stat_flex_counter_id_info_t *flex_egr_stat_ctr_info =
                                     NULL;

        flex_egr_custom_mode_info = (void *)(per_pipe_mode_vfp + 1);
        for (mode = BCM_CUSTOM_EGRESS_MODE_START;
            mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE;
            mode++) {
            if (flex_egr_custom_mode_info->custom_mode != 0) {
                flex_custom_egress_mode.offset_mode =
                flex_egr_custom_mode_info->offset_mode;
                flex_custom_egress_mode.AllocPool =
                flex_egr_custom_mode_info->AllocPool;
                flex_custom_egress_mode.used = 1;
                flex_custom_egress_mode.hint.value = 0;
                flex_custom_egress_mode.hint.type = 0;

                if (flex_egr_custom_mode_info->fg_id != 0xffffffff) {
                    flex_custom_egress_mode.hint.type =
                    bcmStatGroupAllocHintEgressFieldGroup;
                    flex_custom_egress_mode.hint.value =
                    flex_egr_custom_mode_info->fg_id;
                }
                flex_custom_egress_mode.num_selectors = 0;
                flex_custom_egress_mode.attr_selectors = NULL;

                /* Total_counters has already been recovered during
                 * BCM_WB_VERSION_1_5 and keep unchanged.
                 */
                {
                    bcm_stat_flex_mode_t tmp_mode =
                       flex_egr_custom_mode_info->custom_mode;
                    if(tmp_mode >= BCM_CUSTOM_EGRESS_MODE_START) {
                       tmp_mode -= BCM_CUSTOM_EGRESS_MODE_START;
                    }
                    flex_custom_egress_mode.total_counters =
                       flex_custom_egress_modes[unit][tmp_mode].total_counters;
                }

                _bcm_esw_stat_flex_update_custom_egress_mode_info(unit,
                    flex_egr_custom_mode_info->custom_mode,
                    &flex_custom_egress_mode);
            }
            flex_egr_custom_mode_info++;
            }

        flex_egr_stat_ctr_info = (void *)flex_egr_custom_mode_info;

        *per_pipe_mode_efp = *(uint32 *)flex_egr_stat_ctr_info;
    }
#endif
    ...
    for(index=0;
        ...
        if (_pool_ctr_register[direction][index] ==
            ...
            if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
                ...
                if (_bcm_esw_stat_pool_reserved(unit, direction, index)) {
                    return BCM_E_NONE;
                }
            }
#endif
    ...
    int       max_index)
{
    ...
    if (!((egress_table == EGR_VLANm) ||
        ...
        (egress_table == EGR_VLAN_XLATE_1_DOUBLEm)  ||
        (egress_table == EGR_VLAN_XLATE_2_DOUBLEm)  ||
        (egress_table == EGR_LPORT_PROFILEm)  ||
        (egress_table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm) ||
...
#if defined (BCM_TRIDENT3_SUPPORT)
    pipe_num = _bcm_esw_get_fp_pipe_num(unit, egress_table);
#endif
    ...
    for(index=0; index <= (max_index - min_index) ; index++) {
        ...
        if (soc_feature(unit, soc_feature_flex_flow)) {
            ...
            for (i = 0; i < SOC_FLOW_DB_MAX_KEY_TYPE; i++) {
                if (SOC_MEM_FIELD_VALID(unit, egress_table, key_typef[i])) {
                    key_type = soc_mem_field32_get(unit, egress_table,
                                 egress_entry_data, key_typef[i]);
                }
            }
            data_type = -1;
            for (i = 0; i < SOC_FLOW_DB_MAX_DATA_TYPE; i++) {
                if (SOC_MEM_FIELD_VALID(unit, egress_table, data_typef[i])) {
                    data_type = soc_mem_field32_get(unit, egress_table,
                                 egress_entry_data, data_typef[i]);
                }
            }
                /* find the memory view based on the memory and key type */
            if (soc_flow_db_mem_to_view_id_get (unit, egress_table,
                                 key_type, data_type,
                    0, NULL, (uint32 *)&mem_view_id) != SOC_E_NONE) {
                tmp_mem = egress_table;
            } else {
                tmp_mem = mem_view_id;
            }
        }
        ...
        if ((soc_feature(unit, soc_feature_flex_flow)) &&
             (mem_view_id)) {
            if (_bcm_esw_stat_flex_get_egress_object(unit, tmp_mem,
                index+min_index, egress_entry_data, &object)
                != BCM_E_NONE) {
                continue;
            }
        } else
        ...
        if (egress_table == EGR_IP_TUNNEL_MPLSm) {
        ...
        } else if (egress_table == EFP_POLICY_TABLEm
#if defined(BCM_TRIDENT3_SUPPORT)
            || ((SOC_IS_TRIDENT3X(unit))
            &&  (egress_table == EFP_POLICY_TABLE_PIPE0m
            ||   egress_table == EFP_POLICY_TABLE_PIPE1m))
            ...
            uint32 stat_dscp_select = 0;
            if (SOC_MEM_FIELD_VALID(unit, egress_table, FLEX_COUNTER_DSCP_SELf)) {
               soc_mem_field_get(unit, egress_table, egress_entry_data,
                                 FLEX_COUNTER_DSCP_SELf, &stat_dscp_select);
               if (stat_dscp_select == 0) {
                    continue;
               }
            }
            _bcm_esw_get_flex_counter_fields_values(
...
#if defined(BCM_TRIDENT3_SUPPORT)
            if (soc_feature(unit, soc_feature_flex_flow)) {
                if (egress_table == EGR_IP_TUNNEL_MPLS_DOUBLE_WIDEm) {
                    sal_memset(flex_entry_data, 0, flex_entry_data_size);
                    if (soc_mem_read(unit, tmp_mem, MEM_BLOCK_ANY,
                                _bcm_esw_stat_flex_table_index_map(unit, egress_table,
                                    exact_index), flex_entry_data) == SOC_E_NONE) {
                        if (soc_mem_field_valid(unit, tmp_mem, VALIDf)) {
                            if (soc_mem_field32_get(unit, tmp_mem,
                                        flex_entry_data, VALIDf) == 0) {
                                LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                                        (BSL_META_U(unit,
                                                    "Table %s  with index %d is Not valid \n"),
                                         SOC_MEM_UFNAME(unit, tmp_mem), exact_index));
                                sal_free(flex_entry_data);
                                return;
                            }
                        }
                        if (soc_mem_field_valid(unit, tmp_mem, DATA_TYPEf)) {
                            data_type = soc_mem_field32_get(unit, tmp_mem,
                                            flex_entry_data, DATA_TYPEf);
                            /* Double Wide DATA_TYPE Check */
                            if (!(data_type & 0x10)) {
                                continue;
                            }
                        }
                    ...
                    _bcm_esw_get_flex_counter_fields_values(
                            unit, index+min_index, tmp_mem,
                            flex_entry_data, object,
                            &offset_mode, &pool_number, &base_idx);
                } else {
                    _bcm_esw_get_flex_counter_fields_values(
                            unit, exact_index, tmp_mem, egress_entry_data, object,
                            &offset_mode, &pool_number, &base_idx);
                }
            } else
#endif
...
#ifdef BCM_WARM_BOOT_SUPPORT
#if defined (BCM_TRIDENT3_SUPPORT)
        if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_7) &&
            soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
            (_bcm_esw_is_fp_table(egress_table))) {
            uint32 cmode = 0;
            bcm_error_t rv;
            uint32 stat_counter_id = 0;

            rv = _bcm_esw_custom_stat_group_id_retrieve(unit, offset_mode,
                        pipe_num, pool_number, base_idx, &stat_counter_id);
            if (rv != BCM_E_NONE) {
                LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(unit,
                                  "Total counters %d\n"),
                ...
                total_counters = flex_custom_egress_modes[unit]
                    [cmode - BCM_CUSTOM_EGRESS_MODE_START].total_counters;
            }
        }else
#endif
#endif
        ...
        if (flex_egress_modes[unit][offset_mode].total_counters == 0) {
            flex_egress_modes[unit][offset_mode].total_counters =
                   _bcm_esw_stat_flex_retrieve_total_counters(unit,
                      bcmStatFlexDirectionEgress, pool_number, offset_mode);
            LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(unit,
                                      "Total counters %d\n"),
                       flex_egress_modes[unit][offset_mode].
        ...
        shr_aidxres_list_reserve_block(
            ...
            [unit][bcmStatFlexDirectionEgress][pipe_num][pool_number],
            base_idx,
            total_counters);
        flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
                      used_entries += total_counters;
        flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
                      attached_entries += total_counters;
        flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
                      used_by_tables |= flex_used_by_table[object];
        SHR_BITSET(flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num]
                      [pool_number].used_by_objects, object);
    ...
    uint32            base_idx)
{
    ...
    BCM_STAT_FLEX_COUNTER_LOCK(unit);
        ...
        flex_pool_stat[unit][bcmStatFlexDirectionIngress][pipe_num]
                      ...
                      total_counters;
        if ((_BCM_FIELD_STAGE_INGRESS == field_stage)) {
            flex_pool_stat[unit][bcmStatFlexDirectionIngress][pipe_num][pool_number].
                used_by_tables |= FLEX_COUNTER_POOL_USED_BY_IFP_POLICY_TABLE;
        } else if (_BCM_FIELD_STAGE_EXACTMATCH == field_stage) {
            flex_pool_stat[unit][bcmStatFlexDirectionIngress][pipe_num][pool_number].
    ...
    int       max_index)
{
    ...
    if (!((ingress_table == PORT_TABm) ||
          ...
          (ingress_table == L3_DEFIP_ALPM_RAWm) ||
...
#if defined (BCM_TRIDENT3_SUPPORT)
        ((SOC_IS_TRIDENT3X(unit)) &&
          ((ingress_table == VLAN_XLATE_1_DOUBLEm)  ||
          (ingress_table == VLAN_XLATE_2_DOUBLEm)  ||
          (ingress_table == LPORT_TABm)  ||
          (ingress_table == L3_ENTRY_QUADm)  ||
          (ingress_table == L3_ENTRY_DOUBLEm)  ||
          (ingress_table == IFP_POLICY_TABLE_PIPE0m)  ||
          (ingress_table == IFP_POLICY_TABLE_PIPE1m)  ||
          (ingress_table == IFP_POLICY_TABLE_WIDE_PIPE0m)  ||
          (ingress_table == IFP_POLICY_TABLE_WIDE_PIPE1m)  ||
          (ingress_table == IFP_POLICY_TABLE_WIDEm)  ||
          (ingress_table == VFP_POLICY_TABLE_PIPE0m)  ||
          (ingress_table == VFP_POLICY_TABLE_PIPE1m)  ||
          (ingress_table == AGM_MONITOR_TABLEm) ||
          (ingress_table == VRF_ATTRS_2m)))  ||
#endif
   ...
   for(index=0; index <= (max_index - min_index) ; index++) {
       ...
       ingress_entry_data = soc_mem_table_idx_to_pointer(unit, ingress_table,
        ...
        if (soc_feature(unit, soc_feature_flex_flow)) {
            key_type = -1;
            for (i = 0; i < SOC_FLOW_DB_MAX_KEY_TYPE; i++) {
                if (SOC_MEM_FIELD_VALID(unit, ingress_table, key_typef[i])) {
                    key_type = soc_mem_field32_get(unit, ingress_table,
                                 ingress_entry_data, key_typef[i]);
                }
            }
            data_type = -1;
            for (i = 0; i < SOC_FLOW_DB_MAX_DATA_TYPE; i++) {
                if (SOC_MEM_FIELD_VALID(unit, ingress_table, data_typef[i])) {
                    data_type = soc_mem_field32_get(unit, ingress_table,
                                 ingress_entry_data, data_typef[i]);
                }
            }
            /* find the memory view based on the memory and key type
             * If not found, try fix view, else try flex view.
             */
            if (soc_flow_db_mem_to_view_id_get (unit, ingress_table,
                                               key_type, data_type,
                    0, NULL, (uint32 *)&mem_view_id) != SOC_E_NONE) {
                tmp_mem = ingress_table;
            } else {
                tmp_mem = mem_view_id;
            }
            rv = _bcm_esw_get_flex_counter_fields_values(
                 unit,index+min_index, tmp_mem,ingress_entry_data,0,
                 &offset_mode,&pool_number,&base_idx);
            if (rv != BCM_E_NONE) {
                continue;
            }
        ...
        if ((soc_feature(unit, soc_feature_flex_flow)) &&
            ...
            if (_bcm_esw_stat_flex_get_ingress_object(
                unit, tmp_mem, index+min_index, ingress_entry_data,
                &object)
                !=BCM_E_NONE) {
                continue;
            }
        ...
        {
            if (_bcm_esw_stat_flex_get_ingress_object(
                     unit, ingress_table, index+min_index,
                     ingress_entry_data, &object)
                     !=BCM_E_NONE) {
                continue;
            }
        }
    ...
    if (direction == bcmStatFlexDirectionIngress) {
       ...
       if ((flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
           ...
           (flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
            ...
            if (soc_mem_read_range(
                ...
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                          (BSL_META_U(unit,
                                      "ING_FLEX_CTR_PRI_CNG_MAPm:Read failure \n")));
       ...
       if ((flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
           ...
           (flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
            ...
            if (soc_mem_read_range(
                ...
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                          (BSL_META_U(unit,
                                      "ING_FLEX_CTR_PKT_PRI_MAPm:Read failure \n")));
       ...
       if (flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
           ...
           if (ing_port_map_dma == NULL) {
               LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "ING_FLEX_CTR_PORT_MAPm:DMA Allocation failure\n")));
               return;
           }
           ...
           if (soc_mem_read_range(
               ...
               LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "ING_FLEX_CTR_PORT_MAPm:Read failure \n")));
           ...
           for (index=0;
                ...
                soc_mem_field_get( unit, ING_FLEX_CTR_PORT_MAPm,
                      (uint32 *)&ing_port_map_dma[index], PORT_FNf,
                      &map_value);
                flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
       ...
       if ((flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
           ...
           if (ing_tos_map_dma == NULL) {
               LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "ING_FLEX_CTR_TOS_MAP:DMA Allocation failure \n")));
               return;
           }
           ...
           if(soc_mem_read_range(
              ...
              LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                        (BSL_META_U(unit,
                                    "ING_FLEX_CTR_TOS_MAPm:Read failure \n")));
       ...
       if ((flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
           ...
           (flex_ingress_modes[unit][mode].ing_attr.cmprsd_attr_selectors.
            ...
            if(soc_mem_read_range(
               ...
               LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "ING_FLEX_CTR_PKT_RES_MAPm:Read failure \n")));
       ...
       if ((flex_egress_modes[unit][mode].egr_attr.cmprsd_attr_selectors.
            ...
            (flex_egress_modes[unit][mode].egr_attr.cmprsd_attr_selectors.
             ...
             if (soc_mem_read_range(
                 ...
                 LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                           (BSL_META_U(unit,
                                       "EGR_FLEX_CTR_PRI_CNG_MAPm:Read failure \n")));
        ...
        if ((flex_egress_modes[unit][mode].egr_attr.cmprsd_attr_selectors.
            ...
            (flex_egress_modes[unit][mode].egr_attr.cmprsd_attr_selectors.
             ...
             if (soc_mem_read_range(
                 ...
                 LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                           (BSL_META_U(unit,
                                       "EGR_FLEX_CTR_PKT_PRI_MAPm:Read failure \n")));
        ...
        if ((flex_egress_modes[unit][mode].egr_attr.cmprsd_attr_selectors.
            ...
            if (egr_tos_map_dma == NULL) {
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                          (BSL_META_U(unit,
                                      "EGR_FLEX_CTR_TOS_MAPm:DMA Allocation failure\n")));
                return;
            }
            ...
            if(soc_mem_read_range(
               ...
               LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "EGR_FLEX_CTR_TOS_MAPm:Read failure \n")));
...
/*
 * Function:
 *      _bcm_esw_stat_flex_check_egress_dvp_1_table
 * Description:
 *      Checks egress_dvp_1 table for flex configuration
 * Parameters:
 *      unit                  - (IN) unit number
 * Return Value:
 *      None
 ...
 */
static void
_bcm_esw_stat_flex_check_egress_dvp_1_table(int unit)
{
    uint32    index=0;
    do {
        _bcm_esw_stat_flex_check_egress_table(unit,EGR_DVP_ATTRIBUTE_1m,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
    } while (index < soc_mem_index_count(unit,EGR_DVP_ATTRIBUTE_1m));

    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
                          "Checked EGRESS:EGR_DVP_ATTRIBUTE_1m %d entries..\n"),
               index-1));
}
/*
 * Function:
...
#ifdef BCM_TRIDENT2PLUS_SUPPORT
    uint32    index=0;
    soc_mem_t mem = EGR_IP_TUNNEL_MPLSm;
...
#endif
    do {
        _bcm_esw_stat_flex_check_egress_table(unit,mem,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
...
/*
 * Function:
 *      _bcm_esw_stat_flex_check_egress_dvp_table
 * Description:
 *      Checks egress_dvp_attr table for flex configuration
 * Parameters:
 *      unit                  - (IN) unit number
 * Return Value:
 *      None
 ...
 */
static void
_bcm_esw_stat_flex_check_egress_dvp_table(int unit)
{
    uint32    index=0;
    do {
        _bcm_esw_stat_flex_check_egress_table(unit,EGR_DVP_ATTRIBUTEm,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
    } while (index < soc_mem_index_count(unit,EGR_DVP_ATTRIBUTEm));

    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
                          "Checked EGRESS:EGR_DVP_ATTRIBUTEm %d entries..\n"),
               index-1));
}
#endif
...
_bcm_esw_stat_flex_check_ingress_vrf_table(int unit)
{
    ...
    if (soc_mem_index_count(unit, ingress_table) <= 0) {
        return;
    }

    do {
        _bcm_esw_stat_flex_check_ingress_table(unit,ingress_table,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
    } while (index<soc_mem_index_count(unit,ingress_table));
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
...
{
    uint32    index=0;
    soc_mem_t ingress_table = L3_ENTRY_IPV4_MULTICASTm;
#if defined (BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)){
        ingress_table = L3_ENTRY_DOUBLEm;
    }
    ...
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
                          "Checked INGRESS: L3_ROUTING_TABLE %d entries..\n"),
               index-1));
}
#endif
...
_bcm_esw_stat_flex_check_ingress_mpls_entry_table(int unit)
{
    ...
    soc_mem_t egress_table = MPLS_ENTRYm;
#if defined (BCM_TRIUMPH3_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit)){
        egress_table = MPLS_ENTRY_EXTDm;
    }
#endif
    do {
        _bcm_esw_stat_flex_check_ingress_table(unit,egress_table,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
    } while (index<soc_mem_index_count(unit,egress_table));
...
_bcm_esw_stat_flex_check_ingress_vfp_policy_table(int unit, uint32 per_pipe_mode)
{
    ...
    if (soc_feature(unit,soc_feature_stat_multi_pipe_support)) {
       /* Per-pipe mode */
       for(pipe_num =0; pipe_num < NUM_PIPE(unit); ++pipe_num) {
          index = 0;
          do {
...
#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)
#ifdef BCM_WARM_BOOT_SUPPORT
/*
 * Function:
 *      _bcm_esw_stat_flex_check_egress_efp_policy_table
 * Description:
...
_bcm_esw_stat_flex_check_egress_efp_policy_table(int unit,
        uint32 per_pipe_mode)
{
    uint32 start_idx = 0, end_idx = 1;
    uint32    index=0;
    uint32    mem_idx = 0;

    if(per_pipe_mode) {
        start_idx = 1;
        end_idx = NUM_PIPE(unit) + 1;
    }

    for( mem_idx = start_idx; mem_idx < end_idx; mem_idx++) {
        index = 0;
    do {
            _bcm_esw_stat_flex_check_egress_table(unit, _efp_mem_arr[mem_idx],
                index, index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
        } while (index < soc_mem_index_count(unit, _efp_mem_arr[mem_idx]));
...
{
    uint32    index=0;
    soc_mem_t egress_table = EGR_VLAN_XLATEm;
#if defined (BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)){
        egress_table = EGR_VLAN_XLATE_1_DOUBLEm;
    }
    ...
    do {
        _bcm_esw_stat_flex_check_egress_table(unit,egress_table,
             index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
        index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
    } while (index<soc_mem_index_count(unit,egress_table));
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
...
_bcm_esw_stat_flex_check_ingress_vlan_xlate_table(int unit)
{
    ...
    soc_mem_t ingress_table[3] = {VLAN_XLATEm, INVALIDm, INVALIDm};
    uint32 tab_idx = 0;

#if defined (BCM_TRIUMPH3_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit)){
        ingress_table[0] = VLAN_XLATE_EXTDm;
    }
#endif
#if defined (BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)){
        ingress_table[0] = VLAN_XLATE_1_DOUBLEm;
        ingress_table[1] = VLAN_XLATE_2_DOUBLEm;
    }
#endif
        while (INVALIDm != ingress_table[tab_idx]){
            do {
                _bcm_esw_stat_flex_check_ingress_table(unit,ingress_table[tab_idx],
                     index,index + BCM_STAT_FLEX_WARMBOOT_READ_CHUNK - 1);
                index += BCM_STAT_FLEX_WARMBOOT_READ_CHUNK;
            } while (index<soc_mem_index_count(unit,ingress_table[tab_idx]));
            tab_idx++;
            index = 0;
        }
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
...
_bcm_esw_stat_flex_check_egress_port_table(int unit)
{
    ...
    soc_mem_t egress_table = EGR_PORTm;
#if defined (BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)){
        egress_table = EGR_LPORT_PROFILEm;
    }
...
_bcm_esw_stat_flex_check_ingress_port_table(int unit)
{
    ...
    soc_mem_t ingress_table = PORT_TABm;
#if defined (BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit)){
        ingress_table = LPORT_TABm;
    }
...
#if defined (BCM_TRIDENT3_SUPPORT)
            if(cmode > BCM_CUSTOM_EGRESS_MODE_START) {
                cmode -= BCM_CUSTOM_EGRESS_MODE_START;
                total_entries =
                    flex_custom_egress_modes[unit][cmode].total_counters;
        } else
#endif
            if (cmode > BCM_CUSTOM_INGRESS_MODE_START) {
                cmode -= BCM_CUSTOM_INGRESS_MODE_START;
                total_entries =
                    flex_custom_ingress_modes[unit][cmode].total_counters;
        }
        }
    }
#endif
...
#endif
    uint32 sel=0,local_scache_size = 0;
    uint8 *stat_flex_scache_ptr = NULL;
             ...
             if (stat_ctr_map.used != 0) {
                 ...
                 if ((stat_ctr_map.mode >= BCM_CUSTOM_INGRESS_MODE_START) && 
                     (stat_ctr_map.mode < BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE)) {
                      ...
                      bcm_stat_flex_direction_t direction;
                      BCM_IF_ERROR_RETURN(
                            _bcm_esw_stat_validate_object(unit, stat_ctr_map.object, &direction));
                      if (direction == bcmStatFlexDirectionIngress) {
                          /* INGRESS SIDE */
                          ...
                          /* EGRESS SIDE */
                          flex_stat_ctr_related_info->used =
                             (flex_egress_modes[unit][stat_ctr_map.mode].total_counters);
                      }
                 }
             }
        }
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
        soc_feature(unit, soc_feature_stat_egr_multi_pipe_support) &&
                   (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_7)) {
       bcm_stat_flex_custom_mode_info_t *flex_egr_custom_mode_info =
                                    NULL;
       bcm_stat_flex_counter_id_info_t *flex_egr_stat_ctr_info =
                                    NULL;

        flex_egr_custom_mode_info = (void *)(per_pipe_mode + 1);
        for (mode = BCM_CUSTOM_EGRESS_MODE_START;
           (mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE) ; mode++) {
             int mode_idx = mode - BCM_CUSTOM_EGRESS_MODE_START;
             if (flex_custom_egress_modes[unit][mode_idx].used != 0) {
                 flex_egr_custom_mode_info->custom_mode = mode;
                 flex_egr_custom_mode_info->offset_mode =
                         flex_custom_egress_modes[unit][mode_idx].offset_mode;

                 flex_egr_custom_mode_info->AllocPool = 0;
                 flex_egr_custom_mode_info->fg_id = 0xffffffff;
                 if (flex_custom_egress_modes[unit][mode_idx].hint.type ==
                                     bcmStatGroupAllocHintEgressFieldGroup){
                     flex_egr_custom_mode_info->AllocPool =
                          flex_custom_egress_modes[unit][mode_idx].AllocPool;
                     flex_egr_custom_mode_info->fg_id =
                          flex_custom_egress_modes[unit][mode_idx].hint.value;
                 }
             } else {
                 flex_egr_custom_mode_info->custom_mode = 0;
             }
             flex_egr_custom_mode_info++;
        }
        flex_egr_stat_ctr_info = (void *)flex_egr_custom_mode_info;

        per_pipe_mode = (uint32 *)flex_egr_stat_ctr_info;
        *per_pipe_mode = !_bcm_esw_get_fp_mode_global(unit,
            bcmFieldQualifyStageEgress);
    }
#endif

    if ((BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_8) &&
            (soc_feature(unit, soc_feature_flex_stat_compression_share))) {
        /* sync compressed shared attribute flags */
        ...
        if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
                ...
                (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_7)) {
            local_scache_size +=
                sizeof(bcm_stat_flex_custom_mode_info_t) *
                (BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE - BCM_CUSTOM_EGRESS_MODE_START)
                + sizeof(uint32);
        }
#endif
        stat_flex_scache_ptr = (uint8 *)&flex_scache_ptr[unit][0] + local_scache_size;
        _bcm_esw_stat_flex_compressed_attr_flags_sync(unit,&stat_flex_scache_ptr);
    }
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
                          "OK \n")));
                                            ...
                                            bcm_stat_flex_direction_t direction)
{
    ...
    for (pool_id=0;pool_id<num_pools[direction];pool_id++) {
         ...
         if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
             if (flex_byte_counter_compaction[unit][direction][pipe_num][pool_id] != NULL) {
                 soc_cm_sfree(unit,
                         flex_byte_counter_compaction[unit][direction][pipe_num][pool_id]);
                 flex_byte_counter_compaction[unit][direction][pipe_num][pool_id] = NULL;
             }
             if (flex_packet_counter_compaction[unit][direction][pipe_num][pool_id] != NULL) {
                 soc_cm_sfree(unit,
                         flex_packet_counter_compaction[unit][direction][pipe_num][pool_id]);
                 flex_packet_counter_compaction[unit][direction][pipe_num][pool_id] = NULL;
             }
             if (flex_packet64_counter_compaction[unit][direction][pipe_num][pool_id] != NULL) {
                 soc_cm_sfree(unit,
                         flex_packet64_counter_compaction[unit][direction][pipe_num][pool_id]);
                 flex_packet64_counter_compaction[unit][direction][pipe_num][pool_id] = NULL;
             }
         }
#ifdef BCM_TOMAHAWK_SUPPORT
         if (SOC_IS_TOMAHAWKX(unit) ||SOC_IS_TRIDENT3X(unit)) {
...
bcm_error_t _bcm_esw_stat_flex_cleanup(int unit)
{
   ...
   int pipe_num = 0;
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
       (void)_bcm_esw_stat_flex_flowtracker_cleanup(unit);
    }
#endif
   ...
   if (soc_feature(unit,soc_feature_stat_multi_pipe_support)) {
      for(pipe_num = 0; pipe_num < _MAX_PIPES_PER_DEV(unit); ++pipe_num) {
         ...
         if(SOC_IS_TRIDENT3X(unit)) {
             _bcm_esw_stat_flex_cleanup_pipe(unit, pipe_num,
                 bcmStatFlexDirectionEgress);
         }
      }
     if(!SOC_IS_TRIDENT3X(unit)) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
    if (flex_custom_egress_modes[unit] != NULL) {
        sal_free(flex_custom_egress_modes[unit]);
        flex_custom_egress_modes[unit] = NULL;
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
        if (_bcm_esw_stat_flex_compressed_shared_attr_map_init(unit,FALSE) !=
                BCM_E_NONE)
            return BCM_E_MEMORY;
    }
                ...
                )
{
    ...
    switch(mem) {
...
#if defined (BCM_TRIDENT3_SUPPORT)
            if (SOC_IS_TRIDENT3X(unit)){
                config_mem = VLAN_XLATE_1_DOUBLEm;
            } else
#endif
            {
        ...
        case PORT_TABm:
            if (SOC_IS_TRIDENT3X(unit)){
                config_mem = LPORT_TABm;
            }
            break;
        case EGR_DVP_ATTRIBUTE_1m:
            if (SOC_IS_TRIDENT3X(unit)){
                config_mem = EGR_DVP_ATTRIBUTEm;
            }
            break;
        case EGR_VLAN_XLATEm:
        case EGR_VLAN_XLATE_2_DOUBLEm:
            if (SOC_IS_TRIDENT3X(unit)){
                config_mem = EGR_VLAN_XLATE_1_DOUBLEm;
            }
            break;
        case EGR_PORTm:
            if (SOC_IS_TRIDENT3X(unit)){
                config_mem = EGR_LPORT_PROFILEm;
            }
            break;
        case EFP_POLICY_TABLE_PIPE0m:
        case EFP_POLICY_TABLE_PIPE1m:
            config_mem = EFP_POLICY_TABLEm;
        break;
#endif
        default:
            config_mem = mem;
...
int _bcm_esw_stat_pool_reserved(int unit,
    bcm_stat_flex_direction_t direction, int pool_id) {
    if((0 == _bcm_stat_ing_pools_rsvd) ||
        (direction != bcmStatFlexDirectionIngress) ) {
        return 0;
    }
    if ((1<<pool_id) & _bcm_stat_ing_pools_rsvd ){
        return 0x1;
    }
...
int _bcm_esw_stat_mod_reserved(
    int unit,
    int mode_id) {

    return (_bcm_stat_mode_rsvd == mode_id);
}

uint32 _bcm_esw_stat_reserved_pools_get(int unit, uint32 num) {
    uint32 reserved_pools = 0;
    bcm_stat_flex_direction_t dir = bcmStatFlexDirectionIngress;
    uint8 pool = 0;
    uint32 entries = 0;
    uint32 smaller_pool_size = 0;
    uint32 smaller_pool_def = 0;

    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        if(SOC_IS_TOMAHAWKX(unit)) {
            smaller_pool_size = 512;
            smaller_pool_def = FLEX_COUNTER_DEFAULT_EM_POLICY_TABLE_POOL_NUMBER;
        } else if (SOC_IS_TRIDENT3(unit)) {
            smaller_pool_size = 1536;
            smaller_pool_def = FLEX_COUNTER_DEFAULT_EM_POLICY_TABLE_POOL_NUMBER;
        } else {
            LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                        (BSL_META_U(unit,
        ...
        for(pool = 0; pool < BCM_STAT_TH_FLEX_COUNTER_MAX_POOL; pool++) {
            /* index 0 can not trigger counter update */
            entries =
                soc_mem_index_count(unit, _ctr_counter_table[dir][pool]) - 0x1;
            if( num > entries) {
                reserved_pools |= (1<<pool);
                num = num - entries;
            } else {
                if (num > 0x0) {
                    if( num <= (smaller_pool_size - 1)) {
                        reserved_pools |= (1<<smaller_pool_def);
                    } else {
                        reserved_pools |= (1<<pool);
                    }
                }
    ...
    return reserved_pools;
}

uint32 _bcm_esw_stat_reserved_num_get(int unit) {
    uint32 num_counters = 0;
    uint8 ft_enable = 0;
    uint32 pipe_flow_limit;
    uint8 pipe = 0;
    char max_flows_pipe_soc_prop_str[50];

    ft_enable = soc_property_get(unit, spn_FLOWTRACKER_ENABLE, 0);
    if(ft_enable) {

        for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
            sal_snprintf(max_flows_pipe_soc_prop_str, 50, "%s%d",
                         "flowtracker_max_flows_pipe", pipe);
            pipe_flow_limit = soc_property_get(unit, max_flows_pipe_soc_prop_str, 0);
            if (pipe_flow_limit > num_counters) {
                /* Counters have to be reserved equally among all the pools, set
                 * num_counters to the pipe having the maximum number of flows.
                 */
                num_counters = pipe_flow_limit;
            }
        }
        ...
        if (num_counters == 0) {
             ...
             * equally
             */
            num_counters = soc_property_get(unit,
...
static bcm_error_t _bcm_esw_stat_flex_flowtracker_cfg(
    int unit, int cleanup) {
    bcm_error_t rv = BCM_E_NONE;
    int idx = 0;
    bcm_stat_flex_direction_t dir = bcmStatFlexDirectionIngress;
    ...
    uint32 reg_val = 0;
    int pipe = 0;
    static const soc_reg_t ing_evict_regs[] = {
    ...
    /* enable pool, disable eviction, enable clear_on_read */
    uint32 clr_on_read = 1;
    uint32 pool_enable = 1;
    uint32 eviction_mode = 0;

    if (cleanup) {
        pool_enable = 0;
        if(soc_feature(unit, soc_feature_counter_eviction)
            && soc_property_get(unit, "ctr_evict_enable", 1)) {
            clr_on_read = 1;
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        for(idx = 0; idx < BCM_STAT_TH_FLEX_COUNTER_MAX_POOL; idx++) {
            if (_bcm_esw_stat_pool_reserved(unit, dir, idx)) {
                reg = _pool_ctr_register[dir][idx];

                SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, REG_PORT_ANY,
                    0, &reg_val));
                soc_reg_field_set(unit, reg, &reg_val,
                    COUNTER_POOL_ENABLEf, pool_enable);
                soc_reg_field_set(unit, reg, &reg_val, CLR_ON_READf, clr_on_read);
                SOC_IF_ERROR_RETURN(soc_reg32_set(unit, reg, REG_PORT_ANY,
                    0, reg_val));
                for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                    reg = SOC_REG_UNIQUE_ACC(unit, ing_evict_regs[idx])[pipe];
                    SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, REG_PORT_ANY,
                        0, &reg_val));
                    soc_reg_field_set(unit, reg, &reg_val, MODEf, eviction_mode);
                    SOC_IF_ERROR_RETURN
                        (soc_reg32_set(unit, reg, REG_PORT_ANY, 0, reg_val));
                }
            }
        }
    }
...
static bcm_error_t _bcm_esw_stat_flex_flowtracker_init(int unit) {
    bcm_error_t rv = BCM_E_NONE;

    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        if (_bcm_stat_ing_pools_rsvd) {
            /* create default mode */
            bcm_stat_group_mode_attr_selector_t attr_selectors;
            sal_memset(&attr_selectors, 0x0, sizeof(attr_selectors));

            attr_selectors.attr = bcmStatGroupModeAttrPktType;
            attr_selectors.attr_value = BCM_STAT_GROUP_MODE_ATTR_ALL_VALUES;
            attr_selectors.counter_offset = 0;

            rv = _bcm_esw_stat_group_mode_id_create(unit,
                BCM_STAT_GROUP_MODE_INGRESS, 0x1, 0x1, &attr_selectors,
                &_bcm_stat_mode_rsvd);
            if((BCM_E_NONE != rv) && (BCM_E_EXISTS != rv)) {
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
                    "Failed to reserve mode for flowtracker: %s\n"), bcm_errmsg(rv)));
                _bcm_stat_mode_rsvd = -1;
            } else {
                LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
                    "Mode(%d) reserved for flowtracker\n"),
                        _bcm_stat_mode_rsvd));
            }

            _bcm_esw_stat_flex_flowtracker_cfg(unit, 0);
        }
    }
    return rv;
}

static bcm_error_t _bcm_esw_stat_flex_flowtracker_cleanup(int unit) {
    bcm_error_t rv = BCM_E_NONE;

    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {

        if (_bcm_stat_mode_rsvd != -1) {
            uint32 tmp_mode_id = _bcm_stat_mode_rsvd;

            _bcm_stat_mode_rsvd = -1;
            _bcm_esw_stat_group_mode_id_destroy(unit, tmp_mode_id);
        }

        _bcm_esw_stat_flex_flowtracker_cfg(unit, 0x1);

        _bcm_stat_num_rsvd = 0;
        _bcm_stat_ing_pools_rsvd = 0;
    }
    return rv;
}
...
/*
 * Function:
 *	_bcm_stat_flex_flowtracker_counter_info_get
 * Description:
 *	Get counter resource reserved for flowtracker
 * Parameters:
 *
 *  unit : (IN)  UNIT number
 *  info : (OUT)  Returns pool id and counter index range
 *
 * Returns:
 *	BCM_E_*
 *
 */
bcm_error_t _bcm_stat_flex_flowtracker_counter_info_get(int unit,
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        if(_bcm_stat_ing_pools_rsvd && (_bcm_stat_mode_rsvd != -1)) {
            info->offset_mode = _bcm_stat_mode_rsvd;
            for(idx = 0; idx < BCM_STAT_TH_FLEX_COUNTER_MAX_POOL; idx++) {
                if (_bcm_esw_stat_pool_reserved(unit, dir, idx)) {
                    info->pool_info[pool_num].pool_id = idx;
                    /* index 0 can not trigger counter update */
                    info->pool_info[pool_num].start_idx = 0x1;
                    info->pool_info[pool_num].end_idx =
                        soc_mem_index_max(unit, _ctr_counter_table[dir][idx]);
                    pool_num++;
                }
            }
        }
    }
    info->pool_num = pool_num;
...
bcm_error_t _bcm_esw_stat_flex_init(int unit)
{
  ...
  char *config_str;
...
#ifdef BCM_WARM_BOOT_SUPPORT
    int used_pool = 0;
    uint32     per_pipe_mode_efp = 0;
    uint8     *stat_flex_ptr = NULL;
    ...
    if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
        if (_bcm_esw_stat_flex_compressed_shared_attr_map_init(unit,TRUE) !=
                BCM_E_NONE)
            return BCM_E_MEMORY;
    }
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        ...
        _bcm_stat_num_rsvd = _bcm_esw_stat_reserved_num_get(unit);
        _bcm_stat_ing_pools_rsvd = _bcm_esw_stat_reserved_pools_get(unit,
            _bcm_stat_num_rsvd);
    }
#endif
    ...
    flex_custom_egress_modes[unit] =
        (bcm_stat_flex_custom_egress_mode_t *) sal_alloc(
            sizeof(bcm_stat_flex_custom_egress_mode_t) *
            BCM_CUSTOM_EGRESS_MODE_MAX_COUNT,
            "flex_custom_egress_modes");
    if (flex_custom_egress_modes[unit] == NULL) {
        _bcm_esw_stat_flex_cleanup(unit);
        return BCM_E_MEMORY;
    }
...
#ifdef BCM_TRIDENT3_SUPPORT
   if (SOC_IS_TRIDENT3X(unit)) {
       soc_mem_t mem = FLEX_CTR_ACTION_MASK_PROFILEm;
       soc_field_t field = POOL_ENABLEf;
       if (SOC_MEM_FIELD_VALID(unit, mem, field )){
           uint32 value =  (1 << soc_mem_field_length(unit, mem, field)) - 1;
           (void)soc_mem_fields32_modify(unit, mem, 0, 1, &field, &value);
       }
   }
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
    /* Initialize Custom Egress Mode structures*/
    for (mode = 0; mode < BCM_CUSTOM_EGRESS_MODE_MAX_COUNT; mode++) {
         sal_memset(&flex_custom_egress_modes[unit][mode],0,
                    sizeof(bcm_stat_flex_custom_egress_mode_t));
         flex_custom_egress_modes[unit][mode].AllocPool = -1;
    }
#endif
...
if (!SOC_IS_MONTEREY(unit)) {
             for(pipe_num = 0; pipe_num < num_pipes; ++pipe_num) {
                 if ((pipe_num > 0) &&
                  ...
                  if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
                      ...
                      sal_memset(flex_byte_counter_compaction[unit][direction][pipe_num][pool_id], 0,
                              sizeof(uint64) * num_ctr);
                      ...
                      sal_memset(flex_packet_counter_compaction[unit][direction][pipe_num][pool_id], 0,
                              sizeof(uint32) * num_ctr);
                      ...
                      sal_memset(flex_packet64_counter_compaction[unit][direction][pipe_num][pool_id], 0,
                              sizeof(uint64) * num_ctr);
                  }
#ifdef BCM_TOMAHAWK_SUPPORT
                  if (SOC_IS_TOMAHAWKX(unit) ||SOC_IS_TRIDENT3X(unit)) {
                  ...
                  flex_base_index_reference_count[unit][direction][pipe_num][pool_id] =
                       sal_alloc(sizeof(uint16) * num_ctr,
                                 "BaseIndexAllocation");
                  if (flex_base_index_reference_count[unit][direction][pipe_num][pool_id] == NULL){
                      LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                                (BSL_META_U(unit,
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        rv = _bcm_esw_stat_flex_flowtracker_init(unit);
        if (rv != BCM_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(unit,
                                  "ERR: flex flowtracker init")));
            _bcm_esw_stat_flex_cleanup(unit);
            return rv;
        }
    ...
    if (IsAnyModeConfigured) {
        ...
        if (soc_mem_is_valid(unit, EGR_DVP_ATTRIBUTE_1m)) {
            _bcm_esw_stat_flex_check_egress_dvp_1_table(unit);
        }
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3X(unit) && soc_mem_is_valid(unit, EGR_DVP_ATTRIBUTEm)) {
            _bcm_esw_stat_flex_check_egress_dvp_table(unit);
        }
#endif
    ...
    if (!soc_feature(unit, soc_feature_centralized_counter)) {
         ...
         */
        rv = soc_counter_extra_register(unit, _bcm_esw_stat_flex_callback);
        if (BCM_FAILURE(rv)) {
            _bcm_esw_stat_flex_cleanup(unit);
            return rv;
        }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_8) {
            /* Alloc space to save compression shared mode flags
             * for ingress/egress direction */
            if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
                local_scache_size[unit] +=
                    2 * sizeof(uint32) * BCM_STAT_FLEX_COUNTER_MAX_SCACHE_SIZE;
            }
        }
        LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
        ...
        switch (rv) {
            case SOC_E_NOT_FOUND:
                ...
                break;
            case SOC_E_NONE:
                /* We have already allocated scache(reload case) confirm size */
                if (flex_scache_allocated_size[unit] != local_scache_size[unit])
                {
                    LOG_WARN(BSL_LS_BCM_FLEXCTR,
                             (BSL_META_U(unit,
                                         "Scache size mismatch\n")));
                    _bcm_esw_stat_flex_cleanup(unit);
                    return BCM_E_INTERNAL;
                }
                break;
        ...
        if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_8)) {
            /* Recover space for compression shared mode flags
             * for ingress/egress directions */
            if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
                local_scache_size[unit] +=
                    2 * sizeof(uint32) * BCM_STAT_FLEX_COUNTER_MAX_SCACHE_SIZE;
            }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_esw_stat_flex_install_stat_id(
                                     ...
                                     &per_pipe_mode_ifp, &per_pipe_mode_vfp,
                                     &per_pipe_mode_efp));
        ...
        if (SOC_IS_TRIDENT2PLUS(unit) || SOC_IS_APACHE(unit)) {
            _bcm_esw_stat_flex_check_ingress_ifp_policy_table(unit,
                                                per_pipe_mode_ifp);
            _bcm_esw_stat_flex_check_egress_efp_policy_table(unit, 0);
        }
#endif
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3X(unit)) {
        ...
        if (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_8 &&
                soc_feature(unit, soc_feature_flex_stat_compression_share)) {
            /* Recover compression shared mode flags */
            scache_size += sizeof(uint32)*BCM_STAT_FLEX_COUNTER_MAX_SCACHE_SIZE;
            scache_size +=  sizeof(bcm_stat_flex_group_mode_related_info_t) *
                BCM_STAT_FLEX_COUNTER_MAX_MODE *
                BCM_STAT_FLEX_COUNTER_MAX_DIRECTION;

            if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
                    (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_4)) {
                scache_size +=  sizeof(bcm_stat_flex_custom_mode_related_info_t) *
                    (BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE -
                     BCM_CUSTOM_INGRESS_MODE_START) + 2*sizeof(uint32);
                scache_size +=
                    sizeof(bcm_stat_flex_counter_id_related_info_t) *
...
#endif
            stat_flex_ptr = (uint8 *)&flex_scache_ptr[unit] + scache_size;
            _bcm_esw_stat_flex_compressed_attr_flags_recover(unit,&stat_flex_ptr);
        }
    }
            ...
            bcm_stat_flex_egr_attr_t *egr_attr)
{
    ...
    flex_egress_modes[unit][mode].flags |= BCM_STAT_GROUP_MODE_EGRESS;
    return BCM_E_NONE;
}
/*
 * Function:
            ...
            bcm_stat_flex_mode_t      mode)
{
    ...
    case bcmStatFlexDirectionIngress:
        ...
        if (SOC_IS_TRIDENT2(unit)) {
            counter_group_num = BCM_STAT_TD2_FLEX_COUNTER_MAX_GROUPS;
        }
#endif
#ifdef BCM_APACHE_SUPPORT
        if (SOC_IS_APACHE(unit)) {
            counter_group_num = BCM_STAT_AP_FLEX_COUNTER_MAX_GROUPS;
        }
#endif
        for ( index = 0; index < counter_group_num; index++) {
         ...
         case bcmStatFlexPacketAttrTypeCompressed:
              ...
              if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
                  /* update compression tables refrence counts */
                  if(_bcm_stat_flex_compressed_attr_tbl_clear(unit,
                              direction,
                              ing_cmprsd_attr_selectors,NULL) != BCM_E_NONE) {
                      return BCM_E_FAIL;
                  }
              } else {
    ...
    case bcmStatFlexDirectionEgress:
         ...
         case bcmStatFlexPacketAttrTypeCompressed:
              ...
              /* Step3: Cleanup map array */
              /* update refrence count of compression tabels in egress direction */
              if (soc_feature(unit, soc_feature_flex_stat_compression_share)) {
                  /* update compression tables refrence counts */
                  if(_bcm_stat_flex_compressed_attr_tbl_clear(unit,
                              direction,NULL,
                              egr_cmprsd_attr_selectors) != BCM_E_NONE) {
                      return BCM_E_FAIL;
                  }
              } else {
    ...
    uint32 *stat_ctr_id)
{
    ...
    *stat_ctr_id = 0;
    ...
    for(i = 1; i < BCM_MAX_STAT_COUNTER_IDS; ++i) {
        ...
        mode = stat_counter_map[unit][i].mode;

        if (offset_mode < BCM_CUSTOM_INGRESS_MODE_START) {
#if defined(BCM_TRIDENT3_SUPPORT)
            if(mode >= BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE) {
                mode = mode - BCM_CUSTOM_EGRESS_MODE_START;
                mode = flex_custom_egress_modes[unit][mode].offset_mode;
            } else
#endif
            {
                mode = mode - BCM_CUSTOM_INGRESS_MODE_START;
                mode = flex_custom_ingress_modes[unit][mode].offset_mode;
            }
        }
            ...
            bcm_stat_flex_custom_ingress_mode_t *flex_ingress_mode)
{
    if (mode > (BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE-1) ||
       (mode <BCM_CUSTOM_INGRESS_MODE_START)) {
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                              "Invalid flex counter ingress mode value %d \n"),
                   mode));
        return BCM_E_PARAM;
    }
...
/*
 * Function:
 *      _bcm_esw_stat_flex_get_custom_egress_mode_info
 * Description:
 *      Get Egress custom Mode related information
 * Parameters:
 *      unit                  - (IN) unit number
 *      mode                  - (IN) Flex Offset Mode
 ...
 *
 */
bcm_error_t _bcm_esw_stat_flex_get_custom_egress_mode_info(
            int                          unit,
            bcm_stat_flex_mode_t         mode,
            bcm_stat_flex_custom_egress_mode_t *flex_egress_mode)
{
    if (mode > (BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE-1) ||
       (mode < BCM_CUSTOM_EGRESS_MODE_START)) {
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                              "Invalid flex counter egress mode value %d \n"),
                   mode));
        return BCM_E_PARAM;
    }
    if(mode >= BCM_CUSTOM_EGRESS_MODE_START){
        mode -= BCM_CUSTOM_EGRESS_MODE_START;
    }

    *flex_egress_mode = flex_custom_egress_modes[unit][mode];
    if (flex_custom_egress_modes[unit][mode].used == 0) {
        return BCM_E_NOT_FOUND;
    }
    return BCM_E_NONE;
}
...
/*
 * Function:
 *      _bcm_esw_stat_flex_update_custom_egress_mode_info
 * Description:
 *      Update Egress Mode related information
 * Parameters:
 *      unit                  - (IN) unit number
 *      mode                  - (IN) Flex Offset Mode
 *      flex_ingress_mode     - (OUT) Flex Egress Mode Info
 * Return Value:
 *      BCM_E_XXX
 * Notes:
 *
 */
bcm_error_t _bcm_esw_stat_flex_update_custom_egress_mode_info(
            int                          unit,
            bcm_stat_flex_mode_t         mode,
            bcm_stat_flex_custom_egress_mode_t *flex_egress_mode)
{
    if ((!flex_egress_mode) || (mode > (BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE-1)) ||
       (mode < BCM_CUSTOM_EGRESS_MODE_START)) {
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                              "Invalid flex counter egress mode value %d \n"),
    ...
    mode -= BCM_CUSTOM_EGRESS_MODE_START;
    flex_custom_egress_modes[unit][mode] = *flex_egress_mode;
    return BCM_E_NONE;
}
            ...
            bcm_stat_flex_egress_mode_t *flex_egress_mode)
{
    ...
    if (mode <= (BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE-1) &&
        (mode >= BCM_CUSTOM_EGRESS_MODE_START)) {
        mode = flex_custom_egress_modes[unit]
                    [mode - BCM_CUSTOM_EGRESS_MODE_START].offset_mode;
    }
#endif
                      ...
                      int num_entries,  int byte_flag)
{
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        if (_bcm_esw_stat_pool_reserved(unit, direction, pool_id)) {
            return BCM_E_NONE;
        }
    }
#endif
                                    ...
                                    int num_entries, int byte_flag)
{
    ...
    for (cnt = 0; cnt < num_entries; cnt++) {
        ...
        if ((byte_flag == 0) || (byte_flag == -1)) { /* packet only */
            uint64 temp64;
            /* If packests64 is not zero, assuming packets64 is used to pass packet value */
            if (COMPILER_64_IS_ZERO(value_ptr->packets64)) {
                COMPILER_64_SET(temp64, 0, value_ptr->packets);
            } else {
                COMPILER_64_SET(temp64, COMPILER_64_HI(value_ptr->packets64),
                    COMPILER_64_LO(value_ptr->packets64));
            }
                                            ...
                                            bcm_stat_value_t *value)
{
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
        if (_bcm_esw_stat_pool_reserved(unit, direction, pool_number)) {
            return BCM_E_NONE;
        }
    }
#endif
            ...
            bcm_stat_value_t *value)
{
    ...
    if (soc_mem_read(unit,table,MEM_BLOCK_ANY,
                _bcm_esw_stat_flex_table_index_map(unit,table,index),
                entry_data) == SOC_E_NONE) {
        soc_field_t vld_fld = INVALIDf;

        if (soc_mem_field_valid(unit, table, VALIDf)) {
            vld_fld = VALIDf;
        }
        if (soc_mem_field_valid(unit, table, BASE_VALID_0f)) {
            vld_fld = BASE_VALID_0f;
        }

        if (vld_fld != INVALIDf) {
            if (soc_mem_field32_get(unit,table,entry_data,vld_fld)==0) {
                LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                            (BSL_META_U(unit,
                            "Table %s  with index %d is Not valid \n"),
                            SOC_MEM_UFNAME(unit, table),index));
        ...
        if (soc_feature(unit, soc_feature_centralized_counter)) {
         ...
         if (byte_flag == 1) {
             ...
             if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
                 COMPILER_64_ADD_64(value->bytes,
                         flex_byte_counter_compaction[unit]
                         [direction][pipe_num][pool_number]
                         [base_idx+offset_index+loop]);
             }
             LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
             ...
             if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
                 value->packets+= flex_packet_counter_compaction[unit]
                     [direction][pipe_num][pool_number][base_idx+offset_index+loop];
                 COMPILER_64_ADD_64(value->packets64,
                         flex_packet64_counter_compaction[unit]
                         [direction][pipe_num][pool_number]
                         [base_idx+offset_index+loop]);
             }
             LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
            ...
            bcm_stat_value_t *value)
{
    ...
    if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id)) {
        ...
        if ((offset_mode >= BCM_CUSTOM_EGRESS_MODE_START) &&
            (offset_mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
            cmode = offset_mode - BCM_CUSTOM_EGRESS_MODE_START;
            offset_mode = flex_custom_egress_modes[unit][cmode].offset_mode;
            total_entries =
                flex_custom_egress_modes[unit][cmode].total_counters;
        }
#endif
            ...
            bcm_stat_value_t *value)
{
    ...
    if (direction == bcmStatFlexDirectionIngress) {
        ...
        if (soc_feature(unit,soc_feature_stat_egr_multi_pipe_support)) {
            stage = _bcm_esw_stat_get_field_stage_from_stat_ctr(unit,
                stat_counter_id);
            _bcm_esw_get_fp_pipe_from_mode(unit, offset_mode, stage,
                &pipe_num);
            if ((offset_mode >= BCM_CUSTOM_EGRESS_MODE_START) &&
                (offset_mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
                cmode = offset_mode - BCM_CUSTOM_EGRESS_MODE_START;
...
#if defined(BCM_TRIDENT2_SUPPORT)
            if (!(SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit))
                   && soc_feature(unit, soc_feature_advanced_flex_counter_dual_pipe)) {
            ...
            bcm_stat_flex_direction_t *direction)
{
    ...
    case bcmStatObjectIngVlanXlateSecondLookup:
      ...
      if (SOC_IS_TRIDENT3X(unit)) {
          if ((*actual_num_tables) < expected_num_tables) {
              table[(*actual_num_tables)++] = VLAN_XLATE_1_DOUBLEm;
          }
          if ((*actual_num_tables) < expected_num_tables) {
              table[(*actual_num_tables)] = VLAN_XLATE_2_DOUBLEm;
          }
          *direction= bcmStatFlexDirectionIngress;
      } else
#endif
    ...
    case bcmStatObjectIngL3Host:
        ...
        if (!soc_feature(unit, soc_feature_l3_extended_host_entry)) {
         ...
         if (SOC_IS_TD2_TT2(unit)) {
            ...
            if (SOC_IS_TRIDENT3X(unit)) {
               if ((*actual_num_tables) < expected_num_tables) {
                   table[(*actual_num_tables)++] = L3_ENTRY_DOUBLEm;
               }
               if ((*actual_num_tables) < expected_num_tables) {
                   table[(*actual_num_tables)++] = L3_ENTRY_QUADm;
               }
            } else
#endif
            {
    ...
    case  bcmStatObjectIngL3Route:
        if (SOC_IS_TD2_TT2(unit)) {
            ...
            if (soc_feature(unit, soc_feature_alpm2)) {
                if ((*actual_num_tables) < expected_num_tables) {
                    table[(*actual_num_tables)]=L3_DEFIP_ALPM_RAWm;
                }
            } else
#endif
#endif
    ...
    case bcmStatObjectIngVxlanDip:
         if (SOC_IS_TD2_TT2(unit)) {
            ...
            if (SOC_IS_TRIDENT3X(unit)) {
                if ((*actual_num_tables) < expected_num_tables) {
                    table[(*actual_num_tables)++] = MPLS_ENTRY_SINGLEm;
                }
                if ((*actual_num_tables) < expected_num_tables) {
                    table[(*actual_num_tables)] = MPLS_ENTRY_SINGLEm;
                }
    ...
    case bcmStatObjectEgrVlanXlate:
         ...
         if (SOC_IS_TRIDENT3X(unit)) {
             if ((*actual_num_tables) < expected_num_tables) {
                 table[(*actual_num_tables)++] = EGR_VLAN_XLATE_1_DOUBLEm;
             }
             if (soc_feature(unit, soc_feature_flex_flow) &&
                 (*actual_num_tables) < expected_num_tables) {
                 table[(*actual_num_tables)] = EGR_VLAN_XLATE_2_DOUBLEm;
             }
             *direction = bcmStatFlexDirectionEgress;
         } else
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3X(unit)) {
            *table = EGR_VLAN_XLATE_1_DOUBLEm;
            *direction = bcmStatFlexDirectionEgress;
        } else
#endif
        {
...
#if defined(BCM_TRIDENT3_SUPPORT)
            if (SOC_IS_TRIDENT3X(unit)) {
                table[(*actual_num_tables)++] = EGR_DVP_ATTRIBUTEm;
            } else
#endif
            {
            ...
            uint32    index)
{
    ...
    uint32 is_view = 0;
    uint32 width = 0;
    soc_mem_t egress_view_table = egress_table; /* memory view or memory */
    int pipe_num = 0;
    uint32 total_counters = 0;

#if defined(BCM_TRIDENT3_SUPPORT)
    if ((soc_feature(unit,soc_feature_flex_flow)) &&
        (soc_flow_db_mem_view_is_valid(unit, egress_table))) {
        /* when egress_table is view_id perform below else
         */
        BCM_IF_ERROR_RETURN(soc_flow_db_mem_view_info_get(unit, egress_table,
                                      &mem_view_info));
        is_view = 1;
        egress_table = mem_view_info.mem;
        width = BITS2BYTES(mem_view_info.width);
        LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
                    "View ID %d mem_id %d\n"),
                     egress_view_table, egress_table));
    ...
    if (!is_view) {
        width =  SOC_MEM_INFO(unit, egress_table).bytes;
    }
    ...
    egress_entry_data_size = WORDS2BYTES(BYTES2WORDS(width));
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
    ...
    if (soc_mem_read(unit, egress_view_table, MEM_BLOCK_ANY,
                 _bcm_esw_stat_flex_table_index_map(unit,egress_table,index),
                 egress_entry_data) == SOC_E_NONE) {
#if defined(BCM_TRIDENT3_SUPPORT)
        if (is_view) {
            if (soc_flow_db_mem_view_entry_is_valid(unit,
                                        egress_view_table,
                                        egress_entry_data) == FALSE) {
                LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                          (BSL_META_U(unit,
                                      "Table %s  with index %d is Not valid \n"),
                           SOC_MEM_UFNAME(unit, egress_table), index));
        ...
        _bcm_esw_get_flex_counter_fields_values(
                 unit,exact_index,egress_view_table,egress_entry_data,object_id,
                 &offset_mode,&pool_number,&base_idx);
        if ((offset_mode == 0) && (base_idx == 0)) {
             LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                       (BSL_META_U(unit,
        ...
        if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
            ...
            if (rv !=BCM_E_NONE) {
                ...
                total_counters = flex_egress_modes[unit][offset_mode].total_counters;
            } else {
                cmode = stat_counter_map[unit][stat_counter_id].mode;
                total_counters = flex_custom_egress_modes[unit][cmode - BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE].
                                                total_counters;
            }
        } else
#endif
        ...
        BCM_STAT_FLEX_COUNTER_LOCK(unit);
        flex_base_index_reference_count[unit][bcmStatFlexDirectionEgress][pipe_num]
                                       [pool_number][base_idx]--;
        flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
                       attached_entries -= total_counters;
        BCM_STAT_FLEX_COUNTER_UNLOCK(unit);
        ...
        if ((flex_base_index_reference_count[unit]
                [bcmStatFlexDirectionEgress][pipe_num] [pool_number][base_idx]) == 0 ) {
            LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(unit,
            ...
            if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
                /*
                 * To maintain legacy behaviour of detach and to handle refrence count
                 * out of sync issues, when compaction is in progress for a base index,
                 * counter values need to be explicitly cleared in bcmStatBaseIdxMove
                 * compaction sequence. 
                 */
                if (compaction_info.base_idx != base_idx &&
                        compaction_info.pool_number != pool_number) {
        ...
        if (
#if defined(BCM_TRIDENT3_SUPPORT)
            (!_bcm_esw_stat_table_is_profile(unit, egress_table)) &&
#endif
            soc_mem_write(unit, egress_view_table, MEM_BLOCK_ALL,
                      _bcm_esw_stat_flex_table_index_map(unit,egress_table,index),
                      egress_entry_data) != SOC_E_NONE) {
            LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                      (BSL_META_U(unit,
        ...
        LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                   ...
                   [bcmStatFlexDirectionEgress][pipe_num][pool_number][base_idx]));
        if (object_id != bcmStatObjectEgrMplsTunnelSecondLabel) {
            if(_bcm_esw_stat_flex_get_egress_object(unit,egress_view_table,
        ...
        if (flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
                           attached_entries  == 0) {
            _bcm_esw_stat_flex_enable_pool(
                    unit,bcmStatFlexDirectionEgress,
            ...
            uint32               pool_number)
{
    ...
    if ((offset_mode >= BCM_CUSTOM_EGRESS_MODE_START)  &&
        (offset_mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
        total_counters = flex_custom_egress_modes[unit]
            [offset_mode - BCM_CUSTOM_EGRESS_MODE_START].total_counters;
        offset_mode = flex_custom_egress_modes[unit]
            [offset_mode - BCM_CUSTOM_EGRESS_MODE_START].offset_mode;
    } else
#endif
    {
        total_counters = flex_egress_modes[unit][offset_mode].total_counters;
    ...
    if (shr_aidxres_list_elem_state(flex_aidxres_list_handle
                                    [unit][bcmStatFlexDirectionEgress][pipe_num]
    ...
    /* Free pool list */
    if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
        for (temp_base_idx =  base_idx;
                temp_base_idx < (base_idx + total_counters - 1);
                temp_base_idx++) {
            if (shr_aidxres_list_free(flex_aidxres_list_handle
                        [unit][bcmStatFlexDirectionEgress][pipe_num][pool_number],
                        temp_base_idx) != BCM_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                        (BSL_META_U(unit,
                                    "Freeing memory Table:%s:encounter problem due entry not found or due to some other issue used_by_table %x \n"),
                         SOC_MEM_UFNAME(unit, egress_table), used_by_table));
                return BCM_E_NOT_FOUND;
            }
        }
    } else {
    ...
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
               ...
               flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num]
               [pool_number].used_by_tables,
               flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num]
               [pool_number].used_entries));
    flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num][pool_number].
    ...
    if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
        flex_packet_counter_compaction[unit]
            [bcmStatFlexDirectionEgress][pipe_num][pool_number]
            [base_idx] = 0;
        COMPILER_64_SET(flex_packet64_counter_compaction[unit]
                [bcmStatFlexDirectionEgress][pipe_num][pool_number]
                [base_idx],0,0);
        COMPILER_64_SET(flex_byte_counter_compaction[unit]
                [bcmStatFlexDirectionEgress][pipe_num][pool_number]
                [base_idx],0,0);
    }
    return BCM_E_NONE;
            ...
            bcm_stat_object_t   object)
{
    ...
    total_counters = flex_ingress_modes[unit][offset_mode].total_counters;
        ...
        if (
#if defined(BCM_TRIDENT3_SUPPORT)
            (!_bcm_esw_stat_table_is_profile(unit, ingress_table)) &&
#endif
            soc_th_ifp_mem_write(unit,ingress_table, MEM_BLOCK_ALL,
                          _bcm_esw_stat_flex_table_index_map(unit,ingress_table,index),
                          ingress_entry_data) != SOC_E_NONE) {
#else
            ...
            uint32    index)
{
    ...
    uint32 is_view = 0;
    uint32 width = 0;
    soc_mem_t ingress_view_table = ingress_table; /* memory view or memory */

#if defined(BCM_TRIDENT3_SUPPORT)
    if ((soc_feature(unit,soc_feature_flex_flow)) &&
        (soc_flow_db_mem_view_is_valid(unit, ingress_table))) {
        /* when ingress_table is view_id perform below else
         */
        BCM_IF_ERROR_RETURN(soc_flow_db_mem_view_info_get(unit,
                                      ingress_table,
                                      &mem_view_info));
        is_view = 1;
        ingress_table = mem_view_info.mem;
        width = BITS2BYTES(mem_view_info.width);
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
                    "View ID %d mem_id %d\n"),
                     ingress_view_table, ingress_table));
    ...
    if (!is_view)
    {
        width =  SOC_MEM_INFO(unit, ingress_table).bytes;
    }
    ...
    pipe_num = _bcm_esw_get_fp_pipe_num(unit, ingress_table);
#endif
    ingress_entry_data_size = WORDS2BYTES(BYTES2WORDS(width));
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
              (BSL_META_U(unit,
                          "Deallocating IngressCounter Table:%s:Index:%d:"
                           " ENTRY_BYTES:%d \n"),
    ...
    if (soc_th_ifp_mem_read(unit, ingress_table, MEM_BLOCK_ANY,
                 ...
                 ingress_entry_data) == SOC_E_NONE)
#else
    if (soc_mem_read(unit, ingress_view_table, MEM_BLOCK_ANY,
                 _bcm_esw_stat_flex_table_index_map(unit,ingress_table,index),
                 ingress_entry_data) == SOC_E_NONE)
#endif /* BCM_TOMAHAWK_SUPPORT */
    {
        ...
        _bcm_esw_get_flex_counter_fields_values(
                 unit,index,ingress_view_table,ingress_entry_data,0,
                 &offset_mode,&pool_number,&base_idx);
        if ((offset_mode == 0) && (base_idx == 0)) {
             LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                       (BSL_META_U(unit,
        ...
        if(_bcm_esw_stat_flex_get_ingress_object(
           unit,ingress_view_table,index,ingress_entry_data,&object) != BCM_E_NONE) {
           sal_free(ingress_entry_data);
           return BCM_E_INTERNAL;
        }
#if defined (BCM_TOMAHAWK_SUPPORT)
    ...
    total_counters = flex_ingress_modes[unit][offset_mode].total_counters;
        ...
        /* Reset Table Values */
        _bcm_esw_set_flex_counter_fields_values(
                 unit,index,ingress_view_table,ingress_entry_data,0,0,0,0);
#if defined(BCM_TOMAHAWK_SUPPORT)
        if (SOC_IS_TOMAHAWKX(unit) &&
            ((IFP_POLICY_TABLEm == ingress_table) ||
             (IFP_POLICY_TABLE_PIPE0m == ingress_table) ||
        ...
        if (soc_th_ifp_mem_write(unit,ingress_table, MEM_BLOCK_ALL,
                          ...
                          ingress_entry_data) != SOC_E_NONE)
#else
        if (soc_mem_write(unit,ingress_view_table, MEM_BLOCK_ALL,
                          _bcm_esw_stat_flex_table_index_map(unit,ingress_table,index),
                          ingress_entry_data) != SOC_E_NONE)
#endif /* BCM_TOMAHAWK_SUPPORT */
        {
            ...
            uint32    pool_number)
{
    ...
    if (!(_bcm_esw_is_exact_match(ingress_table)
        || (SOC_IS_TRIDENT3X(unit) && _bcm_esw_is_ifp_table(ingress_table)))) {
          LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
    ...
    if (offset_mode >= BCM_CUSTOM_INGRESS_MODE_START) {
        ...
        mode_id = offset_mode;
        total_counters = flex_ingress_modes[unit][mode_id].total_counters;
    }

    object = bcmStatObjectIngExactMatch;

    pipe_num = _bcm_esw_get_fp_pipe_num(unit, ingress_table);
    if (((mode_id == 0) && (base_idx == 0)) ||
          (flex_base_index_reference_count[unit][bcmStatFlexDirectionIngress][pipe_num]
                                   [pool_number][base_idx] <= 0)) {
         LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                   (BSL_META_U(unit,
    ...
    if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
        ...
        rv = _bcm_esw_custom_stat_group_id_retrieve(unit, mode_id, pipe_num,
                                                    pool_number, base_idx,
                                                    &stat_counter_id);
        if (rv !=BCM_E_NONE) {
            LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                              "pipe %x Invalid mode value %d %d %d\n"),
                   pipe_num, mode_id, pool_number, base_idx));
            _bcm_esw_stat_get_counter_id(
                          unit, flex_ingress_modes[unit][mode_id].group_mode,
                          object,mode_id,pool_number,base_idx,&stat_counter_id);
        }
    } else {
        _bcm_esw_stat_get_counter_id(
                      unit, flex_ingress_modes[unit][mode_id].group_mode,
...
#endif
        {
            /* clear pipe0 info when all other pipes don't contain same object */
            int tmp_pipe = 0;
            if (pipe_num) {
                flex_pool_stat[unit][bcmStatFlexDirectionIngress]
                              [pipe_num][pool_number].used_by_tables &=
            ...
            for (tmp_pipe = 0; tmp_pipe < NUM_PIPE(unit); tmp_pipe++) {
                if (SHR_BITGET(flex_pool_stat[unit][bcmStatFlexDirectionIngress]
                       [tmp_pipe][pool_number].used_by_objects, object)) {
                    if(tmp_pipe == pipe_num) {
                        continue;
                    }
            ...
            void                *egress_entry_data1)
{
    ...
    if (mode >= BCM_CUSTOM_INGRESS_MODE_START) {
        uint32 tmp_mode = mode + (BCM_CUSTOM_EGRESS_MODE_START & 0xffff0000) -
            BCM_CUSTOM_EGRESS_MODE_START;
        total_counters = flex_custom_egress_modes[unit]
            [tmp_mode].total_counters;
        mode = flex_custom_egress_modes[unit]
            [tmp_mode].offset_mode;
    } else
#endif
    {
        total_counters = flex_egress_modes[unit][mode].total_counters;
    }
    ...
    if (shr_aidxres_list_elem_state(flex_aidxres_list_handle
                                    [unit][bcmStatFlexDirectionEgress]
                                    [pipe_num][pool_number],base_idx) != BCM_E_EXISTS) {
        LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
                              "Wrong base index %u \n"),
                   base_idx));
    ...
    if (base_idx_l != 0) {
        if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
            /*
             * If counter movement is in progress allow the new base index
             * programming to continue counter collection without delay.
             * Decrement old base index refrence count and destroy old stat
             * counter id in bcmStatCustomBaseIdxMove.
             */
            if (compaction_info.base_idx != base_idx_l &&
                    compaction_info.pool_number != pool_number_l &&
    ...
    if (egress_entry_data1 == NULL) {
        if (
            ...
            (!_bcm_esw_stat_table_is_profile(unit, egress_table)) &&
#endif
            soc_mem_write(unit,egress_table, MEM_BLOCK_ALL,
                    _bcm_esw_stat_flex_table_index_map(unit,egress_table,index),
    ...
    if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
       (cmode >= BCM_CUSTOM_EGRESS_MODE_START) &&
        (cmode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
        bcm_error_t rv = BCM_E_CONFIG;
    ...
    if (flex_base_index_reference_count[unit][bcmStatFlexDirectionEgress][pipe_num]
                                       [pool_number][base_idx] == 0) {
            ...
            uint32               pool_number)
{
    ...
    sal_memset(egress_entry_data, 0, width);
    ...
    _bcm_esw_get_flex_counter_fields_values(
                 unit,exact_index,egress_view_table,
            ...
            uint32               *pool_number)
{
...
#endif
        int            total_counters = 0;
        bcm_stat_flex_mode_t mode = mode_id;

#if defined (BCM_TRIDENT3_SUPPORT)
        if ((mode_id >= BCM_CUSTOM_EGRESS_MODE_START)  &&
            (mode_id < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
            total_counters = flex_custom_egress_modes[unit][mode_id -
                                     BCM_CUSTOM_EGRESS_MODE_START].total_counters;
            mode = flex_custom_egress_modes[unit][mode_id -
                                     BCM_CUSTOM_EGRESS_MODE_START].offset_mode;
        } else
#endif
        {
            total_counters = flex_egress_modes[unit][mode_id].total_counters;
        }
  ...
  case EFP_POLICY_TABLE_PIPE0m:
  case EFP_POLICY_TABLE_PIPE1m:
      field_stage = _bcm_esw_stat_get_field_stage_from_table(egress_table);
   ...
   if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
       /* Override default pool number with user configured pool */
       pool_number_l = default_pool_number = 0;
       if (*pool_number <= BCM_STAT_FLEX_COUNTER_MAX_POOL) {
           default_pool_number = *pool_number;
           pool_number_l = *pool_number;
       }
       base_idx_l = *base_idx;

       if (_bcm_stat_flex_reserve_base_index(unit,
                   bcmStatFlexDirectionEgress,
                   pool_number_l,
                   total_counters,base_idx_l,
                   used_by_table,pipe_num,object) == BCM_E_NONE){
           _bcm_esw_stat_get_counter_id(
                   unit, flex_egress_modes[unit][mode].group_mode,
      ...
      if ((flex_pool_stat[unit][bcmStatFlexDirectionEgress]
          ...
          (flex_pool_stat[unit][bcmStatFlexDirectionEgress]
           [0][pool_number_l].used_by_tables & used_by_table_selection_criteria)) {
            ...
            if ((SOC_IS_TD2P_TT2P(unit) || SOC_IS_TRIDENT3X(unit)) &&
           ...
           if (shr_aidxres_list_alloc_block(flex_aidxres_list_handle
                   [unit][bcmStatFlexDirectionEgress][pipe_num][pool_number_l],
                   total_counters,
                   &base_idx_l) == BCM_E_NONE) {
               flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num]
               ...
               flex_pool_stat[unit][bcmStatFlexDirectionEgress][pipe_num]
                    [pool_number_l].used_entries += total_counters;
               LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
                                     "Allocated  counter Table:%s with pool_number:%d"
                                      "mode:%d base_idx:%d ref_count %d\n"),
               ...
               shr_aidxres_list_state(
                   flex_aidxres_list_handle[unit][bcmStatFlexDirectionEgress][pipe_num]
                                           [pool_number_l],
            ...
            void                 *ingress_entry_data1)
{
    ...
    if ((soc_feature(unit,soc_feature_flex_flow)) &&
        ...
        /* when ingress_table is view_id perform below else
         */
        BCM_IF_ERROR_RETURN(soc_flow_db_mem_view_info_get(unit, ingress_table,
                                      &mem_view_info));
        is_view = 1;
        ingress_table = mem_view_info.mem;
        width = BITS2BYTES(mem_view_info.width);
    ...
    if (!(_bcm_esw_is_exact_match(ingress_table)
        ...
        if (ingress_entry_data1  == NULL ) {
            ...
            sal_memset(ingress_entry_data, 0, width);
            if (soc_mem_read(unit, ingress_view_table, MEM_BLOCK_ANY,
                             _bcm_esw_stat_flex_table_index_map(unit,ingress_table,index),
                             ingress_entry_data) != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_BCM_FLEXCTR,
        ...
        _bcm_esw_get_flex_counter_fields_values(
                   unit,index,ingress_view_table,ingress_entry_data_temp,0,
                   &offset_mode_l,&pool_number_l,&base_idx_l);
        if (base_idx_l != 0) {
            if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
                /*
        ...
        if (ingress_entry_data1 == NULL) {
            if (
                ...
                (!_bcm_esw_stat_table_is_profile(unit, ingress_table)) &&
#endif
                soc_mem_write(unit, ingress_view_table, MEM_BLOCK_ALL,
                              _bcm_esw_stat_flex_table_index_map(unit,ingress_table,index),
                              ingress_entry_data_temp) != SOC_E_NONE) {
                sal_free(ingress_entry_data);
                return    BCM_E_INTERNAL;
        ...
        if(_bcm_esw_stat_flex_get_ingress_object(
           unit,ingress_view_table,index,ingress_entry_data_temp,&object) !=BCM_E_NONE) {
           if (ingress_entry_data1 == NULL) {
               sal_free(ingress_entry_data);
           }
           return BCM_E_INTERNAL;
            ...
            uint32               *pool_number)
{
    ...
    if (soc_feature(unit, soc_feature_uc_flowtracker_learn) &&
        soc_feature(unit, soc_feature_uc_flowtracker_export)) {
            if (_bcm_esw_stat_pool_reserved(unit,
            ...
            bcm_stat_group_mode_t     group_mode)
{
    ...
    if (direction == bcmStatFlexDirectionIngress) {
        ...
        if ((offset_mode >= BCM_CUSTOM_EGRESS_MODE_START) &&
        (offset_mode < BCM_STAT_FLEX_CUSTOM_EGRESS_COUNTER_MAX_MODE)) {
            offset_mode = flex_custom_egress_modes[unit][offset_mode -
                                     BCM_CUSTOM_EGRESS_MODE_START].offset_mode;
        }
#endif
            ...
            bcm_stat_object_t *object)
{
    ...
    case VLAN_XLATE_1_DOUBLEm:
    case VLAN_XLATE_2_DOUBLEm:
        if ((soc_feature(unit,soc_feature_flex_flow)) && (mem_view_id)) {
...
#if defined(BCM_TRIDENT3_SUPPORT) & defined(INCLUDE_L3)
        if ((soc_feature(unit,soc_feature_flex_flow)) &&
            (_bcm_vp_used_get(unit, table_index, _bcmVpTypeFlow))) {
            ...
            bcm_stat_object_t *object)
{
    ...
    if (memp->flags & SOC_MEM_FLAG_MULTIVIEW) {
        ...
        if (soc_mem_field_valid(unit, egress_table, KEY_TYPEf)) {
                soc_mem_field_get(unit, egress_table,
                                   entry, KEY_TYPEf, &key_type);
...
#if defined(BCM_TRIDENT3_SUPPORT)
            if (soc_mem_field_valid(unit, egress_table, DATA_TYPEf)) {
                    soc_mem_field_get(unit, egress_table, entry, DATA_TYPEf,
                            &key_type);
            }
            if (key_type == _BCM_VXLAN_EGR_NEXT_HOP_L2OTAG_VIEW) {
                soc_mem_field_get(unit, egress_table, entry, L2_OTAG__DVPf,
                        &dvp);
            } else
#endif
            if ((sal_strcmp(memp->views[key_type],"SD_TAG") == 0)) {
                soc_mem_field_get(unit, egress_table, entry, SD_TAG__DVPf,
                        &dvp);
            }
        }
#endif
    ...
    case EGR_DVP_ATTRIBUTEm:
        if ((soc_feature(unit,soc_feature_flex_flow)) && (mem_view_id)) {
            *object = bcmStatObjectEgrDvpAttr;
            break;
        } else
#endif
        egress_table = EGR_DVP_ATTRIBUTEm;
...
#if defined (BCM_TRIDENT3_SUPPORT)
            if (soc_mem_field_valid(unit, egress_table, DATA_TYPEf)) {
                soc_mem_field_get(unit, egress_table, entry, DATA_TYPEf, &key_type);
            } else
#endif
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if ((soc_feature(unit,soc_feature_flex_flow)) &&
            (soc_flow_db_mem_view_is_valid(unit, table_info[index].table))) {
            /* when ingress_table is view_id perform below else
             */
            ...
            if (soc_mem_read(unit, table_info[index].table, MEM_BLOCK_ANY,
                      ...
                      table_info[index].index), entry) == SOC_E_NONE) {
                if (soc_flow_db_mem_view_entry_is_valid(unit,
                                            table_info[index].table,
                                            entry) == FALSE) {
                     continue;
                }
                if (table_info[index].direction ==
                    bcmStatFlexDirectionIngress) {
                    if (_bcm_esw_stat_flex_get_ingress_object(unit,
                             table_info[index].table,table_info[index].index,
                             entry, &object) != BCM_E_NONE) {
                        return BCM_E_NONE;
                    }
                    group=flex_ingress_modes[unit][offset_mode].group_mode;
                } else {
                    if (_bcm_esw_stat_flex_get_egress_object(unit,
                             table_info[index].table,table_info[index].index,
                            entry, &object) != BCM_E_NONE) {
                        return BCM_E_NONE;
                    }
                ...
                _bcm_esw_get_flex_counter_fields_values(
                            ...
                            entry, object, &offset_mode, &pool_number, &base_idx);
                if ((offset_mode == 0) && (base_idx == 0)) {
                     continue;
                 }
                _bcm_esw_stat_get_counter_id(
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit))
        && (object == (bcm_stat_object_t)bcmStatObjectIngExactMatch)) {
        *direction = bcmStatFlexDirectionIngress;
        return BCM_E_NONE;
    }
#endif
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_IS_TRIDENT3X(unit) && ((object == bcmStatObjectEgrFieldStageEgress)
        || (object == bcmStatObjectEgrMplsTunnelLabel))) {
        *direction=bcmStatFlexDirectionEgress;
        return BCM_E_NONE;                            
    }                                                 
         ...
         direction++ )
    {
              ...
              offset_mode++)
        {
                 ...
                 pool_number++)
            {
                /* use the least one of the configured pool size and mem table */
                num_ctr = soc_mem_index_count(unit,
                                 _ctr_counter_table[direction][pool_number]);
                if (size_pool[direction] < num_ctr) {
                    num_ctr = size_pool[direction];
                }
                for (base_index = 0; base_index < num_ctr; base_index++)
                {
...
{
    int rv = BCM_E_NONE;
    uint32 free_count = 0,largest_free = 0;
    uint32 alloc_count = 0;

    if (soc_feature(unit, soc_feature_flex_stat_compaction_support)) {
        flex_pool_stat[unit][direction]
            [pipe_num][pool_number_l].used_entries += total_counters;
        /* Reserve a base index from the pool */
        rv = shr_aidxres_list_reserve(flex_aidxres_list_handle[unit]
                [direction][pipe_num][pool_number_l],
                base_idx, base_idx + total_counters - 1);
        if (rv == BCM_E_NONE) {
            flex_pool_stat[unit][direction]
                [pipe_num][pool_number_l].used_by_tables |= used_by_table;
            SHR_BITSET(flex_pool_stat[unit][direction]
                    [pipe_num][pool_number_l].used_by_objects, object);
        } else {
            flex_pool_stat[unit][direction]
                [pipe_num][pool_number_l].used_entries -= total_counters;
        }
        if(shr_aidxres_list_state(flex_aidxres_list_handle[unit]
                    [direction][pipe_num][pool_number_l],
                    NULL,NULL,NULL,NULL,&free_count,&alloc_count,&largest_free,
                    NULL) != BCM_E_NONE) {
            LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
...
{
    uint32 new_base_idx = 0,old_base_idx = 0;
    uint32 new_pool_id = 0,old_pool_id = 0;
    uint32 old_object = 0,old_mode_id = 0;
    int old_idx =0,new_idx = 0;
    int rv = BCM_E_NONE,alloc_size = 0;
    int total_counters = 0,pipe_num = 0;
    bcm_stat_flex_direction_t direction = 0;
    bcm_stat_group_mode_t group_mode = bcmStatGroupModeSingle;
    uint32 stat_counter_id=0;
    uint32 old_pkt_ctr_value = 0;
    uint32 *dma_ptr_old = NULL;
    soc_mem_t pool_mem_old;
    uint32 old_byte_ctr[2];
    int num_entries,entry_words;
    uint64 old_byte_ctr_value,old_pkt64_ctr_value;
    ing_flex_ctr_counter_table_0_entry_t *hw_entry_old = NULL;

    if (counter_info_new == NULL)
        return BCM_E_PARAM;
    new_pool_id  = counter_info_new->ctr_tbl_pool_id;
    new_base_idx = counter_info_new->ctr_tbl_pool_base_idx;
    old_pool_id  = counter_info_old.ctr_tbl_pool_id;
    old_base_idx = counter_info_old.ctr_tbl_pool_base_idx;
    old_mode_id  = counter_info_old.ctr_tbl_mode_id;
    old_object   = counter_info_old.object;

    BCM_IF_ERROR_RETURN(_bcm_esw_stat_validate_object(unit,old_object,
                &direction));
    if (direction == bcmStatFlexDirectionIngress) {
        total_counters = flex_ingress_modes[unit][old_mode_id].total_counters;
        group_mode = flex_ingress_modes[unit][old_mode_id].group_mode;
    } else if (direction == bcmStatFlexDirectionEgress) {
        total_counters = flex_egress_modes[unit][old_mode_id].total_counters;
        group_mode = flex_egress_modes[unit][old_mode_id].group_mode;
    }

    /*
     * Add the old counter values (PACKET_COUNTERf,BYTE_COUNTEf)
     * pointing old base index to new base index.
     * Call counter sync.Delete old stat counter id.
     */
    ...
    dma_ptr_old=soc_cm_salloc(unit,alloc_size, "dma_ptr_old");
    if (dma_ptr_old== NULL) {
        return BCM_E_MEMORY;
    }

    sal_memset(dma_ptr_old, 0,alloc_size);
    pool_mem_old = _ctr_counter_table[direction][old_pool_id];
    ...
    if (soc_mem_read_range(
                ...
                pool_mem_old,
                MEM_BLOCK_ANY,
                soc_mem_index_min(unit,pool_mem_old),
                soc_mem_index_max(unit,pool_mem_old),
                dma_ptr_old) != BCM_E_NONE) {
        soc_cm_sfree(unit,dma_ptr_old);
        BCM_STAT_FLEX_COUNTER_UNLOCK(unit);
        return BCM_E_INTERNAL;
    }

    new_idx = new_base_idx;
    for (old_idx=old_base_idx; old_idx < (old_base_idx + total_counters);
            old_idx++) {
        hw_entry_old = soc_mem_table_idx_to_pointer(unit, pool_mem_old,
        ...
        soc_mem_field_get(unit,
                pool_mem_old,
                (uint32 *)hw_entry_old,
                PACKET_COUNTERf,&old_pkt_ctr_value);

        /* Byte count */
        soc_mem_field_get(unit,
                pool_mem_old,
                (uint32 *)hw_entry_old,
                BYTE_COUNTERf, old_byte_ctr);

        /* Update Compaction Counter Soft Copy */
        flex_packet_counter_compaction[unit][direction][pipe_num]
            [new_pool_id][new_idx] += old_pkt_ctr_value;
        COMPILER_64_SET(old_pkt64_ctr_value, 0, old_pkt_ctr_value);
        COMPILER_64_SET(old_byte_ctr_value, old_byte_ctr[1],
                old_byte_ctr[0]);
        COMPILER_64_ADD_64(flex_byte_counter_compaction[unit]
                [direction][pipe_num][new_pool_id][new_idx],
                old_byte_ctr_value);
        COMPILER_64_ADD_64(flex_packet64_counter_compaction[unit]
                [direction][pipe_num][new_pool_id][new_idx],
                old_pkt64_ctr_value);

        /* Reset old counter values */
        old_pkt_ctr_value = 0;
        old_byte_ctr[0]=old_byte_ctr[1]=0;
        soc_mem_field_set(
                unit,
                pool_mem_old,
                (uint32 *)hw_entry_old,
    ...
    /*
     * Decrement refrence count and
     * delete old stat counter id
     */
    _bcm_esw_stat_get_counter_id(unit, group_mode,
            old_object, old_mode_id, old_pool_id,
            old_base_idx, &stat_counter_id);

    flex_base_index_reference_count[unit][bcmStatFlexDirectionIngress][pipe_num]
        [old_pool_id][old_base_idx]--;
    flex_pool_stat[unit][bcmStatFlexDirectionIngress][pipe_num][old_pool_id].
        attached_entries -= total_counters;

    rv = _bcm_esw_stat_group_destroy(unit, stat_counter_id);
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
            (BSL_META_U(unit,
                        "Deleted old stat counter id: %d"
                        "Counters synced from old_base_idx:%d to new_base_idx:%d\n"),
             stat_counter_id,old_base_idx,new_base_idx));
...
int _bcm_stat_flex_dump_pool_info(int unit)
{
    uint32 free_count = 0,largest_free = 0;
    uint32 alloc_count = 0;
    uint32 pool_number_l=0,pipe_num=0;
    uint32    num_pools[BCM_STAT_FLEX_COUNTER_MAX_DIRECTION];
    uint32    size_pool[BCM_STAT_FLEX_COUNTER_MAX_DIRECTION];
    bcm_stat_flex_direction_t    direction=bcmStatFlexDirectionIngress;
    ...
    LOG_CLI((BSL_META_U(unit,
                    "INFO: ingress_pools:%d num_flex_egress_pools:%d "
                    "size_flex_ingress_pool:%d size_flex_egress_pool:%d"
                    "MAX_DIRECTION %d MAX_MODE %d MAX_POOL %d\n"),
                num_pools[bcmStatFlexDirectionIngress],
                num_pools[bcmStatFlexDirectionEgress],
    ...
    for (direction = bcmStatFlexDirectionIngress;
            direction < bcmStatFlexDirectionCount;
            direction++) {
        for (pool_number_l= 0; pool_number_l < num_pools[direction]; pool_number_l++) {
            shr_aidxres_list_state(flex_aidxres_list_handle[unit]
                    [direction][pipe_num][pool_number_l],
                    NULL,NULL,NULL,NULL,&free_count,&alloc_count,&largest_free,
                    NULL);
            LOG_CLI((BSL_META_U(unit,
                            "Pool Number %d Direction %s   "
                            "free_count:%d alloc_count:%d  "
                            "used_by_tables:%d used_entries:%d\n"),
                        pool_number_l,(direction == bcmStatFlexDirectionIngress) ? "Ingress":"Egress",free_count,alloc_count,
                        flex_pool_stat[unit][direction][pipe_num]
                        [pool_number_l].used_by_tables,
                        flex_pool_stat[unit][direction][pipe_num]
                        [pool_number_l].used_entries));
        }
