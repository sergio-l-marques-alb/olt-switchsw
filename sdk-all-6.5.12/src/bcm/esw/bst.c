...
_bcm_bst_td2_intr_to_resources(int unit, uint32 *flags)
{
    ...
    if ((fval = soc_reg64_field32_get(unit, THDU_BST_STATr,
        ...
        if (fval & 0x4) {
            ...
            index[0] = soc_reg64_field32_get(unit, THDU_BST_STATr, rval_uc,
                                             fld[0]);
            if (NUM_PIPE(unit) > 1) {
                if (index[0] >= 256) {
                    index[1] = index[0] - 256;
...
_bcm_bst_td2_init(int unit)
{
    ...
    if (SOC_IS_MONTEREY(unit)) {
        ...
        pres->index_min = 0;
        pres->index_max = (68 * 8) * NUM_PIPE(unit);
    } else
#endif
    if (SOC_IS_APACHE(unit)) {
    ...
    idx_count = soc_mem_index_count(unit, MMU_THDU_XPIPE_BST_PORTm) - 1;
    _BCM_BST_RESOURCE_STAT_INIT(pres, idx_count * NUM_PIPE(unit),
    ...
    idx_count = soc_mem_index_count(unit, MMU_THDM_DB_PORTSP_BST_0m) - 1;
    _BCM_BST_RESOURCE_STAT_INIT(pres, idx_count * NUM_PIPE(unit),
                   ...
                   soc_field_t field, uint32 *resval)
{
  ...
  if ((resInfo->flags & _BCM_BST_CMN_RES_F_PORTED) &&
    ...
    if (mem != INVALIDm) {
        ...
        SOC_IF_ERROR_RETURN(
            soc_reg32_get(unit, reg, REG_PORT_ANY, reg_index, &rval));
        if (op & BST_OP_GET) {
            *resval = soc_reg_field_get(unit, reg, rval, field);
            ...
            SOC_IF_ERROR_RETURN(
                soc_reg32_set(unit, reg, REG_PORT_ANY, reg_index, rval));
        }
    }
...
#if defined(BCM_APACHE_SUPPORT)
                    if (SOC_IS_APACHE(unit)) {
                        port = idx % SOC_APACHE_NUM_MMU_PORTS_PER_PIPE;
                        pool = idx / SOC_APACHE_NUM_MMU_PORTS_PER_PIPE;
                    } else
#elif defined(BCM_TRIDENT2_SUPPORT)
                    if (SOC_IS_TRIDENT2(unit)) {
                        port = idx % _TD2_MMU_PORTS_PER_PIPE;
                        pool = idx / _TD2_MMU_PORTS_PER_PIPE;
                    }
#endif
                    idx_offset = port * 4 + pool + pipe_offset;
                } else
#endif /* BCM_TRIDENT2_SUPPORT || BCM_APACHE_SUPPORT */
                {
                    idx_offset = idx + pipe_offset;
...
#define BST_TBLS "BST TBLS"
    sal_strncpy(ctrl.name, BST_TBLS, sizeof(BST_TBLS));
    ...
    rv = soc_sbusdma_desc_create(unit, &ctrl, cfg,
