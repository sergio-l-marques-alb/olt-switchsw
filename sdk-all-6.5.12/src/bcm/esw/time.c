...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_APACHE_SUPPORT)
                                         ...
                                         int *adjusted_phy_port)
{
...
#endif
    {
        ...
        lane = (lane_swap >> (((phy_port - 1) % 4) * 4)) & 0xf;
    }
                                 ...
                                 uint32 *phy_port)
{
...
#endif
    {
        ...
        BCM_IF_ERROR_RETURN(bcm_esw_port_phy_control_get(unit, first_log_port,
                                BCM_PORT_PHY_CONTROL_LANE_SWAP, &lane_swap));
        for (lane_indx = 0; lane_indx < 4; lane_indx++) {
            if ( ((lane_swap >> (lane_indx * 4)) & 0xf) == adjusted_lane) {
                break;
            }
        }
    }
...
#define BCM_TIME_APACHEX_PORT_VALID(x) ((x) >= 1 && (x) <= 71 )
#define BCM_TIME_APACHEX_LCPLL_PORT_BASE (72)
...
#define BCM_TIME_TD3_PORT_VALID(x) ((x) >= 1 && (x) <= 128) /* Section 10.2.4, TD3 top spec */
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_APACHE_SUPPORT)

    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3(unit) || SOC_IS_APACHE(unit)) {
        /* tomahawk  & TD3 */
