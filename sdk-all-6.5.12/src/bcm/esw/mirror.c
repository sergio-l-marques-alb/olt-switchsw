...
/* [31:24] entry_offset, [23:16] mapped profile index
   [15:8]  egress_edit_ctrl_id [7:0] ingress edit ctrl id */
static uint32 encap_to_edit_ctrl_id[BCM_MIRROR_MTP_COUNT]={0x00};

/* ECD = EDIT CTRL ID */
#define MIRROR_ECD_INGRESS(x) ((x) & 0xFF)
#define MIRROR_ECD_EGRESS(x)  (((x) >> 8) & 0xFF)
#define MIRROR_ECD_INDEX(x)   (((x) >> 16) & 0xFF)
#define MIRROR_ECD_OFFSET(x)  (((x) >> 24) & 0xFF)
...
STATIC int
_bcm_td3_mirror_encap_profile_index_get(int unit, int edit_ctrl_id,
                                        int dest_id, int *profile_index)
{
    uint32 id = encap_to_edit_ctrl_id[dest_id];

    if ((MIRROR_ECD_EGRESS(id)  == edit_ctrl_id) ||
        (MIRROR_ECD_INGRESS(id) == edit_ctrl_id)) {
        *profile_index = MIRROR_ECD_INDEX(id);
        return (SOC_E_NONE);
    }
...
_bcm_td3_mirror_encap_profile_index_clear(int unit, int dest_id)
{
    ...
    /* clear ingress encap info */
    if (encap_to_edit_ctrl_id[dest_id]) {
        mapped_profile_index = MIRROR_ECD_INDEX(id);
...
_bcm_td3_mirror_encap_profile_index_set(int unit, int edit_ctrl_id, int local,
                                        ...
                                        void **entries)
{
    int rv = BCM_E_NONE, prof_idx = 0, mirror_edit_ctrl_id=0;
    int encap_profile_index = 0, mapped_profile_index = 0, entry_offset=0;
    uint32 cancun_app = 0, id = encap_to_edit_ctrl_id[dest_id];
    uint64 mpi;
    egr_mirror_table_entry_t   table_entry, *mirror_table_p;
    ...
    if (MIRROR_ECD_VALID(id)) {
        ...
        mapped_profile_index = MIRROR_ECD_INDEX(id);
        COMPILER_64_SET(mpi, 0, (uint32)mapped_profile_index);

        rv = soc_cancun_cch_app_set(unit, cancun_app, mpi);
        if (rv) return (rv);
    ...
    /* Check # of times this edit_ctrl_id is already programmed */
    for (prof_idx = 0; prof_idx < 16; prof_idx++) {
        BCM_IF_ERROR_RETURN (soc_mem_read(unit, EGR_MIRROR_TABLEm,
                MEM_BLOCK_ANY, prof_idx, &table_entry));

        mirror_edit_ctrl_id = soc_EGR_MIRROR_TABLEm_field32_get(unit,
                                  &table_entry, MIRROR_EDIT_CTRL_IDf);
        if (mirror_edit_ctrl_id == edit_ctrl_id) {
            entry_offset++;
        }
    }

    /* This particular edit_ctrl_id is alread programmed. Set the entry offset. */
    if (entry_offset) {
        ...
        soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p,
                              MIRROR_ENGINE_PROFILE_OFFSETf, entry_offset);

        for (prof_idx = 0; prof_idx < ENCAP_PROF_INDEX_VAR_CNT; prof_idx++) {
            id = encap_to_edit_ctrl_id[prof_idx];
            if ((MIRROR_ECD_INGRESS(id) == edit_ctrl_id) ||
                (MIRROR_ECD_EGRESS(id)  == edit_ctrl_id)) {
                /* profile index is already set */
                mapped_profile_index = MIRROR_ECD_INDEX(id)+entry_offset;

                encap_to_edit_ctrl_id[dest_id] = ((entry_offset << 24) |
                                          (mapped_profile_index << 16) |
                                          (edit_ctrl_id << (egress*8)));

                *profile_index = mapped_profile_index;
                return (SOC_E_NONE);
            }
        }
        /* did not find edit_ctrl_id.., s/w error */
        return (SOC_E_INTERNAL);
    }

    /* First of its kind. Map a profile index */
    for (prof_idx = 0; prof_idx < ENCAP_PROF_INDEX_VAR_CNT; prof_idx++) {
        if (encap_profile_index_bmap & (0x01 << ((prof_idx*4)+1))) {
            continue;
        }
        mapped_profile_index = (prof_idx*4)+1;
        break;
    }

    /* If mapped index is already taken, find a un-used one */
    if ((encap_profile_index_bmap & (0x01 << mapped_profile_index)) ||
        (mapped_profile_index > 15)) {
                    ...
                    bcm_mirror_destination_t  *mirror_dest, void **entries)
{
    ...
    if (_is_edit_ctrl_id_sflow(edit_ctrl_id) ||
        ...
        if (_is_edit_ctrl_id_ipv6(edit_ctrl_id)) {
            ...
            *p_profile_header = mirror_dest->tos;
            ...
            /* From Length move to Time to Live */
            p_profile_header+=6;
        ...
        *(uint16*)p_profile_header = soc_ntohs(mirror_dest->udp_dst_port);
        ...
        if (_is_edit_ctrl_id_vxlan(edit_ctrl_id)) {
            ...
            *p_profile_header = PKT_ENCAP_VXLAN;
             p_profile_header+= sizeof(uint32);

            /* 24 bits VNI, 8 bits reserved */
            *(uint32*)p_profile_header = soc_ntohl(mirror_dest->vni << 8);
             p_profile_header+= sizeof(uint32);
        }
        if (_is_edit_ctrl_id_vntag(edit_ctrl_id)) {
    ...
    if (len_offset) {
        ...
        if (IS_E_PORT(unit, lcl_port) == 0) { /* HG PORT */
                              ...
                              int flags, void **entries)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_td3_mirror_flex_editor_header_create(unit,
                edit_ctrl_id, mirror_dest, entries));
                                    ...
                                    int dest_flags, void **entries)
{
    ...
    if (BCM_VLAN_VALID(mirror_dest->vlan_id)) {
        int lcl_port = mirror_dest->gport & MIRROR_PORT_MASK;
        soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
                             MIRROR_VLAN_VLDf,
                             IS_E_PORT(unit, lcl_port)? 0:1);
        soc_mem_field32_set (unit, EGR_MIRROR_TABLEm, mirror_table_p, 
...
_bcm_trident3_mirror_sflow_tunnel_set(int unit, int index,
                int flags, int dest_flags, void **entries)
{
    uint32  edit_ctrl_id;
...
int
_bcm_td3_mirror_sflow_tunnel_set(int unit, int index,
                           bcm_gport_t *trunk_arr, int flags)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_trident3_mirror_sflow_tunnel_set(unit, index, 
                                                       flags, 0, entries));

    BCM_IF_ERROR_RETURN(_bcm_td3_egr_mirror_table_entry_add(unit, entries,
                             &profile_index));

    /* Supply the correct profile index to the selected MTPs */
    BCM_IF_ERROR_RETURN(_bcm_egr_mirror_encap_entry_mtp_update(unit, index,
                                                    profile_index, flags));
    return (BCM_E_NONE);
}
                             ...
                             uint16 *tpid, uint16 *vlan)
{
    ...
    BCM_IF_ERROR_RETURN (_bcm_td3_mirror_encap_profile_index_get(unit,
                              edit_ctrl_id, 0,
                              &encap_profile_index));
...
_bcm_td3_mirror_encap_sw_dump(int unit)
{
    ...
    int encap_profile_index=0, edit_ctrl_id = 0;
    ...
    for (i = 0; i < num_entries; i ++) {
        ...
        edit_ctrl_id = soc_mem_field32_get (unit, EGR_MIRROR_TABLEm,
                         entries[EGR_MIRROR_TABLE_ENTRIES],
                         MIRROR_EDIT_CTRL_IDf);

        rv  = _bcm_td3_mirror_encap_profile_index_get(unit,
                             edit_ctrl_id, i, &encap_profile_index);
        if (SOC_FAILURE(rv)) {
            LOG_CLI((BSL_META_U(unit,
                      " *** Error getting encap profile index: %d ***\n"), rv));
            break;
        }
        ...
        if (encap_profile_index) {
            ...
            rv = soc_mem_read(unit, FLEX_EDITOR_MIRROR_ENCAP_0_PROFILE_2_TABLEm,
                    MEM_BLOCK_ANY, encap_profile_index, &profile_2);
            if (rv) break;
            rv = soc_mem_read(unit, FLEX_EDITOR_MIRROR_ENCAP_0_PROFILE_3_TABLEm,
                    MEM_BLOCK_ANY, encap_profile_index, &profile_3);
            if (rv) break;
    ...
    if (_is_edit_ctrl_id_sflow(edit_ctrl_id) ||
        ...
        *****************************************/
        ...
        if (_is_edit_ctrl_id_vxlan(edit_ctrl_id)) {
            ...
            /* 24 bits VNI, 8 bits reserved */
            mirror_dest->vni = soc_ntohl(*(uint32*)p_profile_header) >> 8;
            p_profile_header+= sizeof(uint32);
        }
        if (_is_edit_ctrl_id_vntag(edit_ctrl_id)) {
            mirror_dest->flags |= BCM_MIRROR_DEST_TUNNEL_NIV;
...
_bcm_td3_mirror_tunnel_reload(int unit, bcm_gport_t dest_id,
                             uint32 profile_index)
{
    int     edit_ctrl_id, encap_profile_index = 0;
    void   *entries[EGR_MIRROR_TABLE_ENTRIES_NUM];
    ...
    BCM_IF_ERROR_RETURN (_bcm_td3_mirror_encap_profile_index_get(unit,
                          edit_ctrl_id,
                          mirror_dest->mirror_dest_id & MIRROR_PORT_MASK,
                          &encap_profile_index));
            ...
            for (mtp_index = 0; mtp_index < mtp_port_count;
                ...
                if (soc_mem_field32_get(unit, EGR_IM_MTP_INDEXm,
                    ...
                    if (MIRROR_MTP_METHOD_IS_DIRECTED_FLEXIBLE(unit)) {
                        ...
                        dest_id = MIRROR_CONFIG_SHARED_MTP_DEST(unit, mtp_index);
                        mir_dest =
                            MIRROR_DEST(unit,
                                MIRROR_CONFIG_SHARED_MTP_DEST(unit, mtp_index));
                                         ...
                                         bcm_gport_t mirror_dest) 
{
    ...
    if (SOC_IS_TRIDENT3X(unit)) {
        BCM_IF_ERROR_RETURN(
            _bcm_td3_mirror_encap_profile_index_clear(unit, mtp_index));
    }
#endif /* BCM_TRIDENT3_SUPPORT */
