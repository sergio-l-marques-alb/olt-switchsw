...
_field_wb_opaque2_set_recover(int unit, _field_entry_t *f_ent, uint32 *ebuf,
                              _field_action_bmp_t *act_bmp);
int
_field_wb_lag_rh_set_recover(int unit, _field_entry_t *f_ent,
                            uint32 *ebuf, _field_action_bmp_t *act_bmp);
int
_field_wb_ecmp2_rh_set_recover(int unit, _field_entry_t *f_ent,
                            uint32 *ebuf, _field_action_bmp_t *act_bmp);
int
_field_wb_hgt_rh_set_recover(int unit, _field_entry_t *f_ent,
                            uint32 *ebuf, _field_action_bmp_t *act_bmp);
...
void* _field_wb_action_recovery_func_ptr[_FieldActionSetCount] = {
    ...
    _field_wb_ecmp2_rh_set_recover, /* _FieldActionEcmp2RhActionSet */
    NULL, /* _FieldActionEcmp1RhActionSet */
    _field_wb_lag_rh_set_recover, /* _FieldActionLagRhActionSet */
    _field_wb_hgt_rh_set_recover, /* _FieldActionHgtRhActionSet*/
...
_field_em_group_entries_sync(int unit, _field_group_t *fg)
{
    ...
    soc_mem_t *em_nar_mem, *em_wide_mem;
    static soc_mem_t em_entry_nar_mem[_FP_MAX_NUM_PIPES+1] = {
    ...
    static soc_mem_t em_entry_wide_mem[_FP_MAX_NUM_PIPES+1] = {
    ...
    em_nar_mem = em_entry_nar_mem;
    em_wide_mem = em_entry_wide_mem;
    ...
    if ((fg->em_mode == _FieldExactMatchMode128) ||
        ...
        num_attr = 5;
        if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
            mem = em_nar_mem[_FP_MAX_NUM_PIPES];
        } else {
            mem = em_nar_mem[fg->instance];
        }
    } else {
        ...
        num_attr = 6;
        if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
            mem = em_wide_mem[_FP_MAX_NUM_PIPES];
        } else {
            mem = em_wide_mem[fg->instance];
        }
    }
    rv = _bcm_field_entry_tcam_parts_count(unit, fg->stage_id,
                                           fg->flags, &parts_count);
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        if (f_pr->group !=NULL)
        {
            ...
            for (idx = 0; (idx < parts_count && f_presel_p != NULL); idx++) {
                 ...
                 * post the version 23, its fixed in the sync code of extractor
                 * and hence below fix is not required.
                 */
...
_field_entry_recover(int unit, _field_group_t *new_fg)
{
    ...
    soc_mem_t *fp_policy_mem_wide = NULL; /* IFP Policy memory wide */
    char *fp_policy_buf_wide[_FP_MAX_NUM_PIPES] =  {0};
                                          /* Buffer to read the FP_POLICY wide table */

    soc_mem_t *fp_action_profile_mem = NULL;/* IFP action profile memeory */
    ...
    soc_mem_t  policy_tcam_wide_mems[_FP_MAX_NUM_PIPES + 1] = {
                                    ...
                                    /* IFP policy memory wide for every Pipe */
    ...
    if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
        ...
        if (soc_feature(unit, soc_feature_ifp_action_profiling)) {
            fp_policy_mem_wide = &policy_tcam_wide_mems[_FP_MAX_NUM_PIPES];
            fp_action_profile_mem = &act_prof_mems[_FP_MAX_NUM_PIPES];
        }
    } else {
        num_pipes = stage_fc->num_pipes;
        ...
        if (soc_feature(unit, soc_feature_ifp_action_profiling)) {
            fp_policy_mem_wide = &policy_tcam_wide_mems[0];
            fp_action_profile_mem = &act_prof_mems[0];
        }
    }
    ...
    for (pipe = 0; pipe < num_pipes; pipe++) {
        ...
        if (soc_feature(unit, soc_feature_ifp_action_profiling)) {
            fp_policy_buf_wide[pipe] = soc_cm_salloc(unit,
                           SOC_MEM_TABLE_BYTES(unit, fp_policy_mem_wide[pipe]),
        ...
        if (soc_feature(unit, soc_feature_ifp_action_profiling)) {
            index_min = soc_mem_index_min(unit, fp_policy_mem_wide[pipe]);
            index_max = soc_mem_index_max(unit, fp_policy_mem_wide[pipe]);
            rv = soc_mem_read_range(unit, fp_policy_mem_wide[pipe], MEM_BLOCK_ALL,
                                   index_min, index_max, fp_policy_buf_wide[pipe]);
            BCM_IF_ERROR_CLEANUP(rv);
            index_min = soc_mem_index_min(unit, fp_action_profile_mem[pipe]);
            index_max = soc_mem_index_max(unit, fp_action_profile_mem[pipe]);
            rv = soc_mem_read_range(unit, fp_action_profile_mem[pipe], MEM_BLOCK_ALL,
        ...
        switch (tlv.type) {
            ...
            case _bcmFieldInternalGroupEntry:
                 ...
                 for (i = 0; i < entry_count; i++) {
                    ...
                    /* for TD3, irrespective of group mode, tcam idx =policy idx
                    ...
                    if (soc_feature(unit, soc_feature_ifp_action_profiling)) {
                        if (!(f_ent->group->flags
                         ...
                         & _FP_GROUP_INTRASLICE_DOUBLEWIDE)) {
                            e_buf = soc_mem_table_idx_to_pointer(
                                         unit,
                                         fp_policy_mem_wide[pipe],
                                         uint32 *,
                                         fp_policy_buf_wide[pipe],
                                         policy_tcam_idx);
                            soc_mem_field_get(unit, fp_policy_mem_wide[pipe],
                                         e_buf, POLICY_DATAf, act_data);
                         } else {
                             soc_mem_field_get(unit, fp_policy_mem[pipe], e_buf,
                            ...
                            act_idx = f_ent->group->action_profile_idx[j];
                                if (act_idx != -1) {
                                    ...
                                    rv = _field_wb_action_profile_parse(unit, f_ent,
                                                ...
                                                &action_bitmap);
                                    BCM_IF_ERROR_CLEANUP(rv);
                                    /* recovey of hw_idx and param's are updated
                                       from h/w tables directly like TH EM
                                       Have to revist the need of below function
                                       after verifying the warmboot regression
                                       tests as part of IFP Legacy bring up*/
                                     #if 0
                                         rv = field_wb_td3_ifp_actions_recover(unit,
                                           e_buf, f_ent, val_array,
                                           hw_idx_array, &action_param_type);
                                     BCM_IF_ERROR_CLEANUP(rv);
                                     #endif
                                }
                            }
                    ...
                    entry_pos++;
                    for (part_index = 0; part_index < parts_count;
                                         part_index ++) {
                        f_ent[part_index].fs->entries[
                            f_ent[part_index].slice_idx] = f_ent + part_index;
...
_field_group_recover(int unit, _field_group_t *fg)
{
    ...
    /* For WB version prior to 23, the extractors are recovered
     * at the end of recovery.
     * post the version 23, its fixed in the sync code of extractors
     * and hence below fix is not required.
     */
    if ((fc->wb_recovered_version) >= BCM_FIELD_WB_VERSION_1_24) {
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        switch (action_arr[action_idx]) {
           ...
           /*         ov_new = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &entry, IFP_ACTIONS__L3_UC_VLAN_DISABLEf);
            ...
            case bcmFieldActionChangeL2Fields:
                if (a_offset.value[1] == 1) {
                    param[0] = BCM_XGS3_EGRESS_IDX_MIN + a_offset.value[0];
                    valid = 1;
                }
                break;
#endif
                            ...
                            uint32 *ebuf, _field_action_bmp_t *act_bmp)
{
    ...
    bcm_field_action_t action_idx;
    int param[6] = {0};
    int hw_index = 0, i = 0, valid = 0, rv = BCM_E_NONE;
    bcm_field_action_t action_arr[] = {
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (SOC_SUCCESS(rv1) && SOC_SUCCESS(rv2)) {
           ...
           if (act_val == ext_ctrl_id) {
              rv1 = soc_cancun_ceh_obj_field_get(unit, "EDIT_CTRL_ID",
                                     "INT_ENABLE_MASK", &ceh_info1);
              rv2 = soc_cancun_ceh_obj_field_get(unit, "EDIT_CTRL_ID",
                                     "INT_ENABLE_MASK", &ceh_info2);
              if (SOC_SUCCESS(rv1) && (SOC_SUCCESS(rv2))) {
                 int edit_ctrl_id;
    
                 ACTION_GET(unit, f_ent, ebuf, _bcmFieldActionEditCtrlId, 0, &a_offset);
        ...
        if (valid == 0) {
           rv1 = soc_cancun_ceh_obj_field_get(unit, "EXTRACTION_CTRL_ID",
                                                 "ADD_ING_OTAG_TO_EGR_OTAG", &ceh_info1);
           rv2 = soc_cancun_ceh_obj_field_get(unit, "EXTRACTION_CTRL_ID",
                                                 "ADD_ING_OTAG_TO_EGR_OTAG_MASK", &ceh_info2);
           if (SOC_SUCCESS(rv1) && SOC_SUCCESS(rv2)) {
              act_val = ceh_info1.value;
              act_val |= ((ceh_info2.value) << ceh_info2.offset);
        ...
        if (valid == 0) {
           ...
           switch (ext_ctrl_id) {
              case _BCM_FIELD_EXTR_CTRL_ID_ING_OUTER_VLAN_INSERT:
                 action = bcmFieldActionAddIngOuterVlanToEgrOuterVlan;
                 valid = 1;
                 break;
                              ...
                              _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (a_offset.value[0] != 0) {
           valid = 1;
           action = bcmFieldActionErspan3HdrSgtAdd; 
        }
                              ...
                              _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (a_offset.value[0] != 0) {
           valid = 1;
           action = bcmFieldActionErspan3HdrVsmIdAdd;
        }
...
/*
 * Function:
 *     _field_wb_lag_rh_set_recover
 * Purpose:
 *      Recover actions that belong to LAG RH set
 *
 * Parameters:
 *      unit     - (IN) StrataSwitch unit #.
 *      f_ent    - (IN) pointer to entry structure which refers
                            ...
                            uint32 *ebuf, _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (valid) {
            f_act = NULL;
            _FP_XGS3_ALLOC(f_act, sizeof(_field_action_t), "FP WB PDD LAG RH Alloc");
            if (f_act == NULL) {
                return BCM_E_MEMORY;
            }
            f_act->action = action_arr[action_idx];
...
/*
 * Function:
 *     _field_wb_ecmp2_rh_set_recover
 * Purpose:
 *      Recover actions that belong to ECMP2 RH set
 *
 * Parameters:
 *      unit     - (IN) StrataSwitch unit #.
 *      f_ent    - (IN) pointer to entry structure which refers
                            ...
                            uint32 *ebuf, _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (valid) {
            f_act = NULL;
            _FP_XGS3_ALLOC(f_act, sizeof(_field_action_t), "FP WB PDD ECMP2 RH Alloc");
            if (f_act == NULL) {
                return BCM_E_MEMORY;
            }
            f_act->action = action_arr[action_idx];
...
/*
 * Function:
 *     _field_wb_hgt_rh_set_recover
 * Purpose:
 *      Recover actions that belong to HiGig Trunk RH set
 *
 * Parameters:
 *      unit     - (IN) StrataSwitch unit #.
 *      f_ent    - (IN) pointer to entry structure which refers
                            ...
                            uint32 *ebuf, _field_action_bmp_t *act_bmp)
{
    ...
    for (action_idx = 0; action_idx < (sizeof(action_arr)/sizeof(int)); action_idx++) {
        ...
        if (valid) {
            f_act = NULL;
            _FP_XGS3_ALLOC(f_act, sizeof(_field_action_t), "FP WB PDD HGT RH Alloc");
            if (f_act == NULL) {
                return BCM_E_MEMORY;
            }
            f_act->action = action_arr[action_idx];
...
_field_em_group_entries_recover(int unit, _field_group_t *fg)
{
    ...
    static soc_mem_t em_entry_wide_mem[_FP_MAX_NUM_PIPES+1] = {
     ...
     static soc_mem_t em_default_entry_mem[_FP_MAX_NUM_PIPES+1] = {
    ...
    if ((fg->em_mode == _FieldExactMatchMode128) ||
        ...
        if (stage_fc->oper_mode == bcmFieldGroupOperModeGlobal) {
            mem = em_nar_mem[_FP_MAX_NUM_PIPES];
            def_policy_mem = em_default_entry_mem[_FP_MAX_NUM_PIPES];
        } else {
            mem = em_nar_mem[fg->instance];
                                    ...
                                    _field_stage_t *stage_fc)
{
    ...
    _field_stage_t  *stage_fc_temp = NULL;
    ...
    /* if no EM group is created, then the stage mode should be same as IFP */
    if ((group_count == 0) &&
        (0 == soc_property_get(unit, spn_FLOWTRACKER_ENABLE, 0))) {
        BCM_IF_ERROR_CLEANUP(_field_stage_control_get(unit,
                             _BCM_FIELD_STAGE_INGRESS,
                              &stage_fc_temp));
        if (stage_fc_temp->oper_mode ==
              bcmFieldGroupOperModePipeLocal) {
            BCM_IF_ERROR_CLEANUP(_bcm_field_wb_group_oper_mode_set(
                        unit, bcmFieldQualifyStageIngressExactMatch,
                        &stage_fc,
                       bcmFieldGroupOperModePipeLocal));
            stage_fc->num_instances = stage_fc_temp->num_pipes;
        }
    }
    ...
    BCM_IF_ERROR_CLEANUP(rv);
    /* the extractor codes will be recoverd forwide mode groups
     * post the version 23, its fixed int he sync code of extractors
     * and hence below fix is not required
     */
    if ((fc->wb_recovered_version) < BCM_FIELD_WB_VERSION_1_24) {
        if (SOC_SUCCESS(rv)) {
                                ...
                                _field_action_bmp_t *act_bmp)
{
    ...
    if (f_ent->group->stage_id == _BCM_FIELD_STAGE_EXACTMATCH) {
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            ...
            if (_FieldExactMatchMode128 == f_ent->group->em_mode) {
