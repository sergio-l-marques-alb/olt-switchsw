                             ...
                             bcmi_keygen_oper_t *keygen_oper)
{
    ...
    BCMI_KEYGEN_IF_ERR_EXIT((num_ext_codes == 0) ? BCM_E_INTERNAL : BCM_E_NONE);
                         ...
                         bcmi_keygen_ext_field_info_t *finfo)
{
    ...
    if (finfo->pri_ctrl_sel[0] != BCMI_KEYGEN_EXT_CTRL_SEL_DISABLE) {
        BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(
            bcmi_keygen_ext_code_part_get(unit, keygen_md, finfo->pri_ctrl_sel[0],
                                          finfo->pri_ctrl_sel_val[0], &part));
    }
