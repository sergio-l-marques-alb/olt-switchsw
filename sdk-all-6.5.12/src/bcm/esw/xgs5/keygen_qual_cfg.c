                   ...
                   bcmi_keygen_md_t **keygen_md)
{
    ...
    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_qual_cfg_info_alloc(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_qual_cfg_info_db_alloc(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_ext_cfg_alloc(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_qual_cfg_id_array_alloc(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_ext_sections_get(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_pmux_process(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_acl_attrs_init(unit, keygen_cfg, *keygen_md));

    BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(bcmi_keygen_ext_section_gran_info_init(unit, *keygen_md));
                         ...
                         bcmi_keygen_md_t *keygen_md)
{
    ...
    while (TRUE) {
        ...
        for (idx = 0; idx < keygen_cfg->qual_info_count; idx++) {
            ...
            sal_memcpy(qual_cfg1, qual_cfg2, sizeof(bcmi_keygen_qual_cfg_t));
        }

        rv = bcmi_keygen_ext_alloc(unit, keygen_cfg, keygen_oper, keygen_md);
        if (BCMI_KEYGEN_FAILURE(_func_rv) && (_func_rv != BCM_E_RESOURCE)) {
            BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(_func_rv);
        } else if (BCMI_KEYGEN_SUCCESS(rv)) {
           break;
        }
        ...
        if (qual_cfg_combinations_tried == qual_cfg_combinations) {
            BCMI_KEYGEN_IF_ERR_VERBOSE_EXIT(rv);
        }
