...
STATIC int
_bcm_xgs5_ft_ip_headers_get(bcm_flowtracker_collector_info_t *collector_info,
                             uint16 *etype,
        ...
        shr_ft_msg_ctrl_collector_create_t *collector_create_msg)
{
     ...
     */
    if (collector_info->transport_type == bcmFlowtrackerCollectorTransportTypeIpfixIpv4Udp) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs5_ft_udp_header_get(collector_info, &udp));
    }

    if (collector_info->transport_type == bcmFlowtrackerCollectorTransportTypeIpfixIpv4Udp) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs5_ft_ip_headers_get(collector_info,
                                          &etype,
                                          &ipv4));
    }
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_xgs5_ft_l2_header_get(collector_info, etype, &l2));
    ...
    if (collector_info->transport_type == bcmFlowtrackerCollectorTransportTypeIpfixIpv4Udp) {
        collector_create_msg->udp_base_checksum =
                              shr_udp_initial_checksum_get(0, &ipv4, NULL, &udp);
    }
                  ...
                  uint16 *encap_length, uint8 *buf)
{
    ...
    for (i = 0; i < template_int_info->num_export_elements; i++) {
        if (template_int_info->elements[i].enterprise == 0) {
            ...
            SHR_PKT_PACK_U8(cur_ptr, template_int_info->elements[i].data_size);
        } else {
            /* Enterprise specific */
            /*
             * +-+--------------------+------------------+
...
/*
 * Function:
 *     _bcm_xgs5_ft_elph_profile_create
 * Purpose:
...
_bcm_xgs5_ft_elph_profile_list_create(int unit)
{
    if (ft_elph_profile_info_list[unit] == NULL) {
        _BCM_FT_ALLOC(ft_elph_profile_info_list[unit], _bcm_int_ft_elph_profile_info_t,
                      ...
                      BCM_INT_FT_MAX_ELEPHANT_PROFILES, "Elephant profile list");
    }
                               ...
                               sal_usecs_t timeout)
{
    ...
    rv = soc_cmic_uc_msg_send_receive(unit, 0,
                                      &send, &reply,
                                      timeout);
    if (rv != SOC_E_NONE){
...
int _bcm_xgs5_ft_eapp_send_ctrl_init_msg (int unit) 
{
    ...
    for (i = 0; i < ctrl_init_msg.num_pipes; i++) {
        ...
        ctrl_init_msg.pipe[i].num_ports = ft_info->pipe_info[i].num_ports;
        sal_memcpy(ctrl_init_msg.pipe[i].ports, ft_info->pipe_info[i].ports, 
                   sizeof(uint16) * ft_info->pipe_info[i].num_ports);
    ...
    ctrl_init_msg.scan_interval_msecs   = ft_info->scan_interval;
                    ...
                    bcm_flowtracker_collector_t collector_id) 
{
    ...
    rv = _bcm_xgs5_ft_eapp_collector_encap_build_pack(
                        &(collector_int_info.collector_info),
                        &collector_create_msg);
    if (BCM_FAILURE(rv)) {
        return rv;
    }

    /* Check if there is space to insert at least one flow */
    if ((collector_create_msg.encap_length +
         BCM_INT_FT_IPFIX_MSG_HDR_LENGTH   +
         BCM_INT_FT_SET_HDR_LENGTH         +
         BCM_INT_FT_RECORD_LENGTH          +
...
int bcm_xgs5_ft_eapp_init(int unit)
{
    ...
    if (!soc_uc_in_reset(unit, uc)) {
        rv = soc_cmic_uc_appl_init(unit, uc, MOS_MSG_CLASS_FT,
                                   SHR_FT_MAX_UC_MSG_TIMEOUT,
                                   FT_SDK_VERSION, FT_UC_MIN_VERSION,
                                   NULL, NULL);
        if (SOC_E_NONE != rv) {
                                   ...
                                   shr_ft_msg_ctrl_elph_profile_filter_t *dst)
{
     ...
     * expects it as bytes/msec
     */
    dst->rate_low_threshold  = src->rate_low_threshold_kbits_sec / 8;
    dst->rate_high_threshold = src->rate_high_threshold_kbits_sec / 8;

    /* Convert the monitoring interval to scan count */
    dst->scan_count = src->monitor_interval_usecs / (ft_info->scan_interval * 1000);
...
int _bcm_xgs5_flowtracker_not_empty;
