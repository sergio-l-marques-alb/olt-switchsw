...
STATIC int trunk_mbr_entry_zero_rsvd[BCM_MAX_NUM_UNITS] = {0};
...
_bcm_trident_trunk_member_bk_init(int unit) 
{
    ...
    /* Reserve entry 0 of TRUNK_MEMBERm, as disabled groups have their
       BASE_PTR set to 0 */
    _BCM_FP_TRUNK_MEMBER_USED_SET(unit, 0);
    trunk_mbr_entry_zero_rsvd[unit] = TRUE;
        ...
        bcm_trunk_member_t *member)
{
    ...
    if (num_egr_ports > 0) {
        ...
        if (trunk_mbr_entry_zero_rsvd[unit] == TRUE) {
            trunk_member_entry_t trunk_member_entry;
            sal_memset(&trunk_member_entry, 0, sizeof(trunk_member_entry_t));
            soc_TRUNK_MEMBERm_field32_set(unit,&trunk_member_entry,
                                          MODULE_IDf,0xff);
            ...
            trunk_mbr_entry_zero_rsvd[unit] = FALSE;
        }
    }
