...
int
bcm_th2_cosq_tcb_queue_resolve(int unit, int pipe, int queue, int *index)
{
    *index = queue - _bcm_th2_cosq_tcb_ucq_offset[pipe];
    return BCM_E_NONE;
}
                                ...
                                int num, bcm_cosq_tcb_buffer_t *record_entries)
{
    ...
    uint8 *tcb_md_buf = NULL;
    int i, j, idx, wrapped;
    int dgpp, sgpp, pipe, qnum, queue, depth, cell_bytes;
    ...
    if (wrapped) {
        ...
        for (i = idx + 1, j = 0; j < num; i++, j++) {
            ...
            BCM_GPORT_LOCAL_SET(record_entries[j].egress_gport, dgpp);
            SOC_IF_ERROR_RETURN(soc_port_pipe_get(unit, dgpp, &pipe));

            sgpp = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, SGPP_SVPf);
            sgpp &= _TCB_SGPP_MASK;
            BCM_GPORT_LOCAL_SET(record_entries[j].ingress_gport, sgpp);

            qnum = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, QNUMf);
            BCM_IF_ERROR_RETURN
                (_bcm_cosq_tcb_queue_resolve(unit, pipe, qnum, &queue));
            cosq = queue % si->port_num_cosq[dgpp];
            BCM_GPORT_UCAST_QUEUE_GROUP_SYSQID_SET(record_entries[j].queue,
                                                   dgpp, cosq);
        ...
        for(i = 0; i < num; i++) {
            ...
            dgpp = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, DGPPf);
            BCM_GPORT_LOCAL_SET(record_entries[i].egress_gport, dgpp);
            SOC_IF_ERROR_RETURN(soc_port_pipe_get(unit, dgpp, &pipe));

            sgpp = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, SGPP_SVPf);
            sgpp &= _TCB_SGPP_MASK;
            BCM_GPORT_LOCAL_SET(record_entries[i].ingress_gport, sgpp);

            qnum = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, QNUMf);
            BCM_IF_ERROR_RETURN
                (_bcm_cosq_tcb_queue_resolve(unit, pipe, qnum, &queue));
            cosq = queue % si->port_num_cosq[dgpp];
            BCM_GPORT_UCAST_QUEUE_GROUP_SYSQID_SET(record_entries[i].queue,
                                                   dgpp, cosq);
                                 ...
                                 int num, bcm_cosq_tcb_event_t *record_entries)
{
    ...
    int idx;
    bcm_cos_queue_t cosq;
    int dgpp, sgpp, pipe, qnum, queue, drop_reason = 0, drop_vector, depth, cell_bytes;
    soc_info_t *si;
    _bcm_cosq_tcb_unit_ctrl_t *tcb_unit_info;
    ...
    for (idx = 0; idx < num; idx++) {
        ...
        BCM_GPORT_LOCAL_SET(record_entries[idx].egress_gport, dgpp);
        SOC_IF_ERROR_RETURN(soc_port_pipe_get(unit, dgpp, &pipe));

        sgpp = soc_TCB_EVENT_BUFFERm_field32_get(unit, ped, SGPP_SVPf);
        sgpp &= _TCB_SGPP_MASK;
        BCM_GPORT_LOCAL_SET(record_entries[idx].ingress_gport, sgpp);

        qnum = soc_TCB_EVENT_BUFFERm_field32_get(unit, ped, QNUMf);
        BCM_IF_ERROR_RETURN
            (_bcm_cosq_tcb_queue_resolve(unit, pipe, qnum, &queue));
        cosq = queue % si->port_num_cosq[dgpp];
        BCM_GPORT_UCAST_QUEUE_GROUP_SYSQID_SET(record_entries[idx].queue,
                                               dgpp, cosq);
                                    ...
                                    bcm_cosq_tcb_config_t *config)
{
    ...
    int type, entity_num, index, pipe, id, propability;
    ...
    switch(type) {
        case _TCB_SCOPE_UCQ:
            ...
            entity_num = soc_reg64_field32_get(unit, TCB_CONFIGr, rval64,
                                               SCOPE_ENTITYf);
            index = (entity_num) < 708 ? 0 : 1;
            pipe = _bcm_th2_cosq_tcb_pipe_table[buffer_id].epipe_member[index];
            id = (entity_num - _bcm_th2_cosq_tcb_ucq_offset[pipe]) %
                  _BCM_TH2_NUM_UCAST_QUEUE_PER_PORT;
            local_mmu_port = (entity_num - _bcm_th2_cosq_tcb_ucq_offset[pipe]) /
                             _BCM_TH2_NUM_UCAST_QUEUE_PER_PORT;
            ...
            BCM_GPORT_UCAST_QUEUE_GROUP_SYSQID_SET(config->gport,
                                                   local_port, id);
            break;
