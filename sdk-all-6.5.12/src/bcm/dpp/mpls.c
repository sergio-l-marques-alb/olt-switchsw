    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if(!eep_alloced) {
            ...
            if (SOC_IS_JERICHO(unit))
            {
                ...
                }else if ( SOC_IS_JERICHO_PLUS(unit) && label_array[indx].encap_access > bcmEncapAccessInvalid) {
                    /*
                     * User could indicate the bank pahse explicitly now
                     * by setting encap_access other than bcmEncapAccessInvalid
                     * bcmEncapAccessTunnel1 : native link layer(1)
                     * bcmEncapAccessTunnel2 : MPLS tunnel 1(3)
                     * bcmEncapAccessTunnel3 : MPLS tunnel 2(4)
                     */
                    local_out_lif_info.app_alloc_info.application_type = (label_array[indx].encap_access == bcmEncapAccessTunnel1)?bcm_dpp_am_egress_encap_app_third_mpls_tunnel:
                                                                                               (label_array[indx].encap_access == bcmEncapAccessTunnel2)?bcm_dpp_am_egress_encap_app_second_mpls_tunnel:bcm_dpp_am_egress_encap_app_mpls_tunnel;
                }else {
                    ...
                    if (allocate_egress) {
                        local_out_lif_info.app_alloc_info.pool_id = dpp_am_res_eep_mpls_tunnel;
                         if ((mpls_port->flags & BCM_MPLS_PORT_EXTENDED ) 
                            ...
                            local_out_lif_info.app_alloc_info.application_type = bcm_dpp_am_egress_encap_app_third_mpls_tunnel;
                        } else if ( SOC_IS_JERICHO_PLUS(unit) && mpls_port->egress_label.encap_access > bcmEncapAccessInvalid) { 
                         /*
                          * User could indicate the bank pahse explicitly now
                          * by setting encap_access other than bcmEncapAccessInvalid
                          * bcmEncapAccessTunnel1 : native link layer(1)
                          ...
                          * bcmEncapAccessTunnel3 : MPLS tunnel 2(4)
                          */
                          local_out_lif_info.app_alloc_info.application_type = (mpls_port->egress_label.encap_access == bcmEncapAccessTunnel1)?bcm_dpp_am_egress_encap_app_third_mpls_tunnel:
                                                                                               (mpls_port->egress_label.encap_access == bcmEncapAccessTunnel2)?bcm_dpp_am_egress_encap_app_second_mpls_tunnel:bcm_dpp_am_egress_encap_app_mpls_tunnel;                      
                        } else {
