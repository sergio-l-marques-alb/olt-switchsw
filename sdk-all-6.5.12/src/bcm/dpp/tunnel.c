 ...
 )
{
    ...
    switch(tunnel_type) {
        ...
        case bcmTunnelTypeIp4In4:
        case bcmTunnelTypeIp6In4:
        /* these are not supported as for tunnel into IPv6, user should use anyIn6*/
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if (lif_usage != _bcmDppLifTypeVxlan) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Next EEP is not VXLAN")));
    }
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    if ((lif_usage != _bcmDppLifTypeIpTunnel) &&
        !(SOC_DPP_CONFIG(unit)->pp.lawful_interception_enable && ((lif_usage == _bcmDppLifTypeVxlan) || (lif_usage == _bcmDppLifTypeLawfulInterception))) &&
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if (update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if ((SOC_DPP_CONFIG(unit)->pp.explicit_null_support && (SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id == global_lif)) ||
                ...
                /*If counter profile was allocated Check validity and copy to local inif info*/
                if (counting_profile_is_allocated){
...
int _bcm_petra_tunnel_terminator_config_action_verify(int unit, bcm_tunnel_terminator_config_key_t *config_key, bcm_tunnel_terminator_config_action_t *config_action) {
    ...
    BCMDNX_IF_ERR_EXIT(rv);
    if (is_ipv6_tunnel) {
        if (soc_sand_msb_bit_on(config_action->tunnel_class) >= 64) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("tunnel class value (%d) is invalid, max nof bits available: %d"),config_action->tunnel_class, 64));
        }
    } else {
