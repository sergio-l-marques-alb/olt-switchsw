    ...
    bcm_mim_vpn_config_t *info)
{
    ...
    bcm_dpp_am_local_inlif_info_t local_inlif_info;    
    uint8 sw_state_is_ingress, sw_state_is_egress, sw_state_is_valid;
    ...
    if (info->flags & BCM_MIM_VPN_BVLAN) { /* B-VID */
      ...
      info->vpn = _BCM_DPP_VLAN_TO_BVID(unit, info->vpn);
        ...
        if (sw_state_is_valid && !update) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_MIM_VPN_REPLACE flag is not set for a VPN that already exists (either in INGRESS or EGRESS)")));
        }
    ...
    bcm_mim_vpn_config_t *info)
{
    ...
    uint8 found;
    uint8 is_wide_entry;
    uint8 sw_state_is_ingress, sw_state_is_egress;
    uint32 sw_state_isid;
    uint8 is_ingress, is_egress;
    uint32 mim_info_flags;
