                          ...
                          int entries)
{
    ...
    if (groupData.groupFlags & _BCM_DPP_FIELD_GROUP_IN_USE) {
        ...
        if ((BCM_E_NONE == result) && entries) {
            ...
            if((groupData.groupFlags & _BCM_DPP_FIELD_GROUP_LARGE) && (groupData.grpMode ==  bcmFieldGroupModeDirect)) {
                LOG_CLI((BSL_META_U(unit,
                                    "%s  Entry information is not available for entries located in KAPS memory.\n"), prefix));
            } else {
                switch (entryType) {
                case _bcmDppFieldEntryTypeDirExt:
