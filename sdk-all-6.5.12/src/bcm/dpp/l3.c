    ...
    bcm_l3_intf_t *intf)
{
  ...
  if(!(intf->l3a_flags & BCM_L3_TRILL_ONLY)) {
  ...
  )
{
  ...
  BCMDNX_INIT_FUNC_DEFS;
      ...
      if(ARAD_KBP_ENABLE_ANY_IPV4UC_PROGRAM && ARAD_KBP_24BIT_FWD) {
    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id) {
        int ip_dummy_lif_inited = 0;
        /*
          * In JER, explicit null wa and IP tunnel share the dummy ip LIF
          */
        if (SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->pp.explicit_null_support) {
            ip_dummy_lif_inited = 1;
        }
        if (!ip_dummy_lif_inited) {
