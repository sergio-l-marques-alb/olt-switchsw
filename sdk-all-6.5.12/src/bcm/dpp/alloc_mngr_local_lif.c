...
static int failover_linked_list_member_add(int unit, int linked_list, int lif_id,  uint8 is_even)
{
  ...
  if (linked_list >= max_nof_lif_linked_lists)
  {
    BCMDNX_ERR_EXIT_MSG(
      ...
      __FUNCTION__,linked_list,max_nof_lif_linked_lists)) ;
  }
  ...
  if (lif_id >= max_nof_lif_linked_list_members)
  {
    BCMDNX_ERR_EXIT_MSG(
      ...
      __FUNCTION__,lif_id,max_nof_lif_linked_list_members)) ;
  }
  /*
  ...
  if (lif_id_handle)
  {
    BCMDNX_ERR_EXIT_MSG(_SHR_E_FULL,(_BSL_BCM_MSG("%s(): This member (lif %d) is already in use!" ),__FUNCTION__,lif_id)) ;
  }
  /*
...
_bcm_dpp_am_local_inlif_is_pack_empty(int unit,uint8 bank_id, uint8 flags, int elem, uint8 *is_pack_empty)
{
    ...
    if ( is_wide_lif )
    {
        /*special 6-pack: the last one includes only 1-Wide LIF and 2-common LIFs*/
        if ( elem_in_bank == _BCM_DPP_AM_INGRESS_LIF_NOF_WIDE_LIFS_IN_BANK - 1) 
        {
            *is_pack_empty = TRUE;
        }
        else
...
_bcm_dpp_am_local_outlif_internal_dealloc(int unit, int local_lif)
{
    ...
    if ( eedb_bank_info.nof_free_entries == _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK(unit) ) 
    {
        eedb_bank_info.valid = FALSE;
...
_bcm_dpp_am_local_outlif_bank_alloc(int unit, int flags, bcm_dpp_am_egress_encap_alloc_info_t *app_alloc_info, uint8 tag, uint8 ext_type,int failover_id, int *bank_id , uint8 *bank_found  )
{
    ...
    for (bank_id_iter = 0 ; ( bank_id_iter < _BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_HALF_BANKS(unit) ) && (*bank_found == FALSE )  ; bank_id_iter++) 
    {
...
_bcm_dpp_am_local_outlif_counting_profile_set(int unit, int counting_profile_id, int base, int size, int double_entry)
{
    ...
    if(base >= SOC_DPP_CONFIG(unit)->l3.nof_rifs)
    {
        ...
        end_bank_id =  (local_base+size)/ SOC_DPP_DEFS_GET(unit, nof_eedb_lines_per_bank);
