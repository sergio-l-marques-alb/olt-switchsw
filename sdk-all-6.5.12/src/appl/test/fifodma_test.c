...
fifodma_debug_print_fifo_info(int unit, char *msg)
{
    ...
    if (fifodma_data.param_debug) {
        ...
        cli_out("\n[%s] valid_entries %d", msg, num_entries);
        cli_out("\n[%s] wr_ptr %d", msg, COMPILER_64_LO(wr_ptr));
        cli_out("\n[%s] rd_ptr %d\n", msg, COMPILER_64_LO(rd_ptr));
    }
}
...
/*         while ((num_valid_entries != fifo_depth ||
                wr_ptr != rd_ptr) &&
               (timeout-- > 0)) { */
        while (COMPILER_64_NE(wr_ptr, rd_ptr) && (timeout-- > 0)) {
            (void) soc_reg_get(unit, L2_MOD_FIFO_WR_PTRr, 0, 0, (&wr_ptr));
            (void) soc_reg_get(unit, L2_MOD_FIFO_RD_PTRr, 0, 0, (&rd_ptr));
            time1 = sal_time_usecs();
            if (timeout % 2 == 0) {
...
int fifodma_test_init(int unit, args_t *a, void **pa)
{
    ...
    cli_out("\nL2_MOD_FIFOm, depth = %d", fifodma_data.fifo_depth);
    cli_out("\nL2_MOD_FIFOm, width = %d", fifodma_data.fifo_width);
    cli_out("\nhost_mem_base = %p", fifodma_data.host_base);
    cli_out("\n");
