...
STATIC cmd_result_t
cmd_sand_dma_get_size(int unit, args_t *a, int *size)
{
    char *sz;
...
STATIC cmd_result_t
cmd_sand_dma_get_count(int unit, args_t *a, int *count)
{
    char *arg;
    ...
    if ((arg = ARG_GET(a)) == NULL &&
        (arg = var_get(CMD_SAND_DMA_COUNT)) == NULL) {
        return CMD_FAIL;
    }
...
STATIC void
cmd_sand_dma_set_count_size(uint32 count, uint32 size)
{
...
STATIC cmd_result_t
cmd_sand_dma_get_laddr(int unit, args_t *a, void **laddr)
{
    char *arg;
    ...
    if ((arg = ARG_GET(a)) == NULL &&
        (arg = var_get(CMD_SAND_DMA_LADDR)) == NULL) {
        return CMD_FAIL;
    }
...
STATIC void
cmd_sand_dma_set_laddr(void *laddr)
{
    char buf[20];

    sal_sprintf(buf, "0x%08x", PTR_TO_INT(laddr));
    var_set(CMD_SAND_DMA_LADDR, buf, TRUE, FALSE);
}

STATIC cmd_result_t
cmd_sand_dma_get_paddr(int unit, args_t *a, uint32 *paddr)
{
    char *arg;
    ...
    /* Parse the logical address */
    if ((arg = ARG_GET(a)) == NULL &&
        (arg = var_get(CMD_SAND_DMA_PADDR)) == NULL) {
        return CMD_FAIL;
    }
...
STATIC void
cmd_sand_dma_set_paddr(uint32 paddr)
{
    char buf[20];
...
STATIC cmd_result_t
cmd_sand_dma_alloc(int unit, args_t *a)
{
    char *arg;
    int   size;
    int   count;
   ...
   /* Get the region name if provided */
    ...
    cli_out("Allocated %d bytes of DMA-able memory at address 0x%08x\n",
            alloc_count, PTR_TO_INT(laddr));
...
STATIC cmd_result_t
cmd_sand_dma_free(int unit, args_t *a)
{
    void *laddr;
...
STATIC cmd_result_t
cmd_sand_dma_l2p(int unit, args_t *a)
{
    void  *laddr;
    uint32 paddr;
...
STATIC cmd_result_t
cmd_sand_dma_p2l(int unit, args_t *a)
{
    uint32  paddr;
    void   *laddr;
...
STATIC cmd_result_t
cmd_sand_dma_flush(int unit, args_t *a)
{
    int   size;
    int   count;
    void *laddr;
...
STATIC cmd_result_t
cmd_sand_dma_inval(int unit, args_t *a)
{
    int   size;
    int   count;
    void *laddr;
...
STATIC cmd_result_t
cmd_sand_dma_load(int unit, args_t *a)
{
    void  *laddr_v;
    uint8 *laddr;
    char *cp;
...
STATIC cmd_result_t
cmd_sand_dma_fill(int unit, args_t *a)
{
    char   *arg;
    int     size;
    void   *laddr;
...
STATIC cmd_result_t
cmd_sand_dma_edit(int unit, args_t *a)
{
    void   *laddr_v;
    uint8  *laddr;
    int     size;
    ...
    for (;;) {
        sal_sprintf(prompt, "0x%08x ", PTR_TO_INT(laddr));
        switch (size) {
        case 1:
            val = *(uint8 *)laddr;
...
cmd_result_t
cmd_sand_dma(int unit, args_t *a)
{
    char    *subcmd;
