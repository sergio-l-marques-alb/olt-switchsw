...
flexport_show(int unit, flexport_info_t *f)
{
    ...
    for (pm = 0; pm < PM_NUM(unit) ; pm++) {
        ...
        cli_out("PM%d:\n", pm);
        phy_base = (pm * MAX_PORTS_PER_PBLK) + 1;
        for(i = 0; i < MAX_PORTS_PER_PBLK; i++) {
            phy_port = phy_base + i;
...
flexport_parse_pm_mode(int unit, flexport_info_t *f, int *auto_assign)
{
    ...
    for (pm = 0; pm < PM_NUM(unit); pm++) {
        ...
        } else if(!sal_strncasecmp(s, "1x25G+1x50G", strlen(s)) ||
                  !sal_strncasecmp(s, "1xHG[27]+1xHG[53]", strlen(s)) ||
                  !sal_strncasecmp(s, "1xHG[25]+1xHG[50]", strlen(s))) {
            req_cnt = 2;
            for (i = 0; i < req_cnt; i++) {
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[i].port = pi[pm].ports[i];
        ...
        } else if(!sal_strncasecmp(s, "1x50G+1x25G", strlen(s)) ||
                  !sal_strncasecmp(s, "1xHG[53]+1xHG[27]", strlen(s)) ||
                  !sal_strncasecmp(s, "1xHG[50]+1xHG[25]", strlen(s))) {
            req_cnt = 2;
            for (i = 0; i < req_cnt; i++) {
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[i].port = pi[pm].ports[i];
        ...
        }else if(!sal_strncasecmp(s, "2x25G", strlen(s)) ||
                  !sal_strncasecmp(s, "2xHG[27]", strlen(s)) ||
                  !sal_strncasecmp(s, "2xHG[25]", strlen(s))) {
            req_cnt = 2;
            for (i = 0; i < req_cnt; i++) {
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[i].port = pi[pm].ports[i];
        ...
        } else if(!sal_strncasecmp(s, "4x1G", strlen(s))) {
            req_cnt = 4;
            for (i = 0; i < req_cnt; i++) {
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[i].port = pi[pm].ports[i];
        ...
        } else if(!sal_strncasecmp(s, "4x2.5G", strlen(s))) {
            req_cnt = 4;
            for (i = 0; i < req_cnt; i++) {
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[i].port = pi[pm].ports[i];
