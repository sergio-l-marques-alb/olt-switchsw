		  ...
		  uint32 *mask, char *mod, int incr)
{
    ...
    if (key_type_fid || data_type_fid) {
        if (!key_type) {
            ...
            rv = soc_flow_db_mem_to_view_id_get (unit, mem, 
                    key_type, data_type, 0, NULL, (uint32 *)&mem_view_id);
        }
	...
	if (!sal_strcasecmp(fmod, "clear")) {
        ...
        for (field_type = SOC_FLOW_DB_FIELD_TYPE_CONTROL;
            ...
            rv = soc_flow_db_mem_view_field_list_get(unit, mem_view_id,
                ...
                for (ix = 0; ix < cnt; ix++) {
                    if (field_type == SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY ||
                        ...
                        (void) soc_flow_db_mem_view_field_name_get(unit, 
                                   mem_view_id, field_id[ix], &field_name);
                    }
                    else {
        ...
        if (field_found == TRUE) {
            rv = soc_flow_db_mem_view_field_info_get(unit,mem_view_id,
...
char cmd_esw_ser_usage[] =
    ...
    "pipe0|pipe1|pipe2|pipe3] [Xor_bank=0|1]\n"
