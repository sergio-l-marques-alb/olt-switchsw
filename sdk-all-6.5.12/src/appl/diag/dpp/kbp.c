...
cmd_dpp_kbp_rop_counter(int unit)
{
    ...
    LOG_CLI((BSL_META_U(unit,
                        "\nInterlaken-ROP Statistics[%s]  \n"),(ARAD_KBP_IS_OP ? "Read clear" : "Accumulate")));
    LOG_CLI((BSL_META_U(unit,
                        "--------------------------------------------------------------------------------\n")));
    if ( ARAD_KBP_IS_OP ) {
        mdio_id = SOC_DPP_CONFIG(unit)->arad->init.elk.kbp_mdio_id[0];
        ...
        for (core = 0;core < num_of_cores; core++) {
            mdio_id = SOC_DPP_CONFIG(unit)->arad->init.elk.kbp_mdio_id[core];
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_kbp_print_rop_counter()", (ARAD_KBP_IS_OP ? kbp_port: core),
                                 mdio_id);
}
...
dpp_kbp_init_kbp_transport(int unit, uint32 core)
{
    ...
    if ((elk->is_master) && (AradAppData[unit] == NULL)) {
    ...
    if (ARAD_KBP_IS_OP) {
        /* Skip the second core init for OP as we connect with only one KBP */
        if (core == 1) {
            AradAppData[unit]->alg_kbp_xpt_p[1] = AradAppData[unit]->alg_kbp_xpt_p[0];
            SOC_EXIT;
        }
        if (elk->is_master) {
            status = kbp_pcie_init(KBP_DEVICE_OP, flags,
...
STATIC int test_kbp_entry_add(int unit, uint32 table_id, uint32 prefix_len, uint8 *data, uint8 *asso_data)
{
    ...
    if(db_entry == NULL) { /* create */
        ...
        kbp_db_add_prefix(db_p, data, prefix_len, &db_entry);
        ...
        kbp_entry_get_ad(db_p, db_entry, &ad_entry);
        kbp_ad_db_update_entry(ad_db_p, ad_entry, asso_data);
    }
