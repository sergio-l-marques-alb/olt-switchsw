...
#if defined(BCM_MONTEREY_SUPPORT)
        if (!(IS_CPRI_PORT(unit, port)) && SOC_IS_MONTEREY(unit)) {
...
#endif /* BCM_SABER2_SUPPORT */
#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TOMAHAWKPLUS_SUPPORT) || defined (BCM_TOMAHAWK2_SUPPORT)
            ...
            if (!(non_dma->flags & _SOC_COUNTER_NON_DMA_VALID) &&
                ...
                if ((ctrl_info.instance_type == SOC_CTR_INSTANCE_TYPE_POOL) ||
                    ...
                    if ((non_dma->mem == MMU_CTR_COLOR_DROP_MEMm) && 
                        !(SOC_IS_TOMAHAWK2(unit) || (SOC_IS_TRIDENT3(unit)))) {
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    max_pipe = soc_property_get(unit, "num_pipe", NUM_PIPE(unit)); /* store locally */
