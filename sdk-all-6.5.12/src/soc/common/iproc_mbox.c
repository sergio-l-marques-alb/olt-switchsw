...
int soc_iproc_mbox_init(int unit)
{
    ...
    for (ucnum = 0; ucnum < max_ucores; ucnum++) {
        ...
        mbox_base = soc_iproc_mbox_base_get(ucnum);
        mbox_size = soc_iproc_mbox_size_get(ucnum) / max_mboxs;
    ...
    for (ucnum = 0; ucnum < max_ucores; ucnum++) {
        ...
        soc_m0ssq_sram_write32(unit, (U0_MBOX_BASE_OFFSET + (ucnum * 8)), (M0SSQ_SHMEM_MBOX_BASE + (ucnum * M0SSQ_SHMEM_MBOX_SIZE)));
        soc_m0ssq_sram_write32(unit, (U0_MBOX_SIZE_OFFSET + (ucnum * 8)), M0SSQ_SHMEM_MBOX_SIZE);
    }
...
int soc_iproc_data_send_nowait(soc_iproc_mbox_info_t *chan, soc_iproc_mbox_msg_t *msg)
{
    ...
    while (sidx < nentries) {
        soc_iproc_mbox_write(unit, ((unsigned long)&dst[didx] - soc_iproc_sram_membase_get()), src[sidx]);
        didx++;
        sidx++;
...
int soc_iproc_data_recv(soc_iproc_mbox_info_t *chan, soc_iproc_mbox_msg_t **msg)
{
    ...
    didx = 0;
    while (didx < nentries) {
        dst[didx] = soc_iproc_mbox_read(unit, ((unsigned long)&src[sidx] - soc_iproc_sram_membase_get()));
        didx++;
        sidx++;
...
int soc_iproc_data_send_wait(soc_iproc_mbox_info_t *chan, soc_iproc_mbox_msg_t *cmd, soc_iproc_mbox_msg_t *resp)
{
    ...
    dst = (uint32 *)resp;
    while (count) {
        if ((soc_iproc_mbox_read(unit, (unsigned long)&src[cmdidx] - soc_iproc_sram_membase_get()) & IPROC_RESP_READY) != 0) {
            break;
        }
