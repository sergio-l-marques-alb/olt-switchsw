...
soc_cmicx_miim_divider_set_ring(int unit, int ring_index, int int_divider, int ext_divider, int out_delay)
{
    ...
    if ((ring_index < CMICX_MIIM_RING_INDEX_START) ||
        (ring_index > CMICX_MIIM_RING_INDEX_END)) {
        LOG_ERROR(BSL_LS_SOC_MIIM, (BSL_META_U(unit, "Invalid CMICx MIIM Ring index %d\n"), ring_index));
        return SOC_E_PARAM;
    }

    reg_addr = soc_reg_addr(unit, ring_control_reg[ring_index], REG_PORT_ANY, 0);
    soc_iproc_getreg(unit, reg_addr, &reg_val);

    if (int_divider != -1) {
        soc_reg_field_set(unit, ring_control_reg[ring_index], &reg_val, CLOCK_DIVIDER_INTf, int_divider);
    }

    if (ext_divider != -1) {
        soc_reg_field_set(unit, ring_control_reg[ring_index], &reg_val, CLOCK_DIVIDER_EXTf, ext_divider);
    }

    if (out_delay != -1) {
        soc_reg_field_set(unit, ring_control_reg[ring_index], &reg_val, MDIO_OUT_DELAYf, out_delay);
    }
...
soc_cmicx_miim_divider_set_all(int unit, int int_divider, int ext_divider, int out_delay)
{
    ...
    for (ring_idx = CMICX_MIIM_RING_INDEX_START; ring_idx <= CMICX_MIIM_RING_INDEX_END; ring_idx++) {
...
/** see .h file   */
int
soc_cmicx_miim_operation(int unit, int is_write, int clause, uint32 phy_id,
                  uint32 phy_reg_addr, uint16 *phy_data)
{
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
        if (is_write && dnxc_is_hw_access_disabled(unit))
        {
    ...
    */
    ...
    if (clause45) {
        ...
        /* For CL22 devices, bits [4:0] represent the REGAD */
