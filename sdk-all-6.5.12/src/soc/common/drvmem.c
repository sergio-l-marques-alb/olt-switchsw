                     ...
                     uint32 *frag_field)
{
    ...
    mask = ((uint32)0xFFFFFFFF) >> (31 - bit_end);
    usr_field[word_start + last_idx] &= mask;
    return SOC_E_NONE;
}
                  ...
                  uint32 *fldbuf)
{
    ...
    uint32 val_field[SOC_MAX_MEM_WORDS];
    ...
    sal_memset(&sf_info,0, sizeof(sf_info));
    rv = soc_mem_view_split_field_info_get(unit,mem_view,field, &sf_info);
    
    if (SOC_SUCCESS(rv)) {
        rv = _soc_mem_view_sub_field_get(fldbuf,
             0, sf_info.offset - 1, val_field);
        if (rv != SOC_E_NONE) {
            return;
        }
        soc_meminfo_fieldinfo_field_set(entbuf, meminfo, &sf_info.field[0], val_field);

        rv = _soc_mem_view_sub_field_get(fldbuf,
             sf_info.offset, sf_info.width - 1, val_field);
        if (rv != SOC_E_NONE) {
            return;
        }
        soc_meminfo_fieldinfo_field_set(entbuf, meminfo, &sf_info.field[1], val_field);
                  ...
                  soc_field_t field, uint32 *fldbuf, uint32 words)
{
    ...
    soc_mem_info_t *meminfo;
    uint32 val_field[SOC_MAX_MEM_WORDS];
    uint32 val_field1[SOC_MAX_MEM_WORDS];
    soc_mem_view_split_field_info_t sf_info;  /* split field info */
    int i;
    int fld_word_len;
    ...
    sal_memset(&sf_info, 0x0, sizeof(sf_info));
    rv = soc_mem_view_split_field_info_get(unit,mem_view,field,&sf_info);

     if (SOC_SUCCESS(rv)) {

        meminfo = &SOC_MEM_INFO(unit, phy_mem);
        (void)soc_meminfo_field_get_helper(meminfo, entbuf, &sf_info.field[0], val_field, words);
        
        (void)soc_meminfo_field_get_helper(meminfo, entbuf, &sf_info.field[1], val_field1, words);
        ...
        rv = _soc_mem_view_sub_field_set(fldbuf, 0, 
                         sf_info.offset - 1, val_field);
        if (rv != SOC_E_NONE) {
            return fldbuf;
        }
        rv = _soc_mem_view_sub_field_set(fldbuf, sf_info.offset,
                        sf_info.width - 1, val_field1);
        if (rv != SOC_E_NONE) {
            return fldbuf;
        }
