...
soc_ramon_stat_is_supported_type(int unit, bcm_port_t port, bcm_stat_val_t type)
{
    ...
    case snmpBcmRxRsFecBitError:
    case snmpBcmRxRsFecSymbolError:
        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_control_pcs_get, (unit, port, &pcs)));

        if(soc_dnxc_port_pcs_64_66_ll_rs_fec != pcs &&
           soc_dnxc_port_pcs_64_66_rs_fec != pcs &&
           soc_dnxc_port_pcs_64_66_15t_ll_rs_fec != pcs &&
           soc_dnxc_port_pcs_64_66_15t_rs_fec != pcs)
        {
...
int soc_ramon_controlled_counter_get(int unit, int counter_id, int port, uint64* val){
...
soc_ramon_stat_get(int unit,bcm_port_t port,uint64 *value,int *counters,int arr_size)
{
    ...
    for (i=0;i<arr_size;i++)
    {
        if (COUNTER_IS_COLLECTED(SOC_CONTROL(unit)->controlled_counters[counters[i]])) {
            ...
            /*counter isn't collected by counter thread*/
            rc = soc_ramon_controlled_counter_get(unit, counters[i], port, &val64);
        }
        SHR_IF_ERR_EXIT(rc);
