...
/* This function is called by the API bcm_switch_control_indexed_set with value of bcmSwitchMplsSpeculativeNibbleMap for key.
 * Its purpose is to change the expected next protocol after the MPLS BOS label for specific nibble value.
 * In order to do that it accepts a nibble value and a value from a predefined enumeration bcm_switch_mpls_next_protocol_t
 * with the possible values for the next protocol field.
 */
int
...
/* This function is called by the API bcm_switch_control_indexed_get with value of bcmSwitchMplsSpeculativeNibbleMap for key.
 * Its purpose is to get the expected next protocol after the MPLS BOS label for specific nibble value.
 * In order to do that it returns a value from a predefined enumeration bcm_switch_mpls_next_protocol_t
 * with the value for next protocol field for a given nibble value.
 */
int
