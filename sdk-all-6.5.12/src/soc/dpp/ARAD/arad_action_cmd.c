  ...
  )
{
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        /* If counter is not invalid perform the same between OW and update */
        if (info->cmd.counter_ptr_1.value == SOC_TMC_ACTION_CMD_SNOOP_MIRROR_INVALID_COUNTER) {
            /* Counting is disabled */
  ...
  )
{
  uint32
    ...
    if (!SOC_IS_QAX(unit)) { /* QAX does not support configuring an ingress MC destination in snoop & mirror profiles */ 
    ...
    {
        /* Get the information from table CGM_SNP_MRR_TM_TABLEm */
        ...
        {
            /* If both IDs are set to BCM_MIRROR_DEST_NO_COUNT, set the STATISTICS_PTR_OW to 0 */
  ...
  )
{
    ...
    if (SOC_IS_QAX(unit))
    {
        /* Get the data from memory CGM_SNP_MRR_TM_TABLE */
        ...
        if (stat_ptr_ow == 0)
        {
            /* If statistic interface is disabled, set the IDs to BCM_MIRROR_DEST_NO_COUNT */
            info->ext_stat_id[0]=BCM_MIRROR_DEST_NO_COUNT;
            info->ext_stat_id[1]=BCM_MIRROR_DEST_NO_COUNT;
        }
        else
        {
            /* If statistic interface is enabled, get the IDs from the memory */
