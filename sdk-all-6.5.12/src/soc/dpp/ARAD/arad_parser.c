...
typedef enum
{
    ...
    ARAD_PARSER_DYNAMIC_MACRO_UDP_PTP,
...
static char *parser_dynamic_macro[ARAD_PARSER_DYNAMIC_MACRO_COUNT] =
{
    ...
    "UDP-PTP",    /* [ARAD_PARSER_DYNAMIC_MACRO_UDP_PTP] = */
...
{   /* SW PFC,                  Name              HW PFC,                         HW PFC MASK,                    IS_PROTO    L4_LOC, Sec,                     hdrtype, VTT,                            PMF                             PLC Profile*/
    ...
    { DPP_PFC_ETH_IPV4_ETH,     "ETHoIPv4oETH",   ARAD_PARSER_PFC_ETH_IPV4_ETH,   ARAD_PARSER_PFC_MATCH_ONE,      IS_ETH | IS_IPV4, 4, DPP_PFC_TYPE_PRIMARY,   0x00121, ARAD_PARSER_PFC_ETH_IPV4_ETH,   ARAD_PARSER_PFC_ETH_IPV4_ETH,   DPP_PLC_PROFILE_PP},
    { DPP_PFC_ETH_TRILL_ETH,    "ETHoTRILLoETH",  ARAD_PARSER_PFC_ETH_TRILL_ETH,  ARAD_PARSER_PFC_MATCH_ONE,      IS_ETH | IS_TRILL,4, DPP_PFC_TYPE_PRIMARY,   0x00151, ARAD_PARSER_PFC_ETH_TRILL_ETH,  ARAD_PARSER_PFC_ETH_TRILL_ETH,  DPP_PLC_PROFILE_PP},
    { DPP_PFC_ETH_MPLS1_ETH,    "ETHoMPLSx1oETH", ARAD_PARSER_PFC_ETH_MPLS1_ETH,  ARAD_PARSER_PFC_MATCH_ONE,      IS_ETH | IS_MPLS, 4, DPP_PFC_TYPE_SECONDARY, 0x00141, ARAD_PARSER_PFC_NOF,            ARAD_PARSER_PFC_ETH_MPLS1_ETH,  DPP_PLC_PROFILE_PP},
    { DPP_PFC_ETH_MPLS2_ETH,    "ETHoMPLSx2oETH", ARAD_PARSER_PFC_ETH_MPLS2_ETH,  ARAD_PARSER_PFC_MATCH_ONE,      IS_ETH | IS_MPLS, 4, DPP_PFC_TYPE_SECONDARY, 0x01441, ARAD_PARSER_PFC_NOF,            ARAD_PARSER_PFC_ETH_MPLS2_ETH,  DPP_PLC_PROFILE_PP},
    { DPP_PFC_ETH_MPLS3_ETH,    "ETHoMPLSx3oETH", ARAD_PARSER_PFC_ETH_MPLS3_ETH,  ARAD_PARSER_PFC_MATCH_ONE,      IS_ETH | IS_MPLS, 4, DPP_PFC_TYPE_SECONDARY, 0x14441, ARAD_PARSER_PFC_NOF,            ARAD_PARSER_PFC_ETH_MPLS3_ETH,  DPP_PLC_PROFILE_PP},
...
#ifdef ENABLE_LATER
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 144, exit, WRITE_IHP_FLP_PFC_IS_ETHr (unit, REG_PORT_ANY, eth_val));
    ...
    *next_addr_base = info_entry->next_addr_base;
    ...
    for (index = 0; index < tbl_size; index++) 
    {
               ...
               || static_to_dynamic_table[index].include_prog_end)) /* Cond 3 */
        {
            ...
            /* Special cases for UDPoIPV4 - operates VxLAN or UDP_Tunnel or PTP macro first
             * and then continues to UDP custom macro.
             * priority: VxLAN > UDP_Tunnel > PTP > basic UDP custom marco
             * If VxLAN/UDP_tunnel/PTP are all not enabled, operate UDP macro immediately after IPv4.
             */
            if ((ARAD_PARSER_PROG_IPV4oE_BASE_START == static_to_dynamic_table[index].instruction_addr)
                ...
                && (ARAD_PARSER_CUSTOM_MACRO_INVALID == dynamic_macro[unit][app_index])) /* if no VxLAN */
            {
                ...
                } else if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ptp_udp_parser_enable", 0)) {
                    app_index = ARAD_PARSER_DYNAMIC_MACRO_UDP_PTP;
                    *next_addr_base = ARAD_PARSER_PROG_UDPoIPV4oE_BASE;
                }
                else if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop || SOC_DPP_CONFIG(unit)->pp.limited_gtp_parsing ){
  ...
  )
{
  ...
  /* Configure PTP UDP Custom Macro is configured */
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ptp_udp_parser_enable", 0))
  {
      res = arad_parser_dynamic_macro_allocate(unit, ARAD_PARSER_DYNAMIC_MACRO_UDP_PTP, &custom_macro);
      SOC_SAND_CHECK_FUNC_RESULT(res, 145, exit);

      ihp_parser_custom_macro_parameters_tbl_data.cstm_word_select       = 0;           /* source_port(2B) + dest_port(2B)*/
      ihp_parser_custom_macro_parameters_tbl_data.cstm_mask_left         = 16;          /* mask source_port */
      ihp_parser_custom_macro_parameters_tbl_data.cstm_mask_right        = 0;
      ihp_parser_custom_macro_parameters_tbl_data.cstm_comparator_mode   = 1;           /* Compare 16 lsb of selected data to 16 lsb of comparator values */
      ihp_parser_custom_macro_parameters_tbl_data.cstm_comparator_mask   = 0xc00;       /* compare to entries 11 and 12 */
      ihp_parser_custom_macro_parameters_tbl_data.cstm_condition_select  = 0;
      ihp_parser_custom_macro_parameters_tbl_data.cstm_shift_sel         = 0;
      ihp_parser_custom_macro_parameters_tbl_data.cstm_shift_a           = 8;           /* UDP size is 8B */
      ihp_parser_custom_macro_parameters_tbl_data.cstm_shift_b           = 8;           /* UDP size is 8B */
      ihp_parser_custom_macro_parameters_tbl_data.augment_previous_stage = 0;           /* visible stage */
      ihp_parser_custom_macro_parameters_tbl_data.qualifier_shift        = 0xf;         /* Is-Range (1), Range-or-Protocol (4) */
      ihp_parser_custom_macro_parameters_tbl_data.qualifier_or_mask      = 0;
      ihp_parser_custom_macro_parameters_tbl_data.qualifier_and_mask     = 0x7c0;       /* Protcol11-Protocol12 */
      ihp_parser_custom_macro_parameters_tbl_data.header_qualifier_mask  = 0x7c0;       /* Protcol11-Protocol12 */
  ...
  if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_PPPOE_MODE, 0))
  {
      if (SOC_DPP_CONFIG(unit)->pp.udp_tunnel_enable)
      {
  ...
  ihp_parser_custom_macro_protocols_tbl_data.cstm_protocol = (ARAD_PP_PTP_UDP_ENCAP_PORT1 - 1);
  res = arad_pp_ihp_parser_custom_macro_protocols_tbl_set_unsafe(
          unit,
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 155, exit);
  ihp_parser_custom_macro_protocols_tbl_data.cstm_protocol = (ARAD_PP_PTP_UDP_ENCAP_PORT2 + 1);
  res = arad_pp_ihp_parser_custom_macro_protocols_tbl_set_unsafe(
          unit,
