...
uint8 sip_sharing_uc = 0;
uint8 sip_sharing_uc_public = 0;
uint8 sip_sharing_uc_rpf = 0;
uint8 sip_sharing_uc_public_rpf = 0;
uint8 sip_sharing_mc = 0;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        switch(opcode)
        {
            case PROG_FLP_IPV6UC:
                sip_sharing_configure = sip_sharing_uc;
                break;
            case PROG_FLP_IPV6UC_RPF:
                sip_sharing_configure = sip_sharing_uc_rpf;
                break;
            case PROG_FLP_IPV6UC_PUBLIC:
                sip_sharing_configure = sip_sharing_uc_public;
                break;
            case PROG_FLP_IPV6UC_PUBLIC_RPF:
                sip_sharing_configure = sip_sharing_uc_public_rpf;
                break;
            case PROG_FLP_IPV6MC:
                sip_sharing_configure = sip_sharing_mc;
                break;
            default:
                break;
        }
        ...
        switch(opcode)
        {
            case PROG_FLP_IPV6UC:
                ...
                break;
            case PROG_FLP_IPV6UC_RPF:
                sip_sharing_uc_rpf = 1;
                break;
            case PROG_FLP_IPV6UC_PUBLIC:
                sip_sharing_uc_public = 1;
                break;
            case PROG_FLP_IPV6UC_PUBLIC_RPF:
                sip_sharing_uc_public_rpf = 1;
                break;
            case PROG_FLP_IPV6MC:
                sip_sharing_mc = 1;
                break;
   ...
   )
{
  ...
  if(is_routing_enabled)
  {
    ...
    if (SOC_IS_ARADPLUS_A0(unit) && SOC_DPP_CONFIG(unit)->pp.oam_statistics) {
      ...
      /* ---------------------------------------*/
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
      /* ipv6uc_with_rpf program ID is allocated 1 and conflict with ETH_ING_LEARNING
       * We don't want to use l3_ipv6_uc_use_tcam to disable the function because
       * TCAM resource is less and no need this function in PON application with IBC port.
       */
      if (SOC_IS_JERICHO(unit) && (!ARAD_PP_FLP_IBC_PORT_ENABLE(unit))) {
  ...
  if (ARAD_PP_FLP_PON_PROG_LOAD()) {
      ...
      if(SOC_IS_JERICHO(unit) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_downmep_pwe_classification", 0) == 0)){
