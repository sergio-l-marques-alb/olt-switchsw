      ...
      )
{
    ...
    *success = TRUE;
    ...
    else
    {
        ...
        if (nof_found > 0) {
            ...
            if ((flags & ARAD_PP_FP_KEY_ALLOC_CE_FLP_NO_SPLIT) && (qual_type == SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES)) {
                if ((algorithm_args->balance_enabled == TRUE) && (total_bits_in_zone[0] < ARAD_PP_FLP_KEY_C_ZONE_SIZE_JERICHO_BITS)) {
                    /* Special case for FLP:
                     * Calculated hole size. KBP assumes ACL key has no hole.
                     */
                    qual_ce_info->qual_size = ARAD_PP_FLP_KEY_C_ZONE_SIZE_JERICHO_BITS - total_bits_in_zone[0];
                } else {
    ...
    while(tmp_size > 0) 
    {
        ...
        /* Fill two bits hole into hardware key */
        if (SOC_IS_QUX(unit) &&
            (tmp_size > 0) &&
            (total_bits_in_zone != NULL) &&
            (*total_bits_in_zone == SOC_DPP_DEFS_GET(unit, field_large_direct_lu_key_min_length))) {
            qual_ce_info->nof_ce++;
            qual_ce_info->ce_nof_bits[qual_ce_info->nof_ce] = 0x2;
            qual_ce_info->ce_lost_bits[qual_ce_info->nof_ce] = 0;
            qual_ce_info->place_cons[qual_ce_info->nof_ce] = ARAD_PP_FP_KEY_CE_HIGH;
            qual_ce_info->group_cons[qual_ce_info->nof_ce] = ARAD_PP_FP_KEY_CE_HIGH;
            qual_ce_info->size_cons[qual_ce_info->nof_ce] = ARAD_PP_FP_KEY_CE_SIZE_16;
        }
                    ...
                    if (SOC_IS_JERICHO_PLUS(unit)) {
                        for (qual_index=0; qual_index<SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++qual_index) {
                            ...
                            if (qual_types[qual_index] == SOC_PPC_FP_QUAL_KEY_AFTER_HASHING) {
                                ...
                                if (SOC_PPC_FP_IS_QUAL_TYPE_USER_DEFINED(qual_types[qual_index])) {
                                    ...
                                    if (found && (inter_qual_info.info.irpp_field == SOC_PPC_FP_QUAL_KEY_AFTER_HASHING)) {
                                        ...
                                        data[1] = 0;
                                        data[2] = 8;
                                        data[2] |= (inter_qual_info.info.qual_nof_bits-1) << 11;
                                        data[2] |= (0x64 - inter_qual_info.info.buffer_lsb) << 4;
                                        data[3] = 0;
                                        data[4] = 0x10 << (key_idx * 8);
                                    }
                                }
                            }
                            ...
                            if (is_slb_hash_found) {
                                ...
                                if (alloc_info->alloc_place == ARAD_PP_FP_KEY_CE_HIGH) {
                                    res = soc_mem_write(unit, IHB_PMF_UPDATE_KEY_GEN_MSBm, MEM_BLOCK_ANY, prog_ndx, data);
                                }
                                else {
                                    res = soc_mem_write(unit, IHB_PMF_UPDATE_KEY_GEN_LSBm, MEM_BLOCK_ANY, prog_ndx, data);
                                }
            ...
            *found = TRUE;
            ...
            for(qual_indx = 0;  (qual_indx < nof_quals) && (*found); ++qual_indx) 
            {
              ...
              if ((flags & ARAD_PP_FP_KEY_ALLOC_CE_FLP_NO_SPLIT) && (new_qual_types[qual_indx] == SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES)) {
                  /* KBP assuems ACL key has no hole. If LSB key has hole, algorithm needs to use empty qulifier to pad it.  */ 
                  if (!((algorithm_args.balance_enabled == TRUE) && (total_bits_in_zone[0] < ARAD_PP_FLP_KEY_C_ZONE_SIZE_JERICHO_BITS)) && (SOC_SAND_DIV_ROUND_UP(total_bits_in_zone[0], SOC_SAND_NOF_BITS_IN_CHAR) == 0)) {
...
{    
    ...
    int cluster_id;
    uint32 nof_share_qual = 0, nof_of_bits;
    SOC_PPC_FP_DATABASE_INFO *fp_database_info = NULL;
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    ...
    if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY) {
        ...
        for (key_ndx = 0; key_ndx <= is_key_320b; key_ndx++) {
            if (is_for_kbp && (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY) && dip_sip_sharing_enabled) {
                nof_copy_bits = buffer_to_key_nof_lsb[key_ndx] + buffer_to_key_nof_msb[key_ndx];
                curr_msb -= nof_copy_bits;
