  ...
  )
{
  ...
  for (action_indx = 0; action_indx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX && action_types[action_indx] != SOC_PPC_FP_ACTION_TYPE_INVALID; ++action_indx) {
      ...
      if (action_types[action_indx] == SOC_PPC_FP_ACTION_TYPE_COUNTER_AND_METER) {
          ...
          exp_action_flags[exp_action_indx] = flags | ARAD_PP_FP_FEM_ALLOC_FES_POLARITY;
          exp_action_indx++;
...
uint32
  arad_pp_fp_action_to_lsbs(
      ...
      SOC_SAND_IN  SOC_PPC_FP_DATABASE_TYPE  db_type,
      SOC_SAND_IN  SOC_PPC_FP_ACTION_TYPE    action_types[SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX], /* actions*/
  ...
  )
{
    ...
    is_action_contain_udh = FALSE;
    action_len_total = 0;
    for(action_indx = 0;  (action_indx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX) && (action_types[action_indx] != SOC_PPC_FP_ACTION_TYPE_INVALID); ++action_indx) {
        if (((action_types[action_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_1)
            || (action_types[action_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_2)) && SOC_IS_JERICHO_PLUS(unit)) {
            is_action_contain_udh = TRUE;
        }
    ...
    if (use_kaps) { /*in case of kaps start from offset 8*/
        if ((db_type == SOC_PPC_FP_DB_TYPE_DIRECT_TABLE) && is_action_contain_udh && (action_len_total >= 60)) {
            zone_length[0] = 0; /* only for UDH direct access mode*/
        } else {
            zone_length[0] = 8;
        }
    }
    ...
    for (algorithm_indx = 0; algorithm_indx < 2; algorithm_indx++) {
        ...
        if (algorithm_indx == 1) {
            ...
            action_lsb_ingress_tcam = 0;
            if (use_kaps &&
                !((db_type == SOC_PPC_FP_DB_TYPE_DIRECT_TABLE) && is_action_contain_udh && (action_len_total >= 60))) {
                /* In case of KAPS and not direct extracting more than 60 bits */
                zone_length[0] = 8;
            } 
            else {
                zone_length[0] = 0;
        ...
        if (direction == ARAD_PP_FP_ACTION_BUFFER_DIRECTION_ACTION_TO_BUFFER) {
            ...
            is_kaps_direct_action_always_valid = ((fp_db_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS) 
                ...
                && ((fp_db_info.action_types[action_val_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_1) 
                    || (fp_db_info.action_types[action_val_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_2))) ? TRUE : FALSE;
                    ...
                    if ((action_vals_lcl[action_val_indx].type >= SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_0) &&
                        ...
                        for (ext_index = 1; ext_index < ext_count; ext_index++) {
                            if (ext_index == (ext_count -1)) {
                                SHR_BITCOPY_RANGE(&(action_val_16_msb_for_flp_0[ext_index - 1]), 0, buffer, action_lsbs[action_val_indx] + ext_index * 32, (len - ext_index * 32));
                            } else {
                ...
                if (direction == ARAD_PP_FP_ACTION_BUFFER_DIRECTION_ACTION_TO_BUFFER) {
                    ...
                    is_kaps_direct_action_always_valid = ((fp_db_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS)
                        ...
                        && ((fp_db_info.action_types[action_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_1) 
                            || (fp_db_info.action_types[action_indx] == SOC_PPC_FP_ACTION_TYPE_USER_HEADER_2))) ? TRUE : FALSE;
                    ...
                    if(SOC_IS_JERICHO(unit)){
                        if ((action_vals[action_val_indx2].type >= SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_0) &&
                            ...
                            for (ext_index = 0; ext_index < 3; ext_index++) {
                                if (action_val_16_msb_for_flp_0[ext_index] != 0) {
                                    action_vals[action_val_indx2 + ext_index + 1].val = action_val_16_msb_for_flp_0[ext_index];
                                    action_vals[action_val_indx2 + ext_index + 1].type = action_vals_lcl[action_val_indx].type;
                                    action_val_indx2++;
                                }
                            }
                        }
  ...
  )
{
  ...
  *qual_length_no_padding = 0;
  ...
  else if (qual_type == SOC_PPC_FP_QUAL_KEY_AFTER_HASHING) 
  {
