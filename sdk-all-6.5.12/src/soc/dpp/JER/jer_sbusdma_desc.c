...
STATIC uint32 jer_sbusdma_desc_commit(int unit, uint8 safe)
{
    ...
    if (SOC_SAND_OK != soc_sand_take_chip_descriptor_mutex(unit)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL, (_BSL_SOCDNX_MSG("Take semaphore error.")));
    }
...
exit:
    ...
    SOCDNX_IF_ERR_REPORT(jer_sbusdma_desc_wait_previous_done(unit));
    if (SOC_SAND_OK != soc_sand_give_chip_descriptor_mutex(unit)) {
        SOCDNX_IF_ERR_REPORT(SOC_E_UNAVAIL);
    }
