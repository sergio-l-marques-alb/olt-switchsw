    ...
    bcm_fabric_module_vector_t *dest_array) 
{
    ...
    if (!flags || (flags & _SHR_FABRIC_MULTICAST_SET_ONLY))
    {
        ...
        {
            /*FE1600 does not support read-modify-write*/
...
/* 
 *read range of multicast groups - the data can be sperated per instance of RTP_MCTm 
...
soc_fe1600_fabric_multicast_multi_read_range(int unit, soc_multicast_t group_min, soc_multicast_t group_max, uint32 *entry_array)
{
    ...
    for (info_index = 0; 
         ...
         info_index++)
    {
        if (group_min < info[info_index].group_max && group_max > info[info_index].group_min) /*check if enries are required from this part*/
        {
            ...
            if (info[info_index].is_first_half == 0)
            {
                for (entry_index = read_index_min; entry_index <= read_index_max; entry_index++)
                {
                    ...
                    soc_mem_field_set(unit, RTP_MCTm, entry, LINK_BIT_MAPf, data /*value to set*/);
                }
            }
        }
    }
