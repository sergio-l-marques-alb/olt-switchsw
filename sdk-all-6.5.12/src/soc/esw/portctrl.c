...
soc_esw_portctrl_setup_ext_phy_add(int unit, portmod_pbmp_t phy_ports)
{
    ...
    int     xphy_logical_port, pport,temp_pport;
    ...
    SOC_PBMP_COUNT(SOC_INFO(unit).management_pbm, mgmt_pbm_count);
    PORTMOD_PBMP_ITER(phy_ports, pport) {
    temp_pport = pport;
        ...
        if((SOC_IS_TRIDENT3X(unit)) && (pport == 128) && (mgmt_pbm_count == 2)) {
            pport = 131;
        }
        ...
        xphy_logical_port = SOC_INFO(unit).port_p2l_mapping[temp_pport];
                            ...
                            phymod_operation_mode_t *phy_op_mode)
{
   ...
   /* Get the timestamp adjust */
    ...
    if (IS_CPRI_PORT(unit, port)) {
        roe_speed_cpri = soc_property_port_get(unit, port,
                         spn_CPRI_PORT_INIT_SPEED_ID, -1);
...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag, void *port_add_info)
{
    ...
    if (pm_type == portmodDispatchTypePm8x50) {
        ...
        rv = (portmod_pm_is_core_initialized(unit, phy_port,
                                             port, &is_init));
        if (rv != SOC_E_UNAVAIL && rv < 0) {
            if (local_add_info != NULL) {
                sal_free(local_add_info);
        ...
        if (!is_init) {
             ...
             */
            rv = portmod_pm_core_init(unit, phy_port, port, 0, add_info);
