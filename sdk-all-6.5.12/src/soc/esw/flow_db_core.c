                               ...
                               soc_flow_db_mem_view_split_field_info_t *sf_info)
{
    ...
    uint32 nFields = 0;
    int rv = -1;
    char f_name0[SOC_FLOW_DB_MAX_FIELD_NAME];
    char f_name1[SOC_FLOW_DB_MAX_FIELD_NAME];
    int len;
    int i;
    char *str_tbl;
    char *name_addr;
    ...
    sal_memset(f_name0,0, sizeof(f_name0));
    sal_memset(f_name1,0, sizeof(f_name1));
    ...
    sal_strcpy(f_name0,name_addr);
    f_name0[len++] = '_';
    f_name0[len++] = '0';
    f_name0[len++] = '_';
    sal_itoa(&f_name0[len], fentry->offset - 1, 10, 0, 0);

    /* name1 */
    sal_strcpy(f_name1,name_addr);
    len = sal_strlen(name_addr);
    f_name1[len++] = '_';
    sal_itoa(&f_name1[len], fentry->offset, 10, 0, 0);
    len = sal_strlen(f_name1);
    f_name1[len++] = '_';
    sal_itoa(&f_name1[len], fentry->width - 1, 10, 0, 0);
    ...
    for (i = 0; i < nFields; i++) {
        ...
        if (!sal_strcmp(name_addr, f_name0)) {
            sf_info->field[0].field_id = field_entry[i].field_id;
            sf_info->field[0].offset   = field_entry[i].offset;
            sf_info->field[0].type   =   field_entry[i].type;
            sf_info->field[0].width  =   field_entry[i].width;
            sf_info->field[0].flags  =   field_entry[i].flags;
            sf_info->field[0].value  =   field_entry[i].value;

        } else if (!sal_strcmp(name_addr, f_name1)) {
            sf_info->field[1].field_id = field_entry[i].field_id;
            sf_info->field[1].offset   = field_entry[i].offset;
            sf_info->field[1].type   =   field_entry[i].type;
            sf_info->field[1].width  =   field_entry[i].width;
            sf_info->field[1].flags  =   field_entry[i].flags;
            sf_info->field[1].value  =   field_entry[i].value;
            sf_info->field[1].field_id = field_entry[i].field_id;
        }
    }
    sf_info->offset = fentry->offset;
