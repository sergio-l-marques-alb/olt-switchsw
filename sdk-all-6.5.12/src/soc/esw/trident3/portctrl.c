...
_soc_td3_portctrl_pm4x10_portmod_init(int unit, int second_mgmt_port)
{
    ...
    int idx;
    uint8_t pPort[] = {TRIDENT3_FIRST_MGMT_PHYSICAL_PORT_NUM, 0 /* unused */, 
                       TRIDENT3_SECOND_MGMT_PHYSICAL_PORT_NUM, 0 /* unused */} ; 
    ...
    for (pmid = 0; PORTMOD_SUCCESS(rv) && (pmid < SOC_TD3_PM4X10_COUNT); pmid++) {
        ...
        SOC_PBMP_PORT_ADD(pm_create_info.phys, 130); /*  SW Workaround: Need to add 
                                                         the dummy port so that lane_index 
                                                         is incremented to 2 for the second
                                                         management port to detect ext PHY */
        if (second_mgmt_port) {
            SOC_PBMP_PORT_ADD(pm_create_info.phys, pPort[2]); /* second mgmt port */
        }
