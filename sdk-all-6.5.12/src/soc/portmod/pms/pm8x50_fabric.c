...
#define PM8X50_FABRIC_FORCE_SINGLE_PLL_SET(unit, pm_info, to_force) \
    SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_force_single_pll], &to_force)

#define PM8X50_FABRIC_FORCE_SINGLE_PLL_GET(unit, pm_info, to_force) \
    SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_force_single_pll], to_force)
...
#define PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index) \
    for (pll_index = nof_plls-1; pll_index >= 0; --pll_index)
...
STATIC
int pm8x50_fabric_squelch_set(int unit, int port, pm_info_t pm_info, int squelch);
...
{   
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        /* PLLs */
        rv = PM8X50_FABRIC_FORCE_SINGLE_PLL_SET(unit, pm_info, info->force_single_pll);
        SOCDNX_IF_ERR_EXIT(rv);

        PM8X50_FABRIC_PLLS_ITER(PM8X50_FABRIC_MAX_NUM_PLLS-1, i) {
...
int pm8x50_fabric_pm_lane_mapping_changed_get(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info, int* lane_mapping_changed)
{
    ...
    *lane_mapping_changed = 0;
    ...
    {
        if (lane_map.lane_map_tx[i] != add_info->lane_map.lane_map_tx[i]) {
...
int pm8x50_fabric_pm_fmac_tx_lane_mapping_set(int unit, int port, pm_info_t pm_info, const phymod_lane_map_t *lane_map)
{
    ...
    PM8X50_FABRIC_PORT_BITMAP_ITER(all_lanes_in_pm_bitmap, lane_index) {
        ...
        field_val = PM8X50_FABRIC_LANE_INDEX_IN_FMAC_GET(lane_index);
        soc_reg_field_set(unit, tx_lane_map_regs[lane_index_in_fmac], &reg_val, tx_lane_map_fields[lane_index_in_fmac], field_val);
        SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, tx_lane_map_regs[lane_index_in_fmac], fmac_blk_id, 0, reg_val));
    }
...
int pm8x50_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_pm_fmac_tx_lane_mapping_set(unit, port, pm_info, &add_info->lane_map));
    ...
    rv = PM8X50_FABRIC_FORCE_SINGLE_PLL_GET(unit, pm_info, &force_single_pll);
    SOCDNX_IF_ERR_EXIT(rv);
    if(force_single_pll)
    {
        PHYMOD_CORE_INIT_F_ONLY_PLL0_IS_ACTIVE_SET(&core_config);
    }

    core_config.ref_clock = fabric_data->ref_clk;
...
int pm8x50_fabric_port_disable_init(int unit, int port, pm_info_t pm_info)
{
    ...
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, 1));
...
int pm8x50_fabric_squelch_set(int unit, int port, pm_info_t pm_info, int squelch)
{
    ...
    phymod_phy_tx_lane_control_t tx_squelch;
    phymod_phy_rx_lane_control_t rx_squelch;
    ...
    if (squelch) {
        tx_squelch = phymodTxSquelchOn;
        rx_squelch = phymodRxSquelchOn;
    } else {
        tx_squelch = phymodTxSquelchOff;
        rx_squelch = phymodRxSquelchOff;
    }
...
int pm8x50_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if (!is_bypass)
    {
        ...
        SOCDNX_IF_ERR_EXIT(phymod_phy_tx_taps_default_get(&phy_access, phymodSignallingMethodNRZ, init_config.tx));
        ...
        for (phyn = 0; phyn < PM8X50_FABRIC_LANES_PER_CORE; phyn++) {
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_phy_lane_access_get(unit, port, pm_info, &params, 1, &phy_access, &nof_phys, NULL));
            /* the speed is not known at this stage. Choosing NRZ defaults */
            SOCDNX_IF_ERR_EXIT(phymod_phy_tx_taps_default_get(&phy_access, phymodSignallingMethodNRZ, &init_config.tx[phyn]));
        }
    ...
    if (is_bypass) {
        ...
        PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index) {
            ...
            SOCDNX_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr_REG32(unit, fmac_blk_id, lane_index_in_fmac, &reg_val));
            soc_reg_field_set(unit, FMAC_FMAL_TX_GENERAL_CONFIGURATIONr, &reg_val, FMAL_N_TX_ILKN_SHARING_ENf, 1);
            SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr_REG32(unit, fmac_blk_id, lane_index_in_fmac, reg_val));
...
int pm8x50_fabric_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    soc_reg_above_64_val_t reg_above64_val;
    uint32 phy_index, bitmap;
    int pll_index, nof_plls, lane_index;
    ...
    /* remove port from its PLL */
    SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_FORCE_SINGLE_PLL_GET(unit, pm_info, &force_single_pll));
    nof_plls = force_single_pll? 1 : 2;

    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
    PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
    {
...
int pm8x50_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
             ...
             PORTMOD_PORT_ENABLE_RX_GET(flags_mask) && PORTMOD_PORT_ENABLE_TX_GET(flags_mask))
    {
        ...
        if(enable){
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_squelch_set(unit, port, pm_info, 0));
...
int pm8x50_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    case portmodLoopbackMacAsyncFifo:
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_FMAL_TX_CONTROL_BURST_CONFIGURATIONr, fmac_blk_id, lane_index_in_fmac, &reg_val));
        soc_reg_field_set(unit, FMAC_FMAL_TX_CONTROL_BURST_CONFIGURATIONr, &reg_val, FMAL_N_CNTRL_BURST_PERIODf, enable ? 0xc : 8);
        SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_FMAL_TX_CONTROL_BURST_CONFIGURATIONr, fmac_blk_id, lane_index_in_fmac, reg_val));
        break;
...
int pm8x50_fabric_port_are_lanes_swapped(int unit, int port, pm_info_t pm_info, int* lanes_are_swapped)
{
    ...
    *lanes_are_swapped = 0;
    ...
    PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index) {
        PM8X50_FABRIC_LANE_FMAC_DATA_GET(pm_info, lane_index, fmac_blk_id, lane_index_in_fmac);
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, tx_lane_map_regs[lane_index_in_fmac], fmac_blk_id, 0, &reg_val));
        lane = soc_reg_field_get(unit, tx_lane_map_regs[lane_index_in_fmac], reg_val, tx_lane_map_fields[lane_index_in_fmac]);
        if(lane != lane_index_in_fmac) {
            *lanes_are_swapped = 1;
            SOC_EXIT;
        }
    }
...
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_MAX(is_retimer_enabled) ((is_retimer_enabled) ? (0xc) : (0xa))
#define PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD(is_retimer_enabled) ((is_retimer_enabled) ? (0xfffff0) : (0x7d0))
...
pm8x50_fabric_port_retimer_set(int unit, soc_port_t port, pm_info_t pm_info, int is_remote_loopback, int enable)
{
    ...
    soc_reg64_field32_set(unit, FMAC_FMAL_TX_DC_AUTO_ADJUSTr, &reg64_val, FMAL_03_TX_DC_AUTO_ADJUST_FIELD_8f, PM8X50_FABRIC_PORT_RETIMER_AUTO_ADJUST_PERIOD(enable));
...
STATIC
int pm8x50_fabric_port_speed_config_speed_to_vco_get(int unit, int port, pm_info_t pm_info, const int speed, portmod_vco_type_t* vco)
{
    SOCDNX_INIT_FUNC_DEFS;
...
STATIC
int pm8x50_fabric_port_speed_config_new_to_legacy_encoding_get(int unit, int port, pm_info_t pm_info, const portmod_fec_t new_encoding, portmod_port_pcs_t* old_encoding)
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    switch (new_encoding)
    {
        case PORTMOD_PORT_PHY_FEC_NONE:
            *old_encoding = PORTMOD_PCS_64B66B;
            break;
        case PORTMOD_PORT_PHY_FEC_BASE_R:
            *old_encoding = PORTMOD_PCS_64B66B_FEC;
            break;
        case PORTMOD_PORT_PHY_FEC_RS_206:
            *old_encoding = PORTMOD_PCS_64B66B_RS_FEC;
            break;
        case PORTMOD_PORT_PHY_FEC_RS_108:
            *old_encoding = PORTMOD_PCS_64B66B_LOW_LATENCY_RS_FEC;
            break;
        case PORTMOD_PORT_PHY_FEC_RS_545:
            *old_encoding = PORTMOD_PCS_64B66B_15T_RS_FEC;
            break;
        case PORTMOD_PORT_PHY_FEC_RS_304:
            *old_encoding = PORTMOD_PCS_64B66B_15T_LOW_LATENCY_RS_FEC;
            break;
        default:
...
STATIC
int pm8x50_fabric_port_speed_config_legacy_to_new_encoding_get(int unit, int port, pm_info_t pm_info, const portmod_port_pcs_t old_encoding, portmod_fec_t* new_encoding)
{
    SOCDNX_INIT_FUNC_DEFS;
    ...
    switch (old_encoding)
    {
        ...
        case PORTMOD_PCS_64B66B_15T_LOW_LATENCY_RS_FEC:
            *new_encoding = PORTMOD_PORT_PHY_FEC_RS_304;
            break;
        default:
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("invalid FEC %d for fabric port"), old_encoding));
    }
...
int pm8x50_fabric_pm_speed_config_validate(int unit, int pm_id, pm_info_t pm_info, const portmod_pbmp_t* ports, int flag, portmod_pm_vco_setting_t* vco_setting)
{
    int port, port_index;
    uint32 ports_count;
    int pll_index, nof_plls, required_pll_index, updated_pll_index;
    portmod_vco_type_t pll_vco_rate_required[PM8X50_FABRIC_MAX_NUM_PLLS];
    uint32 pll_active_lanes_bitmap_required[PM8X50_FABRIC_MAX_NUM_PLLS];
    uint32 required_plls_total_bitmap;
    ...
    uint32 pll_active_lanes_bitmap_curr_updated[PM8X50_FABRIC_MAX_NUM_PLLS];
    uint32 force_single_pll = 0;
    ...
    PM8X50_FABRIC_PLLS_INIT(pll_vco_rate_required, pll_active_lanes_bitmap_required);
    PM8X50_FABRIC_PLLS_INIT(pll_vco_rate_curr, pll_active_lanes_bitmap_curr);
    PM8X50_FABRIC_PLLS_INIT(pll_vco_rate_curr_updated, pll_active_lanes_bitmap_curr_updated);

    /** get - should we use only PLL0 OR both {PLL0, PLL1} for this Chip */
    SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_FORCE_SINGLE_PLL_GET(unit, pm_info, &force_single_pll));
    nof_plls = force_single_pll? 1 : 2;
    ...
    PORTMOD_PBMP_ITER(*ports, port)
    {
        SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_speed_to_vco_get(unit, port, pm_info, vco_setting->speed_config_list[port_index].speed, &required_vco));

        if ((required_vco != pll_vco_rate_required[0]) && (required_vco != pll_vco_rate_required[1]))
        {
            PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
            {
                if (pll_vco_rate_required[pll_index] == portmodVCOInvalid)
                {
                    pll_vco_rate_required[pll_index] = required_vco;
                    break;
                }
            }
            if (pll_index < 0)
            {
                LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "The given speeds of the following ports require more than %d PLLs:\n"), (force_single_pll? 1 : 2)));
                PORTMOD_PBMP_ITER(*ports, port)
                {
                    LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "\n port %d\n"), port));
                }
        ...
        /** add bits of current port to the matching PLL bitmap */
        SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
        pll_index = (required_vco == pll_vco_rate_required[0])? 0 : 1;
        PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index)
        {
            SHR_BITSET(&pll_active_lanes_bitmap_required[pll_index], lane_index);
        }

        ++port_index;
    }

    /** 2. get curr VCO rates and bitmaps of the PLL/s and copy them to curr_updated array */
    PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
    {
        ...
        pll_active_lanes_bitmap_curr_updated[pll_index] = pll_active_lanes_bitmap_curr[pll_index];
    }

    /** 3. see if VCO rate of PLL/s needs to be changed */
    /** 3.1. freeing current PLL/s with VCO rate that is no longer needed */
    PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
    {
        ...
        if ((pll_vco_rate_curr[pll_index] != portmodVCOInvalid)
                && (pll_vco_rate_curr[pll_index] != pll_vco_rate_required[0]) && (pll_vco_rate_curr[pll_index] != pll_vco_rate_required[1]))
        {
            required_plls_total_bitmap = pll_active_lanes_bitmap_required[0] | pll_active_lanes_bitmap_required[1];
            /* if all the lanes of the current PLL are included in the required new PLLs */
            if (PM8X50_FABRIC_BITMAP_IS_INCLUDED(pll_active_lanes_bitmap_curr[pll_index], required_plls_total_bitmap))
            {
                pll_vco_rate_curr_updated[pll_index] = portmodVCOInvalid;
                pll_active_lanes_bitmap_curr_updated[pll_index] = 0;
            }
        }
    }

    /** 3.2. adding new PLL/s (VCO rates) */
    PM8X50_FABRIC_PLLS_ITER(nof_plls, required_pll_index)
    {
        /* a PLL needs to be added only if it has a valid rate and its VCO is different from existing VCOs */
        if ((pll_vco_rate_required[required_pll_index] != portmodVCOInvalid)
                && (pll_vco_rate_required[required_pll_index] != pll_vco_rate_curr_updated[0]) && (pll_vco_rate_required[required_pll_index] != pll_vco_rate_curr_updated[1]))
        {
            PM8X50_FABRIC_PLLS_ITER(nof_plls, updated_pll_index)
            {
                if (pll_vco_rate_curr_updated[updated_pll_index] == portmodVCOInvalid)
                {
                    pll_vco_rate_curr_updated[updated_pll_index] = pll_vco_rate_required[required_pll_index];
                    break;
                }
            }
            /* didn't find a free PLL */
            if (updated_pll_index < 0)
            {
                LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "The given speeds of the following ports combined with existing speeds require more than %d PLLs:\n"), (force_single_pll? 1 : 2)));
                PORTMOD_PBMP_ITER(*ports, port)
                {
                    LOG_ERROR(BSL_LS_SOC_PORT, (BSL_META_U(unit, "\n port %d\n"), port));
                }
 ...
 * The function received OVCO rate in vco[0] and TVCO rate in vco[1].
...
int pm8x50_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, int is_serdes_reset, int num_vco, const portmod_vco_type_t* vco)
{
    int pll_index, nof_plls;
    portmod_vco_type_t pll_vco_rate[PM8X50_FABRIC_MAX_NUM_PLLS];
    uint32 pll_active_lanes_bitmap[PM8X50_FABRIC_MAX_NUM_PLLS];
    uint32 force_single_pll = 0;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if (is_serdes_reset)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("is_serdes_reset is supposed to be 0 for fabric")));
    }
    if (num_vco != PM8X50_FABRIC_MAX_NUM_PLLS)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("this function expects to receive an array of all PLLs in the PM - num_vco is %d, but it is supposed to be %d"), num_vco, PM8X50_FABRIC_MAX_NUM_PLLS));
    }
    ...
    if (force_single_pll && (vco[1] != portmodVCOInvalid))
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("requesting PLL1 (unused PLL!) VCO rate change to %d"), vco[pll_index]));
    }

    PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
    {
        /* if requested VCO is different from current VCO */
        if (vco[pll_index] != pll_vco_rate[pll_index])
        {
            pll_vco_rate[pll_index] = vco[pll_index];
            SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_PLL_VCO_RATE_SET(unit, pm_info, pll_vco_rate[pll_index], pll_index));
...
{
    uint32 encoding_properties = 0;
    portmod_port_pcs_t old_encoding;
    uint32 phy_index, bitmap;
    portmod_vco_type_t required_vco = portmodVCOInvalid;
    ...
    uint32 pll_active_lanes_bitmap[PM8X50_FABRIC_MAX_NUM_PLLS];
    portmod_vco_type_t pll_vco_rate[PM8X50_FABRIC_MAX_NUM_PLLS];
    int lane_index, pll_index, nof_plls;
    ...
    /* are we in single or dual PLL mode */
    SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_FORCE_SINGLE_PLL_GET(unit, pm_info, &force_single_pll));
    ...
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_speed_to_vco_get(unit, port, pm_info, speed_config->speed, &required_vco));
    ...
    if ((new_pll_state.pll0_lanes_bitmap == pll_active_lanes_bitmap[0]) && (new_pll_state.pll1_lanes_bitmap == pll_active_lanes_bitmap[1]))
    {
        SOCDNX_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_active_lanes_bitmap], &new_pll_state.pll0_lanes_bitmap, 0));
        SOCDNX_IF_ERR_EXIT(SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD, pm_info->wb_vars_ids[wb_pll_active_lanes_bitmap], &new_pll_state.pll1_lanes_bitmap, 1));
        PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
        {
    ...
    /** 5. configure the FEC- unrelated to phymod_phy_speed_config - done in the PM. RS-FEC configuration depends on the speed, so this step is done after speed configuration */
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_new_to_legacy_encoding_get(unit, port, pm_info, speed_config->fec, &old_encoding));

    if (old_encoding == PORTMOD_PCS_64B66B_FEC)
    {
        PORTMOD_ENCODING_EXTRCT_CIG_FROM_LLFC_SET(encoding_properties);
    } else if (PM8X50_FABRIC_ENCODING_IS_RS_FEC(old_encoding))
    {
        PORTMOD_ENCODING_LOW_LATENCY_LLFC_SET(encoding_properties);
    }
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_encoding_set(unit, port, pm_info, encoding_properties, old_encoding));
