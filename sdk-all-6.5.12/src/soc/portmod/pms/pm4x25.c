...
#define PM4x25_WB_BUFFER_VERSION        (10)
...
int portmod_pm4x25_wb_upgrade_func(int unit, void *arg, int recovered_version, int new_version)
{
    pm_info_t pm_info;
    int i, adjust_index;
    int nof_phys, dynamic_state, pll_divider_req;
...
int _pm4x25_port_tsc_is_master_pll(int unit, int port, pm_info_t pm_info)
{
   ...
   _SOC_IF_ERR_EXIT(_pm4x25_pm_version_get(unit, port, pm_info, &pm_version));
   ...
   if (PORTMOD_PM4x25_IS_GEN1_GEN2(pm_version)) {
    ...
    } else if (PORTMOD_PM4x25_IS_GEN3_28nm(pm_version)) {
...
int _pm4x25_pm_serdes_core_init(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if ((PORTMOD_PM4x25_IS_GEN3_28nm(pm_version) || PORTMOD_CPM4x25_IS_GEN3_28nm(pm_version))
        ...
        speed_config.eth_of_mixed_mode = add_info->init_config.cpm_core_mode;
...
int _pm4x25_pm_enable(int unit, int port, pm_info_t pm_info, int port_index, const portmod_port_add_info_t* add_info, int enable)
{
    ...
    uint32 txpi_lane_select[portmodPortModeCount][PM4X25_LANES_PER_CORE]= {
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        if(enable) {
            ...
            if(PORTMOD_PM4x25_IS_GEN3_28nm(pm_version)){
                ...
                _SOC_IF_ERR_EXIT(_pm4x25_txpi_lane_select_set(unit, port, pm_info, (uint32 *)txpi_lane_select[port_mode]));
             }
...
STATIC
int _pm4x25_port_ts_adjust_get(int unit, int port, pm_info_t pm_info,
                                     ...
                                     int phy_init_flags)
{
    ...
    if (config->flags & PHYMOD_INTF_F_UPDATE_SPEED_LINKUP) {
        ...
        /*Set Timestamp Delay*/
        rv = _pm4x25_port_ts_adjust_get(unit, port, pm_info, config->speed, &ts_adjust);
        rv = clmac_timestamp_delay_set(unit, port, ts_adjust);
        /* PM VERSION and CLMAC BYTE ADJUST aren't valid before TH2 */
        if (!PORTMOD_PM4x25_IS_GEN1(pm_version)) {
            rv = clmac_timestamp_byte_adjust_set(unit, port, config->speed);
        }
...
int pm4x25_port_update(int unit, int port, pm_info_t pm_info, const portmod_port_update_control_t* update_control)
{
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            ...
            /*Set Timestamp Delay*/
            _SOC_IF_ERR_EXIT(_pm4x25_port_ts_adjust_get(unit, port, pm_info, interface_config.speed, &ts_adjust));
                     ...
                     pm_info_t pm_info, const portmod_port_mode_info_t* mode)
{
    ...
    if(PORTMOD_PM4x25_IS_GEN3_28nm(pm_version)){
        /* Set txpi lane select */
        _SOC_IF_ERR_EXIT(_pm4x25_txpi_lane_select_set(unit, port, pm_info, (uint32 *)txpi_lane_select[mode->cur_mode]));
    }
                              ...
                              int phy_port, int flags)
{
    ...
    /* set port mode */
    /* _SOC_IF_ERR_EXIT(pm4x25_port_mode_set(unit, port, pm_info, pmode)); */
...
_pm4x25_init_plls_get(int unit, int port, pm_info_t pm_info, portmod_vcos_speed_config_t *speed_config, int size, phymod_dual_plls_t *dual_plls)
{
    ...
    size = 1;
    pm4x25_port_vcos_get(unit, port, speed_config, size, &dual_vcos);
   ...
   if (speed_config->eth_of_mixed_mode == portmodCpmMixEthCpri) {
       /* PLL1 is used for ETH ports */
       dual_plls->pll1_div = dual_plls->pll0_div;
...
int
pm4x25_port_vcos_get(int unit, int port, const portmod_vcos_speed_config_t* speed_config_list, int size, portmod_dual_vcos_t* dual_vco)
{
    int index = 0;
    int vco_bitmap[4] = {0};
    int vco_bmp = 0;
    ...
    if (speed_config_list->eth_of_mixed_mode) {
        switch (vco_bmp){
            case VCO_20G:
                dual_vco->vco_0 = portmodVCO20G;
            break;
            case VCO_25G:
