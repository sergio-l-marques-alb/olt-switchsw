    ...
    uint8 is_index)
{
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, (*hash_table), **hash_table, /*number of elements*/ 1, DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state hash table");
    ...
    (*hash_table)->ptr_size = ptr_size;
    (*hash_table)->null_ptr = null_ptr;
    (*hash_table)->key_size = key_size;
    (*hash_table)->data_size = data_size;
    (*hash_table)->is_index = is_index;
    
    if (init_info->print_cb_name != NULL) {
        sal_strncpy((*hash_table)->print_cb_name, init_info->print_cb_name, SW_STATE_CB_DB_NAME_STR_SIZE-1);
    }
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, (*hash_table)->keys, uint8, max_nof_elements * key_size,
        DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state hash table keys");
    ...
    if (!is_index) {
        DNX_SW_STATE_ALLOC(unit, module_id, (*hash_table)->data, uint8, max_nof_elements * data_size,
            DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state hash table data");
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, (*hash_table)->next, uint8, max_nof_elements * ptr_size,
        DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state hash table next");
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, (*hash_table)->lists_head, uint8, max_nof_elements * ptr_size,
        DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state hash table lists head");
     ...
     */
    sw_state_occ_bm_create(unit, module_id, &btmp_init_info, &((*hash_table)->memory_use));
...
{
    uint8 *cur_key, *next;
    uint32 key_size, ptr_size, null_ptr, hash_val, ptr_long[1], tmp_buf[1], next_node;
    ...
    for (indx = 0; indx < expected_nof_elements; ++indx)
    {
        ...
        while (ptr_long != null_ptr)
        {
            ...
            if (sal_strncmp(hash_table->print_cb_name, "", SW_STATE_CB_DB_NAME_STR_SIZE-1) != 0) {
                sal_strncpy(cb_db.function_name, hash_table->print_cb_name, SW_STATE_CB_DB_NAME_STR_SIZE-1);
                dnx_sw_state_hash_table_print_cb_get_cb(&cb_db, 0, &print_cb);
                print_cb(unit, cur_key, cur_data);
            } else {
                /* print the key */
                _DNX_SW_STATE_PRINT(unit, "Key: 0x");
                for (print_indx = 0; print_indx < key_size; ++print_indx)
                {
                    _DNX_SW_STATE_PRINT(unit, "%02x", cur_key[key_size - print_indx - 1]);
                }
                ...
                if (hash_table->is_index) {
                    ...
                    {
                        _DNX_SW_STATE_PRINT(unit, "%02x", cur_data[data_size - print_indx - 1]);
                    }                
                } else {
                    _DNX_SW_STATE_PRINT(unit,"%u", ptr_long);
                }
            }
 ...
 * \remarks
 *   _DNX_SW_STATE_PRINT is used for the printing.
 */
static shr_error_e
    ...
    sw_state_htbl_t hash_table)
{
    ...
    for (indx = 0; indx < expected_nof_elements; ++indx)
    {
        ...
        ptr_long = 0;
        SHR_IF_ERR_EXIT(utilex_U8_to_U32(next, ptr_size, &ptr_long));
        _DNX_SW_STATE_PRINT(unit, " entry %2u: Pointer Value %3d ", indx, (int) ptr_long);
    ...
    sw_state_htbl_t hash_table)
{
    ...
    _DNX_SW_STATE_PRINT(unit, "key_size   : %u\n", key_size);
    _DNX_SW_STATE_PRINT(unit, "data_size  : %u\n", data_size);
    _DNX_SW_STATE_PRINT(unit, "init_info.max_nof_elements : %u\n", max_nof_elements);
    _DNX_SW_STATE_PRINT(unit, "init_info.expected_nof_elements: %u\n", expected_nof_elements);
exit:
    SHR_FUNC_EXIT;
}
