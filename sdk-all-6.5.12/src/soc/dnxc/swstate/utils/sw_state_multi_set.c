    ...
    sw_state_multi_set_info_t * init_info)
{
    ...
    uint32 member_size, nof_members;
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, *multi_set, **multi_set, /* number of elements */ 1,
                       DNX_SW_STATE_ALLOC_AFTER_INIT_EXCEPTION, "sw_state multi set");
     ...
     */
    if (nof_members == 0 || member_size == 0)
    {
    ...
    if (max_duplications > 1)
    {
        ...
        DNX_SW_STATE_ALLOC(unit, module_id, (*multi_set)->ref_counter, DNX_SW_STATE_BUFF,
                           nof_members * (*multi_set)->counter_size,
        ...
        DNX_SW_STATE_MEMSET(unit, module_id, (*multi_set)->ref_counter, 0, 0x00,
                            nof_members * (*multi_set)->counter_size, 0, "sw_state multi set ref counter");
    ...
    uint8 clear_on_print)
{
    ...
    while (!SW_STATE_HASH_TABLE_ITER_IS_END(&iter))
    {
        ...
        if (org_ref_count)
        {
            ...
            if (print_header)
            {
                _DNX_SW_STATE_PRINT(unit, "| %-8s|", "Index");
                _DNX_SW_STATE_PRINT(unit, "  ");
                for (member_index = 0; member_index < multi_set->init_info.member_size; member_index++)
                {
                    _DNX_SW_STATE_PRINT(unit, "%2s", "  ");
                }
                _DNX_SW_STATE_PRINT(unit, "| %-8s|\n\r", "ref_cnt");
                print_header = 0;
            }
            _DNX_SW_STATE_PRINT(unit, "| %-8u|", data_indx);
            _DNX_SW_STATE_PRINT(unit, "0x");
            for (member_index = 0; member_index < multi_set->init_info.member_size; member_index++)
            {
                _DNX_SW_STATE_PRINT(unit, "%02x", key[multi_set->init_info.member_size - member_index - 1]);
            }
            _DNX_SW_STATE_PRINT(unit, "| %-8u|\n\r", org_ref_count);
        }
    }
exit:
    SHR_FUNC_EXIT;
