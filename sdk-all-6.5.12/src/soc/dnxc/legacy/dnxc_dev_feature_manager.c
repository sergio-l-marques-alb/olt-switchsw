...
static shr_error_e dnxc_keywords_cb (int unit,void  *value,int nof_ranges,void *data)
{
...
static shr_error_e dnxc_range_cb (int unit,void  *value,int nof_ranges,void *data)
{
    uint32 v;
    int i;
...
static shr_error_e dnxc_otn_enable_cb (int unit,void  *value,int nof_ranges,void *data)
{
    uint16 dev_id; 
    uint8 rev_id;
    ...
    if (dev_id==BCM88675_DEVICE_ID  || dev_id==BCM88375_DEVICE_ID) {
        uint32 val;
        SHR_IF_ERR_EXIT(READ_ECI_OGER_1004r_REG32(unit,&val));
    ...
    SHR_IF_ERR_EXIT(dnxc_keywords_cb(unit,value,nof_ranges,data));
...
shr_error_e dnxc_is_property_value_permited(int unit,const char *property,void *value) {
    int i,j,k;
    ...
    for (i=0;i<len;i++) {
        ...
        for (j=0;features_array[j].feature!=DNXC_INVALID_FEATURE;j++) {
            ...
            if (rc!= _SHR_E_NONE) {
                SHR_ERR_EXIT(_SHR_E_FAIL, "Property:%s  blocked for device %s\n", property, soc_dev_name(unit));  
            }
...
shr_error_e dnxc_property_get(int unit, const char *name, uint32 defl,uint32 *value)
{
    SHR_FUNC_INIT_VARS(unit);
    DNXC_LEGACY_FIXME_ASSERT;
    *value = soc_property_get(unit,name, defl);

    SHR_IF_ERR_EXIT(dnxc_is_property_value_permited(unit,name,value));

exit:
    SHR_FUNC_EXIT;
}

shr_error_e dnxc_property_suffix_num_get(int unit, int num, const char *name, const char *suffix, uint32 defl,uint32 *value)
{
    SHR_FUNC_INIT_VARS(unit);
    DNXC_LEGACY_FIXME_ASSERT;
...
shr_error_e dnxc_property_get_str(int unit, const char *name,char **value)
{
    SHR_FUNC_INIT_VARS(unit);
