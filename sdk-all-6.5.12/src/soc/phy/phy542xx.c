                         ...
                         int copper_enable)
{
    ...
    if (PHY_IS_BCM54180(pc) || PHY_IS_BCM54182(pc) || PHY_IS_BCM54185(pc) || PHY_IS_BCM54140(pc) ) {
        SOC_IF_ERROR_RETURN(
...
phy_bcm542xx_link_up(int unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_phyctrl_notify(unit, port, phyEventInterface, IS_GMII_PORT(unit, port) ? SOC_PORT_IF_GMII : SOC_PORT_IF_SGMII));
    ...
    */
    ...
    if ( (pc->macsec_enable) &&
         (pc->macsec_mac_policy == PHY_MAC_SWITCH_DUPLEX_GATEWAY) && (speed == 10 || speed == 100)) {
        int duplex;
