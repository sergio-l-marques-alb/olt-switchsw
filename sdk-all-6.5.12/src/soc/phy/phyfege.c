/*
 * $Id: phyfege.c,v 1.29 Broadcom SDK $
...
phy_fe_init(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM5324(pc) && pc->phy_rev == ROBO5324_PHY_REV_A0) {
        uint16 tmp;

        /* enable MII shadow registers */
        tmp = 0x8b;
        SOC_IF_ERROR_RETURN
            (WRITE_PHY_REG(unit, pc, 0x1f, tmp));

        /* set PGA_FIX_EN */
        tmp = 0x100;
        SOC_IF_ERROR_RETURN
            (WRITE_PHY_REG(unit, pc, 0x19, tmp));

        /* disable MII shadow registers */
        tmp = 0xb;
        SOC_IF_ERROR_RETURN
            (WRITE_PHY_REG(unit, pc, 0x1f, tmp));
...
phy_fe_ge_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if(PHY_IS_BCM53101_A0(pc) || PHY_IS_BCM53280_A0(pc)) {
        if (speed == 10) {
            SOC_IF_ERROR_RETURN(
                PHYFEGE_REG_WRITE(unit, pc, 0x0, 0x0, 0x1f,0x8b));
            /* Power up RX block */
            SOC_IF_ERROR_RETURN(
                PHYFEGE_REG_WRITE(unit, pc, 0x0, 0x0, 0x14, 0x4000));
            sal_udelay(1);
             /* Power down RX block */
            SOC_IF_ERROR_RETURN(
                PHYFEGE_REG_WRITE(unit, pc, 0x0, 0x0, 0x14, 0x0000));
            SOC_IF_ERROR_RETURN(
                PHYFEGE_REG_WRITE(unit, pc, 0x0, 0x0, 0x1f,0xb));
        }
    }
...
#ifdef BCM_ROBO_SUPPORT
    if (SOC_IS_ROBO(unit)) {
extern int phy_cdMain(int unit, soc_port_t port, soc_port_cable_diag_t *status);

        return phy_cdMain(unit, port, status);
    }
#endif /* BCM_ROBO_SUPPORT */
