...
/*
 * $Id: sand_integer_arithmetic.c,v 1.11 Broadcom SDK $ Integer Logarithmic { 
 */
    ...
    uint32 *exp)
{
    ...
    if ((mnt == NULL) ||
        (exp == NULL) ||
        (mnt_nof_bits > 15) || (exp_nof_bits > 10) || (mnt_nof_bits == 0) || (exp_nof_bits == 0) || (mnt_inc > 1))
    {
        /*
         * Invalid Inputs
         */
...
/*****************************************************
*NAME
*   utilex_complex_comlicated_to_mnt_exp
*TYPE:
*  PROC
*DATE:
*  06/06/2010
...
*CALLING SEQUENCE:
*   utilex_compute_comlicated_to_mnt_exp(mnt, exp, eq_const_multi, eq_const_mnt_inc, x)
*INPUT:
*  DIRECT:
*    uint32  mnt,
    ...
    uint32 silent)
{
    ...
    uint32
        abs_val_numerator_i,
        abs_val_denominator_i, exact_abs_val_numerator_i, exact_abs_val_denominator_i, max_val_i, mnt, exp;
...
exit:
    if (res && !silent)
    {
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("utilex_mnt_binary_fraction_test() returned with an error.\n\r")));
    }
    return res;
}
    ...
    uint32 silent)
{
    ...
    if (!silent)
    {
        LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("****%s**** START\n"), proc_name));
    }
    ...
    while (--nof_test)
    {
        ...
        {
            LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("**** Test Type 1: %s - FAIL\n"), proc_name));
            LOG_INFO(BSL_LS_SOC_COMMON,
                     (BSL_META
                      ("**** answer_1 != answer_2, test_number = 0x%X, nof_test=%u, seed=%u \n"),
    ...
    while (--nof_test)
    {
        ...
        if (answer_1 != answer_2)
        {
            LOG_INFO(BSL_LS_SOC_COMMON, (BSL_META("**** Test Type 2: %s - FAIL\n"), proc_name));
            LOG_INFO(BSL_LS_SOC_COMMON,
                     (BSL_META
                      ("**** answer_1 != answer_2, test_number = 0x%X, nof_test=%u, seed=%u \n"),
                      test_number, nof_test, seed));
