...
board_cmd_start(char *name, int flagidx)
{
    ...
    if (name == NULL || sal_strlen(name) == 0) {
        ...
        name = board_probe();
        if (name == NULL) {
            printk("Board probe failed\n");
            goto fail;
        }
    } else {
        /* find */
        if (!board_find(name)) {
            printk("Cannot find board '%s'\n", name);
            goto fail;
        }
    }
    ...
    if (BCM_FAILURE(rv)) {
        printk("Cannot attach board '%s':%s (%d)\n",
    ...
    if (flagidx < 0) {
        ...
        if (flagstr) {
            if (isint(flagstr)) {
                ...
                printk("Could not parse board_flags=%s\n",flagstr);
                goto fail;
            }
        } else {
    ...
    if (BCM_FAILURE(rv)) {
        printk("Cannot get board attributes for %s:%s (%d)\n",
               name, bcm_errmsg(rv), rv);
        goto fail;
    }
    ...
    if (attr == NULL) {
        printk("Cannot alloc memory for attributes\n");
        goto fail;
    }
            ...
            if (value) {
                if (isint(value)) {
                    rv = board_attribute_set(config_name,
                                             parse_integer(value));
                    if (BCM_FAILURE(rv)) {
                        printk("Cannot set board attribute %s=%s:%s (%d)\n",
                               config_name, value, bcm_errmsg(rv), rv);
                        goto fail;
                    }
                } else {
                    printk("Could not parse %s=%s\n",
                           config_name, value);
                    goto fail;
                }
            }
...
board_cmd_register(char *name)
{
    ...
    if (BCM_FAILURE(rv)) {
        printk("board_driver_builtins_get: %s (%d)\n", bcm_errmsg(rv), rv);
        goto fail;
    }
    ...
    if (add) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            printk("Cannot add board driver %s:%s (%d)\n",
                   name, bcm_errmsg(rv), rv);
        }
    } else {
        printk("Board driver %s not found\n", name);
    }
...
board_cmd_unregister(char *name)
{
    ...
    if (del) {
        ...
        if (BCM_SUCCESS(rv)) {
            ...
            printk("Cannot delete board driver %s:%s (%d)\n",
                   name, bcm_errmsg(rv), rv);
        }
    } else {
        printk("Board driver %s not registered\n", name);
    }
...
board_cmd_attribute_list(char *name)
{
    ...
    if (BCM_FAILURE(rv)) {
        printk("board_attributes_get: %s (%d)\n", bcm_errmsg(rv), rv);
        return CMD_FAIL;
    }
...
board_cmd(int unit, args_t *args)
{
    ...
    if (!init) {
        rv = board_init();
        if (BCM_FAILURE(rv)) {
            printk("board_init failed:%s (%d)\n", bcm_errmsg(rv), rv);
            return CMD_FAIL;
        }
        rv = board_driver_builtins_add();
        if (BCM_FAILURE(rv)) {
            printk("board_driver_builtins_add failed:%s (%d)\n",
                   bcm_errmsg(rv), rv);
            return CMD_FAIL;
        }
        init=TRUE;
