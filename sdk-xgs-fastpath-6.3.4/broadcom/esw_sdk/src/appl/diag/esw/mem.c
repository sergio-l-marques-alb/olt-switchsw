...
cmd_esw_mem_write(int unit, args_t *a)
{
	...
	soc_cm_debug(DK_ERR, "ERROR: Table %s is read-only\n",
    ...
    if (use_slam && count > 1) {
        ...
        for (index = start; index < start + count; index++) {
            if (acc_type) {
                if ((r = soc_mem_pipe_select_write(unit, mem, copyno,
                                                   acc_type,
                                                   no_cache ? -index : index,
                                                   entry)) < 0) {
                    printk("Write ERROR: table %s(Y).%s[%d]: %s\n",
                           SOC_MEM_UFNAME(unit, mem), copyno_str,
                           index, soc_errmsg(r));
                    goto done;
                }
            } else if ((r = soc_mem_write(unit, mem, copyno,
...
esw_ser_inject(int unit, args_t *a) {
    ...
    if (parse_default_fill(&pt) < 0) {
        printk("Invalid default fill:\n");
        return rv;
    } else if(parse_arg_eq(a, &pt) < 0) {
        soc_cm_print("%s: Invalid option %s\n", ARG_CMD(a), ARG_CUR(a));
        return rv;
    }
    if (!(parse_memory_name(unit,&(mem),mem_name,&block,0) >=0)) {
        soc_cm_print("Invalid memory selected.\n");
        return rv;
    }
    if (!sal_strcasecmp(pipe_name, "pipe_x")) {
        ...
        soc_cm_print("Invalid pipe selected. Valid entries are:"
                     " 'pipe_x' 'pipe_y\n");
        return rv;
    }
    if (!SOC_SUCCESS(soc_ser_inject_support(unit, mem, pipe))){
        if (soc_ser_inject_support(unit,mem,pipe) == SOC_E_FAIL) {
            soc_cm_print("Injection failed as the system is not initialized.\n");
            rv = CMD_FAIL;
            return rv;
        } else {
            soc_cm_print("The selected memory: %s is valid, but SER correction "
    ...
    if (index < 0) {
        soc_cm_print("Invalid index selected");
        return rv;
    }
    if (soc_ser_inject_error(unit, mem, pipe, block, index) == SOC_E_FAIL) {
        soc_cm_print("Injection failed as the system is not initialized.\n");
        rv = CMD_FAIL;
    } else {
        soc_cm_print ("Error injected on %s at index %d %s\n", mem_name, index,
                      pipe_name);
        rv = CMD_OK;
    }
...
cmd_esw_ser(int unit, args_t *a){
    ...
    if (arg1 != NULL && !sal_strcasecmp(arg1, "inject")) {
        ...
        soc_cm_print("Info option not yet implemented\n");
        rv = CMD_NOTIMPL;
    } else if (arg1 != NULL && !sal_strcasecmp(arg1, "show")) {
        soc_cm_print("Show option not yet implemented\n");
        rv = CMD_NOTIMPL;
    } else if (arg1 != NULL && !sal_strcasecmp(arg1, "log")) {
        soc_cm_print("Log option not yet implemented\n");
        rv = CMD_NOTIMPL;
    } else {
        soc_cm_print("Invalid mode selected.\n");
    }
    return rv;
}
