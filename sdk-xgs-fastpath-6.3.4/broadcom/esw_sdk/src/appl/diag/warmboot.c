/*
 * $Id: 61f561b4c413f99231c8ec127084633d58022ac1 $
...
appl_scache_file_read_func(int unit, uint8 *buf, int offset, int nbytes)
{
    ...
    {
        printk("Unit %d: Mutex take failed\n", unit);
        return SOC_E_FAIL;
    }
    ...
    {
        printk("Unit %d: Mutex give failed\n", unit);
        return SOC_E_FAIL;
    }
    return SOC_E_NONE;
}
...
#ifndef NO_FILEIO
    if (!scache_nm[unit]) {
    printk("Unit %d: Scache file is not set\n", unit);
    return -1;
    }
    if (sal_mutex_take(file_lock[unit],sal_mutex_FOREVER))
    {
...
appl_scache_file_open(int unit, int warm_boot, char *filename)
{
    ...
    printk("Unit %d: Error opening scache file\n", unit);
    return -1;
    }
    scache_nm[unit] =
