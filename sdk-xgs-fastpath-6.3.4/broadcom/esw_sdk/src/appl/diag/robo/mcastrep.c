/*
 * $Id: mcastrep.c 1.11 Broadcom SDK $
...
if_robo_mcastrep(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(subcmd, "group") == 0) {
        ...
        if (sal_strcasecmp(subcmd, "create") == 0 ) {
            ...
            printk(" Mcast Group %d created! %s for %s\n", mcrep_id, 
                    ((flags & BCM_MULTICAST_WITH_ID) ? 
            ...
            printk(" McastRep group(0x%x) destroied!\n", mcrep_id);
            return CMD_OK;
        ...
        if (sal_strcasecmp(subcmd, "show") == 0 ) {
            ...
            if ((port == -1) || (vp_id == -1)){
                printk("  ERROR: Missing port/vport id!\n\n");
                return CMD_USAGE;
            }
            ...
            printk(" Mcast group 0x%x,port 0x%x and subport 0x%x "
                    ">> EncapID=0x%x\n",
        ...
        subcmd = ARG_GET(a);
        if (subcmd == NULL) {
            printk("%s: ERROR: Missing subcommand. (add|delete)\n", 
                    ARG_CMD(a));
            return CMD_USAGE;
        }
        if (sal_strcasecmp(subcmd, "add") == 0) {
            ...
            if (port == -1){
                printk("  ERROR: Missing port id!\n\n");
                return CMD_USAGE;
            }
            
            if (encap_id != -1){
                printk("  Message: Encap-ID will be ignored!\n\n");
            }

            if (SOC_IS_TBX(unit)){
                type = (encap_id == -1) ?
                        BCM_MULTICAST_TYPE_L2 : BCM_MULTICAST_TYPE_SUBPORT;
            } else {
                type = BCM_MULTICAST_TYPE_L2;
            }

            /* reassign type to ROBO's specific type */
            ...
            printk(" Port=%d has added to Mcast group 0x%x \n", 
                    port, mcrep_id);
            return CMD_OK;
            ...
            printk(" Port=%d has deleted from Mcast group 0x%x \n", 
                    port, mcrep_id);
            return CMD_OK;
            ...
            printk(" McastRep group(0x%x) was cleared.\n", mcrep_id);
