...
if_robo_bpdu(int unit, args_t *a)
{
    ...
    if (!sal_strcasecmp(subcmd, "add") ||
        ...
        if (arg_index > (count - 1) ) {       
            printk("%s ERROR: index=%d invalid (support %d BUPUs)\n", 
                    ARG_CMD(a), arg_index, count);
            return CMD_FAIL;
        }
        if (BCM_MAC_IS_ZERO(arg_macaddr)){
            printk("Zero MAC address is invalid for BPDU ADD\n");
            return CMD_FAIL;
        }
        ...
        if (rv != BCM_E_NONE) {
            if (rv == BCM_E_FULL){
                printk("\t No Free Space for new BPDU addr\n");
            }
            printk("%s ERROR: %s\n", ARG_CMD(a), bcm_errmsg(rv));
            return CMD_FAIL;
        } else {
            printk("\t New BPDU added!(id=%d)\n", idx_used);
        }
        return CMD_OK;
    }
    ...
    if (!sal_strcasecmp(subcmd, "del") ||
         ...
         */
        if (arg_index == -1){
            printk("\t BPDU DEL allowed existed BPDU index only.\n");
            return CMD_FAIL;
        }
        rv = DRV_DEV_PROP_GET(unit, 
                DRV_DEV_PROP_BPDU_NUM, (uint32 *) &count);
    ...
    if (! sal_strcasecmp(subcmd, "show") || 
        ...
        printk("unit %d supports %d BPDU entries\n", unit, count);
        bcm_l2_cache_addr_t_init(&addr);
