/*
 * $Id: hash_mem_test.c 1.6 Broadcom SDK $
 *
...
test_generic_hash(int unit, args_t *a, void *p)
{
    ...
    if (htd->opt_verbose) {
        printk("Starting generic hash test: iter: %d, inc: %d\n", iter, incr);
    }    
    ...
    if (htd->opt_verbose) {
        printk("Num of configured banks: %d\n", bcount);
    }
    if (htd->opt_banks != -1) {
        bmask = 0;
        ...
        if (htd->opt_verbose) {
            printk("Supported banks mask: 0x%x\n", bmask);
        }
        ...
        printk("Num of requested banks: %d\n", req_bcount);
    }
    htd->restore = 0;
    if (htd->offset_count) {
        if (htd->opt_verbose) {
            printk("Requested offsets: %d\n", htd->offset_count);
        }
        ...
        for (ix = 0; ix < htd->offset_count; ix++) {
            ...
            if (inc_mode && (htd->offsets[ix] < 48)) {
                ...
                if (htd->opt_verbose) {
                    printk("Updating offset for bank %d from %d to 48\n", 
            ...
            if (htd->opt_verbose) {
                printk("Set offset %d in place of %d for bank %d\n", 
    ...
    for (ix = 0; ix < iter; ix++) {
        ...
        if (index == hw_index) {
            if (htd->opt_verbose) {
                printk("Matching s/w and h/w insert(%d) index: %d\n", ix, index);
            }
            ...
            if (index != hw_index) {
                ...
                if (htd->opt_verbose) {
                    printk("Matching s/w and h/w search(%d) index: %d\n", ix, index);
                }
            }
        } else {
            printk("Mismatch in s/w and h/w insert index: %d vs %d\n", index, hw_index);
            ifail++;
            sskip++;
        }        
    ...
    printk("Insert result: %d iter(s) passed out of %d iter(s), "
           "%d iter(s) skipped.\n", 
           iter-ifail-full, iter, full);
    printk("Search result: %d iter(s) passed out of %d iter(s), "
           "%d iter(s) skipped.\n", 
           iter-sfail-full-sskip, iter, full+sskip);
    printk("Delete result: %d entr(y/ies) deleted.\n",
           iter-dfail-full);
done:
    if (buff) {
        sal_free(buff);
...
test_generic_hash_ov(int unit, args_t *a, void *p)
{
    ...
    if (htd->opt_verbose) {
        printk("Starting generic hash overflow test\n");
    }
    return 0;
}
    ...
    if (htd->restore) {
        for (ix = 0; ix < htd->offset_count; ix++) {
            if (htd->restore & (1 << htd->config_banks[ix])) {
                ...
                if (htd->opt_verbose) {
                    printk("Restore offset %d for bank %d\n", 
