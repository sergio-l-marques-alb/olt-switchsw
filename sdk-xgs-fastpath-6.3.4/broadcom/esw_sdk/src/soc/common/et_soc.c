...
#endif
    SOC_DEBUG_PRINT((DK_VERBOSE,"----- interrupt connect-------\n"));
...
et_soc_debug(int unit)
{
    ...
    if (SOC_KNET_MODE(et_soc->dev)) {
        soc_cm_print("txq_cnt %d txq_done_cnt %d\n",
            et_soc->txq_cnt,et_soc->txq_done_cnt);
        dv = et_soc->txq_done_head;    
        soc_cm_print("last_tx_seq_no %d tx_don_head %p\n",
            last_tx_seq_no, (void *)et_soc->txq_done_head);
        while (dv){
            soc_cm_print("desc_seqno %d desc_status 0x%x\n",
        ...
        for (i = 0; i < NUMRXQ; i++){
            soc_cm_print("chan:%d rxq_head %p rxq_tail %p rxq_cnt %d \n",
                i, (void *)et_soc->rxq_head[i], (void *)et_soc->rxq_tail[i],
                et_soc->rxq_cnt[i]);
            soc_cm_print("\t rxq_done_head %p rxq_done_tail %p "
                "last_rx_seq_no[%d] %d\n",
                (void *)et_soc->rxq_done_head[i], (void *)et_soc->rxq_done_tail[i],
                i,last_rx_seq_no[i]);
            soc_cm_print("\t knet_rxq_cnt %d, cmdrx_seqno %d\n",
...
et_soc_rx_chain(int unit, eth_dv_t *dv_chain)
{
    ...
    if (SOC_KNET_MODE(et_soc->dev)) {
        ...
        SOC_DEBUG_PRINT((DK_DMA,"chain updated rxq_head:%p rxq_tail %p \n"
...
et_soc_rx_chain_get(int unit, int chan, int flags)
{
    ...
    if (SOC_KNET_MODE(et_soc->dev)) {
        ...
        if (flags == ET_RXCHAIN_F_GET_FREE_DV){
            ...
            SOC_DEBUG_PRINT((DK_VERBOSE|DK_DMA,"rx_fill : %p\n",
                (void *)dv_avail));
            return dv_avail;
        }
        ...
        if (flags == ET_RXCHAIN_F_GET_DONE_DV) { 
            if(et_soc->rxq_head[chan] == et_soc->rxq_done_head[chan]){
                SOC_DEBUG_PRINT((DK_DMA,"done_chain rxq_head:%p rxq_tail %p \n"
                    "rxq_done_head:%p rxq_done_tail:%p\n",
                    (void *)et_soc->rxq_head[chan],(void *)et_soc->rxq_tail[chan],
                    (void *)et_soc->rxq_done_head[chan],(void *)et_soc->rxq_done_tail[chan]));

                SOC_DEBUG_PRINT((DK_VERBOSE,"return NULL dv \n"));
                return NULL;
            } else {           
...
et_soc_knet_rx_event_update(int unit, int chan, uint32 seq_no)
{
    ...
    if (last_rx_seq_no[chan] != seq_no){
        ...
        while (dv_avail){
            ...
            if (dv_avail == et_soc->rxq_done_tail[chan]){
                SOC_DEBUG_PRINT((DK_VERBOSE|DK_DMA,
        ...
        if (found) {
            SOC_DEBUG_PRINT((DK_DMA,"before rxq_head:%p rxq_tail %p \n"
                "rxq_done_head:%p rxq_done_tail:%p\n",
                (void *)et_soc->rxq_head[chan],(void *)et_soc->rxq_tail[chan],
                (void *)et_soc->rxq_done_head[chan],(void *)et_soc->rxq_done_tail[chan]));
            ...
            SOC_DEBUG_PRINT((DK_DMA,"dv_avail %p next %p\n",
                (void *)dv_avail,(void *)dv_avail->dv_next));

            SOC_DEBUG_PRINT((DK_DMA,"updated rxq_head:%p rxq_tail %p \n"
                "rxq_done_head:%p rxq_done_tail:%p\n",
                (void *)et_soc->rxq_head[chan],(void *)et_soc->rxq_tail[chan],
                (void *)et_soc->rxq_done_head[chan],(void *)et_soc->rxq_done_tail[chan]));
    ...
    SOC_DEBUG_PRINT((DK_DMA,"event update rxq_done_head[%d]:%p" 
...
void et_soc_knet_rxfill(int unit, int chan)
{
    ...
    while ((dv = et_soc_rx_chain_get(unit, chan, ET_RXCHAIN_F_GET_FREE_DV))){
        ...
        SOC_DEBUG_PRINT((DK_VERBOSE,"set RXFILL dv %p seq_no %d dcb %x\n",
...
et_soc_done_knet_rx(int unit, int chan)
{
    ...
    while ((dv = et_soc_rx_chain_get(unit, chan, ET_RXCHAIN_F_GET_DONE_DV))){
        ...
        SOC_DEBUG_PRINT((DK_VERBOSE,"et_soc_done_knet_rx ==> sendup \n"));    
        et_soc_sendup(et_soc, dv);
    
        if (++sendup > 16) {
            SOC_DEBUG_PRINT((DK_DMA,"et_soc_done_knet_rx ==> sendup %d\n\n",sendup)); 
