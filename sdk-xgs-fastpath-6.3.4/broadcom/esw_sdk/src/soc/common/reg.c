...
void _soc_snoop_reg(int unit, soc_block_t block, int acc, uint32 addr, 
    ...
    if (soc_cm_debug_check(DK_TESTS) == 0) {
        return;
    }
    if ((block == 0) && (acc == 0)) {
               ...
               uint32 addr, soc_reg_above_64_val_t data)
{
    ...
    for(i=SOC_REG_ABOVE_64_MAX_SIZE_U32-1 ; i>=0 ; i--) {
        if(0 == i) {
            ...
            if(1 == first_non_zero) {
                soc_cm_debug(DK_REG,"0x%08x_",data[i]);
            }
        }
    }
...
_soc_reg64_get(int unit, soc_block_t block, int acc, uint32 addr, uint64 *reg)
{
    ...
    schan_msg.readcmd.header.srcblk = acc;

#if defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
...
soc_direct_reg_get(int unit, int cmic_block, uint32 addr, uint32 dwc_read, uint32 *data)
{
    ...
    schan_msg_clear(&schan_msg);
    schan_msg.readcmd.header.opcode = READ_REGISTER_CMD_MSG;
    schan_msg.readcmd.header.datalen = dwc_read * 4;
    schan_msg.readcmd.header.srcblk = SOC_BLOCK2SCH(unit, CMIC_BLOCK(unit));
    schan_msg.readcmd.header.dstblk = cmic_block;   
#ifdef BCM_CMICM_SUPPORT
    ...
    for(i = 0; i <  (schan_msg.readresp.header.datalen/4) ; i++)
...
soc_direct_reg_set(int unit, int cmic_block, uint32 addr, uint32 dwc_write, uint32 *data)
{
    ...
    schan_msg.writecmd.header.datalen = dwc_write * 4;
    schan_msg.writecmd.header.dstblk = cmic_block;
#ifdef BCM_CMICM_SUPPORT
    if(soc_feature(unit, soc_feature_cmicm)) {
...
#if defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
    /* required on XGS3. Optional on other devices */ 
    schan_msg.readcmd.header.dstblk = block;
#endif /* BCM_XGS3_SWITCH_SUPPORT || defined(BCM_SIRIUS_SUPPORT) */
...
soc_reg_get(int unit, soc_reg_t reg, int port, int index, uint64 *data)
{
    ...
    addr = soc_reg_addr_get(unit, reg, port, index, &block, &acc_type);
    if (SOC_REG_IS_64(unit, reg)) {
        soc_port_t _port;
        soc_block_types_t regblktype = SOC_REG_INFO(unit, reg).block;
        int blk, bindex;
               ...
               uint32 *data)
{
...
#endif
    {
        ...
        /* Check result */
        if (!SOC_FAILURE(rv) && 
            ((schan_msg.readresp.header.opcode != READ_REGISTER_ACK_MSG) 
            || (schan_msg.readresp.header.ebit))) {
            soc_cm_debug(DK_ERR,
                         "soc_reg32_read: "
                         "invalid S-Channel reply, expected READ_REG_ACK:\n");
...
#if !defined(SOC_NO_NAMES)
        soc_cm_print("reg %s is > 32 bit , but called with soc_reg32_get\n", soc_reg_name[reg]);
#endif
    }
...
#if defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
        /* required on XGS3. Optional on other devices */ 
        schan_msg.readcmd.header.dstblk = ((addr >> SOC_BLOCK_BP) & 0xf) | 
                                      (((addr >> SOC_BLOCK_MSB_BP) & 0x3) << 4);
#endif /* BCM_XGS3_SWITCH_SUPPORT || defined(BCM_SIRIUS_SUPPORT) */
...
_soc_reg64_set(int unit, soc_block_t block, int acc, uint32 addr, uint64 data)
{
    ...
    schan_msg.writecmd.address = addr;
    schan_msg.writecmd.header.srcblk = acc;
                ...
                uint32 data)
{
    ...
    schan_msg.writecmd.header.dstblk = ((addr >> SOC_BLOCK_BP) & 0xf) | 
                                  (((addr >> SOC_BLOCK_MSB_BP) & 0x3) << 4);
#endif /* BCM_XGS3_SWITCH_SUPPORT || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)*/
    schan_msg.writecmd.data[0] = data;
...
#if !defined(SOC_NO_NAMES)
        soc_cm_print("reg %s is not 32 bit\n", soc_reg_name[reg]);
#endif
#endif
                  ...
                  soc_field_t field, uint32 value)
{
   ...
   if  (VALUE_TOO_BIG_FOR_FIELD) {
#if !defined(SOC_NO_NAMES)
        soc_cm_print("reg %s field %s is too big\n",
                     soc_reg_name[reg], soc_fieldnames[field]);
#endif
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    base = SOC_REG_INFO(unit, reg).offset;
    soc_cm_debug(DK_REG+DK_VERBOSE, "base: %x ", base);
...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    *acc_type = (SOC_REG_INFO(unit, reg).flags >> SOC_REG_FLAG_ACCSHIFT) & 7;
    ...
    *blk = 0;/* not really needed, just to avoid coverity defect */
    ...
    if (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype)) {
        ...
        case SOC_BLK_IDR:
            ...
            break;
        case SOC_BLK_IHB:
            block = IHB_BLOCK(unit);
            break;
        case SOC_BLK_IHP:
            block = IHP_BLOCK(unit);
            break;
        case SOC_BLK_IPS:
            block = IPS_BLOCK(unit);
            break;
        case SOC_BLK_IPT:
            block = IPT_BLOCK(unit);
            break;
        case SOC_BLK_IQM:
            block = IQM_BLOCK(unit);
            break;
        case SOC_BLK_IRE:
        ...
        case SOC_BLK_RTP:
            ...
            break;
        case SOC_BLK_SCH:
            block = SCH_BLOCK(unit);
            break;
        case SOC_BLK_EPNI:
            block = EPNI_BLOCK(unit);
            break;
        case SOC_BLK_DRCA:
...
#endif /* defined(BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined(BCM_PETRAB_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) */
