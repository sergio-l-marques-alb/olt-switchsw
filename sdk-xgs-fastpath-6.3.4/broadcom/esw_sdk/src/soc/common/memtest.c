/*
 * $Id: memtest.c 1.45.2.1 Broadcom SDK $
...
ser_test_mem_write(int unit, ser_test_data_t *test_data)
{
    ...
    if (test_data->acc_type != _SOC_ACC_TYPE_PIPE_ANY) {
        rv = soc_mem_pipe_select_write(unit, test_data->mem, test_data->mem_block,
                test_data->acc_type, test_data->index, test_data->entry_buf);
    } else {
...
ser_test_mem_read(int unit, ser_test_data_t *test_data)
{
    ...
    if (test_data->acc_type == _SOC_ACC_TYPE_PIPE_Y) {
        rv = soc_mem_pipe_select_read(unit, test_data->mem, test_data->mem_block,
                            ...
                            soc_port_t port, soc_acc_type_t acc_type, int *error_count)
{
    ...
    soc_ser_create_test_data(unit, tmp_entry, field_data, parity_enable_reg,
                             ...
                             mem_block, port, acc_type, &test_data);
 ...
 */
void
soc_ser_create_test_data(int unit, uint32 *tmp_entry, uint32 *fieldData,
                              ...
                              ser_test_data_t *test_data)
{
    ...
    /*An INVALIDf passed in for test_field implies we should select any valid field.*/
    if ((test_field == INVALIDf || test_field == 0) &&
        (test_data->mem_info != NULL)) {
        test_data->test_field = test_data->mem_info->fields[test_data->mem_info->nFields-1].field;
    } else {
        test_data->test_field = test_field;
    }
    /*If an invalid test field was selected, attempt to replace it with a valid one.*/
 ...
 */
soc_error_t
soc_ser_inject_error(int unit, soc_mem_t mem, int pipe,
                     int blk, int index)
{
    if( ser_test_functions[unit] != NULL &&
        ser_test_functions[unit]->inject_error_f != NULL) {
        return (*(ser_test_functions[unit]->inject_error_f))(unit, mem,
                    pipe, blk, index);
    }
    return SOC_E_FAIL;
}
