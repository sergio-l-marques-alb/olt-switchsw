 ...
 * $Id: counter.c 1.227.2.9 Broadcom SDK $
                 ...
                 int zero, int sync_mode, uint64 *val)
{
    ...
    if (sync_mode == TRUE) {
        ...
        if (!SOC_COUNTER_INVALID(unit, ctr_reg)) {
          ...
          soc_counter_collect_non_dma_entries(unit, port, port_index,
                                              ar_idx, ctr_reg);    
    ...
    for (; id < non_dma_counter_end; id++) {
        ...
        if (non_dma->mem != INVALIDm) {
            ...
            if (ctr_reg == INVALIDr) {
                ...
                /* Coolect counter values for a specific port */
                dma_mem = non_dma->dma_mem[0];
...
soc_counter_thread(void *unit_vp)
{
    ...
    while ((interval = soc->counter_interval) != 0) {
         ...
         */
        if ((soc->counter_interval)) {
            soc_counter_collect_non_dma_entries(unit, -1, -1, -1, -1);
        }
        ...
        if (err) {
                soc_cm_debug(DK_ERR, "AbnormalThreadExit:"
