/*
 * $Id: cosq.c 1.74.2.4 Broadcom SDK $
...
soc_td2_init_invalid_pointers(int unit)
{
    ...
    if (_td2_invalid_ptr[unit][SOC_TD2_NODE_LVL_L0] > SOC_TD2_NUM_MMU_PORTS_PER_PIPE) {
        return SOC_E_PARAM;
    }
...
int soc_td2_lb_lls_init(unit)
{
    ...
    rv = soc_td2_port_lls_init(unit, LB_PORT(unit), _td2_port_lls_config, 1);
    if (rv < 0) {
        return SOC_E_INTERNAL;
    }
    return 0;
        ...
        if ((l0_index >= 1) && (l0_index <= 4)) {
            for (l1_index = 0; l1_index < 8; l1_index++) {
                if (conn_map & (1U<<l1_index)) {               
                    ...
                    soc_cm_print("    L2.uc : INDEX=%d Mode=%s WEIGHT=%d\n",
                    ...
                    if (((mc_group_mode == 1) && (l0_index == 4)) ||
                        ...
                        soc_cm_print("    L2.mc : INDEX=%d Mode=%s WEIGHT=%d\n",
