/*
 * $Id: katana.c 1.152 Broadcom SDK $
                      ...
                      _soc_katana_mmu_sub_block_type_1_info_t * sub_block_info)
{
    ...
    SOC_IF_ERROR_RETURN(READ_CI_ERRORr(unit, port, &en_rval));
    stat_rval &= (~en_rval);
    if (stat_rval == 0) {
        /* How did this interrupt get asserted then??? */
        return SOC_E_NONE; 
...
STATIC _soc_ser_parity_info_t _soc_kt_ser_parity_info[] = {
...
{
    return soc_ser_init(unit, _soc_kt_ser_parity_info,
...
{
    return soc_ser_mem_clear(unit, _soc_kt_ser_parity_info, mem);
}
...
{
    soc_process_ser_parity_error(unit, _soc_kt_ser_parity_info,
                                 _SOC_PARITY_TYPE_SER);
    return;
}
...
_soc_katana_mmu_init_helper(int unit)
{
    ...
    int           saber_2hg_8ge_config = 0;
    uint16        dev_id = 0;
    uint8         rev_id = 0;

    soc_cm_get_id(unit, &dev_id, &rev_id);
    if ((soc_property_get(unit, spn_BCM5644X_CONFIG, 0) == 1) &&
        ((dev_id == BCM56240_DEVICE_ID) ||
         (dev_id == BCM56241_DEVICE_ID) ||
         (dev_id == BCM56242_DEVICE_ID) ||
         (dev_id == BCM56243_DEVICE_ID))) {
        saber_2hg_8ge_config = 1;
    }
    ...
    ka_mmu_params.num_replication_per_pkt = 27; /*c45*/
    ...
    intbuf_pool_size = (ka_mmu_params.available_intbuf * max_intbuf_size) / 100; /*c90*/
    emapool_size = max_intbuf_size - intbuf_pool_size; /*c91*/
    ...
    if (all_ext_mem_port && saber_2hg_8ge_config) {
        ipthd_per_pg_min_intbuf_ingpool = 9;/*c144*/
    }
    ...
    ipthd_total_shared_eqe = (soc_feature(unit,
                          soc_feature_mmu_reduced_internal_buffer) ?
                          SB_MMU_EGRESS_QUEUE_ENTRIES :
                          KA_MMU_EGRESS_QUEUE_ENTRIES) * 1024 -
    ...
    opthd_total_shared_queue_eqe = (soc_feature(unit,
                                    soc_feature_mmu_reduced_internal_buffer) ?
                                    SB_MMU_EGRESS_QUEUE_ENTRIES :
                                    KA_MMU_EGRESS_QUEUE_ENTRIES) * 
    ...
    soc_cm_print("\nipthd_total_hdrm_ema %d", ipthd_total_hdrm_ema);
    ...
    for(i = 0; i < SOC_REG_NUMELS(unit, OP_QUEUE_CONFIG1_THDORQEIr); i++) {
        ...
        if ((i >= 0) && (i < 8)) { /*MC traffic queue 0 to 7*/
...
#define QENTRY_OFFSET       64
    ...
    PBMP_ALL_ITER(unit, port) {
        pg_num = IS_MXQ_PORT(unit, port) ? 7 : 0;
    ...
    PBMP_MXQ_ITER(unit, port) {
        ...
        SOC_IF_ERROR_RETURN(WRITE_THDIEMA_PG_MIN_CELLr(unit, port, 7, rval));
        soc_reg_field_set(unit, PG_MIN_CELLr, &rval, PG_MINf, 0x31);
        SOC_IF_ERROR_RETURN(WRITE_PG_MIN_CELLr(unit,port,7,rval));
        SOC_IF_ERROR_RETURN(WRITE_THDIRQE_PG_MIN_CELLr(unit, port, 7, rval));
        SOC_IF_ERROR_RETURN(WRITE_THDIQEN_PG_MIN_CELLr(unit, port, 7, rval));
