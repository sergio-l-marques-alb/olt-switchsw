                      ...
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    DRV_FP_VERB(("_drv_tbx_ivm_qual_value_set : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_drv_tbx_ivm_qual_value_set : data= 0x%x, 0x%x, \n",
                      ...
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    DRV_FP_VERB(("_drv_tbx_ivm_qual_value_get : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_drv_tbx_ivm_qual_value_get : data= 0x%x, 0x%x, \n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
                      ...
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    DRV_FP_VERB(("_drv_tbx_evm_qual_value_set : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_drv_tbx_evm_qual_value_set : data= 0x%x, 0x%x,\n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
                      ...
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    DRV_FP_VERB(("_drv_tbx_evm_qual_value_get : fld_index = 0x%x\n", fld_index));
    SOC_IF_ERROR_RETURN(
  ...
  DRV_FP_VERB(("_drv_tbx_evm_qual_value_get : data= 0x%x, 0x%x,\n",
        drv_entry->key_data[0], drv_entry->key_data[1]));
...
_drv_tb_cfp_entry_slice_id_change(int unit, drv_cfp_entry_t *drv_entry, int new_id)
{
    ...
    if (j > 0) {
        ...
        DRV_FP_VERB(("%s old_id %d new_id %d\n",FUNCTION_NAME(), old_id, new_id));        
        for (i = 0; i < COUNTOF(cfp_qual_add); i++) {
            if (cfp_qual_add[i] != -1) {
                sal_memset(p_data, 0, 4 * sizeof(uint32)); 
                sal_memset(p_mask, 0, 4 * sizeof(uint32)); 
    ...
    DRV_FP_VVERB(("%s set new slice_id %d to tcam\n", FUNCTION_NAME(),temp));
    SOC_IF_ERROR_RETURN(
        DRV_CFP_FIELD_SET
            ...
            drv_cfp_entry_t *drv_entry, int *slice_id)
{
    ...
    DRV_FP_VVERB(("DRV_FP: %s slice_id %d sliceid_map %x flags %x flag_mask %x qual %d\n",
    ...
    if (flags  ==  _DRV_CFP_FRAME_ANY) {
        ...
        if (valid == -1) {
            conflict = TRUE;
            DRV_FP_VVERB(("common qual only for _DRV_CFP_FRMAE_ANY\n"));
            goto end;
        } else {            
            for ( i = 0; i < COUNTOF(_tb_frame_any_qset); i++){
            ...
            if (qual_count != 0) {
                conflict = TRUE;
                DRV_FP_VVERB(("_DRV_CFP_FRMAE_ANY is not allowed since the non-common qual has been existed.\n"));
                goto end;
            }
        }
        ...
        for (i = 0; i < CFP_53280_SLICE_CHAIN_UDF_BASE; i++){
            if (SHR_BITGET(temp_qset.udf_map, i) != 0){
                conflict = TRUE;
                DRV_FP_VVERB(("_DRV_CFP_FRMAE_ANY is not allowed since the udf has been existed.\n"));
                goto end;
            }
        }
        ...
        if (match_slice & CFP_53280_SLICE_MAP_FRAME_ANY){
            ...
            if (sliceid_map & match_slice){
                ...
                if(new_id == -1) {
                    conflict = TRUE;
                    DRV_FP_VVERB(("_DRV_CFP_FRMAE_ANY is not allowed since no availabe slice match.\n"));
                    goto end;
                }
                update = TRUE;
            }
    ...
    DRV_FP_VVERB(("%s flag: _DRV_CFP_FRAME_ANY new_id %d (%d) new_frame %d (%d) slice_map %d\n",
        ...
        if (valid == 1) {
            DRV_FP_VVERB(("_DRV_CFP_FRAME_IPANY is not allowed since fix udf qual has been existed.\n"));
            conflict = TRUE;
            goto end;
        }
        for (i = 0; i < CFP_53280_SLICE_CHAIN_UDF_BASE; i++){
            if (SHR_BITGET(temp_qset.udf_map, i) != 0){
                conflict = TRUE;
                DRV_FP_VVERB(("_DRV_CFP_FRAME_IPANY is not allowed since the udf has been existed.\n"));
                goto end;
            }
        }    
    ...
    if (match_slice & flags_mask){
        ...
        if (sliceid_map & match_slice){
            ...
            if(new_id == -1) {
                conflict = TRUE;
                DRV_FP_VVERB(("%s is not allowed since no availabe slice match.\n",flag_name));
                goto end;
            }
        }
        update = TRUE;
    ...
    DRV_FP_VVERB(("DRV_FP: %s id %d flags %x \n",FUNCTION_NAME(),id, drv_entry->flags)); 
    return SOC_E_NONE;
}
            ...
            drv_cfp_entry_t *drv_entry, int *slice_id)
{
    ...
    if (drv_entry->flags & _DRV_CFP_FIX_UDF_ALL_VALID) {
        for (i = 0; i <COUNTOF(_tb_qual_udf_map); i ++) {
            if (TEST_CFP_53280_FIX_UDF(drv_entry, i)) {
                DRV_FP_VVERB(("DRV_FP: _tb_qual_udf_map %d\n", i));
                      ...
                      uint32 *p_data, uint32 *p_mask)
{
    ...
    DRV_FP_VERB(("_field_qual_value_set : fld_index = 0x%x\n", fld_index));
        ...
        case drvFieldActionNewClassId:           
            if (param1 == _DRV_FP_ACTION_PRIVATE){
                if (param0 <= MAX_USER_FLOW_ID(unit)) {
                    DRV_FP_ERR(("DRV_FP: private used flow id %d is out of range\n",param0));
                    return SOC_E_PARAM;
                }
            } else {
                if (param0 > MAX_USER_FLOW_ID(unit)){
                    DRV_FP_ERR(("DRV_FP: flow id %d is out of range\n",param0));
                    return SOC_E_PARAM;
                }
            }
...
_drv_tbx_fp_entry_alloc(int unit, int stage_id, void **entry)
{
    ...
    *entry = sal_alloc(memsize, "field drv entry");
    ...
    DRV_FP_VVERB(("DRV_FP: _tb_fp_entry_alloc %p\n",*entry));
    return SOC_E_NONE;
}
...
_drv_tbx_fp_entry_tcam_block_move(int unit, int stage_id, void *tcam_idx_old, int dest_index, void *amount)
{
    ...
    DRV_FP_VVERB(("DRV_FP: %s slice_idx src: %d  dst:%d amount %d\n",
...
_drv_tbx_fp_entry_tcam_move(int unit, int stage_id, void *drv_entry, int amount, void *counter)
{
    ...
    DRV_FP_VVERB(("DRV_FP: %s slice_idx old: %d  new:%d amount %d\n",
...
_drv_tbx_fp_entry_ivm_tcam_policy_install(int unit, void *entry, int tcam_idx)
{
   ...
   DRV_VM_FIELD_SET
    ...
    if (SOC_FAILURE(retval)) {
        DRV_FP_ERR(("FP Error: Write IVM Act entry index = %d fail.\n",  
            tcam_idx));
        return retval;
    }
    retval = DRV_VM_ENTRY_WRITE
    ...
    if (SOC_FAILURE(retval)) {
        DRV_FP_ERR(("FP Error: Write IVM Key entry index = %d fail.\n",  
            tcam_idx));
        return retval;
    }
...
_drv_tbx_fp_entry_evm_tcam_policy_install(int unit, void *entry, int tcam_idx)
{
   ...
   DRV_VM_FIELD_SET
    ...
    if (SOC_FAILURE(retval)) {
        DRV_FP_ERR(("FP Error: Write EVM Act entry index = %d fail.\n",  
            tcam_idx));
        return retval;
    }
    retval = DRV_VM_ENTRY_WRITE
    ...
    if (SOC_FAILURE(retval)) {
        DRV_FP_ERR(("FP Error: Write EVM Key entry index = %d fail.\n",  
            tcam_idx));
        return retval;
    }
                              ...
                              int *tcam_chain_idx, int enable)
{
    ...
    DRV_FP_VVERB(("DRV_FP: %s tcam_idx %d chain_idx %d\n",FUNCTION_NAME(),
...
_drv_tbx_fp_id_control_init(int unit)
{
    ...
    if (MAX_USER_FLOW_ID(unit) > (VM_FLOW_SIZE(unit) - 2)){
        ...
        SOC_ERROR_PRINT((DK_ERR, "TB_FLOW_ID_SIZE should't large than %d\n",
            VM_FLOW_SIZE(unit) - 2));
        return SOC_E_CONFIG;
    }
    return SOC_E_NONE;
...
drv_tbx_fp_tcam_parity_check(int unit, drv_fp_tcam_checksum_t *drv_fp_tcam_chksum)
{
    ...
    SOC_IF_ERROR_RETURN(rv);
    DRV_FP_VVERB(("DRV_FP: %s reg_val hi %x lo %x\n",
...
_drv_tbx_fp_policer_config_b0(int unit, void *entry, drv_policer_config_t *policer_cfg)
{
    ...
    DRV_FP_VERB(("%s mode %d CIR %s EIR %s \n", FUNCTION_NAME(), policer_cfg->mode,
...
_drv_tbx_stat_mode_support(int unit, int stage_id, drv_policer_mode_t policer_mode, void *mode)    
{
    ...
    type1 = type2 = type3 =-1;
    rv = drv_tbx_fp_stat_type_get(unit, stage_id, f_stat, policer_mode, &type1, &type2, &type3);
    return rv;
}
