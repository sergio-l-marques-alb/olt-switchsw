...
_drv_gex_arl_mem_clear(int unit, int mcast)
{
    ...
    if (soc_feature(unit, soc_feature_mac_learn_limit)) {
        ...
        if (SOC_FAILURE(rv)){
            soc_cm_debug(DK_WARN, "Failed on reset ARL LRN_CNT!\n");
            return SOC_E_INTERNAL;
        }
    }
        ...
        uint32 flags)
{
    ...
    soc_cm_debug(DK_MEM, "%s:mem=0x%x,flags=0x%x,entry[0-2]=%08x-%08x-%08x\n", 
    ...
    if (soc_feature(unit, soc_feature_mac_learn_limit)) {
        ...
        */
        soc_cm_debug(DK_ARL,"%s,%d, SA learn count must review!\n",
                FUNCTION_NAME(), __LINE__);
