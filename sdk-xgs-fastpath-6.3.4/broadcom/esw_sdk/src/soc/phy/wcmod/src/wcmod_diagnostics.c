/*
 * $Id: wcmod_diagnostics.c 1.63 Broadcom SDK $
 *
...
int wcmod_diag_topology(wcmod_st *ws)
{
  ...
  if (data & XGXSBLK0_XGXSCONTROL_RLOOP_MASK) {
    strcpy(msg, "Rloop enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  } else {
    strcpy(msg, "Rloop NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  }
  /* Check G-loop */
  READ_WC40_COMBO_IEEE0_MIICNTLr(ws->unit, ws, &data);
  if (data & COMBO_IEEE0_MIICNTL_GLOOPBACK_MASK) {
    strcat(msg, "Gloop enabled in COMBO_IEEE0_MIICNTL_NAME\n");
  } else {
    strcat(msg, "Gloop NOT enabled in COMBO_IEEE0_MIICNTL_NAME\n");
  }
  ws->lane_num_ignore = 1;
  ...
  for (n = 0; n < 4; ++n) {
    ...
    if (data & (0x1u << (XGXSBLK1_LANECTRL2_RLOOP1G_SHIFT + n))) {
      strcat(msg, "Rloop-1G     enabled, ");
    } else {
      strcat(msg, "Rloop-1G NOT enabled, ");
    }

    /* Check G-loop 1g -- reads the same register as r-loop 1g */
    if (data & (0x1u << (XGXSBLK1_LANECTRL2_GLOOP1G_SHIFT + n))) {
      strcat(msg, "Gloop-1G     enabled in XGXSBLK1_LANECTRL2_NAME\n");
    } else {
      strcat(msg, "Gloop-1G NOT enabled in XGXSBLK1_LANECTRL2_NAME\n");
    }
  }
  ...
  strcat(msg,"TX digital-to-analog lane swaps in XGXSBLK8_TXLNSWAP1_NAME\n");
  for(n = 0; n < 4; ++n) {
  ...
  strcat(msg, "TX analog-to-digital lane swap in XGXSBLK8_TXLNSWAP2_NAME\n");
  for(n = 0; n < 4; ++n) {
  ...
  strcat(msg, "RX analog-to-digital lane swaps in XGXSBLK8_RXLNSWAP1_NAME\n");
  for(n = 0; n < 4; ++n) {
  ...
  strcat(msg, "RX digital-to-analog lane swap in XGXSBLK8_RXLNSWAP2_NAME\n");
  for(n = 0; n < 4; ++n) {
  ...
  if (data & TX0_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strcat(msg, "Lane 0: TX polarity flipped in TX0_ANATXACONTROL0_NAME\n");
  } else {
    strcat(msg, "Lane 0: TX polarity NOT flipped in TX0_ANATXACONTROL0_NAME\n");
  }
  ...
  if (data & RX0_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strcat(msg,"Lane 0: RX polarity flipped in RX0_ANARXCONTROLPCI_NAME\n");
  } else {
    strcat(msg,"Lane 0: RX polarity NOT flipped in RX0_ANARXCONTROLPCI_NAME\n");
  }
  ...
  if (data & TX1_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strcat(msg,"Lane 1: TX polarity flipped in TX1_ANATXACONTROL0_NAME \n");
  } else {
    strcat(msg,"Lane 1: TX polarity NOT flipped in TX1_ANATXACONTROL0_NAME\n");
  }
  ...
  if (data & RX1_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strcat(msg,"Lane 1: RX polarity     flipped in RX1_ANARXCONTROLPCI_NAME\n");
  } else {
    strcat(msg,"Lane 1: RX polarity NOT flipped in RX1_ANARXCONTROLPCI_NAME\n");
  }
  ...
  if (data & TX2_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strcat(msg,"Lane 2: TX polarity     flipped in TX2_ANATXACONTROL0_NAME \n");
  } else {
    strcat(msg,"Lane 2: TX polarity NOT flipped in TX2_ANATXACONTROL0_NAME\n");
  }
  ...
  if (data & RX2_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strcat(msg,"Lane 2: RX polarity     flipped in RX2_ANARXCONTROLPCI_NAME\n");
  } else {
    strcat(msg,"Lane 2: RX polarity NOT flipped in RX2_ANARXCONTROLPCI_NAME\n");
  }
  ...
  if (data & TX3_ANATXACONTROL0_TXPOL_FLIP_MASK) {
    strcat(msg,"Lane 3: TX polarity     flipped in TX3_ANATXACONTROL0_NAME \n");
  } else {
    strcat(msg,"Lane 3: TX polarity NOT flipped in TX3_ANATXACONTROL0_NAME \n");
  }
  ...
  if (data & RX3_ANARXCONTROLPCI_RX_POLARITY_R_MASK) {
    strcat(msg,"Lane 3: RX polarity     flipped in RX3_ANARXCONTROLPCI_NAME\n");
  } else {
    strcat(msg,"Lane 3: RX polarity NOT flipped in RX3_ANARXCONTROLPCI_NAME\n");
  }
  printf("%s\n", msg);
  return SOC_E_NONE;
}
...
int wcmod_diag_general(wcmod_st *ws)
{
  ...
  case 0:  strcat(msg, "A"); break;
  case 1:  strcat(msg, "B"); break;
  case 2:  strcat(msg, "C"); break;
  case 3:  strcat(msg, "D"); break;
  ...
  case MODEL_SERDES_CL73_COMBO_CORE:
                             strcat(msg, "Serdes_cl73_combo_core"); break;
  case MODEL_XGXS_16G    :   strcat(msg, "XGXS_16G");               break;
  case MODEL_HYPERCORE   :   strcat(msg, "Hypercore");              break;
  case MODEL_HYPERLITE   :   strcat(msg, "Hyperlite");              break;
  case MODEL_PCIE_G2_PIPE:   strcat(msg, "PCIE_G2_PIPE");           break;
  case MODEL_1p25GBd_SERDES: strcat(msg, "1.25GBd_Serdes");         break;
  case MODEL_SATA2      :    strcat(msg, "Sata2");                  break;
  case MODEL_QSGMII     :    strcat(msg, "QSGMII");                 break;
  case MODEL_XGXS10G    :    strcat(msg, "XGXS10G");                break;
  case MODEL_WARPCORE   :    strcat(msg, "Warpcore");               break;
  case MODEL_XGXS_CL73  :    strcat(msg, "XGXS_CL73");              break;
  case MODEL_SERDES_CL73:    strcat(msg, "Serdes_cl73");            break;
  case MODEL_WARPLITE   :    strcat(msg, "WARPLITE");               break;
  case MODEL_QUADSGMII  :    strcat(msg, "QUADSGMII");              break;
  case MODEL_WARPCORE_C :    strcat(msg, "WARPCORE_C");             break;
  default               :    strcat(msg, "UNKNOWN");                break;
  }
  printf ("%s\n", msg); strcpy (msg, "");
  ...
  if (data & PLL_ANAPLLSTATUS_PLLSEQDONE_MASK) {
    strcpy (msg,"PLL startup sequence done\n");
  } else {
    strcpy (msg,"PLL startup sequence not done\n");
  }

  if (data & PLL_ANAPLLSTATUS_PLLSEQPASS_MASK) {
    strcat (msg,"PLL startup sequence passed and PLL is locked\n");
  } else {
    strcat (msg,"PLL startup sequence FAILED. PLL Lock status UNKNOWN\n");
  }

  /* PLL frequence detection sequence */
  if (data & PLL_ANAPLLSTATUS_FREQDONE_MASK) {
    strcat (msg,"PLL frequency detection sequence done\n");
  } else {
    strcat (msg,"PLL frequency detection sequence NOT done\n");
  }

  if (data & PLL_ANAPLLSTATUS_FREQPASS_MASK) {
    strcat (msg,"PLL frequency detection sequence passed\n");
  } else {
    strcat (msg,"PLL frequency detection sequence FAILED\n");
  }

  /* VCO range selection sequence */
  if (data & PLL_ANAPLLSTATUS_VCODONE_MASK) {
    strcat (msg,"VCO range selection sequence done\n");
  } else {
    strcat (msg,"VCO range selection sequence NOT done\n");
  }

  if (data & PLL_ANAPLLSTATUS_VCOPASS_MASK) {
    strcat (msg,"VCO range selection sequence passed\n");
  } else {
    strcat (msg,"VCO range selection sequence FAILED\n");
  }
  ...
  */
  ...
  /* Frequency divider ratio */
    strcpy (msg,"PLL frequency divider = ");
  switch((data & PLL_ANAPLLSTATUS_PLL_MODE_AFE_MASK) >> PLL_ANAPLLSTATUS_PLL_MODE_AFE_SHIFT) {
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div32: strcat(msg,"32");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div36: strcat(msg,"36");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div40: strcat(msg,"40");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div42: strcat(msg,"42");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div48: strcat(msg,"48");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div50: strcat(msg,"50");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div52: strcat(msg,"52");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div54: strcat(msg,"54");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div60: strcat(msg,"60");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div64: strcat(msg,"64");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div66: strcat(msg,"66");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div68: strcat(msg,"68");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div70: strcat(msg,"70");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div80: strcat(msg,"80");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div92: strcat(msg,"92");break;
   case PLL_ANAPLLSTATUS_PLL_MODE_AFE_div100:strcat(msg,"100");break;
   default:                              strcat(msg,"UNKNOWN");break;
  }
  printf ("%s", msg); strcpy (msg, "");
  ...
  if (data & XGXSBLK0_XGXSCONTROL_PLL_BYPASS_MASK) {
    strcpy(msg,"PLL bypassedin XGXSBLK0_XGXSCONTROL_NAME\n");
  } else {
    strcpy(msg,"PLL not bypassed in XGXSBLK0_XGXSCONTROL_NAME\n");
  }
  printf ("%s", msg); strcpy (msg, "");
  ...
  if (data & GP2_REG_GP2_8_MICRO_INIT_DONE_MASK) {
    strcpy(msg,"Micro-controller init done");
  } else {
    strcpy(msg,"Micro-controller init NOT done");
  }
  printf ("%s", msg); strcpy (msg, "");
  return SOC_E_NONE;
}
...
int wcmod_diag_link(wcmod_st *ws)
{
    ...
    if (cntl == 0x1) {
        ...
        strcat(msg, "Lane 0:  RX sequencer done\n");
        } else {
        strcat(msg, "Lane 0:  RX sequencer NOT done\n");
        }

        if (data & RX0_ANARXSTATUS_SYNC_STATUS_SYNC_STATUS_MASK) {
        strcat(msg, "\t     Synced\n");
        } else {
        strcat(msg, "\t NOT Synced\n");
        }
        ...
        strcat(msg, "Lane 1:  RX sequencer done\n");
        } else {
        strcat(msg, "Lane 1:  RX sequencer NOT done\n");
        }
        ...
        strcat(msg, "Lane 2:  RX sequencer done\n");
        } else {
        strcat(msg, "Lane 2:  RX sequencer NOT done\n");
        }
        ...
        strcat(msg, "Lane 3:  RX sequencer done\n");
        } else {
        strcat(msg, "Lane 3:  RX sequencer NOT done\n");
        }
        ...
        case xgxsBlk0_operationModes_IndLane_OS8: strcat(msg, "Indep_OS8"); break;
        case xgxsBlk0_operationModes_IndLane_OS5:  strcat(msg, "Indep_OS5"); break;
        case XGXSBLK0_XGXSCONTROL_MODE_10G_ComboCoreMode:strcat(msg, "combo");break;
        default: strcat(msg, "UNKNOWN"); break;
        }
        ...
        if (strlen(getSpeedString(speed)) > sizeof(msg)) {
            ...
            strcat(msg, getSpeedString(speed));
            printf ("%s\n", msg);
        }
...
int wcmod_diag_internal_tfc(wcmod_st *ws)
{
  ...
  for (n = 0; n < 4; ++n) {
	...
	strcat(msg, ", NOT inverted\n");
      }
      /* Check PRBS order */
      switch ((data & (XGXSBLK1_LANEPRBS_PRBS_ORDER0_MASK << shift)) >> (shift + XGXSBLK1_LANEPRBS_PRBS_ORDER0_SHIFT)) {

       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs7:  strcat(msg, "PRBS 7") ;break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs15: strcat(msg, "PRBS 15");break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs23: strcat(msg, "PRBS 23");break;
       case XGXSBLK1_LANEPRBS_PRBS_ORDER0_prbs31: strcat(msg, "PRBS 31");break;
       default:                      strcat(msg, "PRBS order unknown\n");break;
      }
    } else {
  ...
  if (data & XGXSBLK0_XGXSCONTROL_PGEN_EN_MASK) {
    strcat(msg, "Pattern generator enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  } else {
    strcat(msg, "Pattern generator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  }

  if (data & XGXSBLK0_XGXSCONTROL_PCMP_EN_MASK) {
    strcat(msg, "Pattern comparator enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  } else {
    strcat(msg,"Pattern comparator NOT enabled in XGXSBLK0_XGXSCONTROL_NAME\n");
  }
  ...
  if (data & DIGITAL4_MISC5_TGEN_DATA_SEL_MASK) {
    strcat(msg,"Test generator data selected in DIGITAL4_MISC5_NAME\n");
  } else {
    strcat(msg,"Test generator data NOT selected in DIGITAL4_MISC5_NAME\n");
  }

  /* test link enabled */
  if (data & DIGITAL4_MISC5_TGEN_LINK_MASK) {
    strcat(msg,"Test generator link enabled in DIGITAL4_MISC5_NAME\n");
  } else {
    strcat(msg,"Test generator link NOT enabled in DIGITAL4_MISC5_NAME\n");
  }

  /* RX CRC */
  if (data & DIGITAL4_MISC5_RCHK_EN_MASK) {
    strcat(msg,"Rx crc enabled in DIGITAL4_MISC5_NAME\n");
  } else {
    strcat(msg,"Rx crc NOT enabled in DIGITAL4_MISC5_NAME\n");
  }
  ...
  if (data & LNRXBERT_RXBERTCTRL_CRCCHK_EN_MASK) {
    strcat(msg, "Rx crc enabled in RXBERT_RXBERTCTRL_NAME\n");
  } else {
    strcat(msg, "Rx crc NOT enabled in RXBERT_RXBERTCTRL_NAME\n");
  }
  ...
  if (data & TXBERT_TXBERTCTRL_CJPAT_EN_MASK) {
    strcat(msg, "CJPat enabled in TXBERT_TXBERTCTRL_NAME\n");
  } else {
    strcat(msg, "CJPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n");
  }

  /* CRPat */
  if (data & TXBERT_TXBERTCTRL_CRPAT_EN_MASK) {
    strcat(msg, "CRPat enabled in TXBERT_TXBERTCTRL_NAME\n");
  } else {
    strcat(msg, "CRPat NOT enabled in TXBERT_TXBERTCTRL_NAME\n");
  }
  ...
  if (data & TX0_ANATXACONTROL0_TX_MDATA_EN_MASK) {
    strcat(msg, "Programmable data enabled\n");
    ...
    strcat(msg, "Programmable data NOT enabled\n");
  }
...
int wcmod_diag_dfe(wcmod_st *ws)
{
  ...
  switch ((data & TX0_TX_DRIVER_IDRIVER_MASK) >> TX0_TX_DRIVER_IDRIVER_SHIFT) {
    case TX0_TX_DRIVER_IDRIVER_v680mV: strcat(msg, "680 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v730mV: strcat(msg, "730 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v780mV: strcat(msg, "780 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v830mV: strcat(msg, "830 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v880mV: strcat(msg, "880 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v930mV: strcat(msg, "930 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v980mV: strcat(msg, "980 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1010mV: strcat(msg, "1010 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1040mV: strcat(msg, "1040 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1060mV: strcat(msg, "1060 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1070mV: strcat(msg, "1070 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1080mV: strcat(msg, "1080 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1085mV: strcat(msg, "1085 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1090mV: strcat(msg, "1090 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1095mV: strcat(msg, "1095 mV\n"); break; 
    case TX0_TX_DRIVER_IDRIVER_v1100mV: strcat(msg, "1100 mV\n"); break; 
    default:                            strcat(msg, "UNKNOWN\n"); break; 
  }
  /* Taps */
  ...
  if (data & CL72_USERB0_CL72_TX_FIR_TAP_REGISTER_TX_FIR_TAP_FORCE_MASK) {
    strcat(msg, "TX fir taps forced\n");
  } else {
    strcat(msg, "TX fir taps NOT forced\n");
  }
...
int wcmod_diag_ieee(wcmod_st *ws)
{
  ...
  if (data & CL72_USERB1_CL72_FAULT_REGISTER_FRAME_LOCK_MASK) {
    strcpy(msg, "CL72 frame locked\n");
  } else {
    strcpy(msg, "CL72 frame NOT locked\n");
  }
...
int wcmod_diag_eee(wcmod_st *ws)
{
  ...
  if (data & CL73_USERB0_CL73_EEECTRL_CL73_EEEEN_MASK) {
    strcpy(msg, "EEE enabled\n");
  } else {
    strcpy(msg, "EEE NOT enabled\n");
  }

  if (data & CL73_USERB0_CL73_EEECTRL_HOLD_EEE_LP_ABILITY_MASK) {
    strcat(msg, "Link partner EEE enabled\n");
  } else {
    strcat(msg, "Link partner EEE NOT enabled\n");
  }
  ...
  if (data & XGXSBLK7_EEECONTROL_LPI_EN_RX_MASK) {
    strcat(msg, "RX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n");
  } else {
    strcat(msg, "RX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n");
  }

  if (data & XGXSBLK7_EEECONTROL_LPI_EN_TX_MASK) {
    strcat(msg,"TX LPI pass-thru enabled in XGXSBLK7_EEECONTROL_NAME\n");
  } else {
    strcat(msg,"TX LPI pass-thru NOT enabled in XGXSBLK7_EEECONTROL_NAME\n");
  }

  READ_WC40_EEE_USERB0_EEE_COMBO_CONTROL0r(ws->unit, ws, &data);

  if (data & EEE_USERB0_EEE_COMBO_CONTROL0_LPI_EN_FORCE_MASK) {
    strcat(msg, "LPI pass-thru forced\n");
  } else {
    strcat(msg, "LPI pass-thru derived from autoneg\n");
  }
  printf("%s\n", msg);
  return SOC_E_NONE;
}
...
wcmod_printfDigital3Up1(unsigned short int data)
{
    char msg[128] = {0};
    strcpy(msg, "\t ") ;
    if(data&0x1)   strcat(msg,"2P5G,"); 
    if(data&0x2)   strcat(msg,"5G_X4,"); 
    if(data&0x4)   strcat(msg,"6G_X4,"); 
    if(data&0x8)   strcat(msg,"10G_HIGIG,"); 
    if(data&0x10)  strcat(msg,"10G_CX4,"); 
    if(data&0x20)  strcat(msg,"12G_X4,"); 
    if(data&0x40)  strcat(msg,"12P5_X4,"); 
    if(data&0x80)  strcat(msg,"13G_X4,"); 
    if(data&0x100) strcat(msg,"15G_X4,"); 
    if(data&0x200) strcat(msg,"16G_X4,"); 
    if(data&0x400) strcat(msg,"20G_X4,"); 
    printf("%s\n", msg); 
}
/* need to review */
STATIC void 
wcmod_printfDigital3Up3(uint16 data)
{
    char msg[128] = {0};
    strcpy(msg, "\t ") ;
    if(data&0x20)  strcat(msg,"40G,"); 
    if(data&0x80)  strcat(msg,"31P5G,"); 
    if(data&0x100) strcat(msg,"25P455G,"); 
    if(data&0x200) strcat(msg,"21G_X4,"); 
    if(data&0x400) strcat(msg,"LAST=1,"); 
    if(data&0x1)   strcat(msg,"Higig2=1,"); 
    if(data&0x2)   strcat(msg,"FEC=1,"); 
    if(data& DIGITAL3_UP3_CL72_MASK)   strcat(msg,"CL72=1,"); 
    printf("%s\n", msg); 
}
