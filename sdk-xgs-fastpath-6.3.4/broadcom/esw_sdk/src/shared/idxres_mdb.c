/*
 * $Id: idxres_mdb.c 1.13 Broadcom SDK $
               ...
               int lock)
{
    ...
    MDB_VVERB((MDB_MSG1("%u elements, bankSize %u -> %u banks with"
 ...
 *
 *  Notes
 *      BCM_E_NONE is not returned here.
 */
int
                        ...
                        shr_mdb_elem_bank_index_t count)
{
    ...
    if ((first < handle->low) ||
        ...
        MDB_ERR((MDB_MSG1("element range %08X..%08X is not valid\n"),
                 first,
                 count));
        /* for this function, we specifically want BCM_E_PARAM */
        result = BCM_E_PARAM;
    }
    if (BCM_E_NONE == result) {
        /* find the head of the block */
        ...
        if (handle->elem[head].list < handle->freeLists) {
            ...
            do {
                ...
                if (handle->elem[head].list >= handle->freeLists) {
                    /* this block is not free */
                    MDB_ERR((MDB_MSG1("block including element %08X (%d elems)"
