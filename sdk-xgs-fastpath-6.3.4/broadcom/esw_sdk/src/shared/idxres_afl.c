/*
 * $Id: idxres_afl.c 1.13 Broadcom SDK $
 ...
 */
#ifdef AIDXRES_DEBUG
#define AIDXRES_DUMP(stuff) soc_cm_print stuff
...
_aidxres_dump_list(const shr_aidxres_list_handle_t list){
    ...
    unsigned int _index;
    unsigned int _max;
    soc_cm_print("Resource list attributes\n");
    soc_cm_print("   first = %08X   last = %08X   low = %08X   high = %08X\n",
                  list->first,
                  list->last,
                  list->valid_low,
                 list->valid_high);
    soc_cm_print("    free = %08X  alloc = %08X  bMax = %08X  bRMax = %08X\n",
                  list->free_count,
                  list->alloc_count,
                  list->bank_max,
                 list->bank_rem_max);
    soc_cm_print("    sCnt = %08X",
                 list->sublist_count);
    for (_sublist = 0; _sublist < list->sublist_count; _sublist++) {
        soc_cm_print("\nFree banks sublist %02X", _sublist);
        soc_cm_print("    head=%08X  free=%08X",
    ...
    for (sublist = 0; sublist < list->sublist_count; sublist++) {
        ...
        if (elem < elems) {
            if (elemDesc[elem].prev != AIDXRES_FIRST_ENTRY) {
                soc_cm_print("aidxres %p element %08X bank %08X sublist %02X"
            ...
            soc_cm_print("aidxres %p bank %08X sublist %02X head "
        ...
        while ((!abort) && (elem <= AIDXRES_MAX_ENTRY)) {
            if (last != elemDesc[elem].prev) {
                soc_cm_print("aidxres %p element %08X..%08X bank %08X sublist"
            ...
            for (offs = 0; offs < elemDesc[elem].elem_count; offs++) {
                if (sublist != elemDesc[elem + offs].sublist) {
                    soc_cm_print("aidxres %p element %08X..%08X bank %08X"
                ...
                if (SHR_BITGET(buffer, elem + offs)) {
                    soc_cm_print("aidxres %p element %08X..%08X bank %08X elem "
        ...
        if ((elem != AIDXRES_FINAL_ENTRY) && (last != AIDXRES_FINAL_ENTRY)) {
            soc_cm_print("aidxres %p element %08X bank %08X sublist %02X tail "
        ...
        if ((!abort) && (free != bankSublist->free_count)) {
            soc_cm_print("aidxres %p bank %08X sublist %02X count "
        ...
        for (elem = 0; elem < elems; /* incr deliberately */) {
            if (SHR_BITGET(buffer, elem)) {
                ...
                for (offs = 0; offs < free; offs++) {
                    if (SHR_BITGET(buffer, elem + offs)) {
                        soc_cm_print("aidxres %p allocated element %08X..%08X"
                    ...
                    if (0 == offs) {
                        ...
                        if (free != elemDesc[elem + offs].elem_count) {
                            soc_cm_print("aidxres %p allocated element"
...
_aidxres_list_check(const shr_aidxres_list_handle_t list)
{
    ...
    if (NULL == buffer) {
        soc_cm_print("aidxres %p unable to validate (c): not enough memory\n",
                     (void*)list);
        return BCM_E_NONE;
    }
    ...
    if (NULL == totalFree) {
        soc_cm_print("aidxres %p unable to validate (f): not enough memory\n",
                     (void*)list);
        sal_free(buffer);
        return BCM_E_NONE;
    }
        ...
        if (list->bank_max >= bank) {
            ...
            while (bank < AIDXRES_FIRST_BANK) {
                if (SHR_BITGET(buffer, bank)) {
                    soc_cm_print("aidxres %p sublist %02X bank %08X has"
                ...
                if (list->bank_list[bslcurr].prev != prevBank) {
                    soc_cm_print("aidxres %p sublist %02X bank %08X prev %08X"
                                 " should be %08X\n",
                                 (void*)list,
                                 sublist,
                                 bank,
                ...
                if ((bank > list->bank_max) && (AIDXRES_FINAL_BANK != bank)) {
                    soc_cm_print("aidxres %p sublist %02X bank %08X next %08X"
            ...
            soc_cm_print("aidxres %p sublist %02X first bank %08X should be"
                         " either %08X or < %08X\n",
                         (void*)list,
                         sublist,
                         bank,
        ...
        for (bank = 0; bank <= list->bank_max; bank++) {
            if (!SHR_BITGET(buffer, bank)) {
                ...
                if (AIDXRES_USED_BANK != list->bank_list[bslcurr].prev) {
                    soc_cm_print("aidxres %p sublist %02X empty bank %08X prev"
                                 " %08X should be %08X\n",
                                 (void*)list,
                                 sublist,
                                 bank,
                ...
                if (AIDXRES_USED_BANK != list->bank_list[bslcurr].next) {
                    soc_cm_print("aidxres %p sublist %02X empty bank %08X next"
                                 " %08X should be %08X\n",
                                 (void*)list,
                                 sublist,
                                 bank,
    ...
    for (sublist = 0; sublist < list->sublist_count; sublist++) {
        if (list->sublist[sublist].free_count != totalFree[sublist]) {
            soc_cm_print("aidxres %p sublist %02X free count %08X incorrect;"
                         " counted %08X\n",
                         (void*)list,
                         sublist,
    ...
    if ((BCM_E_NONE != result) &&
        ...
        soc_cm_print("aidxres %p appears corrupt; dumping it\n", (void*)list);
