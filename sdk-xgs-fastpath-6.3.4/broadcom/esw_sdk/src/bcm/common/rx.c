 ...
 */
#define MARK_PKT_PROCESSED(_pkt, unit, chan, dv, idx)                   \
    do {                                                                \
...
_bcm_common_rx_init(int unit)
{
    ...
    if (rx_ctl[unit] != NULL) {
        if (_bcm_common_rx_active(unit)) {
            if ((rv = _bcm_common_rx_stop(unit, NULL)) < 0) {
                RX_DEBUG((DK_ERR, "Error in RX Init: RX Stop returned %s\n",
                          bcm_errmsg(rv)));
                return rv;
            }
        }
    ...
    if (SOC_UNIT_VALID(unit) && SOC_IS_ARAD(unit)){
             queue_size = sizeof(rx_queue_t) * BCM_RX_COS_ARAD ;
    } else 
#endif
    if (SOC_UNIT_VALID(unit) && SOC_IS_KATANA2(unit)) {
        queue_size = sizeof(rx_queue_t) * NUM_CPU_COSQ(unit) ;
    } else {
    ...
    if (!bcm_rx_pool_setup_done()) {
        RX_DEBUG((DK_RX, "RX: Starting rx pool\n"));
    ...
    _BCM_RX_SYSTEM_UNLOCK; 
    RX_DEBUG((DK_RX, "RX: Initialized unit %d\n", unit));
...
{  
    ...
    if (RX_UNIT_STARTED(unit)) { 
        RX_DEBUG((DK_RX, "RX start %d:  Already started\n", unit)); return BCM_E_BUSY;
    }
    ...
    RX_DEBUG((DK_RX, "RX: Starting unit %d\n", unit));

#if defined(BCM_RPC_SUPPORT) || defined(BCM_RCPU_SUPPORT)
    if (rx_ctl[unit]->user_cfg.rx_alloc != RX_DEFAULT_ALLOC ||
            rx_ctl[unit]->user_cfg.rx_free != RX_DEFAULT_FREE) {
        RX_DEBUG((DK_WARN,
               "RX WARNING: Not using rx_pool alloc/free with %s.\n",
    ...
    if (RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit)) {
        ...
        FOREACH_SETUP_CHANNEL(unit, chan) {
            ...
            if (rv < 0) {
                RX_DEBUG((DK_ERR, "RX: Error on setup unit %d, chan %d\n",
                             unit, chan));
                rx_cleanup(unit);
                return rv;
            }
...
_bcm_common_rx_stop(int unit, bcm_rx_cfg_t *cfg)
{
    ...
    _BCM_RX_START_LOCK;
    RX_DEBUG((DK_RX, "RX: Stopping unit %d\n", unit));
    ...
    if (rx_control.thread_running) {
        ...
        if (!rx_control.thread_exit_complete) {
            RX_DEBUG((DK_WARN, "RX %d: Thread %p running after signaled "
                             ...
                             uint8 priority, uint32 flags)
{
    ...
    RX_DEBUG((DK_RX | DK_VERBOSE, "RX: %s registered %s%s.\n",
                 name,
                          ...
                          uint32 flags)
{
    ...
    RX_DEBUG((DK_RX, "RX: Registering %s on %d, cosq 0x%x flags 0x%x%s\n",
    ...
    if (RX_IS_REMOTE(unit) && !RX_IS_RCPU(unit)) {
        ...
        if (rv < 0) {
            RX_DEBUG((DK_WARN, "RX: rlink connect unit %d "
                ...
                uint8 priority, void *cookie, uint32 flags)
{
    ...
    RX_DEBUG((DK_RX, "RX: Registering %s on %d, flags 0x%x%s\n",
                 name, unit, flags, flags & BCM_RCO_F_INTR ? "(intr)" : ""));

    if (!(flags & BCM_RCO_F_COS_ACCEPT_MASK) &&
            !(flags & BCM_RCO_F_ALL_COS)) {
        RX_DEBUG((DK_WARN, "RX unit %d: "
                     "Registering callback with no COS accepted.\n", unit));
        RX_DEBUG((DK_WARN, "    Callbacks will not occur to %s\n",
                     name));
    }
             ...
             uint8 priority,  bcm_cos_queue_t cosq)
{
    ...
    if (RX_IS_REMOTE(unit) && !RX_IS_RCPU(unit)) {
        ...
        if (rx_ctl[unit]->rc_callout == NULL) { /* No callouts, disconnect */
            ...
            if (rv < 0) {
                RX_DEBUG((DK_WARN, "RX: rlink disconnect unit %d "
                          "returned %d: %s\n", unit, rv, bcm_errmsg(rv)));
            }
        }
    }
    ...
    RX_DEBUG((DK_RX, "RX: Unregistered %s on %d\n", name, unit));
    sal_free((void *)rco);
            ...
            if (reg_val & bit) {
                if (*chan_id == -1) {
                    *chan_id = ix;
                    ...
                    RX_DEBUG((DK_WARN, "rx_queue_channel_get: Warning: "
                    "Multiple channels assigned to the COS 0x%x for unit %d\n", 
                    queue_id, unit));
                    return BCM_E_INTERNAL;
                }
            }
...
rx_chain_start(int unit, int chan, dv_t *dv)
{
    ...
    RX_DEBUG((DK_RX + DK_VERBOSE, "RX: Starting %d/%d/%d\n",
...
rx_chain_start_or_sched(int unit, int chan, dv_t *dv)
{
    ...
    RX_DEBUG((DK_RX + DK_VERBOSE, "RX: Chain. glob tok %d.\n",
                 RX_TOKENS(unit)));
    ...
    if (!global_usecs) { /* Can start immediately */
        ...
        RX_DEBUG((DK_RX, "RX: Scheduling %d/%d/%d in %d us; "
                     "cur %u; sleep %u\n", unit, chan, DV_INDEX(dv),
...
rx_thread_pkts_process(int unit, int cos, int count)
{
    ...
    while (pkt_list) {
        ...
        if (drop_all) {
            if (RX_IS_RCPU(unit)) {
                ...
                MARK_PKT_PROCESSED(pkt_list, unit, pkt_list->dma_channel, 
                                   pkt_list->_dv, pkt_list->_idx);
            }
            queue->rate_disc++;
        } else {
#ifdef BCM_RCPU_SUPPORT
    ...
    RX_VERY_VERBOSE((DK_RX + DK_VERBOSE, "Processed (%d) packets", count));
    return rv;
}
        ...
        if (SOC_UNIT_VALID(unit)) {
            ...
            FOREACH_SETUP_CHANNEL(unit, chan) {
                ...
                if (soc_dma_abort_channel_total(unit, chan) < 0) {
                    RX_DEBUG((DK_WARN,
                              "RX: Error aborting DMA channel %d\n", chan));
                }
...
rx_pkt_thread(void *param)
{
    ...
    RX_DEBUG((DK_RX, "RX:  Packet thread starting\n"));
    ...
    while (rx_control.thread_running) {
        ...
        while (BCM_SUCCESS(rx_control.rx_sched_cb(rx_control.rx_unit_first, 
            ...
            if (BCM_FAILURE(rx_thread_pkts_process(unit, cosq, count))) {
                RX_DEBUG((DK_RX, "Packet rx failed - check the scheduler.\n"));
            }
            /* 
    ...
    RX_DEBUG((DK_RX, "RX: Packet thread exitting\n"));
...
rx_rcpu_process_packet(int unit, bcm_pkt_t *pkt)
{
    ...
    for (rco = rx_ctl[unit]->rc_callout; rco; rco = rco->rco_next) {
        ...
        case BCM_RX_HANDLED:
            ...
            RX_DEBUG((DK_PACKET | DK_VERBOSE, "rx: pkt handled by %s\n",
                         rco->rco_name));
            rco->rco_pkts_handled++;
            break;
        case BCM_RX_HANDLED_OWNED:
            ...
            pkt->alloc_ptr = NULL;
            bcm_rx_remote_pkt_free(pkt);
            RX_DEBUG((DK_PACKET | DK_VERBOSE, "rx: pkt owned by %s\n",
                         rco->rco_name));
            rx_ctl[unit]->pkts_owned++;
            rco->rco_pkts_owned++;
            break;
        default:
            RX_DEBUG((DK_WARN, "rx_process_packet: unit=%d: "
...
rx_process_packet(int unit, bcm_pkt_t *pkt)
{
    ...
    if (soc_cm_debug_check(DK_PACKET)) {
        ...
        RX_DEBUG((DK_RX + DK_PACKET, "rx_process_packet: packet in\n"));
        if ((RX_IS_LOCAL(unit) && SOC_UNIT_VALID(unit))
            || RX_IS_RCPU(unit)) {
            if (soc_cm_debug_check(DK_VERBOSE + DK_DMA)) {
    ...
    if (!NON_INTR_CALLOUTS(unit)) {
        ...
        MARK_PKT_PROCESSED(pkt, unit, pkt->dma_channel, dv, idx);
        rx_ctl[unit]->no_hndlr++;
        return;
    }
    ...
    for (rco = rx_ctl[unit]->rc_callout; rco; rco = rco->rco_next) {
        ...
        case BCM_RX_HANDLED:
            handled = TRUE;
            MARK_PKT_PROCESSED(pkt, unit, chan, dv, idx);
            RX_DEBUG((DK_PACKET | DK_VERBOSE, "rx: pkt handled by %s\n",
                         rco->rco_name));
            rco->rco_pkts_handled++;
            break;
...
rx_channel_dv_setup(int unit, int chan)
{
    ...
    case DV_RX: /* Already RX - check flags */
        if (RX_CHAN_FLAGS(unit, chan) & BCM_RX_F_OVERSIZED_OK) {
            ...
            RX_DEBUG((DK_VERBOSE, "rx: accept oversized pkts\n"));
        }
        break;
    case DV_NONE: /* Need to configure the channel */
        ...
        break;
    default: /* Problem */
        RX_DEBUG((DK_ERR, "Incompatible channel setup for %d/%d\n",
                     unit, chan));
        return BCM_E_PARAM;
    }
    ...
    if (dma_flags) {
        ...
        if (rv < 0) {
            RX_DEBUG((DK_ERR, "Could not configure channel %d on %d\n",
                         chan, unit));
            return rv;
        }
    }
    ...
    for (i = 0; i < RX_CHAINS(unit, chan); i++) {
        ...
        for (j = 0; j < RX_PPC(unit); j++) {
            ...
            pkt->blk_count = 1;
            if (SOC_IS_XGS12_FABRIC(unit) && !RX_IGNORE_HG(unit)) {
    ...
    if (RX_IS_LOCAL(unit) || RX_IS_RCPU(unit)) {
        FOREACH_SETUP_CHANNEL(unit, chan) {
            if (RX_CHAINS(unit, chan) < 0) {
                RX_DEBUG((DK_WARN, "rx_config %d %d: Warning: chains < 0.",
                             unit, chan));
                RX_CHAINS(unit, chan) = 0;
            } else {
                chan_count++;
                if (RX_CHAINS(unit, chan) > RX_CHAINS_MAX) {
                    RX_DEBUG((DK_WARN, "rx_config %d %d: Warning: "
                                 "Bad chain cnt %d.  Now %d.\n",
                                 unit, chan, RX_CHAINS(unit, chan), RX_CHAINS_MAX));
        ...
        if (cfg->pkts_per_chain <= 0 ||
            ...
            RX_DEBUG((DK_WARN, "rx_config: Warning: bad pkts/chn %d. "
        ...
        if (SOC_UNIT_VALID(unit) &&
            ...
            FOREACH_SETUP_CHANNEL(unit, chan) {
                if (cos_bmp & RX_CHAN_COS(unit, chan)) {
                    RX_DEBUG((DK_WARN,
                                 "rx_config: Warning: COS overlap may not "
            ...
            if ((cos_bmp = (0xff & ~cos_bmp))) {
                RX_DEBUG((DK_WARN, "rx_config: Warning: Not mapping "
                             "COS 0x%x for unit %d\n", cos_bmp, unit));
            }
        } else {
            if (chan_count > 1) {
                RX_DEBUG((DK_WARN, "rx_config: Warning: Activating more "
    ...
    for (i = 0; i < RX_PPC(unit); i++) {
        ...
        if (pkt->_pkt_data.data == NULL) {
            ...
            if (BCM_FAILURE(rv)) {/* Failed to allocate a pkt for this DV. */
                if (!warned) {
                    warned = 1;
                    RX_DEBUG((DK_WARN, "RX: Failed to allocate mem\n"));
                }
...
_bcm_common_rx_show(int unit)
{
    ...
    for (i = 0; i <= RX_QUEUE_MAX(unit); i++) {
        ...
        RX_PRINT(("    Queue %d: PPS %d. CurPkts %d. TotPkts %d. "
                     ...
                     queue->rate_disc,
                     queue->qlen_disc));
        RX_DEBUG((DK_VERBOSE, "        Tokens %d. Fill %u. Max %d. "
