		...
		int *loopback)
{
    ...
    if (0 == TK371X_PORT_VALID( port)){
        soc_cm_debug(DK_PORT, "Return value: BCM_E_PORT: invalid PORT : %d\n", (port));
        return BCM_E_PORT;
    }
		...
		bcm_port_config_t *config)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_config_get()..\n");
    if (0 == BCM_TK371X_UNIT_VALID(unit)){
    	return BCM_E_UNIT;
    }
    if (NULL == config){
	...
	soc_cm_debug(DK_PORT, "before call soc marcos:\n");
    sal_memset(pbuf, 0, sizeof(pbuf));
    SOC_PBMP_FMT(config->fe, pbuf);
    soc_cm_debug(DK_PORT, "config->fe=%s\n", pbuf);
    SOC_PBMP_FMT(config->ge, pbuf);
    soc_cm_debug(DK_PORT, "config->ge=%s\n", pbuf);
    SOC_PBMP_FMT(config->port, pbuf);
    soc_cm_debug(DK_PORT, "config->port=%s\n", pbuf);
    SOC_PBMP_FMT(config->all, pbuf);
    soc_cm_debug(DK_PORT, "config->all=%s\n", pbuf);
    SOC_PBMP_FMT(config->llid, pbuf);
    soc_cm_debug(DK_PORT, "config->llid=%s\n", pbuf);
    SOC_PBMP_FMT(config->pon, pbuf);
    soc_cm_debug(DK_PORT, "config->pon=%s\n", pbuf);
    ...
    soc_cm_debug(DK_PORT, "after call soc marcos:\n");
    sal_memset(pbuf, 0, sizeof(pbuf));
    SOC_PBMP_FMT(config->fe, pbuf);
    soc_cm_debug(DK_PORT, "config->fe=%s\n", pbuf);
    SOC_PBMP_FMT(config->ge, pbuf);
        ...
        int *value)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_control_get(),"
			     " unit=%d, port=%d, type=%d\n", unit, port, (int)type);
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
		soc_cm_debug(DK_PORT, "Return value: BCM_E_PORT, Invaild PORT : %d\n", (port));
		return BCM_E_PORT;
	}
	    ...
	    int value)
{
	...
	if (0 == TK371X_PORT_VALID( port)){
		soc_cm_debug(DK_PORT, "Return BCM_E_PORT, invalid PORT : %d\n", (port));
		return BCM_E_PORT;
	}
	    ...
	    int *enable)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_enable_get..\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
    ...
    soc_cm_debug(DK_PORT, "bcm_port_enable_get: u=%d p=%d rv=%d enable=%d\n",
             unit, port, rv, *enable);
    return rv;
}
	    ...
	    int enable)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_enable_set.., unit=%d\n", unit);
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
    ...
    if ((enable == 0) || (enable == 1)){
		...
		soc_cm_debug(DK_PORT, "Return BCM_E_PARAM, invalid value : %d\n", (enable));
    	return BCM_E_PARAM;
    }
    rv = _bcm_ea_port_enable_set(unit, port, state);
    soc_cm_debug(DK_PORT, "_bcm_ea_port_enable_set:unit=%d port=%d rv=%d enable=%d\n",
             unit, port, rv, enable);
    return rv;
}
	    ...
	    int *size)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_frame_max_get.., unit=%d\n", unit);
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
    ...
    soc_cm_debug(DK_PORT, "_bcm_ea_port_frame_max_get:unit=%d port=%d rv=%d size=%d\n",
             unit, port, rv,*size);
	return rv;
}
	    ...
	    int size)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_frame_max_set.., unit=%d\n", unit);
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
	...
	soc_cm_debug(DK_PORT, "_bcm_ea_port_frame_max_set:unit=%d port=%d rv=%d size=%d\n",
		 unit, port, rv, size);
	return rv;
}
...
bcm_tk371x_port_init(int unit)
{
	...
	soc_cm_debug(DK_PORT, "bcm_tk371x_port_init, unit = %d\n", unit);
...
bcm_tk371x_port_learn_get(
	    ...
	    uint32 *flags)
{
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_learn_get..\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
...
bcm_tk371x_port_learn_modify(
	    ...
	    uint32 remove)
{
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_learn_modify..\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
...
bcm_tk371x_port_learn_set(
	    ...
	    uint32 flags)
{
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_learn_set..\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
	    ...
	    int *status)
{
	...
	soc_cm_debug(DK_PORT, "BCM API : bcm_tk371x_port_link_status_get..\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
		...
		bcm_pbmp_t *obp)
{
	...
	char pbuf[100];
	soc_cm_debug(DK_PORT, "ibp:\n");
    sal_memset(pbuf, 0, sizeof(pbuf));
    SOC_PBMP_FMT(ibp, pbuf);
    soc_cm_debug(DK_PORT, "ibp=%s\n", pbuf);
	...
	soc_cm_debug(DK_PORT, "obp:\n");
    sal_memset(pbuf, 0, sizeof(pbuf));
    SOC_PBMP_FMT((*obp), pbuf);
    soc_cm_debug(DK_PORT, "obp=%s\n", pbuf);
	return BCM_E_NONE;
}
		...
		bcm_port_info_t *info)
{
    ...
    soc_cm_debug(DK_PORT, "BCM API : _bcm_tk371x_port_selective_get()..\n");
    mask = info->action_mask;
		...
		bcm_port_pon_info_t *pon)
{
	...
	soc_cm_debug(DK_PORT, "bcm_tk371x_port_pon_info_get...\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
	...
	if (rv != OK){
		soc_cm_debug(DK_PORT, "_soc_ea_pon_reg_status_get Return BCM_E_INTERNAL, invalid value=%d\n", rv);
		return BCM_E_FAIL;
	}
    ...
    soc_cm_debug(DK_PORT, "_soc_ea_pon_reg_status_get:unit=%d port=%d rv=%d\n",
             unit, port, rv);
    soc_cm_debug(DK_PORT, "pon->enable=%d\n", pon->enable);
	soc_cm_debug(DK_PORT, "pon->linkstatus=%d\n", pon->linkstatus);
	soc_cm_debug(DK_PORT, "pon->llid=%d\n", pon->llid);
	soc_cm_debug(DK_PORT, "pon->loopback=%d\n", pon->loopback);
	soc_cm_debug(DK_PORT, "pon->oam_discovery_status=%d\n", pon->oam_discovery_status);
	soc_cm_debug(DK_PORT, "pon->olt_mac_addr=");
	for (i = 0; i < sizeof(bcm_mac_t); i++){
		soc_cm_debug(DK_PORT, "%02X", pon->olt_mac_addr[i]);
	}
		...
		bcm_port_pon_info_t *pon)
{
	...
	soc_cm_debug(DK_PORT, "bcm_tk371x_port_pon_info_set...\n");
	if (0 == BCM_TK371X_UNIT_VALID((unit))){
		return BCM_E_UNIT;
	}
	if (0 == TK371X_PORT_VALID( port)){
	...
	if (1 == TK371X_PON_PORT_VALID(port)){ 
		if (pon->enable != 0 && pon->enable != 1){
			soc_cm_debug(DK_PORT, "Return BCM_E_PARAM, invalid value is %d\n", pon->enable);
			return BCM_E_PARAM;
		}

		pon_state = (pon->enable == 1) ? 0 : 1;
		rv = TkExtOamSetEponAdmin(unit, port, pon_state);
		soc_cm_debug(DK_PORT, "TkExtOamSetEponAdmin:unit=%d port=%d rv=%d, pon->enable=%d\n",
