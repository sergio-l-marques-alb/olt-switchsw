                              ...
                              bcm_regex_match_t* match, int count)
{
#if defined(BCM_TRIUMPH3_SUPPORT)
    int i, rv;
    int non_policy_matches = 0;
    int policy_matches = 0;
    ...
    if (SOC_IS_TRIUMPH3(unit)) {
        if (count > 0) {
            ...
            if (policy_matches != 0 && non_policy_matches != 0) {
                return BCM_E_PARAM;
            }
        }
...
int bcm_esw_regex_init(int unit)
{
    ...
    info->meter_base_hw_index = field_stage->num_meter_pools * field_stage->meter_pool[0]->pool_size;
...
STATIC int _regex_policy_phys_create(int unit, bcm_regex_policy_t policy, _regex_policy_t **policy_p)
{
    ...
    REGEX_VVERB(("FT(unit %d) vverb: BEGIN %s(policy=%d)\n",
    ...
    if (device->num_policies >= ESW_REGEX_POLICIES_MAX(unit)) {
        REGEX_ERR(("FT(unit %d) Error: No room.\n", unit));
        return (BCM_E_RESOURCE);
    }

    policy_ent = sal_alloc(sizeof(*policy_ent), "regex_policy");
    if (policy_ent == NULL) {
        REGEX_ERR(("FT(unit %d) Error: allocation failure for policy_entry\n", unit));
        return (BCM_E_MEMORY);
    }
                                              ...
                                              _regex_policy_action_t *vn_del_pa)
{
    ...
    if (NULL != sa_pa) {
        SAL_MAC_ADDR_FROM_UINT32(intf.l3a_mac_addr, sa_pa->param);
    }

    if (NULL != vid_pa) {
        intf.l3a_vid = vid_pa->param[0];
    }
...
STATIC int _regex_policy_install(int unit, bcm_regex_policy_t policy)
{
    ...
    REGEX_VVERB(("FT(unit %d) vverb: %s(%d)\n", unit, FUNCTION_NAME(), policy));
                               ...
                               uint32 param3, _regex_policy_action_t **pa)
{
    *pa = sal_alloc(sizeof(**pa), "regex_policy_action");
    if (NULL == *pa) {
        REGEX_ERR(("FT(unit %d) Error: allocation failure for regex_policy_action\n", unit));
        return (BCM_E_MEMORY);
    }
    sal_memset(*pa, 0, sizeof(**pa));
...
int _regex_policy_action_add(int unit, bcm_regex_policy_t policy, _regex_policy_action_t *pa)
{
    ...
    rv = device->functions->policy_action_support_check(unit,
                                                        pa->action);
    if (BCM_FAILURE(rv)) {
        REGEX_ERR(("FT(unit %d) Error: action=%s is not supported.\n",
                   unit,
    ...
    while (pa_iter != NULL) {
        ...
        rv = device->functions->policy_action_conflict_check(unit,
                                                            ...
                                                            pa->action);
        if (BCM_FAILURE(rv)) {
            REGEX_ERR(("FT(unit %d) Error: action=%s conflicts with existing action in policy=%d\n",
                       unit,
                       _regex_policy_action_name(pa->action),
                       policy));
            return (rv);
    ...
    if (BCM_FAILURE(rv)) {
        REGEX_ERR(("FT(unit %d) Error: action=%s parameters check failed (%d)\n",
                                          ...
                                          uint32 param3)
{
    ...
    REGEX_VVERB(("FT(unit %d) vverb: %s(policy=%d, action=%s, p0=%d, p1=%d, p2=%d)\n",
                 unit, FUNCTION_NAME(), policy,
    ...
    if (BCM_FAILURE(rv)) {
        REGEX_ERR(("FT(unit %d) Error: failure in %s()\n", unit, FUNCTION_NAME()));
        ESW_REGEX_UNLOCK(unit);
        return rv;
    }
                                          ...
                                          uint32 *param3)
{
    ...
    if (pa == NULL) {
        ESW_REGEX_UNLOCK(unit);
        REGEX_VERB(("FT(unit %d) Error: action not in policy=%d\n", unit, policy));
        return (BCM_E_NOT_FOUND);
    }
                                    ...
                                    bcm_field_action_t action, uint32 param0, uint32 param1)
{
    ...
    REGEX_VVERB(("FT(unit %d) vverb: %s(policy=%d, action=%s, p0=%d, p1=%d)\n",
                 unit, FUNCTION_NAME(), policy,
    ...
    if ((SOC_PORT_ADDR_MAX(unit) > 31) &&
        ...
        REGEX_ERR(("FT(unit %d) Error: Use bcm_regex_policy_action_ports_add api.\n", unit));
        return (BCM_E_PARAM);
    }
    ...
    if (BCM_FAILURE(rv)) {
        REGEX_ERR(("FT(unit %d) Error: failure in _regex_policy_action_alloc()\n", unit));
        ESW_REGEX_UNLOCK(unit);
        return rv;
    }
                                          ...
                                          bcm_pbmp_t pbmp)
{
    ...
    if ((action != bcmFieldActionRedirectPbmp) &&
        ...
        (action != bcmFieldActionEgressPortsAdd) &&
        (action != bcmFieldActionRedirectBcastPbmp)) {
        REGEX_ERR(("FT(unit %d) Error: Incorrect action parameter\n", unit));
        return (BCM_E_PARAM);
    }
                                  ...
                                  bcm_policer_t *pid)
{
    ...
    if (0 == (pol_cfg->flags & BCM_POLICER_WITH_ID)) {
        BCM_IF_ERROR_RETURN(_regex_policer_id_alloc(unit, pid));
    } else {
...
void _bcm_esw_policy_dump(int unit, bcm_regex_policy_t policy)
{
    ...
    device = ESW_REGEX_INFO(unit);
    if (NULL == device) {
        soc_cm_print("Regex module not initialized.\n");
        return;
    }

    /* List the global policy data */
    soc_cm_print("Number of policies: %d\n", device->num_policies);
    soc_cm_print("Minimum index: %d\n", device->policy_index_min);
    soc_cm_print("Maximum index: %d\n", device->policy_index_max);

    if (NULL == device->policies) {
        soc_cm_print("No policies are created.\n");
        return;
    }
...
void _bcm_esw_policer_dump(int unit, bcm_policer_t policer_id)
{
    ...
    if (_REGEX_INVALID_INDEX == policer_id) {
        ...
        for (i = 0; i < sizeof(device->policers)/sizeof(device->policers[0]); i++) {
            if (NULL != device->policers[i]) {
                soc_cm_print("Policer %d (0x%x)\n", device->policers[i]->pid, device->policers[i]->pid);
                count++;
            }
        }
        soc_cm_print("Policers in use: %d\n", count);
    } else {
        ...
        if ((NULL != device->policers[i]) &&
            ...
            soc_cm_print("Policer %d (0x%x), flags = 0x%x\n", f_pl->pid, f_pl->pid, f_pl->hw_flags);
            soc_cm_print("\tSW Ref Count = %d, HW Ref Count = %d\n", f_pl->sw_ref_count, f_pl->hw_ref_count);
            soc_cm_print("\tLevel = %d, Pool Index = %d, HW Index = %d\n", f_pl->level, f_pl->pool_index, f_pl->hw_index);
        } else {
            soc_cm_print("Policer not found.\n");
        }
    }
