...
#if defined(BCM_KATANA_SUPPORT) 
               if (SOC_IS_KATANAX(unit)) {
                   rv = bcm_kt_mpls_label_stat_get(unit, label,port, stat, val);
               } else
#endif
#if defined(BCM_TRIUMPH3_SUPPORT)
               if (SOC_IS_TRIUMPH3(unit)) {
                   rv = bcm_tr3_mpls_label_stat_get(unit, label,port,stat, val);
               } else
#endif
               {
#if defined(BCM_TRIDENT2_SUPPORT)
                   rv = td2_mpls_label_stat_get(unit, label,port,stat, val);
#endif
               }
               bcm_tr_mpls_unlock(unit);
    }
...
#if defined(BCM_KATANA_SUPPORT) 
               if (SOC_IS_KATANAX(unit)) {
                   rv = bcm_kt_mpls_label_stat_get32(unit, label,port,stat,val);
               } else
#endif
#if defined(BCM_TRIUMPH3_SUPPORT)
               if (SOC_IS_TRIUMPH3(unit)) {
                   rv = bcm_tr3_mpls_label_stat_get32(unit,label,port,stat,val);
               } else
#endif
               {
#if defined(BCM_TRIDENT2_SUPPORT)
                   rv = td2_mpls_label_stat_get32(unit,label,port,stat,val);
#endif
               }
               bcm_tr_mpls_unlock(unit);
    }
...
int bcm_esw_mpls_port_stat_counter_get(
            int                  unit, 
            bcm_vpn_t            vpn, 
            bcm_gport_t          port, 
            bcm_mpls_stat_t      stat, 
 ...
 *
 */
int bcm_esw_mpls_label_stat_counter_get(
            int                  unit, 
            bcm_mpls_label_t     label, 
            bcm_gport_t          port, 
            bcm_mpls_stat_t      stat, 
