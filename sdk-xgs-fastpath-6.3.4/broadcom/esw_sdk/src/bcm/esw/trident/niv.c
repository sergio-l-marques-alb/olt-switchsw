/*
 * $Id: niv.c 1.28.2.1 Broadcom SDK $
...
bcm_trident_niv_reinit(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        /* Read virtual interface ID, multicast flag, and SD-tag */
...
_bcm_trident_niv_nh_info_set(int unit, int nh_index, int multicast,
        ...
        _bcm_trident_niv_sd_tag_t *sd_tag, int vp)
{
...
_bcm_trident_niv_egress_match_ex_port(int unit,
        ...
        int *match)
{
    int flags_match, vif_match, vlan_match;
    int service_tpid_match, service_vlan_match;
    int service_pri_match, service_cfi_match;
    int service_qos_map_id_match;
    ...
    bcm_niv_egress_t *niv_egress)
{
    ...
    BCM_PBMP_ITER(new_pbmp, local_port) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_trident_niv_port_cnt_update(unit, gport, vp, TRUE));
    }
    ...
    bcm_niv_egress_t *niv_egress)
{
    ...
    BCM_PBMP_ITER(pbmp, local_port) {
        BCM_IF_ERROR_RETURN
            (bcm_esw_port_gport_get(unit, local_port, &gport));
        BCM_IF_ERROR_RETURN
            (_bcm_trident_niv_port_cnt_update(unit, gport, vp, FALSE));
    }
    ...
    bcm_niv_egress_t *niv_egress_array)
{
    ...
    BCM_IF_ERROR_RETURN
        (READ_ING_DVP_TABLEm(unit, MEM_BLOCK_ANY, vp, &dvp_entry));
    soc_ING_DVP_TABLEm_field32_set(unit, &dvp_entry, NEXT_HOP_INDEXf,
            NIV_INFO(unit)->invalid_next_hop_index);
    BCM_IF_ERROR_RETURN
        (WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, vp, &dvp_entry));
    ...
    *count = 0;
    ...
    while (NULL != curr_ptr) {
        if (array_size > 0) {
            ...
            niv_egress_array[*count].egress_if = curr_ptr->next_hop_index +
