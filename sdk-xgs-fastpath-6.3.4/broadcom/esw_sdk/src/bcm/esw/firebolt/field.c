/* $Id: field.c 1.578.2.3 Broadcom SDK $
...
int
_field_slice_expanded_status_get(int unit, _field_stage_t *stage_fc, 
                                 int *expanded)
{
    int i, vslice, slice_idx = 0, vgroup = -1, max = -1, slice_num;
    ...
    for (vslice = 0; vslice < slice_num; vslice++) {
        ...
        stage_fc->vmap[0][vslice].vmap_key = slice_idx;
        stage_fc->vmap[0][vslice].virtual_group = vgroup;
        stage_fc->vmap[0][vslice].priority = vgroup;
    ...
    for (vslice = 0; vslice < slice_num; vslice++) {
        ...
        for (i = 0; i < slice_num; i++) {
           if ((stage_fc->vmap[0][vslice].virtual_group == 
                stage_fc->vmap[0][i].virtual_group)) {
                if (i > max) {
                    max = i;
                }
            }
        ...
        if ((max != vslice) && (max != -1)) {
            expanded[stage_fc->vmap[0][vslice].vmap_key] = 1;
        }
    }
    return BCM_E_NONE;
}
...
int 
_field_physical_to_virtual(int unit, int slice_idx, _field_stage_t *stage_fc)
{
    int vslice, slice_num;
    slice_num = SOC_IS_HAWKEYE(unit) ? 4 : (SOC_IS_KATANAX(unit) ? 12 : 16);
    for (vslice = 0; vslice < slice_num; vslice++) {
        if (stage_fc->vmap[0][vslice].vmap_key == slice_idx) {
            return vslice;
        }
    }
    return -1;
    ...
    if (BCM_SUCCESS(_field_fb_meter_index_in_use(unit, fs,
        ...
        switch (meter_conf->meter_mode) {
            ...
            case BCM_FIELD_METER_MODE_FLOW: /* 1 */
                ...
                mem_idx = stage_fc->slices[f_pl->pool_index].start_tcam_idx
    ...
    for (i = 0; i < num_fields; i++) {
        ...
        case METER_PAIR_MODEf:
            ...
            meter_conf.meter_test_even = PolicyGet(unit, mem, 
                                                   ...
                                                   METER_TEST_EVENf);
            BCM_IF_ERROR_RETURN
                (_field_fb_meter_recover(unit, f_ent, &meter_conf, part, pid));
            break;
                           ...
                           )
{
    ...
    while (current) {
        ...
        if (BCM_PBMP_MEMBER(current->pbmp, port) || (port == -1)) {
            *gid = current->gid;
                                      ...
                                      int slice_num, int *multigroup)
{
    ...
    FP_VERB(("Read slice index %d @ byte %d\n", slice_idx, fc->scache_pos));
    if (multigroup) {
    ...
    while (next_group_valid) {
        ...
        if (fc->flags & _FP_STABLE_SAVE_LONG_IDS) {
            ...
            fc->scache_pos++;
            gid |= (buf[fc->scache_pos] << 4); /* GID_hi */
            port = 0;
    ...
    FP_VERB(("Done reading slice @ %d.\n", fc->scache_pos));
    return rv;
}

int
_field_entry_info_retrieve(int unit, bcm_field_entry_t *eid, int *prio,
                           _field_control_t *fc, int multigroup, int *prev_prio,
                           bcm_field_stat_t *sid, bcm_policer_t *pid)
{
    ...
    *eid = *sid = *pid = 0;
    ...
    if (fc->flags & _FP_STABLE_SAVE_LONG_IDS) {
        ...
        *eid |=  buf[fc->scache_pos]; /* EID_lo */
        fc->scache_pos++;
        *eid |= (buf[fc->scache_pos] & 0xF) << 8; /* EID_hi */
    ...
    FP_VERB(("Read entry id %d @ byte %d.\n", *eid, fc->scache_pos - 2));
    ...
    if (fc->flags & _FP_STABLE_SAVE_LONG_IDS) {
        ...
        if (pol_present) {
            *pid |= buf[fc->scache_pos]; /* PID_lo */
            fc->scache_pos++;
        }
        if (stat_present) {
            *sid |= buf[fc->scache_pos] & 0xF; /* SID_lo */
            if (pol_present) {
                *pid |= buf[fc->scache_pos] >> 4; /* PID_hi */
            }
            fc->scache_pos++;
            *sid |= (buf[fc->scache_pos] << 4); /* SID_hi */
            fc->scache_pos++;
        } else if (pol_present) {
            *pid |= buf[fc->scache_pos] >> 4; /* PID_hi */
            fc->scache_pos++;
                           ...
                           int slice_idx)
{
    ...
    while (fg_ptr != NULL) {
        ...
        if (fc->l2warm) {
            ...
            BCM_IF_ERROR_RETURN
                (_field_group_info_retrieve(unit, port, &gid, &priority, &qset, fc));
            if (gid != -1) {
                ...
                for (j = 0; j < f_qual_arr->conf_sz; j++) {
                    ...
                    if (selector_match) {
                        ...
                        if (k == fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].size) {
                            ...
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset = 
                                f_qual_arr->conf_arr[j].offset.offset;
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width = 
                                f_qual_arr->conf_arr[j].offset.width;
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset1 = 
                                f_qual_arr->conf_arr[j].offset.offset1;
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width1 = 
                                f_qual_arr->conf_arr[j].offset.width1;
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset2 = 
                                f_qual_arr->conf_arr[j].offset.offset2;
                            fg_ptr->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width2 = 
                                f_qual_arr->conf_arr[j].offset.width2;
...
_field_fb2_stage_ingress_reinit(int unit, _field_control_t *fc, 
                                _field_stage_t *stage_fc)
{
    int idx, slice_idx, vslice_idx,index_min, index_max, ratio, rv = BCM_E_NONE;
    int entries_per_slice, group_found, mem_sz, parts_count, qual_idx;
    int i, j, k, pri_tcam_idx, part_index, slice_number, expanded[16];
    ...
    if ((rv = _field_slice_expanded_status_get(unit, stage_fc, 
                                               expanded)) < 0) {
        goto cleanup;
    }
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        for (idx = 0; idx < fs->entry_count / ratio; idx++) {
            ...
            sid = pid = -1;
            if (fc->l2warm) {
                rv = _field_entry_info_retrieve(unit, &eid, &prio, fc, 
                ...
                for (j = 0; j < fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].size; j++) {
                    ...
                    for (k = 0; k < f_qual_arr->conf_sz; k++) {
                        if (_mask_is_set(unit, &f_qual_arr->conf_arr[k].offset,
                            ...
                            if (qualified) {
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].field = 
                                    f_qual_arr->conf_arr[k].offset.field;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].offset = 
                                    f_qual_arr->conf_arr[k].offset.offset;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].width = 
                                    f_qual_arr->conf_arr[k].offset.width;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].offset1 = 
                                    f_qual_arr->conf_arr[k].offset.offset1;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].width1 = 
                                    f_qual_arr->conf_arr[k].offset.width1;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].offset2 = 
                                    f_qual_arr->conf_arr[k].offset.offset2;
                                fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[j].width2 = 
                                    f_qual_arr->conf_arr[k].offset.width2;
                            }
                            break;
                        } 
                    }
            ...
            if (fc->l2warm) {
                ...
                f_ent->prio = (vslice_idx << 10) | (entries_per_slice - idx);
            }
        ...
        if (efp_slice_mode != 3) { /* Invalid value */
            ...
            for (i = 0; i < parts_count; i++) {
                for (j = 0; j < list_size[i]; j++) {
                    ...
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].field = 
                        f_qual_arr->conf_arr[k].offset.field;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset = 
                        f_qual_arr->conf_arr[k].offset.offset;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width = 
                        f_qual_arr->conf_arr[k].offset.width;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset1 = 
                        f_qual_arr->conf_arr[k].offset.offset1;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width1 = 
                        f_qual_arr->conf_arr[k].offset.width1;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].offset2 = 
                        f_qual_arr->conf_arr[k].offset.offset2;
                    fg->qual_arr[_FP_ENTRY_TYPE_DEFAULT][i].offset_arr[size - 1].width2 = 
                        f_qual_arr->conf_arr[k].offset.width2;
                               ...
                               _field_stage_t *stage_fc)
{
    ...
    int entries_per_slice, group_found, mem_sz, parts_count, qual_idx;
    int i, j, k, pri_tcam_idx, part_index, slice_number, prio, prev_prio; 
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        if (paired) {
            ...
            if (intraslice) {
                dbl_wide_key_sec = soc_reg_field_get(unit, VFP_KEY_CONTROLr,
                            vfp_key, _vfp_slice_wide_mode_field[slice_idx]);
                hw_sels.sec_slice[1].intraslice = 1;
                hw_sels.sec_slice[1].fpf2 = dbl_wide_key_sec;
            }
        }
        ...
        for (idx = 0; idx < fs->entry_count / ratio; idx++) {
            ...
            if (fc->l2warm) {
                ...
                f_ent->prio = (slice_idx << 10) | (entries_per_slice - idx);
            }
            rv = _field_group_entry_add(unit, fg, f_ent);
            if (BCM_FAILURE(rv)) {
                sal_free(f_ent);
    ...
    bcm_port_t port, _field_fb_sel_codes_t *hw_sel_codes)
{
    ...
    if (fg_p == NULL)
    {
        ...
        {
                rv = _field_group_id_generate(unit, &group_id);
            ...
            ++qualifier_id)
        {
            ...
            if (config_index < stage_qualifier_p->conf_sz)
            {
                ...
                if (qualifier_id_index == part_qualifier_p->size)
                {
                    ...
                    offset_p->field   = config_p->offset.field;
                    offset_p->offset  = config_p->offset.offset;
                    offset_p->width   = config_p->offset.width;
                    offset_p->offset1 = config_p->offset.offset1;
                    offset_p->width1  = config_p->offset.width1;
                    offset_p->offset2 = config_p->offset.offset2;
                    offset_p->width2  = config_p->offset.width2;
    ...
    if (fg_p->slices == NULL)
    {
        fg_p->slices = stage_fc->slices + slice_index;
    }
    ...
    _field_stage_t *stage_fc)
{
    ...
    FP_VVERB(("Beginning ingress stage recovery.\n"));
    ...
    for (slice_index = 0; slice_index < stage_fc->tcam_slices; ++slice_index)
    {
        ...
        fs_p = stage_fc->slices + slice_index;
        ...
        if ((entry_index == fs_p->entry_count) && !fc->l2warm)
        {
            ...
            FP_VVERB(("    No valid groups and entries in slice.\n"));
        ...
        for (entry_index = 0; entry_index < fs_p->entry_count; ++entry_index)
        {
            ...
            for (fg_p = fc->groups; fg_p != NULL; fg_p = fg_p->next)
            {
                ...
                if (SOC_PBMP_EQ(fg_p->pbmp, entry_pbmp))
                {
                    /* Found a match */
            ...
            if (fg_p == NULL)
            {
                ...
                FP_ERR(("Could not find a matching group for entry %d.\n",
                    entry_index));
            ...
            _FP_FB_IF_FAILURE_CLEANUP(
                _bcm_field_entry_tcam_parts_count(unit, fg_p->flags,
                    &part_count));
            ...
            FP_VVERB(("        Entry matches group %d.\n", fg_p->gid));
            ...
            if (fc->l2warm)
            {
                rv = _field_entry_info_retrieve(unit, &entry_id, &prio, fc,
                    multigroup, &prev_prio, &stat_id, &policer_id);

                if (BCM_FAILURE(rv))
                {
                    FP_ERR(("Failed to retrieve entry info."));
            ...
            else
            {
                ...
                entry_id = _bcm_field_last_alloc_eid_get();
            }
            ...
            for (part_index = 0; part_index < part_count; ++part_index)
            {
                ...
                rv = _bcm_field_tcam_part_to_entry_flags(part_index,
                        fg_p->flags, &entry_p->flags);
                if (BCM_FAILURE(rv)) {
                    sal_free(entries_p);
                    goto cleanup;
                }
                ...
                rv = _bcm_field_entry_part_tcam_idx_get(unit, entry_p,
                ...
                rv = _bcm_field_tcam_idx_to_slice_offset(unit, stage_fc,
                        part_tcam_index, &part_slice_index,
                ...
                entry_p->fs = stage_fc->slices + part_slice_index;
                ...
                rv = _field_fb_actions_recover(unit,
                    (uint32 *)policy_entry_p, entry_p, part_index,
                    FP_POLICY_TABLEm, stat_id, policer_id);
                if (BCM_FAILURE(rv)) {
                    sal_free(entries_p);
                    goto cleanup;
                }
            ...
            FP_VVERB(("        Entry id = %d, priority = %d.\n", entry_p->eid,
                entry_p->prio));
    ...
    FP_VVERB(("Ingress stage recovery complete.\n"));
...
cleanup:
    ...
    FP_VVERB(("Leaving ingress stage recovery.\n"));
                                     ...
                                     uint32 p_entry_sec[])
{
    ...
    if ((mode = soc_mem_field32_get(unit, policy_mem, 
        ...
        if (f_ent_pri->fs->slice_number == f_pl->pool_index) {
            ...
            sal_memcpy(p_entry_sec, p_entry_pri, soc_mem_entry_words(unit, policy_mem)*sizeof(uint32)); 
            sal_memset(p_entry_pri, 0, soc_mem_entry_words(unit, policy_mem)*sizeof(uint32)); 
            soc_mem_field32_set(unit, policy_mem, (uint32 *) p_entry_sec, 
        ...
        if (f_ent_pri->fs->slice_number == f_pl->pool_index) {
            sal_memcpy(p_entry_pri, p_entry_sec, soc_mem_entry_words(unit, policy_mem)*sizeof(uint32)); 
            sal_memset(p_entry_sec, 0, soc_mem_entry_words(unit, policy_mem)*sizeof(uint32)); 
            soc_mem_field32_set(unit, policy_mem, (uint32 *) p_entry_pri, 
                                METER_PAIR_MODEf, mode);
            soc_mem_field32_set(unit, policy_mem, (uint32 *) p_entry_pri,
                                METER_INDEX_EVENf, f_pl->hw_index);
    ...
    for (idx = parts_count - 1; idx >= 0; idx--) {
...
#ifdef BCM_TRIUMPH_SUPPORT
        {
            if ((SOC_IS_TR_VL(unit)) && 
                 ...
                 */
        if ((SOC_IS_SC_CQ(unit)) && (stage_id == _BCM_FIELD_STAGE_INGRESS)) {
                        rv = soc_mem_read(unit, FP_TCAM_Xm, MEM_BLOCK_ANY, tcam_idx_old[idx], e);
            BCM_IF_ERROR_RETURN(rv);

            rv = soc_mem_write(unit, FP_TCAM_Xm, MEM_BLOCK_ALL, tcam_idx_new[idx], e);
            BCM_IF_ERROR_RETURN(rv);

            rv = soc_mem_read(unit, FP_TCAM_Ym, MEM_BLOCK_ANY, tcam_idx_old[idx], e);
            BCM_IF_ERROR_RETURN(rv);

            rv = soc_mem_write(unit, FP_TCAM_Ym, MEM_BLOCK_ALL, tcam_idx_new[idx], e);
            BCM_IF_ERROR_RETURN(rv);
...
_bcm_field_fb_entry_remove(int unit, _field_entry_t *f_ent, int tcam_idx)
{
    ...
    if (SOC_IS_KATANAX(unit)) {
        if (f_ent->actions != NULL) {
            if ((f_ent->actions->action == bcmFieldActionFabricQueue) &&
                (f_ent->fs->stage_id == _BCM_FIELD_STAGE_INGRESS)){
                sal_memset(&ing_queue_entry, 0, sizeof(ing_queue_map_entry_t));
                param0 = f_ent->actions->param[0];
                param1 = f_ent->actions->param[1];
                ...
                if ((param1 & BCM_FABRIC_QUEUE_DEST_OFFSET) ||
                    ...
                    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(param0) ||
                        ...
                        if (SOC_IS_KATANA2(unit)) {
                            BCM_IF_ERROR_RETURN(_bcm_kt2_cosq_index_resolve(
                                unit, param0, 0,
