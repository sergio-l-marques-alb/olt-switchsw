...
#define BCM_RX_WB_DEFAULT_VERSION                 BCM_RX_WB_VERSION_1_0
...
_bcm_esw_rx_recover(int unit)
{
   ...
   if (soc_feature(unit, soc_feature_packet_rate_limit)) {
      ...
      if (SOC_IS_TD2_TT2(unit)) {
          for (cosq = 0; cosq <= RX_QUEUE_MAX(unit); cosq++) {
               BCM_IF_ERROR_RETURN(bcm_td2_cosq_port_pps_get(unit, cpu_port,
                                            cosq, &RX_COS_PPS(unit, cosq)));
               BCM_IF_ERROR_RETURN(bcm_td2_cosq_port_burst_get(unit, cpu_port,
      ...
      if (SOC_IS_TRIUMPH3(unit)) {
          for (cosq = 0; cosq <= RX_QUEUE_MAX(unit); cosq++) {
               BCM_IF_ERROR_RETURN(bcm_tr3_cosq_port_pps_get(unit, cpu_port,
                                            cosq, &RX_COS_PPS(unit, cosq)));
               BCM_IF_ERROR_RETURN(bcm_tr3_cosq_port_burst_get(unit, cpu_port,
                                            cosq, &RX_COS_BURST(unit, cosq)));
          }
      } 
#endif
...
_bcm_katana2_cpu_cos_map_key [] =
{
    ...
    bcmRxReasonInvalid,                 /* MPLS_ALERT_LABEL */
    bcmRxReasonInvalid,                 /* MPLS_ILLEGAL_RESERVED_LABEL */
};
#endif
