/*
 * $Id: l3.c 1.132.2.3 Broadcom SDK $
...
_bcm_tr3_l3_esm_add(int unit, _bcm_l3_cfg_t *l3cfg, int nh_idx)
{
    ...
    /* Perform lookup */
    rv = soc_tr3_search_ext_mem(unit, &mem, bufp, &l3cfg->l3c_hw_index);
    if (BCM_E_NONE == rv) { /* Entry exists */
        if (l3cfg->l3c_flags & BCM_L3_REPLACE) {
            tbl_free_entry = l3cfg->l3c_hw_index;
        } else {
            return rv;
        }
    ...
    *cf_count = 0;
    ...
    for (bidx = 0; bidx < mem_banks.count; bidx++) {
        ...
        for (i = 0; (i < num_ent) && (*cf_count < cf_max);) {
            ...
            i += soc_ism_get_bucket_offset(unit, mem, -1, bufp, l3_entry);
        }
    } 
    return BCM_E_NONE;
}
...
_bcm_tr3_defip_pair128_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_l3_expanded_defip_table)) {/* HX4, Ranger */
        defip_config = soc_property_get(unit, spn_IPV6_LPM_128B_ENABLE,
                                            ((ranger_device) ? 0 : 1));

        switch (defip_config) {
          case 1: /* Config C3 */
            ipv6_128_depth = 2 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = _BCM_HX4_RANGER_DEFIP_TCAM_DEPTH;
    }
            break;
          case 2: /* Config C2 */
            ipv6_128_depth = _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = 0;
    }
            break;
          case 3: /* Config C4 */
            ipv6_128_depth = 3 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = 0;
    }
            break;
          case 4: /* Config C5 */
            ipv6_128_depth = 4 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = 0;
    }
            break;
          case 6: /* Config C24 */
            ipv6_128_depth = 8 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = 0;
    }
            break;
          case 8: /* Config C26 */
            ipv6_128_depth = 16 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            if (ranger_device) {
                ipv6_128_depth = 0;
    } 
            break;
...
_bcm_tr3_l3_get(int unit, _bcm_l3_cfg_t *l3cfg, int *nh_idx)
{
    ...
    if (BCM_E_NOT_FOUND == rv && BCM_TR3_ESM_HOST_TBL_PRESENT(unit)) {   
        /* Not found in ISM, search ESM */
        return (_bcm_tr3_l3_esm_get(unit, l3cfg, nh_idx, &embd));
    } else if (BCM_FAILURE(rv)) { 
        /* ISM search failed */
        return rv;
    } else {                      
    ...
    if (BCM_TR3_ESM_HOST_TBL_PRESENT(unit)) {
        ...
        for(tbl = 0; tbl < num_tables; tbl++) {
            ...
            for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
                ...
                /* Iterate over all the entries - show matching ones. */
                for (idx = idx_min; idx <= idx_max; idx++) {
                    while (idx < _TR3_ESM_HOST_TBL_FREE_ENTRY(unit,
                                                   (tbl+(2 * ipv6)))) {
                        /* Reset buffer before read. */
                        sal_memset(&l3cfg, 0, sizeof(_bcm_l3_cfg_t));
                        l3cfg.l3c_flags = flags;
...
bcm_tr3_l3_replace(int unit, _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (BCM_XGS3_L3_MCAST_ENTRY(l3cfg)) {
        ...
        embd_new = BCM_TR3_L3_USE_EMBEDDED_NEXT_HOP(unit, l3cfg->l3c_intf, nh_idx_old);
                        ...
                        _bcm_l3_cfg_t *l3cfg, int *nh_idx)
{
    ...
    if (l3cfg->l3c_flags & BCM_L3_IPMC) {
        ...
        mem = (v6) ? L3_ENTRY_2m : L3_ENTRY_1m;
...
_bcm_hx4_l3_defip_init(int unit)
{
    ...
    /* defip_key_sel_1 = 0000_0000_0000_0000_0000_0000_0000_0000 = 0x0 */
    defip_key_sel_1 = 0x0;

    /* defip_key_sel_0 reg. & set depth of v4, v6/64 route table */ 
    switch (defip_config) {
      case 1: /* Config C3 defip_key_sel_0 = 0000_0000_0000_0011 = 0x3 */
        defip_key_sel_0 = 0x3;
        ipv6_64_depth = 4 * _BCM_TR3_DEFIP_TCAM_DEPTH;
        defip_cam_enable = 0xf;
        if (ranger_device) {
            ipv6_64_depth = 0;
        }
        break;
      case 2: /* Config C2 defip_key_sel_0 = 0000_0000_0000_0001 = 0x1 */
        defip_key_sel_0 = 0x1;
        ...
        break;
      case 3: /* Config C4 defip_key_sel_0 = 0000_0000_0000_0111 = 0x7 */
        defip_key_sel_0 = 0x7;
        ipv6_64_depth = 2 * _BCM_TR3_DEFIP_TCAM_DEPTH;
        defip_cam_enable = 0xf;
        if (ranger_device) {
            ipv6_64_depth = 0;
        }
        break;
      case 4: /* Config C5 defip_key_sel_0 = 0000_0000_0000_1111 = 0xf */
        defip_key_sel_0 = 0xf;
        ipv6_64_depth = 0;
        defip_cam_enable = 0xf;
        break;
      case 5: /* Config C23 defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
        defip_key_sel_0 = 0x0;
        ipv6_64_depth = 16 * _BCM_TR3_DEFIP_TCAM_DEPTH;
        defip_cam_enable = 0xff;
        if (ranger_device) {
            ipv6_64_depth = 0;
        }
        break;
      case 6: /* Config C24 defip_key_sel_0 = 0000_0000_1111_1111 = 0xff */
        defip_key_sel_0 = 0xff;
        ipv6_64_depth = 0;
        defip_cam_enable = 0xff;
        break;
      case 7: /* Config C25 defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
        defip_key_sel_0 = 0x0;
        ipv6_64_depth = 32 * _BCM_TR3_DEFIP_TCAM_DEPTH;
        defip_cam_enable = 0xffff;
        if (ranger_device) {
            ipv6_64_depth = 0;
        }
        break;
      case 8: /* Config C26 defip_key_sel_0 = 0000_0000_1111_1111 = 0xff */
        defip_key_sel_0 = 0xff;
        ipv6_64_depth = 0;
        defip_cam_enable = 0xffff;
        break;
      default: /* Config C1 defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
      case 0: 
        defip_key_sel_0 = 0x0;
    ...
    BCM_IF_ERROR_RETURN(WRITE_L3_DEFIP_CAM_ENABLEr(unit, defip_cam_enable));
...
_bcm_tr3_l3_defip_init(int unit)
{
    ...
    switch (defip_config) {
      ...
      case 1:
        ...
        break;
      default: /* reset values correspond to 'case 0' above */
        break;
    }
    } else { /* Tr3 case */
...
_bcm_hx4_l3_defip_urpf_enable(int unit, int enable)
{
    ...
    if (enable) {
        ...
        switch (defip_config) {
          ...
          case 2:
          case 3:
            /* defip_key_sel_0 = 0000_0000_0000_0101 = 0x5 */
            /* defip_key_sel_1 = 0000_0000_0000_0000_0000_0000_1111_0000 = 0xf0 */
            /* defip_cam_enable= 0000_0000_0000_1111 = 0xf */
            defip_key_sel_0 = 0x5;
            defip_key_sel_1 = 0xf0;
            defip_cam_enable = 0xf;
            ipv6_64_depth = 2 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_depth = _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset =_BCM_TR3_DEFIP_TCAM_DEPTH;
            break;
          case 4: /* Config C8 */
            /* defip_key_sel_0 = 0000_0000_0000_1111 = 0xf */
            /* defip_key_sel_1 = 0000_0000_0000_0000_0000_0000_1111_0000 = 0xf0 */
            /* defip_cam_enable= 0000_0000_0000_1111 = 0xf */
            defip_key_sel_0 = 0xf;
            defip_key_sel_1 = 0xf0;
            ...
            ipv6_64_depth = 0;
            ipv6_128_depth = 2 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset = 2 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            break;
          case 5: /* Config C27 */
            /* defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
            /* defip_key_sel_1 = 0000_0000_0000_0000_1111_1111_0000_0000 = 0xff00 */
            /* defip_cam_enable= 0000_0000_1111_1111 = 0xff */
            defip_key_sel_0 = 0x0;
            ...
            break;
          case 6: /* Config C28 */
            /* defip_key_sel_0 = 0000_0000_1111_1111 = 0xff */
            /* defip_key_sel_1 = 0000_0000_0000_0000_1111_1111_0000_0000 = 0xff00 */
            /* defip_cam_enable= 0000_0000_1111_1111 = 0xff */
            defip_key_sel_0 = 0xff;
            defip_key_sel_1 = 0xff00;
            ...
            ipv6_64_depth = 0;
            ipv6_128_depth = 4 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset = 4 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            break;
          case 7: /* Config C29 */
            /* defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
            /* defip_key_sel_1 = 1111_1111_1111_1111_0000_0000_0000_0000 = 0xffff0000 */
            /* defip_cam_enable= 1111_1111_1111_1111 = 0xffff */
            defip_key_sel_0 = 0x0;
            defip_key_sel_1 = 0xffff0000;
            defip_cam_enable = 0xffff;
            ipv6_64_depth = 16 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_depth = 0;
            ipv6_128_dip_offset = 0;
            break;
          case 8: /* Config C30 */
            /* defip_key_sel_0 = 1111_1111_1111_1111 = 0xffff */
            /* defip_key_sel_1 = 1111_1111_1111_1111_0000_0000_0000_0000 = 0xffff0000 */
            /* defip_cam_enable= 1111_1111_1111_1111 = 0xffff */
            defip_key_sel_0 = 0xffff;
            defip_key_sel_1 = 0xffff0000;
            defip_cam_enable = 0xffff;
            ipv6_64_depth = 0;
            ipv6_128_depth = 8 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset = 8 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            break;
          default:
          case 0: /* Config C6 */
            /* defip_key_sel_0 = 0000_0000_0000_0000 = 0x0 */
...
_bcm_tr3_l3_defip_urpf_enable(int unit, int enable)
{
    ...
    if (enable) { 
        if (soc_feature(unit, soc_feature_l3_reduced_defip_table)) {/*Apollo2*/
            ...
            switch (defip_config) {
              case 1:
                /* L3_DEFIP_KEY_SELr = 011100000101 */ 
                soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                                       V6_KEY_SEL_CAM0_1f, 0x1);
                soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                                       V6_KEY_SEL_CAM4_5f, 0x1);
                ipv6_64_depth      = _BCM_TR3_DEFIP_TCAM_DEPTH; 
                ipv6_128_depth     = _BCM_TR3_DEFIP_TCAM_DEPTH; 
                ipv6_128_dip_offset= _BCM_TR3_DEFIP_TCAM_DEPTH;
                break;
              case 0:
              ...
              default: 
                /* L3_DEFIP_KEY_SELr = 011100000000 */ 
                break;
            }
        } else { /* TR3 */
        ...
        switch (defip_config) {
          ...
          case 0: /* Default ISM route table config - non URPF mode */
          default:
                ipv6_64_depth      = 6 * _BCM_TR3_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset = 0;
            ipv6_128_depth      = 0;
...
_bcm_tr3_l3_intf_mtu_get(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    BCM_IF_ERROR_RETURN(BCM_XGS3_MEM_READ(unit, ING_L3_NEXT_HOP_ATTRIBUTE_1_INDEXm,
                                          intf_info->l3i_index, 
                                          mtu_idx_value_buf_p));
    mtu_profile_index = 
 ...
 */
int
_bcm_tr3_l3_ecmp_grp_add(int unit, int ecmp_grp, void *buf, int max_paths)
{
    ...
    if (BCM_XGS3_L3_ENT_REF_CNT(BCM_XGS3_L3_TBL_PTR(unit,
        ...
        BCM_XGS3_L3_ENT_REF_CNT_INC(data.tbl_ptr, ecmp_idx, max_paths);
        ecmp_table_incr = 1;
    }

    sal_memcpy(&ecmp_grp_entry, ecmp_grp_null_entry, sizeof(ecmp_count_entry_t));

    for (idx = 0, nh_cycle_idx = 0; idx < max_paths; idx++, nh_cycle_idx++) {
        sal_memcpy(&ecmp_entry, ecmp_null_entry, sizeof(ecmp_entry_t));
 ...
 */
int
_bcm_tr3_l3_ecmp_grp_del(int unit, int ecmp_grp, int max_grp_size)
{
    int idx, ecmp_idx;
    _bcm_l3_tbl_op_t data;
    void *ecmp_null_entry, *initial_ecmp_null_entry;
 ...
 */
int 
bcm_tr3_l3_egress_ecmp_dlb_destroy(int unit, bcm_l3_egress_ecmp_t *ecmp)
{
    int ecmp_group_idx;
        ...
        bcm_l3_egress_t *l3_egress) 
{
    ...
    if (l3_egress->encap_id == 0) {
         ...
         */
        rv = bcm_tr3_ipmc_l3_intf_next_hop_get(unit, l3_egress->intf, &nh_index_out);
        if (rv == BCM_E_NONE) {
             ...
             */
            BCM_IF_ERROR_RETURN
                (bcm_tr3_ipmc_l3_intf_next_hop_l3_egress_set(unit, l3_egress->intf));
            rv = BCM_E_NONE;
        } else {
            return rv;
        }
    ...
    if (soc_feature(unit, soc_feature_virtual_port_routing)) {
        ...
        if (l3_egress->encap_id > 0 &&
            ...
            if (_bcm_vp_used_get(unit, vp, _bcmVpTypeNiv)) {
                ...
                soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                        L3MC__VNTAG_DST_VIFf, eport.extended_port_vid);
                if (eport.pcp_de_select == BCM_EXTENDER_PCP_DE_SELECT_DEFAULT) {
                    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            L3MC__ETAG_PCP_DE_SOURCEf, 2);
                    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            L3MC__ETAG_PCPf, eport.pcp);
                    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            L3MC__ETAG_DEf, eport.de);
                } else if (eport.pcp_de_select == BCM_EXTENDER_PCP_DE_SELECT_PHB) {
                    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            L3MC__ETAG_PCP_DE_SOURCEf, 3);
                    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                            L3MC__ETAG_DOT1P_MAPPING_PTRf, 0);
                }
...
bcm_tr3_ecmp_dlb_sw_dump(int unit)
{
    ...
    soc_cm_print("  ECMP DLB Info -\n");

    /* Print ECMP DLB next hop info */
    soc_cm_print("    ECMP DLB Next Hop Info -\n");
    for (i = 0; i < ECMP_DLB_INFO(unit)->ecmp_dlb_nh_info_size; i++) {
        if (ECMP_DLB_NH_INFO(unit, i).valid) {
            soc_cm_print("      Next hop index %d is ",
                    ECMP_DLB_NH_INFO(unit, i).nh_index);
            current_ptr = ECMP_DLB_NH_INFO(unit, i).nh_membership_list;
            while (NULL != current_ptr) {
                soc_cm_print("member %d in group %d",
    ...
    soc_cm_print("    ECMP DLB Groups Used:");
    for (i = 0; i < soc_mem_index_count(unit, DLB_ECMP_GROUP_CONTROLm); i++) {
        if (_BCM_ECMP_DLB_ID_USED_GET(unit, i)) {
            soc_cm_print(" %d", i);
        }
    ...
    soc_cm_print("    ECMP DLB Flowset Table Blocks Used:");
    ...
    soc_cm_print("    ECMP DLB Member IDs Used:");
    ...
    soc_cm_print("    Expected load min threshold: %d mbps\n",
            ECMP_DLB_INFO(unit)->ecmp_dlb_exp_load_min_th);
    soc_cm_print("    Expected load max threshold: %d mbps\n",
            ECMP_DLB_INFO(unit)->ecmp_dlb_exp_load_max_th);
    soc_cm_print("    Imbalance min threshold: %d percent\n",
            ECMP_DLB_INFO(unit)->ecmp_dlb_imbalance_min_th);
    soc_cm_print("    Imbalance max threshold: %d percent\n",
            ECMP_DLB_INFO(unit)->ecmp_dlb_imbalance_max_th);
...
_bcm_tr3_esm_host_tbl_sw_dump(int unit) {
    ...
    int tbl;
    for (tbl = 0; tbl < _BCM_TR3_NUM_ESM_HOST_TABLES ; tbl++) {
        soc_cm_print("ESM Host table %d, free entry: %d\n", tbl, 
