/*
 * $Id: time.c 1.74.26.1 Broadcom SDK $
...
_bcm_esw_time_hw_clear(int unit, bcm_time_if_t intf_id)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        ...
        soc_reg_field_set(
            unit, CMIC_BS0_CONFIGr, &regval, MODEf, TIME_MODE_OUTPUT);
        ...
        soc_reg_field_set(
            unit, CMIC_BS1_CONFIGr, &regval, MODEf, TIME_MODE_OUTPUT);
...
_bcm_esw_time_interface_heartbeat_install(int unit, bcm_time_if_t id)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        ...
        READ_CMIC_BS0_HEARTBEAT_UP_DURATIONr(unit, &interval);
        soc_reg_field_set(unit, CMIC_BS0_HEARTBEAT_UP_DURATIONr, &interval, 
                          UP_DURATIONf, threshold); 
        WRITE_CMIC_BS0_HEARTBEAT_UP_DURATIONr(unit, interval);
        READ_CMIC_BS0_HEARTBEAT_DOWN_DURATIONr(unit, &interval);
        soc_reg_field_set(unit, CMIC_BS0_HEARTBEAT_DOWN_DURATIONr, &interval, 
                          DOWN_DURATIONf, threshold); 
        WRITE_CMIC_BS0_HEARTBEAT_DOWN_DURATIONr(unit, interval);

        READ_CMIC_BS1_HEARTBEAT_UP_DURATIONr(unit, &interval);
        soc_reg_field_set(unit, CMIC_BS1_HEARTBEAT_UP_DURATIONr, &interval, 
                          UP_DURATIONf, threshold); 
        WRITE_CMIC_BS1_HEARTBEAT_UP_DURATIONr(unit, interval);
        READ_CMIC_BS1_HEARTBEAT_DOWN_DURATIONr(unit, &interval);
        soc_reg_field_set(unit, CMIC_BS1_HEARTBEAT_DOWN_DURATIONr, &interval, 
                          DOWN_DURATIONf, threshold); 
        WRITE_CMIC_BS1_HEARTBEAT_DOWN_DURATIONr(unit, interval);
    }
#endif /* BCM_TIME_V3_SUPPORT || BCM_KATANA2_SUPPORT || BCM_HELIX4_SUPPORT */
#if (defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT))
    /* Install heartbeat and external trigger clock interval */
...
_bcm_esw_time_interface_dual_bs_install(int unit, bcm_time_if_t intf_id)
{
    ...
    if ((intf->flags & BCM_TIME_ENABLE)) {
         ...
         */
        soc_reg_field_set(
                unit, CMIC_BS0_CONFIGr, &regval, BS_CLK_OUTPUT_ENABLEf, 1);
        soc_reg_field_set(
                unit, CMIC_BS0_CONFIGr, &regval, BS_HB_OUTPUT_ENABLEf, 1);
        soc_reg_field_set(
                unit, CMIC_BS0_CONFIGr, &regval, BS_TC_OUTPUT_ENABLEf, 1);
    } else {
        soc_reg_field_set(
                unit, CMIC_BS0_CONFIGr, &regval, BS_CLK_OUTPUT_ENABLEf, 0);
        soc_reg_field_set(
                unit, CMIC_BS0_CONFIGr, &regval, BS_TC_OUTPUT_ENABLEf, 0);
    }

    if ((intf->flags & BCM_TIME_LOCKED)) {
        soc_reg_field32_modify(
                unit, CMIC_BS0_OUTPUT_TIME_0r, REG_PORT_ANY, LOCKf, 1);
    } else {
        soc_reg_field32_modify(
                unit, CMIC_BS0_OUTPUT_TIME_0r, REG_PORT_ANY, LOCKf, 0);
    }


    if (intf->flags & BCM_TIME_INPUT) {
        /* Set Broadsync in Input mode */
        soc_reg_field_set(unit, CMIC_BS0_CONFIGr, &regval, MODEf, 
                          TIME_MODE_INPUT);
    } else {
        /* Configure Accuracy, offset and drift for broadsync output mode */
        soc_reg_field_set(unit, CMIC_BS0_CONFIGr, &regval, MODEf, 
                                   ...
                                   bcm_time_capture_t *time)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        ...
        READ_CMIC_BS0_OUTPUT_TIME_1r(unit, &regval);
        COMPILER_64_SET(time->free.seconds, 0,  
                soc_reg_field_get(unit, CMIC_BS0_OUTPUT_TIME_1r, 
                    regval, SECOND_TIMEf)); 
        READ_CMIC_BS0_OUTPUT_TIME_2r(unit, &regval);
        time->free.nanoseconds =  
            soc_reg_field_get(unit, CMIC_BS0_OUTPUT_TIME_2r, 
                    regval, NS_TIMEf); 
    } else 
#endif /* BCM_TIME_V3_SUPPORT || BCM_KATANA2_SUPPORT || BCM_HELIX4_SUPPORT */
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    ...
    defined(BCM_KATANA2_SUPPORT) || defined(BCM_HELIX4_SUPPORT)
        if (soc_feature(unit, soc_feature_time_v3) || 
            ...
            data2 = soc_reg_field_get(unit, CMIC_BS0_INPUT_TIME_2r, regval, DATAf);
            READ_CMIC_BS0_INPUT_TIME_1r(unit, &regval);
            data1 = soc_reg_field_get(unit, CMIC_BS0_INPUT_TIME_1r, regval, DATAf);
            READ_CMIC_BS0_INPUT_TIME_0r(unit, &regval);
            data0 = soc_reg_field_get(unit, CMIC_BS0_INPUT_TIME_0r, regval, DATAf);
        } else
#endif /* BCM_TIME_V3_SUPPORT || BCM_KATANA2_SUPPORT || BCM_HELIX4_SUPPORT */
        {
...
_bcm_esw_time_capture_get (int unit, bcm_time_if_t id, bcm_time_capture_t *time)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        ...
        if (TIME_CAPTURE_MODE_HEARTBEAT == soc_reg_field_get(unit, 
                      CMIC_TIMESYNC_TIME_CAPTURE_MODEr, regval, TIME_CAPTURE_MODEf)) {

            READ_CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr(unit, &regval);
            soc_reg_field_set(unit, 
                CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr, &regval, BSYNC0_TX_HB_STATUS_ENABLEf, 1);
            soc_reg_field_set(unit, 
                CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr, &regval, TIME_CAPTURE_ENABLEf, 1);
        ...
        WRITE_CMIC_TIMESYNC_TIME_CAPTURE_MODEr(unit, orgval);
    } else
#endif /* BCM_TIME_V3_SUPPORT || BCM_KATANA2_SUPPORT || BCM_HELIX4_SUPPORT */
#if (defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT))
    if (SOC_IS_KATANA(unit) || SOC_IS_TRIUMPH3(unit) ||
...
bcm_esw_time_heartbeat_enable_set (int unit, bcm_time_if_t id, int enable)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        ...
        soc_reg_field_set(unit, CMIC_TIMESYNC_INTERRUPT_ENABLEr, &regval, 
                          INT_ENABLEf, (enable) ?
                          TIME_CAPTURE_MODE_HEARTBEAT : 0);
        WRITE_CMIC_TIMESYNC_INTERRUPT_ENABLEr(unit, regval);

        READ_CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr(unit, &regval);
        /* Configure the HW to raise interrupt on every heartbeat */
        soc_reg_field_set(unit, CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr, &regval, 
                            TIME_CAPTURE_ENABLEf, 1 );
        soc_reg_field_set(unit, CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr, &regval, 
                            BSYNC0_TX_HB_STATUS_ENABLEf, 1 );
        soc_reg_field_set(unit, CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr, &regval, 
                            BSYNC0_RX_HB_STATUS_ENABLEf, 1 );
        WRITE_CMIC_TIMESYNC_TIME_CAPTURE_CONTROLr(unit, regval);
...
bcm_esw_time_heartbeat_enable_get (int unit, bcm_time_if_t id, int *enable)
{
    ...
    if (soc_feature(unit, soc_feature_time_v3) || 
        SOC_IS_KATANA2(unit) || SOC_IS_HELIX4(unit)) {
        READ_CMIC_TIMESYNC_INTERRUPT_ENABLEr(unit, &regval);
        *enable = soc_reg_field_get(unit, CMIC_TIMESYNC_INTERRUPT_ENABLEr,
                                    regval, INT_ENABLEf); 
...
bcm_esw_time_trigger_enable_get (int unit, bcm_time_if_t id, uint32 *mode_flags)
{
    ...
    if (SOC_IS_KATANA2(unit) || SOC_IS_HELIX4(unit)) {
        ...
        capture_mode = soc_reg_field_get(unit, CMIC_TIMESYNC_INTERRUPT_ENABLEr,
                                    regval, INT_ENABLEf); 
...
_bcm_esw_time_bs_init(int unit, bcm_time_interface_t *intf)
{
    ...
    if (SOC_IS_KATANA(unit) || SOC_IS_TRIUMPH3(unit) || SOC_IS_TD2_TT2(unit)) {
        ...
        if (SOC_IS_KATANA2(unit) || SOC_IS_HELIX4(unit)) {
            ...
            /* m CMIC_TIIMESYNC_TS0_FREQ_CTRL_UPPER ENABLE=1 */
            READ_CMIC_TIMESYNC_TS0_COUNTER_ENABLEr(unit, &regval);
            soc_reg_field_set(unit, CMIC_TIMESYNC_TS0_COUNTER_ENABLEr, &regval, ENABLEf, 1);
            WRITE_CMIC_TIMESYNC_TS0_COUNTER_ENABLEr(unit, regval);
        } else 
