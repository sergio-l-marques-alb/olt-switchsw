 ...
 */
{
	...
	printk("NET POOL: Unit %d\n", u);
	netPoolShow(socend_seu[u].seu_netpool_id);
    } else {
	printk("Invalid unit: %d\n", u);
    }
}
 ...
 */
{
    ...
    debugk(DK_END, "socend_mempool_alloc: Unit %d %sinitialized\n",
	...
	if (!seu->seu_blks.memArea || !seu->seu_cluster.memArea ||
	    ...
	    printk("socend_mempool_init: Failed for unit %d\n", se->se_unit);
 ...
 */
{
    ...
    if (NULL == is) {
	printk("socend_load: unexpected NULL pointer for device name\n");
	return(NULL);
    } else if ('\0' == *is) {		/* Copy in our device name */
	strcpy(is, "sc");
	return(NULL);
    }
    debugk(DK_END, "socend_load: %s\n", is);
    ...
    if (NULL == (se = malloc(sizeof(*se)))) {
      printk("socend_load: failed to allocate memory for %s\n", is);
      return(NULL);
    }
	...
	printk("socend_load: Invalid soc device: %d\n", se->se_unit);
	free(se);
	return(NULL);
    }
	...
	if (rt) {
	    printk("socend_load: Invalid MAC address specified: %s\n",
		   mac_str);
	    free(se);
	    return(NULL);
	}
 ...
 */
{
    ...
    debugk(DK_END, "socend_unload: %s%d:\n", eo->devObject.name,
	   eo->devObject.unit);
 ...
 */
{
    ...
    debugk(DK_END, "socend_ioctl: unit %d\n", se->se_unit);

    switch((unsigned int)cmd) {
    case EIOCSFLAGS:
	debugk(DK_END, "socend_ioctl: %s%d: EIOCSFLAGS(0x%x)\n",
	...
	break;
    case EIOCGFLAGS:
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGFLAGS\n",
	...
	debugk(DK_END, "socend_ioctl: %s%d: EIOCSADDR\n",
	       eo->devObject.name, eo->devObject.unit);
	if (NULL == data) {
	    error = EINVAL;
	} else {
		...
		debugk(DK_END, "socend_ioctl: Deleting old mac address\n");
		(void)bcm_l2_addr_delete(se->se_unit, se->se_mac, se->se_vlan);
	    }
	    ENET_COPY_MACADDR(data, se->se_mac);
	    debugk(DK_END, "socend_ioctl: Setting new mac address\n");
		...
		printk("socend_ioctl: mac_configure_address failed: "
		       "Network Device %s%d\n",
		       eo->devObject.name, eo->devObject.unit);
		ENET_COPY_MACADDR(mac_zero, se->se_mac);
		error = EIO;
	...
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGADDR\n",
	       eo->devObject.name, eo->devObject.unit);
	if (NULL == data) {
	    error = EINVAL;
	} else {
	...
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGFBUF\n",
	       eo->devObject.name, eo->devObject.unit);
	if (data == NULL) {
	    error = EINVAL;
	} else {
	...
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGMWIDTH\n",
	       eo->devObject.name, eo->devObject.unit);
	if (NULL == data) {
	    error = EINVAL;
	} else {
    ...
    case EIOCPOLLSTART:
    case EIOCPOLLSTOP:
	debugk(DK_END, "socend_ioctl: %s%d: MULTI/POLL\n",
	       eo->devObject.name, eo->devObject.unit);
	error = ENOSYS;
	break;
    case EIOCGMIB2:		/* Get MIB2 Table */
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGMIB2\n",
	       eo->devObject.name, eo->devObject.unit);
	if (data == NULL) {
	    error = EINVAL;
	} else {
	...
	debugk(DK_END, "socend_ioctl: %s%d: EIOCGNAME\n",
	       eo->devObject.name, eo->devObject.unit);
	if (NULL == data) {
	    error = EINVAL;
	} else {
	...
	break;
#if VX_VERSION == 62 || VX_VERSION == 64 || VX_VERSION == 66
        case EIOCGRCVJOBQ:
    debugk(DK_END, "socend_ioctl: %s%d: EIOCGRCVJOBQ\n",
       eo->devObject.name, eo->devObject.unit);
    if (NULL == data) {
	    error = EINVAL;
    } else {
        END_RCVJOBQ_INFO *qinfo = (END_RCVJOBQ_INFO *)data;
        debugk(DK_END, "socend_ioctl: %s%d: numRcvJobQs input#: %d\n",
    ...
    default:
	debugk(DK_END, "socend_ioctl: %s%d: Unknown IOCTL\n",
	       eo->devObject.name, eo->devObject.unit);
	error = EINVAL;
	break;
    }
    debugk(DK_END, "socend_ioctl: %s%d: cmd=0x%x Return(%d)\n",
 ...
 */
{
    debugk(DK_END, "Socend send done, in. pkt %p. ck %p.  ", 
 ...
 */
{
...
#if defined(BROADCOM_DEBUG)
    if (debugk_check(DK_END+DK_PACKET)) {
    ...
    rv = _socend_l2_lookup(se->se_unit,
                      ...
                      pkt);
    if(rv == ERROR){
        printk("socend_send : can't get egress port(s) by _socend_l2_lookup.\n");
        return(ERROR);
        }
 ...
 */
{
    ...
    debugk(DK_END, "socend_start: Device(%s), unit %d\n",
           eo->devObject.name, se->se_unit);
    if (!ENET_CMP_MACADDR(se->se_mac, mac_zero)) {
	printk("socend_start: No valid mac address assigned: "
	       "Network Device %s%d\n",
	       eo->devObject.name, eo->devObject.unit);
    }
    ...
    debugk(DK_END, "socend_start: Starting bcm_rx\n");
    if (!bcm_rx_active(se->se_unit)) {
        ...
        printk("RX Thread is already running\n");
    }

    debugk(DK_END, "socend_start: adding l2 addr\n");
    bcm_l2_addr_t_init(&l2addr, se->se_mac, se->se_vlan);
    ...
    if (rv < 0) {
	printk("socend_start: %s%d bcm_l2_addr_add failed: %s\n",
	       eo->devObject.name, eo->devObject.unit, bcm_errmsg(rv));
	return(ERROR);
    }

    debugk(DK_END, "socend_start: registering with bcm_rx\n");
    ...
    if(i == SOC_END_HANDLER_PRIO_COUNT){
        printk("socend_start: socend count is larger %d\n", SOC_END_HANDLER_PRIO_COUNT);
	return(ERROR);
    }
    ...
    if (rv < 0) {
        printk("Warning:  Could not register socend_receive from RX\n");
	return(ERROR);
    }

    END_FLAGS_SET(eo, IFF_UP | IFF_RUNNING | IFF_BROADCAST);
    debugk(DK_END, "socend_start: Started Network Interface: %s%d\n",
	   eo->devObject.name, eo->devObject.unit);
    return(OK);
}
 ...
 */
{
    ...
    for (i = 0; i < SOC_END_HANDLER_PRIO_COUNT; i++) {
        if ((uint32)socend_prio_map[i].socend == (uint32)se){
            ...
            if (rv < 0) {
                printk("Warning:  Could not unregister socend_receive from RX\n");
                return(ERROR);
            }  
            break;
        }
    ...
    if(i == SOC_END_HANDLER_PRIO_COUNT){
        printk("socend_stop: can't find the socend from socend_prio_map table\n");
        return(ERROR);
    }
    (void)bcm_l2_addr_delete(se->se_unit, se->se_mac, se->se_vlan);
    ...
    debugk(DK_END, "socend_stop: Device(%s)\n", eo->devObject.name);
    return(OK);
}
 ...
 */
{
    ...
    printk("socend_poll_send 0x%x 0x%x\n", (int)se, (int)eo);
    return(ENOSYS);
}
 ...
 */
{
    ...
    printk("socend_poll_receive 0x%x 0x%x\n", (int)se, (int)eo);
    return(ERROR);
}
 ...
 */
{
    ...
    if (debugk_check(DK_END)) {
	format_macaddr(mac_str, (uint8 *)a);
	debugk(DK_END, "socend_mcast_add: %s%d %s\n",
	...
	if (ret >= BCM_E_NONE) {
...
#endif
	    if (ret < 0) {
		printk("socend_mcast_add: %s%d bcm_amux_register failed: %s\n",
		       eo->devObject.name, eo->devObject.unit,
		       bcm_errmsg(ret));
	    ...
	    printk ("bcm_mcast_addr_add failed ERROR : %s\n",
		    bcm_errmsg(ret));
	    debugk(DK_END, "socend_mcast_add: Unit %d Device %s%d "
		   "Software RESET\n",
 ...
 */
{
	...
	if (ret == BCM_E_NONE) {
	    ...
	    printk ("bcm_mcast_addr_remove failed ERROR : %s\n",
		    bcm_errmsg(ret));
	    debugk(DK_END,
		   "socend_mcast_del: Unit %d Device %s%d Software RESET\n",
		   se->se_unit, eo->devObject.name, eo->devObject.unit);
	}
    }
    return(ENOSYS);
 ...
 */
{
    ...
    debugk(DK_END, "socend_mcast_get: %s%d\n", eo->devObject.name,
	   eo->devObject.unit);
 ...
 */
{
    enet_hdr_t	*th;		/* Tagged/Untagged header pointers */
    const uint16 dec_length = sizeof(th->en_tag_ctrl)+sizeof(th->en_tag_len);
    unsigned char machdr[12];

    th = (enet_hdr_t *)mb->mBlkHdr.mData;
    if (ENET_TAGGED(th)) {		/* If tagged - strip */
	/* The address that is been accessed should be 4-bytes aligned */
	sal_memcpy(&th->en_tag_ctrl, &th->en_tag_len, /* ORDER --- SECOND */
		   mb->mBlkHdr.mLen - 16);
	sal_memcpy(machdr, mb->mBlkHdr.mData, 12);
	sal_memcpy(mb->mBlkHdr.mData+2, machdr, 12); /* ORDER --- SECOND */
	mb->mBlkHdr.mData += 2; /* Update SENS pointer */
	mb->mBlkPktHdr.len =
	mb->mBlkHdr.mLen  -= dec_length; /* Update SENS length */
    }
}
    ...
    if (pkt_rv == BCM_RX_NOT_HANDLED && enqueue) {
        if (SOC_IS_ESW(unit)) {
            ...
            rv = netJobAdd((FUNCPTR)socend_receive_netjob,
                       ...
                       4,
                       5);
            debugk(DK_END, "netJobAdd rv=0x%x OK=%x\n", rv, OK);
