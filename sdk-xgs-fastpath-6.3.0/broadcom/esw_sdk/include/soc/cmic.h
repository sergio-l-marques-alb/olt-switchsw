...
#define IRQ_BLOCK(bit)                 (1<<(bit) ) /* Bit 0-31  */
 ...
 * addr1: For Robo switch register(default): Register Page.
 *        For 'Reg bus type=SOC_EXTERNAL_PHY_BUS_CPUMDIO': PHY id.
 * addr2: For Robo switch register(default): Register Offset;.
...
extern int soc_robo_macsec_miim_write(int unit, uint32 phy_id,
                            uint32 phy_reg_addr, uint16 phy_wr_data);
extern int soc_robo_macsec_miim_read(int unit, uint32 phy_id,
                            uint32 phy_reg_addr, uint16 *phy_rd_data);
