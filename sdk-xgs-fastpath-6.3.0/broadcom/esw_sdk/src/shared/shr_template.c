/*
 * $Id: shr_template.c 1.18 Broadcom SDK $
 * 
...
#define TEMPLATE_MESSAGE_ENABLE TRUE
#define TEMPLATE_EXCESS_VERBOSITY TRUE
#if 0 
#undef BCM_DEBUG
#define BCM_DEBUG(flags, stuff)   soc_cm_print stuff
#endif 
#if TEMPLATE_MESSAGE_ENABLE
#define TEMPLATE_DEBUG(flags, stuff)  BCM_DEBUG(flags | BCM_DBG_API, stuff)
#else /* TEMPLATE_MESSAGE_ENABLE */
#define TEMPLATE_DEBUG(flags, stuff)
#endif /* TEMPLATE_MESSAGE_ENABLE */
#define TEMPLATE_OUT(stuff)           TEMPLATE_DEBUG(BCM_DBG_API, stuff)
#define TEMPLATE_WARN(stuff)          TEMPLATE_DEBUG(BCM_DBG_WARN, stuff)
#define TEMPLATE_ERR(stuff)           TEMPLATE_DEBUG(BCM_DBG_ERR, stuff)
#define TEMPLATE_VERB(stuff)          TEMPLATE_DEBUG(BCM_DBG_VERBOSE, stuff)
#define TEMPLATE_VVERB(stuff)         TEMPLATE_DEBUG(BCM_DBG_VVERBOSE, stuff)
#if TEMPLATE_EXCESS_VERBOSITY
#define TEMPLATE_EVERB(stuff)         TEMPLATE_DEBUG(BCM_DBG_VVERBOSE, stuff)
#else /* TEMPLATE_EXCESS_VERBOSITY */
#define TEMPLATE_EVERB(stuff)
#endif /* TEMPLATE_EXCESS_VERBOSITY */
/* Please do not change the following seven lines */
#if defined(__GNUC__) && !defined(__PEDANTIC__)
#define TEMPLATE_MSG(string) "%s[%d]%s" string, __FILE__, __LINE__, __FUNCTION__
#define TEMPLATE_MSG1(string) "%s[%d]%s: " string, __FILE__, __LINE__, __FUNCTION__
#else /* defined(__GNUC__) && !defined(__PEDANTIC__) */
#define TEMPLATE_MSG(string) "%s[%d]" string, __FILE__, __LINE__
#define TEMPLATE_MSG1(string) "%s[%d]: " string, __FILE__, __LINE__
 ...
 */
static volatile sal_mutex_t _g_templateLock = NULL;
static _shr_template_unit_desc_t *_g_unitTemplateDesc[BCM_LOCAL_UNITS_MAX];
 ...
 */
#define TEMPLATE_LOCK_TAKE \
    if (!_g_templateLock) { \
        TEMPLATE_ERR((TEMPLATE_MSG1("no unit has been initialised yet\n"))); \
        return BCM_E_INIT; \
    } \
    if (sal_mutex_take(_g_templateLock, sal_sem_FOREVER)) { \
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to takee global lock\n"))); \
        return BCM_E_INTERNAL; \
    }
#define TEMPLATE_LOCK_GIVE \
    if (sal_mutex_give(_g_templateLock)) { \
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to release global lock\n"))); \
        return BCM_E_INTERNAL; \
    }
...
#define TEMPLATE_POOL_PARAM_TEMPLATE_ID_CHECK(_thisPool,_template) \
      if (!(_template >= thisPool->template_low_id && _template <= (thisPool->template_low_id + thisPool->template_count-1))) { \
        TEMPLATE_ERR((TEMPLATE_MSG1("_template id %d is not in correct range. Should be between thisPool->template_low_id %d template_count %d\n") \
...
static int 
shr_mtemplate_dump(shr_mtemplate_handle_t handle);
static int 
                  ...
                  int num_template_pools)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%d, %d, %d) enter\n"),
               unit,
               num_template_types,
               num_template_pools));
    ...
    if (!_g_templateLock) {
        TEMPLATE_VVERB((TEMPLATE_MSG1("create global resource lock\n")));
        tempLock = sal_mutex_create("global resource mutex");
        if (!tempLock) {
            TEMPLATE_ERR((TEMPLATE_MSG1("unable to create global resource lock\n")));
            return BCM_E_RESOURCE;
        }
        /* claim the newly created lock */
        if (sal_mutex_take(tempLock, sal_sem_FOREVER)) {
            TEMPLATE_ERR((TEMPLATE_MSG1("unable to claim global resource lock\n")));
            if (tempLock) {
        ...
        _g_templateLock = tempLock;
        TEMPLATE_VVERB((TEMPLATE_MSG1("init unit resource pointers\n")));
        /* clear the unit pointers */
        sal_memset(&(_g_unitTemplateDesc[0]),
                   0x00,
                   sizeof(_g_unitTemplateDesc[0]) * BCM_LOCAL_UNITS_MAX);
        /* deal with contention */
        if (_g_templateLock != tempLock) {
             ...
             *  big of a problem.
             */
            TEMPLATE_WARN((TEMPLATE_MSG1("contention detected; free disused lock\n")));
            if (sal_mutex_give(tempLock)) {
                TEMPLATE_ERR((TEMPLATE_MSG1("unable to release temporary lock\n")));
    ...
    TEMPLATE_LOCK_GIVE;
    TEMPLATE_VVERB((TEMPLATE_MSG("(%d, %d, %d) return %d (%s)\n"),
               unit,
               num_template_types,
               num_template_pools,
               result,
...
shr_template_detach(int unit)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%d) enter\n"), unit));
    ...
    /* release the lock and return the result */
    TEMPLATE_LOCK_GIVE;
    TEMPLATE_VVERB((TEMPLATE_MSG("(%d) return %d (%s)\n"),
               unit,
               result,
               _SHR_ERRMSG(result)));
    return result;
...
shr_template_dump(int unit)
{
    ...
    /* perform the action */
    return shr_mtemplate_dump(thisUnit);
}
               ...
               int *template)               
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %08X) enter\n"),
               (void*)handle,
               template_type,
               flags));
    ...
    if (flags & (~SHR_TEMPLATE_MANAGE_SINGLE_FLAGS)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("invalid flags %08X\n"),
    ...
    result = _shr_template_managements_mgrs[thisPool->templateManagerType].alloc(thisPool,
     ...
     TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %08X, %p, %d, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               flags,
               data,
               ...
               int *template)               
{
    ...
    if ((flags & (SHR_TEMPLATE_MANAGE_SET_WITH_ID)) == 0) {
        TEMPLATE_ERR((TEMPLATE_MSG1("WITH_ID must be specify %08X\n"),
                 flags));
        return BCM_E_PARAM;
    }
    ...
    result = _shr_template_managements_mgrs[thisPool->templateManagerType].alloc_group(thisPool,
     ...
     TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %08X, %p, %d, %d, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               flags,
               data,
               ...
               int *is_last)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %d) enter\n"),
               (void*)handle,
               template_type,
               template));
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %d, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               template,
               *is_last,
                      ...
                      int *is_allocated)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %08X, %d) enter\n"),
               (void*)handle,
               template_type,
               flags,
               old_template));
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %08X, %d, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               flags,
               old_template,
                      ...
                      void *data)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               template,
               result,
                ...
                int num_template_pools)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %d) enter\n"),
              (void*)handle,
              num_template_types,
              num_template_pools));
    ...
    if (!handle) {
        TEMPLATE_ERR((TEMPLATE_MSG1("obligatory OUT argument must not be NULL\n")));
        result = BCM_E_PARAM;
    }
    if (1 > num_template_pools) {
        TEMPLATE_ERR((TEMPLATE_MSG1("resource pools %d; must be > 0\n"), num_template_pools));
        result =  BCM_E_PARAM;
    }
    if (1 > num_template_types) {
        TEMPLATE_ERR((TEMPLATE_MSG1("resource types %d; must be > 0\n"), num_template_types));
        result =  BCM_E_PARAM;
    }
    if (BCM_E_NONE != result) {
    ...
    if (!tempHandle) {
        /* alloc failed */
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to allocate %u bytes for info\n"),
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(&(%p), %d, %d) return %d (%s)\n"),
              (void*)(*handle),
              num_template_types,
              num_template_pools,
              result,
...
shr_mtemplate_get(shr_mtemplate_handle_t handle,
                 ...
                 int *num_template_pools)
{
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %p, %p) enter\n"),
               (void*)handle,
               num_template_types,
               num_template_pools));
                 ...
                 const char *name)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %s, %d, %d, %d, %d, %p, \"%s\") enter\n"),
               (void*)handle,
               pool_id,
    ...
    if ((0 > manager) || (SHR_TEMPLATE_MANAGE_COUNT <= manager)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("template manager type %d not supported\n"),
                 manager));
        return BCM_E_PARAM;
    }
    if (0 >= data_size) {
        TEMPLATE_ERR((TEMPLATE_MSG1("data size must be positive\n")));
        return BCM_E_PARAM;
    }
    if (0 >= max_entities) {
        TEMPLATE_ERR((TEMPLATE_MSG1("maximum entities must be positive\n")));
        return BCM_E_PARAM;
    }
    if (0 > template_low_id) {
        TEMPLATE_ERR((TEMPLATE_MSG1("template low id cant be negative \n")));
        return BCM_E_PARAM;
    }
    if (0 >= template_count) {
        TEMPLATE_ERR((TEMPLATE_MSG1("template count must be positive \n")));
        return BCM_E_PARAM;
    }

    TEMPLATE_PARAM_NULL_CHECK(handle,extras);
    if ((handle->pool[pool_id]) && (handle->pool[pool_id]->refCount)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("%p pool %d (%s) can not be changed because it"
    ...
    if (BCM_E_NONE == result) {
        ...
        if (oldPool) {
            ...
            if (BCM_E_NONE != result) {
                ...
                TEMPLATE_ERR((TEMPLATE_MSG1("unable to destroy %p old pool %d (%s):"
                                  " %d (%s)\n"),
                         (void*)handle,
                         pool_id,
                         oldPool->name,
                ...
                if (BCM_E_NONE != xresult) {
                    TEMPLATE_ERR((TEMPLATE_MSG1("unable to destroy new pool for %p pool"
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %s, %d, %d, %d, %d, %p, \"%s\") return %d (%s)\n"),
               (void*)handle,
               pool_id,
                      ...
                      const char **name)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %p, %p, %p, %p, %p, %p, %p) enter\n"),
               (void*)handle,
               pool_id,
               manager,
               template_low_id,
                      ...
                      const char *name)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %d, \"%s\") enter\n"),
               (void*)handle,
               template_type,
               pool_id,
               name?name:noname));
    ...
    if ((handle->template[template_type]) && (handle->template[template_type]->refCount)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("%p template type %d (%s) can not be changed"
    ...
    if (tempType) {
        ...
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to allocate %u bytes for %p resource"
                          " type %d\n"),
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %d, \"%s\") return %d (%s)\n"),
               (void*)handle,
               template_type,
               pool_id,
               name?name:noname,
                  ...
                  const char **name)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG("(%p, %d, %p, %p) enter\n"),
               (void*)handle,
               template_type,
               pool_id,
               name));
...
_shr_mtemplate_destroy_data(_shr_template_unit_desc_t *unitData)
{
    ...
    for (i = 0; i < unitData->templateTypeCount; i++) {
        if (unitData->template[i]) {
            ...
            if (type->refCount) {
                TEMPLATE_WARN((TEMPLATE_MSG1("%p type %d (%s): still in use (%d)\n"),
                          unitData,
                          i,
                          &(type->name[0]),
                          type->refCount));
    ...
    for (i = 0;
        ...
        if (unitData->pool[i]) {
            ...
            if (pool->refCount) {
                TEMPLATE_WARN((TEMPLATE_MSG1("%p pool %d (%s): unexpectedly still"
            ...
            if (BCM_E_NONE != result) {
                TEMPLATE_ERR((TEMPLATE_MSG1("%p pool %d (%s): unable to destroy:"
                                  " %d (%s)\n"),
                         unitData,
                         i,
                         &(pool->name[0]),
...
shr_mtemplate_dump(shr_mtemplate_handle_t handle)
{
    ...
    soc_cm_print("  template type count = %d\n", handle->templateTypeCount);
    soc_cm_print("  template pool count = %d\n", handle->templatePoolCount);
    for (index = 0; index < handle->templatePoolCount; index++) {
        thisPool = handle->pool[index];
        if (thisPool) {
            soc_cm_print("  template pool %d (%s):\n",
                         index,
                         &(thisPool->name[0]));
            soc_cm_print("    method = %d (%s)\n",
                         thisPool->templateManagerType,
                         _shr_template_managements_mgrs[thisPool->templateManagerType].name);
            soc_cm_print("    range  = %d..%d (%d templates allowed)\n",
                         thisPool->template_low_id,
                         thisPool->template_low_id + thisPool->template_count - 1,
                         thisPool->template_count);
            soc_cm_print("    refcnt = %d\n", thisPool->refCount);
        } else {
            soc_cm_print("  template pool %d is not in use\n", index);
        }
    }
    for (index = 0; index < handle->templateTypeCount; index++) {
        ...
        if (thisTemplate) {
            ...
            soc_cm_print("    resource pool   = %d (%s)\n",
                         thisTemplate->templatePoolId,
                         &(thisPool->name[0]));
            soc_cm_print("    active elements = %d\n",
                         thisTemplate->refCount);
        } else {
            soc_cm_print("  template type %d is not in use\n", index);
        }
    }
    return result;
}
                    ...
                    int template_type)
{
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d) enter\n"),
               (void*)handle,
               template_type));
    ...
    TEMPLATE_VVERB((TEMPLATE_MSG1("(%p, %d) return %d (%s)\n"),
               (void*)handle,
               template_type,
               result,
               _SHR_ERRMSG(result)));
                                     ...
                                     const char *name)
{
    ...
    *desc = sal_alloc(sizeof(**desc),"Hash template main descriptor");
    if (!(*desc)) {
        /* alloc failed */
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to allocate %u bytes for main descriptor\n"),
    ...
    if (!((*desc)->extras)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to allocate %u bytes for data\n"),
                 (unsigned int)(sizeof(*info))));
        sal_free(*desc);
        *desc = NULL;
        return BCM_E_MEMORY;
    }
    (*desc)->name = sal_alloc(sal_strlen(name) + 1,"Hash template name");
    if (!((*desc)->name)) {
        TEMPLATE_ERR((TEMPLATE_MSG1("unable to allocate %u bytes for data\n"),
                 (unsigned int)(sal_strlen(name))));
    ...
    sal_memset((*desc)->name, 0x00, sal_strlen(name));
