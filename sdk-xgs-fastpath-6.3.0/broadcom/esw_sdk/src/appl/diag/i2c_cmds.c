/*
 * $Id: i2c_cmds.c 1.119 Broadcom SDK $
 *
...
#ifdef SHADOW_SVK
    if ((dev_name == NULL) || (strncmp(dev_name, "mux6", 4) != 0) || 
        (strncmp(dev_name, "mux7", 4) != 0)) {
...
_i2c_mux_default_dev_name_get(void)
{
    ...
    if (soc_get_board_id() == G27) {
        dev_name = I2C_MUX_6;
               ...
               (soc_get_board_id() == G33) ||
               (soc_get_board_id() == G34) || (soc_get_board_id() == G35) ||
               (soc_get_board_id() == G38) || (soc_get_board_id() == G39)){
        dev_name = I2C_MUX_3;
    } else {
        dev_name = I2C_LPT_0;
    }
...
cmd_i2c(int unit, args_t *a)
{
    ...
    soc_i2c_show_log(unit, TRUE);
        ...
        for(i=0; i<len; i++) {
            rv = soc_i2c_read_byte_data(unit, saddr, comm, &buffer[i]);
            if (SOC_FAILURE(rv)) {
                break;
            }
            if (i%16 == 0) printk("\n%02x:", i);
    ...
    while ((op = ARG_GET(a)) != NULL) {
        ...
        if ( !sal_strncasecmp(function,"probe", strlen(function))) {
             ...
             */
            if ( ((flags != 0) || (speed > 0)) &&
                 ((rv=soc_i2c_attach(unit, flags | SOC_I2C_NO_PROBE, speed)) < 0) ) {
...
#ifdef BCM_CALADAN3_SVK
bb_vparams_t
bb_current[MAX_VOLTAGES_PER_BOARD] = {
...
int bb_caladan3_voltage(int unit, int bx, char *source, char* value)
{
    ...
    if( value ) {
       ...
       if (!sal_strncasecmp(value,"nominal",strlen(value))) {
         ...
         */
       if (!sal_strncasecmp(value,"high",strlen(value))) {
           printk("Shifting %s voltage to Margin High\n", source);
           cmd = I2C_SMM_SET_MARGIN_HIGH;
       } else if (!sal_strncasecmp(value,"low",strlen(value))) {
...
#endif
#ifdef BCM_CALADAN3_SVK
    if(!sal_strncasecmp(function, "current", strlen(function))){
        /* Handle Caladan3 SVK */
...
cmd_pio(int unit, args_t *a)
{
    ...
    if ((dev == -1) && (port == -1)) { 
        ...
        for(i=0; i < max; i++) {
            if ((strstr(fields[i].name, field_name) != NULL) ||
                 (all_fields == 1)) {
                dev = fields[i].dev;
                port = fields[i].port;
                pos = fields[i].pos;
