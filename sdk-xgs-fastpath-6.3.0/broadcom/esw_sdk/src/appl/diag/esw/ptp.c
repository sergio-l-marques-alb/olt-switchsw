/*
 * $Id: ptp.c 1.66.8.1 Broadcom SDK $
...
output_current_debug(void* owner, void* time_as_ptr, void *unit_as_ptr, void *unused_2, void* unused_3)
{
    ...
    for (stack_id = 0; stack_id < PTP_MAX_STACKS_PER_UNIT; ++stack_id) {
        ...
        if (_bcm_esw_ptp_unit_array[unit].memstate == PTP_MEMSTATE_INITIALIZED &&
            _bcm_esw_ptp_unit_array[unit].stack_array[stack_id].memstate == PTP_MEMSTATE_INITIALIZED) {
...
#define PTP_PORT_USAGE \
    ...
    "                    logDelayrequestInterval=<intv> TimestampMechanism=<ToP|RCPU|PHY|Unimac32>\n\t" \
...
cmd_ptp_channels(int unit, args_t *a)
{
    ...
    if (parse_cmp("Set", arg, 0)) {
        ...
        num_channels = 3;
        rv = _bcm_esw_ptp_input_channels_status_get(unit, cur_stack_id, cur_clock_num, &num_channels, channel_status);
...
#define PTP_SIGNAL_USAGE \
    ...
    " PTP SIGnals Set Pin=<pin#> Frequency=<Hz> PhaseLock[=y/n] Width=<ns> Offset=<ns>\n\t"    \
...
cmd_ptp_signals(int unit, args_t *a)
{
    ...
    if (parse_cmp("Get", arg, 0)) {
        ...
        for (i = 0; i < PTP_MAX_OUTPUT_SIGNALS; i++) {
            ...
            {
                printk("Signal[%d]: pin=%d, frequency=%d, phaselock=%c, width=%d, offset=%d\n",
                            i,sig_arr[i].pin,sig_arr[i].frequency,
                            sig_arr[i].phase_lock ? 'y' : 'n',
                            sig_arr[i].pulse_width_ns, sig_arr[i].pulse_offset_ns);
            }
        }
        if (rv) {
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
...
#else
        slave.tx_timestamp_mech = bcmPTPMac32CorrectionTimestamps;
#endif
...
{
    static bcm_ptp_peer_dataset_t dataset;
    unsigned index;
    int port_num;
    parse_table_t pt;
    ...
    PTP_IF_ERROR_RETURN(_bcm_ptp_peer_dataset_get(0, PTP_STACK_ID_DEFAULT, 
                            PTP_CLOCK_NUMBER_DEFAULT, port_num, &dataset));

    printk("Number of Peers: %d\n", dataset.num_peers);
    for (index = 0; index < dataset.num_peers; index++) {
        printk("Peer Number: %d\n", index);
        printk("\t Port Number : %d\n", dataset.peer[index].localPortNum);
        printk("\t Clock ID    : %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
               dataset.peer[index].clockID[0],
               dataset.peer[index].clockID[1],
               dataset.peer[index].clockID[2],
               dataset.peer[index].clockID[3],
               dataset.peer[index].clockID[4],
               dataset.peer[index].clockID[5],
               dataset.peer[index].clockID[6],
               dataset.peer[index].clockID[7]);

        switch (dataset.peer[index].port_addr.addr_type) {
        case bcmPTPUDPIPv4:
            printk("\t IPv4        : %d.%d.%d.%d\n",
                   dataset.peer[index].port_addr.address[0], dataset.peer[index].port_addr.address[1],
                   dataset.peer[index].port_addr.address[2], dataset.peer[index].port_addr.address[3]);
            break;
        case bcmPTPUDPIPv6:
            {
                int i;

                printk("\t IPv6        : %02x%02x", dataset.peer[index].port_addr.address[0], dataset.peer[index].port_addr.address[1]);
                for (i = 2; i < 16; i += 2) {
                    printk(":%02x%02x", dataset.peer[index].port_addr.address[i], dataset.peer[index].port_addr.address[i + 1]);
                }
                printk("\n");
            }
            break;
        case bcmPTPIEEE8023:
            printk("\t L2: %02x:%02x:%02x:%02x:%02x:%02x\n",
                   dataset.peer[index].port_addr.address[0], dataset.peer[index].port_addr.address[1],
                   dataset.peer[index].port_addr.address[2], dataset.peer[index].port_addr.address[3],
                   dataset.peer[index].port_addr.address[4], dataset.peer[index].port_addr.address[5]);
            break;
        default:
            printk("\t ERROR.  Unexpected address type (%d)\n", dataset.peer[index].port_addr.addr_type);
            break;
        }
        
        printk("\t RxAnn    : %llu\n", (long long unsigned)dataset.peer[index].rx_announces);
        printk("\t RxSync   : %llu\n", (long long unsigned)dataset.peer[index].rx_syncs);
        printk("\t RxFlwup  : %llu\n", (long long unsigned)dataset.peer[index].rx_followups);
        printk("\t RxRej    : %llu\n", (long long unsigned)dataset.peer[index].rejected);

        printk("\t RxDlyRq  : %llu\n", (long long unsigned)dataset.peer[index].rx_delayreqs);
        printk("\t RxDlyRsp : %llu\n", (long long unsigned)dataset.peer[index].rx_delayresps);
        printk("\t RxMgmt   : %llu\n", (long long unsigned)dataset.peer[index].rx_mgmts);
        printk("\t RxSig    : %llu\n", (long long unsigned)dataset.peer[index].rx_signals);

        printk("\t TxAnn    : %llu\n", (long long unsigned)dataset.peer[index].tx_announces);
        printk("\t TxSync   : %llu\n", (long long unsigned)dataset.peer[index].tx_syncs);
        printk("\t TxFlwup  : %llu\n", (long long unsigned)dataset.peer[index].tx_followups);

        printk("\t TxDlyRq  : %llu\n", (long long unsigned)dataset.peer[index].tx_delayreqs);
        printk("\t TxDlyRsp : %llu\n", (long long unsigned)dataset.peer[index].tx_delayresps);
        printk("\t TxMgmt   : %llu\n", (long long unsigned)dataset.peer[index].tx_mgmts);
        printk("\t TxSig    : %llu\n", (long long unsigned)dataset.peer[index].tx_signals);
    }
...
cmd_ptp_packet_counters_get(int unit, args_t *a)
{
    ...
    clock = &_bcm_esw_ptp_unit_array[unit].stack_array[arg_stack_id].clock_array[arg_clock_num];
...
#define PTP_TELECOM_USAGE \
    ...
    "\t                           [TimeStamp=<|ToP|RCPU|PHY|Unimac32|>]\n"           \
        ...
        if (opcode == ARP_REQUEST) {
            if (arp_protocol == PROTOCOL_IPV4) {
                ...
                for (clock_num = 0; clock_num < PTP_MAX_STACKS_PER_UNIT; ++clock_num) {
                    _bcm_ptp_clock_cache_t *clock =
                        &_bcm_esw_ptp_unit_array[unit].stack_array[stack_id].clock_array[clock_num];
...
#else
    amsg->slave->tx_timestamp_mech = bcmPTPMac32CorrectionTimestamps;
#endif
