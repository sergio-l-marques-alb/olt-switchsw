/*
 * $Id: port.c 1.267.2.2 Broadcom SDK $
...
_phy_diag_mfg(int unit, bcm_pbmp_t pbmp, args_t *args) 
{
    ...
    if (data_len) {
        ...
        switch (test) {
        case 1:
            fprintf(ofp, "PHY_DIAG_CTRL_MFG_HYB_CANC\n");
            break;
        case 2:
            fprintf(ofp, "PHY_DIAG_CTRL_MFG_DENC\n");
            break;
        }
    }
        ...
        DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
            ...
            if (buf_ptr[0]) {
                ...
                while ( i < data_len) {
                    ...
                    fprintf(ofp, "0x%08x", soc_ntohl_load(dptr) );
                    dptr += 4;
                    i += 4;
                    if (i >= data_len) {
                        fprintf(ofp, "\n");
...
_phy_diag_state(int unit, bcm_pbmp_t pbmp, args_t *args) 
{
    ...
    if (data_len) {
        ...
        switch (test) {
        case 1:
            fprintf(ofp, "PHY_DIAG_CTRL_STATE_TRACE1\n");
            break;
        case 2:
            fprintf(ofp, "PHY_DIAG_CTRL_STATE_TRACE2\n");
            break;
        case 3:
            fprintf(ofp, "PHY_DIAG_CTRL_STATE_WHERE_AM_I\n");
            break;
        case 4:
            fprintf(ofp, "PHY_DIAG_CTRL_STATE_TEMP\n");
            break;
        }
    }
            ...
            if (buf_ptr[0]) {
                ...
                if (test_cmd == PHY_DIAG_CTRL_STATE_WHEREAMI) {
                    ...
                    while ( i < data_len) {
                        ...
                        fprintf(ofp, "0x%08x", soc_letohl_load(&dptr[0]));
                        dptr += 4;
                        i += 4;
                        if (i >= data_len) {
                            fprintf(ofp, "\n");
...
_phy_diag_eyescan(int unit, bcm_pbmp_t pbmp,args_t *args) 
{
    ...
    parse_table_add(&pt, "counter", PQ_DFL|PQ_INT, (void *)(0), 
                    &(params.counter), NULL);
     ...
     if (c != NULL && sal_strcasecmp(c, "oam") == 0) {
        ...
        if ((c = ARG_CUR(a)) != NULL) {
            ...
            DPORT_SOC_PBMP_ITER(u, pbm, dport, p) {
                ...
                if (flags & (1U << 5)) {
                     if(!strcmp(ts_format, "ntp")) {
                        timestamp = 1;
                    } else if(!strcmp(ts_format, "ptp")) {
                        timestamp = 0;
                    } else {
                        return CMD_USAGE;
                    }
