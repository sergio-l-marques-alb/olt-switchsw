/*
 * $Id: memrand.c 1.61 Broadcom SDK $
...
mem_rand_init(int unit, args_t *a, void **p)
{
    ...
    if (SOC_IS_ARAD(unit) && (soc_mem_is_writeonly(unit, rw->mem) || soc_mem_is_signal(unit, rw->mem))) {
...
#if defined (BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined (BCM_DFE_SUPPORT)|| defined (BCM_PETRA_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
    if (soc_mem_cpu_write_control(unit, rw->mem, rw->copyno, TRUE) < 0) {
...
#ifdef BCM_POLAR_SUPPORT
                        if (SOC_IS_POLAR(unit)) {
                            printk("Compare ERROR: table %s[%d] iteration %d\n",
                                   SOC_ROBO_MEM_UFNAME(unit, rw->mem),
                                   addr[i], iter+1);
                        } else
#endif /* BCM_POLAR_SUPPORT */
...
#if defined (BCM_ESW_SUPPORT) || defined(BCM_SIRIUS_SUPPORT) || defined (BCM_DFE_SUPPORT)|| defined (BCM_PETRA_SUPPORT) || defined(BCM_CALADAN3_SUPPORT)
    if (soc_mem_cpu_write_control(unit, rw->mem, rw->copyno, FALSE) < 0) {
