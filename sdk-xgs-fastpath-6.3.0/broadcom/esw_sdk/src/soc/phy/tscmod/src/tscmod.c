/*----------------------------------------------------------------------
 * $Id: tscmod.c 1.86.2.3 Broadcom SDK $
...
STATIC int phy_tscmod_an_set(int unit, soc_port_t port, int an);
STATIC int phy_tscmod_lb_set(int unit, soc_port_t port, int enable);
STATIC int phy_tscmod_ability_advert_set(int unit, soc_port_t port,
                                        soc_port_ability_t *ability);
STATIC int phy_tscmod_ability_local_get(int unit, soc_port_t port,
                                       soc_port_ability_t *ability);
STATIC int phy_tscmod_speed_set(int unit, soc_port_t port, int speed);
STATIC int phy_tscmod_speed_get(int unit, soc_port_t port, int *speed);
...
STATIC int phy_tscmod_control_set(int unit, soc_port_t port,
                       soc_phy_control_t type, uint32 value) ;

STATIC int phy_tscmod_control_get(int unit, soc_port_t port,
                                  soc_phy_control_t type, uint32 *value) ;
                         ...
                         uint16 *uc_ver, uint16 *uc_crc)
{
   ...
   if ((_phy_tscmod_firmware_set_helper[unit] != NULL) && (DEV_CFG_PTR(pc)->load_mthd == 2)) {
      ...
      if((tsc->verbosity & TSCMOD_DBG_SCAN)==0) tsc->verbosity = 0 ;
...
int _phy_tscmod_tx_reset(int unit, phy_ctrl_t  *pc, int enable)
{
    ...
    if(!enable) {
       ...
       /* no need to reset CREDIT_CONTROL */
...
tscmod_an_cl72_control_set(int unit, phy_ctrl_t *pc)
{
   ...
   if((tsc->an_type==TSCMOD_CL37)||(tsc->an_type==TSCMOD_CL37_BAM)) {
      if(pCfg->an_cl72) {
...
_phy_tscmod_force_cl72_sw_link_recovery(int unit, soc_port_t port,int link)
{
   ...
   if(FORCE_CL72_STATE(pc)== TSCMOD_FORCE_CL72_IDLE) {
      ...
      if(FORCE_CL72_TICK_CNT(pc)>=2) {
         ...
         if(tsc->verbosity & TSCMOD_DBG_FSM)
            printf("%-22s: u=%0d p=%0d link=%0d state=%0x restart\n", __func__, tsc->unit,
             ...
             (!(tsc->ctrl_type&TSCMOD_CTRL_TYPE_CL72_DP0_DIS)) ) {
      ...
      for(lane=lane_s; lane<lane_e; lane++) {
         ...
         tsc->per_lane_control = TSCMOD_DIAG_CL72_LOCK ;
      ...
      if(cl72_lock) {
         ...
         if(tsc->verbosity & TSCMOD_DBG_FSM)
            printf("%-22s: u=%0d p=%0d link=%0d state=%0x wait_training_done\n", __func__, tsc->unit,
                ...
                (!(tsc->ctrl_type&TSCMOD_CTRL_TYPE_CL72_DP1_DIS)) ) {
      cl72_done = 1 ;
      for(lane=lane_s; lane<lane_e; lane++) {
         tsc->lane_select = _tscmod_get_lane_select(unit, tsc->port, lane) ;
         tsc->this_lane   = lane ;
   ...
   *txdrv   = TXDRV_6GOS2_INX ;
   ...
   case 20000:
      if(tsc->port_type   == TSCMOD_SINGLE_PORT ) {
         ...
         if(DEV_CFG_PTR(pc)->hg_mode) {
            if( DEV_CFG_PTR(pc)->line_intf & TSCMOD_IF_XGMII ) {
...
_phy_tscmod_port_init_pll_lock_wait(int unit, soc_port_t port)
{
    ...
    */
    ...
    SOC_IF_ERROR_RETURN
        (phy_tscmod_ability_local_get(unit, port, &ability));

    SOC_IF_ERROR_RETURN
        (phy_tscmod_ability_advert_set(unit, port, &ability));
    ...
    if((tsc->an_type != TSCMOD_AN_TYPE_ILLEGAL)&&
       ...
       SOC_IF_ERROR_RETURN
           (phy_tscmod_an_set(unit, port, TRUE)) ;
    ...
    if(tsc->verbosity&TSCMOD_DBG_SPD)
       printf("%-22s: u=%0d p=%0d actual_spd_vec=%0x\n", __func__, tsc->unit, tsc->port,actual_spd_vec) ;
...
STATIC int
phy_tscmod_sw_rx_los_link_get(int unit, soc_port_t port, int *link)
{
...
phy_tscmod_link_get(int unit, soc_port_t port, int *link)
{
   ...
   *link = tsc->accData;
   ...
   if (tsc->verbosity & TSCMOD_DBG_LINK)
      printf("%-22s: u=%0d p=%0d link=%0d\n", __func__, unit, port, *link) ;
   ...
   if(*link==0) {
      ...
      if(tsc->ctrl_type & (TSCMOD_CTRL_TYPE_LB|TSCMOD_CTRL_TYPE_PRBS_ON|TSCMOD_CTRL_TYPE_RMT_LB)) {
         ...
         if(FORCE_CL72_ENABLED(pc)) {
            ...
            rv = phy_tscmod_sw_rx_los_link_get(unit, port, link) ;
         } else {
...
phy_tscmod_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    if((tsc->verbosity &(TSCMOD_DBG_LINK|TSCMOD_DBG_PATH)))
       printf("%-22s: u=%0d p=%0d enable=%0d sel=%0x ln=%0d dxgxs=%0d cl72_en=%0d\n", __func__,
              tsc->unit, tsc->port, enable, tsc->lane_select, tsc->this_lane, tsc->dxgxs, FORCE_CL72_ENABLED(pc)) ;
...
_phy_tscmod_speed_set(int unit, soc_port_t port, int speed)
{
   ...
   TSCMOD_DEV_CFG_t  *pCfg;
   int         actual_spd_vec ;
   int         lane, lane_s, lane_e,  tmp_sel, tmp_lane = 0 ;
   int         rv, pll_mode_change ;
...
phy_tscmod_an_set(int unit, soc_port_t port, int an)
{
   ...
   tscmod_st *tsc, *tsc2;
   TSCMOD_TX_DRIVE_t tx_drv[NUM_LANES];
   int rv, an_type, need_set_port_mode, data ;
   ...
   if(tsc->verbosity&(TSCMOD_DBG_AN|TSCMOD_DBG_LINK))
      printf("%-22s: u=%0d p=%0d an=(%0x) cfg_cl73=%0x\n", __func__, unit, port, an, pCfg->cl73an) ;
   ...
   if( an ) {
      if( pCfg->cl73an ) {  /* imply XE port for TSCMOD_CL73 */
         ...
         case TSCMOD_CL73_HPAM:    an_type = TSCMOD_HPAM     ;  break ;
         default:
            break ;
         }
      } else {
      ...
      if(tsc->an_type == an_type ) {
         ...
         if( (an_type == TSCMOD_CL37_BAM)||(an_type == TSCMOD_CL37) ||
             (an_type == TSCMOD_CL37_SGMII)) {
            if( (tsc->an_type == TSCMOD_CL37_SGMII )||
                (tsc->an_type == TSCMOD_CL37)) {
               need_set_port_mode =0 ;
            } else {
      ...
      if(need_set_port_mode) {
         ...
         PBMP_ALL_ITER(unit0, port0) {
            ...
            if ((pc2->phy_mode == pc->phy_mode)) {
               if(tsc->verbosity&TSCMOD_DBG_INIT)
                  printf("%-22s PBMP_ALL_ITER u=%0d p=%0d AN scan orrected tsc->plldiv=%0d\n", 
                         __func__, unit0, port0, tsc->plldiv) ;
               pDesc2 = (TSCMOD_DEV_DESC_t *)(pc2 + 1);
               tsc2   = (tscmod_st *)( pDesc2 + 1);
               tsc2->plldiv = tsc->plldiv ;
      ...
      */
      ...
      if(an_type == TSCMOD_CL37_BAM) {
         ...
         if(pCfg->pdetect1000x) data |= 0x1 ;
         if((pCfg->pdetect10g) && (tsc->port_type==TSCMOD_SINGLE_PORT))   data |= 0x2 ;
      }
      tsc->per_lane_control = data ;
      tscmod_tier1_selector("PARALLEL_DETECT_CONTROL", tsc, &rv);
...
phy_tscmod_lb_set(int unit, soc_port_t port, int enable)
{
   ...
   tscmod_st    *tsc;
   TSCMOD_DEV_DESC_t     *pDesc;
   int value, rv, en_val, tmp_select ;
   uint32 data, i;
   ...
   if(tsc->verbosity & TSCMOD_DBG_PATH)
      printf("%-22s: u=%0d p=%0d lb en=%0d rdbk=%x tx lb ctl=%x\n",
          __func__, tsc->unit, tsc->port, enable, value, data);
...
tscmod_diag_poke(tscmod_st *tsc, int fb, int val)
{
   ...
   rv = SOC_E_NONE ;
   printf("%s u=%0d p=%0d ctrl_type=%0x\n", __func__, tsc->unit, tsc->port,
          tsc->ctrl_type) ;
   addr = 0 ; data = 0 ;
   switch(fb) {
   ...
   void *arg)       /* command argument based on op_type/op_cmd */
{
   ...
   default:
      ...
      if (op_type == PHY_DIAG_CTRL_SET) {
         rv = phy_tscmod_control_set(unit,port,op_cmd,PTR_TO_INT(arg));
      } else if (op_type == PHY_DIAG_CTRL_GET) {
         rv = phy_tscmod_control_get(unit,port,op_cmd,(uint32 *)arg);
      }
      break ;
   }
...
phy_tscmod_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if((tsc.model_type == TSCMOD_MODEL_TYPE_ILLEGAL)) {
        return SOC_E_NOT_FOUND;
    }
                       ...
                       soc_phy_event_t event, uint32 value)
{
    ...
    case phyEventAutoneg:
        rv = (phy_tscmod_an_set(unit, port, value));
        break;
...
_phy_tscmod_notify_duplex(int unit, soc_port_t port, uint32 duplex)
{
   ...
   if ((!PHY_SGMII_AUTONEG_MODE(unit, port)) &&
       (!PHY_PASSTHRU_MODE(unit, port))) {
      SOC_IF_ERROR_RETURN
         (phy_tscmod_an_set(unit, port, FALSE));
   }
...
_phy_tscmod_notify_speed(int unit, soc_port_t port, uint32 speed)
{
   ...
   /* Update speed */
   SOC_IF_ERROR_RETURN
      (phy_tscmod_speed_set(unit, port, speed));
...
_phy_tscmod_stop(int unit, soc_port_t port)
{
   ...
   /* 'Stop' only for speeds < 10G. 
      if(10000 <= speed) {  return SOC_E_NONE; }  */
...
_phy_tscmod_notify_interface(int unit, soc_port_t port, uint32 intf)
{
   ...
   if (intf != SOC_PORT_IF_SGMII) {
      ...
      SOC_IF_ERROR_RETURN
         (phy_tscmod_speed_set(unit, port, 1000));
   }
   return SOC_E_NONE;
}
...
int _tscmod_chip_an_init_done(int unit,int chip_num)
{
    ...
    PBMP_ALL_ITER(unit, port) {  /* value of port will be changed after macro expanded */
        pc = INT_PHY_SW_STATE(unit, port);
        SOC_DEBUG_PRINT((DK_PHY, "%-22s PBMP_ALL_ITER u=%0d p=%0d\n", __func__, unit, port)) ;
        if (pc == NULL) {
            continue;
        }
...
tscmod_uc_status_dump(int unit, soc_port_t port)
{
    ...
    printf("\nuC ver=%x crc=%x T=%0d T_idx=%0d %s 0x%0x 0x%0x\n\n", pCfg->uc_ver, pCfg->uc_crc, temper, temper_idx,
           tscmod_version_msg(), pCfg->cidH, pCfg->cidL) ; 
    ...
    */
    ...
    for (indx = 0; indx < size; indx++) {
        pDesc = &uc_desc[indx];
        soc_cm_print("%02d %02d %8s  %04d %07d %09d| %04d  %4d %4d "
           "|%02d  %4s %02d   %03d %03d |%03d %03d 0x%04x  "
           "|%04d %04d %04d %04d |%4d %4d %3d %3d %3d %3d |0x%04x|%02x\n",
