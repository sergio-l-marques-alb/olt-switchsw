/*
 * $Id: phy54880.c 1.99.58.1 Broadcom SDK $
...
/*
 * Function:
 *      _phy_89500_A0_lpf_init
 * Purpose:
 *      Function to initialize the LPF for automotive EMC.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
 * Returns:
...
STATIC int
_phy_89500_A0_lpf_init(int unit, soc_port_t port)
{
    phy_ctrl_t        *pc;
    ...
    /* raise peak */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_EXP_BR_LDS_CTRLr(unit, pc, 0x10FF));
    
    /* restart LDS */
    SOC_IF_ERROR_RETURN
        (WRITE_PHY54880_LRE_CTRLr(unit, pc, 0x3000));

    /* force to slave mode, 100Mbps, 1-pair */
    SOC_IF_ERROR_RETURN
...
_phy_54880_reset_setup(int unit, soc_port_t port)
{
    ...
    if (PHY_IS_BCM89500_A0(pc) && br_config->enable) {
        /* version 2 LPF for automotive EMC */
        _phy_89500_A0_lpf_init(unit, port);
    } 

    if (PHY_IS_BCM89500_B0(pc) && !br_config->enable) {
