/*
 * $Id: phy84328.c 1.95.2.4 Broadcom SDK $
...
static unsigned int phy84328_maj_rev = 1102;
...
#define PHY84328_SINGLE_PORT_MODE(_pc)  ((_pc)->speed_max >= 40000)
...
STATIC int _phy_84328_intf_type_check(int unit, soc_port_t port, phy84328_intf_side_t side,
...
/*
 * Checks are done by verifying PMD_CONTROL_2 contents. This register has both line and system 
 * side instances so check against both
 */

STATIC int
_phy_84328_intf_type_check(int unit, soc_port_t port, phy84328_intf_side_t side, 
						   soc_port_if_t intf_type)
{
	uint16		data;
	int			matched, retries = 0;
	int			rv = SOC_E_NONE;
    phy_ctrl_t  *pc = EXT_PHY_SW_STATE(unit, port);
	phy84328_intf_cfg_t *line_intf;
	...
	do {
		rv = READ_PHY84328_MMF_PMA_PMD_REG(unit, pc, 
										   PHY84328_DEV1_PMD_CONTROL_2_REGISTER, &data);
		if (SOC_FAILURE(rv)) {
			phy84328_counters_t *counters = &(COUNTERS(pc));
			counters->intf_chk_err++;
			_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
			return SOC_E_NONE;
		}
		data &= PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPESELECTION_MASK;

		switch (intf_type) {
		case SOC_PORT_IF_SR:
		case SOC_PORT_IF_SFI:
		case SOC_PORT_IF_GMII:
			matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR);
			break;
		case SOC_PORT_IF_CR:
			matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR);
			break;
		case SOC_PORT_IF_KR:
			if (side == PHY84328_INTF_SIDE_LINE) {
				matched = AN_EN(pc) ?
					PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR :
					PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_KR;
			} else {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR);
			}
			break;
		case SOC_PORT_IF_XFI:
			...
			break;
		case SOC_PORT_IF_KR4:
			matched = (side == PHY84328_INTF_SIDE_LINE) ?
				(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_KR4) :
				(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR4);
			break;
		case SOC_PORT_IF_SR4:
			if (line_intf->speed == 42000) {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR);
			} else {
				matched = (side == PHY84328_INTF_SIDE_LINE) ?
					(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR4) :
					(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_LR4);
			} 
			break;
		case SOC_PORT_IF_CR4:
			if (line_intf->speed == 42000) {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR);
			} else if (side == PHY84328_INTF_SIDE_LINE) {
				matched = (AN_EN(pc)) ?
					(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_CR4) :
					(data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_SR4);
			} else {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_LR4);
			}
			break;
		case SOC_PORT_IF_XLAUI:
			matched = (side == PHY84328_INTF_SIDE_LINE) ?
		...
		case SOC_PORT_IF_LR4:
			if (line_intf->speed == 42000) {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_LR);
			} else {
				matched = (data == PHY84328_DEV1_PMD_CONTROL_2_REGISTER_PMA_TYPE_LR4);
			}
			break;
		...
		default:
			matched = 0;
			retries = PHY84328_UPDATE_CHECK_MAX_RETRIES;
		}

		if (!matched) {
			retries++;
			sal_udelay(PHY84328_UPDATE_CHECK_DELAY);
		}
	} while (!matched && (retries < PHY84328_UPDATE_CHECK_MAX_RETRIES));

	/* back to default line side registers */
	if (side == PHY84328_INTF_SIDE_SYS) {
		_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
	}
	if (!matched) {
		...
		SOC_DEBUG_PRINT((DK_ERR,
						 "84328 intf check %s: u=%d p=%d side=%s type=%s (1.%x=0x%x) retries=%d\n", 
						 matched ? "passed" : "failed", unit, port,
						 (side == PHY84328_INTF_SIDE_SYS) ? "system" : "line",
						 phy84328_intf_names[intf_type], PHY84328_DEV1_PMD_CONTROL_2_REGISTER,  
						 data, retries));
	}
	return (matched ? SOC_E_NONE : SOC_E_FAIL);
}
...
_phy_84328_micro_pause(int unit, soc_port_t port, const char *loc)
{
	...
	while (checks < 1000) {
		...
		rv = READ_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REG_1, &data);
		if (rv != SOC_E_NONE) {
			goto fail;
		}
		if ((data & mask) == 0) {
...
_phy_84328_config_update(int unit, soc_port_t port)
{
	...
	SOC_IF_ERROR_RETURN(_phy_84328_micro_pause(unit, port, "config update"));
	...
	SOC_IF_ERROR_RETURN(_phy_84328_micro_resume(unit, port));
...
_phy_84328_polarity_flip_tx(int unit, soc_port_t port, uint16 cfg_pol)
{
    ...
    if ((PHY84328_SINGLE_PORT_MODE(pc))) {
		...
		flip = (cfg_pol == POL_CONFIG_ALL_LANES) ? 1 : 0;
		SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_tx_set(unit, port, flip));
...
_phy_84328_polarity_flip_rx(int unit, soc_port_t port, uint16 cfg_pol)
{
    ...
    if ((PHY84328_SINGLE_PORT_MODE(pc))) {
		...
		if (CUR_DATAPATH(pc) != PHY84328_DATAPATH_20) {
			_phy_84328_micro_pause(unit, port, "polarity rx");
		}
        for (lane = 0; lane < PHY84328_NUM_LANES; lane++) {
			flip = 0;
			if ((cfg_pol == POL_CONFIG_ALL_LANES) || 
		...
		_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
		if (CUR_DATAPATH(pc) != PHY84328_DATAPATH_20) {
			_phy_84328_micro_resume(unit, port);
		}

    } else {  /* quad mode */

		flip = (cfg_pol == POL_CONFIG_ALL_LANES);
		if (CUR_DATAPATH(pc) == PHY84328_DATAPATH_20) {
			_phy_84328_intf_side_regs_select(unit, port, PHY84328_INTF_SIDE_LINE);
			SOC_IF_ERROR_RETURN(_phy_84328_polarity_flip_rx_20bit(unit, port, flip));
		} else {
...
_phy_84328_init_pass2(int unit, soc_port_t port)
{
	 ...
	 */
	SOC_IF_ERROR_RETURN(WRITE_PHY84328_MMF_PMA_PMD_REG(unit, pc, PHY84328_DEV1_GP_REG_0, 0x0));
	...
	if (MOD_AUTO_DETECT(pc)) {
		...
		SOC_DEBUG_PRINT((DK_ERR, "PHY84328 module auto detect enabled: u=%d p=%d\n",
						 unit, port));
		data = PHY84328_DEV1_GP_REG_0_ENABLE_QSFP_MOD_DETECT_MASK |
			PHY84328_DEV1_GP_REG_0_ENABLE_SFP_MOD_DETECT_MASK;
		
		mask = PHY84328_DEV1_GP_REG_0_ENABLE_QSFP_MOD_DETECT_MASK |
			PHY84328_DEV1_GP_REG_0_ENABLE_SFP_MOD_DETECT_MASK;
...
phy_84328_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS2) ||
        (PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_DEFAULT)) {
		return _phy_84328_init_pass2(unit, port);
	}
...
phy_84328_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
	...
	sal_memset(ability, 0, sizeof(soc_port_ability_t));
	pa_speed = PHY84328_SINGLE_PORT_MODE(pc) ? SOC_PA_SPEED_40GB :
...
STATIC int
_phy_84328_do_rxseq_restart(int unit, soc_port_t port)
{
	uint16 data;
	phy_ctrl_t *pc = EXT_PHY_SW_STATE(unit, port);

	_phy_84328_micro_pause(unit, port, "rxseq restart1");
	SOC_IF_ERROR_RETURN
	...
	_phy_84328_micro_pause(unit, port, "rxseq restart 2");
	SOC_IF_ERROR_RETURN
									 ...
									 uint32 *rx_seq_done, uint32 *no_cdr_lanes)
{
	...
	*no_cdr_lanes = 0;
    ...
    if (side == PHY84328_INTF_SIDE_SYS) {
        _phy_84328_micro_pause(unit, port, "cdr lanes get");
    }
								   ...
								   uint32 value)
{
	...
	if (side == PHY84328_INTF_SIDE_SYS) {
		_phy_84328_micro_pause(unit, port, "preemphasis set");
	}
								   ...
								   phy84328_intf_side_t side, uint32 *value)
{
	...
	if (side == PHY84328_INTF_SIDE_SYS) {
		_phy_84328_micro_pause(unit, port, "preemphasis get");
	}
								 ...
								 phy84328_intf_side_t side, uint32 value)
{
	...
	if (side == PHY84328_INTF_SIDE_SYS) {
		_phy_84328_micro_pause(unit, port, "tx driver set");
	}
								 ...
								 phy84328_intf_side_t side, uint32 *value)
{
	...
	if (side == PHY84328_INTF_SIDE_SYS) {	
		_phy_84328_micro_pause(unit, port, "tx driver get");
	}
...
_phy_84328_debug_info(int unit, soc_port_t port)
{
	...
	soc_cm_print("Port %-2d: rev=%d:%d, chiprev=%04x, micro ver(1.%04x)=%04x, micro(1.%04x)=%04x, die temp=%d(C)%s\n"
				 "     pri:offs=%d:%d, mdio=0x%x, datapath=%d, srl=%s, mod auto=%s, sync=%s, link mon=%s\n",
	...
	counters = &(COUNTERS(pc));
	soc_cm_print(" Counters: link down=%d, intf updates=%d, rxLOS restarts=%d, no cdr=%d, micro nopause=%d\n"
	...
	for (side = PHY84328_INTF_SIDE_LINE; side <= PHY84328_INTF_SIDE_SYS; side++) {
		...
		if (PHY84328_SINGLE_PORT_MODE(pc)) {
			...
			if (side == PHY84328_INTF_SIDE_SYS) {
				_phy_84328_micro_pause(unit, port, "");
			}
			for (lane = 0; lane < PHY84328_NUM_LANES; lane++) {
...
_phy_84328_control_driver_supply_set(int unit, soc_port_t port, phy84328_intf_side_t side,
									 soc_port_phy_control_driver_supply_t value)
{
	uint16 tx_mode = 0;
							 ...
							 soc_port_phy_control_driver_supply_t *value)
{
	...
	switch (tx_mode) {
	case 4:
		*value = SOC_PHY_CONTROL_DRIVER_SUPPLY_1_5V_1_0V;
		break;
	case 0:
		*value = SOC_PHY_CONTROL_DRIVER_SUPPLY_1_0V_700MV;
		break;
	case 1:
		*value = SOC_PHY_CONTROL_DRIVER_SUPPLY_1_5V_700MV;
		break;
	default:
...
_phy_84328_diag_dsc(int unit, soc_port_t port, int intf, int lane)
{
	...
	_phy_84328_micro_pause(unit, port, "dsc");
...
_phy_84328_speed_set(int unit, soc_port_t port, int speed)
{
	...
	if (int_pc != NULL) {
		if (sys_intf->type == SOC_PORT_IF_GMII) {
    ...
    if (PHY84328_SINGLE_PORT_MODE(pc)) {
		...
		case 1000:
			...
			if (sys_intf->type == SOC_PORT_IF_KR) {
				...
				sys_intf->type = SOC_PORT_IF_GMII;
			}
			break;
		case 10000:
			/* 
...
_phy_84328_intf_type_set(int unit, soc_port_t port, soc_port_if_t pif, int must_update)
{
    ...
    if (_phy_84328_intf_is_single_port(pif)) {
		...
		/* 
		 * CR4 is unique in that there may be a need to change the datapath
		 * depending on whether AN is also changing
		 */
		...
		if ((line_intf->speed == 1000) && (!_phy_84328_intf_type_1000(pif))) {
            SOC_DEBUG_PRINT((DK_ERR, 
							 "84328 invalid interface for single port speed 1000: u=%d p=%d\n",
                             unit, port));
			return SOC_E_CONFIG;
		}
	...
	if (update) {
		...
		SOC_IF_ERROR_RETURN(_phy_84328_intf_type_check(unit, port, PHY84328_INTF_SIDE_LINE,
													   pif));
	}
