/*
 * $Id: wc40.c 1.202.2.2 Broadcom SDK $ 
...
_phy_wc40_firmware_force_cl72_config(int unit, soc_port_t port, int enable)
{
    ...
    if(pll_reset && enable) {
        ...
        soc_cm_print("WC forced cl72 release pll resetl port:%d\n", port);
    }
...
_phy_wc40_combo_core_init(int unit, soc_port_t port)
{
    ...
    */
    ...
    SOC_IF_ERROR_RETURN
        (MODIFY_WC40_TXB_ANATXACONTROL0r(unit, pc, 0x0,
            0x00,
...
_phy_wc40_ind_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if ((speed == 11000) && (SOC_IS_HELIX4(unit))) {
        /* need to bring down the link */
        /* stop PLL sequencer. Bring down link on all ports */
        SOC_IF_ERROR_RETURN
            (MODIFY_WC40_XGXSBLK0_XGXSCONTROLr(unit, pc, 0x00,
                                           0,
...
_wc40_soft_an_cl73kr2(int unit, soc_port_t port, int an)
{
    ...
    if(SOFT_KR2_TIMER_VALID(pc)) {
        ...
        SOC_IF_ERROR_RETURN
            (MODIFY_WC40_TX0_TX_DRIVERr(unit, pc, 0x0, 0x0000, 0x8000));
    }
    return SOC_E_NONE;
}
                ...
                */ 
                if(!(data16 & AN_IEEE1BLK_AN_LP_BASEPAGEABILITY0_NP_MASK)
                   ...
                   && (!SOFT_KR2_TIMER_VALID(pc))) {
                    ...
                    /* Start Tx Squelch */
                    SOC_IF_ERROR_RETURN
                        (MODIFY_WC40_TX0_TX_DRIVERr(unit, pc, 0x0, 0x8000, 0x8000));
