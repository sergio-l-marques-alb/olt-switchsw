/*
 * $Id: wcmod_main.c 1.13.10.1 Broadcom SDK $
 *
...
wcmod_spd_intfc_set wcmod_spd_intf_s2e(char* s)
{
  ...
  for (i = 0;i < WCMOD_SPD_ILLEGAL;i++) {
    if (!strcmp (s, e2s_wcmod_spd_intfc_set[i])) {
      return i;
    }
  }
...
int wcmod_tier1_selector(char* cs,wcmod_st* ws, int *returnValue)
{
  ...
  if (ws->verbosity > 0) printf("%-22s(%-18s): Unit/Port/Phy_ad %d/%d/0x%x\n",
                                      __func__,cs,ws->unit,ws->port,ws->phy_ad);
  #ifndef _SDK_WCMOD_
  ...
  for(lane = 0; lane < num_lane; lane++) {
    ...
    if (ws->dxgxs > 3)  {
      if (ws->verbosity > 0) printf ("%s Error: Invalid dxgxs value:%d", __func__, ws->dxgxs);
      return SOC_E_ERROR;
    }
...
#define IF_WCM_HNDL_CMP(_str,_func)  if(!strcmp((_str),cs)){rv=(_func)(ws);}
...
int searchPCSFuncTable(char* selStr, wcmod_st* ws)
{
  ...
  while (wcmod_pcsFuncTable[index].p != NULL) {
    if (!strcmp(selStr, wcmod_pcsFuncTable[index].p)) {
  ...
  if (ws->verbosity > 0) printf ("%s Internal Error. Bad Search %s ", __func__, selStr);
