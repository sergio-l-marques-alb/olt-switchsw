...
soc_robo_macsec_bypass_set(int unit, soc_port_t port, uint32 value)
{
...
#ifndef NSP_FPGA_PLATFORM        
#if BCM_NSP_MACSEC_DEVICE_READY
    uint32  macsec_reg_addr, macsec_reg_val;
#endif  /* BCM_NSP_MACSEC_DEVICE_READY */
#endif  /* NSP_FPGA_PLATFORM */
#endif /* BCM_NORTHSTARPLUS_SUPPORT */
    
    soc_cm_debug(DK_ERR, "%s,port=%d,val=%d...\n", 
    ...
    if (!switchmacsec_attached) {
        soc_cm_debug(DK_ERR, "%s,port %d has no MACSEC attached!\n", 
                FUNCTION_NAME(), port);
        return SOC_E_PORT;
    }
...
soc_robo_info_config(int unit, soc_control_t *soc)
{
    ...
    case BCM5389_DEVICE_ID:
        ...
        robo->arch_type = SOC_ROBO_ARCH_DINO8;
        si->num_cpu_cosq = 4;
        break;
        ...
        case BCM53020_DEVICE_ID:
            ...
            SOC_PBMP_CLEAR(pbmp_valid);
            SOC_PBMP_PORT_ADD(pbmp_valid, 0);
            SOC_PBMP_PORT_ADD(pbmp_valid, 4);
            SOC_PBMP_PORT_ADD(pbmp_valid, 5);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 0);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 4);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 5);
            
#else /* !NSP_FPGA_PLATFORM */
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 0);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 1);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 2);
            SOC_PBMP_PORT_ADD(si->gmii_pbm, 3);
...
soc_robo_mgt_init(int unit)
{
    ...
    uint32 imp1_rgmii = 0;
#endif /* BCM_53125 || BCM_53128 || BCM_POLAR_SUPPORT || NORTHSTAR || NS+ */
    ...
    if (unit == soc_mii_unit) {
        ...
        if (SOC_IS_ROBO_ARCH_VULCAN(unit) || SOC_IS_DINO8(unit)) {
            ...
            if (SOC_IS_LOTUS(unit)) {
				...
				if (SOC_IS_NORTHSTARPLUS(unit)) {
					temp = 0x3F308; /* P5 MUX value register */
					reg_value = CMREAD(soc_eth_unit, temp);
					imp1_rgmii = ((reg_value & 0x7) == 0x2)? 1 : 0;
				}
#endif /* BCM_NORTHSTARPLUS_SUPPORT */
#endif /* NSP_FPGA_PLATFORM */
                if (imp1_rgmii) {
