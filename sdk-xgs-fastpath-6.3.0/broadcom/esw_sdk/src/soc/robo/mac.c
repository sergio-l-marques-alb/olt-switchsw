...
mac_robo_fe_init(int unit, soc_port_t port)
{
    ...
    soc_cm_debug(DK_VERBOSE, "mac_robo_fe_init\n");
...
mac_robo_fe_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /* initialize per port mac */
    soc_cm_debug(DK_VERBOSE, "mac_robo_fe_enable_set\n");
...
mac_robo_fe_enable_get(int unit, soc_port_t port, int *enable)
{
    ...
    /* initialize per port mac */
    soc_cm_debug(DK_VERBOSE, "mac_robo_fe_enable_get\n");
...
mac_robo_fe_ge_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    soc_cm_debug(DK_VERBOSE, "mac_robo_fe_ge_duplex_set: u=%d p=%d FD=%x\n",
        unit, port, duplex);
...
mac_robo_fe_ge_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    if (SOC_IS_HARRIER(unit)) {
     ...
     soc_cm_debug(DK_VERBOSE, "mac_robo_fe_ge_duplex_get: u=%d p=%d FD=%x\n",
        unit, port, *duplex);
...
mac_robo_fe_ge_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_speed_set: u=%d p=%d sp=%d\n",
        unit, port, speed);
...
#define _TB_SPD_STATUS_MASK_PORT   0x3
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_speed_get: u=%d p=%d sp=%d\n", 
        unit, port, *speed);
    return SOC_E_NONE;
}
...
{
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_pause_addr_set:\n");
...
mac_robo_fe_ge_pause_addr_get(int unit, soc_port_t port, 
    sal_mac_addr_t pause_mac)
{
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_pause_addr_get:\n");
...
{
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_loopback_set\n");    
    if (loopback == TRUE){
        return SOC_E_UNAVAIL;
    }
...
{
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_loopback_get\n");
    *loopback = FALSE;    
    return SOC_E_NONE;
}
...
{
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_interface_set\n");  
    return SOC_E_UNAVAIL;
}
...
mac_robo_fe_ge_ability_get(int unit, soc_port_t port, soc_port_mode_t *mode)
{
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_ability_get: ability=0x%x\n", 
        *mode);
    return (SOC_E_NONE);
}
        ...
        soc_port_ability_t *ability)
{
    ...
    if (IS_FE_PORT(unit, port)) {
        ...
        if (SOC_IS_ROBO53262(unit) || SOC_IS_DINO8(unit)) {
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_ability_get\n");
    return (SOC_E_NONE);
}
...
mac_robo_fe_ge_frame_max_set(int unit, soc_port_t port, int size)
{
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_frame_max_set\n"); 
...
mac_robo_fe_ge_frame_max_get(int unit, soc_port_t port, int *size)
{
    ...
    soc_cm_debug(DK_VERBOSE,"mac_robo_fe_ge_frame_max_get\n");  
...
mac_robo_ge_init(int unit, soc_port_t port)
{
    ...
    /* initialize per port mac */
    soc_cm_debug(DK_VERBOSE, "mac_robo_ge_init\n");
...
mac_robo_ge_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /* initialize per port mac */
    soc_cm_debug(DK_VERBOSE, "mac_robo_ge_enable_set\n");
...
mac_robo_ge_enable_get(int unit, soc_port_t port, int *enable)
{
    ...
    /* initialize per port mac */
    soc_cm_debug(DK_VERBOSE, "mac_robo_ge_enable_get\n");
    ...
    defined(BCM_NORTHSTARPLUS_SUPPORT)
        if ((SOC_IS_POLAR(unit) || SOC_IS_NORTHSTAR(unit) || 
            SOC_IS_NORTHSTARPLUS(unit)) 
            && (port == specified_port_num)) {
            port_ctrl_addr = DRV_REG_ADDR(unit, P7_CTLr, 0, 0);
        } else 
#endif /* BCM_POLAR_SUPPORT || BCM_NORTHSTAR_SUPPORT || NS+ */
        {
            port_ctrl_addr = DRV_REG_ADDR(unit, G_PCTLr, port, 0);
