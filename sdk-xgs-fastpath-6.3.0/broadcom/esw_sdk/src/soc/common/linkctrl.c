/*
 * $Id: linkctrl.c 1.7 Broadcom SDK $
...
soc_linkctrl_linkscan_config(int unit, pbmp_t hw_mii_pbm, pbmp_t hw_direct_pbm)
{
    ...
    if (SOC_IS_SIRIUS(unit) || SOC_IS_CALADAN3(unit)) {
        return (LINKSCAN_CONFIG(unit, hw_mii_pbm, hw_direct_pbm));
    }
 ...
 *     the port numbers in some devices, such as SBX devices,
