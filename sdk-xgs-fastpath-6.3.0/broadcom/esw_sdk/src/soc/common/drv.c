/*
 * $Id: drv.c 1.139.8.2 Broadcom SDK $
...
soc_endian_config(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicm)) {
        ...
        if (soc_feature(unit, soc_feature_iproc) &&
            ...
            /* Set PCI PIO endian in PCIe core and not in CMIC */
            soc_cm_iproc_write(unit, PAXB_ENDIANESS, pio_endian);
            if (soc_cm_iproc_read(unit, PAXB_ENDIANESS) != 0) {
                pio_endian = 0;
                big_packet = 1;
                big_other = 0;
            }
...
soc_pci_ep_config(int unit, int pcie)
{
    ...
    if (soc_feature(unit, soc_feature_iproc) &&
        ...
        if (pcie == 0) {
            ...
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_1_OARR_2_UPPERr, REG_PORT_ANY, 0, &blk, &acc_type), 1);
        }
    }
#endif
}
