    ...
    while (_ihandlers) {
        ...
        for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
            if (_ictrl[dev].handler != NULL && !_ictrl[dev].paused) {
                ...
                cmc = SOC_PCI_CMC(dev);
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_ARAD_SUPPORT) 
                if ((soc_pci_read(dev, CMIC_CMCx_IRQ_STAT0_OFFSET(cmc)) & SOC_CMCx_IRQ0_MASK(dev,cmc)) || 
                    (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT1_OFFSET(cmc)) & SOC_CMCx_IRQ1_MASK(dev,cmc)) ||
                    (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT2_OFFSET(cmc)) & SOC_CMCx_IRQ2_MASK(dev,cmc))) {
                    _ictrl[dev].handler(_ictrl[dev].data);
                } else {
                    if (soc_feature(dev, soc_feature_cmicm_multi_dma_cmc)) {
                        if ((soc_pci_read(dev, CMIC_CMCx_IRQ_STAT0_OFFSET(SOC_ARM_CMC(dev, 0))) & 
                             SOC_CMCx_IRQ0_MASK(dev, SOC_ARM_CMC(dev, 0))) || 
                            (soc_pci_read(dev, CMIC_CMCx_IRQ_STAT0_OFFSET(SOC_ARM_CMC(dev, 1))) &
                             SOC_CMCx_IRQ0_MASK(dev, SOC_ARM_CMC(dev, 1)))) {
                            _ictrl[dev].handler(_ictrl[dev].data);
                        }
                    }
                    if ((soc_feature(dev, soc_feature_extended_cmic_error)) || (soc_feature(dev, soc_feature_short_cmic_error))) {
