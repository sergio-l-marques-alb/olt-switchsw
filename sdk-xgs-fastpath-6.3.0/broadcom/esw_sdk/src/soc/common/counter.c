/*
 * $Id: counter.c 1.201.2.4 Broadcom SDK $
                                   ...
                                   int *non_dma_entries)
{
    soc_control_t *soc;
    soc_counter_non_dma_t *non_dma0, *non_dma1, *non_dma2;
    int num_entries, alloc_size, entry_words;
    uint32 *buf;

    soc = SOC_CONTROL(unit);
    *non_dma_entries = 0;

    /* EGR_PERQ_XMT_COUNTERS is the largest table to be DMA'ed */
    num_entries = soc_mem_index_count(unit, EGR_PERQ_XMT_COUNTERS_Xm);
    entry_words = soc_mem_entry_words(unit, EGR_PERQ_XMT_COUNTERS_Xm);
    alloc_size = 2 * num_entries * sizeof(uint32) * entry_words;
    ...
    non_dma0->base_index = non_dma_start_index + *non_dma_entries;
    non_dma0->entries_per_port = 48; /* cpu port has max number of queues */
    non_dma0->num_entries = 2 * num_entries;
    non_dma0->mem = EGR_PERQ_XMT_COUNTERSm;
    non_dma0->reg = INVALIDr;
    non_dma0->field = PACKET_COUNTERf;
    non_dma0->cname = "PERQ_PKT";
    non_dma0->dma_buf[0] = buf;
    non_dma0->dma_buf[1] = &buf[num_entries * entry_words];
    non_dma0->dma_index_max[0] = num_entries - 1;
    non_dma0->dma_index_max[1] = num_entries - 1;
    non_dma0->dma_mem[0] = EGR_PERQ_XMT_COUNTERS_Xm;
    non_dma0->dma_mem[1] = EGR_PERQ_XMT_COUNTERS_Ym;
    *non_dma_entries += non_dma0->num_entries;
...
#ifdef BCM_SBUSDMA_SUPPORT
STATIC sbusdma_desc_handle_t _soc_port_counter_handles[SOC_MAX_NUM_DEVICES][SOC_MAX_NUM_PORTS][3];
#endif
                               ...
                               uint64 *value)
{
    ...
    soc_sand_rv = MBCM_DPP_DRIVER_CALL_WITHOUT_DEV_ID(unit, mbcm_dpp_port_to_interface_map_get, soc_sand_dev_id, port, 
                                                  &ifp_mapping, &out_mapping);
...
soc_petra_counter_collect64(int unit, int discard)
{
    ...
    soc_port_t          port;
    uint64              ctr_new;
    int                 mal, mal_port, index, port_base;
...
#define _EP_COUNTERS "EP COUNTERS"
        ...
        if (SOC_BLOCK_IS_CMP(unit, SOC_PORT_BLOCK(unit, port), SOC_BLK_GPORT) ) {
    ...
    if (soc->blk_ctr_desc_count) {
        ...
        for (bindex = 0; bindex < soc->blk_ctr_desc_count; bindex++) {
            ...
            if (cnt) {
                ...
                sal_strncpy(ctrl.name, "BLK CTR", 7);
...
#ifdef BCM_ARAD_SUPPORT
                    SOC_DPP_ALLOW_WARMBOOT_WRITE_NO_ERR(rv = soc_counter_collect64(unit, FALSE, -1, INVALIDr));
#endif
                } else {
                    rv = SOC_E_NONE;
                }
