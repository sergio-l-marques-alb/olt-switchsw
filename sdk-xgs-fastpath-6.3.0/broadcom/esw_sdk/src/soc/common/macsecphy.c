/*
 * $Id: macsecphy.c 1.63 Broadcom SDK $
...
soc_switchmacsec_init(int unit, soc_port_t port)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "%s: u=%d port=%d\n",
    ...
    if (macsec_enable) {
        ...
        if(rv != BMACSEC_E_NONE) {
            ...
            soc_cm_debug(DK_ERR, 
                    "%s:reassign rv to 0 for simulation!\n", FUNCTION_NAME());
            rv = 0;
    ...
    if (!SOC_SUCCESS(rv)) {
        soc_cm_debug(DK_ERR, "soc_switchmacsec_init: MACSEC init for"
                     " u=%d p=%d FAILED\n", unit, port);
    } else {
        soc_cm_debug(DK_PHY, "soc_switchmacsec_init: MACSEC init for"
                     " u=%d p=%d SUCCESS\n", unit, port);
    }
#if !BCM_NSP_MACSEC_DEVICE_READY
    soc_cm_debug(DK_ERR, "%s,%d,Reassign rv=0 for simulation!\n",
