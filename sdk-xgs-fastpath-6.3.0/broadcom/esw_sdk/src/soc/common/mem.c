/*
 * $Id: mem.c 1.402.2.15 Broadcom SDK $
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        if (soc_feature(unit, soc_feature_table_dma) &&
            ...
            if ((table = soc_cm_salloc(unit, cache_size, "dma")) != NULL) {
                if (soc_mem_read_range(unit, mem, blk, index_min, index_cnt - 1,
...
#if 0 /* The following is an invalid criteria for not doing DMA */
 ...
 */
STATIC int
_soc_xgs3_mem_dma(int unit, soc_mem_t mem, unsigned array_index, int copyno,
             int index_min, int index_max, void *buffer)
{
    ...
    start_addr = soc_mem_addr_get(unit, mem, array_index, copyno, index_min, &at);
#if (defined  BCM_CMICM_SUPPORT)  
                  ...
                  int index_min, int index_max, void *buffer)
{
    ...
    if (soc_feature(unit, soc_feature_sbusdma)) {
        SOC_IF_ERROR_RETURN
            (_soc_mem_array_sbusdma_read(unit, mem, array_index, copyno, index_min, index_max,
                                   buffer));
    } else
#endif
    {
...
#ifdef BCM_SBUSDMA_SUPPORT
    if (soc_feature(unit, soc_feature_sbusdma)) {
        rv = _soc_mem_array_sbusdma_write(unit, mem, array_index, array_index, copyno, index_begin, index_end,
                                    buffer_ptr, FALSE, -1);
    } else
#endif
    {
              ...
              void *entry_data)
{
    ...
    if (index_valid && 
        ...
        sal_memcpy(entry_data_cache, cache + index * entry_dw, entry_dw * 4);
        cache_consistency_check = 1;
#else
        sal_memcpy(entry_data, cache + index * entry_dw, entry_dw * 4);
        if (!SOC_MEM_FORCE_READ_THROUGH(unit)) {
            goto done;
        }
#endif
    }
...
return_data:
    ...
    /*
     * Add read data to memory cache if it is enabled. Skip mems with h/w learnt entries. 
     */
                         ...
                         int copyno, int index_min, int index_max, void *buffer)
{
    ...
    defined(BCM_ARAD_SUPPORT)
        ...
        if (SOC_IS_SIRIUS(unit) || SOC_IS_XGS3_SWITCH(unit) || 
            ...
            vmap = SOC_MEM_STATE(unit, mem).vmap[copyno];
            entry_dw = soc_mem_entry_words(unit, mem);
            /* Skip mems with h/w learned entries. */
...
_retry_op:
        ...
        if ((rv = soc_schan_op(unit, &schan_msg,
            ...
            if (soc_feature(unit, soc_feature_shared_hash_mem)) {
                ...
                rv1 = soc_ser_sram_correction(unit, mem, blk, index2);
                if (rv1 == SOC_E_NONE) {
                    goto _retry_op;
                } 
            }
    ...
    if ((soc_feature(unit, soc_feature_ism_memory) && soc_mem_is_mview(unit, mem)) ||
        ...
        if (banks && banks != SOC_MEM_HASH_BANK_ALL) {
            if (soc_feature(unit, soc_feature_shared_hash_mem)) {
                ...
                schan_msg.gencmd.address |= (~banks & SOC_HASH_BANK_MASK_ISM);
            }
        }
    } 
            ...
            if (bmap & (1 << i)) {
                ...
                if (SOC_FAILURE(rc)) {
                    if (rc == SOC_E_FAIL) {
                        ...
                        soc_cm_debug(DK_VERBOSE, "Failure in bucket: %d\n", 
                                     index);
                        ...
                        for (e = 0; e < entries; e++) {
                            ...
                            if (CACHE_VMAP_TST(vmap, entry_base+e)) {
                                ...
                                if (soc_feature(unit, soc_feature_ism_memory) && 
                                    ...
                                    if (sal_memcmp(key, match_key, entry_dw * 4)) {
                                        continue;
                                    } else {
                                        match = 1;
                                    }
              ...
              void *entry_data)
{
    ...
    if ((mem != L3_DEFIP_ALPM_IPV6_128m) &&
        ...
        soc_cm_debug(DK_ERR,
                     "soc_mem_alpm_read: not supported non-ALPM memories %s\n",
                     SOC_MEM_NAME(unit, mem));
        return SOC_E_PARAM;
    }
    ...
    /* Apply ALPM address mods */
    maddr &=  ~(0x7ffff);
    maddr |=  ((entry & 0x7) << 16) | ((index & 0x3ff) << 2)| (bank & 0x3);
    ...
    defined(BCM_PETRA_SUPPORT)|| defined(BCM_DFE_SUPPORT)
        ...
        schan_msg.readcmd.header.dstblk = ((maddr >> SOC_BLOCK_BP) & 0xf) |
                                  (((maddr >> SOC_BLOCK_MSB_BP) & 0x1) << 4);
#endif /* BCM_XGS3_SWITCH_SUPPORT || BCM_[[SIRIUS_SUPPORT || BCM_PETRA_SUPPORT|| BCM_DFE_SUPPORT*/
    }
               ...
               void *entry_data)
{
    ...
    if (soc_feature(unit, soc_feature_xy_tcam) &&
        ...
        (!(meminfo->flags & SOC_MEM_FLAG_EXT_CAM))) {
        entry_data_ptr = converted_entry_data;
        _soc_mem_tcam_dm_to_xy(unit, mem, 1, entry_data, entry_data_ptr, NULL);
    } else
#endif /* BCM_TRIDENT_SUPPORT || BCM_SHADOW_SUPPORT */
    {
        entry_data_ptr = entry_data;
...
_retry_op:
     ...
     */
    if (2 + entry_dw > CMIC_SCHAN_WORDS(unit)) {
       soc_cm_debug(DK_WARN, "soc_mem_generic_insert: assert will fail for memory %s\n", SOC_MEM_NAME(unit, mem));
    }
    rv = soc_schan_op(unit, &schan_msg, entry_dw + 2, entry_dw + 2, allow_intr);
    ...
    if ((schan_msg.header.cpu) || (rv == SOC_E_FAIL)) {
        ...
        if (type == SCHAN_GEN_RESP_TYPE_FULL) {
            ...
            if (soc_feature(unit, soc_feature_shared_hash_mem)) {
                ...
                rv1 = soc_ser_sram_correction(unit, mem, copyno, index);
                if (rv1 == SOC_E_NONE) {
                    goto _retry_op;
                } 
            }
 ...
 */
int ser_test_mem(int unit, soc_reg_t parity_enable_reg, int tcam_parity_bit,
	...
	uint16 indexesToTest[3];
	soc_mem_info_t *memInfo = &SOC_MEM_INFO(unit,mem);
	soc_field_t testField;
	int numIndexesToTest, j, startErrorCount, mem_err, read_through;
	uint32 fieldMask, badData, rv;
	char *mem_name;
	testField = (memInfo->fields[0].field);
	indexesToTest[0] = soc_mem_index_min(unit,mem);
	fieldMask = 1;
	startErrorCount = *error_count;
#ifdef SOC_MEM_NAME
	mem_name = SOC_MEM_NAME(unit, mem);
#else
	mem_name = "SOC_MEM_NAME undefined";
#endif
	...
	case SER_FIRST_MID_LAST_INDEX:
		indexesToTest[1] = soc_mem_index_max(unit,mem);
		indexesToTest[2] = indexesToTest[1] - indexesToTest[0];
		numIndexesToTest = 3;
		break;
	case SER_ALL_INDEXES:
		numIndexesToTest = soc_mem_index_max(unit,mem) + 1;
		break;
	...
	for (j = 0; j < numIndexesToTest; j++) {
		...
		if (test_type == SER_ALL_INDEXES){
			...
			index = indexesToTest[j];
		}
		/*Disable Parity*/
		if ((rv = _ser_test_parity_control(unit, parity_enable_reg,
										   tcam_parity_bit, hw_parity_field, port, 0)) < 0) {
			soc_cm_debug(DK_ERR, "unit %d %s entry %d mem parity disable error\n",
									 unit, mem_name, index);
			return rv;
		}

		/*Read the TCAM memory into a temporary buffer,
		 * Modify that temporary buffer, and then write it back to the memory*/
		if ((rv = soc_mem_read(unit, mem, mem_block, index, tmp_entry)) < 0) {
			soc_cm_debug(DK_ERR, "unit %d %s entry %d mem read error\n",
						 unit, mem_name, index);
			return SOC_E_MEMORY;
		}
		soc_mem_field_get(unit, mem, tmp_entry, testField, fieldData);
		if ((fieldData[0] & fieldMask) == 1) {
			fieldData[0]--;
		} else {
			fieldData[0]++;
		}
		badData = fieldData[0];
		/*Disable cache read/write until test is over*/
		soc_mem_field_set(unit, mem, tmp_entry, testField, fieldData);
		if ((soc_mem_write(unit, mem, mem_block, index, tmp_entry)) < 0) {
		...
		soc_mem_field_get(unit, mem, tmp_entry, testField, fieldData);
		if (badData != fieldData[0]) {
			soc_cm_print("Induction Error for mem: %s\n", mem_name);
			return SOC_E_MEMORY;
		}

		/*Enable Parity (Even if it was not enabled before*/
		if ((rv = _ser_test_parity_control(unit, parity_enable_reg,
										   tcam_parity_bit, hw_parity_field, port, 1)) < 0) {
			soc_cm_debug(DK_ERR, "unit %d %s entry %d mem parity enable error\n",
									 unit, mem_name, index);
			return rv;
		}
		/*force_read_through*/
		...
		/*Re-read the memory entry with cache disabled, in order to induce the error*/
		/*Enable NACK on read */
		mem_err = SOC_MEM_RETURN_SCHAN_ERR(unit, mem);
		SOC_MEM_RETURN_SCHAN_ERR_SET(unit, mem, TRUE);
		if ((rv = soc_mem_read(unit, mem, mem_block, index, tmp_entry)) < 0) {
			if (SOC_FAILURE(rv)) {
				soc_cm_debug(DK_ERR,
		...
		SOC_MEM_RETURN_SCHAN_ERR_SET(unit, mem, mem_err);
		...
		soc_mem_field_get(unit, mem, tmp_entry, testField, fieldData);
		if ((SOC_MEM_INFO(unit, mem).flags & SOC_MEM_SER_FLAGS) == SOC_MEM_FLAG_SER_ENTRY_CLEAR){
			if (0 != fieldData[0]) {
				(*error_count)++;
				soc_cm_debug(DK_ERR, "SER failed to clear mem %s index %d\n",
						mem_name, index);
			}
		}else if (badData == fieldData[0]) {
			(*error_count)++;
			soc_cm_debug(DK_ERR, "SER failed to correct mem %s index %d\n",
					mem_name, index);
		}
		SOC_MEM_FORCE_READ_THROUGH_SET(unit, read_through);
	}
	if (startErrorCount != *error_count) {
		soc_cm_print("SER failed to correct memory %s\n", mem_name);
	}
	return SOC_E_NONE;
}
 ...
 * Purpose:
 *      Disables/enables parity for the provided register. (Currently only supports TR2)
 * Parameters:
 *      unit       	- (IN) Device Number
 *      reg			- (IN) The register containing the parity setting
 *      bit			- (IN) If this memory is for a TCAM, the bit in the reg to set (Otherwise -1)
 *      enable_field- (IN) If not a TCAM, this is the field to toggle
 *      port		- (IN) If not a TCAM, the port for this reg (PORT_ANY is fine for TCAMS)
 *      enable		- (IN) 1 if parity should be enabled, 0 if it should be disabled.
 *
 * Returns:
 *      The number of failed tests (This can be more than one per memory if test_type is anything
 *      other than SER_SINGLE_INDEX.)
...
int _ser_test_parity_control(int unit, soc_reg_t reg, int bit,
	...
	uint8 parityEnableMask;
	uint32 regData, regAddr;
	if (bit >= 0){
		/*This register controls a TCAM memory*/
		regAddr = soc_reg_addr(unit, reg, REG_PORT_ANY, 0);
		/*The control register bitmap  corresponds with the _soc_ser_parity_info_t structure*/
		parityEnableMask = 1 << (bit+1);
		parityEnableMask = ~parityEnableMask;
		parityEnableMask = parityEnableMask >> 1;
		if(enable){
			soc_pci_getreg(unit, regAddr, &regData);
			regData = regData | (1 << bit);
			soc_pci_write(unit, regAddr, regData);
		}
		else{
			soc_pci_getreg(unit, regAddr, &regData);
			regData = regData & parityEnableMask;
			soc_pci_write(unit, regAddr, regData);
		}
	}
	else{
