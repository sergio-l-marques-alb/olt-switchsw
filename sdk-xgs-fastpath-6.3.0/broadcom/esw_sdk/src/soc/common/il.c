...
int soc_sbx_caladan3_il_init(int unit, int ifnum)
{
  uint32 data;
  /*int ifnum = SOC_BLOCK_PORT(unit, port);*/
  int wc;
  static int g_anWarpCore_PhyIDs[2][3] = { {0x81, 0xa1, 0xc1}, {0x95, 0x99, 0xb5} };

  /* RX */

  soc_reg32_get(unit, IL_RX_CORE_CONFIG0r, ifnum, 0, &data);
  /*“The IL Transmitter and Receiver on Caladan3 will support Non-interleaved 
    packet transmission only.”*/
  soc_reg_field_set(unit, IL_RX_CORE_CONFIG0r, &data, CTL_RX_PACKET_MODEf, 1);
  soc_reg_field_set(unit, IL_RX_CORE_CONFIG0r, &data, CTL_RX_BURSTMAXf, 3);
  soc_reg32_set(unit, IL_RX_CORE_CONFIG0r, ifnum, 0, data);

  soc_reg32_set(unit, IL_RX_CONFIGr, ifnum, 0, 16110);  /* IL_RX_MAX_PACKET_SIZE */

  /* enable all 64 channels - should based on actual number. */
  soc_reg32_set(unit, IL_RX_CHAN_ENABLE0r, ifnum, 0, 0xffffffff);
  soc_reg32_set(unit, IL_RX_CHAN_ENABLE1r, ifnum, 0, 0xffffffff);
  soc_reg32_set(unit, IL_TX_CHAN_ENABLE0r, ifnum, 0, 0xffffffff);
  soc_reg32_set(unit, IL_TX_CHAN_ENABLE1r, ifnum, 0, 0xffffffff);

  /* Since IL protects with CRC we could forgo FCS */
  soc_reg32_set(unit, IL_IEEE_CRC32_CONFIGr, ifnum, 0, 0);


  /* TX */
  soc_reg32_get(unit, IL_TX_CONFIGr, ifnum, 0, &data);
  soc_reg_field_set(unit, IL_TX_CONFIGr, &data, IL_TX_MAX_PACKET_SIZEf, 16110);
  soc_reg32_set(unit, IL_TX_CONFIGr, ifnum, 0, data);

  soc_reg32_get(unit, IL_TX_CORE_CONFIG0r, ifnum, 0, &data);
  soc_reg_field_set(unit, IL_TX_CORE_CONFIG0r, &data, CTL_TX_BURSTSHORTf, 1);
  soc_reg_field_set(unit, IL_TX_CORE_CONFIG0r, &data, CTL_TX_BURSTMAXf, 3);
  soc_reg32_set(unit, IL_TX_CORE_CONFIG0r, ifnum, 0, data);

  soc_reg32_get(unit, IL_TX_CORE_CONTROL0r, ifnum, 0, &data);
  soc_reg_field_set(unit, IL_TX_CORE_CONTROL0r, &data, SWO_CTL_TX_ENABLEf, 1);
  soc_reg_field_set(unit, IL_TX_CORE_CONTROL0r, &data, SWCTRL_CTL_TX_ENABLEf, 1);
  soc_reg32_set(unit, IL_TX_CORE_CONTROL0r, ifnum, 0, data);

  /* IL_TX_CORE_CONFIG2.CTL_TX_LAST_LANE = 12 */
  ...
  soc_reg32_get(unit, IL_GLOBAL_CONTROLr, ifnum, 0, &data);
  soc_reg_field_set(unit, IL_GLOBAL_CONTROLr, &data, SOFT_RESETf, 0);
  soc_reg32_set(unit, IL_GLOBAL_CONTROLr, ifnum, 0, data);

  /* init stats memory */
  soc_reg32_set(unit, IL_MEMORY_INITr, ifnum, 0, 0x1f);

  /* force synchronization */
  soc_reg32_set(unit, IL_RX_CORE_CONTROL0r, ifnum, 0, 1);
  sal_usleep(1000);
  soc_reg32_set(unit, IL_RX_CORE_CONTROL0r, ifnum, 0, 0);

  

#if 0  /* is WC driver doing this ?*/
  /* tx1g fifo reset procedure via MDIO. */
  for (wc = 0; wc < 3; ++wc ) {
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8061, 0x2000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8071, 0x2000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8081, 0x2000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8091, 0x2000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8061, 0x0000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8071, 0x0000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8081, 0x0000);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8091, 0x0000);
  }
#endif

#if 1
  /* Remove Master Slave wake up mode in WC */
  for (wc = 0; wc < 3; ++wc ) {
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8197, 0x8000);
  }
  for (wc = 0; wc < 3; ++wc ) {
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8000, 0x253f);
  }

  /* set speed to 10.3125 */
  for (wc = 0; wc < 3; ++wc ) {
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8308, 0x7a04);
  }
  for (wc = 0; wc < 3; ++wc ) {
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8000, 0x52f);
    soc_miimc45_write(unit, g_anWarpCore_PhyIDs[ifnum][wc], 0x1, 0x8000, 0x252f);
  }
#endif  

  soc_reg32_get(unit, IL_GLOBAL_CONFIGr, ifnum, 0, &data);
  soc_reg_field_set(unit, IL_GLOBAL_CONFIGr, &data, CFG_FLOWCONTROL_TYPEf, 0); /* 0=inband, 1=oob */
  ...
  soc_reg32_set(unit, IL_GLOBAL_CONFIGr, ifnum, 0, data);
...
mac_il_init(int unit, soc_port_t port)
{
    ...
    if(SOC_IS_CALADAN3(unit)) {
      int ifnum;
      ifnum = 1;
      return soc_sbx_caladan3_il_init(unit, ifnum);
    }
#endif
