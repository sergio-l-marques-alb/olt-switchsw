/*
 * $Id: er_tcam_type1.c 1.74 Broadcom SDK $
...
tr3_write_reg(int unit, int addr, uint32 d0_msb, uint32 d1, uint32 d2_lsb)
{
    ...
    if (SOC_DEBUG_CHECK(SOC_DBG_TCAM)) {
        soc_cm_print("Write Addr=0x%08x ", addr);
        soc_cm_print("Data = 0x%08x 0x%08x 0x%08x\n", d0_msb, d1, d2_lsb);
    }
                              ...
                              uint32 *data)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        dbus[1] = MAKE_WORD(GET_BYTE(tmp, 0), GET_BYTE(tmp, 1),
                            GET_BYTE(tmp, 2), 
                            /*SET_XY_WRITE |*/ SET_VBIT | (GET_BYTE(tmp, 3) & 0x3) | 0x2);
                               ...
                               int *index0, int *index1)
{
    ...
    if (len == 2) {
        dbus[2] = MAKE_WORD(GET_BYTE(data[3], 0), 
                            GET_BYTE(data[3], 1),
                            GET_BYTE(data[3], 2),
                            GET_BYTE(data[3], 3));
        dbus[5] = MAKE_WORD(GET_BYTE(data[4], 0), 
                            GET_BYTE(data[4], 1),
                            GET_BYTE(data[4], 2),
                            GET_BYTE(data[4], 3));
        dbus[4] = MAKE_WORD(GET_BYTE(data[5], 0), 
                            GET_BYTE(data[5], 1),
                            GET_BYTE(data[5], 2),
                            GET_BYTE(data[5], 3));
        byte_index = 20;
...
tr_tcam_init_type1(int unit)
{
    ...
    if (tcam_base && tcam_base0 && tcam_base1) {
        soc_cm_debug(DK_ERR, "tr_tcam_init_type1: unit %d has both normal "
...
soc_tr3_free_ltr(int unit, int ltr_lib_idx)
{
    ...
    if (nl_inst == NULL) {
        soc_cm_debug(DK_ERR, "unit %d: Attempting to free invalid ltr. ", unit);
        soc_cm_debug(DK_ERR, "Lib idx: %d\n", ltr_lib_idx);
        return SOC_E_PARAM;
    }
...
tr3_tcam_init(int unit)
{
    ...
    tcam_info = SOC_CONTROL(unit)->tcam_info;
    if (tcam_info == NULL) {
        soc_cm_print("unit: %d No external tables configured\n", unit);
        return SOC_E_NONE;
    }
    partitions = tcam_info->partitions;
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        case 0x02: /* 512k 40-bit records (256k 80-bit entries) 20M */
            partitions[TCAM_PARTITION_RAW].num_entries = 256 * 1024 *
                                                        tcam_info->num_tcams;
    		blks_per_dev = TR3_BLKS_PER_DEV_64;
            break;
        case 0x03: /* 1024k 40-bit records (512k 80-bit entries) 40M  */
            partitions[TCAM_PARTITION_RAW].num_entries = 512 * 1024 * 
                                                        tcam_info->num_tcams;
    		blks_per_dev = TR3_BLKS_PER_DEV_128;
            break;
        default:
            soc_cm_print("Bad device ID register:0x%08x. TCAM init failed\n", 
                          d2);
			return SOC_E_BADID;
        }
    } else {
        /* return error code */
        soc_cm_print("Failure reading NL11K DEV ID register\n");
        return rv;
    }
    ...
    if (tcam_base > partitions[TCAM_PARTITION_RAW].num_entries) {
        soc_cm_debug(DK_ERR, "tr3_tcam_init: unit %d number of entries "
                     "configured exceeds physical device size\n",
                     unit);
        return SOC_E_PARAM;
    }
        ...
        if (partitions[part].tcam_width_shift == 3) {
            ...
            for (i = 0; i < partitions[part].num_entries; i++) {
                SOC_IF_ERROR_RETURN(soc_tr3_tcam_write_entry(unit, 
                                         TCAM_PARTITION_RAW, addr, mask, data));
                SOC_IF_ERROR_RETURN(soc_tr3_tcam_write_entry(unit, 
                                        TCAM_PARTITION_RAW, addr+1, mask, data));
                addr += 8;
    ...
    for (dev = 0; dev < tcam_info->num_tcams; dev++) {
        ...
        if (tcam_base) {
            ...
            ltr->key_info[1].seg_len[1] = 8;
            ltr->key_info[1].seg_offset[1] = 14; 
            ltr->key_info[1].seg_len[2] = 8;
            ltr->key_info[1].seg_offset[2] = 23; 
            ltr->key_info[1].seg_len[3] = 5;
            ltr->key_info[1].seg_offset[3] = 42; 
            ltr->key_info[1].seg_len[4] = 1;
            ltr->key_info[1].seg_offset[4] = 50; 
            ...
            /* segment length greater than 16 bytes has to be broken up into
             * pieces each less than 16 bytes */
            ltr->key_info[0].seg_len[1] = 8;
            ltr->key_info[0].seg_offset[1] = 14; 
            ltr->key_info[0].seg_len[2] = 8;
            ltr->key_info[0].seg_offset[2] = 23; 
           ...
           /* IP4C L2 FWD */
            ...
            ltr->key_info[0].seg_offset[1] = 12; 
            ltr->key_info[0].seg_len[2] = 5;
            ltr->key_info[0].seg_offset[2] = 26; 
            ltr->key_info[0].seg_len[3] = 5;
           ...
           /* ACLIP6S L2 fwd ACL SA */
            ...
            ltr->key_info[0].seg_offset[2] = 32; 
            ltr->key_info[0].seg_len[3] = 5;
            ltr->key_info[0].seg_offset[3] = 46; 
            ltr->key_info[0].seg_len[4] = 5;
            ltr->key_info[0].seg_offset[4] = 54; 
    ...
    soc_cm_print("TR3 tcam init done\n");
    return SOC_E_NONE;
}
#endif /* BCM_TRIUMPH3_SUPPORT */
