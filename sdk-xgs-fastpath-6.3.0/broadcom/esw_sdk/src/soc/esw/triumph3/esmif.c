...
nl_mdio_prbs_chk_err(int unit, uint16 mdio_portid, uint16 crx)
{
    ...
    for(mdio_devid = mdio_devid_min; mdio_devid <= mdio_devid_max; ++mdio_devid) {
        ...
        if(err_count_l0 || err_count_l1 || err_count_l2 || err_count_l2) {
            SOC_DEBUG_PRINT((DK_WARN, "PRBS error count at mdio_portid %d mdio_devid %d: \n",mdio_portid, mdio_devid));
          SOC_DEBUG_PRINT((DK_WARN, "\t->lane0=%d\n",err_count_l0)); 
          SOC_DEBUG_PRINT((DK_WARN, "\t->lane1=%d\n",err_count_l1)); 
          SOC_DEBUG_PRINT((DK_WARN, "\t->lane2=%d\n",err_count_l2)); 
          SOC_DEBUG_PRINT((DK_WARN, "\t->lane3=%d\n",err_count_l3));
          prbs_pass = 0;    
        }
    }
    if(prbs_pass) {
...
chk_etu_global_intr_sts(int unit, uint32 intr_mask, unsigned num_nl)
{
    ...
    if ((~soc_reg_field_get(unit, ETU_GLOBAL_INTR_STSr, intr_mask, RX_CW_ERRf)
        ...
        */
        ...
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, ETU_RX_CW_ERR_INFO2r,   0, 0, &dummy));
...

{
...

    {
        ...
        for (wcl_num = 0; wcl_num < NUM_WCLS; ++wcl_num) {
            SOC_IF_ERROR_RETURN(soc_reg32_get(unit, WCL_CTLr, 0, wcl_num, &wcl_ctl_buf));
            soc_reg_field_set(unit, WCL_CTLr, &wcl_ctl_buf, TXD1G_FIFO_RSTBf, 0xf);
            SOC_IF_ERROR_RETURN(soc_reg32_set(unit, WCL_CTLr, 0, wcl_num, wcl_ctl_buf));
        }
    }
                          ...
                          unsigned *fme_cost210) 
{
    ...
    if (calc_optional_cost) {
        strcpy(proc_name, "esm_calc_new_OPTIONAL_fme_cost210");
    } else {
        strcpy(proc_name, "esm_calc_new_fme_cost210");
    }
...
init3(int unit, unsigned num_nl)
{
    ...
    idle_req_rsp_latency_ns = soc_property_get(unit, "idle_req_rsp_latency_ns", 0);
...
{
    const unsigned max_init1_attempts = 2,   /* <HP> Check this </HP> */
        en_nl_parity_scan  = 1,
