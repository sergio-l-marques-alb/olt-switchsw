...
_bcm_tr2_wlan_reinit(int unit)
{
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        tnl_idx = soc_EGR_WLAN_DVPm_field32_get(unit, &egr_wlan_dvp, 
                                                TUNNEL_INDEXf);
        sal_memset(tnl_entry, 0, SOC_MAX_MEM_FIELD_WORDS);
                       ...
                       uint32 value)
{
    int rv = BCM_E_NONE;
    WLAN_LOCK(unit);
    rv = _bcm_tr2_lport_field_set(unit, wlan_port_id, field, value);
    WLAN_UNLOCK(unit);
    return rv;
}
...
bcm_tr2_wlan_port_untagged_vlan_set(int unit, bcm_gport_t port, bcm_vlan_t vid)
{
    ...
    rv = _bcm_tr2_lport_field_get(unit, port, TAG_ACTION_PROFILE_PTRf, 
                                  &old_profile);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    ...
    rv = _bcm_tr2_lport_field_get(unit, port, PORT_PRIf, &(action.priority));
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
    ...
    rv = _bcm_tr2_lport_field_set(unit, port, PORT_VIDf, vid);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
    /* Point to the new profile */
    rv = _bcm_tr2_lport_field_set(unit, port, TAG_ACTION_PROFILE_PTRf, profile);
    if (rv < 0) {
        WLAN_UNLOCK(unit);
        return rv;
    }
                                    ...
                                    bcm_vlan_t *vid_ptr)
{
    ...
    rv = _bcm_tr2_lport_field_get(unit, port, PORT_VIDf, &value);
...
{
    int value, rv = BCM_E_NONE;
    rv = _bcm_tr2_lport_field_get(unit, port, PORT_PRIf, &value);
    *prio_ptr = value;
    return rv;
}
