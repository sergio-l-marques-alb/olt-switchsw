/*
 * $Id: subport.c 1.32.26.2 Broadcom SDK $
                             ...
                             bcm_gport_t *group)
{
    ...
    for (priority=0; priority < 8; priority++) {
         ...
         if (soc_feature(unit, soc_feature_subport_enhanced)) {
              /* Write the group VLAN ID into CLASS_ID field of EGR_L3_NEXT_HOP entry */
...
_bcm_tr2_subport_port_delete(int unit, int l3_idx, int vp)
{
    ...
    /* Delete the old vlan translate profile entry */
    rv = _bcm_trx_vlan_action_profile_entry_delete(unit, ing_profile_idx);
    /* Reason for commenting the error: 
       In bcm_tr2_subport_port_add() function, during addition of subports, if we pass same pkt_vlan, 
       it is accepting and creating the ing_profile_idx and also deletes the ing_profile_idx in the next step if it is already exists (with same VLAN).
       In such cases, it is getting failed (with ENTRY_NOT_FOUND error) in this function as it is already deleted in port_add function */
#if 0
    BCM_IF_ERROR_RETURN(rv);
#endif
