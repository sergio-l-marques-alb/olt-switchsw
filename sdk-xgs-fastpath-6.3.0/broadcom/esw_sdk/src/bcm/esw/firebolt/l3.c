/*
 * $Id: l3.c 1.817.2.2 Broadcom SDK $
...
_bcm_esw_l3_warmboot_recover(int unit, int32 *egrmode, int32 *ingrmode,
        int32 *hostmode, int32 *intfMapmode)
{
    /*
...
_bcm_tunnel_initiator_reinit(int unit) {
    ...
    for (i = idx_min; i <= idx_max; i++) {
        ...
        if (soc_EGR_IP_TUNNELm_field32_get(unit, &egr_tnl_entry, ENTRY_TYPEf) == 0) {
...
bcm_xgs3_l3_fbx_defip_init(int unit)
{
    ...
    if (SOC_IS_TRIDENT2(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_td2_l3_defip_init(unit)); 
    } else 
#endif /* BCM_TRIDENT2_SUPPORT*/
#if defined(BCM_TRIUMPH_SUPPORT) 
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_TRIDENT2(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_td2_l3_defip_deinit(unit));
    } else
#endif /* BCM_TRIDENT2_SUPPORT */
#if defined(BCM_TRIUMPH_SUPPORT) 
    if (SOC_IS_TR_VL(unit)) {
...
_bcm_xgs3_l3_ecmp_group_init(int unit)
{
    ...
    /* 
     *  NOTE: This is not required for devices that support
     *  dynamic ecmp group sizes (BCM5662x and later).
     *-----------------------------------------------------------------
    ...
    if (!soc_feature(unit, soc_feature_l3_dynamic_ecmp_group)) {
        ...
        if (BCM_XGS3_L3_HWCALL_CHECK(unit, max_grp_size_set)) {
            rv = BCM_XGS3_L3_HWCALL_EXEC(unit, max_grp_size_set)
                                              (unit, max_grp_sz);
            BCM_IF_ERROR_RETURN(rv);
        }
    }
...
bcm_xgs3_nh_get(int unit, int nh_idx, bcm_l3_egress_t *nh_info)
{
    ...
    if ( soc_feature(unit, soc_feature_l2gre)) {
         ...
         BCM_IF_ERROR_RETURN(rv);
         if(nh_info->flags & BCM_L3_L2GRE_ONLY) {
              return rv;
         }
    }
#endif /* BCM_TRIUMPH3_SUPPORT */
    ...
    if ( soc_feature(unit, soc_feature_vxlan)) {
         ...
         BCM_IF_ERROR_RETURN(rv);
         if(nh_info->flags & BCM_L3_VXLAN_ONLY) {
              return rv;
         }
    }
#endif /* BCM_TRIDENT2_SUPPORT */
...
void
bcm_xgs3_trunk_nh_store(int unit, bcm_port_t port, int index)
{
...
uint32 protection_flags=0;
   ...
   /* Store api flags within egr object flags */
...
#endif
#ifdef BCM_TRIUMPH2_SUPPORT
    if (soc_feature(unit, soc_feature_virtual_switching) && egr->encap_id > 0) {
        *intf = BCM_XGS3_DVP_EGRESS_IDX_MIN + index;
    } else
#endif
    {
    ...
    if ( soc_feature(unit, soc_feature_l2gre) && (egr->flags & BCM_L3_L2GRE_ONLY)){
         ...
         L3_IF_ERROR_CLEANUP_ELSE_RETURN(bcm_tr3_l2gre_egress_set(unit, index));
    }
#endif /* BCM_TRIUMPH3_SUPPORT */
    ...
    if ( soc_feature(unit, soc_feature_vxlan) && (egr->flags & BCM_L3_VXLAN_ONLY)){
         ...
         L3_IF_ERROR_CLEANUP_ELSE_RETURN(bcm_td2_vxlan_egress_set(unit, index));
    }
#endif /* BCM_TRIDENT2_SUPPORT */
                        ...
                        bcm_if_t *intf)
{
    ...
    if (soc_feature(unit, soc_feature_virtual_port_routing)) {
        if (soc_EGR_L3_NEXT_HOPm_field32_get(unit, &next_hop, L3__DVP_VALIDf)) {
            vp = soc_EGR_L3_NEXT_HOPm_field32_get(unit, &next_hop, L3__DVPf);
...
_bcm_xgs3_l3_tnl_term_get(int unit, bcm_tunnel_terminator_t *tnl_info)
{
    ...
    if (SOC_IS_TRIUMPH2(unit) || SOC_IS_APOLLO(unit) || 
            SOC_IS_VALKYRIE2(unit) || SOC_IS_TRIDENT(unit) ||
            SOC_IS_KATANA(unit) || SOC_IS_TRIDENT2(unit)) {
        BCM_IF_ERROR_RETURN
                         ...
                         uint32 *egr_entry_ptr, bcm_l3_egress_t *nh_entry)
{
    ...
    if (soc_feature(unit, soc_feature_virtual_port_routing)) {
        if (soc_mem_field32_get(unit, mem, hw_entry_p, L3__DVP_VALIDf)) {
            vp = soc_mem_field32_get(unit, mem, hw_entry_p, L3__DVPf);
            if (_bcm_vp_used_get(unit, vp, _bcmVpTypeNiv)) {
                nh_entry->encap_id = vp;
    ...
    if (SOC_MEM_FIELD_VALID(unit, mem, ENTRY_TYPEf)) {
         ...
         if (entry_type == 0x4) { /* == WLAN PROFILE */
            int vp = 0;
            vp = soc_mem_field32_get(unit, mem, hw_entry_p, WLAN__DVPf);
            BCM_GPORT_WLAN_PORT_ID_SET(nh_entry->port, vp);
         }
...
#if 0
                BCM_IF_ERROR_RETURN(_bcm_kt_cosq_gport_construct(unit, 
                                        nh_entry->module, nh_entry->port, 
                                                   queue_id, &nh_entry->port));
#else 
                nh_entry->port = (24 << 26 | nh_entry->port << 16 | queue_id);
...
_bcm_fb_l3_intf_vrf_unbind(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    sal_memset(&vlan_control, 0, sizeof(vlan_control));       
    ret_val = bcm_esw_vlan_control_vlan_get(unit, intf_info->l3i_vid, &vlan_control);
    if ((BCM_E_NOT_FOUND == ret_val) || (BCM_E_PARAM == ret_val) || \
        (BCM_E_UNAVAIL == ret_val)) {
    ...
    vlan_control.vrf = BCM_L3_VRF_DEFAULT;
    ret_val = bcm_esw_vlan_control_vlan_set(unit, intf_info->l3i_vid, vlan_control);
...
_bcm_fb_l3_intf_nat_realm_id_set(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    vlan_control.nat_realm_id = intf_info->l3i_nat_realm_id;
    ret_val = bcm_esw_vlan_control_vlan_set(unit, intf_info->l3i_vid, 
                                            vlan_control);
    return (ret_val);
}
...
_bcm_fb_l3_intf_nat_realm_id_get(int unit, _bcm_l3_intf_cfg_t *intf_info)
{
...

    {
        ret_val = bcm_esw_vlan_control_vlan_get(unit, intf_info->l3i_vid, 
                                            &vlan_control);
        nat_realm_id = vlan_control.nat_realm_id;
    }
...
_bcm_fb_nh_add(int unit, int idx, void *buf, int max_paths)
{
    ...
    if (soc_feature(unit, soc_feature_virtual_port_routing) &&
        ...
        if (_bcm_vp_used_get(unit, nh_entry->encap_id, _bcmVpTypeNiv)) {
            ...
            soc_mem_field32_set(unit, mem, &eg_entry,
                             L3__VNTAG_DST_VIFf, niv_port.virtual_interface_id);
            soc_mem_field32_set(unit, mem, &eg_entry,
                                L3__DVPf, nh_entry->encap_id);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__DVP_VALIDf, TRUE);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__VNTAG_Pf,
                           (niv_port.flags & BCM_NIV_PORT_MULTICAST) ? 1 : 0);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__VNTAG_ACTIONSf, 0x1);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__VNTAG_FORCE_Lf, 0x0);
            ...
            soc_mem_field32_set(unit, mem, &eg_entry,
                             L3__VNTAG_DST_VIFf, ep.extended_port_vid);
            if (ep.pcp_de_select == BCM_EXTENDER_PCP_DE_SELECT_DEFAULT) {
                ...
                soc_mem_field32_set(unit, mem, &eg_entry, L3__ETAG_DOT1P_MAPPING_PTRf, 0);
            ...
            soc_mem_field32_set(unit, mem, &eg_entry,
                                L3__DVPf, nh_entry->encap_id);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__DVP_VALIDf, TRUE);
            soc_mem_field32_set(unit, mem, &eg_entry, L3__VNTAG_ACTIONSf, 0x2);
    ...
    if (soc_feature(unit, soc_feature_wlan) &&
        nh_entry->encap_id > 0) {
        if (_bcm_vp_used_get(unit, nh_entry->encap_id, _bcmVpTypeWlan)) {
...
_bcm_fbx_lpm_get(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    switch (mem) {
      ...
      case EXT_IPV4_DEFIPm:
        ...
        case L3_DEFIP_128m:
#if defined(BCM_KATANA_SUPPORT)
            if (SOC_IS_KATANAX(unit) && (kt_depth128 > 0)) {
...
_bcm_xgs3_l3_hw_call_init(int unit)
{
    ...
    defined(BCM_HAWKEYE_SUPPORT)
        ...
        td2_hw_call.lpm_get       = _bcm_td2_l3_lpm_get;
        td2_hw_call.lpm_add       = _bcm_td2_l3_lpm_add;
        td2_hw_call.lpm_del       = _bcm_td2_l3_lpm_del;
        td2_hw_call.lpm_update_match  = _bcm_td2_l3_lpm_update_match;
