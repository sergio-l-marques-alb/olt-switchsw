    ...
    bcm_l3_nat_id_t nat_id)
{
    ...
    if (soc_feature(unit, soc_feature_nat)) {
        ...
        /* Decrement refcount */
        if (BCM_L3_NAT_EGRESS_GET_REF_COUNT(unit, nat_id) > 2) {
            /* if ref count > 1, return ok */
...
#if defined(__INCLUDE_l3_egress_nat_SUPPORT)
    /* Validate L3 EGRESS NAT Info */

    /* Call bcm_l3_egress_nat_get_info(unit, info) to return the unique index */
#endif
                                 ...
                                 bcm_l3_nat_ingress_t *nat_info)
{
    ...
    case L3_ENTRY_IPV4_MULTICASTm:
        ...
        soc_L3_ENTRY_IPV4_MULTICASTm_field32_set(unit, hw_buf, 
                                                NAT__DST_DISCARDf, 
                              nat_info->flags & BCM_L3_NAT_INGRESS_DST_DISCARD);
