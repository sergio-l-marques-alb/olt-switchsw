...
/* Function:
 *      _bcm_esw_l2gre_multicast_leaf_entry_check
 * Purpose:
 *      Get  L2GRE Tunnel terminator Info
 * Parameters:
 *      unit     - Device Number
...
int
_bcm_esw_l2gre_multicast_leaf_entry_check(int unit,  bcm_ip_t mc_ip_addr, int flag)
{
    int rv = BCM_E_NONE;
    ...
    if (soc_feature(unit, soc_feature_l2gre)) {
         rv = bcm_tr3_l2gre_lock(unit);
         if ( rv == BCM_E_NONE ) {
              rv = bcm_tr3_l2gre_multicast_leaf_entry_check(unit, mc_ip_addr, flag);
              bcm_tr3_l2gre_unlock (unit);
         }
    }
#endif
 ...
 *
 */
 bcm_error_t   bcm_esw_l2gre_stat_counter_get(
                            int                 unit, 
                            bcm_gport_t         port,
                            bcm_vpn_t           vpn,
