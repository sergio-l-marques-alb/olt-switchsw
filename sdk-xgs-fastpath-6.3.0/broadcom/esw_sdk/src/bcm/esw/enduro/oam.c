/*
 * $Id: oam.c 1.141.8.1 Broadcom SDK $
 * 
    ...
    while (1) {
        if (oam_info_p->event_thread_id == NULL) {
            ...
            if (sal_time_usecs() < timeout){
                ...
                /*Timeout*/
                soc_cm_debug(DK_ERR, "BHH event thread not exited.\n");
                BCM_IF_ERROR_RETURN(bcm_en_oam_detach(unit));
                return BCM_E_INTERNAL;
            }
        }
...
_bcm_en_oam_bhh_hw_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_common_rx_queue_channel_set(unit, oam_info_p->cpu_cosq, chan_id));
...
/*		    soc_cm_print("%s: ****** OAM BHH State ******\n", __func__);*/
    ...
    soc_cm_debug(DK_VERBOSE, "BHH callback thread exiting\n");
    oam_info_p->event_thread_kill = 0;
    ...
    int *num_lm, int *mdl_index)
{
    ...
    *created_endpoint_p = endpoint_p;
	...
	if (is_remote)
	{
	    ...
	    if (replace && endpoint_p->is_remote) {
		if (vp != 0) {
		    _bcm_en_oam_make_rmep_key(unit, &l3_key, endpoint_info->name,
					      endpoint_info->level, 0, vp);
		} else {
		    _bcm_en_oam_make_rmep_key(unit, &l3_key, endpoint_info->name,
					      endpoint_info->level, endpoint_info->vlan, src_glp);
		}
