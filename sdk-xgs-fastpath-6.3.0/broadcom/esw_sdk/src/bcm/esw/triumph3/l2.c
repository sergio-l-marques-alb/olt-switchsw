...
#define BCM_MAC_LIMIT_MAX(_u_) (soc_feature(_u_, soc_feature_esm_support))       ?  \
                                ((soc_mem_index_count(_u_, L2_ENTRY_1m) - 1)     +  \
                                 (soc_mem_index_count(_u_, EXT_L2_ENTRY_1m) - 1) +  \
                                 (soc_mem_index_count(_u_, EXT_L2_ENTRY_2m) - 1)):  \
                                 (soc_mem_index_count(_u_, L2_ENTRY_1m) - 1)
                ...
                if (_BCM_TR3_L2_2_FIELD32_MEMACC_GET(unit, l2_entry_1, KEY_TYPE) ==
                    ...
                    if (_bcm_vfi_used_get(unit, vfi, _bcmVfiTypeMpls)) {
                         _BCM_MULTICAST_GROUP_SET(l2addr->l2mc_index,
                                _BCM_MULTICAST_TYPE_VPLS, l2addr->l2mc_index);
                    } else
#endif /* INCLUDE_L3 */
                    {
                         _BCM_MULTICAST_GROUP_SET(l2addr->l2mc_index,
                                _BCM_MULTICAST_TYPE_MIM, l2addr->l2mc_index);
                    }
                }
                isGport = 0;
            } else {
                           ...
                           int limit)
{
    ...
    if (limit < 0) {
        ...
        init_entry = soc_mem_entry_null(unit,mem);
                    ...
                    bcm_l2_addr_t *l2addr)
{
    ...
    if ((rval) && BCM_MAC_IS_MCAST(l2addr->mac) &&
                  !(_BCM_MULTICAST_IS_SET(l2addr->l2mc_index))) {
...
#if 0
                    int vpg;
                    vpg = soc_L2_ENTRY_ONLYm_field32_get(unit, l2x_entry, );
