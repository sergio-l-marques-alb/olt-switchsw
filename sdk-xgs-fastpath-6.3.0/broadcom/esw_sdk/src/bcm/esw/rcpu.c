                         ...
                         int *hdr_size, int *blk_ofst, int *byte_ofst)
{
    ...
    *hdr_size = hdr_buf - buf_start;
    ...
    soc_cm_debug(DK_TX, "_tx_rcpu_higig_hdr_setup: flags 0x%08x\n", pkt->flags);
...
_bcm_rcpu_tx(int unit, bcm_pkt_t *pkt, void * cookie)
{
    ...
    if ((BCM_PKT_NO_VLAN_TAG(pkt) && rcpu_pkt_len < 60) ||
        ...
        soc_cm_debug(DK_ERR, "_bcm_rcpu_tx: Discarding %s runt packet %s higig header %d\n", 
    ...
    if (soc_cm_debug_check(DK_TX)) {
    	int j;
        soc_cm_debug(DK_TX, "_bcm_rcpu_tx: packet: ");
    	for (j = 0; j < payload_len + sizeof(rcpu1_cmic_pkt_hdr_t); j++) {
            soc_cm_debug(DK_TX, "%.2x", tx_buf[j]);
    	}
    	soc_cm_debug(DK_TX, "\n");
    }
