/*
 * $Id: telecom.c 1.16 Broadcom SDK $
 * 
        ...
        if (telecom_g8265_verbose_level) {                                 \
            ...
            bcm_ptp_telecom_g8265_pktmaster_t *lcl_gm =                    \
                &telecom_fifo.events[telecom_fifo.head].pktmaster;         \
                                                                           \
            soc_cm_print("Telecom Event: %s @ %u\n",                       \
    ...
    void *unused)
{
    ...
    if (telecom.network_option == bcm_ptp_telecom_g8265_network_option_disable) {
         ...
         */
        if (BCM_FAILURE(rv = bcm_esw_ptp_telecom_g8265_network_option_set(unit, ptp_id, clock_num,
                bcm_ptp_telecom_g8265_network_option_disable))) {
            PTP_ERROR_FUNC("bcm_esw_ptp_telecom_g8265_network_option_set()");
        }
        return;
    }
    ...
    if (BCM_FAILURE(rv = bcm_esw_ptp_foreign_master_dataset_get(unit, ptp_id,
            clock_num, PTP_CLOCK_PORT_NUMBER_DEFAULT, &FMds))) {
        PTP_ERROR_FUNC("bcm_esw_ptp_foreign_master_dataset_get()");
        dpc_recall_usec = PTP_TELECOM_CONTROLLER_DPC_TIME_USEC_IDLE;
        goto telecom_dpc_recall;
    }
    ...
    uint8 flags)
{
    ...
    case bcmPTPUDPIPv4:
        soc_cm_print("Master IP Address (IPv4): %d.%d.%d.%d\n",
        ...
        break;
    case bcmPTPUDPIPv6:
        soc_cm_print("Master IP Address (IPv6): %02x%02x:%02x%02x:%02x%02x:"
        ...
        break;
    default:
        soc_cm_print("Master IP Address:\n");
    }
