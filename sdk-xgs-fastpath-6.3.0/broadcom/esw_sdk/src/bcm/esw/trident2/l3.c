...
/* L3 LPM state */
_bcm_td2_l3_defip_pair128_table_t *l3_td2_defip_pair128[BCM_MAX_NUM_UNITS];
 ...
 */
STATIC int
_td2_defip_pair128_hash(int unit, _bcm_defip_cfg_t *lpm_cfg, uint8 *hash) 
{
    uint32 key[BCM_TD2_DEFIP_PAIR128_HASH_SZ];
    ...
    *hash = (_shr_crc16b(0, (void *)key,
                         WORDS2BITS((BCM_TD2_DEFIP_PAIR128_HASH_SZ))) & 0xff);
    return (BCM_E_NONE);
}
 ...
 */
STATIC int 
_td2_defip_pair128_reinit(int unit, int idx, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    BCM_IF_ERROR_RETURN(_td2_defip_pair128_hash(unit, lpm_cfg, &hash));

    /* Set software structure info. */
    BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, idx, lkup_plen, hash);

    /* Update software usage counter. */
    BCM_TD2_DEFIP_PAIR128_USED_COUNT(unit)++;
 ...
 */
STATIC int 
_bcm_td2_defip_pair128_deinit(int unit)
{
    if (BCM_TD2_DEFIP_PAIR128(unit) == NULL) {
        return (BCM_E_NONE);
    }

    /* De-allocate any existing structures. */
    if (BCM_TD2_DEFIP_PAIR128_ARR(unit) != NULL) {
        sal_free(BCM_TD2_DEFIP_PAIR128_ARR(unit));
        BCM_TD2_DEFIP_PAIR128_ARR(unit) = NULL;
    }
    sal_free(BCM_TD2_DEFIP_PAIR128(unit));
    BCM_TD2_DEFIP_PAIR128(unit) = NULL;
 ...
 */
STATIC int
_bcm_td2_defip_pair128_init(int unit)
{
    int rv;
    int mem_sz;   
    int defip_config;
    ...
    /* De-allocate any existing structures. */
    if (BCM_TD2_DEFIP_PAIR128(unit) != NULL) {
        rv = _bcm_td2_defip_pair128_deinit(unit);
        BCM_IF_ERROR_RETURN(rv);
    }

    /* Allocate cam control structure. */
    mem_sz = sizeof(_bcm_td2_l3_defip_pair128_table_t);
    BCM_TD2_DEFIP_PAIR128(unit) = sal_alloc(mem_sz, "td2_l3_defip_pair128");
    if (BCM_TD2_DEFIP_PAIR128(unit) == NULL) {
        return (BCM_E_MEMORY);
    }

    /* Reset cam control structure. */
    sal_memset(BCM_TD2_DEFIP_PAIR128(unit), 0, mem_sz);

    defip_config = soc_property_get(unit, spn_IPV6_LPM_128B_ENABLE, 1);
    if (defip_config) {
        ipv6_128_depth      = 2 * _BCM_TD2_DEFIP_TCAM_DEPTH;
    } else { /* tcams are not paired - no ipv6/65-ipv6/128 routes */
        ipv6_128_depth      = 0;
    }

    BCM_TD2_DEFIP_PAIR128_TOTAL(unit) = ipv6_128_depth;
    BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit) = 0;
    BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit) = ipv6_128_depth - 1;
    mem_sz = ipv6_128_depth * sizeof(_bcm_td2_l3_defip_pair128_entry_t);
    BCM_TD2_DEFIP_PAIR128_ARR(unit) = 
                          sal_alloc(mem_sz, "td2_l3_defip_pair128_entry_array");
    if (BCM_TD2_DEFIP_PAIR128_ARR(unit) == NULL) {
        BCM_IF_ERROR_RETURN(_bcm_td2_defip_pair128_deinit(unit));
        return (BCM_E_MEMORY);
    }
    sal_memset(BCM_TD2_DEFIP_PAIR128_ARR(unit), 0, mem_sz);
 ...
 */
STATIC int 
_td2_defip_pair128_get_key(int unit, uint32 *hw_entry, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    /* Extract ip6 address. */
    _td2_defip_pair128_ip6_addr_get(unit,mem, hw_entry, lpm_cfg->defip_ip6_addr);
 ...
 */
STATIC int 
_td2_defip_pair128_match(int unit, _bcm_defip_cfg_t *lpm_cfg, 
                         uint32 *hw_entry, int *hw_index)
{
    ...
    for (index = 0; index <= BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit); index++) {

        /* Route prefix length comparison. */
        if (lkup_plen != BCM_TD2_DEFIP_PAIR128_ARR(unit)[index].prefix_len) {
            continue;
        }

        /* Route lookup key hash comparison. */
        if (hash != BCM_TD2_DEFIP_PAIR128_ARR(unit)[index].entry_hash) {
            continue;
        }
        ...
        rv =  _td2_defip_pair128_get_key(unit, hw_entry, &candidate);
        if (BCM_FAILURE(rv)) {
            break;
        }
    ...
    if (index > BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit))  {
        return (BCM_E_NOT_FOUND);
    }
...
/*
 * Function:
 *      _td2_defip_pair128_entry_clear
 * Purpose:
 ...
 */
STATIC int 
_td2_defip_pair128_entry_clear(int unit, int hw_index)  
{
    ...
    /* Reset sw entry. */
    BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, hw_index, 0, 0);

    /* Reset urpf source lookup entry. */
    if (SOC_URPF_STATUS_GET(unit)) {
        BCM_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                            (hw_index + BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit)),
                            &soc_mem_entry_null(unit, L3_DEFIP_PAIR_128m)));
        BCM_IF_ERROR_RETURN
            (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                 (hw_index+ BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit)), hw_entry));
    }
    return (BCM_E_NONE);
}
 ...
 */
STATIC INLINE int 
_td2_defip_pair128_parse(int unit, uint32 *hw_entry,
 ...
 */
STATIC int 
_td2_defip_pair128_shift(int unit, int idx_src, int idx_dest)  
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS]; /* HW entry buffer. */

    /* Don't copy uninstalled entries. */
    if (0 != BCM_TD2_DEFIP_PAIR128_ARR(unit)[idx_src].prefix_len) {
        ...
        if (SOC_URPF_STATUS_GET(unit)) {
            ...
            BCM_IF_ERROR_RETURN
                (BCM_XGS3_MEM_READ(unit, L3_DEFIP_PAIR_128m, 
                              idx_src + BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit),
                              hw_entry));

            /* Write entry to the destination index + half tcam. */
            BCM_IF_ERROR_RETURN
                (BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                            idx_dest +  BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit),
                            hw_entry));
        }
    }

    /* Copy software portion of the entry. */ 
    sal_memcpy (&BCM_TD2_DEFIP_PAIR128_ARR(unit)[idx_dest],
                &BCM_TD2_DEFIP_PAIR128_ARR(unit)[idx_src], 
                sizeof(_bcm_td2_l3_defip_pair128_entry_t));
...
/*
 * Function:
 *      _td2_defip_pair128_shift_range
 * Purpose:
 *      Shift entries in the tcam. 
 * Parameters:
 *      unit        - (IN)SOC unit number.
 ...
 */
STATIC int 
_td2_defip_pair128_shift_range(int unit, int idx_start, int idx_end)  
{
    int idx;     /* Iteration index. */
    ...
    if (idx_end > idx_start) {
        for (idx = idx_end - 1; idx >= idx_start; idx--) {
            rv = _td2_defip_pair128_shift(unit, idx, idx + 1);  
            if (BCM_FAILURE(rv)) { 
                break;
            }
        }
        /* Delete last entry from hardware & software. */
        rv = _td2_defip_pair128_entry_clear(unit, idx_start);
    }
 ...
 */
STATIC int 
_td2_defip_pair128_idx_alloc(int unit, _bcm_defip_cfg_t *lpm_cfg, int *hw_index,
                             int nh_ecmp_idx)
{
    int  lkup_plen;                   /* Vrf weighted lookup prefix.    */
    uint8 hash;                       /* Entry hash value.              */
    ...
    int  range_end = BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit);
    ...
    if (BCM_TD2_DEFIP_PAIR128_USED_COUNT(unit) ==
                      BCM_TD2_DEFIP_PAIR128_TOTAL(unit)) {
        return (BCM_E_FULL);
    }
     ...
     */ 
    for (idx = 0; idx <= BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit); idx++) {
        /* Remember free entry before installation location. */
        if (0 == BCM_TD2_DEFIP_PAIR128_ARR(unit)[idx].prefix_len) {
            free_idx_before = idx;
            continue;
        }
        /* Route prefix length comparison. */
        if (lkup_plen < BCM_TD2_DEFIP_PAIR128_ARR(unit)[idx].prefix_len) {
            range_start = idx;
            continue;
        }
        break;
     ...
     */ 
    for (idx = BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit); idx >= 0; idx--) {
    ...
    if (range_start == BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit)) {
        /* Shift all entries from free_idx_before till the end 1 up. */   
        rv = _td2_defip_pair128_shift_range(unit, BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit), free_idx_before);

        if (BCM_SUCCESS(rv)) { 
            /* Allocated index is last TCAM entry. */
            *hw_index = BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit);
            BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, *hw_index, lkup_plen, hash);
        }
        return (rv);
    }
    ...
    if (range_end < 0) {
        ...
        rv = _td2_defip_pair128_shift_range(unit, 0, free_idx_after);

        if (BCM_SUCCESS(rv)) {
            /* Allocated index is first TCAM entry. */
            *hw_index = 0;
            BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, 0, lkup_plen, hash);
        }
        return (rv);
    }
    ...
    if (shift_min <= 0) {
        rv = _td2_defip_pair128_shift_range(unit, range_start, free_idx_before);
        if (BCM_SUCCESS(rv)) {
            *hw_index= range_start;
            BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, range_start, lkup_plen, hash);
        }
    } else {
        rv = _td2_defip_pair128_shift_range(unit, range_end, free_idx_after);
        if (BCM_SUCCESS(rv)) {
            *hw_index = range_end;
            BCM_TD2_DEFIP_PAIR128_ENTRY_SET(unit, range_end, lkup_plen, hash);
        }
    }
    return (rv);
}
 ...
 */
STATIC int 
_bcm_td2_l3_defip_pair128_get(int unit, _bcm_defip_cfg_t *lpm_cfg, 
                              int *nh_ecmp_idx) 
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS];/* Hw entry buffer.          */
    bcm_ip6_t mask;                          /* Subnet mask.              */
    ...
    rv =  _td2_defip_pair128_match(unit, lpm_cfg, hw_entry, &hw_index);
    BCM_IF_ERROR_RETURN(rv);

    lpm_cfg->defip_index = hw_index;

    /* Parse matched entry. */
    rv = _td2_defip_pair128_parse(unit, hw_entry, lpm_cfg, nh_ecmp_idx);
    BCM_IF_ERROR_RETURN(rv);
 ...
 */
STATIC int
_bcm_td2_l3_defip_pair128_add(int unit, _bcm_defip_cfg_t *lpm_cfg,
                              int nh_ecmp_idx) 
{
    ...
    if (BCM_XGS3_L3_INVALID_INDEX == lpm_cfg->defip_index) {
        rv = _td2_defip_pair128_idx_alloc(unit, lpm_cfg, &hw_index, nh_ecmp_idx);
        BCM_IF_ERROR_RETURN(rv);
    } else {
    ...
    _td2_defip_pair128_ip6_addr_set(unit, mem, hw_entry, lpm_cfg->defip_ip6_addr);

    /* Set ip mask. */
    _td2_defip_pair128_ip6_mask_set(unit, mem, hw_entry, mask);
    ...
    if (SOC_URPF_STATUS_GET(unit)) {
        ...
        rv = BCM_XGS3_MEM_WRITE(unit, L3_DEFIP_PAIR_128m, 
                                ...
                                hw_entry);
        if (BCM_FAILURE(rv)) {
            _td2_defip_pair128_entry_clear(unit, hw_index);
            return (rv);
        }
    }
 ...
 */
STATIC int 
_bcm_td2_l3_defip_pair128_del(int unit, _bcm_defip_cfg_t *lpm_cfg) 
{
    uint32 hw_entry[SOC_MAX_MEM_FIELD_WORDS];/* Hw entry buffer.          */
    ...
    rv = _td2_defip_pair128_entry_clear(unit, hw_index);
    if (BCM_SUCCESS(rv)) {
        ...
        /* Parse entry. */
        rv = _td2_defip_pair128_parse(unit, temp_hw_entry,
                                                &temp_lpm, &temp_nh_ecmp_idx);
        BCM_IF_ERROR_RETURN(rv);
        
        /* Extract entry address vrf */
        rv =  _td2_defip_pair128_get_key(unit, temp_hw_entry, &temp_lpm);
        BCM_IF_ERROR_RETURN(rv);

        /* Add the copied entry to L3_DEFIP_PAIR_128 table */
        temp_lpm.defip_index = BCM_XGS3_L3_INVALID_INDEX;
        rv = _bcm_td2_l3_defip_pair128_add(unit, &temp_lpm, temp_nh_ecmp_idx);
        BCM_IF_ERROR_RETURN(rv);
        ...
        rv = BCM_E_UNAVAIL;
        rv = _bcm_td2_l3_defip_pair128_del(unit, &temp_lpm);
        return rv;
    }
    return (rv);
}
...
/*
 * Function:
 *      _bcm_td2_l3_defip_pair128_update_match
 * Purpose:
 ...
 */
STATIC int
_bcm_td2_l3_defip_pair128_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    int idx;                                 /* Iteration index.           */
    int rv;                                  /* Operation return status.   */
    ...
    for (idx = 0; idx < defip_pair128_tbl_size; idx++) {
        ...
        /* Parse  the entry. */
        _td2_defip_pair128_parse(unit, hw_entry, &lpm_cfg, &nh_ecmp_idx);
       lpm_cfg.defip_index = idx;

        /* Fill entry ip address &  subnet mask. */
        _td2_defip_pair128_get_key(unit, hw_entry, &lpm_cfg);
        ...
        if (SOC_WARM_BOOT(unit)) {
            rv = _td2_defip_pair128_reinit(unit, idx, &lpm_cfg);
            if (BCM_FAILURE(rv)) {
                soc_cm_sfree(unit, lpm_tbl_ptr);
                return rv;
            }
 ...
 */
int
_bcm_td2_l3_defip_init(int unit)
{
    int defip_config;
    ...
    if (soc_property_get(unit, spn_L3_ALPM_ENABLE, 0)) {
        ...
        if (BCM_SUCCESS(rv)) {
            soc_cm_print("ALPM mode Initialized\n");
        } else {
            soc_cm_print("ALPM mode Failed, retVal = %d\n", rv);
        }
        return rv;
    }
#endif /* ALPM_ENABLE */
    ...
    switch (defip_config) {
      case 0:
         /* No 65-128 bit prefix IPv6 routes.
          * L3_DEFIP_KEY_SELr = 0 (reset value) */
        ipv6_64_depth      = 8 * _BCM_TD2_DEFIP_TCAM_DEPTH;
        break;
      case 1:
        /* Supports mixed IPv4, 64 bit prefix IPv6 and 128 bit prefix IPv6 routes.
         * upto 16K IPv4 routes only or
         * upto 8K IPv4 and upto 4K 64 bit prefix IPv6 routes only, or,
         * upto 2K 128 bit IPv6 and upto 4K 64 bit prefix IPv6 routes, or,
         * a combination of IPv4, 64/128 bit prefix IPv6 routes.
         * L3_DEFIP_KEY_SELr = 000000000011 */ 
        soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                           V6_KEY_SEL_CAM0_1f, 0x1);
        soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                           V6_KEY_SEL_CAM2_3f, 0x1);
        ipv6_64_depth      = 4 * _BCM_TD2_DEFIP_TCAM_DEPTH;
        break;
    ...
    BCM_IF_ERROR_RETURN(_bcm_td2_l3_defip_mem_get(unit, 0, 0, &mem_v4));
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_td2_l3_defip_mem_get(unit, BCM_L3_IP6, 0, &mem_v6));

    /* Get memory for IPv6 entries with prefix length > 64. */
    BCM_IF_ERROR_RETURN
        (_bcm_td2_l3_defip_mem_get(unit, BCM_L3_IP6,
                                  BCM_XGS3_L3_IPV6_PREFIX_LEN,
                                  &mem_v6_128));

    /* Initialize IPv6 entries lookup engine. */
    if (mem_v6 != mem_v6_128) {
        BCM_IF_ERROR_RETURN(_bcm_td2_defip_pair128_init(unit));
    }
 ...
 */
int
_bcm_td2_l3_lpm_get(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    switch (mem) {
      case L3_DEFIP_PAIR_128m:
        return _bcm_td2_l3_defip_pair128_get(unit, lpm_cfg, nh_ecmp_idx);
      default:
 ...
 */
int
_bcm_td2_l3_lpm_add(int unit, _bcm_defip_cfg_t *lpm_cfg, int nh_ecmp_idx)
{
    soc_mem_t mem = L3_DEFIPm;
    ...
    switch (mem) {
      case L3_DEFIP_PAIR_128m:
        return _bcm_td2_l3_defip_pair128_add(unit, lpm_cfg, nh_ecmp_idx);
      default:
 ...
 */
int
_bcm_td2_l3_lpm_del(int unit, _bcm_defip_cfg_t *lpm_cfg)
{
    soc_mem_t mem = L3_DEFIPm;
    ...
    switch (mem) {
      case L3_DEFIP_PAIR_128m:
        return _bcm_td2_l3_defip_pair128_del(unit, lpm_cfg);
      default: 
 ...
 */
int
_bcm_td2_l3_lpm_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    soc_mem_t mem = L3_DEFIPm;  /* Route table memory.      */

    BCM_IF_ERROR_RETURN(_bcm_td2_l3_defip_mem_get(unit, trv_data->flags, 0, &mem));

    switch (mem) {
      case L3_DEFIP_PAIR_128m:
        return _bcm_td2_l3_defip_pair128_update_match(unit, trv_data);
      default: 
        return _bcm_fb_lpm_update_match(unit, trv_data);
    }
 ...
 */
int
_bcm_td2_l3_defip_urpf_enable(int unit, int enable)
{
    int defip_config;
    uint32 defip_key_sel_val = 0;
    int ipv6_64_depth = 0;
    ...
    if (enable) { 
        ...
        switch (defip_config) {
          case 1:
            /* L3_DEFIP_KEY_SELr = 111100000101 */ 
            soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                                   V6_KEY_SEL_CAM0_1f, 0x1);
            soc_reg_field_set(unit, L3_DEFIP_KEY_SELr, &defip_key_sel_val, 
                                   V6_KEY_SEL_CAM4_5f, 0x1);
            ipv6_64_depth      = 2 * _BCM_TD2_DEFIP_TCAM_DEPTH;
            ipv6_128_depth     = _BCM_TD2_DEFIP_TCAM_DEPTH; 
            ipv6_128_dip_offset= _BCM_TD2_DEFIP_TCAM_DEPTH;
            break;
          case 0:
          default: 
            /* L3_DEFIP_KEY_SELr = 111100000000 */ 
            ipv6_64_depth      = 4 * _BCM_TD2_DEFIP_TCAM_DEPTH;
            break;
        }
    } else { /* Disable urpf */
        ...
        switch (defip_config) {
          case 1:
            ...
            break;   
          case 0: /* Default route table config - non URPF mode */
          default:
            ipv6_64_depth      = 8 * _BCM_TD2_DEFIP_TCAM_DEPTH;
            ipv6_128_dip_offset = 0;
            ipv6_128_depth      = 0;
    ...
    SOC_LPM_STATE_START(unit, (MAX_PFX_INDEX)) = - 1;
    SOC_LPM_STATE_END(unit, (MAX_PFX_INDEX))   = - 1;
    BCM_TD2_DEFIP_PAIR128_URPF_OFFSET(unit)    = ipv6_128_dip_offset;
    BCM_TD2_DEFIP_PAIR128_IDX_MAX(unit)        = ipv6_128_depth - 1;
    BCM_TD2_DEFIP_PAIR128_TOTAL(unit)          = ipv6_128_depth;
    BCM_IF_ERROR_RETURN(WRITE_L3_DEFIP_KEY_SELr(unit, defip_key_sel_val));
...
#if defined(__INCLUDE_L3_ROUTE_SUPPORT)
    /* Validate L3 ROUTE Info */

    /* Call bcm_l3_route_get_info(unit, info) to return the unique index */
#endif

    /* Ingress L3 ROUTE */
    table_info[*num_of_tables].table=L3_DEFIPm;
    table_info[*num_of_tables].index=index;
    table_info[*num_of_tables].direction=bcmStatFlexDirectionIngress;
    (*num_of_tables)++;

    table_info[*num_of_tables].table=L3_DEFIP_AUX_SCRATCHm;
    table_info[*num_of_tables].index=index;
    table_info[*num_of_tables].direction=bcmStatFlexDirectionIngress;
    (*num_of_tables)++;

    table_info[*num_of_tables].table=L3_DEFIP_ALPM_IPV4m;
    table_info[*num_of_tables].index=index;
    table_info[*num_of_tables].direction=bcmStatFlexDirectionIngress;
    (*num_of_tables)++;
...
_bcm_td2_l3_ip4_options_profile_scache_len_get(int unit, int *wb_alloc_size)
{
    ...
    *wb_alloc_size = _BCM_IP4_OPTIONS_LEN;
        ...
        if (stable_size == 0) { /* level 1 */
            ...
            for (idx = 0; idx < _BCM_IP4_OPTIONS_LEN; idx++) {
                ...
                if (hw_idx != _BCM_IP_OPTION_REINIT_INVALID_HW_IDX) {
                    ...
                    L3_INFO(unit)->ip4_options_bitmap[idx] = hw_idx;
                }
            }
                                ...
                                _bcm_defip_cfg_t *lpm_cfg)
{
    uint8 *ip6;                 /* Ip6 address.       */
    uint8 mask[BCM_IP6_ADDRLEN];        /* Subnet mask.       */
    uint32 ip6_word, i;            /* Temp storage.      */
...
_bcm_td2_alpm_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    int rv = BCM_E_NONE;        /* Operation return status.     */
    int idx_start = 0;
    int idx_end = 0, bkt_idx, bkt_count = 24, bkt_ptr, bkt_addr;
    ...
    soc_mem_t alpm_mem = L3_DEFIP_ALPM_IPV4m, pivot_mem = L3_DEFIPm;
    ...
    for (idx = idx_start; idx < idx_end; idx++) {
        ...
        /* Each lpm entry contains upto 48 IPV4 entries. Check all */           
