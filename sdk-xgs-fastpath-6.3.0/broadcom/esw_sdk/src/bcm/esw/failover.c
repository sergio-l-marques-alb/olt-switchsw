/*
 * $Id: failover.c 1.26 Broadcom SDK $
 *
...
int 
bcm_esw_failover_status_set(int unit, bcm_failover_element_t *failover, int enable)
{
    int rv = BCM_E_UNAVAIL;
#ifdef BCM_TRIUMPH3_SUPPORT
    if (soc_feature(unit, soc_feature_failover) && SOC_IS_TRIUMPH3(unit)) {
...
int 
bcm_esw_failover_status_get(int unit, bcm_failover_element_t *failover, int *value)
{
    int rv = BCM_E_UNAVAIL;
#ifdef BCM_TRIUMPH3_SUPPORT
    if (soc_feature(unit, soc_feature_failover) && SOC_IS_TRIUMPH3(unit)) {
...
int
_bcm_esw_failover_prot_nhi_create(int unit, uint32 flags, int nh_index, int prot_nh_index,
...
int
_bcm_esw_failover_prot_nhi_update (int unit, int old_nh_index, int new_nh_index)
...
#ifdef BCM_TRIUMPH3_SUPPORT
    if (soc_feature(unit, soc_feature_failover) && SOC_IS_TRIUMPH3(unit)) {
         rv = bcm_tr3_failover_prot_nhi_update (unit, old_nh_index, new_nh_index);
         return rv;
    }
#endif
...
#ifdef BCM_TRIUMPH2_SUPPORT
    if (soc_feature(unit, soc_feature_failover)) {
         rv = bcm_tr2_failover_prot_nhi_update (unit, old_nh_index, new_nh_index);
         return rv;
    }
#endif
    return rv;
...
int
bcm_esw_failover_ring_config_get(int unit, bcm_failover_ring_t *failover_ring)
{
    int rv = BCM_E_UNAVAIL;
...
int
bcm_esw_failover_ring_config_set(int unit, bcm_failover_ring_t *failover_ring)
{
    int rv = BCM_E_UNAVAIL;
