...
typedef int (*_bcm_bst_pre_sync_cb)(int unit);
typedef int (*_bcm_bst_post_sync_cb)(int unit);
...
typedef struct _bcm_bst_cmn_unit_info_s {
    ...
    _bcm_bst_resource_info_t        resource_tbl[_bcmResourceMaxCount];
...
#define _BCM_BST_THRESHOLD_INFO(pe,pmem,preg,pf)            \
...
#define _BCM_BST_THRESHOLD_INFO2(pe,pmem1,pmem2,preg1,preg2,pf)     \
...
#define _BCM_BST_RESOURCE_MIN       _bcmResourceDevice

#define _BCM_BST_RESOURCE_ITER(bi,res)              \
        for (res = _BCM_BST_RESOURCE_MIN;           \
             res < _bcmResourceMaxCount; res++)
...
#endif
#if defined(BCM_TRIUMPH3_SUPPORT)
STATIC int _bcm_bst_sync_hw_trigger(int unit, int res, int port, int index);
STATIC int _bcm_bst_sync_hw_snapshot(int unit, int res, int port, int index);
#endif

STATIC int _bcm_bst_cmn_profile_set_hw(int unit, int pipe, int hw_index, 
                                       _bcm_bst_resource_t res,
                                       bcm_cosq_bst_profile_t *profile, 
                                       uint32 *p_profile);
STATIC int _bcm_bst_cmn_io_op(int unit, _bcm_bst_op_t op, int hw_index, 
            _bcm_bst_resource_t res, soc_mem_t mem, 
...
_bcm_bst_triumph3_intr_to_resources(int unit, uint32 *flags)
{
    ...
    if (soc_reg64_field32_get(unit, THDI_BST_TRIGGER_STATUSr, 
        ...
        fval = soc_reg64_field32_get(unit, THDI_BST_TRIGGER_STATUSr, rval64, 
                                        TRIGGER_TYPEf);
        if (fval & 0x1) {
            res = _bcmResourcePriGroupShared;
        }

        if (fval & 0x2) {
            res = _bcmResourcePriGroupHeadroom;
        }
    ...
    if (soc_reg_field_get(unit, MISCCONFIGr, rval, BST_TRACKING_MODEf)) {
        _bcm_bst_sync_hw_trigger(unit, res, -1, index);
    } else {
        _bcm_bst_sync_hw_snapshot(unit, res, -1, index);
    }
...
_bcm_bst_triumph3_init(int unit)
{
    ...
    /* _bcmResourceDevice */
    pres = _BCM_BST_RESOURCE(unit, _bcmResourceDevice);
    ...
    _BCM_BST_THRESHOLD_INFO(pres, THDI_PORT_SP_CONFIGm, 
                            INVALIDr, BST_THRESHOLD_PROFILEf);
    ...
    pres = _BCM_BST_RESOURCE(unit, _bcmResourcePriGroupShared);
    pres->index_min = 0;
    ...
    pres = _BCM_BST_RESOURCE(unit, _bcmResourcePriGroupHeadroom);
    pres->index_min = 0;
    pres->index_max = (63 * 8) - 1;
    _BCM_BST_RESOURCE_FLAG_SET(pres, _BCM_BST_CMN_RES_F_PROFILEID);
    _BCM_BST_RESOURCE_PBMP_SET(pres, PBMP_ALL(unit));
    ...
    /* _bcmResourceMcast */
    pres = _BCM_BST_RESOURCE(unit, _bcmResourceMcast);
    pres->index_min = 0;
    pres->index_max = 47;
    _BCM_BST_RESOURCE_FLAG_SET(pres, _BCM_BST_CMN_RES_F_PROFILEID);
    _BCM_BST_RESOURCE_PBMP_SET(pres, PBMP_CMIC(unit));
    _BCM_BST_THRESHOLD_INFO(pres, INVALIDm, 
                          OP_CPU_QUEUE_BST_THRESHOLDr, BST_PROFILEf);
    ...
    /* _bcmResourceUcast */
    pres = _BCM_BST_RESOURCE(unit, _bcmResourceUcast);
    pres->index_min = 0;
...
{
    uint32 flags_tr = 0, fval;
    uint64 rval;
    
    BCM_IF_ERROR_RETURN(READ_THDO_BST_STATr(unit, &rval));
    if ((soc_reg64_field32_get(unit, THDO_BST_STATr, 
                               rval, UC_Q_BST_STAT_TRIGGEREDf)) ||
        (soc_reg64_field32_get(unit, THDO_BST_STATr, 
                               rval, MC_CPU_Q_BST_STAT_TRIGGEREDf))) {
        flags_tr |= BCM_COSQ_QUEUE;
    }

    if (soc_reg64_field32_get(unit, THDO_BST_STATr, 
                              rval, UC_PORT_SP_BST_STAT_TRIGGEREDf)) {
        flags_tr |= BCM_COSQ_PORT_POOL;
    }

    if (soc_reg64_field32_get(unit, THDO_BST_STATr, 
                              rval, MC_SP_BST_STAT_TRIGGEREDf)) {
        flags_tr |= BCM_COSQ_DEVICE_EGRESS_POOL;
    }

    BCM_IF_ERROR_RETURN(READ_THDI_BST_TRIGGER_STATUSr(unit, &rval));

    if (soc_reg64_field32_get(unit, THDO_BST_STATr, 
                              rval, PG_SHARED_TRIGGER_IDf)) {
        flags_tr |= BCM_COSQ_PRIGROUP;
    }

    fval = soc_reg64_field32_get(unit, THDO_BST_STATr, rval, TRIGGER_TYPEf);
    if (fval & 0x3) {
        flags_tr |= BCM_COSQ_PRIGROUP;
    }
    ...
    if (fval & 0x8) {
        flags_tr |= BCM_COSQ_DEVICE_INGRESS_POOL;
    }
...
_bcm_bst_td2_init(int unit)
{
    ...
    _BCM_BST_THRESHOLD_INFO(pres, INVALIDm, 
                            MMU_THDM_DB_POOL_MCUC_BST_THRESHOLDr, BST_THRESHOLDf);
    _BCM_BST_RESOURCE_STAT_INIT(pres, 4, "bst egrpool");
    ...
    _BCM_BST_THRESHOLD_INFO(pres, INVALIDm, 
            THDI_BST_SP_GLOBAL_SHARED_PROFILEr, SP_GLOBAL_SHARED_PROFILEf);
    ...
    _BCM_BST_THRESHOLD_INFO2(pres, THDI_PORT_SP_CONFIG_Xm, 
            THDI_PORT_SP_CONFIG_Ym, INVALIDr, INVALIDr, SP_BST_PROFILE_SHAREDf);
    ...
    _BCM_BST_RESOURCE_STAT_INIT(pres, 
               soc_mem_index_count(unit, THDI_PORT_SP_BST_Xm), "bst port pool");
    ...
    _BCM_BST_THRESHOLD_INFO2(pres, THDI_PORT_PG_CONFIG_Xm, 
          THDI_PORT_PG_CONFIG_Ym, INVALIDr, INVALIDr, PG_BST_PROFILE_SHAREDf);
    ...
    _BCM_BST_RESOURCE_STAT_INIT(pres, 53 * 8, "bst pg");
    ...
    _BCM_BST_THRESHOLD_INFO2(pres, THDI_PORT_PG_CONFIG_Xm,
        THDI_PORT_PG_CONFIG_Ym, INVALIDr, INVALIDr, PG_BST_PROFILE_HDRMf);

    _BCM_BST_STAT_INFO2(pres, THDI_PORT_PG_BST_Xm, THDI_PORT_PG_BST_Ym,
                        INVALIDr, INVALIDr, PG_BST_STAT_HDRMf);

    _BCM_BST_RESOURCE_STAT_INIT(pres, 53 * 8, "bst pg hdrm");
    ...
    _BCM_BST_THRESHOLD_INFO2(pres, MMU_THDU_XPIPE_CONFIG_QUEUEm, 
        MMU_THDU_YPIPE_CONFIG_QUEUEm, INVALIDr, INVALIDr, Q_WM_MAX_THRESHOLDf);
    ...
    _BCM_BST_RESOURCE_STAT_INIT(pres, 
          2 * soc_mem_index_count(unit, MMU_THDU_XPIPE_BST_QUEUEm), "bst ucast");
...
_bcm_bst_sync_thread(void *cookie)
{
    ...
    soc_tr3_set_bst_callback(unit, &_bcm_bst_hw_event_cb);
...
STATIC int
_bcm_bst_cmn_profile_delete_hw(int unit, _bcm_bst_resource_t res, 
                               int profile_index)
{
    _bcm_bst_resource_info_t *resInfo;
    
    resInfo = _BCM_BST_RESOURCE(unit, res);
...
_bcm_bst_default_profile_init(int unit)
{
    ...
    _BCM_BST_RESOURCE_ITER(bst_info, res) {
        resInfo = _BCM_BST_RESOURCE(unit, res);
    ...
    _BCM_BST_RESOURCE_ITER(bst_info, res) {
        ...
        if (resInfo->flags & _BCM_BST_CMN_RES_F_PROFILEID) {
            for (pipe = 0; pipe < _BCM_MAX_PIPES; pipe++) {
                ...
                if (!wb) {
                    SOC_IF_ERROR_RETURN(_bcm_bst_cmn_profile_set_hw(unit, pipe, 
                                        -1, res, &profile, &profile_index));
                }
                for (ei = resInfo->index_min; ei <= resInfo->index_max; ei++) {
                    if (wb) {
                        SOC_IF_ERROR_RETURN(        
                           _bcm_bst_cmn_io_op(unit, _bcmBstOpGet, ei, res, 
            ...
            #if 0
            /* No profile, but direct thresholds, mark them to very
             * high by default. */
            for(idx = resInfo->index_min; idx < resInfo->index_max; idx++) {
                SOC_IF_ERROR_RETURN(_bcm_bst_cmn_profile_set_hw(unit, idx, res, 
                                                                &profile, NULL));
                            ...
                            uint32 *p_profile)
{
    ...
    if ((res >= _bcmResourceMaxCount) || (profile == NULL)) {
        return BCM_E_PARAM;
    }
    ...
    if (resInfo->flags & _BCM_BST_CMN_RES_F_PROFILEID) {
        if (hw_index >= 0) {
            SOC_IF_ERROR_RETURN(        
               _bcm_bst_cmn_io_op(unit, _bcmBstOpGet, hw_index, res, 
                                  resInfo->threshold_mem[pipe],
                                  resInfo->threshold_reg[pipe],
                                  resInfo->threshold_field,
                                  &old_profile_index));
        ...
        if (profile_index != 0xffffffff) {
            if (hw_index >= 0) {
                SOC_IF_ERROR_RETURN(        
                    _bcm_bst_cmn_io_op(unit, _bcmBstOpSet, hw_index, res, 
                                       resInfo->threshold_mem[pipe],
                                       resInfo->threshold_reg[pipe],
                                       resInfo->threshold_field, 
                                       &profile_index));
        ...
        if (old_profile_index != 0xffffffff) {
            SOC_IF_ERROR_RETURN(_bcm_bst_cmn_profile_delete_hw(unit, res, 
                                                    old_profile_index));
        }
    } else {
        SOC_IF_ERROR_RETURN(        
                            ...
                            bcm_cosq_bst_profile_t *profile)
{
    ...
    if (resInfo->flags & _BCM_BST_CMN_RES_F_PROFILEID) {
        ...
        SOC_IF_ERROR_RETURN(
           _bcm_bst_cmn_io_op(unit, _bcmBstOpGet, profile_index, res, 
                              resInfo->profile_mem,
                              resInfo->profile_reg,
                        ...
                        uint32 flags, bcm_cosq_bst_profile_t *profile)
{
    ...
    do {
        rv = bst_info->handlers.resolve_index(unit, gport, cosq, flags,
           &res, &pipe, &start_hw_index, &end_hw_index, &rcb, &cb_data, &bcm_rv);
        if (bcm_rv) {
            return bcm_rv;
        }
        ...
        for (ii = start_hw_index; ii <= end_hw_index; ii++) {
            if (op == _BcmBstProfileGet) {
                SOC_IF_ERROR_RETURN(
                    _bcm_bst_cmn_profile_get_hw(unit, pipe, ii, res, profile));
            } else {
                SOC_IF_ERROR_RETURN(
                    _bcm_bst_cmn_profile_set_hw(unit, pipe, ii, res, profile, NULL));
            }
        }
                        ...
                        uint32 flags, uint64 *pstat)
{
    ...
    void *cb_data = NULL; 
    int bcm_rv, rcb = 0;
    int start_hw_index, end_hw_index, pipe;
    ...
    rv = bst_info->handlers.resolve_index(unit, gport, cosq, flags,
                                       &res, &pipe, &start_hw_index, 
                                       &end_hw_index, &rcb, &cb_data, &bcm_rv);
    if (rv != _BCM_BST_RV_OK) {
    ...
    COMPILER_64_SET(*pstat, 0, resInfo->p_stat[start_hw_index]);
                           ...
                           void *user_data)
{
    ...
    _bcm_bst_cb_ret_t rv;
    void *cb_data = NULL; 
    int bcm_rv, ii, fi, rcb = 0;
    int start_hw_index, end_hw_index, tflag, pipe;
    uint64  sval;
    ...
    for (fi = 0; fi < 32; fi++) {
        ...
        do {
            rv = bst_info->handlers.resolve_index(unit, gport, cosq, tflag, 
                                        &res, &pipe, &start_hw_index, 
                                        &end_hw_index, &rcb, &cb_data, &bcm_rv);
            if (bcm_rv) {
                return bcm_rv;
            ...
            for (ii = start_hw_index; ii <= end_hw_index; ii++) {
                COMPILER_64_SET(sval, 0, resInfo->p_stat[start_hw_index]);

                rv = cb(unit, gport, cosq, tflag, sval, user_data);
...
#if defined(BCM_TRIUMPH3_SUPPORT)
STATIC int _bcm_bst_res_to_flags(int res)
{
    switch (res) {
        case _bcmResourceDevice:
            return BCM_COSQ_DEVICE;
        break;
        case _bcmResourceEgrPool:
            return BCM_COSQ_DEVICE_EGRESS_POOL;
        break;
        case _bcmResourceIngPool:
            return BCM_COSQ_DEVICE_INGRESS_POOL;
        break;
        case _bcmResourcePortPool:
            return BCM_COSQ_PORT_POOL;
        break;
        case _bcmResourcePriGroupShared:
            return BCM_COSQ_PRIGROUP;
        break;
        case _bcmResourcePriGroupHeadroom:
            return BCM_COSQ_HEADROOM;
        break;
        case _bcmResourceUcast:
            return BCM_COSQ_QUEUE;
        break;
    }
    return 0;
}
...
STATIC int
_bcm_bst_sync_hw_trigger(int unit, int res, int port, int index)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    int rv, pipe;
    ...
    for (pipe = 0; pipe < _BCM_MAX_PIPES; pipe++) {
        if ((mem = resInfo->stat_mem[pipe]) != INVALIDm) {
            ...
            resInfo->p_stat[index] = soc_mem_field32_get(unit, mem, 
                                                        entry, resInfo->stat_field);
            soc_mem_field32_set(unit, mem, entry, resInfo->stat_field, 0);

            SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, index, entry));
        } else if ((reg = resInfo->stat_reg[pipe]) != INVALIDr) {
            rv = soc_reg32_get(unit, reg, REG_PORT_ANY, index, &rval);
            if (rv) {
                return BCM_E_INTERNAL;
            }
            resInfo->p_stat[index] = soc_reg_field_get(unit, reg, 
    ...
    if (bst_info->post_sync) {
        bst_info->post_sync(unit);
    }

    /* reverse map the inedx to port/cos pair */
    bst_info->handlers.reverse_resolve_index(unit, res, port, index, &gport, &cosq);
    
    soc_event_generate(unit, SOC_SWITCH_EVENT_MMU_BST_TRIGGER, 
                        _bcm_bst_res_to_flags(res), gport, cosq);

    return BCM_E_NONE;
}

STATIC int
_bcm_bst_sync_hw_snapshot(int unit, int res, int port, int index)
{
    _bcm_bst_cmn_unit_info_t *bst_info;
    _bcm_bst_resource_info_t *resInfo;
    int mem_wsz, idx, rv, pipe;
    void *pentry;
    char *dmabuf;
    uint32 rval;
    soc_mem_t mem;
