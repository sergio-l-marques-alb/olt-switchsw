...
/* HURRICANE3 for no_src_check_port_id is 0x3f */
...
bcm_tr_ipmc_init(int unit)
{
    ...
    if (SOC_MEM_IS_VALID(unit, EGR_IPMCm)) {
        ...
        size = SOC_MEM_TABLE_BYTES(unit, EGR_IPMCm);
        mbuf = soc_cm_salloc(unit, size, "EGR_IPMCm");
        if (NULL == mbuf) {
            return BCM_E_MEMORY;
        }
        sal_memset(mbuf, 0, size);
        for (i = 0; i < IPMC_GROUP_NUM(unit); i++) {
            egr_ipmc_entry = soc_mem_table_idx_to_pointer(unit, EGR_IPMCm, 
                                        egr_ipmc_entry_t *, mbuf, i);
            sal_memcpy(egr_ipmc_entry, &egr_entry, sizeof(egr_ipmc_entry_t));
        }
        rv = soc_mem_write_range(unit, EGR_IPMCm, MEM_BLOCK_ANY,
                                 soc_mem_index_min(unit, EGR_IPMCm),
                                 soc_mem_index_max(unit, EGR_IPMCm),
                                 mbuf);
        soc_cm_sfree(unit, mbuf);
        if (rv < 0) {
            sal_free(info->ipmc_group_info);
