                                                ...
                                                int stm_index)
{
    ...
    BCM_IF_ERROR_RETURN(
        soc_mem_read(unit, PORT_TABm, MEM_BLOCK_ALL, port, &ptab_entry)); 
    ...
    if (SOC_MEM_FIELD_VALID(unit, LPORT_TABm, V4L3_ENABLEf)) {
        value = soc_mem_field32_get(unit, PORT_TABm, 
                                    &ptab_entry, V4L3_ENABLEf);
...
_bcm_coe_subtag_subport_default_egr_lport_entry_set(int unit, int port)
{
    ...
    int          i, index, priority, cfi, color;
    ...
    BCM_IF_ERROR_RETURN(soc_profile_mem_add(unit, egr_pri_cng_profile[unit], 
                                            ...
                                            &values[3]));
    values[3] = values[3]/_BCM_EGR_PRI_CNG_MAP_CHUNK;
    BCM_IF_ERROR_RETURN(
        bcm_esw_port_egr_lport_fields_set(
            unit, port, EGR_LPORT_PROFILE_LPORT_TAB, 4, fields, values));

    BCM_IF_ERROR_RETURN(
        bcm_esw_port_egr_lport_field_set(
            unit, port, EGR_LPORT_PROFILE_PORT_1, OUTER_TPID_ENABLEf, 1));
                                 ...
                                 bcm_gport_t *gport)
{
    ...
    _bcm_subtag_subport_port_info[unit][subtag_index].nh_index = 
        BCM_XGS3_DVP_EGRESS_IDX_MIN + nh_index;
#endif
    ...
    if (soc_feature(unit, soc_feature_untethered_otp)) {
        if ((type == bcmSwitchSubportCoEEtherType) ||
            ...
            if (SOC_BOND_INFO_FEATURE_GET(unit,
                    socBondInfoFeatureNoCoeSubtag8100) && (type == 0x8100)) {
                return BCM_E_PARAM;
            }
        }
    }
...
bcmi_xgs5_subport_wb_recover(int unit, uint16 version, uint8 **scache_ptr)
{
    ...
    if (BCM_PBMP_NOT_NULL(SOC_INFO(unit).subtag_allowed_pbm)) {
        ...
        SCACHE_BYTE_RESTORE(*scache_ptr,
                            &si->subtag_pbm,
                            SHR_BITALLOCSIZE(SOC_MAX_NUM_PORTS));
        sal_memcpy(&si->subtag.bitmap, &si->subtag_pbm,
                   SHR_BITALLOCSIZE(SOC_MAX_NUM_PORTS));
