...
int _bcm_td2_vlan_vp_sync(int unit, uint8 **scache_ptr)
{
    ...
    uint8 *vlan_vp_scache_ptr;

    vlan_vp_scache_ptr = *scache_ptr;
    if (vlan_vp_scache_ptr == NULL) {
        return BCM_E_PARAM;
    }
    ...
    for (vp = 0; vp < num_vp; vp++) {
        ...
        vlan_vp_scache_ptr += sizeof(bcm_gport_t);

        /* sync VLAN_VP_INFO(unit, vp)->flags */
        sal_memcpy(vlan_vp_scache_ptr,
                   &(vlan_vp->flags), sizeof(uint32));

        vlan_vp_scache_ptr += sizeof(uint32);
    }
