...
#ifdef PORTMOD_SUPPORT
    pbmp_t p_pbmp;
    int rv = BCM_E_NONE, valid, flags=0, lane=0, num_lanes;
    portctrl_pport_t pport;
    ...
    if (SOC_SUCCESS(rv)) {
        if ((!init_flag) || (init_flag == PORTMOD_PORT_ADD_F_INIT_CORE_PROBE)) {
            ...
            for (lane = 0; lane < num_lanes; lane++) {
                ...
                SOC_PBMP_PORT_ADD(p_pbmp, SOC_INFO(unit).port_l2p_mapping[port] + lane);
            }
...
#define IS_FULL_SPEED_SET_REQUIRED(cur_speed, new_speed) \
    ...
    if (IS_FULL_SPEED_SET_REQUIRED(cur_speed, speed)) {
        ...
        /* disable MAC and PHY */
        BCM_IF_ERROR_RETURN(bcmi_esw_portctrl_enable_set(unit, port, 0));
        /* reconfigure chip (PGW/TDM/IP/EP/MMU) */
        BCM_IF_ERROR_RETURN(_bcm_esw_portctrl_speed_chip_reconfigure(
                                    unit, port, speed));
        /* reconfigure MAC & PHY */
        BCM_IF_ERROR_RETURN(_bcm_esw_portctrl_speed_interface_config_set(
                                    unit, port, pport, speed));
        /* enable MAC and PHY */
        BCM_IF_ERROR_RETURN(bcmi_esw_portctrl_enable_set(unit, port, 1));
        /* restore loopback */
        if (loopback != BCM_PORT_LOOPBACK_NONE) {
            BCM_IF_ERROR_RETURN(bcmi_esw_portctrl_loopback_set(unit,
                                            port,
...
bcmi_esw_portctrl_master_get(int unit, bcm_gport_t port, int *ms)
{
    ...
    /* We return BCM_PORT_MS_NONE since the underlying phy control functions are null
...
_bcm_esw_portctrl_detach(int unit, pbmp_t pbmp, pbmp_t *detached)
{
    ...
    SOC_PBMP_ITER(pbmp, port) {
        ...
        portmod_port_add_info_t_init(unit, &port_add_info);
...
bcmi_esw_portctrl_encap_xport_set(int unit, bcm_gport_t port, int mode)
{
    ...
    if (SOC_MEM_IS_VALID(unit, EGR_MTUm)) {
        ...
        if (soc_mem_field_valid(unit, mem, MTU_ENABLEf)) {
            soc_mem_field_set(unit, mem, entry, MTU_ENABLEf, &field_val_clear);
        }
    ...
    if (!soc_feature(unit, soc_feature_hg_no_speed_change)) {
        ...
        if (to_higig) {
            ...
            ability.speed_full_duplex &= ~non_ieee_speed;
            speed_max = SOC_CONTROL(unit)->info.port_speed_max[port];
            force_speed = SOC_PORTCTRL_HG2_TO_IEEE_BW_GET(speed_max);
        }
...
/*
 * Function:
 *      _bcm_esw_portctrl_mixed_sister_encap_check
 * Purpose:
 *      Check if the encap type for all sister ports in same PortMacro are same.
 *
 * Parameters:
 *      unit - StrataSwitch unit #
 *      port - StrataSwitch port #
 ...
 */
STATIC int
_bcm_esw_portctrl_mixed_sister_encap_check(int unit, bcm_port_t port, int mode)
{
    int rv = BCM_E_UNAVAIL;

    PORT_LOCK(unit);
    if (BCM_ESW_PORT_DRV(unit)!= NULL) {
        if (BCM_ESW_PORT_DRV(unit)->mixed_sister_encap_check != NULL) {
            rv = BCM_ESW_PORT_DRV(unit)->mixed_sister_encap_check(unit, port,
                                                                  mode);
        }
    }
    PORT_UNLOCK(unit);
...
bcmi_esw_portctrl_encap_set(int unit, bcm_gport_t port, int mode, int force)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_esw_portctrl_mixed_sister_encap_check(unit, port, mode));
