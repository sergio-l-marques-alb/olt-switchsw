                            ...
                            bcm_oob_fc_rx_intf_id_t intf_id)
{
    ...
    char *sysport_buf, *tc2pri_buf;
    uint32 old_sysport_list[_TH_MAX_RX_MEM_ENTRY];
    uint64 old_tc2pri_list[_TH_MAX_RX_MEM_ENTRY];
    uint32 new_sysport_list[_TH_MAX_RX_MEM_ENTRY];
    uint64 new_tc2pri_list[_TH_MAX_RX_MEM_ENTRY];
    int rv = BCM_E_INTERNAL;
    int mem_wsz = 0;
    int i = 0, j = 0, k = 0;
    void *pentry;
    ...
    for (i = 0; i < _TH_NUM_OOB_FC_RX_INTF; i++) {
        BCM_IF_ERROR_RETURN
            (_bcm_th_oob_fc_rx_get_intf_info(unit, &old_info[i], i));
    }
