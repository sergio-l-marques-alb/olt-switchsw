...
_field_type_map_t *recovery_type_map;
...
tlv_read(int unit, _field_tlv_t *tlv, uint8 *ptr, uint32 *position)
{
    ...
    /* Read Length */
    if (recovery_type_map[tlv->type].flags
        & _FP_WB_TLV_LEN_PRESENT) {
    ...
    if (!(recovery_type_map[tlv->type].flags
          & _FP_WB_TLV_NO_VALUE)) {
        if (tlv->basic_type == _bcmFieldInternalVariable) {
            read_size = recovery_type_map[tlv->type].size;
        } else if (tlv->basic_type == _bcmFieldInternalArray) {
            read_size = ((tlv->length) * recovery_type_map[tlv->type].size);
        } else {
            return BCM_E_INTERNAL;
        }
        if (recovery_type_map[tlv->type].flags
    ...
    if (read_size > 0) {
        ...
        BCM_IF_ERROR_RETURN(
             _tlv_print_array(unit, tlv->value, read_size,
                              recovery_type_map[tlv->type].size));
    }
    if (read_size == 0) {
        tlv->value = NULL;
    }
        ...
        switch (tlv.type)
        {
            ...
            case _bcmFieldInternalEntryActionsPbmp:
                 ...
                 wb_entry->action_count = (tlv.length 
                                * static_type_map[tlv.type].size * 8);
                                   ...
                                   _field_stage_t   *stage_fc)
{
    ...
    if (enum_properties_type == BCM_FIELD_TYPE_ENUM_PROPERTIES) {
        ...
        _FP_XGS3_ALLOC(recovery_type_map, (sizeof (_field_type_map_t)
                       * enum_total_count),
                       "enum type map");
        ...
        for (enum_count = 0; enum_count < enum_total_count; enum_count++) {
            ...
            sal_memcpy(&recovery_type_map[enum_count], &temp_type_map,
                       sizeof(_field_type_map_t));
        }
    } else {
        recovery_type_map = (_field_type_map_t *)&static_type_map;
        fc->scache_pos -= sizeof(uint32);
    }
...
cleanup:
    ...
    if (enum_properties_type == BCM_FIELD_TYPE_ENUM_PROPERTIES) {
        if (recovery_type_map != NULL) {
            sal_free(recovery_type_map);
        }
    }
