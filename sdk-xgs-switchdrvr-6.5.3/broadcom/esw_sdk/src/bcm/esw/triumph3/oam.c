                                ...
                                int *mpls_count, bcm_if_t *l3_intf_id)
{
    ...
    if (BCM_SUCCESS
        ...
        for (i = 0; (i < label_count) && (count < max_count); i++) {
            _bcm_tr3_oam_bhh_mpls_label_get(label_array[label_count - 1 - i].label,
                                       label_array[label_count - 1 - i].exp,
                                       0,
                                       label_array[label_count - 1 - i].ttl,
                                       &mpls[count++]);
        }
    }
