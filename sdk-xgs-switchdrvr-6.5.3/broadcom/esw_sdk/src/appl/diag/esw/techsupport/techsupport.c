...
char command_techsupport_usage[] =
    ...
    "techSupport l3uc [chg] [diag] [reg] "
    "[mem] - collects unicast L3 debug information\n";
extern cmd_result_t command_techsupport(int unit, args_t *a);

extern mbcm_techsupport_t mbcm_trident2plus_techsupport[];
...
static struct techsupport_cmd {
    ...
    {"basic", techsupport_basic },
    {"l3uc",  techsupport_l3uc },
...
command_techsupport(int unit, args_t *a)
{
    ...
    for (; mbcm_techsupport[unit]->techsupport_feature_name; mbcm_techsupport[unit]++) {
         if (feature_name == NULL) {
             cli_out("Error: INVALID INPUT\n");
             cli_out("For valid input use 'techSupport help' Command\n\n");
             return CMD_FAIL;
         } else if (sal_strcasecmp(feature_name, mbcm_techsupport[unit]->techsupport_feature_name) == 0) {
             techsupport_feature_data = mbcm_techsupport[unit]->techsupport_data;
