...
portmod_port_diag_status_get(int unit, int port, portmod_port_diag_status_t *diag_status){
    ...
    ret = portmod_port_interface_config_get(unit, port, &config);    
    if(ret != SOC_E_NONE){     
    ...
    ret = portmod_port_link_get(unit, port, &diag_status->link);
    if(ret != SOC_E_NONE){     
    ...
    sal_memcpy(&diag_status->phys, &diag_info.phys, sizeof(soc_pbmp_t));
...
portmod_port_diag_autoneg_get(int unit, int port, portmod_port_diag_autoneg_t* diag_autoneg){
    ...
    /* get flow control status */
    ret = portmod_port_ability_local_get(unit, port, &diag_autoneg->local_ability);
    if(ret != SOC_E_NONE){     
        cli_out("ERROR: Getting port local ability failed: %s\n", soc_errmsg(ret));     
        return CMD_FAIL;                                    
    } 
    ret = portmod_port_ability_advert_get(unit, port, &diag_autoneg->adv_ability);
    if(ret != SOC_E_NONE){     
        cli_out("ERROR: Getting port advertisement ability failed: %s\n", soc_errmsg(ret));     
        return CMD_FAIL;                                    
    } 
    ret = portmod_port_ability_remote_get(unit, port, &diag_autoneg->remote_ability);
    if(ret != SOC_E_NONE){     
...
portmod_info_pm_summary_diag(int unit){
    ...
    for( pm = 0; pm <= MAX_PMS; pm++){
        ...
        BCM_PBMP_ITER(diag_info.ports, port){
            break;
        }
        if (port >= MAX_PORTS){
...
portmod_info_pm_detail_diag(int unit, int pm){
    ...
    if(CMD_OK != ret){
        ...
        ref_clk += sal_strlen("phymodRefClk");
    }
    /*display PM lane map*/
...
portmod_info_port_detail_diag(int unit, int port){
    ...
    if(CMD_OK != rv) {
        ...
        mode_str += sal_strlen("portmodPortMode");
    }
    cli_out("Core Mode: %s\n", mode_str);
...
portmod_status_port_summary_diag(int unit){
    ...
    BCM_PBMP_ITER(PBMP_PORT_ALL(unit), port){
        ...
        BCM_PBMP_ITER(diag_status.phys, phy){
            is_virtual = FALSE;
            break;
        }
        if(is_virtual){
...
portmod_port_ability_display(portmod_port_ability_t *port_ability){
    ...
    char *port_ability_fcmap_string[] = _SHR_PA_FCMAP_STRING;
    ...
    string_len = sizeof(port_ability_fcmap_string)/sizeof(char *);
    cli_out("FCMAP:             |");
    port_ability->fcmap= 15;
    for (i = 0;i < string_len;i++){
        if (port_ability->fcmap&(1<<i)){
            cli_out("%s|", port_ability_fcmap_string[i]);
        }        
    }
    cli_out("\n");
...
portmod_phy_diag(int unit, args_t *a){
    ...
    if (!ARG_CNT(a)) {  
        ...
        if(CMD_OK != ret){
            ...
            osr_str += sal_strlen("phymodOversample");
        }
        ...
        if(CMD_OK != ret){
            ...
            pmd_str += sal_strlen("phymodPmdMode");
        }
...
portmod_eyescan_diag(int unit, args_t *a){
    ...
    if (!ARG_CNT(a)) {  
        ...
        ret = portmod_port_interface_config_get(unit, port, &interface_config);
        if(ret != SOC_E_NONE){    
        ...
        ret = phymod_diag_eyescan_run(phy_access, line_rates, nof_phys, mode, &eyescan_options);
        if(ret != SOC_E_NONE){    
