...
cache_mem_test_write_iter_callback(int unit, soc_mem_t mem, void* data)
{
    ...
    if (mem == EPNI_LINK_LAYER_OR_ARP_NEW_FORMATm || mem == EPNI_PROTECTION_ENTRYm || mem == EPNI_TRILL_FORMAT_FULL_ENTRYm || mem == EPNI_TRILL_FORMAT_HALF_ENTRYm || mem == KAPS_TCMm || mem == SER_ACC_TYPE_MAPm)
    {
        return 0;
    }
