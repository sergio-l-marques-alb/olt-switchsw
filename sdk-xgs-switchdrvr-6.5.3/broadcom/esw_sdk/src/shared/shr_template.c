 ...
 */
static shr_template_to_stream_t     *hash_compare_to_stream_cb;
static shr_template_from_stream_t   *hash_compare_from_stream_cb;
                ...
                int cb_arr_length)
{
    ...
    if (cb_arr_length > 0) {
         ...
         */
        hash_compare_to_stream_cb = sal_alloc(cb_arr_length * sizeof(shr_template_to_stream_t), "hash_compare_to_stream_cb");
        if (!hash_compare_to_stream_cb) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("Can't allocate memory for hash compare callback array\n")));
            return _SHR_E_MEMORY;
        }

        sal_memcpy(hash_compare_to_stream_cb, to_stream_cb_arr, cb_arr_length * sizeof(shr_template_to_stream_t));

        hash_compare_from_stream_cb = sal_alloc(cb_arr_length * sizeof(shr_template_from_stream_t), "hash_compare_from_stream_cb");
        if (!hash_compare_from_stream_cb) {
            sal_free(hash_compare_to_stream_cb); /* Free previous allocation. */
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("Can't allocate memory for hash compare callback array\n")));
            return _SHR_E_MEMORY;
        }

        sal_memcpy(hash_compare_from_stream_cb, from_stream_cb_arr, cb_arr_length * sizeof(shr_template_from_stream_t));
    }
...
err:
    if (result != _SHR_E_NONE) {
        if (hash_compare_to_stream_cb) {
            sal_free(hash_compare_to_stream_cb);
            hash_compare_to_stream_cb = NULL;
        }
        if (hash_compare_from_stream_cb){
            sal_free(hash_compare_from_stream_cb);
            hash_compare_from_stream_cb = NULL;
        }
    }
    LOG_DEBUG(BSL_LS_SOC_COMMON,
              (BSL_META("(%d, %d, %d) return %d (%s)\n"),
                                     ...
                                     int *template)
{
    ...
    if (!val) {
        ...
        /* move to buffer */
        res = hash_compare_to_stream_cb[extra](data,val,desc.data_size);
        if (res != BCM_E_NONE) {
            goto exit;
        }
                                        ...
                                        void *data)
{
    ...
    if (!val) {
        ...
        if (!SOC_SAND_FAILURE(soc_sand_rv) && ref_count != 0) {
            res = (hash_compare_from_stream_cb[extra](data,val,(desc.data_size)));
            if (res != BCM_E_NONE) {
