...
#if defined(BCM_88375) || defined(BCM_88675) || defined(BCM_88470)
...
_soc_cmic_uc_msg_system_thread(void *unit_vp) {
    ...
    while (1) {
        while (soc_cmic_uc_msg_receive_internal(unit, uC, MOS_MSG_CLASS_SYSTEM,
                   &rcv, sal_sem_FOREVER, 1) != SOC_E_NONE) {
        ...
        if (rcv.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_LOG) {
            ...
            if (rcv.s.len == 0) {
                ...
                send.s.data = soc_htonl(soc_cm_l2p(unit, endian_buf));
                send_rv = soc_cmic_uc_msg_send(unit, uC, &send,
                                               soc->uc_msg_send_timeout);
            }
            ...
            LOG_CLI((BSL_META_U(unit,
                                "%c"), rcv.s.data & 0x0ff));
#if defined (INCLUDE_UC_ULINK)
        } else if (rcv.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_LINK_REQ) {
...
/*
 * Function
 *      soc_cmic_uc_msg_receive_internal
 * Purpose:
...
{
    /* Do the receive, but not as the system messaging thread */
                          ...
                          soc_cmic_uc_appl_cb_t *shutdown_cb, void *cb_data)
{
    ...
    if(msg_class == MOS_MSG_CLASS_BFD) {
        bfd_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif

#if defined(INCLUDE_BHH)
    if(msg_class == MOS_MSG_CLASS_BHH) {
        bhh_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif

#if defined(INCLUDE_ETH_LM_DM)
    if(msg_class == MOS_MSG_CLASS_ETH_LM_DM) {
        eth_lm_dm_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif
#if defined(INCLUDE_MPLS_LM_DM)
    if(msg_class == MOS_MSG_CLASS_MPLS_LM_DM) {
        mpls_lm_dm_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif
    return (SOC_E_NONE);
}
    ...
    soc_cmic_uc_ts_data_t *ts_data)
{
    ...
    uint64 event_ts_minus_ts0_nsec = COMPILER_64_INIT(0,0);
    uint64 ts_48bitmask = COMPILER_64_INIT(0,0); /* 48 bit mask */
    uint64 sign_mask64 = COMPILER_64_INIT(0xFFFF0000,0);
    ...
    COMPILER_64_MASK_CREATE(ts_48bitmask, 48, 0);
    COMPILER_64_AND(read_ts0[0], ts_48bitmask);

    if ((!COMPILER_64_BITTEST(read_ts0[0], 46) && !COMPILER_64_BITTEST(read_ts[0], 47)) ||
        (COMPILER_64_BITTEST(read_ts0[0], 46) && COMPILER_64_BITTEST(read_ts0[0], 47))) {

         if (COMPILER_64_BITTEST(read_ts0[0], 47)) {
            COMPILER_64_OR(read_ts0[0], sign_mask64);
         }
         if (COMPILER_64_BITTEST(read_ts[0], 47)) {
            COMPILER_64_OR(read_ts0[0], sign_mask64);
         }
    }

    /* ts_minus_ts0_nsec = (read_ts - read_ts0) */
    COMPILER_64_COPY(event_ts_minus_ts0_nsec, read_ts[0]);
    COMPILER_64_SUB_64(event_ts_minus_ts0_nsec, read_ts0[0]);
     ...
     */
    COMPILER_64_COPY(ts_data->hwts_ts1, read_ts1[0]);  /* = ts1 + (read_ts - read_ts0) => event_ts1 */
    COMPILER_64_ADD_64(ts_data->hwts_ts1, event_ts_minus_ts0_nsec);
...

    {
        ...
        uint64 full_time_nsecs_64 = COMPILER_64_INIT(0, 0);
        int isneg_ts_minus_ts0_nsec = 0;

        COMPILER_64_SET(full_time_nsecs_64, 0, read_full_time[0].nanoseconds);

        /* full_time_nsec += event_ts_minus_ts0_nsec */
        COMPILER_64_COPY(abs_ts_minus_ts0_nsec, event_ts_minus_ts0_nsec);
        COMPILER_64_ADD_64(abs_ts_minus_ts0_nsec, full_time_nsecs_64);

        /* Division / multiplication are unsigned, so use absolute value of
         * difference 
         */
         if (COMPILER_64_LT(read_ts[0], read_ts0[0]) &&
               COMPILER_64_BITTEST(abs_ts_minus_ts0_nsec, 63)){
            COMPILER_64_SET(temp,0,0);
            COMPILER_64_SUB_64(temp, abs_ts_minus_ts0_nsec);
            COMPILER_64_COPY(abs_ts_minus_ts0_nsec, temp);
            isneg_ts_minus_ts0_nsec = 1;
         }

        COMPILER_64_COPY(abs_ts_minus_ts0_sec, abs_ts_minus_ts0_nsec);
        COMPILER_64_UDIV_64(abs_ts_minus_ts0_sec, one_billion);

        /* abs_ts_minus_ts0_nsec -= (abs_ts_minus_ts0_sec * 1e9) */
        COMPILER_64_COPY(temp, abs_ts_minus_ts0_sec);
        COMPILER_64_UMUL_32(temp, COMPILER_64_LO(one_billion));
        COMPILER_64_SUB_64(abs_ts_minus_ts0_nsec, temp);

        if(!isneg_ts_minus_ts0_nsec) {
            ts_data->time.nanoseconds = COMPILER_64_LO(abs_ts_minus_ts0_nsec);
            ...
            COMPILER_64_COPY(temp, one_billion);
            COMPILER_64_SUB_64(temp, abs_ts_minus_ts0_nsec);
