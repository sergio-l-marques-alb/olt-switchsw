...
_mac_xl_timestamp_byte_adjust_set(int unit, soc_port_t port)
{
    ...
    if (SOC_REG_PORT_VALID(unit, MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr, port) &&
        ...
        SOC_IF_ERROR_RETURN(READ_MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr(unit, port, &ctrl));
        soc_reg64_field32_set(unit, MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr, &ctrl,
                          TX_TIMER_BYTE_ADJUSTf, 8);
        SOC_IF_ERROR_RETURN(WRITE_MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr(unit, port, ctrl));
