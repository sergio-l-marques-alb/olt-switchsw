...
_xlmac_port_mode_update(int unit, soc_port_t port, int hg_mode)
{
    ...
    if (hg_mode != SOC_ENCAP_IEEE) { /* HiGig Mode */
        ...
        soc_reg64_field32_set (unit, XLMAC_RX_CTRLr, &rx_ctrl, STRICT_PREAMBLEf, 1); 
        ...
        soc_reg64_field32_set (unit, XLMAC_RX_CTRLr, &rx_ctrl, STRICT_PREAMBLEf, 0);
...
int xlmac_tx_average_ipg_set(int unit, soc_port_t port, int val)
{
    ...
    val = val < 64 ? 64 : (val > 248 ? 248 : (val + 7) & (0x1f << 3));
