/* $Id: nd_hw.c,v 1.20 Broadcom SDK $
              ...
              AG_U16        *p_data)
{
    ...
    /* LOG_CLI((BSL_META("n_offset:0x%08lx\n"), n_offset)); */
	...
	LOG_CLI((BSL_META("%s: Bad offset:0x%08lx\n"), __func__, n_addr));
	return;
    }
    ...
    {
      if (AG_ND_HW_ACCESS_WRITE == e_action)
          LOG_CLI((BSL_META("w: addr=0x%06lX data=0x%04hX\n"), n_addr, *p_data));

      if (AG_ND_HW_ACCESS_READ == e_action)
          LOG_CLI((BSL_META("r: addr=0x%06lX data=0x%04hX\n"), n_addr, *p_data));
    }
#endif
}
