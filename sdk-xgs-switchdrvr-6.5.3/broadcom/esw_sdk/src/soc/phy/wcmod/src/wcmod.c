/*
 * $Id: wcmod.c,v 1.394 Broadcom SDK $ 
...
_phy_wcmod_ind_init(int unit, soc_port_t port)
{
    ...
    if ((!SOC_RECONFIG_TDM || !SOC_PBMP_MEMBER(SOC_CONTROL(unit)->mac_phy_skip_pbm, port)) &&
        !SOC_WARM_BOOT(unit)) {
#endif
    if (!_wcmod_chip_init_done(unit,pc->chip_num,pc->phy_mode)) {
        /* configure and initialize the resource shared by all 4 lanes */
...
_phy_wcmod_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    tmp_lane_select     = ws->lane_select;        /* save this */
    ws->lane_select = WCMOD_LANE_BCST; /* go to broadcast mode */
    /*we need to set bit 11,10 to 1 of reg0x82ec for the cl72 to be working properly*/
    /* however these two bits need to turn off for 10G XAUI mode */
...
phy_wcmod_tx_lane_get(int unit, soc_port_t port, int *this_lane, int *swapped_lane)
{
    ...
    int chip_num = 0;
    int num_lanes = 0;
    int chip_base_port = 0; /* the first port of each chip */
    int tx_lane_swap = 0;    
    ...
    if(SOC_IS_CALADAN3(unit)){
        ...
        chip_num= pc->chip_num;
        *this_lane = chip_num * num_lanes + lane_num;
        if(pc->phy_mode == PHYCTRL_ONE_LANE_PORT){
            chip_base_port = chip_num * num_lanes;
        }else if(pc->phy_mode == PHYCTRL_QUAD_LANE_PORT){
            chip_base_port = port;
        }
        
        /* get chip_base_port's lane_swap info*/
        pc = INT_PHY_SW_STATE(unit, chip_base_port);
        pDesc = (WCMOD_DEV_DESC_t *)(pc + 1);
        pCfg = &pDesc->cfg;
        tx_lane_swap =  pCfg->txlane_map[0];
        *swapped_lane = ((tx_lane_swap>>(lane_num*4))&0xf) + chip_num*num_lanes;
    }
...
wcmod_eye_margin(int unit, soc_port_t port, int type)
{
   phy_ctrl_t      *pc;
    wcmod_st            *ws;      /*wcmod structure */
    WCMOD_DEV_DESC_t    *pDesc;
    int lane_start, lane_end, tmp_lane, i;
    WCMOD_EYE_DIAG_INFOt veye_info[4];
    ...
    /* clear the memory */
    WCMOD_MEM_SET(veye_info, 0, sizeof(veye_info));
    for (i = 0; i < 4; i++) {
    ...
    for (i = lane_start; i <= lane_end; i++) {
        ...
        ws->this_lane = i;
        SOC_IF_ERROR_RETURN
            (_wcmod_eye_margin_data_get(ws, &veye_info[i], type));
        SOC_IF_ERROR_RETURN
            (_wcmod_eye_margin_ber_cal(&veye_info[i], type));
        SOC_IF_ERROR_RETURN
            (_wcmod_eye_margin_diagram_cal(&veye_info[i], type));
    }
    ws->this_lane = tmp_lane;
