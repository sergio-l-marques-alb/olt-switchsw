    ...
    *tx_index = TXDRV_DFT_INX;
    ...
    case 10000:
        if ((speed_config->line_interface & PHY8806X_IF_SFI) ||
           (speed_config->line_interface & PHY8806X_IF_SR) ||
           (speed_config->line_interface & PHY8806X_IF_CR)) {
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    speed_config->an_fec = soc_property_port_get(unit, port, spn_PHY_AN_FEC, 0);
    speed_config->fs_cl72 = soc_property_port_get(unit, port, spn_PORT_INIT_CL72, 0);
    speed_config->an_cl72 = soc_property_port_get(unit, port, spn_PHY_AN_C72, 0);
    ...
    if (port_num_lanes == 1) {
        if (PHY_EXTERNAL_MODE(unit, port) || !(speed_config->fiber_pref)) {
            speed_config->line_interface =  PHY8806X_IF_XFI;
            speed_config->interface =  SOC_PORT_IF_XFI;
        } else {
            speed_config->line_interface =  PHY8806X_IF_SFI;
            speed_config->interface =  SOC_PORT_IF_SFI;
        }
    } else { 
            speed_config->line_interface =  PHY8806X_IF_XLAUI;
            speed_config->interface =  SOC_PORT_IF_XLAUI;
    }
...
/*     soc_phy_info_t *pi = NULL; */
    ...
    LOG_INFO(BSL_LS_SOC_PHY, (BSL_META_U(unit,
        "u=%d p=%d PHYCTRL_INIT_STATE = %u speed_max = %d - %s\n"),
         ...
         SOC_CONTROL(unit)->soc_flags & SOC_F_ALL_MODULES_INITED ? "FLEX" : "REGULAR" ));
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) ||
        ...
        LOG_ERROR(BSL_LS_SOC_PHY,
              (BSL_META_U(unit,
                          "phy_8806x_init: FCMAP ENABLE %d, port %d, dev_addr 0x%x\n"),
        ...
        if (soc_property_port_get(unit, port,spn_MDIO_FIRMWARE_DOWNLOAD_MASTER, 0) == pc->port) {
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS4) ||
        ...
        if ((pc->fcmap_enable) && (FLAGS(pc) & PHY8806X_BMP0)) {
            SOC_IF_ERROR_RETURN
        ...
        if ((pc->fcmap_enable) && (FLAGS(pc) & PHY8806X_BMP1)) {
            SOC_IF_ERROR_RETURN
                (READ_TOP_FC_MODE_CONTROL_REG(unit, pc, &temp32));
                temp32 |= 1 << 1;
            SOC_IF_ERROR_RETURN
...
#if defined(INCLUDE_FCMAP)
         /* Release LC PLLs out of reset */
        if ((pc->fcmap_enable) && (FLAGS(pc) & (PHY8806X_BMP0 | PHY8806X_BMP1))) {
            SOC_IF_ERROR_RETURN
...
phy_8806x_link_get(int unit, soc_port_t port, int *link)
{
    ...
    *link = 0;
    ...
    if ( an ) {
        if ( !an_done ) {
            #if 0
            printf("phy_8806x_link_get: AN not done, return \n");
            #endif
            *link = 0;
            return(SOC_E_NONE);
        }
    ...
    *link = (link_st & VS_MISC_STATUS_LL_STATUS_BIT_MASK);
...
#if defined(INCLUDE_FCMAP)
        if (pc->fcmap_enable) {
            /* Disable AN and set speed to max */
            PHY_AUTO_NEGOTIATE_SET (int_pc->pd, unit, port, 0); 
            PHY_SPEED_SET(int_pc->pd, unit, port, int_pc->speed_max);
            return SOC_E_NONE;
        }
#endif
...
phy_8806x_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    if ( an ) {
        if ( !an_done ) {
            #if 0
            printf("phy_8806x_speed_get: AN not done, return \n");
            #endif
            *speed = 0;
            return(SOC_E_NONE);
        }
...
phy_8806x_firmware_set(int unit, int port, int offset, uint8 *data,int len)
{
    ...
    switch (offset) {
        ...
        if ( FLAGS(pc) & PHY8806X_CHM) {
            SOC_IF_ERROR_RETURN
                (WRITE_TOP_SOFT_RESET_REG(unit, pc, (0xff << 6) ));
        }
        ...
        if ( FLAGS(pc) & PHY8806X_BMP0 ) {
            ...
            if ((pc->fcmap_enable) && (pc->phy_rev == 0)){
                SOC_IF_ERROR_RETURN
        ...
        case PHYCTRL_UCODE_BCST_END:
            ...
            if ( FLAGS(pc) & (PHY8806X_BMP0 | PHY8806X_BMP1) ) {
                ...
                sal_usleep(1000000);
                /* line */
                tsc_reg_read_test_base(unit, port, 0, 1, 0, 0xd150);
                /* sys */
                tsc_reg_read_test_base(unit, port, 1, 1, 0, 0xd150);
#endif
             }
...
phy_8806x_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    if (PHY8806X_40G_10G_INTF(pif)) {
        intf &= ~PHY8806X_40G_10G_INTF_ALL;  /* clear all 10G/40G interfaces */
        intf |= 1 << pif;
    } else {
        intf |= 1 << pif;
    }
...
STATIC int
phy8806x_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
...
phy_8806x_linkup(int unit, soc_port_t port)
{
    ...
    if (pc->fcmap_enable) {
        /* Notify 10G speed */
        SOC_IF_ERROR_RETURN
            (soc_phyctrl_notify(unit, port, phyEventSpeed, int_pc->speed_max));
    }
#endif
...
phy_8806x_fw_stat_read(int unit, int port, int offset,  uint32 result[], int result_size)
{
    ...
    if (sal_mutex_take(xmod_mutex[unit], 100000) < 0)
    {
        LOG_WARN(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
