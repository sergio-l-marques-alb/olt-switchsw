...
STATIC int _phy8486x_xfi_2p5g_mode_set(int unit, phy_ctrl_t *pc, uint16  mode_2p5g);
...
phy_8481_init(int unit, soc_port_t port)
{
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS3) ||
        ...
        if ( PHY_IS_BCM8486X(pc) ) {    /* Mako supports 5G/2.5G speed */
            uint32 xfi_2500x = soc_property_port_get(unit, port,
                                                     spn_PHY_SERDES, 0);
            if ( xfi_2500x == 2500 ) {
            ...
            SOC_IF_ERROR_RETURN(
                    _phy8486x_xfi_2p5g_mode_set(unit, pc, data16) );
        }
...
_phy_8481_check_firmware(int unit,phy_ctrl_t *pc) {
    ...
    if ( (PHY_IS_BCM8485X(pc) && ((fw_ver_main > 0) || ((fw_ver_main == 0) && (fw_ver_branch >= 3 )))) ||
        ...
        if ( ((fw_ver_main == 1) && (fw_ver_branch >= 1)) ||
              (fw_ver_main >  1) || (PHY_IS_BCM8486X(pc)) ) {
            FLAGS(pc) |= PHY8483X_ATOMIC_ACCESS_SHD;
        }
...
_phy_8481_copper_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if ( enable ) {         /* enable loopback */
        ...
        if ( speed >= 2500 ) {     /*  10G / 5G / 2.5G  */
            ...
            if (PHY_IS_BCM8486X(pc) || PHY_IS_BCM8484X(pc)) {
            ...
            /* 1000BASE-T/100BASE-TX MII Contol Reg. DEVAD=7, Addr=0xFFE0    */
            if ( PHY_IS_BCM8484X(pc) || PHY_IS_BCM8486X(pc) ) {
                if ( speed == 100 ) {
...
phy_8481_xehg_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    if ( PHY_IS_BCM8486X(pc) & (! PHY_REPEATER(unit, port)) ) {
...
_phy_8481_copper_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if ( PHY_IS_BCM8486X(pc) ) {   /* Mako support 5G/2.5G */
...
_phy_8481_copper_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    if ( PHY_IS_BCM8486X(pc) || PHY_IS_BCM8484X(pc) ) {
        ...
        if ( linkup ) {            /* if 8486x and line side link is up, get  */
            return SOC_E_NONE;     /* speed info from Core Config Status Reg. */
        }
...
phy_8481_link_up(int unit, soc_port_t port)
{
    ...
    case 2500:
        if ( ! LAST_IN_CHAIN(pc) ) {
            ...
            if (LINK_DOWN(pc)) {
                SOC_IF_ERROR_RETURN
                    (soc_phyctrl_notify(unit, port, phyEventSpeed,
                            (PHY_REPEATER(unit, port)) ? 10000 : speed));
                LINK_DOWN(pc) = 0;
            }
            if (PHY_IS_BCM8484X(pc)) {
                if (!(pc->flags & PHYCTRL_SYS_SIDE_CTRL)) {
...
_phy_8481_halt(int unit, phy_ctrl_t *pc)
{
    ...
    if (PHY_IS_BCM8485X_AND_UP(pc)) {
        if ( PHY_EXT_ROM_BOOT(pc) ) {
            ...
            data16 = (PHY_IS_BCM8486X(pc)) ? 0x48f0 : 0x0040;
            SOC_IF_ERROR_RETURN
...
STATIC int
_phy8486x_xfi_2p5g_mode_set(int unit, phy_ctrl_t *pc, uint16 mode_2p5g)
{
    uint16 args[2];
    ...
    SOC_IF_ERROR_RETURN
        (PHY84834_TOP_LEVEL_CMD_SET(unit, pc,
                    (uint16)PHY84834_DIAG_CMD_SET_XFI_2P5G_MODE_V2, args, 1) );
    return (SOC_E_NONE);
}
