                                 ...
                                 int block_idx)
{
    ...
    for (info_index = 0; ; info_index++) {
        ...
        /* some tables have been removed in 56270 */
        if (!SOC_MEM_IS_VALID(unit, info->mem)) {
            continue;
        }
                                   ...
                                   int reg_mem, uint32 address)
{
    ...
    /* Loop through each place-and-route block entry */
    for (route_block_index = 0; route_block_index < block_num; route_block_index++) {
        route_block ++;
        /* coverity[overrun-local:FALSE] */
        cmic_bit = route_block->cmic_bit;
