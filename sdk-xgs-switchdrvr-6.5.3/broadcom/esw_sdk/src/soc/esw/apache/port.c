...
/*
 * Function:
 *      _soc_ap_soc_info_update
 * Purpose:
 *      Update the SOC_INFO SW data structure with the FlexPort
 *      information.
 * Parameters:
 *      unit           - (IN) Unit number.
 *      nport          - (IN) Number of elements in array resource.
 ...
 *      SOC_E_XXX
 * Notes:
 *      Must pause linkscan, take COUNTER_LOCK, and SOC_CONTROL_LOCK.
 */
int
_soc_ap_soc_info_update(int unit,
                          int nport, soc_port_resource_t *resource,
                          int pre_count, soc_port_resource_t *pre_resource,
                          int post_count, soc_port_resource_t *post_resource)
{
    int rv;
    soc_info_t *si = &SOC_INFO(unit);
    soc_port_resource_t *pr;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_PORT,
                    (BSL_META_U(unit,
                                "Update SOC_INFO logical=%d physical=%d\n"),
                     logic_port, phy_port));
    ...
    /* Update port ptype bitmap and block information */
    rv = _soc_ap_soc_info_ptype_block_update(unit,
                                             pre_count, pre_resource,
                                             post_count, post_resource);

    /* Update counter map */
    if (SOC_SUCCESS(rv)) {
        rv = _soc_ap_soc_info_counter_update(unit,
                                             pre_count, pre_resource,
                                             post_count, post_resource);
    }
                                ...
                                soc_ap_info_t *pre_soc_info)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_apache_mmu_rqe_phy_port_mapping_set(unit, pre_count, pre_resource));
#endif
