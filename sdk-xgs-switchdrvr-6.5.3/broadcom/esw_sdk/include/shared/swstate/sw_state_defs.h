 ...
 */
/*#define SHR_SW_STATE_VERIFICATIONS 1*/
 ...
 */
#define SW_STATE_AUTOSYNC_OPTIMIZATION 1
...
#define SHR_SW_STATE_ALLOC(unit, location, size, flags, dbg_string)\
    shr_sw_state_alloc(unit, (uint8**) &location, size, flags);
...
#define SHR_SW_STATE_ALLOC_BITMAP(unit, location, nof_bits_to_alloc, flags, dbg_string)\
    shr_sw_state_alloc(unit, (uint8**) &location, SHR_BITALLOCSIZE(_nof_bits_to_alloc) + 1, flags);\
    shr_sw_state_shr_bitdcl_nof_bits_update(unit, (uint8 *)(location), (nof_bits_to_alloc));
