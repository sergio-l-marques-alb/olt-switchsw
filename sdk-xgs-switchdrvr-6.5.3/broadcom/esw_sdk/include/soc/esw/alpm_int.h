...
#define ALPM_WAR_VRF_BIT10_MASK     0x400
...
do {\
    (bank_disable) = soc_alpm_cmn_mode_get((u));\
...
#define ALPM_BKT_IDX(u, bid)        (((bid) >> BKTID_SHIFT) & SOC_TH_ALPM_BKT_MASK(u))
...
typedef struct alpm_bkt_usg_s {
    ...
    int16 vrf_type[4];  /* Different vrf type can't be merged (Glb or vrf)  */
...
#define SOC_ALPM_BS_BKT_USAGE_PIVOT(u, bidx) \
    bkt_usage[(u)][ALPM_BKT_IDX(u, bidx)].pivots[ALPM_BKT_SIDX(bidx)]

#define SOC_ALPM_BS_BKT_USAGE_VRF(u, bidx) \
    bkt_usage[(u)][ALPM_BKT_IDX(u, bidx)].vrf_type[ALPM_BKT_SIDX(bidx)]
...
#define SOC_TH_ALPM_BKT_ENTRY_TO_IDX(u, idx)       \
            (ALPM_BKTID((((idx) >> (soc_alpm_cmn_banks_get(u) / 2)) & SOC_TH_ALPM_BKT_MASK(u)), 0))
/* given bucket index shift left to bucket entry idx position */
#define SOC_TH_ALPM_BKT_IDX_TO_ENTRY(u, idx)       \
            (ALPM_BKT_IDX(u, idx) << (soc_alpm_cmn_banks_get(u) / 2))
...
extern void soc_th_alpm_bank_disb_get(int u, int vrf, uint32 *bkdis);
