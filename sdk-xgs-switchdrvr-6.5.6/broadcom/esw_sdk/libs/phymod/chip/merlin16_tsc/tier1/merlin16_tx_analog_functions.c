...
err_code_t merlin16_tsc_apply_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2) {
  ...
  uint8_t osr_mode;  
  err_code_t failcode = merlin16_tsc_validate_txfir_cfg(pre, main, post1, post2);
  ...
  if (!failcode) {
    ...
    if (osr_mode == MERLIN16_TSC_OSX2) {
      EFUN(wr_ams_tx_enable_os_2(1));
      EFUN(wr_ams_tx_enable_os_4(0));
    }
    else if (osr_mode == MERLIN16_TSC_OSX4) {
...
err_code_t merlin16_tsc_read_tx_afe(srds_access_t *sa__, enum srds_tx_afe_settings_enum param, int8_t *val) {
  ...
  switch(param) {
    case TX_AFE_PRE:
      EFUN(merlin16_tsc_INTERNAL_get_tx_pre(sa__, val));
      break;
    case TX_AFE_MAIN:
      EFUN(merlin16_tsc_INTERNAL_get_tx_main(sa__, val));
      break;
    case TX_AFE_POST1:
      EFUN(merlin16_tsc_INTERNAL_get_tx_post1(sa__, val));
      break;
    case TX_AFE_POST2:
      EFUN(merlin16_tsc_INTERNAL_get_tx_post2(sa__, val));
      break;
    default:
      return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
      break;
...
err_code_t merlin16_tsc_config_tx_hpf(srds_access_t *sa__, uint8_t val) {  /* KD */
...
err_code_t merlin16_tsc_rd_tx_hpf_config(srds_access_t *sa__, uint8_t *pval) {  /* KD */
