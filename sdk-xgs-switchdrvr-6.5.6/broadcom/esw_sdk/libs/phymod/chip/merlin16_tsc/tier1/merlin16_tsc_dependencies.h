...
/** @file merlin16_tsc_dependencies.h
 * Dependencies to be provided by IP User
 */

#ifndef MERLIN16_TSC_API_DEPENDENCIES_H
#define MERLIN16_TSC_API_DEPENDENCIES_H
 ...
 * If using Serdes MDIO controller to access the registers, implement this function using merlin16_tsc_pmd_mdio_mwr_reg(..)
 *
 * If NOT using a Serdes MDIO controller or the Serdes PMI Masked write feature, please use the following code to
 * implement this function
 *
 *    merlin16_tsc_pmd_wr_reg(addr, ((merlin16_tsc_pmd_rd_reg(addr) & ~mask) | (mask & (val << lsb))));
 *
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_tsc_delay_ns(srds_access_t *sa__, uint16_t delay_ns);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_tsc_delay_us(srds_access_t *sa__, uint32_t delay_us);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_tsc_delay_ms(srds_access_t *sa__, uint32_t delay_ms);
#else
err_code_t merlin16_tsc_delay_ms(uint32_t delay_ms);
#endif
