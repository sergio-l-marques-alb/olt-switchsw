/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin16_tsc_internal.c                                         *
...
uint32_t merlin16_tsc_INTERNAL_mult_with_overflow_check(uint32_t a, uint32_t b, uint8_t *of) {
    uint16_t c,d;
    uint32_t r,s;
    if (a > b) return merlin16_tsc_INTERNAL_mult_with_overflow_check(b, a, of);
    *of = 0;
    c = b >> 16;
    d = UINT16_MAX & b;
    r = a * c;
...
err_code_t merlin16_tsc_INTERNAL_verify_merlin16_tsc_info(merlin16_tsc_info_t const *test_info)
{
    if (test_info->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in merlin16_tsc_info signature.  Expected 0x%08X at index %d, but received 0x%08X.\n",
...
err_code_t merlin16_tsc_INTERNAL_set_p1_threshold(srds_access_t *sa__, int8_t threshold) {
  ...
  EFUN(merlin16_tsc_INTERNAL_check_uc_lane_stopped(sa__));  /* make sure uC is stopped to avoid race conditions */
...
uint8_t merlin16_tsc_INTERNAL_rdb_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    ...
    EPSTM(rddata = merlin16_tsc_rdb_uc_ram(sa__, err_code_p, addr)); /* Use Micro register interface for reading RAM */
    return (rddata);
}
...
uint16_t merlin16_tsc_INTERNAL_rdw_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  ...
  EPSTM(rddata = merlin16_tsc_rdw_uc_ram(sa__, err_code_p, (addr))); /* Use Micro register interface for reading RAM */
  return (rddata);
}
...
err_code_t merlin16_tsc_INTERNAL_wrb_uc_var(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {
    ...
    return (merlin16_tsc_wrb_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}
...
err_code_t merlin16_tsc_INTERNAL_wrw_uc_var(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
    ...
    return (merlin16_tsc_wrw_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}
...
err_code_t merlin16_tsc_INTERNAL_event_log_dump_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_tsc_INTERNAL_event_log_dump_state_t * const state_ptr = (merlin16_tsc_INTERNAL_event_log_dump_state_t *)arg;
...
err_code_t merlin16_tsc_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    if (micro_num >= merlin16_tsc_info_ptr->micro_count) {
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    if (bypass_micro) {
        ...
        if (merlin16_tsc_info_ptr->trace_memory_descending_writes) {
            ++rd_idx;
            if (rd_idx >= merlin16_tsc_info_ptr->trace_mem_ram_size) {
                rd_idx = 0;
            }
        } else {
            if (rd_idx == 0) {
                rd_idx = merlin16_tsc_info_ptr->trace_mem_ram_size;
            }
            --rd_idx;
        }
    } else {
        /* Start Read to stop uC logging and read the word at last event written by uC */
        EFUN(merlin16_tsc_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));
        ESTM(rd_idx = rdcv_trace_mem_rd_idx());
    }
    
    EFUN_PRINTF(( "\n  DEBUG INFO: trace memory read index = 0x%04x\n", rd_idx));
    
    EFUN_PRINTF(("  DEBUG INFO: trace memory size = 0x%04x\n\n", merlin16_tsc_info_ptr->trace_mem_ram_size));
    ...
    if (merlin16_tsc_info_ptr->trace_memory_descending_writes) {
        ...
        EFUN(merlin16_tsc_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  merlin16_tsc_info_ptr->trace_mem_ram_base,
                                                  merlin16_tsc_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  merlin16_tsc_info_ptr->trace_mem_ram_size,
                                                  arg,
         ...
         * So read using ascending addresses using block read
         */
        EFUN(merlin16_tsc_INTERNAL_rdblk_uc_generic_ram_descending(sa__,
                                                             merlin16_tsc_info_ptr->trace_mem_ram_base,
                                                             merlin16_tsc_info_ptr->trace_mem_ram_size,
                                                             rd_idx,
                                                             merlin16_tsc_info_ptr->trace_mem_ram_size,
    ...
    if (!bypass_micro) {
        ...
        EFUN(merlin16_tsc_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));
    }
...
{       
    ...
    uint16_t vl = merlin16_tsc_INTERNAL_ladder_setting_to_mV(sa__, var/8, ladder_range);    
    uint16_t vu = merlin16_tsc_INTERNAL_ladder_setting_to_mV(sa__, SRDS_MIN(31,var/8+1), ladder_range);
    return (vl + (vu-vl)*(var&7)/8);
}

err_code_t merlin16_tsc_INTERNAL_get_osr_mode(srds_access_t *sa__, merlin16_tsc_osr_mode_st *imode) {
    merlin16_tsc_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(merlin16_tsc_osr_mode_st));
...
err_code_t merlin16_tsc_INTERNAL_read_lane_state(srds_access_t *sa__, merlin16_tsc_lane_state_st *istate) {  
  ...
  merlin16_tsc_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(merlin16_tsc_lane_state_st));

  if(!istate) 
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(merlin16_tsc_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  ...
  ENULL_MEMSET(&osr_mode, 0, sizeof(merlin16_tsc_osr_mode_st));
  ...
  EFUN(merlin16_tsc_INTERNAL_sigdet_status(sa__,&state.sig_det, &state.sig_det_chg));
  ...
  EFUN(merlin16_tsc_INTERNAL_get_rx_pf_main(sa__, &state.pf_main));
  ESTM(state.pf_hiz = rd_pf_hiz());
  ESTM(state.pf2_ctrl = rd_pf2_lowp_ctrl());
  EFUN(merlin16_tsc_INTERNAL_get_rx_vga(sa__, &state.vga));
  ESTM(state.dc_offset = rd_dc_offset_bin());
  ESTM(ladder_range = rd_p1_thresh_sel());
  EFUN(merlin16_tsc_INTERNAL_get_p1_threshold(sa__, &state.p1_lvl_ctrl));         
  state.p1_lvl = merlin16_tsc_INTERNAL_ladder_setting_to_mV(sa__, state.p1_lvl_ctrl, ladder_range);
  state.m1_lvl = 0;
   /* ESTM(state.pf_bst = rd_ams_rx_pkbst()); KD */

  EFUN(merlin16_tsc_INTERNAL_get_rx_dfe1(sa__, &state.dfe1));  
  EFUN(merlin16_tsc_INTERNAL_get_rx_dfe2(sa__, &state.dfe2));  
  EFUN(merlin16_tsc_INTERNAL_get_rx_dfe3(sa__, &state.dfe3));  
  EFUN(merlin16_tsc_INTERNAL_get_rx_dfe4(sa__, &state.dfe4));
  EFUN(merlin16_tsc_INTERNAL_get_rx_dfe5(sa__, &state.dfe5));
  ...
  EFUN(merlin16_tsc_INTERNAL_get_tx_pre(sa__, &state.txfir_pre));
  EFUN(merlin16_tsc_INTERNAL_get_tx_main(sa__, &state.txfir_main));
  EFUN(merlin16_tsc_INTERNAL_get_tx_post1(sa__, &state.txfir_post1));
  EFUN(merlin16_tsc_INTERNAL_get_tx_post2(sa__, &state.txfir_post2)); 
  ESTM(state.heye_left = merlin16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(state.heye_right = merlin16_tsc_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(state.veye_upper = merlin16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(state.veye_lower = merlin16_tsc_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));
...
err_code_t merlin16_tsc_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {     
  ...
  EFUN(merlin16_tsc_INTERNAL_read_lane_state(sa__, &state));
 
  lane_idx = merlin16_tsc_get_lane(sa__); 
...
err_code_t merlin16_tsc_INTERNAL_read_core_state(srds_access_t *sa__, merlin16_tsc_core_state_st *istate) {  
  merlin16_tsc_core_state_st state;
  struct merlin16_tsc_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(merlin16_tsc_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct merlin16_tsc_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(merlin16_tsc_read_die_temperature(sa__, &die_temp));
  ...
  EFUN(merlin16_tsc_INTERNAL_read_pll_div(sa__, &state.pll_div));
  EFUN(merlin16_tsc_INTERNAL_pll_lock_status(sa__, &state.pll_lock, &state.pll_lock_chg));
...
err_code_t merlin16_tsc_INTERNAL_display_core_state_no_newline(srds_access_t *sa__) {
  ...
  EFUN(merlin16_tsc_INTERNAL_read_core_state(sa__, &state));
    EFUN_PRINTF(("%02d  "              ,  merlin16_tsc_get_core(sa__)));
    ...
    EFUN(merlin16_tsc_INTERNAL_display_pll_to_divider(sa__, state.pll_div));
...
err_code_t merlin16_tsc_INTERNAL_set_uc_core_config(srds_access_t *sa__, struct merlin16_tsc_uc_core_config_st struct_val) {
    {   uint8_t reset_state;
        ...
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin16_tsc_INTERNAL_set_uc_core_config(..) called without core_dp_s_rstb=0\n"));
    ...
    EFUN(merlin16_tsc_INTERNAL_update_uc_core_config_word(&struct_val));
...
/** Extract the refclk frequency in Hz, based on a merlin16_tsc_pll_refclk_enum value. */
static err_code_t _merlin16_tsc_get_refclk_in_hz(enum merlin16_tsc_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        case MERLIN16_TSC_PLL_REFCLK_50MHZ:          *refclk_in_hz =  50000000; break;
        case MERLIN16_TSC_PLL_REFCLK_125MHZ:         *refclk_in_hz = 125000000; break;
        case MERLIN16_TSC_PLL_REFCLK_156P25MHZ:      *refclk_in_hz = 156250000; break;
        case MERLIN16_TSC_PLL_REFCLK_161P1328125MHZ: *refclk_in_hz = 161132813; break;
        default:
 ...
 */
static err_code_t _merlin16_tsc_get_divisor_ratio(enum merlin16_tsc_pll_div_enum div, uint16_t *numerator, uint16_t *denominator)
{
    switch (div) {
        case MERLIN16_TSC_PLL_DIV_52P751515:  *denominator = 162;  *numerator = 3071; break;
        case MERLIN16_TSC_PLL_DIV_58P181818:  *denominator =  16;  *numerator =  275; break;
        case MERLIN16_TSC_PLL_DIV_62P060606:  *denominator =  53;  *numerator =  854; break;
        case MERLIN16_TSC_PLL_DIV_67P878788:  *denominator =  56;  *numerator =  825; break;
        case MERLIN16_TSC_PLL_DIV_66P460703:  *denominator =  43;  *numerator =  647; break;
        case MERLIN16_TSC_PLL_DIV_66P743079:  *denominator =  58;  *numerator =  869; break;
        case MERLIN16_TSC_PLL_DIV_68P570764:  *denominator =  84;  *numerator = 1225; break;
        case MERLIN16_TSC_PLL_DIV_68P856242:  *denominator =  65;  *numerator =  944; break;
        case MERLIN16_TSC_PLL_DIV_69P152458:  *denominator = 102;  *numerator = 1475; break;
        case MERLIN16_TSC_PLL_DIV_69P389964:  *denominator = 124;  *numerator = 1787; break;
        case MERLIN16_TSC_PLL_DIV_69P818182:  *denominator =  96;  *numerator = 1375; break;
        case MERLIN16_TSC_PLL_DIV_71P112952:  *denominator = 177;  *numerator = 2489; break;
        case MERLIN16_TSC_PLL_DIV_54P4:       *denominator =  34;  *numerator =  625; break;
        case MERLIN16_TSC_PLL_DIV_60:         *denominator =   3;  *numerator =   50; break;
        case MERLIN16_TSC_PLL_DIV_64:         *denominator =   8;  *numerator =  125; break;
        case MERLIN16_TSC_PLL_DIV_66:         *denominator =  33;  *numerator =  500; break;
        case MERLIN16_TSC_PLL_DIV_68:         *denominator =  17;  *numerator =  250; break;
        case MERLIN16_TSC_PLL_DIV_68P537598:  *denominator = 127;  *numerator = 1853; break;
        case MERLIN16_TSC_PLL_DIV_68P828796:  *denominator = 104;  *numerator = 1511; break;
        case MERLIN16_TSC_PLL_DIV_70:         *denominator =   7;  *numerator =  100; break;
        case MERLIN16_TSC_PLL_DIV_70P713596:  *denominator = 106;  *numerator = 1499; break;
        case MERLIN16_TSC_PLL_DIV_71P008:     *denominator = 205;  *numerator = 2887; break;
        case MERLIN16_TSC_PLL_DIV_71P31347:   *denominator = 177;  *numerator = 2482; break;
        case MERLIN16_TSC_PLL_DIV_71P5584:    *denominator = 118;  *numerator = 1649; break;
        case MERLIN16_TSC_PLL_DIV_72:         *denominator =   9;  *numerator =  125; break;
        case MERLIN16_TSC_PLL_DIV_73P335232:  *denominator = 250;  *numerator = 3409; break;
        case MERLIN16_TSC_PLL_DIV_73P6:       *denominator =  46;  *numerator =  625; break;
        case MERLIN16_TSC_PLL_DIV_75:         *denominator =   3;  *numerator =   40; break;
        case MERLIN16_TSC_PLL_DIV_80:         *denominator =   2;  *numerator =   25; break;
        case MERLIN16_TSC_PLL_DIV_82P5:       *denominator =  33;  *numerator =  400; break;
        case MERLIN16_TSC_PLL_DIV_85P671997:  *denominator =  58;  *numerator =  677; break;
        case MERLIN16_TSC_PLL_DIV_86P036:     *denominator = 130;  *numerator = 1511; break;
        case MERLIN16_TSC_PLL_DIV_87P5:       *denominator =   7;  *numerator =   80; break;
        case MERLIN16_TSC_PLL_DIV_88P392:     *denominator =  83;  *numerator =  939; break;
        case MERLIN16_TSC_PLL_DIV_88P76:      *denominator = 199;  *numerator = 2242; break;
        case MERLIN16_TSC_PLL_DIV_89P141838:  *denominator = 243;  *numerator = 2726; break;
        case MERLIN16_TSC_PLL_DIV_89P447998:  *denominator = 128;  *numerator = 1431; break;
        case MERLIN16_TSC_PLL_DIV_90:         *denominator =   9;  *numerator =  100; break;
        case MERLIN16_TSC_PLL_DIV_91P669037:  *denominator = 296;  *numerator = 3229; break;
        case MERLIN16_TSC_PLL_DIV_92:         *denominator =  23;  *numerator =  250; break;
        case MERLIN16_TSC_PLL_DIV_100:        *denominator =   1;  *numerator =   10; break;
        case MERLIN16_TSC_PLL_DIV_170:        *denominator =  17;  *numerator =  100; break;
        case MERLIN16_TSC_PLL_DIV_187P5:      *denominator =   3;  *numerator =   16; break;
        case MERLIN16_TSC_PLL_DIV_200:        *denominator =   1;  *numerator =    5; break;
        case MERLIN16_TSC_PLL_DIV_206P25:     *denominator =  33;  *numerator =  160; break;
        default:
...
/** Get the Refclk frequency in Hz, based on the merlin16_tsc_pll_div_enum value and VCO frequency. */
static err_code_t _merlin16_tsc_get_refclk_from_div_vco(uint32_t *refclk_freq_hz, enum merlin16_tsc_pll_div_enum div, uint32_t vco_freq_khz, uint8_t refclk_doubler_en) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_tsc_get_divisor_ratio(div, &numerator, &denominator));
    *refclk_freq_hz = ((vco_freq_khz+(denominator>>1)) / denominator) * numerator;
    if (refclk_doubler_en > 0) *refclk_freq_hz /= 2;
    return (ERR_CODE_NONE);
}

/** Get the VCO frequency in kHz, based on the reference clock frequency and merlin16_tsc_pll_div_enum value. */
static err_code_t _merlin16_tsc_get_vco_from_refclk_div(uint32_t refclk_freq_hz, enum merlin16_tsc_pll_div_enum div, uint32_t *vco_freq_khz,  uint8_t refclk_doubler_en) {
    uint16_t numerator, denominator;
...
static const enum merlin16_tsc_pll_div_enum _merlin16_tsc_div_candidates[] = {
    MERLIN16_TSC_PLL_DIV_52P751515,
    MERLIN16_TSC_PLL_DIV_54P4,
    MERLIN16_TSC_PLL_DIV_58P181818,
    MERLIN16_TSC_PLL_DIV_60,
    MERLIN16_TSC_PLL_DIV_62P060606,
    MERLIN16_TSC_PLL_DIV_64,
    MERLIN16_TSC_PLL_DIV_66,
    MERLIN16_TSC_PLL_DIV_66P460703,
    MERLIN16_TSC_PLL_DIV_66P743079,
    MERLIN16_TSC_PLL_DIV_67P878788,
    MERLIN16_TSC_PLL_DIV_68,
    MERLIN16_TSC_PLL_DIV_68P537598,
    MERLIN16_TSC_PLL_DIV_68P570764,
    MERLIN16_TSC_PLL_DIV_68P828796,
    MERLIN16_TSC_PLL_DIV_68P856242,
    MERLIN16_TSC_PLL_DIV_69P152458,
    MERLIN16_TSC_PLL_DIV_69P389964,
    MERLIN16_TSC_PLL_DIV_69P818182,
    MERLIN16_TSC_PLL_DIV_70,
    MERLIN16_TSC_PLL_DIV_70P713596,
    MERLIN16_TSC_PLL_DIV_71P008,
    MERLIN16_TSC_PLL_DIV_71P112952,
    MERLIN16_TSC_PLL_DIV_71P31347,
    MERLIN16_TSC_PLL_DIV_71P5584,
    MERLIN16_TSC_PLL_DIV_72,
    MERLIN16_TSC_PLL_DIV_73P335232,
    MERLIN16_TSC_PLL_DIV_73P6,
    MERLIN16_TSC_PLL_DIV_75,
    MERLIN16_TSC_PLL_DIV_80,
    MERLIN16_TSC_PLL_DIV_82P5,
    MERLIN16_TSC_PLL_DIV_85P671997,
    MERLIN16_TSC_PLL_DIV_86P036,
    MERLIN16_TSC_PLL_DIV_87P5,
    MERLIN16_TSC_PLL_DIV_88P392,
    MERLIN16_TSC_PLL_DIV_88P76,
    MERLIN16_TSC_PLL_DIV_89P141838,
    MERLIN16_TSC_PLL_DIV_89P447998,
    MERLIN16_TSC_PLL_DIV_90,
    MERLIN16_TSC_PLL_DIV_91P669037,
    MERLIN16_TSC_PLL_DIV_92,
    MERLIN16_TSC_PLL_DIV_100,
    MERLIN16_TSC_PLL_DIV_170,
    MERLIN16_TSC_PLL_DIV_187P5,
    MERLIN16_TSC_PLL_DIV_200,
    MERLIN16_TSC_PLL_DIV_206P25
};

static const uint8_t _merlin16_tsc_div_candidates_count = sizeof(_merlin16_tsc_div_candidates) / sizeof(_merlin16_tsc_div_candidates[0]);

static err_code_t _merlin16_tsc_check_div(enum merlin16_tsc_pll_div_enum div) {
    uint8_t i, found = 0;
    for (i=0; i<_merlin16_tsc_div_candidates_count; i++) {
        found |= div == _merlin16_tsc_div_candidates[i];
    }
    if (!found) {
...
static err_code_t _merlin16_tsc_check_vco_freq_khz(uint32_t vco_freq_khz) {
...
/** Find the entry out of _merlin16_tsc_div_candidates that is closest to matching refclk_freq_hz and vco_freq_khz. */
static err_code_t _merlin16_tsc_get_div(uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum merlin16_tsc_pll_div_enum *div, uint8_t refclk_doubler_en) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_merlin16_tsc_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(_merlin16_tsc_get_vco_from_refclk_div(refclk_freq_hz, _merlin16_tsc_div_candidates[i], &actual_vco_freq_khz, refclk_doubler_en));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *div = _merlin16_tsc_div_candidates[i];
        }
    }
...
err_code_t merlin16_tsc_INTERNAL_resolve_pll_parameters(enum merlin16_tsc_pll_refclk_enum refclk,
    ...
    const uint8_t given_param_count = (((refclk == MERLIN16_TSC_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*div == MERLIN16_TSC_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    const uint32_t original_vco_freq_khz = *vco_freq_khz;
    enum merlin16_tsc_pll_div_enum auto_div = 0;
    ...
    if (*div != MERLIN16_TSC_PLL_DIV_UNKNOWN) {
        EFUN(_merlin16_tsc_check_div(*div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_merlin16_tsc_check_vco_freq_khz(*vco_freq_khz));
    }

    if (refclk == MERLIN16_TSC_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_merlin16_tsc_get_refclk_from_div_vco(refclk_freq_hz, *div, *vco_freq_khz, refclk_doubler_en));
    } else {
        EFUN(_merlin16_tsc_get_refclk_in_hz(refclk, refclk_freq_hz));
    }

    if (*vco_freq_khz == 0) {
        /* Determine VCO frequency from refclk and divider */
        EFUN(_merlin16_tsc_get_vco_from_refclk_div(*refclk_freq_hz, *div, vco_freq_khz, refclk_doubler_en));
    }
     ...
     */
    EFUN(_merlin16_tsc_get_div(*refclk_freq_hz, *vco_freq_khz, &auto_div, refclk_doubler_en));
    if (*div == MERLIN16_TSC_PLL_DIV_UNKNOWN) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                EFUN(merlin16_tsc_INTERNAL_print_uc_dsc_error(sa__, cmd));
...
err_code_t merlin16_tsc_INTERNAL_rdblk_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_tsc_INTERNAL_rdblk_callback_arg_t * const cast_arg = (merlin16_tsc_INTERNAL_rdblk_callback_arg_t *)arg;
