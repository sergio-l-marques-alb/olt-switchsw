...
err_code_t merlin16_tsc_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    EFUN(merlin16_tsc_INTERNAL_resolve_pll_parameters(refclk, &refclk_freq_hz, &div, &vco_freq_khz, 0));
    ...
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: merlin16_tsc_configure_pll(..) called without core_dp_s_rstb=0\n"));
...
} /* merlin16_tsc_configure_pll */

err_code_t merlin16_tsc_INTERNAL_read_pll_div(srds_access_t *sa__, uint32_t *div) {
    uint16_t ndiv_int;
    uint32_t ndiv_frac;
