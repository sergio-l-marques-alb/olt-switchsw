 ...
 *  Revision      :  $Id: falcon_furia_functions.c 1017 2015-04-24 22:07:57Z kirand $ *
 *                                                                                 *
...
err_code_t falcon_furia_display_eye_scan( const phymod_access_t *pa ) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;

    EFUN(falcon_furia_display_eye_scan_header( pa, 1));

    /* start horizontal acquisition */
    {   err_code_t err_code = falcon_furia_meas_eye_scan_start( pa, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((falcon_furia_meas_eye_scan_done( pa ), err_code));
        }
    }
    ...
    for (y = 62;y>=-62;y=y-2)
    {
        {   err_code_t err_code = falcon_furia_read_eye_scan_stripe( pa, &stripe[0], &status);
            if (err_code) {
                EFUN((falcon_furia_meas_eye_scan_done( pa ), err_code));
            }
        }
        EFUN(falcon_furia_display_eye_scan_stripe( pa, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(falcon_furia_meas_eye_scan_done( pa ));
    EFUN(falcon_furia_display_eye_scan_footer( pa, 1));

    return(ERR_CODE_NONE);
}

/* This function is for Falcon and is configured for passive mode */
err_code_t falcon_furia_meas_lowber_eye( const phymod_access_t *pa, const struct falcon_furia_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(falcon_furia_pmd_uc_cmd_with_data( pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            EFUN(falcon_furia_delay_us(1000));
            ESTM(status=rdv_usr_diag_status());
            EFUN_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0);
#else
        EFUN(falcon_furia_poll_diag_done( pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        EFUN(falcon_furia_prbs_err_count_ll( pa, &errors));
...
err_code_t falcon_furia_display_lowber_eye( const phymod_access_t *pa, const struct falcon_furia_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,i,z;
    ...
    j = 0;
    for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
      ESTM_PRINTF(("%6dmV : ", _ladder_setting_to_mV(y, 0                 )));
...
err_code_t falcon_furia_meas_eye_scan_start( const phymod_access_t *pa, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_read_eye_scan_stripe( const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
    uint32_t val[2] = {0,0};
    uint16_t sts = 0;
    int8_t   i;
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = falcon_furia_poll_diag_eye_data(pa,&val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t falcon_furia_display_eye_scan_stripe( const phymod_access_t *pa, int8_t y,uint32_t *buffer) {
...
err_code_t falcon_furia_display_eye_scan_header( const phymod_access_t *pa, int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t falcon_furia_display_eye_scan_footer( const phymod_access_t *pa, int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
/*eye_scan_status_t falcon_furia_read_eye_scan_status() */
err_code_t falcon_furia_read_eye_scan_status( const phymod_access_t *pa, uint16_t *status) {
...
err_code_t falcon_furia_meas_eye_scan_done( const phymod_access_t *pa ) {
  EFUN(falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t falcon_furia_start_ber_scan_test( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_read_ber_scan_data( const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t falcon_furia_eye_margin_proj(const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(falcon_furia_start_ber_scan_test( pa, ber_scan_mode, timer_control, max_error_control));
    ...
    if(verbose > 5) {
        ...
        EFUN(falcon_furia_poll_diag_done( pa, &sts,timer_control*2000));
    }
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(falcon_furia_read_ber_scan_data( pa,  &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,50));
...
err_code_t falcon_furia_display_lane_state_hdr( const phymod_access_t *pa ) {
  EFUN_PRINTF(("LN (CDRxN  , UC_CFG,RST,STP) "));
  ...
  EFUN_PRINTF(("TXPPM TXEQ(n1,m,p1,2,3)   EYE(L,R,U,D)  "));
  EFUN_PRINTF(("LINK_TIME"));
  EFUN_PRINTF(("\n"));
  return (ERR_CODE_NONE);
}

err_code_t falcon_furia_display_lane_state_legend( const phymod_access_t *pa ) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t falcon_furia_display_lane_state( const phymod_access_t *pa ) {
  err_code_t err_code = _falcon_furia_display_lane_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t falcon_furia_display_core_state_hdr( const phymod_access_t *pa ) {
  EFUN_PRINTF(("CORE RST_ST  PLL_PWDN  UC_ATV   COM_CLK   UCODE_VER  AFE_VER   LIVE_TEMP   AVG_TMON   RESCAL   VCO_RATE  ANA_VCO_RANGE  PLL_DIV    PLL_LOCK\n"));
  return (ERR_CODE_NONE);
}

err_code_t falcon_furia_display_core_state_line( const phymod_access_t *pa ) {
  err_code_t err_code = _falcon_furia_display_core_state_no_newline( pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t falcon_furia_display_core_state_legend( const phymod_access_t *pa ) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t falcon_furia_display_core_config( const phymod_access_t *pa ) {
    uint16_t  vco_mhz, vco_mhz_int, vco_mhz_frec;
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    ...
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        ...
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
            EFUN(falcon_furia_get_uc_core_config( pa, &core_cfg));
        {
    vco_mhz      = (uint16_t)core_cfg.vco_rate_in_Mhz;
    vco_mhz_int  = (vco_mhz / 1000);
    vco_mhz_frec = (vco_mhz % 1000);
    EFUN_PRINTF((    "uC Config VCO Rate   = %d (~%d.%dGHz)\n"                , core_cfg.field.vco_rate
                                                                              , vco_mhz_int
                                                                              , vco_mhz_frec                ));
...
err_code_t falcon_furia_display_lane_config( const phymod_access_t *pa ) {
    ...
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         falcon_furia_get_uc_lane_cfg( pa, &lane_cfg)      );
...
err_code_t falcon_furia_display_core_state( const phymod_access_t *pa ) {
    EFUN(falcon_furia_display_core_state_hdr( pa ));
    EFUN(falcon_furia_display_core_state_line( pa ));
    EFUN(falcon_furia_display_core_state_legend( pa ));
    return ERR_CODE_NONE;
}



err_code_t falcon_furia_display_lane_debug_status( const phymod_access_t *pa ) {
    /* startup */
    ...
    EFUN(falcon_furia_get_usr_ctrl_disable_startup( pa, &ds));
    ...
    EFUN(                                                            falcon_furia_get_usr_ctrl_disable_startup_dfe( pa, &dsd) );
    ...
    EFUN(                                                           falcon_furia_get_usr_ctrl_disable_steady_state( pa, &dss) );
    ...
    EFUN(                                                      falcon_furia_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd) );
...
err_code_t falcon_furia_stop_uc_lane( const phymod_access_t *pa, uint8_t enable) {

  if (enable) {
    return(falcon_furia_pmd_uc_control( pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(falcon_furia_pmd_uc_control( pa, CMD_UC_CTRL_RESUME,50));
  }
}


err_code_t falcon_furia_stop_uc_lane_status( const phymod_access_t *pa, uint8_t *uc_lane_stopped) {
...
err_code_t falcon_furia_stop_rx_adaptation( const phymod_access_t *pa, uint8_t enable) {
...
err_code_t falcon_furia_request_stop_rx_adaptation( const phymod_access_t *pa ) {

  return(falcon_furia_pmd_uc_control_return_immediate( pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t falcon_furia_pmd_uc_cmd_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  err_code_t err_code;
  uint16_t   cmddata;

  err_code = falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1( pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate falcon_furia ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(falcon_furia_pmd_wr_reg(pa,DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */

  return(ERR_CODE_NONE);
}

err_code_t falcon_furia_pmd_uc_cmd( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {

  uint8_t uc_dsc_error_found;

  EFUN(falcon_furia_pmd_uc_cmd_return_immediate( pa, cmd, supp_info));    /* Invoke falcon_furia_uc_cmd and return control immediately */

  EFUN(falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1( pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate falcon_furia ready for command */
...
err_code_t falcon_furia_pmd_uc_cmd_with_data_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data) {
  uint16_t   cmddata;
  err_code_t err_code;
...
err_code_t falcon_furia_pmd_uc_cmd_with_data( const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {

    uint8_t uc_dsc_error_found;

    EFUN(falcon_furia_pmd_uc_cmd_with_data_return_immediate( pa, cmd, supp_info, data)); /* Invoke falcon_furia_uc_cmd and return control immediately */
...
err_code_t falcon_furia_pmd_uc_control_return_immediate( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(falcon_furia_pmd_uc_cmd_return_immediate( pa, CMD_UC_CTRL, (uint8_t) control));
}

err_code_t falcon_furia_pmd_uc_control( const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(falcon_furia_pmd_uc_cmd( pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}

err_code_t falcon_furia_pmd_uc_diag_cmd( const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(falcon_furia_pmd_uc_cmd( pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t falcon_furia_rdbl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
...

    {
        ...
        EPNULL_MEMSET(&uc_lane_info, 0, sizeof(uc_lane_info));
        EPFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EPFUN(falcon_furia_rdb_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));    /* Direct RAM access */
        }
        else {
            ...
            EPFUN(falcon_furia_pmd_uc_cmd( pa, CMD_READ_UC_LANE_BYTE, (uint8_t)addr, 50));          /* Use Serdes FW DSC Command Interface for reading RAM */
...
int8_t falcon_furia_rdbls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) falcon_furia_rdbl_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t falcon_furia_rdwl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  if (addr%2 != 0) {                                                                /* Validate even address */
...

    {
      ...
      EPFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EPFUN(falcon_furia_rdw_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));                    /* Direct RAM access */
      }
      else {
          if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                       /* uC command only support 8-bit addr */
              *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
          ...
          EPFUN(falcon_furia_pmd_uc_cmd( pa, CMD_READ_UC_LANE_WORD, (uint8_t)addr, 50));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t falcon_furia_rdwls_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) falcon_furia_rdwl_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Lane Byte Write */
err_code_t falcon_furia_wrbl_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {


    {
        falcon_furia_uc_lane_info_st uc_lane_info;
        ENULL_MEMSET(&uc_lane_info, 0, sizeof(uc_lane_info));

        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrb_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                      /* uC command only support 8-bit addr */
            ...
            return (falcon_furia_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_BYTE, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Lane Byte Signed Write */
err_code_t falcon_furia_wrbls_uc_var( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (falcon_furia_wrbl_uc_var( pa, addr, wr_val));
}

/* Micro RAM Lane Word Write */
err_code_t falcon_furia_wrwl_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    }
    {
        ...
        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrw_ram(pa, uc_lane_info.lane_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size || addr > 255 ) {                   /* uC command only support 8-bit addr */
            ...
            return (falcon_furia_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_LANE_WORD, (uint8_t)addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Lane Word Signed Write */
err_code_t falcon_furia_wrwls_uc_var( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (falcon_furia_wrwl_uc_var( pa, addr,wr_val));
}
...
uint8_t falcon_furia_rdbc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  if(!err_code_p) {
...

    {
      ...
      EPFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EPFUN(falcon_furia_rdb_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
      }
      else {
          ...
          EPFUN(falcon_furia_pmd_uc_cmd( pa, CMD_READ_UC_CORE_BYTE, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
          EPSTM(rddata = (uint8_t)rd_uc_dsc_data());                                            /* Read_Value read from uc_dsc_data field */
      }
    }
...
int8_t falcon_furia_rdbcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) falcon_furia_rdbc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t falcon_furia_rdwc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  if (addr%2 != 0) {                                                                /* Validate even address */
...

    {
      ...
      EPFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
      if (uc_lane_info.is_direct_ram_access_avail) {
          EPFUN(falcon_furia_rdw_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
      }
      else {
          if (addr >= uc_lane_info.lane_ram_size) {
              *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
          ...
          EPFUN(falcon_furia_pmd_uc_cmd( pa, CMD_READ_UC_CORE_WORD, addr, 10));                   /* Use Serdes FW DSC Command Interface for reading RAM */
...
int16_t falcon_furia_rdwcs_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) falcon_furia_rdwc_uc_var( pa, err_code_p, addr));
}

/* Micro RAM Core Byte Write  */
err_code_t falcon_furia_wrbc_uc_var( const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {
...

    {
        ...
        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrb_ram(pa, uc_lane_info.core_var_base_address+(uint16_t)addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            if (addr >= uc_lane_info.lane_ram_size) {                 
            ...
            return (falcon_furia_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_BYTE, addr, (uint16_t)wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}
...
err_code_t falcon_furia_wrbcs_uc_var( const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (falcon_furia_wrbc_uc_var( pa, addr, wr_val));
}

/* Micro RAM Core Word Write  */
err_code_t falcon_furia_wrwc_uc_var( const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    {   falcon_furia_uc_lane_info_st uc_lane_info;
        ...
        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrw_ram(pa, uc_lane_info.core_var_base_address+addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        } else {
            if (addr >= uc_lane_info.lane_ram_size) {                   
              return (_error(ERR_CODE_INVALID_RAM_ADDR));
            }
            return (falcon_furia_pmd_uc_cmd_with_data( pa, CMD_WRITE_UC_CORE_WORD, addr, wr_val, 10)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}

/* Micro RAM Core Word Signed Write */
err_code_t falcon_furia_wrwcs_uc_var( const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(falcon_furia_wrwc_uc_var( pa, addr,wr_val));
}
...
err_code_t falcon_furia_set_uc_core_config( const phymod_access_t *pa, struct falcon_furia_uc_core_config_st struct_val) {
    {   
#ifndef ATE_LOG
        uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: falcon_furia_set_uc_core_config( pa, ..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n",falcon_furia_get_lane(pa),reset_state));
            return (_error(ERR_CODE_CORE_DP_NOT_RESET));
        }
#endif
    }
    if(struct_val.vco_rate_in_Mhz > 0) {
        struct_val.field.vco_rate = (uint8_t)((struct_val.vco_rate_in_Mhz<<1)/125) - 224;
    }
...
err_code_t falcon_furia_set_usr_ctrl_core_event_log_level( const phymod_access_t *pa, uint8_t core_event_log_level) {
  return(wrcv_usr_ctrl_core_event_log_level(core_event_log_level));
}

err_code_t falcon_furia_set_uc_lane_cfg( const phymod_access_t *pa, struct falcon_furia_uc_lane_config_st struct_val) {
#ifndef ATE_LOG
    uint8_t reset_state;

    ESTM(reset_state = rd_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: falcon_furia_set_uc_lane_cfg( pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",falcon_furia_get_lane(pa),reset_state));
...
err_code_t falcon_furia_set_usr_ctrl_lane_event_log_level( const phymod_access_t *pa, uint8_t lane_event_log_level) {
  return(wrv_usr_ctrl_lane_event_log_level(lane_event_log_level));
}

err_code_t falcon_furia_set_usr_ctrl_disable_startup( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte( pa, &set_val));
  return(wrv_usr_ctrl_disable_startup_functions_word(set_val.word));
}

err_code_t falcon_furia_set_usr_ctrl_disable_startup_dfe( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_dfe_functions_st set_val) {
...
err_code_t falcon_furia_set_usr_ctrl_disable_steady_state( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte( pa, &set_val));
  return(wrv_usr_ctrl_disable_steady_state_functions_word(set_val.word));
}

err_code_t falcon_furia_set_usr_ctrl_disable_steady_state_dfe( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_dfe_functions_byte( pa, &set_val));
...
err_code_t falcon_furia_get_uc_core_config(const phymod_access_t *pa, struct falcon_furia_uc_core_config_st *get_val)
{
...
err_code_t falcon_furia_get_usr_ctrl_core_event_log_level( const phymod_access_t *pa, uint8_t *core_event_log_level) {
...
err_code_t falcon_furia_get_uc_lane_cfg( const phymod_access_t *pa, struct falcon_furia_uc_lane_config_st *get_val) {
...
err_code_t falcon_furia_get_usr_ctrl_lane_event_log_level( const phymod_access_t *pa, uint8_t *lane_event_log_level) {
...
err_code_t falcon_furia_get_usr_ctrl_disable_startup( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t falcon_furia_get_usr_ctrl_disable_startup_dfe( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t falcon_furia_get_usr_ctrl_disable_steady_state( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t falcon_furia_get_usr_ctrl_disable_steady_state_dfe( const phymod_access_t *pa, struct falcon_furia_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t falcon_furia_get_clk90_offset_adjust( const phymod_access_t *pa, int8_t *adjust) {
  if (!adjust) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_furia_set_clk90_offset_adjust( const phymod_access_t *pa, int8_t adjust) {
...
err_code_t falcon_furia_get_clk90_offset_override( const phymod_access_t *pa, uint8_t *override) {
  if (!override) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon_furia_set_clk90_offset_override( const phymod_access_t *pa, uint8_t enable, uint8_t override) {        
...
err_code_t falcon_furia_get_clk90_offset( const phymod_access_t *pa, int8_t *offset) {
...
err_code_t falcon_furia_poll_diag_eye_data(const phymod_access_t *pa,uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1( const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
...
#else
  {   uint16_t rddata;
      EFUN_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(falcon_furia_delay_us(timeout_ms));
      } else {
          EFUN(falcon_furia_delay_ms(timeout_ms));
      }
      EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          EFUN_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      EFUN_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t falcon_furia_tx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(falcon_furia_tx_rptr_mode_timing( pa, enable)); /* Enable jitter filter enable */
    EFUN(_ull_tx_mode_setup( pa, enable));
    return (ERR_CODE_NONE);
}

/* Configure ULL mode for RX */
err_code_t falcon_furia_rx_ull_config( const phymod_access_t *pa, uint8_t enable) {
    EFUN(falcon_furia_rx_rptr_mode_timing( pa, enable));
    EFUN(_ull_rx_mode_setup( pa, enable));
    return (ERR_CODE_NONE);
}

/* TX PI setup for Repeater Mode */
err_code_t falcon_furia_tx_rptr_mode_timing( const phymod_access_t *pa, uint8_t enable) {
        ...
        EFUN(falcon_furia_delay_us(25));                            /* Wait for TX_PI to settle */
    return (ERR_CODE_NONE);
}

/* RX setup for Repeater Mode timing */
err_code_t falcon_furia_rx_rptr_mode_timing( const phymod_access_t *pa, uint8_t enable) {
    if(enable) {
...
err_code_t falcon_furia_reg_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(falcon_furia_pmd_rdt_reg(pa,addr,&rddata));
...
err_code_t falcon_furia_uc_core_var_dump( const phymod_access_t *pa ) {
  uint8_t addr;
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {
    ...
    ESTM_PRINTF(("%02x ", falcon_furia_rdbc_uc_var( pa, __ERR, addr)));
  }
  return (ERR_CODE_NONE);
}


err_code_t falcon_furia_uc_lane_var_dump( const phymod_access_t *pa ) {
  uint8_t     rx_lock, uc_stopped = 0;
  uint16_t    addr;

  falcon_furia_uc_lane_info_st lane_info;

  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",falcon_furia_get_lane(pa)));
  ...
  if (rx_lock == 1) {
      ...
      if (!uc_stopped) {
          EFUN(falcon_furia_stop_rx_adaptation( pa, 1));
      }
  } else {
      EFUN(falcon_furia_pmd_uc_control( pa, CMD_UC_CTRL_STOP_IMMEDIATE,200));
  }

    EFUN(falcon_furia_get_uc_ln_info(pa, &lane_info));

    for (addr = 0x0; addr < lane_info.lane_ram_size; addr++) {
      if (!(addr % 26))  {
        EFUN_PRINTF(("\n%04x ",addr));
      }
      ESTM_PRINTF(("%02x ", falcon_furia_rdbl_uc_var( pa, __ERR, addr)));
    }

  if (rx_lock == 1) {
      if (!uc_stopped) {
          EFUN(falcon_furia_stop_rx_adaptation( pa, 0));
      }
  } else {
      EFUN(falcon_furia_stop_rx_adaptation( pa, 0));
  }
...
err_code_t falcon_furia_display_diag_data( const phymod_access_t *pa, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",falcon_furia_get_core(pa),falcon_furia_get_lane(pa)));
    EFUN(falcon_furia_version( pa, &api_version));
    ...
    if(diag_level & SRDS_DIAG_LANE) {
        struct falcon_furia_detailed_lane_status_st lane_st;
        USR_MEMSET(&lane_st, 0, sizeof(lane_st));
        EFUN(falcon_furia_log_full_pmd_state( pa, &lane_st));
        EFUN(falcon_furia_disp_full_pmd_state( pa, &lane_st,1));
    } else {
        EFUN(falcon_furia_display_lane_state_hdr( pa ));
        EFUN(falcon_furia_display_lane_state( pa ));
    }
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(falcon_furia_read_event_log( pa, trace_mem,EVENT_LOG_HEX));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(falcon_furia_display_eye_scan( pa ));
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(falcon_furia_reg_dump( pa ));
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(falcon_furia_uc_core_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(falcon_furia_uc_lane_var_dump( pa ));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(falcon_furia_display_lane_debug_status( pa ));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_falcon_furia_display_ber_scan_data( pa, ber_mode, timer_control,  err_threshold));
...
/* Required Diagnostic Functions */
err_code_t falcon_furia_diag_access( const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(falcon_furia_pmd_rdt_reg(pa,i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(falcon_furia_pmd_rdt_reg(pa,addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(falcon_furia_pmd_wr_reg(pa,addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", falcon_furia_rdbc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,falcon_furia_rdbc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", falcon_furia_rdbl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,falcon_furia_rdbl_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", falcon_furia_rdwc_uc_var( pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,falcon_furia_rdwc_uc_var( pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", falcon_furia_rdwl_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,falcon_furia_rdwl_uc_var( pa, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = falcon_furia_rdbc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(falcon_furia_wrbc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = falcon_furia_rdbl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(falcon_furia_wrbl_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = falcon_furia_rdwc_uc_var( pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(falcon_furia_wrwc_uc_var( pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = falcon_furia_rdwl_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(falcon_furia_wrwl_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _falcon_furia_rdb_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_falcon_furia_rdb_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _falcon_furia_rdb_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_falcon_furia_wrb_uc_var( pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _falcon_furia_rdw_uc_var( pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_falcon_furia_rdw_uc_var( pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _falcon_furia_rdw_uc_var( pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_falcon_furia_wrw_uc_var( pa, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(falcon_furia_pmd_uc_cmd_with_data( pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_falcon_furia_display_ber_scan_data( pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    case SRDS_EN_BREAKPOINT: 
    case SRDS_GOTO_BREAKPOINT:
    case SRDS_RD_BREAKPOINT:
    case SRDS_DIS_BREAKPOINT:
    default: EFUN_PRINTF(("Invalid request type falcon_furia_diag_access\n"));
    }
...
err_code_t falcon_furia_version( const phymod_access_t *pa, uint32_t *api_version) {

    if(!api_version) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    *api_version = 0xA10203;
    return (ERR_CODE_NONE);
}
...
err_code_t falcon_furia_read_event_log( const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(falcon_furia_event_log_stop( pa ));

    EFUN(falcon_furia_event_log_readmem( pa, trace_mem));

    EFUN(falcon_furia_event_log_display( pa, trace_mem, display_mode));

    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_event_log_stop( const phymod_access_t *pa ) {
    ...
    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));

    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_event_log_readmem( const phymod_access_t *pa, uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
    ...
    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));

    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_event_log_display( const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
        ...
        while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
            ...
            if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                if (!is_last_event_timewrap) {                                                         /* display the rest of previous event info */
                    EFUN_PRINTF((","));
                    EFUN(_falcon_display_event( pa, prev_event,(uint8_t)prev_rr,prev_cursor,curr_cursor,post_cursor,supp_info));
                ...
                EFUN(falcon_furia_uc_lane_idx_to_system_id(pa, uc_lane_id_str, cc));
...
err_code_t falcon_furia_display_state ( const phymod_access_t *pa ) {
    EFUN(falcon_furia_display_core_state( pa ));
    EFUN(falcon_furia_display_lane_state_hdr( pa ));
    EFUN(falcon_furia_display_lane_state( pa ));
    EFUN(falcon_furia_display_lane_state_legend( pa ));
    return(ERR_CODE_NONE);
}

err_code_t falcon_furia_display_config ( const phymod_access_t *pa ) {
    EFUN(falcon_furia_display_core_config( pa ));
    EFUN(falcon_furia_display_lane_config( pa ));
    return(ERR_CODE_NONE);
}
...
err_code_t falcon_furia_force_die_temperature ( const phymod_access_t *pa, int16_t die_temp) {
...
err_code_t falcon_furia_read_die_temperature ( const phymod_access_t *pa, int16_t *die_temp) {

    uint16_t die_temp_sensor_reading;

    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
...
err_code_t falcon_furia_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    EFUN_PRINTF((" Function 'falcon_furia_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
