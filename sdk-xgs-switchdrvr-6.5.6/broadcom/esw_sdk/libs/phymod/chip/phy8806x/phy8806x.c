...
int phy8806x_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    xmod_tx.sys_side = (phy->port_loc == phymodPortLocSys) ? 1 : 0;

    xmod_tx.pre   = tx->pre;
    xmod_tx.main  = tx->main;
    xmod_tx.post  = tx->post;
    xmod_tx.post2 = tx->post2;
    xmod_tx.post3 = tx->post3;
    xmod_tx.amp   = tx->amp;
...
int phy8806x_phy_tx_get(const phymod_phy_access_t* phy, phymod_tx_t* tx)
{
    ...
    buftxptr = (uint8_t *)xmodtxbuff;
    WRITE_XMOD_ARG_BUFF(buftxptr, &xmod_tx, sizeof(xmod_tx_t));
    xmodtxlen = GET_XMOD_ETHERNET_CMD_IN_LEN(XMOD_PHY_TX_GET);
    xmodtxlen = GET_XMOD_BUF_WORD_LEN(xmodtxlen);
    ...
    tx->pre = xmod_tx.pre;
    tx->main = xmod_tx.main;
    tx->post = xmod_tx.post;
    tx->post2 = xmod_tx.post2;
    tx->post3 = xmod_tx.post3;
    tx->amp = xmod_tx.amp;
...
int phy8806x_phy_rx_set(const phymod_phy_access_t* phy, const phymod_rx_t* rx)
{
    ...
    xmod_rx.sys_side = (phy->port_loc == phymodPortLocSys) ? 1 : 0;

    xmod_rx.vga.enable = rx->vga.enable;
    xmod_rx.vga.value = rx->vga.value;

    xmod_rx.num_of_dfe_taps = rx->num_of_dfe_taps; /* number of elements in DFE array */

    for (i = 0; i < XMOD_NUM_DFE_TAPS; i++) {
        xmod_rx.dfe[i].enable = rx->dfe[i].enable;
        xmod_rx.dfe[i].value = rx->dfe[i].value;
    }

    xmod_rx.peaking_filter.enable = rx->peaking_filter.enable;
    xmod_rx.peaking_filter.value = rx->peaking_filter.value;

    xmod_rx.low_freq_peaking_filter.enable = rx->low_freq_peaking_filter.enable;
    xmod_rx.low_freq_peaking_filter.value = rx->low_freq_peaking_filter.value;
...
int phy8806x_phy_rx_get(const phymod_phy_access_t* phy, phymod_rx_t* rx)
{
    ...
    xmodrxlen = GET_XMOD_ETHERNET_CMD_IN_LEN(XMOD_PHY_RX_GET);
    ...
    rv = phy_8806x_xmod_command(soc_core->unit, soc_core->port, XMOD_CMD_MODE_ETH(XMOD_PHY_RX_GET),
                             xmodrxbuff, xmodrxlen, NULL, 0);

    /* retrieve the argument from the xmod rx buffer */
    bufrxptr = (uint8_t *)xmodrxbuff;
    READ_XMOD_ARG_BUFF(bufrxptr, &xmod_rx, sizeof(xmod_rx_t));

    rx->vga.enable = xmod_rx.vga.enable;
    rx->vga.value = xmod_rx.vga.value;

    xmod_rx.num_of_dfe_taps = rx->num_of_dfe_taps = xmod_rx.num_of_dfe_taps; /* number of elements in DFE array */

    for (i = 0; i < XMOD_NUM_DFE_TAPS; i++) {
        rx->dfe[i].enable = xmod_rx.dfe[i].enable;
        rx->dfe[i].value = xmod_rx.dfe[i].value;
    }

    rx->peaking_filter.enable = xmod_rx.peaking_filter.enable;
    rx->peaking_filter.value = xmod_rx.peaking_filter.value;

    rx->low_freq_peaking_filter.enable = xmod_rx.low_freq_peaking_filter.enable;
    rx->low_freq_peaking_filter.value = xmod_rx.low_freq_peaking_filter.value;
