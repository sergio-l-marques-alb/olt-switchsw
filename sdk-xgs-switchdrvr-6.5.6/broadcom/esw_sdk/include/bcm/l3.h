...
#define BCM_L3_OVERLAY_ENCAP            BCM_L3_TGID /* LL encapsulation, used for
                                                      ...
                                                      routed packets */
#define BCM_L3_RPE                      (1 << 10)  /* Pick up new priority
                                                      (COS). */
#define BCM_L3_ENCAP_SPACE_OPTIMIZED    BCM_L3_RPE /* Optimize encapsulation
                                                      database resources. */
#define BCM_L3_IPMC                     (1 << 11)  /* Set IPMC for real IPMC
                                                      entry. */
#define BCM_L3_L2TOCPU                  (1 << 12)  /* Packet to CPU unrouted,
...
#define BCM_L3_HOST_AS_ROUTE            (1 << 16)  /* Use LPM if host table
                                                      full. */
...
#define BCM_L3_SECONDARY                (1 << 21)  /* Secondary L3 interface
                                                      ...
                                                      L3_CASCADED) */
#define BCM_L3_CASCADED                 (1 << 21)  /* Indicates an L3 egress
                                                      interface pointed by
                                                      another L3 egress
                                                      interface (Must NOT be
                                                      used with L3_SECONDARY) */
#define BCM_L3_ROUTE_LABEL              (1 << 22)  /* Indicates that MPLS label
...
#define BCM_L3_KEEP_DSTMAC              (1 << 25)  /* Disable DA replacement for
                                                      L3UC packets. */
#define BCM_L3_KEEP_VLAN                (1 << 26)  /* Disable VLAN replacement
                                                      for L3UC packets. */
#define BCM_L3_KEEP_TTL                 (1 << 27)  /* Disable TTL replacement
                                                      for L3UC packets. */
#define BCM_L3_TRILL_ONLY               (1 << 28)  /* Specific for TRILL Nexthop */
#define BCM_L3_L2GRE_ONLY               (1 << 29)  /* Specific for L2GRE Nexthop */
#define BCM_L3_QUEUE_MAP                (1 << 30)  /* Use queue map to derive
                                                      queue number */
#define BCM_L3_DEREFERENCED_NEXTHOP     (1 << 30)  /* Indicates this is a wider
                                                      L3 entry. */
...
#define BCM_L3_SRC_DST_NAT_REALM_ID     (1 << 28)  /* To set the nat realm id to
                                                      both source nat id and
                                                      destionation nat id. */

/* L3 module flags2. */
#define BCM_L3_FLAGS2_NIV_ENCAP_LOCAL   (1 << 0)   /* Apply VNTAG_ACTION on
                                                      outgoing packet to a HiGig
                                                      Port. */
#define BCM_L3_FLAGS2_FIELD_ONLY        (1 << 1)   /* Specific for Field
                                                      Nexthop. */
#define BCM_L3_FLAGS2_UNDERLAY          (1 << 2)   /* Create underlay next hop
                                                      on a virtual port. */
#define BCM_L3_FLAGS2_SRC_DST_MAC_SWAP  (1 << 3)   /* Swap outgoing packets L2
                                                      header SRC/DST MAC fields,
                                                      for IFP L3 egress objects
                                                      only. */
...
#define BCM_L3_FLAGS2_NO_ECMP_OVERLAP   (1 << 5)   /* Create next hop on the
                                                      index without overlapping
                                                      ECMP indexes */
#define BCM_L3_FLAGS2_SCALE_ROUTE       (1 << 6)   /* Scale the number of
                                                      routes. */
#define BCM_L3_FLAGS2_RAW_ENTRY         (1 << 7)   /* Insert raw data payload */
#define BCM_L3_FLAGS2_SKIP_HIT_CLEAR    (1 << 8)   /* Skip hit clear when
                                                      creating a FEC to enhance
                                                      time performance. */
#define BCM_L3_FLAGS2_ROUTE_NO_MOVE     (1 << 9)   /* A flag only to be used
                                                      with BCM_L3_REPLACE flag
                                                      for avoiding host entries
                                                      move from the route table
                                                      to the host table. */
#define BCM_L3_FLAGS2_MPLS_PHP          (1 << 10)  /* Specific nexthop for MPLS
                                                      PHP. */
...
typedef struct bcm_l3_host_s {
    uint32 l3a_flags;           /* See BCM_L3_xxx flag definitions. */
    uint32 l3a_flags2;          /* See BCM_L3_FLAGS2_xxx flag definitions. */
    bcm_vrf_t l3a_vrf;          /* Virtual router instance. */
...
/* Initialize a bcm_l3_ecmp_member_t structure. */
...
/* Attach already created AGM entry to a l3 ECMP group. */
extern int bcm_l3_ecmp_agm_attach(
    int unit, 
    bcm_if_t l3_ecmp_id, 
    bcm_switch_agm_id_t agm_id);

/* Detach already created AGM entry from a l3 ECMP group. */
extern int bcm_l3_ecmp_agm_detach(
    int unit, 
    bcm_if_t l3_ecmp_id, 
    bcm_switch_agm_id_t agm_id);

/* Retrieve a AGM id attached to a l3 ECMP group. */
