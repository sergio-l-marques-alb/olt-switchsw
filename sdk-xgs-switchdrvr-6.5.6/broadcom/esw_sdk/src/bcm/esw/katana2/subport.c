   ...
   */
    ...
    if (SOC_IS_SABER2(unit)) {
        ...
        for (index = 0; index < SOC_INFO(unit).lp_ports_max; index++) {
            start_index = _BCM_SB2_MAX_INT_STREAMS_PER_LINKPHY_PORT * index;
            end_index   = 
            (_BCM_SB2_MAX_INT_STREAMS_PER_LINKPHY_PORT * index) + 
            _BCM_SB2_MAX_INT_STREAMS_PER_LINKPHY_PORT -1;
...
#if defined(BCM_SABER2_SUPPORT)
        if (SOC_IS_SABER2(unit) && !(SOC_IS_METROLITE(unit))) {
            end = start + (si->lp_streams_per_port_max - 1);
        }
#endif
        SOC_IF_ERROR_RETURN
