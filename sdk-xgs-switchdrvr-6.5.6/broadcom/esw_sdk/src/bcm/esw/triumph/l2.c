...
#ifdef VERDOR_BROADCOM
                        /* TD2PLUS-1974 Workaround: shouldn't set REMOTE_TRUNK
                         * SW learning needs to be used in stacking configurations.
                         */
#endif
                    } else
#endif
...
#ifdef BCM_APACHE_SUPPORT
        if (SOC_IS_APACHE(unit)) {
            rv = soc_apache_hash_bank_info_get(unit, L2Xm, bank,
                                                 &entries_per_bank,
                                                 &entries_per_row,
                                                 &entries_per_bucket,
            ...
            bucket = soc_ap_l2x_bank_entry_hash(unit, bank,
                                                (uint32 *)l2x_entry);
        } else
#endif /* BCM_APACHE_SUPPORT */
#ifdef BCM_TRIDENT2_SUPPORT
                             ...
                             int *cf_count)
{
    ...
    *cf_count = 0;
    ...
    for (bank = 0; bank < num_banks; bank++) {
        ...
        if (SOC_IS_APACHE(unit)) {
            BCM_IF_ERROR_RETURN
                (soc_apache_hash_bank_info_get(unit, L2Xm, bank,
                                                 &entries_per_bank,
                                                 &entries_per_row,
                                                 &entries_per_bucket,
                                                 &bank_base, &bucket_offset));
        ...
        for (slot = 0; slot < entries_per_bucket; slot++) {
            ...
            if (!SOC_IS_TOMAHAWKX(unit) && !SOC_IS_APACHE(unit) && SOC_IS_TD2_TT2(unit)) {
                BCM_IF_ERROR_RETURN
...
_bcm_tr_l2_station_entry_last_sid_set(int unit, int overlay, int sid)
{
    if (overlay) {
        if (sid) {
            last_allocated_sid_2 = sid;
        } else {
            last_allocated_sid_2 = _BCM_L2_STATION_ID_2_BASE - 1;
        }
    } else {
        if (sid) {
            last_allocated_sid = sid;
        } else {
            last_allocated_sid = _BCM_L2_STATION_ID_BASE - 1;
        }
    }
