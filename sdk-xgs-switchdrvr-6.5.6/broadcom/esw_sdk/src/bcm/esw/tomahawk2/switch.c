...
bcm_switch_binding_t tomahawk2_bindings[] =
{
                                ...
                                int arg, int *found)
{
    ...
    *found = 0;
    /* tomahawk2_bindings can only handle switch type that requires configuration in one field 
       within one register. If the switch type need to configure multiple fields, it need to be 
       processed with switch case method, added in the following template:
    ...
    for (i = 0; i < COUNTOF(tomahawk2_bindings); i++) {
        b = &tomahawk2_bindings[i];
        if (b->type == type) {
