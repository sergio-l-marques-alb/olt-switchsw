...
/*     soc_phy_info_t *pi = NULL; */
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) ||
        ...
        /* temp32 = HOST2LE32(1); */ /* Uncomment to turn on retrieving the MT2 log buffer via the "phy mt2 uart" command. */ 
...
phy_8806x_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    *pif = interface_config.interface_type; /* coverity[mixed_enums] */
...
STATIC int _phy8806x_dump_buffer(int unit, int port, char *arg)
{
    ...
    avail_buf_size = (ch[0] | (ch[1] << 4) | (ch[2] << 8) | (ch[3] << 12)) - 8; /* 8 for rounding the AXI buffer */
