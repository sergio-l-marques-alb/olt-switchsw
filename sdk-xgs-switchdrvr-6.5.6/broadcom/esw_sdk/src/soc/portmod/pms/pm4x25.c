        ...
        if(PORTMOD_PBMP_MEMBER(port_phys_in_pm, phy)) {
            if( PM_4x25_INFO(pm_info)->nof_phys[temp] >= 2 ){
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                        _SOC_IF_ERR_EXIT(portmod_xphy_core_access_get(unit, xphy_id, &core_access));
                        _SOC_IF_ERR_EXIT(_pm4x25_core_probe(unit,port, pm_info, &core_access, &probe, 1));
                                     ...
                                     int phy_init_flags)
{
        ...
        if (config->flags & PHYMOD_INTF_F_INTF_PARAM_SET_ONLY) {
            _SOC_IF_ERR_EXIT(
                portmod_port_chain_phy_access_get(
                    unit, port, pm_info, phy_access,(1+MAX_PHYN), &nof_phys));
            ...
            if (nof_phys==1) {
                _SOC_IF_ERR_EXIT(
                    _pm4x25_interface_check(unit, port, config, config->interface, &is_valid));
            } else {
                _SOC_IF_ERR_EXIT(
                    _pm4x25_ext_phy_interface_check(
                        unit, port, &phy_access[nof_phys-1], config, config->interface, &is_valid));
            }
    ...
    if(!(PM_4x25_INFO(pm_info)->in_pm12x10)){
       ...
       _SOC_IF_ERR_EXIT
         ...
         */
        if (!(nof_phys>1 && PORTMOD_PHY_ACC_CMD_FOR_PHY_GET(phy_access, 0))) {
            _SOC_IF_ERR_EXIT
...
int pm4x25_port_update(int unit, int port, pm_info_t pm_info, const portmod_port_update_control_t* update_control)
{
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            _SOC_IF_ERR_EXIT(pm4x25_port_interface_config_get(unit, port, pm_info, &interface_config));
            /* update MAC */
            _SOC_IF_ERR_EXIT(clmac_enable_set(unit, port, 0, 0));
            flags = CLMAC_SPEED_SET_FLAGS_SOFT_RESET_DIS;
