...
int portmod_xphy_wb_buffer_init(int unit, uint32 wb_buffer_index, portmod_xphy_info_t* xphy_core_info)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_VAR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "is_legacy_ext_phy",
                          wb_buffer_index, sizeof(uint32), NULL, VERSION(2));
    _SOC_IF_ERR_EXIT(rv);
...
int portmod_chain_xphy_add_by_index(int unit, int xphy_idx, int xphy_addr, const phymod_core_access_t* core_access )
{
    ...
    /* check for user_acc, if empty, fill it with default. */
    if(NULL == core_access->access.user_acc) {
        local_user_access = sal_alloc(sizeof(portmod_default_user_access_t), "portmod_user_access");
        if(local_user_access == NULL){
            _SOC_EXIT_WITH_ERR(SOC_E_MEMORY, ("portmod_default_user_access NULL parameter"));
        }

        _SOC_IF_ERR_EXIT(portmod_default_user_access_t_init(unit, local_user_access));
        local_user_access->mutex = sal_mutex_create("xphy mutex");
        local_user_access->unit = unit;
        _xphy_info[unit][xphy_idx]->core_access.access.user_acc = local_user_access;
    }
    ...
    if(SOC_WARM_BOOT(unit)){
        ...
        rv = XPHY_LEGACY_PHY_PRESENT_GET(unit,_xphy_info[unit][xphy_idx],
                            &is_legacy_phy_present);
        _SOC_IF_ERR_EXIT(rv);

        if (is_legacy_phy_present) {
            PORTMOD_IS_LEGACY_PHY_SET(
                &_xphy_info[unit][xphy_idx]->core_access.access);
        }
...
int _xphy_resource_release(int unit, int xphy_idx)
{
    ...
    if( xphy_info != NULL ) {
        /* release resource for  user_acc */
        if(NULL != xphy_info->core_access.access.user_acc) {
            
            local_user_access = (portmod_default_user_access_t *)xphy_info->core_access.access.user_acc;
            if(NULL != local_user_access->mutex) {
                sal_mutex_destroy(local_user_access->mutex);
                local_user_access->mutex = NULL;
            }
            sal_free(xphy_info->core_access.access.user_acc);
            xphy_info->core_access.access.user_acc = NULL;
        }
...
int portmod_xphy_core_access_set(int unit, uint32 xphy_addr, phymod_core_access_t *core_access)
{
    int xphy_idx, rv;
    SOC_INIT_FUNC_DEFS;
    ...
    rv = XPHY_LEGACY_PHY_PRESENT_SET(unit,_xphy_info[unit][xphy_idx], PORTMOD_IS_LEGACY_PHY_GET(&core_access->access));
    _SOC_IF_ERR_EXIT(rv);
...
int portmod_xphy_core_access_get(int unit, uint32 xphy_addr, phymod_core_access_t *core_access)
{
    int rv;
    int xphy_idx ;
    int is_legacy_phy;
    ...
    if (is_legacy_phy) {
        ...
        PORTMOD_IS_LEGACY_PHY_CLR( &core_access->access);
    }
...
int unit = 0, port = 0;
    ...
    if (PORTMOD_PHY_ACC_CMD_FOR_PHY_GET(phy_access, 0)) {
                                              ...
                                              portmod_port_ability_t * port_ability)
{
    ...
    /* When PORTMOD_PHY_ACC_CMD_FOR_PHY_GET(phy_access, 0) returns 1,
...
int portmod_port_phychain_autoneg_restart_set(const phymod_phy_access_t* phy_access,
                                                              const int chain_length)
{
    int rv = PHYMOD_E_UNAVAIL;
    int phy_index;
...
phymod_lane_map_t lane_map;
    ...
    while(((rv == PHYMOD_E_NONE) || (rv == PHYMOD_E_UNAVAIL)) && (phy_index >= 0)) {
        ...
        if(phy_index > 0) { /* external phy */
            ...
            SOC_IF_ERROR_RETURN(portmod_xphy_core_access_get(unit, core[phy_index].access.addr, &core_access));
...
int portmod_port_phychain_multi_get(const phymod_phy_access_t* phy_access,
                                    const int chain_length,
