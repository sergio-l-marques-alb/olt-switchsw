...
int portmod_port_legacy_loopback_set(int unit, int port, phymod_loopback_mode_t lb, uint32_t enable) {
    ...
    if (rv != SOC_E_PARAM) {
        ...
        if ((rv == SOC_E_UNAVAIL) && ((control == SOC_PHY_CONTROL_LOOPBACK_INTERNAL) || (control == SOC_PHY_CONTROL_LOOPBACK_PMD))) {
...
int portmod_port_legacy_ability_remote_get(int unit, int port, phymod_autoneg_ability_t* an_ability) {
    ...
    rv = portmod_port_interface_config_get(unit, port, &interface_config);
    if (rv < SOC_E_NONE) {
        return rv;
    }
