 ...
 *
 * $Id: g2p3_em32_internal.h,v 1.15 Broadcom SDK $
 *
...
typedef struct g2p3_em32CompState_s {
  ...
  uint32           preRehashSeed;
  uint8            preRehashSeedBankDirty;
  ...
  uint32 *deviceKeyTable[emTableCount];
  uint32 *keyTableCopy[emTableCount];
  
  /* software copy of table data; in device format */
  uint8  *devicePayloads;
  uint16  payloadSize;
  
  /* Key & Payload data, formatted for DMA */
  uint32 *dmaKeyTable[emTableCount];
  uint32 *dmaPayloadTable[emTableCount];
  uint32 *dmaDupKeyTable[emTableCount];
  ...
  uint16 totEntries;
  uint16 maxEntries;
  uint16 dirtyPaylEntries;
  uint16 cutoffTotalFlush;
  uint16 dirtyEntries[emTableCount][emBankCount];
  uint8  forceFlush[emBankCount]; /* Force flush on top/ or bottom 
                                     * of current bank */
