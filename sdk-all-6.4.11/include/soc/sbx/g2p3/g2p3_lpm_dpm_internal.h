...
/*
 * $Id: g2p3_lpm_dpm_internal.h,v 1.5 Broadcom SDK $
 *
...
typedef struct g2p3_lpmDPM_s {
    ...
    uint32 dmaBankA;                    /* which SRAM bank to write to */
    uint32 dmaBankB;
    uint32 numBanks;                    /* number of banks for payload */
    uint32 wordsPerDevicePayload;       /* how many words per payload */ 
    ...
    uint32 nextWord;           /* next word of current payload */
    uint32 dmaBuf[1];          /* buffer for packing payloads */
} g2p3_lpmDPM_t;

#define PAYLOAD_MGR_SIZE(nDmaBufWords) \
    (sizeof(g2p3_lpmDPM_t) + sizeof(uint32) * ((nDmaBufWords) - 1))
...
typedef struct g2p3_lpmDevicePayload_s {
    ...
    uint32 refs;                       /* number of references */
    g2p3_lpmSramAddr address;           /* where it lives in device memory */
    uint16 extraBits;                  /* extra ten bits to be stored
                                            in payload node */
