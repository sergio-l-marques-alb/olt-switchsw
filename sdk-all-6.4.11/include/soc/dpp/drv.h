...
#define SOC_DPP_IP_TUNNEL_TERM_DB_DIP_SIP_VRF (64) /* IP tunnel with {dip, sip, vrf, IP.protocol, GRE.ethertype, is-tunnel-gre} as key. */
...
typedef struct soc_dpp_config_pp_s {
  ...
  uint8 egress_membership_mode; /* Relevant for Jericho-B0 and above, 0 = VSI (default), 1 = VLAN. In 6.5.x default is VLAN */
  uint8 outer_vid_existence_class_mode; /* Relevant for Jericho-B0 in 6.4.x only. 0 = Legacy mode(default), 1 = New mode */
  /* pwe vid search */
  ...
  SOC_PPD_AC_ID     mim_local_out_lif; 
  ...
  uint8   sys_rsrc_mgmt;          /* system resource management: 0 all global. 1: advanced */
    ...
    uint8   oam_use_event_fifo_dma; /* Relevant only to Jericho */
    uint8   oam_use_report_fifo_dma; /* Relevant only to Jericho */
    uint32 oam_dma_threshold; /* Relevant only to ARAD+ */
    uint32 oam_dma_event_threshold; /* Relevant only to Jericho */
  ...
  uint8   mldp_support;
  SOC_PPD_LIF_ID global_mldp_dummy_lif_id[2]; /* In-LIF-ID of IN-LIF-dummy for temination of mLDP,must be set if mLDP is support */
