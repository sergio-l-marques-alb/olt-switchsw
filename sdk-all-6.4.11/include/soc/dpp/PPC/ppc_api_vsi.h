...
typedef struct
{
   ...
   *  ID). SOC_PPD_VSI_FID_IS_VSID: indicates that the FID is the
   *  VSID. Other profiles enable shared learning, by calling
   *  soc_ppd_frwrd_mact_fid_profile_to_fid_map_set()In T20E:
   *  Must be set to 'SOC_PPD_VSI_FID_IS_VSID'
   *  IF fid_profile_id != SOC_PPD_VSI_FID_IS_VSID, then this FID-
   ...
   *  
   *  When fid_profile_id is not 'SOC_PPD_VSI_FID_IS_VSID', then the
...
typedef struct
{
  SOC_SAND_MAGIC_NUM_VAR
  /*
   *  The use of this structure is for Soc_petra-B only. Hub or
   *  Spoke. The user may set different defaults for each
   *  orientation.
   */
  ...
  /*
   *  User may set up to 4 different profiles.set Per VSI, see
   *  soc_ppd_vsi_info_set()Range: 0 - 3.
   */
  uint32 default_forward_profile;

} SOC_PPC_VSI_DEFAULT_FRWRD_KEY;
