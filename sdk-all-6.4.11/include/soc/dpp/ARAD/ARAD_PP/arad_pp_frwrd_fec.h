...
typedef struct
{
  ARAD_PP_FEC_TYPE fec_entry_type[SOC_DPP_DEFS_MAX(NOF_FECS)];
...
/*********************************************************************
* NAME:
 *   arad_pp_frwrd_fec_table_dma
 * FUNCTION:
 *   Trigger DMA block operation to insert FEC entries (to PPDB_A_FEC_SUPER_ENTRY_BANK table) stored in the shadow.
 * INPUT:
 *  unit id
 *  start_index: start DMA from the specified index (of PPDB_A_FEC_SUPER_ENTRY_BANK)
 *  nof_entries: number of entries (of PPDB_A_FEC_SUPER_ENTRY_BANK) to be used in the DMA block
 * REMARKS:
 *  FEC table shadow must be allocated in DMA-able memory.
 *  This is doen when SOC property custom_feature_mem_cache_dma is set to '1' PPDB_A_FEC_SUPER_ENTRY_BANK caching is enabled.
 *
*********************************************************************/
