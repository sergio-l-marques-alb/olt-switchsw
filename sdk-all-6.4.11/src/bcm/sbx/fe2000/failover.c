/*
 * $Id: failover.c,v 1.10 Broadcom SDK $
...
int
_bcm_fe2000_failover_hier_set(int unit, bcm_failover_t failover_id, int enable)
{
    int rv = BCM_E_NONE;
...
int 
_bcm_fe2000_failover_hier_get(int unit, bcm_failover_t failover_id, int *enable)
{
    int rv = BCM_E_NONE;
    soc_sbx_g2p3_rr_hier_t rr_hier;
...
bcm_fe2000_failover_set(int unit, bcm_failover_t failover_id, int enable)
{
    ...
    rr_size = soc_sbx_g2p3_rr_table_size_get(unit);

    if (failover_id > rr_size) {
        failover_id = failover_id - rr_size;
        return _bcm_fe2000_failover_hier_set(unit, failover_id, enable);
    }
    rv = _sbx_gu2_resource_test(unit, SBX_GU2K_USR_RES_PROTECTION,
                                failover_id);
    if (rv != BCM_E_EXISTS) {
...
bcm_fe2000_failover_get(int unit, bcm_failover_t failover_id, int *enable)
{
    ...
    if (failover_id > rr_size) {
        failover_id = failover_id - rr_size;
        return _bcm_fe2000_failover_hier_get(unit, failover_id, enable);
    }
