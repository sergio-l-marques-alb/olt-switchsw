/*
 * $Id: subport.c,v 1.35 Broadcom SDK $
...
_bcm_tr2_subport_gport_used(int unit, bcm_gport_t port)
{
    ...
    if (BCM_GPORT_IS_SUBPORT_GROUP(port)) {
        ...
        if ((vp < 0) || (vp > (_TR2_SUBPORT_NUM_VP - 8))) {
                         ...
                         bcm_gport_t *port)
{
   ...
   /* *********************************************************** */
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, VLAN_ACTION_VALIDf)) {
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, VLAN_ACTION_VALIDf, 1);
    }
    ...
    if ((soc_feature(unit, soc_feature_lltag)) &&
        ...
        if (BCM_SUBPORT_MATCH_PKT_VID == config->criteria) {
            ...
            soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__IVIDf, 
                                        config->inner_vlan);
        } else if (BCM_SUBPORT_MATCH_PKT_VID_OUTER_VLAN == config->criteria) {
            soc_VLAN_XLATEm_field32_set(unit, &ing_vent, KEY_TYPEf,
                                        KT2_PT_XLT_KEY_TYPE_LLVID_OVID);
            soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__OVIDf, 
                                        config->outer_vlan);
        }   

        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__LLVIDf, config->pkt_vlan);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__MODULE_IDf, modid);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__PORT_NUMf, local_port);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__TAG_ACTION_PROFILE_PTRf, 
                                    ing_profile_idx);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__NEW_OVIDf, ovid);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__NEW_OPRIf, config->int_pri);
        /* Group VP */
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__SOURCE_VPf, vp_base); 
    } else {
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, KEY_TYPEf, key_type_value);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, OVIDf, config->pkt_vlan);
...
_bcm_tr2_subport_port_delete(int unit, int l3_idx, int vp)
{
    ...
    if ((soc_feature(unit, soc_feature_lltag)) &&
        ...
        if (config.criteria == BCM_SUBPORT_MATCH_PKT_VID) {
            soc_VLAN_XLATEm_field32_set(unit, &ing_vent, KEY_TYPEf,
                    KT2_PT_XLT_KEY_TYPE_LLVID);
            soc_VLAN_XLATEm_field32_set(unit, &ing_vent, LLTAG__LLVIDf, 
                    config.pkt_vlan);
        ...
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, KEY_TYPEf, 
                    key_type_value);
        soc_VLAN_XLATEm_field32_set(unit, &ing_vent, OVIDf, pkt_vlan);
    }
...
_bcm_tr2_subport_port_get(int unit, int l3_idx,
                         bcm_subport_config_t *config)
{
    int vp=-1, rv, vp_base, vt_index, nh_index=-1, nh_group=-1,  group_en = 0, nh_profile_idx=0;
    egr_l3_intf_entry_t l3_intf;
    egr_l3_next_hop_entry_t egr_nh;
    nhi_group_tc_profile_entry_t nhi_entry;
    ing_dvp_table_entry_t dvp;
    ...
    if (soc_feature(unit, soc_feature_subport_enhanced)) {
        ...
        if (soc_feature(unit, soc_feature_lltag) && (group_en == 1)) {
            ...
            config->pkt_vlan = soc_EGR_VLAN_XLATEm_field32_get(unit, &egr_vent, 
                                                NEW_OTAG_VPTAGf);
        }
	}	
