 ...
 */
STATIC int
_bcm_td2_alpm_128_clear_hit(int u, void *key_data, int index)
{
    soc_mem_t   mem;
 ...
 */
STATIC int
_bcm_td2_alpm_clear_hit(int u, void *key_data, int index)
{
    int         ipv6;
    soc_mem_t   mem;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    ...
    mem = (ipv6) ? L3_DEFIP_ALPM_IPV6_64m : L3_DEFIP_ALPM_IPV4m;
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            if (ipv6 && tmp_idx) {
                ...
                if ((vrf_id == SOC_L3_VRF_OVERRIDE) || (ipmc_route)) {
                    ...
                    _bcm_td2_lpm_ent_parse(unit, &lpm_cfg, &nh_ecmp_idx, 
                                           lpm_entry);
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            if (tmp_idx) {
                ...
                if ((vrf_id == SOC_L3_VRF_OVERRIDE) || (ipmc_route)) {
                    ...
                    sal_memset(&lpm_cfg, 0, sizeof(_bcm_defip_cfg_t));
                    (void) _td2_defip_pair128_get_key(unit, (uint32 *) lpm_entry, 
                                                      &lpm_cfg);
                    (void) soc_alpm_128_lpm_vrf_get(unit, lpm_entry, 
                                                    &lpm_cfg.defip_vrf, 
                                                    &nh_ecmp_idx);
                    (void) _bcm_td2_alpm_128_ent_parse(unit, L3_DEFIP_PAIR_128m, 
                                                       (uint32 *) lpm_entry, 
                                                       &lpm_cfg, &nh_ecmp_idx);
                    lpm_cfg.defip_index = idx;
                    if (trv_data->op_cb) {
            ...
            for (bkt_idx = 0; bkt_idx < bkt_count; bkt_idx++) {
                ...
                /* Parse the entry. */
                _bcm_td2_alpm_128_ent_parse(unit, L3_DEFIP_PAIR_128m, 
                                            (uint32 *) lpm_entry, 
                                            &lpm_cfg, &nh_ecmp_idx);
...
_bcm_td2_alpm_128_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (idx = (idx_end - 1); idx >= idx_start; idx--) {    
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            if (bkt_ptr == 0) {
                ...
                _bcm_td2_lpm_128_ent_parse(unit, &lpm_cfg, &nh_ecmp_idx, lpm_entry);
            ...
            for (bkt_idx = 0; bkt_idx < bkt_count; bkt_idx++) {
                ...
                _soc_alpm_raw_mem_read(0, alpm_mem, raw_entry, entry_num, alpm_entry);
                ...
                phy_idx = SOC_ALPM_128_ADDR_LWR(phy_idx ) << 1;
    ...
    for (idx = 0; idx < def_arr_idx; idx++) {
        ...
        raw_entry = soc_mem_table_idx_to_pointer(unit, L3_DEFIP_ALPM_RAWm,
                defip_alpm_raw_entry_t *, alpm_tbl_ptr, bkt_addr & 0xFFFF);
        _soc_alpm_raw_mem_read(0, alpm_mem, raw_entry, bkt_addr >> 16, alpm_entry);
...
_bcm_td2_alpm_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (idx = (idx_end - 1); idx >= idx_start; idx--) {
        ...
        for (tmp_idx = tmp_start; tmp_idx != tmp_end; tmp_idx += tmp_delta) {
            ...
            if ((lpm_cfg.defip_vrf == BCM_L3_VRF_OVERRIDE) || (ipmc_route)) {
...
_bcm_td2_alpm_find(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx)
{
    ...
    case L3_DEFIP_PAIR_128m:
        ...
        _bcm_td2_alpm_128_ent_parse(unit, L3_DEFIP_PAIR_128m, 
                                    ...
                                    nh_ecmp_idx);
        /* OVERRIDE VRF Processing */ 
        _td2_defip_pair128_get_key(unit, (uint32 *)&lpm_128_entry, lpm_cfg);
...
bcm_td2_ecmp_rh_init(int unit)
{
    ...
    alloc_size = num_ecmp_rh_flowset_entries * sizeof(rh_ecmp_flowset_entry_t);
    ...
    index_min = 0;
    index_max = index_min + num_ecmp_rh_flowset_entries - 1;
