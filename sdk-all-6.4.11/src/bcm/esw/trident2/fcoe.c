                               ...
                               bcm_if_t nh_idx)
{
    ...
    if (rv == SOC_E_NONE) {
        ...
        if (soc_mem_field32_get(unit, mem, lookup, FCOE__ECMPf)) {
            ...
            BCM_IF_ERROR_RETURN(bcm_xgs3_ecmp_group_del(unit, old_nh_idx));
        } else {
...
_bcm_td2_fcoe_route_add_prefix(int unit, bcm_fcoe_route_t *route)
{
    ...
    if (BCM_FAILURE(rv)) {
        if (route->flags & BCM_FCOE_MULTIPATH) {
            (void) bcm_xgs3_ecmp_group_del(unit, nh_idx);
        } else {
                                    ...
                                    soc_mem_t mem, uint32 *lookup) 
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, mem, FCOE__NEXT_HOP_INDEXf)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, FCOE__ECMPf) &&
            ...
            rv = bcm_xgs3_ecmp_group_del(unit, next_hop_index);
        } else {
                                  ...
                                  bcm_fcoe_route_t *route, int flag)
{
    ...
    if (flag == FCOE_ROUTE_READ_RETURN_NHID) {
        if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
            if (route->flags & BCM_FCOE_MULTIPATH) {
                route->intf = nexthop + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
            } else {
                route->intf = nexthop + BCM_XGS3_EGRESS_IDX_MIN;
            }
            return BCM_E_NONE;
        }
    }
...
_bcm_td2_fcoe_route_find_defip(int unit, bcm_fcoe_route_t *route)
{
    ...
    if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
        if (route->flags & BCM_FCOE_MULTIPATH) {
            route->intf = nh_idx + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
        } else {
            route->intf = nh_idx + BCM_XGS3_EGRESS_IDX_MIN;
        }
    }
    return BCM_E_NONE;
}
...
_bcm_td2_fcoe_sw_dump(int unit)
{
    ...
    for (vsan_id = 0; vsan_id <= FCOE_MAX_VSAN_ID; vsan_id++) {
        if ((vsan_id % 16) == 0) {
            LOG_CLI((BSL_META_U(unit,
                                "Vsan ID (from %04d): "), vsan_id)); 
        }
        LOG_CLI((BSL_META_U(unit,
                            "%d "), _bcm_td2_vsan_id_creation_state_get(unit, vsan_id)));
        if ((vsan_id %16) == 15) {
