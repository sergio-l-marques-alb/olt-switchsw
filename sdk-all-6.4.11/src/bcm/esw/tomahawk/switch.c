...
STATIC int
_th_agm_stat_get_table_info(int unit, bcm_switch_agm_id_t agm_id,
...
STATIC int
_th_agm_stat_id_get(int unit, bcm_switch_agm_id_t agm_id,
                    ...
                    uint32 *stat_counter_id)
{
    ...
    if (attr->period_num > 0) {
        total_cnt = attr->num_members * attr->period_num;
    } else {
        total_cnt = attr->num_members;
    }

    stat_config.flags = BCM_STAT_GROUP_MODE_INGRESS;
    stat_config.total_counters = total_cnt;
...
STATIC int
_th_agm_stat_id_clear(int unit, uint32 stat_counter_id)
{
    BCM_IF_ERROR_RETURN(bcm_esw_stat_group_destroy(unit, stat_counter_id));
    return BCM_E_NONE;
}
...
STATIC int
_th_agm_stat_detach(int unit, bcm_switch_agm_id_t agm_id)
{
    soc_mem_t                  table;
    ...
    BCM_IF_ERROR_RETURN(_th_agm_stat_get_table_info(unit, agm_id,
    ...
    for (count = 0; count < num_of_tables; count++) {
        if ((table_info[count].direction == direction) &&
            ...
            if (direction == bcmStatFlexDirectionIngress) {
                return _bcm_esw_stat_flex_detach_ingress_table_counters(
...
STATIC int
_th_agm_stat_attach(int unit, bcm_switch_agm_id_t agm_id, uint32 counter_id)
{
    soc_mem_t                  table;
    bcm_stat_flex_direction_t  direction;
                         ...
                         bcm_switch_agm_info_t *agm_info)
{
    ...
    rv = _th_agm_stat_id_get(unit, agm_id, agm_info, &counter_id);
    if (BCM_FAILURE(rv)) {
        AGM_UNLOCK(unit);
        return rv;
    }
    ...
    rv = _th_agm_stat_attach(unit, agm_id, counter_id);
    if (BCM_FAILURE(rv)) {
        _th_agm_stat_id_clear(unit, counter_id);
        AGM_UNLOCK(unit);
        return rv;
    }
    ...
    if (BCM_FAILURE(rv)) {
        (void)_th_agm_stat_detach(unit, agm_id);
        (void)_th_agm_stat_id_clear(unit, counter_id);
        AGM_UNLOCK(unit);
        return rv;
    }
                             ...
                             int enable)
{
    ...
    if (enable && AGM_INFO(unit, agm_id).attr.period_num > 0) {
                           ...
                           bcm_switch_agm_stat_t *stat_arr)
{
    ...
    rv = _th_agm_stat_get_table_info(unit, agm_id, &num_of_tables, &table_info[0]);
    if (BCM_FAILURE(rv)) {
        goto _free_exit;
    }
    ...
    if (table_info[0].direction == direction) {
        ...
        for (i = 0; i < nstat; i++) {
            rv = _bcm_esw_stat_counter_get(unit, sync_mode, table_info[0].index,
                    table_info[0].table, byte_flag, counter_indexes[i],
                    &counter_values[i]);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_SWITCH, (BSL_META_U(unit,
...
bcm_th_switch_agm_destroy(int unit, bcm_switch_agm_id_t agm_id)
{
    ...
    rv = _th_agm_stat_detach(unit, agm_mnt->attr.agm_id);
    if (BCM_FAILURE(rv)) {
        AGM_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_SWITCH, (BSL_META_U(unit,
    ...
    rv = _th_agm_stat_id_clear(unit, agm_mnt->counter_id);
    if (BCM_FAILURE(rv)) {
        AGM_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_SWITCH, (BSL_META_U(unit,
