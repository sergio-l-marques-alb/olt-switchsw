...
#if defined(BCM_RIOT_SUPPORT) && defined(BCM_TRIDENT2PLUS_SUPPORT)
        /* Check for Egress-Tunnel 
         * in case of RIOT, DVP is not set as network port for vxlan.
         * read the flag status from DVP_ATTRIBUTE_TABLE */     
        if ((BCMI_RIOT_IS_ENABLED(unit))) {
            BCM_IF_ERROR_RETURN(soc_mem_read(unit, EGR_DVP_ATTRIBUTEm,
                                            MEM_BLOCK_ALL, vp,
                                            &egr_dvp_attribute));
            ...
            if (soc_mem_field_valid(unit, EGR_DVP_ATTRIBUTEm, VXLAN__DVP_IS_NETWORK_PORTf)
                ...
                egress_tunnel_flag = soc_mem_field32_get(unit, EGR_DVP_ATTRIBUTEm,
                                                &egr_dvp_attribute,
