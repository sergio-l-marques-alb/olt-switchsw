                         ...
                         soc_field_t *result_name)
{
    ...
    vp_type = soc_EGR_DVP_ATTRIBUTEm_field32_get(unit,
                (egr_dvp_attribute_entry_t *)dvp_entry, VP_TYPEf);
    switch (legacy_name) {
        case EN_EFILTERf:
        ...
        for (i = index_min; i <= index_max; i++) {
            ...
            vlan_mpls_entry =
                soc_mem_table_idx_to_pointer(unit, VLAN_MPLSm,
                                             ...
                                             vlan_mpls_buf, i);
            profile_ptr =
                soc_VLAN_MPLSm_field32_get(unit, vlan_mpls_entry,
...
bcm_td2p_vp_group_sw_dump(int unit)
{
    ...
    for (i = 0; i < VP_GROUP_BK(unit)->num_ing_vp_group; i++) {
        cli_out("\n  Ingress Group = %d\n", i);
        cli_out("    GPP_VP Count = %d\n", ING_VP_GROUP(unit, i)->gpp_vp_count);
        cli_out("    STP State = %d\n", ING_VP_GROUP(unit, i)->stp_state);
        cli_out("    Untag State = %d\n", ING_VP_GROUP(unit, i)->ut_state);
    ...
    for (i = 0; i < VP_GROUP_BK(unit)->num_eg_vp_group; i++) {
        ...
        cli_out("    STP State = %d\n", EG_VP_GROUP(unit, i)->stp_state);
        cli_out("    Untag State = %d\n", EG_VP_GROUP(unit, i)->ut_state);
...
_bcm_td2p_vp_vlan_vfi_bitmap_get(int unit, bcm_gport_t vp_gport, SHR_BITDCL *vlan_vfi_bitmap)
{
    ...
    for (i = 0; i < vlan_vfi_count; i++) {
        if (i < BCM_VLAN_COUNT) {
            ...
            mc_index_array[0] = soc_VLAN_TABm_field32_get(
                                   unit, vlan_tab_entry, BC_IDXf);
            mc_index_array[1] = soc_VLAN_TABm_field32_get(
                                    unit, vlan_tab_entry, UMC_IDXf);
            mc_index_array[2] = soc_VLAN_TABm_field32_get(
                                            ...
                                            uint32 *prof_index)
{
    ...
    *prof_index = soc_mem_field32_get(unit, mem,
                vlan_vfi_entry, MEMBERSHIP_PROFILE_PTRf);
                            ...
                            int stp_state, int ut_state, int egress, int *vp_group)
{
    ...
    *vp_group = vpg;
        ...
        SHR_BIT_ITER(vlan_vfi_bmap, vlan_vfi_count, vlan_vfi) {
            ...
            if(stg != BCM_STG_DEFAULT) {
                ...
                BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_stp_state_set(
                                unit, stg, gport, egress, stp_state));
            }
        }
    }

    BCM_IF_ERROR_RETURN(_bcm_td2p_vp_group_resolve_port_num(
                                    unit, port, &port_offset));
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_td2p_vp_group_id_set(unit, port, vpg, egress));
        ...
        if(egress) {
            ...
            */
            BCM_IF_ERROR_RETURN(bcm_esw_port_egr_lport_field_get(
                            unit, port, EGR_LPORT_PROFILE_LPORT_TAB,
    ...
    if (BCM_GPORT_IS_SUBPORT_PORT(port)) {
        ...
        if (egress) {
            BCM_IF_ERROR_RETURN(bcm_esw_port_egr_lport_field_set(unit, port,
                            EGR_LPORT_PROFILE_LPORT_TAB, filter_field, filter));
        } else {
...
bcm_td2p_vp_group_vlan_vpn_stg_set(int unit, int vlan_vpn, int egress, int stg)
{
    ...
    return soc_mem_field32_modify(unit, mem, index, STGf, stg);
}
...
bcm_td2p_vp_vlan_member_set(int unit, bcm_gport_t gport, uint32 flags)
{
    ...
    if (flags & BCM_PORT_VLAN_MEMBER_VP_VLAN_MEMBERSHIP) { /* hash table */
        ...
        if (flags & BCM_PORT_VLAN_MEMBER_EGRESS) {
            if (filter_disable == old_efilter) {
                BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_port_filter_set(
                                    unit, gport, TRUE, filter_hash_table));
            }
        } else {
           if (filter_hash_table == old_efilter) {
                BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_port_filter_set(
        ...
        if (((flags & BCM_PORT_VLAN_MEMBER_INGRESS) &&
                ing_vpg_managed && (filter_disable == old_ifilter)) ||
                ((flags & BCM_PORT_VLAN_MEMBER_EGRESS) &&
                egr_vpg_managed && (filter_disable == old_efilter))) {
            vlan_vfi_count = BCM_VLAN_COUNT + soc_mem_index_count(unit, VFIm);
        ...
        if (flags & BCM_PORT_VLAN_MEMBER_EGRESS) {
            if (filter_disable == old_efilter) {
                if (egr_vpg_managed) {
                    rv = _bcm_td2p_vp_group_join(
                        unit, gport, vlan_vfi_bitmap, _BCM_DEFAULT_STP_STATE,
                        _BCM_DEFAULT_UNTAGGED_STATE, TRUE, &vp_group);
                    VP_GROUP_CLEANUP(rv);
                }

                /* Enable egress vp group filtering */
                rv = bcm_td2p_vp_group_port_filter_set(
...
{
    int cur_vp_group = 0;
    int filter = 0, filter_hash_table, filter_vp_grp;
                          ...
                          int old_profile_index, int *profile_index)
{
    ...
    int vfi_profile_flags_map[][3] = {
            ...
            {IPMCV4_L2_ENABLEf,    BCM_VLAN_IP4_MCAST_L2_DISABLE,   1},
            {IPMCV6_L2_ENABLEf,    BCM_VLAN_IP6_MCAST_L2_DISABLE,   1},
            {LEARN_DISABLEf,       BCM_VLAN_LEARN_DISABLE,          0},
            {INVALIDf,             0,                               0}
        };

    if (_BCM_INVALID_VFI_PROFILE != old_profile_index) {
        BCM_IF_ERROR_RETURN(soc_mem_read(unit, VFI_PROFILEm,
            MEM_BLOCK_ANY, old_profile_index, &vfi_profile_entry));
    } else {
