...
#define _CTR_COUNTER_TABLE_IFP(_unit, _dir, _pipe_num, _pool, _mem) {        \
    if (!SOC_IS_TOMAHAWK(unit) || _bcm_esw_get_fp_mode_global(_unit,        \
...
_bcm_esw_get_max_pool_groups(int unit) {
    ...
    return ((SOC_IS_TOMAHAWK(unit)) ?  BCM_STAT_TH_FLEX_COUNTER_MAX_GROUPS: 
    ...
    soc_field_t *base_idx_field)
{
    ...
    *base_idx_field=FLEX_CTR_BASE_COUNTER_IDXf;
    ...
    case EGR_IP_TUNNEL_MPLSm:
         if (soc_mem_field32_get(unit, table, data,
                                    MPLS_LABEL_0f) > 0) {
             key_type_index = 0;
         } else if (soc_mem_field32_get(unit, table, data,
                                    MPLS_LABEL_1f) > 0) {
             key_type_index = 1;
         } else if (soc_mem_field32_get(unit, table, data,
                                    MPLS_LABEL_2f) > 0) {
            ...
            if (flex_group_mode_related_info->flags != 0) {
                ...
                if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_6)) {
                    ...
                    if (flex_group_mode < bcmStatGroupModeFlex1) {
                        ...
                        _bcm_esw_stat_attr_fill(unit, flex_group_mode,
   ...
   for(index=0; index <= (max_index - min_index) ; index++) {
       ingress_entry_data = soc_mem_table_idx_to_pointer(unit, ingress_table,
        ...
        if (soc_feature(unit, soc_feature_use_flex_ctr_oam_lm)) {
            ...
            enable = 0;
            if (BCM_E_PARAM !=  _bcm_esw_stat_flex_ingress_group_oam_status
                                              (unit, pool_group_id,
                                              &enable)) {
                if (enable ) {
...
bcm_error_t _bcm_esw_stat_flex_init(int unit)
{
  ...
  char *config_str;
    ...
    if ((num_pools[bcmStatFlexDirectionIngress] >
                          ((SOC_IS_TOMAHAWK(unit))? BCM_STAT_TH_FLEX_COUNTER_MAX_POOL: 
                                                    BCM_STAT_FLEX_COUNTER_MAX_POOL)) ||
        (num_pools[bcmStatFlexDirectionEgress] >
         BCM_STAT_FLEX_COUNTER_MAX_POOL)) {
         LOG_ERROR(BSL_LS_BCM_FLEXCTR,
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (soc_feature(unit, soc_feature_flex_counter_opaque_stat_id) &&
                    (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_4)) {
            local_scache_size +=  sizeof(bcm_stat_flex_custom_mode_related_info_t) *
                                  (BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE - 
...
#if defined(BCM_TOMAHAWK_SUPPORT)
        if (IsAnyModeConfigured && SOC_IS_TOMAHAWK(unit)) {
        /* 10) Ingress: IFP_POLICY_TABLE Table */
            _bcm_esw_stat_flex_check_ingress_ifp_policy_table(unit, per_pipe_mode_ifp);
        /* 11) Ingress: AGM */
            ...
            bcm_stat_value_t *value)
{
    ...
    if (soc_mem_read(unit,table,MEM_BLOCK_ANY,
        ...
        _bcm_esw_get_flex_counter_fields_values(
                 unit,index,table,entry_data, &offset_mode,(uint32 *)&pool_number,&base_idx);
        if ((offset_mode == 0) && (base_idx == 0)) {
             LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                       (BSL_META_U(unit,
                                   ...
                                   uint32                    counter_idx)
{
    ...
    static uint8 toggle_stat[SOC_MAX_NUM_DEVICES] = {0};
    uint8     toggle = toggle_stat[unit];
#endif
...
#if defined(BCM_TRIDENT2_SUPPORT)
            if (!SOC_IS_TOMAHAWK(unit) && soc_feature(unit, soc_feature_advanced_flex_counter_dual_pipe)) {
            ...
            void                *egress_entry_data1)
{
    ...
    _bcm_esw_get_flex_counter_fields_values(
                 unit,index,egress_table,egress_entry_data_temp,
    ...
    _bcm_esw_set_flex_counter_fields_values(
             unit,index,egress_table,egress_entry_data_temp,mode,pool_number,base_idx);
    ...
    switch(ingress_table) {
        ...
        case VFP_POLICY_TABLE_PIPE3m:
            ...
            if (field_stage == bcmFieldQualifyStageIngress) {
              ...
              * from a specific pool
              */
             if (BCM_STAT_FLEX_ING_POLICY_GROUP_AUTO != *pool_number) {
...
#if defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_TRIDENT2PLUS_SUPPORT)
    if ((SOC_IS_TOMAHAWK(unit) || SOC_IS_TD2P_TT2P(unit))&&
        (bcmStatObjectIngFieldStageIngress == object)) {
        *direction = bcmStatFlexDirectionIngress;
        return BCM_E_NONE;
    }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWK(unit)) && (object == (bcm_stat_object_t)bcmIntStatObjectIngAgm)) {
        *direction = bcmStatFlexDirectionIngress;
        return BCM_E_NONE;
    }
#endif
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if ((SOC_IS_TOMAHAWK(unit)) && (object == (bcm_stat_object_t)bcmStatObjectIngExactMatch)) {
        *direction = bcmStatFlexDirectionIngress;
        return BCM_E_NONE;
    }
#endif
