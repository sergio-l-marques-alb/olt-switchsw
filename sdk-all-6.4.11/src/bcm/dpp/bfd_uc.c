...
STATIC int _bcm_petra_bfd_uc_trap_code_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info, uint32 *trap_code) {
    ...
    } else if (BCM_GPORT_IS_SYSTEM_PORT(endpoint_info->remote_gport) ||
...
/**
 * RX: Destroy trap code for remote gport.
 * 
 * @author lingyong (10/11/2015)
 * 
 * @param unit 
 ...
 * @return int 
 */
STATIC int _bcm_petra_bfd_uc_trap_code_destroy(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    int rv; 
    uint32 trap_code;   
    uint8 no_traps;
    uint8 is_internal_trap;
    ...
    if (BCM_GPORT_IS_SYSTEM_PORT(endpoint_info->remote_gport) ||
        BCM_GPORT_IS_MODPORT(endpoint_info->remote_gport) ||
        BCM_GPORT_IS_LOCAL(endpoint_info->remote_gport)) {
        /* use user defined destination.*/
        rv = _bcm_dpp_oam_bfd_trap_code_from_remote_gport(unit, endpoint_info->remote_gport, &trap_code, TRUE/*meta_data*/, &classifier_mep_entry);
...
STATIC int _bcm_petra_bfd_uc_m_hop_set_classifer(int unit, bcm_bfd_endpoint_info_t *endpoint_info, uint32 trap_code, bfd_endpoint_config_t *endpoint_config) {
    ...
    if (!trap_code) {
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW) {
            rv = sw_state_access[unit].dpp.bcm.bfd.trap_info.trap_ids.get(unit, SOC_PPD_BFD_TRAP_ID_UC_IPV6, &trap_code); 
            BCMDNX_IF_ERR_EXIT(rv);
        } else {
                                        ...
                                        bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (!update || (endpoint_info->flags & BCM_BFD_ENDPOINT_ENCAP_SET)) {
        ...
        if ((SOC_IS_JERICHO(unit)) || (endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP)) {
...
bcm_petra_bfd_uc_endpoint_destroy(int unit, bcm_bfd_endpoint_t endpoint)
{
    ...
    if (endpoint_config->info.remote_gport != BCM_GPORT_INVALID) {
        BCM_IF_ERROR_RETURN
            (_bcm_petra_bfd_uc_trap_code_destroy(unit, &endpoint_config->info));
    } 

    if (SOC_IS_JERICHO(unit) || (endpoint_config->info.flags & BCM_BFD_ENDPOINT_MULTIHOP)) {
        /* Use the OAM classifier*/
        BCM_IF_ERROR_RETURN
