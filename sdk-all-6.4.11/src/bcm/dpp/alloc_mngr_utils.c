...
bcm_dpp_am_attach(int unit)
{
    ...
    if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)) {
        ...
        /* Init global lif database - must be done before initializing the global lif modules. */
        if (!SOC_WARM_BOOT(unit)) {
...
_bcm_dpp_templates_create(int unit, int nof_template_pools)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (template_id = 0 ; template_id < SOC_DPP_CONFIG(unit)->am.nof_am_template_ids ; template_id++)   {
            ...
            /* we free this memory in _shr_mtemplate_destroy_data */
                    /* Get how many pools to allocate for the resource. */
        pool_id = 0;
        if (!sw_state_cb_db[unit].dpp.shr.template_info.pool) {
           sw_state_cb_db[unit].dpp.shr.template_info.pool = sal_alloc(nof_template_pools * sizeof(_shr_template_pool_desc_cb_db_t)
                                                                    ,"sw_state_cb_db[unit].dpp.shr.template_info.pool");
        }
        for (template_id = 0 ; template_id < SOC_DPP_CONFIG(unit)->am.nof_am_template_ids ; template_id++)   {
            rv = bcm_dpp_am_template_to_nof_pools(unit, template_id, &nof_pools_for_template);
            BCMDNX_IF_ERR_EXIT(rv);
            for (pool_idx = 0 ; pool_idx < nof_pools_for_template ; pool_idx++) {
                rv = _bcm_dpp_template_warm_boot_setup(unit, template_id, pool_idx, pool_id);
                BCMDNX_IF_ERR_EXIT(rv);
                pool_id++;
            }
        }
...
bcm_dpp_am_resource_to_nof_pools(int unit, int res_id, uint8 *nof_pools_per_res){
    ...
    switch (res_id) {
        ...
        case dpp_am_res_local_outlif:
            *nof_pools_per_res = _BCM_DPP_AM_EGRESS_LIF_NOF_EEDB_HALF_BANKS;
            break;
        case dpp_am_res_global_inlif:
            *nof_pools_per_res = (SOC_IS_JERICHO(unit)) ? _BCM_DPP_AM_NOF_GLOBAL_IN_LIF_BANKS(unit) : BCM_DPP_AM_DEFAULT_NOF_POOLS;
            break;
        case dpp_am_res_global_outlif:
            /* 6_4 ISSU workaround : allocate the same number of out lif pools in arad mode, instead of saving space by not allocating them. */
            *nof_pools_per_res = (SOC_IS_JERICHO(unit)) ? _BCM_DPP_AM_NOF_GLOBAL_IN_LIF_BANKS(unit) : BCM_DPP_AM_DEFAULT_NOF_POOLS;
            break;
        case dpp_am_res_obs_inlif:
        case dpp_am_res_obs_eg_encap:
            *nof_pools_per_res = (SOC_IS_ARAD(unit) && SOC_IS_ARADPLUS_AND_BELOW(unit)) ? _BCM_DPP_AM_LIF_NOF_BANKS : BCM_DPP_AM_DEFAULT_NOF_POOLS;
            break;
	    ...
	    case dpp_am_res_meter_b:
            *nof_pools_per_res = SOC_IS_ARAD(unit) ? SOC_DPP_CONFIG(unit)->meter.nof_meter_cores : BCM_DPP_AM_DEFAULT_NOF_POOLS;
			break;
...
bcm_dpp_am_template_to_nof_pools(int unit, int template_id, uint8 *nof_pools){
	...
	case dpp_am_template_meter_profile_b_low:
    	*nof_pools = SOC_IS_ARAD(unit) ?  SOC_DPP_CONFIG(unit)->meter.nof_meter_cores : BCM_DPP_AM_DEFAULT_NOF_POOLS;	
		break;
