...
_bcm_dpp_am_global_lif_init(int unit){
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        count = bcm_dpp_am_global_lif_bank_bit_count * _BCM_DPP_AM_NOF_GLOBAL_IN_LIF_BANKS(unit);
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_id_is_legal
 * Purpose:
 *     Given a global lif id and sync flags, returns wether the id is legal on this device.
 *  
 * Parameters:
 *     unit         - (IN) Device number
 *     sync_flags   - (IN) _BCM_DPP_AM_SYNC_LIF_INGRESS, _BCM_DPP_AM_SYNC_LIF_EGRESS or both for sync entry.
 *     global_lif   - (IN) A global lif id to be tests.
 * Returns: 
 *     TRUE - if lif id is legal.
 *     FALSE - if lif id is illegal.
 */
uint8
_bcm_dpp_am_global_lif_id_is_legal(int unit, uint32 sync_flags, int global_lif){
    ...
    if (sync_flags & _BCM_DPP_AM_SYNC_LIF_EGRESS) {
        is_legal &= _BCM_DPP_AM_GLOBAL_OUT_LIF_ID_IS_LEGAL(unit, global_lif);
    }
...
_bcm_dpp_am_global_lif_internal_alloc(int unit, uint32 alloc_flags, uint32 sync_flags, int *global_lif){
    ...
    if (alloc_flags & (SW_STATE_RES_ALLOC_WITH_ID)) {
        ...
        if (!_bcm_dpp_am_global_lif_id_is_legal(unit, sync_flags, *global_lif)) {
...
_bcm_dpp_am_global_lif_internal_dealloc(int unit, uint32 sync_flags, int global_lif_id){
    ...
    /* Input validation. */
    if (!_bcm_dpp_am_global_lif_id_is_legal(unit, sync_flags, global_lif_id)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Given lif id is illegal.")));
    }
    ...
    /* Validate bank exists and matches type */
    rv = _bcm_dpp_am_global_lif_is_legal_bank(unit, sync_flags, 
                          _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_LEGAL_ALLOCATE_WITH_ID | _BCM_DPP_AM_GLOBAL_LIF_BANK_IS_LEGAL_ALLOCATED_BANK_ONLY, 
                          bank_id, &is_legal);
    BCMDNX_IF_ERR_EXIT(rv);
...
/*
 * Function:
 *     _bcm_dpp_am_global_lif_bank_status_set
 * Purpose:
...
_bcm_dpp_am_global_lif_bank_is_legal(int unit, int bank_id, uint8 is_ingress, uint8 *is_legal){
    ...
    uint8 is_legal_range;
    BCMDNX_INIT_FUNC_DEFS;

    if (is_ingress) {
        /* Bank is legal if its first entry is a legal global lif. */
        *is_legal = _BCM_DPP_AM_GLOBAL_IN_LIF_ID_IS_LEGAL(unit, _BCM_DPP_AM_GLOBAL_LIF_BANK_LOW_ID(bank_id));
    } else {
        /* Fist, the bank's first entry must be a legal global lif.. */
        is_legal_range = _BCM_DPP_AM_GLOBAL_OUT_LIF_ID_IS_LEGAL(unit, _BCM_DPP_AM_GLOBAL_LIF_BANK_LOW_ID(bank_id));
        ...
        /* Second, egress global lifs can't be allocated in direct mapped banks. */
...
_bcm_dpp_am_global_lif_is_legal_bank_by_type(int unit, int bank_id, uint8 is_ingress, uint8 is_sync, uint32 bank_alloc_flags, uint8 *is_legal) {
    ...
    /* Check if this bank is not completely illegal. If it is, don't make any more tests. */
...
_bcm_dpp_am_global_lif_allocate_new_bank(int unit, uint32 sync_flags, int *bank_id, uint8 *is_success) {
    ...
    *is_success = FALSE;
    ...
    for (bank_id_iter = _BCM_DPP_AM_NOF_GLOBAL_IN_LIF_BANKS(unit) - 1 ; bank_id_iter >= 0 ; bank_id_iter--) {
