  ...
  )
{
    ...
    /* LCOV_EXCL_START */
    /* at given place */
    if(alloc_flags & BCM_DPP_AM_FLAG_ALLOC_WITH_ID) {
        ...
        if(*local_lif_index <= eg_encap_range_info.ll_limit) {
            ...
            if(_Bcm_dpp_sync_lif_eep) {
                lif_id = *local_lif_index;
                rv = bcm_dpp_am_l2_vpn_pwe_alloc(unit, _BCM_DPP_AM_L2_PWE_TERM_DEFAULT, alloc_flags, 2, &lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    *fec_index = (SOC_PPD_FEC_ID)BCM_L3_ITF_VAL_GET(intf);
  ...
  )
{
  ...
  /* Verify LIF is LL */
  BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, eep_index, NULL, &usage) );
...
STATIC int _bcm_ppd_ipv4_uc_route_info_to_ppd_get(
    int                                 unit,
    bcm_l3_route_t                      *info,
    SOC_PPD_FRWRD_IPV4_UC_ROUTE_KEY     *route_key,
...
)
{
  ...
  if(SOC_IS_ARAD(unit)){
      ...
      /* FEC + MAC --> forward decision = FEC + ARP (EEI)*/
         ...
         rv = _bcm_dpp_gport_fix_fec_type(unit, BCM_L3_ITF_VAL_GET(info->l3a_intf), SOC_PPD_FEC_TYPE_IP_UC);
         BCMDNX_IF_ERR_EXIT(rv);
  ...
  )
{
  ...
  rv = _bcm_ppd_ipv4_host_info_to_ppd_get(unit, info, &host_key, &routing_info);
  BCMDNX_IF_ERR_EXIT(rv);
  ...
  )
{
    ...
    SOC_PPD_VRF_ID                   vrf_ndx;
    SOC_PPD_FRWRD_IPV6_VPN_ROUTE_KEY route_key;
    SOC_PPD_FRWRD_IP_ROUTE_STATUS    route_status;
    SOC_PPD_FRWRD_IP_ROUTE_LOCATION  location;
    SOC_PPD_FRWRD_DECISION_INFO      route_info;
...
)
{
  ...
  if (is_host) {
      /* EEP is valid this host points to: FEC + EEP */
      if (BCM_L3_ITF_TYPE_IS_FEC(info->l3a_intf) && encap_id != 0) {
    ...
    for (vrf_ndx=BCM_L3_VRF_DEFAULT; vrf_ndx<SOC_DPP_CONFIG(unit)->l3.max_nof_vrfs; vrf_ndx++)
    {
      ...
      while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&block_range.start.payload))
      {
        ...
        for(idx = 0; idx < nof_entries; idx++)
        {
          ...
          entry.l3a_intf = route_infos[idx].dest_id;
  ...
  )
{
    ...
    BCMDNX_ALLOC(host_keys, sizeof(SOC_PPD_FRWRD_IPV4_HOST_KEY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_host_get_block.host_keys");
    if (host_keys == NULL) {        
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("failed to allocate memory")));
    }

    /*initialize routes_info and host_keys with 0*/
    sal_memset(routes_info, 0, sizeof(SOC_PPD_FRWRD_IPV4_HOST_ROUTE_INFO)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES);
    sal_memset(host_keys, 0, sizeof(SOC_PPD_FRWRD_IPV4_HOST_KEY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES);
    ...
    int unit)
{
    ...
    if (init == TRUE) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id &&
            ...
            rv = _bcm_dpp_lif_sw_resources_delete(unit, SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id, -1, _BCM_DPP_GPORT_SW_RESOURCES_INGRESS);
            BCMDNX_IF_ERR_EXIT(rv); 
        }
  ...
  if (status == BCM_E_NONE) {
      if (!vsi_info.enable_my_mac) {
        ...
        if (!(flags & _BCM_PETRA_L3_INTF_IGNORE_NOT_FOUND)) {
          BCMDNX_ERR_EXIT_VERB_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Intf not found")));
        }
      }
    ...
    bcm_if_t *if_id)
{
    ...
    if (is_egress) {    
        ...
        if ((encap_with_id) && (flags & BCM_L3_REPLACE)) { /* second call */
            ...
            status = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, tunnel_local_lif, NULL, &lif_type);
            BCMDNX_IF_ERR_EXIT(status);
        ...
        */
        ...
        if(SOC_IS_ARAD(unit) && (egr->intf > 0) && BCM_L3_ITF_TYPE_IS_RIF(egr->intf)) { /* update outrif to point to LL, needed only in ARAD */
...
/**
 * Check if the API is bcm_l3_egress_multipath_* (only valid for
 * Petra-B). If yes return BCM_E_NONE. If no return 
 * BCM_E_CONFIG. 
 */
int bcm_petra_l3_ecmp_check_api_is_multipath(int unit)
{
  ...
  if (SOC_IS_ARAD(unit)) {
      BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, 
                     (_BSL_BCM_MSG_STR("Error - The bcm_l3_egress_multipath_* APIs are deprecated.\n Use bcm_l3_ecmp_* APIs instead.\n")));
  }
...
/**
 * Check if the API is bcm_l3_egress_ecmp_* (not the case only 
 * for Petra). If yes return BCM_E_NONE. If no return 
 * BCM_E_CONFIG. 
 */
  ...
  )
{
  ...
  *err = ECMP_SIMPLE_FEC_MGMT_ERR_NO_ERROR;
  ...
  for (intf_idx = 0; intf_idx < intf_count; intf_idx++) {
    ...
    if (intf_idx > 0) {
      SOC_PPD_FEC_ID prev_fec_ndx = BCM_L3_ITF_VAL_GET(intf_array[intf_idx-1]);
      SOC_PPD_FEC_ID curr_fec_ndx = BCM_L3_ITF_VAL_GET(intf_array[intf_idx]);
    ...
    bcm_if_t *intf_id)
{
    ...
    if (if_id != SOC_PPD_RIF_NULL) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            int old_is_last = 0;
            rif_info.ipmc_bridge_v4mc_enable = (ing_intf->flags & BCM_L3_INGRESS_L3_MCAST_L2) != 0;
    ...
    bcm_l3_ingress_t *ing_intf)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (rif_info.ipmc_bridge_v4mc_enable) {
    ...
    bcm_l3_route_t *info)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        /* check this is not default route, this restriction does not apply from Jericho */
