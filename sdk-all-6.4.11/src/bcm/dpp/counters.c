...
int _bcm_dpp_counter_dma_attach(_bcm_dpp_counter_state_t *unitData){
    ...
    if (SOC_IS_ARAD(unit)) {
        for (index = 0; index < (SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)); index++) {
            if (unitData->proc[index].valid & BCM_DPP_COUNTER_PROC_INFO_VALID) {
   ...
   int                                  range_id) 
{
    ...
    int is_ingress = (source == SOC_TMC_CNT_SRC_TYPE_ING_PP);
    int max_lif_stack_to_count = SOC_TMC_CNT_LIF_COUNTING_NUMBER_OF_STACK_IDS(unitData->unit, source);
    uint8 found = 0;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_NULL_CHECK(unitData);
    ...
    for (found = 0, stack_level_to_count = bcmStatCounterLifStackId0, lif_counting_mask = 0; 
         lif_counting_mask <= SOC_TMC_CNT_LIF_COUNTING_MASK_MAX(unitData->unit, source);
...
_bcm_dpp_counter_unit_state_verify(_bcm_dpp_counter_state_t *unitData, soc_port_t crps_index) {
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unitData->unit))
    {
        if (unitData->proc[crps_index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_VOQ)
        {
            BCM_EXIT;
...
bcm_dpp_counter_config_set(
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_PROC_INFO_ALL, unitData, NULL);
    BCMDNX_IF_ERR_EXIT(result);
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_counter_config_clear(int unit, soc_port_t crps_index, uint8 clear_cache){
    ...
    result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_DIAGS | COUNTER_UPDATE_SELECT_PROC_INFO_ALL, unitData, NULL);
    BCMDNX_IF_ERR_EXIT(result);
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dpp_counter_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        result = _bcm_dpp_counter_unit_select_update(unit, COUNTER_UPDATE_SELECT_THREAD, tempUnit, NULL);
        BCMDNX_IF_ERR_EXIT(result);
