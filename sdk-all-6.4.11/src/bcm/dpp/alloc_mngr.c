...
#define _DPP_AM_TEMPLATE_SNOOP_CMD_MAX_ENTITIES(unit) (_BCM_PETRA_NOF_LOCAL_PORTS(unit) + SOC_PPD_NOF_TRAP_CODES)
...
#define _DPP_AM_TEMPLATE_MAID_48_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_MPLS_PUSH_PROFILE_COUNT(unit) ((SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"push_profile_advanced_enable",0))? 15 : 7)
...
_bcm_dpp_pp_resource_setup(int unit, int res_id, int core_id, int pool_id) {
    ...
    switch (res_id) {
        case dpp_am_res_vsi_vlan:
            ...
            p_info.start = SOC_PPD_VSI_LOW_CNFG_VALUE;
            p_info.count = 4096;
    ...
    break;
        ...
        case dpp_am_res_global_outlif:
            if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)){
                p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
                p_info.start = _BCM_DPP_AM_GLOBAL_LIF_BANK_LOW_ID(core_id);
        ...
        case dpp_am_res_field_direct_extraction_entry_id:
            ...
            p_info.max_elements_per_allocation = 1;
            break;
        case dpp_am_res_ISSU_reserve_1: 
        case dpp_am_res_ISSU_reserve_2:
        case dpp_am_res_ISSU_reserve_3:
        case dpp_am_res_ISSU_reserve_4:
        case dpp_am_res_ISSU_reserve_5:
            p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
            p_info.count = 1;
            p_info.start = 0;
            desc_str = "Reserved res";
            p_info.max_elements_per_allocation = 1;
            break;
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_pwe,flags,count,&elem);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        *lif = elem;
        ...
        elem = *lif;
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,flags,count,&elem);
...
#endif /* BCM_ARAD_SUPPORT */
    {
      rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_pwe,1,elem);
      BCMDNX_IF_ERR_EXIT(rv);

      rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,1,elem);
      BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,flags,1,lif);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic, 1,lif));
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,1,lif);
        BCMDNX_IF_ERR_EXIT(rv);
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_global,flags,1,eep);
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_global, 1,eep));
    }
...
#endif /* BCM_ARAD_SUPPORT */
    rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_global,1,eep);
    BCMDNX_IF_ERR_EXIT(rv);
...
#endif /* BCM_ARAD_SUPPORT */
        {
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_ip_tnl,flags,1,&elem);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            *lif = elem;
            ...
            elem = *lif;
            rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,flags,1,&elem);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        *lif = elem;
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_ip_tnl,1,elem);
        BCMDNX_IF_ERR_EXIT(rv);
...
#endif /* BCM_ARAD_SUPPORT */
    {
        rv = dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_mpls_tunnel,flags,count,eep);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_mpls_tunnel,1,eep));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
#endif /* BCM_ARAD_SUPPORT */
        rv = dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_eep_mpls_tunnel,1,eep);
        BCMDNX_IF_ERR_EXIT(rv);
...
#endif /* BCM_ARAD_SUPPORT */
    {
      BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_lif_dynamic,1,elem));
    }
...
_bcm_dpp_template_setup(int unit, int template_id, int core_id, int pool_id) {
    ...
    sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.to_stream = _bcm_dpp_am_template_memcpy_to_stream;
    sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.from_stream = _bcm_dpp_am_template_memcpy_from_stream;
    ...
    case dpp_am_template_egress_interface_unicast_thresh:
        ...
        case dpp_am_template_user_defined_traps:
            ...
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.to_stream = _bcm_dpp_am_template_user_defined_trap_to_stream;
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.from_stream = _bcm_dpp_am_template_user_defined_trap_from_stream;
        ...
        case dpp_am_template_snoop_cmd:
            ...
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.to_stream = _bcm_dpp_am_template_snoop_cmd_to_stream;
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.from_stream = _bcm_dpp_am_template_snoop_cmd_from_stream;
    ...
    case dpp_am_template_vsq_pg_tc_mapping:
        ...
        case dpp_am_template_queue_discount_cls:
            ...
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.to_stream = _bcm_dpp_am_template_discount_cls_to_stream;
            sw_state_cb_db[unit].dpp.shr.template_info.pool[pool_id].extras.from_stream = _bcm_dpp_am_template_discount_cls_from_stream;
         ...
         case dpp_am_template_ptp_port_profile:
          ...
          case dpp_am_template_l3_rif_mac_termination_combination:
             if (SOC_IS_ARAD(unit)) {
                 ...
                 if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "use_rif_profile_full_range", 1)) {
                     t_info.max_entities = _DPP_AM_TEMPLATE_L3_RIF_MAC_TERMINATION_COMBINTION_MAX_ENTITIES;
                 }else{/* this is done to support ISSU in 6_4_x */
                     t_info.max_entities = _DPP_AM_TEMPLATE_L3_RIF_MAC_TERMINATION_COMBINTION_MAX_ENTITIES-1;                 
                 }
...
/* 
 * Initialize template management pool data function pointers in warmboot
 */
int
...
int _bcm_dpp_am_template_trap_egress_init(int unit,int template_init_id,SOC_SAND_OUT SOC_PPD_TRAP_EG_ACTION_PROFILE_INFO *init_data)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_trap_egress, flags, init_data,SOC_PPD_NOF_TRAP_EG_TYPES, &is_allocated,&template_init_id);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    *template = type;
    ...
    if (*is_last) {
    	...
    	for (trap_index = 0; trap_index < _BCM_NUMBER_OF_VIRTUAL_TRAPS ; trap_index++)
    	{
			...
			if (new_virtual_traps_p.soc_ppd_prog == type + SOC_PPD_TRAP_CODE_PROG_TRAP_0)
				break;
    	}
...
int _bcm_dpp_am_template_l3_rif_mact_ermination_combination_init(int unit, uint32 *profile_info, int template)
{
  ...
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "use_rif_profile_full_range", 1)) {
      ...
      rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_l3_rif_mac_termination_combination, SHR_TEMPLATE_MANAGE_SET_WITH_ID, 
                                           profile_info, _DPP_AM_TEMPLATE_L3_RIF_MAC_TERMINATION_COMBINTION_MAX_ENTITIES-1, &is_allocated, &template);
  }
