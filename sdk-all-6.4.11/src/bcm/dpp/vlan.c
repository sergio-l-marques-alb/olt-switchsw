...
{   
    ...
    if (is_init) {
        ...
        if (SOC_IS_JERICHO_B0(unit) && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "roo_remark_profile", 0)) {
    ...
    int unit)
{
    ...
    for (idx = SOC_PPD_VSI_LOW_CNFG_VALUE; idx < BCM_VLAN_COUNT; idx++) {
    ...
    uint32 flags) /* flags : same as BCM_VLAN_GPORT_ADD_XXX */
{
    ...
    if (SOC_IS_JERICHO_B0(unit) && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "roo_remark_profile", 0) && BCM_PBMP_NOT_NULL(ubmp)) {
    ...
    if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && (SOC_DPP_CONFIG(unit)->pp.egress_membership_mode == 1) && (update_membership == 1) && (vid == 0)) {
    ...
    bcm_vlan_action_set_t *action)
{
    ...
    rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, out_ac, NULL, &lif_usage);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *forced_tpid_profile)
{
    ...
    /*
         BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag determines that the packet is untagged after EVE.
         The definition is later used in outer tag acceptance desicion and outbound mirroring.
         The flag is supported when:
         - SOC is Jericho-B0 (and soc property outer_vid_existence_class_mode is 1(new)) and above.
         - BCM_VLAN_ACTION_SET_EGRESS flag is set
         - Outer vid source of the command is EMPTY (verified below)
    */
    if (action->flags & BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED) {
        if (!SOC_IS_JERICHO_B0_AND_ABOVE(unit)) {
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag is not supported for this device")));
        }
        if (!SOC_DPP_CONFIG(unit)->pp.outer_vid_existence_class_mode) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag is not supported in legacy mode")));
        }
        if (is_ingress) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag is not supported for Ingress actions")));
        }
    }
    ...
    /* Handle the Egress variation. The Set is done to a SW DB and to the HW */
        ...
        /* Verify BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag is set only if outer_vid_source is empty */
        if ((action->flags & BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED) &&
             eg_command_info.outer_tag.vid_source != SOC_PPC_EG_VLAN_EDIT_TAG_VID_SRC_EMPTY) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED flag cannot be used with the supplied VLAN translate action")));
        }
        ...
        while (_bcm_petra_vlan_edit_eg_command_id_find(unit, action_id, is_first_lookup, &eg_command_key) == BCM_E_EXISTS) {
            ...
            if (SOC_DPP_CONFIG(unit)->pp.outer_vid_existence_class_mode)
            {
                uint32 pkt_is_tagged = (action->flags & BCM_VLAN_ACTION_SET_OUTER_VID_UNTAGGED) ? 0 : 1;
                BCMDNX_IF_ERR_EXIT(MBCM_PP_DRIVER_CALL(unit, mbcm_pp_eg_vlan_edit_packet_is_tagged_set,
                                            (unit,
                                             &eg_command_key,
                                             pkt_is_tagged)));
    ...
    bcm_vlan_port_translation_t *vlan_port_translation)
{
    ...
    if ((lif_usage_type != _bcmDppLifTypeVlan) && (lif_usage_type != _bcmDppLifTypeExtender)) {
    ...
    /* Handle the Egress scenario */
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            BCMDNX_IF_ERR_EXIT(MBCM_PP_DRIVER_CALL(unit, mbcm_pp_extender_eve_etag_format_set,
                                                    (unit,
                                                     lif_eg_info->edit_info.edit_profile,
...
int _bcm_dpp_vlan_port_create(int unit, bcm_vlan_port_t *vlan_port)
{
    ...
    if (_BCM_DPP_GPORT_INFO_IS_LOCAL_PORT(gport_info) && !remote_lif) {
            ...
            if (is_ingress) {
                ...
                if ((SOC_IS_JERICHO(unit) || SOC_IS_ARAD(unit)) &&
                    (vlan_port->criteria == BCM_VLAN_PORT_MATCH_PORT) &&
                    (gport_info.flags & _BCM_DPP_GPORT_INFO_F_IS_LAG)) {
                    uint32 lag_port, lag_pp_port;
                    int lag_core;
                    SOC_SAND_OUT SOC_PPD_PORT_INFO     lag_port_info;

                    SOC_PPD_PORT_INFO_clear(&lag_port_info);
                    /* set port info */
