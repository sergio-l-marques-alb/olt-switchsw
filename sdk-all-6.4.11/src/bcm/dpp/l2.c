...
/* defines */
#define _BCM_PETRA_L2_FLAGS_SUPPORTED_PB                                \
...
#define _BCM_PETRA_L2_FLAGS_SUPPORTED_ARAD                              \
    (BCM_L2_DISCARD_SRC             | BCM_L2_DISCARD_DST                | \
     BCM_L2_STATIC                  | BCM_L2_TRUNK_MEMBER               | \
     BCM_L2_MCAST                   | BCM_L2_ENTRY_OVERFLOW             | \
     BCM_L2_HIT                     | _BCM_L2_ENCAP_VALID)


/* Supported flags for L2 */
#define _BCM_PETRA_L2_FLAGS_SUPPORTED(unit)                             \
    (SOC_IS_PETRAB(unit) ? _BCM_PETRA_L2_FLAGS_SUPPORTED_PB : _BCM_PETRA_L2_FLAGS_SUPPORTED_ARAD)
...
#define _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED(unit)                       \
    (SOC_IS_PETRAB(unit) ? _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_PB :     \
     (SOC_IS_ARADPLUS(unit) ? _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_ARAD_PLUS : _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_ARAD))
#else /* Not BCM_88660_A0 */
#define _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED(unit)                       \
    (SOC_IS_PETRAB(unit) ? _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_PB : _BCM_PETRA_L2_LIMIT_FLAGS_SUPPORTED_ARAD)
#endif
...
#define _BCM_PETRA_L2_MAC_LIMIT_GET(limit) ((limit) & (~SOC_PPD_FRWRD_MACT_LEARN_VMAC_LIMIT))
...
SOC_PPD_LLP_SA_AUTH_MAC_INFO              
*_bcm_l2_auth_traverse_mac_infos[BCM_LOCAL_UNITS_MAX] = {NULL};



/* define locks 
   #define L2_LOCK(unit)  sal_mutex_take(sw_state_sync_db[(_unit)].dpp.l2_lock, sal_mutex_FOREVER)
   #define L2_UNLOCK(unit) sal_mutex_give(sw_state_sync_db[(_unit)].dpp.l2_lock) 
*/ 


#define L2_LOCK(unit)  
#define L2_UNLOCK(unit)
...
/* in Soc_petra there are 8 profiles per port*/
...
/* Defines used in L2-Caching, set formatting for returned *index */
#define _BCM_PETRA_L2_CACHE_INDX_PORT_SHIFT (0)
#define _BCM_PETRA_L2_CACHE_INDX_PORT_MASK (0xff)

#define _BCM_PETRA_L2_CACHE_INDX_PROFILE_SHIFT (8)
#define _BCM_PETRA_L2_CACHE_INDX_PROFILE_MASK (0xff)

#define _BCM_PETRA_L2_CACHE_INDEX_TYPE_RESERVED_MC (0)
#define _BCM_PETRA_L2_CACHE_INDEX_TYPE_PROG (1)
#define _BCM_PETRA_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC (2)




#define _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT (28)
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_MASK (0x3)

/*For the new index_control management (in sw db) the index_type will be set to 3.
In case the index_control is to value 3 it states that the data for this index is stored in sw db. 
Otherwise, the old parsing of the index will be applied. 
This bit is used for ISSU backward compatibility in 6.4. 
The code for the new_index_type does not exist in 6.5 and higher versions.*/
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL (0x3)
#define _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL_MASK (0xFFFF) /*Number of new indexes are max 260 hence this mask is suffiecient*/



#define _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_VSI_SHIFT (0)
#define _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_VSI_MASK (0xffff)

#define _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_MAC_SHIFT (16)
#define _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_MAC_MASK (0xff)

#define _BCM_DPP_L2_CAHCE_INVALID_INDEX -1

#define _BCM_PETRA_L2_CACHE_INDX_MAX_VAL ((_BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL << _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) + _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL)


/* 
  * internal flag indicating the outlif valid bit is set.
  * Used in cases that outlif value is NULL but the outlif valid bit is still set. 
  */ 
#define _BCM_L2_ENCAP_VALID    (0x10000000)
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_get
* Purpose:   Get he control index array structure, maps index to values for bcm_l2_cache_set/get
                To perform a correct get function need port, mac_lsb and type of virtual trap, this what SW DB hold
* Params:
* unit            -  which unit is it
* index_control_p - The whole SW DB of index control type
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_get(int unit,_bcm_l2_cache_index_t *index_control_p)
{
   BCMDNX_INIT_FUNC_DEFS;
   BCMDNX_IF_ERR_EXIT(
   L2_ACCESS.index_control.get(unit,index_control_p));
exit:
    BCMDNX_FUNC_RETURN;
}

/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_entry_find
* Purpose:    Check if certian entry present in SW DB, comparison by all entry params
* Params:
* unit         -  which unit is it
* index_type   - (IN) type of trap in SW DB (MC/PROG..) to compare
* mac_lsb      - (IN) 8 bits of DA mac address to compare
* port         - (IN) ingress port of packet to compare
* prog_profile - (IN) programmable trap profile to comapre
* index_used_p - (OUT) in case match found return the index in SW DB array otherwise -1
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_entry_find(int unit,_bcm_l2_cache_index_type_e index_type,
                                            int mac_lsb,int port, int prog_profile,int *index_used_p)
{
    _bcm_l2_cache_index_t index_arry;
    int i;
    BCMDNX_INIT_FUNC_DEFS;
    *index_used_p = _BCM_DPP_L2_CAHCE_INVALID_INDEX; /*invalidate index*/
    
    BCMDNX_IF_ERR_EXIT(
    _bcm_dpp_l2_cache_sw_db_index_control_get(unit,&index_arry));
    
    for(i = 0 ; i < _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL; i++)
    {
        if( index_arry.index_entry[i].index_type == index_type &&
            index_arry.index_entry[i].mac_lsb == mac_lsb &&
            index_arry.index_entry[i].port == port &&
            index_arry.index_entry[i].prog_profile == prog_profile)
        {
            *index_used_p = i;
            break;
        }
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_entry_get
* Purpose:   get values of certain index fron SW DB index control
* Params:
* unit          -  which unit is it
* index_sw_db   - (IN) index SW DB index which values to return
* index_entry_p - (OUT) the structure of parameters to fill from SW DB
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_entry_get(int unit,
                                            int index_sw_db,_bcm_l2_cache_index_control_t *index_entry_p)
{
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(
    L2_ACCESS.index_control.index_entry.get(unit,index_sw_db,index_entry_p));
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_entry_set
* Purpose:   Set certain entry of index control array to needed structure
* Params:
* unit          -  which unit is it
* index_sw_db   - (IN) index SW DB index which valus to set
* index_entry_p - (IN) the structure of parameters to fill the SW DB with
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_entry_set(int unit,
                                            int index_sw_db,_bcm_l2_cache_index_control_t *index_entry_p)
{
    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_IF_ERR_EXIT(
    L2_ACCESS.index_control.index_entry.set(unit,index_sw_db,index_entry_p));
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_free_entry_get
* Purpose:   Get the first entry that has INVALID means this entry is free
* Params:
* unit               -  which unit is it
* free_entry_index_p - (OUT) The index of first free entry in SW DB
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_free_entry_get(int unit, int *free_entry_index_p)
{
    _bcm_l2_cache_index_t index_arry;
    int i;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    for(i = 0 ; i < _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL; i++)
    {
        if(index_arry.index_entry[i].index_type == _BCM_L2_CACHE_INDEX_TYPE_INVALID)
        {
            *free_entry_index_p = i;
            break;
        }
    }
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_delete
* Purpose:   Delete certain entry of index control, Set it to INVALID which means its free
* Params:
* index_to_delete  - (IN) which index in SW DB to delete, set it to INVALID
* unit -  which unit is it
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_delete(int unit,int index_to_delete)
{
    BCMDNX_INIT_FUNC_DEFS;

    if(index_to_delete < 0 || index_to_delete > _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("index to delete out of range")));
    }
     BCMDNX_IF_ERR_EXIT(
    L2_ACCESS.index_control.index_entry.index_type.set(unit,index_to_delete,_BCM_L2_CACHE_INDEX_TYPE_INVALID));
...
/*****************************************************************************
* Function:  _bcm_dpp_l2_cache_sw_db_index_control_delete
* Purpose:   Set all entry of index control to INVALID which meas its free
* Params:
* unit -  which unit is it
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_sw_db_index_control_delete_all(int unit)
{
    int i;
    BCMDNX_INIT_FUNC_DEFS;
    for(i = 0; i < _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL; i ++)
    {
         BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_l2_cache_sw_db_index_control_delete(unit,i));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_out_lif_overlay_ll_match_add(int unit, bcm_l2_egress_t *egr, int lif)
{
    ...
    rv = _bcm_dpp_lif_to_sw_resources(unit, -1, lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_lif_sw_resources_set(unit, -1, lif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources);
    BCMDNX_IF_ERR_EXIT(rv);
 ...
 */
int
_bcm_petra_vid_to_vsi(int unit, bcm_vlan_t vid, SOC_PPD_VSI_ID *soc_ppd_vsi)
{
    BCMDNX_INIT_FUNC_DEFS;
    *soc_ppd_vsi = vid;
 ...
 */
int
_bcm_petra_vid_to_rif(int unit, bcm_vlan_t vid, SOC_PPD_RIF_ID *rif)
{
    SOC_PPD_VSI_ID
        vsi;
    int rv;
 ...
 */
int
_bcm_petra_vid_from_vsi(int unit, SOC_PPD_VSI_ID soc_ppd_vsi, bcm_vlan_t* vid)
{
    BCMDNX_INIT_FUNC_DEFS;
    *vid = soc_ppd_vsi;
 ...
 */
int
_bcm_petra_vsi_to_fid(int unit, SOC_PPD_VSI_ID vsi, SOC_PPD_FID *fid)
{
    BCMDNX_INIT_FUNC_DEFS;
    *fid = vsi;
 ...
 */
int
_bcm_petra_fid_to_vsi(int unit, SOC_PPD_FID fid, SOC_PPD_VSI_ID *vsi)
{
    BCMDNX_INIT_FUNC_DEFS;
    *vsi = fid;
 ...
 */
int
_bcm_petra_l2_to_petra_entry(int unit, bcm_l2_addr_t *l2addr, int key_only, SOC_PPD_FRWRD_MACT_ENTRY_KEY *l2_entry_key, SOC_PPD_FRWRD_MACT_ENTRY_VALUE *l2_entry_val)
{ 
    ...
    /* clear structures */
    SOC_PPD_FRWRD_MACT_ENTRY_KEY_clear(l2_entry_key);
    
    /* map key */
    l2_entry_key->key_type = SOC_PPD_FRWRD_MACT_KEY_TYPE_MAC_ADDR;
    ...
    SOC_PPD_FRWRD_MACT_ENTRY_VALUE_clear(l2_entry_val);
    ...
    if (l2addr->flags & BCM_L2_MCAST || l2addr->l2mc_group > 0) { 
        ...
        SOC_PPD_FRWRD_DECISION_LAG_SET(unit, &(l2_entry_val->frwrd_info.forward_decision), l2addr->tgid, soc_sand_rv);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    ...
    if (l2addr->flags & _BCM_L2_ENCAP_VALID) {
        l2_entry_val->frwrd_info.forward_decision.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
    ...
    if(SOC_IS_PETRAB(unit)){
         /* LCOV_EXCL_START */
        if (l2addr->group != 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("group has to be zero for Petra-B")));
        }
         /* LCOV_EXCL_STOP */
    }
#ifdef BCM_ARAD_SUPPORT
    if(SOC_IS_ARAD(unit)){
        if (l2addr->group != 0) {
            if (l2_entry_val->frwrd_info.forward_decision.additional_info.eei.type != SOC_PPD_EEI_TYPE_EMPTY){
    ...
    if (SOC_IS_ARADPLUS_A0(unit))
    {
        ...
        {
            if (l2_entry_val->frwrd_info.forward_decision.additional_info.eei.type == SOC_PPD_EEI_TYPE_EMPTY &&
                l2_entry_val->frwrd_info.forward_decision.additional_info.outlif.type == SOC_PPD_OUTLIF_ENCODE_TYPE_NONE) {
                l2_entry_val->frwrd_info.forward_decision.additional_info.outlif.type = SOC_PPD_OUTLIF_ENCODE_TYPE_RAW;
            }
        }
 ...
 */
int
_bcm_petra_l2_from_petra_entry(int unit, bcm_l2_addr_t *l2addr, int key_only, SOC_PPD_FRWRD_MACT_ENTRY_KEY *l2_entry_key, SOC_PPD_FRWRD_MACT_ENTRY_VALUE *l2_entry_val)
{ 
    int     rv = BCM_E_NONE;
    SOC_PPD_FID fid;
    SOC_PPD_VSI_ID vsi;
    ...
    if (l2_entry_key->key_type != SOC_PPD_FRWRD_MACT_KEY_TYPE_MAC_ADDR) {
    ...
    /* is it Multicast entry */
    if (l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_MC){
    ...
    else if (l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_TRAP){
    ...
    if(l2_entry_val->frwrd_info.forward_decision.type == SOC_PPD_FRWRD_DECISION_TYPE_UC_LAG) {
    ...
    if ((l2_entry_val->frwrd_info.forward_decision.additional_info.outlif.type == SOC_PPD_OUTLIF_ENCODE_TYPE_RAW) && 
        (l2_entry_val->frwrd_info.forward_decision.additional_info.outlif.val == 0)) {
        l2addr->flags |= _BCM_L2_ENCAP_VALID;
    }
...
int
_bcm_petra_l2_from_petra_entry_arad_format(int unit, bcm_l2_addr_t *l2addr, int key_only, SOC_PPD_FRWRD_MACT_ENTRY_KEY *l2_entry_key, SOC_PPD_FRWRD_MACT_ENTRY_VALUE *l2_entry_val)
{ 
    int     rv = BCM_E_NONE;
    SOC_PPD_FID fid;
    SOC_PPD_VSI_ID vsi;
...
int
_bcm_petra_l2_from_petra_bmact_entry(int unit, bcm_l2_addr_t *l2addr, int key_only, SOC_PPD_BMACT_ENTRY_KEY *l2_entry_key, SOC_PPD_BMACT_ENTRY_INFO *l2_entry_val)
{ 
    int     rv = BCM_E_NONE;
    bcm_mac_t mac_t;
    bcm_vlan_t vlan;
    ...
    vlan = _BCM_DPP_VLAN_TO_BVID(l2_entry_key->b_vid);
...
int
_bcm_petra_l2_to_petra_bmact_entry(int unit, bcm_l2_addr_t *l2addr, int key_only, SOC_PPD_BMACT_ENTRY_KEY *l2_entry_key, SOC_PPD_BMACT_ENTRY_INFO *l2_entry_val)
{ 
    int     rv = BCM_E_NONE;
    ...
    /* clear structures */
    SOC_PPD_BMACT_ENTRY_KEY_clear(l2_entry_key);
    ...
    if (SOC_IS_ARAD(unit)) {
        l2_entry_key->flags |= SOC_PPD_BMACT_ENTRY_TYPE_FRWRD;
    }
                      ...
                      bcm_l2_addr_t *l2addr)
{
    ...
    if ((_BCM_DPP_VLAN_IS_BVID(l2addr->vid)) || ((SOC_DPP_CONFIG(unit)->pp.test2) && (l2addr->vid & 0x8000))) { /* vid is B-VID */

        SOC_PPD_BMACT_ENTRY_KEY l2_entry_key;
        SOC_PPD_BMACT_ENTRY_INFO l2_entry_val;
        ...
        rv = _bcm_petra_l2_to_petra_bmact_entry(unit, &l2addr_cpy, FALSE, &l2_entry_key, &l2_entry_val);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        SOC_PPD_FRWRD_MACT_ENTRY_KEY l2_entry_key;
        SOC_PPD_FRWRD_MACT_ENTRY_VALUE l2_entry_val;
        ...
        if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit) &&
            ...
            if (BCM_GPORT_IS_SET(l2addr->port)) {
                ...
                soc_sand_rv = soc_ppd_frwrd_mact_entry_pack(soc_sand_dev_id,
                                                            TRUE,
                                                            SOC_PPD_FRWRD_MACT_ADD_TYPE_INSERT,
                                                            &l2_entry_key,
                                                            &l2_entry_val,
                                                            l2_entry.entry,
                                                            &data_len);
...
#endif
        {
            soc_sand_rv = soc_ppd_frwrd_mact_entry_add(soc_sand_dev_id,SOC_PPD_FRWRD_MACT_ADD_TYPE_INSERT,&l2_entry_key,&l2_entry_val,&failure_indication);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
                         ...
                         bcm_vlan_t vid)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(l2addr.vid)) { /* vid is B-VID */
...
STATIC int _remove_sw_mact_entries(int unit,
                                   ...
                                   SOC_PPD_FRWRD_MACT_TABLE_TYPE table_type)
{
...
exit:
    if (rv!=BCM_E_NONE || soc_sand_rv!=SOC_SAND_OK) 
    {
...
{
    SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_RULE  rule;
    SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION      action;
    SOC_PPD_FRWRD_MACT_TABLE_TYPE table_type;
    ...
    *restore = 0;
    ...
    SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_RULE_clear(&rule);
    SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_clear(&action);
    rule.key_type = SOC_PPD_FRWRD_MACT_KEY_TYPE_MAC_ADDR;

    /* if masked match supply use val-mask for high resolution of selection */
    if (match_mask_l2addr == NULL) {
        /* rule.value_rule.compare_mask = SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_USE_MASK;*/
    }
    /* MAC in rule*/
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_VLAN) {
        ...
        if(SOC_IS_PETRAB(unit)){
             /* LCOV_EXCL_START */
            rule.key_rule.mac.fid_mask = 0x3FFF;
             /* LCOV_EXCL_STOP */
        }
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_TRUNK) {
        ...
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL | SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_TYPE;
        /* mask trunk */
        if (match_mask_l2addr != NULL) {
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_PORT) {
        ...
        if (rule.value_rule.val.frwrd_info.forward_decision.additional_info.outlif.type != SOC_PPC_OUTLIF_ENCODE_TYPE_NONE ||
            ...
            rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO;
        }
    ...
    */
    ...
    if ((flags & BCM_L2_DELETE_STATIC)) {/* consider both dynamic + static */
        ...
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
    ...
    if ((trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_IGNORE_DISCARD_SRC)) {/* consider both SA-Discard 1/0*/
        ...
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA;
    ...
    if ((trvrs_flags & _BCM_PETRA_TRAVERSE_MATCH_IGNORE_DES_HIT)) {/* consider both DES-hit 1/0*/
        ...
        rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED;
    ...
    if(SOC_IS_ARAD(unit)){
        if (match_l2addr->group != 0) {
            if (rule.value_rule.val.frwrd_info.forward_decision.additional_info.eei.type != SOC_PPD_EEI_TYPE_EMPTY){
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("group ID must be zero for given gport type")));
            }
            rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_GROUP;
    ...
    if (trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_DEL) {
        ...
        action.type = SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_TYPE_REMOVE;
    }
    else if(trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_REPLACE) {
        /* set action to be update */
        action.type = SOC_PPD_FRWRD_MACT_TRAVERSE_ACTION_TYPE_UPDATE;
        ...
        if (trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_NEW_TRUNK) {
            ...
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            action.update_mask |= (SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL|SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL);
        }
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            action.update_mask |= (SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL|SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL|SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO);
        ...
        if ((trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_DISCARD_SRC_SET)) {/* set SA-discard */
            action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA;
        ...
        if ((trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_DES_HIT_CLEAR)) {/* set SA-discard */
            action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED;
        }
        ...
        if (replace_l2addr != NULL && replace_l2addr->group != 0) {
            ...
            if(SOC_IS_ARAD(unit)){
                ...
                action.update_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_GROUP;
    ...
    if (hw_acc || !SOC_DPP_IS_EM_HW_ENABLE) {
        table_type = SOC_PPD_FRWRD_MACT_TABLE_SW_ONLY; 
    }
    else
    {
        table_type = SOC_PPD_FRWRD_MACT_TABLE_SW_HW;
    }
    ...
    if (hw_acc) {
        ...
        soc_sand_rv = _remove_sw_mact_entries(unit, &rule, SOC_PPD_FRWRD_MACT_TABLE_SW_ONLY);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    else /* do it one by one, not in ARAD this is get-by rule is hw-accelerated */
    {
        ...
        uint32                                
            nof_entries = 0;
        SOC_PPD_FRWRD_MACT_ADD_TYPE add_type = SOC_PPD_FRWRD_MACT_ADD_TYPE_TRANSPLANT;
            ...
            for (indx = 0; indx < nof_entries; ++indx) {
                ...
                if (trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_REPLACE) {
                    ...
                    if (trvrs_flags & _BCM_PETRA_TRAVERSE_ACTION_DYNAMIC_SET) { /* Set dynamic bit */
                        l2addr.flags &= ~BCM_L2_STATIC;
                        add_type = SOC_PPD_FRWRD_MACT_ADD_TYPE_INSERT;
                    }
                         ...
                         int unit)
{
    int freeze_cnt;
    int rv = BCM_E_NONE;
    SOC_PPD_FRWRD_MACT_AGING_INFO  aging_info;
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
                      ...
                      bcm_l2_addr_t *l2addr)
{
    ...
    if (_BCM_DPP_VLAN_IS_BVID(vid)) { /* vid is B-VID */
                             ...
                             void *fn_data)
{
    ...
    if (dma_supported) 
    {
        /* Unset the DMA FIFO as the OLP's destination */
    ...
    if (SOC_MAC_LIMIT_PER_TUNNEL_ENABLE_GET(unit)) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.vmac_enable) {
            ...
            soc_sand_rv = soc_ppd_frwrd_mact_app_to_prefix_get(unit, 
                                                               SOC_PPD_FRWRD_MACT_PREFIX_APP_ID_OMAC, 
                                                               &(vmac_prefix.prefix[0]));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

            soc_sand_rv = soc_ppd_frwrd_mact_app_to_prefix_get(unit, 
                                                               SOC_PPD_FRWRD_MACT_PREFIX_APP_ID_OMAC_2_VMAC, 
                                                               &(vmac_prefix.prefix[1]));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            
            soc_sand_rv = soc_ppd_frwrd_mact_app_to_prefix_get(unit, 
                                                               SOC_PPD_FRWRD_MACT_PREFIX_APP_ID_VMAC, 
                                                               &(vmac_prefix.prefix[2]));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            soc_sand_rv = soc_ppd_frwrd_mact_app_to_prefix_get(unit, 
                                                               SOC_PPD_FRWRD_MACT_PREFIX_APP_ID_VMAC_2_OMAC, 
                             ...
                             bcm_l2_learn_limit_t *limit)
{
    ...
    SOC_PPD_FRWRD_MACT_MAC_LIMIT_INFO limit_info;
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
    unsigned int soc_sand_dev_id;
    SOC_PPD_FRWRD_MACT_MAC_LIMIT_GLBL_INFO  mac_limit_glb;    
#ifdef BCM_88660_A0
    _bcm_dpp_gport_hw_resources gport_hw_resources;
    ...
    if (limit->flags & BCM_L2_LEARN_LIMIT_SYSTEM) {
        ...
        if(mac_limit_glb.glbl_limit == SOC_PPD_FRWRD_MACT_NO_GLOBAL_LIMIT){
            limit->limit = -1;
        }
        else{
    ...
    if (limit->flags & BCM_L2_LEARN_LIMIT_PORT) {
        if (SOC_PPD_FRWRD_MACT_LEARN_LIMIT_MODE != SOC_PPC_FRWRD_MACT_LEARN_LIMIT_TYPE_LIF) {
        ...
        if (SOC_PPD_FRWRD_MACT_LEARN_LIMIT_MODE != SOC_PPC_FRWRD_MACT_LEARN_LIMIT_TYPE_VSI) {
    ...
    if (_L2_LEARN_LIMIT_PER_TUNNEL_IS_ENABLED(unit, limit)) {
        ...
        if (!_BCM_PPD_IS_PON_PP_PORT(gport_info.local_port)) {
    ...
    /* In case of LIF limit, the existing VSI limit table is used. Therefore, the LIF value should be
       mapped to the VSI value, according to exact mapping that is done by the HW */
        ...
        rv = _bcm_dpp_gport_to_hw_resources(unit, limit->port, _BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS | _BCM_DPP_GPORT_HW_RESOURCES_STRICT_CHECK, 
                                                 &gport_hw_resources);
        BCMDNX_IF_ERR_EXIT(rv);    
        ...
        soc_sand_rv = soc_ppd_frwrd_mact_limit_mapped_val_to_table_index_get(soc_sand_dev_id,
                                                                             gport_hw_resources.local_in_lif, &limit_tbl_idx, &is_limit_tbl_idx_reserved);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                             ...
                             bcm_l2_learn_limit_t *limit)
{
    ...
    if(limit->flags & BCM_L2_LEARN_LIMIT_SYSTEM) {
        ...
        if( limit->limit == -1){
            mac_limit_glb.glbl_limit = SOC_PPD_FRWRD_MACT_NO_GLOBAL_LIMIT;
        }
        else{
    ...
    if (set_using_common_limits == TRUE) {
        ...
        if (learn_profile_first_appear) {
            SOC_PPD_FRWRD_MACT_MAC_LIMIT_INFO_clear(&limit_info);
            limit_info.is_limited = 1;
            limit_info.nof_entries = new_limit;
            /* for, PB, max limit --> no limit, as max limit = mac size */
            if(SOC_IS_PETRAB(unit) && new_limit == SOC_DPP_DEFS_GET(unit, max_mact_limit))
            {
                 /* LCOV_EXCL_START */
                limit_info.is_limited = 0;
                limit_info.nof_entries = 0;
                 /* LCOV_EXCL_STOP */
            }
            limit_info.action_when_exceed = SOC_PPD_FRWRD_MACT_LIMIT_EXCEED_NOTIFY_TYPE_NONE;
...
_bcm_l2_pkt_eth_header_build(
                             ...
                             uint32                       header[SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE]
                             )
{
                                       ...
                                       bcm_l2_learn_msgs_config_t *learn_msg_config)
{
    ...
    int start_bit = 0;
    uint8 src_extension = 0;
    SOC_PPD_FRWRD_MACT_MSG_DISTR_INFO distr_info;
    ...
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("VLAN is not supported in learn message"))); 
    }
    ...
    SOC_PPD_FRWRD_MACT_MSG_DISTR_INFO_clear(&distr_info);
    ...
    if(SOC_IS_PETRAB(unit)){
         /* LCOV_EXCL_START */
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL_WITHOUT_DEV_ID(unit, mbcm_dpp_hpu_itmh_build,
                                                           (&olp_port_itmh, &olp_itmh_stream)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         /* LCOV_EXCL_STOP */
    }
#ifdef BCM_ARAD_SUPPORT
    ...
    start_bit = SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE * 32; /* SOC_SAND_U32_NOF_BITS; */

    distr_info.header[SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE - 1] = olp_itmh_stream.base;
    start_bit -= 32; /* tm header base size */

    if(SOC_IS_PETRAB(unit)){
         /* LCOV_EXCL_START */
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ports_itmh_extension_get,(unit, SOC_TMC_OLP_PORT_ID, &src_extension))); 
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv); 
        /* LCOV_EXCL_STOP */
    }
    else{
    ...
    if(src_extension == 1) {
        distr_info.header[SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE -2] = olp_itmh_stream.extention_src_port << 16;
        start_bit -= 16;
    }
    ...
    if(learn_msg_config->flags & BCM_L2_LEARN_MSG_ETH_ENCAP)
    {
        ...
        {
            distr_info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ETH_O_RAW;
        }
        else
        {
            distr_info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ETH_O_OTMH;
        }

        if (olp_transmit_header_type == SOC_TMC_PORT_HEADER_TYPE_RAW)
        {
            distr_info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ETH_O_RAW;
        }
        else
        {
            distr_info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ETH_O_ITMH;
        }
    }
    else
    {
        if (olp_receive_header_type == SOC_TMC_PORT_HEADER_TYPE_RAW)
        {
            distr_info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
        }
        else
        {
            distr_info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_OTMH;
        }

        if (olp_transmit_header_type == SOC_TMC_PORT_HEADER_TYPE_RAW)
        {
            distr_info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
        }
        else
        {
            distr_info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ITMH;
        }
    }
                                       ...
                                       bcm_l2_learn_msgs_config_t *learn_msg_config)
{
    ...
    olp_itmh_stream.base = distr_info.header[SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE - 1];
    start_bit -= 32; /* tm header base size */
    ...
    if(src_extension == 1) {
        olp_itmh_stream.extention_src_port = distr_info.header[SOC_PPD_FRWRD_MACT_MSG_HDR_SIZE -2] >> 16;
        start_bit -= 16;
    }
    ...
    if(SOC_IS_PETRAB(unit)){
         /* LCOV_EXCL_START */
        soc_sand_rv = (MBCM_DPP_DRIVER_CALL_WITHOUT_DEV_ID(unit, mbcm_dpp_hpu_itmh_parse,
                                                           (&olp_itmh_stream, &olp_port_itmh)));
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
         /* LCOV_EXCL_STOP */
    }
#ifdef BCM_ARAD_SUPPORT
    ...
    if(distr_info.header_type == SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ETH_O_ITMH)
    {
        learn_msg_config->flags |= BCM_L2_LEARN_MSG_ETH_ENCAP;
        rv = _bcm_l2_pkt_eth_header_parse(learn_msg_config,start_bit,distr_info.header);
        BCMDNX_IF_ERR_EXIT(rv);
    }
                                         ...
                                         bcm_l2_addr_distribute_t *distribution)
{
    ...
    bcm_l2_addr_distribute_t
        old_distribution;
    int learn_flags=0, trans_flags=0, del_flags=0, age_refresh_flags=0, new_handle_profile;
    SOC_PPD_FRWRD_MACT_EVENT_HANDLE_KEY handle_key;
    SOC_PPD_FRWRD_MACT_EVENT_HANDLE_INFO handle_info;
    int learn_profile_first_appear;
    SOC_PPD_FRWRD_MACT_MAC_LIMIT_INFO limit_info;
    ...
    SOC_PPD_FRWRD_MACT_EVENT_HANDLE_KEY_clear(&handle_key);
    SOC_PPD_FRWRD_MACT_EVENT_HANDLE_INFO_clear(&handle_info);
    ...
    /*
      rv = _bcm_petra_l2_limit_get_by_index(unit,vsi_info.mac_learn_profile_id, &old_limit, &old_learn_profile);
      BCM_IF_ERROR_RETURN(rv); 
    */ 
    ...
    if(first_handle_appear != 0) {
        ...
        {
            /* learn */
            handle_key.event_type = SOC_PPD_FRWRD_MACT_EVENT_TYPE_LEARN;
            ...
            /* transplant */
            handle_key.event_type = SOC_PPD_FRWRD_MACT_EVENT_TYPE_TRANSPLANT;
            soc_sand_rv = soc_ppd_frwrd_mact_event_handle_info_get(soc_sand_dev_id,&handle_key,&handle_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            /* aged-out*/
            handle_key.event_type = SOC_PPD_FRWRD_MACT_EVENT_TYPE_AGED_OUT;
            soc_sand_rv = soc_ppd_frwrd_mact_event_handle_info_get(soc_sand_dev_id,&handle_key,&handle_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            handle_key.event_type = SOC_PPD_FRWRD_MACT_EVENT_TYPE_REFRESH;
            soc_sand_rv = soc_ppd_frwrd_mact_event_handle_info_get(soc_sand_dev_id,&handle_key,&handle_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                              ...
                              )
{
    ...
    /* more flags that are not support in Soc_petra-B */
    if(SOC_IS_PETRAB(unit)) {
         /* LCOV_EXCL_START */
        if (flags & (BCM_L2_REPLACE_DES_HIT_CLEAR|BCM_L2_REPLACE_MIGRATE_PORT)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("unsupported flags")));
        }
        /* has to be set in soc_petra-B*/
        if (!(flags & (BCM_L2_REPLACE_IGNORE_DES_HIT))) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("BCM_L2_REPLACE_IGNORE_DES_HIT has to be set")));
        }
         /* LCOV_EXCL_STOP */
    }
                                    ...
                                    )
{
    ...
    /* trap-code */
    new_val = action_profile->trap_code - SOC_PPD_TRAP_CODE_RESERVED_MC_0;

    /* trap */
    tmp = (action_profile->frwrd_action_strength) != 0;
    new_val |= (tmp) << 3;
    
    /* snoop */
    tmp = (action_profile->snoop_action_strength) != 0;
    new_val |= (tmp) << 4;
...
_bcm_petra_l2_cache_allocate_trap_fill(
                                       ...
                                       SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO *snoop_info
                                       )
{
    int rv = BCM_E_NONE;
    ...
    SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO_clear(snoop_info);
    ...
    if(addr->flags & BCM_L2_CACHE_SETPRI) {
        ...
        trap_info->bitmap_mask |= SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_TC;
        trap_info->strength = (SOC_DPP_CONFIG(unit)->pp.default_trap_strength);
    }
    ...
    if(trap_info->dest_info.frwrd_dest.type == SOC_PPD_NOF_FRWRD_DECISION_TYPES) {
        SOC_PPD_FRWRD_DECISION_DROP_SET(unit, &(trap_info->dest_info.frwrd_dest), soc_sand_rv);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
                                      ...
                                      )
{
    ...
    if(trap_info->strength != 0)
    {
        ...
        if(trap_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_DEST)
        { 
            switch(trap_info->dest_info.frwrd_dest.type)
            {
                case SOC_PPD_FRWRD_DECISION_TYPE_MC:
                    ...
                    break;
                case SOC_PPD_FRWRD_DECISION_TYPE_DROP:  /* if this drop trap, set discard flag */
                    addr->flags |= BCM_L2_CACHE_DISCARD;
                    break;
                case SOC_PPD_FRWRD_DECISION_TYPE_UC_LAG:
                    addr->flags |= BCM_L2_CACHE_TRUNK; /* fallrhtough still want to update the port */
                default:
        ...
        if(trap_info->bitmap_mask & SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE_TC)
        {
                          ...
                          int index)
{
    ...
    rv = bcm_petra_l2_cache_set_unset(
            ...
            index, 
            &empty_addr, 
            &index_lcl /*No use in this case*/,
            TRUE /* is-for-delete */);
    BCMDNX_IF_ERR_EXIT(rv);

    /*This check is only done in 6.4 because only here need to check if its new type index 
            Since new type index is saved to SW DB and needs to be deleted*/
    indx_type = (((index) >> _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_TYPE_MASK);
    if(indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL)
    {
         index = index & _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL_MASK;
         BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_l2_cache_sw_db_index_control_delete(unit,index));  
    }
                              ...
                              int unit)
{
    ...
    SOC_PPD_LLP_TRAP_RESERVED_MC_KEY
        mc_key;
    SOC_PPD_ACTION_PROFILE
        action_profile;
    SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER 
        prog_init_data;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO_clear(&trap_info);
        SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER_clear(&prog_init_data);
        
        trap_info.strength = 0;
        /* these init has no affect, as strength = 0 and not enabled in the trap */
        /* need to bypass verify */
        trap_info.dest_info.frwrd_dest.type = SOC_PPD_FRWRD_DECISION_TYPE_DROP; /* not affective as strength/mask is zero */
        ...
        /* disable traps */
        for(trap_code = SOC_PPD_TRAP_CODE_RESERVED_MC_0; trap_code <= SOC_PPD_TRAP_CODE_RESERVED_MC_7;  ++trap_code) {
            
            /* disable traps */
            soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id,trap_code,&(trap_info));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        action_profile.trap_code = SOC_PPD_TRAP_CODE_RESERVED_MC_0;
        ...
        for(port_profile = 0; port_profile < _BCM_PETRA_L2_CACHE_RM_PORT_PROFILES; ++port_profile) { 
            for(mac_lsb = 0; mac_lsb < _BCM_PETRA_L2_NOF_RM_MAC; ++mac_lsb) {
                ...
                SOC_PPD_LLP_TRAP_RESERVED_MC_KEY_clear(&mc_key);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        /* clear prog traps: */
        for(trap_code = SOC_PPD_TRAP_CODE_PROG_TRAP_0; trap_code <= SOC_PPD_TRAP_CODE_PROG_TRAP_3;  ++trap_code) {
    ...
    if (!SOC_WARM_BOOT(unit)) {
        SOC_PPD_LLP_TRAP_PORT_INFO_clear(&port_trap_info);
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_l2_cache_sw_db_index_control_delete_all(unit));
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
...
* Purpose:   Get needed values from software state of index control
* Params:
* unit           - which unit is it
* index_type_p   -(OUT) Type of index which is saved to SW DB (MC/PROG..)
...
* index_sw_db     - (IN) the idex to SW DB array which hold the needed data 
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
STATIC int _bcm_dpp_l2_cache_index_values_get(int unit,_bcm_l2_cache_index_type_e *index_type_p,
                                            int *mac_lsb_p,int *port_p, int *prog_profile_p,int index_sw_db)
{
    _bcm_l2_cache_index_control_t index_entry;
    BCMDNX_INIT_FUNC_DEFS;

    if(index_sw_db < 0 || index_sw_db > _BCM_NUMBER_OF_L2_CACHE_INDEX_CONTROL)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("index is out of range")));
    }
    BCMDNX_IF_ERR_EXIT(
    _bcm_dpp_l2_cache_sw_db_index_control_entry_get(unit,index_sw_db,&index_entry));

    if(index_type_p)
    {
         /* verify  param */
        if( index_entry.index_type == _BCM_L2_CACHE_INDEX_TYPE_INVALID || 
             index_entry.index_type >= _BCM_L2_CACHE_INDEX_TYPE_MAX)
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Bad type")));
        }
        *index_type_p = index_entry.index_type;
    }
    if(mac_lsb_p)
    {
        *mac_lsb_p = index_entry.mac_lsb;
    }
    if(port_p)
    {
         /* verify  param */
        if( index_entry.port >= _BCM_PETRA_NOF_LOCAL_PORTS(unit)) 
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("port is out of range")));
        }
        *port_p = index_entry.port;
    }
    if(prog_profile_p)
    {
        *prog_profile_p = index_entry.prog_profile;
    }
...
int bcm_petra_l2_cache_get(
                       ...
                       bcm_l2_cache_addr_t *addr)
{
    int port, prog_trap_profile, indx_type;
    _bcm_l2_cache_index_type_e index_type_new;
    int mac_lsb = 0;
    SOC_PPD_VSI_ID vsi = (uint32)-1;
    SOC_PPD_LLP_TRAP_RESERVED_MC_KEY
        mc_key;
    SOC_PPD_ACTION_PROFILE
        action_profile;
    ...
    SOC_PPD_VSI_L2CP_HANDLE_TYPE handle_type;
    int rv = BCM_E_NONE;
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
    uint32  pp_port;
    ...
    /*Only use for 6.4, if its now new index type means need to parse it old way
        otherwise just read the data from SW DB
        This case was done for ISSU since if the set was done before ISSU and than user call get the index is old type and not new*/
    if(indx_type != _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL)
    {
        switch(indx_type)
        {
            case _BCM_PETRA_L2_CACHE_INDEX_TYPE_RESERVED_MC:
                indx_type = _BCM_L2_CACHE_INDEX_TYPE_RESERVED_MC;
                break;
            case _BCM_PETRA_L2_CACHE_INDEX_TYPE_PROG:
                indx_type = _BCM_L2_CACHE_INDEX_TYPE_PROG;
                break;
            case _BCM_PETRA_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC:
                indx_type = _BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC;
                break;
            default:
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Bad index input")));
                break;
                
        }  
        port  = (((index) >> _BCM_PETRA_L2_CACHE_INDX_PORT_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_PORT_MASK);
        prog_trap_profile = (((index) >> _BCM_PETRA_L2_CACHE_INDX_PROFILE_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_PROFILE_MASK);
        
        if (indx_type == _BCM_L2_CACHE_INDEX_TYPE_RESERVED_MC) {
            mac_lsb = prog_trap_profile;
        }
        else if (indx_type == _BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC) 
        {
            mac_lsb = (((index) >> _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_MAC_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_MAC_MASK);
            vsi = (((index) >> _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_VSI_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_VSI_RSRVD_MC_VSI_MASK);
        }
    }
    else
    {/*If we got here means index was created new way - all params are in SW DB need to read from there*/
        index = index & _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL_MASK;
        BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_l2_cache_index_values_get(unit,&index_type_new,&mac_lsb,&port,&prog_trap_profile,index));
        indx_type = index_type_new;
        
        if (indx_type == _BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC)
        {     
            vsi = port;
        }
    ...
    else if(indx_type == _BCM_L2_CACHE_INDEX_TYPE_RESERVED_MC) /* reserved MC */ 
    {
    ...
    if(indx_type == _BCM_L2_CACHE_INDEX_TYPE_PROG) 
    {
        ...
        if(port_trap_info.trap_enable_mask & (SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << prog_trap_profile)) {
            rv = _bcm_dpp_am_template_l2_prog_trap_data_get(unit, port, prog_trap_profile, &trap_qual, &_bcm_trap_info);
            BCMDNX_IF_ERR_EXIT(rv);

            /* get snoop info */
            soc_sand_rv = soc_ppd_trap_snoop_profile_info_get(soc_sand_dev_id,prog_trap_profile + SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0,&snoop_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    
            rv = _bcm_petra_l2_cache_allocate_trap_get(unit,addr,&_bcm_trap_info.trap, &snoop_info);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        else
        {
        ...
        if(snoop_info.strength != 0 || _bcm_trap_info.trap.strength != 0) 
        {
            ...
            if (trap_qual.enable_bitmap & SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT_ETHER_TYPE) 
            {
                addr->ethertype = trap_qual.l2_info.ether_type;
                addr->ethertype_mask = 0xffff;
            }
            if (trap_qual.enable_bitmap & SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT_SUB_TYPE)
            {
    ...
    else if (indx_type == _BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC) 
    {
        ...
        case SOC_PPD_VSI_L2CP_HANDLE_TYPE_PEER:
            trap_code = SOC_PPD_TRAP_CODE_ETH_L2CP_PEER;
            break;
        case SOC_PPD_VSI_L2CP_HANDLE_TYPE_DROP:
            trap_code = SOC_PPD_TRAP_CODE_ETH_L2CP_DROP;
            break;
        default:
            trap_code = -1;
            break;
...
* Purpose:   Set the needed info in SW db, 
* Params:
* unit         - 
* index_type   - Type of index which is saved to SW DB (MC/PROG..)
* mac_lsb      - The last 8 bits of mac da address for which API was called
* port         -  entrance port of the packet
* prog_profile - profile of programable traps
* index_used_p -  out param which hold the index of SW DB index the data was saved to
* Return:    (int) ERROR type in case of error otherwise BCM_E_NONE
*******************************************************************************/
...
{
    int rv = BCM_E_NONE;
    int index_of_entry_in_sw_db = _BCM_DPP_L2_CAHCE_INVALID_INDEX;
    _bcm_l2_cache_index_control_t index_entry;
    BCMDNX_INIT_FUNC_DEFS;
    if(index_type == _BCM_L2_CACHE_INDEX_TYPE_INVALID || index_type >= _BCM_L2_CACHE_INDEX_TYPE_MAX)
    {
        rv = BCM_E_PARAM;
    }
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    BCMDNX_IF_ERR_EXIT(
    _bcm_dpp_l2_cache_sw_db_index_control_entry_find
        (unit,index_type,mac_lsb,port,prog_profile,&index_of_entry_in_sw_db));

    /*if we have such entry*/
    if( index_of_entry_in_sw_db != _BCM_DPP_L2_CAHCE_INVALID_INDEX)
    {
        *index_used_p = index_of_entry_in_sw_db;
    }
    else
    {
        index_entry.index_type = index_type;
        index_entry.mac_lsb = mac_lsb;
        index_entry.port = port;
        index_entry.prog_profile = prog_profile;
        
        BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_l2_cache_sw_db_index_control_free_entry_get(unit,index_used_p));
        BCMDNX_IF_ERR_EXIT(
        _bcm_dpp_l2_cache_sw_db_index_control_entry_set(unit,*index_used_p,&index_entry));
    }
...
* Params:
* unit          - which unit is it
* index         - ony valid in case of delete otherwise its -1
* addr          - Structure which hold mac addrress and port etc. which configure the virtual traps
* index_used    - index of SW DB wich type of _bcm_l2_cache_index_control_t
                       ...
                       int is_for_delete)
{
    ...
    int old_profile, rm_is_last, new_profile = 0, rm_is_first;
    int prog_is_first,prog_old_profile, prog_is_last = 0;
    ...
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO trap_clean;
    SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO snoop_info_clean;
    _bcm_dpp_gport_info_t gport_info;
    uint32  pp_port;
    int     core;
    uint32 new_val = 0;
    ...
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO_clear(&_bcm_trap_info.trap);
    SOC_PPD_TRAP_FRWRD_ACTION_PROFILE_INFO_clear(&trap_clean);
    trap_clean.dest_info.frwrd_dest.type = SOC_PPC_FRWRD_DECISION_TYPE_DROP;
    SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO_clear(&snoop_info);
    SOC_PPD_TRAP_SNOOP_ACTION_PROFILE_INFO_clear(&snoop_info_clean);
    ...
    SOC_PPD_ACTION_PROFILE_clear(&action_info);
    SOC_PPD_LLP_TRAP_RESERVED_MC_KEY_clear(&mc_key);

    if((is_for_delete == FALSE) && (index != -1)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("index has to be -1")));/* index has to be -1*/
    }
    ...
    if ((addr->dest_port < 0) && (addr->dest_port != BCM_GPORT_INVALID)) { /* When deleting action set is called with BCM_GPORT_INVALID */
    ...
    if (addr->dest_port != BCM_GPORT_TYPE_NONE && (addr->flags & BCM_L2_CACHE_DISCARD)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Illegal both are set designation and discard flag")));
    }

    if(addr->src_port_mask != 0 || addr->lookup_class != 0 || addr->encap_id != 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Not supported mask values")));
    }

    /* unsupported flags */
    if(addr->flags & (BCM_L2_CACHE_MIRROR|BCM_L2_CACHE_L3|BCM_L2_CACHE_REMOTE_LOOKUP|BCM_L2_CACHE_DESTPORTS))
    {
        ...
        (addr->vlan_mask == (bcm_vlan_t)(-1))) 
    {
        ...
        if (addr->flags & BCM_L2_CACHE_TUNNEL) {
            ...
            if ((trap_id == SOC_PPD_TRAP_CODE_ETH_L2CP_PEER) || (trap_id == SOC_PPD_TRAP_CODE_ETH_L2CP_DROP)) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Single action per API is supported")));
            }
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_id == SOC_PPD_TRAP_CODE_ETH_L2CP_PEER)){
            handle_type = SOC_PPD_VSI_L2CP_HANDLE_TYPE_PEER;
        }
        else if ((BCM_GPORT_IS_TRAP(addr->dest_port)) && (trap_id == SOC_PPD_TRAP_CODE_ETH_L2CP_DROP)){
        ...
        rv = _bcm_dpp_am_template_vsi_egress_profile_exchange(unit, 0/*flags*/, vsi, &vsi_egress_profile_data, &vsi_egress_old_profile, 
                                                              &vsi_egress_profile_is_last, &egress_profile, &vsi_egress_profile_is_first);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = _bcm_dpp_am_template_vsi_ingress_profile_exchange(unit, 0/*flags*/, vsi, &vsi_ingress_profile_data, &vsi_ingress_old_profile, 
        ...
        if (vsi_ingress_profile_is_last || vsi_egress_profile_is_last) {
            ...
            rv = _bcm_dpp_l2_vsi_profile_update(unit, vsi_egress_old_profile, vsi_ingress_old_profile, vsi_egress_profile_data_clean, vsi_ingress_profile_data_clean);
            BCMDNX_IF_ERR_EXIT(rv);
        }

        if (vsi_ingress_profile_is_first || vsi_egress_profile_is_first) {
            rv = _bcm_dpp_l2_vsi_profile_update(unit, egress_profile, ingress_profile, vsi_egress_profile_data, vsi_ingress_profile_data);
            BCMDNX_IF_ERR_EXIT(rv);
        }
        ...
        vsi_info.profile_ingress = (vsi_info.profile_ingress & 0xFFFFFFFC) | (ingress_profile & 0x3); /* profile is only 2 lsbits of range 0-15 */
        ...
        if(is_for_delete == FALSE)
        {
            ...
            _bcm_dpp_l2_cache_index_values_set(unit,_BCM_L2_CACHE_INDEX_TYPE_VSI_RESERVED_MC,mac_lsb,vsi,0,index_used));
            /*This is done only for 6.4 ISSU reason. to mark the index was created new way - holds data in SW DB*/
            *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL << _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) | *index_used;          
        }/*In case of deletion the index is delete in bcm_petra_l2_cache_delete*/
    }
    /*is reserved MC and port */
    /* for reserved MC profile has to be -1 */
        ...
        (addr->ethertype_mask == 0x0) && (addr->vlan_mask == 0)) 
    {
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            rv  = _bcm_dpp_am_template_l2_trap_rm_exchange(unit, BCM_DPP_AM_TEMPLATE_FLAG_CHECK, port_i, mac_lsb, &_bcm_trap_info,
                                                           &old_trap_code, &trap_is_last, &new_trap_code, &trap_is_first, &virtual_traps_p);
            BCMDNX_IF_ERR_EXIT(rv);
            break;
        }
        ...
        mc_key.da_mac_address_lsb = mac_lsb;
        SOC_PPD_ACTION_PROFILE_clear(&action_info);
        action_info.trap_code = (new_trap_code-1) + SOC_PPD_TRAP_CODE_RESERVED_MC_0;
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            ...
            else
            {
                rv  = _bcm_dpp_am_template_l2_cache_rm_data_get(unit, port_i, &rsrvd_info);
                BCMDNX_IF_ERR_EXIT(rv);
                /*mac_buff is a mask of  snoop_on 1bit| trap_on 1bit| trap_code 3bit*/
            ...
            rv  = _bcm_dpp_am_template_l2_cache_rm_exchange(unit, 0, port_i, &rsrvd_info, &old_profile, &rm_is_last, &new_profile, &rm_is_first);
            BCMDNX_IF_ERR_EXIT(rv);
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) 
        {
            ...
            rv  = _bcm_dpp_am_template_l2_trap_rm_exchange(unit, 0, port_i, mac_lsb, &_bcm_trap_info, &old_trap_code, &trap_is_last, &new_trap_code, &trap_is_first,&virtual_traps_p);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            action_info.trap_code = SOC_PPD_TRAP_CODE_RESERVED_MC_0 + (new_trap_code-1);

            if ( (BCM_GPORT_IS_TRAP(addr->dest_port)) && (_BCM_TRAP_ID_IS_VIRTUAL(trap_id)) )
            {
                rv =  soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id,virtual_traps_p.soc_ppd_reserved_mc,&_bcm_trap_info.trap);
                BCM_SAND_IF_ERR_EXIT(rv);
                /*Connect snoop to trap*/
                rv =  soc_ppd_trap_snoop_profile_info_set(soc_sand_dev_id,virtual_traps_p.soc_ppd_reserved_mc,&snoop_info);
                BCM_SAND_IF_ERR_EXIT(rv);
            }
            ...
            {
                soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id, action_info.trap_code, &(_bcm_trap_info.trap));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            if (trap_is_last && (old_trap_code != 0)) {
                trap_code = SOC_PPD_TRAP_CODE_RESERVED_MC_0 + (old_trap_code-1);
                soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id, trap_code, &trap_clean);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            ...
            if(rm_is_first && (new_trap_code != 0)) {
                ...
                mc_key.reserved_mc_profile = new_profile;
                soc_sand_rv = soc_ppd_llp_trap_reserved_mc_info_set(soc_sand_dev_id, &mc_key, &action_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }

            if (rm_is_last && (old_trap_code != 0)) {
                /* clean-up previous configuration */
                SOC_PPC_ACTION_PROFILE_clear(&action_info);
                action_info.trap_code = (old_trap_code-1) + SOC_PPD_TRAP_CODE_RESERVED_MC_0;
                mc_key.reserved_mc_profile = old_profile;
                soc_sand_rv = soc_ppd_llp_trap_reserved_mc_info_set(soc_sand_dev_id, &mc_key, &action_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
            ...
            port_trap_info.reserved_mc_profile = new_profile;
            ...
            {
                BCM_SAND_IF_ERR_EXIT(
                _bcm_dpp_l2_cache_index_values_set(unit,_BCM_L2_CACHE_INDEX_TYPE_RESERVED_MC,mac_lsb,port_i,0,index_used));
                 /*This is done only for 6.4 ISSU reason. to mark the index was created new way - holds data in SW DB*/
               *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL << _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) | *index_used;
            } /*In case of deletion the index is delete in bcm_petra_l2_cache_delete*/
            break;
    ...
    /*is programmable trap */
        ...
        SOC_PPD_LLP_TRAP_PROG_TRAP_QUALIFIER_clear(&trap_qual);
        ...
        if(nof_bits > 0) {
            trap_qual.enable_bitmap |= SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT_DA;
        }
        if(addr->flags & BCM_L2_CACHE_SUBTYPE) {
            trap_qual.l2_info.sub_type_bitmap = 0xf;
            trap_qual.enable_bitmap |= SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT_SUB_TYPE;
        }

        if((addr->ethertype_mask == 0xffff)) {
            trap_qual.l2_info.ether_type = addr->ethertype;
            trap_qual.enable_bitmap |= SOC_PPD_LLP_TRAP_PROG_TRAP_COND_SELECT_ETHER_TYPE;
        }
        ...
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
            ...
            if (is_for_delete == FALSE) {
                ...
                rv  = _bcm_dpp_am_template_l2_prog_trap_alloc(unit, 0, &trap_qual, &_bcm_trap_info, &new_profile, &prog_is_first, &virtual_traps_p, is_virtual_traps_p_set);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                /* De-alloc */
                /*Only done in 6.4 since only in 6.4 there are different index , one that in SW DB which is new
                            And one is bitmask which is old*/
                 indx_type = (((index) >> _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_TYPE_MASK);
                if(indx_type == _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL)
                { /*if new index type read from SW DB prog profile*/
                    index = index & _BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL_MASK;
                    _bcm_dpp_l2_cache_index_values_get(unit,0,0,0,&new_profile,index);
                }
                else
                { /*If old read from index*/
                    new_profile = (((index) >> _BCM_PETRA_L2_CACHE_INDX_PROFILE_SHIFT) & _BCM_PETRA_L2_CACHE_INDX_PROFILE_MASK);
                }

                /* this trap is enabled in the given trap, get it's content*/
                soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id, core, pp_port, &port_trap_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                if(port_trap_info.trap_enable_mask & (SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile)) {
                    rv  = _bcm_dpp_am_template_l2_prog_trap_free(unit, is_virtual_traps_p_set, new_profile, &prog_old_profile, &prog_is_last);
                    BCM_IF_ERROR_RETURN(rv);
                } else {
                    /* unexpected index */
                    BCM_EXIT;
            ...
            new_trap_code = new_profile + SOC_PPD_TRAP_CODE_PROG_TRAP_0;


            if ( (BCM_GPORT_IS_TRAP(addr->dest_port)) && (_BCM_TRAP_ID_IS_VIRTUAL(trap_id)) ){
         
                rv =  soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id,virtual_traps_p.soc_ppd_prog,&_bcm_trap_info.trap);
                BCM_SAND_IF_ERR_EXIT(rv);

                /*Connect snoop to trap*/
                rv = soc_ppd_trap_snoop_profile_info_set(soc_sand_dev_id,virtual_traps_p.soc_ppd_prog,&snoop_info);
                BCM_SAND_IF_ERR_EXIT(rv);            
            }
            ...
            if(((is_for_delete == FALSE) && prog_is_first) 
               || (is_for_delete && prog_is_last)) {
                ...
                /* update trap result */
                soc_sand_rv = soc_ppd_trap_frwrd_profile_info_set(soc_sand_dev_id,new_trap_code,&(_bcm_trap_info.trap));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                soc_sand_rv = soc_ppd_trap_snoop_profile_info_set(soc_sand_dev_id,new_trap_code,&snoop_info);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                /* update trap condition */
                soc_sand_rv = soc_ppd_llp_trap_prog_trap_info_set(soc_sand_dev_id,new_profile,&trap_qual);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            }
            /* update port config */
            soc_sand_rv = soc_ppd_llp_trap_port_info_get(soc_sand_dev_id,core,pp_port,&port_trap_info);
            ...
            if ((is_for_delete == FALSE) && (port_trap_info.trap_enable_mask & (SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile))) {
                rv  = _bcm_dpp_am_template_l2_prog_trap_free(unit, is_virtual_traps_p_set, new_profile, &prog_old_profile, &prog_is_last);
                BCM_IF_ERROR_RETURN(rv);
            } else {
                if (is_for_delete == FALSE) {
                    port_trap_info.trap_enable_mask |= (SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile);
                } else {
                    port_trap_info.trap_enable_mask &= ~(SOC_PPD_LLP_TRAP_PORT_ENABLE_PROG_TRAP_0 << new_profile);
                }
                soc_sand_rv = soc_ppd_llp_trap_port_info_set(soc_sand_dev_id,core,pp_port,&port_trap_info);
            ...
            {
                BCM_SAND_IF_ERR_EXIT(
                _bcm_dpp_l2_cache_index_values_set(unit,_BCM_L2_CACHE_INDEX_TYPE_PROG,0,port_i,new_profile,index_used));
                 /*This is done only for 6.4 ISSU reason. to mark the index was created new way - holds data in SW DB*/
                *index_used = (_BCM_PETRA_L2_CACHE_INDX_TYPE_NEW_INDEX_CONTROL << _BCM_PETRA_L2_CACHE_INDX_TYPE_SHIFT) | *index_used;
            }/*In case of deletion the index is delete in bcm_petra_l2_cache_delete*/
            break;
 ...
 */
{
    ...
    if (l2cp_profile_info->nof_entries > 0)
    {
        for (i=0; i < l2cp_profile_info->nof_entries; i++)
        {
            ...
            /* here the low and hi are swapped intentionally according to HW requirements */
            COMPILER_64_SET(new_bitmap, low, hi);
    ...
    if (trap_type == bcmRxTrapL2cpDrop)
    {
        ...
        sal_memcpy((uint32 *)(&(reg_above_64))+(profile_idx<<1), &bit_mask, BCM_L2CP_PROFILE_LEN_IN_BYTES);
...

{
	...
	if (trap_type == bcmRxTrapL2cpDrop)
	{
	    rv = READ_IHP_MEF_L_2_CP_DROP_BITMAPr(unit, 0, reg_above_64);
	    BCM_SAND_IF_ERR_EXIT(rv);
	    /* prototype code in arad_pp_vsi.c */
	    /* update in the buffer only one profile; the buffer is defined as 20 units of uint32;
	     * each profile occupies 2 units of uint32;
	     * offset in buffer is used according to profile index.*/
	}
                               ...
                               bcm_dpp_vsi_ingress_profile_t ingress_profile_data)
{
    ...
    {
        if (egress_profile > 0) {
            soc_sand_rv = soc_ppd_vsi_egress_mtu_set(soc_sand_dev_id, egress_profile, egress_profile_data.mtu);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }

    SOC_PPD_VSI_L2CP_KEY_clear(&l2cp_key);
...
{
    BCMDNX_INIT_FUNC_DEFS;
    /*The size is fictional since its shifts on index being taken into account*/
    *size = _BCM_PETRA_L2_CACHE_INDX_MAX_VAL;
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}
                        ...
                        bcm_vlan_t vlan)
{
    ...
    if (SOC_IS_ARAD(unit)) {
     ...
     /* LCOV_EXCL_START */
    /* vlan has to be simple vlan */
    BCM_DPP_VLAN_CHK_ID(unit,vlan);

    /* check if this Vlan already associated with A mac */
    rv = _bcm_petra_l2_tunnel_get(unit,vlan,exist_mac,&flags);
    BCMDNX_IF_ERR_EXIT(rv);
    /* if this vlan already associated with MAC then check we are not changing it */
    if(flags != 0) {
        /* not as exist MAC */
        if(!_BCM_PETRA_L2_IS_MAC_EQUAL(mac,exist_mac)){
            BCMDNX_ERR_EXIT_MSG(BCM_E_RESOURCE, (_BSL_BCM_MSG("set more than one mac to same vlan"))); /* set more than one mac to same vlan: no resources */
        }
    }

    /* set MAC*/
    rv = _bcm_petra_mpls_intf_create(unit,mac,vlan);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
    /* LCOV_EXCL_STOP */
                           ...
                           bcm_vlan_t vlan)
{
    ...
    /* get MAC associated with the given vlan */
    rv = _bcm_petra_l2_tunnel_get(unit,vlan,exist_mac,&flags);
    BCMDNX_IF_ERR_EXIT(rv);
    
    /* if MPLS routing is not enable then nothing to do*/
    if (!(flags & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS)) {
        BCM_EXIT; /* no such mac vlan on port, nothting to do */
    }

    /* if mac not exist, nothing to delete */
    if(!_BCM_PETRA_L2_IS_MAC_EQUAL(mac,exist_mac)){
        BCM_EXIT; /* no such mac vlan on port, nothting to do */
    }
    
    /* this mac exist on the vlan, disable mpls routing on RIF */
    rv = _bcm_petra_vid_to_rif(unit,vlan,&rif_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if(!(rif_info.routing_enablers_bm & SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("RIF not exist")));
    }

    /* disable MPLS routing on this VSI  */
    rif_info.routing_enablers_bm &= ~SOC_PPD_RIF_ROUTE_ENABLE_TYPE_MPLS;

    soc_sand_rv = soc_ppd_rif_vsid_map_set(soc_sand_dev_id,rif_id,&rif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                               ...
                               int unit)
{
    ...
    /* just run over all Vlans/RIFs and disable mpls routing */
    for(vlan = 0; vlan <= BCM_VLAN_MAX; ++vlan) {
        /* this mac exist on the vlan, disable mpls routing on RIF */
        rv = _bcm_petra_vid_to_rif(unit,vlan,&rif_id);
        BCMDNX_IF_ERR_EXIT(rv);
                  ...
                  int unit)
{
    ...
    uint32 soc_sand_rv;
    unsigned int soc_sand_dev_id;
    SOC_PPD_FRWRD_MACT_OPER_MODE_INFO oper_mode_info;
    soc_info_t  *si;
    ...
    */
    ...
    /* for traverse */
    BCMDNX_ALLOC(_bcm_l2_traverse_mact_keys[unit], sizeof(SOC_PPD_FRWRD_MACT_ENTRY_KEY) * _BCM_PETRA_L2_TRVRS_ITER_BLK_SIZE,"l2 traverse keys");
    if(_bcm_l2_traverse_mact_keys[unit] == NULL)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Malloc Fail")));
    }

    BCMDNX_ALLOC(_bcm_l2_traverse_mact_vals[unit], sizeof(SOC_PPD_FRWRD_MACT_ENTRY_VALUE) * _BCM_PETRA_L2_TRVRS_ITER_BLK_SIZE,"l2 traverse info");
    if(_bcm_l2_traverse_mact_vals[unit] == NULL)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Malloc Fail")));
    }
    ...
    BCMDNX_ALLOC(_bcm_l2_auth_traverse_mac_infos[unit], sizeof(SOC_PPD_LLP_SA_AUTH_MAC_INFO) * _BCM_PETRA_L2_TRVRS_ITER_BLK_SIZE,"l2 auth traverse info");
    if(_bcm_l2_auth_traverse_mac_infos[unit] == NULL)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_MEMORY, (_BSL_BCM_MSG("Malloc Fail")));
    }
    ...
    rv = _bcm_petra_l2_event_handle_buff_build(BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,
												BCM_L2_ADDR_DIST_SET_NO_DISTRIBUTER, BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,&buf);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        SOC_PPD_FRWRD_MACT_OPER_MODE_INFO_clear(&oper_mode_info);
        ...
        if (olp_receive_header_type == SOC_TMC_PORT_HEADER_TYPE_RAW)
        {
            oper_mode_info.shadow_msgs_info.info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
            oper_mode_info.learn_msgs_info.info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
        }
        else
        {
            oper_mode_info.shadow_msgs_info.info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_OTMH;
            oper_mode_info.learn_msgs_info.info.olp_receive_header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_OTMH;
        }

        if (olp_transmit_header_type == SOC_TMC_PORT_HEADER_TYPE_RAW)
        {
            oper_mode_info.shadow_msgs_info.info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
            oper_mode_info.learn_msgs_info.info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_RAW;
        }
        else
        {
            oper_mode_info.shadow_msgs_info.info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ITMH;
            oper_mode_info.learn_msgs_info.info.header_type = SOC_PPD_FRWRD_MACT_MSG_HDR_TYPE_ITMH;
        }

        if (soc_property_suffix_num_get(unit,-1, spn_CUSTOM_FEATURE, "egress_independent_default_mode", 0)) {
            /* Default value for tests that do not want extra DSP messages */
            oper_mode_info.learning_mode = SOC_PPD_FRWRD_MACT_LEARNING_MODE_EGRESS_INDEPENDENT;
        }
        else
        {
            /* Learn default value */
            oper_mode_info.learning_mode = SOC_PPD_FRWRD_MACT_LEARNING_MODE_INGRESS_DISTRIBUTED;
        }

        oper_mode_info.shadow_mode = SOC_PPD_FRWRD_MACT_SHADOW_MODE_NONE;
        oper_mode_info.learn_msgs_info.type = SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_MSG_AGGR;
        oper_mode_info.shadow_msgs_info.type = SOC_PPD_FRWRD_MACT_EVENT_PATH_TYPE_DIRECT_ACCESS;
                         ...
                         bcm_l2_station_t *station)
{
    ...
    if ((station->flags & BCM_L2_STATION_OAM) && !station->vlan_mask) { /* OAM endpoint */
        ...
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_my_cfm_mac_set, (unit, &my_mac, station->src_port));
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (station->src_port_mask != 0) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("src port is relevent only if flag BCM_L2_STATION_MIM is set")));
        }
        ...
        if (station->vlan_mask == 0) { /* vsi is not valid - set my-Mac global MSB */
            ...
            *station_id = station->vlan;
            ...
            /* check mac mask is correct - LSB */
            if (SOC_IS_ARAD(unit)) { /* LSB is 12b' */
                soc_sand_rv = soc_sand_pp_mac_address_string_parse("000000000FFF", &mac_mask);
            }
            else { /* LSB is 6b' */
                soc_sand_rv = soc_sand_pp_mac_address_string_parse("00000000003F", &mac_mask);
            }
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            soc_sand_rv = soc_sand_pp_mac_address_are_equal(&mac_mask, &my_mac_mask, &is_equal);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            if (!is_equal) {
                if (SOC_IS_ARAD(unit)) {
                    ...
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("mac_mask must be 00000000003F")));
                }
            }

            if (SOC_IS_ARAD(unit)) { /* LSB is 12b' */
                my_mac.address[1] &= ARAD_PP_MYMAC_VSI_LSB_MAX1;
            }
            else { /* LSB is 6b' */
                my_mac.address[0] &= SOC_PB_PP_MYMAC_VSI_LSB_MAX;
            }
                         ...
                         bcm_l2_station_t *station)
{
    ...
    if (BCM_PETRA_L2_STATION_IS_OAM_CFM(station_id)) { /* OAM endpoint */
        ...
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_my_cfm_mac_get, (unit, &my_mac, port));
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if (vsi == BCM_PETRA_L2_STATION_GLOBAL_MSB) { /* vsi is not valid */
            ...
            if (SOC_IS_ARAD(unit)) { /* LSB is 12b' */
                ...
                soc_sand_rv = soc_sand_pp_mac_address_string_parse("00000000003F", &my_mac_mask);
            }
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            rv = _bcm_petra_mac_from_sand_mac(station->dst_mac_mask, &my_mac_mask);
            BCMDNX_IF_ERR_EXIT(rv);
                            ...
                            int station_id)
{
    ...
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        /* Delete entry.*/
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_my_cfm_mac_delete, (unit, &my_mac, port));
        BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_petra_l2_auth_from_sand_auth_info(
                                           ...
                                           SOC_PPD_LLP_SA_AUTH_MAC_INFO *mac_auth_info)
{
    int rv = BCM_E_NONE;
    bcm_module_t modid;
    bcm_port_t mode_port;
    ...
    if (mac_auth_info->expect_system_port.sys_id != SOC_PPD_LLP_SA_AUTH_ACCEPT_ALL_PORTS) {
    ...
    if (mac_auth_info->expect_tag_vid != SOC_PPD_LLP_SA_AUTH_ACCEPT_ALL_VIDS) {
                                         ...
                                         SOC_PPD_LLP_SA_AUTH_MAC_INFO *mac_auth_info)
{
    ...
    SOC_PPD_LLP_SA_AUTH_MAC_INFO_clear(mac_auth_info);
                             ...
                             int unit)
{
    ...
    unsigned int soc_sand_dev_id;
    uint32 soc_sand_rv;
    SOC_PPD_LLP_SA_AUTH_MATCH_RULE rule;
    SOC_SAND_TABLE_BLOCK_RANGE block_range;
...
/* for deleting all, ignore port and vid to match every entry */
    SOC_PPD_LLP_SA_AUTH_MATCH_RULE_clear(&rule);
    rule.port.sys_id = SOC_PPD_IGNORE_VAL;
    rule.vid = SOC_PPD_IGNORE_VAL;
    rule.rule_type = SOC_PPD_LLP_SA_MATCH_RULE_TYPE_ALL;
    ...
    for (indx = 0; indx < _BCM_PETRA_L2_TRVRS_ITER_BLK_SIZE; indx++) {
        ...
        SOC_PPD_LLP_SA_AUTH_MAC_INFO_clear(&_bcm_l2_auth_traverse_mac_infos[unit][indx]);
    }
...
int _bcm_dpp_l2_event_fifo_interrupt_handler(int unit, uint32 *buf) {
    ...
    bcm_l2_addr_t 
        l2addr;
    SOC_PPD_FRWRD_MACT_EVENT_BUFFER   
        event_buf;
    SOC_PPD_FRWRD_MACT_EVENT_INFO   
        mact_event;
    ...
    SOC_PPD_FRWRD_MACT_EVENT_BUFFER_clear(&event_buf);
    SOC_PPD_FRWRD_MACT_EVENT_INFO_clear(&mact_event);

    event_buf.buff_len = SOC_PPD_FRWRD_MACT_EVENT_BUFF_MAX_SIZE;
    ...
    switch (mact_event.type) {
        case SOC_PPD_FRWRD_MACT_EVENT_TYPE_LEARN:
            _BCM_PETRA_L2_CB_RUN(unit, &l2addr, BCM_L2_CALLBACK_LEARN_EVENT);
            break;
        case SOC_PPD_FRWRD_MACT_EVENT_TYPE_TRANSPLANT:
            _BCM_PETRA_L2_CB_RUN(unit, &l2addr, BCM_L2_CALLBACK_MOVE_EVENT);
            break;
        case SOC_PPD_FRWRD_MACT_EVENT_TYPE_AGED_OUT:
...
_bcm_petra_l2_egress_create_verify(int unit, bcm_l2_egress_t *egr) {
    ...
    if (egr->flags & BCM_L2_EGRESS_REPLACE) {
        ...
        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, local_outlif, NULL, &usage) );
 ...
 */
STATIC int 
_bcm_petra_l2_egress_eedb_roo_ll_eth_type_get(int unit, int outlif, int *eth_type_index_ndx) {
    int rv;    
...
bcm_petra_l2_egress_create(int unit, bcm_l2_egress_t *egr)
{
    ...
    if (!update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            rv = _bcm_dpp_gport_alloc_global_and_local_lif(unit, alloc_flags, &global_outlif, NULL, &local_out_lif_info, TRUE);
            BCM_IF_ERROR_RETURN(rv);
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (egr->mpls_extended_label_value) {
            ...
            eth_type_index_entry.tpid_0 = egr->outer_tpid; 
            eth_type_index_entry.tpid_1 = egr->inner_tpid;
        }
        ...
        if (!egr->mpls_extended_label_value) {
            ...
            if (update) {
                ...
                /* get the current index for CfgEtherTypeIndex from eedb entry */
                rv = _bcm_petra_l2_egress_eedb_roo_ll_eth_type_get(unit, local_ll_outlif, &old_eth_type_index_ndx); 
                BCMDNX_IF_ERR_EXIT(rv);
        ...
        } else if (!mpls_extended_label_ether_type_index_table_is_set) {
    ...
    bcm_l2_egress_t *egr)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_lif_to_sw_resources(unit, -1, local_outlif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS, &gport_sw_resources));
    ...
    bcm_if_t encap_id)
{
    ...
    /* free used outlif (SW) */
    rv = (SOC_IS_JERICHO(unit)) ? _bcm_dpp_gport_delete_global_and_local_lif(unit, global_outlif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, local_outlif, TRUE) : bcm_dpp_am_l3_eep_dealloc(unit, local_outlif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_lif_sw_resources_delete(unit, -1, local_outlif, _BCM_DPP_GPORT_SW_RESOURCES_EGRESS);
    BCMDNX_IF_ERR_EXIT(rv); 
