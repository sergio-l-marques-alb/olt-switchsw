...
_bcm_dpp_extender_lif_resources_validate_get(int unit, int extender_port_id, _bcm_dpp_gport_hw_resources *extender_lif_resources, uint8 *is_local, uint8 *remote_lif){
    ...
    rv = _bcm_dpp_gport_l2_intf_gport_has_remote_lif(unit, extender_port_id, remote_lif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!(*remote_lif)) {
        if (extender_lif_resources->local_in_lif != _BCM_GPORT_ENCAP_ID_LIF_INVALID) {
            rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, extender_lif_resources->local_in_lif, _BCM_GPORT_ENCAP_ID_LIF_INVALID, &lif_type, NULL);
        } else {
            rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, extender_lif_resources->local_out_lif, NULL, &lif_type);
        }
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        _bcm_dpp_gport_hw_resources *extender_lif_resources)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        rv = _bcm_dpp_gport_delete_global_and_local_lif(unit, global_lif, extender_lif_resources->local_in_lif, 
                                                        extender_lif_resources->local_out_lif, TRUE);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        SOC_PPC_L2_LIF_EXTENDER_INFO *in_extender_info)
{
    ...
    if (is_update) {
        ...
        rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_l2_lif_extender_get, (unit, in_extender_key, (SOC_PPD_LIF_ID *)&in_lif_id, in_extender_info, &found));
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        bcm_extender_port_t *extender_port)
{
    ...
    if (!is_remote_lif) {
        ...
        rv = _bcm_dpp_lif_sw_resources_lif_usage_set(unit, _BCM_GPORT_ENCAP_ID_LIF_INVALID, extender_lif_resources.local_out_lif, 0, _bcmDppLifTypeExtender);
        BCMDNX_IF_ERR_EXIT(rv);
    }
        ...
        bcm_gport_t    extender_port_id)
{
    ...
    if (!remote_lif) {
        ...
        rv = _bcm_dpp_lif_sw_resources_delete(unit, extender_lif_resources.local_in_lif, extender_lif_resources.local_out_lif, 
