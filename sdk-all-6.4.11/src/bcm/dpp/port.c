 ...
 */
void
_bcm_dpp_port_outlif_profile_preset(int unit, SOC_PPD_EG_ENCAP_ENTRY_INFO *encap_entry_info, uint32 *outlif_profile){
            ...
            if (BCM_GPORT_IS_SET(port)) {
                if (BCM_GPORT_IS_LOCAL(port)) {
                    ...
                    if (BCM_DPP_PORT_IS_INTERFACE(port)) {
                        ...
                        if (port < FABRIC_LOGICAL_PORT_BASE(unit)) {
                            is_local_port = 1; 
                        }
                    }
                }
         ...
         bcm_port_ability_t *ability_mask)
{
  ...
  BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (SOC_IS_PETRAB(unit)) {
        BCM_PBMP_ITER(gport_info.pbmp_local_ports, local_port) {
            info.action_mask |= (BCM_PORT_ATTR_DUPLEX_MASK | BCM_PORT_ATTR_SPEED_MAX_MASK);
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    if (SOC_IS_JERICHO(unit)) { /* shift QSGMII lanes */
        ...
        if (interface_info->interface == SOC_PORT_IF_QSGMII) {
            BCM_PBMP_CLEAR(phy_ports);
            SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_qsgmii_id_to_phy_pbmp_get, (unit, port, interface_info->interface_id, nof_lanes, &phy_ports)));
        }
    }
    ...
    rv = sw_state_access[unit].dpp.soc.arad.tm.logical_ports_info.is_tm_pph_present_enabled.set(unit,port,is_tm_pph_present_enabled);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    port_info.flags = (SOC_PPD_PORT_EGQ_PP_PPCT_TBL |
                       SOC_PPD_PORT_IHB_PINFO_FLP_TBL |
                       SOC_PPD_PORT_IHP_PINFO_LLR_TBL);
        ...
        for(prs_nfo.inner_tpid = SOC_PPD_LLP_PARSE_TPID_INDEX_NONE; prs_nfo.inner_tpid < SOC_PPD_NOF_LLP_PARSE_TPID_INDEXS; ++prs_nfo.inner_tpid) {
            for (prs_nfo.is_outer_prio = FALSE; prs_nfo.is_outer_prio <= TRUE; ++prs_nfo.is_outer_prio) {
                ...
                case _bcm_petra_tpid_profile_outer_c_tag:
                    if(prs_nfo.outer_tpid == SOC_PPD_LLP_PARSE_TPID_INDEX_NONE) {   /* untagged */
                        pkt_frmt_info.tag_format = SOC_SAND_PP_ETHERNET_FRAME_VLAN_FORMAT_NONE;
                    }
                    else{   /* c-tagged */
                        pkt_frmt_info.tag_format = SOC_SAND_PP_ETHERNET_FRAME_VLAN_FORMAT_C_TAG;
    ...
    _bcm_petra_ac_key_map_type_t ac_key_map_type)
{
    ...
    if (old_tpid_profile != new_tpid_profile) {
      ...
      port_info.flags = (SOC_PPD_PORT_IHP_PINFO_LLR_TBL | SOC_PPD_PORT_EPNI_PP_PCT_TBL);
   ...
   const SOC_OCC_MGMT_OUTLIF_APP kind)
{
  ...
  soc_sand_rv =  _bcm_dpp_gport_to_hw_resources(unit, port,_BCM_DPP_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS, &gport_hw_resources);
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

  soc_sand_rv =
      soc_ppd_eg_encap_entry_get(unit,
                                 SOC_PPD_EG_ENCAP_EEP_TYPE_LIF_EEP,
                                 gport_hw_resources.local_out_lif, 0,
                                 encap_entry_info, next_eep,
                                 &nof_entries);
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

/* arg holds the two bit value, of ptagged type */
  soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_set,
                                    (unit, SOC_OCC_MGMT_TYPE_OUTLIF, kind, arg,
                                     &(encap_entry_info->entry_val.pwe_info.outlif_profile)));
  BCMDNX_IF_ERR_EXIT(soc_sand_rv);

  soc_sand_rv =
      soc_ppd_eg_encap_pwe_entry_add(unit, gport_hw_resources.local_out_lif, &encap_entry_info->entry_val.pwe_info, next_eep[0]);
  BCM_SAND_IF_ERR_EXIT(soc_sand_rv); 
                           ...
                           bcm_port_control_t type, int value)
{
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
         ...
         case bcmPortControlFcoeNetworkPort:
            ...
            if (port_info.is_n_port == (!value)){
                ...
                port_info.flags = (SOC_PPD_PORT_IHB_PINFO_FLP_TBL | 
                                   SOC_PPD_PORT_IHP_VTT_PP_PORT_CONFIG_TBL);
        ...
        case bcmPortControlMacInMac:
              ...
              if (value == 0 || value == 1) { /* disable/enable MiM */ 
                  ...
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                      ...
                      port_info.flags = (SOC_PPD_PORT_IHB_PINFO_FLP_TBL |
                                         SOC_PPD_PORT_EPNI_PP_PCT_TBL |
                                         SOC_PPD_PORT_EGQ_PP_PPCT_TBL);
                      soc_sand_rv = soc_ppd_port_info_set(unit, core, soc_ppd_port, &port_info);
                      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        case bcmPortControlTCPriority:
            if (value == 0 || value == 1) { /* disable/enable */
                if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "preserving_dscp_enabled", 0)) {
                    ...
                    soc_sand_rv = soc_ppd_port_info_get(unit, core, soc_ppd_port, &port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    port_info.flags = SOC_PPD_PORT_ALL_TBL;
                    soc_sand_rv = soc_ppd_port_info_set(unit, core, soc_ppd_port, &port_info);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        case bcmPortControlMplsEncapsulateExtendedLabel:
            {
                if (SOC_IS_JERICHO(unit))  {
                    if (soc_property_get(unit, spn_ROO_EXTENSION_LABEL_ENCAPSULATION, 0)) {
                        ...
                        rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, -1, out_lif_id, NULL, &lif_usage);
                        BCMDNX_IF_ERR_EXIT(rv);
        ...
        case bcmPortControlNonEtagDrop:
            {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    port_info.tunnel_termination_profile = (value) ? SOC_PPC_PORT_DEFINED_TT_PROFILE_PORT_EXTENDER_CB
                                                                  : SOC_PPC_PORT_DEFINED_TT_PROFILE_DEFAULT;
        ...
        case bcmPortControlMplsSpeculativeParse:
            ...
            port_info.flags = SOC_PPD_PORT_IHP_PP_PORT_INFO_TBL;
            soc_sand_rv = soc_ppd_port_info_set(unit, core, soc_ppd_port, &port_info);
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            break;
        default:
    ...
    if (SOC_DPP_PP_ENABLE(unit)) {
      ...
      case bcmPortControlExtenderType:
          ...
          if (SOC_IS_ARADPLUS(unit)) {
              ...
              if ((extender_info.ing_ecid_mode == SOC_PPC_EXTENDER_PORT_ING_ECID_KEEP) 
                  && (port_info.vlan_translation_profile == SOC_PPC_PORT_DEFINED_VT_PROFILE_EXTENDER_PE)) {
      ...
      case bcmPortControlNonEtagDrop:
          {
              ...
              if (SOC_IS_ARADPLUS(unit)) {
                  ...
                  *value = (port_info.tunnel_termination_profile == SOC_PPC_PORT_DEFINED_TT_PROFILE_PORT_EXTENDER_CB) ? TRUE : FALSE;
              } else {
                  BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("Port Extender isn't supported on the device")));
              }
              break;
                                   ...
                                   bcm_port_t port)
{
    ...
    port_info.initial_action_profile.trap_code = _BCM_PETRA_UD_DFLT_TRAP;
    port_info.flags = SOC_PPD_PORT_IHP_PINFO_LLR_TBL;
    ...
    int *egress_rate)
{
    ...
    *egress_rate = 0;
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) { 
        ...
        if (SOC_IS_JERICHO(unit)) {
          snoop_info.cmnd_type = SOC_TMC_CMD_TYPE_SNOOP;
          soc_sand_rv = (MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_action_cmd_mirror_get, (unit,snoop_action_info.snoop_cmnd,&snoop_info))); 
        } else {
    ...
    int size)
{
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      port_info.flags = SOC_PPD_PORT_EPNI_PP_PCT_TBL;
      soc_sand_rv = soc_ppd_port_info_set(unit,core,soc_ppd_port_i,&port_info);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
...
{    
   ...
   *flags = 0;
   ...
   if(lif_info.type & (SOC_PPC_LIF_ENTRY_TYPE_AC|SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP|SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP)){
       ...
       learn_enable = lif_info.value.extender.learn_record.learn_type != SOC_PPD_L2_LIF_AC_LEARN_DISABLE;
   }
   else{
    ...
    uint32 flags)
{
   ...
   if(lif_info->type & (SOC_PPC_LIF_ENTRY_TYPE_AC|SOC_PPD_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP|SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP)){
       ...
       if (lif_info->value.extender.service_type != SOC_PPD_L2_LIF_AC_SERVICE_TYPE_MP && learn_enable) {
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("%s: cannot enable learning for P2P service "), FUNCTION_NAME()));
       }
       lif_info->value.extender.learn_record.learn_type = (learn_enable)? SOC_PPD_L2_LIF_AC_LEARN_INFO : SOC_PPD_L2_LIF_AC_LEARN_DISABLE;
    ...
    )
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      */
      ...
      port_info.flags = (SOC_PPD_PORT_IHP_PINFO_LLR_TBL |
                         SOC_PPD_PORT_EPNI_PP_PCT_TBL |
                         SOC_PPD_PORT_IHP_PP_PORT_INFO_TBL);
      soc_sand_rv = soc_ppd_port_info_set(unit,core,soc_ppd_port_i,&port_info);
      BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
     /*
    ...
    bcm_port_match_info_t *match)
{
    ...
    rv = _bcm_dpp_lif_sw_resources_lif_usage_get(unit, gport_hw_resources.local_in_lif, gport_hw_resources.local_out_lif, &in_lif_usage, &out_lif_usage);
    BCMDNX_IF_ERR_EXIT(rv);
