    ...
    if (!SOC_WARM_BOOT(unit)) {
		...
		if(SOC_IS_JERICHO(unit)){
			...
			_DPP_POLICER_METER_CORES_ITER(core_index){
				...
				/* map all meters to max rate */
				rv = jer_pp_mtr_eth_policer_glbl_profile_set(soc_sand_dev_id, core_index, policer_indx, &dpp_profile_info);
   ...
   int *npolicers) 
{
	...
	if (SOC_IS_JERICHO(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("%s: NOT SUPPORTED ON JERICHO!!\nUse: bcm_policer_aggregate_group_create.\n"), FUNCTION_NAME()));
    }
    ...
    int *npolicers)
{
	...
	if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("%s: NOT SUPPORTED BELOW JERICHO!!\nUse: bcm_policer_group_create.\n"), FUNCTION_NAME()));
    }
...
bcm_petra_policer_set(int unit, bcm_policer_t policer_id, bcm_policer_config_t *pol_cfg) 
{
	...
	if (SOC_IS_JERICHO(unit)) {
		soc_sand_rv = jer_pp_mtr_eth_policer_glbl_profile_set(soc_sand_dev_id, core_id, policer_indx, &policer_info);
		BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
	}else{
                          ...
                          bcm_policer_config_t *pol_cfg) 
{
	...
	if (SOC_IS_JERICHO(unit)) {
		soc_sand_rv = jer_pp_mtr_eth_policer_glbl_profile_get(soc_sand_dev_id, core_id, policer_indx, &policer_info);
		BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
	}else{
   ...
   bcm_policer_t *policer_id) 
{
	...
	SOC_SAND_IF_ERR_EXIT(bcm_port_get(unit, port, &dummy_flags, &interface_info, &mapping_info));
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
		if (SOC_IS_JERICHO(unit)) {
				for (pkt_type = 0; pkt_type < SOC_PPD_NOF_MTR_ETH_TYPES; ++pkt_type) {
					...
					soc_sand_rv = jer_pp_mtr_eth_policer_glbl_profile_map_get(
					   unit,
					   core_id,
					   port_i,
					   pkt_type,
