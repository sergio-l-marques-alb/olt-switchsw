            ...
            *outlif_valid = 1;
            ...
            if(mirror_dest->flags & BCM_MIRROR_DEST_TUNNEL_WITH_SPAN_ID) {/*span_id*/
                ...
                else if (SOC_IS_JERICHO(unit))/* Jericho and above*/
                {
                    ...
                    SOC_PPC_EG_ENCAP_EEDB_DATA_ERSPAN_FORMAT_SET(soc_sand_dev_id, SOC_PPD_EG_ENCAP_ERSPAN_PRIO_VAL, SOC_PPD_EG_ENCAP_ERSPAN_TRUNC_VAL, 
                                      ...
                                      bcm_mirror_options_t options)
{
    ...
    BCM_DPP_UNIT_CHECK(unit);                      
    if (flags & ~(BCM_MIRROR_PORT_INGRESS | BCM_MIRROR_PORT_EGRESS | BCM_MIRROR_PORT_UNTAGGED_ONLY | BCM_MIRROR_PORT_TAGGED_DEFAULT
#ifdef BCM_ARAD_SUPPORT
        | (SOC_IS_ARAD(unit) ? BCM_MIRROR_PORT_EGRESS_ACL : 0)
#endif
        )) {
    ...
    if (flags & BCM_MIRROR_PORT_EGRESS_ACL) { /* enable of outbound mirroring (reserve reassembly context and recycle channel) for other applications */
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (options.recycle_cmd > 0) {
                 ...
                 */
                SOC_SAND_IF_ERR_RETURN(soc_ppd_eg_pmf_mirror_params_set(unit,action_profile_id,&config)) ;
            }
        }
    }
exit:
