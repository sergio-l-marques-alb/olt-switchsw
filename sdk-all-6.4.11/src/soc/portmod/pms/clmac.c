...
int clmac_speed_set(int unit, soc_port_t port, int flags, int speed)
{
    ...
    if (enable) {
        SOC_IF_ERROR_RETURN(clmac_enable_set(unit, port, 0, 0));
    }
...
int clmac_strict_preamble_set(int unit, soc_port_t port, int value)
{
    ...
    rv = soc_reg_field32_modify(unit, CLMAC_RX_CTRLr, port, STRICT_PREAMBLEf,
                                value);
...
int clmac_lag_failover_disable(int unit, int port)
{
    ...
    return (WRITE_CLMAC_CTRLr(unit, port, mac_ctrl));
}   
...
int clmac_reset_fc_timers_on_link_dn_set (int unit, soc_port_t port, int val)
{
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_RX_LSS_CTRLr(unit, port, &reg_val));

    soc_reg64_field32_set (unit, CLMAC_RX_LSS_CTRLr, &reg_val,
                           RESET_FLOW_CONTROL_TIMERS_ON_LINK_DOWNf, val);

    SOC_IF_ERROR_RETURN(WRITE_CLMAC_RX_LSS_CTRLr(unit, port, reg_val));
    return (SOC_E_NONE);
}
                          ...
                          const portmod_pfc_config_t* pfc_cfg)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_PFC_TYPEr(unit ,port, rval));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_PFC_OPCODEr(unit ,port, rval));
    ...
    return (WRITE_CLMAC_PFC_DAr(unit, port, rval));
}
...
int clmac_e2ecc_hdr_get (int unit, int port, uint32_t *words)
{
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_E2ECC_MODULE_HDR_0r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_E2ECC_MODULE_HDR_1r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_E2ECC_DATA_HDR_0r(unit, port, &rval64));
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_E2ECC_DATA_HDR_1r(unit, port, &rval64));
...
int clmac_e2ecc_hdr_set (int unit, int port, uint32_t *words)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_E2ECC_MODULE_HDR_0r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_E2ECC_MODULE_HDR_1r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_E2ECC_DATA_HDR_0r(unit, port, rval64));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_E2ECC_DATA_HDR_1r(unit, port, rval64));
...
int clmac_frame_spacing_stretch_set(int unit, int port, int spacing)
{
    ...
    SOC_IF_ERROR_RETURN(READ_CLMAC_TX_CTRLr(unit, port, &mac_ctrl));
    ...
    SOC_IF_ERROR_RETURN(WRITE_CLMAC_TX_CTRLr(unit, port, mac_ctrl));
