/*----------------------------------------------------------------------
 * $Id: tscmod.c,v 1.169 Broadcom SDK $
...
phy_tscmod_sw_rx_los_uc_link_handler(int unit, soc_port_t port, int *link)
{
   ...
   case RXLOS_RESET:
      ...
      if(*link) {
         ...
         if(DEV_CFG_PTR(pc)->sw_rx_los.enable & TSCMOD_RXLOS_LINK_UP_FAST_WAR) {
            DEV_CFG_PTR(pc)->sw_rx_los.start_time = sal_time_usecs() + (TSCMOD_RXLOS_TIMEOUTp1 << factor) ;
         } else {
            DEV_CFG_PTR(pc)->sw_rx_los.start_time = sal_time_usecs() + (TSCMOD_RXLOS_TIMEOUTp25 << factor) ;
         }
      } else {
         current_time = sal_time_usecs() ;
   ...
   case RXLOS_INITIAL_LINK_UP:
      if(*link) {
         ...
         current_time = sal_time_usecs() ;
         if(DEV_CFG_PTR(pc)->sw_rx_los.enable & TSCMOD_RXLOS_LINK_UP_FAST_WAR) {
             DEV_CFG_PTR(pc)->sw_rx_los.count_time = current_time + (TSCMOD_RXLOS_TIMEOUTp1 << factor) ;
         } 
         DEV_CFG_PTR(pc)->sw_rx_los.start_time = current_time + (TSCMOD_RXLOS_TIMEOUTp5 << factor) ;
      } else {
         if(tsc->verbosity & TSCMOD_DBG_FSM)
...
_phy_tscmod_control_prbs_tx_invert_data_set(tscmod_st *tsc, uint32 value)
{
   ...
   ctl  =  ((value & 0x1)<< PATGEN0_PRBSCTRL_PRBS_INV_TX_SHIFT) ;
                       ...
                       soc_phy_event_t event, uint32 value)
{
    ...
    case phyEventLpiBypass:
        rv = _phy_tscmod_control_set(unit, port, SOC_PHY_CONTROL_EEE, value);
        break ;
    default:
        rv = SOC_E_UNAVAIL;
        break;
...
STATIC char* tscmod_cl37bam_ability_msg0(int data) {
   ...
   if(data&(1<<TSCMOD_BAM37ABL_40G))        strncat(msg,"40G ", 4) ;
   if(data&(1<<TSCMOD_BAM37ABL_32P7G))      strncat(msg,"32.7G ", 6) ;
   if(data&(1<<TSCMOD_BAM37ABL_31P5G))      strncat(msg,"31.5G ", 6) ;
   if(data&(1<<TSCMOD_BAM37ABL_25P455G))    strncat(msg,"25G ", 4) ;
   if(data&(1<<TSCMOD_BAM37ABL_21G_X4))     strncat(msg,"21G ", 4) ;
   if(data&(1<<TSCMOD_BAM37ABL_20G_X2_CX4)) strncat(msg,"20G_X2_CX4 ", 11) ;
   if(data&(1<<TSCMOD_BAM37ABL_20G_X2))     strncat(msg,"20G_X2 ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_20G_X4))     strncat(msg,"20G_X4 ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_20G_X4_CX4)) strncat(msg,"20G_X4_CX4 ", 11) ;
   if(data&(1<<TSCMOD_BAM37ABL_16G_X4))     strncat(msg,"16G_X4 ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_15P75G_R2))  strncat(msg,"15.75G_R2 ", 10) ;
   if(data&(1<<TSCMOD_BAM37ABL_15G_X4))     strncat(msg,"15G_X4 ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_13G_X4))     strncat(msg,"13G_X4 ", 7) ;
...
STATIC char* tscmod_cl37bam_ability_msg1(int data) {
   ...
   if(data&(1<<TSCMOD_BAM37ABL_12P7_DXGXS))  strncat(msg,"12.7G_DX ", 9) ;
   if(data&(1<<TSCMOD_BAM37ABL_12P5_X4))     strncat(msg,"12.5G_X4 ", 9) ;
   if(data&(1<<TSCMOD_BAM37ABL_12G_X4))      strncat(msg,"12G_X4 ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_10P5G_DXGXS)) strncat(msg,"10.5G_DX ", 9) ;
   if(data&(1<<TSCMOD_BAM37ABL_10G_X2_CX4))  strncat(msg,"10G_X2_CX4 ", 11) ;
   if(data&(1<<TSCMOD_BAM37ABL_10G_DXGXS))   strncat(msg,"10G_DX ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_10G_CX4))     strncat(msg,"10G_CX4 ", 8);
   if(data&(1<<TSCMOD_BAM37ABL_10G_HIGIG))   strncat(msg,"10G_HG ", 7) ;
   if(data&(1<<TSCMOD_BAM37ABL_6G_X4))       strncat(msg,"6G_X4 ", 6)  ;
   if(data&(1<<TSCMOD_BAM37ABL_5G_X4))       strncat(msg,"5G_X4 ", 6)  ;
   if(data&(1<<TSCMOD_BAM37ABL_2P5G))        strncat(msg,"2.5G ", 5) ;
...
STATIC char* tscmod_line_intf_msg(uint32 line_intf) {
   ...
   if(line_intf&TSCMOD_IF_SR)   strncat(msg,"SR ",3) ;
   if(line_intf&TSCMOD_IF_SR4)  strncat(msg,"SR4 ",4) ;
   if(line_intf&TSCMOD_IF_KR)   strncat(msg,"KR ",3) ;
   if(line_intf&TSCMOD_IF_KR2)  strncat(msg,"KR2 ",4) ;
   if(line_intf&TSCMOD_IF_KR4)  strncat(msg,"KR4 ",4) ;
   if(line_intf&TSCMOD_IF_CR)   strncat(msg,"CR ",3) ;
   if(line_intf&TSCMOD_IF_CR)   strncat(msg,"CR2 ",3) ;
   if(line_intf&TSCMOD_IF_CR4)  strncat(msg,"CR4 ",4) ;
   if(line_intf&TSCMOD_IF_XFI)  strncat(msg,"XFI ",4) ;
   if(line_intf&TSCMOD_IF_SFI)  strncat(msg,"SFI ",4) ;
   if(line_intf&TSCMOD_IF_XLAUI)strncat(msg,"XLAUI ",6) ;
   if(line_intf&TSCMOD_IF_XGMII)strncat(msg,"XGMII ",6) ;
   return (char *)msg;
}

STATIC int tscmod_tr_if_mode(int soc_port) {
    int tscmod_if ;
    if(soc_port < SOC_PORT_IF_SPAUI) {  /* SOC_PORT_IF_XAUI is mapped to SOC_PORT_IF_XGMII */
        tscmod_if = soc_port ;
    } else {
        switch(soc_port) {
...
STATIC char* tscmod_iintf_msg(int iintf) {
   ...
   if(iintf&TSCMOD_IIF_XLAUI)  strncat(msg,"IIF_XLAUI ", 10) ;
   if(iintf&TSCMOD_IIF_SR4)    strncat(msg,"IIF_SR4 ", 8) ;
   if(iintf&TSCMOD_IIF_SFPDAC) strncat(msg,"IIF_SFPDAC ", 11) ;
   if(iintf&TSCMOD_IIF_NOXFI)  strncat(msg,"IIF_NOXFI ", 10) ;
   return (char *)msg;
}
