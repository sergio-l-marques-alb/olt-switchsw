 ...
 *  54140       4     4 Copper / Fiber        SGMII
 *  54290/80    8     8 Copper only           SGMII
 *  54180       8     8 Copper only           SGMII
                         ...
                         int copper_enable)
{
    ...
    phy_bcm542xx_direct_reg_write(unit, pc,
                                  PHY_BCM542XX_DSP_TAP10_REG_OFFSET,
                                  0x011B);
    /* Disable Super Isolate for 54292/54291  (they share the same chip ID) */
    if ( PHY_IS_BCM54292(pc) ) {
        /* only 54292/292 have Super Isolate enabled by default */
        phy_bcm542xx_direct_reg_modify(unit, pc,
...
phy_bcm542xx_init(int unit, soc_port_t port)
{
    ...
    if ( PHY_IS_BCM54240(pc) || PHY_IS_BCM54285(pc) ||
         ...
         PHY_IS_BCM54210(pc) || PHY_IS_BCM54220(pc) || PHY_IS_BCM54140(pc) ) {
        fiber_capable = 1;
    }
...
phy_bcm542xx_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    /* if disabling a combo port, power down both copper & fiber media */
    combo_down = ((! enable) && pc->automedium);

	if ( PHY_COPPER_MODE(unit, port) || combo_down ) {
        SOC_IF_ERROR_RETURN
            (PHY_MODIFY_BCM542XX_MII_CTRLr(unit, pc, power,
                                           PHY_BCM542XX_MII_CTRL_PWR_DOWN));
    }
    if ( PHY_FIBER_MODE(unit, port)  || combo_down ) {
        SOC_IF_ERROR_RETURN
...
phy_bcm542xx_eee_enable(int unit, soc_port_t port, int mode, int enable)
{
    ...
    /* AutogrEEEn is not supported in the embedded GPHY IP core */
    if ((mode == SOC_PHY_CONTROL_EEE_AUTO) && (model == PHY_BCM5428X_MODEL)) {
        return SOC_E_UNAVAIL;
    }
...
phy_bcm542xx_eee_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    /*
     * AutogrEEEn with optional variable/fixed latency modes 
     * are not supported in the embedded GPHY IP core. 
     */
    if (((type == SOC_PHY_CONTROL_EEE_AUTO_IDLE_THRESHOLD) ||
          (type == SOC_PHY_CONTROL_EEE_AUTO_FIXED_LATENCY)) && 
          (model == PHY_BCM5428X_MODEL)) {
        return SOC_E_UNAVAIL;
    }
