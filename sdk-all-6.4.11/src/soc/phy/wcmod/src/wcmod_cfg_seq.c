/*
 * $Id: wcmod_cfg_seq.c,v 1.119 Broadcom SDK $
 *
...
int wcmod_set_port_mode(wcmod_st* pc)           /* SET_PORT_MODE */
{
  ...
  MODIFY_WC40_CL72_USERB0_CL72_MISC4_CONTROLr(pc->unit, pc, 0xc00, 0xc00);
...
int wcmod_prbs_control(wcmod_st* pc)                /* PRBS_CONTROL */
{
  ...
  SOC_IF_ERROR_RETURN (MODIFY_WC40_XGXSBLK1_LANECTRL0r(pc->unit, pc, 0,
                     XGXSBLK1_LANECTRL0_CL36_PCS_EN_RX_MASK |
                     XGXSBLK1_LANECTRL0_CL36_PCS_EN_TX_MASK)); /* 4 bits, one bit per lane */
  SOC_IF_ERROR_RETURN (MODIFY_WC40_XGXSBLK1_LANECTRL2r(pc->unit, pc, 0,
  ...
  if (data16 & mask) {
    SOC_IF_ERROR_RETURN (WRITE_WC40_XGXSBLK1_LANECTRL1r(pc->unit, pc, 0xffff));
            ...
            /*need to  enable cya bit */
                ...
                SOC_IF_ERROR_RETURN
                    (MODIFY_WC40_DIGITAL5_MISC6r(pc->unit, pc,
                                                  ...
                                                  DIGITAL5_MISC6_RX_OS8_OS3_ALT_BITS_CYA_MASK));
                SOC_IF_ERROR_RETURN 
                    (WRITE_WC40_XGXSBLK1_LANECTRL1r(pc->unit, pc, 0x0000));
            }
        }                                                      
  } else {
    SOC_IF_ERROR_RETURN (WRITE_WC40_XGXSBLK1_LANECTRL1r(pc->unit, pc, 0x5555)); 
    /*for the warpcore 8.25 and 3.3 os mode, need to enable cya bit */
        if (pc->model_type == WCMOD_WC_C0) {
            uint16 data_tmp = 0;;
            /*first check speed less than 2.5G */
...
int wcmod_power_control(wcmod_st* pc)
{
  ...
  if ((pc->model_type) == WCMOD_QS_A0) {
      ...
      /* if is SGMII mode */
      if (pc->port_type == WCMOD_COMBO) {
          MODIFY_WC40_XGXSBLK1_LANECTRL3r(pc->unit, pc,
                                     (power_down << XGXSBLK1_LANECTRL3_PWRDWN_FORCE_SHIFT),
