...
_mac_xl_timestamp_byte_adjust_set(int unit, soc_port_t port)
{
    ...
    if (SOC_REG_PORT_VALID(unit, MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr, port) &&
        ...
        SOC_IF_ERROR_RETURN(READ_MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr(unit, port, &ctrl2));
        soc_reg_field_set(unit, MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr, &ctrl2,
                          TX_TIMER_BYTE_ADJUSTf, 8);
        SOC_IF_ERROR_RETURN(WRITE_MXQ_XLMAC_TIMESTAMP_BYTE_ADJUSTr(unit, port, ctrl2));
...
#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TD2_TT2(unit) && IS_HG_PORT(unit, port)) {
...
_mac_xl_timestamp_delay_set(int unit, soc_port_t port, int speed)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_10G,
                (BSL_META_U(unit,
                            "mac_xl_timestamp_delay_set: unit %d port %s\n"),
                 unit, SOC_PORT_NAME(unit, port)));
...
mac_xl_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    if (SOC_IS_GREYHOUND(unit) && (mode == SOC_ENCAP_HIGIG2_LITE)) {
        /* GH specific : 
