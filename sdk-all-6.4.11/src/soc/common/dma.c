...
soc_dma_monitor_thread(void *p)
{
    ...
    while (1) {
        ...
        if (sdc_mtg->rx_chain_intr != sdc_mtg->rx_chain_intr_prev) {
            ...
            cmic_drv[unit].chan_desc_done_intr_enable(unit, chan);
...
soc_dma_tx_purge(int unit, dma_chan_t c)
{
    ...
    if (soc_feature(unit, soc_feature_txdma_purge)) {
        ...
        if (rv >= 0) {
            ...
            rv = cmic_drv[unit].chan_tx_purge(unit, c, dv);
...
soc_dma_abort_channel(int unit, dma_chan_t channel)
{
    ...
    rv = cmic_drv[unit].chan_abort(unit, channel);
...
dv_t *
soc_dma_done_reload(int unit, uint32 chan)
{
...
soc_dma_process_done_desc(int unit, dv_t *dv_chain, dv_t *dv)
{
    ...
    while (dv != NULL) {
        ...
        for (i = dv->dv_dcnt; i < dv->dv_vcnt; i++) {
            ...
            LOG_VERBOSE(BSL_LS_SOC_DMA, /* VERBOSE */
                        (BSL_META_U(unit,
                                    "soc_dma_process_done_desc "
                                    "at %p flags=0x%x\n"),
                         (void *)dcb, (uint32)flags));

            /* Check for reload DCB */
            if (SOC_DCB_RELOAD_GET(unit, dcb)) {
                LOG_DEBUG(BSL_LS_SOC_DMA,
                          (BSL_META_U(unit,
                                      "soc_dma_process_done_desc "
                                      "(is rld) dv=%p dcb=%p\n"),
                           (void *)dv, (void *)dcb));
        ...
        LOG_DEBUG(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit,
                                "soc_dma_process_done_desc "
                                "dv_next = %p, dv_chain = %p\n"),
                     (void *)dv->dv_next, (void *)dv->dv_chain));
...
soc_dma_done_desc(int unit, uint32 chan)
{
    ...
    /* Clear interrupt */
    cmic_drv[unit].chan_desc_done(unit, chan);
...
soc_dma_done_chain(int unit, uint32 chan)
{
    ...
    cmic_drv[unit].chan_chain_done(unit, chan, sdc_mtg->mitigation_started);
...
void
soc_dma_chan_started_clear(int unit, dma_chan_t c)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    sdc_t         *sc = &soc->soc_channels[c];
    uint32        soc_dma_lock;
...
soc_dma_chan_config(int unit, dma_chan_t c, dvt_t type, uint32 flags)
{
    ...
    cmic_drv[unit].chan_config(unit, c, type, flags);
...
soc_dma_handle_knet_event(kcom_msg_t *kmsg, unsigned int len, void *cookie)
{
    if (kmsg->hdr.type == KCOM_MSG_TYPE_EVT &&
        ...
        if (dma_info->flags & KCOM_DMA_INFO_F_RX_DONE) {
            soc_dma_done_knet(unit, &soc->soc_channels[1]);
        }
...
soc_dma_attach(int unit, int reset)
{
    ...
    if (soc_feature(unit, soc_feature_continuous_dma)) {
        if (soc_property_get(unit, spn_PDMA_CONTINUOUS_MODE_ENABLE, 0)) {
            ...
            LOG_INFO(BSL_LS_SOC_DMA,
                     (BSL_META_U(unit,
                                 "Enabling continuous DMA mode\n")));
        } else {
            soc_cmicm_dma_drv_init(unit, &cmic_drv[unit]);
            LOG_INFO(BSL_LS_SOC_DMA,
                     (BSL_META_U(unit,
                                 "Enabling legacy DMA mode\n")));
        }
    }
#endif
#ifdef BCM_CMICM_SUPPORT
...
soc_dma_poll_channel(int unit, dma_chan_t c)
{
    ...
    if (((sc->sc_flags & SOC_DMA_F_POLL)) && (sc->sc_dv_active)) {
        ...
        done_detected = 0;
        cmic_drv[unit].chan_poll(unit, c,
