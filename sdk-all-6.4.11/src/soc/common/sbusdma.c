    ...
    for (array_index = array_index_start; array_index <= array_index_end; ++array_index) {
        if (!flags || (flags & SOC_MEM_WRITE_SET_ONLY)) {
            ...
            if (SOC_IS_DPP(unit) || SOC_IS_DFE(unit)) {
                ...
                if (mem == KAPS_TCMm) {
                    soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &rval,
                                  INCR_SHIFTf, 3);/*Each entry is 64bits while the indexation is 8bit*/
                    soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &rval,
                                  WORDSWAP_IN_64BIT_SBUSDATAf, 1);/*Necessary for using uint64*/
                } else{
                    soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &rval,
                                  INCR_SHIFTf, 0);
                }
                                ...
                                void *null_entry)
{
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        rv = _soc_mem_array_sbusdma_write(unit, 0, mem, array_index_min, array_index_max, blk, index_min, 
...
_soc_sbusdma_desc(void *unit_vp)
{
    ...
    while((interval = SOC_SBUSDMA_DM_TO(unit)) != 0) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit,
                                "_soc_sbusdma_desc: Process. cmc=%d. ch=%d\n"), cmc, ch));
...
/*
 * Function:
 *      _soc_sbusdma_desc_endian_config (private)
 * Purpose:
 *      Configure endianess of sbus dma descriptor
 * Parameters:
 *      unit - unit number
        desc - Pointer to sbus dma descriptor
 */

STATIC void
_soc_sbusdma_desc_endian_config(int unit, soc_sbusdma_desc_t *desc)
{
    int  big_pio, big_packet, big_other;
                        ...
                        sbusdma_desc_handle_t *desc_handle)
{
    ...
    if (ctrl->cfg_count == 1) { /* single mode */
        ...
        if (!(ctrl->flags & SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC) || !ctrl->hw_desc) {
            ...
            _soc_sbusdma_desc_endian_config(unit, swd->desc);
...
_soc_l2mod_sbus_fifo_field_set(int unit, soc_field_t field, int enable)
{
    ...
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIDENT(unit) || 
        SOC_IS_TITAN2X(unit) || SOC_IS_TITAN(unit)) {
