...
static    soc_pbmp_t skip_default_lls_pbmp; 
...
_soc_td2_fifo_ser_info_t _soc_td2_ip_ser_info[] = {
    ...
    { ILPM_SER_CONTROLr, L3_DEFIP_ALPM_PARITY_ENf, 0, L3_DEFIP_ALPM_RAWm, INVALIDr, "" },    
            ...
            for (pgw = 0; pgw < _TD2_PGWS_PER_QUAD; pgw++) {
                ...
                for (blk = 0; blk < _TD2_XLPS_PER_PGW; blk++) {
                    ...
                    for (index = 0; index < _TD2_PORTS_PER_XLP; index++) {
                        ...
                        if (port_bandwidth == 127000) {
                            max_quad_core_bandwidth[i] += 7000;
                        } else if (port_bandwidth == 106000) {
                            max_quad_core_bandwidth[i] += 6000;
                        } else if (port_bandwidth == 42000) {
...
soc_trident2_port_config_init(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    int index, tsc_id;
    int port_count[_TD2_PIPES_PER_DEV];
    soc_100g_lane_config_t lane_config;
    ...
    for (port = 0; port < num_port; port++) {
        ...
        if (SOC_IS_TD2P_TT2P(unit)) {
            ...
            if (oversub) {
                SOC_PBMP_PORT_ADD(oversub_pbm, port);
            }
        }
#endif
        ...
        if (SOC_IS_TRIDENT2PLUS(unit)) {
            ...
            if (port_bandwidth >= 100000) {
                if (SOC_SUCCESS(soc_td2p_port_lane_config_get(unit, phy_port,
                                                              &mode_lane,
                                                              &found))) {
                    if (found) {
                ...
                if (SOC_SUCCESS(soc_td2p_port_autoneg_core_get(unit, phy_port,
                                                               &an_core,
                                                               &found))) {
                    if (found) {
                        fallback_lane = an_core;
                    }
                }
            }
        }
...
int
soc_trident2_lls_bmap_alloc_port(int unit, int port)
{
    soc_control_t *soc;
    soc_info_t *si;
...
/*
 * LLS creation will be skipped on TD2 only when all the valid ports bits were 
 * configured to 1 for spn_PBMP_SKIP_DEFAULT_LLS. 
 */
int
soc_td2_is_skip_default_lls_creation(int unit) {
    soc_pbmp_t valid_port_pbmp;
    soc_info_t *si;
    int skip = 0;
    
    si = &SOC_INFO(unit);   
    valid_port_pbmp = si->port.bitmap;

    if (SOC_PBMP_NOT_NULL(skip_default_lls_pbmp) &&
        SOC_PBMP_EQ(skip_default_lls_pbmp, valid_port_pbmp)) {
        skip = 1;
    } 
            ...
            if ((_soc_td2p_ser_block_info[i].type == _SOC_TD2P_SER_TYPE_MEM) &&
                ...
                for (j = 0; ser_en_info[j].mem != INVALIDm; j++) {
                    ...
                    for (k = 0; k < num_instance_to_test; k++) {
                        ...
                        rv = _soc_trident2_perform_ser_test(unit, &test_data,
