...
sbZfFe2000PmProfileTimerMemory_Unpack(sbZfFe2000PmProfileTimerMemory_t *pToStruct,
  ...
  (pToStruct)->uResv0 =  (uint32)  ((pFromData)[0] >> 7) & 0x01;

  /* Unpack Member: uType */
  (pToStruct)->uType =  (uint32)  ((pFromData)[0] >> 6) & 0x01;

  /* Unpack Member: uProfMode */
  (pToStruct)->uProfMode =  (uint32)  ((pFromData)[0] >> 4) & 0x03;

  /* Unpack Member: uResv1 */
  (pToStruct)->uResv1 =  (uint32)  (pFromData)[3] ;
  (pToStruct)->uResv1 |=  (uint32)  (pFromData)[2] << 8;
  (pToStruct)->uResv1 |=  (uint32)  (pFromData)[1] << 16;
  (pToStruct)->uResv1 |=  (uint32)  ((pFromData)[0] & 0x0f) << 24;

  /* Unpack Member: uResv2 */
  (pToStruct)->uResv2 =  (uint32)  (pFromData)[4] ;

  /* Unpack Member: bInterrupt */
  (pToStruct)->bInterrupt =  (uint32)  ((pFromData)[5] >> 7) & 0x01;

  /* Unpack Member: bReset */
  (pToStruct)->bReset =  (uint32)  ((pFromData)[5] >> 6) & 0x01;

  /* Unpack Member: bStrict */
  (pToStruct)->bStrict =  (uint32)  ((pFromData)[5] >> 5) & 0x01;

  /* Unpack Member: bStarted */
  (pToStruct)->bStarted =  (uint32)  ((pFromData)[5] >> 4) & 0x01;

  /* Unpack Member: uDeadline */
  (pToStruct)->uDeadline =  (uint32)  (pFromData)[7] ;
  (pToStruct)->uDeadline |=  (uint32)  (pFromData)[6] << 8;
  (pToStruct)->uDeadline |=  (uint32)  ((pFromData)[5] & 0x0f) << 16;
...
uint32 sbZfFe2000PmProfileTimerMemory_Pack32(sbZfFe2000PmProfileTimerMemory_t *pFrom, 
                                              ...
                                              uint32 nMaxToDataIndex)
{
  uint8 uBuffer[SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_WORDS*4];
  uint32 uIndex =0;
  uint32 *pBuffer = NULL;
  uint32 uNumberOfWords = SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_WORDS;
                                               ...
                                               uint32 nMaxToDataIndex)
{
  ...
  uint32 uBuffer[SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_WORDS*4];
  uint32 uNumberOfWords = SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_WORDS;
  uint32 uIndex =0;
  ...
  sbZfFe2000PmProfileTimerMemory_Unpack(pToData , (uint8 *) &uBuffer[0], SB_ZF_FE_2000_PM_PROFTIMERMEMORY_SIZE_IN_BYTES);
  return uNumberOfWords;
}
