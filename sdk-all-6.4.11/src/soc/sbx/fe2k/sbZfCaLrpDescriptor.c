/*
 * $Id: sbZfCaLrpDescriptor.c,v 1.2 Broadcom SDK $
...
#include "sbTypes.h"
#include "sbZfCaLrpDescriptor.hx"
...
sbZfCaLrpDescriptor_Pack(sbZfCaLrpDescriptor_t *pFrom,
  ...
  int i;
  int size = SB_ZF_ZFCALRPDESCRIPTOR_SIZE_IN_BYTES;
  ...
  (pToData)[191] |= ((pFrom)->m_uContinueByte) & 0xFF;

  /* Pack Member: m_uHeaderType0 */
  (pToData)[190] |= ((pFrom)->m_uHeaderType0 & 0x0f) <<4;

  /* Pack Member: m_uHeaderType1 */
  (pToData)[190] |= ((pFrom)->m_uHeaderType1 & 0x0f);

  /* Pack Member: m_uHeaderType2 */
  (pToData)[189] |= ((pFrom)->m_uHeaderType2 & 0x0f) <<4;

  /* Pack Member: m_uHeaderType3 */
  (pToData)[189] |= ((pFrom)->m_uHeaderType3 & 0x0f);

  /* Pack Member: m_uHeaderType4 */
  (pToData)[188] |= ((pFrom)->m_uHeaderType4 & 0x0f) <<4;

  /* Pack Member: m_uHeaderType5 */
  (pToData)[188] |= ((pFrom)->m_uHeaderType5 & 0x0f);

  /* Pack Member: m_uHeaderType6 */
  (pToData)[187] |= ((pFrom)->m_uHeaderType6 & 0x0f) <<4;

  /* Pack Member: m_uHeaderType7 */
  (pToData)[187] |= ((pFrom)->m_uHeaderType7 & 0x0f);

  /* Pack Member: m_uHeaderType8 */
  (pToData)[186] |= ((pFrom)->m_uHeaderType8 & 0x0f) <<4;

  /* Pack Member: m_uHeaderType9 */
  (pToData)[186] |= ((pFrom)->m_uHeaderType9 & 0x0f);

  /* Pack Member: m_uLocation0 */
  (pToData)[185] |= ((pFrom)->m_uLocation0) & 0xFF;

  /* Pack Member: m_uLocation1 */
  (pToData)[184] |= ((pFrom)->m_uLocation1) & 0xFF;

  /* Pack Member: m_uLocation2 */
  (pToData)[183] |= ((pFrom)->m_uLocation2) & 0xFF;

  /* Pack Member: m_uLocation3 */
  (pToData)[182] |= ((pFrom)->m_uLocation3) & 0xFF;

  /* Pack Member: m_uLocation4 */
  (pToData)[181] |= ((pFrom)->m_uLocation4) & 0xFF;

  /* Pack Member: m_uLocation5 */
  (pToData)[180] |= ((pFrom)->m_uLocation5) & 0xFF;

  /* Pack Member: m_uLocation6 */
  (pToData)[179] |= ((pFrom)->m_uLocation6) & 0xFF;

  /* Pack Member: m_uLocation7 */
  (pToData)[178] |= ((pFrom)->m_uLocation7) & 0xFF;

  /* Pack Member: m_uLocation8 */
  (pToData)[177] |= ((pFrom)->m_uLocation8) & 0xFF;

  /* Pack Member: m_uLocation9 */
  (pToData)[176] |= ((pFrom)->m_uLocation9) & 0xFF;

  /* Pack Member: m_uStreamSelector */
  (pToData)[175] |= ((pFrom)->m_uStreamSelector & 0x07) <<5;

  /* Pack Member: m_uBuffer */
  (pToData)[174] |= ((pFrom)->m_uBuffer) & 0xFF;
  (pToData)[175] |= ((pFrom)->m_uBuffer >> 8) & 0x1f;

  /* Pack Member: m_uExceptIndex */
  (pToData)[173] |= ((pFrom)->m_uExceptIndex) & 0xFF;

  /* Pack Member: m_uSourceQueue */
  (pToData)[172] |= ((pFrom)->m_uSourceQueue) & 0xFF;

  /* Pack Member: m_uTruncationCommand */
  (pToData)[171] |= ((pFrom)->m_uTruncationCommand & 0x07) <<5;

  /* Pack Member: m_uHeaderCopy */
  (pToData)[171] |= ((pFrom)->m_uHeaderCopy & 0x01) <<4;

  /* Pack Member: m_uMirrorIndex */
  (pToData)[171] |= ((pFrom)->m_uMirrorIndex & 0x0f);

  /* Pack Member: m_uCopyCount */
  (pToData)[169] |= ((pFrom)->m_uCopyCount & 0x0f) <<4;
  (pToData)[170] |= ((pFrom)->m_uCopyCount >> 4) &0xFF;

  /* Pack Member: m_uCrcMode */
  (pToData)[169] |= ((pFrom)->m_uCrcMode & 0x03) <<2;

  /* Pack Member: m_uDropBit */
  (pToData)[169] |= ((pFrom)->m_uDropBit & 0x01) <<1;

  /* Pack Member: m_uEnqueue */
  (pToData)[169] |= ((pFrom)->m_uEnqueue & 0x01);

  /* Pack Member: m_uDestinationQueue */
  (pToData)[168] |= ((pFrom)->m_uDestinationQueue) & 0xFF;

  /* Pack Member: m_uXferLength */
  (pToData)[166] |= ((pFrom)->m_uXferLength & 0x3f) <<2;
  (pToData)[167] |= ((pFrom)->m_uXferLength >> 6) &0xFF;

  /* Pack Member: m_uPacketMarking */
  (pToData)[166] |= ((pFrom)->m_uPacketMarking & 0x03);

  /* Pack Member: m_uResv1 */
  (pToData)[165] |= ((pFrom)->m_uResv1 & 0x03) <<6;

  /* Pack Member: m_uFrameLength */
  (pToData)[164] |= ((pFrom)->m_uFrameLength) & 0xFF;
  (pToData)[165] |= ((pFrom)->m_uFrameLength >> 8) & 0x3f;

  /* Pack Member: m_uResv2 */
  (pToData)[160] |= ((pFrom)->m_uResv2) & 0xFF;
  (pToData)[161] |= ((pFrom)->m_uResv2 >> 8) &0xFF;
  (pToData)[162] |= ((pFrom)->m_uResv2 >> 16) &0xFF;
  (pToData)[163] |= ((pFrom)->m_uResv2 >> 24) &0xFF;

  /* Pack Member: m_uResv3 */
  (pToData)[156] |= ((pFrom)->m_uResv3) & 0xFF;
  (pToData)[157] |= ((pFrom)->m_uResv3 >> 8) &0xFF;
  (pToData)[158] |= ((pFrom)->m_uResv3 >> 16) &0xFF;
  (pToData)[159] |= ((pFrom)->m_uResv3 >> 24) &0xFF;
  ...
  return SB_ZF_ZFCALRPDESCRIPTOR_SIZE_IN_BYTES;
}
...
sbZfCaLrpDescriptor_Unpack(sbZfCaLrpDescriptor_t *pToStruct,
  ...
  (pToStruct)->m_uContinueByte =  (uint32)  (pFromData)[191] ;

  /* Unpack Member: m_uHeaderType0 */
  (pToStruct)->m_uHeaderType0 =  (uint32)  ((pFromData)[190] >> 4) & 0x0f;

  /* Unpack Member: m_uHeaderType1 */
  (pToStruct)->m_uHeaderType1 =  (uint32)  ((pFromData)[190] ) & 0x0f;

  /* Unpack Member: m_uHeaderType2 */
  (pToStruct)->m_uHeaderType2 =  (uint32)  ((pFromData)[189] >> 4) & 0x0f;

  /* Unpack Member: m_uHeaderType3 */
  (pToStruct)->m_uHeaderType3 =  (uint32)  ((pFromData)[189] ) & 0x0f;

  /* Unpack Member: m_uHeaderType4 */
  (pToStruct)->m_uHeaderType4 =  (uint32)  ((pFromData)[188] >> 4) & 0x0f;

  /* Unpack Member: m_uHeaderType5 */
  (pToStruct)->m_uHeaderType5 =  (uint32)  ((pFromData)[188] ) & 0x0f;

  /* Unpack Member: m_uHeaderType6 */
  (pToStruct)->m_uHeaderType6 =  (uint32)  ((pFromData)[187] >> 4) & 0x0f;

  /* Unpack Member: m_uHeaderType7 */
  (pToStruct)->m_uHeaderType7 =  (uint32)  ((pFromData)[187] ) & 0x0f;

  /* Unpack Member: m_uHeaderType8 */
  (pToStruct)->m_uHeaderType8 =  (uint32)  ((pFromData)[186] >> 4) & 0x0f;

  /* Unpack Member: m_uHeaderType9 */
  (pToStruct)->m_uHeaderType9 =  (uint32)  ((pFromData)[186] ) & 0x0f;

  /* Unpack Member: m_uLocation0 */
  (pToStruct)->m_uLocation0 =  (uint32)  (pFromData)[185] ;

  /* Unpack Member: m_uLocation1 */
  (pToStruct)->m_uLocation1 =  (uint32)  (pFromData)[184] ;

  /* Unpack Member: m_uLocation2 */
  (pToStruct)->m_uLocation2 =  (uint32)  (pFromData)[183] ;

  /* Unpack Member: m_uLocation3 */
  (pToStruct)->m_uLocation3 =  (uint32)  (pFromData)[182] ;

  /* Unpack Member: m_uLocation4 */
  (pToStruct)->m_uLocation4 =  (uint32)  (pFromData)[181] ;

  /* Unpack Member: m_uLocation5 */
  (pToStruct)->m_uLocation5 =  (uint32)  (pFromData)[180] ;

  /* Unpack Member: m_uLocation6 */
  (pToStruct)->m_uLocation6 =  (uint32)  (pFromData)[179] ;

  /* Unpack Member: m_uLocation7 */
  (pToStruct)->m_uLocation7 =  (uint32)  (pFromData)[178] ;

  /* Unpack Member: m_uLocation8 */
  (pToStruct)->m_uLocation8 =  (uint32)  (pFromData)[177] ;

  /* Unpack Member: m_uLocation9 */
  (pToStruct)->m_uLocation9 =  (uint32)  (pFromData)[176] ;

  /* Unpack Member: m_uStreamSelector */
  (pToStruct)->m_uStreamSelector =  (uint32)  ((pFromData)[175] >> 5) & 0x07;

  /* Unpack Member: m_uBuffer */
  (pToStruct)->m_uBuffer =  (uint32)  (pFromData)[174] ;
  (pToStruct)->m_uBuffer |=  (uint32)  ((pFromData)[175] & 0x1f) << 8;

  /* Unpack Member: m_uExceptIndex */
  (pToStruct)->m_uExceptIndex =  (uint32)  (pFromData)[173] ;

  /* Unpack Member: m_uSourceQueue */
  (pToStruct)->m_uSourceQueue =  (uint32)  (pFromData)[172] ;

  /* Unpack Member: m_uTruncationCommand */
  (pToStruct)->m_uTruncationCommand =  (uint32)  ((pFromData)[171] >> 5) & 0x07;

  /* Unpack Member: m_uHeaderCopy */
  (pToStruct)->m_uHeaderCopy =  (uint32)  ((pFromData)[171] >> 4) & 0x01;

  /* Unpack Member: m_uMirrorIndex */
  (pToStruct)->m_uMirrorIndex =  (uint32)  ((pFromData)[171] ) & 0x0f;

  /* Unpack Member: m_uCopyCount */
  (pToStruct)->m_uCopyCount =  (uint32)  ((pFromData)[169] >> 4) & 0x0f;
  (pToStruct)->m_uCopyCount |=  (uint32)  (pFromData)[170] << 4;

  /* Unpack Member: m_uCrcMode */
  (pToStruct)->m_uCrcMode =  (uint32)  ((pFromData)[169] >> 2) & 0x03;

  /* Unpack Member: m_uDropBit */
  (pToStruct)->m_uDropBit =  (uint32)  ((pFromData)[169] >> 1) & 0x01;

  /* Unpack Member: m_uEnqueue */
  (pToStruct)->m_uEnqueue =  (uint32)  ((pFromData)[169] ) & 0x01;

  /* Unpack Member: m_uDestinationQueue */
  (pToStruct)->m_uDestinationQueue =  (uint32)  (pFromData)[168] ;

  /* Unpack Member: m_uXferLength */
  (pToStruct)->m_uXferLength =  (uint32)  ((pFromData)[166] >> 2) & 0x3f;
  (pToStruct)->m_uXferLength |=  (uint32)  (pFromData)[167] << 6;

  /* Unpack Member: m_uPacketMarking */
  (pToStruct)->m_uPacketMarking =  (uint32)  ((pFromData)[166] ) & 0x03;

  /* Unpack Member: m_uResv1 */
  (pToStruct)->m_uResv1 =  (uint32)  ((pFromData)[165] >> 6) & 0x03;

  /* Unpack Member: m_uFrameLength */
  (pToStruct)->m_uFrameLength =  (uint32)  (pFromData)[164] ;
  (pToStruct)->m_uFrameLength |=  (uint32)  ((pFromData)[165] & 0x3f) << 8;

  /* Unpack Member: m_uResv2 */
  (pToStruct)->m_uResv2 =  (uint32)  (pFromData)[160] ;
  (pToStruct)->m_uResv2 |=  (uint32)  (pFromData)[161] << 8;
  (pToStruct)->m_uResv2 |=  (uint32)  (pFromData)[162] << 16;
  (pToStruct)->m_uResv2 |=  (uint32)  (pFromData)[163] << 24;

  /* Unpack Member: m_uResv3 */
  (pToStruct)->m_uResv3 =  (uint32)  (pFromData)[156] ;
  (pToStruct)->m_uResv3 |=  (uint32)  (pFromData)[157] << 8;
  (pToStruct)->m_uResv3 |=  (uint32)  (pFromData)[158] << 16;
  (pToStruct)->m_uResv3 |=  (uint32)  (pFromData)[159] << 24;

  /* Unpack Member: m_uPacketDataByte0 */
  (pToStruct)->m_uPacketDataByte0 =  (uint8)  (pFromData)[155] ;

  /* Unpack Member: m_uPacketDataByte1 */
  (pToStruct)->m_uPacketDataByte1 =  (uint8)  (pFromData)[154] ;

  /* Unpack Member: m_uPacketDataByte2 */
  (pToStruct)->m_uPacketDataByte2 =  (uint8)  (pFromData)[153] ;

  /* Unpack Member: m_uPacketDataByte3 */
  (pToStruct)->m_uPacketDataByte3 =  (uint8)  (pFromData)[152] ;

  /* Unpack Member: m_uPacketDataByte4 */
  (pToStruct)->m_uPacketDataByte4 =  (uint8)  (pFromData)[151] ;

  /* Unpack Member: m_uPacketDataByte5 */
  (pToStruct)->m_uPacketDataByte5 =  (uint8)  (pFromData)[150] ;

  /* Unpack Member: m_uPacketDataByte6 */
  (pToStruct)->m_uPacketDataByte6 =  (uint8)  (pFromData)[149] ;

  /* Unpack Member: m_uPacketDataByte7 */
  (pToStruct)->m_uPacketDataByte7 =  (uint8)  (pFromData)[148] ;

  /* Unpack Member: m_uPacketDataByte8 */
  (pToStruct)->m_uPacketDataByte8 =  (uint8)  (pFromData)[147] ;

  /* Unpack Member: m_uPacketDataByte9 */
  (pToStruct)->m_uPacketDataByte9 =  (uint8)  (pFromData)[146] ;

  /* Unpack Member: m_uPacketDataByte10 */
  (pToStruct)->m_uPacketDataByte10 =  (uint8)  (pFromData)[145] ;

  /* Unpack Member: m_uPacketDataByte11 */
  (pToStruct)->m_uPacketDataByte11 =  (uint8)  (pFromData)[144] ;

  /* Unpack Member: m_uPacketDataByte12 */
  (pToStruct)->m_uPacketDataByte12 =  (uint8)  (pFromData)[143] ;

  /* Unpack Member: m_uPacketDataByte13 */
  (pToStruct)->m_uPacketDataByte13 =  (uint8)  (pFromData)[142] ;

  /* Unpack Member: m_uPacketDataByte14 */
  (pToStruct)->m_uPacketDataByte14 =  (uint8)  (pFromData)[141] ;

  /* Unpack Member: m_uPacketDataByte15 */
  (pToStruct)->m_uPacketDataByte15 =  (uint8)  (pFromData)[140] ;

  /* Unpack Member: m_uPacketDataByte16 */
  (pToStruct)->m_uPacketDataByte16 =  (uint8)  (pFromData)[139] ;

  /* Unpack Member: m_uPacketDataByte17 */
  (pToStruct)->m_uPacketDataByte17 =  (uint8)  (pFromData)[138] ;

  /* Unpack Member: m_uPacketDataByte18 */
  (pToStruct)->m_uPacketDataByte18 =  (uint8)  (pFromData)[137] ;

  /* Unpack Member: m_uPacketDataByte19 */
  (pToStruct)->m_uPacketDataByte19 =  (uint8)  (pFromData)[136] ;

  /* Unpack Member: m_uPacketDataByte20 */
  (pToStruct)->m_uPacketDataByte20 =  (uint8)  (pFromData)[135] ;

  /* Unpack Member: m_uPacketDataByte21 */
  (pToStruct)->m_uPacketDataByte21 =  (uint8)  (pFromData)[134] ;

  /* Unpack Member: m_uPacketDataByte22 */
  (pToStruct)->m_uPacketDataByte22 =  (uint8)  (pFromData)[133] ;

  /* Unpack Member: m_uPacketDataByte23 */
  (pToStruct)->m_uPacketDataByte23 =  (uint8)  (pFromData)[132] ;

  /* Unpack Member: m_uPacketDataByte24 */
  (pToStruct)->m_uPacketDataByte24 =  (uint8)  (pFromData)[131] ;

  /* Unpack Member: m_uPacketDataByte25 */
  (pToStruct)->m_uPacketDataByte25 =  (uint8)  (pFromData)[130] ;

  /* Unpack Member: m_uPacketDataByte26 */
  (pToStruct)->m_uPacketDataByte26 =  (uint8)  (pFromData)[129] ;

  /* Unpack Member: m_uPacketDataByte27 */
  (pToStruct)->m_uPacketDataByte27 =  (uint8)  (pFromData)[128] ;

  /* Unpack Member: m_uPacketDataByte28 */
  (pToStruct)->m_uPacketDataByte28 =  (uint8)  (pFromData)[127] ;

  /* Unpack Member: m_uPacketDataByte29 */
  (pToStruct)->m_uPacketDataByte29 =  (uint8)  (pFromData)[126] ;

  /* Unpack Member: m_uPacketDataByte30 */
  (pToStruct)->m_uPacketDataByte30 =  (uint8)  (pFromData)[125] ;

  /* Unpack Member: m_uPacketDataByte31 */
  (pToStruct)->m_uPacketDataByte31 =  (uint8)  (pFromData)[124] ;

  /* Unpack Member: m_uPacketDataByte32 */
  (pToStruct)->m_uPacketDataByte32 =  (uint8)  (pFromData)[123] ;

  /* Unpack Member: m_uPacketDataByte33 */
  (pToStruct)->m_uPacketDataByte33 =  (uint8)  (pFromData)[122] ;

  /* Unpack Member: m_uPacketDataByte34 */
  (pToStruct)->m_uPacketDataByte34 =  (uint8)  (pFromData)[121] ;

  /* Unpack Member: m_uPacketDataByte35 */
  (pToStruct)->m_uPacketDataByte35 =  (uint8)  (pFromData)[120] ;

  /* Unpack Member: m_uPacketDataByte36 */
  (pToStruct)->m_uPacketDataByte36 =  (uint8)  (pFromData)[119] ;

  /* Unpack Member: m_uPacketDataByte37 */
  (pToStruct)->m_uPacketDataByte37 =  (uint8)  (pFromData)[118] ;

  /* Unpack Member: m_uPacketDataByte38 */
  (pToStruct)->m_uPacketDataByte38 =  (uint8)  (pFromData)[117] ;

  /* Unpack Member: m_uPacketDataByte39 */
  (pToStruct)->m_uPacketDataByte39 =  (uint8)  (pFromData)[116] ;

  /* Unpack Member: m_uPacketDataByte40 */
  (pToStruct)->m_uPacketDataByte40 =  (uint8)  (pFromData)[115] ;

  /* Unpack Member: m_uPacketDataByte41 */
  (pToStruct)->m_uPacketDataByte41 =  (uint8)  (pFromData)[114] ;

  /* Unpack Member: m_uPacketDataByte42 */
  (pToStruct)->m_uPacketDataByte42 =  (uint8)  (pFromData)[113] ;

  /* Unpack Member: m_uPacketDataByte43 */
  (pToStruct)->m_uPacketDataByte43 =  (uint8)  (pFromData)[112] ;

  /* Unpack Member: m_uPacketDataByte44 */
  (pToStruct)->m_uPacketDataByte44 =  (uint8)  (pFromData)[111] ;

  /* Unpack Member: m_uPacketDataByte45 */
  (pToStruct)->m_uPacketDataByte45 =  (uint8)  (pFromData)[110] ;

  /* Unpack Member: m_uPacketDataByte46 */
  (pToStruct)->m_uPacketDataByte46 =  (uint8)  (pFromData)[109] ;

  /* Unpack Member: m_uPacketDataByte47 */
  (pToStruct)->m_uPacketDataByte47 =  (uint8)  (pFromData)[108] ;

  /* Unpack Member: m_uPacketDataByte48 */
  (pToStruct)->m_uPacketDataByte48 =  (uint8)  (pFromData)[107] ;

  /* Unpack Member: m_uPacketDataByte49 */
  (pToStruct)->m_uPacketDataByte49 =  (uint8)  (pFromData)[106] ;

  /* Unpack Member: m_uPacketDataByte50 */
  (pToStruct)->m_uPacketDataByte50 =  (uint8)  (pFromData)[105] ;

  /* Unpack Member: m_uPacketDataByte51 */
  (pToStruct)->m_uPacketDataByte51 =  (uint8)  (pFromData)[104] ;

  /* Unpack Member: m_uPacketDataByte52 */
  (pToStruct)->m_uPacketDataByte52 =  (uint8)  (pFromData)[103] ;

  /* Unpack Member: m_uPacketDataByte53 */
  (pToStruct)->m_uPacketDataByte53 =  (uint8)  (pFromData)[102] ;

  /* Unpack Member: m_uPacketDataByte54 */
  (pToStruct)->m_uPacketDataByte54 =  (uint8)  (pFromData)[101] ;

  /* Unpack Member: m_uPacketDataByte55 */
  (pToStruct)->m_uPacketDataByte55 =  (uint8)  (pFromData)[100] ;

  /* Unpack Member: m_uPacketDataByte56 */
  (pToStruct)->m_uPacketDataByte56 =  (uint8)  (pFromData)[99] ;

  /* Unpack Member: m_uPacketDataByte57 */
  (pToStruct)->m_uPacketDataByte57 =  (uint8)  (pFromData)[98] ;

  /* Unpack Member: m_uPacketDataByte58 */
  (pToStruct)->m_uPacketDataByte58 =  (uint8)  (pFromData)[97] ;

  /* Unpack Member: m_uPacketDataByte59 */
  (pToStruct)->m_uPacketDataByte59 =  (uint8)  (pFromData)[96] ;

  /* Unpack Member: m_uPacketDataByte60 */
  (pToStruct)->m_uPacketDataByte60 =  (uint8)  (pFromData)[95] ;

  /* Unpack Member: m_uPacketDataByte61 */
  (pToStruct)->m_uPacketDataByte61 =  (uint8)  (pFromData)[94] ;

  /* Unpack Member: m_uPacketDataByte62 */
  (pToStruct)->m_uPacketDataByte62 =  (uint8)  (pFromData)[93] ;

  /* Unpack Member: m_uPacketDataByte63 */
  (pToStruct)->m_uPacketDataByte63 =  (uint8)  (pFromData)[92] ;

  /* Unpack Member: m_uPacketDataByte64 */
  (pToStruct)->m_uPacketDataByte64 =  (uint8)  (pFromData)[91] ;

  /* Unpack Member: m_uPacketDataByte65 */
  (pToStruct)->m_uPacketDataByte65 =  (uint8)  (pFromData)[90] ;

  /* Unpack Member: m_uPacketDataByte66 */
  (pToStruct)->m_uPacketDataByte66 =  (uint8)  (pFromData)[89] ;

  /* Unpack Member: m_uPacketDataByte67 */
  (pToStruct)->m_uPacketDataByte67 =  (uint8)  (pFromData)[88] ;

  /* Unpack Member: m_uPacketDataByte68 */
  (pToStruct)->m_uPacketDataByte68 =  (uint8)  (pFromData)[87] ;

  /* Unpack Member: m_uPacketDataByte69 */
  (pToStruct)->m_uPacketDataByte69 =  (uint8)  (pFromData)[86] ;

  /* Unpack Member: m_uPacketDataByte70 */
  (pToStruct)->m_uPacketDataByte70 =  (uint8)  (pFromData)[85] ;

  /* Unpack Member: m_uPacketDataByte71 */
  (pToStruct)->m_uPacketDataByte71 =  (uint8)  (pFromData)[84] ;

  /* Unpack Member: m_uPacketDataByte72 */
  (pToStruct)->m_uPacketDataByte72 =  (uint8)  (pFromData)[83] ;

  /* Unpack Member: m_uPacketDataByte73 */
  (pToStruct)->m_uPacketDataByte73 =  (uint8)  (pFromData)[82] ;

  /* Unpack Member: m_uPacketDataByte74 */
  (pToStruct)->m_uPacketDataByte74 =  (uint8)  (pFromData)[81] ;

  /* Unpack Member: m_uPacketDataByte75 */
  (pToStruct)->m_uPacketDataByte75 =  (uint8)  (pFromData)[80] ;

  /* Unpack Member: m_uPacketDataByte76 */
  (pToStruct)->m_uPacketDataByte76 =  (uint8)  (pFromData)[79] ;

  /* Unpack Member: m_uPacketDataByte77 */
  (pToStruct)->m_uPacketDataByte77 =  (uint8)  (pFromData)[78] ;

  /* Unpack Member: m_uPacketDataByte78 */
  (pToStruct)->m_uPacketDataByte78 =  (uint8)  (pFromData)[77] ;

  /* Unpack Member: m_uPacketDataByte79 */
  (pToStruct)->m_uPacketDataByte79 =  (uint8)  (pFromData)[76] ;

  /* Unpack Member: m_uPacketDataByte80 */
  (pToStruct)->m_uPacketDataByte80 =  (uint8)  (pFromData)[75] ;

  /* Unpack Member: m_uPacketDataByte81 */
  (pToStruct)->m_uPacketDataByte81 =  (uint8)  (pFromData)[74] ;

  /* Unpack Member: m_uPacketDataByte82 */
  (pToStruct)->m_uPacketDataByte82 =  (uint8)  (pFromData)[73] ;

  /* Unpack Member: m_uPacketDataByte83 */
  (pToStruct)->m_uPacketDataByte83 =  (uint8)  (pFromData)[72] ;

  /* Unpack Member: m_uPacketDataByte84 */
  (pToStruct)->m_uPacketDataByte84 =  (uint8)  (pFromData)[71] ;

  /* Unpack Member: m_uPacketDataByte85 */
  (pToStruct)->m_uPacketDataByte85 =  (uint8)  (pFromData)[70] ;

  /* Unpack Member: m_uPacketDataByte86 */
  (pToStruct)->m_uPacketDataByte86 =  (uint8)  (pFromData)[69] ;

  /* Unpack Member: m_uPacketDataByte87 */
  (pToStruct)->m_uPacketDataByte87 =  (uint8)  (pFromData)[68] ;

  /* Unpack Member: m_uPacketDataByte88 */
  (pToStruct)->m_uPacketDataByte88 =  (uint8)  (pFromData)[67] ;

  /* Unpack Member: m_uPacketDataByte89 */
  (pToStruct)->m_uPacketDataByte89 =  (uint8)  (pFromData)[66] ;

  /* Unpack Member: m_uPacketDataByte90 */
  (pToStruct)->m_uPacketDataByte90 =  (uint8)  (pFromData)[65] ;

  /* Unpack Member: m_uPacketDataByte91 */
  (pToStruct)->m_uPacketDataByte91 =  (uint8)  (pFromData)[64] ;

  /* Unpack Member: m_uPacketDataByte92 */
  (pToStruct)->m_uPacketDataByte92 =  (uint8)  (pFromData)[63] ;

  /* Unpack Member: m_uPacketDataByte93 */
  (pToStruct)->m_uPacketDataByte93 =  (uint8)  (pFromData)[62] ;

  /* Unpack Member: m_uPacketDataByte94 */
  (pToStruct)->m_uPacketDataByte94 =  (uint8)  (pFromData)[61] ;

  /* Unpack Member: m_uPacketDataByte95 */
  (pToStruct)->m_uPacketDataByte95 =  (uint8)  (pFromData)[60] ;

  /* Unpack Member: m_uPacketDataByte96 */
  (pToStruct)->m_uPacketDataByte96 =  (uint8)  (pFromData)[59] ;

  /* Unpack Member: m_uPacketDataByte97 */
  (pToStruct)->m_uPacketDataByte97 =  (uint8)  (pFromData)[58] ;

  /* Unpack Member: m_uPacketDataByte98 */
  (pToStruct)->m_uPacketDataByte98 =  (uint8)  (pFromData)[57] ;

  /* Unpack Member: m_uPacketDataByte99 */
  (pToStruct)->m_uPacketDataByte99 =  (uint8)  (pFromData)[56] ;

  /* Unpack Member: m_uPacketDataByte100 */
  (pToStruct)->m_uPacketDataByte100 =  (uint8)  (pFromData)[55] ;

  /* Unpack Member: m_uPacketDataByte101 */
  (pToStruct)->m_uPacketDataByte101 =  (uint8)  (pFromData)[54] ;

  /* Unpack Member: m_uPacketDataByte102 */
  (pToStruct)->m_uPacketDataByte102 =  (uint8)  (pFromData)[53] ;

  /* Unpack Member: m_uPacketDataByte103 */
  (pToStruct)->m_uPacketDataByte103 =  (uint8)  (pFromData)[52] ;

  /* Unpack Member: m_uPacketDataByte104 */
  (pToStruct)->m_uPacketDataByte104 =  (uint8)  (pFromData)[51] ;

  /* Unpack Member: m_uPacketDataByte105 */
  (pToStruct)->m_uPacketDataByte105 =  (uint8)  (pFromData)[50] ;

  /* Unpack Member: m_uPacketDataByte106 */
  (pToStruct)->m_uPacketDataByte106 =  (uint8)  (pFromData)[49] ;

  /* Unpack Member: m_uPacketDataByte107 */
  (pToStruct)->m_uPacketDataByte107 =  (uint8)  (pFromData)[48] ;

  /* Unpack Member: m_uPacketDataByte108 */
  (pToStruct)->m_uPacketDataByte108 =  (uint8)  (pFromData)[47] ;

  /* Unpack Member: m_uPacketDataByte109 */
  (pToStruct)->m_uPacketDataByte109 =  (uint8)  (pFromData)[46] ;

  /* Unpack Member: m_uPacketDataByte110 */
  (pToStruct)->m_uPacketDataByte110 =  (uint8)  (pFromData)[45] ;

  /* Unpack Member: m_uPacketDataByte111 */
  (pToStruct)->m_uPacketDataByte111 =  (uint8)  (pFromData)[44] ;

  /* Unpack Member: m_uPacketDataByte112 */
  (pToStruct)->m_uPacketDataByte112 =  (uint8)  (pFromData)[43] ;

  /* Unpack Member: m_uPacketDataByte113 */
  (pToStruct)->m_uPacketDataByte113 =  (uint8)  (pFromData)[42] ;

  /* Unpack Member: m_uPacketDataByte114 */
  (pToStruct)->m_uPacketDataByte114 =  (uint8)  (pFromData)[41] ;

  /* Unpack Member: m_uPacketDataByte115 */
  (pToStruct)->m_uPacketDataByte115 =  (uint8)  (pFromData)[40] ;

  /* Unpack Member: m_uPacketDataByte116 */
  (pToStruct)->m_uPacketDataByte116 =  (uint8)  (pFromData)[39] ;

  /* Unpack Member: m_uPacketDataByte117 */
  (pToStruct)->m_uPacketDataByte117 =  (uint8)  (pFromData)[38] ;

  /* Unpack Member: m_uPacketDataByte118 */
  (pToStruct)->m_uPacketDataByte118 =  (uint8)  (pFromData)[37] ;

  /* Unpack Member: m_uPacketDataByte119 */
  (pToStruct)->m_uPacketDataByte119 =  (uint8)  (pFromData)[36] ;

  /* Unpack Member: m_uPacketDataByte120 */
  (pToStruct)->m_uPacketDataByte120 =  (uint8)  (pFromData)[35] ;

  /* Unpack Member: m_uPacketDataByte121 */
  (pToStruct)->m_uPacketDataByte121 =  (uint8)  (pFromData)[34] ;

  /* Unpack Member: m_uPacketDataByte122 */
  (pToStruct)->m_uPacketDataByte122 =  (uint8)  (pFromData)[33] ;

  /* Unpack Member: m_uPacketDataByte123 */
  (pToStruct)->m_uPacketDataByte123 =  (uint8)  (pFromData)[32] ;

  /* Unpack Member: m_uPacketDataByte124 */
  (pToStruct)->m_uPacketDataByte124 =  (uint8)  (pFromData)[31] ;

  /* Unpack Member: m_uPacketDataByte125 */
  (pToStruct)->m_uPacketDataByte125 =  (uint8)  (pFromData)[30] ;

  /* Unpack Member: m_uPacketDataByte126 */
  (pToStruct)->m_uPacketDataByte126 =  (uint8)  (pFromData)[29] ;

  /* Unpack Member: m_uPacketDataByte127 */
  (pToStruct)->m_uPacketDataByte127 =  (uint8)  (pFromData)[28] ;

  /* Unpack Member: m_uPacketDataByte128 */
  (pToStruct)->m_uPacketDataByte128 =  (uint8)  (pFromData)[27] ;

  /* Unpack Member: m_uPacketDataByte129 */
  (pToStruct)->m_uPacketDataByte129 =  (uint8)  (pFromData)[26] ;

  /* Unpack Member: m_uPacketDataByte130 */
  (pToStruct)->m_uPacketDataByte130 =  (uint8)  (pFromData)[25] ;

  /* Unpack Member: m_uPacketDataByte131 */
  (pToStruct)->m_uPacketDataByte131 =  (uint8)  (pFromData)[24] ;

  /* Unpack Member: m_uPacketDataByte132 */
  (pToStruct)->m_uPacketDataByte132 =  (uint8)  (pFromData)[23] ;

  /* Unpack Member: m_uPacketDataByte133 */
  (pToStruct)->m_uPacketDataByte133 =  (uint8)  (pFromData)[22] ;

  /* Unpack Member: m_uPacketDataByte134 */
  (pToStruct)->m_uPacketDataByte134 =  (uint8)  (pFromData)[21] ;

  /* Unpack Member: m_uPacketDataByte135 */
  (pToStruct)->m_uPacketDataByte135 =  (uint8)  (pFromData)[20] ;

  /* Unpack Member: m_uPacketDataByte136 */
  (pToStruct)->m_uPacketDataByte136 =  (uint8)  (pFromData)[19] ;

  /* Unpack Member: m_uPacketDataByte137 */
  (pToStruct)->m_uPacketDataByte137 =  (uint8)  (pFromData)[18] ;

  /* Unpack Member: m_uPacketDataByte138 */
  (pToStruct)->m_uPacketDataByte138 =  (uint8)  (pFromData)[17] ;

  /* Unpack Member: m_uPacketDataByte139 */
  (pToStruct)->m_uPacketDataByte139 =  (uint8)  (pFromData)[16] ;

  /* Unpack Member: m_uPacketDataByte140 */
  (pToStruct)->m_uPacketDataByte140 =  (uint8)  (pFromData)[15] ;

  /* Unpack Member: m_uPacketDataByte141 */
  (pToStruct)->m_uPacketDataByte141 =  (uint8)  (pFromData)[14] ;

  /* Unpack Member: m_uPacketDataByte142 */
  (pToStruct)->m_uPacketDataByte142 =  (uint8)  (pFromData)[13] ;

  /* Unpack Member: m_uPacketDataByte143 */
  (pToStruct)->m_uPacketDataByte143 =  (uint8)  (pFromData)[12] ;

  /* Unpack Member: m_uPacketDataByte144 */
  (pToStruct)->m_uPacketDataByte144 =  (uint8)  (pFromData)[11] ;

  /* Unpack Member: m_uPacketDataByte145 */
  (pToStruct)->m_uPacketDataByte145 =  (uint8)  (pFromData)[10] ;

  /* Unpack Member: m_uPacketDataByte146 */
  (pToStruct)->m_uPacketDataByte146 =  (uint8)  (pFromData)[9] ;

  /* Unpack Member: m_uPacketDataByte147 */
  (pToStruct)->m_uPacketDataByte147 =  (uint8)  (pFromData)[8] ;

  /* Unpack Member: m_uPacketDataByte148 */
  (pToStruct)->m_uPacketDataByte148 =  (uint8)  (pFromData)[7] ;

  /* Unpack Member: m_uPacketDataByte149 */
  (pToStruct)->m_uPacketDataByte149 =  (uint8)  (pFromData)[6] ;

  /* Unpack Member: m_uPacketDataByte150 */
  (pToStruct)->m_uPacketDataByte150 =  (uint8)  (pFromData)[5] ;

  /* Unpack Member: m_uPacketDataByte151 */
  (pToStruct)->m_uPacketDataByte151 =  (uint8)  (pFromData)[4] ;

  /* Unpack Member: m_uPacketDataByte152 */
  (pToStruct)->m_uPacketDataByte152 =  (uint8)  (pFromData)[3] ;

  /* Unpack Member: m_uPacketDataByte153 */
  (pToStruct)->m_uPacketDataByte153 =  (uint8)  (pFromData)[2] ;

  /* Unpack Member: m_uPacketDataByte154 */
  (pToStruct)->m_uPacketDataByte154 =  (uint8)  (pFromData)[1] ;

  /* Unpack Member: m_uPacketDataByte155 */
  (pToStruct)->m_uPacketDataByte155 =  (uint8)  (pFromData)[0] ;
...
sbZfCaLrpDescriptor_InitInstance(sbZfCaLrpDescriptor_t *pFrame) {
  ...
  pFrame->m_uHeaderType0 =  (unsigned int)  0;
  pFrame->m_uHeaderType1 =  (unsigned int)  0;
  pFrame->m_uHeaderType2 =  (unsigned int)  0;
  pFrame->m_uHeaderType3 =  (unsigned int)  0;
  pFrame->m_uHeaderType4 =  (unsigned int)  0;
  pFrame->m_uHeaderType5 =  (unsigned int)  0;
  pFrame->m_uHeaderType6 =  (unsigned int)  0;
  pFrame->m_uHeaderType7 =  (unsigned int)  0;
  pFrame->m_uHeaderType8 =  (unsigned int)  0;
  pFrame->m_uHeaderType9 =  (unsigned int)  0;
  pFrame->m_uLocation0 =  (unsigned int)  0;
  pFrame->m_uLocation1 =  (unsigned int)  0;
  pFrame->m_uLocation2 =  (unsigned int)  0;
  pFrame->m_uLocation3 =  (unsigned int)  0;
  pFrame->m_uLocation4 =  (unsigned int)  0;
  pFrame->m_uLocation5 =  (unsigned int)  0;
  pFrame->m_uLocation6 =  (unsigned int)  0;
  pFrame->m_uLocation7 =  (unsigned int)  0;
  pFrame->m_uLocation8 =  (unsigned int)  0;
  pFrame->m_uLocation9 =  (unsigned int)  0;
  pFrame->m_uStreamSelector =  (unsigned int)  0;
  pFrame->m_uBuffer =  (unsigned int)  0;
  pFrame->m_uExceptIndex =  (unsigned int)  0;
  pFrame->m_uSourceQueue =  (unsigned int)  0;
  pFrame->m_uTruncationCommand =  (unsigned int)  0;
  pFrame->m_uHeaderCopy =  (unsigned int)  0;
  pFrame->m_uMirrorIndex =  (unsigned int)  0;
  pFrame->m_uCopyCount =  (unsigned int)  0;
  pFrame->m_uCrcMode =  (unsigned int)  0;
  pFrame->m_uDropBit =  (unsigned int)  0;
  pFrame->m_uEnqueue =  (unsigned int)  0;
  pFrame->m_uDestinationQueue =  (unsigned int)  0;
  pFrame->m_uXferLength =  (unsigned int)  0;
  pFrame->m_uPacketMarking =  (unsigned int)  0;
