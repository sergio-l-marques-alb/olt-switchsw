/*
 * $Id: sbZfCaPeDiagPV2Etc.c,v 1.2 Broadcom SDK $
...
sbZfCaPeDiagPV2Etc_Unpack(sbZfCaPeDiagPV2Etc_t *pToStruct,
  ...
  (pToStruct)->m_uQid =  (uint32)  ((pFromData)[2] >> 2) & 0x3f;
  (pToStruct)->m_uQid |=  (uint32)  (pFromData)[3] << 6;

  /* Unpack Member: m_uTest */
  (pToStruct)->m_uTest =  (uint32)  ((pFromData)[2] >> 1) & 0x01;

  /* Unpack Member: m_uDummy */
  (pToStruct)->m_uDummy =  (uint32)  ((pFromData)[1] >> 4) & 0x0f;
  (pToStruct)->m_uDummy |=  (uint32)  ((pFromData)[2] & 0x01) << 4;

  /* Unpack Member: m_uVid */
  (pToStruct)->m_uVid =  (uint32)  (pFromData)[0] ;
  (pToStruct)->m_uVid |=  (uint32)  ((pFromData)[1] & 0x0f) << 8;
