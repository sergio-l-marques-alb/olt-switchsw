/*
 * $Id: g2p3_lpm_comp.c,v 1.18 Broadcom SDK $
 *
...
sbStatus_t
soc_sbx_g2p3_lpm_pack0(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
    int s;
...
sbStatus_t
soc_sbx_g2p3_lpm_pack1(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack2(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack3(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack4(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack5(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack6(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
...
sbStatus_t
soc_sbx_g2p3_lpm_pack7(void *dest, uint16 *destExtra, void *payload,
                       g2p3_lpmClassifierId_t clsA,
                       g2p3_lpmClassifierId_t clsB,
                       uint32 dropMaskLo, uint32 dropMaskHi, uint32 unit)
{
                          ...
                          soc_sbx_g2p3_complex_pack_f_t pack, void *dentry)
{
    ...
    int npbanks, pbank0odd, pwidth;
    int slabsize;
    int sram_factor;
    int master, found = FALSE;
    soc_sbx_g2p3_state_t *master_fe = NULL;
    ...
    sbStatus_t (*ipack)(void *dest, uint16 *destExtra, void *payload,
                        g2p3_lpmClassifierId_t clsA,
                        g2p3_lpmClassifierId_t clsB,
                        uint32 dropMaskLo, uint32 dropMaskHi,
                        uint32 unit);
    soc_sbx_g2p3_table_manager_t *tm = NULL;

    *state = NULL;

    /* continue init only with master unit. For slave units, we
     * use return the handle of master lpm. We scan master unit's
     * tableinfo to find a matching table. NOTE: this is assuming
     * each table has a unique name.
     */
    if (!SOC_IS_SBX_MASTER(unit)) {
	/* this is not a master unit, we return the lpmc handle of master
	 * unit, since master unit is defined as the first unit with matching
	 * dev_id and finished "init soc", SDK should already created all of these 
	 * lpm tables on master unit, so the lpmc handle of master unit should
	 * always be valid here.
	 */
	master = SOC_SBX_MASTER(unit);
	assert(SOC_CONTROL(master));
	assert(SOC_SBX_CONTROL(master));
	
	master_fe = (soc_sbx_g2p3_state_t *) SOC_SBX_CONTROL(master)->drv;
	assert(master_fe);

	/* search on master tableinfo to find table with matching name */
	tm = master_fe->tableinfo;
	for (i = 0; i < SOC_SBX_G2P3_TABLE_MAX_ID; i++) {
	    if (!sal_strcasecmp(name, tm->tables[i].name)) {
		*state = tm->tables[i].cstate;
		found = TRUE;
		break;
	    }
	}
	...
	/* master unit and all previous slave units should already inserted 0/0 route.
	 * we use resetCommit feature to add 0/0 route to the newly added unit.
	 * This will reissue all the DMAs on other units, but assumed to be OK
	 * since it's going to overwrite tables with same value.
	 */
	lpmc = *state;	
	if (lpmc) {
	    s = g2p3_lpmInitCommit(lpmc->lpm);
    ...
    *state = lpmc;
    ...
    if (s == SB_OK) {
      uint64 uuZero = COMPILER_64_INIT(0,0);
        s = g2p3_lpmAdd(lpmc->lpm, uuZero, 0,
                           ...
                           uint8 *ikey)
{
    ...
    if (s == SB_OK) {
        s = g2p3_lpmWalk(lpmc->lpm, uuZero, 0, (g2p3_lpmAddress_t *) ikey, &len, 
