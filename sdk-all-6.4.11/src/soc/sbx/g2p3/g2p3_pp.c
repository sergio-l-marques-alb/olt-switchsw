/*
 * $Id: g2p3_pp.c,v 1.90 Broadcom SDK $
 *
...
soc_sbx_g2p3_pp_broad_shield_check_set_get( soc_sbx_g2p3_state_t *pFe,
                                int eType, uint8 *pbEnable, uint8 bSetGet);

static sbStatus_t
soc_sbx_g2p3_pp_rule_stream_set(int unit, uint32 uStream, int uCamId, int uEntry);

static sbStatus_t
soc_sbx_g2p3_pp_rule_enable_set(int unit, uint32  uValid, int uCamId, int uEntry);

static sbStatus_t
soc_sbx_g2p3_pp_rule_enable_get(int unit, uint32 *uValid, int uCamId, int uEntry);
...
sbStatus_t
soc_sbx_g2p3_pp_oam_rx_match_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_oam_rx_match_get(int unit, uint32* uValid)
{
    int uCamId = SB_G2P3_FE_PP_RULE_S0_ETH_OAM_RX_CAM_ID;
    int uEntry = SB_G2P3_FE_PP_RULE_S0_ETH_OAM_RX_ENTRY;
...
sbStatus_t
soc_sbx_g2p3_pp_oam_tx_match_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_oam_tx_match_get(int unit, uint32* uValid)
{
    /* feature needs be implemented - currently controlled by soc property */
    return SOC_E_NONE;
}
...
sbStatus_t
soc_sbx_g2p3_pp_oam_up_match_set(int unit, uint32 uValid)
{
    int uCamId;
    int uEntry;
    sbStatus_t rv;
    ...
    uEntry = SB_G2P3_FE_PP_RULE_S1_EGR_ETYPE1_OAM_ENTRY;
    ...
    uEntry = SB_G2P3_FE_PP_RULE_S1_EGR_ETYPE1_VLAN_OAM_ENTRY;
...
sbStatus_t
soc_sbx_g2p3_pp_ipv6mc_str_sel_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_v4mc_str_sel_match_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_v4mc_str_sel_match_get(int unit, uint32* uValid)
{
    int uCamId = SB_G2P3_FE_PP_RULE_S0_ETH_IPMULTICAST_CAM_ID;
    int uEntry = SB_G2P3_FE_PP_RULE_S0_ETH_IPMULTICAST_ENTRY;
    return (soc_sbx_g2p3_pp_rule_enable_get(unit, uValid, uCamId, uEntry));
...
sbStatus_t
soc_sbx_g2p3_pp_v4uc_str_sel_match_set(int unit, uint32 uValid)
{
    sbStatus_t rv;
...
sbStatus_t
soc_sbx_g2p3_pp_v4uc_str_sel_match_get(int unit, uint32* uValid)
{
    int uCamId = SB_G2P3_FE_PP_RULE_S7_L4_NO_HASH_ROUTE_CAM_ID;
    int uEntry = SB_G2P3_FE_PP_RULE_S7_L4_NO_HASH_ROUTE_ENTRY;
    /*  uEntry = SB_G2P3_FE_PP_RULE_S7_L4_HASH_ROUTE_ENTRY; */
...
sbStatus_t
soc_sbx_g2p3_pp_ipv6_str_sel_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_tagged_mpls_set(int unit, uint32 uValid)
{
...
sbStatus_t
soc_sbx_g2p3_pp_tagged_mpls_get(int unit, uint32* uValid)
{
    int uCamId = SB_G2P3_FE_PP_RULE_S2_VLAN_MPLS1_CAM_ID;
    int uEntry = SB_G2P3_FE_PP_RULE_S2_VLAN_MPLS1_ENTRY;
                     ...
                     sbIngressConfigParams_p_t iParams)
{
  ...
  COMPILER_64_ZERO(pPpRuleMgr->IpTunneldb);
  pPpRuleMgr->Layer4Filterdb            = 0x0;
  COMPILER_64_ZERO(pPpRuleMgr->LocalStationMatchdb);
  ...
  if( ( SB_G2P3_FE_PP_RULE_MAX_CAM0 >= SB_G2P3_FE_PP_NUM_OF_RULES_PER_CAM ) ||
      ...
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META_U(unit,
                           "%s FATAL ERROR: Number Of CAM Rules Exceed the Limit \n"), FUNCTION_NAME()));
  } else {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META_U(unit,
                           "Total # CAM0 Rules:%d \n"), SB_G2P3_FE_PP_RULE_MAX_CAM0));
...
soc_sbx_g2p3_pp_hw_init(int unit, sbFeInitAsyncCallback_f_t cb, void *id)
{
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       "%s: Calling soc_sbx_g2p3_pp_hash_templates_init\n"), FUNCTION_NAME()));
  if( SB_OK != (status = soc_sbx_g2p3_pp_hash_templates_init(pFe)) ) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META_U(unit,
                         "error: soc_sbx_g2p3_pp_hash_templates_init\n")));
    cb(id, status);
    return status;
  }
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       "%s: Calling soc_sbx_g2p3_pp_rule_encode\n"), FUNCTION_NAME()));
  if( SB_OK != (status = soc_sbx_g2p3_pp_rule_encode(unit)) ) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META_U(unit,
                         "error: soc_sbx_g2p3_pp_rule_encode\n")));
    cb(id, status);
    return status;
  }
  ...
  LOG_INFO(BSL_LS_SOC_COMMON,
           (BSL_META_U(unit,
                       "%s: Calling soc_sbx_g2p3_pp_cam_default_init\n"), FUNCTION_NAME()));
  if( SB_OK != (status = soc_sbx_g2p3_pp_cam_default_init(unit)) ) {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META_U(unit,
                         "error: soc_sbx_g2p3_pp_cam_default_init\n")));
    cb(id, status);
    return status;
  }
  ...
  if (SOC_WARM_BOOT(unit)) {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META_U(unit,
                           "%s: Calling soc_sbx_g2p3_pp_cam_db_recover\n"), FUNCTION_NAME()));
      status = soc_sbx_g2p3_pp_cam_db_recover(unit);
      if (SB_OK != status) {
          LOG_INFO(BSL_LS_SOC_COMMON,
                   (BSL_META_U(unit,
                               "error: soc_sbx_g2p3_pp_cam_db_recover\n")));
          cb(id, status);
          return status;          
      }
      ...
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META_U(unit,
                           "%s: Calling soc_sbx_g2p3_pp_hash_templates_recover\n"),
                FUNCTION_NAME()));
      status = soc_sbx_g2p3_pp_hash_templates_recover(pFe);
      if (SB_OK != status) {
          LOG_INFO(BSL_LS_SOC_COMMON,
                   (BSL_META_U(unit,
                               "error: soc_sbx_g2p3_pp_hash_templates_recover\n")));
          cb(id, status);
          return status;          
      }
  }
...
soc_sbx_g2p3_pp_hash_templates_init(soc_sbx_g2p3_state_t *pFe)
{
  ...
  uint32 uRtHdrOffset =  SOC_SBX_G2P3_ERH_LEN(pFe);
  soc_sbx_g2p3_pp_rule_mgr_t *pPpParams  = pFe->pPpMgr;


  /* SB_G2P3_FE_PP_RTHDR_ETH_HASH_TEMPLATE */
  COMPILER_64_ZERO(uuMask);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+0);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+1);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+2);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+3);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+4);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+5);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+6);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+7);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+8);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+9);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+10);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask,uRtHdrOffset+11);

  uTemplateIndex = SB_G2P3_FE_PP_BYTE_HASH_TINDEX(SB_G2P3_FE_PP_RTHDR_ETH_HASH_TEMPLATE);
  pPpParams->ByteHashdb[uTemplateIndex].udb[0]  = COMPILER_64_HI(uuMask);
  pPpParams->ByteHashdb[uTemplateIndex].udb[1]  = COMPILER_64_LO(uuMask);
  pPpParams->ByteHashdb[uTemplateIndex].bValid  = TRUE;


  /* Hash: SB_G2P3_FE_PP_ETH_HASH_TEMPLATE */
  COMPILER_64_ZERO(uuMask);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 1)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 2)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 3)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 4)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 5)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 6)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 7)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 8)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 9)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 10) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 11) ;
  ...
  COMPILER_64_ZERO(uuMask);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_PROTOCOL_BYTE_OFFSET)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_DA_BYTE_OFFSET)    ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_DA_BYTE_OFFSET+1)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_DA_BYTE_OFFSET+2)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_DA_BYTE_OFFSET+3)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_SA_BYTE_OFFSET)    ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_SA_BYTE_OFFSET+1)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_SA_BYTE_OFFSET+2)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV4_SA_BYTE_OFFSET+3) ;
  ...
  COMPILER_64_ZERO(uuMask);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET)    ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+1)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+2)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+3)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+4)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+5)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+6)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+7)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+8)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+9)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+10) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+11) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+12) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+13) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+14) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_DA_BYTE_OFFSET+15) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET)    ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+1)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+2)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+3)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+4)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+5)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+6)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+7)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+8)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+9)  ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+10) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+11) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+12) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+13) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+14) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, SB_G2P3_FE_IPV6_SA_BYTE_OFFSET+15) ;
  ...
  COMPILER_64_ZERO(uuMask);
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0)   ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+1) ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 2)   ;
  SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 2+1);
  ...
  COMPILER_64_SET(uuMask, 0, 0xFFF);
  
  /* uuMask = SB_G2P3_FE_PP_BIT_HASH_MASK_GEN(uuMask, SB_G2P3_FE_VID_BIT_OFFSET); */
  uTemplateIndex = 0;
...
sbStatus_t
soc_sbx_g2p3_pp_hash_templates_set (soc_sbx_g2p3_state_t *pFe, uint32 fieldMask)
{
  /* Byte Hash Templates
   * It's a 40B mask in network byte order that we have to
   * configure and it is applied at the checker offset of the
  ...
  if (fieldMask & SB_G2P3_PSC_VID)
      COMPILER_64_SET(uuMask, 0, 0x00000FFF);
  ...
  pPpParams->BitHashdb[uTemplateIndex].udb[0]  = COMPILER_64_LO(uuMask);
  pPpParams->BitHashdb[uTemplateIndex].udb[1] = 0;
  pPpParams->BitHashdb[uTemplateIndex].udb[2] = 0;
  pPpParams->BitHashdb[uTemplateIndex].udb[3] = 0;
  pPpParams->BitHashdb[uTemplateIndex].udb[4] = 0;
...
soc_sbx_g2p3_pp_hash_templates_recover(soc_sbx_g2p3_state_t *pFe)
{
    ...
    COMPILER_64_SET(uuData, pPpParams->ByteHashdb[uHashId].udb[0], 
                    pPpParams->ByteHashdb[uHashId].udb[1]);
    
    /* SB_G2P3_PSC_MAC_DA psc */    
    COMPILER_64_ZERO(uuMask);
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0)   ;
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+1) ;
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+2) ;
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+3) ;
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+4) ;
    SB_G2P3_FE_PP_BYTE_HASH_MASK_GEN(uuMask, 0+5);
    
    uuTmp = uuData;
    COMPILER_64_AND(uuTmp, uuMask);
    if (!COMPILER_64_IS_ZERO(uuTmp)) {
        if (COMPILER_64_NE(uuTmp, uuMask)) {
            /* the masks changed, update the recovery sequence */
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META("PPE Hash incompatible\n")));
    ...
    uuTmp = uuData;
    COMPILER_64_AND(uuTmp, uuMask);
    if (((uFieldMask & SB_G2P3_PSC_IP_DA) && (COMPILER_64_NE(uuTmp,uuMask))) ||
        (!(uFieldMask & SB_G2P3_PSC_IP_DA) && (!COMPILER_64_IS_ZERO(uuTmp)))) {
        /* the masks changed, update the recovery sequence */
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("PPE Hash incompatible\n")));
        return SB_UCODE_INCOMPATIBLE_ERR_CODE;
    ...
    uuTmp = uuData;
    COMPILER_64_AND(uuTmp, uuMask);
    if (((uFieldMask & SB_G2P3_PSC_IP_SA) && (COMPILER_64_NE(uuTmp, uuMask))) ||
         (!(uFieldMask & SB_G2P3_PSC_IP_SA) && (!COMPILER_64_IS_ZERO(uuTmp)))) {
        /* the masks changed, update the recovery sequence */
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META("PPE Hash incompatible\n")));
        return SB_UCODE_INCOMPATIBLE_ERR_CODE;
...
sbStatus_t
soc_sbx_g2p3_pp_hash_templates_get (soc_sbx_g2p3_state_t *pFe, uint32 *fieldMask)
{
  soc_sbx_g2p3_pp_rule_mgr_t *pPpParams  = pFe->pPpMgr;
...
soc_sbx_g2p3_l2cpmac_set_ext(int unit, int il2cpi, soc_sbx_g2p3_l2cpmac_t *e)
{
    sbStatus_t rv = SOC_E_NONE;
    int uCamId = SB_G2P3_FE_PP_LSM_MATCH_CAM_ID;
    int uEntry = SB_G2P3_FE_PP_RULE_S0_ETH_L2CP_P0_ENTRY + il2cpi;
    int uOffset = 0;
    int nPortId = (e->useport) ? e->port : SB_G2P3_FE_PP_PORT_GLOBAL;
    soc_sbx_g2p3_6_byte_t uData;
    soc_sbx_g2p3_6_byte_t uMask;
    ...
    soc_sbx_g2p3_6_byte_t *pMask)
{
    ...
    *pPortId = SB_G2P3_FE_PP_CSTATE_PID_GET(pCamEntry->m_uData6);
    ...
    if (uOffset == 0) {
        (*pData)[0] = (uint32)((pCamEntry->m_uData5 >> 24) & 0xFF);
        (*pData)[1] = (uint32)((pCamEntry->m_uData5 >> 16) & 0xFF);
        (*pData)[2] = (uint32)((pCamEntry->m_uData5 >>  8) & 0xFF);
        (*pData)[3] = (uint32)((pCamEntry->m_uData5 >>  0) & 0xFF);
        (*pData)[4] = (uint32)((pCamEntry->m_uData4 >> 24) & 0xFF);
        (*pData)[5] = (uint32)((pCamEntry->m_uData4 >> 16) & 0xFF);
    
        (*pMask)[0] = (uint32)((pCamEntry->m_uMask5 >> 24) & 0xFF);
        (*pMask)[1] = (uint32)((pCamEntry->m_uMask5 >> 16) & 0xFF);
        (*pMask)[2] = (uint32)((pCamEntry->m_uMask5 >>  8) & 0xFF);
        (*pMask)[3] = (uint32)((pCamEntry->m_uMask5 >>  0) & 0xFF);
        (*pMask)[4] = (uint32)((pCamEntry->m_uMask4 >> 24) & 0xFF);
        (*pMask)[5] = (uint32)((pCamEntry->m_uMask4 >> 16) & 0xFF);
    } else if (uOffset == 12) {
                               /* pCamEntry->m_uData5 = ERH[11:8] */
                               /* pCamEntry->m_uData4 = ERH[ 7:4] */
                               /* pCamEntry->m_uData3 = ERH[ 3:0] */
        (*pData)[0] = (uint32)((pCamEntry->m_uData2 >> 24) & 0xFF);
        (*pData)[1] = (uint32)((pCamEntry->m_uData2 >> 16) & 0xFF);
        (*pData)[2] = (uint32)((pCamEntry->m_uData2 >>  8) & 0xFF);
        (*pData)[3] = (uint32)((pCamEntry->m_uData2 >>  0) & 0xFF);
        (*pData)[4] = (uint32)((pCamEntry->m_uData1 >> 24) & 0xFF);
        (*pData)[5] = (uint32)((pCamEntry->m_uData1 >> 16) & 0xFF);
                               /* pCamEntry->m_uMask5 = ERH[11:8] */
                               /* pCamEntry->m_uMask4 = ERH[ 7:4] */
                               /* pCamEntry->m_uMask3 = ERH[ 3:0] */
        (*pMask)[0] = (uint32)((pCamEntry->m_uMask2 >> 24) & 0xFF);
        (*pMask)[1] = (uint32)((pCamEntry->m_uMask2 >> 16) & 0xFF);
        (*pMask)[2] = (uint32)((pCamEntry->m_uMask2 >>  8) & 0xFF);
        (*pMask)[3] = (uint32)((pCamEntry->m_uMask2 >>  0) & 0xFF);
        (*pMask)[4] = (uint32)((pCamEntry->m_uMask1 >> 24) & 0xFF);
        (*pMask)[5] = (uint32)((pCamEntry->m_uMask1 >> 16) & 0xFF);
    } else {
        /* Need to adjust for QESS ERH */
        /* Program the mask and data!! */
        return SOC_E_UNAVAIL;
...
{
  sbStatus_t status = SB_OK;
  uint32 uEvent0 =0, uEvent1 = 0;
  uint32 uMask   =0, uRegId  = 0;
...
#define READ_PP_IPV4_ICMP_PROTOCOL(unit) \
    ...
    uRegId = 1;
  } else if(eType == SB_FE2000_INV_IPV6_VER) {
    uMask = SB_G2P3_FE_PP_EVENT0_MASK(pFe->regSet, INV_IPV6_VER);
  } else if(eType == SB_FE2000_IPV6_PKT_LEN_ERR) {
    uMask = SB_G2P3_FE_PP_EVENT0_MASK(pFe->regSet, IPV6_PKT_LEN_ERR);
  } else if(eType == SB_FE2000_IPV6_SA_OR_DA_LOOPBACK) {
    uMask = SB_G2P3_FE_PP_EVENT1_MASK(pFe->regSet, IPV6_SA_OR_DA_LPBK);
    uRegId = 1;
  } else {
    LOG_CLI((BSL_META("ignore unrecognized broadshield type 0x%x\n"), eType));
  }
...
{
  soc_sbx_g2p3_state_t *pFe = SOC_SBX_CONTROL(unit)->drv;
  uint32 cam_size = (!SOC_IS_SBX_FE2KXT(unit))
	            ? SB_ZF_CAPPCAMCONFIGURATIONENTRY_SIZE_IN_WORDS
	            : SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_SIZE_IN_WORDS;
  uint32 uCfgBuffer[SB_ZF_C2PPCAMCONFIGURATIONINSTANCE0ENTRY_SIZE_IN_WORDS];
  uint32 ram_size = (!SOC_IS_SBX_FE2KXT(unit))
	            ? SB_ZF_CAPPCAMRAMCONFIGURATIONENTRY_SIZE_IN_WORDS
	            : SB_ZF_C2PPCAMRAMCONFIGURATIONINSTANCE0ENTRY_SIZE_IN_WORDS;
  uint32 uCfgRamBuffer[SB_ZF_C2PPCAMRAMCONFIGURATIONINSTANCE0ENTRY_SIZE_IN_WORDS];
  uint32 addr = ((uCamId << SAND_HAL_FE2000_FIELD_SHIFT(pFe->regSet, PP_TCAM_CONFIG, MEM_ACC_CTRL_INSTANCE)) | uCamEntryIndex);

  sbG2P3FePpCamEntry_Pack(unit, pCamEntry, (uint8*)uCfgBuffer);
  sbG2P3FePpRamEntry_Pack(unit, pRamEntry, (uint8*)uCfgRamBuffer);
                     ...
                     sbG2P3FePpRamEntry_t *pRamEntry)
{
  ...
  sbG2P3FePpCamEntry_Unpack(unit, pCamEntry, (uint8 *) &uCfgBuffer[0]);
  sbG2P3FePpRamEntry_Unpack(unit, pRamEntry, (uint8 *) &uCfgRamBuffer[0]);
  return SB_OK;
}

sbStatus_t
soc_sbx_g2p3_pp_psc_set (int unit, uint32 fieldMask) {
    return soc_sbx_g2p3_pp_psc_config(unit, fieldMask);
}

#define ACH_CONTROL 0x10000000

int
soc_sbx_g2p3_ach_channel_type_set_ext(int unit,
                                      int iindex,
                                      soc_sbx_g2p3_ach_channel_type_t *e)
{
    ...
    if (e->ping) {
        data = ACH_CONTROL | e->ping;
         uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PING_RAW_TBD_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PING_RAW_TBD_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                           & (pPpMgr->cam[uCamId].ramdb[uEntry]) );

         if (SOC_SUCCESS(rv)) {
             uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_PING_CAM_ID;
             uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_PING_ENTRY;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
             rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                               & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
    ...
    if (SOC_SUCCESS(rv) && (e->ipbfd)) {
         data = ACH_CONTROL | e->ipbfd;
         uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_IP_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_IP_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                           & (pPpMgr->cam[uCamId].ramdb[uEntry]) );
         if (SOC_SUCCESS(rv)) {
                 rv = soc_sbx_g2p3_oam_bfd_ip_achword_set(unit, data);
         }
    }


    if (SOC_SUCCESS(rv) && (e->rawbfd)) {
         data = ACH_CONTROL | e->rawbfd;
         uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_RAW_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_RAW_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
         ...
         if (SOC_SUCCESS(rv)) {
             uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_BFD_RAW_TBD_CAM_ID;
             uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_BFD_RAW_TBD_ENTRY;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
             rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                           & (pPpMgr->cam[uCamId].ramdb[uEntry]) );
             if (SOC_SUCCESS(rv)) {
                 rv = soc_sbx_g2p3_oam_bfd_raw_achword_set(unit, data);
             }
         }
    }


    if (SOC_SUCCESS(rv) && (e->psc)) {
         data = ACH_CONTROL | e->psc;
         uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PSC_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PSC_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
         ...
         if (SOC_SUCCESS(rv)) {
             rv = soc_sbx_g2p3_oam_psc_achword_set(unit, data);
         }
    }

    if (SOC_SUCCESS(rv) && (e->y1731)) {
         data = ACH_CONTROL | e->y1731;
         uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_Y1731_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_Y1731_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
    ...
    if (SOC_SUCCESS(rv) && (e->mcc)) {
         data = ACH_CONTROL | e->mcc;
         uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MCC_ACH_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MCC_ACH_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData4 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask4 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                           & (pPpMgr->cam[uCamId].ramdb[uEntry]) );

         if (SOC_SUCCESS(rv)) {
             uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_MCC_ACH_CAM_ID;
             uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_MCC_ACH_ENTRY;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData3 = data;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask3 = 0xF000FFFF;
             rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                               & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                               & (pPpMgr->cam[uCamId].ramdb[uEntry]) );
             ...
             if (SOC_SUCCESS(rv)) {
                 uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MCC_CAM_ID;
                 uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MCC_ENTRY;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
                 rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                                   & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
    ...
    if (SOC_SUCCESS(rv) && (e->scc)) {
         data = ACH_CONTROL | e->scc;
         uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_SCC_ACH_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_SCC_ACH_ENTRY;
         uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_SCC_ACH_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_SCC_ACH_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData4 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask4 = 0xF000FFFF;
         ...
         if (SOC_SUCCESS(rv)) {
             uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_SCC_ACH_CAM_ID;
             uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_SCC_ACH_ENTRY;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData3 = data;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask3 = 0xF000FFFF;
             rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                               & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
             ...
             if (SOC_SUCCESS(rv)) {
                 uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_SCC_ENTRY;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
                 rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                               & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
    ...
    if (SOC_SUCCESS(rv) && (e->loop)) {
         data = ACH_CONTROL | e->loop;
         uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MPLSTPLOOP_ACH_CAM_ID;
         uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MPLSTPLOOP_ACH_ENTRY;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData4 = data;
         pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask4 = 0xF000FFFF;
         rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                           & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
                                           & (pPpMgr->cam[uCamId].ramdb[uEntry]) );
         if (SOC_SUCCESS(rv)) {
             uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_MPLSTPLOOP_ACH_CAM_ID;
             uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_MPLS2_GAL_LABEL_MPLSTPLOOP_ACH_ENTRY;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData3 = data;
             pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask3 = 0xF000FFFF;
             rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                               & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
             ...
             if (SOC_SUCCESS(rv)) {
                 uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MPLSTPLOOP_CAM_ID;
                 uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MPLSTPLOOP_ENTRY;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 = data;
                 pPpMgr->cam[uCamId].cfgdb[uEntry].m_uMask5 = 0xF000FFFF;
                 rv = soc_sbx_g2p3_pp_cam_rule_write(unit,uCamId, uEntry,
                                                   & (pPpMgr->cam[uCamId].cfgdb[uEntry]),
...
int
soc_sbx_g2p3_ach_channel_type_get_ext(int unit,
                                      int iindex,
                                      soc_sbx_g2p3_ach_channel_type_t *e)
{
    int uCamId, uEntry;
    ...
    uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_PING_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_PING_ENTRY;
    e->ping = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_BFD_RAW_TBD_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_BFD_RAW_TBD_ENTRY;
    e->rawbfd = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);
    uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_IP_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_BFD_IP_ENTRY;
    e->ipbfd = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PSC_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_PSC_ENTRY;
    e->psc = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_Y1731_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_GAL_MPLS_LSP_Y1731_ENTRY;
    e->y1731 = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MCC_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_MCC_ENTRY;
    e->mcc = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_SCC_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S4_PWE_ACH_CTL_SCC_ENTRY;
    e->scc = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);

    uCamId = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MPLSTPLOOP_ACH_CAM_ID;
    uEntry = SB_G2P3_FE_PP_RULE_S3_MPLS1_GAL_LABEL_MPLSTPLOOP_ACH_ENTRY;
    e->loop = (pPpMgr->cam[uCamId].cfgdb[uEntry].m_uData5 & 0xFFFF);
