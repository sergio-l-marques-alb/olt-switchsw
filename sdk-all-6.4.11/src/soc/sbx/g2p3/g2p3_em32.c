...
/******************************************************************************
 *
 * $Id: g2p3_em32.c,v 1.31 Broadcom SDK $
 *
...
g2p3_em32Recover(void *em32Ctxt)
{
  ...
  if (SOC_FAILURE(rv)) {
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(unit,
                          "Failed to recover cuckoo seed: %d\n"), rv));
    return SB_NOT_FOUND;
  }
  ...
  buffer_size = 2 * pEm32->tableSize * sizeof(uint32);
  ...
  for (table = emTableStart; table < emTableCount; table++) {
    ...
    if (SOC_FAILURE(rv)) {
      LOG_CLI((BSL_META_U(unit,
                          "Failed to DMA payload table %d/%d: %s\n"),
               table, bank, soc_errmsg(rv)));
      goto done;
    }
...
g2p3_em32FindNextKey(g2p3_em32CompState_p_t pEm32,
                     uint32 *keyTable[emTableCount],
                     uint32 seed, 
                     emTable_t *table, uint32 *table_idx,
                     uint32 *next_key) 
{
  int32   theSeed, crc, half_crc, tmp_key;
...
g2p3_em32KeyLookup(g2p3_em32CompState_p_t pEm32,
                   uint32 *keyTable[emTableCount],
                   uint32 seed, uint32 key, 
                   emTable_t *table, uint32 *table_idx)
{
  uint32 crc, half_crc, device_crc;
               ...
               sbFe2000DmaAddr_t            dupRightBase)
{
  ...
  tableBytes = pEm32Comp->tableSize * sizeof(uint32);
  status = cParams->sbmalloc (cParams->clientData, SB_ALLOC_INTERNAL, 
                              tableBytes, &vp, NULL);
  if (status != SB_OK)
    return status;
  
  pEm32Comp->leftKey = (uint32 *) vp;
  ...
  for (table = emTableStart; table < emTableCount; table++) {
    ...
    pEm32Comp->deviceKeyTable[table] = (uint32 *) vp;
    ...
    pEm32Comp->keyTableCopy[table] = (uint32 *) vp;
    ...
    pEm32Comp->dmaKeyTable[table] = (uint32 *) vp;
    ...
    if(pEm32Comp->useDupBank) {
      ...
      pEm32Comp->dmaDupKeyTable[table] = (uint32 *) vp;
    ...
    pEm32Comp->dmaPayloadTable[table] = (uint32 *) vp;
  ...
  pEm32Comp->devicePayloads = (uint8 *) vp;
  ...
  status = cParams->sbmalloc (cParams->clientData, SB_ALLOC_OTHER_DMA,
                              pEm32Comp->slabWords * sizeof(uint32), 
...
{ 
  ...
  err = cParams->sbfree(cParams->clientData, SB_ALLOC_INTERNAL,
                        pEm32->tableSize * sizeof(uint32), pEm32->leftKey, junk);
  if (err)
    return (err);
  
  /* rightKey */
  err = cParams->sbfree(cParams->clientData, SB_ALLOC_INTERNAL,
                        pEm32->tableSize * sizeof(uint32), pEm32->rightKey, junk);
  if (err)
    return (err);
  ...
  err = cParams->sbfree(cParams->clientData, SB_ALLOC_OTHER_DMA,
          pEm32->slabWords * sizeof(uint32), pEm32->slabOp->slab, 
...
flushtop:
  ...
  if (emComp->commitState < emCommitStateSeed) {
    ...
    if (emComp->flushMode == emFlushModeTable) {
      ...
      LOG_INFO(BSL_LS_SOC_MEM,
               (BSL_META("%s TableFlush: emAddr/feAddr/words= 0x%08x / 0x%08x / %d  (d[flush]=0x%08x\n"),
        ...
        slab->feAddress = 
          SB_FE2000_DMA_MAKE_ADDRESS (emComp->dmaDupInfo[table].key.bank,
                                      (emComp->dmaDupInfo[table].key.base + 
                                       ...
                                       emComp->currFlushTablePos));
        LOG_INFO(BSL_LS_SOC_MEM,
                 (BSL_META("%s Duplicate Cuckoo TableFlush: "
      ...
      while ((slabWords >= 3*SB_FE2000_SLAB_SIZE(emComp->payloadSize / (3*sizeof (uint32)))) && 
        ...
        LOG_INFO(BSL_LS_SOC_MEM,
                 (BSL_META("%s Word flush: emAddr/feAddr= 0x%08x / 0x%08x \n"),
                  commitStateNames[emComp->commitState],
                  (emComp->dmaInfo[table].payload.base +
                   emComp->bankOffset[emComp->currBank] + 
                   emComp->currFlushTablePos),
...
{
  uint32 i, tableBytes, paylIdx, key;
  uint8 *pd, *ps;
  emTable_t table;

  tableBytes = pEm32->tableSize * sizeof(uint32);
  for (table = emTableStart; table < emTableCount; table++) {
  ...
  for (i = 0; i < pEm32->tableSize; i++) {
    ...
    for (table = emTableStart; table < emTableCount; table++) {
      ...
      pd = (uint8 *) &pEm32->dmaKeyTable[table][i];
 ...
 * ---------------------------------------------------------------------------*/
static int
g2p3_em32Insert (g2p3_em32CompState_p_t pEm32, uint32 key, 
                 emTable_t *insTable, uint32 *insPos)
{
  uint32 h1crc, h1idx, h2crc, h2idx, tempKey, devTableEntry, tempEntry, moves;
  uint32 lKey;
...
g2p3_em32Rehash (g2p3_em32CompState_p_t pEm32)
{
  emTable_t table;
  uint32 tableBytes;
  uint32 insTable, insPos, oldSeed; 
  uint32 key, idx, payload_idx;
  uint32 h1, h2;
  int i, rv;
...
{
  uint32 h1, h2;
  uint32 res, i, paylSlotIdx;
  uint8 paylSlotFound;
  uint32 insPos;
  emTable_t insTable;
  uint32 rehashCount, rehashRes; 
  g2p3_em32CompState_p_t pEm32 = (g2p3_em32CompState_p_t)em32Ctxt;
  ...
  LOG_INFO(BSL_LS_SOC_MEM,
           (BSL_META("h1=0x%08x h2=0x%08x key=0x%08x\n"),
            h1, h2, key));
