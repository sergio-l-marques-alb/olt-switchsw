...
/*
 * $Id: g2p3_cls_c2.c,v 1.16 Broadcom SDK $
...
extern int
soc_sbx_g2p3_cls_schema_layout_init(soc_sbx_g2p3_state_t *pFe, uint32 isC2);
extern sbStatus_t
...
soc_sbx_g2p3_cls_ruleset_relink(soc_sbx_g2p3_state_t *pFe,
                               uint32 uRs,
                               uint32 uNextRs,
                               uint32 uDmaBlockOffset,
                               uint32 uInstOffset,
                               uint8 uToLinkList);

int
soc_sbx_g2p3_cls_ruleset_linkage_dump(uint32 unit,
                                      uint8 uProgram,
                                      uint8 uLogProgramNumber,
                                      soc_sbx_g2p3_cls_schema_enum_t sType,
                                      uint32 uBank);
...
soc_sbx_g2p3_cls_commit_group_c2(soc_sbx_g2p3_state_t *pFe,
                          ...
                          soc_sbx_g2p3_cls_rule_db_e_t eDb,
                          uint32 uBank,
                          uint8 uLogProgNumber,
                          soc_sbx_g2p3_cls_group_t *pClsGroup,
                          pfRuleCb pfRuleIter,
                          pfFinalizeCb pfFinalize,
                          uint32 uLastCommitRs,
                          int8 iRc,
                          uint32 *uFirstRs);
...
int soc_sbx_g2p3_cls_ruleset_free_count_get(soc_sbx_g2p3_state_t *pFe,
                                            uint32 uSchemaSize,
                                            uint32 uBank,
                                            uint32 *pRc0FreeRsCount,
                                            uint32 *pRc1FreeRsCount);
...
void soc_sbx_g2p3_cls_dump_rsbd_and_mem(soc_sbx_g2p3_state_t *pFe) {
    ...
    /*dump RSDB */
    LOG_CLI((BSL_META("RSDB:\n------------------------------\n")));
    for (i=0; i<pClsComp->uMaxRs*SB_FE2000_NUM_RC_INSTANCES; i++) {
        LOG_CLI((BSL_META("  Rs: 0x%x Rc: 0x%x COM: 0x%x alloc: 0x%x BA: 0x%x length: 0x%x prog: 0x%x sType: 0x%x, group: 0x%x, nextRs: 0x%x\n"),
...
soc_sbx_g2p3_cls_wb_ruleset_recover(soc_sbx_g2p3_state_t *pFe, 
    ...
    uint32 uSb, uBlock, uNumDmaBlocks, uId, uInst, uInstIndex; /* uSchemaInst */
    uint32 uPatternIdx, uPatternWord;
    uint32 uBuffer[SB_ZF_C2RCDMAFORMAT_SIZE_IN_WORDS];
    uint32 uCount = 0;
    ...
    for (uBlock=0; uBlock<uNumDmaBlocks; uBlock++){
        ...
        sbZfC2RcDmaFormat_Unpack(&zDmaFrameDecode, (uint8*)uBuffer, SB_ZF_C2RCDMAFORMAT_SIZE_IN_BYTES);
        ...
        for (uInst=0; uInst < SB_FE2000XT_RC_INST_PER_DMA; uInst++){
            ...
            for (uPatternIdx=0; uPatternIdx < SB_FE2000XT_RC_WORDS_PER_PATTERN * SB_FE2000XT_RC_NUM_SUPERBLOCKS; uPatternIdx++){
                ...
                sbZfC2RcDmaFormat_GetPatternWord(&(((uint32*)pPatternDecode)[3-uPatternWord]),
                                                 uSb,
                                                 uInst,
                                                 uPatternWord,
                                                 (uint32*)(&zDmaFrameDecode));
            }
        }
    }
...
sbStatus_t
soc_sbx_g2p3_cls_wb_group_recover(soc_sbx_g2p3_state_t *pFe, soc_sbx_g2p3_cls_schema_t *pSchema, soc_sbx_g2p3_cls_rule_db_e_t eDb, uint32 uGroupId, uint32 uRuleCount){
...
soc_sbx_g2p3_cls_schema_init_c2(soc_sbx_g2p3_state_t *pFe)
{
  ...
  if (wbc){
      ...
      LOG_WARN(BSL_LS_SOC_COMMON,
               (BSL_META("[%d:%s]:Obtained scache pointer=0x%08x, %d bytes, "
                         "version=%d.%d\n"), pFe->unit, FUNCTION_NAME(),
      ...
      if (wbc->version > G2P3_CLS_WB_CURRENT_VERSION) {
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META("[%d:%s]: Upgrade scenario not supported.  "
          ...
          LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META("[%d:%s]: Downgrade scenario not supported.  "
                              "Current version=%d.%d  found %d.%d\n"),pFe->unit, FUNCTION_NAME(),
                     SOC_SCACHE_VERSION_MAJOR(G2P3_CLS_WB_CURRENT_VERSION),
                     SOC_SCACHE_VERSION_MINOR(G2P3_CLS_WB_CURRENT_VERSION),
                     SOC_SCACHE_VERSION_MAJOR(wbc->version),
                  ...
                  void* pWbData)
{
  ...
  uint32 uIngressProgEndAddress = 0;
  sbStatus_t err;
  soc_sbx_g2p3_cls_t *pClsCompiler = NULL;
  soc_sbx_g2p3_cls_prog_attr_t *pProgAttr = NULL;
  uint32 uRules, uBankedMode, uSize;
  uint8 v6Enable, uLogProgNumber=0;
  int16 iRc, iBank;
 ...
 /* set banked mode based on a SOC property */
  ...
  for (iRc = 0; iRc < G2P3_MAX_RC_INSTANCE; iRc++) {
      ...
      result = shr_mem_avl_create(&pClsCompiler->mem_avl_ptr[iRc][iBank],
                                   ...
                                   64);
      if (result !=0) {
          LOG_CLI((BSL_META("CLS: shr_mem_avl_create failed for RC:%d Bank:%d\n"),
                   iRc, iBank));
      }
  }
  ...
  LOG_CLI((BSL_META("RCE ingrSchemaLen: 0x%x\n"), ingrSchemaLen));
  LOG_CLI((BSL_META("RCE egrSchemaLen: 0x%x\n"), egrSchemaLen));
#endif
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("IRT table size: 0x%x\n"), uRules));
#endif
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("ERT table size: 0x%x\n"), uRules));
#endif
...
do { \
    ...
    if (pSchemaHandler->schema.ops[0].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uDbType);
        COMPILER_64_SET(uuTmp2, 0, pRule->bDbTypeAny);
    ...
    if (pSchemaHandler->schema.ops[1].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTC);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTCAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[2].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagURG);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagURGAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[3].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagACK);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagACKAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[4].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagPSH);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagPSHAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[5].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagRST);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagRSTAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[6].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagSYN);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagSYNAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[7].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uTcpFlagFIN);
        COMPILER_64_SET(uuTmp2, 0, pRule->bTcpFlagFINAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[8].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uVlan);
        COMPILER_64_SET(uuTmp2, 0, pRule->bVlanAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[9].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uL4SrcPortLow);
        COMPILER_64_SET(uuTmp2, 0, pRule->uL4SrcPortHigh);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if ((pSchemaHandler->schema.ops[10].bEnabled) ||
        ...
        COMPILER_64_SET(uuTmp1, 0, pRule->uL4DstPortLow);
        COMPILER_64_SET(uuTmp2, 0, pRule->uL4DstPortHigh);
            sbFe2000XtClsEncodePattern(uRuleId,
                                   uuTmp1,
                                   uuTmp2,
    ...
    if ((pSchemaHandler->schema.ops[10].bEnabled) ||
        (pSchemaHandler->schema.ops[11].bEnabled)) {
        if ((pRule->uL4SrcPortLow == 0) && (pRule->uL4SrcPortHigh == 0)) {
            COMPILER_64_SET(uuTmp1, 0, pRule->uNextHeaderLow);
            COMPILER_64_SET(uuTmp2, 0, pRule->uNextHeaderHigh);
             sbFe2000XtClsEncodePattern(uRuleId,
                                   uuTmp1, 
                                   uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[12].bEnabled) {
        COMPILER_64_SET(uuTmp2, 0, pRule->uuIpv6DaLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               pRule->uuIpv6DaLS, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[13].bEnabled) {
        COMPILER_64_SET(uuTmp2, 0, pRule->uuIpv6SaLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               pRule->uuIpv6SaLS, 
                               uuTmp2,
    ...
    uint32                               *isNull)
{
    ...
    soc_sbx_g2p3_cls_ace_pattern_ipv6_p_t  pRule = vRule;
    uint8 dummy;
    uint64 uuField1 = COMPILER_64_INIT(0,0), uuField2 = COMPILER_64_INIT(0,0), uuField3 = COMPILER_64_INIT(0,0);
...
do { \
    ...
    if (pSchemaHandler->schema.ops[0].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uDbType = COMPILER_64_LO(uuField1);
        pRule->bDbTypeAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
        pRule->bDbTypeAny = 1;
    }
    if (pSchemaHandler->schema.ops[1].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTC = COMPILER_64_LO(uuField1);
        pRule->bTCAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
        pRule->bTCAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[2].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagURG = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagURGAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[3].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagACK = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagACKAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[4].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagPSH = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagPSHAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[5].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagRST = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagRSTAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[6].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagSYN = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagSYNAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[7].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uTcpFlagFIN = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagFINAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
    ...
    if (pSchemaHandler->schema.ops[8].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uVlan = COMPILER_64_LO(uuField1);
        pRule->bVlanAny = COMPILER_64_LO(uuField2) & 0xFF;
    }else{
        pRule->bVlanAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[9].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uL4SrcPortLow = COMPILER_64_LO(uuField1);
        pRule->uL4SrcPortHigh = COMPILER_64_LO(uuField2);
    }else{
    ...
    */
    ...
    if ((pSchemaHandler->schema.ops[10].bEnabled) ||
        ...
        if (!((pRule->uL4SrcPortLow == 0) && (pRule->uL4SrcPortHigh == 0))) {
            sbFe2000XtClsDecodePattern(uRuleId,
                                   ...
                                   pPattern,
                                   &uuField3);
            pRule->uL4DstPortLow = COMPILER_64_LO(uuField1);
            pRule->uL4DstPortHigh = COMPILER_64_LO(uuField2);
        }
    }
    ...
    if ((pSchemaHandler->schema.ops[10].bEnabled) ||
        ...
        if ((pRule->uL4SrcPortLow == 0) && (pRule->uL4SrcPortHigh == 0)) {
             sbFe2000XtClsDecodePattern(uRuleId,
                                   ...
                                   pPattern,
                                   &uuField3);
             pRule->uNextHeaderLow = COMPILER_64_LO(uuField1);
             pRule->uNextHeaderHigh = COMPILER_64_LO(uuField2);
        }
    }
    ...
    if (pSchemaHandler->schema.ops[12].bEnabled) {
        ...
        pRule->uuIpv6DaLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uuIpv6DaLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[13].bEnabled) {
        ...
        pRule->uuIpv6SaLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uuIpv6SaLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[14].bEnabled) {
        ...
        pRule->bPortAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bPortAny = 1;
    }
    if (pRule->bPortAny) {
        COMPILER_64_ZERO(pRule->uuPbmp);
        COMPILER_64_NOT(pRule->uuPbmp);
    }
...
sbStatus_t soc_sbx_g2p3_cls_rr_init(soc_sbx_g2p3_state_t *pFe)
{
  ...
  uint32 uOffs, uData, uStride, uRce, uCounter;
  uint32 uSet, uSbFilter;
  soc_sbx_g2p3_cls_t *pClsComp = NULL;
  ...
  for (uRs=0; uRs<uMaxRs; uRs++) {
    ...
    for (uSbFilter=0; uSbFilter<G2P3_MAX_CLS_RULES_PER_RULESET_C2; uSbFilter++){
      ...
      if (pRsDb[uRs].uRcInstance) {
        sbFe2000XtImfDriver_Rc1ResultResolverWrite((uint32)pFe->regSet, uRrAddress, zfResult.m_uData);
        sbFe2000XtImfDriver_Rc1ResultResolverWrite((uint32)pFe->regSet, uRrAddress+1, 0);
      } else {
        sbFe2000XtImfDriver_Rc0ResultResolverWrite((uint32)pFe->regSet, uRrAddress, zfResult.m_uData);
        sbFe2000XtImfDriver_Rc0ResultResolverWrite((uint32)pFe->regSet, uRrAddress+1, 0);
      }
      zfResult.m_uData++;
    }
  }
...
do { \
    ...
    if (pSchemaHandler->schema.ops[0].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uFragment);
        COMPILER_64_SET(uuTmp2, 0, pRule->bFragmentAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
                               &pSchemaHandler->schema.ops[0],
    ...
    if ((pSchemaHandler->schema.ops[2].bEnabled) ||
        (pSchemaHandler->schema.ops[3].bEnabled)) {
        if ((!pRule->bEtherTypeAny) && (!pRule->uEtherType)) {
            COMPILER_64_SET(uuTmp1, 0, pRule->uLlcDsapSsapLow);
            COMPILER_64_SET(uuTmp2, 0, pRule->uLlcDsapSsapHigh);
             sbFe2000XtClsEncodePattern(uRuleId,
                                   uuTmp1, 
                                   uuTmp2,
                                   &pSchemaHandler->schema.ops[3],
    ...
    if ((pSchemaHandler->schema.ops[7].bEnabled) ||
       (pSchemaHandler->schema.ops[8].bEnabled)) {
        if ((!pRule->bEtherTypeAny) && (!pRule->uEtherType)) {
            COMPILER_64_SET(uuTmp1, 0, pRule->uLlcCtrlDataLow);
            COMPILER_64_SET(uuTmp2, 0, pRule->uLlcCtrlDataHigh);
            sbFe2000XtClsEncodePattern(uRuleId,
                                   uuTmp1,
                                   uuTmp2,
                                   &pSchemaHandler->schema.ops[8],
    ...
    if (pSchemaHandler->schema.ops[11].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uPriority);
        COMPILER_64_SET(uuTmp2, 0, pRule->bPriorityAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
                               &pSchemaHandler->schema.ops[11],
    ...
    if (pSchemaHandler->schema.ops[12].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uDscp);
        COMPILER_64_SET(uuTmp2, 0, pRule->bDscpAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
                               &pSchemaHandler->schema.ops[12],
    ...
    if (pSchemaHandler->schema.ops[13].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uEcn);
        COMPILER_64_SET(uuTmp2, 0, pRule->bEcnAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
                               &pSchemaHandler->schema.ops[13],
    ...
    if (pSchemaHandler->schema.ops[14].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uProtocol);
        COMPILER_64_SET(uuTmp2, 0, pRule->bProtocolAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
                               &pSchemaHandler->schema.ops[14],
    ...
    if (pSchemaHandler->schema.ops[15].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uIpv4Sa);
        COMPILER_64_SET(uuTmp2, 0, pRule->uIpv4SaLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[16].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uIpv4Da);
        COMPILER_64_SET(uuTmp2, 0, pRule->uIpv4DaLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[17].bEnabled) {
        COMPILER_64_SET(uuTmp1, 0, pRule->uEtherType);
        COMPILER_64_SET(uuTmp2, 0, pRule->bEtherTypeAny);
        sbFe2000XtClsEncodePattern(uRuleId,
                               uuTmp1, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[18].bEnabled) {
        COMPILER_64_SET(uuTmp2, 0, pRule->uuSmacLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               pRule->uuSmac, 
                               uuTmp2,
    ...
    if (pSchemaHandler->schema.ops[20].bEnabled) {
        COMPILER_64_SET(uuTmp2, 0, pRule->uuDmacLen);
        sbFe2000XtClsEncodePattern(uRuleId,
                               pRule->uuDmac, 
                               uuTmp2,
...
void soc_sbx_g2p3_cls_g2p3_schema_decoder_c2(
    ...
    uint32                            *isNull)
{
    uint32 uLocationSave, uLocation = 0;
    uint32 uRuleId     = 0;
    uint32 uRuleSb     = 0;
    sbZfFe2000RcSbPattern_t *pPattern = NULL;
    soc_sbx_g2p3_cls_ace_pattern_p_t    pRule = vRule;
...
do { \
    ...
    if (pSchemaHandler->schema.ops[0].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern, 
                               &uuField3);
        pRule->uFragment = COMPILER_64_LO(uuField1);
        pRule->bFragmentAny = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[1].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagURG = COMPILER_64_LO(uuField1); 
        pRule->bTcpFlagURGAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagURGAny = 1;
    }
    ...
    */
    ...
    if ((pSchemaHandler->schema.ops[2].bEnabled) ||
        (pSchemaHandler->schema.ops[3].bEnabled)) {
         sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
         pRule->uLlcDsapSsapLow = COMPILER_64_LO(uuField1);
         pRule->uLlcDsapSsapHigh = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[4].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagACK = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagACKAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagACKAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[5].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagPSH = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagPSHAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagPSHAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[6].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagRST = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagRSTAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagRSTAny = 1;
    }
    ...
    */
    ...
    if ((pSchemaHandler->schema.ops[7].bEnabled) ||
       (pSchemaHandler->schema.ops[8].bEnabled)) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uLlcCtrlDataLow = COMPILER_64_LO(uuField1);
        pRule->uLlcCtrlDataHigh = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[9].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagSYN = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagSYNAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagSYNAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[10].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uTcpFlagFIN = COMPILER_64_LO(uuField1);
        pRule->bTcpFlagFINAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bTcpFlagFINAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[11].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uPriority = COMPILER_64_LO(uuField1);
        pRule->bPriorityAny = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[12].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uDscp = COMPILER_64_LO(uuField1);
        pRule->bDscpAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bDscpAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[13].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uEcn = COMPILER_64_LO(uuField1);
        pRule->bEcnAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bEcnAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[14].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uProtocol = COMPILER_64_LO(uuField1);
        pRule->bProtocolAny = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[15].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uIpv4Sa = COMPILER_64_LO(uuField1);
        pRule->uIpv4SaLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uIpv4SaLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[16].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uIpv4Da = COMPILER_64_LO(uuField1);
        pRule->uIpv4DaLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uIpv4DaLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[17].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               pPattern,
                               &uuField3);
        pRule->uEtherType = COMPILER_64_LO(uuField1);
        pRule->bEtherTypeAny = COMPILER_64_LO(uuField2);
    }else{
    ...
    if (pSchemaHandler->schema.ops[18].bEnabled) {
        ...
        pRule->uuSmacLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uuSmacLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[19].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uVlan = COMPILER_64_LO(uuField1);
        pRule->bVlanAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bVlanAny = 1;
    }
    ...
    if (pSchemaHandler->schema.ops[20].bEnabled) {
        ...
        pRule->uuDmacLen = COMPILER_64_LO(uuField2);
    }else{
        pRule->uuDmacLen = 0;
    }
    ...
    if (pSchemaHandler->schema.ops[21].bEnabled) {
        sbFe2000XtClsDecodePattern(uRuleId,
                               ...
                               &uuField3);
        pRule->uDbType = COMPILER_64_LO(uuField1);
        pRule->bDbTypeAny = COMPILER_64_LO(uuField2);
    }else{
        pRule->bDbTypeAny = 1;
    }
                ...
                uint32 uRc)
{
  ...
  uint32 uPhysicalProgramNumber =0;
  uint8  uProgramNumber=0, uLogProgNumber=0;
...
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("After adj schema correction: \n")));
    LOG_CLI((BSL_META("Setting RC 0x%x to ruleset 0x%x, program base address 0x%x,"),
             uRc, uBaseRs, pClsComp->sRsDb[uBaseRs].uBaseAddress));
    LOG_CLI((BSL_META(" Phys Prog Num: 0x%x\n"), uPhysicalProgramNumber));
#endif
                ...
                uint32 uRs)
{
  ...
  uint32 uPhysicalProgramNumber =0;
  uint8  uProgramNumber=0, uLogProgNumber = 0;
  uint32 uDummy, uFirstRs = -1, uLastRs = -1;
  soc_sbx_g2p3_cls_prog_attr_t *pNewActProgAttr = NULL;
  soc_sbx_g2p3_cls_bank_transfer_t *pBankHandler   = NULL;
  uint32 *pDmaBase                      = NULL;
  sbFe2000DmaOp_t  *pDmaOp                = NULL;
  soc_sbx_g2p3_cls_t *pClsComp = NULL;
  sbZfFe2000RcSbPattern_t *pPattern       = NULL;
  uint32 uNumberOfDmaBlocks =  (pSchema->uInstCount) / SB_FE2000XT_RC_INST_PER_DMA;
  sbZfC2RcDmaFormat_t zDmaBlock;
  uint32 uDmaBlock, uInst, uInstIndex, uSchemaInst, uPatternIdx, uSb, uPatternWord;
  uint32 uPrevRs = -1, uNextRs = -1, uBaseRs = -1; 
  uint32 uSetNumber,uJumpTarget;
  uint32 uDmaBlockJump=0, uAdjDmaBlockJump=0, uAdjSchemaLen;
  uint32 uJumpOffset=0, uAdjJumpOffset=0;
  /* uint32 uJumpInst; */
  uint8 bCache=0;
...
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("  Ingress Set: 0x%x\n"), uSetNumber));
#endif
  }else{
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("  Egress Set: 0x%x\n"), uSetNumber));
#endif
  }
  ...
  pDmaBase      = (uint32 *) pBankHandler->pDmaMemory;
  pDmaBase  = pDmaBase;
  ...
  for (uDmaBlock=0; uDmaBlock < uNumberOfDmaBlocks; uDmaBlock++) {
      ...
      for (uInst=0; uInst < SB_FE2000XT_RC_INST_PER_DMA; uInst++) {
          ...
          for (uPatternIdx=0; uPatternIdx < SB_FE2000XT_RC_WORDS_PER_PATTERN * SB_FE2000XT_RC_NUM_SUPERBLOCKS; uPatternIdx++) {
              ...
              sbZfC2RcDmaFormat_SetPatternWord((uint32*)(&zDmaBlock),
                                               uSb,
                                               uInst,
                                               uPatternWord,
                                               ((uint32*)pPattern)[3 - uPatternWord]);
          }
      }

    /* Pack the DMA buffers */
    sbZfC2RcDmaFormat_Pack(&zDmaBlock, (uint8*)uBuffer, SB_ZF_C2RCDMAFORMAT_SIZE_IN_BYTES);
    for(uId=0; uId < SB_ZF_C2RCDMAFORMAT_SIZE_IN_WORDS; uId++) {
  ...
  if ( (uPrevRs == -1) || \
    ...
    /* this first ruleset in this RC instance, update program mappings */
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("Setting RC 0x%x to ruleset 0x%x,"), uRcInstance, uRs));
    LOG_CLI((BSL_META(" program base address 0x%x, Phys Prog Num: 0x%x\n"),
                ...
                pfFinalizeCb pfFinalize)
{
  ...
  uint32 uLastRs, uAllocedRs, uNextRs, uRs = -1, uOldRs;
  uint32 uRulesInSet;
  uint32 uOrphRs = -1, uOrphRuleOffset;
  uint32 uRuleOffset;
  int32 nRuleIndex, nRs, uGroupRs;
  uint32 uNewBank = 0, uOldBank = 1, uTemp, uBankedMode = 0;
  uint8 uProgramNumber;
  uint8 uLogProgNumber = 0;
  uint32 uNewBackUpProgramNumber, uPhysicalProgramNumber;
  soc_sbx_g2p3_cls_prog_attr_t *pNewActProgAttr = NULL;
  soc_sbx_g2p3_cls_prog_attr_t *pNewBkupProgAttr = NULL;
  void * dummy;
  soc_sbx_g2p3_cls_ace_pattern_u_t dummy_pattern;
  uint32 uAdjFirstRs = -1, uAdjLastRs = -1;
  soc_sbx_g2p3_cls_schema_enum_t adjSchemaType;
  int8 uNextGroupId = -1;
#ifdef DYNAMIC_MEMMGR
  uint8 uRc;
  ...
  int32 index;
  int8 iRcPrefNext = -1;
  int8 uAllocCondition = 0;
  soc_sbx_g2p3_cls_schema_t *pSchema=pSchema1;
  soc_sbx_g2p3_cls_schema_t *pSchemaSwapRs[G2P3_MAX_RC_INSTANCE];
  uint32   uSwapRsRuleCount[G2P3_MAX_RC_INSTANCE];
  uint8   bSwapInit[G2P3_MAX_RC_INSTANCE];
  uint32 uDirty = 0, uRcInstance;
  ...
  for (index = (uGroupCount - 1); index >= 0; index--) {
      ...
      uGroupId = pClsGroups[index].uGroupId;
#ifdef RULESET_DEBUG
      LOG_CLI((BSL_META("Commit DB: 0x%x, uGroupId: 0x%x, Rule Count: 0x%x\n"),
...
#ifdef RULESET_DEBUG
      LOG_CLI((BSL_META("Currently allocated rulesets: 0x%x\n"), uAllocedRs));
...
#ifdef RULESET_DEBUG
          LOG_CLI((BSL_META("Allocating additional rulesets: 0x%x\n"), (uNumRs - uAllocedRs)));
#endif
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("linkage prior to commits\n")));
  soc_sbx_g2p3_cls_ruleset_linkage_dump(pFe->unit, eDb, uLogProgNumber, 
                                                pSchema->sType, uNewBank);
#endif
  uNumRs = 0;
...
#ifdef RULESET_DEBUG
      LOG_CLI((BSL_META("nRs: 0x%x, oldRs: 0x%x\n"), nRs, uOldRs));
#endif
      SB_ASSERT(uOldRs != -1);
...
#ifdef RULESET_DEBUG
              LOG_CLI((BSL_META("Setting RC 0x%x to ruleset 0x%x,"),
                       uRcInstance, uNextRs));
              LOG_CLI((BSL_META(" program base address 0x%x, Phys Prog Num: 0x%x\n"),
      ...
      if (uDirty) {
        ...
        if (uBankedMode == 0) {
              ...
              if (nRs == 0) {
                ...
                while (uOrphRs != -1) {
#ifdef RULESET_DEBUG
                   LOG_CLI((BSL_META("Freeing orphaned ruleset: 0x%x\n"), uOrphRs));
#endif
...
#ifdef RULESET_DEBUG
              LOG_CLI((BSL_META("Allocated RC0 dummy ruleset: 0x%x\n"), uRs));
#endif
...
#ifdef RULESET_DEBUG
                LOG_CLI((BSL_META("Replacing prealloted swap ruleset for Rc%d\n"), uRc));
#endif
                /* final rule set may not be full */
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("linkage after commits\n")));
  soc_sbx_g2p3_cls_ruleset_linkage_dump(pFe->unit, eDb, uLogProgNumber,
                                               pSchema->sType, uNewBank);
#endif
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("Linking Rs: 0x%x to Rs: 0x%x\n"), uRs, uNextRs));
#endif
...
#ifdef RULESET_DEBUG
  LOG_CLI((BSL_META("Get first last sType[%d], uRc[%d], first[%d], last[%d]\n"),
                ...
                pfFinalizeCb pfFinalize)
{
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
    sbStatus_t err = SB_OK;
    uint32 uRs, uPrevRs, uFirstRs, uPrevGroupRs, uNextGroupRs;
    uint32 uOrphRs = -1, uOrphRuleOffset;
    uint32 uRuleOffset, uJumpBlock, uJumpOffset;
    int32 nRuleIndex;
    uint32 uNewBank = 0, uTemp;
    uint8 uProgramNumber, uLogProgNumber = 0, bDummy = 0;
    uint32 uPhysicalProgramNumber;
    void * dummy;
    soc_sbx_g2p3_cls_ace_pattern_u_t dummy_pattern;
    uint32 uRc0StartAddress = -1, uRc1StartAddress = -1;
    pClsComp = pFe->pClsComp;
...
#ifdef RULESET_DEBUG
        LOG_CLI((BSL_META("Setting RC1 to program base address 0x%x,"),
                 uRc1StartAddress));
        LOG_CLI((BSL_META(" Phys Prog Num: 0x%x\n"), uPhysicalProgramNumber));
#endif
...
#ifdef RULESET_DEBUG
        LOG_CLI((BSL_META("Setting RC0 to program base address 0x%x,"),
                 uRc0StartAddress));
        LOG_CLI((BSL_META(" Phys Prog Num: 0x%x\n"), uPhysicalProgramNumber));
#endif
...
soc_sbx_g2p3_cls_update_mem_usage(soc_sbx_g2p3_state_t *pFe,
                                  ...
                                  soc_sbx_g2p3_cls_rule_db_e_t eDb)
{
    uint32 uLastRs, uAllocedRs, uRs;
    uint32 uRsUsed[G2P3_MAX_RC_INSTANCE], uMemUsed[G2P3_MAX_RC_INSTANCE];
    uint32 uRsTobeUsed[G2P3_MAX_RC_INSTANCE];
    uint32 uMemTobeUsed[G2P3_MAX_RC_INSTANCE];
    uint8 uLogProgNumber = 0, uRc;
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
    sbStatus_t err = SB_OK;
                            ...
                            int16   uGroupId)
{
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
    sbStatus_t err = SB_OK;
    uint32 uRs, uRc0FirstRs, uRc0LastRs, uRc1FirstRs;
    uint8 uProgramNumber;  /* uPhysicalProgramNumber */
    uint8 uLogProgNumber = 0;
    int32 nRuleIndex;
    uint32 uNewBank = 0;
    uint32 uRuleOffset;
...
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("Allocated RC1 dummy ruleset - pushing group to RC1: 0x%x\n"),
             uRs));
#endif
...
soc_sbx_g2p3_cls_commit_push_to_rc1(soc_sbx_g2p3_state_t *pFe,
                            ...
                            pfFinalizeCb pfFinalize)
{
    uint32 uRs, uRc0FirstRs, uRc0LastRs, uRc0GroupRsCount = 0;
    uint32 uRc0FreeRsCount, uRc1FreeRsCount, uNumRs, uFirstRs;
    uint32 uRcInstance = 0;
    int16 uGroupId, uNextGroupId, index;
    uint8 uLogProgNumber = 0, uProgramNumber;
    int8 uAllocCondition = 0;
    int8 iRcPrefNext = -1;
    sbStatus_t err = SB_OK;
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
...
soc_sbx_g2p3_cls_commit_push_to_rc0(soc_sbx_g2p3_state_t *pFe,
                            ...
                            pfFinalizeCb pfFinalize)
{
    uint32 uRs, uRc1FirstRs, uRc1LastRs, uRc1GroupRsCount = 0;
    uint32 uRc0FreeRsCount, uRc1FreeRsCount, uFirstRs, uAllocedRs;
    uint32 uRcInstance = 1;
    int32 uNeededRs, uRsToStayInRc1, uNumRs;
    int16 uGroupId, uNextGroupId = -1, index, uThisGroupIndex;
    uint8 uLogProgNumber = 0, uProgramNumber;
    int8 uAllocCondition = 0;
    int8 iRcPrefNext = -1;
    int8 nRuleIndex;
...
soc_sbx_g2p3_cls_ruleset_alloc(soc_sbx_g2p3_state_t *pFe,
                               ...
                               int8  *uAllocCondition)
{
  uint32 uCount = uNumRs, uBankedMode;
  soc_sbx_g2p3_cls_t *pClsComp = NULL;
  soc_sbx_g2p3_cls_schema_enum_t sType;
  soc_sbx_g2p3_cls_ruleset_t *pRsDb = NULL;
  uint32 uRs;
  uint32 uRc0LastRs = -1, uRc0NextRs = -1, uRc1FirstRs = -1, uTempRs;
  uint32 uFreeRsCount = 0;
  /* uint32 uFirstRs; */
  uint32 uLastRs, uNextRs, uPrevRs, uGroupLastRs, uDummy;
  uint32 uNextGroupFirstRs = -1, uPrevGroupLastRs = -1, uStartRs;
  int32 uFreeRsMemCountRc0 = 0, uFreeRsMemCountRc1 = 0, uRc;
  int32 uFreeRsCountRc0 = 0, uFreeRsCountRc1 = 0, uFreeCount ;
  int result;
  sbStatus_t err = SB_OK;
  ...
  *iRcPrefNext = -1;
  ...
  if (uGroupLastRs != -1) {
    ...
    LOG_CLI((BSL_META("Ruleset_alloc: Last ruleset for this group (%d) :0x%x\n"),
  ...
  if (uGroupLastRs == -1) {
#ifdef RULESET_ALLOC_DEBUG
    LOG_CLI((BSL_META("Ruleset_alloc: No ruleset for this group (%d) found\n"),
             uGroupId));
#endif
      /* get current last ruleset in program */
      err = soc_sbx_g2p3_cls_ruleset_get_last(pFe,
  ...
  if (iRcPref == 0) {
          ...
          /* there is some space in Rc1, we can push some Rs to Rc1
             and manage */
              ...
              /* this Group is at end of RC0, its easy to add RS in Rc1 */
                  *uAllocCondition = 1;
#ifdef RULESET_ALLOC_DEBUG
                  LOG_CLI((BSL_META("Ruleset_alloc: Group is at end of Rc0")));
                  LOG_CLI((BSL_META("), should push rulesets to Rc1\n"));
#endif
                  return SB_OK;
              } else {
...
#ifdef RULESET_ALLOC_DEBUG
                  LOG_CLI((BSL_META("Ruleset_alloc: Group in Rc0")));
                  LOG_CLI((BSL_META("), should push rulesets to Rc1\n"));
#endif
                  *uAllocCondition = 2;
                  return SB_OK;
...
#ifdef RULESET_ALLOC_DEBUG
          LOG_CLI((BSL_META("Ruleset_alloc: Resource not available\n")));
#endif
          return SOC_E_RESOURCE;
      }
  } else if (iRcPref == 1) {
      if (uFreeRsCountRc1 < uNumRs) {
          ...
          /* there is some space in RC0, we can push some Rs to RC0
             and manage */
#ifdef RULESET_ALLOC_DEBUG
             LOG_CLI((BSL_META("Ruleset_alloc: should push rulesets to Rc0\n")));
#endif
             *uAllocCondition = 3;
             return SB_OK;
          }
...
#ifdef RULESET_ALLOC_DEBUG
          LOG_CLI((BSL_META("Ruleset_alloc: RC preference is 0, should be 1\n")));
#endif
          return -1;
        }
        uPrevRs = uRc0LastRs;
...
#ifdef RULESET_ALLOC_DEBUG
          LOG_CLI((BSL_META("Ruleset_alloc: RC preference is 1, should be 0\n")));
#endif
          return -1;
        }
...
#ifdef RULESET_DEBUG
        LOG_CLI((BSL_META("ALLOC 0x%x\n"), uRs));
#endif
#if 0
        LOG_CLI((BSL_META("AVL tree prior to MALLOC of RS: 0x%x addr: 0x%x size: 0x%x\n"), uRs, pRsDb[uRs].uBaseAddress, pSchema->uInstCount));
...
#ifdef RULESET_ALLOC_DEBUG
        LOG_CLI((BSL_META("Ruleset_alloc: PrevRs:0x%x AllocRs:0x%x NextRs:0x%x\n"),
                 uPrevRs, uRs, uNextRs));
#endif
...
{
    sbStatus_t err = SB_OK;
    uint32 uLastRs, uOldRs, uAllocedRs, uRs, uPrevRs = -1;
    /* uint32 uNextRs; */
    uint32 uNewBank = 0, uTemp;
    uint32 uOrphRs = -1, uOrphRuleOffset;
    uint32 uRuleOffset, uRulesInSet;
    int32 nRuleIndex, nRs;
    uint8 uRc;
    uint8 uNumRs;
    int16 uGroupId;
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
    ...
    adq_t *pRuleElem = NULL;
    adq_t *pRuleElemCache[G2P3_MAX_RC_INSTANCE];
    uint32 uRuleCountCache[G2P3_MAX_RC_INSTANCE];
...
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("Committing group %d\n"), uGroupId));
#endif
    ...
    *uFirstRs = uRs;
    ...
    for (uRc = 0; uRc < G2P3_MAX_RC_INSTANCE; uRc++) {
        ...
        if (pClsComp->isSwapRsUsed[uRc] == 1) {
            ...
            pClsComp->sRsDb[uOldRs].uCommitted = 0;
#ifdef RULESET_DEBUG
            LOG_CLI((BSL_META("Swap Ruleset for RC%d freed: ruleset %d\n"),
                     uRc, uOldRs));
#endif
        }
    }
...
#ifdef RULESET_DEBUG
        LOG_CLI((BSL_META("using preallocated swap Rs 0x%x\n"), uRs));
#endif
...
#endif 
#ifdef RULESET_DEBUG
      LOG_CLI((BSL_META("ALLOC FOR SWAP 0x%x (%d)\n"), uRs, pRsDb[uRs].uBaseAddress));
#endif
      pRsDb[uRs].uAlloc = 1;
      *pOffset = pRsDb[uRs].uRuleOffset;
      *pRs = uRs;
...
#ifdef RULESET_DEBUG
    LOG_CLI((BSL_META("FREEING RC%d 0x%x (%d)\n"), pRs->uRcInstance, uRs, 
             pRs->uBaseAddress));
#endif
#ifdef DYNAMIC_MEMMGR
...
int soc_sbx_g2p3_cls_ruleset_free_count_get(soc_sbx_g2p3_state_t *pFe,
                                            ...
                                            uint32 *pRc1FreeRsCount)
{
    int32 uRc0FreeRsMemCount, uRc1FreeRsMemCount;
    uint32 uRc0FreeRsCount = 0, uRc1FreeRsCount = 0;
    uint32 uRs;
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
...
int soc_sbx_g2p3_cls_ruleset_get_free(soc_sbx_g2p3_state_t *pFe, 
                                      uint32 *pTotalRs)
{
    uint32 uRs, uTotalRs=0, uMaxRs=0;
    soc_sbx_g2p3_cls_ruleset_t *pRsDb = NULL;
    soc_sbx_g2p3_cls_schema_t *pSchema = NULL;
...
soc_sbx_g2p3_cls_group_ruleset_identify( soc_sbx_g2p3_state_t *pFe,
                                         ...
                                         uint32 uLastRs)
{
    int16 uGroupIndex = uGroupCount - 1;
    uint32 uRs, uPrevRs = -1;
    /* uint8 uRc0EndGroup = -1; */
    uint8 uRc1StartGroup = -1;
    soc_sbx_g2p3_cls_ruleset_t *pRsDb = NULL;
    soc_sbx_g2p3_cls_t *pClsComp = NULL;
    int8 iRc;
...
#ifdef RULESET_DEBUG
            LOG_CLI((BSL_META("GroupIndex: %d GroupId : %d  iRc: %d\n"), uGroupIndex,
                                      ...
                                      uint32 uBank)
{
  ...
  LOG_CLI((BSL_META("Program 0x%x\n--------------------------\n"), uProgram));
  LOG_CLI((BSL_META("uRs (RuleOffset) (BaseAddress-Size)(Group)\n")));
  while (uRs != -1) {
    LOG_CLI((BSL_META("0x%x (0x%x) (0x%x-0x%x) (G%d) <--"), uRs, pRsDb[uRs].uRuleOffset, pRsDb[uRs].uBaseAddress, pRsDb[uRs].uSize, pRsDb[uRs].uGroupId));
    uRs = pRsDb[uRs].uPrevRs;
  }
  LOG_CLI((BSL_META("\n")));
#ifdef DYNAMIC_MEMMGR
...
soc_sbx_g2p3_cls_wb_init_c2(soc_sbx_g2p3_state_t *pFe, void **wbOut)
{
    uint32 i, p, dummy, uData, uOffset;
    uint32 j, uLength, delta;
    ...
    *wbOut = wbRc;
    ...
    for (i=0; i < G2P3_MAX_RC_INSTANCE; i++) {
        ...
        status = thin_malloc(0, SB_ALLOC_CLS_DMA,
                             uLength * sizeof(uint32), (void*)&wbRc[i].dmaBuffer, &wbRc[i].dmaHandle);
        if (status != SB_OK) {
            return status;
        }
