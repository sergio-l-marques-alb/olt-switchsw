...
#endif
    int dev;
    uint16 peer_dev_id, peer_drv_dev_id;
    uint8  peer_rev_id, peer_drv_rev_id;
...
/* MCM added bm9600 section */
    ...
    case BCM88020_DEVICE_ID:
        si->chip_type = SOC_INFO_CHIP_TYPE_FE2000;
        si->modid_max = 31;
    ...
    case BCM88025_DEVICE_ID:
        si->chip_type = SOC_INFO_CHIP_TYPE_FE2000XT;
        si->modid_max = 31;
    ...
    if (soc_property_get(unit, spn_HOST_MEMORY_TABLE_SHARE, FALSE)) {
    	for (dev = 0; dev < soc_ndev; dev++) {
    	    if (SOC_NDEV_IDX2DEV(dev) == unit) {
        		/* ignore itself */
        		continue;
    	    }
    	    if (soc_attached(SOC_NDEV_IDX2DEV(dev)) && 
    		(SOC_CONTROL(SOC_NDEV_IDX2DEV(dev))->soc_flags & SOC_F_INITED)) {
        		/* device that has done init soc */
        		SOC_IF_ERROR_RETURN(soc_cm_get_id(SOC_NDEV_IDX2DEV(dev), &peer_dev_id, &peer_rev_id));
        		SOC_IF_ERROR_RETURN(soc_cm_get_id_driver(peer_dev_id, peer_rev_id,
        							 &peer_drv_dev_id, &peer_drv_rev_id));

        		/* device with matching device id and driver device id */
        		if ((peer_dev_id == dev_id) && (peer_drv_dev_id == drv_dev_id)) {
        		    /* elect the device to be master of this unit.
        		     * update master unit database
        		     */
        		    sbx->master_unit = SOC_NDEV_IDX2DEV(dev);
        		    SOC_SBX_CONTROL(SOC_NDEV_IDX2DEV(dev))->slave_units[SOC_SBX_CONTROL(SOC_NDEV_IDX2DEV(dev))->num_slaves] = unit;
        		    SOC_SBX_CONTROL(SOC_NDEV_IDX2DEV(dev))->num_slaves++;
        		    break;
        		}
    	    }
    	}
...
soc_sbx_attach(int unit)
{
	...
	/* if either table or slam dma is enabled, create mutex for all sbus dma channels
	 * NOTE: tslamDmaMutex and tdmaDmaMutex are still used as marker for 
	 * soc_mem_read_dmaable and soc_mem_write_dmaable
	 */
...
soc_sbx_init(int unit)
{
    ...
    if (SOC_IS_SBX_FE2KXT(unit)) {
      ...
      if (ucodestr != NULL && (sal_strcmp(ucodestr, "c1_g2p3_mplstp") == 0)) {
          SOC_SBX_CFG(unit)->mplstp_ena = 1;
      } else {
          SOC_SBX_CFG(unit)->mplstp_ena = 0;
      }
