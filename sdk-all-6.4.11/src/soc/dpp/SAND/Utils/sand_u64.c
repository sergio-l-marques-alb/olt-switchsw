...
/* $Id: sand_u64.c,v 1.9 Broadcom SDK $
 * utility function to be use by 'soc_sand_u64_device_u64_long_high'
 */
STATIC void
...
*  03/03/2004
*FUNCTION:
*  Devide SOC_SAND_U64 with uint32 and load result into uint32.
...
{  /*  Computes quotient result = x / y, remainder = x mod y
  ...
  */
  uu[1] = SOC_SAND_HIHALF(uu[0]);  /* (0u3) */
  uu[0] = SOC_SAND_TOHIGH(SOC_SAND_LOHALF(uu[0])) | u1;  /* (u2u1) */
  soc_sand_u64_device_u64_long_high_inner(uu, qhat, v1, v0);
  if (SOC_SAND_HIHALF(uu[1]) != 0)
  {  /* Add back */
    qhat--;
  ...
  */
  uu[1] = SOC_SAND_HIHALF(uu[0]);  /* (0u2) */
  uu[0] = SOC_SAND_TOHIGH(SOC_SAND_LOHALF(uu[0])) | u0;  /* (u1u0) */
  soc_sand_u64_device_u64_long_high_inner(uu, qhat, v1, v0);
  if (SOC_SAND_HIHALF(uu[1]) != 0)
  {  /* Add back */
    qhat--;
