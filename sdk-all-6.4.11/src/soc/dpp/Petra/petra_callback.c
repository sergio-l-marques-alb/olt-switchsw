/* $Id: petra_callback.c,v 1.7 Broadcom SDK $
...
uint32
  soc_petra_callback_user_callback_function(
  ...
  )
{
  ...
  SOC_PETRA_USER_CALLBACK_STRUCT
    *callback_struct = (SOC_PETRA_USER_CALLBACK_STRUCT*)buffer;
  SOC_PETRA_INTERRUPT_SINGLE_STRUCT
    value;

  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PETRA_CALLBACK_USER_CALLBACK_FUNCTION);

  unit = callback_struct->unit;

  res = soc_petra_interrupt_single_clear_unsafe(
          unit,
          soc_petra_interrupt_cause2monitored_cause(unit, callback_struct->interrupt_cause),
          &value
        );
  ...
  /* Clear interrupt */
  res = soc_petra_interrupt_clear_unsafe(
          unit,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_petra_callback_user_callback_function()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PETRA_CALLBACK_FUNCTION_REGISTER_UNSAFE);
  ...
  if (callback_exist)
  {
    SOC_SAND_SET_ERROR_CODE(SOC_PETRA_CALLBACK_ALREADY_REGISTERED_ERR, 20, exit);
  }

  res = soc_petra_interrupt_specific_cause_monitor_start_unsafe(
          unit,
          interrupt_cause,
          &monitored_cause
        );
  ...
  /* No need to Free. Will be freed upon the call of soc_petra_callback_function_unregister */
  SOC_PETRA_ALLOC(user_callback_struct, SOC_PETRA_USER_CALLBACK_STRUCT, 1);
  ...
  callback_func        = soc_petra_callback_user_callback_function;
  callback_struct_size = sizeof(SOC_PETRA_USER_CALLBACK_STRUCT);
  ...
  user_callback_struct = ((SOC_PETRA_USER_CALLBACK_STRUCT*)callback_struct);
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_petra_callback_function_register_unsafe()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PETRA_CALLBACK_ALL_FUNCTIONS_UNREGISTER_UNSAFE);
  ...
  res = soc_petra_interrupt_all_specific_cause_monitor_stop_unsafe(
          unit
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_petra_callback_all_functions_unregister_unsafe()",0,0);
}
