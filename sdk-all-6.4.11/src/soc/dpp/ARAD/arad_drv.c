...
soc_dpp_arad_str_prop_mc_tbl_mode(int unit)
{
    ...
    max_nof_egr_groups = max_nof_ingr_groups = SOC_IS_ARADPLUS_AND_BELOW(unit) ? ARAD_MULTICAST_TABLE_MODE : JER_MAX_NOF_MC_GROUPS;
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        /* We add one ingress group for the recycle interface traffic workaround */
...
soc_arad_default_config_meter_get(int unit)
{
	...
	if (sharing_mode >= ARAD_PP_NOF_MTR_SHARING_MODES) {
	...
	if (color_resolution_mode >= ARAD_PP_NOF_MTR_COLOR_RESOLUTION_MODES) {
...
soc_arad_default_config_l3_get(int unit)
{
    ...
    dpp_l3->actual_nof_routing_enablers_bm = soc_property_get(unit, spn_NUMBER_OF_INRIF_MAC_TERMINATION_COMBINATIONS, SOC_PPD_RIF_MAX_NOF_ROUTING_ENABLERS_BITMAPS);
    if(SOC_IS_JERICHO(unit)){
        ...
        } else if(dpp_l3->actual_nof_routing_enablers_bm > 8 && SOC_DPP_CONFIG(unit)->pp.ipmc_l3mcastl2_mode == 1) {/*4 bits + 1*/
            /*In case ipmc_l3mcastl2_mode SOC property is on the profile third bit will be allocated for
             * IPv4 MC L2 program selection and the number of MAC termination combination will be smaller in 1 bit*/
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Maximum INRIF_MAC_TERMINATION_COMBINATIONS is 8 when the  ipmc_l3mcastl2_mode SOC is enabled .")));
        } else if (dpp_l3->actual_nof_routing_enablers_bm > 8) {/*4 bits*/
            dpp_l3->intf_class_max_value = 0;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 0;
        } else if(dpp_l3->actual_nof_routing_enablers_bm > 4) {/*3 bits*/
            dpp_l3->intf_class_max_value = 1;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 1;
        } else if(SOC_DPP_CONFIG(unit)->l3.actual_nof_routing_enablers_bm > 2) {/*2 bits*/
            dpp_l3->intf_class_max_value = 3;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 2;
        } else {/*1 bit*/
            dpp_l3->intf_class_max_value = 7;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 3;
        }
    }
...
soc_arad_default_config_general_get(int unit)
{
    ...
    dpp_pp->egress_membership_mode = soc_property_get(unit, spn_EGRESS_MEMBERSHIP_MODE, 0);

    /* outer-vid existence classification mode */
    dpp_pp->outer_vid_existence_class_mode = 0;
    if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "outer_vid_existence_class_mode", 0))
    {
        dpp_pp->outer_vid_existence_class_mode = 1;
    }
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        if (SOC_IS_JERICHO(unit) && dpp_pp->oam_use_fifo_dma) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, 
                             (_BSL_SOCDNX_MSG("soc property \"OAMP_FIFO_DMA_ENABLE\" may only be set on Arad+")));
        }
        dpp_pp->oam_use_event_fifo_dma = soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_ENABLE, dpp_pp->oam_use_fifo_dma); 
        dpp_pp->oam_use_report_fifo_dma = soc_property_get(unit, spn_OAMP_FIFO_DMA_REPORT_INTERFACE_ENABLE, dpp_pp->oam_use_fifo_dma);
        ...
        dpp_pp->oam_dma_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_THRESHOLD, dpp_pp->oam_dma_event_threshold);
        dpp_pp->oam_dma_threshold = (dpp_pp->oam_dma_threshold > ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY) ?
            ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY : dpp_pp->oam_dma_threshold; 

        dpp_pp->oam_dma_report_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_REPORT_INTERFACE_THRESHOLD, dpp_pp->oam_dma_threshold);
        ...
        if (!SOC_IS_JERICHO(unit)) { /* Arad+ only */

            /* Due to documentation mistake, Arad+ should support the ..._event_interface... soc properties as well */

            dpp_pp->oam_use_fifo_dma = (dpp_pp->oam_use_fifo_dma) || soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_ENABLE, 0);

            dpp_pp->oam_dma_event_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_THRESHOLD, ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY+1);
            if (dpp_pp->oam_dma_event_threshold > ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY) {
                dpp_pp->oam_dma_event_threshold =  soc_property_get(unit, spn_OAMP_FIFO_DMA_EVENT_INTERFACE_THRESHOLD, ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY);
                dpp_pp->oam_dma_event_threshold = (dpp_pp->oam_dma_event_threshold > ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY)?
                                                ARAD_PP_OAM_LARGEST_NUMBER_OF_ENTRIES_FOR_DMA_HOST_MEMORY: dpp_pp->oam_dma_event_threshold;
            }
        }
    ...
    if ((propval)) {
        if (sal_strcmp(propval, "NONE") == 0) {
            ...
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("IPv6 unsupported for property value (\"%s\")."), propval));
            /* dpp_pp->micro_bfd_support = SOC_DPP_ARAD_MICRO_BFD_SUPPORT_IPv4; */
    ...
    if (SOC_IS_ARAD_B1_AND_BELOW(unit)){
        ...
        dpp_pp->bfd_ipv4_single_hop = soc_property_get(unit, spn_BFD_IPV4_SINGLE_HOP_EXTENDED,  0) ==2 ||
             dpp_pp->micro_bfd_support == SOC_DPP_ARAD_MICRO_BFD_SUPPORT_IPv4 ||   dpp_pp->micro_bfd_support == SOC_DPP_ARAD_MICRO_BFD_SUPPORT_IPv4_6; 
    }
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        else if (soc_property_get(unit, spn_OAM_STATISTICS_PER_MEP_ENABLED, 0)) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                                         (_BSL_SOCDNX_MSG(spn_OAM_STATISTICS_PER_MEP_ENABLED" currently is not supported for this device.")));
        }
    ...
    /* This feature currenly cannot co-exist with ERSPAN, since they both are using same resources */
    if(dpp_pp->bfd_extended_ipv4_src_ip && soc_property_get(unit, spn_BCM886XX_ERSPAN_TUNNEL_ENABLE, 1)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("BFD-Extended-IPv4-SIP cannot co-exist with Erspan tunnel.")));
    }
...
STATIC int
soc_arad_str_prop_stat_if_report_mode_get(int unit, int sif_port, SOC_TMC_STAT_IF_REPORT_MODE *stat_if_report_mode)
{
    char *propkey, *propval;
    ...
    propval = soc_property_suffix_num_str_get(unit, sif_port, propkey, "core");    
    *stat_if_report_mode = SOC_TMC_STAT_NOF_IF_REPORT_MODES;
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("When working with statistics interface, report mode must be configured \n")));
    }
...
STATIC int
soc_arad_prop_stat_if_reports_per_packet_get(int unit, int sif_port,SOC_TMC_STAT_IF_NOF_STAT_REPORTS_PER_PKT *nof_reports_per_pkt)
{
    uint32 propval;

    SOCDNX_INIT_FUNC_DEFS;   
    propval = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_REPORTS_PER_PACKET, "core", -1);
...
STATIC int
soc_arad_prop_stat_if_report_size_get(int unit, int sif_port, SOC_TMC_STAT_IF_REPORT_SIZE *report_size)
{
    char *propval, *propkey;
    SOCDNX_INIT_FUNC_DEFS;
    propkey = spn_STAT_IF_REPORT_SIZE;
    propval = soc_property_suffix_num_str_get(unit, sif_port, spn_STAT_IF_REPORT_SIZE, "core");
    if (propval)
    {
...
soc_arad_info_config(int unit) 
{
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        if (SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) {
            ...
            SOC_DPP_CONFIG(unit)->tm.ingress_mc_id_alloc_range_end = SOC_DPP_CONFIG(unit)->tm.nof_mc_ids - 1;
        } else {
        ...
        if (SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids) {
            ...
            SOC_DPP_CONFIG(unit)->tm.egress_mc_id_alloc_range_end = SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids - 1;
        } else {
    ...
    for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
        ...
        if (!(SOC_PORT_IS_ELK_INTERFACE(flags) || SOC_PORT_IS_STAT_INTERFACE(flags) || SOC_PORT_IS_SAME_QPAIR(flags))) {
            if (SOC_WARM_BOOT(unit)) 
            {
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_interface_type_get(unit, port_i, &interface));
          ...
          if (!dpp_pp->ipv6_with_rpf_2pass_exists)
          {
              ...
              if (dpp_pp->ipv6_fwd_pass2_rcy_local_port >= 0) 
              {
                ...
                else if ((header_type_in == SOC_TMC_PORT_HEADER_TYPE_INJECTED_2) || (header_type_in == SOC_TMC_PORT_HEADER_TYPE_INJECTED_2_PP)) 
                {
                    ...
                    if(((port_i % 256) == ARAD_OAMP_PORT_ID) && SOC_IS_JERICHO(unit) &&
                        soc_property_get(unit, spn_OAM_STATISTICS_PER_MEP_ENABLED, 0) &&
                ...
                if ((pp_port != -1) && !pp_port_configured[pp_port]) {
                    ...
                    if (header_type_out == SOC_TMC_PORT_HEADER_TYPE_CPU) {
                        rv = sw_state_access[unit].dpp.soc.arad.tm.logical_ports_info.is_tm_pph_present_enabled.set(unit,port_i,1);
                        SOCDNX_IF_ERR_EXIT(rv);
                    }
    ...
    if ((propval == NULL) || (sal_strcmp(propval, "SHARED") == 0)){
    ...
    */
    ...
    /* FQP/PQP new calendar building algorithm*/
    dpp_arad->init.fqp_pqp_new_algo_enable = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "enhanced_mux_cal", FALSE);
    ...
    if (SOC_IS_ARAD_B0_AND_ABOVE(unit)) {
        ...
        for(port=0; port<SOC_MAX_NUM_PORTS ; port++){
            dpp_arad->init.stat_if.stat_if_info[0].stat_if_port_enable[port] = soc_property_port_get(unit, port, spn_STAT_IF_ENABLE, FALSE);
    ...
    dpp_arad->init.stat_if.stat_if_info[0].rate = soc_property_get(unit, spn_STAT_IF_RATE, 0x0);
    ...
    if (dpp_arad->init.stat_if.stat_if_enable) {
        ...
        if (SOC_IS_JERICHO(unit))
        {
            ...
            SOCDNX_IF_ERR_EXIT(rv);
            if (dpp_arad->init.stat_if.core_mode == SOC_TMC_SIF_CORE_MODE_DEDICATED)
            {
                nof_sif_ports = 2;
            }
        }

        for (sif_port = 0; sif_port < nof_sif_ports; sif_port++)
        {
            rv = soc_arad_str_prop_stat_if_report_mode_get(unit, sif_port ,&(dpp_arad->init.stat_if.stat_if_info[sif_port].mode));
            SOCDNX_IF_ERR_EXIT(rv);

            dpp_arad->init.stat_if.stat_if_info[sif_port].is_idle_reports_present = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_IDLE_REPORTS_PRESENT, "core", TRUE);
            /* READ NUMBER OF REPORTS PER PACKET */
            
            if (SOC_IS_JERICHO(unit))
            {
                rv = soc_arad_prop_stat_if_reports_per_packet_get(unit, sif_port ,&(dpp_arad->init.stat_if.stat_if_info[sif_port].if_nof_reports_per_packet));
                SOCDNX_IF_ERR_EXIT(rv);
            }

            if (dpp_arad->init.stat_if.stat_if_info[sif_port].mode == SOC_TMC_STAT_IF_REPORT_MODE_BILLING || 
                dpp_arad->init.stat_if.stat_if_info[sif_port].mode == SOC_TMC_STAT_IF_REPORT_MODE_BILLING_INGRESS ||
                (SOC_IS_ARAD_B0_AND_ABOVE(unit) && SOC_TMC_STAT_IF_REPORT_MODE_BILLING_QUEUE_NUMBER == dpp_arad->init.stat_if.stat_if_info[sif_port].mode))
            {

                /* Jericho only soc properties */
                if (SOC_IS_JERICHO(unit))
                {
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_billing_ingress_queue_stamp = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_BILLING_INGRESS_QUEUE_STAMP_ENABLE, "core", FALSE);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_billing_ingress_drop_reason = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_BILLING_INGRESS_DROP_REASON_ENABLE, "core", FALSE);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_billing_filter_reports_ingress = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_BILLING_FILTER_REPORTS, "ingress_core", FALSE);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_billing_filter_reports_egress = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_BILLING_FILTER_REPORTS, "egress_core", FALSE);
                }

                /* Irrelevant soc properties for jericho*/
                else
                {
                    rv = soc_arad_str_prop_stat_if_billing_pkt_size_get(unit, &(dpp_arad->init.stat_if.stat_if_info[sif_port].if_pkt_size));
                    SOCDNX_IF_ERR_EXIT(rv);                  
                }

                dpp_arad->init.stat_if.stat_if_info[sif_port].if_report_mc_once = soc_property_get(unit, spn_STAT_IF_REPORT_MULTICAST_SINGLE_COPY, TRUE);                    
            }
            else if (dpp_arad->init.stat_if.stat_if_info[sif_port].mode == SOC_TMC_STAT_IF_REPORT_QSIZE)
            {
                /* Jericho only soc properties */
                if (SOC_IS_JERICHO(unit))
                {
                    rv = soc_arad_prop_stat_if_report_size_get(unit, sif_port ,&(dpp_arad->init.stat_if.stat_if_info[sif_port].report_size));
                    SOCDNX_IF_ERR_EXIT(rv);  
                    
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_qsize_queue_min = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SELECTIVE_REPORT_QUEUE_MIN, "core", ARAD_STAT_IF_REPORT_QSIZE_QUEUE_MIN); 
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_qsize_queue_max = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SELECTIVE_REPORT_QUEUE_MAX, "core", JER_STAT_IF_REPORT_QSIZE_QUEUE_MAX);              
                }
                /* Irrelevant soc properties for jericho*/
                else
                {
                ...
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_queue_min          = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_QUEUE_MIN, "core", ARAD_STAT_IF_REPORT_SCRUBBER_QUEUE_MIN);
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_queue_max          = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_QUEUE_MAX, "core", ARAD_STAT_IF_REPORT_SCRUBBER_QUEUE_MAX);
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_rate_min           = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_RATE_MIN, "core", ARAD_STAT_IF_REPORT_SCRUBBER_DISABLE);
                dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_rate_max           = soc_property_suffix_num_get(unit, sif_port, spn_STAT_IF_SCRUBBER_RATE_MAX, "core", ARAD_STAT_IF_REPORT_SCRUBBER_DISABLE);                
                
                for(ind = 0; ind < ARAD_STAT_IF_REPORT_THRESHOLD_LINES; ++ind) {
                    
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_buffer_descr_th[ind] = soc_property_port_suffix_num_get(unit, ind, sif_port, spn_STAT_IF_SCRUBBER_BUFFER_DESCR_TH,"core", ARAD_STAT_IF_REPORT_THRESHOLD_IGNORED);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_bdb_th[ind] = soc_property_port_suffix_num_get(unit, ind, sif_port, spn_STAT_IF_SCRUBBER_BDB_TH, "core", ARAD_STAT_IF_REPORT_THRESHOLD_IGNORED);
                    dpp_arad->init.stat_if.stat_if_info[sif_port].if_scrubber_uc_dram_buffer_th[ind]  = soc_property_port_suffix_num_get(unit, ind, sif_port, spn_STAT_IF_SCRUBBER_UC_DRAM_BUFFER_TH, "core", ARAD_STAT_IF_REPORT_THRESHOLD_IGNORED);
                }
            }
        }
        ...
        if (SOC_IS_ARAD_B0_AND_ABOVE(unit) && (!SOC_IS_JERICHO(unit))) {
            ...
            dpp_arad->init.stat_if.stat_if_info[0].if_qsize_queue_max          = soc_property_get(unit, spn_STAT_IF_SELECTIVE_REPORT_QUEUE_MIN, ARAD_STAT_IF_REPORT_QSIZE_QUEUE_MAX);
        }
#endif
    ...
    if (dpp_arad->init.pp_enable) {
      ...
      if (SOC_DPP_CONFIG(unit)->arad->pp_op_mode.mpls_info.mpls_coupling_enable && dpp_pp->evb_enable) {
      ...
      dpp_pp->extend_p2p_global_enable = FALSE;
      for (port_i = 0; port_i < SOC_MAX_NUM_PORTS ; ++port_i) {
          dpp_pp->extend_p2p_port_enable[port_i] = soc_property_port_get(unit, port_i, spn_BCM886XX_PORT_EXTEND_P2P, 0);
          if (dpp_pp->extend_p2p_port_enable[port_i]) {
              dpp_pp->extend_p2p_global_enable = TRUE;
          }
      } 
     ...
     */ 
      ...
      SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
          for (i=0; i<SOC_TMC_COSQ_TOTAL_FLOW_REGIONS; i++) {
...
int soc_arad_dma_mutex_init(int unit)
{
    ...
    /* Table DMA timeout, interrupt mode and mutex allocation according to SOC property configuration */
    soc->tableDmaMutex = NULL;        
    soc->tableDmaIntr = NULL;     
...
soc_arad_pp_init(int unit)
{
    ...
    {
      rv = chip_sim_em_init(unit, SOC_SAND_DEV_ARAD);
      SOCDNX_IF_ERR_EXIT(rv);
    }
        ...
        processor */
    {
        ...
        if (ipmc_enable && ipv6_enable && !SOC_IS_JERICHO(unit)) {
        ...
        for (port_i = 0; port_i < SOC_MAX_NUM_PORTS ; ++port_i) {
            if (dpp_pp->extend_p2p_port_enable[port_i]) {
                ...
                SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_pp_port_get(unit, port_i, &pp_port, &core));
                 ...
                 port_info.flags = SOC_PPD_PORT_IHB_PINFO_FLP_TBL;
        ...
        if (dpp_pp->pon_application_enable) {
            ...
            for (port_i = 0; port_i < SOC_MAX_NUM_PORTS ; ++port_i) {   
                ...
                if (IS_PON_PORT(unit, port_i)) {   
                    ...
                    port_info.flags = SOC_PPD_PORT_IHP_VTT_PP_PORT_CONFIG_TBL;
                    SOCDNX_SAND_IF_ERR_RETURN(soc_ppd_port_info_set(unit,core,pp_port,&port_info));   
                }
...
soc_arad_default_ofp_rates_set(int unit) {
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        SOC_PBMP_ITER(ports_bm, port_i) {
            ...
            if(SOC_PORT_IS_STAT_INTERFACE(flags) || SOC_PORT_IS_VIRTUAL_RCY_INTERFACE(flags) || SOC_PORT_IS_ELK_INTERFACE(flags)) {
              continue;
            }
        ...
        rv = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_single_port_rate_hw_set,(unit, 0, 0));  
        if(SOC_FAILURE(rv)) {
         SOCDNX_EXIT_WITH_ERR(rv, (_BSL_SOCDNX_MSG_STR( "ERROR: failed to set sch port rate to hw")));
        }

        /*Set single ports egq rates to hw*/
        rv = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_egq_single_port_rate_hw_set,(unit, core, first_port));
