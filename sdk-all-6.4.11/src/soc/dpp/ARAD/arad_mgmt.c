...
/* value was calculated in the following way:
    (4k for every 10G of the port type) * (amount of possible ports of that speed)
    10G  ports : (4 * 1024 * 1)  * (60) = 245760
    40G  ports : (4 * 1024 * 4)  * (12) = 196608
    100G ports : (4 * 1024 * 10) * (6)  = 245760
    200G ports : (4 * 1024 * 20) * (3)  = 245760
    biggest value was chosen as default */
#define ARAD_MGMT_IQM_OCB_PRM_DEFAULT_SIZE          (245760)
/* same as default size only devided by common ocb buff size (256) */
#define ARAD_MGMT_IQM_OCB_PRM_DEFAULT_BUFF_SIZE     (960)
  ...
  )
{
  ...
  *all_down = TRUE;
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 1110, exit, READ_RTP_LINK_ACTIVE_MASKr(unit, &rtp_mask));
    ...
    )
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT( arad_mgmt_ocb_prm_value_to_field_value( unit, ARAD_MGMT_IQM_OCB_PRM_DEFAULT_SIZE, &field_value));
    ocb_info->th_words[0] = field_value;
    ocb_info->th_words[1] = field_value;
    SOCDNX_SAND_IF_ERR_EXIT( arad_mgmt_ocb_prm_buff_size_value_to_field_value( unit, ARAD_MGMT_IQM_OCB_PRM_DEFAULT_BUFF_SIZE, &field_value));
    ocb_info->th_buffers[0] = field_value;
    ocb_info->th_buffers[1] = field_value;
  ...
  )
{
  ...
  res =  arad_mgmt_ocb_prm_buff_size_value_to_field_value(unit,
           info->th_buffers[0],
           &field_val
         );
  SOC_SAND_CHECK_FUNC_RESULT(res, 12, exit);
  ...
  res =  arad_mgmt_ocb_prm_value_to_field_value(unit,
           info->th_words[0],
           &field_val
         );
    ...
    SOC_SAND_IN  int       unit)
{
    ...
    if (!SOC_IS_ARDON(unit)) {
        ...
        if (soc_property_get(unit, spn_BCM886XX_ROO_ENABLE, 0) || soc_property_get(unit, spn_BCM886XX_ERSPAN_TUNNEL_ENABLE, 1)) {
            field = 0x0;
        }
        else {
        ...
        /* IGMP feature: enable compatible-mc after tunnel-termination and upgrade second stage parsing for IGMP */
         field = 0x0;
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 170, exit, READ_IHP_VTT_GENERAL_CONFIGS_1r(unit, REG_PORT_ANY, &reg_val));
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 180, exit, WRITE_IHP_VTT_GENERAL_CONFIGS_1r(unit, REG_PORT_ANY, reg_val));
...
int arad_mgmt_nof_block_instances(int unit, soc_block_types_t block_types, int *nof_block_instances) 
{
    ...
    switch(block_types[0]) {
        case SOC_BLK_CFC:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_CFC; 
            break;
        case SOC_BLK_CGM:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_CGM;
            break;
        case SOC_BLK_CLP:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_CLP;
            break;
        case SOC_BLK_CRPS:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_CRPS;
            break;
        case SOC_BLK_DRCA:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCA;
            break;
        case SOC_BLK_DRCB:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCB;
            break;
        case SOC_BLK_DRCBROADCAST:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCBROADCAST;
            break;
        case SOC_BLK_DRCC:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCC;
            break;
        case SOC_BLK_DRCD:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCD;
            break;
        case SOC_BLK_DRCE:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCE;
            break;
        case SOC_BLK_DRCF:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCF;
            break;
        case SOC_BLK_DRCG:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCG;
            break;
        case SOC_BLK_DRCH:
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_DRCH;
            break;
        case SOC_BLK_ECI:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_ECI;
            break;
        case SOC_BLK_EGQ:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_EGQ;
            break;
        case SOC_BLK_EPNI:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_EPNI;
            break;
        case SOC_BLK_FCR:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_FCR;
            break;
        case SOC_BLK_FCT:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_FCT;
            break;
        case SOC_BLK_FDR:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_FDR;
            break;
        case SOC_BLK_FDA:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_FDA;
            break;
        case SOC_BLK_FDT:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_FDT;
            break;
        case SOC_BLK_FMAC:  
        ...
        case SOC_BLK_FSRD:    
            *nof_block_instances = SOC_DPP_DEFS_GET(unit, nof_instances_fsrd);
            break;
        case SOC_BLK_IDR:           
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IDR;
            break;            
        case SOC_BLK_IHB:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IHB; 
            break;
        case SOC_BLK_IHP:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IHP; 
            break;
        case SOC_BLK_IPS:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IPS; 
            break;
        case SOC_BLK_IPT:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IPT; 
            break;
        case SOC_BLK_IQM:     
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IQM; 
            break;
        case SOC_BLK_IRE:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IRE; 
            break;
        case SOC_BLK_IRR:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_IRR; 
            break;
        case SOC_BLK_MESH_TOPOLOGY:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_MESH_TOPOLOGY;                   
            break;
        case SOC_BLK_MMU:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_MMU;   
            break;
        case SOC_BLK_NBI:     
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_NBI;   
            break;
        case SOC_BLK_OAMP:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_OAMP;  
            break;
        case SOC_BLK_OCB:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_OCB;   
            break;
        case SOC_BLK_OLP:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_OLP;   
            break;
        case SOC_BLK_OTPC:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_OTPC;  
            break;
        case SOC_BLK_RTP:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_RTP;  
            break;
        case SOC_BLK_SCH:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_SCH;   
            break;
        case SOC_BLK_XLP:    
            *nof_block_instances = SOC_ARAD_NOF_INSTANCES_XLP;
            break;
        default:
    ...
    *avs_val = 0;
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        *avs_val = soc_reg_field_get(unit, ECI_AVS_STATUSr, reg_val, AVS_STATUS_VALUEf);
        ...
        SOCDNX_IF_ERR_EXIT(soc_reg32_get(unit, ECI_RESCAL_CTRLr, REG_PORT_ANY, 0, &reg_val));
        SHR_BITCOPY_RANGE(avs_val, 0, &reg_val, 10, 3);
    }
