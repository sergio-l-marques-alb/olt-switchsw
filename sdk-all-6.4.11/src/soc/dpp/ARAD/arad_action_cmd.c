...
#define ARAD_ACTION_CMD_SNOOP_MIRROR_TABLE_1_OFFSET(cmd_type)				 ( SNOOP_MIRROR_MACRO_CHOOSER(cmd_type, 16 - (SOC_DPP_IMP_DEFS_GET(unit,mirror_snoop_dest_tm_table_offset)), ((SOC_DPP_IMP_DEFS_GET(unit,mirror_snoop_dest_tm_table_offset)))))
/*
...
dpp_snoop_mirror_stamping_config_get(
   ...
   SOC_SAND_OUT  SOC_TMC_ACTION_CMD_SNOOP_MIRROR_STAMPING_INFO  *info
    ...
    if (cmnd_type == SOC_TMC_CMD_TYPE_SNOOP && 
        ...
        res = READ_IPT_STAMPING_FABRIC_HEADER_ENABLEr(unit, &data64);
        SOCDNX_IF_ERR_EXIT(res);
...
dpp_snoop_mirror_stamping_config_set(
   ...
   SOC_SAND_IN  SOC_TMC_ACTION_CMD_SNOOP_MIRROR_STAMPING_INFO  *info
    ...
    if (cmnd_type == SOC_TMC_CMD_TYPE_SNOOP) {
        ...
        res = WRITE_IPT_STAMPING_FABRIC_HEADER_ENABLEr(unit, data64);
        SOCDNX_IF_ERR_EXIT(res);
    }
  ...
  )
{
  ...
  res = arad_irr_snoop_mirror_table1_tbl_set_unsafe(
          unit,
          ARAD_ACTION_CMD_SNOOP_MIRROR_TABLE_1_OFFSET(cmd_type) + action_ndx,
          &snp_mrr_tbl1_data
        );
  ...
  )
{
  uint32
    ...
    if (SOC_IS_JERICHO(unit)) {
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 50,  exit, ARAD_REG_ACCESS_ERR ,dpp_snoop_mirror_stamping_config_set(unit, cmd_type, action_ndx, &info->stamping));
    }
  ...
  )
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        res = dpp_snoop_mirror_stamping_config_get(unit, cmd_type, action_ndx, &info->stamping); 
