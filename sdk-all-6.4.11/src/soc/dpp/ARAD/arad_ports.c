  ...
  )
{
  ...
  SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {    
      ...
      for (pp_port = 0; pp_port < MAX_PORTS_IN_CORE ; ++pp_port)
      {
           ...
           */
          if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
            if (loop_ndx == 1 && SOC_DPP_CONFIG(unit)->pp.oam_ccm_2_fhei_eg_fake_injection_port == pp_port) {
                fld_val = pp_port;
            }
          }
  ...
  )
{
    ...
    if (PBMP_MEMBER(vendor_pp_port_bm, local_port)) {
    ...
    )
{
    ...
    SOC_PPD_LAG_MEMBER  *current_member;
    ...
    )
{
    ...
    SOC_PPD_LAG_INFO lag_info;
    SOC_PPD_LAG_MEMBER lag_member_sw_db_format;
    ...
    )
{
    ...
    SOC_PPD_LAG_INFO lag_info;
    SOC_PPD_LAG_MEMBER lag_member_sw_db_format;
    SOC_PPD_LAG_MEMBER *moved_lag_member;
    ...
    )
{
    ...
    uint32 memlock = 0;
    uint32 lag_is_mine_data[2];    
    SOC_PPD_LAG_INFO old_lag_info;
    const SOC_PPD_LAG_MEMBER *current_lag_member;
    ARAD_SYSPORT org_sys_port;
    ARAD_PP_EGQ_DSP_PTR_MAP_TBL_DATA egq_dsp_ptr_map_tbl_data;
    ARAD_IRR_LAG_TO_LAG_RANGE_TBL_DATA irr_lag_to_lag_range_tbl_data;
    ARAD_IRR_LAG_MAPPING_TBL_DATA irr_lag_mapping_tbl_data;
   ...
   )
{
    ...
    if (interface != SOC_PORT_IF_ERP) {
        ...
        SOCDNX_IF_ERR_EXIT(rv);
        rv = sw_state_access[unit].dpp.soc.arad.tm.logical_ports_info.is_tm_pph_present_enabled.get(unit, port, &is_tm_pph_present_enabled);
        SOCDNX_IF_ERR_EXIT(rv);
