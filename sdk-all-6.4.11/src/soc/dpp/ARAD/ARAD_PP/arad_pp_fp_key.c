       ...
       )
{
    const ARAD_PMF_CE ce_array_ipv4_match[6] = {
                ...
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_INITIAL_VID}             
               };
    const ARAD_PMF_CE ce_array_efp_match[8] = {
                ...
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_CMPRSD_INNER_VID},
                {1,   0, 0, 11, 0, 0, 0, SOC_PPC_FP_QUAL_CMPRSD_OUTER_VID},
                {1,   0, 0, 7,  0, 0, 0, SOC_PPC_FP_QUAL_VLAN_DOMAIN},
  ...
  for(alloc_place_ndx = start_place_ndx; alloc_place_ndx <= end_place_ndx; alloc_place_ndx++) {
      ...
      for(alloc_exact = 1; alloc_exact >= 0; --alloc_exact) {
          for (cycle = cycle_first; cycle <= cycle_last; ++cycle) {
              ...
              ce_rsrc |= arad_pp_fp_key_ce_rsrc_normalize(unit, stage, db_id, flags, 0 /*cur_rsrc1*/);
  ...
  )
{
  ...
  *progs_bmp_lcl = progs_bmp;
  ...
  while(prog_result != 0) {
     ...
     */
    cur_rsrc1_normalized = arad_pp_fp_key_ce_rsrc_normalize(unit, stage, db_id, flags, cur_rsrc1); 
    ...
    if(nof_cycles == 2) {
        ...
        cur_rsrc2_normalized = arad_pp_fp_key_ce_rsrc_normalize(unit, stage, db_id, flags, cur_rsrc2); 
          ...
          for (i = 0; i < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; i++) {
              ...
              if (is_exists) {              
                  ...
                  for (j = i; j < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX - nof_shared_quals; j++) {
                      if (j < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX) {/* this is done for coverity */
...
{    
...
#ifdef ARAD_PP_FP_DEBUG_PRINTS
    {
        ...
        for (i = 0; i < nof_ces_to_handle; i++) {
            LOG_CLI((BSL_META("   ""\tqual_type = %s, qual_lsb=%d\n"), ARAD_PP_FP_QUAL_TYPE_to_string(ce_array[i].qual_type), ce_array[i].qual_lsb));        
        }

        LOG_CLI((BSL_META("   ""\nInput qualifiers:\n")));
        for (i = 0; i < nof_ces_to_handle+5; i++) {
            LOG_CLI((BSL_META("   ""\tqual_type = %s (%d), val = %d\n"), ARAD_PP_FP_QUAL_TYPE_to_string(qual_vals[i].type), qual_vals[i].type, qual_vals[i].val.arr[0]));                
        }
    }
#endif
    ...
    if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY) {
        ...
        for (key_ndx = 0; key_ndx <= is_key_320b; key_ndx++) {
            ...
            LOG_DEBUG(BSL_LS_SOC_FP,(BSL_META_U(unit,"LSB  Buffer-->Key:  ce-id:%d, type:%s, curr_msb %d, nof_copy_bits %d val %X %X %X mask %X\n\r"),
                       ce_ids[ce_indx], ARAD_PP_FP_QUAL_TYPE_to_string(ce_array[ce_ids[ce_indx]].qual_type), curr_msb, nof_copy_bits, value_out[160 * key_ndx - 1], value_out[160 * key_ndx], value_out[160 * key_ndx + 1], mask_out[160 * key_ndx]));
            ...
            LOG_DEBUG(BSL_LS_SOC_FP,(BSL_META_U(unit,"MSB  Buffer-->Key:  ce-id:%d, type:%s, curr_msb %d, nof_copy_bits %d skip_lsb %d val %X mask %X\n\r"),
                       ce_ids[ce_indx], ARAD_PP_FP_QUAL_TYPE_to_string(ce_array[ce_ids[ce_indx]].qual_type), curr_msb, nof_copy_bits, skip_lsb, value_out[(skip_lsb+160 * key_ndx)/32], mask_out[(skip_lsb+ 160 * key_ndx)/32]));
    ...
    for (key_ndx = 0; key_ndx <= is_key_320b; key_ndx++) {
        ...
        for(ce_indx = 0; ce_indx < nof_ces_to_handle; ++ce_indx) {
            ...
            if(!qual_found) {
                ...
                LOG_DEBUG(BSL_LS_SOC_FP,
                          (BSL_META_U(unit," db_id %d  qual:%s  qual is masked. skip to %d (%d bits)\n\r"), db_id, ARAD_PP_FP_QUAL_TYPE_to_string(qual_type_lcl), buf_msb, sw_db_ce.msb + 1));
                continue;
            }
                ...
                if (direction == ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER) {
                    ...
                    LOG_DEBUG(BSL_LS_SOC_FP,
                              (BSL_META_U(unit,
                                          ...
                                          "qual_vals[qual_index].is_valid.arr:%08x %08x \n\r"), db_id, ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), copied_val, copied_maks,
                    ...
                    LOG_DEBUG(BSL_LS_SOC_FP,
                              (BSL_META_U(unit,
                                          ...
                                          "mask[buf_msb - nof_copy_bits %d]:%08x \n\r"), db_id, ARAD_PP_FP_QUAL_TYPE_to_string(sw_db_ce.qual_type), copied_val, copied_maks,
