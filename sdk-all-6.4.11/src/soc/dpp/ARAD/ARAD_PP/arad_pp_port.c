...
{ 
  ...
  if (!SOC_DPP_PP_ENABLE(unit) && (port_info->flags & ARAD_PP_PORT_IHP_PINFO_LLR_TBL)) {
  ...
  /************************************************************************/
  if (port_info->flags & ARAD_PP_PORT_IHP_PP_PORT_INFO_TBL) {
    res = arad_pp_ihp_pp_port_info_tbl_set_unsafe(unit, core_id, local_port_ndx, &pp_port_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  }

  if (port_info->flags & ARAD_PP_PORT_IHP_VTT_PP_PORT_CONFIG_TBL) {
  ...
  if (port_info->flags & ARAD_PP_PORT_IHP_PINFO_LLR_TBL) {
    res = arad_pp_ihp_pinfo_llr_tbl_set_unsafe(unit, core_id, local_port_ndx, &pinfo_llr_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 34, exit);
  }

  if (port_info->flags & ARAD_PP_PORT_IHB_PINFO_FLP_TBL) {
    res = arad_pp_ihb_pinfo_flp_tbl_set_unsafe(unit, core_id, local_port_ndx, &pinfo_flp_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 36, exit);
  }
  /* In orginal implementation,  EGQ_PCT.CGM_PORT_PROFILE always is updated 
     * when  set port information. After checking codes, there is no special 
     * BCM API to set it, so keep the orginal implementation and ignore hardware
     * tables configured flags checking
     */
  res = arad_egq_pct_tbl_set_unsafe(unit, core_id, local_port_ndx, &pct_tbl);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 38, exit);
  
  if (port_info->flags & ARAD_PP_PORT_EPNI_PP_PCT_TBL) {
    res = arad_pp_epni_pp_pct_tbl_set_unsafe(unit, core_id, local_port_ndx, &epni_pp_pct_tbl);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 40, exit);
  }

  if (port_info->flags & ARAD_PP_PORT_EGQ_PP_PPCT_TBL) {
    res = arad_pp_egq_pp_ppct_tbl_set_unsafe(unit, core_id, local_port_ndx, &egq_pp_ppct_tbl);
    SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  }

  if (port_info->flags & ARAD_PP_PORT_IHP_VTT_PP_PORT_VSI_PROFILE_TBL) {
