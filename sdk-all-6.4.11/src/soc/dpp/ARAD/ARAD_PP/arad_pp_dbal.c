...
STATIC uint32 arad_pp_dbal_next_available_key_get(int unit, ARAD_FP_DATABASE_STAGE stage, uint32 prog_id, SOC_DPP_DBAL_PHYSICAL_DB_TYPES db_type, SOC_DPP_DBAL_PROGRAM_KEYS* key_id);
...
STATIC uint32 arad_pp_dbal_entry_get_id_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 entry_id, ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8 *found);
...
STATIC uint32 
    arad_pp_dbal_next_available_key_get(int unit, ARAD_FP_DATABASE_STAGE stage, uint32 prog_id, SOC_DPP_DBAL_PHYSICAL_DB_TYPES db_type, SOC_DPP_DBAL_PROGRAM_KEYS* key_id)
{
    ...
    arad_pp_dbal_tcam_table_init(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, ARAD_FP_DATABASE_STAGE stage)
{
    ...
    if (table.additional_table_info == SOC_DPP_DBAL_ATI_NONE) {
        ...
        if (stage != ARAD_FP_DATABASE_STAGE_INGRESS_FLP) {
            ...
            if (tcam_db_id == ARAD_TCAM_MAX_NOF_LISTS) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("Invalid tcam_db_id for table_id = %d key_type = %d\n"), table_id, table.additional_table_info));
            }
        } else {
  ...
  arad_pp_dbal_table_clear_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
  ...
  while (!SOC_SAND_HASH_TABLE_ITER_IS_END(&iter))
  {
      ...
      sal_memcpy(&(entry_tbl_id), &(stream[ARAD_PP_ISEM_ACCESS_TCAM_KEY_LEN_BYTES*2]),  4);
    ...
    arad_pp_dbal_table_clear(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
		if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0) || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "support_large_tcam_hash_table", 1)) {
        		SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_clear_tcam(unit, table_id));
		}else{
        	SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("to enable use custom_feature_support_large_tcam_hash_table=1")));
		}
        break;
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_A:
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_dbal_table_clear - SEM_A not in use")));
        break;

    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_B:
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_dbal_table_clear - SEM_B not in use")));
        break;
        ...
        for (i = 0; i < nof_entries; i++) {
            ...
            if ((!is_table_mode) || (prefix == logical_prefix)) {
                ...
                if (!is_table_mode) {
                    ...
                    arad_pp_dbal_entry_dump(unit, ARAD_FP_DATABASE_STAGE_INGRESS_FLP, table_id, NULL, &(entry_key_array[ i*SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S]) );
                }
  ...
  arad_pp_dbal_table_print_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (is_table_mode) {
        LOG_CLI((BSL_META("\nPrinting all entries for table %s\n\n"), table.table_name));
        /*LOG_CLI((BSL_META("\tTotal entries in table = %d\n"),table.nof_entries_added_to_table ));*/
    }else{
    ...
    while (!SOC_SAND_HASH_TABLE_ITER_IS_END(&iter))
    {
        ...
        if(!is_table_mode){
            if (entry_tbl_id != 0) {                
                ...
                if ((table.is_table_initiated) && (table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM)) {
                    ...
                    LOG_CLI((BSL_META("strange table id received by hash\n")));
                }
            }            
        }
        if (table_id == entry_tbl_id)
        {
            ...
            if (found) {                
                ...
                LOG_CLI((BSL_META(" hit_bit=%d, priority=%d"), hit_bit, priority));                
        ...
        for (i = 0; i < nof_entries; i++) {
            ...
            if ((!is_table_mode) || (prefix == table.db_prefix)) {
                ...
                if (!is_table_mode) {
                    ...
                    arad_pp_dbal_entry_dump(unit, ARAD_FP_DATABASE_STAGE_INGRESS_VT, table_id, NULL, &(entry_key_array[ i*SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S]) );
                }
    ...
    arad_pp_dbal_table_print(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
		if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0) || soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "support_large_tcam_hash_table", 1)) {        
			...
			LOG_CLI((BSL_META("to enable use custom_feature_support_large_tcam_hash_table=1.\n")));
		}
        break;
...
uint32
    arad_pp_dbal_block_get_tcam(int unit, ARAD_PP_IP_ROUTING_TABLE_RANGE *block_range_key, SOC_DPP_DBAL_SW_TABLE_IDS table_id,
                           ARAD_PP_FP_QUAL_VAL *qual_vals_array, void* payload,  uint32 *nof_entries)
{
    uint8  is_vrf_mode = 0;
    uint32 data_indx, entry_tbl_id, priority;
...
uint32
    arad_pp_dbal_qualifier_to_instruction(int unit, ARAD_FP_DATABASE_STAGE stage, uint8 is_32b_ce, uint32 nof_bits, SOC_PPC_FP_QUAL_TYPE qual_type, int is_msb, uint32* ce_instr_encoding)
{
    uint8                           found = FALSE;    
    ARAD_PMF_CE_IRPP_QUALIFIER_INFO irpp_qual_info;    
    ...
    if (found) {
        ...
        if (nof_bits > ((is_32b_ce)? ARAD_PMF_LOW_LEVEL_NOF_BITS_CE_32B_MAX : ARAD_PMF_LOW_LEVEL_NOF_BITS_CE_16B_MAX)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG(" dbal_qualifier_to_instruction qual_type %s, qual nof_bits = %d, ce_size = %d"),
                                             ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), nof_bits,
        ...
        if (!found) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("dbal_qualifier_to_instruction Stage %s, qual_type %s, in msb %s, Number of bits %d, Qual lsb %d,"
                                                  ...
                                                  ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), is_ce_in_msb ? "TRUE" : "FALSE", 
...
uint32
    arad_pp_dbal_key_inst_set(int unit, int program_id, ARAD_FP_DATABASE_STAGE stage, int key_id, SOC_DPP_DBAL_QUAL_INFO qual_info, int is_msb, int ce_id)
{
    uint8                           found = FALSE;
    ...
    /* Same key at both lookup since it is much more simple */
        ...
        if (found) {
         ...
         /* ce_packet_header_info.offset = header_qual_info.lsb - qual_info.qual_offset - (qual_info.qual_nof_bits - 1); */
    ...
    for (curr_key_indx = 0; curr_key_indx < nof_valid_keys; curr_key_indx++){
        ...
        if (keys_to_table_id[curr_key_indx].key_id == SOC_DPP_DBAL_PROGRAM_NOF_KEYS) { /* in this case we will allocate the next available key */
            ...
            if (table.physical_db_type != SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal use of key for DB type")));
            }else{
    ...
    arad_pp_dbal_entry_payload_to_lem_payload(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, void* payload, ARAD_PP_LEM_ACCESS_PAYLOAD* lem_payload)
{
    ...
    for (i = 0; i < table.nof_actions; i++) {
        if (table.action[i] == SOC_PPC_FP_ACTION_TYPE_FLP_LEM_1ST_RESULT_0) {
            ...
            lem_payload->flags = ARAD_PP_LEM_ACCESS_ASD_RAW_DATA;
        }
    ...
    arad_pp_dbal_entry_add_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32 priority,  SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    sal_memcpy(&(stream[ARAD_PP_ISEM_ACCESS_TCAM_KEY_LEN_BYTES*2]), &(table_id), 4);
    ...
    arad_pp_dbal_entry_get_id_tcam(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 entry_id, ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8 *found)
{
    ...
    for(i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; ++i) {
        ARAD_PP_FP_QUAL_VAL_clear(&qual_vals[i]);
    }
...
uint32
  arad_pp_dbal_entry_lem_buffer_to_key(int unit, uint32 table_id, uint32 buffer[ARAD_PP_FP_TCAM_ENTRY_SIZE], ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    SOC_DPP_DBAL_TABLE_INFO table;
    int i, offset_fix = 0;
        ...
        if (qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES && qual_type != SOC_PPC_FP_QUAL_IRPP_ALL_ONES){
            ...
            if (user_qual_idx != SOC_PPC_FP_NOF_QUALS_PER_DB_MAX){  /*found user qualifier*/
                if  (qual_vals[user_qual_idx].is_valid.arr[0] == 0 && qual_vals[user_qual_idx].is_valid.arr[1] == 0){
                    ...
                    LOG_CLI((BSL_META("Error: valid qualifier %s after don't care qualifier\n"),SOC_PPC_FP_QUAL_TYPE_to_string(qual_vals[user_qual_idx].type)));
                }
            }
        }
    }
    ...
    arad_pp_dbal_phisycal_db_dump(int unit, int mode)
{
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
		if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "support_large_tcam_hash_table", 1)) {        
	...
	case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP:
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
        SOCDNX_SAND_IF_ERR_EXIT(arad_kbp_print_diag_all_entries(unit));
...
uint32
    arad_pp_dbal_entry_dump(int unit, ARAD_FP_DATABASE_STAGE stage, SOC_DPP_DBAL_SW_TABLE_IDS table_id, ARAD_PP_FP_QUAL_VAL in_qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 in_data[ARAD_PP_FP_TCAM_ENTRY_SIZE])
{
    SOC_DPP_DBAL_TABLE_INFO     table;
    ...
    for (i = 0; i < table.nof_qualifiers; i++) {
        ...
        LOG_CLI((BSL_META("%s=0x%x  "), (ARAD_PP_FP_QUAL_TYPE_to_string(in_qual_vals[i].type)), in_qual_vals[i].val.arr[0]));
    ...
    if (!no_buffer_print) {
        ...
        for (i = SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S - 1; i >= 0; i--) {
            if (i == SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S - 1) {
 ...
 */
uint32
    arad_pp_dbal_entry_input_validate(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, ARAD_PP_FP_QUAL_VAL in_qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    SOC_DPP_DBAL_TABLE_INFO table;
    ...
    for (i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; i++) {
        ...
        if ((j == table.nof_qualifiers) && (in_qual_vals[i].type != -1)) {
            LOG_CLI((BSL_META("WARNING:: qualifier %s (%d) not exists value = %08x\n"), ARAD_PP_FP_QUAL_TYPE_to_string(in_qual_vals[i].type), in_qual_vals[i].type, in_qual_vals[i].val.arr[0]));
            found_error = 1;
        }
    }
	...
	arad_pp_dbal_diag_entry_manage(int unit, SOC_DPP_DBAL_DIAG_ENTRY_MANAGE_MODE mode, int table_id, args_t* arg)
{
	...
	if (ARG_CNT(arg) < table.nof_qualifiers){
		...
		for (i = 0; i < table.nof_qualifiers; i++) {
			LOG_CLI((BSL_META("<%s (%d bits)> "), ARAD_PP_FP_QUAL_TYPE_to_string(table.qual_info[i].qual_type), table.qual_info[i].qual_nof_bits));
		}
		LOG_CLI((BSL_META("\n\n")));
		SOCDNX_FUNC_RETURN;
	}
    ...
    arad_pp_dbal_table_info_dump(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (table.db_prefix == DBAL_PREFIX_NOT_DEFINED) {
        ...
        case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
            if (table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM && table.additional_table_info == ARAD_PP_ISEM_ACCESS_KEY_TYPE_DUMMY) {
                LOG_CLI((BSL_META("  Dummy tcam table - using the key but not allocating tcam resources \n")));
            } else{
			    uint32 access_profile_id;                    
                LOG_CLI((BSL_META("  Table SW handle: %d\n"), (table.additional_table_info)));
    ...
    if (table.nof_actions > 0) {
        ...
        for (i = 0; i < table.nof_actions; i++) {
            LOG_CLI((BSL_META("  \tAction %s\n"), (ARAD_PP_FP_ACTION_TYPE_to_string(table.action[i]) )));
        }
    }
...
uint32
    arad_pp_dbal_qualifier_to_instruction_dump(int unit,ARAD_FP_DATABASE_STAGE stage, int is_32b_ce, int nof_bits, SOC_PPC_FP_QUAL_TYPE qual_type)
{
    uint32 ce_inst;
    ...
    LOG_CLI((BSL_META("qualifier = %s, calculated value = %d \n"), ARAD_PP_FP_QUAL_TYPE_to_string(qual_type), ce_inst));
...
uint32
    arad_pp_dbal_last_packet_signal_dump(int unit, int core_id, SOC_DPP_DBAL_SW_TABLE_IDS table_id, int lookup_number, ARAD_FP_DATABASE_STAGE  stage)
{
    uint32 ind;
    ...
    if ((table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS)) {
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)        
        LOG_CLI((BSL_META("\t calling KAPS diagnostics (both lookup are printed, you are interested in lookup = %d) \n\n"), lookup_number));
...
#define NUM_OF_STAGES  3
    ...
    for (stage = 0; stage < NUM_OF_STAGES; stage++) {
        ...
        if (prog_id[stage] == -1) { /* no program invoked continue to next stage */
            ...
            if (num_of_progs[stage] != 1) {
                LOG_CLI((BSL_META("  WARNING!!! more than one program was invoked at this stage. "
                                  "Info may be inccorect, to receive verified info please resend last packet and run diag command again\n\n")));
            }
        ...
        for (i = first_table_id[stage]; i < last_table_id[stage]; i++) {
            ...
            if ((table.additional_table_info == ARAD_PP_ISEM_ACCESS_KEY_TYPE_DUMMY) && (table.physical_db_type == SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM)) {
                continue;
            }
...
int arad_pp_dbal_flp_entry_test(uint32 unit, int table_id, uint8 is_tcam) {
	...
	ARAD_PP_LEM_ACCESS_PAYLOAD_clear(&payload_to_check); 
    ...
    if (is_tcam) {
        ...
        if ((payload_to_check.dest != value[0]) && (payload_to_check.asd != value[1])) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("VTT entry test failed table_id %d, found = %d, dest = %d, asd=%d value not valid"), table_id, found, payload_to_check.dest, payload_to_check.asd));
        }
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_entry_delete(unit, table_id, qual_vals, &success));
   ...
   arad_pp_dbal_flp_dynamic_tables_init(int unit)
{
    ...
    {
        action_types[ind] = SOC_PPC_FP_ACTION_TYPE_INVALID;
    }
    ...
    action_types[0] = SOC_PPC_FP_ACTION_TYPE_FLP_LEM_1ST_RESULT_0;
    action_types[1] = SOC_PPC_FP_ACTION_TYPE_FLP_LEM_1ST_RESULT_1;
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_actions_set(unit, lem_table_id, action_types));
...
int
    arad_pp_dbal_program_to_string(int unit, ARAD_FP_DATABASE_STAGE stage, int cam_line, const char**str, uint8* prog_id)
{
    soc_error_t rv;
    ...
    arad_pp_signal_mngr_signal_get(int unit, int core_id, SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE signal_id, uint32 val[24], int* size_of_signal_in_bits)
{
    ...
    if(sig_info.is_valid) {
        ...
        if(sig_info.signal_offset > 0) {
            offset_in_bytes = (sig_info.signal_offset + 7)/8;
            for(ind = 0; ind < num_of_bytes; ++ind) {
