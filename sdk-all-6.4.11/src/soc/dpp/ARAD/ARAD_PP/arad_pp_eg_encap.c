...
#define ARAD_PP_EG_ENCAP_PROFILE_NDX_MAX                         ((SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"push_profile_advanced_enable",0))? 15 : 7)
...
#define ARAD_PP_EG_ENCAP_PUSH_PROFILE_MAX                        ((SOC_IS_JERICHO_B0_AND_ABOVE(unit) && soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"push_profile_advanced_enable",0))? 15 : 7)
    ...
    SOC_SAND_IN  ARAD_PP_EG_ENCAP_PROTECTION_INFO       *protection_info)
{
  ...
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_PROTECTION_INFO, unit, protection_info, 10, exit);
    ...
    SOC_SAND_OUT ARAD_PP_EG_ENCAP_PROTECTION_INFO   *protection_info)
{
    ...
    if (is_egress_wide_entry && (ext_type == 0)) {
        ...
        ARAD_PP_EG_ENCAP_PROTECTION_INFO_clear(protection_info);
    }
  ...
  )
{
  ...
  ARAD_PP_EG_ENCAP_ENTRY_INFO               encap_entry_info[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
  uint32 next_eep_dummy[SOC_PPD_NOF_EG_ENCAP_EEP_TYPES];
  uint32 nof_entries;
  ...
  res = arad_pp_eg_encap_entry_get_unsafe(unit, SOC_PPD_EG_ENCAP_EEP_TYPE_TUNNEL_EEP, lif_eep_ndx,
  ...
  if ((nof_entries >= 1) && (encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_MPLS_ENCAP) &&
      ...
      tbl_data.mpls2_command = ARAD_PP_MPLS_COMMAND_TYPE_PUSH + encap_entry_info[0].entry_val.mpls_encap_info.tunnels[1].push_profile;
  } else if (SOC_IS_JERICHO_B0_AND_ABOVE(unit)) {
      /* In Jericho B0 and above, default label value is configurable */
      tbl_data.mpls2_label = soc_property_get(unit, spn_MPLS_ENCAP_INVALID_VALUE, 0);
  }
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(lif_eep_ndx, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_EG_ENCAP_LIF_EEP_NDX_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_PWE_INFO, unit, pwe_info, 20, exit);
  SOC_SAND_ERR_IF_ABOVE_NOF(next_eep, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_EG_ENCAP_NEXT_EEP_OUT_OF_RANGE_ERR, 30, exit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_MPLS_ENCAP_INFO, unit, mpls_encap_info, 20, exit);
  ...
  )
{
    ...
    /* Since we generally don't clear eedb entries, it's possible that the entry will be "dirty" with
       a lif from another phase. Ignore this case. */
    if (soc_sand_get_error_code_from_error_word(res) == ARAD_PP_EG_ENCAP_INCOMPATIBLE_TYPE_ERR) {
        *found = FALSE;
        ARAD_PP_DO_NOTHING_AND_EXIT;
    }
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_EG_ENCAP_PROTECTION_INFO_verify()",0,0);
}
  ...
  )
{
  ...
  {
    ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_MPLS_TUNNEL_INFO, unit, &(info->tunnels[ind]), 10, exit);
  }
  ...
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_PROTECTION_INFO, unit, &(info->protection_info), 30, exit);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_PWE_INFO, unit, &(info->pwe_info), 13, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_EG_ENCAP_POP_INFO, &(info->pop_info), 14, exit);
  ARAD_PP_STRUCT_VERIFY_UNIT(ARAD_PP_EG_ENCAP_MPLS_ENCAP_INFO, unit, &(info->mpls_encap_info), 15, exit);
