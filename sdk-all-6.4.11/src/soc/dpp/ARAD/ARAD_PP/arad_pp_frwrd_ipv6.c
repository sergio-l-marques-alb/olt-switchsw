    ...
    )
{
    ...
    if (use_tcam) {
        ...
        * without VRF value consideration hence the VRF is masked the priority lowered in case of a public entry.
        */
        ...
        res = arad_pp_dbal_entry_add(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE, qual_vals, 0/*priority*/,  &payload, success);
    }
#endif
    ...
    *hit_bit = 0;
    ...
    if (use_tcam) {
        ...
        */
        ...
        res = jer_pp_kaps_db_enabled(unit, vrf_ndx);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        res = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE, qual_vals, &payload, 0/*priority*/, NULL/*hit_bit*/, found);
	...
	for (d_word_ndx = 0; d_word_ndx < 4; d_word_ndx++) {
		for (bit_ndx = 0; bit_ndx < 32; bit_ndx++) {
			if (!SHR_BITGET(&mask->param[0].value[3-d_word_ndx], bit_ndx)) {
				route_key->prefix_len = d_word_ndx*32 + 16 - (bit_ndx&0x00000010) + 15 - (bit_ndx&0xF);
				found_lpm = 1;
			}
		}
		if (found_lpm) {
    ...
    )
{
    ...
    DBAL_QUAL_VAL_ENCODE_IPV6_DIP_LOW(&qual_vals[1], route_key->group.address, mask);
    DBAL_QUAL_VAL_ENCODE_IPV6_DIP_HIGH(&qual_vals[2], route_key->group.address, mask);
    /* Only use full or no mask for inrif, consistent with ipv4 inrif_valid */
    DBAL_QUAL_VAL_ENCODE_IN_RIF(&qual_vals[4], route_key->inrif.val, route_key->inrif.val > 0 ? SOC_SAND_U32_MAX : 0);

    if (use_tcam) {
        priority = SOC_SAND_U32_MAX - (uint32)soc_sand_bitstream_get_nof_on_bits(&route_key->inrif.mask, 1);
  ...
  )
{
  ...
  int current_location = ARAD_PP_FRWRD_IP_ROUTE_LOCATION_LPM;
  ...
  if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0))) {
      current_location = ARAD_PP_FRWRD_IP_ROUTE_LOCATION_TCAM;
  }
  ...
  DBAL_QUAL_VAL_ENCODE_VRF(&qual_vals_array[0], vrf_ndx, SOC_SAND_U32_MAX);

  /* Get the entries */
  route_ndx = *nof_entries;
  ...
  for(indx = route_ndx; indx < *nof_entries; indx++)
  {
      /* Decode the key */
  ...
  )
{
  ...
  else
  {
      ...
      res = arad_pp_frwrd_ipv6_uc_or_vpn_get_payload(
              unit,
              route_info,
              payload_data);
      SOC_SAND_CHECK_FUNC_RESULT(res, 5, exit);
  ...
  )
{
...
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030) */
  {
      ...
      for(indx = 0; indx < *nof_entries; ++indx)
      {
        ...
        res = arad_pp_frwrd_ipv6_uc_or_vpn_decode_payload(unit, &(actions[indx]), &(route_infos[indx]));
        SOC_SAND_CHECK_FUNC_RESULT(res, 70, exit);
  ...
  )
{
  ...
  if(ARAD_KBP_ENABLE_IPV6_MC) 
  {
       ...
       */
      uint32 vrf_ndx;
      for (vrf_ndx = block_range_key->start.payload.arr[1]; vrf_ndx < SOC_DPP_CONFIG(unit)->l3.max_nof_vrfs; vrf_ndx++) {
          res = arad_pp_frwrd_ipv6_kaps_dbal_route_get_block(
                    unit,
          ...
          SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
          /* reset block_range_key if we haven't reached entries_to_act/scan, unless we have reached max vrf */
          if (block_range_key->start.payload.arr[1] == SOC_SAND_UINT_MAX) {
              if (vrf_ndx == SOC_DPP_CONFIG(unit)->l3.max_nof_vrfs - 1) {
                  break;
              } else {
                  soc_sand_u64_clear(&block_range_key->start.payload);
              }
          } else {
              break;
          }
