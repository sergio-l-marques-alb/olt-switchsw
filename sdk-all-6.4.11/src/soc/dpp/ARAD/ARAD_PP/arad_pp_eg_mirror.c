  ...
  )
{
  ...
  for (i = 0; i <= DPP_MIRROR_ACTION_NDX_MAX; ++i) {
    ...
    if (i > 0) { /* for all valid outbound mirror profile , set highest strength for forward action packet to drop */      
      ...
      soc_mem_field32_set(unit, IHP_RECYCLE_COMMANDm, &value, CPU_TRAP_CODEf, ARAD_PP_TRAP_CODE_INTERNAL_LLR_ACCEPTABLE_FRAME_TYPE0); /* trap code SOC_PPC_TRAP_CODE_USER_DEFINED_27 dest=queue 128K-1 */  
  ...
  )
{
  ...
  if (ref_count) { /* If the vid was found */
    ...
    for (internal_vid_ndx = 0; internal_vid_ndx < ARAD_PP_EG_MIRROR_VID_DFLT_NDX; ++internal_vid_ndx) {
      res = arad_pp_sw_db_eg_mirror_port_vlan_is_exist_get(unit, core_id, out_port_ndx, internal_vid_ndx, &last_appear);
      SOC_SAND_CHECK_FUNC_RESULT(res, 90, exit);
      if (last_appear) { /* this vlan id index is used */
        break;
      }
    }
    if (!last_appear && !(config.mirror_command)) { /* no vlan id nor the default are mirrored for the port */
      int enabled = 1;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(local_port_ndx, SOC_DPP_DEFS_GET(unit, nof_logical_ports), ARAD_PP_PORT_OUT_OF_RANGE_ERR, 10, exit);
...
uint32
  arad_pp_eg_mirror_port_appl_set_unsafe(
    ...
    SOC_SAND_IN  ARAD_PP_PORT   local_port, /* Local port ID */
  ...
  )
{
  ...
  else
  {
    ...
    info->outbound_port_ndx = rcy_tm_port; /*soc_mem_field32_get(unit,  get_context_map_id(unit), &context, PORT_TERMINATION_CONTEXTf);    */
  }
