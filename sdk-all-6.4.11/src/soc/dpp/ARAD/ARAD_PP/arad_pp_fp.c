...
#define ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_MAX_LENGTH                       (14)
#define ARAD_PP_FP_DIRECT_TABLE_KAPS_NOF_LINES                            (1<<12)
...
ARAD_PP_FP_DIP_SIP_CLUSTER_INFO dip_sip_sharing_cluster[ARAD_PP_FP_NOF_DIP_SIP_CLUSTERS ] = 
  ...
  {{"IPv4",
    ...
    {PROG_FLP_IPV4UC, PROG_FLP_IPV4UC_RPF, PROG_FLP_IPV4COMPMC_WITH_RPF, -1, -1},
   ...
   {"IPv6",
    ...
    {PROG_FLP_IPV6UC, PROG_FLP_IPV6UC_RPF, PROG_FLP_IPV6MC, -1, -1},
...
void
  ...
  arad_pp_flp_app_to_prog_index_get((unit), PROG_FLP_IPV6UC_RPF, &prog_index);

  if (prog_index == prog_id) {
    prog_id = PROG_FLP_IPV6UC_RPF;
  }
...
void
  dip_sip_sharing_qual_info_get(int unit, int prog_id, ARAD_PP_FP_QUAL_TYPE qual_type, int* is_exists, ARAD_PP_FP_SHARED_QUAL_INFO* shared_qual_info){
...
uint32
  arad_pp_fp_dip_sip_sharing_is_sharing_enabled_for_db(int unit, ARAD_FP_DATABASE_STAGE stage, uint32 db_id_ndx, uint32* is_enabled, int* in_cluster_id){
    ...
    )
{
  ...
  {
    if ((qual_vals[qual_type_ndx].type != ARAD_PP_NOF_FP_QUAL_TYPES) && (qual_vals[qual_type_ndx].type != BCM_FIELD_ENTRY_INVALID))
    {
      is_type_found = FALSE;
  ...
  for (action_type_ndx = 0; action_type_ndx < ARAD_PP_FP_NOF_ACTIONS_PER_DB_MAX; ++action_type_ndx)
  {
    ...
    {
      is_type_found = FALSE;
      for (action_type_ndx2 = 0; (action_type_ndx2 < ARAD_PP_FP_NOF_ACTIONS_PER_DB_MAX) && (is_type_found == FALSE); ++action_type_ndx2)
      {
  ...
  )
{
  ...
  if (total_length == 0)
  {
    for (qual_ndx = 0; qual_ndx < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; qual_ndx++)
    {
      if ((info->qual_types[qual_ndx] == BCM_FIELD_ENTRY_INVALID) || ((info->qual_types[qual_ndx] == ARAD_PP_NOF_FP_QUAL_TYPES)))
      {
        continue;
      }
  ...
  )
{
    ...
    if (db_info->db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE && db_info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS ) 
    {
  ...
  arad_pp_fp_dbal_table_create(int unit, uint32 db_id_ndx)
{
  ...
  nof_qualifiers = 0;
  for(i = 0; ((i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX) && (fp_database_info.qual_types[i]!= ARAD_PP_NOF_FP_QUAL_TYPES) && (fp_database_info.qual_types[i]!= BCM_FIELD_ENTRY_INVALID) && (nof_qualifiers < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX - 1)); i++){
  ...
  )
{
 ...
 /* Check if there is a key-change action or qualifier */
   ...
   */
   ...
   if(info->db_type == SOC_PPC_FP_DB_TYPE_DIRECT_TABLE) {
       max_key_size = (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS) ? ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_MAX_LENGTH : SOC_DPP_DEFS_GET(unit, tcam_big_bank_key_nof_bits);
    ...
    */
       ...
       && !(info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS))
    {
        ...
        for (db_id_other_ndx = 0; db_id_other_ndx < ARAD_PP_FP_NOF_DBS; ++db_id_other_ndx)
        {
             ...
             */
            if(other_db_info.db_type != ARAD_PP_FP_DB_TYPE_DIRECT_EXTRACTION)
            {
   ...
   */
    ...
    if(info->db_type == ARAD_PP_FP_DB_TYPE_FLP)
    {
      ...
      */
      ...
      {
        ARAD_PP_FP_QUAL_TYPE new_qual_types[ARAD_PP_FP_NOF_QUALS_PER_DB_MAX];
        ...
        sal_memcpy(new_qual_types, info->qual_types, sizeof(ARAD_PP_FP_QUAL_TYPE) * ARAD_PP_FP_NOF_QUALS_PER_DB_MAX);
            ...
            if (dip_sip_sharing_enabled) {            
              for (i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; i++) {
                ...
                if (is_exists) {                                    
                  ...
                  for (j = i; j < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX - nof_shared_quals; j++) {
  ...
  )
{
   ...
   */
    ...
    case ARAD_PP_FP_DB_TYPE_DIRECT_TABLE:
      key_max_length =  (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS) ? ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_MAX_LENGTH : SOC_DPP_DEFS_GET(unit, tcam_big_bank_key_nof_bits);
      break;
...
exit_without_field_hardware_update:
     ...
     */
    if ((fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_TCAM) 
        || ((fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) && !( fp_database_info.flags &  SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS ))
        || (fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_EGRESS))
    {
  ...
  )
{
  ...
  for (qual_type_ndx = 0; qual_type_ndx < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX ; ++qual_type_ndx)
  {
    if ((info->qual_vals[qual_type_ndx].type == ARAD_PP_NOF_FP_QUAL_TYPES) || ((info->qual_vals[qual_type_ndx].type == BCM_FIELD_ENTRY_INVALID))){
      break;
    }
    ...
    dt_bit_max = (is_tcam_dt) ? ARAD_TCAM_KEY_LENGTH_DIRECT_TABLE_PER_BANK(unit, bank_ndx) : ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_MAX_LENGTH ;
...
{
    uint32  res = SOC_SAND_OK,
            entry_ndx_dt, entry_ndx_dt_max, entry_dt_first = 0, is_dt_bit_meaningful = 0, phase_ndx, bank_id = 0;
            ...
            if (is_tcam_dt) {
              if (phase_ndx == 0) {
                ...
                else if(SOC_IS_JERICHO(unit))
                {
                    ...
                    dma_offset = ( pmf_db_info.kaps_db_id << ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_MAX_LENGTH) + entry_ndx_dt ; 
                    res = jer_pp_kaps_dma_byffer_to_kaps_payload_buffer_encode(unit, action.value, data_bytes);
...
uint32 
    arad_pp_fp_ivl_entry_add(int unit, ARAD_FP_DATABASE_STAGE stage, uint32 db_id_ndx, uint32 entry_id_ndx, const  ARAD_PP_FP_ENTRY_INFO *info, uint32 result[ARAD_TCAM_ACTION_MAX_LEN], SOC_SAND_SUCCESS_FAILURE *success)
{
    uint32 res = 0;
  ...
  )
{
  ...
  if ((fp_database_info.db_type == ARAD_PP_FP_DB_TYPE_DIRECT_TABLE) && !( fp_database_info.flags &  SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_KAPS )) {
    if (0 == fp_entry.nof_db_entries) {
      /* allocate bank */
  ...
  if ( (!is_large_direct_extraction) && (!use_fes)) {
      ...
      *success = SOC_SAND_SUCCESS;
      ...
      for (pmf_pgm_ndx = pmf_pgm_ndx_min; pmf_pgm_ndx < pmf_pgm_ndx_max; ++pmf_pgm_ndx)
      {
          ...
          if ((info->qual_vals[0].type != ARAD_PP_NOF_FP_QUAL_TYPES) && (info->qual_vals[0].type != BCM_FIELD_ENTRY_INVALID)) {
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  case ARAD_PP_FP_CONTROL_TYPE_ACE_POINTER_ONLY:
  ...
  case ARAD_PP_FP_CONTROL_TYPE_ACE_POINTER_OUT_LIF:
      sal_memset(tbl_data, 0x0, ARAD_PP_FP_KEY_GEN_VAR_MAX_IN_LONGS * sizeof(uint32));
      /* Set / get the table field */
  ...
  case ARAD_PP_FP_CONTROL_TYPE_HDR_USER_DEF:
    ...
    if (control_ndx->db_id == 0) {
        ...
        if ((qual_type >= ARAD_PP_NOF_FP_QUAL_TYPES) || (qual_type == BCM_FIELD_ENTRY_INVALID)) {
  ...
  for (cycle_ndx = 0; cycle_ndx < ARAD_PMF_NOF_CYCLES; ++cycle_ndx)
  {
    for (fem_macro_ndx = 0; fem_macro_ndx < ARAD_PP_FP_NOF_MACROS; ++fem_macro_ndx)
    {
      ...
      if (fem_entry.is_for_entry == TRUE)
      {
        ...
        if ((qual_val.type != ARAD_PP_NOF_FP_QUAL_TYPES) && ((qual_val.type != BCM_FIELD_ENTRY_INVALID))) {
 ...
 *   SOC_SAND_IN  ARAD_PP_FP_ACTION_TYPE  in_action_type -
  ...
  )
{
  ...
  if (stage >= ARAD_NOF_FP_DATABASE_STAGES) {
    LOG_CLI(
      (BSL_META_U(unit,
        "%s(): Parameter stage (%d) is out of range. Max is %d\r\n"),
        __FUNCTION__,stage,ARAD_NOF_FP_DATABASE_STAGES-1
    )) ;
    res = SOC_E_PARAM ;
    FUNC_RESULT_SOC_PRINT(res) ;
    goto exit ;
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_fp_print_all_fems_for_stage()", 0, 0);
}
/*
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_fp_print_fes_info_for_stage()", 0, 0);
}
/*
 * }
 */
  ...
  )
{
  ...
  {
      if ((info->qual_types[ind] == ARAD_PP_NOF_FP_QUAL_TYPES) || (info->qual_types[ind] == BCM_FIELD_ENTRY_INVALID))
      {
        end_of_list = TRUE;
      }
    ...
    {
      SOC_SAND_ERR_IF_NOT_EQUALS_VALUE(info->qual_types[ind], BCM_FIELD_ENTRY_INVALID, ARAD_PP_FP_QUAL_TYPES_END_OF_LIST_ERR, 111, exit);
    }
  }
  ...
  )
{
  ...
  for (ind = 0; ind < ARAD_PP_FP_NOF_QUALS_PER_DB_MAX; ++ind)
  {
      if ((info->qual_vals[ind].type != ARAD_PP_NOF_FP_QUAL_TYPES) && (info->qual_vals[ind].type != BCM_FIELD_ENTRY_INVALID))
      {
  ...
  )
{
  ...
  if ((info->type == ARAD_PP_NOF_FP_QUAL_TYPES) || (info->type == BCM_FIELD_ENTRY_INVALID))
  {
  ...
  )
{
  ...
  case ARAD_PP_FP_CONTROL_TYPE_ACE_POINTER_OUT_LIF:
    SOC_SAND_ERR_IF_ABOVE_MAX(info->val[0], SOC_DPP_DEFS_GET(unit, nof_out_lifs) - 1, ARAD_PP_FP_VAL_OUT_OF_RANGE_ERR, 31, exit);
    break;
