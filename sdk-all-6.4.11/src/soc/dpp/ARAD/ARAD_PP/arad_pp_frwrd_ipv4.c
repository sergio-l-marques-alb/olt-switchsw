    ...
    )
{
  ...
  /* Set Key Mask */
  lcl_mask = (route_key->group == 0) ? 0 : SOC_SAND_BITS_MASK(31, 0);
...
{
    uint32 payload, res, dest_val = 0, prefix_len, priority;
    ARAD_PP_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0)) {
         ...
         */
        priority = SOC_SAND_NOF_BITS_IN_UINT32-prefix_len;
    }
    DBAL_QUAL_VAL_ENCODE_VRF(&qual_vals[0], route_key->vrf_ndx, SOC_SAND_U32_MAX);
    DBAL_QUAL_VAL_ENCODE_FWD_IPV4_DIP(&qual_vals[1], route_key->group, prefix_len);
    /*allowing default route for the VRF for ipv4mc*/
    if (route_key->group_prefix_len == 0) {
        dest_val = JER_PP_KAPS_DEFAULT_ENCODE(0);
    }
    DBAL_QUAL_VAL_ENCODE_FWD_IPV4_SIP(&qual_vals[2], route_key->source.ip_address, route_key->source.prefix_len);
    DBAL_QUAL_VAL_ENCODE_IN_RIF(&qual_vals[3], route_key->inrif, route_key->inrif_valid ? SOC_SAND_U32_MAX : 0);

    dest_val += route_info->dest_id.dest_val;
    if (route_info->dest_id.dest_type == SOC_SAND_PP_DEST_MULTICAST) {
        payload = JER_PP_KAPS_MC_ENCODE(dest_val); /* MC encoding in PP destination */
    }
    else if (route_info->dest_id.dest_type == SOC_SAND_PP_DEST_FEC) {
        payload = JER_PP_KAPS_FEC_ENCODE(dest_val); /* FEC encoding in PP destination */
    }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_pp_frwrd_ipv4_mc_rpf_kaps_dbal_route_add()",0,0);
}
    ...
    )
{
    ...
    if (route_key->group != 0) {/*allowing default route for the VRF for ipv4mc*/
        DBAL_QUAL_VAL_ENCODE_FWD_IPV4_DIP(&qual_vals[1], route_key->group, route_key->group_prefix_len);
    }else {
        /* No bad shift we do the shift only if the third parameter is not zero */
        /* coverity[large_shift:FALSE] */
        DBAL_QUAL_VAL_ENCODE_FWD_IPV4_DIP(&qual_vals[1], route_key->group, 0);
    }
    DBAL_QUAL_VAL_ENCODE_FWD_IPV4_SIP(&qual_vals[2], route_key->source.ip_address, route_key->source.prefix_len);
    DBAL_QUAL_VAL_ENCODE_IN_RIF(&qual_vals[3], route_key->inrif, route_key->inrif_valid ? SOC_SAND_U32_MAX : 0);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_pp_frwrd_ipv4_mc_rpf_kaps_dbal_route_get()",0,0);
