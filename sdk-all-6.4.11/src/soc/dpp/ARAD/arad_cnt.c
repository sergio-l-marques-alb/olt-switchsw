...
#define ARAD_EPNI_BASE_VAL_TM                                   (SOC_IS_JERICHO(unit) ? 0 : (0x8000))/* Disable bit 15 of EPNI TM */
...
/*the fields of ECI_FIFO_DMA_SEL */
CONST soc_field_t sel[4] =  {FIFO_DMA_0_SELf, FIFO_DMA_1_SELf, FIFO_DMA_2_SELf, FIFO_DMA_3_SELf}; 
/* } */
/*************
 * FUNCTIONS *
 *************/
  ...
  )
{
  ...
  /* verify that the MAX NOF COUNTER as defined in the code is not smaller than the one returned for specific unit. */
  /* otherwise, it may cause memory overwrite */
  ...
  if (SOC_IS_JERICHO(unit)) {
      /* enabling SIF - move out of soft reset*/
...
if (!SAL_BOOT_PLISIM ) {
  ...
  rv = _soc_mem_sbus_fifo_dma_start_memreg((unit), ch[dma_ndx], FALSE /*is_mem*/, 0, reg_crps_cache_rd_results, MEM_BLOCK_ANY, 0, ARAD_CNT_CACHE_LENGTH(unit), host_buff);
  if (SOC_FAILURE(rv)) {
  ...
  if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      /*this calculration is needed for Arad since there is one register with fields in it.*/
  ...
  if (!SOC_WARM_BOOT(unit) && SOC_IS_JERICHO(unit)) {
      /*Mark DMA channel as used for CRPS
        CMC0 FIFO DMA Channel source FIFO selection. 
        ...
        0x7 - PPDB CPU REPLY.
        */
      rv = soc_reg_above_64_field32_modify(unit, ECI_FIFO_DMA_SELr, REG_PORT_ANY, 0, sel[dma_ndx], dma_ndx);
      if (SOC_FAILURE(rv)) {
          SOC_SAND_SET_ERROR_CODE(ARAD_CNT_START_FIFO_DMA_ERR, 55, exit);   
      }
  }
}
#endif /* ARAD_CNT_USE_DMA */
 ...
 * Parameters:
 *      unit                  - (IN) Unit number.
 *      source                - (IN) Counter source to configure. Only the values SOC_TMC_CNT_SRC_TYPE_ING_PP and SOC_TMC_CNT_SRC_TYPE_EPNI are legal sources.
 *      command_id            - (IN) Defines the counter command to configure.
 *      lif_counting_mask     - (IN) Defines the mask of range IDs per stack ID level, for which the packet will be counted.
 *      lif_stack_to_count    - (IN) Issue a counter command with the LIF in lif_stack_to_count in the packet LIF stack.
 * Returns:
 *      SOC_E_xxx
 */
uint32 
       ...
       )
{
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        /*
         * each of the 4 LIFs is mapped to a 2b value according to range:
         *      2'b00 - lif invalid,
         ...
         *      table is acccessed with the upper 5b.
         *      The lower 3b is used to select CounterSource pair from a line.
         */
        ...
        /* Bits (2N+1):2N holds Counter0Source.Bits (2N+3):(2N+2) holds Counter1Source.*/
        ...
        for (counter_0_mode_is_lif = 0; counter_0_mode_is_lif < 2; counter_0_mode_is_lif++)
        {
            ...
            {
                /*Counter-0-Mode is LIF, Counter-1-Mode is LIF, LIF-0-Range , LIF-1-Range, LIF-2-Range.*/
                /* HW bug was which prevent from counting Lif4 was fixed. therefore, different handling */
 ...
 *      command_id            - (IN)  Defines the counter command to configure.
 *      lif_counting_mask     - (IN)  Defines the mask of range IDs per stack ID level, for which the packet will be counted.
 *      lif_stack_to_count    - (OUT) Issue a counter command with the LIF in lif_stack_to_count in the packet LIF stack.
 * Returns:
 *      SOC_E_xxx
 */
uint32 
...
{   
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        /*
         * each of the 4 LIFs is mapped to a 2b value according to range:
         *      2'b00 - lif invalid, 2'b01 - lif not in range, 2'b10 - lif in range 0, 2'b11 - lif in range 1
         *      8b conactanted value of all LIFs is constrcuted, ordered according to usage order, MSB to LSB (bits 7:6 are always VT0 LIF).
         *      table is acccessed with the upper 5b.
         *      the lower 3b is used to select CounterSource pair from a line.
         */
        ...
        rv = READ_EPNI_PP_COUNTER_TABLEm(unit, EPNI_BLOCK(unit, SOC_CORE_ALL), 0 /*pp_port_ndx are configured symmetrically*/, data);
        SOCDNX_IF_ERR_EXIT(rv);
 ...
 *      unit      - (IN) Unit number.
 *      source    - (IN) Counter source to configure. Only the values SOC_TMC_CNT_SRC_TYPE_ING_PP and SOC_TMC_CNT_SRC_TYPE_EPNI are legal sources.
 *      range_id  - (IN) The range to be configured. Only 0 and 1 are valid values.
 *      range     - (IN) The range configuration.
 * Returns:
 *      SOC_E_xxx
 */
uint32 
...
{   
    ...
    if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        /* 
         * in Arad Devices (not in Jericho) 
         * IHP_COUNTER_IN_LIF_RANGE_0r is IHP_REG_01CC
         *  COUNTER_IN_LIF_RANGE_0_LOW is FIELD_0_15
         *  COUNTER_IN_LIF_RANGE_0_HIGH is FIELD_16_31
         * IHP_COUNTER_IN_LIF_RANGE_1r is IHP_REG_01CD
         *  COUNTER_IN_LIF_RANGE_1_LOW is FIELD_0_15
         *  COUNTER_IN_LIF_RANGE_1_HIGH is FIELD_16_31
         */
        ...
        /* 
         * in Arad Devices (not in Jericho) 
         * IHP_COUNTER_IN_LIF_RANGE_0r is IHP_REG_01CD 
         *  COUNTER_IN_LIF_RANGE_0_LOW is FIELD_0_15
         *  COUNTER_IN_LIF_RANGE_0_HIGH is FIELD_16_31
         * IHP_COUNTER_IN_LIF_RANGE_1r is IHP_REG_01CE 
         *  COUNTER_IN_LIF_RANGE_1_LOW is FIELD_0_15
         *  COUNTER_IN_LIF_RANGE_1_HIGH is FIELD_16_31
         */
 ...
 *      source    - (IN)  Counter source to configure. Only the values SOC_TMC_CNT_SRC_TYPE_ING_PP and SOC_TMC_CNT_SRC_TYPE_EPNI are legal sources.
 *      range_id  - (IN)  The range to be configured. Only 0 and 1 are valid values.
 *      range     - (OUT) The range configuration.
 * Returns:
 *      SOC_E_xxx
 */
uint32 
...
/*
 * Function:
 *      arad_cnt_lif_counting_base_val_set
 * Purpose:
 *      Set Counting range for processor_ndx,
 *      The Counting range is calulated by base_val and we_val
 *  
 * Parameters:
 *      in int                       unit,
 *      in SOC_TMC_CNT_PROCESSOR_ID  processor_ndx,
 *      in SOC_TMC_CNT_COUNTERS_INFO *info
 * Returns:
 *      SOC_E_xxx
 */
uint32 
   ...
   )
{
    ...
    if (source == SOC_TMC_CNT_SRC_TYPE_EPNI || 
        ...
        if ((info->mode_eg.type == SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF) && 
            base_val >= SOC_DPP_DEFS_GET(unit, nof_local_lifs)) {
            LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "Invalid LIF base-value %d.\n"), base_val));
            SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
        }
        base_offset_field_val =  
            ((info->num_counters - ((base_val * info->custom_mode_params.set_size) % info->num_counters)) % info->num_counters /*in case base_val is zero the offset is zero*/) / info->custom_mode_params.set_size;
        base_counter_we_val = 
            ((base_val * info->custom_mode_params.set_size) / info->num_counters);
        if (source == SOC_TMC_CNT_SRC_TYPE_EPNI) {
            if (command_id == 0) {
                switch (info->mode_eg.type) {
                case ARAD_CNT_MODE_EG_TYPE_PMF:
                    rv = soc_reg_above_64_field32_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_0_BASEf, base_offset_field_val);
                    SOCDNX_IF_ERR_EXIT(rv);
                    break;
                case SOC_TMC_CNT_MODE_EG_TYPE_TM_PORT:
                case SOC_TMC_CNT_MODE_EG_TYPE_TM:
                    rv = soc_reg_above_64_field32_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_1_BASEf, ARAD_EPNI_BASE_VAL_TM); /*ignore base for TM commands*/
                    SOCDNX_IF_ERR_EXIT(rv);
                ...
                case ARAD_CNT_MODE_EG_TYPE_VSI:
                    ...
                    break;
                default:
                    LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "Invalid mode %s for source %s.\n"), SOC_TMC_CNT_MODE_EG_TYPE_to_string(info->mode_eg.type), SOC_TMC_CNT_SRC_TYPE_to_string(source)));
                    SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
                }
            } else {/*(command_id == 1)*/
                switch (info->mode_eg.type) {
                case ARAD_CNT_MODE_EG_TYPE_PMF:
                    rv = soc_reg_above_64_field32_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_0_BASEf, base_offset_field_val);
                    SOCDNX_IF_ERR_EXIT(rv);
                    break;
                case SOC_TMC_CNT_MODE_EG_TYPE_TM_PORT:
                case SOC_TMC_CNT_MODE_EG_TYPE_TM:
                    rv = soc_reg_above_64_field32_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_1_BASEf, ARAD_EPNI_BASE_VAL_TM); /*ignore base for TM commands*/
                    SOCDNX_IF_ERR_EXIT(rv);
                    base_offset_field_val = 0;
                    base_counter_we_val = 0;
                    break;
        ...
        if (SOC_IS_JERICHO(unit)) {
            rv = soc_reg_above_64_field32_modify(unit, CRPS_CRPS_CNT_SRC_CFGr, REG_PORT_ANY, processor_ndx, CRPS_N_CNT_BIT_NUMf, 
                                                 (base_offset_field_val == 0) ? (info->we_val + base_counter_we_val) : (info->we_val + base_counter_we_val + 1));
            SOCDNX_IF_ERR_EXIT(rv);
        }
    } else {
        if (source == SOC_TMC_CNT_SRC_TYPE_VOQ) {
            sal_memset(&crps_iqm_cmd, 0, sizeof(ARAD_CNT_CRPS_IQM_CMD));
            rv = arad_cnt_crps_iqm_cmd_get(unit, processor_ndx, command_id, &crps_iqm_cmd);
            SOCDNX_IF_ERR_EXIT(rv);
            ...
            /*Top Queue - the maximum possible.*/
      ...
      SOC_SAND_IN int                     command_id)
{
    ...
    if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        /* 
        Register EPNI_CFG_PP_LIF_COUNTER_MODE 
        filed CFG_PP_LIF_COUNTER_MODE 
            0 - Arad Compatible Mode. Take 16 lsb from lifs.
            1 - JerichoModeA. pointers = lifs. No additional data on msb.
            2 - JerichoModeB - counter pointer0 = lif17:0, pointer1 = lif15:0, msb = pp-port7:0
            3 - JerichoModeC - counter pointer0 = lif17:0, pointer1 = lif15:0, msb = pp-port5:0 sys-mc1:0
        */
...
#if ARAD_CNT_USE_DMA
if (!SAL_BOOT_PLISIM ) {
  res = arad_cnt_engine_to_fifo_dma_index(unit, proc_id, &dma_dnx, &cache_length);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);

  res = arad_cnt_dma_set_unsafe(unit, dma_dnx, cache_length);
  ...
  if (info->src_type == SOC_TMC_CNT_SRC_TYPE_EPNI){
      res = arad_cnt_epni_regs_set_unsafe(unit, info->src_type, info->src_core, info->mode_eg.type, info->command_id);
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 1201, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CRPS_CRPS_PREFETCH_TIMERS_CFGr, REG_PORT_ANY,  processor_ndx, CRPS_N_ALGORITHM_TMR_ENf,  !SOC_TMC_CNT_FORMAT_IS_WIDE(info->format)));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 1221, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CRPS_CRPS_PREFETCH_TIMERS_CFGr, REG_PORT_ANY,  processor_ndx, CRPS_N_SEQUENTIAL_TMR_ENf,  !SOC_TMC_CNT_FORMAT_IS_WIDE(info->format)));
  if (!SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 1222, exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, CRPS_CRPS_PREFETCH_TIMERS_CFGr, REG_PORT_ANY,  processor_ndx, CRPS_N_OPPORTUNISTIC_TMR_ENf, !SOC_TMC_CNT_FORMAT_IS_WIDE(info->format))); 
  }
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 1223, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CRPS_CRPS_GENERAL_CFGr, REG_PORT_ANY,  processor_ndx, CRPS_N_CACHE_READ_ENf,  !SOC_TMC_CNT_FORMAT_IS_WIDE(info->format)));
...
)
{
    ...
    if (format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS) {
        COMPILER_64_SET(*byte_cnt, 0, COMPILER_64_LO(field_val)); /*In packets and packet format the  bytes_cnt has the second value*/
  ...
  )
{
 ...
 uint64
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  12,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, rd_processor_read.reg_index, rd_processor_read.rd_req_type_f, ARAD_CNT_READ_DIRECT_READ));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  14,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, rd_processor_read.reg_index, rd_processor_read.rd_req_addr_f, counter_ndx));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, rd_processor_read.reg_index, rd_processor_read.rd_req_f,  0x1));
