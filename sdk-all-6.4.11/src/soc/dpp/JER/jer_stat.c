...
STATIC int 
soc_jer_nif_controlled_counter_get(int unit, int counter_id, int port, uint64* val)
{
    int rv;
    uint32 val_32;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    if(!IS_SFI_PORT(unit,port)) {
        ...
        case soc_jer_counters_rx_eth_stats_drop_events:
            if (!IS_IL_PORT(unit,port) && !IS_QSGMII_PORT(unit, port)) {
                SOC_DPP_ALLOW_WARMBOOT_WRITE(soc_jer_port_prd_drop_count_get(unit, port, &val_32), rv); 
                SOCDNX_IF_ERR_EXIT(rv);
...
soc_controlled_counter_t soc_jer_controlled_counter[] = {
	...
	},
    {
		...
		"TX snmp stats no errors",
        _SOC_CONTROLLED_COUNTER_FLAG_TX | _SOC_CONTROLLED_COUNTER_FLAG_HIGH | _SOC_CONTROLLED_COUNTER_FLAG_MAC | _SOC_CONTROLLED_COUNTER_FLAG_NIF,
		COUNTER_IDX_NOT_COLLECTED
	},
    {
        soc_jer_nif_controlled_counter_get,
...
soc_error_t 
soc_jer_stat_controlled_counter_enable_get(int unit, soc_port_t port, int index, int *enable) {
    ...
    *enable = 1;
    ...
    if (index == soc_jer_counters_ilkn_rx_pkt_counter || 
        index == soc_jer_counters_ilkn_tx_pkt_counter || 
        index == soc_jer_counters_ilkn_rx_byte_counter ||
        index == soc_jer_counters_ilkn_tx_byte_counter ||
        index == soc_jer_counters_ilkn_rx_err_pkt_counter ||
        index == soc_jer_counters_ilkn_tx_err_pkt_counter ) {
