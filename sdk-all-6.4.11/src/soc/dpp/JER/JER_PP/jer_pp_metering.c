  ...
  )
{
	...
	soc_mem_t
	  ...
	  eth_mtr_prfCfg_fields_mem = MTRPS_EM_MCDA_PRFCFG_SHARING_DISm;
	  ...
	  res = soc_mem_write(soc_sand_dev_id, eth_mtr_prfCfg_0_mem, MTRPS_EM_BLOCK(unit, core_id), 0, (void*)eth_mtr_prfCfg_above_64_val); 
	  SOCDNX_IF_ERR_EXIT(res);
	  res = soc_mem_write(soc_sand_dev_id, eth_mtr_prfCfg_1_mem, MTRPS_EM_BLOCK(unit, core_id), 0, (void*)eth_mtr_prfCfg_above_64_val); 
	  SOCDNX_IF_ERR_EXIT(res);
  ...
  )
{
  ...
  ARAD_IDR_ETHERNET_METER_PROFILES_TBL_DATA
	...
	SOCDNX_IF_ERR_EXIT(jer_pp_metering_init_eth_policer_default_config(unit,ARAD_SW_DB_CORE_0));
	...
	SOCDNX_IF_ERR_EXIT(jer_pp_metering_init_eth_policer_default_config(unit,ARAD_SW_DB_CORE_1));
  ...
  uint32
	...
	*/
	...
	if (init_bucket_level == 1) {
	    ...
	    soc_reg32_get(unit, MTRPS_EM_ENABLE_DYNAMIC_MEMORY_ACCESSr, core_id, 0, (void*)&write_access_enable);
		if (0 == write_access_enable) {
		res = soc_reg32_set(unit, MTRPS_EM_ENABLE_DYNAMIC_MEMORY_ACCESSr, core_id, 0, 1);
			SOCDNX_IF_ERR_EXIT(res);
		}
		...
		if (0 == write_access_enable) {
		res = soc_reg32_set(unit, MTRPS_EM_ENABLE_DYNAMIC_MEMORY_ACCESSr, core_id, 0, 0);
			SOCDNX_IF_ERR_EXIT(res);
		}
	...
	)
{
  ...
  uint32
	...
	eth_mtr_bck_lvl_mem			= MTRPS_EM_MCDB_DYNAMICm;
	...
	*/
	...
	if(glbl_info_enable == TRUE){
		...
		*/
		...
		res = soc_mem_write(soc_sand_dev_id, eth_mtr_prfCfg_0_mem, MTRPS_EM_BLOCK(unit, core_id), prfCfg_tbl_entry, (void*)eth_mtr_prfCfg_above_64_val); 
		SOCDNX_IF_ERR_EXIT(res);
		res = soc_mem_write(soc_sand_dev_id, eth_mtr_prfCfg_1_mem, MTRPS_EM_BLOCK(unit, core_id), prfCfg_tbl_entry, (void*)eth_mtr_prfCfg_above_64_val); 
		SOCDNX_IF_ERR_EXIT(res);
		...
		/* Set pointer in plfSel table to deault 0*/
  ...
  )
{
  ...
  int
	...
	if (glbl_profile_idx == 0) { /* profile index 0 means to unset the port from any policer */
		soc_mem_field32_set(unit, eth_mtr_config_mem, &eth_mtr_config_val, GLOBAL_METER_PROFILE_VALIDf, 0);
	}else{
...
uint32
	...
	rv;
soc_mem_t
	mem[2][2] = {{MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm,
				  MRPS_MCDB_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm},
				 {MTRPS_EM_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm,
				  MTRPS_EM_MCDB_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm}};
...
soc_field_t
	...
	rv = soc_mem_read(unit, mem[0][meter_group], MRPS_BLOCK(unit, core_id), line_number, &table_line);
	...
	rv = soc_mem_write(unit, mem[0][meter_group], MRPS_BLOCK(unit, core_id), line_number, &table_line);
	SOCDNX_IF_ERR_EXIT(rv);

	rv = soc_mem_write(unit, mem[1][meter_group], MTRPS_EM_BLOCK(unit, core_id), line_number, &table_line);
	SOCDNX_IF_ERR_EXIT(rv);
...
uint32
    ...
    rv;
soc_mem_t
	mem[2] = {MRPS_MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm,
		      MRPS_MCDB_IN_PP_PORT_MAP_AND_MTR_PRF_MAPm};
soc_field_t
	field[2][4] = {{MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_0f, 
		            MCDA_IN_PP_PORT_MAP_AND_MTR_PRF_MAP_1f, 
	...
	*packet_size = 0;
	...
	rv = soc_mem_read(unit, mem[meter_group], MRPS_BLOCK(unit, core_id), line_number, &table_line);
	SOCDNX_IF_ERR_EXIT(rv);
...
uint32
	...
	rv;
soc_mem_t
	mem[2] = {MRPS_MCDA_IN_PP_PORT_MAPm, MRPS_MCDB_IN_PP_PORT_MAPm};
soc_field_t
...
uint32
	...
	rv;
soc_mem_t
	mem[2][2] = {{MRPS_MCDA_IN_PP_PORT_MAPm, 
		          MRPS_MCDB_IN_PP_PORT_MAPm},
	             {MTRPS_EM_MCDA_IN_PP_PORT_MAPm, 
		          MTRPS_EM_MCDB_IN_PP_PORT_MAPm}};
soc_field_t
	port_profile_field[2][4] = {{MCDA_IN_PP_PORT_MAP_0f, 
		                         MCDA_IN_PP_PORT_MAP_1f, 
								 MCDA_IN_PP_PORT_MAP_2f, 
