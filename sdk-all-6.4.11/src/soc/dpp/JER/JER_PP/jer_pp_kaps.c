...
#define KAPS_KBP_SDK_BB_ENTRY_RATIO 5000
...
void jer_kaps_sw_init(int unit)
{
          ...
          db_idx++ ) 
    {
        ...
        if (db_size > 0) 
        {
            if (SOC_IS_QAX(unit) && jer_kaps_db_config_info[unit][db_idx].clone_of_db_id != JER_KAPS_IP_NOF_DB) {
                ...
                if (db_size < 10000) {
                    /* Assume number of BBs is smaller than 10000 */
                    jer_kaps_db_config_info[unit][db_idx].db_size = db_size * KAPS_KBP_SDK_BB_ENTRY_RATIO;
                } else {
    ...
    for(search_idx = 0; search_idx < JER_KAPS_NOF_SEARCHES; search_idx++) 
    {
         ...
         *  In this case, 2.
         */
        jer_kaps_search_config_info[unit][search_idx].max_tables_num = JER_KAPS_MAX_NUM_OF_PARALLEL_SEARCHES;
    }
