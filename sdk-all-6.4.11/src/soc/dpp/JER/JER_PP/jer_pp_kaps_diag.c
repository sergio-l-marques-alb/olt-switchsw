   ...
   )
{
    ...
    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,key_blk,2,155,0,5);
    ...
    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,key_blk,1,255,252,5);
    rv = soc_sand_bitstream_get_any_field(regs_val, 0, 3, tmp_buffer);
    SOCDNX_SAND_IF_ERR_EXIT(rv);
    SHR_BITCOPY_RANGE(search_keys[0], 0, tmp_buffer, 0, 4);


    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,key_blk,1,77,0,5);
    ...
    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,119,100,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,JER_KAPS_AD_WIDTH_IN_BITS,&ad_array[0]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,59,40,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,JER_KAPS_AD_WIDTH_IN_BITS,&ad_array[1]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,89,70,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,JER_KAPS_AD_WIDTH_IN_BITS,&ad_array[2]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,29,10,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,JER_KAPS_AD_WIDTH_IN_BITS,&ad_array[3]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    /*status*/

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,99,98,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,2,&status[0]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,39,38,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,2,&status[1]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,69,68,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,2,&status[2]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,9,8,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,2,&status[3]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);



    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,97,90,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,8,&prio_len[0]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,37,30,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,8,&prio_len[1]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,67,60,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,8,&prio_len[2]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);

    JER_PP_DIAG_FLD_READ(&fld,core_id,ARAD_IHP_ID,ad_blk,0,7,0,5);
    rv = soc_sand_bitstream_get_any_field(regs_val,0,8,&prio_len[3]);
    SOCDNX_SAND_IF_ERR_EXIT(rv);
...
int jer_pp_kbp_sdk_ver_test(int unit)
{
    ...
    const char* kbp_sdk_ver_arr[] =       {"KBP SDK 1.3.11", "KBP SDK 1.3.12"};
    const uint32 sw_sdk_ver_arr_6_5[]  = {0x65100000, 0x65200000};
    const uint32 sw_sdk_ver_arr_6_4[]  = {0x64700000, 0x64800000};
