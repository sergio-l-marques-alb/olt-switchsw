...
#define JER_ITM_SYS_RED_Q_BOUNDARY_TH_MNT_MSB   7
...
#define JER_ITM_SYS_RED_Q_BOUNDARY_TH_EXP_LSB   8
...
#define JER_ITM_VOQ_ECN_CAPABLE_RJCT_MASK           (0x1 << 7)    /* For ECN capable we mask out only WRED tests */
...
*/
#define JER_ITM_VSQ_RJCT_ALL_MASK 0x3F /* mask out all tests for VSQ A-F */
...
int jer_itm_drop_global_thresholds_init(int unit)
{
    ...
    SOC_DPP_CORES_ITER(SOC_DPP_CORE_ALL(unit), core_index) {
        ...
        glbl_drop.bds[0].set   = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1018) / 1024);  /* dram dp 0 - 99.5% */
        glbl_drop.bds[0].clear = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1018) / 1024) - 50; 
        glbl_drop.ocb_bds[0].set   = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1008) / 1024); /* ocb dp 0 - 98.5% */
        glbl_drop.ocb_bds[0].clear = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1008) / 1024) - 50;
        glbl_drop.bds[1].set   = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1017) / 1024);  /* dram dp 1 - 99.4% */
        glbl_drop.bds[1].clear = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1017) / 1024) - 50; 
        glbl_drop.ocb_bds[1].set   = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1005) / 1024); /* ocb dp 1 - 98.2% */
        glbl_drop.ocb_bds[1].clear = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1005) / 1024) - 50;
        glbl_drop.bds[2].set   = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1016) / 1024);  /* dram dp 2 - 99.3% */
        glbl_drop.bds[2].clear = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1016) / 1024) - 50; 
        glbl_drop.ocb_bds[2].set   = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1002) / 1024); /* ocb dp 2 - 97.9% */
        glbl_drop.ocb_bds[2].clear = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 1002) / 1024) - 50;
        glbl_drop.bds[3].set   = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1015) / 1024);  /* dram dp 3 - 99.2% */
        glbl_drop.bds[3].clear = ((((core_index == 0) ? dbuffs_bdries->mnmc_0.size : dbuffs_bdries->mnmc_1.size) * 1015) / 1024) - 50; 
        glbl_drop.ocb_bds[3].set   = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 999) / 1024); /* ocb dp 3 - 97.6% */
        glbl_drop.ocb_bds[3].clear = ((((core_index == 0) ? dbuffs_bdries->ocb_0.size : dbuffs_bdries->ocb_1.size) * 999) / 1024) - 50;  
  ...
  )
{
    ...
    /*configured by jer_itm_glob_rcs_drop_set();*/
    ...
    /*configured by jer_itm_dyn_total_thresh_set();*/
    ...
    /*configured by jer_itm_resource_allocation_set()*/
    reg_above_64[2] = 3;
    ...
    SOCDNX_IF_ERR_EXIT(res);
    if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_LOOSE) { /* Loose mode */
    ...
    } else if (SOC_DPP_CONFIG(unit)->jer->tm.cgm_mgmt_guarantee_mode == SOC_TMC_ITM_CGM_MGMT_GUARANTEE_STRICT) { /* Strict mode */
    ...
    soc_reg64_field32_set(unit, IQM_RJCT_GLBL_TESTS_MASKr, &reg64, ECN_RJCT_MASKf, JER_ITM_VOQ_ECN_CAPABLE_RJCT_MASK); /* mask out only WRED tests*/
  ...
  res = WRITE_IPST_CREDIT_ARBITER_CONFIGr(unit, data); 
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_index) {
        ...
        soc_reg_field_set(unit, IQM_BURST_SETTINGSr, &data, BURST_SIZEf, 6);
  ...
  )
{
  ...
  if ( color_resolution_mode == ARAD_PP_MTR_COLOR_RESOLUTION_MODE_DEFAULT && SOC_DPP_CONFIG(unit)->tm.is_petrab_in_system) {
      ...
      for (eth_mtr_dp_indx = 0; eth_mtr_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++eth_mtr_dp_indx) {
		  for (mtr_dp_indx = 0; mtr_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++mtr_dp_indx) {
			  ...
			  else /* PP Mode */
			  {
				  ...
				  if (color_resolution_mode == ARAD_PP_MTR_COLOR_RESOLUTION_MODE_RED_DIFF) {
					  if (mtr_dp_indx == 2) {
...
uint32
  jer_itm_dyn_total_thresh_set(
    ...
    SOC_SAND_IN int    core,
    SOC_SAND_IN uint8  is_ocb_only,
    SOC_SAND_IN int32* reservation_increase_array /* the (signed) amount in which the thresholds should decrease (according to 100% as will be set for DP 0) */
  )
{
    uint32
        res;
    ...
    SOC_DPP_CORES_ITER(core, core_id){
        ...
        reservation_increase = reservation_increase_array[core_id];
        ...
        if (reservation_increase) {
            ...
            if (SOC_DPP_CONFIG(unit)->tm.guaranteed_q_mode == SOC_DPP_GUARANTEED_Q_RESOURCE_MEMORY) {
                ...
                /*SOCDNX_IF_ERR_EXIT(WRITE_IQM_BDB_DYN_SIZE_RJCT_TH_CFGr(unit, SOC_CORE_ALL, reg_above_64)); *not configured by APIs*/
            } else {
  ...
  ) 
{
    ...
    SOC_DPP_CORES_ITER(core, core_id) {
        ...
        data = 0;
        if (adjust_type == SOC_TMC_ITM_PKT_SIZE_ADJUST_QUEUE) { /* per queue (credit class) compensation */
            
            offset = index; /* the index to the table is the credit class profile since only port profile 0 is used */
            ...
            for (i = 0; i < 8; i++) { 
                ...
                SOCDNX_IF_ERR_EXIT(WRITE_IQM_SCH_HAPMm(unit, IQM_BLOCK(unit, core_id), offset, &data));
            }
  ...
  ) 
{
    ...
    if (adjust_type == SOC_TMC_ITM_PKT_SIZE_ADJUST_QUEUE) { /* per queue (credit class) compensation */
        ...
        offset = index; /* the index to the table is the credit class profile since only port profile 0 is used */
        SOCDNX_IF_ERR_EXIT(READ_IQM_SCH_CPMm(unit, IQM_BLOCK(unit, core_id), offset, &data));
        ...
        SOCDNX_IF_ERR_EXIT(READ_IQM_SCH_HAPMm(unit, IQM_BLOCK(unit, core_id), offset, &data));
...
/*
Get Jericho ingress congestion statistics.
*/
  ...
  )
{
  ...
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_OCCUPIED_BD_COUNTERr, core, 0, OCCUPIED_BD_COUNTERf, &fld_val));
  ...
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_BD_COUNTERr, core, 0, FREE_BD_COUNTERf, &fld_val));
  stats->bd2_free = fld_val; /* place the value into the 32 bits integer */		

  /* Current number of free Full-Multicast Type Dbuffs */
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_FULL_MULTICAST_DBUFFS_COUNTERr, core, 0, FREE_FULL_MULTICAST_DBUFFS_COUNTERf, &fld_val));
  stats->db_full_mul_free = fld_val; /* place the value into the 32 bits integer */	

  /* Current number of free Mini-Multicast Type Dbuffs */
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_U_MNMC_DBUFFS_COUNTERr, core, 0, FREE_U_MNMC_DBUFFS_COUNTERf, &fld_val));
  stats->db_mini_mul_free = fld_val; /* place the value into the 32 bits integer */		

  /* Free BDBs minumum occupancy indication */                                                                          
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_BDB_MINIMUM_OCCUPANCYr, core, 0, FREE_BDB_MINIMUM_OCCUPANCYf, &fld_val));
  ...
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_FULL_MULTICAST_DBUFFS_MINIMUM_OCCUPANCYr, core, 0, FREE_FULL_MULTICAST_DBUFFS_MINIMUM_OCCUPANCYf, &fld_val));
  ...
  SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_read(unit, IQM_FREE_MNMC_DBUFFS_MINIMUM_OCCUPANCYr, core, 0, FREE_MNMC_DBUFFS_MINIMUM_OCCUPANCYf, &fld_val));
