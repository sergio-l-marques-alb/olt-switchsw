/* $Id: ppc_api_eg_encap.c,v 1.30 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_eg_encap.c
*
  ...
  )
{
  ...
  /* info->entry_type is not touched for backward compatibility */
    ...
    ) 
{
  ...
  /* for a+, the full sa is saved in EEDB,
     for jer, only 12lsbs */
  ...
  /* for a+ only: in Jericho, tpid and ethertype is saved in an additional table (eth type index table) */
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "ll_info:")));
  SOC_PPC_EG_ENCAP_LL_INFO_print(&(info->ll_info));
  LOG_CLI((BSL_META_U(unit,
                      "vsi_info:")));
  ...
  )
{
  ...
  switch(info->entry_type)
  {
    ...
    case SOC_PPC_EG_ENCAP_ENTRY_TYPE_LL_ENCAP:
      LOG_CLI((BSL_META_U(unit,
                          "LL Encapsulation:\n\r")));
      SOC_PPC_EG_ENCAP_LL_INFO_print(&(info->entry_val.ll_info));
    break;
