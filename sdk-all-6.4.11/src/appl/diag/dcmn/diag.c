...
STATIC int diag_print_rejected_ERPP(int unit) {
   ...
   if (num_of_rejects != 0) {
       if (SOC_IS_JERICHO(unit)) {
           cli_out("|                                                                                       |%-85s  |\n", reject_names_ln);
           if (BCM_E_NONE != WRITE_EGQ_ERPP_DISCARD_INTERRUPT_REGISTERr(unit, REG_PORT_ANY, val32)) {
              return CMD_FAIL;
           } 
       } else {
           cli_out("|                                                             |%-59s  |\n", reject_names_ln); 
...
STATIC int diag_print_iqm_rejects_and_erpp_discards(int unit, uint8 core_disp) {
    int      erpp_current_field[MAX_NUM_OF_CORES], iqm_current_field[MAX_NUM_OF_CORES];
    uint32   erpp_reg_val[MAX_NUM_OF_CORES];
    uint64   iqm_reg_val[MAX_NUM_OF_CORES], erpp_reg_val64[MAX_NUM_OF_CORES];
    char     iqm_str_buf[MAX_NUM_OF_CORES][LINE_WIDTH / 2], erpp_str_buf[MAX_NUM_OF_CORES][LINE_WIDTH / 2];
    int      core = 0;
...
#endif
    {
        ...
        if (!core_disp) {
           ...
           if (SOC_IS_JERICHO(unit)) {
               /* reset EGQ_ERPP_DISCARD_INTERRUPT_REGISTER on Jericho by writing 1s*/
...
int jericho_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    rc = diag_print_iqm_rejects_and_erpp_discards(unit, core_disp);
    if (rc != CMD_OK) {
        return rc;
    }
