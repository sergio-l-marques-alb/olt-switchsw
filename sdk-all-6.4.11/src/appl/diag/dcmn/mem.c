...
cmd_result_t
_soc_arad_mem_array_wide_access(int unit, soc_mem_t mem, unsigned array_index, int copyno, int index, void *entry_data,
    ...
    for (k = index; k < index + count; k++) {
        ...
        if(wide) {
            if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                i =  _soc_arad_mem_array_wide_access(unit, mem, array_index, copyno, k, entry, 1);
            } else {
                i =  dcmn_mem_array_wide_access(unit, mem, array_index, copyno, k, entry, 1);
            }
...
dpp_all_mem_get(int unit, int flags)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (!soc_mem_is_valid(unit, mem) || filter_mems(unit , mem)) {
            continue;
        }
    ...
    SOC_MEM_BLOCK_ITER(unit, mem, blk) {
        ...
        for (index = start; index < start + count; index++) {
            ...
            if(1 == mod)
            {
                if (wide) {
                    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                        r = _soc_arad_mem_array_wide_access(unit, mem, array_index, blk, index, entry, 1);
                    } else {
                        r = dcmn_mem_array_wide_access(unit, mem, array_index, blk, index, entry, 1);
                    }
                } else {
            ...
            if (wide) {
                if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                    r = _soc_arad_mem_array_wide_access(unit, mem, array_index, blk, no_cache ? -index : index, entry, 0);
                } else {
                    r = dcmn_mem_array_wide_access(unit, mem, array_index, blk, no_cache ? -index : index, entry, 0);
                }
            } else {
...
_dpp_mem_ins_del_lkup(int unit, args_t *a, int oper)
{
    ...
    if (parse_memory_name(unit, &mem, memname, &copyno, &array_index) < 0) {
        LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "ERROR: unknown table \"%s\"\n"), tab));
        goto done;
    }
    ...
    if (soc_mem_is_readonly(unit, mem)) {
        LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "ERROR: Table %s is read-only\n"), SOC_MEM_UFNAME(unit, mem)));
        goto done;
    }
    ...
    if ( entry_dw >= CMIC_SCHAN_WORDS(unit)
        ...
        if (mem != OCB_OCBM_EVENm && mem != OCB_OCBM_ODDm) {
            LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "Error: can't write unknown wide memory %s\n"), SOC_MEM_UFNAME(unit, mem)));
            rv = CMD_FAIL;
            goto done;
        }
    }
    ...
    for (index = start; index < start + count; index++) {
		...
		if (oper == DPP_ACCESS_OP_INSERT_EM) {
            if(SOC_FAILURE(soc_dpp_p3_mem_insert(unit, mem,copyno,entry))) {
                rv = CMD_FAIL;
                goto done;
            }
		}
...
STATIC int mem_in_tr7_schan_skip_list_for_jericho_emulation(int unit,  soc_mem_t mem)
{
    ...
    /* memories failed due to scahn  timeout in tr 7*/
        ...
        case EPNI_TRILL_FORMAT_HALF_ENTRYm:
        ...
        case EPNI_PROTECTION_ENTRYm:
        case IQM_PAKCET_DESCRIPTOR_MEMORY_A_DYNAMICm:
        case EPNI_LINK_LAYER_OR_ARP_NEW_FORMATm:
        case EPNI_TRILL_FORMAT_FULL_ENTRYm:
