...
/* Sleep to wait for the timeout*/
    sal_sleep(1);
    if(test_oam_timeout_event_counts <  endpoint-1){   
...
/* Sleep to wait for the timein */
    sal_sleep(1);
    if(test_oam_timein_event_counts < endpoint-1){
...
oamp_test(int unit, args_t *a, void *p) {
    ...
    if (!init_param->option) {
        ...
        return test_run_bfd(unit, init_param->port1, init_param->port2, init_param->numEndpoints, init_param->seed, init_param->numStates);
