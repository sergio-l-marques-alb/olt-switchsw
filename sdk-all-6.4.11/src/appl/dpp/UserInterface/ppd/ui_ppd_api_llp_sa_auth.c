...
{   
  ...
  SOC_PPD_LLP_SA_AUTH_PORT_INFO_clear(&prm_port_auth_info);
...
{   
  ...
  SOC_PPD_LLP_SA_AUTH_PORT_INFO_print(&prm_port_auth_info);
...
{   
  ...
  SOC_PPD_LLP_SA_AUTH_MAC_INFO_clear(&prm_mac_auth_info);
...
{   
  ...
  SOC_PPD_LLP_SA_AUTH_MAC_INFO_print(&prm_mac_auth_info);
...
{   
  uint32 
    ret;   
  SOC_PPD_LLP_SA_AUTH_MATCH_RULE   
    prm_rule;
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_LLP_SA_AUTH_MATCH_RULE_clear(&prm_rule);
  ...
  for (index = 0; index < 10; index++) {
      ...
      SOC_PPD_LLP_SA_AUTH_MAC_INFO_clear(&prm_auth_info_arr[index]);
  }
  ...
  { 
    ...
    prm_rule.rule_type = SOC_PPD_LLP_SA_MATCH_RULE_TYPE_ALL;
  }
  /* Call function */
  ...
  for (index = 0; index < prm_nof_entries; index++) {
      ...
      SOC_PPD_LLP_SA_AUTH_MAC_INFO_print(&prm_auth_info_arr[index]);
      cli_out("\n\r");
  }
