...
#ifdef UI_METERING
/******************************************************************** 
 *  Function handler: mtr_glbl_info_set (section metering)
 ********************************************************************/
int 
  ui_ppd_api_metering_mtr_glbl_info_set(
...
{   
  ...
  soc_sand_proc_name = "soc_ppd_mtr_glbl_info_set"; 
 
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MTR_GLBL_INFO_clear(&prm_mtr_glbl_info);
 
  /* Get parameters */ 

  /* This is a set function, so call GET function first */                                                                                
  ret = soc_ppd_mtr_glbl_info_get(
          unit,
          &prm_mtr_glbl_info
        );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_mtr_glbl_info_get - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_mtr_glbl_info_get");   
    goto exit; 
  } 

  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_MTR_GLBL_INFO_SET_MTR_GLBL_INFO_SET_MTR_GLBL_INFO_UPDATE_ALL_COPIES_ID,1)) 
  { 
    UI_MACROS_GET_NUMMERIC_VAL(SOC_PARAM_PPD_MTR_GLBL_INFO_SET_MTR_GLBL_INFO_SET_MTR_GLBL_INFO_UPDATE_ALL_COPIES_ID);  
    UI_MACROS_CHECK_GET_VAL_OF_ERROR; 
    prm_mtr_glbl_info.update_all_copies = (uint8)param_val->value.ulong_value;
  } 


  /* Call function */
  ret = soc_ppd_mtr_glbl_info_set(
          unit,
          &prm_mtr_glbl_info
        );
  if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK) 
  { 
    send_string_to_screen(" *** soc_ppd_mtr_glbl_info_set - FAIL", TRUE); 
    soc_sand_disp_result_proc(ret, "soc_ppd_mtr_glbl_info_set");   
    goto exit; 
  } 
...
/******************************************************************** 
 *  Function handler: mtr_glbl_info_get (section metering)
 ********************************************************************/
int 
  ui_ppd_api_metering_mtr_glbl_info_get(
    CURRENT_LINE *current_line 
  ) 
{   
  uint32 
  ...
  UI_MACROS_INIT_FUNCTION("ui_ppd_api_metering"); 
  soc_sand_proc_name = "soc_ppd_mtr_glbl_info_get"; 
  ...
  SOC_PPD_MTR_GLBL_INFO_print(&prm_mtr_glbl_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MTR_GROUP_INFO_clear(&prm_mtr_group_info);
...
{   
  ...
  SOC_PPD_MTR_GROUP_INFO_print(&prm_mtr_group_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MTR_BW_PROFILE_INFO_clear(&prm_bw_profile_info);
  SOC_PPD_MTR_BW_PROFILE_INFO_clear(&prm_exact_bw_profile_info);
  ...
  SOC_PPD_MTR_BW_PROFILE_INFO_print(&prm_exact_bw_profile_info);
...
{   
  ...
  SOC_PPD_MTR_BW_PROFILE_INFO_print(&prm_bw_profile_info);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MTR_METER_ID_clear(&prm_meter_ins_ndx);
...
{   
  ...
  unit = soc_ppd_get_default_unit(); 
  SOC_PPD_MTR_BW_PROFILE_INFO_clear(&prm_policer_info);
...
{   
  ...
  SOC_PPD_MTR_BW_PROFILE_INFO_print(&prm_policer_info);
...
{   
  ...
  {  
    ...
    UI_MACROS_CHECK_GET_VAL_OF_ERROR;   
    prm_port_ndx = (SOC_PPD_PORT)param_val->value.ulong_value;
  } 
  else 
  {    
...
{   
  ...
  if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_MTR_GLBL_INFO_SET_MTR_GLBL_INFO_SET_ID,1)) 
  { 
    ui_ret = ui_ppd_api_metering_mtr_glbl_info_set(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_MTR_GLBL_INFO_GET_MTR_GLBL_INFO_GET_ID,1)) 
  { 
    ui_ret = ui_ppd_api_metering_mtr_glbl_info_get(current_line); 
  } 
  else if(!search_param_val_pairs(current_line,&match_index,SOC_PARAM_PPD_MTR_METERS_GROUP_INFO_SET_MTR_METERS_GROUP_INFO_SET_ID,1)) 
  { 
