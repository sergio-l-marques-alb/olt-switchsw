...
int mirror_destinations_arad_test_1(int unit, int device_type) {
    ...
    int default_nof_mc_groups = (dev_type ==  device_type_jericho ? (32*1024) : (16*1024));
...
int mirror_egress_vlan_id(int unit){
    int vlan, rv; 
    int found_ports_num, found_ports[10], found_port_cores[2] = {0};
    ...
    for (i = 0; i < found_ports_num; i++) {
        ...
        if (mapping_info.core == 0 && !found_port_cores[0]){
            port = mapping_info.tm_port; /*avoiding cint types conflict*/
            BCM_GPORT_LOCAL_SET(gport0, port);
            found_port_cores[0] = 1;
        } else if(mapping_info.core == 1 && !found_port_cores[1]){
            port = mapping_info.tm_port;
            BCM_GPORT_LOCAL_SET(gport1, port);
            found_port_cores[1] = 1;
        }

        if (found_port_cores[0] && found_port_cores[1]) {
            break;
        }
    }
    ...
    BCM_GPORT_LOCAL_SET(mdst.gport, 14);
    mdst.flags = 0;
