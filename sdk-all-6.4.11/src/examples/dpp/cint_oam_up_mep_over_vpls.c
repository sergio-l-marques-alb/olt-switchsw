...
int oam_create_lsp_for_ccp(int unit, int outPort) {
    ...
    sal_memcpy(l3Egr.mac_addr, egrDa, sizeof(egrDa));
    l3Egr.port = outPort;
    bcm_stk_modid_get(unit, &mod);
    l3Egr.module = mod;
    l3Egr.intf = egrLable.tunnel_id;
    l3Egr.flags = BCM_L3_ENCAP_SPACE_OPTIMIZED;

    rv = bcm_l3_egress_create(unit, flags, &l3Egr, &egrIfId);
    if (rv != BCM_E_NONE) {
        printf("Error, bcm_l3_egress_create (vlan %d, port %d, intf %d) failed, rv=%s \n",
               outVlan, outPort,l3Egr.intf, bcm_errmsg(rv));
        return rv;
    }

    out_tunnel_id = l3Egr.encap_id;
    tunnel_egr_if = egrIfId;
    return rv;
}
...
int oam_pw_port_create(int unit, int outPort) {
    ...
    BCM_GPORT_MPLS_PORT_ID_SET(mpls_port.mpls_port_id, 80*1024-50);
...
int oam_up_mep_over_vpls_run_with_defaults(int unit, int port, int port2) {
    ...
    if (rv != BCM_E_NONE) {
        printf("Error,oam_create_lsp_for_ccp(%s) \n",bcm_errmsg(rv));
        return rv;
    }   
