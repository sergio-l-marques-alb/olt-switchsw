...
mpls_tunnels_config(
    ...
    rv = create_l3_egress(unit, BCM_L3_EGRESS_ONLY, port, eg_vlan, label_array[0].tunnel_id, cross_connect_info.nh_mac, egress_intf, &encap_id);
    if (rv != BCM_E_NONE) {
...
mpls_port_create_ingress_egress(
    ...
    mpls_port->flags = BCM_MPLS_PORT_EGRESS_TUNNEL|BCM_MPLS_PORT_WITH_ID|BCM_MPLS_PORT_ENCAP_WITH_ID|BCM_MPLS_PORT_COUNTED;
