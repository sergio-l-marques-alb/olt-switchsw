...
int oamp_server_set_server_endpoints(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2) {
    ...
    printf("Down MEP created on server (unit %d) with id %d\n", server_unit, ep_server_down.id);
    ...
    rv = port_to_system_port(client_unit, client_port2, &ep_server_up.tx_gport); /* After recycling, source port (on PTCH) will be the client's port */
    if (rv != BCM_E_NONE) {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
    ...
    /* Adding TLV */
    ep_server_up.flags |= BCM_OAM_ENDPOINT_INTERFACE_STATE_UPDATE;
    ep_server_up.interface_state = BCM_OAM_INTERFACE_TLV_UP;
    ...
    printf("Up MEP created on server (unit %d) with id %d\n", server_unit, ep_server_up.id);
...
int oamp_server_example(int server_unit, int client_unit, int server_port1, int server_port2, int client_port1, int client_port2) {
    ...
    /* Configure a trap which updates the destination to be the OAMP of the server unit.
       This trap will be used by the client's down MEPs */
    ...
    if (!is_device_or_above(server_unit, JERICHO) || use_11b_maid) {
        field_oam_advanced_features(server_unit);
    }
