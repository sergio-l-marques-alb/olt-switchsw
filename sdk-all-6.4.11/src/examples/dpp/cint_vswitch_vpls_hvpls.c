...
/* $Id: cint_vswitch_vpls_hvpls.c,v 1.22 Broadcom SDK $
...
vswitch_vpls_hvpls_info_init(int unit, int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan, int vpn_id, int two_calls) {
    ...
    vswitch_vpls_hvpls_info.pwe_outlif_primary = 0x7000;  
    vswitch_vpls_hvpls_info.pwe_outlif_protected = 0x7002; 
...
vswitch_vpls_hvpls_fec_mpls_port_create(int unit){
    int rv;
    bcm_mpls_port_t mpls_port_secondary, mpls_port_primery;
    int gport2_fec_b, gport2_fec_a; 
    ...
    /* create two FEC entries, each point to <MPLS-tunnel-FEC,LIF> and share the same Failver-id*/
    pwe_encap_id = vswitch_vpls_hvpls_info.pwe_outlif_protected;

    rv = mpls_port__fec_only__create(unit, 
                                     BCM_GPORT_INVALID, 
                                     vswitch_vpls_hvpls_info.pwe_failover_id_fec,
                                     0,/* this is secondary*/
                                     pwe_encap_id,
                                     &gport2_fec_b,
                                     vswitch_vpls_hvpls_info.mpls_fec,/*egress_tunnel_if*/
                                     &mpls_port_secondary);
    if (rv != BCM_E_NONE) {
    ...
    BCM_L3_ITF_SET(pwe_encap_id, BCM_L3_ITF_TYPE_LIF, vswitch_vpls_hvpls_info.pwe_outlif_primary);
    ...
    vswitch_vpls_hvpls_info.pwe_fec = mpls_port_secondary.mpls_port_id;
...
vswitch_vpls_hvpls_run(int unit){
    ...
    vswitch_vpls_hvpls_info.ll_intf_id = create_l3_egress_encap_params.encap_id;
    ...
    rv = bcm_failover_create(unit, BCM_FAILOVER_FEC, &vswitch_vpls_hvpls_info.mpls_failover_id_fec);
    if (rv != BCM_E_NONE) {
    ...
    create_l3_egress_params.gport = mpls_lsr_info_1.eg_port;  /* outgoing port */
    create_l3_egress_params.vlan = mpls_lsr_info_1.eg_vid; 
    create_l3_egress_params.intf = vswitch_vpls_hvpls_info.mpls_tunnel_id_protected;
    ...
    vswitch_vpls_hvpls_info.mpls_fec = create_l3_egress_params.l3_egress_id;

    /* Create primery FEC */
    create_l3_egress_params.intf = vswitch_vpls_hvpls_info.mpls_tunnel_id_primary;
