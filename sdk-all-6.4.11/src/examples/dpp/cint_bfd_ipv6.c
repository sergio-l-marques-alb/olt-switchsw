 ...
 */
int bfd_ipv6_service_init(int unit, int in_port, int in_port_2, int out_port, int out_port_2, int is_trunk, int remote_port, int is_user_trap) {
    int rv = 0;
    int i;
    ...
    use_user_defined_trap = is_user_trap;
...
int bfd_ipv6_intf_init(int unit, int type, int is_trunk){
    ...
    if (!is_trunk) {
        rv = create_l3_egress(unit, 0, bfd_out_port[0], out_vsi, l3_eg_int, next_hop_mac, &fec, &encap_id); 
    } else {
        rv = create_l3_egress(unit, 0, trunk_gport[1], out_vsi, l3_eg_int, next_hop_mac, &fec, &encap_id);
    }    
    if (rv != BCM_E_NONE) {
 ...
 */
int bfd_ipv6_run_with_defaults(int unit, int endpoint_id, int is_acc, int is_single_hop, bcm_bfd_auth_type_t auth_type, int auth_index, int type, int is_trunk) {
    int rv;
    ...
    if (bfd_remote_port != BCM_GPORT_INVALID) {
        if (use_user_defined_trap) {
            ...
            BCM_GPORT_LOCAL_SET(bfd_endpoint_info.remote_gport, bfd_remote_port);
        }
    } 
