 ...
 */
int set_vsq_tail_drop_settings(int unit,bcm_gport_t gport,int dp,int max_size, int max_size_bds)
{
    bcm_error_t rv = BCM_E_NONE;
    uint32 flags;
    ...
    printf("Set Tail drop Settings unit %d, gport 0x%x, max_size_bds %d \n",
        unit,gport,max_size_bds);

    
    if (device_type == DEVICE_TYPE_ARAD) {
        /* BCM_COSQ_GPORT_SIZE_BYTES is supported only for Arad */
    ...
    gport_size.size_min = 0;
    gport_size.size_max = max_size_bds;
    rv = bcm_cosq_gport_color_size_set(unit,gport,cosq,dp,flags,&gport_size);
    if (rv != BCM_E_NONE) {
        printf("bcm_cosq_gport_color_size_set() failed $rv\n");
        printf("gport: (0x%08x), cosq: %d, dp: %d, flags 0x%08x\n",
...
int run_wred_example(int unit,int destination_local_port, int base_queue)
{
    ...
    for (drop_precedence = 0; drop_precedence < 4; drop_precedence++) {
        ...
        max_queue_size_bds = 1000000;
        rv = set_vsq_tail_drop_settings(unit,vsqA,drop_precedence,max_queue_size, max_queue_size_bds);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }   
...
{
    bcm_error_t rv = BCM_E_NONE;  
    int max_queue_size, max_queue_size_bds;
    int dp;
    int index;
    ...
    for (dp = 0; dp < 4; dp++) {
        max_queue_size = 500;
        max_queue_size_bds = 10;
        rv = set_vsq_tail_drop_settings(unit,vsqA,dp,max_queue_size, max_queue_size_bds);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }            
        max_queue_size = max_queue_size * 2;
        max_queue_size_bds = max_queue_size_bds * 2;
        rv = set_vsq_tail_drop_settings(unit,vsqB,dp,max_queue_size, max_queue_size_bds);
        if (rv != BCM_E_NONE) {
            printf("(%s) \n",bcm_errmsg(rv));
            return rv;
        }
