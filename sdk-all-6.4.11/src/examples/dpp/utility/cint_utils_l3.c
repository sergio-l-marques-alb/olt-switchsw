...
int l3__intf_rif__create(int unit, create_l3_intf_s *l3_intf){
    ...
    if (l3_intf->vrf == 0 && is_device_or_above(unit,JERICHO)) {
        /* We need to update the in_rif to have public searches enabled */
        l3_intf->vrf_valid = 1;
    }
    ...
    if (l3_intf->vrf_valid || l3_intf->rpf_valid) {
        ...
        if ((l3_intf->ingress_flags & BCM_L3_INGRESS_GLOBAL_ROUTE) || (l3_intf->vrf == 0 && is_device_or_above(unit,JERICHO))) {
