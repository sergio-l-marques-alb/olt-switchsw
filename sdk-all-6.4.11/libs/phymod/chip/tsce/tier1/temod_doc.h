...
For auto negotiationotiation, we need to first identify logic lane 0 which will
carry out page exchanges and speed negotiation. Further the CL73 auto
negotiation could negotiate to 100G, 40G, 10G KR, 10G-XAUI, or even 1G.
For 40G, the design would require a 4-lane XLMAC bandwidth.  Thus for the 4-4-2
configuration, only the lane 0 of the first or second core can be used for auto
negotiationotiation if 40G is a required advertisable speed.  For the 3-4-3
configuration , only the second core can be used. Similarly for 2-4-4
configuration, only the second and third core's lane 0 can be used.  Proper port
configuration requires to setup the correct XLMAC out of three XLMACs for speeds
less than 100G. But for 100G ports, the logic lane 0 is not always in the first
core. So some of the configuration settings are BRCM TD2+ specific and we have
to ensure the future BRCM products are backward compatible.
...
For PCS Configuration, The reference clock freq only matters for AN. And used
for tick generation, that is used in AN timers.
...
\section  tsce_faq TSCE frequently asked questions (FAQ)
  ...
  - What are the different firmware load methods?
   ...
   load. Internal load is slow load. External load is recommended way as it is
   faster
*/
