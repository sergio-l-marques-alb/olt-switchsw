...
STATIC
int _qtce_st_hto_interface_config_set(const temod_device_aux_modes_t *device, int lane_num, int speed_vec, uint32_t *new_pll_div, int16_t *os_mode)
{
...
{  
    ...
    if(config->device_aux_modes !=NULL){    
        PHYMOD_IF_ERR_RETURN
            (_qtce_st_hto_interface_config_set(config->device_aux_modes, start_lane, new_speed_vec, &new_pll_div, &new_os_mode)) ;
    }
    ...
    */
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
             (qtce_phy_firmware_lane_config_set(&pm_phy_copy, firmware_lane_config));
    }
...
int qtce_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
   ...
   if ((an->num_lane_adv == 4) || (an->an_mode == phymod_AN_MODE_CL37BAM)) {
     phy_copy.access.lane_mask = 0x1 << start_lane;
     PHYMOD_IF_ERR_RETURN
        (qtce_phy_firmware_core_config_set(&phy_copy, firmware_core_config_tmp));
    }

    for (i = 0; i < num_lane; i++) {
        phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN
            (qtce_phy_firmware_lane_config_set(&phy_copy, firmware_lane_config));
    }
    ...
    /* qmod_autoneg_control invokes disabling CL72 and spd_trigger when en=0 */

    PHYMOD_IF_ERR_RETURN
        (qmod_autoneg_control(&phy_copy.access, &an_control));
