    ...
    if (config->data_rate != MADURA_SPD_100G &&
        ...
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
           ...
           if (!is_lane_used) {
            ...
            /* Program Modes */
            if (((config->data_rate == MADURA_SPD_40G  || config->data_rate == MADURA_SPD_42G) && (port_idx == 0)) ||
                 ((config->data_rate == MADURA_SPD_10G || config->data_rate == MADURA_SPD_11G) && (port_idx < 2))) {
...
int _madura_interface_set(const phymod_access_t *pa, uint16_t ip,  const phymod_phy_inf_config_t* config)
{
    ...
    if ( intf == phymodInterfaceER4 || intf == phymodInterfaceSR ||
        ...
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM,
               (_PHYMOD_MSG("Invalid interface type as .. ")));
    }
...
int _madura_interface_set_retimer_ull_mode_set(const phymod_access_t *pa, uint16_t ip,  const phymod_phy_inf_config_t* config, uint16_t Tx_timing_mode)
{
    uint16_t config_reg=0;
    uint16_t lane_mask = pa->lane_mask;
    BCMI_MADURA_MST_MISC0r_t mst_misc0;
    ...
    if((config_reg & 0x6060)== 0x6060 ){ /*if ULL mode is set then not need to set to repeater/retimer*/ 
	    PHYMOD_DEBUG_ERROR(("ULL already set so cant set retimer/ULL\n"));
	    return PHYMOD_E_NONE;
    }
    ...
    if(( Tx_timing_mode == MADURA_TX_REPEATER_ULL)&& ((  lane_mask == 0x1 )||(lane_mask == 0x2)||(lane_mask == 0x4)||(lane_mask == 0x8)||(lane_mask == 0x10)||(lane_mask == 0x20)||(lane_mask == 0x40)||(lane_mask == 0x80)) ){ 
    /*write ULL on all misc registers in 10G*/   
        BCMI_MADURA_MST_MISC0r_SET(mst_misc0,config_reg);
	BCMI_MADURA_MST_MISC1r_SET(mst_misc1,config_reg);
	BCMI_MADURA_MST_MISC2r_SET(mst_misc2,config_reg);
	BCMI_MADURA_MST_MISC3r_SET(mst_misc3,config_reg);
	...
	PHYMOD_IF_ERR_RETURN(
	    BCMI_MADURA_WRITE_MST_MISC3r(pa,mst_misc3));
    }else if (lane_mask == 0xF || lane_mask == 0x3 || lane_mask == 0x1 ) {
	    BCMI_MADURA_MST_MISC0r_SET(mst_misc0,config_reg);
	    PHYMOD_IF_ERR_RETURN(
			    BCMI_MADURA_WRITE_MST_MISC0r(pa,mst_misc0));
    } else  if (lane_mask == 0x2) {
...
int _madura_phy_interface_config_get(const phymod_access_t *pa, uint32_t flags, phymod_phy_inf_config_t *config)
{
    ...
    if (!lane_mask) {
        PHYMOD_DEBUG_ERROR(("Invalid Lanemask as %x\n",lane_mask));
        return PHYMOD_E_PARAM;
    }
...
int _madura_cfg_fw_ull_dp(const phymod_access_t* pa, phymod_datapath_t op_datapath) 
{
   ...
   /*is 0 i.e not enabled. It will be set to repeater/retimer by other api*/
       return PHYMOD_E_NONE; 
   }
    ...
    /* Set TX timing mode */
    MADURA_IF_ERR_RETURN_FREE( config_cpy.device_aux_modes, (
        _madura_interface_set_retimer_ull_mode_set(pa, ip, &config_cpy,  
                         Tx_timing_mode ))); 
