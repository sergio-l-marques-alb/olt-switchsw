 ...
 *  Revision      :  $Id: falcon2_dino_internal.c 1193 2015-10-21 21:50:52Z kirand $ *
 *                                                                                 *
...
{
    if (err_code != 0) {
        USR_PRINTF(("ERROR: SerDes err_code = %s\n", falcon2_dino_e2s_err_code(err_code)));
    }
    return err_code;
}
...
err_code_t falcon2_dinoset_lms_hoffset ( const phymod_access_t *pa, char delta) {
    if (delta != 0) {
...
SDK_STATIC err_code_t falcon2_dino_display_event(const phymod_access_t *pa,uint8_t event_id,
                          uint8_t entry_len,
                          uint8_t prev_cursor,
                          uint8_t curr_cursor,
                          uint8_t post_cursor,
...
/*SDK_STATIC*/ err_code_t _falcon2_dino_read_lane_state( const phymod_access_t *pa, falcon2_dino_lane_state_st *istate) {
  ...
  FALCON2_DINO_ESTM(state.reset_state = falcon2_dino_rd_lane_dp_reset_state());
...
SDK_STATIC err_code_t _falcon2_dino_display_lane_state_no_newline( const phymod_access_t *pa ) {
  ...
  const char* e2s_tx_osr_mode_enum[13] = {
  	"x1 ",
  	"x2 ",
  	"x4 ",
  	"ERR",
  ...
  if (state.tx_pll_select != 255) {
      FALCON2_DINO_EFUN_PRINTF(("   %01x, %01x,%1d,%1d)",state.reset_state,state.stop_state,state.tx_pll_select,state.rx_pll_select));
  } else {
...
SDK_STATIC err_code_t _falcon2_dino_display_core_state_no_newline( const phymod_access_t *pa ) {
    ...
    FALCON2_DINO_EFUN_PRINTF(("  %x,%02x  "         ,  state.core_reset, state.core_status));
    FALCON2_DINO_EFUN_PRINTF(("    %1d     "        ,  state.pll_pwrdn));
    FALCON2_DINO_EFUN_PRINTF(("   %1d    "          ,  state.uc_active));
    FALCON2_DINO_EFUN_PRINTF((" %3d.%2dMHz"         , (state.comclk_mhz/4),((state.comclk_mhz%4)*25)));    /* comclk in Mhz = heartbeat_count_1us / 4 */
    ...
    FALCON2_DINO_EFUN_PRINTF(("  %2d.%2dGHz ",state.vco_rate_mhz/1000, state.vco_rate_mhz % 1000));
...
SDK_STATIC err_code_t _falcon2_dino_display_pll_to_divider( const phymod_access_t *pa, uint8_t val) {
        ...
        case 0x13:  FALCON2_DINO_EFUN_PRINTF((" 198    "));
            break;
        default:    FALCON2_DINO_EFUN_PRINTF((" xxxxxxx "));
