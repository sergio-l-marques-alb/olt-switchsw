 ...
 */
#define ARAD_CREDIT_SIZE_BYTES_MIN 1

/*
 *  Maximal credit size, in Byte resolution
 */
#define ARAD_CREDIT_SIZE_BYTES_MAX ((8 * 1024) - 1)
...
#define ARAD_PLUS_FAP_CREDIT_VALUE_LOCAL 0
#define ARAD_PLUS_FAP_CREDIT_VALUE_REMOTE 1
#define ARAD_PLUS_FAP_CREDIT_VALUE_MAX 1
...
typedef enum
{
   ...
   * simple
   */
  ARAD_INIT_PDM_MODE_SIMPLE = 0,
  /*
   * Reduced
   */
  ARAD_INIT_PDM_MODE_REDUCED = 1,
  /*
   * numbet of modes
   */
  ARAD_INIT_PDM_NOF_MODES = 2
}ARAD_INIT_PDM_MODE;
...
typedef enum
{
  /* For short range connection - 1.5V */
  ARAD_MGMT_EXT_VOL_MOD_HSTL_1p5V=0,
  /* For long range connection - 3.3V */
  ARAD_MGMT_EXT_VOL_MOD_3p3V,
  /* Short range connection- 1.5V HSTL internal reference VDDO/2 */
...
typedef struct{
   ...
   * statistic info
   */
  ARAD_STAT_IF_REPORT_INFO stat_if_info;
 ...
 */
uint32
  arad_plus_mgmt_credit_worth_set_unsafe(
    SOC_SAND_IN  int    unit,
    SOC_SAND_IN  uint16    credit_worth_local,
    SOC_SAND_IN  uint16    credit_worth_remote
  );
 ...
 */
uint32
  arad_plus_mgmt_credit_worth_get_unsafe(
    SOC_SAND_IN  int    unit,
    SOC_SAND_OUT uint16    *credit_worth_local,
    SOC_SAND_OUT uint16    *credit_worth_remote
  );
