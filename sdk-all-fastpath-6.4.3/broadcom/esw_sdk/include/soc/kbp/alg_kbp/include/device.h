...
#define KBP_SDK_VERSION "KBP SDK 1.3.0"
 ...
 *
 * @retval KBP_OK on success.
 * @retval error on any error conditions.
 */

kbp_status kbp_device_print(struct kbp_device *device, FILE *fp);

/**
 * IFSR: In-Field-Soft-Repair
 *
 * This will read out the error occured locations and fix them
 * if the locations are not fixable return the error code.
 *
 * @param device The KBP device handle.
 *
 * @retval KBP_OK on success; fix the locations and re-write the entry at that location.
 * @retval error on any error conditions.
 */
