...
const DIAG_ALLOC_POOL_DEF pools[] =
{
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    |                Full                    |        Long     |   Short   |                 Pool                       |         Number Of        |              Function for          |          Function for              
    |              Pool name                 |     Pool Name   | Pool Name |                  ID                        |            Banks         |              SW Info Print         |      direct(HW) Info Print         
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    {  "Ingress LIF bank allocation"          , "InLif"         , "IL"     , dpp_res_arad_pool_ingress_lif_bank_start    , _BCM_DPP_AM_LIF_NOF_BANKS, NULL                              , cmd_diag_alloc_hw_lif_get_block   },
    {  "Egress encapsulation bank allocation" , "OutLif"        , "OL"     , dpp_res_arad_pool_egress_encap_bank_start   , _BCM_DPP_AM_LIF_NOF_BANKS, NULL                              , NULL                              },
    {  "FECs for global use"                  , "FEC"           , "FEC"    , dpp_am_res_fec_global                     , 1                        , diag_alloc_system_table_print_fec , cmd_diag_alloc_hw_fec_get_block   },
    {  "VLAN translation ingress usage"       , "InVLAN"        , "IVE"    , dpp_am_res_vlan_edit_action_ingress       , 1                        , diag_alloc_system_table_print_ive , cmd_diag_alloc_hw_ive_print       },
    {  "VLAN translation egress usage"        , "OutVLAN"       , "EVE"    , dpp_am_res_vlan_edit_action_egress        , 1                        , diag_alloc_system_table_print_eve , cmd_diag_alloc_hw_eve_print       },
    {  "VSIs for TB VLANS"                    , "VLAN"          , "VLAN"   , dpp_am_res_vsi_vlan                       , 1                        , NULL                              , NULL                              },
    {  "VSIs for MSTP"                        , "VSI"           , "VSI"    , dpp_am_res_vsi_mstp                       , 1                        , NULL                              , NULL                              },
    {  "FEC Failover id (Jericho)"            , "FEC_Failover"  , "FECF"   , dpp_am_res_failover_fec_id                , 1                        , NULL                              , NULL                              },
    {  "Ingress Failover id (Jericho)"        , "Ing_Failover"  , "INGF"   , dpp_am_res_failover_ingress_id            , 1                        , NULL                              , NULL                              },
    {  "Egress Failover id (Jericho)"         , "Eg_Failover"   , "EGF"    , dpp_am_res_failover_egress_id             , 1                        , NULL                              , NULL                              },
    {  "Failover id (Arad+ and below)"        , "Failover"      , "Fail"   , dpp_am_res_failover_common_id             , 1                        , NULL                              , NULL                              },
    {  "EEPs for local use"                   , "EEP"           , "EEP"    , dpp_am_res_eep_local                      , 1                        , NULL                              , NULL                              },
    {  "QOS INGRESS LABEL MAP ID"             , "QOS_ELSP"      , "QOS_E"  , dpp_am_res_qos_ing_elsp                   , 1                        , NULL                              , NULL                              },
    {  "QOS INGRESS LIF/COS IDs"              , "COS"           , "COS"    , dpp_am_res_qos_ing_lif_cos                , 1                        , NULL                              , NULL                              },
    {  "QOS INGRESS PCP PROFILE IDs"          , "PCP_VLAN"      , "PCP_V"  , dpp_am_res_qos_ing_pcp_vlan               , 1                        , NULL                              , NULL                              },
    {  "QOS INGRESS COS OPCODE IDs"           , "COS_Opcode"    , "COS_O"  , dpp_am_res_qos_ing_cos_opcode             , 1                        , NULL                              , NULL                              },
    {  "QOS EGRESS REMARK QOS IDs"            , "QOS_Remark"    , "QOS_R"  , dpp_am_res_qos_egr_remark_id              , 1                        , NULL                              , NULL                              },
    {  "QOS EGRESS MPLS PHP QOS IDs"          , "QOS_MPLS_PHP"  , "QOS_M"  , dpp_am_res_qos_egr_mpls_php_id            , 1                        , NULL                              , NULL                              },
    {  "number of meters in processor A"      , "Meter_A"       , "M_A"    , dpp_am_res_meter_a                        , 1                        , NULL                              , NULL                              },
    {  "number of meters in processor B"      , "Meter_B"       , "M_B"    , dpp_am_res_meter_b                        , 1                        , NULL                              , NULL                              },
    {  "SW handles of policer"                , "Policer"       , "POL"    , dpp_am_res_ether_policer                  , 1                        , NULL                              , NULL                              },
#ifdef BCM_ARAD_SUPPORT
    {  "ECMP id"                              , "ECMP"          , "ECMP"   , dpp_am_res_ecmp_id                        , 1                        , NULL                              , NULL                              },
#endif
    {  "QOS EGRESS L2 I TAG PROFILE IDs"      , "QOS_I_TAG"     , "QOS_I"  , dpp_am_res_qos_egr_l2_i_tag               , 1                        , NULL                              , NULL                              },
#ifdef BCM_88660
    {  "QOS EGRESS DSCP/EXP MARKING PROFILE ID,s", "QOS_Mark"    , "QOS_M"  , dpp_am_res_qos_egr_dscp_exp_marking       , 1                        , NULL                              , NULL                              },
#endif                                           
    {  "RP id"                                 , "IPMC_RP"       , "IPMC_RP", dpp_am_res_rp_id                          , 1                        , NULL                              , NULL                             },
    {  "OAM MA Index"                          , "OAM_MA"        , "OAM_M"  , dpp_am_res_oam_ma_index                   , 1                        , NULL                              , NULL                             },
    {  "OAM MEP Index for short MA name format", "OAM_MEPSort"   , "OAM_S"  , dpp_am_res_oam_mep_id_short               , 1                        , NULL                              , NULL                             },
    {  "OAM MEP Index for ccm MA name format"  , "OAM_MEPLong"   , "OAM_L"  , dpp_am_res_oam_mep_id_long                , 1                        , NULL                              , NULL                             },
    {  "OAM RMEP Index"                        , "OAM_RMEP"      , "OAM_R"  , dpp_am_res_oam_rmep_id                    , 1                        , NULL                              , NULL                             },
    {  "OAM trap code upmep ftmh header"       , "OAM_TrapCode"  , "OAM_T"  , dpp_am_res_oam_trap_code_upmep_ftmh_header, 1                        , NULL                              , NULL                             },
    {  "Profiles for PON use"                  , "PON_CHN_Prof"  , "PON"    , dpp_am_res_pon_channel_profile            , 1                        , NULL                              , NULL                             },
    {  "Trill virtual nickname"                , "TrillNick"     , "TRILL"  , dpp_am_res_trill_virtual_nick_name        , 1                        , NULL                              , NULL                             }
};
#endif
...
const char*  DIAG_ACTION_VID_to_string( bcm_vlan_action_t enum_val)
{
    const char* str = NULL;
    switch(enum_val) 
    {
...
const char*  DIAG_ACTION_PCP_to_string( bcm_vlan_action_t enum_val)
{
    const char* str = NULL;
    switch(enum_val) 
    {
...
cmd_result_t diag_alloc_gport_info_get(int unit, 
    ...
    ret = _bcm_dpp_gport_to_lif(unit, 
                          gport_info->gport_id, 
                          &(gport_info->lif_id), 
    ...
    if (SOC_DPP_ENCAP_ID_IS_EGRESS_ONLY(encap_id)) {
        gport_info->lif_id = -1;
    } 
    /* if gport is ingress only, outlif not exist */
    else if (SOC_DPP_ENCAP_ID_IS_INGRESS_ONLY(encap_id)) {
    ...
    if (gport_info->encoding == diag_alloc_gport_info_encoding_one_plus_one_protection) {
        /* multicast id */
        gport_info->multicast_id = SOC_DPP_MC_GROUP_FROM_ENCAP_ID(encap_id);
        /* is primary */
        gport_info->multicast_is_primary = SOC_DPP_MC_WORKING_FROM_ENCAP_ID(encap_id);
    }

    /* get forwarding database info */

    if (gport_params->forwarding_database && !SOC_DPP_ENCAP_ID_IS_INGRESS_ONLY(encap_id)) {
        /* for vlan port id */ 
        if (gport_info->gport_type == diag_alloc_gport_info_gport_type_vlan ||
            gport_info->gport_type == diag_alloc_gport_info_gport_type_mpls) {
            ret = _bcm_dpp_sw_db_hash_vlan_find(unit,
                                                (&gport_info->gport_id), 
                                                (shr_htb_data_t*)(void *)&gport_info->forwarding_info_data,
                                                FALSE);
            if (ret != BCM_E_NONE) {
...
void diag_alloc_gport_info_print(int unit, 
    ...
    if (gport_info->lif_id > 0) {
        cli_out(",LIF-ID: %d", gport_info->lif_id);
    }
    /* print fec if exist */
    ...
    if (gport_params->resource) {
        /* inLif */
        if (gport_info->lif_id <= 0) {
            cli_out("InLif:  Not-Exist\n");
        } else if (gport_info->is_remote) {
            cli_out("InLif: Remote %d\n", gport_info->lif_id);
        } else {
            cli_out("InLif:  Exists %d\n", gport_info->lif_id);
        }

        /* outLif */
        if (gport_info->out_lif_id <= 0) {
            cli_out("OutLif: Not-Exist\n");
        } else if (gport_info->is_remote) {
            cli_out("OutLif: Remote %d\n", gport_info->out_lif_id);
        } else {
            cli_out("OutLif: Exists %d\n", gport_info->out_lif_id);
        }
...
cmd_diag_alloc_sw_common_eval_object(int unit, 
                                        ...
                                        int pool_idx) { 
    int pool_id_start = -1; /* first pool id */
    int nbrOfPools = -1; /* number of pools */
    int pool_id; /* current pool id, from pool_id_start to pool_id_start + nbrOfPools-1 */
    int low_id; /* base id for the current pool */
    int nbr_entries; /* number of entries in the pool */ 
    uint32 ret;   
    shr_res_pool_info_t pool_info; /* get number of used id for current pool */
    ...
    pool_id_start = pools[pool_idx].pool_id_start;
    nbrOfPools    = pools[pool_idx].number_of_pools;

    /* for each pool id, display ids */
    for (pool_id=pool_id_start; pool_id<pool_id_start+nbrOfPools; ++pool_id) {
        /* get the lowest id for the current pool and the number of entries */
        ret = shr_res_pool_get(unit,pool_id,0,&low_id,&nbr_entries,0,0);
        if (ret != BCM_E_NONE) {
        ...
        ret = shr_res_pool_info_get(unit, pool_id, &pool_info);
        if (ret != BCM_E_NONE) {
    ...
    if(pool_info.used)
        ...
        while (number_entries_used_found != pool_info.used && id<low_id+nbr_entries-1) {

            /* found an id in use to display */
            if (shr_res_check(unit,pool_id,1,id) == BCM_E_EXISTS) {
...
cmd_diag_alloc_hw_ive_print(int unit, diag_alloc_system_table_parameters *parameters)
{
    ...
    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_KEY command_key;
    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_INFO command_info;

    is_advanced_mode = SOC_DPP_IS_VLAN_TRANSLATE_MODE_ADVANCED(unit);

    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_KEY_clear(&command_key);
    SOC_PPD_LIF_ING_VLAN_EDIT_COMMAND_INFO_clear(&command_info);

    for (command_id = parameters->from; command_id <= parameters->to; command_id++) {
        is_first_action_print=1;
        if (shr_res_check(unit,dpp_am_res_vlan_edit_action_ingress,1,command_id) == BCM_E_EXISTS){
                    ...
                    if (command_id==tmp_command_id) {
                        ...
                        if (is_advanced_mode) {
                            cli_out("| %-7d|| %-7d| %-8d|| %-7d| %-8d| %-9s | %-6d | %-14s || %-9s | %-6d | %-14s |\n\r",
                                    ...
                                    SOC_PPD_LIF_ING_VLAN_EDIT_TAG_PCP_DEI_SRC_to_string(command_info.outer_tag.pcp_dei_source), command_info.outer_tag.tpid_index, SOC_PPD_LIF_ING_VLAN_EDIT_TAG_VID_SRC_to_string(command_info.outer_tag.vid_source),
                                    SOC_PPD_LIF_ING_VLAN_EDIT_TAG_PCP_DEI_SRC_to_string(command_info.inner_tag.pcp_dei_source), command_info.inner_tag.tpid_index, SOC_PPD_LIF_ING_VLAN_EDIT_TAG_VID_SRC_to_string(command_info.inner_tag.vid_source));
                        }
                        else{
...
cmd_diag_alloc_hw_eve_print(int unit, diag_alloc_system_table_parameters *parameters)
{
    ...
    SOC_PPD_EG_VLAN_EDIT_COMMAND_KEY_clear(&command_key);
    SOC_PPD_EG_VLAN_EDIT_COMMAND_INFO_clear(&command_info);
    ...
    for (entry_id = parameters->from; entry_id <= parameters->to; entry_id++) {
        ...
        if (is_advanced_mode) {
                cli_out("| %-7d|| %-7d| %-8d|| %-7d| %-9s | %-6d | %-14s || %-9s | %-6d | %-14s |\n\r",
                        ...
                        SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_to_string(command_info.outer_tag.pcp_dei_source), command_info.outer_tag.tpid_index, SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_to_string(command_info.outer_tag.vid_source),
                        SOC_PPD_EG_VLAN_EDIT_TAG_PCP_DEI_SRC_to_string(command_info.inner_tag.pcp_dei_source), command_info.inner_tag.tpid_index, SOC_PPD_EG_VLAN_EDIT_TAG_VID_SRC_to_string(command_info.inner_tag.vid_source));
            }
            else{
...
cmd_diag_alloc_hw_lif_table_get_block_and_print(int unit, 
    ...
    uint32 cur_indx,
           nof_entries_to_print,
           total_to_print;
    SOC_PPD_LIF_ENTRY_INFO *prm_entries_array=NULL;
    uint32 prm_nof_entries, 
           total_nof_entries=0;
    uint32 ret;   
    ...
    prm_entries_array = (SOC_PPD_LIF_ENTRY_INFO*)sal_alloc(sizeof(SOC_PPD_LIF_ENTRY_INFO) * prm_block_range->entries_to_act, "prm_entries_array");
...
cmd_diag_alloc_hw_lif_get_block(int unit, 
    ...
    prm_rule.entries_type_bm = SOC_PPD_LIF_ENTRY_TYPE_ALL;
    prm_block_range.entries_to_act = DIAG_ALLOC_BLOCK_SIZE;
...
cmd_diag_alloc_init(int unit, args_t *a, int pool_idx) {
    ...
    ret = diag_alloc_system_table_parameters_parse(unit, a, &parameters);
    if (ret != CMD_OK) {
...
cmd_diag_alloc_all_print(int unit)
{
    ...
    int pool_id_start; /* first pool id */
    int nbrOfPools; /* number of pools */
    int pool_id; /* current pool id, from pool_id_start to pool_id_start + nbrOfPools-1 */
    int low_id; /* base id for the current pool */
    int lowest_id;
    int nbr_entries; /* number of entries in the pool */ 
    ...
    for (i = 0; i < sizeof(pools) / sizeof(DIAG_ALLOC_POOL_DEF); i++) {
        ...
        tot_used_entries=0;
        tot_nbr_entries=0;
        pool_id_start = pools[i].pool_id_start; 
        nbrOfPools = pools[i].number_of_pools; 
                                ...
                                char* allocated_resource)
{
    ...
    cli_out("\n\t\tCosq scheduler allocation manager\n\n");
    cli_out("\t\t=================================\n\n");

    if (-1 != flow_id) {
        quad_start_flow_id = (flow_id/4)*4;
        while(flow_count-- > 0) {
            rc = bcm_dpp_am_cosq_get_region_type(unit,flow_id,&region_type);
            if (rc != BCM_E_NONE) {
                return CMD_FAIL;
            }
            rc = bcm_dpp_am_cosq_fetch_quad(unit,flow_id,region_type,&is_allocated);
            if (rc != BCM_E_NONE) {
                return CMD_FAIL;
            }
    ...
    if (allocated_resource) {
        ...
        rc = bcm_dpp_am_cosq_fetch_allocated_resources(unit, flow_type, &allocated_ref);
        if (rc != BCM_E_NONE) {
            return CMD_FAIL;
        }
