...
dpp_diag_egq_graphic(int unit, int port_num) {
   ...
   if (dpp_diag_port_num_valid(unit,port_num)) {
      ...
      rv = soc_port_sw_db_tm_port_to_out_port_priority_get(unit, core, pps_port_index, &port_nof_q_pairs);
      if(BCM_FAILURE(rv)) { 
          return CMD_FAIL;
      }
      rv = soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, pps_port_index, &pps_base_q_pair);
      if(BCM_FAILURE(rv)) {
          return CMD_FAIL;
      }
      ...
      if (soc_port_sw_db_tm_to_local_port_get(unit, core, pps_port_index, &local_port_gport)<0) return CMD_FAIL; /*map tm port(pps_port_index) to local port*/
