...
#define PW_INIT_STRINGS \
    ...
    int         pkt_size;
    bcm_rx_cfg_t  rx_cfg;
    COMPILER_UINT64 rx_cosq_bmp [ N_DMA_CHAN+1]; /* queues to monitor on each channel */
...
char pw_usage[] =
    ...
    "\tchannel x [cosq=bmp L0node=number] [y z] - configure PW to use the specified RX channels\n"
    "\t                           - bmp is a cosq bitmap for specified channel\n"
    "\t                           - number <0..5> is a MMU L0 node where bmp cosq connected to\n"
...
_pw_start_op(int unit)
{
	...
	pu->rx_cfg.global_pps = pu->rate;
        ...
        /* update cosq for each channel */
        for (chan = 0; chan < BCM_RX_CHANNELS; chan++) {
            int iter;
            for ( iter = 0; iter < 2; ++iter ) {
                int queue = ( iter == 0 )? 0 : 32;

                cosqs = ( iter == 0 )? u64_L ( pu -> rx_cosq_bmp[chan] ) :
                                       u64_H ( pu -> rx_cosq_bmp[chan] );
                while ( cosqs ) {
                    if ( cosqs & 0x1 ) {
                        rv = bcm_rx_queue_channel_set(unit, queue, chan);
                        if ( rv < 0 ) {
                            cli_out( 
                                    "PW queue channel set: Cannot set queue %d: for channel %d. (%s)\n", 
                                    queue, chan, bcm_errmsg(rv));
                            return rv;
                        }
                    }
                    cosqs = cosqs >> 1;
...
static bcm_rx_chan_cfg_t default_rx_chan_cfg = {
 ...
 */
{
	...
	if (pu->pu_channel) {
        ...
        for (chan = 0; chan < BCM_RX_CHANNELS; chan++) {
            cli_out("    Channel %d:  Chains %d. PPS %d. COSBMP 0x%08x%08x.\n",
                   chan, rx_cfg -> chan_cfg[chan].chains,
                   rx_cfg -> chan_cfg[chan].rate_pps,
                   u64_H ( pu -> rx_cosq_bmp[chan] ), 
                   u64_L ( pu -> rx_cosq_bmp[chan] ));
        }
	...
	if (!sal_strcasecmp(c, "start")) {
		...
		return(CMD_FAIL);
            ...
            while ((c = ARG_CUR(a)) != NULL && !parse_mask(c, pw_channel_table,
                ...
                /* check if there is cosq config*/
                if (!sal_strncasecmp(c, "cosq=", 5)) {
                    sal_memcpy(&(rx_cfg -> chan_cfg[chan]), &default_rx_chan_cfg,
                               sizeof(bcm_rx_chan_cfg_t));
                    pval = parse_uint64(c + 5);
                    COMPILER_64_SET( pu -> rx_cosq_bmp[chan], COMPILER_64_HI(pval), COMPILER_64_LO(pval));
                    ARG_NEXT(a);    /* Bump arg if matched report */

                    c = ARG_CUR(a);
                    if ( c != NULL ) {
                        if (!sal_strncasecmp(c, "L0node=", 7)) {
                            l0_node = parse_integer(c + 7);
                            if (( l0_node >= 0 ) && ( l0_node < 6 )) {
                                COMPILER_64_BITSET( (pu -> rx_cosq_bmp[chan]), ( 58 + l0_node ));
                                ARG_NEXT(a);    /* Bump arg if matched report */
                                continue;
                            } else {
...
pw_dump_packet_rx(int unit, pup_t *pup, uint32 report)
{
    ...
    if (report & REPORT_RAW) {
        ...
        if ((pkt->flags & BCM_PKT_STK_F_SRC_PORT) && 
            ...
            cli_out("%sdest-gport %d. src-gport %d. "
            ...
            cli_out("%sdest-port %d. dest-mod %d. src-gport %d. "
                    "opcode %d. %s matched %d. classification-tag %d.\n",
            ...
            cli_out("%sdest-gport %d. %s %d. src-mod %d. "
                    "opcode %d. %s matched %d. classification-tag %d.\n",
                    pfx, pkt->dst_gport, 
        ...
        cli_out("%sdest-port %d. dest-mod %d. %s %d. src-mod %d. "
                "opcode %d. %s matched %d. classification-tag %d.\n",
                pfx, pkt->dest_port, pkt->dest_mod,
                (pkt->flags & BCM_PKT_F_TRUNK) ? "src-trunk" : "src-port",
                (pkt->flags & BCM_PKT_F_TRUNK) ? pkt->src_trunk : pkt->src_port, 
 ...
 */
{
    ...
    if (SOC_IS_ROBO(unit)) {
        ...
        if (rv < 0) {
        	...
        	cli_out("Failed to DMA descriptor\n");
        }
