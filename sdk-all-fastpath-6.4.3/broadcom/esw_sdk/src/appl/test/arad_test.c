...
int arad_bist_test(int unit, args_t *a, void *p)
{
    ...
    SOC_PBMP_ITER(abt->dram_ndx, dram_ndx) {
        ...
        if (SOC_IS_ARAD(unit) && (!SOC_DPP_CONFIG(unit)->arad->init.dram.is_valid[dram_ndx])) { 
            LOG_INFO(BSL_LS_APPL_TESTS,
                     (BSL_META_U(unit,
                                 "arad_bist_test: Disabled Interface=%d, skipping BIST\n"), dram_ndx));
            continue; 
        } 
        ...
        if (SOC_SAND_FAILURE(soc_sand_rc)) {
           LOG_INFO(BSL_LS_APPL_TESTS,
                    (BSL_META_U(unit,
                                "Error: arad_dram_read_bist_err_cnt () Failed:\n")));
           arad_bist_fail = 0x1;
           goto done;
        }
        ...
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Number of errors occured: 0x%x\n"), bist_err_occur));
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Number of full errors: %d\n"), bist_full_err_cnt));
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Number of single errors: %d\n"), bist_single_err_cnt));
        LOG_INFO(BSL_LS_APPL_TESTS,
                 (BSL_META_U(unit,
                             "Number of global errors: %d\n"), bist_global_err_cnt));
