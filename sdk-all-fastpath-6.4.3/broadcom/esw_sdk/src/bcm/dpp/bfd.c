...
int _bcm_bfd_mep_db_entry_struct_set(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
    ...
    if (endpoint_info->type == bcmBFDTunnelTypePweGal && SOC_IS_JERICHO(unit)) {
    ...
    if ((endpoint_info->type == bcmBFDTunnelTypeUdp) || (endpoint_info->type == bcmBFDTunnelTypeMpls)) {
		...
		|| (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop == 1) ) {
			...
			if ((SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop == 1) &&
                ...
                rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_REF_COUNTER, &ref_counter);
                BCMDNX_IF_ERR_EXIT(rv);
                if (ref_counter == 0) {
                    int temp = endpoint_info->ip_tos;
                    rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_TOS_VALUE, &temp);
                    BCMDNX_IF_ERR_EXIT(rv);
                } else {
                    int tos = 0;
                    rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_TOS_VALUE, &tos);
                    BCMDNX_IF_ERR_EXIT(rv);
                ...
                ref_counter++;
                rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_REF_COUNTER, &ref_counter);
                BCMDNX_IF_ERR_EXIT(rv);
            } else {
...
int _bcm_bfd_mep_db_entry_struct_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
	...
	case SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_M_HOP:	
		...
		if (mep_db_entry->tos_ttl_profile == _BCM_BFD_IPV4_SINGLE_HOP_TTL_PROFILE) {
			int temp;
			rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_TOS_VALUE, &temp);
			BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_mep_db_entry_dealloc(int unit, SOC_PPD_OAM_OAMP_MEP_DB_ENTRY *mep_db_entry) {
	...
	if (mep_db_entry->tos_ttl_profile == _BCM_BFD_IPV4_SINGLE_HOP_TTL_PROFILE) {
		int tmp;
		rv = SOC_DPP_WB_ENGINE_GET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_REF_COUNTER, &tmp);
		BCMDNX_IF_ERR_EXIT(rv);
		if (tmp>0) {
			tmp--;
			rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_REF_COUNTER, &tmp);
			BCMDNX_IF_ERR_EXIT(rv);
		}
	}
...
int _bcm_bfd_classifier_mep_entry_struct_get(int unit, bcm_bfd_endpoint_info_t *endpoint_info, SOC_PPD_OAM_CLASSIFIER_MEP_ENTRY *classifier_mep_entry, int *dip_ndx) {
    ...
    if ( (endpoint_info->flags & BCM_BFD_ENDPOINT_IN_HW )==0 || SOC_IS_ARADPLUS_AND_BELOW(unit) ) {
        /* In other cases the type has already been set according to the MEP DB.*/
...
int bcm_petra_bfd_init(int unit) {
	...
	rv = soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_BFD_S_HOP);
	BCMDNX_IF_ERR_EXIT(rv);

	rv = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_S_HOP_TOS_VALUE, &tmp);
	BCMDNX_IF_ERR_EXIT(rv);
...
/* In warmboot oam init is called first and inits DBs, so we don't need to init them from BFD */
    ...
    if (soc_property_get(unit, spn_MCS_LOAD_UC0, 0)) {
        (void)bcm_petra_bfd_uc_init(unit);
    }
...
#endif /*BCM_WARM_BOOT_SUPPORT*/    
    {
		...
		if (soc_property_get(unit, spn_BFD_ECHO_ENABLED, 0) == 1) { 
			int trap_code_tmp = SOC_PPC_TRAP_CODE_BFD_IPV4_YOUR_DISCR_NOT_FOUND;
			rv = bcm_petra_rx_trap_type_create(unit, BCM_RX_TRAP_WITH_ID, bcmRxTrapUserDefine, &trap_code_tmp);
			BCMDNX_IF_ERR_EXIT(rv);
			
			rv = _bcm_dpp_rx_trap_set(unit, SOC_PPD_TRAP_CODE_BFD_IPV4_YOUR_DISCR_NOT_FOUND, &trap_config_cpu, _BCM_OAM_TRAP_FWD_OFFSET_INDEX);
			BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_bfd_endpoint_create(int unit, bcm_bfd_endpoint_info_t *endpoint_info) {
    ...
    if (bcm_petra_bfd_uc_is_init(unit)) {
        if (endpoint_info->flags & BCM_BFD_ENDPOINT_IPV6) {
            ...
            _BCM_BFD_UC_MEP_INDEX_TO_INTERNAL(id_internal, endpoint_info->id); 
            endpoint_info->id = id_internal;
            rv = bcm_petra_bfd_uc_endpoint_create(unit, endpoint_info);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if ((endpoint_info->type != bcmBFDTunnelTypeUdp) && (endpoint_info->type != bcmBFDTunnelTypeMpls) && (endpoint_info->type != bcmBFDTunnelTypePweControlWord)
        ...
        if (SOC_IS_ARAD_B1_AND_BELOW(unit) || (endpoint_info->type != bcmBFDTunnelTypePweRouterAlert)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
    ...
    if (SOC_IS_JERICHO(unit) && ((endpoint_info->type == bcmBFDTunnelTypePweRouterAlert) || (endpoint_info->type == bcmBFDTunnelTypePweTtl))) {
    ...
    if (!_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info)) {
        ...
        if (endpoint_info->tx_gport != BCM_GPORT_INVALID &&  !BCM_GPORT_IS_SYSTEM_PORT(endpoint_info->tx_gport) &&
            !BCM_GPORT_IS_MODPORT(endpoint_info->tx_gport) &&  !BCM_GPORT_IS_TRUNK(endpoint_info->tx_gport)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
	...
	/*check if we are in bfd ipv4 single hop extended mode*/
	if (SOC_DPP_CONFIG(unit)->pp.bfd_ipv4_single_hop == 1){
		if ((endpoint_info->type == bcmBFDTunnelTypeUdp) && !(endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) && (is_default==0)
		&& (endpoint_info->flags &BCM_BFD_ENDPOINT_IN_HW)){
			is_1_hop_extended =1; /* 1= accelerated 2 = non accelerated*/
		}
		else if ((endpoint_info->type == bcmBFDTunnelTypeUdp) && !(endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) && (is_default==0)){
			is_1_hop_extended =2;
		}
	}
        ...
        if (is_default) {
            ...
            if (_BCM_BFD_ENDPOINT_IS_ACCELERATED_TO_OAMP(*endpoint_info) &&
                ...
                if (endpoint_info->type == bcmBFDTunnelTypeMpls)  {
                    ...
                    if ((endpoint_info->ip_ttl != 1)) {
                        /*TTL&TOS*/
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                            (_BSL_BCM_MSG("Error: For BFD over MPLS supporting only ttl 1. Given value: %d"), endpoint_info->ip_ttl));
                    }
                    ...
                    if ((endpoint_info->flags & BCM_BFD_ENDPOINT_MULTIHOP) == 0) {
                        if ((endpoint_info->ip_ttl != 255)) {
                            /*TTL&TOS*/
                            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                                                (_BSL_BCM_MSG("Error: For IP One Hop supporting only ttl 255. Given value: %d"), endpoint_info->ip_ttl));
                        }
						if (!is_1_hop_extended){
							if (endpoint_info->dst_ip_addr && SOC_IS_ARADPLUS_AND_BELOW(unit)) {
									BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
													(_BSL_BCM_MSG("Error: For IP One Hop: dest ip address field should remain blank")));
							}
							if (SOC_IS_JERICHO(unit) &&  endpoint_info->ip_subnet_length >32) {
								BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
													(_BSL_BCM_MSG("Error: IP subnet length must be <=32"))); 
							} else if (SOC_IS_ARADPLUS_AND_BELOW(unit) &&  endpoint_info->ip_subnet_length ) {
								BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
            ...
            if (endpoint_info->flags & BCM_BFD_ENDPOINT_HW_ACCELERATION_SET) {
                ...
                if (!(endpoint_info->flags & BCM_BFD_ECHO) && (is_1_hop_extended ==0))
				 {
                    SOC_PPD_OAM_LIF_PROFILE_DATA_clear(&profile_data);
					...
					/*accelerated*/
					if ((is_1_hop_extended == 1) && (is_default == 0) && !(endpoint_info->flags & BCM_BFD_ENDPOINT_UPDATE)) {
						rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_bfd_ipv4_1_hop_lem_entry_add, (unit, endpoint_info->local_discr, 1));
						BCMDNX_IF_ERR_EXIT(rv);
						classifier_mep_entry.ma_index = endpoint_info->local_discr; /*in single hop the ma_index will save the local discriminator. 
																					This will be used for deleting the entry from the LEM*/
					}
					/* non accelerated*/
					if ((is_1_hop_extended == 2) && (is_default == 0) && !(endpoint_info->flags & BCM_BFD_ENDPOINT_UPDATE)) {
						rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_bfd_ipv4_1_hop_lem_entry_add, (unit, endpoint_info->local_discr, 0));
						BCMDNX_IF_ERR_EXIT(rv);
						classifier_mep_entry.ma_index = endpoint_info->local_discr; /*in single hop the ma_index will save the local discriminator. 
																					This will be used for deleting the entry from the LEM*/
					}
...
int bcm_petra_bfd_endpoint_destroy(
              ...
              || (classifier_mep_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP)) )
 {
	...
	if (is_single_hop_extended)/*sigle hop extended mode*/ {
		rv= MBCM_PP_DRIVER_CALL(unit, mbcm_pp_bfd_ipv4_1_hop_lem_entry_remove, (unit, classifier_mep_entry.ma_index)); 
		BCMDNX_IF_ERR_EXIT(rv);
	}
