...
_bcm_dpp_out_lif_match_delete(int unit, int out_lif)
{
	...
	rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_OUTLIF_INFO, &new_match, out_lif);
	BCMDNX_IF_ERR_EXIT(rv);
...
{
    int rv = BCM_E_NONE, in_lif_id, out_lif_id, fec_id;
    ...
    *used_type = _bcmDppLifTypeAny;
    ...
    rv = _bcm_dpp_gport_to_lif(unit, gport, &in_lif_id, &out_lif_id, &fec_id, NULL);
    if (rv != BCM_E_NOT_FOUND) {
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if(is_local_conf || _Bcm_dpp_sync_all){
            ...
            if (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)) {
                ...
                if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
                    eep_pwe_info.lif_profile = old_pwe_info->lif_profile;
                }
                rv = arad_pp_profile_mgmt_if_profile_val_set(
                    unit,
                    SOC_PPC_PROFILE_MGMT_TYPE_OUTLIF,
                    SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_ORIENTATION,
                    profile_split_horizon_val,
                    &(eep_pwe_info.lif_profile)
                    );
                BCMDNX_IF_ERR_EXIT(rv);
            } else {
 ...
 )
{
    ...
    if ((BCM_GPORT_IS_VLAN_PORT(gport_id) || BCM_GPORT_IS_MPLS_PORT(gport_id)) && (SOC_DPP_ENCAP_TYPE_GET(gport_id) == _DPP_ENCAP_ID_LIF_VAL)){
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_l2_interface_determine_ingress_egress(gport_id, &is_ingress, &is_egress));
    }
    ...
    if(BCM_GPORT_IS_MPLS_PORT(gport_id) && (is_egress && !is_ingress)) {
        ...
        if(gport_type_encoding == _BCM_DPP_GPORT_TYPE_ENCAP) {
            if (lif_id != NULL) {
              *lif_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID; /* Egrees only */
            }
    ...
    if (BCM_GPORT_IS_MPLS_PORT(port_id)){
        if (!remote_lif && ((is_local) || (_Bcm_dpp_sync_all))) {
            ...
            if (is_egress && is_ingress) {
                rv = bcm_dpp_am_l2_vpn_pwe_eep_dealloc(unit,out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
                ...
                rv = bcm_dpp_am_mpls_eep_dealloc(unit, out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
            else if (!is_egress && is_ingress) {
            ...
            if (is_egress) {
                /* Remove entry from SW DB */
                rv = _bcm_dpp_out_lif_match_delete(unit,out_lif_id);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
    ...
    SOC_PPC_EG_ENCAP_PWE_INFO *old_pwe_info)
{
    ...
    if (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)) {
        ...
        if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
            pwe_info->lif_profile = old_pwe_info->lif_profile;
        }
        rv = arad_pp_profile_mgmt_if_profile_val_set(
            unit,
    ...
    */
    if(encap_with_id) {
        ...
        if (lif_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID) { /* If the lif id is valid, then then WITH_ID flag is for the lif allocation. */
            lif_flags = BCM_DPP_AM_FLAG_ALLOC_WITH_ID;
        }
    ...
    bcm_mpls_port_t *   mpls_port)
{
   ...
   _bcm_lif_type_e used_type;
    ...
    if (!soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0) && (mpls_port->network_group_id > 0)) {
