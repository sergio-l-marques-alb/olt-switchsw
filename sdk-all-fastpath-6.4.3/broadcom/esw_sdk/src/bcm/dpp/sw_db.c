...
#define IS_IMMEDIATE_SYNC(unit) (0)
...
/************************************************************************/
/*                   sync/immediate_sync functions                      */
/************************************************************************/

#ifdef BCM_WARM_BOOT_SUPPORT_VLAN_REMOVED 

int
_bcm_sw_db_multi_device_sync_flags_sync(int unit)
{
...
int
_bcm_sw_db_multi_device_sync_flags_immediate_sync(int unit)
{
    BCMDNX_INIT_FUNC_DEFS;
...
int
_bcm_sw_db_l3_immediate_sync(int unit)
{ 
    BCMDNX_INIT_FUNC_DEFS;
...
int
_bcm_sw_db_switch_sync(int unit)
{ 
    BCMDNX_INIT_FUNC_DEFS;
    
    /* make sure to include dos attack and urpf mode */
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_sw_db_switch_urpf_mode_set(int unit, int urpf_mode)
{
    ...
    SOC_DPP_STATE(unit)->l3_state->urpf_mode = urpf_mode;

    MARK_SCACHE_DIRTY(unit);
#ifdef BCM_WARM_BOOT_SUPPORT
    rv = _bcm_dpp_wb_l3_update_urpf_mode(unit);
#endif
...
_bcm_sw_db_switch_urpf_mode_get(int unit, int *urpf_mode)
{
    ...
    *urpf_mode = SOC_DPP_STATE(unit)->l3_state->urpf_mode;
...
int _bcm_sw_db_sync(int unit)
{
    ...
    if (pp_enable) {
        rv = _bcm_dpp_vlan_all_wb_state_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (pp_enable) {
        rv = _bcm_sw_db_vlan_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }

    rv = _bcm_sw_db_multi_device_sync_flags_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (pp_enable) {
        ...
        rv = _bcm_sw_db_l3_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    if (pp_enable) {
        
        rv = _bcm_sw_db_switch_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }

#endif /*BCM_WARM_BOOT_SUPPORT_VLAN_REMOVED*/ 

    rv = _bcm_dpp_wb_port_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
         
    rv = _bcm_dpp_wb_stack_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
         
    rv = _bcm_dpp_wb_trunk_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
         
    rv = _bcm_dpp_wb_counters_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    if (pp_enable) {
      rv = _bcm_dpp_wb_gport_sync(unit);
      BCMDNX_IF_ERR_EXIT(rv);
    }

    rv = _bcm_dpp_wb_cosq_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    if (pp_enable){ 
        rv = _bcm_dpp_wb_qos_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (pp_enable) {
        ...
        rv = _bcm_dpp_field_all_wb_state_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
#endif /* _BCM_DPP_FIELD_WARM_BOOT_SUPPORT */
    if (pp_enable) {
        rv = _bcm_dpp_wb_l2_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = _bcm_dpp_wb_l3_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);

        if (SOC_DPP_CONFIG(unit)->trill.mode) {
            rv = _bcm_dpp_wb_trill_sync(unit);
            BCMDNX_IF_ERR_EXIT(rv);
        }
     	 
        rv = _bcm_dpp_wb_policer_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);

       rv = _bcm_dpp_wb_stg_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = _bcm_dpp_wb_vswitch_sync(unit);
        BCMDNX_IF_ERR_EXIT(rv);
    }
#ifdef BCM_ARAD_SUPPORT
#endif

    rv = _bcm_dpp_wb_mirror_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);

    if (SOC_IS_PETRAB(unit)) { /* Petra only multicast code */
        BCMDNX_IF_ERR_EXIT(_bcm_dpp_wb_multicast_sync(unit));
    }

    rv = _bcm_dpp_wb_switch_sync(unit);
    BCMDNX_IF_ERR_EXIT(rv);
...
#ifdef BCM_WARM_BOOT_SUPPORT
#ifdef BCM_PETRAB_SUPPORT
  rv =_bcm_dpp_wb_mirror_update_mirror_mode(unit);
  if (BCM_E_NONE != rv) {
      BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG_NO_UNIT("unable to update WB mirror mode unit"
                                                 " %d : %d (%s)\n"),
                                unit,
                                rv,
                                _SHR_ERRMSG(rv)));
...
#ifdef BCM_WARM_BOOT_SUPPORT
#ifdef BCM_PETRAB_SUPPORT
  rv =_bcm_dpp_wb_mirror_update_port_profile(unit, port);
  if (BCM_E_NONE != rv) {
      BCMDNX_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG_NO_UNIT("unable to update WB port profile unit"
                                                 " %d port %d : %d (%s)\n"),
                                unit,
                                port,
                                rv,
