    ...
    if (is_init) {
        ...
        if (SOC_DPP_CONFIG(unit)->pp.pon_application_enable) {
            for (port_i = 0; port_i < _BCM_PPD_NOF_PON_PP_PORT; port_i++) {
                BCM_PBMP_PORT_REMOVE(_bcm_dpp_vlan_unit_state[unit]->vlan_info.defl_ubmp,
                                port_i);
            }
        }
    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop != SOC_PPC_AC_ID_INVALID) {
        ...
        rv = _bcm_dpp_in_lif_ac_match_add(unit, &vlan_port, SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != SOC_PPC_AC_ID_INVALID) {
        ...
        rv = _bcm_dpp_in_lif_ac_match_add(unit, &vlan_port, SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    uint32 flags) /* flags : same as BCM_VLAN_GPORT_ADD_XXX */
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        rv = _bcm_pbmp_filter_by_core(unit, core, pbmp_sum, &core_pbmp);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = pbmp_to_ppd_port_bitmap(unit, &core_pbmp, ports, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_pbmp_t pbmp)
{
    ...
    bcm_port_t   port_i;
    bcm_pbmp_t   pbmp_sum, pbmp_neg, vlan_domain_remove, vlan_domain_left, core_pbmp;
    uint8    tagged;
    SOC_PPD_PORT soc_ppd_port_i;
    ...
    bcm_pbmp_t *ubmp)
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        rv = pbmp_from_ppd_port_bitmap(unit, &core_pbmp, ports, SOC_PPD_VLAN_MEMBERSHIP_BITMAP_SIZE);
        BCMDNX_IF_ERR_EXIT(rv);
