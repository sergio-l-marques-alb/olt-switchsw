                                ...
                                int group_size)
{
    ...
    BCM_IF_ERROR_RETURN(READ_L3_ECMP_COUNTm(unit, MEM_BLOCK_ANY, ecmp_group_idx,
                &ecmp_count_entry));
    /* 
     * In RH, member count should be the log(count) to base 2
     */ 
    switch (group_size) {
      case 64: radix = 6;
               break; 
    ...
    /* Set the decoded radix in the count field */
    soc_L3_ECMP_COUNTm_field32_set(unit,  
                &ecmp_count_entry, COUNTf, radix);
 ...
 */
int
bcm_th_ecmp_rh_get_multipath(int unit, int ecmp_group_idx, int intf_size,
                             bcm_if_t *intf_arr, int *intf_count)
{
    bcm_if_t *intf_arr_ptr;

    if (NULL == intf_arr || NULL == intf_count) {
       return (BCM_E_PARAM);
    }
                                               ...
                                               bcm_if_t *ex_new_intf_array)
{
    ...
    for (i = 0; i < num_entries; i++) {
        ...
        rh_intf_array[i] = new_intf_array[member_id];
    }
...
bcm_th_l3_ecmp_rh_member_sync(int unit, uint8 **scache_ptr)
{
    ...
    end_scache_ptr = *scache_ptr + 
           (RH_ECMP_MEMBER_TBL_MAX * (sizeof(int16) + sizeof(bcm_if_t)));
 ...
 */
int
bcm_th_l3_ecmp_rh_member_recover(int unit, uint8 **scache_ptr)
{
    bcm_if_t **rh_intf_arr_ptr;
    int stable_size = 0;
    int intf_count;
        ...
        if (stable_size == 0) { /* level 1 */
            ...
            while ((*scache_ptr) < end_scache_ptr) {
                ...
                if (ecmp_group_index != 0) {
                    ...
                    *scache_ptr += ((sizeof(int16) + sizeof(bcm_if_t)) * intf_count);
                } else {
