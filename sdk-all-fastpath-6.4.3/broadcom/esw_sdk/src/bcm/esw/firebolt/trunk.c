/*
 * $Id: trunk.c,v 1.174 Broadcom SDK $
...
typedef struct {
    _xgs3_member_info_t member_info[_BCM_FP_TRUNK_NUM_MAX + _BCM_FABRIC_TRUNK_NUM_MAX];
...
_bcm_xgs3_trunk_member_init(int unit)
{
    if (_xgs3_trunk_member_info[unit] == NULL) {
        _xgs3_trunk_member_info[unit] =
            sal_alloc(sizeof(_xgs3_trunk_member_info_t),
    ...
    sal_memset(_xgs3_trunk_member_info[unit], 0,
            sizeof(_xgs3_trunk_member_info_t));
    return BCM_E_NONE;
}
 ...
 */
int 
_bcm_xgs3_trunk_mod_port_map_reinit(int unit) 
{
    ...
    for (idx = soc_mem_index_min(unit, mem); idx <= idx_max; idx++) {
        ...
        tid = soc_mem_field32_get(unit, mem, (uint32 *)&stm, TGIDf);
...
#ifdef BCM_TRX_SUPPORT
    if (soc_feature(unit, soc_feature_trunk_group_overlay)) {
        idx = (mod * (SOC_PORT_ADDR_MAX(unit) + 1)) + port;
    } else
#endif
    {
        idx = (mod << SOC_TRUNK_BIT_POS(unit)) + port;
    }
                                    ...
                                    int hw_count, bcm_port_t *ftp)
{
    ...
    if (hw_count) { /* Enabling HG trunk failover */
        ...
        fs_ports = SOC_IS_SC_CQ(unit) ?
            ...
            SOC_IS_HURRICANEX(unit) ? _BCM_HURRICANE2_TRUNK_FAILOVER_PORT_NUM :
                             ...
                             _esw_trunk_add_info_t *add_info)
{
    ...
    if ((NULL == add_info) || (0 != add_info->flags)) {
    ...
    for (tix = 0; tix < add_info->num_ports; tix++) {
        ...
        hwft->flags[num_hwft_ports] = add_info->flags;
                           ...
                           trunk_private_t *t_info)
{
    ...
    if (SOC_IS_HBX(unit) || SOC_IS_TRX(unit)) {
        ...
        if(!SOC_IS_HURRICANEX(unit) && !SOC_IS_GREYHOUND(unit)) { /* PR 34459 */
            soc_reg64_field32_set(unit, HG_TRUNK_GROUPr, &val64,
                                  HIGIG_TRUNK_SIZEf, add_info->num_ports-1);
        }
        soc_reg64_field32_set(unit, HG_TRUNK_GROUPr, &val64,
                ...
                int num_joining, bcm_gport_t *joining_arr)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_src_mod_port_table_index_get(unit, mod,
                port, &stm_idx));
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            for (i = 0; i < num_joining; i++) {
                ...
                /* Delete port's old VLAN range profile */
                  ...
                  BCM_IF_ERROR_RETURN(_bcm_trx_vlan_range_profile_entry_delete(
                ...
                BCM_IF_ERROR_RETURN(WRITE_SOURCE_TRUNK_MAP_TABLEm(unit,
        ...
        trunk_private_t *t_info)
{
    ...
    for (i = 0; i < add_info->num_ports; i++) {
        if (add_info->member_flags[i] & BCM_TRUNK_MEMBER_INGRESS_DISABLE &&
            add_info->member_flags[i] & BCM_TRUNK_MEMBER_EGRESS_DISABLE) {
                return BCM_E_PARAM;
        }
                                   ...
                                   bcm_pbmp_t ports_status)
{
    ...
    PBMP_ITER(ports_active, port) {
        ...
        if (tid >= 1 && tid < _BCM_FP_TRUNK_NUM_MAX + 1) {
            ...
            (void)soc_mem_write(unit, TRUNK_GROUPm, MEM_BLOCK_ALL,
                                tid, &tg_entry);
...
#ifdef BCM_WARM_BOOT_SUPPORT
#define TRUNK_LOCK(unit)        soc_mem_lock(unit, TRUNK_GROUPm)
#define TRUNK_UNLOCK(unit)      soc_mem_unlock(unit, TRUNK_GROUPm)
...
_xgs3_trunk_reinit(int unit, int ngroups_fp, trunk_private_t *trunk_info)
{
    ...
    if (SOC_IS_FBX(unit)) {
        ...
        trunk_private_t *t_info; 
        trunk_group_entry_t tg_entry;
        bcm_module_t egr_hw_mods[BCM_XGS3_TRUNK_MAX_PORTCNT];
        bcm_port_t egr_hw_ports[BCM_XGS3_TRUNK_MAX_PORTCNT];
        uint32 egr_member_flags[BCM_XGS3_TRUNK_MAX_PORTCNT] = { 0 };
        ...
        rv = BCM_E_NONE;
        tgsize = soc_mem_entry_words(unit, TRUNK_GROUPm);
        tgsize = WORDS2BYTES(tgsize);

        for (tid = 0; tid < ngroups_fp; tid++, trunk_info++) {
             SOC_IF_ERROR_RETURN
                (soc_mem_read(unit, TRUNK_GROUPm, MEM_BLOCK_ANY, tid,
                              &tg_entry));
             if (sal_memcmp(&tg_entry, soc_mem_entry_null(unit, TRUNK_GROUPm),
                            tgsize) == 0) {
                 /*
                  * Invalid trunk ID
                  */
         ...
         */
        if (BCM_SUCCESS(rv)) {
            rv = _bcm_xgs3_trunk_mod_port_map_reinit(unit);
        }
