                                           ...
                                           int weight)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport)) /* ingress*/
    {
        if (port != -1)
        {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d, Invalid port id\n"), unit));
        }
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_weight_set,(unit, pipe, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    ...
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport)) /* middle */
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_mid_weight_set,(unit, pipe, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                           ...
                                           int *weight)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport)) /* ingress*/
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_weight_get,(unit, pipe, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    ...
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport)) /* middle */
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_mid_weight_get,(unit, pipe, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport)) /* egress */
    {
                                       ...
                                       bcm_cosq_threshold_t *threshold)
{
    ...
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_threshold_set,(unit, pipe, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport))
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_mid_threshold_set,(unit, pipe, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_threshold_set,(unit, pipe, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       bcm_cosq_threshold_t *threshold)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_threshold_get,(unit, pipe, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport))
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_mid_threshold_get,(unit, pipe, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_threshold_get,(unit, pipe, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       uint32 flags)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_rate_set,(unit, pipe, shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_rate_set,(unit, shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       uint32 *flags)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_rx_rate_get,(unit, pipe, &shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        ...
        rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_rate_get,(unit, &shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    } else {
                               ...
                               int arg)
{
    ...
    DFE_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_rate_enable_set,(unit, pipe, arg));
    BCMDNX_IF_ERR_EXIT(rc);
                               ...
                               int *arg)
{
    ...
    DFE_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_cosq_pipe_tx_rate_enable_get,(unit, pipe, arg));
    BCMDNX_IF_ERR_EXIT(rc);
