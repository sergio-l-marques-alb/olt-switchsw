/*
 * $Id: hash.c,v 1.1.2.4 Broadcom SDK $
...
int
soc_tomahawk_hash_bank_bitmap_get(int unit, soc_mem_t mem, uint32 *bitmap)
{
    int count;
...
STATIC int
_soc_th_hash_entry_to_key(int unit, void *entry, uint8 *key, soc_mem_t mem,
                          soc_field_t *field_list)
{
    ...
    case EGR_VLAN_XLATEm:
        ...
        if (soc_feature(unit, soc_feature_robust_hash)) {
            if (ROBUSTHASH(unit)->egr_xlate.enable == 1) {
                if ((val_bits % 8) != 0) {
                    byte_align = 0;
                }
            }
    ...
    case MPLS_ENTRYm:
        ...
        /* If robust hash is enabled, byte boundry is adjusted after robust
           hash key computation */
        if (soc_feature(unit, soc_feature_robust_hash)) {
            if (ROBUSTHASH(unit)->mpls_entry.enable == 1) {
                if ((val_bits % 8) != 0) {
                    byte_align = 0;
                }
            }
                ...
                int key_nbits, void *base_entry, uint8 *key)
{
    ...
    if (use_lsb && (hash_offset + SOC_CONTROL(unit)->hash_bits_l2x > 48)) {
...
int
soc_th_l2x_base_entry_to_key(int unit, uint32 *entry, uint8 *key)
{
    ...
    return _soc_th_hash_entry_to_key(unit, entry, key, L2Xm, field_list);
}
                       ...
                       uint32 *entry)
{
    ...
    key_nbits = soc_th_l2x_base_entry_to_key(unit, entry, key);
...
int
soc_th_l3x_base_entry_to_key(int unit, uint32 *entry, uint8 *key)
{
    soc_mem_t mem;
    ...
    return _soc_th_hash_entry_to_key(unit, ptr, key, mem, field_list);
}
                      ...
                      uint32 *entry)
{
    ...
    key_nbits = soc_th_l3x_base_entry_to_key(unit, entry, key);
...
int
soc_th_exact_match_base_entry_to_key(int unit, uint32 *entry, uint8 *key)
{
    soc_mem_t mem;
    soc_field_t field_list[5];
    ...
    return _soc_th_hash_entry_to_key(unit, entry, key, mem, field_list);
}
                              ...
                              int use_lsb, uint32 *entry)
{
    ...
    key_nbits = soc_th_exact_match_base_entry_to_key(unit, entry, key);
...
int
soc_th_mpls_base_entry_to_key(int unit, void *entry, uint8 *key)
{
    soc_field_t field_list[2];
    ...
    field_list[1] = INVALIDf;
    return _soc_th_hash_entry_to_key(unit, entry, key, MPLS_ENTRYm,
                                     field_list);
}
...
soc_th_mpls_entry_hash(int unit, int hash_sel, int bank, uint32 *entry)
{
    ...
    key_nbits = soc_th_mpls_base_entry_to_key(unit, entry, key);

#ifdef SOC_ROBUST_HASH
    if (soc_feature(unit, soc_feature_robust_hash)) {
        if (ROBUSTHASH(unit)->mpls_entry.enable == 1) {
            SOC_IF_ERROR_RETURN
                (soc_robust_hash_get(unit, &(ROBUSTHASH(unit)->mpls_entry),
                                     bank, key, key_nbits));
            key_nbits += (key_nbits % 8) ? 4: 0;
        }
        key_nbits += ROBUST_HASH_KEY_SPACE_WIDTH;
    }
#endif /* SOC_ROBUST_HASH */
...
int
soc_th_vlan_xlate_base_entry_to_key(int unit, void *entry, uint8 *key)
{
    soc_mem_t mem;
    soc_field_t field_list[2];
...
soc_th_vlan_xlate_entry_hash(int unit, int hash_sel, int bank, uint32 *entry)
{
    ...
    sal_memset(key, 0, sizeof(key));
    key_nbits = soc_th_vlan_xlate_base_entry_to_key(unit, entry, key);
#ifdef SOC_ROBUST_HASH
    if (soc_feature(unit, soc_feature_robust_hash)) {
        if (ROBUSTHASH(unit)->ing_xlate.enable == 1) {
            SOC_IF_ERROR_RETURN
                (soc_robust_hash_get(unit, &(ROBUSTHASH(unit)->ing_xlate),
                                     bank, key, key_nbits));
            key_nbits += (key_nbits % 8) ? 4: 0;
        }
        key_nbits += ROBUST_HASH_KEY_SPACE_WIDTH;
...
int
soc_th_egr_vlan_xlate_base_entry_to_key(int unit, void *entry, uint8 *key)
{
    soc_field_t field_list[2];
    ...
    field_list[1] = INVALIDf;
    return _soc_th_hash_entry_to_key(unit, entry, key, EGR_VLAN_XLATEm,
                                     field_list);
}
                                  ...
                                  uint32 *entry)
{
    ...
    key_nbits = soc_th_egr_vlan_xlate_base_entry_to_key(unit, entry, key);

#ifdef SOC_ROBUST_HASH
    if (soc_feature(unit, soc_feature_robust_hash)) {
        if (ROBUSTHASH(unit)->egr_xlate.enable == 1) {
            SOC_IF_ERROR_RETURN
                (soc_robust_hash_get(unit, &(ROBUSTHASH(unit)->egr_xlate), bank,
                                     key, key_nbits));
            key_nbits += (key_nbits % 8) ? 4: 0;
        }
        key_nbits += ROBUST_HASH_KEY_SPACE_WIDTH;
...
int
soc_th_ing_vp_vlan_member_base_entry_to_key(int unit, void *entry, uint8 *key)
{
    ...
    return _soc_th_hash_entry_to_key(unit, entry, key,
                                     ING_VP_VLAN_MEMBERSHIPm, field_list);
}

uint32
soc_th_ing_vp_vlan_member_entry_hash(int unit, int hash_sel, uint32 *entry)
{
    int             key_nbits;
    uint8           key[SOC_MAX_MEM_WORDS * 4];
    uint32          index;

    key_nbits = soc_th_ing_vp_vlan_member_base_entry_to_key(unit, entry, key);
...
soc_th_ing_vp_vlan_member_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    ...
    return soc_th_ing_vp_vlan_member_entry_hash(unit, hash_sel, entry);
}
...
int
soc_th_egr_vp_vlan_member_base_entry_to_key(int unit, void *entry, uint8 *key)
{
    static soc_field_t field_list[] = {
        KEYf,
        INVALIDf
...
uint32
soc_th_egr_vp_vlan_member_entry_hash(int unit, int hash_sel, uint32 *entry)
{
    int             key_nbits;
    uint8           key[SOC_MAX_MEM_WORDS * 4];
    uint32          index;

    key_nbits = soc_th_egr_vp_vlan_member_base_entry_to_key(unit, entry, key);
...
soc_th_egr_vp_vlan_member_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    ...
    return soc_th_egr_vp_vlan_member_entry_hash(unit, hash_sel, entry);
}
...
int
soc_th_ing_dnat_address_type_base_entry_to_key(int unit, void *entry,
                                               uint8 *key)
{
    static soc_field_t field_list[] = {
        KEYf,
    ...
    return _soc_th_hash_entry_to_key(unit, entry, key, ING_DNAT_ADDRESS_TYPEm,
                                     field_list);
}

uint32
soc_th_ing_dnat_address_type_entry_hash(int unit, int hash_sel, uint32 *entry)
{
    int             key_nbits;
    uint8           key[SOC_MAX_MEM_WORDS * 4];
    uint32          index;

    key_nbits = soc_th_ing_dnat_address_type_base_entry_to_key(unit, entry,
                                                                key);
...
soc_th_ing_dnat_address_type_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    ...
    return soc_th_ing_dnat_address_type_entry_hash(unit, hash_sel, entry);
}
