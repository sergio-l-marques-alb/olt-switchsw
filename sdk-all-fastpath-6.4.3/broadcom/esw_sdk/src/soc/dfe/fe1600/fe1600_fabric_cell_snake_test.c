    ...
    uint32 flags)
{
    ...
    { 
        ...
        if (SOC_IS_BCM88754_A0(unit))
        {
            port_power_flags = SOC_DFE_PORT_CONTROL_FLAGS_RX_SERDES_IGNORE;
        } else {
            port_power_flags = (flags & SOC_DFE_ENABLE_EXTERNAL_LOOPBACK) ? SOC_DFE_PORT_CONTROL_FLAGS_RX_SERDES_IGNORE : 0; 
        }
        for(p = 0 ; p < SOC_DFE_DEFS_GET(unit, nof_links) ; p++) {
            SOCDNX_IF_ERR_EXIT(soc_fe1600_port_control_power_set(unit, p, port_power_flags, soc_dfe_port_power_off));
        }
    }
    ...
    { 
        ...
        for(p = 0 ; p < SOC_DFE_DEFS_GET(unit, nof_links) ; p++) {
            ...
            SOCDNX_IF_ERR_EXIT(soc_fe1600_port_control_power_set(unit, p, port_power_flags, soc_dfe_port_power_on));
        }
    }
