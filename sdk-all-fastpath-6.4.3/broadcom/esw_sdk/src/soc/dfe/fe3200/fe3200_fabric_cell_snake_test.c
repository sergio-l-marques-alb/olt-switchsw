/*
 * $Id: fe3200_fabric_cell_snake_test.c,v 1.30 Broadcom SDK $
 *
    ...
    uint32 flags)
{
    ...
    for(link_idx=0;link_idx<144;link_idx++) {
        ...
        rv = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, link_idx, BCM_PORT_PHY_CONTROL_TX_RESET, 1));
        SOCDNX_IF_ERR_EXIT(rv);
    }
    for(link_idx=0;link_idx<144;link_idx++) {
        rv = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, link_idx, BCM_PORT_PHY_CONTROL_RX_RESET, 1));
        SOCDNX_IF_ERR_EXIT(rv);
    }
    ...
    /*Default reset - all blocks except FSRD*/
    ...
    rv = MBCM_DFE_DRIVER_CALL(unit, mbcm_dfe_drv_blocks_reset, (unit, 1 /*force blocks*/, &reg_val_above_64));
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    *field_val = ((!(flags & SOC_DFE_ENABLE_MAC_LOOPBACK)) ? 0x1:0x0);
    ...
    for(link_idx=0;link_idx<144;link_idx++) {
        DFE_LINK_INPUT_CHECK(unit, link_idx);    
        rv = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, link_idx, BCM_PORT_PHY_CONTROL_RX_RESET, 0));
        SOCDNX_IF_ERR_EXIT(rv);
    }
    for(link_idx=0;link_idx<144;link_idx++) {
        rv = MBCM_DFE_DRIVER_CALL(unit,mbcm_dfe_port_phy_control_set,(unit, link_idx, BCM_PORT_PHY_CONTROL_TX_RESET, 0));
        SOCDNX_IF_ERR_EXIT(rv);
    }
