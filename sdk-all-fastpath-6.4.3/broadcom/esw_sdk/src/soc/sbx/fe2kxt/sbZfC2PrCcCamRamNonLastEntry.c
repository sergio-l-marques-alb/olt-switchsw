...
sbZfC2PrCcCamRamNonLastEntry_Unpack(sbZfC2PrCcCamRamNonLastEntry_t *pToStruct,
  ...
  (pToStruct)->m_uReserve3 =  (uint32)  (pFromData)[8] ;
  (pToStruct)->m_uReserve3 |=  (uint32)  ((pFromData)[9] & 0x1f) << 8;

  /* Unpack Member: m_uReserve2 */
  (pToStruct)->m_uReserve2 =  (uint32)  (pFromData)[4] ;
  (pToStruct)->m_uReserve2 |=  (uint32)  (pFromData)[5] << 8;
  (pToStruct)->m_uReserve2 |=  (uint32)  (pFromData)[6] << 16;
  (pToStruct)->m_uReserve2 |=  (uint32)  (pFromData)[7] << 24;

  /* Unpack Member: m_uReserve1 */
  (pToStruct)->m_uReserve1 =  (uint32)  ((pFromData)[2] >> 6) & 0x03;
  (pToStruct)->m_uReserve1 |=  (uint32)  (pFromData)[3] << 2;

  /* Unpack Member: m_uShift */
  (pToStruct)->m_uShift =  (uint32)  ((pFromData)[2] ) & 0x3f;

  /* Unpack Member: m_uNextState */
  (pToStruct)->m_uNextState =  (uint32)  ((pFromData)[0] >> 2) & 0x3f;
  (pToStruct)->m_uNextState |=  (uint32)  (pFromData)[1] << 6;

  /* Unpack Member: m_uE2ECC */
  (pToStruct)->m_uE2ECC =  (uint8)  ((pFromData)[0] >> 1) & 0x01;

  /* Unpack Member: m_uLast */
  (pToStruct)->m_uLast =  (uint8)  ((pFromData)[0] ) & 0x01;
