...
{
    int instance, ilkn, clport, count, base_port;
    uint32 config = 0;
    ...
    for (instance=0; 
        ...
        if (SOC_SUCCESS(status)) {
            ...
            clport = soc_sbx_block_find(unit, SOC_BLK_CLPORT, instance);
            ilkn = soc_sbx_block_find(unit, SOC_BLK_IL, instance);
            if (instance) {
                pi = &port_map->fabric_port_info[port];
            } else {
                pi = &port_map->line_port_info[port];
                    ...
                    if (qi->num_dqueue > 1) {
                        if (ilkn == pi->blk) {
                            ...
                            if (qi->spri_set) {
                                SOC_IF_ERROR_RETURN(
                                    soc_sbx_caladan3_sws_pt_strict_priority_config(unit,
                                        PT_INSTANCE(instance), 0, qi->spri0_qid,
                                        qi->spri1_qid));
                            } else {
                                SOC_IF_ERROR_RETURN(
                                    soc_sbx_caladan3_sws_pt_wdrr_scheduler_config(unit,
                                        PT_INSTANCE(instance), 0, TRUE));
                            }
                            ...
                            if (qi->spri_set) {
                                SOC_IF_ERROR_RETURN(
                                    soc_sbx_caladan3_sws_pt_strict_priority_config(unit,
                                        PT_INSTANCE(instance), base_port,
                                        qi->spri0_qid, qi->spri1_qid));
                            } else {
                                SOC_IF_ERROR_RETURN(
                                    soc_sbx_caladan3_sws_pt_wdrr_scheduler_config(unit,
                                        PT_INSTANCE(instance), base_port, TRUE));
                            }
                        } else {
                            LOG_ERROR(BSL_LS_SOC_COMMON,
                                      (BSL_META_U(unit,
...
{
    int ilkn, clport, count COMPILER_ATTRIBUTE((unused)), base_port, cur_port;
    uint32 config = 0;
...
soc_sbx_caladan3_hpte_hipri_queues_config(int unit) 
{
    ...
    char buffer[128];
    soc_sbx_caladan3_port_map_t *port_map;
    soc_info_t *si COMPILER_ATTRIBUTE((unused));
    ...
    if (SOC_SUCCESS(status)) {
        ...
        SOC_SBX_C3_BMP_ITER_RANGE(lqinfo->squeue_bmp, SOC_SBX_CALADAN3_SWS_LINE_SQUEUE_BASE,
            ...
            SOC_SBX_C3_BMP_ITER_RANGE(fqinfo->dqueue_bmp, SOC_SBX_CALADAN3_SWS_FABRIC_DQUEUE_BASE,
                ...
                if (SOC_SBX_C3_BMP_MEMBER_IN_RANGE(qtemp.dqueue_bmp, SOC_SBX_CALADAN3_SWS_FABRIC_DQUEUE_BASE, dqueue)) {
                    continue;
                }
...
soc_sbx_caladan3_sws_ipte_init(int unit)
{
    ...
    uint32 regval=0;
    int p = 0, b = 0, r = 0;
    uint32 grp0_base[2] = {0xff, 0xff}, grp0_ciel[2] = {0, 0};
    uint32 grp1_base[2] = {0xff, 0xff}, grp1_ciel[2] = {0, 0};
    uint32 grp0_range[2] = {0}, grp1_range[2] = {0};
    int i;
    ...
    for (p = 0; p < SOC_SBX_CALADAN3_PORT_MAP_ENTRIES; p++) {
        ...
        if (port_info->physical_intf == 0) {
            if (qi->dqueue_base < grp0_base[0]) {
                grp0_base[0] = qi->dqueue_base;
            }
            if (qi->dqueue_base > grp0_ciel[0]) {
                grp0_ciel[0] = qi->dqueue_base;
            }
        } else {
            if (qi->dqueue_base < grp1_base[0]) {
                grp1_base[0] = qi->dqueue_base;
            }
            if (qi->dqueue_base > grp1_ciel[0]) {
                grp1_ciel[0] = qi->dqueue_base;
            }
        }
        port_info = &port_map->fabric_port_info[p];
        if (port_info->physical_intf == 1) {
            if (qi->dqueue_base < grp0_base[1]) {
                grp0_base[1] = qi->dqueue_base;
            }
            if (qi->dqueue_base > grp0_ciel[1]) {
                grp0_ciel[1] = qi->dqueue_base;
            }
        } else {
            if (qi->dqueue_base < grp1_base[1]) {
                grp1_base[1] = qi->dqueue_base;
            }
            if (qi->dqueue_base > grp1_ciel[1]) {
                grp1_ciel[1] = qi->dqueue_base;
            }
        }
    }
    grp0_range[0] = grp0_ciel[0] - grp0_base[0] + 1;
    grp1_range[0] = grp1_ciel[0] - grp1_base[0] + 1;
    grp0_range[1] = grp0_ciel[1] - grp0_base[1] + 1;
    grp1_range[1] = grp1_ciel[1] - grp1_base[1] + 1;

    grp0_range[0] = (grp0_range[0] > 0xff) ? 0 : grp0_range[0];
    grp1_range[0] = (grp1_range[0] > 0xff) ? 0 : grp1_range[0];
    grp0_range[1] = (grp0_range[1] > 0xff) ? 0 : grp0_range[1];
    grp1_range[1] = (grp1_range[1] > 0xff) ? 0 : grp1_range[1];

    if (grp1_base[0] < grp0_base[0]) {
        b = grp1_base[0]; r = grp1_range[0];
        grp1_base[0] = grp0_base[0];
        grp1_range[0] = grp0_range[0];
        grp0_base[0] = b; grp0_range[0] = r;
    }
    if (grp1_base[1] < grp0_base[1]) {
        b = grp1_base[1]; r = grp1_range[1];
        grp1_base[1] = grp0_base[1];
        grp1_range[1] = grp0_range[1];
        grp0_base[1] = b; grp0_range[1] = r;
    }
    grp0_base[0] = (grp0_range[0] == 0) ? 0 : grp0_base[0];
    grp1_base[0] = (grp1_range[0] == 0) ? 0 : grp1_base[0];
    grp0_base[1] = (grp0_range[1] == 0) ? 0 : grp0_base[1];
    grp1_base[1] = (grp1_range[1] == 0) ? 0 : grp1_base[1];
    if (SOC_SBX_CALADAN3_SWS_LINE_DQUEUE_BASE < grp0_base[0]) {
        grp0_base[0] = SOC_SBX_CALADAN3_SWS_LINE_DQUEUE_BASE;
    }
    if (SOC_SBX_CALADAN3_SWS_FABRIC_DQUEUE_BASE  < grp0_base[1]) {
        grp0_base[1] = SOC_SBX_CALADAN3_SWS_FABRIC_DQUEUE_BASE;
    }
    if (port_map->borrowed_wcore_count > 0) {
        grp1_range[1] = grp1_range[0];
        grp1_base[1] = grp1_base[0];
        grp1_base[0] += grp1_range[0] + 1;
        grp1_range[0] = SOC_SBX_CALADAN3_SWS_QUEUE_PER_REGION -
                           grp0_range[0] - grp1_range[0];
    }
    if (0 == grp1_range[0]) {
        grp0_range[0] = SOC_SBX_CALADAN3_SWS_QUEUE_PER_REGION;
    }
    if (0 == grp1_range[1]) {
        grp0_range[1] = SOC_SBX_CALADAN3_SWS_QUEUE_PER_REGION;
    }
...
soc_sbx_caladan3_sws_pt_client_calendar_auto_set(int unit, sws_config_t *sws_cfg)
{
     ...
     */
    soc_sbx_caladan3_port_map_t *port_map;
    soc_sbx_caladan3_port_map_info_t *lpinfo, *fpinfo COMPILER_ATTRIBUTE((unused));
