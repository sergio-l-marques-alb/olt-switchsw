...
/******************************************************************************
 *
 * $Id: sbEMLComp.c,v 1.28 Broadcom SDK $
 *
...
static INLINE uint32 hashT2Seed (sbEML_p_t em, sbEMLTable2_p_t t2,
                                   uint32 l1hash, uint32 seed);
...
static sbStatus_t commit (sbEML_p_t em, uint32 *pnRunLength);
static int t2_cmp(void *a, void *b);

#define block(mutex) \
        sal_mutex_take(mutex, sal_mutex_FOREVER)
#define unblock(mutex) \
 ...
 *
 * ---------------------------------------------------------------------------*/
static INLINE uint32 hashPKey(sbEMLPackedKey_t key, uint n);
...
#if 0
  LOG_CLI((BSL_META("hash1 (%d)"), n));
  for (i = 0; i < n; i++)
    LOG_CLI((BSL_META(" 0x%08x"), SAND_FE_TO_HOST_32(key[i])));
  LOG_CLI((BSL_META("\n        -> 0x%08x\n"), k));
#endif
 ...
 *
 * ---------------------------------------------------------------------------*/
static INLINE uint32 eqPKey (sbEMLPackedKey_t a,
...
static INLINE uint32
hashT2Seed (sbEML_p_t em, sbEMLTable2_p_t t2, uint32 l1hash, uint32 seed)
{
  ...
  if (em->new_hash) {
    ...
    k = sbCrc32(l1hash + (seed << 16)) ;

#if 0
    LOG_CLI((BSL_META("hash2 (%d)"), n));
    for (i = 0; i < n; i++)
      LOG_CLI((BSL_META(" 0x%08x"), SAND_FE_TO_HOST_32(pkey[i])));
    LOG_CLI((BSL_META("\n   0x%08x + 0x%08x"), l1hash, seed));
    LOG_CLI((BSL_META("\n        -> 0x%08x\n"), k));
#endif
                ...
                void *payloadMgr, sbFe2000DmaMgr_t *dmaCtxt)
{
  ...
  *pEmCtxt = (sbEMLCtx_p)v;
  ...
  if (em->maxDirty > 0) {
    ...
    err = em->cParams->sbmalloc (clientData, SB_ALLOC_INTERNAL,
                                 em->maxDirty * sizeof(uint8), &v, NULL);
    if (err)
      return err;
  ...
  if (em->sharedFreeList) {
    ...
    if (em->ipv6) {
      ...
      emp->zeroHdl = sbPayloadAlloc (em->payloadMgr,
                        (em->payload_size/sizeof(uint32)), 0, &err);
    }
    else {
...
sbEMLCompRecover (sbEMLCtx_p *pEmCtxt)
{
  ...
  if (rv != SB_OK) {
    LOG_CLI((BSL_META_U(unit,
                        "Failed to DMA table one: 0x%08x\n"), rv));
    goto done;
  }
...
nextseed:
    ...
    if (found) {
      ...
      newt2size = (highestSlot - (highestSlot%4) + 4) *
                    (em->payload_size/(2*sizeof(uint32)));
    }
...
#if 0
      if (i > 4) {
        LOG_CLI((BSL_META("bucket has %d entries\n"), i));
        for (p = t1->entries; p; p = p->next) {
          em->getKey(em->ctx, PAYLOAD(p), pkey);
          LOG_CLI((BSL_META("%x %x \n"), SAND_FE_TO_HOST_32(pkey[0]), SAND_FE_TO_HOST_32(pkey[1])));
        } 
      }
#endif
      /* no dups, we're done */
...
sbEMLDelPackedKey (sbEML_p_t em, sbEMLPackedKey_t pkey)
{
  ...
  uint i;
  uint slotix[SB_EML_MAX_TABLE2_SIZE];
  uint32 highS, highestSlot, goldenSeed, found;
  uint32 seed = 0;
  uint32 mask = 0;
  uint32 m = 0;
  sbEMLTable2_p_t p;
 ...
 * ---------------------------------------------------------------------------*/
sbStatus_t
sbEMLCommit (sbEMLCtx_p emCtxt, uint32 *pnRunLength)
{
  sbEML_p_t em = (sbEML_p_t) emCtxt;
  struct emcomp *emp = em->emp;
  sbStatus_t err;
           ...
           struct slabTransfer *st, uint size)
{
  ...
  /* size in bytes */
  SB_ASSERT (size/sizeof(uint32) < cParams->maximumContiguousDmaMalloc);
  st->slabMemory = NULL;
  ...
  st->slabEnd = (uint32 *)st->slabMemory + size / sizeof(uint32);
 ...
 * ---------------------------------------------------------------------------*/
sbStatus_t
flushDirty(sbEML_p_t em, uint32 *rl)
{
  struct emcomp *emp = em->emp;
  sbFe2000DmaSlab_t *slab;
  sbEMLTable1_p_t t1;
  ...
  for (t1 = emp->headDirty; *rl > 0 && t1; --*rl, t1 = t1->nextDirty) {
    ...
    if (em->ipv6) {
      ...
      slab = em->slabSetTable1ptr (em->ctx, t1 - emp->table1,
                      ...
                      (em->payload_size/(2 * sizeof(uint32)))) - 1) >>
  ...
  SB_ASSERT ((uint32 *)SB_FE2000_SLAB_NEXT(slab) <
 ...
 * ---------------------------------------------------------------------------*/
static sbStatus_t
commit (sbEML_p_t em, uint32 *pnRunLength)
{
  sbStatus_t err;
...
initSRAM (sbEML_p_t em)
{
  ...
  do {
    ...
    while ((uint32 *)slab + em->nbank * SIZEOFW(sbFe2000DmaSlab_t) <
            emp->commitSlab.slabEnd && emp->initOffs < em->table1_count) {
      slab->words = em->payload_size/(2*sizeof(uint32));
