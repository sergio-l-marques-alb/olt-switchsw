...
int soc_sbx_g3p1_pvv2e_add_ext_sim(int unit,  int ivrf ,  int iport ,  int iovid ,  int iivid , soc_sbx_g3p1_pvv2e_t *entry)
{
    int nextPrintPos = 0;
    int bufferSizeRemain = MAX_BUFFER_SIZE;
    int numCharPrinted;
    ...
    numCharPrinted =  sal_snprintf(&send_buffer[nextPrintPos], bufferSizeRemain, "vidop 0x%x ", entry->vidop);
    nextPrintPos += numCharPrinted;
    bufferSizeRemain -= numCharPrinted;
    if (bufferSizeRemain == 0) return SOC_E_PARAM;
...
int soc_sbx_g3p1_pvv2e_delete_ext_sim(int unit,  int ivrf ,  int iport ,  int iovid ,  int iivid )
{
    int nextPrintPos = 0;
    int bufferSizeRemain = MAX_BUFFER_SIZE;
    int numCharPrinted;
...
int soc_sbx_g3p1_pvv2e_get_ext_sim(int unit,  int ivrf ,  int iport ,  int iovid ,  int iivid , soc_sbx_g3p1_pvv2e_t *entry)
{
    int nextPrintPos = 0;
    int bufferSizeRemain = MAX_BUFFER_SIZE;
    int numCharPrinted;
    ...
    while ((fieldName != NULL) && (fieldValue != NULL)) {       
        ...
        /* _shr_ctoi() is same as atoi() but available in all toochains */
            ...
            if (!sal_strcasecmp(fieldName, "vidop"))        { entry->vidop = _shr_ctoi(fieldValue); }    
