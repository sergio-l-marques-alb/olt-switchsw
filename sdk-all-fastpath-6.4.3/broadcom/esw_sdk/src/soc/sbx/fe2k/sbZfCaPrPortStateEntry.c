/*
 * $Id: sbZfCaPrPortStateEntry.c,v 1.2 Broadcom SDK $
...
sbZfCaPrPortStateEntry_Unpack(sbZfCaPrPortStateEntry_t *pToStruct,
  ...
  (pToStruct)->m_nReserved =  (uint32)  ((pFromData)[6] >> 2) & 0x3f;

  /* Unpack Member: m_bTestPkt */
  (pToStruct)->m_bTestPkt =  (uint32)  ((pFromData)[6] >> 1) & 0x01;

  /* Unpack Member: m_nBuffer */
  (pToStruct)->m_nBuffer =  (uint32)  ((pFromData)[4] >> 5) & 0x07;
  (pToStruct)->m_nBuffer |=  (uint32)  (pFromData)[5] << 3;
  (pToStruct)->m_nBuffer |=  (uint32)  ((pFromData)[6] & 0x01) << 11;

  /* Unpack Member: m_nLength */
  (pToStruct)->m_nLength =  (uint32)  ((pFromData)[3] >> 2) & 0x3f;
  (pToStruct)->m_nLength |=  (uint32)  ((pFromData)[4] & 0x1f) << 6;

  /* Unpack Member: m_nNumPages */
  (pToStruct)->m_nNumPages =  (uint32)  ((pFromData)[2] >> 3) & 0x1f;
  (pToStruct)->m_nNumPages |=  (uint32)  ((pFromData)[3] & 0x03) << 5;

  /* Unpack Member: m_bStartOfPkt */
  (pToStruct)->m_bStartOfPkt =  (uint32)  ((pFromData)[2] >> 2) & 0x01;

  /* Unpack Member: m_nPageAddr */
  (pToStruct)->m_nPageAddr =  (uint32)  ((pFromData)[0] >> 6) & 0x03;
  (pToStruct)->m_nPageAddr |=  (uint32)  (pFromData)[1] << 2;
  (pToStruct)->m_nPageAddr |=  (uint32)  ((pFromData)[2] & 0x03) << 10;

  /* Unpack Member: m_nLineAddr */
  (pToStruct)->m_nLineAddr =  (uint32)  ((pFromData)[0] >> 5) & 0x01;

  /* Unpack Member: m_nWordNumber */
  (pToStruct)->m_nWordNumber =  (uint32)  ((pFromData)[0] >> 1) & 0x0f;

  /* Unpack Member: m_bDropPkt */
  (pToStruct)->m_bDropPkt =  (uint32)  ((pFromData)[0] ) & 0x01;
