...
#define SOC_PB_CNT_MODE_ING_NO_COLOR_FLD_VAL                       (0)
#define SOC_PB_CNT_MODE_ING_COLOR_RES_LOW_FLD_VAL                  (1)
#define SOC_PB_CNT_MODE_ING_COLOR_RES_HI_FLD_VAL                   (2)
      ...
      )
{
  ...
  {
    switch(counter_info.mode_ing)
    {
    case SOC_PB_CNT_MODE_ING_NO_COLOR:
      cnt_ndx = counter_ndx * 2;
      break;

    case SOC_PB_CNT_MODE_ING_COLOR_RES_LOW:
      cnt_ndx = counter_ndx * 4;
      break;

    case SOC_PB_CNT_MODE_ING_COLOR_RES_HI:
...
uint32
  soc_pb_cnt_get_ingress_mode(
    SOC_SAND_IN  SOC_PB_CNT_MODE_ING ing,
  ...
  )
{
  ...
  case SOC_PB_CNT_MODE_ING_NO_COLOR:
    fld_val = SOC_PB_CNT_MODE_ING_NO_COLOR_FLD_VAL;
    break;
  
  case SOC_PB_CNT_MODE_ING_COLOR_RES_LOW:
    fld_val = SOC_PB_CNT_MODE_ING_COLOR_RES_LOW_FLD_VAL;
    break;
   
  case SOC_PB_CNT_MODE_ING_COLOR_RES_HI:
    fld_val = SOC_PB_CNT_MODE_ING_COLOR_RES_HI_FLD_VAL;
    break;
...

{
  ...
  else /* Ingress */
  {
    ...
    switch(fld_val)
    {
    case SOC_PB_CNT_MODE_ING_NO_COLOR_FLD_VAL:
      info->mode_ing = SOC_PB_CNT_MODE_ING_NO_COLOR;
      break;

    case SOC_PB_CNT_MODE_ING_COLOR_RES_LOW_FLD_VAL:
      info->mode_ing = SOC_PB_CNT_MODE_ING_COLOR_RES_LOW;
      break;

    case SOC_PB_CNT_MODE_ING_COLOR_RES_HI_FLD_VAL:
      info->mode_ing = SOC_PB_CNT_MODE_ING_COLOR_RES_HI;
      break;
  ...
  )
{
  ...
  for(indx = 0;  (indx < SOC_PB_CNT_CACHE_LENGTH) && (more_counters_2read == TRUE);  indx++)
  {
    ...
    if (fld_val == 0x1)
    {
      ...
      cnt_res[nof_read_cnt].pkt_cnt = fld_val2;
  ...
  for (indx = 0 ;  indx < nof_read_cnt; indx++)
  {
    ...
    result_arr->cnt_result[indx].pkt_cnt = cnt_res[indx].pkt_cnt;
  ...
  )
{
  ...
  */
  ...
  read_rslt->pkt_cnt = tbl_data.packets_counter;
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->mode_ing, SOC_PB_CNT_COUNTERS_INFO_MODE_ING_MAX, SOC_PB_CNT_MODE_ING_OUT_OF_RANGE_ERR, 11, exit);
