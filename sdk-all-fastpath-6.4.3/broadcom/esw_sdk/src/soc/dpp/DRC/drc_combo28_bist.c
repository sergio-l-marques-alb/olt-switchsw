/*
 * $Id: drc_combo28_cb.c,v 1.1.2.2 Broadcom SDK $
...
/* 
 * MPR BIST parameters cinfiguration  
 */
    ...
    uint8 mpr_pattern[SHMOO_COMBO28_BIST_MPR_NOF_PATTERNS])
{
    ...
    /* check that calib use MPR is set */
    SOCDNX_IF_ERR_EXIT( soc_reg32_get(unit, DRCA_INTIAL_CALIB_USE_MPRr, REG_PORT_ANY, 0, &reg_val));
    ...
    if(mpr_stagger_mode == TRUE){        
        /* If this bit is not asserted BIST will generate commands with a configurable weight per MPR location. */
    ...
    soc_reg64_field_set(unit, DRCA_BIST_CONFIGURATIONSr, &reg64_val, MPR_READOUT_MODEf, reg64_field_val); /* If this bit asserted BIST read MPR read out content. */
    ...
    /* Set MPR pattern for stagger mode */
    if (mpr_stagger_mode == TRUE) {
        /* check clam_shell  */
        ...
        SOC_DPP_DRC_COMBO28_BIST_MPR_INFO *mpr_info)
{
    ...
    if(((info->arad_bist_flags & (SOC_DPP_DRC_COMBO28_BIST_FLAGS_STOP | SOC_DPP_DRC_COMBO28_BIST_FLAGS_GET_DATA)) == 0)
        ...
        rv = soc_dpp_drc_combo28_configure_bist(
                  unit,
                  drc_ndx,
                  info->write_weight, /*  write_weight */
                  info->read_weight, /* read_weight */
                  (info->pattern_mode   == SOC_DPP_DRC_COMBO28_BIST_DATA_PATTERN_BIT_MODE) ? TRUE : FALSE, /* bit_pattern_mode */
                  (info->arad_bist_flags & SOC_DPP_DRC_COMBO28_BIST_FLAGS_TWO_ADDRESS_MODE) ? TRUE : FALSE, /* TWO_ADDRESS_MODE) */
                  ...
                  (info->pattern_mode   == SOC_DPP_DRC_COMBO28_BIST_DATA_PATTERN_ADDR_MODE) ? TRUE : FALSE, /* PATTERN_ADDR_MODE */
                  (info->arad_bist_flags & SOC_DPP_DRC_COMBO28_BIST_FLAGS_INFINITE) ? 0 : info->bist_num_actions,
                  info->bist_start_address, /*bist_start_address*/
                  info->bist_end_address,  /* bist_end_address */
                  (info->arad_bist_flags & SOC_DPP_DRC_COMBO28_BIST_FLAGS_BG_INTERLEAVE) ? TRUE : FALSE,/*BIST_BG_INTERLEAVE */
                  (info->arad_bist_flags & SOC_DPP_DRC_COMBO28_BIST_FLAGS_SINGLE_BANK_TEST) ? TRUE : FALSE, /*BIST_SINGLE_BANK_TEST */
                  (info->arad_bist_flags & SOC_DPP_DRC_COMBO28_BIST_FLAGS_ADDRESS_PRBS_MODE) ? TRUE : FALSE, /* DRAM_BIST_ADDRESS_PRBS_MODE */                  
                  pattern_lcl,
                  data_seed_lcl);            
        SOCDNX_IF_ERR_EXIT(rv);
    ...
    uint32 bist_dbi_seeds)
{
    ...
    /* Set GDDR5 BIST period from disabling GDDR5 BIST until All data from dram is due to arrive. 
       This register should reflect worst case read data latency.*/    
    SOCDNX_IF_ERR_EXIT( soc_reg32_get(unit, DRC_GDDR_5_BIST_FINISH_PRDl[drc_ndx], REG_PORT_ANY, 0, &reg_val));
    soc_reg_field_set(unit, DRC_GDDR_5_BIST_FINISH_PRDl[drc_ndx], &reg_val, GDDR_5_BIST_FINISH_PRDf, 64/*field val*/);
    ...
    /* Set Bist total number of commands.*/
    ...
    if (adt_enable == TRUE) {
        ...
        for (i = 0; i < (SOC_DPP_DRC_COMBO28_BIST_NOF_PATTERNS / 2); ++i) {
            reg_val = COMPILER_64_LO(bist_data_pattern[i]);
            SHR_BITCOPY_RANGE(field_above64_val, 0, &reg_val, 0, 18);  
...
/*  
 * GDDR5 BIST read error counters 
 */
        ...
        SOC_DPP_DRC_COMBO28_GDDR5_BIST_INFO *info)
{
    ...
    rv = soc_dpp_drc_combo28_gddr5_bist_configure(
                      unit,
                      drc_ndx,
                      info->fifo_depth,
                      info->num_commands,
