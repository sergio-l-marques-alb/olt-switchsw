...
#define SOC_JER_TBL_FDT_IPT_MESH_MC_LINE_LENGTH     128
...
static soc_mem_t jer_tbls_88675_excluded_mem_list[] = {
...
uint32 soc_jer_sch_tbls_init(int unit)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_fill_table_with_entry(unit, SCH_FC_MAP_FCMm, MEM_BLOCK_ALL, table_entry));
    SOCDNX_SAND_IF_ERR_EXIT(arad_fill_table_with_entry(unit, SCH_CH_NIF_RATES_CONFIGURATION_CNRCm, MEM_BLOCK_ALL, table_entry));
    SOCDNX_SAND_IF_ERR_EXIT(arad_fill_table_with_entry(unit, SCH_ONE_PORT_NIF_CONFIGURATION_OPNCm, MEM_BLOCK_ALL, table_entry));
...
uint32 soc_jer_fdt_tbls_init(int unit)
{
    ...
    uint32 mesh_config_data[4] = {0xcccccccc, 0xcccccccc, 0xcccccccc, 0xcccccccc};
    SOCDNX_INIT_FUNC_DEFS;

    if (SOC_DPP_IS_MESH(unit)) {
        /* each 4 bits represent a diffferent mc id. multicast replication to local egq is configured in bits 2:3 */
        SHR_BITCOPY_RANGE(table_default, 0, mesh_config_data, 0, SOC_JER_TBL_FDT_IPT_MESH_MC_LINE_LENGTH);
    } else {
