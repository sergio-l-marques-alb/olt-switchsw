...
#define ARAD_MC_ING_DESTINATION_ID_MASK_IS_LAG 0x1000
#define ARAD_MC_ING_DESTINATION_ID_MASK_LAG 0x7fff
...
)
{
    ...
    if (DPP_MCDS_TYPE_IS_INGRESS(entry_type)) { /* set ingress entry pointer */
        ...
        *next_entry = DPP_MC_EGRESS_LINK_PTR_END;
          ...
          case 1:
            {
                ...
                } else if (bitmap != DPP_MC_EGR_BITMAP_DISABLED && ++found <= max && core == 0) { /* replication not disabled for this entry */
                    dpp_add_egress_replication_bitmap(mcds, cud, bitmap);
                }
            }
            break;
          ...
          case 2:
            {
                ...
                if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
                    if (++found <= max) {
                        dpp_add_egress_replication_port_cud(mcds, core, soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_4m, mcdb_entry, OUTLIF_2f), port);
                    }
                }
            }
            break;
        ...
        if (port != DPP_MULT_EGRESS_PORT_INVALID) { /* replication not disabled for this entry part */
            if (++found <= max) {
                dpp_add_egress_replication_cud(mcds, core, port);
            }
        }
