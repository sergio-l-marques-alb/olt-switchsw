...
soc_arad_default_config_l3_get(int unit)
{
    ...
    dpp_l3->actual_nof_routing_enablers_bm = SOC_PPD_RIF_MAX_NOF_ROUTING_ENABLERS_BITMAPS;
	
    if(SOC_IS_JERICHO(unit)){
        if (dpp_l3->actual_nof_routing_enablers_bm > 8) {/*4 bits*/
    ...
    */
    ...
    switch(val)
    {
      ...
      if (SOC_IS_DPP_DRC_COMBO28(unit)) {
            ...
            if (SOC_IS_ARDON(unit)) {
                ...
                /* A is the master of B. C and D also have ref clk. 0xd=b1101. */
                ...
                if (dram_bitmap != 0) {
                    ...
                    /* A is the master of B, C is the master of D, F is the master of E, H is the master of G. */
    ...
    if (dpp_arad->init.pp_enable) {
     ...
     /* Default trap/snoop strength */
      ...
      SOC_DPP_CONFIG(unit)->pp.nof_eve_action_mappings = SOC_PPC_NOF_EGRESS_VLAN_EDIT_ACTION_MAPPINGS(unit);
      SOC_DPP_CONFIG(unit)->pp.nof_eve_action_ids = SOC_PPC_NOF_EGRESS_VLAN_EDIT_ACTION_IDS(unit);
...
soc_arad_bring_up_interrupts(int unit)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_dpp_wb_engine_init_buffer(unit, SOC_DPP_WB_ENGINE_BUFFER_DPP_INTERRUPTS));
...
soc_arad_default_ofp_rates_set(int unit) {
    ...
    SOC_PBMP_ITER(ports_bm, port_i) {
      ...
      rv = arad_ofp_rates_sch_single_port_rate_sw_set(unit, core, tm_port, sch_rate);
      if(SOC_FAILURE(rv)) {
         SOCDNX_EXIT_WITH_ERR(rv, (_BSL_SOCDNX_MSG_STR( "ERROR: failed to set port sch rate for port %d"), tm_port));
      }
                
      /*egq*/
      rv = arad_ofp_rates_egq_single_port_rate_sw_set(unit, core, tm_port, egq_rate);
      if(SOC_FAILURE(rv)) {
      ...
      {
         /*egq*/
         rv = arad_ofp_rates_egq_port_priority_rate_sw_set(unit,core,tm_port,priority_i,egq_rate);
         if(SOC_FAILURE(rv)) {
         ...
         /*sch*/
         rv =  arad_ofp_rates_sch_port_priority_rate_set(unit,core, tm_port,priority_i,sch_rate);
         if(SOC_FAILURE(rv)) {
      ...
      if (nof_priorities == ARAD_TCG_NOF_PRIORITIES_SUPPORT)
      {
        ...
        {          
            /*egq*/
            rv =  arad_ofp_rates_egq_tcg_rate_sw_set(unit,core,tm_port,tcg_i,tcg_egq_rate_arr[tcg_i]);
            if(SOC_FAILURE(rv)) {
    ...
    for (ndx = 0; ndx < ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB; ++ndx) {
        if(chan_arbs_first_ports[ndx].is_valid){
            /*sch*/
            rv = arad_ofp_rates_sch_single_port_rate_hw_set(unit, core, chan_arbs_first_ports[ndx].chan_arb_first_port);
            if(SOC_FAILURE(rv)) {
    ...
    /*egq*/
    rv = arad_ofp_rates_egq_single_port_rate_hw_set(unit,core,first_port);
    if(SOC_FAILURE(rv)) {
    ...
    for (ndx = 0; ndx < ndx_max; ++ndx) {
      if (if_ids_first_ports[ndx].is_valid) {
          rv = arad_sch_if_shaper_rate_set(unit, core, if_ids_first_ports[ndx].if_first_port, if_ids_first_ports[ndx].sch_total_if_rate);
          if(SOC_FAILURE(rv)) {
    ...
    for (chan_arb_id = 0; chan_arb_id < ARAD_OFP_RATES_EGQ_NOF_CHAN_ARB; ++chan_arb_id) {
      if (chan_arbs_first_ports[chan_arb_id].is_valid) {
          rv = arad_ofp_rates_egq_interface_shaper_set(unit,core,chan_arbs_first_ports[chan_arb_id].chan_arb_first_port, SOC_TMC_OFP_SHPR_UPDATE_MODE_OVERRIDE, chan_arbs_first_ports[chan_arb_id].egq_total_if_rate);
          if(SOC_FAILURE(rv)) {
