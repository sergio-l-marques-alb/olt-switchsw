  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_NIF_IPG_SET);

  SOC_SAND_CHECK_DRIVER_AND_DEVICE; 

  res = arad_nif_ipg_set_verify(
          unit,
          port,
          ipg
        );
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_nif_ipg_set()", port, ipg);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_NIF_IPG_GET);

  SOC_SAND_CHECK_DRIVER_AND_DEVICE; 

  res = arad_nif_ipg_get_verify(
          unit,
          port
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
exit_semaphore:
  ...
  ARAD_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_nif_ipg_get()", port, 0);
}
