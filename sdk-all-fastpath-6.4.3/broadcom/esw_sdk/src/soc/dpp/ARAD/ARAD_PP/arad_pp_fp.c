  ...
  )
{
  ...
  *success = SOC_SAND_FAILURE_OUT_OF_RESOURCES;
  ...
  if ((info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) || (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) || (info->flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB)) {
  ...
  )
{
  ...
  if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_SINGLE_BANK )
  {
         is_single_bank = TRUE;
  }
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  if ((fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) ||
      (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) ||
      (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB))
  {
      is_fp_db_extended_fwrd = TRUE;
  }
  ...
  else 
  {
    ...
    if (stage == SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP) {
        ...
        /* Insertion in KBP */
        if (is_fp_db_extended_fwrd) {
          route_key.key.elk_fwd.priority = entry_id_ndx; /* priority saves the entry ID */
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
     if (is_fp_db_extended_fwrd)
     {/* when is_fp_db_extended_fwrd, this function called from entry add.
         entry get is not supported in this mode. this should be change to support get functionality. 
         we can use the entry index or using the arad_pp_ip_tcam_entry_hw_general_get, needs more research.*/
        uint32 logical_entry_size_in_bytes, table_payload_in_bytes, tbl_id;
        if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) {
            tbl_id = ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED;
        } else if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) {
            tbl_id = ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6;
        } else {
            tbl_id = ARAD_KBP_FRWRD_TBL_ID_EXTENDED_P2P;
        }
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)        
    if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_FWRD_DB) {
      tcam_key_type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(ARAD_KBP_FRWRD_TBL_ID_LSR_IP_SHARED);
    }
    if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_IPV6_FWRD_DB) {
      tcam_key_type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(ARAD_KBP_FRWRD_TBL_ID_EXTENDED_IPV6);
    }
    if (fp_database_info.flags & SOC_PPC_FP_DATABASE_INFO_FLAGS_EXTENDED_P2P_FWRD_DB) {
      tcam_key_type = ARAD_IP_TCAM_FROM_KBP_FRWRD_IP_TBL_ID(ARAD_KBP_FRWRD_TBL_ID_EXTENDED_P2P);
    }
