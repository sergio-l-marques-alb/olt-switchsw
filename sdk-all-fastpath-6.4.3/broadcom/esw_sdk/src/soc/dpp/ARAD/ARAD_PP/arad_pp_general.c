...
#define ARAD_PP_ENCODE_NOF_BITS_19_MC_ID                         (16)
...
#define ARAD_PP_ENCODE_DEST_PREFIX_NOF_BITS_19_FEC_PTR            (SOC_IS_JERICHO(unit)? 2: 4)
#define ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_FEC_PTR               (SOC_IS_JERICHO(unit)? 1: 4)
#define ARAD_PP_ENCODE_DEST_PREFIX_NOF_BITS_19_MC_ID              (SOC_IS_JERICHO(unit)? 2: 3)
#define ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_MC_ID                 (SOC_IS_JERICHO(unit)? 2: 5)
...
#define ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_SYS_PORT              (SOC_IS_JERICHO(unit)? 1: 4)
...
#define ARAD_PP_ASD_NOF_BITS_OUTLIF_VALUE                        (16)
...
#define ARAD_PP_EEI_ENCAP_PTR_NOF_BITS                       (16)
    ...
    )
{
  ...
  switch(fwd_decision_type)
  {
    ...
    case ARAD_PP_FRWRD_DECISION_TYPE_UC_LAG: 
      ...
      value = ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_SYS_PORT;
    break;
    /*
     *  Explicit flow.
     */
  ...
  case ARAD_PP_FRWRD_DECISION_TYPE_MC:
      nof_bits = ARAD_PP_ENCODE_DEST_PREFIX_NOF_BITS_19_MC_ID;
      value = ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_MC_ID;
    break;
  ...
  case ARAD_PP_FRWRD_DECISION_TYPE_FEC:
       nof_bits = ARAD_PP_ENCODE_DEST_PREFIX_NOF_BITS_19_FEC_PTR;
       value = ARAD_PP_ENCODE_DEST_PREFIX_VALUE_19_FEC_PTR;
    break;
   /*
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_PP_FWD_DECISION_EEI_TYPE_FIND);
  ...
  )
{
  ...
  else if(fwd_decision->additional_info.eei.type == ARAD_PP_EEI_TYPE_OUTLIF) /* outlif */
  {
    ...
    res = soc_sand_bitstream_set_any_field(
            ...
            0,
            ARAD_PP_EEI_ENCAP_PTR_NOF_BITS,
            &(asd_buffer_lcl)
          );    
  ...
  )
{
  ...
  case ARAD_PP_FRWRD_DECISION_TYPE_UC_FLOW:
     ...
     */
  case ARAD_PP_FRWRD_DECISION_TYPE_MC:
    SOC_SAND_ERR_IF_ABOVE_MAX(info->dest_id, (1<<ARAD_PP_ENCODE_NOF_BITS_19_MC_ID)-1, ARAD_PP_FRWRD_DEST_MC_ID_OUT_OF_RANGE_ERR, 30, exit);
    break;
   ...
   */
    ...
    case ARAD_PP_OUTLIF_ENCODE_TYPE_EEP:
      SOC_SAND_ERR_IF_ABOVE_MAX(info->additional_info.outlif.val, SOC_DPP_DEFS_GET(unit, nof_out_lifs), ARAD_PP_FRWRD_DEST_OUTLIF_VAL_OUT_OF_RANGE_ERR, 46, exit);
      break;

    case ARAD_PP_OUTLIF_ENCODE_TYPE_RAW:
    case ARAD_PP_OUTLIF_ENCODE_TYPE_RAW_INVALID:
      SOC_SAND_ERR_IF_ABOVE_MAX(info->additional_info.outlif.val, (1<<ARAD_PP_ASD_NOF_BITS_OUTLIF_VALUE)-1, ARAD_PP_FRWRD_DEST_OUTLIF_VAL_OUT_OF_RANGE_ERR, 46, exit);
      break;
 ...
 */
  ...
  case ARAD_PP_EEI_TYPE_OUTLIF:
      SOC_SAND_ERR_IF_ABOVE_MAX(info->additional_info.eei.val.outlif , (1<<ARAD_PP_EEI_ENCAP_PTR_NOF_BITS)-1, ARAD_PP_FRWRD_DEST_OUTLIF_VAL_INVALID_ERR, 85, exit);
  break;
        ...
        )
{
    ...
    *additional_info = 0;
    *learn_buffer = fwd_decision_type == ARAD_PP_FRWRD_DECISION_TYPE_FEC ? (1<<15) ^ dest_id  
                    :(fwd_decision_type == ARAD_PP_FRWRD_DECISION_TYPE_UC_FLOW ?
                                    dest_id >> 2 :
                                    0 );
...
arad_pp_init_orientation_maps(int unit){ 
    int res;
    uint32
        sh_mask, /* split horizon mask in profile; orientation bit is 1, other profile bits are 0 */
    ...
    res = arad_pp_profile_mgmt_if_profile_get_type_mask(unit, SOC_PPC_PROFILE_MGMT_TYPE_INLIF, SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_ORIENTATION, &sh_mask);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    /* get the split horizon relevant bits of the profile */
    res = arad_pp_profile_mgmt_if_profile_get_type_mask(unit, SOC_PPC_PROFILE_MGMT_TYPE_OUTLIF, SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_ORIENTATION, &sh_mask);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
