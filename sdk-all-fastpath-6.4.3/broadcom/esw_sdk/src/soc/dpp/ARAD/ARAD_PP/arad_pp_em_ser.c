...
ARAD_PP_FRWRD_MACT_ENTRY_KEY    l2_traverse_mact_keys[ARAD_PP_EM_SER_LEM_TRVRS_ITER_BLK_SIZE];
ARAD_PP_FRWRD_MACT_ENTRY_VALUE  l2_traverse_mact_vals[ARAD_PP_EM_SER_LEM_TRVRS_ITER_BLK_SIZE];
...
uint32 arad_entry_index[ARAD_PP_EM_SER_DB_TYPE_NOF_TYPES][ARAD_PP_EM_SER_NOF_HASH_RESULTS] = 
            {{16375, 5903, 22433, 27896, 9645, 5150, 26058, 16115}, /* LEM */
             {1957, 1828, 50, 887, 3229, 70, 2892, 603},            /* ISA */
             {1957, 1828, 50, 887, 3229, 70, 2892, 603},            /* ISB */
...
uint32 arad_plus_entry_index[ARAD_PP_EM_SER_DB_TYPE_NOF_TYPES][ARAD_PP_EM_SER_NOF_HASH_RESULTS] = 
            {{23527, 2702, 17517, 20818, 25905, 29029, 4039, 2007}, /* LEM*/
             {2702, 3139, 2818, 4028, 3663, 37, 3232, 1956},        /* ISA */
             {2702, 3139, 2818, 4028, 3663, 37, 3232, 1956},        /* ISB */
             {586, 1978, 1381, 1105, 1808, 35, 853, 1135},          /* ESEM */
             {895, 1553, 1832, 1577, 1944, 1884, 14, 275},          /* OEMA */
             {376, 16, 260, 91, 672, 450, 495, 572},                /* OEMB */
             {565, 729, 1369, 1405, 1281, 185, 682, 350}            /* RMAPEM */
            };
...
/*
 * Get SER type according to address
 */
...
uint32 arad_pp_em_ser_mact_counter_fix(int unit)
{
    ...
    ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE    rule_val;
    ...
    ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE_clear(&rule_val);
...
uint32 arad_pp_em_ser_lem_fid_counters_update(int unit)
{
    ...
    uint32 tbl_data[1];
    SOC_SAND_TABLE_BLOCK_RANGE                      block_range;
    ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE          rule;
    ...
    while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter))) {
        /* activate traverse for MACT */
        res = arad_pp_frwrd_mact_get_block(unit,
                                           &rule,
                                           &block_range,
...
uint32 arad_pp_em_ser_lem_lif_counters_update(int unit)
{
    ...
    range0_min = ARAD_PP_FRWRD_MACT_LEARN_LIF_RANGE_BASE(0);
    range0_max = range0_min + ARAD_PP_FRWRD_MACT_LIMIT_RANGE_MAP_SIZE;
    range1_min = ARAD_PP_FRWRD_MACT_LEARN_LIF_RANGE_BASE(1);
...
uint32 arad_pp_em_ser_align_shadow_and_hw_lem(int unit)
{
    ...
    uint32 entry_index, entry_type, indx, nof_entries;
    ...
    for(entry_index = 0; entry_index < ARAD_CHIP_SIM_LEM_TABLE_SIZE; entry_index++) {
        ...
        if (is_valid) {
            ...
            if ((is_found) == FALSE) {
                ...
                if ((request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_SA_AUTH) || (request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_TRILL_ADJ)) {
                  entry_type = ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3;
                } 
                /* set entry type as host format so we can check the msb for format 3b (for roo application) */
                else if (request.key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_IP_HOST) {
...
uint32 arad_pp_em_ser_is_dynamic_get(uint32 unit, uint32 *key_data, uint32 *payload_data, uint8 *is_dynamic)
{
    ...
    if ((key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_SA_AUTH) || (key.type == ARAD_PP_LEM_ACCESS_KEY_TYPE_TRILL_ADJ)) {
      entry_type = ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3;
    } 
    /* set entry type as host format so we can check the msb for format 3b (for roo application) */
...
void arad_pp_em_ser_lem_recover(void *p1, void *p2, void *p3, void *p4, void *p5)
{
    ...
    if (!is_ser_exist) {
        ...
        /* Get entry's key */
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_IHP_MACT_DIAGNOSTICS_INDEXr(unit, entry_offset));
            ...
            if (is_valid) {
                ...
                if (is_found) {
                    ...
                    if (!is_dynamic) {
                        ...
                        /* Write payload with dynamic bit set */
                        res = soc_mem_array_write(unit,
                                                  IHP_MEM_590000m,
                                                  array_index,
                                                  MEM_BLOCK_ANY,             
...
void arad_pp_em_ser_em_recover(void *p1, void *p2, void *p3, void *p4, void *p5)
{
        ...
        (em_ser_type == ARAD_PP_EM_SER_TYPE_RMAPEM_KEYT)) 
    {
        ...
        if (is_valid) {
            /* Get entry's key */
            res = soc_reg32_set(unit, ser_info.diagnostics_index_reg, REG_PORT_ANY, 0, entry_offset);
            SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
            ...
            if (is_found) {
                ...
                /* not supposed to get here. Assumming a single bit flip. */
                res = SOC_SAND_ERR;
            }
        }
        else {
            /* Delete entry directly by setting the payload to all zeros */
            res = soc_mem_array_write(unit,
                                      ser_info.pldt_mem,
                                      array_index,
                                      MEM_BLOCK_ANY,            
