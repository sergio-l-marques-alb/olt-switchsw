...
#define _ARAD_PP_PROFILE_MGMT_EGRESS_PROFILE_ALLOCATE_AC_BIT(outlif_bit_map, profile_bit)\
do {\
    uint32 ac_bit; uint8 occupied;\
    for (ac_bit=0; ac_bit < SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_AC_LIF_NOF_BITS_MAX ; ++ac_bit) {\
                                soc_sand_rv = soc_sand_occ_bm_is_occupied(outlif_bit_map, ac_bit, &occupied);\
    ...
    if (occupied) {\
        SOCDNX_EXIT_WITH_ERR(SOC_E_RESOURCE, (_BSL_SOCDNX_MSG("Error, not enough resources out lif profile\n")));\
    }\
    soc_sand_rv = soc_sand_occ_bm_occup_status_set(outlif_bit_map, ac_bit, 1/*occupied*/);\
    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);\
    profile_bit = ac_bit;\
}while (0)
   ...
   ) 
{
    ...
    SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE 
        outlif_profile_types_pri[SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_NOF];
#endif
    ...
    inlif_profile_types_pri[5]=SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_ORIENTATION;
    inlif_profile_types_pri[6]=SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_USER;
#ifdef BCM_88675_A0
    /* Set priorities to the different applications according to the restrictions */
    outlif_profile_types_pri[0]=SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_ORIENTATION; /*restricted to 3 lsbs*/
    outlif_profile_types_pri[1]=SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_PCP; /*restricted to 3 lsbs */
    outlif_profile_types_pri[2]=SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_DEFAULT_MEP; /*restricted to 3 lsbs */
    outlif_profile_types_pri[3]=SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_TAGGED_MODE;
    outlif_profile_types_pri[4]=SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_USER;
#endif
    /* Init WB buffer */
    ...
    for (type_ndx = 0; type_ndx < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_NOF; type_ndx++) {
        switch (inlif_profile_types_pri[type_ndx]) {
        case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_SIMPLE_SAME_INTERFACE:
            if (soc_property_get(unit, spn_BCM886XX_LOGICAL_INTERFACE_BRIDGE_FILTER_ENABLE, 0)) {
                ...
                for (bit_index = 0; bit_index < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS_TRANSFERED_TO_EGRESS; bit_index++) {
                ...
                res = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_INLIF_PROFILE_MASK, &inlif_profile_types_pri[type_ndx], bit_index);
                SOCDNX_IF_ERR_EXIT(res);
            }
            break;
#ifdef BCM_88660_A0
            ...
            if (SOC_IS_ARADPLUS(unit)) {
                ...
                if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_BCM886XX_QOS_L3_L2_MARKING, 0)) {
                    ...
                    for (dscp_bit = 0; dscp_bit < SOC_PPC_PROFILE_MGMT_NOF_DSCP_BITS/*nof bits to allocate*/; dscp_bit++) {
                        ...
                        soc_sand_rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_INLIF_PROFILE_MASK, &inlif_profile_types_pri[type_ndx], bit_index);
...
#endif
#ifdef BCM_88675_A0
        case SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_TYPE_ORIENTATION:
            if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)) {
                for (bit_index = 0; bit_index < SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS; bit_index++) {
                    soc_sand_rv = soc_sand_occ_bm_is_occupied(inlif_bit_map, bit_index, &occupied);
                    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);
                    if (!occupied) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        /* Outlif Profile Allocation */
        for (type_ndx = 0; type_ndx < SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_NOF; type_ndx++) {
            switch (outlif_profile_types_pri[type_ndx]) {
            case SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_ORIENTATION:
                if (soc_property_get(unit, spn_SPLIT_HORIZON_FORWARDING_GROUPS_MODE, 0)) {
                    int profile_bit;
                    for (profile_bit = 0; profile_bit < 2; profile_bit++) { 
                        for (bit_index = 0; bit_index < SOC_PPC_PROFILE_MGMT_OUTLIF_AC_NOF_BITS; bit_index++) { /* restricting to 3LSB only, since AC profile has only 3 bits */
                            soc_sand_rv = soc_sand_occ_bm_is_occupied(outlif_bit_map, bit_index, &occupied);
                            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);
                            if (!occupied) {
                                break;
                            }
                        ...
                        soc_sand_rv = soc_sand_occ_bm_occup_status_set(outlif_bit_map, bit_index, 1/*occupied*/);
                        SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);

                        res = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_OUTLIF_PROFILE_MASK, &outlif_profile_types_pri[type_ndx], bit_index);
                        SOCDNX_IF_ERR_EXIT(res);
                    }
                }
                break;
            ...
            /* OAM outlif profile - just one bit.*/
            case  SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_PCP:
                if (SOC_INFO(unit).oamp_port[0] == ARAD_OAMP_PORT_ID) {
                    nof_profile_bits = SOC_PPC_OAM_NUMBER_OF_BITS_USED_BY_OUTLIF_PCP;
                    while (nof_profile_bits--) {
                        _ARAD_PP_PROFILE_MGMT_EGRESS_PROFILE_ALLOCATE_AC_BIT(outlif_bit_map,bit_index);
                        soc_sand_rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_OUTLIF_PROFILE_MASK, &outlif_profile_types_pri[type_ndx], bit_index);
                        SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                }
                break;
            case  SOC_PPC_PROFILE_MGMT_OUTLIF_PROFILE_TYPE_OAM_DEFAULT_MEP:
                nof_profile_bits = soc_property_get(unit, spn_BCM886XX_OAM_DEFAULT_PROFILE_EGRESS, 0);
                while (nof_profile_bits--) {
                    _ARAD_PP_PROFILE_MGMT_EGRESS_PROFILE_ALLOCATE_AC_BIT(outlif_bit_map, bit_index);
                    soc_sand_rv = SOC_DPP_WB_ENGINE_SET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_SW_DB_OUTLIF_PROFILE_MASK, &outlif_profile_types_pri[type_ndx], bit_index);
                    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_rv);
