...
#define _ARAD_PP_OAM_RMEP_EMC_SHIFT_BFD (SOC_IS_JERICHO(unit)?14: 16)
#define _ARAD_PP_OAM_RMEP_EMC_SHIFT_ETH (SOC_IS_JERICHO(unit)?14: 13)
...
#define _ARAD_PP_OAM_BFD_YOUR_DISC_MAX ((1<<16)-1)
...
#define _ARAD_PP_OAM_RMEP_SCAN(freq_arad_khz)          SOC_SAND_DIV_ROUND_DOWN(16384 * 20, SOC_SAND_DIV_ROUND_DOWN(freq_arad_khz, 1000))
...
#define _ARAD_PP_OAM_BFD_IS_MEP_TYPE_USING_YOUR_DISC(mep_type)  \
  ((mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP) ||       \
   ...
   0/*(mep_type == SOC_PPD_OAM_MEP_TYPE_BFD_O_MPLS) In Lif solution*/)
...
#define _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(trap_code, internal_trap_code)                                                       \
  do {                                                                                                                            \
  ...
  )
{
  ...
  if (_ARAD_PP_OAM_BFD_IS_MEP_TYPE_USING_YOUR_DISC(classifier_mep_entry->mep_type)) {
      ...
      SOC_SAND_ERR_IF_ABOVE_NOF(classifier_mep_entry->lif, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 110, exit);
  }
    ...
    )
{
  ...
  for (flp_program = 1; flp_program <= ARAD_PP_FLP_KEY_PROGRAM_LAST; flp_program ++) {
	  ...
	  for (indx = 0; indx < SOC_DPP_DEFS_GET(unit, nof_flp_programs); ++indx) {
		  uint8 prog_usage;
		  arad_pp_sw_db_flp_prog_index_to_app_get(unit, indx, &prog_usage);
...
0;
    ...
    res = soc_mem_read_range(unit, IHP_OAMBm, MEM_BLOCK_ANY, 0,IHP_OAMBm, oamb_buffer->buffer);
    SOC_SAND_SOC_IF_ERROR_RETURN(res,10,exit,res); 
  ...
  )
{
    ...
    if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            soc_reg_field_set(unit, OAMP_MODE_REGISTERr, &reg, REPORT_MODEf, 0); /* Normal report mode.*/
        }
	}
  ...
  )
{
  ...
  if (is_bfd && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_lsp_support", 0) ) {
...
uint32
  ...
  for (inlif_profile = 0; inlif_profile < (1<<SOC_PPC_PROFILE_MGMT_INLIF_PROFILE_NOF_BITS); inlif_profile++) {
      /* For each profile, extract the oam profile bits and set the same value in the mapping (1:1) */
      res = arad_pp_profile_mgmt_if_profile_get_val_from_map(unit,
  ...
  )
{
  ...
  if (is_bfd) {
      ...
      soc_reg_field_set(unit, IHP_OAM_IDENTIFICATION_ENABLEr, &reg, OAM_ONE_HOP_BFD_ENABLEf, 
                        !soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_lsp_support", 0)); /* Currently BFD LSP and BFD over IPV4 one hop are mutually exclusive.*/
  ...
  /* Setting traps */
  if (is_bfd) {
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(SOC_PPC_TRAP_CODE_USER_DEFINED_DROP_TRAP, oam_trap_code);
      ...
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(SOC_PPC_TRAP_CODE_USER_DEFINED_DFLT_TRAP, oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_FRWRD, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_bfd->trap_ids[SOC_PPD_BFD_TRAP_ID_CPU], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_TRAP_CODE_TRAP_TO_CPU, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_bfd->trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_CC_MPLS_TP], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_TRAP_CODE_OAMP_BFD_CC_MPLS_TP, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_bfd->trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_IPV4], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_TRAP_CODE_OAMP_BFD_IPV4, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_bfd->trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_MPLS], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_TRAP_CODE_OAMP_BFD_MPLS, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 214, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_bfd->trap_ids[SOC_PPD_BFD_TRAP_ID_OAMP_PWE], oam_trap_code);
      ...
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_FRWRD, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 236, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->trap_ids[SOC_PPD_OAM_TRAP_ID_RECYCLE], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_RECYCLE, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 236, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->trap_ids[SOC_PPD_OAM_TRAP_ID_SNOOP], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_SNOOP, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 236, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->trap_ids[SOC_PPD_OAM_TRAP_ID_ERR_LEVEL], oam_trap_code);
      res = SOC_DPP_WB_ENGINE_SET_VAR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_OAM_TRAP_CODE_TRAP_TO_CPU_LEVEL, &oam_trap_code);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 236, exit);
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->trap_ids[SOC_PPD_OAM_TRAP_ID_ERR_PASSIVE], oam_trap_code);
      ...
      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->upmep_trap_ids[SOC_PPD_OAM_UPMEP_TRAP_ID_CPU], mirror_trap_code);

#ifdef  _ARAD_PP_OAM_LOOPBACK_UPMEP_SUPPORTED /* LoopBack upmep is not supported in ARAD */

      _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(init_trap_info_oam->upmep_trap_ids[SOC_PPD_OAM_UPMEP_TRAP_ID_RECYCLE], mirror_trap_code);
  ...
  if (!is_second_init) {
	  ...
	  for (port_index = 0; port_index < ARAD_PORT_NOF_PP_PORTS; port_index++) {
          res = soc_mem_read(unit, pinfo_flp_mem, MEM_BLOCK_ANY, port_index, &entry);
          SOC_SAND_CHECK_FUNC_RESULT(res, 24, exit);
          soc_mem_field32_set(unit, pinfo_flp_mem, &entry, ENABLE_PP_INJECTf, 1);
          res = soc_mem_write(unit, pinfo_flp_mem, MEM_BLOCK_ANY, port_index, &entry);
	  ...
	  if (!is_bfd && ((egress_count * ingress_count) == 0)) {
		  LOG_WARN(BSL_LS_SOC_OAM,
                           (BSL_META_U(unit,
  ...
  )
{
    ...
    if (!is_bfd) {
        res = arad_pp_sw_db_oam_my_mac_lsb_terminate(unit);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    }
  ...
  )
{
  ...
  ARAD_PP_SW_DB_OAM_MY_MAC_LSB_REF_COUNT        *ref_count = NULL;
  int       core=0; 
  ...
  if ((dst_mac_address->address[5] & 0x1) != 1) {
      ...
      ARAD_ALLOC(ref_count, ARAD_PP_SW_DB_OAM_MY_MAC_LSB_REF_COUNT, 1, "arad_pp_oam_my_cfm_mac_set.ref_count");
      res = arad_pp_sw_db_oam_my_mac_lsb_ref_count_get(unit, ref_count);
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
      ref_count->ref_count[_ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT_INDEX(table_index, dst_mac_address)]++;
      res = arad_pp_sw_db_oam_my_mac_lsb_ref_count_set(unit, ref_count);
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
...
soc_error_t
  arad_pp_oam_my_cfm_mac_get(int unit, SOC_SAND_PP_MAC_ADDRESS *dst_mac_address, uint32 table_index){
    ...
    SOCDNX_ALLOC(ref_count, ARAD_PP_SW_DB_OAM_MY_MAC_LSB_REF_COUNT, 1, "arad_pp_oam_my_cfm_mac_delete.ref_count");

    /* Get counters to check if this lsb is configured for this port. */
    rv = arad_pp_sw_db_oam_my_mac_lsb_ref_count_get(unit, ref_count);
  ...
  )
{
  ...
  ARAD_ALLOC(ref_count, ARAD_PP_SW_DB_OAM_MY_MAC_LSB_REF_COUNT, 1, "arad_pp_oam_my_cfm_mac_delete.ref_count");
  res = arad_pp_sw_db_oam_my_mac_lsb_ref_count_get(unit, ref_count);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ref_count->ref_count[_ARAD_PP_OAM_MY_MAC_LSB_REF_COUNT_INDEX(table_index, dst_mac_address)]--;
  res = arad_pp_sw_db_oam_my_mac_lsb_ref_count_set(unit, ref_count);
      ...
      for (mip_type = 0; mip_type <nof_mp_types_to_set; mip_type++) {
          ...
          if (is_bfd) {
              ...
              *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
              ...
              _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[internal_opcode], oam_payload.cpu_trap_code);
              ...
              for (is_my_cfm_mac = 0; is_my_cfm_mac <=1; is_my_cfm_mac++) {
                  ...
                  for (opcode = 1; opcode < SOC_PPD_OAM_ETHERNET_PDU_OPCODE_COUNT;  opcode++) {
                      ...
                      *meter_disable_lcl = profile_data->mip_profile_data.meter_disable; 
                      oam_payload.meter_disable = (SHR_BITGET(meter_disable_lcl, internal_opcode) != 0);
                      if (is_my_cfm_mac) {
                          _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(profile_data->mep_profile_data.opcode_to_trap_code_unicast_map[internal_opcode], oam_payload.cpu_trap_code);
  ...
  )
{
    ...
    *meter_disable_lcl = profile_data->mep_profile_data.meter_disable;
    ...
    if (oam_trap_code > 0) {
        _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(oam_trap_code, oam_payload->cpu_trap_code); 
    }
    else {
                  ...
                  case SOC_PPD_OAM_MP_TYPE_MIP_OR_PASSIVE_MATCH_PLUS:
                      ...
                      if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) { /* MIP */
                          ...
                          *meter_disable_lcl = profile_data->mip_profile_data.meter_disable;
                          ...
                          /* MIP is active from both sides - active and passive.
                      ...
                      if ((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && (profile_data->mp_type_passive_active_mix)) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit) ) { 
                          /* Passive: For Jericho insert seperate entries for MIP, passive.
  ...
  if (!SOC_PPD_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type) ||
      ...
      if (!SOC_PPD_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) {
          ...
          if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
              ...
              if ( (found && ((oem1_prev_payload.mp_profile == ARAD_PP_OAM_PROFILE_PASSIVE) || (!is_mep))) || !found) {
                  /* If previous entry is passive or we add a mip - update it (otherwise do nothing) */
                  res = arad_pp_oam_classifier_oem1_entry_set_unsafe(unit, &oem1_key, &oem1_payload); 
                  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
              } else {
  ...
  )
{
  ...
  if ((mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) ||
	  ...
	  if (SOC_IS_ARAD_B1_AND_BELOW(unit)) {
		  /* in Arad+ and above this bit is controlled by the HW.*/
      ...
      if (mep_db_entry->mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM) { /* Only relevant for Ethernet */
          ...
          if (SOC_IS_ARADPLUS_A0(unit)) {
              ...
              arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_UP_MEP_MAC, &reg_val);
  ...
  )
{
  ...
  rmep_scan_time = _ARAD_PP_OAM_RMEP_SCAN(arad_chip_kilo_ticks_per_sec_get(unit));
  if (SOC_SAND_DIV_ROUND_DOWN(rmep_db_entry->ccm_period*1000, rmep_scan_time) <= 0x3FF) {
      lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD;
      lifetime = ((rmep_db_entry->ccm_period*1000 / rmep_scan_time) < 1) ? 1 : (rmep_db_entry->ccm_period*1000 / rmep_scan_time);
  }
  else if (SOC_SAND_DIV_ROUND_DOWN(rmep_db_entry->ccm_period*10, rmep_scan_time) <= 0x3FF){
      lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD_100;
      lifetime =(((rmep_db_entry->ccm_period*10) / rmep_scan_time) < 1) ? 1 : ((rmep_db_entry->ccm_period*10) / rmep_scan_time);
  }    
  else {
      lifetime_units = _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD_10000;
      lifetime = ((rmep_db_entry->ccm_period / (rmep_scan_time*10)) < 1) ? 1 : (rmep_db_entry->ccm_period / (rmep_scan_time*10));
  }
  ...
  )
{
  ...
  if (lifetime_units == _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD) {
      rmep_db_entry->ccm_period = SOC_SAND_DIV_ROUND(lifetime*rmep_scan_time, 1000);
  }
  else if (lifetime_units == _ARAD_PP_OAM_OAMP_RMEP_DB_LIFETIME_UNITS_SCAN_PERIOD_100){
      rmep_db_entry->ccm_period = SOC_SAND_DIV_ROUND(lifetime*rmep_scan_time, 10);
  }
  else {
      rmep_db_entry->ccm_period = lifetime*rmep_scan_time*10;
  } 
  ...
  )
{
    ...
    SOC_SAND_ERR_IF_ABOVE_NOF(lif, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 20, exit); 
exit:
    ...
    *is_mp_type_flexible = TRUE;
    ...
    if (*found_profile) {
        *profile = oem1_payload.mp_profile;
        if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
            *found_mep = JERICHO_PP_OAM_EXTRACT_MDL_MP_TYPE_FROM_MP_TYPE_VECTOR_BY_LEVEL(oem1_payload.mp_type_vector, md_level) !=0;
        } else {
  ...
  )
{
  ...
  ipv6_addr.address[0] = dip;
  ipv6_addr.address[1] = 0xFFFF;
  ipv6_addr.address[2] = 0;
  ipv6_addr.address[3] = 0;

  valid = 1;
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_IHP_MY_BFD_DIPm(unit, MEM_BLOCK_ANY, dip_index, reg_above_64));
  soc_IHP_MY_BFD_DIPm_field_set(unit, reg_above_64, DIPf, ipv6_addr.address);
  ...
  )
{
  ...
  if (valid) {
      ...
      dip->address[0] = ipv6_addr.address[0];
  }
  else {
      dip->address[0] = 0;
  }
    ...
    if (interrupt_message[unit][last_interrupt_message_num[unit]] == empty_fifo_value || 
        ...
        *rmeb_db_ndx = *rmeb_db_ndx_lcl;
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            *rmep_state = event_data.rmep_state;
            if (event_data.loc == 1) {
  ...
  )
{
    ...
    if (mep_db_entry->is_1DM) {
        ...
        /* In case update was called and previously this field was set*/
       ...
       )
{
    ...
    *found_bitmap=0;
    ...
    while (mep_type == SOC_PPD_OAM_MEP_TYPE_LM || mep_type==SOC_PPD_OAM_MEP_TYPE_DM
        ...
        ++cur;
        SHR_BITSET(found_bitmap,mep_type );
        GET_TYPE_OF_OAMP_MEP_DB_ENTRY(mep_type, cur, reg_data);
    }
  ...
  )
{
    ...
    if (!is_lm) { /* Reset the MEP_PE_PROFILE*/
        ...
        if (SOC_IS_ARADPLUS_A0(unit)) {
            ...
            arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_UP_MEP_MAC, &mep_pe_prof); 
        }
       ...
       )
{
    ...
    if (next_is_itmh) {
        ...
        fwd_dst_info = (mep_db_entry.mep_type == SOC_PPD_OAM_MEP_TYPE_ETH_OAM)? (0xc0000 | entry) : (0xa0000 | mep_db_entry.egress_if);
       ...
       )
{
    ...
    _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(trap_code,internal_trap_code);
  ...
  )
{
    ...
    for(lookup_reg_idx = 0; lookup_reg_idx < nof_last_lookup_regs; ++lookup_reg_idx) {
        ...
        key = soc_reg64_field_get(unit,last_lookup_regs[lookup_reg_idx], oema_ll_reg,OEMA_LAST_LOOKUP_KEYf);
        result = soc_reg64_field_get(unit, last_lookup_regs[lookup_reg_idx], oema_ll_reg, OEMA_LAST_LOOKUP_RESULTf);
        found = soc_reg64_field_get(unit, last_lookup_regs[lookup_reg_idx], oema_ll_reg, OEMA_LAST_LOOKUP_RESULT_FOUNDf);
    ...
    for(lookup_reg_idx = 0; lookup_reg_idx < nof_last_lookup_regs; ++lookup_reg_idx) {
        ...
        SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);
        key =soc_reg64_field_get(unit, last_lookup_regs[lookup_reg_idx], oemb_ll_reg, OEMB_LAST_LOOKUP_KEYf);
        result = soc_reg64_field_get(unit, last_lookup_regs[lookup_reg_idx], oemb_ll_reg, OEMB_LAST_LOOKUP_RESULTf);
        found =  soc_reg64_field_get(unit, last_lookup_regs[lookup_reg_idx], oemb_ll_reg, OEMB_LAST_LOOKUP_RESULT_FOUNDf);
        if (COMPILER_64_LO(found) || COMPILER_64_LO(key)) {
    		key32 = COMPILER_64_LO(key);
    		result32[0] = COMPILER_64_LO(result); 
...
{ 
    ...
    if ((port_type == SOC_PORT_IF_OAMP) &&  (opaque_pt_attr != _ARAD_PP_OAM_PTCH_OPAQUE_VALUE)) {
        ...
        _ARAD_PP_OAM_TRAP_CODE_TO_INTERNAL(trap_code,oam_trap_code);
