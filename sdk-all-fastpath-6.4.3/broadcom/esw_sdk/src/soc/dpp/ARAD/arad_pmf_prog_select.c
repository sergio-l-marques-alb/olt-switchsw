    ...
    for (db_id_ndx = 0; db_id_ndx < ARAD_PP_FP_NOF_DBS; ++db_id_ndx)
    {
        ...
        if (SHR_BITGET(sw_db_info.progs, pmf_pgm_from)) {
            ...
            *prog_used_cycle_bmp_lcl = sw_db_info.prog_used_cycle_bmp;
            ...
            if (switch_key_egq) {
                /* Invert the Keys */
                sw_db_info.used_key[pmf_pgm_to][0] = (SOC_DPP_DEFS_GET(unit, nof_egress_pmf_keys) - 1) - sw_db_info.used_key[pmf_pgm_to][0];
                sw_db_info.used_key[pmf_pgm_to][1] = (SOC_DPP_DEFS_GET(unit, nof_egress_pmf_keys) - 1) - sw_db_info.used_key[pmf_pgm_to][1];
            }
        }
