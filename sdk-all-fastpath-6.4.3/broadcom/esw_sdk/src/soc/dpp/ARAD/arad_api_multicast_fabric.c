...
{
  uint32  res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MULT_FABRIC_CREDIT_SOURCE_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  SOC_SAND_CHECK_NULL_INPUT(info);
  SOC_SAND_CHECK_NULL_INPUT(exact_info);

  res = arad_mult_fabric_credit_source_verify(
          unit,
          info
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = arad_mult_fabric_credit_source_set_unsafe(
    unit,
    info,
    exact_info
  );
...
exit_semaphore:
   SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mult_fabric_credit_source_set()",0,0);
}
...
{
  uint32  res;
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MULT_FABRIC_CREDIT_SOURCE_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
  ...
  res = arad_mult_fabric_credit_source_get_unsafe(
    unit,
    info
  );
  SOC_SAND_CHECK_FUNC_RESULT(res, 100, exit_semaphore);
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mult_fabric_credit_source_get()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MULT_FABRIC_ENHANCED_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mult_fabric_enhanced_set()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_MULT_FABRIC_ENHANCED_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_mult_fabric_enhanced_get()",0,0);
}
