...
uint32 arad_kbp_cpu_lookup_reply(
    ...
    soc_reg_above_64_val_t data)
 {
     ...
     SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg_above_64_get(unit, IHB_LOOKUP_REPLY_DATAr, REG_PORT_ANY, 0, data));
...
/*  when ARAD_KBP_ROP_OPTIMIZATION we are transmitting packets without reading the counter so we need to zero the counter before reading */
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 70, exit, soc_reg32_get(unit, IHB_CPU_LOOKUP_REPLY_COUNTERr, REG_PORT_ANY, 0, &reg_val));        
#endif
    ...
    if (lsb_enable == 0) {
        if (msb_trigger_is_on == 0) {
            ...
            soc_reg_field_set(unit, IHB_CPU_RECORD_CONTROLr, &reg_val,  CPU_RECORD_DATA_MSB_TRIGGERf, 1);
            soc_reg_field_set(unit, IHB_CPU_RECORD_CONTROLr, &reg_val,  CPU_RECORD_DATA_LSB_TRIGGERf, 0);
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg32_set(unit, IHB_CPU_RECORD_CONTROLr, REG_PORT_ANY, 0, reg_val));
            SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on = 1;
        }
    } else {
        if (msb_trigger_is_on == 1) {
            ...
            soc_reg_field_set(unit, IHB_CPU_RECORD_CONTROLr, &reg_val,  CPU_RECORD_DATA_MSB_TRIGGERf, 0);
            soc_reg_field_set(unit, IHB_CPU_RECORD_CONTROLr, &reg_val,  CPU_RECORD_DATA_LSB_TRIGGERf, 1);
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, soc_reg32_set(unit, IHB_CPU_RECORD_CONTROLr, REG_PORT_ANY, 0, reg_val));
            SOC_DPP_CONFIG(unit)->arad->init.elk.msb_trigger_on = 0;
        }
    }
    reg_val = 0x0;
    ...
    if (lsb_enable == 0x1) {
        ...
        for(;;){
            ...
            if(reg_val != 0) {
                LOG_ERROR(BSL_LS_SOC_TCAM,
                          (BSL_META_U(unit,
                                      "Error in %s(): register IHB_CPU_LOOKUP_REPLY_COUNTERr not asserted correctly."), FUNCTION_NAME()));
            ...
            if (soc_timeout_check(&to)) {
              LOG_ERROR(BSL_LS_SOC_TCAM,
                        (BSL_META_U(unit,
                                    "Error in %s(): IHB_CPU_LOOKUP_REPLY register LOOKUP_REPLY_VALID field not asserted correctly."), FUNCTION_NAME()));
              SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
              break;
            }
        }

    SOC_SAND_SOC_IF_ERROR_RETURN(res, 120, exit, arad_kbp_cpu_lookup_reply(unit, data));
    ...
    if (read_data != NULL){
        sal_memcpy(read_data, data, SOC_REG_ABOVE_64_MAX_SIZE_U32 * sizeof(uint32));
    }
    ...
    arad_kbp_lut_write(int unit, uint8 addr, arad_kbp_lut_data_t* lut_data, soc_reg_above_64_val_t lut_data_raw)
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        res = aradplus_kbp_cpu_record_send(unit, ARAD_KBP_CPU_WR_LUT_OPCODE, msb_data, lsb_data, 0, NULL);
    } else
#endif 
    {
