      ...
      for (; size_left; --size_left) {
        ...
        if (port == _SHR_GPORT_INVALID) { /* CUD only replication */
          if (nof_active_cores > 1) { /* add to the appropriate core */
            core = DPP_CUD2CORE_GET_CORE(mcds, cud);
          }
...
)
{
  ...
  uint16 size_left = mcds->nof_reps[0], *specific_nof_reps;
  uint32 core = 0; /* number of core which may be >0 when multiple linked lists exist for the group */
  SOCDNX_INIT_FUNC_DEFS;
  *out_err = _SHR_E_NONE;
    ...
    if (dest == _SHR_GPORT_INVALID) { /*outlif only replication */
      if (mcds->cud2core) { /* get core from CUD */
          if ((core = DPP_CUD2CORE_GET_CORE(mcds, cud)) >= SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores) {
              SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("CUD 0x%x to be removed is not assigned to a core"), cud));
          }
      }
  ...
  *egress_groups_open_data_ptr = NULL;
  ...
  { /* allocate memory for the arrays */
    ...
    if (nof_active_cores > 1 ) {
      ...
      SOCDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, (void**)&mcds->cud2core, bytes_to_alloc, "mcds-mc-prev_entries"));
