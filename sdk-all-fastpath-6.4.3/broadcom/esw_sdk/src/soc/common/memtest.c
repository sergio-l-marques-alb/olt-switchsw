                         ...
                         int index, ser_test_data_t *test_data)
{
    ...
    if ((test_field == INVALIDf) || (test_field == 0) ||
        !SOC_MEM_FIELD_VALID(unit, mem, test_field)
       ) {
        ...
        if (!found && (test_data->mem_info != NULL)) {
           ...
           for (i = 0; i < field_num; i++) {
               ...
               field_length = soc_mem_field_length(unit, mem,
                ...
                _soc_ser_test_t test_type, int * error_count)
{
    ...
    for (j = 0; j < numIndexesToTest; j++) {
        ...
        rv = (ser_test_mem_read(unit, test_data));
        if (SOC_FAILURE(rv)) {
            (*error_count)++;
#ifdef _SOC_SER_ENABLE_CLI_DBG
            LOG_CLI((BSL_META_U(unit,
        ...
        /*Inject error:*/
        rv = (soc_ser_test_inject_error(unit, test_data, 0));
        if (SOC_FAILURE(rv)) {
            (*error_count)++;
#ifdef _SOC_SER_ENABLE_CLI_DBG
            LOG_CLI((BSL_META_U(unit,
