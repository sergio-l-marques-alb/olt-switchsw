...
_mac_cl_drain_cells(int unit, soc_port_t port, int notify_phy)
{
    ...
    SOC_IF_ERROR_RETURN(soc_egress_drain_cells(unit, port,
                                               SAL_BOOT_QUICKTURN ?
                                               250000000 : 250000));
...
mac_cl_init(int unit, soc_port_t port)
{
    ...
    soc_reg64_field32_set(unit, CLMAC_TX_CTRLr, &tx_ctrl, AVERAGE_IPGf,
                          (ipg / 8) & 0x1f);
    soc_reg64_field32_set(unit, CLMAC_TX_CTRLr, &tx_ctrl, CRC_MODEf, 2);
        ...
        if (!SOC_IS_TRIDENT2PLUS(unit)) {
            ...
            if (IS_XE_PORT(unit, port)) {
                *speed = 40000;
                ...
                if (SOC_INFO(unit).port_num_lanes[port] == 1) {
                    *speed = 10000;
                    if (fval == 16) {
                        *speed = 25000;
                    }
    ...
    if (!SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        if (to_hg_port) {
            ...
            if (soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE, 0)) {
                soc_reg_field_set(unit, CPORT_CONFIGr, &rval, HIGIG2_MODEf, 1);
            }
        } else {
            soc_reg_field_set(unit, CPORT_CONFIGr, &rval, HIGIG_MODEf, 0);
        }
...
mac_cl_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    if (!SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        if (IS_E_PORT(unit, port) && mode != SOC_ENCAP_IEEE) {
            /* CE -> HG */
            SOC_IF_ERROR_RETURN(_mac_cl_port_mode_update(unit, port, TRUE));
        } else if (IS_ST_PORT(unit, port) && mode == SOC_ENCAP_IEEE) {
            /* HG -> CE */
            SOC_IF_ERROR_RETURN(_mac_cl_port_mode_update(unit, port, FALSE));
        }
                  ...
                  int value)
{
    ...
    case SOC_MAC_PASS_CONTROL_FRAME:
        ...
        soc_reg64_field32_set(unit, CLMAC_RX_CTRLr, &rval, RX_PASS_CTRLf,
                              value ? 1 : 0);
                  ...
                  int *value)
{
    ...
    case SOC_MAC_PASS_CONTROL_FRAME:
        SOC_IF_ERROR_RETURN(READ_CLMAC_RX_CTRLr(unit, port, &rval));
        *value = soc_reg64_field32_get(unit, CLMAC_RX_CTRLr, rval,
                                       RX_PASS_CTRLf);
        break;
                          ...
                          soc_port_ability_t *ability)
{
    ...
    if (!SOC_IS_TRIDENT2PLUS(unit)) {
        ...
        port_speed_max = 25000 * si->port_num_lanes[port]; 
        port_speed_max = si->port_speed_max[port] < port_speed_max ?
            si->port_speed_max[port] : port_speed_max;
    }
