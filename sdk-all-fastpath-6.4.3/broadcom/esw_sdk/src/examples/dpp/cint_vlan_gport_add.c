/* 
* $Id: cint_vlan_gport_add.c,v 1.2 Broadcom SDK $
*
...
int vlan_gport_get_egress_multicast_vsi(int unit, bcm_vlan_t vsi, bcm_gport_t gport){
	...
	int port_max=10, port_count, i, tested_port, ret_val = FALSE, mask = 255;
...
int vlan_gport_basic_example(){
	...
	vlan_gport_bridgingInit();


	/*
	* Case 1: Setting membership for port 15, vlan 20 
	...
	*/
	...
	/*
	* Case 2: Setting tagged mode per sysPort == 15 and  vlan == 20
	...
	*/
	...
	/*
	* Case 3: Setting egress vsi membership  per sysPort == 15 and  vsi == 10
	* Get function check verifies whether we get vlan_gport_info.flags ==  0, which indicates that we are in tagged mode
	* After toggling we should get:  vlan_gport_info.flags == BCM_VLAN_GPORT_ADD_UNTAGGED, which indicates untagged mode 
	* Another toggling should bring us back to tagged mode  
	*/
	...
	/*
	* Case 4: Setting membership of port in vsi egress multicast group per sysPort == 15 and  vsi == 10
