...
int C4_DEFAULT_E2E_SCHED_FLAGS = (BCM_COSQ_GPORT_SCHEDULER | BCM_COSQ_GPORT_SCHEDULER_HR_DUAL_WFQ);
...
int C4_SCHED_EIR_KBITS_SEC_MAX_MARGIN = 10; /* 10 Kbps */
...
*/
    ...
    port = idx + 1;
    system_port = idx + 1;
    BCM_GPORT_MODPORT_SET(dest_gport[idx], c4_my_modid, port);
    BCM_GPORT_MODPORT_SET(src_gport[idx], c4_my_modid, port);
    ...
    /* CL (sf2) setup */
    BCM_COSQ_GPORT_COMPOSITE_SF2_SET(c4_gport_ucast_scheduler_sf2[idx],c4_gport_ucast_scheduler[idx]); 
    ...
    BCM_COSQ_GPORT_COMPOSITE_SF2_SET(c4_gport_ucast_scheduler1_sf2[idx],c4_gport_ucast_scheduler1[idx]); 
    ...
    rv = bcm_cosq_gport_add(c4_unit, src_gport[idx], num_cos, 
                flags, &c4_gport_ucast_voq_connector_group0[idx]);
    ...
    flags = BCM_COSQ_GPORT_VOQ_CONNECTOR | default_flags;
    rv = bcm_cosq_gport_add(c4_unit, src_gport[idx], num_cos, 
                flags, &c4_gport_ucast_voq_connector_group1[idx]);
...
int
e2e_composite_se_verify_mode(int unit, bcm_gport_t gport, int cosq, int exp_mode, int exp_weight)
{
    int                           rc = BCM_E_NONE;
    int                           mode, weight;
        ...
        switch(nbr_sched) {
            case 0:
                for (nbr_sched_flw = 0; nbr_sched_flw < C4_NBR_SCHEDULER_FLOWS; nbr_sched_flw++) {
                    ...
                    rc = e2e_composite_se_verify_mode(c4_unit, gport_ucast_scheduler, 0, exp_mode, exp_weight);
                    if (rc != BCM_E_NONE) {
                        return(rc);
                    }
