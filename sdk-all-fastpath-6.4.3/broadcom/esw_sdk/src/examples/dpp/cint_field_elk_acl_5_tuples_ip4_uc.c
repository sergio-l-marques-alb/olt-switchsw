...
/* $Id: cint_field_elk_acl_5_tuples_ip4_uc.c,v 1.3 Broadcom SDK $
                                             ...
                                             /* in */ int is_serial_rpf) 
{
  ...
  for (stage_idx = 0; stage_idx < nof_stages; stage_idx++) {
      ...
      result = bcm_field_qualify_Stage(unit, presel_id[stage_idx] | BCM_FIELD_QUALIFY_PRESEL, field_stage[stage_idx]);
      if (BCM_E_NONE != result) {
      ...
      for (app_type_idx = 0; app_type_idx < nof_app_types; app_type_idx++) {
          result = bcm_field_qualify_AppType(unit, presel_id[stage_idx] | BCM_FIELD_QUALIFY_PRESEL, app_type[app_type_idx]);
          if (BCM_E_NONE != result) {
      ...
      if (stage_idx == 1) { /* Ingress FP stage */
          if (is_serial_rpf) {
              ...
              result = bcm_field_qualify_ExternalHit1(unit, presel_id[stage_idx] | BCM_FIELD_QUALIFY_PRESEL, 0x1, 0x1);
          }
          else {
             /* Use the hit indication 2 because the ELK ACL will be in lookup 2 */
              result = bcm_field_qualify_ExternalHit2(unit, presel_id[stage_idx] | BCM_FIELD_QUALIFY_PRESEL, 0x1, 0x1);
          }
          if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_Stage\n");
            return result;
...
int elk_acl_5_tuples_ipv4_uc_example(int unit) 
{
  ...
  int is_aradplus, is_serial_rpf = 0;

  result = is_arad_plus(unit, &is_aradplus);
  if (result != BCM_E_NONE) {
      printf("Error, in is_arad_plus\n");
      return rv;
  }
