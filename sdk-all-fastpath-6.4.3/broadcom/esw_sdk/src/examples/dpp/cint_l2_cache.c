...
void l2_cache_key_fill(int port, bcm_mac_t mac_add, bcm_mac_t mac_add_mask, uint8 subtype, bcm_l2_cache_addr_t* cache_addr){
    ...
    /* mac mask, can be all-ones, or consequitive from msb to lsb */
...
l2_cache_init(int unit, int second_unit, int dest_mac1_high, int dest_mac1_low, int dest_mac2_high, int dest_mac2_low, int src_port, int dest_port, uint8 subtype, int trap_id, int vlan, int is_reserved_vlan){
    ...
    /* flags, doesn't include BCM_L2_CACHE_LOOKUP: that means packet will be forwarded to dest_port instead of being bridged */
    flags = 0;
 ...
 *      l2_cache_run_with_defaults(0, 1, 1, 5);
 *  
 * mac 01:80:c2:00:00:02: Flags doesn't include BCM_L2_CACHE_LOOKUP: 
 *                            that means packet will be forwarded to 
 *                            dest_port instead of being bridged.
 *                                             
