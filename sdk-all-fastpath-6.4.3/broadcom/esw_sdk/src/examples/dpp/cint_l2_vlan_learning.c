...
l2_vlan_learning_run(int unit, int port_1, int port_2){
    ...
    if (!is_jericho) {
        ...
        rv = open_ingress_mc_group_num(unit, l2_learning_info.mc_id, 1, 0, port_1, port_2);
        if (rv != BCM_E_NONE) {
            printf("Error, open_ingress_mc_group_num \n");
            printf("mc_id=%d\n", l2_learning_info.mc_id);
            return rv;
        }
    }
    ...
    if (is_jericho) {
        learn_msgs_mc.dest_port = l2_learning_info.olp_port;
    }
    else {
...
/* if olp then msgs will be sent to olp, same goes for cpu */
int open_ingress_mc_group_num(int unit, int mc_group_id, int olp, int cpu, int port_1, int port_2) {
    ...
    if (olp) {
        /* add to olp port to port list */
        l2_learning_info.ports[l2_learning_info.nof_ports++] = l2_learning_info.olp_port;
    }
    if (cpu) {
        /* add to cpu port to port list */
        l2_learning_info.ports[l2_learning_info.nof_ports++] = l2_learning_info.cpu_port;
    }

    /* add all ports to mc group */
    for (i=0; i<l2_learning_info.nof_ports; i++) {
        rv = bcm_multicast_ingress_add(unit, mc_group, l2_learning_info.ports[i], 0);
        if (rv != BCM_E_NONE) {
            printf("Error, in mc ingress add, mc_group=%d, dest_gport=0x%x \n", mc_group, l2_learning_info.ports[i]);
            return rv;
        }
    }
...
l2_per_vlan_learning_run(int unit, int port_1, int port_2){
    ...
    /* set learnign mode to egress distributed */
