/* $Id: cint_l3_multipath.c,v 1.10 Broadcom SDK $
...
int
create_l3_intf(int unit, int port, int vlan, int *intf)
{
  int rc;
  bcm_pbmp_t pbmp;
 ...
 */
int
create_l3_egress(int unit, uint32 flags, int port, int vlan, int ingress_intf, int *intf, int *encap_id) 
{  
...
l3_multipath_run(int unit, int unit_2nd, int in_port, int vlan, int nof_paths, int *out_port_arr, int *modid_arr)
{
  ...
  rc = create_l3_intf(unit ,in_port, vlan, &ingress_intf);
  if (rc!=BCM_E_NONE)
  {
    printf("create_l3_intf\n"); 
    print rc;
    return rc;
  }
  ...
  {
    BCM_GPORT_MODPORT_SET(gport, modid_arr[i], out_port_arr[i]);
    /* it is assumed modid == unit id */
    rc = create_l3_egress(modid_arr[i]  ,CINT_NO_FLAGS , gport, vlan, ingress_intf, &egress_intf[i], &encap_id[i]);
    if (unit_2nd >=0)
    {
      rc = create_l3_egress(1-modid_arr[i],BCM_L3_WITH_ID, gport, vlan, ingress_intf, &egress_intf[i], &encap_id[i]);
    }    
  }
