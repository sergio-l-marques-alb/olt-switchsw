...
int create_ring_protection_ports(int *units_ids, int nof_units, int out_sysport, int in_sysport)
{
    ...
    for (gport_num = 0; gport_num < MAX_RING_GPORTS; gport_num++) {
        ...
        printf ("Allocated gport[%ld] = %ld\n", gport_num, g_gport[gport_num].gport_id);
    }
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        for (j = 0 ; j < 4 ; j++) {
            rv = init_vlan(unit,vswitch_metro_mp_info.p_vlans[j]);
            if (rv != BCM_E_NONE) {
 ...
 */
int perform_fast_flush_example_1(int unit, int erp_mode)
{
    ...
    if (erp_mode) {
        ...
        block_state = 1;
        rv = erp_block(unit, erp_failover_id_idx, block_state);
        if (rv != BCM_E_NONE) {
    ...
    rv = perform_fast_flush(unit, g_gport[2].gport_id);
    if (rv != BCM_E_NONE) {
 ...
 */
int perform_fast_flush_example_2(int unit, int erp_mode)
{
    int rv, erp_failover_id_idx, block_state;
    ...
    rv = perform_fast_flush(unit, g_gport[0].gport_id);
    if (rv != BCM_E_NONE) {
        printf("perform_fast_flush_example_1 failure in perform_fast_flush for LIF - %ld: %s\n",
