 ...
 * 
 ...
 cint
 print oam_run_with_defaults_mpls_tp(0,13,14,15);
 ...
 * @return int 
 */
int oam_example_over_tunnel(int unit,  bcm_oam_endpoint_type_t type) {
  bcm_error_t rv;
...
int oam_run_with_defaults_mpls_tp (int unit, int port1, int port2, int port3) {
  bcm_error_t rv;
  ...
  rv = oam_example_over_tunnel(unit, bcmOAMEndpointTypeBHHMPLS);
  if (rv != BCM_E_NONE) {
      printf("(%s) \n",bcm_errmsg(rv));
      return rv;
  }
...
int oam_run_with_defaults_pwe (int unit, int port1, int port2, int port3) {
  ...
  rv = oam_example_over_tunnel(unit, bcmOAMEndpointTypeBHHPwe);
  if (rv != BCM_E_NONE) {
      printf("(%s) \n",bcm_errmsg(rv));
      return rv;
  }
