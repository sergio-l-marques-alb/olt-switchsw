 ...
 *         To enable protection set is_protection variable
 */
...
int is_protection =0; /*enable protrection mode*/

/********** 
  functions
 ********** */

int is_arad_plus(int unit, int *set_for_yes)
{
    bcm_info_t info;
    ...
    *set_for_yes = (((info.device == 0x8660) || (info.device == 0x8675)) ? 1 : 0);
 ...
 *  - MPLS label 300
 */
int example_rpf_protection(int *units_ids, int nof_units, int in_sysport, int out_sysport, bcm_l3_ingress_urpf_mode_t urpf_mode){
	is_protection=1;
	int rc=0;
...
int example_rpf(int *units_ids, int nof_units, int in_sysport, int out_sysport, bcm_l3_ingress_urpf_mode_t urpf_mode){
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = create_l3_intf(unit,flags,open_vlan,in_sysport,vlan_l,vrf,mac_address_l, &ing_intf_l); 
        if (rv != BCM_E_NONE) {
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = create_l3_intf(unit,flags,open_vlan,out_sysport,vlan_r,vrf,mac_address_r, &ing_intf_r); 
        if (rv != BCM_E_NONE) {
    ...
    for (i = 0 ; i < nof_units ; i++){
		...
		if (is_protection==0) {
			...
			rv = create_l3_egress(unit,flags,out_sysport,vlan_r,0,mac_address_next_hop_r, &fec[0], &encap_id[0]); 
		}
	...
	if (is_protection == 0) {
	   ...
	   /* IPuc add routes/host to Routing table: from R to L*/
		...
		result = field_ip_rpf_entry_add(unit, grp, &entry);
		if (BCM_E_NONE != result) {
...
int example_rpf_per_rif_urpf_mode(int unit, int in_sysport, int out_sysport) {
    ...
    if (rv != BCM_E_NONE) {
        printf("Error checking if the device is Arad+.\n");
        print rv;
        return rv;
    }
