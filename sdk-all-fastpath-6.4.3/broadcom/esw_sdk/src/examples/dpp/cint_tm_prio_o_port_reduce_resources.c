    ...
    for (modid = 0; modid < g_info.nof_devices; modid++) {
        for (idx = 0; idx < g_info.num_ports + g_info.num_internal_ports; idx++) {
            ...
            if (!g_info.central_cpu) {
                ...
                BCM_COSQ_GPORT_VOQ_CONNECTOR_SET(g_info.gport_ucast_voq_connector_group[modid][idx], voq_connector_id);
            }
           ...
           /*  printf("modid=%d, idx=%d, port=%d, voq_connector_id=0x%x, gport_ucast_voq_connector_group[modid][idx]=0x%x\n", modid, idx, port, voq_connector_id, g_info.gport_ucast_voq_connector_group[modid][idx]); */
            rv = bcm_cosq_gport_add(unit, g_info.dest_local_gport[idx], g_info.num_cos, flags, &g_info.gport_ucast_voq_connector_group[modid][idx]);
            if (BCM_FAILURE(rv)) {
