...
#define VGMOD_ID0       0x0143
#define VGMOD_ID1       0x0109
#define VGMOD_MODEL     0x10

#define VXMOD_ID0       0x0143
#define VXMOD_ID1       0x0109
#define VXMOD_MODEL     0x11   
...
int viper_core_identify(const phymod_core_access_t* core, uint32_t core_id, uint32_t* is_identified)
{
    ...
    *is_identified = 0;
    ...
    if ((COMBO_ID1r_GET(id1) == VGMOD_ID0) && (COMBO_ID2r_GET(id2) == VGMOD_ID1)){
        /* PHY IDs match - now check PCS modeli */
        ioerr += READ_SERDESID1r(pm_acc, &serdes_id);
...
/*
 * set lane swapping for core, just used for VXMOD  
   //10G only
 * The tx swap is composed of PCS swap and after that the PMD swap.
 * The rx swap is composed of PCS swap and after that the PMD swap.
 *
                                      ...
                                      phymod_sequencer_operation_t operation)
{
    ...
    switch (operation) {
        case phymodSeqOpStop:
             viper_pll_disable(core);
             break;

        case phymodSeqOpStart:
             viper_pll_enable(core); 
             break;
