...
{
	out_be32(reg, val);//{ static int cnt= 100;if(cnt){ cnt--;printk("%u|mpc8xxx_spi_write_reg|reg:%8.8x|val:%8.8x\n",jiffies,reg,val); }};
}

static inline u32 mpc8xxx_spi_read_reg(__be32 __iomem *reg)
{
//	u32 val= in_be32(reg);
//	{ static int cnt= 100;if(cnt){ cnt--;printk("%u|mpc8xxx_spi_read_reg|reg:%8.8x|val:%8.8x\n",jiffies,reg,val); }};
//	return val;
	return in_be32(reg);
}
