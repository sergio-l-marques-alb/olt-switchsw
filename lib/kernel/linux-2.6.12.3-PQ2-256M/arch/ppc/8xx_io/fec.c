...
static void set_multicast_list(struct net_device *dev)
{
	...
	inline int TESTA_MEM(volatile unsigned int *addr, int n_uint) {
		int i;
		int rv= 1;
		for(i=0; (i<n_uint) && (rv);++i)
		{
			addr[i]= 0xAAAAAAAA;
			if((rv= (0xAAAAAAAA == addr[i])))
			{
				addr[i]= 0x55555555;
				rv= (0x55555555 == addr[i]);
			}
			if(!rv) printk("TESTA_MEM:FALHA:addr: %p\n", &addr[i]);
		}
		return rv;
	}
...
static int __init fec_enet_init(void)
{
	...
	*/
	...
	cbd_base= dma_alloc_coherent(NULL, PAGE_SIZE, &mem_addr, GFP_KERNEL | GFP_DMA);

	printk("************ cbd_base:%p|b: %p|TESTA_MEM:%d\n", cbd_base, mem_addr, TESTA_MEM(cbd_base, PAGE_SIZE/sizeof(unsigned int)));
	...
	*/
	...
	for (i=0; i<FEC_ENET_RX_PAGES; i++) {
		...
		/* Allocate a page.
		*/
		ba= dma_alloc_coherent(NULL, PAGE_SIZE, &mem_addr, GFP_KERNEL | GFP_DMA);
		printk("************ ba:%p|b: %p|TESTA_MEM:%d\n", ba, mem_addr,TESTA_MEM(ba, PAGE_SIZE/sizeof(unsigned int)));
