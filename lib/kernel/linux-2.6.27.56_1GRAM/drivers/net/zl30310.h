...
/*
 * $Id: zl30310.h 4936 2011-01-07 16:30:45Z JK $
 *
...
/* The maximum number of packets to be handled in one call of zl30310_poll */
...
#define DEVICE_NAME "%s: Gianfar Ethernet Controller (Microsemi ZL3031x) Version 1.3-skbr, "
#define DRV_NAME "zl30310-enet"
extern const char zl30310_driver_name[];
extern const char zl30310_driver_version[];
...
#define IEVENT_MMRD		0x00000400
#define IEVENT_MMWR		0x00000200
...
(IEVENT_RXC | IEVENT_BSY | IEVENT_EBERR | IEVENT_MSRO | \
 ...
 | IEVENT_MAG | IEVENT_MMRD | IEVENT_MMWR)
...
#define IMASK_MMRD		0x00000400
#define IMASK_MMWR		0x00000200
...
#define GFAR_EXTRA_STATS_LEN (sizeof(struct zl30310_extra_stats)/sizeof(u64))
...
struct zl30310 {
	...
	u8	res18[12];
	u8	zl30310_mii_regs[24];	/* See gianfar_phy.h */
	u8	res19[4];
...
struct zl30310_ptp_time {
	u32	high;
	u32	low;
};

struct zl30310_ptp_data_t {
	int	key;
	struct	zl30310_ptp_time	item;
};

struct zl30310_ptp_circular_t {
	int	front;
	int	end;
	int	size;
	struct	zl30310_ptp_data_t	*data_buf;
};
...
extern void zl30310_free_recycle_queue(struct zl30310_skb_handler *sh, int lock_flag);
...
struct zl30310_private {
	...
	struct zl30310_skb_handler skb_handler;
#endif
	...
	struct zl30310_extra_stats extra_stats;
	struct zl30310_ptp_circular_t rx_time_sync;
	struct zl30310_ptp_circular_t rx_time_del_req;
	struct zl30310_1588 __iomem *ptimer;

	/* LFC stuff */
	u8 free_bd_thres;
};

static inline u32 zl30310_read(volatile unsigned __iomem *addr)
{
	u32 val;
	val = in_be32(addr);
	return val;
...
static inline void zl30310_write(volatile unsigned __iomem *addr, u32 val)
{
	out_be32(addr, val);
}

extern irqreturn_t zl30310_receive(int irq, void *dev_id);
extern int startup_zl30310(struct net_device *dev);
extern void stop_zl30310(struct net_device *dev);
extern void zl30310_halt(struct net_device *dev);

extern void zl30310_1588_start(struct net_device *dev);
extern void zl30310_1588_stop(struct net_device *dev);
extern int zl30310_ptp_init(struct zl30310_private *priv,
			struct gianfar_platform_data *einfo);
extern void zl30310_ptp_cleanup(struct zl30310_private *priv);
extern int zl30310_ptp_do_txstamp(struct sk_buff *skb);
extern void zl30310_ptp_store_rxstamp(struct net_device *dev, struct sk_buff *skb);
extern int zl30310_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd);

extern void zl30310_phy_test(struct mii_bus *bus, struct phy_device *phydev,
		int enable, u32 regnum, u32 read);
void zl30310_init_sysfs(struct net_device *dev);
