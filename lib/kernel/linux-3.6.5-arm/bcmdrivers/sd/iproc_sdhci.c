...
static int __init sdhci_platform_probe(struct platform_device *pdev)
{
    ...
    iomem_base = IOMEM(HW_IO_PHYS_TO_VIRT(IPROC_SDIO_IDM_IO_CONTROL_DIRECT));
    val = readl(iomem_base);
    val |= 1 << IPROC_SDIO_IDM_IO_CONTROL_DIRECT__CMD_COMFLICT_DISABLE;
    writel(val,iomem_base);
#endif
