...
int fsl_sleep_init(struct fsl_sleep_platform_data *sleep, struct device_node *node)
{
...
#ifdef __SUPPORT_ZL30314__
static const char *zl30310_tx_intr  = "eth1_tx";
static const char *zl30310_rx_intr  = "eth1_rx";
static const char *zl30310_err_intr = "eth1_error";
#endif
...
#ifdef __SUPPORT_ZL30314__
// This block is new but since CONFIG_GFAR_PTP is not defined
/*
...
static int __init zl30310_of_init(void)
{
	struct device_node *np;
	unsigned int i;
	struct platform_device *gfar_dev;
	...
	for (np = NULL, i = 0;
	     (np = of_find_compatible_node(np, "network", "zarlink")) != NULL;
	     i++) {
		struct resource r[5];
		struct device_node *phy, *mdio;
		struct gianfar_platform_data gfar_data;
        ...
        //JVM
		/* If we aren't the FEC we have multiple interrupts */
		if (model && (strcasecmp(model, "eTSEC")==0)) {             //Diferente!!!!!!!!
			r[1].name = zl30310_tx_intr;
		        of_irq_to_resource(np, 0, &r[1]);                   //Diferente!!!!!!!!

			r[2].name = zl30310_rx_intr;
			of_irq_to_resource(np, 1, &r[2]);

			r[3].name = zl30310_err_intr;
			of_irq_to_resource(np, 2, &r[3]);

			n_res += 2;
		}

		gfar_dev = platform_device_register_simple("zl30310-eth", i, &r[0], n_res);
        ...
        printk("zl30310_of_init: gfar_data.mac_addr=%s\n", gfar_data.mac_addr);

        //JVM: Removed FSL_GIANFAR_DEV_HAS_VLAN
		if (model && !strcasecmp(model, "TSEC"))
			gfar_data.device_flags =
			    FSL_GIANFAR_DEV_HAS_GIGABIT |
			    FSL_GIANFAR_DEV_HAS_COALESCE |
