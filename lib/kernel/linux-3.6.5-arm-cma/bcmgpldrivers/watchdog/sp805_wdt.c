...
static unsigned int wdt_get_clear_bootstatus(void)
{
	...
	void __iomem *wdt_bootsatus = IOMEM(HW_IO_PHYS_TO_VIRT(IPROC_SP805_WDT_BOOTSTATUS));

	reg = readl_relaxed(wdt_bootsatus);
	bootstatus = reg & (1 << IPROC_SP805_WDT_BOOTSTATUS_BIT);
	if(bootstatus) {
...
sp805_wdt_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	dev_info(&adev->dev, "timtout=%d sec, nowayout=%d\n", 
