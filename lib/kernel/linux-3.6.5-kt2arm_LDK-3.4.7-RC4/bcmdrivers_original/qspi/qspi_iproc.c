...
volatile int gvDBG= 1;
#define DBG if(gvDBG== 1) printk
...
{
    priv->idm_qspi->io_ctrl_direct |= mask << 2;
}
    ...
    int width, int addrlen, int hp)
{
    ...
    if (!error) {
        ...
        priv->bspi_hw->flex_mode_enable = flex_mode ?
            QSPI_BSPI_FLEX_MODE_ENABLE_MASK
            : 0;
    ...
    const struct bcmspi_parms *xp)
{
    ...
    if (priv->pos.msg == NULL || xp->bits_per_word > 8) {
        ...
        priv->mspi_hw->spcr0_msb = 0x80 |    /* Master */
            (bits << 2) |
            (xp->mode & 3);
    } else {
        ...
        if (p.trans->bits_per_word <= 8) {
            ...
            if (buf) {
                ...
                if (p.mspi_16bit) {
                    ...
                    buf[p.byte] =
                        priv->mspi_hw->rxram[(slot << 1) + 0] & 0xff;
                    DBG("RD %02x\n", buf ? buf[p.byte] : 0xff);
                    buf[p.byte + 1] =
                        priv->mspi_hw->rxram[(slot << 1) + 1] & 0xff;
            ...
            if (buf) {
                buf[p.byte] =
                    ((priv->mspi_hw->rxram[(slot << 1) + 1] & 0xff) << 0) |
                    ((priv->mspi_hw->rxram[(slot << 1) + 0] & 0xff) << 8);
        ...
        if (p.trans->bits_per_word <= 8) {
            ...
            if (priv->pos.mspi_16bit) {
                ...
                priv->mspi_hw->txram[(slot << 1) + 1] = buf ?
                    (buf[p.byte] & 0xff) : 0xff;
                DBG("WR %02x\n", buf ? buf[p.byte] : 0xff);
                priv->mspi_hw->cdram[slot] = 0xce;
            } else {
                priv->mspi_hw->cdram[slot] = 0x8e;
            }
    ...
    struct spi_message *msg)
{
    ...
    if (priv->raf_next_len != 0) {
        ...
        if (bcmspi_is_4_byte_mode(priv)) {
            priv->bspi_hw->flash_upper_addr_byte = addr & 0xFF000000;
        ...
        if ((addr & 0xFF000000) != priv->bspi_hw->flash_upper_addr_byte) {
            bcmspi_flush_prefetch_buffers(priv);
            /* Update upper address byte */
            priv->bspi_hw->flash_upper_addr_byte = addr & 0xFF000000;
        }
    ...
    priv->bspi_hw_raf->start_address = addr & 0x00FFFFFF;
    priv->bspi_hw_raf->num_words = (len + 3) >> 2;
    ...
    if (is_bspi_chip_select(priv, msg->spi->chip_select)) {
        ...
        if (trans && trans->len && trans->tx_buf) {
            ...
            default:
                ...
                priv->bspi_hw->flash_upper_addr_byte = 0x80;
                break;
            }
        }
    }
