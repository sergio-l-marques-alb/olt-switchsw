...
#define DBG if(gvDBG== 0) printk //TIAGO
...
{
    DBG("%s(%d) bspi_cs 0x%.4x, cs 0x%.4x\n", __FUNCTION__, __LINE__, priv->bspi_chip_select, cs);
...
static int bcmspi_transfer_one(struct spi_master *master, struct spi_message *msg)
{
    ...
    DBG("%s(%d)\n", __FUNCTION__, __LINE__);
    DBG("spi->chip_select 0x%.2x\n", msg->spi->chip_select);
 #if 0 //Tiago - não processar conteudo do buffer como sendo comando de flash
    if (is_bspi_chip_select(priv, msg->spi->chip_select)) {
        ...
        if (trans && trans->len && trans->tx_buf) {
            ...
            DBG("%s(%d)\n command: 0x%.2x", __FUNCTION__, __LINE__, command);
            switch (command) {
    ...
    //writel(1, (volatile uint32_t *) ChipcommonA_GPIOOut_VA);
    spin_unlock_irqrestore(&priv->lock, flags);
