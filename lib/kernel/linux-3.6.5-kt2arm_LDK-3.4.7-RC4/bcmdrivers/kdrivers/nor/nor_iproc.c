...
static int __init s29gl_mtd_init(void)
{
    ...
    /* Check boot device */
    val = __REG32(IPROC_DMU_BASE_VA + IPROC_DMU_STRAPS_OFFSET) & 
            (7 << IPROC_STRAP_BOOT_DEV_SHIFT);
    if (val == IPROC_STRAP_BOOT_DEV_NAND) {
        /* If booting from NAND, PNOR cannot be used */
        return -ENODEV;
    } else if (val != IPROC_STRAP_BOOT_DEV_PNOR) {
        ...
        *nor_enable |= 
        ...
        val = __REG32(HW_IO_PHYS_TO_VIRT(ICFG_PNOR_STRAPS)) & 
                (1 << ICFG_PNOR_STRAPS__PNOR_SRAM_MW_R);
        if (val) {
            /* 16-bit */
            __REG32(HW_IO_PHYS_TO_VIRT(PNOR_set_opmode)) |= 
                (1 << PNOR_set_opmode__set_mw_R);
        } else {
            /* 8-bit */
            __REG32(HW_IO_PHYS_TO_VIRT(PNOR_set_opmode)) &= 
                ~(1 << PNOR_set_opmode__set_mw_R);
        }
        __REG32(HW_IO_PHYS_TO_VIRT(PNOR_direct_cmd)) |= 
            (2 << PNOR_direct_cmd__cmd_type_R);
#endif 
    }
    ...
    if (!s29gl_map.virt) {
        ...
        if (nor_enable) {
            /* revert to NAND mode */
            *nor_enable &= ~(1UL << PNOR_NAND_SEL_REG_PNOR_SEL);
        }
        return -EIO;
    }
