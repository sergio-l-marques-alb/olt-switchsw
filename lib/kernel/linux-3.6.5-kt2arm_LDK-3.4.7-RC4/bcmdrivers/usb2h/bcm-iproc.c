...
static int __init usbh_init(void)
{
...
#if (defined(CONFIG_MACH_HX4) || defined(CONFIG_MACH_KT2))
	int clk_enable, k;
	unsigned int iClk, USBClk, usbdgpiopwr, pllStatus;
	...
	//__raw_writel(0x2, ChipcommonA_GPIOOutEn_VA); / TIAGO
    __raw_writel(0x0,ChipcommonA_GPIOOutEn_VA); // TIAGO
	...
	{
		USBClk |= 0x01000000;
		__raw_writel(USBClk, IPROC_USB_PHY_CTRL_VA);
		pllStatus = __raw_readl(IPROC_WRAP_MISC_STATUS_VA);
		for (k = 0; k < 100000; k++)
		{
			if ((pllStatus & 2) == 2)
			{
