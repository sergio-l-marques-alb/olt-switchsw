...
static void xgs_serdes_reset(struct phy_device *phy_dev)
{
    ...
    static struct phy_device *phy_dev_bcm_serdes;
...
static int initFlag=0, my_value=0;


struct kobj_attribute my_attr = __ATTR(etx_value, 0660, sysfs_show, sysfs_store);

static ssize_t sysfs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf) {

    return sprintf(buf, "sysfs_show -> %d", my_value);
}
 
static ssize_t sysfs_store(struct kobject *kobj, struct kobj_attribute *attr,const char *buf, size_t count) {
        sscanf(buf,"%d",&my_value);
        printk("sysfs_store: my_value -> %d\n", my_value);
        xgs_serdes_config_init(phy_dev_bcm_serdes);
        printk("xgs_serdes_config_init!\n");
        return count;
}
...
static int sysfs_myInit(void) {
    ...
    if (initFlag) {
        printk("Module already initialized!!!");
        return 0;
    }
    
    //This Function will be called from Init function            
    /*Creating a directory in /sys/kernel/ */                    
    kobj_ref = kobject_create_and_add("bcmserdes_sysfs",kernel_kobj); 
...
////This Function will be called from Init function
///*Creating a directory in /sys/kernel/ */
//kobj_ref = kobject_create_and_add("etx_sysfs",devices_kobj);
// 
///*Creating sysfs file for etx_value*/
