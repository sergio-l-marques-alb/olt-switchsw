...
static int tg3_rx(struct tg3_napi *tnapi, int budget)
{
	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		if (desc->err_vlan & RXD_ERR_MASK) {
            printk("%s: ERROR MASK %08X\n\r", __FUNCTION__, desc->err_vlan);
		drop_it:
			tg3_recycle_rx(tnapi, tpr, opaque_key,
				       desc_idx, *post_ptr);
		drop_it_no_recycle:
			/* Other statistics kept track of by card. */
            printk("%s: tp->rx_dropped++ %d\n\r", __FUNCTION__, tp->rx_dropped);
			tp->rx_dropped++;
			goto next_pkt;
		}
		...
		if (len > TG3_RX_COPY_THRESH(tp)) {
			...
			skb_size = tg3_alloc_rx_data(tp, tpr, opaque_key,
						    *post_ptr, &frag_size);
			if (skb_size < 0) {
                printk("%s: ERROR skb_size < 0\n\r", __FUNCTION__, skb_size);
				goto drop_it;
            }
