/*
 * $Id: drv.h 1.600.2.5 Broadcom SDK $
...
#define SOC_IS_EASYRIDER(unit)  ((!SOC_INFO(unit).spi_device) && \
                (SOC_INFO(unit).chip & SOC_INFO_CHIP_EASYRIDER))
#else
...
#define SOC_IS_ARAD_A0(unit)    (SOC_INFO(unit).driver_type == SOC_CHIP_BCM88650_A0)
#define SOC_IS_ARAD_B0(unit)    ((SOC_INFO(unit).driver_type == SOC_CHIP_BCM88650_B0) || (SOC_INFO(unit).driver_type == SOC_CHIP_BCM88650_B1))
#define SOC_IS_ARAD_B1(unit)     (SOC_INFO(unit).driver_type == SOC_CHIP_BCM88650_B1) 
#define SOC_IS_ARADPLUS_A0(unit) (SOC_INFO(unit).driver_type == SOC_CHIP_BCM88660_A0)
#define SOC_IS_ARAD_B0_AND_ABOVE(unit) ((SOC_INFO(unit).chip_type == SOC_INFO_CHIP_TYPE_ARAD) && (!SOC_IS_ARAD_A0(unit)))
...
#define SOC_IS_FE1600_B0(unit) (SOC_INFO(unit).driver_type == SOC_CHIP_BCM88750_B0)
#define SOC_IS_FE1600_B0_AND_ABOVE(unit) (!SOC_IS_FE1600_A0(unit))
#define SOC_IS_DFE_TYPE(dev_type) ((dev_type) == BCM88750_DEVICE_ID) 
...
#define SOC_IS_DPP(unit) (SOC_IS_PETRAB(unit) || SOC_IS_ARAD(unit) || SOC_IS_ACP(unit))
...
typedef enum {
    ...
    SOC_INFO_CHIP_TYPE_EASYRIDER,
    SOC_INFO_CHIP_TYPE_FIREBOLT,
    SOC_INFO_CHIP_TYPE_EASYRIDER10, 
...
#define SOC_INFO_CHIP_EASYRIDER10    0x00000800    /* 5661x */
...
#define   SOC_F_RESET              (SOC_F_ATTACHED|SOC_F_INITED|SOC_F_POLLED|SOC_F_KNET_MODE)
    ...
    int         tcam_protect_write;     /* Duplicate existing entry before
                                         * overwrite */
    int         skip_cache_use;         /* Mem test mode to avoid cache use */
    int         force_read_through;     /* Read from h/w even if cached */
    int         parity_correction;      /* SER enabled */
    int         return_reg_error;       /* Return error on SER error */
    int         l3_defip_index_remap;   /* l3_defip index remap offset */
    int         l3_defip_urpf;          /* urpf_route_enabled */
...
soc_ser_mem_clear(int unit, _soc_ser_parity_info_t *_soc_tr2_ser_parity_info,
                  soc_mem_t mem);
extern int
soc_cmicm_ser_mem_clear(int unit, _soc_ser_parity_info_t *_soc_tr2_ser_parity_info,
                        soc_mem_t mem);
