...
format_spec_long_integer(char *buf, uint32 *val, int nval)
{
    ...
    while (--i >= 0) {                  /* Print rest of words, if any */
        ...
        sal_sprintf(buf + strlen(buf), "0x%08X", val[i]);
    }
}
...
parse_memory_name(int unit, soc_mem_t *result, char *str, int *copyno, unsigned *array_index)
{
    ...
    blk = COPYNO_ALL;
    numblk = -1;
    if (( ((aip = strchr(tmp, '[')) != NULL) && ((cp = strchr(aip+1, ']')) != NULL) && (cp-aip > 1) )  ||
        ( ((aip = strchr(tmp, '(')) != NULL) && ((cp = strchr(aip+1, ')')) != NULL) && (cp-aip > 1) ) )   { /* handle memory array */
