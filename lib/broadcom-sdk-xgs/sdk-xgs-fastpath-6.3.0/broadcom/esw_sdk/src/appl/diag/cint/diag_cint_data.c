/*
 * $Id: diag_cint_data.c 1.40 Broadcom SDK $
 *
...
#if (defined(BCM_88650_A0) || defined(BCM_88750_A0)) && defined(INCLUDE_INTR)
...
#define DIAG_CINT_DATA_VERB(stuff)         SOC_DEBUG(SOC_DBG_VERBOSE, stuff)
...
void mc_test_set_assert_type(int atype);
void mc_test_set_debug_level(int level);
...
static int diag_test_mc(int unit, char *test_name)
{
    ...
    printk("ERROR: unknown test name, use one of: mc_open, mc_basic, mc_mid_size,  print_all, print_errors_info, print_errors, assert, assert_0, assert_none, get_mc_asserts, get_nof_mcids, get_nof_egress_bitmap_groups\n");
    return SOC_E_PARAM;
}
...
static int diag_test_mc2(int unit, char *test_name, int param)
{
    ...
    printk("ERROR: unknown test name, use one of: mc_replications get_irdb\n");
    return SOC_E_PARAM;
}
#endif /* BCM_88650 */
