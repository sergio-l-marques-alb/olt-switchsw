    ...
    bcm_port_t port, _field_raven_sel_codes_t *hw_sel_codes)
{
    ...
    if (fg_p == NULL)
    {
        ...
        if (group_id != -1)
        {
            ...
            {
                if ((rv = _field_group_id_generate(unit, &group_id)) == BCM_E_NONE) {
                unsigned vslice;
            
                for (vslice = 0; ; ++vslice) {
                    if (vslice >= COUNTOF(stage_fc->vmap[0])) {
                        rv = BCM_E_INTERNAL;
                        
                        break;
                    }
                    
                    if (stage_fc->vmap[0][vslice].vmap_key == slice_index) {
                        priority = stage_fc->vmap[0][vslice].priority;
    ...
    for (part_index = 0; part_index <= slice_mode; ++part_index)
    {
            ...
            ++qualifier_id)
        {
            ...
            if (config_index < stage_qualifier_p->conf_sz)
            {
                ...
                part_qualifier_p = fg_p->qual_arr + part_index;
    ...
    uint32 mode, uint32 counter_index, int part_index, bcm_field_stat_t sid)
{
    ...
    else
    {
        ...
        rv = _field_stat_array_init(unit, stat_p, stat_count, stat_arr);
    ...
    bcm_policer_t pid)
{
    ...
    else
    {
        ...
        FP_VVERB(("Creating policer id %d.\n", policer_id));
        ...
        switch (mode) {
            ...
            case BCM_FIELD_METER_MODE_FLOW:
                ...
                meter_table_index
                    = stage_fc->slices[policer_p->pool_index].start_tcam_idx
                        + 2 * policer_p->hw_index + meter_offset;
...
int _field_raven_actions_recover(int unit,
    fp_policy_table_entry_t *policy_entry_p, _field_entry_t *entry_p,
        int part_index, bcm_field_stat_t sid, bcm_policer_t pid)
{
        ...
        if (field_value != 0)
        {
            ...
            case MIRRORf:
                ...
                if (field_value & 0x01)
                {
                    ...
                    BCM_IF_ERROR_RETURN(_field_action_alloc(unit,
                        bcmFieldActionMirrorIngress, param0, param1, 0, 0, &fa_p));
                ...
                if (field_value & 0x02)
                {
                    ...
                    BCM_IF_ERROR_RETURN(_field_action_alloc(unit,
                        bcmFieldActionMirrorEgress, param0, param1, 0, 0, &fa_p));
            ...
            case METER_PAIR_MODEf:
                BCM_IF_ERROR_RETURN(_field_raven_meter_recover(unit,
                    entry_p, policy_entry_p, part_index, pid));
            ...
            if (action != bcmFieldActionCount)
            {
                ...
                BCM_IF_ERROR_RETURN(_field_action_alloc(unit, action, param0,
                    param1, 0, 0, &fa_p));
    ...
    _field_stage_t *stage_fc)
{
    ...
    char *fp_slice_entry_port_sel_buffer = NULL;
    ...
    if (fc->l2warm) {
        sal_memcpy(&version, fc->scache_ptr, sizeof(uint16));
        ...
        } else if (version != BCM_WB_DEFAULT_VERSION) {
            
            return BCM_E_INTERNAL; /* Invalid version number */
        }
    ...
    /* Read FP_SLICE_ENTRY_PORT_SEL table into memory */

    _FP_RAVEN_IF_FAILURE_CLEANUP(_field_table_read(unit,
        FP_SLICE_ENTRY_PORT_SELm, &fp_slice_entry_port_sel_buffer,
        "FP_SLICE_ENTRY_PORT_SEL buffer"));
    ...
    _FP_RAVEN_IF_FAILURE_CLEANUP(_field_slice_expanded_status_get(unit,
        stage_fc, slice_is_expanded_flags));
    ...
    for (slice_index = 0; slice_index < stage_fc->tcam_slices; ++slice_index)
    {
        ...
        for (fg_p = fc->groups; fg_p != NULL; fg_p = fg_p->next) {
            for (extra_qual = 0; extra_qual < 2; extra_qual++) {
                ...
                for (i = 0; i < part_count; i++) {
                    for (k = 0; k < fg_p->qual_arr[i].size; k++) {
                        if (fg_p->qual_arr[i].qid_arr[k] == qual_idx) {
                            break; /* Qualifier already added */
                        }
                    }
                    if (k == fg_p->qual_arr[i].size) {
                        size = fg_p->qual_arr[i].size + 1;
                        qid_arr = NULL;
                        ...
                        if (size) {
                            sal_memcpy(qid_arr, fg_p->qual_arr[i].qid_arr,
                                       fg_p->qual_arr[i].size * 
                                       sizeof (uint16));
                            sal_memcpy(q_offset_arr, 
                                       fg_p->qual_arr[i].offset_arr, 
                                       fg_p->qual_arr[i].size * 
                                       sizeof (_bcm_field_qual_offset_t));
                            if (fg_p->qual_arr[i].size) {
                                sal_free(fg_p->qual_arr[i].qid_arr);
                                sal_free(fg_p->qual_arr[i].offset_arr);
                            }
                        }
                        fg_p->qual_arr[i].qid_arr = qid_arr;
                        fg_p->qual_arr[i].offset_arr = q_offset_arr;
                        fg_p->qual_arr[i].qid_arr[size - 1] = qual_idx;
                        fg_p->qual_arr[i].offset_arr[size - 1].field = 
                            f_qual_arr->conf_arr[0].offset.field;
                        fg_p->qual_arr[i].offset_arr[size - 1].offset = 
                            f_qual_arr->conf_arr[0].offset.offset;
                        fg_p->qual_arr[i].offset_arr[size - 1].width = 
                            f_qual_arr->conf_arr[0].offset.width;
                        fg_p->qual_arr[i].offset_arr[size - 1].offset1 = 
                            f_qual_arr->conf_arr[0].offset.offset1;
                        fg_p->qual_arr[i].offset_arr[size - 1].width1 = 
                            f_qual_arr->conf_arr[0].offset.width1;
                        fg_p->qual_arr[i].offset_arr[size - 1].offset2 = 
                            f_qual_arr->conf_arr[0].offset.offset2;
                        fg_p->qual_arr[i].offset_arr[size - 1].width2 = 
                            f_qual_arr->conf_arr[0].offset.width2;
                        fg_p->qual_arr[i].size = size;
        ...
        for (entry_index = 0; entry_index < fs_p->entry_count; ++entry_index)
        {
            ...
            FP_VVERB(("Entry PBMP: %s\n", pbmp_string));

            /* Find a matching group */

            for (fg_p = fc->groups; fg_p != NULL; fg_p = fg_p->next)
            {
                FP_VVERB(("        Checking group %d...\n", fg_p->gid));
            ...
            for (part_index = 0; part_index < part_count; ++part_index)
            {
                ...
                _FP_RAVEN_IF_FAILURE_CLEANUP(
                    _bcm_field_tcam_part_to_entry_flags(part_index,
                        fg_p->flags, &entry_p->flags));

                /* Get index of entry in tcam */

                _FP_RAVEN_IF_FAILURE_CLEANUP(
                    _bcm_field_entry_part_tcam_idx_get(unit, entry_p,
                        primary_tcam_index, part_index, &part_tcam_index));

                /* Get slice and index within the slice */

                _FP_RAVEN_IF_FAILURE_CLEANUP(
                    _bcm_field_tcam_idx_to_slice_offset(unit, stage_fc,
                ...
                _FP_RAVEN_IF_FAILURE_CLEANUP(_field_raven_actions_recover(unit,
                    policy_entry_p, entry_p, part_index,
                    stat_id, policer_id));
            }

            vslice_index = _field_physical_to_virtual(unit, slice_index, stage_fc);
    ...
    for (slice_index = 0; slice_index < stage_fc->tcam_slices; ++slice_index)
    {
        ...
        if (entry_index == fs_p->entry_count)
        {
            ...
            FP_VVERB(("    No valid entries in slice.\n"));
        ...
        if (fc->l2warm)
        {
            ...
            if (BCM_FAILURE(rv))
            {
                FP_ERR(("Failed to retrieve group IDs in slice %d.",
                    slice_index));
        ...
        for (i = 0; i < stage_fc->tcam_slices; i++) {
            if ((stage_fc->vmap[0][vslice_index].virtual_group == 
                stage_fc->vmap[0][i].virtual_group) && (i != vslice_index)) {
                if (i > max) {
                    max = i;
                }
            } 
        ...
        while (fg_p != NULL) {
            ...
            fs_p = &fg_p->slices[0];
            if (fs_p->slice_number == master_slice) {
                break;
            }
            fg_p = fg_p->next;
        }
        ...
        for (entry_index = 0; entry_index < fs_p->entry_count; ++entry_index)
        {
            ...
            _FP_RAVEN_IF_FAILURE_CLEANUP(_field_group_entry_add(unit,
                fg_p, entries_p));
        }
...
cleanup:
    ...
    if (fp_slice_entry_port_sel_buffer != NULL) 
    {
        soc_cm_sfree(unit, fp_slice_entry_port_sel_buffer);
    }
