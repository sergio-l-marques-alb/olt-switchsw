/*
 * $Id: switch.c 1.3.56.1 Broadcom SDK $
...
typedef struct _bcm_flex_hash_match_arr_s {
	...
	int             qual_size;	/* Data qualifier size */
...
typedef struct _bcm_flex_hash_entry_s {
	...
	int             hash_offset[FLEX_HASH_MAX_OFFSET];	/* Flex hash
								 * offset count */
 ...
 */
STATIC int
_bcm_flex_hash_table_write(int unit, flex_hash_entry_t * hash_entry)
{
	...
	for (fld = 0; fld < FLEX_HASH_MAX_FIELDS; fld++) {
		data[fld / 2] |= hash_entry->match_arr[fld].qual_match <<
		    (((fld & 0x1) * hash_entry->match_arr[fld].qual_size));
		mask[fld / 2] |= hash_entry->match_arr[fld].qual_mask <<
		    (((fld & 0x1) * hash_entry->match_arr[fld].qual_size));
	}
	...
	for (fld = 0; fld < FLEX_HASH_MAX_OFFSET; fld++) {
		soc_mem_field32_set(unit, FLEX_HASH_TCAM, &flex_tcam_entry,
				    _bcm_td2_flex_hash_data[fld], data[fld]);
		soc_mem_field32_set(unit, FLEX_HASH_TCAM, &flex_tcam_entry,
				    _bcm_td2_flex_hash_mask[fld], mask[fld]);
		soc_mem_field32_set(unit, FLEX_HASH_OFFSET, &flex_offset_entry,
				    _bcm_td2_flex_hash_mask[fld], (mask[fld] & 0xFF));
			 ...
			 int qual_count, int *qual_array)
{
	...
	for (id = 0; id < qual_count; id++) {
		...
		rv = bcm_esw_field_data_qualifier_get(unit, qual_array[id],
						      &data_qual);
		if (BCM_FAILURE(rv)) {
			return (rv);
		}
		...
		hash_entry->match_arr[id].qual_id = qual_array[id];
		hash_entry->match_arr[id].qual_size = data_qual.length;
	}
				 ...
				 bcm_hash_entry_t * entry_id)
{
	 ...
	 * has the same sequence of offsets from UDF1.4 .. UDF1.7 for flex hash
	 */
				  ...
				  uint32 offset)
{
	...
	if (++hash_entry->offset_count > 2) {
		return BCM_E_RESOURCE;
	}
	hash_entry->hash_offset[hash_entry->offset_count - 1] = offset;

	rv = _bcm_flex_hash_table_write(unit, hash_entry);
	if (BCM_FAILURE(rv)) {
		return rv;
	}
...
bcm_td2_switch_hash_entry_reinstall(int unit,
				    bcm_hash_entry_t entry_id, uint32 offset)
{
	...
	if ((offset == 0) && (cnt == FLEX_HASH_MAX_OFFSET)) {
		for (cnt = 0; cnt < FLEX_HASH_MAX_OFFSET; cnt++) {
