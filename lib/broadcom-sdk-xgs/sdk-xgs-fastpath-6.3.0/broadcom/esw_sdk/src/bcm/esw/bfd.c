 ...
 */
_esw_bfd_drv_t    *_esw_bfd_drv[BCM_MAX_NUM_UNITS] = {0};
...
#define BFD_INIT(unit)                            \
    ...
    } else if (_ESW_BFD_DRV(unit) == NULL) {      \
        return BCM_E_UNAVAIL;                     \
...
bcm_esw_bfd_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_bfd)) {
        ...
        if (SOC_IS_KATANAX(unit)) {
            result = _bcm_kt_bfd_init(unit);
        } else
#endif
#if defined(BCM_TRIUMPH3_SUPPORT)
        if (SOC_IS_TRIUMPH3(unit)) {
            result = _bcm_tr3_bfd_init(unit);
        } else
#endif
        {
...
bcm_esw_bfd_detach(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_bfd)) {
        ...
        if ((_ESW_BFD_DRV(unit) != NULL) &&
            (_ESW_BFD_DRV(unit)->detach != NULL)) {
            result = _ESW_BFD_DRV(unit)->detach(unit);
        }
                            ...
                            bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_create != NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_create(unit, endpoint_info);
    }
                         ...
                         bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_get != NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_get(unit, endpoint,
                                                  endpoint_info);
    }
                             ...
                             bcm_bfd_endpoint_t endpoint)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_destroy != NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_destroy(unit, endpoint);
    }
...
bcm_esw_bfd_endpoint_destroy_all(int unit)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_destroy_all != NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_destroy_all(unit);
    }
                          ...
                          bcm_bfd_endpoint_t endpoint)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_poll!= NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_poll(unit, endpoint);
    }
                           ...
                           void *user_data)
{
    ...
    if (_ESW_BFD_DRV(unit)->event_register != NULL) {
        result = _ESW_BFD_DRV(unit)->event_register(unit,
                                                    event_types,
                                                    cb, user_data);
    }
                             ...
                             bcm_bfd_event_cb cb)
{
    ...
    if (_ESW_BFD_DRV(unit)->event_unregister != NULL) {
        result = _ESW_BFD_DRV(unit)->event_unregister(unit,
                                                      event_types, cb);
    }
                              ...
                              uint8 clear)
{
    ...
    if (_ESW_BFD_DRV(unit)->endpoint_stat_get != NULL) {
        result = _ESW_BFD_DRV(unit)->endpoint_stat_get(unit, endpoint,
                                                       ctr_info, clear);
    }
                          ...
                          bcm_bfd_auth_sha1_t *sha1)
{
    ...
    if (_ESW_BFD_DRV(unit)->auth_sha1_set != NULL) {
        result = _ESW_BFD_DRV(unit)->auth_sha1_set(unit, index, sha1);
    }
                          ...
                          bcm_bfd_auth_sha1_t *sha1)
{
    ...
    if (_ESW_BFD_DRV(unit)->auth_sha1_get != NULL) {
        result = _ESW_BFD_DRV(unit)->auth_sha1_get(unit, index, sha1);
    }
                                     ...
                                     bcm_bfd_auth_simple_password_t *sp)
{
    ...
    if (_ESW_BFD_DRV(unit)->auth_simple_password_set != NULL) {
        result = _ESW_BFD_DRV(unit)->auth_simple_password_set(unit,
                                                              index, sp);
    }
                                     ...
                                     bcm_bfd_auth_simple_password_t *sp)
{
    ...
    if (_ESW_BFD_DRV(unit)->auth_simple_password_get != NULL) {
        result = _ESW_BFD_DRV(unit)->auth_simple_password_get(unit,
                                                              index, sp);
    }
