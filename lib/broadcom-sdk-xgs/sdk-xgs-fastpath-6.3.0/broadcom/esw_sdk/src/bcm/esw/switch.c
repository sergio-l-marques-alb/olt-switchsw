/*
 * $Id: switch.c 1.634.4.10 Broadcom SDK $
...
#define TDTT    SOC_INFO_CHIP_TD_TT
...
_bcm_xgs3_urpf_route_enable(int unit,  int arg)
{
    ...
    if (SOC_IS_TRIUMPH3(unit) &&
       !BCM_TR3_ESM_LPM_TBL_PRESENT(unit, EXT_IPV4_DEFIPm)) {
        BCM_IF_ERROR_RETURN(_bcm_tr3_l3_defip_urpf_enable(unit, arg));
    }
#endif

#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TRIDENT2(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_td2_l3_defip_urpf_enable(unit, arg));
    }
#endif
    SOC_CONTROL_LOCK(unit);
    ...
    SOC_IF_ERROR_RETURN(soc_mem_clear(unit, L3_DEFIPm, MEM_BLOCK_ALL, 0));
    if (SOC_MEM_IS_ENABLED(unit, L3_DEFIP_PAIR_128m)) {
        SOC_IF_ERROR_RETURN(soc_mem_clear(unit, L3_DEFIP_PAIR_128m,
                                          MEM_BLOCK_ALL, 0));
    }
...
bcm_switch_binding_t xgs3_bindings[] =
{
    ...
    { bcmSwitchOamUnknownVersionDrop, TR2 | AP | VL2 | EN | TD | TR3 | HX4 | KT | KT2
...
#if defined(BCM_TRIUMPH_SUPPORT)
    { bcmSwitchDosAttackToCpu, TRVL|HU2,
    ...
    { bcmSwitchL3UrpfRouteEnableExternal, TRVL | TD2,
       ...
       NULL, soc_feature_urpf},
    { bcmSwitchMplsPWControlWordToCpu, TRVL,
...
defined(BCM_TRIDENT_SUPPORT)
    { bcmSwitchAlternateStoreForward, HBGW | SCQ | TDTT,
...
/*
 * Function:
 *      _bcm_switch_fcoe_ethertype_set
 * Description:
 *      Set the ethertype field for FCOE
 * Parameters:
 *      unit        -  unit number
 *      arg         - argument to set as Ethrtype
 * Returns:
...
STATIC int
_bcm_switch_fcoe_ethertype_set(int unit, int arg) 
{
    uint32   rval;
...
/*
 * Function:
 *      _bcm_switch_fcoe_ethertype_get
 * Description:
 *      Get the ethertype field for FCOE
 * Parameters:
 *      unit        - unit number 
 *      arg         - Ethrtype to get
 * Returns:
...
STATIC int
_bcm_switch_fcoe_ethertype_get(int unit, int *arg) 
{
    uint32   rval;
    int      enable;
 ...
 return BCM_E_NONE;
    ...
    ||  defined(BCM_ENDURO_SUPPORT) || defined(BCM_TRIDENT_SUPPORT)
        ...
        case bcmSwitchFcoeEtherType:
            if (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH3(unit)) {
                return _bcm_switch_fcoe_ethertype_set(unit, arg);
            }
        break;
#endif
#if defined(BCM_FIREBOLT_SUPPORT) 
    ...
    case bcmSwitchL2OverflowEvent:
        ...
        case bcmSwitchMiMDefaultSVPValue:
            if (SOC_IS_TRIDENT(unit) || SOC_IS_TRIDENT2(unit) ||
                ...
                soc_reg_field_set(unit, MIM_DEFAULT_NETWORK_SVPr, &rval,
                                  SVPf, arg);
                BCM_IF_ERROR_RETURN(WRITE_MIM_DEFAULT_NETWORK_SVPr(unit, rval));
                return BCM_E_NONE;
            }
            break;
#endif
                           ...
                           int *arg)
{
 ...
 return BCM_E_NONE;
    ...
    ||  defined(BCM_ENDURO_SUPPORT) || defined(BCM_TRIDENT_SUPPORT)
        ...
        case bcmSwitchLoadBalanceHashSelect:
            {
                ...
                BCM_IF_ERROR_RETURN(soc_reg_get(unit, reg, REG_PORT_ANY, 0, 
                                    &rval));
                *arg = soc_reg64_field32_get(unit, reg, rval, LBID_RTAGf);
                return (BCM_E_NONE);
            }
            break;
#if defined(BCM_TRIDENT_SUPPORT) && defined(INCLUDE_L3)
        ...
        case bcmSwitchFcoeEtherType:
            if (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH3(unit)) {
                return _bcm_switch_fcoe_ethertype_get(unit, arg);
            }
        break;
#endif
#if defined(BCM_FIREBOLT_SUPPORT) 
        ...
        case bcmSwitchL3SrcHitEnable:
            {
                ...
                BCM_IF_ERROR_RETURN(soc_reg_get(unit, reg, REG_PORT_ANY, 0, &rval));
                *arg = soc_reg64_field32_get(unit, reg, rval, L3SRC_HIT_ENABLEf);
                return (BCM_E_NONE);
            }
            break;
#endif /* INCLUDE_L3*/
        ...
        case bcmSwitchL2DstHitEnable:
            {
                ...
                BCM_IF_ERROR_RETURN(soc_reg_get(unit, reg, REG_PORT_ANY, 0, &rval));
                *arg = soc_reg64_field32_get(unit, reg, rval, L2DST_HIT_ENABLEf);
                return (BCM_E_NONE);
            }
            break;
        case bcmSwitchHashDualMoveDepth:
    ...
    case bcmSwitchL2OverflowEvent:
        ...
        case bcmSwitchOamOlpChipEtherType:
            if (SOC_IS_KATANA2(unit) && (soc_feature(unit, soc_feature_oam))) {
                ...
                *arg = soc_reg64_field32_get(unit, IARB_OLP_CONFIGr, rval, 
                                         ETHERTYPEf);
                return BCM_E_NONE;
            } 
            return BCM_E_UNAVAIL;
