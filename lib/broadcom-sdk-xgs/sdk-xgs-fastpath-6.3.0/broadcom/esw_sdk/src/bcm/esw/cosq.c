/*
 * $Id: cosq.c 1.129.2.1 Broadcom SDK $
    ...
    int *classifier_id)
{
    ...
    if (classifier->flags & BCM_COSQ_CLASSIFIER_L2 ||
        ...
        if (soc_feature(unit, soc_feature_service_queuing)) {
            BCM_IF_ERROR_RETURN(bcm_tr3_cosq_service_classifier_id_create(unit, 
    ...
    int classifier_id)
{
    ...
    if (_BCM_COSQ_CLASSIFIER_IS_FIELD(classifier_id)) {
        if (soc_feature(unit, soc_feature_field_ingress_cosq_override)) {
            BCM_IF_ERROR_RETURN
                (bcm_tr3_cosq_field_classifier_id_destroy(unit, classifier_id));
        } else {
            return (BCM_E_UNAVAIL);
        }
    } else 
    ...
    int classifier_id)
{
    ...
    if (_BCM_COSQ_CLASSIFIER_IS_FIELD(classifier_id)) {
        if (soc_feature(unit, soc_feature_field_ingress_cosq_override)) {
            BCM_IF_ERROR_RETURN
                (bcm_tr3_cosq_field_classifier_map_clear(unit, classifier_id));
        } else {
            return (BCM_E_UNAVAIL);
        }
    } else 
