/*
 * $Id: management.c 1.21 Broadcom SDK $
 *
...
/*     soc_cm_print("mgmt_send_tx(len:%d)\n", message_len); */
/*     _bcm_ptp_dump_hex(message, message_len, 0); */

    rv = stack_p->tx(unit, ptp_id, PTP_MESSAGE_TO_TOP, message, message_len);
    ...
    if (!response_data) {
        SOC_DEBUG_PRINT((DK_ERR, "Unexpected NULL response\n"));
    }
    ...
    if (tlvtype == bcmPTPTlvTypeManagementErrorStatus) {
        SOC_DEBUG_PRINT((DK_ERR | DK_VERBOSE, "Error response: 0x%04x\n",
    ...
    if (mgmt_id != cmd) {
        SOC_DEBUG_PRINT((DK_ERR | DK_VERBOSE, "Bad mgmt ID: %04x vs %04x\n", mgmt_id, cmd));
        goto dispose_of_resp;
    }
...
/*               bcm_errmsg(BCM_E_NONE), (unsigned)sal_time_usecs());           */
    ...
    rv = stack_p->tx(unit, ptp_id, PTP_TUNNEL_TO_TOP, payload, payload_len);
...
/*               bcm_errmsg(BCM_E_NONE), (unsigned)sal_time_usecs());             */
    ...
    rv = stack_p->tx(unit, ptp_id, PTP_TUNNEL_FROM_TOP, payload, payload_len);
...
/*     soc_cm_print("external_tx(type:%d len:%d)\n", (int)transport, message_len); */
/*     _bcm_ptp_dump_hex(message, message_len, 0); */
    ...
    case PTP_MESSAGE_TO_TOP:
    {
        ...
        *cursor++ = 0x00;
        ...
        /*   IPv4 total length */
        _bcm_ptp_uint16_write(cursor, 20 + 8 + message_len); cursor += 2;
        ...
        *cursor++ = 0;
        ...
        _bcm_ptp_uint16_write(cursor, 8 + message_len);  cursor += 2;  /* UDP length */
...
/*         _bcm_ptp_dump_hex(BCM_PKT_IEEE(mgmt_packet), mgmt_packet->pkt_data[0].len, 0); */
    ...
    case PTP_TUNNEL_TO_TOP:
    {
        ...
        uint16 udp_length = message_len + PTP_UDPHDR_SIZE_OCTETS;
        ...
        sal_memcpy(BCM_PKT_IEEE(tunnel_packet) + i, message, message_len);
...

{
    ...
    if (stack_p->int_state.mboxes->status[0] != MBOX_STATUS_EMPTY) {
        SOC_DEBUG_PRINT((DK_ERR, "PTP mbox contention\n"));

        /* wait for to-TOP buffer to be free */
        while (stack_p->int_state.mboxes->status[0] != MBOX_STATUS_EMPTY && wait_iter < 100000) {
            ++wait_iter;
            sal_usleep(1);
        }
        ...
        if (stack_p->int_state.mboxes->status[0] != MBOX_STATUS_EMPTY) {
            ...
            SOC_DEBUG_PRINT((DK_VERBOSE, "Wait to send outgoing to ToP: %d\n", wait_iter));
        }
    }

    sal_memcpy(msg_data, message, message_len);

    stack_p->int_state.mboxes->mbox[0].data_len = message_len;
    stack_p->int_state.mboxes->mbox[0].clock_num = 0;  

    /* Send the right kind of message */
    switch (transport) {
    case PTP_MESSAGE_TO_TOP:
        stack_p->int_state.mboxes->status[0] = MBOX_STATUS_ATTN_TOP_CMD;
        break;
    case PTP_TUNNEL_TO_TOP:
        stack_p->int_state.mboxes->status[0] = MBOX_STATUS_ATTN_TOP_TUNNEL_TO_TOP;
        break;
    case PTP_TUNNEL_FROM_TOP:
        stack_p->int_state.mboxes->status[0] = MBOX_STATUS_ATTN_TOP_TUNNEL_EXTERNAL;
        break;
    default:
    ...
    bcm_ptp_stack_id_t ptp_id)
{
    ...
    SOC_DEBUG_PRINT((DK_ERR, "Failed async Tx to ToP.  No clear\n"));
    ...
    int interval)
{
    ...
    sal_memcpy(message + PTP_PTPHDR_SRCPORT_OFFSET_OCTETS,
               slave_info->clock_identity, sizeof(bcm_ptp_clock_identity_t));
    ...
    rv = bcm_esw_ptp_clock_port_identity_get(unit, ptp_id, clock_num, slave_info->local_port_number, &portid);
    if (BCM_FAILURE(rv)) {
        goto release_return;
    }
    ...
    _bcm_ptp_uint16_write(message + PTP_SIGHDR_TGTPORT_OFFSET_OCTETS, slave_info->local_port_number);
    ...
    if (tlv_type == bcmPTPTlvTypeRequestUnicastTransmission) {
        ...
        case bcmPTPUDPIPv4:
            ...
            _bcm_ptp_uint32_write(message + i, slave_info->peer_address.ipv4_addr);
        ...
        case bcmPTPUDPIPv6:
            ...
            sal_memcpy(message + i, slave_info->peer_address.ipv6_addr, sizeof(bcm_ip6_t));
...
/*     soc_cm_print("sig_send_tx(len:%d)\n", message_len); */
/*     _bcm_ptp_dump_hex(message, message_len, 0); */
