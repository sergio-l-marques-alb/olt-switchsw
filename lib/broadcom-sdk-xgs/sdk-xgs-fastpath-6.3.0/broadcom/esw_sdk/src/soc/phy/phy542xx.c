/*
 * $Id: phy542xx.c 1.38.2.2 Broadcom SDK $ 
...
phy_bcm54282_init(int unit, soc_port_t port, int automedium) 
{
    ...
    */
    ...
    if (pc_base == NULL) {
        soc_cm_print("u=%d p=%d primary and offset not set correctly\n", unit, port);
        return SOC_E_FAIL;
    }
...
phy_bcm542xx_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if(PHY_COPPER_MODE(unit, port)) {
        ...
        if((model == PHY_BCM5428X_MODEL) || (speed == 1000)) {
            ...
            SOC_IF_ERROR_RETURN(
                    PHY_WRITE_BCM542XX_MII_CTRLr(unit, pc, mii_ctrl));
            SOC_IF_ERROR_RETURN(
                    PHY_MODIFY_BCM542XX_MII_GB_CTRLr(unit, pc, 0x00, 0x1800));
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_direct_reg_modify(unit, pc, 0x28, 0, 0x8000));
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_direct_reg_modify(unit, pc, 0x2c, 0, 0x0010));

        } else {
            /* Clear loopback setting for 1000M */
            SOC_IF_ERROR_RETURN(
                    PHY_MODIFY_BCM542XX_MII_CTRLr(unit, pc, 
                         0, PHY_BCM542XX_MII_CTRL_LPBK_EN));
            /* Set loopback setting for 100/10M */
            SOC_IF_ERROR_RETURN(
                    PHY_MODIFY_BCM542XX_MII_GB_CTRLr(unit, pc, 
                        enable? 0x1800 : 0x00, 0x1800));
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_direct_reg_modify(unit, pc,
                                                0x28, enable? 0x8000 : 0, 0x8000));
            SOC_IF_ERROR_RETURN
                (phy_bcm542xx_direct_reg_modify(unit, pc,
                                                0x2c, enable? 0x0010 : 0, 0x0010));
        }
    } else  if(PHY_FIBER_MODE(unit, port)) {
...
{
    uint16  dev_port = 0;
    phy_ctrl_t *pc = NULL, *pc_port1 = NULL;
    int phy_id_orig = 0, phy_base_addr = 0;
    soc_phy_chip_info_t *chip_info = NULL;
    soc_port_t primary_port;
    int offset = 0;
