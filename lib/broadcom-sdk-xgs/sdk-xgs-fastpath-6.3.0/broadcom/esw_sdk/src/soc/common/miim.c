                  ...
                  uint32 phy_reg_addr, uint16 *phy_data)
{
    ...
    if (rv != SOC_E_NONE) {
        soc_cm_debug(DK_ERR, "_soc_fe1600_linkscan_pause failed\n");
        return rv;
    }
    ...
    if(soc_cm_debug_check(DK_VERBOSE) || is_write) {
        if (clause45){
    ...
    if(SOC_E_NONE != lsrv) {
        soc_cm_debug(DK_ERR, "_soc_fe1600_linkscan_continue failed\n");
        rv = lsrv;
    }
                  ...
                  uint32 phy_reg_addr, uint16 *phy_data)
{
    int rv = SOC_E_NONE;
    int clause45, cmc;
    uint32 reg_val, real_phy_id, real_bus_id, internal_select;
                  ...
                  uint16 phy_reg_addr, uint16 phy_wr_data)
{
    ...
    if (SOC_CONTROL(unit)->miimIntrEnb) {
        ...
        if (sal_sem_take(SOC_CONTROL(unit)->miimIntr,
            ...
            soc_cm_debug(DK_MIIM | DK_VERBOSE,"  Interrupt received\n");
        }
        ...
        if (rv == SOC_E_NONE) {
            soc_cm_debug(DK_MIIM | DK_VERBOSE, "  Done in %d polls\n", to.polls);
        }
