/*
 * $Id: trident2.c 1.205.2.9 Broadcom SDK $
...
#define _TD2_MMU_TOTAL_CELLS_60K     61440 /* 64k cells */
#define _TD2_MMU_TOTAL_CELLS_64K     65536 /* 64k cells */

#define _TD2_MMU_CELLS_RSVD_IP       256

#define _TD2_MMU_TOTAL_CELLS(unit)  _TD2_MMU_TOTAL_CELLS_60K
                          ...
                          char *prefix_str)
{
    ...
    for (info_index = 0; ; info_index++) {
        ...
        case _SOC_PARITY_TYPE_MMU_THDO:
            SOC_IF_ERROR_RETURN
                (_soc_trident2_ser_process_mmu_thdo(unit, block_info_idx,
                                                    info, prefix_str));
            break;
#endif
        case _SOC_PARITY_TYPE_START_ERR:
...
STATIC _soc_generic_ser_info_t _soc_td2_tcam_ser_info[] = {
    ...
    { L2_USER_ENTRYm, INVALIDm, _SOC_SER_TYPE_PARITY, _SOC_SER_PARITY_4BITS,
      _SOC_SER_INTERLEAVE_MOD2,
      { {0, 80}, {1, 80}, {81, 160}, {82, 160} }, 0, 0, 0 },
    { IP_MULTICAST_TCAMm, INVALIDm, _SOC_SER_TYPE_PARITY, _SOC_SER_PARITY_4BITS,
      _SOC_SER_INTERLEAVE_MOD2,
    ...
    { EFP_TCAMm, INVALIDm, _SOC_SER_TYPE_PARITY, _SOC_SER_PARITY_4BITS,
      _SOC_SER_INTERLEAVE_MOD2,
      { {0, 235}, {1, 235}, {238, 473}, {239, 473} }, 0, 0, 0 },
    { FP_TCAMm, INVALIDm, _SOC_SER_TYPE_PARITY, _SOC_SER_PARITY_1BIT,
      _SOC_SER_INTERLEAVE_NONE,
      { {0, 165}, {1, 165}, {166, 329}, {167, 329} }, 0, 0, 0 },
    { INVALIDm },
};
...
{
    return soc_generic_ser_init(unit, _soc_td2_tcam_ser_info);
}
...
{
    soc_generic_ser_process_error(unit, _soc_td2_tcam_ser_info,
                                  _SOC_PARITY_TYPE_SER);
}
    ...
    if (SOC_CONTROL(unit)->l3_defip_urpf) { /* URPF mode */
        if (wide == 0) {
            if (index > _TD2_INDEX_2K_ENTRIES) {
                return index + SOC_CONTROL(unit)->l3_defip_index_remap/2+_TD2_OFFSET_2K_ENTRIES;
            }
                                    ...
                                    int enable)
{
    ...
    case L3_DEFIP_ALPM_IPV6_64_1m:
    case L3_DEFIP_ALPM_IPV6_128m:
        if (enable || soc_mem_index_count(unit, L3_DEFIP_ALPM_RAWm) > 0) {
            fval = 0xf;
        } else {
            fval = 0;
        }
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, ISS_MEMORY_CONTROL_84r, REG_PORT_ANY,
                                    BYPASS_ISS_MEMORY_LPf, fval));
        break;
    case MMU_INTFI_PFC_ST_TBLm:
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, INTFI_CFGr, REG_PORT_ANY,
                                    PFC_ST_TBL_DISABLEf, enable ? 1: 0));
        break;
    case LINK_STATUSm:
        SOC_IF_ERROR_RETURN
            (soc_reg_field32_modify(unit, SW2_HW_CONTROLr, REG_PORT_ANY,
...
soc_trident2_num_cosq_init(int unit)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        soc_td2_get_def_qbase(unit, port, _SOC_TD2_INDEX_STYLE_UCAST_QUEUE,
                                &uc_base, &uc_numq);
        soc_td2_get_def_qbase(unit, port, _SOC_TD2_INDEX_STYLE_MCAST_QUEUE,
                                &mc_base, &mc_numq);
...
soc_trident2_tsc_map_get(int unit, uint32 *tsc_map)
{
    ...
    switch (dev_id) {
    case BCM56850_DEVICE_ID: /* fall through */
 ...
 */
int
soc_trident2_port_config_init(int unit, uint16 dev_id)
{
    soc_info_t *si;
    ...
    int max_quad_io_bandwidth[_TD2_QUADS_PER_DEV];
    ...
    switch (dev_id) {
    case BCM56850_DEVICE_ID: /* fall through */
    case BCM56853_DEVICE_ID: /* fall through */
    case BCM56750_DEVICE_ID:
    case BCM56830_DEVICE_ID:
        si->bandwidth = 960000;
        si->frequency = 760;
        for (quad = 0; quad < _TD2_QUADS_PER_DEV; quad++) {
            max_quad_io_bandwidth[quad] = 320000;
            max_quad_core_bandwidth[quad] = 240000;
        }
        break;
    case BCM56851_DEVICE_ID:
        si->bandwidth = 720000;
    ...
    case BCM56852_DEVICE_ID:
        ...
        si->frequency = 760;
        for (quad = 0; quad < _TD2_QUADS_PER_DEV; quad++) {
            max_quad_io_bandwidth[quad] = 240000;
            max_quad_core_bandwidth[quad] = 240000;
        }
        break;
    case BCM56854_DEVICE_ID:
        si->bandwidth = 720000;
        si->frequency = 608;
        max_quad_io_bandwidth[0] = max_quad_io_bandwidth[3] = 160000;
        max_quad_io_bandwidth[1] = max_quad_io_bandwidth[2] = 200000;
        max_quad_core_bandwidth[0] = max_quad_core_bandwidth[3] = 160000;
        max_quad_core_bandwidth[1] = max_quad_core_bandwidth[2] = 200000;
        si->frequency = 608;
        break;
    ...
    if (max_quad_io_bandwidth[0] > max_quad_core_bandwidth[0]) {
        si->oversub_pbm =
    ...
    for (port = 0; port < num_port; port++) {
        ...
        SOC_PORT_GROUP_SET(unit, port, -1);
        SOC_PORT_SERDES_SET(unit, port, -1);
        SOC_PORT_NUM_LANES_SET(unit, port, -1);
    }

    SOC_PORT_P2L_MAPPING_SET(unit, phy_port_cpu, port_cpu);
    SOC_PORT_P2M_MAPPING_SET(unit, phy_port_cpu, mmu_port_cpu);
    SOC_PORT_P2L_MAPPING_SET(unit, phy_port_lb, port_lb);
    SOC_PORT_P2M_MAPPING_SET(unit, phy_port_lb, mmu_port_lb);;
    ...
    for (port = 0; port < num_port; port++) {
        ...
        /* 16 ports per PGW_CL */
        SOC_PORT_GROUP_SET(unit, port, (phy_port - 1) / _TD2_PORTS_PER_PGW);
        SOC_PORT_SERDES_SET(unit, port, (phy_port - 1) / _TD2_PORTS_PER_XLP);
        if (si->port_speed_max[port] > 20000) {
            SOC_PORT_NUM_LANES_SET(unit, port, 4);
        } else if (si->port_speed_max[port] > 10000) {
            SOC_PORT_NUM_LANES_SET(unit, port, 2);
        } else {
            SOC_PORT_NUM_LANES_SET(unit, port, 1);
        }
    ...
    for (pipe = 0; pipe < _TD2_PIPES_PER_DEV; pipe++) {
        ...
        for (pgw = 0; pgw < _TD2_PGWS_PER_PIPE; pgw++) {
            phy_port = 1 + pipe * _TD2_PORTS_PER_PIPE +
                pgw * _TD2_PORTS_PER_PGW + (pgw & 1 ? 20 : 0);
            port = si->port_p2l_mapping[phy_port];
...
_soc_trident2_tdm_init(int unit)
{
    ...
    int rv;
    soc_pbmp_t pbmp;
    int port, phy_port, mmu_port, block;
    int pipe, pgw, xlp, tsc, group, slot, base, index, count, id;
    int speed_max, weight, start_cnt, oversub, cut_through;
    uint32 xlport_map;
    soc_reg_t reg;
    soc_mem_t mem;
    ...
    static soc_reg_t pgw_ovs_spacing_regs[] = {
    ...
    static soc_field_t pgw_spacing_fields[] = {
    ...
    static soc_mem_t iarb_tdm_mems[2] ={
    ...
    si = &SOC_INFO(unit);
    cut_through = soc_property_get(unit, "cut_through", 0);
    ...
    if (_soc_td2_tdm.tdm_bw) { /* 720g special handling */
    ...
    if (SOC_DEBUG_CHECK(SOC_DBG_TDM)) {
        for (pgw = 0; pgw < _TD2_PGWS_PER_DEV; pgw += 2) {
    ...
    for (pgw = 0; pgw < _TD2_PGWS_PER_DEV; pgw += 2) {
        ...
        if (!si->block_valid[PGW_CL_BLOCK(unit, pgw)] &&
            ...
            /* Both master and slave are not in used */
            continue;
        }

        port = pgw | SOC_REG_ADDR_INSTANCE_MASK;

        SOC_IF_ERROR_RETURN(soc_reg_get(unit, PGW_TDM_CONTROLr, port, 0,
                                        &ctrl_rval64));
        ...
        SOC_IF_ERROR_RETURN
            (soc_reg_set(unit, PGW_TDM_CONTROLr, port, 0, ctrl_rval64));
    }
    ...
    for (pgw = 0; pgw < _TD2_PGWS_PER_DEV; pgw++) {
        block = PGW_CL_BLOCK(unit, pgw);
        port = SOC_BLOCK_PORT(unit, block);
        if (port < 0) { /* the PGW_CL block is not in used */
            continue;
        }
        ...
        for (xlp = 0; xlp < _TD2_XLPS_PER_PGW; xlp++) {
            ...
            SOC_IF_ERROR_RETURN(soc_reg_get(unit, reg, port, 0, &ctrl_rval64));
            for (index = 0; index < _TD2_PORTS_PER_XLP; index++) {
                ...
                soc_reg64_field32_set(unit, reg, &ctrl_rval64,
                                      pgw_obm_bypass_fields[index], oversub);
                soc_reg64_field32_set(unit, reg, &ctrl_rval64,
                                      pgw_obm_oversub_fields[index], oversub);
            }
            SOC_IF_ERROR_RETURN(soc_reg_set(unit, reg, port, 0, ctrl_rval64));
        }
    }
    ...
    for (pipe = 0; pipe < _TD2_PIPES_PER_DEV; pipe++) {
        ...
        for (group = 0; group < _MMU_OVS_GROUP_COUNT; group++) {
            ...
            if (phy_port >= NUM_EXT_PORTS) {
                ...
                weight = (speed_max + (speed_max < 20000 ? 2499 : 0)) / 2500;
            }
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if (SOC_PBMP_MEMBER(si->oversub_pbm, port)) { /* oversubscription port */
            if (si->port_speed_max[port] < 40000 || !cut_through) {
                soc_mem_field32_set(unit, EGR_EDB_XMIT_CTRLm, entry,
                                    WAIT_FOR_2ND_MOPf, 1);
            } else {
                start_cnt = si->frequency > 633 ? 13 : 15;
                soc_mem_field32_set(unit, EGR_EDB_XMIT_CTRLm, entry,
                                    START_CNTf, start_cnt);
            }
        } else { /* line rate port */
            if (cut_through) {
...
_soc_td2_ledup_init(int unit)
{
    ...
    #define P2L_VALUE(i,x) ((si->port_p2l_mapping[i] == -1)? 0 : \
                                (((si->port_p2l_mapping[i] - x) < 0) || \
                                ((si->port_p2l_mapping[i] - x) > 63)) ? 0: \
                                        (si->port_p2l_mapping[i] - x))
    ix = 1;
    rval = 0;
    uP1_off = 0;
    ...
    {
        int i;
        for (i=ix; i<128 && (si->port_p2l_mapping[i]==-1);i++);
        uP1_off = si->port_p2l_mapping[i] - 1;
    }
...
_soc_trident2_misc_init(int unit)
{
    ...
    freq = si->bandwidth == 960000 ? 760 : 608;
                                     ...
                                     soc_mem_t mem, int arr_sz, int arr_off)
{
    ...
    msz = soc_mem_index_count(unit, mem);
    if (arr_off >= msz) {
        arr_off -= msz;
    }

    return (mmu_port * arr_sz) + arr_off;
}
...
_soc_td2_mmu_init_dev_config(int unit, _soc_mmu_device_info_t *devcfg)
{
    ...
    devcfg->mmu_total_cell = _TD2_MMU_TOTAL_CELLS(unit) - _TD2_MMU_CELLS_RSVD_IP; /* cells for IP */
...
STATIC int
_soc_td2_default_pg_headroom(int unit, soc_port_t port, int lossless)
{
...
_soc_td2_mmu_config_buf_default(int unit, _soc_mmu_cfg_buf_t *buf)
{
    ...
    */
    ...
    max_packet_cells = _MMU_CFG_MMU_BYTES_TO_CELLS(_TD2_MMU_MAX_PACKET_BYTES +
                                              _TD2_MMU_PACKET_HEADER_BYTES,
                                              _TD2_MMU_BYTES_PER_CELL);
    jumbo_frame_cells = _MMU_CFG_MMU_BYTES_TO_CELLS(_TD2_MMU_JUMBO_FRAME_BYTES +
                                               _TD2_MMU_PACKET_HEADER_BYTES,
                                               _TD2_MMU_BYTES_PER_CELL);
    default_mtu_cells = _MMU_CFG_MMU_BYTES_TO_CELLS(_TD2_MMU_DEFAULT_MTU_BYTES +
                                               _TD2_MMU_PACKET_HEADER_BYTES,
                                               _TD2_MMU_BYTES_PER_CELL);

    total_pool_size = _TD2_MMU_TOTAL_CELLS(unit) - _TD2_MMU_CELLS_RSVD_IP;
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        soc_td2_get_def_qbase(unit, port, _SOC_TD2_INDEX_STYLE_MCAST_QUEUE, 
                       NULL, &tmp);
        numq += tmp;
        mcq_entry_reserved += numq * per_q_mcq_entry_guarantee;
        soc_td2_get_def_qbase(unit, port, _SOC_TD2_INDEX_STYLE_UCAST_QUEUE, 
                       NULL, &tmp);
        numq += tmp;
            ...
            if (idx == 7) {
                ...
                if (lossless) {
                    ...
                    buf_prigroup->headroom = 
                        _soc_td2_default_pg_headroom(unit, port, lossless);
...
_soc_td2_mmu_config_buf_set_hw(int unit, _soc_mmu_cfg_buf_t *buf)
{
    ...
    int index1;
    int lossless = soc_property_get(unit, spn_MMU_LOSSLESS, 1);
    int pool_resume = 0;
    uint16      dev_id;
    uint8       rev_id;
    ...
    rval = 0;
    fval = _TD2_MMU_TOTAL_CELLS(unit) - _TD2_MMU_CELLS_RSVD_IP;
    soc_reg_field_set(unit, CFAPFULLTHRESHOLDSETr, 
                                        &rval, CFAPFULLSETPOINTf, fval);
    SOC_IF_ERROR_RETURN(WRITE_CFAPFULLTHRESHOLDSETr(unit, rval));
    rval = 0;
    soc_reg_field_set(unit, CFAPFULLTHRESHOLDRESETr, &rval, CFAPFULLRESETPOINTf,
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < _TD2_MMU_NUM_POOL; idx++) {
            buf_port_pool = &buf_port->pools[idx];
            midx = SOC_TD2_MMU_PIPED_MEM_INDEX(unit, port, mem, _TD2_MMU_NUM_POOL, idx);
        ...
        for (idx = 0; idx < _TD2_MMU_NUM_PG; idx++) {
            ...
            midx = SOC_TD2_MMU_PIPED_MEM_INDEX(unit, port, mem, _TD2_MMU_NUM_PG, idx);
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            soc_mem_field32_set(unit, mem, entry0, 
                                Q_SPIDf, buf_queue[idx].pool_idx);
            ...
            rval = 0;
            SOC_IF_ERROR_RETURN(
              WRITE_MMU_THDM_DB_QUEUE_RESUME_OFFSET_PROFILE_YELLOWr(unit, 0, 1));
            
            rval = 0;
            SOC_IF_ERROR_RETURN(
              WRITE_MMU_THDM_DB_QUEUE_RESUME_OFFSET_PROFILE_REDr(unit, 0, 1));
        }
        ...
        for (idx = 0; idx < _TD2_MMU_NUM_POOL; idx++) {
            ...
            index1 = SOC_TD2_MMU_PIPED_MEM_INDEX(unit, port, mem2, 
                                                 _TD2_MMU_NUM_POOL, idx);
...
soc_td2_mmu_config_init(int unit, int test_only)
{
    ...
    _soc_td2_mmu_init_dev_config(unit, &devcfg);
    _soc_td2_mmu_config_buf_default(unit, buf);
    ...
    if (!test_only) {
        ...
        rv = _soc_td2_mmu_config_buf_set_hw(unit, buf);
    }
...
STATIC int
_soc_trident2_port_lanes_update_tdm(int unit, int port_base, int lanes,
                                    int cur_lanes, int *phy_ports,
                                    int phy_ports_len)
{
    soc_info_t *si = &SOC_INFO(unit);
    soc_reg_t reg, *reg_list[2];
    soc_mem_t mem;
    soc_field_t field;
    int phy_port_base, mmu_port_base, pgw_inst;
    int blk, bindex;
    int cur_ids[4], cur_ids_len;
    int ids[4], ids_len;
    int pipe, base, index, slot, slot_len, seq, count, reg_index, i;
    uint64 rval64;
    uint32 entry[SOC_MAX_MEM_WORDS];
    uint32 rval, fval;
    static soc_reg_t pgw_tdm_regs[] = {
    ...
    if (lanes > cur_lanes) { /* port(s) to be removed */
        for (i = 0; i < phy_ports_len; i++) {
            cur_ids[1 + i] = phy_ports[i];
        }
        cur_ids_len += phy_ports_len;
    } else { /* port(s) to be added */
        for (i = 0; i < phy_ports_len; i++) {
            ids[1 + i] = phy_ports[i];
        }
        ids_len += phy_ports_len;
    }

    /* Update PGW line rate ports and oversubscription ports TDM */
    reg_list[0] = pgw_tdm_regs;
    reg_list[1] = pgw_ovs_tdm_regs;
    pgw_inst = (si->port_group[port_base] & 0x6) | SOC_REG_ADDR_INSTANCE_MASK;
    for (reg_index = 0; reg_index < 2; reg_index++) {
        seq = 0;
        for (base = 0; base < _PGW_TDM_LENGTH;
             base += _PGW_TDM_SLOTS_PER_REG) {
            reg = reg_list[reg_index][base / _PGW_TDM_SLOTS_PER_REG];
    ...
    for (i = 0; i < SOC_DRIVER(unit)->port_num_blktype; i++) {
        ...
        if (SOC_BLOCK_INFO(unit, blk).type == SOC_BLK_PGW_CL) {
    ...
    if (soc_reg64_field32_get(unit, reg, rval64,
        ...
        fval = lanes > cur_lanes ? 0 : 1;
        for (i = 0; i < phy_ports_len; i++) {
            index = bindex % _TD2_XLPS_PER_PGW + phy_ports[i] - phy_port_base;
            soc_reg64_field32_set(unit, reg, &rval64,
    ...
    if (lanes > cur_lanes) { /* port(s) to be removed */
        for (i = 0; i < phy_ports_len; i++) {
            cur_ids[1 + i] = si->port_p2m_mapping[phy_ports[i]] & 0x3f;
        }
        cur_ids_len += phy_ports_len;
    } else { /* port(s) to be added */
        for (i = 0; i < phy_ports_len; i++) {
            ids[1 + i] = si->port_p2m_mapping[phy_ports[i]] & 0x3f;
        }
        ids_len += phy_ports_len;
    }
    ...
    for (slot = 0; slot < slot_len; slot += 2) {
        ...
        for (index = 0; index < 2; index++) {
            ...
            soc_mem_field32_set(unit, mem, entry, field, ids[seq % ids_len]);
            seq++;
            count++;
        }
        if (count > 0) {
 ...
 *      lanes         - Number of lanes to be used for the port
 *      cur_lanes     - (OUT) Number of lanes current configured on the port
 *      phy_ports     - (OUT) Array of added/removed physical port number
 *      phy_ports_len - (OUT) Length of phy_ports array
 *
 ...
 */
int
soc_trident2_port_lanes_set(int unit, soc_port_t port_base, int lanes,
                            int *cur_lanes, int *phy_ports, int *phy_ports_len)
{
    ...
    uint32 values[2];
    int mode, cur_mode;
    int pipe, phy_port_base, port, mmu_port, i;
    int blk, bindex;
    uint32 rval;
    ...
    static const soc_field_t port_fields[] = {
        PORT0f, PORT1f, PORT2f, PORT3f
    };
    ...
    for (i = 0; i < SOC_DRIVER(unit)->port_num_blktype; i++) {
        ...
        if (si->port_speed_max[port_base] >= 1000000) {
            if (SOC_BLOCK_INFO(unit, blk).type == SOC_BLK_CPORT) {
                bindex = SOC_PORT_IDX_BINDEX(unit, phy_port_base, i);
                break;
            }
        } else {
    ...
    if (lanes == 4) {
        ...
        if (cur_mode == SOC_TD2_PORT_MODE_QUAD) {
            mode = bindex == 0 ?
        ...
        if (cur_mode == SOC_TD2_PORT_MODE_DUAL) {
            mode = bindex == 0 ?
    ...
    *phy_ports_len = 0;
    if (lanes > *cur_lanes) { /* Figure out which port(s) to be removed */
        if (lanes == 4) {
            if (cur_mode == SOC_TD2_PORT_MODE_TRI_012 ||
                cur_mode == SOC_TD2_PORT_MODE_QUAD) {
            ...
            if (cur_mode == SOC_TD2_PORT_MODE_TRI_023 ||
                cur_mode == SOC_TD2_PORT_MODE_QUAD) {
        ...
        if (lanes == 2) {
            ...
            if (cur_mode == SOC_TD2_PORT_MODE_SINGLE) {
                if (si->port_p2l_mapping[phy_port_base + 2] != -1) {
                    phy_ports[(*phy_ports_len)++] = phy_port_base + 2;
                }
                if (si->port_p2l_mapping[phy_port_base + 3] != -1) {
    ...
    if (lanes > *cur_lanes) { /* port(s) to be removed */
        for (i = 0; i < *phy_ports_len; i++) {
            SOC_IF_ERROR_RETURN
                (WRITE_EGR_ENABLEm(unit, MEM_BLOCK_ALL, phy_ports[i], &entry));
        }
    }
    ...
    if (*phy_ports_len > 0) {
        /* Update TDM */
        SOC_IF_ERROR_RETURN
            (_soc_trident2_port_lanes_update_tdm(unit, port_base, lanes,
                                                 *cur_lanes, phy_ports,
                                                 *phy_ports_len));
    }
    ...
    /* Reset XLPORT */
    rval = 0;
    soc_reg_field_set(unit, XLPORT_SOFT_RESETr, &rval, port_fields[0], 1);
    if (lanes < *cur_lanes) { /* port(s) to be added */
        for (i = 0; i < *phy_ports_len; i++) {
            soc_reg_field_set(unit, XLPORT_SOFT_RESETr, &rval,
                              port_fields[phy_ports[i] - phy_port_base], 1);
        }
    }
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_SOFT_RESETr(unit, port_base, rval));
    SOC_IF_ERROR_RETURN(WRITE_XLPORT_SOFT_RESETr(unit, port_base, 0));
    ...
    if (lanes < *cur_lanes) { /* port(s) to be added */
        for (i = 0; i < *phy_ports_len; i++) {
            mmu_port = si->port_p2m_mapping[phy_ports[i]];
            SOC_IF_ERROR_RETURN
                (soc_reg32_set(unit, reg, REG_PORT_ANY, mmu_port & 0x3f, 0));
        }
    }
    ...
    if (lanes == 4 || *cur_lanes == 4) { /* reset all 4 lanes */
        values[0] = 4;
