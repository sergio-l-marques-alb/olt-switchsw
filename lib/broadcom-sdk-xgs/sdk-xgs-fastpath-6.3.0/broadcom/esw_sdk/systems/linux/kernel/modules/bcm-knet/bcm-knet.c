/*
 * $Id: bcm-knet.c 1.74.12.2 Broadcom SDK $
...
"Weight of NAPI interfaces (default 16)");
...
xgs_irq_mask_disable(bkn_switch_info_t *sinfo, int chan, int update_hw)
{
    ...
    if (chan == XGS_DMA_TX_CHAN) {
        ...
        mask = 0x180 << (chan - XGS_DMA_RX_CHAN);
    }
...
{
    DEV_WRITE32(sinfo, CMICM_DMA_STAT_CLRr, 1 << chan);
...
xgsm_dma_chan_abort(bkn_switch_info_t *sinfo, int chan, int polls)
{
    ...
    /* Abort the channel */
    DEV_READ32(sinfo, CMIC_DMA_CTRLr, &ctrl); 
    DEV_WRITE32(sinfo, CMIC_DMA_CTRLr, ctrl | 0x4); 
...
static int
bkn_api_rx_mirror(bkn_switch_info_t *sinfo, int chan, bkn_desc_info_t *desc)
{
    bkn_dcb_chain_t *dcb_chain;
    uint32_t *dcb;
    uint32_t dcb_stat;
...
bkn_rx_restart(bkn_switch_info_t *sinfo, int chan, int enable_intr)
{
    ...
    if (sinfo->rx[chan].running == 0 && sinfo->rx[chan].free) {
        ...
        dev_dma_chan_clear(sinfo, XGS_DMA_RX_CHAN + chan);
        dev_irq_mask_enable(sinfo, XGS_DMA_RX_CHAN + chan, enable_intr);
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        ...
        if (sinfo->rx[chan].free > 0) {
            if (use_napi) {
                ...
                bkn_napi_schedule(sinfo->dev, &sinfo->napi);
            } else {
                bkn_rx_restart(sinfo, chan, 1);
            }
        } else {
...
bkn_do_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcbs_done < budget) {
        ...
        if (filter) {
            ...
            case KCOM_DEST_T_API:
                DBG_FLTR(("Send to Rx API\n"));
                bkn_api_rx_mirror(sinfo, chan, desc);
                break;
            case KCOM_DEST_T_NETIF:
                found = 0;
                priv = NULL;
...
bkn_rx_desc_done(bkn_switch_info_t *sinfo, int chan)
{
    ...
    if (use_napi) {
        ...
        if (sched_poll_ok) {
            ...
            for (chan = 0; chan < NUM_RX_CHAN; chan++) {
                dev_irq_mask_disable(sinfo, XGS_DMA_RX_CHAN + chan, 0);
            }
        }
    } else {
...
xgs_isr(bkn_switch_info_t *sinfo)
{
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        if (dma_stat & DS_DESC_DONE_TST(XGS_DMA_RX_CHAN + chan)) {
            dev_dma_desc_clear(sinfo, XGS_DMA_RX_CHAN + chan);
            bkn_rx_desc_done(sinfo, chan);
        }

        if (dma_stat & DS_CHAIN_DONE_TST(XGS_DMA_RX_CHAN + chan)) {
            dev_dma_chain_clear(sinfo, XGS_DMA_RX_CHAN + chan);
            sinfo->rx[chan].running = 0;
...
bkn_poll(struct net_device *dev, int *budget)
{
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        rx_dcbs_done = bkn_do_rx(sinfo, chan, cur_budget);
        *budget -= rx_dcbs_done;
        cur_budget -= rx_dcbs_done;
        dev->quota -= rx_dcbs_done;
        if (sinfo->rx[chan].all_done == 0) {
            all_done = 0;
        }
    }
...
bkn_poll(struct napi_struct *napi, int budget)
{
    ...
    for (chan = 0; chan < NUM_RX_CHAN; chan++) {
        rx_dcbs_done = bkn_do_rx(sinfo, chan, budget);
        budget -= rx_dcbs_done;
        if (sinfo->rx[chan].all_done == 0) {
            all_done = 0;
        }
                    ...
                    int count, int *eof, void *data)
{
    ...
    *eof = 1;
    ...
    list_for_each(list, &_sinfo_list) {
        ...
        for (chan = 0; chan < NUM_RX_CHAN; chan++) {
            gmodule_pprintf(&ptr, "  Rx%d syncerr %10u\n",
...
_init(void)
{
    ...
    for (idx = 0; idx < num_dev; idx++) {
        ...
        if ((dev_type & (BDE_PCI_DEV_TYPE | BDE_ICS_DEV_TYPE)) == 0) {
            DBG_WARN(("Not PCI/ICS device - skipping\n"));
            continue;
        }
