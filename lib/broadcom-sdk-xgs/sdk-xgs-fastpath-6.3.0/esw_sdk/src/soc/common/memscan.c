/*
 * $Id: memscan.c 1.11 Broadcom SDK $
...
_soc_mem_scan_thread(void *unit_vp)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        ...
        if (soc_feature(unit, soc_feature_ser_parity)) {
            ...
            soc_cm_debug(DK_VERBOSE | DK_SOCMEM, "Unit:%d Scan mem: %s\n",
...
skip_mem_cache_compare:
    ...
    if (soc_feature(unit, soc_feature_ser_parity)) {
        soc_cm_debug(DK_TESTS, "Done: %d mem scan iterations\n", ++scan_iter);
        } else {
            soc_cm_debug(DK_TESTS, "Done: %d mem entries checked\n", entries_pass);
        }
    if (soc->mem_scan_interval != 0) {
