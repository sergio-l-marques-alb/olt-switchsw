/*----------------------------------------------------------------------
 * $Id: tscmod_cfg_seq.c 1.57.2.2 Broadcom SDK $
...
int tscmod_autoneg_get(tscmod_st* pc)               /* AUTONEG_GET */
{
   ...
   if(pc->per_lane_control==TSCMOD_AN_GET_CL73_ABIL) {   /* reading cl73 */
      ...
      if(data&(1<<1)) ability |=(1<<TSCMOD_ABILITY_20G_KR2) ;
...
int tscmod_autoneg_control(tscmod_st* pc)
{
  ...
  data = cl73_bam_code << AN_X4_ABILITIES_LOCAL_DEVICE_CL73_BAM_ABILITIES_CL73_BAM_CODE_SHIFT |
           hppam_20gkr2 << AN_X4_ABILITIES_LOCAL_DEVICE_CL73_BAM_ABILITIES_HPAM_20GKR2_SHIFT;
  ...
  */ 
  ...
  data = cl37_bam_enable << AN_X4_ABILITIES_ENABLES_CL37_BAM_ENABLE_SHIFT |
         ...
         0 << AN_X4_ABILITIES_ENABLES_CL37_AN_RESTART_SHIFT | 
         0 << AN_X4_ABILITIES_ENABLES_CL73_AN_RESTART_SHIFT ;
  ...
  if(pc->verbosity&TSCMOD_DBG_AN)
     printf("%-22s: u=%0d p=%0d Autoneg mode(%s) setting cl37_en=%0d cl73_en=%0d d=%0x verbosity=%x\n", 
              __func__, pc->unit, pc->port, e2s_tscmod_an_type[pc->an_type], cl37_enable, cl73_enable, data, pc->verbosity);
...
{   
   ...
   if((pc->port_type ==TSCMOD_MULTI_PORT)||(pc->port_type ==TSCMOD_DXGXS)||
      ...
      printf("%-22s: ERROR port_type=%0d undefined\n", __func__, pc->port_type) ;
   }
   accData   = 0 ;
   ...
   case 0: /* 0, 1, 2, 3 */
      ...
      case TSCMOD_DXGXS:        port_mode_sel = (pc->this_lane)?1:2; break ;
      case TSCMOD_SINGLE_PORT:  port_mode_sel = 4;  accData =1 ; break ;
      default: break ;
      }
      break ;
   case 1: /* 0, 1, {2,3} */
      ...
      case TSCMOD_DXGXS:        port_mode_sel = (pc->this_lane==2)?1:3; break ;
      case TSCMOD_SINGLE_PORT:  port_mode_sel = 4;  accData =1 ; break ;
      default: break ;
      }
      break ;
   case 2: /* {0,1}, 2, 3 */
      ...
      case TSCMOD_DXGXS:        port_mode_sel = (pc->this_lane==2)?3:2;  break ;
      case TSCMOD_SINGLE_PORT:  port_mode_sel = 4;  accData =1 ; break ;
      default: break ;
      }
      break ;
   case 3:  /* {0,1}, {2,3} */
      switch(pc->port_type) {
      case TSCMOD_MULTI_PORT:   port_mode_sel = (pc->this_lane>=2)?2:1; accData =1; break ;
      case TSCMOD_DXGXS:        port_mode_sel = 3;  break ;
      case TSCMOD_SINGLE_PORT:  port_mode_sel = 4;  accData =1 ; break ;
      default: break ;
      }
      break ;
   case 4: /* {0,1,2,3} */
      ...
      case TSCMOD_MULTI_PORT:   port_mode_sel = 0;  break ;
      case TSCMOD_DXGXS:        port_mode_sel = 3;  break ;
      case TSCMOD_SINGLE_PORT:  port_mode_sel = 4;  accData =0; break ;
      default: break ;
      }
      break ;
   default:
      printf("%-22s: ERROR port_mode_sel=%0d undefined\n", 
...
If per_lane_control = TSCMOD_CTL_AN_MODE_INIT (0x2), then the PLL and RX path are not reset.
...
int tscmod_set_an_port_mode(tscmod_st* pc)    /* SET_AN_PORT_MODE */
{
  ...
  if(pc->verbosity&TSCMOD_DBG_AN)
     printf("%-22s: u=%0d p=%0d AN(%s) setting lane_sel=%x \
this_lane=%0d dxgxs=%0d\n", __func__, pc->unit, pc->port, 
            e2s_tscmod_an_type[pc->an_type], pc->lane_select, 
  ...
  if ((pc-> an_type == TSCMOD_CL37) | (pc-> an_type == TSCMOD_CL37_BAM)) {
#if defined (_DV_TRIDENT2)
  ...
  if ((pc-> an_type == TSCMOD_CL73) | (pc-> an_type == TSCMOD_HPAM) | 
     ...
     plldiv = 40 ;
     printf("ERROR: %-22s u=%0d p=%0d should not be called\n",  
            __func__, pc->unit, pc->port) ;
  }
...
consistent for all ports\n", __func__, pc->unit, pc->port, plldiv_r_val, plldiv); 
     }
  ...
  mask |= MAIN0_SETUP_PORT_MODE_SEL_MASK ;  /* always 4-port mode */
  /* 0x9000 */
  SOC_IF_ERROR_RETURN (MODIFY_WC40_MAIN0_SETUPr(pc -> unit, pc, data, mask)); 
  ...
  if(pc->verbosity&TSCMOD_DBG_AN)
     printf("%-22s: u=%0d p=%0d AN(%s) clear credits setting lane_sel=%x this_lane=%0d dxgxs=%0d\n", 
            __func__, pc->unit, pc->port, e2s_tscmod_an_type[pc->an_type], pc->lane_select, pc->this_lane, pc->dxgxs);
...
int tscmod_parallel_detect_control(tscmod_st* pc)    /* PARALLEL_DETECT_CONTROL */
{
   uint16 data, mask ;
   data = 0 ; 
   if(pc->per_lane_control & 0x1) data |= AN_X4_ABILITIES_CONTROLS_PD_KX_EN_MASK; /* 1g */
   if(pc->per_lane_control & 0x2) data |= AN_X4_ABILITIES_CONTROLS_PD_KX4_EN_MASK; /* 10g */

   mask = AN_X4_ABILITIES_CONTROLS_PD_KX_EN_MASK |
          AN_X4_ABILITIES_CONTROLS_PD_KX4_EN_MASK;
...
int tscmod_tx_loopback_control(tscmod_st* pc)           /* TX_LOOPBACK_CONTROL  */
{
   ...
   printf("%-22s: u=%0d p=%0d data=%x ", __func__, 
...
int tscmod_rx_loopback_control(tscmod_st* pc)
{
   ...
   if(pc->verbosity & TSCMOD_DBG_PATH)
      printf("%-22s: u=%0d p=%0d rloop ctrl=%x N=%x P=%x R=%x data=%x mask=%x mode=%d(1=PCS;0=PMD)\n",
             __func__, pc->unit, pc->port, p_l_control, dataN, dataP, dataR, data, mask, mode);
...
int tscmod_encode_set(tscmod_st* pc)         /* ENCODE_SET */
{
  ...
  if((pc->spd_intf == TSCMOD_SPD_10_SGMII) |
     ...
     /* ------------------------------------ 0xa000 */
     mld_swap_en       = 0 ; 
  } else if (pc->spd_intf == TSCMOD_SPD_21000) {  /* 20G XAIU */
     /* ------------------------------------ 0xc111 */
     encode_mode       = 0x01;         
     cl49_bypass_txsm  = 1 ;   
     hg2_message_invalid_code_enable = 0 ;
...
int tscmod_set_spd_intf(tscmod_st* pc)                  /* SET_SPD_INTF */
{
  ...
  */
  ...
  if (pc->spd_intf > TSCMOD_SPD_ILLEGAL) {
    ...
    return SOC_E_PARAM;
     ...
     if (pc->verbosity & TSCMOD_DBG_SPD) 
        printf("%-22s: %s[%d]\n", __func__, e2s_tscmod_spd_intfc_type[pc->spd_intf],
  ...
  if        (spd_intf == TSCMOD_SPD_10_SGMII)   {
     ...
     plldiv = 66 ;
     if(pc->iintf==TSCMOD_IIF_XLAUI) firmware_mode = 3 ;
     else if(pc->iintf==TSCMOD_IIF_SR4) firmware_mode = 1 ;
     else firmware_mode = 0 ;
  ...
  if( (pc->plldiv != plldiv)|| (pc->plldiv != plldiv_r_val)||port_mode_sel_restart) {  /* re-do PLL sequence!! */
     pc->accData = plldiv ;
     if(pc->verbosity & TSCMOD_DBG_SPD) 
        printf("%-22s: p=%0d should restart new plldiv:%d from %0d reg=%0d restart=%0d\n",
               __func__, pc->port, plldiv, pc->plldiv, plldiv_r_val, port_mode_sel_restart);
     if( pc->per_lane_control & 0x1 ) {
        if(pc->verbosity&TSCMOD_DBG_SPD)
           printf("WARNING %-22s: p=%0d should restart new plldiv:%d from (%0d reg=%0d) or mode\
 restart=%0d first. set_spd_intf skipped. cntl=%0x pc=%p\n", 
                  __func__, pc->port, plldiv, pc->plldiv, plldiv_r_val, port_mode_sel_restart, pc->per_lane_control, (void *)pc);
...
int tscmod_rx_seq_control(tscmod_st* pc)         /* START_RX_SEQ or STOP_RX_SEQ */
{
   ...
   if ((new_lane != 0)&&(new_lane != 1)&&(new_lane != 2)&&(new_lane != 3)) {
      printf("%-22s: Error: invalid this_lane %d\n",__func__, new_lane);
      return SOC_E_ERROR;
   }
  ...
  if (cntl) {
    if(pc->spd_intf == TSCMOD_SPD_10_SGMII) {
      ...
      if (pc->plldiv == 40) {
       ...
       printf("%-22s ERROR: p=%0d spd_intf=%d undefined %s cntl=%0x verb=%x\n", __func__, pc->port, pc->spd_intf,
    ...
    data_credit0     = sgmii_spd_switch << TX_X4_CREDIT0_CREDIT0_SGMII_SPD_SWITCH_SHIFT | 
                       clockcnt0 << TX_X4_CREDIT0_CREDIT0_CLOCKCNT0_SHIFT;
    data_clkcnt1     = clockcnt1 << TX_X4_CREDIT0_CREDIT1_CLOCKCNT1_SHIFT |
                       clockcnt1_rsv << TX_X4_CREDIT0_CREDIT1_RESERVED0_SHIFT;
    ...
    mask_credit0 = TX_X4_CREDIT0_CREDIT0_SGMII_SPD_SWITCH_MASK |
                   TX_X4_CREDIT0_CREDIT0_CLOCKCNT0_MASK;
    mask_loopcnt = TX_X4_CREDIT0_LOOPCNT_LOOPCNT0_MASK | TX_X4_CREDIT0_LOOPCNT_LOOPCNT1_MASK;
...
This function is used to enable/disable rx lane (RSTB_LANE) or read back control bit for 
that per_lane_control is 1 or 0.
If per_lane_control is 0xa, only read back RSTB_LANE.
...
int tscmod_rx_lane_control(tscmod_st* pc)         /* RX_LANE_CONTROL */
{
  ...
  if (cntl==0xa) {
    ...
    if( data & RX_X4_CONTROL0_PMA_CONTROL_0_RSTB_LANE_MASK ) pc->accData =1 ;
    else                                                     pc->accData =0 ;
...
int tscmod_soft_reset(tscmod_st* pc)                      /* SOFT_RESET */
{
   ...
   if( cntl == TSCMOD_SRESET_AN_UNLOCK) {  /* jir CRTSC-616 workaround trial
      ...
      if(val) data = DSC2B0_DSC_MISC_CTRL0_RXSEQSTART_MASK ;
      else    data = 0 ;     /* enable */
      ...
      if(val) data = ANATX_ACONTROL6_DISABLE_TX_R_MASK ; 
      else    data = 0 ;
      ...
      if(val) data = ANATX_TX_DRIVER_ELEC_IDLE_MASK ; /* EIDLE */
      else    data = 0 ;                              /* normal */
...
int tscmod_prbs_check(tscmod_st* pc)
{
   ...
   if(data&PATGEN0_TESTMISCSTATUS_PRBS_LOCK_MASK) {
      if(dataH & PATGEN0_PRBSSTAT0_PRBS_LOCK_LOST_LH_MASK ) {
         /* locked now but lost before */
         pc->accData = dataH & ~PATGEN0_PRBSSTAT0_PRBS_LOCK_LOST_LH_MASK ; 
         pc->accData = (pc->accData<<16)| dataL|1  ;  /* locked once, so one error at least */
      } else {
         pc->accData = dataH ; 
         pc->accData = (pc->accData<<16)| dataL ; 
      }
   } else {
      if(dataH & PATGEN0_PRBSSTAT0_PRBS_LOCK_LOST_LH_MASK ) {
...
int tscmod_prbs_mode(tscmod_st* pc)                /* PRBS_MODE */
{
  ...
  /* JIRA PHY-804 */
  prbs_inv_rx = prbs_inv_rx ? 0: 1 ; 
  prbs_inv_tx = prbs_inv_tx ? 0: 1 ; 
...
int tscmod_lane_swap(tscmod_st *pc)
{
  ...
  if(pc->verbosity & TSCMOD_DBG_FUNC)
     printf("%s u=%0d p=%0d map=%x tx_map=%x rx_map=%x\n", __func__, pc->unit, pc->port, map, tx_map, rx_map) ;
...
int tscmod_tx_rx_polarity(tscmod_st *pc)
{
   ...
   if(pc->verbosity&TSCMOD_DBG_PATH)
      printf("%-22s: u=%0d p=%0d tx=%0d vec=%x mask=%x lane_s=%0d size=%0d\n", 
             __func__, pc->unit, pc->port, is_tx, vec, mask, lane_s, lane_size);
...
UC_INFO_B1_VERSIONr 0xf010 : cmd/status registers.
UC_INFO_B1_TARGETr  0xf011 : lower address.
UC_INFO_B1_CRCr     0xf015 : upper address.
UC_WRDATAr          0xffc3 : data.
...
{
   uint16 mask, dd, cmd, wr_data ; int wr, ram, addr, rv, loop, cnt ; 
   ...
   if(pc->per_lane_control & TSCMOD_UC_SYNC_WR_MODE) {
      wr = 1  ; mask = (pc->per_lane_control>>4) & 0xffff ; 
   } 
   if(ram==0) addr |= (pc->this_lane<<16) ;
   /* clear cmd */ 
   SOC_IF_ERROR_RETURN
      (WRITE_WC40_UC_INFO_B1_VERSIONr(pc->unit, pc, 0)) ;
   /* set addr, set data if write */
      
   /* lower address */
   dd = addr  ;
   SOC_IF_ERROR_RETURN(WRITE_WC40_UC_INFO_B1_TARGETr(pc->unit, pc, dd)) ;
   ...
   if(wr && (mask==0xffff)) { /* write 2B */
      ...
      SOC_IF_ERROR_RETURN(WRITE_WC40_UC_WRDATAr(pc->unit, pc, wr_data)) ;
   } else {  /* read-modify-write, or read */
      /* cmd = cmd */
   }
   SOC_IF_ERROR_RETURN(WRITE_WC40_UC_INFO_B1_CRCr(pc->unit, pc, dd)) ;
  
   /* write the cmd */
   cmd |= TSCMOD_UC_SYNC_REQ_MASK ;
   SOC_IF_ERROR_RETURN
      (WRITE_WC40_UC_INFO_B1_VERSIONr(pc->unit, pc, cmd)) ;
   
   /* wait for uC done for write or read */
   loop = 0 ; cnt = 0 ; 
   while(1) {
      SOC_IF_ERROR_RETURN(READ_WC40_UC_INFO_B1_VERSIONr(pc->unit, pc, &dd)) ;
      if( dd & TSCMOD_UC_SYNC_DONE_MASK ) break ;
      if(loop++ >= 1000) {
         loop = 0 ;  cnt ++ ; 
         printf("%-22s: u=%0d p=%0d wait done pending\n", __func__, pc->unit, pc->port) ;
      }
      if(cnt >10) {
         printf("%-22s: u=%0d p=%0d wait aborted\n", __func__, pc->unit, pc->port) ;
         return SOC_E_ERROR ; 
      }
   }
   ...
   if(wr) {
      ...
      SOC_IF_ERROR_RETURN(READ_WC40_UC_WRDATAr(pc->unit, pc, &dd)) ;
...
\details
FEC is controlled by fields in #tscmod_st::per_lane_control. The bit assignments
