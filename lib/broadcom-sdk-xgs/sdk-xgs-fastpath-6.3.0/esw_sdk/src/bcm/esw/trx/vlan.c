/*
 * $Id: vlan.c 1.170.2.1 Broadcom SDK $
                ...
                int use_svp)
{
    ...
    data32[0] = 0;
    data32[1] = 0; 
    soc_VLAN_XLATE_EXTDm_field_get(unit, vxxent, XLATE__DATA_0f, data32);

    /* clear the extra unused bits */
    data32[1] &= 0x3f;
    soc_VLAN_XLATEm_field_set(unit, vxent, DATAf,data32);
                ...
                vlan_xlate_extd_entry_t *vxxent)
{
    ...
    data32[0] = 0;
    data32[1] = 0; 
    soc_VLAN_XLATEm_field_get(unit, vxent, DATAf,data32);
    soc_VLAN_XLATE_EXTDm_field_set(unit, vxxent, XLATE__DATA_0f, data32);
    ...
    if (soc_VLAN_XLATEm_field32_get(unit, vxent, 
                                   SVP_VALIDf)) {
        value = soc_VLAN_XLATEm_field32_get(unit, vxent, 
                                   SOURCE_VPf);
        soc_VLAN_XLATE_EXTDm_field32_set(unit, vxxent, 
    ...
    if (SOC_IS_KATANA2(unit))
    {
        if (port != BCM_GPORT_INVALID)
        {
            ...
            if (BCM_GPORT_IS_TRUNK(port)) {
                ...
                BCM_IF_ERROR_RETURN(bcm_esw_trunk_get(unit, trunk_out,
                                NULL, 1, &member, &port_count));
                BCM_IF_ERROR_RETURN(_bcm_esw_gport_resolve(unit, member.gport,
                        &mod_out, &port_out, &trunk_out, &tmp_id));
            }

            /* Read the PORT_TABm entry  for  port
             * read VT_PORT_TYPE_SELECTf/VT_PORT_TYPE_SELECT_2f and
             * set the SOURCE_TYPEf accordingly */
            BCM_IF_ERROR_RETURN(soc_mem_read(unit, PORT_TABm, MEM_BLOCK_ANY,
                                port_out, &ptab));
            if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VT_KEY_TYPEf)) {
                pkey_type=soc_PORT_TABm_field32_get(unit, &ptab, VT_KEY_TYPEf);
            }
            if (SOC_MEM_FIELD_VALID(unit, PORT_TABm, VT_KEY_TYPE_2f)) {
                pkey_type2=soc_PORT_TABm_field32_get(unit, &ptab, VT_KEY_TYPE_2f);
            }
            if (pkey_type == key_type_value)
            {
                port_select = soc_PORT_TABm_field32_get(unit, &ptab, VT_PORT_TYPE_SELECTf);
            }
            else if (pkey_type2 == key_type_value)
            {
                port_select = soc_PORT_TABm_field32_get(unit, &ptab, VT_PORT_TYPE_SELECT_2f);
            }
            else
            {
                port_select = 0;
        ...
        } /* VALID PORT (PORT != BCM_GPORT_INVALID) */
    } /* SOC_IS_KATANA2*/
