...
extern int soc_th_alpm_bucket_assign(int u, int *log_bkt, int vrf, int v6);
...
#define ALPM_PHY_BKT(u, log)        (((log) >> SUB_BKT_SHIFT) & SOC_TH_ALPM_BKT_MASK(u))
...
/* TH: 13, AP: 14 */
#define SOC_ALPM_BUCKET_BITS(u) (SOC_IS_TOMAHAWK(u) ? 13 : 14)

#define SOC_TH_ALPM_AIDX_TO_PHY_BKT(u, aidx)       \
            (((aidx) >> (soc_alpm_cmn_banks_get(u) / 2)) & SOC_TH_ALPM_BKT_MASK(u))

#define SOC_TH_ALPM_AIDX_TO_SUB_BKT(u, aidx)       \
            ((aidx) >> ALPM_ENT_INDEX_BITS)
...
#define SOC_TH_ALPM_AIDX(u, phy, sub, bank, ent)          \
            ((sub) << ALPM_ENT_INDEX_BITS               | \
             (ent) << (SOC_ALPM_BUCKET_BITS(u) +          \
                       (soc_alpm_cmn_banks_get(u) / 2)) | \
             (phy) << (soc_alpm_cmn_banks_get(u) / 2)   | \
             (bank))
...
extern int soc_th_alpm_hit_bits_update(int u, alpm_mem_prefix_array_t *pa,
                                       int *ori_hptr, int *new_hptr);
...
extern uint32 soc_th_alpm_bank_dis(int u, int vrf);
extern int _soc_th_alpm_bkt_entry_cnt(int u, soc_mem_t alpm_mem);

/* For bucket sharing */
extern int _soc_th_alpm_128_bucket_move(int u, soc_mem_t mem, int log_bkt_from, int log_bkt_to);
extern void soc_th_alpm_bucket_usage_incr(int u, int log_bkt, int pivot_index, int vrf, soc_mem_t mem, int diff);
extern void soc_th_alpm_bucket_usage_decr(int u, int log_bkt, int clear, soc_mem_t mem, int diff);
