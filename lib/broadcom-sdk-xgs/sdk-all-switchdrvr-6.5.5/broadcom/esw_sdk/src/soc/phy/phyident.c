/*
 * $Id: phyident.c,v 1.476 Broadcom SDK $
    ...
    if (port == 5) {
        if(SOC_IS_VULCAN(unit)) {
            ...
            *phy_addr = temp;
            ...
            if (sys_if == SOC_PORT_IF_SGMII) {
                *phy_addr = 4;
                *phy_addr_int = 4;
            } else if (sys_if == SOC_PORT_IF_RGMII) {
                rv = REG_READ_MDIO_WAN_ADDRr(unit, &reg_val);
                soc_MDIO_WAN_ADDRr_field_get(unit, &reg_val,
                   ADDR_WANf, &temp);
                *phy_addr = temp;
...
#ifdef BCM_ROBO_SUPPORT
#ifdef BCM_STARFIGHTER3_SUPPORT
    if ((SOC_IS_STARFIGHTER3(unit)) && (port == 5) &&
        soc_property_port_get(unit, port,
                        spn_PHY_SYS_INTERFACE, 0) == SOC_PORT_IF_SGMII) {
        int_phyd = &phy_viper_drv;
        rv = PHY_PROBE(int_phyd, unit, int_pc);
        if ((rv == SOC_E_NONE) || (rv == SOC_E_UNAVAIL)) {
            LOG_INFO(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
                     "<%d>  Viper driver attach status\n"), rv));
        }
    } else if (_chk_gmii(unit, port, &_null_phy_entry, 0xffff, 0xffff, pi)) {
