...
/* Recover the KBP  */
int arad_kbp_recover_run_recovery_sequence(int unit, uint32 core, int mdio_id, uint32 retries, int level)
{
    ...
    for (i = 0; i < retries; ++i) {       
        ...
        if (level == 1) {
            ...
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("KBP recover doesn't support level %d"), level));
        }
        if(rv == SOC_E_NONE){
            /* KBP Recovery succeeded */
            LOG_INFO(BSL_LS_SOC_TCAM,
                     (BSL_META_U(unit,
                                 "KBP recovery succeeded within %d iterations for Level %d.\n"), i + 1, level));
            break;
        } else {
            if (i < retries - 1) {
                LOG_INFO(BSL_LS_SOC_TCAM,
                         (BSL_META_U(unit,
                                     "KBP recovery iteration %d/%d failed, trying again for Level %d.\n"), i + 1, retries, level));
            } else {
