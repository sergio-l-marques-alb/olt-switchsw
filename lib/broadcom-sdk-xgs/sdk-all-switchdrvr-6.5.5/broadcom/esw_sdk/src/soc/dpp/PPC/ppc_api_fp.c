/* $Id: ppc_api_fp.c,v 1.79 Broadcom SDK $
...
/******************************************************************
*
* FILENAME:       DuneDriver/ppc/src/soc_ppc_api_fp.c
*
* MODULE PREFIX:  ppc
*
* FILE DESCRIPTION:
  ...
  )
{
  ...
  sal_memset(info, 0x0, sizeof(SOC_TMC_PMF_PGM_MGMT_PARAMS));
  SOC_PPC_PMF_PFG_INFO_clear(&(info->pfg_info));
  SOC_TMC_PORT_PP_PORT_INFO_clear(&(info->pp_port));
  SOC_SAND_MAGIC_NUM_SET;
  ...
  )
{
  ...
  info->hdr_format = SOC_PPC_PP_NOF_FP_PKT_HDR_TYPES;
  ...
  )
{
	...
	for(cycle = 0; cycle < SOC_PPC_FP_NOF_CYCLES ; cycle++)
	{
		...
		{
			info->valid[cycle][idx]=0;
			info->fem_entry_id[cycle][idx] = 0;
			info->de_entry_id[cycle][idx] = 0;
			SOC_PPC_FP_DIR_EXTR_ENTRY_INFO_clear(&(info->de_entry[cycle][idx]));
		}
	}
            ...
            for(db_id = 0; db_id < SOC_PPC_FP_NOF_DBS; ++db_id) {
                ...
                if(pgm_idx == 0) {
                    for(fem_group_idx = 0; fem_group_idx < SOC_PPC_FP_NOF_MACROS; ++fem_group_idx) {
                        SOC_PPC_FP_RESOURCE_PMF_FEM_ENTRY_clear(&info->fem[cycle_idx][db_id][fem_group_idx]);
                    }
                }
            }
...
CONST static char*
  ...
  /* Special case for user-defined headers qualifiers */
  ...
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER0:
    str = "pfq0"; /* "packet_format_qualifier0" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1:
    str = "pfq1"; /* "packet_format_qualifier1" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1_INNER_TAG:
    str = "pfq1_inner_tag"; /* packet_format_qualifier1_inner_tag" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1_PRIORITY:
    str = "pfq1_next_priority"; /* "packet_format_qualifier1_next_priority" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1_OUTER_TAG:
    str = "pfq1_outer_tag"; /* "packet_format_qualifier1_outer_tag" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1_NEXT_PROTOCOL:
    str = "pfq1_next_protocol"; /* "packet_format_qualifier1_next_protocol" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER1_ENCAPSULATION:
    str = "pfq1_next_encapsulation"; /* "packet_format_qualifier1_next_encapsulation" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER2:
    str = "pfq2"; /* "packet_format_qualifier2" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER2_BOS:
    str = "pfq2_bos"; /* "packet_format_qualifier2_bos" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER2_NEXT_PROTOCOL:
    str = "pfq2_next_protocol"; /* "packet_format_qualifier2_next_protocol" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER2_IP_FRAGMENTED:
    str = "pfq2_ip_fragmented"; /* "packet_format_qualifier2_ip_fragmented"; */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER2_IP_HAS_OPTIONS:
    str = "pfq2_ip_has_options"; /* "packet_format_qualifier2_ip_has_options"; */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER3:
    str = "pfq3"; /* "packet_format_qualifier3"; */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER3_IP_FRAGMENTED:
    str = "pfq3_ip_fragmented"; /* "packet_format_qualifier3_ip_fragmented"; */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER4:
    str = "pfq4"; /* "packet_format_qualifier4" */
    break;
  case SOC_PPC_FP_QUAL_PACKET_FORMAT_QUALIFIER5:
    str = "pfq5"; /* "packet_format_qualifier5"; */
    break;
  case SOC_PPC_FP_QUAL_FORWARDING_ACTION_STRENGTH:
    str = "fwd_action_strength"; /* "forwarding_action_strength" */
    break;
  case SOC_PPC_FP_QUAL_FORWARDING_ACTION_METER_TRAFFIC_CLASS:
    str = "fwd_action_meter_traffic_class"; /* "forwarding_action_meter_traffic_class"; */
    break;
  ...
  case SOC_PPC_FP_QUAL_FORWARDING_OFFSET_EXTENSION:
    str = "fwd_offset_extension"; /* "forwarding_offset_extension" */
    break;
  case SOC_PPC_FP_QUAL_FORWARDING_HEADER_ENCAPSULATION:
    str = "fwd_header_encapsulation"; /* "forwarding_header_encapsulation" */
    break;
  ...
  )
{
  ...
  case SOC_PPC_FP_DB_TYPE_DIRECT_TABLE:
    str = "DT"; /* was "direct_table" */
  break;
  case SOC_PPC_FP_DB_TYPE_DIRECT_EXTRACTION:
    str = "DE"; /* was "direct_extraction" */
  break;
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "PFG info: ")));
  SOC_PPC_PMF_PFG_INFO_print(&(info->pfg_info));
  ...
  )
{
  ...
  for (ind = 0; ind < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; ++ind)
  {
    if (info->actions[ind].type != SOC_PPC_FP_ACTION_TYPE_INVALID)
    {
      LOG_CLI((BSL_META_U(unit,
                          "actions[%u]:"),ind));
      SOC_PPC_FP_DIR_EXTR_ACTION_VAL_print(&(info->actions[ind]));
    }
  }
  LOG_CLI((BSL_META_U(unit,
                      "priority: %u\n\r"),info->priority));
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "hdr_format: %s, "),SOC_TMC_FP_PKT_HDR_TYPE_to_string(info->hdr_format)));
  LOG_CLI((BSL_META_U(unit,
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
                      "Parser:")));
  SOC_PPC_FP_PACKET_DIAG_PARSER_print(&(info->parser));
  LOG_CLI((BSL_META_U(unit,
                      "Pgm:")));
  ...
  )
{
    ...
    if(info->valid)
    {
        ...
        arad_egress_pmf_db_action_get_unsafe(0, info->action_type, &action_lsb, &action_size, &action_valid);
  ...
  )
{
    ...
    for(idx = 0; idx < info->nof_actions && idx < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX; idx++)
    {
            ...
            if(type == SOC_PPC_FP_DB_TYPE_EGRESS)
                SOC_PPC_FP_RESOURCE_EGRESS_ACTION_print(&(info->action[idx]));
            else
  ...
  )
{
  ...
  LOG_CLI((BSL_META_U(unit,
	...
	for(cycle = 0; cycle < SOC_PPC_FP_NOF_CYCLES; cycle++)
	{
		for(idx = 0; idx < SOC_PPC_FP_NOF_MACROS; idx++)
		{
			if(info->valid[cycle][idx])
			{
				LOG_CLI((BSL_META_U(unit,
                                                    "\tvalid[%u][%u] = %u, "), cycle, idx, info->valid[cycle][idx]));
				LOG_CLI((BSL_META_U(unit,
                                                    "fem_entry_id[%u][%u] = %u, "), cycle, idx, info->fem_entry_id[cycle][idx]));
				LOG_CLI((BSL_META_U(unit,
                                                    "de_entry_id[%u][%u] = %u, "), cycle, idx, info->de_entry_id[cycle][idx]));
				SOC_PPC_FP_DIR_EXTR_ENTRY_INFO_print(&(info->de_entry[cycle][idx]));
			}
		}
	}
exit:
    ...
    if(info->nof_ces)
    {
      ...
      for(key_idx = 0; key_idx < SOC_PPC_FP_KEY_NOF_KEYS_PER_DB_MAX; key_idx++)
      {
          for(idx = 0; idx < info->nof_ces; idx++)
          {
            ...
            {
                SOC_PPC_FP_RESOURCE_KEY_print(&(info->key_qual[idx]));
            }
          }
      }
    }
  ...
  )
{
    ...
    if(info->is_used) {
        ...
        LOG_CLI((BSL_META_U(unit,
                            "%-15d"),info->db_strength));
        LOG_CLI((BSL_META_U(unit,
                            "%-15d"),info->entry_strength));
        LOG_CLI((BSL_META_U(unit,
                            "%-10d"),info->entry_id));
        LOG_CLI((BSL_META_U(unit,
                            "%-50s"),SOC_PPC_FP_ACTION_TYPE_to_string(info->action_type)));
  ...
  )
{
    ...
    if(info->is_used){
                ...
                for(db_id = 0; db_id < SOC_PPC_FP_NOF_DBS; ++db_id) {
                    for(fes_id = 0; fes_id < 32; ++fes_id) {
                        if(info->fes[pgm_idx][cycle_idx][db_id][fes_id].is_used) {
                            ...
                            LOG_CLI((BSL_META_U(unit,
                                                "%-10d"),fes_id));
                            SOC_PPC_FP_RESOURCE_PMF_FES_print(&info->fes[pgm_idx][cycle_idx][db_id][fes_id]);
                        }
                    }
                }
            }
        ...
        for(cycle_idx = 0; cycle_idx < SOC_PPC_FP_NOF_CYCLES; ++cycle_idx) {
            for(db_id = 0; db_id < SOC_PPC_FP_NOF_DBS; ++db_id) {
                for(fem_group_idx = 0; fem_group_idx < SOC_PPC_FP_NOF_MACROS; ++fem_group_idx) {
                    if(info->fem[cycle_idx][db_id][fem_group_idx].is_used) {
                        if(!is_header_printed) {
                            LOG_CLI((BSL_META_U(unit,
                                                "\n\r\n\r")));
                            LOG_CLI((BSL_META_U(unit,
                        ...
                        LOG_CLI((BSL_META_U(unit,
                                            "%-10d"),fem_idx));
                        SOC_PPC_FP_RESOURCE_PMF_FEM_ENTRY_print(&info->fem[cycle_idx][db_id][fem_group_idx]);
                    }
                }
            }
        }
  ...
  )
{
  ...
  for(stage = 0; stage < SOC_PPC_NOF_FP_DATABASE_STAGES; stage++)
  {
 	  ...
 	  for(idx = 0; idx < nof_lines; idx++)
	  {
			if(info->presel[stage][idx].is_valid)
			{
				...
				LOG_CLI((BSL_META_U(unit,
                                                    "%-8u"), idx));
				SOC_PPC_FP_RESOURCE_PRESEL_print(&(info->presel[stage][idx]));
			}
	  }
	}
	LOG_CLI((BSL_META_U(unit,
  ...
  SOC_PPC_FP_RESOURCE_AVAILABLE_print(&info->available);
  LOG_CLI((BSL_META_U(unit,
                      "\n\n************************************** ERROR INFO *******************************************\n\n\r")));
