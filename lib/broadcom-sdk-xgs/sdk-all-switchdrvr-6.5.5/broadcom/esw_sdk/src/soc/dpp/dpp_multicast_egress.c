...
STATIC uint32 dpp_egress_group_get_core_replication(
    ...
    SOC_SAND_IN  dpp_mc_id_t           group_mcid, /* input: group mcid */
    SOC_SAND_OUT dpp_mc_core_bitmap_t  *out_cores) /* output: cores replicated to */
{
...
STATIC uint32 dpp_egress_group_set_core_replication(
    SOC_SAND_IN  int                   unit,       /* input: device */
    SOC_SAND_IN  dpp_mc_id_t           group_mcid, /* input: group mcid, SOC_DPP_CONFIG(unit)->tm.nof_mc_ids means set all groups */
    SOC_SAND_IN  dpp_mc_core_bitmap_t  cores)      /* input: cores to replicate to */
{
    int index, offset; /* index in the table being changed and bit offset inside the table entry */
    ...
    for (mode = 0; mode < 2; ++mode) { /* Set hardware fore ingress and for egress if needed */
        ...
        if (group_mcid == SOC_DPP_CONFIG(unit)->tm.nof_mc_ids) { /* set core replication of all groups */
            ...
            uint32s_per_entry = groups_per_entry * bits_per_group / SOC_SAND_NOF_BITS_IN_UINT32; /* assumes no remainder */

            data[0] = ((uint32)cores) << offset_in_group_bits; /* set the bits of one group; */
            for (i = DPP_FDA_BITS_PER_GROUP; i < SOC_SAND_NOF_BITS_IN_UINT32; i *= 2) { /* fill a uint32 with the group */
                data[0] |= data[0] << i;
            }
            ...
            if (groups_remainder > 0) {
                ...
                i = groups_remainder / SOC_SAND_NOF_BITS_IN_UINT32; /* offset in uint32s */
                groups_remainder = groups_remainder % SOC_SAND_NOF_BITS_IN_UINT32; /* offset of bits to clear in uint32 */
...
uint32 dpp_mult_cud_to_port_map_set(
    ...
    SOC_SAND_IN SOC_TMC_FAP_PORT_ID port  /* input local logical port mapped to */
)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (port == _SHR_GPORT_INVALID) {
            ...
            DPP_CUD2CORE_GET_CORE(unit, cud, previous_core); /*get core from sw*/
...
uint32 dpp_mult_cud_to_port_map_get(
    SOC_SAND_IN  int                 unit, /* input device */
    SOC_SAND_IN  uint32              cud,  /* input cud/outlif */
    SOC_SAND_OUT SOC_TMC_FAP_PORT_ID *port /* output local logical port */
)
{
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
    uint32 entry[2];
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        DPP_CUD2CORE_GET_CORE(unit, cud, core); /*get core from sw*/
        if (core == DPP_CUD2CORE_UNDEF_VALUE) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("CUD isn't mapped to port")));
        }
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_to_local_port_get(unit, core, tm_port, &logical_port)); /*set port to local logical port*/
        *port = logical_port;
    } else {
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(dpp_mult_does_group_exist(unit, group_mcid, TRUE, is_open)) ; /* check if the group is open */
...
)
{
    ...
    dpp_mc_core_bitmap_t non_zero_cores = DPP_MC_CORE_BITAMAP_NO_CORES; /* The cores whose ports have replications */
    dpp_mc_core_bitmap_t prev_cores_with_reps, new_cores_with_reps; /* The cores whose ports previously had/now have replications */
    ...
    SOCDNX_IF_ERR_EXIT(dpp_egress_group_get_core_replication(unit, entry_offset, &prev_cores_with_reps)); /* get egress cores replicated to */
...
uint32 dpp_mult_eg_bitmap_group_get(
    ...
    SOC_SAND_OUT SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *group     /* output TM port bitmap */
)
{
    SOCDNX_INIT_FUNC_DEFS;
...
STATIC uint32 dpp_mult_eg_add_group_block(
    ...
    SOC_SAND_INOUT dpp_rep_data_t              **port_outlif_reps, /* A pointer to the non couple port+CUD replications to work on */
    SOC_SAND_INOUT dpp_rep_data_t              **cud_reps          /* A pointer to the CUD only replications to work on */
)
{
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
    ...
    for (left = outlif_entries; left; --left, ++index) { /* add outlif entries */
        rep = (*cud_reps)++;
        rep2 = (*cud_reps)++;
        SOCDNX_IF_ERR_EXIT(dpp_mult_eg_write_entry_cud_noptr( unit, index, rep, rep2, (*cud_reps)++, last_entry - index, prev)); /* write to hardware */
        prev = index;
    }
...
static INLINE uint32 dpp_mult_eg_write_cud2_entry_struct(
    SOC_SAND_IN    int               unit,
    SOC_SAND_IN    uint8             cud2_type,   /* 2nd CUD type of the group/linked list */
    SOC_SAND_INOUT arad_mcdb_entry_t *cud2_entry, /* entry to write to */
    SOC_SAND_IN    uint32            cud2,        /* 2nd CUD value to write */
    SOC_SAND_IN    uint32            next_entry   /* the next entry */
) {
    SOCDNX_INIT_FUNC_DEFS;
...
STATIC uint32 dpp_mult_eg_write_pointer_entry(
    ...
    SOC_SAND_INOUT dpp_rep_data_t **couples_reps,   /* The port+CUD replication couples to work on */
    SOC_SAND_INOUT dpp_rep_data_t **port_cud_reps,  /* The port+CUD replications not in couples to work on */
    SOC_SAND_INOUT dpp_rep_data_t **cud_only_reps,  /* The CUD only replications to work on */
    SOC_SAND_INOUT dpp_rep_data_t **bitmap_reps     /* The bitmap+CUD replications to work on */
)
{
    dpp_rep_data_t *rep;
    SOCDNX_INIT_FUNC_DEFS;
...
STATIC uint32 dpp_mult_eg_write_pointer_entry_struct(
    ...
    SOC_SAND_IN    uint32 next_entry,               /* the mcdb index of the next entry which will be the pointer in the entry */
    SOC_SAND_IN    uint16 nof_reps_in_core,         /* The number of replications in the current core */
    SOC_SAND_INOUT uint16 *nof_port_couples,        /* The number of replication couples with the same outlif and with the first port < 127 moved to */
    SOC_SAND_INOUT uint16 *nof_remaining_ports,     /* The number of remaining port+outlif replications now at the end of the port+outlif replications */
    SOC_SAND_INOUT uint16 *nof_outlif_replications, /* The number of outlif only replications in the mcds */
    SOC_SAND_INOUT uint16 *nof_bitmap_replications, /* The number of bitmap+outlif only replications in the mcds */
    ...
    SOC_SAND_INOUT dpp_rep_data_t **port_cud_reps,  /* The port+CUD replications not in couples to work on */
    SOC_SAND_INOUT dpp_rep_data_t **cud_only_reps,  /* The CUD only replications to work on */
    SOC_SAND_INOUT dpp_rep_data_t **egr_bitmap_reps /* The bitmap+CUD replications to work on */
)
{
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
    dpp_rep_data_t *rep, *rep2;
...
)
{
    ...
    dpp_mc_core_bitmap_t non_zero_cores = DPP_MC_CORE_BITAMAP_NO_CORES; /* The cores whose ports have replications */
    dpp_mc_core_bitmap_t prev_cores_with_reps, new_cores_with_reps = 0; /* The cores whose ports previously had/now have replications */
    dpp_rep_data_t *rep, *start_of_cud_reps, *start_of_cud2_reps, *couples_dst, *rep_ptr, *rep_ptr2;
    arad_mcdb_entry_t *cud2_entry; /* entry pointer used to write a 2nd CUD entry in MCDB */
    ...
    *out_err = _SHR_E_NONE;
    ...
    DPP_MC_FOREACH_CORE(cores_to_set, core_bm, core) {
        ...
        pointer_entry = list_end == DPP_MC_EGRESS_LINK_PTR_END ? 0 : 1; /* number of pointer entries needed at the end of the block, must be 1 or 0 */

        nof_reps_left_in_core = mcds->nof_reps[core];
        rep = mcds->reps + DPP_MCDS_GET_REP_INDEX(core, 0);
        if (nof_reps_left_in_core) {
            non_zero_cores |= DPP_MC_CORE_BITAMAP_CORE_0 << core; /* mark that the core has replications for core replication */
        ...
        do { /* loop over the 2nd CUD */
            ...
            if ((created_linked_list_start = is_group_start && nof_reps_left_in_core == 0)) {
                ...
                if (cud2 != DPP_MC_NO_2ND_CUD) { /* If a 2nd CUD was specified, the first MCDB entry we create will be one specifying the 2nd CUD */
            ...
            if (!created_linked_list_start && cud2 != DPP_MC_NO_2ND_CUD) {
                ...
                dpp_mult_eg_write_cud2_entry_struct(unit, mcds->info_2nd_cud, cud2_entry, cud2, linked_lists[core]); /* write the needed hardware fields to the entry structure */
                SOCDNX_IF_ERR_EXIT(dpp_mcds_write_entry(unit, block_start)); /* write to hardware */
        ...
        DPP_MC_ASSERT( /* there must not be unprocessed replications belonging to this 2nd CUD */
    ...
    if (is_group_start) { /* handle replication to egress cores */
        SOCDNX_IF_ERR_EXIT(dpp_egress_group_get_core_replication(unit, group_id, &prev_cores_with_reps)); /* get egress cores replicated to */
...
)
{
    ...
    DPP_MC_FOREACH_CORE(cores_to_set, core_bm, core) { /* For each core handle its existing linked list or its first linked list entries */
        ...
        if (!group_exists && group_entry_id != MCDS_INGRESS_LINK_END(mcds)) {
            if (DPP_MCDS_TYPE_IS_USED(entry_type)) { /* relocate conflicting entries if needed */
                ...
                DPP_MCDS_SET_TYPE(mcds, group_entry_id, DPP_MCDS_TYPE_VALUE_EGRESS_START); /* mark the first group entry as the start of an ingress group */
            }
    ...
    SOCDNX_IF_ERR_EXIT(dpp_mcds_copy_replications_from_arrays(unit, 1, 1, group_size, reps)); /* copy group replications to mcds */

    SOCDNX_IF_ERR_EXIT(mcds->set_egress_linked_list( /* build the group, including the first entry */
...
STATIC uint32 dpp_mcds_remove_replications_from_egress_group(
    ...
    SOC_SAND_IN uint32      cud,         /* The CUD of the replication to be removed */
    SOC_SAND_IN uint32      cud2         /* The 2nd CUD of the replication to be removed */
)
{
    dpp_mcds_base_t *mcds = dpp_get_mcds(unit);
    const uint32 entry_index = DPP_MCDS_GET_EGRESS_GROUP_START(mcds, group_mcid, core); /* table index of the first entry in the group */
            ...
            switch (format) {
              ...
              case 0: /* Entry containing two port+CUD replications sharing the same CUD */
                ...
                if (correct_cud2 && is_port_cud_replication && soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_1m, mcdb_entry, OUTLIF_1f) == cud) {
                    ...
                    if (dest == port) {
                        found = 1;
                        soc_mem_field32_set(unit, IRR_MCDB_EGRESS_FORMAT_1m, mcdb_entry, PP_DSP_1Af, DPP_MULT_EGRESS_PORT_INVALID); /* remove replication */
                        ...
                        found = 1;
                        soc_mem_field32_set(unit, IRR_MCDB_EGRESS_FORMAT_1m, mcdb_entry, PP_DSP_1Bf, DPP_MULT_EGRESS_PORT_INVALID); /* remove replication */
                        if (port == DPP_MULT_EGRESS_PORT_INVALID) {
                            remove_entry = 1;
                        }
                    }
              ...
              case 1: /* bitmap or 2nd CUD entry */
                ...
                if (soc_mem_field32_get(unit, IRR_MCDB_EGRESS_FORMAT_2m, mcdb_entry, NEW_FORMATf)) { /* This is a 2nd CUD specification entry */
                    ...
                    soc_mem_field32_set(unit, IRR_MCDB_EGRESS_FORMAT_2m, mcdb_entry, BMP_PTRf, DPP_MC_EGR_BITMAP_DISABLED); /* remove replication */
              ...
              default:
                if (format < 8) { /* two CUD only replications */
                    ...
                    if (correct_cud2 && is_cud_only_replication) {
                        ...
                        if (outlif1 == cud) {
                            found = 1;
                            soc_mem_field32_set(unit, IRR_MCDB_EGRESS_FORMAT_3m, mcdb_entry, OUTLIF_1f, DPP_MC_EGR_OUTLIF_DISABLED); /* remove replication */
                            if (outlif2 == DPP_MC_EGR_OUTLIF_DISABLED) { /* replication not disabled for this entry part */
                                remove_entry = 1;
                            }
                        } else if (outlif2 == cud) {
                            found = 1;
                            soc_mem_field32_set(unit, IRR_MCDB_EGRESS_FORMAT_3m, mcdb_entry, OUTLIF_2f, DPP_MC_EGR_OUTLIF_DISABLED); /* remove replication */
...
)
{
    ...
    if (group_size > DPP_MULT_MAX_EGRESS_REPLICATIONS - nof_reps) {
...
)
{
    ...
    if (!is_open) { /* group is not open */
        ...
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("currently supporting the removal of only one replication")));
    }

    /* Set changed_cores to contain the cores of the replication to be removed */
    SOCDNX_IF_ERR_EXIT(dpp_mcds_copy_replications_from_arrays(unit, TRUE, TRUE, nof_reps, reps)); /* Clear the replications in mcds and add the replication */
    for (core = 0; core < SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores; ++core) {
        if (mcds->nof_reps[core] > 0) {
            changed_cores |= DPP_MC_CORE_BITAMAP_CORE_0 << core;
        }
    ...
    SOCDNX_IF_ERR_EXIT(dpp_mult_remove_replication(unit, DPP_MCDS_TYPE_VALUE_EGRESS_START, reps->dest, reps->cud, reps->additional_cud, out_err, &changed_cores));
    ...
    /* recreate the group with the extra replication */
    {
        ...
        if (*out_err) { /* If failed due to lack of free entries */
            ...
            DPP_MC_FOREACH_CORE(changed_cores, core_bm, core) {
                ret1 = dpp_mcds_remove_replications_from_egress_group(unit, core, group_mcid, reps->dest, reps->cud, reps->additional_cud);
                if (ret2 == SOC_E_NONE) {
                    ret2 = ret1;
                }
            }
...
)
{
    ...
    SOCDNX_IF_ERR_EXIT(dpp_egress_group_get_core_replication(unit, multicast_id_ndx, &old_cores)); /* get egress cores replicated to */
    if (old_cores != new_cores) {
        SOCDNX_IF_ERR_EXIT(dpp_egress_group_set_core_replication(unit, multicast_id_ndx, new_cores)); /* mark no replications to cores */
    }
...
/*********************************************************************
*   Add port replications to an Egress-Multicast bitmap group. - replaced by dpp_mult_eg_bitmap_group_bm_add
*********************************************************************/
...
uint32 dpp_mult_eg_bitmap_group_bm_add(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
    SOC_SAND_IN  SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *add_bm, /* TM ports to add */
    SOC_SAND_OUT SOC_TMC_ERROR                         *out_err /* return possible errors that the caller may want to ignore */
)
{
...
/*********************************************************************
*   Removes a port replication of an egress multicast bitmap group. - replaced by dpp_mult_eg_bitmap_group_bm_remove
*********************************************************************/
...
uint32 dpp_mult_eg_bitmap_group_bm_remove(
    SOC_SAND_IN  int                 unit,
    SOC_SAND_IN  dpp_mc_id_t         multicast_id_ndx,
    SOC_SAND_IN  SOC_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *rem_bm, /* TM ports to remove */
    SOC_SAND_OUT SOC_TMC_ERROR                         *out_err /* return possible errors that the caller may want to ignore */
)
{
    unsigned nof_tm_port_words = (SOC_TMC_NOF_FAP_PORTS_PER_CORE / SOC_SAND_NOF_BITS_IN_UINT32) * SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores;
