 ...
 * if is_revert is set - ILKN 1/3/5 bitmap will be reversed    *  
 *                                                             */
int
soc_jer_nif_ilkn_phys_aligned_pbmp_get(int unit, soc_port_t port, soc_pbmp_t *phys_aligned, int is_revert) 
{
    ...
    SOC_PBMP_ITER(phys, phy_i){
        ...
        SOC_PBMP_PORT_ADD(*phys_aligned, ((offset & 1) && is_revert) ? JER_NIF_ILKN_MAX_NOF_LANES - phy - 1 : phy); 
    }
...
soc_jer_nif_ilkn_lane_map_get(int unit, soc_port_t port, int *ilkn_lane_map_override, int ilkn_lane_map[JER_NIF_ILKN_MAX_NOF_LANES]) 
{
    ...
    char *map_str;
    int mapping, lane, i, map_index, temp;
    ...
    *ilkn_lane_map_override = 0; /*no ovverride by default*/
    ...
    if (*ilkn_lane_map_override == 1) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_jer_nif_ilkn_phys_aligned_pbmp_get(unit, port, &ilkn_pbmp, 1));

        /* if ILKN1/3/5 mapping should be reverted as well (since aligned pmbp is reverted) */
        if (offset & 1) {
            for (lane = 0; lane < nof_lanes / 2; lane++){
                temp = ilkn_lane_map[lane];
                ilkn_lane_map[lane] = ilkn_lane_map[nof_lanes - 1 - lane];
                ilkn_lane_map[nof_lanes - 1 - lane] = temp;
            }
        }
...
soc_jer_portmod_xphy_core_config_update(int unit, int lport, int pport, int phy_idx, uint32_t xphy_addr) {
    ...
    SOCDNX_IF_ERR_EXIT(portmod_xphy_core_access_get(unit, xphy_addr, &core_access));
...
soc_jer_portmod_register_external_phys(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_MAX_NUM_PORTS; ++port_i) {
        ...
        SOCDNX_IF_ERR_EXIT(portmod_xphy_add(unit, xphy_mdio_addr, &core_access));
...
soc_jer_port_ilkn_config_get(int unit, soc_port_t port, portmod_port_add_info_t* add_info)
{
    ...
    add_info->ilkn_burst_min = soc_property_port_get(unit, offset, spn_ILKN_BURST_MIN, add_info->ilkn_burst_short);
...
soc_jer_portmod_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp, int is_init_sequence)
{
    ...
    uint32 is_master, flags, counter_flags, nof_lanes;
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        SOC_PBMP_ITER(pbmp, port) {
            ...
            if (SOC_IS_QUX(unit)) {
                ...
                SOC_PBMP_ITER(add_info.phys, phy){
                    /* phy 9-36 is in PM4x2p5 which is not implemented*/
                    if ((phy > 8) && (phy < 37)) {
                    SOC_EXIT;
                    }
                }        
            ...
            } else if (SOC_IS_QAX(unit) && add_info.interface_config.interface == SOC_PORT_IF_ILKN) {
        ...
        if(broadcast_load_fabric && (!SOC_IS_QAX(unit) || SOC_IS_QAX_WITH_FABRIC_ENABLED(unit))) {
...
soc_jer_portmod_port_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    if (dcmn_device_block_for_feature(unit, DCMN_JER_NIF_24_48_FEATURE)) {
        SOCDNX_IF_ERR_EXIT(soc_jer_portmod_port_quad_get(unit, port, &quad_bmp));
        /* Check the port's QUADs - limitations are only for QUADs for 12 to 17 */
...
soc_jer_port_ilkn_hrf_config_close_path(int unit, int port, int offset){
    ...
    if (num_lanes <= 12) {
        ...
        if (!tdm_channel_exist || shr_hrf) {
            ...
            if ((offset == 0) && (num_lanes <= 12) && (!SOC_IS_JERICHO_PLUS_ONLY(unit))) {
            ...
            if (offset > 1) {
                ...
                if (((offset & 1) == 0) && (num_lanes <= 12) && (!SOC_IS_JERICHO_PLUS_ONLY(unit))) {
        ...
        if (!close_only_tdm_path /*close full path, not just tdm*/) {
            reg = (offset < 2) ? NBIH_HRF_RX_MEM_CONFIGr : NBIL_HRF_RX_MEM_CONFIGr; 
            SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, reg, reg_port, offset & 1, 0x1)); /*HW default*/
...
soc_jer_port_ilkn_hrf_config_open_path(int unit, int port, int offset)
{
    ...
    if (num_lanes <= 12) {
        ...
        if (nof_channels > 1 && !open_tdm_path) {
        ...
        if (free_quad_for_ilkn != -1 && (!tdm_channel_exist || shr_hrf)) {
            reg = (offset < 2) ? NBIH_HRF_RX_MEM_CONFIGr : NBIL_HRF_RX_MEM_CONFIGr;
    ...
    if (SOC_PORT_IS_ELK_INTERFACE(flags)) {
        reg = (offset < 2) ? NBIH_HRF_RX_MEM_CONFIGr : NBIL_HRF_RX_MEM_CONFIGr;
        SOCDNX_IF_ERR_EXIT(soc_reg32_set(unit, reg, reg_port, offset & 1, 0));
...
soc_jer_port_open_ilkn_path(int unit, int port) {
    ...
    if (is_master) {
        ...
        * it is needed to set this fields even if only ILKN 1/3/5 is active*/ 
        if (offset < 2 && SOC_IS_JERICHO_PLUS_ONLY(unit) && (num_lanes <= 12)) {
        ...
        } else if (((offset & 1) == 0) && (num_lanes <= 12)) {
        ...
        if(offset >= 2) {
            ...
            if (SOC_IS_JERICHO_PLUS_ONLY(unit) && (num_lanes <= 12)) {
...
soc_jer_port_open_nbi_path(int unit, int port) {
    ...
    int pm_index = 0, reg_port, qmlf_index, core, nbi_inst, max_ports_nbih, max_ports_nbil, nof_lanes_nbi;
    ...
    if (!SOC_IS_QUX(unit)) {
        ...
        pm_index = (first_lane % max_ports_nbil) / NUM_OF_LANES_IN_PM;
    }
...
uint32 jer_port_link_up_mac_update(int unit, soc_port_t port, int link)
{
    ...
    if (link && !IS_SFI_PORT(unit, port) && !IS_IL_PORT(unit, port)) {
        ...
        if ((BCM_E_NONE == rv) && (an_status.enabled) && (an_status.locked)) {
            SOCDNX_IF_ERR_EXIT
                (portmod_port_interface_config_get(unit, port, &if_config));
            ...
            if (if_config.speed != serdes_speed) {
...
STATIC int soc_jer_nif_priority_quad_high_set(int unit, int core, uint32 quad)
{
    ...
    SOCDNX_INIT_FUNC_DEFS;
    if (SOC_IS_QUX(unit)) {
        prio_reg = NIF_RX_REQ_PIPE_0_HIGH_ENr;
    } else {
...
STATIC int soc_jer_nif_priority_quad_low_set(int unit, int core, uint32 quad)
{
    ...
    if (SOC_IS_QUX(unit)) {
        prio_reg = NIF_RX_REQ_PIPE_0_LOW_ENr;
    } else {
...
soc_jer_phy_nif_measure(int unit, soc_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane)
{
    ...
    *lane -= 1; /*make lane 0-based*/
    ...
    is_tscf = ((pll_type == SOC_JER_NIF_PLL_TYPE_PMH) || (SOC_IS_JERICHO_PLUS_ONLY(unit) && pll_type == SOC_JER_NIF_PLL_TYPE_PML1));
    ...
    /* VCO devider is 40 for TSCF and 20 for TSCE */
    vco_div = (is_tscf) ? 40 : 20;
    ...
    *one_clk_time_measured_remainder = ((((total_time_measured_int*10000) + (total_time_measured_remainder)) / (sync_sth_cnt * vco_div)) % 10000);
    ...
    if ((is_tscf) && (interface_type != BCM_PORT_IF_SGMII)){
        os_int = 1;
        os_remainder = 0;
    }
...
soc_jer_phy_nif_pll_div_get(int unit, soc_port_t port, soc_dcmn_init_serdes_ref_clock_t *ref_clk, int *p_div, int *n_div, int *m0_div)
{
    ...
    if (SOC_IS_QUX(unit)) {
        ...
        if (pll_type == SOC_JER_NIF_PLL_TYPE_PMH)
            ...
            *n_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_PLL_CONFIGr, reg_val,  NIF_PMH_PLL_CFG_NDIVf);
            *p_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_PDIVf);
            *m0_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_CH_0_MDIVf);
            *ref_clk = SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pmh_in;
    	} else {
            rv = READ_ECI_NIF_PMX_PLL_CONFIGr(unit, reg_val);
            SOCDNX_IF_ERR_EXIT(rv);
		
            *n_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMX_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_NDIVf);
            *p_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMX_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_PDIVf);
            *m0_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMX_PLL_CONFIGr, reg_val,NIF_PMH_PLL_CFG_CH_0_MDIVf);
            *ref_clk = SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pmh_in;
        }
#endif
    } else {
...
*/
int
soc_jer_nif_sku_restrictions(int unit, bcm_pbmp_t phy_pbmp, bcm_port_if_t interface, uint32 protocol_offset, uint32 is_kbp){
        ...
        if (dcmn_device_block_for_feature(unit,DCMN_JER_NIF_24_44_FEATURE)) {
            ...
            /*QUAD 9 is invalid*/
            if(SOC_PBMP_MEMBER(phy_pbmp, 37) ||
               SOC_PBMP_MEMBER(phy_pbmp, 41) ||
               SOC_PBMP_MEMBER(phy_pbmp, 45) ||
               SOC_PBMP_MEMBER(phy_pbmp, 49)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't mapped to Quad 9"),
                                                  soc_dev_name(unit)));
            }
        }

        if (dcmn_device_block_for_feature(unit,DCMN_JER_NIF_44_16_FEATURE)) {

            /*QUADs 6,7 and 12 are invalid*/
            if(SOC_PBMP_MEMBER(phy_pbmp, 25) ||
               ...
               SOC_PBMP_MEMBER(phy_pbmp, 88)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't mapped to Quads 6,7 and 12"),
                                                  soc_dev_name(unit)));
            }
        }

        if (dcmn_device_block_for_feature(unit,DCMN_JER_NIF_48_4_FEATURE)) {

            /*QUADs 6,7,8,9 and 10 are invalid*/
            if(SOC_PBMP_MEMBER(phy_pbmp, 25) ||
               SOC_PBMP_MEMBER(phy_pbmp, 26) ||
               SOC_PBMP_MEMBER(phy_pbmp, 27) ||
               SOC_PBMP_MEMBER(phy_pbmp, 28) ||
               ...
               SOC_PBMP_MEMBER(phy_pbmp, 68)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't mapped to Quads 6,7,8,9 and 10"),
                                                  soc_dev_name(unit)));
            }
        }

        if (dcmn_device_block_for_feature(unit,DCMN_JER_NIF_40_4_FEATURE)) {

            /*QUADs 6,7,8,9,10,12 and 13 are invalid*/
            if(SOC_PBMP_MEMBER(phy_pbmp, 25) ||
               SOC_PBMP_MEMBER(phy_pbmp, 26) ||
               SOC_PBMP_MEMBER(phy_pbmp, 27) ||
               SOC_PBMP_MEMBER(phy_pbmp, 28) ||
               ...
               SOC_PBMP_MEMBER(phy_pbmp, 92)) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't mapped to Quads 6,7,8,9,10,12 and 13"),
                                                  soc_dev_name(unit)));
            }
        }
        ...
        /* 2 ILKN ports for KBP only */
        if (dcmn_device_block_for_feature(unit,DCMN_JER_2_ILKN_PORTS_FEATURE)) {
            if (interface == SOC_PORT_IF_ILKN &&
                protocol_offset >= 2) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can use ILKN 0-1 only."),
                                                  soc_dev_name(unit)));
                } else if (interface == SOC_PORT_IF_ILKN && !is_kbp) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can use ILKN only for KBP."),
                                                  soc_dev_name(unit)));
                }
        }
