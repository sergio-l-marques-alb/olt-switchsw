...
arad_pp_flp_dbal_kaps_table_prefix_get(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 *table_prefix, uint32 *table_prefix_len)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
        ...
        num_of_dynamic_tables++; /* we have 2 tables for FIB scale */
               ...
               (table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS) || (table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_flp_dbal_kaps_table_prefix_get - kaps table_id is disabled")));
    }
    ...
    if ((SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long)) {
        ...
        num_of_dynamic_tables++; /* we have 2 tables for IPv6 FIB scale */
               ...
               (table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS) || (table_id == SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("arad_pp_flp_dbal_kaps_table_prefix_get - kaps table_id is disabled")));
    }
    ...
    */
    ...
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE:
        *table_prefix = JER_KAPS_IPV6_UC_TABLE_PREFIX;
        *table_prefix_len = JER_KAPS_TABLE_PREFIX_LENGTH;
        break;
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KAPS:
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_RPF_KAPS:
        *table_prefix = JER_KAPS_IPV6_UC_TABLE_PREFIX;
        *table_prefix_len = JER_KAPS_TABLE_PREFIX_LENGTH;
        break;
    ...
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS:
        if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
            dynamic_table_prefix++;
        }
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_LONG_KAPS:
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS:
        if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
            dynamic_table_prefix++;
        }
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_SHORT_KAPS:
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS:
        if ((SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long)) {
            dynamic_table_prefix++;
        }
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_KAPS:
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS:
        if ((SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long)) {
            dynamic_table_prefix++;
        }
    /*
    ...
    */
    ...
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_KAPS:
    case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_KAPS:
...
uint32 
    arad_pp_flp_dbal_ipv4uc_lem_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    int                     table_id;
    char*                   table_name;
    SOC_PPC_FP_QUAL_TYPE    vrf_qual_type;
    uint8                   vrf_qual_nof_bits;
    uint8                   vrf_qual_is_in_header;

    SOCDNX_INIT_FUNC_DEFS;

    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_LEM;
        table_name    = "FLP IPv4 UC VRF UPDATE LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_LEM;
        table_name    = "FLP IPv4 UC LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        qual_info[1].qual_nof_bits = vrf_qual_nof_bits;
        qual_info[1].qual_type = vrf_qual_type;
      
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_PP_FLP_IPV4_KEY_OR_MASK, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                 SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, 2, ARAD_PP_LEM_ACCESS_KEY_TYPE_IP_HOST, qual_info, table_name));
...
uint32
    arad_pp_flp_dbal_ipv4uc_lem_route_scale_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LEM;
        table_name    = "FLP IPv4 UC VRF UPDATE SCALE ROUTE LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_LEM;
        table_name    = "FLP IPv4 UC SCALE ROUTE LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        qual_info[qualifiers_counter].qual_nof_bits = vrf_qual_nof_bits;
        qual_info[qualifiers_counter++].qual_type = vrf_qual_type;

        if (vrf_update) {
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, DBAL_PREFIX_NOT_DEFINED, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, SOC_DPP_DBAL_ATI_NONE, qual_info, table_name));
        } else {
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_is_initiated(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LEM, &is_table_initiated));
            if (!is_table_initiated) {
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, DBAL_PREFIX_NOT_DEFINED, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, SOC_DPP_DBAL_ATI_NONE, qual_info, table_name));
            } else {
                SOC_DPP_DBAL_TABLE_INFO vrf_update_table;
                SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LEM, &vrf_update_table));
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, vrf_update_table.db_prefix, vrf_update_table.db_prefix_len,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, vrf_update_table.additional_table_info, qual_info, table_name));
            }
        }
    }
...
uint32
    arad_pp_flp_dbal_ipv6uc_lem_route_scale_long_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0, qualifiers_counter = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_LEM;
        table_name    = "FLP IPv6 UC VRF UPDATE SCALE ROUTE LEM LONG";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_LEM;
        table_name    = "FLP IPv6 UC SCALE ROUTE LEM LONG";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        if (vrf_update) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_is_initiated(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_LEM, &is_table_initiated));
            if (!is_table_initiated) {
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, DBAL_PREFIX_NOT_DEFINED, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, SOC_DPP_DBAL_ATI_NONE, qual_info, table_name));
            } else {
                SOC_DPP_DBAL_TABLE_INFO vrf_update_table;
                SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_LEM, &vrf_update_table));
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, vrf_update_table.db_prefix, vrf_update_table.db_prefix_len,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, vrf_update_table.additional_table_info, qual_info, table_name));
            }
        }
...
uint32
    arad_pp_flp_dbal_ipv6uc_lem_route_scale_short_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0, qualifiers_counter = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_LEM;
        table_name    =  "FLP IPv6 UC VRF UPDATE SCALE ROUTE LEM SHORT";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_SHORT_LEM;
        table_name    =  "FLP IPv6 UC SCALE ROUTE LEM SHORT";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        if (vrf_update) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_is_initiated(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_LEM, &is_table_initiated));
            if (!is_table_initiated) {
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, DBAL_PREFIX_NOT_DEFINED, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, SOC_DPP_DBAL_ATI_NONE, qual_info, table_name));
            } else {
                SOC_DPP_DBAL_TABLE_INFO vrf_update_table;
                SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.soc.arad.pp.dbal_info.dbal_tables.get(unit, SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_LEM, &vrf_update_table));
                SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, vrf_update_table.db_prefix, vrf_update_table.db_prefix_len,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, qualifiers_counter, vrf_update_table.additional_table_info, qual_info, table_name));
            }
        }
...
uint32
    arad_pp_flp_dbal_ipv4uc_rpf_lem_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_LEM;
        table_name    = "FLP IPv4 UC VRF UPDATE RPF LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_LEM;
        table_name    = "FLP IPv4 UC RPF LEM";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        qual_info[1].qual_type = vrf_qual_type;
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_PP_FLP_IPV4_KEY_OR_MASK, ARAD_PP_LEM_ACCESS_KEY_PREFIX_SIZE_IN_BITS,
                                                 SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM, 2, 0, qual_info,table_name ));
    }
...
uint32
    arad_pp_flp_dbal_ipv6uc_tcam_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE;
        table_name    = "FLP IPv6 UC VRF UPDATE TCAM";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE;
        table_name    = "FLP IPv6 UC TCAM";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    ...
    if (!is_table_initiated) {
        ...
        qual_info[5].qual_type = vrf_qual_type;
        qual_info[5].qual_offset = 0;
        qual_info[5].qual_nof_bits = 8;

        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id,
                DBAL_PREFIX_NOT_DEFINED, 0, SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM,6, SOC_DPP_DBAL_ATI_NONE, qual_info, table_name));
    }
...
uint32 
    arad_pp_flp_dbal_ipv4uc_kaps_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_KAPS;
        table_name    = "FLP IPv4 UC VRF UPDATE KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
        vrf_qual_nof_bits = 14;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;
        table_name    = "FLP IPv4 UC KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
        vrf_qual_nof_bits = 0;
    }
    ...
    if (!is_table_initiated) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, table_prefix, table_prefix_len,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS, qualifiers_counter, 0, qual_info, table_name));
    }
...
uint32
    arad_pp_flp_dbal_ipv4uc_kaps_route_scale_long_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len, qualifiers_counter = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS;
        table_name    = "FLP IPv4 UC VRF UPDATE SCALE LONG KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_LONG_KAPS;
        table_name    = "FLP IPv4 UC SCALE LONG KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
...
uint32
    arad_pp_flp_dbal_ipv4uc_kaps_route_scale_short_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len, qualifiers_counter = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS;
        table_name    = "FLP IPv4 UC VRF UPDATE SCALE SHORT KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_SHORT_KAPS;
        table_name    = "FLP IPv4 UC SCALE SHORT KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
...
uint32
    arad_pp_flp_dbal_ipv6uc_kaps_route_scale_long_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len, qualifiers_counter = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS;
        table_name    = "FLP IPv6 UC VRF UPDATE SCALE LONG KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_KAPS;
        table_name    = "FLP IPv6 UC SCALE LONG KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
...
uint32
    arad_pp_flp_dbal_ipv6uc_kaps_route_scale_short_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len, qualifiers_counter = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS;
        table_name    = "FLP IPv6 UC VRF UPDATE SCALE SHORT KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_SHORT_KAPS;
        table_name    = "FLP IPv6 UC SCALE SHORT KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, table_prefix, table_prefix_len,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS, qualifiers_counter, 0, qual_info, table_name));
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_stage_create(unit, table_id, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, 0/*lookup_number*/, 0/*program_id*/ ));
    }
...
uint32 
    arad_pp_flp_dbal_ipv4uc_rpf_kaps_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len, qualifiers_counter = 0;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_KAPS;
        table_name    = "FLP IPv4 UC VRF UPDATE RPF KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KAPS;
        table_name    = "FLP IPv4 UC RPF KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
...
uint32 
    arad_pp_flp_dbal_ipv4uc_kbp_table_create(int unit, uint8 vrf_update, int is_rpf)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_FWD_KBP;
        table_name    = "FLP IPv4 UC VRF UPDATE FWD KBP";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
        kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_NO_DB_UPDATE;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_FWD_KBP;
        table_name    = "FLP IPv4 UC FWD KBP";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
        kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY;
    }
    ...
    if (!is_table_initiated) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_KBP_FRWRD_TBL_ID_IPV4_UC_RPF_0, 0,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP, 3, kbp_table_flag, qual_info, table_name));
    }

    if (is_rpf) {
        if (vrf_update) {
            table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_KBP;
            table_name    = "FLP IPv4 UC VRF UPDATE RPF KBP";
            vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
            kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_NO_DB_UPDATE;
        } else {
            table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KBP;
            table_name    = "FLP IPv4 UC RPF KBP";
            vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
            kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY;
        }
        ...
        if (!is_table_initiated) {
		    ...
		    qual_info[2].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
			qual_info[2].qual_nof_bits = 2;
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_KBP_FRWRD_TBL_ID_IPV4_UC_RPF_1, 0,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP, 3, kbp_table_flag, qual_info, table_name));
        }
    }
...
uint32 
    arad_pp_flp_dbal_ipv6uc_kbp_table_create(int unit, uint8 vrf_update, int is_rpf)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    int                     table_id;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_FWD_KBP;
        table_name    = "FLP IPv6 UC VRF UPDATE FWD KBP";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
        kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_NO_DB_UPDATE;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_FWD_KBP;
        table_name    = "FLP IPv6 UC FWD KBP";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
        kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY;
    }
    ...
    if (!is_table_initiated) {
        ...
        qual_info[4].qual_nof_bits = vrf_qual_nof_bits;
        qual_info[4].qual_type = vrf_qual_type;
		qual_info[5].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
		qual_info[5].qual_nof_bits = 2;
        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_KBP_FRWRD_TBL_ID_IPV6_UC_RPF_0, 0,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP, 6, kbp_table_flag, qual_info, table_name));
    }

    if (is_rpf) {
        if (vrf_update) {
            table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_RPF_KBP;
            table_name    = "FLP IPv6 UC VRF UPDATE RPF KBP";
            vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
            kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_NO_DB_UPDATE;
        } else {
            table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KBP;
            table_name    = "FLP IPv6 UC RPF KBP";
            vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
            kbp_table_flag = SOC_DPP_DBAL_ATI_KBP_LOOKUP_ONLY;
        }
        ...
        if (!is_table_initiated) {
            ...
            qual_info[5].qual_type = SOC_PPC_FP_QUAL_IRPP_ALL_ZEROES;
            qual_info[5].qual_nof_bits = 2;
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, ARAD_KBP_FRWRD_TBL_ID_IPV6_UC_RPF_1, 0,
                                                         SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP, 6, kbp_table_flag, qual_info, table_name));
        }
    }
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
uint32 
    arad_pp_flp_dbal_ipv6uc_kaps_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE;
        table_name    = "FLP IPv6 UC VRF UPDATE KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE;
        table_name    = "FLP IPv6 UC KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
    ...
    if (!is_table_initiated) {
        ...
        qual_info[4].qual_type = SOC_PPC_FP_QUAL_HDR_FWD_IPV6_DIP_HIGH;
        qual_info[4].qual_nof_bits = 32;
        qual_info[5].qual_nof_bits = vrf_qual_nof_bits;
        qual_info[5].qual_type = vrf_qual_type;

        SOCDNX_IF_ERR_EXIT(arad_pp_dbal_table_create(unit, table_id, table_prefix, table_prefix_len,
                                                     SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KAPS, 6, 0, qual_info, table_name));
...
uint32 
    arad_pp_flp_dbal_ipv6uc_rpf_kaps_table_create(int unit, uint8 vrf_update)
{
    SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX];
    int is_table_initiated = 0;
    uint32 table_prefix, table_prefix_len;
    ...
    if (vrf_update) {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_RPF_KAPS;
        table_name    =  "FLP IPv6 UC VRF UPDATE RPF KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_TT_LOOKUP0_PAYLOAD;
    } else {
        table_id      = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KAPS;
        table_name    =  "FLP IPv6 UC RPF KAPS";
        vrf_qual_type = SOC_PPC_FP_QUAL_IRPP_VRF;
    }
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_qualifier_full_size_get(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, SOC_PPC_FP_QUAL_IRPP_VRF, &vrf_qual_nof_bits, &vrf_qual_is_in_header));
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_rpf_lem_table_create(unit,vrf_update));
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_lem_table_create(unit,vrf_update));

#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    if(ARAD_KBP_ENABLE_IPV4_RPF){    /* in case of KBP is enabled */        
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kbp_table_create(unit,vrf_update,1));
    }else {
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_rpf_kaps_table_create(unit,vrf_update));
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kaps_table_create(unit,vrf_update));
    }
#endif /*#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)*/

    keys_to_table_id[0].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
    keys_to_table_id[0].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_LEM;
    keys_to_table_id[0].lookup_number = 1;

    keys_to_table_id[1].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B;
    keys_to_table_id[1].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_LEM;
    keys_to_table_id[1].lookup_number = 2;
    ...
    if(ARAD_KBP_ENABLE_IPV4_RPF){    /* in case of KBP is enabled */        
        if (vrf_update) {
            ...
            keys_to_table_id[2].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_FWD_KBP;
            keys_to_table_id[2].lookup_number = 0;

            keys_to_table_id[3].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
            keys_to_table_id[3].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_KBP;
    ...
    {
        keys_to_table_id[2].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
        keys_to_table_id[2].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_RPF_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_RPF_KAPS;
        keys_to_table_id[2].lookup_number = 1;
        keys_to_table_id[2].public_lpm_lookup_size = 0;

        keys_to_table_id[3].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B_MSB;
        keys_to_table_id[3].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;
        keys_to_table_id[3].lookup_number = 2;
    ...
    arad_pp_flp_dbal_ipv4_dc_program_tables_init(int unit)
{
    ...
    /* LEM lookup */
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_lem_table_create(unit,FALSE));    
   ...
   )
{
    ...
    {
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kbp_table_create(unit, vrf_update, 0));
        if (vrf_update) {
            keys_to_table_id[1].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
            keys_to_table_id[1].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_FWD_KBP;
            keys_to_table_id[1].lookup_number = 0;
            num_of_tables = 2;
        } else{
            keys_to_table_id[1].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
            keys_to_table_id[1].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_MASTER_KBP;
            keys_to_table_id[1].lookup_number = 2;
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kaps_table_create(unit, vrf_update));
        if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_lem_route_scale_table_create(unit, vrf_update));
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kaps_route_scale_short_table_create(unit, vrf_update));
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kaps_route_scale_long_table_create(unit, vrf_update));
        } else {
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_kaps_table_create(unit, vrf_update));
        }

        keys_to_table_id[1].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
        keys_to_table_id[1].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;
        keys_to_table_id[1].lookup_number = 2;
        keys_to_table_id[1].public_lpm_lookup_size = 0;
    }
#endif /*defined(INCLUDE_KBP) && !defined(BCM_88030)*/
    keys_to_table_id[0].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
    keys_to_table_id[0].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_LEM;
    keys_to_table_id[0].lookup_number = 2;
    ...
    if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length) {
        ...
        keys_to_table_id[1].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_LONG_KAPS;
        keys_to_table_id[1].lookup_number = 1;
        keys_to_table_id[1].public_lpm_lookup_size = 0;

        keys_to_table_id[2].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
        keys_to_table_id[2].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_LEM;
        keys_to_table_id[2].lookup_number = 2;

        keys_to_table_id[3].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
        keys_to_table_id[3].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_ROUTE_SCALE_SHORT_KAPS;
        keys_to_table_id[3].lookup_number = 2;
        keys_to_table_id[3].public_lpm_lookup_size = 0;
   ...
   )
{
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_rpf_lem_table_create(unit,FALSE));
    SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_lem_table_create(unit,FALSE));
    ...
    if(ARAD_KBP_ENABLE_IPV4_MC){
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_rpf_kaps_table_create(unit,FALSE));
   ...
   )
{
    ...
    if (custom_prgrm) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv4uc_lem_table_create(unit,vrf_update)); /* lem private lookup */            
    }
    ...
    if (custom_prgrm) {
        ...
        keys_to_table_id[num_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KAPS;
        ...
        keys_to_table_id[num_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B;
        keys_to_table_id[num_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_VRF_UPDATE_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_LEM;
        keys_to_table_id[num_tables++].lookup_number = 2;
   ...
   )
{
    ...
    if (ARAD_KBP_ENABLE_IPV6_UC) {
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_kbp_table_create(unit,vrf_update,1));
        if (vrf_update) {
            keys_to_table_id[0].key_id = SOC_DPP_DBAL_PROGRAM_NOF_KEYS;
            keys_to_table_id[0].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_FWD_KBP;
            ...
            keys_to_table_id[1].sw_table_id = SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_RPF_KBP;
            keys_to_table_id[1].lookup_number = 2;
            keys_to_table_id[1].public_lpm_lookup_size = 0;
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_kaps_table_create(unit,vrf_update));
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_rpf_kaps_table_create(unit,vrf_update));

        keys_to_table_id[0].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
        keys_to_table_id[0].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_RPF_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_RPF_KAPS;
        keys_to_table_id[0].lookup_number = 1;
        ...
        keys_to_table_id[1].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B;
        keys_to_table_id[1].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE;
        keys_to_table_id[1].lookup_number = 2;
   ...
   )
{
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0))) {
        SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_tcam_table_create(unit,vrf_update));
        keys_to_table_id[num_of_tables].lookup_number = 1;
        keys_to_table_id[num_of_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_C;
        keys_to_table_id[num_of_tables++].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE;
    } else {
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
        if (ARAD_KBP_ENABLE_IPV6_UC) {
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_kbp_table_create(unit,vrf_update,0));
            if (!vrf_update) {
            ...
            keys_to_table_id[num_of_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_FWD_KBP : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_FWD_KBP;
            keys_to_table_id[num_of_tables].lookup_number = 0;
            keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;
        } else {
            uint8 is_dynamic;
            ...
            if ((SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long)) {
                SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_lem_route_scale_short_table_create(unit,vrf_update));
                SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_lem_route_scale_long_table_create(unit,vrf_update));
                /* The short table is not used for CEs, we use the long table for both KAPS searches to conserve CEs */
                SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_kaps_route_scale_short_table_create(unit,vrf_update));
                SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_kaps_route_scale_long_table_create(unit,vrf_update));

                num_of_tables = 0;

                /* Search in the special KAPS and LEM tables */
                keys_to_table_id[num_of_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A;
                keys_to_table_id[num_of_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_SHORT_LEM;
                keys_to_table_id[num_of_tables].lookup_number = 2;
                keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;

                keys_to_table_id[num_of_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_B;
                keys_to_table_id[num_of_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_LEM : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_LEM;
                keys_to_table_id[num_of_tables].lookup_number = 1;
                keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;

                keys_to_table_id[num_of_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_C;
                keys_to_table_id[num_of_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_LONG_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_LONG_KAPS;
                keys_to_table_id[num_of_tables].lookup_number = 1;
                keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;

                keys_to_table_id[num_of_tables].key_id = SOC_DPP_DBAL_PROGRAM_KEY_A_MSB;
                keys_to_table_id[num_of_tables].sw_table_id = vrf_update ? SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_VRF_UPDATE_ROUTE_SCALE_SHORT_KAPS : SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_ROUTE_SCALE_SHORT_KAPS;
                keys_to_table_id[num_of_tables].lookup_number = 2;
                keys_to_table_id[num_of_tables++].public_lpm_lookup_size = 0;
            }
        }
    ...
    SOCDNX_IF_ERR_EXIT(arad_pp_dbal_program_to_tables_associate(unit, prog_id, SOC_PPC_FP_DATABASE_STAGE_INGRESS_FLP, keys_to_table_id, NULL, num_of_tables));
   ...
   )
{
    ...
    {        
        ...
        if (ARAD_KBP_ENABLE_IPV6_MC) {
            ...
            SOCDNX_IF_ERR_EXIT(arad_pp_flp_dbal_ipv6uc_rpf_kaps_table_create(unit,FALSE));
