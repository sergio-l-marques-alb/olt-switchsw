...
soc_qax_port_open_ilkn_path(int unit, int port) {
    ...
    if (is_master) {
        ...
        if ( SOC_QAX_IS_ILKN24(offset) ) {
            /*for ILKN0/2, need to enable "small" port for <=12 lanes and all ports (0,1,2,3) for >12 lanes */
            if (num_lanes > 12) {
                /*enable all ports*/
                soc_reg_field_set(unit, NBIH_ENABLE_INTERLAKENr, &reg_val, qax_enable_ilkn_fields[0], 1);
                soc_reg_field_set(unit, NBIH_ENABLE_INTERLAKENr, &reg_val, qax_enable_ilkn_fields[1], 1);
                soc_reg_field_set(unit, NBIH_ENABLE_INTERLAKENr, &reg_val, qax_enable_ilkn_fields[2], 1);
                soc_reg_field_set(unit, NBIH_ENABLE_INTERLAKENr, &reg_val, qax_enable_ilkn_fields[3], 1);
            } else {
            ...
            field_value = ((num_lanes > 12) || (speed == 23000 && num_lanes > 10) ? 0 : 1);
        ...
        if (offset < 2) {
            /* pll configuration to allow ILKN on both PM4x25 and PM4x10 */
            SOCDNX_IF_ERR_EXIT(READ_ECI_NIF_PML_0_PLL_CONFIGr(unit, reg_above_64_val));
            soc_reg_field_set(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_above_64_val, PML_0_PLL_MSC_CTRL_VEC_REF_SELf, 0);
            soc_reg_field_set(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_above_64_val, PML_0_PLL_CTRL_VEC_ON_CHIP_REF_CMLf, 0);
            soc_reg_field_set(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_above_64_val, PML_0_PLL_CTRL_VEC_ON_CHIP_REF_SELf, 0);
            ...
            soc_reg_field_set(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_above_64_val, PML_1_PLL_MSC_CTRL_VEC_REF_SELf, 0);
            soc_reg_field_set(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_above_64_val, PML_1_PLL_CTRL_VEC_ON_CHIP_REF_CMLf, 1);
            soc_reg_field_set(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_above_64_val, PML_1_PLL_CTRL_VEC_ON_CHIP_REF_SELf, 1);
        ...
        *rx_req_tdm_en = soc_reg_field_get(unit, NBIH_RX_REQ_PIPE_0_TDM_ENr, reg_val, RX_REQ_PIPE_0_TDM_ENf);
        ...
        if (SOC_IS_QUX(unit)) {
            ...
            SOC_PBMP_ITER(quad_bmp, qmlf_index) {
                ...
                /*
                SHR_BITSET(rx_req_low_en_64, qmlf_index);
                SHR_BITSET(rx_req_low_en_64, qmlf_index + 32);
                SHR_BITSET(rx_req_low_en_64, qmlf_index + 64);
                SHR_BITSET(rx_req_low_en_64, qmlf_index + 96);     
                */
            }
        ...
        if (SOC_IS_QUX(unit)) {
            ...
            *rx_req_high_en = soc_reg_field_get(unit, reg, reg_val, RX_REQ_PIPE_0_HIGH_ENf);
            SHR_BITOR_RANGE(rx_req_high_en, rx_req_low_en, 0, 16, rx_req_high_en);
        } else {
...
soc_qax_nif_ilkn_pbmp_get(int unit, soc_port_t port, uint32 ilkn_id, soc_pbmp_t* phys, soc_pbmp_t* src_pbmp)
{
    ...
    char* propval;
    char* propkey;
    int ilkn_wrap_index, ilkn_wrap_pm, lanes_to_add;
    ...
    if((propval != NULL) || (src_pbmp != NULL)) {
        ...
        while (lanes > 0)
        {
        	...
        	SOC_PBMP_PORTS_RANGE_ADD(phy_pbmp, ilkn_wrap[ilkn_wrap_pm].phy, lanes_to_add);
...
int
soc_qax_port_prd_drop_count_get(int unit, soc_port_t port, uint32 *count)
{
    ...
    if (!IS_IL_PORT(unit,port)) {
        ...
        {    
            ...
            *count = soc_reg_field_get(unit, NBIH_PRD_PKT_DROP_CNT_HRFr, reg_val, PRD_PKT_DROP_CNT_HRFf);
        }
        else if (SOC_QAX_IS_ILKN12(offset)) 
        {   
            SOCDNX_IF_ERR_EXIT(READ_NBIL_PRD_PKT_DROP_CNT_HRF_0r(unit, nbil_reg_port, &reg_val));
            *count = soc_reg_field_get(unit, NBIL_PRD_PKT_DROP_CNT_HRF_0r, reg_val, PRD_PKT_DROP_CNT_HRF_0f);
        }
    }
...
*/
int
soc_qax_nif_sku_restrictions(int unit, bcm_pbmp_t phy_pbmp, bcm_port_if_t interface, uint32 protocol_offset, uint32 is_kbp){
       ...
       if (dcmn_device_block_for_feature(unit,DCMN_QAX_NIF_12_24_FEATURE)) {
          ...
          /*Lanes 34 and 35 of QUAD 8, QUAD 3 and QUAD 11 are invalid for QAX SKUs*/
          if(SOC_PBMP_MEMBER(phy_pbmp, 13) ||
             ...
             SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Device %s can't be mapped to lanes 34 and 35 of Quad 8, Quad 3 and Quad 11."),
                                               soc_dev_name(unit)));
          }
       }
exit:
