  ...
  )
{
    ...
    if (SOC_IS_QUX(unit) && (cal_mode == SOC_TMC_FC_CAL_MODE_ILKN_INBND)) {
  ...
  )
{
    ...
    reg = IHB_COE_CFGSr;
    for (pp_intf_id = 0; pp_intf_id < (SOC_IS_QAX(unit) ? 1 : SOC_TMC_FC_NOF_PP_INTF); pp_intf_id++) {
  ...
  )
{
    ...
    if (SOC_IS_QUX(unit) && (cal_mode_ndx == SOC_TMC_FC_CAL_MODE_ILKN_INBND)) {
        SOCDNX_IF_ERR_EXIT(SOC_E_UNAVAIL);
    }
