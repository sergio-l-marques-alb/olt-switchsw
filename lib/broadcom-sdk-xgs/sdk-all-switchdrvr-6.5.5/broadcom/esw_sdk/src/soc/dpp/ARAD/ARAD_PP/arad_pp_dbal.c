    ...
    arad_pp_dbal_sem_entry_key_build(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 entry_key[ARAD_PP_FP_TCAM_ENTRY_SIZE])
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_fp_key_value_buffer_mapping(unit, SOC_PPC_FP_DATABASE_STAGE_INGRESS_VT, table_id, ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER, 0, 1, qual_vals, entry_key, entry_mask ));
                              ...
                              int nof_qualifiers, uint32 additional_info, SOC_DPP_DBAL_QUAL_INFO  qual_info[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], char* table_name)
{
    ...
    for (i = 0; i < nof_qualifiers; i++) {
        if (qual_info[i].qual_nof_bits > DBAL_MAX_QUALIFIER_LENGTH) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal qualifier nof_bits")));}

        if (qual_info[i].qual_offset%4 != 0) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal qualifier offset, must be multiple of 4 %d"), qual_info[i].qual_offset));}

        if (qual_info[i].qual_offset > (DBAL_MAX_QUALIFIER_LENGTH*2 -1)) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("illegal qualifier offset")));}

        if ((qual_info[i].qual_type > SOC_PPC_NOF_FP_QUAL_TYPES) || (qual_info[i].qual_type == BCM_FIELD_ENTRY_INVALID)) {
    ...
    while(!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter))){
        ...
        for (i = 0; i < nof_entries; i++) {
            ...
            case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_ESEM:
                ...
                SOCDNX_SAND_IF_ERR_EXIT(arad_pp_esem_buffer_to_key(unit, &(entry_key_array[i*SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S]), &esem_key, &vsi_db_id));
                ...
                if (vsi_db_id == ARAD_PP_ESEM_KEY_VSI_DB_ID_VAL) {
                    ...
                    LOG_CLI((BSL_META_U(unit,
                                    "\tESEM key: VD: %d, VSI: %d, CV_ID %d\n"),  esem_key.key_info.vd_vsi.vd, esem_key.key_info.vd_vsi.vsi, esem_key.key_info.vd_cvid.cvid));
                }
    ...
    arad_pp_dbal_entry_add_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32 priority,  SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_tcam_kbp_lpm_route_add(unit, table.db_prefix, 0, prefix_len, elk_data, elk_ad_value, success));
    ...
    arad_pp_dbal_entry_delete_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], SOC_SAND_SUCCESS_FAILURE *success)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_tcam_kbp_lpm_route_remove(unit, table.db_prefix, prefix_len, elk_data));
    ...
    arad_pp_dbal_entry_get_kbp(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* found)
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(arad_kbp_table_size_get(unit, table.db_prefix, &table_size_in_bytes, &table_payload_in_bytes));
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_dbal_entry_key_to_kbp_buffer(unit, &table, table_size_in_bytes, qual_vals, &prefix_len, elk_data));
    SOCDNX_SAND_IF_ERR_EXIT(arad_pp_tcam_kbp_lpm_route_get(unit, table.db_prefix, prefix_len, elk_data, elk_ad_value, found));
    if (*found) {
...
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
STATIC uint32
    arad_pp_dbal_entry_get_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* found)
{
    uint32  prefix_len;
    uint8   data_bytes[JER_KAPS_KEY_BUFFER_NOF_BYTES];
    uint8   ad_bytes[JER_KAPS_AD_BUFFER_NOF_BYTES];
...
uint32
    arad_pp_dbal_iterator_get_next_lem(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    SOC_DPP_DBAL_TABLE_INFO table;
    ...
    if ((table_iterator->read_keys != NULL) && (table_iterator->read_vals != NULL) && (table_iterator->last_nof_entries != 0)) {
        ...
        SOCDNX_IF_ERR_EXIT(arad_pp_frwrd_em_dest_to_fec(unit, &table_iterator->read_vals[entry_index], (SOC_PPC_FRWRD_IPV4_HOST_ROUTE_INFO *)payload));
    ...
    arad_pp_dbal_iterator_get_next_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, uint32 flags, SOC_PPC_FP_QUAL_VAL qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], void* payload, uint32* priority, uint8* hit_bit, uint8* found)
{
    ...
    *found = FALSE;
    ...
    /* Attempt to get the entry first from the private table and then from the public */
    for (i=0; i<2; i++) {
        if (i==0) {
    ...
    arad_pp_dbal_phisycal_db_dump(int unit, int mode)
{
	...
	case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_KBP:
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
        SOCDNX_SAND_IF_ERR_EXIT(arad_kbp_print_diag_all_entries(unit, "all"));
    ...
    arad_pp_dbal_entry_dump(int unit, SOC_PPC_FP_DATABASE_STAGE stage, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL in_qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX], uint32 in_data[ARAD_PP_FP_TCAM_ENTRY_SIZE])
{
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_SEM_B:
        ...
        if (buffer_to_qualifier_mode) {            
            SOCDNX_SAND_IF_ERR_EXIT(arad_pp_fp_key_value_buffer_mapping(unit, stage, table_id, ARAD_PP_FP_KEY_BUFFER_DIRECTION_BUFFER_TO_KEY, 0, 1, qual_vals, in_data, mask));
    ...
    case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_TCAM:
        ...
        if (buffer_to_qualifier_mode) {            
            ...
            SOCDNX_SAND_IF_ERR_EXIT(arad_pp_fp_key_value_buffer_mapping(unit, stage, table_id, ARAD_PP_FP_KEY_BUFFER_DIRECTION_KEY_TO_BUFFER, 0, 1, in_qual_vals, data, mask));
        }        
    ...
    arad_pp_dbal_entry_input_validate(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id, SOC_PPC_FP_QUAL_VAL in_qual_vals[SOC_PPC_FP_NOF_QUALS_PER_DB_MAX])
{
    ...
    for (i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; i++) {
        ...
        if ((j == table.nof_qualifiers) && (in_qual_vals[i].type != -1)) {
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_arad_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {                           
    ...
    {/*KBP_2_FLP_RES      		*/ TRUE,  ARAD_IHP_ID,   4,       0,     128,    1,     128,        0},
    {/*KBP_2_FLP_ERR      		*/ TRUE,  ARAD_IHP_ID,   4,       0,       0,    0,       1,        0},
...
STATIC SOC_DPP_SIGNAL_MNGR_SIGNAL_INFO arad_pp_signal_mngr_jericho_plus_signals_table[SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_NOF_TYPES] = {
    ...
    {/*FLP_2_KBP_OPCODE         */ TRUE,  ARAD_IHP_ID,  32,       0,       7,    0,       8,        0},
    {/*FLP_2_KBP_KEY_PART1      */ TRUE,  ARAD_IHP_ID,  32,       0,     255,    8,     248,        0},
    {/*FLP_2_KBP_KEY_PART2      */ TRUE,  ARAD_IHP_ID,  32,       1,     255,    0,     256,      248},
    {/*FLP_2_KBP_KEY_PART3      */ TRUE,  ARAD_IHP_ID,  32,       2,     255,    0,     256,      504},
    {/*FLP_2_KBP_KEY_PART4      */ TRUE,  ARAD_IHP_ID,  32,       3,     255,    0,     256,      760},
    {/*FLP_2_KBP_KEY_PART5      */ TRUE,  ARAD_IHP_ID,  32,       4,       7,    0,       8,     1016},
    {/*KBP_2_FLP_RES_LSB_PART1  */ TRUE,  ARAD_IHP_ID,   4,       8,     255,  237,      19,        0},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES_LSB_PART2  */ TRUE,  ARAD_IHP_ID,   4,       9,     108,    0,     109,       19},   /*signal is taken form the flp2pmf*/
    {/*KBP_2_FLP_RES            */ TRUE,  ARAD_IHP_ID,   4,       8,     255,  237,      19,        0},   /*signal is taken form the flp2pmf*/
