...
_soc_saber2_avs_i2c_voltage_get(int unit, uint32 *val)
{
    ...
    if ((rv = soc_i2c_device(unit, dac)->driver->ioctl(unit, adc,
...
STATIC int sb2_dac_devs_init (int unit, char *dev_name)
{
    ...
    if ((dac = soc_i2c_devopen(unit, I2C_LTC3880_41,
                            flags, 0)) < 0) {
        cli_out("Could not open %s:%s\n",
                I2C_LTC3880_41, bcm_errmsg(dac));
        return SOC_E_FAIL;
    }
    soc_i2c_device(unit, dac)->driver->load(unit,dac,NULL,0);

    if ((adc = soc_i2c_devopen(unit, I2C_LTC3880_41,
                            flags, 0)) < 0) {
        cli_out("Could not open %s:%s\n",
                I2C_LTC3880_41, bcm_errmsg(dac));
       return BCM_E_FAIL;
    }
    soc_i2c_device(unit, dac)->driver->load(unit,adc,NULL,0);
...
{    
    ...
    if (sb2_dac_devs_init(0, I2C_MUX_3) < 0) {
...
soc_saber2_avs_init(int unit)
{
    ...
    _sb2_avs_functions.init = &_soc_saber2_avs_info_init;
    _sb2_avs_functions.deinit = &_soc_saber2_avs_info_deinit;
