...
_soc_alpm_ctrl_t soc_alpm_control[SOC_MAX_NUM_DEVICES];
...
static int soc_th_alpm_lpm_insert(int u, void *entry_data, int *index);
...
static int
_soc_th_alpm_move_inval(int u, soc_mem_t mem, int v6,
                        alpm_mem_prefix_array_t *pa, int *new_index);
...
soc_alpm_ctrl_init(int u)
{
    ...
    ALPM_CTRL(u).force_aux_op = soc_property_get(u, "l3_alpm_force_aux_op", 1);
...
_soc_th_alpm_hit_bits_sw_update(int u, alpm_mem_prefix_array_t *pa,
                                int *ori_idx, int *new_idx, int *match_num)
{
    int rv = SOC_E_NONE;
    int i, vrf, ipv6, count;
    payload_t *pfx = NULL;
    trie_t *pivot = NULL;
    trie_bpm_cb_user_data_t user_data;
                             ...
                             int key_index)   /* key location */
{
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        SOC_IF_ERROR_RETURN(_soc_th_alpm_aux_op(u,
            ...
            &aux_entry, TRUE, &hit, &tcam_index, &log_bkt));
...
{
    int rv = SOC_E_NONE;
    int i, lp, loops, idmin, idcnt, count;
    int index;
    defip_aux_hitbit_update_entry_t hent;
    soc_mem_t mem = L3_DEFIP_AUX_HITBIT_UPDATEm;
    int index_cnt = soc_mem_index_count(u, mem);

    count = pa->count;
    loops = (count + index_cnt - 1) / index_cnt;
    for (lp = 0; lp < loops; lp++) {
        index = 0;
        idmin = lp * index_cnt;
        idcnt = (count > (lp + 1) * index_cnt) ? (lp + 1) * index_cnt : count;

        for (i = idmin; i < idcnt; i++) {
            index = i % index_cnt;
            sal_memset(&hent, 0, sizeof(hent));
            if (ori_hptr[i] <= 0) {
                soc_mem_field32_set(u, mem, &hent, VALIDf, 0);
            } else {
                soc_mem_field32_set(u, mem, &hent, VALIDf, 1);
                soc_mem_field32_set(u, mem, &hent, HITBIT_PTRf,
                                    ori_hptr[i]);
                soc_mem_field32_set(u, mem, &hent, REPLACEMENT_PTRf,
                                    new_hptr[i]);
            }
            SOC_IF_ERROR_RETURN(
                soc_mem_write(u, mem, MEM_BLOCK_ANY, index, &hent));
        }

        /* Reset trailing entries */
        for (i = (index + 1); i < index_cnt; i++) {
            sal_memset(&hent, 0, sizeof(hent));
            SOC_IF_ERROR_RETURN(
                soc_mem_write(u, mem, MEM_BLOCK_ANY, i, &hent));
        }
    ...
    for (lp = 0; lp < loops; lp++) {
        ...
        for (i = idmin; i < idcnt; i++) {
            ...
            if (ori_hptr[i] <= 0) {
                ...
                soc_mem_field32_set(u, mem, &hent, VALIDf, 1);
                soc_mem_field32_set(u, mem, &hent, HITBIT_PTRf,
                                    _soc_th_alpm_rpf_entry(u, ori_hptr[i]));
                soc_mem_field32_set(u, mem, &hent, REPLACEMENT_PTRf,
                                    _soc_th_alpm_rpf_entry(u, new_hptr[i]));
            }
            SOC_IF_ERROR_RETURN(
                soc_mem_write(u, mem, MEM_BLOCK_ANY, index, &hent));
        }
...
int
soc_th_alpm_hit_bits_update(int u, alpm_mem_prefix_array_t *pa,
                            int *ori_idx, int *new_idx)
{
    int rv = SOC_E_NONE;
    int cb_cnt = 1;
    ...
    if (ALPM_CTRL(u).hit_idx_upd) {
        rv = _soc_th_alpm_hit_bits_sw_update(u, pa, ori_idx, new_idx, &cb_cnt);
    }

    if ((SOC_SUCCESS(rv) && cb_cnt > 0)  || ALPM_CTRL(u).force_aux_op) {
        rv = _soc_th_alpm_hit_bits_hw_update(u, pa, ori_idx, new_idx);
    }
...
_soc_th_alpm_bucket_link(int u, int v6, int pivot_index, int log_bkt)
{
    ...
    rv = soc_mem_read(u, lpm_mem, MEM_BLOCK_ANY, lpm_index, &lpm_entry);
    SOC_IF_ERROR_RETURN(rv);
    if ((!v6) && (pivot_index & 1)) {
    ...
    if (SOC_URPF_STATUS_GET(u)) {
        ...
        rv = READ_L3_DEFIPm(u, MEM_BLOCK_ANY, _urpf_index, &lpm_entry);
        SOC_IF_ERROR_RETURN(rv);
...
/* Move all entries from one logical bucket to another, and update
 * related bookeeping info
 */
static int
_soc_th_alpm_bucket_move(int u, soc_mem_t mem, int log_bkt_from, int log_bkt_to)
{
    int         *new_index = NULL, *old_index = NULL;
    int         rv = SOC_E_NONE, rv2, i, idx_sz;
    int         key_index, pivot_index;
    ...
    alpm_mem_prefix_array_t *pa = NULL;

    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    defip_alpm_ipv4_1_entry_t alpmv4_entry, alpmv4_sip_entry;
    defip_alpm_ipv6_64_1_entry_t alpmv6_entry, alpmv6_sip_entry;

    int         phy_bkt_from, sub_bkt_from, sub_bkt_to;

#define _ALPM_IF_ERROR_GOTO_FREE(_rv) \
    do { if ((_rv) < 0) { goto _free_exit; } } while(0)

    idx_sz = sizeof(int) * MAX_PREFIX_PER_BUCKET;
    new_index = sal_alloc(idx_sz, "new_index_move");
    old_index = sal_alloc(idx_sz, "old_index_move");
    pa = sal_alloc(sizeof(alpm_mem_prefix_array_t), "prefix_array");
    if (new_index == NULL || old_index == NULL || pa == NULL) {
        rv = SOC_E_MEMORY;
        goto _free_exit;
    }
    ...
    is_v6 = (L3_DEFIP_ALPM_IPV6_64m == mem ||
             L3_DEFIP_ALPM_IPV6_64_1m == mem ||
             L3_DEFIP_ALPM_IPV6_128m == mem);

    SOC_ALPM_ENTRY_BUF(is_v6, mem, bufp, alpmv4_entry, alpmv6_entry);
    SOC_ALPM_ENTRY_BUF(is_v6, mem, sip_bufp, alpmv4_sip_entry, alpmv6_sip_entry);

    if (!SOC_ALPM_BU_SUB_BKT_IS_SET(u, phy_bkt_from, sub_bkt_from)) {
        rv = SOC_E_INTERNAL;
        goto _free_exit;
    }
    pivot_index = SOC_ALPM_BU_PIVOT_IDX(u, phy_bkt_from, sub_bkt_from);
    trie = PIVOT_BUCKET_TRIE(ALPM_TCAM_PIVOT(u, pivot_index));
    if (SOC_ALPM_BU_GLOBAL(u, phy_bkt_from)) {
        vrf = SOC_VRF_MAX(u) + 1;
    }
    bank_disable = soc_th_alpm_bank_dis(u, vrf);
    ...
    if (mem == L3_DEFIP_ALPM_IPV6_128m) {
        pa->v6_key = L3_DEFIP_MODE_128;
    }
    pa->vrf_id = PIVOT_BUCKET_VRF(ALPM_TCAM_PIVOT(u, pivot_index));
    rv = trie_traverse(trie, _soc_th_alpm_mem_prefix_array_cb,
                       pa, _TRIE_INORDER_TRAVERSE);
    _ALPM_IF_ERROR_GOTO_FREE(rv);
     ...
     */
    sal_memset(new_index, -1, idx_sz);
    sal_memset(old_index, -1, idx_sz);
    ...
    for (i = 0; i < pa->count; i ++) {
        ...
        if (pfx->index > 0) {
            ...
            if (SOC_URPF_STATUS_GET(u)) {
                rv = soc_mem_read_no_cache(u, 0, mem, 0, MEM_BLOCK_ANY,
                        _soc_th_alpm_rpf_entry(u, pfx->index), sip_bufp);
                if (SOC_FAILURE(rv)) {
                    goto _rollback;
                }
                soc_mem_field32_set(u, mem, sip_bufp, SUB_BKT_PTRf, sub_bkt_to);
            }
            ...
            old_index[i] = pfx->index;
        }
    } /* for */

    /* Connect TCAM entry to new bucket */
    rv = _soc_th_alpm_bucket_link(u, is_v6, pivot_index, log_bkt_to);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_th_alpm_bucket_link failed, pivot %d log_bkt %d\n"),
                              pivot_index, log_bkt_to));
        goto _rollback;
    }

    /* Invalide old entries */
    rv = _soc_th_alpm_move_inval(u, mem, is_v6, pa, new_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_th_alpm_move_inval failed\n")));
        goto _rollback;
    }

    /* Update hit bits */
    rv = soc_th_alpm_hit_bits_update(u, pa, old_index, new_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
    ...
    /* Update TCAM mgmt info */
    PIVOT_BUCKET_INDEX(ALPM_TCAM_PIVOT(u, pivot_index)) = log_bkt_to;
    if (pa->count == 0){
        /* When the original bucket does not contain any entries */
        SOC_ALPM_BU_SUB_BKT_CLR(u, ALPM_PHY_BKT(u, log_bkt_from), ALPM_SUB_BKT(u, log_bkt_from));
        SOC_ALPM_BU_PIVOT_IDX(u, ALPM_PHY_BKT(u, log_bkt_from), ALPM_SUB_BKT(u, log_bkt_from)) = 0;
        SOC_ALPM_BU_SUB_BKT_SET(u, ALPM_PHY_BKT(u, log_bkt_to), ALPM_SUB_BKT(u, log_bkt_to));
        SOC_ALPM_BU_PIVOT_IDX(u, ALPM_PHY_BKT(u, log_bkt_to), ALPM_SUB_BKT(u, log_bkt_to)) = pivot_index;
    } else {
        soc_th_alpm_bucket_usage_decr(u, log_bkt_from, TRUE, mem, pa->count);
        soc_th_alpm_bucket_usage_incr(u, log_bkt_to, pivot_index, vrf, mem, pa->count);
    }

_rollback:
    if (SOC_FAILURE(rv)) {
        /* Connect TCAM back to original bucket */
        rv2 = _soc_th_alpm_bucket_link(u, is_v6, pivot_index, log_bkt_from);
        _ALPM_IF_ERROR_GOTO_FREE(rv2);

        /* Delete entry in new bucket */
        for (i = 0; i < pa->count; i ++) {
            if (new_index[i] == -1) {
                continue;
            }
...
STATIC int
_soc_th_alpm_physical_bucket_merge(int u, soc_mem_t mem, int phy_bkt_from, int phy_bkt_to)
{
    ...
    for (sub_bkt_from = 0; sub_bkt_from < SOC_TH_MAX_SUB_BUCKETS; sub_bkt_from++) {
        ...
        for (sub_bkt_to = 0; sub_bkt_to < SOC_TH_MAX_SUB_BUCKETS; sub_bkt_to++) {
            ...
            if (mem == L3_DEFIP_ALPM_IPV6_128m) {
                rv = _soc_th_alpm_128_bucket_move(u, mem, ALPM_LOG_BKT(phy_bkt_from, sub_bkt_from),
                                                  ALPM_LOG_BKT(phy_bkt_to, sub_bkt_to));
            } else {
                rv = _soc_th_alpm_bucket_move(u, mem, ALPM_LOG_BKT(phy_bkt_from, sub_bkt_from),
                                              ALPM_LOG_BKT(phy_bkt_to, sub_bkt_to));
            }
            SOC_IF_ERROR_RETURN(rv);
            break;
    ...
    _alpm_dbg_cntr[u].bkt_merge++;

    return SOC_E_NONE;
}

STATIC int
_soc_th_alpm_can_physical_buckets_merge(int u, int phy_bkt1, int phy_bkt2)
{
...
/* Try to allocate a new logic bucket by merging or carving out a piece */
static int
_soc_th_alpm_bucket_assign(int u, soc_mem_t mem, int *log_bkt)
{
    ...
    /* We are here because all physical buckets are assigned.
     * We now can either :
     *      a) merge two physical buckets into one bucket to free the other, or
     *      b) assign a logic bucket from the least occupied physical bucket.
     */


    /* Find two quailfied physical buckets and merge */
        ...
        for (cnt = 1; cnt <= (bkt_cnt / 2); cnt++) {
            ...
            for (phy_bkt = 1; phy_bkt < SOC_TH_ALPM_MAX_BKTS(u); phy_bkt++) {
                if (SHR_BITGET(bkt_bmp, phy_bkt) &&
                    ...
                    if (phy_bkt1 == 0) {
                        phy_bkt1 = phy_bkt;
                    } else if (_soc_th_alpm_can_physical_buckets_merge(u, phy_bkt1, phy_bkt)) {
                        phy_bkt2 = phy_bkt;
                        break;
                    }
                }
            ...
            for (cnt2 = cnt + 1;
                ...
                for (phy_bkt = 1; phy_bkt < SOC_TH_ALPM_MAX_BKTS(u); phy_bkt++) {
                    if (SHR_BITGET(bkt_bmp2, phy_bkt) &&
                        ...
                        if (_soc_th_alpm_can_physical_buckets_merge(u, phy_bkt1, phy_bkt)) {
                            phy_bkt2 = phy_bkt;
                            break;
                        }
                    }
            ...
            if (phy_bkt2) {
                 ...
                 */
                SOC_IF_ERROR_RETURN(
                    _soc_th_alpm_physical_bucket_merge(u, alpm_mem[view],
                                                      phy_bkt1, phy_bkt2));
    ...
    /* Cannot merge two physical buckets into one.
     * Try to alloc a sub bucket from least occupied bucket of
     * identical memory view.
     */
    bkt_cnt = _soc_th_alpm_bkt_entry_cnt(u, mem);
    /* We need (bkt_cnt - 1) here because only 1 available entry is not
     * enough for _trie_v6_splitable / _trie_splitable
     *
     * E.g. If available(phy_bkt) == 1, but split_trie->count == 2. We may get a
     * SOC_E_FULL from insert_bkt.
     */
    ...
    for (cnt = 1; cnt < bkt_cnt - 1; cnt++) {
        ...
        if (phy_bkt < SOC_TH_ALPM_MAX_BKTS(u)) {
            for (cnt2 = 0; cnt2 < SOC_TH_MAX_SUB_BUCKETS; cnt2++) {
                if (!SOC_ALPM_BU_SUB_BKT_IS_SET(u, phy_bkt, cnt2)) {
                    *log_bkt = ALPM_LOG_BKT(phy_bkt, cnt2);
                    return SOC_E_NONE;
                }
            }
        }
...
/* Update bucket usage bookeeping info */
void
soc_th_alpm_bucket_usage_decr(int u, int log_bkt, int clear, soc_mem_t mem, int diff)
{
    int phy_bkt, sub_bkt;
    int count, view;
...
static int _soc_th_alpm_update_bpm_len(int u, int new_tcam_index, int v6,
                                    int bpm_len)
{
    int rv, tmp0, tmp1, index;
    defip_aux_table_entry_t entry;
    soc_mem_t aux_mem = L3_DEFIP_AUX_TABLEm;

    index = new_tcam_index >> (v6 ? 0 : 1);
    rv = soc_mem_read(u, aux_mem, MEM_BLOCK_ANY, index, &entry);
    SOC_IF_ERROR_RETURN(rv);
    if (v6) {
        soc_mem_field32_set(u, aux_mem, &entry, BPM_LENGTH0f, bpm_len);
        soc_mem_field32_set(u, aux_mem, &entry, BPM_LENGTH1f, bpm_len);
    } else {
        if (new_tcam_index & 1) {
            soc_mem_field32_set(u, aux_mem, &entry, BPM_LENGTH1f, bpm_len);
        } else {
    ...
    tmp0 = soc_mem_field32_get(u, aux_mem, &entry, DB_TYPE0f);
    tmp1 = soc_mem_field32_get(u, aux_mem, &entry, DB_TYPE1f);

    /* write entry back */
    rv = soc_mem_write(u, aux_mem, MEM_BLOCK_ANY, index, &entry);
    SOC_IF_ERROR_RETURN(rv);
    ...
    if (SOC_URPF_STATUS_GET(u)) {
        ...
        soc_mem_field32_set(u, aux_mem, &entry, DB_TYPE0f, tmp0 + 1);
        soc_mem_field32_set(u, aux_mem, &entry, DB_TYPE1f, tmp1 + 1);

        index += (2*soc_mem_index_count(u, L3_DEFIP_PAIR_128m) +
                        ...
                        alpm_mem_prefix_array_t *pa, int *new_index)
{
    ...
    defip_alpm_ipv4_1_entry_t alpmv4_entry;
    defip_alpm_ipv6_64_1_entry_t alpmv6_entry;
    int entsz, sz;
     ...
     */
    entsz = v6 ? sizeof(alpmv6_entry) : sizeof(alpmv4_entry);
    sz = entsz * pa->count;
    rb_bufp = sal_alloc(sz, "rb_bufp");
    rb_sip_bufp = sal_alloc(sz, "rb_sip_bufp");
    rb_index = sal_alloc(sizeof(*rb_index) * pa->count, "roll_back_index");
    if (rb_bufp == NULL || rb_sip_bufp == NULL || rb_index == NULL) {
        rv = SOC_E_MEMORY;
        goto _rb_free;
    }

    sal_memset(rb_index, -1, sizeof(*rb_index) * pa->count);

    for (i = 0; i < pa->count; i ++) {
        payload_t *prefix = pa->prefix[i];

        if (prefix->index >= 0) {
            rv = soc_mem_read_no_cache(u, 0, mem, 0, MEM_BLOCK_ANY, prefix->index,
                                       (uint8 *)rb_bufp + i * entsz);
    ...
    if (rollback) {
        for (; i >= 0; i--) {
            ...
            rv2 = soc_mem_write(u, mem, MEM_BLOCK_ALL,
                                _soc_th_alpm_rpf_entry(u, rb_index[i]),
                                (uint8 *)rb_sip_bufp + i * entsz);
            if (SOC_FAILURE(rv2)) {
                break;
            }
...
void
_soc_th_alpm_rollback_pivot_add(int u, int v6, defip_entry_t *plpm_entry,
                                void *key_data, int tcam_index,
                                alpm_pivot_t *pivot_pyld)
{
    ...
    (void)soc_th_alpm_lpm_vrf_get(u, key_data, &vrf_id, &vrf);

    /* delete new pivot */
    /* free pivot lpm entry */
    rv = soc_th_alpm_lpm_delete(u, plpm_entry);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "_soc_alpm_insert: Failure to free new prefix"
                              "at %d\n"), soc_th_alpm_logical_idx(u, L3_DEFIPm,
                              tcam_index, v6)));
    }
    ...
    if (ALPM_TCAM_PIVOT(u, tcam_index << (v6 ? 1 : 0)) != NULL) {
    ...
    ALPM_TCAM_PIVOT(u, tcam_index << (v6 ? 1 : 0)) = NULL;
}

int
_soc_th_alpm_rollback_bkt_move(int u, int v6, void *key_data, soc_mem_t mem,
                  alpm_pivot_t *old_pivot_pyld, alpm_pivot_t *new_pivot_pyld,
                  alpm_mem_prefix_array_t *p_prefix_array,
                  int *new_index, int new_insert)
{
    trie_node_t *delp = NULL;
    uint32      prefix[5], length;
    trie_t      *pfx_trie;
    int         vrf, vrf_id, i, default_route = 0;
    defip_alpm_ipv4_1_entry_t alpmv4_entry;
    defip_alpm_ipv6_64_1_entry_t alpmv6_entry;
    defip_alpm_ipv6_128_entry_t alpmv6_128_entry;
    ...
    bucket_handle = PIVOT_BUCKET_HANDLE(new_pivot_pyld);
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "prefix create failed\n")));
        return rv;
    }

    /* Assign entry buf based on table being used */
    if (mem == L3_DEFIP_ALPM_IPV6_128m) {
        SOC_ALPM_ENTRY_BUF_128(v6, mem, bufp, 0, alpmv6_128_entry);
        (void)soc_th_alpm_128_lpm_vrf_get(u, key_data, &vrf_id, &vrf);
    } else {
    ...
    for (i = 0; i < p_prefix_array->count; i ++) {
        payload_t *pfx = p_prefix_array->prefix[i];
        ...
        if (new_index[i] != -1) {
            ...
            rv = soc_mem_write(u, mem, MEM_BLOCK_ANY, new_index[i], bufp);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
            ...
            if (SOC_URPF_STATUS_GET(u)) {
                rv = soc_mem_write(u, mem, MEM_BLOCK_ANY,
                                    _soc_th_alpm_rpf_entry(u, new_index[i]), bufp);
                if (SOC_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_SOC_ALPM,
                              (BSL_META_U(u,
        ...
        delp = NULL;
        rv = trie_delete(PIVOT_BUCKET_TRIE(new_pivot_pyld), pfx->key, pfx->len, &delp);
        ...
        if (pfx->index > 0) {
            rv = trie_insert(PIVOT_BUCKET_TRIE(old_pivot_pyld), pfx->key,
                              NULL, pfx->len, (trie_node_t *) payload);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
    ...
    if (new_insert == -1) {
        ...
        rv = trie_delete(PIVOT_BUCKET_TRIE(old_pivot_pyld), prefix,
                          length, &delp);
        payload = (payload_t *)delp;
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                           "Expected to clear prefix"
    ...
    rv = soc_th_alpm_bucket_release(u, PIVOT_BUCKET_INDEX(new_pivot_pyld), vrf, v6);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                      "New bucket release failure: %d\n"),
                      PIVOT_BUCKET_INDEX(new_pivot_pyld)));
    }
    /* delete new bucket trie */
    trie_destroy(PIVOT_BUCKET_TRIE(new_pivot_pyld));
    sal_free(bucket_handle);
    sal_free(new_pivot_pyld);
    sal_free(new_index);
...
int _soc_th_alpm_free_pfx_trie(int u, trie_t *pfx_trie, trie_t *bkt_trie,
                               payload_t *new_pfx_pyld, int *new_index,
                               int log_bkt, int vrf, int v6)
{
    int rv = SOC_E_NONE;
    trie_node_t *delp = NULL;
    payload_t   *tmp1 = NULL;
    ...
    (void)trie_delete(pfx_trie, new_pfx_pyld->key, new_pfx_pyld->len, &delp);
    tmp2 = (payload_t *)delp;
    if (tmp2 != NULL) {
        (void)trie_delete(bkt_trie, tmp2->key, tmp2->len, &delp);
        tmp1 = (payload_t *)delp;
        if (tmp1 != NULL) {
            sal_free(tmp1);
        }
    ...
    if (log_bkt != -1) {
        ...
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "unit %d Unable to free bucket_id %d"),
                   u, log_bkt));
        }
    }
...
static int
_soc_th_alpm_init_pivot(int u, int v6, alpm_pfx_info_t *pi, trie_t *pfx_trie,
                        uint32 *pivot, uint32 length,
                        trie_node_t *split_trie_root, defip_entry_t *lpm_entry,
                        uint32 *bpm_len, uint32 *bpm)
{
    trie_node_t *lpmp = NULL;
    defip_alpm_ipv4_1_entry_t alpmv4_entry;
    defip_alpm_ipv6_64_1_entry_t alpmv6_entry;
    ...
    pivot_pyld_new = pi->pivot_pyld;
    old_tcam_index = pivot_pyld_new->tcam_index;

    /* Get table memory. */
    flex = VRF_FLEX_IS_SET(u, pi->vrf_id, pi->vrf, v6, 1);
    ...
    if (tmp_pyld->bkt_ptr) {
        ...
        if (tmp_pyld->bkt_ptr == pi->new_pfx_pyld) {
            /* this means the new route is the bucket's default */
            sal_memcpy(bufp, pi->alpm_data, v6 ?
        ...
        rv = _soc_th_alpm_lpm_ent_init(u, bufp, mem, v6, pi->vrf_id, pi->log_bkt,
        ...
        *bpm_len = ((payload_t *)(tmp_pyld->bkt_ptr))->len;
        if (ALPM_CTRL(u).trie_propagate) {
            (void)trie_bpm_mask_get(pfx_trie,
                    ((payload_t *)(tmp_pyld->bkt_ptr))->key, *bpm_len, bpm);
        }
    } else {
         ...
         */
        rv = soc_mem_read(u, L3_DEFIPm, MEM_BLOCK_ANY,
                   soc_th_alpm_logical_idx(u, L3_DEFIPm, old_tcam_index >> 1, 1),
                   lpm_entry);
        if ((!v6) && (old_tcam_index & 1)) {
            /* move entry to 0 half */
            rv = soc_th_alpm_lpm_ip4entry1_to_0(u, lpm_entry, lpm_entry, 0);
        }
        /* Need to reset ALG_HIT_IDX0 to 0 */
        soc_mem_field32_set(u, L3_DEFIPm, lpm_entry, ALG_HIT_IDX0f, 0);
    }
    ...
    if (v6) {
        rv = trie_init(_MAX_KEY_LEN_144_, &PIVOT_BUCKET_TRIE(pivot_pyld_new));
    } else {
        rv = trie_init(_MAX_KEY_LEN_48_, &PIVOT_BUCKET_TRIE(pivot_pyld_new));
    }
    PIVOT_BUCKET_TRIE(pivot_pyld_new)->trie = split_trie_root;
    PIVOT_BUCKET_INDEX(pivot_pyld_new) = pi->log_bkt;
    PIVOT_BUCKET_VRF(pivot_pyld_new) = pi->vrf;
    ...
    _soc_th_alpm_lpm_ent_key_init(u, pivot, length, pi->vrf, v6,
                             lpm_entry, 0);

    SOC_MEM_OPT_F32_SET(u, L3_DEFIPm, lpm_entry, ALG_BKT_PTR0f,
                        ALPM_PHY_BKT(u, pi->log_bkt));
    SOC_MEM_OPT_F32_SET(u, L3_DEFIPm, lpm_entry, ALG_SUB_BKT_PTR0f,
                        ALPM_SUB_BKT(u, pi->log_bkt));

    /* This is returned to caller */
    pi->pivot_pyld = pivot_pyld_new;
    return rv;
}

static int
_soc_th_alpm_bucket_split(int u, int v6, alpm_pfx_info_t *pi, int *new_tcam_index,
                          int *index_ptr)
{
    ...
    void        *bufp, *sip_bufp;
    alpm_pivot_t *old_pivot = pi->pivot_pyld;
    defip_entry_t lpm_entry;
    soc_mem_t   mem;
    trie_t      *pivot_trie = NULL;
    alpm_mem_prefix_array_t prefix_array;
    int         *new_index = NULL;
    int         *old_index = NULL;
    int         rv = SOC_E_NONE, i, new_insert = -1;
    int         tcam_index, log_bkt_old = 0;
    int         old_bkt_cnt = 0, new_bkt_cnt = 0;

    /* Get table memory. */
    SOC_ALPM_TABLE_MEM(u, v6, mem, VRF_FLEX_IS_SET(u, pi->vrf_id, pi->vrf, v6, 1));
    SOC_ALPM_ENTRY_BUF(v6, mem, bufp, alpmv4_entry, alpmv6_entry);
    SOC_ALPM_ENTRY_BUF(v6, mem, sip_bufp, alpmv4_sip_entry, alpmv6_sip_entry);

    bank_disable = soc_th_alpm_bank_dis(u, pi->vrf);

    /* prefix trie */
    if (v6) {
        pfx_trie = VRF_PREFIX_TRIE_IPV6(u, pi->vrf);
    } else {
        pfx_trie = VRF_PREFIX_TRIE_IPV4(u, pi->vrf);
    }

    trie = PIVOT_BUCKET_TRIE(pi->pivot_pyld);
    log_bkt_old = pi->log_bkt;

    /* Split the bucket, find a pivot */
    /* Insert PIVOT and assign a bucket, move entries to new bucket
     * and insert the entry
     */
    rv = soc_th_alpm_bucket_assign(u, &pi->log_bkt, pi->vrf, v6);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "Unable to allocate "
        ...
        pi->log_bkt = -1;
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   new_index, pi->log_bkt, pi->vrf, v6);
        return rv;
    }
     ...
     *
     * When we reach this condition, means
     * a). We don't have enougth physical bucket.
     * b). all bucket util > 50%.
     */
    i  = _soc_th_alpm_bkt_entry_cnt(u, mem);
    i -= SOC_ALPM_BU_COUNT(u, ALPM_PHY_BKT(u, pi->log_bkt));

    /* make sure alloced bucket is freed on all subsequent errors */
    rv = trie_split(trie, v6 ? _MAX_KEY_LEN_144_ : _MAX_KEY_LEN_48_, FALSE,
                    pivot, &length, &split_trie_root, NULL, FALSE, i);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
                              "Could not split bucket")));
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie,
                                   pi->new_pfx_pyld, new_index,
                                   pi->log_bkt, pi->vrf, v6);
        return rv;

    }

    rv = _soc_th_alpm_init_pivot(u, v6, pi, pfx_trie, pivot, length,
                                 split_trie_root, &lpm_entry, &bpm_len, bpm);
    if (rv != SOC_E_NONE) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
    ...
    sal_memset(&prefix_array, 0, sizeof(prefix_array));
    prefix_array.v6_key = v6;
    prefix_array.vrf_id = PIVOT_BUCKET_VRF(pi->pivot_pyld);
    /* Move prefix enteries(from new trie) to new pivot */
    rv = trie_traverse(PIVOT_BUCKET_TRIE(pi->pivot_pyld),
                       _soc_th_alpm_mem_prefix_array_cb, &prefix_array,
                       _TRIE_INORDER_TRAVERSE);

    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "Bucket split failed")));
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   new_index, pi->log_bkt, pi->vrf, v6);
        return rv;
     ...
     * - delete entries from old bucket
     */
    new_index = sal_alloc(sizeof(*new_index)*prefix_array.count, "new_index");
    if (new_index == NULL) {
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   new_index, pi->log_bkt, pi->vrf, v6);
        return SOC_E_MEMORY;
    ...
    old_index = sal_alloc(sizeof(*old_index)*prefix_array.count, "new_index");
    if (old_index == NULL) {
        _soc_th_alpm_free_pfx_trie(u, pfx_trie, trie, pi->new_pfx_pyld,
                                   new_index, pi->log_bkt, pi->vrf, v6);
        return SOC_E_MEMORY;
    ...
    sal_memset(old_index, -1, sizeof(*old_index) * prefix_array.count);
    ...
    for (i = 0; i < prefix_array.count; i ++) {
         ...
         * bucket, no point reading */
            ...
            if (SOC_FAILURE(rv)) {
                ...
                (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data,
                                                     mem, old_pivot,
                                                     pi->pivot_pyld,
                                                     &prefix_array,
                                                     new_index, new_insert);
                sal_free(old_index);
                return rv;
            ...
            soc_mem_field32_set(u, mem, bufp, SUB_BKT_PTRf,
                                ALPM_SUB_BKT(u, pi->log_bkt));
            soc_mem_field32_set(u, mem, sip_bufp, SUB_BKT_PTRf,
                                ALPM_SUB_BKT(u, pi->log_bkt));

            /* entry shouldn't exist, insert the entry into the RAM */
            rv = _soc_th_alpm_insert_in_bkt(u, mem, pi->log_bkt,
                                            bank_disable, bufp, e,
                                            &key_index, v6);
            if (SOC_SUCCESS(rv)) {
                if (SOC_URPF_STATUS_GET(u)) {
            ...
            soc_mem_field32_set(u, mem, pi->alpm_data, SUB_BKT_PTRf,
                                ALPM_SUB_BKT(u, pi->log_bkt));
            rv = _soc_th_alpm_insert_in_bkt(u, mem, pi->log_bkt,
                                            bank_disable, pi->alpm_data, e,
                                            &key_index, v6);
            if (SOC_SUCCESS(rv)) {
                new_insert = i;
                *index_ptr = key_index;
                ...
                if (SOC_URPF_STATUS_GET(u)) {
                    soc_mem_field32_set(u, mem, pi->alpm_sip_data, SUB_BKT_PTRf,
                                        ALPM_SUB_BKT(u, pi->log_bkt));
                    rv = soc_mem_write(u, mem, MEM_BLOCK_ANY,
                                       _soc_th_alpm_rpf_entry(u, key_index),
                                       pi->alpm_sip_data);
        ...
        if (SOC_FAILURE(rv)) {
            ...
            LOG_ERROR(BSL_LS_SOC_ALPM,
                      (BSL_META_U(u,
                       ...
                       prefix_array.prefix[i]->key[2],
                       prefix_array.prefix[i]->key[3],
                       prefix_array.prefix[i]->key[4], pi->log_bkt));
            (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data, mem,
                                                  old_pivot, pi->pivot_pyld,
                                                  &prefix_array, new_index,
                                                  new_insert);
            sal_free(old_index);
            return rv;
        }
    ...
    rv = soc_th_alpm_lpm_insert(u, &lpm_entry, new_tcam_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u,
        ...
        if (rv == SOC_E_FULL) {
            VRF_PIVOT_FULL_INC(u, pi->vrf, v6);
        }
        (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data, mem,
                                              old_pivot, pi->pivot_pyld,
                                              &prefix_array, new_index,
    ...
    *new_tcam_index = soc_th_alpm_physical_idx(u, L3_DEFIPm,
                                               *new_tcam_index, v6);

    rv = _soc_th_alpm_update_bpm_len(u, *new_tcam_index, v6, bpm_len);

    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "Unable to init bpm_len "
                              "for index %d\n"), *new_tcam_index));
        _soc_th_alpm_rollback_pivot_add(u, v6, &lpm_entry, pi->key_data,
                                        *new_tcam_index, pi->pivot_pyld);
        (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data, mem,
                                              old_pivot, pi->pivot_pyld,
                                              &prefix_array, new_index,
                                              new_insert);
    ...
    if (v6) {
        pivot_trie = VRF_PIVOT_TRIE_IPV6(u, pi->vrf);
    } else {
        pivot_trie = VRF_PIVOT_TRIE_IPV4(u, pi->vrf);
    }

    /* insert into pivot trie */
    rv = trie_insert(pivot_trie, pi->pivot_pyld->key,
            ALPM_CTRL(u).trie_propagate ? bpm : NULL,
            pi->pivot_pyld->len, (trie_node_t *)pi->pivot_pyld);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "failed to insert into pivot trie\n")));
        (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data, mem,
                                              old_pivot, pi->pivot_pyld,
                                              &prefix_array, new_index,
                                              new_insert);
    ...
    tcam_index = *new_tcam_index << (v6 ? 1 : 0);
    ALPM_TCAM_PIVOT(u, tcam_index) = pi->pivot_pyld;
    PIVOT_TCAM_INDEX(pi->pivot_pyld) = tcam_index;
    PIVOT_TCAM_BPMLEN(pi->pivot_pyld) = bpm_len;

    rv = _soc_th_alpm_move_inval(u, mem, v6, &prefix_array, new_index);
    if (SOC_FAILURE(rv)) {
        _soc_th_alpm_rollback_pivot_add(u, v6, &lpm_entry, pi->key_data,
                                        *new_tcam_index, pi->pivot_pyld);
        (void) _soc_th_alpm_rollback_bkt_move(u, v6, pi->key_data, mem,
    ...
    rv = soc_th_alpm_hit_bits_update(u, &prefix_array, old_index, new_index);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_ALPM,
                  (BSL_META_U(u, "update_hit_bits failed, count %d\n"),
                              prefix_array.count));
    ...
    if (new_insert == -1) {
        ...
        soc_mem_field32_set(u, mem, pi->alpm_data, SUB_BKT_PTRf,
                            ALPM_SUB_BKT(u, PIVOT_BUCKET_INDEX(old_pivot)));
        rv = _soc_th_alpm_insert_in_bkt(u, mem, PIVOT_BUCKET_INDEX(old_pivot),
                                        bank_disable, pi->alpm_data,
                                        e, &key_index, v6);
        ...
        *index_ptr = key_index;
        pi->new_pfx_pyld->index = key_index;
    }

    /* Update old bucket usage */
    soc_th_alpm_bucket_usage_decr(u, log_bkt_old, FALSE, mem, old_bkt_cnt);
    /* Update new bucket usage */
    soc_th_alpm_bucket_usage_incr(u, pi->log_bkt, tcam_index, pi->vrf, mem, new_bkt_cnt);

    PIVOT_BUCKET_ENT_CNT_UPDATE(pi->pivot_pyld);
    VRF_BUCKET_SPLIT_INC(u, pi->vrf, v6);
    return rv;
}
...
{
    alpm_pivot_t *pivot_pyld_new, *old_pivot;
    defip_aux_scratch_entry_t aux_entry;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    ...
    trie_node_t *lpmp = NULL, *delp = NULL;
    ...
    if (rv == SOC_E_FULL) {
        /* 3 is least payload count of splitable trie tree */
        hit = _soc_th_alpm_bkt_entry_cnt(u, mem);
        if (hit > 4) {
            hit = 4;
        }

        if (PIVOT_BUCKET_TRIE(ALPM_TCAM_PIVOT(u, tcam_index))->trie == NULL ||
                PIVOT_BUCKET_COUNT(ALPM_TCAM_PIVOT(u, tcam_index)) < hit) { /* find a new home*/
            int log_bkt_new;
            rv = soc_th_alpm_bucket_assign(u, &log_bkt_new, vrf, v6);
            SOC_IF_ERROR_RETURN(rv);

            rv = _soc_th_alpm_bucket_move(u, mem, log_bkt, log_bkt_new);
            SOC_IF_ERROR_RETURN(rv);
            log_bkt = log_bkt_new;
            soc_mem_field32_set(u, mem, alpm_data, SUB_BKT_PTRf,
                                ALPM_SUB_BKT(u, log_bkt));
    ...
    pivot_pyld_new = ALPM_TCAM_PIVOT(u, tcam_index);
    trie = PIVOT_BUCKET_TRIE(pivot_pyld_new);
    /* existing pivot could move around. Save it */
    old_pivot = pivot_pyld_new;
    ...
    sal_memcpy(payload->key, prefix, sizeof(prefix));
    payload->len    = length;
    /* Needs update if mem_insert returns SOC_E_FULL, updated below */
    ...
    if (split) {
        pi.key_data         = key_data;
        pi.new_pfx_pyld     = payload;
        pi.pivot_pyld       = pivot_pyld_new;
        pi.alpm_data        = alpm_data;
        pi.alpm_sip_data    = alpm_sip_data;
        pi.log_bkt          = log_bkt;
        pi.vrf_id           = vrf_id;
        pi.vrf              = vrf;
        rv = _soc_th_alpm_bucket_split(u, v6, &pi, &new_tcam_index, index_ptr);
        if (rv != SOC_E_NONE) {
           return rv;
        }
        /* New Pivot */
    ...
    if (0) {
        ...
        soc_mem_field32_set(u, L3_DEFIP_AUX_SCRATCHm, &aux_entry, ALG_HIT_IDXf,
                            _soc_th_alpm_rpf_entry(u, *index_ptr));
...
#ifdef CHECK_CLEAN_BKT
static int
_soc_th_alpm_bkt_count(int u, soc_mem_t mem, int log_bkt, int vrf, void *bufp)
{
    int bank_num;
    int entry_num;
    int bank_count;
    ...
    while (bkt_count--) {
        ...
        if (soc_mem_field32_get(u, mem, bufp, VALIDf)) {
            LOG_CLI(("mem %s log_bkt %d vrf %d bank_disable %d bkt_count %d "
                     "entry_num %d, bank_num %d, addr %d\n",
                     SOC_MEM_NAME(u, mem), log_bkt, vrf, bank_disable, bkt_count,
                    ...
                    int tcam_index, int key_index)  /* TCAM entry data */
{
    ...
    defip_alpm_ipv4_1_entry_t alpmv4_entry, alpmv42;
    defip_alpm_ipv6_64_1_entry_t alpmv6_entry, alpmv62;
    defip_aux_scratch_entry_t aux_entry;
    uint32      e[SOC_MAX_MEM_FIELD_WORDS];
    soc_mem_t mem;
    void      *bufp, *bufp2;
    ...
    if (!(ALPM_PREFIX_IN_TCAM(u, vrf_id))) {
        ...
        /* Update usage stat */
        soc_th_alpm_bucket_usage_decr(u, PIVOT_BUCKET_INDEX(pivot_pyld), empty_bucket, mem, 1);
        if (empty_bucket) { /* Empty bucket */
            /* Release Bucket */
            rv = soc_th_alpm_bucket_release(u, PIVOT_BUCKET_INDEX(pivot_pyld), vrf, v6);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META_U(u,
            ...
            assert(_soc_th_alpm_bkt_count(u, mem, delete_bucket, vrf,
                                            bufp) == 0);
#endif /* CHECK_CLEAN_BKT */
        }
    }
...
soc_th_alpm_vrf_add(int u, int vrf, int v6)
{
    ...
    rv = soc_th_alpm_bucket_assign(u, &log_bkt, vrf, v6);
    if (SOC_FAILURE(rv)) {
        goto _err_exit;
    }
    SOC_MEM_OPT_F32_SET(u, L3_DEFIPm, lpm_entry, ALG_BKT_PTR0f,
    ...
    rv = soc_th_alpm_lpm_insert(u, &tmp_lpm, &index);
    if (SOC_FAILURE(rv)) {
        goto _err_exit;
    }
                   ...
                   int pivot_info)
{
    ...
    if (ALPM_PREFIX_IN_TCAM(u, vrf_id) || (ipmc_route) || (flex_v6_64)) {
        ...
        rv = soc_th_alpm_lpm_insert(u, lpm_entry, &index);
                             ...
                             int do_urpf)
{
    ...
    for (i = 0; i < ent_num; i++) {
        ...
        SOC_IF_ERROR_RETURN(soc_mem_read_no_cache(u, 0, mem, 0, MEM_BLOCK_ANY,
                                                  index, (void *)bufp));
...
soc_th_alpm_shared_mem_init(int u)
{
    ...
    soc_th_alpm_bucket_assign(u, &defip_table_size, 1, 1);
    ...
    if (soc_property_get(u, "l3_alpm_vrf_share_bucket", 0) &&
        ...
        /* Bucket 0 is reserved. Just assign it */
        soc_th_alpm_bucket_assign(u, &defip_table_size, SOC_VRF_MAX(u) + 1, 1);
    } else {
        SOC_TH_ALPM_GLB_BUCKET_BMAP(u) = SOC_TH_ALPM_VRF_BUCKET_BMAP(u);
    }
...
int
soc_th_alpm_bucket_assign(int u, int *log_bkt, int vrf, int v6)
{
    ...
    if (i == SOC_TH_ALPM_BUCKET_COUNT(u)) {
        ...
        return _soc_th_alpm_bucket_assign(u, mem, log_bkt);
    }
...
soc_th_alpm_bucket_release(int u, int log_bkt, int vrf, int v6)
{
    ...
    if (SOC_URPF_STATUS_GET(u)) {
        soc_alpm_cmn_bkt_view_set(u, _soc_alpm_rpf_entry(u, key_index),
                                  INVALIDm);
    }
    if (step_count == 2) {
...
int _lpm_fb_urpf_entry_replicate(int u, int index, uint32 *e)
{
    ...
    return _soc_th_alpm_write_pivot(u, MEM_BLOCK_ANY, index + src_tcam_offset,
                                    index, e);
}
...
int _lpm_fb_entry_shift(int u, int from_ent, int to_ent)
{
    ...
    if (rv >= 0) {
        rv = _lpm_fb_urpf_entry_replicate(u, to_ent, e);
    }
    if (rv < 0) {
...
int _lpm_fb_shift_pfx_down(int u, int pfx, int ipv6)
{
    ...
    if ((!ipv6) && (SOC_ALPM_LPM_STATE_END(u, pfx) != SOC_ALPM_LPM_STATE_START(u, pfx))) {
        ...
        if ((v0 == 0) || (v1 == 0)) {
            ...
            if (rv >= 0) {
                rv = _lpm_fb_urpf_entry_replicate(u, prev_ent, e);
            }
            if (rv < 0) {
...
int _lpm_free_slot_delete(int u, int pfx, int ipv6, void *e, int slot)
{
    ...
    if (!ipv6) { /* IPV4 */
        ...
        if (to_ent != from_ent) {
            ...
            if ((rv = _soc_th_alpm_write_pivot(u, MEM_BLOCK_ANY, to_ent, to_ent, et)) < 0)
            {
        ...
        if ((rv = _soc_th_alpm_write_pivot(u, MEM_BLOCK_ANY, from_ent, from_ent, ef)) < 0) {
        ...
        if (to_ent != from_ent) {
            ...
            if ((rv = _soc_th_alpm_write_pivot(u, MEM_BLOCK_ANY, to_ent, from_ent, ef)) < 0)
            {
 ...
 */
static int
soc_th_alpm_lpm_insert(int u, void *entry_data, int *lpm_index)
{
    int         pfx;
    int         index;
    int         ipv6;
    ...
    *lpm_index = index;
    ...
    if (rv == SOC_E_NONE) {
        ...
        rv = _soc_th_alpm_write_pivot(u, MEM_BLOCK_ANY, index, index,
                                   entry_data);
        if (rv >= 0) {
        ...
        int *hit, int *tcam_index, int *log_bkt)
{
     ...
     */
    if (ALPM_CTRL(u).trie_propagate) {
        rv = _soc_th_alpm_aux_sw_op(u, aux_op, aux_entry, &cb_cnt);
    }
    ...
    if ((SOC_SUCCESS(rv) && cb_cnt > 0) || ALPM_CTRL(u).force_aux_op) {
        ...
        if ((aux_op == INSERT_PROPAGATE || aux_op == DELETE_PROPAGATE) &&
            ...
            val = soc_mem_field32_get(u, mem, aux_entry, IP_LENGTHf);
            val = (val == 0) ? 1 : 0;
...
soc_th_alpm_dbg_urpf(int u)
{
    ...
    for (phy_bkt = 0; phy_bkt <= SOC_TH_ALPM_MAX_BKTS(u) / 2; phy_bkt++) {
        ...
        for (i = 0; i < SOC_TH_MAX_BUCKET_ENTRIES - 1; i++) {
            ...
            rv = soc_mem_read(u, mem, MEM_BLOCK_ANY, index, e);
            if (SOC_FAILURE(rv)) {
                continue;
            }
            ...
            rv = soc_mem_read(u, mem, MEM_BLOCK_ANY, _soc_th_alpm_rpf_entry(u, index), eu);
            if (SOC_FAILURE(rv)) {
                continue;
            }
        ...
        for (bank = 0; bank < 4; bank++) {
            ...
            for (i = 0; i < entmax; i++) {
                ...
                j = (i << (tmp + alpm_bkt_bits)) |
                    (phy_bkt << tmp) | bank;
                rv1 = soc_mem_read(u, lpm_mem, MEM_BLOCK_ANY, j, e);
                if (rv1 == 0 &&
