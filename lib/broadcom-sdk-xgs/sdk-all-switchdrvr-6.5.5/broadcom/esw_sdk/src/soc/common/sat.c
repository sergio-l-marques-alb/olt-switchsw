   ...
   /* For writing on dynamic tables (in the OAMP), have to enable dynamic memory access */
    ...
    if (SOC_IS_QAX(unit)) { 
        ...
        for (priority = 0; priority < SOC_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
            ...
            rv = WRITE_OAMP_SAT_TX_GEN_PRM_ENTRY_2m(unit, MEM_BLOCK_ANY, SAT_TX_GEN_PRM_ENTRY_2_TBL_INDX(gtf_id, priority)+1, tx_gen_prm_entry2_data);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_SAT,
                          (BSL_META_U(unit,
    ...
    )
{
    ...
    rv = _soc_sat_gtf_rate_from_internal_rate_convert(unit, high_mnt_val, high_exp_val, crdt_per_pkt,&(config->high_threshold));
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in _soc_sat_gtf_rate_from_internal_rate_convert\n"), soc_errmsg(rv)));
    ...
    rv = _soc_sat_gtf_rate_from_internal_rate_convert(unit, low_mnt_val, low_exp_val, crdt_per_pkt,&(config->low_threshold));
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in _soc_sat_gtf_rate_from_internal_rate_convert\n"), soc_errmsg(rv)));
    ...
    )
{
    ...
    if (packet_info->payload.payload_type == socSatPayloadConstant4Bytes) {
        ...
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 1);
        /* USE_8_BYTE_PATTERNf */
        offset = SOC_SAT_USE_8_BYTE_PATTERN_OFFSET;
        field = 0;
        SHR_BITCOPY_RANGE(reg_above64_val, offset, &field, 0, 1);
    }
    else if (packet_info->payload.payload_type == socSatPayloadConstant8Bytes) {
