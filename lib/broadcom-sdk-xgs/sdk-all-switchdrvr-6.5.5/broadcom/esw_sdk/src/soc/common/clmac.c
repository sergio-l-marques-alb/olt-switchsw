...
#define CLMAC_RUNT_THRESHOLD_GE    0x40   /* Runt threshold value for GE ports */
#define CLMAC_RUNT_THRESHOLD_XE    0x40   /* Runt threshold value for XE ports */
...
mac_cl_init(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_TOMAHAWK(unit) && IS_HG_PORT(unit, port)) {
        int runt;
        runt = soc_property_port_get(unit, port, spn_HIGIG2_HDR_MODE,
                   soc_feature(unit, soc_feature_no_higig_plus) ? 1 : 0) ?
                       CLMAC_RUNT_THRESHOLD_HG2 : CLMAC_RUNT_THRESHOLD_HG1;
        soc_reg64_field32_set(unit, CLMAC_RX_CTRLr, &rx_ctrl, RUNT_THRESHOLDf, runt);
    }
...
mac_cl_encap_set(int unit, soc_port_t port, int mode)
{
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        ...
        runt = ((mode == SOC_ENCAP_HIGIG2) || (mode == SOC_ENCAP_HIGIG2_LITE)) ?
                    CLMAC_RUNT_THRESHOLD_HG2 : ((mode == SOC_ENCAP_HIGIG) ?
                        CLMAC_RUNT_THRESHOLD_HG1 : CLMAC_RUNT_THRESHOLD_XE);
        SOC_IF_ERROR_RETURN
