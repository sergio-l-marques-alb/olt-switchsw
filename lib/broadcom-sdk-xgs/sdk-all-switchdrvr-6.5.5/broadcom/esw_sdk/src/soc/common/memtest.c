...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_POLAR_SUPPORT)
/*
 * Table memory test
 */
...
soc_ser_test_inject_full(int unit, uint32 flags, ser_test_data_t *test_data)
{
    ...
    } else if (flags & SOC_INJECT_ERROR_DONT_MAP_INDEX) {
        return soc_ser_test_inject_error(unit, test_data, 
                                         SOC_INJECT_ERROR_DONT_MAP_INDEX);
    } else {
        return soc_ser_test_inject_error(unit, test_data, 0);
    }
}
...
#endif /* BCM_ESW_SUPPORT || BCM_SBX_SUPPORT || BCM_SAND_SUPPORT || BCM_POLAR_SUPPORT */
