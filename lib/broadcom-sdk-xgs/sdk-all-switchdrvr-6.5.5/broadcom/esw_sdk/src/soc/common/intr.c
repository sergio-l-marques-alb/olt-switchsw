...
void soc_cmn_error(void *unit_vp, void *d1, void *d2, void *d3, void *d4)
{
    ...
    for (i = 0; i < interrupt_num; i++) {
        ...
        if (SOC_IS_JERICHO(unit) || SOC_IS_FE3200(unit)) {
            ...
            if (((interrupt_flags & SOC_INTERRUPT_DB_FLAGS_BCM_AND_USR_CB) == 0) && SHR_BITGET(&interrupt_flags, SOC_INTERRUPT_DB_FLAGS_CORR_ACT_OVERRIDE_ENABLE)) {
                continue;
            }
        }
#endif
...
int soc_interrupt_get_block_index_from_port(int unit, int interrupt_id, int port)
{
    ...
    if(SOC_IS_ARAD(unit)){
        if(SOC_BLOCK_IN_LIST(block_types, SOC_BLK_CLP)) {
            ...
            index = SOC_IS_JERICHO(unit) ? SOC_PORT_BLOCK_NUMBER(unit, port) : SOC_PORT_BLOCK_NUMBER(unit, port) + SOC_MAX_NUM_CLP_BLKS;
        } 
    }
#endif /* BCM_ARAD_SUPPORT */
