    ...
    *blk = 0;/* not really needed, just to avoid coverity defect */
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(regblktype, portblktype))) {
        ...
        case SOC_BLK_NIF_ILU:
            block = NIF_ILU_BLOCK(unit, blkport);
            break;
        case SOC_BLK_NIF_CDU:
            block = NIF_CDU_BLOCK(unit, blkport);
            break;		
        case SOC_BLK_NIF_NMG:
            block = NIF_NMG_BLOCK(unit, blkport);
            break;
	    case SOC_BLK_MCP:
        ...
        case SOC_BLK_RQP:
            ...
            break;
        case SOC_BLK_EQM:
            block = EQM_BLOCK(unit, blkport);
            break;
        case SOC_BLK_EPS_SPR:
            block = EPS_SPR_BLOCK(unit, blkport);
            break;
        case SOC_BLK_FQP:
            block = FQP_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DHA:
            block = DHA_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DHB:
            block = DHB_BLOCK(unit, blkport);
            break;
        case SOC_BLK_MDB:
