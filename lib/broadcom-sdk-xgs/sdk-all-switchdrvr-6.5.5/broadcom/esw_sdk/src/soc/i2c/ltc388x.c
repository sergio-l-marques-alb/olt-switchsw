...
ltc388x_ioctl(int unit, int devno, int opcode, void* data, int len)
{
   ...
   case I2C_LTC_IOC_NOMINAL:
    ...
    case I2C_LTC_IOC_READ_IOUT:
        ...
        ((i2c_adc_t*)data)->val =
        ((i2c_adc_t*)data)->delta = 0;
        /* For Apache/Maverick/FB5 Three phase support */
        if ((SOC_CONTROL(unit)->board_type == 0x94) ||
            (SOC_CONTROL(unit)->board_type == 0x96)) {
            if (SOC_CONTROL(unit)->board_type == 0x94) {
            ...
            for (iter = 0; iter < 100; iter++) {
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    rv = ltc388x_read_current(unit, devno + 1, &fval1, I2C_LTC3880_CH0);
                    fval += fval1;
                } else {
        ...
        if ((SOC_CONTROL(unit)->board_type == 0x94) ||
            ...
            for (iter = 0; iter < 100; iter++) {
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    rv = ltc388x_read_power(unit, devno + 1, &fval1, I2C_LTC3880_CH0);
                    fval += fval1;
                } else {
        ...
        if (DEV_DAC_PARAMS(dev)) {
            ...
            if ((SOC_CONTROL(unit)->board_type == 0x94) ||
                ...
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    /* Set VDD_Core on Channel 0 of I2C_LTC3884_SADDR43 */
                    SOC_IF_ERROR_RETURN
                        (ltc388x_dac_set_ch_voltage(unit, devno + 1, I2C_LTC3880_CH0, voltage));
                } else {
                    SOC_IF_ERROR_RETURN
                        (ltc388x_dac_set_ch_voltage(unit, devno, ch, voltage));
                }
