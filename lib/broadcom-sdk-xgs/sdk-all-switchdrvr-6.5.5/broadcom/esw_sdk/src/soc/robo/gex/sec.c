        ...
        */
                {
                    if ((rv = REG_READ_PORT_EAP_CONr(
                        unit, port, (void *)&reg_value64)) < 0) {
                        return rv;
                    }
                }
                temp = 0;
                soc_PORT_EAP_CONr_field_set(unit, (void *)&reg_value64,
                    EAP_BLK_MODEf, &temp);
#if defined(BCM_POLAR_SUPPORT) || defined(BCM_NORTHSTAR_SUPPORT) || \
    defined(BCM_NORTHSTARPLUS_SUPPORT)
                if ((SOC_IS_POLAR(unit) || SOC_IS_NORTHSTAR(unit) || 
...
drv_gex_security_get(int unit, uint32 port, uint32 *state, uint32 *mask)
{
    ...
    soc_PORT_EAP_CONr_field_get(unit, (void *)&reg_value64,
        EAP_MODEf, &temp);
    if (temp) {
        soc_PORT_EAP_CONr_field_get(unit, (void *)&reg_value64,
            EAP_BLK_MODEf, &temp);
...
drv_gex_security_egress_set(int unit, soc_pbmp_t bmp, int enable)
{
    ...
    defined(BCM_NORTHSTARPLUS_SUPPORT)
        if ((SOC_IS_POLAR(unit) || SOC_IS_NORTHSTAR(unit) || 
            ...
            && (port == specified_port_num)) {
            SOC_IF_ERROR_RETURN(
                REG_READ_PORT_EAP_CON_P7r(unit, (void *)&reg_value64));
        } else 
#endif /* BCM_POLAR_SUPPORT || BCM_NORTHSTAR_SUPPORT || 
        * BCM_NORTHSTARPLUS_SUPPORT
        */
        {
            SOC_IF_ERROR_RETURN(
                REG_READ_PORT_EAP_CONr(unit, port, (void *)&reg_value64));
        }
        ...
        SOC_IF_ERROR_RETURN(soc_PORT_EAP_CONr_field_set
            (unit, (void *)&reg_value64, EAP_BLK_MODEf, &temp));
    ...
    defined(BCM_NORTHSTARPLUS_SUPPORT)
        if ((SOC_IS_POLAR(unit) || SOC_IS_NORTHSTAR(unit) || 
            ...
            && (port == specified_port_num)) {
            SOC_IF_ERROR_RETURN(
                REG_WRITE_PORT_EAP_CON_P7r(unit, (void *)&reg_value64));
        } else 
#endif /* BCM_POLAR_SUPPORT || BCM_NORTHSTAR_SUPPORT || 
        * BCM_NORTHSTARPLUS_SUPPORT
        */
        {
            SOC_IF_ERROR_RETURN(
                REG_WRITE_PORT_EAP_CONr(unit, port, (void *)&reg_value64));
        }
    }
