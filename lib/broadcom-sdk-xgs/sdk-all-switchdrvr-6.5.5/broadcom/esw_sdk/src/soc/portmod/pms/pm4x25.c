...
#define PM4x25_WB_BUFFER_VERSION        (5)
...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_2D_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "lane_connection_db",
                          ...
                          PM4X25_LANES_PER_CORE, MAX_PHYN, VERSION(5));
    _SOC_IF_ERR_EXIT(rv);
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    if(!SOC_WARM_BOOT(unit)){
        ...
        nof_phys = 1;
        for(i = 0 ; i < PM4X25_LANES_PER_CORE ; i++){
            PM4x25_PORT_NUM_PHYS_SET(unit, pm_info, nof_phys, i);
        }
                             ...
                             const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
        ...
        if(!core_is_initialized){
            ...
            /*if all initialization of SerDes have been done, then set the core_is_initialized*/
...
STATIC
int _pm4x25_ext_phy_interface_check(int unit, int port,
                               ...
                               soc_port_if_t interface, int *is_valid)
{
    switch (interface_config->speed) {
        case 100000:
        case 106000:
            if ((SOC_PORT_IF_CAUI_C2C == interface) ||
                                   ...
                                   phymod_interface_t *interface_type)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_line_intf_from_config_get(config, &interface));
    _SOC_IF_ERR_EXIT(_pm4x25_interface_check(unit, port, config, interface,
                                             &is_valid));
    if (nof_phys > 1) {
        /* EXT PHY interface */
        _SOC_IF_ERR_EXIT
            (_pm4x25_ext_phy_interface_check(unit, port, config, interface,
                                                 &is_valid));
    }

    if (!is_valid ||
        /* phy_init_flags is PORTMOD_INIT_F_ALL_PHYS only when this function is called 
         * during the init process by pm4x25_port_attach(). That is, only during the
         * init process the default interface type of a 42G HG2 port without ext PHY
         * is being overwritten by _pm4x25_default_interface_get(). Otherwise the
         * default interface type comes from soc_esw_portctrl_config_get() for the
         * XGS devices. In the future the default interface types of different port
         * mode, port speed with or without ext PHY could also be modified using
         * _pm4x25_default_interface_get() here.
         */
    ...
    _SOC_IF_ERR_EXIT(PM4x25_INTERFACE_TYPE_SET(unit, pm_info, interface, port_index));
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    if(PM_4x25_INFO(pm_info)->nof_phys[port_index] > 0) {
        ...
        if (nof_phys > 1)
            portmod_port_line_intf_map_fix(&phy_access[nof_phys-1], port, config->speed, phy_config->interface_type, &phy_config->interface_type);
    ...
    rv = pm4x25_port_interface_config_set(unit, port, pm_info, config, PORTMOD_INIT_F_ALL_PHYS);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x25_port_interface_config_get(int unit, int port, pm_info_t pm_info, portmod_port_interface_config_t* config)
{
    ...
    if(is_interface_restore) {
        rv = portmod_intf_from_phymod_intf(unit,
                    phy_config.interface_type, &(config->interface));
        _SOC_IF_ERR_EXIT(rv);
    } else {
        ...
        if( (phy_config.data_rate == 10000) &&
            phy_config.interface_type == phymodInterfaceSFI){
            if(prev_phymod_if == phymodInterfaceSR)
                phy_config.interface_type = phymodInterfaceSR;
            else if(prev_phymod_if == phymodInterfaceLR)
                phy_config.interface_type = phymodInterfaceLR;
        }

        /* portmod_port_phychain_interface_config_get() cannot distinguish among 10G XFI/KR.
               Following section of code classifies the 10G backplane interface by reading the previously
               set PHYMOD interface. */
        if( (phy_config.data_rate == 10000) &&
            phy_config.interface_type == phymodInterfaceXFI){
            if(prev_phymod_if == phymodInterfaceKR)
                phy_config.interface_type = phymodInterfaceKR;
        }

        /* portmod_port_phychain_interface_config_get() cannot distinguish among 11/12G XFI/KR.
               Following section of code classifies the 11/12G backplane interface by reading the previously
               set PHYMOD interface. */
        if(((phy_config.data_rate == 11000) || (phy_config.data_rate == 12000)) &&
            phy_config.interface_type == phymodInterfaceKR){
            if(prev_phymod_if == phymodInterfaceXFI)
                phy_config.interface_type = phymodInterfaceXFI;
        }

        /* portmod_port_phychain_interface_config_get() cannot distinguish among SR/LR.
               Following section of code classifies the 20/25G optical interface by reading the previously
               set PHYMOD interface. */
        if(((phy_config.data_rate == 20000) || (phy_config.data_rate == 25000)) &&
            phy_config.interface_type == phymodInterfaceSR){
            if(prev_phymod_if == phymodInterfaceSR)
                phy_config.interface_type = phymodInterfaceSR;
            else if(prev_phymod_if == phymodInterfaceLR)
        ...
        /* portmod_port_phychain_interface_config_get() cannot distinguish among SR2/LR2.
               Following section of code classifies the 20/40/50G optical interface by reading the previously
               set PHYMOD interface. */
        if(((phy_config.data_rate == 20000) || (phy_config.data_rate == 40000) ||
            (phy_config.data_rate == 50000)) &&
            phy_config.interface_type == phymodInterfaceSR2){
            if(prev_phymod_if == phymodInterfaceSR2)
                phy_config.interface_type = phymodInterfaceSR2;
            else if(prev_phymod_if == phymodInterfaceLR2)
                phy_config.interface_type = phymodInterfaceLR2;
        }

        /* portmod_port_phychain_interface_config_get() cannot distinguish among SR4/LR4.
               Following section of code classifies the 40G/100G optical interface by reading the previously
               set PHYMOD interface. */
        if(((phy_config.data_rate == 40000) || (phy_config.data_rate == 100000)) &&
            phy_config.interface_type == phymodInterfaceSR4){
            if(prev_phymod_if == phymodInterfaceSR4)
                phy_config.interface_type = phymodInterfaceSR4;
            else if(prev_phymod_if == phymodInterfaceLR4)
                phy_config.interface_type = phymodInterfaceLR4;
        }

        /* portmod_port_phychain_interface_config_get() cannot distinguish among 40G KR2/XLAUI2.
               Following section of code classifies the 40G backplane interface by reading the previously
               set PHYMOD interface. */
        if((phy_config.data_rate == 40000)&&
            phy_config.interface_type == phymodInterfaceKR2){
            if(prev_phymod_if == phymodInterfaceXLAUI2)
                phy_config.interface_type = phymodInterfaceXLAUI2;
        }
    }
    ...
    rv = portmod_intf_from_phymod_intf(unit, phy_config.interface_type, &config->serdes_interface);
    _SOC_IF_ERR_EXIT(rv);
...
{   
    ...
    PM4x25_PORT_LANE_CONNECTION_DB_SET(unit, pm_info, candidate_lane_conn, phyn-1, lane_index);
    ...
    if (PM_4x25_INFO(pm_info)->nof_phys[lane_index] <= phyn){
        ...
        PM4x25_PORT_NUM_PHYS_SET(unit, pm_info, PM_4x25_INFO(pm_info)->nof_phys[lane_index], lane_index);
    }
