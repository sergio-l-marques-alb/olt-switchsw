...
int dnx_fabric_pm_phys_get(int unit, pm_info_t pm_info, soc_pbmp_t* phys)
{
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    SOCDNX_INIT_FUNC_DEFS;
...
int dnx_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOC_PBMP_ASSIGN(pm_port_phys, fabric_data->phys);
    SOC_PBMP_AND(pm_port_phys, add_info->phys);
...
int dnx_fabric_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    SOC_PBMP_AND(port_phys_in_pm, fabric_data->phys);
...
int dnx_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
    ...
    {
        /*In ILKN we do Rx only by enable/disable RX FMAC*/
        SOCDNX_IF_ERR_EXIT(dnx_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
    ...
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_UNAVAIL , (_BSL_SOCDNX_MSG("Supported options are only MAC + RX or all - MAC, PHY, TX, RX")));
    }
exit:
    SOCDNX_FUNC_RETURN; 
...
int dnx_fabric_port_enable_get(int unit, int port, pm_info_t pm_info, int flags, int *enable)
{
    uint32 reg_val, flags_mask = flags;
    uint32 field_val[1];
    uint32 is_bypass, mac_reset, bitmap;
    dnx_fabric_t fabric_data = pm_info->pm_data.dnx_fabric;
    uint32 phy_index;
    phymod_phy_access_t phy_access;
    phymod_phy_reset_t phy_reset;
    ...
    *nof_phys = 1;
    ...
    if (is_bypass) {
        ...
        if (params->lane == PORTMOD_ALL_LANES_ARE_ACTIVE) {
            ...
            phy_access_arr[0].access.lane_mask = 0xf & (1 << params->lane); 
...
int dnx_fabric_port_lane_map_set(int unit, int port, pm_info_t pm_info, const phymod_lane_map_t* lane_map)
{
    int rv;
    SOCDNX_INIT_FUNC_DEFS;
...
int dnx_fabric_port_lane_map_get(int unit, int port, pm_info_t pm_info, phymod_lane_map_t* lane_map)
{
    int rv;
    SOCDNX_INIT_FUNC_DEFS;
