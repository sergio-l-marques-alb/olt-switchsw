...
int _pmOsILKN_port_num_pipes_get(int unit, int port, pm_info_t pm_info, uint32* num_pipes)
{
    ...
    *num_pipes = PORTMOD_DIV_ROUND_UP( (serdes_speed * nof_lanes ) , ( (core_clk_khz / 1000) * 67) );
...
int pmOsILKN_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    soc_pbmp_t mirror_bmp, phys_temp, temp_phys_aligned;
    ...
    int speed, core_clk_khz, take_4_segments;
    int temp_enable;
    SOC_INIT_FUNC_DEFS;
    ...
    if (pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en) /* ILKN over Fabric */
    {
        SOC_PBMP_ITER(ilkn_data->phys, port_i)
        {
            ...
            SOC_PBMP_PORT_ADD(temp_phys_aligned, lane);
    ...
    take_4_segments = ( ((speed >= 21875) && (nof_lanes > 10) && (core_clk_khz <= OS_ILKN_CORE_CLOCK_600000KHZ)) ? 1 : 0); /* Take 4 segments for lower core clocks in high rates */
    ...
    if ((nof_lanes > OS_ILKN_LANES_PER_CORE_TWO_ACTIVE_PORTS) || take_4_segments) {
...
pmOsILKN_port_logical_lane_order_get(int unit, int port, pm_info_t pm_info, int lane_order_max_size, int* lane_order, int* lane_order_actual_size)
{
    ...
    SOC_PBMP_ITER(temp_phys_aligned, port_i){
                                        ...
                                        int phy_init_flags)
{
    ...
    _SOC_IF_ERR_EXIT(pmOsILKN_port_enable_get(unit, port, pm_info, 0x0, &port_enable));
    _SOC_IF_ERR_EXIT(pmOsILKN_port_enable_set(unit, port, pm_info, 0x0, 0));
    ...
    /* PM reset */
    _SOC_IF_ERR_EXIT(pmOsILKN_port_enable_set(unit, port, pm_info, 0x0, port_enable));
                                      ...
                                      int max_phys, phymod_phy_access_t* access, int* nof_phys, int* is_most_ext)
{
    ...
    if (params->lane != PORTMOD_ALL_LANES_ARE_ACTIVE){
        ...
        SOC_PBMP_ITER(temp_phys_aligned, lane_index) {
            active_lane_counter ++;
