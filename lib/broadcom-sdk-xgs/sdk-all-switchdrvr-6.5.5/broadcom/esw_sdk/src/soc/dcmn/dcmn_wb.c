...
/* counter, counting the number of dispatcher BCM api calls in the stack trace */
int    wb_nested_api_cnt[SOC_MAX_NUM_DEVICES];
...
int dcmn_bcm_warm_boot_api_test_reset_conds (int unit) {
    ...
    if (sal_thread_self() == sal_thread_main_get()) {
        ...
        nested_api_counter = dcmn_wb_api_nesting_get(unit);
...
/** decide if to skip the current reset or not
  ...
  * retval - nof_skip - the number of tests we are going to skip (if it's the first of them)
  */

_dcmn_wb_skip_decision_t dcmn_bcm_warm_boot_api_test_reset_skip_decider(int unit, int *nof_skip) {
...
void dcmn_wb_api_nesting_inc(int unit)
{
    wb_nested_api_cnt[unit]++;
}

void dcmn_wb_api_nesting_dec(int unit)
{
    wb_nested_api_cnt[unit]--;
}

int dcmn_wb_api_nesting_get(int unit)
{
    return wb_nested_api_cnt[unit];
}
...
int dcmn_bcm_warm_boot_api_test_reset(int unit, int8* is_reset) {
...
#ifdef CRASH_RECOVERY_SUPPORT
    cr_enabled = SOC_CR_ENABALED(unit);
    if (cr_enabled) SOC_CR_DISABLE(unit); /* don't perform commits */
    soc_hw_set_immediate_hw_access(unit); /* freeze hw_log */
#endif
    if (dcmn_bcm_warm_boot_api_test_reset_conds(unit)) {
        ...
        skip_decision = dcmn_bcm_warm_boot_api_test_reset_skip_decider(unit, &nof_skip);
        ...
        soc_dcmn_wb_test_counter_get(unit, &test_counter);
#ifdef BCM_WARM_BOOT_API_TEST
        if (skip_decision != _dcmnWbSkipFasterRegression)
...
#else
                      LOG_WARN(BSL_LS_BCM_COMMON, (BSL_META_U(unit,
                        "  --> *** WB BCM API *** skipping %d warm reboot tests for faster regression\n") , nof_skip));
#endif
                        break;
            case _dcmnDontSkip:
            default:
    ...
    if (cr_enabled) SOC_CR_ENABLE(unit);
#endif
    return 0;
}
