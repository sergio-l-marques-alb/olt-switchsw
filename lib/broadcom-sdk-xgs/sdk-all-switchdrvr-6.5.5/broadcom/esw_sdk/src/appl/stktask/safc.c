                                     ...
                                     int unit, int modid, int dest)
{
    ...
    if (dest) {
        if (info->safc_only) {
           /* If this unit carries only SAFC traffic, then only 
            * one modport should be active for any destination. 
            * Check if a modport is already present for a given
            * destination. If so delete and add modport derived
            * from the SAFC algorithm. 
            */
            rv = bcm_stk_modport_get(unit, modid, &d_port);
            BCM_IF_ERROR_RETURN(rv);
            if (d_port != s_port) {
                rv = bcm_stk_modport_delete(unit, modid, d_port);
                BCM_IF_ERROR_RETURN(rv);
            }
        }
