/*
 * $Id: shell.c,v 1.235 Broadcom SDK $
...
sh_swap_unit_vars(int new_unit)
{
    ...
    if (new_unit >= 0) {
        ...
        if ((SOC_IS_SABER2(new_unit)) && (!(SOC_IS_METROLITE(new_unit)))) {
            /* The firmware images for embedded applications are same for SB2 A0 and B0
            Hence, the fw_prefix should be BCM56260_A0 for B0 also */
            var_set("fw_prefix", (char *)"BCM56260_A0", FALSE, FALSE);
        }

		if (SOC_IS_APACHE(new_unit)) {
            /* The firmware images for embedded applications are same for Apache A0 and B0
            Hence, the fw_prefix should be BCM56560_A0 for B0 also */
            var_set("fw_prefix", (char *)"BCM56560_A0", FALSE, FALSE);
        }
    ...
    if (new_unit >= 0) {
        ...
        if (SOC_IS_ROBO(new_unit)) {
            command_mode_set(new_unit, ROBO_CMD_MODE);
#if defined(BCM_SBX_SUPPORT)
        } else if (SOC_IS_SBX(new_unit)) {
            command_mode_set(new_unit, SBX_CMD_MODE);
#endif
#if defined(BCM_EA_SUPPORT)
        } else if(SOC_IS_EA(new_unit)) {
            command_mode_set(new_unit, EA_CMD_MODE);
#endif
 ...
 */
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (SOC_IS_SBX(SOC_NDEV_IDX2DEV(i))) {
            if (soc_sbx_shutdown(SOC_NDEV_IDX2DEV(i)) < 0) {
                return(CMD_FAIL);
            }
        } else
 ...
 */
{
    ...
    else
    {
        ...
        if (command_mode_get(unit) == BCMX_CMD_MODE) {
#ifdef    INCLUDE_BCMX
...
*/
    ...
    if (cur_mode[unit] == BCMX_CMD_MODE) {
...
typedef struct {
    uint8 source_list[bslLayerCount][bslSourceCount];
    uint8 source_count[bslLayerCount];
} bsl_source_list_t;
 ...
 */
{
    ...
    for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
        if (CMDEV(dev).dev.info) {
            cli_out("%s: Unit %d (%s): %sattached%s\n",
                    ARG_CMD(a),
                    SOC_NDEV_IDX2DEV(dev), soc_dev_name(SOC_NDEV_IDX2DEV(dev)),
                    soc_attached(SOC_NDEV_IDX2DEV(dev)) ? "" : "not ",
                    (unit == SOC_NDEV_IDX2DEV(dev)) ? " (current unit)" : "");
        }
    }
    ...
    while ((c = ARG_GET(a)) != NULL) {
        if (*c == '*') {  /* Attach (report) all devices */
            for (dev = 0; dev < SOC_MAX_NUM_DEVICES; dev++) {
                if (CMDEV(dev).dev.info) {
                    if (!soc_attached(SOC_NDEV_IDX2DEV(dev))) {
                        if (sysconf_attach(SOC_NDEV_IDX2DEV(dev)) < 0) {
                            cli_out("%s: Error: Could not attach unit: %d\n",
                                    ARG_CMD(a), SOC_NDEV_IDX2DEV(dev));
                            return(CMD_FAIL);
                        }
                    } else {
                        cli_out("Unit %d is attached\n", SOC_NDEV_IDX2DEV(dev));
                    }
                }
            } /* end of for loop */
            return CMD_OK;
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
static char *
...
#define SIG_BUFF_LEN   1000

static void
appl_warmboot_signatures(int unit)
{
    int i;
    char buff[SIG_BUFF_LEN] __attribute__((unused));

    for (i=0; i<SIG_BUFF_LEN-1; i++)
        buff[i] = ' ';
    buff[SIG_BUFF_LEN-1] = 0;

#ifndef __KERNEL__ 
    cli_out("\nWarmboot signatures:\n");
    cli_out("-------------------------------\n");
    soc_sbx_warmboot_signatures(unit);
    cli_out("-------------------------------\n");   
#endif
}
#endif
...
appl_warmboot_storage_requirements(int unit)
{
    ...
    uint32 module;
#if defined(BCM_SBX_SUPPORT)
    uint32 storage_per_module[SOC_SBX_WB_MODULE__COUNT+3];
#else
...
#if defined(BCM_SBX_SUPPORT)
    for (module = 0; module < SOC_SBX_WB_MODULE__COUNT+3; module++) {
        storage_per_module[module] = 0;
    }

    for (module = SOC_MODULE_COUNT_START; module < SOC_SBX_WB_MODULE__COUNT; module++) {
        SOC_SCACHE_HANDLE_SET(handle, unit, module, 0);
        rv = soc_scache_ptr_get(unit, handle, &scache_ptr,
                                &storage_per_module[module]);  
        if (SOC_SUCCESS(rv)) {
            cli_out("%s module: %d bytes\n",
                         soc_sbx_module_name(unit, module),
                         storage_per_module[module]);
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT))
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
    "Parameters: [on | off | show | shutdown | storage | usage | memoryuse | sw_state_size\n"
    "| test_mode <on|off|dvapi|no_wb_test|allow_wb_test|skip <$count>>]\n"
#elif defined(BCM_SBX_SUPPORT)
    "Parameters: [on | off | show | storage | signature | usage | test_mode <on|off>]\n"
#else
    "Parameters: [on | off | show | shutdown | storage | usage]\n"
#endif
...
#endif /* (BCM_XGS3_SWITCH_SUPPORT || BCM_SBX_SUPPORT || BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)*/

#if (defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT))
#if (defined(BCM_SBX_SUPPORT))
        else if (!sal_strcasecmp(c, "signature")) {
            appl_warmboot_signatures(u);
        }
#endif /* BCM_SBX_SUPPORT */
		   ...
		   if (0 == ARG_CNT(a)) {
		    ...
		    if (!sal_strcasecmp(test_status, "on")) {
                 /* enable autosync mode */
                       ...
                       soc_dcmn_wb_test_mode_set(u,_DCMN_BCM_WARM_BOOT_API_TEST_MODE_CRASH_RECOVERY_ROLL_BACK);
#endif
                       ...
                       int skip;
                       char *skip_string;
                       skip_string = ARG_GET(a);
                       if (!skip_string) {
                          cli_out("%s: skip count parameter missing: %s\n", ARG_CMD(a), skip_string);
                          return CMD_USAGE;
                       }
                       skip = sal_ctoi(skip_string, 0);
                       soc_dcmn_wb_test_skip_multiple_wb_tests_set(u,skip);
                    }else {
                       return(CMD_USAGE);
                    }
		}
...
#endif
#if defined(BCM_XGS_SUPPORT) || defined(BCM_SBX_SUPPORT)|| defined(BCM_SAND_SUPPORT)
    } else if (!sal_strcasecmp(c, "noreset")) {
    if ((rv = sh_do_init(u, FALSE)) < 0) {
        cli_out("%s: Unable to initialize device %d\n",
                ARG_CMD(a), u);
...
#ifdef BCM_SBX_SUPPORT
        if (SOC_IS_SBX(u)) {
        /* cli_out("This option is not supported on SBX devices\n"); */
            rv = CMD_OK;
        return rv;
        }
#endif
...
_bg_cmd(void *cookie)
{
    ...
    case CMD_INTR:
        break;
    default:
        cli_out("bg: error in job %d: code %d\n", info->job, rv);
        break;
    }
...
#include "../epil/epil_functions.h"
char cmd_epil_usage[] =
"Parameters\n"
"\tsesto_sample   -- runs Sesto sample application - PRBS test\n"
"\tsesto_init     -- init Sesto. Default to 100G\n"
"\tdiag           -- DSC dump. sub-options are eyescan, dump, config\n"
"\tphy_id=<mdio>  -- MDIO address of XPHY\n"
"\tif=<side>      -- which side? sys|line. Default to line\n"
"\tiftype=<intf>  -- Interface type KR/CR/OP. Default ot CR\n"
"\tspeed=<speed>  -- Speed value. Default to 100G\n"
"\teyescan        -- dump eyescan\n" 
"\tdsc            -- dump dsc\n" 
"\tconfig         -- dump lane config\n"; 

int epil_trace=0;
int epil_rdwr=0;

cmd_result_t
cmd_epil(int unit, args_t *args) {

    enum { 
        NO_ACTION=0,
        RUN_SESTO_SAMPLE = 0x1, 
        RUN_SESTO_INIT = 0x2,
        RUN_DIAG = 0x4
    };
    enum {
    ...
    uint32_t action = NO_ACTION;
    uint32_t diag_flag = DUMP_DSC;  
    uint32_t i;
    char *iftype = "CR"; /* type of interface such as CR, KR */
    char *side="line";     /* which side if=line|sys */ 

    if ( args->a_argc == 1 ) { return CMD_USAGE; }

    for(i=0; i < args->a_argc; i++ ) {
        if (!sal_strcasecmp("sesto_sample", args->a_argv[i])) { 
            action |= RUN_SESTO_SAMPLE; 
        } else if (!sal_strcasecmp("sesto_init", args->a_argv[i])) {
            action |= RUN_SESTO_INIT; 
        } else if (!sal_strcasecmp("diag", args->a_argv[i])) {
            action |= RUN_DIAG;
        } else if (!sal_strcasecmp("eyescan", args->a_argv[i])) {
            diag_flag |= EYESCAN;
        } else if (!sal_strcasecmp("dsc", args->a_argv[i])) {
            diag_flag |= DUMP_DSC; 
        } else if (!sal_strcasecmp("config", args->a_argv[i])) {
            diag_flag |= DUMP_CONFIG; 
        }
    }

    parse_table_init(unit, &pt);
    parse_table_add(&pt, "phy_id", PQ_DFL | PQ_INT, (void *)(0), &phy_id, NULL);
    parse_table_add(&pt, "speed", PQ_DFL | PQ_INT, (void *)0, &speed, NULL);
    parse_table_add(&pt, "iftype", PQ_DFL | PQ_STRING, (void *)(0), &iftype, NULL);
    parse_table_add(&pt, "if", PQ_DFL | PQ_STRING, (void *)(0), &side, NULL);
    ...
    if ( action & RUN_SESTO_SAMPLE ) {
        sesto_sample(unit, phy_id);    
    }
    if ( action & RUN_SESTO_INIT ) {
        sesto_init(unit, phy_id, speed);    
    }

    return CMD_OK;
}

#endif /* PHYMOD_EPIL_SUPPORT */
