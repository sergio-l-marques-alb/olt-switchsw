...
diag_dnx_export_stat_path_dump(int unit, path_dump_info_t *path_dump_info_p)
{
    ...
    if (ret_val == SOC_PPC_DIAG_OK) {  
        ...
        ret = diag_dnx_is_port_special(unit, prm_rcvd_pkt_info.in_pp_port, BCM_PORT_INTERNAL_OLP, &is_olp_port);
        if(ret != CMD_OK) {
            return ret;
        }

        /* check if port is OAMP */ 
        ret = diag_dnx_is_port_special(unit, prm_rcvd_pkt_info.in_pp_port, BCM_PORT_INTERNAL_OAMP, &is_oamp_port);
        if(ret != CMD_OK) {
            return ret;
        }
