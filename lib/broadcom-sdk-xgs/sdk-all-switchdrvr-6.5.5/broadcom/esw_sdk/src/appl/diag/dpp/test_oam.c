...
test_run_oam(int unit, int port_1, int port_2, int numEndpoints, int seed, int numUpdate) {
   ...
   timein_event_count   = sal_alloc(numEndpoints*2*4,"timein_event_count"); /* Length = 4words * 2 * numEndpoints */
   timeout_event_count  = sal_alloc(numEndpoints*2*4,"timein_event_count"); /* Length = 4words * 2 * numEndpoints */
   rmeps                = sal_alloc(numEndpoints*2*4,"timein_event_count"); /* Length = 4words * 2 * numEndpoints */
   lmeps                = sal_alloc(numEndpoints*2*4,"timein_event_count"); /* Length = 4words * 2 * numEndpoints */
   gport_list           = sal_alloc(numEndpoints*2*4,"timein_event_count"); /* Length = 4words * 2 * numEndpoints */
   ...
   */
   ...
   if (((time_acc) > max_total_endpoint_create_time) && (time_no > 0)) {
