...
extern int diag_sand_all_reg_get(int unit, int is_debug);

static cmd_result_t diag_sand_do_dump_table_field(int unit, soc_mem_t mem, unsigned array_index, int copyno, int index, int count, int flags, char *field);
static void diag_sand_mem_entry_dump_field(int unit, soc_mem_t mem, void *buf, char* field_name);
...
STATIC void
diag_sand_check_global(int unit, soc_mem_t mem, char *s, int *is_global)
{
...
STATIC int
diag_sand_collect_args_with_view(args_t *a, char *valstr, char *first,
...
STATIC int
diag_sand_mem_modify_fields(int unit, soc_mem_t mem, uint32 *entry,
...
STATIC int
diag_sand_parse_dwords(int unit, int count, uint32 *dw, args_t *a)
{
    char        *s;
    int         i;
 ...
 */
STATIC void
diag_sand_mem_list_summary(int unit, char *substr_match)
{
    soc_mem_t   mem;
    int         i, copies, dlen;
    int         found = 0;
...
STATIC int
diag_sand_mem_list_cache(int unit, soc_mem_t mem, void* en)
{
...
STATIC int
diag_sand_mem_list_dynamic(int unit, soc_mem_t mem, void* en)
{
...
STATIC int
diag_sand_active_intr_check(int unit, int inter)
{
...
STATIC int
diag_sand_mem_list_ser_action(int unit)
{
    int nof_interrupts;
    int inter = 0;
    ...
    for (inter = 0; inter < nof_interrupts; inter++) {
        ...
        if (diag_sand_active_intr_check(unit, inter)) {
            ...
            rc = bcm_switch_event_control_get(unit, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, bcm_switch_event_control, &is_enable);
            if (rc != BCM_E_NONE)
            {
        ...
        bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "%-65s, %-6d, %-35s, %-10d, %-11d, %s\n",
                                        ...
                                        interrupts[inter].recurring_action_cycle_time,
                                        interrupts[inter].recurring_action_cycle_counting,
                                        diag_sand_int_corr_act_name[recur_action]);
#endif
    }
...
cmd_result_t
cmd_dnx_mem_list(int unit, args_t *a)
{
    soc_mem_info_t      *m;
    soc_field_info_t    *fld;
    char                *tab, *s;
...
static cmd_result_t
diag_sand_do_dump_table(int unit, soc_mem_t mem, unsigned array_index,
...
STATIC cmd_result_t
diag_sand_do_dump_table_field(int unit, soc_mem_t mem, unsigned array_index,
 ...
 */
static void
diag_sand_mem_entry_dump_field(int unit, soc_mem_t mem, void *buf, char* field_name)
{
...
STATIC int 
diag_sand_filter_mems(int unit , int mem){
    switch (mem) {
...
STATIC int
diag_sand_mem_get_all(int unit, int flags)
{
    soc_mem_t mem;
    int i, rv;
...
static void
diag_sand_pci_print_config(int unit)
{
    uint32 data;
    int
        cap_len,
...
cmd_result_t
cmd_dnx_dump(int unit, args_t *a)
{
    soc_mem_t mem;
    char *arg1, *arg2, *arg3;
    ...
    if(!sal_strcasecmp(arg1, "soc")) {
        rv = diag_sand_all_reg_get(unit,is_debug);
        goto done;
    }
...
STATIC cmd_result_t
diag_sand_mem_write(int unit, args_t *a, int mod)
{
    ...
    if (!isint(s)) {
        ...
        if (view_len == 0) {
            ...
            if (diag_sand_collect_args_with_view(a, valstr, s, view, unit, mem)
                < 0) {
...
cmd_result_t
cmd_dnx_mem_write(int unit, args_t *a)
{
    return diag_sand_mem_write(unit, a, 0);
}


char cmd_dnx_mem_modify_usage[] =
    "Parameters: <TABLE>[.<COPY>] <ENTRY> <ENTRYCOUNT>\n\t"
    "        <FIELD>=<VALUE>[,...]\n\t"
    "Read/modify/write field(s) of a table entry(s).\n";

cmd_result_t
cmd_dnx_mem_modify(int unit, args_t *a)
{
