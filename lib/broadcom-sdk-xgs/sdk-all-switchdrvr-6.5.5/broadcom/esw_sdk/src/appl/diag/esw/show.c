        ...
        if (soc_feature(unit, soc_feature_logical_port_num)) {
            if (!soc_feature(unit, soc_feature_two_ingress_pipes) &&
                ...
                cli_out("             pm(pgw)  logical  "
                       "physical   lanes   speed_max   mmu   ucq/Numq    mcq/Numq\n");
            } else {
            ...
            PBMP_ALL_ITER(unit, port) {
                ...
                int num_lanes, speed_max, pm, pgw;
                ...
                if (!soc_feature(unit, soc_feature_two_ingress_pipes) &&
                    !soc_feature(unit, soc_feature_multi_pipe_mapped_ports)) {
                    cli_out("  %8s   %2d(%d)     %3d       %3d       "
                            "%2d       %6d   %3d    %4d/%-4d   %4d/%-4d\n",
                            SOC_INFO(unit).port_name[port], (pm < 0 ? 0: pm),
                            (pgw < 0 ? 0 : pgw), port, phy_port, num_lanes,
                            speed_max, mmu_port, uc_cosq, uc_numq, cosq, numq);
                } else {
                    cli_out("  %8s    %3d   %3d       %3d       "
                            "%3d      %4d/%-4d            %4d/%-4d\n",
