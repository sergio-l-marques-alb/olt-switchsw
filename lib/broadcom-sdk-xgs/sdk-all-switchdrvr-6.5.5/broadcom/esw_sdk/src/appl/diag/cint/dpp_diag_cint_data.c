...
static int diag_mbist(int unit, int skip_errors)
{
    ...
    if (SOC_IS_DPP(unit) || SOC_IS_DFE(unit)) {
        uint32 bist_orig_val = soc_property_get(unit,spn_BIST_ENABLE,0);
        char command[256];
        uint16 dev_id;
        uint8 rev_id;
        appl_dcmn_init_param_t init_param;
        void *p;
        args_t *ap = sal_alloc(sizeof(args_t), "args");

        if(ap == NULL) {
            return SOC_E_MEMORY;
        }
        sal_memset(ap, 0, sizeof(args_t));
        sal_memset(&init_param, 0x0, sizeof(appl_dcmn_init_param_t));
        ap->a_argv[0]="ALL";
        ap->a_argc=0;
        init_param.unit=unit;
        init_param.no_attach=1;
        init_param.modid = -1;    
        init_param.base_modid = -1;
        appl_dcmn_init_usage_parse(unit, ap, &init_param);
        p = &init_param;
        soc_cm_get_id (unit, &dev_id, &rev_id);
        sal_sprintf(command,"config delete %s.BCM8%x",spn_BIST_ENABLE,dev_id);
        sh_process_command(unit,command);
        sal_sprintf(command,"config add %s.BCM8%x=%d",spn_BIST_ENABLE,dev_id,1);
        sh_process_command(unit,command);
        if ((rv = init_deinit_test(unit, ap, p))) { /* perform deinit and init */
            cli_out("MBIST failed\n");
        }
        sal_sprintf(command,"config delete %s.BCM8%x",spn_BIST_ENABLE,dev_id);
        sh_process_command(unit,command);
        sal_sprintf(command,"config add %s.BCM8%x=%d",spn_BIST_ENABLE,dev_id,bist_orig_val);
        sh_process_command(unit,command);
        sal_free(ap);
