            ...
            for (i = 0; i < alist.count; i++) {
                ...
                if (!isint(valstr)) {  /* Must modify registers */
                    ...
                    if (mod) {
                        rv = CMD_OK;
                        if (soc_cpureg == SOC_REG_INFO(unit,ainfo->reg).regtype) {
                            addr = soc_reg_addr_get(unit, ainfo->reg, ainfo->block, ainfo->idx, SOC_REG_ADDR_OPTION_WRITE, &blk, &acc_type);
                            ...
                            rv = soc_custom_reg_above_64_get(unit, ainfo->reg, ainfo->port, SOC_REG_IS_ARRAY(unit, ainfo->reg) ? ainfo->idx : 0, regval);
                        } else {
...
char cmd_dpp_pcie_reg_usage[] =
    "\n\tRead/Write By pcie\n\t"
    "Usages:\n\t"
    "pcie [iproc] write <reg_address> <value>\n\t"
    "pcie [iproc] read <reg_address>\n\t"
    "Notice:\n\t"
...
cmd_dpp_pcie_reg(int unit, args_t* a)
{
    ...
    if ((!function) || (!addr)) {
        ...
        iproc ? 
          CMVEC(unit).iproc_write(&CMDEV(unit).dev, int_addr, int_value) :
        ...
        int_value = iproc ? 
          CMVEC(unit).iproc_read(&CMDEV(unit).dev, int_addr) :
          CMVEC(unit).read(&CMDEV(unit).dev, int_addr);
...
#endif /* (defined(LINUX) || defined(UNIX)) && (defined(__DUNE_GTO_BCM_CPU__) || defined(__DUNE_WRX_BCM_CPU__)) */
