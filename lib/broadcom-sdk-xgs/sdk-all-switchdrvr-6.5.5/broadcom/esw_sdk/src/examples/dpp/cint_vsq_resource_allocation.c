...
cint_gl_vsq_resource_allocate(int unit)
{
    ...
    for (core = 0; core < NOF_CORES; ++core) {
        ...
        for (is_ocb_only = 0; is_ocb_only < 2; ++is_ocb_only) {
            ...
            for (pool_id = 0; pool_id < 2; ++pool_id) {
                ...
                if (BCM_FAILURE(rv)) {
                    printf("cint_gl_vsq_resource_allocate: bcm_cosq_resource_allocation_set failed, core=%d, is_ocb_only=%d pool_id=%d.\n", core, is_ocb_only, pool_id);
                    return(rv);
                }
            }
        }
    ...
    printf("cint_gl_vsq_resource_allocate: PASS\n");
    return rv;
}
...
cint_src_vsqs_create(int unit, bcm_port_t src_port, bcm_gport_t *src_vsq_gport, bcm_gport_t *pg_base_vsq_gport)
{
    ...
    for (cosq = 0; cosq < vsq_config.numq; cosq++) {
        ...
        vsq_config.pg_attributes[cosq].is_lossles = 1;  /* For loss queue (not using headroom), change to 0 */
    }
    ...
    if (BCM_FAILURE(rv)) {
        printf("cint_src_vsqs_create: bcm_cosq_src_vsqs_gport_add failed, src_port(%d), Error (%d, %s)\n", src_port, rv, bcm_errmsg(rv));
        return(rv);
    } else {
        printf("cint_src_vsqs_create: bcm_cosq_src_vsqs_gport_add success, src_port(%d) src_vsq_gport(0x%x) pg_base_vsq_gport(0x%x)\n",
                src_port, *src_vsq_gport, *pg_base_vsq_gport);
    }

    printf("cint_src_vsqs_create: PASS\n");
    return rv;
}
 ...
 */
int
cint_src_vsqs_resource_allocate(int unit, int numq, uint8 pool_id, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
    bcm_error_t rv;
    int index = -1;
    int offset = -1;
    ...
    printf("cint_src_vsqs_resource_allocate: PASS\n");
    return rv;
}
...
cint_src_vsqs_pfc_setup(int unit, int numq, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
    ...
    printf("cint_src_vsqs_pfc_setup: PASS\n");
    return rv;
}
 ...
 */
int
cint_vsq_resource_allocation_setup(int unit, bcm_port_t src_port, int nof_dest_ports, int *dest_port_base_queue)
{
    bcm_error_t rv;
