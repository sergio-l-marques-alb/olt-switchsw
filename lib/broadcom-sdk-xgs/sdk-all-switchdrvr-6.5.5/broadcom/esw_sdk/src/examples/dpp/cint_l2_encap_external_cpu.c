 ...
 * ANY FAILURE OF ESSENTIAL PURPOSE OF ANY LIMITED REMEDY.$SDK/src/examples/dpp/cint cint_l2_encap_external_cpu.c
 * 
 * configuration
...
/*  Create preselector with qualifer src Port */
	...
	rv = bcm_field_qualify_InPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, src_port, src_port_mask);
	if (rv != BCM_E_NONE) 
	{
