...
int tx_packet_via_bcm(int unit, char *data, int *ptch) {
    char tmp, data_iter;
    ...
    rv = bcm_port_get(unit, ptch[1], &dummy_flags, &interface_info, &mapping_info);
    if (BCM_FAILURE(rv)) {
    ...
    pkt_info->_pkt_data.data[1] = mapping_info.tm_port;
