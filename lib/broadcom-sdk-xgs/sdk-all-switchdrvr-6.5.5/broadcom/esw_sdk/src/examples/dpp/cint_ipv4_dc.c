...
int ipv4_dc_drop_by_pmf(int unit, int hitbit){
	...
	rv = configure_pmf_action_by_hit_bit_example(unit,hitbit,bcmFieldAppTypeIp4DoubleCapacity, bcmFieldActionDrop, 0, 0);
	if (rv) {
...
int ipv4_dc_drop_by_pmf_by_value(int unit, int hitbit, uint32 val0, uint32 val1, uint32 mask0, uint32 mask1){
	...
	rv = configure_pmf_action_by_hit_value_example(unit, hitbit, val, mask, bcmFieldAppTypeIp4DoubleCapacity, 0, bcmFieldActionDrop, 0, 0, 0);
	if (rv) {
