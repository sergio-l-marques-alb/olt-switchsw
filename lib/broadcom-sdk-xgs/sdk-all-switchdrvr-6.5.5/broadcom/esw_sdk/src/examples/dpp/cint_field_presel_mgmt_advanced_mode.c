                        ...
                        /* in */ bcm_port_t egr_port )
{
    ...
    if (!in_stage) {
        result = bcm_field_qualify_InPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, ing_port, 0xffffffff);	
        if (BCM_E_NONE != result) {
    ...
    {
        result = bcm_field_qualify_OutPort(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, egr_port, 0xffffffff);	
        if (BCM_E_NONE != result) {
	...
	result = bcm_field_entry_install(unit,  presel_id | BCM_FIELD_QUALIFY_PRESEL);
    if (BCM_E_NONE != result) {
                        ...
                        /* in */ bcm_port_t egr_port )
{
	...
	if(in_stage == 0) {
        result = bcm_field_qualify_IpType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldIpTypeIpv4Any);	
        if (BCM_E_NONE != result) {
                        ...
                        /* in */ bcm_port_t egr_port )
{
	...
	/* Cretae a presel entity */
	result = bcm_field_presel_create_id(unit, low_presel_id);
	if (BCM_E_NONE != result) {
		printf("Error in bcm_field_presel_create_id\n");
		auxRes = bcm_field_presel_destroy(unit, presel_id);
    return result;
	...
	/* Depending on stage, configure the presel stage and in/out port qualifier */	
    result = bcm_field_qualify_Stage(unit, low_presel_id | BCM_FIELD_QUALIFY_PRESEL, stage);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_qualify_Stage\n");
        return result;
    }
    ...
    if (!in_stage) {
        result = bcm_field_qualify_InPort(unit, low_presel_id | BCM_FIELD_QUALIFY_PRESEL, ing_port, 0xffffffff);	
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_InPort\n");
            return result;
        }
    ...
    {
        result = bcm_field_qualify_OutPort(unit, low_presel_id | BCM_FIELD_QUALIFY_PRESEL, egr_port, 0xffffffff);	
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_OutPort\n");
            return result;
        }
	...
	result = bcm_field_entry_install(unit,  low_presel_id | BCM_FIELD_QUALIFY_PRESEL);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_entry_install\n");
    }
  ...
  /*create program and attach group for higher prioirity presel*/
	/* Cretae a presel entity */
	...
	/* Depending on stage, configure the presel stage and in/out port qualifier */	
    result = bcm_field_qualify_Stage(unit, high_presel_id | BCM_FIELD_QUALIFY_PRESEL, stage);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_qualify_Stage\n");
        return result;
    }
	...
	if(in_stage == 0) {
        result = bcm_field_qualify_IpType(unit, high_presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldIpTypeIpv4Any);	
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_IpType\n");
            return result;
        }
    } else {
        result = bcm_field_qualify_ForwardingType(unit, high_presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeL2);	
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_IpType\n");
            return result;
        }
	...
	result = bcm_field_entry_install(unit,  high_presel_id | BCM_FIELD_QUALIFY_PRESEL);
    if (BCM_E_NONE != result) {
        printf("Error in bcm_field_entry_install\n");
    }
