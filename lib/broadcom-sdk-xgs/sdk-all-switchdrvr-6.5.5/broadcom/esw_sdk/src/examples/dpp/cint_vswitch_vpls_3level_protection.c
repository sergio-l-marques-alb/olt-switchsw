...
void
vswitch_vpls_hvpls_info_init(int unit, int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan, int vpn_id) {
...
vswitch_vpls_hvpls_ingress_mpls_port_create(int unit){
    ...
    for (is_primary=0; is_primary<=1; is_primary++) {
        ...
        if (mpls_termination_label_index_enable) {
            ...
            BCM_MPLS_INDEXED_LABEL_SET(&mpls_port_ingress.match_label, val, 1);
        }
...
vswitch_vpls_3level_protection_run_with_defaults(int unit){
    ...
    vswitch_vpls_hvpls_info_init(unit,1,13,13,10,20,6202);
...
vswitch_vpls_3level_protection_run_with_defaults_dvapi(int unit, int acP, int pweP){
    ...
    vswitch_vpls_hvpls_info_init(unit,acP,pweP,pweP,10,20,6202);
