...
int verbose1 = 1;
int tunnel_id;
nph_service_info_egress_s nph_service_egress_info_for_protection[4];
...
int nph_service_info_egress_init(uint32 index, bcm_port_t np_port, uint32 ps, uint32 output_fp, bcm_gport_t gport)
{
    ...
    nph_service_egress_info_for_protection[index].np_port = np_port;
    nph_service_egress_info_for_protection[index].ps = ps;
    nph_service_egress_info_for_protection[index].output_fp = output_fp;
    nph_service_egress_info_for_protection[index].gport = gport;
...
int nph_vpws_service_egress_run(int unit)
{
    ...
    for (index=0; index<nph_nof_services; index++) {
        ...
        vp.port = nph_service_egress_info_for_protection[index].np_port;
        vp.flags = BCM_VLAN_PORT_CREATE_INGRESS_ONLY | BCM_VLAN_PORT_INGRESS_WIDE;
        vp.criteria = BCM_VLAN_PORT_MATCH_PORT_TUNNEL;
        vp.match_tunnel_value = (nph_service_egress_info_for_protection[index].ps << 17) | nph_service_egress_info_for_protection[index].output_fp;
        ...
        nph_service_egress_info_for_protection[index].vlan_port_id = vp.vlan_port_id;

        printf("Created VLAN port for {ps, output_fp} (%d, 0x%x)\n", nph_service_egress_info_for_protection[index].ps, nph_service_egress_info_for_protection[index].output_fp);

        bcm_vswitch_cross_connect_t_init(&gports);
        gports.flags = BCM_VSWITCH_CROSS_CONNECT_DIRECTIONAL;
        gports.port1 = nph_service_egress_info_for_protection[index].vlan_port_id;
        gports.port2 = nph_service_egress_info_for_protection[index].gport;
       ...
       /*for p2p destination service on egress chip, it should be egress only style*/
        if (BCM_GPORT_IS_MPLS_PORT(gports.port2)
             &&((BCM_GPORT_SUB_TYPE_LIF_EXC_GET(gports.port2) == BCM_GPORT_SUB_TYPE_LIF_EXC_EGRESS_ONLY))){
              ...
              gports.encap2 = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(nph_service_egress_info_for_protection[index].gport);
        }
...
int nph_mpls_egr_fwd_code_and_offset_set(int unit)
{
    ...
    uint8 mask;
    int pri=10;
    int fwd_code_b_f_flag_qual_id,fwd_code_sub_type_qual_id;
    int rv;
    ...
    fwd_code_sub_type_qual_id = data_qual.qual_id;
    ...
    fwd_code_b_f_flag_qual_id = data_qual.qual_id;
    ...
    for (i=0; i<sizeof(field_0)/sizeof(field_0[0]); i++) {
        ...
        bcm_field_qualify_InPort(unit, ent,ilk_port,0xffffffff);

        data = field_0[i];
        mask = mask_0[i];

        rv = bcm_field_qualify_data(unit, ent, fwd_code_sub_type_qual_id, &data, &mask, 1);
        if (rv != BCM_E_NONE) {
        ...
        if (b_f_flag[i] != 0) {
            ...
            rv = bcm_field_qualify_data(unit, ent, fwd_code_b_f_flag_qual_id, &data, &mask, 1);
            if (rv != BCM_E_NONE) {
...
int protection_mpls_lsr_frr_group_create(int unit)
{
    ...
    data_entry_init(pro_data_entry_info.backuplsp_outport,pro_data_entry_info.backuplsp_flowid,pro_data_entry_info.backuplsp_outslot,pro_data_entry_info.lsp_fp,0/*PS:0b000*/,1,3);
    data_format_entry_create(unit,&pro_data_entry_info.backuplsp_eedb_index);

    data_entry_init(pro_data_entry_info.primarylsp_outport,pro_data_entry_info.primarylsp_flowid,pro_data_entry_info.primarylsp_outslot,pro_data_entry_info.lsp_fp,1/*PS:0b000*/,1,3);
...
int protection_mpls_lsr_frr_egress_config(int unit)
{
    ...
    int pp_port = ilk_port;


    mapping_info.user_define_value=((nph_uc&0x1)<<13) | ((nph_nni&0x7)<<10)|(nph_sub_type_swap << 6) |18/*del_len 18*/;
