...
_bcm_tr3_extender_reinit(int unit)
{
    ...
    uint32 entry_type, vp, trunk_bit, vntag_action;
    bcm_trunk_t tgid;
    ...
    for (i = index_min; i <= index_max; i++) {
        ...
        vp = soc_EGR_L3_NEXT_HOPm_field32_get(unit, egr_nh_entry,
                SD_TAG__DVPf);
        /* Extender Port addition will always use below ETAG setting
         * in EGR_L3_NEXT_HOP. If this setting is absent it is not an
         * extender port.
         */
        ...
        switch (soc_EGR_L3_NEXT_HOPm_field32_get(unit, egr_nh_entry,
            ...
            case 3:
                ...
                break;
            default:
                /* Above two cases cover the Extender ports type and in any
                 * other case we can skip reinit in level 1 Warm Boot.
                 */
                continue;
        }
                         ...
                         bcm_extender_port_t *extender_port)
{
    ...
    if (!(extender_port->flags & BCM_EXTENDER_PORT_REPLACE)) {
        ...
        rv = _bcm_tr3_extender_nh_info_set(unit, extender_port, vp, drop,
                                              &nh_index);
        if (rv < 0) {
            goto cleanup;
        }
        ...
        drop = (extender_port->flags & BCM_EXTENDER_PORT_DROP) ? 1 : 0;
        BCM_IF_ERROR_RETURN
            (_bcm_tr3_extender_nh_info_set(unit, extender_port, vp, drop,
                                           &nh_index));
