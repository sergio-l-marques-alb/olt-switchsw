                           ...
                           _field_entry_wb_info_t *f_ent_wb_info)
{
    ...
    if ((fc->wb_recovered_version) >= BCM_FIELD_WB_VERSION_1_15) {
          ...
          if ((fc->wb_recovered_version) >= BCM_FIELD_WB_VERSION_1_20) {
             if (SHR_BITGET(f_ent_wb_info->action_bmp->w, _bcmFieldActionNoParamMirrorIngress)) {
                f_ent_wb_info->ingress_mirror = buf[fc->scache_pos];
                fc->scache_pos++;
                f_ent_wb_info->ingress_mirror |= buf[fc->scache_pos] << 8;
                fc->scache_pos++;
                f_ent_wb_info->ingress_mirror |= buf[fc->scache_pos] << 16;
                fc->scache_pos++;
                f_ent_wb_info->ingress_mirror |= buf[fc->scache_pos] << 24;
                fc->scache_pos++;
             }
             if (SHR_BITGET(f_ent_wb_info->action_bmp->w, _bcmFieldActionNoParamMirrorEgress)) {
                f_ent_wb_info->egress_mirror = buf[fc->scache_pos];
                fc->scache_pos++;
                f_ent_wb_info->egress_mirror |= buf[fc->scache_pos] << 8;
                fc->scache_pos++;
                f_ent_wb_info->egress_mirror |= buf[fc->scache_pos] << 16;
                fc->scache_pos++;
                f_ent_wb_info->egress_mirror |= buf[fc->scache_pos] << 24;
                fc->scache_pos++;
             }
                                ...
                                _field_stage_t *stage_fc)
{
    ...
    while (group != NULL) {
           if (group->stage_id == _BCM_FIELD_STAGE_INGRESS) {
               if (!BCM_PBMP_EQ(PBMP_ALL(unit),fg->pbmp)) {
...
#define BCM_IF_ERROR_RETURN_WITH_FREE(_rv_, e, p, max_idx) {    \
    ...
    e = (uint32 *) sal_alloc(SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32), "ACL Control");
        if (!e) {
            return (BCM_E_MEMORY);
        }

        p = (uint32 **) sal_alloc(_FP_MAX_ENTRY_WIDTH * sizeof(uint32*), "ACL Control");
        if (!p)
        {
            sal_free(e);
            return (BCM_E_MEMORY);
        ...
        {
            p[i]= (uint32 *) sal_alloc (SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32), "ACL Control");
            if (!p[i])
            {
                while (i > 0) {
                    sal_free(p[--i]);
