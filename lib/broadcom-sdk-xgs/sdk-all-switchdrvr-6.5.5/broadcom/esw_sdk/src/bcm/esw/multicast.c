...
#if defined (BCM_SABER2_SUPPORT)
            if (SOC_IS_SABER2(unit)){
                _bcm_sb2_ipmc_port_ext_queue_count_increment(unit, mc_index, temp_port);
            } else
#endif
            {
...
#if defined(BCM_KATANA_SUPPORT) && defined(INCLUDE_L3)
        if (SOC_IS_KATANAX(unit)) {
            /* Deleting all the current replication lists before setting the new
             * list for the given multicast group.
             */
