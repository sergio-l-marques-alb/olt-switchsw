    ...
    bcm_stat_lif_counting_t *lif_counting)
{
    ...
    rv = bcm_dpp_counter_lif_range_get(unit, lif_counting_profile, &source, &(lif_counting->source.command_id), /* T.B.D - stif_id, */ &range);  
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_stat_lif_counting_t *lif_counting)
{
    ...
    if(SOC_IS_JERICHO_PLUS(unit) && lif_counting->source.type == bcmStatCounterSourceEgressTransmitOutlif)
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported API for device type.")));        
    }
    BCMDNX_NULL_CHECK(lif_counting);
...
*/    
    ...
    rv = bcm_dpp_counter_lif_range_set(unit, lif_counting_profile, source, lif_counting->source.command_id /*for jericho - command ID is equal to range ID*/, &range);
    BCMDNX_IF_ERR_EXIT(rv);
