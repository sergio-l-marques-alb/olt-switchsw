 ...
 */
static void  _bcm_oam_counter_base_id_to_classifier_counter(bcm_oam_endpoint_info_t * endpoint_info_ptr, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_ptr) {
    classifier_mep_ptr->counter = endpoint_info_ptr->lm_counter_base_id; 
    classifier_mep_ptr->flags |= (endpoint_info_ptr->lm_flags & BCM_OAM_LM_PCP)? SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP : 0; 
}


static void  _bcm_oam_counter_base_id_from_classifier_counter(bcm_oam_endpoint_info_t * endpoint_info_ptr, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_ptr) {
    endpoint_info_ptr->lm_counter_base_id = classifier_mep_ptr->counter;
    endpoint_info_ptr->lm_flags |= (classifier_mep_ptr->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP)? BCM_OAM_LM_PCP : 0;
}
...
_bcm_dpp_oam_bfd_flexible_verification_expected_crc16_get(_BCM_OAM_OAMP_CRC_BUFFER buffer, SOC_PPC_OAM_OAMP_CRC_MASK *mask, uint16 *crc_p){
    ...
    for (byte_iteration = 0 ; byte_iteration < (_BCM_OAM_OAMP_CRC_BUFFER_SIZE / _BCM_OAM_OAMP_CRC_CALC_NOF_BYTES_PER_ITERATION) ; byte_iteration++){
        ...
        for (cur_byte = 0 ; cur_byte < _BCM_OAM_OAMP_CRC_CALC_NOF_BYTES_PER_ITERATION ; cur_byte++) {
            ...
            if (byte < SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE) {
                byte_val = (buffer[byte] & mask->msb_mask[byte]); 
            } else {
                byte_val = (SHR_BITGET(mask->lsbyte_mask, byte - SOC_PPC_OAM_OAMP_CRC_MASK_LSB_BYTE_SIZE)) ? buffer[byte] : 0;
            }
