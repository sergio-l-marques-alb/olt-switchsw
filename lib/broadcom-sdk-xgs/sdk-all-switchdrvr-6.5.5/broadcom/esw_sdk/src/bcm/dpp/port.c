    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
#if defined(BCM_ARAD_SUPPORT)
        if (lc != NULL) {
            (void)bcm_petra_port_enable_get(unit, port_i, &link);
            if (link == TRUE) {
                if (SOC_PBMP_MEMBER(lc->pbm_override_ports, port_i)) {
                    if(!SOC_PBMP_MEMBER(lc->pbm_override_link, port_i)) {
                        link = FALSE;
                    }
                }
            }
        ...
        if (lc != NULL) {
            if ((loopback == BCM_PORT_LOOPBACK_NONE) || BCM_FAILURE(rv)) {
                (void)_bcm_link_force(unit, port, FALSE, FALSE);
            } else {
    ...
    switch (type)
    {
        ...
        case bcmPortControlReserved281:
            if (SOC_DPP_CONFIG(unit)->pp.custom_feature_vrrp_scaling_tcam) {
                ...
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            } else if (SOC_DPP_CONFIG(unit)->pp.custom_feature_mpls_port_termination) {
                ARAD_PP_IHP_VTT_IN_PP_PORT_CONFIG_TBL_DATA vtt_in_pp_port_conf_tbl;
                SOC_PPC_PORT soc_ppd_port = 0;
                int core;
                ...
                /* For MPLS port termination - Port is configured with profile in order for program selection to happen only for this specific port   */ 
                vtt_in_pp_port_conf_tbl.vlan_translation_profile = SOC_PPC_PORT_DEFINED_VT_PORT_TERMINATION_MPLS;
                soc_sand_rv = arad_pp_ihp_vtt_in_pp_port_config_tbl_set_unsafe(unit, core, soc_ppd_port, &vtt_in_pp_port_conf_tbl);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                vtt_in_pp_port_conf_tbl.termination_profile = SOC_PPC_PORT_DEFINED_TT_PROFILE_PORT_TERMINATION_MPLS;
                soc_sand_rv = arad_pp_ihp_vtt_in_pp_port_config_tbl_set_unsafe(unit, core, soc_ppd_port, &vtt_in_pp_port_conf_tbl);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
int bcm_petra_port_add(int unit, bcm_port_t port, uint32 flags, bcm_port_interface_info_t *interface_info, bcm_port_mapping_info_t *mapping_info)  
{
    ...
    SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_nif_sku_restrictions, (unit, phy_ports, interface_info->interface, interface_info->interface_id, 0)));
...
int bcm_petra_tpid_profile_info_set(
      int unit, int port_profile,int port_profile_eg, _bcm_petra_tpid_profile_t profile_type, int accept_mode, _bcm_petra_dtag_mode_t dtag_mode, _bcm_petra_ac_key_map_type_t ac_key_map_type, uint8 cep_port, uint8 evb_port
    )
{
    ...
    for(prs_nfo.outer_tpid = SOC_PPC_LLP_PARSE_TPID_INDEX_NONE; prs_nfo.outer_tpid < SOC_PPC_NOF_LLP_PARSE_TPID_INDEXS; ++prs_nfo.outer_tpid) {
        for(prs_nfo.inner_tpid = SOC_PPC_LLP_PARSE_TPID_INDEX_NONE; prs_nfo.inner_tpid < SOC_PPC_NOF_LLP_PARSE_TPID_INDEXS; ++prs_nfo.inner_tpid) {
            for (prs_nfo.is_outer_prio = FALSE; prs_nfo.is_outer_prio <= TRUE; ++prs_nfo.is_outer_prio) {
                ...
                if(prs_nfo.outer_tpid == SOC_PPC_LLP_PARSE_TPID_INDEX_NONE && (prs_nfo.inner_tpid != SOC_PPC_LLP_PARSE_TPID_INDEX_NONE|| prs_nfo.is_outer_prio)) {
                    continue;
                }
                is_double_tagged = 0;
                ...
                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_llp_parse_packet_format_eg_info_get, (soc_sand_dev_id, port_profile_eg, &prs_nfo,&pkt_frmt_info_eg));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                /* set tag format according to profile type and found TPIDs */
                /* packet with I-tag is not considered tagged */ 
                BCMDNX_IF_ERR_EXIT(_bcm_petra_tpid_profile_to_tag_format(
                                        unit, 
                                        profile_type, 
                                        prs_nfo.outer_tpid, 
                                        prs_nfo.inner_tpid, 
                ...
                switch (accept_mode) {
                    ...
                    case BCM_PORT_DISCARD_UNTAG:
                        pkt_frmt_info.action_trap_code = (is_tagged)?SOC_PPC_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_ACCEPT:SOC_PPC_TRAP_CODE_ACCEPTABLE_FRAME_TYPE_DROP;
                    break;
                ...
                if (is_mim_mode && prs_nfo.outer_tpid != SOC_PPC_LLP_PARSE_TPID_INDEX_ISID_TPID && prs_nfo.inner_tpid != SOC_PPC_LLP_PARSE_TPID_INDEX_ISID_TPID) {
                ...
                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_llp_parse_packet_format_eg_info_set, (soc_sand_dev_id, port_profile_eg, &prs_nfo,&pkt_frmt_info_eg));
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                BCM_DPP_UNIT_CHECK(unit);
           }
        }
...
_bcm_petra_port_assigned_tpids_get(
       int unit, SOC_PPC_PORT ppd_port_id, uint16 tpids[_BCM_PETRA_PORT_NOF_TPID_VALS], uint32 *nof_tpids
    )
{
   SOC_PPC_PORT_INFO port_info;
   uint32 tpid_indx=0;
...
_bcm_petra_port_tpid_indexes_get(
       int unit, SOC_PPC_PORT ppd_port_id, uint32 tpid_val, uint16 *tpid_vals, uint32 nof_tpid_vals,
    ...
    )
{
   ...
   BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      * get accept mode for this port
      ...
      if (!_BCM_DPP_TPID_PARSE_ADVANCED_MODE(unit)) {
          rv = bcm_petra_tpid_profile_info_set(unit, port_info.port_profile,port_info.port_profile_eg, new_profile_type, accept_mode, dtag_mode, ac_key_map_type, port_info.port_type == SOC_SAND_PP_PORT_L2_TYPE_CEP,evb_port);
          BCMDNX_IF_ERR_EXIT(rv);
      }
      else {
...
{   
    ...
    BCMDNX_ALLOC(lif_entry_info, sizeof(SOC_PPC_LIF_ENTRY_INFO), "_bcm_petra_tpid_profile_update.lif_entry_info");
    ...
    if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_PWE) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_ISID)) {
...
bcm_petra_port_tpid_class_set(int unit, bcm_port_tpid_class_t *tpid_class)
{
    ...
    int old_tpid_class_tmpl_eg,is_last_tpid_eg, new_tpid_class_tmpl_eg,is_first_tpid_eg;
    ...
    if (is_egress) {    
        ...
        rv = _bcm_dpp_am_template_port_tpid_class_eg_data_get(unit, gport_info.local_port,&tpid_class_buffer);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        /* now buffer ready with all information allocate port profile match this buffer */
        rv = _bcm_dpp_am_template_port_tpid_class_eg_exchange(unit,0,gport_info.local_port,&tpid_class_buffer,&old_tpid_class_tmpl_eg,&is_last_tpid_eg,&new_tpid_class_tmpl_eg,&is_first_tpid_eg);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if ((is_first_acceptable)||(is_first_tpid_eg)) {
            ...
            if ((new_acceptable_tmpl != old_acceptable_tmpl)||(new_tpid_class_tmpl_eg != old_tpid_class_tmpl_eg)) {
                for(prs_nfo.outer_tpid = SOC_PPC_LLP_PARSE_TPID_INDEX_NONE; prs_nfo.outer_tpid < SOC_PPC_NOF_LLP_PARSE_TPID_INDEXS; ++prs_nfo.outer_tpid) {
                    for(prs_nfo.inner_tpid = SOC_PPC_LLP_PARSE_TPID_INDEX_NONE; prs_nfo.inner_tpid < SOC_PPC_NOF_LLP_PARSE_TPID_INDEXS; ++prs_nfo.inner_tpid) {
                        for (prs_nfo.is_outer_prio = FALSE; prs_nfo.is_outer_prio <= TRUE; ++prs_nfo.is_outer_prio) {
                             if((is_first_tpid_eg)&&(new_tpid_class_tmpl_eg != old_tpid_class_tmpl_eg)){
                                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_llp_parse_packet_format_eg_info_get, (unit, old_tpid_class_tmpl_eg, &prs_nfo,&pkt_frmt_info));
                                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                                /* ppd call*/
                                soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_llp_parse_packet_format_eg_info_set,(unit, new_tpid_class_tmpl_eg, &prs_nfo,&pkt_frmt_info));
                                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                             }
                             if((is_first_acceptable)&&(new_acceptable_tmpl != old_acceptable_tmpl)){
                               /* get "egress acceptable frame filter" from old profile */
                               soc_sand_rv = soc_ppd_eg_filter_port_acceptable_frames_get(unit, old_acceptable_tmpl, &prs_nfo, &eg_filter_port_accept);
                               BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                               
                               /* update with "egress acceptable frame filter" for new profile */
                               soc_sand_rv =  soc_ppd_eg_filter_port_acceptable_frames_set(unit, new_acceptable_tmpl, port_info.port_profile_eg, &prs_nfo, eg_filter_port_accept);
                               BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                             }
                        }
                    }
                ...
                for(indx2 = 0; indx2 < port_tpid_nof_indexes[1]; ++indx2) {
                    for(indx3 = 0; indx3 < nof_prirority_types; ++indx3) {
                        ...
                        *buffer_lcl = tpid_class_acceptable_frame_info.buffer;
                        ...
                        /* ppd call*/
                        soc_sand_rv = soc_ppd_eg_filter_port_acceptable_frames_set(unit, (uint32)new_acceptable_tmpl, port_info.port_profile, &prs_nfo, eg_filter_port_accept);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                }
            }
...
bcm_petra_port_tpid_class_get(int unit, bcm_port_tpid_class_t *tpid_class)
{
    ...
    rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, local_port, &soc_ppd_port_i, &core));
    BCMDNX_IF_ERR_EXIT(rv);
    BCM_DPP_UNIT_CHECK(unit);
    /* 2. get port associated TPIDs */
    ...
    bcm_gport_t *tunnel_port_id)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_pp_port_mapping_bypass", 0) == 1) {
