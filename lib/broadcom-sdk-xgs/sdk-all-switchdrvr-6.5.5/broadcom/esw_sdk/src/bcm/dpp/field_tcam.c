...
STATIC int
_bcm_dpp_field_tcam_entry_info_get_from_sw_state(bcm_dpp_field_info_OLD_t *unitData,
                                                 _bcm_dpp_field_ent_idx_t entry,
                                                 uint8 *found,
...
/* build entry action data from sw state */
    ...
    if (egTrapInfo.bitmap_mask) {
         ...
         */
        if(!isExternalTcam) {
            /* retrieve egrTrapProfile from sw state */
            result = FIELD_ACCESS.entryTc.egrTrapProfile.get(unit, entry, &egrTrapProfile);
        }
                      ...
                      SOC_PPC_FP_ENTRY_INFO     *entInfoTc)
{
    ...
    if (SOC_IS_DONE_INIT(unit) && BCM_UNIT_DO_HW_READ_WRITE(unit)){
        return _bcm_dpp_field_tcam_entry_info_get_from_sw_state(unitData, entry, okay, entInfoTc);
    }
#endif /* CRASH_RECOVERY_SUPPORT */
