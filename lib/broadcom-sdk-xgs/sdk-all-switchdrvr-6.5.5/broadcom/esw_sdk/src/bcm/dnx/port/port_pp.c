  ...
  uint32 vlan_domain)
{
  ...
  SHR_INVOKE_VERIFY_DNX(dnx_port_pp_vlan_domain_set_verify
                        (unit, port, vlan_domain));

  /*
   * Get Port + Core 
   */
  SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_phy_port
                  (unit, port, 0 /*operations */ , &gport_info));

  /*
   * Write to INGRESS PORT table
   */
  SHR_IF_ERR_EXIT(dbal_entry_handle_take
                  (unit, DBAL_TABLE_INGRESS_PORT, &entry_handle_id));

  SHR_IF_ERR_EXIT(dbal_entry_field32_set
                  (unit, entry_handle_id, DBAL_FIELD_MODE_IS_KEY, DBAL_FIELD_PP_PORT,
                   gport_info.internal_port));
  SHR_IF_ERR_EXIT(dbal_entry_field32_set
                  (unit, entry_handle_id, DBAL_FIELD_MODE_IS_KEY, DBAL_FIELD_CORE_ID,
                   gport_info.core_id));

  SHR_IF_ERR_EXIT(dbal_entry_field32_set
                  (unit, entry_handle_id, DBAL_FIELD_MODE_IS_VALUE, DBAL_FIELD_VLAN_DOMAIN,
                   vlan_domain));
  ...
  SHR_IF_ERR_EXIT(dbal_entry_commit
                  (unit, entry_handle_id, DBAL_COMMIT_NORMAL));

  /*
   * Write to EGRESS PORT table
   */
  SHR_IF_ERR_EXIT(dbal_entry_handle_take
                  (unit, DBAL_TABLE_EGRESS_PORT, &entry_handle_id));
