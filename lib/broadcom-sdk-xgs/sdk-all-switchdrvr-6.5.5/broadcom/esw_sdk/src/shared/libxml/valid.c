...
fail:
    ...
    if ((warn) && ((ret != 1) && (ret != -3))) {
	    char expr[MAX_CONTENT_SIZE];
	    char list[MAX_CONTENT_SIZE];

	    expr[0] = 0;
	    xmlSnprintfElementContent(&expr[0], MAX_CONTENT_SIZE, cont, 1);
	    list[0] = 0;
#ifndef LIBXML_REGEXP_ENABLED
	    if (repl != NULL)
		xmlSnprintfElements(&list[0], MAX_CONTENT_SIZE, repl, 1);
	    else
#endif /* LIBXML_REGEXP_ENABLED */
		xmlSnprintfElements(&list[0], MAX_CONTENT_SIZE, child, 1);
