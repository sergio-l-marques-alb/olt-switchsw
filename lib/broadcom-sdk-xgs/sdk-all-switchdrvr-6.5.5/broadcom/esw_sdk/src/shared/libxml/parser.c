...
xmlParseReference(xmlParserCtxtPtr ctxt) {
	...
	/*
	 * This is a bit ackish but this seems the best
	 ...
	 * ack - maybe we should have further tests to determine
	 * what to do.
	 */
...
xmlCreatePushParserCtxt(xmlSAXHandlerPtr sax, void *user_data, 
    ...
    if (sax != NULL) {
#ifdef LIBXML_SAX1_ENABLED
    xmlSAXHandlerV1 *xmlDefaultSAXHandlerTmp = &xmlDefaultSAXHandler;
    if (ctxt->sax != (xmlSAXHandlerPtr) xmlDefaultSAXHandlerTmp)
#endif /* LIBXML_SAX1_ENABLED */
	    xmlFree(ctxt->sax);
...
xmlParseInNodeContext(xmlNodePtr node, const char *data, int datalen,
	...
	while ((cur != NULL) && (cur->type == XML_ELEMENT_NODE)) {
		...
		if (ctxt->dict) {
		    ...
		    if(nsPush(ctxt, iprefix, ihref) > 0)
		        nsnr++;
		}
		ns = ns->next;
	    }
