...
/* $Id: cint_mpls_elsp.c,v 1.5 Broadcom SDK $
...
int mpls_elsp_create_mapping(int unit, int second_unit, int outp){
    ...
    /* Initialise data */
    rv = mpls_lsr_init(outp, outp, 0x11, 0x22, 5000, 8000, 100,200,0);

    mpls_elsp_create_vlan(unit,second_unit);
    
    if (rv != BCM_E_NONE){
        printf("Error in mpls_lsr_init.");
        return rv;
    }
...
mpls_elsp_config_with_exp(int unit, int second_unit, int exp){
    ...
    if (second_unit >= 0)
    {
        ...
        rv = create_l3_egress(second_unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid, ingress_intf, mpls_lsr_info_1.next_hop_mac, &egress_intf, &encap_id);
        if (rv != BCM_E_NONE) {
        ...
        /* create egress object */
        rv = create_l3_egress(unit, BCM_L3_WITH_ID, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid, ingress_intf, mpls_lsr_info_1.next_hop_mac, &egress_intf, &encap_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_egress\n");
            return rv;
        }
    ...
    {
        /* create egress object */
        rv = create_l3_egress(unit, CINT_NO_FLAGS, mpls_lsr_info_1.eg_port, mpls_lsr_info_1.eg_vid, ingress_intf, mpls_lsr_info_1.next_hop_mac, &egress_intf, &encap_id);
        if (rv != BCM_E_NONE) {
            printf("Error, in create_l3_egress\n");
            return rv;
        } 
