...
int          L4_NUM_ELEMENTS = 24;
int          L3_NUM_ELEMENTS = 24;
                  ...
                  int num_l2_se)
{
    ...
    */
    ...
    BCM_PBMP_ITER(port_set,port) {
        ...
        for (prio=0; prio < HR_PER_PORTS; prio++) {
        ...
        for (l4_index = 0; l4_index < L4_NUM_ELEMENTS; l4_index++) {
            ...
            for (l3_index = 0; l3_index < L3_NUM_ELEMENTS; l3_index++) {
                for (se_elem = 0; se_elem < L2_NUM_ELEMENTS; se_elem++) {
                    ...
                    if ((rv = bcm_cosq_gport_add(unit, portmod_gport, 4, flags,
                                        &vc_group)) != BCM_E_NONE) {
...
void cint_tm_rate_no_limit (int port_rate)
{
    int level;

    if (port_rate != -1)
        cint_change_port_rate(0,2,port_rate);

    for (level=2; level <= 4; level++) {
        cint_change_se_rate(0,level,0,0,0);
    }
    cint_change_vc_rate(0,0x20,0,0);
}
