...
int oamp_server_example(int server_unit, int client_unit, int port1, int port2, int port3,int recycle_port) {
	...
	rv = set_counter_source_and_engines(unit, &counter_base_id_up_mep);
	BCM_IF_ERROR_RETURN(rv); 
	rv = set_counter_source_and_engines(unit, &counter_base_id_down_mep);
	BCM_IF_ERROR_RETURN(rv); 
...
int create_acc_endpoint(int unit, bcm_oam_endpoint_info_t *local_ep, bcm_oam_endpoint_info_t *remote_ep, int is_up, int group_id) {
	...
	if (first_call) {
		...
		rv = set_counter_source_and_engines(unit, &lm_counter_base_ids[0]);
		BCM_IF_ERROR_RETURN(rv);
		rv = set_counter_source_and_engines(unit, &lm_counter_base_ids[1]);
		BCM_IF_ERROR_RETURN(rv); 
        rv = set_counter_source_and_engines(unit, &lm_counter_base_ids[2]);
        BCM_IF_ERROR_RETURN(rv);
        first_call=0; 

		/* For testing only*/
		lm_counter_base_ids[0] = lm_counter_base_ids[1] = lm_counter_base_ids[2] = 5;
...
/* When using PCP the 3 LSBs of the counter must be 0. The following cints return an appropriate counter base.*/
    rv = set_counter_source_and_engines(unit, &up_mep_lm_counter_base);
    BCM_IF_ERROR_RETURN(rv); 
    rv = set_counter_source_and_engines(unit, &down_mep_lm_counter_base);
    BCM_IF_ERROR_RETURN(rv); 
