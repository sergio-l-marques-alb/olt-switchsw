...
int setup_tdm(int unit, int local_port_id, int dest_id, int is_mc, int add_crc,  
    ...
    /* Set the User Field */
    if (user_field_count > 32) {
        printf("Cant set a user field of over 32 bits");
...
int arad_tdm_example_multi_device (int src_unit, int dst_unit,
    ...
    if(is_mc){
        open_egress_mc_group(dst_unit, mc_group_id, mc_ports, mc_cuds, 3);
...
int arad_tdm_outter_header_example (int unit,
    ...
    /* Setup the MC or UC configuration */
    if(is_mc){
        open_egress_mc_group(unit, mc_group_id, mc_ports, mc_cuds, 3);
        rv = setup_tdm(unit, local_port_id, 0, 1, add_crc, bcmFabricTdmEditingNoChange,bcmFabricTdmEditingNoChange, user_field_size, user_field_data);
        if (rv != BCM_E_NONE)
            return rv;
    } else {
