...
#define VIPER_IF_XFI    (1 << SOC_PORT_IF_XFI)
#define VIPER_IF_SFI    (1 << SOC_PORT_IF_SFI)
#define VIPER_IF_XLAUI  (1 << SOC_PORT_IF_XLAUI)
#define VIPER_IF_XGMII  (1 << SOC_PORT_IF_XGMII)
#define VIPER_IF_ILKN   (1 << SOC_PORT_IF_ILKN)


#define VIPER_CL37_HR2SPM_W_10G 5
#define VIPER_CL37_HR2SPM       4
 ...
 */
STATIC int
viper_chip_bonding_swap(int unit, soc_port_t port, int is_tx, int offset) 
{
    soc_info_t *si;
    int phy_port;
    si = &SOC_INFO(unit);
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    int core_num;
    int phy_num_lanes;
    int lane_map_rx = 0, lane_map_tx = 0, chip_bonding_swap;
    int i;
    int fiber_pref = 0;
    ...
    */
    ...
    lane_map_rx = soc_property_port_suffix_num_get(unit, port, core_num,
                                        spn_XGXS_RX_LANE_MAP, "core", lane_map_rx);

    lane_map_rx_l = lane_map_rx ;
    
    is_tx = 0 ;
    chip_bonding_swap = viper_chip_bonding_swap(unit, port, is_tx, logical_lane_offset) ;
    ...
    lane_map_tx = soc_property_port_suffix_num_get(unit, port, core_num,
                                        spn_XGXS_TX_LANE_MAP, "core", lane_map_tx);    
    is_tx = 1 ;
    chip_bonding_swap = viper_chip_bonding_swap(unit, port, is_tx, logical_lane_offset) ;
    for (i=0; i<NUM_LANES; i++) {
        if(chip_bonding_swap) {
    ...
    if (port_num_lanes == 1) {
        if (PHY_EXTERNAL_MODE(unit, port) || !(speed_config->fiber_pref)) {
            ...
            speed_config->line_interface =  SOC_PORT_IF_XAUI;
    }
                              ...
                              soc_port_ability_t *ability)
{
    ...
    speed_full_duplex|= PHYMOD_BAM_CL37_CAP_10G_HIGIG_GET(reg37_ability)?  SOC_PA_SPEED_10GB:0;*/ 
    /* 4-lane */
...
phy_viper_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
    ...
    case phymodInterfaceKR:
    {
        ...
        phy_viper_an_get(unit, port, &an, &an_done);
        if (!an) { 
            if (PHY_EXTERNAL_MODE(unit, port) || !(pCfg->speed_config.fiber_pref)) {
                *pif = SOC_PORT_IF_XFI;
            } else {
 ...
 */
STATIC int
viper_uc_status_dump(int unit, soc_port_t port, void *arg)
{
    phy_ctrl_t* pc; 
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
...
/* 
 * viper_tx_fir_pre_set
 */
STATIC int
viper_tx_fir_pre_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    int                 idx;
...
/* 
 * viper_per_lane_driver_current_set
 */
STATIC int
viper_per_lane_driver_current_set(soc_phymod_ctrl_t *pmc, int lane, uint32 value)
{
    soc_phymod_phy_t    *p_phy;
    uint32              lane_map;
    phymod_phy_access_t pm_phy_copy, *pm_phy;
...
/* 
 * viper_driver_current_set
 */
STATIC int
viper_driver_current_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    int                 idx;
 ...
 */
STATIC int 
viper_rx_vga_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
...
STATIC
int
viper_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
...
phy_viper_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_TX_FIR_PRE:
        rv = viper_tx_fir_pre_set(pmc, value);
        break;
    ...
    case SOC_PHY_CONTROL_DRIVER_CURRENT_LANE0:
        rv = viper_per_lane_driver_current_set(pmc, 0, value);
        break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT_LANE1:
        rv = viper_per_lane_driver_current_set(pmc, 1, value);
        break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT_LANE2:
        rv = viper_per_lane_driver_current_set(pmc, 2, value);
        break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT_LANE3:
        rv = viper_per_lane_driver_current_set(pmc, 3, value);
        break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT:
        rv = viper_driver_current_set(pmc, value);
        break;
 ...
 */
STATIC int 
viper_rx_vga_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
...
phy_viper_per_lane_control_set(int unit, soc_port_t port, int lane, soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        ...
        break; 
    case SOC_PHY_CONTROL_DRIVER_CURRENT:
        rv = viper_per_lane_driver_current_set(pmc, lane, value);
        break;
    case SOC_PHY_CONTROL_PRE_DRIVER_CURRENT:
        rv = SOC_E_UNAVAIL;
        break;
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        ...
        break; 
    case SOC_PHY_CONTROL_DRIVER_CURRENT:
        rv = viper_per_lane_driver_current_get(pmc, lane, value);
        break;
    case SOC_PHY_CONTROL_PRE_DRIVER_CURRENT:
        rv = SOC_E_UNAVAIL;
        break;
