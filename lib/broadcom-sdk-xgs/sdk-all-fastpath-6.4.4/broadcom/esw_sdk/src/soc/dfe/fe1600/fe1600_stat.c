/*
 * $Id: fe1600_stat.c,v 1.10 Broadcom SDK $
 *
...
int soc_fe1600_controlled_counter_get(int unit, int counter_id, int port, uint64* val){
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_FMAL_STATISTICS_OUTPUT_CONTROLr(unit, blk_idx, control_reg));
...
soc_controlled_counter_t soc_fe1600_controlled_counter[] = {
    ...
    {
        soc_fe1600_controlled_counter_get,
        soc_fe1600_counters_rx_lfec_fec_correctable_error, /*soc_fe1600_counters_rx_bec_crc_error , soc_fe1600_counters_rx_8b_10b_disparity_errors*/
    ...
    {
        soc_fe1600_controlled_counter_get,
        soc_fe1600_counters_rx_lfec_fec_uncorrectable_erros, /*soc_fe1600_counters_rx_bec_rx_fault, soc_fe1600_counters_rx_8b_10b_code_errors*/
...
soc_fe1600_stat_get(int unit,bcm_port_t port,uint64 *value,int *counters,int arr_size)
{
    ...
    for (i=0;i<arr_size;i++)
    {
        if (COUNTER_IS_COLLECTED(SOC_CONTROL(unit)->controlled_counters[counters[i]])) {
            ...
            /*counter isn't collected by counter thread*/
            rc = soc_fe1600_controlled_counter_get(unit, counters[i], port, &val64);
        }
