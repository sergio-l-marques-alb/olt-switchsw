...
soc_fe1600_set_mesh_topology_config(int unit) {
    ...
    if (!SOC_IS_FE1600(unit))
    {
        ...
        /*lsb*/
        if(SOC_DFE_CONFIG(unit).system_is_vcs_128_in_system) {
            soc_reg_field_set(unit, MESH_TOPOLOGY_REG_011Br, &reg_val32, REG_11B_CONFIG_1f, 15);
        } else if (SOC_DFE_CONFIG(unit).system_contains_multiple_pipe_device) {
            soc_reg_field_set(unit, MESH_TOPOLOGY_REG_011Br, &reg_val32, REG_11B_CONFIG_1f, 20); 
        } else {
            soc_reg_field_set(unit, MESH_TOPOLOGY_REG_011Br, &reg_val32, REG_11B_CONFIG_1f, 24); 
        }
        
        soc_reg_field_set(unit, MESH_TOPOLOGY_REG_011Br, &reg_val32, REG_11B_CONFIG_2f, 0); 
...
int
soc_fe1600_drv_graceful_shutdown_set(int unit, soc_pbmp_t active_links, int shutdown, soc_pbmp_t unisolated_links) 
{
    int rv;
    soc_port_t port;
    SOCDNX_INIT_FUNC_DEFS;
