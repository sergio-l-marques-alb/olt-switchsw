...
int tdm_td2p_vec_load(unsigned char **map, int bw, int port_speed, int yy, int len, int num_ext_ports);
...
**/
int tdm_core_vbs_scheduler( tdm_mod_t *_tdm )
{
	unsigned char dim_reduce;
	int g, i=0, j, k, l, v,
		timeout=TIMEOUT, vec_slot_cnt=0, lr_slot_cnt=0, pspd, fpnode,
		tsc_pipeline[ARR_FIXED_LEN], xx=0, x0=0, triport_op=BOOL_FALSE, vec_id, ns_min_cnt, rotation_step=DEFAULT_STEP_SIZE, main_pass_done, main_pass_timeout,
		cadence_cnt=0, cadence_step_size, cadence_ref_idx, redis_100g_cnt, redis_100g_port,
		cadence_step, cadence_step_limit, cadence_blocks, retrace_num=0,/*retrace_result,*/
		cap_tsc=_tdm->_chip_data.soc_pkg.num_ext_ports, cap_lpbk=0, cap_lkbk=0,
		*tdm_pipe_main;
	struct node tdm_ll;
	/* struct node* tdm_ll = (struct node *) TDM_ALLOC(sizeof(struct node),"core linked list"); */
	...
	if (_tdm->_core_data.vars_pkg.os_enable && _tdm->_core_data.vars_pkg.lr_enable) {
		...
		for (j=0; j<_tdm->_chip_data.soc_pkg.lr_idx_limit; j++) {
			if (_tdm->_core_data.vmap[0][j]==_tdm->_chip_data.soc_pkg.num_ext_ports) {
				vec_slot_cnt++;
			}
			if (j!=0 && _tdm->_core_data.vmap[0][j]!=_tdm->_chip_data.soc_pkg.num_ext_ports && _tdm->_core_data.vmap[0][(j-vec_slot_cnt-1)]!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
				break;
			}
		}
	}
		...
		if (_tdm->_core_data.vars_pkg.os_enable) {
			for (i=1; i<_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.lr_vec_cnt; i++) {
				...
				if ( ((pspd!=tdm_count_param_spd(_tdm))) ) {
					...
					pspd=tdm_count_param_spd(_tdm);
					_tdm->_core_data.vars_pkg.m_tdm_count_param_spd.x_pos=i;
					if (pspd%tdm_count_param_spd(_tdm)==0) {
						cadence_blocks=1;
						cadence_ref_idx=i-1;
						_tdm->_core_data.vars_pkg.m_tdm_count_param_spd.round=BOOL_FALSE;
	...
	for (v=0; v<_tdm->_core_data.vmap_max_wid; v++) {
		TDM_PRINT1("%d",v);
		if ((v+1)<_tdm->_core_data.vmap_max_wid) {
			if ((v+1)/10==0) {
				TDM_PRINT0("___");
			}
			else {
		...
		for (j=0; j<_tdm->_core_data.vmap_max_wid; j++) {
			if (tdm_fit_singular_cnt(_tdm->_core_data.vmap,i,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports)>0) {
				TOKEN_CHECK(_tdm->_core_data.vmap[j][i]) {
					...
					if (_tdm->_core_data.vmap[j][i]==_tdm->_core_data.vmap[0][0]) {
						retrace_num++;
					}
				}
			}
	...
	tdm_ll_print(&tdm_ll);
	/* if (_tdm->_core_data.vars_pkg.os_enable && _tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.lr_50==BOOL_FALSE && _tdm->_core_data.vars_pkg.lr_100==BOOL_FALSE && _tdm->_core_data.vars_pkg.lr_120==BOOL_FALSE && _tdm->_core_data.vars_pkg.lr_1==BOOL_FALSE) {
		TDM_PRINT0("\tRetracing linked list...\n");
		j=0;
		for (i=0; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
			if (_tdm->_core_data.vmap[0][i]!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
				TDM_PRINT2("\t\t| Pass = %0d of %0d |\n",(++j),retrace_num);
				retrace_result=tdm_ll_retrace(&tdm_ll,_tdm,i);
				if (retrace_result!=0) {
