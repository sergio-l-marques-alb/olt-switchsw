...
**/
void
tdm_ll_append(struct node *llist, unsigned char port_append)
{
	struct node *mbox,
...
**/
void
tdm_ll_insert(struct node *llist, unsigned char port_insert, int idx)
{
	int i;
	struct node *mbox,
				*right=llist,
...
tdm_ll_strip(struct node *llist, tdm_mod_t *_tdm, int cadence_start_idx, int *pool, int *s_idx, int token)
{
	...
	unsigned char start_port=( (int) _tdm->_chip_data.soc_pkg.num_ext_ports );
	struct node *list = llist;
