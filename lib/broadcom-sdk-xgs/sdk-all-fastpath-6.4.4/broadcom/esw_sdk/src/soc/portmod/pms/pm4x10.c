...
extern int pm4x10_phy_control_set(int u, soc_port_t p, pm_info_t pm, soc_phy_control_t type, phymod_tx_t*, uint32, uint32 v);
...
{                          
    ...
    _SOC_IF_ERR_EXIT(READ_XLPORT_SOFT_RESETr(unit, port, &reg_val));
    old_val = reg_val;

    soc_reg_field_set(unit, XLPORT_SOFT_RESETr, &reg_val, port_fields[idx], 1); 

    _SOC_IF_ERR_EXIT(WRITE_XLPORT_SOFT_RESETr(unit, port, reg_val));
    _SOC_IF_ERR_EXIT(WRITE_XLPORT_SOFT_RESETr(unit, port, old_val));
...
static int _xlport_mode_get(int unit, int port, int bindex, 
                      portmod_core_port_mode_t *core_mode, int *cur_lanes)
{
    ...
    _SOC_IF_ERR_EXIT(READ_XLPORT_MODE_REGr(unit, port, &rval));
...
static int _xlport_mode_set(int unit, int port, portmod_core_port_mode_t cur_mode)
{
    uint32_t rval; 
    int      mode;
    ...
    _SOC_IF_ERR_EXIT(WRITE_XLPORT_MODE_REGr(unit, port, rval));

exit:
    SOC_FUNC_RETURN;
}

static int _xlport_lag_failover_status_toggle(int unit, int port)
{
    uint32_t rval;
    ...
    /* Toggle link bit to notify IPIPE on link up */
    _SOC_IF_ERR_EXIT(READ_XLPORT_LAG_FAILOVER_CONFIGr(unit, port, &rval));
    soc_reg_field_set(unit, XLPORT_LAG_FAILOVER_CONFIGr, &rval, LINK_STATUS_UPf, 1);
    _SOC_IF_ERR_EXIT(WRITE_XLPORT_LAG_FAILOVER_CONFIGr(unit, port, rval));
...
int pm4x10_port_cntmaxsize_get(int unit, int port, pm_info_t pm_info, int *val)
{
    ...
    if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
        ...
        rv = READ_XLPORT_CNTMAXSIZEr(unit, port, (uint32_t *)val);
    }
    return rv;
}
...
int pm4x10_port_cntmaxsize_set(int unit, int port, pm_info_t pm_info, int val)
{
    ...
    if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
        ...
        rv = WRITE_XLPORT_CNTMAXSIZEr(unit, port, (uint32_t)val);
    }
    return rv;
}
...
int pm4x10_port_modid_set (int unit, int port, pm_info_t pm_info, int value)
{
    ...
    _SOC_IF_ERR_EXIT(READ_XLPORT_CONFIGr(unit, port, &rval));
    soc_reg_field_set(unit, XLPORT_CONFIGr, &rval, MY_MODIDf, modid);
    _SOC_IF_ERR_EXIT(WRITE_XLPORT_CONFIGr(unit, port, rval));
                        ...
                        const portmod_port_add_info_t* add_info, int enable)
{
    ...
    if(enable) {
        ...
        /* Power Save */
        _SOC_IF_ERR_EXIT(READ_XLPORT_POWER_SAVEr(unit, port, &reg_val));
        soc_reg_field_set(unit, XLPORT_POWER_SAVEr, &reg_val, XPORT_CORE0f, 0); 
        _SOC_IF_ERR_EXIT(WRITE_XLPORT_POWER_SAVEr(unit, port, reg_val));
        ...
        if(i==0 /*internal phy */) {
            ...
            core_conf.lane_map.num_of_lanes = PM4X10_LANES_PER_CORE;
...
STATIC
int _pm4x10_pm_port_init(int unit, int port, int internal_port,
                         const portmod_port_add_info_t* add_info, int enable)
{
    int rv;
    ...
    if(enable) {
        ...
        if (SOC_REG_IS_VALID(unit, PGW_CNTMAXSIZEr)) {
            ...
            _SOC_IF_ERR_EXIT(READ_XLPORT_CNTMAXSIZEr(unit, port, &reg_val));
            soc_reg_field_set(unit, XLPORT_CNTMAXSIZEr, &reg_val, CNTMAXSIZEf, 1518);
            _SOC_IF_ERR_EXIT(WRITE_XLPORT_CNTMAXSIZEr(unit, port, reg_val));
        }
                        ...
                        const portmod_port_interface_config_t* config)
{
    ...
    if(!pm_is_bypassed){
        ...
        /* set encapsulation */
        rv = xlmac_encap_set (unit, port, 0, 
                              PHYMOD_INTF_MODES_HIGIG_GET(config) ? 
    ...
    for(i=0 ; i<PM4X10_LANES_PER_CORE ; i++) {
        _SOC_IF_ERR_EXIT(phymod_phy_media_type_tx_get(&phy_access_lane, phymodMediaTypeChipToChip, &(phy_init_config->tx[i])));
    }
                        ...
                        const portmod_port_add_info_t* add_info)
{
    ...
    uint32     pm_is_active = 0, pm_is_bypassed = 0, bitmap;
    ...
    pm_is_bypassed = ((add_info->interface_config.interface == SOC_PORT_IF_CAUI) ||
                      (add_info->interface_config.interface == SOC_PORT_IF_ILKN))? 1: 0;
    rv = SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD, 
    ...
    /* initalize port */
    rv = _pm4x10_pm_port_init(unit, port, port_index, add_info, 1);
    _SOC_IF_ERR_EXIT(rv);
...
int pm4x10_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    rv = _pm4x10_pm_port_init(unit, port, port_index, NULL, 0);
    _SOC_IF_ERR_EXIT(rv);
                            ...
                            int flags, int enable)
{
    ...
    if(enable){
        if((PORTMOD_PORT_ENABLE_MAC_GET(actual_flags)) && (!is_bypassed)){
            rv = xlmac_enable_set(unit, port, 0, 1);
            _SOC_IF_ERR_EXIT(rv);
        }
                        ...
                        const portmod_port_interface_config_t* config)
{
    ...
    if (!(config->flags & PHYMOD_INTF_F_SET_SPD_TRIGGER) && (!pm_is_bypassed)){
        ...
        /* set encapsulation */
        rv = xlmac_encap_set (unit, port, 0, config->encap_mode);
        _SOC_IF_ERR_EXIT(rv);
    }
                                      ...
                                      portmod_port_interface_config_t* config)
{
    ...
    int                          port_index;
    uint32_t                     bitmap;
    int flags=0, nof_phys = 0, rv = 0, ref_clock = PM_4x10_INFO(pm_info)->ref_clk;
    ...
    /* KR from phymod could be aliasing among KR, SR and etc */
    if(phy_interface_config.interface_type==phymodInterfaceKR) {
        if (!PHYMOD_INTF_MODES_FIBER_GET(&phy_interface_config)) {
            phy_interface_config.interface_type=phymodInterfaceXFI ;
        } else {
            if(interface_config->interface==SOC_PORT_IF_SR) {
    ...
    switch(loopback_type){
        ...
        case portmodLoopbackPhyRloopPCS: /*slide*/
             if(PM_4x10_INFO(pm_info)->nof_phys == 0) {
                ...
                _SOC_IF_ERR_EXIT(portmod_port_phychain_loopback_get(phy_access,
                                                                    nof_phys,
                                                                    phymod_lb_type,
                                                                    &tmp_enable));
                *enable = tmp_enable;
                  ...
                  portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    if (enable)
        _SOC_IF_ERR_EXIT(_xlport_lag_failover_status_toggle(unit, port));
                                   ...
                                   portmod_port_ability_t* ability)
{
    ...
    int                 max_speed = 11000;
    int                 is_dual_lane_port = 1, num_of_lanes ;
    uint32              bitmap;
    ...
    num_of_lanes      = PM_4x10_INFO(pm_info)->lane_map.num_of_lanes;
...
int pm4x10_port_mac_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    ...
    switch(port_index) {
        case 0:
            _SOC_IF_ERR_EXIT(READ_XLPORT_XGXS0_LN0_STATUS0_REGr(unit, port, 
                                        &reg_val));
        ...
        case 1:
            _SOC_IF_ERR_EXIT(READ_XLPORT_XGXS0_LN1_STATUS0_REGr(unit, port, 
                                         &reg_val));
        ...
        case 2:
            _SOC_IF_ERR_EXIT(READ_XLPORT_XGXS0_LN2_STATUS0_REGr(unit, port, 
                                          &reg_val));
        ...
        case 3:
            _SOC_IF_ERR_EXIT(READ_XLPORT_XGXS0_LN3_STATUS0_REGr(unit, port, 
                                           &reg_val));
...
int pm4x10_ext_phy_attach_to_pm(int unit, pm_info_t pm_info, const phymod_core_access_t *ext_phy_access)
{
                        ...
                        const portmod_port_update_control_t* update_control)
{
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            /* update MAC */
            _SOC_IF_ERR_EXIT(xlmac_enable_set(unit, port, 0, 0));
            _SOC_IF_ERR_EXIT(xlmac_speed_set(unit, port, autoneg.data_rate));
...
{
    return(_xlport_lag_failover_status_toggle(unit, port));
}
...
{
    return (_xlport_mode_set(unit, port, mode->cur_mode));
}
                     ...
                     pm_info_t pm_info, portmod_port_mode_info_t *mode)
{
    ...
    return(_xlport_mode_get(unit, port, port_index, &mode->cur_mode, &mode->lanes));
}
        ...
        default:
            if(op_type == PHY_DIAG_CTRL_SET) {
                ...
                if (!SAL_BOOT_SIMULATION) {
                    SOC_IF_ERROR_RETURN(pm4x10_phy_control_set(unit,
                        port,pm_info, op_cmd, &ln_txparam, lane_map, PTR_TO_INT(arg)));
                }
                ...
                if (!SAL_BOOT_SIMULATION) {
                    SOC_IF_ERROR_RETURN(pm4x10_phy_control_get(unit,
                        port,pm_info,op_cmd, &ln_txparam, lane_map, (uint32 *)arg));
                }
            } else {
...
{
    return (xlmac_drain_cell_get(unit, port,
...
{
    return(xlmac_drain_cell_stop (unit, port, 
               drain_cells->rx_pfc_en,
               drain_cells->llfc_en,
               drain_cells->rx_pause,
               drain_cells->tx_pause));
...
{
    return (xlmac_soft_reset_set(unit, port, val));
}
