    ...
    )
{
    ...
    SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_e2e_interfaces_occ, sch_offset, TRUE));
    ...
    )
{
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        for (sch_offset = 0; sch_offset < SOC_DPP_IMP_DEFS_GET(unit, nof_core_interfaces); sch_offset++) {        
            ...
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_is_occupied(modified_e2e_interfaces_occ, sch_offset, &is_modified));
            if (!is_modified) {
                continue;
            }

            /* mark E2E interface as not modified */
            SOCDNX_SAND_IF_ERR_EXIT(soc_sand_occ_bm_occup_status_set(modified_e2e_interfaces_occ, sch_offset, FALSE));
