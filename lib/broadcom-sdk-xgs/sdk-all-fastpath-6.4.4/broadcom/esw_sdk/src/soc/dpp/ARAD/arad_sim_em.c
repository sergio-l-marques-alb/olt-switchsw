            ...
            )
{
  ...
  *success = TRUE;
  ...
  multi_set = &Em.blocks[unit][id].multi_set;
  ...
  status = multi_set->init_info.set_entry_fun(
    unit, 
    0, 
    Em.blocks[unit][id].base,
    key_id * Em.blocks[unit][id].data_nof_bytes,
                                  ...
                                  )
{
  ...
  *found = TRUE;
  status = multi_set->init_info.get_entry_fun(
              unit, 
              0, 
              Em.blocks[unit][id].base,
              indx * Em.blocks[unit][id].data_nof_bytes ,
              Em.blocks[unit][id].data_nof_bytes,
              data8
              );
  if (status != OK)
  ...
  )
{
  ...
  *found = TRUE;
  status = multi_set->init_info.get_entry_fun(
              ...
              0, 
              Em.blocks[unit][id].base,
              entry_offset * Em.blocks[unit][id].data_nof_bytes ,
              Em.blocks[unit][id].data_nof_bytes,
              data8
              );
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
             ...
             )
{
  ...
  for (blk_ndx = 0; blk_ndx < block_cnt; blk_ndx++) {
    ...
    nof_addresses = block_p->end_address - block_p->start_address + 1;
      ...
      block_p->multi_set.init_info.get_entry_fun = chip_sim_em_key_map_get_entry;
      block_p->multi_set.init_info.set_entry_fun = chip_sim_em_key_map_set_entry;
      block_p->multi_set.init_info.max_duplications = 1;/*maximal packets to store from each content*/
      block_p->multi_set.init_info.sec_handle = 0;
      block_p->multi_set.init_info.prime_handle = device_id;
      block_p->multi_set.init_info.member_size = block_p->key_size; /* Soc_pb_em_blocks[table_type] */
      block_p->multi_set.init_info.nof_members = block_p->end_address - block_p->start_address + 1; /*maximal packets to store*/

      block_p->multi_set.init_info.wb_var_index = SOC_DPP_WB_ENGINE_VAR_CHIP_SIM_MULTI_SET +
             ...
             )
{
  ...
  for (block_p=Em.blocks[unit]; block_p->read_result_address!=INVALID_ADDRESS; block_p++)
  {
      if (NULL != block_p->base)
      {
        ...
        soc_sand_multi_set_destroy(&(block_p->multi_set));
      }
      block_p->base = NULL;
  ...
  )
{
  ...
  res = soc_sand_multi_set_get_next(multi_set, &iter, key8, &hash_data_ndx, &ref_count);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);


  while (!SOC_SAND_HASH_TABLE_ITER_IS_END(&iter)) {

      res = multi_set->init_info.get_entry_fun(
                  unit, 
                  0, 
                  Em.blocks[unit][id].base,
                  hash_data_ndx * Em.blocks[unit][id].data_nof_bytes,
                  Em.blocks[unit][id].data_nof_bytes,
                  data8
                  );
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
