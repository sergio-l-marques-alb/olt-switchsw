...
soc_dpp_arad_str_prop_mc_tbl_mode(int unit)
{
    ...
    uint32 recycle_workaround = 1; /* must be 0 or 1, do we need a recycle interface workaround for Jericho */
    ...
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids = soc_property_get(unit,spn_MULTICAST_INGRESS_GROUP_ID_RANGE_MAX, default_nof_ingr_groups - 1) + 1 + recycle_workaround;
        if (SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids > max_nof_ingr_groups || SOC_DPP_CONFIG(unit)->tm.nof_ingr_mc_ids - recycle_workaround > max_nof_ingr_groups) {
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (SOC_DPP_CONFIG(unit)->arad->init.fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_SINGLE_FAP ||
            SOC_DPP_CONFIG(unit)->arad->init.fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_MESH ||
            SOC_DPP_CONFIG(unit)->arad->init.fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_BACK2BACK) {
            SOC_DPP_CONFIG(unit)->tm.mc_mode |= DPP_MC_EGR_CORE_MESH_MODE; /* hardware controls whether each egress group is active */
            ...
            SOC_DPP_CONFIG(unit)->tm.mc_mode |= DPP_MC_EGR_CORE_FDA_MODE; /* hardware controls whether each egress group is active */
        }
        SOC_DPP_CONFIG(unit)->tm.ingress_mc_max_cud = JER_MC_MAX_ING_CUD;
        SOC_DPP_CONFIG(unit)->tm.egress_mc_max_cud = JER_MC_MAX_ING_CUD;
    }
...
soc_arad_default_config_tm_get(int unit)
{
    ...
    for (i = 0; i < SOC_DPP_CONFIG(unit)->tm.max_interlaken_ports; i++) {
        ...
        SOC_DPP_CONFIG(unit)->tm.fc_inband_intlkn_calender_length[i][SOC_TMC_CONNECTION_DIRECTION_TX] = SOC_DPP_DEFS_GET(unit, fc_inb_cal_len_max);
...
soc_arad_default_config_l3_get(int unit)
{
    ...
    if(SOC_IS_JERICHO(unit)){
        if(dpp_l3->actual_nof_routing_enablers_bm > 16){/* ilegal value */
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Maximum INRIF_MAC_TERMINATION_COMBINATIONS is 16.")));
        ...
        } else if(dpp_l3->actual_nof_routing_enablers_bm == 2) {/*1 bit*/
            dpp_l3->intf_class_max_value = 7;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 3;
        } else {            
            dpp_l3->intf_class_max_value = 15;
            dpp_l3->bit_offset_of_the_routing_enablers_profile = 4;
        }        
    }
...
soc_arad_default_config_general_get(int unit)
{
    ...
    dpp_pp->bfd_ipv4_single_hop = (soc_property_get(unit, spn_BFD_IPV4_SINGLE_HOP_EXTENDED, 1) && SOC_IS_ARADPLUS_A0(unit));
    ...
    dpp_pp->bfd_ipv6_enable = (soc_property_get(unit, spn_BFD_IPV6_ENABLE, 0) && SOC_IS_ARADPLUS_A0(unit));
    ...
    if(dpp_pp->bfd_extended_ipv4_src_ip && soc_property_get(unit, spn_BCM886XX_ERSPAN_TUNNEL_ENABLE, 0)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("BFD-Extended-IPv4-SIP cannot co-exist with Erspan tunnel.")));
    }
    dpp_pp->oam_statistics = (soc_property_get(unit, spn_OAM_STATISTICS_PER_MEP_ENABLED, 0) && SOC_IS_ARADPLUS_A0(unit));  
    ...
    dpp_pp->mldp_dummy_lif_id[0] = soc_property_get(unit,spn_DEFAULT_LOGICAL_INTERFACE_MPLS_1_LABEL_BUD_MULTICAST, -1);
    dpp_pp->mldp_dummy_lif_id[1] = soc_property_get(unit,spn_DEFAULT_LOGICAL_INTERFACE_MPLS_2_LABELS_BUD_MULTICAST, -1);
    if((dpp_pp->mldp_dummy_lif_id[0] != -1) && (dpp_pp->mldp_dummy_lif_id[1] != -1))
...
int
soc_arad_allocate_tm_port_in_range_and_recycle_channel(int unit, int core, int max_port, int *tm_port, uint32 *channel)
{
    soc_port_t port;
...
soc_arad_fmq_isq_hr_init(int unit, bcm_core_t core, uint8 is_fmq)
{
    ...
    if (!is_fmq && (SOC_DPP_CONFIG(unit)->tm.hr_isq[core] == SOC_TMC_SCH_PORT_ID_INVALID_ARAD) /* PORT ISQ */) {
        ...
        if (!found)
        {
            SOCDNX_IF_ERR_EXIT(arad_ps_db_find_free_non_binding_ps(unit, core, &base_q_pair));
        }
...
soc_arad_ports_info_config(int unit) 
{
    ...
    int core = BCM_DPP_AM_DEFAULT_CORE_ID ; /* ARAD plus and below only */
    soc_dpp_config_pp_t *dpp_pp;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (soc_property_get(unit, spn_NUM_OAMP_PORTS, 0)) {
            si->oamp_port[0] = ARAD_OAMP_PORT_ID;
        }
...
soc_arad_info_config(int unit) 
{
     ...
     * Init NIF ports
     */
    dpp_arad->init.nif_recovery_enable = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "nif_recovery_enable", 0);
    ...
    for (ind = 0; ind < ARAD_NIF_NOF_SYNCE_CLK_IDS ; ind++) {
        if (SOC_IS_JERICHO(unit)) {
            dpp_arad->init.synce.conf[ind].port_id = soc_property_suffix_num_get(unit, ind, spn_SYNC_ETH_CLK_TO_PORT_ID_CLK, "clk_", 0xffffffff); 
        } else {
    ...
    */
    ...
    switch(val)
    {
      ...
      if (SOC_IS_DPP_DRC_COMBO28(unit)) {
            ...
            if (SOC_IS_ARDON(unit)) {
                ...
                SOC_PBMP_WORD_SET(dpp_arad->init.drc_info.ref_clk_bitmap, 0, 0xd);
                ...
                if (dram_bitmap != 0) {
                    ...
                    SOC_PBMP_WORD_SET(dpp_arad->init.drc_info.ref_clk_bitmap, 0, 0xA5);
    ...
    dpp_arad->init.stat_if.stat_if_info.rate = soc_property_get(unit, spn_STAT_IF_RATE, 0x0);
    ...
    if (dpp_arad->init.pp_enable) {
      ...
      if (SOC_DPP_CONFIG(unit)->arad->pp_op_mode.mpls_info.fast_reroute_labels_enable && dpp_pp->explicit_null_support) {
      ...
      if (dpp_pp->evb_enable && dpp_pp->explicit_null_support) {
      ...
      if ((dpp_pp->explicit_null_support || dpp_pp->gal_support) && (!dpp_pp->tunnel_termination_in_tt_only))  {
            ...
            for (port_i = 0; port_i < SOC_MAX_NUM_PORTS ; ++port_i) {   
                ...
                if (IS_E_PORT(unit, port_i)) {
                    ...
                    for (vlan = 0; vlan < 4096; vlan++) {   
                        SOCDNX_SAND_IF_ERR_RETURN(soc_ppd_eg_vlan_edit_port_vlan_transmit_outer_tag_set(unit,core,port_i,vlan,TRUE));   
                    }
                }
            }   
        }
...
soc_arad_default_ofp_rates_set(int unit) {
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        SOC_PBMP_ITER(ports_bm, port_i) {
            ...
            if (interface == SOC_PORT_IF_ERP) {
                egress_offset = arad_sw_db_egr_ports_erp_interface_get(unit, core);
            } else {
            ...
            for (priority_i = 0; priority_i < nof_priorities; ++priority_i)
            {
               ...
               /*sch*/
               rv = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_port_priority_rate_set,(unit, core, tm_port ,priority_i, sch_rate));
               if(SOC_FAILURE(rv)) {
