...
exit_semaphore:
  ...
  ARAD_PP_DO_NOTHING_AND_EXIT;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_eg_mirror_port_dflt_set()", local_port_ndx, 0);
}
  ...
  )
{
  ...
  SOC_SAND_TAKE_DEVICE_SEMAPHORE;
  res = arad_pp_eg_mirror_port_appl_set_unsafe(unit, core_id, local_port_ndx, enable);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit_semaphore);
