...
/* $Id: arad_header_parsing_utils.c,v 1.15 Broadcom SDK $
...
/*
 *  Petra Legacy ITMH-Extension-source-port
 */
...
*/
  ...
  if (info->extension.enable == TRUE)
  {
    ...
    {
/*
      res = arad_petra_dest_sys_port_info_verify(&(info->extension.src_port));
...
/*
    res = arad_dest_info_verify(&(info->base.destination));
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
*/
    ...
    SOC_SAND_OUT ARAD_HPU_ITMH_HDR        *itmh)
{
    ...
    itmh_prog_mode = (soc_property_get(unit, spn_ITMH_PROGRAMMABLE_MODE_ENABLE, FALSE)) ? TRUE : FALSE ; 

        /* itmh_prog_mode = FALSE : 88650 Mode */     
