/* $Id: pb_ofp_rates.c,v 1.15 Broadcom SDK $
...
#define SOC_PB_OFP_RATES_OLP_GUARANTEED_KBPS             (1 * 1024 * 1024)

/* Initial rate deviation for calendar-building algorithm. */
#define SOC_PB_OFP_RATES_CAL_RATE_DEVIATION              (100000000)

/* Number of clocks to traverse a single calendar slot in the EGQ */
#define SOC_PB_EGQ_CAL_SLOT_TRAVERSE_IN_CLOCKS           (2)

/* EGQ bandwidth The unit is 1/256th byte */
#define SOC_PB_EGQ_UNITS_VAL_IN_BITS                     (256 / 8)

/* Indication of an uninitialized entry */
#define SOC_PB_OFP_RATES_ILLEGAL_SCHEDULER_ID            (0xfff)

/* The initial rate given to all port before random test */
#define SOC_PB_OFP_RATES_TEST_RANDOM_INITIAL_PORT_RATE   (1000000)

/* */
#define SOC_PB_OFP_RATES_INVALID_PORT_ID                 (SOC_PETRA_NOF_FAP_PORTS)

/* */
#define SOC_PB_OFP_RATES_NOF_ITERATIONS                  (2)

/* $Id: pb_ofp_rates.c,v 1.15 Broadcom SDK $
 *	Maximal allowed rate difference between requested
 *  and actual, in the rate-configuration test
 */
#define SOC_PB_OFP_RATES_TST_MAX_SCH_DELTA_PERCENT       (5)
#define SOC_PB_OFP_RATES_TST_MAX_EGQ_DELTA_PERCENT       (5)

#define SOC_PB_OFP_RATES_CAL_NIF_BASE_SCH_OFFSET          0x0800
#define SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX                  0x0080
#define SOC_PB_OFP_RATES_CALCAL_LEN_EGQ_MAX               0x0080
#define SOC_PB_OFP_RATES_CAL_LEN_SCH_MAX                  0x0400
#define SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET               0x1000
 ...
 *************/
/* { */
#define SOC_PB_OFP_RATES_IS_MAL_XAUI_SKIP(mal_id, answer) \
  {                                                       \
    SOC_PB_NIF_TYPE nif_type;                             \
    if(!SOC_PB_NIF_IS_SCND_MAL(mal_id)) {                 \
      answer = FALSE;                                     \
    }                                                     \
    res = soc_pb_nif_mal_type_get(unit, mal_id, &nif_type);\
    SOC_SAND_CHECK_FUNC_RESULT(res, 7777, exit);          \
    answer = (nif_type == SOC_PB_NIF_TYPE_XAUI);          \
  }
/* } */
...
STATIC void
  soc_pb_ofp_rates_update_dev_changed_set(
  ...
  )
{
  ...
  res = soc_pb_ofp_rates_update_device_set_unsafe(
          unit,
          TRUE
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

  soc_pb_ofp_rates_update_dev_changed_set(unit, FALSE);
  ...
  if (pp_enable)
  {
    res = soc_petra_sch_if_shaper_rate_set_unsafe(
            unit,
            SOC_PETRA_IF_ID_OLP,
            SOC_PB_OFP_RATES_OLP_GUARANTEED_KBPS,
            &exact
          );
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_init()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_ccmmal2mal(
    SOC_SAND_IN  SOC_PB_OFP_RATES_EGQ_MAL_ID   ccm_ndx
  )
{
  switch(ccm_ndx)
  {
  case SOC_PB_OFP_RATES_EGQ_MAL_00:
  case SOC_PB_OFP_RATES_EGQ_MAL_01:
  case SOC_PB_OFP_RATES_EGQ_MAL_02:
  case SOC_PB_OFP_RATES_EGQ_MAL_03:
  case SOC_PB_OFP_RATES_EGQ_MAL_04:
  case SOC_PB_OFP_RATES_EGQ_MAL_05:
  case SOC_PB_OFP_RATES_EGQ_MAL_06:
  case SOC_PB_OFP_RATES_EGQ_MAL_07:
  case SOC_PB_OFP_RATES_EGQ_MAL_08:
  case SOC_PB_OFP_RATES_EGQ_MAL_09:
  case SOC_PB_OFP_RATES_EGQ_MAL_10:
  case SOC_PB_OFP_RATES_EGQ_MAL_11:
  case SOC_PB_OFP_RATES_EGQ_MAL_12:
  case SOC_PB_OFP_RATES_EGQ_MAL_13:
  case SOC_PB_OFP_RATES_EGQ_MAL_14:
  case SOC_PB_OFP_RATES_EGQ_MAL_15:
    return ccm_ndx;
  case SOC_PB_OFP_RATES_EGQ_MAL_CPU:
    return SOC_PB_MAL_ID_CPU;
  case SOC_PB_OFP_RATES_EGQ_MAL_RCY:
    return SOC_PB_MAL_ID_RCY;
  default:
    return SOC_PB_OFP_RATES_EGQ_MAL_INVALID;
  }
}

STATIC SOC_PB_OFP_RATES_EGQ_MAL_ID
  soc_pb_ofp_rates_mal2ccmmal(
  ...
  )
{
  ...
  case SOC_PB_MAL_ID_OLP:
    return SOC_PB_OFP_RATES_EGQ_MAL_CPU;
  case SOC_PB_MAL_ID_RCY:
    return SOC_PB_OFP_RATES_EGQ_MAL_RCY;
  default:
    return mal_ndx;
  }
}
...
STATIC uint8
  soc_pb_ofp_rates_is_cal_entry_dummy(
  ...
  )
{
  ...
  dummy_entry.port_idx = SOC_PB_OFP_RATES_INVALID_PORT_ID;
...
STATIC uint32
  soc_pb_ofp_rates_mal_egq_from_mal_id(
    uint32                             mal_ndx
  )
{
  uint32
  ...
  switch(mal_type) {
    case SOC_PB_MAL_TYPE_NIF:
      egq_mal_id = mal_ndx;
      break;
    case SOC_PB_MAL_TYPE_CPU:
    case SOC_PB_MAL_TYPE_OLP:
      egq_mal_id = SOC_PB_OFP_RATES_EGQ_MAL_CPU;
      break;
    case SOC_PB_MAL_TYPE_RCY:
      egq_mal_id = SOC_PB_OFP_RATES_EGQ_MAL_RCY;
      break;
    default:
      egq_mal_id = SOC_PB_OFP_RATES_EGQ_MAL_INVALID;
      break;
    }
...
STATIC uint32
  soc_pb_ofp_rates_special_id_detect(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  mal_type = soc_pb_mal_type_from_id(mal_ndx);
  SOC_PB_MAL2IF_NDX(mal_ndx, if_id);
  ...
  if (SOC_PETRA_IS_MAL_EQUIVALENT_ID(mal_ndx))
  {
    ...
    is_channelized = SOC_SAND_NUM2BOOL((mal_type != SOC_PB_MAL_TYPE_ERP));
  }
  else
  {
    nif_type = soc_pb_nif_id2type(if_id);
    is_channelized = SOC_SAND_NUM2BOOL(SOC_PB_NIF_IS_TYPE_ID(ILKN, if_id) || SOC_PB_NIF_IS_TYPE_ID(XAUI, if_id) || SOC_PB_NIF_IS_TYPE_ID(RXAUI, if_id) || SOC_PB_IS_CPU_IF_ID(if_id) || SOC_PB_IS_RCY_IF_ID(if_id));
  }
  ...
  if (SOC_PB_NIF_IS_TYPE_GMII(nif_type))
  {
     ...
     *  that is mapped to an interface with channelized index.
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    if_id = mapping.if_id;
    is_channelized = !(SOC_PB_NIF_IS_GMII_ONLY_ID(soc_pb_nif2intern_id(if_id)));
    is_sgmii = TRUE;
  }

  *is_channelized_id = is_channelized;
  *is_sgmii_id = is_sgmii;
  *interface_id = if_id;

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_special_id_detect()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_cal_per_len_build(
    SOC_SAND_IN   int                   unit,
  ...
  )
{
  ...
  for (port = 0; port < nof_ports; ++port)
  {
    soc_sand_u64_multiply_longs(SOC_PB_OFP_RATES_CAL_RATE_DEVIATION, ports_rates[port], &u64_1);
    rem = soc_sand_u64_devide_u64_long(&u64_1, total_credit_bandwidth, &u64_2);
    soc_sand_u64_to_long(&u64_2, &(temp1));
    temp1 = (rem > 0 ? temp1 + 1 : temp1);

    soc_sand_u64_multiply_longs(SOC_PB_OFP_RATES_CAL_RATE_DEVIATION, slots_rates[port], &u64_1);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_cal_per_len_build()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_cal_len_calculate(
    SOC_SAND_IN  int                   unit,
    SOC_SAND_IN  uint32                    *ports_rates,
    SOC_SAND_IN  uint32                   nof_ports,
    SOC_SAND_IN  uint32                    total_credit_bandwidth,
  ...
  )
{
   ...
   */
  min_deviation = SOC_PB_OFP_RATES_CAL_RATE_DEVIATION;
  best_tentative_len = 0;
  for (tentative_len = 1; tentative_len <= max_calendar_len; ++tentative_len)
  {
    res = soc_pb_ofp_rates_cal_per_len_build(
            unit,
            ports_rates,
            nof_ports,
            total_credit_bandwidth,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_cal_len_calculate()",0,0);
}
  ...
  )
{
  ...
  {
    calendar[slot_idx] = SOC_PB_OFP_RATES_ILLEGAL_SCHEDULER_ID;
  }
  ...
  while (port_num_slots[max_port_idx] > 0)
  {
    ...
    {
      if (calendar[slot_idx] == SOC_PB_OFP_RATES_ILLEGAL_SCHEDULER_ID)
      {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in soc_pb_ofp_rates_fixed_len_cal_build()", 0, 0);
}

STATIC uint32
  soc_pb_ofp_rates_active_calendars_retrieve_sch(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  *sch_cal_len = 0;
  ...
  switch(mal_type) {
  case SOC_PB_MAL_TYPE_NIF:
    offset = SOC_PB_OFP_RATES_CAL_NIF_BASE_SCH_OFFSET * (mal_ndx / 8) + ((mal_ndx % 8) * SOC_PETRA_OFP_NOF_RATES_CAL_SETS + sch_to_get) * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
    break;
  case SOC_PB_MAL_TYPE_CPU:
  case SOC_PB_MAL_TYPE_OLP:
    offset = SOC_PB_OFP_RATES_CAL_CPU_BASE_SCH_OFFSET + sch_to_get * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
    break;
  case SOC_PB_MAL_TYPE_RCY:
    offset = SOC_PB_OFP_RATES_CAL_RCY_BASE_SCH_OFFSET + sch_to_get * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
    break;
  /* must have default. Otherwise, compilation error */
  /* coverity[dead_error_begin : FALSE] */
  default:
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_active_calendars_retrieve_sch()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_from_sch_ports_rates_to_calendar(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  if (recalc)
  {
    res = soc_pb_ofp_rates_cal_len_calculate(
            unit,
            ports_rates,
            nof_ports,
            total_credit_bandwidth,
     ...
     */
    res = soc_pb_ofp_rates_fixed_len_cal_build(
            unit,
            port_nof_slots,
            nof_ports,
            *calendar_len,
  ...
  {
    res = soc_pb_ofp_rates_active_calendars_retrieve_sch(
            unit,
            mal_ndx,
            calendar,
            calendar_len
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_from_sch_ports_rates_to_calendar()",0,0);
}
...
STATIC uint32
  soc_pb_ofp_rates_fill_shaper_calendar_credits(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
    SOC_SAND_IN  uint32                     *ports_rates,
    SOC_SAND_IN  uint32                    nof_ports,
  ...
  )
{
  ...
  for (slot_idx = 0; slot_idx < calendar_len; ++slot_idx)
  {
    ...
    if (port_in_slot == SOC_PB_OFP_RATES_INVALID_PORT_ID)
    {
      continue;
    }

    if (port_in_slot == SOC_PB_OFP_RATES_ILLEGAL_SCHEDULER_ID)
    {
  ...
  */
  ...
  for(port_idx = 0; port_idx < SOC_PETRA_NOF_FAP_PORTS; ++port_idx)
  {
    ...
    soc_sand_u64_multiply_longs(ports_rates[port_idx], calcal_length * (calendar_len + 1) * SOC_PB_EGQ_CAL_SLOT_TRAVERSE_IN_CLOCKS * SOC_PB_EGQ_UNITS_VAL_IN_BITS, &u64_1);
  ...
  for (slot_idx = 0; slot_idx < calendar_len; ++slot_idx)
  {
    ...
    if (port_idx == SOC_PB_OFP_RATES_INVALID_PORT_ID)
    {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_fill_shaper_calendar_credits()",slot_idx,port_idx);
}
 ...
 */
STATIC uint32
  soc_pb_ofp_rates_active_calendars_retrieve_egq(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  *egq_cal_len = 0;
  ...
  case SOC_PB_MAL_TYPE_NIF:
    ...
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_nif_cal_len_reg[reg_idx].nif_ch_spr_cal_len[fld_idx]);
    break;
  case SOC_PB_MAL_TYPE_CPU:
  case SOC_PB_MAL_TYPE_OLP:
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_other_calendars_length_reg.cpu_spr_cal_len);
    break;
  case SOC_PB_MAL_TYPE_RCY:
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_other_calendars_length_reg.rcy_spr_cal_len);
    break;
  case SOC_PB_MAL_TYPE_ERP:
    SOC_PETRA_DO_NOTHING_AND_EXIT;
    break;
   ...
   *  Calendar Length
   */
  SOC_PETRA_IMPLICIT_FLD_IGET(*egq_cal_len_fld, egq_len, egq_to_get, 50, exit);
   ...
   */
  offset = egq_to_get * SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX;
  ...
  if (remove_dummies)
  {
    ...
    {
      cal_slot = egq_cal->slots + slot;
      if (soc_pb_ofp_rates_is_cal_entry_dummy(cal_slot))
      {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_active_calendars_retrieve_egq()",0,0);
}
  ...
  )
{
  ...
  uint32
    nof_dup_entries = 0,
    last_port_entry = SOC_PB_OFP_RATES_INVALID_PORT_ID;
  ...
  SOC_PETRA_ALLOC(slots_ports, uint32, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX);
  SOC_PETRA_ALLOC(temp_buff, uint32, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX);
  ...
  if (recalc)
  {
    ...
    do
    {
      ...
      if ((multiplier *= 2) > SOC_PB_OFP_RATES_NOF_ITERATIONS)
      {
    ...
    for (idx = 0; idx < loc_calendar_len; ++idx)
    {
      if (last_port_entry == slots_ports[idx])
      {
        SOC_SAND_ERR_IF_ABOVE_MAX(
          loc_calendar_len, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX,
          SOC_PETRA_OFP_RATES_CAL_LEN_INVALID_ERR, 22, exit
        );

        /* Space the calendar, inserting dummies between duplicate entries */
        SOC_PETRA_COPY(temp_buff, slots_ports, uint32, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX);
        SOC_PETRA_COPY(temp_buff + idx + 1, slots_ports + idx, uint32, loc_calendar_len - idx);
        SOC_PETRA_COPY(slots_ports, temp_buff, uint32, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX);
        slots_ports[idx] = SOC_PB_OFP_RATES_INVALID_PORT_ID;
  ...
  {
    res = soc_pb_ofp_rates_active_calendars_retrieve_egq(
            unit,
            mal_ndx,
            calendar,
            &loc_calendar_len,
  ...
  res = soc_pb_ofp_rates_fill_shaper_calendar_credits(
          unit,
          mal_ndx,
          ports_rates,
          nof_ports,
  ...
  /* Add the dummy tail */
  calendar->slots[loc_calendar_len - 1].port_idx = SOC_PB_OFP_RATES_INVALID_PORT_ID;
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_from_egq_ports_rates_to_calendar()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_from_calendar_to_ports_sch_rate(
    SOC_SAND_IN  int                    unit,
  ...
  )
{
  ...
  if ((calendar_len == 1) && (calendar->slots[0] == SOC_PETRA_FAP_PORT_ID_INVALID))
  {
    SOC_PETRA_DO_NOTHING_AND_EXIT;
  }
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_from_calendar_to_ports_sch_rate()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_from_calendar_to_ports_egq_rate(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  for (slot_id = 0; slot_id < calendar_len - 1; ++slot_id)
  {
    ...
    if (slot->port_idx == SOC_PB_OFP_RATES_INVALID_PORT_ID)
    {
      continue;
    }
  ...
  for (port_idx = 0; port_idx < nof_ports; ++port_idx)
  {
    ...
    rem = soc_sand_u64_devide_u64_long(&u64_1, calcal_length * calendar_len * SOC_PB_EGQ_UNITS_VAL_IN_BITS * SOC_PB_EGQ_CAL_SLOT_TRAVERSE_IN_CLOCKS, &u64_2);
    soc_sand_u64_to_long(&u64_2, &(calc));
    calc = (rem > 0 ? calc + 1 : calc);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_from_calendar_to_ports_egq_rate()",0,0);
}
 ...
 */
STATIC uint32
  soc_pb_ofp_rates_active_calendars_config(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  egq_mal_id = soc_pb_ofp_rates_mal_egq_from_mal_id(mal_ndx);
  if (egq_mal_id != SOC_PB_OFP_RATES_EGQ_MAL_INVALID)
  {
  ...
  SOC_SAND_ERR_IF_OUT_OF_RANGE(
    sch_cal_len, 1, SOC_PB_OFP_RATES_CAL_LEN_SCH_MAX,
  ...
  {
    SOC_SAND_ERR_IF_OUT_OF_RANGE(
      egq_cal_len, 1, SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX,
  ...
  case SOC_PB_MAL_TYPE_NIF:
    ...
    fld_idx = SOC_PETRA_FLD_IDX_GET(mal_ndx, SOC_PETRA_MALS_IN_NIF_GRP);
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_nif_cal_len_reg[reg_idx].nif_ch_spr_cal_len[fld_idx]);
    sch_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.ch_nif_cal_config_reg[mal_ndx].cal_len[sch_to_set]);
    sch_cal_sel_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.ch_nif_cal_config_reg[mal_ndx].dvscalendar_sel_ch_nifxx);
    break;
  case SOC_PB_MAL_TYPE_CPU:
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_other_calendars_length_reg.cpu_spr_cal_len);
    sch_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.cpu_cal_configuration_reg.cal_len[sch_to_set]);
    sch_cal_sel_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.cpu_cal_configuration_reg.dvscalendar_sel_cpu);
    break;
  case SOC_PB_MAL_TYPE_RCY:
    egq_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_other_calendars_length_reg.rcy_spr_cal_len);
    sch_cal_len_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.rcy_cal_configuration_reg.cal_len[sch_to_set]);
    sch_cal_sel_fld = SOC_PETRA_REG_DB_ACC_REF(regs->sch.rcy_cal_configuration_reg.dvscalendar_sel_rcy);
    break;
  default:
    SOC_SAND_SET_ERROR_CODE(SOC_PETRA_OFP_RATES_MAL_TYPE_INVALID_ERR, 45, exit);
   ...
   *  Calendar length
   */
  SOC_PETRA_IMPLICIT_FLD_ISET(*egq_cal_len_fld, egq_len, egq_to_set, 50, exit);

  if (!egq_only)
  {
    SOC_PETRA_IMPLICIT_FLD_SET(*sch_cal_len_fld, sch_len, 60, exit);
  }
  ...
  {
    offset = egq_to_set * SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX;
    ...
    if (egq_data.ofp_index != SOC_PB_OFP_RATES_INVALID_PORT_ID)
    {
  ...
  if (!egq_only)
  {
    ...
    switch(mal_type) {
    case SOC_PB_MAL_TYPE_NIF:
      offset = SOC_PB_OFP_RATES_CAL_NIF_BASE_SCH_OFFSET * (mal_ndx / 8) + ((mal_ndx % 8) * SOC_PETRA_OFP_NOF_RATES_CAL_SETS + sch_to_set) * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
      break;
    case SOC_PB_MAL_TYPE_CPU:
      offset = SOC_PB_OFP_RATES_CAL_CPU_BASE_SCH_OFFSET + sch_to_set * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
      break;
    case SOC_PB_MAL_TYPE_RCY:
      offset = SOC_PB_OFP_RATES_CAL_RCY_BASE_SCH_OFFSET + sch_to_set * SOC_PB_OFP_RATES_CAL_LEN_SCH_OFFSET;
      break;
    /* must default. Otherwise - compilation error */
    /* coverity[dead_error_begin:FALSE] */
    default:
    ...
    for (slot = 0; slot < sch_cal_len; ++slot)
    {
      ...
      if (sch_cal->slots[slot] > SOC_PETRA_NOF_FAP_PORTS)
      {
  ...
  {
    fld_val = sch_to_set;
    SOC_PETRA_IMPLICIT_FLD_SET(*sch_cal_sel_fld, fld_val, 110, exit);
  }

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_active_calendars_config()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_active_calendars_retrieve(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
    SOC_SAND_OUT SOC_PETRA_OFP_RATES_CAL_SCH      *sch_cal,
    SOC_SAND_OUT uint32                     *sch_cal_len,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_active_calendars_retrieve()",0,0);
}
...
STATIC uint32
  soc_pb_ofp_rates_egq_shaper_rate_to_internal(
    SOC_SAND_IN  int                    unit,
  ...
  )
{
  ...
  {
    soc_sand_u64_multiply_longs(rate_kbps, (1000 * SOC_PB_EGQ_UNITS_VAL_IN_BITS), &calc);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_egq_shaper_rate_to_internal()",0,0);
}
...
STATIC uint32
  soc_pb_ofp_rates_egq_shaper_rate_from_internal(
    SOC_SAND_IN  int                    unit,
  ...
  )
{
  ...
  else
  {
    ...
    soc_sand_u64_devide_u64_long(&calc, (1000 * SOC_PB_EGQ_UNITS_VAL_IN_BITS), &calc2);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_egq_shaper_rate_from_internal()",0,0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_sw_db_set()",0,0);
}
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_sw_db_get()",0,0);
}

STATIC uint32
  soc_pb_ofp_rates_active_mal_build(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  SOC_PETRA_CLEAR(active_mals, uint8, SOC_PB_OFP_RATES_EGQ_NOF_MALS);

  res = soc_petra_sw_db_egr_ports_active_mals_get(
          unit,
          SOC_PB_NOF_MAC_LANES,
          active_mals
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  for (entry_idx = 0; entry_idx < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++entry_idx)
  {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_active_mal_build()",0,0);
}
...
*********************************************************************/
STATIC uint32
  soc_pb_ofp_rates_internal_unsafe(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
    SOC_SAND_IN  uint8                    recalc,
  ...
  )
{
  ...
  SOC_PETRA_OFP_RATES_MAL_SHPR_INFO
    ...
    shaper_to_set;
  uint8
    active_mals[SOC_PB_OFP_RATES_EGQ_NOF_MALS];
  ...
  for (entry_idx = 0; entry_idx < ofp_rate_tbl->nof_valid_entries; ++entry_idx)
  {
    ...
    res = soc_pb_ofp_rates_special_id_detect(
            unit,
            mal_ndx,
            ofp_idx,
            &is_channelized_id,
    ...
    mapped_mal_id = SOC_PB_IF2MAL_NDX(if_id);
    ...
    if (mapped_mal_id == mal_ndx)
    {
      ...
      {
        /*
         *  XAUI/SPAUI/CPU(+OLP)/RCY/SGMII-0, 2, 4, 6,..
         */
  ...
  /* Rates to calendars */
  res = soc_pb_ofp_rates_from_sch_ports_rates_to_calendar(
          unit,
          mal_ndx,
          recalc,
          sch_rates,
          ...
          sch_mal_rate_requested,
          SOC_PB_OFP_RATES_CAL_LEN_SCH_MAX,
          sch_calendar,
          &sch_calendar_len
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);

  res = soc_pb_ofp_rates_from_egq_ports_rates_to_calendar(
          unit,
          mal_ndx,
          recalc,
          egq_rates,
          ...
          SOC_PB_OFP_RATES_CAL_LEN_EGQ_MAX,
          egq_calendar,
          &egq_calendar_len
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);

  res = soc_pb_ofp_rates_active_mal_build(
          unit,
          mal_ndx,
  ...
  /* Write to device */
  res = soc_pb_ofp_rates_active_calendars_config(
          unit,
          mal_ndx,
          sch_calendar,
          sch_calendar_len,
  ...
  if (shaper)
  {
    ...
    res = soc_pb_ofp_rates_mal_shaper_set_unsafe(
            unit,
            mal_ndx,
            &shaper_to_set,
            &exact_shaper_to_set
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_internal_unsafe()",mal_ndx,0);
}
  ...
  )
{
  uint32
    res = SOC_SAND_OK;
  uint32
    egq_calcal_rates[SOC_PB_OFP_RATES_EGQ_NOF_MALS];
  uint32
    egq_calcal_instances[SOC_PB_OFP_RATES_EGQ_NOF_MALS];
  uint32
...
/* The stucture SOC_PETRA_OFP_RATES_TBL_INFO is very big */
  ...
  for (idx = 0; idx < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++idx)
  {
  ...
  res = soc_pb_ofp_rates_sw_db_set(
          unit,
          mal_ndx,
          ofp_rate_tbl
        );
  ...
  if (soc_petra_sw_db_ofp_rates_update_device_get(unit) == TRUE)
  {
    ...
    egq_calcal_mal_rate_requested = 0;
    for (mal_i = 0; mal_i < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++mal_i)
    {
      SOC_PB_OFP_RATES_IS_MAL_XAUI_SKIP(mal_i, is_mal_xaui_skip);
      if (is_mal_xaui_skip)
      {
        continue;
      }
      soc_petra_PETRA_OFP_RATES_TBL_INFO_clear(temp_rate_tbl);
      res = soc_pb_ofp_rates_sw_db_get(
              unit,
              soc_pb_ofp_rates_ccmmal2mal(mal_i),
              temp_rate_tbl,
              FALSE
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
    ...
    res = soc_pb_ofp_rates_from_rates_to_calendar(
            unit,
            egq_calcal_rates,
            SOC_PB_OFP_RATES_EGQ_NOF_MALS,
            egq_calcal_mal_rate_requested,
            SOC_PB_OFP_RATES_CALCAL_LEN_EGQ_MAX,
            egq_calcal_calendar,
    ...
    for (idx = 0; idx < egq_calcal_calendar_len; ++idx)
    {
      ...
      res = soc_petra_egq_ccm_tbl_set_unsafe(
              unit,
              idx + (cal2set) * SOC_PB_OFP_RATES_CALCAL_LEN_EGQ_MAX,
              &ccm_tbl_data
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 60, exit);
    }
    ...
    for (mal_i = 0; mal_i < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++mal_i)
    {
      ...
      cc_mal_id = soc_pb_ofp_rates_ccmmal2mal(mal_i);

      SOC_PB_OFP_RATES_IS_MAL_XAUI_SKIP(cc_mal_id, is_mal_xaui_skip);
      if (is_mal_xaui_skip)
      {
        continue;
      }
      ...
      if (egq_calcal_instances[mal_i] != 0)
      {
		...
		/*	CC_MAL_ID is never OLP
			Recalc and set Shaper for CPU if mal_ndx is OLP */
		recalc = SOC_SAND_NUM2BOOL((mal_ndx == cc_mal_id) ||											/* ReCalc requested MAL */
							((mal_ndx == SOC_PETRA_MAL_ID_OLP) && (cc_mal_id == SOC_PETRA_MAL_ID_CPU)) ||	/* ReCalc CPU as well if requested OLP */
							(update_dev_changed));													/* ReCalc if update device */
        /* petra-b code. Almost not in use. Ignore coverity defects */
        /* coverity[overrun-call] */
        res = soc_pb_ofp_rates_internal_unsafe(
                unit,
                cc_mal_id,
                recalc,
				((mal_ndx == cc_mal_id) ||													/* Set shaper for requested MAL */
				((mal_ndx == SOC_PETRA_MAL_ID_OLP) && (cc_mal_id == SOC_PETRA_MAL_ID_CPU)) ||		/* Set shaper for CPU as well if requested OLP */
				(update_dev_changed) ? shaper : NULL),
                temp_rate_tbl
             );
        SOC_SAND_CHECK_FUNC_RESULT(res, 80, exit);
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_set_unsafe()",mal_ndx,0);
}
  ...
  )
{
  ...
  res = soc_pb_mal_equivalent_id_verify(
          mal_ndx
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  res = soc_pb_ofp_rates_mal_shaper_verify(
          unit,
          mal_idx_last,
          shaper
        );
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_verify()",mal_ndx,0);
}
    ...
    )
{
  ...
  /* Read from device */
  res = soc_pb_ofp_rates_active_calendars_retrieve(
          unit,
          mal_ndx,
          sch_calendar,
          &sch_calendar_len,
   ...
   */
  res = soc_pb_ofp_rates_from_calendar_to_ports_sch_rate(
          unit,
          sch_calendar,
          sch_calendar_len,
          sch_mal_rate,
  ...
  res = soc_pb_ofp_rates_from_calendar_to_ports_egq_rate(
          unit,
          mal_ndx,
          egq_calendar,
          egq_calendar_len,
  ...
  res = soc_pb_ofp_rates_mal_shaper_get_unsafe(
          unit,
          mal_ndx,
          shaper
        );
...
exit:
  ...
  SOC_PETRA_FREE(sch_calendar);
  SOC_PETRA_FREE(egq_calendar);
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_get_unsafe()",mal_ndx,0);
}
  ...
  )
{
  ...
  for (ofp_rate_tbl->nof_valid_entries = 0, mal_i = 0; mal_i < SOC_PB_OFP_RATES_EGQ_NOF_MALS; mal_i++)
  {
    ...
    else
    {
      ...
      res = soc_pb_ofp_rates_get_unsafe(
              unit,
              soc_pb_ofp_rates_ccmmal2mal(mal_i),
              &shaper,
              ofp_table
    ...
    for (ofp_i = 0; ofp_i < ofp_table->nof_valid_entries; ++ofp_i)
    {
      if (ofp_table->rates[ofp_i].port_id != SOC_PETRA_OFP_RATES_ILLEGAL_PORT_ID)
      {
        ...
        ofp_rate_tbl->rates[ofp_rate_tbl->nof_valid_entries].egq_rate = soc_petra_sw_db_ofp_rates_nof_instances_get(unit, cc_mal_id) ? ofp_table->rates[ofp_i].egq_rate : 0;
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_all_ofp_rates_get_unsafe()",mal_i,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PETRA_OFP_RATES_UPDATE_UNSAFE);
  ...
  res = soc_pb_ofp_rates_set_unsafe(
          unit,
          mal_idx,
          shaper,
          ofp_rate_tbl
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_single_port_set_unsafe()",ofp_ndx,0);
}
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_single_port_verify()",ofp_ndx,0);
}
...
/* 
* COVERITY * 
* checked well, and no overun found. */
/* coverity[overrun-call] */
  res = soc_pb_ofp_rates_get_unsafe(
          unit,
          mal_idx,
...
exit:
  SOC_PETRA_FREE(ofp_rate_tbl);
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_single_port_get_unsafe()",ofp_ndx,0);
}
...
STATIC uint32
  soc_pb_ofp_rates_egq_shaper_config(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
  ...
  )
{
  ...
  switch(mal_type) {
  case SOC_PB_MAL_TYPE_NIF:
    egq_shpr_rate_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_nif_reg[mal_ndx].spr_rate);
    egq_shpr_burst_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_nif_reg[mal_ndx].spr_max_burst);
    break;
  case SOC_PB_MAL_TYPE_CPU:
    egq_shpr_rate_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_eci_ports_reg.cpu_spr_rate);
    egq_shpr_burst_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_eci_ports_reg.cpu_spr_max_burst);
    break;
  case SOC_PB_MAL_TYPE_RCY:
    egq_shpr_rate_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_recycle_interface_ports_reg.rcy_spr_rate);
    egq_shpr_burst_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_for_recycle_interface_ports_reg.rcy_spr_max_burst);
    break;
  case SOC_PB_MAL_TYPE_ERP:
    SOC_PETRA_DO_NOTHING_AND_EXIT;
    break;
  ...
  if (rate_enable)
  {
    res = soc_pb_ofp_rates_egq_shaper_rate_to_internal(
            unit,
            rate,
            &fld_val
          );
    ...
    {
      /* Set shaper rate */
      SOC_PETRA_IMPLICIT_FLD_ISET(*egq_shpr_rate_fld, fld_val, cal_set, 30, exit);
      /* Set shaper max burst */
      SOC_PETRA_IMPLICIT_FLD_ISET(*egq_shpr_burst_fld, 0x3ff, cal_set, 30, exit);
    }
  }
   ...
   */
  SOC_PETRA_IMPLICIT_FLD_IGET(*egq_shpr_rate_fld, fld_val, SOC_PETRA_OFP_RATES_CAL_SET_A, 50, exit);

  res = soc_pb_ofp_rates_egq_shaper_rate_from_internal(
          unit,
          fld_val,
          exact_rate
        );
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_egq_shaper_config()",mal_ndx,0);
}
...
*********************************************************************/
STATIC uint32
  soc_pb_ofp_rates_egq_shaper_retrieve(
    SOC_SAND_IN  int                    unit,
    SOC_SAND_IN  uint32                    mal_ndx,
    SOC_SAND_IN  uint8                    rate_enable,
  ...
  )
{
  ...
  switch(mal_type) {
  case SOC_PB_MAL_TYPE_NIF:
    if (mal_ndx >= SOC_PETRA_EGR_SHPR_CONF_NIF_NOF_REGS) {
        SOC_SAND_SET_ERROR_CODE(SOC_PETRA_OFP_RATES_MAL_TYPE_INVALID_ERR, 10, exit);
    }
    egq_shpr_rate_fld = SOC_PETRA_REG_DB_ACC_REF(regs->egq.egress_shpr_conf_nif_reg[mal_ndx].spr_rate);
    break;
  ...
  if (rate_enable)
  {
    SOC_PETRA_IMPLICIT_FLD_IGET(*egq_shpr_rate_fld, fld_val, SOC_PETRA_OFP_RATES_CAL_SET_A, 30, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_egq_shaper_retrieve()",mal_ndx,0);
}
  ...
  )
{
  ...
  if (if_id == SOC_PETRA_NIF_ID_NONE)
  {
    /*
     *	Probably a disabled MAL - exit without any action.
     */
    SOC_PETRA_DO_NOTHING_AND_EXIT;
  }
...
* checked well, and no overun found. */
/* coverity[overrun-call] */
  res = soc_pb_ofp_rates_egq_shaper_config(
          unit,
          mal_ndx,
          egq_rate_enable,
          egq_rate_to_set,
...
exit:
  SOC_PETRA_FREE(mal_ports_db);
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_mal_shaper_set_unsafe()",mal_ndx,0);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_mal_shaper_verify()",mal_ndx,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(SOC_PETRA_OFP_RATES_MAL_SHAPER_GET_UNSAFE);
  ...
  res = soc_pb_ofp_rates_egq_shaper_retrieve(
          unit,
          mal_ndx,
          TRUE,
          &egq_rate_to_get
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_mal_shaper_get_unsafe()",mal_ndx,0);
}
  ...
  )
{
  ...
  nof_fatp_ports = fatp_info.nof_ports;
  fatp_base_port = fatp_info.base_port_id;
  ...
  for (port_idx = fatp_base_port; port_idx < fatp_base_port + nof_fatp_ports; ++port_idx)
  {
    sch_per_port_rate = (port_idx == fatp_base_port)?fatp_rate->sch_rate:0;
    ...
    res = soc_pb_interface_id_verify(
            map_info.if_id
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    mal_idx = SOC_PB_IF2MAL_NDX(map_info.if_id);
...
exit:
  ...
  SOC_PETRA_FREE(ofp_rate_tbl);
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_fat_pipe_rate_set_unsafe()",fatp_ndx,0);
}
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(fatp_ndx, (SOC_PB_NIF_NOF_FATP_IDS-1), SOC_PB_FATP_NDX_OUT_OF_RANGE_ERR, 10, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_fat_pipe_rate_verify()",fatp_ndx,0);
}
...
/* coverity[overrun-call] */
  ...
  if (ofp_rate_tbl->rates[0].port_id != fatp_base_port)
  {
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_fat_pipe_rate_get_unsafe()",fatp_ndx,0);
}
  ...
  )
{
  ...
  uint32
    egq_calcal_passes = 0,
    curr_port_index = SOC_PB_OFP_RATES_INVALID_PORT_ID,
    last_port_index = SOC_PB_OFP_RATES_INVALID_PORT_ID,
    idx = 0;
  uint32
    egq_calcal_calendar_len,
    curr_port_crdit = 0,
  ...
  SOC_PETRA_ALLOC(egq_cal_lens, uint32, SOC_PB_OFP_RATES_EGQ_NOF_MALS);
  SOC_PETRA_ALLOC(current_zero, uint32, SOC_PB_OFP_RATES_EGQ_NOF_MALS);
  SOC_PETRA_ALLOC(current_slot, uint32, SOC_PB_OFP_RATES_EGQ_NOF_MALS);
  SOC_PETRA_ALLOC(egq_calendar, SOC_PETRA_OFP_RATES_CAL_EGQ, SOC_PB_OFP_RATES_EGQ_NOF_MALS);

  for (idx = 0; idx < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++idx)
  {
    res = soc_pb_ofp_rates_active_calendars_retrieve_egq(
            unit,
            soc_pb_ofp_rates_ccmmal2mal(idx),
            egq_calendar + idx,
            egq_cal_lens + idx,
            FALSE
          );
  ...
  for (next_time_stop = FALSE, egq_calcal_passes = 1; ; ++egq_calcal_passes)
  {
    for (idx = 0; idx < egq_calcal_calendar_len; ++idx)
    {
      res = soc_petra_egq_ccm_tbl_get_unsafe(
              unit,
              idx + (act_cal) * SOC_PB_OFP_RATES_CALCAL_LEN_EGQ_MAX,
              &ccm_tbl_data
            );
      SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    ...
    if (!sal_memcmp(current_slot, current_zero, SOC_PB_OFP_RATES_EGQ_NOF_MALS * sizeof(uint32)))
    {
      if (next_time_stop)
      {
        break;
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_egq_calendar_validate_unsafe()", egq_calcal_passes, idx);
}
  ...
  )
{
  ...
  {
    soc_pb_ofp_rates_update_dev_changed_set(unit, TRUE);
...
exit:
  SOC_PETRA_FREE(ofp_rate_tbl);
  SOC_PETRA_FREE(mal_ports_db);
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_update_device_set_unsafe()",mal_idx,ofp_idx);
}
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_update_device_get_unsafe()",0,0);
}
  ...
  )
{
  ...
  SOC_PETRA_OFP_RATES_MAL_SHPR_INFO
    shaper[SOC_PB_OFP_RATES_EGQ_NOF_MALS];
  uint8
  ...
  for(ccm_idx = 0; ccm_idx < SOC_PB_OFP_RATES_EGQ_NOF_MALS; ++ccm_idx)
  {
...
/* coverity[overrun-call] */
    res = soc_pb_ofp_rates_get_unsafe(
            unit,
            soc_pb_ofp_rates_ccmmal2mal(ccm_idx),
            &shaper[ccm_idx],
            ofp_rate_tbl_get
          );
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit_restore);

    init_rate = SOC_PB_OFP_RATES_TEST_RANDOM_INITIAL_PORT_RATE / ofp_rate_tbl_get->nof_valid_entries;
  ...
  for(idx1 = 0; idx1 < SOC_PETRA_NOF_FAP_PORTS; ++idx1)
  {
    ...
    if (mal_idx != SOC_PETRA_IF_ID_NONE)
    {
      ccm_idx = soc_pb_ofp_rates_mal2ccmmal(mal_idx);

      res = soc_pb_ofp_rates_single_port_get_unsafe(
              unit,
              idx1,
              &dummy_shaper,
      ...
      res = soc_pb_ofp_rates_single_port_set_unsafe(
              unit,
              idx1,
              &shaper[ccm_idx],
              &ofp_rate_tbl_set->rates[idx1]
      ...
      for(idx2 = 0; idx2 < ofp_rate_tbl_get->nof_valid_entries; ++idx2)
      {
        ...
        if (mal_idx != SOC_PETRA_IF_ID_NONE)
        {
          if (ofp_rate_tbl_set->rates[ofp_rate_tbl_get->rates[idx2].port_id].port_id != SOC_PETRA_OFP_RATES_ILLEGAL_PORT_ID)
          {
            ...
            if (
                (delta_rate_sch > SOC_PB_OFP_RATES_TST_MAX_SCH_DELTA_PERCENT) ||
                (delta_rate_egq > SOC_PB_OFP_RATES_TST_MAX_EGQ_DELTA_PERCENT)
               )
            {
              soc_sand_os_printf(
...
exit:
  ...
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in soc_pb_ofp_rates_test_random_unsafe()",idx1,idx2);
}
