...
/* $Id: arad_pp_frwrd_trill.c,v 1.57 Broadcom SDK $
    ...
    Arad_pp_procedure_desc_element_frwrd_trill[] =
{
  ...
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET_UNSAFE),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET_PRINT),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET_VERIFY),
  SOC_PROCEDURE_DESC_ELEMENT_DEF(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET_UNSAFE),
    ...
    )
{
  ...
  *add_info = 0;
  ...
  em_dest.type = ARAD_PP_FRWRD_DECISION_TYPE_MC;
    ...
    )
{
  uint32
    num_bits;
  ARAD_PP_TRILL_MC_MASKED_FIELDS
    masked_fields;
  uint32
    res = SOC_SAND_OK;
  ...
  ARAD_PP_TRILL_MC_MASKED_FIELDS_clear(&masked_fields);
...
/* Two modes <msb ... lsb>:
  ...
  /* In legacy VL mode:    Lookup key = {Inner-Eth.FID,       ESADI, Dist-Nick}
     In FGL 1-tag program: Lookup key = {12'b0, Inner-Eth.outer_tag, ESADI, Dist-Nick}
     In FGL 2-tag program: Lookup key = {Inner-Eth.inner_tag, Inner-Eth.outer_tag, ESADI, Dist-Nick} */
    ...
    )
{
    ...
    /* The priority is irrelevant for TRILL records */
    priority = 0; 
    ...
    )
{
    ...
    /* Encode Key into buffer according to the following format:
     *  
     * {Esadi(1) in 31, FID(14:0) in 30:16, Tree-nick(15:0) in 15:0}
     *  
     */
...
/*  reg_val = DEFAULT_TRILL_ETHER_TYPE;*/
  ...
  /* SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, ECI_GLOBAL_PP_5r, REG_PORT_ANY, 0, ETHERTYPE_TRILLf,  reg_val)); */

  /* VSI assignment mode in VTT configuration */

  

  /* Trill-prefix to the eei when building a trill tunne
   * Until Arad+, for the learning data: the eei (nickname) was: Cfg-TRILL-EEI-msb (8), TRILL-Header.Ing-Nick(16) 
     For Jericho, for the learning data: No trill prefix to the eei. The eei is saved in the lif.learningData. */
  ...
  /* Workaround for ARAD-A0: encap_id for non-trill ports in multicast ID */
  ...
  if SOC_IS_ARADPLUS_AND_BELOW(unit) {
      fld_val = 0x14; /* ETH packet (SA+DA+ETH_TYPE) */
  ...
  )
{
  ...
  ARAD_PP_L2_LIF_TRILL_INFO_clear(trill_info);
  ...
  if (found)
  {
    *lif_index = sem_entry.sem_result_ndx;
    ...
    /* get learn info from the lif.
     * Learn info changes between arad+ and Jericho. 
     * In arad+: the learn info is FEC only. (Link Layer) 
     *           The egress nickname is built using the packet. 
     *           (packet.trill_header.ingress_nickname)        
     * In Jer: the learn data is FEC + EEI (nickname)
     *         Same as mact payload. */

      payload_data[0] = COMPILER_64_LO(data.learn_info);
      payload_data[1] = COMPILER_64_HI(data.learn_info);

      /* parse learn info buffer to dest + additional info */
      ...
      /* get forward decision (learn data) from dest + additional info */
      res = arad_pp_fwd_decision_in_buffer_parse(
         unit, 
      ...
      trill_info->protection_pass_value = data.protection_path;
      trill_info->protection_pointer = data.protection_pointer;
  ...
  )
{
  ...
  if (!ignore_key)
  {
    ...
    sem_entry.sem_result_ndx = lif_index;
    /* hana 
    sem_entry.sem_op_code = ARAD_PP_LIF_OPCODE_NO_COS; 
    */ 
  ...
  */
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    ...
    /* vsi base is not considered as VSI-assignment mode mask it.*/
    data.vsi = 0x0; /* max */
    data.tt_learn_enable = trill_info->learn_enable;
    data.tpid_profile = 0;
    data.service_type = ARAD_PP_RIF_ISEM_RES_SERVICE_TYPE;
    data.vsi_assignment_mode = 3; /* USe what came from isem-b */

    /* Use the supplied Protection info */
    data.protection_path = trill_info->protection_pass_value;
    data.protection_pointer = trill_info->protection_pointer;
    ...
    /* build dest and asd according to forwarding decision */
    res = arad_pp_fwd_decision_in_buffer_build(
       unit, 
       ARAD_PP_FRWRD_DECISION_APPLICATION_TYPE_DFLT,
       &(trill_info->learn_info), 
    ...
    /* arad+: frwrd decision: dest: FEC, asd: EEI */
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        payload.flags = ARAD_PP_FWD_DECISION_PARSE_EEI; 
    } 
    /* for jer: frwrd decision: FEC, asd: NONE */
    else if (SOC_IS_JERICHO(unit)) {
        /* indicate no additional data */
        payload.flags = ARAD_PP_FWD_DECISION_PARSE_DEST; 
    }

    /* build payload buffer according to dest and asd */
    ...
    res = arad_pp_sw_db_lif_table_entry_use_set(
            unit,
            lif_index,
            ARAD_PP_LIF_ENTRY_TYPE_TRILL_NICK,
            0 /* not in use for Trill */
          );
  ...
  )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  *lif_index = (ARAD_PP_LIF_ID) sem_entry.sem_result_ndx;
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_TRILL_MC_MASKED_FIELDS, masked_fields, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(fwd_decision->dest_id, SOC_DPP_DEFS_GET(unit, nof_fecs), ARAD_PP_FEC_ID_OUT_OF_RANGE_ERR, 30, exit);
  ...
  )
{
  ...
  /* If arad_pp_l2_lif_trill_uc_get_internal_unsafe does not read from LEM
   * but only from In-LIF table, then there is also no need to read from ELK
   */
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  /* Check if Exteral Lookup device is used */
  if(ARAD_KBP_ENABLE_TRILL_UC) 
...
/*********************************************************************
*     Map nick-name to a FEC entry ID. Used to associate the FEC as learning information, 
 *     upon receiving packets with the Nick-Name as the source address
 *     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(lif_index, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  SOC_SAND_ERR_IF_ABOVE_MAX(nickname_key, ARAD_PP_FRWRD_TRILL_NICKNAME_KEY_MAX, ARAD_PP_FRWRD_TRILL_NICKNAME_KEY_OUT_OF_RANGE_ERR, 20, exit);

  res = ARAD_PP_L2_LIF_TRILL_INFO_verify(unit, trill_info);
  SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
  ...
  )
{
...
#endif
  {
    ...
    /*This proc cam be called for :                                                       */
    /*bcm_trill_multicast_add with only vid & mc_group - Only the LEM entry is created.   */
    /*bcm_trill_multicast_source_add with ingr_nick_name - Only the TCAM entry is created */
    /*others - both are creatd.                                                           */

    /***************************************************************/
    /*  add LEM entry when Trill transparent service not enabled   */
    /*  or                                                         */
    /*  add TCAM entry when Trill transparent service is enabled   */
    /***************************************************************/
    /* in VL mode && prune mode: fid != 0
       in VL mode && not prune mode, fid is ignored */
    ...
    {      
      if (trill_mc_key->tts == 0) {
        ...
        key.type = ARAD_IP_TCAM_ENTRY_TYPE_TRILL_MC;
        key.key.trill_mc = *trill_mc_key;
        key.vrf_ndx = 0;
        ...
        action.value[0] = payload.dest & 0x7ffff;
  ...
  )
{
  ...
  res = ARAD_PP_TRILL_MC_ROUTE_KEY_verify(unit, trill_mc_key);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ...
  )
{
  ...
  {
    /***************************************************************/
    /*  get LEM entry when Trill transparent service not enabled   */
    /*  or                                                         */
    /*  get TCAM entry when Trill transparent service is enabled   */
    /***************************************************************/
    if (trill_mc_key->tts == 0) {
  ...
  )
{
  ...
  {
    /******************************************************************/
    /*  remove LEM entry when Trill transparent service not enabled   */
    /*  or                                                            */
    /*  remove TCAM entry when Trill transparent service is enabled   */
    /******************************************************************/
    if (trill_mc_key->tts == 0) {
      ARAD_PP_LEM_ACCESS_REQUEST_clear(&request);
      ARAD_PP_LEM_ACCESS_ACK_STATUS_clear(&ack);
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_TRILL_ADJ_INFO, mac_auth_info, 20, exit);
  ...
  )
{
  ...
  /*soc_sand_os_memset(mac_auth_info, 0x0, sizeof(ARAD_PP_TRILL_ADJ_INFO));*/
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET_UNSAFE);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_SET_VERIFY);

  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO, glbl_info, 10, exit);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET_VERIFY);
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_GET_UNSAFE);

  SOC_SAND_CHECK_NULL_INPUT(glbl_info);

  ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_clear(glbl_info);
...
uint32
  ARAD_PP_TRILL_MC_MASKED_FIELDS_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_TRILL_MC_MASKED_FIELDS_verify()",0,0);
}

uint32
  ARAD_PP_TRILL_MC_ROUTE_KEY_verify(
    SOC_SAND_IN  int                        unit,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_TRILL_MC_ROUTE_KEY_verify()",0,0);
}

uint32
  ARAD_PP_TRILL_ADJ_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_TRILL_ADJ_INFO_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_TRILL_GLOBAL_INFO_verify()",0,0);
}
uint32
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(vsi, ARAD_PP_VSI_ID_MAX, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 10, exit);
  SOC_SAND_ERR_IF_ABOVE_MAX(high_vid, ARAD_PP_VSI_ID_MAX, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 20, exit);
  SOC_SAND_ERR_IF_ABOVE_MAX(low_vid, ARAD_PP_VSI_ID_MAX, ARAD_PP_VSI_ID_OUT_OF_RANGE_ERR, 30, exit);
