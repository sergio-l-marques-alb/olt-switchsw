  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_ITM_GLOB_RCS_DROP_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_itm_glob_rcs_drop_set()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_ITM_VSQ_WRED_GEN_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  SOC_SAND_CHECK_NULL_INPUT(info);
  if (SOC_IS_JERICHO(unit)) {
    /*irelevent for jericho*/
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_itm_vsq_wred_gen_set()",0,0);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_ITM_VSQ_WRED_GEN_GET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;
...
exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_itm_vsq_wred_gen_get()",0,0);
}
...
{
  SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_ITM_DYN_TOTAL_THRESH_SET);
  SOC_SAND_CHECK_DRIVER_AND_DEVICE;

  SOC_SAND_TAKE_DEVICE_SEMAPHORE;

  SOC_SAND_CHECK_FUNC_RESULT(arad_itm_dyn_total_thresh_set_unsafe(unit, reservation_increase), 100, exit_semaphore);

exit_semaphore:
  SOC_SAND_GIVE_DEVICE_SEMAPHORE;
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_itm_dyn_total_thresh_set()", reservation_increase, 0);
}
  ...
  )
{
  ...
  if (is_ocb_only) {
      ...
      res = arad_itm_vsq_in_group_size_get(vsq_group, &vsq_in_group_size);
      SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
*    3. [vsq_group = 2 ,vsq_in_group_ndx, RANGE: 0 - 63]--> global (36-99).
*    4. [vsq_group = 3 ,vsq_in_group_ndx, RANGE: 0 - 255]--> global (100-355).
*    5. [vsq_group = 4 ,vsq_in_group_ndx, RANGE: 0 - 31]--> global (0-31).
*    6. [vsq_group = 5 ,vsq_in_group_ndx, RANGE: 0 - 255]--> global (0-255).
*********************************************************************/
uint32
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS_NO_DEVID(ARAD_ITM_VSQ_CONVERT_GROUP_INDX_TO_GLOBAL_INDX);
  ...
  )
{
  ...
  if (vsq_ndx < ARAD_ITM_VSQ_GROUP_CTGRY_SIZE * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_CTGRY;
    *vsq_in_group_ndx = vsq_ndx % ARAD_ITM_VSQ_GROUP_CTGRY_SIZE;
    *is_ocb_only = vsq_ndx / ARAD_ITM_VSQ_GROUP_CTGRY_SIZE;
  }
  else if (vsq_ndx<(ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE) * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS;
    *vsq_in_group_ndx = (vsq_ndx - (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE * vsq_is_ocb_max)) % ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE;
    *is_ocb_only = (vsq_ndx - (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE * vsq_is_ocb_max)) / ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE;
  }
  else if (vsq_ndx< (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE) * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS;
    *vsq_in_group_ndx = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE) * vsq_is_ocb_max)) % ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE;
    *is_ocb_only = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE) * vsq_is_ocb_max)) / ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE;
  }
  else if(vsq_ndx < (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
          + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE) * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_STTSTCS_TAG;
    *vsq_in_group_ndx = (vsq_ndx - 
         ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE) * vsq_is_ocb_max)) % ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE;
    *is_ocb_only = (vsq_ndx - 
         ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE) * vsq_is_ocb_max)) / ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE;
  }
  else if(vsq_ndx < (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
          + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE + ARAD_ITM_VSQ_GROUP_LLFC_SIZE) * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_LLFC;
    *vsq_in_group_ndx = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
      + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE) * vsq_is_ocb_max)) % ARAD_ITM_VSQ_GROUP_LLFC_SIZE;
    *is_ocb_only = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
      + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE) * vsq_is_ocb_max)) / ARAD_ITM_VSQ_GROUP_LLFC_SIZE;
  }
  else if(vsq_ndx < (ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
    + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE + ARAD_ITM_VSQ_GROUP_LLFC_SIZE + ARAD_ITM_VSQ_GROUP_PFC_SIZE) * vsq_is_ocb_max)
  {
    *vsq_group = ARAD_ITM_VSQ_GROUP_PFC;
    *vsq_in_group_ndx = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
      + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE + ARAD_ITM_VSQ_GROUP_LLFC_SIZE) * vsq_is_ocb_max)) % ARAD_ITM_VSQ_GROUP_PFC_SIZE;
    *is_ocb_only = (vsq_ndx - ((ARAD_ITM_VSQ_GROUP_CTGRY_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE + ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE
      + ARAD_ITM_VSQ_GROUP_STTSTCS_TAG_SIZE + ARAD_ITM_VSQ_GROUP_LLFC_SIZE) * vsq_is_ocb_max)) / ARAD_ITM_VSQ_GROUP_PFC_SIZE;
  }
  else
