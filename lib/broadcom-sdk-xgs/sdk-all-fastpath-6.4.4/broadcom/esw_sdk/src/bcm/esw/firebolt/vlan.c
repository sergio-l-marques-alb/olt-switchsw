/*
 * $Id: vlan.c,v 1.263 Broadcom SDK $
...
_bcm_fb2_outer_tpid_init(int unit) {
    ...
    for (index = 0; index < BCM_MAX_TPID_ENTRIES; index++) {
        ...
        OUTER_TPID_ENTRY(unit, index) = soc_reg_field_get(unit, ING_OUTER_TPIDr,
                                                          outer_tpid, TPIDf);
    }
 ...
 */
static int
_bcm_xgs3_vlan_table_init(int unit, bcm_vlan_data_t *vd, int table)
{
    vlan_tab_entry_t    ve;
    bcm_pbmp_t          pbm, temp_pbm;
    int                 rv;
    ...
    if (TABLE_HAS_T_BITMAP(unit, table)) {
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            soc_mem_pbmp_field_set(unit, table, &ve, PORT_BITMAPf,
                               &vd->port_bitmap);
        }
        else {
            soc_mem_pbmp_field_set(unit, table, &ve, PORT_BITMAPf,
    ...
    if (SOC_MEM_FIELD_VALID(unit, table, L3_IIFf)) {
       soc_mem_field32_set(unit, table, &ve, L3_IIFf, vd->vlan_tag);
    }
#endif
#if defined(BCM_EASY_RELOAD_SUPPORT)
    if (SOC_IS_RELOADING(unit)) { 
 ...
 || defined(BCM_RAVEN_SUPPORT)
    ...
    if (SOC_MEM_IS_VALID(unit, VLAN_MPLSm)) {
        ...
        if (soc_feature(unit, soc_feature_mim)) {
              soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, MIM_TERM_ENABLEf, 0x1);
        }
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vlan_filter)) {
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_vlan_vfi_membership_init(unit, vd->vlan_tag,
                                               vd->port_bitmap));
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
...
_bcm_xgs3_vlan_table_create(int unit, bcm_vlan_t vid, int table)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, table, L3_IIFf)) {
        ...
        if (ingress_map_mode) {
            if (use_default) {
                /* Disable 1:1 mapping by Pointing to default entry*/
                soc_mem_field32_set(unit, table, &vt, L3_IIFf, BCM_L3_INGRESS_INTERFACE_INVALID);
            }
        } else {
            soc_mem_field32_set(unit, table, &vt, L3_IIFf, vid);
        }
...
static int
_bcm_xgs3_vlan_table_port_remove(int unit, bcm_vlan_t vid,
                                 pbmp_t pbmp, int table)
{
    vlan_tab_entry_t            vt;
    int                         rv;
    bcm_pbmp_t                  cur_pbmp;
...
bcm_xgs3_vlan_stg_get(int unit, bcm_vlan_t vid, bcm_stg_t *stg_ptr)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_vlan_valid_check(unit, VLAN_TABm, &vt, vid));

    *stg_ptr = soc_mem_field32_get(unit, VLAN_TABm, &vt, STGf);
...
_bcm_xgs3_vlan_table_reload(int unit, vbmp_t *bmp, int *count, int table)
{
    ...
    for (index = soc_mem_index_min(unit, table);
         ...
         if (soc_mem_field32_get(unit, table, vlan_entry, VALIDf)) {
             if (TABLE_HAS_T_BITMAP(unit, table)) {
                 soc_mem_pbmp_field_get(unit, table, vlan_entry,
                                        PORT_BITMAPf, &cur_pbmp);
                 if (BCM_PBMP_IS_NULL(cur_pbmp)) {
                     if ((EGR_VLANm != table) &&
                         !SHR_BITGET(bmp->w, index)) {
...
bcm_xgs3_vlan_init(int unit, bcm_vlan_data_t *vd)
{
    ...
    if (SOC_IS_FBX(unit)) {
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_vlan_table_init(unit, vd, EGR_VLANm));
    }
#else
    ...
    return _bcm_xgs3_vlan_table_init(unit, vd, VLAN_TABm);
}
   ...
   int l3IntfMapMode=0;
    ...
    if (SOC_MEM_IS_VALID(unit, VLAN_MPLSm)) {
        ...
        if (l3IntfMapMode) {
           ...
           if (!SHR_BITGET(vlan_info[unit].pre_cfg_bmp.w,vid)) { 
               /* Disable 1:1 mapping */
               soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, L3_IIFf, 
                                   BCM_L3_INGRESS_INTERFACE_INVALID);
           }
        } else {
            soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, L3_IIFf, vid);
        }
        if (soc_feature(unit, soc_feature_mpls)) {
              soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, MPLS_ENABLEf, 0x1);
        }
    ...
    return _bcm_xgs3_vlan_table_create(unit, vid, VLAN_TABm);
}
...
bcm_xgs3_vlan_destroy(int unit, bcm_vlan_t vid)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_delete_vlan_vfi_membership(unit, vid));
    }
#endif
    ...
    /* Must call on EGR_VLANm before VLAN_TABm on Firebolt 2 */
#ifdef BCM_TH_LATENCY_EXCLUDE
    if (SOC_IS_FBX(unit) && !SOC_IS_KATANA2(unit)) {
#else
                       ...
                       pbmp_t ing_pbmp)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_ing_vlan_vfi_membership_add(unit, vid, pbmp));
        BCM_IF_ERROR_RETURN
            (bcm_td2p_egr_vlan_vfi_membership_add(unit, vid, pbmp));
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
    return BCM_E_NONE;
}
...
int
bcm_xgs3_vlan_port_remove(int unit, bcm_vlan_t vid, pbmp_t pbmp)
{
#ifdef BCM_TH_LATENCY_EXCLUDE
    if (SOC_IS_FBX(unit)) {
#else
...
#endif
        BCM_IF_ERROR_RETURN
            (_bcm_xgs3_vlan_table_port_remove(unit, vid, pbmp, EGR_VLANm));
    }

    BCM_IF_ERROR_RETURN
        (_bcm_xgs3_vlan_table_port_remove(unit, vid, pbmp, VLAN_TABm));

#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_ing_vlan_vfi_membership_delete(unit, vid, pbmp));
        BCM_IF_ERROR_RETURN
            (bcm_td2p_egr_vlan_vfi_membership_delete(unit, vid, pbmp));
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
    return BCM_E_NONE;
}
                       ...
                       pbmp_t *ing_pbmp)
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        BCM_IF_ERROR_RETURN
            (bcm_td2p_vlan_table_port_get(unit, vid, &local_ing_pbmp, &local_pbmp));
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
...
bcm_xgs3_vlan_stg_set(int unit, bcm_vlan_t vid, bcm_stg_t stg)
{
    ...
    return _bcm_xgs3_vlan_table_stg_set(unit, vid, stg, VLAN_TABm);
}
 ...
 || defined(BCM_RAVEN_SUPPORT)
    ...
    if (_BCM_VPN_VFI_IS_SET (vid)) {
        if (soc_feature(unit, soc_feature_vfi_profile)) {
            return bcmi_td2p_vlan_control_vpn_get(unit, vid,
                                                        valid_fields, control);
        } else {
            return (BCM_E_PARAM);
        }
    }
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_VRF_MASK) {
            if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VRF_IDf)) {              
               control->vrf = soc_mem_field32_get(unit, VLAN_TABm, &vt, VRF_IDf);
                        ...
                        && (!ingress_map_mode) && !SOC_IS_HURRICANEX(unit)
        ...
        if (soc_feature(unit, soc_feature_mpls) &&
            SOC_MEM_IS_VALID(unit, VLAN_MPLSm)) {
            ...
            if (0 == soc_mem_field32_get(unit, VLAN_MPLSm,
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_BROADCAST_GROUP_MASK) {
            if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, BC_IDXf)) {
                if (soc_mem_field32_get(unit, VLAN_TABm, &vt, BC_IDXf)) {
                    rv = _bcm_tr_multicast_ipmc_group_type_get(unit,
                        soc_mem_field32_get(unit, VLAN_TABm, &vt, BC_IDXf),
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_UNKNOWN_MULTICAST_GROUP_MASK) {
            if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, UMC_IDXf)) {
                if (soc_mem_field32_get(unit, VLAN_TABm, &vt, UMC_IDXf)) {
                    rv = _bcm_tr_multicast_ipmc_group_type_get(unit,
                        soc_mem_field32_get(unit, VLAN_TABm, &vt, UMC_IDXf),
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_UNKNOWN_UNICAST_GROUP_MASK) {
            if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, UUC_IDXf)) {
                if (soc_mem_field32_get(unit, VLAN_TABm, &vt, UUC_IDXf)) {
                    rv = _bcm_tr_multicast_ipmc_group_type_get(unit,
                        soc_mem_field32_get(unit, VLAN_TABm, &vt, UUC_IDXf),
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_NAT_REALM_ID_MASK) {
            if (soc_feature(unit, soc_feature_nat) && 
                ...
                rv = _bcm_tr_l3_intf_nat_realm_id_get(unit,
                                    ingress_map_mode ? control->ingress_if : vid,
                          ...
                          uint32 valid_fields, bcm_vlan_control_vlan_t *control)
{
    ...
    if (0 == soc_feature(unit, soc_feature_vlan_ctrl)) {
        return (BCM_E_UNAVAIL);
    }
    ...
    if (_BCM_VPN_VFI_IS_SET (vid)) {
        if (soc_feature(unit, soc_feature_vfi_profile)) {
            return bcmi_td2p_vlan_control_vpn_set(unit, vid,
                                                        valid_fields, control);
        } else {
            return (BCM_E_PARAM);
        }
    ...
    if (valid_fields & BCM_VLAN_CONTROL_VLAN_NAT_REALM_ID_MASK) {
        ...
        if (soc_feature(unit, soc_feature_nat) && (SOC_MEM_IS_VALID(unit, L3_IIFm)) 
                        && (SOC_MEM_FIELD_VALID(unit, L3_IIFm, SRC_REALM_IDf))) {
            /* validate realm id param */
            if ((control->nat_realm_id < 0) || (control->nat_realm_id > 3)) {
                return BCM_E_PARAM;
            }
            BCM_IF_ERROR_RETURN(_bcm_tr_l3_intf_nat_realm_id_set(unit,
                                    ingress_map_mode ? control->ingress_if : vid,
                                    control->nat_realm_id));
        }
    }
    ...
    if (valid_fields & BCM_VLAN_CONTROL_VLAN_BROADCAST_GROUP_MASK) {
        if (soc_mem_field_valid(unit, VLAN_TABm, BC_IDXf)) {
            if (0 == control->broadcast_group) {
                soc_mem_field32_set(unit, VLAN_TABm, &vt, BC_IDXf, 0);
            }
        } 
    }
    if (valid_fields & BCM_VLAN_CONTROL_VLAN_UNKNOWN_MULTICAST_GROUP_MASK) {
        if (soc_mem_field_valid(unit, VLAN_TABm, UMC_IDXf)) {
            if (0 == control->unknown_multicast_group) {
                soc_mem_field32_set(unit, VLAN_TABm, &vt, UMC_IDXf, 0);
            }
        }
    }
    if (valid_fields & BCM_VLAN_CONTROL_VLAN_UNKNOWN_UNICAST_GROUP_MASK) {
        if (soc_mem_field_valid(unit, VLAN_TABm, UUC_IDXf)) {
            if (0 == control->unknown_unicast_group) {
                soc_mem_field32_set(unit, VLAN_TABm, &vt, UUC_IDXf, 0);
            }
        }
    }
    ...
    if (soc_feature(unit, soc_feature_trill)) {
        ...
        if (valid_fields & BCM_VLAN_CONTROL_VLAN_TRILL_NONUNICAST_GROUP_MASK) {
            ...
            if ( rv == BCM_E_NONE ) {
                if (BCM_SUCCESS(bcm_td_trill_multicast_check(unit, control->trill_nonunicast_group))) {
    ...
    if (valid_fields & BCM_VLAN_CONTROL_VLAN_VP_MCAST_MASK) {
        if (control->vp_mc_ctrl == bcmVlanVPMcControlEnable) {
            if (SOC_MEM_FIELD_VALID(unit, VLAN_TABm, VIRTUAL_PORT_ENf)) {
                ...
                for (i = 0; i < 3; i++) {
                    mc_idx = soc_mem_field32_get(unit, VLAN_TABm, &vt, 
                                                group_type[i]);
    ...
    if (soc_feature(unit, soc_feature_vfi_from_vlan_tables) && 
        ...
        if (_BCM_VPN_VFI_IS_SET (control->vpn)) {
                ...
                soc_mem_field32_set(unit, VLAN_MPLSm, &vlan_mpls, 
                                    VFIf, vfi_index);   
                rv = WRITE_VLAN_MPLSm(unit, MEM_BLOCK_ALL, (int)vid, &vlan_mpls);              
                soc_mem_unlock(unit, VLAN_MPLSm);
                if (BCM_FAILURE(rv)) {
                    soc_mem_unlock(unit, VLAN_TABm);
                    return rv; 
...
void _bcm_fb2_vlan_profile_sw_dump(int unit)
{
    ...
    for (index = 0; index <= index_max; index++) {
        ...
        LOG_CLI((BSL_META_U(unit,
                            " TPID       = 0x%x\n"), 
                 VLAN_PROFILE_ENTRY(unit, index).outer_tpid)); 
        LOG_CLI((BSL_META_U(unit,
                            " Flags      = 0x%x\n"), 
                 VLAN_PROFILE_ENTRY(unit, index).flags)); 
        LOG_CLI((BSL_META_U(unit,
                            " IP6 PFM    = 0x%x\n"), 
                 VLAN_PROFILE_ENTRY(unit, index).ip6_mcast_flood_mode));
        LOG_CLI((BSL_META_U(unit,
                            " IP4 PFM    = 0x%x\n"), 
                 VLAN_PROFILE_ENTRY(unit, index).ip4_mcast_flood_mode));
        LOG_CLI((BSL_META_U(unit,
                            " L2 PFM     = 0x%x\n"), 
                 VLAN_PROFILE_ENTRY(unit, index).l2_mcast_flood_mode));
        LOG_CLI((BSL_META_U(unit,
                            " Ref count  = %d\n\n"), VLAN_PROFILE_REF_COUNT (unit, index)));
    }
