...
extern int bcm_port_config_get(
     ...
     * Force link up
     */
    _bcm_esw_link_force(unit, 0 /*flags*/, mii_port, TRUE, TRUE); 
...
int bcm_esw_ces_crm_dma_msg_send_receive(int unit, uint8 t_subclass, uint8 *data, 
	...
	LOG_CLI((BSL_META_U(unit,
                            "Received message type error or missmatch, "
                            "mclass:%d  subclass:%d\n"),
                 rx.s.mclass, rx.s.subclass));
	ret = BCM_E_INTERNAL;
    }
...
#if 0
	LOG_CLI((BSL_META_U(unit,
                            "4 Service:%2d Encap:%d Label:%04lx Enable:%lu ret:0x%08x\n"), 
                 ndMap.n_channel_id,
...
#if 0      
            LOG_CLI((BSL_META_U(unit,
                                "rclock config flags=%x enbale=%d  "
                                "t1-e1=%x struct=%x brgout=%x rclock=%x port=%x "
                                "recovery=%x service=%x \n"),
                     rclock_msg.flags,
                     rclock_msg.b_enable,
...
#if 0
    LOG_CLI((BSL_META_U(unit,
                        "rclock record config: enbale=%d   "
                        "outputbrg=%x rclock=%x port=%x recovery=%x service=%x \n"),
...
#if 0
	LOG_CLI((BSL_META_U(unit,
                            "\nRcr=%d: state %d-%s , seconds: active %d , "
                            "locked %d  ,  freq: %d.%d\n\n"), 
