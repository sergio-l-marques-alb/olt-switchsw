...
#ifdef BCM_TRIUMPH3_SUPPORT
extern int _bcm_hx4_ibod_syn_port_down (int unit, int port);
#endif
...
_bcm_esw_link_fault_get(int unit, int port, int *fault)
{
    ...
    if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port) ||
        ...
        if (SOC_USE_PORTCTRL(unit)) {
            bcmi_esw_portctrl_clear_rx_lss_status_get(unit, port, &lcl_fault,
                                                      &rmt_fault);
            ...
            if (cur_link != new_link) {
                if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port)) {
                    if (!(SOC_INFO(unit).port_speed_max[port] >= 100000)) {
                        if (!new_link) {
                            _bcm_hx4_ibod_syn_port_down (unit, port);
                        }
                    }
                }
            }
