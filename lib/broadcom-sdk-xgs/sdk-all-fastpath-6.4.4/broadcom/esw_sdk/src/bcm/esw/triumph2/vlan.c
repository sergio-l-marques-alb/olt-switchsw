...
_bcm_td2_vlan_match_vp_replace(int unit, int vp, int new_vp, int *old_vp)
{
    ...
    *old_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent, SOURCE_VPf);
    ...
    /* Insert new entry */
    rv = soc_mem_insert(unit, VLAN_XLATEm, MEM_BLOCK_ALL, &old_vent);
    if (rv == SOC_E_EXISTS) {
        rv = BCM_E_NONE;
    } else {
        return BCM_E_INTERNAL;
        ...
        int *vp_lag_vp)
{
    ...
    /* Lookup existing entry */
    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ALL,
                        &entry_index, &vent, &old_vent, 0);
    if (SOC_FAILURE(rv)) {
        VLAN_VIRTUAL_UNLOCK(unit);
        return rv;
    ...
    *vp_lag_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent, SOURCE_VPf);
 ...
 */
STATIC int
_tr2_vlan_vp_match_delete(int unit, int vp, int *old_vp)
{
    vlan_xlate_entry_t vent, old_vent;
    int key_type = 0;
    uint32 profile_idx;
    ...
    if ((rv == SOC_E_NONE) && soc_VLAN_XLATEm_field32_get(unit, &old_vent, VALIDf)) {
        if (old_vp != NULL) {
            *old_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent,
                                                  SOURCE_VPf);
        }
        profile_idx = soc_VLAN_XLATEm_field32_get(unit, &old_vent,
                                                  TAG_ACTION_PROFILE_PTRf);       
...
STATIC int
_bcm_tr2_vlan_vp_match_delete(int unit, int vp, int *old_vp)
{
#if defined(BCM_TRIUMPH3_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit)) {
...
#endif
    {
        return _tr2_vlan_vp_match_delete(unit, vp, old_vp);
    }
}
                         ...
                         bcm_vlan_port_t *vlan_vp)
{
    ...
    if (!(vlan_vp->flags & BCM_VLAN_PORT_REPLACE)) { /* Create new VLAN VP */
         ...
         */
        BCM_IF_ERROR_RETURN
            (_bcm_tr2_vlan_vp_match_delete(unit, vp, &old_vp));
#ifdef BCM_TRIDENT2_SUPPORT
    ...
    vlan_vp->encap_id = nh_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
...
cleanup:
    if (!(vlan_vp->flags & BCM_VLAN_PORT_REPLACE)) {
        ...
        (void) _bcm_tr2_vlan_vp_match_delete(unit, vp, NULL);
    }
...
_bcm_tr2_vlan_vp_destroy(int unit, bcm_gport_t gport)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_tr2_vlan_vp_match_delete(unit, vp, NULL));
 ...
 *      addflag  - (IN) indicate if this function is called in vlan port add/delete
 *                      operation. Note: The logic for delete is not realized yet. 
 * Returns:
 *      BCM_E_XXX
 */
int
bcm_tr2_update_vlan_pbmp(int unit, bcm_vlan_t vlan,
        bcm_pbmp_t *pbmp, int addflag)
{
    int rv = BCM_E_NONE;
    vlan_tab_entry_t vtab;
    egr_vlan_entry_t egr_vtab;
    bcm_pbmp_t l2_l3_pbmp;
    ...
    /*when addflag=1,the original pbmp for vlan check should be read out and ORed with the new pbmp*/
    if (addflag) {
        if (soc_mem_field_valid(unit, VLAN_TABm, ING_PORT_BITMAPf)) {
            soc_mem_pbmp_field_get(unit, VLAN_TABm, &vtab, ING_PORT_BITMAPf, &l2_l3_pbmp);
        } else {
            soc_mem_pbmp_field_get(unit, VLAN_TABm, &vtab, PORT_BITMAPf, &l2_l3_pbmp);
        }
        BCM_PBMP_OR(*pbmp, l2_l3_pbmp);
    }
    ...
    if (soc_mem_field_valid(unit, EGR_VLANm, PORT_BITMAPf) != 0) {
                           ...
                           int flags)
{
    ...
    int ing_vp_group_added = FALSE, eg_vp_group_added = FALSE, vlan_vfi = vlan, is_vfi;
    _bcm_vp_vlan_mbrship_key_type_e key_type;

    /* Init is_vfi flag to zero */
    is_vfi = 0;
    ...
    /* Set the key_type tp vp_vlan_type by default */
    ...
    if (BCM_GPORT_IS_VXLAN_PORT(gport)) {
        ...
        vxlan_port.vxlan_port_id = gport;
        BCM_IF_ERROR_RETURN(bcm_td2_vxlan_port_get(unit, vlan, &vxlan_port));
        if (vxlan_port.flags & BCM_VXLAN_PORT_MATCH_NONE) {
            phy_port_trunk = BCM_GPORT_INVALID;
        } else {
            phy_port_trunk = vxlan_port.match_port;
        }

        /* Set the key_type as vp_vfi type */
        key_type = _bcm_vp_vlan_mbrship_vp_vfi_type;

        /* Get the associated VFI value */
        _BCM_VPN_GET(vlan_vfi, _BCM_VPN_TYPE_VFI, vlan);

        /* Mark that we are dealing a VFI */
        is_vfi = 1;
    }
    ...
    /* Do these checks for non-subports */
    if(!is_glp_port) {
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            BCM_IF_ERROR_RETURN(bcm_td2p_update_vlan_pbmp(unit, vlan,
                    &l2_l3_pbmp)); 
        } else
#endif
        {
            BCM_IF_ERROR_RETURN(bcm_tr2_update_vlan_pbmp(unit, vlan, &l2_l3_pbmp, _BCM_VLAN_PORT_ADD));
        }
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
            if (!bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
                ...
                if ((ing_filter_flags & BCM_PORT_VLAN_MEMBER_INGRESS) &&
                    ...
                    /*BCM_IF_ERROR_RETURN
                        (bcm_td2p_ing_vp_group_move(unit, vp, vlan, TRUE)); */
                    BCM_IF_ERROR_RETURN(
            ...
            if (!bcm_td2p_egr_vp_group_unmanaged_get(unit)) {
                ...
                if ((eg_filter_flags & BCM_PORT_VLAN_MEMBER_EGRESS) &&
                        !(eg_filter_flags &
                            BCM_PORT_VLAN_MEMBER_VP_VLAN_MEMBERSHIP)) {
                    /*BCM_IF_ERROR_RETURN
                        (bcm_td2p_eg_vp_group_move(unit, vp, vlan, TRUE)); */
                    BCM_IF_ERROR_RETURN(
                        bcm_td2p_vp_group_port_move(unit, gport, vlan, 
                                    BCMI_VP_GROUP_ADD, BCMI_VP_GROUP_EGRESS, flags));
...
#endif
        {
            BCM_IF_ERROR_RETURN(bcm_tr2_update_vlan_pbmp(unit, vlan, &l2_l3_pbmp, _BCM_VLAN_PORT_DELETE));
        }
        ...
        if (!bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
            ...
            if ((ing_filter_flags & BCM_PORT_VLAN_MEMBER_INGRESS) &&
                    !(ing_filter_flags &
                        BCM_PORT_VLAN_MEMBER_VP_VLAN_MEMBERSHIP)) { 
                /* BCM_IF_ERROR_RETURN
                    (bcm_td2p_ing_vp_group_move(unit, vp, vlan, FALSE)); */
                    BCM_IF_ERROR_RETURN(
                        bcm_td2p_vp_group_port_move(unit, gport, vlan, 
        ...
        if (!bcm_td2p_egr_vp_group_unmanaged_get(unit)) {
            ...
            if ((eg_filter_flags & BCM_PORT_VLAN_MEMBER_EGRESS) &&
                    !(eg_filter_flags &
                        BCM_PORT_VLAN_MEMBER_VP_VLAN_MEMBERSHIP)) {
                /* BCM_IF_ERROR_RETURN
                    (bcm_td2p_eg_vp_group_move(unit, vp, vlan, FALSE)); */
                BCM_IF_ERROR_RETURN(
                    bcm_td2p_vp_group_port_move(unit, gport, vlan, 
                                                BCMI_VP_GROUP_DELETE, 
...
bcm_tr2_vlan_gport_delete_all(int unit, bcm_vlan_t vlan) 
{
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        if (!bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
            BCM_IF_ERROR_RETURN(bcm_td2p_ing_vp_group_vlan_delete_all(unit,
                        vlan));
        }
        if (!bcm_td2p_egr_vp_group_unmanaged_get(unit)) {
            BCM_IF_ERROR_RETURN(bcm_td2p_eg_vp_group_vlan_delete_all(unit,
                        vlan));
        }
    } else
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
