...
bcm_th_ipmc_repl_init(int unit)
{
     ...
     */
    _th_repl_info[unit]->num_pipes = 4;
...
_bcm_th_port_pipe_get(int unit, int port, int *pipe)
{
    ...
    /* Convert logical port to physical port */
    si = &SOC_INFO(unit);
    phy_port = si->port_l2p_mapping[port];
    
    /* Get pipeline number */
    if ((phy_port >= 0 && phy_port <= 32) ||
        (phy_port == 132)) {
        *pipe = 0;
                ...
                int intf_count)
{
    ...
    if (slice == 0) {
        if (phy_port == 133) {
        ...
        } else if (phy_port <= 64 && phy_port >= 33) {
            ...
            count_shift = (phy_port - 33) * 2 % 32;
            /* If count width is 3 bits, and the physical port is 1 less than
             * a multiple of 4, i.e. 35, 39, 43 ... 63, the number of shift
             * positions is decreased by 1.
             */
            if (count_width == 3) {
                if ((phy_port + 1) % 4 == 0) {
                    count_shift--;
            ...
            copy_count_buf[(phy_port - 33) * 2 / 32] &= ~(count_mask << count_shift);
            copy_count_buf[(phy_port - 33) * 2 / 32] |= (copy_count << count_shift);
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_set(unit,
                    &initial_copy_count_entry, ICC_BIT_FP_PYf, copy_count_buf);
        } else if (phy_port <= 32 && phy_port >= 1) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_get(unit,
                    &initial_copy_count_entry, ICC_BIT_FP_PXf, copy_count_buf);
            count_mask = (1 << count_width) - 1;
            count_shift = (phy_port - 1) * 2 % 32;
            /* If count width is 3 bits, and the physical port is 1 less than
             * a multiple of 4, i.e. 3, 7, 11 ... 31, the number of shift
             * positions is decreased by 1.
             */
            if (count_width == 3) {
                if ((phy_port + 1) % 4 == 0) {
        ...
        if (phy_port == 135) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_LP_PYf, copy_count);
        } else if (phy_port == 134) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_LP_PXf, copy_count);
        } else if (phy_port == 131) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field32_set(unit,
                    &initial_copy_count_entry, ICC_BIT_MPf, copy_count);
        } else if (phy_port <= 128 && phy_port >= 97) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_get(unit,
                    &initial_copy_count_entry, ICC_BIT_FP_PYf, copy_count_buf);
            count_mask = (1 << count_width) - 1;
            count_shift = (phy_port - 97) * 2 % 32;
            /* If count width is 3 bits, and the physical port is 1 less than
             * a multiple of 4, i.e. 99, 103, 107 ... 127, the number of shift
             * positions is decreased by 1.
             */
            if (count_width == 3) {
                if ((phy_port + 1) % 4 == 0) {
            ...
            copy_count_buf[(phy_port - 97) * 2 / 32] &= ~(count_mask << count_shift);
            copy_count_buf[(phy_port - 97) * 2 / 32] |= (copy_count << count_shift);
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_set(unit,
                    &initial_copy_count_entry, ICC_BIT_FP_PYf, copy_count_buf);
        } else if (phy_port <= 96 && phy_port >= 65) {
            soc_MMU_REPL_GROUP_INITIAL_COPY_COUNTm_field_get(unit,
                    &initial_copy_count_entry, ICC_BIT_FP_PXf, copy_count_buf);
            count_mask = (1 << count_width) - 1;
            count_shift = (phy_port - 65) * 2 % 32;
            /* If count width is 3 bits, and the physical port is 1 less than
             * a multiple of 4, i.e. 67, 71, 75 ... 95, the number of shift
             * positions is decreased by 1.
             */
            if (count_width == 3) {
                if ((phy_port + 1) % 4 == 0) {
