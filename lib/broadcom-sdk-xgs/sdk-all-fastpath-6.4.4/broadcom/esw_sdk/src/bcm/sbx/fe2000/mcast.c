/*
 * $Id: mcast.c,v 1.50 Broadcom SDK $
                           ...
                           uint32 *new_mcGroup)
{
    ...
    if (BCM_PBMP_NOT_NULL(pbmp)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_mcast_addr_add: pbmp contains ports not"
                               " in pbmp_all\n")));
        return BCM_E_PARAM;
    }
    ...
    if (BCM_PBMP_NOT_NULL(pbmp)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_mcast_addr_add: ubmp contains ports not"
                               " in the pbmp\n")));
        return BCM_E_PARAM;
    }
    ...
    if (!BCM_PBMP_EQ(pbmp, mcAddr->ubmp)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_mcast_addr_add: ports are not tagged in the"
    ...
    if (BCM_E_NONE == result) {
        ...
        LOG_VERBOSE(BSL_LS_BCM_MCAST,
                    (BSL_META_U(unit,
                                "_bcm_fe2000_mcast_addr_add: updating existing"
                                 " VID+DMACa\n")));
        ...
        LOG_VERBOSE(BSL_LS_BCM_MCAST,
                    (BSL_META_U(unit,
                                "_bcm_fe2000_mcast_addr_add: adding new VID+DMACa\n")));
        resflags |= _alloc_dm;
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_MCAST,
                      (BSL_META_U(unit,
                                  "_bcm_fe2000_set_mcPort2Etc(*,%d,%d)"
                                   " returned %d (%s)\n"),
                       unit,
                       groupIndex,
                       result,
                              ...
                              uint32 spec_flags)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_MCAST,
                (BSL_META_U(unit,
                            "_bcm_fe2000_mcast_addr_remove(%d,"
    ...
    if ((BCM_VLAN_MIN >= vid) || (vsi_max <= vid)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_mcast_addr_remove: %03X is not a valid VID\n"),
                   vid));
        return BCM_E_PARAM;
    }
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE != result) {
            LOG_ERROR(BSL_LS_BCM_MCAST,
                      (BSL_META_U(unit,
                                  "_bcm_fe2000_mcast_addr_remove: get %d:ft[%08X] for"
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_mcast_addr_remove: get "
    ...
    if (BCM_E_NONE == result) {
        ...
        if (BCM_E_NONE == result) {
            ...
            LOG_ERROR(BSL_LS_BCM_MCAST,
                      (BSL_META_U(unit,
                                  "_bcm_fe2000_mcast_addr_remove: delete "
                                   " " L2_6B_MAC_FMT " resulted in"
                                   " %d (%s)\n"),
                       L2_6B_MAC_PFMT(mac), result,
                       _SHR_ERRMSG(result)));
        ...
        if (BCM_E_NONE == result) {
            ...
            if (BCM_E_NONE != result) {
                LOG_ERROR(BSL_LS_BCM_MCAST,
                          (BSL_META_U(unit,
                                      "_bcm_fe2000_mcast_addr_remove: set %d:ft[0x%08X]"
                          ...
                          bcm_mcast_addr_t *mcAddr)
{
    ...
    if ((BCM_VLAN_MIN >= mcAddr->vid) || (vsi_max <= mcAddr->vid)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_port_add: %03X is not a valid VID\n"),
                   mcAddr->vid));
        return BCM_E_PARAM;
    }
                             ...
                             bcm_mcast_addr_t *mcAddr)
{
    ...
    if ((BCM_VLAN_MIN >= mcAddr->vid) || (vsi_max <= mcAddr->vid)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_port_remove: %03X is not a valid VID\n"),
                   mcAddr->vid));
        return BCM_E_PARAM;
    }
                          ...
                          bcm_mcast_addr_t *mcAddr)
{
    ...
    if ((BCM_VLAN_MIN >= vid) || (vsi_max <= vid)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_port_get: %03X is not a valid VID\n"),
                   vid));
        return BCM_E_PARAM;
    }
    ...
    if (BCM_E_NONE != result) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "_bcm_fe2000_vlan_port_fetch(*,%d,%03X,*,*)"
                               " returned %d (%s)\n"),
                   unit,
                   vid,
                   result,
                       ...
                       bcm_port_t port)
{
    ...
    if ((BCM_VLAN_MIN >= vlan) || (vsi_max <= vlan)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_leave: %03X is not a valid VID\n"),
                   vlan));
        return BCM_E_PARAM;
    }
                      ...
                      bcm_pbmp_t *all_router_pbmp)
{
    ...
    if ((BCM_VLAN_MIN >= vlan) || (vsi_max <= vlan)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_join: %03X is not a valid VID\n"),
                   vlan));
        return BCM_E_PARAM;
    }
    ...
    if (!BCM_PBMP_EQ(pbmp, mcAddr->pbmp)) {
        ...
        LOG_ERROR(BSL_LS_BCM_MCAST,
                  (BSL_META_U(unit,
                              "bcm_fe2000_mcast_join: port is not in the vlan\n")));
        return BCM_E_PARAM;
    }
    ...
    if ((BCM_E_NONE == result) || (BCM_E_EXISTS == result)) {
        LOG_VERBOSE(BSL_LS_BCM_MCAST,
                    (BSL_META_U(unit,
                                "bcm_fe2000_mcast_join: group exists; adding port\n")));
        ...
        LOG_VERBOSE(BSL_LS_BCM_MCAST,
                    (BSL_META_U(unit,
                                "bcm_fe2000_mcast_join: group does not exist; adding group\n")));
