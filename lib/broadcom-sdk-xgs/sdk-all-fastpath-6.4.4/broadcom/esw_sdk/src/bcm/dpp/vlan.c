...
#define SHR_BITGET_VLAN_BMP(_vlan)\
    SHR_BITGET(sw_state[unit]->dpp.bcm.vlan->vlan_info.vlan_bmp, _vlan)

#define SHR_BITSET_VLAN_BMP(_vlan)\
    SHR_BITSET(sw_state[unit]->dpp.bcm.vlan->vlan_info.vlan_bmp, _vlan)

#define SHR_BITCLR_VLAN_BMP(_vlan)\
    SHR_BITCLR(sw_state[unit]->dpp.bcm.vlan->vlan_info.vlan_bmp, _vlan)
                                  ...
                                  int *exists)
{
    ...
    *exists = (0 != SHR_BITGET_VLAN_BMP(vlan));
                                  ...
                                  int exists)
{
    ...
    already = SHR_BITGET_VLAN_BMP(vlan);
    if (exists) {
        if (!already) {
            /* did not exist before */
            BCMDNX_IF_ERR_EXIT(VLAN_ACCESS.vlan_info.count.get(unit, &count));
            count++;
            BCMDNX_IF_ERR_EXIT(VLAN_ACCESS.vlan_info.count.set(unit, count));
        }
        SHR_BITSET_VLAN_BMP(vlan);
    } else {
        if (already) {
            /* did exist before */
            BCMDNX_IF_ERR_EXIT(VLAN_ACCESS.vlan_info.count.get(unit, &count));
    ...
    int unit)
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop != SOC_PPC_AC_ID_INVALID) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* First, allocate the drop global lif as set by soc property */
            rv = bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS | BCM_DPP_AM_FLAG_ALLOC_WITH_ID, 
                                              (int*)&SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            inlif_info.glif = SOC_DPP_CONFIG(unit)->pp.global_lif_index_drop; /* pre-alloced Global LIF ID*/ 
            rv = _bcm_dpp_am_local_inlif_alloc(unit, 0, &inlif_info);
            BCMDNX_IF_ERR_EXIT(rv);

            /*call with local lif allocated*/
        ...
        SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop = lif_drop_local_lif;

    } else { /* Drop global lif is invalid */
        SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop = SOC_PPC_AC_ID_INVALID;
    }
    ...
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != SOC_PPC_AC_ID_INVALID) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* First, allocate the drop global lif as set by soc property */
            rv = bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS | BCM_DPP_AM_FLAG_ALLOC_WITH_ID, 
                                              (int*)&(SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple));
            BCMDNX_IF_ERR_EXIT(rv);

            /* local lif allocation*/
            inlif_info.app_type = bcm_dpp_am_ingress_lif_app_ingress_ac;
            inlif_info.local_lif_flags = BCM_DPP_AM_IN_LIF_FLAG_COMMON;
            inlif_info.glif = SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple; /* pre-alloced Global LIF ID*/ 
            rv = _bcm_dpp_am_local_inlif_alloc(unit, 0, &inlif_info);
            BCMDNX_IF_ERR_EXIT(rv);
        ...
        /* Update global config with the local lif */
        SOC_DPP_CONFIG(unit)->pp.local_lif_index_simple = lif_simple_local_lif;

    } else { /* Simple local lif is invalid */
        SOC_DPP_CONFIG(unit)->pp.local_lif_index_simple = SOC_PPC_AC_ID_INVALID;
    }


    /* By default, set all ports to default LIF simple , otherwise to drop */
    if (SOC_DPP_CONFIG(unit)->pp.global_lif_index_simple != SOC_PPC_AC_ID_INVALID) {
      default_lif_id = SOC_DPP_CONFIG(unit)->pp.local_lif_index_simple; /* In case simple is set by default all ports mapped to simple */          
    } else {
      default_lif_id = SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop;      
    }
...
_bcm_petra_vlan_deinit(int unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(VLAN_ACCESS.free(unit));
    BCM_EXIT;
exit:
    BCMDNX_FUNC_RETURN;
}
    ...
    int unit)
{
    ...
    for (idx = BCM_VLAN_MIN; idx < BCM_VLAN_COUNT; idx++) {
        if ((!SHR_BITGET_VLAN_BMP(idx)) || (idx == defl)) {
            continue;
        }
            ...
            if (port_info.dflt_egress_ac == 0) {
                if (!is_alloc_ac) {
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        rv = bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_EGRESS, &global_out_ac);
                        BCMDNX_IF_ERR_EXIT(rv);

                        out_lif_info.glif = global_out_ac;
                        ...
                        rv = _bcm_dpp_am_local_outlif_alloc(unit, 0, &out_lif_info);
                        BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_L2_LIF_AC_KEY *in_ac_key)
{
  ...
  if (gport_type == _BCM_DPP_GPORT_TYPE_SIMPLE) {
    ...
    if (BCM_GPORT_IS_VLAN_PORT(port)) {
        ...
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit, port, &global_lif_id, &lif_id, NULL, &fec_id, &is_local);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    SOC_PPD_AC_ID *out_ac)
{
    ...
    SOC_PPD_EG_AC_VBP_KEY vbp_key;
    SOC_PPD_EG_AC_CEP_PORT_KEY cep_key;
    int fec_id, global_lif_id = 0, lif_id, is_local, core;
    SOC_SAND_SUCCESS_FAILURE soc_sand_success;
    ...
    if (gport_type == _BCM_DPP_GPORT_TYPE_SIMPLE) {
      ...
      if (!is_cvid) {
        ...
        if (BCM_GPORT_IS_VLAN_PORT(port)) {
          ...
          rv = _bcm_dpp_gport_to_global_and_local_lif(unit, port, &global_lif_id, NULL, &lif_id, &fec_id, &is_local);
          BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (found == FALSE && is_allocate_in_case_of_not_found) {
      /* Get a fresh Out-AC */
        if (SOC_IS_JERICHO(unit)) {
            ...
            out_lif_info.glif = global_lif_id;
            out_lif_info.app_alloc_info.pool_id = dpp_am_res_eg_out_ac;
            out_lif_info.app_alloc_info.application_type = bcm_dpp_am_egress_encap_app_out_ac;
    ...
    bcm_vlan_action_set_t *action)
{
  ...
  if (in_ac_key.key_type == SOC_PPD_L2_LIF_AC_MAP_KEY_TYPE_NONE) {
      rv = _bcm_dpp_gport_to_global_and_local_lif(unit,port, &global_lif, &local_lif, NULL, NULL, &is_local);
      BCMDNX_IF_ERR_EXIT(rv);
      lif_index = local_lif;
  }
  ...
  if (found == FALSE) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            /* First, allocate the drop global lif as set by soc property */
            rv = bcm_dpp_am_global_lif_alloc(unit, BCM_DPP_AM_FLAG_ALLOC_INGRESS, &global_lif);
            BCMDNX_IF_ERR_EXIT(rv);

            /* local lif allocation*/
            inlif_info.app_type = bcm_dpp_am_ingress_lif_app_ingress_ac;
            inlif_info.local_lif_flags = BCM_DPP_AM_IN_LIF_FLAG_COMMON;
            inlif_info.glif = global_lif; 
            rv = _bcm_dpp_am_local_inlif_alloc(unit, 0, &inlif_info);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_vlan_t inner_vlan)
{
  ...
  unsigned int allocSize = 0;
  int          is_local, global_lif, local_lif;
  SOC_PPD_PORT                     soc_ppd_port;
  int                              core=SOC_CORE_INVALID;
  _bcm_dpp_gport_info_t 		   gport_info;
    ...
    int *arg)
{
    ...
    {                
        ...
        if ( (lif_id != SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop) &&
             (lif_id != default_lif)) {
        ...
        *arg = (lif_id == SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop) ? 1:0;        
    }
    break;
    ...
    int arg)
{
    ...
    PBMP_ITER(pbmp_local_ports,port) {
        ...
        {                
            ...
            lif_id = (arg) ? (SOC_DPP_CONFIG(unit)->pp.local_lif_index_drop):(default_lif);
    ...
    bcm_vlan_port_t *vlan_port)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(vlan_port->vlan_port_id) ) {
        ...
        rv = _bcm_dpp_gport_to_global_and_local_lif(unit, vlan_port->vlan_port_id, &global_lif_id, &in_lif_id, &out_lif_id,
                                   &fec_id, &is_local);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        if(ac_info.learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
            ...
            BCMDNX_IF_ERR_EXIT(l2_interface_calculate_port_id(unit, act_fec, global_lif_id, vlan_port, BCM_GPORT_VLAN_PORT));
        }
    ...
    bcm_vlan_port_translation_t *vlan_port_translation)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, vlan_port_translation->gport, &global_lif_id, &local_in_lif_id, &local_out_lif_id, NULL, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_vlan_port_translation_t *vlan_port_translation)
{
    ...
    SOC_PPD_LIF_ENTRY_INFO *lif_ing_info = NULL;
    SOC_PPD_EG_AC_INFO *lif_eg_info = NULL;
    int global_lif_id, local_in_lif_id, local_out_lif_id;
    _bcm_lif_type_e lif_usage_type, in_lif_usage_type, out_lif_usage_type;
    BCMDNX_INIT_FUNC_DEFS;
