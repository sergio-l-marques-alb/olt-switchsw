    ...
    bcm_gport_t    port)
{
    ...
    int global_lif_id, local_in_lif_id, local_out_lif_id, gport_type;
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
	unsigned int soc_sand_dev_id;
    ...
    rv = _bcm_dpp_gport_resolve(unit,port,0, &dest_id,&global_lif_id,&gport_type);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, port, &global_lif_id, &local_in_lif_id, &local_out_lif_id, NULL, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    soc_sand_rv = soc_ppd_lif_table_entry_get(soc_sand_dev_id,local_in_lif_id,lif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        ...
        if (!is_ingress_only) {
            if(vlan_port.flags & BCM_VLAN_PORT_NETWORK) {
                ...
                if (split_horizon_filter_enable) {
                    if (SOC_IS_PETRAB(unit)) {
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(soc_sand_dev_id,local_out_lif_id,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else {
                        /* ARAD */
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(soc_sand_dev_id,local_out_lif_id,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_HUB);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
                }
             }
                ...
                if(split_horizon_filter_enable){
                    /* if egress hairpin is enabled set egress state as well*/
                    if (SOC_IS_PETRAB(unit)) {
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_ac_orientation_set(soc_sand_dev_id,local_out_lif_id,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    } else {
                        /* ARAD */
                        soc_sand_rv = soc_ppd_eg_filter_split_horizon_out_lif_orientation_set(soc_sand_dev_id,local_out_lif_id,SOC_SAND_PP_HUB_SPOKE_ORIENTATION_SPOKE);
                        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    }
    ...
    /* 
		...
		learn_gport = _bcm_dpp_in_lif_match_learn_gport_get(unit, local_in_lif_id);
    ...
    soc_sand_rv = soc_ppd_lif_table_entry_update(soc_sand_dev_id,local_in_lif_id,lif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    /* set map to vsi */
    rv = _bcm_dpp_in_lif_match_vsi_set(unit, local_in_lif_id, vsi);
    BCMDNX_IF_ERR_EXIT(rv);
...
{
    /* SOC_SAND_PP_DESTINATION_ID dest_id;*/
    int local_lif, global_lif;/*, gport_type;*/
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv;
    unsigned int soc_sand_dev_id;
    SOC_PPD_LIF_ENTRY_INFO *lif_info = NULL;
    ...
    rv = _bcm_dpp_am_template_l2_flooding_exchange(unit,local_lif,&flooding_info,&old_template,&is_last,&new_template,&is_allocated);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    /* update the encap-id */
    soc_sand_rv = soc_ppd_lif_table_entry_get(soc_sand_dev_id,local_lif,lif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    ...
    soc_sand_rv = soc_ppd_lif_table_entry_update(soc_sand_dev_id,local_lif,lif_info);
    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

    /* set map to NULL VSI */
    rv = _bcm_dpp_in_lif_match_vsi_set(unit, local_lif, 0);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_vlan_t *    vsi)
{
    ...
    *vsi = 0;
    ...
    /* resolve gort to LIF */
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit,port, &global_lif_id, &in_lif_id, NULL, NULL, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);

    /* from LIF to VSI */
    rv= _bcm_dpp_lif_to_vsi(unit,in_lif_id,&vsi_val, TRUE, NULL);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int verify)
{
    ...
    rv = _bcm_dpp_gport_to_global_and_local_lif(unit, gport, &global_lif_index, (int*) local_lif_index, NULL, NULL, &is_local);
    BCMDNX_IF_ERR_EXIT(rv);
