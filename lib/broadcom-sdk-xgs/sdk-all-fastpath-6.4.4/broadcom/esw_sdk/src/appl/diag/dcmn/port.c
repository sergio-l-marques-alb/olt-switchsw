...
#define _call_bpi(pbm, pbm_mask) \
    ...
    PBMP_ITER(tmp_pbm, p) { \
        brief_port_info(unit, BCM_PORT_NAME(unit, p), &info_all[p], mask); \
    }
...
char if_dpp_phy_usage[] =
    ...
    "             power, prbs ,int and <blank> for read and write\n"
#else 
    "Subcommands:\n"
    "               int <ports>\n"
    "               info\n"
    ...
    "               diag <ports> <eyescan|dsc|loopback> <parameters>\n"
    "               measure <ports> [<tx|rx>]\n"
...
if_dpp_phy(int unit, args_t *a)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        cli_out("Error: Petra don't support PHY command\n");
        return CMD_FAIL;
    }
#endif
...
#if defined(BCM_88750_SUPPORT) || defined(BCM_ARAD_SUPPORT)
    ...
    if (sal_strcasecmp(c, "info") == 0) {
        ...
        if(soc_feature(unit, soc_feature_portmod)){
            cli_out("The unit does not support phy info\n"); 
            return CMD_FAIL;
        }
        else{
    ...
    if (ARG_CNT(a) == 0) {    /*  show information for all registers */
        if(soc_feature(unit, soc_feature_portmod) || SOC_IS_ARDON(unit)){
...
_phy_prbs_dcmn(int unit, args_t *args)
{
    ...
    if ((port_str = ARG_GET(args)) == NULL) {
        cli_out("        prbs: set, get, or clear internal PRBS generator/checker.\n"
                "        Example: phy prbs 6 set mode=phy loopback=true interval=5\n"
                "                 phy prbs 6 get\n");
        return CMD_FAIL;
    }
...
_phy_diag_portmod_dsc(int unit, bcm_pbmp_t pbmp,args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        rv = portmod_port_phy_lane_access_get(unit, port, &params, MAX_CORE_ACCESS_STRUCTURES_PER_PORT, phy_access, &nof_phys_structs, NULL);
        if(rv < 0 ){
            return CMD_FAIL;
        }
        rv = phymod_diag_dsc(phy_access, nof_phys_structs);
        if(rv < 0 ){
            return CMD_FAIL;
        }
    }
...
_phy_diag_dcmn(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(cmd_str, "dsc") == 0) {
        ...
        cli_out("valid sub option: eyescan, LoopBack\n");
        rv = CMD_FAIL;
    }
...
jer_phy_measure_nif_pll(int unit, bcm_port_t port, int *ch0_out_int, int *ch0_out_remainder)
{
    ...
    if (pll_type == SOC_JER_NIF_PLL_TYPE_PMH) {
        ...
        n_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMH_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_NDIVf);
        p_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMH_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_PDIVf);
        m0_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PMH_PLL_CONFIGr, reg_val, NIF_PMH_PLL_CFG_CH_0_MDIVf);
        ...
        n_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_val, NIF_PML_0_PLL_CFG_NDIVf);
        p_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_val, NIF_PML_0_PLL_CFG_PDIVf);
        m0_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_0_PLL_CONFIGr, reg_val, NIF_PML_0_PLL_CFG_CH_0_MDIVf);
        ...
        n_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_val, NIF_PML_1_PLL_CFG_NDIVf);
        p_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_val, NIF_PML_1_PLL_CFG_PDIVf);
        m0_div = soc_reg_above_64_field32_get(unit, ECI_NIF_PML_1_PLL_CONFIGr, reg_val, NIF_PML_1_PLL_CFG_CH_0_MDIVf);
        ref_clk = SOC_DPP_CONFIG(unit)->jer->pll.ref_clk_pml_in[1];
    }

    rv = jer_phy_measure_pll_calc(ref_clk, n_div, p_div, m0_div, ch0_out_int, ch0_out_remainder);
    if (rv != BCM_E_NONE) {
        return CMD_FAIL;
    }
...
STATIC int
jer_phy_diag_calc_os(phymod_phy_access_t* phy_access, int* os_int, int* os_remainder) {
    int rv, osr_mode;
    phymod_phy_diagnostics_t phy_diag;

    rv = phymod_phy_diagnostics_t_init(&phy_diag);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed in phymod_phy_diagnostics_t_init: %s\n", bcm_errmsg(rv));
        return BCM_E_FAIL;
    }

    rv = phymod_phy_diagnostics_get(phy_access, &phy_diag);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed in phymod_phy_diagnostics_get: %s\n", bcm_errmsg(rv));
        return BCM_E_FAIL;
    }

    osr_mode = phy_diag.osr_mode;
 ...
 * We can't measure the serdes freq directly, but we can measure the sync_eth counter, and reconstruct the serdes freq from it by multiplying it with the blocks deviders.
 *
 * Serdes freq                                                                Sync_eth counter
 *   _         --> VCO --> Oversample --> PM synce_div --> NBIH synce div -->     _
 * _| |_                                                                        _| |_
 */
int
_phy_jer_nif_measure(int unit, bcm_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane) {
    uint32 sync_sth_cnt;
    int serdes_freq_int_temp, serdes_freq_remainder_temp;
    int total_time_measured_int, total_time_measured_remainder;
    int number_of_gtimer_cycles = 10240;
    ...
    int one_bit_clk_period_int, one_bit_clk_period_remainder;
    soc_dpp_config_arad_t *dpp_arad;
    uint32 reg_val, default_configuration, pmh_synce_rstn_prev = 0, synce_prev_config = 0;
    ...
    rv = soc_jer_port_pll_type_get(unit, port, &pll_type);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to get PLL type: %s\n",
                bcm_errmsg(rv));
        return BCM_E_FAIL;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to read NBIH_GTIMER_CONFIGURATION: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }

    /* Get physical lane in the range of 1-72 */
    if(SOC_IS_DIRECT_PORT(unit, port)) {
        *lane = port - SOC_INFO(unit).physical_port_offset;
    } else {
        *lane = SOC_INFO(unit).port_l2p_mapping[port] - 1;
    ...
    rv = soc_jer_qsgmii_offsets_remove(unit, *lane, lane);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed soc_jer_qsgmii_offsets_remove: %s\n", bcm_errmsg(rv));
        goto exit;
    }
    ...
    rv = READ_NBIH_GTIMER_CONFIGURATIONr(unit, &reg_val);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to read NBIH_GTIMER_CONFIGURATION: %s\n",
                bcm_errmsg(rv));
        goto exit;
    ...
    /* Config NBIH Gtimer */
    soc_reg_field_set(unit, NBIH_GTIMER_CONFIGURATIONr, &reg_val, GTIMER_ENABLEf, 0x1);
    soc_reg_field_set(unit, NBIH_GTIMER_CONFIGURATIONr, &reg_val, GTIMER_CYCLEf, number_of_gtimer_cycles);
    soc_reg_field_set(unit, NBIH_GTIMER_CONFIGURATIONr, &reg_val, GTIMER_RESET_ON_TRIGGERf, 0x0);
    rv = WRITE_NBIH_GTIMER_CONFIGURATIONr(unit, reg_val);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to write NBIH_GTIMER_CONFIGURATION: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to write ECI_GP_CONTROL_9: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to read NBIH_SYNC_ETH_CFG: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    synce_read = 1;
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to write NBIH_SYNC_ETH_CFG: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to read NBIH_GTIMER_TRIGGER: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to write NBIH_GTIMER_TRIGGER: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    while (reg_val == 0x1) {
        ...
        if(10 == counter){
            cli_out("ERROR: Waiting for NBIH_GTIMER_TRIGGER timeout\n");
            rv = BCM_E_TIMEOUT;
            goto exit;
        }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to read NBIH_SYNC_ETH_COUNTER: %s\n",
                bcm_errmsg(rv));
        goto exit;
    }
    ...
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to init access params: %s\n", bcm_errmsg(rv));
        goto exit;
    }
    rv = phymod_phy_access_t_init(&phy_access);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to init phy access: %s\n", bcm_errmsg(rv));
        goto exit;
    }

    rv = portmod_port_phy_lane_access_get(unit, port, &params, max_phys, &phy_access, &nof_phys, NULL);
    if (rv != BCM_E_NONE) {
    ...
    if (pll_type != SOC_JER_NIF_PLL_TYPE_PMH) {
        ...
        rv = BCMI_TSCE_XGXS_READ_MAIN0_SYNCE_CTLr(&phy_access.access, &synce_ctrl);
        if (rv != BCM_E_NONE) {
            cli_out("ERROR: Failed to read BCMI_TSCE_XGXS_READ_MAIN0_SYNCE_CTL: %s\n", bcm_errmsg(rv));
            goto exit;
        }
        if (phy_access.access.lane_mask & 0x1) {
            ...
            cli_out("ERROR: Wrong lane\n");
            goto exit;
        }
    } else {
        BCMI_TSCF_XGXS_MAIN0_SYNCE_CTLr_t synce_ctrl;
        rv = BCMI_TSCF_XGXS_READ_MAIN0_SYNCE_CTLr(&phy_access.access, &synce_ctrl);
        if (rv != BCM_E_NONE) {
            cli_out("ERROR: Failed to read BCMI_TSCF_XGXS_READ_MAIN0_SYNCE_CTL: %s\n",
                    bcm_errmsg(rv));
            goto exit;
        }
        if (phy_access.access.lane_mask & 0x1) {
    ...
    soc_reg_field_set(unit, NBIH_GTIMER_CONFIGURATIONr, &reg_val, GTIMER_ENABLEf, 0x0);
    rv = WRITE_NBIH_GTIMER_CONFIGURATIONr(unit, reg_val);
    if (rv != BCM_E_NONE) {
        cli_out("ERROR: Failed to write NBIH_GTIMER_CONFIGURATION: %s\n",
                bcm_errmsg(rv));
    ...
    *one_clk_time_measured_remainder = ((((total_time_measured_int*10000) + (total_time_measured_remainder)) / (sync_sth_cnt * vco_div)) % 10000);
    ...
    /* Get oversample */
    rv = jer_phy_diag_calc_os(&phy_access, &os_int, &os_remainder);
    if (rv != BCM_E_NONE) {
        goto exit;
    }
...
exit:
    ...
    if (WRITE_NBIH_GTIMER_CONFIGURATIONr(unit, default_configuration) != BCM_E_NONE) {
        cli_out("ERROR: Failed to write back the default configuration to NBIH_GTIMER_CONFIGURATION.\n");
    }

    if (pmh_synce_read) {
        if (READ_ECI_GP_CONTROL_9r(unit, reg_val_above_64) != BCM_E_NONE) {
            cli_out("ERROR: Failed to write back the original configuration to NBIH_SYNC_ETH_CFG.\n");
        } else {
            soc_reg_above_64_field32_set(unit, ECI_GP_CONTROL_9r, reg_val_above_64, PMH_SYNCE_RSTNf, pmh_synce_rstn_prev);
            if (WRITE_ECI_GP_CONTROL_9r(unit, reg_val_above_64) != BCM_E_NONE) {
                cli_out("ERROR: Failed to write back the original configuration to ECI_GP_CONTROL_9.\n");
            }
        }
    }

    if (synce_read) {
        if (WRITE_NBIH_SYNC_ETH_CFGr(unit, 0, synce_prev_config) != BCM_E_NONE) {
            cli_out("ERROR: Failed to write back the original configuration to NBIH_SYNC_ETH_CFG.\n");
        }
    }
...
int diag_dcmn_phy_measure_port(int unit, bcm_port_t port, int is_rx, uint32* rate_int, uint32* rate_remainder)
{
    ...
    if(SOC_IS_DFE(unit) || (SOC_IS_ARAD(unit) && IS_SFI_PORT(unit, port))){
        lane = port;
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        if(SOC_IS_DFE(unit) || (SOC_IS_ARAD(unit) && IS_SFI_PORT(unit, port))){
...
#endif         
            rv = diag_dcmn_phy_measure_port(unit, port, is_rx, &rate_int, &rate_remainder);
            if (rv != BCM_E_NONE)
            {
            ...
            if (SOC_IS_JERICHO(unit)) {
                rv = jer_phy_measure_fabric_pll(unit, port, &ch0_out_int, &ch0_out_remainder);
                if(CMD_OK != rv) return CMD_FAIL;
...
#ifdef BCM_88675_A0
            if (SOC_IS_JERICHO(unit)) {
                rv = _phy_jer_nif_measure(unit, port, &type_of_bit_clk, &one_clk_time_measured_int, &one_clk_time_measured_remainder, &serdes_freq_int, &serdes_freq_remainder, &port_lane);
            ...
            cli_out("Measured rate for port %1d (WC%1d,L-%1d) is %d.%d and PLL is %3d.%-3d\n", port, port_lane / SOC_DCMN_NOF_LINKS_IN_MAC,
