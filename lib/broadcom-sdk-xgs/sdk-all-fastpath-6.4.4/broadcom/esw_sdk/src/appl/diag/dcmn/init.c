...
appl_dpp_convert_modport_to_base_voq_connector(int local_port,int ingress_modid)
{
    ...
    voq_conn_id = (num_of_512 * 1024) + voq_conn_internal_id;
...

    {
        ...
        for (modid_idx = 0; modid_idx < g_dii.nof_devices; modid_idx++) {
            for (idx = 0; idx < g_dii.num_ports; idx++) {
                ...
                if (g_dii.base_modid == 0) {
                    voq_connector_id  = 
                        appl_dpp_convert_modport_to_base_voq_connector(port,modid_idx*MAX_MODIDS_PER_DEVICE);
                        BCM_COSQ_GPORT_VOQ_CONNECTOR_CORE_SET(g_dii.gport_ucast_voq_connector_group[modid_idx][idx],voq_connector_id, mapping_info.core);
                }
                ...
                rv = bcm_cosq_voq_connector_gport_add(unit, &config, &g_dii.gport_ucast_voq_connector_group[modid_idx][idx]);
                if (BCM_FAILURE(rv)) {
                    LOG_ERROR(BSL_LS_APPL_COMMON,
                              (BSL_META_U(unit,
...
STATIC int appl_l2_init(int unit)
{
    int rv = BCM_E_NONE;
...
{
    int rv = BCM_E_NONE;
    bcm_field_group_config_t grp, grp2, grp3, grp4, grp5;
    ...
    bcm_field_data_qualifier_t data_qual, data_qual2, data_qual3, data_qual4 ; 
    bcm_field_group_t entry_groups[14]={123,124,123,123,123,123,125,125,125,125,125,126,127,127};
    uint32 entry_ndx, nof_itmh_entries = 14;
   ...
   */
    ...
    data_qual4.offset = 36 ; 
    data_qual4.length = 20; 
    data_qual4.qual_id = 63;
    ...
    for (entry_ndx = 0; entry_ndx < nof_itmh_entries; entry_ndx++) {
     ...
     /*build extraction fields*/
      ...
      case 5: /* Traffic Class */
          ...
          break;
      case 6: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
          q_data = 0x8;
          q_mask = 0xf;
     ...
     case 8:  /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = outlif*/
      ...
      case 9: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = multicast*/
          q_data = 0x9;
          q_mask = 0xf;
          rv = bcm_field_qualify_data(unit, ent, data_qual3.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
      ...
      case 11: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS field =1 , type = ISQ*/
          q_data = 0xA;
          q_mask = 0xf;
          rv = bcm_field_qualify_data(unit, ent, data_qual3.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
       ...
       case 12: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS - remove ITMH + extension = 7 Bytes*/
          q_data = 0x8;
          q_mask = 0x8;
          rv = bcm_field_qualify_data(unit, ent, data_qual3.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
       ...
       case 13: /* ITMH_DESTINATION_INFO EXTENSION_EXISTS =0- remove ITMH = 4 Bytes*/
          q_data = 0x0;
          q_mask = 0x8;
          rv = bcm_field_qualify_data(unit, ent, data_qual3.qual_id, &q_data, &q_mask, 1);
          if (BCM_FAILURE(rv)) {
      ...
      rv = bcm_field_direct_extraction_action_add(unit,
                                                  ent,
                                                  ext_action,
                                                  (entry_ndx == 4 || entry_ndx == 12 || entry_ndx == 13 ) ? 2 : 1 /* count */,
                                                  (entry_ndx == 4 || entry_ndx == 12 || entry_ndx == 13 ) ? ext_field2 : &ext_field );
        ...
        if (! option ) {
          ...
          if (card_type == LINE_CARD_GFA_PETRA_B_INTERLAKEN) {
            ...
            CMVEC(unit).write(&CMDEV(unit).dev, 0x80154, 0x23);
                ...
                thresh.words = 50;
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_HIGH, SOC_PETRA_EGR_PORT_THRESH_TYPE_0, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_HIGH, SOC_PETRA_EGR_PORT_THRESH_TYPE_1, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_HIGH, SOC_PETRA_EGR_PORT_THRESH_TYPE_2, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_HIGH, SOC_PETRA_EGR_PORT_THRESH_TYPE_3, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_LOW, SOC_PETRA_EGR_PORT_THRESH_TYPE_0, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_LOW, SOC_PETRA_EGR_PORT_THRESH_TYPE_1, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_LOW, SOC_PETRA_EGR_PORT_THRESH_TYPE_2, &thresh, &exact_thresh);
                soc_petra_egr_ofp_fc_set(unit, 0,SOC_PETRA_EGR_Q_PRIO_LOW, SOC_PETRA_EGR_PORT_THRESH_TYPE_3, &thresh, &exact_thresh);
