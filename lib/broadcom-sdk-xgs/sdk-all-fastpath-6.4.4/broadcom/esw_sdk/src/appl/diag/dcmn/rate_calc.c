/* 
 * $Id: rate_calc.c,v 1.6 Broadcom SDK $
 *
...
void print_packet_rate32(uint32 counter_result, char *counter_name) {
   cli_out("%-40s = %d Kpps", counter_name, (counter_result) / 10);
   cli_out("\n");
}

void print_packet_rate(uint64 val64, char *counter_name) {
...
void print_bit_rate(uint64 val64, char *counter_name) {
   uint32 counter_result  = (get_field32(COMPILER_64_LO(val64), 0, 31) * 8 / 10000 + (get_field32(COMPILER_64_HI(val64), 0, 13) * 3435974));
   uint32 counter_OVF     = get_field32(COMPILER_64_HI(val64), 14, 14);
   cli_out("%-40s = %d Mbps", counter_name, (counter_result));
   if (counter_OVF != 0) {
       cli_out(" (ok)");
   }
   cli_out("\n");
...
int enable_sch_gtimer(int unit, int core, int nof_clocks_int) {
   ...
   if (SOC_IS_JERICHO(unit)) {
       soc_reg_field_set(unit, SCH_GTIMER_CONFIGURATIONr, &val32, GTIMER_CYCLEf, nof_clocks_int);
       soc_reg_field_set(unit, SCH_GTIMER_CONFIGURATIONr, &val32, GTIMER_ENABLEf, 1);
       soc_reg_field_set(unit, SCH_GTIMER_CONFIGURATIONr, &val32, GTIMER_RESET_ON_TRIGGERf, 1);
       if (WRITE_SCH_GTIMER_CONFIGURATIONr(unit, core, val32) != BCM_E_NONE) return BCM_E_FAIL;
   } else {
...
int disable_sch_gtimer(int unit, int core) {
   ...
   if (SOC_IS_JERICHO(unit)) {
       soc_reg_field_set(unit, SCH_GTIMER_CONFIGURATIONr, &val32, GTIMER_ENABLEf, 0);
       if (WRITE_SCH_GTIMER_CONFIGURATIONr(unit, core, val32) != BCM_E_NONE) return BCM_E_FAIL;
   } else {
       if (READ_SCH_SCHEDULER_CONFIGURATION_REGISTERr(unit, core, &val32) != BCM_E_NONE) return BCM_E_FAIL;
...
int get_otm_port(int unit, int core, int local_port_id, int *otm_port) {
   uint64  val64;
   COMPILER_64_ZERO(val64);
   if (READ_EGQ_DSP_PTR_MAPm(unit, EGQ_BLOCK(unit, core), local_port_id, &val64) != BCM_E_NONE) {
       return BCM_E_FAIL;
   }
   *otm_port = get_field32(COMPILER_64_HI(val64), 0, 7);
   return CMD_OK;
}
...
int get_ps_mode(int unit, int core, int local_port_id, uint32 *ps_mode) {
   ...
   COMPILER_64_ZERO(val64);
   if (get_otm_port(unit, core, local_port_id, &otm_port) != CMD_OK) return BCM_E_FAIL;
   ps_num   = get_field32(otm_port, 3, 7);
...
int calc_sch_rate(int unit, uint32 num_of_credits, int clock_radio, int *sch_rate_int, int *sch_rate_remainder) {
   uint32 credit_size;
   ...
   *sch_rate_int = (credit_size * num_of_credits ) / (1000000000/(8*clock_radio));
   *sch_rate_remainder = ((credit_size * num_of_credits) / (1000000000 / (8*clock_radio*1000))) % 1000; 
...
int calc_sch_ps_rate(int unit, int local_port_id) {
   ...
   if (SOC_IS_JERICHO(unit)) {
     nof_clocks = SOC_JER_CORE_FREQ_KHZ_DEFAULT * 100; /* 72,000,000 clocks --> 0.1 sec*/
   } else {
   ...
   for (i = 0; i < max_tc; i++) {
      ...
      sal_usleep(150000);    /* Wait 0.15 sec */
   ...
   for (i = 0; i < max_tc; i++) {
      ...
      if (calc_sch_rate(unit, counter_result, clock_radio, &sch_rate_in_gbps_int, &sch_rate_in_gbps_remainder) != BCM_E_NONE) return BCM_E_FAIL;
      if (calc_sch_rate(unit, qCreditRes, clock_radio, &qCreditRateInGbps_int, &qCreditRateInGbps_remainder) != BCM_E_NONE) return BCM_E_FAIL;
   ...
   if (READ_SCH_DBG_DVS_CREDIT_COUNTER_CONFIGURATIONr(unit, core, &val32) != BCM_E_NONE) return BCM_E_FAIL;
   soc_reg_field_set(unit, SCH_DBG_DVS_CREDIT_COUNTER_CONFIGURATIONr, &val32, CNT_BY_PORTf, 0);
   if (WRITE_SCH_DBG_DVS_CREDIT_COUNTER_CONFIGURATIONr(unit, core, val32) != BCM_E_NONE) return BCM_E_FAIL;
...
int calc_sch_flow_bw(int unit, int flow_id) {
   ...
   if (SOC_IS_JERICHO(unit)) {
       nof_clocks_int = SOC_JER_CORE_FREQ_KHZ_DEFAULT * 100; /* 72,000,000 clocks --> 0.1 sec*/
   } else {
...
int gtimer_enable(int unit, char* block_name, int period_num) {
   ...
   if (!sal_strncasecmp(block_name, "FMAC", strlen(block_name))) {
   ...
   else if (!sal_strncasecmp(block_name, "SCH", strlen(block_name)) && SOC_IS_ARADPLUS_AND_BELOW(unit)) {
      ...
      if (!sal_strncasecmp(block_name, "IQM", strlen(block_name))&& SOC_IS_ARADPLUS_AND_BELOW(unit)) {
...
cmd_dpp_gtimer(int unit, args_t *a) {
   ...
   while (NULL != option) {
      if (!sal_strncasecmp(option, "?", strlen(option))) {
      ...
      else if (!sal_strncasecmp(option, "enable", strlen(option))) {
         mode_given++;
      ...
      } else if (!sal_strncasecmp(option, "trigger", strlen(option))) {
         mode_given++;
         if ((block_name = var_get(CMD_DPP_GTIMER_BLOCKNAME)) == NULL) {
            cli_out("block_name is NULL\n");
            return CMD_FAIL;
         }
            
         if ((rv = gtimer_trigger(unit, block_name)) != CMD_OK) {
            cli_out("Failed(%d) trigger gtimer\n", rv);
            retCode = CMD_FAIL;
         }
      } else if (!sal_strncasecmp(option, "stop", strlen(option))) {
         mode_given++;
         if ((block_name = var_get(CMD_DPP_GTIMER_BLOCKNAME)) == NULL) {
            cli_out("block_name is NULL\n");
            return CMD_FAIL;
         ...
         if ((rv = gtimer_stop(unit, block_name)) != CMD_OK) {
