 ...
 *  Revision      :  $Id: falcon_furia_functions.c 845 2015-01-12 21:29:37Z kirand $ *
 *                                                                                 *
...
err_code_t falcon_furia_display_eye_scan( const phymod_access_t *pa ) {
  ...
  err_code = falcon_furia_meas_eye_scan_start( pa, EYE_SCAN_HORIZ);
  if (err_code) {
    falcon_furia_meas_eye_scan_done( pa );
    return (err_code);
  }

  falcon_furia_display_eye_scan_header( pa, 1);
  /* display stripe */
  for (y = 62;y>=-62;y=y-2) { 
 	err_code = falcon_furia_read_eye_scan_stripe( pa, &stripe[0], &status);
	if (err_code) {
		falcon_furia_meas_eye_scan_done( pa );
		return (err_code);
	}

    falcon_furia_display_eye_scan_stripe( pa, y,&stripe[0]);
	USR_PRINTF(("\n"));  
  }  
  /* stop acquisition */
  err_code = falcon_furia_meas_eye_scan_done( pa );
  if (err_code) {
	return (err_code);
  }

  falcon_furia_display_eye_scan_footer( pa, 1);
...
err_code_t falcon_furia_display_lowber_eye( const phymod_access_t *pa, const struct falcon_furia_eyescan_options_st eyescan_options, uint32_t *buffer) {
	...
	j = 0;
    for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
        ESTM(USR_PRINTF(("%6dmV : ",_ladder_setting_to_mV(y,0))));
...
err_code_t falcon_furia_meas_eye_scan_start( const phymod_access_t *pa, uint8_t direction) {
  ...
  if(direction == EYE_SCAN_VERTICAL) {
	err_code = falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_VSCAN_EYE,200);
  } else {
	  err_code = falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_START_HSCAN_EYE,200);
  }
  	if (err_code) {
		return (err_code);
	}
...
err_code_t falcon_furia_read_eye_scan_stripe( const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
	...
	*status = 0;
	for(i=0;i<32;i++) {
		err_code = falcon_furia_poll_diag_eye_data(pa,&val[0], &sts, 200);
 		*status |= sts & 0xF000;
		if (err_code) {
			return (err_code);
		}
...
err_code_t falcon_furia_meas_eye_scan_done( const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = falcon_furia_pmd_uc_diag_cmd( pa, CMD_UC_DIAG_DISABLE,200);
 ...
 ESTM(sts = rdv_usr_diag_status());
	...
	*cnt = (sts & 0x00FF)/3;
	for(i=0;i < *cnt;i++) {
		...
		err_code = falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout);
		if(err_code) return(err_code);
		...
		err_code = falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout);
		if(err_code) return(err_code);
...
err_code_t falcon_furia_display_lane_state( const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _falcon_furia_display_lane_state_no_newline( pa );
  USR_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t falcon_furia_display_core_state_line( const phymod_access_t *pa ) {
  err_code_t err_code;
  err_code = _falcon_furia_display_core_state_no_newline( pa ); 
  USR_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t falcon_furia_display_lane_config( const phymod_access_t *pa ) {
  ...
  err_code = falcon_furia_get_uc_lane_cfg( pa, &lane_cfg);
  if (err_code) {
    return (err_code);
  }
...
err_code_t falcon_furia_display_lane_debug_status( const phymod_access_t *pa ) {
  ...
  err_code = falcon_furia_get_usr_ctrl_disable_startup( pa, &ds);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_get_usr_ctrl_disable_startup_dfe( pa, &dsd);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_get_usr_ctrl_disable_steady_state( pa, &dss);
  if (err_code) {
    return (err_code);
  }
  ...
  err_code = falcon_furia_get_usr_ctrl_disable_steady_state_dfe( pa, &dssd);
  if (err_code) {
    return (err_code);
  }
...
uint16_t falcon_furia_rdwl_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  ...
  EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(falcon_furia_rdw_ram(pa, &rddata, uc_lane_info.lane_var_base_address+addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size || addr > 255) {                       /* uC command only support 8-bit addr */                 
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
...
uint16_t falcon_furia_rdwc_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
  if (uc_lane_info.is_direct_ram_access_avail) {
	  EFUN(falcon_furia_rdw_ram(pa, &rddata, uc_lane_info.core_var_base_address+(uint16_t)addr, 1));                    /* Direct RAM access */
  }
  else {
	  if (addr >= uc_lane_info.lane_ram_size) {         
		  *err_code_p = ERR_CODE_INVALID_RAM_ADDR;                                  /* Error Code indicating Out of Range access */
...
#else
  { uint16_t rddata;
      USR_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
      ...
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          ...
          USR_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }     
      USR_PRINTF(("// ATE_LOG falcon_furia_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t falcon_furia_uc_core_var_dump( const phymod_access_t *pa ) {
  ...
  for (addr = 0x0; addr < 0xFF; addr++) {  
    rddata = falcon_furia_rdbc_uc_var( pa, &err_code, addr);
    if (err_code) {
      return (err_code);
    }
...
err_code_t falcon_furia_uc_lane_var_dump( const phymod_access_t *pa ) {
  ...
  USR_PRINTF(("********************************************\n")); 
    ...
    err_code = falcon_furia_get_uc_ln_info(pa, &lane_info);
    if (err_code) {
      return (err_code);
    }

    for (addr = 0x0; addr < lane_info.lane_ram_size; addr++) {  
      rddata = falcon_furia_rdbl_uc_var( pa, &err_code, addr);
      if (err_code) {
        return (err_code);
      }
	...
	if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
		...
		while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
			...
			if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                falcon_furia_uc_lane_idx_to_system_id(pa, uc_lane_id_str, cc);
...
err_code_t falcon_furia_display_state ( const phymod_access_t *pa ) {
  ...
  if (!err_code) err_code = falcon_furia_display_core_state( pa );
  if (!err_code) err_code = falcon_furia_display_lane_state_hdr( pa );
  if (!err_code) err_code = falcon_furia_display_lane_state( pa );
...
err_code_t falcon_furia_display_config ( const phymod_access_t *pa ) {
  ...
  if (!err_code) err_code = falcon_furia_display_core_config( pa );
  if (!err_code) err_code = falcon_furia_display_lane_config( pa );
...
#else
    *die_temp = 410-((48705/10000)*die_temp_sensor_reading); 
#endif
    return(ERR_CODE_NONE);    
}
