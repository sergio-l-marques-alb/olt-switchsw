/*
 * $Id: field_int.h,v 1.99 Broadcom SDK $
 ...
 *  read the cached state and use that (the original behaviour).
 *
 ...
 *  be FALSE because it does not honour the prefix argument, but can be useful
 ...
 *
 *  The result of this is, for example, to get the lower nybble of byte 1 in
 *  Dune bit numbering mode requires the programmable field to start at bit 12
 *  and run for four bits; to get the same nybble in BCM bit numbering mode
 *  requires the field to start at bit 11 and run for four bits.  It's more
 *  confusing when spanning bytes.  To get the lower nybble of byte 0 and the
 *  upper nybble of byte 1, Dune mode would start at bit 4 and run for eight
 ...
 *  caes the actual bit values does not change (so the value of a byte is the
 ...
 */
#define _BCM_PETRA_FIELD_GROUP_LIMIT(_u) SOC_PPD_FP_NOF_DBS
...
#define _BCM_PETRA_FIELD_DQ_LIMIT(_u) \
    ((SOC_IS_ARAD(_u)?(SOC_PPD_FP_QUAL_HDR_USER_DEF_LAST - SOC_PPD_FP_QUAL_HDR_USER_DEF_0 + 1): \
     (SOC_IS_PETRAB(_u)?10: \
     0)))
#else
...
/*
 *  Shift distance and masks for processor & set in 'counter' field
 *  Set the Stat-Id format
 */
/*
 * Set the Stat-ID encoding shared between Soc_petra-B and Arad
 * Format: | Counter-Processor 31:30 | Counter-ID 29:0 |
 */

#define _BCM_PETRA_CTR_PROC_MASK ((1 << 2) - 1)
/* Avoid a value of 30 for the proc shift because a signed stat-id will be negative */
#define _BCM_PETRA_CTR_PROC_SHIFT 29 /* minimum 20 for the Statistic-Report Counter in Arad */
#define _BCM_PETRA_CTR_SET_MASK ((1 << _BCM_PETRA_CTR_PROC_SHIFT) - 1)
#define _BCM_PETRA_CTR_SET_SHIFT 0
#define _BCM_PETRA_PROC_CNTR_FROM_STAT(_proc, _ctr, _stat) \
    _proc = ((_stat) >> _BCM_PETRA_CTR_PROC_SHIFT) & _BCM_PETRA_CTR_PROC_MASK; \
    _ctr = ((_stat) >> _BCM_PETRA_CTR_SET_SHIFT) & _BCM_PETRA_CTR_SET_MASK
#define _BCM_PETRA_STAT_FROM_PROC_CNTR(_stat, _proc, _ctr) \
    _stat = (((_proc) & _BCM_PETRA_CTR_PROC_MASK) << _BCM_PETRA_CTR_PROC_SHIFT)\
            | (((_ctr) & _BCM_PETRA_CTR_SET_MASK) << _BCM_PETRA_CTR_SET_SHIFT)
 ...
 *  value for hte type to indicate NULL (since zero is valid).  Basically this
...
typedef uint16 _bcm_dpp_field_presel_idx_t;
typedef uint8 _bcm_dpp_field_dq_idx_t;
 ...
 */
#define _BCM_DPP_NOF_LOCAL_PORTS(unit) (SOC_IS_PETRAB(unit) ? SOC_TMC_NOF_FAP_PORTS_PETRA : \
                             (SOC_IS_JERICHO(unit) ? SOC_TMC_NOF_FAP_PORTS_JERICHO : SOC_TMC_NOF_FAP_PORTS_ARAD))

#define _BCM_DPP_NOF_PORTS_PER_CORE(unit) (SOC_IS_PETRAB(unit) ? SOC_TMC_NOF_FAP_PORTS_PETRA : SOC_TMC_NOF_FAP_PORTS_PER_CORE)
...
#ifdef BCM_ARAD_SUPPORT
#define _BCM_DPP_NOF_FIELD_DATAS  64 /* hard-coded, since doesn't compile in kernel mode
                                        (SOC_PPD_FP_QUAL_HDR_USER_DEF_LAST - SOC_PPD_FP_QUAL_HDR_USER_DEF_0 + 1) */
#else
 ...
 */
typedef SHR_BITDCL _bcm_dpp_field_qual_set_t[_SHR_BITDCLSIZE(SOC_PPD_NOF_FP_QUAL_TYPES)];
typedef SHR_BITDCL _bcm_dpp_field_action_set_t[_SHR_BITDCLSIZE(SOC_PPD_NOF_FP_ACTION_TYPES)];
...
#define _BCM_DPP_FIELD_ENT_IS_INTTCAM(unit, _entry) \
     ((/* causes CONSTANT_EXPRESSION_RESULT Coverity defect
        * because _bcmDppFieldEntryTypeInternalTcam is 0.
        * if in the feature this vaulue is changed - reinsert it
      (_BCM_DPP_FIELD_ENT_BIAS(unit, InternalTcam) <= _entry) && */((unitData->entryTcLimit + _BCM_DPP_FIELD_ENT_BIAS(unit, InternalTcam)) > _entry))? TRUE: FALSE)

#define _BCM_DPP_FIELD_ENT_IS_EXTTCAM(unit, _entry) \
     (((_BCM_DPP_FIELD_ENT_BIAS(unit, ExternalTcam) <= _entry) && ((unitData->entryExtTcLimit + _BCM_DPP_FIELD_ENT_BIAS(unit, ExternalTcam)) > _entry))? TRUE: FALSE)

#define _BCM_DPP_FIELD_ENT_IS_DIR_EXT(unit, _entry) \
     (((_BCM_DPP_FIELD_ENT_BIAS(unit, DirExt) <= _entry) && ((unitData->entryDeLimit + _BCM_DPP_FIELD_ENT_BIAS(unit, DirExt)) > _entry))? TRUE: FALSE)
...
#define _BCM_DPP_FIELD_TCAM_ENTRY_LIMIT(_entryType) \
    ((_entryType == _bcmDppFieldEntryTypeInternalTcam)? unitData->entryTcLimit: unitData->entryExtTcLimit)
#define _BCM_DPP_FIELD_TCAM_ENTRY_TC_FREE(_entryType) \
    ((_entryType == _bcmDppFieldEntryTypeInternalTcam)? unitData->entryIntTcFree: unitData->entryExtTcFree)
#define _BCM_DPP_FIELD_TCAM_ENTRY_TC_COUNT_OPERATION(_entryType, _operation) \
    if ((_entryType == _bcmDppFieldEntryTypeInternalTcam)) {unitData->entryIntTcCount _operation} \
    else {unitData->entryExtTcCount _operation}
#define _BCM_DPP_FIELD_TCAM_ENTRY_TC_FREE_OPERATION(_entryType, _operation) \
    if ((_entryType == _bcmDppFieldEntryTypeInternalTcam)) {unitData->entryIntTcFree _operation} \
    else {unitData->entryExtTcFree _operation}
 ...
 *  some of them are not combinable (they are noted so in their description).
 *
...
typedef struct _bcm_dpp_field_device_group_mode_bits_s {
    ...
    SOC_PPD_FP_DATABASE_TYPE hwType;            /* hardware database type */
 ...
 *  honoured.  If impliedActionSet is FALSE, a group must have an action set
 ...
 *  defined stage, sharesEntriesTc is -1.  If this stage shareds hardware TCAM
 ...
 *  a previously defined stage, sharesEntriesDe is -1.  If this stage shareds
...
typedef struct _bcm_dpp_field_device_stage_info_s {
    ...
    SOC_PPD_FP_DATABASE_STAGE hwStageId;        /* hardware stage ID */
 ...
 *  SOC_PPD_NOF_FP_PREDEFINED_ACL_KEYS if there is no predefined key.
 *
...
typedef struct _bcm_dpp_field_device_st_mapping_s {
    ...
    SOC_PPD_FP_PREDEFINED_ACL_KEY predefKey;    /* predefined key */
 ...
 *
 *  typeNames points to an array of strings taht give a name to each of the
 ...
 *  _BCM_DPP_FIELD_GROUP_CHANGED - group has been changed
 ...
 *  the IPs with MPLS and L2, it requires the application make addiitonal
...
#define _BCM_DPP_FIELD_GROUP_CHANGED                0x00000008
...
#define _BCM_DPP_FIELD_GROUP_USE_PREDEFINED_DB      0x00400000 /* mapped SOC_PPC_FP_DATABASE_INFO_FLAGS_USE_DBAL */
...
#define _BCM_DPP_FIELD_GROUP_TINFO                  0x00001EC1
...
typedef struct _bcm_dpp_field_group_s {
    ...
    _bcm_dpp_field_qual_set_t pqset;            /* DPP qualifiers */
    _bcm_dpp_field_action_set_t paset;          /* DPP actions */
    uint32 groupTypes;                          /* this group's types */
    ...
    int maxEntryPriorities;                     /* max entry priorities in group */
    /* kept in backing store to make it easier */
    _bcm_dpp_field_stage_idx_t stage;           /* this group's stage */
    SOC_PPD_FP_PREDEFINED_ACL_KEY predefKey;    /* predefined key (if used) */
    /* needs to be kept in backing store */
    uint32 groupFlags;                          /* group flags */
    bcm_field_group_mode_t grpMode;             /* group mode */
 ...
 *
 *  PPD layer says qualifier >= SOC_PPD_NOF_FP_QUAL_TYPES is invalid, but it
 *  also curiously provides SOC_PPD_FP_QUAL_IRPP_INVALID as an alternative
 ...
 *  SOC_PPD_FP_QUAL_IRPP_INVALID instead of SOC_PPD_NOF_FP_QUAL_TYPES.
 */
#define _BCM_DPP_FIELD_PPD_QUAL_VALID(_qual) \
     ((SOC_PPD_FP_QUAL_IRPP_INVALID != (_qual)) && \
      (SOC_PPD_NOF_FP_QUAL_TYPES > (_qual)))
typedef struct _bcm_dpp_field_qual_s {
    ...
    SOC_PPD_FP_QUAL_TYPE hwType;                /* hardware qualifier type */
 ...
 *  but some will.  For example, SOC_PPD_FP_ACTION_TYPE_EGR_TRAP will use flags
 *  to indicate which value of SOC_PPD_TRAP_ACTION_PROFILE_OVERWRITE* is
 *  intended.
 *
 *  PPD layer says action type >= SOC_PPD_NOF_FP_ACTION_TYPES is invalid, but
 *  it also curiously provides SOC_PPD_FP_ACTION_TYPE_INVALID to indicate
 *  invalid action type.  The PPD code usually seems to set
 *  SOC_PPD_FP_ACTION_TYPE_INVALID as the invalid type on reading, but the
 *  other is also possible.  We will set SOC_PPD_FP_ACTION_TYPE_INVALID on
 *  writing as well, but still need to check the other mode when reading.
 */
#define _BCM_DPP_FIELD_PPD_ACTION_VALID(_action) \
     ((SOC_PPD_FP_ACTION_TYPE_INVALID != (_action)) && \
      (SOC_PPD_NOF_FP_ACTION_TYPES > (_action)))
typedef struct _bcm_dpp_field_tc_p_act_s {
    /* can be retrieved from hardare */
    SOC_PPD_FP_ACTION_TYPE hwType;              /* hardware action type */
...
typedef struct _bcm_dpp_field_de_act_s {
    ...
    SOC_PPD_FP_DIR_EXTR_ACTION_VAL hwParam;     /* hardware parameters */
    /* must be kept in backing store */
    bcm_field_action_t bcmType;                 /* BCM action type */
 ...
 *  _BCM_DPP_FIELD_ENTRY_WANT_HW - entry wants to be in haardware
 ...
 */
#define _BCM_DPP_NOF_BCM_ACTIONS_PER_DB_MAX SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX
/*
...
typedef struct _bcm_dpp_field_entry_common_s {
    ...
    _bcm_dpp_field_qual_t entryQual[SOC_PPD_FP_NOF_QUALS_PER_DB_MAX]; /* qualifiers */
    /* needs to be kept in backing store */
...
typedef struct _bcm_dpp_field_entry_s {
    ...
    _bcm_dpp_field_tc_p_act_t tcActP[SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX];
...
typedef struct _bcm_dpp_field_entry_ext_info_s {
    _bcm_dpp_field_qual_t entryQual[SOC_PPD_FP_NOF_QUALS_PER_DB_MAX]; /* qualifiers */
    _bcm_dpp_field_tc_p_act_t tcActP[SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX]; /* ppd actions */
...
typedef struct _bcm_dpp_field_entry_dir_ext_s {
    _bcm_dpp_field_entry_common_t entryCmn;     /* common data for all types */
    _bcm_dpp_field_de_act_t deAct[SOC_PPD_FP_NOF_ACTIONS_PER_DB_MAX]; /* actions */
...
typedef struct _bcm_dpp_field_stage_s {
    /* derivable from config or hardware */
    const _bcm_dpp_field_device_stage_info_t *devInfo;/* ptr to stage descr. */
    bcm_field_qset_t stgQset;                   /* stage qualifiers */
    bcm_field_qset_t qsetType[_BCM_DPP_FIELD_MAX_GROUP_TYPES + 1];/* type qset */
    _bcm_dpp_field_qual_set_t ppqset;           /* presel PPD qual set */
    bcm_field_aset_t stgAset;                   /* stage actions */
    int groupRes;                               /* groups resource type */
    int entryRes;                               /* entry resource type */
    int entryDeRes;                             /* dir ext entry res type */
    const _bcm_dpp_field_device_group_mode_bits_t *modeBits[bcmFieldGroupModeCount];
 ...
 *  the same heap cell.
 *
 *  _BCM_DPP_FIELD_UNIT_STAT_CACHED controls whether the stat code uses the
 *  cached or direct version of the functions.  It is controlled by the field
 *  control bcmFieldControlStatSyncEnable.  When stat sync enabled (this flag
 *  is FALSE, the default condition), the field APIs will use uncached access
 *  to the counter processors, enabling reads and writes to apply immediately.
 ...
 *  version is not recognised by some component of the module. This should
 ...
 */
#define _BCM_DPP_FIELD_UNIT_STAT_CACHED     0x00000001
#ifdef BCM_WARM_BOOT_SUPPORT
#define _BCM_DPP_FIELD_UNIT_WB_RESTORE_FAIL 0x10000000
#define _BCM_DPP_FIELD_UNIT_WB_VERSION_FAIL 0x20000000
#define _BCM_DPP_FIELD_UNIT_WB_FORMAT_FAIL  0x40000000
#define _BCM_DPP_FIELD_UNIT_WB_COMMIT_FAIL  0x80000000
#define _BCM_DPP_FIELD_UNIT_WB_ANY_FAILURE  0xF0000000
typedef struct _bcm_dpp_field_info_wb_s {
    ...
    unsigned int offsetUnit;                    /* offset to unit data */
    unsigned int offsetRange;                   /* offset to range data */
    unsigned int offsetDataField;               /* offset to data field data */
    unsigned int offsetPresel;                  /* offset to presel data */
    unsigned int offsetStage;                   /* offset to stage data */
    unsigned int offsetGroup;                   /* offset to groups data */
    unsigned int offsetTcamEntry;               /* offset to TCAM entry data */
    unsigned int offsetExtTcamEntry;            /* offset to external TCAM entry data */
    unsigned int offsetDirExtEntry;             /* offset to dir ext ent data */
    unsigned int bcmFieldQualifyCount;          /* bcmFieldQualifyCount */
    unsigned int bcmFieldActionCount;           /* bcmFieldActionCount */
    unsigned int tcamBcmActionLimit;            /* TCAM entry BCM action lim */
    unsigned int ppdActionLimit;                /* PPD layer action limit */
    unsigned int ppdActionCount;                /* max PPD action value */
    unsigned int ppdQualLimit;                  /* PPD layer qualifier limit */
    unsigned int ppdQualCount;                  /* max PPD qualifier value */
    unsigned int preselLimit;                   /* preselector limit */
    unsigned int dataFieldLimit;                /* data field limit */
    unsigned int tcamEntryLimit;                /* TCAM entry limit */
    unsigned int extTcamEntryLimit;             /* external TCAM entry limit */
    unsigned int dirextEntryLimit;              /* Dirext Extraction ent lim */
    unsigned int groupLimit;                    /* group limit */
    unsigned int stageLimit;                    /* stage limit */
    unsigned int rangeLimit;                    /* range of one type limit */
    unsigned int rangeTypes;                    /* range type count */
} _bcm_dpp_field_info_wb_t;
#endif /* def BCM_WARM_BOOT_SUPPORT */
typedef struct _bcm_dpp_field_info_s {
    /* derivable from config or hardware */
    int unit;                                   /* unit ID */
    sal_mutex_t unitLock;                       /* unit lock */
    unsigned int totalSize;                     /* unit info size */
    uint32 unitHandle;                          /* PPD device ID */
    const _bcm_dpp_field_device_info_t *devInfo;/* pointer to device info */
    const int32 *qualMaps[bcmFieldQualifyCount];/* qualifier mappings */
    const int32 *actMaps[bcmFieldActionCount];  /* action mappings */
    bcm_field_qualify_t ppdQual[SOC_PPD_NOF_FP_QUAL_TYPES];/* PPD -> BCM qual map */
    ...
    shr_mres_handle_t hwResIds;                 /* hardware resources lists */
    ...
    /* some parts config/hardware, others need to be in backing store */
    _bcm_dpp_field_stage_t *stageD;             /* pointer to stage data */
    _bcm_dpp_field_group_t *groupD;             /* pointer to group table */
    _bcm_dpp_field_entry_t *entryTc;            /* pointer to internal entry table */
    _bcm_dpp_field_entry_ext_t *entryExtTc;     /* pointer to external entry table */
    _bcm_dpp_field_entry_ext_info_t *extTcamInfo; /* pointer to external TCAM info table */
    _bcm_dpp_field_entry_dir_ext_t *entryDe;    /* ptr to dir ext entry tbl */
    SOC_SAND_OCC_BM_PTR extTcamBmp;             /* pointer to occupation bitmap for external TCAM info table */
    /* must be kept in backing store */
    uint32 unitFlags;                           /* unit flags */
    SHR_BITDCL **rangeInUse;                    /* range in-use information */
    _bcm_dpp_field_datafield_bits dataFieldInUse; /* programmable fields in use */
    _bcm_dpp_field_datafield_bits dataFieldOfsBit; /* offset is bit resolution */
    _bcm_dpp_field_datafield_bits dataFieldLenBit; /* length is bit resolution */
...
#ifdef BCM_WARM_BOOT_SUPPORT
    _bcm_dpp_field_info_wb_t wb;                /* warm boot information */
#endif /* def BCM_WARM_BOOT_SUPPORT */
} _bcm_dpp_field_info_t;
extern _bcm_dpp_field_info_t *_bcm_dpp_field_unit_info[BCM_MAX_NUM_UNITS];
...
/*-----------------------------------------------------------------------------
 *
 *  The struct definition below is used in _bcm_arad_field_qual_info decleration
 *  table in field_hwconfig.c. Please refer to comments of the table's decleration
 *  for further explenations and clarifications.
 */
typedef struct _bcm_arad_field_device_qual_info_layer_s {
    ...
    int32 i_l2_a;
    int32 i_l2_b;     /* chain 0,1 of map 0 */
    int32 i_ipv4_a;
    int32 i_ipv4_b; /* chain 0,1 of map 1 */
    int32 i_ipv6_a;
    int32 i_ipv6_b; /* chain 0,1 of map 2 */
    int32 i_mpls_a;
    int32 i_mpls_b; /* chain 0,1 of map 3 */
    int32 i_psel_a;
    int32 i_psel_b; /* chain 0,1 of map 4 */
    int32 f_l2_a;
    int32 f_l2_b;     /* chain 0,1 of map 0 */
    int32 f_ipv4_a;
    int32 f_ipv4_b; /* chain 0,1 of map 1 */
    int32 f_ipv6_a;
    int32 f_ipv6_b; /* chain 0,1 of map 2 */
    int32 f_mpls_a;
    int32 f_mpls_b; /* chain 0,1 of map 3 */
    int32 f_psel_a;
    int32 f_psel_b; /* chain 0,1 of map 4 */
    int32 s_l2_a;
    int32 s_l2_b;     /* chain 0,1 of map 0 */
    int32 s_ipv4_a;
    int32 s_ipv4_b; /* chain 0,1 of map 1 */
    int32 s_ipv6_a;
    int32 s_ipv6_b; /* chain 0,1 of map 2 */
    int32 s_mpls_a;
    int32 s_mpls_b; /* chain 0,1 of map 3 */
    int32 s_psel_a;
    int32 s_psel_b; /* chain 0,1 of map 4 */
    int32 e_l2_a;
    int32 e_l2_b;     /* chain 0,1 of map 5 */
    int32 e_ipv4_a;
    int32 e_ipv4_b; /* chain 0,1 of map 6 */
    int32 e_ipv6_a;
    int32 e_ipv6_b; /* chain 0,1 of map 7 */
    int32 e_mpls_a;
    int32 e_mpls_b; /* chain 0,1 of map 8 */
    int32 e_psel_a;
    int32 e_psel_b; /* chain 0,1 of map 9 */
 ...
 *
 *  The struct definition below is used in _bcm_arad_field_action_info Arad mapping decleration
 *  table in field_hwconfig.c. Please refer to comments of the table's decleration
 *  for further explenations and clarifications.
 */
...
extern _bcm_dpp_field_device_info_t _bcm_petra_field_device_info;
...
#define _DPP_FIELD_COMMON_LOCALS \
    _bcm_dpp_field_info_t *unitData;          /* unit information */ \
...
#define _DPP_FIELD_UNIT_CHECK(_unit, _unitData) \
    ...
    if (!(_unitData)) { \
        BCMDNX_ERR_EXIT_MSG(BCM_E_INIT, (_BSL_BCM_MSG("uninitialised"))); \
    }
#define _DPP_FIELD_UNIT_LOCK(_unitData) \
    if (sal_mutex_take((_unitData)->unitLock, sal_mutex_FOREVER)) { \
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("unable to take lock"))); \
    }
#define _DPP_FIELD_UNIT_UNLOCK(_unitData) \
    if (sal_mutex_give((_unitData)->unitLock)) { \
 ...
 *      (in) entry = entry ID
 *      (out) entryCommon = where to put pointer to common data for entry
 *      (out) entryExtCommon = where to put pointer to external common data for entry 
 ...
 */
extern int
_bcm_dpp_field_entry_qualifier_get_single(_bcm_dpp_field_info_t *unitData,
                                          bcm_field_entry_t entry,
 ...
 */
extern int
_bcm_dpp_field_entry_qualifier_get(_bcm_dpp_field_info_t *unitData,
                                   bcm_field_entry_t entry,
                                   bcm_field_qualify_t type,
                                   int count,
                                   uint64 *data,
 ...
 *  Purpose
 *    Translate BCM HeaderFormat to PPD PKT_HDR_STK_TYPE
 *  Parameters
 *    (in) header_format = BCM header format
 *    (out) hdr_stk_type = PPD header stack type
 *  Returns
 *    bcm_error_t (cast as int)
...
/*
 *  Function
 *    _bcm_dpp_field_HeaderFormatSet_bcm_to_ppd
 *  Purpose
 *    Translate a BCM layer field header set to PPD header bitmap
 *  Arguments
 *    (in) bcm_fhs = pointer to BCM field header set
 *    (out) soc_petra_fhs = pointer to buffer for Soc_petra field bitmap
 *  Returns
 *      int (implied cast from bcm_error_t)
 *                    BCM_E_NONE if sccessful
 *                    BCM_E_* appropriately otherwise
...
/*
 *  Function
 *    _bcm_dpp_field_HeaderFormatSet_ppd_to_bcm
 *  Purpose
 *    Translate a PPD header bitmap to a BCM layer field header set
 *  Arguments
 *    (in) soc_petra_hfs = pointer Soc_petra field bitmap
 *    (out) bcm_hfs = pointer to BCM field header set
 *  Returns
 *      int (implied cast from bcm_error_t)
 *                    BCM_E_NONE if sccessful
 *                    BCM_E_* appropriately otherwise
...
/*
 *  Function
 *    _bcm_dpp_field_HeaderFormatSet_ppd_member_to_HeaderFormat_ppd
 *  Purpose
 *    Translate a single bit from a PPD headerFormatSet to a PPD headerFormat
 *  Arguments
 *    (in) soc_petra_hfs = PPD layer header format set
 *    (out) hf = pointer to where to put PPD header format
 *  Returns
 *      int (implied cast from bcm_error_t)
 *                    BCM_E_NONE if sccessful
 *                    BCM_E_* appropriately otherwise
 *  Notes
 *    Only translates a single bit at a time
 */
extern int
_bcm_dpp_field_HeaderFormatSet_ppd_member_to_HeaderFormat_ppd(const uint32 soc_petra_hfs,
 ...
 *      mention the trouble of initialising such a table).
 */
extern int
 ...
 *     qualifier is not 'special', then it passes the responsbility back to the
 ...
 */
extern int
_bcm_petra_field_data_qualifier_init(_bcm_dpp_field_info_t *unitData);
 ...
 */
extern int
_bcm_petra_field_data_qualifier_dump(_bcm_dpp_field_info_t *unitData,
                                     const char *prefix);
#endif /* def BROADCOM_DEBUG */
 ...
 *  Parameters
 *    (in) _bcm_dpp_field_info_t *unitData = pointer to unit data
 *    (in) SOC_PPD_FP_QUAL_TYPE hwType = PPD layer qualifier type
 *    (out) unsigned int *bcmIndex = where to put BCM layer index
 */
extern int
_bcm_petra_field_map_ppd_udf_to_bcm(_bcm_dpp_field_info_t *unitData,
                                    SOC_PPD_FP_QUAL_TYPE hwType,
 ...
 */
extern int
_bcm_petra_field_map_bcm_udf_to_ppd(_bcm_dpp_field_info_t *unitData,
                                    unsigned int bcmIndex,
                                    SOC_PPD_FP_QUAL_TYPE *hwType);
 ...
 */
extern int
_bcm_petra_field_data_qualifier_bits_get(_bcm_dpp_field_info_t *unitData,
                                         SOC_PPD_FP_QUAL_TYPE hwType,
                                         unsigned int *bits);
 ...
 */
extern int
_bcm_dpp_field_range_init(_bcm_dpp_field_info_t *unitData);
 ...
 */
extern int
_bcm_dpp_field_range_dump(_bcm_dpp_field_info_t *unitData,
                            const char *prefix);
#endif /* def BROADCOM_DEBUG */
 ...
 *      (in) SOC_PPD_FP_QUAL_VAL *qualHwData = qualifier array from hardware
 *      (in) char prefix = prefix for each line of the dump
 *   Returns
 *      int (implied cast from bcm_error_t)
 *                    BCM_E_NONE if sccessful
 ...
 */
extern int
_bcm_dpp_field_entry_range_dump(_bcm_dpp_field_info_t *unitData,
 ...
 *        BCM_E_* appropriately if not
 *  Notes
 *     If the entry is alreay in a group, it will remove the entry from that
 ...
 *
 *      This behaviour implies a peculiar result when dealing with uncommitted
 ...
 *        BCM_E_* appropriately if not
 *  Notes
 *    Might return an error for mismatch or just dipslay a warning, according
 *    to configuration.
 *
 ...
 */
extern int
_bcm_petra_field_data_qualifier_get(_bcm_dpp_field_info_t *unitData,
                                    int qual_id,
