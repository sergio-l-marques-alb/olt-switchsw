/*
 * $Id: wb_engine.h,v 1.4 Broadcom SDK $
...
#define SOC_WB_ENGINE_ADD_DYNAMIC_BUFF(_engine_id, _buff, _buff_string, _upgrade_func, _version, _is_dynamic, _is_only_copy, _is_added_after_release)\
                     if (_buff == buffer_id) {\
...
#define SOC_WB_ENGINE_ADD_BUFF(_engine_id, _buff, _buff_string, _upgrade_func, _version, _is_only_copy, _is_added_after_release)\
    SOC_WB_ENGINE_ADD_DYNAMIC_BUFF(_engine_id, _buff, _buff_string, _upgrade_func, _version, 0 /*not dynamic*/, _is_only_copy, _is_added_after_release)
