...
typedef enum
{
  ...
  ARAD_EM_TABLE_ID_LAST = 8 /* do not change!! need to change also in wb engine and add ISSU treatment */
...
#define CHIP_SIM_LEM_KEY_SIZE     (SOC_DPP_LEM_WIDTH_PETRAB + SOC_SAND_NOF_BITS_IN_BYTE - 1)/SOC_SAND_NOF_BITS_IN_BYTE
#define CHIP_SIM_LEM_DATA_SIZE    (SOC_PB_PP_LEM_ACCESS_PAYLOAD_NOF_BITS + SOC_SAND_NOF_BITS_IN_BYTE - 1)/SOC_SAND_NOF_BITS_IN_BYTE
#define CHIP_SIM_LEM_TABLE_SIZE   (SOC_PB_PP_FRWRD_MACT_TABLE_NOF_ENTRIES)  

#define SOC_PB_CHIP_SIM_TABLE_KEY_MAX_SIZE       SOC_SAND_MAX(SOC_SAND_MAX(CHIP_SIM_SVEM_KEY_SIZE, CHIP_SIM_LEM_KEY_SIZE), SOC_SAND_MAX(CHIP_SIM_ISEM_KEY_SIZE, CHIP_SIM_ESEM_KEY_SIZE))
#define SOC_PB_CHIP_SIM_TABLE_DATA_MAX_SIZE      SOC_SAND_MAX(SOC_SAND_MAX(CHIP_SIM_SVEM_DATA_SIZE, CHIP_SIM_LEM_DATA_SIZE), SOC_SAND_MAX(CHIP_SIM_ISEM_DATA_SIZE, CHIP_SIM_ESEM_DATA_SIZE))
...
extern CHIP_SIM_EM_BLOCK Arad_em_blocks[SOC_MAX_NUM_DEVICES][ARAD_EM_TABLE_ID_LAST + 1];
...
void chip_sim_em_free(uint32 unit);

uint32 chip_sim_em_init(uint32 device_id, SOC_SAND_DEVICE_TYPE device_type);
