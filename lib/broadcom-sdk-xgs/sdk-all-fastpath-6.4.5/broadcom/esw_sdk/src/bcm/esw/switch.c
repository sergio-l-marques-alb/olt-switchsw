...
#define _BCM_SYNC_MODULE(unit, mod, rtn, rv) {                \
            rv = rtn(unit);                                   \
...
_bcm_switch_control_sync(int unit, int arg)
{
...
#endif /* BCM_SHADOW_SUPPORT */
    {
        ...
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_VLAN, _bcm_esw_vlan_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_STG, _bcm_esw_stg_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_COSQ,_bcm_esw_cosq_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_MIRROR, _bcm_esw_mirror_sync, rv);

#ifdef BCM_TRIUMPH2_SUPPORT
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_UDF, _bcm_esw_udf_scache_sync, rv);
#endif /* BCM_TRIUMPH2_SUPPORT */
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_FIELD, 
                         _bcm_esw_field_scache_sync, rv);
        if (rv == BCM_E_UNAVAIL) {
             rv = BCM_E_NONE;
        }
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_RX, _bcm_esw_rx_sync, rv);
        ...
        if (SOC_IS_TRX(unit) && !(SOC_IS_TITAN(unit) || SOC_IS_TITAN2(unit))) {
            if (soc_feature(unit, soc_feature_virtual_switching)) {
                _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_COMMON,
                                 _bcm_esw_virtual_sync, rv);
     
            }
        }
#endif /* BCM_TRX_SUPPORT */
         _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_L3, _bcm_esw_l3_sync, rv);

         _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_IPMC, _bcm_esw_ipmc_sync, rv);

         if (soc_feature(unit, soc_feature_mpls)) {
             _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_MPLS, _bcm_esw_mpls_sync, rv);
         }

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_MULTICAST, 
                         _bcm_esw_multicast_sync, rv);

#endif /* INCLUDE_L3 */

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_TRUNK, _bcm_esw_trunk_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_LINKSCAN, _bcm_esw_link_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_PORT, _bcm_esw_port_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_OAM, _bcm_esw_oam_sync, rv);

#if defined(INCLUDE_BFD)
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_BFD, _bcm_esw_bfd_sync, rv);
#endif
        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_QOS, _bcm_esw_qos_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_STK, _bcm_esw_stk_sync, rv);
        ...
        if (SOC_IS_TRIDENT(unit) || SOC_IS_TRIDENT2X(unit) ||
            ...
            if (soc_feature(unit, soc_feature_trill)) {
                _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_TRILL, _bcm_esw_trill_sync, rv);
            }
        }
#endif /* BCM_TRIDENT_SUPPORT */
        ...
        if (soc_feature(unit, soc_feature_vxlan)) {
            _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_VXLAN, _bcm_esw_vxlan_sync, rv);
        }
#endif /* BCM_TRIDENT2_SUPPORT */

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_NIV, _bcm_esw_niv_sync, rv);

        _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_SUBPORT, _bcm_esw_subport_sync, rv);
        ...
        if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
            SOC_IS_TD2_TT2(unit)) {
             _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_GLB_METER,
                              _bcm_esw_global_meter_policer_sync, rv);
        }

        if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
            SOC_IS_TD2_TT2(unit)) {
            _BCM_SYNC_MODULE(unit, SHR_BPROF_BCM_STAT, _bcm_esw_stat_sync, rv);
        }
    ...
    if (_BCM_SYNC_SUCCESS(rv) &&
        ...
        (SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIUMPH3(unit) || SOC_IS_TOMAHAWK(unit))) {
                          ...
                          bcm_switch_control_t type, int arg)
{
    ...
    if (SOC_IS_TD_TT(unit) || SOC_IS_TRIUMPH3(unit)) {
        ...
        sub_field_width[1] = 16;  /* HASH_B0 (16bit)  */
        sub_field_width[2] = 4;   /* LBN (4bit)       */
        sub_field_width[3] = 5;   /* MH.DPORT (5 bit) */
...
#if defined(BCM_TRIDENT_SUPPORT)
    { bcmSwitchL3InterfaceUrpfEnable, TD | TR3 | HX4 | KT | KT2 | TD2 | TD2P | TH,
...
defined(BCM_TRIDENT_SUPPORT)
    ...
    { bcmSwitchIpmcSameVlanPruning, TD,
        ...
        _bool_invert, 0 },
    { bcmSwitchHashNivSrcIfEtagSvidSelect0, TD | TR3 | HX4 | KT | TD2 | TD2P | TH,
        RTAG7_HASH_CONTROL_2r, RTAG7_HASH_BIN_0_SELECT_Af,
        NULL, 0 },
    { bcmSwitchHashNivSrcIfEtagSvidSelect1, TD | TR3 | HX4 | KT | TD2 | TD2P | TH,
        RTAG7_HASH_CONTROL_2r, RTAG7_HASH_BIN_0_SELECT_Bf,
        NULL, 0 },
    { bcmSwitchHashNivDstIfEtagVidSelect0, TD | TR3 | HX4 | KT | TD2 | TD2P | TH,
        RTAG7_HASH_CONTROL_2r, RTAG7_HASH_BIN_1_SELECT_Af,
        NULL, 0 },
    { bcmSwitchHashNivDstIfEtagVidSelect1, TD | TR3 | HX4 | KT | TD2 | TD2P | TH,
    ...
    { bcmSwitchMeterAdjust, TRVL,
        ...
        NULL, soc_feature_meter_adjust },
    { bcmSwitchMeterAdjust, AP | TR2 | TD | TR3 | HX4 | VL2 | TD2 | TD2P | TH,
    ...
    { bcmSwitchPrioDropToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, DOT1P_ADMITTANCE_DROP_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivPrioDropToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, DOT1P_ADMITTANCE_DROP_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivInterfaceMissToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, NIV_FORWARDING_DROP_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivRpfFailToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, NIV_RPF_CHECK_FAIL_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivTagInvalidToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, VNTAG_FORMAT_DROP_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivTagDropToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, DISCARD_VNTAG_PRESENT_TOCPUf,
        NULL, soc_feature_niv},
    { bcmSwitchNivUntagDropToCpu, TD | TR3 | HX4 | TD2 | TD2P | TH,
...
#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    { bcmSwitchExtenderInterfaceMissToCpu, TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, NIV_FORWARDING_DROP_TOCPUf,
        NULL, soc_feature_port_extension},
    { bcmSwitchExtenderRpfFailToCpu, TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, NIV_RPF_CHECK_FAIL_TOCPUf,
        NULL, soc_feature_port_extension},
    { bcmSwitchEtagInvalidToCpu, TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, VNTAG_FORMAT_DROP_TOCPUf,
        NULL, soc_feature_port_extension},
    { bcmSwitchEtagDropToCpu, TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, DISCARD_VNTAG_PRESENT_TOCPUf,
        NULL, soc_feature_port_extension},
    { bcmSwitchNonEtagDropToCpu, TR3 | HX4 | TD2 | TD2P | TH,
        CPU_CONTROL_0r, DISCARD_VNTAG_NOT_PRESENT_TOCPUf,
        NULL, soc_feature_port_extension},
    ...
    { bcmSwitchMplsIllegalReservedLabelToCpu, TR3 | HX4 | TD2 | TD2P | TH | KT2,
        ...
        NULL, 0},
#endif /* BCM_TRIUMPH3_SUPPORT || BCM_TRIDENT2_SUPPORT || BCM_KATANA2_SUPPORT */
    ...
    { bcmSwitchHashField1Ip6FlowLabel, TD | TR3 | HX4 | TD2 | TD2P | TH,
       RTAG7_HASH_CONTROL_2r, ENABLE_FLOW_LABEL_IPV6_Bf,
        NULL, 0},
    { bcmSwitchMacroFlowHashUseMSB, TD | TR3 | HX4 | TD2 | TH,
       RTAG7_HASH_CONTROL_2r, MACRO_FLOW_HASH_BYTE_SELf,
        NULL, 0},
#endif
#if defined(BCM_TRIDENT_SUPPORT) || defined(BCM_KATANA_SUPPORT)
    { bcmSwitchHashHg2PktFieldsEnable, TD | TR3 | HX4 | KT | TD2 | TD2P | TH,
...
#endif
#if defined(BCM_TOMAHAWK_SUPPORT)
    { bcmSwitchSampleFlexRandomSeed, TH,
        SFLOW_ING_FLEX_RAND_SEEDr, SEEDf,
        NULL, 0}
#endif /* BCM_TOMAHAWK_SUPPORT */
};
#endif /* BCM_XGS3_SWITCH_SUPPORT */
...
{
    if (SOC_IS_TOMAHAWK(unit)) {
        return soc_th_latency_wb_memsz_get(unit, mem_sz);
    } else {
        *mem_sz = 0;
        return BCM_E_NONE;
    }
...
{
    if (SOC_IS_TOMAHAWK(unit)) {
        return soc_th_latency_wb_recover(unit, wb_data);
    } else {
        return BCM_E_NONE;
    } 
}
