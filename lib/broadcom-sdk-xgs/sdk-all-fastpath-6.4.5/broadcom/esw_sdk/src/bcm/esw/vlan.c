...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
                if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
                    rv = bcm_td2p_update_vlan_pbmp(unit, vid,
                            &l2_l3_pbmp); 
                } else
#endif
                {
...
bcm_esw_vlan_port_remove(int unit, bcm_vlan_t vid, pbmp_t pbmp)
{
    ...
    rv = _bcm_vlan_port_remove(unit, vid, pbmp, NULL);
                ...
                if (BCM_GPORT_IS_VXLAN_PORT(port)) {                
                    BCM_IF_ERROR_RETURN
                        (_bcm_esw_dvp_vtkey_type_value_get(unit, arg, 
                                                    &key_type_value));
                    return (bcm_td2p_vxlan_dvp_vlan_xlate_key_set(unit, port, 
                                                             key_type_value));
 ...
 || defined(BCM_TRX_SUPPORT)
        ...
        case bcmVlanPortTranslateEgressKey:
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_xlate_key_on_dvp)) {
                ...
                if (BCM_GPORT_IS_VXLAN_PORT(port)) {
                    BCM_IF_ERROR_RETURN
                        (bcm_td2p_vxlan_dvp_vlan_xlate_key_get(unit, port, 
                                                         &key_type_value));  
    ...
    if (soc_feature(unit, soc_feature_wlan) && BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
            if ((flags & BCM_VLAN_PORT_INGRESS_ONLY)) {
                ...
                BCM_IF_ERROR_RETURN(rv); 
                rv = bcm_td2p_vlan_vp_group_set(unit, VLAN_TABm,
                        vlan, BCM_GPORT_VP_GROUP_GET(port),TRUE);
                BCM_IF_ERROR_RETURN(rv); 
            }
            if ((flags & BCM_VLAN_PORT_EGRESS_ONLY)) {
                rv = bcm_td2p_egr_vp_group_unmanaged_set(unit,TRUE);
                BCM_IF_ERROR_RETURN(rv); 
                rv = bcm_td2p_vlan_vp_group_set(unit, EGR_VLANm,
                        vlan, BCM_GPORT_VP_GROUP_GET(port),TRUE);
                BCM_IF_ERROR_RETURN(rv); 
            }
        } else
...
bcm_esw_vlan_gport_delete(int unit, bcm_vlan_t vlan, bcm_gport_t port)
{
    ...
    if (soc_feature(unit, soc_feature_wlan) && BCM_GPORT_IS_WLAN_PORT(port)) {
        ...
        if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
            rv = bcm_td2p_vlan_vp_group_set(unit, VLAN_TABm,
                    vlan, BCM_GPORT_VP_GROUP_GET(port), FALSE);
            BCM_IF_ERROR_RETURN(rv);
                                      ...
                                      bcm_gport_t port, int flags)
{
...
#endif /* BCM_TRIUMPH2_SUPPORT && INCLUDE_L3 */
    {
        ...
        rv = _bcm_vlan_port_remove(unit, vlan, pbmp, group_types);
    }
    return rv;
}
        ...
        if (soc_mem_field32_get(unit, VLAN_TABm, &vtab, VIRTUAL_PORT_ENf)) {
            ...
            if (soc_feature(unit, soc_feature_vlan_vfi_membership)){
                if (bcm_td2p_ing_vp_group_unmanaged_get(unit)) {
                    ...
                    rv = bcm_td2p_vlan_vp_group_set(unit,
                            VLAN_TABm, vlan, -1, FALSE);
                    if (rv != BCM_E_NONE) {
                        soc_mem_unlock(unit, VLAN_TABm);
                        return rv;
                    }
