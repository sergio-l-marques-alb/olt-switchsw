...
#endif /* defined(BCM_TIME_V3_SUPPORT) || defined(BCM_KATANA2_SUPPORT) ||
          defined(BCM_HELIX4_SUPPORT) || defined(BCM_HURRICANE2_SUPPORT) ||
          defined(BCM_GREYHOUND_SUPPORT) || defined(BCM_SABER2_SUPPORT) */
...
#endif /* defined(BCM_TIME_V3_SUPPORT) || defined(BCM_KATANA2_SUPPORT) ||
          defined(BCM_HELIX4_SUPPORT) || defined(BCM_HURRICANE2_SUPPORT) ||
          defined(GREYHOUND_SUPPORT) || defined(BCM_SABER2_SUPPORT) */
#if defined(BCM_KATANA_SUPPORT) || defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
        SOC_IS_TD2_TT2(unit)) {
...
#endif /* defined(BCM_TIME_V3_SUPPORT) || defined(BCM_KATANA_SUPPORT) ||
          defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT) ||
          defined(BCM_HURRICANE2_SUPPORT) || defined(BCM_GREYHOUND_SUPPORT) ||
          defined(BCM_SABER2_SUPPORT) */
...
bcm_esw_time_interface_add (int unit, bcm_time_interface_t *intf)
{
    ...
    if (intf->flags & BCM_TIME_SYNC_STAMPER) {
        ...
        if (_bcm_ptp_running(unit) == BCM_E_NONE) {
            ...
            if (SOC_IS_KATANA(unit) || SOC_IS_TD2_TT2(unit) || SOC_IS_TRIUMPH3(unit)) {
                /* CMICM */
                if (needs_restart) {
            ...
            } else if (SOC_IS_KATANA2(unit) || SOC_IS_HELIX4(unit) ||
                       SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) ||
                       SOC_IS_SABER2(unit)) {
                /* IPROC / CMICD */
                if (needs_restart) {
                    soc_reg_field32_modify(unit, CMIC_BS0_CONFIGr, REG_PORT_ANY, BS_TC_OUTPUT_ENABLEf, 0);
                    /* use "lock" field as a signal on startup that the servo offset should be included */
                    soc_reg_field32_modify(unit, CMIC_BS0_OUTPUT_TIME_0r, REG_PORT_ANY, LOCKf, include_servo_offset);

                    sal_usleep(two_cycle_delay_usec);

                    soc_reg_field32_modify(unit, CMIC_BS0_CONFIGr, REG_PORT_ANY, BS_TC_OUTPUT_ENABLEf, tc_output_enable);

                    sal_usleep(two_cycle_delay_usec);
                }
                soc_reg_field32_modify(unit, CMIC_BS0_OUTPUT_TIME_0r, REG_PORT_ANY, LOCKf, locked);
            }
    ...
    if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) ||
     ...
     defined(BCM_TOMAHAWK_SUPPORT) || defined(BCM_HURRICANE2_SUPPORT) || \
     defined(BCM_GREYHOUND_SUPPORT) || defined(BCM_SABER2_SUPPORT))
    if (SOC_IS_KATANA2(unit) || SOC_IS_HELIX4(unit) ||
        SOC_IS_TOMAHAWK(unit) || SOC_IS_HURRICANE2(unit) ||
        SOC_IS_GREYHOUND(unit) || SOC_IS_SABER2(unit)) {
...
#if defined(BCM_TIME_V3_SUPPORT) || \
    defined(BCM_HELIX4_SUPPORT) || defined(BCM_KATANA2_SUPPORT) || \
    defined(BCM_HURRICANE2_SUPPORT) || defined(BCM_GREYHOUND_SUPPORT) || \
    defined(BCM_SABER2_SUPPORT)
    if (soc_feature(unit, soc_feature_time_v3) ||
...
#endif /* defined(BCM_TIME_V3_SUPPORT) || defined(BCM_HELIX4_SUPPORT) ||
...
/*****************************************************************************/
    if (soc_feature(unit, soc_feature_time_v3) ||
        ...
        if (SOC_IS_HELIX4(unit)) {
        ...
        } else if (SOC_IS_GREYHOUND(unit) || SOC_IS_SABER2(unit)) {
            /* Feature unavailable on devices without BroadSync support. */
            if (soc_feature(unit, soc_feature_time_v3_no_bs)) {
                return BCM_E_UNAVAIL;
            }
            ...
            /*
             * NDIV 10-bit integer part.
             * NB: This calculation appears unused for IPROC logic herein.
             */
            READ_TOP_BROAD_SYNC0_LCPLL_FBDIV_CTRL_1r(unit, &regval);
            bs_ndiv_int = ((soc_reg_field_get(unit, TOP_BROAD_SYNC0_LCPLL_FBDIV_CTRL_1r,
                regval, BROAD_SYNC0_LCPLL_FBDIV_1f) & 0xffc0) >> 6);            
        } else {
            /* KATANA2, HURRICANE2 */
            READ_TOP_BROAD_SYNC0_LCPLL_FBDIV_CTRL_1r(unit, &regval);
            bs_ndiv_int = ((soc_reg_field_get(unit, TOP_BROAD_SYNC0_LCPLL_FBDIV_CTRL_1r,
                regval, BROAD_SYNC0_LCPLL_FBDIV_1f) & 0xff00) >> 8);
        }
