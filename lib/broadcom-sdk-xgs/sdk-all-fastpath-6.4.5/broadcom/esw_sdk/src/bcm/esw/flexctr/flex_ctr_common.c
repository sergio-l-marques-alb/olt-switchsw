...
#define _CTR_COUNTER_TABLE_IFP(_unit, _dir, _pipe_num, _pool, _mem) {        \
    if (!SOC_IS_TOMAHAWK(unit) || _bcm_esw_get_ifp_mode_global(_unit) ||    \
...
#define BCM_STAT_WB_DEFAULT_VERSION            BCM_WB_VERSION_1_3
...
/*
 * Function:
 *     _bcm_esw_get_ifp_mode_global 
 * Description:
 *      Helper function to return 1 if configured in Global mode
 */
#if defined (BCM_TOMAHAWK_SUPPORT)
int
_bcm_esw_get_ifp_mode_global(int unit) {
...
static int
_bcm_esw_get_ifp_pipe_num(int unit, soc_mem_t ingress_table)
{
    int pipe_num = 0;

    if (!_bcm_esw_get_ifp_mode_global(unit)) {
         if(ingress_table == IFP_POLICY_TABLE_PIPE0m) {
             pipe_num = 0;
         } else if(ingress_table == IFP_POLICY_TABLE_PIPE1m) {
             pipe_num = 1;
         } else if(ingress_table == IFP_POLICY_TABLE_PIPE2m) {
             pipe_num = 2;
         } else if(ingress_table == IFP_POLICY_TABLE_PIPE3m) {
             pipe_num = 3;
         }
    }
    return pipe_num; 
...
bcm_error_t
_bcm_esw_get_ifp_pipe_pool_from_mode(int unit, int offset_mode, int *pipe, int *pool) {
    uint32 hint_val = 0;
...
bcm_error_t
_bcm_esw_get_ifp_pipe_from_mode(int unit, int offset_mode, int *pipe) {
    bcm_error_t rv = BCM_E_CONFIG;
    int pool = 0;
    *pipe = 0;
    rv = _bcm_esw_get_ifp_pipe_pool_from_mode(unit, offset_mode, pipe, &pool);
    return rv;
}
#endif
 ...
 */
static
bcm_error_t _bcm_esw_stat_flex_install_stat_id(int unit,uint32  *scache_ptr, uint32 *per_pipe_mode)
{
    uint32                    index=0;
    ...
    *per_pipe_mode = 0;
    ...
    if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_3)) {
       ...
       if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_4)) {
           ...
           *per_pipe_mode = *(uint32 *)flex_custom_mode_related_info;
       }
    } else {
        LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                    (BSL_META_U(unit,
...
#if defined(BCM_TOMAHAWK_SUPPORT)
                  _bcm_esw_get_ifp_pipe_from_mode(unit, offset_mode, &pipe_num);
#endif
       ...
       ingress_entry_data = soc_mem_table_idx_to_pointer(unit, ingress_table,
        ...
        if (SOC_IS_TOMAHAWK(unit)) {
            if ((IFP_POLICY_TABLEm == ingress_table) ||
...
#if 0
                flex_ingress_modes[unit][offset_mode].hint.type =
                                             bcmStatGroupAllocHintIFP;
                flex_ingress_modes[unit][offset_mode].hint.value = 
                                             ((pipe_num << 16) | pool_number);
#endif
                 ...
                 */
                _bcm_esw_stat_get_counter_id(unit, bcmStatGroupModeFlex1,
...
bcm_error_t _bcm_esw_stat_flex_sync(int unit)
{
    ...
    if (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_4) {
        flex_custom_mode_related_info = (void *)flex_group_mode_related_info;
        for (mode=BCM_CUSTOM_INGRESS_MODE_START; 
           (mode < BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE) ; mode++) {
             if (flex_custom_ingress_modes[unit][mode].used != 0) {
                 ...
                 flex_custom_mode_related_info->offset_mode = 
                             flex_custom_ingress_modes[unit][mode].offset_mode;

                 flex_custom_mode_related_info->AllocPool = 0;
                 flex_custom_mode_related_info->fg_id = 0xffffffff;
                 if (flex_custom_ingress_modes[unit][mode].hint.type == 
                                     bcmStatGroupAllocHintIngressFieldGroup) {
                     flex_custom_mode_related_info->AllocPool = 
                              flex_custom_ingress_modes[unit][mode].AllocPool;
                     flex_custom_mode_related_info->fg_id = 
                              flex_custom_ingress_modes[unit][mode].hint.value;
                 }
                 flex_custom_mode_related_info++;
             }
        }
#ifdef BCM_TOMAHAWK_SUPPORT
        per_pipe_mode = (uint32 *)flex_custom_mode_related_info;
        *per_pipe_mode = !_bcm_esw_get_ifp_mode_global(unit);
#endif
    }
                                            ...
                                            bcm_stat_flex_direction_t direction)
{
    ...
    for (pool_id=0;pool_id<num_pools[direction];pool_id++) {
         ...
         SHR_BITCLR_RANGE(flex_pool_stat[unit][direction][pipe_num][pool_id].used_by_objects, 0, bcmStatObjectMaxValue);
...
bcm_error_t _bcm_esw_stat_flex_init(int unit)
{
    ...
    if (IsAnyModeConfigured) {
        ...
        /* 10) Ingress: IFP_POLICY_TABLE Table */
            _bcm_esw_stat_flex_check_ingress_ifp_policy_table(unit, per_pipe_mode);
        }
#endif
#endif
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_STAT_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_4) {
            local_scache_size +=  sizeof(bcm_stat_flex_custom_mode_related_info_t) *
                                  (BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE - 
                                   BCM_CUSTOM_INGRESS_MODE_START) + sizeof(uint32);
        }
        LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                  (BSL_META_U(unit,
    ...
    if (SOC_WARM_BOOT(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_esw_stat_flex_install_stat_id(
                                     unit,flex_scache_ptr[unit], &per_pipe_mode));
#if defined(BCM_TOMAHAWK_SUPPORT)
            ...
            bcm_stat_value_t *value)
{
    ...
    case IFP_POLICY_TABLE_PIPE3m:
         _bcm_esw_get_ifp_pipe_num(unit, table);
#endif
         direction= bcmStatFlexDirectionIngress;
         break;
    default:
            ...
            bcm_stat_value_t *value)
{
    ...
    case IFP_POLICY_TABLE_PIPE2m:
    case IFP_POLICY_TABLE_PIPE3m:
         pipe_num = _bcm_esw_get_ifp_pipe_num(unit, table);
#endif
    case IFP_POLICY_TABLEm:
         direction= bcmStatFlexDirectionIngress;
         break;
            ...
            bcm_stat_value_t *value)
{
    ...
    if ((offset_mode >= BCM_CUSTOM_INGRESS_MODE_START) &&
        (offset_mode < BCM_STAT_FLEX_CUSTOM_INGRESS_COUNTER_MAX_MODE)) {
        offset_mode = flex_custom_ingress_modes[unit][offset_mode - BCM_CUSTOM_INGRESS_MODE_START].
                                                offset_mode;
    }
    _bcm_esw_get_ifp_pipe_from_mode(unit, offset_mode, &pipe_num);
#endif
...
void tomahawk_stat_flex_callback (int unit)
{
    ...
    for (direction = bcmStatFlexDirectionIngress;
         ...
         for (pool_id = 0; pool_id < num_pools[direction]; pool_id++) {
             maxpipe = 1;
             if (pipe_local_mode && (direction == bcmStatFlexDirectionIngress) && 
				           (FLEX_COUNTER_POOL_USED_BY_IFP_POLICY_TABLE ==
                           flex_pool_stat[unit][bcmStatFlexDirectionIngress]
                                         [0][pool_id].used_by_tables)) {
                   ...
                   if (!COMPILER_64_IS_ZERO(packet64_count)) {
                       ...
                       LOG_VERBOSE(BSL_LS_BCM_FLEXCTR,
                                   (BSL_META_U(unit,
                                    ...
                                    COMPILER_64_HI(flex_packet64_counter[unit]
                                    [pipe_num][direction][pool_id][index]),
            ...
            uint32    index)
{
...
#if defined (BCM_TOMAHAWK_SUPPORT)
    pipe_num = _bcm_esw_get_ifp_pipe_num(unit, ingress_table);
#endif
    ingress_entry_data_size = WORDS2BYTES(BYTES2WORDS(
                              SOC_MEM_INFO(unit, ingress_table).bytes));
    LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
    ...
    if (soc_mem_read(unit, ingress_table, MEM_BLOCK_ANY,
       ...
       LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
        ...
        /* Disable flex pool for this stat on no flex entries */
        if (flex_pool_stat[unit][pipe_num][bcmStatFlexDirectionIngress][pool_number].
                           attached_entries == 0) {
            _bcm_esw_stat_flex_enable_pool(
                     unit,
                     bcmStatFlexDirectionIngress,
...
#if defined (BCM_TOMAHAWK_SUPPORT)
    _bcm_esw_get_ifp_pipe_from_mode(unit, mode, &pipe_num);
#endif
    /* Decrement reference count */
    BCM_STAT_FLEX_COUNTER_LOCK(unit);
    flex_base_index_reference_count[unit][bcmStatFlexDirectionIngress]
            ...
            void                 *ingress_entry_data1)
{
    ...
    if (IFP_POLICY_TABLE_PIPE0m == ingress_table) {
       pipe_num = 0;
    } else if (IFP_POLICY_TABLE_PIPE1m == ingress_table) {
       pipe_num = 1;
    } else if (IFP_POLICY_TABLE_PIPE2m == ingress_table) {
       pipe_num = 2;
    } else if (IFP_POLICY_TABLE_PIPE3m == ingress_table) {
       pipe_num = 3;
    } 
    if (shr_aidxres_list_elem_state(
            flex_aidxres_list_handle[unit][bcmStatFlexDirectionIngress]
    ...
    if (_bcm_esw_stat_flex_attach_ingress_table_counters_update(
        unit, pool_number, pipe_num, base_idx, mode) != BCM_E_NONE) {
        return BCM_E_INTERNAL;
    }
            ...
            uint32               *pool_number)
{
    ...
    switch(ingress_table) {
        ...
        case IFP_POLICY_TABLE_PIPE3m:
            ...
            _bcm_esw_get_ifp_pipe_pool_from_mode(unit, mode_id, &pipe_num, &pool);
#endif
    ...
    do {
       ...
       if ((flex_pool_stat[unit][bcmStatFlexDirectionIngress][pipe_num]
           ...
           if (shr_aidxres_list_alloc_block(flex_aidxres_list_handle[unit]
               ...
               flex_pool_stat[unit][bcmStatFlexDirectionIngress]
                    [pipe_num][pool_number_l].used_entries += flex_ingress_modes
                                                    [unit][mode].total_counters;
               LOG_DEBUG(BSL_LS_BCM_FLEXCTR,
                         (BSL_META_U(unit,
