...
#ifdef BCM_KATANA2_SUPPORT 
STATIC int 
_kt2_l2_delete_all_by_hw(int unit, int vfi) 
{ 
    int rv = BCM_E_NONE; 
    ...
    l2_bulk_match_mask_entry_t l2_bulk_match_mask;
    l2_bulk_match_data_entry_t l2_bulk_match_data;
    ...
    soc_mem_field32_set(unit, L2Xm, &l2x_entry, KEY_TYPEf, 
                        (1 << field_len) - 1); 
    sal_memcpy(&l2_bulk_match_mask, &l2x_entry, sizeof(l2x_entry));
    ...
    sal_memcpy(&l2_bulk_match_data, &l2x_entry, sizeof(l2x_entry));
    /*Set match data*/
...
#ifdef BCM_KATANA2_SUPPORT        
        if (SOC_IS_KATANA2(unit)) {
            rv = _kt2_l2_delete_all_by_hw(unit, match_type); 
        } else 
#endif        
        {    
            rv = _tr3_l2_delete_all_by_hw(unit, match_type);
        }    
        if (rv < 0) {
