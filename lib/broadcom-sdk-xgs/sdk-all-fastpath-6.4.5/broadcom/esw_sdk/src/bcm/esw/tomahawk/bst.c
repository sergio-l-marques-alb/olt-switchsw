...
_bcm_bst_th_intr_to_resources(int unit, uint32 *flags)
{
    ...
    reg = THDI_BST_TRIGGER_STATUS_32r;
    ...
    for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
        ...
        if ((fval = soc_reg64_field32_get(unit, THDU_BST_STATr,
            ...
            if (fval & 0x1) {
                ...
                index[res_ct] =
                    soc_reg64_field32_get(unit, THDU_BST_STATr, rval_uc, fld);
                intr_detected = TRUE;
            }
                    ...
                    bcm_bst_stat_id_t bid, int port, int index)
{
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        if ((base_mem = resInfo->stat_mem[pipe]) != INVALIDm) {
            int xpe = 0;
            for (xpe = 0; xpe < NUM_XPE(unit); xpe++) {
