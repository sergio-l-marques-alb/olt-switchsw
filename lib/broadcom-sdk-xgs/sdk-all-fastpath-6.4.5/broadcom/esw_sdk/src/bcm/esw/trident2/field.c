...
_field_sc960_egress_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
   ...
   _FP_QUAL_TWO_SLICE_SEL_ADD(unit, stage_fc,
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyForwardingVlanId,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 210, 12);
...
/* Present in Global Mask Field */
    ...
    if (SOC_IS_TRIDENT2PLUS(unit) &&
            soc_feature(unit, soc_feature_td2p_fp_sw_war)) {
...
/* TOS_FN_LOW */
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyLoopback,
...
_field_td2_egress_qualifiers_init(int unit, _field_stage_t *stage_fc)
{
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyOutPort,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 185, 7);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyIpType,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 192, 5);
    _FP_QUAL_TWO_SLICE_SEL_ADD(unit, stage_fc, bcmFieldQualifySrcClassL2,
                               _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5,
                               _bcmFieldSliceSelEgrClassF1, 0,
                               197, 13);
    ...
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyForwardingVlanId,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 210, 12);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyVrf,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 210, 13);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyVpn,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 210, 13);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyForwardingType,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 223, 2);
    _FP_QUAL_ADD(unit, stage_fc, bcmFieldQualifyDrop,
                 _bcmFieldSliceSelFpf3, _BCM_FIELD_EFP_KEY5, 225, 1);
                      ...
                      int *tcam_idx_old, int *tcam_idx_new)
{
    ...
    for (idx = 0; idx < parts_count; idx++) {
         ...
         */
        if (SOC_IS_TRIDENT2X(unit) && (stage_id == _BCM_FIELD_STAGE_INGRESS)) {
                                          ...
                                          uint8             *mask)
{
	...
	*mask = 0x7;
        break;
    case BCM_FIELD_VXLT_LOOKUP_STATUS_FIRST_HIT:
        *data = 0x1;
	*mask = 0x1;
        break;
    case BCM_FIELD_VXLT_LOOKUP_STATUS_SECOND_HIT:
        *data = 0x2;
        *mask = 0x2;
	break;
                                              ...
                                              uint8             *mask)
{
    ...
    case 0x0:
        *data = BCM_FIELD_VXLT_LOOKUP_STATUS_NO_HIT;
        break;
    case 0x1:
        *data = BCM_FIELD_VXLT_LOOKUP_STATUS_FIRST_HIT;
        break;
    case 0x2:
                               ...
                               )
{
    ...
    if (_BCM_FIELD_STAGE_EXTERNAL != f_ent->group->stage_id) {
        ...
        switch (fa->action) {
            ...
            case bcmFieldActionPortPrioIntCosQNew:
            case bcmFieldActionRpPortPrioIntCosQNew:
                if (fa->param[1] > ((0x1 << 12)-1)) {
                    return (BCM_E_PARAM);
                }
                PolicyCheck(unit, mem, R_COS_INT_PRIf,
                        ((fa->param[0] << 12) | fa->param[1]));
                return (BCM_E_NONE);
                break;
            case bcmFieldActionYpPortPrioIntCosQNew:
                if (fa->param[1] > ((0x1 << 12)-1)) {
                          ...
                          )
{
    ...
    switch (fa->action) {
        ...
        case bcmFieldActionPortPrioIntCosQNew:
            ...
            /* Move the profile index to 12th Position to derive the 
               IFP_COS_MAP_INDEX in HW */
            PolicySet(unit, mem, buf, R_COS_INT_PRIf,
                      (_BCM_COSQ_CLASSIFIER_FIELD_GET(fa->param[0]) << 12 |  
                       fa->param[1]));
            ...
            LOG_DEBUG(BSL_LS_BCM_FP,
                    (BSL_META_U(unit,"FP(unit %d) vverb:"
                                ...
                                "Writing  HintIds information @ byte %d.\n"),
                                                        unit, fc->scache_pos));
            BCM_IF_ERROR_RETURN (_bcm_hints_scache_hintid_sync (unit,
                        &buf[fc->scache_pos]));
        }
    }
 ...
 *     fc               - (IN) Device field control structure pointer
 *     stage_fc         - (IN) FP stage control info
 *     qset             - (IN) EFP group qualifier set
