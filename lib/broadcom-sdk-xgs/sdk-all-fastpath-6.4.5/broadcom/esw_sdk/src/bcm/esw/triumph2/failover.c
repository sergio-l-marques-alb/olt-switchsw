...
{
    int idx, index_min, index_max, prot_grp_idx;
    ...
    for (idx = index_min; idx <= index_max; idx++) {
        SOC_IF_ERROR_RETURN(soc_mem_read(unit, INITIAL_PROT_NHI_TABLEm, MEM_BLOCK_ANY,
                                          idx, &prot_nhi_entry));
        if (soc_INITIAL_PROT_NHI_TABLEm_field32_get(unit, &prot_nhi_entry, fld) != 0) {
            
            prot_grp_idx = soc_INITIAL_PROT_NHI_TABLEm_field32_get(unit, &prot_nhi_entry, fld_grp);

            if (_BCM_FAILOVER_PROT_GROUP_MAP_USED_GET(unit, prot_grp_idx)) {
...
bcm_tr2_failover_init(int unit)
{
    ...
    _bcm_failover_bookkeeping_t *failover_info = FAILOVER_INFO(unit);
     ...
     if (SOC_IS_SABER2(unit)) {
         mmu_num_prot_group = soc_mem_index_count(unit, MMU_PROT_GROUP_TABLEm);
...
#if defined(BCM_SABER2_SUPPORT)
    if (SOC_IS_SABER2(unit)) {
        num_entry = soc_mem_index_count(unit, MMU_PROT_GROUP_TABLEm);
...
STATIC void
_bcm_tr2_failover_set_mmu_prot_group_table_entry (int unit, bcm_failover_t  prot_index)
{
   _BCM_FAILOVER_MMU_PROT_GROUP_MAP_USED_SET(unit, prot_index);
}
...
#if defined(BCM_SABER2_SUPPORT)
    mmu_prot_group_table_entry_t  mmu_prot_group_entry;
    int local_failover_id;
...
#if defined(BCM_SABER2_SUPPORT)
        if (flags & BCM_FAILOVER_ENCAP) {
            sal_memset(&mmu_prot_group_entry, 0, sizeof(mmu_prot_group_table_entry_t));
            local_failover_id = *failover_id;
            _BCM_GET_FAILOVER_ID(local_failover_id);
            rv = soc_mem_write(unit, MMU_PROT_GROUP_TABLEm,
                    MEM_BLOCK_ALL, local_failover_id,
                    &mmu_prot_group_entry);
            if (rv < 0) {
                _bcm_tr2_failover_clear_mmu_prot_group_table_entry(unit, local_failover_id);
                return BCM_E_RESOURCE;
            }
...
{
    int rv = BCM_E_UNAVAIL;
    mmu_prot_group_table_entry_t  prot_group_entry;
   ...
   /* Clear entry */
    BCM_IF_ERROR_RETURN (soc_mem_read(unit, MMU_PROT_GROUP_TABLEm, 
                             MEM_BLOCK_ANY, failover_id, &prot_group_entry));
    sal_memset(&prot_group_entry, 0, 
                             sizeof(mmu_prot_group_table_entry_t));
    rv = soc_mem_write(unit, MMU_PROT_GROUP_TABLEm,
                             MEM_BLOCK_ALL, failover_id,
                             &prot_group_entry);
    if (rv < 0) {
         _bcm_tr2_failover_set_mmu_prot_group_table_entry(unit, failover_id);
         return BCM_E_RESOURCE;
    }
    return rv;
}
    ...
    if (ecmp < 0) {
        ...
        if (soc_mem_field_valid(unit, INITIAL_L3_ECMP_GROUPm, BASE_PTRf)) {
            base_ptr = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                    BASE_PTRf);
        } else if (soc_mem_field_valid(unit, INITIAL_L3_ECMP_GROUPm, BASE_PTR_0f)){
            base_ptr = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                    BASE_PTR_0f);
        }
        if (soc_mem_field_valid(unit, INITIAL_L3_ECMP_GROUPm, COUNTf)) {
            count    = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                    COUNTf);
        } else if (soc_mem_field_valid(unit, INITIAL_L3_ECMP_GROUPm, COUNT_0f)) {
            count    = soc_mem_field32_get(unit, INITIAL_L3_ECMP_GROUPm, hw_buf, 
                    COUNT_0f);
        }
    }
...
bcm_tr2_failover_id_check(int unit, bcm_failover_t failover_id)
{
    ...
    if (type & _BCM_FAILOVER_1_1_MC_PROTECTION_MODE) {
        ...
        num_count = soc_mem_index_count(unit, MMU_PROT_GROUP_TABLEm);
