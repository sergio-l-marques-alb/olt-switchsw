/*
 * $Id: subport.c,v 1.54 Broadcom SDK $
    ...
    if (SOC_IS_SC_CQ(unit)) {
        ...
        if (NULL == _sc_subport_port_l3idx_map[unit]) {
            _sc_subport_port_l3idx_map[unit] =
                sal_alloc(SUBPORT_NUM_GROUP * sizeof(int16), "sc subport l3 idx");
            if (_sc_subport_port_l3idx_map[unit] == NULL) {
                _bcm_tr_subport_free_resource(unit);
                return BCM_E_MEMORY;
            }
...
bcm_tr_subport_group_create(int unit, bcm_subport_group_config_t *config,
                             bcm_gport_t *group)
{
    int gport_id, rv, vp_base = -1, nh_index = -1, is_local_modid;
    ...
    if (SOC_IS_TR_VL(unit)) {
        ...
        rv = WRITE_ING_DVP_TABLEm(unit, MEM_BLOCK_ALL, vp_base, &dvp);
        if (rv < 0) {
            goto cleanup;
        }
...
int bcm_esw_triumph_subport_not_empty;
