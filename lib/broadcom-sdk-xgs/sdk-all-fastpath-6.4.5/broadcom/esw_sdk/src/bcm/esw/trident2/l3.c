...
_bcm_td2_l3_entry_add(int unit, _bcm_l3_cfg_t *l3cfg, int nh_idx)
{
    ...
    if (embedded_nh) {
        ...
        if (l3cfg->l3c_flags & BCM_L3_TGID) {
            ...
            modid = (SOC_MODID_MAX(unit) & l3cfg->l3c_modid) << port_mask_len;
            glp = modid | port_id;
        }
        ...
        bcm_if_t *intf_array)
{
    ...
    /* 
     * In case of Riot, RH_ECMP_FLOWSET table is divided into two halves.
...
#ifdef BCM_RIOT_SUPPORT
    if (l3_bk_info->riot_enable &&
       (l3_bk_info->l3_ecmp_levels > BCMI_L3_ECMP_SINGLE_LEVEL) &&
