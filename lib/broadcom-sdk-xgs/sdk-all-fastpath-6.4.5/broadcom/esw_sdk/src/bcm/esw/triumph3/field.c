                               ...
                               )
{
    ...
    if (_BCM_FIELD_STAGE_EXTERNAL != f_ent->group->stage_id) {
        ...
        case bcmFieldActionRpPortPrioIntCosQNew:
            ...
            PolicyCheck(unit, mem, R_COS_INT_PRIf, ((arg << 9 ) | arg2));
            return (BCM_E_NONE);
        case bcmFieldActionYpPortPrioIntCosQNew:
            if (arg2 > ((0x1 << 9) - 1)) {
                return (BCM_E_PARAM);
            }
            PolicyCheck(unit, mem, Y_COS_INT_PRIf, ((arg << 9 ) | arg2));
            return (BCM_E_NONE);
        case bcmFieldActionGpPortPrioIntCosQNew:
            if (arg2 > ((0x1 << 9) - 1)) {
                return (BCM_E_PARAM);
            }
            PolicyCheck(unit, mem, G_COS_INT_PRIf, ((arg << 9 ) | arg2));
            return (BCM_E_NONE);
        ...
        case bcmFieldActionCosQNew:
            /* Apply this action for Gp/Yp/Rp unicast packets. */
             if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(fa->param[0])) {
                 ...
                 PolicySet(unit, mem, buf, R_COS_INT_PRIf, index);
                 PolicySet(unit, mem, buf, Y_COS_INT_PRIf, index);
                 PolicySet(unit, mem, buf, G_COS_INT_PRIf, index);

                 PolicySet(unit, mem, buf, R_CHANGE_COS_OR_INT_PRIf,
                                 _FP_ACTION_UCAST_QUEUE_CHANGE);
                 PolicySet(unit, mem, buf, Y_CHANGE_COS_OR_INT_PRIf,
                                 _FP_ACTION_UCAST_QUEUE_CHANGE);
                 PolicySet(unit, mem, buf, G_CHANGE_COS_OR_INT_PRIf,
                                 _FP_ACTION_UCAST_QUEUE_CHANGE);
                             ...
                             )
{
    ...
    switch (stage_id) {
        case  _BCM_FIELD_STAGE_INGRESS:
            ...
            f_mp = stage_fc->meter_pool[_FP_DEF_INST][pool_num];
            ...
            case BCM_FIELD_METER_MODE_FLOW: /* 1 */
                switch (stage_id) {
                    case _BCM_FIELD_STAGE_INGRESS:
                        ...
                        mem_idx =
                            (stage_fc->slices[_FP_DEF_INST][f_pl->pool_index].start_tcam_idx* 2)
                    ...
                    case _BCM_FIELD_STAGE_EGRESS:
                        ...
                        mem_idx =
                            stage_fc->slices[_FP_DEF_INST][f_pl->pool_index]
                            .start_tcam_idx + (2 * f_pl->hw_index) + meter_offset;
                        _bcm_field_trx_meter_rate_burst_recover
                            (unit,
                             EFP_METER_TABLEm,
                             mem_idx,
                                    ...
                                    )
{
    ...
    for (slice_idx = 0; slice_idx < stage_fc->tcam_slices; slice_idx++) {
        ...
        for (idx = 0; idx < slice_ent_cnt; idx++) {
            ...
            sid = pid = -1;
            if (fc->l2warm) {
                /* Recover f_ent_wb structure from here */
                /* Allocate f_ent_wb structure and pass it to
                 * corresponding function */
                if (version >= BCM_WB_VERSION_1_8) {
                    _FP_XGS3_ALLOC(f_ent_wb, sizeof (_field_entry_wb_t),
                ...
                if (BCM_FAILURE(rv)) {
                    sal_free(f_ent);
                    _field_free_wb_entry (unit, &f_ent_wb);
                    goto cleanup;
                }
            } else {
