...
#define _DPP_AM_TEMPLATE_BFD_IP_DIP_PROFILE_MAX_ENTITIES (SOC_PPD_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit)) 
...
#define _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES(unit) (SOC_TMC_ITM_NOF_VSQS * SOC_DPP_DEFS_GET(unit, nof_cores))
    ...
    break;
        ...
        case dpp_am_res_local_inlif_wide:
         ...
         case dpp_am_res_local_outlif:
            ...
            {
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->count = _BCM_DPP_AM_EGRESS_LIF_NOF_ENTRIES_PER_HALF_BANK; 
                p_info->start =  0;
        ...
        case dpp_am_res_global_outlif:
            if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)){
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = _BCM_DPP_AM_GLOBAL_LIF_EGRESS_BANK_LOW_ID(core_id);
        ...
        case dpp_am_res_global_inlif:
            if (SOC_IS_JERICHO(unit) && SOC_DPP_PP_ENABLE(unit)){
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
                p_info->start = _BCM_DPP_AM_GLOBAL_LIF_INGRESS_BANK_LOW_ID(core_id);
                p_info->count = _BCM_DPP_AM_GLOBAL_LIF_NOF_ENTRIES_PER_BANK;
                extra = NULL;
        ...
        case dpp_am_res_obs_inlif:
            if (SOC_IS_ARAD(unit) && SOC_IS_ARADPLUS_AND_BELOW(unit) && SOC_DPP_PP_ENABLE(unit)){
                p_info->res_type = SHR_RES_ALLOCATOR_BITMAP;
    ...
    bcm_vlan_t *vsi)
{
    ...
    sw_state_access[unit].dpp.bcm.vlan.vlan_info.count.get(unit, &count);
    count++;
    sw_state_access[unit].dpp.bcm.vlan.vlan_info.count.set(unit, count);
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Supported only by Arad(+)"), flags));
    }
...
#endif /* BCM_ARAD_SUPPORT */
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Supported only by Arad(+)")));
    }
        ...
        int eep)
{
    ...
    if(SOC_IS_PETRAB(unit)) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("l2 encapsulation for external cpu is not supported for PB")));
    }
    ...
    int *inlif_id)
{
    ...
    rv = dpp_am_res_alloc(unit, core_id, res_pool, alloc_flags, 1, inlif_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *outlif_id)
{
    ...
    rv = dpp_am_res_alloc(unit, core_id, dpp_am_res_local_outlif, flags, 1, outlif_id);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int *outlif_id)
{
    ...
    rv = dpp_am_res_alloc_align(unit,core_id,dpp_am_res_local_outlif,alloc_flags,align,offset, count, outlif_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
bcm_dpp_am_global_lif_is_alloced(int unit, uint32 flags, int global_lif){
    ...
    if (is_ingress && is_egress) {
        /* BCM_RETURN_VAL_EXIT(_bcm_dpp_am_global_sync_lif_internal_is_alloced(unit, global_lif)); */
...
bcm_dpp_am_global_lif_dealloc(int unit, uint32 flags, int global_lif){
    ...
    if (is_ingress && is_egress) {
        ...
        BCM_RETURN_VAL_EXIT(_bcm_dpp_am_global_egress_lif_internal_dealloc(unit, global_lif));
    } else /*is_ingress*/ { 
        BCM_RETURN_VAL_EXIT(_bcm_dpp_am_global_ingress_lif_internal_dealloc(unit, global_lif));
    }
exit:
    BCMDNX_FUNC_RETURN;
}
   ...
   int *eep)
{
    ...
    if (SOC_IS_ARAD(unit)) {
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("EEDB trill allocation is not availabled")));
    }
    ...
    case dpp_am_template_egress_interface_unicast_thresh:
        ...
        case dpp_am_template_oam_mep_profile_non_accelerated:
            if (SOC_IS_ARAD(unit)) {
                ...
                t_info->count = (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)
                                 || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit))
                                        ? _DPP_AM_TEMPLATE_OAM_MEP_PROFILE_NON_ACC_COUNT_ARAD_PLUS - (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit) ? 1 : 0) /* In Jericho, Last profile is statically allocated for 'FWD' */
        ...
        case dpp_am_template_vsq_rate_cls:
            ...
            t_info->max_entities = _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES(unit);
...
/* TPID profile - Start */
STATIC int
_bcm_dpp_am_template_tpid_profile_port_tpid_profile_mapping_get(int unit, int pp_port, int core, int *tpid_profile)
{
    uint32 soc_sand_rv;
    int soc_sand_dev_id;
    SOC_PPD_PORT_INFO port_info;
...
int _bcm_dpp_am_template_vsi_egress_profile_data_get(int unit, int vsi, bcm_dpp_vsi_egress_profile_t * data)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mtu_advanced_mode", 0)) {
        ...
        if (old_profile != 0) {
            soc_sand_rv = soc_ppd_vsi_egress_mtu_get(unit, old_profile, &(data->mtu));
            BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
        }
    }
    else {
...
int _bcm_dpp_am_template_bfd_ip_dip_alloc(int unit, int flags,const SOC_SAND_PP_IPV6_ADDRESS *data, int *is_allocated, int *profile_ndx)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ip_dip, flags, data, is_allocated, profile_ndx);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_ip_dip_free(int unit, int profile_ndx, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ip_dip, profile_ndx, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_ip_dip_data_get(int unit, int profile_ndx, SOC_SAND_PP_IPV6_ADDRESS* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_bfd_ip_dip, profile_ndx, data);
...
STATIC int
_bcm_dpp_am_template_vsq_rate_cls_mapping_get(int unit,int core_id, SOC_TMC_ITM_VSQ_NDX vsq_id,int *rate_cls)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
...
int _bcm_dpp_am_template_vsq_rate_cls_init(int unit,int template_init_id,const bcm_dpp_cosq_vsq_rate_class_info_t* data_rate_cls)
{
    ...
    /* Add all entities to template init id with given data */
    result = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_vsq_rate_cls, flags, data_rate_cls, _DPP_AM_TEMPLATE_VSQ_RATE_CLS_MAX_ENTITIES(unit), &is_allocated,&template);
    BCMDNX_IF_ERR_EXIT(result);
...
int _bcm_dpp_am_template_vsq_rate_cls_data_get(int unit, int core_id, int vsq_id, bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls)
{
    ...
    int rc = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_vsq_rate_cls_mapping_get(unit, core_id, vsq_id, &rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_vsq_rate_cls_exchange(int unit, int core_id, int vsq_id, bcm_dpp_cosq_vsq_rate_class_info_t * data_rate_cls, int *old_rate_cls, int *is_last, int *new_rate_cls,int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = _bcm_dpp_am_template_vsq_rate_cls_mapping_get(unit, core_id, vsq_id, &tmp_old_rate_cls);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_cosq_sched_class_allocate(int unit, int core, int flags, int *data, int *is_allocated, int *class_template)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, core /* BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, flags, data, is_allocated, class_template);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_cosq_sched_class_free(int unit, int core, int class_template, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, core /* BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, class_template, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_cosq_sched_class_data_get(int unit, int core, int class_template, int *data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, core /* BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, class_template, data);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
_bcm_dpp_am_template_cosq_sched_class_exchange(int unit, int core, int flags, int *data, int old_class_template, int *is_last, int *class_template, int *is_allocated)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_exchange(unit, core /* BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, flags, data, old_class_template, &temp_is_last, class_template, is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_cosq_sched_class_ref_get(int unit, int core, int template, uint32 *ref_count)
{
    ...
    rc = dpp_am_template_ref_count_get(unit, core /* BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, template, ref_count);
...
_bcm_dpp_am_template_cosq_sched_class_allocate_group(int unit, int core, uint32 flags, int *data, int ref_count, int *is_allocated, int *template)
{
    ...
    rc = dpp_am_template_allocate_group(unit, core /* was BCM_DPP_AM_DEFAULT_CORE_ID */, dpp_am_template_cosq_sched_class, flags, (void *)data, ref_count, is_allocated, template);
...
int _bcm_dpp_am_template_ingress_flow_tc_mapping_data_get(int unit, int core_id, int dest_ndx,SOC_TMC_ITM_TC_MAPPING *tc_mapping)
{
  uint32 profile_ndx;
  int rc = BCM_E_NONE;
  int core = (core_id == BCM_CORE_ALL) ? BCM_DPP_AM_DEFAULT_CORE_ID : core_id;
  BCMDNX_INIT_FUNC_DEFS;
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_init(int unit, int priority, int template_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *generic_bm)
{
    ...
    rc = dpp_am_template_allocate_group(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_pool, SHR_TEMPLATE_MANAGE_SET_WITH_ID, 
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_data_get(int unit, int priority, int generic_bm_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *generic_bm)
{
    ...
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_pool, generic_bm_id,generic_bm);
    BCMDNX_IF_ERR_EXIT(rc);
...
int _bcm_dpp_am_template_fc_generic_pfc_mapping_exchange(int unit, int priority, int old_generic_bm_id, SOC_TMC_FC_PFC_GENERIC_BITMAP *new_generic_bm, int *is_last, int *new_mapping_profile,int *is_allocated)
{
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_CORE_ID, template_pool, flags, new_generic_bm,
...
int _bcm_dpp_am_template_vrrp_dealloc_all(int unit, int profile_index, int *dealloc_count)
{
    ...
    rv = dpp_am_template_free_all(unit, BCM_DPP_AM_DEFAULT_CORE_ID, dpp_am_template_l3_vrrp, profile_index, dealloc_count);
    BCMDNX_IF_ERR_EXIT(rv);
