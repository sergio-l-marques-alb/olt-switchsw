...
int htbl_iteration_callback(int unit, shr_htb_key_t really_the_event_data,shr_htb_data_t really_the_flags ){
    ...
    if (event_info->is_oam) {
        if (_g_oam_event_cb[unit][event_info->event_number]) {
            ...
            /*finally, the callback*/
            _g_oam_performance_event_cb[unit][event_info->event_number](unit, event_info->event_number, mep_info.ma_index, rmeb_db_ndx,
   ...
   void *user_data)
{
	...
	rv = _bcm_dpp_gport_to_global_lif(unit, endpoint_info->gport, (int*)&lif, NULL, &fec_id, &is_local);
	BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_dpp_oam_error_trap_set(int unit, uint32 oamp_error_trap_id, bcm_gport_t dest_port)
{
    ...
    if (!BCM_GPORT_IS_SYSTEM_PORT(dest_port)) {
		BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Error: OAMP error traps destination must be system port.\n")));
    }

    rv = _bcm_dpp_gport_to_phy_port(unit, dest_port, _BCM_DPP_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info);
    BCMDNX_IF_ERR_EXIT(rv);
		...
		if (existing_endpoint_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
            ...
            if (endpoint_info_lcl->gport != BCM_GPORT_INVALID || existing_endpoint_info.gport != BCM_GPORT_INVALID) {
                if (endpoint_info_lcl->type == bcmOAMEndpointTypeEthernet) {
                    ...
                    rv = _bcm_dpp_gport_to_global_lif(unit, endpoint_info_lcl->gport,&lif1, NULL, &dont, &care);
                    BCMDNX_IF_ERR_EXIT(rv);
                    rv = _bcm_dpp_gport_to_global_lif(unit, existing_endpoint_info.gport, &lif2, NULL, &dont, &care);
                    BCMDNX_IF_ERR_EXIT(rv);
            ...
            if (endpoint_info_lcl->mpls_out_gport != BCM_GPORT_INVALID || existing_endpoint_info.mpls_out_gport != BCM_GPORT_INVALID) {
                /* gport_compare will not suffice for gports returned by MPLS APIs*/
                int lif1, lif2, dont, care;
                rv = _bcm_dpp_gport_to_global_lif(unit, endpoint_info_lcl->mpls_out_gport, &lif1, NULL, &dont, &care);
                BCMDNX_IF_ERR_EXIT(rv);
                rv = _bcm_dpp_gport_to_global_lif(unit, existing_endpoint_info.mpls_out_gport, &lif2, NULL, &dont, &care);
                BCMDNX_IF_ERR_EXIT(rv);
                port_is_equal = (lif1 == lif2);
                if (!port_is_equal) {
            ...
            if (endpoint_info_lcl->gport != BCM_GPORT_INVALID) {
                ...
                if (update_counter) {
                    if (endpoint_info_lcl->lm_counter_base_id > 0) {
                        rv = _bcm_dpp_oam_set_counter(unit, endpoint_info_lcl->lm_counter_base_id, ((endpoint_info_lcl->lm_flags & BCM_OAM_LM_PCP)!=0), new_classifier_mep_entry.lif);
                        BCMDNX_IF_ERR_EXIT(rv);
                    }
...
int bcm_petra_oam_loss_get(int unit, bcm_oam_loss_t * loss_ptr) {
    ...
    if (eth1731_profile_data.report_mode) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Endpoint %d uses reports for LM/DM."), loss_ptr->id));
    }
...
int bcm_petra_oam_delay_add(int unit, bcm_oam_delay_t *delay_ptr) {
    ...
    if (SOC_IS_JERICHO(unit) && (delay_ptr->period)) {
...
int bcm_petra_oam_delay_get(int unit, bcm_oam_delay_t *delay_ptr) {
    ...
    if (eth1731_profile_data.report_mode) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Endpoint %d uses reports for LM/DM."), delay_ptr->id));
    }
