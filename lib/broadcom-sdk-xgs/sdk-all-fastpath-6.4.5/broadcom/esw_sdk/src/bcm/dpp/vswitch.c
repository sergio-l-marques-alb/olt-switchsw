...
#define SHR_BITCOPY_RANGE_VSI_USAGE_GET(_dst, _dst_start, _vsi_start, _nof_bits)\
        SHR_BITCOPY_RANGE(_dst, _dst_start, sw_state[unit]->dpp.bcm.vswitch->vsi_usage, _vsi_start, _nof_bits)

#define SHR_BITCOPY_RANGE_VSI_USAGE_SET(_src, _src_start, _vsi_start, _nof_bits)\
        SHR_BITCOPY_RANGE(sw_state[unit]->dpp.bcm.vswitch->vsi_usage, _vsi_start, _src, _src_start, _nof_bits)
    ...
    int    unit)
{
    ...
    BCMDNX_IF_ERR_EXIT(VSWITCH_ACCESS.vsi_usage.free(unit));
    BCMDNX_IF_ERR_EXIT(VSWITCH_ACCESS.free(unit));
...
bcm_petra_vswitch_init(
    int    unit)
{
    int rv = BCM_E_NONE, vsi_usage_entries;
    int vsi_alloc_flags;
    ...
    vsi_usage_entries = (((_BCM_GPORT_NOF_VSIS + 31) / 32) * _BCM_DPP_VSI_TYPE_NOF_BITS);
    BCMDNX_IF_ERR_EXIT(VSWITCH_ACCESS.vsi_usage.alloc(unit, vsi_usage_entries));
    ...
    bcm_gport_t    port)
{
    ...
    /* set map to vsi */
    rv = _bcm_dpp_in_lif_match_vsi_set(unit, gport_hw_resources.local_in_lif, vsi);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int         other_encap) 
{
    ...
    if (!SOC_DPP_IS_SYS_RSRC_MGMT_ADVANCED(unit)) {
        rv = _bcm_dpp_in_lif_match_peer_gport_set(unit, local_lif_index, gport);
        BCMDNX_IF_ERR_EXIT(rv);
    }
