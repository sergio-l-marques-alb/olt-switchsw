...
bcm_petra_port_deinit(int unit)
{
    ...
    rv = PORT_ACCESS.dpp_port_config.bcm_port_tpid_info.free(unit);
    BCMDNX_IF_ERR_EXIT(rv);
                                ...
                                bcm_port_control_t type, int value)
{
    ...
    switch(type) {
       ...
       /* Flow Control */
        ...
        case bcmPortControlTxEnable:
            BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                rv = MBCM_DPP_SOC_DRIVER_CALL(unit,mbcm_dpp_port_control_tx_nif_enable_set,(unit, port_ndx, value));
                BCMDNX_IF_ERR_EXIT(rv);
            }
            break;
        ...
        case bcmPortControlFloodUnknownUcastGroup:
          {
            ...
            if (SOC_IS_ARAD(unit)) {
                ...
                /* Set flooding */
                rv = _bcm_petra_vlan_flooding_per_lif_set(unit, port, gport_hw_resources.local_in_lif, is_port_flooding,
                                     is_lif_flooding, uc, mc, bc);
                BCMDNX_IF_ERR_EXIT(rv);
            } else {
