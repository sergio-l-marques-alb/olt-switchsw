...
#define _BCM_L2_EGRESS_UNSUPPORTED_FLAGS (BCM_L2_EGRESS_DEST_MAC_PREFIX5_REPLACE)
                                       ...
                                       bcm_l2_learn_msgs_config_t *learn_msg_config)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_hdr_type_out_get(unit,si->olp_port[0],&olp_receive_header_type));
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_hdr_type_in_get(unit,si->olp_port[0],&olp_transmit_header_type));
                    ...
                    int unit)
{
    ...
    BCMDNX_IF_ERR_CONT(rv);
    BCMDNX_IF_ERR_EXIT(L2_ACCESS.free(unit));
exit:
    _BCM_DPP_SWITCH_API_END(unit);
    BCMDNX_FUNC_RETURN;
}
                  ...
                  int unit)
{
    ...
    */
    ...
    rv = _bcm_petra_l2_event_handle_buff_build(BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,
												BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER, BCM_L2_ADDR_DIST_SET_LEARN_DISTRIBUTER,&buf);
    BCMDNX_IF_ERR_EXIT(rv);
...
_bcm_petra_l2_egress_create_verify(int unit, bcm_l2_egress_t *egr) {
    ...
    int rv; 
    uint8 support_quarter_entry = FALSE; /* temporary variable. To remove when we support eedb 1/4 entry */
    ...
    if (egr->flags & BCM_L2_EGRESS_WITH_ID) {
        ...
        global_outlif = _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id); 
    ...
    if (egr->flags & BCM_L2_EGRESS_REPLACE) {
        ...
        {
            local_outlif = global_outlif = _BCM_PETRA_L3_ITF_VAL_GET(egr->encap_id);
        }

        BCMDNX_IF_ERR_EXIT( _bcm_dpp_lif_usage_get(unit, -1, local_outlif, NULL, &usage) );
        ...
        /* eedb 1/4 entry is not supported. Therefore outer_vlan lsbs are not supported. Check only 8 MSBS of outer_vlan.
           Remove this code when eedb 1/4 entry is supported */
        else {
            if (!(egr->flags & BCM_L2_EGRESS_OUTER_VLAN_REPLACE) 
        ...
        if (SOC_IS_JERICHO(unit)) {
            /* eedb 1/4 entry is not supported. Therefore inner pcp dei and inner vlan are not supported. 
               Remove the test when eedb 1/4 entry is supported */
            if (support_quarter_entry) {
                /* check inner vlan */
...
bcm_petra_l2_egress_create(int unit, bcm_l2_egress_t *egr)
{
    ...
    if (!update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            local_out_lif_info.local_lif_flags |= (DPP_IS_FLAG_SET(egr->flags, BCM_L2_EGRESS_WIDE)) ? BCM_DPP_AM_OUT_LIF_FLAG_WIDE : 0;
            ...
            rv = bcm_dpp_am_l3_eep_alloc(unit,_BCM_DPP_AM_L3_EEP_TYPE_ROO_LINKER_LAYER,alloc_flags ,&local_outlif);
            BCM_IF_ERROR_RETURN(rv);
            global_outlif = local_outlif;
        }
    }

    /* update allocated encap id (overylay arp eedb index) */
    _BCM_PETRA_L3_ITF_SET(egr->encap_id,_BCM_PETRA_L3_ITF_ENCAP, global_outlif); 
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if (update) {
            ...
            /* get the current index for CfgEtherTypeIndex from eedb entry */
            rv = _bcm_petra_l2_egress_eedb_roo_ll_eth_type_get(unit, local_outlif, &old_eth_type_index_ndx); 
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = soc_ppd_eg_encap_overlay_arp_data_entry_add(unit, local_outlif, &ll_encap_info);
    BCMDNX_IF_ERR_EXIT(rv);

    /* Update SW DB */
    rv = _bcm_dpp_out_lif_overlay_ll_match_add(unit, local_outlif);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (egr->l3_intf > 0) {
        if (_BCM_PETRA_L3_ITF_IS_ENCAP(egr->l3_intf)) {
            ...
            rv = _bcm_tunnel_intf_ll_eep_set(unit,egr->l3_intf,tunnel_eep_index, local_outlif);
            BCMDNX_IF_ERR_EXIT(rv);
        } 
    ...
    bcm_l2_egress_t *egr)
{
    ...
    _bcm_lif_type_e usage;
    int global_outlif = 0, local_outlif = 0;  /* used to get allocated outlif/eedb index */
    uint8 is_wide_entry = 0; /* indicates whether lif is in wide entry */
    ...
    /* Verifying whether this tunnel is a wide entry */
    if (SOC_IS_JERICHO(unit)) {

        rv = MBCM_PP_DRIVER_CALL(unit,mbcm_pp_lif_is_wide_entry,(unit, local_outlif, FALSE, &is_wide_entry));
        BCMDNX_IF_ERR_EXIT(rv);

        egr->flags |= is_wide_entry ? BCM_L2_EGRESS_WIDE : 0;
    }
    ...
    bcm_if_t encap_id)
{
    ...
    rv = _bcm_dpp_out_lif_match_delete(unit, local_outlif);
    BCMDNX_IF_ERR_EXIT(rv);
