...
int bcm_common_sat_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (BCM_SUCCESS(rv) && SOC_IS_SABER2(unit)) {
            rv = bcm_sb2_sat_oamp_enable(unit, 1);
        }
#endif
    ...
    )
{
    ...
    SAT_VALUE_MAX_CHECK(config->packet_context_id, (MAX_NUM_OF_CORES+1), "packet_context_id");
#endif
    ...
    )
{
    ...
    SAT_VALUE_CHECK(ctf_id, _BCM_SAT_CTF_ID_MIN, _BCM_SAT_CTF_ID_MAX, "ctf_id");
    psat_data = &(sat_data[unit]);
    SAT_CTF_ID_NOT_EXIST_EXIT(psat_data, ctf_id);
