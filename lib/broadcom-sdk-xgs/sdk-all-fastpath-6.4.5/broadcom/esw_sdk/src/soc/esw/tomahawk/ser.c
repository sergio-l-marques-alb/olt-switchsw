        ...
        if (soc_reg_field_get(unit, reg, rval, ecc_field)) {
            ...
            case IDB_OBM7_QUEUE_ECC_STATUSr:
                ...
                soc_event_generate(unit, SOC_SWITCH_EVENT_PARITY_ERROR,
                                   ...
                                   entry_idx); 
                break;
            case IDB_IS_TDM_CAL_ECC_STATUSr:
                if (double_bit) {
                    soc_event_generate(unit, SOC_SWITCH_EVENT_PARITY_ERROR,
                                   SOC_SWITCH_EVENT_DATA_ERROR_FATAL,
                                   INVALIDm, 
            ...
            if (regmem == _SOC_TH_SER_MEM) {
                ...
                if (non_sbus == 0) {
                    ...
                    if (spci.mem != INVALIDm) {
                        ...
                        rv = soc_th_field_mem_mode_get(unit, spci.mem,
                                                       &mem_mode);
                        if ((rv == SOC_E_NONE) &&
                            (mem_mode == _SOC_SER_MEM_MODE_PIPE_UNIQUE) &&
                            (spci.mem != L3_TUNNEL_DATA_ONLYm &&
...
int
soc_th_ifp_index_invalid(int unit, soc_mem_t mem, int index)
{
    int slice_num, slice_type, rv, pipe;
    int cfg_slice_enabled = 0;
    int cfg_slice_type = 0;

    if (!SOC_MEM_IS_VALID(unit, mem)) {
        return TRUE; /* skip mem_acc */
    }

    switch (mem) {
    case IFP_TCAMm:
        pipe = -1; slice_num = index/512; slice_type = 0; break;
    case IFP_TCAM_PIPE0m:
        pipe = 0;  slice_num = index/512; slice_type = 0; break;
    case IFP_TCAM_PIPE1m:
        pipe = 1;  slice_num = index/512; slice_type = 0; break;
    case IFP_TCAM_PIPE2m:
        pipe = 2;  slice_num = index/512; slice_type = 0; break;
    case IFP_TCAM_PIPE3m:
        pipe = 3;  slice_num = index/512; slice_type = 0; break;

    case IFP_TCAM_WIDEm:
        pipe = -1; slice_num = index/256; slice_type = 1; break;
    case IFP_TCAM_WIDE_PIPE0m:
        pipe = 0;  slice_num = index/256; slice_type = 1; break;
    case IFP_TCAM_WIDE_PIPE1m:
        pipe = 1;  slice_num = index/256; slice_type = 1; break;
    case IFP_TCAM_WIDE_PIPE2m:
        pipe = 2;  slice_num = index/256; slice_type = 1; break;
    case IFP_TCAM_WIDE_PIPE3m:
        pipe = 3;  slice_num = index/256; slice_type = 1; break;

    default: /* not one of the ifp tcam views */
    ...
    if (SOC_FAILURE(rv)) {
         ...
         * We are in pipe-unique mode, and we pass 'pipe = -1' (user is
         * accessing IFP_TCAM or IFP_TCAM_WIDE mode. These acccesses are illegal
         * because SER detection logic does not know which parity bits to
         * update/compare - so it is ok to mark these accesses as invalid */
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
...
STATIC int
_soc_th_ifp_slice_mode_check(int unit, soc_mem_t mem, int *slice_skip)
{
 ...
 *    Here we can use standard soc_th_ifp_slice_mode_get() as defined earlier,
 *    but to keep the code same - we are not.
 */
int
soc_th_ifp_tcam_dma_read(int unit, soc_mem_t mem, int blk, uint32 *table)
{
    ...
    for (slice = 0; slice < 12; slice++) {
        ...
        SOC_IF_ERROR_RETURN(
            _soc_th_ifp_slice_mode_check(unit, mem,
                                         &slice_skip));
        if (!slice_skip) {
            LOG_VERBOSE(BSL_LS_SOC_SER,
                    (BSL_META_U(unit,
 ...
 */
int
soc_th_check_scrub_skip(int unit, soc_mem_t mem)
{
    if (!SOC_MEM_IS_VALID(unit,mem)) {
        return 1; /* skip */
    }
    if (soc_mem_is_cam(unit, mem) &&
        ...
        int rv = soc_th_field_mem_mode_get(unit, mem, &mem_mode);
    ...
    */
        ...
        case CPU_COS_MAPm: /* not protected by SER engine also */
                        ...
                        int *num_inst_to_scrub, int *acc_type_list)
{
    ...
    soc_block_t blk_type = SOC_BLOCK_TYPE(unit, copyno);
    ...
    *num_inst_to_scrub = 0;
    ...
    if ((mem_acc_type == _SOC_TH_ACC_TYPE_DUPLICATE) &&
        ...
        /* last term: sw has not already created per-instance unique views */
