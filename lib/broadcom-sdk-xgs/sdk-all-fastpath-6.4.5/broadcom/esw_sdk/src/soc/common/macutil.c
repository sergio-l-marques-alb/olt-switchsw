...
_soc_egress_metering_freeze(int unit, soc_port_t port, void **setting)
{
    ...
    case SOC_CHIP_BCM56960: /* Need to add code to recover shaper */
    default:
        break;
    }
...
int
soc_mmu_is_hw_flush_capable(int unit)
{
    if (SOC_IS_TOMAHAWK(unit) &&
        (soc_property_get(unit, "mmu_hardware_flush", 0))) {
        return TRUE;
    }
    return FALSE;
}
...
STATIC int
soc_mmu_port_empty_check(int unit, soc_port_t port, int *empty)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_egress_cell_count(unit, port, &cell_ct));

    if (((cell_ct != 0) && (*empty == TRUE)) ||
        ((cell_ct == 0) && (*empty == FALSE))) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Port Flush(Port %s). Hardware and Software "
                              "Empty status are Not in Sync. "
                              "H/w status %d, S/w Count %d\n"),
                   SOC_PORT_NAME(unit, port), *empty, cell_ct));
    }
...
soc_mmu_port_flush_hw(int unit, soc_port_t port, uint32 drain_timeout)
{
    ...
    uint32 rval;
    int pipe, empty;
    soc_reg_t flush_reg = Q_SCHED_PORT_FLUSHr;
    soc_timeout_t to;
    ...
    mmu_port %= SOC_TH_MMU_PORT_STRIDE;
#endif
    COMPILER_64_SET(rval64, (1 << (mmu_port - 32)), (1 << mmu_port));

    SOC_IF_ERROR_RETURN
        (soc_mmu_port_empty_check(unit, port, &empty));
    ...
    rval = 0;
    soc_reg_field_set(unit, Q_SCHED_RQE_SNAPSHOTr, &rval, INITIATEf, 1);
...
soc_egress_drain_cells(int unit, soc_port_t port, uint32 drain_timeout)
{
    ...
    if (soc_mmu_is_hw_flush_capable(unit) == TRUE) {
...
#if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2(unit)) {
        soc_timeout_t to;
        int count = 0;
