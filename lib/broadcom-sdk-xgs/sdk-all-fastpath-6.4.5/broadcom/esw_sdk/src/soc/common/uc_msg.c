 ...
 * Purpose:
 *      Shutdown messaging, notifying registered applications, and halt the uCs
 * Parameters:
 *      unit - unit number
 * Returns:
 *      SOC_E_xxx
...
soc_cmic_uc_msg_shutdown_halt(int unit) {
    ...
    /* Notify registered applications that shutdown is about to happen */
    for (uC = 0; uC < COUNTOF(soc->uc_msg_active); uC++) {
        for (appl = 0; appl <= MAX_MOS_MSG_CLASS; ++appl) {
            if (soc->uc_msg_appl_cb[uC][appl]) {
                (soc->uc_msg_appl_cb[uC][appl])(unit, uC, SOC_CMIC_UC_SHUTDOWN_STARTING,
    ...
    /* Halt each uC, and notify registered applications that the core is halted */
    for (uC = 0; uC < COUNTOF(soc->uc_msg_active); uC++) {
        soc_uc_reset(unit, uC);
        for (appl = 0; appl <= MAX_MOS_MSG_CLASS; ++appl) {
            if (soc->uc_msg_appl_cb[uC][appl]) {
                (soc->uc_msg_appl_cb[uC][appl])(unit, uC, SOC_CMIC_UC_SHUTDOWN_HALTED,
                                              soc->uc_msg_appl_cb_data[uC][appl]);
            }
            soc->uc_msg_appl_cb[uC][appl] = NULL;
        }
    }
