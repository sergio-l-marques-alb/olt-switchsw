/*
 * $Id: memscan.c,v 1.22 Broadcom SDK $
...
#if defined(BCM_TOMAHAWK_SUPPORT)
            && (!SOC_IS_TOMAHAWK(unit)) /* mem can be invalid in some latency_modes */
#endif
           ) {
                              ...
                              int *idx, int *idx_count)
{
   ...
   if (SOC_IS_TOMAHAWK(unit)) {
      ...
      soc_ifp_slice_mode_get = soc_th_ifp_slice_mode_get;
   }
#endif /* BCM_TOMAHAWK_SUPPORT */
   ...
   SOC_IF_ERROR_RETURN
       (soc_ifp_slice_mode_get(unit, pipe, cur_slice_num, &cfg_cur_slice_type,
   ...
   if (end_slice_num != cur_slice_num) {
       SOC_IF_ERROR_RETURN
           (soc_ifp_slice_mode_get(unit, pipe, end_slice_num,
                                   &cfg_end_slice_type,
...
_soc_mem_scan_thread(void *unit_vp)
{
    ...
    int     non_tcam_iterarions = 0;
    int     non_tcam_pass = 0;
#ifdef BCM_ESW_SUPPORT
    _soc_ser_correct_info_t spci;
    uint32  error_index;
#endif
    non_tcam_iterarions = soc_property_get(unit, spn_MEM_SCAN_NON_TCAM_ITERATIONS, 0);
    ...
    defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
        ...
        defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)                            
                    ...
                    if (entries_interval == soc->mem_scan_rate) {
                        ...
                        if (soc_feature(unit, soc_feature_ser_parity)) {
                            non_tcam_pass++;
                            if (non_tcam_pass == non_tcam_iterarions) {
