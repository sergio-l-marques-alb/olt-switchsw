...
int
ser_test_mem_index_remap(int unit, ser_test_data_t *test_data)
{
                         ...
                         int index, ser_test_data_t *test_data)
{
    ...
    (void)ser_test_mem_index_remap(unit, test_data);
...
#ifdef BCM_TRIDENT2_SUPPORT
    else if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2(unit)) {
             ...
             _soc_ser_test_t test_type, int *error_count)
{
    ...
    for (j = 0; j < numIndexesToTest; j++) {
        ...
        if (test_data->badData != test_data->field_buf[0]) {
            LOG_CLI((BSL_META_U(unit,
                                "SER failed. Injection Error for mem: %s field: %s\n"),
                     test_data->mem_name,
                     test_data->field_name));
            (*error_count)++;
            return SOC_E_FAIL;
        ...
        rv = ser_test_mem_read(unit, SER_TEST_MEM_F_READ_FUNC_VIEW , test_data); /* read func_view */
        ...
        sal_usleep(1000000); /*wait for memory and register ops to complete.*/
        if (SAL_BOOT_QUICKTURN) {
         ...
         */
        if (test_data->mem != test_data->mem_fv) {
            rv = (ser_test_mem_read(unit, 0, test_data)); /* rd error_inject view */
            if (SOC_FAILURE(rv)) {
                (*error_count)++;
#ifdef _SOC_SER_ENABLE_CLI_DBG
                LOG_ERROR(BSL_LS_SOC_COMMON,
