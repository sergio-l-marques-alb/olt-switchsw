...
/* $Id: arad_pp_llp_mirror.c,v 1.12 Broadcom SDK $
  ...
  )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  if (first_appear)
  {
	  /* get the desired reg and field by internal_vid_ndx */
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(local_port_ndx, ARAD_PP_PORT_MAX, ARAD_PP_PORT_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_LLP_MIRROR_PORT_DFLT_INFO, dflt_mirroring_info, 20, exit);
  ...
  )
{
  ...
  ARAD_PP_LLP_MIRROR_PORT_DFLT_INFO_clear(dflt_mirroring_info);
...
uint32
  ARAD_PP_LLP_MIRROR_PORT_DFLT_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_LLP_MIRROR_PORT_DFLT_INFO_verify()",0,0);
}
