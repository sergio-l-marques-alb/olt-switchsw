  ...
  )
{
   ...
   */
  hash_tbl = &Soc_petra_sw_db.soc_petra_device_sw_db[unit]->frwrd_ip.route_key_to_entry_id;
  ...
  )
{
  ...
  multi_set_info->init_info.nof_members = SOC_PB_EGR_NOF_TM_PORT_PROFILES;
  ...
  multi_set_info->init_info.set_entry_fun = soc_pb_sw_db_buffer_set_entry;
  multi_set_info->init_info.max_duplications = SOC_SAND_U32_MAX - 1;
  multi_set_info->init_info.member_size = SOC_PB_STACK_PRUN_BMP_LEN * sizeof(uint32);
  multi_set_info->init_info.nof_members = 2;
  multi_set_info->init_info.sec_handle = 0;
  multi_set_info->init_info.prime_handle = unit;
  /* will not work (petra code changed, wont work but will compile) */
  res = soc_sand_multi_set_create(
  ...
  )
{
  ...
  multi_set_info->init_info.max_duplications = SOC_SAND_U32_MAX - 1;
  multi_set_info->init_info.member_size = sizeof(uint32);
  multi_set_info->init_info.nof_members = SOC_PB_PMF_NOF_HEADER_PROFILES;
  multi_set_info->init_info.sec_handle = 0;
  multi_set_info->init_info.prime_handle = unit;
  /* will not work (petra code changed, wont work but will compile) */
  res = soc_sand_multi_set_create(
  ...
  multi_set_info->init_info.max_duplications = SOC_SAND_U32_MAX - 1;
  multi_set_info->init_info.member_size = sizeof(uint32);
  multi_set_info->init_info.nof_members = SOC_PB_PMF_NOF_PP_PORT_PROFILES;
  multi_set_info->init_info.sec_handle = 0;
  multi_set_info->init_info.prime_handle = unit;
  /* will not work (petra code changed, wont work but will compile) */
  res = soc_sand_multi_set_create(
  ...
  multi_set_info->init_info.max_duplications = SOC_SAND_U32_MAX - 1;
  multi_set_info->init_info.member_size = sizeof(uint32);
  multi_set_info->init_info.nof_members = SOC_PB_PMF_NOF_PGM_PROFILES;
  multi_set_info->init_info.sec_handle = 0;
  multi_set_info->init_info.prime_handle = unit;
  /* will not work (petra code changed, wont work but will compile) */
  res = soc_sand_multi_set_create(
  ...
  for (tcam_index = 0; tcam_index <SOC_PB_TCAM_MAX_NOF_LISTS ; ++tcam_index )
  {
    if (Soc_petra_sw_db.soc_petra_device_sw_db[unit]->tcam.tcam_db[tcam_index].valid)
    {
      res = soc_sand_sorted_list_destroy(
            unit,
            &(Soc_petra_sw_db.soc_petra_device_sw_db[unit]->tcam.tcam_db[tcam_index].priorities)
          );
      SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit); 
    }
  }
  ...
  )
{
  ...
  *buff_byte_size += size;
  ...
  for (tcam_index = 0; tcam_index <SOC_PB_TCAM_MAX_NOF_LISTS ; ++tcam_index )
  {
    if (dev_ptr->tcam.tcam_db[tcam_index].valid)
    {
      res = soc_sand_sorted_list_get_size_for_save(
          unit,
          &dev_ptr->tcam.tcam_db[tcam_index].priorities,
          &size
        );
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
      *buff_byte_size += size; 
  ...
  for (tcam_index = 0; tcam_index <SOC_PB_TCAM_MAX_NOF_LISTS ; ++tcam_index )
  {
    if (dev_ptr->tcam.tcam_db[tcam_index].valid)
    {
      *buff_byte_size += (sizeof(SOC_PB_TCAM_LOCATION) * dev_ptr->tcam.tcam_db[tcam_index].entry_id_to_location.init_info.table_size);     
    }
  }
    ...
    )
{
  ...
  *tcam_info = *(dev_ptr->tcam_info);
  ...
  for (tcam_index = 0; tcam_index <SOC_PB_TCAM_MAX_NOF_LISTS ; ++tcam_index )
  {
    if (dev_ptr->tcam.tcam_db[tcam_index].valid)
    {
      size = (sizeof(SOC_PB_TCAM_LOCATION) * dev_ptr->tcam.tcam_db[tcam_index].entry_id_to_location.init_info.table_size);
      if((additional_data - additional_data_orig + size) > additional_data_size) {
  ...
  )
{
  ...
  *(dev_ptr->tcam_info) = *tcam_info;
  ...
  for (tcam_index = 0; tcam_index <SOC_PB_TCAM_MAX_NOF_LISTS ; ++tcam_index )
  {
    if (dev_ptr->tcam.tcam_db[tcam_index].valid)
    {
      SOC_PETRA_ALLOC_ANY_SIZE(dev_ptr->tcam.tcam_db[tcam_index].location_tbl, SOC_PB_TCAM_LOCATION, dev_ptr->tcam.tcam_db[tcam_index].entry_id_to_location.init_info.table_size);
      sal_memcpy(dev_ptr->tcam.tcam_db[tcam_index].location_tbl, additional_data, (sizeof(SOC_PB_TCAM_LOCATION) * dev_ptr->tcam.tcam_db[tcam_index].entry_id_to_location.init_info.table_size));
      additional_data += (sizeof(SOC_PB_TCAM_LOCATION) * dev_ptr->tcam.tcam_db[tcam_index].entry_id_to_location.init_info.table_size);  
    }
  }
