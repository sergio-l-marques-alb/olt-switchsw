     ...
     SOC_SAND_OUT uint32 *base_address)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_pp_to_local_port_get(unit, core_id, port, &local_port));
     ...
     SOC_SAND_OUT uint32 *logical_port)
{
    ...
    SOCDNX_IF_ERR_EXIT(get_context_map_base_address(unit,core_id,port,&base_address));
  ...
  )
{
  ...
  uint32 context, channel; /* Reassemble context and recycle interface channel */
  ...
  res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, core_id, out_port_ndx, &end_q_pair);
  ...
  if (first_appear) { /* New VLAN, a VID was just created for it, add it to hardware */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 30, exit, ARAD_REG_ACCESS_ERR,
      READ_EPNI_MIRROR_VID_REGr(unit, core_id, reg_above_64_val)); /* read current hardware mapping */
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 40, exit, ARAD_REG_ACCESS_ERR,
      WRITE_EPNI_MIRROR_VID_REGr(unit, core_id, reg_above_64_val)); /* write the modified hardware mapping */
  }


  /* Set reassembly context (mirror channel) for port in EGQ_PORT_CONFIGURATION_TABLE_PCTm if not set already */
  res = soc_port_sw_db_pp_port_to_base_q_pair_get(unit,core_id, out_port_ndx,  &base_q_pair);
  ...
  if (!soc_mem_field32_get(unit, EGQ_PCTm, entry, MIRROR_ENABLEf)) { /* if disabled, need to allocate and set a context */
    ...
    res = alloc_reassembly_context_and_recycle_channel_unsafe(unit, core_id, out_port_ndx, &context, &channel); /* allocate reassembly context */
  ...
  )
{
  ...
  if (ref_count) { /* If the vid was found */
    ...
    if (!last_appear && !(config.mirror_command)) { /* no vlan id nor the default are mirrored for the port */
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit,core_id,out_port_ndx, channel, &context));
  ...
  )
{
  ...
  int mirror_profile_table_offset;
  uint8 port_vlan_exist, oam_is_init;
  uint32 context, channel;
  ARAD_PORT2IF_MAPPING_INFO port_mapping_info;
  ...
  res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, core_id, local_port_ndx, &end_q_pair);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);
  ...
  *prev_profile =  local_config.mirror_command;
  ...
  if (config->mirror_command && !(*prev_profile)) { /* port default will start being mirrored */
    ...
    uint32 base_q_pair;
    res = soc_port_sw_db_pp_port_to_base_q_pair_get(unit, core_id, local_port_ndx,  &base_q_pair);
    ...
    if (!soc_mem_field32_get(unit, EGQ_PCTm, entry_context, MIRROR_ENABLEf)) { /* if disabled, need to allocate and set a context */
      res = alloc_reassembly_context_and_recycle_channel_unsafe(unit, core_id, local_port_ndx, &context, &channel); /* allocate reassembly context */
  ...
  /* If mirroring of the port is disabled (setting a profile of 0 and the previous profile was not 0;
     if the port's reassembly context (mirror channel) is not used any more (outbound port not mirrored) and OAM is disabled, release it */
  res = arad_pp_sw_db_oam_enable_get(unit, &oam_is_init);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 140, exit);

  if (is_disable && !has_mirrored_vlans && !oam_is_init) {
    /* Remove reassembly context from hardware and deallocate it. */
    int enabled = 1;

    res = soc_port_sw_db_pp_port_to_base_q_pair_get(unit, core_id, local_port_ndx, &q_pair);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 150, exit);

    res = soc_port_sw_db_pp_port_to_out_port_priority_get(unit, core_id, local_port_ndx, &nof_pairs);
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 221, exit, ARAD_REG_ACCESS_ERR,get_recycling_port(unit,core_id,local_port_ndx, channel, &context));
