...
/* 1588 supported encapsulation */
   int ieee_1588_identification_cam[ARAD_PP_1588_IDENTIFICATION_CAM_TABLE_ENTRIES_NUM][9] =
    ...
    for(i = 0; i <= index_max ; i++) {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHB_PINFO_LBPm(unit,  MEM_BLOCK_ANY, i, tbl_data));
        soc_IHB_PINFO_LBPm_field32_set(unit, tbl_data, INGRESS_P2P_DELAYf, 0);
        SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, WRITE_IHB_PINFO_LBPm(unit, MEM_BLOCK_ANY, i, tbl_data));
  ...
  )
{
    ...
    for(i = 0; i < index_max + 1; i++) {
        ...
        if(0 != (info->flags & ARAD_PP_PTP_PORT_TIMESYNC_ONE_STEP_TIMESTAMP)) {
            /* 1588 protocol TC implementation, need to updated CF in following 4 messages */
            if(ARAD_PP_PTP_1588_PKT_SYNC        == msg_type ||
               ARAD_PP_PTP_1588_PKT_DELAY_REQ   == msg_type ||
               ARAD_PP_PTP_1588_PKT_PDELAY_REQ  == msg_type ||
               ARAD_PP_PTP_1588_PKT_PDELAY_RESP == msg_type) {
        ...
        } else if(0 != (info->flags & ARAD_PP_PTP_PORT_TIMESYNC_TWO_STEP_TIMESTAMP)) {
            /* 1588 protocol TC implementation, need to updated CF in following 4 messages */
            if(ARAD_PP_PTP_1588_PKT_SYNC        == msg_type ||
               ARAD_PP_PTP_1588_PKT_DELAY_REQ   == msg_type ||
               ARAD_PP_PTP_1588_PKT_PDELAY_REQ  == msg_type ||
        ...
        if( ((info->pkt_drop & (1 << msg_type)) == 0) && ((info->pkt_tocpu & (1 << msg_type)) == 0) ) {
            action_index = ARAD_PP_PTP_ACTION_FIELD_ACTION_INDEX_FWD;
        }
        else if( ((info->pkt_drop & (1 << msg_type)) != 0) ) {
            action_index = ARAD_PP_PTP_ACTION_FIELD_ACTION_INDEX_DROP;
        }
        else if( ((info->pkt_tocpu & (1 << msg_type)) != 0) ) {
            action_index = ARAD_PP_PTP_ACTION_FIELD_ACTION_INDEX_TRAP;
        }
        ...
        switch(profile) {
            case ARAD_PP_PTP_IN_PP_PORT_PROFILE_0:
                action_field = ACTIONf;
                break;
            case ARAD_PP_PTP_IN_PP_PORT_PROFILE_1:
                action_field = ACTION_1f;
                break;
            case ARAD_PP_PTP_IN_PP_PORT_PROFILE_2:
                action_field = ACTION_2f;
                break;
            case ARAD_PP_PTP_IN_PP_PORT_PROFILE_3:
  ...
  )
{
    ...
    res = soc_mem_read(unit, pinfo_flp_mem, MEM_BLOCK_ANY, local_port_ndx, entry);
    SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    soc_mem_field32_set(unit, pinfo_flp_mem, entry, IEEE_1588_PROFILEf, profile);
    res = soc_mem_write(unit, pinfo_flp_mem, MEM_BLOCK_ANY, local_port_ndx, entry);
    SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);


    if(0 != info->ptp_enabled) {

        /* External MAC is relevant only in case PTP is enabled */

        propval = soc_property_port_get_str(unit, local_port_ndx, spn_EXT_1588_MAC_ENABLE);    

        ext_mac_enable = FALSE;
        ...
        if(TRUE == ext_mac_enable) {
            ...
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, READ_IHB_PINFO_LBPm(unit,  MEM_BLOCK_ANY, local_port_ndx, entry));
            ...
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 40, exit, WRITE_IHB_PINFO_LBPm(unit, MEM_BLOCK_ANY, local_port_ndx, entry));
    ...
    /* enabling/disabling 1588 MAC Tx stamping */
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 10, exit, READ_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit,  MEM_BLOCK_ANY, local_port_ndx, entry));
    soc_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm_field32_set(unit, entry, IEEE_1588_MAC_ENABLEf, info->ptp_enabled ? 1 : 0);
    SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, WRITE_EPNI_PACKET_PROCESSING_PORT_CONFIGURATION_TABLE_PP_PCTm(unit, MEM_BLOCK_ANY, local_port_ndx, entry));
