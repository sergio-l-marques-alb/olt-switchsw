      ...
      uint32 *regs_val)
{
    ...
    ARAD_PP_DIAG_FLD_READ(&fld, prm_blk, prm_addr_msb, prm_addr_lsb, prm_fld_msb, prm_fld_lsb, 20);
  ...
  )
{
  ...
  if (pkt_trace & ARAD_PP_DIAG_PKT_TRACE_TRAP)
  {
    ...
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  130,  exit, ARAD_REG_ACCESS_ERR,READ_IHP_VTT_TRAPS_ENCOUNTEREDr(unit, SOC_CORE_ALL,  &field64));
    /* FLP traps */
    COMPILER_64_SET(field64,0,clear_val);
  ...
  )
{
  ...
  {
    if (SOC_IS_JERICHO(unit)) {
        ARAD_PP_DIAG_FLD_READ(&fld,ARAD_IHP_ID,27,0,47,0, 5);
    } else {
  ...
  )
{
  ...
  /* vlan-edit-info*/
  if (SOC_IS_JERICHO(unit)) {
      ARAD_PP_DIAG_FLD_READ(&fld,ARAD_IHB_ID,2,0,149,116,5);
  }
  else {
...
uint32
  arad_pp_diag_prge_first_instr_get(int unit, uint32 *first_instruction) {
    ...
    res = arad_pp_diag_dbg_val_get_unsafe(unit, ARAD_EPNI_ID, &fld, regs_val);
