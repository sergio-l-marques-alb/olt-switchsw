    ...
    for (in_dp_indx = 0; in_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++in_dp_indx)  {
      for (eth_mtr_dp_indx = 0; eth_mtr_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++eth_mtr_dp_indx) {
		  for (eth_mtr_invalid_indx = 0; eth_mtr_invalid_indx < 2; ++eth_mtr_invalid_indx) {
			  ...
			  SOCDNX_IF_ERR_EXIT(READ_IDR_MRPS_0_IN_DP_MAPPINGm(unit, MEM_BLOCK_ANY, entry_indx, &tbl_data));
			  ...
			  SOCDNX_IF_ERR_EXIT(WRITE_IDR_MRPS_0_IN_DP_MAPPINGm(unit, MEM_BLOCK_ANY, entry_indx, &tbl_data));
			  SOCDNX_IF_ERR_EXIT(WRITE_IDR_MRPS_1_IN_DP_MAPPINGm(unit, MEM_BLOCK_ANY, entry_indx, &tbl_data));
    ...
    for (in_dp_indx = 0; in_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++in_dp_indx)  {
      for (eth_mtr_dp_indx = 0; eth_mtr_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++eth_mtr_dp_indx) {
		  for (mtr_dp_indx = 0; mtr_dp_indx < ARAD_NOF_DROP_PRECEDENCE; ++mtr_dp_indx) {
			  ...
			  SOCDNX_IF_ERR_EXIT(READ_IDR_DROP_PRECEDENCE_MAPPINGm(unit, MEM_BLOCK_ANY, entry_indx, &tbl_data));
			  ...
			  SOCDNX_IF_ERR_EXIT(WRITE_IDR_DROP_PRECEDENCE_MAPPINGm(unit, MEM_BLOCK_ANY, entry_indx, &tbl_data));
