...
static uint32
  arad_pp_l3_src_bind_key_to_sem_key_unsafe (
    SOC_SAND_IN  int                                       unit,
    SOC_SAND_IN  ARAD_PP_SRC_BIND_IPV4_ENTRY                 *ipv4_info,
    SOC_SAND_IN  ARAD_PP_SRC_BIND_IPV6_ENTRY                 *ipv6_info,
    ...
    )
{
  ...
  num_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IP_SPOOF_DHCP;
  ...
  key->param[0].value[1] &= 0x1ff;

  num_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_IP_SPOOF_DHCP;
  key->param[1].nof_bits = num_bits;
    ...
    )
{
  ...
  if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) && lif_id)
  {
    ...
    key->param[1].value[0] = ARAD_PP_SRC_BIND_GET_16LSB(lif_id);
  }
...
STATIC uint32
    arad_pp_src_bind_lem_payload_build(
      SOC_SAND_IN  int                                     unit,
      SOC_SAND_IN  ARAD_PP_FRWRD_DECISION_INFO               *frwrd_decision,
    ...
    )
{
  ...
  if(frwrd_decision->additional_info.outlif.type != ARAD_PP_OUTLIF_ENCODE_TYPE_NONE) {
      payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
  }
  else if(frwrd_decision->additional_info.eei.type != ARAD_PP_EEI_TYPE_EMPTY) {
    ...
    )
{
  ...
  case ARAD_PP_SRC_BIND_TCAM_ID_IPV6_STATIC:
  ...
  )
{
  ...
  tcam_access_info.user_data                           = ARAD_PP_SRC_BIND_TCAM_ID_IPV6_STATIC;

  res = arad_tcam_access_create_unsafe(
          unit,
          ARAD_PP_SRC_BIND_TCAM_ID_IPV6_STATIC,
          &tcam_access_info,
          &success
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);      
    ...
    )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(src_bind_info->lif_ndx, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_SRC_BIND_IPV4_ENTRY, src_bind_info, 20, exit);
    ...
    )
{
  ...
  ARAD_PP_FRWRD_IPV4_VPN_ROUTE_KEY
    route_key;
  ARAD_PP_FRWRD_IPV4_UC_ROUTE_KEY
    uc_route_key;
  uint16 spoof_id;
  ...
  ARAD_PP_FRWRD_IPV4_UC_ROUTE_KEY_clear(&uc_route_key);
  ...
  if (!found)
  {
    if (src_bind_info->smac_valid == 0)
    {
      if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) &&
          ...
          if (src_bind_info->vsi_id) {
            ...
            res = arad_pp_frwrd_ipv4_vrf_route_add(unit, 
            ...
            res = arad_pp_frwrd_ipv4_uc_route_add(unit,
                                                  &uc_route_key,
                                                  src_bind_info->lif_ndx,
                                                  success);
          }
    ...
    else
    {
      ...
      /* SEM configuration */
      srcbind_arb_occ = arad_sw_db_src_bind_arb_occ_get(unit);
      if (!ignore_key)
      {
        if (!found_isem) {
  ...
  if (src_bind_info->smac_valid == 0)
  {
    if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) &&
        ...
        if (src_bind_info->vsi_id) {
          ...
          route_key.subnet.prefix_len = src_bind_info->prefix_len;
          route_key.subnet.ip_address = src_bind_info->sip;
          res = arad_pp_frwrd_ipv4_vrf_route_get(unit, 
                                               src_bind_info->vsi_id,
                                               &route_key, 
                                               TRUE,
                                               &fec_id,
          ...
          uc_route_key.subnet.prefix_len = src_bind_info->prefix_len;
          uc_route_key.subnet.ip_address = src_bind_info->sip;
          res = arad_pp_frwrd_ipv4_uc_route_get(unit, 
                                                 &uc_route_key, 
                                                 TRUE,
                                                 &fec_id,
                                                 &route_status,
    ...
    if (src_bind_info->smac_valid == 0)
    {
      if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit) &&
          ...
          if (src_bind_info->vsi_id) {
            ...
            route_key.subnet.prefix_len = src_bind_info->prefix_len;
            route_key.subnet.ip_address = src_bind_info->sip;
            res = arad_pp_frwrd_ipv4_vrf_route_remove(unit, 
                                                    src_bind_info->vsi_id,
                                                    &route_key, 
                                                     success);
          } else {
            ...
            uc_route_key.subnet.prefix_len = src_bind_info->prefix_len;
            uc_route_key.subnet.ip_address = src_bind_info->sip;
            res = arad_pp_frwrd_ipv4_uc_route_remove(unit, 
                                                     &uc_route_key, 
                                                     success);
          }
          SOC_SAND_CHECK_FUNC_RESULT(res, 13, exit); 
  ...
  *nof_entries = 0;
  ...
  for (read_index = 0; read_index < *nof_entries; ++read_index)
  {
    if ((read_keys[read_index].type == ARAD_PP_LEM_ACCESS_KEY_TYPE_IPV4_SPOOF_STATIC))
    {
      ...
      ipv4_status[valid_index] = ARAD_PP_FRWRD_IP_ROUTE_STATUS_COMMITED;

      if(read_vals[read_index].flags & ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED)
      {
        ipv4_status[valid_index] |= ARAD_PP_FRWRD_IP_ROUTE_STATUS_ACCESSED;
      }
  ...
  )
{
  ...
  for(indx = 0; indx < *nof_entries; ++indx)
  {
    ...
    if(ipv6_status)
    {
      ipv6_status[indx] = ARAD_PP_FRWRD_IP_ROUTE_STATUS_COMMITED;
    }
  }
exit:
  if(keys != NULL)
    ...
    )
{
  uint32
    res = SOC_SAND_OK;
  uint32
    flags = ARAD_PP_FRWRD_IP_LPM_ONLY;
    ...
    )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(src_bind_info->lif_ndx, SOC_DPP_DEFS_GET(unit, nof_local_lifs), ARAD_PP_LIF_ID_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_SRC_BIND_IPV6_ENTRY, src_bind_info, 20, exit);
    ...
    )
{
  ...
  res = arad_pp_flp_app_to_prog_index_get(unit,
                                          ARAD_PP_FLP_KEY_PROGRAM_ETHERNET_PON_DEFAULT_DOWNSTREAM,
                                          &prog_id);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  case ARAD_PP_COMPRESSION_TCAM_ID_SIP:
  ...
  )
{
  ...
  tcam_access_info.min_banks                           = 0;
  tcam_access_info.prefix_size                         = 4;
  tcam_access_info.user_data                           = ARAD_PP_COMPRESSION_TCAM_ID_SIP;

  res = arad_tcam_access_create_unsafe(
          unit,
          ARAD_PP_COMPRESSION_TCAM_ID_SIP,
          &tcam_access_info,
          &success
        );
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);      
    ...
    )
{
  ...
  sal_memcpy(&(key.key.ip6_compression), ip6_compression, sizeof(ARAD_PP_IPV6_COMPRESSED_ENTRY));
  key.vrf_ndx = 0;
    ...
    )
{
  ...
  *success = SOC_SAND_SUCCESS;
  sip_compression = ip6_compression->flags & ARAD_PP_IP6_COMPRESSION_SIP; 
  if (sip_compression) {
    ...
    )
{
  ...
  *success = SOC_SAND_SUCCESS;
  ...
  if (sip_compression) {
    ...
    /* TCAM */
    if (tcam_found && 
       (ip6_compression->flags & ARAD_PP_IP6_COMPRESSION_TCAM)) {
      ARAD_PP_CLEAR(&sem_key, ARAD_PP_ISEM_ACCESS_KEY, 1);
      arad_pp_ip6_compression_key_to_sem_key(unit, ip6_compression, &sem_key);
      res = arad_pp_isem_access_entry_remove_unsafe(
              unit,
    ...
    if (lem_found && 
        !ARAD_PP_SRC_BIND_IP6_COMPRESSION_PREFIX(ip6_compression->is_spoof, ip6_compression->ip6_tt_compressed_result) &&
        (ip6_compression->flags & ARAD_PP_IP6_COMPRESSION_LEM)) {
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_SRC_BIND_IPV4_ENTRY_verify()",0,0);
}
