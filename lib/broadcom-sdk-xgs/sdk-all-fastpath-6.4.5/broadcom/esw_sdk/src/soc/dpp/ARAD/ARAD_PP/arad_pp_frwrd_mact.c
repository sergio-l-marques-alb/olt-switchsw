...
/* 
 * $Id: arad_pp_frwrd_mact.c,v 1.68 Broadcom SDK $
...
#define ARAD_PP_FRWRD_MACT_ADD_TYPE_MAX                          (ARAD_PP_NOF_FRWRD_MACT_ADD_TYPES-1)
#define ARAD_PP_FRWRD_MACT_KEY_TYPE_MAX                          (ARAD_PP_NOF_FRWRD_MACT_KEY_TYPES-1)
...
#define ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_MAX              (ARAD_PP_NOF_FRWRD_MACT_TRAVERSE_ACTION_TYPES-1)
...
#define ARAD_PP_LEM_OUTLIF_NOF_BITS (19) 
...
/* Defines whether 1 in the rule bitmap means ignore. Don't care means that ignore is according to SOC_DPP_IS_EM_HW_ENABLE*/
typedef enum
{
  ...
  Arad_pp_error_desc_element_frwrd_mact[] =
{
   ...
   */
  {
    ARAD_PP_FRWRD_MACT_ADD_TYPE_OUT_OF_RANGE_ERR,
    "ARAD_PP_FRWRD_MACT_ADD_TYPE_OUT_OF_RANGE_ERR",
    "The parameter 'add_type' is out of range. \n\r "
    "The range is: 0 - ARAD_PP_NOF_FRWRD_MACT_ADD_TYPES-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
  ...
  {
    ARAD_PP_FRWRD_MACT_KEY_TYPE_OUT_OF_RANGE_ERR,
    "ARAD_PP_FRWRD_MACT_KEY_TYPE_OUT_OF_RANGE_ERR",
    "The parameter 'key_type' is out of range. \n\r "
    "The range is: 0 - ARAD_PP_NOF_FRWRD_MACT_KEY_TYPES-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
  ...
  },
  {
    ...
    "The range is: 0 - ARAD_PP_NOF_FRWRD_MACT_TRAVERSE_ACTION_TYPES-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
  {
  ...
  },
  {
    ...
    "The range is: 0 - ARAD_PP_NOF_FRWRD_MACT_PREFIX_APP_IDS-1.\n\r ",
    SOC_SAND_SVR_ERR,
    FALSE
  },
...
uint32
  arad_pp_frwrd_lem_sw_traverse_internal_unsafe(
    ...
    SOC_SAND_IN  ARAD_PP_LEM_ACCESS_KEY                     *rule_key,
    SOC_SAND_IN  ARAD_PP_LEM_ACCESS_KEY                     *rule_key_mask,
    SOC_SAND_IN  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE  *rule_val,
...
STATIC uint32
  _arad_pp_frwrd_lem_get_block_unsafe(
    ...
    SOC_SAND_IN ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE   *value_rule,
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CONFIGURATIONr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_ACCESS_SHAPERf,  access_in_clks));
    ...
    /* The following initializations are covering an issue that should be fixed. During the driver's boot these registers get dirty */
    if (SOC_IS_JERICHO(unit)) {
        /* Fix an error in Jericho. This register should be initialized to 0 by default */
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR, soc_reg_field32_modify(unit, PPDB_B_LARGE_EM_MASTER_JR_TO_AR_TRANSLATION_ERR_EVENT_COUNTERr, REG_PORT_ANY, LARGE_EM_MASTER_JR_TO_AR_TRANSLATION_ERR_EVENT_COUNTER_OVERFLOWf, 0));
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR, soc_reg_field32_modify(unit, PPDB_B_LARGE_EM_MASTER_JR_TO_AR_TRANSLATION_ERR_EVENT_COUNTERr, REG_PORT_ANY, LARGE_EM_MASTER_JR_TO_AR_TRANSLATION_ERR_EVENT_COUNTERf, 0));
    ...
    )
{
  ...
  if(mac_entry_value->frwrd_info.forward_decision.additional_info.eei.type != ARAD_PP_EEI_TYPE_EMPTY){
      payload->flags = ARAD_PP_LEM_ACCESS_ASD_EEI;
  }
  else if(mac_entry_value->frwrd_info.forward_decision.additional_info.outlif.type != ARAD_PP_OUTLIF_ENCODE_TYPE_NONE) {
      payload->flags = (mac_entry_value->frwrd_info.forward_decision.additional_info.outlif.type == ARAD_PP_OUTLIF_ENCODE_TYPE_RAW_INVALID) ?
          ARAD_PP_LEM_ACCESS_ASD_OUTLIF_INVALID : ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
  }
  else if(mac_entry_value->frwrd_info.forward_decision.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC) {
      /* When decision type is FEC, LIF valid bit should be unset. */
      payload->flags = ARAD_PP_LEM_ACCESS_ASD_NONE;
  }
  else{
      /* When in-LIF is null, the learned entry in HW has payload with out-LIF valid bit set.
       * We would like to build the payload the same way it is learned in HW, so that in case of CPU learning,
       * the payload will not change and cause transplant events. */ 
      payload->flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
  }

  if (mac_entry_value->group != 0) {
      if (payload->flags == ARAD_PP_LEM_ACCESS_ASD_OUTLIF || payload->flags == ARAD_PP_LEM_ACCESS_ASD_NONE) {
          arad_pp_frwrd_mact_payload_build_group(mac_entry_value,payload);
      }
      else { /* cannot set group in this format */
...
STATIC uint32
  arad_pp_frwrd_mact_payload_mask_build(
      SOC_SAND_IN  int                   unit,
      SOC_SAND_IN  ARAD_PP_FRWRD_MACT_ENTRY_VALUE  *entry_val,
      SOC_SAND_IN  ARAD_PP_FRWRD_MACT_ENTRY_VALUE  *entry_val_mask,
      ...
      SOC_SAND_IN  uint32                    mask_val /*1: 0 is consider, 0: 1 to consider*/
    )
{
  ARAD_PP_LEM_ACCESS_PAYLOAD
    payload,
    payload_lem_mask;
  ARAD_PP_FRWRD_MACT_ENTRY_VALUE  
    tmp_entry_val;
  uint8
    payload_valid=FALSE,
    is_eei_mc = 0,
...
#ifdef BCM_88660_A0
  /* Build the payload and payload mask for SLB compare or replace. */
  if (SOC_IS_ARADPLUS(unit) && (compare_mask & (SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_COMPARE_SLB | 
    ...
    /* Counter-Pointer: */
    /*   D S X 00 XXXXXX XX XXXXXXXXXXXX E -COUNTER-POINTER- */ 

    /* > 4 4 4 33 333333 33 222222222211 1 11111110000000000 */
    /* > 2 1 0 98 765432 10 987654321098 7 65432109876543210 */

    /* Note the format differenece (for counter-pointer bit 0 is one char to the right). */
    
    /* SLB entries: */
    /*   D S X 01 XXXXXX XX XXXXXXXXXXXXXXX X LAG-MEMBER,GRP */ 
    /*   D S X 10 XXXXXX XX ------FEC------ X XXXXXXXXXXXXXX */ 
    /*   D S X 11 XXXXXX XX ------FEC------ X LAG-MEMBER,GRP */
     
    /* > 4 4 4 33 333333 33 222222222211111 1 11110000000000 */
    /* > 2 1 0 98 765432 10 987654321098765 4 32109876543210 */

    /* > 1 0 0 00 000000 33 222222222211111 1 11110000000000 */
    /* > 0 9 8 76 543210 10 987654321098765 4 32109876543210 */
    /* Remark - In the spec the 15th bit of LAG (bit 14) is not ignored. */
    /*          However because LAG member and group is at most 14 bits, it should be ignored. */

    /* Legend: */
    /* > - A visualisation aid line (not format line). */
    /* X - ignore */
    /* D - Dynamic (ignore) */
    /* S - Static (ignore) */
    /* E - 0/1 */
    ...
    /* Exactly one of compare and replace should be selected. */
    ...
    /* These checks are only done for compare. */
    /* For replace we allow updating anything in the FEC/LAG part. */
    if (compare_mask & SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_COMPARE_SLB) {
      /* If mask.match_X_entries == 0 or match_X_entries == 0, then X's mask must be 0. */
      ...
      /* At least one item must be valid and unmasked (match_X_entries = mask.match_X_entries = 1). */
      /* The point is to avoid hitting the 00 entries. */
      if (!((match_lag && match_lag_mask) || (match_fec && match_fec_mask)))
      {
        SOC_SAND_SET_ERROR_CODE(0, 4000, exit);
      }
    } else /* if (compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_REPLACE_SLB) */ {
      /* In case of an update, we do not update the SLB format part. */
      match_lag_mask = 0;
      match_fec_mask = 0;
    }

    /* Set data. */
    /* REMARK: We set the value anyway even if the data is masked (should not affect anything). */

    /* It is valid, however we don't want the default parsing given using payload_valid == TRUE. */
    payload_valid = FALSE;
    ...
    /* We go from low bit to high bit. */
  ...
  /* check input validity if EEI given then destination has to be FEC or MC and not masked */
  if(compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO) {

      /* check dest-type value is MC or FEC */
      if(entry_val->frwrd_info.forward_decision.additional_info.eei.type != SOC_PPC_EEI_TYPE_EMPTY) {
          /* check that dest-type is considered */
          if(!(compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_TYPE)){
              SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_TYPE_OUT_OF_RANGE_ERR, 10, exit);
          }
          if(entry_val->frwrd_info.forward_decision.type == ARAD_PP_FRWRD_DECISION_TYPE_FEC){
              is_eei_fec = 1;
          }
          else if(entry_val->frwrd_info.forward_decision.type == ARAD_PP_FRWRD_DECISION_TYPE_MC){
              is_eei_mc = 1;
          }
          else {/* for EEI,dest-type has to be supplied*/
              SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_TYPE_OUT_OF_RANGE_ERR, 20, exit);
          }
          /* check that group is not considered for EEI*/
          if(compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_GROUP){
              SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_TYPE_OUT_OF_RANGE_ERR, 25, exit);
          }

      }
      else { /* not EEI format so either dest-type and value are valid or none of them */

          if(((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_TYPE)) && (!(compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_TYPE))){
  ...
  /* destination */  
  if ((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL) != 0)
  {
    if (is_eei_mc) {/* is EEI-MC */
        /* payload_mask[0] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_MC_NOF_BITS - 1, 0);*/
    ...
    else if (is_eei_fec) {/* is EEI-FEC */
        /*payload_mask[0] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_FEC_NOF_BITS - 1, 0);*/
        tmp = ~entry_val_mask->frwrd_info.forward_decision.dest_id;
        SHR_BITCOPY_RANGE(&payload_mask[0],0,&tmp,0,SOC_DPP_DEFS_GET(unit, fec_nof_bits));
    }
    else {/* not eei type */
        /* if destination is lag then mask member-id*/
        if (entry_val->frwrd_info.forward_decision.type == SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG)
        {
            ...
            /*payload_mask[0] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_DEST_NOF_BITS - 1, 0);*/
            tmp = ~entry_val_mask->frwrd_info.forward_decision.dest_id;
            SHR_BITCOPY_RANGE(&payload_mask[0],0,&tmp,0,ARAD_PP_LEM_ACCESS_DEST_NOF_BITS);
            payload_mask[0] |= SOC_SAND_BITS_MASK(13,10-lag_mode);/* mask member ID*/
        }
        else
        {
            /* payload_mask[0] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_DEST_NOF_BITS - 1, 0); */
  ...
  /* additional info, EEI / outLIF */
  if ((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO) != 0)
  {
      ...
      if (is_eei_mc) {/* is EEI-MC */
          res = soc_sand_bitstream_set_any_field(
                  ...
                  ARAD_PP_LEM_EEI_NOF_BITS+1, /* include type */
                  (payload_mask)
                );
          ...
          res = soc_sand_bitstream_set_any_field(
                  ...
                  ARAD_PP_LEM_EEI_NOF_BITS+2, /* include type */
                  (payload_mask)
                );
          ...
          /* entry type set only when additional value is set */
          res = soc_sand_bitstream_set_any_field(
                  &tmp,
                  ARAD_PP_LEM_LIF_VALUE_LSB,
                  ARAD_PP_LEM_OUTLIF_NOF_BITS+1, /* include valid*/
                  (payload_mask)
                );
  ...
  if ((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC) != 0)
  {
    payload_mask[1] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_IS_DYN_LSB, ARAD_PP_LEM_ACCESS_IS_DYN_LSB);
  }

  if ((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA) != 0)
  {
    payload_mask[1] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_DROP_SA, ARAD_PP_LEM_ACCESS_DROP_SA);
  }

  if ((compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_GROUP) != 0)
  {
    /* set group */
    /* payload_mask[1] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_GROUP_MSB,ARAD_PP_LEM_ACCESS_GROUP_LSB);*/
      tmp = ~(entry_val_mask->group);
      SHR_BITCOPY_RANGE(&payload_mask[1],ARAD_PP_LEM_ACCESS_GROUP_LSB,&tmp,0,3);
    /* set also entry type so not conflict with EEI entry */
    payload_mask[1] &= SOC_SAND_ZERO_BITS_MASK(ARAD_PP_LEM_ACCESS_ENTRY_TYPE_MSB, ARAD_PP_LEM_ACCESS_ENTRY_TYPE_LSB);
  }

  if (compare_mask & (
                      ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DROP_SA|
                      ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC|
                      ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ADDITIONAL_INFO|
                      ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL|
                      ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_GROUP
                     )
     )
  {
    payload_valid = TRUE;
  ...
  if (payload_valid)
  {
    ...
    /* just to prevent fail on parsing */
    sal_memcpy(&tmp_entry_val,(entry_val),sizeof(ARAD_PP_FRWRD_MACT_ENTRY_VALUE));
    if(tmp_entry_val.frwrd_info.forward_decision.type == ARAD_PP_NOF_FRWRD_DECISION_TYPES)
    {
      tmp_entry_val.frwrd_info.forward_decision.type = ARAD_PP_FRWRD_DECISION_TYPE_UC_PORT;
    }
    ...
    )
{
  ...
  {
    mask += ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_DEST_VAL;
  }
    ...
    )
{
  ...
  {
  case ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR:
  ...
  case ARAD_PP_FRWRD_MACT_KEY_TYPE_IPV4_MC:
  ...
  case ARAD_PP_NOF_FRWRD_MACT_KEY_TYPES:
    key->type = ARAD_PP_LEM_ACCESS_KEY_TYPE_IPV4_MC;
    key->nof_params = ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_IPV4_MC;
    key->param[0].value[0] = SOC_SAND_GET_BITS_RANGE(mac_entry_key->key_val.ipv4_mc.dip, ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IPV4_MC-1, 0);
    key->param[0].value[1] = 0;
  ...
  if (out_of_range) {
    SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_KEY_TYPE_OUT_OF_RANGE_ERR, 20, exit);
  }
    ...
    )
{
  ...
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_clear(&mac_entry_key);
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_clear(&mac_entry_key_rule);
...
uint32
  arad_pp_frwrd_mact_key_parse(
      SOC_SAND_IN  int                   unit,
      SOC_SAND_IN  ARAD_PP_LEM_ACCESS_KEY       *key,
      SOC_SAND_OUT ARAD_PP_FRWRD_MACT_ENTRY_KEY *mac_entry_key
    )
{
  uint32
    reg_ndx,
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_MAC:
    mac_entry_key->key_type = ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR;
    for (mac_add_ndx = 0; mac_add_ndx < SOC_SAND_PP_MAC_ADDRESS_NOF_U8; ++mac_add_ndx)
    {
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_IPV4_MC:
    mac_entry_key->key_type = ARAD_PP_FRWRD_MACT_KEY_TYPE_IPV4_MC;
    mac_entry_key->key_val.ipv4_mc.dip = key->param[0].value[0] + ARAD_PP_FRWRD_MACT_IPV4_MC_DIP_BASE_VAL;
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_SLB:
    if (SOC_IS_ARADPLUS(unit) && soc_property_get(unit, spn_RESILIENT_HASH_ENABLE, 0)) {
      ...
      SOC_SAND_VERIFY(key->nof_params == ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_SLB);
      SOC_SAND_VERIFY(key->param[0].nof_bits == ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_SLB);
      SOC_SAND_VERIFY(key->param[1].nof_bits == ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_SLB);
      SOC_SAND_VERIFY(key->param[2].nof_bits == ARAD_PP_LEM_ACCESS_KEY_PARAM2_IN_BITS_FOR_SLB); 
      SOC_SAND_VERIFY(key->param[3].nof_bits == 0); 
      SOC_SAND_VERIFY(key->param[4].nof_bits == 0);  
      */ 

      mac_entry_key->key_type = ARAD_PP_FRWRD_MACT_KEY_TYPE_SLB;
...
#ifdef PLISIM
   mac_entry_key->key_type = ARAD_PP_NOF_FRWRD_MACT_KEY_TYPES;
#endif
...
STATIC uint32
  arad_pp_frwrd_mact_rule_from_key_build(
      ...
      SOC_SAND_IN ARAD_PP_LEM_ACCESS_KEY                  *rule_key,
      SOC_SAND_IN ARAD_PP_LEM_ACCESS_KEY                  *rule_key_mask,
      SOC_SAND_OUT ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE *rule
    )
{
  uint32
    res;
  ...
  SOC_SAND_CHECK_NULL_INPUT(rule);
  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE_clear(rule);
    ...
    )
{
  ...
  ARAD_PP_FRWRD_MACT_ENTRY_VALUE_clear(mac_entry_value);
  ...
  mac_entry_value->accessed = (payload->flags & ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED)?1:0;
  ...
  )
{
  ...
  switch(app_id)
  {
    case ARAD_PP_FRWRD_MACT_PREFIX_APP_ID_OMAC:
      cur_app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_ETH(unit);
      break;
    case ARAD_PP_FRWRD_MACT_PREFIX_APP_ID_OMAC_2_VMAC:
      cur_app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_OMAC_2_VMAC;
      break;
    case ARAD_PP_FRWRD_MACT_PREFIX_APP_ID_VMAC:
      cur_app_id = ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_VMAC;
      break;
    case ARAD_PP_FRWRD_MACT_PREFIX_APP_ID_VMAC_2_OMAC:
...
/*********************************************************************
*     Convert  L2 entry from PPC MAC entry to a 
*     CPU request payload register format
*     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
...
)
{
    ...
    if (insert)
    {
        switch(add_type)
        {
        case ARAD_PP_FRWRD_MACT_ADD_TYPE_INSERT:
          access_cmd = ARAD_PP_LEM_ACCESS_CMD_INSERT;
            break;

        case ARAD_PP_FRWRD_MACT_ADD_TYPE_TRANSPLANT:
          access_cmd = ARAD_PP_LEM_ACCESS_CMD_TRANSPLANT;
            break;

        case ARAD_PP_FRWRD_MACT_ADD_TYPE_LEARN:
          access_cmd = ARAD_PP_LEM_ACCESS_CMD_LEARN;
            break;

        case ARAD_PP_FRWRD_MACT_ADD_TYPE_REFRESH:
          access_cmd = ARAD_PP_LEM_ACCESS_CMD_REFRESH;
            break;

        default:
          SOC_SAND_SET_ERROR_CODE(ARAD_PP_FRWRD_MACT_ADD_TYPE_OUT_OF_RANGE_ERR, 10, exit);
        }
    }
    else
    {
  ...
  )
{
 uint32
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(add_type, ARAD_PP_FRWRD_MACT_ADD_TYPE_MAX, ARAD_PP_FRWRD_MACT_ADD_TYPE_OUT_OF_RANGE_ERR, 10, exit);
  res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_verify(unit, mac_entry_key);
  SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_VALUE, mac_entry_value, 30, exit);
  ...
  )
{
 uint32
  ...
  res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_verify(unit, mac_entry_key);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_VALUE, mac_entry_value, 20, exit);
...
/* Remove a mact entry only from the SW shadow and not from the HW table */
 ...
 *     rule, and forule_keyr matching entries perform the given action.
 *     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
  ...
  )
{
   ...
   */
/*
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE, rule, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION, action, 20, exit);
*/
  ARAD_PP_DO_NOTHING_AND_EXIT;
...
/*    access_in_clks,*/
  ...
  ARAD_PP_FRWRD_MACT_TRAVERSE_STATUS_INFO_clear(status);
...
/*  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CONFIGURATIONr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_ACCESS_SHAPERf, &access_in_clks));*/
  ...
  status->time_to_finish.sec = time_remaning_ms / ARAD_PP_FRWRD_MACT_NOF_MS_IN_SEC;
  status->time_to_finish.mili_sec = time_remaning_ms % ARAD_PP_FRWRD_MACT_NOF_MS_IN_SEC;
  ...
  {
    status->action.type = ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_REMOVE;
  }
  else if (fld_val == 0x1)
  {
    status->action.type = ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_RETRIEVE;
  }
  else
  {
    if ((COMPILER_64_LO(tbl_data.action_transplant_payload_mask) == 0x0) && (COMPILER_64_HI(tbl_data.action_transplant_payload_mask) == 0x0))
    {
      status->action.type = ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_COUNT;
    }
    else
    {
      status->action.type = ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_UPDATE;
      /*
  ...
  )
{
  ...
  switch (mode->state) {
  case ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_NORMAL:
  /*case ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_AGGREGATE:*/ 
  ...
  break;
  case ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_RESET:
      arad_pp_frwrd_lem_traverse_db_invalidate(unit);
  break;
  case ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_RUN:
...
/*********************************************************************
*     Traverse the SW shadow MAC Table entries (in specified range), and
  ...
  )
{
    ...
    ARAD_PP_LEM_ACCESS_KEY                  *lem_key        = NULL; /* was key */
    ARAD_PP_FRWRD_MACT_ENTRY_KEY            mac_key;
    ARAD_PP_FRWRD_MACT_ENTRY_VALUE          mac_entry_value;
    ...
    ARAD_PP_LEM_ACCESS_KEY_clear(&rule_key);
    ARAD_PP_LEM_ACCESS_KEY_clear(&rule_key_mask);
    ARAD_PP_FRWRD_MACT_ENTRY_KEY_clear(&mac_key);
    ARAD_PP_FRWRD_MACT_ENTRY_VALUE_clear(&mac_entry_value);
    ...
    payload_array = (ARAD_PP_LEM_ACCESS_PAYLOAD*) sal_alloc( sizeof(ARAD_PP_LEM_ACCESS_PAYLOAD) * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
    if (payload_array == NULL)
    {
        SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 20, exit);
    }
    sal_memset(payload_array, 0, sizeof(ARAD_PP_LEM_ACCESS_PAYLOAD) * CHIPSIM_MAX_TRAVERSE_SIZE);

    lem_key_array = (ARAD_PP_LEM_ACCESS_KEY*) sal_alloc( sizeof(ARAD_PP_LEM_ACCESS_KEY) * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
    if (lem_key_array == NULL)
    {
        SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 30, exit);
    }
    sal_memset(lem_key_array, 0, sizeof(ARAD_PP_LEM_ACCESS_KEY) * CHIPSIM_MAX_TRAVERSE_SIZE);
    ...
    *nof_entries = 0;
    for (i = 0; i < nof_total_entries; i++)
    {
        ...
        /* check key if key is not totally masked */
        if (rule->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR
                && rule->key_rule.mac.fid_mask == 0
                && SOC_SAND_PP_MAC_ADDRESS_IS_ZERO(rule->key_rule.mac.mac_mask))
        {
            check_key = 0;
        }
        else if (rule->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_IPV4_MC
        ...
        else
        {
            ...
            default:
                SOC_SAND_SET_ERROR_CODE(
                        ARAD_PP_FRWRD_MACT_KEY_TYPE_OUT_OF_RANGE_ERR, 20, exit);
            }
        }
        ...
        ARAD_COPY(&(mact_keys[*nof_entries]), &(mac_key),         ARAD_PP_FRWRD_MACT_ENTRY_KEY,   1);
        ARAD_COPY(&(mact_vals[*nof_entries]), &(mac_entry_value), ARAD_PP_FRWRD_MACT_ENTRY_VALUE, 1);
        (*nof_entries)++;
...
{
    uint32
        matched_dyn_entries             = 0,
        matched_stat_entries            = 0;
    uint32                                  
	    res                             = SOC_SAND_OK;
    ...
    *nof_entries = 0;
    ...
    /*     Split rule_val into 2 rules: one for dynamic entries (which is executed immediately) and
     *     second for static entries */
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            int                                             hw_access;
            ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE          new_rule                        = *rule;
            uint8                                           do_dynamic                      = 1,
                                                            do_static                       = 1;

            if (rule->value_rule.compare_mask & SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC )
            {
                if (rule->value_rule.val.aging_info.is_dynamic == TRUE)
                    do_static = 0;
                else  /*(rule_val->val.aging_info.is_dynamic == FALSE)*/
                    do_dynamic = 0;
            }
            ...
            if (do_dynamic)
            {
                /* just consider dynamic entries*/
                new_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                new_rule.value_rule.val.aging_info.is_dynamic = TRUE;
                soc_hw_set_immediate_hw_access(unit, &hw_access);
                /* Get dynamic entries block from HW. Will be executed immediately.*/
                res = _arad_pp_frwrd_mact_get_block_unsafe(unit, &new_rule, block_range, mact_keys, mact_vals, &matched_dyn_entries);
                SOC_SAND_CHECK_FUNC_RESULT(res, 2000, exit);

                /* Restore the original value of the HwLogDoStandardReadWrite*/
                soc_hw_restore_immediate_hw_access(unit, hw_access);
            }
            /*Entries are Static --> HW Log */
            if  (do_static)
            {
                /* just consider static entries*/
                new_rule.value_rule.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                new_rule.value_rule.val.aging_info.is_dynamic = FALSE;

                SOC_SAND_TBL_ITER_SET_BEGIN(&block_range->iter);

                 /* Get static entries block from SW Shadow in order to get correct results. Will be executed immediately.*/
                res = arad_pp_frwrd_mact_get_sw_block_unsafe(unit, &new_rule, block_range, (mact_keys + matched_dyn_entries), (mact_vals + matched_dyn_entries), &matched_stat_entries);
                SOC_SAND_CHECK_FUNC_RESULT(res, 2010, exit);

            }

            *nof_entries = matched_dyn_entries + matched_stat_entries;
...
/*    entries_to_scan,*/
  ...
  uint32
    reply_ready;
  ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION
    action;
  uint32
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
                       "Entering arad_pp_frwrd_mact_get_block_unsafe \n\r")));
#endif
...
/*
  ...
  {
    entries_to_scan = DPP_PP_LEM_NOF_ENTRIES(unit);
  }
*/

  /* if input iter is zero need to activate flush-db/traverse */
  /*  if we start new iteration stop previous one if it's working*/
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
    ...
    action.type = ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_RETRIEVE;
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
        ...
        /* 1. clear all exist replies */
        if(current_index != 0) {
            /* make it finish as soon as possible */
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
                       "After arad_pp_frwrd_mact_traverse_internal_unsafe: traverse_nof_matched_entries %d \n\r"), traverse_nof_matched_entries));
#endif

    /* don't sleep lazy! and start get results */
  }

  /* prepare rules for SW compare */

  /* check key if key is not totally masked */
  if(rule->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR && rule->key_rule.mac.fid_mask == 0 && SOC_SAND_PP_MAC_ADDRESS_IS_ZERO(rule->key_rule.mac.mac_mask)) {
      check_key = 0;
  }
  else if(rule->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_IPV4_MC && rule->key_rule.ipv4_mc.dip_mask == 0 && rule->key_rule.ipv4_mc.fid_mask == 0) {
      check_key = 0;
  }
  else{
      check_key = 1;
  ...
  reply_ready = 1;
  current_index = 1;
  /* loop while there are events, or traverse no at the end*/
...
#endif
    /* if what in FIFO is enough for this read, then stop flush run and read fifo*/
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
...
#endif
        /* wait till FIFO is full or flush is done*/
        current_index = 1;
        fifo_size = 0;
        ...
        if(block_range->entries_to_act == ARAD_PP_MACT_REPLY_FIFO_SIZE + nof_valid_entries) {
            while(fifo_size < ARAD_PP_MACT_REPLY_FIFO_SIZE && current_index !=0) {
                /* The reply fifo threshold for flush machine is 127, so we expect the fifo_size to be 127 or less.
                 * There are 2 exceptions to this assumption:
                 * 1. MACT shuffling during flush machine operation may cause fifo_size to be 128. 
                 * 2. Static configuration (CPU) after the flush machine already filled the fifo with 127 entries 
                 *    will cause fifo_size 128.
                 *  
                 * In order to support these exceptions we should not assume max fifo_size 127. It may be 128! */
                ...
                /* Read the current index only after the fifo size */
                SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CURRENT_INDEXr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_CURRENT_INDEXf, &current_index));
                fifo_size += 1;
            }
#if ARAD_PP_MACT_DEBUG_PRINT
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
            ...
            /* if fifo full and it's enough for this read then stop flush db */
            if(fifo_size >= ARAD_PP_MACT_REPLY_FIFO_SIZE) {
                if(current_index==0) {
                    /* index zero, then mark reach end*/
                    SOC_SAND_TBL_ITER_SET_END(&(block_range->iter));
                }
                else{
                ...
                /* stop report hit, make sure no new reports*/
    ...
    /* Read the current index before the reply ready */
    ...
    /* read and parse reply to lem structs */
      ...
      /* next, parse from lem struct to PPD struct */
      /* ultra check that only relevant keys returned */

      /*
       *	Check if the key follows the rules first for MAC and IPv4 MC compatible addresses
       */
      /* traverse returned as reply of type defrag */
      ...
      if(check_key) {
          ...
          if (!is_key_for_rule) { /* not matching key */
              continue;
          }
      }
      res = arad_pp_frwrd_mact_payload_convert(
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
        ...
        ARAD_COPY(&(mact_keys[nof_valid_entries]), &(mac_key), ARAD_PP_FRWRD_MACT_ENTRY_KEY, 1);
        ARAD_COPY(&(mact_vals[nof_valid_entries]), &(mac_entry_value), ARAD_PP_FRWRD_MACT_ENTRY_VALUE, 1);
        ...
        if(nof_valid_entries + filtered_entries >= block_range->entries_to_act)
        {
#if ARAD_PP_MACT_DEBUG_PRINT
          LOG_INFO(BSL_LS_SOC_FORWARD,
                   (BSL_META_U(unit,
...
#endif
          /* set iter to HW place */
          SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CURRENT_INDEXr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_CURRENT_INDEXf, &current_index));
          /* block_range->iter = current_index;*/
          *nof_entries = nof_valid_entries;
            /* read counter to clear it */
#if ARAD_PP_MACT_DEBUG_PRINT
  ...
  )
{
    ...
    SOC_SAND_CHECK_NULL_INPUT(rule_key);
    SOC_SAND_CHECK_NULL_INPUT(rule_key_mask);
    /* SOC_SAND_CHECK_NULL_INPUT(value_rule); if null ignore */
    ...
    *nof_matched_entries = 0;
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        ...
        {
            int                                             hw_access;
            ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE    new_rule_val;
            uint8                                           do_dynamic                      = TRUE,
                                                            do_static                       = TRUE;

            SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE_clear(&new_rule_val);

            if (rule_val)
            {
                new_rule_val = *rule_val;

                if (rule_val->compare_mask & SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC )
                {
                    if (rule_val->val.aging_info.is_dynamic == TRUE)
                        do_static = FALSE;
                    else  /*(rule_val->val.aging_info.is_dynamic == FALSE)*/
                        do_dynamic = FALSE;
                }
            ...
            {
                /* just consider dynamic entries*/
                new_rule_val.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                new_rule_val.val.aging_info.is_dynamic = TRUE;
                soc_hw_set_immediate_hw_access(unit, &hw_access);
                /* Traverse command for DYNAMIC entries will be executed immediately.*/
                res = _arad_pp_frwrd_lem_traverse_by_mac_limit_per_tunnel_internal_unsafe(unit, rule_key, rule_key_mask, &new_rule_val, action, &matched_dyn_entries);
                /* Restore the original value of the HwLogDoStandardReadWrite*/
                soc_hw_restore_immediate_hw_access(unit, hw_access);
            ...
            /*Handle static entries --> HW Log */
            if  (do_static)
            {
                uint32 dummy = 0;
                /* just consider static entries*/
                new_rule_val.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                new_rule_val.val.aging_info.is_dynamic = FALSE;

                /* Save the traverse command for STATIC entries into the HW Log. Will be executed later. Don't wait_till_finish */
                res = _arad_pp_frwrd_lem_traverse_by_mac_limit_per_tunnel_internal_unsafe(unit, rule_key, rule_key_mask, &new_rule_val, action, &dummy);
                SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

                /* Update the traverse command into the SW Shadow. Will be executed immediately.*/
                res = arad_pp_frwrd_lem_sw_traverse_internal_unsafe(
                        unit,
                        rule_key,
                        rule_key_mask,
                        rule_val/*&new_rule_val*/,
                        action,
                        &matched_stat_entries
                      );
                SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);

            }

            *nof_matched_entries = matched_dyn_entries + matched_stat_entries;
  ...
  )
{
  ...
  ARAD_PP_LEM_ACCESS_KEY_ENCODED
    key_in_buffer_mask,
    key_in_buffer;
  ARAD_PP_FRWRD_MACT_TRAVERSE_STATUS_INFO
    status;
  uint8
    traverse_mode;
  soc_reg_t
 ...
 /* compare for key in bits 69-54 (FID)*/
  ...
  SHR_BITCOPY_RANGE(&tbl_data.compare_key_20_data,16, key_in_buffer.buffer, (lem_key_length-4), 4); /* 4 is hard coded in the key construction */
  /* set mask*/
  tbl_data.compare_key_20_mask = 0;
  SHR_BITCOPY_RANGE(&tbl_data.compare_key_20_mask,0, key_in_buffer_mask.buffer, 48, 15);

  /* ignore bit 16 has no meaning */
  SHR_BITCOPY_RANGE(&tbl_data.compare_key_20_mask,15, &one_val, 0, 1);

  SHR_BITCOPY_RANGE(&tbl_data.compare_key_20_mask,16, key_in_buffer_mask.buffer, (lem_key_length-4), 4); /* 4 is hard coded in the key construction */
  /* set payload */
  ...
  if (rule_val != NULL) 
  {
      ...
      tbl_data.compare_accessed_mask = (rule_val->compare_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED)?0:1;
  }
  else
  {
  ...
  tbl_data.action_transplant_accessed_clear = SOC_SAND_BOOL2NUM(action->update_mask & ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED);
  if (action->type == ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_REMOVE)
  {
  ...
  if(action->type == ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_RETRIEVE)
  {
  ...
  else if (action->type == ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_UPDATE)
  {
    res = arad_pp_frwrd_mact_payload_mask_build(
            unit,
            &action->updated_val,
  ...
  /* get next available place in flush db */
  ...
  /*just write rule, without activate flush machine */
  if (traverse_mode == ARAD_PP_FRWRD_MACT_TRAVERSE_STATE_AGGREGATE ) 
  {
    /* mark entry in use */
   ...
   */
  current_index = DPP_PP_LEM_NOF_ENTRIES(unit); /* Should not be decreased by 1  */
  ...
  /* Now all matched entries are flushed, ARM CPU may still doesn't read 
   * all entries from reply-FIFO. wait until all entries in reply_FIFO are readed.
   */
  ...
  /* now that flush is done invalidate flush db entry */
  ...
  )
{
    ...
    SOC_SAND_CHECK_FUNC_RESULT(res, 35, exit);
    /* allocate recourses */
    key = (uint32*)sal_alloc(sizeof(uint32) * SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_frwrd_lem_sw_traverse_internal_unsafe");
    if (key == NULL)
    {
        SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 10, exit);
    }
    value= (uint32*)sal_alloc(sizeof(uint32) * ARAD_PP_LEM_ACCESS_PAYLOAD_IN_UINT32S * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_frwrd_lem_sw_traverse_internal_unsafe");
    if (value == NULL)
    {
        SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 20, exit);
    }

    /* Get the content(key, values) from the chipsim */
    res = chip_sim_em_get_block(unit, ARAD_CHIP_SIM_LEM_BASE,  ARAD_CHIP_SIM_LEM_KEY, ARAD_CHIP_SIM_LEM_PAYLOAD,
                                (void *)&tbl_data, key, value, nof_matched_entries);

    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    /* Traverse: Apply Action*/
    switch (action->type)
    {
        ...
        case    SOC_PPC_FRWRD_MACT_TRAVERSE_ACTION_TYPE_UPDATE:
            for (i = 0; i < *nof_matched_entries; i++)
            {
                res = chip_sim_exact_match_entry_add_unsafe(unit,
                                                            ...
                                                            &key[i * SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S],
                                                            ARAD_CHIP_SIM_LEM_KEY,
                                                            &value[i * ARAD_PP_LEM_ACCESS_PAYLOAD_IN_UINT32S],/* payload_data */
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            ...
            if (do_dynamic)
            {
                ...
                soc_hw_set_immediate_hw_access(unit, &hw_access);
                /* Traverse command for DYNAMIC entries will be executed immediately.*/
                res = _arad_pp_frwrd_lem_traverse_internal_unsafe(unit, rule_key, rule_key_mask, &new_rule_val, block_range, action, wait_till_finish, &matched_dyn_entries);
                /* Restore the original value of the HwLogDoStandardReadWrite*/
                soc_hw_restore_immediate_hw_access(unit, hw_access);
            ...
            if  (do_static)
            {
                ...
                /* Save the traverse command for STATIC entries into the HW Log. Will be executed later. Don't wait_till_finish */
                res = _arad_pp_frwrd_lem_traverse_internal_unsafe(unit, rule_key, rule_key_mask, &new_rule_val, block_range, action, 0, &dummy);
                SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  ...
  )
{
  ...
  /* iteration only supported for GET and count functionality */
  if((action->type != ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_COUNT && action->type != ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_RETRIEVE)
     && block_range->iter != 0
     ) {
      SOC_SAND_SET_ERROR_CODE(ARAD_PP_FEATURE_NOT_SUPPORTED_ERR, 10, exit);
  }

    /* if action is to count, and no mask then just read HW register*/
  check_key = 0;
  if(action->type == ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_COUNT && block_range->iter == 0) {
      /* check key if key is not totally masked */
      ...
      /* if action is to count, and no mask then just read HW register*/
        /* check payload if select is not none */
      if (rule_val != NULL) {
          check_payload = rule_val->compare_mask != ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_NONE;
      }
#if ARAD_PP_MACT_DEBUG_PRINT
      LOG_INFO(BSL_LS_SOC_FORWARD,
               (BSL_META_U(unit,
  ...
  if (entry_offset == ARAD_PP_MACT_NOF_FLUSH_ENTRIES) { /* no place */
      SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 120, exit);
  }
  ...
  /* clear hit indication before start next traverse */
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, PPDB_B_LARGE_EM_FLU_MACHINE_HIT_COUNTERr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_HIT_COUNTERf, &fld_val));
...
/*    SOC_SAND_CHECK_NULL_INPUT(value_rule);*/
    SOC_SAND_CHECK_NULL_INPUT(block_range);
    ...
    *nof_entries = 0;
    ...
    if (SOC_IS_DONE_INIT(unit))
    {
        if (BCM_UNIT_DO_HW_READ_WRITE(unit))
        {
            ...
            if (value_rule)
            {
                new_rule_val = *value_rule;

                if (value_rule->compare_mask & SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC )
                {
                    if (value_rule->val.aging_info.is_dynamic == TRUE)
                        do_static = 0;
                    else  /*(rule_val->val.aging_info.is_dynamic == FALSE)*/
                        do_dynamic = 0;
                }
            }
            /*Entries are Dynamic --> Get from HW */
            if (do_dynamic)
            {
                /* just consider dynamic entries*/
                new_rule_val.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                ...
                soc_hw_set_immediate_hw_access(unit, &hw_access);
                /* Traverse command for DYNAMIC entries will be executed immediately.*/
                res = _arad_pp_frwrd_lem_get_block_unsafe(unit, key, key_mask, &new_rule_val, access_only, block_range, read_keys, read_vals, &matched_dyn_entries);
            ...
            /*Entries are Static --> Get from cache */
            if  (do_static)
            {
                /* consider static entries*/
                new_rule_val.compare_mask |= SOC_PPD_FRWRD_MACT_TRAVERSE_MATCH_SELECT_IS_DYNAMIC;
                new_rule_val.val.aging_info.is_dynamic = FALSE;
                 /* Traverse command - get block from the SW Shadow. Will be executed immediately.*/
                res = arad_pp_frwrd_lem_get_sw_block_by_key_unsafe(unit, key, key_mask, &new_rule_val, block_range, (read_keys + matched_dyn_entries), (read_vals + matched_dyn_entries), &matched_stat_entries);
                SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
            }
  ...
  )
{
...
#ifdef PLISIM
  if (SAL_BOOT_PLISIM) {
	  /* No implementation in pcid. Return error */
	  SOC_SAND_SET_ERROR_CODE(ARAD_PP_FEATURE_NOT_SUPPORTED_ERR, 8050, exit);
  }
#endif
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
    ...
    if (access_only & ARAD_PP_FRWRD_LEM_GET_BLOCK_CLEAR_ACCESSED) {
      action.update_mask = ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_SELECT_ACCESSED;
    }
    else{
      action.update_mask = 0;
    }
  ...
  while(current_index != 0 || reply_ready)
  {
    ...
    if(block_range->entries_to_act >= ARAD_PP_MACT_REPLY_FIFO_SIZE && flush_in_run) {
        ...
        if(block_range->entries_to_act == ARAD_PP_MACT_REPLY_FIFO_SIZE + nof_valid_entries) {
            ...
            LOG_INFO(BSL_LS_SOC_FORWARD,
                     (BSL_META_U(unit,
                                 "fifo_size != FIFO_SIZE, current_index value %d, fifo_size %d\n\r"), current_index, fifo_size));
#endif
            }
            ...
            if(fifo_size >= ARAD_PP_MACT_REPLY_FIFO_SIZE) {
                ...
                LOG_INFO(BSL_LS_SOC_FORWARD,
                         (BSL_META_U(unit,
                                     "fifo_size = %d, current_index value %d, block_range->iter %d\n\r"), fifo_size, current_index, block_range->iter));
#endif
                /* stop flush db*/
                SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  6,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, PPDB_B_LARGE_EM_FLU_MACHINE_CURRENT_INDEXr, REG_PORT_ANY, 0, LARGE_EM_FLU_MACHINE_CURRENT_INDEXf,  0));
                flush_in_run = 0;
    ...
    if(!reply_ready)
    {
            ...
            LOG_INFO(BSL_LS_SOC_FORWARD,
                     (BSL_META_U(unit,
                                 "!reply_ready, current_index value %d, block_range->iter %d\n\r"), current_index, block_range->iter));
#endif
      continue;
    }
    ...
    /* read and parse reply to lem structs */
      ...
      {
        /* no need to read, key as payload is returned as well */
        /*
        ...
        */ 
        ...
        payload->flags |= (request_all.is_learned_first_by_me)?ARAD_PP_LEM_ACCESS_ENTRY_ACCESSED:0;
        /*
        ...
        if (is_found == TRUE)
        {
          ...
          if (is_param_for_rule == TRUE)
          {
            ...
            LOG_INFO(BSL_LS_SOC_FORWARD,
                     (BSL_META_U(unit,
                                 "current_index value %d, nof_entries %d\n\r"), current_index, nof_valid_entries));
#endif
            /* clear entries in FIFO */
            reply_ready = 1;
            while(reply_ready) {
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
           (BSL_META_U(unit,
                       "get-all: current_index value %d, nof_entries %d\n\r"), current_index, nof_valid_entries));
#endif
  ...
  )
{
  ...
  res = ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE_verify(res, rule);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
...
uint32
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_MAC_ADDR_verify(
    SOC_SAND_IN  int                                   unit,
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->fid, SOC_DPP_DEFS_GET(unit, nof_fids), ARAD_PP_FID_OUT_OF_RANGE_ERR, 11, exit);

  /* SOC_SAND_MAGIC_NUM_VERIFY(info); */

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_KEY_MAC_ADDR_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_IPV4_MC_verify(
    SOC_SAND_IN  int                                  unit,
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_KEY_IPV4_MC_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_VAL_verify(
    SOC_SAND_IN  int                              unit,
  ...
  )
{
  ...
  res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_MAC_ADDR_verify(unit, &(info->mac));
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_IPV4_MC_verify(unit, &(info->ipv4_mc));
  SOC_SAND_CHECK_FUNC_RESULT(res, 11, exit);

  /* SOC_SAND_MAGIC_NUM_VERIFY(info); */

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_KEY_VAL_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_KEY_verify(
    SOC_SAND_IN  int                          unit,
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->key_type, ARAD_PP_FRWRD_MACT_KEY_TYPE_MAX, ARAD_PP_FRWRD_MACT_KEY_TYPE_OUT_OF_RANGE_ERR, 10, exit);
  if (info->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR)
  {
    res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_MAC_ADDR_verify(unit, &(info->key_val.mac));
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
  }
  else
  {
    res = ARAD_PP_FRWRD_MACT_ENTRY_KEY_IPV4_MC_verify(unit, &(info->key_val.ipv4_mc));
    SOC_SAND_CHECK_FUNC_RESULT(res, 21, exit);
  }
  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_KEY_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_AGING_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_AGING_INFO_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_FRWRD_INFO_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_FRWRD_INFO_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_ENTRY_VALUE_verify(
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_FRWRD_INFO, &(info->frwrd_info), 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_AGING_INFO, &(info->aging_info), 11, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_ENTRY_VALUE_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TIME_verify(
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TIME_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_MAC_verify(
    SOC_SAND_IN  int                                            unit,
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_NOF(info->fid, SOC_DPP_DEFS_GET(unit, nof_fids), ARAD_PP_FID_OUT_OF_RANGE_ERR, 10, exit);
  SOC_SAND_ERR_IF_ABOVE_NOF(info->fid_mask, SOC_DPP_DEFS_GET(unit, nof_fids), ARAD_PP_FID_OUT_OF_RANGE_ERR, 11, exit);
  /*ARAD_PP_STRUCT_VERIFY(SOC_SAND_PP_MAC_ADDRESS, &(info->mac), 12, exit);*/
  /*ARAD_PP_STRUCT_VERIFY(SOC_SAND_PP_MAC_ADDRESS, &(info->mac_mask), 13, exit);*/

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_MAC_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_IPV4_MC_verify(
    SOC_SAND_IN  int                                                unit,
  ...
  )
{
   ...
   */
  SOC_SAND_ERR_IF_ABOVE_NOF(info->fid, SOC_DPP_DEFS_GET(unit, nof_fids), ARAD_PP_FID_OUT_OF_RANGE_ERR, 12, exit);
  SOC_SAND_ERR_IF_ABOVE_NOF(info->fid_mask, SOC_DPP_DEFS_GET(unit, nof_fids), ARAD_PP_FID_OUT_OF_RANGE_ERR, 13, exit);

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_IPV4_MC_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE_verify(
  ...
  )
{
  ...
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_VALUE, &(info->val), 10, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE_verify(
    SOC_SAND_IN  int                                    unit,
  ...
  )
{
  ...
  if (info->key_type == ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR)
  {
    res = ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_MAC_verify(unit, &(info->key_rule.mac));
    SOC_SAND_CHECK_FUNC_RESULT(res, 11, exit);
  }
  else /* ARAD_PP_FRWRD_MACT_KEY_TYPE_MAC_ADDR */
  {
    res = ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_KEY_RULE_IPV4_MC_verify(unit, &(info->key_rule.ipv4_mc));
   ...
   */
/*  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE, &(info->value_rule), 13, exit); */

  SOC_SAND_MAGIC_NUM_VERIFY(info);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_verify(
  ...
  )
{
  ...
  SOC_SAND_ERR_IF_ABOVE_MAX(info->type, ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_TYPE_MAX, ARAD_PP_FRWRD_MACT_ACTION_TYPE_OUT_OF_RANGE_ERR, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_ENTRY_VALUE, &(info->updated_val), 11, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION_verify()",0,0);
}

uint32
  ARAD_PP_FRWRD_MACT_TRAVERSE_STATUS_INFO_verify(
    SOC_SAND_IN  int                                     unit,
  ...
  )
{
  ...
  res = ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_RULE_verify(unit, &(info->rule));
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_TRAVERSE_ACTION, &(info->action), 11, exit);
  ARAD_PP_STRUCT_VERIFY(ARAD_PP_FRWRD_MACT_TIME, &(info->time_to_finish), 12, exit);
...
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR("error in ARAD_PP_FRWRD_MACT_TRAVERSE_STATUS_INFO_verify()",0,0);
}
  ...
  )
{
  ...
  uint32 mask_val = 1;
   ...
   /*ARAD_PP_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE  *rule_val = NULL;*/
   /*SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE rule_val = NULL;*/
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
 ...
 /* compare for key in bits 69-54 (FID)*/
  ...
  tbl_data->compare_key_20_data = 0;
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_data,0, key_in_buffer.buffer, 48, 16);
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_data,16, key_in_buffer.buffer, 70, 4);
  /* set mask*/
  tbl_data->compare_key_20_mask = 0;
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_mask,0, key_in_buffer_mask.buffer, 48, 15);
  ...
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_mask,16, key_in_buffer_mask.buffer, 70, 4);
