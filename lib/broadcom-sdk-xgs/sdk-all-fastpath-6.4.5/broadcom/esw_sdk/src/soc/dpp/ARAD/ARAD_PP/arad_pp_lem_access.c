...
#define ARAD_PP_LEM_ENTRY_TYPE_IS_MC_EEI_LSB (SOC_IS_JERICHO(unit)?42:40)
...
arad_pp_lem_access_sa_drop_get(
   ...
   SOC_SAND_OUT  uint8                       *sa_drop
    ...
    if (entry_type != ARAD_PP_LEM_ENTRY_TYPE_FORMAT_3B) {
        uint32 res;
        uint32 sa_drop_32 = 0;
        res = soc_sand_bitstream_get_any_field(
                payload_data,
...
if (key_type_msb != ARAD_PP_LEM_ACCESS_KEY_PREFIX_FOR_SA_AUTH)
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_IP_SPOOF_DHCP:
    ...
    key->param[0].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IP_SPOOF_DHCP;
    key->param[1].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_IP_SPOOF_DHCP;
    ...
    )
{
  ...
  /* in the PMF we will copy the payload.asd (inlif) to the trap qualifier therefor we need the offset to be aligned*/
  if (request_all->request.key.type==ARAD_PP_LEM_ACCESS_KEY_TYPE_BFD_SINGLE_HOP) {
      int tmp=0xff;
      fld_value[0]= 0;
      fld_value[1]= 0;
      fld_value[0] = request_all->payload.dest;
      tmp = tmp & request_all->payload.asd;
      tmp = tmp << 24;
      fld_value[0] = fld_value[0] | tmp;
      tmp=0xff00;
      tmp = tmp & request_all->payload.asd;
      tmp = tmp >> 8;
      fld_value[1]=tmp;
  ...
  )
{
  ...
  break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_BFD_SINGLE_HOP:
	  SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_params, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_IP_HOST, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_IP_HOST, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_OUT_OF_RANGE_ERR, 22, exit);
  break;
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_OAM_STATISTICS:
	  SOC_SAND_ERR_IF_OUT_OF_RANGE(info->nof_params, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_FC_LOCAL, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_FOR_FC_LOCAL, ARAD_PP_LEM_ACCESS_KEY_NOF_PARAMS_OUT_OF_RANGE_ERR, 22, exit);
  break;
 case ARAD_PP_LEM_ACCESS_KEY_TYPE_IP6_SPOOF_STATIC:
    ...
    )
{
    ...
    key = (uint32*)sal_alloc(sizeof(uint32) * SOC_DPP_DEFS_MAX_LEM_WIDTH_IN_UINT32S * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
    if (key == NULL)
    {
        SOC_SAND_SET_ERROR_CODE(SOC_PPD_ERR_OUT_OF_RESOURCES, 10, exit);
    }
    data_out= (uint32*)sal_alloc(sizeof(uint32) * ARAD_PP_LEM_ACCESS_PAYLOAD_IN_UINT32S * CHIPSIM_MAX_TRAVERSE_SIZE, "arad_pp_lem_block_get");
    if (data_out == NULL)
    {
    ...
    res = chip_sim_em_get_block(unit, ARAD_CHIP_SIM_LEM_BASE,  ARAD_CHIP_SIM_LEM_KEY, ARAD_CHIP_SIM_LEM_PAYLOAD,
                                (void *)tbl_data, key, data_out, nof_entries);
...
int arad_pp_bfd_one_hop_lem_entry_add(int unit,int is_ipv6,uint32 local_discriminator,uint16 oam_id,int is_accelerated) {
	...
	ARAD_PP_TRAP_CODE		trap_code=0; /* = SOC_PPC_BFD_TRAP_ID_OAMP_IPV4;*/
	...
	request.key.param[0].value[0] = local_discriminator;
    request.key.param[0].value[1] = 0; 
	request.key.param[1].value[0] = 0xff; /*ttl*/
    request.key.param[1].value[1] = 0; 
    request.key.param[0].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM0_IN_BITS_FOR_IP_HOST; /*32*/
    request.key.param[1].nof_bits = ARAD_PP_LEM_ACCESS_KEY_PARAM1_IN_BITS_FOR_SA_AUTH; /*8*/
	...
	if (is_ipv6) {
	    if (is_accelerated) {
            res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_INIT_TRAP_INFO_TRAP_IDS, &trap_code, SOC_PPD_BFD_TRAP_ID_UC_IPV6); 
    		SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 212, exit);
        } else {
            trap_code = SOC_PPC_TRAP_CODE_OAM_CPU;
    	}            
    } else {
    	if (is_accelerated) {
    		res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_ARAD_PP_BFD_INIT_TRAP_INFO_TRAP_IDS, &trap_code, SOC_PPD_BFD_TRAP_ID_OAMP_IPV4); 
	...
	payload.flags = ARAD_PP_LEM_ACCESS_ASD_OUTLIF;
	payload.dest = (internal_trap_code | 0x600); /* 0x6e3;*/
	if (is_ipv6) {
	    payload.asd = oam_id; 
	} else {
	    payload.asd = ((uint16)local_discriminator);
	}
...
int arad_pp_bfd_one_hop_lem_entry_remove(int unit,uint32 local_discriminator) {
