  ...
  )
{
    ...
    if (otm_shp_en_f) {
        ...
        /* Enable port shaper */
        res = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_single_port_rate_sw_set,(unit, core, tm_port, egq_rate));
        SOCDNX_IF_ERR_EXIT(res);

        /*Write changes to hardware*/
        res = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_ofp_rates_sch_single_port_rate_hw_set,(unit, core, tm_port));
    }
    ...
    )
{
    ...
    modified_e2e_interfaces_occ = arad_sw_db_egr_modified_e2e_interfaces_occ_get(unit,core);
