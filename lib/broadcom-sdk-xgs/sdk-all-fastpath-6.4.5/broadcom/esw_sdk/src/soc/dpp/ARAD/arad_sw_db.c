   ...
   )
{
  ...
  for(i=0 ; i < nof_cores ; i++) {
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority[use_core + i][offset].priority_shaper_rate = rate;
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority[use_core + i][offset].valid = valid;
        ...
        res = arad_wb_db_egr_port_priority_update_rates_state(unit, use_core + i, offset);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  *rate = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority[use_core][offset].priority_shaper_rate;
  *valid = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.port_priority[use_core][offset].valid;
  return SOC_SAND_OK;
}
   ...
   )
{
  ...
  for(i=0 ; i < nof_cores ; i++) {
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_shaper[use_core + i][offset].tcg_shaper_rate = rate;
      Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_shaper[use_core + i][offset].valid = valid;
        ...
        res = arad_wb_db_egr_tcg_shaper_update_rates_state(unit, use_core + i, offset);
        SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
   ...
   )
{
  ...
  *rate = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_shaper[use_core][offset].tcg_shaper_rate;
  *valid = Arad_sw_db.arad_device_sw_db[unit]->arad_sw_db_egr_ports.tcg_shaper[use_core][offset].valid;
  return SOC_SAND_OK;
}
  ...
  if (SOC_WARM_BOOT(unit)) {
      ...
      for (tcam_db_id = 0; tcam_db_id < ARAD_TCAM_MAX_NOF_LISTS; ++tcam_db_id)
      {
          ...
          if (restore_data->priorities[tcam_db_id].is_exist) {
              ...
              res = soc_sand_sorted_list_create(unit, priorities);
              SOC_SAND_CHECK_FUNC_RESULT(res, 50, exit);
          }
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
  res = sw_state_access[unit].dpp.soc.arad.tm.kbp_frwrd_ip.location_tbl.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 20, exit);

  res = sw_state_access[unit].dpp.soc.arad.tm.kbp_frwrd_ip.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030) */
  ...
  )
{
  ...
  for (stage = 0; stage < ARAD_NOF_FP_DATABASE_STAGES; stage ++) {
      ...
      {
          res = sw_state_access[unit].dpp.soc.arad.tm.pmf.fem_entry.set(unit, stage, fem_ndx, fem_entry);
          SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
      }
  }
  ...
  )
{
  ...
  if(!SOC_WARM_BOOT(unit)) {
      ...
      res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.sysport2basequeue.alloc(unit, ARAD_NOF_SYS_PHYS_PORTS_GET(unit) * SOC_DPP_CONFIG(unit)->core_mode.nof_active_cores);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 254, exit);
      ...
      res = sw_state_access[unit].dpp.soc.arad.tm.arad_egr_ports.alloc(unit);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 330, exit);

#if defined(INCLUDE_KBP) && !defined(BCM_88030)
      res = sw_state_access[unit].dpp.soc.arad.tm.kbp_info.alloc(unit);
  ...
  )
{
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.pmf.free(unit);
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.tcam_mgmt.free(unit);
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.tm_info.sysport2basequeue.free(unit);
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.arad_egr_ports.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 330, exit);

#if defined(INCLUDE_KBP) && !defined(BCM_88030)
    res = sw_state_access[unit].dpp.soc.arad.tm.kbp_info.free(unit);
    SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 300, exit);
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030) */
  ...
  )
{
    ...
    if (is_forbidden) {
        SHR_BITSET(sw_state[unit]->dpp.soc.arad.tm->tcam->tcam_db[tcam_db_id].forbidden_dbs, tcam_db_other);
    }
    else {
        SHR_BITCLR(sw_state[unit]->dpp.soc.arad.tm->tcam->tcam_db[tcam_db_id].forbidden_dbs, tcam_db_other);
    }
  ...
  )
{
    ...
    *is_forbidden  = (SHR_BITGET(sw_state[unit]->dpp.soc.arad.tm->tcam->tcam_db[tcam_db_id].forbidden_dbs, tcam_db_other) > 0);
  ...
  )
{
  ...
  res = sw_state_access[unit].dpp.soc.arad.tm.tcam.db_location_tbl.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 120, exit);

  res = sw_state_access[unit].dpp.soc.arad.tm.tcam.global_location_tbl.free(unit);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 130, exit);

  res = sw_state_access[unit].dpp.soc.arad.tm.tcam.free(unit);
  ...
  )
{
    ...
    if(is_used) {
        SHR_BITSET(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].fp_info.entry_bitmap[bank_index], entry_index); 
    }
    else {
        SHR_BITCLR(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].fp_info.entry_bitmap[bank_index], entry_index); 
    }
  ...
  )
{
    ...
    *is_used  = ((SHR_BITGET(sw_state[unit]->dpp.soc.arad.tm->pmf[stage].fp_info.entry_bitmap[bank_index], entry_index) > 0 ) ? 1 : 0);
  ...
  )
{
    ...
    for (i = 0; i < SOC_DPP_DEFS_GET_TCAM_NOF_BANKS(unit); i++) {
        for (j = 0; j < SOC_DPP_DEFS_MAX_TCAM_NOF_LINES_IN_LONGS; j++) {
            rv = sw_state_access[unit].dpp.soc.arad.tm.pmf.fp_info.entry_bitmap.set(unit, stage, i, j, 0x0);
            SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 10, exit);
        }
    }
