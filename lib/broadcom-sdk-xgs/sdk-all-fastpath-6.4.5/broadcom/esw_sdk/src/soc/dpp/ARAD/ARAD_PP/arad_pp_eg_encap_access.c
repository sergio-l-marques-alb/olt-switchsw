...
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_DROP_LSB(start_index)       (37 + start_index)
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_DROP_MSB(start_index)       (37 + start_index)
...
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_OAM_LIF_SET_LSB(start_index)       (36 + start_index)
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_OAM_LIF_SET_MSB(start_index)       (36 + start_index)
...
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_LSB(start_index)       (34 + start_index)
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_MSB(start_index)       (35 + start_index)
#define ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_NOF_BITS(start_index)  SOC_SAND_RNG_COUNT(ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_LSB(start_index), ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_MSB(start_index))
...
#define ARAD_PP_EG_ENCAP_ACCESS_NOF_EEDB_ENTRIES                         ( ARAD_PP_EG_ENCAP_NOF_REGULAR_EEDB_BANKS * ARAD_PP_EG_ENCAP_NOF_ENTRIES_PER_HALF_EEDB_BANK ) 
#define ARAD_PP_EG_ENCAP_ACCESS_NOF_TOP_BANK_ENTRIES                     ( ARAD_PP_EG_ENCAP_NOF_ENTRIES_PER_TOP_BANK)
/* } */
/*************
 * MACROS    *
 *************/
...
#endif
  uint32
    table_entry[ARAD_PP_EG_ENCAP_ACCESS_FORMAT_TBL_ENTRY_SIZE]= {0x0,0x6,0x0C0}; /* Initialize EEDB table with actions DROP.
                                                                                    This configuration for arad is fixed and will not be changed.  */

  SOC_SAND_INIT_ERROR_DEFINITIONS(0);

   /* initialize EEEDB table with NONE format. */
...
/* In ARADPLUS and above, the "empty" lif is a lif with the ISID format MSBs (since ISID lifs are not used) with the AC "use as data"
  ...
  )
{
  ...
  else if (access_prefix == ARAD_PP_EG_ENCAP_ACCESS_PREFIX_TYPE_TRILL) {
  ...
  )
{
  ...
  start_index = 0; /* 1 trill eedb entry per eedb entry. */
  ...
  res = soc_sand_bitstream_get_any_field(data, ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_LSB(start_index), ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_NOF_BITS(start_index), &tbl_data->my_nickname_index);
  ...
  )
{
  ...
  res = soc_sand_bitstream_set_any_field(&tbl_data->my_nickname_index, ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_LSB(start_index), ARAD_PP_EG_ENCAP_ACCESS_TRILL_ENTRY_MY_NICK_NAME_INDEX_NOF_BITS(start_index), data);
  SOC_SAND_CHECK_FUNC_RESULT(res, 45, exit);
  ...
  )
{
   ...
   SOC_SAND_INIT_ERROR_DEFINITIONS(ARAD_PP_EG_ENCAP_ACCESS_LOCAL_OUTLIF_ADDITIONAL_DATA_LOCATION_GET);
    ...
    eedb_bank_id = ARAD_PP_EG_ENCAP_ACCESS_OFFSET_TO_BANK_ID(unit,ARAD_PP_EG_ENCAP_ACCESS_OUT_LIF_TO_OFFSET(outlif));

    res = SOC_DPP_WB_ENGINE_GET_ARR(unit, SOC_DPP_WB_ENGINE_VAR_JER_EEDB_BANK_INFO, &eedb_bank_info, eedb_bank_id*2);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    *bank_id =  (ARAD_PP_EG_ENCAP_NOF_REGULAR_EEDB_BANKS + eedb_bank_info.extension_bank / ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK );
   
    *entry_id = (ARAD_PP_EG_ENCAP_ACCESS_OFFSET_TO_INTERNAL_OFFSET(unit, ARAD_PP_EG_ENCAP_ACCESS_OUT_LIF_TO_OFFSET(outlif)) ) / ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK + ( ARAD_PP_EG_ENCAP_NOF_ENTRIES_PER_EEDB_BANK /ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK * ( eedb_bank_info.extension_bank % ARAD_PP_EG_ENCAP_NOF_TOP_BANKS_PER_EEDB_BANK )  );
  ...
  )
{
   ...
   bank_id = ARAD_PP_EG_ENCAP_ACCESS_OFFSET_TO_TOP_BANK_ID(entry_offset);
   internal_offset = ARAD_PP_EG_ENCAP_ACCESS_OFFSET_TO_TOP_BANK_INTERNAL_OFFSET(entry_offset);
