...
/*Used for source type field value macros, in Arad legacy signaling.*/
#define _LSB 0
#define _MSB 1
...
#define ARAD_CNT_FORMAT_TO_FLD_VAL(unit, format, fld_val)      \
...
/* 
 * 0x0: the each memory entry is divided to two counters:
 *  octets counter at bits 63:28 (36 bits) and packets counter at bits 27:0 (28 bits). not valid in double mode
 *  
 * In jericho we read only one register and parse its' results according to format 
 * Packets And Bytes (26b for packets counter, 33b for bytes counter.) 
 * Only packets (59b) 
 * Only Bytes(59b) 
 * Double packet count (which doubles our total number of counter sets). (26b *2) 
 */
...
#define JER_CNT_PKT_CNT_GET(pkt_cnt ,field_val, format)                                                              \
...
#define JER_CNT_BYTE_CNT_GET(byte_cnt ,field_val, format)                                                               \
    COMPILER_64_ZERO(byte_cnt);                                                                                         \
    if (format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS) {                                                                   \
        COMPILER_64_SET(byte_cnt, 0, COMPILER_64_LO(field_val));                                                        \
    } else if (format != SOC_TMC_CNT_FORMAT_PKTS) {                                                                     \
        COMPILER_64_SET(byte_cnt,                                                                                       \
           (JER_CNT_BYTE_CNT_MASK_HI(format) & (COMPILER_64_HI(field_val)  >> (32 - JER_CNT_BYTE_CNT_SHIFT(format)))) , \
           ((JER_CNT_BYTE_CNT_MASK_HI(format) & COMPILER_64_HI(field_val)) << (32 - JER_CNT_BYTE_CNT_SHIFT(format))) |  \
           (JER_CNT_BYTE_CNT_MASK_LO(format) & (COMPILER_64_LO(field_val)  >> JER_CNT_BYTE_CNT_SHIFT(format))));        \
    }
/* 
 *  
 * In Arad:
 *    0 : CrPsn takes cnt commands from IRPP A, LSB
 *    1 : CrPsn takes cnt commands from IRPP A, MSB
 *    2 : CrPsn takes cnt commands from IRPP B, LSB
 *    3 : CrPsn takes cnt commands from IRPP B, MSB
 * In Jericho:
 *    0x4:  CrPsn takes cnt commands from IRPP0 A 
 *    0x5:  CrPsn takes cnt commands from IRPP0 B
 *    0x6:  CrPsn takes cnt commands from IRPP1 A
 *    0x7:  CrPsn takes cnt commands from IRPP1 B
 */
...
/*
 * In Arad:
 *     10: CrPsn takes cnt commands from IQM A                               
 *     11: CrPsn takes cnt commands from IQM B                               
 *     12: CrPsn takes cnt commands from IQM C                               
 *     13: CrPsn takes cnt commands from IQM D                               
 * In Jericho:
 *     0x0:  CrPsn takes cnt commands from IQM0 A
 *     0x1:  CrPsn takes cnt commands from IQM0 B
 *     0x2:  CrPsn takes cnt commands from IQM1 A
 *     0x3:  CrPsn takes cnt commands from IQM1 B
 *
 */ 
...
/*
 * In Arad:
 *     4 : CrPsn takes cnt commands from EGQ LSB
 *     5 : CrPsn takes cnt commands from EGQ MSB
 * In Jericho: 
 *     0x8:  CrPsn takes cnt commands from EGQ0 A
 *     0x9:  CrPsn takes cnt commands from EGQ0 B
 *     0xa:  CrPsn takes cnt commands from EGQ1 A
 *     0xb:  CrPsn takes cnt commands from EGQ1 B
 *
 */
...
/* 
 * In Arad:                                                                      
 *     14: CrPsn takes cnt commands from IHB A (OAM, first quarter of range) 
 *     15: CrPsn takes cnt commands from IHB B (OAM, second quarter of range)
 *     16: CrPsn takes cnt commands from IHB C (OAM, third quarter of range) 
 *     17: CrPsn takes cnt commands from IHB D (OAM, forth quarter of range) 
 * In Jericho                                                                    
 *     0x10: CrPsn takes cnt commands from IHB0                              
 *     0x11: CrPsn takes cnt commands from IHB1                              
 */
...
/*
 * Jericho Only: 
 *   0x12: CrPsn takes cnt commands from IPT0
 *   0x13: CrPsn takes cnt commands from IPT1
 */
...
/*
 * Jericho Only: 
 *   0x14: CrPsn takes cnt commands from EGQ0TM
 *   0x15: CrPsn takes cnt commands from EGQ1TM
 */
...
/* 
 * In Arad:
 *   6 : CrPsn takes cnt commands from EPNI A, LSB                         
 *   7 : CrPsn takes cnt commands from EPNI A, MSB                         
 *   8 : CrPsn takes cnt commands from EPNI B, LSB                         
 *   9 : CrPsn takes cnt commands from EPNI B, MSB                         
 * In Jericho:  
 *   0xc:  CrPsn takes cnt commands from EPNI0 A
 *   0xd:  CrPsn takes cnt commands from EPNI0 B
 *   0xe:  CrPsn takes cnt commands from EPNI1 A
 *   0xf:  CrPsn takes cnt commands from EPNI1 B
 */
...
/* 
 * in Arad and Ardon the encoding is Threshold = Th[9:0]<<Th[14:10], Jericho is the encoding is Threshold = Th[61:0]
 * in Arad 2^22, 2^24 in Jericho and Ardon
 */
#define ARAD_CNT_PKT_TRESH(unit)     (SOC_IS_JERICHO(unit) ? (1 << 24) : (SOC_IS_ARDON(unit) ? (0x5803) : (0x5801)))
/* 
 * in Arad and Ardon the encoding is Threshold = Th[9:0]<<Th[14:10], Jericho is the encoding is Threshold = Th[61:0]
 * in Arad 2^29, 2^32 in Jericho and Ardon 
 */
...
STATIC uint32
    ...
    if (proc_id < SOC_DPP_DEFS_GET(unit, nof_counter_processors)) {
        ...
        if (proc_id == SOC_DPP_DEFS_GET(unit, nof_counter_processors) + 1) {
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
     /*Set all mode to count in non-double mode*/
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      *group_size = info->custom_mode_params.nof_counters;
  ...
  )
{
    ...
    uint32     *host_buff; 
    soc_reg_t reg_crps_cache_rd_results;
#endif /* ARAD_CNT_USE_DMA */
...
if (!SAL_BOOT_PLISIM ) {
  ...
  rv = _soc_mem_sbus_fifo_dma_stop((unit), ch[dma_ndx]);
  if (SOC_FAILURE(rv)) {
      SOC_SAND_SET_ERROR_CODE(ARAD_CNT_START_FIFO_DMA_ERR, 53, exit);   
  }

  rv = _soc_mem_sbus_fifo_dma_start_memreg((unit), ch[dma_ndx],
                                           FALSE /*is_mem*/, 0, reg_crps_cache_rd_results, MEM_BLOCK_ANY, 0,
  ...
  start_addr = soc_reg_addr_get(unit, reg_crps_cache_rd_results, REG_PORT_ANY, 0, FALSE, &blk, &at);
  start_addr = start_addr + dma_ndx * entry_words;
  soc_pci_write(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_SBUS_START_ADDRESS_OFFSET(cmc, ch[dma_ndx]), start_addr);
}
#endif /* ARAD_CNT_USE_DMA */
  ARAD_DO_NOTHING_AND_EXIT;
exit:
    ...
    SOC_SAND_OUT soc_field_t           *fields)
{
    soc_reg_t
        ...
        egq_tm_counter_bmap_reg[] =  {EGQ_0_TM_DO_NOT_COUNT_OFFSETf, EGQ_1_TM_DO_NOT_COUNT_OFFSETf},
   ...
   int index, field_offset = 0;
    soc_field_t
       ...
       irpp_counter_bmap_fields[] = {
           _BMAP_FIELDS(IRPP, 0, A), 
           _BMAP_FIELDS(IRPP, 0, B),
           _BMAP_FIELDS(IRPP, 1, A),
           _BMAP_FIELDS(IRPP, 1, B)},
       epni_counter_bmap_fields[] = {_BMAP_FIELDS(EPNI, 0, A), _BMAP_FIELDS(EPNI, 0, B), _BMAP_FIELDS(EPNI, 1, A), _BMAP_FIELDS(EPNI, 1, B)},
       egq_tm_counter_bmap_fields[] =  {_BMAP_FIELDS(EGQ, 0, _TM), _BMAP_FIELDS(EGQ, 0, _TM)},
       ...
       SOC_SAND_IN  ARAD_CNT_COUNTERS_INFO *info)
{
    ...
    soc_reg_t src_btmap_reg = INVALIDr;
    soc_field_t bmap_fields[ARAD_CNT_BMAP_OFFSET_COUNT];
    uint32 reg_val = 0;
    uint32 res = SOC_SAND_OK;
   ...
   if (info->src_type == ARAD_CNT_SRC_TYPE_OAM || info->src_type == ARAD_CNT_SRC_TYPES_IPT_LATENCY){
       /* Always 1 count for OAM */
       /* Always 1 count for IPT latency*/
    ...
    res = _arad_cnt_do_not_count_field_by_src_type_get(unit, proc_id, info->src_core, info->src_type, info->command_id, &do_not_count_field);
    ...
    if (SOC_IS_JERICHO(unit)) {
        res = _jer_cnt_counter_bmap_mem_by_src_type_get(unit, proc_id, info->src_core, info->src_type, info->command_id, &src_btmap_reg, bmap_fields);
...
{
    soc_field_t irpp_fields[] = {IRPP_0_A_MODEf, IRPP_0_B_MODEf, IRPP_1_A_MODEf, IRPP_1_B_MODEf};
    soc_field_t egq_fields[] = {EGQ_0_A_MODEf, EGQ_0_B_MODEf, EGQ_1_A_MODEf, EGQ_1_B_MODEf};
    soc_field_t epni_fields[] = {EPNI_0_A_MODEf, EPNI_0_B_MODEf, EPNI_1_A_MODEf, EPNI_1_B_MODEf};
    soc_field_t iqm_fields[] = {IQM_0_A_MODEf, IQM_0_B_MODEf, IQM_1_A_MODEf, IQM_1_B_MODEf};
    soc_field_t egq_tm_fields[] ={EGQ_0_TM_MODEf, EGQ_1_TM_MODEf};
    soc_field_t ihb_fields[] = {IHB_0_MODEf, IHB_1_MODEf};
    soc_field_t ipt_fields[] = {IPT_0_MODEf, IPT_1_MODEf};
    SOCDNX_INIT_FUNC_DEFS;

    switch (src_type) {
    case ARAD_CNT_SRC_TYPE_ING_PP:
        *field = irpp_fields[(src_core * 2) + command_id];
        break;
    case ARAD_CNT_SRC_TYPE_VOQ:
    case ARAD_CNT_SRC_TYPE_STAG:
    case ARAD_CNT_SRC_TYPE_VSQ:
    case ARAD_CNT_SRC_TYPE_CNM_ID:
        *field = iqm_fields[(src_core * 2) + command_id];
        break;
    case ARAD_CNT_SRC_TYPE_EGR_PP:
        *field = egq_fields[src_core];
        break;
    case ARAD_CNT_SRC_TYPE_OAM:
        *field = ihb_fields[src_core];
        break;
    case ARAD_CNT_SRC_TYPES_IPT_LATENCY:
        *field = ipt_fields[src_core];
        break;
    case ARAD_CNT_SRC_TYPE_EPNI:
      *field = epni_fields[(src_core * 2) + command_id];
      break;
    case ARAD_CNT_SRC_TYPES_EGQ_TM:
      *field = egq_tm_fields[src_core];
      break;
    default:
      ...
      )
{
    ...
    if (eg_mode_type == ARAD_CNT_MODE_EG_TYPE_OUTLIF) {
        ...
        {
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 29, exit, READ_EPNI_COUNTER_SOURCE_MAPm(unit, EPNI_BLOCK(unit, src_core), pp_port_ndx, data));
            soc_EPNI_COUNTER_SOURCE_MAPm_field32_set(unit, data, base_val_fld, 1); 
            SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, WRITE_EPNI_COUNTER_SOURCE_MAPm(unit, EPNI_BLOCK(unit, src_core), pp_port_ndx, data));
        }
    }
   
    if (is_counter_0) {
        COMPILER_64_SET(field64,0,base_val_pmf);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  25,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_0_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_tm);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  26,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_1_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_vsi);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  27,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_2_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_outlif);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  28,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_0_BASEr, src_core, 0, COUNTER_0_MODE_3_BASEf, field64));
    } else {
        COMPILER_64_SET(field64,0,base_val_pmf);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  35,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_0_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_tm);    
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  36,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_1_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_vsi);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  37,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_2_BASEf, field64));
        COMPILER_64_SET(field64,0,base_val_outlif);
        SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  38,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_modify(unit, EPNI_COUNTER_1_BASEr, src_core, 0, COUNTER_1_MODE_3_BASEf, field64));
    }
...
#if ARAD_CNT_USE_DMA
if (!SAL_BOOT_PLISIM ) {
  res = arad_cnt_engine_to_fifo_dma_index(unit, proc_id, &dma_dnx);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 5, exit);

  res = arad_cnt_dma_set_unsafe(unit, dma_dnx);
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  9,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CRPS_CRPS_CNT_SRC_CFGr, REG_PORT_ANY,  proc_id, CRPS_N_CNT_SRCf,  src_type));
  if (SOC_IS_JERICHO(unit)) {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 10, exit, ARAD_REG_ACCESS_ERR, soc_reg_above_64_field32_modify(unit, CRPS_CRPS_CNT_SRC_CFGr, REG_PORT_ANY,  proc_id, CRPS_N_CNT_BIT_NUMf,  info->we_val));
  }
  ...
  if ((info->src_type == ARAD_CNT_SRC_TYPE_VOQ) || (info->src_type == ARAD_CNT_SRC_TYPE_STAG) || 
      ...
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res, 14, exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, crps_iqm_cmd.src_cfg_r, src_core, 0, crps_iqm_cmd.src_type_f,  info->src_type - ARAD_CNT_SRC_TYPE_VOQ));
  }
  ...
  if (info->src_type == SOC_TMC_CNT_SRC_TYPE_EPNI){
      res = arad_cnt_epni_regs_set_unsafe(unit, info->src_type, info->src_core, info->mode_eg.type, info->command_id, info->mode_eg.base_val);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 30, exit);
  }
  if (info->src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP) {
        res = arad_pmf_counter_set(unit, command_id, info->mode_eg.type);
        SOC_SAND_CHECK_FUNC_RESULT(res, 40, exit);
  }


  /* Set the egress work mode */
  if(info->src_type == SOC_TMC_CNT_SRC_TYPE_EGR_PP) {
  ...
  if (info->src_type == ARAD_CNT_SRC_TYPE_VOQ) {
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1160,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, crps_iqm_cmd.src_cfg_r, src_core, 0, crps_iqm_cmd.queue_shift_f, arad_cnt_convert_enum_val2_queue_shift(info->voq_cnt.q_set_size)));
  
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1170,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, crps_iqm_cmd.cfg_1_r, src_core, 0, crps_iqm_cmd.base_q_f,  info->voq_cnt.start_q));

    /* Compute the top Queue = Base-Queue + (32K / #counters-per-group) * #VOQs per Counter */
    top_q = info->voq_cnt.start_q + (((ARAD_COUNTER_NDX_MAX(unit, proc_id) + 1) / nof_counters) * info->voq_cnt.q_set_size);
    SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1171,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, crps_iqm_cmd.cfg_2_r, src_core, 0, crps_iqm_cmd.top_q_f, top_q));
  }
  ...
  ARAD_CNT_FORMAT_TO_FLD_VAL(unit, info->format, format_field_val);
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      /* 
        2. configure relevent mode relvent mode*/
  ...
  if (info->replicated_pkts == SOC_TMC_CNT_REPLICATED_PKTS_FRWRD) {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1260,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRR_STATIC_CONFIGURATIONr, src_core, 0, COUNT_ALL_COPIESf, 0));
  } else {
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  1270,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IRR_STATIC_CONFIGURATIONr, src_core, 0, COUNT_ALL_COPIESf, 1));
  }

  
  /* In wide mode disable the algorithmic read */
  if ((info->format == ARAD_CNT_FORMAT_PKTS) || 
      (info->format == ARAD_CNT_FORMAT_BYTES) || 
      (info->format == ARAD_CNT_FORMAT_MAX_QUEUE_SIZE) || 
	  (info->format == SOC_TMC_CNT_FORMAT_IHB_COMMANDS) || 
      (info->format == ARAD_CNT_FORMAT_IPT_LATENCY && SOC_IS_JERICHO(unit))) {
  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      if (info->we_val >= (0x1 << soc_reg_field_length(unit, CRPS_CRPS_CNT_SRC_CFGr, CRPS_N_CNT_BIT_NUMf))) {
          /*The Wtite Enable bitmap is limited to the number of bits in register CRPS_CRPS_CNT_SRC_CFGr fiels CRPS_N_CNT_BIT_NUMf*/
          SOC_SAND_SET_ERROR_CODE(ARAD_PROCESSOR_NDX_OUT_OF_RANGE_ERR, 44, exit);
      }
  } else if (SOC_IS_ARADPLUS_AND_BELOW(unit) && info->we_val > 2) {
      /*The Wtite Enable bitmap is limited to 2 - LSB/MSB only*/
      SOC_SAND_SET_ERROR_CODE(ARAD_PROCESSOR_NDX_OUT_OF_RANGE_ERR, 45, exit);
  }
  ...
  )
{
  ...
  soc_reg_above_64_val_t
    reg_above_64;
  /*FIME shalita: in Jericho we use IQM_IQM_CNT_CMD_CONFIG_A, IQM_IQM_CNT_CMD_CONFIG_B*/
  ARAD_CNT_CRPS_IQM_CMD 
      crps_iqm_cmd;
  soc_reg_t
      base_val_reg;
 ...
 if (!SOC_UNIT_NUM_VALID(unit)) {
  ...
  if (SOC_IS_JERICHO(unit)) { /*ger counter pointer range*/
  ...
  if (ARAD_CNT_SRC_TYPE_IS_EGR_PP_FLD(unit,fld_val)) {
      ...
      res = arad_cnt_crps_iqm_cmd_get(unit, proc_id, info->command_id, &crps_iqm_cmd);
      ...
      info->src_type = ARAD_CNT_SRC_TYPE_OAM;
  } else if (ARAD_CNT_SRC_TYPE_IS_EPNI_FLD_VAL(unit, fld_val)) {
  ...
  if (info->src_type == ARAD_CNT_SRC_TYPE_EPNI) {
    ...
    case ARAD_CNT_MODE_EG_TYPE_PMF_FLD_VAL:
      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  74,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field64_read(unit, base_val_reg, src_core, 0, base_val_fld + (COUNTER_0_MODE_0_BASEf - COUNTER_0_MODE_0_BASEf), &field64));
      info->mode_eg.type = ARAD_CNT_MODE_EG_TYPE_VSI;
      COMPILER_64_TO_32_LO(info->mode_eg.base_val, field64);
    break;
    }
...
/* check if configurable mode*/
  ...
  if (info->src_type == ARAD_CNT_SRC_TYPE_VOQ)
  {
      ...
      info->voq_cnt.q_set_size = 1 << queue_shift;

      SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  130,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, crps_iqm_cmd.cfg_1_r, src_core, 0, crps_iqm_cmd.base_q_f, &info->voq_cnt.start_q));
  }
    ...
    *fifo_dma_index = 0;
    ...
    if (SOC_IS_JERICHO(unit)) {
        if (crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors)) {
            /*Assuming that nof_counter_fifos in a power of 2*/
            *fifo_dma_index = ((crps_index & ~(SOC_DPP_DEFS_GET(unit, nof_counter_fifos) - 1)) / (SOC_DPP_DEFS_GET(unit, nof_counter_fifos)));
        } else if (crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)){
            *fifo_dma_index = SOC_DPP_DEFS_GET(unit, nof_counter_fifos) - 1;  /*Small CRPSs are connected to the last fifo*/
        } else {
     ...
     )
{
    ...
    *offset = 0;
    if (SOC_IS_JERICHO(unit)) {
        if (crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors)) {
            *offset = crps_index % SOC_DPP_DEFS_GET(unit, nof_counter_processors);
        } else if (crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)){
            *offset = (crps_index % SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)) + SOC_DPP_DEFS_GET(unit, nof_counter_processors);
        } else {
            LOG_ERROR(BSL_LS_SOC_STAT,(BSL_META_U(unit, "counter processor %u out of range\n"), crps_index));
            SOCDNX_SAND_IF_ERR_EXIT(SOC_E_PARAM);
        }
...
if (!SAL_BOOT_PLISIM ) {
   ...
   rval = soc_pci_read(unit, CMIC_CMCx_FIFO_CHy_RD_DMA_STAT_OFFSET(cmc, ch[dma_ndx]));
   arad_sw_db_cnt_buff_and_index_get(unit, dma_ndx, &host_buff, &buff_index);
  ...
  LOG_DEBUG(BSL_LS_SOC_STAT,
   ...
   _soc_mem_sbus_fifo_dma_get_num_entries(unit, ch[dma_ndx], &cnt);
  ...
  for(indx = 0;  indx < counters_2read;  indx++)
  {
    ...
    ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_data_valid, fld_val, reg_val, 11, exit);
    valid = fld_val[0];
    ...
    if(valid){
        if (SOC_IS_JERICHO(unit)) {
            uint64
                cnt_val;
            ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_engine_num, fld_val, reg_val, 12, exit);
            result_arr->cnt_result[nof_read_cnt].engine_id = fld_val[0];
            ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_pkt_cnt, fld_val, reg_val, 13, exit);
            if (result_arr->cnt_result[nof_read_cnt].engine_id >= SOC_DPP_DEFS_GET(unit, nof_counter_processors) + SOC_DPP_DEFS_GET(unit, nof_small_counter_processors)) {
                SOC_SAND_SET_ERROR_CODE(ARAD_CNT_START_FIFO_DMA_ERR, 14, exit);
            }

            COMPILER_64_SET(cnt_val, fld_val[1], fld_val[0]);
            JER_CNT_PKT_CNT_GET(result_arr->cnt_result[nof_read_cnt].pkt_cnt, cnt_val, counter_info_array[result_arr->cnt_result[nof_read_cnt].engine_id].format);
            JER_CNT_BYTE_CNT_GET(result_arr->cnt_result[nof_read_cnt].byte_cnt, cnt_val, counter_info_array[result_arr->cnt_result[nof_read_cnt].engine_id].format);
        } else {
            /*in Arad the fifo index is the same as the engine index*/
            result_arr->cnt_result[nof_read_cnt].engine_id = dma_ndx;
            ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_pkt_cnt, fld_val, reg_val, 15, exit);
            COMPILER_64_SET(result_arr->cnt_result[nof_read_cnt].pkt_cnt, 0, fld_val[0]);

            ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_oct_cnt, fld_val, reg_val, 16, exit);
            COMPILER_64_SET(result_arr->cnt_result[nof_read_cnt].byte_cnt, fld_val[1], fld_val[0]);
        }
        ARAD_FLD_FROM_REG_ABOVE_64(reg, rd_result_fields.cache_rd_cnt_addr, fld_val, reg_val, 17, exit);
...
#if ARAD_CNT_USE_DMA
if (!SAL_BOOT_PLISIM ) {
  (void)_soc_mem_sbus_fifo_dma_set_entries_read(unit, ch[dma_ndx], nof_read_cnt);
  ...
  )
{
 ...
 uint64
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  12,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, processor_ndx, rd_processor_read.rd_req_type_f, ARAD_CNT_READ_DIRECT_READ));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  14,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, processor_ndx, rd_processor_read.rd_req_addr_f, counter_ndx));
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, rd_processor_read.rd_processor_read_r, REG_PORT_ANY, processor_ndx, rd_processor_read.rd_req_f,  0x1));
  ...
  */
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN(res, 20, exit, soc_reg_above_64_get(unit, rd_result.rd_result_r, REG_PORT_ANY, processor_ndx, reg_val));

  soc_reg_above_64_field_get(unit, rd_result.rd_result_r, reg_val, rd_result.rd_data_valid_f, valid_bit);
  if (valid_bit[0]) {
      if (SOC_IS_JERICHO(unit)) {
          COMPILER_64_ZERO(cnt_val);
          val64 = soc_reg_above_64_field64_get(unit, rd_result.rd_result_r, reg_val, rd_result.rd_pkt_cnt_f);
          COMPILER_64_ADD_64(cnt_val, val64);
          /* 
           * In jericho we read only one register and parse its' results according to format
           * Packets And Bytes (26b for packets counter, 33b for bytes counter.)
           * Only packets (59b)
           ...
           * Double packet count (which doubles our total number of counter sets). (26b *2)
           */
          JER_CNT_PKT_CNT_GET(read_rslt->pkt_cnt, cnt_val, counter_info->format);         
          JER_CNT_BYTE_CNT_GET(read_rslt->byte_cnt, cnt_val, counter_info->format);
      } else {
          soc_reg_above_64_field_get(unit, rd_result.rd_result_r, reg_val, rd_result.rd_pkt_cnt_f, pkt_cnt);
          COMPILER_64_SET(read_rslt->pkt_cnt, 0, pkt_cnt[0]);

          soc_reg_above_64_field_get(unit, rd_result.rd_result_r, reg_val, rd_result.rd_oct_cnt_f, oct_cnt);
  ...
  )
{
  ...
  counter_id = ((queue_ndx - info.voq_cnt.start_q) / info.voq_cnt.q_set_size);
