    ...
    void *userdata)
{
    ...
    if (dcmn_intr_interrupt_recurring_detect(unit, block_instance, interrupt_id)) {
        /*recurring action*/
        rc = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].func_arr_recurring_action(unit, block_instance, interrupt_id, NULL);
        SOCDNX_IF_ERR_EXIT(rc);
    } else {
        if (SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].func_arr!=NULL) {
            rc = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[interrupt_id].func_arr(unit, block_instance, interrupt_id, NULL); 
            SOCDNX_IF_ERR_EXIT(rc);
        }
    }
