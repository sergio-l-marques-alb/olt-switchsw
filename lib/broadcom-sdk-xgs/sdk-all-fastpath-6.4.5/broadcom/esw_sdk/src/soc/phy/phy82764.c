/*
 * $Id: phy82764.c,v 1.1.2.25 Broadcom SDK $
...
typedef struct phy82764_device_cfg_aux_mode{
    ...
    PHY82764_CORE_TYPE passthru_sys_side_core;
    uint16_t reserved;
...
STATIC int
phy_82764_tx_lane_squelch_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value);
STATIC int
...
int _phy82764_sys_side_lane_map_get(int unit, soc_port_t port, phymod_phy_access_t *pm_phy_copy)
{
    phy_ctrl_t *pc;
...
phy82764_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    /* phy82764_device_cfg_aux_mode_t assignment */
    interface_config.device_aux_modes = (phy82764_device_cfg_aux_mode_t *) pCfg->device_cfg_aux_modes;
...
phy82764_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    /*mem for phy82764_device_cfg_aux_mode_t */
    interface_config.device_aux_modes = sal_alloc(sizeof(phy82764_device_cfg_aux_mode_t), "phy82764_device_cfg_aux_mode");
...
int _phy82764_speed_to_lane_map_get(phy_ctrl_t *pc, uint32 *line_lane_map, uint32 *sys_lane_map)
{
    int pt_mode = 0;
    int pt_enzo_mode = 0;

    pt_mode = soc_property_port_get(pc->unit, pc->port,
                              "phy82764_pt_mode", 0);
    pt_enzo_mode = soc_property_port_get(pc->unit, pc->port,
                              "phy82764_pt_enzo_mode", 0);
    *line_lane_map = 0xF;
    *sys_lane_map = 0xF;
    switch(pc->speed_max) {
        case PHY82764_SPD_40G:
            if ((pt_mode == TRUE) && (pt_enzo_mode != TRUE)) {
                *line_lane_map = 0x33;
                *sys_lane_map = 0xf; 
                pc->phy_mode = PHYCTRL_QUAD_LANE_PORT;
            } else if (pt_enzo_mode) {
                *line_lane_map = 0xf;
                *sys_lane_map = 0xf;
                pc->phy_mode = PHYCTRL_QUAD_LANE_PORT;
            } else {
        ...
        case PHY82764_SPD_10G:
            if (pt_enzo_mode) {
              ...
              *sys_lane_map = *line_lane_map;
            } else {
                if (pc->lane_num > 1) {
                    *line_lane_map = 0x10;
                    *line_lane_map <<= (pc->lane_num - 2);
                    *sys_lane_map = 0x1;
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    if (soc_property_port_get(pc->unit, pc->port, "82764_glue", 0)) {
...
_phy_82764_init_pass1(int unit, soc_port_t port)
{
    ...
    if (DEVID(pc) == SESTO_ID_82764) { 
        ...
        sal_strncpy (dev_name, "BCM82764", sal_strlen("BCM82764"));
    } else {
        /* coverity[secure_coding] */
        sal_strncpy (dev_name, "Sesto", sal_strlen("Sesto"));
    }
...
_phy_82764_init_pass2(int unit, soc_port_t port)
{
    ...
    interface_config.device_aux_modes = sal_alloc(sizeof(phy82764_device_cfg_aux_mode_t), "phy82764_device_cfg_aux_mode");
    pCfg->device_cfg_aux_modes = (phy82764_device_cfg_aux_mode_t *)(interface_config.device_aux_modes) ;
    pCfg->device_cfg_aux_modes->pass_thru = soc_property_port_get(unit, port, "phy82764_pt_mode", 0);
    pCfg->device_cfg_aux_modes->BCM84793_capablity = soc_property_port_get(unit, port,
                                   "phy82764_pt_enzo_mode", 0);
    /* One time config for BCM82764 */
    pCfg->device_cfg_aux_modes->gearbox_100g_inverse_mode = 0;
    pCfg->device_cfg_aux_modes->passthru_sys_side_core = PHY82764_25G_CORE;

    SOC_IF_ERROR_RETURN(_phy82764_speed_to_lane_map_get(pc, &line_lane_map, &sys_lane_map));
    ...
    sys_intf = soc_property_port_get(unit, port, spn_PHY_SYS_INTERFACE, 0);
    if (sys_intf) {
...
phy_82764_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    sal_memset(ability, 0, sizeof(*ability));
    if (pc->speed_max == PHY82764_SPD_40G) {
        pa_speed = SOC_PA_SPEED_40GB | SOC_PA_SPEED_10GB;
    } else if (pc->speed_max == PHY82764_SPD_10G) {
...
STATIC int
phy_82764_firmware_set(int unit, int port, int offset, uint8 *array,int datalen)
{
...
/* 
 * phy_82764_loopback_remote_get
 */
STATIC int 
phy_82764_loopback_remote_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t    *pm_phy;
    uint32_t               enable;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_prbs_tx_poly_get
 */
STATIC int
phy_82764_prbs_tx_poly_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_rx_poly_get
 */
STATIC int
phy_82764_prbs_rx_poly_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_prbs_tx_invert_data_get
 */
STATIC int
phy_82764_prbs_tx_invert_data_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_rx_invert_data_get
 */
STATIC int
phy_82764_prbs_rx_invert_data_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_tx_enable_get
 */
STATIC int
phy_82764_prbs_tx_enable_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    uint32_t flags = 0;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_prbs_rx_enable_get
 */
STATIC int
phy_82764_prbs_rx_enable_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    uint32_t flags = 0;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_prbs_rx_status_get
 */
STATIC int
phy_82764_prbs_rx_status_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t    *pm_phy;
    phymod_prbs_status_t   prbs_tmp;
...
/*
 * phy_82764_rx_low_freq_filter_get
 */
STATIC int
phy_82764_rx_low_freq_filter_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_rx_peak_filter_get
 */
STATIC int 
phy_82764_rx_peak_filter_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_rx_vga_set
 */
STATIC int 
phy_82764_rx_vga_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_rx_tap_get
 */
STATIC int 
phy_82764_rx_tap_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, int tap, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_Rx_seq_done_get
 */
STATIC int
phy_82764_rx_seq_done_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_phy_access_t pm_phy_copy;
 ...
 */
STATIC int 
phy_82764_fec_enable_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
    return(SOC_E_UNAVAIL);
}
/* 
 * phy_82764_tx_fir_pre_get
 */
STATIC int
phy_82764_tx_fir_pre_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t *pm_phy;
...
/* 
 * phy_82764_tx_fir_main_get
 */
STATIC int
phy_82764_tx_fir_main_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_tx_fir_post_get
 */
STATIC int
phy_82764_tx_fir_post_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_tx_fir_post2_get
 */
STATIC int
phy_82764_tx_fir_post2_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_tx_fir_post3_get
 */
STATIC int
phy_82764_tx_fir_post3_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_rx_signal_detect_get
 */
STATIC int
phy_82764_rx_signal_detect_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
   /* $$$ Need to add diagnostic API */
   return(SOC_E_UNAVAIL);
}
/* 
 * phy_82764_rx_ppm_get
 */
STATIC int
phy_82764_rx_ppm_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    /* $$$ Need to add diagnostic API */
    return(SOC_E_UNAVAIL);
}
...
/* 
 * phy_82764_tx_polarity_get
 */
STATIC int 
phy_82764_tx_polarity_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_polarity_t    polarity;
    phymod_phy_access_t pm_phy_copy;
...
/* 
 * phy_82764_rx_polarity_get
 */
STATIC int 
phy_82764_rx_polarity_get(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_polarity_t    polarity;
    phymod_phy_access_t pm_phy_copy;
...
phy_82764_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_POLYNOMIAL:
      rv = phy_82764_prbs_tx_poly_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_INVERT_DATA:
      rv = phy_82764_prbs_tx_invert_data_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_ENABLE:
      rv = phy_82764_prbs_tx_enable_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_POLYNOMIAL:
      rv = phy_82764_prbs_rx_poly_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_INVERT_DATA:
      rv = phy_82764_prbs_rx_invert_data_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_ENABLE:
      rv = phy_82764_prbs_rx_enable_get(pmc, port, intf, value);
      break;
    ...
    case SOC_PHY_CONTROL_PRBS_RX_STATUS:
      rv = phy_82764_prbs_rx_status_get(pmc, port, intf, value);
      break;
    /* LOOPBACK */
    case SOC_PHY_CONTROL_LOOPBACK_REMOTE:
       rv = phy_82764_loopback_remote_get(pmc, port, intf, value);
       break;
    case SOC_PHY_CONTROL_LOOPBACK_PMD:
       rv = phy_82764_loopback_internal_pmd_get(pmc, port, intf, value);
       break;
    /* PREEMPHASIS */
    ...
    case SOC_PHY_CONTROL_TX_FIR_PRE:
        ...
        rv = phy_82764_tx_fir_pre_get(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_MAIN:
        /* assume they are all the same as lane 0 */
        rv = phy_82764_tx_fir_main_get(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST:
        /* assume they are all the same as lane 0 */
        rv = phy_82764_tx_fir_post_get(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST2:
        /* assume they are all the same as lane 0 */
        rv = phy_82764_tx_fir_post2_get(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST3:
        /* assume they are all the same as lane 0 */
        rv = phy_82764_tx_fir_post3_get(pmc, port, intf, value);
        break;
    ...
    case SOC_PHY_CONTROL_RX_PEAK_FILTER:
        rv = phy_82764_rx_peak_filter_get(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_RX_LOW_FREQ_PEAK_FILTER:
        rv = phy_82764_rx_low_freq_filter_get(pmc, port, intf, value);
        break;

    /* RX VGA */
    case SOC_PHY_CONTROL_RX_VGA:
        rv = phy_82764_rx_vga_get(pmc, port, intf, value);
        break;

    /* RX TAP */
    case SOC_PHY_CONTROL_RX_TAP1:
        rv = phy_82764_rx_tap_get(pmc, port, intf, 0, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP2:
        rv = phy_82764_rx_tap_get(pmc, port, intf, 1, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP3:
        rv = phy_82764_rx_tap_get(pmc, port, intf, 2, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP4:
        rv = phy_82764_rx_tap_get(pmc, port, intf, 3, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP5:
        rv = phy_82764_rx_tap_get(pmc, port, intf, 4, value);
        break;
    ...
    case SOC_PHY_CONTROL_RX_SIGNAL_DETECT:
      rv = phy_82764_rx_signal_detect_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_RX_SEQ_DONE:
      rv = phy_82764_rx_seq_done_get(pmc, port, intf, value);
      break;
    case SOC_PHY_CONTROL_RX_PPM:
      rv = phy_82764_rx_ppm_get(pmc, port, intf, value);
      break;
    /* FEC */
    case SOC_PHY_CONTROL_FORWARD_ERROR_CORRECTION:
      rv = phy_82764_fec_enable_get(pmc, value);
      break;
    case SOC_PHY_CONTROL_RX_POLARITY:
        rv = phy_82764_rx_polarity_get(pmc, port, intf, value);;
        break;
    case SOC_PHY_CONTROL_TX_POLARITY:
        rv = phy_82764_tx_polarity_get(pmc, port, intf, value);
        break;
    /* POWER */
    ...
    case SOC_PHY_CONTROL_TX_LANE_SQUELCH:
        rv = phy_82764_tx_lane_squelch_get(pmc, port, intf, value);
    break;
    default:
       rv = SOC_E_UNAVAIL;
       break;
...
/* 
 * phy_82764_loopback_remote_set
 */
STATIC int 
phy_82764_loopback_remote_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    int                    idx;
    phymod_phy_access_t pm_phy_copy;
 ...
 */
STATIC int 
phy_82764_fec_enable_set(soc_phymod_ctrl_t *pmc, uint32 value)
{
    return(SOC_E_UNAVAIL);
}

STATIC int
phy_82764_sdk_poly_to_phymod_poly(uint32 sdk_poly, uint32 *phymod_poly){
    switch(sdk_poly){
...
/* 
 * phy_82764_prbs_tx_poly_set
 */
STATIC int
phy_82764_prbs_tx_poly_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_rx_poly_set
 */
STATIC int
phy_82764_prbs_rx_poly_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_tx_invert_data_set
 */
STATIC int
phy_82764_prbs_tx_invert_data_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_rx_invert_data_set
 */
STATIC int
phy_82764_prbs_rx_invert_data_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_prbs_t        prbs;
    uint32_t flags = 0;
...
/* 
 * phy_82764_prbs_tx_enable_set
 */
STATIC int
phy_82764_prbs_tx_enable_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    int      idx;
    uint32_t flags = 0;
    phymod_phy_access_t  *pm_phy;
...
/* 
 * phy_82764_prbs_rx_enable_set
 */
STATIC int
phy_82764_prbs_rx_enable_set(soc_phymod_ctrl_t *pmc, soc_port_t port, uint32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    uint32_t flags = 0;
    int                    idx;
        ...
        if(intf == PHY82764_SYS_SIDE) {
            switch (pc->speed_max) {    
                case PHY82764_SPD_40G: 
                    if ((pCfg->device_cfg_aux_modes->pass_thru) || 
                        (pCfg->device_cfg_aux_modes->BCM84793_capablity)) {
                        if (lane > 3) {
                            return SOC_E_CONFIG;
                        }
                        *lane_map = 0x1 << lane; 
                ...
                case PHY82764_SPD_10G:
                    ...
                    if (pCfg->device_cfg_aux_modes->BCM84793_capablity) {
            ...
            switch (pc->speed_max) {    
                case PHY82764_SPD_40G: 
                    ...
                    } else if (pCfg->device_cfg_aux_modes->BCM84793_capablity) {
                        *lane_map = 0x1 << lane;   
                        found = 1; 
                    } else {
                        if (pc->lane_num > 1) {
...
/* 
 * phy_82764_preemphasis_set
 */
STATIC int
phy_82764_preemphasis_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    int                 idx;
...
/* 
 * phy_82764_tx_fir_post3_set
 */
STATIC int
phy_82764_tx_fir_post3_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_tx_t         phymod_tx;
    int                 idx;
...
/* 
 * phy_82764_tx_lane_squelch
 */
STATIC int
phy_82764_tx_lane_squelch(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_phy_tx_lane_control_t  tx_control;
    int                 idx;
...
/* 
 * phy_82764_rx_peak_filter_set
 */
STATIC int 
phy_82764_rx_peak_filter_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
 ...
 */
STATIC int 
phy_82764_rx_vga_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
...
STATIC int
phy_82764_rx_tap_release(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, int tap)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
...
/* 
 * phy_82764_rx_tap_set
 */
STATIC int 
phy_82764_rx_tap_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, int tap, uint32 value)
{
    phymod_phy_access_t *pm_phy;
    phymod_rx_t         phymod_rx;
    int                 idx;
...
/* 
 * phy_82764_tx_polarity_set
 */
STATIC int 
phy_82764_tx_polarity_set(soc_phymod_ctrl_t *pmc, soc_port_t port, phymod_polarity_t *cfg_polarity, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_polarity_t    polarity;
    int                  idx;
...
/* 
 * phy_82764_rx_polarity_set
 */

STATIC int 
phy_82764_rx_polarity_set(soc_phymod_ctrl_t *pmc, soc_port_t port, phymod_polarity_t *cfg_polarity, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_polarity_t    polarity;
    int                  idx;
...
/* 
 * phy_82764_rx_low_freq_filter_set
 */
STATIC int
phy_82764_rx_low_freq_filter_set(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, uint32 value)
{
    phymod_phy_access_t  *pm_phy;
    phymod_rx_t          phymod_rx;
    int                  idx;
...
STATIC int
phy_82764_phy_dump(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf)
{
    phymod_phy_access_t     *pm_phy;
    int                     idx;
    phymod_phy_access_t     pm_phy_copy;
...
phy_82764_control_set(int unit, soc_port_t port, soc_phy_control_t type, uint32 value)
{
    ...
    /* decoupled PRBS */
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_POLYNOMIAL:
        rv = phy_82764_prbs_tx_poly_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_INVERT_DATA:
        rv = phy_82764_prbs_tx_invert_data_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_TX_ENABLE:
        rv = phy_82764_prbs_tx_enable_set(pmc, port, intf, value);
        break; 
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_POLYNOMIAL:
        rv = phy_82764_prbs_rx_poly_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_INVERT_DATA:
        rv = phy_82764_prbs_rx_invert_data_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_ENABLE:
        rv = phy_82764_prbs_rx_enable_set(pmc, port, intf, value);
        break;
    ...
    /* LOOPBACK */
    case SOC_PHY_CONTROL_LOOPBACK_PMD:
        rv = phy_82764_loopback_internal_pmd_set(pmc, port, intf, value);
       break;
    case SOC_PHY_CONTROL_LOOPBACK_REMOTE:
        rv = phy_82764_loopback_remote_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_PRE:
        rv = phy_82764_tx_fir_pre_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_MAIN:
        rv = phy_82764_tx_fir_main_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST:
        rv = phy_82764_tx_fir_post_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST2:
        rv = phy_82764_tx_fir_post2_set(pmc, port, intf, value);
        break;
    case SOC_PHY_CONTROL_TX_FIR_POST3:
        rv = phy_82764_tx_fir_post3_set(pmc, port, intf, value);
        break;
    /* PREEMPHASIS */
    case SOC_PHY_CONTROL_PREEMPHASIS_LANE0:
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS_LANE3:
        ...
        break;
    case SOC_PHY_CONTROL_PREEMPHASIS:
        rv = phy_82764_preemphasis_set(pmc, port, intf, value);
        break;
    ...
    case SOC_PHY_CONTROL_DRIVER_CURRENT_LANE3:
        ...
        break;
    case SOC_PHY_CONTROL_DRIVER_CURRENT:
        rv = phy_82764_driver_current_set(pmc, port, intf, value);
        break;
    /* TX LANE SQUELCH */
    case SOC_PHY_CONTROL_TX_LANE_SQUELCH:
        rv = phy_82764_tx_lane_squelch(pmc, port, intf, value);
        break;
    /* RX PEAK FILTER */
    case SOC_PHY_CONTROL_RX_PEAK_FILTER:
        rv = phy_82764_rx_peak_filter_set(pmc, port, intf, value);
        break;

    /* RX VGA */
    case SOC_PHY_CONTROL_RX_VGA:
        rv = phy_82764_rx_vga_set(pmc, port, intf, value);
        break;
    /* RX TAP */
    case SOC_PHY_CONTROL_RX_TAP1:
        rv = phy_82764_rx_tap_set(pmc, port, intf, 0, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP2:
        rv = phy_82764_rx_tap_set(pmc, port, intf, 1, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP3:
        rv = phy_82764_rx_tap_set(pmc, port, intf, 2, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP4:
        rv = phy_82764_rx_tap_set(pmc, port, intf, 3, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP5:
        rv = phy_82764_rx_tap_set(pmc, port, intf, 4, value);
        break;
    case SOC_PHY_CONTROL_RX_TAP1_RELEASE:       /* $$$ tbd $$$ */
       rv = phy_82764_rx_tap_release(pmc, port, intf, 0);
       break;
    case SOC_PHY_CONTROL_RX_TAP2_RELEASE:       /* $$$ tbd $$$ */
       rv = phy_82764_rx_tap_release(pmc, port, intf, 1);
       break;
    case SOC_PHY_CONTROL_RX_TAP3_RELEASE:       /* $$$ tbd $$$ */
       rv = phy_82764_rx_tap_release(pmc, port, intf, 2);
       break;
    case SOC_PHY_CONTROL_RX_TAP4_RELEASE:       /* $$$ tbd $$$ */
       rv = phy_82764_rx_tap_release(pmc, port, intf, 3);
       break;
    case SOC_PHY_CONTROL_RX_TAP5_RELEASE:       /* $$$ tbd $$$ */
       rv = phy_82764_rx_tap_release(pmc, port, intf, 4);
       break;
    /* PHASE INTERPOLATOR */
    case SOC_PHY_CONTROL_PHASE_INTERP:
    ...
    /* POLARITY */
    case SOC_PHY_CONTROL_RX_POLARITY:
        rv = phy_82764_rx_polarity_set(pmc, port, &pCfg->phy_polarity_config, intf, value);;
        break;
    case SOC_PHY_CONTROL_TX_POLARITY:
        rv = phy_82764_tx_polarity_set(pmc, port, &pCfg->phy_polarity_config, intf, value);
        break;
    /* RESET */
    ...
    case SOC_PHY_CONTROL_POWER:
       ...
       break;
    case SOC_PHY_CONTROL_RX_LOW_FREQ_PEAK_FILTER:
        rv = phy_82764_rx_low_freq_filter_set(pmc, port, intf, value); 
    break;
    case SOC_PHY_CONTROL_FORWARD_ERROR_CORRECTION:
       rv = phy_82764_fec_enable_set(pmc, value);
       break;
    case SOC_PHY_CONTROL_RX_SEQ_TOGGLE:
        rv = phy_82764_rx_seq_restart(pmc, port, intf, value);
    break;
    case SOC_PHY_CONTROL_DUMP:
        rv = phy_82764_phy_dump(pmc, port, intf);
    break;
    case SOC_PHY_CONTROL_CL72:
...
STATIC int phy_82764_diag_eyescan(soc_port_t port, soc_phymod_ctrl_t *pmc, int32 intf, int32 lane)
{
    phymod_phy_access_t     *pm_phy;
    int                     idx = 0, rv = 0;
    phymod_phy_access_t     pm_phy_copy;
...
STATIC int 
phy_82764_diag_dsc(soc_phymod_ctrl_t *pmc, soc_port_t port, int32 intf, int lane)
{
    phymod_phy_access_t  *pm_phy;
    int                    idx;
    phymod_phy_access_t pm_phy_copy;
    ...
    void *arg)       /* command argument based on op_type/op_cmd */
{
    ...
    switch(op_cmd) {
        case PHY_DIAG_CTRL_DSC:
            rv = phy_82764_diag_dsc(pmc, port, intf, lane);
            break;
        case PHY_DIAG_CTRL_START_FAST_EYESCAN:
            rv =  phy_82764_diag_eyescan(port, pmc, intf, lane);
        break;
        default:
