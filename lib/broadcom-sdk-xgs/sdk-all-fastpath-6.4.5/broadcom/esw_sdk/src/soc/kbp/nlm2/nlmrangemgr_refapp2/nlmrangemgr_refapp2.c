 ...
 */
/* 
 * $Id: nlmrangemgr_refapp2.c,v 1.4 Broadcom SDK $
 * $Copyright: Copyright 2012 Broadcom Corporation.
 * This program is the proprietary software of Broadcom Corporation
 * and/or its licensors, and may only be used, duplicated, modified
 * or distributed pursuant to the terms and conditions of a separate,
...
#include "nlmrangemgr_refapp2.h"

#ifndef NLM_NO_DEVMGR
/* Initialize and Transport Layer */
...
int NlmRangeMgrRefApp__InitRangeMgr( NlmRangeMgrRefAppData *refAppData_p )
{
	 ...
	 */
	NlmRangeDbAttrSet          db_attrs[NUM_OF_RANGE_DATABASES] = { {32, 16, NLM_RANGE_3B_ENCODING},
...
int NlmRangeMgrRefApp__ConfigSearches( NlmRangeMgrRefAppData *refAppData_p )
{
    ...
    NlmCm__printf("\n\n\t Search Configuration for Range\n");
	NlmCm__printf("\tRange A Extraction Start Byte = %d,  Range B Extraction Start Byte = %d\n",
					srch_attrs->m_extraction_startByte_rangeA,
					srch_attrs->m_extraction_startByte_rangeB);
	NlmCm__printf("\tRange C Extraction Start Byte = %d,  Range D Extraction Start Byte = %d\n",
					srch_attrs->m_extraction_startByte_rangeC,
					srch_attrs->m_extraction_startByte_rangeD);
	NlmCm__printf("\t[Key#0] : Range A KeyInsert StartByte = %d, Range B KeyInsert StartByte = %d\n",
					srch_attrs->m_keyInsert_startByte_rangeA[ NLMDEV_KEY_0],
					srch_attrs->m_keyInsert_startByte_rangeB[ NLMDEV_KEY_0]);
	NlmCm__printf("\t[Key#1] : Range C KeyInsert StartByte = %d, Range D KeyInsert StartByte = %d\n",
...
int NlmRangeMgrRefApp__AddRecords( NlmRangeMgrRefAppData *refAppData_p )
{
    ...
    nlm_u32    tbl_id, lsb_numBits, msb_numBits, mask_val;
    nlm_u16    block_index, iter1;
#endif
	NlmReasonCode   reason;
	nlm_u32    range_id0, range_id1;
	nlm_u16    iter, iter0;
	...
	for( iter = 0; iter < 2; iter++)
	{
        ...
        NlmCm__printf("\n\t Range Encodings for Range Id %d of Range Database with Id %d are as follows\n",
            range_id0, db0_p->m_id);
        for(iter0 = 0; iter0 < enc0_p->m_num_entries; iter0++)
        {
            NlmCm__printf("\t Data: 0x%04x Mask : 0x%04x\n", (enc0_p->m_entries_p + iter0)->m_data,
        ...
        NlmCm__printf("\n\t Range Encodings for Range Id %d of Range Database with Id %d are as follows\n",
            range_id1, db1_p->m_id);
        for(iter0 = 0; iter0 < enc1_p->m_num_entries; iter0++)
        {
            NlmCm__printf("\t Data: 0x%04x Mask : 0x%04x\n", (enc1_p->m_entries_p + iter0)->m_data,
                  (enc1_p->m_entries_p + iter0)->m_mask);
        }
...
#ifndef NLM_NO_DEVMGR
int NlmRangeMgrRefApp2__PerformSearches( NlmRangeMgrRefAppData *refAppData_p )
{
	...
	else
	{
		...
		for( iter = 0; iter < NLMDEV_NUM_PARALLEL_SEARCHES; iter++ ){
			...
			if( NLMDEV_HIT == search_result.m_hitOrMiss[ iter ] ){
...
#endif
int nlmrangemgr_refapp2_main(int argc, char	*argv[])
{
    ...
    NlmCm__memset( &refAppData, 0, sizeof(NlmRangeMgrRefAppData) );

	/* create default memory allocator */
    refAppData.alloc_p = NlmCmAllocator__ctor( &refAppData.alloc_bdy );
    if( refAppData.alloc_p == NULL )
    {
        NlmCm__printf("\n\tCould not create Memory Allocator. Exiting..!!!!!\n");
...
#ifndef NLM_NO_DEVMGR
	/* Do a search now. */
	if( NLMERR_OK != NlmRangeMgrRefApp2__PerformSearches( &refAppData ) )
	{
