/*
 * $Id: cint_pon_mact_limit.c,v 1.4 Broadcom SDK $
 ...
 * Example: pon_mact_limit_init(0, 1, 128, 16);
 */
...
int get_mac_per_ac(int unit, int pon_port, int tunnel_id)
{
	...
	rv = bcm_l2_matched_traverse(0,flags, &match_addr, call_back_get_block, &a);
    if (rv != BCM_E_NONE)
    {
	...
	a = 0;
	rv = bcm_l2_matched_traverse(0,flags, &match_addr, call_back_count, &a);
    if (rv != BCM_E_NONE)
    {
