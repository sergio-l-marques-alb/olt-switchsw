/* $Id: cint_fcoe_route.c,v 1.2 Broadcom SDK $
 ...
 *   Examples
 *   --------
 *   (1)run transit: print fcoe_transit_example(0, src_port, dst_port);
 *   (2)run fcf: print fcoe_fcf_example(0,src_port, dst_port, entry_id, insertion_flags, mask);
...
/* print find_route(0,0x00ffff00,0x00ffffff,0,BCM_FCOE_HOST_ROUTE);*/
...
int fcf_workaround(int *units_ids, int nof_units){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        /* work Around in pmf: to fix the offset of forwarding header*/
        rv = fcf_wa_setup(unit,0,fix_gport,10);
        if (rv != BCM_E_NONE) {
            printf("Error, FCF header work around setting unit %d \n", unit);
        }        
    }
    return rv;
}
...
int fcf_traps_set(int *units_ids, int nof_units,uint8 drop){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (rv != BCM_E_NONE) {
            printf("Error, FCF work around setting unit %d \n", in_sysport, unit);
        }
    }
...
{    
    int open_vlan = 1; /* if 1 need to create vlan */
    int rv;
    
    /*** create ingress router interface ***/
    rv = create_l3_intf(unit, flags, open_vlan, src_port, in_vlan, 0, my_mac, ing_intf_in); 
    if (rv != BCM_E_NONE) {
        printf("Error, create ingress interface, in_sysport=%d, in unit %d \n", in_sysport, unit);
    }

	/*** create egress router interface ***/    
    rv = create_l3_intf(unit,flags,open_vlan, dst_port, out_vlan, 0, my_mac, ing_intf_out); 
    if (rv != BCM_E_NONE) {
        printf("Error, create egress interface, dst_port=%d, in unit %d \n", dst_port, unit);
    }
...
int fcoe_fcf_zone_filter_enable(int unit)
{
    ...
    /* work Around in pmf: to drop packet that zoning lookup fail for them */
    rv = lem_zoning_wa_setup(unit,1,pmf_group_id_for_zoning);
    if (rv != BCM_E_NONE) {
        printf("Error, FCF zoning work around setting unit %d \n", unit);
    }
...
int fcoe_fcf_example(int unit,int in_sysport, int out_sysport, uint32 insertion_flags, int mask, int vft_2)
{
    ...
    for (indx = 0; indx < 3; indx++) {
        rv = create_l3_egress(unit,0,out_sysport,in_vlan,ing_intf_out,next_hop_mac_array[indx], &fec[indx], &encap_id[indx]); 

        if (rv != BCM_E_NONE) {
            printf("Error, create_l3_egress unit %d, \n", unit);
        }
...
int fcf_zoning_example(int *units_ids, int nof_units, int in_sysport, int out_sysport){
    ...
    /* work around needed for FCF, fix header building*/
    ...
    for (indx = 0; indx < nof_next_hops; indx++) {
        ...
        for (i = 0 ; i < nof_units ; i++){
            unit = units_ids[i];
            rv = create_l3_egress(unit,flags1,out_sysport,out_vlan,ing_intf_out,next_hop_mac, &fec[indx], &encap_id[indx]); 
            if (rv != BCM_E_NONE) {
