/*
 * $Id: cint_queue_tests.c,v 1.14 Broadcom SDK $
 *
 ...
 */
int get_core_from_port(int unit, bcm_gport_t gport)
{
    bcm_error_t rc ;
    int core, ret ;
    uint32 dummy_flags ;
...
int test_create_sysport_and_queue_bundle_from_port(int unit, int nof_qs_in_bundle, bcm_gport_t dest_port, int sysport, bcm_gport_t *out_q_base, bcm_gport_t *orig_sysport_port_dest) {
    ...
    } else if ((rv = bcm_stk_sysport_gport_set(unit, sysport_gport, dest_port)) != BCM_E_NONE) {
 ...
 */
int test_sysport_to_modport_to_queue_bundle_mapping(int unit) {
	int sysport = 3000;
	...
	int qid = 760;
    int rv, i;
	int nof_qs_in_bundle_get, flags;
	
	bcm_gport_t mod_port, gport_test_get, base_queues, sysport_gport, out_q_base, out_q_base_get;
	
	BCM_GPORT_MODPORT_SET(mod_port, mod_id, port);
	BCM_GPORT_SYSTEM_PORT_ID_SET(sysport_gport, sysport);
	...
	for (i = 0; i < 2; ++i) {
		if ((rv = bcm_stk_sysport_gport_get(unit, ports[i], &gport_test_get)) != BCM_E_NONE) {
			...
			printf ("problem in result from bcm_stk_sysport_gport_get %d\n");
			rv = BCM_E_CONFIG;
...
int test_delete_sysport_and_queue_bundle(int unit, bcm_gport_t q_base, int sysport, bcm_gport_t orig_sysport_port_dest) {
    ...
    } else if ((rv = bcm_stk_sysport_gport_set(unit, sysport_gport, orig_sysport_port_dest)) != BCM_E_NONE) {
...
int test_voq_attributes(int unit, int chip_rev) /* chip_rev == 0 means ARAD_A0, 3 is Arad+, 4 is jericho */
{
    int rv;
    int test_q_num_in_q_bundle = 4;
    int test_first_queue = 64 * 1024 - 256;
...
int test_voq_attributes_ingress_queue_bundle(int unit, int chip_rev) /* chip_rev == 0 means ARAD_A0, 3 is Arad+, 4 is jericho */
{
    int rv;
    int test_q_num_in_q_bundle = 4;
    int test_first_queue = 64 * 1024 - 256;
...
int test_voq_attributes_range(int unit, int chip_rev, int test_q_num_in_q_bundle, int test_first_queue, int test_last_queue) /* chip_rev == 0 means ARAD_A0, 3 is Arad+ */
{
    ...
    dt1.credit_request_watchdog_status_msg_gen = (chip_rev == 3 || chip_rev == 4) ? 0 : 125;
    ...
    dt1.credit_request_satisfied_backoff_enter_thresh = 16000;
    dt1.credit_request_satisfied_backoff_exit_thresh = 17000;
    dt1.credit_request_satisfied_backlog_enter_thresh = 2040;
    dt1.credit_request_satisfied_backlog_exit_thresh = 4099;
    ...
    int orig_discount;
    printf("4. Configure queues' Credit Class.\n");
    ...
    if (chip_rev == 3 || chip_rev == 4) { /* check Arad+ two credit values selected per Q type */
        ...
        BCM_GPORT_MODPORT_SET(sysport_destinations[0], mod1, a_port);
        BCM_GPORT_MODPORT_SET(sysport_destinations[1], mod2, a_port);
...
/*
 ...
 Works on queues it creates, using set and get operations.
 */
int test_credit_watchdog_old(int unit)
{
    int rv;
    ...
    if ((rv = verify_credit_watchdog_thresholds (unit, &dt1, &dt2, &dt3, 3, 0,
         1, 0, 10, 0, "after setting queue range to 160 - 1024*90-1"))) { return rv; }
    ...
    if ((rv = verify_credit_watchdog_thresholds (unit, &dt1, &dt2, &dt3, 3, 0,
         1, 0, 10, 0, "after setting queue range to 16 - 80*1024-1"))) { return rv; }
    ...
    if ((rv = verify_credit_watchdog_thresholds (unit, &dt1, &dt2, &dt3, 3, 0,
         1, 0, 10, 0, "after disabling with queue range 16 - 80*1024-1"))) { return rv; }
 ...
 */
int test_credit_watchdog_common_fsm(int unit, int is_arad_plus)
{
    int i, rv;
    bcm_cosq_delay_tolerance_t dt;
 ...
 */
int test_credit_watchdog(int unit, int is_arad_plus)
{
    int i, j, rv;
    bcm_cosq_delay_tolerance_t dt;
    ...
    if (is_arad_plus) {
        if ((rv = test_credit_watchdog_common_fsm(unit, is_arad_plus))) {
    ...
    if ((rv = test_credit_watchdog_old(unit)) != BCM_E_NONE) {
