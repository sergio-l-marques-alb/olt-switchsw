...
int bcm_l3_ip6_with_ttl(int unit, int second_unit, int in_port, int out_port, int ttl_threshold) {
  ...
  if (second_unit<0)
  {
      ...
      if (is_device_or_above(unit,JERICHO)) {
          /* create in-rif */
          /* TTL/hop-limit threshold in valid only only in the egress, so for in-rif it is set to max - 255 */
          rc = create_l3_intf(unit ,CINT_NO_FLAGS, in_port, in_vlan + 2, 0, mac_l3_ingress, 255, &ingress_intf[2]);
          if (rc != BCM_E_NONE) {
            printf ("create_l3_intf failed: %d \n", rc);
            return rc;
          }
          ...
          /* create out-rif */
          rc = create_l3_intf(unit ,CINT_NO_FLAGS, out_port, out_vlan + 2, 0, mac_l3_ingress, ttl_threshold, &ingress_intf[3]);
          if (rc != BCM_E_NONE) {
            printf ("create_l3_intf failed: %d \n", rc);
            return rc;
          }
          ...
          /* create fec for UC IPV6 entry */
          rc = create_l3_egress(unit,CINT_NO_FLAGS, out_port, out_vlan + 2, ingress_intf[3], mac_l3_egress, &egress_intf[1], &encap_id[1]);
          if (rc != BCM_E_NONE) {
            printf ("create_l3_egress failed: %d \n", rc);
            return rc;
          }
