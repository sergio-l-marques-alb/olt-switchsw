...
vswitch_vpls_hvpls_info_init(int unit, int ac_port, int pwe_in_port, int pwe_out_port, int ac_port1_outer_vlan, int ac_port1_inner_vlan, int vpn_id) {
    ...
    vswitch_vpls_hvpls_info.use_two_calls = 0;

    vswitch_vpls_hvpls_info.vlan = 200;

    vswitch_vpls_hvpls_info.pwe_outlif_primary = 0x10000;  
    vswitch_vpls_hvpls_info.pwe_outlif_protected = 0x10002; 
...
mpls_tunnels_config(int unit){
    ...
    for (is_primary=0; is_primary<=1; is_primary++) {
        ...
        if (mpls_termination_label_index_enable) {
            BCM_MPLS_INDEXED_LABEL_SET(&label_for_pop_entry_out, label_for_pop_entry_out, vswitch_vpls_info_1.access_index_1);
            BCM_MPLS_INDEXED_LABEL_SET(&label_for_pop_entry_in, label_for_pop_entry_in, 1);
        }
...
vswitch_vpls_hvpls_egress_mpls_port_create(int unit){
    ...
    mpls_port_egress.flags2 = BCM_MPLS_PORT2_EGRESS_ONLY; /* Indicate creation of PWE Out-LIF only */
    ...
    /* ***** PWE outlif primary **** */
    mpls_port_egress.egress_failover_id = 0/*vswitch_vpls_hvpls_info.pwe_failover_id_outlif*/ ;
    ...
    mpls_port__egress_tunnel_if_egress_only__set(unit, vswitch_vpls_info_1.tunnel_id, 1, &mpls_port_1);
    ...
    rv = multicast__mpls_port_add(unit, vswitch_vpls_shared_info_1.vpn, modport, mpls_port_egress.mpls_port_id , 1/*is_egress*/);
    if (rv != BCM_E_NONE) {
    ...
    mpls_port_egress.egress_failover_port_id = mpls_port_egress.mpls_port_id;
    mpls_port_egress.egress_failover_id = 0/*vswitch_vpls_hvpls_info.pwe_failover_id_outlif*/ ;
...
vswitch_vpls_hvpls_fec_mpls_port_create(int unit){
    ...
    vswitch_vpls_interface_encap_id_set(vswitch_vpls_hvpls_info.pwe_outlif_protected, &pwe_encap_id);
    ...
    vswitch_vpls_interface_encap_id_set(vswitch_vpls_hvpls_info.pwe_outlif_primary, &pwe_encap_id);
...
vswitch_vpls_hvpls_mpls_port_single_call_create(int unit){
    ...
    for (is_primary=0;is_primary<=1;is_primary++) {
        ...
        mpls_port.egress_failover_id = 0/*vswitch_vpls_hvpls_info.pwe_failover_id_outlif*/ ;
        ...
        if (verbose1) {
            printf("ADDING MPLS PORT (primary)\n");
        }
        ...
        /* Adding mpls port to multicast */
        rv = multicast__mpls_port_add(unit, vswitch_vpls_shared_info_1.vpn, modport, mpls_port.mpls_port_id, 1/*is_egress*/);
        if (rv != BCM_E_NONE) {
            printf("Error, multicast__mpls_port_add\n");
            return rv;
        }
...
vswitch_vpls_hvpls_run(int unit){
    ...
    rv = mpls_port__vswitch_vpls_vpn_create__set_(unit, vswitch_vpls_info_1.vpn_id);
    if (rv != BCM_E_NONE) {
...
int
vswitch_vpls_hvpls_run_with_defaults_dvapi(int unit, int acP, int pweP){
    int rv = BCM_E_NONE;
