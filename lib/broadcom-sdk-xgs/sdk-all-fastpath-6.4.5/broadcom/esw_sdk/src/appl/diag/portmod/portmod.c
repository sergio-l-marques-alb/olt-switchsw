...
portmod_port_diag_status_get(int unit, int port, portmod_port_diag_status_t *diag_status){
    ...
    portmod_port_local_fault_status_get(unit, port, &diag_status->lf);
    if(ret != SOC_E_NONE){     
        cli_out("ERROR: Getting port local fault status failed: %s\n", soc_errmsg(ret));     
        return CMD_FAIL;                                    
    }  
    portmod_port_remote_fault_status_get(unit, port, &diag_status->rf);
    if(ret != SOC_E_NONE){     
...
STATIC cmd_result_t
portmod_port_diag_phy_info_get(int unit, int port, int phyn, int lane, phymod_phy_diagnostics_t* diag_phy){
    int ret, nof_phys;
...
portmod_port_ability_display(portmod_port_ability_t *port_ability){
    ...
    port_ability->speed_half_duplex = 15;
    ...
    port_ability->speed_full_duplex = 0xf4;
    string_len = sizeof(port_ability_speed_string)/sizeof(char *);
...
portmod_phy_diag(int unit, args_t *a){
    ...
    if (!ARG_CNT(a)) {  
        ...
        ret = portmod_port_diag_phy_info_get(unit, port, phyn, lane, &phy_diag);
        if(ret != SOC_E_NONE){      
...
portmod_phy_usage(int unit){
    cli_out("command:\n");
    cli_out("  phy port=<port-id> [phychain=<phyn> lane=<lane number>]\n");
    cli_out("example:\n");
...
portmod_eyescan_diag(int unit, args_t *a){
    ...
    if (!ARG_CNT(a)) {  
        ...
        ret = portmod_port_phy_lane_access_get(unit, port, &access_param, PHYMOD_CONFIG_MAX_CORES_PER_PORT, phy_access, &nof_phys, NULL);
        if(ret != SOC_E_NONE){    
