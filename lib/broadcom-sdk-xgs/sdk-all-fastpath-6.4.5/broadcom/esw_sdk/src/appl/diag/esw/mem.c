...
cmd_esw_mem_write(int unit, args_t *a)
{
    ...
    int         enable_ifp_slice_mode_check = 1;
    int         tmp_mem;
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (tab != NULL && sal_strcasecmp(tab, "nocheck_ifp_slice_mode") == 0) {
        enable_ifp_slice_mode_check = 0;
	tab = ARG_GET(a);
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
    idx = ARG_GET(a);
    ...
    if (SOC_IS_TOMAHAWK(unit)) {
        if (enable_ifp_slice_mode_check &&
            (mem == IFP_TCAMm || mem == IFP_TCAM_WIDEm ||
             mem == IFP_TCAM_PIPE0m || mem == IFP_TCAM_WIDE_PIPE0m ||
             mem == IFP_TCAM_PIPE1m || mem == IFP_TCAM_WIDE_PIPE1m ||
             mem == IFP_TCAM_PIPE2m || mem == IFP_TCAM_WIDE_PIPE2m ||
             mem == IFP_TCAM_PIPE3m || mem == IFP_TCAM_WIDE_PIPE3m)) {
            use_slam = 0; /* simplify */
        }
    }
#endif /* BCM_TOMAHAWK_SUPPORT */
            ...
            if (SOC_IS_TOMAHAWK(unit)) {
                if (enable_ifp_slice_mode_check) {
                    ...
                    if ((mem == IFP_TCAMm || mem == IFP_TCAM_WIDEm) &&
                        ...
                        specific_pipe) {
                        if (acc_type == _SOC_MEM_ADDR_ACC_TYPE_PIPE_0) {
                            tmp_mem = SOC_MEM_UNIQUE_ACC(unit, mem)[0];
                        } else if (acc_type == _SOC_MEM_ADDR_ACC_TYPE_PIPE_1) {
                            tmp_mem = SOC_MEM_UNIQUE_ACC(unit, mem)[1];
                        } else if (acc_type == _SOC_MEM_ADDR_ACC_TYPE_PIPE_2) {
                            tmp_mem = SOC_MEM_UNIQUE_ACC(unit, mem)[2];
                        } else if (acc_type == _SOC_MEM_ADDR_ACC_TYPE_PIPE_3) {
                            tmp_mem = SOC_MEM_UNIQUE_ACC(unit, mem)[3];
                        }
                    }
                    if (soc_th_ifp_index_invalid(unit, tmp_mem, index)) {
                        continue;
                    }
                }
            }
...
#ifdef BCM_TOMAHAWK_SUPPORT
                if (SOC_IS_TOMAHAWK(unit)) {
                    mview = _soc_tomahawk_alpm_bkt_view_get(unit, index);
                } else 
#endif
                {
...
#ifdef BCM_TOMAHAWK_SUPPORT
            if (SOC_IS_TOMAHAWK(unit)) {            
                view = _soc_tomahawk_alpm_bkt_view_get(unit, index);        
            } else 
#endif
            {
