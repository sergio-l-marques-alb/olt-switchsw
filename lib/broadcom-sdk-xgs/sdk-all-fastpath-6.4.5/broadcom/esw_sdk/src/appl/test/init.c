...
STATIC int init_aggressive_alloc_debug_testing(int itertaion_idx)
{
    ...
    static int32 after_deinit_sw_state_nof_allocs = 0;
    int32 after_deinit_init_deinit_sw_state_nof_allocs;
    ...
    if(itertaion_idx == 0) {     
        ...
        after_deinit_sw_state_nof_allocs = shr_sw_state_nof_allocations_get();
    ...
    if(itertaion_idx == 1) {
        ...
        if(after_deinit_init_deinit_dma_usage > after_deinit_dma_usgae) {
            ...
            /* we don't return. We continue to check if there are unfree allocations done be sal_alloc or sw state allocs */
        }
        after_deinit_init_deinit_sw_state_nof_allocs = shr_sw_state_nof_allocations_get();
        if(after_deinit_init_deinit_sw_state_nof_allocs > after_deinit_sw_state_nof_allocs) {
            rv = -1;
            cli_out("Error: There are sw state allocations that haven't been freed.\n");
...
exit:
    ...
    if (rv != BCM_E_NONE) {
        ...
        cli_out("!! note that when running in gdb diff is not printed to screen.\n   use breakpoint on this line in order and manually diff the files immediately after the break!!\n");
...
init_deinit_test(int unit, args_t *a, void *p)
{
    ...
    {   
        ...
        for (j = 0; j < loop; j++) {
            ...
            if((!init_param->no_sync) && (init_param->warmboot)) {  
