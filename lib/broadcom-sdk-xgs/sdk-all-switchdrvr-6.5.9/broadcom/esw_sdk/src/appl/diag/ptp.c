...
#if defined(TEST_G8273_2_SECT_7_4_1)
/* Enable this code to start thread to handle ql_change to test the
* transient response as per the Section 7.4.1 of G8273. 2*/

static volatile sal_thread_t esmc_thread_id = SAL_THREAD_ERROR;
STATIC void esmc_ql_change_thread(void *cookie);
    ...
    for (index = 0; index < dataset.num_foreign_masters; index++) {
        ...
        if (SOC_HAS_PTP_INTERNAL_STACK_SUPPORT(unit)) {
            cli_out("    Module Number: %u\n",
                    BCM_GPORT_MODPORT_MODID_GET(dataset.foreign_master[index].phy_port));
            cli_out("    Physical Port number: %u\n",
                    BCM_GPORT_MODPORT_PORT_GET(dataset.foreign_master[index].phy_port));
        }
    }
...
#define PTP_CLOCK_USAGE \
    ...
    "\t                  PROFile=<0=8265.1, 1=8275.1> OffsetScaledLogVar=<> P2=<>\n"\
...
cmd_ptp_servo_perf_get(int unit, args_t *a)
{
...
#ifdef COMPILER_HAS_LONGLONG
    {
        ...
        uint64 decval_;
        unsigned decfrac;
        if (config.servo == bcmPTPServoTypeSymm)
        {
...
cmd_tdpll_instance(int unit, args_t *a)
{
    ...
    if (SOC_IS_APACHE(unit) || SOC_IS_QAX(unit)) {
        /* Retain end_index */
    } else {
...
#define TDPLL_OUTPUTCLK_USAGE_QAX \
    ...
    "\t                                 | Pml0Pll       | Pml1Pll       |\n" \
    "\t                                 | 1588-Hybrid   | GPIO0 | GPIO1 |\n" \
    "\t                                 | GPIO2 | GPIO3 | GPIO4 | GPIO5 | PmhPll\n" \
    "\t TDPLL OUTputclk Set <index> ENable=<y/n>\n"                    \
    "\t                     FREQuency=<value> TSFREQuency= <value>\n"        \
    "\t                     DerivFREQuency=<value>\n"
#endif
...
cmd_tdpll_outputclk(int unit, args_t *a)
{
    ...
    char *arg_index_str_qax[TDPLL_OUTPUT_CLOCK_NUM_MAX+1] =
        {"BroadSync0", "BroadSync1", "Pml0Pll", "Pml1Pll", "1588-Hybrid",
         "GPIO0", "GPIO1", "GPIO2", "GPIO3", "GPIO4", "GPIO5", "PmhPll", 0};
#endif
...
#elif defined (BCM_QAX_SUPPORT)
	if (clock_index_str && SOC_IS_QAX(unit)) {
		arg_clock_index = 0;
...
cmd_tdpll_esmc(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        if (invalid_value != arg_ql_index) {
            ...
            /* Enable this code to start thread to handle ql_change to test the
             * transient response as per the Section 7.4.1 of G8273. 2*/
            esmc_thread_id = sal_thread_create("bcmESMC", SAL_THREAD_STKSZ,
				     100,
				     esmc_ql_change_thread, NULL);
#endif                     
        }
...
/* Function: esmc_ql_change_thread
 * Description: Sample code to disable the reference clock to test
 *              transient response as per the Section 7.4.1 of G8273.2
 *              This is only sample code and for production quality we need to use
 *              queues and semaphore.
 */
#if defined(TEST_G8273_2_SECT_7_4_1)
STATIC void
esmc_ql_change_thread(void *cookie)
{
    while (1)  
    { 
        sal_sleep(1);
        /* Simple implementation of reading and action on the state change in DB */
        /* Recommended to get Queue for communication between threads */
            ...
            if (network_option == bcm_esmc_network_option_g781_I) {
               cli_out("Network Option  : %d\n", network_option);
                if (ql_change_data.current_ql >= bcm_esmc_g781_I_ql_sec) {
                    bcm_tdpll_input_clock_enable_set(0, 0, ql_change_data.input_clk_index, FALSE);
                    cli_out("Disable the Clock  : %d\n", ql_change_data.input_clk_index);
                } else { 
                    bcm_tdpll_input_clock_enable_set(0, 0, ql_change_data.input_clk_index, TRUE);
