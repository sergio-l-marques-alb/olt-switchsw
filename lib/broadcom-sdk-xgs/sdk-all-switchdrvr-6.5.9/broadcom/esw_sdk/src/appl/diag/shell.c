...
#include <appl/diag/dpp/memories_and_registers.h>
#endif
...
char sh_state_usage[] =
    ...
    "               directory:\n"
    "                          [directory name]\n"
    "Example: state hw optimized directory /projects/NTSW_SW_USRS_8/yourname/\n";
...
{
    char *c;
    char memory_or_register_name[STATE_DUMP_ARRAYS_SIZES];
    char hw_read_command[512];
    int index_mem_reg = 0;
#ifdef BCM_WARM_BOOT_API_TEST
    cmd_result_t result_mkdir = CMD_OK;
    char state_dump_count[2] = {'0','\0'};
    char directory_create_comand[STATE_DUMP_ARRAYS_SIZES];
    char state_directory_name[STATE_DUMP_ARRAYS_SIZES];
    char sw_state_file_name[STATE_DUMP_ARRAYS_SIZES];
    char *dump_directory;
#endif/* BCM_WARM_BOOT_API_TEST */
    COMPILER_REFERENCE(u);
    ...
    if (0 == ARG_CNT(a)) return(CMD_USAGE);
        ...
        if (!sal_strcasecmp(c, "dump")){
            if (0 == ARG_CNT(a)) return(CMD_USAGE);
            c = ARG_GET(a);
            if (!sal_strcasecmp(c, "set_directory")){
                if (0 == ARG_CNT(a)) return(CMD_USAGE);
                dump_directory = ARG_GET(a);
                soc_dcmn_wb_default_dump_directory_set(u, dump_directory);
            } else if (!sal_strcasecmp(c, "optimized")) {
                if (0 == ARG_CNT(a)) return(CMD_USAGE);
                c = ARG_GET(a);
                if (!sal_strcasecmp(c, "directory")){
                    if (0 == ARG_CNT(a)) return(CMD_USAGE);
                    c = ARG_GET(a);
                    do{
                        sal_memset(state_directory_name,0,STATE_DUMP_ARRAYS_SIZES);
                        state_dump_count[0]++;
                        sal_strcpy(state_directory_name, c);
                        sal_strcat(state_directory_name, "dump_");
                        sal_strcat(state_directory_name, state_dump_count);
                        sal_strcat(state_directory_name, "/");
                        sal_memset(directory_create_comand,0,STATE_DUMP_ARRAYS_SIZES);
                        sal_strcat(directory_create_comand, "mkdir ");
                        sal_strcat(directory_create_comand, state_directory_name);
                        result_mkdir = sh_process_command(u, directory_create_comand);
                    }while( result_mkdir != CMD_OK);
                    sal_memset(sw_state_file_name,0,STATE_DUMP_ARRAYS_SIZES);
                    sal_strcat(sw_state_file_name, state_directory_name);
                    sal_strcat(sw_state_file_name, "sw_state_dump_file.txt");
                    sal_strcat(state_directory_name,"hw_state_dump/");
                    sal_memset(directory_create_comand,0,STATE_DUMP_ARRAYS_SIZES);
                    sal_strcat(directory_create_comand, "mkdir ");
                    sal_strcat(directory_create_comand, state_directory_name);
                    if (sh_process_command(u, directory_create_comand) != CMD_OK) return(CMD_USAGE);
                    shr_sw_state_stride_dump(u, sw_state_file_name);
                    for (index_mem_reg = 0;index_mem_reg < MEMORIES_AND_REGISTERS_OPTIMIZED_COUNT;index_mem_reg++){
                        sal_memset(memory_or_register_name,0,STATE_DUMP_ARRAYS_SIZES);
                        sal_strcpy(memory_or_register_name, memories_and_registers_optimized[index_mem_reg]);
                        sal_memset(hw_read_command,0,STATE_DUMP_ARRAYS_SIZES);
                        sal_strcat(hw_read_command, "acc read changed name=^");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, "$ wb file=");
                        sal_strcat(hw_read_command, state_directory_name);
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, ".xml");
                        sh_process_command(u, hw_read_command);
                    }
                } else {
                    return(CMD_USAGE);
                }
            } else if (!sal_strcasecmp(c, "directory")){
                if (0 == ARG_CNT(a)) return(CMD_USAGE);
                c = ARG_GET(a);
                do{
                    sal_memset(state_directory_name,0,STATE_DUMP_ARRAYS_SIZES);
                ...
                if (sh_process_command(u, directory_create_comand) != CMD_OK) return(CMD_USAGE);
                shr_sw_state_stride_dump(u, sw_state_file_name);
                for (index_mem_reg = 0;index_mem_reg < MEMORIES_AND_REGISTERS_COUNT;index_mem_reg++){
                    sal_memset(memory_or_register_name,0,STATE_DUMP_ARRAYS_SIZES);
                    sal_strcpy(memory_or_register_name, memories_and_registers[index_mem_reg]);
                    sal_memset(hw_read_command,0,STATE_DUMP_ARRAYS_SIZES);
                    sal_strcat(hw_read_command, "acc read changed name=^");
                    sal_strcat(hw_read_command, memory_or_register_name);
                    sal_strcat(hw_read_command, "$ wb file=");
...
#endif
         if (!sal_strcasecmp(c, "hw")) {
            if (0 == ARG_CNT(a)) return(CMD_USAGE);
            c = ARG_GET(a);
            if (!sal_strcasecmp(c, "optimized")){
                if (0 == ARG_CNT(a)) return(CMD_USAGE);
                c = ARG_GET(a);
                if (!sal_strcasecmp(c, "directory")){
                    if (0 == ARG_CNT(a)) return(CMD_USAGE);
                    c = ARG_GET(a);
                    for (index_mem_reg = 0;index_mem_reg < MEMORIES_AND_REGISTERS_OPTIMIZED_COUNT;index_mem_reg++){
                        sal_strcpy(memory_or_register_name, "");
                        sal_strcpy(memory_or_register_name, memories_and_registers_optimized[index_mem_reg]);
                        sal_strcpy(hw_read_command, "");
                        sal_strcat(hw_read_command, "acc read changed name=^");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, "$ wb file=");
                        sal_strcat(hw_read_command, c);
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, ".xml");
                        sh_process_command(u, hw_read_command);
                    }
                ...
                if (0 == ARG_CNT(a)) return(CMD_USAGE);
                c = ARG_GET(a);
                if(CMDEV(u).dev.dev_id == BCM88690_DEVICE_ID) {
                    for (index_mem_reg = 0;index_mem_reg < MEMORIES_JERICHO2_COUNT;index_mem_reg++){
                        sal_strcpy(memory_or_register_name, "");
                        sal_strcpy(memory_or_register_name, memories_jericho2[index_mem_reg]);
                        sal_strcpy(hw_read_command, "");
                        sal_strcat(hw_read_command, "acc read changed name=^");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, "$ wb file=");
                        sal_strcat(hw_read_command, c);
                        sal_strcat(hw_read_command, "mem_");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, ".xml");
                        sh_process_command(u,hw_read_command);
                    }
                    for (index_mem_reg = 0;index_mem_reg < REGISTERS_JERICHO2_COUNT;index_mem_reg++){
                        sal_strcpy(memory_or_register_name, "");
                        sal_strcpy(memory_or_register_name, registers_jericho2[index_mem_reg]);
                        sal_strcpy(hw_read_command, "");
                        sal_strcat(hw_read_command, "acc read changed name=^");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, "$ wb file=");
                        sal_strcat(hw_read_command, c);
                        sal_strcat(hw_read_command, "reg_");
                        sal_strcat(hw_read_command, memory_or_register_name);
                        sal_strcat(hw_read_command, ".xml");
                        sh_process_command(u,hw_read_command);
                    }
