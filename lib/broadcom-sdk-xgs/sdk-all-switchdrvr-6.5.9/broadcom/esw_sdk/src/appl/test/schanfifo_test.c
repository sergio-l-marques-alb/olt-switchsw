...
schanfifo_test_cleanup(int unit, void *pa)
{
    ...
    if (b) {
        ...
        rv = soc_schan_fifo_control(unit, CTL_FIFO_RESP_FREE,
                                    b->schanfifoResp.data);
...
schanfifo_test_init(int unit, args_t *a, void **pa)
{
    ...
    *pa = (void *) b;
    ...
    /* schan_fifo_resp_t */
    rv = soc_schan_fifo_control(unit, CTL_FIFO_RESP_ALLOC, &b->schanfifoResp);
    if (BCM_FAILURE(rv))
        goto done;
    ...
    b->schanfifoMsg.resp = &b->schanfifoResp;
...
{
    if (b->testName) {
        int             per_sec, usec_per, kb_per_sec;
        sal_usecs_t     td = 0;
        char            plus = ' ', lessthan = ' ';
        int             i;
        ...
        if (b->async_en == 1) {
            ...
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,
                        ...
                        "driver_time %8d\n"),
                        0, (int) b->async_stime[0],
                        async_cnt, (int) b->async_etime[async_cnt], (int)td));
...
schan_fifo_reg32_get(int unit, soc_reg_t reg, int port, int index, uint32 *data, benchmark_t *b)
{
    ...
    if (/* b->schanfifoMsg.num >= CMIC_SCHAN_FIFO_CMD_SIZE_MAX || */
        b->schanfifoMsg.size >= SCHANFIFO_SOC_CMD_BUFF_DEPTH) {
...
schan_fifo_mem_write(int unit, soc_mem_t mem, int copyno, int index, void *entry_data, benchmark_t *b)
{
    ...
    if (/* (b->schanfifoMsg.num >= CMIC_SCHAN_FIFO_CMD_SIZE_MAX) || */
        (b->schanfifoMsg.size + cmd_dw + entry_dw >= SCHANFIFO_SOC_CMD_BUFF_DEPTH)) {
        rv = schan_fifo_flush(unit, b);
    }
