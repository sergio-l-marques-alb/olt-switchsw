...
shr_access_reg_get(int unit, uint32 base_address, int cmic_block_id, int offset, uint8 opcode, rhlist_t *hit_list)
{
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        ...
        sal_strncpy(hit_entry->block_n, SOC_BLOCK_NAME(unit, block_id), RHNAME_MAX_SIZE);
...
#define CMODEL_HIT_INFO_SIZE 17
    ...
    int flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(cmodel_read_hit_bits(unit, 0, &hit_data_length, &hit_data));

    if((hit_data_length % CMODEL_HIT_INFO_SIZE) != 0)
    {
    ...
    while(hit_data_length >= CMODEL_HIT_INFO_SIZE)
    {
        ...
        hit_data_length -= CMODEL_HIT_INFO_SIZE;
