/*
 * $Id: cmix_schan_fifo.c,v 1.00 Broadcom SDK $
                                 ...
                                 void *data, void *wparam)
{
    ...
    if (rv == SOC_E_NONE) {
        /* start the other channel if already programmed */
        if (_soc_cmicx_schan_fifo_chstate_get(unit, (ch ^ 1)) == CHAN_PROG) {
            _soc_cmicx_schan_fifo_ch_start(unit, (ch ^ 1));
        }
    }
...

{
     ...
     * number of command should be 1 less than actual, this will change
     */
    val = soc_pci_read(unit, CMIC_SCHAN_FIFO_CHx_CTRL(ch));
    soc_reg_field_set(unit, CMIC_COMMON_POOL_SCHAN_FIFO_0_CH0_CTRLr,
                               &val, NUMB_OF_COMMANDSf, (msg->num - 1));
    soc_reg_field_set(unit, CMIC_COMMON_POOL_SCHAN_FIFO_0_CH0_CTRLr,
                      &val, SUMMARY_UPDATE_INTERVALf,
                   (msg->interval < 0)? SCHAN_FIFO_SUMMARY_UPDATE_INTERVAL : \
                        msg->interval);
    ...
    /* start the channel if other channel is free */
    if (_soc_cmicx_schan_fifo_chstate_get(unit, (ch ^ 1)) == CHAN_FREE) {
