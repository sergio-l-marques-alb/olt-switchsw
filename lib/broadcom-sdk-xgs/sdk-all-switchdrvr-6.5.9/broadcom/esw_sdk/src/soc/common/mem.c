...
_soc_mem_view_entry_dump_common(int unit, soc_mem_t mem_view_id, void *buf, char *prefix, int vertical, uint32 bsl_flags)
{
    ...
    for (field_type = SOC_FLOW_DB_FIELD_TYPE_CONTROL; 
        ...
        rv = soc_flow_db_mem_view_field_list_get(unit, mem_view_id, field_type, 100, field_id, &cnt);
        if (SOC_SUCCESS(rv)) {
            for (i = 0; i < cnt; i++) {
#if !defined(SOC_NO_NAMES)
                sal_memset(fval, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
                soc_mem_field_get(unit, mem_view_id, buf, field_id[i], fval);
                ...
                (void) soc_flow_db_mem_view_field_info_get(unit, mem_view_id,
                        field_id[i], &ff_info);
                ...
                if (field_type == SOC_FLOW_DB_FIELD_TYPE_LOGICAL_KEY ||
                    ...
                    (void) soc_flow_db_mem_view_field_name_get(unit, mem_view_id,
...
soc_mem_entry_dump_common(int unit, soc_mem_t mem, void *buf, char *prefix, int vertical, char *field_names, uint32 bsl_flags)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flow)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, flex_mem, DATA_TYPEf)) {
            data_type = soc_mem_field32_get(unit, flex_mem, buf, DATA_TYPEf);
        }
        if (SOC_MEM_FIELD_VALID(unit, flex_mem, KEY_TYPEf)) {
            key_type = soc_mem_field32_get(unit, flex_mem, buf, KEY_TYPEf);
        }

        if (!key_type) {
            rv = soc_flow_db_mem_to_view_id_get (unit, flex_mem, 
                    SOC_FLOW_DB_KEY_TYPE_INVALID, data_type,
                    0, NULL, (uint32 *)&mem_view_id);
        } else {
            rv = soc_flow_db_mem_to_view_id_get (unit, flex_mem, key_type, 
                    data_type, 0, NULL, (uint32 *)&mem_view_id);
        }

        if (SOC_SUCCESS(rv)) {
            if (soc_flow_db_mem_view_is_valid(unit, mem_view_id)) {
...
soc_mem_cache_set(int unit, soc_mem_t mem, int copyno, int enable)
{
...
#if defined(BCM_PETRA_SUPPORT)
    if(((org_mem == IHB_FEC_SUPER_ENTRYm) || (org_mem == IHB_FEC_ENTRYm))  &&
       SOC_IS_QAX(unit) && (soc_mem_base(unit,IHB_FEC_SUPER_ENTRYm) == soc_mem_base(unit,IHB_FEC_ENTRYm))) {
        mem = (org_mem == IHB_FEC_SUPER_ENTRYm)? IHB_FEC_ENTRYm : IHB_FEC_SUPER_ENTRYm;
    }
#endif /* BCM_PETRA_SUPPORT */
    ...
    defined(BCM_GREYHOUND_SUPPORT)
        ...
        if (!(SOC_IS_TD_TT(unit) || SOC_IS_ENDURO(unit) || SOC_IS_KATANAX(unit) || SOC_IS_DPP(unit) ||
             (SOC_IS_TRIUMPH3(unit)) || SOC_IS_TRIUMPH(unit) || SOC_IS_GREYHOUND(unit) ||
...
_soc_mem_defip_index_is_invalid(int unit, soc_mem_t mem, int index)
{
    ...
    if (soc_feature(unit, soc_feature_l3_defip_map)) {
        if (mem == L3_DEFIPm ||
            ...
            mem == L3_DEFIP_HIT_ONLY_Ym ||
            mem == L3_DEFIP_HIT_ONLYm) {
            index_max = soc_mem_index_count(unit, L3_DEFIPm) +
                        soc_mem_index_count(unit, L3_DEFIP_PAIR_128m) * 2 - 1;

            if (soc_feature(unit, soc_feature_l3_1k_defip_table)) {
                int tcam_size = SOC_L3_DEFIP_TCAM_DEPTH_GET(unit);
                int num_tcams =  soc_mem_index_count(unit, L3_DEFIP_PAIR_128m) ? 1 : 0;
            ...
            mem == L3_DEFIP_PAIR_128_HIT_ONLY_Ym ||
            mem == L3_DEFIP_PAIR_128_HIT_ONLYm) {
            index_max = soc_mem_index_count(unit, L3_DEFIPm) / 2 +
                                    ...
                                    int vchan)
{
    ...
    if (soc_mem_slamable(unit, mem, copyno)) {
        ...
        if (NULL != meminfo->snoop_cb &&
            (SOC_MEM_SNOOP_WRITE & meminfo->snoop_flags)) {
            meminfo->snoop_cb(unit, mem, SOC_MEM_SNOOP_WRITE, copyno, effective_min,
                              effective_max, buffer, meminfo->snoop_user_data);
        }
        if (cache_buffer) {
            sal_free(cache_buffer);
        }
                              ...
                              then the buffer must be allocated using soc_cm_salloc(). */
{
    ...
    if (soc_dma_debug_property_get(unit)) {
        ...
        if (rv == SOC_E_NONE) {
            ...
            /* Loop over the block instances ignoring broadcast write block, and update cache */
            SOC_MEM_BLOCK_ITER(unit, mem, blk) {
                if (copyno != COPYNO_ALL && copyno != blk) {
                    continue;
                }
...
typedef struct _soc_breadth_bucket2_s {
    ...
    union {
        ...
        exact_match_double_entry_t em_double[4];
        exact_match_quad_entry_t em_quad[4];
...
#define MEM_IS_EXACT_MATCH_DOUBLE(mem) (EXACT_MATCH_DOUBLEm == mem)
#define MEM_IS_EXACT_MATCH_QUAD(mem) (EXACT_MATCH_QUADm == mem)
    ...
    do {
        ...
        for (ent = 0; ent < NUM_ENT; ent += ent_incr) {
            ...
            if (MEM_IS_VLAN_XLATE(mem)) {
                ...
                ent_incr  = cb->width[ent];
            } else if (MEM_IS_EXACT_MATCH_QUAD(mem)) {
                se        = &cb->e.em_quad[ent];
                ent_incr  = cb->width[ent];
            } else if (MEM_IS_EXACT_MATCH_DOUBLE(mem)) {
                se        = &cb->e.em_double[ent];
                ent_incr  = cb->width[ent];
            ...
            for (bix = 0; bix < num_banks; bix++) {
                ...
                if (MEM_IS_EXACT_MATCH_4(mem)) {
                    ...
                    soc_mem_field_get(unit, base_mem, bkt_entry[3], VALID_0f, &v[3]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[2], VALID_0f, &v[2]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[1], VALID_0f, &v[1]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[0], VALID_0f, &v[0]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[3], KEY_TYPE_0f, &k[3]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[2], KEY_TYPE_0f, &k[2]);
                    soc_mem_field_get(unit, base_mem, bkt_entry[1], KEY_TYPE_0f, &k[1]);
...
#endif /* BCM_TOMAHAWK_SUPPORT */                 
#ifdef BCM_TRIDENT3_SUPPORT
                    if (MEM_IS_EXACT_MATCH_QUAD(mem)) {
                        me = &sb->e.em_quad[sent];                        
                    } else if (MEM_IS_EXACT_MATCH_DOUBLE(mem)) {
                        me = &sb->e.em_double[sent];
...
#ifdef BCM_TRIDENT3_SUPPORT
                        if (MEM_IS_EXACT_MATCH_QUAD(mem)) {
                            me = &pbi->e.em_quad[cbi->src_ent];
                        } else if (MEM_IS_EXACT_MATCH_DOUBLE(mem)) {
                            me = &pbi->e.em_double[cbi->src_ent];
                        } else if (MEM_IS_VLAN_XLATE(mem)) {
        ...
        for (bkt_idx = 0; bkt_idx < num_cl_bkts; bkt_idx++) {
...
#ifdef BCM_TRIDENT3_SUPPORT
                if (MEM_IS_EXACT_MATCH_QUAD(mem)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.em_quad[cb->src_ent];
                } else if (MEM_IS_EXACT_MATCH_DOUBLE(mem)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.em_double[cb->src_ent];
                } else if (MEM_IS_VLAN_XLATE(mem)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.vlan[cb->src_ent];
                } else if (MEM_IS_EGR_VLAN_XLATE(mem)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.egr_vlan[cb->src_ent];
                } else if (MEM_IS_MPLS(mem)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.mpls[cb->src_ent];
                } else if (SOC_IS_TRIDENT3X(unit)) {
                    se = &buckets[level - 1][cb->src_bkt_idx].e.l3_single[cb->src_ent];
                } else 
#endif /* BCM_TRIDENT3_SUPPORT */
                {
            ...
            for (ent = 0; ent < NUM_ENT; ent += ent_incr) {
                ...
                if (MEM_IS_EXACT_MATCH_QUAD(mem)) {
                    ...
                    me = &cb->e.em_double[ent];
                } else if (MEM_IS_VLAN_XLATE(mem)) {
                    me = &cb->e.vlan[ent];
                } else if (MEM_IS_EGR_VLAN_XLATE(mem)) {
                    me = &cb->e.egr_vlan[ent];
                } else if (MEM_IS_MPLS(mem)) {
                    me = &cb->e.mpls[ent];
                } else if (SOC_IS_TRIDENT3X(unit)) {
                        me = &cb->e.l3_single[ent];
                } else 
#endif /* BCM_TRIDENT3_SUPPORT */
                {
                    me = &cb->e.l3[ent];
                ...
                if (SOC_IS_TRIDENT3X(unit)) {
                    rv = soc_td3_shared_hash_mem_bucket_read
                                (unit, ent, k[ent], &ent_incr, base_mem,
                                 &src_mem, bkt_entry[ent / SRAM_ENTRIES_PER_BUCKET], me);
                    if (SOC_FAILURE(rv)) {
                        goto cleanup;
                    }
    ...
    for (bix = 0; bix < bi.num_banks; bix++) {
        ...
        if (mem != L2Xm) {
            if (mem == L3_ENTRY_ONLYm || 
                ...
                v[3] = soc_mem_field32_get(unit, em_mem, &bkt_entry[3], VALID_0f);
                v[2] = soc_mem_field32_get(unit, em_mem, &bkt_entry[2], VALID_0f);
                v[1] = soc_mem_field32_get(unit, em_mem, &bkt_entry[1], VALID_0f);
                v[0] = soc_mem_field32_get(unit, em_mem, &bkt_entry[0], VALID_0f);
                k[3] = soc_mem_field32_get(unit, em_mem, &bkt_entry[3], KEY_TYPE_0f);
                k[2] = soc_mem_field32_get(unit, em_mem, &bkt_entry[2], KEY_TYPE_0f);
                k[1] = soc_mem_field32_get(unit, em_mem, &bkt_entry[1], KEY_TYPE_0f);
                k[0] = soc_mem_field32_get(unit, em_mem, &bkt_entry[0], KEY_TYPE_0f);
                       ...
                       int host_entries, void *host_buf)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx) &&
        ...
        return soc_fifodma_start(unit, chan, mem, copyno,
                                            host_entries, host_buf);
    }
