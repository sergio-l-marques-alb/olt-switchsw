...
_cmicm_sbusdma_reg_array_read(int unit, soc_sbusdma_reg_param_t  *param)
{
    ...
    LOG_VERBOSE(BSL_LS_SOC_DMA,
             (BSL_META_U(unit,
              ...
              param->index_end, param->flags, param->buffer));
   ...
   if (param->copyno == COPYNO_ALL) {
    ...
    if (SOC_IS_CALADAN3(unit)) {
        soc_sbx_caladan3_sbusdma_cmc_ch_map(unit, param->mem,
    ...
    if (0 != (param->flags & _SOC_SER_FLAG_MULTI_PIPE)) {
        if ((param->flags & _SOC_SER_FLAG_ACC_TYPE_MASK) ||
            (soc_feature(unit, soc_feature_unique_acc_type_access))) {

            /* Override ACC_TYPE in opcode */
            acc_type = param->flags & _SOC_SER_FLAG_ACC_TYPE_MASK;
        }
    }
    ...
    for (array_index = param->array_id_start;
        ...
        if (!param->flags || (param->flags & SOC_MEM_WRITE_SET_ONLY)) {
            ...
            if (SOC_IS_CALADAN3(unit)) {
                ...
                soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &rval,
                                  INCR_SHIFTf, 0);
                /* Caladan3 chip uses a fast polling technique
            ...
            if (soc->tslamDmaIntrEnb) {
                ...
                do {
                    ...
                    if (soc_reg_field_get(unit, CMIC_CMC0_SBUSDMA_CH0_STATUSr,
                        ...
                        LOG_VERBOSE(BSL_LS_SOC_DMA,
                                    (BSL_META_U(unit,
                                     "HW dma write poll time: %d usecs,"
                                     "[%d nsecs per op]\n"),
                                     diff_time, diff_time*1000/count));
#endif
                        rv = SOC_E_NONE;
        ...
        cache = SOC_MEM_STATE(unit, param->mem).cache[param->copyno];
        vmap = SOC_MEM_STATE(unit, param->mem).vmap[param->copyno];
...
_cmicm_sbusdma_reg_clear_specific(int unit, soc_sbusdma_reg_param_t  *param)
{
    ...
    if (SOC_MEM_IS_ARRAY(unit, param->mem)) {
        ...
        if (maip) {
            if (param->array_id_end >= maip->numels) {
                param->array_id_end = maip->numels - 1;
            }
        } else {
            param->array_id_end = 0;
        }
        if (param->array_id_start > param->array_id_end) {
