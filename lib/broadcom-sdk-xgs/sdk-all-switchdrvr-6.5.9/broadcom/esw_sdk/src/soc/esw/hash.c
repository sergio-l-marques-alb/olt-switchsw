/*
 * $Id: hash.c,v 1.115 Broadcom SDK $
...
soc_xgs3_l3_ecmp_hash(int unit, soc_xgs3_ecmp_hash_t *data)
{
    ...
    if (SOC_IS_APOLLO(unit) || SOC_IS_TRIUMPH2(unit)) {
        crc_val_shift = 24;
        mask = 0xFF;
    } else if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TD_TT(unit)) {
       crc_val_shift = 22;
       mask = 0x3FF;
    }
                     ...
                     int *bucket_array, int *index_array, uint8 *hash_key)
{
    ...
    for (bank = 0; bank < *num_banks; bank++) {
        ...
        if (SOC_IS_APACHE(unit)) {
            SOC_IF_ERROR_RETURN(
                soc_apache_hash_bank_info_get(unit, L2Xm, bank, NULL,
                                              &entries_per_row, NULL,
                                              &bank_base, &bucket_offset));
            bucket = soc_ap_l2x_bank_entry_hash(unit, bank, base_entry);
        } else
#endif
#ifdef BCM_TRIDENT2_SUPPORT
        if (!SOC_IS_TOMAHAWKX(unit) && SOC_IS_TD2_TT2(unit)) {
            SOC_IF_ERROR_RETURN(
                soc_trident2_hash_bank_info_get(unit, L2Xm, bank, NULL,
                                                &entries_per_row, NULL,
                                                &bank_base, &bucket_offset));
            bucket = soc_td2_l2x_bank_entry_hash(unit, bank, base_entry);
        } else
#endif
#ifdef BCM_TOMAHAWK_SUPPORT
        if (SOC_IS_TOMAHAWKX(unit)) {
            SOC_IF_ERROR_RETURN(
                soc_tomahawk_hash_bank_info_get(unit, L2Xm, bank, NULL,
                                                &entries_per_row, NULL,
                                                &bank_base, &bucket_offset));
            bucket = soc_th_l2x_bank_entry_hash(unit, bank, base_entry);
        }
#endif
