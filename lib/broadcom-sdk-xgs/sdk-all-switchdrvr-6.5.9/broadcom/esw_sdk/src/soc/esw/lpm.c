...
{
    if (SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, defip_mode0_field)) {        
        /* IPV6 entry */                                                
...
_soc_fb_lpm_prefix_length_get(int u, void *entry, int *pfx_len) 
{
    ...
    if (soc_mem_field_valid(u, L3_DEFIPm, KEY_TYPE0f)) {
        key_type = soc_mem_field32_get(u, L3_DEFIPm, entry, KEY_TYPE0f);
    } else {
        key_type = 0;
    }
...
soc_fb_lpm_insert_index(int u, void *entry_data, int index)
{
    ...
    if (ipv6) {
        if (!(ipv6 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, defip_mode1_field))) {
            SOC_LPM_UNLOCK(u);
            return (SOC_E_PARAM);
        }
    }
                                 ...
                                 int *pfx_len)
{
    ...
    mode = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry, defip_mode0_field);
