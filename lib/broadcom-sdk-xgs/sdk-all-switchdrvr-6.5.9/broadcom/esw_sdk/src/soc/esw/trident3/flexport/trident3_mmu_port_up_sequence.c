...
*
* $File:  trident3_mmu_port_up_sequence.c
*/
...
/*! @fn int soc_trident3_mmu_port_up_sequence(int unit,
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state_t)
{
    ...
    soc_trident3_mmu_get_bst_mode(unit, &bst_mode);
    soc_trident3_mmu_get_pktstat_mode(unit, &pktstat_mode);
    
    COMPILER_64_SET(temp64, 0, TD3_MMU_FLUSH_OFF);
	...
	if (pipe_flexed == 1) {
	    soc_trident3_mmu_vbs_port_flush(unit, port_schedule_state_t, pipe, temp64);
    ...
    for (port=0; port<port_schedule_state_t->nport; port++) {
	    ...
	    oversub_ratio =
            ...
            soc_trident3_mmu_set_mmu_to_phy_port_mapping(
            ...
            soc_trident3_mmu_reinit_ct_setting(
                unit, &port_schedule_state_t->resource[port],
            ...
            soc_trident3_mmu_clear_mtro_bucket_mems(
                unit, &port_schedule_state_t->resource[port]);
            ...
            soc_trident3_mmu_wred_clr(unit,
                                       &port_schedule_state_t->resource[port]);
            soc_trident3_mmu_thdi_setup(unit,
                                         &port_schedule_state_t->resource[port],
                                         lossy);
            ...
            soc_trident3_mmu_ctr_clr(unit,
                                      &port_schedule_state_t->resource[port]);
	    if (bst_mode == TD3_MMU_BST_WMARK_MODE) {
	        /* Clear THDI BST in watermark mode..*/
                soc_trident3_mmu_thdi_bst_clr(
                    unit, &port_schedule_state_t->resource[port]);
                /* Clear THDU BST counters in watermark mode*/
                soc_trident3_mmu_thdu_bst_clr(
                    unit, &port_schedule_state_t->resource[port]);
                /* Clear THDM BST counters in watermark mode*/
                soc_trident3_mmu_thdm_bst_clr(
                    unit, &port_schedule_state_t->resource[port]);
	    }
	    if (pktstat_mode == TD3_PKTSTATS_ON) {
                /* Clear PktStat counters in THDU for Queues*/
                soc_trident3_mmu_thdu_pktstat_clr(
                    unit, &port_schedule_state_t->resource[port]); 
            }
            ...
            if (port_schedule_state_t->resource[port].oversub == 1) {
                ...
                soc_trident3_mmu_reinit_dip_fsaf_setting(
                    unit, &port_schedule_state_t->resource[port]);
            }
        } else {
	     soc_trident3_mmu_set_mmu_to_phy_port_mapping(
...
int
soc_trident3_flex_mmu_port_up(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state_t)
{
    int port;
    for (port=0; port<port_schedule_state_t->nport; port++) {
        if (port_schedule_state_t->resource[port].physical_port != -1) {
            ...
            soc_trident3_mmu_clear_prev_ep_credits(
                unit, &port_schedule_state_t->resource[port]);
        }
    }
    return SOC_E_NONE;
