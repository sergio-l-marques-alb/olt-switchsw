...
STATIC soc_flow_db_flow_rec_t legacy_flow_map[] =
{
    {"CLASSIC_VXLAN", SOC_FLOW_DB_FLOW_ID_LEGCAY_VXLAN},
...
int _soc_flow_db_load(int unit, uint8* buf, int buf_words)
{
    ...
    SOC_IF_ERROR_RETURN(
          _soc_flow_db_memcpy_letohl((uint32 *)flow_db->flow_map,
                           (uint32 *)buf, buf_words));
...
int soc_flow_db_status_get(int unit)
{
   ...
   if (!(cc->flags & SOC_CANCUN_FLAG_CFH_LOADED))  {
       LOG_ERROR(BSL_LS_BCM_FLOW,
       (BSL_META_U(0,
         "\n ERROR: Cancun Flex DB file CFH is not loaded")));
       return SOC_E_INIT;
   }
   ...
   if (flow_map == NULL) {
       LOG_ERROR(BSL_LS_BCM_FLOW,
       (BSL_META_U(0,
         "\n ERROR: Cancun Flex DB - flow map not initialized")));
       return SOC_E_INIT;
   }
...
int _soc_flow_db_flow_handle_get(int unit,
     ...
     ******************************/
    for (i = 0, flow_entry = legacy_flow_map;
        i < SOC_FLOW_DB_MAX_LEGACY_FLOWS;
        i++, flow_entry++) {
        if (sal_strncmp(name, flow_entry->name,
                 sal_strlen(flow_entry->name))== 0) {
                           ...
                           uint32 *option_id)
{
    ...
    if (entry_num !=0) {
        for (i = 0; i < entry_num; i++) {
            if ((sal_strncmp(name, flow_op_entry->name,
                 sal_strlen(flow_op_entry->name))== 0) &&
                                    ...
                                    uint32  *field_id)
{
    ...
    if (entry_num != 0) {
        for (i = 0; i < entry_num; i++) {
            if ((sal_strcmp(name, lg_fl_entry->name)== 0) &&
                             ...
                             uint32                    *view_id)
{
    ...
    *view_id = 0;
    ...
    for (vid = SOC_FLOW_DB_VIEW_IDX_START;
        ...
        field_entry = (soc_flow_db_field_rec_t *)pf;
        if (!key_type_match)  {
            for (j = 0; j < SOC_FLOW_DB_MAX_KEY_TYPE; j++) {
        ...
        if (!data_type_match) {
            for (j = 0; j < SOC_FLOW_DB_MAX_DATA_TYPE; j++) {
