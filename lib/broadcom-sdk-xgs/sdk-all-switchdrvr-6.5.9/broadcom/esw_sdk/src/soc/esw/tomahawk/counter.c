                               ...
                               int *base_index, int *num_entries)
{
    ...
    case SOC_COUNTER_NON_DMA_PORT_OBM_FC_EVENTS:    
        ...
        *num_entries = 1;
        ...
        if (SOC_IS_TOMAHAWK2(unit)) {
            SOC_IF_ERROR_RETURN
                (soc_tomahawk2_port_obm_info_get(unit, port, &obm_id, &lane));
            if (obm_id >= _TH2_PBLKS_PER_PIPE) {
                return SOC_E_PARAM;
            }
        } else {
            SOC_IF_ERROR_RETURN
    ...
    for (i = 0; i < count; i++) {
        if (mmu_mem == FALSE) {
            ...
            if (non_dma_parent->mem != MMU_CTR_COLOR_DROP_MEMm || 
                (non_dma_parent->mem == MMU_CTR_COLOR_DROP_MEMm && SOC_IS_TOMAHAWK2(unit))) {
                for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                    non_dma_extra->dma_mem[pipe] =
                                  ...
                                  int *non_dma_entries)
{
    ...
    *non_dma_entries = 0;
    ...
    if (SOC_IS_TOMAHAWK2(unit)) {
        soc_ctr_ctrl->port_obm_info_get = soc_tomahawk2_port_obm_info_get;
    } else {
