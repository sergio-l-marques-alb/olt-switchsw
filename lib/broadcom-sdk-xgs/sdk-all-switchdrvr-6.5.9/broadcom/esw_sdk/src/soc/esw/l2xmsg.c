...
#define L2X_ENTRY_EQL(unit, e1, e2, sz) \
    (!_soc_mem_cmp_l2x_sync(unit, (void *) e1, (void *) e2, sz))
...
soc_l2x_sync_delete(int unit, uint32 *del_entry, int index, uint32 flags)
{
    ...
    if (L2X_ENTRY_EQL(unit, del_entry, tab_p, l2x_data[unit].entry_bytes)) {
#if defined(BCM_TSN_SUPPORT)
        /* Decrease TSN/SR flowset reference count for manual L2 deletion */
        if (soc_feature(unit, soc_feature_tsn)) {
            soc_l2x_callback(unit,
                             SOC_L2X_ENTRY_TSN_SR_FLOWSET_REF_CNT,
                             (l2x_entry_t *)tab_p, NULL);
        }
                     ...
                     SHR_BITDCL *p_chunk_cb_map)
{
    ...
    for (io = 0; io < SOC_L2X_BUCKET_SIZE; io++,
        ...
        if (L2X_ENTRY_EQL(unit, old_p, new_p, l2x_data[unit].entry_bytes)) {
            if (delete_marked) {
                /* Entry got learnt again after delete */
                if (callback_suppress) {
                    /* Indicate re-learn */
    ...
    for (in = 0; in < SOC_L2X_BUCKET_SIZE; in++,
        ...
        if (L2X_ENTRY_EQL(unit, new_p, old_p, l2x_data[unit].entry_bytes)) {
            continue;
        }
