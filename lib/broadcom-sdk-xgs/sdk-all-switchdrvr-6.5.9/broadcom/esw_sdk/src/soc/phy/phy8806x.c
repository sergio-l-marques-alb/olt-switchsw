...
_8806x_xmod_command(void *data)
{
    ...
    soc_phymod_core_t *soc_core = PHYMOD_ACC_USER_ACC(bdp->pa);
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    soc_mac_probe(unit, port, &(speed_config->macd));
...
#endif
        if (!port_is_higig != !port_was_higig) {
    ...
    if (speed_config->port_mode != ETH_REPEATER) {
        ...
        *link = (link_st & VS_MISC_STATUS_LL_STATUS_BIT_MASK) >> 3;
        ...
        if (speed_config->repeater_an_phase == REPEATER_AN_PHASE_NONE) {
            ...
            SOC_IF_ERROR_RETURN(PHY_LINK_GET(int_pc->pd, unit, port, &int_link));

            /* Read the MT2 Link status */
            rv = READ_LINK_STATUS_REG(unit, pc, &link_st);
            *link = (link_st & VS_MISC_STATUS_LL_STATUS_BIT_MASK) >> 3;
             ...
             * (a) If the system and line side of MT2 device are UP, but the switch internal PHY PCS link is DOWN.
             * (b) If the system and line side of MT2 device are UP, switch internal PHY PCS link is UP but 
             *     the MAC is seeing LOCAL or REMOTE fault.
             */ 
            if (((*link) && !(int_link)) || 
                ((*link) && (int_link) && (local_fault || remote_fault))) {
                trials++;
            ...
            if (speed_config->repeater_an_phase == REPEATER_AN_PHASE_0) {
                if (an_control.flags & REPEATER_AN_PHASE0_LOCKED) {
                    if (REPEATER_AN_PHASE0_CL74 & an_control.flags)
                        PHY_CONTROL_SET(int_pc->pd, unit, port, SOC_PHY_CONTROL_FORWARD_ERROR_CORRECTION, 1);
                    else
