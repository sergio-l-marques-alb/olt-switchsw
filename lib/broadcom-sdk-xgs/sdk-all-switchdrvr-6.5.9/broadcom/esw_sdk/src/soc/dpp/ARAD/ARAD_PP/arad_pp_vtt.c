...
/*66*/    {ARAD_PP_DBAL_VT_PROG_SECTION_OAM_ONE_TAG                                       , arad_pp_dbal_vt_program_section_oam_one_tag_property_set               , {-1}    , 0               },
/*67*/    {ARAD_PP_DBAL_VT_PROG_SECTION_OAM_TWO_TAGS                                      , arad_pp_dbal_vt_program_section_oam_two_tags_property_set               , {-1}    , 0               },
/*68*/    {ARAD_PP_DBAL_VT_PROG_INDEXED_VDxINITIALVID_MPLS4_L1L2                                  , arad_pp_dbal_vt_program_indexed_vdxinitialvid_mpls4_l1l2_property_set                   , {-1}    , 0    },
/*69*/    {ARAD_PP_DBAL_VT_PROG_INDEXED_VDxOUTERVID_MPLS4_L1L2                                  , arad_pp_dbal_vt_program_indexed_vdxoutervid_mpls4_l1l2_property_set                   , {-1}    , 0    },
...
void arad_pp_dbal_tt_program_index_as_order_l2_l3_property_set(int unit, ARAD_PP_DBAL_VTT_PROGRAM_PROPERTY  *program_property, uint32 unused)
{
    ...
    program_property->vtt_table_index[1]  = SW_DB_DBAL2VTT(SOC_DPP_DBAL_SW_TABLE_ID_MPLS_L3_L13IDX_SEM_B);
...
void arad_pp_dbal_tt_program_index_as_order_l2_l2_property_set(int unit, ARAD_PP_DBAL_VTT_PROGRAM_PROPERTY  *program_property, uint32 unused)
{
    ...
    program_property->vtt_table_index[1]  = SW_DB_DBAL2VTT(SOC_DPP_DBAL_SW_TABLE_ID_MPLS_L2_L13IDX_SEM_B);
...
void arad_pp_dbal_tt_program_index_as_order_l1_l1_property_set(int unit, ARAD_PP_DBAL_VTT_PROGRAM_PROPERTY  *program_property, uint32 unused)
{
    ...
    program_property->vtt_table_index[1]  = SW_DB_DBAL2VTT(SOC_DPP_DBAL_SW_TABLE_ID_MPLS_L1_L13IDX_SEM_B);
...
int32 arad_pp_dbal_tt_cam_mpls_index_as_order_set(int unit, ARAD_PP_IHP_VTT2ND_KEY_PROGRAM_SELECTION_CAM_TBL_DATA   *prog_selection, int next_line, uint32 function)
{
    ...
    switch(function)
    {
        case 1:
            prog_selection->packet_format_code             = ARAD_PARSER_PFC_MPLS3_ETH; /* At least 3 labels */
            prog_selection->packet_format_code_mask        = ARAD_PARSER_PFC_MATCH_ANY_ENCAP; /* ignore only 3:4 bits as 0:2 tell us about MPLS3; and bit 5 that it is ethernet packet */
            prog_selection->isb_lookup_found              = (0x1); /*VT Sem B match*/
            prog_selection->isb_lookup_found_mask    = (0x0);
            break;
        case 2:
        ...
        case 3:
            ...
            prog_selection->isb_lookup_found              = (0x0);/*VT Sem B miss*/
            prog_selection->isb_lookup_found_mask    = (0x0);
            break;
        case 4:
...
int32 arad_pp_dbal_tt_cam_ipv4_for_explicit_null_set(int unit, ARAD_PP_IHP_VTT2ND_KEY_PROGRAM_SELECTION_CAM_TBL_DATA   *prog_selection,int next_line, uint32 unused)
{
    ...
    if (next_line == 0)
    {
            prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV4_MPLS1_ETH;  /*(IHP_PFC_PART_IPV4 (3b???,(0x2))) */
            prog_selection->packet_format_code_mask = ARAD_PARSER_PFC_MATCH_MPLS_ENCAP_IPVx;
		    prog_selection->packet_format_qualifier_2       = (0x0); 
		    prog_selection->packet_format_qualifier_2_mask  = (0x7FB);
				return 1;    	  
    }
    ...
    if (next_line == 1)
    {
            prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV4_MPLS2_ETH;  /*(IHP_PFC_PART_IPV4 (3b???,(0x2))) */
            prog_selection->packet_format_code_mask = ARAD_PARSER_PFC_MATCH_MPLS_ENCAP_IPVx;
		    prog_selection->packet_format_qualifier_2       = (0x0); 
    ...
    if (next_line == 2)
    {
            prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV4_MPLS3_ETH;  /*(IHP_PFC_PART_IPV4 (3b???,(0x2))) */
            prog_selection->packet_format_code_mask = ARAD_PARSER_PFC_MATCH_MPLS_ENCAP_IPVx;
		    prog_selection->packet_format_qualifier_2       = (0x0); 
...
int32 arad_pp_dbal_tt_cam_ip_disable_on_lif_set(int unit, ARAD_PP_IHP_VTT2ND_KEY_PROGRAM_SELECTION_CAM_TBL_DATA   *prog_selection,int next_line, uint32 type)
{
    ...
    if (type == 1) {
        prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV4_ETH;
        prog_selection->packet_format_code_mask = ARAD_PARSER_PFC_MATCH_ANY_ENCAP_ANY_TYPE_ANY_IP; /* I think that may ARAD_PARSER_PFC_MATCH_ANY_ENCAP is more appropriate */
        prog_selection->vt_in_lif_profile_mask = 0xF&(~v4_mask);
        prog_selection->vt_in_lif_profile          = v4_mask;

    } else if(type==2){
        prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV4_ETH;
        prog_selection->packet_format_code_mask = ARAD_PARSER_PFC_MATCH_ANY_ENCAP; /* I think that may ARAD_PARSER_PFC_MATCH_ANY_ENCAP is more appropriate */
        prog_selection->vt_in_lif_profile_mask = 0xF&(~v4_mask);
        prog_selection->vt_in_lif_profile          = v4_mask;
    } else if (type==3) {
        prog_selection->packet_format_code      = ARAD_PARSER_PFC_IPV6_ETH;
