...
#define KBP_OP_CORE_CLK_898_4_MHZ       898400000
#define KBP_OP_CORE_CLK_833_3_MHZ       833300000
#define KBP_OP_CORE_CLK_718_8_MHZ       718800000
#define KBP_OP_CORE_CLK_600_6_MHZ       600600000
#define KBP_OP_CORE_CLK_400_4_MHZ       400400000
#define KBP_OP_CORE_CLK_302_7_MHZ       302700000
#define KBP_OP_CORE_CLK_249_MHZ         249000000
#define KBP_OP_CORE_CLK_900_MHZ         900000000
#define KBP_OP_CORE_CLK_720_MHZ         720000000
#define KBP_OP_CORE_CLK_600_MHZ         600000000
#define KBP_OP_CORE_CLK_500_MHZ         500000000
#define KBP_OP_CORE_CLK_400_MHZ         400000000
#define KBP_OP_CORE_CLK_300_MHZ         300000000
#define KBP_OP_CORE_CLK_250_MHZ         250000000

#define KBP_ROP_PACKET_SIZE_MAX         256
#define KBP_ROP_RECORDS_IN_PACKET_MAX   50
STATIC void arad_kbp_op_core_clk_get(uint16_t cpsel, uint32_t *op_core_clk);
STATIC uint32 arad_kbp_adjust_egw_config(int unit, uint32 kbp_mdio_id,
                                        int ilkn_num_lanes, int ilkn_rate);
    ...
    kbp_reset_f kbp_reset)
{
    ...
    if (elk->tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) {
        ...
        rc = arad_kbp_adjust_egw_config(unit, kbp_mdio_id, ilkn_num_lanes, ilkn_rate);
        SOC_SAND_CHECK_FUNC_RESULT(rc, 10, exit);
    } else {
   ...
   LOG_VERBOSE(BSL_LS_SOC_TCAM,
    ...
    if(ARAD_KBP_TO_SOC_RESULT(rc) != SOC_SAND_OK){
        if ((elk->kbp_connect_mode == ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL) && (elk->is_master)) {
            rc = KBP_OK;
        } else {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                      (BSL_META_U(unit,
                                        ...
                                        int ilkn_num_lanes, int ilkn_rate)
{
    ...
    handle.device = unit;
    handle.kbp_mdio_id = kbp_mdio_id;
    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4c, &value);
    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4d, &value);
    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4e, &value);
    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4f, &value);

    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4e, &value);
    /* Bit[4-3] of cpsel is bit[13-12] of read value of 0x4e.
     * Bit[2-1] of cpsel is bit[15-14] of read value of 0x4e.
     * Bit[0] of cpsel is bit[0] of read value of 0x4f.
     */
    cpsel = ((value >> 12) & 3) << 3;
    cpsel |= ((value >> 14) & 3) << 1;
    arad_kbp_callback_mdio_read(&handle, 0, 0, 0x4f, &value);
    ...
    arad_kbp_op_core_clk_get(cpsel, &op_core_clk_ticks);
    core_clk_ticks = arad_chip_ticks_per_sec_get(unit);
    op_core_clk_mhz = op_core_clk_ticks / 1000000;
    /*
     * Shaper configuration
     */
    ilkn_total_mbits_rate = ilkn_num_lanes * ilkn_rate;
    ...
    ilkn_rx_mbits_rate = ilkn_num_lanes * ilkn_rate * 64 / 67;
    ilkn_rx_packet_rate = ilkn_rx_mbits_rate / 64 / ((KBP_ROP_PACKET_SIZE_MAX / 8) + 1);
    max_record_rate_over_ilkn = ilkn_rx_packet_rate * KBP_ROP_RECORDS_IN_PACKET_MAX * 1000000;
    op_exp_spr_dly_mega = max_record_rate_over_ilkn / op_core_clk_mhz;

    if (op_exp_spr_dly_mega > spr_dly_mega) {
        spr_dly_mega = op_exp_spr_dly_mega;
    }
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_kbp_adjust_egw_config()", 0, 0);
}
...
STATIC void arad_kbp_op_core_clk_get(uint16_t cpsel, uint32_t *op_core_clk)
{
    ...
    {
        case 0x10:
            *op_core_clk = KBP_OP_CORE_CLK_898_4_MHZ;
            break;

        case 0x11:
            *op_core_clk = KBP_OP_CORE_CLK_833_3_MHZ;
            break;

        case 0x12:
            *op_core_clk = KBP_OP_CORE_CLK_718_8_MHZ;
            break;

        case 0x13:
            *op_core_clk = KBP_OP_CORE_CLK_600_6_MHZ;
            break;

        case 0x14:
            *op_core_clk = KBP_OP_CORE_CLK_500_MHZ;
            break;

        case 0x15:
            *op_core_clk = KBP_OP_CORE_CLK_400_4_MHZ;
            break;

        case 0x16:
            *op_core_clk = KBP_OP_CORE_CLK_302_7_MHZ;
            break;

        case 0x17:
            *op_core_clk = KBP_OP_CORE_CLK_249_MHZ;
            break;

        case 0x18:
            *op_core_clk = KBP_OP_CORE_CLK_900_MHZ;
            break;
        ...
        case 0x1a:
            *op_core_clk = KBP_OP_CORE_CLK_720_MHZ;
            break;

        case 0x1b:
            *op_core_clk = KBP_OP_CORE_CLK_600_MHZ;
            break;
        ...
        case 0x1d:
            *op_core_clk = KBP_OP_CORE_CLK_400_MHZ;
            break;

        case 0x1e:
            *op_core_clk = KBP_OP_CORE_CLK_300_MHZ;
            break;

        case 0x1f:
            *op_core_clk = KBP_OP_CORE_CLK_250_MHZ;
            break;
