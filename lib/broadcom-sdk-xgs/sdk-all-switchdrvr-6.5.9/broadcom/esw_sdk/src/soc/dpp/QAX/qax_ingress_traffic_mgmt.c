...
#define QAX_ITM_NOF_TOTAL_WORDS(unit)      ((SOC_DPP_CONFIG(unit)->jer->dbuffs.dbuffs_bdries.mnmc_0.size) /* Number of buffers */ \
                                                 * (SOC_DPP_CONFIG(unit)->arad->init.dram.dbuff_size /* buffer size in bytes -- 4K for QAX */ \
                                                 / QAX_ITM_WORDS_RESOLUTION))

#define QAX_ITM_NOF_SRAM_WORDS(unit)    (SOC_DPP_DEFS_GET(unit, ocb_memory_size) \
                                            * (1024 * 1024 / 8 /* MBits -> Bytes */ \
                                                / QAX_ITM_WORDS_RESOLUTION))

/* When there are no DRAMs nof_words==nof_sram_words */
#define QAX_ITM_NOF_WORDS(unit)         ((SOC_DPP_CONFIG(unit)->arad->init.dram.nof_drams != 0) ? \
                                            QAX_ITM_NOF_TOTAL_WORDS(unit) : QAX_ITM_NOF_SRAM_WORDS(unit))
  ...
  )
{
    ...
    BCM_DPP_CORES_ITER(BCM_CORE_ALL, core_index) {
        ...
        /* Words */
        guaranteed_resources.guaranteed[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES].total = QAX_ITM_NOF_WORDS(unit);

        /* SRAM words */
        guaranteed_resources.guaranteed[SOC_TMC_INGRESS_THRESHOLD_SRAM_BYTES].total = QAX_ITM_NOF_SRAM_WORDS(unit);
    ...
    SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_index) {
        ...
        ingress_congestion_mgmt.global[SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES].total = QAX_ITM_NOF_WORDS(unit);
  ...
  )
{
    ...
    } else
    {
        ...
        /* 
       * if min_thresh is 0, we can't use C2 to calc MaxProb, 
       * so calculate if from C1
       */
  ...
  )
{
    ...
    /* Validate VSQ shared pools resources reject thresholds */
    for (rsrc_type = 0; rsrc_type < SOC_TMC_INGRESS_THRESHOLD_NOF_TYPES; ++rsrc_type) {
        if (rsrc_type == SOC_TMC_INGRESS_THRESHOLD_TOTAL_BYTES) {
            max_threshold = QAX_ITM_NOF_WORDS(unit) * QAX_ITM_WORDS_RESOLUTION;
        ...
        for (color = 0; color < SOC_TMC_NOF_DROP_PRECEDENCE; ++color) {
            if (info->global_shrd_pool_0[rsrc_type][color].set > max_threshold) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("VSQ Pool 0 SET threshold (%u) is above max threshold (%u)"),
                    info->global_shrd_pool_0[rsrc_type][color].set, max_threshold));
            }
            if (info->global_shrd_pool_0[rsrc_type][color].clear > max_threshold) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("VSQ Pool 0 CLEAR threshold (%u) is above max threshold (%u)"),
                    info->global_shrd_pool_0[rsrc_type][color].clear, max_threshold));
            }
            if (info->global_shrd_pool_1[rsrc_type][color].set > max_threshold) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("VSQ Pool 0 SET threshold (%u) is above max threshold (%u)"),
                    info->global_shrd_pool_1[rsrc_type][color].set, max_threshold));
            }
            if (info->global_shrd_pool_1[rsrc_type][color].clear > max_threshold) {
                SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("VSQ Pool 0 CLEAR threshold (%u) is above max threshold (%u)"),
                    info->global_shrd_pool_1[rsrc_type][color].clear, max_threshold));
            }
        }
    }
    ...
    for (sram_type = 0; sram_type < SOC_TMC_INGRESS_THRESHOLD_NOF_SRAM_TYPES; ++sram_type) {
        max_threshold = (sram_type == 0) ? (QAX_ITM_NOF_SRAM_PDBS(unit)) : (QAX_ITM_NOF_SRAM_BUFFERS(unit));
        for (color = 0; color < SOC_TMC_NOF_DROP_PRECEDENCE; ++color) {
    ...
    for (color = 0; color < SOC_TMC_NOF_DROP_PRECEDENCE; ++color) {
        if (info->global_free_dram[color].set > QAX_ITM_NOF_DRAM_BDBS(unit)) {
            SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
        }
        if (info->global_free_dram[color].clear > QAX_ITM_NOF_DRAM_BDBS(unit)) {
            SOCDNX_IF_ERR_EXIT(SOC_E_PARAM);
        }
    }
