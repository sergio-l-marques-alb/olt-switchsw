...
/* For HLM supported devices like QAX, first range (which includes counter pointer 0),
   must be skipped. Otherwise, for single counter processor command packets, counter pointer 0
   will be used (default) for the second counter processor command and compete with other
   pointers for the same range. */
int skip_first_counter_range_init(int unit) {
    int rv;
...
int set_counter_source_and_engines(int unit, int *counter_base, int port) {
   ...
   /* HLM supported devices cannot use first counter processor range for OAM */
   rv = skip_first_counter_range_init(unit);
   BCM_IF_ERROR_RETURN(rv);
   ...
   if (current_lm_counter_base[core] % NUMBER_OF_COUNTERS_PER_COUNTER_SOURCE == 0) {
       ...
       counter_engine.engine_id = next_available_counter_engine++;
