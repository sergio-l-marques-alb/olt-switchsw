...
int oam_hierarchical_lm_example(int unit, int port1, int port2) {
    ...
    if(is_pcp) {
        oam_qos_map_create(unit,gport2,BCM_GPORT_INVALID,BCM_GPORT_INVALID);
    }
    ...
    /* In case of PCP set the flag*/
    mep_3.lm_flags = BCM_OAM_LM_PCP * is_pcp;
    /*RX*/
    mep_3.gport = gport1;
    ...
    if (!hierarchical_lm_is_acc) {
        ...
        /* In case of PCP set the flag*/
        mep_4.lm_flags = BCM_OAM_LM_PCP * is_pcp;
    ...
    /* In case of PCP set the flag*/
    mep_6.lm_flags = BCM_OAM_LM_PCP * is_pcp;
...
int oam_hierarchical_lif_lm_example(int unit, int in_sysport, int out_sysport) {
    ...
    /* Configure the counters to use PCP*/
    if(is_pcp) {
        oam_qos_map_create(unit,gport1,gport2,gport3);
    }
    ...
    for (i = 0; i < 3; i++) {
        ...
        meps_info[i].lm_counter_base_id = counter_base_ids[counters_to_use[i]];
    }
