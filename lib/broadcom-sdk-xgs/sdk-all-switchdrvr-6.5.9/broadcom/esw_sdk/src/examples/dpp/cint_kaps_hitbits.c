 ...
 * To enable hitbits in KAPS for a specific table, run from the BCM shell:
 *    kbp kaps_hitbit table="FLP IPv4 UC KAPS" enable=1
 *    kbp kaps_hitbit table="FLP IPv4 MC KAPS" enable=1
 *    kbp kaps_hitbit table="FLP IPv6 UC KAPS" enable=1
 *    kbp kaps_hitbit table="FLP IPv6 MC KAPS" enable=1
 * The table names are as they appear in the output of "diag dbal Tables_Info".
 ...
 * exit;
 */
int kaps_hitbits_ipv_4_6_uc_mc_fill_entries(int unit, int out_port) {
    int rc,i;
    bcm_l3_intf_t l3if;
    ...
    int vlan = 5;
    bcm_multicast_t ipmc_group = 0x2000001 + vlan;
    ...
    sal_memcpy(l3eg.mac_addr, mac_l3_egress_add, 6);
    l3eg.vlan   = vlan;
    l3eg.port   = out_port;
    ...
    rc = bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L3 | BCM_MULTICAST_WITH_ID, &ipmc_group);
    if(rc) {
    ...
    for (prefix_length = 32; prefix_length >= 4; prefix_length-=1) {
        bcm_l3_route_t_init(&route);
    ...
    for (round = 0; round < 3; round++) {
        if (iteration == 0) {
            ...
            for (public_private_i = 0; public_private_i < 2; public_private_i++) {
                ...
                rc = bcm_l3_route_get(unit, &route);
                if (rc != BCM_E_NONE) {
                  printf ("IPv%d: bcm_l3_route_add failed: %d. prefix_length = %d.\n", ipv4?4:6, rc, prefix_length);
                  return rc;
                }
            ...
            if (ipv4) {
                /*We're expecting a total of 31 routes, 32-4+1 and 2 default routes*/
                expected_entry_counter = 31;
            } else {
                flags |= BCM_L3_IP6;
...
int kaps_hitbits_mc_verify_entries_traverse(int unit, bcm_ipmc_addr_t *info, void *user_data)
{
    ...
    if ((((prefix_length <= kaps_mc_prefix_length_hit_low) || (prefix_length >= kaps_mc_prefix_length_hit_high)) || (prefix_length == 0)) && (round != 2)) {
        if ((info->flags & BCM_IPMC_HIT) == 0) {
            printf ("IPv4 MC: HITBIT expected to be set, but it is not set. prefix_length = %d. round = %d.\n", prefix_length, round);
            print *info;
            return -1;
        } else {
            printf ("IPv4 MC: HITBIT correctly set. prefix_length = %d. round = %d.\n", prefix_length, round);
        }
    } else {
        if ((info->flags & BCM_IPMC_HIT) != 0) {
            printf ("IPv4 MC: HITBIT expected not to be set, but it is set. prefix_length = %d. round = %d.\n", prefix_length, round);
            print *info;
            return -1;
        } else {
            printf ("IPv4 MC: HITBIT correctly not set. prefix_length = %d. round = %d.\n", prefix_length, round);
        }
    }
