    ...
    char *filepath)
{
  ...
  if (ISEMPTY(filename))
  {
    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(0, "Filename not defined\n")));
    return _SHR_E_PARAM;
  }
  ...
  {
    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(0, "Directory:%s is unavailable\n"), filepath));
    return _SHR_E_PARAM;
  }

  if ((sal_strlen(sdk_dir) + sal_strlen(location) + sal_strlen(filename)) >= RHFILE_MAX_SIZE)
  {
    LOG_WARN(BSL_LOG_MODULE, (BSL_META_U(0, "PATH %s is too long\n"), sdk_dir));
    return _SHR_E_INTERNAL;
  }
