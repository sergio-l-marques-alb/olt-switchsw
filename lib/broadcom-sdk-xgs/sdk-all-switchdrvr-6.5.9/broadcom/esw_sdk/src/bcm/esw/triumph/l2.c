...
#endif
#if defined(BCM_GREYHOUND2_SUPPORT)
        } else if (bcmi_gh2_vxlan_vfi_used_get(unit, vfi)) {
...
#if defined(BCM_GREYHOUND2_SUPPORT) && defined(INCLUDE_L3)
        } else if (((dest_type_is_nhi == 0x4) || (dest_type_is_nhi == 0x5)) &&
...
bcm_tr_l2_addr_add(int unit, bcm_l2_addr_t *l2addr)
{
    ...
    if (soc_feature(unit, soc_feature_tsn)) {
        ...
        if (l2addr->tsn_flowset != 0) {
            BCM_IF_ERROR_RETURN(
                bcmi_esw_tsn_flowset_ref_inc(unit, l2addr->tsn_flowset));
        }
    }
#if defined(BCM_TSN_SR_SUPPORT)
    if (soc_feature(unit, soc_feature_tsn_sr)) {
        ...
        if (l2addr->sr_flowset != 0) {
            BCM_IF_ERROR_RETURN(
                bcmi_esw_tsn_sr_flowset_ref_inc(unit, l2addr->sr_flowset));
        }
        ...
        if (!BCM_PBMP_IS_NULL(l2addr->sa_source_filter_pbmp)) {
            ...
            if (BCM_FAILURE(rv)) {
                ...
                if (l2addr->sr_flowset != 0) {
                    BCM_IF_ERROR_RETURN(
                        bcmi_esw_tsn_sr_flowset_ref_dec(unit,
                                                        l2addr->sr_flowset));
                }
                return rv;
            }
    ...
    if ((rv == BCM_E_FULL) && (l2addr->flags & BCM_L2_REPLACE_DYNAMIC)) {
        ...
        if (soc_feature(unit, soc_feature_tsn)) {
            ...
            if (l2addr->tsn_flowset != 0) {
                BCM_IF_ERROR_RETURN(
                    bcmi_esw_tsn_flowset_ref_dec(unit, l2addr->tsn_flowset));
            }
        }
#if defined(BCM_TSN_SR_SUPPORT)
        if (soc_feature(unit, soc_feature_tsn_sr)) {
            ...
            if (l2addr->sr_flowset != 0) {
                BCM_IF_ERROR_RETURN(
                    bcmi_esw_tsn_sr_flowset_ref_dec(unit, l2addr->sr_flowset));
            }
    ...
    if (soc_feature(unit, soc_feature_mysta_profile)) {
        ...
        if (soc_mem_field32_get(unit, tcam_mem, &entry, VALIDf) != 0) {
            ...
            rv = _bcm_l2_mysta_profile_entry_delete(unit,
                                (_BCM_L2_STATION_ID_OVERLAY(s_ent->sid))?1:0,
                                mysta_profile_id);
            if (BCM_FAILURE(rv)) {
                SC_UNLOCK(sc);
                return (rv);
    ...
    if (SOC_IS_KATANAX(unit)
        ...
        if (soc_feature(unit, soc_feature_mysta_profile)) {
            ...
            rv = _bcm_l2_mysta_profile_entry_get(unit, &entry_prof,
                                                 (_BCM_L2_STATION_ID_OVERLAY(station_id))?1:0,
                                                 mysta_profile_id);
            if (BCM_FAILURE(rv)) {
                SC_UNLOCK(sc);
                return rv;
