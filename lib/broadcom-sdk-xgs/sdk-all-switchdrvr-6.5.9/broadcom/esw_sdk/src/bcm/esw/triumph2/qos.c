    ...
    for (idx = min_idx; idx < max_idx; idx++) {
        ...
        if (mem == L3_IIFm && field == TRUST_DSCP_PTRf) {
            ...
            if (field_len == _BCM_TR_L3_TRUST_DSCP_PTR_BIT_SIZE) { 
                ...
                if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWKX(unit)) {
                    if (hw_prof_idx == 0x7f) {
                        continue;
                    } 
                } else {
...
bcm_tr2_qos_map_add(int unit, uint32 flags, bcm_qos_map_t *map, int map_id)
{
    ...
    uint32 ing_pri_map[SOC_MAX_MEM_FIELD_WORDS];
    ing_untagged_phb_entry_t ing_untagged_phb;
    ...
    case _BCM_QOS_MAP_TYPE_EGR_DSCP_TABLE:
        if (!_BCM_QOS_EGR_DSCP_TABLE_USED_GET(unit, id)) {
...
#endif
            {
                ...
                if (SOC_IS_TRIDENT3X(unit)) {
                    soc_EGR_ZONE_2_QOS_MAPPING_TABLEm_field32_set(unit, entry, QOSf, map->dscp);
            ...
            if ((flags & BCM_QOS_MAP_INGRESS) == BCM_QOS_MAP_INGRESS) {
                ...
                if ((flags & BCM_QOS_MAP_L3) == BCM_QOS_MAP_L3) {
                    ...
                    if (SOC_IS_TD2_TT2(unit)) { /* Get l3 iif profile entry */
                        ...
                        if (BCM_SUCCESS(rv)) {
                            l3_index = soc_mem_field32_get(unit, L3_IIFm,
                                           (uint32 *)&entry, L3_IIF_PROFILE_INDEXf);
                            rv = _bcm_l3_iif_profile_entry_get(unit,
                                     l3_index, 1, entries);

                            if (BCM_SUCCESS(rv)) {
                                profile_idx = soc_mem_field32_get(unit, L3_IIF_PROFILEm,
