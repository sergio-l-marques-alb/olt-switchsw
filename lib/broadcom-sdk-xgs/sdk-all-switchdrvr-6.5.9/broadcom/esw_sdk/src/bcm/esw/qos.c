...
bcm_esw_qos_init(int unit)
{
    ...
    if ((SOC_IS_TD2_TT2(unit) || SOC_IS_KATANA2(unit)) &&
        ...
        BCM_IF_ERROR_RETURN(_bcm_dscp_table_entry_add(unit, entries_buf,
                                                      64, &dscp_index));
...
#if defined(BCM_TRIDENT2_SUPPORT)
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIDENT3(unit)) {
...
bcm_esw_qos_map_create(int unit, uint32 flags, int *map_id)
{
    ...
    if (SOC_IS_GREYHOUND2(unit)) {
        if ((flags & BCM_QOS_MAP_MPLS) ||
            ((flags & BCM_QOS_MAP_L3) && (flags & BCM_QOS_MAP_INGRESS))) {
            return rv;
        }
    }
