...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        return (_bcm_td3_pfc_deadlock_recovery_begin(unit,
                                      cos, pipe, pipe_mmu_port));
    }
#endif
...
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        return (_bcm_td3_pfc_deadlock_recovery_reset(unit));
    }
#endif
