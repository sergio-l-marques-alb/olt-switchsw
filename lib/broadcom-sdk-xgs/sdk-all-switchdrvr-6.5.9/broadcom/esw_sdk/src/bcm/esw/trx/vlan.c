        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            for (i = 0; i < soc_mem_index_count(unit, port_mem); i++) {
                if (!SOC_IS_TRIDENT3X(unit)) {
                    /* TD3TBD  Invalid port error - skip for now */
                                            ...
                                            void *return_ent)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, mem, MPLS_ACTIONf)) {
        ...
        value = soc_mem_field32_get(unit, mem, vent, MPLS_ACTIONf) & 0x2;

#if defined(BCM_TRIDENT2_SUPPORT)
        if (soc_feature(unit, soc_feature_vlan_xlate_iif_with_dummy_vp) &&
            soc_property_get(unit, spn_VLAN_ACTION_DUMMY_VP_RESERVED, 0) &&
            dummy_svp_chk) {
            svp_valid = soc_mem_field32_get(unit, mem, vent, MPLS_ACTIONf) & 0x1;
            ret_svp_valid =
                soc_mem_field32_get(unit, mem, return_ent, MPLS_ACTIONf) & 0x1;
