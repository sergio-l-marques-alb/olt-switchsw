...
#define POL_CHECK_NULL_PARAMETER(_u_, _parameter_)               \
    do {                                                         \
        if (NULL == (_parameter_)) {                             \
...
#define POL_CHECK_NULL_DEV_INSTANCE(_u_, _instance_)                          \
    do {                                                                      \
        if (NULL == (_instance_)) {                                           \
            LOG_ERROR(BSL_LS_BCM_POLICER,                                     \
                      (BSL_META_U((_u_),                                      \
                                  "Error: NULL driver function \n")));        \
            return BCM_E_UNAVAIL;                                             \
        }                                                                     \
    } while (0)

/* Function Enter/Leave Tracing */
#define POL_FUNCTION_TRACE(_u_, _str_)                                        \
    do {                                                                      \
...
/* Function VERBOSE Tracing */
#define POL_FUNCTION_DEBUG(_u_, fmt, _args_)                                  \
    do {                                                                      \
...
#define POL_MEM_ALLOC _GLOBAL_METER_XGS3_ALLOC

#define POL_MEM_FREE(_mem_)                                      \
    do {                                                         \
        if (NULL != (_mem_)) {                                   \
            sal_free(_mem_);                                     \
            (_mem_) = NULL;                                      \
...
#define POL_GET_LOG2(_n_, _power_)                                          \
    do {                                                                    \
...
#define BCMI_GLOBAL_METER_HASH_SIZE                  (0x100)
#define BCMI_GLOBAL_METER_HASH_MASK                  (0xff)
#define BCMI_POLICER_GLOBAL_METER_MAX_SCACHE_ENTRIES (128)
#define BCMI_POLICER_METER_FINER_GRAN_RATE_BOUNDARY  (8)
#define BCMI_POLICER_METER_FINER_GRAN_BURST_BOUNDARY (4)
#define BCMI_POLICER_METER_GRAN_SHFIT                (8)
#define BCMI_POLICER_METER_GRAN_LOWER_UNIT           (1000)

#define BCMI_POLICER_SVC_METER_TCAM_SIZE \
    SOC_INFO(unit).global_meter_tcam_size

#define BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE \
    SOC_INFO(unit).global_meter_offset_mode_max

#define BCMI_POLICER_SVC_METER_ACTION_SIZE \
    SOC_INFO(unit).global_meter_action_size

#define BCMI_POLICER_SVC_METER_POOL_SIZE \
    SOC_INFO(unit).global_meter_size_of_pool

/* Global Meter platform-specific information */
STATIC const bcmi_global_meter_dev_info_t
    *bcmi_global_meter_dev_info[BCM_MAX_NUM_UNITS];

/* Initialization flag for bcmi_global_meter_dev_info_t info structure */
STATIC int
    bcmi_global_meter_dev_info_initialized = FALSE;

#define BCM_SVM_DEV_INFO_GET(_u_) (bcmi_global_meter_dev_info[(_u_)])

#define CHECK_GLOBAL_METER_DEV_INFO_INIT(_u_)                  \
    do {                                                       \
        if (!soc_feature(unit, soc_feature_global_meter_v2)) { \
            return BCM_E_UNAVAIL;                              \
        }                                                      \
        if (!BCM_SVM_DEV_INFO_GET(_u_)) {                      \
            return BCM_E_UNAVAIL;                              \
        }                                                      \
    } while (0)

typedef struct pkt_res_final_offset_s {
    uint8 final_offset;
    bcm_policer_group_mode_attr_pkt_type_t attr_pkt_type;
}pkt_res_final_offset_t;

static bcmi_global_meter_policer_control_t
    **global_meter_policer_bk[BCM_MAX_NUM_UNITS];

static bcmi_policer_global_meter_action_bk_t
    *global_meter_action_bk[BCM_MAX_NUM_UNITS];

shr_aidxres_list_handle_t
    action_list_handle[BCM_MAX_NUM_UNITS] = { NULL };
shr_aidxres_list_handle_t
    policer_list_handle[BCM_MAX_NUM_UNITS] = { NULL };

#define BCMI_POLICER_BK_GET(_u_) \
    (global_meter_policer_bk[(_u_)])

#define BCMI_POLICER_HASH_BK_GET(_u_, _hash_) \
    (global_meter_policer_bk[(_u_)][(_hash_)])

#define BCMI_ACTION_BK_GET(_u_) \
    (global_meter_action_bk[(_u_)])

#define BCMI_ACTION_ID_BK_GET(_u_, _action_id_) \
    (global_meter_action_bk[(_u_)][(_action_id_)])

#define BCMI_ACTION_LIST_HANDLE_GET(_u_) \
    (action_list_handle[(_u_)])

#define BCMI_POLICER_LIST_HANDLE_GET(_u_) \
    (policer_list_handle[(_u_)])

static bcmi_policer_svm_bk_t bcmi_bk_svm[BCM_MAX_NUM_UNITS];

#define GLOBAL_METER_BK_LOCK(unit) \
        sal_mutex_take(bcmi_bk_svm[unit].mutex, sal_mutex_FOREVER);

#define GLOBAL_METER_BK_UNLOCK(unit) \
        sal_mutex_give(bcmi_bk_svm[unit].mutex);

#define BCMI_SVM_BK_GET(_u_) \
    (&bcmi_bk_svm[(_u_)])
#define BCMI_SVM_OFFSET_MODE_BK_GET(_u_, _offset_mode_) \
    (&bcmi_bk_svm[(_u_)].offset_mode_v[_offset_mode_])

/* SVM TCAM usage bitmap operations */
#define BCMI_SVM_TCAM_INTF_GET(_u_, _intf_) \
        SHR_BITGET(BCMI_SVM_BK_GET(_u_)->tcam_intf_bitmap, (_intf_))
#define BCMI_SVM_TCAM_INTF_SET(_u_, _intf_) \
        SHR_BITSET(BCMI_SVM_BK_GET(_u_)->tcam_intf_bitmap, (_intf_))
#define BCMI_SVM_TCAM_INTF_CLR(_u_, _intf_) \
        SHR_BITCLR(BCMI_SVM_BK_GET(_u_)->tcam_intf_bitmap, (_intf_))

/* SVM Offset Mode usage bitmap operations */
#define BCMI_OFFSET_MODE_ID_INTF_GET(_u_, _intf_) \
        SHR_BITGET(BCMI_SVM_BK_GET(_u_)->offset_mode_intf_bitmap, (_intf_))
#define BCMI_OFFSET_MODE_ID_INTF_SET(_u_, _intf_) \
        SHR_BITSET(BCMI_SVM_BK_GET(_u_)->offset_mode_intf_bitmap, (_intf_))
#define BCMI_OFFSET_MODE_ID_INTF_CLR(_u_, _intf_) \
        SHR_BITCLR(BCMI_SVM_BK_GET(_u_)->offset_mode_intf_bitmap, (_intf_))

/* SVM Mode ID & Offset Mode conversion */
#define OFFSET_MODE_TO_MODE_ID(_offset_mode_) \
        (_offset_mode_ + 1)

#define MODE_ID_TO_OFFSET_MODE(_mode_id_) \
        (_mode_id_ - 1)

/*
 * SVM Policer ID encoding:
 * 1bit (reserved) + 3-bit mode_id (1~4) + hw_policer_idx
 */
#define POLICER_IDX_TO_POLICER_ID(_offset_mode_, _policer_idx_) \
        (_policer_idx_) | \
        (OFFSET_MODE_TO_MODE_ID(_offset_mode_) << \
         BCM_POLICER_GLOBAL_METER_MODE_SHIFT)

#define POLICER_ID_TO_POLICER_IDX(_policer_id_) \
        ((_policer_id_) & BCM_POLICER_GLOBAL_METER_INDEX_MASK)

#define POLICER_ID_MODE_ID_GET(_policer_id_) \
        (((_policer_id_) & BCM_POLICER_GLOBAL_METER_MODE_MASK) >> \
         BCM_POLICER_GLOBAL_METER_MODE_SHIFT)

#define POLICER_ID_OFFSET_MODE_GET(_policer_id_) \
        (MODE_ID_TO_OFFSET_MODE(POLICER_ID_MODE_ID_GET(_policer_id_)))

#define POLICER_ID_API_SECTION 1
#define MODE_ID_API_SECTION 1
#define ACTION_API_SECTION 1
...
bcmi_global_meter_dev_info_init(int unit)
{
    ...
    if (BCM_FAILURE(rv) || NULL == bcmi_global_meter_dev_info[unit]) {
    ...
    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
}

STATIC const char
*bcm_policer_group_mode_attr_t_str[bcmPolicerGroupModeAttrCount] = {
    "Invalid String", /* Since bcm_policer_group_mode_attr_t start from 1 */
    "bcmPolicerGroupModeAttrUdf",                  /* 1 */
    "bcmPolicerGroupModeAttrFieldIngressColor",
    "bcmPolicerGroupModeAttrIntPri",
    "bcmPolicerGroupModeAttrVlan",
    "bcmPolicerGroupModeAttrOuterPri",             /* 5 */
    "bcmPolicerGroupModeAttrInnerPri",
    "bcmPolicerGroupModeAttrPort",
    "bcmPolicerGroupModeAttrTosDscp",
    "bcmPolicerGroupModeAttrTosEcn",
    "bcmPolicerGroupModeAttrPktType",              /* 10 */
    ...
    "bcmPolicerGroupModeAttrEtherType"             /* 15 */
};

STATIC const char
*bcm_policer_group_mode_t_str[bcmPolicerGroupModeCount] = {
    "bcmPolicerGroupModeSingle",                   /* 0 */
    "bcmPolicerGroupModeTrafficType",
    "bcmPolicerGroupModeDlfAll",
    "bcmPolicerGroupModeDlfIntPri",
    "bcmPolicerGroupModeTyped",
    "bcmPolicerGroupModeTypedAll",                 /* 5 */
    "bcmPolicerGroupModeTypedIntPri",
    "bcmPolicerGroupModeSingleWithControl",
    "bcmPolicerGroupModeTrafficTypeWithControl",
    "bcmPolicerGroupModeDlfAllWithControl",
    "bcmPolicerGroupModeDlfIntPriWithControl",     /* 10 */
    "bcmPolicerGroupModeTypedWithControl",
    "bcmPolicerGroupModeTypedAllWithControl",
    "bcmPolicerGroupModeTypedIntPriWithControl",
    "bcmPolicerGroupModeDot1P",
    "bcmPolicerGroupModeIntPri",                   /* 15 */
    "bcmPolicerGroupModeIntPriCng",
    "bcmPolicerGroupModeSvpType",
    "bcmPolicerGroupModeDscp",
    "bcmPolicerGroupModeCascade",
    "bcmPolicerGroupModeCascadeWithCoupling",      /* 20 */
    "bcmPolicerGroupModeInnerDot1P",
    "bcmPolicerGroupModeEgressSingle",
    "bcmPolicerGroupModeEgressIntPri",
    "bcmPolicerGroupModeShortIntPri",
    "bcmPolicerGroupModeIntPriCascade",            /* 25 */
    "bcmPolicerGroupModeIntPriCascadeWithCoupling",
    "bcmPolicerGroupModePreemptable"
};

STATIC const uint32
attr_to_bit_selector[bcmPolicerGroupModeAttrCount] = {
    /* Since bcm_policer_group_mode_attr_t start from 1 */
    0,
    /* bcmPolicerGroupModeAttrUdf = 1 */
    BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_UDF_ATTR_BITS,
    /* bcmPolicerGroupModeAttrFieldIngressColor */
    ...
    /* bcmPolicerGroupModeAttrIngNetworkGroup = 11 */
    ...
    /* bcmPolicerGroupModeAttrPreemptable */
    BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PREEMPTABLE_ATTR_BITS,
    /* bcmPolicerGroupModeAttrEtherType */
    BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_ETHERTYPE_ATTR_BITS
};

/*
 * Function:
 *     check_attr_to_bit_selector
 * Purpose:
 *     Check if attr_to_bit_selector is mapping correctly.
 * Parameters:
 * Returns:
 *     none
 */
STATIC void
check_attr_to_bit_selector(void)
{
    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrUdf] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_UDF_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrOuterPri] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_OUTER_DOT1P_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrInnerPri] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_INNER_DOT1P_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrPort] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_INGRESS_PORT_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrTosDscp] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_TOS_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrTosEcn] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_TOS_ECN_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrPktType] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PKT_RESOLUTION_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrIngNetworkGroup] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_SVP_TYPE_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrDrop] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_DROP_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrPacketTypeIp] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_IP_PKT_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrPreemptable] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_PREEMPTABLE_ATTR_BITS);

    assert(attr_to_bit_selector[bcmPolicerGroupModeAttrEtherType] ==
        BCM_POLICER_SVC_METER_UNCOMPRESSED_USE_ETHERTYPE_ATTR_BITS);
}

/*
 * Function:
 *     pkt_tcam_attr_bits_t_init
 * Purpose:
 *     Init pkt_tcam_attr_bits_t
 * Parameters:
 *     pkt_attr_selectors - pointer instance to pkt_tcam_attr_bits_t
 * Returns:
 *     BCM_E_XXX
 */
void
pkt_tcam_attr_bits_t_init(pkt_tcam_attr_bits_t *attr)
{
    /* Parameter NULL error handling */
    if (NULL == attr) {
        return;
    ...
    sal_memset(attr, 0, sizeof(pkt_tcam_attr_bits_t));
}

/*
 * Function:
 *     bcmi_policer_tcam_attr_selectors_t_init
 * Purpose:
 *     Init bcmi_policer_tcam_attr_selectors_t
 * Parameters:
 *     pkt_attr_selectors - pointer instance to bcmi_policer_tcam_attr_selectors_t
 * Returns:
 *     BCM_E_XXX
 */
STATIC void
bcmi_policer_tcam_attr_selectors_t_init(
    bcmi_policer_tcam_attr_selectors_t *pkt_attr_selectors)
{
    /* Parameter NULL error handling */
    if (NULL == pkt_attr_selectors) {
        return;
    }

    sal_memset(pkt_attr_selectors, 0,
        sizeof(bcmi_policer_tcam_attr_selectors_t));
}

/*
 * Function:
 *     bcmi_policer_svm_offset_mode_bk_t_init
 * Purpose:
 *     Init bcmi_policer_svm_offset_mode_bk_t
 * Parameters:
 *     bk_svm_mode_info - pointer instance to bcmi_policer_svm_offset_mode_bk_t
 * Returns:
 *     BCM_E_XXX
 */
STATIC void
bcmi_policer_svm_offset_mode_bk_t_init(
    bcmi_policer_svm_offset_mode_bk_t *bk_svm_mode_info)
{
    /* Parameter NULL error handling */
    if (NULL == bk_svm_mode_info) {
        return;
    }
    bk_svm_mode_info->ref_cnt = 0;
    bk_svm_mode_info->used = 0;
    bk_svm_mode_info->is_custom_group = FALSE;
    bk_svm_mode_info->group_mode = bcmPolicerGroupModeCount;
    bk_svm_mode_info->group_mode_type = bcmPolicerGroupModeTypeCount;
    bk_svm_mode_info->no_of_policers = 0;
    bk_svm_mode_info->no_of_attr_selectors = 0;
    bk_svm_mode_info->attr_selectors = NULL;
    bk_svm_mode_info->pkt_attr_v_cnt = 0;
    bk_svm_mode_info->pkt_attr_v = NULL;
}

/*
 * Function:
 *     bcmi_global_meter_policer_control_t_init
 * Purpose:
 *     Init bcmi_global_meter_policer_control_t
 * Parameters:
 *     policer_control - pointer instance to bcmi_global_meter_policer_control_t
 * Returns:
 *     BCM_E_XXX
 */
STATIC void
bcmi_global_meter_policer_control_t_init(
    bcmi_global_meter_policer_control_t *policer_control)
{
    /* Parameter NULL error handling */
    if (NULL == policer_control) {
        return;
    }
    policer_control->pid = 0;
    policer_control->action_id = NULL;
    policer_control->ref_count = 0;
    policer_control->no_of_policers = 0;
    policer_control->grp_mode = bcmPolicerGroupModeCount;
    policer_control->next = NULL;
}
...
/*
 * Function:
 *      bcmi_policer_tcam_attr_dump
 * Purpose:
 *      dump the config of Global Meter
 * Parameters:
 *      unit   - (IN) Unit number
 *      prefix - (IN) Prefix string for dump message
 *      attr   - (IN) Global Meter Config
 * Returns:
 *      BCM_E_xxx
 */
STATIC void
bcmi_policer_tcam_attr_dump(
    int unit,
    char *prefix,
    pkt_tcam_attr_bits_t *attr)
{
    int i;

    if ((NULL == prefix) || (NULL == attr)) {
        return;
    }
    ...
    if (bsl_check(bslLayerBcm, bslSourcePolicer, bslSeverityDebug, unit)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                             "%s: ref_count %d, tcam_idx %d, final_offset %d\n"),
                             prefix,
                             attr->ref_count,
                             attr->tcam_idx,
                             attr->final_offset));
        ...
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                             "%s: attr_bits_selector 0x%8x\n"),
                             prefix,
                             attr->attr_bits_selector));
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                             "%s: udf0 value 0x%4x, mask 0x%4x\n"),
                             prefix,
                             attr->udf.udf0,
                             attr->udf_mask.udf0));
        ...
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                             "%s: udf1 value 0x%4x, mask 0x%4x\n"),
                             prefix,
                             attr->udf.udf1,
                             attr->udf_mask.udf1));

        for (i = bcmPolicerGroupModeAttrUdf;
             i < bcmPolicerGroupModeAttrCount; i++) {
            if (attr->attr_bits_selector & attr_to_bit_selector[i]) {
                LOG_DEBUG(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                                      "%s: [%d] value %4x, mask %4x, %s\n"),
                                      prefix,
                                      i,
                                      attr->value[i],
                                      attr->value_mask[i],
                                      bcm_policer_group_mode_attr_t_str[i]));
            }
        }
    }
}
...
/*
 * Function:
 *     bcmi_policer_tcam_allocate_check
 * Purpose:
 *     Check TCAM size is enough or not
 * Parameters:
 *     Unit            - (IN) unit number
 *     pkt_attr_v_cnt  - (IN) pkt_attr_v_cnt
 * Returns:
 *     BCM_E_XXX
 */
STATIC bcm_error_t
bcmi_policer_tcam_allocate_check(int unit, uint32 pkt_attr_v_cnt)
{
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Initialization check */
    CHECK_GLOBAL_METER_INIT(unit);
    bk_svm_info = BCMI_SVM_BK_GET(unit);

    /* Check if total TCAM is enough or not */
    if (pkt_attr_v_cnt > BCMI_POLICER_SVC_METER_TCAM_SIZE) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "TCAM entry (%d) isn't enough, need %d TCAMs\n"),
                              BCMI_POLICER_SVC_METER_TCAM_SIZE,
                              pkt_attr_v_cnt));
        return BCM_E_UNAVAIL;
    }

    /* Check if empty TCAM is enough */
    if (pkt_attr_v_cnt > bk_svm_info->empty_tcam_idx_cnt) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "No enough empty TCAM(%d), need %d TCAMs\n"),
                              bk_svm_info->empty_tcam_idx_cnt,
                              pkt_attr_v_cnt));
...
/*
 * Function:
 *     bcmi_policer_tcam_configurate_hw
 * Purpose:
 *     Check TCAM size is enough or not
 * Parameters:
 *     Unit            - (IN) unit number
    ...
    bcmi_policer_tcam_attr_selectors_t *pkt_attr_selectors)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, pkt_attr_selectors);

    /* Initialization check */
    CHECK_GLOBAL_METER_INIT(unit);
    CHECK_GLOBAL_METER_DEV_INFO_INIT(unit);
    gm_dev_info = BCM_SVM_DEV_INFO_GET(unit);

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->tcam_keys_set);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->tcam_keys_delete);

    /* Configured TCAM HW with pkt_attr_selectors */
    for (sel = 0; sel < pkt_attr_selectors->pkt_attr_v_cnt; sel++) {
        rv = gm_dev_info->tcam_keys_set(
                unit, &pkt_attr_selectors->pkt_attr_v[sel]);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "HW Operation failed(rv = %d)\n"),
                                  rv));
            /* Reset the TCAM to defaul value */
            for (sel2 = 0; sel2 <= sel; sel2++) {
                rv2 = gm_dev_info->tcam_keys_delete(
                        unit, pkt_attr_selectors->pkt_attr_v[sel2].tcam_idx);
                if (BCM_FAILURE(rv2)) {
                    /* tcam_keys_delete shouldn't return error, need to check */
                    LOG_ERROR(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "tcam_keys_delete failed\n")));
                    return rv2;
                }
            }
            return rv;
...
/*
 * Function:
 *      bcmi_policer_offset_mode_validate
 * Purpose:
 *      Validate the given offset_mode
 * Parameters:
 *     Unit             - (IN) unit number
 *     offset_mode      - (IN) Meter offset_mode
 * Returns:
 *     BCM_E_XXX
 */
STATIC bcm_error_t
bcmi_policer_offset_mode_validate(int unit, uint32 offset_mode)
{
    POL_FUNCTION_TRACE(unit, "IN");
    if (offset_mode >= BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE)
    {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Invalid Offset mode.\n")));
        POL_FUNCTION_TRACE(unit, "OUT Invalid Offset mode");
        return BCM_E_PARAM;
...
/*
 * Function:
 *      bcmi_policer_mode_id_validate
 * Purpose:
 *      Validate the given mode_id
 * Parameters:
 *     Unit                  - (IN) unit number
 *     mode_id               - (IN) mode_id
 * Returns:
 ...
 */
STATIC bcm_error_t
bcmi_policer_mode_id_validate(int unit, uint32 mode_id)
{
    int rv = BCM_E_NONE;
    ...
    /* Validate offset_mode */
    offset_mode = MODE_ID_TO_OFFSET_MODE(mode_id);
    rv = bcmi_policer_offset_mode_validate(unit, offset_mode);
...
/*
 * Function:
 *     bcmi_policer_svc_meter_reserve_mode
 * Purpose:
 ...
 *     group_mode_type       - (IN) Policer group mode type
 *     npolicers             - (IN) Number of policers
 *     pkt_attr_selectors    - (IN) tcam packet attribute selectors
 * Returns:
 *     BCM_E_XXX
 */
STATIC bcm_error_t
    ...
    bcmi_policer_tcam_attr_selectors_t *pkt_attr_selectors)
{
    ...
    bcmi_policer_svm_offset_mode_bk_t *bk_svm_mode_info = NULL;
    int sel;
    ...
    GLOBAL_METER_BK_LOCK(unit);
    bk_svm_mode_info = BCMI_SVM_OFFSET_MODE_BK_GET(unit, offset_mode);
    bk_svm_mode_info->ref_cnt = 0;
    bk_svm_mode_info->used = 1;
    bk_svm_mode_info->group_mode_type = group_mode_type;
    bk_svm_mode_info->group_mode = group_mode;
    bk_svm_mode_info->no_of_policers = npolicers;

    POL_MEM_FREE(bk_svm_mode_info->pkt_attr_v);
    bk_svm_mode_info->pkt_attr_v_cnt = pkt_attr_selectors->pkt_attr_v_cnt;
    POL_MEM_ALLOC(
        bk_svm_mode_info->pkt_attr_v,
        bk_svm_mode_info->pkt_attr_v_cnt * sizeof(pkt_tcam_attr_bits_t),
        "pkt_attr_v in offset mode instance");
    if (NULL == bk_svm_mode_info->pkt_attr_v) {
        GLOBAL_METER_BK_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    for (sel = 0; sel < pkt_attr_selectors->pkt_attr_v_cnt ; sel++) {
        sal_memcpy(&bk_svm_mode_info->pkt_attr_v[sel],
                   &pkt_attr_selectors->pkt_attr_v[sel],
                   sizeof(pkt_tcam_attr_bits_t));
    }
    /* Configured TCAM HW with pkt_attr_selectors */
    rv = bcmi_policer_tcam_configurate_hw(unit, pkt_attr_selectors);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_BK_UNLOCK(unit);
        return rv;
    }
    ...
    /*
     * if and only if using bcmi_esw_policer_group_mode_id_create need
     * to record attr_selectors
     */
    bk_svm_mode_info->is_custom_group = FALSE;
    bk_svm_mode_info->no_of_attr_selectors = 0;
    POL_MEM_FREE(bk_svm_mode_info->attr_selectors);
...
STATIC int
bcmi_esw_policer_group_mode_id_delete(
    int unit,
    uint32 mode_id);

/*
 * Function:
 *      bcmi_policer_svc_meter_unreserve_mode
 * Purpose:
    ...
    bcm_policer_svc_meter_mode_t offset_mode)
{
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->tcam_keys_delete);
    /* Delete TCAM and Reset TCAM HW selector keys */
    for (sel = 0; sel < bk_svm_mode_info->pkt_attr_v_cnt ; sel++) {
        GLOBAL_METER_LOCK(unit);
        rv = gm_dev_info->tcam_keys_delete(
                unit, bk_svm_mode_info->pkt_attr_v[sel].tcam_idx);
        ...
        GLOBAL_METER_BK_LOCK(unit);
        /* Delete the TCAM idx */
        BCMI_SVM_TCAM_INTF_CLR(unit, bk_svm_mode_info->pkt_attr_v[sel].tcam_idx);
        bk_svm_info->empty_tcam_idx_cnt++;
        if (bk_svm_info->valid_tcam_idx_cnt > 0) {
            bk_svm_info->valid_tcam_idx_cnt--;
        } else {
            GLOBAL_METER_BK_UNLOCK(unit);
            LOG_WARN(BSL_LS_BCM_POLICER,
                     (BSL_META_U(unit,
                                 "Need to check valid_tcam_idx_cnt logic\n")));
            return BCM_E_INTERNAL;
        }
        GLOBAL_METER_BK_UNLOCK(unit);
    }
    /* Delete the offset mode */
    rv = bcmi_esw_policer_group_mode_id_delete(
            unit, OFFSET_MODE_TO_MODE_ID(offset_mode));

    BCM_IF_ERROR_RETURN(rv);

    POL_FUNCTION_DEBUG(unit,
        "Free %d TCAM", bk_svm_mode_info->pkt_attr_v_cnt);
    POL_FUNCTION_DEBUG(unit,
        "%d empty TCAM left", bk_svm_info->empty_tcam_idx_cnt);
...
/*
 * Function:
 *     bcmi_policer_svc_meter_delete_mode
 * Purpose:
    ...
    bcm_policer_svc_meter_mode_t offset_mode)
{
    ...
    rv = bcmi_policer_svc_meter_unreserve_mode(unit, offset_mode);
    BCM_IF_ERROR_RETURN(rv);
...
/*
 * Function:
 *     bcmi_policer_svc_meter_inc_mode_ref_cnt
 * Purpose:
...
/*
 * Function:
 *     bcmi_policer_svc_meter_dec_mode_ref_cnt
 * Purpose:
    ...
    bcm_policer_svc_meter_mode_t offset_mode)
{
    ...
    if (bk_svm_mode_info->is_custom_group) {
        /* Deletion of the mode is done through
           bcm_policer_group_mode_id_destroy */
        GLOBAL_METER_BK_UNLOCK(unit);
        POL_FUNCTION_TRACE(unit, "OUT:is_custom_group");
        return BCM_E_NONE;
    }
    if (bk_svm_mode_info->ref_cnt == 0) {
        rv = bcmi_policer_svc_meter_delete_mode(unit, offset_mode);
    }
    GLOBAL_METER_BK_UNLOCK(unit);
    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
...
#if MODE_ID_API_SECTION
/*
 * Function:
 *     bcmi_esw_policer_group_mode_fillup_tcam_idx
 * Purpose:
 *     update offset mode
 * Parameters:
 *     unit            - (IN) unit number
 *     pkt_attr_v      - (IN) Packet TCAM attributes
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
bcmi_esw_policer_group_mode_fillup_tcam_idx(
    int unit,
    pkt_tcam_attr_bits_t *pkt_attr_v)
{
    uint32 e_tcam_idx;
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, pkt_attr_v);
    ...
    GLOBAL_METER_BK_LOCK(unit);
    /* Check if empty tcam is enough */
    if (bk_svm_info->empty_tcam_idx_cnt == 0) {
        GLOBAL_METER_BK_UNLOCK(unit);
    ...
    for (e_tcam_idx = bk_svm_info->empty_tcam_idx_start;
         e_tcam_idx <= bk_svm_info->empty_tcam_idx_end;
         e_tcam_idx++) {
        if (BCMI_SVM_TCAM_INTF_GET(unit, e_tcam_idx) == 0) {
            /* fill with the found empty tcam idx */
            BCMI_SVM_TCAM_INTF_SET(unit, e_tcam_idx);

            /* Update valid start and end */
            if (e_tcam_idx < bk_svm_info->valid_tcam_idx_start) {
                bk_svm_info->valid_tcam_idx_start = e_tcam_idx;
            } else if (e_tcam_idx > bk_svm_info->valid_tcam_idx_end) {
                bk_svm_info->valid_tcam_idx_end = e_tcam_idx;
            }
            bk_svm_info->empty_tcam_idx_start = e_tcam_idx + 1;
            bk_svm_info->valid_tcam_idx_cnt++;
            if (bk_svm_info->empty_tcam_idx_cnt > 0) {
                bk_svm_info->empty_tcam_idx_cnt--;
            }

            pkt_attr_v->tcam_idx = e_tcam_idx;
            break;
        }
    }
    GLOBAL_METER_BK_UNLOCK(unit);
...
/*
 * Function:
 *      bcmi_esw_policer_group_empty_mode_id_get
 * Purpose:
 *      Get an empty mode_id
 * Parameters:
 *      unit     - (IN) Unit number
 *      mode_id   - (OUT) Meter Mode ID
 * Returns:
 *      BCM_E_NONE - Success on finding a empty mode_id
 */
STATIC int
bcmi_esw_policer_group_empty_mode_id_get(
    int unit,
    uint32 *mode_id)
{
    ...
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    /* Empty offset_mode */
    uint32 e_offset_mode;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, mode_id);
    ...
    /* Check if empty mode_id is enough */
    if (bk_svm_info->empty_offset_mode_cnt == 0) {
        LOG_ERROR(BSL_LS_BCM_TSN,
                  (BSL_META_U(unit,
                              "Not enough empty mode_id\n")));
        return BCM_E_RESOURCE;
    }

    GLOBAL_METER_BK_LOCK(unit);
    /* Try to find an empty id */
    for (e_offset_mode = bk_svm_info->empty_offset_mode_start;
         e_offset_mode <= bk_svm_info->empty_offset_mode_end;
         e_offset_mode++) {
        if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, e_offset_mode) == 0) {

            /* An empty id found, set bit map as used */
            BCMI_OFFSET_MODE_ID_INTF_SET(unit, e_offset_mode);

            /* Update valid start and end */
            if (bk_svm_info->valid_offset_mode_cnt == 0) {
                bk_svm_info->valid_offset_mode_start = e_offset_mode;
                bk_svm_info->valid_offset_mode_end = e_offset_mode;
            } else if (e_offset_mode <
                       bk_svm_info->valid_offset_mode_start) {
                bk_svm_info->valid_offset_mode_start = e_offset_mode;
            } else if (e_offset_mode >
                       bk_svm_info->valid_offset_mode_end) {
                bk_svm_info->valid_offset_mode_end = e_offset_mode;
            }
            bk_svm_info->valid_offset_mode_cnt++;
            if (bk_svm_info->empty_offset_mode_cnt > 0) {
                bk_svm_info->empty_offset_mode_cnt--;
            }
            *mode_id = OFFSET_MODE_TO_MODE_ID(e_offset_mode);

            /* Update the empty start, assume next one is real empty */
            bk_svm_info->empty_offset_mode_start = e_offset_mode + 1;

            /* An empty id found, break and return BCM_E_NONE */
            rv = BCM_E_NONE;
            break;
        } else {
            /* Not empty id, and then update the bk empty start */
    ...
    POL_FUNCTION_DEBUG(unit, "mode_id %d\n", *mode_id);
    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
}
 ...
 * Parameters:
 *   unit     - (IN) unit number
 *   mode_id  - (IN) Destroy Policer Group Mode with given mode_id
 * Returns:
 *   BCM_E_NONE - Success on destroying the given mode_id
 */
STATIC int
bcmi_esw_policer_group_mode_id_delete(
    int unit,
    uint32 mode_id)
{
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    bcmi_policer_svm_offset_mode_bk_t *bk_svm_mode_info = NULL;
    uint32 destroy_offset_mode;
    uint32 n_valid_offset_mode;
    ...
    bk_svm_info = BCMI_SVM_BK_GET(unit);
    destroy_offset_mode = MODE_ID_TO_OFFSET_MODE(mode_id);
    bk_svm_mode_info = BCMI_SVM_OFFSET_MODE_BK_GET(unit, destroy_offset_mode);

    /* Free allocate memory and reset bcmi_policer_svm_offset_mode_bk_t */
    POL_MEM_FREE(bk_svm_mode_info->attr_selectors);
    POL_MEM_FREE(bk_svm_mode_info->pkt_attr_v);
    bcmi_policer_svm_offset_mode_bk_t_init(bk_svm_mode_info);

    GLOBAL_METER_BK_LOCK(unit);
    /* Destroy bit map */
    BCMI_OFFSET_MODE_ID_INTF_CLR(unit, destroy_offset_mode);

    /* Update empty start and end */
    if (destroy_offset_mode < bk_svm_info->empty_offset_mode_start) {
        bk_svm_info->empty_offset_mode_start = destroy_offset_mode;
    } else if (destroy_offset_mode > bk_svm_info->empty_offset_mode_end) {
        bk_svm_info->empty_offset_mode_end = destroy_offset_mode;
    }

    /* Update valid, find a next valid start or next valid end */
    n_valid_offset_mode = destroy_offset_mode;
    if (bk_svm_info->valid_offset_mode_cnt > 1) {
        if (n_valid_offset_mode == bk_svm_info->valid_offset_mode_start) {
            do {
                n_valid_offset_mode++;
            } while (BCMI_OFFSET_MODE_ID_INTF_GET(unit, n_valid_offset_mode) == 0);
            bk_svm_info->valid_offset_mode_start = n_valid_offset_mode;
        } else if (n_valid_offset_mode == bk_svm_info->valid_offset_mode_end) {
            do {
                n_valid_offset_mode--;
            } while (BCMI_OFFSET_MODE_ID_INTF_GET(unit, n_valid_offset_mode) == 0);
            bk_svm_info->valid_offset_mode_end = n_valid_offset_mode;
        }
    }
    bk_svm_info->valid_offset_mode_cnt--;
    bk_svm_info->empty_offset_mode_cnt++;
...
/*
 * Function:
 *     bcmi_esw_policer_group_mode_id_associate
 * Purpose:
 *     This function is used to reserve svm meter mode and
 *     update h/w register/table
 * Parameters:
 *     unit                  - (IN) unit number
 *     flags                 - (IN) flags
 *     group_mode_type       - (IN) type of group mode
 *     pkt_attr_selectors    - (IN) tcam packet attribute selectors
 *     mode_id               - (OUT) mode_id which used by policer group
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    uint32 *mode_id)
{
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->group_mode_type_supported);

    rv = gm_dev_info->group_mode_type_supported(unit, group_mode_type);
    BCM_IF_ERROR_RETURN(rv);

    pkt_attr_v_cnt = pkt_attr_selectors->pkt_attr_v_cnt;

    GLOBAL_METER_BK_LOCK(unit);
    /* Check if TCAM size if enough or not */
    rv = bcmi_policer_tcam_allocate_check(unit, pkt_attr_v_cnt);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_BK_UNLOCK(unit);
        return rv;
    }

    /* Get an empty mode_id(offset_mode) */
    rv = bcmi_esw_policer_group_empty_mode_id_get(unit, mode_id);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_BK_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_TSN,
                  (BSL_META_U(unit,
                              "no empty mode_id(rv = %d)\n"),
                              rv));
        return rv;
    }

    /* Allocate empty TCAM for each pkt_attr_v with mode and offset_mode */
    for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
        /* Assign an empty TCAM idx */
        rv = bcmi_esw_policer_group_mode_fillup_tcam_idx(
                unit, &pkt_attr_selectors->pkt_attr_v[sel]);
        if (BCM_FAILURE(rv)) {
            /*
             * This case shouldn't happen since the TCAM size is enough
             * (already checked in above bcmi_policer_tcam_allocate_check)
             */
            GLOBAL_METER_BK_UNLOCK(unit);
            return rv;
        }
        ...
        /* Assign mode id */
        pkt_attr_selectors->pkt_attr_v[sel].mode_id = *mode_id;
        /* Assign offset_mode */
        pkt_attr_selectors->pkt_attr_v[sel].offset_mode =
            MODE_ID_TO_OFFSET_MODE(*mode_id);
    }
    GLOBAL_METER_BK_UNLOCK(unit);
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_mode_fillup_tcam_values
 * Purpose:
 *     Fill up temporary tcam structures in appropriate order for efficient
 *     management of user input
 * Parameters:
 *     unit                  - (IN) unit number
 *     pol_offset            - (IN) flags
 *     attr                  - (IN) group mode packet attr type
 *     attr_value            - (IN) attribute value
 *     attr_value_mask       - (IN) mask of attribute value
 *     pkt_attr_v            - (OUT) Packet TCAM attributes
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    pkt_tcam_attr_bits_t *pkt_attr_v)
{
    ...
    /* "OR" the added attr to current attr_bits_selector */
    pkt_attr_v->attr_bits_selector |= attr_to_bit_selector[attr];
    pkt_attr_v->value[attr] = attr_value;
    pkt_attr_v->value_mask[attr] = attr_value_mask;
    pkt_attr_v->final_offset = pol_offset;
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_mode_fillup_values
 * Purpose:
 ...
 *     pkt_attr_selectors    - (OUT) tcam packet attribute selectors
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
...
{
    int rv;
    uint32 sel = 0, mul_sel = 0;
    uint32 pol_offset = 0, max_pol_offset = 0;
    SHR_BITDCL *pol_offset_int_f_bitmap;
    uint32 shr_bitdcl_size;
    uint32 acutal_total_policers = 0;
    bcm_policer_group_mode_attr_t attr;
    uint32 attr_value = 0, attr_value_mask = 0;
    const bcmi_global_meter_dev_info_t *gm_dev_info = NULL;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, attr_selectors);
    POL_CHECK_NULL_PARAMETER(unit, pkt_attr_selectors);
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->group_mode_attr_validate);

    /* Loop all selectors checking if group_mode_attr & value is supported */
    for (sel = 0; sel < num_selectors; sel++) {
        if (BCM_POLICER_ATTR_MULTI & attr_selectors[sel].flags) {
            for (mul_sel = 0;
                 mul_sel < attr_selectors[sel].multi_attr_count;
                 mul_sel++) {
                rv = gm_dev_info->group_mode_attr_validate(
                        unit,
                        attr_selectors[sel].multi_attr[mul_sel],
                        attr_selectors[sel].multi_attr_value[mul_sel]);
                BCM_IF_ERROR_RETURN(rv);
            }
        } else {
            rv = gm_dev_info->group_mode_attr_validate(
                    unit,
                    attr_selectors[sel].attr,
                    attr_selectors[sel].attr_value);
            BCM_IF_ERROR_RETURN(rv);
        }
    }

#define POL_OFFSET_INTF_GET(_intf_) \
        SHR_BITGET(pol_offset_int_f_bitmap, (_intf_))
#define POL_OFFSET_INTF_SET(_intf_) \
        SHR_BITSET(pol_offset_int_f_bitmap, (_intf_))

    /* Allocate usage bitmap */
    shr_bitdcl_size = SHR_BITALLOCSIZE(num_selectors);
    pol_offset_int_f_bitmap =
        sal_alloc(shr_bitdcl_size, "pol_offset_int_f_bitmap");
    if (NULL == pol_offset_int_f_bitmap) {
        LOG_ERROR(BSL_LS_BCM_TSN,
                  (BSL_META_U(unit,
    ...
    sal_memset(pol_offset_int_f_bitmap, 0, shr_bitdcl_size);

    /* Loop all selectors */
    for (sel = 0; sel < num_selectors; sel++) {

        /* make sure the attr bits selector is 0 */
        pkt_attr_selectors->pkt_attr_v[sel].attr_bits_selector = 0;

        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "pkt_attr_v [sel] %d:\n"
                              "pol_offset %d, flags %d \n"),
                              sel,
                              attr_selectors[sel].policer_offset,
                              attr_selectors[sel].flags));
        /* Final Offset */
        pol_offset = attr_selectors[sel].policer_offset;

        /* Record used Policer Offset */
        if (!POL_OFFSET_INTF_GET(pol_offset)) {
                /* new Policer Offset found,
                 * the same policer offset means the same policer
                 */
            POL_OFFSET_INTF_SET(pol_offset);
            acutal_total_policers++;
        }

        /* Record Maximum Policer Offset */
        if (max_pol_offset < pol_offset) {
            max_pol_offset = pol_offset;
        }

        /* Fill attributes */
        if (BCM_POLICER_ATTR_MULTI & attr_selectors[sel].flags) {
            POL_FUNCTION_DEBUG(
                unit, "flag 0x%x, Multiple attributes for TCAM\n",
                attr_selectors[sel].flags);
            /* Multiple attributes for TCAM */
            for (mul_sel = 0;
                 mul_sel < attr_selectors[sel].multi_attr_count;
                 mul_sel++) {

                attr = attr_selectors[sel].multi_attr[mul_sel];
                attr_value =
                    attr_selectors[sel].multi_attr_value[mul_sel];
                attr_value_mask =
                    attr_selectors[sel].multi_attr_value_mask[mul_sel];

                /* Copy to pkt_attr_selectors */
                bcmi_esw_policer_custom_group_mode_fillup_tcam_values(
                    unit,
                    pol_offset,
                    attr,
            ...
            POL_FUNCTION_DEBUG(
                unit, "flag 0x%x, Single attribute for TCAM\n",
                attr_selectors[sel].flags);
            /* Single attribute for TCAM */
            attr = attr_selectors[sel].attr;
            attr_value = attr_selectors[sel].attr_value;
            attr_value_mask = attr_selectors[sel].attr_value_mask;
        ...
        (void)bcmi_policer_tcam_attr_dump(
                unit, "pkt_attr_v", &pkt_attr_selectors->pkt_attr_v[sel]);
    }

    /* Give pkt_attr_v(TCAM entries) number */
    pkt_attr_selectors->pkt_attr_v_cnt = num_selectors;

    /* Check if total_policers equal to acutal_total_policers */
    if (total_policers != acutal_total_policers) {
        POL_MEM_FREE(pol_offset_int_f_bitmap);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "total_policers(%d) should equal to "
                              "acutal_total_policers(%d)\n"),
                              total_policers,
                              acutal_total_policers));
        return BCM_E_PARAM;
    }

    /* Check Maximum Policer Offset match calculated total policer # */
    if (max_pol_offset != acutal_total_policers - 1) {
        POL_MEM_FREE(pol_offset_int_f_bitmap);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Maximum policer offset(%d) should equal to"
                              " acutal_total_policers(%d) - 1.\n"
                              "Please check 'policer_offset' of "
    ...
    /* Give total policer number */
    pkt_attr_selectors->total_policers = total_policers;

    POL_FUNCTION_DEBUG(unit, "total_policers %d", total_policers);
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_mode_id_existed_check
 * Purpose:
 ...
 *     num_selectors         - (IN) number of selectors
 *     attr_selectors        - (IN) list of attributes selectors
 *     mode_id               - (OUT) pointer to meter mode_id
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    uint32 *mode_id)
{
    ...
    /* No valid mode_id */
    if (bk_svm_info->valid_offset_mode_cnt == 0) {
        POL_FUNCTION_TRACE(unit, "OUT No valid mode_id");
        return BCM_E_NONE;
    }

    /* Loop through all valid mode_id and try to find a match one */
    for (offset_mode_idx = bk_svm_info->valid_offset_mode_start;
         offset_mode_idx <= bk_svm_info->valid_offset_mode_end;
         offset_mode_idx++) {

        if (!BCMI_OFFSET_MODE_ID_INTF_GET(unit, offset_mode_idx)) {
            continue;
        }

        bk_svm_mode_info = BCMI_SVM_OFFSET_MODE_BK_GET(unit, offset_mode_idx);
        if (bk_svm_mode_info->group_mode_type != group_mode_type) {
            continue;
        }

        /* Check num selectors */
        if (bk_svm_mode_info->no_of_attr_selectors == num_selectors) {
            if (bk_svm_mode_info->attr_selectors != NULL) {
                if (sal_memcmp(&attr_selectors[0],
                               bk_svm_mode_info->attr_selectors,
                               sizeof(bcm_policer_group_mode_attr_selector_t) *
                               num_selectors) == 0) {
                    *mode_id = OFFSET_MODE_TO_MODE_ID(offset_mode_idx);
                    LOG_ERROR(BSL_LS_BCM_POLICER,
                              (BSL_META_U(unit,
                                          "Same attr_selectors existed")));
                    return BCM_E_EXISTS;
                }
            }
        }
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_mode_id_info_update
 * Purpose:
 *     update offset mode info
 * Parameters:
 *     unit                  - (IN) unit number
 *     mode_id               - (IN) meter mode_id
 *     group_mode_type       - (IN) type of group mode
 *     num_selectors         - (IN) number of selectors
 *     attr_selectors        - (IN) list of attributes selectors
 *     pkt_attr_selectors    - (IN) tcam packet attribute selectors
...
bcmi_esw_policer_custom_group_mode_id_info_update(
    int unit,
    bcm_policer_svc_meter_mode_t mode_id,
    bcm_policer_group_mode_type_t group_mode_type,
    int num_selectors,
    bcm_policer_group_mode_attr_selector_t *attr_selectors,
    bcmi_policer_tcam_attr_selectors_t *pkt_attr_selectors)
{
    ...
    total_policers = pkt_attr_selectors->total_policers;

    bk_svm_mode_info->no_of_policers = total_policers;
    bk_svm_mode_info->is_custom_group = TRUE;
    bk_svm_mode_info->group_mode_type = group_mode_type;

    POL_FUNCTION_DEBUG(unit, "mode_id %d", mode_id);
    POL_FUNCTION_DEBUG(unit, "offset_mode %d", offset_mode);
    POL_FUNCTION_DEBUG(unit, "group_mode_type %d", group_mode_type);
    POL_FUNCTION_DEBUG(unit, "num_selectors %d", num_selectors);
    POL_FUNCTION_DEBUG(unit, "total_policers %d", total_policers);

    /* Store bcm_policer_group_mode_attr_selector_t start */

    /* Free memory */
    POL_MEM_FREE(bk_svm_mode_info->attr_selectors);

    bk_svm_mode_info->no_of_attr_selectors = num_selectors;

    POL_MEM_ALLOC(
        bk_svm_mode_info->attr_selectors,
        num_selectors * sizeof(bcm_policer_group_mode_attr_selector_t),
        "attr selectors");
    if (NULL == bk_svm_mode_info->attr_selectors) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to allocate memory for "
                              "attr_selectors\n")));
    ...
    sal_memcpy(bk_svm_mode_info->attr_selectors,
               attr_selectors,
               sizeof(bcm_policer_group_mode_attr_selector_t) * num_selectors);
    /* Store bcm_policer_group_mode_attr_selector_t end */

    /* Store pkt_tcam_attr_bits_t start */
    POL_MEM_FREE(bk_svm_mode_info->pkt_attr_v);
    ...
    /* Configured TCAM HW with pkt_attr_selectors */
    BCM_IF_ERROR_RETURN(
        bcmi_policer_tcam_configurate_hw(unit, pkt_attr_selectors));

    /* Store pkt_tcam_attr_bits_t end */
    POL_FUNCTION_TRACE(unit, "OUT");
    return BCM_E_NONE;
}
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_mode_id_create
 * Purpose:
 *     This function is used to configure customized group mode
 * Parameters:
 *     unit                  - (IN) unit number
 *     flags                 - (IN) flags
 *     group_mode_type       - (IN) type of group mode
    ...
    uint32 *mode_id)
{
    ...
    rv = bcmi_esw_policer_custom_group_mode_id_existed_check(
            unit, flags, group_mode_type,
            num_selectors, attr_selectors, mode_id);
    BCM_IF_ERROR_RETURN(rv);

    (void)bcmi_policer_tcam_attr_selectors_t_init(&pkt_attr_selectors);
    POL_MEM_ALLOC(
        pkt_attr_selectors.pkt_attr_v,
        sizeof(pkt_tcam_attr_bits_t) * BCMI_POLICER_SVC_METER_TCAM_SIZE,
        "allocate pkt_attr_v");

    /* Create temportary pkt information from selectors */
    rv = bcmi_esw_policer_custom_group_mode_fillup_values(
            unit, flags, group_mode_type,
            total_policers, num_selectors, attr_selectors,
            &pkt_attr_selectors);
    if (BCM_FAILURE(rv)) {
        POL_MEM_FREE(pkt_attr_selectors.pkt_attr_v);
        return rv;
    }

    GLOBAL_METER_LOCK(unit);
    /* Add a new mode_id associated with pkt_attr_selectors information */
    rv = bcmi_esw_policer_group_mode_id_associate(unit,
            flags, group_mode_type, &pkt_attr_selectors, mode_id);
    if (BCM_FAILURE(rv)) {
        POL_MEM_FREE(pkt_attr_selectors.pkt_attr_v);
        GLOBAL_METER_UNLOCK(unit);
        return rv;
    ...
    /* Update above pkt information to bookkeeping */
    rv = bcmi_esw_policer_custom_group_mode_id_info_update(
            unit, *mode_id, group_mode_type, num_selectors,
            attr_selectors, &pkt_attr_selectors);
    if (BCM_FAILURE(rv)) {
        bcmi_esw_policer_group_mode_id_delete(unit, *mode_id);
        POL_MEM_FREE(pkt_attr_selectors.pkt_attr_v);
        GLOBAL_METER_UNLOCK(unit);
        return rv;
    }
...
/*
 * Function:
 *     bcmi_esw_policer_group_mode_id_existed_check
 * Purpose:
 *     Check if given attribute selectors list is already
 *     configured or not
 * Parameters:
 ...
 *     npolicers             - (IN) Number of policers
 *     pkt_attr_selectors    - (IN) tcam packet attribute selectors
 *     mode_id               - (OUT) pointer to found existed mode_id
 *     existed               - (OUT) existed or not existed
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    int *existed)
{
    ...
    POL_CHECK_NULL_PARAMETER(unit, pkt_attr_selectors);
    POL_CHECK_NULL_PARAMETER(unit, mode_id);
    POL_CHECK_NULL_PARAMETER(unit, existed);

    POL_FUNCTION_DEBUG(unit, "group_mode %d", group_mode);
    POL_FUNCTION_DEBUG(unit, "group_mode %s",
        bcm_policer_group_mode_t_str[group_mode]);
    POL_FUNCTION_DEBUG(unit, "npolicers %d", npolicers);
    ...
    *existed = FALSE;
    ...
    /* No valid mode_id */
    if (bk_svm_info->valid_offset_mode_cnt == 0) {
        /* no exsiting mode_id, thus don't need to check */
        POL_FUNCTION_TRACE(unit, "OUT");
        return BCM_E_NONE;
    }
    ...
    for (offset_mode_idx = bk_svm_info->valid_offset_mode_start;
        ...
        bk_svm_mode_info = BCMI_SVM_OFFSET_MODE_BK_GET(unit, offset_mode_idx);
        if (bk_svm_mode_info->group_mode != group_mode) {
            continue;
        }
        if (bk_svm_mode_info->no_of_policers != npolicers) {
            continue;
        }
        *mode_id = OFFSET_MODE_TO_MODE_ID(offset_mode_idx);
        *existed = TRUE;
...
/*
 * Function:
 *     bcmi_esw_policer_group_mode_id_create
 * Purpose:
 *     Create Customized Policer Group mode for given Policer Attributes
 * Parameters:
 *     unit                  - (IN) unit number
 *     flags                 - (IN) flags
 *     group_mode_type       - (IN) modes of policer group creation -
    ...
    uint32 *mode_id)
{
    ...
    if (group_mode_type >= bcmPolicerGroupModeTypeCount) {
        return (BCM_E_PARAM);
    }
    ...
    if ((group_mode_type == bcmPolicerGroupModeTypeNormal) &&
        (num_selectors > BCMI_POLICER_SVC_METER_TCAM_SIZE)) {
        return (BCM_E_PARAM);
    }

    rv = bcmi_esw_policer_custom_group_mode_id_create(
            unit, flags, group_mode_type,
    ...
    POL_FUNCTION_DEBUG(unit,
        "Created policer group with mode id %x \n", *mode_id);
    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
}
...
/*
 * Function:
 *     bcmi_esw_policer_group_mode_id_get
 * Purpose:
 ...
 *     num_selectors         - (IN) Number of Selectors for policer Group Mode
 *     flags                 - (OUT) flags
 *     group_mode_type       - (OUT) modes of policer group creation -
 *                                   normal/cascade/cascade with coupling.
    ...
    uint32 *actual_num_selectors)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, flags);
    POL_CHECK_NULL_PARAMETER(unit, group_mode_type);
    POL_CHECK_NULL_PARAMETER(unit, total_policers);
    POL_CHECK_NULL_PARAMETER(unit, attr_selectors);
    POL_CHECK_NULL_PARAMETER(unit, actual_num_selectors);
    ...
    /* Validate mode_id */
    BCM_IF_ERROR_RETURN(
        bcmi_policer_mode_id_validate(unit, mode_id));

    offset_mode = MODE_ID_TO_OFFSET_MODE(mode_id);
    bk_svm_mode_info = BCMI_SVM_OFFSET_MODE_BK_GET(unit, offset_mode);

    *total_policers = bk_svm_mode_info->no_of_policers;
    *actual_num_selectors = bk_svm_mode_info->no_of_attr_selectors;
    *group_mode_type = bk_svm_mode_info->group_mode_type;

    if (*actual_num_selectors > num_selectors) {
        return (BCM_E_PARAM);
    }

    for (i = 0; i < *actual_num_selectors; i++) {
        sal_memcpy (&attr_selectors[i],
                    &bk_svm_mode_info->attr_selectors[i],
                    sizeof(bcm_policer_group_mode_attr_selector_t));
    }
    POL_FUNCTION_TRACE(unit, "OUT");
    return BCM_E_NONE;
...
/*
 * Function:
 *   bcmi_esw_policer_group_mode_id_destroy
 * Purpose:
 *   Destroy Customized Policer Group mode with given mode_id.
 * Parameters:
 *   unit     - (IN) unit number
 *   mode_id  - (IN) Destroy Policer Group Mode with given mode Id
 * Returns:
 *   BCM_E_NONE - Success on destroying the given mode_id
 */
STATIC int
    ...
    uint32 mode_id)
{
    ...
    rv = bcmi_esw_policer_group_mode_id_delete(unit, mode_id);

    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
}

#endif /* MODE_ID_API_SECTION */

#if POLICER_ID_API_SECTION
/*
 * Function:
 *     global_meter_policer_id_alloc
 * Purpose:
 ...
 */
STATIC int
bcmi_global_meter_policer_id_alloc(
    int unit,
    int numbers,
    bcm_policer_t *pid)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, pid);
    ...
    rv = shr_aidxres_list_alloc_block(
            BCMI_POLICER_LIST_HANDLE_GET(unit),
            numbers,
            (uint32 *)pid);
    BCM_IF_ERROR_RETURN(rv);

    POL_FUNCTION_DEBUG(
        unit, "allocated %d policers\n", numbers);
    POL_FUNCTION_DEBUG(
        unit, "allocated policer with pid %d", *pid);
...
/*
 * Function:
 *     bcmi_global_meter_free_allocated_policer_on_error
 * Purpose:
    ...
    uint32 policer_index)
{
    ...
    for (index = 0;index < numbers; index++) {
        base_index = policer_index + index;
...
/*
 * Function:
 *     bcmi_global_meter_policer_control_alloc
 * Purpose:
 *     Allocate a new bcmi_global_meter_policer_control_t.
 * Parameters:
 *     unit                  - (IN) unit number
 *     npolicers             - (IN) Number of policers created
    ...
    bcmi_global_meter_policer_control_t **policer_control)
{
    ...
    POL_CHECK_NULL_PARAMETER(unit, policer_control);

    POL_MEM_ALLOC(
        *policer_control,
        sizeof(bcmi_global_meter_policer_control_t),
        "Global meter policer");
    if (NULL == (*policer_control)) {
        return (BCM_E_MEMORY);
    }
    (void)bcmi_global_meter_policer_control_t_init(*policer_control);

    POL_MEM_ALLOC(
        (*policer_control)->action_id,
        sizeof(uint32) * npolicers,
        "actions list of Global meter policer");

    if (NULL == (*policer_control)->action_id) {
        POL_MEM_FREE(*policer_control);
        return (BCM_E_MEMORY);
    }
    POL_FUNCTION_TRACE(unit, "OUT");
    return BCM_E_NONE;
...
/*
 * Function:
 *     bcmi_policer_svc_meter_create_mode
 * Purpose:
...
/*
 * Function:
 *     bcmi_esw_policer_svc_meter_create_mode
 * Purpose:
 *     Function to create an Meter offset mode
 * Parameters:
 *     Unit                  - (IN) unit number
 *     pkt_attr_selectors    - (IN) Packet attribute selectors
 *     group_mode            - (IN) Policer group mode
 *     group_mode_type       - (IN) Policer group mode type
 *     npolicers             - (IN) Number of policers created
 *     mode_id               - (OUT) Meter mode id
    ...
    uint32 *mode_id)
{
    ...
    /* Check if mode info already exists */
    rv = bcmi_esw_policer_group_mode_id_existed_check(
            unit, group_mode, npolicers,
            pkt_attr_selectors, mode_id, &existed);
    BCM_IF_ERROR_RETURN(rv);

    if (existed) {
        POL_FUNCTION_DEBUG(
            unit, "mode_id %d already existed", *mode_id);
    } else {
        rv = bcmi_policer_svc_meter_create_mode(
                unit, pkt_attr_selectors, group_mode,
                group_mode_type, mode_id);
        BCM_IF_ERROR_RETURN(rv);
        offset_mode = MODE_ID_TO_OFFSET_MODE(*mode_id);

        rv = bcmi_policer_svc_meter_reserve_mode(
                unit, offset_mode, group_mode,
                group_mode_type, npolicers, pkt_attr_selectors);
        BCM_IF_ERROR_RETURN(rv);
    }
...
/*
 * Function:
 *      bcmi_esw_get_policer_id_from_index_offset
 * Purpose:
    ...
    bcm_policer_t *pid)
{
    ...
    if (index == 0) {
        LOG_WARN(BSL_LS_BCM_POLICER,
                 (BSL_META_U(unit,
                             "0 is reserved invalid table index\n")));
    } else {
        *pid = POLICER_IDX_TO_POLICER_ID(offset_mode, index);
    }
...
/*
 * Function:
 *     bcmi_esw_policer_table_index_get
 * Purpose:
    ...
    int *index)
{
    ...
    /* Input parameters check. */
    POL_CHECK_NULL_PARAMETER(unit, index);

    *index = POLICER_ID_TO_POLICER_IDX(policer_id);

    if (*index >= soc_mem_index_count(unit, SVM_METER_TABLEm)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Invalid table index for "
                                "SVM_METER_TABLE\n")));
        return BCM_E_PARAM;
    }
...
/*
 * Function:
 *     bcmi_esw_policer_offset_mode_get
 * Purpose:
 *     Get the offset_mode with given a policer id.
 * Parameters:
 *     Unit                  - (IN) unit number
 *     policer_id            - (IN) policer Id
 *     offset_mode                 - (OUT) HW policer index
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    int *offset_mode)
{
    ...
    /* Input parameters check. */
    POL_CHECK_NULL_PARAMETER(unit, offset_mode);

    *offset_mode = POLICER_ID_OFFSET_MODE_GET(policer_id);
...
STATIC int
bcmi_esw_global_meter_base_policer_get(
    int unit,
    bcm_policer_t policer_id,
    bcmi_global_meter_policer_control_t **policer_p);

/*
 * Function:
 *     bcmi_esw_policer_validate
 * Purpose:
    ...
    bcm_policer_t policer_id)
{
    ...
    int index = 0;
    uint32 offset_mode = 0;
    bcmi_global_meter_policer_control_t *policer_control = NULL;
    ...
    /* valid the policer id is create in current SW database or not */
    rv = bcmi_esw_global_meter_base_policer_get(unit, policer_id, &policer_control);
    BCM_IF_ERROR_RETURN(rv);

    /* get the HW index for the policer */
    rv = bcmi_esw_policer_table_index_get(unit, policer_id, &index);
    BCM_IF_ERROR_RETURN(rv);
    ...
    if (index == 0) {
        /* Default HW value will be 0 and it's not a valid table index */
        return BCM_E_PARAM;
    }

    /* Get mode id from policer id */
    offset_mode = POLICER_ID_OFFSET_MODE_GET(policer_id);
...
/*
 * Function:
 *      bcmi_esw_policer_increment_ref_count
 * Purpose:
    ...
    bcm_policer_t policer_id)
{
    ...
    /* validate policer id */
    rv = bcmi_esw_policer_validate(unit, policer_id);
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
...
/*
 * Function:
 *      bcmi_esw_policer_decrement_ref_count
 * Purpose:
...
/*
 * Function:
 *     bcmi_esw_get_policer_from_table
 * Purpose:
    ...
    int skip_read)
{
    bcm_error_t rv = BCM_E_UNAVAIL;
    const bcmi_global_meter_dev_info_t *gm_dev_info = NULL;
    int meter_index, meter_offset_mode;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Input parameters check. */
    POL_CHECK_NULL_PARAMETER(unit, data);
    POL_CHECK_NULL_PARAMETER(unit, policer_id);
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policer_to_table_get);
    rv = gm_dev_info->policer_to_table_get(unit,
                                           table,
                                           index,
                                           data,
                                           &meter_index,
    ...
    *policer_id = POLICER_IDX_TO_POLICER_ID(meter_offset_mode, meter_index);
 ...
 */
STATIC int
bcmi_esw_add_policer_to_table(int unit,
    bcm_policer_t policer_id,
    soc_mem_t table,
    int index,
    void *data)
{
    ...
    rv = bcmi_esw_get_policer_from_table(unit, table, index, data,
                                         &current_policer_id, skip_read);
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
    ...
    /* Set policer id */
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_table_index_get(unit, policer_id, &meter_index));
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_offset_mode_get(unit, policer_id, &meter_offset_mode));

    GLOBAL_METER_LOCK(unit);

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policer_to_table_set);
    rv = gm_dev_info->policer_to_table_set(unit,
                                           table,
                                           index,
                                           data,
                                           meter_index,
    ...
    rv = bcmi_esw_policer_increment_ref_count(unit, policer_id);
    BCM_IF_ERROR_RETURN(rv);

    /* decrement current hw policer ref_cnt */
    rv = bcmi_esw_policer_validate(unit, current_policer_id);

    /* Don't need to handle fail case since existed index could be 0 */
    if (BCM_SUCCESS(rv)) {
        rv = bcmi_esw_policer_decrement_ref_count(unit, current_policer_id);
        BCM_IF_ERROR_RETURN(rv);
    }
...
/*
 * Function:
 *     bcmi_esw_delete_policer_from_table
 * Purpose:
 ...
 *     BCM_E_XXX
 */
int bcmi_esw_delete_policer_from_table(
    int unit,
    bcm_policer_t policer_id,
    int table,
    int index,
    void *data)
{
    ...
    if (current_policer_id != policer_id) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Policer Id passed is different from the "
                              "one that is configured in the table."
    ...
    rv = bcmi_esw_policer_decrement_ref_count(unit, policer_id);
    BCM_IF_ERROR_RETURN(rv);
...
/*
 * Function:
 *     bcmi_esw_global_meter_read_config_from_hw
 * Purpose:
    ...
    bcm_policer_config_t *pol_cfg)
{
    ...
    uint32 pkt_bytes = 0, tsn_modified = 0, average_pkt_size = 0;
    uint32 flags = 0;
    int index = 0, e_idx, c_idx, p_idx;
    uint32 meter_gran_mode = 0;
    uint32 pkbits_sec, pkbits_burst, ckbits_sec, ckbits_burst;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Input parameters check. */
    POL_CHECK_NULL_PARAMETER(unit, pol_cfg);

    (void)bcm_policer_config_t_init(pol_cfg);
    ...
    /* get the HW index for the policer */
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_table_index_get(unit, policer_id, &index));

    /* Policy Table */
    p_idx = index ;

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->meter_table_get);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_mode_get);

    rv = gm_dev_info->policy_mode_get(
                        unit,
                        p_idx,
                        &pol_cfg->mode,
                        &pol_cfg->flags);
    ...
    rv = gm_dev_info->meter_table_get(
                        unit,
                        c_idx,
                        &bucket_size,
                        &bucket_count,
    ...
    if (tsn_modified == 1) {
        pol_cfg->mode = bcmPolicerModeSrTcmTsn;
    }
    pol_cfg->average_pkt_size = average_pkt_size;
    ...
    rv = _bcm_xgs_bucket_encoding_to_kbits(
            refresh_rate, bucket_size, granularity, flags,
            &ckbits_sec, &ckbits_burst);
    ...
    /* ckbits_sec = (pol_cfg->ckbits_sec << 8) + (pol_cfg->cbits_sec_lower << 8) / 1000 */
    if (meter_gran_mode) {
        pol_cfg->ckbits_sec =
            ckbits_sec >> BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->cbits_sec_lower =
            (ckbits_sec * BCMI_POLICER_METER_GRAN_LOWER_UNIT) >>
             BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->cbits_sec_lower -=
            (pol_cfg->ckbits_sec * BCMI_POLICER_METER_GRAN_LOWER_UNIT);

        pol_cfg->ckbits_burst =
            ckbits_burst >> BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->cbits_burst_lower =
            (ckbits_burst * BCMI_POLICER_METER_GRAN_LOWER_UNIT) >>
             BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->cbits_burst_lower -=
            (pol_cfg->ckbits_burst * BCMI_POLICER_METER_GRAN_LOWER_UNIT);
    } else {
    ...
    /* pkbit_sec = (pol_cfg->pkbits_sec << 8) + (pol_cfg->pbits_sec_lower << 8) / 1000 */
    if (meter_gran_mode) {
        pol_cfg->pkbits_sec =
            pkbits_sec >> BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->pbits_sec_lower =
            (pkbits_sec * BCMI_POLICER_METER_GRAN_LOWER_UNIT) >>
             BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->pbits_sec_lower -=
            (pol_cfg->pkbits_sec * BCMI_POLICER_METER_GRAN_LOWER_UNIT);

        pol_cfg->pkbits_burst =
            pkbits_burst >> BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->pbits_burst_lower =
            (pkbits_burst * BCMI_POLICER_METER_GRAN_LOWER_UNIT) >>
             BCMI_POLICER_METER_GRAN_SHFIT;
        pol_cfg->pbits_burst_lower -=
            (pol_cfg->pkbits_burst * BCMI_POLICER_METER_GRAN_LOWER_UNIT);
    } else {
        pol_cfg->pkbits_sec = pkbits_sec;
        pol_cfg->pkbits_burst = pkbits_burst;
    }
...
/*
 * Function:
 *     bcmi_esw_global_meter_clear_config_on_hw
 * Purpose:
 *     Clear policer configuration to meter table.
 * Parameters:
 *     unit                  - (IN) unit number
    ...
    bcm_policer_t policer_id)
{
    ...
    uint32 granularity = 0;     /* Policer granularity */
    uint32 average_pkt_size = 0;
    uint32 c_bucketcount = 0, e_bucketcount;
    int index = 0, e_idx, c_idx, p_idx;
    bcm_policer_config_t pol_cfg;
    uint32 meter_gran_mode = 0;
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->meter_table_set);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_mode_set);
    ...
    /* Write to HW */
    rv = gm_dev_info->meter_table_set(
                        unit,
                        c_idx,
                        pol_cfg.mode,
                        pol_cfg.flags,
    ...
    rv = gm_dev_info->policy_mode_set(
                        unit,
                        p_idx,
                        pol_cfg.mode,
                        pol_cfg.flags);
...
/*
 * Function:
 *      bcmi_esw_global_meter_write_config_to_hw
 * Purpose:
 *      Write policer configuration to meter table.
 * Parameters:
 *     unit                  - (IN) unit number
 *     pol_cfg               - (IN) policer configuration
 *     policer_id            - (IN) policer id
    ...
    bcm_policer_t policer_id)
{
    ...
    /*
     * Gran   Meter  Bytes       Min           Max
     * mode   Gran   /Token   Refresh Rate MaxRefresh Rate
     *  1      0    1/2^15      32bps         8Mbps
     *         1    1/2^14      64bps        16Mbps
     *         2    1/2^13     128bps        32Mbps
     *         3    1/2^12     256bps        64Mbps
     *         4    1/2^11     512bps       128Mbps
     *         5    1/2^10      1Kbps       256Mbps
     *         6    1/2^9       2Kbps       512Mbps
     *         7    1/2^8       4Kbps         1Gbps
     * Gran   Meter  Bytes       Min           Max
     * mode   Gran   /Token   Refresh Rate MaxRefresh Rate
     *  0      0    1/2^7       8Kbps         2Gbps
     *         1    1/2^6      16Kbps         4Gbps
     *         2    1/2^5      32Kbps         8Gbps
     *         3    1/2^4      64Kbps        16Gbps
     *         4    1/2^3     128Kbps        32Gbps
     *         5    1/2^2     256Kbps        64Gbps
     *         6    1/2^1     512Kbps       128Gbps
     *         7    1/2^0    1024Kbps       256Gbps
     */

    /* pkbit_sec = (pol_cfg->pkbits_sec << 8) + (pol_cfg->pbits_sec_lower << 8) / 1000 */
    if (pol_cfg->pkbits_sec < BCMI_POLICER_METER_FINER_GRAN_RATE_BOUNDARY &&
        pol_cfg->pkbits_burst < BCMI_POLICER_METER_FINER_GRAN_BURST_BOUNDARY &&
        pol_cfg->ckbits_sec < BCMI_POLICER_METER_FINER_GRAN_RATE_BOUNDARY &&
        pol_cfg->ckbits_burst < BCMI_POLICER_METER_FINER_GRAN_BURST_BOUNDARY) {
        pkbits_sec =
            (pol_cfg->pkbits_sec << BCMI_POLICER_METER_GRAN_SHFIT) +
            (pol_cfg->pbits_sec_lower << BCMI_POLICER_METER_GRAN_SHFIT) /
             BCMI_POLICER_METER_GRAN_LOWER_UNIT;
        pkbits_burst =
            (pol_cfg->pkbits_burst << BCMI_POLICER_METER_GRAN_SHFIT) +
            (pol_cfg->pbits_burst_lower << BCMI_POLICER_METER_GRAN_SHFIT) /
             BCMI_POLICER_METER_GRAN_LOWER_UNIT;
        ckbits_sec =
            (pol_cfg->ckbits_sec << BCMI_POLICER_METER_GRAN_SHFIT) +
            (pol_cfg->cbits_sec_lower << BCMI_POLICER_METER_GRAN_SHFIT) /
             BCMI_POLICER_METER_GRAN_LOWER_UNIT;
        ckbits_burst =
            (pol_cfg->ckbits_burst << BCMI_POLICER_METER_GRAN_SHFIT) +
            (pol_cfg->cbits_burst_lower << BCMI_POLICER_METER_GRAN_SHFIT) /
             BCMI_POLICER_METER_GRAN_LOWER_UNIT;

        meter_gran_mode = 1;
    } else {
        pkbits_sec = pol_cfg->pkbits_sec;
        pkbits_burst = pol_cfg->pkbits_burst;
        ckbits_sec = pol_cfg->ckbits_sec;
        ckbits_burst = pol_cfg->ckbits_burst;
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->meter_table_bitsize_get);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->meter_table_set);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_mode_set);

    /* Read from HW */
    rv = gm_dev_info->meter_table_bitsize_get(
                        unit,
                        c_idx,
                        &refresh_bitsize,
                        &bucket_max_bitsize,
    ...
    if (pkbits_sec > ckbits_sec) {
        rv = _bcm_xgs_kbits_to_bucket_encoding(pkbits_sec,
                                               pkbits_burst,
        ...
        BCM_IF_ERROR_RETURN(rv);
         ...
         rv = _bcm_xgs_kbits_to_bucket_encoding(ckbits_sec,
                                                ckbits_burst,
                                                flags, refresh_bitsize,
                                                bucket_max_bitsize,
                                                &c_refresh_rate,
    ...
    if ((ckbits_sec == 0) && (ckbits_burst == 0)) {
        rv = _bcm_xgs_kbits_to_bucket_encoding(pkbits_sec,
                                               pkbits_burst,
                                               flags, refresh_bitsize,
                                               bucket_max_bitsize,
    ...
    bcmi_global_meter_policer_control_t **policer_p)
{
    ...
    /* Input parameters check. */
    POL_CHECK_NULL_PARAMETER(unit, policer_p);

    /* Initialization check */
    CHECK_GLOBAL_METER_INIT(unit);
    CHECK_GLOBAL_METER_DEV_INFO_INIT(unit);

    hash_max = policer_id & BCMI_GLOBAL_METER_HASH_MASK;
    for (hash_index = 0; hash_index <= hash_max; hash_index++) {
        policer_control = BCMI_POLICER_HASH_BK_GET(unit, hash_index);
        while (NULL != policer_control) {
            /* Match entry id. */
            if (policer_id >= policer_control->pid &&
                policer_id < policer_control->pid + policer_control->no_of_policers) {
                *policer_p = policer_control;
                POL_FUNCTION_TRACE(unit, "OUT found");
                return (BCM_E_NONE);
            }
            policer_control = policer_control->next;
        }
    }
    /* Policer with policer_id was not found. */
    POL_FUNCTION_TRACE(unit, "OUT not found");
    return (BCM_E_NOT_FOUND);
}

int _bcm_global_meter_min_burst_size_set(
    bcm_policer_config_t *pol_cfg);

STATIC int
bcmi_esw_policer_action_attach(
    int unit,
    bcm_policer_t policer_id,
    uint32 action_id);

STATIC int
bcmi_esw_policer_action_attach_get(
    int unit,
    bcm_policer_t policer_id,
    uint32 *action_id);
...
/*
 * Function:
 *      bcmi_esw_global_meter_policer_set
 * Purpose:
    ...
    bcm_policer_config_t *pol_cfg)
{
    ...
    /* lower must be 0 ~ 999 */
    if (pol_cfg->cbits_sec_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT ||
        pol_cfg->cbits_burst_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT ||
        pol_cfg->pbits_sec_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT ||
        pol_cfg->pbits_burst_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT ||
        pol_cfg->max_cbits_sec_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT ||
        pol_cfg->max_pbits_sec_lower >= BCMI_POLICER_METER_GRAN_LOWER_UNIT) {
        return BCM_E_PARAM;
    }
    ...
    if (BCM_SUCCESS(rv)) {
       /* Check whether the burst size is >= min required. If not, set it
        ...
        rv = bcmi_esw_global_meter_write_config_to_hw(unit,
                pol_cfg, policer_id);
        if (BCM_FAILURE(rv)) {
            GLOBAL_METER_UNLOCK(unit);
            rv2 = bcmi_esw_global_meter_clear_config_on_hw(unit, policer_id);
    ...
    GLOBAL_METER_UNLOCK(unit);
    if (pol_cfg->action_id != BCMI_POLICER_GLOBAL_METER_INVALID_ACTION_ID) {
        rv = bcmi_esw_policer_action_attach(unit, policer_id, pol_cfg->action_id);
        BCM_IF_ERROR_RETURN(rv);
    }
...
/*
 * Function:
 *     bcmi_esw_global_meter_policer_get
 * Purpose:
 *     Get the configuration info for a given policer id
 * Parameters:
 *     unit                  - (IN) unit number
 *     policer_id            - (IN) policer id
 *     pol_cfg               - (OUT) Policer control info
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    bcm_policer_config_t *pol_cfg)
{
    ...
    GLOBAL_METER_LOCK(unit);
    rv = bcmi_esw_global_meter_read_config_from_hw(unit,
            policer_id, pol_cfg);
    GLOBAL_METER_UNLOCK(unit);
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
    ...
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_action_attach_get(unit, policer_id, &pol_cfg->action_id));
...
/*
 * Function:
 *     bcmi_esw_global_meter_policer_delete
 * Purpose:
 *     Internal function that destroys a policer and readjusts action
 *     usage reference count and policer allocation management data structure.
 * Parameters:
 *     unit                  - (IN) unit number
 *     policer_control       - (IN) policer control info
 * Returns:
    ...
    bcmi_global_meter_policer_control_t *policer_control)
{
    ...
    /* Decrease the reference count of actions. */
    for (idx = 0; idx < policer_control->no_of_policers; idx++) {
        if (BCMI_ACTION_ID_BK_GET(
            unit, policer_control->action_id[idx]).reference_count > 0)
        {
            BCMI_ACTION_ID_BK_GET(
            unit, policer_control->action_id[idx]).reference_count--;
        }
    }

    /* get the HW index for the policer to be deleted */
    rv = bcmi_esw_policer_table_index_get(
            unit, policer_control->pid, &policer_idx);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    bcmi_global_meter_free_allocated_policer_on_error(
        unit, numbers, policer_idx);

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->mem_range_clear);

    /* Clear SVM METER TABLE */
    rv = gm_dev_info->mem_range_clear(unit, policer_idx, numbers);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
    }

    /* Delete policer from policers hash. */
    _GLOBAL_METER_HASH_REMOVE(
        BCMI_POLICER_BK_GET(unit), bcmi_global_meter_policer_control_t,
        policer_control, (policer_control->pid & BCMI_GLOBAL_METER_HASH_MASK));

    /* decrement mode reference count  */
    bcmi_policer_svc_meter_dec_mode_ref_cnt(unit, offset_mode);
    GLOBAL_METER_UNLOCK(unit);

    /* De-allocate policer descriptor. */
    POL_MEM_FREE(policer_control->action_id);
    POL_MEM_FREE(policer_control);
...
/*
 * Function:
 *     bcmi_esw_global_meter_policer_destroy
 * Purpose:
    ...
    bcm_policer_t policer_id)
{
    ...
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Invalid policer id %x(%d)\n"),
                                policer_id, policer_id));
        return (rv);
    }

    GLOBAL_METER_LOCK(unit);
    rv = bcmi_esw_global_meter_base_policer_get(unit,
            policer_id, &policer_control);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to get policer control for "
                              "policer id %x(%d)\n"),
                              policer_id,
                              policer_id));
        return (rv);
    }
    rv = bcmi_esw_global_meter_policer_delete(unit, policer_control);
    GLOBAL_METER_UNLOCK(unit);
...
/*
 * Function:
 *     bcmi_esw_global_meter_policer_destroy_all
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_global_meter_policer_destroy_all(int unit)
{
    int rv = BCM_E_NONE;
    int idx = 0;
    bcmi_global_meter_policer_control_t *policer_control = NULL;
    ...
    for (idx = 0; idx < BCMI_GLOBAL_METER_HASH_SIZE; idx++) {
        policer_control = BCMI_POLICER_HASH_BK_GET(unit, idx);
...
/*
 * Function:
 *     bcmi_esw_global_meter_policer_traverse
 * Purpose:
    ...
    void *user_data)
{
    ...
    /* Iterate over all hash buckets. */
    for (hash_index = 0; hash_index < BCMI_GLOBAL_METER_HASH_SIZE; hash_index++) {
        policer_control = BCMI_POLICER_HASH_BK_GET(unit, hash_index);
        /* Iterate all entries in each bucket. */
        while (NULL != policer_control) {
            rv = bcmi_esw_global_meter_policer_get(unit,
                    policer_control->pid, &cfg);
            if (BCM_FAILURE(rv)) {
                break;
            }
...
/*
 * Function:
 *     bcmi_esw_policer_envelop_create
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_policer_envelop_create(int unit,
    uint32 flags,
...
/*
 * Function:
 *      bcmi_esw_policer_envelop_group_create
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_policer_envelop_group_create(int unit,
    uint32 flags,
...
/*
 * Function:
 *     bcmi_esw_policer_custom_group_create
 * Purpose:
 *     Create a group of policers based on the mode.
 * Parameters:
 *     unit                  - (IN) unit number
    ...
    uint32 *npolicers)
{
    ...
    int policer_idx;
    bcmi_global_meter_policer_control_t *policer_control = NULL;
    bcm_policer_group_mode_t group_mode = bcmPolicerGroupModeSingle;

    POL_FUNCTION_TRACE(unit, "IN");

    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, policer_id);
    POL_CHECK_NULL_PARAMETER(unit, npolicers);

    POL_FUNCTION_DEBUG(unit, "flags %x", flags);
    POL_FUNCTION_DEBUG(unit, "mode_id %d", mode_id);
    POL_FUNCTION_DEBUG(unit, "macro_flow_policer_id %d",
    ...
    *npolicers = bk_svm_mode_info->no_of_policers;

    POL_FUNCTION_DEBUG(unit, "offset_mode %d", offset_mode);
    POL_FUNCTION_DEBUG(unit, "npolicers %d", *npolicers);

    GLOBAL_METER_LOCK(unit);
    rv = bcmi_global_meter_policer_id_alloc(
            unit, *npolicers, &policer_idx);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
    ...
    rv = bcmi_global_meter_policer_control_alloc(
            unit, *npolicers, &policer_control);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        bcmi_global_meter_free_allocated_policer_on_error(
            unit, *npolicers, policer_idx);
    ...
    *policer_id = POLICER_IDX_TO_POLICER_ID(offset_mode, policer_idx);

    /* Set policer configuration */
    policer_control->pid = *policer_id;
    policer_control->no_of_policers =  *npolicers;
    policer_control->grp_mode = group_mode;

    /* increment mode reference count for new policer  */
    rv = bcmi_policer_svc_meter_inc_mode_ref_cnt(unit, offset_mode);
    if (BCM_FAILURE(rv)) {
    ...
    _GLOBAL_METER_HASH_INSERT(BCMI_POLICER_BK_GET(unit),
                              policer_control,
                              (*policer_id & _GLOBAL_METER_HASH_MASK));

    GLOBAL_METER_UNLOCK(unit);
    POL_FUNCTION_DEBUG(unit, "policer_id 0x%x", *policer_id);
    POL_FUNCTION_TRACE(unit, "OUT");
    return rv;
}
    ...
    int *member_count)
{
    ...
    /* verify arguments, desired array elements to be retrived */
    if ((member_max > 0) && (member_array == NULL)) {
        return BCM_E_PARAM;
    }

    rv = bcmi_esw_policer_validate(unit, base_policer_id);
    BCM_IF_ERROR_RETURN(rv);

    GLOBAL_METER_LOCK(unit);
    rv = bcmi_esw_global_meter_base_policer_get(
            unit, base_policer_id, &policer_control);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    *member_count = policer_control->no_of_policers;
    ...
    if (member_max < *member_count) {
        indices_to_fill = member_max;
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
            "member_count(%d) of base_policer_id(%d)"
            "is large than the member_array size(%d)"),
            *member_count,
            base_policer_id,
            member_max));
    } else {
        indices_to_fill = *member_count;
    }
...
/*
 * Function:
 *     bcmi_esw_policer_group_set_mode_and_map
 * Purpose:
 *     create group set from given policer group mode and offset_map
 * Parameters:
 *     unit                  - (IN) unit number
 *     mode                  - (IN) Policer group mode
 *     mapping               - (IN) customized offset mapping
 *     npolicers             - (OUT) Number of policers
 *     pkt_attr_selectors    - (OUT) Packet attribute selectors
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
bcmi_esw_policer_group_set_mode_and_map(
    int unit,
    bcm_policer_group_mode_t group_mode,
    bcm_policer_map_t *mapping,
    int *npolicers,
    bcmi_policer_tcam_attr_selectors_t *pkt_attr_selectors)
{
    int rv = BCM_E_NONE;
    const bcmi_global_meter_dev_info_t *gm_dev_info = NULL;
    int count = 0;          /* Number of offset mappings */
    uint8 *final_offset = NULL;
    uint8 base_final_offset = 0;
    uint32 pkt_attr_pkt_type_cnt = 0, pkt_attr_v_cnt = 0;
    bcm_policer_group_mode_attr_t attr = bcmPolicerGroupModeAttrPktType;
    pkt_tcam_attr_bits_t *pkt_attr_v = NULL;
    int sel = 0;
    uint32 pol_offset = 0, max_pol_offset = 0;
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->group_mode_supported);

    /* Check if group mode is supported */
    rv = gm_dev_info->group_mode_supported(unit, group_mode);
    if (BCM_FAILURE(rv)) {
        /* dump all support group_mode */
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Supported group mode:\n")));
        for (group_mode = bcmPolicerGroupModeSingle;
             group_mode < bcmPolicerGroupModeCount;
             group_mode++) {
            rv = gm_dev_info->group_mode_supported(unit, group_mode);
            if (BCM_SUCCESS(rv)) {
                LOG_VERBOSE(BSL_LS_BCM_POLICER,
    ...
    /* Check group mode support user defined "mapping->offset_map" */
    switch (group_mode) {
        ...
        case bcmPolicerGroupModePreemptable:
        {
            /* Supported */
            if (mapping != NULL) {
                final_offset = mapping->offset_map;
                count = mapping->count;
            }
            break;
        }
        default:
        {
            /* Not Supported and mapping should be NULL */
            if (mapping != NULL) {
                LOG_ERROR(BSL_LS_BCM_POLICER,
                          (BSL_META_U(unit,
                    "group_mode(%d = %s) doesn't not support "
                    "flexible mapping of offset. offset_map should be NULL\n"),
                    group_mode,
                    bcm_policer_group_mode_t_str[group_mode]));
                return BCM_E_PARAM;
            }
    ...
    switch (group_mode) {
        ...
        case bcmPolicerGroupModeSingle:
        {
            *npolicers = 1;
            pkt_attr_v_cnt = 1;
            pkt_attr_v =
                &pkt_attr_selectors->pkt_attr_v[0];
            pkt_attr_v->attr_bits_selector = 1;
            break;
        }
        /********************************************************************/
        /* A dedicated policer per traffic type Unicast, multicast,         */
        /* broadcast                                                        */
        /* 1) KNOWN_L2UC_PKT | KNOWN_L3UC_PKT | UNKNOWN_L3UC_PKT            */
        /* 2) KNOWN_L2MC_PKT|UNKNOWN_L2MC_PKT|KNOWN_IPMC_PKT|               */
        /*     UNKNOWN_IPMC_PKT                                             */
        /* 3) L2BC_PKT|                                                     */
        /********************************************************************/
        case bcmPolicerGroupModeTrafficType:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
                /* 1) CONTROL_PKT|BPDU_PKT */
                {0, bcmPolicerGroupModeAttrPktTypeKnownL2UC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownL3UC},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownL3UC},
                /* 2) KNOWN_L2MC_PKT|UNKNOWN_L2MC_PKT|
                KNOWN_IPMC_PKT|UNKNOWN_IPMC_PKT */
                {1, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                /* 3) L2BC_PKT */
                {2, bcmPolicerGroupModeAttrPktTypeKnownL2UC}
            };

            *npolicers = 3;

            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 3 + 4 + 1;

            if (pkt_attr_v_cnt > BCMI_POLICER_SVC_METER_TCAM_SIZE) {
                return BCM_E_RESOURCE;
            }

            attr = bcmPolicerGroupModeAttrPktType;
            for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
                pkt_attr_v->value_mask[attr] =
                    BCMI_POLICER_GLOBAL_METER_ATTR_MASK_ALL;
                pkt_attr_v->value[attr] =
                    pkt_res_final_offset_v[sel].attr_pkt_type;
                pkt_attr_v->final_offset =
                    pkt_res_final_offset_v[sel].final_offset;
            }
            break;
        }
        /*****************************************************************/
        ...
        /*    UNKNOWN_L2MC_PKT|KNOWN_L3UC_PKT|                           */
        ...
        case bcmPolicerGroupModeDlfAll:
        {
            ...
            {
                /* 1) L2DLF_PKT(UNKNOWN_L2UC_PKT) */
                {0, bcmPolicerGroupModeAttrPktTypeUnknownL2UC},
                /* 2) */
                {1, bcmPolicerGroupModeAttrPktTypeUnknown},
                {1, bcmPolicerGroupModeAttrPktTypeControl},
                {1, bcmPolicerGroupModeAttrPktTypeBPDU},
                {1, bcmPolicerGroupModeAttrPktTypeL2BC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownL2UC},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownL3UC},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownL3UC},
                {1, bcmPolicerGroupModeAttrPktTypeKnownMpls},
                {1, bcmPolicerGroupModeAttrPktTypeKnownMplsL3},
                {1, bcmPolicerGroupModeAttrPktTypeKnownMplsL2},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownMpls},
                {1, bcmPolicerGroupModeAttrPktTypeKnownMim},
                {1, bcmPolicerGroupModeAttrPktTypeUnknownMim},
                {1, bcmPolicerGroupModeAttrPktTypeKnownMplsMulticast}
            };
        ...
        case bcmPolicerGroupModeTyped:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
                ...
                {1, bcmPolicerGroupModeAttrPktTypeKnownL3UC},
                /* 3) */
                {2, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                /* 4) L2BC_PKT */
                {3, bcmPolicerGroupModeAttrPktTypeL2BC}
            };

            *npolicers = 4;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 1 + 2 + 4 + 1;
        ...
        case bcmPolicerGroupModeTypedAll:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
                ...
                {3, bcmPolicerGroupModeAttrPktTypeL2BC},
                /* 5) */
                {4, bcmPolicerGroupModeAttrPktTypeUnknown},
                {4, bcmPolicerGroupModeAttrPktTypeControl},
                {4, bcmPolicerGroupModeAttrPktTypeBPDU},
                {4, bcmPolicerGroupModeAttrPktTypeUnknownL2UC},
                {4, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                {4, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {4, bcmPolicerGroupModeAttrPktTypeKnownMpls},
                {4, bcmPolicerGroupModeAttrPktTypeKnownMplsL3},
                {4, bcmPolicerGroupModeAttrPktTypeKnownMplsL2},
                {4, bcmPolicerGroupModeAttrPktTypeUnknownMpls},
                {4, bcmPolicerGroupModeAttrPktTypeKnownMim},
                {4, bcmPolicerGroupModeAttrPktTypeUnknownMim},
                {4, bcmPolicerGroupModeAttrPktTypeKnownMplsMulticast}
            };

            *npolicers = 5;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 1 + 2 + 4 + 1 + 13;
        ...
        case bcmPolicerGroupModeSingleWithControl:
        {
            ...
            {
                /* 1) */
                {0, bcmPolicerGroupModeAttrPktTypeUnknown},
                {0, bcmPolicerGroupModeAttrPktTypeL2BC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownL2UC},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownL2UC},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownL3UC},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownL3UC},
                {0, bcmPolicerGroupModeAttrPktTypeKnownMpls},
                {0, bcmPolicerGroupModeAttrPktTypeKnownMplsL3},
                {0, bcmPolicerGroupModeAttrPktTypeKnownMplsL2},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownMpls},
                {0, bcmPolicerGroupModeAttrPktTypeKnownMim},
                {0, bcmPolicerGroupModeAttrPktTypeUnknownMim},
                {0, bcmPolicerGroupModeAttrPktTypeKnownMplsMulticast},
                /* 2) CONTROL_PKT|BPDU_PKT */
                {1, bcmPolicerGroupModeAttrPktTypeControl},
                {1, bcmPolicerGroupModeAttrPktTypeBPDU}
            };
        ...
        case bcmPolicerGroupModeTrafficTypeWithControl:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
               ...
               KNOWN_IPMC_PKT | UNKNOWN_IPMC_PKT */
                ...
                {1, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                /* 3) L2BC_PKT */
                {2, bcmPolicerGroupModeAttrPktTypeL2BC},
                /* 4) CONTROL_PKT|BPDU_PKT */
                {3, bcmPolicerGroupModeAttrPktTypeControl},
                {3, bcmPolicerGroupModeAttrPktTypeBPDU}
            };

            *npolicers = 4;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 3 + 4 + 1 + 2;
        ...
        case bcmPolicerGroupModeDlfAllWithControl:
        {
            ...
            {
                /* 1) CONTROL_PKT|BPDU_PKT */
                {0, bcmPolicerGroupModeAttrPktTypeControl},
                {0, bcmPolicerGroupModeAttrPktTypeBPDU},
                /* 2) L2DLF_PKT(UNKNOWN_L2UC_PKT) */
                {1, bcmPolicerGroupModeAttrPktTypeUnknownL2UC},
                /* 3) */
                {2, bcmPolicerGroupModeAttrPktTypeUnknown},
                {2, bcmPolicerGroupModeAttrPktTypeControl},
                {2, bcmPolicerGroupModeAttrPktTypeBPDU},
                {2, bcmPolicerGroupModeAttrPktTypeL2BC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownL2UC},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownL3UC},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownL3UC},
                {2, bcmPolicerGroupModeAttrPktTypeKnownMpls},
                {2, bcmPolicerGroupModeAttrPktTypeKnownMplsL3},
                {2, bcmPolicerGroupModeAttrPktTypeKnownMplsL2},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownMpls},
                {2, bcmPolicerGroupModeAttrPktTypeKnownMim},
                {2, bcmPolicerGroupModeAttrPktTypeUnknownMim},
                {2, bcmPolicerGroupModeAttrPktTypeKnownMplsMulticast}
            };

            *npolicers = 3;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 2 + 1 + 18;
        ...
        case bcmPolicerGroupModeTypedWithControl:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
                ...
                /* 4) KNOWN_L2MC_PKT|UNKNOWN_L2MC_PKT|
                            KNOWN_IPMC_PKT|UNKNOWN_IPMC_PKT */
                {3, bcmPolicerGroupModeAttrPktTypeKnownL2MC},
                {3, bcmPolicerGroupModeAttrPktTypeUnknownL2MC},
                {3, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {3, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                /* 5) L2BC_PKT */
                {4, bcmPolicerGroupModeAttrPktTypeL2BC}
            };

            *npolicers = 5;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 2 + 1 + 2 + 4 + 1;
        ...
        /* 2) UNKNOWN_L3UC_PKT                                               */
        /* 3) KNOWN_L2UC_PKT | KNOWN_L3UC_PKT                                */
        /* 4) KNOWN_L2MC_PKT|UNKNOWN_L2MC_PKT|KNOWN_IPMC_PKT|                */
        /*    UNKNOWN_IPMC_PKT                                               */
        /* 5) L2BC_PKT                                                       */
        ...
        case bcmPolicerGroupModeTypedAllWithControl:
        {
            pkt_res_final_offset_t pkt_res_final_offset_v[] =
            {
                ...
                {4, bcmPolicerGroupModeAttrPktTypeL2BC},
                /* 6) */
                {5, bcmPolicerGroupModeAttrPktTypeUnknown},
                {5, bcmPolicerGroupModeAttrPktTypeUnknownL2UC},
                {5, bcmPolicerGroupModeAttrPktTypeUnknownIPMC},
                {5, bcmPolicerGroupModeAttrPktTypeKnownIPMC},
                {5, bcmPolicerGroupModeAttrPktTypeKnownMpls},
                {5, bcmPolicerGroupModeAttrPktTypeKnownMplsL3},
                {5, bcmPolicerGroupModeAttrPktTypeKnownMplsL2},
                {5, bcmPolicerGroupModeAttrPktTypeUnknownMpls},
                {5, bcmPolicerGroupModeAttrPktTypeKnownMim},
                {5, bcmPolicerGroupModeAttrPktTypeUnknownMim},
                {5, bcmPolicerGroupModeAttrPktTypeKnownMplsMulticast}
            };

            *npolicers = 6;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_v_cnt = 2 + 1 + 2 + 4 + 1 + 11;
        ...
        case bcmPolicerGroupModeDot1P:
        {
            *npolicers = 8;
            ...
            /* customize defined count & final_offset */
            if (mapping != NULL) {
                pkt_attr_v_cnt = count;
                *npolicers = pkt_attr_v_cnt;
            }
            ...
            attr = bcmPolicerGroupModeAttrOuterPri;
            for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
                pkt_attr_v->value[attr] = sel;
                pkt_attr_v->value_mask[attr] =
                    BCMI_POLICER_GLOBAL_METER_ATTR_MASK_ALL;
                pkt_attr_v->final_offset = (mapping == NULL) ?
                    sel : final_offset[sel];
            }
            break;
        }
        /* ********************************************************* */
        ...
        case bcmPolicerGroupModeIntPri:
        {
            *npolicers = 16;
            ...
            attr = bcmPolicerGroupModeAttrIntPri;
            for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
        ...
        /* *************************************************************** */
        /* N+1 policers where in the base counter is used for dlf and next */
        /* N are used as per Cos                                           */
        ...
        case bcmPolicerGroupModeDlfIntPri:
        {
            ...
            *npolicers = 17;
            ...
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_pkt_type_cnt = 1;
            pkt_attr_v_cnt = pkt_attr_pkt_type_cnt + 16;
            ...
            attr = bcmPolicerGroupModeAttrPktType;
            for (sel = 0; sel < pkt_attr_pkt_type_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
                pkt_attr_v->value_mask[attr] =
                ...
                /* for following final_offset */
                if (base_final_offset < pkt_attr_v->final_offset) {
                    base_final_offset = pkt_attr_v->final_offset;
                }
            }

            attr = bcmPolicerGroupModeAttrIntPri;
            for (sel = pkt_attr_pkt_type_cnt; sel < pkt_attr_v_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
                pkt_attr_v->value[attr] =
                    sel - pkt_attr_pkt_type_cnt;
                pkt_attr_v->value_mask[attr] =
                    BCMI_POLICER_GLOBAL_METER_ATTR_MASK_ALL;
                pkt_attr_v->final_offset =
                    base_final_offset + sel - pkt_attr_pkt_type_cnt;
            }
            break;
        }
        /* ***************************************************************** */
        ...
        case bcmPolicerGroupModeTypedIntPriWithControl:
        {
            ...
            *npolicers = 21;
            /* following is array size of pkt_res_final_offset_v */
            pkt_attr_pkt_type_cnt = 2 + 1 + 2 + 2 + 1;
            pkt_attr_v_cnt = pkt_attr_pkt_type_cnt + 16;
        ...
        /* ************************************************** */
        /* A dedicated counter for control, preemptable and   */
        /* non-preemptable packet                             */
        /* 1) preemptable PKT                                 */
        /* 2) none-preemptable PKT                            */
        /* ************************************************** */
        case bcmPolicerGroupModePreemptable:
        {
            *npolicers = 2;
            ...
            attr = bcmPolicerGroupModeAttrPreemptable;
            for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
                pkt_attr_v =
                    &pkt_attr_selectors->pkt_attr_v[sel];
                pkt_attr_v->attr_bits_selector = attr_to_bit_selector[attr];
    ...
    for (sel = 0; sel < pkt_attr_v_cnt; sel++) {
        pkt_attr_v = &pkt_attr_selectors->pkt_attr_v[sel];

        (void)bcmi_policer_tcam_attr_dump(
            unit, "pkt_attr_v", pkt_attr_v);

        /* Final Offset */
        pol_offset = pkt_attr_v->final_offset;
    ...
    /* Check Maximum Policer Offset */
    if (max_pol_offset >= *npolicers) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Maximum policer offset(%d) should less than"
                              " total policer number(%d).\n"),
                              max_pol_offset,
                              *npolicers));
        return BCM_E_PARAM;
    }

    pkt_attr_selectors->total_policers = *npolicers;
    pkt_attr_selectors->pkt_attr_v_cnt = pkt_attr_v_cnt;
    ...
    int *npolicers)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, flow_info);
    POL_CHECK_NULL_PARAMETER(unit, policer_id);
    POL_CHECK_NULL_PARAMETER(unit, npolicers);
    ...
    if (NULL == pkt_attr_selectors.pkt_attr_v) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to allocate memory for "
                              "pkt_attr_selectors.pkt_attr_v\n")));
    ...
    /* Create temporary pkt information from group_mode */
    rv = bcmi_esw_policer_group_set_mode_and_map(
            unit, group_mode, offset_map, npolicers, &pkt_attr_selectors);
    ...
    if (group_mode == bcmPolicerGroupModeCascade) {
        group_mode_type = bcmPolicerGroupModeTypeCascade;
    } else if (group_mode == bcmPolicerGroupModeCascadeWithCoupling) {
        group_mode_type = bcmPolicerGroupModeTypeCascadeWithCoupling;
    } else {
        group_mode_type = bcmPolicerGroupModeTypeNormal;
    }

    if (*npolicers >= 1) {
        rv = bcmi_esw_policer_svc_meter_create_mode(unit,
                &pkt_attr_selectors, group_mode, group_mode_type,
                *npolicers, &mode_id);
        if (BCM_FAILURE(rv)) {
            POL_MEM_FREE(pkt_attr_selectors.pkt_attr_v);
            GLOBAL_METER_UNLOCK(unit);
...
#endif /* POLICER_ID_API_SECTION */

#if ACTION_API_SECTION
/*
 * Function:
 *     bcmi_esw_policer_action_create
 * Purpose:
    ...
    uint32 *action_id)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, action_id);

    CHECK_GLOBAL_METER_INIT(unit);
    rv = shr_aidxres_list_alloc_block(
            BCMI_ACTION_LIST_HANDLE_GET(unit), 1, (uint32 *)action_id);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    BCMI_ACTION_ID_BK_GET(unit, *action_id).actions_count = 0;
    BCMI_ACTION_ID_BK_GET(unit, *action_id).used = 1;

    POL_FUNCTION_DEBUG(unit, "action_id (%d) created", *action_id);
    return rv;
}

/*
 * Function:
 *     bcmi_esw_policer_action_add
 * Purpose:
 ...
 *     param0                - (IN) Parameter associated with action
 * Returns:
 *     BCM_E_XXX
 */
STATIC int
    ...
    uint32 param0)
{
    ...
    POL_FUNCTION_TRACE(unit, "IN");
    POL_FUNCTION_DEBUG(unit, "action_id %d", action_id);
    POL_FUNCTION_DEBUG(unit, "bcm_policer_action_t %d", action);
    POL_FUNCTION_DEBUG(unit, "param0 %d", param0);
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->action_supported);

    rv = gm_dev_info->action_supported(unit, action);
    BCM_IF_ERROR_RETURN(rv);

    if (BCMI_ACTION_ID_BK_GET(unit, action_id).used != 1)
    {
        LOG_DEBUG(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    if (BCMI_ACTION_ID_BK_GET(unit, action_id).reference_count != 0) {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    /* Since there is no HW action table for GH2, we need to store actions */
    idx = BCMI_ACTION_ID_BK_GET(unit, action_id).actions_count;
    BCMI_ACTION_ID_BK_GET(unit, action_id).actions[idx] = action;
    BCMI_ACTION_ID_BK_GET(unit, action_id).param[idx] = param0;
    BCMI_ACTION_ID_BK_GET(unit, action_id).actions_count++;
...
/*
 * Function:
 *     bcmi_esw_policer_action_destroy
 * Purpose:
    ...
    uint32 action_id)
{
    ...
    if (BCMI_ACTION_ID_BK_GET(unit, action_id).used != 1)
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Action Id(%d) specified doesn't exist\n"),
                              action_id));
        return BCM_E_PARAM;
    }
    if (BCMI_ACTION_ID_BK_GET(unit, action_id).reference_count != 0)
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Action Id(%d) specified still being used \n"),
                              action_id));
        return BCM_E_BUSY;
    }
    GLOBAL_METER_LOCK(unit);
    ...
    rv = shr_aidxres_list_free(
            BCMI_ACTION_LIST_HANDLE_GET(unit), action_id);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
    ...
    BCMI_ACTION_ID_BK_GET(unit, action_id).actions_count = 0;
    BCMI_ACTION_ID_BK_GET(unit, action_id).used = 0;
    GLOBAL_METER_UNLOCK(unit);
    ...
    uint32 *action_id)
{
    ...
    /* validate policer id */
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_validate(unit, policer_id));

    GLOBAL_METER_BK_LOCK(unit);
    rv = bcmi_esw_global_meter_base_policer_get(
            unit, policer_id, &policer_control);
    if (BCM_FAILURE(rv)) {
        GLOBAL_METER_BK_UNLOCK(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    action_id_idx = policer_id - policer_control->pid;

    /* get action id */
    *action_id = policer_control->action_id[action_id_idx];
...
/*
 * Function:
 *     bcmi_esw_policer_action_detach
 * Purpose:
    ...
    uint32 action_id)
{
    ...
    int policer_index = 0;
    uint32 action_id_idx;
    uint64 reset = COMPILER_64_INIT(0, 0);

    POL_FUNCTION_TRACE(unit, "IN");

    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "Detach action_id (%d) on policer_id (%d)"),
    ...
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_table_index_get(unit, policer_id, &policer_index));

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_action_set);

    /* write action table entry */
    rv = gm_dev_info->policy_action_set(unit,
                                        policer_index,
                                        reset,
                                        reset,
                                        reset);
    ...
    if (BCMI_ACTION_ID_BK_GET(unit, action_id).reference_count > 0) {
        BCMI_ACTION_ID_BK_GET(unit, action_id).reference_count--;
    }

    action_id_idx = policer_id - policer_control->pid;

    /* reset action id in policer control */
    policer_control->action_id[action_id_idx] =
        BCMI_POLICER_GLOBAL_METER_INVALID_ACTION_ID;
    GLOBAL_METER_UNLOCK(unit);
    ...
    uint32 action_id)
{
    ...
    bcmi_global_meter_policer_control_t *policer_control = NULL;
    int policer_index = 0;
    uint64 green_action = COMPILER_64_INIT(0, 0);
    uint64 yellow_action = COMPILER_64_INIT(0, 0);
    uint64 red_action = COMPILER_64_INIT(0, 0);
    uint32 idx = 0, action_id_idx;
    bcm_policer_action_t action;
    uint32 param0;
    ...
    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "Attach action_id(%d) to policer_id (%d)\n"),
                            action_id, policer_id));

    if (action_id == BCMI_POLICER_GLOBAL_METER_INVALID_ACTION_ID) {
        return BCM_E_PARAM;
    }
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_action_set);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_action_get);
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->policy_action_param_configure);
    ...
    if (policer_control->action_id[action_id_idx] == action_id) {
        GLOBAL_METER_UNLOCK(unit);
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
                                "Action Id %d(%x) passed is the same as the "
                                "one in policer control\n"),
                                action_id, action_id));
        return (BCM_E_NONE);
    }
    ...
    /* set action to policy table */
    BCM_IF_ERROR_RETURN(
        bcmi_esw_policer_table_index_get(unit, policer_id, &policer_index));

    /* read action table entry */
    rv = gm_dev_info->policy_action_get(
            unit, policer_index,
    ...
    /* read from bk and set action to policy table */
    for (idx = 0;
         idx < BCMI_ACTION_ID_BK_GET(unit, action_id).actions_count;
         idx++) {
        action = BCMI_ACTION_ID_BK_GET(unit, action_id).actions[idx];
        param0 = BCMI_ACTION_ID_BK_GET(unit, action_id).param[idx];


        rv = gm_dev_info->policy_action_param_configure(
                unit, action, param0,
                &green_action, &yellow_action, &red_action);
    ...
    /* write action table entry */
    rv = gm_dev_info->policy_action_set(
            unit, policer_index,
            green_action, yellow_action, red_action);
    ...
    /* increment action usage reference count */
    BCMI_ACTION_ID_BK_GET(unit, action_id).reference_count++;
    policer_control->action_id[action_id_idx] = action_id;
    GLOBAL_METER_UNLOCK(unit);
...
/* end Action API's */
#endif /* ACTION_API_SECTION */

#ifdef BCM_WARM_BOOT_SUPPORT
/*
 * Function:
 *     bcmi_esw_global_meter_policer_sync
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_global_meter_policer_sync(int unit)
{
    int rv = BCM_E_NONE;
    ...
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    bcmi_policer_svm_offset_mode_bk_t *bk_svm_mode_info = NULL;
    uint32 bmp_size, table_size;
    int i;
    uint32 ref_cnt = 0;
    uint32 used = 0;
    bcm_policer_group_mode_t group_mode = bcmPolicerGroupModeCount;
    bcm_policer_group_mode_type_t group_mode_type =
        bcmPolicerGroupModeTypeCount;
    uint32 no_of_policers = 0;
    uint32 no_of_attr_selectors = 0;
    uint32 pkt_attr_v_cnt = 0;
    ...
    /* 1. tcam usage bitmap */
    size += SHR_BITALLOCSIZE(BCMI_POLICER_SVC_METER_TCAM_SIZE);

    /* 2. offset_mode usage bitmap */
    size += SHR_BITALLOCSIZE(BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE);

    /* 3. bcmi_policer_global_meter_action_bk_t */
    size += sizeof(bcmi_policer_global_meter_action_bk_t) *
            BCMI_POLICER_SVC_METER_ACTION_SIZE;

    /* 4. shr_aidxres_list_handle_t */
    size += sizeof(shr_aidxres_list_handle_t) * 2;

    /* 5. bcmi_policer_svm_offset_mode_bk_t */
    size += sizeof(bcmi_policer_svm_offset_mode_bk_t) *
            BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;

    /* 6. bcm_policer_group_mode_attr_selector_t */
    size += sizeof(bcm_policer_group_mode_attr_selector_t) *
            BCMI_POLICER_SVC_METER_TCAM_SIZE;

    /* 7. pkt_tcam_attr_bits_t */
    size += sizeof(pkt_tcam_attr_bits_t) *
            BCMI_POLICER_SVC_METER_TCAM_SIZE;

    /* 8. bcmi_global_meter_policer_control_t */
    size += sizeof(bcmi_global_meter_policer_control_t) *
            BCMI_POLICER_GLOBAL_METER_MAX_SCACHE_ENTRIES;

    SOC_IF_ERROR_RETURN(_bcm_esw_scache_ptr_get(unit, scache_handle,
        FALSE, size, &policer_state, BCM_WB_DEFAULT_VERSION, NULL));
    ...
    /* 1. Copy tcam usage bitmap */
    bmp_size = BCMI_POLICER_SVC_METER_TCAM_SIZE;
    for (i = 0; i < bmp_size; i++) {
        sal_memcpy(policer_state, &bk_svm_info->tcam_intf_bitmap[i],
                   sizeof(SHR_BITDCL));
        policer_state += sizeof(SHR_BITDCL);
    }

    /* 2. Copy offset_mode usage bitmap */
    bmp_size = BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
    for (i = 0; i < bmp_size; i++) {
        sal_memcpy(policer_state, &bk_svm_info->offset_mode_intf_bitmap[i],
                   sizeof(SHR_BITDCL));
        policer_state += sizeof(SHR_BITDCL);
    }
    ...
    /* 3. Copy actions bk */
    table_size =
        sizeof(bcmi_policer_global_meter_action_bk_t) *
        BCMI_POLICER_SVC_METER_ACTION_SIZE;
    sal_memcpy(policer_state, BCMI_ACTION_BK_GET(unit),
               table_size);
    policer_state += table_size;

    /* 4. Copy actions & policer list handle */
    table_size = sizeof(shr_aidxres_list_handle_t);
    sal_memcpy(policer_state, BCMI_ACTION_LIST_HANDLE_GET(unit),
               table_size);
    policer_state += table_size;

    table_size = sizeof(shr_aidxres_list_handle_t);
    sal_memcpy(policer_state, BCMI_POLICER_LIST_HANDLE_GET(unit),
               table_size);
    policer_state += table_size;

    /* 5. bcmi_policer_svm_offset_mode_bk_t */
    for (offset_mode = 0;
         offset_mode < BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
         offset_mode++) {
        if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, offset_mode)) {
            bk_svm_mode_info =
                BCMI_SVM_OFFSET_MODE_BK_GET(unit, offset_mode);

            ref_cnt = bk_svm_mode_info->ref_cnt;
            used = bk_svm_mode_info->used;
            no_of_policers = bk_svm_mode_info->no_of_policers;
            no_of_attr_selectors = bk_svm_mode_info->no_of_attr_selectors;
            group_mode = bk_svm_mode_info->group_mode;
            group_mode_type = bk_svm_mode_info->group_mode_type;
            pkt_attr_v_cnt = bk_svm_mode_info->pkt_attr_v_cnt;

            sal_memcpy(policer_state, &ref_cnt,
                SHR_BITALLOCSIZE(uint32));
            policer_state += sizeof(uint32);

            sal_memcpy(policer_state, &used,
                SHR_BITALLOCSIZE(uint32));
            policer_state += sizeof(uint32);

            sal_memcpy(policer_state, &no_of_policers,
                SHR_BITALLOCSIZE(uint32));
            policer_state += sizeof(uint32);

            sal_memcpy(policer_state, &no_of_attr_selectors,
                SHR_BITALLOCSIZE(uint32));
            policer_state += sizeof(uint32);

            sal_memcpy(policer_state, &pkt_attr_v_cnt,
                SHR_BITALLOCSIZE(uint32));
            policer_state += sizeof(uint32);

            sal_memcpy(policer_state, &group_mode,
                SHR_BITALLOCSIZE(bcm_policer_group_mode_t));
            policer_state += sizeof(bcm_policer_group_mode_t);

            sal_memcpy(policer_state, &group_mode_type,
                SHR_BITALLOCSIZE(bcm_policer_group_mode_type_t));
            policer_state += sizeof(bcm_policer_group_mode_type_t);

            /* 6. Save attr_selectors info */
            for (i = 0; i < no_of_attr_selectors; i++) {
                sal_memcpy(policer_state,
                           &bk_svm_mode_info->attr_selectors[i],
                           sizeof(bcm_policer_group_mode_attr_selector_t));
                policer_state += sizeof(bcm_policer_group_mode_attr_selector_t);
            }

            /* 7. Save pkt_attr_v info */
            for (i = 0; i < pkt_attr_v_cnt; i++) {
                sal_memcpy(policer_state,
                           &bk_svm_mode_info->pkt_attr_v[i],
                           sizeof(pkt_tcam_attr_bits_t));
                policer_state += sizeof(pkt_tcam_attr_bits_t);
            }
        }
    }

    /* 8. go through policer control bookkeep structure and get policer
       entries */
    for (hash_index = 0; hash_index < BCMI_GLOBAL_METER_HASH_SIZE; hash_index++) {
        policer_control = BCMI_POLICER_HASH_BK_GET(unit, hash_index);
        while ((NULL != policer_control) &&
               (count < BCMI_POLICER_GLOBAL_METER_MAX_SCACHE_ENTRIES)) {
            sal_memcpy(policer_state, policer_control,
                sizeof(bcmi_global_meter_policer_control_t));
            policer_state += sizeof(bcmi_global_meter_policer_control_t);
            count++;
            policer_control = policer_control->next;
        }
        if (count == BCMI_POLICER_GLOBAL_METER_MAX_SCACHE_ENTRIES) {
            return rv;
        }
    }
    return rv;
...
/*
 * Function:
 *      bcmi_esw_global_meter_policer_recover_from_scache
 * Purpose:
 ...
 */
STATIC int
bcmi_esw_global_meter_policer_recover_from_scache(int unit)
{
    int rv = BCM_E_NONE;
    int stable_size;
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        /* 1. recovery tcam usage bitmap */
        bmp_size = BCMI_POLICER_SVC_METER_TCAM_SIZE;
        for (i = 0; i < bmp_size; i++) {
            sal_memcpy(&bk_svm_info->tcam_intf_bitmap[i], policer_state,
                       sizeof(SHR_BITDCL));
            policer_state += sizeof(SHR_BITDCL);
        }

        /* 2. recovery offset_mode usage bitmap */
        bmp_size = BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
        for (i = 0; i < bmp_size; i++) {
            sal_memcpy(&bk_svm_info->offset_mode_intf_bitmap[i], policer_state,
                       sizeof(SHR_BITDCL));
            policer_state += sizeof(SHR_BITDCL);
        }
        ...
        table_size =
            sizeof(bcmi_policer_global_meter_action_bk_t) *
            BCMI_POLICER_SVC_METER_ACTION_SIZE;
        sal_memcpy(BCMI_ACTION_BK_GET(unit), policer_state,
                   table_size);
        policer_state += table_size;

        /* 4. Copy actions & policer list handle */
        table_size = sizeof(shr_aidxres_list_handle_t);
        sal_memcpy(BCMI_ACTION_LIST_HANDLE_GET(unit), policer_state,
                   table_size);
        policer_state += table_size;

        table_size = sizeof(shr_aidxres_list_handle_t);
        sal_memcpy(BCMI_POLICER_LIST_HANDLE_GET(unit), policer_state,
                   table_size);
        policer_state += table_size;
        ...
        for (offset_mode = 0;
            ...
            if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, offset_mode)) {
                ...
                sal_memcpy(&ref_cnt, policer_state,
                    SHR_BITALLOCSIZE(uint32));
                policer_state += sizeof(uint32);

                sal_memcpy(&used, policer_state,
                    SHR_BITALLOCSIZE(uint32));
                policer_state += sizeof(uint32);

                sal_memcpy(&no_of_policers, policer_state,
                    SHR_BITALLOCSIZE(uint32));
                policer_state += sizeof(uint32);

                sal_memcpy(&no_of_attr_selectors, policer_state,
                    SHR_BITALLOCSIZE(uint32));
                policer_state += sizeof(uint32);

                sal_memcpy(&pkt_attr_v_cnt, policer_state,
                    SHR_BITALLOCSIZE(uint32));
                policer_state += sizeof(uint32);

                sal_memcpy(&group_mode, policer_state,
                    SHR_BITALLOCSIZE(bcm_policer_group_mode_t));
                policer_state += sizeof(bcm_policer_group_mode_t);

                sal_memcpy(&group_mode_type, policer_state,
                    SHR_BITALLOCSIZE(bcm_policer_group_mode_type_t));
                policer_state += sizeof(bcm_policer_group_mode_type_t);

                bk_svm_mode_info->ref_cnt = ref_cnt;
                bk_svm_mode_info->used = used;
                bk_svm_mode_info->no_of_policers = no_of_policers;
                bk_svm_mode_info->no_of_attr_selectors = no_of_attr_selectors;
                bk_svm_mode_info->group_mode = group_mode;
                bk_svm_mode_info->group_mode_type = group_mode_type;

                /* 6. recovery attr_selectors info */
                for (i = 0; i < no_of_attr_selectors; i++) {
                    sal_memcpy(&bk_svm_mode_info->attr_selectors[i],
                               policer_state,
                               sizeof(bcm_policer_group_mode_attr_selector_t));
                    policer_state +=
                        sizeof(bcm_policer_group_mode_attr_selector_t);
                ...
                /* 7. recovery pkt_attr_v info */
                for (i = 0; i < pkt_attr_v_cnt; i++) {
                    sal_memcpy(&bk_svm_mode_info->pkt_attr_v[i],
                               policer_state,
                               sizeof(pkt_tcam_attr_bits_t));
                    policer_state += sizeof(pkt_tcam_attr_bits_t);
                }
        ...
        /* 8. read from scache and create policer bookkeep data structure */
        for (scache_index = 0;
             scache_index < BCMI_POLICER_GLOBAL_METER_MAX_SCACHE_ENTRIES;
             scache_index++) {
            /* Allocate policer descriptor. */
            POL_MEM_ALLOC(policer_control,
                sizeof(bcmi_global_meter_policer_control_t),
            "Global meter policer");
            sal_memcpy(policer_control, &policer_state[scache_index],
                sizeof(bcmi_global_meter_policer_control_t));

            policer_id = policer_control->pid;
            if ((policer_id & BCM_POLICER_GLOBAL_METER_INDEX_MASK) > 0) {
                /* Insert policer into policers hash. */
                _GLOBAL_METER_HASH_INSERT(
                    BCMI_POLICER_BK_GET(unit),
                    policer_control,
                    (policer_id & _GLOBAL_METER_HASH_MASK));
            } else {
                /* De-allocate policer descriptor. */
        ...
        /* Calcuate the cnt/start/end */
        bmp_size = BCMI_POLICER_SVC_METER_TCAM_SIZE;
        bk_svm_info->empty_tcam_idx_cnt = bmp_size;
        bk_svm_info->valid_tcam_idx_cnt = 0;
        bk_svm_info->valid_tcam_idx_start = 0;
        bk_svm_info->valid_tcam_idx_end = 0;

        for (i = 0; i < bmp_size; i++) {
            if (BCMI_SVM_TCAM_INTF_GET(unit, i)) {
                bk_svm_info->valid_tcam_idx_cnt++;
            }

            if (BCMI_SVM_TCAM_INTF_GET(unit, i) &&
                bk_svm_info->valid_tcam_idx_start == 0) {
                bk_svm_info->valid_tcam_idx_start = i;
            }

            if (BCMI_SVM_TCAM_INTF_GET(unit, i) &&
                bk_svm_info->valid_tcam_idx_end < i) {
                bk_svm_info->valid_tcam_idx_end = i;
            }

            if (!BCMI_SVM_TCAM_INTF_GET(unit, i) &&
                bk_svm_info->empty_tcam_idx_start == 0) {
                bk_svm_info->empty_tcam_idx_start = i;
            }

            if (!BCMI_SVM_TCAM_INTF_GET(unit, i) &&
                bk_svm_info->empty_tcam_idx_end < i) {
                bk_svm_info->empty_tcam_idx_end = i;
            }
        }
        bk_svm_info->empty_tcam_idx_cnt -=
        ...
        bmp_size = BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
        bk_svm_info->empty_offset_mode_cnt = bmp_size;
        bk_svm_info->valid_offset_mode_cnt = 0;
        bk_svm_info->valid_offset_mode_start = 0;
        bk_svm_info->valid_offset_mode_end = 0;
        for (i = 0; i < bmp_size; i++) {
            if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, i)) {
                bk_svm_info->valid_offset_mode_cnt++;
            }

            if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, i) &&
                bk_svm_info->valid_offset_mode_start == 0) {
                bk_svm_info->valid_offset_mode_start = i;
            }

            if (BCMI_OFFSET_MODE_ID_INTF_GET(unit, i) &&
                bk_svm_info->valid_offset_mode_end < i) {
                bk_svm_info->valid_offset_mode_end = i;
            }

            if (!BCMI_OFFSET_MODE_ID_INTF_GET(unit, i) &&
                bk_svm_info->empty_offset_mode_start == 0) {
                bk_svm_info->empty_offset_mode_start = i;
            }

            if (!BCMI_OFFSET_MODE_ID_INTF_GET(unit, i) &&
                bk_svm_info->empty_offset_mode_end < i) {
                bk_svm_info->empty_offset_mode_end = i;
            }
        }
        bk_svm_info->empty_offset_mode_cnt -=
 ...
 */
STATIC int
bcmi_esw_global_meter_reinit(int unit)
{
    bcm_error_t rv = BCM_E_UNAVAIL;
    ...
    rv = bcmi_esw_global_meter_policer_recover_from_scache(unit);
    if (BCM_FAILURE(rv)) {
        LOG_VERBOSE(BSL_LS_BCM_POLICER,
                    (BSL_META_U(unit,
 ...
 */
STATIC int
bcmi_esw_global_meter_cleanup(int unit)
{
    int rv = BCM_E_NONE;
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    const bcmi_global_meter_dev_info_t *gm_dev_info = NULL;
    ...
    POL_MEM_FREE(bk_svm_info->tcam_intf_bitmap);

    if (bk_svm_info->offset_mode_v != NULL) {
        for (i = 0; i < BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE; i++) {
            POL_MEM_FREE(bk_svm_info->offset_mode_v[i].attr_selectors);
            POL_MEM_FREE(bk_svm_info->offset_mode_v[i].pkt_attr_v);
        }
    }
    POL_MEM_FREE(bk_svm_info->offset_mode_v);
    POL_MEM_FREE(bk_svm_info->offset_mode_intf_bitmap);

    /* mutex destroy */
    if (NULL != bk_svm_info->mutex) {
        sal_mutex_destroy(bk_svm_info->mutex);
        bk_svm_info->mutex = NULL;
    }
    ...
    if (NULL != BCMI_ACTION_LIST_HANDLE_GET(unit)) {
        shr_aidxres_list_destroy(BCMI_ACTION_LIST_HANDLE_GET(unit));
        BCMI_ACTION_LIST_HANDLE_GET(unit) = NULL;
    }

    /* Free handles of index management for meter policer */
    if (NULL != BCMI_POLICER_LIST_HANDLE_GET(unit)) {
        shr_aidxres_list_destroy(BCMI_POLICER_LIST_HANDLE_GET(unit));
        BCMI_POLICER_LIST_HANDLE_GET(unit) = NULL;
    }

    if (NULL != BCMI_POLICER_BK_GET(unit)) {
        POL_MEM_FREE(BCMI_POLICER_BK_GET(unit));
    }

    if (NULL != BCMI_ACTION_BK_GET(unit)) {
        POL_MEM_FREE(BCMI_ACTION_BK_GET(unit));
    }
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->mem_clear);

    rv = gm_dev_info->mem_clear(unit);
    BCM_IF_ERROR_RETURN(rv);

    global_meter_status[unit].initialised = FALSE;
 ...
 */
STATIC int
bcmi_esw_global_meter_init(int unit)
{
    int rv = BCM_E_NONE;
    bcmi_policer_svm_bk_t *bk_svm_info = NULL;
    const bcmi_global_meter_dev_info_t *gm_dev_info = NULL;
    uint32 global_meter_ctrl_disable = 0;
    uint32 table_size;
    uint32 action_size = 0;
    uint32 size_bank = 0, block_factor = 0;
    int i = 0;
    bcm_policer_global_meter_config_t global_meter_config_v;
    ...
    /* assert check that attr_to_bit_selector is added corrected */
    (void)check_attr_to_bit_selector();

    /* Initial all unit with default value FALSE. */
    if (FALSE == bcmi_global_meter_dev_info_initialized) {
        for (i = 0; i < BCM_MAX_NUM_UNITS; i++) {
            BCM_SVM_DEV_INFO_GET(unit) = NULL;
        }
    ...
    if (TRUE == global_meter_status[unit].initialised) {
        bcmi_esw_global_meter_cleanup(unit);
    }

    /* ******************************************** */
    /*
     * Check if  Service meter Block is enable or not in config.bcm
     * global_meter_control = 1 means enable
     * global_meter_control = 0 means disable
     */
    if (!soc_property_get(unit, spn_GLOBAL_METER_CONTROL, 1)) {
        global_meter_ctrl_disable = BCM_POLICER_GLOBAL_METER_DISABLE;
    }

    LOG_DEBUG(BSL_LS_BCM_POLICER,
              (BSL_META_U(unit,
                          "global_meter_ctrl_disable = %d, "
                          "enable %d\n"),
                          global_meter_ctrl_disable,
                          global_meter_ctrl_disable ==
                          BCM_POLICER_GLOBAL_METER_DISABLE ? 0 : 1
                          ));
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->svm_enable_set);

    if (BCM_POLICER_GLOBAL_METER_DISABLE == global_meter_ctrl_disable) {
        rv = gm_dev_info->svm_enable_set(unit, FALSE);
        BCM_IF_ERROR_RETURN(rv);
        global_meter_status[unit].initialised = FALSE;
        LOG_WARN(BSL_LS_BCM_POLICER,
                 (BSL_META_U(unit,
        ...
        rv = gm_dev_info->svm_enable_set(unit, TRUE);
        BCM_IF_ERROR_RETURN(rv);
    }
    /* ******************************************** */
    ...
    /* ******************************************** */
    /*
     * Always enable  SEOP metering all the time in SVM block.
     * It's usable for any metering mode.
     */

    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->seop_enable_set);

    rv = gm_dev_info->seop_enable_set(unit, 1);
    BCM_IF_ERROR_RETURN(rv);
    /* ******************************************** */
    ...
    /* Allocate usage bitmap */
    table_size = BCMI_POLICER_SVC_METER_TCAM_SIZE;
    if ((table_size) > (soc_mem_index_max(unit, SVC_METER_OFFSET_TCAMm) + 1))
    {
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    POL_MEM_ALLOC(
        bk_svm_info->tcam_intf_bitmap,
        SHR_BITALLOCSIZE(table_size),
        "TCAM intf_bitmap");
    if (bk_svm_info->tcam_intf_bitmap == NULL) {
        bcmi_esw_global_meter_cleanup(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to allocate memory for "
    ...
    bk_svm_info->valid_tcam_idx_start = 0;
    bk_svm_info->valid_tcam_idx_end = 0;
    bk_svm_info->empty_tcam_idx_cnt = table_size;
    bk_svm_info->empty_tcam_idx_start = 0;
    bk_svm_info->empty_tcam_idx_end = table_size - 1;

    /* Allocate usage bitmap */
    table_size = BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
    POL_MEM_ALLOC(
        bk_svm_info->offset_mode_intf_bitmap,
        SHR_BITALLOCSIZE(table_size),
        "Offset Mode intf_bitmap");
    if (bk_svm_info->offset_mode_intf_bitmap == NULL) {
        bcmi_esw_global_meter_cleanup(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to allocate memory for "
                              "offset_mode_intf_bitmap\n")));
        return BCM_E_MEMORY;
    }
    ...
    bk_svm_info->valid_offset_mode_start = 0;
    bk_svm_info->valid_offset_mode_end = 0;
    bk_svm_info->empty_offset_mode_cnt = table_size;
    bk_svm_info->empty_offset_mode_start = 0;
    bk_svm_info->empty_offset_mode_end = table_size - 1;

    if (bk_svm_info->mutex == NULL) {
        bk_svm_info->mutex = sal_mutex_create("Global meter bk mutex");
        if (bk_svm_info->mutex == NULL) {
            bcmi_esw_global_meter_cleanup(unit);
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
                                  "Unable to create bk mutex\n")));
            return BCM_E_MEMORY;
        }
    }
    ...
    /* Allocate policer offset mode instance. */
    table_size = BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE;
    POL_MEM_ALLOC(
        bk_svm_info->offset_mode_v,
        table_size * sizeof(bcmi_policer_svm_offset_mode_bk_t),
        "offset mode instance");
    if (NULL == bk_svm_info->offset_mode_v) {
        bcmi_esw_global_meter_cleanup(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
                              "Unable to allocate memory for "
    ...
    for (i = 0; i < BCMI_POLICER_SVC_METER_MAX_OFFSET_MODE; i++) {
        bcmi_policer_svm_offset_mode_bk_t_init(
            &(bk_svm_info->offset_mode_v[i]));
    }

    /* Allocate meter action management data structure */
    action_size = BCMI_POLICER_SVC_METER_ACTION_SIZE;
    ...
    POL_MEM_ALLOC(
        BCMI_ACTION_BK_GET(unit),
        action_size * sizeof(bcmi_policer_global_meter_action_bk_t),
    ...
    if (shr_aidxres_list_create(
                      &BCMI_ACTION_LIST_HANDLE_GET(unit),
                      0, (action_size - 1),
                      0, (action_size - 1),
                      8,
    ...
    rv = shr_aidxres_list_reserve_block(
                          BCMI_ACTION_LIST_HANDLE_GET(unit), 0, 1);
    if (BCM_FAILURE(rv)) {
        bcmi_esw_global_meter_cleanup(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
    ...
    size_bank = BCMI_POLICER_SVC_METER_POOL_SIZE;
    table_size = BCMI_POLICER_SVC_METER_TCAM_SIZE;
    POL_GET_LOG2(table_size, block_factor);
    ...
    if (shr_aidxres_list_create(
          ...
          0, (size_bank - 1),
          block_factor,
          "global_meter_policer") != BCM_E_NONE)
    {
        bcmi_esw_global_meter_cleanup(unit);
        LOG_ERROR(BSL_LS_BCM_POLICER,
                  (BSL_META_U(unit,
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (SOC_WARM_BOOT(unit)) {
        rv = bcmi_esw_global_meter_reinit(unit);
        if (BCM_FAILURE(rv)) {
            bcmi_esw_global_meter_cleanup(unit);
            LOG_ERROR(BSL_LS_BCM_POLICER,
                      (BSL_META_U(unit,
...
#endif /* BCM_WARM_BOOT_SUPPORT */
    global_meter_status[unit].initialised = TRUE;

    LOG_VERBOSE(BSL_LS_BCM_POLICER,
                (BSL_META_U(unit,
                            "GLOBAL METER FEATURE initialised\n")));
    /*
     * Init default HW values
     */
    bcm_policer_global_meter_config_t_init(&global_meter_config_v);

    /*
     * Enable Global Meter Inter Frame Gap
     * IFG value must be configured by switch API
     * bcmSwitchMeterAdjustInterframeGap (per port)
     */
    global_meter_config_v.ifg_enable = 1;

    /* Set order of source tables in the case of conflict source happened. */
    global_meter_config_v.source_order_count = 7;
    global_meter_config_v.source_order[0] =
        bcmPolicerGlobalMeterSourceFieldStageLookup;
    global_meter_config_v.source_order[1] =
        bcmPolicerGlobalMeterSourceDstMac;
    global_meter_config_v.source_order[2] =
        bcmPolicerGlobalMeterSourceSrcMac;
    global_meter_config_v.source_order[3] =
        bcmPolicerGlobalMeterSourceVlanTranslateFirst;
    global_meter_config_v.source_order[4] =
        bcmPolicerGlobalMeterSourceVlanTranslateSecond;
    global_meter_config_v.source_order[5] =
        bcmPolicerGlobalMeterSourceVlan;
    global_meter_config_v.source_order[6] =
        bcmPolicerGlobalMeterSourcePort;

    /* Sets Global Meter Configuration */
    bcm_esw_policer_global_meter_config_set(unit, &global_meter_config_v);
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_get_policer_id_from_index_offset(
                unit, index, offset_mode, pid);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_table_index_get(unit, policer, index);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_cleanup(unit);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_init(unit);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
/*
 * Function:
 *     _bcm_esw_is_global_policer
 * Purpose:
 *     Validate the given policer id
 * Parameters:
 *     Unit              - (IN) unit number
 *     policer           - (IN) policer Id
 *     is_global_policer - (OUT) is globle policer or not
 * Returns:
 *     BCM_E_XXX
 */
...
{
    POL_CHECK_NULL_PARAMETER(unit, is_global_policer);
    *is_global_policer = FALSE;

#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        int rv = bcmi_esw_policer_validate(unit, policer_id);
        if (BCM_SUCCESS(rv)) {
            *is_global_policer = TRUE;
        } else {
            *is_global_policer = FALSE;
        }
    }
    ...
    if (SOC_IS_KATANAX(unit) || SOC_IS_TRIUMPH3(unit) || SOC_IS_APACHE(unit)) {
        ...
        offset_mode = ((policer_id & BCM_POLICER_GLOBAL_METER_MODE_MASK) >>
                       BCM_POLICER_GLOBAL_METER_MODE_SHIFT);
        *is_global_policer = (offset_mode) ? TRUE : FALSE ;
    }
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        POL_CHECK_NULL_PARAMETER(unit, policer);
        return bcmi_esw_policer_validate(unit, *policer);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_increment_ref_count(unit, policer);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_decrement_ref_count(unit, policer);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_add_policer_to_table(unit,
                  policer, table, index, data);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_offset_mode_get(unit,
                  policer_id, offset_mode);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
    return rv;
}
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_get_policer_from_table(unit,
                  table, index, data, policer, skip_read);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_delete_policer_from_table(unit,
                  policer, table, index, data);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_create(unit, action_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_add(unit, action_id, action, param0);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_destroy(unit, action_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_get(unit,
                policer_id, pol_cfg);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_set(unit,
                policer_id, pol_cfg);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_destroy_all(unit);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_traverse(unit, cb, user_data);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_destroy(unit, policer_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_envelop_create(unit,
              flags, macro_flow_policer_id, policer_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        rv = bcmi_esw_policer_group_create(unit, mode, &flow_info, NULL,
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        rv = bcmi_esw_policer_group_create(unit, mode, &flow_info, offset_map,
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_envelop_group_create(unit,
            flag, mode, macro_flow_policer_id, policer_id, npolicers);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_group_mode_id_create(unit,
                  flags, type, total_policers, num_selectors,
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_group_mode_id_destroy(unit, mode_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_group_mode_id_get(unit,
                  mode_id, num_selectors, flags, type, total_policers,
                  attr_selectors, actual_num_selectors);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_custom_group_create(unit,
            flags, mode_id, macro_flow_policer_id, policer_id, npolicers);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_group_get(unit,
            base_policer_id, member_max, member_array, member_count);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_attach_get(unit, policer_id, action_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_detach(unit, policer_id, action_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_policer_action_attach(unit, policer_id, action_id);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
    ...
    bcm_policer_global_meter_config_t *config)
{
    ...
    /* Parameter NULL error handling */
    POL_CHECK_NULL_PARAMETER(unit, config);

    (void)bcmi_global_meter_dump_config(unit, "[set]", config);
    ...
    gm_dev_info = BCM_SVM_DEV_INFO_GET(unit);
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->ifg_set);
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->source_order_set);
    ...
    bcm_policer_global_meter_config_t *config)
{
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->ifg_get);
    ...
    /* Check if the chip specific function existed */
    POL_CHECK_NULL_DEV_INSTANCE(unit, gm_dev_info->source_order_get);
    ...
    GLOBAL_METER_UNLOCK(unit);
    (void)bcmi_global_meter_dump_config(unit, "[get]", config);
...
#if defined(BCM_GLOBAL_METER_V2_SUPPORT)
    if (soc_feature(unit, soc_feature_global_meter_v2)) {
        return bcmi_esw_global_meter_policer_sync(unit);
    }
#endif /* BCM_GLOBAL_METER_V2_SUPPORT */
