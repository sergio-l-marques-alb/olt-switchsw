    ...
    if (endpoint_info->lm_counter_base_id > 0) {
        ...
        if (set_value) {
            ...
            for (crps_index = 0; crps_index < SOC_DPP_DEFS_GET(unit, nof_counter_processors); crps_index++) {
                rv = bcm_dpp_counter_config_get(unit, crps_index, &enabled, &src_core, &src_type, &command_id, &command_base_offset,
                        &eg_type, &format, &we_val, &custom_mode_params, &q_set_size, &stag_lsb, &counter_profile_offset);
                BCMDNX_IF_ERR_EXIT(rv);

                if (!enabled || (src_type != SOC_TMC_CNT_SRC_TYPE_INGRESS_OAM && src_type != SOC_TMC_CNT_SRC_TYPE_EGRESS_OAM)) {
                    continue;
                }
                                            ...
                                            ARAD_PP_OAM_DEFAULT_EP_ID default_id)
{
	...
	if (SOC_IS_ARAD_B1_AND_BELOW(unit)) { /* Arad default profile */
        ...
        rv = _bcm_dpp_endpoint_counters_set(unit, endpoint_info_lcl, _BCM_OAM_INVALID_LIF, _BCM_OAM_INVALID_LIF, 1);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    int is_sat_loopback)
{
    ...
    if (is_mip) {
        ...
        if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
            ...
            sal_memcpy(&profile_data_passive, &profile_data, sizeof(SOC_PPC_OAM_LIF_PROFILE_DATA));
            SOC_PPC_OAM_LIF_PROFILE_DATA_clear(&old_profile_data_passive);
            sal_memcpy(&old_profile_data_passive, &old_profile_data, sizeof(SOC_PPC_OAM_LIF_PROFILE_DATA));
        } else {
            SOC_PPC_OAM_LIF_PROFILE_DATA_clear(&profile_data_passive);
...
int _bcm_oam_lm_dm_remove_shared(int unit,bcm_oam_endpoint_t endpoint_id , int is_lm, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (_BCM_OAM_MEP_INDEX_IS_SERVER_CLIENT_SIDE(endpoint_id) == 0) {
        ...
        if (is_lm) {
            ...
            eth1731_profile_data.lmm_offset = 0;
            eth1731_profile_data.lmr_offset = 0;
            ...
            eth1731_profile_data.dmm_offset = 0;
            eth1731_profile_data.dmr_offset = 0;
...
int sat_loopback_add_rx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    /*  LB trap ID used by both DownMEP as well as UPMEP. */
...
int bcm_petra_oam_tst_rx_add(int unit,bcm_oam_tst_rx_t *tst_ptr) {
    ...
    /* TST trap ID used by both UPMEP and DOWNMEP */
...
int  bcm_petra_oam_sd_sf_detection_add(int unit,bcm_oam_sd_sf_detection_t  *sd_sf_ptr)
{
    ...
    for( sd_sf_profile_id = 0; sd_sf_profile_id < SOC_DPP_DEFS_GET(unit,oamp_number_of_sd_sf_profiles); sd_sf_profile_id++){
       soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_oamp_sd_sf_profile_get,(unit,sd_sf_profile_id, &sd_sf_profile));
       BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
       BCM_DPP_UNIT_CHECK(unit);
       if ((sd_sf_profile.sd_set_thresh == sd_sf_ptr->sd_set_threshold)
          ...
          &&(sd_sf_profile.alert_method == (sd_sf_ptr->flags&BCM_OAM_SD_SF_FLAGS_ALERT_METHOD))
          &&(sd_sf_profile.supress_alerts == ((sd_sf_ptr->flags&BCM_OAM_SD_SF_FLAGS_ALERT_SUPRESS) >> 1))){
          profile_found = 1;
          break;
       }
   }
   ...
   /*if profile found, don't need to allocate new profile, just use old one*/
    if(!profile_found){
                                                ...
                                                void *user_data)
{
    ...
    int                     rv;
    uint8                   mdl, found, is_mip, found_count = 0;
