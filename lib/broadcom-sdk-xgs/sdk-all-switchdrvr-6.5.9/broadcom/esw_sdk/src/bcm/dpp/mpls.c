    ...
    int unit)
{
    ...
    if ((SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp) {
        ARAD_EGR_PROG_EDITOR_PROGRAMS prge_program;
        ...
        if ((SOC_DPP_CONFIG(unit)->arad->init.ports.is_stacking_system == 0x1) || (SOC_DPP_CONFIG(unit)->arad->init.ports.add_dsp_extension_enable)) {
            prge_program = ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_JER_MC;
        }
        else {
            prge_program = ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_JER_OAMTS;
        }
