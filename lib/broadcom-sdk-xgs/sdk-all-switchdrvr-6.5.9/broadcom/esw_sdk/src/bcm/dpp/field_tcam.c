            ...
            switch (propActs[bcmIndex].bcmType) {
                ...
                case bcmFieldActionExternalValue6Set:
                    ...
                    if (propActs[bcmIndex].bcmParam1) {
						...
						if(SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311){
                        	hwAct[ppdIndex + 1].hwType = SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_6;
                        	hwAct[ppdIndex + 1].hwParam = propActs[bcmIndex].bcmParam1;
                        	/* two hardware actions are used here */
                        	actCount = 2;
						}else
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
						if(SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311){
                        	hwAct[ppdIndex + 1].hwType = SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_7;
                        	hwAct[ppdIndex + 1].hwParam = propActs[bcmIndex].bcmParam1;
                        	/* two hardware actions are used here */
                        	actCount = 2;
						}else
                            ...
                            *param1 = 0;
							...
							if ((index < SOC_PPC_FP_NOF_ACTIONS_PER_DB_MAX-1) && SOC_IS_JERICHO(unit)&&
								((entInfo.actions[index].type == SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_1)||
								 ...
								 (entInfo.actions[index].type == SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_6)||
								 (entInfo.actions[index].type == SOC_PPC_FP_ACTION_TYPE_FLP_ACTION_7))
								) {
                                *param1 = entInfo.actions[index+1].val;
                            }
                        }
