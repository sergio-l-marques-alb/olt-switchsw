/*----------------------------------------------------------------------
 * $Id: temod16_cfg_seq.c,v 1.6 Broadcom SDK $
...
int temod16_pmd_lock_get(PHYMOD_ST* pc, int* lockStatus)
{
...
int temod16_autoneg_set(PHYMOD_ST* pc,
    temod16_an_ability_t *an_ability_st) /* AUTONEG_SET */
 {
  ...
  if (an_ability_st->cl37_adv.an_bam_speed1)
      AN_X4_LOC_DEV_OVER1G_ABIL1r_SET(reg_over1g_abilities1, (an_ability_st->cl37_adv.an_bam_speed1 & 0xfff ));
  ...
  AN_X4_LOC_DEV_CL73_BASE_ABIL0r_FECf_SET(reg_cl73_base_abilities, ( an_ability_st->cl73_adv.an_fec & 3));
  PHYMOD_IF_ERR_RETURN(MODIFY_AN_X4_LOC_DEV_CL73_BASE_ABIL0r(pc, reg_cl73_base_abilities));
...
int temod16_autoneg_control(PHYMOD_ST* pc, temod16_an_control_t *an_control)
{
  ...
  switch (an_control->an_type) {
      ...
      */
      ...
      if (an_control->an_property_type & TEMOD16_AN_PROPERTY_ENABLE_SGMII_TO_CL37_AUTO )
  ...
  if (pmd_touched == 0) {
    ...
    if (pll_mode & 0x80000000) {
      ...
      PLL_CTL8r_I_NDIV_INTf_SET(reg_ctl8, (pll_ndiv) & 0x0000ffff);
       ...
       temod16_get_pll_ndiv(sc_pmd_entry[speed].pll_mode, &pll_ndiv);
...
#endif
#ifdef _SDK_TEMOD16_
int temod16_set_an_port_mode(PHYMOD_ST* pc, int num_of_lanes, int starting_lane, int single_port)    /* SET_AN_PORT_MODE */
{
  uint16_t rdata;
...
*/
int temod16_tx_loopback_control(PHYMOD_ST* pc, int enable, int starting_lane, int port_type)           /* TX_LOOPBACK_CONTROL  */
{
...
int _init_pcs_an(PHYMOD_ST* pc)             
{
  ...
  temod16_set_an_port_mode(pc, pc->no_of_lanes, pc->this_lane, pc->single_port);
...
int temod16_pll_config_get(const phymod_access_t *pa, temod16_pll_mode_type  *pll_mode)
{
     ...
     */ 
    switch (pll_div) {
        case 0x00000046UL: *pll_mode = TEMOD16_PLL_MODE_DIV_70;
            break;
        case 0x00000042UL: *pll_mode = TEMOD16_PLL_MODE_DIV_66;
            break;
        case 0x00000040UL: *pll_mode = TEMOD16_PLL_MODE_DIV_64;
            break;
        case 0x0000003CUL: *pll_mode = TEMOD16_PLL_MODE_DIV_60;
            break;
        case 0x0000005CUL: *pll_mode = TEMOD16_PLL_MODE_DIV_92;
            break;
        case 0x00000050UL: *pll_mode = TEMOD16_PLL_MODE_DIV_80;
            break;
...
int temod16_clause72_control(PHYMOD_ST* sa__, int enable)                /* CLAUSE_72_CONTROL */
{
    err_code_t __err;
    __err=ERR_CODE_NONE;
    __err=wr_cl72_ieee_training_enable(enable); if (__err) return(__err);
...
int temod16_osdfe_on_lkup_get(PHYMOD_ST* pc, temod16_spd_intfc_type spd_intf, uint32_t* osdfe_on)
{
    ...
    *osdfe_on = sc_pmd_entry[speed_id].osdfe_on;
...
int temod16_scrambling_dis_lkup_get(PHYMOD_ST* pc, temod16_spd_intfc_type spd_intf, uint32_t* scrambling_dis)
{
    ...
    temod16_get_mapped_speed(spd_intf, &speed_id);
    *scrambling_dis = sc_pmd_entry[speed_id].scrambling_dis;
...
{
    if (enable) {
        temod16_rx_squelch_set(pc, 0);
        temod16_tx_squelch_set(pc, 0);
    } else {
        temod16_rx_squelch_set(pc, 1);
        temod16_tx_squelch_set(pc, 1);
    }
...
int temod16_disable_get(PHYMOD_ST *pa, uint32_t enable ) { return 0; } 
...
int temod16_pll_reset_enable_set(PHYMOD_ST *pa , int enable) { return 0; }
...
int temod16_autoneg_get(PHYMOD_ST* pc, int* an_enabled, int* an_complete) { return 0; }
