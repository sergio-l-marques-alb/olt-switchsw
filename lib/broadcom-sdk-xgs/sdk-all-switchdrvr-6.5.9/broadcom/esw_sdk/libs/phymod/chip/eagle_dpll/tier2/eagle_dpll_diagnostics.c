...
#define EAGLE2PLL_PHY_ACCESS_PLLIDX_SET(_phy_access) \
    do{\
        ...
        (_phy_access)->access.pll_idx = eagle2_tsc2pll_get_current_pll(&((_phy_access)->access));\
    }while(0)
...
STATIC
int _eagle_dpll_prbs_poly_phymod_to_eagle(phymod_prbs_poly_t phymod_poly, enum srds_prbs_polynomial_enum *eagle_poly)
{
    switch(phymod_poly){
...
STATIC
int _eagle_dpll_prbs_poly_tsce_to_phymod(eagle_prbs_polynomial_type_t tsce_poly, phymod_prbs_poly_t *phymod_poly)
{
    switch(tsce_poly){
...
int eagle_dpll_phy_prbs_config_set(const phymod_phy_access_t* phy, uint32_t flags , const phymod_prbs_t* prbs)
{
    ...
    PHYMOD_IF_ERR_RETURN(_eagle_dpll_prbs_poly_phymod_to_eagle(prbs->poly, &eagle_poly));
...
int eagle_dpll_phy_prbs_config_get(const phymod_phy_access_t* phy, uint32_t flags , phymod_prbs_t* prbs)
{
    ...
    if (PHYMOD_PRBS_DIRECTION_TX_GET(flags)) {
        ...
        PHYMOD_IF_ERR_RETURN(_eagle_dpll_prbs_poly_tsce_to_phymod(tsce_poly, &config_tmp.poly));
...
int eagle_dpll_phy_pattern_enable_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    ...
    EAGLE2PLL_PHY_ACCESS_PLLIDX_SET(&phy_copy);
    PHYMOD_IF_ERR_RETURN
    (eagle2_tsc2pll_tx_shared_patt_gen_en_get(&phy_copy.access, &enable_8));
...
int eagle_dpll_core_diagnostics_get(const phymod_core_access_t* core, phymod_core_diagnostics_t* diag)
{
    ...
    EAGLE2PLL_PHY_ACCESS_PLLIDX_SET(&core_copy);
...
/* NOT SUPPORTED IN EAGLE YET
    ...
    */
    ...
    eagle2_tsc2pll_read_pll_range(&core_copy.access, &diag->pll_range);
...
int eagle_dpll_phy_diagnostics_get(const phymod_phy_access_t* phy, phymod_phy_diagnostics_t* diag)
{
    ...
    PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_osr_mode_get(&phy_copy.access, &osr_mode));
    PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_osr_mode_to_enum(osr_mode, &diag->osr_mode));
    PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_signal_detect(&phy_copy.access, &diag->signal_detect));
...
int eagle_dpll_phy_pmd_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    ...
    */
    ...
    if (!cmd_str)
        type = (uint32_t)TEMOD2PLL_DIAG_STATE;
    else if (!PHYMOD_STRCMP(cmd_str, "ber"))
        type = (uint32_t)TEMOD2PLL_DIAG_BER;
    else if (!PHYMOD_STRCMP(cmd_str, "config"))
        type = (uint32_t)TEMOD2PLL_DIAG_CFG;
    else if (!PHYMOD_STRCMP(cmd_str, "CL72"))
        type = (uint32_t)TEMOD2PLL_DIAG_CL72;
    else if (!PHYMOD_STRCMP(cmd_str, "debug"))
        type = (uint32_t)TEMOD2PLL_DIAG_DEBUG;
    else if (!PHYMOD_STRCMP(cmd_str,"STD"))
        type = (uint32_t)TEMOD2PLL_DIAG_DSC_STD;
    else
        type = (uint32_t)TEMOD2PLL_DIAG_STATE;
...
STATIC err_code_t eagle_dpll_phy_meas_lowber_eye (const phymod_access_t *pa,
                                             const phymod_phy_eyescan_options_t *eyescan_options,
                                             uint32_t *buffer)
{
    struct eagle2_tsc2pll_eyescan_options_st e_options;
    phymod_access_t pa_copy;

    PHYMOD_MEMCPY(&pa_copy, pa, sizeof(pa_copy));
    pa_copy.pll_idx = eagle2_tsc2pll_get_current_pll(&pa_copy);
    ...
    return (eagle2_tsc2pll_meas_lowber_eye(&pa_copy, e_options, buffer));
}
STATIC err_code_t eagle_dpll_phy_display_lowber_eye (const phymod_access_t *pa,
                                                const phymod_phy_eyescan_options_t *eyescan_options,
                                                uint32_t *buffer)
{
    struct eagle2_tsc2pll_eyescan_options_st e_options;
    ...
    return(eagle2_tsc2pll_display_lowber_eye (&pa_copy, e_options, buffer));
}


static void  _eagle_dpll_diag_uc_reg_dump(const phymod_access_t *pa)
{
...
STATIC int eagle_dpll_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32_t flags)
{
    ...
    for(j=0; j< PHYMOD_CONFIG_MAX_LANES_PER_CORE; j++) { /* Loop for all lanes. */
      ...
      if(PHYMOD_EYESCAN_F_ENABLE_GET(flags)) {
          PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_meas_eye_scan_start(&(phy_copy.access), 0));
      ...
      if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
          ...
          for (ii = 31; ii >= -31; ii--) {
              if (ii == 31) {
                  ...
                  PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_display_eye_scan_header(&(phy_copy.access), 1));
              }

              rc  = eagle2_tsc2pll_read_eye_scan_stripe(&(phy_copy.access), stripe, &status);
              if(rc != PHYMOD_E_NONE) {
                _eagle_dpll_diag_uc_reg_dump(&(phy_copy.access));
                PHYMOD_IF_ERR_RETURN(rc);
              }
              PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_display_eye_scan_stripe(&(phy_copy.access), ii, &stripe[0]));

              PHYMOD_DIAG_OUT(("\n"));
          }

          if (rc == PHYMOD_E_NONE) {
            PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_display_eye_scan_footer(&(phy_copy.access), 1));
            PHYMOD_DIAG_OUT(("\n"));
          }
      }

      if(PHYMOD_EYESCAN_F_DONE_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_meas_eye_scan_done(&(phy_copy.access)));
      }
    }
...
STATIC int eagle_dpll_diagnostics_eyescan_run_lowber(
    ...
    )
{
    ...
    if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
        ...
        rv = eagle_dpll_phy_meas_lowber_eye (&(phy_copy.access), eyescan_options, buffer);
        if (rv < 0) {
            goto cleanup;
        }
        rv = eagle_dpll_phy_display_lowber_eye (&(phy_copy.access), eyescan_options, buffer);
        if (rv < 0) {
            goto cleanup;
        }
    }
    ...
    if(PHYMOD_EYESCAN_F_DONE_GET(flags)) {
        PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_pmd_uc_cmd(&(phy_copy.access), CMD_CAPTURE_BER_END, 0, 2000));
    }
...
int eagle_dpll_diagnostics_eye_margin_proj( const phymod_phy_access_t* phy, uint32_t flags,
                                        const phymod_phy_eyescan_options_t* eyescan_options) {
  ...
  if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
      ...
      for (i = 0; i < num_lane; i++) {
          ...
          if (found == 0) {
              PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_get_uc_core_config (&(phy_copy.access), &core_cfg));
              PHYMOD_IF_ERR_RETURN(eagle2_tsc2pll_osr_mode_get(&phy_copy.access, &osr_mode));
          ...
          eagle2_tsc2pll_eye_margin_proj(&phy_copy.access, data_rate, eyescan_options->ber_proj_scan_mode,
...
int eagle_dpll_phy_eyescan_run(const phymod_phy_access_t* phy, uint32_t flags, phymod_eyescan_mode_t mode, const phymod_phy_eyescan_options_t* eyescan_options)
{
    ...
    switch(mode) {
        case phymodEyescanModeFast:
            return eagle_dpll_diagnostics_eyescan_run_uc(phy, flags);
        case phymodEyescanModeLowBER:
            return eagle_dpll_diagnostics_eyescan_run_lowber(phy, flags, eyescan_options);
        case phymodEyescanModeBERProj:
            return eagle_dpll_diagnostics_eye_margin_proj(phy, flags, eyescan_options);
        default:
