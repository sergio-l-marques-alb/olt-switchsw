 ...
 *
 */
bool cmicx_sbus_dma_update(cmicx_sbus_dma_data_t *sbus_dma_data)
{
    ...
    /* if we have an access running, then handle descriptor requesting, counter setup/decrement and command completion if necessary. */
        ...
        if ((sbus_dma_data->mode==CMIC_SBUS_DMA_DESCRIPTOR_MODE) && (sbus_dma_data->sequence_pending==false)) {
            ...
            do {
                ...
                cmicx_pcid_mem_read(sbus_dma_data->cur_desc_address,(void *)mem,CMIC_SBUS_DMA_WORDS_PER_DESCRIPTOR*CMICX_BYTES_PER_WORD);
        ...
        if (sbus_dma_data->sbus_command_countdown==0) {
            ...
            if (sbus_dma_data->pend_clocks_countdown==0) {
                ...
                if (sbus_dma_data->command_is_write==true) {
                    ...
                    cmicx_pcid_mem_read(temp_address,(void *)&(command[2]),(sbus_dma_data->req_words*CMICX_BYTES_PER_WORD));
                ...
                if (cmic_schan_sbus_command_complete(command,ack_command,((sbus_dma_data->command_is_write==false) ? sbus_dma_data->resp_words : 0),&num_words)==true) {
                    ...
                    if (sbus_dma_data->command_is_write==false) {
                        ...
                        if (sbus_dma_data->dma_wr_to_nullspace==false) {
                            cmicx_pcid_mem_write(temp_address,(void *)&(ack_command[1]),(ack_num_words-1)*CMICX_BYTES_PER_WORD);
