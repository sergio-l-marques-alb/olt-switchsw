...
/* Retrieves connection setup between two nodes. */
...
/* Retrieve Link enable. */
extern int bcm_fabric_crossbar_enable_get(
    int unit, 
    uint64 *xbars);

/* Retrieve runtime Link enable configuration. */
...
/* Retrieve packet length adjustment. */
...
/* Retrieve failover/APS configuration. */
...
#define BCM_FABRIC_MAX_MULTICAST_MODULES    (144)      /* Max number of mosules
                                                          in
...
/* Initialize a fabric link remote pipe mapping structure. */
...
/* Set fifo_type thresholds per pipe */
...
/* Get fifo_type thresholds per pipe */
...
/* 
 * Set fabric bandwidth profile. For multi-core systems, this is the
 * 'shared' profile. Each link has entries corresponding to all the rci
 * levels. Thus table size is the (number of links) * (rci levels)
 */
...
/* 
 * Retrieve fabric bandwidth profile. For multi-core systems, this is the
 * 'shared' profile. Each link has entries corresponding to all the rci
 * levels. Thus table size is the (number of links) * (rci levels)
 */
...
/* 
 * Set fabric bandwidth profile per core (for multi-core systems only).
 * Each link has entries corresponding to all the rci levels. Thus table
 * size is the (number of links) * (rci levels)
 */
...
/* 
 * Retrieve fabric bandwidth profile per core (for multi-core systems
 * only). Each link has entries corresponding to all the rci levels. Thus
 * table size is the (number of links) * (rci levels)
 */
...
/* 
 * Retrieve port editing configuration. flags field has to be specifed.
...
/* Retrieve port routing configuration. */
...
/* Set PCP mode per VOQ/destination device. */
...
/* Get PCP mode per VOQ/destination device. */
...
/* Set minimum number of links per destination device. */
...
/* Get minimum number of links per destination device. */
...
/* Set the replications of the related to each port */
