 ...
 *  Revision      :  $Id: merlin_tx_analog_functions.c 775 2014-11-11 04:37:38Z kirand $ *
 *                                                                                       *
...
err_code_t merlin_sesto_apply_txfir_cfg( const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2) {
  ...
  err_code_t failcode = merlin_sesto_validate_txfir_cfg( pa, pre, main, post1, post2);

  if (!failcode) {
    wr_ams_tx_post2to1(post2==0);           /* Convert post2 slices to post1, if post2 is 0 */
    wr_ams_tx_en_pre(pre>0);                /* Enable the TXFIR blocks */
    wr_ams_tx_en_post1(post1>0);
    wr_ams_tx_en_post2(post2>0);
    wr_txfir_pre_override((uint8_t)pre);
    wr_txfir_main_override((uint8_t)main);    
    wr_txfir_post_override((uint8_t)post1);
    wr_txfir_post2((uint8_t)post2);

    if ((pre+main+post1+post2) > 45) { 
      wr_ams_tx_refcalshunt(5); 
    } else {
      wr_ams_tx_refcalshunt(4);
    }
  }
...
err_code_t merlin_sesto_config_tx_hpf( const phymod_access_t *pa, uint8_t val) {
...
err_code_t merlin_sesto_rd_tx_hpf_config( const phymod_access_t *pa, uint8_t *pval) {
