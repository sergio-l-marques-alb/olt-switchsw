...
err_code_t merlin16_tsc_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {

  char       patt_final[245] = "";
  char       patt_mod[245]   = "", bin[5] = "";
  uint8_t    str_len, i, k, j = 0;
  uint8_t    offset_len, actual_patt_len = 0, hex = 0;
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by merlin16_tsc_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by merlin16_tsc_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(merlin16_tsc_INTERNAL_calc_patt_gen_mode_sel(&mode_sel,&zero_pad_len,patt_length));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    /* Hexadecimal Pattern */
    for (i=2; i < str_len; i++) {
      EFUN(merlin16_tsc_INTERNAL_compute_bin(pattern[i],bin));
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(merlin16_tsc_INTERNAL_compute_hex(bin, &hex));
...
err_code_t merlin16_tsc_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length) {
  uint8_t zero_pad_len = 0; /* suppress warnings, changed by merlin16_tsc_INTERNAL_calc_patt_gen_mode_sel() */
  uint8_t mode_sel     = 0; /* suppress warnings, changed by merlin16_tsc_INTERNAL_calc_patt_gen_mode_sel() */
...
err_code_t merlin16_tsc_config_tx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv) {
...
err_code_t merlin16_tsc_get_tx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
    uint8_t val;
...
err_code_t merlin16_tsc_tx_prbs_en(srds_access_t *sa__, uint8_t enable) {
...
err_code_t merlin16_tsc_get_tx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
...
err_code_t merlin16_tsc_tx_prbs_err_inject(srds_access_t *sa__, uint8_t enable) {
  /* PRBS Error Insert.
...
err_code_t merlin16_tsc_config_rx_prbs(srds_access_t *sa__, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
...
/* get PRBS Checker */
err_code_t merlin16_tsc_get_rx_prbs_config(srds_access_t *sa__, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
  uint8_t val;
...
err_code_t merlin16_tsc_rx_prbs_en(srds_access_t *sa__, uint8_t enable) {
...
err_code_t merlin16_tsc_get_rx_prbs_en(srds_access_t *sa__, uint8_t *enable) {
...
err_code_t merlin16_tsc_prbs_chk_lock_state(srds_access_t *sa__, uint8_t *chk_lock) {
    if(!chk_lock) {
...
err_code_t merlin16_tsc_prbs_err_count_ll(srds_access_t *sa__, uint32_t *prbs_err_cnt) {
    uint16_t rddata;
...
err_code_t merlin16_tsc_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    if(!prbs_err_cnt || !lock_lost) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    EFUN(merlin16_tsc_prbs_err_count_ll(sa__, prbs_err_cnt));
    *lock_lost    = (*prbs_err_cnt >> 31);
    *prbs_err_cnt = (*prbs_err_cnt & 0x7FFFFFFF);
    return (ERR_CODE_NONE);
}

err_code_t merlin16_tsc_display_detailed_prbs_state_hdr(void) {
...
err_code_t merlin16_tsc_display_detailed_prbs_state(srds_access_t *sa__) {
    uint32_t err_cnt = 0;
    uint8_t lock_lost = 0;
    uint8_t enabled;

    ESTM_PRINTF(("  %d ",merlin16_tsc_get_lane(sa__)));
    ...
    EFUN(merlin16_tsc_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
