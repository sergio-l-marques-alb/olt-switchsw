...
int read_madura_pll_mode(const phymod_access_t *pa, uint16_t ip, uint16_t alternate_core)
{
    ...
    PHYMOD_IF_ERR_RETURN (
              _madura_set_slice_reg (pa, 
                        ip, MADURA_DEV_PMA_PMD,(alternate_core==0)?1:4 ));
    PHYMOD_IF_ERR_RETURN (
...
int _madura_config_dut_mode_reg(const phymod_phy_access_t *pa, const phymod_phy_inf_config_t* config) 
{
    ...
    if (config->data_rate != MADURA_SPD_100G &&
        ...
        for (port_idx=0; port_idx < no_of_ports; port_idx++) {
           ...
           if (!is_lane_used) {
            ...
            if (((config->data_rate == MADURA_SPD_40G  || config->data_rate == MADURA_SPD_42G || config->data_rate == MADURA_SPD_50G ) && (port_idx == 0)) ||
                 ((config->data_rate == MADURA_SPD_10G || config->data_rate == MADURA_SPD_11G || config->data_rate == MADURA_SPD_25G) && (port_idx < 2))) {
		...
		if (config->data_rate == MADURA_SPD_25G) { /*and change pll*/
...
int _madura_pll_config_state_get(const phymod_phy_access_t *phy, const phymod_phy_inf_config_t* config, uint16_t *pll_cfg_state)
{
    ...
    for (ip = MADURA_2X_FALCON_CORE; ip <= MADURA_FALCON_CORE; ip++) {
		MADURA_IF_ERR_RETURN_FREE(cfg.device_aux_modes, (
              ...
              ((ip == MADURA_2X_FALCON_CORE) && (lane_mask & 0xF0)) ?  4 : 0)));
        ...
        if (ip == MADURA_FALCON_CORE) {
            fal_pll_lock    = BCMI_MADURA_PMD_PLL_STSr_PMD_PLL0_LOCKf_GET(pll_sts);
        } else {
            fal_2x_pll_lock = BCMI_MADURA_PMD_PLL_STSr_PMD_PLL0_LOCKf_GET(pll_sts);
        }
    }
    /*
    ...
    if (((fal_pll_lock != 1) || (fal_2x_pll_lock != 1)) || (config->ref_clock != cfg.ref_clock) ||
        ...
        ((cfg.data_rate == MADURA_SPD_100G || cfg.data_rate == MADURA_SPD_106G) &&
         (config->data_rate != MADURA_SPD_100G || config->data_rate != MADURA_SPD_106G)))) {
        *pll_cfg_state = 1;
    }else if(config->data_rate != cfg.data_rate){
    ...
    if (ip == MADURA_FALCON_CORE) {
        if (MADURA_MIDX_IS_4X25G_PLUS(mindex0)) {
            ...
            switch(mindex1) {
               ...
               case MADURA_MIDX_10GPT_1:
                    if(MADURA_PLL_MODE_165 == read_madura_pll_mode(pa,ip,0)){
        ...
        if (MADURA_MIDX_IS_4X25G_PLUS(mindex0)) {
            ...
            switch(mindex1) {
               ...
               /* coverity[fallthrough] */
               case MADURA_MIDX_10GPT_1:
                    if(MADURA_PLL_MODE_165 == read_madura_pll_mode(pa,ip,1)){
                    config->data_rate = MADURA_SPD_25G;
		    }else{
                    config->data_rate = MADURA_SPD_10G;
		    }
...
int _madura_force_tx_training_status_get(const phymod_access_t *pa, phymod_cl72_status_t* status)
{
    ...
    for (lane = 0; lane < max_lane; lane ++) {
        if (lane_mask & (1 << lane)) {
            MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
                _madura_set_slice_reg (pa, ip, MADURA_DEV_PMA_PMD, lane)));
...
int _madura_tx_set(const phymod_access_t *phy, const phymod_tx_t* tx)
{
    ...
    for(lane = 0; lane < max_lane; lane ++) {
	    if (lane_mask & (1 << lane)) {
		    ...
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_PRE, tx->pre)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_MAIN, tx->main)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_POST1, tx->post)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_POST2, tx->post2)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_POST3, tx->post3)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_write_tx_afe(pa, TX_AFE_AMP, tx->amp)));
...
int _madura_tx_get(const phymod_access_t *phy, phymod_tx_t* tx)
{
    ...
    for(lane = 0; lane < max_lane; lane ++) {
	    if (lane_mask & (1 << lane)) {
		    ...
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_PRE, &tx->pre)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_MAIN, &tx->main)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_POST1, &tx->post)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_POST2, &tx->post2)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_POST3, &tx->post3)));
		    MADURA_IF_ERR_RETURN_FREE(config.device_aux_modes, (
		    falcon2_madura_read_tx_afe(pa, TX_AFE_AMP, &tx->amp)));
	 }
    }
    /*Reset DEV1 slice register*/	     	
    PHYMOD_FREE(config.device_aux_modes);
