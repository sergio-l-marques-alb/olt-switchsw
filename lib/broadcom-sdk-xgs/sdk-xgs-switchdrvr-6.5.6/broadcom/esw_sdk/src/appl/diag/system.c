...
diag_shell(void)
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        {
            cmd_dnx_verify(SOC_NDEV_IDX2DEV(i));
        }
#endif
#if !defined(__KERNEL__)
        if (SOC_IS_DPP(SOC_NDEV_IDX2DEV(i)) && !SOC_IS_DNX_TEST_DEVICE(SOC_NDEV_IDX2DEV(i))) {
            if(appl_dpp_diag_sig_init(SOC_NDEV_IDX2DEV(i)) == NULL) {
                LOG_VERBOSE(BSL_LS_APPL_SHELL, (BSL_META_U(SOC_NDEV_IDX2DEV(i), "Signal DB was not initialized\n")));
            }
        }
#endif
#endif 
