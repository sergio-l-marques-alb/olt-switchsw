/*
 * $Id: tksdiag.c,v 1.88 Broadcom SDK $
...
ct_cpudb(int unit, args_t *args)
{
    ...
    if (!bcmx_registered) {
        ...
        if ((rv = bcm_stack_attach_register(ct_attach_callback)) < 0) {
            cli_out("Could not register BCMX with stack attach: %s\n",
                    bcm_errmsg(rv));
        } else {
    ...
    if ((sal_strcasecmp(subcmd, "NEWDB") == 0) ||
        ...
        if (bcmx_registered) {
            ...
            if ((rv = bcm_stack_attach_unregister(ct_attach_callback)) < 0) {
                cli_out("Could not unregister BCMX with stack attach: %s\n",
                        bcm_errmsg(rv));
            } else {
                cli_out("cpudb BCMX callback unregistered\n");
                bcmx_registered = FALSE;
            }
        }
#else
        cli_out("cpudb: BCMX is not enabled\n");
#endif
    } else {
