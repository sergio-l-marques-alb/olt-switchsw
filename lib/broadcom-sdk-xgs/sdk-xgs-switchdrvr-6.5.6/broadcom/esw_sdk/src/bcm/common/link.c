...
typedef struct _ls_control_s {
    ...
    int               link_fault_chk_en;  /* Enable Link fault checking in Linkscan thread */
 ...
 */
STATIC int
_bcm_link_fault_get(int unit, int port, int *fault)
{
    int rv, speed;
    uint32 flags;
    int value;
...
_bcm_linkscan_update_port(int unit, int port)
{
    ...
    int                 rv;
    int                 cur_fault = FALSE, new_fault = FALSE;
    ...
    cur_fault = SOC_PBMP_MEMBER(lc->pbm_remote_fault, port);
    ...
    if (new_link && (lc->link_fault_chk_en == 1) && BCM_PBMP_MEMBER(lc->pbm_sw, port)) {
        rv = _bcm_link_fault_get(unit, port, &new_fault);
        if (rv == BCM_E_UNAVAIL) {
            rv = BCM_E_NONE;
    ...
    if (new_link) {
#ifdef BCM_CALADAN3_SUPPORT
        /* clear up remote/local fault, do this for C3 only for now */
        if (SOC_IS_CALADAN3(unit)) {
    ...
    info.linkstatus = new_fault ? BCM_PORT_LINK_STATUS_REMOTE_FAULT : new_link;
...
_bcm_linkscan_thread(int unit)
{
    ...
    while ((interval = lc->interval_us) != 0) {
        ...
        if (BCM_PBMP_IS_NULL(lc->pbm_sw) &&
            BCM_PBMP_IS_NULL(lc->pbm_remote_fault)) {
...
bcm_common_linkscan_mode_set_pbm(int unit, pbmp_t pbm, int mode)
{
    ...
    PBMP_ITER(pbm, port) {
    	  if (SOC_IS_QUX(unit)) {
            /*For qux phy 9-36 is in PM4x2P5 which is currently not implemented */
            int phy;
