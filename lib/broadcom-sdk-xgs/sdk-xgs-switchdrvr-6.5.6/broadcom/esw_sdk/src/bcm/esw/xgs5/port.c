 ...
 */
STATIC int
_bcmi_xgs5_port_encap_validate(int unit, int encap)
{
    uint32 encap_mask;
    ...
    case BCM_PORT_ENCAP_HIGIG2_IP_GRE:
        ...
        break;
    case BCM_PORT_ENCAP_SBX:
        encap_mask = BCM_PA_ENCAP_SBX;
        break;
                                        ...
                                        bcm_port_resource_t *resource)
{
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (pr->physical_port != -1) {
            ...
            /* Check Encapsulation */
            BCM_IF_ERROR_RETURN
                (_bcmi_xgs5_port_encap_validate(unit, pr->encap));
        }
    }
                                ...
                                int *op)
{
    ...
    soc_info_t *si = &SOC_INFO(unit);
    int lport, pport, pre_pport;
    pbmp_t att_pbm, remap_pbm, lane_pbm, speed_pbm;
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    int speed, lanes, cur_encap, encap;
    int rv;
    bcm_port_if_t intf;
    ...
    asf_prof = soc_property_get(unit, spn_ASF_MEM_PROFILE,
                                BCMI_PORT_DRV_DEV_INFO(unit)->asf_prof_default);
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        /* If the port is attached to PHY_NULL, use encap stored in SOC_INFO.
         * Else, get the encap from MAC/PHY */
        if (BCM_SUCCESS(rv) && (intf != SOC_PORT_IF_NULL)) {
            rv = bcm_esw_port_encap_get(unit, port, &cur_encap);
            if (BCM_SUCCESS(rv)) {
                encap = cur_encap;
            }
        }
        ...
        in_port_map->port_num_lanes[port] = si->port_num_lanes[port];
    }
                                ...
                                soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0, pr = port_schedule_state->resource; i < nport; i++, pr++) {
        ...
        if (pr->op & (BCMI_XGS5_PORT_RESOURCE_OP_DEL |
            ...
            if (bcm_esw_linkscan_enable_port_get(unit, pr->logical_port) !=
                BCM_E_DISABLED) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    oversub_mode = SOC_PBMP_NOT_NULL(si->oversub_pbm) ? 1 : 0;
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        ...
        SOC_PBMP_ITER(si->pipe_pbm[pipe], logic_port) {
            ...
            bandwidth[pipe] += si->port_init_speed[logic_port];
        }
    }
    ...
    for (i = 0, pr = port_schedule_state->resource; i < nport; i++, pr++) {
        ...
        if (pr->physical_port == -1 &&
            ...
            bandwidth[pipe] -= si->port_init_speed[pr->logical_port];
        } else if (pr->physical_port != -1 &&
                   pr->op & (BCMI_XGS5_PORT_RESOURCE_OP_ADD|
                             BCMI_XGS5_PORT_RESOURCE_OP_REMAP |
                             BCMI_XGS5_PORT_RESOURCE_OP_ACTIVE)) {
            bandwidth[pr->pipe] += pr->speed;
        } else if (pr->physical_port != -1 &&
                   pr->op & (BCMI_XGS5_PORT_RESOURCE_OP_LANES|
                                 ...
                                 uint32 mask)
{
    soc_port_resource_t *pr;
    soc_port_map_type_t *post_pi = &port_schedule_state->out_port_map;
    int nport, check_ovs_lr, check_ovs_mix_speed, check_pll, check_mix_encap;
    int *sister_ports, len;
    int logic_port, phy_port, blk, bindex, btype_idx, first_p;
    int speed, first_speed, num_lanes, encap, first_encap, ovs, first_ovs;
    ...
    SHR_BIT_ITER(blk_bmp, SOC_MAX_NUM_BLKS, blk) {
        ...
        for (i = 0; i < BCMI_XGS5_PORTS_PER_PBLK; i++) {
            ...
            rv = soc_esw_portctrl_pll_div_get(unit, phy_port, speed, num_lanes,
                                              encap, &pll_div);
            if (BCM_FAILURE(rv)) {
                LOG_VERBOSE(BSL_LS_BCM_PORT,
            ...
            if (ovs && check_ovs_mix_speed && (first_speed != speed)) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                 ...
                                 pbmp_t del_pbm_log)
{
    ...
    SOC_PBMP_ITER(add_pbm_log, logic_port) {
        ...
        reg = is_clport ? CLPORT_CNTMAXSIZEr : XLPORT_CNTMAXSIZEr;
        rval = 0;
        ...
        reg = is_clport ? CLPORT_FLOW_CONTROL_CONFIGr :
                             ...
                             soc_port_schedule_state_t *port_schedule_state)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcmi_xgs5_port_soc_info_ptype_block_add(unit, port_schedule_state));

    /* Add counter map */
    BCM_IF_ERROR_RETURN
         (_bcmi_xgs5_port_soc_counter_add(unit, port_schedule_state));
                            ...
                            soc_port_schedule_state_t *port_schedule_state)
{
    ...
    /* Clear counter map */
    BCM_IF_ERROR_RETURN
        (_bcmi_xgs5_port_soc_counter_delete(unit, port_schedule_state));
    ...
    BCM_IF_ERROR_RETURN
        (_bcmi_xgs5_port_soc_info_ptype_block_delete(unit, port_schedule_state));
    ...
    for (pipe = 0; pipe < si->num_pipe; pipe++) {
        ...
        SOC_PBMP_ITER(pr_bmp->del_pbm_phy[pipe], local_phy_port) {
            ...
            for (port_idx = 0; port_idx < SOC_DRIVER(unit)->port_num_blktype;
                ...
                block_valid[blk] = si->block_valid[blk];
                if (!block_valid[blk]) {
                    return BCM_E_INTERNAL;
                }
                block_valid[blk]--;
                            ...
                            soc_port_schedule_state_t *port_schedule_state)
{
     ...
     */
    BCM_IF_ERROR_RETURN
        (_bcmi_xgs5_port_resource_detach(unit, port_schedule_state));
    ...
    BCM_IF_ERROR_RETURN
        (_bcmi_xgs5_port_resource_attach(unit, port_schedule_state));
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                     (BSL_META_U(unit,
                             "FlexPort operations 0x%x: %s %s %s %s %s %s %s\n"),
                      op,
...
bcmi_xgs5_port_attach_dscp(int unit, bcm_port_t port)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_dscp_table_entry_add(unit, entries_buf, DSCP_CODE_POINT_CNT,
                                   &dscp_index));
    BCM_IF_ERROR_RETURN
...
bcmi_xgs5_port_attach_frame_length_check(int unit, bcm_port_t port)
{
    ...
    if (reg != INVALIDr) {
        ...
        rval |= 1 << 18; /* set PFC frame detected indicator SOC_IS_TD2_TT2(unit)|| SOC_IS_HELIX4(unit) */ 
...
bcmi_xgs5_port_attach_stg(int unit, bcm_port_t port)
{
    /* STG */
    BCM_IF_ERROR_RETURN
        (bcm_esw_stg_stp_set(unit, BCM_STG_DEFAULT, port, BCM_STG_STP_FORWARD));
    return BCM_E_NONE;
}
...
bcmi_xgs5_port_attach_vlan(int unit, bcm_port_t port)
{
    ...
    BCM_IF_ERROR_RETURN
        (bcm_esw_vlan_port_add(unit, BCM_VLAN_DEFAULT, pbmp, pbmp));
...
bcmi_xgs5_port_detach_stg(int unit, bcm_port_t port)
{
    /* STG */
    BCM_IF_ERROR_RETURN
        (bcm_esw_stg_stp_set(unit, BCM_STG_DEFAULT, port, BCM_STG_STP_DISABLE));
    return BCM_E_NONE;
}
...
bcmi_xgs5_port_detach_vlan(int unit, bcm_port_t port)
{
    ...
    BCM_PBMP_PORT_ADD(pbmp, port);
    BCM_IF_ERROR_RETURN
        (bcm_esw_vlan_port_remove(unit, BCM_VLAN_DEFAULT, pbmp));
