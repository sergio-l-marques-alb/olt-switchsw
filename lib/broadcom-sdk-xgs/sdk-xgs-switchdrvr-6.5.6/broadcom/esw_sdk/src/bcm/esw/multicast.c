                                            ...
                                            bcm_gport_t subscriber_queue) 
{
...
#if defined(BCM_KATANA2_SUPPORT)
    if (SOC_IS_KATANA2(unit) && !SOC_IS_SABER2(unit)) {
        rv = _bcm_kt2_cosq_index_resolve(unit, subscriber_queue, 0,
    ...
    if (SOC_IS_KATANA(unit)) {
        rv = _bcm_kt_cosq_index_resolve(unit, subscriber_queue, 0,
    ...
    if (!local_count && (rv != BCM_E_NOT_FOUND)) {
...
{
    bcm_gport_t *cur_port_array;
    bcm_if_t *cur_encap_array;
    bcm_gport_t *cur_q_array;
    int if_max = BCM_MULTICAST_PORT_MAX;
    bcm_error_t rv = BCM_E_NONE;
    int if_index, if_cur_index, cur_port_count = 0;

    /*get the current port_array, encap array and q_array */
    ...
    for (if_index = 0; if_index < port_count; if_index++) {
        ...
        if (if_cur_index == cur_port_count) {
            ...
            rv = _bcm_esw_multicast_egress_subscriber_set(unit, group,
                     1, &port_array[if_index], &encap_id_array[if_index],
                     &subscriber_queue_array[if_index]);
            if (BCM_FAILURE(rv)) {
                sal_free (cur_port_array);
                sal_free (cur_encap_array);
