...
_bcm_th2_hgt_lag_dlb_quality_map_profile_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (port = 0;
            ...
            BCM_IF_ERROR_RETURN
                (WRITE_DLB_HGT_LAG_QUANTIZE_CONTROLm(unit, MEM_BLOCK_ALL,
                                                port,
                                                &quantize_control_entry));
        }
...
bcm_th2_hgt_lag_dlb_init(int unit)
{
    ...
    int port, pipe;
    dlb_hgt_lag_quantize_control_entry_t quantize_control_entry;
    dlb_hgt_lag_port_quality_update_measure_control_entry_t measure_entry;
    dlb_hgt_lag_eem_configuration_entry_t eem_entry;
    int sample_rate;
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        for (port = 0;
                port < soc_mem_index_count(unit,
                    DLB_HGT_LAG_PORT_QUALITY_UPDATE_MEASURE_CONTROLm);
                port++) {
            rv = READ_DLB_HGT_LAG_PORT_QUALITY_UPDATE_MEASURE_CONTROLm(unit, MEM_BLOCK_ANY,
                    port, &measure_entry);
            if (BCM_FAILURE(rv)) {
                bcm_th2_hgt_lag_dlb_deinit(unit);
                return rv;
            }
            soc_DLB_HGT_LAG_PORT_QUALITY_UPDATE_MEASURE_CONTROLm_field32_set(unit,
            ...
            rv = WRITE_DLB_HGT_LAG_PORT_QUALITY_UPDATE_MEASURE_CONTROLm(unit, MEM_BLOCK_ALL,
                    port, &measure_entry);
            if (BCM_FAILURE(rv)) {
                bcm_th2_hgt_lag_dlb_deinit(unit);
                return rv;
        ...
        for (port = 0;
            ...
            rv = READ_DLB_HGT_LAG_QUANTIZE_CONTROLm(unit, MEM_BLOCK_ANY,
                    port, &quantize_control_entry);
            if (BCM_FAILURE(rv)) {
                bcm_th2_hgt_lag_dlb_deinit(unit);
                return rv;
            ...
            soc_DLB_HGT_LAG_QUANTIZE_CONTROLm_field32_set(unit,
                    ...
                    XPE_PORT_QSIZE_THRESHOLD_SCALING_FACTORf,
                    2);
            rv = WRITE_DLB_HGT_LAG_QUANTIZE_CONTROLm(unit, MEM_BLOCK_ALL,
                    port, &quantize_control_entry);
            if (BCM_FAILURE(rv)) {
                bcm_th2_hgt_lag_dlb_deinit(unit);
                return rv;
