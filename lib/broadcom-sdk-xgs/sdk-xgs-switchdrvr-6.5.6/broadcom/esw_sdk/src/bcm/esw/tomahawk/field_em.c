...
/*
 * Function:
 *  _field_th2_em_validate_view
 * Purpose:
 *   Validate Exact Match View in TH2.
 * Parameters:
 *   unit     - (IN) BCM device number.
 *   action - (IN) field action.
 *   aset_size  - (IN/OUT) Action Per Set Size.
 *   aset_cnt  - (IN/OUT) Action Per Set Count.
 * Returns:
 *   BCM_E_XXX
 * Notes:
 */
...
int
_field_th2_em_validate_view(int unit, bcm_field_action_t action, uint8 *aset_size, uint8* aset_cnt)
{
    switch (action) {
        ...
        case bcmFieldActionEgressTimeStampInsertCancel:
            aset_size[_FieldEmActionTimeStampSet] +=
                              EM_ACTION_TIMESTAMP_SET_SIZE;
            aset_cnt[_FieldEmActionTimeStampSet]++;
            break;
        case bcmFieldActionDynamicTrunkAllEnable:
        case bcmFieldActionDynamicTrunkAllCancel:
            aset_size[_FieldEmActionDlbHgtLagSet] +=
                              EM_ACTION_DLB_HGT_LAG_SET_SIZE;
            aset_cnt[_FieldEmActionDlbHgtLagSet]++;
            break;
         case bcmFieldActionPacketTraceEnable:
            aset_size[_FieldEmActionInstrumentationSet] +=
                          EM_ACTION_INSTRUMENTATION_SET_SIZE;
            aset_cnt[_FieldEmActionInstrumentationSet]++;
            break;
#ifdef INCLUDE_L3
        case bcmFieldActionDynamicEcmpEnable:
        case bcmFieldActionDynamicEcmpCancel:
            aset_size[_FieldEmActionDlbEcmpSet] +=
                              EM_ACTION_DLB_ECMP_SET_SIZE;
            aset_cnt[_FieldEmActionDlbEcmpSet]++;
            break;
        case bcmFieldActionDgm:
        case bcmFieldActionDgmThreshold:
        case bcmFieldActionDgmBias:
        case bcmFieldActionDgmCost:
            aset_size[_FieldEmActionDlbAlternatePathControlSet] +=
                          EM_ACTION_DLB_ALTERNATE_PATH_CONTROL_SET_SIZE;
            aset_cnt[_FieldEmActionDlbAlternatePathControlSet]++;
            break;
         case bcmFieldActionRecoverableDropCancel:
            aset_size[_FieldEmActionProtectionSwitchingSet] +=
                          EM_ACTION_PROTECTION_SWITCHING_SET_SIZE;
            aset_cnt[_FieldEmActionProtectionSwitchingSet]++;
            break;
#endif /*INCLUDE_L3*/
        default:
            break;
...
_field_th_em_validate_view(int unit, uint16 key_size, _field_group_t *fg)
{
    ...
    if (SOC_IS_TOMAHAWK2(unit)) {
        counter_size = EM_ACTION_COUNTER_SET_SIZE_TH2;
    } else
#endif /* BCM_TOMAHAWK2_SUPPORT */
    {
        counter_size = EM_ACTION_COUNTER_SET_SIZE;
    }
        ...
        if (BCM_FIELD_ASET_TEST(aset, a_idx)) {
            switch (a_idx) {
                ...
                case bcmFieldActionStatGroup:
                    aset_size[_FieldEmActionCounterSet] += counter_size;
                ...
                if (SOC_IS_TOMAHAWK2(unit)) {
                    BCM_IF_ERROR_RETURN
                        (_field_th2_em_validate_view(unit, a_idx, aset_size, aset_cnt));
                    break;
                } else
#endif /*BCM_TOMAHAWK2_SUPPORT*/
                {
                                   ...
                                   uint32 *ap_idx)
{
    ...
    for (fa = f_ent->actions; fa != NULL; fa = fa->next) {
        if (_FP_ACTION_VALID & fa->flags) {
            switch (fa->action) {
                ...
                case bcmFieldActionDgmBias:
                case bcmFieldActionDgmThreshold:
                    acts = &acts_buf[_FieldEmActionDlbAlternatePathControlSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        DLB_ALTERNATE_PATH_CONTROL_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
                case bcmFieldActionDynamicEcmpEnable:
                case bcmFieldActionDynamicEcmpCancel:
                    acts = &acts_buf[_FieldEmActionDlbEcmpSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        DLB_ECMP_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
#endif /*INCLUDE_L3*/
                case bcmFieldActionDynamicTrunkAllEnable:
                case bcmFieldActionDynamicTrunkAllCancel:
                    acts = &acts_buf[_FieldEmActionDlbHgtLagSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        DLB_HGT_LAG_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
                case bcmFieldActionIngressTimeStampInsert:
                ...
                case bcmFieldActionEgressTimeStampInsert:
                case bcmFieldActionEgressTimeStampInsertCancel:
                    acts = &acts_buf[_FieldEmActionTimeStampSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        TIMESTAMP_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
                case bcmFieldActionPacketTraceEnable:
                    acts = &acts_buf[_FieldEmActionInstrumentationSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        INSTRUMENTATION_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
#ifdef INCLUDE_L3
                case bcmFieldActionRecoverableDropCancel:
                    acts = &acts_buf[_FieldEmActionProtectionSwitchingSet];
                    rv = _bcm_field_th_misc_action_set(unit, f_ent,
                                                       fa, &acts->data);
                    BCM_IF_ERROR_RETURN(rv);
                    soc_mem_field32_set(unit, mem, &act_prof_entry,
                                        PROTECTION_SWITCHING_SET_ENABLEf, 1);
                    acts->valid = 1;
                    fa->flags &= ~_FP_ACTION_DIRTY;
                    break;
#endif /*INCLUDE_L3*/
    ...
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                   act_offset, counter_size);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += counter_size;
    }
#ifdef BCM_TOMAHAWK2_SUPPORT
#ifdef INCLUDE_L3
    idx = _FieldEmActionDlbAlternatePathControlSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_DLB_ALTERNATE_PATH_CONTROL_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_DLB_ALTERNATE_PATH_CONTROL_SET_SIZE;
    }
    idx = _FieldEmActionDlbEcmpSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_DLB_ECMP_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_DLB_ECMP_SET_SIZE;
    }
#endif /*INCLUDE_L3*/
    idx = _FieldEmActionDlbHgtLagSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_DLB_HGT_LAG_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_DLB_HGT_LAG_SET_SIZE;
    }
    idx = _FieldEmActionTimeStampSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_TIMESTAMP_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_TIMESTAMP_SET_SIZE;
    }
    idx = _FieldEmActionInstrumentationSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_INSTRUMENTATION_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_INSTRUMENTATION_SET_SIZE;
    }
#ifdef INCLUDE_L3
    idx = _FieldEmActionProtectionSwitchingSet;
    if (acts_buf[idx].valid) {
        rv = _bcm_field_th_val_set(abuf, &acts_buf[idx].data,
                                                  act_offset,
                                  EM_ACTION_PROTECTION_SWITCHING_SET_SIZE);
        BCM_IF_ERROR_RETURN(rv);
        act_offset += EM_ACTION_PROTECTION_SWITCHING_SET_SIZE;
    }
#endif /*INCLUDE_L3*/
#endif /*BCM_TOMAHAWK2_SUPPORT*/
    /* Set Profile Entry. */
