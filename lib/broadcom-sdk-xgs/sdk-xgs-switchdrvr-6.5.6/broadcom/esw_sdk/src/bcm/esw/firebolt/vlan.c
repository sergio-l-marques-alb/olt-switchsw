...
_bcm_fb2_outer_tpid_init(int unit) {
    ...
    if (soc_mem_is_valid(unit, SYSTEM_CONFIG_TABLEm)) {
        ...
        index_min = soc_mem_index_min(unit, SYSTEM_CONFIG_TABLEm);
        index_max = soc_mem_index_max(unit, SYSTEM_CONFIG_TABLEm);
        for (i = index_min; i <= index_max; i++) {
            BCM_IF_ERROR_RETURN(READ_SYSTEM_CONFIG_TABLEm(
                unit, MEM_BLOCK_ANY, i, &entry));
            enabled = soc_SYSTEM_CONFIG_TABLEm_field32_get(
                unit, &entry, OUTER_TPID_ENABLEf);
...
_bcm_xgs3_vlan_table_init(int unit, bcm_vlan_data_t *vd, int table)
{
    ...
    /*This function will be called with VLAN_TABm, EGR_VLANm, and VLAN_2_TABm in TH2.
     *In TH2, VLAN_2_TABm has STGf while VLAN_TABm does not.
     *The check is to prevent STGf being set wrongly.
     */
    ...
    if (SOC_MEM_IS_VALID(unit, VLAN_MPLSm)) {
        ...
        if (l3IntfMapMode) {
           ...
           if (!SHR_BITGET(vlan_info[unit].pre_cfg_bmp.w,vid)) {
            ...
            if ((soc_feature(unit, soc_feature_l3_iif_under_4k)) &&
                    (soc_mem_field32_fit(unit, VLAN_MPLSm, L3_IIFf, vid) != SOC_E_NONE)) {
                       ...
                       pbmp_t ing_pbmp)
{
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_xgs3_vlan_table_port_add(unit, vid, pbmp, ubmp, ing_pbmp, VLAN_TABLE(unit)));
