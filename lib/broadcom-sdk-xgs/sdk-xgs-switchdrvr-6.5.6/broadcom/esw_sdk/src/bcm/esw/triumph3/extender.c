/*
 * $Id: extender.c,v 1.11 Broadcom SDK $
...
_bcm_tr3_extender_reinit(int unit)
{
    ...
    uint32 entry_type = 0, vp = 0, trunk_bit, vntag_action = 0;
        ...
        if (!(EXTENDER_PORT_INFO(unit, vp)->flags & BCM_EXTENDER_PORT_MATCH_NONE)) {
            if (entry_type == _BCM_EXTENDER_EGR_NEXT_HOP_SDTAG_VIEW) {
                ...
                if (BCM_EXTENDER_PCP_DE_SELECT_DEFAULT == pcp_de_select_mode) {
                    ...
                    rv = bcm_td2_qos_egr_etag_profile2id(unit, etag_dot1p_mapping_ptr, &map_id);
                    if (rv == BCM_E_NONE) {
                ...
                if (trunk_bit) {
                    tgid = soc_ING_L3_NEXT_HOPm_field32_get(unit, ing_nh_entry, TGIDf);
                    BCM_GPORT_TRUNK_SET(EXTENDER_PORT_INFO(unit, vp)->port, tgid);
                } else {
                    modid = soc_ING_L3_NEXT_HOPm_field32_get(unit, ing_nh_entry, MODULE_IDf);
                    port_num = soc_ING_L3_NEXT_HOPm_field32_get(unit, ing_nh_entry, PORT_NUMf);
...
bcm_tr3_extender_init(int unit)
{
     ...
     */
    EXTENDER_INFO(unit)->invalid_next_hop_index = BCM_XGS3_L3_BLACK_HOLE_NH_IDX;
        ...
        int vp)
{
    ...
    /* SVP */
    soc_VLAN_XLATEm_field32_set(unit, &vent, VIF__MPLS_ACTIONf, 
                                    ...
                                    bcm_vlan_t vlan, int vp)
{
    ...
    if (BCM_VLAN_VALID(vlan)) {
        BCM_IF_ERROR_RETURN(
            _bcm_esw_vlan_xlate_key_type_value_get(unit,
                                                   ...
                                                   &key_type));
        soc_VLAN_XLATEm_field32_set(unit, &vent, VIF__VLANf, vlan);
    } else {
        BCM_IF_ERROR_RETURN(
            _bcm_esw_vlan_xlate_key_type_value_get(unit,
    ...
    soc_VLAN_XLATEm_field32_set(unit, &vent, VIF__SRC_VIFf, extended_port_vid);
    ...
    rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ALL, &entry_index,
                        &vent, &old_vent, 0);
    if (SOC_FAILURE(rv)) {
    ...
    old_vp = soc_VLAN_XLATEm_field32_get(unit, &old_vent, VIF__SOURCE_VPf);
    if (vp != old_vp) {
        soc_mem_unlock(unit, VLAN_XLATEm);
        return BCM_E_NOT_FOUND;
    }
    
    rv = soc_mem_delete_return_old(unit, VLAN_XLATEm,
                         ...
                         bcm_extender_port_t *extender_port)
{
    ...
    if (!(extender_port->flags & BCM_EXTENDER_PORT_REPLACE)) {
        ...
        /* Configure DVP table */
        intf = nh_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
    ...
    bcm_extender_egress_t *extender_egress)
{
    ...
    intf = BCM_XGS3_DVP_EGRESS_IDX_MIN + nh_index;
    BCM_IF_ERROR_RETURN(
    ...
    if (extender_egress->flags & BCM_EXTENDER_EGRESS_L3) {
        extender_egress->egress_if = nh_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
    }
    else {
        extender_egress->egress_if = nh_index + BCM_XGS3_EGRESS_IDX_MIN;
    }
        ...
        int array_size, bcm_extender_egress_t *extender_egress_array)
{
    ...
    intf = BCM_XGS3_DVP_EGRESS_IDX_MIN + EXTENDER_INFO(unit)->invalid_next_hop_index;
            ...
            if (array_size > 0) {
                ...
                if (curr_ptr->flags & BCM_EXTENDER_EGRESS_L3) {
                    extender_egress_array[*count].egress_if =
                        curr_ptr->next_hop_index + BCM_XGS3_DVP_EGRESS_IDX_MIN;
                } else {
                    extender_egress_array[*count].egress_if =
                        curr_ptr->next_hop_index + BCM_XGS3_EGRESS_IDX_MIN;
                }
            }
            (*count)++;
            if (*count == array_size) {
