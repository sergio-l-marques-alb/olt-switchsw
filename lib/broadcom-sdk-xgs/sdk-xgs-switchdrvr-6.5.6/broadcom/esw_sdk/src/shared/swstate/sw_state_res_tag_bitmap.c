/*
 * $Id: sw_state_res_tag_bitmap.c,v 1.6 Broadcom SDK $
...
#define SW_STATE_RES_TAG_BITMAP_FINAL_SEARCH_LIMIT (bmp_data.nextAlloc)
#else /* SW_STATE_RES_TAG_BITMAP_SEARCH_RESUME */
#define SW_STATE_RES_TAG_BITMAP_FINAL_SEARCH_LIMIT (bmp_data.count - count)
...
#define RES_BMP_ACCESS sw_state_access[unit].dnx.shareddnx.res_tag_bmp_info
#else 
    ...
    if (flags & SW_STATE_RES_TAG_BITMAP_ALLOC_WITH_ID) {
        ...
        if (_SHR_E_NONE == result) {
            if (flags & SW_STATE_RES_TAG_BITMAP_ALLOC_REPLACE) {
                ...
                if (_SHR_E_NONE == result) {
                    result = _sw_state_res_tag_bitmap_tag_check(unit, flags, handle, index, count, tag, &offset);
                    SW_STATE_ACCESS_ERROR_CHECK(result);
                    if (offset) {
        ...
        if (_SHR_E_NONE == result) {
            ...
            index = bmp_data.nextAlloc;
            while (index <= bmp_data.count - count) {
                result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index, &temp);
                SW_STATE_ACCESS_ERROR_CHECK(result);

                while (temp &&
                       (index > bmp_data.count - count)) {
                    index++;
                    result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index, &temp);
                    SW_STATE_ACCESS_ERROR_CHECK(result);
                }
                if (index <= bmp_data.count - count) {
            ...
            if ((_SHR_E_NONE != result) && (0 < bmp_data.nextAlloc)) {
                /* no space, so try space before last successful alloc */
#else /* SW_STATE_RES_TAG_BITMAP_SEARCH_RESUME */
                result = _SHR_E_RESOURCE;
#endif /* SW_STATE_RES_TAG_BITMAP_SEARCH_RESUME */
                index = 0;
                while (index < SW_STATE_RES_TAG_BITMAP_FINAL_SEARCH_LIMIT) {
                    result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index, &temp);
                    SW_STATE_ACCESS_ERROR_CHECK(result);

                    while (temp &&
                           (index < SW_STATE_RES_TAG_BITMAP_FINAL_SEARCH_LIMIT)) {
                        index++;
                        result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index, &temp);
                        SW_STATE_ACCESS_ERROR_CHECK(result);
                    }
                    if (index < SW_STATE_RES_TAG_BITMAP_FINAL_SEARCH_LIMIT) {
                        /* have a candidate; see if block is big enough */
                        result = _SHR_E_NONE;
                        for (offset = 0; offset < count; offset++) {
                            result = RES_BMP_ACCESS.bitmap_lists.data.bit_get(unit, bmp_index, index + offset, &temp);
