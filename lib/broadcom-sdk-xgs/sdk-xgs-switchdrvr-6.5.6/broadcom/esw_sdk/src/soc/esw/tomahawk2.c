    ...
    for (xpe = 0; xpe < _TH2_XPES_PER_DEV; xpe ++) {
        ...
        for (pipe = 0; pipe < _TH2_PIPES_PER_DEV; pipe ++) {
            if (pipe_map & (1 << pipe)) {
                ...
                port_num = port_num + mgmt[pipe] - cpu[pipe] - lb[pipe];
...
#define _SOC_TH2_DEF_TEMP_MAX   110
     ...
     * Default on: pvtmon[0-8] (enable all pvtmons). */
    temp_mask = soc_property_get(unit, "temp_monitor_select", ((1 << 9) - 1));
