...
STATIC soc_error_t _soc_th2_ser_tcam_control_reg_get(soc_mem_t mem,
...
soc_tomahawk_ser_enable_all(int unit, int enable)
{
    ...
    if (enable) {
        ...
        if (SOC_IS_TOMAHAWK2(unit)) {
            ...
            (void)soc_th2_tcam_engine_enable(unit, FALSE);
            (void) _soc_th2_tcam_wrapper_enable(unit, FALSE);
        }
#endif
    }
    return SOC_E_NONE;
...
#ifdef BCM_TOMAHAWK2_SUPPORT
                        if ((SOC_MEM_INFO(unit, mem).flags & SOC_MEM_FLAG_CAM) && SOC_IS_TOMAHAWK2(unit)) {
                            if ( SOC_E_NONE ==
                                _soc_th2_ser_tcam_control_reg_get(spci.mem,
                                &parity_enable_reg, &parity_enable_field)) {
                                LOG_VERBOSE(BSL_LS_SOC_SER,
                                          (BSL_META_U(unit,
...
static _soc_th2_tcam_wrapper_info_t _soc_th2_tcam_wrapper_table[] = {
    ...
    if (SOC_IS_TOMAHAWK2(unit)) {
        for (i = 0; _soc_th2_tcam_wrapper_table[i].mem != INVALIDm; i++) {
            ...
            soc_ser_create_test_data(unit, tmp_entry, field_data,
                                     ...
                                     SOC_MEM_ACC_TYPE(unit, _soc_th2_tcam_wrapper_table[i].mem),
                                     mem_index, &test_data);
...
STATIC soc_error_t
_soc_th2_ser_tcam_control_reg_get(soc_mem_t mem,
                                           soc_reg_t   *ser_control_reg,
                                           soc_field_t *ser_enable_field)
{
    int i;
