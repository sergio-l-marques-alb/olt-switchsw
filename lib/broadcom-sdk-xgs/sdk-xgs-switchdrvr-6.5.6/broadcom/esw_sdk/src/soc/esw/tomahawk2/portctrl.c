...
soc_th2_portctrl_pm_port_config_get(int unit, soc_port_t port, void *port_config)
{
    ...
    for (core_index = 0; core_index < core_num; core_index++) {
        ...
        for (lane = 0; lane < SOC_PM4X10_NUM_LANES; lane++) {
            ...
            if (core_index && (logical_port == -1)) {
                if (IS_CXX_PORT(unit, f_logical_port) && IS_CL_PORT(unit, f_logical_port)) {
                    ...
                    continue; /*logical port is -1*/
                }
            }

            _soc_th2_portctrl_pm_port_polarity_get(unit, logical_port, phy_port, lane, &tx_polarity, &rx_polarity);

            polarity.rx_polarity |= (IS_CXX_PORT(unit, f_logical_port) && IS_CL_PORT(unit, f_logical_port)) ?
                          ((rx_polarity >> (core_index * SOC_PM4X10_NUM_LANES)) & 0xf) : rx_polarity;
            polarity.tx_polarity |= (IS_CXX_PORT(unit, f_logical_port) && IS_CL_PORT(unit, f_logical_port)) ?
                        ((tx_polarity >> (core_index * SOC_PM4X10_NUM_LANES)) &0xf) : tx_polarity;
        }
        ...
        _soc_th2_portctrl_pm_port_lanemap_get(unit, logical_port, phy_port, core_index, &txlane_map, &rxlane_map);
