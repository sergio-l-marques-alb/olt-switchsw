                               ...
                               soc_port_t input_port)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            if (IS_AP_HSP_PORT(unit,port) && (soc_mem_field_length(unit, mem2,
                Q_E2ECC_DS_CONFIGf) > 2)) 
            {   
                soc_mem_field32_set(unit, mem2, entry0, Q_E2ECC_DS_CONFIGf, 4);
            } else { 
                soc_mem_field32_set(unit, mem2, entry0, Q_E2ECC_DS_CONFIGf, 1);
            }
