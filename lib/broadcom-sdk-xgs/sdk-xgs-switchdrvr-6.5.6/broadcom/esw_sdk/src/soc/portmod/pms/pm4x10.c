        ...
        if (params->lane != -1){ /* specific lane */
            ...
            if( serdes_lane != -1){    
                ...
                if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                    _SOC_IF_ERR_EXIT ( portmod_xphy_core_access_get(unit, xphy_id, &core_access));
                               ...
                               soc_port_if_t interface, int *is_valid)
{
    ...
    is_legacy_phy = PORTMOD_IS_LEGACY_PHY_GET(&phy_acc->access);
    ...
    rv = portmod_port_line_interface_is_valid(phy_acc, port, data_rate, phymod_intf,
                        ...
                        const portmod_port_init_config_t* init_config)
{
    ...
    if (nof_phys > 1){
        ...
        SOC_IF_ERROR_RETURN(
                portmod_port_phychain_tx_set(phy_access, nof_phys,
                                             ext_phy_tx_params));
    }
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                        ...
                        if( !core_probed ){
                            ...
                            _SOC_IF_ERR_EXIT(portmod_xphy_core_access_set(unit, xphy_id, &core_access));
                        }
                    }
                }
            }
            ...
            if( PM_4x10_INFO(pm_info)->nof_phys[temp] >= 2 ){
                ...
                for(i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if(xphy_id != PORTMOD_XPHY_ID_INVALID){
                        ...
                        if (PORTMOD_IS_LEGACY_PHY_GET(&core_access.access)) {
                            ...
                            int* enable)
{
    ...
    if (PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)) {
        ...
        _SOC_IF_ERR_EXIT
            (portmod_port_phychain_tx_lane_control_get(phy_access, nof_phys, &tx_control));
        _SOC_IF_ERR_EXIT
            (portmod_port_phychain_rx_lane_control_get(phy_access, nof_phys, &rx_control));
                        ...
                        int phy_init_flags)
{
    ...
    if(config->flags & PHYMOD_INTF_F_INTF_PARAM_SET_ONLY) {
        ...
        /* Check if the interface type being written into the software state is
         * valid or not. If not, the operation should be aborted.
         */
        if (nof_phys==1) {
            _SOC_IF_ERR_EXIT(
                _pm4x10_interface_check(
                    unit, port, pm_info, config, config->interface, &is_valid));
        } else {
            _SOC_IF_ERR_EXIT(
                _pm4x10_ext_phy_interface_check(
                    unit, port, &phy_access_arr[nof_phys-1], config, config->interface, &is_valid));
        }
    ...
    if( nof_phys > 1 ){
        ...
        if (!(config->flags & (PHYMOD_INTF_F_SET_SPD_TRIGGER | PHYMOD_INTF_F_UPDATE_SPEED_LINKUP))) {
            ...
            if (!is_legacy_phy) {
                ...
                _SOC_IF_ERR_EXIT(
                    portmod_port_phychain_tx_set(&phy_access_arr[nof_phys-1],
                                                 1, ext_phy_tx_params));
            }
        }
    }
    ...
    if (!(config->flags & (PHYMOD_INTF_F_SET_SPD_TRIGGER | PHYMOD_INTF_F_UPDATE_SPEED_LINKUP))) {
        ...
        if (port_dynamic_state) {
            ...
            _SOC_IF_ERR_EXIT(
                portmod_port_phychain_tx_set(phy_access_arr, 1, tx_params));
        }
    }
     ...
     */
    if (!(nof_phys>1 && PORTMOD_PHY_ACC_CMD_FOR_PHY_GET(phy_access_arr, 0))) {
        _SOC_IF_ERR_EXIT
                                      ...
                                      soc_port_if_t* interface)
{
    ...
    if (nof_phys==1) {
        ...
        is_legacy_present = ((portmod_default_user_access_t *)(phy_access[nof_phys-1].access.user_acc))->is_legacy_phy_present;

        if (!is_legacy_present) {
            _SOC_IF_ERR_EXIT
                (portmod_port_phymod_xphy_default_interface_get(&(phy_access[nof_phys-1]),
                                                                config, interface));
        } else {
                                   ...
                                   portmod_port_ability_t* ability)
{
     ...
     */
    if (nof_phys > 1 && !PORTMOD_USER_ACC_CMD_FOR_PHY_GET(phy_access, 0)) {
...
{   
    ...
    if(PM_4x10_INFO(pm_info)->nof_phys[port_index] == 0) {
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL,
                          (_SOC_MSG("Autoneg isn't supported")));
    } else {
        _SOC_IF_ERR_EXIT
            (portmod_port_chain_phy_access_get(unit, port, pm_info, 
                                               phy_access ,(1+MAX_PHYN),
...
int pm4x10_port_link_get(int unit, int port, pm_info_t pm_info, int* link)
{
    ...
    _SOC_IF_ERR_EXIT(PM4x10_AN_CL37_GET(unit, pm_info, &an_cl37, port_index));
     if(an_cl37 == PORTMOD_CL37_SGMII_COMBO) {
        _SOC_IF_ERR_EXIT(portmod_port_phychain_autoneg_get(phy_access,
                                                           nof_phys,
                                                           &an, &an_done));
        if (an.enable) {
            cl37_sgmii_war = 1;
                                 ...
                                 portmod_port_ability_t* ability)
{
    ...
    _SOC_IF_ERR_EXIT
        (portmod_port_chain_phy_access_get(unit, port, pm_info, 
                                           ...
                                           &nof_phys));
    _SOC_IF_ERR_EXIT
        (portmod_port_phychain_autoneg_ability_get(phy_access, nof_phys,
                                 ...
                                 portmod_port_ability_t* ability)
{
     ...
     */ 
    if (nof_phys > 1) {
      portmod_port_line_interface_map_for_xphy(&(phy_access[nof_phys-1]), port, line_interface, &temp_line_intf);
    ...
    _SOC_IF_ERR_EXIT
        (portmod_port_phychain_autoneg_ability_set(phy_access, nof_phys, &an_ability, &port_ability));
                        ...
                        const portmod_port_update_control_t* update_control)
{
    ...
    portmod_port_interface_config_t interface_config;
    int port_index, rv = 0, ref_clock, max_speed;
    uint32_t                    bitmap;
    phymod_phy_inf_config_t     phy_interface_config;
    soc_port_if_t   interface = SOC_PORT_IF_NULL;
    phymod_phy_access_t phy_access[1+MAX_PHYN];
...
int pm4x10_port_notify(int unit, int port, pm_info_t pm_info, int link) {
    ...
    if (nof_phys > 1) {
        if (link) {
            PORTMOD_PHY_ACC_CMD_FOR_PHY_SET(phy_access, 0);
            ...
            PORTMOD_PHY_ACC_CMD_FOR_PHY_CLR(phy_access, 0);
        } else {
            /*TBD*/
        }
    } 
