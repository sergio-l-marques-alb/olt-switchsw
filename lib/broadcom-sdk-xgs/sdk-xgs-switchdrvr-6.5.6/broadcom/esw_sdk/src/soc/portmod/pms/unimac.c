...
#if defined(PORTMOD_PM4x10Q_SUPPORT) || defined(PORTMOD_PM_QTC_SUPPORT)
...
int unimac_tx_average_ipg_set(int unit, soc_port_t port, int ipg_val)
{
    ...
    SOC_IF_ERROR_RETURN(WRITE_TX_IPG_LENGTHr(unit, port, ipg_val));
...
int unimac_tx_average_ipg_get(int unit, soc_port_t port, int *ipg_val)
{
    ...
    *ipg_val = soc_reg_field_get(unit, TX_IPG_LENGTHr, reg_val, TX_IPG_LENGTHf);
...
int unimac_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    /*SOC_IF_ERROR_RETURN(unimac_tx_average_ipg_set(unit, port, ));*/
...
int unimac_eee_set(int unit, int port, const portmod_eee_t* eee)
{
    ...
    if (speed > SOC_UNIMAC_MAX_EEE_SPEED) {
        LOG_VERBOSE(BSL_LS_SOC_COMMON,(BSL_META_U(unit, "max speed for EEE is 1000Mbps")));
        return (SOC_E_PARAM);
    }
                          ...
                          const portmod_pfc_config_t* pfc_cfg)
{
    ...
    return (WRITE_MAC_PFC_DA_0r(unit, port, reg_val));
}
...
int unimac_pass_control_frame_set(int unit, int port, int value)
{
    ...
    soc_reg_field_set(unit, MAC_PFC_CTRLr, &reg_val, RX_PASS_PFC_FRMf, value ? 1 : 0);
...
int unimac_rx_vlan_tag_get(int unit, soc_port_t port, int *outer_vlan_tag, int *inner_vlan_tag)
{
    ...
    *outer_vlan_tag = enable ? soc_reg_field_get(unit, TAG_0r, reg_val, FRM_TAG_1f) : -1;
...
int unimac_pause_control_set(int unit, soc_port_t port, const portmod_pause_control_t *control)
{
    ...
    soc_reg_field_set(unit, COMMAND_CONFIGr, &command_config, PAUSE_IGNOREf, control->tx_enable ? 0 : 1);
...
int unimac_pause_control_get(int unit, soc_port_t port, portmod_pause_control_t *control)
{
    ...
    control->rx_enable = soc_reg_field_get(unit, COMMAND_CONFIGr, command_config, IGNORE_TX_PAUSEf) ?  FALSE : TRUE;
