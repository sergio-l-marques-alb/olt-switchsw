                                   ...
                                   int copyno, int index_min, int index_max, void *buffer, int vchan)
{
    ...
    if (SOC_IS_DPP(unit) && (entry_dw >= CMIC_SCHAN_WORDS(unit))) {
        ...
        for (index = index_min; index <= index_max; index++, count++) {
            buf = soc_mem_table_idx_to_pointer(unit, mem, void *,
                                               buffer, count);
            rv = dcmn_mem_array_wide_access(unit, SOC_MEM_NO_FLAGS, mem, array_index, MEM_BLOCK_ALL, index, buf, 1);
            if (SOC_FAILURE(rv)) {
                                      ...
                                      int array_index, void *entry_data)
{
     ...
     */
    if (SOC_IS_TRIDENT(unit) && mem == MMU_THDO_OFFSET_0m) {
        if (soc_mem_is_valid(unit, THDO_OFFSET_0Am)) {
            _soc_mem_write_range_cache_update(unit, THDO_OFFSET_0Am, 
                                              array_index, blk, no_cache,
                                              index_min, index_max, entry_data);
        }
        if (soc_mem_is_valid(unit, THDO_OFFSET_0Bm)) {
            _soc_mem_write_range_cache_update(unit, THDO_OFFSET_0Bm, 
                                              array_index, blk, no_cache,
                                              index_min, index_max, entry_data);
        }
    }
