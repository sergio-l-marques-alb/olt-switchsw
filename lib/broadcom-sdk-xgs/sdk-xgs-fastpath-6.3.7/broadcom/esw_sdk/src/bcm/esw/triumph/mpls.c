/*
 * $Id: mpls.c 1.551.2.4 Broadcom SDK $
    ...
    /* Warm Boot */
        ...
        if (stable_size > alloc_sz) {
            ...
            if (recovered_ver >= BCM_WB_VERSION_1_2) {
                 SHR_BITCOPY_RANGE(mpls_info->egr_mpls_hw_idx, 0,
                                (SHR_BITDCL *)mpls_state, 0, num_mpls_map);
                mpls_state += SHR_BITALLOCSIZE(num_mpls_map);
                ...
                additional_scache_sz = SHR_BITALLOCSIZE(num_mpls_map);
            }
...
bcm_tr_mpls_sync(int unit)
{
    ...
    if (BCM_WB_DEFAULT_VERSION >= BCM_WB_VERSION_1_2) {
        SHR_BITCOPY_RANGE((SHR_BITDCL *)mpls_state, 0,
                        mpls_info->egr_mpls_hw_idx, 0, num_mpls_map);
        mpls_state += SHR_BITALLOCSIZE(num_mpls_map);
    }
                                   ...
                                   bcm_if_t old_tunnel_if)
{
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_ENCAP_WITH_ID) {
        ...
        if (old_tunnel_if == mpls_port->egress_tunnel_if) {
    ...
    if (old_vp_nh_index != -1) {
        ...
        if (old_macda_idx != -1) {
            (void)_bcm_mac_da_profile_entry_delete(unit, old_macda_idx);
        }
                            ...
                            bcm_if_t *old_tunnel_if)
{
    ...
    *old_tunnel_if = 0;
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_ENCAP_WITH_ID) {
        rv = bcm_tr_mpls_get_vp_nh (unit, *vp_nh_index, old_tunnel_if);
        if (rv == BCM_E_NOT_FOUND) { 
            *old_tunnel_if = 0;
    ...
    if (mpls_port->flags & BCM_MPLS_PORT_REPLACE) {
       ...
       /* Allocate MPLS VP Next Hop */
        ...
        if ((mpls_port->flags & BCM_MPLS_PORT_ENCAP_WITH_ID) && (!*old_tunnel_if)) {
        ...
        if ((mpls_port->flags & BCM_MPLS_PORT_REPLACE) &&
            ...
            if ((ing_nh_info.qtag > 0) &&
               ...
               /*  ING_QUEUE_MAP needs to be reset */
                ...
                _BCM_MPLS_CLEANUP(
                     BCM_XGS3_MEM_WRITE(unit, ING_QUEUE_MAPm,
                                        ing_nh_info.qtag, &ing_queue_entry));
                ing_nh_info.qtag = 0;
            ...
            if ((mpls_port->flags & BCM_MPLS_PORT_REPLACE) &&
                ...
                if (old_match_label != mpls_port->match_label) {
                    ...
                    * it must be released first using stat_detach*/
                    soc_cm_debug(DK_ERR, "The flex stat counter %d for the "
...
_bcm_tr_mpls_vpws_port_add(int unit, bcm_vpn_t vpn, bcm_mpls_port_t *mpls_port)
{
    ...
    int customer_drop=0, is_local = 0, rv = BCM_E_PARAM, nh_index = 0, old_nh_index = -1; 
    ...
    bcm_if_t old_tunnel_if = 0;
        ...
        /* Remember old nh_index */
        old_nh_index = nh_index;
        ...
        switch (vp_valid_flag) {
              ...
              default:
                            ...
                            rv = _bcm_tr_mpls_l2_nh_info_add(unit, mpls_port, active_vp, customer_drop, 1,
                                         &nh_index, &local_port, &is_local, &old_tunnel_if);
                            if (rv < 0) {
                                return rv;
                            }
...
_bcm_tr_mpls_vpls_port_add(int unit, bcm_vpn_t vpn, bcm_mpls_port_t *mpls_port)
{
    ...
    int drop, i, is_local = 0, rv = BCM_E_PARAM, nh_index = 0, old_nh_index = 0;
                           ...
                           bcm_mpls_stat_t stat, uint64 *val)
{
    ...
    if (soc_feature(unit, soc_feature_gport_service_counters)) {
        ...
        if (BCM_GPORT_IS_MPLS_PORT(port)) {
            ...
            rv = _bcm_esw_flex_stat_ext_get(unit, 0, _bcmFlexStatTypeMplsLabel,
                                            mpls_fsh,
