...
_bcm_hr2_dual_port_mode_pilot_tx(int unit, int port, bcm_pkt_t *pkt) {
    ...
    soc_cm_debug(DK_PORT | DK_VERBOSE, "old TPOK(port=%d)=0x%x, 0x%x\n", \
                 port, COMPILER_64_HI(old_tpok), COMPILER_64_LO(old_tpok));
    soc_cm_debug(DK_PORT | DK_VERBOSE, "new TPOK(port=%d)=0x%x, 0x%x\n", \
...
bcm_hr2_dual_port_mode_reinit(int unit) {
    ...
    if (check_required) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (BCM_PBMP_MEMBER(PBMP_PORT_ALL(unit), test_ports[i])) {
                ...
                if (rv < 0) {
                    soc_cm_debug(DK_ERR, "Dual port mode WAR Failed on port %d\n", test_ports[i]);
                }
    ...
    soc_cm_debug(DK_VERBOSE, "Dual port mode WAR: took %d usec\n",
