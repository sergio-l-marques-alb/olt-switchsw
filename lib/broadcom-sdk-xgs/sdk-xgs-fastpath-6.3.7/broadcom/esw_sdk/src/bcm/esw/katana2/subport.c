/*
 * $Id: subport.c 1.52 Broadcom SDK $
...
_bcm_linkphy_subport_stat_callback(int unit)
{
    ...
    if (linkphy_temp_ctr[unit] == NULL){
        SUBPORT_WARN(("Unit %d Not Initilized or attached\n", unit));
    }
                ...
                for (index = 0; index <= (_BCM_KT2_LINKPHY_COUNTER_MAX - 1);
                    ...
                    SUBPORT_VERB(
                        ("%s_DEBUG_COUNTER%d.lp%d index:%d "
                         ...
                         direction ? "TXLP" : "RXLP", 
                         ((pool_id == 3) ? 11 : ((pool_id == 2) ? 10 :
                             ((pool_id == 1) ? 9 : 8))),
                         block_id, index,
                         ((pool_id == 3) ? "Frame    " : ((pool_id == 2) ? "FrameByte" :
...
_bcm_linkphy_subport_counter_init(int unit)
{
    ...
    if (linkphy_temp_ctr[unit] != NULL){
        SUBPORT_WARN(("WARN:Freeing linkphy_temp_ctr\n"));
        soc_cm_sfree(unit,linkphy_temp_ctr[unit]);
        linkphy_temp_ctr[unit] = NULL;
    }
    ...
    if (linkphy_temp_ctr[unit] == NULL) {
        SUBPORT_WARN(("linkphy temp allocation for unit:%d failed\n", unit));
        _bcm_linkphy_subport_counter_cleanup(unit);
        return BCM_E_MEMORY;
    }
    sal_memset(linkphy_temp_ctr[unit], 0, temp_alloc_size);
    SUBPORT_VVERB(("linkphy temp counter size:%d \n",temp_alloc_size));
        ...
        for (block_id = 0; block_id < _BCM_KT2_LINKPHY_BLOCK_MAX; block_id++) {
            for (pool_id = 0; pool_id < _BCM_KT2_LINKPHY_POOL_MAX; pool_id++) {
                ...
                if (soc_mem_write_range(unit, mem, copyno,
                    ...
                    SUBPORT_ERR(("ERROR: zeroing LinkPHY counter failed\n"));
                    soc_cm_sfree(unit,linkphy_temp_ctr[unit]);
                    linkphy_temp_ctr[unit] = NULL;
                }
            }
    ...
    for (direction=0; direction < _BCM_KT2_LINKPHY_DIRECTION_MAX; direction++) {
        for (block_id = 0; block_id < _BCM_KT2_LINKPHY_BLOCK_MAX; block_id++) {
            /* fragment byte */
            if (linkphy_frag_byte_ctr[unit][direction][block_id] != NULL) {
                SUBPORT_WARN(("WARN:Freeing linkphy_frag_byte_ctr\n"));
                soc_cm_sfree(unit,
                    linkphy_frag_byte_ctr[unit][direction][block_id]);
                linkphy_frag_byte_ctr[unit][direction][block_id] = NULL;
            }
            ...
            if (linkphy_frag_byte_ctr[unit][direction][block_id] == NULL) {
                SUBPORT_ERR(("linkphy_frag_byte_ctr failed for "
            ...
            /* fragment */
            if (linkphy_frag_ctr[unit][direction][block_id] != NULL) {
                SUBPORT_WARN(("WARN:Freeing linkphy_frag_ctr\n"));
                soc_cm_sfree(unit,
                    linkphy_frag_ctr[unit][direction][block_id]);
                linkphy_frag_ctr[unit][direction][block_id] = NULL;
            }
            ...
            if (linkphy_frag_ctr[unit][direction][block_id] == NULL) {
                SUBPORT_ERR(("linkphy_frag_ctr failed for "
                    " unit:%d,dir:%d, block:%d \n", unit, direction, block_id));
                soc_cm_sfree(unit,
                    linkphy_frag_byte_ctr[unit][direction][block_id]);
                linkphy_frag_byte_ctr[unit][direction][block_id] = NULL;
            ...
            /* frame byte */
            if (linkphy_frame_byte_ctr[unit][direction][block_id] != NULL) {
                SUBPORT_WARN(("WARN:Freeing linkphy_frame_byte_ctr\n"));
                soc_cm_sfree(unit,
                    linkphy_frame_byte_ctr[unit][direction][block_id]);
            }
            linkphy_frame_byte_ctr[unit][direction][block_id] = 
            ...
            if (linkphy_frame_byte_ctr[unit][direction][block_id] == NULL) {
                SUBPORT_ERR(("linkphy_frame_byte_ctr failed for "
                     "unit:%d,dir:%d, block:%d \n", unit, direction, block_id));
                soc_cm_sfree(unit,
                    linkphy_frag_byte_ctr[unit][direction][block_id]);
                soc_cm_sfree(unit,
            ...
            /* frame */
            if (linkphy_frame_ctr[unit][direction][block_id] != NULL) {
                SUBPORT_WARN(("WARN:Freeing linkphy_frame_ctr\n"));
                soc_cm_sfree(unit,
                    linkphy_frame_ctr[unit][direction][block_id]);
            }
            linkphy_frame_ctr[unit][direction][block_id] = 
            ...
            if (linkphy_frame_ctr[unit][direction][block_id] == NULL) {
                SUBPORT_ERR(("linkphy_frame_ctr failed for "
                    "unit:%d,dir:%d block:%d\n", unit, direction, block_id));
                soc_cm_sfree(unit,
                    linkphy_frag_byte_ctr[unit][direction][block_id]);
                soc_cm_sfree(unit,
    ...
    uint64 val)
{
    ...
    if (BCM_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: _bcm_linkphy_subport_port_get failed \n"
            "for subport 0x%x\n", subport));
        return rv;
    }
    ...
    rv = _bcm_linkphy_port_ext_stream_id_base_get(unit, port,
            &ext_stream_id_base);
    if (BCM_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Linkphy port %d external "
...
_bcm_linkphy_subport_init(int unit)
{
    ...
    SOC_PBMP_ITER(si->linkphy_pbm, port) {
        if (!_BCM_KT2_PORT_SUPPORTS_LINKPHY(unit, port)) {
            SUBPORT_ERR(("ERROR: config variable pbmp_linkphy "
   ...
   */
    ...
    if (_bcm_dev_int_stream_id_bitmap[unit] == NULL) {
        ...
        if (_bcm_dev_int_stream_id_bitmap[unit] == NULL) {
            SUBPORT_ERR(("ERROR:subport_init: dev_int_stream_id_bitmap "
                " alloc failed\n"));
    ...
    if (_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_GROUP(group)) {
        ...
        BCM_PBMP_ITER(SOC_INFO(unit).linkphy_pp_port_pbm, pp_port) {
            ...
            if (BCM_FAILURE(rv) && SOC_CB_ABORT_ON_ERR(unit)) {
                SUBPORT_ERR(("ERROR: subport group 0x%x traverse failed \n"
                              ...
                              bcm_gport_t *gport)
{
    ...
    *******************************/
    ...
    if (!_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_GROUP(config->group)) {
        SUBPORT_ERR(("ERROR: subport group 0x%x is not LinkPHY group\n",
            config->group));
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_LINKPHY_SUBPORT_GROUP_USED_GET(unit, sp_group_idx)) {
        SUBPORT_ERR(("ERROR: subport group 0x%x does not exist. group id =%d\n",
    ...
    if (si->port_num_subport[port] <= 0) {
        SUBPORT_ERR(("ERROR: %d number of subports available for port %d\n",
                    si->port_num_subport[port], port));
        return BCM_E_CONFIG;
    }

    if (config->num_streams > BCM_SUBPORT_CONFIG_MAX_STREAMS) {
        SUBPORT_ERR(("ERROR: num_streams %d cannot be greater than %d\n",
    ...
    if (config->prop_flags & BCM_SUBPORT_PROPERTY_PHB) {
        if ((config->int_pri < 0 || config->int_pri >= (1 << 4)) ||
            ((config->color != bcmColorGreen) &&
             ...
             SUBPORT_ERR(("ERROR: int_pri %d or color %d is invalid\n",
    ...
    rv = _bcm_linkphy_port_int_stream_id_base_get(unit, port,
            &int_stream_id_base);
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Linkphy port %d internal "
            "stream id base get failed\n", port));
        return rv;
    }
    ...
    for (i = 0; i < config->num_streams; i++) {
        ...
        if (config->stream_id_array[i] >=
            ...
            SUBPORT_ERR(("ERROR: stream_id %d is out of range for port %d\n",
        ...
        */
        ...
        if (_BCM_KT2_LINKPHY_DEV_INT_STREAM_ID_USED_GET(unit,
                dev_int_stream_id)) {
            SUBPORT_ERR(("ERROR: Device internal stream_id %d already used.\n",
                dev_int_stream_id));
            return BCM_E_EXISTS;
        }
    }
    ...
    *************************************/
    ...
    if (i == (si->port_subport_base[port] + si->port_num_subport[port])) {
        SUBPORT_ERR(("ERROR: All %d subport pp_port already used.\n",
    ...
    *********************************/
    for (i = 0; i < config->num_streams; i++) {
        ...
        */
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: device internal stream id  %d to pp_port %d "
        ...
        */
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: TX active stream bitmap enable failed.\n"
        ...
        */
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: TX data buffer address set failed.\n"
        ...
        rv = _bcm_linkphy_port_rx_active_stream_bitmap_set(unit,
                port, int_stream_id, enable);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: RX active stream enable failed\n"
                " port %d, internal stream_id = %d.\n", port, int_stream_id));
        ...
        rv = _bcm_linkphy_port_rx_int_stream_map_port_set(unit,
                port, int_stream_id, enable);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: RX stream map for port enable failed.\n"
                "port %d, internal stream_id = %d.\n", port, int_stream_id));
    ...
    */
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: pp_port id %d to physical port %d "
            "map failed.\n", pp_port_index, port));
        for (i = 0; i < config->num_streams; i++) {
                int_stream_id = config->stream_id_array[i] - ext_stream_id_base;
            dev_int_stream_id = int_stream_id + int_stream_id_base;
                                 ...
                                 bcm_gport_t subport_port_gport)
{
    ...
    if (!_BCM_KT2_GPORT_IS_LINKPHY_SUBPORT_PORT(unit, subport_port_gport)) {
        SUBPORT_ERR(("ERROR: gport 0x%x is not LinkPHY subport port type.\n",
                     subport_port_gport));
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_VALID_GET(unit, pp_port_index)) {
        SUBPORT_ERR(("ERROR: LinkPHY subport port gport 0x%x is invalid\n",
            subport_port_gport));
        return BCM_E_NOT_FOUND;
    }
    ...
    num_streams = _BCM_KT2_LINKPHY_SUBPORT_PORT_INFO_NUM_STREAMS_GET(unit,
                      pp_port_index);
    if (num_streams == -1) {
        SUBPORT_ERR(("ERROR: Subport port 0x%x does not have any streams.\n",
            subport_port_gport));
        return BCM_E_NOT_FOUND;
    }
    ...
    rv = _bcm_linkphy_port_int_stream_id_base_get(unit, port,
            &int_stream_id_base);
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Linkphy subport group internal stream id "
    ...
    rv = _bcm_linkphy_port_ext_stream_id_base_get(unit, port,
            &ext_stream_id_base);
    if (BCM_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Linkphy subport group external stream id "
            "base get failed for port %d.\n", port));
        return rv;
    }
    ...
    */
    ...
    for (i = 0; i < num_streams; i++) {
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: RX flow control disable failed\n"
            ...
            if (!(SAL_BOOT_SIMULATION)) {
                ...
                while (poll_active) {
                    ...
                    if (soc_timeout_check(&timeout)) {
                        SUBPORT_ERR(("ERROR: RX stream id %d flush timeout :"
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: RX active stream disable failed\n"
                " port %d, internal stream_id = %d.\n", port, int_stream_id));
            return rv;
        }
        ...
        rv = _bcm_linkphy_port_rx_int_stream_map_port_set(unit,
                port, int_stream_id, disable);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: RX stream map for port clear failed.\n"
                "port %d, internal stream_id = %d.\n", port, int_stream_id));
            return rv;
        }
        ...
        rv = _bcm_linkphy_port_tx_active_stream_set(unit,
                port, int_stream_id, disable);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: TX active stream bitmap disable failed.\n"
                "port %d, internal stream_id = %d.\n", port, int_stream_id));
            return rv;
        }
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: TX data buffer address clear failed.\n"
                "port %d, internal stream_id = %d, start_addr = %d "
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Cleaning pp_port id %d to physical port "
                    "map failed.\n", pp_port_index));
        return rv;
    }
                              ...
                              bcm_subport_config_t *config)
{
    ...
    if (!BCM_GPORT_IS_SUBPORT_GROUP(subport_group)) {
        SUBPORT_ERR(("ERROR:Invalid LinkPHY subport port gport 0x%x\n",
            subport_port_gport));
        return BCM_E_PARAM;
    }
    ...
    rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, dev_int_stream_id,
            &dev_sid_to_pp_entry);
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: DEVICE_STREAM_ID_TO_PP_PORT_MAPm read "
            "failed.for index %d\n", dev_int_stream_id));
        return rv;
    }
                                   ...
                                   void *user_data)
{
    ...
    BCM_PBMP_ITER(SOC_INFO(unit).linkphy_pp_port_pbm, pp_port_id) {
        ...
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: subport port traverse failed \n"
    ...
    BCM_PBMP_ITER(si->subtag_allowed_pbm, port) {
        ...
        if (port < SOC_MAX_NUM_PORTS) {
            ...
            if (_bcm_subtag_vlan_id_bitmap[unit][port] == NULL) {
                ...
                if (_bcm_subtag_vlan_id_bitmap[unit][port] == NULL) {
                    SUBPORT_ERR(("ERROR:_bcm_subtag_vlan_id_bitmap "
                                " alloc failed\n"));
                             ...
                             bcm_gport_t *gport)
{
    ...
    *******************************/
    ...
    if (!BCM_GPORT_IS_SUBPORT_GROUP(config->group)) {
        SUBPORT_ERR(("ERROR: group 0x%x is not subport group gport\n",
                    config->group));
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(config->group)) {
        SUBPORT_ERR(("ERROR: subport group 0x%x is not SubTag group\n",
            config->group));
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_SUBTAG_SUBPORT_GROUP_USED_GET(unit, sp_group_idx)) {
        SUBPORT_ERR(("ERROR: group 0x%x does not exist. group id =%d\n",
            config->group, sp_group_idx));
        return BCM_E_NOT_FOUND;
    }
    ...
    if (!SOC_PBMP_MEMBER(si->subtag_pbm, port)) {
        SUBPORT_ERR(("ERROR: Port %d is not member of pbmp_subport\n", port));
        return BCM_E_PORT;
    }
    ...
    if ((config->pkt_vlan & 0xFFF) <= BCM_VLAN_NONE) {
        SUBPORT_ERR(("ERROR: (config->pkt_vlan & 0xFFF) %d is invalid\n",
    ...
    if (config->prop_flags & BCM_SUBPORT_PROPERTY_PHB) {
        if (config->int_pri < 0 || config->int_pri > 15) {
            SUBPORT_ERR(("ERROR: int_pri %d is invalid\n", config->int_pri));
            return BCM_E_PARAM;
        }
        if ((config->color != bcmColorGreen) &&
             (config->color != bcmColorYellow) &&
             (config->color != bcmColorRed)) {
             SUBPORT_ERR(("ERROR: color %d is invalid\n", config->color));
            return BCM_E_PARAM;
        }
    }
    ...
    if (_BCM_KT2_SUBTAG_VLAN_ID_USED_GET(unit, port, vlan_id)) {
        SUBPORT_ERR(("ERROR: VLAN id %d already used for port %d\n",
            vlan_id, port));
        return BCM_E_EXISTS;
    }
    ...
    **********************************************************/
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Free entry not found in "
    ...
    ***********************************************/
    ...
    rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, subtag_index,
            &subtag_to_pp_entry);
    if (BCM_FAILURE(rv)) {
        SUBPORT_ERR(("SUBPORT_TAG_TO_PP_PORT_MAPm read failed at index %d\n",
                    subtag_index));
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: (VLAN id %d, port %d) to pp_port %d "
                                ...
                                bcm_gport_t subport_port_gport)
{
    ...
    if (!_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_PORT(unit, subport_port_gport)) {
        SUBPORT_ERR(("ERROR: gport 0x%x is not SubTag subport port type.\n",
                     subport_port_gport));
        return BCM_E_PARAM;
    }
    ...
    if (!_BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VALID_GET(unit, pp_port_index)) {
        SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x is invalid\n",
            subport_port_gport));
        return BCM_E_NOT_FOUND;
    }
    ...
    if (subtag_to_pp_port_tab_idx < 0 ||
        ...
        SUBPORT_ERR(("ERROR: SubTag subport port gport 0x%x is not found.\n"
    ...
    */
    ...
    rv = soc_mem_write(unit, mem, MEM_BLOCK_ANY, subtag_to_pp_port_tab_idx,
            &subport_tag_to_pp_port_map_entry);
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: subport_tag %d to pp_port %d "
                             ...
                             bcm_subport_config_t *config)
{
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_PORT(unit, subport_port_gport)) {
        ...
        if (!BCM_GPORT_IS_SUBPORT_GROUP(subport_group)) {
            SUBPORT_VERB(("ERROR:Invalid SubTag subport port gport 0x%x\n",
                subport_port_gport));
            return BCM_E_PARAM;
        }

        if (!_BCM_KT2_SUBTAG_SUBPORT_PORT_INFO_VALID_GET(unit, pp_port)) {
            SUBPORT_VERB(("ERROR: SubTag subport port gport 0x%x not found\n",
                subport_port_gport));
            return BCM_E_NOT_FOUND;
        }
    ...
    if (SOC_MEM_IS_VALID(unit, NHI_GROUP_TC_PROFILEm)) {
        for (nhi_group = 0; nhi_group <= 0xf; nhi_group++) {
            for (int_pri = 0; int_pri <= 0xf; int_pri++) {
                ...
                if (SOC_FAILURE(rv)) {
                    SUBPORT_ERR(("ERROR: NHI_GROUP_TC_PROFIE init failed\n"));
                    return rv;
                }
            }
        }
...
bcm_kt2_subport_init(int unit)
{
    ...
    if (_bcm_subport_group_bitmap[unit] == NULL) {
        ...
        if (_bcm_subport_group_bitmap[unit] == NULL) {
            SUBPORT_ERR(("ERROR:subport_init: group bitmap alloc failed\n"));
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY init failed.\n"));
    ...
    if (soc_feature(unit, soc_feature_subtag_coe) &&
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: SubTag init failed.\n"));
    ...
    if (soc_feature(unit, soc_feature_lltag)) {
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: tc profile init failed.\n"));
    ...
    if (_bcm_subport_pp_port_bitmap[unit] == NULL) {
        ...
        if (_bcm_subport_pp_port_bitmap[unit] == NULL) {
            SUBPORT_ERR(("ERROR:subport_init: pp_port bitmap alloc failed\n"));
    ...
    if (NULL == _bcm_subport_mutex[unit]) {
        ...
        if (_bcm_subport_mutex[unit] == NULL) {
            SUBPORT_ERR(("ERROR:linkphy_subtag subport mutex create failed\n"));
                             ...
                             bcm_gport_t *group)
{
    ...
    if ((config->flags & BCM_SUBPORT_GROUP_TYPE_LINKPHY) &&
        (config->flags & BCM_SUBPORT_GROUP_TYPE_SUBPORT_TAG)) {
        SUBPORT_ERR(("ERROR:Either LinkPHY or SubTag group can "
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Invalid gport 0x%x for subport group create.\n",
                    config->port));
        return BCM_E_PORT;
    }
    ...
    if ((config->flags & BCM_SUBPORT_GROUP_TYPE_SUBPORT_TAG) &&
        ...
        SUBPORT_ERR(("ERROR: SubportTag/CoE should be enabled"
                    " for port %d\n", port));
        return BCM_E_PORT;
    }

    if (_bcm_subport_group_count[unit] == _BCM_KT2_SUBPORT_GROUP_MAX) {
        SUBPORT_ERR(("ERROR: All %d subport groups already used.\n",
    ...
    if (config->flags & BCM_SUBPORT_GROUP_WITH_ID) {
        ...
        * Check that only subtag/linkphy group id bits (8-0) are configured*/
        if ((group_id >> _BCM_KT2_SUBPORT_GROUP_PORT_SHIFT)
            ...
            SUBPORT_ERR(("ERROR: subport group id = %d should be within 0 to"
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) && 
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY subport group create failed"
                " for port %d. rv = %d(%s)\n",
        ...
        rv  =_bcm_subtag_subport_group_create(unit,
                is_trunk, port, group_id, group);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: SubTag subport group create failed"
                " for port %d. rv = %d(%s)\n",
                config->port, rv, soc_errmsg(rv)));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return rv;
                              ...
                              bcm_gport_t group)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        if (BCM_PBMP_IS_NULL(si->linkphy_pbm)) {
            SUBPORT_ERR(("ERROR: LinkPHY ports not configured\n"));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return BCM_E_PARAM;
        }
        ...
        if (BCM_PBMP_IS_NULL(si->subtag_pbm)) {
            SUBPORT_ERR(("ERROR: SubTag ports not configured\n"));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return BCM_E_PARAM;
        }
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: subport group 0x%x destroy failed. rv= %d(%s)\n",
                          ...
                          bcm_subport_group_config_t *config)
{
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: subport group get failed for 0x%x, rv = %d(%s)\n",
                 group, rv, soc_errmsg(rv)));
        _BCM_KT2_SUBPORT_UNLOCK(unit);
        return rv;
    }
...

{
    ...
    *trunk_id = BCM_TRUNK_INVALID;
    ...
    if (!BCM_GPORT_IS_SUBPORT_GROUP(subport_group_gport)) {
        SUBPORT_WARN(("gport 0x%x is not subport gport\n",
            subport_group_gport));
        return BCM_E_NONE;
    }
    ...
    if ((soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        if(!_BCM_KT2_SUBPORT_GROUP_USED_GET(unit, group_id)) {
            SUBPORT_WARN(("subport gport 0x%x is not used\n",
                subport_group_gport));
            return BCM_E_NONE;
        }
                         ...
                         bcm_gport_t *gport)
{
    ...
    if (!BCM_GPORT_IS_SUBPORT_GROUP(config->group)) {
        SUBPORT_ERR(("ERROR: config->group 0x%x is not subport group gport\n",
            config->group));
        return BCM_E_PARAM;
    }
    ...
    if(!_BCM_KT2_SUBPORT_GROUP_USED_GET(unit, group_id)) {
        SUBPORT_ERR(("ERROR:config->group 0x%x is not found\n", config->group));
        return BCM_E_NOT_FOUND;
    }

    if (_bcm_subport_pp_port_count[unit] == _BCM_KT2_SUBPORT_PORT_MAX) {
        SUBPORT_ERR(("ERROR:All %d subport pp_port used\n",
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Subport port add failed. rv = %d(%s)\n",
            rv, soc_errmsg(rv)));
        _BCM_KT2_SUBPORT_UNLOCK(unit);
        return rv;
    }
                            ...
                            bcm_gport_t subport_port_gport)
{
    ...
    if (!BCM_GPORT_IS_SUBPORT_PORT(subport_port_gport)) {
        SUBPORT_ERR(("ERROR: gport 0x%x is not subport port gport\n",
            subport_port_gport));
        return BCM_E_PARAM;
    }
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR: Subport port 0x%x delete failed. rv = %d(%s)\n",
                         ...
                         bcm_subport_config_t *config)
{
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_VERB(("ERROR: Subport port 0x%x get failed. rv=%d(%s)\n",
                 subport_port_gport, rv, soc_errmsg(rv)));
        _BCM_KT2_SUBPORT_UNLOCK(unit);
        return rv;
    }
                              ...
                              void *user_data)
{
    ...
    if (SOC_FAILURE(rv)) {
        SUBPORT_ERR(("ERROR:Subport port traverse failed. rv = %d(%s)\n",
                    rv,soc_errmsg(rv)));
        _BCM_KT2_SUBPORT_UNLOCK(unit);
        return rv;
    }
                              ...
                              bcm_trunk_t *trunk_id, int *id)
{
    ...
    *trunk_id = BCM_TRUNK_INVALID;
    ...
    if (!BCM_GPORT_IS_SUBPORT_PORT(subport_port_gport)) {
        SUBPORT_ERR(("ERROR: gport 0x%x is not subport gport\n",
            subport_port_gport));
        return BCM_E_PARAM;
    }
...
bcm_kt2_subport_cleanup(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe)) {
        if (BCM_PBMP_NOT_NULL(si->linkphy_pbm)) {
            ...
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: LinkPHY cleanup failed\n"));
                return rv;
            }
        }
    }
    if (soc_feature(unit, soc_feature_subtag_coe)) {
        if (BCM_PBMP_NOT_NULL(si->subtag_allowed_pbm)) {
            ...
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: SubTag cleanup failed\n"));
                return rv;
            }
        }
    }
                             ...
                             bcm_subport_group_linkphy_config_t *linkphy_config)
{
    ...
    if (!BCM_PBMP_MEMBER(si->linkphy_pbm, port)) {
        SUBPORT_ERR(("ERROR: Invalid port %d for LinkPHY config get\n", port));
        return BCM_E_PORT;
    }
                             ...
                             bcm_subport_group_linkphy_config_t *linkphy_config)
{
    ...
    if (!BCM_PBMP_MEMBER(si->linkphy_pbm, port)) {
        SUBPORT_ERR(("ERROR: Invalid port %d for LinkPHY config set\n", port));
        return BCM_E_PORT;
    }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_BASE_STREAM_ID) {
        ...
        base_stream_id_max = _BCM_KT2_LINKPHY_STREAMS_MAX -
                             _BCM_KT2_LINKPHY_PER_PORT_STREAMS_MAX;
        if ((base_stream_id % 8) || (base_stream_id > base_stream_id_max)) {
            SUBPORT_ERR(("ERROR:base stream id should be "
                "multiple of 8 and <= %d\n", base_stream_id_max));
            return BCM_E_PARAM;
        }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_HEADER_MODE) {
        if ((linkphy_config->header_mode !=
            ...
            SUBPORT_ERR(("ERROR: invalid LinkPHY header mode %d\n",
                linkphy_config->header_mode));
            return BCM_E_PARAM;
        }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_RX_NEAREND_MAC) {
        rv = _bcm_linkphy_reg_mac_addr_set(unit, port,
                ...
                linkphy_config->rx_nearend_mac);
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY RX nearend MAC set failed\n"));
            return rv;
        }
    }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_RX_FAREND_MAC) {
        rv = _bcm_linkphy_reg_mac_addr_set(unit, port,
                ...
                linkphy_config->rx_farend_mac);
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY RX farend MAC set failed\n"));
            return rv;
        }
    }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_RX_DFC_DEST_MAC) {
        rv = _bcm_linkphy_reg_mac_addr_set(unit, port,
                ...
                linkphy_config->rx_dfc_dest_mac);
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY RX DFC dest MAC set failed\n"));
            return rv;
        }
    }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_TX_NEAREND_MAC) {
        rv = _bcm_linkphy_reg_mac_addr_set(unit, port,
                ...
                linkphy_config->tx_nearend_mac);
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY TX nearend MAC set failed\n"));
            return rv;
        }
    }
    ...
    if (mask & BCM_SUBPORT_LINKPHY_TX_FAREND_MAC) {
        rv = _bcm_linkphy_reg_mac_addr_set(unit, port,
                ...
                linkphy_config->tx_farend_mac);
        if (BCM_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: LinkPHY TX farend MAC set failed\n"));
            return rv;
        }
    }
...
bcm_kt2_subport_counter_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe)) {
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: subport counter init failed\n"));
            return rv;
        }
    }
    if (soc_feature(unit, soc_feature_subtag_coe)) {
...
bcm_kt2_subport_counter_cleanup(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe)) {
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: subport counter cleanup failed\n"));
            return rv;
        }
    }
    if (soc_feature(unit, soc_feature_subtag_coe)) {
    ...
    uint64 val)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: Subport 0x%x stat set failed. rv = %d(%s)\n",
    ...
    uint64 *val)
{
    ...
    if (soc_feature(unit, soc_feature_linkphy_coe) &&
        ...
        rv = _bcm_linkphy_subport_port_stat_get(unit,
                port, stream_id, stat_type, val);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: Subport 0x%x stat get failed. rv = %d(%s)\n",
                port, rv, soc_errmsg(rv)));
            _BCM_KT2_SUBPORT_UNLOCK(unit);
            return rv;
        }
                                     ...
                                     uint16 tpid)
{
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(gport)) {
        ...
        if (port > soc_mem_index_max(unit, mem)) {
            SUBPORT_ERR(("ERROR: Invalid port %d in subport group 0x%x\n",
                port, gport));
        }
        rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, port,
            &ing_physical_port_entry);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR:ING_PHYSICAL_PORT_TABLEm read failed\n"));
            return rv;
        }
        if (soc_mem_field32_get(unit, mem, &ing_physical_port_entry,
            ...
            rv = soc_mem_field32_modify(unit, mem , port, SUBPORT_TAG_TPIDf,
                tpid);
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: SubTag TPID setting failed for port %d\n",
                    port));
                return rv;
            }
        } else {
            SUBPORT_ERR(("ERROR: Port %d is not SubTag Cascaded\n", port));
            return BCM_E_PORT;
        }
        mem = EGR_PHYSICAL_PORTm;
        rv = soc_mem_read(unit, mem, MEM_BLOCK_ANY, port,
        ...
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR:EGR_PHYSICAL_PORTm read failed\n"));
            return rv;
        }
        rv = soc_mem_field32_modify(unit, mem , port, SUBPORT_TAG_TPIDf, tpid);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: SubTag TPID setting failed for egr_port %d\n",
                        port));
            return rv;
        }
    } else {
        SUBPORT_ERR(("ERROR: Invalid gport 0x%x for SubTag TPID set\n", gport));
        return BCM_E_PARAM;
    }
    return BCM_E_NONE;
}
                                        ...
                                        uint16 tpid)
{
    ...
    if (_BCM_KT2_GPORT_IS_SUBTAG_SUBPORT_GROUP(gport)) {
        ...
        if (soc_mem_field32_get(unit, mem, &ing_physical_port_entry,
            ...
            if (SOC_FAILURE(rv)) {
                SUBPORT_ERR(("ERROR: SubTag TPID delete failed for port %d\n",
                    port));
                return rv;
            }
        } else {
        ...
        rv = soc_mem_field32_modify(unit, mem , port, SUBPORT_TAG_TPIDf, 0);
        if (SOC_FAILURE(rv)) {
            SUBPORT_ERR(("ERROR: SubTag TPID delete failed for egr_port %d\n",
                        port));
            return rv;
        }
    } else {
            ...
            for(stat = 0; stat <= 7; stat++) {
                ...
                if (rv < 0) {
                    ...
                    if (!COMPILER_64_IS_ZERO(val)) {
                        soc_cm_print("Subport(0x%x) Port(%02d) "
        ...
        if (flag == 0) {
            ...
            if (sid != -1) {
                if (sid >= _BCM_KT2_LINKPHY_PER_PORT_STREAMS_MAX) {
                    soc_cm_print("invalid stream ID\n");
                    return BCM_E_PARAM;
                } else {
                    for (block_id = 0;
...
_bcm_kt2_subport_sw_dump(int unit)
{
    ...
    soc_cm_print("Subport stream ids and pp_port used:\n");
