/*
 * $Id: ddrtest.c 1.32 Broadcom SDK $
...
ddr_test_init(int unit, args_t *a, void **p)
{
    ...
    if (dt == NULL) {
        ...
        if (dt == NULL) {
            soc_cm_debug(DK_ERR, "%s: cannot allocate memory test data\n", ARG_CMD(a));
            return -1;
        }
    ...
    if (SOC_IS_CALADAN3(unit)) {
        parse_table_add(&pt, "BurstSize", PQ_INT, (void *) (-1),
            &ddr_burst_size, NULL);
    } else 
#endif
...
#ifdef BCM_CALADAN3_SUPPORT
        if (SOC_IS_CALADAN3(unit)) {
            soc_reg_field_set(unit, CI_DDR_TESTr, &uData, WRITE_ONLYf, dt->write_test); 
            soc_reg_field_set(unit, CI_DDR_TESTr, &uData, READ_ONLYf, dt->read_test); 
        }
#endif
...
int DumpDDRResults(int unit,
    ...
    for(ci = ci_start; ci < ci_end; ci++) {
        if (iter_count != -1 ) {
            soc_cm_debug(DK_TESTS, "Iteration:%d CI%d Fail:0x%08x%08x Pass:0x%08x%08x Timedout:0x%08x%08x\n",
                         iter_count,ci,
            ...
            soc_cm_debug(DK_TESTS, "CI%d Fail:0x%08x%08x Pass:0x%08x%08x \n",ci,
                         COMPILER_64_HI(pFailedCount[ci]),COMPILER_64_LO(pFailedCount[ci]),
                         COMPILER_64_HI(pPassedCount[ci]),COMPILER_64_LO(pPassedCount[ci]));
        }
...
int DDRTest(ddr_test_params_t *testparams) {
    ...
    if (SOC_IS_CALADAN3(unit))  {
        if (dt->ddr_burst < 0) {
            ...
            if (SOC_DDR3_NUM_ROWS(unit) > 32*1024) {
                uDDRBurstSize = (1 << 25) -1;
            } else if (SOC_DDR3_NUM_ROWS(unit) > 16*1024) {
                uDDRBurstSize = (1 << 24) -1;
            } else if (SOC_DDR3_NUM_ROWS(unit) > 8*1024) {
                uDDRBurstSize = (1 << 23) -1;
            } else {
                uDDRBurstSize = (1 << 22) -1;
            }
        }
    } 
#endif
    ...
    if (DDR_STANDARD_TEST == ddr_test ||
        ...
        soc_cm_debug(DK_TESTS, "start_addr=0x%x,addr_step_inc=0x%x,burst_size=%d\n",uStartAddr,uStepSize,uDDRBurstSize);
        if (DDR_STANDARD_TEST == ddr_test) {
            soc_cm_debug(DK_TESTS, "using pattern=0x%8x, and alt_pattern=0x%8x\n",pattern,~pattern);
        }
    }
    ...
    for(iter_count = 0; iter_count < uDDRIter; iter_count++) {
        ...
        if (( rv = StartDDRFunctionalTest(dt, ci_start,
                                          ci_end, ddr_test)) < 0 ) {
            soc_cm_debug(DK_ERR, "%s failed to start %s test.\n",SOC_CHIP_STRING(unit),
                         DDRMemTestStr[ddr_test]);
            return rv;
        }
        ...
        if (( rv = CheckDDRFunctionalTest(dt,
            ...
            soc_cm_debug(DK_ERR, "%s failed %s test.\n",SOC_CHIP_STRING(unit),
                         DDRMemTestStr[ddr_test]);
...
int DDRIndirectTest(ddr_test_params_t *testparams) {
    ...
    for(ci=ci_start;ci<ci_end;ci++) {
        WRITE_DDR40_PHY_WORD_LANE_0_READ_FIFO_CLEARr(unit, ci, 0);
        WRITE_DDR40_PHY_WORD_LANE_1_READ_FIFO_CLEARr(unit, ci, 0);
        soc_cm_debug(DK_TESTS, "Filling ci%d\n", ci);
        for (bank=start_bank;bank<end_bank;bank++) {
            for(row=0;row<max_row;row++) {
                for (col=0;col<max_col;col++) {
                    ...
                    soc_cm_debug(DK_TESTS, "Filling ci%d bank[%d],row[0x%x],cols[0x%x-0x%x] \n",
    ...
    for(ci=ci_start;ci<ci_end;ci++) {
        soc_cm_debug(DK_TESTS, "Verifying ci%d\n", ci);
        for (bank=start_bank;bank<end_bank;bank++) {
            for(row=0;row<max_row;row++) {
                for (col=0;col<max_col;col++) {
                    pla_addr = bank;
                    pla_addr |= col << 3;
                    pla_addr |= row << 9;
                    soc_cm_debug(DK_TESTS, "Verifing ci%d bank[%d],row[0x%x],cols[0x%x-0x%x] \n",
                                 ci,bank,row,col<<4,(col<<4)+0xf);
                    ...
                    for (i = 0; i < 8; i++ ) {
                        ...
                        if (uDataRD[i] != comp_data) {
                            soc_cm_debug(DK_ERR, "Data compare failure at pla_addr:(0x%x)\n",pla_addr);
                            soc_cm_debug(DK_ERR, "Expected (0x%x) got (0x%x)\n",
                                         comp_data,uDataRD[i]);
                            /* Compare Error */
                            return -1;
                        }
...
ddr_test(int unit, args_t *a, void *p)
{
    ...
    if (soc && !(soc->soc_flags & SOC_F_INITED)) {
        ...
        if (SOC_FAILURE(rv)) {
            soc_cm_debug(DK_ERR, "Unit %d: soc reset failed got (%d)\n", unit, rv);
            return -1;
        }
    }
#endif
...
{
    soc_cm_debug(DK_TESTS, "DDR Mem Tests Done\n");
...
ddr_bist_test_init(int unit, args_t *a, void **p)
{
    ...
    if (dt == NULL) {
        ...
        if (*dram_ndx_srt == '\0') {
            ...
            BCM_PBMP_PORT_ADD(dt->dram_ndx, 0);
...
ddr_bist_test_done(int unit, void *p)
{
    ...
    soc_cm_debug(DK_TESTS, "DDR Bist Tests Done\n");
    sal_free(ddr_test_params[unit]);
    ddr_test_params[unit] = NULL;
    return BCM_E_NONE;
}
    ...
    if (SOC_IS_KATANA2(unit) ) {
        ...
        SOC_PBMP_ITER(dt->dram_ndx, ci) {
            if (dt->run_info.ddr_bist_flags & DDR_BIST_ALL_ADDRESS) {
                ...
                dt->run_info.ddr_bist_flags = DDR_BIST_ALL_ADDRESS | DDR_BIST_CONS_ADDR_8_BANKS;
                ...
                if (dt->run_info.bist_end_address > max_bist_end_addr ) {
                    soc_cm_print("bist_end_address:%x exceeded max bist addr:%x"
                                 " so Resetting to max \n",
               ...
               if (_ddr_bist_test_start(dt->unit, ci, &(dt->run_info)) < 0) {
                    printk("Error: First _ddr_bist_test_start () Failed:\n");
                    return BCM_E_FAIL;
                }
                ...
                if (_ddr_bist_test_start(dt->unit, ci, &(dt->run_info)) < 0) {
                     printk("Error: Second _ddr_bist_test_start () Failed:\n");
                     return BCM_E_FAIL;
                 }
            } else { /* All Address */
                if (dt->run_info.bist_end_address > max_bist_end_addr ) {
                ...
                if (dt->run_info.bist_start_address > max_bist_start_addr ) {
                    soc_cm_print("bist_start_address:%x exceeded max bist "
                                 "addr:%x so Resetting to max - 1 \n",
            ...
            if ((bist_err_occured != 0x0)  || (bist_full_err_cnt != 0x0)  || (bist_single_err_cnt != 0x0)  || (bist_global_err_cnt != 0x0)){
                printk("\nCI: %d \nResults:\n", ci);
                printk("Number of errors occured: 0x%x\n", bist_err_occured);
                printk("Number of full errors: %d\n", bist_full_err_cnt);
                printk("Number of single errors: %d\n", bist_single_err_cnt);
                printk("Number of global errors: %d\n", bist_global_err_cnt);
                return BCM_E_FAIL;
            }
        }
        return BCM_E_NONE;
...
#endif /* defined(BCM_DDR3_SUPPORT) || defined(BCM_ARAD_SUPPORT) */
