...
static uint32 _soc_cmic_uc_msg_base(int unit)
{
    if (SOC_IS_HELIX4(unit) || SOC_IS_KATANA2(unit)) {
        return (soc_property_get(unit, "uc_msg_base_ovride", 0x1b07f008));
    }
...
{
    uint32 msg_base = _soc_cmic_uc_msg_base(unit);
    uint32 bs_reg_base = MSG_BS_CACHE(msg_base);
    return bs_reg_base + sizeof(mos_msg_bs_reg_cache_t) * bs_num;
}
