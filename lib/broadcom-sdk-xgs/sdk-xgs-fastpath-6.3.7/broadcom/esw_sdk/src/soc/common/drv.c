 ...
 * $Id: drv.c 1.145.2.1 Broadcom SDK $    
...
soc_property_get_str(int unit, const char *name)
{
    if (unit >= 0) {
        ...
        if (soc_family_suffix[unit]) {
            ...
            sal_strcpy(name_exp + l, soc_family_suffix[unit]);
            if ((s = soc_cm_config_var_get(unit, name_exp)) != NULL) {
                return s;
            }
        }
...
soc_pci_ep_config(int unit, int pcie)
{
    ...
    if (soc_feature(unit, soc_feature_iproc) &&
        ...
        soc_cm_iproc_write(unit, soc_reg_addr_get(unit, DMU_PCU_PCIE_SLAVE_RESET_MODEr, REG_PORT_ANY, 0, &blk, &acc_type), 1);
        ...
        if (pci_num == 0) {
            ...
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_0_PCIE_EP_AXI_CONFIGr, REG_PORT_ANY, 0, &blk, &acc_type), 0);
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_0_OARR_2r, REG_PORT_ANY, 0, &blk, &acc_type), 1);
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_0_OARR_2_UPPERr, REG_PORT_ANY, 0, &blk, &acc_type), 1);
        } else {
            ...
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_1_PCIE_EP_AXI_CONFIGr, REG_PORT_ANY, 0, &blk, &acc_type), 0);
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_1_OARR_2r, REG_PORT_ANY, 0, &blk, &acc_type), 1);
            soc_cm_iproc_write(unit, soc_reg_addr_get(unit, PAXB_1_OARR_2_UPPERr, REG_PORT_ANY, 0, &blk, &acc_type), 2);
...
soc_autoz_get(int unit, soc_port_t port, int *enable)
{
  ...
  defined(BCM_PETRA_SUPPORT)|| defined(BCM_DFE_SUPPORT) || defined(BCM_CALADAN3_G3P1_SUPPORT)
...
soc_wc_xgxs_pll_check(int unit, soc_port_t port, int reg_idx)
{
    ...
    SOC_ERROR_PRINT((DK_ERR, "unit %d %s TXPLL not locked\n",
...
soc_wc_xgxs_power_down(int unit, soc_port_t port, int reg_idx)
{
    ...
    SOC_IF_ERROR_RETURN(soc_reg_set(unit, reg, port, 0, rval64));
    SOC_DEBUG_PRINT((DK_VERBOSE, "Power down wc for port: %d\n", port));
    return SOC_E_NONE;
}
#endif /* BCM_TRIDENT_SUPPORT */
...
soc_tsc_xgxs_pll_check(int unit, soc_port_t port, int reg_idx)
{
    ...
    SOC_ERROR_PRINT((DK_ERR, "unit %d TSC %d TXPLL not locked\n",
