/*
 * $Id: cm.c 1.296.2.5 Broadcom SDK $
...
void soc_cm_dump_info(int dev)
{
   ...
   soc_cm_print("==================== SHARED MEMORY INFO(BEGIN) ===========\n");
   soc_cm_print("Shared_start:%p Shared_end:%p\n",shared_start,shared_end);
   while(p != NULL) {
         size_words = (p->size + 3) / 4;
         soc_cm_print("Info p=:%p: Start :0x%x: Desc:%s: Size:%d:"
                      "ModifiedSize:%d End:0x%x\n",
                       p, p->start_sentinel, p->description, p->size,
                       p->modified_size, p->user_data[size_words]);
         p = p->next;
   }
   soc_cm_print("==================== SHARED MEMORY INFO(END)   ===========\n");
   return;
}
#endif
