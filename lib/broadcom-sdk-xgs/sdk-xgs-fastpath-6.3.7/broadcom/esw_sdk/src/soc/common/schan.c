 ...
 * $Id: 90239cd3a855050dd278b1090532862612bd9171 $
...
#define IS_WRITE_ALLOWED_DURING_WB(unit) \
     ...
     */
    if (SOC_IS_ARAD(unit) && 
        msg->header.opcode != READ_REGISTER_CMD_MSG &&
        msg->header.opcode != READ_MEMORY_CMD_MSG &&
...
#endif /* CMICM Support */
            {
                ...
                if (schanCtrl & SC_MSG_NAK_TST) {
                    ...
                    do {
                        ...
                        case READ_MEMORY_CMD_MSG:
                        case READ_REGISTER_CMD_MSG:
                            soc_ser_mem_nack(INT_TO_PTR(unit), 
                                INT_TO_PTR(msg->gencmd.address), 
                                INT_TO_PTR(msg->gencmd.header.dstblk), 
                                msg->header.opcode == READ_REGISTER_CMD_MSG ? INT_TO_PTR(1) : 0, 0);
                            break;
            ...
            if (SOC_IS_TRIUMPH3(unit)) {
                if (msg->header.opcode == WRITE_REGISTER_CMD_MSG) {
                    ...
                    if (soc_cm_debug_check(DK_VERBOSE)) {
                        soc_cm_print("soc_schan_op: operation attempt timed out\n");
    ...
    if (rv == SOC_E_TIMEOUT) {
        if (soc_cm_debug_check(DK_ERR)) {
            soc_cm_debug(DK_ERR, "SchanTimeOut:soc_schan_op operation timed out\n");
            soc_schan_dump(unit, msg, dwc_write);
        }
    }
