...
phy_hl65_lb_get(int unit, soc_port_t port, int *enable)
{
   ...
   if (PHY_HC65_FABRIC_MODE(unit, port)) {
        ...
        *enable = ((data & lane_mask) == lane_mask);
        SOC_DEBUG_PRINT((DK_PHY, "phy_hc65_loopback_get: "
                     "u=%d p=%d lb=%d\n", unit, port, *enable));
...
_phy_hl65_control_prbs_polynomial_set(int unit, soc_port_t port, uint32 value)
{
    ...
    if (PHY_HC65_FABRIC_MODE(unit, pc->port)) {
      ...
      data |= value << (PHY_HL65_LANEPRBS_LANE_SHIFT * pc->lane_num);
        ...
        if (value) {
            /* set all 4 lanes */
            data |= ((value << 12) | (value << 8) | (value << 4) | value); 
        } else {
            data &= ((value << 12) | (value << 8) | (value << 4) | value); 
        }
    }	
...
_phy_hl65_control_prbs_tx_invert_data_set(int unit, soc_port_t port, uint32 value)
{
    ...
    if (value > 0x8000) {
        SOC_IF_ERROR_RETURN
	    (WRITE_HL65_XGXSBLK1_LANEPRBSr(unit, pc, value));
    } else {
        SOC_IF_ERROR_RETURN
	    (WRITE_HL65_XGXSBLK1_LANEPRBSr(unit, pc, data));
    }
                                          ...
                                          uint32 value)
{
    ...
    if (value) {
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK0_XGXSCONTROLr(unit, pc, 0x203f));
        
        sal_usleep(1000);
        
        /* Set double wide div/1: 0x8016=0xffff */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK1_LANECTRL1r(unit, pc, 0xffff));
        
        /* clear multiPRT mode: 0x800d=0x0003 */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK0_MMDSELECTr(unit, pc, 0x4003));
        
        /* Set in individual lane mode, Disable PLL Sequencer: 0x8000=0x063f */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK0_XGXSCONTROLr(unit, pc, 0x063f));

        /* Turn AN off: 0x0=0x0 */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_IEEE0BLK_MIICNTLr(unit, pc, 0));
        
        /* Turn CL36 PCS off: 0x8015=0x0 */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK1_LANECTRL0r(unit, pc, 0));
	...
	data |= 0x8888;
        ...
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_RXB_ANARXCONTROLr(unit, pc, 0x1c47));
        
        /* Enable PLL Sequencer: 0x8000=0x263f */
        SOC_IF_ERROR_RETURN
            (WRITE_HL65_XGXSBLK0_XGXSCONTROLr(unit, pc, 0x263f));
                                         ...
                                         uint32 value)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "hl_65: Equalization tune start\n"));
    return SOC_E_NONE;
}
...
_phy_hl65_tuning_done_get(int unit, soc_port_t port,int lane_num, uint16 *done)
{
    ...
    *done = 0;
    ...
    /* If tuning is done */
    /* if (uData & 0x2000) {
     *	BCM_ERR(("hc(%02d:%02d) Tuning Done indication received\n", uPhyAddr, uLaneAddr));
     *	*bTuningDone = TRUE;
     *}
     */
    *done = (data & 
                            ...
                            uint16 *dfe_tap_bin_avg)
{
    ...
    *dfe_tap_bin_avg = avg_dfe;
    ...
    /* BCM_ERR(("hc(%02d:%02d) VGA avg(%d)\n", uPhyAddr, uLaneAddr, uAvgVga));
     * BCM_ERR(("hc(%02d:%02d) DFE avg(%d)\n", uPhyAddr, uLaneAddr, uAvgDfe));
     */
    SOC_DEBUG_PRINT((DK_PHY, "hc(%02d) VGA avg(%d)\n", port, avg_vga));
    SOC_DEBUG_PRINT((DK_PHY, "hc(%02d) DFE avg(%d)\n", port, avg_dfe));
                                     ...
                                     uint32 *value)
{
    ...
    *value = FALSE;
    ...
    /*if (!bSigDetected) {
     *  BCM_ERR(("ERROR: Receiver indicates that no signal detected, check far end enable.\n"));
     *}
     */
    if (!sig_detected) {
        SOC_DEBUG_PRINT((DK_ERR, 
    ...
    /* if (!bTuningDone) {
     *   BCM_ERR(("ERROR: Tuning error port(%d), no tuning done indication from hardware\n", port));
     */
    if (!tuning_done) {
        SOC_DEBUG_PRINT((DK_ERR, 
                                          ...
                                          int lane_num)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 1: check tuning done\n"));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 2: compute vga_sum, dfe_tap_bin average\n"));
    SOC_IF_ERROR_RETURN
        (_phy_hl65_tuning_status_get(unit, port, pc->lane_num,
                                     &vga_sum_avg, &dfe_tap_bin_avg));

    /* Step 3: fix vga_sum and dfe_tab_bin  */
    SOC_DEBUG_PRINT((DK_PHY, "Step 3: fix vga_sum, dfe_tap_bin\n"));
    reg_addr = 0x8215 + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
                                              ...
                                              phy_hl65_dsc_cfg_t dsc_cfg)
{
     ...
     *     ==0   2xOSR 
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 4: verify clk90_phase_offset\n"));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 5: enable clk90 offset override\n"));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 6: set pi_phase_delta\n"));
    if (dsc_cfg == PHY_HL65_DSC_CFG_1X_OSR) {
        ...
        SOC_DEBUG_PRINT((DK_ERR,"ERROR: hc(%02d) invalid DSC config\n", 
                         port));
        return SOC_E_PARAM;
    }
                                               ...
                                               int lane_num, uint32 inc)
{
     ...
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 7: set phase offset moving direction\n"));
    reg_addr = 0x8213 + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 8: toggle pi phase strobe\n"));
                                       ...
                                       
{
     ...
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 11: restore values\n"));
    reg_addr = 0x8213 + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 12: release vga_write, dfe_write enable\n"));
    reg_addr = 0x8215 + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    data &= ~(DSC1B0_DFE_VGA_CTRL1_DFE_WRITE_VAL_MASK |
                                                  ...
                                                  uint32 value)
{
    ...
    if (value > max) {
        SOC_DEBUG_PRINT((DK_ERR, "Invalid value %d (0..%d)\n", 
                         value, max-1));
        return SOC_E_PARAM;
    }
    ...
    SOC_DEBUG_PRINT((DK_PHY, "horizontal adjusting to %d from %d steps %d\n",
                                            ...
                                            int lane_num)
{
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 4: read slice offset status\n"));
    ...
    SOC_DEBUG_PRINT((DK_PHY, "Step 5: write slice offset status\n"));
    reg_addr = 0x821b + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
                                             ...
                                             int lane_num, uint32 inc)
{
     ...
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 6: perturb data even slicer\n"));
    reg_addr = 0x821b + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    ...
    if (inc == 1) {
        ...
        SOC_DEBUG_PRINT((DK_ERR, "ERROR: hc(%02d) diag invalid operation\n", 
                         port));
        return SOC_E_PARAM;
    }
                                                ...
                                                int lane_num, uint32 value)
{
    ...
    SOC_IF_ERROR_RETURN
        (_phy_hl65_serdes_tune_margin_vertical_value_get(unit, port, 
                                                         ...
                                                         &cur_value));
    if (cur_value != value) {
        SOC_DEBUG_PRINT((DK_WARN, "Adjusted value %d does not match "
                         "requested value %d\n", cur_value, value));
    }
    return SOC_E_NONE;
}
                                           ...
                                           int lane_num)
{
     ...
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 9: restore slice offset\n"));
    reg_addr = 0x821b + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
     ...
     */
    SOC_DEBUG_PRINT((DK_PHY, "Step 10: release vga_write, dfe_write enable\n"));
    reg_addr = 0x8215 + (lane_num * 0x10);
    SOC_IF_ERROR_RETURN
        (HL65_REG_READ(unit,pc,0x0,reg_addr, &data));
    data &= ~(DSC1B0_DFE_VGA_CTRL1_DFE_WRITE_VAL_MASK |
...
phy_hl65_uc_status_dump(int unit, soc_port_t port)
{
    ...
    for (lane_num = 0; lane_num < MAX_LANES; lane_num++) {
        ...
        if( print_menu == TRUE) { 
            ...
            soc_cm_print("LN PPM  PPM_XFR clk90_ofs " 
                         "PF   SL_TRGT VGA  DFE  " 
        ...
        soc_cm_print("%02d %04d %07d %09d "
                     "%04d %07d %04d %04d "
                     "0x%06x 0x%06x %04d %04d "
                     "%04d %2d %3d %3d "
                     "%2d\n",
           lane_num, pDesc->integ_reg, pDesc->integ_reg_xfer, pDesc->clk90_offset,
           pDesc->pf_ctrl, pDesc->slicer_target, pDesc->vga_sum, pDesc->dfe_bin,
           pDesc->tx_os, pDesc->tx_br,  pDesc->postc_metric, pDesc->offset_pe,
           pDesc->offset_ze, pDesc->offset_me, pDesc->offset_po, pDesc->offset_zo, 
           pDesc->offset_mo);
