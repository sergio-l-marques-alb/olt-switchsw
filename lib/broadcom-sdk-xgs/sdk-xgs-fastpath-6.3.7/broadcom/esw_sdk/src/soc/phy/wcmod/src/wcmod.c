/*
 * $Id: wcmod.c 1.388.2.6 Broadcom SDK $ 
...
phy_wcmod_link_get(int unit, soc_port_t port, int *link)
{
    ...
    if (!SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), pc->port)) {
        if (SOC_INFO(unit).port_num_lanes[pc->port] >= 10) {
            wcmod_st  *mld, *slave0, *slave1, *wcmod;
            int is_non_ieee_am = 0;
            int       num_cores = 3, tmpLane = 0;
            slave0 = (wcmod_st *)(ws + 1);
            slave1 = (wcmod_st *)(ws + 2);
            mld   =  (wcmod_st *)( ws + 3);
            ...
            if (data16 & 0x4) {
              ...
              *link = 0;
              /*SW WAR for JIRA SDK-56837. Look for rxSeqDone bit if one of the active 
              lanes has a rxSeqDone set to 0 then set the AM space to non IEEE value that would 
              bring LPs PCS link down */
              /* Start with slave 2 - (100G 4 4 2 config for TRIUMPH3) */
              ...
              while (num_cores > 0) {
                  ...
                  tmpLane = wcmod->this_lane;
                  for (index = lane_start; index <= lane_end; index++) {
                      switch(index) {
                          case 0:
                              wcmod->lane_select = WCMOD_LANE_0_0_0_1;
                          break;
                          case 1:
                              wcmod->lane_select = WCMOD_LANE_0_0_1_0;
                          break;
                          case 2:
                              wcmod->lane_select = WCMOD_LANE_0_1_0_0;
                          break;
                          case 3:
                              wcmod->lane_select = WCMOD_LANE_1_0_0_0;
                          break;
                          default:
                          break;     
                      }    
                      wcmod->this_lane = index;
                      wcmod->per_lane_control = 0x10;
                      SOC_IF_ERROR_RETURN (wcmod_tier1_selector("RX_SEQ_CONTROL", wcmod, &rv));
                      if (!wcmod->accData) {
                          is_non_ieee_am = 1;
                          break;
                      }    
                  }
                  wcmod->this_lane = tmpLane;
                  wcmod->per_lane_control = 0;
                  if (is_non_ieee_am) {
                       data16 = 0;
                      /* Program AM apacing to non IEEE Value */
                      SOC_IF_ERROR_RETURN (WC40_REG_READ(unit, mld, 0x0, 0x8420, &data16));
