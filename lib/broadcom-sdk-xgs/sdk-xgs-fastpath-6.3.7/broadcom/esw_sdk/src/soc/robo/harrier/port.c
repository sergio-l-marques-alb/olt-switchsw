...
_drv_harrier_set_MAC_auto_negotiation(int unit, int port, int enable)
{
  ...
  soc_cm_debug(DK_PORT, "_drv_harrier_set_MAC_auto_negotiation.\n");
...
drv_harrier_port_set(int unit, soc_pbmp_t bmp, uint32 prop_type, uint32 prop_val)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_port_set: unit=%d bmp=%x %x\n", \
    ...
    switch (prop_type) {
        case DRV_PORT_PROP_SPEED:
            soc_cm_debug(DK_PORT,"drv_harrier_port_set: PROP_SPEED\n");
        ...
        case DRV_PORT_PROP_DUPLEX:
            soc_cm_debug(DK_PORT,"drv_harrier_port_set: PROP_DUPLEX\n");
            switch (prop_val) {
        ...
        case DRV_PORT_PROP_RESTART_AUTONEG:
            soc_cm_debug(DK_PORT,"drv_harrier_port_set: PROP_AUTONEG\n");
        ...
        case DRV_PORT_PROP_TX_PAUSE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_TX_PAUSE\n");
            PBMP_ITER(bmp, port) {
                /* set TX PAUSE */
                p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
                if (p_mac != NULL) {
        ...
        case DRV_PORT_PROP_RX_PAUSE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_RX_PAUSE\n");
            PBMP_ITER(bmp, port) {
                 /* set TX PAUSE */
                p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
                if (p_mac != NULL) {
        ...
        case DRV_PORT_PROP_LOCAL_ADVERTISE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: LOCAL_ADVER\n");
            PBMP_ITER(bmp, port) {
        ...
        case DRV_PORT_PROP_MAC_ENABLE:
            ...
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_MAC_ENABLE\n");
            PBMP_ITER(bmp, port) {
                /* MAC register(s) should be set also */
                p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
                if (p_mac != NULL) {
        ...
        case DRV_PORT_PROP_ENABLE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_ENABLE\n");
            PBMP_ITER(bmp, port) {
        ...
        case DRV_PORT_PROP_EGRESS_PCP_REMARK:
        case DRV_PORT_PROP_EGRESS_CFI_REMARK:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_ENABLE_SET\n");
            PBMP_ITER(bmp, port) {
        ...
        case DRV_PORT_PROP_IPG_FE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_IPG_FE\n");
        ...
        case DRV_PORT_PROP_IPG_GE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_IPG_GE\n");
            if (SOC_PBMP_EQ(bmp, PBMP_ALL(unit))) { /* per system */
                if ((rv = REG_READ_SWMODEr(unit, &reg_value)) < 0) {
                    return rv;
                }
        ...
        case DRV_PORT_PROP_JAM:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_JAM\n");
        ...
        case DRV_PORT_PROP_BPDU_RX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_BPDU_RX\n");
        ...
        case DRV_PORT_PROP_MAC_LOOPBACK:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_MAC_LOOPBACK\n");
            PBMP_ITER(bmp, port) {
                p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
                if (p_mac != NULL) {
        ...
        case DRV_PORT_PROP_PHY_LOOPBACK:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_PHY_LOOPBACK\n");
            PBMP_ITER(bmp, port) {
                /* Special case :
        ...
        case DRV_PORT_PROP_PHY_MDIX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_PHY_MDIX\n");
            PBMP_ITER(bmp, port) {
        ...
        case DRV_PORT_PROP_MS:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_MS not support\n");
            PBMP_ITER(bmp, port) {
                rv = soc_phyctrl_master_set(unit, port, prop_val);
            }
            break;
        case DRV_PORT_PROP_SEC_MAC_MODE_NONE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_SEC_MODE_NONE\n");
            PBMP_ITER(bmp, port) {
        ...
        case DRV_PORT_PROP_SEC_MAC_MODE_DYNAMIC_SA_MATCH:
            ...
            break;
        case DRV_PORT_PROP_SEC_MAC_MODE_EXTEND:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_SEC_MAC_MODE_EXTEND\n");
            PBMP_ITER(bmp, port) {
                rv = _drv_harrier_port_security_mode_set(unit, port, prop_type, 0);
            }
            break;
        ...
        case DRV_PORT_PROP_SEC_MAC_MODE_SIMPLIFY:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_SEC_MAC_MODE_SIMPLIFY\n");
            PBMP_ITER(bmp, port) {
                rv = _drv_harrier_port_security_mode_set(unit, port, prop_type, 0);
            }
            break;
        ...
        case DRV_PORT_PROP_PHY_RESET:
            ...
            break;
        case DRV_PORT_PROP_DTAG_MODE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_DTAG_MODE\n");
            rv = _drv_harrier_port_property_enable_set(unit, 0, prop_type, prop_val);
            break;
        case DRV_PORT_PROP_DTAG_ISP_PORT:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_DTAG_ISP_PORT\n");
            PBMP_ITER(bmp, port) {
                rv = _drv_harrier_port_property_enable_set(
                    unit, port, prop_type, prop_val);
            }
        ...
        case DRV_PORT_PROP_DTAG_TPID:
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_DTAG_TPID\n");
        ...
        case DRV_PORT_PROP_802_1X_MODE :
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_802_1X_MODE\n");
            rv = SOC_E_PARAM;
            break;
        case DRV_PORT_PROP_802_1X_BLK_RX :
            soc_cm_debug(DK_PORT, "drv_harrier_port_set: PROP_802_1X_BLK_RX\n");
            rv = SOC_E_PARAM;
            break;
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_port_set: Exit\n");
    return rv;
}
...
int drv_harrier_port_get(int unit, int port, uint32 prop_type, uint32 *prop_val)
{
    ...
    soc_cm_debug(DK_PORT, "drv_harrier_port_get: unit=%d port=%d\n", unit, port);

    SOC_ROBO_PORT_INIT(unit);
    p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
    switch (prop_type) {
        case DRV_PORT_PROP_SPEED:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Speed\n");
            temp = 0;
            ...
            break;
        case DRV_PORT_PROP_DUPLEX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Duplex\n");
            temp = 0;
            ...
            break;
        case DRV_PORT_PROP_AUTONEG:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Autoneg\n");
            ...
            *prop_val = (autoneg) ? DRV_PORT_STATUS_AUTONEG_ENABLE :
            ...
            break;
        case DRV_PORT_PROP_TX_PAUSE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: TX Pause\n");
            if (p_mac != NULL) {
            ...
            break;
        case DRV_PORT_PROP_RX_PAUSE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: RX Pause\n");
            if (p_mac != NULL) {
                if (p_mac->md_pause_get != NULL) {
                    rv = MAC_PAUSE_GET(p_mac,
            ...
            break;
        case DRV_PORT_PROP_LOCAL_ADVERTISE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Local Advertise\n");
            rv = soc_phyctrl_adv_local_get(unit, port, prop_val);
            break;
        case DRV_PORT_PROP_REMOTE_ADVERTISE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Remote Advertise\n");
            ...
            break;
        case DRV_PORT_PROP_PORT_ABILITY:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Port Ability\n");
            ...
            *prop_val |= phy_ability & SOC_PM_ABILITY_PHY;
            break;
        case DRV_PORT_PROP_MAC_ABILITY:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: MAC Ability\n");
            if (p_mac != NULL) {
                if (p_mac->md_ability_get != NULL) {
                    rv = MAC_ABILITY_GET(
                        p_mac, unit, port, &mac_ability);
            ...
            *prop_val = mac_ability;
            break;
        case DRV_PORT_PROP_PHY_ABILITY:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PHY Ability\n");
            rv = soc_phyctrl_ability_get(unit, port, prop_val);
            break;
        case DRV_PORT_PROP_INTERFACE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Interface\n");
            if (p_mac != NULL) {
            ...
            break;
        case DRV_PORT_PROP_ENABLE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Enable\n");
        ...
        case DRV_PORT_PROP_EGRESS_PCP_REMARK:
        case DRV_PORT_PROP_EGRESS_CFI_REMARK:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: Enable Get\n");
            rv = _drv_harrier_port_property_enable_get(
                unit, port, prop_type, prop_val);
            break;
        case DRV_PORT_PROP_IPG_FE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: IPG FE\n");
            if ((rv = REG_READ_SWMODEr(unit, &reg_value)) < 0) {
                return rv;
            }
            ...
            break;
        case DRV_PORT_PROP_IPG_GE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: IPG GE\n");
            if ((rv = REG_READ_SWMODEr(unit, &reg_value)) < 0) {
                return rv;
            }
            soc_SWMODEr_field_get(unit, &reg_value,
            ...
            break;
        case DRV_PORT_PROP_JAM:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: JAM\n");
            if (p_mac != NULL) {
                if (p_mac->md_pause_get != NULL) {
                    rv = MAC_PAUSE_GET(
            ...
            break;
        case DRV_PORT_PROP_BPDU_RX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: BPDU RX\n");
            if ((rv = REG_READ_GMNGCFGr(unit, &reg_value)) < 0) {
                return rv;
            }
        ...
        case DRV_PORT_PROP_PHY_MEDIUM:
            ...
            break;
        case DRV_PORT_PROP_PHY_MDIX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_PHY_MDIX\n");
            rv = soc_phyctrl_mdix_get(unit, port, prop_val);
            break;
        case DRV_PORT_PROP_PHY_MDIX_STATUS:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_PHY_MDIX_STATUS\n");
            rv = soc_phyctrl_mdix_status_get(unit, port, prop_val);
            break;
        case DRV_PORT_PROP_MS:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_MS\n");
            rv = soc_phyctrl_master_get(unit, port, (int *) prop_val);
            break;
        case DRV_PORT_PROP_SEC_MAC_MODE_NONE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_SEC_MODE_NONE\n");
        ...
        case DRV_PORT_PROP_PHY_CABLE_DIAG:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_PHY_CABLE_DIAG\n");
            rv = soc_phyctrl_cable_diag(unit, port, 
                                        (soc_port_cable_diag_t *)prop_val);
            break;
        case DRV_PORT_PROP_PHY_LINK_CHANGE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_PHY_LINKCHANGE\n");
            rv = soc_phyctrl_link_change(unit, port, (int *) prop_val);
            break;
        case DRV_PORT_PROP_DTAG_MODE:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_DTAG_MODE\n");
            rv = _drv_harrier_port_property_enable_get(unit, 0, prop_type, prop_val);
            break;
        case DRV_PORT_PROP_DTAG_ISP_PORT:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_DTAG_ISP_PORT\n");
            rv = _drv_harrier_port_property_enable_get(unit, port, prop_type, prop_val);
            break;
        case DRV_PORT_PROP_DTAG_TPID:
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_DTAG_TPID\n");
            if ((rv = REG_READ_ISP_VIDr(unit, &reg_value)) < 0) {
                return rv;
            }
            ...
            *prop_val = temp;
            break;
        case DRV_PORT_PROP_802_1X_MODE :
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_802_1X_MODE\n");
            rv = SOC_E_PARAM;
            break;
        case DRV_PORT_PROP_802_1X_BLK_RX :
            soc_cm_debug(DK_PORT, "drv_harrier_port_get: PROP_802_1X_BLK_RX\n");
            rv = SOC_E_PARAM;
            break;
        case DRV_PORT_PROP_MAC_BASE_VLAN:
...
drv_harrier_port_status_get(int unit, uint32 port, uint32 status_type, uint32 *val)
{
    ...
    if (status_type == DRV_PORT_STATUS_DETACH){
        *val = TRUE;
        ...
        soc_cm_debug(DK_PORT, "drv_harrier_port_status_get: DETACH %s\n",
            *val ? "OK" : "FAIL");
        return SOC_E_NONE;
    }
    ...
    switch (status_type)
    {
        ...
        case DRV_PORT_STATUS_LINK_SPEED:
            ...
            *val = (speed == 2500) ? DRV_PORT_STATUS_SPEED_2500M :
            ...
            soc_cm_debug(DK_PORT, "drv_hrrier_port_status_get: SPEED = %d\n",
                *val);
            break;
        case DRV_PORT_STATUS_LINK_DUPLEX:
            soc_cm_debug(DK_PORT, "drv_harrier_port_status_get: DUPLEX\n");
        ...
        case DRV_PORT_STATUS_PROBE:
            ...
            *val = okay;
            ...
            soc_cm_debug(DK_PORT, "drv_harrier_port_status_get: PROBE %s\n",
                *val ? "OK" : "FAIL");
            break;
        ...
        case DRV_PORT_STATUS_PHY_DRV_NAME:
            soc_cm_debug(DK_PORT, "drv_harrier_port_status_get: PHY_DRV_NAME\n");
            SOC_ROBO_PORT_INIT(unit);            
