/* $Id: chip_sim_task.c,v 1.10 Broadcom SDK $
    ...
    )
{
  ...
  return -1;
#else
  return taskNameToId(Chip_sim_task_name);
#endif
}
    ...
    )
{
  ...
  sal_msleep(600);
#ifdef __VXWORKS__
  sim_tid = chip_sim_task_get_tid();
  /*
   * No such function on Windows or linux
   */
  if (OK == taskIdVerify(sim_tid))
  {
    /*
     * If the simulator task is STILL running, kill it.
     * Check the need to release semaphores or enable interrupt
     */

    status = soc_sand_os_task_delete(sim_tid) ;
  }
#endif
