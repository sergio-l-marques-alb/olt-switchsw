...
#include "blackhawk7_v1l8p2/tier1/public/blackhawk7_v1l8p2_api_uc_vars_rdwr_defns_public.h"
    ...
    switch (tx_control) {
        ...
        case phymodTxElectricalIdleEnable:
            if (fw_lane_config.LaneConfigFromPCS == 0) {
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_electrical_idle_set(&phy_copy.access, 1));
            } else {
                return PHYMOD_E_PARAM;
            }
            break;
        case phymodTxElectricalIdleDisable:
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_electrical_idle_set(&phy_copy.access, 0));
            break;
        case phymodTxSquelchOn:
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable(&phy_copy.access, 1));
            break;
        case phymodTxSquelchOff:
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable(&phy_copy.access, 0));
            break;
        default:
            return PHYMOD_E_PARAM;
    }
...
int tscbh_gen2_phy_tx_lane_control_get(const phymod_phy_access_t* phy, phymod_phy_tx_lane_control_t *tx_control)
{
    ...
    PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable_get(&pm_phy_copy.access, &tx_disable));

    /* next check if PMD loopback is on */
    if (tx_disable) {
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_dig_lpbk_get(&pm_phy_copy.access, &lb_enable));
...
int tscbh_gen2_phy_rx_lane_control_set(const phymod_phy_access_t* phy, phymod_phy_rx_lane_control_t rx_control)
{
    ...
    switch (rx_control) {
        ...
        case phymodRxSquelchOn:
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect(&pm_phy_copy.access, 1, 0));
            break;
        case phymodRxSquelchOff:
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect(&pm_phy_copy.access, 0, 0));
            break;
        default:
            return PHYMOD_E_PARAM;
    }
...
int tscbh_gen2_phy_rx_lane_control_get(const phymod_phy_access_t* phy, phymod_phy_rx_lane_control_t* rx_control)
{
    ...
    PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect_get(&pm_phy_copy.access, &force_en, &force_val));
...
int _tscbh_gen2_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    blackhawk_ucode = blackhawk7_v1l8p2_ucode;
    blackhawk_ucode_len = blackhawk7_v1l8p2_ucode_len;

    switch(init_config->firmware_load_method){
    case phymodFirmwareLoadMethodInternal:
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_ucode_load(&core_copy.access, blackhawk_ucode, blackhawk_ucode_len));
        break;
    case phymodFirmwareLoadMethodExternal:
        PHYMOD_NULL_CHECK(init_config->firmware_loader);
        PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_ucode_init(&core_copy.access));
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_pram_firmware_enable(&core_copy.access, 1, 0));
        PHYMOD_IF_ERR_RETURN(init_config->firmware_loader(core, blackhawk_ucode_len, blackhawk_ucode));
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_pram_firmware_enable(&core_copy.access, 0, 0));
        break;
...
int tscbh_gen2_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps = NRZ_6TAP;
...
int tscbh_gen2_core_lane_map_set(const phymod_core_access_t* core, const phymod_lane_map_t* lane_map)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_map_lanes(&core_copy.access, TSCBH_NOF_LANES_IN_CORE, pmd_tx_addr, pmd_rx_addr));
...
int _tscbh_gen2_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (!is_warm_boot) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_lane_soft_reset_get(&phy_copy.access, &rst_status));
            if (!rst_status) PHYMOD_IF_ERR_RETURN (blackhawk7_v1l8p2_lane_soft_reset(&phy_copy.access, 1));
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_set_uc_lane_cfg(&phy_copy.access, serdes_firmware_config));
            if (!rst_status) PHYMOD_IF_ERR_RETURN (blackhawk7_v1l8p2_lane_soft_reset(&phy_copy.access, 0));
        }
    }
...
int tscbh_gen2_phy_polarity_get(const phymod_phy_access_t* phy, phymod_polarity_t* polarity)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_tx_rx_polarity_get(&phy_copy.access, &temp_pol.tx_polarity, &temp_pol.rx_polarity));
...
int tscbh_gen2_phy_tx_get(const phymod_phy_access_t* phy, phymod_tx_t* tx)
{
    ...
    if (tx->tap_mode == phymodTxTapMode3Tap) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP0, &val));
            tx->pre = val;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP1, &val));
            tx->main = val;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP2, &val));
            tx->post = val;
            tx->pre2 = 0;
            tx->post2 = 0;
            tx->post3 = 0;
        ...
        PHYMOD_IF_ERR_RETURN
            ...
            tx->main = val;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP3, &val));
            tx->post = val;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP4, &val));
            tx->post2 = val;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_read_tx_afe(&phy_copy.access, TX_AFE_TAP5, &val));
            tx->post3 = val;
    }
...
int tscbh_gen2_phy_tx_override_get(const phymod_phy_access_t* phy, phymod_tx_override_t* tx_override)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_tx_pi_control_get(&phy_copy.access, &temp_value));
                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
    ...
    *new_pll_state = *old_pll_state;
    ...
    /* Remove pmd_tx_disable_pin_dis it may be asserted because of ILKn */
        for (i = 0; i < num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_pmd_tx_disable_pin_dis_set(&pm_phy_copy.access, 0));
        }
        pm_phy_copy.access.lane_mask = lane_mask_backup;
      ...
      /* previous config is ethernet and request is ILKN mode */
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_pmd_tx_disable_pin_dis_set(&pm_phy_copy.access, 1));
        }
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pll_pwrdn_get(&pm_phy_copy.access, &ovco_is_pwrdn));

    /* if ovco is NOT pwoer down, then get the ovco div*/
    if (!ovco_is_pwrdn) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&pm_phy_copy.access, &ovco_pll_div));
    }
    ...
    if (PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_pll_pwrdn_get(&pm_phy_copy.access, &tvco_is_pwrdn));
    }
    ...
    if (!tvco_is_pwrdn) {
        ...
        pm_phy_copy.access.pll_idx = tvco_pll_index;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&pm_phy_copy.access, &tvco_pll_div));
    }
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_speed_config_get(speed_config->data_rate, ref_clk_is_156p25, &request_pll_div, &is_pam4, &osr_mode));
    }
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_lane_pll_selection_set(&pm_phy_copy.access, pll_index));
    }
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_osr_mode_set(&pm_phy_copy.access, osr_mode));
        }
    }
   ...
   for (i = 0; i < num_lane; i++) {
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_channel_loss_set(&pm_phy_copy.access, loss_in_db));
    }
                                        ...
                                        phymod_phy_speed_config_t* speed_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_get_vco_from_refclk_div(&phy_copy.access, refclk_in_hz, pll_div, &vco_rate, 0));
...
STATIC
int _tscbh_gen2_speed_id_to_speed_config_get(const phymod_phy_access_t* phy,
                                        int speed_id,
                                        int num_lane,
                                        phymod_phy_speed_config_t* speed_config)
{
...
int tscbh_gen2_phy_speed_config_get(const phymod_phy_access_t* phy, phymod_phy_speed_config_t* speed_config)
{
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        if (ref_clk == TBHMOD_REF_CLK_156P25MHZ) {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_get_vco_from_refclk_div(&phy_copy.access, REF_CLOCK_156P25, pll_div, &vco_freq_khz, 0));
        } else {
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_get_vco_from_refclk_div(&phy_copy.access, REF_CLOCK_312P5, pll_div, &vco_freq_khz, 0));
        }  
...
int tscbh_gen2_phy_cl72_set(const phymod_phy_access_t* phy, uint32_t cl72_en)
{
    ...
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_get_uc_lane_cfg(&pm_phy_copy.access, &serdes_firmware_config));
    ...
    case phymodLoopbackGlobalPMD :
        if (enable) {
            ...
            for (i = 0; i < num_lane; i++) {
                ...
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pmd_force_signal_detect(&phy_copy.access,  (int) enable, (int) 0));
            }
            phy_copy.access.lane_mask = 1 << start_lane;
            PHYMOD_IF_ERR_RETURN
                (tbhmod_enable_set(&phy_copy.access));
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            phy_copy.access.lane_mask = 0x1 << (i + start_lane);
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_tx_disable(&phy_copy.access, enable));
...
int _tscbh_gen2_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_core_dp_reset(&core_copy.access, 1));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_uc_active_get(&phy_access.access, &uc_enable));
    ...
    if (init_config->interface.ref_clock != phymodRefClk156Mhz) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_refclk_set(&core_copy.access, init_config->interface.ref_clock));
        PHYMOD_IF_ERR_RETURN
    ...
    ucode.ucode_size = blackhawk7_v1l8p2_ucode_len;
    /* blackhawk_ucode_len = blackhawk7_v1l8p2_ucode_len; */
#ifndef TSCBH_PMD_CRC_UCODE_VERIFY
    blackhawk_ucode = blackhawk7_v1l8p2_ucode;
#endif
    ...
    PHYMOD_IF_ERR_RETURN
    (blackhawk7_v1l8p2_uc_reset_with_info(&core_copy.access , 1, ucode));
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
         ...
         */
        rv = blackhawk7_v1l8p2_ucode_load_verify(&core_copy.access, (uint8_t *) blackhawk_ucode, blackhawk_ucode_len);
        if (rv != PHYMOD_E_NONE) {
    ...
    if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
        ...
        /*PHYMOD_IF_ERR_RETURN(
            blackhawk7_v1l8p2_start_ucode_crc_calc(&core_copy.access, blackhawk_ucode_len)); */
#endif
    }
...
int _tscbh_gen2_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (init_config->interface.ref_clock == phymodRefClk156Mhz) {
        refclk = BLACKHAWK7_V1L8P2_PLL_REFCLK_156P25MHZ;
        ...
        refclk = BLACKHAWK7_V1L8P2_PLL_REFCLK_312P5MHZ;
        /* first check tvco index */
        if (tvco_pll_index == 1) {
            PHYMOD_IF_ERR_RETURN(
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        /* 6. Release uc reset */
        PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_uc_reset_with_info(&core_copy.access , 0, ucode));

        /* 7. Wait for uc_active = 1 */
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_wait_uc_active(&phy_access.access));

        for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_pmd_ln_h_rstb_pkill_override(&phy_access_copy.access, 0x1));
        }
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_init_blackhawk7_v1l8p2_info(&core_copy.access));
        ...
        if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
            ...
            cmd = CMD_CALC_CRC;
            PHYMOD_IF_ERR_RETURN(
                blackhawk7_v1l8p2_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(&core_copy.access, 100, cmd));
#else
            PHYMOD_IF_ERR_RETURN(
                blackhawk7_v1l8p2_check_ucode_crc(&core_copy.access, blackhawk7_v1l8p2_ucoDe_crc, 200));
#endif
        }
        ...
        for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
            phy_access_copy.access.lane_mask = 1 << lane;
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_pmd_ln_h_rstb_pkill_override(&phy_access_copy.access, 0x0));
        }
    } else {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_firmware_load_none_init_blackhawk7_v1l8p2_info(&core_copy.access));
    }
    ...
    if (init_config->pll0_div_init_value != phymod_TSCBH_PLL_DIVNONE) {
        core_copy.access.pll_idx = 0;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_configure_pll_refclk_div(&core_copy.access,
                                                      refclk,
    ...
    default to use PLL1 */
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_micro_clk_source_select(&core_copy.access, 1));
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_core_dp_reset(&core_copy.access, 0));
    core_copy.access.pll_idx = 1;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_core_dp_reset(&core_copy.access, 0));
...
int tscbh_gen2_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
         ...
         */
        PHYMOD_IF_ERR_RETURN
             (blackhawk7_v1l8p2_lane_cfg_fwapi_data1_set(&pm_phy_copy.access, 3));
    }
    ...
    pm_phy_copy.access.lane_mask = 1 << 0;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pll_pwrdn_get(&pm_phy_copy.access, &pll_power_down));
    ...
    if (!pll_power_down) {
        ...
        cnt = 0;
        while (cnt < 500) {
            PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_pll_lock_get(&pm_phy_copy.access, &pll_lock));
            cnt = cnt + 1;
            if (pll_lock) {
                break;
            } else {
...
int _tscbh_gen2_phy_autoneg_ability_to_vco_get(const phymod_autoneg_advert_abilities_t* an_advert_abilities,
                                         ...
                                         phymod_phy_pll_state_t* new_pll_adv_state)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_tscbh_gen2_phy_autoneg_ability_to_vco_get(an_advert_abilities, &request_vco));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pll_pwrdn_get(&phy_copy.access, &ovco_is_pwrdn));
    if (!ovco_is_pwrdn) {
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&phy_copy.access, &ovco_pll_div));
    }

    /* Get TVCO info */
    phy_copy.access.pll_idx = phy->access.tvco_pll_index;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&phy_copy.access, &tvco_pll_div));
    ...
    for (i = requested_pll_num; i >= 0; i--) {
        if (request_pll_div[i] == ovco_pll_div) {
            ...
            if (ovco_is_pwrdn) {
                /* Power up PLL0 */
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_core_pwrdn(&phy_copy.access, PWR_ON));
            }
            /*toggle core dp reset */
            phy_copy.access.lane_mask = 0x1;
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_core_dp_reset(&phy_copy.access, 1));

            /*config the PLL to the requested VCO */
            if (ref_clk == TBHMOD_REF_CLK_312P5MHZ) {
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_configure_pll_refclk_div(&phy_copy.access,
                                                            BLACKHAWK7_V1L8P2_PLL_REFCLK_312P5MHZ,
                                                            request_pll_div[i]));
            } else {
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_configure_pll_refclk_div(&phy_copy.access,
                                                            BLACKHAWK7_V1L8P2_PLL_REFCLK_156P25MHZ,
                                                            request_pll_div[i]));
            }
            /* release core soft reset */
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_core_dp_reset(&phy_copy.access, 0));
...
int tscbh_gen2_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    if (an->enable) {
        ...
        for (i = 0; i < num_lane; i++) {
            phy_copy.access.lane_mask = 1 << (start_lane + i);
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_v1l8p2_lane_pll_selection_set(&phy_copy.access, phy->access.tvco_pll_index));
        }
    }
    ...
    if (an->enable) {
        ...
        phy_copy.access.pll_idx = phy->access.tvco_pll_index;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_INTERNAL_read_pll_div(&phy_copy.access, &pll_1_div));

        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_get_vco_from_refclk_div(&phy_copy.access, refclk_in_hz, pll_1_div, &vco_rate, 0));
                            ...
                            uint32_t pll_div)
{
    ...
    pm_phy_copy.access.lane_mask = 1 << 0;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pll_pwrdn_get(&pm_phy_copy.access, &pll_is_pwrdn));

    /* if PLL is power down, need to power up first */
    if (pll_is_pwrdn) {
        /* Power up ovco if it's power down */
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_core_pwrdn(&pm_phy_copy.access, PWR_ON));
    }

    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_core_dp_reset(&pm_phy_copy.access, 1));

    /*next re-config pll divider */
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_configure_pll_refclk_div(&pm_phy_copy.access,
                                                  refclk,
                                                  pll_div));

    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_core_dp_reset(&pm_phy_copy.access, 0));
...
int tscbh_gen2_timesync_enable_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t enable)
{
    ...
    if (PHYMOD_TIMESYNC_ENABLE_F_CORE_GET(flags)) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_clk4sync_enable_set(&phy_copy.access, enable, fclk_div_mode));
    }
...
int tscbh_gen2_timesync_enable_get(const phymod_phy_access_t* phy, uint32_t flags, uint32_t* enable)
{
    ...
    if (PHYMOD_TIMESYNC_ENABLE_F_CORE_GET(flags)) {
        phy_copy.access.pll_idx = phy_copy.access.tvco_pll_index;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_v1l8p2_clk4sync_enable_get(&phy_copy.access, enable, &fclk_div_mode));
    }
...
/*Set timesync adjust*/
int tscbh_gen2_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t ts_am_norm_mode)
{
    ...
    /* 1.3 Get FEC type */
    PHYMOD_IF_ERR_RETURN
        (_tscbh_gen2_speed_id_to_speed_config_get(phy, speed_id, num_lane, &speed_config));
...
int tscbh_gen2_phy_rx_ppm_get(const phymod_phy_access_t* phy, int16_t* rx_ppm)
{
    ...
    pm_phy_copy.access.lane_mask = 1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_rx_ppm(&pm_phy_copy.access, rx_ppm));
    return PHYMOD_E_NONE;
}
...
int tscbh_gen2_phy_pll_powerdown_get(const phymod_phy_access_t* phy, uint32_t pll_index, uint32_t* powerdown)
{
    ...
    phy_copy.access.lane_mask = 1 << 0;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_pll_pwrdn_get(&phy_copy.access, powerdown));
