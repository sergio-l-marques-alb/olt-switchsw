 ...
 * @return Serdes Info pointer
 */
blackhawk_tsc_info_t *blackhawk_tsc_INTERNAL_get_blackhawk_tsc_info_ptr(srds_access_t *sa__);

/** Get Serdes Info pointer and initialize it if signature doesn't match
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @return Serdes Info pointer
 */
blackhawk_tsc_info_t *blackhawk_tsc_INTERNAL_get_blackhawk_tsc_info_ptr_with_check(srds_access_t *sa__);
 ...
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors)
 */
err_code_t blackhawk_tsc_INTERNAL_verify_blackhawk_tsc_info(blackhawk_tsc_info_t *blackhawk_tsc_info_ptr, srds_access_t *sa__);
 ...
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
err_code_t blackhawk_tsc_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum blackhawk_tsc_pll_div_enum div, uint32_t *vco_freq_khz, enum blackhawk_tsc_pll_option_enum pll_option);
