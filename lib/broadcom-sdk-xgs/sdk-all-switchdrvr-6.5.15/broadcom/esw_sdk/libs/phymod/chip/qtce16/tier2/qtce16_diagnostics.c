...
int qtce16_phy_pmd_info_dump(const phymod_phy_access_t* phy, void* arg)
{
    int start_lane, num_lane, lane_id, sub_port;
    phymod_phy_access_t phy_copy;
    int i = 0;
    ...
    if (!cmd_str) {
        type = (uint32_t)QMOD16_DIAG_DSC;
    } else if (!PHYMOD_STRCMP(cmd_str, "ber")) {
        type = (uint32_t)QMOD16_DIAG_BER;
    } else if (!PHYMOD_STRCMP(cmd_str, "config")) {
        type = (uint32_t)QMOD16_DIAG_CFG;
    } else if (!PHYMOD_STRCMP(cmd_str, "CL72")||!PHYMOD_STRCMP(cmd_str, "cl72")) {
        type = (uint32_t)QMOD16_DIAG_CL72;
    } else if (!PHYMOD_STRCMP(cmd_str, "debug")) {
        type = (uint32_t)QMOD16_DIAG_DEBUG;
    } else if (!PHYMOD_STRCMP(cmd_str, "state")) {
        type = (uint32_t)QMOD16_DIAG_STATE;
    } else {
        type = (uint32_t)QMOD16_DIAG_DSC;
    }
...
int qtce16_phy_pcs_info_dump(const phymod_phy_access_t* phy, void* arg)
{
    int lane_id, sub_port;
    phymod_phy_access_t phy_copy;
    ...
    PHYMOD_IF_ERR_RETURN (qmod16_diag_disp(&phy_copy.access, (char*)arg));
