...
static blackhawk_tsc_info_t blackhawk_tsc_info[NUM_SERDES_INFO_TABLES];
...
err_code_t blackhawk_tsc_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data, enum blackhawk_tsc_prbs_chk_timer_selection_enum timer_sel) {
    ...
    if((time_ms <= 448) && (timer_sel == USE_HW_TIMERS)) {
        ...
        if(ber_data->lcklost == 0) {
            ...
            struct prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak = {0};
 ...
 */
static err_code_t _blackhawk_tsc_get_divisor_ratio(srds_access_t *sa__, enum blackhawk_tsc_pll_div_enum div, uint16_t *numerator, uint16_t *denominator)
{
    switch (div) {
...
static err_code_t _blackhawk_tsc_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum blackhawk_tsc_pll_div_enum div, uint32_t vco_freq_khz, enum blackhawk_tsc_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk_tsc_get_divisor_ratio(sa__, div, &numerator, &denominator));
...
static err_code_t _blackhawk_tsc_check_div(srds_access_t *sa__, enum blackhawk_tsc_pll_div_enum div) {
    uint8_t i, found = 0;
...
static err_code_t _blackhawk_tsc_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum blackhawk_tsc_pll_div_enum *div, enum blackhawk_tsc_pll_option_enum pll_option) {
...
err_code_t blackhawk_tsc_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    if (pll_option == BLACKHAWK_TSC_PLL_OPTION_DISABLE_VERIFY) {
    ...
    if (*div != BLACKHAWK_TSC_PLL_DIV_UNKNOWN) {
        EFUN(_blackhawk_tsc_check_div(sa__, *div));
    }
    ...
    if (refclk == BLACKHAWK_TSC_PLL_REFCLK_UNKNOWN) {
        ...
        EFUN(_blackhawk_tsc_get_refclk_from_div_vco(sa__, refclk_freq_hz, *div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_blackhawk_tsc_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
