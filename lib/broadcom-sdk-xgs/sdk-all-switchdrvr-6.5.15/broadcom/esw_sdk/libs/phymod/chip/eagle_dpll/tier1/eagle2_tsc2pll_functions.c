/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  eagle2_tsc2pll_functions.c                                        *
 *  Created On    :  29/04/2013                                                    *
 *  Created By    :  Kiran Divakar                                                 *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: eagle2_tsc2pll_functions.c 1570 2017-08-30 22:21:06Z kirand $ *
 *                                                                                 *
...
/** @file eagle2_tsc2pll_functions.c
...
#include "eagle2_tsc2pll_functions.h"
#include "eagle2_tsc2pll_field_access_c.h"
#include "eagle2_tsc2pll_internal_c.h"
#include "eagle2_tsc2pll_dv_functions_c.h"



#include "eagle2_tsc2pll_pll_config_c.h"
...
err_code_t eagle2_tsc2pll_display_eye_scan (const phymod_access_t *pa) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;

    EFUN(eagle2_tsc2pll_display_eye_scan_header (pa, 1));
    /* start horizontal acquisition */
    {   err_code_t err_code = eagle2_tsc2pll_meas_eye_scan_start (pa, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((eagle2_tsc2pll_meas_eye_scan_done (pa ), err_code));
        }
    }
    ...
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = eagle2_tsc2pll_read_eye_scan_stripe (pa, &stripe[0], &status);
            if (err_code) {
                EFUN((eagle2_tsc2pll_meas_eye_scan_done (pa ), err_code));
            }
        }
        EFUN(eagle2_tsc2pll_display_eye_scan_stripe (pa, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(eagle2_tsc2pll_meas_eye_scan_done (pa ));
    EFUN(eagle2_tsc2pll_display_eye_scan_footer (pa, 1));

    return(ERR_CODE_NONE);
}

/* This function is for Falcon and is configured for passive mode */
err_code_t eagle2_tsc2pll_meas_lowber_eye (const phymod_access_t *pa, const struct eagle2_tsc2pll_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(eagle2_tsc2pll_pmd_uc_cmd_with_data (pa, CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            EFUN(eagle2_tsc2pll_delay_us(1000));
            ESTM(status=rdv_usr_diag_status());
            EFUN_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0);
#else
        EFUN(eagle2_tsc2pll_poll_diag_done (pa, &status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            eagle2_tsc2pll_osr_mode_st osr_mode;
            ENULL_MEMSET(&osr_mode, 0, sizeof(eagle2_tsc2pll_osr_mode_st));
            EFUN(_eagle2_tsc2pll_get_osr_mode (pa, &osr_mode));
            if(osr_mode.tx_rx == EAGLE2_TSC2PLL_OSX1) {
                EFUN(eagle2_tsc2pll_prbs_err_count_ll (pa, &errors));
            } else if(osr_mode.tx_rx == EAGLE2_TSC2PLL_OSX2) {
...
err_code_t eagle2_tsc2pll_display_lowber_eye (const phymod_access_t *pa, const struct eagle2_tsc2pll_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,i,z;
    ...
    j = 0;
    for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
      ESTM_PRINTF(("%6dmV : ", _ladder_setting_to_mV (pa, y, rd_p1_thresh_sel())));
...
err_code_t eagle2_tsc2pll_meas_eye_scan_start (const phymod_access_t *pa, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(eagle2_tsc2pll_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(eagle2_tsc2pll_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_read_eye_scan_stripe (const phymod_access_t *pa, uint32_t *buffer,uint16_t *status){
    uint32_t val[2] = {0,0};
    uint16_t sts = 0;
    int8_t   i;
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = eagle2_tsc2pll_poll_diag_eye_data (pa, &val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t eagle2_tsc2pll_display_eye_scan_stripe (const phymod_access_t *pa, int8_t y,uint32_t *buffer) {
...
err_code_t eagle2_tsc2pll_meas_eye_density_data (const phymod_access_t *pa, const struct eagle2_tsc2pll_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  int8_t y,x,z;
  int16_t i;
  int8_t hzcnt;
  /*uint32_t errcnt; */
  ...
  EFUN(eagle2_tsc2pll_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_DENSITY,2000));
  i = 0;hzcnt=0;
  ESTM_PRINTF(("Calculating %d\n", rd_cnt_d_minus_m1()));
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    EFUN(_set_p1_threshold (pa, y));
    EFUN(_move_clkp1_offset (pa, eyescan_options.horz_min-1));   /* Walk back 32 steps */
    EFUN(_move_clkp1_offset (pa, 1));                              /* Walk forward one (net -31). This sets up the registers for 1 write increments */
    hzcnt = eyescan_options.horz_min;
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         EFUN(_trnsum_clear_and_enable (pa ));
         EFUN(eagle2_tsc2pll_poll_dsc_state_equals_uc_tune (pa, 2000));
    ...
    EFUN(_move_clkp1_offset (pa, -hzcnt));     /* Walk back center */
    EFUN_PRINTF(("\n"));
  }
  EFUN_PRINTF(("\n"));
  *buffer_size = i;
...
err_code_t eagle2_tsc2pll_display_eye_density_data (const phymod_access_t *pa, const struct eagle2_tsc2pll_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t buffer_size) {
    int8_t x,y,i,z;
    int16_t j; /* buffer pointer */
    int32_t val;
    int32_t maxval = 0;
    ...
    for (y = eyescan_options.vert_max-1;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
        EFUN_PRINTF(("%6dmV : ",(_ladder_setting_to_mV (pa, y, range) + _ladder_setting_to_mV (pa, y+1, range))/2));
        for (z = -31; z < eyescan_options.horz_min; ++z) {
            EFUN_PRINTF((" "));    /* add leading spaces if any */
        }
        for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
...
err_code_t eagle2_tsc2pll_display_eye_scan_header (const phymod_access_t *pa, int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t eagle2_tsc2pll_display_eye_scan_footer (const phymod_access_t *pa, int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
/*eye_scan_status_t eagle2_tsc2pll_read_eye_scan_status (pa, void) */
err_code_t eagle2_tsc2pll_read_eye_scan_status (const phymod_access_t *pa, uint16_t *status) {
...
err_code_t eagle2_tsc2pll_meas_eye_scan_done (const phymod_access_t *pa) {
  EFUN(eagle2_tsc2pll_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t eagle2_tsc2pll_start_ber_scan_test (const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_read_ber_scan_data (const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t eagle2_tsc2pll_eye_margin_proj (const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(eagle2_tsc2pll_start_ber_scan_test (pa, ber_scan_mode, timer_control, max_error_control));
    ...
    if(verbose > 5) {
        do {
            EFUN(eagle2_tsc2pll_delay_us(2000000));
            ESTM(sts = rdv_usr_diag_status());
            EFUN_PRINTF(("sts=%04x\n",sts));
        ...
        EFUN(eagle2_tsc2pll_poll_diag_done (pa, &sts,timer_control*2000));
    }
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(eagle2_tsc2pll_read_ber_scan_data (pa,  &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END,0x00,50));
 ...
 /* }                                                                  */
    ...
    EFUN(eagle2_tsc2pll_display_ber_scan_data (pa, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t eagle2_tsc2pll_display_ber_scan_data (const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
err_code_t eagle2_tsc2pll_display_lane_state_hdr (const phymod_access_t *pa) {
  EFUN_PRINTF(("LN "));
  EFUN_PRINTF(("(CDRxN      , UC_CFG,RST,STP,PLL)  "));
...
err_code_t eagle2_tsc2pll_display_lane_state_legend (const phymod_access_t *pa) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t eagle2_tsc2pll_display_lane_state (const phymod_access_t *pa) {
  err_code_t err_code = _eagle2_tsc2pll_display_lane_state_no_newline (pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t eagle2_tsc2pll_get_eye_margin_est (const phymod_access_t *pa, int *left_eye_mUI, int *right_eye_mUI, int *upper_eye_mV, int *lower_eye_mV) {
  uint8_t ladder_range = 0;

  ESTM(ladder_range = rd_p1_thresh_sel());

  ESTM(*left_eye_mUI = _eye_to_mUI(pa, rdv_usr_sts_heye_left()));
  ESTM(*right_eye_mUI = _eye_to_mUI(pa, rdv_usr_sts_heye_right()));
  ESTM(*upper_eye_mV = _eye_to_mV(pa, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = _eye_to_mV(pa, rdv_usr_sts_veye_lower(), ladder_range));
  return (ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_display_core_state_hdr (const phymod_access_t *pa) {
           
  EFUN_PRINTF(("SerDes type = eagle2_tsc2pll\n"));
...
err_code_t eagle2_tsc2pll_display_core_state_line (const phymod_access_t *pa) {
  err_code_t err_code = _eagle2_tsc2pll_display_core_state_no_newline (pa );
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t eagle2_tsc2pll_display_core_state_legend (const phymod_access_t *pa) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
  EFUN_PRINTF(("****                          Legend of Entries in display_core_state()                                   ****\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
#ifdef SERDES_DUAL_PLLS
  EFUN_PRINTF(("*  RST              : Core Reset State PLL0{active,occured,held}, Core Reset State PLL1{active,occured,held} *\n"));
#else
...
err_code_t eagle2_tsc2pll_display_core_config (const phymod_access_t *phy_pa) {
    phymod_access_t *pa;
    ...
    EFUN_PRINTF((    "***********************************\n\n"));
    pa = &pa_copy;
    ENULL_MEMCPY(pa, phy_pa, sizeof(phymod_access_t));
    {
        struct   eagle2_tsc2pll_uc_core_config_st core_cfg;
        struct   eagle2_tsc2pll_uc_core_config_st pll1_cfg;

        ENULL_MEMSET(&pll1_cfg, 0, sizeof(pll1_cfg));
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
        {
            uint8_t pll_orig;
            ESTM(pll_orig = eagle2_tsc2pll_get_pll(pa));
            EFUN(eagle2_tsc2pll_set_pll(pa, 0));
            EFUN(eagle2_tsc2pll_get_uc_core_config(pa, &core_cfg));
            EFUN(eagle2_tsc2pll_set_pll(pa, 1));
            EFUN(eagle2_tsc2pll_get_uc_core_config(pa, &pll1_cfg));
            EFUN(eagle2_tsc2pll_set_pll(pa, pll_orig));
        }
        {
    EFUN_PRINTF((    "uC Config VCO Rate   = %d (~%d.%03dGHz), %d (~%d.%03dGHz)\n", core_cfg.field.vco_rate
                                                                                  ...
                                                                                  , ((uint16_t)pll1_cfg.vco_rate_in_Mhz / 1000)
                                                                                  , ((uint16_t)pll1_cfg.vco_rate_in_Mhz % 1000)               ));
    EFUN_PRINTF((    "Core Config from PCS = %d, %d\n\n"                          , core_cfg.field.core_cfg_from_pcs
                                                                                  , pll1_cfg.field.core_cfg_from_pcs));
        }
    }
    ESTM_PRINTF((    "Lane Addr 0          = %d\n"                            , rdc_lane_addr_0()   ));
    ESTM_PRINTF((    "Lane Addr 1          = %d\n"                            , rdc_lane_addr_1()   ));
    ESTM_PRINTF((    "Lane Addr 2          = %d\n"                            , rdc_lane_addr_2()   ));
    ESTM_PRINTF((    "Lane Addr 3          = %d\n"                            , rdc_lane_addr_3()   ));
    ESTM_PRINTF((    "TX Lane Map 0        = %d\n"                            , rdc_tx_lane_map_0() ));
    ESTM_PRINTF((    "TX Lane Map 1        = %d\n"                            , rdc_tx_lane_map_1() ));
    ESTM_PRINTF((    "TX Lane Map 2        = %d\n"                            , rdc_tx_lane_map_2() ));
    ESTM_PRINTF((    "TX Lane Map 3        = %d\n\n"                          , rdc_tx_lane_map_3() ));
    return(ERR_CODE_NONE);
}


err_code_t eagle2_tsc2pll_display_lane_config (const phymod_access_t *pa) {
    struct eagle2_tsc2pll_uc_lane_config_st lane_cfg;

    ENULL_MEMSET(&lane_cfg, 0, sizeof(lane_cfg));

    EFUN_PRINTF(("\n\n*************************************\n"                                          ));
    ESTM_PRINTF((   "**** SERDES LANE %d CONFIGURATION ****\n"  , eagle2_tsc2pll_get_lane(pa)                     ));
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         eagle2_tsc2pll_get_uc_lane_cfg (pa, &lane_cfg)      );
    ...
    ESTM_PRINTF((    "TXFIR Override Enable       = %d\n"       , rd_txfir_override_en()                ));
...
err_code_t eagle2_tsc2pll_display_core_state (const phymod_access_t *pa) {
    EFUN(eagle2_tsc2pll_display_core_state_hdr (pa ));
    EFUN(eagle2_tsc2pll_display_core_state_line (pa ));
    EFUN(eagle2_tsc2pll_display_core_state_legend (pa ));
    return ERR_CODE_NONE;
}

err_code_t eagle2_tsc2pll_display_lane_debug_status (const phymod_access_t *pa) {
    /* startup */
    struct eagle2_tsc2pll_usr_ctrl_disable_functions_st     ds;
    struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct eagle2_tsc2pll_usr_ctrl_disable_functions_st     dss;
    struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(eagle2_tsc2pll_get_usr_ctrl_disable_startup (pa, &ds));
    ...
    EFUN(                                                            eagle2_tsc2pll_get_usr_ctrl_disable_startup_dfe (pa, &dsd) );
    ...
    EFUN(                                                           eagle2_tsc2pll_get_usr_ctrl_disable_steady_state (pa, &dss) );
    ...
    EFUN(                                                      eagle2_tsc2pll_get_usr_ctrl_disable_steady_state_dfe (pa, &dssd) );
...
err_code_t eagle2_tsc2pll_stop_uc_lane (const phymod_access_t *pa, uint8_t enable) {

  if (enable) {
    return(eagle2_tsc2pll_pmd_uc_control (pa, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(eagle2_tsc2pll_pmd_uc_control (pa, CMD_UC_CTRL_RESUME,50));
  }
}


err_code_t eagle2_tsc2pll_stop_uc_lane_status (const phymod_access_t *pa, uint8_t *uc_lane_stopped) {
...
err_code_t eagle2_tsc2pll_stop_rx_adaptation (const phymod_access_t *pa, uint8_t enable) {
...
err_code_t eagle2_tsc2pll_request_stop_rx_adaptation (const phymod_access_t *pa) {

  return(eagle2_tsc2pll_pmd_uc_control_return_immediate (pa, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t eagle2_tsc2pll_ucode_crc_verify (const phymod_access_t *pa, uint16_t ucode_len,uint16_t expected_crc_value) {
    uint16_t calc_crc;

    EFUN(eagle2_tsc2pll_pmd_uc_cmd_with_data (pa, CMD_CALC_CRC,0,ucode_len,200));
...
err_code_t eagle2_tsc2pll_start_ucode_crc_calc (const phymod_access_t *pa, uint16_t ucode_len) {

    EFUN(eagle2_tsc2pll_pmd_uc_cmd_with_data_return_immediate (pa, CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_check_ucode_crc (const phymod_access_t *pa, uint16_t expected_crc_value, uint32_t timeout_ms) {
    uint16_t calc_crc;
    err_code_t err_code;

    err_code = eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 (pa, timeout_ms);     /* Poll for uc_dsc_ready_for_cmd = 1 to indicate eagle2_tsc2pll ready for command */
    if (err_code) {
...
err_code_t eagle2_tsc2pll_pmd_uc_cmd_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  err_code_t err_code;
  uint16_t   cmddata;

  err_code = eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 (pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate eagle2_tsc2pll ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */

  return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_pmd_uc_cmd (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {

  EFUN(eagle2_tsc2pll_pmd_uc_cmd_return_immediate (pa, cmd, supp_info));    /* Invoke eagle2_tsc2pll_uc_cmd and return control immediately */

  EFUN(eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 (pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate eagle2_tsc2pll ready for command */
  return(ERR_CODE_NONE);
}


err_code_t eagle2_tsc2pll_pmd_uc_cmd_with_data_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data) {
  uint16_t   cmddata;
  err_code_t err_code;
...
err_code_t eagle2_tsc2pll_pmd_uc_cmd_with_data (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {

    EFUN(eagle2_tsc2pll_pmd_uc_cmd_with_data_return_immediate (pa, cmd, supp_info, data)); /* Invoke eagle2_tsc2pll_uc_cmd and return control immediately */

    EFUN(eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 (pa, timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate eagle2_tsc2pll ready for command */
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_pmd_uc_control_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(eagle2_tsc2pll_pmd_uc_cmd_return_immediate (pa, CMD_UC_CTRL, (uint8_t) control));
}

err_code_t eagle2_tsc2pll_pmd_uc_control (const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}

err_code_t eagle2_tsc2pll_pmd_uc_diag_cmd (const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t eagle2_tsc2pll_rdbl_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = eagle2_tsc2pll_rdb_uc_ram (pa, err_code_p, (LANE_VAR_RAM_BASE+addr+(eagle2_tsc2pll_get_lane(pa)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro RAM Lane Byte Signed Read */
int8_t eagle2_tsc2pll_rdbls_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) eagle2_tsc2pll_rdbl_uc_var (pa, err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t eagle2_tsc2pll_rdwl_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = eagle2_tsc2pll_rdw_uc_ram (pa, err_code_p, (LANE_VAR_RAM_BASE+addr+(eagle2_tsc2pll_get_lane(pa)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */
...
int16_t eagle2_tsc2pll_rdwls_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) eagle2_tsc2pll_rdwl_uc_var (pa, err_code_p, addr));
}

/* Micro RAM Lane Byte Write */
err_code_t eagle2_tsc2pll_wrbl_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {


    if(addr < LANE_VAR_RAM_SIZE) {
        return (eagle2_tsc2pll_wrb_uc_ram (pa, (LANE_VAR_RAM_BASE+addr+(eagle2_tsc2pll_get_lane(pa)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
...
err_code_t eagle2_tsc2pll_wrbls_uc_var (const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
  return (eagle2_tsc2pll_wrbl_uc_var (pa, addr, wr_val));
}

/* Micro RAM Lane Word Write */
err_code_t eagle2_tsc2pll_wrwl_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (eagle2_tsc2pll_wrw_uc_ram (pa, (LANE_VAR_RAM_BASE+addr+(eagle2_tsc2pll_get_lane(pa)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t eagle2_tsc2pll_wrwls_uc_var (const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
  return (eagle2_tsc2pll_wrwl_uc_var (pa, addr,wr_val));
}
...
uint8_t eagle2_tsc2pll_rdbc_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = eagle2_tsc2pll_rdb_uc_ram (pa, err_code_p, (CORE_VAR_RAM_BASE+addr)));                      /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Byte Signed Read */
int8_t eagle2_tsc2pll_rdbcs_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) eagle2_tsc2pll_rdbc_uc_var (pa, err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t eagle2_tsc2pll_rdwc_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = eagle2_tsc2pll_rdw_uc_ram (pa, err_code_p, (CORE_VAR_RAM_BASE+addr)));                  /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Word Signed Read */
int16_t eagle2_tsc2pll_rdwcs_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) eagle2_tsc2pll_rdwc_uc_var (pa, err_code_p, addr));
}

/* Micro RAM Core Byte Write  */
err_code_t eagle2_tsc2pll_wrbc_uc_var (const phymod_access_t *pa, uint8_t addr, uint8_t wr_val) {


    if(addr < CORE_VAR_RAM_SIZE) {
    return (eagle2_tsc2pll_wrb_uc_ram (pa, (CORE_VAR_RAM_BASE+addr), wr_val));                                /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t eagle2_tsc2pll_wrbcs_uc_var (const phymod_access_t *pa, uint8_t addr, int8_t wr_val) {
  return (eagle2_tsc2pll_wrbc_uc_var (pa, addr, wr_val));
}

/* Micro RAM Core Word Write  */
err_code_t eagle2_tsc2pll_wrwc_uc_var (const phymod_access_t *pa, uint8_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    if(addr < CORE_VAR_RAM_SIZE) {
        return (eagle2_tsc2pll_wrw_uc_ram (pa, (CORE_VAR_RAM_BASE+addr), wr_val));                                 /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t eagle2_tsc2pll_wrwcs_uc_var (const phymod_access_t *pa, uint8_t addr, int16_t wr_val) {
  return(eagle2_tsc2pll_wrwc_uc_var (pa, addr,wr_val));
}
...
err_code_t eagle2_tsc2pll_set_uc_core_config (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_core_config_st struct_val) {
    {   
#ifndef ATE_LOG
        uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: eagle2_tsc2pll_set_uc_core_config (pa, ..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n",eagle2_tsc2pll_get_lane(pa),reset_state));
    ...
    switch (eagle2_tsc2pll_get_pll(pa)) {
    case 0:
...
err_code_t eagle2_tsc2pll_set_usr_ctrl_core_event_log_level (const phymod_access_t *pa, uint8_t core_event_log_level) {
  return(wrcv_usr_ctrl_core_event_log_level(core_event_log_level));
}

err_code_t eagle2_tsc2pll_set_uc_lane_cfg (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_lane_config_st struct_val) {
#ifndef ATE_LOG
    uint8_t reset_state;

    ESTM(reset_state = rd_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: eagle2_tsc2pll_set_uc_lane_cfg (pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",eagle2_tsc2pll_get_lane(pa),reset_state));
...
err_code_t eagle2_tsc2pll_set_usr_ctrl_lane_event_log_level (const phymod_access_t *pa, uint8_t lane_event_log_level) {
  return(wrv_usr_ctrl_lane_event_log_level(lane_event_log_level));
}

err_code_t eagle2_tsc2pll_set_usr_ctrl_disable_startup (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte (pa, &set_val));
  return(wrv_usr_ctrl_disable_startup_functions_byte(set_val.byte));
}

err_code_t eagle2_tsc2pll_set_usr_ctrl_disable_startup_dfe (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st set_val) {
...
err_code_t eagle2_tsc2pll_set_usr_ctrl_disable_steady_state (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st set_val) {
  EFUN(_check_uc_lane_stopped (pa ));                     /* make sure uC is stopped to a race conditions */
  EFUN(_update_usr_ctrl_disable_functions_byte (pa, &set_val));
  return(wrv_usr_ctrl_disable_steady_state_functions_byte(set_val.byte));
}

err_code_t eagle2_tsc2pll_set_usr_ctrl_disable_steady_state_dfe (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_dfe_functions_byte (pa, &set_val));
...
/******************************************************************/
err_code_t eagle2_tsc2pll_get_uc_core_config (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_core_config_st *get_val)
{
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_core_event_log_level (const phymod_access_t *pa, uint8_t *core_event_log_level) {
...
err_code_t eagle2_tsc2pll_get_uc_lane_cfg (const phymod_access_t *pa, struct eagle2_tsc2pll_uc_lane_config_st *get_val) {
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_lane_event_log_level (const phymod_access_t *pa, uint8_t *lane_event_log_level) {
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_disable_startup (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_disable_startup_dfe (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_disable_steady_state (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t eagle2_tsc2pll_get_usr_ctrl_disable_steady_state_dfe (const phymod_access_t *pa, struct eagle2_tsc2pll_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t eagle2_tsc2pll_get_clk90_offset_adjust (const phymod_access_t *pa, int8_t *adjust) {
  if (!adjust) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t eagle2_tsc2pll_set_clk90_offset_adjust (const phymod_access_t *pa, int8_t adjust) {
...
err_code_t eagle2_tsc2pll_get_clk90_offset_override (const phymod_access_t *pa, uint8_t *override) {
  if (!override) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t eagle2_tsc2pll_set_clk90_offset_override (const phymod_access_t *pa, uint8_t enable, uint8_t override) {        
...
err_code_t eagle2_tsc2pll_get_clk90_offset (const phymod_access_t *pa, int8_t *offset) {
    int8_t clk90_offset;
...
err_code_t eagle2_tsc2pll_poll_diag_done (const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(loop>10) {
         EFUN(eagle2_tsc2pll_delay_us(10*timeout_ms));
     }
 }
...
/* poll for microcontroller to populate the dsc_data register */
err_code_t eagle2_tsc2pll_poll_diag_eye_data (const phymod_access_t *pa, uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 uint16_t dscdata;
 if(!data || !status) {
     return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 (const phymod_access_t *pa, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, DSC_A_DSC_UC_CTRL, &rddata));
    if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
      if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
        ...
        EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, DSC_A_DSC_UC_CTRL,0x80));
...
#else
  {   uint16_t rddata;
      EFUN_PRINTF(("// ATE_LOG eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(eagle2_tsc2pll_delay_us(1000 * timeout_ms));
      } else {
          EFUN(eagle2_tsc2pll_delay_ms(timeout_ms));
      }
      EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          EFUN_PRINTF(("// ATE_LOG eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      EFUN_PRINTF(("// ATE_LOG eagle2_tsc2pll_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t eagle2_tsc2pll_poll_dsc_state_equals_uc_tune (const phymod_access_t *pa, uint32_t timeout_ms) {
  uint16_t loop;
  uint16_t dsc_state;
...
err_code_t eagle2_tsc2pll_init_pram_for_uc_load (const phymod_access_t *pa, uint16_t ucode_len) {
    uint8_t result;
    ...
    EFUN(wrc_micro_system_reset_n(0x1));                  /* De-assert reset to micro */
    EFUN(wrc_micro_pram_if_flop_bypass(1));
    ...
    EFUN(eagle2_tsc2pll_delay_us(500));
    ...
    /* Wait 8 pram clocks */
    /* call eagle2_tsc2pll_finish_pram_load */
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_finish_pram_load (const phymod_access_t *pa) {
...
err_code_t eagle2_tsc2pll_reg_dump (const phymod_access_t *pa) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, addr,&rddata));
...
err_code_t eagle2_tsc2pll_uc_core_var_dump (const phymod_access_t *pa) {
  uint8_t addr;
  ...
  for (addr = 0x0; addr < CORE_VAR_RAM_SIZE; addr++) {
    ...
    ESTM_PRINTF(("%02x ", eagle2_tsc2pll_rdbc_uc_var (pa, __ERR, addr)));
  }
  return (ERR_CODE_NONE);
}


err_code_t eagle2_tsc2pll_uc_lane_var_dump (const phymod_access_t *pa) {
  uint8_t     rx_lock, uc_stopped = 0;
  uint16_t    addr;


  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",eagle2_tsc2pll_get_lane(pa)));
  ...
  if (rx_lock == 1) {
      ...
      if (!uc_stopped) {
          EFUN(eagle2_tsc2pll_stop_rx_adaptation (pa, 1));
      }
  } else {
      EFUN(eagle2_tsc2pll_pmd_uc_control (pa, CMD_UC_CTRL_STOP_IMMEDIATE,200));
  }
    ...
    for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
      ...
      ESTM_PRINTF(("%02x ", eagle2_tsc2pll_rdbl_uc_var (pa, __ERR, addr)));
    }

  if (rx_lock == 1) {
      if (!uc_stopped) {
          EFUN(eagle2_tsc2pll_stop_rx_adaptation (pa, 0));
      }
  } else {
      EFUN(eagle2_tsc2pll_stop_rx_adaptation (pa, 0));
  }
...
err_code_t eagle2_tsc2pll_display_diag_data (const phymod_access_t *pa, int32_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",eagle2_tsc2pll_get_core(),eagle2_tsc2pll_get_lane(pa)));
    EFUN(eagle2_tsc2pll_version (pa, &api_version));
    ...
    if (rx_lock == 1) {
        ...
        EFUN(eagle2_tsc2pll_display_lane_state_hdr (pa ));
        EFUN(eagle2_tsc2pll_display_lane_state (pa ));
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(eagle2_tsc2pll_read_event_log (pa, trace_mem,EVENT_LOG_HEX));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(eagle2_tsc2pll_display_eye_scan (pa ));
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(eagle2_tsc2pll_reg_dump (pa ));
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(eagle2_tsc2pll_uc_core_var_dump (pa ));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(eagle2_tsc2pll_uc_lane_var_dump (pa ));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(eagle2_tsc2pll_display_lane_debug_status (pa ));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_eagle2_tsc2pll_display_ber_scan_data (pa, ber_mode, timer_control,  err_threshold));
...
/* Required Diagnostic Functions */
err_code_t eagle2_tsc2pll_diag_access (const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", eagle2_tsc2pll_rdbc_uc_var (pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,eagle2_tsc2pll_rdbc_uc_var (pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", eagle2_tsc2pll_rdbl_uc_var (pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,eagle2_tsc2pll_rdbl_uc_var (pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", eagle2_tsc2pll_rdwc_uc_var (pa, __ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,eagle2_tsc2pll_rdwc_uc_var (pa, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", eagle2_tsc2pll_rdwl_uc_var (pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,eagle2_tsc2pll_rdwl_uc_var (pa, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = eagle2_tsc2pll_rdbc_uc_var (pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(eagle2_tsc2pll_wrbc_uc_var (pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = eagle2_tsc2pll_rdbl_uc_var (pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(eagle2_tsc2pll_wrbl_uc_var (pa, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = eagle2_tsc2pll_rdwc_uc_var (pa, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(eagle2_tsc2pll_wrwc_uc_var (pa, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = eagle2_tsc2pll_rdwl_uc_var (pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(eagle2_tsc2pll_wrwl_uc_var (pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _eagle2_tsc2pll_rdb_uc_var (pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_eagle2_tsc2pll_rdb_uc_var (pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _eagle2_tsc2pll_rdb_uc_var (pa, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_eagle2_tsc2pll_wrb_uc_var (pa, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _eagle2_tsc2pll_rdw_uc_var (pa, __ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_eagle2_tsc2pll_rdw_uc_var (pa, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _eagle2_tsc2pll_rdw_uc_var (pa, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_eagle2_tsc2pll_wrw_uc_var (pa, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(eagle2_tsc2pll_pmd_uc_cmd_with_data (pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_eagle2_tsc2pll_display_ber_scan_data (pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    case SRDS_EN_BREAKPOINT: 
    case SRDS_GOTO_BREAKPOINT:
    case SRDS_RD_BREAKPOINT:
    case SRDS_DIS_BREAKPOINT:
    default: EFUN_PRINTF(("Invalid request type eagle2_tsc2pll_diag_access\n"));
    }
...
err_code_t eagle2_tsc2pll_version (const phymod_access_t *pa, uint32_t *api_version) {

    if(!api_version) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    *api_version = 0xA1021E;
    return (ERR_CODE_NONE);
}
...
err_code_t eagle2_tsc2pll_read_event_log (const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(eagle2_tsc2pll_event_log_stop (pa ));

    EFUN(eagle2_tsc2pll_event_log_readmem (pa, trace_mem));

    EFUN(eagle2_tsc2pll_event_log_display (pa, trace_mem, display_mode));

    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_event_log_stop (const phymod_access_t *pa) {
    ...
    EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 1000));

    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_event_log_readmem (const phymod_access_t *pa, uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 1000));
    ...
    EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 1000));

    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_event_log_display (const phymod_access_t *pa, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
        ...
        while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
            ...
            if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                EFUN(eagle2_tsc2pll_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t eagle2_tsc2pll_display_state (const phymod_access_t *pa) {
    EFUN(eagle2_tsc2pll_display_core_state (pa ));
    EFUN(eagle2_tsc2pll_display_lane_state_hdr (pa ));
    EFUN(eagle2_tsc2pll_display_lane_state (pa ));
    EFUN(eagle2_tsc2pll_display_lane_state_legend (pa ));
    return(ERR_CODE_NONE);
}

err_code_t eagle2_tsc2pll_display_config (const phymod_access_t *pa) {
    EFUN(eagle2_tsc2pll_display_core_config (pa ));
    EFUN(eagle2_tsc2pll_display_lane_config (pa ));
    return(ERR_CODE_NONE);
}
...
err_code_t eagle2_tsc2pll_force_die_temperature (const phymod_access_t *pa, int16_t die_temp) {
...
err_code_t eagle2_tsc2pll_read_die_temperature (const phymod_access_t *pa, int16_t *die_temp) {

    uint16_t die_temp_sensor_reading;

    EFUN(eagle2_tsc2pll_pmd_uc_cmd (pa, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,100));
...
err_code_t eagle2_tsc2pll_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    EFUN_PRINTF((" Function 'eagle2_tsc2pll_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
