...
static err_code_t _blackhawk7_v1l8p2_pll_powerdown(srds_access_t *sa__, uint8_t pwrdn_en) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_POWERDOWN) {
	UNUSED(refclk_freq_hz);
        _blackhawk7_v1l8p2_pll_powerdown(sa__, 0x1);
        return (ERR_CODE_NONE);
    }

    EFUN(blackhawk7_v1l8p2_INTERNAL_resolve_pll_parameters(sa__, refclk, &refclk_freq_hz, &div, &vco_freq_khz, pll_option));
    ...
    /* Clear PLL powerdown */
    _blackhawk7_v1l8p2_pll_powerdown(sa__, 0x0);

    /* Use LBVCO for 21.0 GHz and below */
    EFUN(wrc_ams_pll_pwrdn_hbvco(0x1));
    EFUN(wrc_ams_pll_pwrdn_lbvco(0x1));
    if (vco_freq_khz > 21000000) {
...

    {
        ...
        const uint16_t div_integer        = SRDS_INTERNAL_GET_PLL_DIV_INTEGER(div);
        const uint32_t div_fraction       = SRDS_INTERNAL_GET_PLL_DIV_FRACTION_NUM(div, div_fraction_width);
        ...
        /* For MMD 8/9 mode (pll_frac_mode - 1) => (60 <= Ndiv < 511) */
        if ((pll_fracn_ndiv_int < 60) || (pll_fracn_ndiv_int > 510)) {
    ...
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DOUBLER_EN) {
        EFUN(wrc_ams_pll_refclk_freq2x_en(0x1));
    } else if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN) {
        EFUN(wrc_ams_pll_refdiv2(1));
        EFUN(wrc_ams_pll_div4_2_sel(1));
    } else if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV4_EN) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_pll_div(srds_access_t *sa__, uint32_t *div) {
