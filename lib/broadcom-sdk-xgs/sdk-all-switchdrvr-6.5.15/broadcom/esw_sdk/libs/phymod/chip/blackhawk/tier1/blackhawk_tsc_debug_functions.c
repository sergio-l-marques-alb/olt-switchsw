...
err_code_t blackhawk_tsc_reg_dump(srds_access_t *sa__, char reg_buffer[][250]) {
  ...
  if (reg_buffer == NULL) {
      ...
      for (addr = 0x0; addr < 0x10; addr++) {
          ...
          USR_STRNCAT(reg_buffer[index], tempBuf, 5);
      }
...
err_code_t blackhawk_tsc_uc_core_var_dump(srds_access_t *sa__, char ram_buffer[][250]) {
  uint8_t addr, rddata;
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < 0xFF; addr++) {
          ...
          USR_STRNCAT(ram_buffer[index], tempBuf, 3);
      }
...
err_code_t blackhawk_tsc_uc_micro_var_dump(srds_access_t *sa__, char ram_buffer[][250]) {
  uint16_t addr;

  if (ram_buffer == NULL) {
      EFUN_PRINTF(("\n**** SERDES UC MICRO %d RAM VARIABLE DUMP ****",(blackhawk_tsc_get_physical_lane(sa__)/2)));
  ...
  else
  {
      ...
      EFUN_PRINTF(("\n**** SERDES UC MICRO %d RAM VARIABLE DECODED ****\n",(blackhawk_tsc_get_physical_lane(sa__)/2)));
      ...
      for (addr = 0x0; addr < 0x10; addr++) {
         ...
         USR_SPRINTF(tempBuf,"%02x ", rddata);
         USR_STRNCAT(ram_buffer[index], tempBuf, 5);
     }
     ram_buffer[index+1][0] = 0xA;
     ram_buffer[index+2][0] = 0;
  }
...
err_code_t blackhawk_tsc_uc_lane_var_dump(srds_access_t *sa__, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      uint8_t rddata, index = 0;
      char tempBuf[250];
      EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DECODED ****\n",blackhawk_tsc_get_lane(sa__)));
...
err_code_t blackhawk_tsc_isolate_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    blackhawk_tsc_info_t * blackhawk_tsc_info_ptr = blackhawk_tsc_INTERNAL_get_blackhawk_tsc_info_ptr_with_check(sa__);
