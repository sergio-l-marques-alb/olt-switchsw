...
uint16_t falcon16_tsc_INTERNAL_get_lane_addr(srds_access_t *sa__, uint16_t addr, uint8_t lane) {
...
uint8_t falcon16_tsc_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    ...
    {
        lane = falcon16_tsc_get_physical_lane(sa__);
        lane_addr_offset = falcon16_tsc_INTERNAL_get_lane_addr(sa__, addr,lane);

        EPSTM(rddata = falcon16_tsc_rdb_uc_ram(sa__, err_code_p, lane_addr_offset)); /* Use Micro register interface for reading RAM */
        return (rddata);
    }
}
...
uint16_t falcon16_tsc_rdwl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
...

    {
        ...
        EPSTM(rddata = falcon16_tsc_rdw_uc_ram(sa__, err_code_p, lane_addr_offset)); /* Use Micro register interface for reading RAM */
        return (rddata);
    }
}
...
err_code_t falcon16_tsc_wrbl_uc_var(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {
...

    {
        ...
        return (falcon16_tsc_wrb_uc_ram(sa__, lane_addr_offset, wr_val));    /* Use Micro register interface for writing RAM */
    }
}
...
err_code_t falcon16_tsc_wrwl_uc_var(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
    ...
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (falcon16_tsc_error(sa__, ERR_CODE_INVALID_RAM_ADDR));
    }

    {
        ...
        return (falcon16_tsc_wrw_uc_ram(sa__, lane_addr_offset, wr_val));    /* Use Micro register interface for writing RAM */
    }
...
uint16_t falcon16_tsc_INTERNAL_get_core_address(srds_access_t *sa__, uint16_t addr, uint8_t lane) {
...
uint8_t falcon16_tsc_rdbc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
    ...
    {
        lane = falcon16_tsc_get_physical_lane(sa__);
        core_addr_offset = falcon16_tsc_INTERNAL_get_core_address(sa__, addr,lane);
        EPSTM(rddata = falcon16_tsc_rdb_uc_ram(sa__, err_code_p, core_addr_offset));    /* Use Micro register interface for reading RAM */
        return (rddata);
    }
}
...
uint16_t falcon16_tsc_rdwc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
...

    {
        ...
        EPSTM(rddata = falcon16_tsc_rdw_uc_ram(sa__, err_code_p, core_addr_offset));    /* Use Micro register interface for reading RAM */
        return (rddata);
    }
}
...
err_code_t falcon16_tsc_wrbc_uc_var(srds_access_t *sa__, uint8_t addr, uint8_t wr_val) {
...

    {
        ...
        return (falcon16_tsc_wrb_uc_ram(sa__, core_addr_offset, wr_val));               /* Use Micro register interface for writing RAM */
    }
...
err_code_t falcon16_tsc_wrwc_uc_var(srds_access_t *sa__, uint8_t addr, uint16_t wr_val) {
...

    {
        ...
        lane = falcon16_tsc_get_physical_lane(sa__);
        core_addr_offset = falcon16_tsc_INTERNAL_get_core_address(sa__, addr,lane);
        return (falcon16_tsc_wrw_uc_ram(sa__, core_addr_offset, wr_val));             /* Use Micro register interface for writing RAM */
    }
}
...
err_code_t falcon16_tsc_rdblk_uc_prog_ram(srds_access_t *sa__, uint8_t *mem, uint16_t addr, uint16_t cnt) {
    ...
    if(!mem) {
        return(falcon16_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
