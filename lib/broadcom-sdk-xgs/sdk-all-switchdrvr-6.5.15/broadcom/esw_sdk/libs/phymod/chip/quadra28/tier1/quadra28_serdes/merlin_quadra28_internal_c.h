/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_quadra28_internal.c                                         *
 *  Created On    :  13/02/2014                                                    *
 *  Created By    :  Justin Gaither                                                *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_quadra28_internal.c 947 2015-03-06 21:11:44Z kirand $ *
 *                                                                                 *
...
/** @file merlin_quadra28_internal.c
 * Implementation of API functions
 */

#include "merlin_quadra28_internal.h"
...
SDK_STATIC int16_t _ladder_setting_to_mV(const phymod_access_t *pa,int8_t ctrl, uint8_t range_250) {
    ...
    {
    int16_t nlv = 0;                                      /* Non-linear value */
...
static err_code_t _ull_tx_mode_setup( const phymod_access_t *pa, uint8_t enable) {
    ...
    EFUN(merlin_quadra28_delay_us(1024));                          /* Wait for phase detector path to settle */
    ...
    EFUN(merlin_quadra28_poll_st_afe_tx_fifo_resetb_equals_1( pa, 1));  /* Check if FIFO reset is reported to be released within 1ms time interval */
...
static uint8_t _merlin_quadra28_rdb_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
...

    {
        ...
        if (uc_lane_info.is_direct_ram_access_avail) {
            EPFUN(merlin_quadra28_rdb_ram(pa, &rddata, addr, 1));    /* Direct RAM access */
        }
        else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            *err_code_p = ERR_CODE_BAD_PTR_OR_INVALID_INPUT;
            return(0);
        }
    }
...
static uint16_t _merlin_quadra28_rdw_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  if (addr%2 != 0) {                                                                /* Validate even address */
...

    {
      ...
      if (uc_lane_info.is_direct_ram_access_avail) {
          EFUN(merlin_quadra28_rdw_ram(pa, &rddata, addr, 1));                    /* Direct RAM access */
      }
      else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            *err_code_p = ERR_CODE_BAD_PTR_OR_INVALID_INPUT;
...
static err_code_t _merlin_quadra28_wrb_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {

    {
        ...
        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrb_ram(pa, addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT)); /* Use Serdes FW DSC Command Interface for writing RAM */
        }
    }
}
/* Micro Global RAM Word Write  */
static err_code_t _merlin_quadra28_wrw_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
    ...
    {   merlin_quadra28_uc_lane_info_st uc_lane_info;
        ...
        EFUN(merlin_quadra28_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(merlin_quadra28_wrw_ram(pa, addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        } else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT)); /* Use Serdes FW DSC Command Interface for writing RAM */
...
static err_code_t _merlin_display_event(const phymod_access_t *pa,uint8_t event_id,
                          uint8_t entry_len,
                          uint8_t prev_cursor,
                          uint8_t curr_cursor,
                          uint8_t post_cursor,
...
static err_code_t _merlin_quadra28_core_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
static err_code_t _merlin_quadra28_lane_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC err_code_t _merlin_quadra28_get_osr_mode( const phymod_access_t *pa, merlin_quadra28_osr_mode_st *imode) {
    merlin_quadra28_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(merlin_quadra28_osr_mode_st));
...
SDK_STATIC err_code_t _merlin_quadra28_read_lane_state( const phymod_access_t *pa, merlin_quadra28_lane_state_st *istate) {

  merlin_quadra28_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(merlin_quadra28_lane_state_st));
...
static err_code_t _merlin_quadra28_display_lane_state_no_newline( const phymod_access_t *pa ) {
  uint16_t lane_idx;
  merlin_quadra28_lane_state_st state;
  ...
  EFUN(_merlin_quadra28_read_lane_state( pa, &state));

  lane_idx = merlin_quadra28_get_lane(pa);
  ...
  if(state.osr_mode.tx_rx != 255) {
      ...
      EFUN_PRINTF(("(%2s%s:%s, 0x%2x,", (state.br_pd_en) ? "BR" : "OS", s,r, state.ucv_config & 0xFF));  /* Show lower byte only */
  }
  EFUN_PRINTF(("   %01x, %01x)",state.reset_state,state.stop_state));
  EFUN_PRINTF(("   %1d   %1d %4d ", state.sig_det, state.rx_lock, state.rx_ppm));
...
SDK_STATIC err_code_t _merlin_quadra28_read_core_state( const phymod_access_t *pa, merlin_quadra28_core_state_st *istate) {
  merlin_quadra28_core_state_st state;
  struct merlin_quadra28_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(merlin_quadra28_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct merlin_quadra28_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(merlin_quadra28_read_die_temperature( pa, &die_temp));
...
static err_code_t _merlin_quadra28_display_core_state_no_newline( const phymod_access_t *pa ) {
    merlin_quadra28_core_state_st state;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_merlin_quadra28_read_core_state( pa, &state));
    EFUN_PRINTF(("%02d  "              ,  merlin_quadra28_get_core()));
    ...
    EFUN(_merlin_quadra28_display_pll_to_divider( pa,   state.pll_div));
...
static err_code_t _update_uc_lane_config_st( const phymod_access_t *pa, struct  merlin_quadra28_uc_lane_config_st *st) {
  uint16_t in = st->word;
...
static err_code_t _update_usr_ctrl_disable_functions_st( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st *st) {
  uint8_t in = st->byte;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_st( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = st->byte;
...
static err_code_t _update_uc_core_config_st( const phymod_access_t *pa, struct merlin_quadra28_uc_core_config_st *st) {
  uint16_t in = st->word;
  ...
  st->vco_rate_in_Mhz = (1000*(22+st->field.vco_rate))>>2;
  return ERR_CODE_NONE;
}

/* fields to word, to write into uC RAM */
static err_code_t _update_uc_core_config_word( const phymod_access_t *pa, struct merlin_quadra28_uc_core_config_st *st) {
  uint16_t in = 0;
...
static err_code_t _update_uc_lane_config_word( const phymod_access_t *pa, struct merlin_quadra28_uc_lane_config_st *st) {
  uint16_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_functions_byte( const phymod_access_t *pa, struct merlin_quadra28_usr_ctrl_disable_functions_st *st) {
  uint8_t in = 0;
...
static err_code_t _update_usr_ctrl_disable_dfe_functions_byte( const phymod_access_t *pa, struct  merlin_quadra28_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = 0;
...
static err_code_t _merlin_quadra28_display_pll_to_divider( const phymod_access_t *pa, uint8_t val) {
...
static err_code_t _merlin_quadra28_display_ber_scan_data( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    merlin_quadra28_osr_mode_st osr_mode;
    struct merlin_quadra28_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(merlin_quadra28_get_uc_core_config( pa, &core_config));
    EFUN(_merlin_quadra28_get_osr_mode( pa, &osr_mode));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(merlin_quadra28_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(merlin_quadra28_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin_quadra28_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,2000));
