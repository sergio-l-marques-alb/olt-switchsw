/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_v1l8p2_config.c                                           *
...
/** @file blackhawk7_v1l8p2_config.c
...
#include "blackhawk7_v1l8p2_version.h"

/* If SERDES_EVAL is defined, then is_ate_log_enabled() is queried to *\
\* know whether to log ATE.  blackhawk7_v1l8p2_access.h provides that function.  */
...
err_code_t blackhawk7_v1l8p2_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  ...
  if (enable) {
      err_code_t err_code;
      err_code = blackhawk7_v1l8p2_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_GRACEFULLY,GRACEFUL_STOP_TIME);
      if(err_code) {
          uint8_t temp=0;
          USR_PRINTF(("Warning Graceful stop request returned error %d; Requesting a forceful stop\n",err_code));
          temp = blackhawk7_v1l8p2_INTERNAL_stop_micro(sa__,0,&err_code);
          /* return value from blackhawk7_v1l8p2_INTERNAL_stop_micro - immediate stop is not required */
    ...
    return(blackhawk7_v1l8p2_pmd_uc_control(sa__, CMD_UC_CTRL_RESUME,50));
  }
}

err_code_t blackhawk7_v1l8p2_request_stop_rx_adaptation(srds_access_t *sa__) {

  return(blackhawk7_v1l8p2_pmd_uc_control_return_immediate(sa__, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t blackhawk7_v1l8p2_set_uc_lane_cfg(srds_access_t *sa__, struct blackhawk7_v1l8p2_uc_lane_config_st struct_val) {
  ...
  if(reset_state < 7) {
      EFUN_PRINTF(("ERROR: blackhawk7_v1l8p2_set_uc_lane_cfg(..) called without ln_dp_s_rstb=0\n"));
      return (_error(ERR_CODE_LANE_DP_NOT_RESET));
  }
  EFUN(blackhawk7_v1l8p2_INTERNAL_update_uc_lane_config_word(&struct_val));
...
err_code_t blackhawk7_v1l8p2_get_uc_core_config(srds_access_t *sa__, struct blackhawk7_v1l8p2_uc_core_config_st *get_val)
{
    
    if(!get_val) {
        return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }
    switch (blackhawk7_v1l8p2_get_pll_idx(sa__)) {
    case  0:
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_update_uc_core_config_st(get_val));
    return ERR_CODE_NONE;
}

err_code_t blackhawk7_v1l8p2_get_uc_lane_cfg(srds_access_t *sa__, struct blackhawk7_v1l8p2_uc_lane_config_st *get_val) {

  if(!get_val) {
     return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
  ESTM(get_val->word = reg_rd_RX_CKRST_CTRL_LANE_UC_CONFIG());
  EFUN(blackhawk7_v1l8p2_INTERNAL_update_uc_lane_config_st(get_val));
  return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_poll_micro_ra_initdone(sa__, 500));/* Poll for micro_ra_initdone = 1 to indicate initialization done */
    ...
    while (ucode_len > 0) {
        EFUN(blackhawk7_v1l8p2_pmd_wr_pram(sa__, *ucode_image));
        --ucode_len;
        ++ucode_image;
    }
    ...
    while (ucode_len_remainder > 0) {
        EFUN(blackhawk7_v1l8p2_pmd_wr_pram(sa__, 0));
        --ucode_len_remainder;
    }
    ...
    EFUN(wrc_micro_pramif_en(0));
    {
        ...
        ESTM(micro_orig = blackhawk7_v1l8p2_get_micro_idx(sa__));
        ESTM(num_micros = rdc_micro_num_uc_cores());
        for (micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            EFUN(blackhawk7_v1l8p2_set_micro_idx(sa__, micro_idx));
            EFUN(wrc_micro_core_clk_en(1));
        }
        EFUN(blackhawk7_v1l8p2_set_micro_idx(sa__, micro_orig));
    }
...
err_code_t blackhawk7_v1l8p2_set_core_config_from_pcs(srds_access_t *sa__, uint8_t core_cfg_from_pcs) {
    struct blackhawk7_v1l8p2_uc_core_config_st core_config = UC_CORE_CONFIG_INIT;
    EFUN(blackhawk7_v1l8p2_get_uc_core_config(sa__, &core_config));
    core_config.field.core_cfg_from_pcs = core_cfg_from_pcs;
    EFUN(blackhawk7_v1l8p2_INTERNAL_set_uc_core_config(sa__, core_config));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_write_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_pf_main(sa__, val));

    case RX_AFE_PF2:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_pf2(sa__, val));

    case RX_AFE_VGA:
      return(blackhawk7_v1l8p2_INTERNAL_set_rx_vga(sa__, val));

    case RX_AFE_DFE1:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_dfe1(sa__, val));

    case RX_AFE_DFE2:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_dfe2(sa__, val));

    case RX_AFE_DFE3:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_dfe3(sa__, val));

    case RX_AFE_DFE4:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_dfe4(sa__, val));

    case RX_AFE_DFE5:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_dfe5(sa__, val));

    case RX_AFE_DFE6:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe6(sa__, val));

    case RX_AFE_DFE7:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe7(sa__, val));

    case RX_AFE_DFE8:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe8(sa__, val));

    case RX_AFE_DFE9:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe9(sa__, val));

    case RX_AFE_DFE10:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe10(sa__, val));

    case RX_AFE_DFE11:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe11(sa__, val));

    case RX_AFE_DFE12:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe12(sa__, val));

    case RX_AFE_DFE13:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe13(sa__, val));

    case RX_AFE_DFE14:
        return (blackhawk7_v1l8p2_INTERNAL_set_rx_dfe14(sa__, val));

    case RX_AFE_PF3:
        return(blackhawk7_v1l8p2_INTERNAL_set_rx_pf3(sa__, val));
    default:
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
}
...
err_code_t blackhawk7_v1l8p2_read_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t *val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pf_main(sa__, val));
        break;
    case RX_AFE_PF2:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pf2(sa__, val));
        break;
    case RX_AFE_VGA:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_vga(sa__, val));
      break;
    case RX_AFE_DFE1:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe1(sa__, val));
        break;
    case RX_AFE_DFE2:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe2(sa__, val));
        break;
    case RX_AFE_DFE3:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe3(sa__, val));
        break;
    case RX_AFE_DFE4:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe4(sa__, val));
        break;
    case RX_AFE_DFE5:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe5(sa__, val));
        break;
    case RX_AFE_DFE6:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe6(sa__, val));
        break;
    case RX_AFE_DFE7:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe7(sa__, val));
        break;
    case RX_AFE_DFE8:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe8(sa__, val));
        break;
    case RX_AFE_DFE9:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe9(sa__, val));
        break;
    case RX_AFE_DFE10:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe10(sa__, val));
        break;
    case RX_AFE_DFE11:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe11(sa__, val));
        break;
    case RX_AFE_DFE12:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe12(sa__, val));
        break;
    case RX_AFE_DFE13:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe13(sa__, val));
        break;
    case RX_AFE_DFE14:
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe14(sa__, val));
        break;
    case RX_AFE_PF3:
        return(blackhawk7_v1l8p2_INTERNAL_get_rx_pf3(sa__, val));
    default:
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk7_v1l8p2_validate_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    ENULL_MEMSET(&txfir, 0, sizeof(blackhawk7_v1l8p2_txfir_st));
    ...
    return blackhawk7_v1l8p2_INTERNAL_validate_full_txfir_cfg(sa__, enable_taps, txfir);
}
...
static uint8_t _blackhawk7_v1l8p2_get_fir_tap_index(enum blackhawk7_v1l8p2_tx_afe_settings_enum param) {
    switch(param) {
...
err_code_t blackhawk7_v1l8p2_write_tx_afe(srds_access_t *sa__, enum blackhawk7_v1l8p2_tx_afe_settings_enum param, int16_t val) {
    uint8_t const tap_num = _blackhawk7_v1l8p2_get_fir_tap_index(param);
    ...
    if (tap_num != tap_num_invalid) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_set_tx_tap(sa__, tap_num, val));
        EFUN(blackhawk7_v1l8p2_INTERNAL_load_txfir_taps(sa__));
    } else {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    return(ERR_CODE_NONE);
...
err_code_t blackhawk7_v1l8p2_read_tx_afe(srds_access_t *sa__, enum blackhawk7_v1l8p2_tx_afe_settings_enum param, int16_t *val) {
    ...
    }
    {
        uint8_t const tap_num = _blackhawk7_v1l8p2_get_fir_tap_index(param);
        if (tap_num != tap_num_invalid) {
            EFUN(blackhawk7_v1l8p2_INTERNAL_get_tx_tap(sa__, tap_num, val));
        } else {
            return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        }
    }
...
err_code_t blackhawk7_v1l8p2_apply_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    return blackhawk7_v1l8p2_INTERNAL_apply_full_txfir_cfg(sa__, enable_taps, txfir);
}
...
err_code_t blackhawk7_v1l8p2_rmt_lpbk(srds_access_t *sa__, uint8_t enable) {
    if (enable) {
        EFUN(blackhawk7_v1l8p2_loop_timing(sa__, enable));
...
/*********************************/
static err_code_t _blackhawk7_v1l8p2_get_micro_num_uc_cores(srds_access_t *sa__, uint8_t *num_micros) {
    ESTM(*num_micros = rdc_micro_num_uc_cores());
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_uc_reset_with_info(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  return blackhawk7_v1l8p2_uc_reset(sa__, enable, ucode_info);
}
...
err_code_t blackhawk7_v1l8p2_uc_reset(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  if (enable) {
...

    {
      ...
      EFUN(blackhawk7_v1l8p2_pmd_wr_reg(sa__, 0xD22E, (0x8000 | (stack_size)<<2)));
    }
  }
  else {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_poll_micro_ra_initdone(sa__, 300)); /* Poll status of data RAM initialization */
 ...
 
    {
        ...
        int micro_idx;
        ESTM(micro_orig = blackhawk7_v1l8p2_get_micro_idx(sa__));
        EFUN(_blackhawk7_v1l8p2_get_micro_num_uc_cores(sa__, &num_micros));
...
err_code_t blackhawk7_v1l8p2_init_blackhawk7_v1l8p2_info(srds_access_t *sa__) {
    ...
    if (blackhawk7_v1l8p2_info_ptr == 0) {
        EFUN_PRINTF(("ERROR:  Info table pointer is null.\n"));
        RELEASE_LOCK_AND_RETURN (_error(ERR_CODE_INFO_TABLE_ERROR));
    }
    if (blackhawk7_v1l8p2_info_ptr->signature == SRDS_INFO_SIGNATURE) {
        /*Already initalized and so check for microcode version and exit with proper status*/
        err_code = blackhawk7_v1l8p2_INTERNAL_match_ucode_from_info(sa__);
        ...
        if (err_code == ERR_CODE_NONE) {
            ...
            RELEASE_LOCK_AND_RETURN (_error(ERR_CODE_MICRO_INIT_NOT_DONE));
        }
    }
    /*Never initialized so far, continue with initialization */

    err_code = blackhawk7_v1l8p2_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END);
    /* Release lock before returning from the call to avoid deadlock */
    if (ERR_CODE_NONE != err_code) {
       RELEASE_LOCK_AND_RETURN(_error(err_code));
    }
    ...
    blackhawk7_v1l8p2_info_ptr->lane_count = info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] & 0xFF;
    blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes = ((info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] & 0x1000000) != 0);

    blackhawk7_v1l8p2_info_ptr->core_var_ram_size = (info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] >> 8) & 0xFF;

    blackhawk7_v1l8p2_info_ptr->lane_var_ram_size = (info_table[INFO_TABLE_OFFS_TRACE_LANE_MEM_SIZE / sizeof(uint32_t)] >> 16) & 0xFFFF;

    blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size = (info_table[INFO_TABLE_OFFS_TRACE_LANE_MEM_SIZE / sizeof(uint32_t)] & 0xFFFF) / blackhawk7_v1l8p2_info_ptr->lane_count;
    blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size = (info_table[INFO_TABLE_OFFS_TRACE_LANE_MEM_SIZE / sizeof(uint32_t)] & 0xFFFF);
    
    blackhawk7_v1l8p2_info_ptr->diag_mem_ram_base = info_table[INFO_TABLE_OFFS_TRACE_MEM_PTR / sizeof(uint32_t)];
    blackhawk7_v1l8p2_info_ptr->trace_mem_ram_base = blackhawk7_v1l8p2_info_ptr->diag_mem_ram_base;
    
    blackhawk7_v1l8p2_info_ptr->core_var_ram_base = info_table[INFO_TABLE_OFFS_CORE_MEM_PTR / sizeof(uint32_t)];
    
    blackhawk7_v1l8p2_info_ptr->micro_var_ram_base = info_table[INFO_TABLE_OFFS_MICRO_MEM_PTR / sizeof(uint32_t)];
    blackhawk7_v1l8p2_info_ptr->micro_var_ram_size = (info_table[INFO_TABLE_OFFS_OTHER_SIZE_2 / sizeof(uint32_t)] >> 4) & 0xFF;

    blackhawk7_v1l8p2_info_ptr->lane_var_ram_base = info_table[INFO_TABLE_OFFS_LANE_MEM_PTR / sizeof(uint32_t)];


    if (info_table_version < 0x34) {
        blackhawk7_v1l8p2_info_ptr->micro_count = 1;
    } else {
        blackhawk7_v1l8p2_info_ptr->micro_count = info_table[INFO_TABLE_OFFS_OTHER_SIZE_2 / sizeof(uint32_t)] & 0xF;
    }

    if (info_table_version < 0x36) {
        blackhawk7_v1l8p2_info_ptr->grp_ram_size = 0x1fd0; /*size of micro stacksize */
    } else {
        blackhawk7_v1l8p2_info_ptr->grp_ram_size = info_table[INFO_TABLE_OFFS_GROUP_RAM_SIZE / sizeof(uint32_t)] & 0xFFFF;
    }
    blackhawk7_v1l8p2_info_ptr->ucode_version = info_table[INFO_TABLE_OFFS_UC_VERSION /sizeof(uint32_t)];
    blackhawk7_v1l8p2_info_ptr->signature = SRDS_INFO_SIGNATURE;
...
err_code_t blackhawk7_v1l8p2_firmware_load_none_init_blackhawk7_v1l8p2_info(srds_access_t *sa__) {
    ...
    blackhawk7_v1l8p2_info_ptr->lane_count = 8;
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_configure_pll_powerdown(srds_access_t *sa__) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, 0, 0, 0, BLACKHAWK7_V1L8P2_PLL_OPTION_POWERDOWN);
}
...
err_code_t blackhawk7_v1l8p2_configure_pll_refclk_div(srds_access_t *sa__,
    ...
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, div, 0, BLACKHAWK7_V1L8P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_v1l8p2_configure_pll_refclk_vco(srds_access_t *sa__,
                                           enum blackhawk7_v1l8p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_v1l8p2_configure_pll_div_vco(srds_access_t *sa__,
                                        enum blackhawk7_v1l8p2_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_NONE);
}
...
err_code_t blackhawk7_v1l8p2_configure_pll_refclk_div_div2refclk(srds_access_t *sa__,
                                           enum blackhawk7_v1l8p2_pll_refclk_enum refclk,
                                           enum blackhawk7_v1l8p2_pll_div_enum div) {
   return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, div, 0, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t blackhawk7_v1l8p2_configure_pll_refclk_vco_div2refclk(srds_access_t *sa__,
                                           enum blackhawk7_v1l8p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t blackhawk7_v1l8p2_configure_pll_div_vco_div2refclk(srds_access_t *sa__,
                                        enum blackhawk7_v1l8p2_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN);
}
/* following routines divide refclk input by 2 to prevent fracn mode */
err_code_t blackhawk7_v1l8p2_configure_pll_refclk_div_div4refclk(srds_access_t *sa__,
                                           enum blackhawk7_v1l8p2_pll_refclk_enum refclk,
                                           enum blackhawk7_v1l8p2_pll_div_enum div) {
   return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, div, 0, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t blackhawk7_v1l8p2_configure_pll_refclk_vco_div4refclk(srds_access_t *sa__,
                                           enum blackhawk7_v1l8p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t blackhawk7_v1l8p2_configure_pll_div_vco_div4refclk(srds_access_t *sa__,
                                        enum blackhawk7_v1l8p2_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_v1l8p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV4_EN);
}
...
err_code_t blackhawk7_v1l8p2_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum blackhawk7_v1l8p2_pll_div_enum div, uint32_t *vco_freq_khz, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_vco_from_refclk_div(sa__, refclk_freq_hz, div, vco_freq_khz, pll_option));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_tx_clock_align(srds_access_t *sa__, int num_lanes, int enable) {
    ...
    ESTM(current_lane = blackhawk7_v1l8p2_get_lane(sa__));
    ...
    EFUN(blackhawk7_v1l8p2_set_lane(sa__,current_lane)); /* return lane to previous state */
    return(ERR_CODE_NONE);
 }
