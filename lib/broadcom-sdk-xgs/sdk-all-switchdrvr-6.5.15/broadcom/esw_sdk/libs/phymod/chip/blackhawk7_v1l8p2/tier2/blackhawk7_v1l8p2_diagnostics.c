...
int blackhawk7_v1l8p2_phy_pmd_info_dump(const phymod_phy_access_t* phy, char *arg)
{
    ...
    blackhawk7_v1l8p2_info_t  *tsc_info_ptr = NULL;
    ...
    tsc_info_ptr = blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr();
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CONFIG:
            ...
            for (i = 0; i < num_lane; i++) {
                ...
                if ((options->ber_proj_hist_errcnt_thresh == 0)) {
                    ...
                    PHYMOD_IF_ERR_RETURN
                        (blackhawk7_v1l8p2_optimize_hist_errcnt_thresh(&phy_copy.access,
                                                                   &err_analyzer,
                                                                   timeout_s,
                ...
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_prbs_error_analyzer_config(&phy_copy.access,
                                                              options->ber_proj_fec_size,
                                                              prbs_errcnt_thresh,
                                                              hist_errcnt_thresh));
                PHYMOD_IF_ERR_RETURN
                    (blackhawk7_v1l8p2_display_prbs_error_analyzer_config(&phy_copy.access, prbs_errcnt_thresh, hist_errcnt_thresh, options->ber_proj_timeout_s));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_START:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN(blackhawk7_v1l8p2_error_analyzer_status_clear(&phy_copy.access));
            }
            break;
        case PHYMOD_BER_PROJ_PHASE_F_COLLECT:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_MEMSET(&err_analyzer, 0, sizeof(blackhawk7_v1l8p2_prbs_err_analyzer_errcnt_st));
                PHYMOD_MEMCPY(err_analyzer.prbs_errcnt, options->ber_proj_prbs_errcnt[i].prbs_errcnt, sizeof(err_analyzer.prbs_errcnt));
                PHYMOD_IF_ERR_RETURN(
                    blackhawk7_v1l8p2_prbs_error_analyzer_accumulate_err_count(&phy_copy.access, &err_analyzer));
...
int blackhawk7_v1l8p2_phy_fast_ber_proj_get(const phymod_phy_access_t* phy, uint32_t* ber_proj_data)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_v1l8p2_INTERNAL_get_BER_data(&phy_copy.access, 100, &ber_data_local));
