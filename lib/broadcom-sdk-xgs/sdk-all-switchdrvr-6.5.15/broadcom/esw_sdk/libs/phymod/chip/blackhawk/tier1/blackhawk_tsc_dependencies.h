...
/** @file blackhawk_tsc_dependencies.h
 * Dependencies to be provided by IP User
 */

#ifndef BLACKHAWK_TSC_API_DEPENDENCIES_H
#define BLACKHAWK_TSC_API_DEPENDENCIES_H
 ...
 */
err_code_t blackhawk_tsc_pmd_rdt_reg(srds_access_t *sa__, uint16_t address, uint16_t *val);
 ...
 * If using Serdes MDIO controller to access the registers, implement this function using blackhawk_tsc_pmd_mdio_mwr_reg(..)
 *
 * If NOT using a Serdes MDIO controller or the Serdes PMI Masked write feature, please use the following code to
 * implement this function
 *
 *    blackhawk_tsc_pmd_wr_reg(addr, ((blackhawk_tsc_pmd_rd_reg(addr) & ~mask) | (mask & (val << lsb))));
 *
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t blackhawk_tsc_delay_ns(srds_access_t *sa__, uint16_t delay_ns);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t blackhawk_tsc_delay_us(srds_access_t *sa__, uint32_t delay_us);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t blackhawk_tsc_delay_ms(srds_access_t *sa__, uint32_t delay_ms);
#else
...
#define blackhawk_tsc_get_physical_lane blackhawk_tsc_get_lane
...
#define blackhawk_tsc_set_physical_lane blackhawk_tsc_set_lane
