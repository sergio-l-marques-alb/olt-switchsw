...
int _tscbh_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    ucode_info_t ucode;
    unsigned int blackhawk_ucode_len;
#ifndef TSCBH_PMD_CRC_UCODE_VERIFY
    unsigned char *blackhawk_ucode;
#endif
    ...
    /* 6. Start CRC calculation */
    if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
#ifdef TSCBH_PMD_CRC_UCODE_VERIFY
        PHYMOD_IF_ERR_RETURN(
            blackhawk_tsc_start_ucode_crc_calc(&core_copy.access, blackhawk_ucode_len));
#endif
    }
...
int _tscbh_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    if (init_config->firmware_load_method != phymodFirmwareLoadMethodNone) {
        ...
        if (PHYMOD_CORE_INIT_F_FIRMWARE_LOAD_VERIFY_GET(init_config)) {
#ifndef TSCBH_PMD_CRC_UCODE
            /* poll the ready bit in 10 ms */
            enum srds_pmd_uc_cmd_enum cmd;
            cmd = CMD_CALC_CRC;
            PHYMOD_IF_ERR_RETURN(
                blackhawk_tsc_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(&core_copy.access, 100, cmd));
#else
            PHYMOD_IF_ERR_RETURN(
                blackhawk_tsc_check_ucode_crc(&core_copy.access, blackhawk_ucoDe_crc, 200));
#endif
        }
...
int tscbh_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    if (PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk_pmd_tx_disable_pin_dis_set(&pm_phy_copy.access, 1));
        PHYMOD_IF_ERR_RETURN
          (tbhmod_init_pcs_ilkn(&phy->access));
    }
    
    PHYMOD_IF_ERR_RETURN
        (tbhmod_update_port_mode(pm_acc, &pll_restart));
#endif
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    PHYMOD_IF_ERR_RETURN
...
/*Set timesync adjust*/
int tscbh_timesync_adjust_set(const phymod_phy_access_t* phy, uint32_t ts_am_norm_mode)
{
    ...
    psll_entry_size = TBHMOD_TS_TABLE_SIZE * TBHMOD_TS_PSLL_BASED_ENTRY_SIZE;
    if (start_lane < 4) {
        /* MPP0 */
        mem_offset = start_lane * TBHMOD_TS_TABLE_SIZE;
    } else {
        /* MPP1 */
        mem_offset = (start_lane - 4) * TBHMOD_TS_TABLE_SIZE;
    }
    ...
    PHYMOD_IF_ERR_RETURN
       (tbhmod_1588_table_index_get(mapped_speed_id, speed_config.fec_type, &ts_table_index));
    ...
    */
     ...
     */
    for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
        PHYMOD_IF_ERR_RETURN
 ...
 * 2. Re-load the config table if error is in UM, AM table.
 * 3. Re-load the 1588 tables and SPEED_ID table for 1b error.
                       ...
                       uint32_t* is_handled)
{
    ...
    uint32_t speed_id_table[TSCBH_HW_SPEED_ID_TABLE_SIZE][TSCBH_SPEED_ID_ENTRY_SIZE];
    int i, j, mem_offset;
    ts_table_entry ts_table;
    ...
    switch (type) {
        case phymodIntrEccAMTable:
            *is_handled = 1;
            ...
            for (i = 0; i < am_table_load_size; i++) {
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemAMTable, i,  &am_table_entry[i][0]));
            }
            break;
        case phymodIntrEccSpeedTable:
            if (intr_status.is_1b_err) {
                for (i = 0; i < TSCBH_HW_SPEED_ID_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_read(&phy_copy.access, phymodMemSpeedIdTable, i, &speed_id_table[i][0]));
                }
                for (i = 0; i < TSCBH_HW_SPEED_ID_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemSpeedIdTable, i, &speed_id_table[i][0]));
                }
            }
            break;
        case phymodIntrEccUMTable:
            *is_handled = 1;
            ...
            for (i = 0; i < um_table_load_size; i++) {
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&phy_copy.access, phymodMemUMTable, i,  &um_table_entry[i][0]));
            }
            break;
        case phymodIntrEccRx1588400g:
            if (intr_status.is_1b_err) {
                for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_read(&phy_copy.access, phymodMemRxLkup1588400G, i, &ts_table[i][0]));
                }
                for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemRxLkup1588400G, i, &ts_table[i][0]));
                }
            }
            break;
        case phymodIntrEccRx1588Mpp1:
             if (intr_status.is_1b_err) {
                for (j = 0; j < TBHMOD_TS_MPP_MEM_TABLE_CNT; j++) {
                    mem_offset = j * TBHMOD_TS_TABLE_SIZE;
                    for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                        PHYMOD_IF_ERR_RETURN
                            (phymod_mem_read(&phy_copy.access, phymodMemRxLkup1588Mpp1,
                                             i + mem_offset, &ts_table[i][0]));
                    }
                    for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                        PHYMOD_IF_ERR_RETURN
        ...
        case phymodIntrEccTx1588400g:
            if (intr_status.is_1b_err) {
                for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_read(&phy_copy.access, phymodMemTxLkup1588400G, i, &ts_table[i][0]));
                }
                for (i = 0; i < TBHMOD_TS_TABLE_SIZE; i++) {
                    PHYMOD_IF_ERR_RETURN
                        (phymod_mem_write(&phy_copy.access, phymodMemTxLkup1588400G, i, &ts_table[i][0]));
                }
            }
            break;
