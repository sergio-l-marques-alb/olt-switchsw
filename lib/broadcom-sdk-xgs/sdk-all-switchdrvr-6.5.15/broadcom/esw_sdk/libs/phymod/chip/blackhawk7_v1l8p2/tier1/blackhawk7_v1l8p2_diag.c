/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_v1l8p2_diag.c                                             *
...
#include "blackhawk7_v1l8p2_diag.h"
...
err_code_t blackhawk7_v1l8p2_test_uc_timestamp(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_TIMESTAMP,100));
...
err_code_t blackhawk7_v1l8p2_display_eye_scan(srds_access_t *sa__) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    ...
    y_step = EYE_SCAN_NRZ_VERTICAL_STEP;
    {
        ...
        else if (pam_mode == PAM4_ER) {
            y_max  = EYE_SCAN_PAM_VERTICAL_IDX_MAX;
            y_step = EYE_SCAN_PAM_VERTICAL_STEP;
            {
                uint16_t version=0;
                ESTM(version = rdcv_common_ucode_version());
    ...
    EFUN(blackhawk7_v1l8p2_display_eye_scan_header(sa__,1));

    /* start horizontal acquisition */
    {   err_code_t err_code = blackhawk7_v1l8p2_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((blackhawk7_v1l8p2_meas_eye_scan_done(sa__), err_code));
        }
    }

    for (y = y_max;y>=-y_max;y=y-y_step)
    {
        {   err_code_t err_code = blackhawk7_v1l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN((blackhawk7_v1l8p2_meas_eye_scan_done(sa__), err_code));
            }
        }
        EFUN(blackhawk7_v1l8p2_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(blackhawk7_v1l8p2_meas_eye_scan_done(sa__));
    EFUN(blackhawk7_v1l8p2_display_eye_scan_footer(sa__,1));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(blackhawk7_v1l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(blackhawk7_v1l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(blackhawk7_v1l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_INTERNAL_poll_diag_data(srds_access_t *sa__, const blackhawk7_v1l8p2_info_t *blackhawk7_v1l8p2_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    const uint32_t lane_diag_size = blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size;
    uint16_t loop;
    ...
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : blackhawk7_v1l8p2_INTERNAL_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
typedef struct {
    uint32_t *buffer_ptr;
    } blackhawk7_v1l8p2_read_eye_scan_stripe_callback_arg_t;

static err_code_t blackhawk7_v1l8p2_read_eye_scan_stripe_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p2_read_eye_scan_stripe_callback_arg_t * const cast_arg = (blackhawk7_v1l8p2_read_eye_scan_stripe_callback_arg_t *)arg;
    UNUSED(sa__);
    *(cast_arg->buffer_ptr++) = blackhawk7_v1l8p2_INTERNAL_float8_to_int32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = blackhawk7_v1l8p2_INTERNAL_float8_to_int32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    const uint8_t lane = blackhawk7_v1l8p2_get_lane(sa__);
    const uint8_t stripe_size = 64;

    blackhawk7_v1l8p2_read_eye_scan_stripe_callback_arg_t arg;
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_verify_blackhawk7_v1l8p2_info(sa__, blackhawk7_v1l8p2_info_ptr));
        ...
        lane_diag_base = blackhawk7_v1l8p2_info_ptr->diag_mem_ram_base + ((lane%blackhawk7_v1l8p2_info_ptr->lane_count) * blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size) + 
            blackhawk7_v1l8p2_info_ptr->grp_ram_size*blackhawk7_v1l8p2_INTERNAL_grp_idx_from_lane(blackhawk7_v1l8p2_get_physical_lane(sa__));

    EFUN(blackhawk7_v1l8p2_INTERNAL_poll_diag_data(sa__, blackhawk7_v1l8p2_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
        
    arg.buffer_ptr = buffer;
    ESTM(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              blackhawk7_v1l8p2_read_eye_scan_stripe_callback));

    diag_rd_ptr = ((uint32_t)diag_rd_ptr + stripe_size) % blackhawk7_v1l8p2_info_ptr->diag_mem_ram_size;
    ESTM(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_display_eye_scan_stripe(srds_access_t *sa__, int8_t y,uint32_t *buffer) {
    ...
    level = blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, y,0);
...
err_code_t blackhawk7_v1l8p2_display_eye_scan_header(srds_access_t *sa__, int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t blackhawk7_v1l8p2_display_eye_scan_footer(srds_access_t *sa__, int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
err_code_t blackhawk7_v1l8p2_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
...
err_code_t blackhawk7_v1l8p2_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(blackhawk7_v1l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}


err_code_t blackhawk7_v1l8p2_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = blackhawk7_v1l8p2_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (blackhawk7_v1l8p2_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t blackhawk7_v1l8p2_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(blackhawk7_v1l8p2_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ESTM(offset_start = rd_uc_dsc_data());
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("offset_start = %d:%dmV\n",offset_start,blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start,0)));
    ...
    if(DIAG_VERBOSE > 5) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_poll_diag_done(sa__, &sts,timer_control*2000));
    }
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(blackhawk7_v1l8p2_read_ber_scan_data(sa__, &errs[0], &time[0], &cnt, 2000));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
 ...
 /* }                                                                  */
    ...
    EFUN(blackhawk7_v1l8p2_display_ber_scan_data(sa__, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)SRDS_ABS(offset_start)));
...
err_code_t blackhawk7_v1l8p2_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
...
#ifndef STANDALONE_EVENT
            if(intrusive) {
                margins[i] = direction*blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset[i], 0);
            } else {
                margins[i] = direction*blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
    ...
    */
    /* EFUN_PRINTF(("%x,%02x,%1f,%x,%x\n", DIAG_MAX_SAMPLES, big_endian, (double) rate, (unsigned int)ber_scan_mode, (unsigned int)max_offset)); */ 
    ...
    EFUN_PRINTF(("%lx", (long unsigned int) total_errs[0]));
    for (jj=1;jj<DIAG_MAX_SAMPLES;jj++) {
        EFUN_PRINTF((",%lx", (long unsigned int) total_errs[jj]));
    }
    EFUN_PRINTF(("\n"));
    EFUN_PRINTF(("%lx,", (long unsigned int) total_time[0]));
    for (jj=1;jj<DIAG_MAX_SAMPLES;jj++) {
        EFUN_PRINTF((",%lx", (long unsigned int) total_time[jj]));
    }
    EFUN_PRINTF(("\n"));
...
err_code_t blackhawk7_v1l8p2_display_lane_state_hdr(srds_access_t *sa__) {
  EFUN_PRINTF(("LN (RX  , CDRxN , UC_CFG,UC_STS,UC_STS_EXT,TX_RX_RST,STP,PLL)  "));
...
err_code_t blackhawk7_v1l8p2_display_lane_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
  ...
  EFUN_PRINTF(("UC_STS_EXT: Extended micro lane status variable\n"));
  ...
  EFUN_PRINTF(("PF(M,L,H) : Peaking Filter Main (0..15), Low Freq (0..7), and High Freq (0..7) settings\n"));
...
err_code_t blackhawk7_v1l8p2_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = blackhawk7_v1l8p2_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t blackhawk7_v1l8p2_display_core_state_hdr(srds_access_t *sa__) {
    char core_type[25] = "blackhawk7_v1l8p2";           
...
err_code_t blackhawk7_v1l8p2_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = blackhawk7_v1l8p2_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t blackhawk7_v1l8p2_display_core_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
/**********************************************/
err_code_t blackhawk7_v1l8p2_display_core_config(srds_access_t *sa__) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    EFUN_PRINTF((    "**** SERDES CORE CONFIGURATION ****\n"  ));
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        struct   blackhawk7_v1l8p2_uc_core_config_st core_cfg = UC_CORE_CONFIG_INIT;
        struct   blackhawk7_v1l8p2_uc_core_config_st pll1_cfg = UC_CORE_CONFIG_INIT;
        {
            uint8_t pll_orig;
            ESTM(pll_orig = blackhawk7_v1l8p2_get_pll_idx(sa__));
            EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__, 0));
            EFUN(blackhawk7_v1l8p2_get_uc_core_config(sa__, &core_cfg));
            EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__, 1));
            EFUN(blackhawk7_v1l8p2_get_uc_core_config(sa__, &pll1_cfg));
...
err_code_t blackhawk7_v1l8p2_display_lane_config(srds_access_t *sa__) {
    struct blackhawk7_v1l8p2_uc_lane_config_st lane_cfg;
    ...
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         blackhawk7_v1l8p2_get_uc_lane_cfg(sa__, &lane_cfg));
...
err_code_t blackhawk7_v1l8p2_display_core_state(srds_access_t *sa__) {
    EFUN(blackhawk7_v1l8p2_display_core_state_hdr(sa__));
    EFUN(blackhawk7_v1l8p2_display_core_state_line(sa__));
    EFUN(blackhawk7_v1l8p2_display_core_state_legend(sa__));
    return ERR_CODE_NONE;
}


err_code_t blackhawk7_v1l8p2_display_lane_debug_status(srds_access_t *sa__) {
    /* startup */
    struct blackhawk7_v1l8p2_usr_ctrl_disable_functions_st     ds;
    struct blackhawk7_v1l8p2_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct blackhawk7_v1l8p2_usr_ctrl_disable_functions_st     dss;
    struct blackhawk7_v1l8p2_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(blackhawk7_v1l8p2_get_usr_ctrl_disable_startup(sa__, &ds));
    ...
    EFUN(                                                            blackhawk7_v1l8p2_get_usr_ctrl_disable_startup_dfe(sa__, &dsd));
    ...
    EFUN(                                                           blackhawk7_v1l8p2_get_usr_ctrl_disable_steady_state(sa__, &dss));
    ...
    EFUN(                                                      blackhawk7_v1l8p2_get_usr_ctrl_disable_steady_state_dfe(sa__, &dssd));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    int rate,direction;
    uint8_t range250;
    blackhawk7_v1l8p2_osr_mode_st osr_mode;
    struct blackhawk7_v1l8p2_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(blackhawk7_v1l8p2_get_uc_core_config(sa__, &core_config));
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_osr_mode(sa__, &osr_mode));

    if(osr_mode.rx > BLACKHAWK7_V1L8P2_OSX4) {
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN_PRINTF(("STARTING OFFSET = %d : %d mV\n",offset_start,blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start, range250)));
    }
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("start done\n"));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = blackhawk7_v1l8p2_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (blackhawk7_v1l8p2_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        if(DIAG_VERBOSE < 5) {
            if (!(i % 4))  {
                EFUN_PRINTF(("\n"));
            }
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN_PRINTF(("%d %d %d ",direction*blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)SRDS_ABS(offset_start)-i, range250),errors,timer_values));
            }
    ...
    EFUN_PRINTF(("\n"));
    EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t blackhawk7_v1l8p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(blackhawk7_v1l8p2_version(sa__, &api_version));
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = blackhawk7_v1l8p2_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
        if(err_code) USR_PRINTF(("Unable to stop microcontroller,  following data is suspect\n"));
    }

    EFUN(blackhawk7_v1l8p2_display_lane_state_hdr(sa__));
    EFUN(blackhawk7_v1l8p2_display_lane_state(sa__));

    if(diag_level & SRDS_DIAG_LANE) {
        struct blackhawk7_v1l8p2_detailed_lane_status_st lane_st;

        ENULL_MEMSET(&lane_st, 0, sizeof(lane_st));
        EFUN(blackhawk7_v1l8p2_log_full_pmd_state(sa__, &lane_st));

        EFUN(blackhawk7_v1l8p2_disp_full_pmd_state(sa__, &lane_st, 1));
    }
    ...
    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(blackhawk7_v1l8p2_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        for (; micro_num<blackhawk7_v1l8p2_info_ptr->micro_count; ++micro_num)
        {
            ...
            EFUN(blackhawk7_v1l8p2_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, blackhawk7_v1l8p2_INTERNAL_event_log_dump_callback));
            EFUN(blackhawk7_v1l8p2_INTERNAL_event_log_dump_callback(sa__, &state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(blackhawk7_v1l8p2_read_event_log(sa__));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(blackhawk7_v1l8p2_display_eye_scan(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(blackhawk7_v1l8p2_reg_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(blackhawk7_v1l8p2_uc_core_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(blackhawk7_v1l8p2_uc_lane_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(blackhawk7_v1l8p2_display_lane_debug_status(sa__));
    }
...
} blackhawk7_v1l8p2_diag_access_callback_state_t;

static err_code_t blackhawk7_v1l8p2_diag_access_read_byte_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p2_diag_access_callback_state_t * const state_ptr = (blackhawk7_v1l8p2_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t blackhawk7_v1l8p2_diag_access_read_word_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p2_diag_access_callback_state_t * const state_ptr = (blackhawk7_v1l8p2_diag_access_callback_state_t *)arg;
    UNUSED(byte_count);
...
/* Required Diagnostic Functions */
err_code_t blackhawk7_v1l8p2_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    blackhawk7_v1l8p2_info_t * const blackhawk7_v1l8p2_info_ptr = blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr();
    blackhawk7_v1l8p2_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(blackhawk7_v1l8p2_pmd_rdt_reg(sa__, i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(blackhawk7_v1l8p2_pmd_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(blackhawk7_v1l8p2_pmd_wr_reg(sa__, addr,tmp2));
                             EFUN_PRINTF(("Register RMW: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK CORE RAM READ BYTE   ****"));
                                 EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           addr + blackhawk7_v1l8p2_info_ptr->core_var_ram_base,
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                                                           blackhawk7_v1l8p2_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,blackhawk7_v1l8p2_rdbc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK LANE RAM READ BYTE   ****"));
                                 EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + blackhawk7_v1l8p2_info_ptr->lane_var_ram_base + (blackhawk7_v1l8p2_get_physical_lane(sa__) * blackhawk7_v1l8p2_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,blackhawk7_v1l8p2_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           ((data+1)>>1)<<1, /* Round up to nearest word count */
                                                                           &state,
                                                                           blackhawk7_v1l8p2_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p2_rdwc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p2_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(blackhawk7_v1l8p2_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(blackhawk7_v1l8p2_wrbl_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(blackhawk7_v1l8p2_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(blackhawk7_v1l8p2_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,blackhawk7_v1l8p2_INTERNAL_rdb_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(blackhawk7_v1l8p2_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,blackhawk7_v1l8p2_INTERNAL_rdw_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = blackhawk7_v1l8p2_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(blackhawk7_v1l8p2_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(blackhawk7_v1l8p2_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
    ...
    default: EFUN_PRINTF(("Invalid request type blackhawk7_v1l8p2_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_display_state (srds_access_t *sa__) {
    EFUN(blackhawk7_v1l8p2_display_core_state(sa__));
    EFUN(blackhawk7_v1l8p2_display_lane_state_hdr(sa__));
    EFUN(blackhawk7_v1l8p2_display_lane_state(sa__));
    EFUN(blackhawk7_v1l8p2_display_lane_state_legend(sa__));
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_display_config (srds_access_t *sa__) {
    EFUN(blackhawk7_v1l8p2_display_core_config(sa__));
    EFUN(blackhawk7_v1l8p2_display_lane_config(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    /* disable force */ 
    if(die_temp == -255) {
...
err_code_t blackhawk7_v1l8p2_display_linktrn_status(srds_access_t *sa__) {
  EFUN_PRINTF(("\n\n***************************\n"                                                                                                                                    ));
