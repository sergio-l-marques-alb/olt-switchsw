/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_v1l8p2_internal.c                                         *
...
/** @file blackhawk7_v1l8p2_internal.c
...
uint32_t blackhawk7_v1l8p2_INTERNAL_mult_with_overflow_check(uint32_t a, uint32_t b, uint8_t *of) {
    uint16_t c,d;
    uint32_t r,s;
    if (a > b) return blackhawk7_v1l8p2_INTERNAL_mult_with_overflow_check(b, a, of);
    *of = 0;
    c = b >> 16;
    d = UINT16_MAX & b;
    r = a * c;
...
static blackhawk7_v1l8p2_info_t blackhawk7_v1l8p2_info;

blackhawk7_v1l8p2_info_t *blackhawk7_v1l8p2_INTERNAL_get_blackhawk7_v1l8p2_info_ptr(void) {
    return &blackhawk7_v1l8p2_info;
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_match_ucode_from_info(srds_access_t *sa__) {
    ...
    if (ucode_version == blackhawk7_v1l8p2_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored blackhawk7_v1l8p2_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    blackhawk7_v1l8p2_info_ptr->ucode_version, ucode_version));
        return(ERR_CODE_UCODE_VERIFY_FAIL);
    }
}

err_code_t blackhawk7_v1l8p2_INTERNAL_verify_blackhawk7_v1l8p2_info(srds_access_t *sa__, blackhawk7_v1l8p2_info_t const *test_info)
{
    err_code_t err_code = ERR_CODE_NONE;
    if (test_info->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in blackhawk7_v1l8p2_info signature.  Expected 0x%08X, but received 0x%08X.\n",
...
int blackhawk7_v1l8p2_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case BLACKHAWK7_V1L8P2_OSX1:      return 1000;
    case BLACKHAWK7_V1L8P2_OSX2:      return 2000;
    case BLACKHAWK7_V1L8P2_OSX4:      return 4000;
    case BLACKHAWK7_V1L8P2_OSX8:      return 8000;
    case BLACKHAWK7_V1L8P2_OSX16P5:   return 16500;
    case BLACKHAWK7_V1L8P2_OSX20P625: return 20625;
    case BLACKHAWK7_V1L8P2_OSX16:     return 16000;
    case BLACKHAWK7_V1L8P2_OSX32:     return 32000;
    case BLACKHAWK7_V1L8P2_OSX21P25:  return 21250;
    default:                    return 1000;
    }
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint32_t *num_bits_per_ms) {
    uint8_t osr_mode = 0;
    ...
    ESTM(prev_pll = blackhawk7_v1l8p2_get_pll_idx(sa__));
    ESTM(pll_select = rd_rx_pll_select());
    EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__,pll_select));
#endif
        EFUN(blackhawk7_v1l8p2_get_uc_core_config(sa__, &core_config));
        *num_bits_per_ms = (((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / blackhawk7_v1l8p2_osr_mode_enum_to_int_x1000(osr_mode))*10;

#if defined(rd_rx_pam4_mode)
    if(pam4_mode > 0) *num_bits_per_ms <<= 1; 
#endif
    EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__,prev_pll));
    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_v1l8p2_INTERNAL_display_BER(srds_access_t *sa__, uint16_t time_ms) {
    char string[32];
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_BER_string(sa__,time_ms,string));
    USR_PRINTF(("%s",string));
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct ber_data_st *ber_data) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    EFUN(blackhawk7_v1l8p2_prbs_err_count_state(sa__, &err_cnt,&lcklost)); /* clear error counters */
    ...
    if(time_ms <= 448) {
        ...
        /* Assumption: if lock is not lost in 10ms, we will use lock lost status for the entire time_ms */
        if(ber_data->lcklost == 0) {
            uint32_t num_bits_per_ms=0;
            uint16_t time_ms_adjusted = 0;
            struct prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak = {0,0,0};

            /* STEP 1: save prbs hardware timer config registers */
            EFUN(blackhawk7_v1l8p2_get_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));

            /* STEP 2: Configure hardware timer to count prbs errors */
            EFUN(blackhawk7_v1l8p2_config_prbs_chk_hw_timer(sa__, time_ms, &time_ms_adjusted));

            /* STEP 3: toggle checker enable to start hardware timers and error counters */
            EFUN(blackhawk7_v1l8p2_prbs_chk_en_toggle(sa__));

            /* STEP 4: wait time_ms + 10ms for hardware timeout */
            EFUN(USR_DELAY_MS(time_ms_adjusted+10));
            EFUN(blackhawk7_v1l8p2_prbs_err_count_state(sa__, &err_cnt,&lcklost));

            /* STEP 5: restore prbs hardware timer config registers */
            EFUN(blackhawk7_v1l8p2_set_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));

            /* STEP 6: Update the BER data structure */
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_prbs_timeout_count_from_time(srds_access_t *sa__, uint16_t time_ms, uint16_t * time_ms_adjusted, uint8_t * timeout_value) {
    uint8_t iter = 0;
...
int16_t blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(srds_access_t *sa__, int8_t ctrl, uint8_t range_250) {
...
uint8_t blackhawk7_v1l8p2_INTERNAL_stop_micro(srds_access_t *sa__, uint8_t graceful, err_code_t *err_code_p) {
   ...
   if (!(stop_state & 0x7F)) {
       if (graceful) {
           EPFUN2((blackhawk7_v1l8p2_stop_rx_adaptation(sa__, 1)),0xFF);
       }
       else {
           EPFUN2((blackhawk7_v1l8p2_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_IMMEDIATE,GRACEFUL_STOP_TIME)),0xFF);
       }
   }
...
uint8_t blackhawk7_v1l8p2_INTERNAL_rdb_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;

    if(!err_code_p) {
        return(0);
    }
    EPSTM(rddata = blackhawk7_v1l8p2_rdb_uc_ram(sa__, err_code_p, addr)); /* Use Micro register interface for reading RAM */
    return (rddata);
}

/* Micro Global RAM Word Read */
uint16_t blackhawk7_v1l8p2_INTERNAL_rdw_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = blackhawk7_v1l8p2_rdw_uc_ram(sa__, err_code_p, (addr))); /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro Global RAM Byte Write  */
err_code_t blackhawk7_v1l8p2_INTERNAL_wrb_uc_var(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {

    return (blackhawk7_v1l8p2_wrb_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}

/* Micro Global RAM Word Write  */
err_code_t blackhawk7_v1l8p2_INTERNAL_wrw_uc_var(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                       /* Validate even address */
      USR_PRINTF(("Error incorrect addr x%04x\n",addr));
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    return (blackhawk7_v1l8p2_wrw_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_event_log_dump_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p2_INTERNAL_event_log_dump_state_t * const state_ptr = (blackhawk7_v1l8p2_INTERNAL_event_log_dump_state_t *)arg;
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
    ...
    if (micro_num >= blackhawk7_v1l8p2_info_ptr->micro_count) {
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    /* Read Current lane so that it can be restored at the end of function */
    current_lane = blackhawk7_v1l8p2_get_lane(sa__);
    /* Blackhawk has 2 lanes per micro, set lane appropriate for the desired micro */
    EFUN(blackhawk7_v1l8p2_set_lane(sa__,micro_num<<1));
    EFUN_PRINTF(("\n\n********************************************\n"));
    ...
    if (bypass_micro) {
        ...
        if (blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes) {
            ++rd_idx;
            if (rd_idx >= blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size) {
                rd_idx = 0;
            }
        } else {
            if (rd_idx == 0) {
                rd_idx = blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size;
            }
            --rd_idx;
        }
    } else {
        /* Start Read to stop uC logging and read the word at last event written by uC */
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, GRACEFUL_STOP_TIME));
        ESTM(rd_idx = rducv_trace_mem_rd_idx());
    }
    
    EFUN_PRINTF(( "\n  DEBUG INFO: trace memory read index = 0x%04x\n", rd_idx));
    
    EFUN_PRINTF(("  DEBUG INFO: trace memory size = 0x%04x\n\n", blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size));
    ...
    if (blackhawk7_v1l8p2_info_ptr->trace_memory_descending_writes) {
        ...
        EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  blackhawk7_v1l8p2_info_ptr->trace_mem_ram_base + blackhawk7_v1l8p2_info_ptr->grp_ram_size*micro_num,
                                                  blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
                                                  arg,
         ...
         * So read using ascending addresses using block read
         */
        EFUN(blackhawk7_v1l8p2_INTERNAL_rdblk_uc_generic_ram_descending(sa__,
                                                             blackhawk7_v1l8p2_info_ptr->trace_mem_ram_base + blackhawk7_v1l8p2_info_ptr->grp_ram_size*micro_num,
                                                             blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
                                                             rd_idx,
                                                             blackhawk7_v1l8p2_info_ptr->trace_mem_ram_size,
    ...
    if (!bypass_micro) {
        ...
        EFUN(blackhawk7_v1l8p2_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 50));
    }
    EFUN(blackhawk7_v1l8p2_set_lane(sa__,current_lane));
    return(ERR_CODE_NONE);
}
...
uint32_t blackhawk7_v1l8p2_INTERNAL_float8_to_int32(float8_t input) {
    uint32_t x;
    if(input == 0) return(0);
    x = (input>>5) + 8;
    if((input & 0x1F) < 3) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    ...
    EFUN(wr_rx_pf_ctrl_val(val));
    return(ERR_CODE_NONE); 
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_pf_main(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)rd_rx_pf_ctrl_val());
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_pf2(srds_access_t *sa__, uint8_t val) {
    ...
    EFUN(wr_rx_pf2_ctrl_val(val));
    return(ERR_CODE_NONE); 
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_pf2(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)rd_rx_pf2_ctrl_val());
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_pf3(srds_access_t *sa__, uint8_t val) {
    ...
    EFUN(wr_ams_rx_eq2_ctrl(val));
    /* EFUN(wr_ams_rx_pf3_ctrl(blackhawk7_v1l8p2_INTERNAL_uint8_to_gray(val))); */
    return(ERR_CODE_NONE); 
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_pf3(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)rd_ams_rx_eq2_ctrl());
    /* ESTM(*val = (int8_t)blackhawk7_v1l8p2_INTERNAL_gray_to_uint8(rd_ams_rx_pf3_ctrl())); */
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_vga(srds_access_t *sa__, int8_t *val) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe3(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = ((rd_rxa_dfe_tap3_status() + rd_rxb_dfe_tap3_status())>>1));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_rx_dfe4(srds_access_t *sa__, int8_t val) {
    ...
    EFUN(wr_rx_afe_override_sel(7));
    EFUN(wr_rxa_dfe_tap4_val(val));
    EFUN(wr_rxa_dfe_tap4_write(1));
    EFUN(wr_rxb_dfe_tap4_val(val));
    EFUN(wr_rxb_dfe_tap4_write(1));
    return(ERR_CODE_NONE); 
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe4(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = ((rd_rxa_dfe_tap4_status() + rd_rxb_dfe_tap4_status())>>1));   
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe5(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap5() + rd_rxb_dfe_tap5())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe6(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap6() + rd_rxb_dfe_tap6())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe7(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap7() + rd_rxb_dfe_tap7())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe8(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap8() + rd_rxb_dfe_tap8())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe9(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap9() + rd_rxb_dfe_tap9())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe10(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap10() + rd_rxb_dfe_tap10())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe11(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap11() + rd_rxb_dfe_tap11())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe12(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap12() + rd_rxb_dfe_tap12())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe13(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap13() + rd_rxb_dfe_tap13())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_rx_dfe14(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (rd_rxa_dfe_tap14() + rd_rxb_dfe_tap14())>>1);
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_v1l8p2_INTERNAL_apply_full_txfir_cfg(srds_access_t *sa__, enum blackhawk7_v1l8p2_txfir_tap_enable_enum enable_taps, blackhawk7_v1l8p2_txfir_st txfir) {
    ...
    failcode = blackhawk7_v1l8p2_INTERNAL_validate_full_txfir_cfg(sa__, enable_taps, txfir);
    if (!failcode) {
        for (tap_num=0; tap_num<6; tap_num++) {
            failcode |= blackhawk7_v1l8p2_INTERNAL_set_tx_tap(sa__, tap_num, txfir.tap[tap_num]);
        }
        if ((enable_taps == NRZ_6TAP) || (enable_taps == PAM4_6TAP)) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_eye_margin_est(srds_access_t *sa__, uint16_t *left_eye_mUI, uint16_t *right_eye_mUI, uint16_t  *upper_eye_mV, uint16_t *lower_eye_mV) {
  ...
  ESTM(*left_eye_mUI  = blackhawk7_v1l8p2_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(*right_eye_mUI = blackhawk7_v1l8p2_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(*upper_eye_mV = blackhawk7_v1l8p2_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = blackhawk7_v1l8p2_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));
...
{       
    ...
    uint16_t vl = blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, var, ladder_range);    
    return (vl);
}

err_code_t blackhawk7_v1l8p2_INTERNAL_get_osr_mode(srds_access_t *sa__, blackhawk7_v1l8p2_osr_mode_st *imode) {
    blackhawk7_v1l8p2_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(blackhawk7_v1l8p2_osr_mode_st));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_lane_state(srds_access_t *sa__, blackhawk7_v1l8p2_lane_state_st *istate) {  
  ...
  blackhawk7_v1l8p2_lane_state_st state;

  ENULL_MEMSET(&state, 0, sizeof(blackhawk7_v1l8p2_lane_state_st));

  if(!istate) 
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(blackhawk7_v1l8p2_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      err_code_t err_code = 0;
      state.stop_state = blackhawk7_v1l8p2_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_sigdet_status(sa__,&state.sig_det, &state.sig_det_chg));
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_decode_br_os_mode(sa__, &state.br_pd_en));
  /* drop the MSB, the result is actually valid modulo 128 */
  /* Also flip the sign to account for d-m1, versus m1-d */

  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pf_main(sa__, &state.pf_main));
  state.pf_hiz = 0;
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pf2(sa__, &state.pf2_ctrl));
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_pf3(sa__, &state.pf3_ctrl));
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_vga(sa__, &state.vga));
  ESTM(state.dc_offset = rd_dc_offset_bin());
  ESTM(state.p1_lvl = rdv_usr_main_tap_est()/32);
  state.p1_lvl = blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)state.p1_lvl, 0);
  if (state.rx_pam_mode == NRZ) {
      ESTM(state.m1_lvl = rd_rx_phase02_status());
      state.m1_lvl = blackhawk7_v1l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)state.m1_lvl, 0);
  ...
  if (state.rx_pam_mode == NRZ) {
      EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe1(sa__, &state.dfe1));
  }
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe2(sa__, &state.dfe2));  
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe3(sa__, &state.dfe3));  
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe4(sa__, &state.dfe4));
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe5(sa__, &state.dfe5));

  EFUN(blackhawk7_v1l8p2_INTERNAL_get_rx_dfe6(sa__, &state.dfe6));
  ...
  ESTM(state.txfir_use_pam4_range = rd_txfir_nrz_tap_range_sel() ? 0 : 1);
  {
      ...
      for (tap_num=0; tap_num<6; ++tap_num) {
          EFUN(blackhawk7_v1l8p2_INTERNAL_get_tx_tap(sa__, tap_num, &state.txfir.tap[tap_num]));
      }
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
  ...
  ESTM(state.link_time = (((uint32_t)rdv_usr_sts_link_time())*8)/10);     /* convert from 80us to 0.1 ms units */
  {
      ...
      EFUN(blackhawk7_v1l8p2_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {     
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_read_lane_state(sa__, &state));
 
  lane_idx = blackhawk7_v1l8p2_get_lane(sa__); 
  ...
  if(state.osr_mode.tx_rx != 255) {
      ...
      r = (char *)e2s_tx_osr_mode_enum[state.osr_mode.rx];
      {
          char *t;
          uint8_t tx, rx;
          t = (char *)blackhawk7_v1l8p2_e2s_rx_pam_mode_enum[state.rx_pam_mode];
          ESTM(tx = rd_tx_pam4_mode()); 
          ESTM(rx = rd_rx_pam4_mode());
          EFUN_PRINTF(("(%3s%s,%2s%s:%s, 0x%04x, 0x%02x , 0x%04x,", t,(tx == rx) ? " ": "~", (state.br_pd_en) ? "BR" : "OS", s,r, state.ucv_config,state.ucv_status,state.ucv_status_ext));
      }
  }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_read_core_state(srds_access_t *sa__, blackhawk7_v1l8p2_core_state_st *istate) {
  blackhawk7_v1l8p2_core_state_st state;
  struct blackhawk7_v1l8p2_uc_core_config_st core_cfg = UC_CORE_CONFIG_INIT;

  ENULL_MEMSET(&state, 0, sizeof(blackhawk7_v1l8p2_core_state_st));
  ...
  ESTM(blackhawk7_v1l8p2_version(sa__, &state.api_version));
  ESTM(state.afe_hardware_version = rdcv_afe_hardware_version());
  ESTM(state.temp_idx             = rdcv_temp_idx());
  {  int16_t                           die_temp = 0;
     EFUN(blackhawk7_v1l8p2_read_die_temperature(sa__, &die_temp));
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_read_pll_div(sa__, &state.pll_div));
  EFUN(blackhawk7_v1l8p2_INTERNAL_pll_lock_status(sa__, &state.pll_lock, &state.pll_lock_chg));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_display_core_state_no_newline(srds_access_t *sa__) {
  blackhawk7_v1l8p2_core_state_st state;
    blackhawk7_v1l8p2_core_state_st state_pll1;
  ...
  EFUN(blackhawk7_v1l8p2_INTERNAL_read_core_state(sa__, &state));
        EFUN(blackhawk7_v1l8p2_set_pll_idx(sa__, 1));
        EFUN(blackhawk7_v1l8p2_INTERNAL_read_core_state(sa__, &state_pll1));
    ...
    EFUN_PRINTF((" %02d "              ,  blackhawk7_v1l8p2_get_core(sa__)));
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_display_pll_to_divider(sa__, state .pll_div));
    EFUN_PRINTF((","));
    EFUN(blackhawk7_v1l8p2_INTERNAL_display_pll_to_divider(sa__, state_pll1.pll_div));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_update_uc_lane_config_st(struct  blackhawk7_v1l8p2_uc_lane_config_st *st) {
  ...
  st->field.force_er = in & BFMASK(1); in >>= 1;
  st->field.force_nr = in & BFMASK(1); in >>= 1;
...
err_code_t blackhawk7_v1l8p2_INTERNAL_update_uc_lane_config_word(struct blackhawk7_v1l8p2_uc_lane_config_st *st) {
  ...
  in <<= 1; in |= st->field.force_nr & BFMASK(1);
  in <<= 1; in |= st->field.force_er & BFMASK(1);
...
err_code_t blackhawk7_v1l8p2_INTERNAL_set_uc_core_config(srds_access_t *sa__, struct blackhawk7_v1l8p2_uc_core_config_st struct_val) {
    {   uint8_t reset_state;
        ...
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: blackhawk7_v1l8p2_INTERNAL_set_uc_core_config(..) called without core_dp_s_rstb=0\n"));
    ...
    EFUN(blackhawk7_v1l8p2_INTERNAL_update_uc_core_config_word(&struct_val));
...
err_code_t blackhawk7_v1l8p2_INTERNAL_get_refclk_in_hz(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_100MHZ:         *refclk_in_hz = 100000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_106P25MHZ:      *refclk_in_hz = 106250000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_122P88MHZ:      *refclk_in_hz = 122880000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_125MHZ:         *refclk_in_hz = 125000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_145P947802MHZ:  *refclk_in_hz = 145947802; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_155P52MHZ:      *refclk_in_hz = 155520000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_156P25MHZ:      *refclk_in_hz = 156250000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_159P375MHZ:     *refclk_in_hz = 159375000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_161P1328125MHZ: *refclk_in_hz = 161132813; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_166P67MHZ:      *refclk_in_hz = 166670000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_166P7974882MHZ: *refclk_in_hz = 166797488; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_167P38MHZ:      *refclk_in_hz = 167380000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_167P4107143MHZ: *refclk_in_hz = 167410714; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_174P703125MHZ:  *refclk_in_hz = 174703125; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_212P5MHZ:       *refclk_in_hz = 212500000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_311p04:         *refclk_in_hz = 311040000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_312P5MHZ:       *refclk_in_hz = 312500000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_425MHZ:         *refclk_in_hz = 425000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_534MHZ:         *refclk_in_hz = 534000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_705MHZ:         *refclk_in_hz = 705000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_175MHZ:         *refclk_in_hz = 175000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_400MHZ:         *refclk_in_hz = 400000000; break;
        case BLACKHAWK7_V1L8P2_PLL_REFCLK_266P67MHZ:      *refclk_in_hz = 266670000; break;
        default:
 ...
 */
static err_code_t _blackhawk7_v1l8p2_get_divisor_ratio(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_div_enum div, uint16_t *numerator, uint16_t *denominator)
{
    switch (div) {
        case BLACKHAWK7_V1L8P2_PLL_DIV_66:         *denominator =  33;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_67:         *denominator =  67;  *numerator = 1000; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_70:         *denominator =   7;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_72:         *denominator =   9;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_76:         *denominator =  19;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_73P6:       *denominator =  46;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_79P2:       *denominator =  99;  *numerator = 1250; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_80:         *denominator =   2;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_82P5:       *denominator =  33;  *numerator =  400; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_84:         *denominator =  21;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_85:         *denominator =  17;  *numerator =  200; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_87P5:       *denominator =   7;  *numerator =   80; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_89P6:       *denominator =  56;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_90:         *denominator =   9;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_96:         *denominator =  12;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_100:        *denominator =   1;  *numerator =   10; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_120:        *denominator =   3;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_127P401984: *denominator = 179;  *numerator = 1405; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_128:        *denominator =  16;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_132:        *denominator =  33;  *numerator =  250; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_136:        *denominator =  17;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_140:        *denominator =   7;  *numerator =   50; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_144:        *denominator =  18;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_147P2:      *denominator =  92;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_152:        *denominator =  19;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_158P4:      *denominator =  99;  *numerator =  625; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_160:        *denominator =   4;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_165:        *denominator =  33;  *numerator =  200; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_168:        *denominator =  21;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_170:        *denominator =  17;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_175:        *denominator =   7;  *numerator =   40; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_180:        *denominator =   9;  *numerator =   50; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_184:        *denominator =  23;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_192:        *denominator =  24;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_198:        *denominator =  99;  *numerator =  500; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_200:        *denominator =   1;  *numerator =    5; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_224:        *denominator =  28;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_240:        *denominator =   6;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_264:        *denominator =  33;  *numerator =  125; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_280:        *denominator =   7;  *numerator =   25; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_330:        *denominator =  33;  *numerator =  100; break;
        case BLACKHAWK7_V1L8P2_PLL_DIV_350:        *denominator =   7;  *numerator =   20; break;
        default:
...
/** Get the Refclk frequency in Hz, based on the blackhawk7_v1l8p2_pll_div_enum value and VCO frequency. */
static err_code_t _blackhawk7_v1l8p2_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum blackhawk7_v1l8p2_pll_div_enum div, uint32_t vco_freq_khz, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk7_v1l8p2_get_divisor_ratio(sa__, div, &numerator, &denominator));
    *refclk_freq_hz = ((vco_freq_khz+(denominator>>1)) / denominator) * numerator;
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DOUBLER_EN) *refclk_freq_hz /= 2;
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN)    *refclk_freq_hz *= 2;
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV4_EN)    *refclk_freq_hz *= 4;
    return (ERR_CODE_NONE);
}

/** Get the VCO frequency in kHz, based on the reference clock frequency and blackhawk7_v1l8p2_pll_div_enum value. */
err_code_t blackhawk7_v1l8p2_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum blackhawk7_v1l8p2_pll_div_enum div, uint32_t *vco_freq_khz, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_blackhawk7_v1l8p2_get_divisor_ratio(sa__, div, &numerator, &denominator));
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static const enum blackhawk7_v1l8p2_pll_div_enum _blackhawk7_v1l8p2_div_candidates[] = {
    BLACKHAWK7_V1L8P2_PLL_DIV_66,
    BLACKHAWK7_V1L8P2_PLL_DIV_67,
    BLACKHAWK7_V1L8P2_PLL_DIV_70,
    BLACKHAWK7_V1L8P2_PLL_DIV_72,
    BLACKHAWK7_V1L8P2_PLL_DIV_76,
    BLACKHAWK7_V1L8P2_PLL_DIV_73P6,
    BLACKHAWK7_V1L8P2_PLL_DIV_79P2,
    BLACKHAWK7_V1L8P2_PLL_DIV_80,
    BLACKHAWK7_V1L8P2_PLL_DIV_82P5,
    BLACKHAWK7_V1L8P2_PLL_DIV_84,
    BLACKHAWK7_V1L8P2_PLL_DIV_85,
    BLACKHAWK7_V1L8P2_PLL_DIV_87P5,
    BLACKHAWK7_V1L8P2_PLL_DIV_89P6,
    BLACKHAWK7_V1L8P2_PLL_DIV_90,
    BLACKHAWK7_V1L8P2_PLL_DIV_96,
    BLACKHAWK7_V1L8P2_PLL_DIV_100,
    BLACKHAWK7_V1L8P2_PLL_DIV_120,
    BLACKHAWK7_V1L8P2_PLL_DIV_127P401984,
    BLACKHAWK7_V1L8P2_PLL_DIV_128,
    BLACKHAWK7_V1L8P2_PLL_DIV_132,
    BLACKHAWK7_V1L8P2_PLL_DIV_136,
    BLACKHAWK7_V1L8P2_PLL_DIV_140,
    BLACKHAWK7_V1L8P2_PLL_DIV_144,
    BLACKHAWK7_V1L8P2_PLL_DIV_147P2,
    BLACKHAWK7_V1L8P2_PLL_DIV_152,
    BLACKHAWK7_V1L8P2_PLL_DIV_158P4,
    BLACKHAWK7_V1L8P2_PLL_DIV_160,
    BLACKHAWK7_V1L8P2_PLL_DIV_165,
    BLACKHAWK7_V1L8P2_PLL_DIV_168,
    BLACKHAWK7_V1L8P2_PLL_DIV_170,
    BLACKHAWK7_V1L8P2_PLL_DIV_175,
    BLACKHAWK7_V1L8P2_PLL_DIV_180,
    BLACKHAWK7_V1L8P2_PLL_DIV_184,
    BLACKHAWK7_V1L8P2_PLL_DIV_192,
    BLACKHAWK7_V1L8P2_PLL_DIV_198,
    BLACKHAWK7_V1L8P2_PLL_DIV_200,
    BLACKHAWK7_V1L8P2_PLL_DIV_224,
    BLACKHAWK7_V1L8P2_PLL_DIV_240,
    BLACKHAWK7_V1L8P2_PLL_DIV_264,
    BLACKHAWK7_V1L8P2_PLL_DIV_280,
    BLACKHAWK7_V1L8P2_PLL_DIV_330,
    BLACKHAWK7_V1L8P2_PLL_DIV_350
};

static const uint8_t _blackhawk7_v1l8p2_div_candidates_count = sizeof(_blackhawk7_v1l8p2_div_candidates) / sizeof(_blackhawk7_v1l8p2_div_candidates[0]);
static err_code_t _blackhawk7_v1l8p2_check_div(srds_access_t *sa__, enum blackhawk7_v1l8p2_pll_div_enum div) {
    uint8_t i, found = 0;
    for (i=0; i<_blackhawk7_v1l8p2_div_candidates_count; i++) {
        found |= div == _blackhawk7_v1l8p2_div_candidates[i];
    }
    if (!found) {
...
static err_code_t _blackhawk7_v1l8p2_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
...
/** Find the entry out of _blackhawk7_v1l8p2_div_candidates that is closest to matching refclk_freq_hz and vco_freq_khz. */
static err_code_t _blackhawk7_v1l8p2_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum blackhawk7_v1l8p2_pll_div_enum *div, enum blackhawk7_v1l8p2_pll_option_enum pll_option) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_blackhawk7_v1l8p2_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_vco_from_refclk_div(sa__, refclk_freq_hz, _blackhawk7_v1l8p2_div_candidates[i], &actual_vco_freq_khz, pll_option));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *div = _blackhawk7_v1l8p2_div_candidates[i];
        }
    }
...
err_code_t blackhawk7_v1l8p2_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    const uint8_t given_param_count = (((refclk == BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*div == BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    const uint32_t original_vco_freq_khz = *vco_freq_khz;
    enum blackhawk7_v1l8p2_pll_div_enum auto_div = BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN;
    ...
    if (pll_option == BLACKHAWK7_V1L8P2_PLL_OPTION_DISABLE_VERIFY) {
    ...
    if (*div != BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN) {
        EFUN(_blackhawk7_v1l8p2_check_div(sa__, *div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_blackhawk7_v1l8p2_check_vco_freq_khz(sa__,*vco_freq_khz));
    }

    if (refclk == BLACKHAWK7_V1L8P2_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_blackhawk7_v1l8p2_get_refclk_from_div_vco(sa__, refclk_freq_hz, *div, *vco_freq_khz, pll_option));
    } else {
        EFUN(blackhawk7_v1l8p2_INTERNAL_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
     ...
     */
    EFUN(_blackhawk7_v1l8p2_get_div(sa__, *refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*div == BLACKHAWK7_V1L8P2_PLL_DIV_UNKNOWN) {
...
err_code_t blackhawk7_v1l8p2_INTERNAL_display_pll_to_divider(srds_access_t *sa__, uint32_t div) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                EFUN(blackhawk7_v1l8p2_INTERNAL_print_uc_dsc_error(sa__, cmd));
...
int8_t blackhawk7_v1l8p2_INTERNAL_afe_slicer_offset_mapping(int8_t x) {
    if (x>=0)
        return(x^((x&2)>>1));   /* XOR bit 0 with bit 1 */
    else
        return(-blackhawk7_v1l8p2_INTERNAL_afe_slicer_offset_mapping(-x));
}

err_code_t blackhawk7_v1l8p2_INTERNAL_rdblk_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    blackhawk7_v1l8p2_INTERNAL_rdblk_callback_arg_t * const cast_arg = (blackhawk7_v1l8p2_INTERNAL_rdblk_callback_arg_t *)arg;
    UNUSED(sa__);
