 ...
 * \param [in] force_stop - see description in 
 *        periodic_event_stop
 */
 ...
 * \param [in] force_stop - in case thread dones't stop 
 *        gracefully:
 *        * if force_stop is 1 the thread will be killed by the 
 *        periodic_event_stop function
 *        * if force_speed is 0 the function will return error
 *        in case thread can't exit gracefully.
 * \param [in] dont_wait -  indicate the thread to stop the next time it is triggered,
 *                          don't wait for it to trigger.
 *                          * if dont_wait is 1 the thread will stop next time it is triggered. in this case force_stop is ignored.
 *                          * if dont_wait is 0 the thread will wait till it is triggered. in this case force_stop is relevant.
 */
