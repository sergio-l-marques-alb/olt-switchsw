...
/**
 * \brief Get ucode full path
 * \param [in] unit - BCM unit id
 * \param [in] filename name of the file, includes relative path
 * \param [out] filepath pointer to string, full file path
 * \return
 *   \retval _SHR_E_NONE if path was successfully found
 *   \retval _SHR_E_PARAM if there was some problem with parameters
 * \remarks
 */
shr_error_e dbx_file_get_ucode_location(
    int unit,
    char *filename,
    char *filepath);
