...
typedef struct
{
  ...
  /*
   *  From QAX, indicates how to build the mpls label using the data entry
   *  follow this entry.
   *  See bcm_mpls_special_label_push_type_t
   */
...
typedef struct
{
  ...
  uint32 outlif_profile;
  /*
   * Reserved bits(3 bits), used for label3[19:18,16] when
   * building mpls label with reserved MPLS profile on QAX.
   */
  uint32 rsv_bits;
...
typedef struct
{
  ...
  /*
   *  Ethernet-Type for custom_1
   */
  uint16 custom1_ether_type;
  /*
   *  Ethernet-Type for custom_2
   */
