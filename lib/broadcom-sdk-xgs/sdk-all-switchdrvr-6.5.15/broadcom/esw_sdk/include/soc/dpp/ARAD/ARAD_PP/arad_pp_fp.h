...
#define ARAD_PP_FP_DIRECT_TABLE_KAPS_KEY_LENGTH (SOC_IS_JERICHO(unit) ? SOC_DPP_JER_CONFIG(unit)->pp.kaps_large_db_bits : 0)
...
typedef struct
{
  /* size of the qualifier in bytes (must be always 2) */
  uint8 size_in_bytes;

  /* offset from the beginning of the key where the qualifier's range ID is located */
  uint8 offset;
