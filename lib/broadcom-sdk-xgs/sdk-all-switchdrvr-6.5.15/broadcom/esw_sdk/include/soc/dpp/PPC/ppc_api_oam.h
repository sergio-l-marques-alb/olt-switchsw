...
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MPLS_TP                    0x800000 /* Indicate it's 1731oLSP MEP*/
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_DOWN_MEP_INJECTION  0x1000000 /*in QAX - set if the vpn field in endpoint create is !=0*/
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_SEAMLESS_BFD_INITIATOR  0x2000000 /*in JERICHO and above, set if SBFD initiator endpoint */
#define SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_BFD_SEAMLESS_BFD_REFLECOTR  0x4000000 /*in JERICHO and above, set if SBFD reflector endpoint */
...
/* 
 * Flex IP TOS feature. flag to indicate
 * whether its a single hop or micro bfd
...
/* 
 * Seamless BFD flag. flag to indicate
 * SBFD initiator or reflector
 * if feature is enabled.
 */
...
typedef struct {
    ...
    uint8 flex_tos_s_hop_m_bfd_flag; /* jericho and above. 
                                      * 0 - multi-hop or flex iptos feature not set, 
                                      * 1 - single hop, 
                                      * 2 - Micro BFD */
    uint8 s_bfd_flag;/* jericho and above.
                      * 0 - none (classical bfd)
                      * 1 - sbfd_initiator
                      * 2 - sbfd_reflector */
    ...
    uint32 maid_48_index; /* index to the oamp_pe_gen_mem */
    ...
    uint8 is_lsp;    /* indicate if MEP type is LSP type */
...
typedef struct {
    ...
    int    Y1731_profile_index; /* Index of Y1731 profile in MEP DB entry*/
...
typedef struct {
    ...
    /* Encoding of number of bits used for 1DM seconds and nano seconds in QAX*/
...
typedef struct {
    ...
    int sbfd_non_acc_src_ip_add_ptr; /* SBFD non-acc endpoint's SRC IP address profile pointer */
