...
#define PORT_SW_DB_PORT_REMOVE(ptype, nport) \
            assert(SOC_INFO(unit).ptype.num < SOC_MAX_NUM_PORTS); \
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).ptype.bitmap, nport)) { /* del port from type bitmap only if already a memeber */ \
...
/*#define SOC_PORT_FLAGS_DISABLED                   0x10*/ /* can be used for a new flag */
