/** \file dnx_data_max_trunk.h
 * 
 * MODULE DATA MAX - 
 * Maintain max value (of all devices) per define
 * 
 * Device Data
 * SW component that maintains per device data
 * For additional details about Device Data Component goto 'dnxc_data_mgmt.h'
 *        
 *     
 * 
 * AUTO-GENERATED BY AUTOCODER!
 * DO NOT EDIT THIS FILE!
 */
/* *INDENT-OFF* */
/*
 * $Copyright: (c) 2018 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 */
#ifndef _DNX_DATA_MAX_TRUNK_H_
/*{*/
#define _DNX_DATA_MAX_TRUNK_H_
/*
 * INCLUDE FILES:
 * {
 */
/*
 * }
 */

/*!
* \brief This file is only used by DNX (JR2 family). Including it by
* software that is not specific to DNX is an error.
*/
#ifndef BCM_DNX_SUPPORT
#error "This file is for use by DNX (JR2) family only!"
#endif

/*
 * MAX DEFINES:
 * {
 */
/**
 * \brief
 * trunk module might be represented with pools in certain architectures, this number represents the number of pools currently supported for this device. each pool has a constant amount of members.
 */
#define DNX_DATA_MAX_TRUNK_PARAMETERS_NOF_POOLS (2)

/**
 * \brief
 * the maximal amount of members that a pool can contain.
 */
#define DNX_DATA_MAX_TRUNK_PARAMETERS_MAX_NOF_MEMBERS_IN_POOL (16384)

/**
 * \brief
 * the maximal amount of groups that a pool can contain.
 */
#define DNX_DATA_MAX_TRUNK_PARAMETERS_MAX_NOF_GROUPS_IN_POOL (8192)

/**
 * \brief
 * bit shifts to describe the pool of a trunk in a System Port Aggregate
 */
#define DNX_DATA_MAX_TRUNK_PARAMETERS_SPA_POOL_SHIFT (14)

/**
 * \brief
 * bit shifts to describe trunk type in a System Port Aggregate
 */
#define DNX_DATA_MAX_TRUNK_PARAMETERS_SPA_TYPE_SHIFT (15)

/**
 * \brief
 * size in bits of small groups in c-lag
 */
#define DNX_DATA_MAX_TRUNK_PSC_CONSISTANT_HASHING_SMALL_GROUP_SIZE_IN_BITS (4)

/**
 * \brief
 * max number of member is trunk group with smooth division as PSC
 */
#define DNX_DATA_MAX_TRUNK_PSC_SMOOTH_DIVISION_MAX_NOF_MEMBER (16)

/**
 * \brief
 * logical number of entries dedicated in the HW per SMD profile
 */
#define DNX_DATA_MAX_TRUNK_PSC_SMOOTH_DIVISION_ENTRIES_PER_PROFILE (256)

/**
 * \brief
 * number of egress trunks
 */
#define DNX_DATA_MAX_TRUNK_EGRESS_TRUNK_NOF (64)

/**
 * \brief
 * number of lb keys per Egress trunk
 */
#define DNX_DATA_MAX_TRUNK_EGRESS_TRUNK_NOF_LB_KEYS_PER_PROFILE (256)

/**
 * \brief
 * invalid out port for egress trunk
 */
#define DNX_DATA_MAX_TRUNK_EGRESS_TRUNK_INVALID_OUT_PORT (255)

/**
 * \brief
 * hw value to indicate first configuration is currently used
 */
#define DNX_DATA_MAX_TRUNK_GRACEFUL_FIRST_HW_CONFIGURATION (0)

/**
 * \brief
 * hw value to indicate second configuration is currently used
 */
#define DNX_DATA_MAX_TRUNK_GRACEFUL_SECOND_HW_CONFIGURATION (1)

/*
 * }
 */

/*}*/
#endif /*_DNX_DATA_MAX_TRUNK_H_*/
/* *INDENT-ON* */
