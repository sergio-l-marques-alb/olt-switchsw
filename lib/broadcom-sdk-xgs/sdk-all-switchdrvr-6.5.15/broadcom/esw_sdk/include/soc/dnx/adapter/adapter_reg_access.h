...
/** Maximum packet size in adapter */
 ...
 *   \param [out] signal_data - Allocated buffer for adapter signal data
...
/**
 * \brief - Start or stop the hit indication record in accordance with the value of the 'is_start' parameter.
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit id
 *   \param [in] is_start - A parameter for start/stop the record of the hit indications. '1' means start and '0' means
 *          stop.
 *
 * \par INDIRECT INPUT:
 *   * None
 ...
 *   * None
 */
int adapter_set_mem_access_recording(
    int unit,
    int is_start);
 ...
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit id
 *   \param [in] block_names - Allocated buffer for adapter block names
 *
 * \par INDIRECT INPUT:
 *   * None
 * \par DIRECT OUTPUT:
