...
#ifndef __DNX_SAND_USER_CALLBACK_H_INCLUDED__
/* { */
#define __DNX_SAND_USER_CALLBACK_H_INCLUDED__
 ...
 *      Buffer is of size DNX_SAND_CALLBACK_BUF_SIZE and is replaced by
 ...
 *      DNX_SAND_CALLBACK_BUF_SIZE, including terminating null).
 *    Fourth parameter - uint32 -
 *      Size of meaningful results, loaded into buffer pointed by second
 *      parameter. Must be smaller than maximal size (DNX_SAND_CALLBACK_BUF_SIZE).
 *    Fifth parameter - uint32 -
 *      Error return value. See formatting rules in ERROR RETURN VALUES.
 *      If error code is not DNX_SAND_OK then this parameter indicates
 ...
 *      Meaning-less if not other specified.
 */
typedef int (*DNX_SAND_USER_CALLBACK_FUNC)(uint32, uint32 *,uint32 **,
  uint32,
  uint32, uint32,
  uint32, uint32, uint32) ;
...
typedef struct
{
   ...
   */
  DNX_SAND_USER_CALLBACK_FUNC callback_func;
   ...
   * parameter.
   * Example: In function 'dnx_sand_read_mem'
   * param1 is 'offset'
   * param2 is 'size'
   * param3 is 'indirect'
   */
   ...
   *   is required.
   *   See 'system_tick_in_ms' in 'dnx_sand_module_open'.
   *   This parameter may not be smaller than
   *   DNX_SAND_MIN_TCM_ACTIVATION_PERIOD (apart from
   *   being set to '0')
   ...
   *   See 'system_tick_in_ms' in 'dnx_sand_module_open'.
   *   This parameter may not be smaller than
   *   DNX_SAND_MIN_TCM_ACTIVATION_PERIOD.
   */
...
} DNX_SAND_USER_CALLBACK;

#ifdef  __cplusplus
}
#endif

/* } __DNX_SAND_USER_CALLBACK_H_INCLUDED__*/
#endif
