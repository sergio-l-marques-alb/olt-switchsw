 ...
 * searched and the master key to get back for the device/instruction search. This wont do
 * search.
 *
 * Note: User has to pass array of 80Byte (640b master key), where the valid database key
 * is filled in the master key and user can use it for the search.
 *
 * for parallel search same master_key to be passed to function, so that only required
...
/**
 * This API allows user to set the LUT opcode for the instruction. And it should be called after instruction install.
