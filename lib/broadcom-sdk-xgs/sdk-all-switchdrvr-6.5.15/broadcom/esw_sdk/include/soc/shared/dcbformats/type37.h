 ...
 *
 * File:        soc/shared/dcbformats/type37.h
 * Purpose:     Define dma control block (DCB) format for a type37 DCB
 *              used by the 53570 (Greyhound2)
 *
 *              This file is shared between the SDK and the embedded applications.
 */

#ifndef _SOC_SHARED_DCBFORMATS_TYPE37_H
#define _SOC_SHARED_DCBFORMATS_TYPE37_H

/*
 * DMA Control Block - Type 37
 * Used on 53570 devices
 * 16 words
 */
typedef struct {
        uint32  addr;                   /* T37.0: physical address */
                                        /* T37.1: Control 0 */
#ifdef  LE_HOST
...
#endif  /* LE_HOST */
        uint32  reason_hi;              /* T37.2: CPU opcode (hi bits) */
        uint32  reason;                 /* T37.3: CPU opcode (low bits) */
#ifdef  LE_HOST
    union {                             /* T37.4 */
        struct {
            uint32  :16,                /* Reserved */
    ...
    union {                             /* T37.4 */
        ...
        uint32  mh0;                    /* T37.6: Module Header word 0 */
        uint32  mh1;                    /* T37.7: Module Header word 1 */
        uint32  mh2;                    /* T37.8: Module Header word 2 */
        uint32  mh3;                    /* T37.9: Module Header word 3 */
#ifdef  LE_HOST
                                        /* T37.10 */
