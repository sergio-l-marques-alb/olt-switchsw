...
extern int soc_hx5_l2_bulk_age_start(int unit, int interval);
extern int soc_hx5_l2_bulk_age_stop(int unit);
...
extern uint32   soc_hx5_exact_match_bank_entry_hash(int unit, int bank,
                                                   uint32 *entry);
extern uint32   soc_hx5_mpls_hash(int unit, int bank, int hash_offset, int use_lsb,
                                  int key_nbits, void *base_entry, uint8 *key1, uint8 *key2);
extern int      soc_hx5_mpls_base_entry_to_key(int unit, int bank, void *entry,
                                              uint8 *key);
extern uint32   soc_hx5_mpls_entry_hash(int unit, int bank, int hash_offset,
                                        int use_lsb, uint32 *entry);
extern uint32   soc_hx5_mpls_bank_entry_hash(int unit, int bank, uint32 *entry);
...
#define SOC_HX5_STAT_ING_FLEX_POOL_MAX   20

/* Num of evict memories is 26, 1 is added for reserved mem */
#define SOC_HX5_NUM_EVICT_MEM            27

/* SUBPORT */
#define HX5_SUBPORT_COE_GROUP_MAX             (512)
#define HX5_SUBPORT_COE_PORT_MAX              (512)
#define HX5_MAX_SUBPORT_COE_PORT_PER_CASPORT  (80)
#define HX5_MAX_SUBPORT_COE_PORT_PER_MOD      (128)
#define HX5_MAX_COE_MODULES                   (5)
...
extern void soc_hx5_subport_init(int unit);

extern void soc_helix5_port_num_adjust(int unit, soc_block_type_t blktype, uint32* phy_port);
