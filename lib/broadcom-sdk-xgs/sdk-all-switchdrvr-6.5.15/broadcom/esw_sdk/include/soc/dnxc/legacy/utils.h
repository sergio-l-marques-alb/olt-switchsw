...
#define DNX_ALLOC_ANY_SIZE(var, type, count,str)                       \
  do {                                                                    \
    if(var != NULL)                                                       \
    {                                                                     \
      DNX_EXIT_WITH_ERR(_SHR_E_PARAM, (_DNX_MSG("Trying to allocate to a non null ptr is forbidden"))); \
    }                                                                     \
    var = (type*)soc_sand_os_malloc_any_size((count) * (uint32)sizeof(type),str); \
    if (var == NULL)                                                      \
    {                                                                     \
      DNX_EXIT_WITH_ERR(_SHR_E_MEMORY, (_DNX_MSG("Failed to allocate memory")));\
    }                                                                     \
