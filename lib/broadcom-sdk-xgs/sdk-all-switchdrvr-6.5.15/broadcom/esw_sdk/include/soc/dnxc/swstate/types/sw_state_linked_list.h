...
* \brief
*  The function will add a new node to the linked list Data
*  Structure as the last node in the list
* \param [in] unit    - Device Id 
* \param [in] module_id  - Module Id
* \param [in] key    - the content of the node's key 
* \param [in] value  - the content of the node's value
...
/**
 * \brief - Returned a ll structure size.
 *
 * \param [in] init_info    - info about the Linked list (size of
 *        data, nof elements etc.)
 * \param [in] flags - 0 - none
 ...
 *   None
 */
int sw_state_ll_create_empty_size_get(
    sw_state_ll_init_info_t * init_info,
    uint32 flags,
    uint32 key_size,
    uint32 value_size);
...
#define SW_STATE_MULTIHEAD_LL_CREATE_EMPTY_SIZE_GET(init_info, flags, key_type, value_type) \
    sw_state_ll_create_empty_size_get(init_info, flags, sizeof(key_type), sizeof(value_type))
