...
#define SW_STATE_MULTI_SET_CREATE(module_id, multi_set, init_info, nof_members, alloc_flags)\
    SHR_IF_ERR_EXIT(sw_state_multi_set_create(unit, module_id, &multi_set, init_info, nof_members, alloc_flags))
...
#define SW_STATE_MULTI_SET_DEFAULT_PRINT(unit, module_id, multi_set)\
        SHR_IF_ERR_EXIT(sw_state_multi_set_print(unit, module_id, *(multi_set), 0))

#define SW_STATE_MULTI_SET_SIZE_GET(init_info)\
    sw_state_multi_set_size_get(init_info)
...
typedef struct
{
     ...
     */
    sw_state_resource_tag_bitmap_t allocation_bitmap;
 ...
 * \param [in] nof_members - Number of member of the multi set. Specified in the relevant sw state xml file.
...
/**
 * \brief - Returns a sw state multi set structure size.
 *
 * \param [in] init_info - Pointed memory contains setup parameters required for for the creation of the hash table.
 *
 * \return
 ...
 *   None
 */
int sw_state_multi_set_size_get(
    sw_state_multi_set_info_t * init_info);
