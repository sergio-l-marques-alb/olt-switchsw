...
#define BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSY_LOW 0          /* Ingress
                                                          oversubscription
                                                          monitor drop buffer
                                                          type lossy low. */
#define BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSY_HIGH 1          /* Ingress
                                                          oversubscription
                                                          monitor drop buffer
                                                          type lossy high. */
#define BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSLESS0 2          /* Ingress
                                                          oversubscription
                                                          monitor drop buffer
                                                          type lossless0. */
#define BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSLESS1 3          /* Ingress
                                                          oversubscription
                                                          monitor drop buffer
                                                          type lossless1. */
#define BCM_SWITCH_CONTROL_IOM_DROP_TYPE_EXPRESS 4          /* Ingress
                                                          oversubscription
                                                          monitor drop buffer
                                                          type express. */
...
typedef enum bcm_switch_control_e {
    ...
    bcmSwitchIomStartTime = 1167,       /* Set sampling window starting
                                           timestamp value in nano seconds. It
                                           must not exceed 17 second from
                                           current time. */
    bcmSwitchIomEpochTime = 1168,       /* Set sampling interval in micro
                                           seconds. Only certain platform
                                           specific discrete points are
                                           supported. */
    bcmSwitchIomEnable = 1169,          /* Used to activate IOM instrumentation
                                           . */
    bcmSwitchIomStatus = 1170,          /* Used to check if sampling window
                                           starting timestamp was correct or
                                           not. */
    bcmSwitchIomSamplingType = 1171,    /* Set sampling type to either port type
                                           (0) or bucket type (1). */
    bcmSwitchIomSamplingMaxUsageType = 1172, /* Set sampling max usage type to either
                                           per port max usage(0) or shared max
                                           usage(1) for entire port macro */
    bcmSwitchIomSamplingMaxUsageReset = 1173, /* Reset max usage to zero after every
                                           sample interval */
    bcmSwitchIomSamplingSelectDropType = 1174, /* Select sampling drop type to be lossy
                                           low
                                           (BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSY_LOW),
                                           lossy
                                           high(BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSY_HIGH)
                                           lossless0(BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSLESS0),
                                           lossless1(BCM_SWITCH_CONTROL_IOM_DROP_TYPE_LOSSLESS1)
                                           or
                                           express(BCM_SWITCH_CONTROL_IOM_DROP_TYPE_EXPRESS). */
    bcmSwitchIomMaxPacketSizeBucketM = 1175, /* Set maximum packet size for bucket M */
    bcmSwitchIomMinPacketSizeBucketB = 1176, /* Set minimum packet size for bucekt B */
    bcmSwitchIomMaxPacketSizeBucketB = 1177, /* Set maximum packet size for bucket B */
    bcmSwitchIomMinPacketSizeBucketC = 1178, /* Set minimum packet size for bucekt C */
    bcmSwitchIomMaxPacketSizeBucketC = 1179, /* Set maximum packet size for bucket C */
    bcmSwitchCounterAdjustEgressTm = 1180, /* Counter compensation specifically for
                                           ETM. */
    bcmSwitchCounterAdjustEgressPp = 1181, /* Counter compensation specifically for
                                           ERPP. */
    bcmSwitchTagPcpDeiSrc = 1182,       /* Set Source of PCP_DEi for SIT */
    bcmSwitchBfdMyDipDestination = 1183, /* Set destination for BFD IP address
                                           forwarding. */
    bcmSwitchEndToEndLatencyPerDestinationPort = 1184, /* Define if end to end latency is made
                                           per destination port or not. */
    bcmSwitchExternalTcamInit = 1185,   /* Init/De-init External TCAM. */
    bcmSwitchHashMultiMoveDepthL3Tunnel = 1186, /* Maximum moves in multi hash mode for
                                           L3 Tunnel tables. */
    bcmSwitchGbpEtherType = 1187,       /* Specify the ethertype to be used for
                                           GBP protocol */
    bcmSwitchVxlanUdpSrcPortSelect = 1188, /* Select UDP source port for VxLAN
                                           tunnel */
    bcmSwitchGpeUdpSrcPortSelect = 1189, /* Select UDP source port for GPE tunnel */
    bcmSwitchGeneveUdpSrcPortSelect = 1190, /* Select UDP source port for GENEVE
                                           tunnel */
    bcmSwitchPreserveOtagBeforeItag = 1191, /* Control for preserved OTAG order wrt.
                                           ITAG */
    bcmSwitchMirrorCpuCosq = 1192,      /* Mirror COS for CPU port as MTP */
    bcmSwitchIntProbeMarkerEnable = 1193, /* INT packet ProbeMarker(UDF) parser
                                           Enable. */
    bcmSwitchIntL4DestPortEnable = 1194, /* INT packet UDP destination port
                                           parser Enable. */
    bcmSwitchFtmhEtherType = 1195,      /* Set FTMH header Ethertype to match */
    bcmSwitchGbpSidDidMcastLookupMissClassid = 1196, /* Default class id to be used when
                                           there is a miss in GBP SID_DID lookup
                                           for multicast flows. */
    bcmSwitchCustomOuterEthertype1 = 1197, /* Outer Ethertype 1 used for extracting
                                           packet fields in custom applications. */
    bcmSwitchCustomOuterEthertype2 = 1198, /* Outer Ethertype 2 used for extracting
                                           packet fields in custom applications. */
    bcmSwitchCustomInnerEthertype1 = 1199, /* Inner Ethertype 1 used for extracting
                                           packet fields in custom applications. */
    bcmSwitchCustomInnerEthertype2 = 1200, /* Inner Ethertype 2 used for extracting
                                           packet fields in custom applications. */
    bcmSwitchCustomOuterProtocolId1 = 1201, /* Outer Protocol ID 1 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomOuterProtocolId2 = 1202, /* Outer Protocol ID 2 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomInnerProtocolId1 = 1203, /* Inner Protocol ID 1 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomInnerProtocolId2 = 1204, /* Inner Protocol ID 2 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomOuterUdpDstPort1 = 1205, /* Outer UDP Dest Port 1 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomOuterUdpDstPort2 = 1206, /* Outer UDP Dest Port 2 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomInnerUdpDstPort1 = 1207, /* Inner UDP Dest Port 1 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchCustomInnerUdpDstPort2 = 1208, /* Inner UDP Dest Port 2 used for
                                           extracting packet fields in custom
                                           applications. */
    bcmSwitchReservedEgressNextHop = 1209, /* Set reserved egress next hop, which
                                           is used to generate L2 header for
                                           IPMC packet that does not have L2
                                           header after tunnel termination. */
    bcmSwitch__Count = 1210             /* Must be last.  Not a usable value. */
...
#define BCM_SWITCH_CONTROL_STR \
    ...
    "IomStatus", \
    "IomSamplingType", \
    "IomSamplingMaxUsageType", \
    "IomSamplingMaxUsageReset", \
    "IomSamplingSelectDropType", \
    "IomMaxPacketSizeBucketM", \
    "IomMinPacketSizeBucketB", \
    "IomMaxPacketSizeBucketB", \
    "IomMinPacketSizeBucketC", \
    "IomMaxPacketSizeBucketC", \
...
#define BCM_SEGMENT_NAME_SIZE   16         /* Segment name size. */

/* Kbp master key segment info. */
typedef struct bcm_switch_kbp_master_key_segment_info_s {
    uint8 segment_name[BCM_SEGMENT_NAME_SIZE]; /* segment name */
