...
/* Remove a Gport from the specified vlan with flags. */
...
/* 
 * Delete an entry based on gport and VLANs from the egress VLAN
 * Translation table.
 */
...
typedef enum bcm_vlan_translate_egress_key_e {
    ...
    bcmVlanTranslateEgressKeyCount = 7  /* Egress VLAN translation key count.
                                           Always the last. */
...
#define BCM_VLAN_PORT_CROSS_CONNECT         0x00800000 /* Creates a P2P vlan
                                                          port. */
#define BCM_VLAN_PORT_USE_SGLP_TPID         0x01000000 /* Use TPID based on
                                                          Source global logical
                                                          port (SGLP) for
                                                          parsing the packet. */
#define BCM_VLAN_PORT_EGRESS_ARP_POINTED    0x02000000 /* Create an ARP pointed
                                                          vlan port. */
#define BCM_VLAN_PORT_STAT_INGRESS_ENABLE   0x04000000 /* Enable statistics on
                                                          ingress side. */
#define BCM_VLAN_PORT_STAT_EGRESS_ENABLE    0x08000000 /* Enable statistics on
                                                          egress side. */
...
typedef enum bcm_vlan_port_match_e {
    ...
    BCM_VLAN_PORT_MATCH_COUNT = 29      /* Must be last. */
...
#if defined(INCLUDE_L3)
/* Initialize structure bcm_vlan_vpn_config_t. */
