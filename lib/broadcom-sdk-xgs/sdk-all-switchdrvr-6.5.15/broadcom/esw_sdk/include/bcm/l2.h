...
#define BCM_L2_EGRESS_RECYCLE_INJECTED_2_PP_HEADER 0x00008000 /* Recycle PTCH-2 header */
...
typedef struct bcm_l2_egress_s {
    ...
    bcm_gport_t dest_port;          /* Destination gport to associate this l2
                                       egress entry with */
    ...
    int vlan_qos_map_id;            /* VLAN PCP-DEI QoS settings. Used only in
                                       Native Routing overlay protocols. */
    int mpls_extended_label_value;  /* An extended label placed on top of a an
...
/* Set the age time of one meta cycle in seconds. */
extern int bcm_l2_age_timer_meta_cycle_set(
    int unit, 
    int meta_cycle_time);

/* Get the age time of one meta cycle in seconds. */
...
/* Enable the BCM L2 learn limit subsystem. */
extern int bcm_l2_learn_limit_enable(
    int unit);

/* Disable the BCM L2 learn limit subsystem. */
...
/* Add an multi L2 address entry to the specified device. */
...
/* Assign profile index to VSI */
extern int bcm_l2_cache_vpn_to_profile_map_set(
    int unit, 
    uint32 vsi, 
    uint32 profile_idx);

/* Get profile index assigned to VSI */
