...
/**
 * \brief
 *   Print a MTU profile template entry.
 * \par DIRECT INPUT
 *   \param [in] unit -the unit number
 *   \param [in] data -Pointer to data \n
 *       \b As \b input - \n
 *       pointer to data of type dnx_rx_trap_mtu_profile_config_t which
 *       includes properties of MTU profile
 *   \param [in] print_cb_data -
 *      Pointer of the print callback data.
 * \par INDIRECT INPUT
 *   None
 ...
 *   * None
 */
void dnx_algo_rx_trap_mtu_profile_print_entry_cb(
    int unit,
    const void *data,
    dnx_algo_template_print_t * print_cb_data);
...
/**
* \brief  
*   Allocate trap id in DNX_ALGO_RX_TRAP_RES_MNGR_ETPP_PREDEFINED pool
* \par DIRECT INPUT: 
*   \param [in] unit     - Unit id
*   \param [in] flags    - Flags
*   \param [in] trap_id_p  -  Pointer to a memory type of int. \n
*           \b As \b output - \n
*           This pocedure loads pointed memory by ETPP trap_id newly allocated
* \par INDIRECT INPUT: 
*   * None
* \par DIRECT OUTPUT: 
*   shr_error_e - Error type
...
/**
 * \brief - Deinitialize rx algorithms
 * 
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit ID
 *   
 ...
 *   * None
 */
shr_error_e dnx_algo_rx_deinit(
    int unit);
 ...
 * \brief - template manager for recycle command
 */
#define DNX_ALGO_RX_TRAP_TEMPLATE_MNGR_MTU_PROFILE              "TRAP_MTU_PROFILE"
/**
