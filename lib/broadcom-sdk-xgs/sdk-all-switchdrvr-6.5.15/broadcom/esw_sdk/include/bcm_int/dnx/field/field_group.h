...
typedef enum
{
    ...
    DNX_FIELD_GROUP_CONTEXT_FLAGS_FIRST = 0x0,
    DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE = DNX_FIELD_GROUP_CONTEXT_FLAGS_FIRST,
    DNX_FIELD_GROUP_COMPARE_FLAGS_NOF
} dnx_field_group_context_flags_e;
...
typedef struct
{
    ...
    /**
     * Holds the size of the action (result) buffer for the KBP field group.
     * */
    uint32 action_size;
    /**
...
typedef struct
{
    ...
    /**
     * DNX Actions applied by the database.For each '2msb combination, Select the
     * list of actions to be applied to matching entries.
...
typedef struct
{
    ...
    /**
     * DNX Actions applied by this context.For each '2msb combination, select the
     * list of actions to be applied to matching entries.
     * Can hold any of the dnx action depending on class and stage:
     * See List of available ones in field_map_data.c 
     * Number of meaningful elements on this array: All elements until
...
typedef struct
{
    ...
    int app_db_id;
    /**
     * Information about the FES quartets used by the context ID.
     */
 ...
 *    None
 * \see
 *   * _bcm_jer2_dpp_field_dir_ext_entry_hardware_install
 */
...
/** 
 * \brief  
 *   Given 'qualifier id', 'field group' and 'context id', get info about this
 *   qualifier within 'key' which has been allocated for it:
 *   Bit location, within 'key', where it resides, and width (bits).
 *   Note that this is the offset on the actual key (and not on 'key template')
 *   For example, if allocation is in 80 bits resolution then offset '0' on
 *   'key template' may be offset '80' on 'key'.
 * \param [in] unit -
 *   Identifier of HW platform.
 * \param [in] fg_id -
 *   Field group on which this qualifier resides.
 * \param [in] context_id -
 *    Indicator for which 'context' this operation is intended.
 * \param [in] qualifier -
 *   Variable of type dnx_field_qual_t. Identifier of the qualifier to search
 *   for within specified field group.
 * \param [out] qualifier_bit_lsb_location_p -
 *   Pointer to uint. This procedure loads pointed memory by the bit location (starting
 *   from the LSB, which is marked as 0) within the carrying key ('key_carrying_qualifier').
 *   Expected range is 0 - 159
 * \param [out] qualifier_bit_width_p -
 *   Pointer to uint. This procedure loads pointed memory by the number of bits which this
 *   qualifier occupies on specified key.
 * \param [out] key_carrying_qualifier_p -
 *   Pointer to dbal_enum_value_field_field_key_e. This procedure loads pointed memory by
 *   the key on which this qualifier resides. This procedure assumes there is only one
 *   relevant key (and not a double key).
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 */
shr_error_e dnx_field_group_n_context_qualifier_info_get(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    dnx_field_qual_t qualifier,
...
* \param [in] unit                     - Device ID
* \param [in] fg_id                    - Field group ID
* \param [out] actions_bit_spread_p    - The amount of bits used by the actions on the payload, including spaces 
*                                        between them.
* \return
*   shr_error_e - Error Type
* \remark
...
*   * None
*/
shr_error_e dnx_field_group_actions_spread_on_payload_get(
    int unit,
    dnx_field_group_t fg_id,
...
*   * None
*/
shr_error_e dnx_field_ace_format_actions_spread_on_payload_get(
    int unit,
    dnx_field_ace_id_t ace_id,
    unsigned int *actions_bit_spread_p);
...
* \param [in] unit         - Device id
* \param [in] flags        - context attach flags (see field_group.h)
 ...
 *   Action arrays to contain all actions per '2msb combination'.
 ...
 *   list of actions that are to be correlated with this '2ms bits' combination.
 ...
 * \param [out] nof_actions - An array whose number of elements is DNX_FIELD_NUM_2MSB_COMBINATIONS.
 *                            Loaded with the number of valid elements in each of the one dimentional arrays that 
 *                            are a part of the two dimensional array dnx_actions.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
...
/**
* \brief
* Compare an actions array with the actions of one 2msb combination of a field group,
* verifying that they are both the same.
* Returns _SHR_E_PARAM if the arrays are not identical.
* \param [in] unit                    - Device ID
* \param [in] fg_id                   - The ID of the field group to whose actions we compare.
* \param [in] dnx_actions             - The actions array to be compared.
*                                       The number of actions in the array is determined by the first element
*                                       that equals DNX_FIELD_QUAL_TYPE_INVALID. If no such element is found, the 
*                                       number of elements is regarded as 
*                                       DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP.
* \param [in] msb_comb                - The 2msb combination to be compared.
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
*   * None
*/
shr_error_e dnx_field_group_action_arr_compare_verify(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_action_t dnx_actions[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP],
...
/**
* \brief
*  Get Context information that was configured for specific Field Group
* \param [in] unit           - Device ID
* \param [in] fg_id          -  Field Group ID
 ...
 * \param [in] unit     - Device ID
 * \param [in] fg_id    - Field group ID
 * \param [out] flags_p  - Flags that were set for the Field Group
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 * \param [in] unit     - Device ID
 * \param [in] ace_id    - ACE ID
 * \param [out] flags_p  - Flags that were set for the ACE format
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
...
/**
 * \brief
 *  Calculates the total number of bits in actions and valid bits for one dimensional
 *  array of dnx_field_action_in_group_info_t.
 * \param [in] unit            -
 *   Device ID
 ...
 *   The number of bits used by all actions combined.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 */
shr_error_e dnx_field_group_calc_actions_bit_size_1_arr(
    int unit,
    dnx_field_stage_e field_stage,
 ...
 *   Only the 'dnx_action' and 'dont_use_valid_bit' are read to calculate the action size of the sizes,
 *   'lsb' field need not be filled.
 * \param [out] actions_size_in_bits  -
 *   Pointer to an array of DNX_FIELD_NUM_2MSB_COMBINATIONS pointers. Each pointer corresponds
 *   to one '2ms bits' combination.
 *   This procedure loads each pointed memory by the sum of sizes, in bit resolution, of all actions and valid bits
 *   in the 1d array of 'actions_info_arr' corresponding to this 2msb combination.
 * \param [out] combined_actions_size_in_bits_p  -
 *   To be loaded with the total number of bits in use by all actions and their valid bits for all 2msb combinations.
 *   At this point assumes that the 2msb combinations are independent of each other and just takes the maximal bit
 *   size of the four.
 * \return
 *   shr_error_e - Error Type
 * \remark
...
/**
 * \brief
 *  Calculates the total number of bits in the payload for function
 *  dnx_field_ace_format_action_length_type_calc().
 *  This function is used for ACE formats.
 * \param [in] unit            -
 *   Device ID
 * \param [out]  actions_info_arr    -
 *   Array of type dnx_field_action_in_group_info_t.
 ...
 *   'lsb' field need not be filled.
 * \param [out] actions_size_in_bits_p  -
 *   The number of bits used by all actions and valid bits together.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * This procedure just uses dnx_field_group_calc_actions_bit_size_1_arr() directly.
 * \see
 *   * dnx_field_group_calc_actions_bit_size().
 */
shr_error_e dnx_field_ace_format_calc_actions_bit_size(
    int unit,
    dnx_field_action_in_group_info_t actions_info_arr[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP],
    unsigned int *actions_size_in_bits_p);
...
/**
 * \brief
 *  Returns information about the entries for the given field group id.
 *  The info returned includes info about the number of inserted entries that
 *  belong to the given fg_id distributed over all the TCAM banks in the
 *  device. This information is returned in the variable bank_entries_count
 *  which is an array of size DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS that the user
 *  needs to allocate before calling this function.
 *  Another piece of information returned by this function through the banks_bmp
 *  variable, is a bitmap of all allocated banks for the given fg_id.
 *
 *  \param [in] unit                - unit
 *  \param [in] fg_id               - The field group to get the information for.
 ...
 *                                    element.
 *  \param [out] banks_bmp_p        - An output bitmap of all allocated banks
 *                                    for the given field group.
 *  \return
 *   \retval _SHR_E_NONE     - Success
 *   \retval _SHR_E_PARAM    - Field group is not of type TCAM
 *   \retval _SHR_E_INTERNAL - No allocated TCAM handler found for the given field
 *                             group
 */
shr_error_e dnx_field_group_tcam_entries_info(
    int unit,
    dnx_field_group_t fg_id,
...
/**
* \brief
*  Get Key ID for a given pair of {context, field_group}
* \param [in] unit            - Device ID
* \param [in] context_id      - Context ID
* \param [in] fg_id           - Field group id that context ID being attached to
* \param [out] key_id_p       - Pointer type of dnx_field_key_id_t that hold the key information
* \return
*   shr_error_e - Error Type
* \remark
*   * None
...
shr_error_e dnx_field_group_key_get(
    int unit,
    dnx_field_context_t context_id,
    dnx_field_group_t fg_id,
    dnx_field_key_id_t *key_id_p);

/* TBD: Consider moving back to static */
shr_error_e dnx_field_group_sw_state_get(
    int unit,
    dnx_field_group_t fg_id,
