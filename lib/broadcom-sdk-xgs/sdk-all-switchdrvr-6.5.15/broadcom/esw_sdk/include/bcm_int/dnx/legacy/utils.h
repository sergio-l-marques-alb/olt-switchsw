...
#define _BCM_DNX_SNOOP_SIZE (SOC_DNX_ACTION_CMD_SIZE_BYTES_ALL_PCKT)
...
#define BCM_DNX_UNIT_CHECK(unit) \
    do { \
    if (((unit) < 0) || ((unit) >= BCM_LOCAL_UNITS_MAX)) { SHR_ERR_EXIT(_SHR_E_UNIT, "Invalid unit!"); }  \
    } while (0)
    

#define BCM_DNX_MIN_CHECK(arg, min) \
    do {   \
        if ((arg) < min) { return _SHR_E_PARAM; } \
    } while (0)
...
#endif
#define BCM_DNX_MAX_TRUNK_ID(unit) (SOC_DNX_PORT_LAGS_MAX)
