...
/** 
  * \brief -  calcultae bank id (crps-meter shmem from engine id - engine 16 > banks 0&6, engine 17 > banks 1&7, ...engine 21 > banks 5&11
  */
#define DNX_CRPS_METER_SHMEM_BANK_ID_CALC(_unit, _engine_id, _section) \
    (_engine_id - dnx_data_meter.mem_mgmt.base_big_engine_for_meter_get(_unit) + \
    ((uint32)_section == dnx_crps_mgmt_shmem_section_high ? dnx_data_meter.mem_mgmt.sections_offset_get(_unit) : 0));
...
/**
  * \brief - return if the given source is never being evicted. example: I-OAM/E-OAM source
  */
#define DNX_CRPS_MGMT_SOURCE_NO_EVICTION(_unit, _source) \
            (_source == bcmStatCounterInterfaceIngressOam || _source == bcmStatCounterInterfaceEgressOam)
