...
/**
 * \brief
 *   This function is invoked to inform TCAM Handler about a DT entry that is being added
 *   for the given handler ID. This procedure is needed for DT handlers with 'AUTO' bank
 *   allocation mode, in order to allocate a bank on the first added entry.
 *
 * \param[in] unit       - Device ID
 * \param[in] handler_id - Handler ID of the DT handler we're adding an entry to
 *
 * \return
 *  \retval _SHR_E_NONE     - Success
 *  \retval _SHR_E_INTERNAL - Function was invoked for a non-DT handler
 */
shr_error_e dnx_field_tcam_handler_dt_entry_add(
    int unit,
    uint32 handler_id);
