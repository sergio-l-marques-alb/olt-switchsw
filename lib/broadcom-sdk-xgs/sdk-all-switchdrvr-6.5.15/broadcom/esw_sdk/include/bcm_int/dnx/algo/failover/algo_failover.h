 ...
 *           each bank in FEC path select will be tagged upon allocating the first element in the bank.
 *           Refer to dnx_algo_res_simple_bitmap_allocate() for details about the input parameters.
 ...
 *           The tag represents FEC hierarchy,
 *           each bank in FEC path select will be tagged upon allocating the first element in the bank.
 *           Refer to dnx_algo_res_simple_bitmap_free() for details about the input parameters.
 *           extra_arguments - pointer to uint32 which represents the tag
 */
...
/**
 * \brief De-Initialize dnx algo failover module.
 *
 * \param [in] unit - The unit number.
 * \return
 *   See shr_error_e
 ...
 *   * None
 */
shr_error_e dnx_algo_failover_deinit(
    int unit);
