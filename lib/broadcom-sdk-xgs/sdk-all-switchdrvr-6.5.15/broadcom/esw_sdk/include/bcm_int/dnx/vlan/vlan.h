...
/**
 * \brief
 * Check that the result-type is AC CROSS CONNECT 
 */
#define BCM_DNX_IS_RESULT_TYPE_AC_CROSS_CONNECT(resultType) \
    ((resultType == DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_W_1_VLAN) || \
     (resultType == DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_P2P_LARGE))

/**
 * \brief
 * Check that the result-type is Native-AC CROSS CONNECT
 */
#define BCM_DNX_IS_RESULT_TYPE_NATIVE_AC_CROSS_CONNECT(resultType) \
    ((resultType == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_VSI_P2P) || \
     (resultType == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_AC_VSI_P2P))

/**
 * \brief
 * Check that the result-type is Native-AC VIRTUAL lif (vlan edit only)
 */
#define BCM_DNX_IS_RESULT_TYPE_NATIVE_AC_VIRTUAL(resultType) \
    ((resultType == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_VSI_MP) || \
     (resultType == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_VSI_P2P) || \
     (resultType == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_ONLY))
 ...
 *   * None
 * \par INDIRECT OUTPUT
 *   * De-Initialized BCM VLAN module
 * \remark
 *   * None
 *
 */
shr_error_e dnx_vlan_deinit(
    int unit);
 ...
 * bcm_dnx_gport_stat_set
 * bcm_dnx_gport_stat_get
 * bxm_dnx_vlan_port_destroy
 */
shr_error_e dnx_vlan_port_egress_virtual_native_key_set(
    int unit,
    uint32 sw_table_handle_id,
    uint32 entry_handle_id);
