...
#define DNX_DATA_FIELD_ENTRY_CORE_ID_SHIFT 24
#define DNX_DATA_FIELD_ENTRY_CORE_ID_MASK  0x3

#define DNX_DATA_FIELD_ENTRY_FG_ID_SHIFT 16
#define DNX_DATA_FIELD_ENTRY_FG_ID_MASK  0xFF

#define DNX_DATA_FIELD_ENTRY_ACCESS_ID_MASK 0xFFFF
...
typedef enum
{
    ...
    DNX_FIELD_TCAM_EXTERNAL = 1
} dnx_field_entry_tcam_type_e;

#define DNX_DATA_FIELD_TCAM_TYPE_SHIFT         31
#define DNX_DATA_FIELD_TCAM_TYPE_MASK          0x1

#define DNX_DATA_FIELD_ENTRY_EXTERNAL_TCAM_ACCESS_ID_MASK   0x3FFFFFF

#define DNX_FIELD_ENTRY_IS_EXTERNAL_TCAM(entry_handle)        \
        ((entry_handle >> DNX_DATA_FIELD_TCAM_TYPE_SHIFT) & DNX_DATA_FIELD_TCAM_TYPE_MASK)
...
#define DNX_FIELD_TCAM_ENTRY_FG_ID(entry_handle)        \
        ((entry_handle >> DNX_DATA_FIELD_ENTRY_FG_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_FG_ID_MASK)

#define DNX_FIELD_EXTERNAL_TCAM_ENTRY_FG_ID(entry_handle)        \
        ((entry_handle >> DNX_DATA_FIELD_ENTRY_EXTERNAL_TCAM_FG_ID_SHIFT) & DNX_DATA_FIELD_ENTRY_EXTERNAL_TCAM_FG_ID_MASK)

#define DNX_FIELD_ENTRY_FG_ID(entry_handle)        \
        DNX_FIELD_ENTRY_IS_EXTERNAL_TCAM(entry_handle) ? \
            (DNX_FIELD_EXTERNAL_TCAM_ENTRY_FG_ID(entry_handle)) : (DNX_FIELD_TCAM_ENTRY_FG_ID(entry_handle))

#define DNX_FIELD_ENTRY_ACCESS_ID(entry_handle)         \
        ((DNX_FIELD_ENTRY_IS_EXTERNAL_TCAM(entry_handle)) ? \
    (entry_handle & DNX_DATA_FIELD_ENTRY_EXTERNAL_TCAM_ACCESS_ID_MASK) : (entry_handle & DNX_DATA_FIELD_ENTRY_ACCESS_ID_MASK))
 ...
 * IS_EXTERNAL_TCAM (1 bit, value=0)
 * NOT USED         (6 bits)
 * CORE ID          (2 bits)
 * FG_ID            (7 bits)
 * ACCESS_ID        (16 bits)
 */
#define DNX_FIELD_TCAM_ENTRY(fg_id, access_id, core_id)  \
                            (DNX_FIELD_ENTRY(DNX_FIELD_TCAM_INTERNAL, (fg_id), (access_id), (core_id)))
 ...
 *
 * EXTERNAL TCAM FG_ID Range is smaller then the standard TCAM FG_ID. It is 0-63 (5 bits) because
 * the access id is 26 bit, 1 bit is left for distinguishing the ExternalTcam field_group_type entry,
 * thus we are left with (32 - 1- 26) = 5 bits for fg_id.
 *
 * EXTERNAL TCAM Entry Id is encoded in the following way (from MSB to LSB):
 * IS_EXTERNAL_TCAM (1 bit, value=1)
 * FG_ID            (5 bits)
 * ACCESS_ID        (26 bits)
 */
#define DNX_FIELD_EXTERNAL_TCAM_ENTRY(fg_id,access_id)  \
                            (DNX_FIELD_ENTRY(DNX_FIELD_TCAM_EXTERNAL, (fg_id), (access_id), 0))

/** This macro encode the access id that passed to TCAM/External_TCAM handler. */
#define DNX_FIELD_ENTRY(fg_type, fg_id, access_id, core_id)  \
                            (((fg_type) << DNX_DATA_FIELD_TCAM_TYPE_SHIFT) | \
                            ((core_id) << DNX_DATA_FIELD_ENTRY_CORE_ID_SHIFT) | \
                            ((fg_id) << (((fg_type) == DNX_FIELD_TCAM_EXTERNAL)? \
                                DNX_DATA_FIELD_ENTRY_EXTERNAL_TCAM_FG_ID_SHIFT : DNX_DATA_FIELD_ENTRY_FG_ID_SHIFT)) | \
                            (access_id))
...
/**
 * Holds possible flags for ACE entry creation/updating.
 * Used in dnx_field_ace_entry_add()
 */
typedef enum
{
    /**
     * ACE entry is created WITH_ID,
     * I.e. user specifies which ACE entry key should be allocated
     */
    DNX_FIELD_ACE_ENTRY_ADD_FLAG_WITH_ID = 0x1,
    /**
     * ACE entry is updated,
     * I.e. the payload of the entry is changed.
     */
    DNX_FIELD_ACE_ENTRY_ADD_FLAG_UPDATE = 0x2,
} dnx_field_ace_entry_add_flags_e;
...
*
* \param [in] unit    - Device ID
* \param [in] address - Address for the entry to delete
*
* \return
*   shr_error_e - Error Type
* \remark
...
* \param [in] unit    - Device ID
* \param [in] address - Address to add entry at
* \param [out] data_p - The entry's data at the given address
*
* \return
*   shr_error_e - Error Type
* \remark
...
*   * None
*/
shr_error_e dnx_field_entry_tcam_hit_per_core_get(
    int unit,
