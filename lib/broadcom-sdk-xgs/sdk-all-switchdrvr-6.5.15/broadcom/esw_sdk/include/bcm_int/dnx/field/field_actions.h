 ...
 * At the moment we use the same polarity for all actions in all groups.
 */
#define DNX_FIELD_ACTION_FES_VALID_BIT_POLARITY (1)
/**
 * Number of qualifiers selected to indicate the 2 ms bits to set in the KEY_DATA
 * (of key_select) which will point to the selected action (in IPPC_FES_2ND_INSTRUCTION)
 */
#define DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS      2
/**
 * Number of bits in the 2 ms bits to set in the KEY_DATA
 * (of key_select) which will point to the selected action (in IPPC_FES_2ND_INSTRUCTION)
 */
#define DNX_FIELD_MAX_NOF_BITS_2MSB_SELECT 2
/**
 * Lowest bit of the two assigned to the '2 ms bits' feature on the 64-bits FES_DATA chunk.
 * Note that on the key itself, it may either be located on bit 30 or on bit 62.
 */
#define DNX_FIELD_BASE_OF_2MSB                   62
/**
 * Maximal number of 32-bits chunks on one key. The '2 ms bits' must be located
 * at the MS bits of any of these chaunks.
 */
#define DNX_FIELD_NUM_LOCATIONS_OF_2MSB               5
/**
 * Minimal number of any 32-bits chunks on one key.
 */
#define DNX_FIELD_MIN_LOCATIONS_OF_2MSB               1
/**
 ...
 *   (DNX_FIELD_KEY_SELECT_BIT_GRANULARITY * n - DNX_FIELD_NUM_2MSB_BITS_ON_QUALIFIERS).
 * where 'n' is between DNX_FIELD_MIN_LOCATIONS_OF_2MSB and DNX_FIELD_NUM_LOCATIONS_OF_2MSB
 * Based on the number of bit of difference bwtween the key selects.
 */
#define DNX_FIELD_KEY_SELECT_BIT_GRANULARITY                  32
/**
 * The qualifier type used to indicate the first bit of the fes_data 2msb.
 */
#define DNX_FIELD_FES_DATA_2MSB_FEATURE_QUAL_BIT_1 (62)
/**
 * The qualifier type used to indicate the second bit of the fes_data 2msb.
 */
#define DNX_FIELD_FES_DATA_2MSB_FEATURE_QUAL_BIT_2 (63)
/**
 * \brief
 *  If 'CMODEL_SUPPORTS_INVALID_FEM_KEY_SELECT' is set to a non-zero value then CMODEL
 *  can take an illegal 'fem key select' and, as a result, carry out no action.
 *  This is very useful since it allows using all 'FEM program's without reserving
 *  a 'defult' one (for 'NO ACTION' operation.
 * \see dnx_field_actions_single_fem_single_context_defaults_set()
 *  
 */
#define CMODEL_SUPPORTS_INVALID_FEM_KEY_SELECT  0
#ifdef ADAPTER_SERVER_MODE
/*
 * { 
 */
/*
 * Enter for compilation under CMODEL.
 */
/*
 ...
 * Currently, CMODEL does not accept the 'illegal' value into 'fem_key_select' so
 * we set it, temporarily, to a standard value until CMODEL is fixed.
 */
#if (CMODEL_SUPPORTS_INVALID_FEM_KEY_SELECT)
/*
 * { 
 */
 ...
 * {
 */
#define DNX_FIELD_INVALID_FEM_KEY_SELECT     DBAL_ENUM_FVAL_FIELD_PMF_A_FEM_KEY_SELECT_DIRECT_1_119_88
/*
 * }
 */
#endif
 ...
 */
/*
 * Enter for compilation under real device.
 */
#define DNX_FIELD_INVALID_FEM_KEY_SELECT     DBAL_ENUM_FVAL_FIELD_PMF_A_FEM_KEY_SELECT_INVALID_FEM_KEY_SELECT
/*
 * }
...
{
    /*
     * The number of valid FES quartets used by the field group,
     * and therefore the number of valid elements in the array fes_quartets.
     */
    unsigned int nof_fes_quartets;
    /*
     ...
     */
        dnx_field_actions_fes_quartet_context_group_get_info_t
        fes_quartets[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP];
...
typedef struct
{
    ...
    dnx_field_action_in_group_info_t
        actions_on_payload_info[DNX_FIELD_NUM_2MSB_COMBINATIONS]
...
/**
* \brief
*  Init Structure dnx_field_action_attach_info_t to invalid params
...
/**
 * \brief
 *    Given, as input, placed actions on actions on up to 'DNX_FIELD_NUM_2MSB_COMBINATIONS' arrays,
 *    get the FESES that would load those actions from specified 'actions table'.
 * \param [in] unit -
 *    Identifier of HW platform.
 * \param [in] field_stage -
 ...
 *    Pointer to a two dimensional array of dnx_field_action_attach_info_t structures
 *    with number of elements:
 *    DNX_FIELD_NUM_2MSB_COMBINATIONS
 *    DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP
...
/**
 * \brief
 *    Writes a single action to a FES.
 ...
 * \param [in] context_id -
 *    Context ID for which the configuration is done.
 * \param [in] fes_add_info_p -
 *    Array of dnx_field_actions_single_fes_add_t, containing information about the action we want to add.
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 *    At this point only support 2msb feature as a 2 bit condition. No one bit and no variable actions.
 * \see
 *   * dnx_field_group_de_context_entries_load
 */
shr_error_e dnx_field_actions_single_fes_add(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    dnx_field_actions_single_fes_add_t * fes_add_info_p);

/**
 * \brief
 *    Reads the qualifiers for the fes_data 2msb and fill a dnx_field_dir_ext_fes_data_2msb_information_t
 *    structure with the information deduced from it.
 * \param [in] unit -
 *    Identifier of HW platform.
 * \param [in] field_stage -
 *    For which of the PMF blocks the configuration is done (iPMF1/2/3, ePMF).
 ...
 *    HW identifier of field group
 * \param [in] context_id -
 *    Context ID to which the qualifiers are attached to.
 * \param [in] feature_2msb_quals -
 *    Array of dnx_field_dir_ext_quals_for_msb_bits_t, number of elements DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS,
 *    with qualifiers containing information for the 2msb feature.
 * \param [out] feature_2msb_info_p
 *    Pointer to a strcuture dnx_field_dir_ext_fes_data_2msb_information_t which will be loaded with the
 *    2msb feature information.
 *    This information includes whether or not we have 2msb feature information, the location on the
 *    key template of the 2msb, the number of bits to use (in the future could support only 1 bit) and
 *    and the data that should be written to the 2msb in order for the action to be valid.
 * \return
 *    \retval Zero - On success
 *    \retval Error - Identifier as per shr_error_e
 * \remark
 ...
 *   * dnx_field_dir_ext_fes_setup_single_action
 */
shr_error_e dnx_field_actions_fes_data_2msb_feature_info(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    dnx_field_dir_ext_quals_for_msb_bits_t * feature_2msb_quals,
    dnx_field_dir_ext_fes_data_2msb_information_t * feature_2msb_info_p);
 ...
 *    Context ID.
 * \param [in] is_dir_ext -
 *    Whether or not the field group is direct extraction.
 *    for direct extraction we disable FES program sharing.
...
/*
 * Currently, CMODEL does not accept the 'illegal' value into 'fem_key_select' so
 * we use 'program 0' as 'default' and set all its entries to 'action invalid'.
 */
#if (CMODEL_SUPPORTS_INVALID_FEM_KEY_SELECT)
/*
 * { 
 ...
 */
/*
 * Note: Since, under cmodel, it is NOT possible to 'neutralize' FEM on
 * any context from doing any action, we use 'FEM programs 0' as 'default'
 * and indicate 'invalid action' for all its FEMs. So, set
 * the first available 'FEM program' to be the one following the default.
 * See dnx_field_actions_single_fem_single_context_defaults_set()
 */
#define FIRST_AVAILABLE_FEM_PROGRAM (DEFAULT_FEM_PROGRAM + 1)
/*
 * }
 */
#endif
 ...
 */
/*
 * Note: Since, under real device, it is possible to 'neutralize' any FEM on
 ...
 *   Identifier of a 'scond field group' to search for on the list of active FEMs. If
 ...
 *    and the value should be 'zero'. Also, 'adder' must be DNX_FIELD_FEM_ADDER_INVALID.
 ...
 *    Note that, for 'FEM id's 0/1, this input must be set to DNX_FIELD_FEM_ADDER_INVALID.
 * \return
 *   \retval Zero - On success
 *   \retval Error - Identifier as per shr_error_e
 * \remark
