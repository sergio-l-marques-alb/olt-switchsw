...
#define DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_F_OCC_BMP "IPMF1_INITIAL_KEY_F_OCC_BMP"
#define DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_G_OCC_BMP "IPMF1_INITIAL_KEY_G_OCC_BMP"
#define DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_H_OCC_BMP "IPMF1_INITIAL_KEY_H_OCC_BMP"
#define DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_I_OCC_BMP "IPMF1_INITIAL_KEY_I_OCC_BMP"
#define DNX_ALGO_FIELD_IPMF1_INITIAL_KEY_J_OCC_BMP "IPMF1_INITIAL_KEY_J_OCC_BMP"
...
/*
 * Related to 'bit-range':
 * This is the 'step' of alignment. If alignment is required, we try to align at any of the
 * 'steps', starting from '0' until the maximal value for aligned bit (DNX_FIELD_KEY_MAX_VALUE_FOR_ALIGNED_BIT).
 * See dnx_algo_field_key_bit_range_alloc_generic().
 */
#define DNX_FIELD_KEY_STEP_VALUE_FOR_ALIGNED_BIT (dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->fem_key_select_resolution_in_bits)
/*
 * Related to 'bit-range':
 * Minimal value for the number of bit, relative to the first bit on 'key template', which is
 * required to be '16 bit aligned' when allocating 'bit-range'
 * See dnx_algo_field_key_bit_range_alloc_generic().
 */
#define DNX_FIELD_KEY_MIN_VALUE_FOR_ALIGNED_BIT ((-1)*DNX_FIELD_KEY_STEP_VALUE_FOR_ALIGNED_BIT)
/*
 * Related to 'bit-range':
...
/*
 * Since 'key part' is signed, make sure to make the mask large
 * enough to contain an extra MS bit to indicate that.
 * This extra bit is used to signal negative values.
 */
#define NOF_BITS_KEY_PART           3
#define FIELD_KEY_PART_SHIFT        0
#define FIELD_KEY_PART_MASK         SAL_UPTO_BIT(NOF_BITS_KEY_PART)
/**
 ...
 *   \param [in] _key_part   -
 *     'key part' (80 MSB, 80 LSB) to place in '_key_allocation_id'
 * \remark
 *   * None
 * \see
 *   * SET_ITEM_IN_PLACE
 ...
 *   * FIELD_STAGE_MASK
 */
#define CONSTRUCT_KEY_ALLOCATION_ID(_key_allocation_id,_fg_type,_field_stage,_context_id,_key_id_0,_key_id_1, _key_part) \
    ...
    SET_ITEM_IN_PLACE(_key_allocation_id,(uint32)_key_part,FIELD_KEY_PART_SHIFT,FIELD_KEY_PART_MASK) ;
 ...
 *   \param [in] _key_part   -
 *     Variable to load 'key part' (80 MSB, 80 LSB) from place in '_key_allocation_id'
 * \remark
 *   * Since
 * \see
 *   * SET_ITEM_IN_PLACE
 ...
 *   * FIELD_STAGE_MASK
 */
#define DECONSTRUCT_KEY_ALLOCATION_ID(_key_allocation_id,_fg_type,_field_stage,_context_id,_key_id_0, _key_id_1,_key_part) \
    ...
    GET_ITEM_FROM_PLACE(_key_allocation_id,_key_part,FIELD_KEY_PART_SHIFT,FIELD_KEY_PART_MASK) ; \
    if (_key_part & SAL_BIT(NOF_BITS_KEY_PART - 1)) \
    { \
        _key_part = DNX_FIELD_KEY_PART_TYPE_INVALID ; \
    };\
 ...
 * see dnx_algo_field_action_fes_priority_get()
 */
typedef struct
{
    /*
...
typedef struct
{
    ...
    dnx_algo_field_action_fes_alloc_out_fes_quartet_change_t
        fes_quartet_change[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_INSTRUCTION_PER_CONTEXT * 5];
    /*
...
/**
 * \brief
 *  Decide which field stage is currently has to be configured
 * \param [in] unit           - Device Id
 * \param [in] asset_p      - BCM actions types
 * \param [out] field_stage_p  - Field stage (PMFA/PMFB...)
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * Currently the function is based on PMF action beeing configured
 * \see
 *   * None
 */
shr_error_e dnx_algo_field_stage_get(
    int unit,
...
/**
 * \brief
 *  The function will hold the algorithm for descion if we would like to allocate FEM or FES
 * \param [in] unit    - Device Id
 * \param [out] is_fes_p  - If TRUE then algo decided to alloc FES
 *                                If FALSE then algo decided to alloc FEM
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 ...
 *   * None
 */
shr_error_e dnx_algo_field_action_fem_or_fes(
    int unit,
    int *is_fes_p);
 ...
 *   uint32 *. Pointer to identifier (handle) of allocated key.
 * \return
 *   shr_error_e - Error Type
 * \remark
 ...
 * \param [in]  unit              -
 *   Device ID
 * \param [in]  key_allocation_id -
 *   Identifier of the key to be freed. Eventually interpreted to:
 *     field_stage      -  Field Stage to allocate key for (IPMF1,2,IPMF3...)
 *     fg_type          -  Database type that being created (TCAM/DE/DT)
 *     context_id       -  Context ID to allocate the key for
 *     key_id           -  Identifier of key (A - J)
 *     key_part_type    -
 *       Indication on which half-key caller wishes to free (MSB or LSB). only
 *       meaningful if only 80bits are required to be freed.
 *       If single full key or double key is to be freed, caller should
 *       fill here: DNX_FIELD_KEY_PART_TYPE_INVALID
 * \return
 *   shr_error_e - Error Type
 * \remark
...
/**
* \brief
*  Allocate placeholder in the initial key F for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] alloc_flags  - allocation flags (DNX_ALGO_TEMPLATE_ALLOCATE..)
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] qual_size    - qualifier size that we try to allocate
* \param [out] initial_offset_p  - Starting index of the allocated range.
* \return
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_f_occupation_bmp_allocate(
    int unit,
    int alloc_flags,
    dnx_field_context_t context_id,
    uint32 qual_size,
...
/**
* \brief
*  Allocate placeholder in the initial key G for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] alloc_flags  - allocation flags (DNX_ALGO_TEMPLATE_ALLOCATE..)
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] qual_size    - qualifier size that we try to allocate
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_g_occupation_bmp_allocate(
    int unit,
    int alloc_flags,
    dnx_field_context_t context_id,
    uint32 qual_size,
...
/**
* \brief
*  Allocate placeholder in the initial key H for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] alloc_flags  - allocation flags (DNX_ALGO_TEMPLATE_ALLOCATE..)
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] qual_size    - qualifier size that we try to allocate
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_h_occupation_bmp_allocate(
    int unit,
    int alloc_flags,
    dnx_field_context_t context_id,
    uint32 qual_size,
...
/**
* \brief
*  Allocate placeholder in the initial key I for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] alloc_flags  - allocation flags (DNX_ALGO_TEMPLATE_ALLOCATE..)
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] qual_size    - qualifier size that we try to allocate
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_i_occupation_bmp_allocate(
    int unit,
    int alloc_flags,
    dnx_field_context_t context_id,
    uint32 qual_size,
...
/**
* \brief
*  Allocate placeholder in the initial key J for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] alloc_flags  - allocation flags (DNX_ALGO_TEMPLATE_ALLOCATE..)
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] qual_size    - qualifier size that we try to allocate
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_j_occupation_bmp_allocate(
    int unit,
    int alloc_flags,
    dnx_field_context_t context_id,
    uint32 qual_size,
...
/**
* \brief
*  De-Allocate placeholder in the initial key F for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] context_id   - Context ID to allocate the given range
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_f_occupation_bmp_deallocate(
    int unit,
    dnx_field_context_t context_id,
    uint32 size,
    int initial_offset);
/**
* \brief
*  De-Allocate placeholder in the initial key G for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] size         - qualifier size that we try to allocate
* \param [in] initial_offset - Starting index of the allocated range.
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_g_occupation_bmp_deallocate(
    int unit,
    dnx_field_context_t context_id,
    uint32 size,
    int initial_offset);
/**
* \brief
*  De-Allocate placeholder in the initial key H for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] size         - qualifier size that we try to allocate
* \param [in] initial_offset - Starting index of the allocated range.
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_h_occupation_bmp_deallocate(
    int unit,
    dnx_field_context_t context_id,
    uint32 size,
    int initial_offset);
/**
* \brief
*  De-Allocate placeholder in the initial key I for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] size         - qualifier size that we try to allocate
* \param [in] initial_offset - Starting index of the allocated range.
 ...
 */
shr_error_e dnx_algo_field_key_ipmf1_initial_key_i_occupation_bmp_deallocate(
    int unit,
    dnx_field_context_t context_id,
    uint32 size,
    int initial_offset);
/**
* \brief
*  De-Allocate placeholder in the initial key J for a given qualifier in IPMF1
* \param [in] unit         - Device Id
* \param [in] context_id   - Context ID to allocate the given range
* \param [in] size         - qualifier size that we try to allocate
* \param [in] initial_offset - Starting index of the allocated range.
...
*   * None
*/
shr_error_e dnx_algo_field_key_ipmf1_initial_key_j_occupation_bmp_deallocate(
    int unit,
    dnx_field_context_t context_id,
    uint32 size,
    int initial_offset);
...
shr_error_e dnx_algo_field_action_update_conditions(
    ...
    dnx_field_action_t fem_encoded_actions[dnx_data_field.stage.stage_info_get(unit,
    ...
    dnx_field_fem_condition_entry_t fem_condition_entry_destination[dnx_data_field.stage.stage_info_get(unit,
                                                                                                        DNX_FIELD_STAGE_IPMF1)->nof_fem_condition],
    uint8 *ignore_actions_p,
    uint8 *already_in_hw_p,
    dnx_field_action_t fem_encoded_actions_destination[dnx_data_field.stage.stage_info_get(unit,
                                                                                           DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index],
    dnx_field_fem_action_entry_t fem_action_entry_destination[dnx_data_field.stage.stage_info_get(unit,
                                                                                                  DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index]);
/*
 * }
 */
...
*   * None
*/
shr_error_e dnx_algo_field_compare_key_id_get(
    int unit,
...
*   * None
*/
shr_error_e dnx_algo_field_hash_key_id_get(
    int unit,
    dnx_field_key_id_t *key_id_p);
...
*   * None
*/
shr_error_e dnx_algo_field_state_table_key_id_get(
    int unit,
    dnx_field_key_id_t *key_id_p);
...
*   * None
*/
shr_error_e dnx_algo_field_entry_kbp_opcode_cs_profile_id_allocate(
    int unit,
    int alloc_flags,
...
*   * dnx_algo_field_entry_tcam_access_id_allocate() description
*/
shr_error_e dnx_algo_field_entry_kbp_opcode_cs_profile_id_deallocate(
    int unit,
    int cs_prodfile_id);
...
*   * dnx_algo_field_init()
*/
shr_error_e dnx_algo_field_action_sw_state_init(
    int unit);
...
/**
 * \brief
 *   This procedure provides the priorities of FES quartets for a context ID and field group.
 * \param [in] unit -
 *   Device ID.
 * \param [in] field_stage -
 *   For which of the PMF blocks the configuration is done (iPMF-1, iPMF2, iPMF-3, ePMF-1).
 ...
 * \param [in] context_id -
 *   Context ID.
 * \param [in] context_state -
 *   Pointer to a dnx_algo_field_action_fes_alloc_single_context_state_in_t structure.
 *   Contains the FES quartets used by the context ID.
 ...
 *   to be loaded with the priorities.
 * \param [out] num_fes_quartets -
 *   Pointer to an unsigned int, to be loaded with the number of actions tin the field group,
 *   and thus the number of valid elements in priorities.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
 * \see
 *   * dnx_field_actions_priority_get()
 */
shr_error_e dnx_algo_field_action_fes_priority_get(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    dnx_algo_field_action_fes_alloc_single_context_state_in_t * context_state,
    dnx_field_action_priority_t priorities[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP],
    unsigned int *num_fes_quartets);
