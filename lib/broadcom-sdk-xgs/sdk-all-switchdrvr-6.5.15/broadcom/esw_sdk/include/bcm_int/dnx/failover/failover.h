...
{                               \
    ...
    SHR_IF_ERR_EXIT(utilex_bitstream_field_swap((uint32 *)&(failover_id), dnx_data_failover.path_select.in_lif_protection_pointer_nof_lsb_get(unit),             \
...
{                       \
   failover_id = 0;     \
   SHR_IF_ERR_EXIT(utilex_bitstream_field_swap((uint32 *)&(protection_pointer), dnx_data_failover.path_select.in_lif_protection_pointer_nof_msb_get(unit),  \
...
/**
 * \brief De-Initialize dnx failover module (protection path select).
 *
 * \param [in] unit - The unit number.
 * \return
 *   See shr_error_e
 ...
 *   * None
 */
shr_error_e dnx_failover_deinit(
    int unit);
/**
