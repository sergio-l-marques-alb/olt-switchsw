...
/**
 * \brief
 *  Spanning Tree Protocol is supported on eth ports only.
 *  The port to be checked should be a local/logical port.
 */
#define BCM_DNX_STG_CHECK_PORT(unit, port)                  \
    if ((port < 0) ||                                       \
        (!IS_E_PORT((unit), (port)) &&                      \
 ...
 *
 * \remark
 *  We haven't defined actions for it currently. Just pick it out for the future.
 *
 * \see
 *  None.
 * 
 */
shr_error_e dnx_stg_deinit(
    int unit);
