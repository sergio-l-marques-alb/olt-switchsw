...
#define DNX_FIELD_ACTION_PRIORITY_FES_PRIORITY_IS_VALID_BIT_SET(priority)                \
        (((priority) & (1 << BCM_FIELD_ACTION_VALID_OFFSET)) != 0)
...
{                                                                                                                 \
    if ((DNX_FIELD_ACTION_PRIORITY_FES_PRIORITY_IS_DONT_CARE_BIT_SET(priority)) &&                                \
        (priority != BCM_FIELD_ACTION_DONT_CARE))                                                                 \
    {                                                                                                             \
        SHR_ERR_EXIT(_SHR_E_PARAM,                                                                                \
                     "Action priority 0x%08x has the \"Care\" bit zero, but isn't \"don't care\" priority.\r\n",  \
                     priority);                                                                                   \
    }                                                                                                             \
    if ((DNX_FIELD_ACTION_PRIORITY_FES_PRIORITY_IS_VALID_BIT_SET(priority) == FALSE) &&                           \
        (priority != DNX_FIELD_ACTION_PRIORITY_INVALID))                                                          \
    {                                                                                                             \
        SHR_ERR_EXIT(_SHR_E_PARAM,                                                                                \
...
#define DNX_FIELD_COMPARE_PAIR_VERIFY(_unit,_pair_id)                                                   \
        ...
        { \
            SHR_ERR_EXIT(_SHR_E_PARAM, \
                         "\r\n" \
                         "Compare pair ID (%d) is out of range (Should be bigger then %d and smaller or equal of %d)\r\n", \
                         _pair_id, 0, dnx_data_field.common_max_val.nof_compare_pairs_get(_unit)); \
        }
/**
...
/**
 * Allow sharing between ACL segments and FWD segments in the KBP based on having the same name
 * and assuming all KBP FWD segments use FWD layer without any offset other than the qualifier base offset.
 */
#define DNX_FIELD_KBP_NAIVE_SHARING_ACL_FWD (0)
/**
 * The maximum number of context selection qualifiers for a single stage.
 */
#define DNX_FIELD_CS_QUAL_NOF_MAX (35)
/**
