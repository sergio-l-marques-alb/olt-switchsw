...
/**
 * \brief
 *  Returns the priority for the entry at the given location in TCAM SWSTATE on the given core
 *
 * \param [in] unit       - Device ID
 * \param [in] core       - Core ID
 * \param [in] handler_id - The handler_id that contains the entry
 * \param [in] location   - The absolute location of the entry inside TCAM Banks
 * \param [out] priority  - The priority of the entry
 *
 * \return
 *  \retval _SHR_E_NONE - Success
 */
shr_error_e dnx_field_tcam_location_algo_priority_get(
    int unit,
    int core,
    uint32 handler_id,
    uint32 location,
