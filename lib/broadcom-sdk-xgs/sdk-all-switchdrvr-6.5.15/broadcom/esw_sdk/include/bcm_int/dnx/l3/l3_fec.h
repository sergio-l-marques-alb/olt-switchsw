 ...
 * \param [in] bank_ids - The bank ID of the resource to be update and also the pair resource if required.
 * \param [out] hierarchy -
 ...
 */
shr_error_e dnx_l3_fec_hierarchy_stage_map_set(
    int unit,
    int *bank_ids,
    uint32 hierarchy);
...
/**
 * \brief
 *   Update the logical start address of the MDB cluster ID with
 *   the start of the range to which the FEC index belongs to.
 * \param [in] unit - The unit number.
 * \param [in] cluster_to_update - hold the information of which clusters should be updated and to what logical address.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 * \see
 *  * dnx_algo_l3_fec_allocate
 */
shr_error_e dnx_l3_fec_mdb_table_resources_set(
    int unit,
    algo_dnx_l3_fec_banks_update_info_t * cluster_to_update);

/**
 * \brief
 *   Initialise the L3 FEC module. Set the needed default values.
 * \param [in] unit - The unit number.
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
