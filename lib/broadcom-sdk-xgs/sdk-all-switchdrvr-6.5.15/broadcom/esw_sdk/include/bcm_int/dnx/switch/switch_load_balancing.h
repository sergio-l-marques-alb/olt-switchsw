...
/**
 * Used to indicate the 'CLIENT_x_CRC_16_SEL' field on LOAD_BALANCING_CRC_SELECTION tcam
 * where 'x' stands for 0 or 1 or 2 or 3 or 4.
 */
typedef uint32 dnx_switch_lb_client_crc_sel_t;
/**
...
typedef struct
{
     ...
     */
    dnx_switch_lb_client_crc_sel_t client_x_crc_16_sel[DNX_DATA_MAX_SWITCH_LOAD_BALANCING_NOF_LB_CLIENTS];
 ...
 *   Used for set/get on IPPB_MPLS_CRC_16_SEED register fields
 * \remark -
 *   Note that the 'lb_crc_seed' element is used
 *   both as input (on 'set') and as output (on 'get').
 * \see -
 *   IPPB_MPLS_CRC_16_SEED
 *   DBAL_TABLE_LOAD_BALANCING_CRC_SEEDS
 */
typedef struct
{
    /*
     * Identifier of crc fuction for which 'seed' is to be set (or
     * for 'get' operation).
     */
    uint16 lb_crc_function_index;
    /**
     * Value of seed to 'load into' (or 'retrieve from') hw
     * register. The seed is a 16 bits value.
     */
    uint16 lb_crc_seed;
...
/**
 * \brief - Deinitialize LOAD_BALANCING module (under SWITCH). \n
 *
 * \par DIRECT_INPUT:
 *   \param [in] unit - Unit-ID
 * \par INDIRECT INPUT:
 ...
 *   shr_error_e
 */
shr_error_e dnx_switch_load_balancing_module_deinit(
    int unit);
 ...
 *   Identifier of HW platform.
 * \param [in] crc_seed_p -
 *   Pointer to structure of type dnx_switch_lb_crc_seed_t which contains
 *   the identifier of the selected crc function (lb_crc_function_index) and
 *   the seed itself (lb_crc_seed).
 *   Identifier must be smaller than the number of supported crc functions (8).
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 * \remark
 ...
 * \param [in] crc_seed_p -
 *   Pointer to structure of type dnx_switch_lb_crc_seed_t which contains
 *   the identifier of the selected crc function (lb_crc_function_index),
 *   as input, and container to get the corresponding seed itself (lb_crc_seed),
 *   as output.
 *   Identifier must be smaller than the number of supported crc functions (8).
 * \return
 *   \retval Zero if no error was detected
