...
typedef struct bcm_async_req_s {
	...
	union {
		...
		} cpri_port_tx_cw_vsd_ctrl_flow_config_get;
		struct {
			int unit;
			int port;
			uint16 group_id;
	...
	/* bcm_module_name has a non int return type */
		...
		struct {
			int unit;
			uint8 * mac_addr;
		} xflow_macsec_station_mac_addr_get;
		struct {
			int unit;
			uint8 * mac_addr;
		} xflow_macsec_station_mac_addr_set;
		struct {
			int unit;
			bcm_xflow_macsec_id_t id;
...
typedef enum {
	...
	BCM_ASYNC_ENTRY_CPRI_PORT_TX_CW_VSD_CTRL_FLOW_CONFIG_GET,
	...
	BCM_ASYNC_ENTRY_XFLOW_MACSEC_STATION_MAC_ADDR_GET,
	BCM_ASYNC_ENTRY_XFLOW_MACSEC_STATION_MAC_ADDR_SET,
