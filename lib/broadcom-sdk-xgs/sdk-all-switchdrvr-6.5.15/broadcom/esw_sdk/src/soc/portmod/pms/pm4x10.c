...
#define PM4x10_WB_BUFFER_VERSION        (9)
...
STATIC
int _pm4x10_txpi_lane_select_set(int unit, int port, pm_info_t pm_info, uint32_t *lane_select);
    ...
    if ((recovered_version <= 7) &&
        ...
        for (i = 0; i < MAX_PORTS_PER_PM4X10; i++) {
             for (adjust_index = 0; adjust_index < PORTMOD_TS_ADJUST_NUM; adjust_index++) {
                  _SOC_IF_ERR_EXIT(
                      PM4x10_PORT_TIME_STAMP_ADJUST_SET(unit, pm_info, &port_ts_adjust, adjust_index, i));
             }
        }
    }
                                            ...
                                            int speed, portmod_port_ts_adjust_t *ts_adjust)
{
    ...
    for (adjust_index = 0; adjust_index < PORTMOD_TS_ADJUST_NUM; adjust_index++) {
         ...
         if ((port_ts_adjust.speed == speed)  ||
             ((port_ts_adjust.speed == -1) && (adjust_index == (PORTMOD_TS_ADJUST_NUM - 1)))) {
                        ...
                        phymod_tx_t ext_phy_tx_params[])
{
    ...
    if(!pm_is_bypassed){
        ...
        _SOC_IF_ERR_EXIT(xlmac_timestamp_byte_adjust_set(unit, port, config->speed));
                            ...
                            int port_index, int phy_acc, const portmod_port_add_info_t* add_info)
{
    ...
    uint32 *txpi_lane_select = (uint32 *)add_info->init_config.lane_map[0].lane_map_tx;
    SOC_INIT_FUNC_DEFS;
    ...
    if (PORTMOD_PM4x10_IS_GEN3_28nm(pm_version)) {
        ...
        _SOC_IF_ERR_EXIT(_pm4x10_txpi_lane_select_set(unit, port, pm_info, txpi_lane_select));
    }
...
/* Following code applies to the phy 54290 */
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            ...
            if (!PORTMOD_PORT_IS_DEFAULT_TIMESTAMP_ADJUSTMENT_UPDATED(port_dynamic_state)) {
                _SOC_IF_ERR_EXIT(_pm4x10_port_timestamp_adjust_wb_get(unit, port, pm_info, interface_config.speed, &ts_adjust));
            _SOC_IF_ERR_EXIT(xlmac_timestamp_delay_set(unit, port, ts_adjust, interface_config.speed));
            }
            _SOC_IF_ERR_EXIT(xlmac_timestamp_byte_adjust_set(unit, port, interface_config.speed));
            if(PM_4x10_INFO(pm_info)->portmod_mac_soft_reset) {
                _SOC_IF_ERR_EXIT(pm4x10_port_soft_reset(unit, port, pm_info, 0));
            }
