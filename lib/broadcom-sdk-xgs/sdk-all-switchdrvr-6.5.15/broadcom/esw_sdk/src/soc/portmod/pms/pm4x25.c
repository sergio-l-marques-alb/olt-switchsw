...
#define PM4x25_WB_BUFFER_VERSION        (11)
    ...
    if ((recovered_version <= 8) &&
        ...
        for (i = 0; i < MAX_PORTS_PER_PM4X25; i++) {
             for (adjust_index = 0; adjust_index < PORTMOD_TS_ADJUST_NUM; adjust_index++) {
                  _SOC_IF_ERR_EXIT(
                     PM4x25_PORT_TIME_STAMP_ADJUST_SET(unit, pm_info, &port_ts_adjust, adjust_index, i));
             }
        }
    }
                   ...
                   int wb_buffer_index, pm_info_t pm_info)
{
    ...
    pm_info->pm_data.pm4x25_db->link_recovery.enabled = TRUE;
    pm_info->pm_data.pm4x25_db->link_recovery.tick_cnt = 0;
    pm_info->pm_data.pm4x25_db->link_recovery.state = PORTMOD_LINK_RECOVERY_STATE_WAIT_FOR_LINK;
                                     ...
                                     int phy_init_flags)
{
    ...
    if (config->flags & PHYMOD_INTF_F_UPDATE_SPEED_LINKUP) {
        ...
        if (!PORTMOD_PM4x25_IS_GEN1(pm_version)) {
            _SOC_IF_ERR_EXIT(clmac_timestamp_byte_adjust_set(unit, port, config->speed));
        }
...
int pm4x25_port_link_get(int unit, int port, pm_info_t pm_info, int flags, int* link)
{
    ...
    if ((nof_phys == 1) && (PM_4x25_INFO(pm_info)->link_recovery.enabled)) {
        ...
        if ((phy_access[0].access.lane_mask == 0xf) &&
            (interface_modes & (PHYMOD_INTF_MODES_FIBER | PHYMOD_INTF_MODES_BACKPLANE))) {
            /* get CL91 */
            PHYMOD_FEC_CL91_SET(fec_enable);
            _SOC_IF_ERR_EXIT
                 (portmod_port_phychain_fec_enable_get(unit, port, phy_access, nof_phys, flags, &fec_enable));
            if (fec_enable) {
                switch (PM_4x25_INFO(pm_info)->link_recovery.state) {

                        case PORTMOD_LINK_RECOVERY_STATE_WAIT_FOR_LINK:
                            /* link is good */
                            if (*link) {
                                PM_4x25_INFO(pm_info)->link_recovery.state = PORTMOD_LINK_RECOVERY_STATE_LINK_GOOD;
                            } else {
                                /* no link yet, keep wait for link */
                                PM_4x25_INFO(pm_info)->link_recovery.tick_cnt += 1;
                                if ((PM_4x25_INFO(pm_info)->link_recovery.tick_cnt) > PORTMOD_LINK_RECOVERY_LINK_WAIT_CNT_LIMIT) {
                                    /* no link after all the try, reset the rx patha again */
                                    PM_4x25_INFO(pm_info)->link_recovery.state = PORTMOD_LINK_RECOVERY_STATE_RESET_RX;
                                }
                            }
                            break;
                        case PORTMOD_LINK_RECOVERY_STATE_RESET_RX:
                             PM_4x25_INFO(pm_info)->link_recovery.tick_cnt = 0;
                            ...
                            if (*link) {
                                ...
                                PM_4x25_INFO(pm_info)->link_recovery.state = PORTMOD_LINK_RECOVERY_STATE_WAIT_FOR_LINK;
                            }
                            break;
...
int pm4x25_port_update(int unit, int port, pm_info_t pm_info, const portmod_port_update_control_t* update_control)
{
    ...
    if(!link) {
        ...
        if(autoneg.enabled && autoneg.locked) {
            ...
            _SOC_IF_ERR_EXIT(_pm4x25_pm_version_get(unit, pm_info, &pm_version));
            if (!PORTMOD_PM4x25_IS_GEN1(pm_version)) {
                _SOC_IF_ERR_EXIT(clmac_timestamp_byte_adjust_set(unit, port, interface_config.speed));
            }
            if(PM_4x25_INFO(pm_info)->portmod_mac_soft_reset) {
                _SOC_IF_ERR_EXIT(pm4x25_port_soft_reset(unit, port, pm_info, 0));
            }
