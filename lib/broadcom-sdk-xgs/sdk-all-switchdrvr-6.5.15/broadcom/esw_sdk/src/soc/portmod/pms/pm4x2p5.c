...
STATIC
int _pm4x2p5_core_probe ( int unit, int port, pm_info_t pm_info, phymod_core_access_t* core_access, int *probe, int is_probe_for_legacy)
{
    ...
    if (core_access->type == phymodDispatchTypeInvalid)
    {
        ...
        if (!is_phymod_probed)
        {
            ...
            if (is_probe_for_legacy)
            {
                ...
                {
                    _SOC_IF_ERR_EXIT(portmod_xphy_is_legacy_phy_set(unit, core_access->access.addr, 1));
                    *probe = 1;
                }
           }
        }
...
int _pm4x2p5_pm_core_probe (int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    _SOC_IF_ERR_EXIT(_pm4x2p5_core_probe(unit, port, pm_info, &PM_4x2P5_INFO(pm_info)->int_core_access,&probe, 0));
                ...
                for (i=0 ; i<nof_ext_phys ; i++) {
                    ...
                    if (xphy_id != PORTMOD_XPHY_ID_INVALID) {
                        ...
                        if ( !core_probed ) {
                            _SOC_IF_ERR_EXIT(portmod_xphy_core_access_get(unit, xphy_id, &core_access, &is_legacy_phy));
                            probe = 0;
                            if (SOC_E_NONE != (_pm4x2p5_core_probe(unit,port, pm_info, &core_access, &probe, 1))) {
