...
#define PMM_WB_BUFFER_VERSION            (5)
...
typedef struct pmm_periodic_info_s {
...
typedef struct pmm_info_s{
    ...
    uint32 max_pms;
    /* callback for ports */
    pmm_periodic_info_t *ports_periodic;
...
int portmod_create(int unit, int flags, int max_ports, int max_phys, int nof_pm_instances, const portmod_pm_instances_t* pm_instances)
{
    ...
    /*callback allocation*/    
    pmm->ports_periodic = sal_alloc(sizeof(pmm_periodic_info_t)*max_ports, "ports_periodic");
    SOC_NULL_CHECK(pmm->ports_periodic);
    sal_memset(pmm->ports_periodic, 0, sizeof(pmm_periodic_info_t)*max_ports);
...
#define _PORTMOD_PE_NAME_MAX 30
        ...
        _SOC_IF_ERR_EXIT(periodic_event_create(unit, &pe_config, &(pmm->periodic_handler)));
    }
...
int portmod_destroy(int unit)
{
    ...
    _SOC_IF_ERR_EXIT(periodic_event_destroy(&(_pmm_info[unit]->periodic_handler), 1 /* force quiting the thread */));

    if (_pmm_info[unit]->ports_periodic != NULL) {
        sal_free(_pmm_info[unit]->ports_periodic);
        _pmm_info[unit]->ports_periodic = NULL;
    }
...
static int portmod_periodic_event(int unit, void* user_data)
{
    ...
    for (port_i=0 ; port_i<_pmm_info[unit]->max_ports ; port_i++) {
        ...
        _SOC_IF_ERR_EXIT(periodic_event_is_quit_signaled_get(_pmm_info[unit]->periodic_handler, &is_quit_signaled));

        if (is_quit_signaled) {
            break;
        }

        port_callback = _pmm_info[unit]->ports_periodic[port_i].ports_callback;
...
{
    int port_i, is_callback_conetxt;
    int min_interval=_PORTMOD_MIN_INTERVAL_NOT_FOUND; /*-2 indicates first value*/
    SOC_INIT_FUNC_DEFS;

    /* This function might be called from main thread or from callback thread
       Therefor the ports_periodic database, and the new interval setting need to be protected.
       Stopping the thread in case of I'm not in the callback context ensure callback thread and main thread doesn't collids:
       --> If I'm in the main thread - the callback is definitely not running as I'm stopping the periodic event
       --> If I'm in the callback context - the main thread isn't in the critical section as otherwise it would stop me
        Note: this method ensure only no collision between main thread and periodic event thread,
        it doesn't allow other multi-threading operations on this function.
    */
    _SOC_IF_ERR_EXIT(periodic_event_is_periodic_context_get(_pmm_info[unit]->periodic_handler, &is_callback_conetxt));
    if (!is_callback_conetxt) {
        _SOC_IF_ERR_EXIT(periodic_event_stop(_pmm_info[unit]->periodic_handler, 0 /*don't force exit*/, 0 /* wait for thread to trigger */));
    }


    /* Update DB*/
    _pmm_info[unit]->ports_periodic[port].ports_callback = callback;
    _pmm_info[unit]->ports_periodic[port].interval = interval;
    

    /* calc min interval and num of callbacks */
    for (port_i=0 ; port_i<_pmm_info[unit]->max_ports ; port_i++) {

        if (_pmm_info[unit]->ports_periodic[port_i].ports_callback != NULL) {

            /* get minimum interval */
            if (_pmm_info[unit]->ports_periodic[port_i].interval != sal_sem_FOREVER) { /*ignore forever as it's -1 and will always be the minimum...*/
                if (min_interval == _PORTMOD_MIN_INTERVAL_NOT_FOUND) {
                    min_interval = _pmm_info[unit]->ports_periodic[port_i].interval;
                }

                if (_pmm_info[unit]->ports_periodic[port_i].interval < min_interval) {
                    min_interval = _pmm_info[unit]->ports_periodic[port_i].interval;
                }
            }
        }
    ...
    _SOC_IF_ERR_EXIT(periodic_event_interval_set(_pmm_info[unit]->periodic_handler, min_interval));

    if (!is_callback_conetxt) {
        _SOC_IF_ERR_EXIT(periodic_event_start(_pmm_info[unit]->periodic_handler));
    }
