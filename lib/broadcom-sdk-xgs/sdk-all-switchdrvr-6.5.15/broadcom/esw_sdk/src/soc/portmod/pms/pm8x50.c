...
#define PM8x50_PHY_ACCESS_GET(unit, port, pm_info, phy_acc) \
    do { \
        uint32 is_bypass; \
        phy_acc = 0; \
        _SOC_IF_ERR_EXIT(PM8x50_IS_BYPASSED_GET(unit, pm_info, &is_bypass)); \
        phy_acc = (is_bypass && PM_8x50_INFO(pm_info)->first_phy != -1) ? (PM_8x50_INFO(pm_info)->first_phy | SOC_REG_ADDR_PHY_ACC_MASK) : port ; \
    } while (0)
...
#define PM8x50_VCO_ALL (PM8x50_VCO_ILKN_20P625G | PM8x50_VCO_ETH_20P625G | PM8x50_VCO_ILKN_25G | PM8x50_VCO_ETH_25P781G | PM8x50_VCO_ILKN_25P781G | PM8x50_VCO_ETH_26P562G | PM8x50_VCO_ILKN_26P562G | PM8x50_VCO_ILKN_27P343G | PM8x50_VCO_ILKN_28P125G)
...
const pm8x50_an_ability_table_entry_t pm8x50_an_ability_table[PM8x50_AN_ABILITY_TABLE_SIZE] =
{
    ...
    {400000, 8, PORTMOD_PORT_PHY_FEC_RS_544_2XN, PORTMOD_PORT_PHY_CONTROL_AUTONEG_MODE_CL73_BAM, portmodVCO26P562G}
};
    ...
    *is_done = 0;
    ...
    switch (an_state) {
       ...
       case PM8X50_PORT_AN_CHECK_PCS_LINKUP:
       {
            ...
            _SOC_IF_ERR_EXIT(pm8x50_port_link_get(unit, port, pm_info, 0, &link));
    ...
    int *phy_acc)
{
    ...
    *phy_acc = 0;
    ...
    if (is_bypassed) {
        PORTMOD_PBMP_ITER(PM_8x50_INFO(pm_info)->phys, pm_first_phy)
        {
            break;
        }
        *phy_acc = (pm_first_phy + port_index) | SOC_REG_ADDR_PHY_ACC_MASK;
    } else {
        *phy_acc = port;
    }
...
STATIC
int _pm8x50_signalling_method_get(int unit, int port, pm_info_t pm_info, const portmod_speed_config_t *speed_config, phymod_phy_signalling_method_t *signalling_method)
{
    int lane_rate;
    int port_index;
    uint32 is_bypassed = 0;
                   ...
                   pm_info_t pm_info)
{
    ...
    int i, rv;
    int bypass_enable, pm_is_active, ts_enable_port_count, is_initiator, pcs_reconfigured, active_lane_map;
...
{
    uint32 raddr, reg_val, tech_process;
    int phy_port, block;
    uint8 at;
    ...
    PORTMOD_PBMP_ITER(PM_8x50_INFO(pm_info)->phys, phy_port) {
        break;
    }

    raddr = soc_reg_addr_get(unit, CDPORT_PM_VERSION_IDr, phy_port, 0,
                             SOC_REG_ADDR_OPTION_PRESERVE_PORT, &block, &at);
    _SOC_IF_ERR_EXIT(_soc_reg32_get(unit, block, at, raddr, &reg_val));
                         ...
                         uint8 *affected_lane_bitmap)
{
     ...
     */
    if (PORTMOD_PM_VERSION_F_PM8X50_REV0_16NM_GET(flags)) {
                       ...
                       portmod_pm_vco_setting_t* vco_select)
{
    ...
    PORTMOD_BIT_COUNT(vcos, vco_count);

    if (vco_count > 2) {
        /* More than 2 vcos are required */
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                          (_SOC_MSG("More than 2 vcos is needed")));
    } else if (PM8x50_VCO_ETH_26P562G_GET(vcos)) {
        vco_select->tvco = portmodVCO26P562G;
        vco_select->is_tvco_new = 1;
        if (PM8x50_VCO_ETH_25P781G_GET(vcos)) {
            vco_select->ovco = portmodVCO25P781G;
            vco_select->is_ovco_new = 1;
        } else if (PM8x50_VCO_ETH_20P625G_GET(vcos)) {
            vco_select->ovco = portmodVCO20P625G;
            vco_select->is_ovco_new = 1;
        }
    } else if (PM8x50_VCO_ETH_25P781G_GET(vcos)) {
        vco_select->tvco = portmodVCO25P781G;
        vco_select->is_tvco_new = 1;
        if (PM8x50_VCO_ETH_20P625G_GET(vcos)) {
            vco_select->ovco = portmodVCO20P625G;
            vco_select->is_ovco_new = 1;
        }
    } else if (PM8x50_VCO_ETH_20P625G_GET(vcos) && PM8X50_TVCO_AVOID_20G) {
        /* If PM8X50_TVCO_AVOID_20G == 1, we need to avoid set TVCO to 20G.
         * In this case, even if only 20G is needed, we wil set
         * TVCO @ 25G, OVCO @ 20G.
         */
        vco_select->tvco = portmodVCO25P781G;
        vco_select->ovco = portmodVCO20P625G;
...
STATIC
int _pm8x50_port_tsc_reset_set(int unit, pm_info_t pm_info, int phy_port, int tvco_pll_index, int in_reset)
{
    ...
    if (SAL_BOOT_SIMULATION) {
        ...
        _SOC_IF_ERR_EXIT(READ_CDPORT_XGXS0_CTRL_REGr(unit, phy_port, &reg_val));
        soc_reg_field_set(unit, CDPORT_XGXS0_CTRL_REGr, &reg_val, TSC_RSTBf, in_reset ? 0 : 1);
        soc_reg_field_set(unit, CDPORT_XGXS0_CTRL_REGr, &reg_val, TSC_CLK_SELf, tvco_pll_index);
        soc_reg_field_set(unit, CDPORT_XGXS0_CTRL_REGr, &reg_val, TSC_PWRDWNf, in_reset ? 1 : 0);
        
        _SOC_IF_ERR_EXIT(WRITE_CDPORT_XGXS0_CTRL_REGr(unit, phy_port, reg_val));
        SOC_EXIT;
    }

    raddr = soc_reg_addr_get(unit, CDPORT_XGXS0_CTRL_REGr, phy_port, 0,
                             SOC_REG_ADDR_OPTION_PRESERVE_PORT, &block, &at);
    _SOC_IF_ERR_EXIT(_soc_reg32_get(unit, block, at, raddr, &reg_val));
                     ...
                     int enable)
{
    ...
    raddr = soc_reg_addr_get(unit, CDPORT_MAC_CONTROLr, phy_port, 0,
                             SOC_REG_ADDR_OPTION_PRESERVE_PORT, &block, &at);
    _SOC_IF_ERR_EXIT(_soc_reg32_get(unit, block, at, raddr, &reg_val));
    ...
    if(enable && is_reset) {
        /* Bring Serdes OOR */
        _SOC_IF_ERR_EXIT(_pm8x50_port_tsc_reset_set(unit, pm_info, phy_port, tvco_pll_index, 0));
        ...
        _SOC_IF_ERR_EXIT(_pm8x50_port_tsc_reset_set(unit, pm_info, phy_port, tvco_pll_index, 1));
    }
...
/*
 * in_reset_only ==> 1: in reset, 0: in + out of reset, 2: out of reset only
 */
STATIC
                       ...
                       const portmod_port_add_info_t* add_info)
{
    ...
    PM_8x50_INFO(pm_info)->first_phy = first_phy;
    ...
    for(ii=0; ii<PM8X50_LANES_PER_CORE; ii++) {
        ...
        if (PORTMOD_PORT_ADD_F_SKIP_SPEED_INIT_GET(add_info)) {
            ...
            _SOC_IF_ERR_EXIT(_pm8x50_signalling_method_get(unit, port, pm_info, speed_config, &signalling_mode));
        }
...
int pm8x50_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    /* flexport workaround for 16nm  flexportHW issue
     and only non-single lane port will  need this SW WAR */
    /* next check num of lane for this port */
    PORTMOD_BIT_COUNT(phy_access.access.lane_mask, num_lane);
    if (num_lane == 1) {
        skip_sw_war = 1;
    }
    ...
    while (!PM8x50_VCO_BMP_EMPTY(required_vcos_bmp_temp))
    {
        ...
        if (is_tvco_in_use)
        {
            if (required_vco == *current_tvco)
            {
                 ...
                 */
                if ( required_pcs_bypass || (!required_pcs_bypass && PM8x50_VCO_ETHERNET_DRIVE_TVCO_RATE(*current_tvco)) )
                {
                    continue;
                }
            }
        ...
        if (PM8x50_VCO_ETHERNET_DRIVE_TVCO_RATE(required_vco))
        {
            if (!is_tvco_in_use)
            {
        ...
        portmod_vco_type_t *current_ovco)
{
    ...
    if ( PM8x50_VCO_BMP_20P625G_ETH_ONLY(required_vcos_bmp) &&
    ...
    /* Serve new VCOs */
    _SOC_IF_ERR_EXIT(
            _pm8x50_vcos_request_allocate_internal(unit, required_new_vcos_bmp, current_tvco, current_ovco));

    /* Serve existing VCOs */
    _SOC_IF_ERR_EXIT(
            _pm8x50_vcos_request_allocate_internal(unit, required_existing_vcos_bmp, current_tvco, current_ovco));
                                  ...
                                  portmod_pm_vco_setting_t* vco_setting)
{
    ...
    _SOC_IF_ERR_EXIT(
            pm8x50_vcos_request_allocate(unit, required_vcos_bmp, &current_tvco, &current_ovco));
                                    ...
                                    portmod_pm_vco_setting_t* vco_setting)
{
    ...
    if (version.tech_process == PM_TECH_16NM) {
        PORTMOD_PM_VERSION_F_PM8X50_REV0_16NM_SET(fec_validate_flags);
    }
                                      ...
                                      portmod_pbmp_t* affected_pbmp)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_pm_version_get(unit, pm_info, &version));
    if (version.tech_process == PM_TECH_16NM) {
        PORTMOD_PM_VERSION_F_PM8X50_REV0_16NM_SET(flags);
    }
    ...
    if ((speed_config->speed == 200000) && (PM_8x50_INFO(pm_info)->tvco == portmodVCO20P625G)) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                          (_SOC_MSG("Can not support 200G when TVCO is 20.625G.")));
                           ...
                           const portmod_vco_type_t* vco)
{
    ...
    _SOC_IF_ERR_EXIT(PM8x50_IS_BYPASSED_GET(unit, pm_info, &is_pm_bypassed));
                                           ...
                                           const portmod_port_speed_ability_t* abilities)
{
    ...
    int is_dual_50g = 0, is_200g = 0, cl73_bam_msa_base_r_req = 0, dual_50g_req = 0;
    ...
    if (is_200g && (PM_8x50_INFO(pm_info)->tvco == portmodVCO20P625G)) {
        _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                           (_SOC_MSG("Can not support 200G when TVCO is 20.625G.")));
    }
    ...
    } else if (rs528_req && PORTMOD_PM_VERSION_F_PM8X50_REV0_16NM_GET(flags)) {
