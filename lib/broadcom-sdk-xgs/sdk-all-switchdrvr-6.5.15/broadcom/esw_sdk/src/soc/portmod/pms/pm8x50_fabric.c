...
{   
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        PM8X50_FABRIC_PLLS_ITER(PM8X50_FABRIC_MAX_NUM_PLLS, i) {
...
int pm8x50_fabric_pm_speed_config_validate(int unit, int pm_id, pm_info_t pm_info, const portmod_pbmp_t* ports, int flag, portmod_pm_vco_setting_t* vco_setting)
{
    ...
    uint32 pll_active_lanes_bitmap_required[1];
    ...
    PM8X50_FABRIC_PLLS_ITER(PM8X50_FABRIC_MAX_NUM_PLLS, pll_index)
    {
        pll_vco_rate_required[pll_index] = portmodVCOInvalid;
        pll_vco_rate_curr_updated[pll_index] = portmodVCOInvalid;
    }
    pll_active_lanes_bitmap_required[0] = 0;
    ...
    /** 1.1. get the required new vco rates from the requested speeds */
    for (speed_index = 0; speed_index < vco_setting->num_speeds; ++speed_index)
    {
        if (vco_setting->speed_for_pcs_bypass_port[speed_index])
        {
            pm8x50_shared_pcs_bypassed_vco_get(vco_setting->speed_config_list[speed_index].speed, &required_vco);
            if (required_vco == portmodVCOInvalid)
            {
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("speed %d not supported"), vco_setting->speed_config_list[speed_index].speed));
            }
        }
        else
        {
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_speed_config_fabric_speed_to_vco_get(unit, vco_setting->speed_config_list[speed_index].speed, &required_vco));
        }
    ...
    /** 1.2. add the lanes of the ports to the required PLLs bitmap */
    PORTMOD_PBMP_ITER(*ports, port)
    {
        SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
        PM8X50_FABRIC_PORT_BITMAP_ITER(bitmap, lane_index)
        {
            SHR_BITSET(&pll_active_lanes_bitmap_required[0], lane_index);
        }
    }
    ...
    PM8X50_FABRIC_PLLS_ITER(nof_plls, pll_index)
    {
                ...
                && (pll_vco_rate_curr[pll_index] != pll_vco_rate_required[0]) && (pll_vco_rate_curr[pll_index] != pll_vco_rate_required[1]))
        {
            ...
            if (PM8X50_FABRIC_BITMAP_IS_INCLUDED(pll_active_lanes_bitmap_curr[pll_index], pll_active_lanes_bitmap_required[0]))
            {
                pll_vco_rate_curr_updated[pll_index] = portmodVCOInvalid;
            }
        }
...
int pm8x50_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, const portmod_vco_type_t* vco)
{
    ...
    if (force_single_pll && (vco_aligned[1] != portmodVCOInvalid))
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("requesting PLL1 (unused PLL!) VCO rate change to %d"), vco_aligned[1]));
    }
...
int pm8x50_fabric_core_add(int unit, int pm_id, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
