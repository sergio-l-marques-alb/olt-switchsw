...
_phy_wc40_ind_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    SOC_IF_ERROR_RETURN
        (MODIFY_WC40_DIGITAL5_MISC6r(unit,pc,0x00,
              DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK, 
              DIGITAL5_MISC6_RESET_RX_ASIC_MASK | DIGITAL5_MISC6_RESET_TX_ASIC_MASK));
    /* 2.5G is not supported when the VCO is operating at 10.3125G. When the VCO is operating at 6.25G,
       XMAC is receiving a clock from the WarpCore that is a divided version of the 6.25G clock.
       ...
       So 4x2.5G on Katana2 and Helix4 using a 6.25G VCO are not supported, only 2x2.5G using a 6.25G VCO is supported.When works on 2x2.5G mode
       property "serdes_1000x_at_6250_vco" should be configured to allow vco change.
    */
    if (SOC_IS_HELIX4(unit) || SOC_IS_KATANA2(unit)) {
        ...
        /*when "serdes_1000x_at_6250_vco" is configured, allow change vco to 6.25G*/
        if (speed == 2500 && (soc_property_port_get(unit, port, spn_SERDES_1000X_AT_6250_VCO, 0)
                              || IS_SINGLE_LANE_IN_DUAL_PORT_MODE(pc))) {
