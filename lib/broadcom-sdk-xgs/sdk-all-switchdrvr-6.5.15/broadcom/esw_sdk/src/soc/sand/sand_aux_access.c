    ...
    device_t *device)
{
    ...
    if ((curTop = dbx_file_get_xml_top(unit, "AccessObjects.xml", "top", 0)) == NULL)
    {
    ...
    if ((curTop = dbx_file_get_xml_top(unit, "AccessObjects.xml", "top",
    ...
    device_t *device)
{
    ...
    /*
     * Go through the entries looking for either specific chip name of family one
     */
    RHDATA_ITERATOR(cur, curSubTop, "device")
    {
        ...
        RHDATA_GET_STR_CONT(cur, "db_name", device->db_name);
        if(!sal_strcasecmp(chip_n, device_n) || !sal_strcasecmp(family_n, device_n))
        {
            /*
