    ...
    uint8 nof_cores_to_broadcast)
{
    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++) {
        ...
        switch (blktype)
        {
            ...
            SAND_SINGLE_BLOCK_CASE(ESB, esb)
...
/*
* Returns if we are running on emulation or not (0).
* Different positive values may reflect different emulation modes.
*/
...
#ifdef BCM_DNX_SUPPORT
    if(SOC_IS_DNX(unit)){
        rv = dnx_data_device.general.emulation_system_get(unit);
    }else
#endif
#ifdef BCM_DNXF_SUPPORT
    if(SOC_IS_DNXF(unit)){
        rv = dnxf_data_device.general.emulation_system_get(unit);
    }else
#endif
#ifdef BCM_DPP_SUPPORT
    if(SOC_IS_DPP(unit)){
...
/*
* Set the following attributes accordong to soc property 
* schanTimeout
* schanIntrEnb
* miimTimeout
* miimIntrEnb
