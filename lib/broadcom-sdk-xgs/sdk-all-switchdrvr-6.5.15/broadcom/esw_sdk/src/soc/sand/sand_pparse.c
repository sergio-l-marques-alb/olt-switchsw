    ...
    parsing_db_t * pdb)
{
    ...
    {
        if ((pdb->next_headers_lst = utilex_rhlist_create("pdb_obj_lst", sizeof(next_headers_t), 1)) == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Could not allocate parsing DB strucutre.\n");
        }
    }
    ...
    signal_output_t * start_item)
{
    ...
    for (; start_item; start_item = utilex_rhlist_entry_get_next(start_item))
    {
        ...
        if (next_id > 0)
        {
             ...
             */
            sal_strncat(RHNAME(start_item), "_0", RHNAME_MAX_SIZE);
        }
        /*
...
packet_ptch(
    ...
    uint32 *enabled)
{
    packet_plugin_arg_t *block_arg = NULL;
    packet_plugin_arg_t *from_arg = NULL;
    packet_plugin_arg_t *attr_arg = NULL;
    ...
    *enabled = 0;
     ...
     * Get arguments
     */
    if (NULL == (block_arg = utilex_rhlist_entry_get_by_name(args, "Block")))
    {
        SHR_IF_ERR_EXIT(_SHR_E_PARAM);
    }
    if (NULL == (from_arg = utilex_rhlist_entry_get_by_name(args, "From")))
    {
        SHR_IF_ERR_EXIT(_SHR_E_PARAM);
    }
    if (NULL == (attr_arg = utilex_rhlist_entry_get_by_name(args, "Attribute")))
    {
        SHR_IF_ERR_EXIT(_SHR_E_PARAM);
    }
    /*
     * Read PTC signal
     */
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core, block_arg->value, from_arg->value,
                                  NULL /* to */ , attr_arg->value, &ptc, 1));
    /*
     * Read PTC-Profile from DBAL
     */
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                        DBAL_FIELD_PORT_TERMINATION_PTC_PROFILE, INST_SINGLE,
                                                        &ptc_profile));
    /*
     * Return whether PTC profile is to have PTCH or not.
     */
    *enabled = (ptc_profile == DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_PTCH2);
    ...
    uint32 * parsed_size)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(sand_signal_parse_get
                             (unit, pdb_entry->struct_name, pdb_entry->block_name, pdb_entry->stage_from_name, NULL,
                              PRINT_BIG_ENDIAN, data, pdb_entry->size, pit),
                             "Couldn't find structure '%s'.%s%s", pdb_entry->struct_name, EMPTY, EMPTY);
    /*
    ...
    int * is_tpid)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        return pparse_dnx_eth_parser_tpid_val_check(unit, tpid_val, is_tpid);
    }
    else
#endif
    {
        return pparse_dpp_eth_parser_tpid_val_check(unit, tpid_val, is_tpid);
    }
}
...
#ifdef BCM_DNX_SUPPORT
static shr_error_e
pparse_mpls_speculative_prohibit_cw_get_dnx(
    int unit,
    int * prohibit_cw)
{
    SHR_FUNC_INIT_VARS(unit);
...
static shr_error_e
pparse_mpls_speculative_prohibit_cw_get(
    int unit,
    int * prohibit_cw)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (SOC_IS_DNX(unit))
    {
        SHR_IF_ERR_EXIT(pparse_mpls_speculative_prohibit_cw_get_dnx(unit, prohibit_cw));
    }
    else
#endif
    {
    ...
    uint32 * parsed_size)
{
    ...
    default:
        ...
        SHR_IF_ERR_EXIT(pparse_mpls_speculative_prohibit_cw_get(unit, &prohibit_cw));
