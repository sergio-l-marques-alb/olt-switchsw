...
static char* sand_intr_corr_act_name[] = {
    ...
    "MDB ECC 1B FIX",
    "MDB ECC 2B FIX",
    "Delete BDB FIFO FULL",   /*SAND_INT_CORR_ACT_DELETE_BDB_FIFO_FULL*/
...
/**
 * \brief
 *   map soc block type to sand block index.
 * \param [in] blktype - soc block type.
 * \return
 *   \sand block index. used to be index of sand_intr_blocks_control_info
 */
...
/**
 * \brief
 *   iter all register to get gmo register and ecc monitor memory register of each block.
 * \param [in] unit - The unit number.
 * \param [in] ainfo - soc register address info.
 * \param [out] data - pointer of block control info structure.
 * \return
...
int sand_set_block_control_regs_info_cb(int unit, soc_regaddrinfo_t* ainfo, void* data)
{
    ...
    soc_block_type_t block = sand_blktype_to_index(SOC_REG_FIRST_BLK_TYPE(reg_info.block));
...
/**
 * \brief
 *   collect block control info. include gmo/ecc monister register; ecc&parity interrupt id.
 * \param [in] unit - The unit number.
 * \return
...
int sand_collect_blocks_control_info(int unit)
{
    ...
    rv = soc_reg_iterate(unit, sand_set_block_control_regs_info_cb, map); 
    if (SOC_FAILURE(rv)) 
    {
...
/**
 * \brief
 *   dump block control info.
 * \param [in] unit - The unit number.
 * \map [in] unit - The point of block control info.
 * \return
 *   \retval Zero if no error was detected. and block info was collect to sand_intr_blocks_control_info
 *   \retval Negative if error was detected. See \ref shr_error_e
 */
        ...
        soc_reg_above_64_val_t* monitor_mask)
{
    ...
    /* gmo register, set CPU_BYPASS_ECC_PARf 0 */
...
int sand_re_enable_block_mem_monitor(int unit, int interrupt_id, soc_mem_t mem, int copyno, soc_reg_above_64_val_t* monitor_mask)
{
    ...
    /* gmo register, resume CPU_BYPASS_ECC_PARf field */
    monitor_mem_reg = sand_intr_blocks_control_info[unit][block].gmo_reg;
    if ((SOC_REG_IS_VALID(unit, monitor_mem_reg)) && (SOC_REG_FIELD_VALID(unit, monitor_mem_reg, CPU_BYPASS_ECC_PARf)))
    {
