...
int cprif_drv_tx_vsd_ctrl_config_get(int unit, int port, cprif_cpri_vsd_config_t* config_info)
{
    ...
    SOC_IF_ERROR_RETURN
        (READ_CPRI_TXFRM_CWA_VSD_CTRL_RSRV_MASK1_CTRL2r(unit, port, &config_info->rsvd_mask[0]));
...
int cprif_drv_fast_clk_bit_time_period_set(int unit, int port, cprimod_port_speed_t speed)
{
    ...
    fast_clk_tp = 0;
    soc_reg_field_set(unit, CPRI_FAST_CLK_CTRLr, &fast_clk_tp, FAST_CLK_PERIODf, fast_clk);
