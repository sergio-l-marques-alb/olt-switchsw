...
static
cprimod_port_speed_t _cprif_map_rsvd4_speed_to_cpri (cprimod_port_rsvd4_speed_mult_t speed)
{
    switch (speed) {
    case cprimodRsvd4SpdMult4X:
        return cprimodSpd3072p0;
    case cprimodRsvd4SpdMult8X:
        return cprimodSpd6144p0;
    default:
        return cprimodSpd6144p0;
    }
    return cprimodSpd6144p0;
                                             ...
                                             cprimod_rsvd4_container_config_t* config_info)
{
    ...
    if (entry.hfrm_sync == 1) {
        if (entry.msg_ts_sync == 1){
            ...
            config_info->frame_sync_mode = cprimodRsvd4FrameSyncModeNoSync;
        }
    } else {
        if (entry.rfrm_sync == 1) {
            if (entry.msg_ts_sync == 1){
                ...
                config_info->frame_sync_mode = cprimodRsvd4FrameSyncModeMsgOffset;
            }
        } else {
            if (entry.msg_ts_sync == 1){
                config_info->frame_sync_mode = cprimodRsvd4FrameSyncModeMsgTsMsgAndMasterOffset;
            } else {
                config_info->frame_sync_mode = cprimodRsvd4FrameSyncModeMsgAndMasterOffset;
            }
        }
    }
...
int cprif_tx_framer_tgen_next_bfn_set(int unit, int port, uint32 tgen_bfn)
{
    SOC_INIT_FUNC_DEFS;
...
int cprif_tx_framer_tgen_next_bfn_get(int unit, int port, uint32* tgen_bfn)
{
    SOC_INIT_FUNC_DEFS;
...
int cprif_tx_framer_tgen_next_hfn_set(int unit, int port, uint32 tgen_hfn)
{
    SOC_INIT_FUNC_DEFS;
...
int cprif_tx_framer_tgen_next_hfn_get(int unit, int port, uint32* tgen_hfn)
{
    SOC_INIT_FUNC_DEFS;
...
int cprif_fast_clk_bit_time_period_set(int unit, int port, cprimod_port_speed_t speed)
{
    SOC_INIT_FUNC_DEFS;
    _SOC_IF_ERR_EXIT(
        cprif_drv_fast_clk_bit_time_period_set(unit, port, speed));
                               ...
                               cprimod_port_speed_t speed)
{
    ...
    SOC_IF_ERROR_RETURN(cprif_drv_cpri_rxpcs_speed_set(unit, port, phy,
                                                       speed));
    SOC_IF_ERROR_RETURN(cprif_fast_clk_bit_time_period_set(unit, port,
                                                       speed));
                               ...
                               cprimod_port_speed_t speed)
{
    ...
    SOC_IF_ERROR_RETURN(cprif_drv_cpri_txpcs_speed_set(unit, port, phy,
                                                       speed));
    SOC_IF_ERROR_RETURN(cprif_fast_clk_bit_time_period_set(unit, port,
                                                       speed));
...
{
    cprimod_port_speed_t cspeed;
    SOC_INIT_FUNC_DEFS;

    cspeed = _cprif_map_rsvd4_speed_to_cpri(speed);
    _SOC_IF_ERR_EXIT(cprif_rx_axc_basic_frame_clear(unit, port));
    SOC_IF_ERROR_RETURN(cprif_drv_rsvd4_rxpcs_speed_set(unit, port, phy, speed));
    SOC_IF_ERROR_RETURN(cprif_fast_clk_bit_time_period_set(unit, port, cspeed));
    /* add following
                    ...
                    const cprimod_port_init_config_t* port_config)
{
    ...
    if (table == cprimod_basic_frame_table_0) {
        _SOC_IF_ERR_EXIT
            (cprimod_basic_frame_parser_active_table_set(unit, port,cprimod_basic_frame_table_1, TRUE));
    } else {
        _SOC_IF_ERR_EXIT
            (cprimod_basic_frame_parser_active_table_set(unit, port,cprimod_basic_frame_table_0, TRUE));
    }
    _SOC_IF_ERR_EXIT
            (cprimod_basic_frame_assembly_active_table_get(unit, port,&table));
    if (table == cprimod_basic_frame_table_0) {
        _SOC_IF_ERR_EXIT
            (cprimod_basic_frame_assembly_active_table_set(unit, port,cprimod_basic_frame_table_1, TRUE));
    } else {
        _SOC_IF_ERR_EXIT
            (cprimod_basic_frame_assembly_active_table_set(unit, port,cprimod_basic_frame_table_0, TRUE));
    }
...
int cprif_cpri_port_rx_vsd_ctrl_flow_get(int unit, int port, uint32 group_id, cprimod_cpri_rx_vsd_flow_info_t* config_info)
{
    ...
    /* get the valid and matching group information. */
    for ( index=0 ; index < CPRIF_VSD_CTRL_MAX_NUM_SECTOR ; index++) {
        if (CPRIF_PBMP_IS_SET(valid_sector_mask,index)) {
            _SOC_IF_ERR_EXIT
                (cprif_drv_rx_vsd_ctrl_flow_config_get(unit, port, index, &flow_config));

            if (flow_config.group_id == group_id) {
                num_sector++;
                CPRIF_PBMP_IDX_MARK(sector_mask, index);
    ...
    config_info->num_sector = num_sector;
    ...
    if (config_info->num_sector != num_sector+1) {
        LOG_CLI(("WARN In cprif_cpri_port_rx_vsd_ctrl_flow_get() group_id  = %d  Discrepency in number of sectors %d %d.\n",
                                                    group_id,
...
int cprif_cpri_port_tx_vsd_ctrl_flow_add(int unit, int port, uint32 group_id, const cprimod_cpri_tx_vsd_flow_info_t* config_info)
{
    ...
    uint32 num_sector=0;
    uint32 sector_index=0;
    uint32 assigned_sector;
    uint32 processed_sector[2]={0};
    int group_ptr;
    SOC_INIT_FUNC_DEFS;
    ...
    /* marked all used grp ptr. */
    for (group_ptr = 0 ; group_ptr < CPRIF_VSD_CTRL_NUM_GROUP_PTR; group_ptr++) {
        _SOC_IF_ERR_EXIT
            (cprif_drv_tx_vsd_ctrl_group_assign_ptr_get(unit, port,
                                                        group_ptr,
                                                        &assigned_sector));
        if (!CPRIF_PBMP_IS_SET(processed_sector, assigned_sector)) {

            /*
             * Convert sector number to section number.
             * assigned_sector need to incrment one to accomodate zero based.
             */
            sector_index = 0;
            for (section = 0; (section < CPRIF_VSD_CTRL_MAX_NUM_SECTION) && (sector_index < assigned_sector+1) ; section++)
            {
                if (CPRIF_PBMP_IS_SET(rsvd_mask, section)) {
                    sector_index++;
                }
            ...
            section--;
            /*
             * If there is matching sector and it is valid.
             */
            if ((sector_index == assigned_sector+1) && (CPRIF_PBMP_IS_SET(ctrl_mask,section))) {
                for (group = 0 ; group < CPRIF_VSD_CTRL_NUM_GROUPS; group++) {
                    _SOC_IF_ERR_EXIT
                        (cprif_drv_tx_vsd_ctrl_group_config_get(unit, port,
                                                                ...
                                                                &grp_ptr_index));
                    for(index = 0; index < (num_sector + 1); index++){
                        if (group_ptr == (grp_ptr_index+index)) {
                            CPRIF_PBMP_IDX_MARK(processed_sector, assigned_sector);
                            if (group  != group_id) {
                                CPRIF_PBMP_IDX_MARK(grp_ptr_mask, grp_ptr_index + index);
                            }
                        }
...
int cprif_cpri_port_tx_vsd_ctrl_flow_get(int unit, int port, uint32 group_id, cprimod_cpri_tx_vsd_flow_info_t* config_info)
{
    uint32 num_sector=0;
...
int cprif_cpri_agnostic_mode_basic_frame_config_set(int unit, int port, cprimod_direction_t dir, int enable)
{
    ...
    if (dir == CPRIMOD_DIR_RX) {
        ...
        _SOC_IF_ERR_EXIT
            (cprif_drv_rx_framer_agnostic_mode_set(unit, port, 0 /* AxC_ID */, enable, cprimod_agnostic_mode_cpri));
...
#endif
    _SOC_IF_ERR_EXIT
        (cprif_drv_port_link_down_reset(unit, port));
exit:
    SOC_FUNC_RETURN;
...
int cprif_cpri_port_rx_tunnel_mode_set(int unit, int port, int enable, uint32 payload_size)
{
    ...
    _SOC_IF_ERR_EXIT
        (cprif_drv_rxpcs_tunnel_mode_set(unit, port, enable, timestamp_interval));
...
int cprif_cpri_port_tx_agnostic_mode_set(int unit, int port, int enable, cprimod_agnostic_mode_type_t mode, uint32 payload_size)
{
    ...
    timestamp_interval = payload_size;
    _SOC_IF_ERR_EXIT
        (cprif_drv_txpcs_agnostic_mode_set(unit, port, enable, mode,  timestamp_interval));
