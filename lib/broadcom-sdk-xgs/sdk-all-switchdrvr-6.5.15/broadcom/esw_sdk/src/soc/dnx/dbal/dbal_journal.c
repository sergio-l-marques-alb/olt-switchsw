 ...
 */
#define DNX_DBAL_JOURNAL_ACCESS_NOTIFY(_unit)                                                           \
    do {                                                                                                \
        if(NULL != dnx_dbal_journal_manager[unit].journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].access_cb) {\
            dnx_dbal_journal_manager[unit].journals[DNX_DBAL_COMPARISON_JOURNAL_NUM].access_cb(_unit);  \
        }                                                                                               \
        if(NULL != dnx_dbal_journal_manager[unit].journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].access_cb) {  \
            dnx_dbal_journal_manager[unit].journals[DNX_DBAL_ROLLBACK_JOURNAL_NUM].access_cb(_unit);    \
        }                                                                                               \
    } while(0)
#else
#define DNX_DBAL_JOURNAL_ACCESS_NOTIFY(_unit)
    ...
    char *stamp)
{
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "dbal journal ERROR: null entry found in journal.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
#endif
    ...
    uint8 *payload)
{
    ...
    {
        case DBAL_ACTION_ENTRY_CLEAR:
            LOG_CLI((BSL_META("Entry type: CLEAR\n")));
            break;
        case DBAL_ACTION_ENTRY_COMMIT:
            LOG_CLI((BSL_META("Entry type: COMMIT\n")));
            break;
        default:
            SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "dbal journal ERROR: unrecognized entry type.\n%s%s%s", EMPTY,
                                     EMPTY, EMPTY);
    ...
    char *stamp)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_dbal_journal_handle_trigger_value_mask(unit, ptr_payload, get_entry_handle));
            ...
            entries_match =
                (sal_memcmp
                 (ptr_payload->phy_entry.payload, get_entry_handle->phy_entry.payload,
                  DBAL_PHYSICAL_RES_SIZE_IN_BYTES) == 0);
        }
...
dnx_dbal_rollback_journal_entry_state_get(
    ...
    dnxc_rollback_journal_entry_state * entry_state)
{
    DNX_DBAL_JOURNAL_DEFS;
        ...
        || (DBAL_ACCESS_METHOD_SW_STATE == table->access_method))
    {
         ...
         */
        *entry_state = DNXC_ROLLBACK_JOURNAL_ENTRY_STATE_ALWAYS_VALID;
    }
             ...
             || (DBAL_ACCESS_METHOD_PEMLA == table->access_method) || (DBAL_ACCESS_METHOD_KBP == table->access_method))
    {
         ...
         */
        *entry_state = DNXC_ROLLBACK_JOURNAL_ENTRY_STATE_VALID;
    }
    else
    {
        /*
         ...
         */
        *entry_state = DNXC_ROLLBACK_JOURNAL_ENTRY_STATE_INVALID;
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL,
    ...
    uint8 is_cmp)
{
    ...
    dnxc_rollback_journal_entry_state entry_state = DNXC_ROLLBACK_JOURNAL_ENTRY_STATE_VALID;

    DNX_DBAL_JOURNAL_DEFS;

    journal_entry.action = action;

    if (is_er && !dnx_dbal_journal_is_table_excluded(unit, ptr_handle->table_id, FALSE))
    {
        /*
    ...
    dnx_err_recovery_journal_e journals)
{
    switch (journals)
    {
        case DNX_DBAL_JOURNAL_ALL:
            return (dnxc_rollback_journal_is_on(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE)
                    || dnxc_rollback_journal_is_on(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
        case DNX_DBAL_JOURNAL_ROLLBACK:
            return dnxc_rollback_journal_is_on(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE);
        case DNX_DBAL_JOURNAL_COMPARISON:
            return dnxc_rollback_journal_is_on(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE);
        default:
            return FALSE;
    }
}
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_destroy(unit, &DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_destroy(unit, &DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_clear(unit, only_head_clear, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_clear(unit, only_head_clear, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_distinct
                        (unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE, &dnx_dbal_comparison_journal_entries_are_distinct));

        SHR_IF_ERR_EXIT_NO_MSG(dnxc_rollback_journal_rollback
                               (unit, only_head_rollback, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_rollback(unit, only_head_rollback, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint32 table_id)
{
    ...
    return (    /* 
               ...
               || (DBAL_TABLE_SAT_REPORT_EVEN_CONFIG == table_id)
               || (DBAL_TABLE_SAT_REPORT_ODD_CONFIG == table_id)
               || (DBAL_TABLE_SAT_RX_EXP_SEQ_NUMBER_EVEN_GET == table_id)
               || (DBAL_TABLE_SAT_RX_EXP_SEQ_NUMBER_ODD_GET == table_id)
               || (DBAL_TABLE_SAT_RX_FLOW_EVEN_SET == table_id)
               || (DBAL_TABLE_SAT_RX_FLOW_ODD_SET == table_id)
               || (DBAL_TABLE_SAT_RX_PKT_PARAM_EVEN_CONFIG == table_id)
               || (DBAL_TABLE_SAT_RX_PKT_PARAM_ODD_CONFIG == table_id) || (DBAL_TABLE_HBMC_INTERRUPTS == table_id)
               /*
                * <----- end of b0 cmodel failures ----->
                */
               || (DBAL_TABLE_CRPS_ENGINE_ENABLE == table_id) || (DBAL_TABLE_SCH_FLOW_SHAPER_DYNAMIC_TABLE == table_id)
               /*
    ...
    uint32 er_flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_entry_insert
                    (unit, ptr_handle, action, &dnx_dbal_journal_rollback_common, is_er, is_cmp));

    DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION.dbal_access_region_is_entry_inserted = TRUE;
    ...
    uint32 *seq_id)
{
    ...
    if (is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_get_head_seq_id(unit, seq_id, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_get_head_seq_id(unit, seq_id, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (get_handle == NULL)
    {
        SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common(unit, entry_handle, DBAL_ACTION_ENTRY_CLEAR, entry_handle->er_flags));
    }
    else
    {
        /*
        ...
        get_handle->is_entry_update = 1;
        get_handle->entry_merged = 1;
        SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common(unit, get_handle, DBAL_ACTION_ENTRY_COMMIT, entry_handle->er_flags));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    DNX_DBAL_JOURNAL_ACCESS_NOTIFY(unit);

    if(0 != entry_handle->er_flags)
    {
        SHR_IF_ERR_EXIT(dnx_dbal_journal_log_add_internal(unit, get_handle, entry_handle));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_log_common(unit, entry_handle, DBAL_ACTION_ENTRY_COMMIT, entry_handle->er_flags));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if(0 != entry_handle->er_flags)
    {
        SHR_IF_ERR_EXIT(dnx_dbal_journal_log_clear_internal(unit, entry_handle));
    }
    ...
    uint8 is_comparison)
{
    ...
    if (!dnxc_rollback_journal_is_on(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE))
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL,
 ...
 */
shr_error_e
dnx_dbal_journal_invalidate_last_on_err(
    int unit,
    dbal_action_access_type_e access_type,
    dbal_action_access_func_e access_func,
    int error)
{
    uint8 entry_inserted = (access_func == DBAL_ACTION_ACCESS_FUNC_ENTRY_COMMIT)
        || (access_func == DBAL_ACTION_ACCESS_FUNC_ENTRY_CLEAR);

    SHR_FUNC_INIT_VARS(unit);

    if (entry_inserted
        && (_SHR_E_NONE != error) && (DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION.dbal_access_region_is_entry_inserted))
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_invalidate_last_inserted_entry(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }

    if (entry_inserted)
    {
        DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION.dbal_access_region_is_entry_inserted = FALSE;
    }

exit:
    if (entry_inserted && (DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION.dbal_access_region_is_entry_inserted))
    {
        DNX_DBAL_ROLLBACK_JOURNAL_TRANSACTION.dbal_access_region_is_entry_inserted = FALSE;
    }
    SHR_FUNC_EXIT;
    ...
    uint8 is_comparison)
{
    ...
    if (!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_print(unit, DNX_DBAL_ROLLBACK_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_print(unit, DNX_DBAL_COMPARISON_JOURNAL_HANDLE));
    }
