...
/** 
 * The following two MACROs are used to shut down the ECC for CS TCAMS while accessing them (HWJERICHO2-2415),
 * according to cs_tcam_ecc_turn_off_while_access feature.
 */
/**
...
{ \
  ...
  if (dnx_data_ingress_cs.hw_workarounds.feature_get(_tcam_cs_unit, dnx_data_ingress_cs_hw_workarounds_disable_ecc_fix_en_before_read_write)) \
  { \
    ...
    char *action)
{
    ...
    int is_tcam = (entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT) ? 1 : 0;
    int logger_action = 0;
    ...
    uint32 k_mask[DBAL_PHYSICAL_KEY_SIZE_IN_WORDS])
{
     ...
     * Take the DATA_OUT from *_TCAM_BANK_REPLYm at odd line (line_index*2+1), to retrive the MASK VALUE
     */
    SHR_IF_ERR_EXIT(soc_mem_write(unit, stage_info->cmd_memory, MEM_BLOCK_ANY, ((line_index * 2) + 1), mem_val));
    SHR_IF_ERR_EXIT(soc_mem_read(unit, stage_info->cmd_memory, MEM_BLOCK_ANY, ((line_index * 2) + 1), mem_val));
    ...
    *valid_bits = soc_mem_field32_get(unit, stage_info->reply_memory, mem_val, stage_info->reply_valid_field);
    ...
    /*
     * Sanity check: Checking that not half entry valif bits found if half entries are not supported in the TCAM.
     */
    ...
    SHR_IF_ERR_EXIT(soc_mem_read(unit, stage_info->cmd_memory, MEM_BLOCK_ANY, line_index * 2, mem_val));

    /*
     * Take the DATA_OUT from *_TCAM_BANK_REPLYm at even line (line_index*2), to retrive the KEY VALUE
     */
    SOC_REG_ABOVE_64_CLEAR(data_out);
    soc_mem_field_get(unit, stage_info->reply_memory, mem_val, stage_info->reply_data_out_field, data_out);
    ...
    dbal_entry_handle_t * entry_handle)
{
     ...
     * Read the entry.
     */
    rv = dbal_tcam_cs_key_read_full_line(unit, stage_info, (entry_num / 2), &valid_bits, key, k_mask);
    ...
    dbal_actions_e dbal_action)
{
    ...
    SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_tcam_cs_key_read_full_line
                              (unit, stage_info, (entry_num / 2), &valid_bits_read, key, k_mask), _SHR_E_NOT_FOUND);
    ...
    SHR_IF_ERR_EXIT(dbal_tcam_cs_key_write_full_line
                    (unit, stage_info, (entry_num / 2), valid_bits_to_write, key, k_mask));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dbal_hl_offset_calculate(unit, entry_handle, INVALIDf, 0,
                                             &(access_info.l2p_fields_info[0].array_offset_info), &array_offset));

    SHR_IF_ERR_EXIT(dbal_hl_mem_block_calc(unit, entry_handle, entry_handle->core_id, memory,
                                           &(access_info.l2p_fields_info[0].block_index_info),
                                           access_info.l2p_fields_info[0].field_id, &block, &num_of_blocks));
    ...
    if (access_info.l2p_fields_info[0].hw_field != INVALIDf)
    {
        /** Get field offset by HW field */
        soc_mem_info_t *meminfo = &SOC_MEM_INFO(unit, memory);
        SOC_FIND_FIELD(access_info.l2p_fields_info[0].hw_field, meminfo->fields, meminfo->nFields, fieldinfo);
        offset = fieldinfo->bp;
        LOG_VERBOSE_EX(BSL_LOG_MODULE, "Offset decided by field %-30s, field offset %d size %d %s\n",
                       SOC_FIELD_NAME(unit, access_info.l2p_fields_info[0].hw_field), offset, data_size, EMPTY);
    }
    else
    {
        /** Get field offset by data offset */
        SHR_IF_ERR_EXIT(dbal_hl_offset_calculate
                        (unit, entry_handle, INVALIDf, 0, &(access_info.l2p_fields_info[0].data_offset_info), &offset));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (access_info.l2p_fields_info[0].hw_field != INVALIDf)
    {
        ...
        SOC_FIND_FIELD(access_info.l2p_fields_info[0].hw_field, meminfo->fields, meminfo->nFields, fieldinfo);
        offset = fieldinfo->bp;
        LOG_INFO_EX(BSL_LOG_MODULE, "offset decided by field %-30s, field offset %d size %d %s\n",
                    SOC_FIELD_NAME(unit, access_info.l2p_fields_info[0].hw_field), offset, table->max_payload_size,
                    EMPTY);
