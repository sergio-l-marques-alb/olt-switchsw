 ...
 */
extern shr_error_e jer2_a0_data_ingress_cs_attach(
    int unit);
/*
 ...
 */
static shr_error_e
dnx_data_ingress_cs_hw_workarounds_init(
    int unit,
    dnxc_data_submodule_t *submodule_data)
{
    SHR_FUNC_INIT_VARS(unit);
     ...
     * Features
     */
    submodule_data->nof_features = _dnx_data_ingress_cs_hw_workarounds_feature_nof;
    DNXC_DATA_ALLOC(submodule_data->features, dnxc_data_feature_t,  submodule_data->nof_features, "_dnxc_data ingress_cs hw_workarounds features");

    submodule_data->features[dnx_data_ingress_cs_hw_workarounds_disable_ecc_fix_en_before_read_write].name = "disable_ecc_fix_en_before_read_write";
    submodule_data->features[dnx_data_ingress_cs_hw_workarounds_disable_ecc_fix_en_before_read_write].doc = "Disables the ECC fixing mechanism before reading or writing to a CS TCAM (HWJERICHO2-2415).";
    submodule_data->features[dnx_data_ingress_cs_hw_workarounds_disable_ecc_fix_en_before_read_write].flags |= DNXC_DATA_F_FEATURE;

    /*
     * Defines
     */
    submodule_data->nof_defines = _dnx_data_ingress_cs_hw_workarounds_define_nof;
    DNXC_DATA_ALLOC(submodule_data->defines, dnxc_data_define_t, submodule_data->nof_defines, "_dnxc_data ingress_cs hw_workarounds defines");

    /*
     * Tables
     */
    submodule_data->nof_tables = _dnx_data_ingress_cs_hw_workarounds_table_nof;
    DNXC_DATA_ALLOC(submodule_data->tables, dnxc_data_table_t, submodule_data->nof_tables, "_dnxc_data ingress_cs hw_workarounds tables");
 ...
 */
int
dnx_data_ingress_cs_hw_workarounds_feature_get(
    int unit,
    dnx_data_ingress_cs_hw_workarounds_feature_e feature)
{
    return dnxc_data_mgmt_feature_data_get(unit, dnx_data_module_ingress_cs, dnx_data_ingress_cs_submodule_hw_workarounds, feature);
}
    ...
    dnxc_data_module_t *module_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_data_ingress_cs_hw_workarounds_init(unit, &module_data->submodules[dnx_data_ingress_cs_submodule_hw_workarounds]));
    /*
     * Attach device module
     */
    if (dnxc_data_mgmt_is_jer2_a0(unit))
    {
        SHR_IF_ERR_EXIT(jer2_a0_data_ingress_cs_attach(unit));
    }
    else
