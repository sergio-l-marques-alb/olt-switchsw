  ...
  )
{
  ...
  /*
   */
  if (_SHR_E_NONE != dnx_sand_delta_list_destroy(Dnx_soc_sand_handles_list) )
  {
    ex = _SHR_E_INTERNAL ;
    goto exit ;
  }
  ...
  )
{
  ...
  /*
   */
  if (_SHR_E_NONE != dnx_sand_handles_delta_list_take_mutex())
  {
    ex = _SHR_E_INTERNAL ;
  ...
  {
    if (_SHR_E_NONE != dnx_sand_delta_list_insert_d(Dnx_soc_sand_handles_list, 0, (void *)item))
    {
      ex = _SHR_E_INTERNAL;
      dnx_sand_os_free(item) ;
      goto exit ;
  ...
  if (_SHR_E_NONE != dnx_sand_handles_delta_list_give_mutex())
  {
    ex = _SHR_E_INTERNAL;
    goto exit ;
  }
  ...
  )
{
  ...
  switch (item->int_or_poll)
  {
    case 1:
    {
      if (_SHR_E_NONE != dnx_sand_tcm_unregister_polling_callback(item->dnx_sand_polling_handle))
      {
        ex = _SHR_E_INTERNAL ;
        goto exit ;
      }
    ...
    case 2:
    {
      if (_SHR_E_NONE != dnx_sand_unregister_event_callback(item->dnx_sand_interrupt_handle))
      {
        ex = _SHR_E_INTERNAL ;
        goto exit ;
      }
   ...
   */
  item->valid_word = 0 ;
  if (_SHR_E_NONE != dnx_sand_delta_list_remove(Dnx_soc_sand_handles_list, (void *)item))
  {
    ex = _SHR_E_INTERNAL ;
    goto exit ;
  }
  ...
  )
{
  ...
  if (dnx_sand_delta_list_is_empty(Dnx_soc_sand_handles_list))
  {
    ex = _SHR_E_INTERNAL ;  /* empty list of handles is legal */
    goto exit ;
  }
  /*
   */
  ...
  while(NULL != curr_node)
  {
    ...
    if (unit == curr_item->unit)
    {
      ...
      switch (curr_item->int_or_poll)
      {
        case 1:
        {
          shr_error_e = dnx_sand_tcm_unregister_polling_callback(curr_item->dnx_sand_polling_handle) ;
          if (_SHR_E_NONE != shr_error_e)
          {
            ex = _SHR_E_INTERNAL;
            goto exit_semaphore ;
          }
        ...
        case 2:
        {
          shr_error_e = dnx_sand_unregister_event_callback(curr_item->dnx_sand_interrupt_handle) ;
          if (_SHR_E_NONE != shr_error_e)
          {
            ex = _SHR_E_INTERNAL;
            goto exit_semaphore ;
