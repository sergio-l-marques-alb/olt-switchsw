...
#define DBAL_CHECK_LOGGER_SEVIRTY_WITH_INFO_VOID()									\
do                                                                                  \
{																					\
	int rv1, rv2; 																	\
	rv1 = bsl_check(bslLayerSocdnx, bslSourceDbaldnx, bslSeverityInfo, unit);		\
	rv2 = bsl_check(bslLayerSocdnx, bslSourceDbalaccessdnx, bslSeverityInfo, unit);	\
	if((!rv1) && (!rv2)) 															\
	{																				\
		return;																		\
	}  												 								\
...
#define DBAL_CHECK_LOGGER_SEVIRTY_WITH_INFO_ERR()									\
do                                                                                  \
{																					\
	int rv1, rv2; 																	\
	rv1 = bsl_check(bslLayerSocdnx, bslSourceDbaldnx, bslSeverityInfo, unit);		\
 ...
 */
uint8 dbal_is_j2c(int unit)
{
    return (SOC_IS_J2C(unit));
}

uint8 dbal_is_j2a0(int unit)
{
    return (SOC_IS_JERICHO_2_A0(unit));
}

uint8 dbal_is_j2b0(int unit)
{
    return (SOC_IS_JERICHO_2_B0(unit));
}
...
{
    int ii;
    if (dbal_mngr[unit].status > DBAL_STATUS_NOT_INITIALIZED)
    {
        return TRUE;
    }
    ...
    int entry_handle_id)
{
    ...
    {
        if (table->keys_info[ii].const_value_valid)
        {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        if (DBAL_RES_INFO.results_info[ii].const_value_valid)
        {
            dbal_entry_value_field_set(unit, entry_handle->handle_id, DBAL_RES_INFO.results_info[ii].field_id,
                                       0, DBAL_POINTER_TYPE_UINT32, &DBAL_RES_INFO.results_info[ii].const_value,
    ...
    uint8 operation)
{
    ...
    {
        dbal_status_e dbal_status = DBAL_STATUS_NOT_INITIALIZED;
        ...
        if (dbal_status < DBAL_STATUS_DEVICE_INIT_DONE)
        {
            LOG_INFO(BSL_LOG_MODULE, (BSL_META_U(unit,
                                                 ...
                                                 "operations on this table are not validated (if table was validated please update XML)\n"),
                                      table->table_name));
        }
    }
    ...
    if(dnx_state_snapshot_manager_is_on(unit))
    {
    ...
    uint32 entry_handle_id)
{
    ...
    else
    {
        ...
        sal_memset(&(dbal_mngr[unit].entry_handles_pool[entry_handle_id]), 0x0, sizeof(dbal_entry_handle_t));
    }
exit:
    SHR_IF_ERR_EXIT(sal_mutex_give(dbal_mngr[unit].handle_mutex));
    SHR_FUNC_EXIT;
    ...
    uint32 update_entry_handle_id)
{
    ...
    sal_memcpy(&update_entry_handle->phy_entry.payload, &orig_entry_handle->phy_entry.payload,
               DBAL_PHYSICAL_RES_SIZE_IN_WORDS * sizeof(uint32));
...
/** field core ID can be set even if it is not exists on the table incase that the tabgle is SBC */
static shr_error_e
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_SET);
    ...
    if (table->core_mode == DBAL_CORE_MODE_SBC && field_id == DBAL_FIELD_CORE_ID)
    {
        DBAL_FIELD_ERR_HANDLE(dbal_entry_key_field_core_id_set(unit, entry_handle, field_val[0]),
                              "error core_id field");
        DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_SET);
        return;
    }
    ...
    DBAL_FIELD_ERR_HANDLE((table_field == NULL), "table field is NUL, KEY field not found");
        ...
        /** validate legal bit size according to the pointer type, (if the field is too big for the pointer type, returns
	 *  error) */
    ...
    if (field_max_size <= 32)
    {
        ...
        if (is_sub_field)
        {
            ...
            ret_val = dbal_fields_max_value_get(unit, field_id, &field_max_value);
            if (ret_val != _SHR_E_NONE)
            {
                DBAL_FIELD_ERR_HANDLE(TRUE, "Cannot get the field max value");
            }
            max_value = field_max_value;
        }

        dbal_fields_print_type_get(unit, field_id, &field_print_type);
                /**  for MAC address no need to validate. the field can be lower than 32 bit but the user can add bigger field, dbal
		 *   should take the mapped bits only */
        if ((field_print_type != DBAL_FIELD_PRINT_TYPE_MAC))
        {
            /**  fields up to 31 bits length will be validated here */
            if (field_val[0] > max_value)
            {
                DBAL_FIELD_ERR_HANDLE_NO_RETURN(TRUE, "Illegal value for field, too big");
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "field value = %d max value %d"), field_val[0], max_value));
                DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_SET);
                return;
            }
            ...
            {
                DBAL_FIELD_ERR_HANDLE_NO_RETURN(TRUE, "Illegal value for field, too low");
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "field value = %d min value %d"), field_val[0],
        ...
        dbal_fields_is_illegal_value(unit, field_id, field_val[0], &is_illegal);
        DBAL_FIELD_ERR_HANDLE((is_illegal), "Illegal value for field, in illegals list");
    }
    ...
    if (field_mask)
    {
        ...
        if (is_sub_field)
        {
            ...
            ret_val = dbal_fields_sub_field_info_get_internal(unit, field_id, parent_field_id, &sub_field_info);
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_fields_sub_field_info_get_internal error");
        ...
        if (encode_type == DBAL_VALUE_FIELD_ENCODE_PREFIX)
        {
            ...
            uint32 mask_value;
            ret_val = dbal_fields_max_size_get(unit, field_id, &prefix_length);
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_fields_max_size_get error");

            mask_value = (1 << (nof_bits - prefix_length)) - 1;
			 
            if(entry_handle->table_id == DBAL_TABLE_OAMP_PE_PROGRAMS_TCAM)
            {
                 mask_value = 0;
            }
            ...
            ret_val =
                dbal_fields_encode_prefix(unit, field_mask, mask_value, prefix_length, nof_bits, encoded_field_mask);
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_fields_encode_prefix error");

            ret_val =
                utilex_bitstream_set_any_field(encoded_field_mask, offset, nof_bits,
        ...
        else if (encode_type == DBAL_VALUE_FIELD_ENCODE_SUFFIX)
        {
            ...
            ret_val = dbal_fields_encode_suffix
                (unit, field_mask, mask_value, field_max_size, nof_bits, encoded_field_mask);
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_fields_encode_suffix error");
        ...
        {
            ret_val =
                utilex_bitstream_set_any_field(field_mask, offset, nof_bits, &(entry_handle->phy_entry.k_mask[0]));
        }

        DBAL_FIELD_ERR_HANDLE(ret_val, "utilex_bitstream_set_any_field from offset error for field mask");
    }
    ...
    uint32 max_val)
{
    ...
    DBAL_FIELD_ERR_HANDLE(((entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM) ||
                           (entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)),
                          "Range actions are not supported for TCAM");

    DBAL_FIELD_ERR_HANDLE(dbal_tables_field_info_get(unit, entry_handle->table_id,
                                                     field_id, 1, entry_handle->cur_res_type, 0, &field_info),
                          "field not found for range");
    ...
    if (field_id == DBAL_FIELD_CORE_ID)
    {
        ...
        {
        DBAL_FIELD_ERR_HANDLE((TRUE), "field not found for range")}
    }
    ...
    if (!is_core_field)
    {
        if (!entry_handle->error_info.error_exists)
        {
            ...
            {
                if (field_id == entry_handle->key_field_ids[ii])
                {
                    entry_handle->key_field_ranges[ii] = max_val - min_val + 1;
                    entry_handle->nof_ranged_fields++;
                    break;
                }
            }
    ...
    int res_type)
{
    ...
    DBAL_FIELD_ERR_HANDLE(iterator_info->is_init, "RESULT_TYPE field cannot be changed on handle with valid iterator");

    DBAL_FIELD_ERR_HANDLE(((res_type > entry_handle->table->nof_result_types)
    ...
    uint32 *field_mask)
{
     ...
     *  type to add the value (fir example for if the field_size is 32bit and with valid indication it is 33 bit user
     *  will add the value by uint32_set() (because its max value is valid) */
    if (!table_field->is_valid_indication_needed)
    {
    ...
    /** field value validations applicable only for fields smaller than 32 bits. for enums the validation is done after
     *  the decoding */
    if (field_max_size <= 32 && field_print_type != DBAL_FIELD_PRINT_TYPE_ENUM)
    {
        uint8 is_illegal = FALSE;
        uint32 max_value = table_field->max_value;
        ...
        {
            uint32 field_max_value = 0;
            DBAL_FIELD_ERR_HANDLE(dbal_fields_max_value_get(unit, field_id, &field_max_value),
        ...
        if (table_field->arr_prefix_size != 0)
        {
            ...
            ret_val = dbal_field_arr_prefix_decode(unit, table_field, field_val[0], field_val);
    ...
    {
        /** Copy the relevant part of the field */
        sal_memset(copied_field_value, 0, sizeof(copied_field_value));
        ret_val = utilex_bitstream_get_any_field(field_val, table_field->offset_in_logical_field,
                                                 table_field->field_nof_bits, copied_field_value);
    ...
    utilex_bitstream_set_any_field(curr_field_value_ptr, table_field->bits_offset_in_buffer,
                                   table_field->field_nof_bits, &(entry_handle->phy_entry.payload[0]));
    ...
    void *field_mask)
{
    ...
    DBAL_FIELD_ERR_HANDLE
        (((table->has_result_type) && (entry_handle->cur_res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED)),
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_GET);
...
exit:
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_GET);
    SHR_FUNC_EXIT;
}
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    else
    {
        ...
        dbal_fields_print_type_get(unit, field_id, &field_print_type);
        if ((table_field_info->arr_prefix_size) && (field_print_type != DBAL_FIELD_PRINT_TYPE_ENUM))
        {
    ...
    uint8 format)
{
    ...
    {
        int nof_key_fields = ((table->table_type == DBAL_TABLE_TYPE_TCAM) ||
                              (table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)) ?
            table->nof_key_fields : entry_handle->nof_key_fields;
        ...
        if (entry_handle->attribute_info[attr_index] != NULL)
        {
            ...
            switch (attr_index)
            {
                ...
                case DBAL_ENTRY_ATTR_RES_AGE:
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "DBAL_ENTRY_ATTR_AGE is not supported");
                    break;
    ...
    uint32 *returned_field_value)
{
    ...
    {
        if (table_field_info->is_valid_indication_needed)
        {
        ...
        {
            SHR_IF_ERR_EXIT(dbal_fields_field_decode(unit, field_id, parent_field_id, field_nof_bits, curr_field_value_ptr,
                                                     field_val_after_decoding));
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    dbal_field_types_basic_info_t *field_info, *sub_field_info;
    ...
    for (ii = 0; ii < field_type_info->nof_defines_values; ii++)
    {
        ...
        {
            char *field_name = dbal_field_types_def_to_string(unit, dbal_field_id_to_type[field_id]);
            /** this is used to cut the start of the string that is equal to the field name*/
            int str_len = sal_strlen(field_name) + 1;
    ...
    if (field_info->nof_child_fields != 0)
    {
        ...
        if (sub_field_id != DBAL_FIELD_EMPTY)
        {
            ...
            if (dbal_fields_is_field_encoded(unit, sub_field_id) || (encode_type == DBAL_VALUE_FIELD_ENCODE_STRUCT))
            {
            ...
            {
                sal_snprintf(buffer_to_print, DBAL_MAX_STRING_LENGTH, "%d (%d)%s", sub_field_val[0], field_val[0],
                             "\0");
                SHR_EXIT();
            }
        }
    ...
    switch (field_info->print_type)
    {
        case DBAL_FIELD_PRINT_TYPE_STRING:
        {
            ...
            utilex_U32_to_U8(field_val, DBAL_FIELD_ARRAY_MAX_SIZE_IN_BYTES, buf);
            ...
            else
            {
                char_count =
                    sal_snprintf(buffer_to_print, DBAL_MAX_STRING_LENGTH, "%s (0x%x) ",
                                 &(field_info->enum_val_info[enum_val].name[str_len]),
                                 field_info->enum_val_info[enum_val].value);
            }
            buffer_to_print += char_count;
            break;
        }
        ...
        {
            int ii;
            int nof_struct_fields = field_info->nof_struct_fields;
            ...
            {
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode(unit, field_id,
                                                                field_info->struct_field_info[ii].struct_field_id,
                                                                sub_struct_val, field_val));
                ...
                if (field_info->struct_field_info[ii].struct_field_id != DBAL_FIELD_RESERVED)
                {
                    dbal_fields_e next_level_field = field_info->struct_field_info[ii].struct_field_id;
                    ...
                    SHR_IF_ERR_EXIT(dbal_field_printable_string_get
                                    (unit, next_level_field, sub_struct_val, NULL, 1, DBAL_FIELD_EMPTY,
                                     field_info->struct_field_info[ii].length, buffer_to_print));
    ...
    uint32 *result)
{
    ...
    for (ii = 0; ii < formula->nof_vars; ii++)
    {
        if (formula->nof_vars == 1)
        {
            ...
            else if (formula->field_id != DBAL_FIELD_EMPTY)
            {
                ...
                /** read field value from entry buffer   */
                ...
                else
                {
                    ...
                    if (rv == _SHR_E_NONE)
                    {
                        ...
                        /*
                         * Enum Fields - need to be translate (again) to HW value
                         */
                        SHR_IF_ERR_EXIT(dbal_fields_print_type_get(unit, formula->field_id, &print_type));
                    ...
                    {
                        rv = dbal_tables_field_info_get_no_err(unit, table_id, formula->field_id, FALSE,
                                                               entry_handle->cur_res_type, 0, &field_info);
                        if (rv == _SHR_E_NONE)
                        {
                        ...
                        else if (rv == _SHR_E_NOT_FOUND)
                        {
                            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Field %s is not part of table %s\n",
                                         dbal_field_to_string(unit, formula->field_id),
                                         entry_handle->table->table_name);
                        }
                        else
                        {
    ...
    curr_result = vars[0];
    for (ii = 1; ii < formula->nof_vars; ii++)
    {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (bit_index = max_bit_index; bit_index >= min_bit_index; --bit_index)
    {
    ...
    uint8 *is_passed)
{
    ...
    for (cond_indx = 0; cond_indx < nof_conditions; cond_indx++)
    {
        ...
        if (condition.field_id != DBAL_FIELD_EMPTY)
        {
            if (is_key)
            {
                SHR_IF_ERR_EXIT(dbal_entry_key_field_get
                                (unit, entry_handle, condition.field_id, compared_value, NULL,
                                 DBAL_POINTER_TYPE_ARR_UINT32));
            }
            else
            {
                dbal_table_field_info_t field_info;
                int res = dbal_tables_field_info_get_no_err(unit, entry_handle->table_id, condition.field_id,
            ...
            SHR_IF_ERR_EXIT(dbal_tables_field_size_get
                            (unit, entry_handle->table_id, condition.field_id, is_key, entry_handle->cur_res_type, 0,
                             &bits_to_compare));
        }
        else
        {
        ...
        switch (condition.type)
        {
            ...
            case DBAL_CONDITION_NOT_EQUAL_TO:
                ...
                {
                    uint32 condition_value;
                    LOG_VERBOSE(BSL_LS_SOCDNX_DBALACCESSDNX,
                                (BSL_META_U(unit, "\t\tkey %d value %d\n"), compared_value[ii], condition.value[ii]));
    ...
    /** Check if hit bit rule was set */
    if(iterator_info->hit_bit_rule_valid == TRUE)
    {
        uint32 attr_val, is_hit;

        SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit,entry_handle->handle_id,
                                                 iterator_info->hit_bit_rule_type,
                                                 &attr_val));
        is_hit = attr_val != 0;
        if(is_hit != iterator_info->hit_bit_rule_is_hit)
        {
            *is_passed = 0;
            SHR_EXIT();
        }
    ...
    dbal_tables_e table_id)
{
    ...
    DBAL_CHECK_LOGGER_SEVIRTY_WITH_INFO_ERR();
...
dbal_logger_internal_disable_set(
    int unit)
{
    DBAL_CHECK_LOGGER_SEVIRTY_WITH_INFO_VOID();
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!table->has_result_type)
    {
        ...
        {
            entry_handle->user_output_info[ii].pointer_type = DBAL_POINTER_TYPE_ARR_UINT32;
    ...
    uint32 *field_mask)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_tables_field_info_get(unit, entry_handle->table_id, field_id, TRUE, 0, 0, &field_info));
        SHR_IF_ERR_EXIT(dbal_fields_print_type_get(unit, field_id, &field_print_type));
        if (field_print_type == DBAL_FIELD_PRINT_TYPE_STRING)
        {
            SHR_ERR_EXIT(_SHR_E_EMPTY, "Rule not supported for field print type string.\n");
        }
        field_max_size = field_info.field_nof_bits;
