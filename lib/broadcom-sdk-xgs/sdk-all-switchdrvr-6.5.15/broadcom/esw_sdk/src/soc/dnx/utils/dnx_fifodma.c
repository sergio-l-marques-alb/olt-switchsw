    ...
    void *host_buff)
{
    ...
    soc_reg_field_set(unit, CMIC_COMMON_POOL_FIFO_CH0_RD_DMA_CFGr, &rval, TIMEOUT_COUNTf, 1000);
    ...
    soc_pci_write(unit, CMIC_COMMON_POOL_FIFO_CHy_RD_DMA_HOSTMEM_THRESHOLD_OFFSET(ch), host_entries / 10);
    ...
    if (valid_count > 0)
    {
        ...
        for (i = 0; i < valid_count; i++)
        {
            ...
            if (callback)
            {
                 ...
                 */
                callback(unit, next_entry_in_host_memory, entry_size);
            }
            handled_count++;
        }
        last_entry_id += i;
