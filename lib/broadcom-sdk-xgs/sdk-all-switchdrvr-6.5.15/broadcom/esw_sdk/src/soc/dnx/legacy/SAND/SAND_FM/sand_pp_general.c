  ...
  )
{
  ...
  SHR_NULL_CHECK(subnet, _SHR_E_PARAM, "subnet");


  if (subnet->prefix_len > DNX_SAND_PP_IPV4_SUBNET_PREF_MAX_LEN)
  {
    SHR_ERR_EXIT(_SHR_E_INTERNAL, "SOC_SAND_PP_IPV4_SUBNET_PREF_OUT_OF_RANGE_ERR");
  }
  ...
  )
{
  ...
  SHR_NULL_CHECK(dest_info, _SHR_E_PARAM, "dest_info");
  SHR_NULL_CHECK(encoded_dest_val, _SHR_E_PARAM, "encoded_dest_val");
  ...
  )
{
  ...
  {
    SHR_ERR_EXIT(_SHR_E_INTERNAL, "SOC_SAND_ILLEGAL_IP_FORMAT");
  }
exit:
  SHR_FUNC_EXIT;
}
  ...
  )
{
  ...
  SHR_NULL_CHECK(ipv4_addr, _SHR_E_PARAM, "ipv4_addr");
