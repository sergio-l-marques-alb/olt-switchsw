    ...
    dbal_fields_e * field_id)
{
    ...
    for (field_index = 0; field_index < DBAL_NOF_FIELDS; field_index++)
    {
    ...
    dbal_tables_e * log_table_id)
{
    ...
    {
        if (sal_strcasecmp(dbal_table_strings[table_index], str) == 0)
        {
    ...
    for (table_index = 0; table_index < nof_tables; table_index++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(DBAL_TBL_IN_SW_STATE.table_name.get(unit, table_index, 0, &table_name));
...
shr_error_e
dbal_hw_entity_string_poting_check(
    int unit,
    int is_reg,
    char *curr_name,
    char *new_name)
{
    ...
    if (is_reg)
    {
        num_of_elements = hl_porting_info->nof_registers;
    }
    else
    {
        num_of_elements = hl_porting_info->nof_memories;
    }

    for (iter = 0; iter < num_of_elements; iter++)
    {
        element_name = is_reg ? hl_porting_info->register_name[iter] : hl_porting_info->memory_name[iter];
        if (sal_strcmp(curr_name, element_name) == 0)
        {
            if (is_reg)
            {
                sal_strcpy(new_name, hl_porting_info->register_mapped_name[iter]);
            }
            else
            {
                sal_strcpy(new_name, hl_porting_info->memory_mapped_name[iter]);
            }
            break;
        }
    }
    ...
    int *hw_entity_id)
{
    ...
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYSER_DBAL_LOGICAL_INIT_ADD_DBAL_TABLE_MAPPING_HL_MEM_REG_HW_ENTITY_STR);

    if (entity_type == DBAL_HL_ENTITY_REGISTER)
    {
        SHR_IF_ERR_EXIT(dbal_hw_entity_string_poting_check(unit, 1, str, hw_entity_name));
        if (sal_strcmp(hw_entity_name, EMPTY) != 0)
        {
            str_to_check = hw_entity_name;
        }
    ...
    {
        SHR_IF_ERR_EXIT(dbal_hw_entity_string_poting_check(unit, 0, str, hw_entity_name));
        if (sal_strcmp(hw_entity_name, EMPTY) != 0)
        {
            str_to_check = hw_entity_name;
        }
...
exit:
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYSER_DBAL_LOGICAL_INIT_ADD_DBAL_TABLE_MAPPING_HL_MEM_REG_HW_ENTITY_STR);
    SHR_FUNC_EXIT;
}
