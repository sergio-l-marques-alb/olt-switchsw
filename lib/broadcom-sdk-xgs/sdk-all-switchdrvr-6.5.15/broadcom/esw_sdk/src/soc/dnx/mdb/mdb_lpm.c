    ...
    uint32 db_id)
{
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        if ((big_kaps_enabled_global == FALSE) || (big_kaps_enabled[db_id] == TRUE))
        {
    ...
    uint32 *uint32_data)
{
    ...
    if ((nof_bytes == MDB_KAPS_KEY_WIDTH_IN_BYTES(unit)) || (nof_bytes == dnx_data_mdb.kaps.ads_byte_width_get(unit))
    ...
    dbal_physical_entry_t * entry)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_entry_lpm_add(unit,
                                          ...
                                          entry->is_update, entry->hitbit, FALSE));
    ...
    uint8 *is_end)
{
    ...
    else
    {
        ...
        while ((app_id != dnx_data_mdb.em.max_nof_tids_get(unit))
        ...
        if (kpb_e != NULL)
        {
            if (kpb_e_info.ad_handle != NULL)
            {
                ...
                if (physical_entry_iterator->hit_bit_flags & DBAL_PHYSICAL_KEY_HITBIT_ACTION)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_lpm_entry_get(unit, dbal_physical_table_id, app_id, entry));
                }
            }
            else
            {
    ...
    uint32 app_id)
{
    ...
    while (is_end == FALSE)
    {
        ...
        {
            SHR_IF_ERR_EXIT(mdb_lpm_entry_delete(unit, dbal_physical_table_id, app_id, &entries[current_entry]));
        }
    }
    ...
    dbal_physical_entry_t * entry)
{
    ...
    {
        entry->payload[0] = (index + 1) & ((1 << dnx_data_mdb.kaps.ad_width_in_bits_get(unit)) - 1);
    }
    ...
    int delete_cmd)
{
    ...
    while (ext_iterator < max_ext_iterations)
    {
        ...
        do
        {
            ...
            SHR_IF_ERR_EXIT(mdb_lpm_test_fill_entry(unit, entry_index, TRUE, &entry));

            timer_usec = sal_time_usecs();
            rv = mdb_lpm_entry_add(unit, dbal_physical_table_id, 0, &entry);
        ...
        do
        {
            ...
            SHR_IF_ERR_EXIT(mdb_lpm_test_fill_entry(unit, entry_index, TRUE, &entry));
            SHR_IF_ERR_EXIT(mdb_lpm_test_fill_entry(unit, entry_index, FALSE, &entry_duplicate));
...
#ifndef ADAPTER_SERVER_MODE
            SHR_IF_ERR_EXIT(mdb_lpm_test_fill_entry(unit, entry_index, TRUE, &entry));
            SHR_IF_ERR_EXIT(mdb_lpm_test_fill_entry(unit, entry_index, FALSE, &entry_duplicate_search));
