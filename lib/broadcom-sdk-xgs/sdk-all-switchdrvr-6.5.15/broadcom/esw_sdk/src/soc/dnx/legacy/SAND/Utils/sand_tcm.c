...
/*
 */
shr_error_e dnx_sand_tcm_set_task_priority( uint32 soc_tcmtask_priority )
{
...
/*
 */
shr_error_e dnx_sand_tcm_set_task_stack_size( uint32 soc_tcmtask_stack_size )
{
      ...
      )
{
  ...
  uint32             *tmp_bitstream, tmp_mask_bitstream[MAX_INTERRUPT_CAUSE / DNX_SAND_NOF_BITS_IN_UINT32]={0};
  ...
  if (unmask != NULL && _SHR_E_NONE != unmask(unit, TRUE))
  {
    ex = _SHR_E_INTERNAL ;
    goto exit_semaphore_chip_desc_and_delta_list ;
  }
    ...
    )
{
  ...
  while(TRUE)
  {
     ...
     */
      ...
      {
        if (_SHR_E_NONE != (err = dnx_sand_tcm_callback_engine_interrupt_serving(msg)) )
        {
          goto exit;
        }
        min_time_between_handling = dnx_sand_general_get_min_time_between_tcm_activation();
  ...
  )
{
   ...
   * empty the list
   */
  ex = dnx_sand_tcm_callback_delta_list_take_mutex();
  if(ex != no_err)
  {
    goto exit;
  }
   ...
   * dnx_sand_tcm_callback_delta_list_give_mutex() ;
   */
  ex = dnx_sand_tcm_set_request_to_die_flag(
          TRUE
        );
   ...
   */
  ex = dnx_sand_delta_list_destroy(
          Dnx_soc_sand_callback_delta_list
        ) ;
  ...
  )
{
   ...
   */
  if (_SHR_E_NONE != (ex = dnx_sand_delta_list_insert_d(Dnx_soc_sand_callback_delta_list, item->when, (void *)item)))
  {
    ex = _SHR_E_INTERNAL ;
    goto exit_sempahore ;
  }
...
exit_sempahore:
  if (_SHR_E_NONE != (ex = dnx_sand_tcm_callback_delta_list_give_mutex()))
  {
    ex = _SHR_E_INTERNAL ;
    goto exit ;
  }
  ...
  )
{
  ...
  */
   ...
   */
  item->valid_word = 0 ;
  if ( _SHR_E_NONE != (dnx_sand_delta_list_remove(Dnx_soc_sand_callback_delta_list, (void *)item)))
  {
    /*
  ...
  if (_SHR_E_NONE != (dnx_sand_os_free(item)))
  {
    ex = _SHR_E_INTERNAL ;
    goto exit_sempahore ;
  }
  ...
  if (local_buf_ptr)
  {
    if(_SHR_E_NONE != (dnx_sand_os_free(local_buf_ptr)))
    {
      ex = _SHR_E_INTERNAL ;
      goto exit_sempahore ;
    }
