    ...
    dbal_entry_handle_t * entry_handle)
{
     ...
     */
    if ((has_hit_bit_get_action || has_hit_bit_clear_action) && iterator_info->hit_bit_rule_valid)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Hit bits rule cannot be set along with hit bit actions. table %s", entry_handle->table->table_name);
    }

    /*
     * hit bit action and rules is supported for MDB which are not TCAM only
     */
    if (has_hit_bit_get_action || has_hit_bit_clear_action || iterator_info->hit_bit_rule_valid)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Hit bits rules/actions can be set to MDB tables which are not TCAM. table %s \n",
                         entry_handle->table->table_name);
        }
    }
    ...
    uint32 iterated_entry_handle_id)
{
    ...
    sal_memcpy(iter_entry_handle->user_output_info, updated_entry_handle->user_output_info,
               sizeof(iter_entry_handle->user_output_info));
    ...
    uint32 entry_handle_id)
{
    ...
    if (iterator_info->actions_bitmap & DBAL_ITER_ACTION_UPDATE)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_iterator_action_payload_buffer_build
                        (unit, entry_handle_id, updated_entry_handle_id, update_entry_handle->table->has_result_type,
    ...
    uint32 attr_val)
{
    ...
    switch (attr_type)
    {
        case DBAL_ENTRY_ATTR_PRIORITY:
            SHR_ERR_EXIT(_SHR_E_PARAM, "DBAL_ENTRY_ATTR_PRIORITY is not supported");
            break;
        ...
        case (DBAL_ENTRY_ATTR_HIT_GET | DBAL_ENTRY_ATTR_HIT_PRIMARY | DBAL_ENTRY_ATTR_HIT_SECONDARY):
            /** Verify hit bit attr rule val is NOT_SET(0) or SET(1) */
            ...
            iterator_info->hit_bit_rule_valid = TRUE;
            iterator_info->hit_bit_rule_is_hit = attr_val;
            iterator_info->hit_bit_rule_type = attr_type;
            break;
    ...
    uint32 attr_type)
{
    ...
    switch (attr_type)
    {
        ...
        case (DBAL_ENTRY_ATTR_HIT_GET | DBAL_ENTRY_ATTR_HIT_PRIMARY | DBAL_ENTRY_ATTR_HIT_SECONDARY):
            if ((entry_handle->table_id == DBAL_TABLE_FWD_MACT) || (entry_handle->table_id == DBAL_TABLE_FWD_MACT_IVL))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "get hit bit is invalid action for table %s\n",
                             entry_handle->table->table_name);
            }
            iterator_info->hit_bit_action_get = TRUE;
            iterator_info->hit_bit_action_get_type = attr_type;
        ...
        case (DBAL_ENTRY_ATTR_HIT_CLEAR | DBAL_ENTRY_ATTR_HIT_SECONDARY):
        case (DBAL_ENTRY_ATTR_HIT_CLEAR | DBAL_ENTRY_ATTR_HIT_PRIMARY | DBAL_ENTRY_ATTR_HIT_SECONDARY):
            iterator_info->hit_bit_action_clear = TRUE;
            iterator_info->hit_bit_action_clear_type = attr_type;
            SHR_IF_ERR_EXIT(dbal_iterator_action_add
    ...
    int *is_end)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_ITER);
...
exit:
    DBAL_FUNC_FREE_VARS;
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_ITER);
    SHR_FUNC_EXIT;
}
