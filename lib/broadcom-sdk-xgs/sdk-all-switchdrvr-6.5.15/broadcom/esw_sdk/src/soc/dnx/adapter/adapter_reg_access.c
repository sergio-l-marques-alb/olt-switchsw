...
#define MAX_PACKET_HEADER_SIZE_ADAPTER 12
 ...
 */
int
adapter_set_mem_access_recording(
    int unit,
    int is_start)
{
    int write_index = 0;
    ...
    if (is_start == 1)
    {
        /** Add opcode (1B) to the buffer for start recording */
        buffer[write_index] = UPDATE_FRM_SOC_RCRD_ACC_ON;
    }
    else
    {
        /** Add opcode (1B) to the buffer for stop recording */
        buffer[write_index] = UPDATE_FRM_SOC_RCRD_ACC_OFF;
    }
    write_index += sizeof(char);
    ...
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "Error: set_recording: writing the opcode failed\n")));
        SHR_SET_CURRENT_ERR(_SHR_E_FAIL);
        SHR_EXIT();
    }
    ...
    uint32 data)
{
     ...
     */
    if (dnxc_ha_is_access_disabled(unit, DNXC_HA_ALLOW_SCHAN))
    {
...
#ifdef VXWORKS
    if (!isdigit((unsigned) *adapter_host))
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "adapter_reg_access_init: can't resolve host names in vxworks\n")));
    }

    srv_addr.sin_addr.s_addr = inet_addr(adapter_host);
#else
    ...
    unsigned char *buf)
{
    ...
    {
        /** Read the length of the packet*/
        if (readn(adapter_rx_tx_fd, &(swapped_header[0]), MAX_PACKET_HEADER_SIZE_ADAPTER) <
            MAX_PACKET_HEADER_SIZE_ADAPTER)
        {
    ...
    int nof_signals)
{
    ...
    /** Add PTC signal */
    sal_memset(signal_data, 0, TX_PACKET_PTC_SIGNAL_WIDTH);
    adapter_send_buffer_add_signal(TX_PACKET_PTC_SIGNAL_ID, TX_PACKET_PTC_SIGNAL_WIDTH, signal_data,
