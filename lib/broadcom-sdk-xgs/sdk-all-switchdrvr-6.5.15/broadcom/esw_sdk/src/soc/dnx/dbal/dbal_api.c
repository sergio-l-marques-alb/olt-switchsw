    ...
    int unit)
{
    ...
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYSER_DBAL_INIT);
    ...
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYSER_DBAL_INIT);
...
shr_error_e
dbal_entry_handle_release_macro(
    int unit,
    uint32 entry_handle_id)
{
    SHR_FUNC_INIT_VARS(unit);
...
dbal_value_field8_request(
    ...
    uint8 *field_val)
{
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_REQUEST);
    dbal_entry_value_field_request(unit, DBAL_POINTER_TYPE_UINT8, entry_handle_id, field_id, inst_id, field_val, NULL);
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_FIELD_REQUEST);
}
    ...
    uint32 attr_val)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_PRIORITY)
    {
        ...
        if (((entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)
                || (entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM))
    ...
    uint32 *attr_val)
{
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_AGE)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal use for AGE API in access method %s",
    ...
    uint8 *field_mask)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));
    DBAL_FIELD_ERR_RETURN_HANDLE((entry_handle->table->table_type != DBAL_TABLE_TYPE_TCAM) &&
                                 (entry_handle->table->table_type != DBAL_TABLE_TYPE_TCAM_DIRECT) &&
                                 (entry_handle->table->table_type != DBAL_TABLE_TYPE_LPM) &&
                                 (entry_handle->table->access_method != DBAL_ACCESS_METHOD_TCAM_CS),
    ...
    uint32 *payload_buffer)
{
    ...
    if (entry_handle->table->has_result_type)
    {
        ...
        else if (entry_handle->table->result_type_mapped_to_sw)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
    ...
    /** Key Updates */
    entry_handle->num_of_fields = entry_handle->table->nof_key_fields;
    ...
    if (res_type_resolution_needed)
    {
        ...
        /** this is not an HW action, just reolution of the result type according to the payload added by the user */
        if (entry_handle->table->access_method == DBAL_ACCESS_METHOD_KBP)
        {
            rv = dbal_kbp_res_type_resolution(unit, entry_handle);
        }
        else
        {
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_GET);
...
exit:
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_GET);
    SHR_FUNC_EXIT;
}
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_SET);
...
exit:
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_SET);
    SHR_FUNC_EXIT;
}
...
dbal_entry_handle_access_id_set(
    /*
     * remove all references other than indirect TCAM MDB
     */
    int unit,
    uint32 entry_handle_id,
    uint32 entry_access_id)
{
    ...
    if (table->table_type == DBAL_TABLE_TYPE_TCAM || table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)
    {
        if ((table->access_method != DBAL_ACCESS_METHOD_MDB) &&
            (table->access_method != DBAL_ACCESS_METHOD_KBP) && (entry_access_id >= table->max_capacity))
        {
        ...
        /** Priority and access_id have the same values for KBP tables */
        if (table->access_method == DBAL_ACCESS_METHOD_KBP)
        {
            entry_handle->phy_entry.prefix_length = entry_access_id;
        }
    }
    else
    {
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_ENTRY_CLEAR);
...
exit:
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_ENTRY_CLEAR);
    SHR_FUNC_EXIT;
}
    ...
    dbal_result_type_update_mode_e mode)
{
    ...
    for (field_index = 0; field_index < table->multi_res_info[new_result_type].nof_result_fields; field_index++)
    {
        ...
        if (field_info.field_nof_bits)
        {
            ...
            val = sub_field_val = 0;
            /** Get field value */
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, field_id, INST_SINGLE, &val));
            val_arr[0] = val;
            SHR_IF_ERR_EXIT(dbal_fields_parent_field_decode_no_err
                            (unit, field_id, val_arr, &sub_field_id, &sub_field_val));

            if (sub_field_id == DBAL_FIELD_EMPTY)
            {
                dbal_entry_value_field32_set(unit, update_entry_handle_id, new_field_id, INST_SINGLE, val);
    ...
    dbal_tables_e table_id)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_DBAL_CLEAR);
...
exit:
    DBAL_FUNC_FREE_VARS;
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_DBAL_CLEAR);
    SHR_FUNC_EXIT;
}
    ...
    dbal_bulk_module_e bulk_module)
{
    ...
    switch (access_method)
    {
        case DBAL_ACCESS_METHOD_MDB:
            ...
            SHR_IF_ERR_EXIT(dbal_flush_start(unit, TRUE, TRUE));
            break;
