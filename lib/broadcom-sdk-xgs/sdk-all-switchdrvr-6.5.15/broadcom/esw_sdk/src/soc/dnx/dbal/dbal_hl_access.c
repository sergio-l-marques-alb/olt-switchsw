...
/**
 *\brief
 *  calculate the offset according to the encode info and the
 *  key fields from the handle.
 */
shr_error_e
    ...
    uint32 *calc_index)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_calculate_offset_from_formula(unit, entry_handle, entry_handle->table_id, mapped_field_id,
                                                           encode_info->formula, calc_index));
    }
    else
    {
    ...
    int *num_of_blocks)
{
    ...
    else
    {
        ...
        if (core_id != DBAL_CORE_ALL)
        {
            ...
            /** calculating the total number of blocks for this register */
            SOC_BLOCK_ITER_ALL(unit, block_indx, SOC_REG_FIRST_BLK_TYPE(SOC_REG_INFO(unit, reg).block))
            {
                (*num_of_blocks)++;
            }

            /** number of blocks to set is half of the total blocks (half for each core) */
            (*num_of_blocks) = ((*num_of_blocks) + 1) / 2;

            /** block receive 0 when core_id==0 if core_id==1 receives  the upper half */
        ...
        else
        {
            ...
            *block = 0;
            ...
            /** the following code was the original code since there is an issue with the access for all blocks in a register the
             *  W.A was added */
#if 0

            (*num_of_blocks) = 1;
            (*block) = REG_PORT_ANY; /**just making sure that this is the value*/
#endif
        }
    }
...
/**
 * \brief checks if the condition passed, if true calculates the offsets
 *
 */
shr_error_e
    ...
    uint32 *group_offset)
{
    ...
    if ((*is_condition_pass))
    {
        if (mem_array_offset)
        {
            /** calculate memory array offset no encoding to the array index means that offset is 0 */
        ...
        if (mem_entry_offset)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_hl_offset_calculate(unit, entry_handle, curr_l2p_info->field_id,
                                                     key[0], &(curr_l2p_info->entry_offset_info), mem_entry_offset));
        }
...
/**
 * \brief
 * this function update the dst hw buffer from the src buffer according to the l2p info, when the dst buffer should be
 * the current HW value to allow read modify write.
 *
 */
shr_error_e
    ...
    uint32 *dst)
{
    ...
    {
        /** handling the case that only part of the HW field is set user can map only part of the HW field size */
...
/**
 * \brief
 * this function update the dst buffer from the src HW buffer according to the l2p info
 */
shr_error_e
    ...
    uint32 *dst)
{
    ...
    /** if there is a field in the mapping, need to find the offset of the field in the hw buffer */
    if (curr_l2p_info->hw_field != INVALIDf)
    {
        if (memory != INVALIDm)
        {
...
/**
 * \brief
 * this function build default entry for non standard memories, usually the default entry is all zero, in cases that the
    ...
    uint32 *default_entry)
{
    ...
    /** updating the default entry according to memories */
    for (field_pos = 0; field_pos < DBAL_HL_MEM_MUL_RES_INFO.num_of_access_fields; field_pos++)
    {
        ...
        if (is_condition_pass)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_init_mem_default_mode_get(unit, cur_memory, &is_non_standard));
            }
            memory = cur_memory;
            if (is_non_standard)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_hl_field_from_hw_buffer_get
                                (unit, entry_handle, &DBAL_HL_MEM_MUL_RES_INFO,
                                 ...
                                 field_pos, default_val, default_entry));
            }
        }
    }

    /** updating the default entry according to registers */
    for (field_pos = 0; field_pos < DBAL_HL_REG_MUL_RES_INFO.num_of_access_fields; field_pos++)
    {
        ...
        if (is_condition_pass)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dbal_hl_reg_default_mode_get
                                (unit, DBAL_HL_REG_MUL_RES_INFO.l2p_fields_info[field_pos].reg[0], &is_non_standard));
            }
...
/**
 * \brief
 * this function calculates the default entry, the default entry in most of the cases is all zero, in cases that the
 * memory is non-standard there is a different default value for the memory.
 *
 */
shr_error_e
    ...
    int *is_default)
{
    ...
    {
        sal_memcpy(payload, entry_handle->phy_entry.payload, DBAL_PHYSICAL_RES_SIZE_IN_WORDS);
                /**Iterate over entry fields, ignore read-only and trigger fields*/
                 ...
                 iter++)
            {
                        ...
                        DBAL_PERMISSION_TRIGGER))
                {
                    ...
                    int nof_bits = end_bit - start_bit;
                                        /**Get default field value, update it in payload used for comparison*/
        ...
        if (sal_memcmp(default_entry, payload, DBAL_PHYSICAL_RES_SIZE_IN_BYTES) == 0)
        {
            (*is_default) = 1;
        }
    }
...
/**
 * \brief
 * this function reads from the table according to the access
 * type.
 */
shr_error_e
    ...
    int result_type)
{
    ...
    /** get is packed fields. If result_type is passed as parameter consider it, otherwise use current */
    ...
    if (is_packed_fields)
    {
        /** In case of packed fields, sw" access type is not supported */
        switch (access_type)
        {
    ...
    int result_type_get)
{
    ...
    /** updating the the entry result type valid incase of result_type_get != DBAL_INVALID_PARAM */
    ...
    for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
    {
        ...
        {
            /** multiple result retrieve the result type */
            if (result_type_get != DBAL_INVALID_PARAM)
            {
        ...
        else
        {
            ...
            {
                /** validate that the specific field was requested */
...
/**
 *  \brief
 *  actual access to HW, writing to memory. this method
 *  calculate the block and array indexes to HW memory and
 *  access it according to the info.
 */
shr_error_e
    ...
    int num_of_entries)
{
    ...
    {
        if (sal_strcmp(l2p_info->group_name, EMPTY) != 0)
        {
            ...
            if (is_array_fill_needed)
            {
                     ...
                     array_index < nof_elm + SOC_MEM_FIRST_ARRAY_INDEX(unit, memory); array_index++)
                {
                    ...
                    else
                    {
                        ...
                        SHR_IF_ERR_EXIT_WITH_LOG(soc_mem_array_write(unit, memory, array_index, block, entry_offset, data),
                                             "Failed writing memory %s port=%d arr_ind=%d", SOC_MEM_NAME(unit,memory), 
                                                 block, array_index);
                    }
                }
                LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "\n")));
 ...
 *\brief
 *
 * running over all result fields that was set and wrting them
 * to HW (according to key). num_of_entries indicates the number of HW consecutive entries to set (DMA operation)
 */
shr_error_e
dbal_hl_memory_set(
    int unit,
    ...
    int num_of_entries)
{
    ...
    for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
    {
        ...
        if (is_condition_pass)
        {
            /** indicate that access occured remove the error indication   */
                ...
                (last_mem_offset_used != mem_entry_offset) || (last_mem_array_offset_used != mem_array_offset))
            {
                ...
                if (last_memory_used != INVALIDm)
                {
                    if (alias_offset > 0)
                    {
                        ...
                        SHR_IF_ERR_EXIT(dbal_hl_memory_write(unit,
                                                             ...
                                                             last_mem_offset_used, last_group_offset, num_of_entries));
                    }
                    else
                    {
                        SHR_IF_ERR_EXIT(dbal_hl_memory_write(unit,
                ...
                if (!entry_handle->overrun_entry)
                {
                    ...
                    SHR_IF_ERR_EXIT_WITH_LOG(soc_mem_array_read
                                    (unit, memory_to_read, mem_array_offset, block, mem_entry_offset, data),
                                            "Failed reading memory %s port=%d arr_ind=%d",
                ...
                {
                    /** should be happen only in init */
                    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "Get skipped due to overrun flag (init only)")));
                }
                ...
                if (curr_l2p_info->alias_memory != INVALIDm)
                {
                    ...
                    if (alias_offset != 0)
                    {
                        ...
                        /** copying the relevant part of the memory (from alias_offset) to begining of data */
                        SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field(data_for_alias, alias_offset,
                                                                       curr_l2p_info->
                                                                       alias_data_offset_info.internal_inparam, data));
                        DBAL_DATA_LOG(data, 1);
            ...
            SHR_IF_ERR_EXIT(dbal_hl_field_to_hw_set
                            (unit, entry_handle, l2p_hl_info, last_memory_used, INVALIDr, field_offset, iter,
                             entry_handle->phy_entry.payload, data));
        }
    }
    ...
    if (last_memory_used != 0)
    {
        if (curr_l2p_info)
        {
            if (alias_offset > 0)
            {
                 ...
                 */
                SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(data, alias_offset,
                                                               curr_l2p_info->alias_data_offset_info.internal_inparam,
                                                               data_for_alias));
 ...
 * \brief
 *  this function reads from the HW according to the key and updated the entry handle buffer.
 *  only requested fields are read from HW. in case of get all fields is set. all fields are requested.
 *  result_type_get: when NULL the function acts in the normal way, when this function called with legal result type
 *  this function only tries to read the result type from HW. PCI registers are also suppoorted.
 */
shr_error_e
    ...
    int group_offset)
{
    ...
    /** if block = REG_PORT_ANY and than nof_blocks should be 1. */
    if ((block == REG_PORT_ANY) && (nof_blocks != 1))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "ilegal block params block=reg_port_any, nof_blocks = %d\n", nof_blocks);
    }
    ...
    else
    {
        ...
        if (sal_strcmp(l2p_info->group_name, EMPTY) != 0)
        {
                        /** incase group offset is -1 and we have a goup need to update all registers in group   */
            all_reg_in_group = 1;
        }
    }
    ...
    {
                /*** check if reg is array and there is no specpfic array index to set*/
        if ((l2p_info->array_offset_info.formula == NULL) && SOC_REG_IS_ARRAY(unit, reg))
        {
            ...
            for (array_iter = 0; array_iter < SOC_REG_NUMELS(unit, reg); array_iter++)
            {
                SHR_IF_ERR_EXIT_WITH_LOG(soc_reg_above_64_set
                                (unit, reg, block, array_iter + SOC_REG_FIRST_ARRAY_INDEX(unit, reg), data),
        ...
        /** updating the reamaining registers in group if needed */
    if (all_reg_in_group)
    {
                            ...
                            else if (soc_customreg == SOC_REG_INFO(unit, reg).regtype)
                            {
                                SHR_IF_ERR_EXIT_WITH_LOG(soc_custom_reg_above_64_set
                                                (unit, reg, block, array_iter + SOC_REG_FIRST_ARRAY_INDEX(unit, reg),
                                                 data),
                                         "Failed writing register %s port=%d arr_ind=%d",
                                         SOC_REG_NAME(unit,reg), block, array_iter + SOC_REG_FIRST_ARRAY_INDEX(unit, reg));
                            }
                            else
                            {
                                SHR_IF_ERR_EXIT_WITH_LOG(soc_reg_above_64_set
                                                (unit, reg, block, array_iter + SOC_REG_FIRST_ARRAY_INDEX(unit, reg),
                                                 data),"Failed writing register %s port=%d arr_ind=%d", 
                                                         SOC_REG_NAME(unit,reg), block,
                                                         array_iter + SOC_REG_FIRST_ARRAY_INDEX(unit, reg));
                            }
                        }
...
/**
 * \brief
 *  this function write from the HW according to the key. only
 *  requested fields are being write to HW. PCI registers are
 *  also supported.
 */
shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    int iter;
    soc_reg_t last_reg_used = INVALIDr, curr_reg;
    uint32 reg_array_offset = 0, last_reg_array_offset = 0, field_offset = 0, group_offset = 0, last_group_offset = 0;
    uint8 is_condition_pass = 0;
    uint32 data[SOC_REG_ABOVE_64_MAX_SIZE_U32] = { 0 };
    uint32 block = REG_PORT_ANY, last_block = REG_PORT_ANY;
...
/**
 * \brief
 *  this function reads from the HW according to the key and
 *  updated the entry handle buffer.
 *  The full register is requested from HW.
 */
shr_error_e
    ...
    int result_type_get)
{
    ...
    if (is_condition_pass)
    {
        ...
        SHR_IF_ERR_EXIT_WITH_LOG(soc_reg_above_64_get(unit, curr_reg, block, reg_array_offset, entry_handle->phy_entry.payload), 
                                 "Failed reading register %s port=%d arr_ind=%d", 
                                 SOC_REG_NAME(unit,curr_reg),block, reg_array_offset);
        DBAL_ACTION_PERFORMED_LOG;
...
/**
 * \brief
 * this function write from the HW according to the key. The
 * whole register is being written
 */
shr_error_e
    ...
    int num_of_entries)
{
    ...
    /**
     *  dbal_hl_pre_access_calculate function accesses
     *  curr_l2p_info->field_id which is not supported for
     *  packed_fields (always 0). field_id is used internally only
     *  to access instance_index. Consequently, we don't support
     *  instance_index different than 0.
     */
    SHR_IF_ERR_EXIT(dbal_hl_pre_access_calculate(unit, entry_handle, curr_l2p_info,
                                                 &is_condition_pass, &mem_array_offset, &mem_entry_offset,
                                                 &field_offset, &group_offset));
...
/**
 * \brief
 * this function writes to the table according to the access
 * type.
 */
shr_error_e
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /** Validate that all the fields are set. (Meaning the user did
         *   set all the fields, or a merged has been done) */
        ...
        /** packed mode configured, can set that no error exists */
        entry_handle->error_info.error_exists = 0;
    }
    else
    {
...
/**
 *  \brief
 *  set an entry in HW. access the HW according to the key
 *  fields set and updates the value fields. access memories
 *  register and SW in this order */
shr_error_e
dbal_hl_entry_set(
    int unit,
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        /** setting the error value to 4, if no access will occur the value remains 4 and it means that the key not mapped
         *  this issues can happen for ML 1 tables */
        entry_handle->error_info.error_exists = 4;

        if (entry_handle->phy_entry.key_size > SAL_UINT32_NOF_BITS)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Key is bigger than uint32 %d not supported\n",
                         entry_handle->phy_entry.key_size);
        }
...
/**
 *  \brief
 *  this function build a default entry that uses to clear the entry
 *  setting the requested key fields and updating the result
 *  fields with the default  values of the entry.
 */
shr_error_e
    ...
    dbal_entry_handle_t * default_handle)
{
    ...
    if ((entry_handle->table->has_result_type) && (default_handle->cur_res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED))
    {
        int rv;
        rv = dbal_hl_res_type_resolution(unit, default_handle);
        if (rv == _SHR_E_NOT_FOUND)
        {
            /** in this case entry not exists, No need to clear the entry */
    ...
    if (entry_handle->table->table_type != DBAL_TABLE_TYPE_TCAM_DIRECT)
    {
        if (entry_handle->table->hl_mapping_multi_res[res_type_idx].is_default_non_standard)
        {
    ...
    /*
     * Default entry for HL TCAM not supporting non-standard
     */
    if (entry_handle->table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)
    {
        /*
         * Set key to zero - we map using hw id
         */
        sal_memset(default_handle->phy_entry.payload, 0, DBAL_PHYSICAL_RES_SIZE_IN_BYTES);
        ...
        /*
         * Update the nof_key_fields - for merging purposes
         * If nof_key_fields is not full than we will merge the entries
         */
...
/**
 *  \brief
 *  this function clears the entry. using new entry handle,
 *  setting the requested key fields and updating the result
 *  fields with the default  values of the entry. evantualy
 *  calling dbal_hl_entry_set with the new handle
 */
shr_error_e
dbal_hl_entry_clear(
    int unit,
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!rv)
    {
        /** if result type was resolved need to clear the entry  if not it means that there is no entry */
...
/**
 *  \brief
 *  deleting all entries in the table. using iterator and
 *  dbal_hl_entry_set_default
 */
shr_error_e
dbal_hl_table_clear(
    int unit,
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    /** In case that the table is TCAM the num_of_iterations is derived from the HW size and not from the key size */
    if (entry_handle->table->table_type != DBAL_TABLE_TYPE_TCAM_DIRECT)
    {
        ...
        while ((!entry_found) && (!iterator_info->is_end))
        {
            ...
            if (is_valid_entry == DBAL_KEY_IS_VALID)
            {
                ...
                if (rv == _SHR_E_NONE)
                {
                     ...
                     * NOTE: in case we have result type we don't need to compare the result type value.. entry can be determined as empty even
                     * if the result type is set (if all other fields equal to default value)
                     */
...
/**
 *  \brief
 *  setting range of entries with the same value to HW, supported only for memories.
 */
shr_error_e
    ...
    int *is_non_standard)
{
    ...
    {
        (*is_non_standard) = 1;
         /**LOG_CLI((BSL_META("reg %s non default\n"), SOC_REG_NAME(unit, reg)));*/
    }
