...
#  define ARR_EG_TYPE_FIELD_SIZE_BITS               6
        ...
        if (NULL == utilex_rhlist_entry_get_by_name(field_list, field->name))
        {
            ...
            if (field->prefix_value)
            {
                ...
                sand_signal_value_to_str(unit, SAL_FIELD_TYPE_NONE, (uint32 *) &field->prefix_value,
                                         field_sig->print_value, field->size, PRINT_BIG_ENDIAN);
    ...
    root_arr_info_t * arr_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_arr_formats_file_get(unit, arr_formats_path));

    curTop = dbx_file_get_xml_top(unit, arr_formats_path, "Root", CONF_OPEN_PER_DEVICE);
    if (curTop == NULL)
    {
