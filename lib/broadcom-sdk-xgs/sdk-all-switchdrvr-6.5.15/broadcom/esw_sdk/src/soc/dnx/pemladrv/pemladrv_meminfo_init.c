...
int parse_meminfo_definition_file(int unit, int restore_after_reset, const char *file_name) 
{
  FILE *fp;
  int ret_val = 0;
  ...
  while((fgets(line, sizeof(line), fp) != NULL) && (ret_val >= 0)) { 
    ...
    if (strncmp(line, KEYWORD_DB, KEYWORD_DB_SIZE) == 0)                                                                    { dnx_pemladrv_db_info_insert(line)                      ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_KEY, KEYWORD_KEY_SIZE) == 0)                                                                  { dnx_pemladrv_db_field_info_insert(line)                ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_RESULT, KEYWORD_RESULT_SIZE) == 0)                                                            { dnx_pemladrv_db_field_info_insert(line)                ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_DIRECT_INFO, KEYWORD_DB_DIRECT_INFO_SIZE) == 0)                                            { dnx_pemladrv_direct_result_chunk_insert(line)          ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_TCAM_KEY_INFO, KEYWORD_DB_TCAM_KEY_INFO_SIZE) == 0)                                        { dnx_pemladrv_tcam_key_chunk_insert(line)               ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_TCAM_RESULT_INFO, KEYWORD_DB_TCAM_RESULT_INFO_SIZE) == 0)                                  { dnx_pemladrv_tcam_result_chunk_insert(line)            ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_EXACT_MATCH_KEY_INFO, KEYWORD_DB_EXACT_MATCH_KEY_INFO_SIZE) == 0)                          { dnx_pemladrv_em_key_chunk_insert(line)                 ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_EXACT_MATCH_RESULT_INFO, KEYWORD_DB_EXACT_MATCH_RESULT_INFO_SIZE) == 0)                    { dnx_pemladrv_em_result_chunk_insert(line)              ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_LONGEST_PERFIX_MATCH_KEY_INFO, KEYWORD_DB_LONGEST_PERFIX_MATCH_KEY_INFO_SIZE) == 0)        { dnx_pemladrv_lpm_key_chunk_insert(line)                ;  ++line_no; continue; }  
    if (strncmp(line, KEYWORD_DB_LONGEST_PERFIX_MATCH_RESULT_INFO, KEYWORD_DB_LONGEST_PERFIX_MATCH_RESULT_INFO_SIZE) == 0)  { dnx_pemladrv_lpm_result_chunk_insert(line)             ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_VIRTUAL_REGISTER_MAPPING, KEYWORD_REGISTER_INFO_SIZE) == 0)                                   { dnx_pemladrv_register_insert(line)                     ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_REG_AND_DBS_NUM_INFO, KEYWORD_REG_AND_DBS_NUM_INFO_SIZE) == 0)                                { dnx_pemladrv_dnx_init_all_db_arr_by_size(line)         ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_DB_SINGLE_CHUNCK_INFO, KEYWORD_DB_SINGLE_CHUNCK_INFO_SIZE) == 0)                              { dnx_pemladrv_init_logical_db_chunk_mapper(line)        ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_VIRTUAL_REGISTER_NOF_FIELDS, KEYWORD_VIRTUAL_REGISTER_NOF_FIELDS_SIZE) == 0)                  { dnx_pemladrv_init_reg_field_info(line)                 ;  ++line_no; continue; }
    if (strncmp(line, KEYWORD_VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS, KEYWORD_VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_SIZE) == 0)  { dnx_pemladrv_init_reg_field_mapper(line)               ;  ++line_no; continue; }
#ifndef BCM_DNX_SUPPORT
...
void free_db_direct_container() {
  ...
  for (i = 0; i < api_info.db_direct_container.nof_direct_dbs; ++i)
  {
    if (NULL == api_info.db_direct_container.db_direct_info_arr[i].result_chunk_mapper_matrix_arr){    /*In case db was not mapped*/
      sal_free(api_info.db_direct_container.db_direct_info_arr[i].field_bit_range_arr);
      api_info.db_direct_container.db_direct_info_arr[i].field_bit_range_arr = NULL;
      continue;
    }
    free_logical_direct_info(&api_info.db_direct_container.db_direct_info_arr[i]);
  }
  sal_free(api_info.db_direct_container.db_direct_info_arr);
...
void free_db_tcam_container() 
{
  ...
  for (i = 0; i < api_info.db_tcam_container.nof_tcam_dbs; ++i) 
  {
    if (NULL == api_info.db_tcam_container.db_tcam_info_arr[i].result_chunk_mapper_matrix_arr){     /*In case db was not mapped*/
      sal_free(api_info.db_tcam_container.db_tcam_info_arr[i].key_field_bit_range_arr);
      sal_free(api_info.db_tcam_container.db_tcam_info_arr[i].result_field_bit_range_arr);
      api_info.db_tcam_container.db_tcam_info_arr[i].key_field_bit_range_arr = NULL;
      api_info.db_tcam_container.db_tcam_info_arr[i].result_field_bit_range_arr = NULL;
      continue;
    }
    free_logical_tcam_info(&api_info.db_tcam_container.db_tcam_info_arr[i]);
  }
  sal_free(api_info.db_tcam_container.db_tcam_info_arr);
...
void free_db_lpm_container() 
{
  ...
  for (i = 0; i < api_info.db_lpm_container.nof_lpm_dbs; ++i) 
  {
    if (NULL == api_info.db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.result_chunk_mapper_matrix_arr){      /*In case db was not mapped*/
      sal_free(api_info.db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.key_field_bit_range_arr);
      sal_free(api_info.db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.result_field_bit_range_arr);
      api_info.db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.key_field_bit_range_arr = NULL;
      api_info.db_lpm_container.db_lpm_info_arr[i].logical_lpm_info.result_field_bit_range_arr = NULL;
      continue;
    }
    free_logical_lpm_info(&api_info.db_lpm_container.db_lpm_info_arr[i]);
  }
  sal_free(api_info.db_lpm_container.db_lpm_info_arr);
...
void free_db_em_container() 
{
  ...
  for (i = 0; i < api_info.db_em_container.nof_em_dbs; ++i) 
  {
    if (NULL == api_info.db_em_container.db_em_info_arr[i].logical_em_info.result_chunk_mapper_matrix_arr){      /*In case db was not mapped*/
      sal_free(api_info.db_em_container.db_em_info_arr[i].logical_em_info.key_field_bit_range_arr);
      sal_free(api_info.db_em_container.db_em_info_arr[i].logical_em_info.result_field_bit_range_arr);
      api_info.db_em_container.db_em_info_arr[i].logical_em_info.key_field_bit_range_arr = NULL;
      api_info.db_em_container.db_em_info_arr[i].logical_em_info.result_field_bit_range_arr = NULL;
      continue;
    }
    free_logical_em_info(&api_info.db_em_container.db_em_info_arr[i]);
  }
  sal_free(api_info.db_em_container.db_em_info_arr);
...
void free_reg_container() 
{
  ...
  sal_free(api_info.reg_container.reg_info_arr);
...
void dnx_pemladrv_mem_line_write(int unit, const char* line) {
  ...
  uint8 is_equal_to_zero = TRUE;
  phy_mem_t phy_mem;
  if (strncmp(line, KEYWORD_UCODE_MEM_WRITE_INFO, KEYWORD_UCODE_MEM_WRITE_INFO_SIZE) == 0) 
  {

    if (sscanf( line, "%s %s %s %u %s", key_word, pem_mem_name, pem_mem_address,
                                        &phy_mem_idx, str_value) != PEM_NOF_MEM_WRITE_TOKEN) 
	  {
	  ...
	  else 
	  {
        ...
        data_nof_chunks = (value_length % 32 == 0) ? value_length / 32 : value_length / 32 + 1;
        for (int i = 0; (i < (int)data_nof_chunks) && is_equal_to_zero; ++i){
        if (data_value[i] != 0)
      ...
      phy_mem.mem_width_in_bits = value_length; 
      phy_mem.reserve = 0;
...
void dnx_pemladrv_reg_line_write(int unit, const char* line) {
  ...
  unsigned int* data_value;
  if (sscanf( line, "%s %s %s %s", key_word, pem_reg_name, pem_reg_address, str_value) != PEM_NOF_REG_WRITE_TOKEN) {
    printf("Bad ucode line format. Skip and continue with next line\n.");
  } else {
    ...
    phy_mem.block_identifier = block_id;
    phy_mem.is_reg = 1;
    phy_mem.mem_address = pem_mem_address_val;
    phy_mem.mem_width_in_bits = value_length;
    phy_mem.reserve = 0;
...
void dnx_pemladrv_db_info_insert(const char* line) {
  ...
  if (sscanf( line, "%s %s %s %d %d %d %d %d %d ", key_word, db_type, db_name_debug, &db_id, &nof_entries, &key_width, &result_width, &nof_key_fields, &nof_result_fields) != PEM_NOF_DB_TOKEN) {
...
void dnx_pemladrv_db_field_info_insert(const char* line) {
...
void dnx_pemladrv_direct_result_chunk_insert(const char* line) {
  unsigned int db_id;
...
void dnx_pemladrv_tcam_key_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
...
void dnx_pemladrv_tcam_result_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
...
void dnx_pemladrv_em_key_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_key_chunk_from_ucode(line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
...
void dnx_pemladrv_em_result_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_result_chunk_from_ucode(line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
...
void dnx_pemladrv_lpm_key_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_key_chunk_from_ucode(line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
...
void dnx_pemladrv_lpm_result_chunk_insert(const char* line) {
  unsigned int db_id;
  DbChunkMapper* chunk_info_ptr = (DbChunkMapper*)sal_calloc(chunk_info_ptr, 1, sizeof(DbChunkMapper));
  db_id = build_cam_result_chunk_from_ucode(line, chunk_info_ptr);
  if (db_id == UINT_MAX) {
...
/*Insert register*/
void dnx_pemladrv_register_insert(const char* line) {
  char                         pem_mem_address[40]; /* this contain hex string address in format 40'hxxxx*/
  ...
  if (sscanf(line, "%s %s %d %d %s %d %d %d%c%d %s %s %s %d %d %d %d %d %d",
    ...
    RegFieldMapper* reg_field_mapping_ptr = &(api_info.reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_arr[mapping_id]);
    api_info.reg_container.reg_info_arr[reg_id].is_mapped = 1;
...
void dnx_pemladrv_init_logical_db_chunk_mapper(const char* line){
...
void dnx_pemladrv_init_reg_field_info(const char* line) {
  ...
  if (sscanf(line, "%s %d %d %d %d", key_word, &reg_id, &nof_fields, &register_total_width, &is_binded) != VIRTUAL_REGISTER_NOF_FIELDS_TOKEN) {
    ...
    api_info.reg_container.reg_info_arr[reg_id].is_binded               = is_binded;
    api_info.reg_container.reg_info_arr[reg_id].reg_field_info_arr      = (LogicalRegFieldInfo*)sal_calloc(api_info.reg_container.reg_info_arr[reg_id].reg_field_info_arr, nof_fields, sizeof(LogicalRegFieldInfo));
    api_info.reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr = (FieldBitRange*)      sal_calloc(api_info.reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr, nof_fields, sizeof(FieldBitRange));
  }
}

/* Init Reg_Field_Mapper*/
void dnx_pemladrv_init_reg_field_mapper(const char* line) {
  char key_word[MAX_NAME_LENGTH];
  ...
  if (sscanf( line, "%s %d %d %d %d %d", key_word, &reg_id, &field_id, &lsb_bit, &msb_bit, &nof_mappings) != VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_TOKEN) {
    ...
    if (nof_mappings > 0)
    {
      api_info.reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr[field_id].is_field_mapped = 1;
    }
    ...
    api_info.reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_arr = (RegFieldMapper*)sal_calloc(api_info.reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_id].reg_field_mapping_arr, nof_mappings, sizeof(RegFieldMapper));
  }
}
...
void init_meminfo_array_for_applets(const char* line) {
  ...
  for (core_idx=0; core_idx < PEM_CFG_API_NOF_CORES; ++core_idx) {
    api_info.applet_info.nof_pending_applets[core_idx][0] = 0;
    api_info.applet_info.nof_pending_applets[core_idx][1] = 0;
  }
  api_info.applet_info.meminfo_curr_array_size         = 0;
  api_info.applet_info.meminfo_array_for_applet        = (PemMemInfoForApplet**)sal_alloc(nof_mems*sizeof(PemMemInfoForApplet*),"");
}
...
void init_pem_applet_reg(const char* line) {
  ...
  assert(core>=0);
  api_info.applet_info.applet_reg_info[core][ingress0_egress_1].address = address;
  api_info.applet_info.applet_reg_info[core][ingress0_egress_1].block_id = block_id;
  api_info.applet_info.applet_reg_info[core][ingress0_egress_1].is_ingress0_egress1 = ingress0_egress_1;
  api_info.applet_info.applet_reg_info[core][ingress0_egress_1].length_in_bits = width_in_bits;
  strncpy(api_info.applet_info.applet_reg_info[core][ingress0_egress_1].name, name, strlen(name));
}
...
void init_pem_applet_mem(const char* line) {
  ...
  core = get_core_from_memory_address(address);
  assert(core>=0);
  api_info.applet_info.applet_mem_info[core][ingress0_egress_1].address = address;
  api_info.applet_info.applet_mem_info[core][ingress0_egress_1].block_id = block_id;
  api_info.applet_info.applet_mem_info[core][ingress0_egress_1].is_ingress0_egress1 = ingress0_egress_1;
  api_info.applet_info.applet_mem_info[core][ingress0_egress_1].length_in_bits = width_in_bits;
  sprintf(api_info.applet_info.applet_mem_info[core][ingress0_egress_1].name, "%s", name);
}
...
#ifdef BCM_DNX_SUPPORT
  ret_val = pem_write(unit, &mem_write, mem_write.is_reg, data_value);
#endif
  sal_free(data_value);
  return ret_val;
}
