...
#define MDB_ENTRY_OFFSET_OFFSET           3
#define MDB_ENTRY_COMMAND_OFFSET          0
...
#define MDB_DIRECT_ADDR_OFFSET_OFFSET(_mem)     (((_mem == DDHB_MACRO_1_ENTRY_BANKm) || (_mem == DDHB_MACRO_0_ENTRY_BANKm)) ? 13 : 14)
#define MDB_DIRECT_ADDR_OFFSET_BITS             (2)
#define MDB_DIRECT_ADDR_OFFSET_MASK(_mem)       ((1 << MDB_DIRECT_ADDR_OFFSET_OFFSET(_mem)) - 1)
...
#define MDB_ENTRY_SET_CONTROL_BITS(command, offset) ((command << MDB_ENTRY_COMMAND_OFFSET) | (offset << MDB_ENTRY_OFFSET_OFFSET))
...
const STATIC uint8 mdb_direct_hit_bit_accessed_indications[MDB_DIRECT_ENTRY_NOF_HIT_BIT_INDEXS][MDB_NOF_ABK_BANKS] = {{DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_A , DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_B},{DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_A, DBAL_PHYSICAL_KEY_SECONDARY_HITBIT_ACCESSED_B}};
    ...
    uint32 *row_in_cluster)
{
    ...
    int cluster_index;
    int format_index = 0;
    const soc_field_t command_fields[DNX_DATA_MAX_MDB_DH_NOF_BUCKET_CLUSTERS] =
        { COMMAND_0f, COMMAND_1f, COMMAND_2f, COMMAND_3f };

    SHR_FUNC_INIT_VARS(unit);

    for (cluster_index = 0; cluster_index < dnx_data_mdb.dh.nof_bucket_clusters_get(unit); cluster_index++)
    {
        if ((clusters_in_use_bitmap & (1 << cluster_index)) != 0)
        {
            control = MDB_ENTRY_SET_CONTROL_BITS(command, clusters_format[format_index]);
            soc_mem_field_set(unit, DDHB_MACRO_0_ENTRY_BANKm, row_data, command_fields[cluster_index], &control);

            SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit_range
                            (row_in_cluster,
                             MDB_DIRECT_ADDR_OFFSET_OFFSET(mem) + MDB_DIRECT_ADDR_OFFSET_BITS * cluster_index,
                             MDB_DIRECT_ADDR_OFFSET_OFFSET(mem) + MDB_DIRECT_ADDR_OFFSET_BITS * cluster_index + 1));
    ...
    uint32 *src_array)
{
    ...
    int global_cluster_idx = (macro_index * dnx_data_mdb.dh.nof_macro_clusters_get(unit)) + cluster_index;
    ...
    if (macro_type == MDB_MACRO_A)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.
                        macro_a_cluster_cache.range_write(unit, global_cluster_idx, row_index,
    ...
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.
                        macro_b_cluster_cache.range_write(unit, global_cluster_idx, row_index,
                                                          BITS2WORDS(DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS),
                                                          row_cache));
    }
    else if (macro_type == MDB_EEDB_BANK)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.eedb_banks_cache.range_write(unit, cluster_index, row_index,
                                                                                    BITS2WORDS
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error. Unrecognized macro_type %d.\n", macro_type);
    }
    ...
    uint32 *dst_array)
{
    ...
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.
                        macro_a_cluster_cache.range_read(unit, global_cluster_idx, row_index,
                                                         BITS2WORDS(DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS),
                                                         row_cache));
    }
    else if (macro_type == MDB_MACRO_B)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.
                        macro_b_cluster_cache.range_read(unit, global_cluster_idx, row_index,
                                                         BITS2WORDS(DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS),
                                                         row_cache));
    }
    else if (macro_type == MDB_EEDB_BANK)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.mdb_cluster_infos.eedb_banks_cache.range_read(unit, cluster_index, row_index,
                                                                                   BITS2WORDS
                                                                                   (DNX_DATA_MAX_MDB_DH_CLUSTER_ROW_WIDTH_BITS),
                                                                                   row_cache));
    }
    ...
    uint32 *row_data)
{
    ...
    if ((cache_enabled == TRUE) && ((mdb_action_apply == DBAL_MDB_ACTION_APPLY_ALL)
                                    || (mdb_action_apply == DBAL_MDB_ACTION_APPLY_SW_SHADOW)))
    {
        int cluster_iter;
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(mdb_direct_update_bucket_row_control
                        (unit, mem, MDB_CLUSTER_READ_INST, cluster_info->active_bitmap << cluster_index_in_bucket,
    ...
    int *cluster_idx)
{
    ...
    int cluster_idx_temp;
    int nof_clusters;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(mdb_db_infos.db.number_of_clusters.get(unit, mdb_physical_table_id, &nof_clusters));

    for (cluster_idx_temp = 0; cluster_idx_temp < nof_clusters; cluster_idx_temp++)
    {
        SHR_IF_ERR_EXIT(mdb_db_infos.db.
                        clusters_info.get(unit, mdb_physical_table_id, cluster_idx_temp, &cluster_info));
    ...
    if (cluster_idx_temp == nof_clusters)
    {
        SHR_SET_CURRENT_ERR(_SHR_E_RESOURCE);
        SHR_EXIT();
    }

    *cluster_idx = cluster_idx_temp;
    ...
    int *macro_index)
{
    ...
    SHR_IF_ERR_EXIT(mdb_direct_get_cluster_index(unit, physical_table, physical_address, &cluster_idx));
    ...
    *macro_index = cluster_info.macro_index;
    ...
    if (overflow_reg != NULL)
    {
        if (*row_in_cluster == nof_rows_in_cluster - 1)
        {
    ...
    dbal_physical_entry_t * entry)
{
    ...
    if (entry_size_type >= DBAL_NOF_ENUM_MDB_DIRECT_PAYLOAD_VALUES)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    SHR_IF_ERR_EXIT(mdb_direct_calc_cluster_info(unit, row_width, entry->payload_size, remainder_size, &cluster_info));

#ifndef ADAPTER_SERVER_MODE
    if ((overflow_reg != INVALIDr)
        && (cluster_info.clusters_offset[MDB_MAX_CLUSTER_PER_DB - 1] == MDB_CLUSTER_OFFSET_NEXT_ROW))
    {
        ...
        for (index = 0; index < MDB_CLUSTER_USE_TWO; index++)
        {
            if (cluster_info.active_bitmap & (1 << index))
            {
                int cluster_index_offset = cluster_info.access_info[index].cluster_offset;
                row_offset =
                    mdb_direct_cluster_data_offset_in_bucket[cluster_index_offset] +
                    cluster_info.access_info[index].cluster_row_offset;
                SHR_BITCOPY_RANGE(overflow_row, row_offset, entry->payload,
                                  cluster_info.access_info[index].entry_offset,
                                  cluster_info.access_info[index].entry_size);
                ...
                if (cache_enabled == TRUE)
                {
                    SHR_IF_ERR_EXIT(mdb_direct_cache_set
                                    (unit, macro_type, macro_index,
                                     (bucket_index * dnx_data_mdb.dh.nof_bucket_clusters_get(unit)) +
                                     cluster_index_in_bucket + cluster_info.access_info[index].cluster_offset,
                                     row_in_cluster + 1, mdb_direct_cluster_data_offset_in_bucket[cluster_index_offset],
                                     overflow_row));
                }
            }
        }
...
#endif
    {
        ...
        if ((cluster_info.nof_used_clusters > MDB_CLUSTER_USE_ONE) || (table_entry_size != MDB_NOF_CLUSTER_ROW_BITS))
        {
            LOG_VERBOSE(BSL_LOG_MODULE,
            ...
            SHR_IF_ERR_EXIT(mdb_direct_table_read_entry_from_cluster
                            (unit, mem, macro_type, macro_index, bucket_index, cluster_index_in_bucket, blk,
                             &cluster_info, entry->mdb_action_apply, row_in_cluster, row_data));
        ...
        for (index = 0; index < MDB_CLUSTER_USE_TWO; index++)
        {
            if (cluster_info.active_bitmap & (1 << index))
            {
                cluster_index = cluster_index_in_bucket + cluster_info.access_info[index].cluster_offset;

                row_offset =
                    mdb_direct_cluster_data_offset_in_bucket[cluster_index] +
                    cluster_info.access_info[index].cluster_row_offset;
                sal_memset(cluster_payload, 0x0, sizeof(cluster_payload));
                SHR_BITCOPY_RANGE(cluster_payload, 0 /* src_offset */ , entry_payload,
                                  cluster_info.access_info[index].entry_offset,
                                  cluster_info.access_info[index].entry_size);
                ...
                if (cache_enabled == TRUE)
                {
                    SHR_IF_ERR_EXIT(mdb_direct_cache_set
                                    (unit, macro_type, macro_index,
                                     ...
                                     row_in_cluster + cluster_info.clusters_offset[index],
...
#ifdef MDB_FPP_DUMPS
                {
                    int print_index;
                    char macro_str[128];
                    uint32 cluster_final_payload[BITS2WORDS(MDB_NOF_CLUSTER_ROW_BITS)];
                    dbal_tables_e logical_table;
                    SHR_IF_ERR_EXIT(dbal_tables_table_by_mdb_phy_get
                                    (unit, dbal_physical_table_id, app_id, &logical_table));
                    sal_memset(cluster_final_payload, 0x0, BITS2WORDS(MDB_NOF_CLUSTER_ROW_BITS) * sizeof(uint32));
                    SHR_BITCOPY_RANGE(cluster_final_payload, 0, row_data,
                                      mdb_direct_cluster_data_offset_in_bucket[cluster_index],
                                      MDB_NOF_CLUSTER_ROW_BITS);
                    SHR_IF_ERR_EXIT(mdb_direct_mem_blk_to_macro_string(unit, mem, blk, macro_str));
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit, "********************MDB-LOGIC-START********************\n")));
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U
                                 (unit,
                                  "//DB(%s)(%s), logical addr 'h%x\n`MDBT_SUBENV.m_dh_regfile_array[\"%s\"].macro_physical_entry_bank_mem_bd_write_entry(.inst(%d), .index(%d), .data(120'h"),
                                 dbal_logical_table_to_string(unit, logical_table), dbal_physical_table_to_string(unit,
                                                                                                                  dbal_physical_table_id),
                                 entry_index, macro_str,
                                 bucket_index * dnx_data_mdb.dh.nof_bucket_clusters_get(unit) + cluster_index,
                                 (row_in_cluster & MDB_DIRECT_ADDR_OFFSET_MASK(mem)) +
                                 cluster_info.clusters_offset[index]));
                    for (print_index = 0; print_index < BITS2WORDS(MDB_NOF_CLUSTER_ROW_BITS); print_index++)
                    {
                        if (print_index == 0)
                        {
                            LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "%06x"),
                                                         (0xffffff &
                                                          cluster_final_payload[BITS2WORDS
                                                                                (MDB_NOF_CLUSTER_ROW_BITS) -
                                                                                print_index - 1])));
                        }
                        else
                        {
                            LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "%08x"),
                                                         cluster_final_payload[BITS2WORDS(MDB_NOF_CLUSTER_ROW_BITS) -
                                                                               print_index - 1]));
                        }
                    }
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit, ")); // offset = %d \n"),
                                 cluster_info.access_info[index].cluster_row_offset));
                    LOG_VERBOSE(BSL_LOG_MODULE,
                                (BSL_META_U(unit, "********************MDB-LOGIC-END********************\n")));
                }
        ...
        /** coverity[ARRAY_VS_SINGLETON:FALSE] */
        SHR_IF_ERR_EXIT(mdb_direct_update_bucket_row_control
                        (unit, mem, MDB_CLUSTER_WRITE_INST, cluster_info.active_bitmap << cluster_index_in_bucket,
        ...
        {
            SHR_IF_ERR_EXIT(mdb_sbusdma_desc_add_mem(unit, mem, bucket_index, blk, row_in_cluster, row_data));
        }
        else
        {
    ...
    if (severity >= bslSeverityVerbose)
    {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U
                     (unit, "entry.key: %d. entry.payload_size: %d, entry.payload_offset: %d, physical_table: %s.\n"),
                     entry->key[0], entry->payload_size, entry->payload_offset, dbal_physical_table_to_string(unit,
                                                                                                              dbal_physical_table_id)));
    ...
    dbal_physical_entry_t * entry)
{
    ...
    SHR_IF_ERR_EXIT(mdb_direct_table_calc_index_address
                    (unit, mdb_physical_table, entry_index, table_entry_size, row_width,
                     ...
                     &cluster_index_in_bucket, &macro_type, &macro_index));
...
#endif
    {
        ...
        SHR_IF_ERR_EXIT(mdb_direct_table_read_entry_from_cluster
                        (unit, mem, macro_type, macro_index, bucket_index, cluster_index_in_bucket, blk, &cluster_info,
                         entry->mdb_action_apply, row_in_cluster, row_data));
    ...
    if(entry->hitbit & DBAL_PHYSICAL_KEY_HITBIT_ACTION)
    {
        ...
        for(hit_bit_index = MDB_DIRECT_ENTRY_HIT_BIT_INDEX_1; hit_bit_index < MDB_DIRECT_ENTRY_NOF_HIT_BIT_INDEXS;hit_bit_index++)
        {
            if(((hit_bit_index == MDB_DIRECT_ENTRY_HIT_BIT_INDEX_1) && _SHR_IS_FLAG_SET(entry->hitbit, DBAL_PHYSICAL_KEY_HITBIT_PRIMARY)) ||
                    ((hit_bit_index == MDB_DIRECT_ENTRY_HIT_BIT_INDEX_2) && _SHR_IS_FLAG_SET(entry->hitbit, DBAL_PHYSICAL_KEY_HITBIT_SECONDARY)))
            {
                soc_reg_above_64_val_t hitbit_row;
                int relevant_cluster;
                int hitbit_offset;
                uint32 hitbit_abk_position = dnx_data_mdb.direct.hit_bit_pos_in_abk_get(unit,hit_bit_index)->hit_bit_position;
                ...
                hit_bit_pos_on_cluster_1 = (((hit_bit_entry_remainder_size % row_width) < MDB_NOF_CLUSTER_ROW_BITS) && (row_width > MDB_NOF_CLUSTER_ROW_BITS)) ? 1 : 0;
                ...
                row_in_cluster_modified = (row_in_cluster + (hit_bit_entry_remainder_size / row_width)) & MDB_DIRECT_ADDR_OFFSET_MASK(mem);


                SHR_IF_ERR_EXIT(utilex_bitstream_set_bit_range
                                (&row_in_cluster_modified,
                                 MDB_DIRECT_ADDR_OFFSET_OFFSET(mem),
                                 MDB_DIRECT_ADDR_OFFSET_OFFSET(mem) +
                                 MDB_DIRECT_ADDR_OFFSET_BITS * dnx_data_mdb.dh.nof_bucket_clusters_get(unit) - 1));

                SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit
                                (&row_in_cluster_modified,
                                 MDB_DIRECT_ADDR_OFFSET_OFFSET(mem) + MDB_DIRECT_ADDR_OFFSET_BITS * relevant_cluster + 1));

                if (entry->hitbit & DBAL_PHYSICAL_KEY_HITBIT_CLEAR)
                {
                    SHR_IF_ERR_EXIT(utilex_bitstream_set_bit
                                    (hitbit_row,
                                     MDB_HITBIT_MASK_OFFSET(unit) + MDB_HITBIT_MASK_SIZE * relevant_cluster + hitbit_offset));
                }

                SHR_IF_ERR_EXIT(soc_mem_array_write
                                (unit, mem_hitbit[MDB_ABK_BANK_A], bucket_index, blk, row_in_cluster_modified, hitbit_row));
                SHR_IF_ERR_EXIT(soc_mem_array_write
                                (unit, mem_hitbit[MDB_ABK_BANK_B], bucket_index, blk, row_in_cluster_modified, hitbit_row));

                SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit
                                (&row_in_cluster_modified,
                                 MDB_DIRECT_ADDR_OFFSET_OFFSET(mem) + MDB_DIRECT_ADDR_OFFSET_BITS * relevant_cluster));


                for (bank_iter = MDB_ABK_BANK_A; bank_iter < MDB_NOF_ABK_BANKS;bank_iter++)
                {
                    SHR_IF_ERR_EXIT(soc_mem_array_read(unit, mem_hitbit[bank_iter], bucket_index, blk, row_in_cluster_modified, hitbit_row));
                    hitbit = 0;
                    SHR_BITCOPY_RANGE(&hitbit, 0, hitbit_row,
                                      MDB_HITBIT_DATA_OFFSET + MDB_HITBIT_DATA_SIZE * relevant_cluster + hitbit_offset, 1);
                    if (hitbit != 0)
                    {
    ...
    mdb_test_mode_e mode)
{
    ...
    index_factor =
        ((MDB_CEILING(MDB_DIRECT_BASIC_ENTRY_SIZE, entry_size)) * MDB_DIRECT_BASIC_ENTRY_SIZE) / entry_index_size;
    ...
    while (cluster_index < nof_clusters)
    {
        ...
        entry_index =
            UTILEX_DIV_ROUND_UP(start_address * row_width,
                                ((UTILEX_DIV_ROUND_UP(entry_size, MDB_DIRECT_BASIC_ENTRY_SIZE)) *
                                 MDB_DIRECT_BASIC_ENTRY_SIZE));
        while (entry_index <
               end_address * row_width / ((UTILEX_DIV_ROUND_UP(entry_size, MDB_DIRECT_BASIC_ENTRY_SIZE)) *
                                          MDB_DIRECT_BASIC_ENTRY_SIZE))
        {
            uint32 timer_usec;
            ...
            if (adapter_table_name[0] != 0)
            {
                ...
                SHR_IF_ERR_EXIT(control_get_table_data
                                (unit, &(adapter_table_name[0]), "mdb", entry_index, tbl_entry_data_allocated_size,
                                 &tbl_entry_size, &(tbl_entry_data[0])));

                SHR_IF_ERR_EXIT(utilex_bitstream_xor
                                ((uint32 *) tbl_entry_data, entry.payload, BITS2WORDS(entry.payload_size)));

                while ((utilex_bitstream_have_one((uint32 *) tbl_entry_data, BITS2WORDS(entry.payload_size)) == 1)
                       && (attempt_counter < 10000))
                {
                    attempt_counter++;
                    SHR_IF_ERR_EXIT(control_get_table_data
                                    (unit, &(adapter_table_name[0]), "mdb", entry_index, tbl_entry_data_allocated_size,
    ...
    dbal_enum_value_field_direct_payload_sizes_e * payload_type)
{
    ...
    {
        dbal_enum_value_field_mdb_eedb_phase_e phase_index = DBAL_NOF_ENUM_MDB_EEDB_PHASE_VALUES;
        switch (mdb_physical_table_id)
        {
            ...
            case DBAL_ENUM_FVAL_MDB_PHYSICAL_TABLE_EEDB_8_DATA:
                ...
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_FAIL, "No matching phase was found to the EEDB physical database.\n");
        }
