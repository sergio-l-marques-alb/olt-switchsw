    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_reg_info_allocate(&pemla_reg_info, &pemla_reg_info_size));
    ...
    for (table_id = 0; table_id < DBAL_NOF_TABLES; table_id++)
    {
        ...
        if (table->access_method == DBAL_ACCESS_METHOD_PEMLA)
        {
             ...
             * update table info 
             */
            table->app_id = pemla_reg_info[pem_reg_index].reg_id;
            ...
            for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
            {
                ...
                for (field_index = 0; field_index < multi_res_info->nof_result_fields; field_index++)
                {
                    ...
                    if (pem_field_index == pemla_reg_info[pem_reg_index].nof_fields)
                    {
                        ...
                        /*
                         * Table has mistmatch - set table to ML 1 (DBAL_MATURITY_PARTIALLY_FUNCTIONAL).
                         */
                    ...
                    {
                        /*
                         * size mistmatch - set to lower boundary.
                         */
                    ...
                    else if (field_info->field_nof_bits < pem_field_info->field_size_in_bits)
                    {
                        ...
                        LOG_VERBOSE(BSL_LOG_MODULE,
                                    (BSL_META_U
                                     (unit, "APPL field size[%d]<ARCH field size[%d] set table status to hwErr\n"),
                                     field_info->field_nof_bits, pem_field_info->field_size_in_bits));
                    }
                }
            }
...
exit:
    SHR_IF_ERR_EXIT(dnx_pemladrv_reg_info_de_allocate(pemla_reg_info, pemla_reg_info_size));
    SHR_FUNC_EXIT;
}
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (field_index = 0; field_index < nof_results_field; field_index++)
    {
        ...
        DBAL_PEM_REG_WRITE_LOG(entry_handle->table->app_id, entry_handle->value_field_ids[field_index],
                               entry_handle->table->pemla_mapping.result_fields_mapping[field_index],
    ...
    SHR_IF_ERR_EXIT(pemladrv_reg_write(unit, entry_handle->table->app_id, pemla_drv_mem));
    ...
    dbal_entry_handle_t * entry_handle)
{
     ...
     *  is fixed */
    SHR_IF_ERR_EXIT(pemladrv_reg_read(unit, entry_handle->table->app_id, pemla_reg_result));
    ...
    for (pemla_field_index = 0; pemla_field_index < pemla_reg_result->nof_fields; pemla_field_index++)
    {
        ...
        DBAL_PEM_REG_READ_LOG(entry_handle->table->app_id, table_field_info->field_id,
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_ALLOC_SET_ZERO(entry_value, entry_payload_size, "pemla entry value array", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    SHR_IF_ERR_EXIT(dnx_pemladrv_full_reg_write
                    (unit, entry_handle->table->app_id, entry_payload_size, (const unsigned int *) entry_value));
