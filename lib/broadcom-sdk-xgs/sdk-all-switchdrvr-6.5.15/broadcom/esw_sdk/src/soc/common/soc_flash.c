/*
 * $Id: soc_flash.c,v 1.00 Broadcom SDK $
                   ...
                   soc_flash_conf_t *flash_conf)
{
    ...
    rv = SOC_E_UNAVAIL;
    for (i = 0; i < sizeof(flash_table) / sizeof(soc_flash_conf_t); ++i) {
        if (flash_table[i].id == id[0]) {
            /* we have a match */
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SOCMEM,
                  (BSL_META_U(unit,
                   "Flash not supported = 0x%x\n"), id[0]));
        goto err_read_id;
    }
    else {
        LOG_ERROR(BSL_LS_SOC_SOCMEM,
                  (BSL_META_U(unit,
                   "Flash supported ID = 0x%x\n"), id[0]));
    }
