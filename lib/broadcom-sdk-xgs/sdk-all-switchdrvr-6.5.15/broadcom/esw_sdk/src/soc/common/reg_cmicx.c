/*
 * $Id: reg_cmicx.c,v 1.0 Broadcom SDK $
...
*soc_cmicx_srch (uint32 addr) {
    ...
    return (cmicx_regs[mid].addr == addr) ? &(cmicx_regs[mid]) : NULL;
}
...
soc_reg_t
soc_cmicx_addr_reg (uint32 addr) {
    soc_cmicx_reg_t *cmreg = soc_cmicx_srch(addr);
    return (cmreg == NULL)?INVALIDr:cmreg->reg;
}

char *
soc_cmicx_addr_name (uint32 addr) {
    soc_cmicx_reg_t *cmreg = soc_cmicx_srch(addr);
