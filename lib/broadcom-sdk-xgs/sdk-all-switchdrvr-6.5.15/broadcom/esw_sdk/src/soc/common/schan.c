                         ...
                         int data_byte_len, int dma, uint32 bank_ignore_mask)
{
    if (soc_feature(unit, soc_feature_sbus_format_v4)) {
        ...
        header->v4.dst_blk = soc_feature(unit, soc_feature_blkid_extended_format) ? (dst_blk >> 4) : dst_blk;
             ...
             int intr)
{
    ...
    if (_soc_schan_drv[unit].soc_schan_op == NULL) {
        ...
        rv =  _soc_schan_drv[unit].soc_schan_op(unit, msg, dwc_write, dwc_read, intr);
    }
                                ...
                                int dwc_read)
{
    ...
    {
        if (msg->header.v3.opcode != READ_REGISTER_CMD_MSG &&
            msg->header.v3.opcode != READ_MEMORY_CMD_MSG &&
            dnxc_ha_is_access_disabled(unit,DNXC_HA_ALLOW_SCHAN))
        {
            LOG_WARN(BSL_LS_SOC_SCHAN,
                (BSL_META_U(unit,
                                "soc_schan_op: ERROR, It is not currently allowed to modify HW\n")));
            return SOC_E_FAIL;
        }
    }
#endif
    ...
    if SOC_IS_DNX(unit) {
        if (msg->header.v3.opcode != READ_REGISTER_CMD_MSG &&
            msg->header.v3.opcode != READ_MEMORY_CMD_MSG &&
            !DNX_ERR_RECOVERY_HW_ACCESS_IS_VALID(unit))
        {
            LOG_WARN(BSL_LS_SOC_SCHAN,
                (BSL_META_U(unit,
                                "soc_schan_op: ERROR, Invalid error recovery hw access\n")));
            return SOC_E_FAIL;
        }
    }
#endif
        ...
        BCM_UNIT_DO_HW_READ_WRITE(unit))
    {
        LOG_WARN(BSL_LS_SOC_SCHAN,
            (BSL_META_U(unit,
                            "soc_schan_op: ERROR, trying to access HW while HW Log journaling is enabled !\n")));
        return SOC_E_FAIL;
    }
#endif /*CRASH_RECOVERY_SUPPORT*/
    if (SOC_IS_SAND(unit) && !SOC_IS_DNX(unit) && !SOC_IS_DNXF(unit) &&
        msg->header.v3.opcode != READ_REGISTER_CMD_MSG &&
        msg->header.v3.opcode != READ_MEMORY_CMD_MSG &&
...
********************************************/
int
soc_schan_timeout_check(int unit, int *rv, schan_msg_t *msg, int cmc, int ch)
{
    if (*rv != SOC_E_TIMEOUT) {
