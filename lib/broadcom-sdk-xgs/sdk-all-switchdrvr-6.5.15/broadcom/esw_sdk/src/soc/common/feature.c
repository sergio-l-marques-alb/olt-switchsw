...
soc_features_bcm56770_a0(int unit, soc_feature_t feature)
{
    ...
    switch (feature) {
    case soc_feature_field_single_pipe_support:
    case soc_feature_mv2_style_flowtracker:
    case soc_feature_mv2_dummy_flowtracker:
    case soc_feature_flex_stat_ing_tcp_flags_support:
    case soc_feature_time_support:
    case soc_feature_iproc:
    case soc_feature_portmod:
...
soc_features_bcm56670_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_iom:
        return TRUE;
    case soc_feature_monterey_a0_fp_sw_war:
        /*
         * Enable this feature for A0 rev of Monterey and its variants.
         * Monterey and its all variants are defined with same rev_id internally
         * with different IDs. So Check for only BCM56670_A0_REV_ID.
         */
        if ((((BCM56670_DEVICE_ID) == (dev_id))
            || ((BCM56671_DEVICE_ID) == (dev_id))
            || ((BCM56672_DEVICE_ID) == (dev_id))
            || ((BCM56675_DEVICE_ID) == (dev_id)))
            && (BCM56670_A0_REV_ID == rev_id)) {
            return TRUE;
        } else {
            return FALSE;
        }
