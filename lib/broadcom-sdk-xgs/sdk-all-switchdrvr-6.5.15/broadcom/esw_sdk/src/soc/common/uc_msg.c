...
#if defined(BCM_JERICHO_2_SUPPORT)
...
#endif
        } else if (rcv.s.subclass == MOS_MSG_SUBCLASS_SYSTEM_EXCEPTION) {
                          ...
                          soc_cmic_uc_appl_cb_t *shutdown_cb, void *cb_data)
{
    ...
    if(msg_class == MOS_MSG_CLASS_FT) {
        ft_msg_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_FLOWTRACKER */
#if defined(INCLUDE_IFA)
    if(msg_class == MOS_MSG_CLASS_IFA) {
        ifa_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_IFA */
#if defined(INCLUDE_TELEMETRY)
    if(msg_class == MOS_MSG_CLASS_ST) {
        st_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_TELEMETRY */
#if defined(INCLUDE_INT)
    if(msg_class == MOS_MSG_CLASS_INT) {
        int_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_INT */
#if defined(INCLUDE_SUM)
    if(msg_class == MOS_MSG_CLASS_SUM) {
        sum_firmware_version = soc_ntohl(rcv.s.data);
    }
#endif /* INCLUDE_SUM */
    return (SOC_E_NONE);
}
