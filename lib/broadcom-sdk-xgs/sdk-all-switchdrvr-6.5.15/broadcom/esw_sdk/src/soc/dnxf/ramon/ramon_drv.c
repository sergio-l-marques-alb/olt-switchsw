...
/* Ramon defines soecifically for the SW workaround
   for the soft reset.
   All these defines should be deleted for future devices
   where the silicon bug should be fixed*/
...
soc_ramon_set_dcml_config(int unit)
{
     ...
     * After bcm_dnxf_port_init the enabled by soc_properties ports 
...
/** This function is only to fix a silicon bug! Should be
 *  removed and all its implementation in future devices */
/*
 * Function:
 *      soc_ramon_drv_soft_init_dch_reg_info_get
 * Purpose:
 *      Collects the register info needed for the post soft reset init of the DCH blocks.
 * Parameters:
 *      unit  -                     (IN)     Unit number.
 *      dch_soft_reset_info -       (OUT)
 *      nof_relevant_dch_registers  (OUT)
 * Returns:
 *      _SHR_E_NONE     No Error  
 ...
 * Purpose:
 *      Sets all values for the DCH blocks collected from this function
 *      soc_ramon_drv_soft_init_dch_reg_info_get
 * Parameters:
 *      unit  -                     (IN)     Unit number.
 *      dch_soft_reset_info -       (IN)
 *      nof_relevant_dch_registers  (IN)
 * Returns:
 *      _SHR_E_NONE     No Error  
 *      _SHR_E_UNAVAIL  Feature unavailable  
 *      _SHR_E_XXX      Error occurred  
...
soc_ramon_drv_soft_init(int unit, uint32 soft_reset_mode_flags)
{
    ...
    /** SW workaround for a silicon bug which doesn't allow a
     *  simple soft reset for Ramon. This code should be deleted
     *  for future devices! */
    if (SOC_CONTROL(unit)->soc_flags & SOC_F_INITED)
    {
        if (SOC_IS_RAMON_A0(unit) || SOC_IS_RAMON_A1(unit) || SOC_IS_RAMON_B0(unit))
        {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Ramon SW reset WA ERROR - Due to silicon bug for Ramon SW WorkAround was triggered. Check if fixed for future device! For more info SDK-118583.");
        }
    }
    ...
    if (soft_reset_mode_flags == SOC_DNXC_RESET_ACTION_IN_RESET || soft_reset_mode_flags == SOC_DNXC_RESET_ACTION_INOUT_RESET)
    {
        ...
        if (SOC_CONTROL(unit)->soc_flags & SOC_F_INITED)
        {
            ...
            if (SOC_IS_RAMON_A0(unit) || SOC_IS_RAMON_A1(unit) || SOC_IS_RAMON_B0(unit))
            {
                ...
                SOC_REG_ABOVE_64_NOT(reg_above_64);
