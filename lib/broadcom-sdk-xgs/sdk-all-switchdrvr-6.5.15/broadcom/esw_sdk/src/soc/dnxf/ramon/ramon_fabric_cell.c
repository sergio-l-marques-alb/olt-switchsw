...
shr_error_e
soc_ramon_fabric_cell_parse_table_get(int unit, soc_dnxc_fabric_cell_type_t cell_type, int valid_data_in_msb, uint32 max_nof_lines, soc_dnxc_fabric_cell_parse_table_t *parse_table, uint32 *nof_lines)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (cell_type == soc_dnxc_fabric_cell_type_sr_cell)
    {
        ...
        if (valid_data_in_msb)
        {
            soc_dnxc_fabric_cell_parse_table_t temp_table_src[] = SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_VALID_DATA_IN_MSB;
        ...
        {
            soc_dnxc_fabric_cell_parse_table_t temp_table_src[] = SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_VALID_DATA_NOT_IN_MSB;
            sal_memcpy(parse_table, temp_table_src, sizeof(soc_dnxc_fabric_cell_parse_table_t) * SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES);
        }
        *nof_lines = SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES;

    } else { /*cpu data cell*/
        if (max_nof_lines < SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Bigger table is required\n");
        }
        ...
        if (valid_data_in_msb)
        {
            soc_dnxc_fabric_cell_parse_table_t temp_table_captured[] = SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_VALID_DATA_IN_MSB;
            sal_memcpy(parse_table, temp_table_captured, sizeof(soc_dnxc_fabric_cell_parse_table_t) * SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES);
        }
        else
        {
            soc_dnxc_fabric_cell_parse_table_t temp_table_captured[] = SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_VALID_DATA_NOT_IN_MSB;
            sal_memcpy(parse_table, temp_table_captured, sizeof(soc_dnxc_fabric_cell_parse_table_t) * SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES);
        }
        *nof_lines = SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES;
    }
...
/* Offset from start of CPU packet */
#define _SOC_RAMON_PARSING_VALID_DATA_IN_MSB_START         (2169)
#define _SOC_RAMON_PARSING_VALID_DATA_IN_MSB_LENGTH        (1)
#define _SOC_RAMON_PARSING_CELL_SIZE_START                 (2157)
#define _SOC_RAMON_PARSING_CELL_SIZE_LENGTH                (8)
#define _SOC_RAMON_PARSING_CELL_IS_SMALL(cell_size) \
    ((cell_size) < 128)
/* if valid_data_in_msb == 1 it means that the cell is adjacent to the header in physical memory.
 * this is relevant only for a small cell ( < 128 byte).
 * a large cell is always adjacent to header. */
shr_error_e
soc_ramon_fabric_cell_valid_data_in_msb_get(int unit, soc_dnxc_fabric_cell_entry_t entry, int *valid_data_in_msb)
{
    uint32 cell_size[1] = {0};
    SHR_FUNC_INIT_VARS(unit);

    SHR_BITCOPY_RANGE(cell_size, 0, (uint32 *) entry, _SOC_RAMON_PARSING_CELL_SIZE_START, _SOC_RAMON_PARSING_CELL_SIZE_LENGTH);

    /* the payload (data) is always in msb (adjacent to header) for a large cell.
     * for a small cell - check the value of the bit */
    *valid_data_in_msb = 1;
    if (_SOC_RAMON_PARSING_CELL_IS_SMALL(cell_size[0]))
    {
        SHR_BITCOPY_RANGE((uint32 *) valid_data_in_msb, 0, (uint32 *) entry, _SOC_RAMON_PARSING_VALID_DATA_IN_MSB_START, _SOC_RAMON_PARSING_VALID_DATA_IN_MSB_LENGTH);
    }
    SHR_FUNC_EXIT;
}
