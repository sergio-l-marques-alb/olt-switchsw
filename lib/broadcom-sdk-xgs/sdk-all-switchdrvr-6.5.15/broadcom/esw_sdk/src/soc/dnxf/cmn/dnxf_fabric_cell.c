...
soc_dnxf_fabric_cell_parse(int unit, soc_dnxc_fabric_cell_entry_t entry, soc_dnxc_fabric_cell_info_t *cell_info)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_cell_valid_data_in_msb_get, (unit, entry, &valid_data_in_msb));
    SHR_IF_ERR_EXIT(rv);

    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_cell_parse_table_get, (unit, cell_info->cell_type, valid_data_in_msb, SOC_DNXC_FABRIC_CELL_PARSE_TABLE_MAX_NOF_LINES, parse_table, &nof_lines));
    SHR_IF_ERR_EXIT(rv);

    rv = soc_dnxc_fabric_cell_parser(unit, entry, parse_table, nof_lines, cell_info);
    SHR_IF_ERR_EXIT(rv);
