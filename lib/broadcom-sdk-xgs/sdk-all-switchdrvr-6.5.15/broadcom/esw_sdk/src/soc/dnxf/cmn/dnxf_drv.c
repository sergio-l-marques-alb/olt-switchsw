    ...
    int unit)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++)
    {
        ...
        if (mem == QRH_MCLBTm)
        {
            /** the memory changed by HW, copied from RTP_RMHMT table */
            continue;
        }
        if (mem == MCT_MCTm)
        {
        ...
        if ((mem == FSRD_FSRD_WL_EXT_MEMm) || (mem == FSRD_FSRD_PROM_MEMm))
        {
            continue;
        }
...
int dnxf_mem_is_in_soc_property_specific(int unit, soc_mem_t mem, int en)
{
...
dnxf_tbl_mem_cache_enable_specific_tbl(int unit, soc_mem_t mem, void* en)
{
    ...
    SOC_MEM_ALIAS_TO_ORIG(unit, mem);
    if (dnxf_mem_is_in_soc_property_specific(unit, mem, cache_enable))
    {
        if(!SOC_MEM_IS_VALID(unit, mem) || !soc_mem_is_cachable(unit, mem))
        {
