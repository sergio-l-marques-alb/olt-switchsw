  ...
  )
{
    ...
    if (SOC_IS_JERICHO_2_ONLY(unit)) { /* in JR2 emulation use only 1st instance of per core blocks */
        switch(SOC_BLOCK_TYPE(unit, SOC_MEM_BLOCK_ANY(unit, mem))) { /* block type */
            case SOC_BLK_KAPS:
            case SOC_BLK_IPPA:
            case SOC_BLK_IPPB:
            case SOC_BLK_IPPC:
    ...
    if (SOC_IS_DNX(unit))
    {
        switch (mem)
        {
            ...
            case EPS_CAL_CAL_INDX_MUXm:
            {
                ...
                SHR_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, reg_shaper, reg_port, 0, OTM_CRDT_TABLE_READ_ENf, 0x0));
                break;
            }
            case EPS_QP_CREDIT_TABLEm:
            {
                ...
                SHR_IF_ERR_EXIT(soc_reg_above_64_get(unit, reg_shaper, reg_port, 0, eps_shaper_set[0]));
                eps_protect_flag[0] = 1;
                SHR_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, reg_shaper, reg_port, 0, QP_CRDT_TABLE_READ_ENf, 0x0));
                break;
            }
            ...
            case EPS_TCG_CBMm:
            {
                ...
                SHR_IF_ERR_EXIT(soc_reg_above_64_get(unit, reg_shaper, reg_port, 0, eps_shaper_set[0]));
                eps_protect_flag[0] = 1;
                SHR_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, reg_shaper, reg_port, 0, TCG_CRDT_TABLE_READ_ENf, 0x0));
                break;
            }
            case INVALIDm:
            {
...
int
dnx_dbal_egq_credit_mem_read_start(
    int unit,
    uint32* old_status)
{
    return _SHR_E_NONE;
...
int
dnx_dbal_egq_credit_mem_read_end(
    int unit,
    uint32* old_status)
{
    return _SHR_E_NONE;
