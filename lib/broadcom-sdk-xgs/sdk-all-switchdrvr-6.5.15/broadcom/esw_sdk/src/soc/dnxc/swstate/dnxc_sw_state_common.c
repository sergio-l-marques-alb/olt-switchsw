...
int dnxc_sw_state_memcpy_common(
     ...
     * Verify input.
     */
    DNXC_SW_STATE_VERIFY(dnxc_sw_state_memcpy_verify_common(unit, module_id, size, dest, src, flags, dbg_string));
...
int dnxc_sw_state_memset_common(
     ...
     * Verify input.
     */
    DNXC_SW_STATE_VERIFY(dnxc_sw_state_memset_verify_common(unit, module_id, dest, value, size, flags, dbg_string));
