 ...
 */
int
soc_dnxc_fabric_broadcast_firmware_loader(int unit, soc_pbmp_t *pbmp)
{
    ...
    for (i = 0 ; i < phymod_firmware_load_info.ucode_len ; i+= UC_TABLE_ENTRY_SIZE)
    {
        ...
        { /*last time*/
            sal_memcpy((uint8 *)wr_data, phymod_firmware_load_info.ucode_ptr + i, phymod_firmware_load_info.ucode_len - i);
        }
 ...
 */
shr_error_e
soc_dnxc_fabric_single_pm_add(int unit, int core, int cores_num, int phy_offset, uint32 bypass_lanes_bitmap, int use_mutex, dnxc_core_address_get_f address_get_func)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        ref_clk_dnx = dnx_data_pll.general.fabric_pll_cfg_get(unit, pll_index)->out_freq;
        if (ref_clk_dnx==DNX_SERDES_REF_CLOCK_BYPASS)
        {
            ref_clk_dnx = dnx_data_pll.general.fabric_pll_cfg_get(unit, pll_index)->in_freq;
        }
    ...
    {
        pm_info.pm_specific_info.pm8x50_fabric.force_single_pll = 1;
    }
    else
    {
 ...
 */
shr_error_e
soc_dnxc_fabric_pms_add(int unit, int cores_num, int phy_offset, uint32 bypass_lanes_bitmap, int use_mutex, dnxc_core_address_get_f address_get_func)
{
    int core = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnxc_fabric_single_pm_add(unit, core, cores_num, phy_offset, bypass_lanes_bitmap, use_mutex, address_get_func));
    }
...
soc_dnxc_port_phy_control_is_supported_validate(int unit, soc_port_t port, soc_phy_control_t type)
{
    ...
    switch (type)
    {
        ...
        case SOC_PHY_CONTROL_SCRAMBLER:
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "SOC_PHY_CONTROL_SCRAMBLER is no longer supported, to control scrambler please use bcm_port_resource_set/get\n");
            break;
        ...
        case SOC_PHY_CONTROL_PRBS_DECOUPLED_RX_ENABLE:
            SHR_ERR_EXIT(_SHR_E_UNAVAIL, "BCM_PORT_PHY_CONTROL_PRBS_DECOUPLED_ controls are no longer supported. To enable PRBS please refer to PRBS section in User Manual\n");
            break;
        ...
        case SOC_PHY_CONTROL_EYE_VAL_VT_D:
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL, "This control is not supported for PAM speed, please use BER projection tool instead.\n");
            }
            break;
        }
        default:
...
soc_dnxc_port_resource_validate(int unit, const bcm_port_resource_t *resource)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
     ...
     {
         SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: flags, physical_port, lanes and encap fields of resource are not in use. They should all be initialized to 0\n", resource->port);
     }
     ...
     {
         SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: setting both FORCE_NS and FORCE_ES is not a valid option. Only one can be set\n", resource->port);
     }
     ...
     if (resource->link_training == 1)
     {
         ...
         {
             SHR_ERR_EXIT(_SHR_E_CONFIG, "port %d: pam4 channel loss configuration - bits [16:21] of phy_lane_config - is expected to be 0 when link training is enabled. pam4 channel loss is %u\n", resource->port, pam4_channel_loss);
         }
     }
...
soc_dnxc_port_fabric_resource_lane_config_default_get(int unit, bcm_gport_t port, bcm_port_resource_t *resource) {
    ...
    BCM_PORT_RESOURCE_PHY_LANE_CONFIG_MEDIUM_SET(resource->phy_lane_config, BCM_PORT_RESOURCE_PHY_LANE_CONFIG_MEDIUM_BACKPLANE);
...
soc_dnxc_port_fabric_resource_default_get(int unit, bcm_gport_t port, uint32 flags, bcm_port_resource_t *resource)
{
    ...
    if ((resource->flags != 0) || (resource->physical_port != 0) || (resource->lanes != 0) || (resource->encap != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: flags, physical_port, lanes and encap fields of bcm_port_resource_t are not in use. They should all be initialized to 0\n", port);
    }
    ...
    if (resource->speed == BCM_PORT_RESOURCE_DEFAULT_REQUEST)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "port %d: there is no given default value for speed. speed is mandatory as input to this API\n", port);
    }
...
soc_dnxc_port_phy_tx_set(int unit, bcm_port_t port, int phyn, int lane, int is_sys_side, int is_init_sequence, bcm_port_phy_tx_t* tx)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        {
			SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_nof_get(unit, port, &num_lane));
			lane_speed = DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_type, 1) ? resource.speed : (resource.speed / num_lane);
        }
    }
#endif
    ...
    if (is_init_sequence && (tx->main == DNXC_PORT_TX_FIR_INVALID_MAIN_TAP))
    {
        ...
        signalling_mode = (DNXC_PORT_PHY_SPEED_IS_PAM4(lane_speed))? phymodSignallingMethodPAM4 : phymodSignallingMethodNRZ;
    ...
    else /* configure Tx params as given to the function */
    {
        ...
        phymod_tx.sig_method = (tx->signalling_mode == bcmPortPhySignallingModeNRZ)? phymodSignallingMethodNRZ : phymodSignallingMethodPAM4;
    }
...
soc_dnxc_port_phy_tx_get(int unit, bcm_port_t port, int phyn, int lane, int is_sys_side, bcm_port_phy_tx_t* tx)
{
    ...
    for (phy_index = 0 ; phy_index < phys_returned; phy_index++){
        ...
        if(phys[phy_index].access.lane_mask) {
            ...
            tx->signalling_mode = (phymod_tx.sig_method == phymodSignallingMethodNRZ)? bcmPortPhySignallingModeNRZ : bcmPortPhySignallingModePAM4;
            break;
        }
    }
...
shr_error_e
soc_dnxc_port_prbs_rx_invert_data_get(int unit, soc_port_t port, soc_dnxc_port_prbs_mode_t mode, int *invert)
{
    int flags = 0;
    phymod_prbs_t config;
    portmod_prbs_mode_t portmod_mode;
...
shr_error_e
soc_dnxc_port_prbs_rx_invert_data_set(int unit, soc_port_t port, soc_dnxc_port_prbs_mode_t mode, int invert)
{
    int flags = 0;
    phymod_prbs_t config;
    portmod_prbs_mode_t portmod_mode;
...
shr_error_e
soc_dnxc_port_disable_and_save_enable_state(int unit, soc_port_t port, int *orig_enabled)
{
    SHR_FUNC_INIT_VARS(unit);
...
shr_error_e
soc_dnxc_port_restore_enable_state(int unit, soc_port_t port, int orig_enabled)
{
    SHR_FUNC_INIT_VARS(unit);
...
shr_error_e soc_dnxc_port_control_cells_fec_bypass_enable_set(int unit, soc_port_t port, int value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SHR_FUNC_INIT_VARS(unit);
 ...
 * disabling FEC bypass means to do error correction (FEC) on those cells.
 * this configuration is relevant only if the FEC is one of the RS FEC types */
shr_error_e soc_dnxc_port_control_cells_fec_bypass_enable_get(int unit, soc_port_t port, int *value)
{
    uint32 properties = 0;
    portmod_port_pcs_t encoding;
    SHR_FUNC_INIT_VARS(unit);
...
static shr_error_e
soc_dnxc_port_fmac_index_get(int unit, int link, int *fmac_index)
{
    SHR_FUNC_INIT_VARS(unit);
...
/** See .h file */
shr_error_e
soc_dnxc_port_rate_egress_ppt_set(int unit, soc_port_t link, uint32 burst, uint32 nof_tiks)
{
    uint32 limit_max = 0;
    int max_length = 0;
    uint64 reg64_val;
...
/** See .h file */
shr_error_e
soc_dnxc_port_rate_egress_ppt_get(int unit, soc_port_t link, uint32 *burst, uint32 *nof_tiks)
{
    uint64 reg64_val;
    int fmac_index = 0; 
    SHR_FUNC_INIT_VARS(unit);
