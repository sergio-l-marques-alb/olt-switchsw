    ...
    while (p <= data_block_header->ptr_offsets_stack_base) {
        ...
        switch (p->type) {
            ...
            case SW_STATE_ALLOC_ELEMENT_MUTEX:
                ...
                *tmp_ptr = (uint8*)sal_mutex_create("sw state restored mutex");
                break;
        }
        p++;
    }
...
int dnxc_sw_state_deinit_wb(int unit, uint32 flags) {
    ...
    while (p <= dnxc_sw_state_data_block_header[unit]->ptr_offsets_stack_base) {
        if (p->type == SW_STATE_ALLOC_ELEMENT_MUTEX) {
            mutex = *((sal_mutex_t*)((uint8*)(dnxc_sw_state_data_block_header[unit]->data_ptr) + p->ptr_offset));
            sal_mutex_destroy(mutex);
            *((sal_mutex_t*)((uint8*)(dnxc_sw_state_data_block_header[unit]->data_ptr) + p->ptr_offset)) = NULL;
        }
        p++;
    }
...
int dnxc_sw_state_free_wb(
    ...
    while (p <= stack_base) {
        if (p->ptr_value == *ptr_location) {
            ...
            } else {  /* SW_STATE_ALLOC_ELEMENT_MUTEX */
                sal_mutex_destroy(*(sal_mutex_t*)ptr_location);
            }
