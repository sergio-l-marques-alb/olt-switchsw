...
#define DNXC_XOR_MEM_DATA_MAX_SIZE       20
    ...
    char *msg)
{
         ...
         current_index++)
    {
        ...
        rc = soc_mem_array_write(unit, mem, shadow_correct_info_p->array_index, shadow_correct_info_p->copyno,
                                 current_index, data_entry);
        SHR_IF_ERR_EXIT(rc);
    }
    ...
    uint32 *data_entry)
{
    ...
    rc = soc_mem_array_read_flags(unit, ecc_1b_correct_info_p->mem, ecc_1b_correct_info_p->array_index,
                                  ecc_1b_correct_info_p->copyno, current_index, data_entry, SOC_MEM_DONT_USE_CACHE);
    SHR_IF_ERR_EXIT(rc);
    ...
    char *msg)
{
    ...
    if (ecc_1b_correct_info_p->mem == FQP_OTM_METER_TABLEm)
    {
         ...
         current_index++)
    {
        ...
        rc = soc_mem_array_write(unit, ecc_1b_correct_info_p->mem, ecc_1b_correct_info_p->array_index,
    ...
    char *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        SHR_IF_ERR_EXIT(dnx_redo_soft_reset_soft_init(unit, 0));
    }
    else
#endif
    {
        rc = soc_device_reset(unit, SOC_DNXC_RESET_MODE_BLOCKS_AND_FABRIC_SOFT_RESET,
                              SOC_DNXC_RESET_ACTION_INOUT_RESET);
        SHR_IF_ERR_EXIT(rc);
    }
exit:
    SHR_FUNC_EXIT;
    ...
    char *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        SHR_EXIT();
#if 0
        SHR_IF_ERR_EXIT(soc_device_reset(unit, SOC_DNXC_RESET_MODE_HARD_RESET, SOC_DNXC_RESET_ACTION_INOUT_RESET));
#endif
    }
#endif
    ...
    void *param2)
{
    ...
    switch (corr_act)
    {
        ...
        case DNXC_INT_CORR_ACT_HANDLE_OAMP_EVENT_FIFO:
            dnx_oam_oamp_dma_event_handler(INT_TO_PTR(unit));
            break;
