 ...
 */
#define DNXC_SW_STATE_VALIDATE_DLL_ENTRY(_dll_entry)                                    \
do {                                                                                    \
...
STATIC int dnxc_sw_state_dll_entry_free(int unit, dnxc_sw_state_allocation_dll_entry_t *dll_entry)
{
     ...
     */
    DNXC_SW_STATE_VALIDATE_DLL_ENTRY(dll_entry);
    ...
    uint32 flags)
{
    ...
    while(entry != NULL) {
         ...
         * validate that the dll entry is intact
         */
        DNXC_SW_STATE_VALIDATE_DLL_ENTRY(entry);
        ...
        { 
            sal_mutex_destroy((sal_mutex_t)entry->ptr);
            *(entry->ptr_location) = NULL;
        }
        else
        {
 ...
 */
     ...
     * Verify input.
     */
    DNXC_SW_STATE_VERIFY(dnxc_sw_state_alloc_verify_common(unit, module_id, ptr_location, nof_elements,element_size, flags, dbg_string));
...
int dnxc_sw_state_alloc_size_plain(int unit, uint32 module, uint8 *location, uint32 *nof_elements, uint32 *element_size)
{
     ...
     * Verify input.
     */
    DNXC_SW_STATE_VERIFY(dnxc_sw_state_alloc_size_verify_common(unit, module, location, nof_elements, element_size));
...
int dnxc_sw_state_free_plain(int unit, uint32 module_id, uint8 **ptr_location, uint32 flags, char *dbg_string)
{
     ...
     * Verify input.
     */
    DNXC_SW_STATE_VERIFY(dnxc_sw_state_free_verify_common(unit, module_id, ptr_location, flags, dbg_string));
    ...
    if (module_id != MULTITHREAD_ANALYZER_MODULE_ID) {
         ...
         */
        if(((dll_entry->ptr_location != ptr_location) && !sw_state_is_flag_on(flags, DNXC_SW_STATE_LINKED_LIST_FREE)) 
