...
#define DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY_PTR_TO_ADDRESS(ptr, old_address, new_address, size) \
    do{ \
        if(((uint8 *)(ptr) - (uint8 *)(old_address) >= 0) \
        && ((uint8 *)(old_address) + (size) - (uint8 *)(ptr) > 0)) { \
...
do {                                                                                      \
    _is_er = dnxc_rollback_journal_is_on(_unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE);   \
    _is_cmp = dnxc_rollback_journal_is_on(_unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE);\
    if((!_is_er) && (!_is_cmp))                                                           \
    {                                                                                     \
        SHR_EXIT();                                                                       \
    }                                                                                     \
    ...
    uint8 *result)
{
     ...
     */
    uint8 *ptr1 = ((dnxc_sw_state_journal_entry_t *)(first_metadata))->data.memcpy_data.ptr;
    uint8 *ptr2 = ((dnxc_sw_state_journal_entry_t *)(second_metadata))->data.memcpy_data.ptr;
    ...
    uint8 *result)
{
     ...
     */
    if(first->data.htbl_data.hash_table != second->data.htbl_data.hash_table)
    {
        *result = TRUE;
        SHR_EXIT();
    }
     ...
     */
    if(sal_memcmp(first_payload, second_payload, first->data.htbl_data.hash_table->key_size) != 0)
    {
        *result = TRUE;
        SHR_EXIT();
    }
    ...
    char *start_msg)
{
    ...
    DNXC_SWSTATE_JOURNAL_ALLOC(unit, dest, char *, string_size, "comparison journal str alloc", DNXC_ROLLBACK_JOURNAL_TYPE_COMPARISON, DNXC_ROLLBACK_JOURNAL_SUBTYPE_SWSTATE);
    ...
    char *stamp)
{
    ...
    if(len > DNXC_ROLLBACK_JOURNAL_MAX_STAMP_SIZE)
    {
        return DNXC_ROLLBACK_JOURNAL_MAX_STAMP_SIZE;
    }
    ...
    uint8 is_comparison)
{
     ...
     */
    char *comparison_perm_excluded[] = {
        "example[0]->rollback_comparison_excluded"
    };

    /*
     * list of permanently excluded stamps for rollback journal
     */
    char *rollback_perm_excluded[] = {
        "example[0]->rollback_comparison_excluded"
    };
    ...
    if(!is_comparison)
    {
         ...
         */
        nof_tables = sizeof(rollback_perm_excluded) / sizeof(char *);
    ...
    else
    {
         ...
         * check permenantly excluded table ids
         */
        nof_tables = sizeof(comparison_perm_excluded) / sizeof(char *);
    ...
    char *stamp)
{
    ...
    if (NULL == entry->data.memcpy_data.ptr || NULL == payload)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "sw state comparison journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
#endif
     ...
     */
    DNXC_SWSTATE_JOURNAL_ALLOC(unit, current_value_buffer, uint8 *, payload_size, "sw state comparison cur val buf", DNXC_ROLLBACK_JOURNAL_TYPE_COMPARISON, DNXC_ROLLBACK_JOURNAL_SUBTYPE_SWSTATE);
    ...
    char *stamp)
{
    ...
    hash_table = entry->data.htbl_data.hash_table;
    ...
    DNXC_SWSTATE_JOURNAL_ALLOC(unit, data_buffer, uint8 *, hash_table->data_size, "sw state comparison cur val buf", DNXC_ROLLBACK_JOURNAL_TYPE_COMPARISON, DNXC_ROLLBACK_JOURNAL_SUBTYPE_SWSTATE);
    ...
    uint8 *traversed_payload_raw)
{
     ...
     */
    if((entry->data.memcpy_data.ptr - (traversed->data.memcpy_data.ptr + traversed_payload_size) >= 0)
    || (traversed->data.memcpy_data.ptr - (entry->data.memcpy_data.ptr + entry_payload_size)) >= 0)
    {
        return _SHR_E_NONE;
    }
     ...
     */
    addr_diff = entry->data.memcpy_data.ptr - traversed->data.memcpy_data.ptr;
    ...
    uint8 *payload)
{
    ...
    LOG_CLI((BSL_META("Entry type: FREE\n")));
    ...
    uint8 *payload)
{
    ...
    LOG_CLI((BSL_META("Entry type: ALLOC\n")));
    ...
    uint8 *payload)
{
    ...
    LOG_CLI((BSL_META("Entry type: MEMCPY\n")));
    ...
    uint8 *payload)
{
    ...
    LOG_CLI((BSL_META("Entry type: HTBL\n")));
    ...
    uint8 *payload)
{
    ...
    LOG_CLI((BSL_META("Entry type: MUTEX\n")));
    ...
    uint8 is_cmp)
{
    ...
    if(is_cmp)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnxc_rollback_journal_entry_new(
                        unit,
    ...
    uint8 *traversed_payload_raw)
{
     ...
     */
    old_location = entry_free->data.free_data.location;
    new_location = *(entry_free->data.free_data.ptr_location);
    ...
    switch (traversed->entry_type)
    {
        case SW_JOURNAL_ENTRY_MEMCPY:
            DNXC_SW_STATE_JOURNAL_UPDATE_ENTRY_ADDRESS(traversed->data.memcpy_data.ptr, old_location, new_location, size);
            break;
        case SW_JOURNAL_ENTRY_ALLOC:
            DNXC_SW_STATE_JOURNAL_UPDATE_ENTRY_ADDRESS(traversed->data.alloc_data.location, old_location, new_location,
                                                  size);
            DNXC_SW_STATE_JOURNAL_UPDATE_ENTRY_PTR_TO_ADDRESS(traversed->data.alloc_data.ptr_location, old_location,
                                                         new_location, size);
            break;
        case SW_JOURNAL_ENTRY_FREE:
            DNXC_SW_STATE_JOURNAL_UPDATE_ENTRY_PTR_TO_ADDRESS(traversed->data.free_data.ptr_location, old_location,
                                                         new_location, size);
            break;
        case SW_JOURNAL_ENTRY_MUTEX_CREATE:
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY_PTR_TO_ADDRESS(traversed->data.mutex_create.ptr_mtx, old_location,
                                                               new_location, size);
            break;
        case SW_JOURNAL_ENTRY_MUTEX_DESTROY:
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY_PTR_TO_ADDRESS(traversed->data.mutex_destroy.ptr_mtx, old_location,
                                                               new_location, size);
            break;
 ...
 */
static shr_error_e
dnxc_sw_state_journal_roll_back_update_mutex_entry_locations(
    int unit,
    uint8 *entry_raw,
    uint8 *entry_payload_raw,
    uint8 *traversed_raw,
    uint8 *traversed_payload_raw)
{
    sal_mutex_t old_mutex;
    sal_mutex_t new_mutex;

    dnxc_sw_state_journal_entry_t *entry_mutex = (dnxc_sw_state_journal_entry_t *) (entry_raw);
    dnxc_sw_state_journal_entry_t *traversed = (dnxc_sw_state_journal_entry_t *) (traversed_raw);

    DNXC_SW_STATE_JRNL_DEFS;

    old_mutex = entry_mutex->data.mutex_destroy.mtx;
    new_mutex = *(entry_mutex->data.mutex_destroy.ptr_mtx);

    switch (traversed->entry_type)
    {
        case SW_JOURNAL_ENTRY_MUTEX_CREATE:
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY(traversed->data.mutex_create.mtx, old_mutex, new_mutex);
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY(*(traversed->data.mutex_create.ptr_mtx), old_mutex, new_mutex);
            break;
        case SW_JOURNAL_ENTRY_MUTEX_TAKE:
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY(traversed->data.mutex_take.mtx, old_mutex, new_mutex);
            break;
        case SW_JOURNAL_ENTRY_MUTEX_GIVE:
            DNXC_SW_STATE_JOURNAL_UPDATE_MUTEX_ENTRY(traversed->data.mutex_give.mtx, old_mutex, new_mutex);
            break;
        default:
            break;
    }
    ...
    char *stamp)
{
    ...
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: invalid entry data detected.\n%s%s%s", EMPTY,
                                 EMPTY, EMPTY);
    }
#endif
    ...
    DNX_SW_STATE_ALLOC_BASIC(unit,
                             entry->module_id,
                             entry->data.free_data.ptr_location,
                             entry->element_size,
                             entry->nof_elements,
    ...
    char *stamp)
{
    ...
    DNX_SW_STATE_FREE_BASIC(unit,
                            entry->module_id,
                            entry->data.alloc_data.ptr_location,
    ...
    char *stamp)
{
    ...
    DNX_SW_STATE_MEMCPY_BASIC(unit, entry->module_id, entry->data.memcpy_data.ptr, payload, payload_size,
    ...
    char *stamp)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_destroy
                    (unit, module_id, entry->data.mutex_create.ptr_mtx, DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    char *stamp)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_create
                    (unit, module_id, entry->data.mutex_destroy.ptr_mtx, "journal_mutex_roll_back",
                     DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    char *stamp)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_give
                    (unit, module_id, entry->data.mutex_take.mtx, DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    char *stamp)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_dispatcher[unit][module_id].mutex_take
                    (unit, module_id, entry->data.mutex_give.mtx, usec, DNXC_SW_STATE_JOURNAL_ROLLING_BACK));
    ...
    uint8 is_comparison)
{
    ...
    if(is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_destroy(unit, &DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_destroy(unit, &DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_comparison)
{
    ...
    if(is_comparison)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_distinct(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE, &dnxc_sw_state_journal_entries_are_distinct));
        SHR_IF_ERR_EXIT_NO_MSG(dnxc_rollback_journal_rollback(unit, FALSE, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_rollback(unit, only_head_rollback, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_comparison)
{
    ...
    if(is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_clear(unit, only_head_clear, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_clear(unit, only_head_clear, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
    ...
    uint8 is_cmp)
{
    ...
    entry.data.free_data.location = *ptr_location;
    entry.data.free_data.ptr_location = ptr_location;
    ...
    uint8 is_cmp)
{
    ...
    entry.module_id = module_id;
    entry.data.alloc_data.location = *ptr_location;
    entry.data.alloc_data.ptr_location = ptr_location;
    ...
    SW_STATE_HASH_TABLE_KEY * const key)
{
    ...
    if(NULL == hash_table)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: Invalid hash table specified for journal entry.\n%s%s%s", EMPTY, EMPTY, EMPTY);

    }

    if(NULL == key)
    {
        SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "sw state journal ERROR: Invalid hash table key specified for journal entry.\n%s%s%s", EMPTY, EMPTY, EMPTY);
    }
#endif
    ...
    DNXC_SWSTATE_JOURNAL_ALLOC(unit, tmp_buff, uint8 *, tmp_buff_size, "tmp_buff hash table swstate journal", DNXC_ROLLBACK_JOURNAL_TYPE_COMPARISON, DNXC_ROLLBACK_JOURNAL_SUBTYPE_SWSTATE);
    ...
    entry.module_id = module_id;
    entry.data.htbl_data.hash_table = hash_table;
    ...
    SW_STATE_HASH_TABLE_KEY * const key)
{
     ...
     */
    if(!dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE))
    {
        SHR_EXIT();
    }
    ...
    uint32 usec)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_take_internal(unit, module_id, mtx, usec, is_er, is_cmp));
    ...
    sal_mutex_t mtx)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_journal_mutex_give_internal(unit, module_id, mtx, is_er, is_cmp));
    ...
    uint32 *seq_id)
{
    ...
    if(is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_get_head_seq_id(unit, seq_id, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_get_head_seq_id(unit, seq_id, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
...
dnxc_sw_state_journal_entries_stamp_internal(
    ...
    va_list args)
{
    char str_to_print[DNXC_ROLLBACK_JOURNAL_MAX_STAMP_SIZE + 1] = { 0 };
    int size = 0;
    ...
    if(dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE))
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_entries_stamp(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE, str_to_print));
    }

    if(dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE))
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_entries_stamp(unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE, str_to_print));
    }
...
dnxc_sw_state_journal_should_stamp(
    int unit)
{
#ifdef DNXC_ROLLBACK_JOURNAL_DEBUG
    return dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE)
        || dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE);
#else
    return dnxc_rollback_journal_is_on(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE);
#endif
}
    ...
    uint8 is_disable)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_rollback_journal_disabled_counter_change(unit, is_disable, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    ...
    uint8 is_comparison)
{
    ...
    if(!is_comparison)
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_print(unit, DNXC_SW_STATE_ROLLBACK_JOURNAL_HANDLE));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnxc_rollback_journal_print(unit, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
