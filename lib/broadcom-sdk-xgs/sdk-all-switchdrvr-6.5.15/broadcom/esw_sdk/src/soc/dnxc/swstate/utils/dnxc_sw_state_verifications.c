    ...
    uint32 flags)
{
    ...
    if (!dnxc_ha_is_access_disabled(unit,DNXC_HA_ALLOW_SW_STATE)) {
    ...
    void* module_ptr)
{
    ...
    if (!sw_state_roots_array[unit]) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 "Trying to access SW State module although sw state was not initialized for the unit (module_id=%d)\n%s%s",
                                 module_id, EMPTY, EMPTY);
    }

    if (!module_ptr) {
        SHR_IF_ERR_EXIT_WITH_LOG(SOC_E_INTERNAL,
                                 "Trying to access SW State module that was'nt initialized (module_id=%d)\n%s%s",
                                 module_id, EMPTY, EMPTY);
    }
