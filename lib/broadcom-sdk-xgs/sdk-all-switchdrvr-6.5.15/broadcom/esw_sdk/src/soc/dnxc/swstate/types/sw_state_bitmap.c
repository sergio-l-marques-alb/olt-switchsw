    ...
    char *dbg_string)
{
    ...
    tmp_storage_nof_bits = source_first + range;
    tmp_storage_size = (_SHR_BITDCLSIZE(tmp_storage_nof_bits) + 1) * sizeof(SHR_BITDCL);

    tmp_storage = (SHR_BITDCL *)sal_alloc(tmp_storage_size, "bitmap range read");
    ...
    DNX_SW_STATE_MEMREAD(unit, tmp_storage, location, 0, tmp_storage_size, flags, dbg_string);

    /* copy bit range from temporary source location to result location */
    SHR_BITCOPY_RANGE(result_location, result_first, tmp_storage, source_first, range);
