...
#include <src/bcm/dnx/algo/res_mngr/res_mngr_internal.h>
    ...
    sw_state_algo_res_t algo_res)
{
    ...
    DNXC_MTA(SHR_IF_ERR_EXIT(dnxc_multithread_analyzer_log_resource_use(unit,
                             ...
                             dnxc_multithread_analyzer_dynamic_sub_res_get(unit, MTA_DYN_SUB_RES_RES_MGR, algo_res->name),
                             TRUE)));
...
/*
 * Verify input for sw_state_algo_res_create.
 */
static shr_error_e
sw_state_algo_res_create_verify(
    int unit,
    sw_state_algo_res_create_data_t * data)
{
    uint8 use_advanced_algorithm;
    int max_name_length = SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH;
    ...
    if (_SHR_IS_FLAG_SET(data->flags, SW_STATE_ALGO_RES_CREATE_INDEXED_RESOURCE))
    {
        if (data->nof_resource_pool_indexes < 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Number of resouce pool indexes must be at least 1. Got: %d",
                         data->nof_resource_pool_indexes);
        }
    }
     ...
     */
    use_advanced_algorithm = _SHR_IS_FLAG_SET(data->flags, SW_STATE_ALGO_RES_CREATE_USE_ADVANCED_ALGORITHM);
    ...
    else if (!use_advanced_algorithm)
    {
        if (data->advanced_algorithm != DNX_ALGO_RESOURCE_ADVANCED_ALGORITHM_BASIC)
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "If the SW_STATE_ALGO_RES_CREATE_USE_ADVANCED_ALGORITHM flag is not set, the advanced algorithm should be %d, but it is %d",
 ...
 */
static shr_error_e
sw_state_algo_res_create_register_data(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    sw_state_algo_res_create_data_t * data)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_verify(unit,algo_res));
    ...
    DNX_SW_STATE_MEMCPY(unit, module_id, algo_res->nof_resource_pool_indexes, &(data->nof_resource_pool_indexes),
                        algo_res->nof_resource_pool_indexes, 0, "Resource nof_resource_pool_indexes.");
    ...
    sw_state_string_strncpy
            (unit, module_id, algo_res->name, sizeof(char)*(SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH-1), data->name);
    ...
    uint32 alloc_flags)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_create(unit:%d, data->name:%s, data->flags:%d, data->first_element:%d, data->nof_elements:%d, data->nof_resource_pool_indexes:%d, data->advanced_algorithm:%d, extra_arguments:%p)\n"),
                     unit, data->name, data->flags, data->first_element, data->nof_elements,
                     data->nof_resource_pool_indexes, data->advanced_algorithm, extra_arguments));
    }

    /*
     * 0. Verify input.
     */
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_create_verify(unit, data));

    /*
     * Set number of indexed pools to 1 and the sub resource index to 0, if we are not using indexed resource.
     */
    if (!(_SHR_IS_FLAG_SET(data->flags, SW_STATE_ALGO_RES_CREATE_INDEXED_RESOURCE)))
    {
        data->nof_resource_pool_indexes = 1;
        data->sub_resource_index = 0;
    }

    /*
     * Set core id to 0, if we are not using dublicated per core resource.
     */
    if (!(_SHR_IS_FLAG_SET(data->flags, SW_STATE_ALGO_RES_CREATE_DUPLICATE_PER_CORE)))
    {
        data->core_id = 0;
    }
    ...
    DNX_SW_STATE_ALLOC(unit, module_id, *algo_res, **algo_res, 1, 0, "*algo_res")
        ...
        SHR_IF_ERR_EXIT(Resource_callbacks_map[data->advanced_algorithm].create_cb
                        (unit, module_id, &((*algo_res)->res_tag_bitmap), data->core_id, data->sub_resource_index, data,
                         extra_arguments, nof_elements, alloc_flags));

    /*
     * 1.2 Register the resource data.
     */
    SHR_IF_ERR_EXIT(sw_state_algo_res_create_register_data(unit, module_id, (*algo_res), data));

exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_create(name:%s) Result:%d\n"), data->name, SHR_GET_CURRENT_ERR()));
    }
    SHR_FUNC_EXIT;
}
 ...
 */
static shr_error_e
sw_state_algo_res_destroy_verify(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res)
{
    ...
    void *extra_arguments)
{
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_destroy(unit:%d, name:%s, extra_arguemnts:%p)\n"), unit,
    ...
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_destroy_verify(unit, module_id, (*algo_res)));
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_destroy(name:%s) Result:%d\n"), (*algo_res)->name,
                     SHR_GET_CURRENT_ERR()));
    }
 ...
 */
static shr_error_e
sw_state_algo_res_element_verify(
    int unit,
    sw_state_algo_res_t algo_res,
    int element)
{
 ...
 */
static shr_error_e
sw_state_algo_res_allocate_verify(
    int unit,
    sw_state_algo_res_t algo_res,
    uint32 flags,
    int *element,
    int nof_elements)
{
    ...
    {
        SHR_IF_ERR_EXIT(sw_state_algo_res_element_verify(unit, algo_res, *element));
    }
...
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_INIT_TIME_ANALYZER_START(unit, DNX_INIT_TIME_ANALYZER_RES_MNGR);

    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_allocate(unit:%d, name:%s, extra_arguments:%p"), unit, algo_res->name,
                     extra_arguments));
        if (_SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID))
        {
    ...
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_allocate_verify(unit, algo_res, flags, element, 1));

    SHR_SET_CURRENT_ERR(sw_state_algo_res_allocate_several
                        (unit, module_id, algo_res, flags | SW_STATE_ALGO_RES_ALLOCATE_USING_SINGLE_ALLOCATE_CB, 1,
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META("\n sw_state_algo_res_allocate(name:%s"), algo_res->name));
    ...
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_RES_MNGR);
    SHR_FUNC_EXIT;
}
 ...
 */
static shr_error_e
sw_state_algo_res_get_delete_verify(
    int unit,
    sw_state_algo_res_t algo_res,
    uint8 nof_elements,
    int element)
    ...
    uint8 *is_allocated)
{
    ...
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_get_delete_verify(unit, algo_res, 1, element));
 ...
 */
shr_error_e
sw_state_algo_res_is_allocated(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    int element,
    uint8 *is_allocated)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_is_allocated(unit:%d, name:%s, element:%d)\n"), unit, algo_res->name,
                     element));
    }

    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_get_verify(unit, algo_res, element, is_allocated));
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META("\n sw_state_algo_res_is_allocated(name:%s"), algo_res->name));
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
    ...
    int element)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_free(unit:%d, name:%s, element:%d)\n"), unit, algo_res->name, element));
    }

    SHR_IF_ERR_EXIT(sw_state_algo_res_element_verify(unit, algo_res, element));

    SHR_IF_ERR_EXIT(sw_state_algo_res_free_several(unit, module_id, algo_res, 1, element));

exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_free(name:%s,) Result:%d\n"), algo_res->name,
                     SHR_GET_CURRENT_ERR()));
    }
    DNX_INIT_TIME_ANALYZER_STOP(unit, DNX_INIT_TIME_ANALYZER_RES_MNGR);
    SHR_FUNC_EXIT;
...
/*
 * Verification function for sw_state_algo_res_nof_free_elements_get.
 * Check taht pointer is not NULL and that resource is valid.
 */
static shr_error_e
sw_state_algo_res_nof_free_elements_get_verify(
    int unit,
    sw_state_algo_res_t algo_res,
...
shr_error_e
sw_state_algo_res_nof_free_elements_get(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    int *nof_free_elements)
{
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_nof_free_elements_get(unit:%d, name:%s)\n"), unit, algo_res->name));
    }

    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_nof_free_elements_get_verify(unit, algo_res, nof_free_elements));
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_nof_free_elements_get(name:%s"), algo_res->name));
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
    ...
    sw_state_algo_res_t algo_res)
{
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_clear(unit:%d, name:%s)\n"), unit, algo_res->name));
    }
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(sw_state_algo_res_is_allocated(unit, module_id, algo_res, current_element, &is_allocated));
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_clear(name:%s) Result:%d\n"), algo_res->name,
                     SHR_GET_CURRENT_ERR()));
    }
...
shr_error_e
sw_state_algo_res_clear_all(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res)
{
    int current_core, current_pool_index, nof_cores_to_use;
    ...
    nof_cores_to_use = (_SHR_IS_FLAG_SET(algo_res->flags, SW_STATE_ALGO_RES_CREATE_DUPLICATE_PER_CORE))
        ? SW_STATE_ALGO_RES_NOF_CORES_GET(unit) : 1;

    for (current_core = 0; current_core < nof_cores_to_use; current_core++)
    {
        for (current_pool_index = 0; current_pool_index < algo_res->nof_resource_pool_indexes; current_pool_index++)
        {
            SHR_IF_ERR_EXIT(sw_state_algo_res_clear
                            (unit, module_id, (algo_res + ((current_core * current_pool_index) + current_pool_index))));
        }
    }
...
static shr_error_e
sw_state_algo_res_get_next_verify(
    int unit,
    sw_state_algo_res_t algo_res,
    int *element)
{
    ...
    int *element)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_get_next(unit:%d, name:%s, element:%d)\n"), unit, algo_res->name,
                     *element));
    }

    SHR_IF_ERR_EXIT(sw_state_algo_res_get_next_verify(unit, algo_res, element));
    ...
    else
    {
        ...
        {
            *element = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
        }
    }
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META("\n sw_state_algo_res_get_next(name:%s"), algo_res->name));
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR, (BSL_META(", element:%d"), *element));
        }
    ...
    int *element)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_allocate_several(unit:%d, name:%s, flags:%d, nof_elements:%d, extra_arguments:%p"),
    ...
    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_allocate_verify(unit, algo_res, flags, element, nof_elements));

    if (_SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_USING_SINGLE_ALLOCATE_CB)
    ...
    if ((rv == _SHR_E_RESOURCE) && (_SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID)))
    {
        /*
    ...
    if ((_SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_SIMULATION))
        && ((rv == _SHR_E_RESOURCE)
            || ((rv == _SHR_E_EXISTS) && (_SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID)))))
    {
...
exit:
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META("\n sw_state_algo_res_allocate_several(name:%s"), algo_res->name));
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
            LOG_VERBOSE(BSL_LS_BCMDNX_TEMPLATEMNGR, (BSL_META(", element:%d"), *element));
        }
    ...
    int element)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_free_several(unit:%d, name:%s, nof_elements:%d, element:%d)\n"),
                     unit, algo_res->name, nof_elements, element));
    }

    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_get_delete_verify(unit, algo_res, nof_elements, element));
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_free_several(name:%s) Result:%d\n"), algo_res->name,
                     SHR_GET_CURRENT_ERR()));
    }
...
shr_error_e
sw_state_algo_res_dump_info_get(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    sw_state_algo_res_dump_data_t * info)
{
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_dump(unit:%d, name:%s)\n"), unit, algo_res->name));
    }
    ...
    info->create_data.nof_resource_pool_indexes = algo_res->nof_resource_pool_indexes;
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_dump(name:%s) Result:%d\n"), algo_res->name,
                     SHR_GET_CURRENT_ERR()));
    }
...
int
sw_state_algo_res_print(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t * algo_res)
{
    sw_state_algo_res_dump_data_t data;
    ...
    SHR_IF_ERR_EXIT(sw_state_algo_res_dump_info_get(unit, module_id, (*algo_res), &data));

    DNX_SW_STATE_PRINT(unit, "\nResource :%s", (*algo_res)->name);
    DNX_SW_STATE_PRINT(unit, " (Advanced Algorithm - %s)", (data.create_data.advanced_algorithm==DNX_ALGO_RESOURCE_ADVANCED_ALGORITHM_BASIC ? "No" : "Yes"));
    ...
    if (data.nof_used_elements)
    {
        ...
        while (1)
        {
            SHR_IF_ERR_EXIT(sw_state_algo_res_get_next(unit, module_id, (*algo_res), &entry_id));

            if ((entry_id == SW_STATE_ALGO_RES_ILLEGAL_ELEMENT) || (entry_id > end))
            {
                break;
            }
...
/*
 * Verification function for sw_state_algo_res_nof_elements_in_range.
 * Check that pointer is not NULL and that resource is valid.
 */
static shr_error_e
sw_state_algo_res_nof_allocated_elements_in_range_get_verify(
    int unit,
    sw_state_algo_res_t algo_res,
    int range_start,
...
shr_error_e
sw_state_algo_res_nof_allocated_elements_in_range_get(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    int range_start,
    ...
    int *nof_allocated_elements)
{
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_nof_elements_in_range(unit:%d, name:%s)\n"), unit, algo_res->name));
    }

    SHR_INVOKE_VERIFY_DNX(sw_state_algo_res_nof_allocated_elements_in_range_get_verify
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_nof_elements_in_range(name:%s"), algo_res->name));
        if (SHR_GET_CURRENT_ERR() != _SHR_E_NONE)
        {
...
shr_error_e
sw_state_algo_res_advanced_algorithm_info_set(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    void *algorithm_info)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_advanced_algorithm_info_set(unit:%d, name:%s)\n"), unit, algo_res->name));
    }
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_advanced_algorithm_info_set(name:%s"), algo_res->name));
...
shr_error_e
sw_state_algo_res_advanced_algorithm_info_get(
    int unit,
    uint32 module_id,
    sw_state_algo_res_t algo_res,
    void *algorithm_info)
{
    ...
    if (LOG_CHECK(BSL_VERBOSE | BSL_LS_BCMDNX_RESMNGR))
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META
                     ("\n sw_state_algo_res_advanced_algorithm_info_get(unit:%d, name:%s)\n"), unit, algo_res->name));
    }
...
exit:
    ...
    {
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR,
                    (BSL_META("\n sw_state_algo_res_advanced_algorithm_info_get(name:%s"), algo_res->name));
        LOG_VERBOSE(BSL_LS_BCMDNX_RESMNGR, (BSL_META(") Result:%d\n"), SHR_GET_CURRENT_ERR()));
    }
    SHR_FUNC_EXIT;
}
 ...
 */
shr_error_e
sw_state_algo_res_dummy_empty_function_to_make_compiler_happy(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_FUNC_EXIT;
