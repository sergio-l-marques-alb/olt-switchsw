    ...
    int key2)
{
    ...
    for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
    {
        if (!sal_strncasecmp(submodules[submodule_index].name, submodule, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            /*
             * look for define with that name in table if member is not empty
             */
            if (!ISEMPTY(member))
            {
                res =
    ...
    char *dnx_data_str)
{
    ...
    /*
     * Parse the string, There must be at least 3 tokens, if there 4 it means we have table
    ...
    if (realtokens == 4)
    {
        ...
        /*
         * There are 3 acceptable formats member(key1,key2) or member(key1) or member()
         * Below loop deals in figuring out which format was provided and calculating the keys
         */
        for (i_ch = 0; i_ch < size; i_ch++)
        {
            ...
            {
                /*
                 * No one of interested us characters found, just continue looking for
                 */
                continue;
            }
        }
    ...
    int key2)
{
    ...
    for (table_index = 0; table_index < nof_tables; table_index++)
    {
        if (!sal_strncasecmp(tables[table_index].name, table_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            {
                if (!sal_strncasecmp
                    (tables[table_index].values[value_index].name, member_name, DNXC_DATA_UTILS_STRING_MAX))
                {
                    /*
                     * If value found - return pointer to data
                     */
