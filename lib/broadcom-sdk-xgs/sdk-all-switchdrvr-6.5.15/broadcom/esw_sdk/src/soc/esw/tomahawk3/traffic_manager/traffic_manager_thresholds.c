/*
 * $Id: tm.c,v 1.1.2.4 Broadcom SDK $
                              ...
                              _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    soc_mem_t mem;
    uint32 entry[SOC_MAX_MEM_WORDS];
    soc_mem_t mmu_thdo_config_uc_qgroup[] = {
    ...
    for (loop = 0; loop <  3; loop++) {
        mem = mmu_thdo_config_uc_qgroup[loop];
        if (mem != INVALIDm) {
            PBMP_ALL_ITER(unit, port) {
        ...
        int lossless)
{
    ...
    int mmu_thdo_per_queue_mem_count = 6;
    ...
    /* Per port per pool */
    soc_mem_t mmu_thdo_config_port_uc_mem[] = {
        MMU_THDO_CONFIG_PORT_UC0m,
        MMU_THDO_CONFIG_PORT_UC1m,
        MMU_THDO_CONFIG_PORT_UC2m
    };
    int mmu_thdo_config_port_uc_mem_count = 3;
    ...
    int mmu_thdo_resume_port_mem_count = 6;


    si = &SOC_INFO(unit);

    for (loop = 0; loop < mmu_thdo_config_port_uc_mem_count; loop++) {
        mem = mmu_thdo_config_port_uc_mem[loop];
        if (mem != INVALIDm) {
            midx = SOC_TH3_MMU_PIPED_MEM_INDEX(unit, port,
                 mmu_thdo_config_port_uc_mem[loop], 0);
            ...
            for (idx = 0; idx < _TH3_MMU_NUM_POOL; idx++) {
                switch (idx) {
                    case 0:
                        ...
                        soc_mem_field32_set(unit, mem, entry,
                            SP0_RED_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        soc_mem_field32_set(unit, mem, entry,
                            SP0_YELLOW_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        break;
                    case 1:
                        soc_mem_field32_set(unit, mem, entry,
                        ...
                        soc_mem_field32_set(unit, mem, entry,
                            SP1_RED_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        soc_mem_field32_set(unit, mem, entry,
                            SP1_YELLOW_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        break;
                    case 2:
                        soc_mem_field32_set(unit, mem, entry,
                        ...
                        soc_mem_field32_set(unit, mem, entry,
                            SP2_RED_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        soc_mem_field32_set(unit, mem, entry,
                            SP2_YELLOW_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        break;
                    case 3:
                        soc_mem_field32_set(unit, mem, entry,
                        ...
                        soc_mem_field32_set(unit, mem, entry,
                            SP3_RED_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        soc_mem_field32_set(unit, mem, entry,
                            SP3_YELLOW_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                        break;
                }
            }
            SOC_IF_ERROR_RETURN
    ...
    if (mem != INVALIDm) {
        ...
        for (idx = 0; idx < _TH3_MMU_NUM_POOL; idx++) {
            ...
            soc_mem_field32_set(unit, mem, entry,
                    SP_RED_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
            soc_mem_field32_set(unit, mem, entry,
                    SP_YELLOW_LIMITf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
            SOC_IF_ERROR_RETURN
                    (soc_mem_write(unit, mem, MEM_BLOCK_ALL,
                                   midx, entry));
        }
    ...
    for (loop = 0; loop < mmu_thdo_resume_port_mem_count; loop++) {
        ...
        if (mem != INVALIDm) {
            for (idx = 0; idx < _TH3_MMU_NUM_POOL; idx++) {
                ...
                soc_mem_field32_set(unit, mem, entry,
                        SHARED_RESUMEf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                soc_mem_field32_set(unit, mem, entry,
                        RED_RESUMEf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                soc_mem_field32_set(unit, mem, entry,
                        YELLOW_RESUMEf,  CEIL(_TH3_MMU_TOTAL_CELLS_PER_ITM, 8));
                SOC_IF_ERROR_RETURN
                        (soc_mem_write(unit, mem, MEM_BLOCK_ALL,
                                       midx, entry));
            }
...
soc_th3_get_port_pg_headroom (int unit, int port_speed,
        ...
        int *pg_hdrm) {
    unsigned int mtu = 0, cable_rtt = 0, pfc_tx_dly = 0, pfc_resp_time = 0,
        max_tx_rx_dly = 0;
    ...
    cable_rtt = CEIL((1250 * (port_speed / 10000) * (cable_length / 100)), 1);
    /* PFC mesage tx delay */
    ...
    *pg_hdrm = CEIL((tot_inflight / _TH3_MMU_BYTES_PER_CELL), 1);
    ...
    LOG_VERBOSE(BSL_LS_SOC_MMU,
          (BSL_META_U(unit,
          "mtu: %u, cable_rtt: %u, pfc_tx_dly : %u, pfc_resp_time : %u,"
