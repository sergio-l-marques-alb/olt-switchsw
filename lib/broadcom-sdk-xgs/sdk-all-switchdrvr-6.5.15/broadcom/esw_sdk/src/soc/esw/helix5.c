...
#define HX5_PORT_BLOCK_BASE_PORT(port)                              \
...
_soc_hx5_port_flex_init(int unit, int is_any_cap)
{
    ...
    if (SOC_PBMP_NOT_NULL(SOC_PORT_DISABLED_BITMAP(unit, all)) || is_any_cap) {
        ...
        for (phy_port = 1; phy_port <= HELIX5_GPHY_PORTS_PER_PIPE; phy_port++) {
            ...
            phy_base = HX5_PORT_BLOCK_BASE_PORT(port);
            /* port_flex_enable_corex=<0...1> */
...
soc_helix5_uft_uat_config(int unit)
{
    ...
    if (mpls_4k_banks) {
        offset_base = 64 % (mpls_4k_banks);
        offset_multiplier = 64 / (mpls_4k_banks);
    }
    ...
    SOC_IF_ERROR_RETURN
        (READ_VLAN_XLATE_1_HASH_CONTROLm(unit, MEM_BLOCK_ANY, 0,
                                                hash_control_entry));
    if (vlan_xlate_1_4k_banks) {
        offset_base = 64 % (vlan_xlate_1_4k_banks);
        offset_multiplier = 64 / (vlan_xlate_1_4k_banks);
    }
    ...
    for(index = 0; index < vlan_xlate_1_4k_banks; index++) {
        ...
        soc_mem_field32_set(unit, IP_UAT_SHARED_BANKS_CONTROLm,
                            shared_4k_hash_control_entry, shared_fields[index +
                            mpls_4k_banks], offset_base + (index *
                            offset_multiplier));
    }
    ...
    SOC_IF_ERROR_RETURN
        (READ_VLAN_XLATE_2_HASH_CONTROLm(unit, MEM_BLOCK_ANY, 0,
                                                hash_control_entry));
    if (vlan_xlate_2_4k_banks) {
        offset_base = 64 % (vlan_xlate_2_4k_banks);
        offset_multiplier = 64 / (vlan_xlate_2_4k_banks);
    }
    ...
    for(index = 0; index < vlan_xlate_2_4k_banks; index++) {
        ...
        soc_mem_field32_set(unit, IP_UAT_SHARED_BANKS_CONTROLm,
                            shared_4k_hash_control_entry, shared_fields[index +
                            mpls_4k_banks + vlan_xlate_1_4k_banks], offset_base
                            + (index * offset_multiplier));
    }
...
_soc_helix5_port_mapping_init(int unit)
{
    ...
    mem = SOC_MEM_UNIQUE_ACC(unit, ING_PHY_TO_IDB_PORT_MAPm)[pipe];
    for (phy_port = 1; phy_port <= _HX5_PORTS_PER_PIPE; phy_port++) {
        port = si->port_p2l_mapping[phy_port];
                               ...
                               int *obm_id, int *lane)
{
    ...
    port_block_base = HX5_PORT_BLOCK_BASE_PORT(port);
    if (lane != NULL) {
...
#define _SOC_HX5_DEF_TEMP_MAX   90
    int index;
...
_soc_hx5_ledup_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx) && !(SAL_BOOT_PLISIM || SAL_BOOT_BCMSIM)) {
        /* On Helix5 (iProc 16), M0 is needed to be rest as re-running rc.soc */
        rval = 1;
        SOC_IF_ERROR_RETURN(WRITE_U0_M0SS_CONTROLr(unit, rval));
        sal_udelay(10);
...
_soc_helix5_misc_init(int unit)
{
    ...
    if (parity_enable) {
        if (!SOC_WARM_BOOT(unit)) {
            ...
            /*SOC_IF_ERROR_RETURN(_soc_helix5_clear_all_port_data(unit));*/
        }
 ...
 *      NONE
 */
void soc_hx5_subport_init(int unit)
{
    soc_td3_subport_init(unit);
}

soc_pstats_tbl_desc_t
_soc_hx5_pstats_tbl_desc_all = {SOC_BLK_NONE, INVALIDm, FALSE, FALSE};
/* pstats support */
soc_pstats_tbl_desc_t _soc_hx5_pstats_tbl_desc[] = {
...
void
soc_helix5_mmu_pstats_tbl_config_all(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    soc_pstats_mem_desc_t *desc, *desc_all;
    int ti, mi, di = 0;

    desc_all = _soc_hx5_pstats_tbl_desc_all.desc;
    ...
    soc->pstats_tbl_desc = &_soc_hx5_pstats_tbl_desc_all;
    soc->pstats_tbl_desc_count = 1;
}

void
soc_helix5_mmu_pstats_tbl_config(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    ...
    soc->pstats_tbl_desc = _soc_hx5_pstats_tbl_desc;
    soc->pstats_tbl_desc_count = COUNTOF(_soc_hx5_pstats_tbl_desc);
    ...
    if (soc_property_get(unit, "pstats_desc_all", 1)) {
        ...
        soc_helix5_mmu_pstats_tbl_config_all(unit);
    }
}

int
soc_helix5_mmu_pstats_mode_set(int unit, uint32 flags)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    int pf = 0;
    uint32 rval = 0;
    ...
    if (flags & _HX5_MMU_PSTATS_HWM_MOD) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                          HWM_MODEf, 1);
        pf |= _HX5_MMU_PSTATS_HWM_MOD;
        if (flags & _HX5_MMU_PSTATS_RESET_ON_READ) {
            soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                              HWM_RESET_ON_READf, 1);
            pf |= _HX5_MMU_PSTATS_RESET_ON_READ;
        }
    }

    if (soc_feature(unit, soc_feature_mor_dma) &&
            (flags & _HX5_MMU_PSTATS_MOR_EN)) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                MOR_ENf, 1);
    }
    if (flags & _HX5_MMU_PSTATS_MOR_EN) {
        pf |= _HX5_MMU_PSTATS_MOR_EN;
    }

    if (flags & _HX5_MMU_PSTATS_ENABLE) {
        soc_reg_field_set(unit, MMU_GCFG_PKTSTAT_OOBSTATr, &rval,
                ENABLEf, 1);
        pf |= _HX5_MMU_PSTATS_ENABLE;
        if ((flags & _HX5_MMU_PSTATS_SYNC) ||
                (flags & _HX5_MMU_PSTATS_HWM_MOD &&
                 flags & _HX5_MMU_PSTATS_RESET_ON_READ)) {
            soc->pstats_mode = PSTATS_MODE_PKT_SYNC;
            pf |= _HX5_MMU_PSTATS_SYNC;
        } else {
...
int
soc_helix5_mmu_pstats_mode_get(int unit, uint32 *flags)
{
    int pf = 0;
    uint32 rval = 0;
    int rv;
...
int
soc_helix5_mmu_pstats_mor_enable(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    uint32 rval;


    if (soc->pstats_flags & _HX5_MMU_PSTATS_MOR_EN) {
...
int
soc_helix5_mmu_pstats_mor_disable(int unit)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    uint32 rval;

    if (!soc_feature(unit, soc_feature_mor_dma) &&
        (soc->pstats_flags & _HX5_MMU_PSTATS_MOR_EN)) {
        SOC_IF_ERROR_RETURN(READ_MMU_GCFG_PKTSTAT_OOBSTATr(unit, &rval));
...
soc_helix5_port_qsgmii_mode_get(int unit, soc_port_t port, int* qsgmii_mode)
{
    ...
    phy_port = si->port_l2p_mapping[port];
    base_port = HX5_PORT_BLOCK_BASE_PORT(port);
...
void
soc_helix5_port_num_adjust(int unit, soc_block_type_t blktype, uint32* phy_port)
{
    if (blktype != SOC_BLK_XLPORT) {
        return;
    }

    *phy_port = ((*phy_port > 4) && (*phy_port < 17)) ? 1 :
                ((*phy_port > 20) && (*phy_port < 33)) ? 17 :
                ((*phy_port > 36) && (*phy_port < 49)) ? 33 : *phy_port;
    return;
}
                              ...
                              void *d3, void *d4)
{
    ...
    for (i = 0; i < 7; i++) {
        if (i & _soc_hx5_temp_mon_mask[unit]) {
...
soc_helix5_icid_get(int unit, int max_size, uint8 *buffer, int *actual_size)
{
    ...
    if (SOC_BOND_INFO(unit)->ic_id == NULL) {
