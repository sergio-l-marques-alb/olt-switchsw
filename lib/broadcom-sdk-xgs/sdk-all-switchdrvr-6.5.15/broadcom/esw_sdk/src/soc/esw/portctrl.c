    ...
    int speedi, portmod_encap_t int_mode)
{
    ...
    if(int_mode == _SHR_PORT_ENCAP_RSVD4) {
        ...
        switch(speedi) {
        case 1228:
            speedo = cprimodSupportedSpd1228p8;
            break;
        case 2457:
            speedo = cprimodSupportedSpd2457p6;
            break;
        case 3072:
            speedo = cprimodSupportedSpd3072p0;
            break;
        case 4915:
            speedo = cprimodSupportedSpd4915p2;
            break;
        case 6144:
            speedo = cprimodSupportedSpd6144p0;
            break;
        case 9830:
            speedo = cprimodSupportedSpd9830p4;
            break;
        case 10137:
            speedo = cprimodSupportedSpd10137p6;
            break;
        case 12165:
            speedo = cprimodSupportedSpd12165p12;
            break;
        case 24330:
            speedo = cprimodSupportedSpd24330p24;
            break;
        default:
            speedo = -1;
            break;
...
#if (defined BCM_HELIX5_SUPPORT) || (defined BCM_MAVERICK2_SUPPORT)
...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag, void *port_add_info)
{
    ...
    if (pm_type == portmodDispatchTypePm8x50) {
        ...
        if (!PORTMOD_CORE_INIT_FLAG_INITIALZIED_GET(is_init)) {
             ...
             */
            rv = portmod_core_add(unit, portmod_pm_id, add_info);
            if (rv != SOC_E_UNAVAIL && rv < 0) {
                if (local_add_info != NULL) {
                    sal_free(local_add_info);
                }
...
soc_esw_portctrl_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    is_macsec_enabled = soc_property_port_get(unit, pport, spn_MACSEC_ENABLE, 0);

    if (is_legacy_phy_present && is_macsec_enabled) {
