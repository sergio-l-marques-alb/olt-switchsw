                           ...
                           ap_pgw_os_tdm_sel_e *ap_pgw_os_tdm_sel, int pgw_num)
{
  ...
  for(i=1; i<= AP_NUM_PHY_PORTS; i=i+4) {
	...
	/*if (*ap_pgw_os_tdm_sel == AP_PGW_OS_MV_880G) {
            for (j=0, token=0; j<40;j++) {
	       if (ovs_tdm_tbl[j] == i) { 
	          token++;
	          if (token%2) ovs_tdm_tbl[j]=i;
	          else ovs_tdm_tbl[j]=AP_OVSB_TOKEN;
	       }
            }
	  } else {*/
            for (j=0, token=0; j<40;j++) {
	        if ((ovs_tdm_tbl[j] == i) || (ovs_tdm_tbl[j] == i+1) || (ovs_tdm_tbl[j] == i+2) || (ovs_tdm_tbl[j] == i+3)) {/*config-8 support - to handle template with 12 slots for 100G port */ 
	          token++;
              if (*ap_pgw_os_tdm_sel == AP_PGW_OS_FB_568_3) ovs_tdm_tbl[j] = i;
	...
	break;
      ...
      break;
      case SPEED_25G:
	if (i == 29 || i == 33 || i== 65 || i == 66 || i == 67 || i == 68 || i==69) { /* Only for Falcon ports enter this processing */
           for (j=0, token=0; j<40;j++) {
	      if (ovs_tdm_tbl[j] == i) { 
	         token++;
	         switch (token) {
