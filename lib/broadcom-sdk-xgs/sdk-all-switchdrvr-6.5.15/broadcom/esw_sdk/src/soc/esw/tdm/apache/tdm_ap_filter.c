...
if (_tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && 
 ...
 /* check only 1G/2.5G is in LR in FB5 */
            if(tdm_port_speed[tdm_pipe_main[i]] == SPEED_1G || tdm_port_speed[tdm_pipe_main[i]] == SPEED_2p5G) {
                 one_slot_cnt++; 
            } else if (tdm_port_speed[tdm_pipe_main[i]] > SPEED_10G) { /* bk: FB5 config can have 10G speed for Mgig */ 
                 /*lr_1g_port = 0;*/
                 break;
            }
        }
    ...
    if(lr_1g_port == 1  && one_slot_cnt > 0){ 
        for (i=0; i<one_slot_cnt; i++) { /* bk: Share OS slots between end and Top without removing any ANC tokens. */
            preserve_last_slot = tdm_pipe_main[199]; /* right shift register with last entry fed to first element. */ 
            for (k=199; k>i; k--) { /* bk: length is 200 ie. 0..199 */
                tdm_pipe_main[k]=tdm_pipe_main[k-1];
            }
            tdm_pipe_main[i]=preserve_last_slot;
        }
    }  
#endif 
