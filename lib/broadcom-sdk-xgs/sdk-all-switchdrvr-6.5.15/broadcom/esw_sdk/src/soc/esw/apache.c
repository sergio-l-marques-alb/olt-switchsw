...
#if defined(BCM_56560_A0) || defined(BCM_56560_B0)
                    if ( SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 6) + 1]) &&
                            !IS_OVERSUB_PORT(unit, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 6) + 1]) &&
                            SOC_PBMP_MEMBER(pbmp, SOC_INFO(unit).port_p2l_mapping[(APACHE_PORTS_PER_TSC * 16) + 1]) &&
                        ...
                        /* TSC6, TSC16 or TSC 17 is in line rate */
                        *core_clk_freq = 797;
