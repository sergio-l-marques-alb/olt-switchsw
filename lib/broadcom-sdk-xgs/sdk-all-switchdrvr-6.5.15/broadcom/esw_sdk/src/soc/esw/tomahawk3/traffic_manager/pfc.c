    ...
    int num_opt_group)
{
    ...
    for (cos_num = TH3_PFC_COS_NUM - 1; cos_num >= 0; cos_num--) {
        int eb_sp = SOC_TH3_PFC_COS_SP(unit, profile_id, cos_num)? 1 : 0;
        if (eb_config->opt_group[cos_num] !=0 &&
            ...
            for (i = cos_num -1; i >= 0; i--) {
                if (eb_config->opt_group[i]
                    ...
                    */
                    eb_sp |= SOC_TH3_PFC_COS_SP(unit, profile_id, i);
                }
            }
    ...
    _soc_th3_pfc_eb_group_config_t *eb_config)
{
    ...
    for (q_num = 0; q_num < TH3_PFC_MMU_QUEUE_NUM; q_num++) {
        ...
        if (_soc_th3_pfc_mmuq_is_mc(unit, q_num) &&
                           SOC_TH3_PFC_UC_ONLY(unit, profile_id, cos_f_q)) {
    ...
    uint32 profile_id)
{
    ...
    switch(type) {
        case socTH3PFCPGProperties:
            if (profile_id < 0 || profile_id > TH3_PFC_PROPERTIES_PROFILE_ID_MAX) {
                return SOC_E_PARAM;
            }
        ...
        case socTH3PFCReceiveClass:
            if (profile_id < 0 || profile_id > TH3_PFC_CLASS_PROFILE_ID_MAX) {
                return SOC_E_PARAM;
            }
