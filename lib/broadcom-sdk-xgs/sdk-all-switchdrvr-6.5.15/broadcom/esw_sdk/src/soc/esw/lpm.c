...
debug_write_l3_defip(int u, int copyno, int index, void *entry_data)
{
    ...
    int i = 0, j = 0;
    int offset = (soc_mem_index_count(u, mem) >> 1);
    soc_mem_t mem = L3_DEFIPm;
                            ...
                            uint32 *rvt_index)
{
    ...
    if (old_index != FB_LPM_HASH_INDEX_NULL) {
        while(index != FB_LPM_HASH_INDEX_NULL && i++ < hash->entry_count) {
            ...
            SOC_IF_ERROR_RETURN(rv);
            if (index & FB_LPM_HASH_IPV6_MASK) {
                rv = READ_L3_DEFIPm(u, MEM_BLOCK_ANY, rindex + tcam_depth, eupr);
                SOC_IF_ERROR_RETURN(rv);
...
int lpm_state_verify(int u, int *err)
{
    ...
    int32 curr_pfx = MAX_PFX_INDEX(unit);
    int32 *pfxmap = NULL;
    int32 *pfxmap2 = NULL;
    int start;
    int end;
    ...
    free_count = SOC_LPM_STATE_F(lpm_state_ptr, MAX_PFX_INDEX(unit));

    while (curr_pfx != -1) {
        next_pfx = SOC_LPM_STATE_N(lpm_state_ptr, curr_pfx);
        if (curr_pfx == MAX_PFX_INDEX(unit)) {
            if (next_pfx == -1) {
                goto done;
            }
            curr_pfx = next_pfx;
