...
_soc_tomahawk3_itm_reg_check(int unit, soc_reg_t reg, int itm, int base_index)
{
    ...
    if (!SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg).block, SOC_BLK_MMU_ITM)) {
        LOG_CLI((BSL_META_U(unit,
...
_soc_tomahawk3_eb_reg_check(int unit, soc_reg_t reg, int eb, int base_index)
{
    ...
    if (!SOC_BLOCK_IN_LIST(SOC_REG_INFO(unit, reg).block, SOC_BLK_MMU_EB)) {
        LOG_CLI((BSL_META_U(unit,
...
#define _TH3_MMU_NUM_CPU_QUEUES_PER_COS 4
    ...
    SOC_IF_ERROR_RETURN(_soc_mmu_tomahawk3_scheduler_profile_reset(unit, sched_profile));
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        SOC_IF_ERROR_RETURN
            (soc_tomahawk3_sched_profile_attach(unit, port, sched_port_profile_map[port]));
    }
    ...
    for (profile = 0; profile < SOC_TH3_MAX_NUM_SCHED_PROFILE; profile++) {
        ...
        SOC_IF_ERROR_RETURN(_soc_scheduler_profile_mapping_setup(unit, sched_profile, profile,
        ...
        for (queue = 0; queue < SOC_TH3_NUM_GP_QUEUES; queue++) {
            ...
            SOC_IF_ERROR_RETURN
                (soc_tomahawk3_sched_profile_set(unit, profile, L0[queue], schedq[queue], mmuq[queue]));
        ...
        PBMP_ALL_ITER(unit, port) {
            if (port_in_profile[port]) {
                for (queue = 0; queue < SOC_TH3_NUM_GP_QUEUES; queue++) {
                    ...
                    SOC_IF_ERROR_RETURN
                        (soc_tomahawk3_l0_cos_set(unit, port, queue, cos_list[queue]));
                }
            }
        }
    }
    ...
    for (queue = 0; queue < SOC_TH3_NUM_GP_QUEUES; queue++) {
        for(i = 0; i < _TH3_MMU_NUM_CPU_QUEUES_PER_COS; i++) {
            ...
            if (cpu_l1 >= SOC_TH3_NUM_CPU_QUEUES) {
                ...
                SOC_IF_ERROR_RETURN
                    (soc_tomahawk3_cpu_l1_to_l0_profile_set(unit, 0, cpu_l1));
            }
        }
    }
...
soc_tomahawk3_idb_init_update(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_tomahawk3_port_schedule_state_basic_setup(unit,
        port_schedule_state));
...
soc_tomahawk3_cdport_init (int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_tomahawk3_port_schedule_state_basic_setup
...
soc_tomahawk3_ep_top_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_tomahawk3_ep_init(unit, port_schedule_state));
...
soc_tomahawk3_slot_pipeline_config(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(
        soc_tomahawk3_ep_set_slot_pipeline_config(unit, port_schedule_state));
    /*Ipost slot pipeline config*/
    SOC_IF_ERROR_RETURN(
        soc_tomahawk3_ip_slot_pipeline_config(unit, port_schedule_state));
...
soc_tomahawk3_enable_ingress_forwarding (int unit)
{
    ...
    SOC_IF_ERROR_RETURN
        (soc_tomahawk3_en_epc_link_bmap(unit, port_schedule_state));
    SOC_IF_ERROR_RETURN
