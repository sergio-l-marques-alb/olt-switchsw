...
soc_hx5_l2x_base_entry_to_key(int unit, int bank, uint32 *entry, uint8 *key)
{
    ...
    return _soc_hx5_hash_entry_to_key(unit, bank, entry, key, L2Xm, field_list);
}
...
soc_hx5_l3x_base_entry_to_key(int unit, int bank, uint32 *entry, uint8 *key)
{
    ...
    return _soc_hx5_hash_entry_to_key(unit, bank, entry, key, mem, field_list);
}
...
uint32
soc_hx5_mpls_hash(int unit, int bank, int hash_offset, int use_lsb,
                  int key_nbits, void *base_entry, uint8 *key1, uint8 *key2)
{
    uint32 lsb_val;
    uint32 hash_mask;
...
int
soc_hx5_mpls_base_entry_to_key(int unit, int bank, void *entry, uint8 *key) {
    ...
    return _soc_hx5_hash_entry_to_key(unit, bank, entry, key,
                                      MPLS_ENTRYm, field_list);
}

uint32
soc_hx5_mpls_entry_hash(int unit, int bank, int hash_offset,
                        int use_lsb, uint32 *entry)
{
    int             key_nbits;
    uint8           key1[SOC_MAX_MEM_WORDS * 4] = {0};
    ...
    /* Robust Hashed Key using Remap and Action Tables A */
    key_nbits = soc_hx5_mpls_base_entry_to_key(unit, 0, entry, key1);
    /* Robust Hashed Key using Remap and Action Tables B */
    key_nbits = soc_hx5_mpls_base_entry_to_key(unit, 1, entry, key2);

    return soc_hx5_mpls_hash(unit, bank, hash_offset, use_lsb,
                             key_nbits, entry, key1, key2);
}

uint32
soc_hx5_mpls_bank_entry_hash(int unit, int bank, uint32 *entry)
{
    int rv, hash_offset, use_lsb;

    rv = soc_hx5_hash_offset_get(unit, MPLS_ENTRYm, bank, &hash_offset,
                                 &use_lsb);
    if (SOC_FAILURE(rv)) {
        return 0;
    }

    return soc_hx5_mpls_entry_hash(unit, bank, hash_offset, use_lsb, entry);
}
                        ...
                        uint32 *bucket)
{
    ...
    case MPLS_ENTRY_SINGLEm:
    case MPLS_ENTRYm:
        *bucket = soc_hx5_mpls_bank_entry_hash(unit, bank, entry);
