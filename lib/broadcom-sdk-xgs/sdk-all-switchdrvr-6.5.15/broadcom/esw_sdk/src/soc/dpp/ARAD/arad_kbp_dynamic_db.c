       ...
       SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    for (pgm_ndx = pgm_ndx_min; pgm_ndx < pgm_ndx_max; ++pgm_ndx)
    {
        ...
        if (SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type != ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311 && ((search_id == 6) || (search_id == 7))) {
        ...
        if(SOC_IS_JERICHO(unit)){
            ...
            for(i = 0; i < nof_shared_segments; i++) {
                search_format.key_segment[i].nof_bytes = shared_quals_info[i].size;
                sal_memcpy(search_format.key_segment[i].name, shared_quals_info[i].name, 20);
                search_format.key_segment[i].type =
                    (SOC_DPP_CONFIG(unit)->arad->init.elk.tcam_dev_type == ARAD_NIF_ELK_TCAM_DEV_TYPE_BCM52311) ? KBP_KEY_FIELD_EM : KBP_KEY_FIELD_TERNARY;
