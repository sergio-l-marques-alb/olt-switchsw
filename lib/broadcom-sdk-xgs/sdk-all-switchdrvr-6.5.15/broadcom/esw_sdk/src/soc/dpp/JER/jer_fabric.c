  ...
  )
{
    ...
    if (!SOC_IS_QAX(unit)) { /*QAX always supports MC mode and do not have FMC_EN bit*/
        ...
        else { /* need to disable FMC to allow 4-dest UC traffic)*/
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        if (is_mesh_hybrid) {
            for (i = 0; i < BCM_FABRIC_NUM_OF_RCI_LEVELS; ++i) {
                rci_config.soc_dpp_fabric_rci_device_level_thresholds[i] /= 2;
            }
        }
    }
  ...
  )
{
    ...
    /*initialize scan rate to build MC all reachable vector*/
  ...
  )
{
    ...
    if (fabric->fabric_pcp_enable) {
         ...
         */
        /* Default value in QAX is zero already (register ITE_PACKET_LATENCY_MEASUREMENT)*/
        if (!SOC_IS_QAX(unit) && !SOC_IS_FLAIR(unit)){
  ...
  )
{
    ...
    }else if (!SOC_IS_FLAIR(unit)){ /*In Jer, mapping pipes to DTQ are determined according to IPT_IPT_0_DEST_PIPE_MAPPING_2_FDT reg.*/
...
/*
 * Init sw_state of allowed fabric links to be the same as HW
 */
STATIC int
  ...
  )
{
    ...
    /* Init sw_state of allowed fabric links */
    if (!SOC_IS_FLAIR(unit)) {
  ...
  )
{
    ...
    if (fabric_pipe_type != soc_dpp_cosq_gport_fabric_pipe_ingress) {
        ...
        if (/* Relevant only when fabric_links_to_core_mapping == DEDICATED. Fixed for new code (Jer2 and beyond) */(1)) {
            switch (pipe) {
            case 0:
  ...
  )
{
    ...
    /*get DQCF info*/
    /* No DQCF max occupancy. Using max occ DBLF of the same packet (which is always >= DQCF)*/
