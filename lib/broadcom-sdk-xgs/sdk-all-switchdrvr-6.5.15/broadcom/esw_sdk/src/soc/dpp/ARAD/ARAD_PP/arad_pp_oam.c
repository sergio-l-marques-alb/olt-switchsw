...
/* Stamp offsets:
   ...
   NOTE: In case of internal opcode being SLM_SLR, since both SLM and SLR have same internal opcode,
   we take external opcode also as argument to make the offset decision. In case the external opcode 
   was some unknown opcode, we cannot make any guesses about what was the intended internal opcode 
   and hence we resort to the offset of SLM. */
...
/* Maximum number of entries that could have extenal data
 * Transmit such entries take 80 clocks for OAMP TX machine
 * This value is divided by 2 as a result that we setting QAX with maid 48 ccm to be sent on different phases 0 or 1
 * In this case each scan loop will send maximum 1.5*1024 entries where each entry take additional 40 clock.*/
  ...
  )
{
    ...
    * 
    *  OAM TCAM KEY for SBFD initiator is formed as follows:
    *    1) Tunnel Termination code is dont-care.
    *    2) Fwd code is IPv4_UC (key->fwd_code)
    *    3) In LIF profile is dont-care.
    *    4) VTT-OAM-LIF-Valid or VTT-OAM-LIF-Inner-Valid is dont-care.
    *    5) VTT-OAM-LIF-Outer-Valid is dont-care for JER+ and above
    *    6) Packet Data
    *           a) Match IP protocol  = UDP (0x11)
    *           b) Match DIP address  = address passed in the key (via key->your_disc field).
    *           c) Match UDP src port = SBFD dedicated UDP port (7784)
    * 
    */

    /* Tunnel-Termination-Code: TTC = X */
    mask=0;
    field_size = 4;
    offset += field_size;
    ...
    /*VTT-OAM-LIF-Valid (Inner for jer+) = X */
    mask = 0;
    val = 0;
    field_size = 1;
    offset += field_size;
    ...
    /* This is where IP header starts */
    packet_data_start_offset = offset;

    /* calculate other offsets */
    ip_protocol_offset_in_bits = 9 * SOC_SAND_NOF_BITS_IN_BYTE; /* Starts at 9th byte of IP header. */
    dip_addr_offset_in_bits = 16 * SOC_SAND_NOF_BITS_IN_BYTE; /* Starts at 16th byte of IP header. */
    udp_src_port_offset_in_bits = SOC_SAND_PP_IP_NO_OPTION_NOF_BITS;/* Start after IP header */
    udp_des_port_offset_in_bits = udp_src_port_offset_in_bits + 16; /*Start after udp src port*/
    ip_ttl_offset_in_bits = 8 * SOC_SAND_NOF_BITS_IN_BYTE; /* Starts at 8th byte of IP header. */

    /* Packet-data-starting-at-forwarding-header */
    /* Match IP protocol = UDP */
    mask = 0xFFFFFFFF;
    val = SOC_SAND_PP_IP_PROTOCOL_UDP; /* IP protocol value for UDP */
    field_size = SOC_SAND_PP_IP_PROTOCOL_NOF_BITS; /* IP protocol is 8 bits */
    ...
    if(flags == _ARAD_PP_OAM_TCAM_FLAGS_SBFD_REFLECTOR_OVER_MPLS)
    {
        ...
        mask = 0xFF;
        val =1;
        field_size = SOC_SAND_PP_IP_TTL_NOF_BITS; /* TTL is 8 bits */
        offset = packet_data_start_offset + ip_ttl_offset_in_bits + field_size;
        soc_sand_bitstream_set_any_field(&val, _ARAD_PP_OAM_FLP_TCAM_ENTRY_SIZE - offset, field_size, entry->value);
        soc_sand_bitstream_set_any_field(&mask, _ARAD_PP_OAM_FLP_TCAM_ENTRY_SIZE - offset, field_size, entry->mask);
    }
    else{
        /* Match DIP = key->your_disc */
        mask = 0xFFFFFFFF;
        val = key->your_disc; /* IP address is passed in your_disc field from caller API */
        field_size = SOC_SAND_PP_IPV4_ADDRESS_NOF_BITS; /* IPv4 address size is 32 bits */
        offset = packet_data_start_offset + dip_addr_offset_in_bits + field_size;
        soc_sand_bitstream_set_any_field(&val, _ARAD_PP_OAM_FLP_TCAM_ENTRY_SIZE - offset, field_size, entry->value);
        soc_sand_bitstream_set_any_field(&mask, _ARAD_PP_OAM_FLP_TCAM_ENTRY_SIZE - offset, field_size, entry->mask);
    }
   /*
    * Match UDP dst port = 7784
    */
    mask = 0xFFFFFFFF;
    val = 7784;
    field_size = 16; /* UDP src port size is 2 bytes */
  ...
  )
{
  ...
  if (_ARAD_PP_OAM_BFD_IS_MEP_TYPE_USING_YOUR_DISC(classifier_mep_entry->mep_type)) {
      ...
      /* Skip active entry if MPLS Egress_only */
  ...
  )
{
  ...
  if (SOC_IS_QUX(unit) && oam_payload->counter_disable == 1) {
      /*
       * In QUX, 
       * 1) we cannot set dis_cnt field in HW. 
       * 2) So, 
       *     a) we disable counter for opcodes which map to default subtypes by pointing to 
       *      subtype 6 which has counter disabled.
       *     b) For opcodes which do not map to default subtypes, we have already disabled the counting
       *       in arad_pp_oam_classifier_init_unsafe function based on soc properties.
       */
  ...
  )
{
    ...
    /* If AradMode SOC property is set, enable Arad ITMH */
    if (soc_property_get(unit, spn_ITMH_ARAD_MODE_ENABLE, 0) && (SOC_IS_JERICHO(unit))) {
        /* In QAX_A0 and QUX the register are internal */
    ...
    if (is_bfd) {
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_discriminator_type_update", 0)) {
            if (!SOC_IS_QAX(unit)) {
                ...
                /*Write to zeroth profile which will be used by BFD by default */
        ...
        /* Disable UDP src port checking in OAMP for BFD IPv4.
         * 
         * 1) Globally for JERICHO and JERICHO_PLUS.
         * 2) It is done in OAMP MEP PROFILE 1 for QAX and above.
         * This is required for Seamless BFD.
         *
         */
    ...
    if (SOC_IS_QAX(unit)) {
        ...
        /* The following equation must be greater than NUM_CLOCKS_SCAN to allow enough time for scanning entire MEP DB:
           ((Number of entries) + (Maximum Number of entries with external data in one scan loop)
           * (Number of clocks per stage) * (Total number of stages / number of active stages).
	...
	uint32 active_stages = 2; /* is the default for 720MHZ */
        COMPILER_64_ZERO(reg64);
	...
	if(num_clocks_per_mep_scan > ARAD_PP_OAM_NUM_CLOCKS_MIN_NEEDED){
            /* solve the following equation:
               num_clocks_per_mep_scan =  ARAD_PP_OAM_NUM_CLOCKS_MIN_NEEDED + ARAD_PP_OAM_NUM_CLOCKS_MIN_NEEDED*(1/active_stages) */
	    active_stages = ((ARAD_PP_OAM_NUM_CLOCKS_MIN_NEEDED)/(num_clocks_per_mep_scan - ARAD_PP_OAM_NUM_CLOCKS_MIN_NEEDED)) + 1;
            /*active stages field is only 4 bits in Jericho */
            active_stages = (active_stages>ARAD_PP_OAM_MAX_NUM_STAGES_IN_MEP_SCAN) ? ARAD_PP_OAM_MAX_NUM_STAGES_IN_MEP_SCAN : active_stages;
	} else {
		/* case we don't have enough cycles */
	    active_stages = ARAD_PP_OAM_MAX_NUM_STAGES_IN_MEP_SCAN; /* max number the deivce allows */
	}
  ...
  )
{
    ...
    if ((((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_downmep_pwe_classification", 0) == 1) && (SOC_IS_ARADPLUS_AND_BELOW(unit))))
        ...
        *  entries. */
        ...
        mask = (SOC_IS_ARADPLUS_AND_BELOW(unit)) ? 0xFFFFFFFF : 0; /* MDL(3b) */
  ...
  )
{
  ...
  oam_tcam_entry_action.oam_pcp = 0;
  oam_tcam_entry_action.oam_stamp_offset = 20+8 + _ARAD_PP_OAM_OPCODE_STAMP_OFFSET(oam_tcam_entry_action.opcode,  oam_tcam_entry_action.opcode); /*IPV4+UDP*/
  oam_tcam_entry_action.my_cfm_mac = 0;
  ...
  )
{
  ...
  if (is_bfd) {
      ...
      if (ARAD_PP_OAM_BFD_SEAMLESS_BFD_FEATURE_ENABLED(unit)) {
          /* 
           * Set UDP src port min to 7784 to 
           * match reflected SBFD packet's src port
           */
  ...
  if (!is_second_init) {
     ...
     if((SOC_IS_JERICHO(unit) && (!SOC_IS_QAX(unit))) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "enable_ccm_pkt_counting", 0) == 1)){
      ...
      /* For 1711, enable 1711-lm counter, disable slm counter(slm subtype is used for lm disable)*/
      ...
      /* For egress injection in QMX, CCM use subtype 7 classifier,don't count lm & dm, to count traffic (using subtype 0), bit0 has to be set*/
      if((SOC_IS_JERICHO(unit) && (!SOC_IS_QAX(unit))) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "enable_ccm_pkt_counting",0) == 1)){
          soc_reg64_field32_set(unit, IHP_OAM_COUNTER_INCREMENT_BITMAPr, &reg_64, OAM_COUNTER_INCREMENTf, 0xbd);
      }
      ...
      /* SDK-126418 : QUX has HW bug in LM.  */
      if (SOC_IS_QUX(unit)) {
#define DISABLE_COUNT_FOR_SUBTYPE(counter_bmp,subtype) (counter_bmp &= ~(1 << subtype))
          ...
          /* Counting enabled by default for all subtypes except subtype = 6 and subtype = LM */
      ...
      if (SOC_IS_QUX(unit)) {
          ...
          /* 0x4000 means direction bit must be in bit 14 in QUX.
           * And correct CPRS pointer 0 engine for ingress.
           */
      ...
      /*
       *  The oam-port-profile is the same between Down mep trap (which is with PPH) and Down MEP inject (which by design is without PPH),
       *  So it needs to use a different subtype,
       *  Subtype 4 is available for this case that Down MEP injected with PPH on COE port
       *  Users should inject LMM/Rs with OAM-TS with sub-type 4 and mep-type 0
      */
      if((SOC_IS_JERICHO(unit) && (!SOC_IS_QAX(unit)))
         ...
         /* subtype is 4, up mep is 0 and oam port profile is 0 */
         OAM_PROCESS_ADD(4, 0, OAM_PROCESS_ACTION_READ_COUNTER_AND_STAMP, reg_above_64);
      }

      /* for subtype 4 in QAX, STAMP OMTS.data - this makes PRGEs work easier */
      if (SOC_IS_QAX(unit)){
...
/* TOD for Jericho handled separatly*/
   ...
   * For Arad+ and below, default will be taken as 0 
   * and jericho and above default will be taken as 1. */
    ...
    )
{
    ...
    index_count = soc_mem_index_count(unit, IHP_OAM_CHANNEL_TYPEm);
    /*Verify whether user provided values are configured or not before deleting them*/
    ...
    )
{
    ...
    /* Set the default channel type into control word/gach */
    mpls_tp_channel_type = _arad_pp_oam_mpls_tp_channel_type_enum_to_channel_type(unit, channel_type_ndx); 
    cw = 0x10000000 |  mpls_tp_channel_type; /*GACH/CW: 0x1000 - Reserved+version; + Channel type */
    switch (channel_type_ndx) {
        case _ARAD_PP_OAM_MPLS_TP_CHANNEL_BFD_CONTROL_ENUM:
            soc_reg_field_set(unit, OAMP_BFD_PWE_CW_0r, &reg, BFD_PWE_CW_0f, cw);
            rv = WRITE_OAMP_BFD_PWE_CW_0r(unit, reg);
...
soc_error_t arad_pp_oam_counter_increment_bitmap_set(int unit, int enable_counting) {
    ...
    if (!enable_counting) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            counter_inc_reg &= (0xff & (~(1 << _ARAD_PP_OAM_SUBTYPE_DEFAULT_EGRESS_OAM))); /* Do not count injected Subtype 6 - will be used by LM*/
        }
    } else {
        counter_inc_reg |= (1 << _ARAD_PP_OAM_SUBTYPE_LM); /* Count subtype 1 (LM,SLM,SLR,etc.)*/
        if (SOC_IS_JERICHO(unit)) {
           counter_inc_reg |= (1 << _ARAD_PP_OAM_SUBTYPE_DEFAULT_EGRESS_OAM); /* Count subtype 6 for injected SLM/SLR only*/
        }
    }
  ...
  )
{
    ...
    if(((internal_opcode_init[internal_opcode].additional_info == ARAD_PP_OAM_OPCODE_LBM) || 
        /* LBR is handled by SAT loopback in JERICHO for UPMEP.
         * So subtype needs to be loopback for egr editor to
         * apply loopback program. So avoid overwriting it.
         */
    ...
    /* Update Opcode sub_type and snoop_strength if trap snoop profile snoop command is non-zero and 
     * opcode action snoop strength is non-zero */
    if (!oam_payload->up_map && (snoop_profile_info.snoop_cmnd > 0) && (opcode_action_snoop_strength) ) { /*ingress snooping*/
        oam_payload->snoop_strength = _ARAD_PP_OAM_SNOOP_STRENGTH;
        oam_payload->sub_type = _ARAD_PP_OAM_SUBTYPE_DEFAULT;
    }
    ...
    for (is_my_cfm_mac = 0; is_my_cfm_mac <= 1; is_my_cfm_mac++) {
        ...
        for (internal_opcode = 1; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++) {
            ...
            for (mp_type = 0; mp_type < SOC_PPC_OAM_MP_TYPE_COUNT_PLUS; mp_type++) {
                ...
                /* UP_MAP is a variable which indicates whether the hit MEP is UPMEP or not. 
                 */
                oam_payload.up_map = ((classifier_mep_entry->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) != 0);
                ...
                case SOC_PPC_OAM_MP_TYPE_MEP_OR_ACTIVE_MATCH_PLUS: /* Mep active match */
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) { /* Arad mode*/
                        ...
                        SOCDNX_SAND_IF_ERR_EXIT(res);
                        /* Active match of DOWNMEP doesnt need UP_MEP = 1 since its DOWNMEP.
                         * Active match of UPMEP doesnt need UP_MEP = 1 since it would be in the egress.
                         * and UP_MEP will be ignored there. UP_MEP can be safely set to 0.
                         */
                        oam_payload.up_map = 0;
                        res = arad_pp_oam_classifier_oam1_entry_set_on_buffer(unit, &oam1_key, &oam_payload, oam_buffer);
                        SOCDNX_SAND_IF_ERR_EXIT(res);
                ...
                case SOC_PPC_OAM_MP_TYPE_MIP_OR_PASSIVE_MATCH_PLUS:
                    ...
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || (!profile_data->mp_type_passive_active_mix) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)
                        ...
                        oam_payload.cpu_trap_code = oam_trap_code;
                        oam_payload.forwarding_strength = _ARAD_PP_OAM_PASSIVE_TRAP_STRENGTH;
                        /* Passive match of DOWNMEP doesnt need UP_MEP = 1 since its DOWNMEP.
                         * Passive match of UPMEP need UP_MEP = 1 since it would be in the ingress.
                         * UP_MEP should be set to 0 if its ingress and not MIP since that would be passive UPMEP.
                         */
                        if (oam1_key.ingress) {
                ...
                case SOC_PPC_OAM_MP_TYPE_BELOW_PLUS: /* Drop Level */
                    oam_payload.sub_type = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE,
                                                                            "oam_additional_FTMH_on_error_packets", 0) ?
                                                        _ARAD_PP_OAM_SUBTYPE_CCM : _ARAD_PP_OAM_SUBTYPE_DEFAULT;
                    /* Level match of DOWNMEP doesnt need UP_MEP = 1 since its DOWNMEP.
                     * Level match of UPMEP doesnt need UP_MEP = 1 since it would be in the egress.
                     * UP_MEP can be safely set to 0.
                     */
                    oam_payload.up_map = 0;
                    if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit)) {
                                                 ...
                                                 SOC_SAND_IN  SOC_PPC_OAM_LIF_PROFILE_DATA  *profile_data)
{
    ...
    for(internal_opcode=0; internal_opcode < SOC_PPC_OAM_OPCODE_MAP_COUNT; internal_opcode++)
    {
        ...
        case ARAD_PP_OAM_OPCODE_LBR:
            ...
            break;
        default:
            /* TST opcode which is mapped to other internal opcode could be implemented
             * through SAT Loopback engine.
             */
  ...
  )
{
  ...
  if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit)){
      ...
      /* Skip passive entry if MPLS Ingress only */
  ...
  )
{
  ...
  if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type) ||
      ...
      /*MPLS Egress only don't set oem1*/
      if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
           ((classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
            (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ||
            (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) ||
      ...
      if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
          /* Fill the O-EM1 separately (If MPLS Egress only mep, don't set oem1 mep entry for this mep)*/
          if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
               ((classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
                (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ||
                (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) ||
          ...
          if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) { /* OAM */
              ...
              if (is_mep) {
                 ...
                 /* In case of MIP creation, preserve the counter value if 
                  * OEM_1 payload have counter index present */
      ...
      /*Don't add ingress table if MPLS Egress only flag*/
      if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
           ((classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
            (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ||
            (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) ||
  ...
  )
{
  ...
  /* delete OEM1-2 entries , MPLS Egress only don't have this entry*/
  if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
       ((classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
        (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ||
        (classifier_mep_entry->mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) ||
    ...
    /* delete bfd OEM2 entry if exists and exit */
    if (found) {
      if (SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) {
  ...
  if (!SOC_PPC_OAM_IS_MEP_TYPE_BFD(classifier_mep_entry->mep_type)) { /* Ethernet and Y1731 */
      ...
      if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
          if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
             ...
             if ((mdl_mp_type == matching_mdl_mp_type) && (mdl_mp_type == _JER_PP_OAM_MDL_MP_TYPE_ACTIVE_MATCH)) {
                 active_active_combo = 1; /* Indicates two MEPS of same MDL and same LIF but different directions */
             }
             /* If its a case of active active on both ingr and egr OEM entries,
              * then its a case of UPMEP and DOWNMEP existing on same levels on
              * same lif. In this case instead of deleting the active side entry,
              * change it to passive so that it will continue to do passive filtering
              * for the other undeleted MEP.
              */
      ...
      if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
          ...
          if ((new_mip_bitmap != 0) || (new_mep_bitmap != 0) || new_mp_type_vector) {
              ...
              if (active_active_combo) {
                  /* There is a MEP of the opposite direction but same level on this LIF,
                   * if it is the only other entity using this OEM-1 entry, reset the
                   * MP-profile to 0
                   */
                  ...
                  if (num_active_meps_mips == 0) {
                      /* No entity is using this entry's active MP-profile, reset it to 0 */
      ...
      if (classifier_mep_entry->passive_side_lif != _BCM_OAM_INVALID_LIF) {
        ...
        if (SOC_PPD_OAM_IS_CLASSIFIER_ARAD_MODE(unit) || SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
          ...
          if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
              /*Use new_mp_type_vector from Active entry for MPLS OAM endpoints as
                it is not programmed for passive entry when oam_downmep_pwe_classification is disabled */
          ...
          if (active_active_combo) {
              /* If its a case of active active on both ingr and egr OEM entries,
               * then its a case of UPMEP and DOWNMEP existing on same levels on
               * same lif. In this case do not delete the passive side entry,
               * It is the active entry for other undeleted MEP.
               */
 ...
 (SOC_IS_ARADPLUS_AND_BELOW(unit) && (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_downmep_pwe_classification", 0) == 0)))))) {
  ...
  if (!((classifier_mep_entry->lif == _BCM_OAM_INVALID_LIF) &&
    ...
    /* delete non-bfd OEM2 entry if exists */
    if (found) {
        res = arad_pp_oam_classifier_oem2_entry_delete_unsafe(unit, &oem2_key);
        SOC_SAND_CHECK_FUNC_RESULT(res, 30, exit);
    }
  ...
  )
{
  ...
  if ((mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ||
      ...
      /*In case the endpoint is modified, store the previous mep-pe-profile*/
      if(SOC_IS_QAX(unit) && !is_new_ep) {
      ...
      if (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) { /* Only relevant for Ethernet */
          ...
          /* Set mep pe profile for down ETH OAM Egress Injection only*/
          if(SOC_IS_QAX(unit) && (mep_db_entry->out_lif !=0) && (!mep_db_entry->is_maid_48) && (!mep_db_entry->is_upmep)) {
              /*in case the endpoint is created, set the new mep-pe-profile*/
            if(is_new_ep){
                ...
                * 16 LSB of Out_Lif will be stored in Generic Memory*/
                ...
                /*In case of modifying endpoint,the previous mep_pe_profile is set*/
          ...
          if (SOC_IS_QAX(unit) && (mep_db_entry->out_lif != 0) && (!mep_db_entry->is_maid_48)) {
             /* Set mep_pe_profile in case of creating or modifying endpoint*/
              if(is_new_ep) {
    ...
    /*Set the mep_pe_profile in case of creating or modifying the endpoint in case maid_48*/
    if(SOC_IS_QAX(unit)) {
      if (mep_db_entry->is_maid_48) {
          if(is_new_ep) {
          /*The function returns the mep pe profile. 
            The function passes as parameters is_upmep,mep_type and outlif.
            Depends on the parameters the correct mep-pe-profile is set.*/
      ...
      if (mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_BFD_O_IPV4_1_HOP) {
          ...
          /* 
           * FLEX IP TOS feature can never enter this if condition.
           * Only special case possible here is MSB turn off for single hop over bundle
           */
          ...
          if (mep_db_entry->s_bfd_flag == SOC_PPC_OAM_S_BFD_INITIATOR) {
              ...
              if (SOC_IS_QAX(unit)) {
                ...
                /* Set the MEP profile to profile ID 1 for UDP src port check disabling */
                soc_mem_field_set(unit, mem, entry_above_64, MEP_PROFILEf, &reg_val); 
              }
          }
          /* 
           * Both features set to true. 
           * This can happen only for single hop over bundle case 
           * with flex tos being true */
          if ((mep_db_entry->disc_type_update) && 
              ...
              /* 
               * Discr MSB off not set, but flex_tos flag set.
               * Should be Single hop No bundle case 
               */ 
              ...
              /*
               * Flex tos feature not set, but discr MSB off set. 
               * Should be Multi hop discr off case 
               */
              arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_MHOP_SHOP_BFD, &reg_val);
              if (reg_val == -1) {
                  SOC_SAND_SET_ERROR_MSG((_BSL_SOCDNX_SAND_MSG("ARAD_PP_OAMP_PE_PROGS_MHOP_SHOP_BFD program not loaded.")));
  ...
  if(!is_new_ep) {
      do
      {
          /* This case is for endpoint update */
          ...
          /* entry_above_64 was read from HW entry. It had a ECC field value, calculated by HW. 
           * When it was read, then few entries were updated and writtenback to HW, the ECC field will be recalculated.
           * So, for comparison we need to set it 0,as this is not in byte boundary(Memory comparison cannot be done leaving it).
           * Once it is written to HW, ECC field will be again calculated.
           */
          reg_val = 0;
          if(SOC_IS_JERICHO(unit)) {
      ...
      /* For endpoint create case the check is not required */
      SOC_SAND_SOC_IF_ERROR_RETURN(res, 30, exit, _ARAD_PP_OAM_INTERNAL_WRITE_OAMP_MEP_DB_x_BY_MEM(unit, mem, MEM_BLOCK_ANY, mep_index, &entry_above_64));
  }
...
static uint32 arad_pp_oam_oamp_rmep_emc_set_unsafe(int unit,
  ...
  uint64  failure;
   ...
   SOC_SAND_SOC_CHECK_FUNC_RESULT(res, 50, exit);
     ...
     if(SOC_IS_QAX(unit)) {
         SOC_SAND_SOC_IF_ERROR_RETURN(res, 100, exit,soc_mem_read_no_cache(unit, SOC_MEM_NO_FLAGS, OAMP_RMEP_DBm, 0 /* Rmep-index zero */, MEM_BLOCK_ANY, 0, reg_above_64));
      ...
      if(SOC_IS_QAX(unit)) {
          ...
          /* Checking if before and after EMC set, MEP_DB_PTR field in RMEP_DB entry 0 has changed due to EMC issue 
           * during updating non-zero RMEP_DB index. */
...
/*********************************************************************
*     Set the UDP src port number to be used in Seamless BFD endpoints
*     into IHP_OAM_BFD_OVER_IP_CFG.UDP_DST_PORT_BFD_MULTI_HOPf to
*     handle SBFD reflected packet's UDP dst port.
*********************************************************************/
...
/*********************************************************************
*     Get the UDP src port number to be used in Seamless BFD endpoints
*     from IHP_OAM_BFD_OVER_IP_CFG.UDP_DST_PORT_BFD_MULTI_HOPf to
*     handle SBFD reflected packet's UDP dst port.
*********************************************************************/
  ...
  )
{
  ...
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "bfd_discriminator_type_update", 0)) {
      /* Turn off the MSB bit in the range and set it to min field */
    ...
    if ((mep_db_entry->is_slm) && (!is_upmep)){
        ...
        if(mep_pe_profile < mep_pe_profile_offset) {
            /* Set SLM OAMP PE program for downmep only.
             If not egress injection or double pointer set the fourth bit.
             The fourth bit is responsible for SLM indication in MEP_PE_PROFILE in case not double pointer or egress injection*/
            reg32_field = mep_pe_profile | (1 << ARAD_PP_OAMP_MEP_PE_PROFILE_SLM_OFFSET); /* Setting the SLM bit in MEP PE profile*/
        } else {
            /* Raise the second bit,that responcible for SLM indication in MEP_PE_PROFILE */
            reg32_field = mep_pe_profile | (1 << ARAD_PP_OAMP_MEP_PE_PROFILE_COUNT_SLM_OFFSET); /* Setting the SLM bit in MEP PE profile for Egress Injection or CCM counting*/
        }
  ...
  )
{
    ...
    if (mep_db_entry->is_1DM) {
        ...
        do
       {
        ...
        SOC_SAND_SOC_IF_ERROR_RETURN(res,32,exit,_ARAD_PP_OAM_INTERNAL_WRITE_OAMP_MEP_DB_x_BY_MEM(unit,mem,MEM_BLOCK_ALL,mep_db_entry->mep_id,reg_data));
        /*Incase of QAX this function is only called to write 1DM OAMP PE Profile and return from here */
        if (SOC_IS_QAX(unit)) {
            ARAD_PP_DO_NOTHING_AND_EXIT;
        }
    } else {
       ...
       )
{
    ...
    soc_reg_above_64_val_t reg_data;
    uint32  mep_type;
    uint32 cur = endpoint_id + 1; /* Assume that LM/DM is always +1 */
    uint32 found_bitmap_lcl[1];

    SOC_SAND_INIT_ERROR_DEFINITIONS(0);

    *found_bitmap_lcl=0; /* Set default value of 0 */

   /* In case of last MEP_DB entry, check and exit */
    ...
    /* In other case, check except in case of last entry, than skip */
  ...
  )
{
    ...
    if (mep_type== SOC_PPC_OAM_MEP_TYPE_DM) { 
        ...
        *removed_index = endpoint_id + 1; 
        ...
        /* This check has been added if the serach for MEP type crosses 
         * table size boundary. In that case it will be trested as mep_type=0 
         * which is default mep_type field in OAMP MEP DB if entry does not exist there.
         */
        ...
        if (is_lm) { /* cases are: LM, DM ; LM,LM-STAT,DM ; LM,LM-STAT, LM*/
            if (mep_type== SOC_PPC_OAM_MEP_TYPE_DM) {
                ...
                *removed_index = endpoint_id+2;
                ...
                /* This check has been added if the serach for MEP type crosses 
                 * table size boundary. In that case it will be trested as mep_type=0 
                 * which is default type field in OAMP MEP DB if entry does not exist there.
                 */
  ...
  )
{
    ...
    /* SDK-126418 : Counter disable should not be set for QUX. 
     * It is achieved through other methods. Silently
     * ignore the counter disable calls. 
     *
     * NOTE: counter_enable in the API is a misnomer. 
     * it is actually counter_disable.
     */
...
soc_error_t arad_pp_oam_interrupt_init_internal(int unit, SOC_PPC_OAM_INTERRUPT_GLOBAL_DATA * interrupt_data, int cmc, int ch) {
    ...
    if (use_dma) {        
        ...
        if (res != SOC_E_EMPTY) {
            ...
            /*Extra interrupt when DMA memory is empty should be ignored at the caller */
            sal_memset(interrupt_data->interrupt_message, 0xff, _ARAD_PP_OAM_SIZE_OF_HOST_BUFFER_FOR_EVENT_FIFO_USED_BY_DMA);
            /*enable the interrupt*/
            soc_cmicm_cmcx_intr0_enable(unit, cmc, IRQ_CMCx_FIFO_CH_DMA(ch));
       ...
       )
{
    ...
    if (event_type == SOC_PPC_OAM_DMA_EVENT_TYPE_STAT_EVENT)
    {
        ...
        if (res == SOC_E_EMPTY) {
            /* SOC_E_EMPTY is harmless. Lets mask the error message */
            SOC_EXIT;
        } else {
            SOCDNX_IF_ERR_EXIT(res);
        }
   ...
   uint32 mask, val = 0;
    ...
    if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "y1711_enabled",0) == 1) ||
        ...
        if ((((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "oam_downmep_pwe_classification", 0) == 1) && (SOC_IS_ARADPLUS_AND_BELOW(unit))))
            ...
            /* Iterate over num_tags for each ttc */
            ...
            if(SOC_IS_ARADPLUS_AND_BELOW(unit)){
                /*There is 0-7 opcodes iteration inside num_tags iteration for ARAD+ and below */ 
                mdl++;
                num_tags += mdl/8;
                mdl = mdl % 8;
            } else {
            ...
            if (forward_code== _ARAD_PP_OAM_FWD_CODE_IPV4_UC) {
                ...
                switch (ttc) {
                case 0:   /* In-case of MIM, it is hard set to 0 and any other case don't use it */
                    ether_type_begin_bit = tcam_entry_key_header_start + 12 * 8;
                    cfm_begin_bit = tcam_entry_key_header_start + 14 * 8;
                    break;
                case SOC_PPC_PKT_TERM_TYPE_MPLS_ETH:
 ...
 *
 * Purpose:
 *      For given MDL, LIF and DIR read
 *      OEM-2 table entry found return EP ID found as 2
 *      OEM-1 table entry found return found as 1 and drive EP Id from
 *            MDL, LIF and DIR
 *      Return corresponding endpoint was not found neither in OEM-2 nor OEM-1 tables
 *
 * Parameters:
 *      unit         - (IN)     SOC unit number.
 *      lif          - (IN)     LIF on which the MEP/MIP is being created
 *      mdl          - (IN)     MDL Level MEP/MIP exists
 *      is_upmep     - (IN)     Direction(Up/Down) on which the MEP/MIP is created
 *      is_mip       - (OUT)    True - MIP is being created
 *      found        - (OUT)    2 - Entry exist on OEM-2 table
 *                              1 - Entry exist on OEM-1 table
 *                              0 - Corresponding endpoint was not found
 *      oam_id       - (OUT)    Entry found in OEM-2 table extract OAM_ID from oem-2 payload
 *
 * Returns:
 *      SOC_E_XXX
 *
        ...
        if (SOC_PPD_OAM_IS_CLASSIFIER_JERICHO_MODE(unit)) {
            ...
            *found = ((oem1_payload.mep_bitmap >> mdl) & 1);
            ...
            if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && (!(*found)) && (mdl != 0 )) {
                ...
                } else { /* oam_classifier_mode=1 utmost two meps can exist on same LIF*/
