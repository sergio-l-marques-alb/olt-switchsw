...
/*
 * This is an estimate of the amount of clock cycles needed for a single
 * Flush Machine access to the LEM
 */
  ...
  )
{
  ...
  /* Pick the correct registers names according to the number of cores */
    ...
    )
{
  ...
  if (SOC_IS_ARADPLUS(unit) && (compare_mask & (SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_COMPARE_SLB | 
    ...
    } else /* if (compare_mask & SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_SELECT_REPLACE_SLB) */ {
  ...
  )
{
  ...
  /* As part of moving to the DBAL Infrastructure the LEM is using the DBAL in
   * the following case and not as it used to in the "else" section */
...
/*********************************************************************
*     Clear the IPMC L2 routing table.
 *     Details: in the H file. (search for prototype)
*********************************************************************/
uint32
  ...
  )
{
   ...
   */
/*
  ARAD_PP_STRUCT_VERIFY(SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_RULE, rule, 10, exit);
  ARAD_PP_STRUCT_VERIFY(SOC_PPC_FRWRD_MACT_TRAVERSE_ACTION, action, 20, exit);
*/
  ARAD_PP_DO_NOTHING_AND_EXIT;
  ...
  )
{
  ...
  /*
   * Get the remaining time to finish
   */
...
/*********************************************************************
*      Checks if the flush machine is in progress. 
 *     return success/fail in is_wait_success 
 *     returns the expected time to finish in waiting_time_in_ms
 *     in case is_wait_success==0
*********************************************************************/
uint32
                                                                                       ...
                                                                                       SOC_SAND_OUT int                                  *flush_not_running)
{
       ...
       /*
        *      Get the number of matched entries and the remaining time for the flush to finish
        */
...
/* Start the flush machine by setting the current index to the end of the MACT. Wait until the flush machine is done before existing the function */
STATIC uint32
  ...
  )
{
    ...
    /* Wait for the flush machine to finish */
  ...
  )
{
   ...
   * Bulk operation are not configured if SIM EM is enabled.
   */
  ...
  )
{
  ...
  {
    /* 
     * Calculate the remaining time: fld_val contains amount of entries that were not accessed yet,
     * And (ARAD_PP_FRWRD_MACT_TICKS_PER_ACCESS / ticks_per_ms) estimates the time in ms to process a single entry
     */
    time_remaning_ms = fld_val * ARAD_PP_FRWRD_MACT_TICKS_PER_ACCESS / ticks_per_ms;

    /* In case time was rounded down to zero, make sure traverse is not stopped */
  ...
  )
{
    ...
    /* Convert the results to MACT format */
...
/*    entries_to_scan,*/
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
    ...
    /* read and parse reply to lem structs */
      ...
      /* Traverse according to age is activated and age does not match */
      if ((value_rule != NULL) &&
...
/*********************************************************************
*      checks if all valid actions in flush machine are delete actions, 
 *     if not then should_wait_be_done=1
 *     else should_wait_be_done=0
*********************************************************************/
uint32
  ...
  )
{
...
#ifdef CRASH_RECOVERY_SUPPORT
  /* perform sanity check for on-demand transaction handling */
  soc_dcmn_cr_ondemand_em_traverse_hit(unit);

  /* Enable immediate hw access for the safe traverse operations */
  ...
  LOG_INFO(BSL_LS_SOC_FORWARD,
	...
	if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "flush_non_blocking_when_all_actions_delete", 0)) {
		...
		should_wait_be_done = !all_are_delete_entries; /* if one of the entries is non-delete action then wait should be done*/
	}

	waiting = wait_till_finish && should_wait_be_done;

#ifdef CRASH_RECOVERY_SUPPORT
    /* If the hw log is journaling, then the traverse action type is an unsafe. In this case insert an additional flush machine polling element to the tail of the hw log */
    if(BCM_UNIT_DO_HW_READ_WRITE(unit)) {
        res = arad_polling(
          unit,
          ARAD_TIMEOUT*5, /* flush machine polling TIMEOUT must be bigger than ARAD_TIMEOUT */
          ARAD_MIN_POLLS,
    ...
    /* invalidate flush DB entries after the flush machine is done*/
    for (entry_offset = 0; entry_offset < ARAD_PP_MACT_NOF_FLUSH_ENTRIES; entry_offset++) {
         ARAD_PP_IHP_MACT_FLUSH_DB_TBL_DATA_clear(&tbl_data);
         tbl_data.compare_valid = 0;
         res = arad_pp_ihp_flush_db_tbl_set_unsafe(
  ...
  /* skip waiting, since journaling is enabled */
  waiting=0;
}
#endif
	if (waiting) {
		...
		/*
		 *	if(waiting == 0) , case were we dont wait.
		 */
		*nof_matched_entries = 0;
	}
...
exit:
#ifdef CRASH_RECOVERY_SUPPORT
    /* restore hwlog journaling if immediate hw access was enabled */
    if(hw_access) {
        soc_hw_restore_immediate_hw_access(unit);
    }
#endif
  ...
  )
{
  ...
  else /* SOC_PPC_FRWRD_MACT_KEY_TYPE_MAC_ADDR */
  {
   ...
   */
/*  ARAD_PP_STRUCT_VERIFY(SOC_PPC_FRWRD_MACT_TRAVERSE_MATCH_VALUE_RULE, &(info->value_rule), 13, exit); */
  ...
  )
{
 ...
 /* compare for key in bits 69-54 (FID)*/
  ...
  /* set key */
  /* {prefix (4 bits),vsi (16 bits}  */
  tbl_data->compare_key_20_data = 0;
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_data,0, key_in_buffer.buffer, 48, 16);
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_data,
                    16, 
                    key_in_buffer.buffer, 
                    SOC_DPP_DEFS_GET(unit, lem_width) - 4, /* use 4 instead of SOC_DPP_DEFS_GET(unit, nof_lem_prefixes) since there are only 4 bits in compare_key_20_data for prefix */
                    4); /* SOC_DPP_DEFS_GET(unit, nof_lem_prefixes)); */
  /* set mask*/
  tbl_data->compare_key_20_mask = 0;
  SHR_BITCOPY_RANGE(&tbl_data->compare_key_20_mask,0, key_in_buffer_mask.buffer, 48, 15);
...
/*********************************************************************
 *     Add an entry to the LEM using DBAL.
 *********************************************************************/
uint32
...
/*********************************************************************
 *     Get an entry to the LEM using DBAL.
 *********************************************************************/
uint32
...
/*********************************************************************
 *     Remove an entry to the LEM using DBAL.
 *********************************************************************/
uint32
