...
/* Ethernet interfaces for DNX devices
   that take less than 4 lanes.*/
...
soc_port_sw_db_port_validate(int unit, soc_port_t port, uint32 channel, uint32 flags, soc_port_if_t interface, soc_pbmp_t phy_ports) 
{
    ...
    SOC_PBMP_ITER(ports_bm, port_i) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            /*if ETH and ILKN ports are on the same quad*/
            if ((interface == SOC_PORT_IF_ILKN || interface_o == SOC_PORT_IF_ILKN) &&
                ...
                /* Assign ETH and ILKN bmps and remove QSGMII offsets */
                ...
                /*Find on which quad is the ETH port and raise all quad bits*/
                _SHR_PBMP_FIRST(eth_bmp, phy);
                quad = (phy-1) / LANES_IN_QUAD;

                if (phy == -1) {
                    /* This case should never been reached because we iterate only on valid ports
                       But still because of Coverity issue it's included*/
                    SOCDNX_EXIT_WITH_ERR(SOC_E_NOT_FOUND, (_BSL_SOCDNX_MSG("CANNOT find relevant PHY")));
                }

                /*Check if there is ILKN configured on the same quad*/
                /* The Bitmaps is One-based*/
                ...
                if (SOC_PBMP_NOT_NULL(quad_bm)) {
                    /*ILKN and ETH ports cannot be configured on the same QUAD*/
    ...
    if (SOC_IS_QUX(unit) && (interface == SOC_PORT_IF_LBG)) {
        ...
        /*
         * check 1. phy port index is greater than max index
         *       2. if phy port conflict among LBG port and internal ports(OAMP, OLP, etc)
         */
...
soc_port_sw_db_snapshot_restore(int unit) {
    ...
    /* deallocate snapshot structures */
