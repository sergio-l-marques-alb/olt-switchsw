  ...
  )
{
    ...
    default:
        ...
        if (SOC_DPP_CONFIG(unit)->pp.lawful_interception_enable) {
            /* sip_data[104 +:  8]        = this.traffic_class;*/
  ...
  )
{
    ...
    {
        eth_hdr_size = 12; /* SA(6) + DA(6) */
    }
    else /* VLAN TAG exists */
    {
        eth_hdr_size = 16; /* SA(6) + DA(6) + VTAG(4) */
    }
  ...
  )
{
    ...
    /* prge_data = {DA(6bytes), SA(6bytes), TAG(4bytes)} ;*/
  ...
  )
{
  ...
  if (SOC_DPP_CONFIG(unit)->pp.ipv6_tunnel_encap_mode == SOC_DPP_IP6_TUNNEL_ENCAP_MODE_LEGACY) {  
      ...
      SOC_SAND_CHECK_FUNC_RESULT(res, 130, exit);
       ...
       /* remove old profiles first if exist*/
       if(found) {
           ARAD_PP_EPNI_PRGE_DATA_TBL_DATA prge_reset_data;
           soc_sand_os_memset(&prge_reset_data, 0x0, sizeof(prge_reset_data)); 
       ...
       if (SOC_DPP_CONFIG(unit)->pp.ipv6_tunnel_encap_mode == SOC_DPP_IP6_TUNNEL_ENCAP_MODE_NORMAL) {        
           ...
           /* write to additional data entry */
  ...
  if (SOC_IS_ARADPLUS(unit)) {
      ...
      if (encap_entry_info[0].entry_type == SOC_PPC_EG_ENCAP_ENTRY_TYPE_IPV6_ENCAP) {
          if ((SOC_DPP_CONFIG(unit)->pp.ipv6_tunnel_encap_mode == SOC_DPP_IP6_TUNNEL_ENCAP_MODE_NORMAL) && 
              ...
              /* remove 1/4 entry for Data entry*/
              COMPILER_64_SET(additional_data,0,0);              
              res = arad_pp_lif_additional_data_set(unit, eep_ndx, 0, additional_data, FALSE);
              SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
              /* remove IPv4 entry*/
              ...
              /* remove 1/4 entry for IPv4 entry*/
              COMPILER_64_SET(additional_data,0,0);              
  ...
  )
{
  ...
  *nof_entries = 0; 
  ...
  {
    /* Have a checking for depth before this code, the depth will be never beyond 7 */
    ...
    if (cur_eep_ndx == ARAD_PP_EG_ENCAP_NEXT_EEP_INVALID) {
      ...
      case ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_IP: 
        ...
        /* Check whether it is IPv4 TOR entry */
        if (outlif_value != 0) { 
            ...
            if (is_wide_entry) {
                ...
                /* Temporary save the UDP dest port in DIP[15:0] */
        ...
        /* Check whether it is IPv6 TOR entry */
        if (outlif_value != 0) { 
            ...
            if (is_wide_entry) {
                ...
                /* Temporary save the UDP dest port in DIP[47:32] */
