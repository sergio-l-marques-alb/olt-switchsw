      ...
      uint32 *regs_val)
{
    ...
    /* Not sure we need this - needed for C-model ARAD */
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;
  ...
  {
    /* Check whether to lookup in TCAM or KAPS */
          ...
          )
  {
    ...
    /*was match*/
    tmp = 0;
    /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "LL", "LEM_Match", tmp, 1); */
    res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_JERICHO(unit)? 47: 45, 1,&tmp);
    SOC_SAND_CHECK_FUNC_RESULT(res, 460, exit);
    if (tmp)
    {
      /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "LL", "LEM_Result", lem_payload_buf, 2); */
    ...
    else
    {
      ...
      if (SOC_IS_QAX(unit)) {
        ...
        /* In case second private lookup did not return any result get second public lookup result */
        if (regs_val[0] == 0) {
  ...
  else
  {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "FLP", "LEM_2nd_Match", tmp, 1); */
    res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_JERICHO(unit)? 47: 45,1,&tmp);
    ...
    if (tmp)
    {
      ...
      /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "FLP", "LEM_2nd_Result", lem_payload_buf, 2); */
      if (SOC_IS_JERICHO(unit)) {
...
/*********************************************************************
*     Update the trap code for the given forwarding decision phase
*********************************************************************/
STATIC void 
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;                                                           
  ...
  /* destination*/
  /* Destination valid located in TM command:
   *                         Arad    - 116
   *                         Jericho - 118
   *                         QAX     - 0*/
  tmp = 0;
  /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Dst_Valid", tmp, 1); */
  ...
  if (tmp)
  {
      ...
      *    Bit 18:17=001 -> destination system port
       */
      dest = 0;
      /* Destination located in TM command:
       *                         Arad    - 147:130
       *                         Jericho - 150:132
       *                         QAX     - 19:1*/
      /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Dst", &dest, 1); */
  ...
  /*TC*/
  /* Traffic Class located in TM command:
   *                         Arad    - 129:127
   *                         Jericho - 131:129
   *                         QAX     - 48:46*/
  tmp = 1;/*always valid*/
  /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "TC", &tmp, 1); */
  if (tmp)
  {
  ...
  /*DP*/
   /* Drop Precedence located in TM command:
    *                         Arad    - 127:125
    *                         Jericho - 129:127
    *                         QAX     - 45:44*/
  tmp = 1;/*always valid*/
  /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "DP", &tmp, 1); */
  if (tmp)
  {
  ...
  /*METER0*/
  tmp = 0;
  /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Meter_B_Update", &tmp, 1); */
  ...
  if (tmp)
  {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Meter_B_Ptr", &pkt_tm_info->meter1, 1); */
  ...
  /*METER1*/
  tmp = 0;
  /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Meter_A_Update", &tmp, 1); */
  ...
  if (tmp)
  {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Meter_A_Ptr", &pkt_tm_info->meter2, 1); */
  ...
  if (tmp)
  {
      ...
      /* dpp_dsig_read(unit, core_id, "IRPP", "LBP", NULL, "Eth_Meter_Ptr", &pkt_tm_info->eth_meter_ptr, 1); */
  ...
  /*CUD*/
  /*Read the CUD after the TM-command since it overrides the regs_val parameter */
  ...
  )
{
  ...
  *ret_val = SOC_PPC_DIAG_OK;                                                           
  ...
  if (cur_eep_type != ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC) {
    ...
    if (tmp) {
      /* ESEM last lookup found */
          ...
          /* Out-AC is ESEM lookup result */
          encap_info->out_ac = tmp;
    } else {
      /* Take from Port default */
        /* dpp_dsig_read(unit, core_id, "ETPP", "Prp", "LL", "Default_SEM_Result", &encap_info->out_ac, 1); */
        if (SOC_IS_JERICHO(unit)) {
    ...
    /* dpp_dsig_read(unit, core_id, "ETPP", "Encap", "LL", "VSI", &tmp, 1); */
    if (SOC_IS_JERICHO_PLUS(unit)) {
  ...
  /* dpp_dsig_read(unit, core_id, "ETPP", "Fwd", "Encap", "Out_TM_Port", &tmp, 1); */
  ...
  /* dpp_dsig_read(unit, core_id, "ETPP", "Fwd", "Encap", "Out_PP_Port", &tmp, 1); */
  ...
  )
{
  ...
  *out_rif = 0;
  ...
  /* Stamp native VSI */
  /* dpp_dsig_read(unit, core_id, "IRPP", "FER", "LBP", "Stamp_Native_VSI", &tmp, 1); */
  if (SOC_IS_JERICHO(unit)) {
  ...
  if (tmp) {
      *rif_is_valid = tmp;
      /* dpp_dsig_read(unit, core_id, "ETPP", "Parser", NULL, "VSI_or_RIF", &out_rif, 1); Part of System_Headers_Record */
  ...
  )
{
  ...
  uint32
    res = SOC_SAND_OK;
  SOC_PPC_L2_LIF_PWE_ADDITIONAL_INFO    pwe_additional_info; /* to do: pass this as an argument to this function*/
  ...
  *ret_val = SOC_PPC_DIAG_OK;  
  ...
  if (db_info->bank_id == 0) {
      if (db_info->lkup_num == 1) { /*second*/
          /* dpp_dsig_read(unit, core_id, "IRPP", "ISEM_A", "TT", "TT_ISEM_A_Result", regs_val, 1) */
          if (SOC_IS_QAX(unit)) {
          ...
          /* dpp_dsig_read(unit, core_id, "IRPP", "ISEM_A", "VT", "VT_ISEM_A_Result", regs_val, 1) */
          if (SOC_IS_QAX(unit)) {
      ...
      if (db_info->lkup_num == 1) { /*second*/
          /* dpp_dsig_read(unit, core_id, "IRPP", "ISEM_B", "TT", "TT_ISEM_B_Result", regs_val, 1) */
          if (SOC_IS_QAX(unit)) {
          ...
          /* dpp_dsig_read(unit, core_id, "IRPP", "ISEM_B", VT", "VTT_ISEM_B_Result", regs_val, 1) */
          if (SOC_IS_QAX(unit)) {
      ...
      if (db_info->lkup_num == 1) { /*second*/
          /* dpp_dsig_read(unit, core_id, "IRPP", "TCAM", "TT", "TT_TCAM_Result", regs_val, 2) */
          if (SOC_IS_QAX(unit)) {
          ...
          /* dpp_dsig_read(unit, core_id, "IRPP", "TCAM", "VT", "VT_TCAM_Result", regs_val, 2) */
          if (SOC_IS_QAX(unit)) {
      ...
      /* dpp_dsig_read(unit, core_id, "IRPP", "LL", "VT", "In_PP_Port", &port, 1) */
      if (SOC_IS_JERICHO_PLUS(unit)) {
  ...
  if (db_info->bank_id == 0 || db_info->bank_id == 1) { /* ISEM-A & B */
     if (db_info->bank_id == 0) {
         /* dpp_dsig_read(unit, core_id, "IRPP", "VT", "ISEM_A", "VT_ISEM_A_Key", regs_val, 2) */
         /* dpp_dsig_read(unit, core_id, "IRPP", "TT", "ISEM_A", "TT_ISEM_A_Key", regs_val, 2) */
         /* Change logic depending on lkup_num */
         if (SOC_IS_QAX(unit)) {
         ...
         /* dpp_dsig_read(unit, core_id, "IRPP", "VT", "ISEM_B", "VT_ISEM_B_Key", regs_val, 2) */
         /* dpp_dsig_read(unit, core_id, "IRPP", "TT", "ISEM_B", "TT_ISEM_B_Key", regs_val, 2) */
         /* Change logic depending on lkup_num */
         if (SOC_IS_QAX(unit)) {
     ...
     if (db_info->lkup_num == 1) { /*second*/
         /* dpp_dsig_read(unit, core_id, "IRPP", "TT", "TCAM", "TT_TCAM_Key", regs_val, 5) */
         /* dpp_dsig_read(unit, core_id, "IRPP", "TT", "TCAM", "TT_TCAM_DB_Profile_Index", regs_val, 1) */
         if (SOC_IS_QAX(unit)) {
          ...
          /* dpp_dsig_read(unit, core_id, "IRPP", "VT", "TCAM", "VT_TCAM_Key", regs_val, 5) */
          if (SOC_IS_QAX(unit)) {
          ...
          /* dpp_dsig_read(unit, core_id, "IRPP", "VT", "TCAM", "VT_TCAM_DB_Profile_Index", regs_val, 1) */
          if (SOC_IS_QAX(unit)) {
    ...
    if(db_info->bank_id != 3)
    {
        ...
        else
        {
            ...
            if ((ihp_lif_table_data.type == 0x0 || /* PWE P2P*/
                    ...
                    (lkup_info->type == SOC_PPC_DIAG_LIF_LKUP_TYPE_MPLS_TUNNEL) /* thought as MPLS */)
            {
  ...
  )
{
  ...
  *valid = FALSE;
  ...
  if (db_info->lkup_num == 0) {
    /* dpp_dsig_read(unit, core_id, "IRPP", "FLP", "LEM", "LEM_2nd_Key", lem_key_buf.buffer, 3) */
    res = soc_sand_bitstream_get_any_field(
            regs_val,
    ...
    tmp = 0;
    /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "FLP", "LEM_2nd_Found", &tmp, 1) */
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "FLP", "LEM", "LEM_1st_Key", lem_key_buf.buffer, 3) */
    res = soc_sand_bitstream_get_any_field(
            regs_val,
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "FLP", "PMF", "LEM_1st_Found", lem_key_buf.buffer, 3) */
    if (SOC_IS_QAX(unit)) {
    ...
    if (tmp) {
      *valid = TRUE;
      /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "FLP", "LEM_1st_Result", lem_payload_buf, 2) */
      res = soc_sand_bitstream_get_any_field(
              regs_val,
    ...
    /*key*/
      ...
      /* dpp_dsig_read(unit, core_id, "IRPP", "LL", "VT", "LEM_Key", lem_payload_buf, 2) */
      res = soc_sand_bitstream_get_any_field(
            regs_val,
    ...
    tmp = 0;
    /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "LL", "LL_LEM_Found", &tmp, 2) */
    res = soc_sand_bitstream_get_any_field(regs_val, SOC_IS_JERICHO(unit)? 114: 45, 1, &tmp);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    if (tmp)
    {
      *valid = TRUE;
      /* dpp_dsig_read(unit, core_id, "IRPP", "LEM", "LL", "LL_LEM_Result", lem_payload_buf, 2) */
      res = soc_sand_bitstream_get_any_field(
              regs_val,
  ...
  *ret_val = SOC_PPC_DIAG_OK;
  ...
  if (cur_eep_type == ARAD_PP_EG_ENCAP_EEDB_ACCESS_TYPE_OUT_AC) {
      ...
      prm_vec_res->ac_tag2.vid = ac_fields.edit_info.vlan_tags[1].vid;
       ...
       /* Read default AC properties from EPNI_DEFAULT_AC_ENTRY */
  ...
  /* dpp_dsig_read(unit, core_id, "ETPP", "LL", NULL, EVEC_Table_Index, regs_val, 1); */
  if (SOC_IS_QAX_A0(unit)) {
  ...
  )
{
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", NULL, "VT", "Fwd_Action_TC", &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", NULL, "VT", "Fwd_Action_DP", &fld_val[1], 1); */
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {  
        ...
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,2,0,117,115,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,2,0,122,121,fld_val[1]); /* DP */
    }
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "TT", NULL, "COS_Profile",         &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", "TT", NULL, "Fwd_Action_TC",       &fld_val[1], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", "TT", NULL, "Fwd_Action_DP",       &fld_val[2], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", "TT", NULL, "Terminated_DSCP_Exp", &fld_val[3], 1); */
    if (SOC_IS_QAX(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,3,16,11,fld_val[0]); /* COS-profile */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,0,58,56,fld_val[1]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,0,62,61,fld_val[2]); /* DP */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,3,35,28,fld_val[3]); /* Terminated-DSCP-EXP */
    } else if (SOC_IS_JERICHO_PLUS(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,3,20,15,fld_val[0]); /* COS-profile */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,0,58,56,fld_val[1]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,0,62,61,fld_val[2]); /* DP */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,3,3,39,32,fld_val[3]); /* Terminated-DSCP-EXP */
    } else if (SOC_IS_JERICHO(unit)) {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", NULL, PMF, "Fwd_Action_TC", &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", NULL, PMF, "Fwd_Action_DP", &fld_val[1], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", NULL, PMF, "In_DSCP_EXP",   &fld_val[2], 1); */
    if (SOC_IS_QAX(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,166,164,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,173,172,fld_val[1]); /* DP */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,98,91,fld_val[2]); /* In-DSCP-EXP */
    } else if (SOC_IS_JERICHO_PLUS(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,171,169,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,178,177,fld_val[1]); /* DP */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHP_ID,core_id,4,8,103,96,fld_val[2]); /* In-DSCP-EXP */
    } else if (SOC_IS_JERICHO(unit)) {
        ...
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,0,7,90,83,fld_val[2]); /* In-DSCP-EXP */
    } else {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", PMF, NULL, "TC",          &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", PMF, NULL, "DP",          &fld_val[1], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", PMF, NULL, "In_DSCP_EXP", &fld_val[2], 1); */
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,0,13,207,205,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,0,13,209,208,fld_val[1]); /* DP */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,0,13,155,148,fld_val[2]); /* In-DSCP-EXP */
    } else if (SOC_IS_JERICHO(unit)) {
    ...
    /* dpp_dsig_read(unit, core_id, "IRPP", "FER", NULL, "TC",          &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", "FER", NULL, "DP",          &fld_val[1], 1); */
    /* dpp_dsig_read(unit, core_id, "IRPP", "FER", NULL, "In_DSCP_EXP", &fld_val[2], 1);  Why no to add it to diag */
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,2,3,55,53,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_IHB_ID,core_id,2,3,57,56,fld_val[1]); /* DP */
    } else if (SOC_IS_JERICHO(unit)) {
    ...
    /* dpp_dsig_read(unit, core_id, "ERPP", "Parser", NULL, "TC",          &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "ERPP", "Parser", NULL, "DP",          &fld_val[1], 1); */
    if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
        ...
        ARAD_PP_DIAG_FLD_GET(ARAD_EGQ_ID,core_id,3,0,3,1,fld_val[0]); /* TC */
        ARAD_PP_DIAG_FLD_GET(ARAD_EGQ_ID,core_id,3,0,5,4,fld_val[1]); /* DP */
    }
    LOG_CLI((BSL_META_U(unit,
    ...
    /* dpp_dsig_read(unit, core_id, "ETPP", "Parser", NULL, "TC",          &fld_val[0], 1); */
    /* dpp_dsig_read(unit, core_id, "ETPP", "Parser", NULL, "DP",          &fld_val[1], 1); */
    /* dpp_dsig_read(unit, core_id, "ETPP", "Term",   NULL, "In_DSCP_EXP", &fld_val[2], 1); */
    ...
    /* dpp_dsig_read(unit, core_id, "ETPP", "Term", NULL, "In_DSCP_EXP",  &fld_val[0], 1);
       dpp_dsig_read(unit, core_id, "ETPP", "Fwd",  NULL, "Out_DSCP_EXP", &fld_val[3], 1);
       dpp_dsig_read(unit, core_id, "ETPP", "Term", NULL, "Header_Code", &eg_cos_aft_fwd_header_code, 1);
       if ((eg_cos_aft_fwd_header_code == 0) ||
          (eg_cos_aft_fwd_header_code == 1) ||
          (eg_cos_aft_fwd_header_code == 2)) {
          dpp_dsig_read(unit, core_id, "ETPP", "Encap", NULL, "DSCP_Remark_Index", &eg_cos_aft_fwd_dscp_remark, 1);
          fld_val[1] = 0;
          ...
          dpp_dsig_read(unit, core_id, "ETPP", "Encap", NULL, "Exp_Remark_Index", &eg_cos_aft_fwd_exp_remark, 1);
         fld_val[1] = 0;
    ...
    /* dpp_dsig_read(unit, core_id, "ETPP", "Encap",   NULL, "Out_DSCP_EXP ", fld_val[0], 1); */
    if (SOC_IS_JERICHO_PLUS(unit)) {
        ARAD_PP_DIAG_FLD_GET(ARAD_EPNI_ID,core_id,8,0,77,70,fld_val[0]); /* Out-DSCP-EXP */
    } else if (SOC_IS_JERICHO(unit)) {
