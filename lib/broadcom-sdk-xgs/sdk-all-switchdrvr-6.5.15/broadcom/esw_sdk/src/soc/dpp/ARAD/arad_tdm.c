...
arad_tdm_local_to_reassembly_context_get(int unit, uint32 port, uint32 *reassembly_context)
{
    ...
    /* In Jericho: TDM reassembly context is mapped statically to port# (512 contexts, each port has 1 context).
     * In QAX: using mapping from logical port to TM port */
    if (SOC_IS_QAX(unit)) {
        int core = 0;
  ...
  )
{
    ...
    {  
        ...
        for (find_context_map_val = 0; find_context_map_val < ARAD_NOF_TDM_CONTEXT_MAP; find_context_map_val++) {
            ...
            /** free tdm context when port it not tdm */
            if (is_tdm == FALSE) {
  ...
  )
{
  ...
  if (is_ingress_enabled) { /* get ingress configuration */
      ...
      if (!SOC_IS_QAX(unit)) {
          ...
          /* In QAX the configuration for enable packet CRC per context is decided
             according the global configuration for CRC enable: PACKET_CRC_ENf */
