  ...
  )
{
  ...
  if (SOC_IS_JERICHO(unit)) {
      ...
      /*
             * In Jericho we need to allocate 8 prefixs for IPV6 64-bit scale route in LEM
             */
      if (SOC_DPP_CONFIG(unit)->pp.enhanced_fib_scale_prefix_length_ipv6_long == 64) {
          ...
          /* 10xxx, take 8 prefix, from 16 to 23 */
  ...
  if (!is_mask) {
      if (SOC_DPP_CONFIG(unit)->pp.l3_source_bind_mode != SOC_DPP_L3_SOURCE_BIND_MODE_DISABLE &&
       ...
       if (SOC_IS_JERICHO(unit)) {
        ...
        SOC_SAND_CHECK_FUNC_RESULT(res, 21, exit);
        /* For IPv6 64 in LEM, prefix values are 16-23, only 2msbs are set */
        tmp = 0x2;
      }
      else {
  ...
  )
{
  ...
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_SA_AUTH:
    ...
    *   ILM
    */
  /*Correspondence with qualifier,separate 20 bits label to 16+4, change number of param 5*/
  case ARAD_PP_LEM_ACCESS_KEY_TYPE_PREFIX_ILM:
    ...
    )
{
    ...
    /* 
     *    when payload is not empty pointer - convert payload 
     */
    if (NULL != payload)
    {
        /*
  ...
  )
{
  ...
  if (SOC_PPC_FRWRD_MACT_LEARN_LIMIT_MODE == SOC_PPC_FRWRD_MACT_LEARN_LIMIT_TYPE_LIF) {
    ...
    /* In remove procedure access flag is not needed */
