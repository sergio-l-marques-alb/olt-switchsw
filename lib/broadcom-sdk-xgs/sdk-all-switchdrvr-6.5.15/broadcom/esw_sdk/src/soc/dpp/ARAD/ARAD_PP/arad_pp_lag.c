...
/* { */
/*
 * The port profile bit is the MSB of the PFC profile (8b) address.
 * This enumeration indicate it supposed to be on, off or configured for the two cases.
 */
typedef enum
{
 /*
  ...
  */
  ...
  /*
   * set this LB vector for the two case of the port profile (on and off) as this PFC profile
   * don't care about the profile state.
   */
  ARAD_PP_PORT_PROFILE_DC
} ARAD_PP_PORT_PROFILE_STATE;

/*
 * The PLC is a four bit value which mapped into a single bit that set the PFC profile entry second MSB bit.
 */
typedef enum
{
 /*
  * plc profile bit is iff
  */
  ARAD_PP_PLC_PROFILE_OFF = 0 ,
 /*
  * plc profile bit is on
  */
...
/*
 * Version 6.5.8 introduce a new scheme of LB that provides a better coverage of hashing
 * scenarios (triggered by the parser_mode SOC property).
 * Below are the new scheme vectors.
 */
CONST STATIC ARAD_PP_LB_HASH_MAP
...
/*
 * Trill isn't part of the default PFCs that are loaded in the new parser mode so when the new LB scheme is
 * used (new parser mode on) the trill vector will be loaded separately according to the device user configuration.
 */
CONST STATIC ARAD_PP_LB_HASH_MAP
...
/*
 * FCOE isn't part of the default PFCs that are loaded in the new parser mode so when the new LB scheme is
 * used (new parser mode on) the trill vector will be loaded separately according to the device user configuration.
 */
CONST STATIC ARAD_PP_LB_HASH_MAP
  ...
  )
{
  ...
  /*
   * In order to match different PFC profiles to  ports with BOS search enable the LB profile
   * (which affect the PFC profile selection) will be configured according to the BOS search configuration.
   */
...
uint32
    ...
    {
        /*Get register which holds the bit to enable ECMP combined hashing*/
...
/*
 * This function collects all the load balancing information
 * (signals, registers and memories) for the diagnostic display.
 */
soc_error_t
  ...
  )
{
    ...
    for(core_id = signals->first_core; core_id <= signals->last_core ; core_id++)
    {
        ...
        } else { /* signals->diag_type == ARAD_PP_DIAG_LAG */
            /* The only way that we can check that the LAG is valid is by the in_port */
