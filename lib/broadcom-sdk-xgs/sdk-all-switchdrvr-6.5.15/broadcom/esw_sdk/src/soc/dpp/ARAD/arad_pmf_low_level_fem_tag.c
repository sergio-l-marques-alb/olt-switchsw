...
static 
   uint32  /* action name, hw-id, size*/
       Jericho_pmf_fem_action_type_encoding[SOC_PPC_NOF_FP_ACTION_TYPES][NUM_ACTION_ELEMENTS] = {
           ...
           {SOC_PPC_FP_ACTION_TYPE_COUNTER_A                ,17,  21+1 /* 22+1(valid bit) only for Statistic-Record */ },
           {SOC_PPC_FP_ACTION_TYPE_COUNTER_B                ,18,  21+1 /* 22+1(valid bit) only for Statistic-Record */ },
           {SOC_PPC_FP_ACTION_TYPE_DP_METER_COMMAND         ,19,  2        },
           {SOC_PPC_FP_ACTION_TYPE_SRC_SYST_PORT            ,20,  16       },
           {SOC_PPC_FP_ACTION_TYPE_FWD_CODE                 ,21,  4        },
           {SOC_PPC_FP_ACTION_TYPE_FWD_OFFSET               ,22,  3+6      },
  ...
  *action_lsb_egress = 0;
  ...
  if (found 
      ...
      *action_size = 22; /* Instead of 17 */ 
      ...
      {
          /*In case of ITMH programmable mode, if action is user_header_2, at least 18 bytes should be taken for class set as action argument.*/
          *action_size =
...
#ifdef BCM_88660_A0
  if (SOC_IS_JERICHO(unit)) {
      /* Polarity is set according to ARAD_PP_FP_FEM_ALLOC_FES_POLARITY flag, if flag is set: polarity is 0 */
