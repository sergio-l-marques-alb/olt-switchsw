    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        if (SOC_DPP_CONFIG(unit)->arad->init.ilkn_tdm_dedicated_queuing == ARAD_MGMT_ILKN_TDM_DEDICATED_QUEUING_MODE_ON) {
            ...
            SOC_PBMP_ITER(ports_bm, port_i) {
                ...
                /* Check that for each is_tdm_queuing_on ILKN there is one TDM_QUEUING_ON port
                 */
    ...
    int unit)
{
    ...
    /* Get the SOC Property value for KAPS direct lookup mode
       Default: 0
       The property is valid only for Jericho and above */
    ...
    int unit)
{
    ...
    if (SOC_IS_QUX(unit)) {
        ...
        /* setting QAX ports status scan values */
        /*               bits 9-4 (scan delay); bits 3-1 (scan port delay); bit 0 (enable) */
        /* processor 0 :        9                              5               0          */

        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_LEDUP0_CTRLr(unit, 0x9A));

        /* Required led clock period is 200 ns and switch clock runs at 250MHz.  */
        /* value = Clk freq * (period/2) = (250 * 10^6)*(100 * 10^-9) = 25       */
        rval = 0;
        ...
        /* Required refresh period is 30 ms and switch clock period is ~4ns(250MHz) */
        /* Then the value should be: (30*10^-3)/(4*10^-9) = 7500000 =0x7270E0 Timeout value in switch clocks */
        rval = 0;
        ...
        /* Required led clock period is 200 ns and switch clock runs at 600MHz.  */
        /* value = Clk freq * (period/2) = (600 * 10^6)*(100 * 10^-9) = 60       */
        rval = 0;
        soc_reg_field_set(unit, CMIC_LEDUP0_CLK_DIVr, &rval, LEDCLK_HALF_PERIODf, 60);
        SOCDNX_IF_ERR_EXIT(WRITE_CMIC_LEDUP0_CLK_DIVr(unit, rval));


        /* Required refresh period is 30 ms and switch clock period is ~1.666ns(600MHz) */
        /* Then the value should be: (30*10^-3)/(1.666...*10^-9) = 18000000 =0x112A880 Timeout value in switch clocks */
        rval = 0;
        ...
        if (non_qsgmii_mode) {
             ...
             * COVERITY
             *
             * This section is unreachable but retained for qsgmii mode.
             */
        ...
        /* PMH1 ports chain is connected to LED2, 24 ports */
        /* Phisical ports status chain to LED1 is following, port36 resides in port0 place:*/
        ...
        /* Required led clock period is 200 ns and switch clock runs at 837.5MHz.  */
        /* value = Clk freq * (period/2) = (837.5 * 10^6)*(100 * 10^-9) = 83.75    */
        rval = 0;
        ...
        /* Required refresh period is 30 ms and switch clock period is ~1.194ns(837.5MHz) */
        /* Then the value should be: (30*10^-3)/(1.194...*10^-9) = 25125000 = 0x17F6088 Timeout value in switch clocks */
        rval = 0;
    ...
    int reset_action)
{
...
#else
  #if defined(BCM_QAX_SUPPORT) || defined(BCM_QMX_SUPPORT) || defined(BCM_QUX_SUPPORT) 
    ...
    /* init hostmem address remap allocation database */
#ifdef BCM_CMICM_SUPPORT
       ...
       if (!SAL_BOOT_PLISIM) 
#endif
           {
               ...
               if (bist_enable || tcam_bist_enable) {
                ...
                /* No matter MBIST enalbe, fix QAX_A0 MBIST arm core */
...
/*#endif                 */         /* JERICHO_HW_IMPLEMENTATION */
    ...
    /* set tcam entry cache enable for Jericho after phase1 */
