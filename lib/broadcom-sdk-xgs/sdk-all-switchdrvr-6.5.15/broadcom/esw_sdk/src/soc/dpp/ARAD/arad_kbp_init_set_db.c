    ...
    SOC_SAND_IN  uint32 table_id)
{
    ...
    if (!SOC_WARM_BOOT(unit) && need_to_init_table) {
        ...
        if (!ARAD_KBP_IS_OP || (db_type == KBP_DB_ACL) || (ARAD_KBP_IS_OP && (db_type == KBP_DB_LPM) && (AradAppData[unit]->g_gtmInfo[table_id].tblInfo.tbl_width > NLM_TBL_WIDTH_160))) {
        ...
        if (!ARAD_KBP_IS_OP && (table_id == ARAD_KBP_FRWRD_TBL_ID_IPV4_DC)) {
            res = kbp_db_set_property(AradAppData[unit]->g_gtmInfo[table_id].tblInfo.db_p,
        ...
        if (!ARAD_KBP_IS_OP && (table_config_info.min_priority >= 0)) {
            res = kbp_db_set_property(AradAppData[unit]->g_gtmInfo[table_id].tblInfo.db_p, 
    ...
    int unit)
{
    ...
    if (SOC_IS_ARADPLUS(unit)) {
        ...
        for (i = 0; i < ARAD_KBP_FRWRD_IP_NOF_TABLES_ARAD_PLUS; i++) {
            if(ARAD_KBP_IS_OP && ARAD_KBP_24BIT_FWD && !ARAD_KBP_ENABLE_IPV4_DC) {
   ...
   int core)
{
    ...
    if (!ARAD_KBP_IS_OP && SOC_IS_ARADPLUS_AND_BELOW(unit)){
    ...
    ARAD_INIT_ELK *elk)
{
    ...
    if (!SOC_WARM_BOOT(unit) && ARAD_KBP_IS_OP && (elk->is_master || (elk->kbp_connect_mode == ARAD_NIF_ELK_TCAM_CONNECT_MODE_DUAL_SMT))) {
        for(db_type = 0; db_type < ARAD_KBP_MAX_NUM_OF_FRWRD_DBS; db_type++) {
            uint32 srch_ndx;
            arad_kbp_lut_data_t kbp_gtm_lut_info = {0};
            int num_of_kbp_searches = ARAD_KBP_NUM_OF_KBP_SEARCHES(unit);
    ...
    ARAD_INIT_ELK   *elk_ptr)
{
    ...
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "ext_flexible_mode", 0)) {
        kbp_dev_type = soc_property_get_str(unit, spn_EXT_TCAM_DEV_TYPE);
        is_kbp = kbp_dev_type ? (!(sal_strcmp("BCM52311", kbp_dev_type)) ? 1 : 0) : 0;
        if (is_kbp != 1) {
        LOG_ERROR(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                        "Error in %s(): ext_tcam_dev_type has to be BCM52311\n"),
                        FUNCTION_NAME()));
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 10, exit);
        }
    }
    ...
    int     no_sync_flag)
{
    ...
    if(NULL == AradAppData[unit])
    {
        ...
        if (elk->is_master) {
            if (AradAppData[unit]->g_gtmInfo != NULL) {
