  ...
  )
{
  ...
  protection_pointer = (pwe_info->protection_pointer == 0 && SOC_IS_JERICHO(unit)) ? -1 : pwe_info->protection_pointer;
    ...
    /*
     * First to write lif table, then ISEM. It will make sure LIF info is ready first, then lif hit condition.
     * otherwise learn infomation will be incorrect under traffic if lif entry isn't clean(entry is used before, now allocated this time).
     */
  ...
  )
{
  ...
  if (ac_key->key_type == SOC_PPC_L2_LIF_AC_MAP_KEY_TYPE_PORT_VLAN_ETH)
  {
    ...
    sem_key->key_info.section_oam.port = ac_key->vlan_domain; /* In-PP-Port */
                                          ...
                                          SOC_SAND_OUT SOC_PPC_FP_QUAL_VAL qual_vals[])
{
    ...
    { /*sem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_CUSTOM_PP_PORT_TUNNEL */
        if (ac_key->outer_vid > 0) {
            ...
            qual_vals[0].is_valid.arr[0] = 0x8001FFFF; /*Bits 31, bits0:16 valid*/
  ...
  )
{
  ...
  if (lif_index != NULL) {
      /*in some error case, input lif index does not equal to removed lif index.
	      in this case, should remove the input lif index from SW DB*/
      if (*lif_index == 0 ) {
  ...
  )
{
  ...
  /* The ISID MP type field was changed in Jericho and above devices */
      ...
      if (found)
      {
        *lif_index = arad_pp_sem_result_to_lif_index(unit, sem_entry.sem_result_ndx);
        ...
        if (!(isem_only)) {
            ...
            if (type == isid_mp_lif_type) { /* ISID MP */
                do
                {
                  ARAD_PP_IHP_LIF_TABLE_ISID_MP_TBL_DATA
                    data;
  ...
  if (SOC_IS_JERICHO(unit))
  {
    if (is_ingress) {
        ...
        if (is_clear)
        {
            ...
            {
                /*
                 * Option 1 - 2 wide data InLIFs
                 * 1st wide data InLIF takes entry n + half entry n+1(MSB)
                 * 2nd wide data InLF takes half entry n+1(LSB) + entry n+2
                 *        bank m even                                                 bank m odd
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                    entry n (normal)            | entry n+1(1st wide data)| entry n+1(2nd wide data)|
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                  entry n+2 (normal)            |               entry n+2(reserved)                 |
                 * -----------------------------------------------------------------------------------------------------
                 *
                 * Option 2 - 1 wide data InLIF + 1 noraml InLIF
                 *        bank m even                                                 bank m odd
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                    entry n (normal)            | entry n+1(1st wide data)| entry n+1(reserved)    |
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                  entry n+2 (normal)            |               entry n+2(reserved)                |
                 * -----------------------------------------------------------------------------------------------------
                 *
                 * Option 3 - 1 wide data InLIF
                 *        bank m even                                                 bank m odd
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                    entry n (normal)            | entry n+1(1st wide data)| entry n+1(reserved)    |
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                  entry n+2 (reserved)            |               entry n+2(reserved)              |
                 * -----------------------------------------------------------------------------------------------------
                 *
                 * entry n is base_lif_id, entry n+1 is ext_lif_id for 1st InLIF
                 */
            ...
            else
            {
                 ...
                 * -----------------------------------------------------------------------------------------------------
                 *
                 * Option 2 - 1 noraml InLIF + 1 wide data InLIF. It's due to create 2 wide data InLIFs,
                 *   replace 1st wide data InLIF as normal LIF/remove 1st wide data InLIF and the entry is taken as normal InLIF, 
                 *
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                    entry n (normal)            | entry n+1(reserved)| entry n+1(2nd wide data)|
                 * ----------------------------------------------------------------------------------------------------- 
                 * |                  entry n+2 (normal)            |               entry n+2(reserved)                 |
                 * -----------------------------------------------------------------------------------------------------
                 *

                 * entry n+2 is base_lif_id, entry n+1 is ext_lif_id for 2nd InLIF
                 */
                res = sw_state_access[unit].dpp.bcm.alloc_mngr_local_lif.inlif_info.data_base.get(unit, (inlif_info.ext_lif_id - 1), &inlif_info_tmp);
                SOCDNX_IF_ERR_EXIT(res);
            }
                ...
                ((inlif_info_tmp.local_lif_flags & BCM_DPP_AM_IN_LIF_FLAG_WIDE) == 0))
            {
                ...
                if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "inlif_additional_data_lock_bit_enable", 0)) {
                    /* The inlif additional data loc bit is bit 56 for LSB and bit 113 for MSB. */
  ...
  if (SOC_IS_JERICHO(unit))
  {
      if (is_ingress) {
          ...
          if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "inlif_additional_data_lock_bit_enable", 0) && (tbl_data.double_data_entry == TRUE)) {
              /* The bits 57 or 113 is lock bit. */
              num_bits -= 1;
          }
