  ...
  )
{
  ...
  if (SOC_IS_QAX(unit)) { /* no cores */
          uint32 reg_32;
          res = READ_IPS_FMC_QNUM_RANGEr(unit, &reg_32);
          SOCDNX_IF_ERR_EXIT(res);

          soc_reg_field_set(unit, IPS_FMC_QNUM_RANGEr, &reg_32, FMC_QNUM_LOWf, queue_range->start);
          soc_reg_field_set(unit, IPS_FMC_QNUM_RANGEr, &reg_32, FMC_QNUM_HIGHf, queue_range->end);

          res = WRITE_IPS_FMC_QNUM_RANGEr(unit, reg_32);
          SOCDNX_IF_ERR_EXIT(res);
  } else {
  ...
  )
{
  ...
  if (SOC_IS_QAX(unit)) { /* no cores */
      ...
      queue_range->start = soc_reg_field_get(unit, IPS_FMC_QNUM_RANGEr, reg_32, FMC_QNUM_LOWf);
      queue_range->end = soc_reg_field_get(unit, IPS_FMC_QNUM_RANGEr, reg_32, FMC_QNUM_HIGHf);
  } else {
      if (core_id == 0 || core_id == SOC_CORE_ALL) {
          res = READ_ECI_GLOBAL_FMC_0r(unit, &reg_value);
          SOCDNX_IF_ERR_EXIT(res);
