...
/* compute the database and offset from key
   ...
   Jericho_plus:
   key [20:16]: BB or BBS (database ID)
   key [15:2]: offset in BB or BBS
   key [1:0]: 32bits in entry
   QAX:
   key [17:14] BB
   key [13:2]: offset in BB
...
#define JER_KAPS_KEY_INFO_DECODE(_unit, _key, _db_id, _offset, _word_off) \
    do { \
        if (SOC_IS_JERICHO_PLUS_ONLY(unit)) { \
            ...
            SHR_BITCOPY_RANGE(&db_id, 0, &key, 12, 4); \
    ...
    if (!is_update) {
        ...
        if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
            ...
            if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                kbp_db_delete_entry(db_p, db_entry); /* rollback */
                kbp_ad_db_delete_entry(ad_db_p, ad_entry); /* rollback*/
                kbp_hb_db_delete_entry(hb_db_p, hb_entry); /* rollback*/
                *success = SOC_SAND_FAILURE_INTERNAL_ERR;
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
            }
        }
      ...
      )
{
    ...
    /*hit_bit in input is considered clear on read*/
                           ...
                           SOC_SAND_OUT   uint8 *data_bytes)
{
    ...
    {  
        if (qual_nof_bits[qual_idx] > bit_offset)
        {
            ...
            data[data_idx] |= val; /* write what you can in this uint64 */
            data_idx++;
        ...
        data[data_idx] |=  val; /* write the qual value in place */
    }
                                      ...
                                      SOC_SAND_OUT JER_KAPS_IP_TBL_ID       *kaps_table_id)
{
    ...
    if (qual_vals == NULL) {
        ...
        /* find the inrif in qual_vals to distinguish between with inrif and without inrif in private tables */
        for (i = 0; i < SOC_PPC_FP_NOF_QUALS_PER_DB_MAX; i++) {
        ...
        /* if inrif is not found, then it belongs in the no inrif table */
        if (i == SOC_PPC_FP_NOF_QUALS_PER_DB_MAX) {
            inrif = 0;
        }
    }

    /* If KBP is enabled and is OP, KBP default routes in KAPS should be in public DB,
     * when the corresponding public program is enabled, or in private DB in all other cases.
     * Do not change vrf if public program is enabled or set vrf != 0 to retrieve private kaps_table_id.
     */
    if(ARAD_KBP_IS_OP) {
        switch(table_id) {
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV4UC_KBP_DEFAULT_ROUTE_KAPS: /* Select private if IPv4 UC Public is disabled */
            if(!ARAD_KBP_ENABLE_IPV4_UC_PUBLIC) {
                vrf = 1;
            }
            break;
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6UC_KBP_DEFAULT_ROUTE_KAPS: /* Select private if IPv6 UC Public is disabled */
            if(!ARAD_KBP_ENABLE_IPV6_UC_PUBLIC) {
                vrf = 1;
            }
            break;
        default: /* Select private in all other cases */
            vrf = 1;
        }
    }
    /* If KBP is enabled and is not OP, KBP default routes in KAPS should all be in private DB.
     * Set vrf != 0 to retrieve private kaps_table_id.
     */
    ...
    if (((vrf == 0) && JER_KAPS_ENABLE_PUBLIC_DB(unit)) || !JER_KAPS_ENABLE_PRIVATE_DB(unit)) {
        ...
        case SOC_DPP_DBAL_SW_TABLE_ID_IPV6MC:
            *kaps_table_id = JER_KAPS_IP_CORE_0_PUBLIC_IPV6_MC_TBL_ID;
            break;
        default: /* Dynamic tables are mapped to IPV4 UC or IPV4 MC */
    ...
    /* Inrif determin the public/private table for IPV4MC in ipmc_lookup_model ==1*/
        ...
        for (i=0; i<2; i++) {
            if (((i==1) && (JER_KAPS_ENABLE_PRIVATE_DB(unit))) || ((i==0) && (JER_KAPS_ENABLE_PUBLIC_DB(unit)))){
                ...
                if ((dbal_table.db_prefix_len > JER_KAPS_TABLE_PREFIX_LENGTH) || /* These are dynamic table, need to check prefix before removal */
                    (dbal_table.additional_table_info != 0)) { /*The KBPSDK does not support removing all the hitbits associated with a single table*/
                             ...
                             SOC_SAND_IN    SOC_PPC_RIF_ID    inrif)
{
    ...
    if (inrif == 0) {
        if (!JER_KAPS_ENABLE(unit)) {
            /* inrif 0 should be retrieved from the public DB */
            /* If the public DB is disabled, inrif==0 is treated as any other inrif */
            res = SOC_SAND_ERR;
            SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
        }
    } else {
        if (!JER_KAPS_ENABLE_PRIVATE_DB(unit)) {
            /* inrif !=0 should be retrieved from the private DB */
            res = SOC_SAND_ERR;
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        }
    }
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE   *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    /* Checks if the action is 64 bit */
    if (mem_len == JER_KAPS_DMA_BUFFER_NOF_BYTES) {
        /* Checking if the LSB = 1, and returning a warning*/
        if (word_off & 1) {
    ...
    if(SOC_IS_JERICHO_PLUS_ONLY(unit)) {
        /* Different logic for Jericho plus, where the entry is not byte-aligned*/
        int zone_size; /* Bits in a single zone */
        int nof_words; /* Total number of zones the buffer can be split into*/
        int bits_diff; /* bits of byte-misalignment ( bits missing to form a word ) of a single entry */
        int shift; /* Bits to shift SRC data within each DST byte */
        uint8 mask; /* Mask applied to split each byte into 2 iterations */
        int src_byte_idx; /* Byte within source buffer to copy from */
        int dst_byte_idx; /* Byte within result buffer to write to */
        /* For Jericho_plus, we have a buffer size of 120 bits, separated into 4 zones of 30 bits.
         * Each entry can occupy either one or two consequitive zones (zones 0+1 or 2+3).
         * The byte misalignment is 2 bits.
         * 
         * Values of the variables are calculated according to the zone the entry is in (word_off)
         *
         * 
         * Zone 0: 
         * /-------- dst_byte 11--------\/-------- dst_byte 12--------\/-------- dst_byte 13--------\/-------- dst_byte 14--------\/-------- dst_byte 15--------\
         * |____________________________/\____________________________/\____________________________/\____________________________/\____________________________/ 
         * |      | src_byte 0  (6b)     |   src_byte 1  (8b)          |   src_byte 2  (8b)          |   src_byte 3  (8b)          | shift                      |
         * \______/\_____________________/\____________________________/\____________________________/\____________________________/\___________________________/
         *
         * shift = 8 bits
         * mask = 11111111
         * updated (MSB) mask = 00111111
         * 
         * 
         * Zone 1: (only for single-zone entry)
         * /-------- dst_byte 7 --------\/-------- dst_byte 8 --------\/-------- dst_byte 9 --------\/-------- dst_byte 10--------\/-------- dst_byte 11--------\
         * |____________________________/\____________________________/\____________________________/\____________________________/\____________________________/ 
         * |              | src_byte 0  (6b)     |   src_byte 1  (8b)          |   src_byte 2  (8b)          |   src_byte 3  (8b)          | shift              |
         * \______________/\_____________________/\____________________________/\____________________________/\____________________________/\___________________/
         *
         * shift = 6 bits
         * mask = 00111111
         * updated (MSB) mask = 000001111
         * 
         * 
         * Zone 2:
         * /-------- dst_byte 3 --------\/-------- dst_byte 4 --------\/-------- dst_byte 5 --------\/-------- dst_byte 6 --------\/-------- dst_byte 7 --------\
         * |____________________________/\____________________________/\____________________________/\____________________________/\____________________________/ 
         * |                      | src_byte 0  (6b)     |   src_byte 1  (8b)          |   src_byte 2  (8b)          |   src_byte 3  (8b)          | shift      |
         * \______________________/\_____________________/\____________________________/\____________________________/\____________________________/\___________/
         *
         * shift = 4 bits
         * mask = 00001111
         * updated (MSB) mask = 000000011
         * 
         * 
         * Zone 3: (only for single-zone entry)
         * /-------- dst_byte 0 --------\/-------- dst_byte 1 --------\/-------- dst_byte 2 --------\/-------- dst_byte 3 --------\
         * |____________________________/\____________________________/\____________________________/\____________________________/ 
         * |   src_byte 0 (6b)    |   src_byte 1   (8b)        |     src_byte 2  (8b)        |   src_byte 3   (8b)         | shift|
         * \______________________/\___________________________/\____________________________/\____________________________/\_____/
         *
         * shift = 2 bits
         * mask = 00000011
         * updated (MSB) mask = 000000000 <-- not used, here no writes to a previous destination byte are done
         * 
         * Below calculations are done to fit the value according to zone:
         */
        nof_words = 4;
        ...
        mask = (1 << shift) - 1;
        /* write start byte = LSB byte of the result zone to start with*/
        dst_byte_idx = BITS2BYTES(JER_KAPS_DMA_DB_WIDTH(unit) - (word_off * zone_size) + 1) - 1;
        /* read start byte = LSB byte of the input data*/
        src_byte_idx = mem_len - 1;
        /* Transfer the input data (without byte 0)*/
        while(src_byte_idx) {
            /* Write the LSBs to the MSB-side of result byte */
            entry_data[dst_byte_idx] &= mask;
            entry_data[dst_byte_idx] |= data[src_byte_idx] << shift;
            /* Write the remaining MSBs to the LSB-side of next result byte */
            dst_byte_idx--;
            entry_data[dst_byte_idx] &= ~mask;
            entry_data[dst_byte_idx] |= data[src_byte_idx] >> (8 - shift);
            /* Go to the next input data byte*/
            src_byte_idx--;
        } /* while(data_byte)*/
        /* Transfer remaining input data byte 0 (treat it differently as it is 2b shorter) */
        entry_data[dst_byte_idx] &= mask;
        entry_data[dst_byte_idx] |= data[src_byte_idx] << shift;
        if(dst_byte_idx > 0) {
            dst_byte_idx--;
            /* Change the mask for the last byte (src_byte 0), as it is taking 2 bits less*/
            mask = (1 << (shift - bits_diff)) - 1;
            entry_data[dst_byte_idx] &= ~mask;
            entry_data[dst_byte_idx] |= (data[src_byte_idx] >> (8 - shift)) & mask;
        }
    } else { /* SOC_IS_JERICHO_PLUS_ONLY */
        ...
        if (SOC_IS_QUX(unit)) {
            ...
            /* For QUX, we have a buffer size of 120 bits, separated into 4 zones. 
             * Zone 0 : 32 bits
             * Zone 1 : 32 bits
             * Zone 2 : 32 bits
             * Zone 3 : 24 bits
    ...
    /* Wait the entry really set into the hardware. */
    res = jer_pp_xpt_wait_dma_done(unit);
    if(ARAD_KBP_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        *success = SOC_SAND_FAILURE_INTERNAL_ERR;
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 70, exit);
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    if(SOC_IS_JERICHO_PLUS_ONLY(unit)) {
        ...
        /* In case we want to write 64 bit, we need to move
         * the indexer 4 bytes backwards (because of endianity, writing 64 bits) */
        if (size == JER_KAPS_DMA_BUFFER_NOF_BYTES) {
            mem_off -= (JER_KAPS_DMA_BUFFER_NOF_BYTES/2);
        }
