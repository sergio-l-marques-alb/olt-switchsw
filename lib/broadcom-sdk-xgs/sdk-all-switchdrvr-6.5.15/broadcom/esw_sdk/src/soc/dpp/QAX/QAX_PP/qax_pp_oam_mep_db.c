...
/*
 * Read 1 DM data and clears it from the reg_data buffer ('clear on
 * read'). The reg_data should be written back after this function
 */
STATIC
...
soc_error_t soc_qax_pp_oam_oamp_lm_dm_set(int unit,
    ...
    case ARAD_PP_OAM_OAMP_MEP_DB_LM_DM_ACTION_ADD_DM_1WAY:
        ...
        /* Setting LAST entry indication so that no more LM/DM entries present */
    ...
    /* if piggyback is defined in case of double pointer injection we need to stamp PWE counter on CCM,
     * so we use default mep_pe_profile(in case the group is not maid_48) and maid_48 profiles(in case the group is maid_48)*/
    ...
    if((lm_dm_info->out_lif != 0) && (lm_dm_info->mep_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) && (lm_dm_mep_db_entry->is_piggyback_down)) {
        ...
        /* piggyback - stamp PWE counter on CCM */
        ...
        /*If the group is maid_48 ,the selected program is ARAD_PP_OAMP_PE_PROGS_MAID_48 */
        ...
        do
        {
            ...
            /*  On rare occasions writing to the HW fails due to unknown reasons. Reading the MEP-DB to verify write and if not trying again.
             *         */
...
soc_error_t soc_qax_pp_oam_oamp_lm_dm_delete(int unit,
    ...
    /*Read the MEP_PE_Profile*/
    soc_mem_field_get(unit, mem, reg_data, MEP_PE_PROFILEf, &mep_pe_profile);

    if(SOC_IS_QAX(unit)){
        /*Get the mep-pe-profile value of ARAD_PP_OAMP_PE_PROGS_SLM_DOWN program*/
        arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_SLM_DOWN, &mep_pe_profile_slm);

         /*Get the mep-pe-profile value of ARAD_PP_OAMP_PE_PROGS_SLM_POINTER program*/
        arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_SLM_POINTER, &mep_pe_profile_slm_pointer);

        /*Get the mep-pe-profile value of ARAD_PP_OAMP_PE_PROGS_SLM_MAID_48 program*/
        arad_pp_oamp_pe_program_profile_get(unit, ARAD_PP_OAMP_PE_PROGS_SLM_MAID_48, &mep_pe_profile_slm_48);

        /*Check the value of mep-pe-profile(Check SLM indication).
          Only if the mep-pe-profile is ARAD_PP_OAMP_PE_PROGS_SLM_DOWN or ARAD_PP_OAMP_PE_PROGS_SLM_MAID_48 unset the slm bit(the forth bit of mep-pe-profile).
          This is relevant only for QAX*/
        if(mep_pe_profile == mep_pe_profile_slm || mep_pe_profile == mep_pe_profile_slm_48 || mep_pe_profile == mep_pe_profile_slm_pointer){
            /* Unsetting the SLM bit in MEP PE profile, the previous mep-pe-profile is set*/
            reg_val = mep_pe_profile & (~(1 << ARAD_PP_OAMP_MEP_PE_PROFILE_SLM_OFFSET));
            /*Set the new mep-pe-profile value*/
            soc_mem_field_set(unit, mem, reg_data, MEP_PE_PROFILEf, &reg_val);
        }
    }
     /* Check if endpoint related to CCM counting or Egress Injection Feature*/
    if (SOC_IS_QAX(unit) && lm_dm_info->out_lif != 0) {

        /*Init the variable is_slm by checking the second bit of the mep_pe_profile
          The second bit is responsible for SLM indication in MEP_PE_PROFILE in case CCM counting or Egress Injection Feature*/
        is_slm = mep_pe_profile & (1 << ARAD_PP_OAMP_MEP_PE_PROFILE_COUNT_SLM_OFFSET);

        /*In case of SLM - unset the second bit of the mep_pe_profile*/
        if(is_slm) {

            /*ARAD_PP_OAMP_PE_PROGS_SLM_EGR_INJ->ARAD_PP_OAMP_PE_PROGS_EGR_INJ*/
            reg_val = mep_pe_profile & (~(1 << ARAD_PP_OAMP_MEP_PE_PROFILE_COUNT_SLM_OFFSET)); /* Unsetting the SLM bit in MEP PE profile for Egress Injection or CCM counting*/

            soc_mem_field_set(unit, mem, reg_data, MEP_PE_PROFILEf, &reg_val);
        }
    }
    /*Check if it is Double Pointer feature*/
    if( lm_dm_info->out_lif != 0 && lm_dm_info->mep_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE && (!is_slm)){

        /*Set the mep_pe_profile only in case of double_pointer feature */
        if(lm_dm_info->is_maid_48) {
            /*In case the group is maid_48 set ARAD_PP_OAMP_PE_PROGS_CCM_COUNT_48_MAID_QAX program, otherwise ARAD_PP_OAMP_PE_PROGS_CCM_COUNT_QAX */
...
qax_pp_oam_bfd_flexible_verification_set(int unit, SOC_PPC_OAM_BFD_FLEXIBLE_VERIFICATION_INFO *info)
{
    ...
    if (info->crc_tcam_index != -1) {
        ...
        /* The table OAMP_CLS_FLEX_CRC_TCAM has been changed to dynamic table in QUX. */
        if (SOC_IS_QUX(unit)) {
