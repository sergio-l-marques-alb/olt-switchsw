...
qux_scrub_read_write_back(int unit, int block_instance, uint32 en_interrupt, char *msg)
{
    ...
    switch (en_interrupt) {
        ...
        case QUX_INT_KAPS_KAPS_ERROR_REGISTER_TECC_ERROR_1:
        {
            ...
            SOCDNX_IF_ERR_EXIT(rc);
            rc = soc_reg32_get(unit, KAPS_RPB_TCAM_SCAN_STATUSr, block_instance, index, &regVal);
            SOCDNX_IF_ERR_EXIT(rc);
            ...
            if (invalidate_n) {
                /* kaps error on multi-bit, invalidate it */
                invalidated_entry = soc_reg_field_get(unit, KAPS_RPB_TCAM_SCAN_STATUSr, regVal, RPB_TCAM_INVALIDATED_OFFSET_Nf);
            }
            if (corrected_n) {
                corrected_entry = soc_reg_field_get(unit, KAPS_RPB_TCAM_SCAN_STATUSr, regVal, RPB_TCAM_CORRECTED_OFFSET_Nf);
            }
