...
int soc_jer_init_sequence_phase1(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
       ...
       LOG_INFO(BSL_LS_SOC_INIT, (BSL_META_U(unit, "\t+ %d: Configure tables defaults\n"),unit));
        ...
        if(!SOC_IS_FLAIR(unit)) {
            etpp_pipe_len = 0x28;    /* set to 40 instead 30 */
        ...
        /* disable EPNI commands to CRPS before engines are configured */
    ...
    else
    {
      ...
      if (SOC_DPP_CONFIG(unit)->arad->init.drc_info.enable) {
          /* Init Dram shmoo for mini shmoo */
          SOCDNX_IF_ERR_EXIT(soc_dpp_drc_combo28_dram_init(unit, &SOC_DPP_CONFIG(unit)->arad->init.drc_info));
      }
    }
#if defined(INCLUDE_KBP)
    ...
    if (SOC_IS_QUX(unit) && !SOC_WARM_BOOT(unit))
    {
        ...
        /* 
         * Read these registers in order for future reads to be correct 
         * (the registers set to maximum on read, but first reads are always 0). 
         */
        SOCDNX_IF_ERR_EXIT(READ_CGM_SRAM_BUFFERS_FREE_MIN_STATUSr(unit, &rval));
        SOCDNX_IF_ERR_EXIT(READ_CGM_SRAM_PDBS_FREE_MIN_STATUSr(unit, &rval));
        SOCDNX_IF_ERR_EXIT(READ_CGM_DRAM_BDBS_FREE_MIN_STATUSr(unit, &rval));			
        SOCDNX_IF_ERR_EXIT(READ_DDP_FBC_CACHE_STATUSr(unit, &r64));
        SOCDNX_IF_ERR_EXIT(READ_DQM_FREE_BDBLL_MIN_SIZEr(unit, &rval));
        SOCDNX_IF_ERR_EXIT(READ_ECGM_CGM_DYN_TH_DEBUGr(unit, reg_val_64));
        ...
        SOCDNX_IF_ERR_EXIT(READ_SPB_PEC_STATUSr(unit, reg_val_64));
