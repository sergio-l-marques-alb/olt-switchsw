  ...
  )
{
  ...
  if (SOC_SAND_GET_BIT(sem_tbl_data.sch_enable,idx) != bit_val_to_set)
  {
    ...
    /* setting max burst to 0 before SEM access (disable the element) */
    flow_id = arad_sch_se2flow_id(se_ndx);

    /* getting number of subflows to check if flow is composite */
    ...
    /* setting max burst to 0 , saving previous value */
    res = arad_sch_flow_max_burst_save_and_set(unit, core, flow_id, 0/*new max*/, &old_max_burst);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    /* in case we have composite flow, set the other flow burst as well to 0 before editing SEM */
    if (fsf_nof_subflows == 2) 
    {
        ...
        /* setting second flow max burst to 0 , saving previous value */
        res = arad_sch_flow_max_burst_save_and_set(unit, core, flow_id_2, 0/*new max*/, &old_max_burst2);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }
    ...
    /*
    * write old max burst to SHDS table (enable the flow)
    */
    res = arad_sch_flow_max_burst_save_and_set(unit, core, flow_id, old_max_burst/*new max*/, &temp_max);
    SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);

    /* if composite element, write old max burst as well (enable the flow )*/
    if (fsf_nof_subflows == 2) 
    {
        res = arad_sch_flow_max_burst_save_and_set(unit, core, flow_id_2, old_max_burst2/*new max*/, &temp_max);
        SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
    }
