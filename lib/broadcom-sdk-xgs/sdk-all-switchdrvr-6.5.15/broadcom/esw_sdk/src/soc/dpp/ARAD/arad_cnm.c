/*
 * $Id: arad_cnm.c,v 1.16 Broadcom SDK $
 *
  ...
  )
{
  ...
  /* CPQ Index is zero-based, so max value is ARAD_CNM_MAX_CP_QUEUES-1 */
  if(cpq_ndx >= ARAD_CNM_MAX_CP_QUEUES)
  {
    SOC_SAND_SET_ERROR_CODE(ARAD_CNM_ILLEGAL_CP_QUEUE_INDEX, 10, exit);
  }

  /* Profile index is zero based, so max value is ARAD_CNM_NOF_PROFILES-1  */
  ...
  )
{
  ...
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  32,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, EGQ_CNM_GENERAL_CONFIGr, SOC_CORE_ALL, 0, CNM_ETHERNET_TYPE_CODEf,  9)); /*pointer to ETH_TYPE_2*/
  SOC_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, IPT_CNM_ETHER_TYPEr, REG_PORT_ANY, 0, CNM_ETHER_TYPEf,  info->ether_type)); /* must match ETH_TYPE_2*/
  ...
  /* Handle OTM port TYPE */
  if(info->ftmh_otm_port == -1)
  {
    /* OTM taken from IFP mapping */
