  ...
  )
{
    ...
    /* Select ILKN-Port1 as the source of ILKN-RX-1 */
  ...
  )
{
    ...
    for(interface_number = 0; interface_number < SOC_TMC_FC_NOF_OOB_IDS; interface_number++)
    {
        ...
        if(tm->fc_oob_mode[interface_number] & SOC_DPP_FC_CAL_MODE_RX_ENABLE)
        {
            ...
            /* Swapping option between INB IF and OOB IF - only relevant to ARAD */
  ...
  )
{
    ...
    /* get mapping from CNM messages to LLFC { */
    ...
    SOC_SAND_OUT  uint32                           *is_valid)
{
    ...
    /*
     *  For Arad, pfc generic bitmap are shared by nif, oob, ilkn_inb, mub
     *  Always return True.
     */
    *is_valid = 1;
    ...
    SOC_SAND_OUT  uint32                            is_set)
{
    ...
    /* Nothing need to be done for Arad*/
  ...
  )
{
    ...
    /* configure HCFC OOB error setting */
    ...
    /* enable/disable HCFC OOB watchdog */
    ...
    /* configure HCFC OOB error status */
    ...
    /* set HCFC watchdog period(unit: clock) */
  ...
  )
{
    ...
    /* get if HCFC OOB watchdog is enabled */
    res = READ_CFC_HCFC_OOB_RX_WD_EN_CFGr(unit, &reg32_data);
    SOCDNX_IF_ERR_EXIT(res);
    ...
    /* get HCFC watchdog period(unit: clock) from first channel */
    SOC_REG_ABOVE_64_CLEAR(reg64_data);
    reg = (if_ndx == SOC_TMC_FC_OOB_ID_A)? CFC_HCFC_OOB_RX_0_WD_PERIOD_CFGr : CFC_HCFC_OOB_RX_1_WD_PERIOD_CFGr;
      ...
      )
{
    ...
    /* 2. setting port config - port level */
  ...
  )
{
    ...
    case ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS:
        ...
        /* No missing break. It is on purpose*/
