  ...
  )
{
    ...
    if (SOC_IS_KALIA(unit) || is_mesh || soc_feature(unit, soc_feature_packet_tdm_marking)) {
        ...
        {
            val = is_mesh_hybrid? 8 : 16;
        }
        SOC_DPP_DEFS_SET(unit, nof_fabric_links, val);

        SOC_DPP_DEFS_SET(unit, nof_instances_fmac, (is_mesh_hybrid? 2 : 4));
        SOC_DPP_DEFS_SET(unit, first_fabric_link_id, 0);
        SOC_DPP_DEFS_SET(unit, nof_sch_active_links, (is_mesh_hybrid? 8 : 16));
        SOC_DPP_DEFS_SET(unit, nof_logical_ports, (is_mesh_hybrid? 520 : 528));
        ...
        SOC_DPP_IMP_DEFS_SET(unit, rci_threshold_nof_congested_links, is_mesh_hybrid? 5 : 8);
    }
  ...
  )
{
    ...
    if (SOC_DPP_CONFIG(unit)->arad->init.fabric.connect_mode == ARAD_FABRIC_CONNECT_MODE_SINGLE_FAP 
         && !soc_feature(unit, soc_feature_packet_tdm_marking)) { /*Fabric DTQs hold traffic to go to FE, so not configured in single FAP*/
        num_of_DTQs = 0;
