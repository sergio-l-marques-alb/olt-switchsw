  ...
  )
{
  ...
  if (bw_profile_info->is_sharing_enabled) {
      ...
      if (bw_profile_info->disable_cir && bw_profile_info->disable_eir 
            && SOC_DPP_CONFIG(unit)->meter.meter_min_rate == bw_profile_info->max_cir 
            && SOC_DPP_CONFIG(unit)->meter.meter_min_rate == bw_profile_info->max_eir) 
      {
            max_cir_mantissa = 0;
            max_cir_mantissa_exponent = 0;
...
/*********************************************************************
*     get Meter-TC map from internal TC 
 *     default 0.
*********************************************************************/
uint32
 ...
 *   PROC
 * FUNCTION:
 *   set the ethernet meter pointer for none ethernet packets. according to HW spec, it is not allowed to set a value > 339.
 * INPUT:
 *     int  unit -Identifier of the device to access.
 *     int  arg - value of the ethernet meter pointer for none ethernet packets.
 * RETURNS:
 ...
 *   PROC
 * FUNCTION:
 *   get the ethernet meter pointer for none ethernet packets. 
 * INPUT:
 *     int  unit -Identifier of the device to access.
 *     int  arg - return value of the ethernet meter pointer for none ethernet packets.
 * RETURNS:
 *   OK or ERROR indication.
*********************************************************************/
soc_error_t
