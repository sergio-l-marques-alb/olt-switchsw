...
typedef struct periodic_event_object_s {
    ...
    int active;                     /* indication whether the thread is currently active. This indication is maintained by the thred itself */
 ...
 *   1. Mark thread is active
 *   2. Iterate unless inidcate to quit
 *      2.a. Wait on a semaphore (until triggered or until
 *      timeout provided by the user is passing
 *      2.b. Execute user callback.
 *      2.c. Check call back error
 *          if error inc error count, otherwise zero error
 *          count.
 *      2.d. If error_count > 0 quit the loop.
 *   3. When thread is about to exit:
 *      3.a indicate active is false.
 *      3.b. do sal_thread_exit.
 * 
 *  */
...
/** see .h file */
int periodic_event_destroy(periodic_event_handler_t* h, int force_stop)
{
    int rv;
    ...
    if (h != NULL) {
        ...
        rv = periodic_event_stop(*h, force_stop, 0);
        if (rv != _SHR_E_NONE) {
...
int periodic_event_trigger(periodic_event_handler_t h)
{
    ...
    /* if thread isn't active - return an error*/
    if (!obj->active) {
        LOG_ERROR(obj->bsl_module, (BSL_META("PE[%s]: Thread isn't active \n"), obj->name));
...
int periodic_event_start(periodic_event_handler_t h) 
{
    ...
    /* if thread is already active - return an error*/
    if (obj->active) {
        LOG_ERROR(obj->bsl_module, (BSL_META("PE[%s]: Thread is already running \n"), obj->name));
...
/** see .h file */
int periodic_event_stop(periodic_event_handler_t h, int force_stop, int dont_wait)
{
    int rv = _SHR_E_NONE;
    soc_timeout_t to;
    periodic_event_object_t *obj = (periodic_event_object_t*)h;
    ...
    while (obj->active == 1) {
        if (soc_timeout_check(&to)) {
            /* don't return error if failed to quit - will destroy teh thread anyway */
    ...
    if (force_stop) {
        ...
        /* Make sure active is down in any case*/
        obj->active = 0;
