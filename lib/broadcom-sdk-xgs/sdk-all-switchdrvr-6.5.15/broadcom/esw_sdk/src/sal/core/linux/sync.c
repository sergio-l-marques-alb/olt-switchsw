 ...
 *   The timed semaphore is implemented as wait queues to support
 *   the interruptible property.
 */
...
{
    sem_ctrl_t *s = sal_alloc(sizeof(*s), desc);
...
#ifdef BROADCOM_DEBUG
#ifdef INCLUDE_BCM_SAL_PROFILE
    SAL_SEM_RESOURCE_USAGE_INCR(_sal_sem_count_curr, _sal_sem_count_max, ilock);
#endif
#endif
...
sal_sem_destroy(sal_sem_t b)
{
     ...
     * The semaphore can be destroyed when it has become inactive.
     */
    while (atomic_read(&s->act)) {
...
#ifdef BROADCOM_DEBUG
#ifdef INCLUDE_BCM_SAL_PROFILE
    SAL_SEM_RESOURCE_USAGE_DECR(_sal_sem_count_curr, ilock);
#endif
#endif
}

static inline int
_sal_down_timeout(sem_ctrl_t *s, int usec)
{
    int rv;

    atomic_inc(&s->cnt);
    rv = wait_event_interruptible_timeout(s->wq, !down_trylock(&s->sem),
...
static inline void
_sal_up(sem_ctrl_t *s)
{
    up(&s->sem);

    if (atomic_read(&s->cnt)) {
...
sal_sem_take(sal_sem_t b, int usec)
{
    ...
    while (1) {
        if (usec == sal_sem_FOREVER) {
            rv = down_interruptible(&s->sem);
        } else {
            rv = _sal_down_timeout(s, usec);
        }
...
sal_sem_give(sal_sem_t b)
{
    ...
    /* Keep active to prevent the semaphore is destroyed prematurely. */
    atomic_inc(&s->act);

    if (s->binary) {
        /*
         * The semahpore must be recounted in an atomic operation
         * for the binary property.
         */
        spin_lock_irqsave(&s->lock, s->flags);
    ...
    if (s->binary) {
        spin_unlock_irqrestore(&s->lock, s->flags);
    }
