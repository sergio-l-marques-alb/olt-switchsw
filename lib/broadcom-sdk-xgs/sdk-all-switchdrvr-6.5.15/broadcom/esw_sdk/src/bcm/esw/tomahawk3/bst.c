...
_bcm_bst_th3_intr_to_resources(int unit, uint32 *flags)
{
    ...
    */
    ...
    for (itm = 0; itm < NUM_ITM(unit); itm++) {
        ...
        fval = soc_reg64_field32_get(unit, reg_type, rval32_type,
                                     MMU_THDO_QUE_TOT_BST_TRIGf);

        if (fval && !bid_detect[_bcmResourceBstStatIdQueueTotal]) {
            fld = MMU_THDO_QUE_TOT_BST_IDf;
            index[res_ct] = soc_reg64_field32_get(unit, reg_type, rval32_type, fld);
            ...
            for(inst = 0; inst < resInfo->num_instance; inst++) {
                for (ei = 0; ei < index_max; ei ++) {
                    ...
                    if (wb) {
                        ...
                        BCM_IF_ERROR_RETURN(
                        _bcm_th3_bst_profile_op(unit, BST_OP_GET | BST_OP_THD,
                            bid, inst, ei, &profile_index));
                    } else if (ei == 0) {
                        if(bid == bcmBstStatIdQueueTotal) {
                            BCM_IF_ERROR_RETURN(_bst_th3_cmn_profile_set_hw
                    ...
                    if(profile_index != 0) {
                        ...
                        BCM_IF_ERROR_RETURN(
                        _bcm_th3_bst_profile_op(unit, BST_OP_SET, bid, inst, ei,
                                                           &profile_index));
                    }
