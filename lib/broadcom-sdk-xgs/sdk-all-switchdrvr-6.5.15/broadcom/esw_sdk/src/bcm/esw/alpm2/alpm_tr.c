...
/* Dump ALPM trace log */
int
alpm_trace_dump(int u, int showflags, char *filename)
{
    int rv = BCM_E_NONE;
    _alpm_log_t *p;
    int egress_idx_min;
    ...
    cli_out("### ALPM trace enable:%d wrap enable:%d wrap:%d count:%d "
            ...
            ALPMTR_WRAP(u), ALPMTR_CNT(u), showflags, filename);
    ...
    if (ALPMTR_CNT(u) > ALPM_TRACE_DUMP_SIZE && !fp) {
