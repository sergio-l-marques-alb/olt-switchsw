...
#define BCM_LINK_PBMP_SCACHE_NUM        BCM_LINK_PBMP_SCACHE_NUM_WB_1_3
                                 ...
                                 int speed, int duplex)
{
    ...
    PBMP_ITER(pbmp, dst_port) {
        ...
        if (dst_port_speed <= speed) {
            dst_mmu_port = si->port_p2m_mapping[si->port_l2p_mapping[dst_port]];
            if (dst_mmu_port < 32) {
...
_bcm_esw_linkscan_serdes_link_update(int unit, int port, int link)
{
    ...
    if (SOC_USE_PORTCTRL(unit) && (!IS_CPRI_PORT(unit, port))) {
                    ...
                    if(eee_en && new_link) {
                        ...
                        if (SOC_USE_PORTCTRL(unit)) {
                            if (current_time > lc->lc_up_time[port-1]) {
                                /* Do not enable native EEE on the MAC if autoEEE is enabled */
                                int r;
                                uint32 auto_eee = 1;
                                r = bcm_esw_port_phy_control_get(unit, port,
