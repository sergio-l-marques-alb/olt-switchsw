...
#define BCM_WB_TELEMETRY_VERSION_1_0      SOC_SCACHE_VERSION(1,0)
#define BCM_WB_TELEMETRY_DEFAULT_VERSION  BCM_WB_TELEMETRY_VERSION_1_0
...
_bcm_xgs5_telemetry_sync(int unit)
{
    ...
    SOC_SCACHE_HANDLE_SET(scache_handle, unit, BCM_MODULE_TELEMETRY, 0);
...
/*
 * Function:
 *     _bcm_telemetry_scache_size_get
 * Purpose:
 ...
 */
STATIC uint32
_bcm_telemetry_scache_size_get(int unit)
{
...
STATIC int _bcm_telemetry_scache_alloc(int unit, int create)
{
    ...
    rqd_size += _bcm_telemetry_scache_size_get(unit);

    SOC_SCACHE_HANDLE_SET(scache_handle, unit, BCM_MODULE_TELEMETRY, 0);

    if (create) {
        rv = _bcm_esw_scache_ptr_get(unit, scache_handle, 1, rqd_size,
                               &scache, BCM_WB_TELEMETRY_DEFAULT_VERSION, NULL);
...
/*
 * Function:
 *      _bcm_telemetry_wb_recover
 * Purpose:
 *      Telemetry WB recovery
 * Parameters:
 *      unit - (IN) Unit number.
 * Returns:
 *      BCM_E_XXX
 * Notes:
 */
STATIC int _bcm_telemetry_wb_recover(int unit)
{
    ...
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        ...
        rv = _bcm_esw_scache_ptr_get(unit, scache_handle, 0, 0,
                                     &scache, BCM_WB_TELEMETRY_DEFAULT_VERSION,
                                     &recovered_ver);
    ...
    BCM_IF_ERROR_RETURN(_bcm_telemetry_scache_alloc(unit, 0));
...
#ifdef BCM_WARM_BOOT_SUPPORT
    if (SOC_WARM_BOOT(unit)) {
        rv = _bcm_telemetry_wb_recover(unit);
    } else {
        rv = _bcm_telemetry_scache_alloc(unit, 1);
    }
#endif /* BCM_WARM_BOOT_SUPPORT */
