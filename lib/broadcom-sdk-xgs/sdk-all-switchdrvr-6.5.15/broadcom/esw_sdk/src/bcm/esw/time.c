                                         ...
                                         int *adjusted_phy_port)
{
    ...
    if (SOC_IS_HELIX5(unit)) {
        ...
        first_phy_port = gh2_tsc_phy_port[((phy_port - 58) / 4)];
    } else {
...
#if defined(BCM_GREYHOUND2_SUPPORT)
#define BCM_TIME_GH2_PHY_PORT_VALID(x) ((x) >= 58 && (x) <= 89) /* Section 7.1.3, GH2 top design spec */
...

{
    ...
    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3(unit) || SOC_IS_APACHE(unit) || SOC_IS_MAVERICK2(unit) || SOC_IS_HELIX5(unit)) {
        /* tomahawk  & TD3 */
        if ((div_out.stage0_mode == bcmTimeSynceModeSDMFracDiv) &&
            ...
            } /* dispatch_type == phymodDispatchTypeTscf */
...

{
            ...
            switch (frequency) {
                case bcmTimeSyncE25MHz: /* 25MHz using SDM mode */
                    ...
                    if (type == phymodDispatchTypeTsce_dpll) {
                        ...
                        } else if (IS_HG_PORT(unit, clk_src_config->port) && (speed == 10000 || speed == 21000 || speed == 42000)) {
    ...
    if (clk_src_config->input_src == bcmTimeSynceInputSourceTypePort) {
#ifdef PORTMOD_SUPPORT
        if ((SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3(unit) || SOC_IS_APACHE(unit) || SOC_IS_MAVERICK2(unit) || SOC_IS_HELIX5(unit)) && soc_feature(unit, soc_feature_portmod)) {
        ...
        bcm_time_ts_counter_t  *counter)
{
    ...
    if(SOC_IS_MONTEREY(unit) || SOC_IS_TOMAHAWK3(unit)) {
        ...
        } else if ( SOC_IS_TOMAHAWK3(unit) && (counter->time_format == bcmTimeFormatPTP) ) {
...

{
    ...
    if ((SOC_IS_TOMAHAWK3(unit)) && (!BCM_FAILURE(rv))) {
        soc_mem_t mem_2 = INVALIDm;
        mem_2 = EGR_TS_UTC_CONVERSION_2m;
        if(mem_2 == INVALIDm) {
            return BCM_E_NONE;
        }
        ...
        int *capture_handle)
{
    ...
    bcm_esw_time_capture_enable(unit, time_capture_input, enable);
                  ...
                  void *d3, void *d4)
{
    ...
    if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS_FIFO_NOT_EMPTYf)) {
    ...
    if (soc_reg_field_get(unit, NS_TS_INT_STATUSr, g_int_status, TS_FIFO_OVERFLOWf)) {
        g_fifo_data_index = 0;
