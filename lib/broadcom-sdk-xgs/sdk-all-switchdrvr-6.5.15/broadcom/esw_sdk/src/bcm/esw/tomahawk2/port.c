                                ...
                                soc_port_schedule_state_t *port_schedule_state)
{
    ...
    rate = sal_alloc(nport * sizeof(uint32), "port rate");
    if (rate == NULL) {
        rv = BCM_E_MEMORY;
        goto cleanup;
    }
    burst = sal_alloc(nport * sizeof(uint32), "port burst");
    if (burst == NULL) {
        rv = BCM_E_MEMORY;
        goto cleanup;
    }
    for (i = 0; i < nport; i++) {
        ...
        (void)bcm_esw_port_rate_egress_get(unit, port[i], &rate[i], &burst[i]);
        if ((rate[i] != 0) || (burst[i] != 0)) {
            (void)bcm_esw_port_rate_egress_set(unit, port[i], 0, 0);
        }
    }
    ...
    for (i = 0; i < nport; i++) {
        if ((rate[i] != 0) || (burst[i] != 0)) {
            (void)bcm_esw_port_rate_egress_set(unit, port[i], rate[i], burst[i]);
        }
    }
