                ...
                void *unit_as_ptr, void* unused_3)
{
    int callback_time;
    _bcm_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_HELIX5_SUPPORT)
...
#ifdef BCM_HELIX5_SUPPORT
    if (SOC_IS_HELIX5(unit)) {
        callback_time = _BCM_HX5_PFC_DEADLOCK_CB_INTERVAL(0);
        if (_BCM_HX5_PFC_DEADLOCK_CB_ENABLED(0)) {
            sal_dpc_time(callback_time, &pfc_deadlock_cb, 0,
                    INT_TO_PTR(sal_time_usecs()), INT_TO_PTR(callback_time),
                    0, 0);
        }
        _BCM_HX5_PFC_DEADLOCK_CB_COUNT(0)++;
    } else
#endif
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit) && !SOC_IS_HELIX5(unit)) {
        callback_time = _BCM_TD3_PFC_DEADLOCK_CB_INTERVAL(0);
        if (_BCM_TD3_PFC_DEADLOCK_CB_ENABLED(0)) {
            sal_dpc_time(callback_time, &pfc_deadlock_cb, 0,
                    INT_TO_PTR(sal_time_usecs()), INT_TO_PTR(callback_time),
                    0, 0);
        }
        _BCM_TD3_PFC_DEADLOCK_CB_COUNT(0)++;
    } else
#endif
    {
        pfc_deadlock_control = _BCM_UNIT_PFC_DEADLOCK_CONTROL(0);
        if (!pfc_deadlock_control) {
            return;
        }
        callback_time = _BCM_PFC_DEADLOCK_CB_INTERVAL(0);
