    ...
    do {                                        \
        ...
        for (pid = 0; pid < ALPM_BKT_PID_CNT; pid++) {
            ...
            if (ALPM_128B(u) && ALPM_BKT_RSVD(u) &&
                ...
                int rsvd_v6_bnks =
                    BPC_BNK_CNT(bp_conf) * ALPM_BKT_RSVD_CNT(u) / 8192;
                ALPM_ALLOC_EG(
                       ...
                       _bcm_defip_cfg_t *lpm_cfg, int ent_idx, uint32 write_flags)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        rv = th_mem_write(u, acb, pvt_node, BPC_BKT_TBL(bp_conf), index, entry);
    }
...
int
th_tcam_table_sz(int u, int pkm)
{
    int sz = 0;
    soc_mem_t mem;
 ...
 */
void
th_alpm_table_ent_dump(int u, int vrf_id, soc_mem_t mem, void *entry,
...
int
th_alpm_table_dump(int u, int vrf_id, soc_mem_t mem, int copyno, int index,
                   int fmt, int ent_idx, int flags)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        th_alpm_table_ent_dump(u, vrf_id, mem, entry, copyno, index, fmt,
...
#define UTIL_85(x) ((x) * 85 / 100)
    ...
    for (pid = 0; pid < ALPM_BKT_PID_CNT; pid++) {
        ...
        if (ALPM_128B(u) && ALPM_BKT_RSVD(u)) {
            int rsvd_v6_bnks =
                BPC_BNK_CNT(bp_conf) * ALPM_BKT_RSVD_CNT(u) / ALPM_PHY_BKT_CNT(u);
            if (pkm == ALPM_PKM_128) {
