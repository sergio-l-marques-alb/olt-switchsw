...
extern int tcam_wb_reinit(int u, int vrf_id, int pkm, int idx, int key_len, int mc);
extern int tcam_wb_reinit_done(int u);
extern int alpm_vrf_deinit(int u, int vrf_id, int ipt);
 ...
 */
STATIC void
alpm_pivot_bpm_len_update(int u)
{
    int         i, vrf_id, ipt;
    _alpm_cb_t  *acb;
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (ipt = 0; ipt < ALPM_IPT_CNT; ipt++) {
            for (vrf_id = 0; vrf_id < ALPM_VRF_ID_MAX(u); vrf_id++) {
                ...
                pfx_trie = ALPM_VRF_TRIE(u, vrf_id, ipt);
                ...
                (void)bcm_esw_alpm_pvt_traverse(u, i, vrf_id, ipt,
 ...
 */
int
alpm_wb_pfx_trie_add(int u, int vrf_id, int ipt,
                     uint32 *key, int key_len, int ent_idx,
                     _alpm_bkt_adata_t *adata,
                     _alpm_bkt_node_t *bkt_node)
{
                   ...
                   uint32 *route_cnt, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    ALPM_IER(alpm_wb_pvt_insert(u, acb, vrf_id, pkm, key, key_len, idx, sub_idx, &pvt_node));
    ...
    for (bnk = 0; bnk < ACB_BNK_PER_BKT(acb, vrf_id); bnk++) {
        ...
        for (eid = 0; eid < ACB_FMT_ENT_MAX(acb, vrf_id, fmt); eid++) {
            ...
            if (ACB_HAS_RTE(acb, vrf_id)) {
                ...
                ALPM_IER(alpm_wb_pfx_trie_add(u, vrf_id, ipt, new_key,
 ...
 */
int
bcm_esw_alpm_warmboot_walk(int u, _bcm_l3_trvrs_data_t *trv_data)
{
    int ipv6, key_mode;
    int idx, sub_idx;
    int vrf_id;
    ...
    if (ALPMC(u) == NULL || !ALPMC(u)->_alpm_inited) {
        return BCM_E_INIT;
    }

    acb = ACB_TOP(u); /* from top to bottom */
    ...
    for (pk = 0; pk < ALPM_PKM_CNT; pk++) {
        ...
        idx_end = tcam_table_size(u, pk);
        /* Walk all TCAM entries */
        for (idx = 0; idx < idx_end; idx++) {
            ALPM_IEG(tcam_entry_read_no_cache(u, pk, lpm_entry, idx, idx));
            ...
            for (sub_idx = 0; sub_idx < ALPM_TCAM_SUBCNT; sub_idx += step_size) {
                int ipt;
                rv_tcam = tcam_valid_entry_mode_get(u, pk, lpm_entry,
                ...
                tcam_entry_vrf_id_get(u, pkm, lpm_entry, sub_idx, &vrf_id);
                /* process pre-pivot ASSOC_DATA */
                tcam_entry_adata_get(u, pkm, lpm_entry, sub_idx, &adata0);
                ...
                tcam_entry_to_key(u, pkm, lpm_entry, sub_idx, key);
                tcam_entry_pfx_len_get(u, pkm, lpm_entry, sub_idx, &key_pfx_len);

                /* only do tcam_reinit once per TCAM index */
                if (tcam_idx_reinit == FALSE) {
                    tcam_wb_reinit(u, vrf_id, pkm, idx, key_pfx_len,
                ...
                if (!use_alpm) {
                    ...
                    if (!ALPM_HIT_SKIP(u) &&
                        tcam_entry_hit_get(u, pkm, lpm_entry, sub_idx)) {
                    ...
                    if (!ACB_VRF_INITED(u, acb, vrf_id, ipt)) {
                        (void)alpm_pvt_trie_init(u, vrf_id, ipt);
                    }
                ...
                tcam_entry_bdata_get(u, pkm, lpm_entry, sub_idx, fent);
    ...
    alpm_pivot_bpm_len_update(u);

    /* Update TCAM state after tcam_reinit */
    ALPM_IEG(tcam_wb_reinit_done(u));
...
alpm_wb_test(int u)
{
    ...
    /* Do ALPM module soft-reinit (ALPM HW mem untouched) */
    ...
    /* TD3ALPMTBD: assuming bcm_esw_alpm_init() doesn't clear Hw memories */
    bcm_esw_alpm_init(u);
    LOG_CLI(("ALPM module soft-reset!\n"));
    LOG_CLI(("ALPM warmboot walk: restoring from Hw mem ...\n"));
    rv = bcm_esw_alpm_warmboot_walk(u, NULL);
...
alpm_pfx_update_match_cb(_alpm_pfx_node_t *pfx_node, void *user_data)
{
    ...
    if (!ALPM_HIT_SKIP(u) && def_rte) {
        int hit_val;
        ALPM_HIT_LOCK(u);
        hit_val = alpm_rte_hit_get(u, vrf_id, bkt_tbl, idx, ent);
        if (hit_val > 0) {
            lpm_cfg.defip_flags |= BCM_L3_HIT;
        }
        ALPM_HIT_UNLOCK(u);
                           ...
                           _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (ipt = 0; ipt < ALPM_IPT_CNT; ipt++) {
        upd_data.ipt = ipt;
        for (vrf_id = 0; vrf_id <= ALPM_VRF_ID_MAX(u); vrf_id++) {
            if (!ACB_VRF_INITED(u, ACB_VRF_BTM(u, vrf_id), vrf_id, ipt)) {
                continue;
            }
            upd_data.vrf_id = vrf_id;
            (void)bcm_esw_alpm_pfx_traverse(u, vrf_id, ipt,
 ...
 */
STATIC int
tcam_direct_route_update_match(int u, _bcm_l3_trvrs_data_t *trv_data)
{
    int ipv6, key_mode;
    int idx, sub_idx;
    int vrf_id;
...
bcm_esw_alpm_update_match(int u, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    ALPM_IEG(tcam_direct_route_update_match(u, trv_data));
    ...
    alpm_pfx_trav_update_match(u, def_rte, &def_rte_cnt, trv_data);
    ...
    for (idx = 0; idx < def_rte_cnt; idx++) {
        ...
        /* Execute operation routine if any. */
        if (trv_data->op_cb) {
            rv = (*trv_data->op_cb)(u, (void *)trv_data,
                                    (void *)&lpm_cfg,
                                    (void *)&nh_ecmp_idx,
                                     &cmp_result);
        }
                             ...
                             _bcm_l3_trvrs_data_t *trv_data1)
{
    ...
    ALPM_DRV(u)->tcam_hw_cleanup(u);

    /* ALPM routes fast delete_all using prefix trie traverse */
    if (BCM_XGS3_L3_DEFIP_IP4_CNT(u)) {
        trv_data1->flags = 0;
        alpm_pfx_trav_update_match(u, NULL, NULL, trv_data1);
    }
