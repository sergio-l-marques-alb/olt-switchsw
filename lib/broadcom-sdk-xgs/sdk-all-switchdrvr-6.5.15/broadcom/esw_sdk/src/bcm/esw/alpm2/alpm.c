...
/* Internal functions */
static int alpm_pvt_update_by_pvt_node(int, _alpm_cb_t *, _alpm_pvt_node_t *, uint32);
static int alpm_bkt_pfx_shrink(int u, _alpm_cb_t *acb, _alpm_pvt_node_t *pvt_node, void *user_data);
    ...
    for (ipt = 0; ipt < ALPM_IPT_CNT; ipt ++) {
        for (vrf_id = 0; vrf_id <= ALPM_VRF_ID_MAX(u); vrf_id++) {
            for (i = 0; i < ACB_CNT(u); i++) {
                ...
                rv = bcm_esw_alpm_pvt_traverse(u, i, vrf_id, ipt,
            ...
            vrf_trie = ALPM_VRF_TRIE(u, vrf_id, ipt);
...
int
alpm_ppg_hitbit(int u, int vrf_id, int ipt, uint32 *key, int key_len)
{
    _alpm_ppg_data_t ppg_data;
...
int
alpm_pvt_trie_init(int u, int vrf_id, int ipt)
{
    int         rv = BCM_E_NONE;
    uint32      max_key_len;
...
alpm_bkt_ent_get_attempt(int u, _alpm_cb_t *acb, _alpm_bkt_pool_conf_t *bp_conf,
                 ...
                 int pfx_len, uint32 *ent_idx, int *fmt_update, int bkt_test)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (BCM_SUCCESS(rv) && fmt_update && !ACB_BKT_FIXED_FMT(acb, vrf_id)) {
            ...
            if (tcam_write && (tcam_write->hw_idx >= 0)) {
                ...
                rv1 = _tcam_entry_write(u, pkm, tcam_write->ent, tcam_write->hw_idx,
                                        tcam_write->hw_idx);
...
static int
alpm_vrf_pfx_trie_init(int u, int vrf_id, int ipt)
{
    int rv = BCM_E_NONE;
...
int
alpm_vrf_pfx_trie_deinit(int u, int vrf_id, int ipt)
{
    int rv = BCM_E_NONE;
    alpm_lib_trie_t  *root = NULL;
    uint32  key[5] = {0};
...
bad:
    if (BCM_FAILURE(rv)) {
        ...
        if (pvt_root_inited) {
            (void)alpm_vrf_pvt_trie_deinit(u, acb, vrf_id, ipt, lpm_cfg);
        } else if (pvt_node_inited) {
            (void)alpm_vrf_pvt_node_delete(u, acb, vrf_id, ipt, lpm_cfg);
        }
    }
...
alpm_vrf_init(int u, _alpm_cb_t *acb, int vrf_id, int ipt, uint8 db_type)
{
    ...
    if (ACB_HAS_RTE(acb, vrf_id)) {
        rv = alpm_vrf_pfx_trie_init(u, vrf_id, ipt);
    }
...
bad:
    if (BCM_FAILURE(rv)) {
        ...
        if (pvt_cfg.bkt_info != NULL) {
            (void)alpm_bkt_free(u, acb, vrf_id, ipt,
                                (_alpm_bkt_info_t *)pvt_cfg.bkt_info);
        }
 ...
 */
int
alpm_vrf_deinit(int u, int vrf_id, int ipt)
{
    int                 i, rv = BCM_E_NONE;
    _bcm_defip_cfg_t    lpm_cfg;
    _alpm_pvt_node_t    *pvt_node = NULL;
    ...
    rv = alpm_vrf_pfx_trie_deinit(u, vrf_id, ipt);
              ...
              _alpm_bkt_info_t *nbkt_info, int max_spl_cnt)
{
    ...
    if (tcam_write != NULL) {
        ...
        if (tcam_write->hw_idx >= 0) {
            int rv1;
            rv1 = _tcam_entry_write(u, ALPM_LPM_PKM(u, lpm_cfg), tcam_write->ent,
...
retry_spl:
    if (rv == BCM_E_FULL && retry_count < retry_max) {
        ...
        if (ACB_IDX(acb) != ACB_VRF_BTM_IDX(u, vrf_id)) {
        ...
        if (ACB_BKT_FIXED_FMT(acb, vrf_id)) {
            uint8 def_fmt;
            if (!bcm_esw_alpm_tcam_avail(u, vrf_id, ipt, lpm_cfg->defip_sub_len, 0) &&
                ACB_HAS_TCAM(acb)) {
                rv = alpm_cb_merge(u, acb, vrf_id, ipt);
                ALPM_IEG(rv);
...
bcm_esw_alpm_ctrl_init(int u)
{
    ...
    if (ALPM_128B(u)) {
        ...
        if (ALPM_BKT_RSVD(u)) {
            /* Based on 8K Buckets */
            ALPM_BKT_RSVD_CNT(u) =
                pair_ent_cnt * 2 * 8192 / (tcam_depth * max_tcams);
        }
    }
...
bcm_esw_alpm_init_done(int u, int rv)
{
    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        LOG_CLI(("*** unit %d: alpm level %d loaded: %d banks in %s-%s %s\n",
                 u, i+2, banks, mode_str[mode+1], ALPM_128B(u) ? "128" : "64",
                 urpf ? "(urpf)" : ""));
                     ...
                     _bcm_l3_trvrs_data_t *trv_data1)
{
    ...
    if (ALPMC(u) && ALPMC(u)->_alpm_inited) {
        ...
        rv = bcm_esw_alpm_fast_delete_all(u, trv_data, trv_data1);
        if (BCM_FAILURE(rv)) {
            ALPM_ERR(("ALPM/TCAM fast_delete_all travel failed (%d)", rv));
        }

        rv = bcm_esw_alpm_ctrl_cleanup(u);
        if (BCM_FAILURE(rv)) {
            ALPM_ERR(("ALPM ctrl cleanup failed (%d)", rv));
        }

        rv = bcm_esw_alpm_tcam_cleanup(u);
        if (BCM_FAILURE(rv)) {
            ALPM_ERR(("ALPM tcam cleanup failed (%d)", rv));
        }

        if (!ALPM_HIT_SKIP(u) && ALPMDH(u)) {
            rv = ALPM_DRV(u)->alpm_hit_cleanup(u);
            if (BCM_FAILURE(rv)) {
                ALPM_ERR(("ALPM hitbit cleanup failed (%d)", rv));
            }
            rv = alpm_dist_hitbit_enable_set(u, us);
        }

        alpm_trace_log(u, ALPM_TRACE_OP_DELETE_ALL, NULL, 0, rv);
    }
...
bcm_esw_alpm_insert(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (ALPM_LPM_DIRECT_RTE(u, lpm_cfg)) {
        ...
        acb = ACB_TOP(u);
        if (!ACB_VRF_INITED(u, acb, vrf_id, ipt)) {
            ALPM_IER(alpm_pvt_trie_init(u, vrf_id, ipt));
        }
    ...
    rv2 = alpm_cb_sanity(u, 0x3, vrf_id, ipt);
...
bcm_esw_alpm_lookup(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (ALPM_LPM_DIRECT_RTE(u, lpm_cfg)) {
        if (!ACB_VRF_INITED(u, ACB_TOP(u), vrf_id, ipt)) {
...
bcm_esw_alpm_delete(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (BCM_SUCCESS(rv) && VRF_ROUTE_CNT(acb, vrf_id, ipt) == 0) {
        rv = alpm_vrf_deinit(u, vrf_id, ipt);
    }
...
bcm_esw_alpm_update(int u, _bcm_defip_cfg_t *lpm_cfg)
{
    ...
    if (BCM_SUCCESS(rv)) {
        rv = bcm_esw_alpm_insert(u, lpm_cfg);
    }
 ...
 */
int
bcm_esw_alpm_pvt_traverse(int u, int acb_idx, int vrf_id, int ipt,
                          bcm_esw_alpm_pvt_trav_cb trav_fn, void *user_data)
{
    ...
    rv = alpm_lib_trie_traverse(ACB_PVT_TRIE(acb, vrf_id, ipt), bcm_esw_alpm_pvt_trav_int_cb,
                       (void *)&trav_st, trieTraverseOrderIn);
    return rv;
}
 ...
 */
int
bcm_esw_alpm_pfx_traverse(int u, int vrf_id, int ipt,
                          bcm_esw_alpm_pfx_trav_cb trav_fn, void *user_data)
{
    ...
    alpm_lib_trie_traverse2(ALPM_VRF_TRIE(u, vrf_id, ipt), bcm_esw_alpm_pfx_trav_int_cb,
    ...
    for (ipt1 = ipt; ipt1 < ALPM_IPT_CNT; ipt1++) {
        if ((ipt1 == ipt) ||
            ...
            for (vrf_id = vrf_min; vrf_id <= vrf_max; vrf_id++) {
                ...
                (void)bcm_esw_alpm_pvt_traverse(u, acb->acb_idx, vrf_id, ipt1,
...
int alpm_table_dump(int u, int vrf_id, soc_mem_t mem, int copyno, int index,
                    int fmt, int ent_idx, int flags)
{
    return ALPM_DRV(u)->alpm_table_dump(u, vrf_id, mem, copyno, index, fmt, ent_idx, flags);
}
 ...
 */
int
alpm_cb_sanity(int u, int acb_bmp, int vrf_id, int ipt)
{
    int rv = BCM_E_NONE;
    int acb_idx;
    _alpm_cb_t *acb = NULL;
    ...
    for (acb_idx = 0; acb_idx < ACB_CNT(u); acb_idx++) {
        ...
        rv = bcm_esw_alpm_pvt_traverse(u, acb->acb_idx, vrf_id, ipt,
