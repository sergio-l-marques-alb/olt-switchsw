...
STATIC int
_bcm_th3_queue_current_usage_bytes_set(int unit, bcm_gport_t port,
        bcm_cos_queue_t cosq, bcm_cosq_stat_t stat, uint64 value) {
    soc_ctr_control_info_t ctrl_info;
    soc_reg_t ctr_reg;
    ...
    if (stat == bcmCosqStatEgressMCQueueBytesCurrent) {
        ...
        if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(port)) {
            ...
            if (cosq < si->port_num_uc_cosq[local_port]) {
                ...
                startq = (cosq - si->port_num_uc_cosq[local_port]);
            }
        }
        ctr_reg = SOC_COUNTER_NON_DMA_QUEUE_CURRENT;
    } else { /*(stat == bcmCosqStatEgressUCQueueBytesCurrent) */
        if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(port) ||
            BCM_GPORT_IS_SCHEDULER(port)) {
            return BCM_E_PARAM;
        }
...
STATIC int
_bcm_th3_pg_current_usage_bytes_set(int unit, bcm_gport_t port,
        bcm_cos_queue_t cosq, bcm_cosq_stat_t stat, uint64 value) {
    soc_ctr_control_info_t ctrl_info;
    soc_reg_t ctr_reg;
    int startq = 0;
    bcm_port_t local_port = -1;
    soc_reg_t ctr_regs_pg_min[_TH3_MMU_NUM_PG] = {
    ...
    soc_reg_t ctr_regs_pg_shared[_TH3_MMU_NUM_PG] = {
    ...
    soc_reg_t ctr_regs_pg_hdrm[_TH3_MMU_NUM_PG] = {
...
_bcm_th3_pg_current_usage_bytes_get(int unit, bcm_gport_t port,
    ...
    ctrl_info.instance = local_port;
    /*sync_mode should always be 0 for current usage bytes*/
    BCM_IF_ERROR_RETURN
        (soc_counter_generic_get(unit, ctr_reg,
                                 ctrl_info, 0, startq, value));
...
STATIC int
_bcm_th3_sp_current_usage_bytes_set(int unit, bcm_gport_t port,
        bcm_cos_queue_t cosq, bcm_cosq_stat_t stat, uint64 value) {
    soc_ctr_control_info_t ctrl_info;
    soc_reg_t ctr_regs_pool[_TH3_MMU_NUM_POOL] =
    {SOC_COUNTER_NON_DMA_SP0_CURRENT, SOC_COUNTER_NON_DMA_SP1_CURRENT,
        SOC_COUNTER_NON_DMA_SP2_CURRENT, SOC_COUNTER_NON_DMA_SP3_CURRENT};
    int startq = 0;
    bcm_port_t local_port = -1;
    int pool = -1;
    soc_reg_t ctr_reg;
...
STATIC int
_bcm_th3_cosq_obm_buf_use_cnt_set(int unit, bcm_port_t port, bcm_cos_queue_t
        cosq, bcm_cosq_stat_t stat, uint64 value)
{
    int pipe, obm_id, subp, pport;
    soc_reg_t reg;
    ...
    BCM_IF_ERROR_RETURN
            (soc_reg64_set(unit, reg, REG_PORT_ANY, subp, rval64));
                     ...
                     bcm_cosq_stat_t stat, uint64 value)
{
    ...
    switch (stat) {
        ...
        case bcmCosqStatEgressMCQueueBytesCurrent:
            /*cell_value = (value + _TH3_MMU_BYTES_PER_CELL - 1)
                                            / _TH3_MMU_BYTES_PER_CELL;*/
            COMPILER_64_ADD_64(cell_value, value);
            COMPILER_64_ADD_64(cell_value, _TH3_MMU_BYTES_PER_CELL);
            COMPILER_64_SUB_64(cell_value, 1);
            COMPILER_64_UDIV_32 (cell_value, _TH3_MMU_BYTES_PER_CELL);
            return _bcm_th3_queue_current_usage_bytes_set(unit, port, cosq, stat,
                    cell_value);
        ...
        case bcmCosqStatIngressPortPGHeadroomBytesCurrent:
            ...
            COMPILER_64_SUB_64(cell_value, 1);
            COMPILER_64_UDIV_32 (cell_value, _TH3_MMU_BYTES_PER_CELL);
            return _bcm_th3_pg_current_usage_bytes_set(unit, port, cosq, stat,
                    cell_value);
        case bcmCosqStatIngressPortPoolSharedBytesCurrent:
            ...
            COMPILER_64_SUB_64(cell_value, 1);
            COMPILER_64_UDIV_32 (cell_value, _TH3_MMU_BYTES_PER_CELL);
            return _bcm_th3_sp_current_usage_bytes_set(unit, port, cosq, stat,
                    cell_value);
        ...
        case bcmCosqStatOBMBufferBytes:
            return _bcm_th3_cosq_obm_buf_use_cnt_set(unit, port, cosq, stat, value);
        default:
            return BCM_E_PARAM;
    }
    return BCM_E_NONE;
