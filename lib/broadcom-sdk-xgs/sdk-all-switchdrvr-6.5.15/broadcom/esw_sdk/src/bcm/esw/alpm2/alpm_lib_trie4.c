...
{
    uint32 val=0, delta=0, diff, bitpos;
    ...
    if (bitpos >= len) {
        ...
        diff = len - bitpos;
        /* coverity[var_deref_op : FALSE] */
        val = key[KEY48_BIT2IDX(pos)] & TRIE_MASK(bitpos);
        val = TRIE_SHL(val, diff, _NUM_WORD_BITS_);
        /* get bits from next word */
        delta = _alpm_lib_key_get_bits(key, pos-bitpos, diff);
        return (delta | val);
    }
}
 ...
 */
static int _key_shift_left(uint32 *key, uint32 shift)
{
    uint32 index=0;
...
static int _trie_inorder_traverse(alpm_lib_trie_node_t *trie, alpm_lib_trie_callback_f cb, void *user_data)
{
    int rv = SOC_E_NONE;
    ...
    if (SOC_SUCCESS(rv)) {
        rv = _trie_inorder_traverse(trie->child[0], cb, user_data);
    }
...
int alpm_lib_trie_find_lpm(alpm_lib_trie_t *trie, uint32 *key, uint32 length, alpm_lib_trie_node_t **payload)
{
    ...
    *payload = NULL;
    ...
    if (trie->trie) {
        if (trie->v6_key) {
            rv = _alpm_lib_trie_v6_find_lpm(trie->trie, key, length, payload,
                                   NULL, NULL, 0);
        } else {
 ...
 *   free_end --  free
 */
static int _trie_skip_node_free(alpm_lib_trie_node_t *trie, uint32 *key, uint32 length)
{
    uint32 lcp=0;
    int bit=0, rv=SOC_E_NONE;
    ...
    if (length > trie->skip_len) {
        if (lcp == trie->skip_len) {
            ...
            /* based on next bit branch left or right */
            if (trie->child[bit]) {
                rv = _trie_skip_node_free(trie->child[bit], key,
                        length - lcp - 1);
                if (SOC_SUCCESS(rv)) {
    ...
    *length = 0;
    ...
    if (trie->trie) {
        ...
        if (SOC_SUCCESS(rv) && (trieSplitStateDone == state)) {
            ...
            assert(split_count > 0);
            if (trie->trie == NULL) {
                alpm_lib_trie_t *c1, *c2;
                alpm_lib_trie_init(48, &c1);
                alpm_lib_trie_init(48, &c2);
                c1->trie = child;
                c2->trie = *split_trie_root;
                LOG_ERROR(BSL_LS_SOC_ALPM,
                          (BSL_META("dumping the 2 child trees\n")));
                alpm_lib_trie_dump(c1, 0, 0);
                alpm_lib_trie_dump(c2, 0, 0);
            }
            /* update the child pointer if child was pruned */
            if (child != NULL) {
                trie->trie = child;
            ...
            alpm_lib_trie_node_t **new_parent)
{
    ...
    if (!parent_trie || length == 0 || !pivot || !new_parent || (length > _MAX_KEY_LEN_48_))
        return SOC_E_PARAM;
