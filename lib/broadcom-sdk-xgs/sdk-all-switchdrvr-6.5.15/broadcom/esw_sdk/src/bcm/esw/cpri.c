    ...
    portmod_cpri_axc_frame_info_t *axc_info_t)
{
    ...
    if (direction == BCM_CPRI_DIR_TX && axc_info->compress_mode != bcmCpriFrameCompressModeNone) {
      if (axc_info->compress_mode == bcmCpriFrameCompressModeCompress) {
        axc_info_t->roe_payload_info.packet_size = axc_info->num_samples_per_roe_pkt * (2*9);
        axc_info_t->roe_payload_info.last_packet_num = 0;
        axc_info_t->roe_payload_info.last_packet_size = axc_info->num_samples_per_roe_pkt * (2*9);
      } else if (axc_info->compress_mode == bcmCpriFrameCompressModeDecompress) {
        axc_info_t->roe_payload_info.packet_size = axc_info->num_samples_per_roe_pkt * (2*15);
        axc_info_t->roe_payload_info.last_packet_num = 0;
        axc_info_t->roe_payload_info.last_packet_size = axc_info->num_samples_per_roe_pkt * (2*15);
      }
    } else if (direction == BCM_CPRI_DIR_TX && axc_info->truncation_enable == 1) {
      if (axc_info->truncation_mode == bcmCpriFrameTruncMode15_to_16 || axc_info->truncation_mode == bcmCpriFrameTruncModeAdd_0) {
        axc_info_t->roe_payload_info.packet_size = axc_info->num_samples_per_roe_pkt * (2*16);
        axc_info_t->roe_payload_info.last_packet_num = 0;
        axc_info_t->roe_payload_info.last_packet_size = axc_info->num_samples_per_roe_pkt * (2*16);
      } else if(axc_info->truncation_mode == bcmCpriFrameTruncMode16_to_15 || axc_info->truncation_mode == bcmCpriFrameTruncModeAdd_1) {
        axc_info_t->roe_payload_info.packet_size = axc_info->num_samples_per_roe_pkt * (2*15);
        axc_info_t->roe_payload_info.last_packet_num = 0;
        axc_info_t->roe_payload_info.last_packet_size = axc_info->num_samples_per_roe_pkt * (2*15);
      }
    ...
    bcm_cpri_rx_vsd_flow_info_t *config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_rx_vsd_flow_get(unit, port, group_id, config));
...
int bcm_esw_cpri_port_tx_cw_vsd_ctrl_flow_config_get(
    int unit,
    int port,
    uint16 group_id,
    bcm_cpri_tx_vsd_flow_info_t *config)
{
    ...
    BCM_IF_ERROR_RETURN
        (portmod_cpri_port_tx_vsd_flow_get(unit, port, group_id, config));
