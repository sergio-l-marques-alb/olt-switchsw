    ...
    switch (type) {
        case bcmCosqInputPriPriorityGroupUcMapping:
            ...
            for (i = 0; i < array_count; i++) {
                ...
                soc_reg64_field_set(unit, MMU_THDI_UC_INPPRI_PG_PROFILEr, &rval,
                    field_pri[i], arg[i]);
            }
            SOC_IF_ERROR_RETURN
        ...
        case bcmCosqInputPriPriorityGroupMcMapping:
            ...
            for (i = 0; i < array_count; i++) {
                ...
                soc_reg64_field_set(unit, MMU_THDI_NONUC_INPPRI_PG_PROFILEr, &rval,
                    field_pri[i], arg[i]);
            }
            SOC_IF_ERROR_RETURN
        ...
        case bcmCosqPriorityGroupServicePoolMapping:
            ...
            for (i = 0; i < array_count; i++) {
                ...
                soc_reg64_field_set(unit, MMU_THDI_PG_PROFILEr, &rval,
                    field_spid[i], arg[i]);
            }
            SOC_IF_ERROR_RETURN
        ...
        case bcmCosqServicePoolOverrideMcPkt:
            ...
            SOC_IF_ERROR_RETURN
                (soc_reg_get(unit, MMU_THDI_MC_SPID_OVERRIDE_CTRLr,
                             REG_PORT_ANY, -1, &rval));
            soc_reg64_field_set(unit, MMU_THDI_MC_SPID_OVERRIDE_CTRLr, &rval,
                    SPIDf, service_pool);
       ...
       (!gport_type && (cosq >= _bcm_th3_get_num_ucq(unit)))) {
        switch (type) {
            case bcmCosqControlEgressPortPoolYellowLimitBytes:
                fld_limit = SP_YELLOW_LIMITf;
                granularity = 8;
                num_mem = sizeof(mmu_thdo_config_portsp_mc_mem) /
                          sizeof(mmu_thdo_config_portsp_mc_mem[0]);
                mem_list = mmu_thdo_config_portsp_mc_mem;
                break;
            case bcmCosqControlEgressPortPoolRedLimitBytes:
                fld_limit = SP_RED_LIMITf;
                granularity = 8;
            ...
            case bcmCosqControlEgressPortPoolSharedResumeBytes:
                fld_limit = SHARED_RESUMEf;
                granularity = 8;
                num_mem = sizeof(mmu_thdo_resume_port_mc_mem) /
                          sizeof(mmu_thdo_resume_port_mc_mem[0]);
                mem_list = mmu_thdo_resume_port_mc_mem;
                break;
            case bcmCosqControlEgressPortPoolYellowResumeBytes:
                fld_limit = YELLOW_RESUMEf;
                granularity = 8;
        ...
        for (loop = 0; loop < 3; loop++) {
           switch (type) {
               ...
               case bcmCosqControlEgressPortPoolSharedLimitBytes:
                   ...
                   break;
               case bcmCosqControlEgressPortPoolSharedResumeBytes:
                   mem = mmu_thdo_resume_port_uc_mem[loop];
                   fld_limit = SHARED_RESUMEf;
                   granularity = 8;
                   break;
               case bcmCosqControlEgressPortPoolRedResumeBytes:
...
_bcm_th3_adjust_delta_hdrm_service_pools(int unit, int itm, int hdrm_pool, int
    ...
    uint32 old_hp_limit[_TH3_ITMS_PER_DEV],
