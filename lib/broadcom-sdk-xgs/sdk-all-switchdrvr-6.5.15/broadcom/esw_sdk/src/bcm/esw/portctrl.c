...
bcmi_esw_portctrl_llfc_set(int unit, bcm_port_t port, bcm_port_control_t type, int value)
{
    ...
    case bcmPortControlSAFCReceive:
        ...
        if (SOC_IS_XGS3_SWITCH(unit)) {
            ...
            if ((!IS_HG_PORT(unit, port)) && (!IS_CPRI_PORT(unit, port))) {
                return BCM_E_UNAVAIL;
            }
            PORT_LOCK(unit);
            rv = portmod_port_llfc_control_get(unit, port, &control);
                                          ...
                                          int value)
{
    ...
    if (SOC_REG_IS_VALID(unit, XPORT_TO_MMU_BKPr) && (pfc_enable == 0)) {
        ...
        BCM_IF_ERROR_RETURN(WRITE_XPORT_TO_MMU_BKP_64r(unit, port, 0));
    }
                                 ...
                                 bcm_port_phy_tx_t *tx)
{
    ...
    rv = soc_esw_portctrl_phy_tx_set(unit, pport, phyn, phy_lane, sys_side, (soc_port_phy_tx_t *)tx);
    PORT_UNLOCK(unit);
                                 ...
                                 bcm_port_phy_tx_t *tx)
{
    ...
    PORT_LOCK(unit);
    rv = soc_esw_portctrl_phy_tx_get(unit, pport, phyn, phy_lane, sys_side, (soc_port_phy_tx_t *)tx);
