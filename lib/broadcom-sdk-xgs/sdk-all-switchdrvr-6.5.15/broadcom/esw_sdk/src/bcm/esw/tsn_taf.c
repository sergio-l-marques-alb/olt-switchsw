...
STATIC bcmi_tsn_taf_profile_t *taf_profile_pending
                               [SOC_MAX_NUM_DEVICES][BCMI_TSN_TAF_GATE_NUM];
STATIC bcmi_tsn_taf_profile_t *taf_profile_active
                               [SOC_MAX_NUM_DEVICES][BCMI_TSN_TAF_GATE_NUM];
    ...
    )
{
    ...
    SHR_BITDCL gate_bitmap[_SHR_BITDCLSIZE(BCMI_TSN_TAF_GATE_NUM)];
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        rv = TSN_TAF_FUNC(
                interrupt_handle_switch,
                    (unit, &need_to_handle,
                     gate_bitmap, _SHR_BITDCLSIZE(BCMI_TSN_TAF_GATE_NUM)));
    }

    if (BCM_SUCCESS(rv) && need_to_handle) {
        SHR_BIT_ITER(gate_bitmap, BCMI_TSN_TAF_GATE_NUM, taf_gate) {
            uint32 use_ptp;
    ...
    if (BCM_SUCCESS(rv) && need_to_handle) {
        for (taf_gate = 0; taf_gate < BCMI_TSN_TAF_GATE_NUM; taf_gate++) {
            uint32 use_ptp;
...
bcmi_esw_tsn_taf_init(int unit)
{
    ...
    bcmi_taf_gate_pri_map_info.table_size = TSN_TAF_DATA(gate_profile_num);
