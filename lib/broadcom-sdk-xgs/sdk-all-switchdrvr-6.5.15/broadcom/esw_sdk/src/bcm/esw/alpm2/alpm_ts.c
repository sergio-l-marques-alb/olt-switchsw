    ...
    for (i = 0; i < ACB_CNT(u); i++) {
        ...
        for (vrf = 0; vrf <= ALPM_VRF_ID_MAX(u); vrf++) {
            ...
            for (ipt = 0; ipt < ALPM_IPT_CNT; ipt++) {
                ...
                (void)bcm_esw_alpm_pvt_traverse(u, i, vrf, ipt,
        ...
        for (vrf = 0; vrf <= ALPM_VRF_ID_MAX(u); vrf++) {
            ...
            for (ipt = 0; ipt < ALPM_IPT_CNT; ipt++) {
                ...
                if (ACB_HAS_RTE(acb, vrf)) {
                    ALPM_INFO(("\n"));
                    (void)bcm_esw_alpm_pfx_traverse(u, vrf, ipt,
            ...
            if (ACB_HAS_RTE(acb, vrf_id)) {
                ...
                if (ALPMC(u)->_alpm_dbg_info_bmp & _ALPM_DBG_INFO_RTE) {
                    ...
                    if (!ALPM_HIT_SKIP(u)) {
                        int hit_val;
                        ALPM_HIT_LOCK(u);
                        hit_val = alpm_rte_hit_get(u, vrf_id, bkt_tbl, idx1, eid);
                        ALPM_HIT_UNLOCK(u);
...
alpm_pfx_hw_route_cb(_alpm_pfx_node_t *pfx_node, void *user_data)
{
    ...
    if (err) {
        ...
        ALPM_ERR(("PFX %p->BKT %p: IP:%s Sublen:%d Dest:%d => ",
                  pfx_node, bkt_node, ip_buf, defip_sub_len, dest));
        ...
        if (err == 1) {
            ...
            ALPM_ERR(("   Current best match Hw route: IP:%s Sublen:%d Dest:%d\n",
                      ip_buf, lpm_cfg.defip_sub_len, nh_ecmp_idx));
        }
 ...
 */
int
alpm_pfx_hw_route_sanity(int u, int *tot_err)
{
    int vrf_id, ipt;
    _alpm_trv_data_t trv_data;
...
bcm_esw_alpm_sanity_check(int u, soc_mem_t mem, int index, int check_sw)
{
    ...
    if (check_sw == 0) {
        rv = alpm_pfx_hw_route_sanity(u, &tot_error);
    }
...
{
    LOG_CLI((BSL_META_U(u, "\n  ALPM Info -\n")));
    bcm_esw_alpm_debug_brief_show(u, 0, 0);
    alpm_cb_stat_dump(u, 3);
}
