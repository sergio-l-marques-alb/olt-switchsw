...
int _bcm_common_init(int unit)
{
   ...
   if (!soc_feature(unit,soc_feature_advanced_flex_counter)) {
    ...
    if(soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        ...
        FT_PDD_PROFILE(unit) = sal_alloc(sizeof(soc_profile_mem_t), "pdd Profile");
        ...
        entry_words = sizeof(bsd_policy_action_profile_entry_t) / sizeof(uint32);

        rv = soc_profile_mem_create(unit, &mem, &entry_words, 1, FT_PDD_PROFILE(unit));
