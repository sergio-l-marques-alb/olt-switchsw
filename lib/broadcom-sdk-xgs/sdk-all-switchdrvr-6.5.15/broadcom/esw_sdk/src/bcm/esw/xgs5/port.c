                                ...
                                int *op)
{
    ...
    pbmp_t att_pbm, remap_pbm, lane_pbm, speed_pbm, encap_pbm;
#if defined(PORTMOD_SUPPORT) && defined(PORTMOD_PM8X50_SUPPORT)
    portmod_dispatch_type_t pm_type;
    int autoneg = 0;
#endif
...
bcmi_xgs5_port_attach_stg(int unit, bcm_port_t port)
{
    /* STG */
    BCM_IF_ERROR_RETURN
        (bcm_esw_vlan_stp_set(unit, BCM_VLAN_DEFAULT, port, BCM_STG_STP_FORWARD));
    return BCM_E_NONE;
}
...
bcmi_xgs5_port_detach_stg(int unit, bcm_port_t port)
{
    /* STG */
    BCM_IF_ERROR_RETURN
        (bcm_esw_vlan_stp_set(unit, BCM_VLAN_DEFAULT, port, BCM_STG_STP_DISABLE));
    return BCM_E_NONE;
}
