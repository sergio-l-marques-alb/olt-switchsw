    ...
    while (TRUE) {
        if (sal_sem_take(latency_monitor_mem[unit][monitor_id].dma_handle_counter_sem, sal_sem_FOREVER) == 0) {
            ...
            if (dbuf) {
                        ...
                        for (k=0; k<LATENCY_MONITOR_BUCKET_COUNT; k++) {
                            ...
                            if (latency_monitor_mem[unit][monitor_id].monitor_mode == bcmLatencyMonitorTimeSeriesMode) {
                                ...
                                if (pipe_lower != -1) {
                                    ...
                                    COMPILER_64_COPY(byte_count_tmp, (sw_buff[sw_buf_idx] >> pkt_ctr_f_width));
                                    packet_count_tmp = (sw_buff[sw_buf_idx]) & ((1 << pkt_ctr_f_width) - 1);
                ...
                /* Call the callback handler after accumilation for all the pipes */
                if ((latency_monitor_mem[unit][monitor_id].monitor_mode == bcmLatencyMonitorTimeSeriesMode) &&
                    (latency_monitor_mem[unit][monitor_id].pipe_map_temp == 0) &&
                    (latency_monitor_mem[unit][monitor_id].cb)) {
                    latency_monitor_mem[unit][monitor_id].cb(unit, monitor_id, latency_monitor_mem[unit][monitor_id].status,
                                            (void *)sw_buff, latency_monitor_mem[unit][monitor_id].sw_buf_size, sw_buf_idx);
                }

                /* Counter accumilation pending for other pipes. Back track the sw_buf_wr_idx index in S/W Ring buffer */
                if ((latency_monitor_mem[unit][monitor_id].monitor_mode == bcmLatencyMonitorTimeSeriesMode) &&
                    (latency_monitor_mem[unit][monitor_id].pipe_map_temp != 0)) {
    ...
    while (latency_monitor_driver_data.run[monitor_id]) {
        ...
        for (; frag < latency_monitor_mem[unit][monitor_id].frag_count; frag++) {
            ...
            if (_check_for_wraparound_condition(unit, monitor_id, frag) != SOC_E_NONE) {
                ...
                if (latency_monitor_mem[unit][monitor_id].cb) {
                    latency_monitor_mem[unit][monitor_id].cb(unit, monitor_id, LATENCY_MONITOR_STATE_ERROR, NULL,0,0);
                }
                BCM_EXIT;
            }
    ...
    bcm_latency_monitor_config_t *config)
{
    ...
    if (latency_monitor_mem[unit][monitor_id].status != LATENCY_MONITOR_STATE_CLEANUP) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_esw_stat_group_create(unit, stat_obj,
        ...
        latency_monitor_mem[unit][monitor_id].cb = NULL;
        latency_monitor_mem[unit][monitor_id].sw_buf_size = 0;
        pool_number = 0;
        base_idx = 0;
        time_step_incr = 0;
    ...
    bcm_latency_monitor_config_t *config)
{
    ...
    if (config->mode == bcmLatencyMonitorTimeSeriesMode) {
        ...
        config->cb = latency_monitor_mem[unit][monitor_id].cb;
    }
    ...
    uint8 enable)
{
    ...
    if (enable) {
        ...
        hist_1us_enable = soc_reg64_field_get(unit, AUX_ARB_CONTROLr, rval, HIST_1US_ENABLEf);
        if (!hist_1us_enable) {
    ...
    bcm_latency_monitor_histogram_t *histogram)
{
    ...
    for (i=0; i<histogram->histogram_size; i++) {
        if (byte_counter_flag)
            histogram->histogram[i] = arr_byte_cnt[cosq][i];
        else
            histogram->histogram[i] = arr_pkt_cnt[cosq][i];
    }
#endif
