                                       ...
                                       int *slice, int *slice_idx)
{
   ...
   /* Input parameters check. */
...
#if defined (BCM_TOMAHAWK3_SUPPORT)
    if (SOC_IS_TOMAHAWK3(unit)) {
        return (_bcm_field_th3_tcam_idx_to_slice_offset (unit, stage_fc,
                                                         f_ent, tcam_idx,
                                                         slice,
                                                         slice_idx));
    }
    ...
    max_entries_in_slice = stage_fc->tcam_sz / stage_fc->tcam_slices;

    if (!(f_ent->group->flags & _FP_GROUP_SPAN_SINGLE_SLICE) ||
        (f_ent->group->flags & _FP_GROUP_INTRASLICE_DOUBLEWIDE)) {
        max_entries_in_slice = max_entries_in_slice/2;
    }
    ...
    if (slice_num == stage_fc->tcam_slices) {
        ...
        for (slice_num = 0; slice_num < stage_fc->tcam_slices; slice_num++) {
            ...
            for (fs_next = &fg->slices[0]; fs_next; fs_next = fs_next->next) {
                ...
                if (fs_next->slice_number == slice_num) {
                    curr_grp_slice = 1;
                    break;
                }
            }
