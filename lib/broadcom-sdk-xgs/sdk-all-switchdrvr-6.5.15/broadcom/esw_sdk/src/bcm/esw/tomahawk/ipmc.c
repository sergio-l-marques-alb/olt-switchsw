...
#define REPL_HEAD_TBL_SPLIT(_u_, _split)                                \
    (_split == 0 ? MMU_REPL_HEAD_TBL_SPLIT0m : MMU_REPL_HEAD_TBL_SPLIT1m)

#define REPL_LIST_TBL_PIPE(_u_, _pipe_)                                 \
    (SOC_MEM_UNIQUE_ACC(_u_, MMU_REPL_LIST_TBLm)[_pipe_])

#define REPL_LIST_TBL(_u_)  (MMU_REPL_LIST_TBLm)
...
_bcm_th3_repl_head_info_init(int unit)
{
    ...
    REPL_HEAD_FREE_LIST(unit, 0, 0)->size = repl_head_tbl_size - 1;
...
_bcm_td3_repl_head_tbl_write(int unit, int pipe, int index, uint32 *entry)
{
    ...
    if (soc_feature(unit, soc_feature_split_repl_head_table)) {
        repl_hd_tbl = REPL_HEAD_TBL_SPLIT_PIPE(unit, 0, pipe);
        if (index >= max_index_offset * 1024) {
            index -= max_index_offset * 1024;
            repl_hd_tbl = REPL_HEAD_TBL_SPLIT_PIPE(unit, 1, pipe);
        }
                            ...
                            int if_max, bcm_if_t *if_array, int *if_count)
{
    ...
    while (repl_entry_ptr != prev_repl_entry_ptr) {
        ...
        if(SOC_IS_TOMAHAWK3(unit)) {
            ...
            nh_mode = soc_MMU_REPL_LIST_TBLm_field32_get(unit, &repl_list_entry,
                            MODEf);
            if(nh_mode) {
                /* sparse mode */
                int i;
