    ...
    uint8                       *channel)
{
    ...
    /* This is workaround: in case of working with multiple CMCs for the CPU, we will save all 4 channels in the PCI_CMC (SOC_PCI_CMC(unit)) to other applications */
   ...
   ) 
{
    ...
    int engine_configured[MAX_NUM_OF_CORES];
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if(command_id != -1)
    {
        SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_id)
        {
            ...
            engine_configured[core_id] = FALSE;
            ...
            {      
                ...
                {
                    engine_configured[core_id] = TRUE;                
            ...
            /* if at least one engine was configure to count out lif from specific core, we verify that the range is match */
            /* case that one core wasn't configured at all is valid case */
            if((engine_configured[core_id] == TRUE) && (start_range_match == FALSE || end_range_match == FALSE))
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, 
                    (_BSL_BCM_MSG("OUT LIF profile range {%d..%d}, command_id=%d, is_double_entry=%d has no corresponding range in core=%d\n"), 
                    range->start, range->end, command_id, is_double_entry, core_id));          
            }
        }
        SOC_DPP_CORES_ITER(SOC_CORE_ALL, core_id)
        {
            if(engine_configured[core_id] == TRUE)
            {
                break;
            }
        }
...
bcm_dpp_counter_lif_range_set(
   ...
   int engine_range_offset
    ...
    if(SOC_IS_JERICHO_PLUS(unit) && source == SOC_TMC_CNT_SRC_TYPE_EPNI)
    {
