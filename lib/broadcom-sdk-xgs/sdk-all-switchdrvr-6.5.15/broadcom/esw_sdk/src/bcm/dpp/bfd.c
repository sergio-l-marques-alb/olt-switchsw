...
static volatile bcm_dpp_bfd_tx_rates_t update_bfd_tx_rates;
                                              ...
                                              _bcm_bfd_mep_db_entry_pointers_to_free * pointers_to_free)
{
   int is_allocated;
    ...
    if (existing_endpoint_info == NULL || existing_tx_rate != tx_rate) {

        if ((existing_endpoint_info != NULL) &&
            (existing_tx_rate != tx_rate) &&
            (update_bfd_tx_rates.ref_count[mep_db_entry->ccm_interval] == 1) &&
            ...
            int        tx_rate_index = 0, temp_rv = BCM_E_MEMORY;
            uint32     temp_tx_rate_value = 0, tx_rate_ref_counter = 0;
            ...
            for (tx_rate_index = 0; tx_rate_index < 8; ++tx_rate_index) {
                ...
                if (tx_rate == temp_tx_rate_value) {
                    ...
                    BCMDNX_IF_ERR_EXIT(rv);
                    mep_db_entry->ccm_interval = tx_rate_index;
            ...
            if (temp_rv != BCM_E_NONE) {
                ...
                update_bfd_tx_rates.tx_rate[mep_db_entry->ccm_interval] = tx_rate;
                rv = BFD_ACCESS.tx_rate_value.set(unit,
                                                  mep_db_entry->ccm_interval,
                                                  tx_rate);
                BCMDNX_IF_ERR_EXIT(rv);
            ...
            rv = _bcm_bfd_tx_rate_profile_alloc(unit, tx_rate, &mep_db_entry->ccm_interval);
            BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if (is_default) {
            ...
            if (*is_endpoint_acc_to_oamp) {
                ...
                if (SOC_IS_QUX(unit) && (endpoint_info_lcl->remote_discr & 0x300)) {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,
                           (_BSL_BCM_MSG("The bits 8 and 9 has been reserved in remote discriminator.")));
                }
            }
        }
    }
    ...
    if (endpoint_info_lcl->flags & BCM_BFD_ENDPOINT_UPDATE) {
        ...
        if (is_default) {
            ...
            if (is_endpoint_acc_to_oamp &&
                ...
                if (endpoint_info_lcl->type == bcmBFDTunnelTypeMpls && (!_BCM_BFD_ENDPOINT_IS_SBFD(unit, endpoint_info_lcl))) {
                    soc_sand_rv = soc_ppd_oam_bfd_mpls_udp_sport_get(unit, &udp_sport);
                    BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                    if (udp_sport == 0) {
