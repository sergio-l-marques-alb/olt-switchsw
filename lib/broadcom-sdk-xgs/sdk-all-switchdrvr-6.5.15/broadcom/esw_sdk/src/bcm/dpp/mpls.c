    ...
    int unit)
{
    ...
    if (SOC_IS_JERICHO(unit) && (!SOC_IS_JERICHO_PLUS(unit)) &&
        ...
        if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "egress_mpls_entropy_label_on_non_bos_enable", 1)) {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_NON_BOS_INDICATION,
                                                                               ARAD_EGR_PROG_EDITOR_PROG_MPLS_NON_BOS_ELI_JER, 1/*entry*/,
                                                                              1 /*value*/,1/*outlif-profile1*/);
            BCMDNX_IF_ERR_EXIT(rv);
            ...
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                               ARAD_EGR_PROG_EDITOR_PROG_MPLS_NON_BOS_ELI_JER, 2/*entry*/,
                                                                              1 /*value*/,2/*outlif-profile1*/);
            BCMDNX_IF_ERR_EXIT(rv);

            if (soc_property_get(unit, spn_MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE, 0)) {
                rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_NON_BOS_INDICATION,
                                                                                   ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_NON_BOS_ELI_JER, 1/*entry*/,
                                                                                  1 /*value*/,1/*outlif-profile1*/);
                BCMDNX_IF_ERR_EXIT(rv);
                
                rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                                   ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_NON_BOS_ELI_JER, 2/*entry*/,
                                                                                  1 /*value*/,2/*outlif-profile1*/);
                BCMDNX_IF_ERR_EXIT(rv);
    ...
    */
	...
	for (i = 2;i < 256; i++)
	{
	    tunnel_term_ip_lif_dummy.if_class = i;
    ...
    uint8 is_second_mpls_label_in_entry)
{
    ...
    if (label_entry->flags & BCM_MPLS_EGRESS_LABEL_ENTROPY_ENABLE) {
        /*Only JR support ELI flag alone at seconde tunnel, EL is inserted by uCode*/
        if (SOC_IS_JERICHO_AND_BELOW(unit)) {
            profile_info.add_entropy_label = !is_second_mpls_label_in_entry?1:0;
        }else {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (ilm_val->additional_info.eei.type == SOC_PPC_EEI_TYPE_MPLS) {
          ...
          if (ilm_val->additional_info.eei.val.mpls_command.command ==
              ...
              info->flags |= (ilm_val->additional_info.eei.val.mpls_command.pop_next_header == SOC_TMC_PKT_FRWRD_TYPE_IPV6_UC)?BCM_MPLS_SWITCH_NEXT_HEADER_IPV6:
                             (ilm_val->additional_info.eei.val.mpls_command.pop_next_header == SOC_TMC_PKT_FRWRD_TYPE_IPV4_UC)?BCM_MPLS_SWITCH_NEXT_HEADER_IPV4:0;
          }
...
exit:
    if((_rv != BCM_E_NONE) && lif_is_alloc) {
        ...
        _bcm_mpls_tunnel_lif_term_profile_exchange(unit, local_in_lif_id, 0x0, /* Default */ 0, NULL);
