    ...
    bcm_gport_t    port)
{
    ...
    if(lif_info->type & (SOC_PPC_LIF_ENTRY_TYPE_AC|SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP|SOC_PPC_LIF_ENTRY_TYPE_MIDDLE_AC_IN_GROUP)){
        ...
        /* if ac is multicast port and ac is primary ac,
         * remove FEC HW configuration and free FEC SW resource
         */
        if (BCM_GPORT_SUB_TYPE_IS_MULTICAST(port)) {
            SOC_PPC_FEC_ID fec_id = _BCM_GPORT_ENCAP_ID_LIF_INVALID;
            ...
            if ((fec_id != _BCM_GPORT_ENCAP_ID_LIF_INVALID) && BCM_GPORT_SUB_TYPE_MULTICAST_PS_GET(port)) {
                rv = bcm_dpp_am_fec_dealloc(unit, _BCM_DPP_AM_FEC_ALLOC_USAGE_STANDARD, 1, fec_id);
                BCMDNX_IF_ERR_EXIT(rv);
