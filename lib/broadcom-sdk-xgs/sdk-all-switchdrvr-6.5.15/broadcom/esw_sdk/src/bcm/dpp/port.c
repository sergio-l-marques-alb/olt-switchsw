...
bcm_petra_port_init(int unit)
{
  ...
  if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "20byte_prefix_header_enable", 0)) {
      ...
      char prefix_property[BCM_DPP_20B_PREFIX_PROPERTY_LENGTH + 1];
      uint32 prefix_data_entry[4];
      SOC_PPC_EG_ENCAP_DATA_INFO data_info;

      SOC_PPC_EG_ENCAP_DATA_INFO_clear(&data_info);
      sal_memset(prefix_property, 0, BCM_DPP_20B_PREFIX_PROPERTY_LENGTH + 1);
      sal_strncpy(prefix_property, "20byte_prefix_header_0_0", BCM_DPP_20B_PREFIX_PROPERTY_LENGTH);
    ...
    switch (type)
    {
        ...
        case bcmPortControlReserved281:
            if (SOC_DPP_CONFIG(unit)->pp.custom_feature_vrrp_scaling_tcam) {
                ...
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
                vtt_in_pp_port_conf_tbl.vlan_translation_profile = value ? SOC_PPC_PORT_DEFINED_VT_PROFILE_VRRP_PORT : SOC_PPC_PORT_DEFINED_VT_PROFILE_DEFAULT;
 ...
 * Purpose:
 *      Wrapper to _bcm_petra_port_link_get, to allow also activate ILKN first packet SW
 *      workaround without harming the BCM API.
 *      
 *      The ILKN first packet SW workaround is activated only if:
...
int bcm_petra_port_remove(int unit, bcm_port_t port, uint32 flags) 
{
    ...
    int              base_q_pair, profile, counter_interval, is_channelized, i, port_j;
    ...
    if (!is_channelized) {
        ...
        /* Update EGQ interface bandwidth */
        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_get(unit,fap_port,0,&dummy1,&kbits_sec_max_get,&dummy2));
        kbits_sec_max_get -= egq_rate*1000;
        BCMDNX_IF_ERR_EXIT(bcm_petra_cosq_gport_bandwidth_set(unit,fap_port,0,0,kbits_sec_max_get,0));
    }
    ...
    if (!IS_LBG_PORT(unit, port)) {
        ...
        /* clear link override for port. For ilkn port, forcing link override will disable the ILKN first packet SW bypass*/
        BCMDNX_IF_ERR_EXIT(_bcm_link_force(unit, port, FALSE, FALSE));
    }
                                       ...
                                       bcm_pbmp_t*               affected_ports )
{
    ...
    if (is_ilkn) {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_port_quad_get,(unit,local_port_internal,&quad_bmp)));
        ...
        case bcmPortControlMacInMac:
              ...
              if (value == 0 || value == 1) { /* disable/enable MiM */ 
                  ...
                  BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_ndx) {
                    ...
                    if (SOC_DPP_CONFIG(unit)->pp.test1 == 1 || SOC_DPP_CONFIG(unit)->pp.test2) {
        ...
        case bcmPortControlFloodUnknownUcastGroup:
          {
            ...
            if (gport_parse_info.type == _bcmDppGportParseTypeSimple) {
                ...
                if (BCM_GPORT_IS_VLAN_PORT(port) || BCM_GPORT_IS_MPLS_PORT(port) || BCM_GPORT_IS_EXTENDER_PORT(port) || BCM_GPORT_IS_VXLAN_PORT(port)) {
   ...
   bcm_error_t *validation_code)
{
    ...
    *validation_code = BCM_E_NONE;
    ...
    if (is_in_lif_discard && (cur_discard_state != _BCM_INGRESS_PROTECTION_POINTER_DEFAULT)) {
                                  ...
                                  bcm_port_timesync_tx_info_t *tx_info)
{
    ...
    if (soc_feature(unit, soc_feature_timesync_support)) {
       ...
       portmod_port_timesync_tx_info_get(unit, port, &info);
