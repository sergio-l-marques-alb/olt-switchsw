 ...
 ) 
{
    ...
    if (SOC_DPP_CONFIG(unit)->pp.ip_lif_dummy_id &&
        ...
        /*Explicit NULL WA and IP share the dummy IP LIF*/
    ...
    /* Indicate it's IP-LIF-Dummy for explicit null
	 *  TTL > 1 use same LIF with IP dummy,
	 *  TTL< 2 use another LIF for customer special process
	 */
    if (SOC_IS_JERICHO(unit) && SOC_DPP_CONFIG(unit)->pp.explicit_null_support && (info->flags & BCM_TUNNEL_TERM_TUNNEL_WITH_ID)) {
        term_key->flags |= SOC_PPC_RIF_IP_TERM_FLAG_USE_DIP_EXPLICIT_NULL_DUMMY;
        term_key->ttl = info->if_class;
        if ((SOC_DPP_CONFIG(unit)->pp.explicit_null_support_lif_id == BCM_GPORT_TUNNEL_ID_GET(info->tunnel_id)) && term_key->ttl != 2) {
            term_key->flags |= SOC_PPC_RIF_IP_TERM_FLAG_KEY_ONLY;
        } else if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "logical_interface_for_explicit_null_ttl0", 0) == BCM_GPORT_TUNNEL_ID_GET(info->tunnel_id)) && term_key->ttl > 0 ) {
            term_key->flags |= SOC_PPC_RIF_IP_TERM_FLAG_KEY_ONLY;
        }
    }
    ...
    bcm_tunnel_terminator_t *info) 
{
    ...
    if (update) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            LOG_CLI((BSL_META("_bcm_petra_tunnel_terminator_add: global_lif 0x%08x local_inlif_info.base_lif_id 0x%08x\n"), global_lif, local_inlif_info.base_lif_id));
