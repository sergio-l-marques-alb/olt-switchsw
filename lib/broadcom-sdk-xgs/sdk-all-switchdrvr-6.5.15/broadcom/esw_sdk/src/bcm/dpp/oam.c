 ...
 * In case of SLM offset increases by 8
 *  */ 
#define _OAM_SET_LMM_OFFSET(mep_db_entry, value, is_piggy_back, is_slm) \
    do {value = (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ? mep_db_entry.tags_num *4 + 0x12 : \
     ((mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) || (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) || (mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1711_MPLS)||(mep_db_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1711_PWE))? 0x10 : 0x0c /*PWE offset */;\
	if (is_piggy_back) {value += 54 ;} \
    else if (is_slm) {value += 8 ;} }while (0)
...
int bcm_petra_oam_init(int unit) {
    ...
    bcm_gport_t oamp_port[SOC_DPP_DEFS_MAX(NOF_CORES)], cpu_port, recycle_port, recycle_port_iter , lbm_recycle_port[2];
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl)
{
    ...
    if ((endpoint_info_lcl->gport == BCM_GPORT_INVALID) &&
        ...
        if ((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit) && !is_mip) ||
           ...
           if (_BCM_OAM_IS_SERVER_CLIENT(endpoint_info_lcl)) {
              ...
              rv = _bcm_dpp_oam_bfd_trap_code_get_and_prepare_for_action(unit, &classifier_mep_entry, endpoint_info_lcl->remote_gport, BCM_GPORT_INVALID, &trap_code, &trap_code);
              BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (is_accelerated) {
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            /*Set DM offsets */
            _OAM_SET_DMM_OFFSET(mep_db_entry, eth_prof.dmm_offset);
            eth_prof.dmm_offset = eth_prof.dmm_offset + user_header_size;
            _OAM_SET_DMR_OFFSET(mep_db_entry, eth_prof.dmr_offset);
            eth_prof.dmr_offset = eth_prof.dmr_offset + user_header_size;
            /*Set LM offsets */
            _OAM_SET_LMM_OFFSET(mep_db_entry,eth_prof.lmm_offset, eth_prof.piggy_back_lm, eth_prof.slm_lm );
            eth_prof.lmm_offset = eth_prof.lmm_offset + user_header_size;
            _OAM_SET_LMR_OFFSET(mep_db_entry,eth_prof.lmr_offset,  eth_prof.piggy_back_lm, eth_prof.slm_lm);
            eth_prof.lmr_offset = eth_prof.lmr_offset + user_header_size;
                ...
                if (!((mep_info.lif == _BCM_OAM_INVALID_LIF) &&
                    ...
                    if (!found) { /* No more meps on this lif */
                        ...
                        mep_type = (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ? bcmOAMEndpointTypeEthernet :
                            ...
                            (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL) ? bcmOAMEndpointTypeBHHPweGAL : -1;
    ...
    int is_sat_loopback)
{
    ...
    rv = _bcm_dpp_oam_bfd_trap_code_get_and_prepare_for_action(unit, &classifier_mep_entry, action_lcl->destination, action_lcl->destination2, &trap_code, &trap_code2);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    if (!is_mip) {
        ...
        profile_data->is_slm = is_slm = BCM_OAM_ACTION_GET(*action_lcl, bcmOAMActionSLMEnable) ? 1 : 0;
        if (is_accelerated) {
            profile_data_acc->is_slm = is_slm;
...
exit:
    ...
    _BCM_RX_CONVERT_SW_GPORT_TO_HW_GPORT(unit,action->destination2);
...
int _manage_DA_with_existing_entries_internal(int unit , int mep_id ,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb, 
                                              SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY *eth_1731_profile, int is_update, uint8* is_ok, uint8 is_dm) {
 ...
 * @return int 
 */
int  _manage_DA_with_existing_entries_loopback(int unit,int mep_id, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb ,  uint8 * is_ok) {
	return _manage_DA_with_existing_entries_internal(unit,mep_id,classifier_mep,msb,lsb,NULL,
													 classifier_mep->flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_HAS_LOOPBACK_REPLY,is_ok, 0);
}
...
int  _manage_DA_with_existing_entries_loss(int unit, int mep_id,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb , 
										    SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY * eth_1731_profile,int is_update, uint8 * is_ok) {
    return _manage_DA_with_existing_entries_internal(unit, mep_id, classifier_mep, msb, lsb, eth_1731_profile, is_update, is_ok, 0/*is dm*/ );
}
...
int  _manage_DA_with_existing_entries_delay(int unit, int mep_id,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb , 
										    SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY * eth_1731_profile,int is_update, uint8 * is_ok) {
	return _manage_DA_with_existing_entries_internal(unit,mep_id,classifier_mep,msb,lsb,eth_1731_profile, is_update,is_ok, 1/*is dm*/ );
}
...
*/
int _bcm_oam_set_classifier_action(int unit, uint32 mep_id, bcm_oam_endpoint_action_t action,int is_lm,int is_1dm_or_piggy_back,SOC_PPC_OAM_MEP_TYPE mep_type ,int is_slm) {
...
int _bcm_oam_lm_dm_remove_shared(int unit,bcm_oam_endpoint_t endpoint_id , int is_lm, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    if (classifier_mep_entry->lif != _BCM_OAM_INVALID_LIF) { /* OAM server: do not set the classifier*/
        ...
        rv = _bcm_oam_set_classifier_action(unit,endpoint_id,action,is_lm, (using_1dm || (is_lm && old_profile->piggy_back_lm)) ,ccm_entry_type, old_profile->slm_lm/*is_slm*/);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
int bcm_petra_oam_loss_add(int unit, bcm_oam_loss_t *loss_ptr)
{
   ...
   /* 
    * SDK-126418 : Qux LM bug. There is a global SLM or LM mode. 
    * Check loss add accordingly. 
    * If global = SLM and loss add without SLM flag, throw E_PARAM
    * If global = LM and loss add with SLM flag, throw E_PARAM
    */
    ...
    if (!_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&classifier_mep_entry)) {
       if (loss_ptr->flags & BCM_OAM_LOSS_SINGLE_ENDED) {
        ...
        if (classifier_mep_entry.lif != _BCM_OAM_INVALID_LIF) { /*Have the classifier send LM packets to the OAMP, but only if the endpoint exists on this device.*/
            ...
            BCM_GPORT_TRAP_SET(action.destination, trap_code, 7, 0);
            rv = _bcm_oam_set_classifier_action(unit, loss_ptr->id, action, 1 /*is_lm */, (loss_ptr->flags & BCM_OAM_LOSS_SINGLE_ENDED) == 0, classifier_mep_entry.mep_type,(loss_ptr->flags & BCM_OAM_LOSS_SLM) == BCM_OAM_LOSS_SLM /* is_slm */);
            BCMDNX_IF_ERR_EXIT(rv);
        }
    }
...
int bcm_petra_oam_delay_add(int unit, bcm_oam_delay_t *delay_ptr) {
    ...
    if (classifier_mep_entry.lif != _BCM_OAM_INVALID_LIF) {
        ...
        rv = _bcm_oam_set_classifier_action(unit, delay_ptr->id, action, 0 /*is_lm */, delay_ptr->flags & BCM_OAM_DELAY_ONE_WAY, classifier_mep_entry.mep_type, old_eth1731_profile.slm_lm/* is_slm */);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
_bcm_oam_diag_iterate_cb(int unit, sw_state_htb_key_t key, sw_state_htb_data_t user_data) {
    ...
    if ( (*(int*)key != -2  ) && ((mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) || (mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
        ...
        (mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP_SECTION))) { /*found an oam endpoint*/
...
int _bcm_dpp_oam_bfd_diagnostics_endpoint_by_id(int unit, int endpoint_id) {
    ...
    if ((mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) || (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
        ...
        (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP_SECTION)) { /*found an oam endpoint*/
        rv = bcm_petra_oam_endpoint_get(unit, endpoint_id, &endpoint_info);
        BCMDNX_IF_ERR_EXIT(rv);
        if (endpoint_info.flags & BCM_OAM_ENDPOINT_REMOTE) {
