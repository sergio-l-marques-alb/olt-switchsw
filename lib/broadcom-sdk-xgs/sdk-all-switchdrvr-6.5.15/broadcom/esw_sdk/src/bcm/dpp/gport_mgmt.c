...
_bcm_dpp_gport_l2_intf_calculate_port_id(int unit, int act_fec, SOC_PPC_LIF_ID global_lif_id, void *gport, int gport_type){
    ...
    if (gport_type != BCM_GPORT_EXTENDER_PORT) {
        ...
        /* Input validation*/
        if (((is_forward_group || is_protected) && (!act_fec && !soc_property_suffix_num_get(unit,-1,spn_CUSTOM_FEATURE,"pon_without_rif_support",0)/*FEC can be 0 when custom_feature_pon_wihout_rif_support=1*/) )             /* Fec gport but no fec */
