    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if ((info->tunnel_if != BCM_IF_INVALID) && (info->tunnel_if != 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "tunnel_if parameter cannot be used in this api. Please use bcm_l3_ingress_create instead with intf_id as mpls tunnel.");
    }
    ...
    uint8 *is_last_term_profile_reference)
{
    ...
    if (is_last_term_profile_reference == NULL)
    {
        SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.allocate_single
                        (unit, _SHR_CORE_ALL, 0, &term_profile_info, NULL, term_profile,
                         is_first_term_profile_reference));
    }
    else
    {
        SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.exchange
                        (unit, _SHR_CORE_ALL, 0, &term_profile_info, old_term_profile, NULL, term_profile,
    ...
    lif_table_mngr_inlif_info_t * lif_info)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PROTECTION_POINTER, INST_SINGLE, protection_pointer);
    ...
    int qos_profile)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PROPAGATION_PROF, INST_SINGLE, propagation_profile);
/*  Disabled due to ARCH
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_BIER_MPLS, INST_SINGLE, bier_mpls_domain);
*/
    ...
    int *qos_profile)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_PROTECTION_PATH, INST_SINGLE, (uint32 *) protection_path));
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, term_profile, &ref_count, &term_profile_info));
    ...
    uint8 *is_last)
{
    ...
    SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.free_single(unit, _SHR_CORE_ALL, term_profile, is_last));
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    int in_global_lif, old_in_global_lif, protection_pointer = 0, failover_id = 0;
    dnx_mpls_termination_profile_t term_profile_info;
    ...
    if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_REPLACE))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_inlif_info(unit, old_local_inlif, old_entry_handle_id, &lif_info));
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                        (unit, old_entry_handle_id, &lif_info, &old_in_global_lif, (uint32 *) &old_term_profile,
                         &old_propag_profile, NULL, NULL, NULL, NULL, NULL));
    ...
    {
        DNX_FAILOVER_ID_GET(failover_id, info->failover_id);
        /** Encode IN-LIF protection pointer format from Failover ID */
        DNX_FAILOVER_ID_PROTECTION_POINTER_ENCODE(protection_pointer, failover_id);
        protection_path = info->failover_tunnel_id ? 0 : 1;
    }
    else
    {
        protection_pointer = dnx_data_failover.path_select.ing_no_protection_get(unit);
    }
    ...
    }
    {
        ...
        SHR_IF_ERR_EXIT(dnx_mpls_term_to_lif_table_manager_info(unit, in_global_lif,
                                                                DNX_IN_LIF_PROFILE_DEFAULT, term_profile,
                                                                propag_profile, info->vpn, protection_pointer,
        ...
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_allocate_local_inlif_and_set_hw
                        (unit, entry_handle_id, &local_inlif, &lif_info));
...
dnx_mpls_tunnel_switch_term_get(
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    int in_global_lif, qos_profile, failover_status, protection_pointer;
    uint32 term_profile;
    ...
    /** Get entry from MPLS IN-LIF table */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_IN_LIF_FORMAT_LSP, &entry_handle_id));
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_inlif_info(unit, local_inlif, entry_handle_id, &lif_info));

    SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                    (unit, entry_handle_id, &lif_info, &in_global_lif, &term_profile, &propag_profile, &info->vpn,
                     &protection_pointer, &info->failover_tunnel_id, &is_stat_enable, &qos_profile));
    ...
    DNX_FAILOVER_ID_PROTECTION_POINTER_DECODE(info->failover_id, protection_pointer);
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_manager_info_to_mpls_term
                    (unit, entry_handle_id, &lif_info, &in_global_lif, &term_profile, &propag_profile, NULL,
                     &protection_pointer, &protection_path, NULL, NULL));
    ...
    void *user_data)
{
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        while (!is_end)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get(unit, local_inlif, SOC_CORE_DEFAULT,
