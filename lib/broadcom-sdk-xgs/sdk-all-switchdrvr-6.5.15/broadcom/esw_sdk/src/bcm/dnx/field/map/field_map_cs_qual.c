    ...
    dbal_fields_e * cs_dnx_qual_p)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "No DNX CS qual for bcmFieldQualify%s\n on stage:%s",
    ...
    dnx_field_presel_qual_data_t * dnx_cs_qual_info)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(cs_qual_map_p->conversion_cb
                        (unit, core, &bcm_cs_qual_info->qual_value, &dnx_cs_qual_info->qual_value));
    }
    ...
    uint32 *cs_qual_index_p)
{
    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
        ...
        {
            if (cs_qual_map_p->field_id == dnx_cs_qual)
            {
    ...
    uint32 *cs_nof_lines_p)
{
     ...
     */
    cs_nof_lines = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_cs_lines;
    if (cs_nof_lines <= 0)
    {
    ...
    int unit)
{
    ...
    for (i_bcm_cs_qual = 0; i_bcm_cs_qual < bcmFieldQualifyCount; i_bcm_cs_qual++)
    {
        ...
        DNX_FIELD_STAGE_CS_QUAL_ITERATOR(dnx_stage)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "BCM CS Qualifier:\"%s\" mapped to:\"%s\" dbal field, has no valid description",
                             dnx_field_bcm_qual_text(unit, i_bcm_cs_qual), dbal_field_to_string(unit, cs_qual_dbal_field));
            }
        }
    }
