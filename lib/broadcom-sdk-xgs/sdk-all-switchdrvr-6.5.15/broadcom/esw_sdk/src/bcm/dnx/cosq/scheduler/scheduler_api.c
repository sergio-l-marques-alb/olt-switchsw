...
/**
 * \brief -
 * verify bcm_dnx_cosq_bandwidth_fabric_adjust_set parameters
 */
int
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_connector_gport_detach(unit, sched_port, input_port, cosq));
    ...
    bcm_gport_t * gport)
{
    ...
    if (flags & BCM_COSQ_SCHEDULER_GPORT_FMQ_RESERVE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_fabric_multicast_scheduler_mode_get(unit, &fmq_mode));
        if (fmq_mode != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
