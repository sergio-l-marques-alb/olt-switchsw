 ...
 */
#define DNX_ALGO_GPM_DESTINATION_INVALID 0x1FFFFF
/**
    ...
    dnx_algo_gpm_gport_phy_info_t * gport_info)
{
    ...
    else if (BCM_GPORT_IS_MODPORT(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_stk_sys_modid_local_core_get(unit, modid, &core_id));
        if (core_id != -1)
        {
            dnx_algo_port_type_e port_type = DNX_ALGO_PORT_TYPE_INVALID;
        ...
        if (operations & DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT)
        {
            SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                         "Setting system port attributes using modport gport is not supported.\n The configuration should use system port explicitly.\n");
        }
    }
    ...
    else if (BCM_GPORT_IS_SYSTEM_PORT(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_stk_sys_sysport_modport_map_get
                        (unit, gport_info->sys_port, (uint32 *) &modid, (uint32 *) &tm_port));
    ...
    {   /* unicast queue */
         ...
         * System Port
         */
        SHR_IF_ERR_EXIT(dnx_ipq_queue_sysport_map_get(unit, queue_id, &(gport_info->sys_port)));
    ...
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "GPORT 0x%x is not physical, and thus cannot be input to dnx_algo_gpm_gport_phy_info_get.\r\n",
                     gport);
    }
    /*
    ...
    uint32 *local_in_lif)
{
    ...
    dbal_enum_value_result_type_ingress_gport_to_vlan_port_info_sw_e res_type;
    ...
    SHR_VAL_VERIFY(res_type, DBAL_RESULT_TYPE_INGRESS_GPORT_TO_VLAN_PORT_INFO_SW_VIRTUAL_NATIVE_AC, _SHR_E_PARAM,
                   "The supplied GPort is not an ingress virtual native AC handle");
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    dnx_algo_gpm_gport_hw_resources_t * gport_hw_resources)
{
    ...
    dnx_egr_pointed_t *egr_pointed_p = NULL;
    int *fec_id = NULL;
    ...
    dnx_egr_pointed_t egr_pointed_dummy = DNX_ALGO_EGR_POINTED_INVALID;
     ...
     */
    if ((global_out_lif) && (egr_pointed_p))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "EGR_POINTED and GLOBAL_LIF resources cannot be requested in the same time.\r\n"
    ...
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_global_resources
                    (unit, gport, &global_in_lif_dummy, &global_out_lif_dummy, &fec_dummy, &egr_pointed_dummy));
    SHR_IF_NOT_NULL_FILL(global_in_lif, global_in_lif_dummy);
    SHR_IF_NOT_NULL_FILL(global_out_lif, global_out_lif_dummy);
    SHR_IF_NOT_NULL_FILL(egr_pointed_p, egr_pointed_dummy);
    ...
    if (local_in_lif)
    {
        if (BCM_GPORT_IS_VLAN_PORT(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get(unit, *local_in_lif, _SHR_CORE_ALL,
    ...
    if (local_out_lif)
    {
         ...
         */
        if (egr_pointed_dummy != DNX_ALGO_EGR_POINTED_INVALID)
        {
            ...
            egr_pointed_flags = 0;
            /*
             * At this point, 'egr_pointed_p' is not NULL becasue dnx_algo_gpm_gport_to_global_resources()
             * loads 'egr_pointed_dummy' with a valid value only if 'gport' is encoded
             * as 'egr_pointed' under 'tunnel' and, in this case, 'egr_pointed_p' is loaded by
             * '&egr_pointed_id' (See code above).
             * However, we add the following 'if' to silence coverity.
             */
            if (egr_pointed_p != NULL)
            {
                *egr_pointed_p = egr_pointed_dummy;
            }
            SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_algo_l3_egr_pointed_to_local_lif_get
                                      (unit, egr_pointed_flags, egr_pointed_dummy, local_out_lif), _SHR_E_NOT_FOUND);
            if (*local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
            {
                SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_get(unit, *local_out_lif,
                                                                                         &gport_hw_resources->outlif_dbal_table_id,
                                                                                         &gport_hw_resources->outlif_dbal_result_type,
                                                                                         &gport_hw_resources->outlif_phase));
            }
        }
    ...
    dnx_algo_gpm_rif_hw_resources_t * rif_hw_resources)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_get(unit, intf,
    ...
    bcm_gport_t * gport)
{
    ...
    /*
     * Not required. Just added to silence false compiler complaint.
     */
    egr_pointed_id = 0;
    ...
    else if (shr_error == _SHR_E_NONE)
    {
        SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_get
                        (unit, local_lif, &dbal_table_id, &dbal_result_type, NULL));
    }
    ...
    uint32 *destination)
{
    ...
    if (BCM_GPORT_IS_TRUNK(port))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                        (unit, DBAL_FIELD_EGRESS_DESTINATION, DBAL_FIELD_DSP_AGG, (uint32 *) &egress_trunk_id,
                         destination));
    }
    else
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set
                        (unit, DBAL_FIELD_EGRESS_DESTINATION, DBAL_FIELD_DSP, &tm_port, destination));
    }
    ...
    uint8 *gport_is_valid_destination)
{
    ...
    *gport_is_valid_destination = TRUE;
              ...
              (BCM_GPORT_IS_MPLS_PORT(gport))) && ((BCM_GPORT_SUB_TYPE_IS_MULTICAST(gport))))
    {
         ...
         */
        uint32 mc_id = BCM_GPORT_SUB_TYPE_MULTICAST_VAL_GET(gport);
        SHR_IF_ERR_EXIT(dbal_fields_parent_field32_value_set(unit, DBAL_FIELD_DESTINATION, DBAL_FIELD_MC_ID, &mc_id,
                                                             destination));
...
{
    uint32 entry_handle_id;
    uint8 replace_destination = FALSE;
    ...
    /*
     * Verify:
     * If replacing an existing gport than only AC can have new info, and only destination field can be different
     */
    if (is_replace)
    {
        /*
        ...
        {
            /*
             * For VLAN PORT only replacing destination is allowed
             */
            if (BCM_GPORT_IS_VLAN_PORT(gport))
            {
                /*
                 * In this case need to delete existing entry, then write the new one
                 */
     ...
     */
    if ((!is_replace) || replace_destination)
    {
        /*
...
{
    algo_gpm_gport_verify_type_e gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_NOF;
    int gport_type_index = 0, is_supported = 0;
    ...
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_PORT;
    }
    else if (BCM_GPORT_IS_LOCAL(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_LOCAL_PORT;
    }
    else if (BCM_GPORT_IS_MODPORT(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_MODPORT;
    }
    else if (BCM_GPORT_IS_DEVPORT(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_DEVPORT;
    }
    else if (BCM_GPORT_IS_TRUNK(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_TRUNK;
    }
    else if (BCM_GPORT_IS_SYSTEM_PORT(gport))
    {
        uint32 system_port;
        ...
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_SYSTEM_PORT;
    }
    else if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_UCAST_QUEUE_GROUP;
    }
    else if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_MCAST_QUEUE_GROUP;
    }
    else if (BCM_GPORT_IS_LOCAL_CPU(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_LOCAL_CPU;
    }
    else if (BCM_PHY_GPORT_IS_PHYN(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_PHYN;
    }
    else if (BCM_PHY_GPORT_IS_PHYN_LANE(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_PHYN_LANE;
    }
    else if (BCM_PHY_GPORT_IS_PHYN_SYS_SIDE(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_PHYN_SYS_SIDE;
    }
    else if (BCM_PHY_GPORT_IS_PHYN_SYS_SIDE_LANE(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_PHYN_SYS_SIDE_LANE;
    }
    else if (BCM_PHY_GPORT_IS_LANE(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_LANE;
    }
    else if (BCM_GPORT_IS_LOCAL_FABRIC(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_LOCAL_FABRIC;
    }
    else if (BCM_GPORT_IS_BLACK_HOLE(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_BLACK_HOLE;
    }
    else if (BCM_GPORT_IS_MCAST(gport))
    {
        gport_type = ALGO_GPM_GPORT_VERIFY_TYPE_MCAST;
    }
    else
    {
    ...
    for (gport_type_index = 0; gport_type_index < nof_allowed_types; gport_type_index++)
    {
        if (gport_type == allowed_types[gport_type_index])
        {
            is_supported = 1;
            break;
        }
