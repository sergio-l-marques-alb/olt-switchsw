    ...
    int tm_domain)
{
    ...
    int entry = 0;
    bcm_trunk_t stk_tid = 0;
    bcm_trunk_t stk_trunk_array[dnx_data_stack.general.nof_tm_domains_max_get(unit)];
    ...
    sal_memset(stk_trunk_array, 0x0, sizeof(bcm_trunk_t) * dnx_data_stack.general.nof_tm_domains_max_get(unit));

    SHR_IF_ERR_EXIT(dnx_stk_trunk_domain_get
                    (unit, tm_domain, dnx_data_stack.general.nof_tm_domains_max_get(unit),
                     stk_trunk_array, &stk_trunk_count));
    ...
    int *member_count)
{
    ...
    for (index = 0; index < dnx_data_stack.general.lag_stack_trunk_resolve_entry_max_get(unit); index++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_ipq_queue_sysport_map_get(unit, dest_base_queue, &sys_port_id));
...
{
    int member_count = 0;
    bcm_trunk_member_t member_array[MAX_NOF_MEMBERS(unit)];

    SHR_FUNC_INIT_VARS(unit);

    SHR_INVOKE_VERIFY_DNX(dnx_stk_trunk_stacking_member_add_verify(unit, trunk_id, member));

    sal_memset(member_array, 0x0, MAX_NOF_MEMBERS(unit) * sizeof(bcm_trunk_member_t));

    SHR_IF_ERR_EXIT(dnx_stk_trunk_stacking_get(unit, trunk_id, MAX_NOF_MEMBERS(unit), member_array, &member_count));
    ...
    bcm_trunk_member_t * member)
{
    ...
    {
        sal_memmove(&member_array[member_del_idx], &member_array[member_count - 1], sizeof(bcm_trunk_member_t));
        SHR_IF_ERR_EXIT(dnx_stk_trunk_stacking_set(unit, trunk_id, member_count - 1, member_array));
    }
