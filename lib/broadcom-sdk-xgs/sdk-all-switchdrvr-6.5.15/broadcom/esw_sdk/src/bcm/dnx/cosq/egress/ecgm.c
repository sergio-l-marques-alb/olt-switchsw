    ...
    int unit)
{
    ...
    /*
     * init port for TM ports only (ports with base queue pair)
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_type_get(unit, port, &port_type));

    if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, port_type))
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_port_handle_init(unit, port, &port_profile_handle));
        SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_interface_handle_init(unit, port, &interface_profile_handle));
    }
...
dnx_ecgm_threshold_tag_get(
    ...
    dnx_ecgm_threshold_data_t * data)
{
    uint32 flags = data->threshold->flags;
    SHR_FUNC_INIT_VARS(unit);
