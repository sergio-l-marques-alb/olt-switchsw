    ...
    bcm_l3_egress_t * egr)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE))
    {
        ...
        /*
         * verify entry result type is not changed
         */
    ...
    uint32 ll_global_lif)
{
    ...
    {
        outlif_info.table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_ARP_NO_VSI;
    }
    if (DNX_QOS_MAP_PROFILE_GET(egr->qos_map_id) != 0)
    {
    ...
    {
        outlif_info.table_specific_flags |= DNX_ALGO_LIF_TABLE_SPECIFIC_ARP_NO_SOURCE_IDX;
    }
    ...
    {
        /** This is indication for lif table manager that statistics are required */
...
*/
static shr_error_e
dnx_l3_egress_src_mac_prefix_get(
    int unit,
    int local_out_lif,
    int *src_mac_prefix)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                    (unit, entry_handle_id, DBAL_FIELD_SOURCE_IDX, &is_field_valid));
    if (is_field_valid == TRUE)
    {

        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_SOURCE_IDX, INST_SINGLE,
                                                            (uint32 *) src_mac_prefix));
        /*
    ...
    bcm_l3_egress_t * egr)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_field_exist_on_dbal_handle
                    (unit, entry_handle_id, DBAL_FIELD_VSI, &is_field_valid));
    if (is_field_valid == TRUE)
    {
    ...
    if (is_field_valid == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.profile_data_get
                        (unit, _SHR_CORE_ALL, src_mac_addr_msb_idx, &ref_count, (void *) &source_address_entry));
        /*
         * assign src_mac_addr
         */
    ...
    bcm_if_t intf)
{
     ...
     * Get src mac prefix
     */
    SHR_IF_ERR_EXIT(dnx_l3_egress_src_mac_prefix_get(unit, local_out_lif, &src_mac_prefix));
    if (src_mac_prefix != -1)
    {
    ...
    int next_local_out_lif)
{
    ...
    while (is_last_tunnel == FALSE)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_NEXT_OUTLIF_POINTER, INST_SINGLE,
                                   &next_tunnel_lif);
    ...
    bcm_l3_egress_t * egr)
{
    ...
    if (!((flags & (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)) == (BCM_L3_KEEP_DSTMAC | BCM_L3_KEEP_VLAN)))
    {
        ...
        if (_SHR_IS_FLAG_SET(egr->flags2, BCM_L3_FLAGS2_SRC_MAC))
        {
            ...
            {
                /**Get src mac prefix */
                SHR_IF_ERR_EXIT(dnx_l3_egress_src_mac_prefix_get(unit, ll_local_lif, &old_src_mac_addr_prefix_profile));
