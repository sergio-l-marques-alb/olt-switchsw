    ...
    int unit)
{
    ...
    nof_channels_in_hbm = dnx_data_dram.hbm.nof_channels_get(unit);
...
{
    int nof_channels = dnx_data_dram.hbm.nof_channels_get(unit);
    SHR_FUNC_INIT_VARS(unit);
    ...
    char *name_str)
{
    ...
    {
        len = sal_snprintf(name_str, DNX_HBMC_STR_NAME_SIZE, "%s_%s_%d.%d", spn_HBM_TUNE, name, hbm_ndx, unit);
    }
    else
    {
        len = sal_snprintf(name_str, DNX_HBMC_STR_NAME_SIZE, "%s_%s_%d_%d.%d", spn_HBM_TUNE, name, hbm_ndx, channel_ndx,
                           unit);
    }
    if (len >= DNX_HBMC_STR_NAME_SIZE)
    {
...
dnx_hbmc_shmoo_hbm16_entry_set(
    ...
    uint32 value)
{
    char name_str[DNX_HBMC_STR_NAME_SIZE];
    ...
    uint32 *value)
{
    ...
    SHR_IF_ERR_EXIT(dnx_hbmc_tune_param_name_string_set(unit, hbm_ndx, channel_ndx, name, name_str));
    if ((s = soc_property_get_str(unit, name_str)) != NULL)
    {
        *valid = TRUE;
        *value = _shr_ctoi(s);
    }
    ...
    int action)
{
    ...
    sal_memset(&config_params, 0x0, sizeof(hbmc_shmoo_config_param_t));
    ...
    switch (action)
    {
        case DNX_HBMC_RESTORE_TUNE_PARAMETERS_FROM_SOC_PROPERTIES:
            shmoo_action = SHMOO_HBM16_ACTION_RESTORE;
            break;
        case DNX_HBMC_RESTORE_TUNE_PARAMETERS_FROM_OTP:
                        /** get properties from OTP into config_params */
                        
                        /** run shmoo on restore mode */
            shmoo_action = SHMOO_HBM16_ACTION_RESTORE;
            break;
        case DNX_HBMC_RUN_TUNE:
                        /** run shmoo on run mode */
            shmoo_action = SHMOO_HBM16_ACTION_RUN_AND_SAVE;
            break;
        case DNX_HBMC_SKIP_TUNE:
                        /** do nothing */
    ...
    SHR_BIT_ITER(dram_bitmap, nof_hbms, hbm_iter)
    {
        ...
        /** if restore mode - get the hmb tune information first (per hbm_index) */
        if (action == DNX_HBMC_RESTORE_TUNE_PARAMETERS_FROM_SOC_PROPERTIES)
        {
            SHR_IF_ERR_EXIT(dnx_hbmc_shmoo_hbm16_restore_from_file(unit, hbm_iter, &config_params));
        }
        /** if restore mode - get the hmb tune information first (per hbm_index) */
        if (action == DNX_HBMC_RESTORE_TUNE_PARAMETERS_FROM_OTP)
        {
            SHR_IF_ERR_EXIT(dnx_hbmc_otp_shmoo_hbm16_restore_from_otp(unit, hbm_iter, &config_params));
        }
        ...
        for (; channel < nof_channels_in_hbm; ++channel)
        {
            ...
            if (action == DNX_HBMC_RESTORE_TUNE_PARAMETERS_FROM_SOC_PROPERTIES)
            {
                ...
                SHR_IF_ERR_EXIT(_hbm16_phy_channel_reset(unit, hbm_iter, channel));
            }
...
dnx_hbmc_tdu_atm_get(
    ...
    const uint32 **atm)
{
    uint32 max_nof_hbms = dnx_data_dram.hbm.nof_hbms_get(unit);
    ...
    int unit)
{
    ...
    dnx_hbmc_phy_tune_action_t tune_action = dnx_data_dram.general_info.tune_mode_on_init_get(unit);
    ...
    {
        for (hbmc_index = 0; hbmc_index < dnx_data_dram.hbm.nof_hbms_get(unit); hbmc_index++)
        {
            ...
            if (SHR_BITGET(dram_bitmap, hbmc_index))
            {
                ...
                for (hbmc_channel = 0; hbmc_channel < dnx_data_dram.hbm.nof_channels_get(unit); hbmc_channel++)
                {
                    if (!SAL_BOOT_PLISIM)
                    {
    ...
    int unit)
{
    ...
    for (int hbm_index = 0; hbm_index < nof_hbms; ++hbm_index)
    {
        ...
        {
            int block;
            block = si->hbc_block[dnx_hbmc_get_sequential_channel(unit, hbm_index, channel)];
            si->block_valid[block] = is_enabled;
        }
    }
    ...
    int unit)
{
    ...
    {
        LOG_CLI_EX("Skip HBMC for emulation!! %s%s%s%s\r\n", EMPTY, EMPTY, EMPTY, EMPTY);
        SHR_EXIT();
    }
#endif

    /** Update SW-State with vendor info */
    ...
    int unit)
{
    ...
    if (TRUE == dnx_data_dram.hbm.dram_temp_monitor_enable_get(unit))
    {
        ...
        /** Stop thread */
        SHR_IF_ERR_EXIT(periodic_event_stop(temp_monitor_handler, 1, 0));
    }
    ...
    int unit)
{
    ...
    if (temp_monitor_handler != NULL)
    {
        SHR_IF_ERR_EXIT(periodic_event_destroy(&temp_monitor_handler, 1));
    }
    ...
    int unit)
{
    ...
    /** Stop and destroy temperature monitoring */
    if (TRUE == dnx_data_dram.hbm.dram_temp_monitor_enable_get(unit))
    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_CONT(dnx_dram_db.is_init(unit, &is_init));
    if (is_init == TRUE)
    {
        /** unregister DRAM power down callback */
        SHR_IF_ERR_EXIT(dnx_dram_db.power_down_callback.callback.get(unit, &callback));
        /** Allow changing SW state during deinit time */
        if (callback != NULL)
        {
            SHR_IF_ERR_EXIT(dnxc_ha_tmp_allow_access_enable(unit, DNXC_HA_ALLOW_SW_STATE));
            SHR_IF_ERR_EXIT(dnx_hbmc_power_down_cb_unregister(unit, callback, NULL));
    ...
    const hbmc_shmoo_bist_info_t * info)
{
     ...
     */
    nof_bist_actions_for_arethmetics = COMPILER_64_INIT(info->bist_num_actions[1], info->bist_num_actions[0]);
...
dnx_hbmc_channel_soft_init(
    ...
    int channel)
{
    uint32 nof_mrs = dnx_data_dram.hbm.nof_mrs_get(unit);
    uint32 mr_vals[nof_mrs];
    SHR_FUNC_INIT_VARS(unit);
    ...
    /** read MRs */
    sal_memset(mr_vals, 0, sizeof(mr_vals));
    for (int mr_index = 0; mr_index < nof_mrs; ++mr_index)
    {
        SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_mr_get(unit, hbm_ndx, channel, mr_index, &mr_vals[mr_index]));
    }
    ...
    for (int mr_index = 0; mr_index < nof_mrs; ++mr_index)
    {
        SHR_IF_ERR_EXIT(dnx_hbmc_dbal_access_mr_set(unit, hbm_ndx, channel, mr_index, mr_vals[mr_index]));
    }
    ...
    uint32 *data)
{
    ...
    for (int current_index = start_index; current_index <= stop_index; ++current_index)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_hbmc_ind_access_physical_address_read(unit, PHY_ADDRESS_GET_MODULE(address),
    ...
    uint32 *pattern)
{
    ...
    for (int current_index = start_index; current_index <= stop_index; ++current_index)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_hbmc_ind_access_physical_address_write(unit, PHY_ADDRESS_GET_MODULE(address),
                                                                   PHY_ADDRESS_GET_CHANNEL(address),
                                                                   PHY_ADDRESS_GET_BANK(address),
                                                                   PHY_ADDRESS_GET_ROW(address),
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnx_hard_reset(unit));
