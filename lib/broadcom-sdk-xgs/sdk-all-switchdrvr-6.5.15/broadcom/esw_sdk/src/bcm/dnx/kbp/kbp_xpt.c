    ...
    int unit)
{
    ...
    if ((!soc_sand_is_emulation_system(unit)) && (!SAL_BOOT_PLISIM) )
    {
        ...
        DNX_KBP_TRY(kbp_pcie_init
                    (Kbp_app_data[unit]->device_type, flags, unit, Kbp_app_data[unit]->dalloc_p,
                     &Kbp_app_data[unit]->device_config, NULL, &Kbp_app_data[unit]->xpt_p));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    if ((Kbp_app_data[unit] == NULL) || (Kbp_app_data[unit]->dalloc_p == NULL) || (Kbp_app_data[unit]->xpt_p == NULL))
    {
        /*
    ...
    {
        DNX_KBP_TRY(kbp_pcie_destroy(Kbp_app_data[unit]->dalloc_p, Kbp_app_data[unit]->xpt_p));
    }
