    ...
    bcm_port_t * rcy_mirror_port)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_interface_and_channel_to_logical_get
                        (unit, core_id, DNX_ALGO_PORT_TYPE_RCY_MIRROR, rcy_channel_id, rcy_mirror_port));
    }
...
{
    dnx_algo_port_type_e forward_port_type;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_type_get(unit, forward_port, &forward_port_type));
    if (!DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, forward_port_type))
    {
...
dnx_mirror_port_to_rcy_port_map_set_verify(
    ...
    bcm_mirror_port_to_rcy_map_info_t * rcy_map_info)
{
    dnx_algo_port_type_e mirror_port_type;
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_type_get(unit, rcy_mirror_port, &mirror_port_type));
    if (!DNX_ALGO_PORT_TYPE_IS_RCY_MIRROR(unit, mirror_port_type))
    {
    ...
    bcm_mirror_port_to_rcy_map_info_t * rcy_map_info)
{
     ...
     */
    memset(per_priority_exist, 0, sizeof(per_priority_exist));
    /** convert gport to local port */
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(unit, forward_port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY,
                                                    &gport_info));
    forward_port = gport_info.local_port;
