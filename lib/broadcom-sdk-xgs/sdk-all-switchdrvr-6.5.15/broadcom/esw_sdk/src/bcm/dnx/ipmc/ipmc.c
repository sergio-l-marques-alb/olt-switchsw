 ...
 */
#define IPMC_MC_ADDRESS_PREFIX 0xe
/*
 * Maximum number of IPMC tables to be traversed
 */
#define MAX_NOF_IPMC_TABLES  14
/*
...
#define IPMC_IS_IPV4_MC_GROUP_VALID(addr, mask) (((addr & mask) >> IPMC_IPV4_GROUP_MASK_SIZE) == \
                                                    ((mask >> IPMC_IPV4_GROUP_MASK_SIZE) & IPMC_MC_ADDRESS_PREFIX))
/*
 * Indicates whether the input address (addr) is an IPV4 multicast address (has a 4 bit prefix of 0xe)
 */
#define IPMC_IS_IPV4_ADDRESS_MULTICAST(addr) (((addr) >> IPMC_IPV4_GROUP_MASK_SIZE) == IPMC_MC_ADDRESS_PREFIX)
/*
 * Indicate whether the IPv6 address is multicast - If its first byte is 0xFF
 */
#define IPMC_IS_IPV6_ADDRESS_MULTICAST(addr, mask) ((addr[0] & mask[0]) == 0xFF)
/*
 ...
 */
#define IPMC_IPV4_SET_GROUP(addr) ((IPMC_MC_ADDRESS_PREFIX << IPMC_IPV4_GROUP_MASK_SIZE) | addr)
/*
 ...
 */
#define IPMC_IS_IPV4_HOST_ENTRY(data) ((data->vid > 0 || data->ing_intf > 0) && IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask) \
 ...
 */
#define IPMC_IS_IPV6_HOST_ENTRY(data) (IPMC_IS_IPV6_NO_MASKING(data->mc_ip6_mask) && (data->vid > 0 || data->ing_intf > 0)\
...
/*
 * Determine whether the bcm_ipmc_addr_t flags mean an IPv4 entry - either route or host.
 * The flags describe an IPv4 entry if neither BCM_IPMC_IP6 not BCM_IPMC_L2 flag is set.
 */
#define IPMC_IS_IPV4_ROUTE_HOST(flags) (!_SHR_IS_FLAG_SET(flags, BCM_IPMC_IP6) \
                                        && !_SHR_IS_FLAG_SET(flags, BCM_IPMC_L2))
...
#define IPMC_KBP_TABLE_TO_RESULT_TYPE_FWD_DEST(table) ( \
        (table == DBAL_TABLE_KBP_IPV4_MULTICAST_TCAM_FORWARD) ? DBAL_RESULT_TYPE_KBP_IPV4_MULTICAST_TCAM_FORWARD_FWD_DEST_W_DEFAULT : \
        (table == DBAL_TABLE_KBP_IPV6_MULTICAST_TCAM_FORWARD) ? DBAL_RESULT_TYPE_KBP_IPV6_MULTICAST_TCAM_FORWARD_FWD_DEST_W_DEFAULT : -1)
/*
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        /** Verify that the flag BCM_IPMC_L2 is not used with the BCM_IPMC_IP6 flag */
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Combination of flags BCM_IPMC_IP6 and BCM_IPMC_L2 is not supported - %u.",
                         data->flags);
        }
    }
    else
    {
        ...
        else
        {
            ...
            if (IPMC_IS_IPV4_HOST_ENTRY(data) && lem_capacity > 0 && data->vrf == 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             "Illegal VRF value 0 for MC host entry when property dnx_data_mdb_lpm_nof_lpm_entries_estimate is 0.");
            }
            /** Verify that the mask of the LPM IPMC entry is consistent */
            if (!IPMC_IS_IPV4_HOST_ENTRY(data) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_TCAM))
            {
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (data->group != 0)
    {
         ...
         */
        if ((IPMC_IS_IPV4_HOST_ENTRY(data) && !_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
            || (IPMC_IS_IPV6_HOST_ENTRY(data) && _SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    ...
    else if (data->l3a_intf != 0)
    {
        ...
        else
        {
             ...
             */
            /*
             * Verify that the FEC value is in range.
             */
            ...
            if (fec_index > dnx_data_l3.fec.max_default_fec_get(unit)
                &&
                ((!_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6) && !IPMC_IS_IPV4_HOST_ENTRY(data)
                  && IPMC_IS_IPV4_FULL_MASKING(data->mc_ip_mask) && IPMC_IS_IPV4_FULL_MASKING(data->s_ip_mask))
                 || (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6) && !IPMC_IS_IPV6_HOST_ENTRY(data)
                     && IPMC_IS_IPV6_FULL_MASKING(data->mc_ip6_mask))))
            {
    ...
    uint32 *entry_handle_id)
{
    ...
    if (data->ing_intf != BCM_IF_INVALID)
    {
        dbal_entry_key_field32_set(unit, *entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, BCM_L3_ITF_VAL_GET(data->ing_intf));
    }
    else
    {
...
dnx_ipmc_entry_tcam_key_set(
    ...
    uint32 *entry_handle_id)
{
    uint32 ipv4_group = IPMC_TABLE_IS_KBP(dbal_table) ? DBAL_FIELD_KBP_IPV4_GROUP : DBAL_FIELD_IPV4_GROUP;
    ...
    uint32 *dbal_table)
{
     ...
     * If the BCM_IPMC_L2 flag is added, then a bridge entry will be created.
    ...
    if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_IP6))
    {
        ...
        else if (IPMC_IS_IPV6_HOST_ENTRY(data))
        {
            *dbal_table = DBAL_TABLE_IPV6_MULTICAST_EM_FORWARD;
        }
        else if (data->vrf > 0 || kaps_public_capacity == 0)
        {
        ...
        {
            *dbal_table = DBAL_TABLE_IPV6_MULTICAST_PUBLIC_LPM_FORWARD;
            allow_raw = 1;
        }
    }
    ...
    else
    {
        ...
        else if (IPMC_IS_IPV4_HOST_ENTRY(data))
        {
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    switch (dbal_sub_field)
    {
        ...
        {
            _BCM_MULTICAST_GROUP_SET(data->group, _BCM_MULTICAST_TYPE_L3, result_value);
            break;
        }
        case DBAL_FIELD_FEC:
        {
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
        ...
        || dbal_table == DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL)
    {
        ...
        dbal_entry->mod_id = field_value;
    }

    SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_IPV4_GROUP, &field_value));
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_KBP_GLOB_IN_LIF, &field_value));
        field_value = IPMC_KBP_IN_LIF_SET(field_value);
    }
    else
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, &field_value));
    }
     ...
     */
    if (dbal_table != DBAL_TABLE_IPV6_MULTICAST_PUBLIC_LPM_FORWARD)
    {
    ...
    bcm_ipmc_addr_t * dbal_entry)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_KBP_IPV4_GROUP, &field_value));
        dbal_entry->mc_ip_addr = IPMC_KBP_IPV4_GROUP_SET(IPMC_IPV4_SET_GROUP(field_value));
    }
    else
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_IPV4_GROUP, &field_value));
        dbal_entry->mc_ip_addr = IPMC_IPV4_SET_GROUP(field_value);
    }
    ...
    if (dbal_table == DBAL_TABLE_IPV4_MULTICAST_PRIVATE_LPM_FORWARD
        ...
        || dbal_table == DBAL_TABLE_KBP_IPV4_MULTICAST_TCAM_FORWARD)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_IPV4_SIP,
                                                              &field_value));
        dbal_entry->s_ip_addr = field_value;
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    if (IPMC_TABLE_IS_KBP(dbal_table))
    {
        ...
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_DESTINATION, INST_SINGLE,
                                            &destination_val);
        SHR_IF_ERR_EXIT(dnx_ipmc_resolve_entry_destination(unit, destination_val, dbal_table, data));
        SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_PRIORITY, (uint32 *) &data->priority));
    }
    else if (dbal_table_type == DBAL_TABLE_TYPE_TCAM)
    {
        SHR_IF_ERR_EXIT(dnx_ipmc_entry_tcam_key_set(unit, dbal_table, data, &entry_handle_id));
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    rv = bcm_dnx_ipmc_find(unit, &data_find);

    if (rv == _SHR_E_NONE && (data->flags & BCM_IPMC_REPLACE))
    {
        is_ipmc_replace = TRUE;
    }
    if (rv == _SHR_E_NONE && (!(data->flags & BCM_IPMC_REPLACE)))
    {
        SHR_ERR_EXIT(_SHR_E_EXISTS, "entry key given already exists and REPLACE flag is disabled\n");
    }
    if (rv == _SHR_E_NOT_FOUND && ((data->flags & BCM_IPMC_REPLACE)))
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "entry key given does not exist and REPLACE flag is enabled\n");
    }
    if (rv != _SHR_E_NONE && rv != _SHR_E_NOT_FOUND)
    {
        SHR_ERR_EXIT(rv, "find ipmc entry fail\n");
    }
    ...
    bcm_ipmc_addr_t * data)
{
    ...
    else if (dbal_table_type == DBAL_TABLE_TYPE_TCAM)
    {
        ...
        /** Clear the entry ID */
        SHR_IF_ERR_EXIT(dbal_entry_clear(unit, entry_handle_id, DBAL_COMMIT));
        dnx_field_entry_access_id_destroy(unit, dbal_table, entry_access_id);
    }
    else
    {
        if (_SHR_IS_FLAG_SET(data->flags, BCM_IPMC_L2))
    ...
    int unit)
{
    ...
    uint32 dbal_mc_tables[MAX_NOF_IPMC_TABLES] = {
    ...
    void *user_data)
{
    uint32 entry_handle_id;
    uint32 table;
    uint32 dbal_tables[MAX_NOF_IPMC_TABLES] = {
        DBAL_TABLE_IPV4_MULTICAST_EM_FORWARD,
        ...
        DBAL_TABLE_IPV6_MULTICAST_PRIVATE_LPM_FORWARD, DBAL_TABLE_IPV6_MULTICAST_PUBLIC_LPM_FORWARD,
        DBAL_TABLE_IPV6_MULTICAST_TCAM_FORWARD,
        DBAL_TABLE_KBP_IPV6_MULTICAST_TCAM_FORWARD
    };
    uint32 nof_tables;
