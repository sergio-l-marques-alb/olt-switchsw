    ...
    uint32 flags)
{
    ...
    /*
     * Operations required when loading port in single stage or Stage1.
     */
    if (!IMB_PORT_ADD_F_INIT_PASS2_GET(flags))
    {
        /** Need to power up FSRD when activating the first link in the FSRD */
        SHR_IF_ERR_EXIT(dnx_fabric_if_port_first_link_in_fsrd_get(unit, port, &is_first_link));

        if (is_first_link == TRUE)
        {
            /** power up FSRD */
            SHR_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_port_fsrd_block_enable_set, (unit, port, 1)));
        }
    }
    ...
    fw_verify_fabric = dnx_data_port.static_add.fabric_fw_load_verify_get(unit);
    SHR_IF_ERR_EXIT(soc_dnxc_fabric_port_probe(unit, port, stage, fw_verify_fabric, &port_config));
    ...
    bcm_port_t port)
{
    ...
    /** Disable port */
    SHR_IF_ERR_EXIT(imb_fabric_port_enable_set(unit, port, 0));

    /** Remove from portmod */
    SHR_IF_ERR_EXIT(portmod_port_remove(unit, port));

    SHR_IF_ERR_EXIT(dnx_fabric_if_port_first_link_in_fsrd_get(unit, port, &is_first_link));
    /** last active link in the fsrd block */
    if (is_first_link == 1)
    {
        /** power down FSRD */
        SHR_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_port_fsrd_block_enable_set, (unit, port, 0)));
    }
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_port_enable_set, (unit, port, enable ? 1 : 0)));
