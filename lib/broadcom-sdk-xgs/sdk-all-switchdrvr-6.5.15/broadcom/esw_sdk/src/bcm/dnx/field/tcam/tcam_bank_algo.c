    ...
    uint8 *collide)
{
    ...
    *collide = FALSE;
    ...
    {
        /*
         * This function has no support for non-PMF stages 
         */
        SHR_EXIT();
    }

    /** We get all handlers that share context with the given handler, and check if one of them is in the bank */
    ...
    uint8 *can_be_used)
{
    ...
    /** DT handlers can reside on OPEN/DT bank modes only */
    if (bank_mode != DNX_FIELD_TCAM_BANK_MODE_OPEN && bank_mode != DNX_FIELD_TCAM_BANK_MODE_RESTRICTED_DT)
    {
        *can_be_used = FALSE;
    }
    else
    ...
    uint8 *can_be_used)
{
    ...
    *can_be_used = TRUE;
     ...
     */
    for (core = 0; core < dnx_data_device.general.max_nof_cores_get(unit); core++)
    {
    ...
    int *bank_id)
{
    ...
    {
        /** Finding a bank to allocate is disabled for 'SELECT' bank allocation mode */
