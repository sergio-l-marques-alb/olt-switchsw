    ...
    dnx_vlan_range_info_t * vlan_range)
{
    ...
    if (is_outer)
    {
        SHR_IF_ERR_EXIT(vlan_db.vlan_range_outer_profile.profile_data_get(unit, _SHR_CORE_ALL, old_vlan_range_profile,
                                                                          &ref_count, &template));
        if (is_add)
        {
        ...
        SHR_IF_ERR_EXIT(vlan_db.vlan_range_outer_profile.exchange
                        (unit, _SHR_CORE_ALL, 0, &template,
                         old_vlan_range_profile, NULL, &new_vlan_range_profile, &is_first, &is_last));
    }
    else
    {
        SHR_IF_ERR_EXIT(vlan_db.vlan_range_inner_profile.profile_data_get(unit, _SHR_CORE_ALL, old_vlan_range_profile,
                                                                          &ref_count, &template));
        if (is_add)
        {
            SHR_IF_ERR_EXIT(dnx_vlan_range_overlap_verify(unit, vlan_range, &template));
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_range_template_update(unit, vlan_range, &template));
        SHR_IF_ERR_EXIT(vlan_db.vlan_range_inner_profile.exchange
                        (unit, _SHR_CORE_ALL, 0, &template,
                         old_vlan_range_profile, NULL, &new_vlan_range_profile, &is_first, &is_last));
    }
    /*
    ...
    bcm_vlan_t inner_vlan_high)
{
    ...
    if (BCM_GPORT_IS_TRUNK(port))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Trunk group needs to be deleted via iterating port members of trunk.");
    }
    if ((outer_vlan_low != BCM_VLAN_INVALID) && (inner_vlan_low != BCM_VLAN_INVALID))
    {
        BCM_DNX_VLAN_CHK_ID(unit, inner_vlan_low);
    ...
    int unit)
{
     ...
     *Clear template
     */
    SHR_IF_ERR_EXIT(vlan_db.vlan_range_outer_profile.clear(unit, _SHR_CORE_ALL));
    SHR_IF_ERR_EXIT(vlan_db.vlan_range_inner_profile.clear(unit, _SHR_CORE_ALL));
    /*
    ...
    bcm_vlan_action_set_t * action)
{
    ...
    SHR_IF_ERR_EXIT(vlan_db.vlan_range_outer_profile.profile_data_get(unit, _SHR_CORE_ALL,
                                                                      vlan_domain_attr.vlan_range_outer_index,
                                                                      &ref_count, &outer_template));
    SHR_IF_ERR_EXIT(vlan_db.vlan_range_inner_profile.profile_data_get(unit, _SHR_CORE_ALL,
