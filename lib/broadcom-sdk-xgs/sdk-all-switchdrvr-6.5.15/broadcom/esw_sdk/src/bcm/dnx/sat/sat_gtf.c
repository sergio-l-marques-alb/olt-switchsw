    ...
    bcm_sat_gtf_t gtf_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sat_nof_free_gtf(unit, &nof_free_gtf));
    ...
    uint32 *rate)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_sand_compute_complex_to_mnt_exp(rate_mnt_val, rate_exp_val, 1, 0, &bytes_per_cycle));
    ...
    else
    {
        ...
        COMPILER_64_UDIV_64(rate64bits, rate_div);
    ...
    uint32 *rate_exp_val)
{
    ...
    else
    {
        if (clocks_per_cycle != 0)
        {
            ...
            clocks_per_cycle = (freq_hz / (DNX_DATA_MAX_SAT_GENERAL_GRANULARITY / DNX_SAT_BITS_PER_BYTE));      /* 1kbits 
                                                                                                                 * per
                                                                                                                 * sec(125 
                                                                                                                 * cycles 
                                                                                                                 * per
        ...
        if (cycles_per_sec != 0)
        {
             ...
             * this calculation using 64bit macros as the multiplication exceeds 32bits
             */
            COMPILER_64_SET(rate64bits, 0, rate);       /* rate is 32bit, copy it to a 64bits variable to be able to */
            COMPILER_64_UMUL_32(rate64bits, 125);
            COMPILER_64_UDIV_64(rate64bits, cycles_per_sec);
     ...
     */
    SHR_IF_ERR_EXIT(dnx_sand_break_to_mnt_exp_round_up
    ...
    bcm_sat_gtf_bandwidth_t * bw)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sand_compute_complex_to_mnt_exp(burst_mnt_val, burst_exp_val, 1, 0, &complex));
    ...
    dnx_sat_gtf_stamp_field_tbl_info_t * stamp_field_info)
{
    ...
    for (i = 0; i < pkt_edit->number_of_stamps; i++)
    {
        ...
        else if (tmp_stamp->stamp_type == bcmSatStampConstant16Bit)
        {
            ...
            {
                stamp_field_info->stamp_16_offset = tmp_stamp_offset_bits / DNX_SAT_BITS_PER_BYTE + 2;
                stamp_field_info->stamp_16_shift =
...
{
    uint32 pkt_data = 0;
    uint32 pkt_hdr_bits = 1024, pkt_bits_remainder = 0;
    ...
    bcm_sat_gtf_packet_config_t * config)
{
    ...
    uint32 header_add[32] = {
        0, 32, 42, 96, 128, 160, 192, 224,
        256, 288, 320, 352, 384, 416, 448, 480,
        521, 544, 576, 608, 640, 672, 704, 736,
    ...
    bcm_sat_gtf_packet_config_t * config)
{
    ...
    SHR_RANGE_VERIFY(gtf_id, DNX_SAT_GTF_ID_MIN, DNX_SAT_GTF_ID_MAX, _SHR_E_PARAM, "gtf_id");
    SHR_RANGE_VERIFY(config->sat_header_type, bcmSatHeaderUserDefined, (bcmSatHeadersCount - 1), _SHR_E_PARAM,
                     "sat_header_type");
    ...
    SHR_RANGE_VERIFY(config->payload.payload_type, bcmSatPayloadConstant8Bytes, (bcmSatPayloadsCount - 1), _SHR_E_PARAM,
                     "payload_type");
    SHR_RANGE_VERIFY(config->packet_context_id, 0, (MAX_NUM_OF_CORES - 1), _SHR_E_PARAM, "packet_context_id");
    ...
    if (!is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: gtf_id  %d does not exist\n", gtf_id);
    }
    ...
    bcm_sat_gtf_rate_pattern_t * config)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_sand_compute_complex_to_mnt_exp(high_mnt_val, high_exp_val, 1, 0, &complex));
    config->high_threshold = complex / 125;

    SHR_IF_ERR_EXIT(dnx_sand_compute_complex_to_mnt_exp(low_mnt_val, low_exp_val, 1, 0, &complex));
    ...
    bcm_sat_gtf_rate_pattern_t * config)
{
    ...
    else if (config->rate_pattern_mode == bcmSatGtfRatePatternSimpleBurst)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Fail in simplebusrt mode, high_threshold(%d) should equals to low_threshold(%d)\n",
                         config->high_threshold, config->low_threshold);
        }
    }
    else if (config->rate_pattern_mode == bcmSatGtfRatePatternCombined)
    {
        if (config->high_threshold < config->low_threshold)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Fail in combined mode, high_threshold(%d) should equals to low_threshold(%d)\n",
                         config->high_threshold, config->low_threshold);
        }
    }
    else if (config->rate_pattern_mode == bcmSatGtfRatePatternInterval)
    {
        if (config->high_threshold < config->low_threshold)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Fail in interval mode, high_threshold(%d) should equals to low_threshold(%d)\n",
                         config->high_threshold, config->low_threshold);
        }
    }
    else
    ...
    uint64 *byte_cnt)
{
    ...
    uint64 packet_cnt_hw_64 = 0;
    uint64 byte_cnt_hw_64 = 0;
    uint32 packet_cnt_sw[2] = { 0 };    /* packet Counter in SW */
    uint32 byte_cnt_sw[2] = { 0 };      /* byte packet Counter in SW */
    uint64 packet_cnt_sw_64 = 0;        /* packet Counter in SW */
    uint64 byte_cnt_sw_64 = 0;  /* byte packet Counter in SW */
    ...
    packet_cnt_hw_64 = (uint64) packet_cnt_hw;
...
bcm_dnx_sat_gtf_stat_multi_get(
    ...
    bcm_sat_gtf_stat_multi_t * stat)
{
    uint64 packet_cnt = 0;      /* Counter transmitted packet */
    uint64 byte_cnt = 0;        /* Counter transmitted byte */
    ...
    stat->transmitted_packet_cnt = packet_cnt;
    stat->transmitted_byte_cnt = byte_cnt;
