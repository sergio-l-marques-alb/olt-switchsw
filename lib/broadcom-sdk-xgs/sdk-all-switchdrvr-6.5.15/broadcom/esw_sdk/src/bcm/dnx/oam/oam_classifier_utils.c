...
/**
 * \brief - Updates OAM LIF DB entry. mp_profile field only. Having
 *   different prefixes (HLM by MDL), the specific entry to
 *   update (key) is a combination of lif + prefix.
 *
 * \param [in] unit - Number of hardware unit used.
 * \param [in] oam_lif_db_key - The lif to update the profile
 *        for, the specific prefix entry to update
 *        the profile for and the direction (ing / eg).
 *        It is according to 1 prefix.
