    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if ((_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_OUTER_EXP) ||
         _SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_OUTER_TTL)) && (info->action != BCM_MPLS_SWITCH_ACTION_PHP)
        && (info->action != BCM_MPLS_SWITCH_ACTION_POP_DIRECT))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "flags BCM_MPLS_SWITCH_OUTER_EXP and BCM_MPLS_SWITCH_OUTER_TTL are supported for PHP actions only");
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
             ...
             || (result_type == DBAL_RESULT_TYPE_MPLS_FWD_FWD_DEST_EEI_STAT))
    {
        if ((info->action == BCM_MPLS_SWITCH_ACTION_PHP) || ((info->action == BCM_MPLS_SWITCH_ACTION_POP_DIRECT)))
        {
             ...
             */
            qos_model =
                (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_OUTER_EXP)) ? DBAL_ENUM_FVAL_MODEL_TYPE_UNIFORM :
                DBAL_ENUM_FVAL_MODEL_TYPE_PIPE;
            ttl_model =
                (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_SWITCH_OUTER_TTL)) ? DBAL_ENUM_FVAL_MODEL_TYPE_UNIFORM :
                DBAL_ENUM_FVAL_MODEL_TYPE_PIPE;
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (BCM_GPORT_IS_MCAST(gport))
    {
        ...
        BCM_MULTICAST_EGRESS_OBJECT_SET(info->mc_group, BCM_GPORT_MCAST_GET(gport));
    }
