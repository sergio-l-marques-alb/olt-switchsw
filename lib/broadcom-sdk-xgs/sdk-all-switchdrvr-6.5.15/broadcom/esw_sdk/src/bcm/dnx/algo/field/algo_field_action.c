...
shr_error_e
dnx_algo_field_action_fem_or_fes(
    int unit,
    int *is_fes_p)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_NULL_CHECK(is_fes_p, _SHR_E_PARAM, "is_fes_p");
...
dnx_field_fem_copy_replace_map_indices(
    int unit,
    dnx_field_fem_condition_entry_t fem_condition_entry_source[dnx_data_field.stage.stage_info_get(unit,
 ...
 *    DNX_FIELD_FEM_BIT_FORMAT_INVALID for bits 4 to 23. Also, 'fem_adder' should be
 *    DNX_FIELD_FEM_ADDER_INVALID. Otherwise, *fem_map_index_p is loaded by
    ...
    unsigned int *already_in_hw_p)
{
    ...
    *already_in_hw_p = 0;
    ...
    shr_error = algo_field_info_sw.action_template_per_fem.allocate_single
        (unit, fem_id, _SHR_CORE_ALL, template_flags, (void *) fem_action_entry_p, NULL, &profile, &first_reference);
    LOG_DEBUG_EX(BSL_LOG_MODULE,
                 "\r\n"
                 "Attempted to allocate template using sw_state_algo_template_allocate(). fem_id %d. Reported error: %s\r\n"
    ...
    dnx_field_action_priority_t priority)
{
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE, "In stage \"%s\" FES ID %d context ID %d priority was set to 0x%08X.\r\n.",
                 dnx_field_stage_text(unit, field_stage), fes_id, context_id, priority);
exit:
    SHR_FUNC_EXIT;
}
    ...
    unsigned int action_number)
{
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE, "In stage \"%s\" FES ID %d FES program ID %d was set to:%s\r\n.",
                 dnx_field_stage_text(unit, field_stage), fes_id, fes_pgm_id, EMPTY);
    ...
    int set)
{
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE, "In stage \"%s\" FES ID %d mask %d was set to \"%s\".\r\n.",
                 dnx_field_stage_text(unit, field_stage), fes_id, mask_id, (set ? "allocated" : "free"));
exit:
    SHR_FUNC_EXIT;
}
 ...
 * \param [out] nof_existing_actions_in_fg_p -
 *   To be loaded with the FES quartets belonging to the field group we allocate for.
 *   This information is used for finding the ordinal place of the new actions in the field group.
 * \return
 *   shr_error_e - Error Type
 * \remark
 *   * None
...
dnx_algo_field_action_fes_allocate_collect_current_state_info(
    ...
    dnx_algo_field_action_fes_alloc_current_state_inclusive_t * current_state_changing_p,
    uint8 after_invalidate_next[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_INSTRUCTION_PER_CONTEXT],
    unsigned int *nof_existing_actions_in_fg_p)
{
    ...
    SHR_NULL_CHECK(nof_existing_actions_in_fg_p, _SHR_E_PARAM, "nof_existing_actions_in_fg_p");

    SHR_IF_ERR_EXIT(dnx_field_map_field_id_dnx_action
                    (unit, field_stage, DBAL_FIELD_INVALIDATE_NEXT, &dnx_action_invalidate_next));
    SHR_IF_ERR_EXIT(dnx_field_map_dnx_to_action_type
                    (unit, field_stage, dnx_action_invalidate_next, &invalidate_next_action_type));
    ...
    (*nof_existing_actions_in_fg_p) = 0;
     ...
     */
    if ((*nof_existing_actions_in_fg_p) > 0 && (*nof_existing_actions_in_fg_p) - 1 != previous_last_ordinal_place)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Number of existing actions in field group %d context %d is %d, does not match "
                     ...
                     (*nof_existing_actions_in_fg_p), fg_id, context_id, previous_last_ordinal_place);
    }
    ...
    dnx_algo_field_action_fes_alloc_fes_quartet_t fes_quartet[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    for (fes_quartet_ndx = 0; fes_quartet_ndx < fes_fg_in_p->nof_fes_quartets; fes_quartet_ndx++)
    {
        ...
        fes_quartet[fes_quartet_ndx].place_in_fg = fes_quartet_ndx + nof_existing_actions_in_fg;
    ...
    int *found_allocation_p)
{
    ...
    while (TRUE)
    {
        ...
        if (is_direction_up)
        {
            if (!(((signed int) fes_id_ndx) <= max_in_array && (*found_allocation_p) == FALSE))
            {
                break;
            }
        }
        else
        {
            ...
            if (!(((signed int) fes_id_ndx) >= min_in_array && (*found_allocation_p) == FALSE &&
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_field_action_fes_allocate_non_mandatory_position
                                    (unit, field_stage, context_id,
                                     ...
                                     dnx_data_field.stage.stage_info_get(unit,
                                                                         field_stage)->nof_fes_instruction_per_context,
                                     FALSE, allow_non_contention_moving, allow_fes_pgm_sharing,
    ...
    if (found_evacuated_only_place && allow_contention_moving)
    {
        ...
        if (is_direction_up)
        {
            fes_id_ndx = MIN(from_here_cannot_move, max_in_array);
        }
        else
        {
            fes_id_ndx = MAX(from_here_cannot_move, min_in_array);
        }
        while (TRUE)
        {
            /** Loop condition.*/
        ...
        if (priority != BCM_FIELD_ACTION_DONT_CARE)
        {
            ...
            if (DNX_FIELD_ACTION_PRIORITY_ARRAY(priority) % 2 != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Priority 0x%04x points to array %d, "
                             ...
                             priority,
                             DNX_FIELD_ACTION_PRIORITY_ARRAY(priority), dnx_field_stage_text(unit, field_stage));
            }
        }
    }
    ...
    dnx_algo_field_action_fes_alloc_out_t * alloc_result_p)
{
    ...
    unsigned int nof_existing_fes_quartets_in_fg;
    uint8 after_invalidate_next[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_INSTRUCTION_PER_CONTEXT];
    uint8 after_invalidate_next_new[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_INSTRUCTION_PER_CONTEXT];
    uint8 banned_fes_id[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_INSTRUCTION_PER_CONTEXT];
    dnx_algo_field_action_fes_alloc_fes_quartet_t fes_quartet[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP];
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_algo_field_action_fes_allocate_verify
                          (unit, field_stage, fg_id, context_id, current_state_in_p, fes_fg_in_p, alloc_result_p));

    /*
     * Collect current state information.
     */
    SHR_INVOKE_VERIFY_DNX(dnx_algo_field_action_fes_allocate_collect_current_state_info
                          (unit, field_stage, fg_id, context_id, current_state_in_p,
                           &current_state_changing, after_invalidate_next, &nof_existing_fes_quartets_in_fg));
    /*
     * Translate New FES quartets information
     */
    SHR_INVOKE_VERIFY_DNX(dnx_algo_field_action_fes_allocate_collect_new_actions_info
                          (unit, field_stage, fg_id, context_id, fes_fg_in_p, nof_existing_fes_quartets_in_fg,
            ...
            SHR_IF_ERR_EXIT(dnx_algo_field_action_fes_allocate_non_mandatory_position
                            (unit, field_stage, context_id,
                             ...
                             fes_id_ndx, 0, dnx_data_field.stage.stage_info_get(unit,
                                                                                field_stage)->nof_fes_instruction_per_context,
                             FALSE, FALSE, allow_fes_pgm_sharing, &current_state_changing, alloc_result_p,
                             &found_allocation));
     ...
     */
    for (fes_change_ndx = 0, fes_quartet_ndx = 0; fes_change_ndx < alloc_result_p->nof_fes_quartet_changes;
         fes_change_ndx++)
    {
        if (alloc_result_p->fes_quartet_change[fes_change_ndx].place_in_alloc != DNX_ALGO_FIELD_ACTION_MOVEMENT)
        {
            ...
            if (alloc_result_p->fes_quartet_change[fes_change_ndx].place_in_alloc != DNX_ALGO_FIELD_ACTION_MOVEMENT &&
                alloc_result_p->fes_quartet_change[fes_change_ndx].place_in_alloc != fes_quartet_ndx)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             ...
                             fes_change_ndx, fes_quartet_ndx);
            }
            fes_quartet_ndx++;
        }
    }
    if (fes_quartet_ndx != fes_fg_in_p->nof_fes_quartets)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
...
/**see H file: algo_field.h */
shr_error_e
dnx_algo_field_action_fes_priority_get(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
    ...
    unsigned int *num_fes_quartets)
{
    ...
    unsigned int fes_quartet_ndx;
    dnx_field_group_t curr_fg_id;
    dnx_field_action_priority_t curr_priority;
    uint8 curr_fes_pgm_id;
    unsigned int max_action_index;
    int action_found;
    uint8 curr_place_in_fg;
    int prio_arr_filled[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP];

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(context_state, _SHR_E_PARAM, "context_state");
    SHR_NULL_CHECK(priorities, _SHR_E_PARAM, "priorities");
    SHR_NULL_CHECK(num_fes_quartets, _SHR_E_PARAM, "num_fes_quartets");
    ...
    /*
     * Initialize prio_arr_filled. Used only as a sanity check to ensure there are no "holes"
     * in the priorities array.
     */
    for (fes_quartet_ndx = 0; fes_quartet_ndx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP;
         fes_quartet_ndx++)
    {
        priorities[fes_quartet_ndx] = FALSE;
    }
    /*
     * Fill the priorities.
     */
    ...
    {
        /*
         * Only check allocated FES IDs.
         */
        curr_fes_pgm_id = context_state->fes_id_info[fes_id_ndx].fes_pgm_id;
        if (curr_fes_pgm_id != DNX_ALGO_FIELD_ZERO_FES_PROGRAM)
        {
            /*
            ...
            switch (field_stage)
            {
                ...
                case DNX_FIELD_STAGE_IPMF2:
                    SHR_IF_ERR_EXIT(dnx_algo_field_action_sw.ipmf2.context_state.
                                    priority.get(unit, context_id, fes_id_ndx, &curr_priority));
                    SHR_IF_ERR_EXIT(dnx_algo_field_action_sw.ipmf2.fes_state.
                                    field_group.get(unit, fes_id_ndx, curr_fes_pgm_id, &curr_fg_id));
                    SHR_IF_ERR_EXIT(dnx_algo_field_action_sw.ipmf2.fes_state.
                                    place_in_fg.get(unit, fes_id_ndx, curr_fes_pgm_id, &curr_place_in_fg));
                    break;
                case DNX_FIELD_STAGE_IPMF3:
                    SHR_IF_ERR_EXIT(dnx_algo_field_action_sw.ipmf3.context_state.
                                    priority.get(unit, context_id, fes_id_ndx, &curr_priority));
            ...
            /*
             * Only return FES quartet belonging to the field group.
             */
            if (curr_fg_id == fg_id)
            {
                /*
                 * Sanity check.
                 */
                if (curr_place_in_fg > DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP - 1)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Found action field group index (%d) larger then the maximum number of "
                                 "actions per group (%d). Context ID %d field group %d. \r\n",
                                 curr_place_in_fg, DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP - 1,
                                 context_id, fg_id);
                }
                /*
                 * Update the priority, and that we updated the action's priority.
                 */
                priorities[curr_place_in_fg] = curr_priority;
                prio_arr_filled[curr_place_in_fg] = TRUE;
                action_found = TRUE;
                /*
                 * Update the maximum action_fg_index
                 */
                if (curr_place_in_fg > max_action_index)
                {
                    max_action_index = curr_place_in_fg;
                }
            }
        }
    }
    /*
     * Check if an action was found.
     */
    if (action_found == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Found no actions for context ID %d field group %d.\r\n", context_id, fg_id);
    }
    /*
     * If we found actions, update the number of action priorities updated.
     */
    (*num_fes_quartets) = max_action_index + 1;
    /*
     * Sanity check, check that there are no uninitialized "holes" in the priorities array.
     */
    for (fes_quartet_ndx = 0;
         fes_quartet_ndx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP
         && fes_quartet_ndx < (*num_fes_quartets); fes_quartet_ndx++)
    {
        if (prio_arr_filled[fes_quartet_ndx] == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Missing action %d in field group %d.\r\n", fes_quartet_ndx, fg_id);
        }
    }
...
/**see H file: algo_field.h */
shr_error_e
dnx_algo_field_action_sw_state_init(
    int unit)
{
    unsigned int fes_id_ndx;
    unsigned int fes_pgm_ndx;
