    ...
    dnx_ingress_ire_counter_info_t * counter_info)
{
   ...
   /** Select the core */
    ...
    counter_info->nif_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->nif_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->nif_pkt_cnt = counter_info->nif_pkt_cnt & ((1 << (counter_size - 1)) - 1);

    /** Get the overflow status and real counter value for RCYH packet count */
    dbal_tables_field_size_get(unit, DBAL_TABLE_INGR_REASSEMBLY_IRE_COUNTER,
                               DBAL_FIELD_RCYH_PACKET_COUNTER, 0, 0, 0, &counter_size);
    counter_info->rcyh_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->rcyh_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->rcyh_pkt_cnt = counter_info->rcyh_pkt_cnt & ((1UL << (counter_size - 1)) - 1);
    ...
    counter_info->rcyl_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->rcyl_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->rcyl_pkt_cnt = counter_info->rcyl_pkt_cnt & ((1UL << (counter_size - 1)) - 1);

    /** Get the overflow status and real counter value for CPU packet count */
    dbal_tables_field_size_get(unit, DBAL_TABLE_INGR_REASSEMBLY_IRE_COUNTER,
                               DBAL_FIELD_CPU_PACKET_COUNTER, 0, 0, 0, &counter_size);
    counter_info->cpu_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->cpu_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->cpu_pkt_cnt = counter_info->cpu_pkt_cnt & ((1UL << (counter_size - 1)) - 1);

    /** Get the overflow status and real counter value for OLP packet count */
    dbal_tables_field_size_get(unit, DBAL_TABLE_INGR_REASSEMBLY_IRE_COUNTER,
                               DBAL_FIELD_OLP_PACKET_COUNTER, 0, 0, 0, &counter_size);
    counter_info->olp_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->olp_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->olp_pkt_cnt = counter_info->olp_pkt_cnt & ((1UL << (counter_size - 1)) - 1);

    /** Get the overflow status and real counter value for OAMP packet count */
    dbal_tables_field_size_get(unit, DBAL_TABLE_INGR_REASSEMBLY_IRE_COUNTER,
                               DBAL_FIELD_OAMP_PACKET_COUNTER, 0, 0, 0, &counter_size);
    counter_info->oamp_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->oamp_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->oamp_pkt_cnt = counter_info->oamp_pkt_cnt & ((1UL << (counter_size - 1)) - 1);
    ...
    counter_info->reg_if_pkt_ovf =
        is_gtimer_enabled ? 0 : ((counter_info->reg_if_pkt_cnt >> (counter_size - 1)) && 0x1);
    counter_info->reg_if_pkt_cnt = counter_info->reg_if_pkt_cnt & ((1UL << (counter_size - 1)) - 1);
    ...
    dnx_ingress_sqm_counter_info_t * counter_info)
{
   ...
   /** Select the core */
    ...
    counter_info->enq_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->pkt_enq_cnt >> (counter_size - 1)) && 0x1);
    counter_info->pkt_enq_cnt = counter_info->pkt_enq_cnt & ((1UL << (counter_size - 1)) - 1);
    ...
    counter_info->deq_pkt_ovf = is_gtimer_enabled ? 0 : ((counter_info->pkt_deq_cnt >> (counter_size - 1)) && 0x1);
    counter_info->pkt_deq_cnt = counter_info->pkt_deq_cnt & ((1UL << (counter_size - 1)) - 1);
    ...
    uint32 *best_timer_cycles)
{
    ...
    else
    {
        ...
        *best_grant_size = -1;
        ...
        {
            /** Get actual rate */
            grant_size = UTILEX_DIV_ROUND((current_timer_cycles * rate), DNXCMN_CORE_CLOCK_MHZ_GET(unit));
            actual_rate = UTILEX_DIV_ROUND((grant_size * DNXCMN_CORE_CLOCK_MHZ_GET(unit)), current_timer_cycles);

            delta = UTILEX_DELTA(rate, actual_rate);
            /** Try to get the minumum delta between the requested and
             *  actual rate */
            if (delta < min_delta)
            {
                /** Check if calcualted grant size will fit in the HW field */
 ...
 * \param cosq - unsused. Must be 0
 * \param kbits_sec_min -unsused. Must be 0
 * \param kbits_sec_max - rate in Kbps
 * \param flags - unsused. Must be 0
 * 
 * \return int 
 */
int
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_receive_shaper_grant_size_and_timer_cycles_calc
    ...
    uint32 *actual_rate)
{
    ...
    else
    {
        ...
        *actual_rate = UTILEX_DIV_ROUND(((grant_size) * DNXCMN_CORE_CLOCK_MHZ_GET(unit)), timer_cycles);
    }
exit:
    SHR_FUNC_EXIT;
}
 ...
 * \param kbits_sec_max - ingress shaper rate. Rate 
 *                 of 0 disables the shaper limit
 * \param flags 
 * 
 * \return int 
 */
 ...
 * 
 * \param unit 
 * \param rate_mpps - the rate in mega packets per second. Rate 
 *                 of 0 disables the shaper limit
 * 
 * \return int 
 */
