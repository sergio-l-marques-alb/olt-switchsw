        ...
        if (Kbp_app_data[unit]->device_type == KBP_DEVICE_OP2)
        {
            ...
            else
            {
                ...
                kbp_config->settings.tap_settings.enable_taps =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->txfir_tap_enable;
                kbp_config->settings.tap_settings.pre2 =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->pre2;
                kbp_config->settings.tap_settings.pre1 = dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->pre;
                kbp_config->settings.tap_settings.mainx =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->main;
                kbp_config->settings.tap_settings.post1 =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->post;
                kbp_config->settings.tap_settings.post2 =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->post2;
                kbp_config->settings.tap_settings.post3 =
                    dnx_data_elk.connectivity.kbp_serdes_tx_taps_get(unit, 0)->post3;
            }
    ...
    bcm_core_t core)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LOOKUP_TIMEOUT_DLY, INST_SINGLE, 0x5E4);
    /*
     * commit the value
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    bcm_core_t core)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TIMER_DELAY, INST_SINGLE, 0x13);
    /*
    ...
    int unit)
{
    ...
    if(port_bmp != 0)
    {
        is_nrz = ((final_speed == 100000) && (nof_lanes == 4)) ? TRUE : FALSE;
        DNX_KBP_TRY(kbp_device_pm_interface_init
            (Kbp_app_data[unit]->xpt_p, (final_speed/1000), port_bmp, is_nrz, link_training));
    }         
