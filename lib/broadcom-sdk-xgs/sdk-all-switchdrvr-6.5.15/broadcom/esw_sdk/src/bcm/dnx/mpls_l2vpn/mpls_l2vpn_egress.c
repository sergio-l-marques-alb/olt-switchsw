    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_out_lif_profile_get_data(unit, out_lif_profile, &out_lif_profile_info, DBAL_TABLE_EEDB_PWE));
...
*  
* \param [in] unit -Relevant unit.
* \param [in] mpls_port_id - l2vpn_info->mpls_port->mpls_port_id.
* \param [in] mpls_port_flags - l2vpn_info->mpls_port->flags.
* \param [in] l2vpn_flags - l2vpn_info->flags.
    ...
    uint8 *esem_cmd_profile_last_reference)
{
    ...
    if (!_SHR_IS_FLAG_SET(l2vpn_flags, DNX_MPLS_L2VPN_FLAG_EGRESS_IML))
    {
        ...
        else
        {
             ...
             */
            if (_SHR_IS_FLAG_SET(mpls_port_flags, BCM_MPLS_PORT_SERVICE_TAGGED))
            {
            ...
            if (_SHR_IS_FLAG_SET(mpls_port_flags, BCM_MPLS_PORT_REPLACE))
            {
                SHR_IF_ERR_EXIT(dnx_port_esem_cmd_data_sw_get(unit, old_esem_cmd, &old_esem_cmd_data));
            ...
            else if (esem_cmd_range_flags & DNX_PORT_ESEM_CMD_RANGE_0_3)
            {
                /** Allocate esem cmd in a larger range which is feasible even though it cases a different entry format.*/
                rv = dnx_port_esem_cmd_exchange(unit, mpls_port_id,
                                                DNX_PORT_ESEM_CMD_ALLOCATE |
                                                DNX_PORT_ESEM_CMD_RANGE_0_7,
                                                DBAL_ENUM_FVAL_ESEM_APP_DB_ID_FODO_OUTLIF,
                                                ESEM_ACCESS_TYPE_ETH_NATIVE_AC,
                                                esem_default_result_profile, old_esem_cmd, new_esem_cmd,
                                                esem_cmd_data, esem_cmd_profile_first_reference,
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_exchange(unit, mpls_port_id,
                                                   ...
                                                   ESEM_ACCESS_TYPE_DUAL_HOME, esem_default_result_profile,
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_egress_lif_esem_cmd_sw_alloc(unit, l2vpn_info->mpls_port->mpls_port_id,
    ...
    if (qos_pipe_profile_first_reference)
    {
        SHR_IF_ERR_EXIT(dnx_qos_egress_network_qos_pipe_profile_set
                        (unit, encap_info.nwk_qos_id, encap_info.label_array[0].exp, encap_info.label_array[0].exp, 0));
    }
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    if (encap_info.esem_command_profile != dnx_data_esem.access_cmd.default_native_get(unit))
    {
        l2vpn_info->mpls_port->flags |= BCM_MPLS_PORT_SERVICE_TAGGED;
    }
    ...
    bcm_gport_t mpls_l2vpn_id)
{
    ...
    if (encap_info.label_array[0].egress_qos_model.egress_ttl == bcmQosEgressModelPipeMyNameSpace)
    {
        SHR_IF_ERR_EXIT(algo_qos_db.qos_egress_ttl_pipe_profile.free_single
                        (unit, _SHR_CORE_ALL, encap_info.ttl_pipe_profile, &ttl_last_reference));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_out_lif_profile_exchange
                    (unit, gport_hw_resources.outlif_dbal_table_id, 0, &out_lif_profile_info,
                     encap_info.out_lif_profile, &new_out_lif_profile));
    ...
    if (encap_info.out_lif_llvp_profile != dnx_data_port_pp.general.default_egress_native_ac_llvp_profile_id_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_egress.free_single
                        (unit, _SHR_CORE_ALL, encap_info.out_lif_llvp_profile, &is_last));
