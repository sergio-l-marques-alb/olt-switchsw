    ...
    else
    {
        ...
        else
        {
            int current_core, core_id =
                (local_lif_info->core_id == (char) _SHR_CORE_ALL) ? _SHR_CORE_ALL : local_lif_info->core_id;
    ...
    int global_lif)
{
    ...
    if (is_ingress)
    {
        ...
        {
            int current_core, core_id;
            core_id = (local_lif_info.core_id == (char) _SHR_CORE_ALL) ? _SHR_CORE_ALL : local_lif_info.core_id;
            DNXCMN_CORES_ITER(unit, core_id, current_core)
            {
                SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.ingress_dpc_lif_to_global_lif_map.set(unit, current_core,
                                                                                              local_lif_info.local_lif,
    ...
    int *global_lif)
{
    ...
    if (is_ingress)
    {
        ...
        {
            int core_id = (local_lif_info->core_id == (char) _SHR_CORE_ALL) ? _SHR_CORE_ALL : local_lif_info->core_id;
    ...
    int unit)
{
     ...
     */
    lif_map_size = dnx_data_lif.in_lif.nof_sbc_local_in_lifs_get(unit);
    SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.ingress_sbc_lif_to_global_lif_map.alloc(unit));
    ...
    lif_map_size = dnx_data_lif.in_lif.nof_dpc_local_in_lifs_get(unit);
    SHR_IF_ERR_EXIT(lif_mngr_db.lif_mapping.ingress_dpc_lif_to_global_lif_map.alloc(unit));
     ...
     */
    max_nof_lifs =
        UTILEX_MAX(dnx_data_lif.in_lif.nof_sbc_local_in_lifs_get(unit),
                   dnx_data_lif.in_lif.nof_dpc_local_in_lifs_get(unit));
     ...
     */
    egress_init_info.max_nof_elements =
        DNX_ALGO_LIF_HASH_TABLE_SIZE_CONVERT(dnx_data_lif.global_lif.nof_glem_lines_get(unit));
    egress_init_info.expected_nof_elements =
        DNX_ALGO_LIF_HASH_TABLE_SIZE_CONVERT(dnx_data_lif.global_lif.nof_glem_lines_get(unit));
