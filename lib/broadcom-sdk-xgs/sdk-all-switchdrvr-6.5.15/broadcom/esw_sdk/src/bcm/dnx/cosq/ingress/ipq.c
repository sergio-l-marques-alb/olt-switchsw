...
static shr_error_e dnx_cosq_ipq_sw_state_db_init(
    int unit);
static shr_error_e dnx_cosq_ipq_sw_state_db_deinit(
    int unit);
    ...
    int unit)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.max_multicast_queue.set(unit, DNX_IPQ_FMQ_BUNDLE_SIZE(unit) - 1));
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_enhance_scheduler_mode.set(unit, FALSE));
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_fmq_allocated.set(unit, FALSE));
    ...
    dnx_ipq_tc_map_t * tc_default_profile)
{
    ...
    for (voq_offset = 0; voq_offset < DNX_COSQ_NOF_TC; ++voq_offset)
    {
        /*
         ...
         * TC6-7 --> FMQ3
         */
        tc_default_profile->tc[voq_offset] = is_fmq ? voq_offset / 2 : voq_offset;
    }
    ...
    int unit)
{
     ...
     */
    create_data.flags = SW_STATE_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE;
    ...
    sal_strncpy(create_data.name, "Ipq tc map voq flow profile", SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);

    /*
     * Create the template manager for the TC flow
     */
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.tc_voq_flow_map.create(unit, &create_data, NULL));
    ...
    sal_strncpy(create_data.name, "Ipq tc map sysport profile", SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
 ...
 */
static shr_error_e
dnx_cosq_ipq_sw_state_db_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    int unit)
{
    ...
    /** deinit sw state db */
    SHR_IF_ERR_CONT(dnx_cosq_ipq_sw_state_db_deinit(unit));

    /*
     * Delete queues bitmap.
     */
    SHR_IF_ERR_CONT(dnx_cosq_ipq_alloc_mngr_deinit(unit));
    ...
    dnx_ipq_tc_map_t * tc_mapping)
{
    ...
    {
        voq_offset = tc_mapping->tc[sys_tc];
        if (is_fmq)
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_fmq_tc_to_voq_offset_map_set(unit, sys_tc, voq_offset));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_set(unit, sys_tc, tc_profile, is_flow, voq_offset));
        }
    }
    ...
    int unit)
{
    ...
    /** 
     * Set default IPQ registers configuration.
     */
    SHR_IF_ERR_EXIT(jer2_ipq_init(unit));

    /**
     * For FMQ simple scheduling mode, only first 4 queues are multicast (base_queue=0),
     * and the rest of the queues will be unicast.
     * The Alloc manager cannot handle this case, since it is set to region granularity.
     * We allocate the first queue quartet as Unicast to make sure it is occupied (dummy queue),
     * so real unicast queues will start with queue 4.
     * For simple FMQ scheduling mode the allocation is done through sw_state
     * dedicated variable, and not through allocation manager.
     * If FMQ scheduling mode will be change to enhance mode, we will
     * deallocate this dummy queue.
     */
    ...
    for (is_flow = 0; is_flow <= 1; ++is_flow)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_to_voq_offset_map_set(unit, profile, is_flow, is_fmq, &default_tc_mapping));
    }
    ...
    /** 
     * Set default multicast fabric TC mapping to HW.
     */
    SHR_IF_ERR_EXIT(jer2_arad_multicast_fabric_init(unit));
    ...
    uint32 tc_profile)
{
    ...
    if (is_flow)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_voq_flow_quartet_to_flow_profile_map_set(unit, flow_quartet, tc_profile));
    }
    else
    {
        sysport = dest_idx;
    ...
    uint32 *tc_profile)
{
    ...
    {
        flow_quartet = dest_idx / min_bundle_size;
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_voq_flow_quartet_to_flow_profile_map_get(unit, flow_quartet, tc_profile));
    }
    else
    {
        sysport = dest_idx;
    ...
    dnx_ipq_tc_map_t * tc_mapping)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_profile_get(unit, dest_ndx, is_flow, &profile_ndx));

    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.profile_data_get(unit, _SHR_CORE_ALL, profile_ndx, &ref_count, tc_mapping));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_sysport_map.profile_data_get(unit, _SHR_CORE_ALL, profile_ndx, &ref_count, tc_mapping));
    }
    ...
    int *is_allocated)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_profile_get(unit, dest_ndx, is_flow, old_map_profile));

    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.exchange(unit, BCM_CORE_ALL, flags, tc_mapping, *old_map_profile, NULL,
                                                 new_map_profile, (uint8 *) is_allocated, (uint8 *) is_last));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_sysport_map.exchange(unit, BCM_CORE_ALL, flags, tc_mapping, *old_map_profile, NULL,
                                                new_map_profile, (uint8 *) is_allocated, (uint8 *) is_last));
    }
exit:
    SHR_FUNC_EXIT;
    ...
    int *base_queue)
{
    ...
    {
        /*
         * In FMQ simple scheduler mode we have only 4 queues (1 queue bundle)
         * at FMQ 0.
         * This FMQ will not be allocated through resource manager, but through sw_state.
         * Note that queue bundle 0 will always be allocated in the queue's bitmap
         * when using FMQ simple scheduler mode.
         */
        ...
        {
            int is_with_id = _SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID);
            int is_fmq_allocated = 0;

            if (is_with_id == TRUE && *base_queue != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Only FMQ 0 can be allocated in FMQ's simple scheduler mode");
            }

            if (num_cos != DNX_IPQ_FMQ_BUNDLE_SIZE(unit))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "FMQ can only have numq=%d in simple scheduler mode",
                             DNX_IPQ_FMQ_BUNDLE_SIZE(unit));
            }

            SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_fmq_allocated.get(unit, &is_fmq_allocated));
            if (is_fmq_allocated == TRUE)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "FMQ is already allocated");
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_fmq_allocated.set(unit, TRUE));
                SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.set(unit,
                                                                   0 / dnx_data_ipq.queues.min_bundle_size_get(unit),
                                                                   DNX_IPQ_FMQ_BUNDLE_SIZE(unit)));
                *base_queue = 0;
                SHR_EXIT();
            }
    ...
    /*
     * Allocate queue bundles 
     */
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.max_multicast_queue.get(unit, &max_mc_queue));
    element = *base_queue / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    alloc_info.queue_type = queue_type;
    alloc_info.nof_queue_bundles_to_alloc = nof_queue_bundles_to_alloc;
    /*
     * The max_mc_queue is relevant to determine the range for unicst, in case of enhanced FMQ mode.
     * in FMQ simple mode we allocate the FMQ quartet in the Alloc manager as Unicast, 
     * so it will be occupied. otherwise, it is handled through SW state.
     */
    alloc_info.max_mc_queue = (is_fmq_enhance_scheduler_mode) ? max_mc_queue : 0;
     ...
     */
    *base_queue = element * DNX_IPQ_MIN_BUNDLE_SIZE(unit);

    /*
     * After allocation done, save queue's num_cos 
     */
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.set(unit, element, num_cos));
...
dnx_cosq_ipq_queue_deallocate(
    ...
    int base_queue)
{
    int element = base_queue / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    int is_fmq_enhance_scheduler_mode = 0;
    int is_fmq_allocated = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.sys_port_ref_counter.set(unit, element, 0));

    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_enhance_scheduler_mode.get(unit, &is_fmq_enhance_scheduler_mode));
    /**
     * FMQ simple scheduling mode is allocated through sw_state, so
     * need to deallocate it only in sw_state and not in alloc manager.
     */
    if (element == 0 && is_fmq_enhance_scheduler_mode == FALSE)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_fmq_allocated.get(unit, &is_fmq_allocated));
        if (is_fmq_allocated == TRUE)
        {
            SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.fmq.is_fmq_allocated.set(unit, FALSE));
            SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.set(unit, element, 0));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "FMQ is not allocated");
        }
    }
    else
    {
...
dnx_cosq_ipq_queue_is_allocated(
    ...
    int *is_allocated)
{
    uint8 sw_state_is_allocated;
    uint32 queue_quartet;
    uint32 min_bundle_size = dnx_data_ipq.queues.min_bundle_size_get(unit);
    SHR_FUNC_INIT_VARS(unit);

    queue_quartet = base_voq / min_bundle_size;
    SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.ipq_alloc_mngr.is_allocated(unit, queue_quartet, &sw_state_is_allocated));
    ...
    uint32 *sysport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_get(unit, queue_quartet, sysport));
    ...
    int *num_cos)
{
     ...
     * element represent bundle of queues, whereas base_queue holds the real queue id 
     */
    element = queue_id / DNX_IPQ_MIN_BUNDLE_SIZE(unit);

    /*
     * Get queue num_cos
     */
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.get(unit, element, &sw_state_cos));
    ...
    int queue_id)
{
     ...
     * Validate queue id 
     */
    if (queue_id > dnx_data_ipq.queues.nof_queues_get(unit) - 1 || queue_id < 0)
    {
    ...
    for (queue_i = range_change_start; queue_i < range_change_end; queue_i += min_bundle_size)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.get(unit, base_queue, &num_cos));
        if (num_cos != 0)
        {
...
dnx_cosq_ipq_tc_mapping_verify(
    ...
    bcm_port_t port)
{
    int base_voq = 0, core = BCM_CORE_ALL, is_allocated;
    SHR_FUNC_INIT_VARS(unit);

    if (BCM_GPORT_IS_UCAST_QUEUE_GROUP(port))
    {
        base_voq = BCM_GPORT_UNICAST_QUEUE_GROUP_QID_GET(port);
        core = BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_GET(port);
    }
    else if (BCM_GPORT_IS_MCAST_QUEUE_GROUP(port))
    {
        base_voq = BCM_GPORT_MCAST_QUEUE_GROUP_QID_GET(port);
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "API bcm_cosq_port_mapping_set() Gport must be configured to BCM_CORE_ALL.");
    }
    /*
     * Verify base queue is allocated
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_is_allocated(unit, base_voq, &is_allocated));
    if (!is_allocated)
    {
...
{
    int min_bundle_size = dnx_data_ipq.queues.min_bundle_size_get(unit);
    uint32 destination, queue_quartet, sys_tc, voq_offset, old_profile, is_fmq;
    dnx_ipq_tc_map_t tc_mapping;
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_cosq_ipq_tc_mapping_verify(unit, port));
    ...
    if (is_flow)
    {
         ...
         */
        queue_quartet = dest_base_queue / min_bundle_size;
        SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.get(unit, queue_quartet, &sw_state_num_cos));
         ...
         * but we always set all profiles to base_queue 0, so effectively dest_ndk = dest_base_queue.
         */
     ...
     * Get old data
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_map_profile_data_get(unit, dest_ndx, is_flow, &tc_mapping));

    /*
     * Set data configuration
     */
    tc_mapping.tc[sys_tc] = voq_offset;

    /*
     * Exchange SW Data
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_map_profile_exchange
                    (unit, dest_ndx, is_flow, &tc_mapping, &old_profile, &new_profile, &is_last, &is_allocated));
    if (is_allocated)
    {
        /*
         ...
         */
        is_fmq = FALSE;
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_to_voq_offset_map_set(unit, new_profile, is_flow, is_fmq, &tc_mapping));
    }
    ...
    for (quartet_index = 0; quartet_index < nof_quartet; quartet_index++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_profile_set(unit, destination, is_flow, new_profile));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
    ...
    int min_bundle_size = dnx_data_ipq.queues.min_bundle_size_get(unit);
    int nof_cores, egress_region_size, nof_quads;
    ...
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.num_cos.get(unit, queue_quad_base, &nof_cos));
    ...
    bcm_cosq_gport_connection_t * gport_connect)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_get(unit, queue_quad_base, &sysport));
    ...
    uint32 *is_stk)
{
    ...
    *is_stk = FALSE;
     ...
     */
    SHR_IF_ERR_EXIT(bcm_dnx_stk_my_modid_get(unit, &local_modid));
    /*
     ...
     */
    SHR_IF_ERR_EXIT(dnx_stk_domain_stk_modid_to_domain_find(unit, local_modid, sysport_modid, &is_same_domain));
    ...
    bcm_gport_t sysport_gport)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_set(unit, sysport, stk_base_queue, TRUE));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int is_composite)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.sys_port_ref_counter.inc(unit, base_voq_qrtt, 1));
    ...
    if (is_composite)
    {
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_flow_map_is_composite_range_set
                        (unit, BCM_CORE_ALL, base_voq_qrtt, num_quartets, is_composite));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_range_set
                    (unit, base_voq_qrtt, num_quartets, sysport));
    ...
    if (!is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_set(unit, sysport, base_voq, TRUE));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_cosq_ingress_queue_bundle_gport_config_t * config)
{
    ...
    for (cosq = 0; cosq < config->numq; cosq++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_iqs_credit_request_profile_map_set(unit, base_voq, cosq, delay_tolerance_profile));
        /*
         * Set new rate class in HW
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_set(unit, base_voq + cosq, rate_class));
        /*
         * update ref count for the new rate class profile
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.voq_rate_class.ref_count.inc(unit, rate_class, 1));
    }
    ...
    bcm_gport_t * gport)
{
    ...
    if (config->flags & BCM_COSQ_GPORT_WITH_ID)
    {
        ...
        if (core != BCM_CORE_ALL)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Add uc queue failed, core must be BCM_CORE_ALL");
        }
    }
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_allocate(unit, alloc_flags, queue_type, numq, &base_voq));

    /*
     * Map the allocated VOQ to and from system port
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_add_queue_map_set(unit, sysport, base_voq, numq, is_flow, is_composite));

    /*
     * Set Queue profiles - credit request profile and rate class according to config info
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_add_queue_profile_set(unit, base_voq, config));

    /*
     * set the output gport with the allocated base voq.
     */
    BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(*gport, BCM_CORE_ALL, base_voq);
    ...
    bcm_gport_t * gport)
{
    ...
    supported_mcast_flags =
        (BCM_COSQ_GPORT_REPLACE | BCM_COSQ_GPORT_WITH_ID | BCM_COSQ_GPORT_MCAST_QUEUE_GROUP | BCM_COSQ_GPORT_COMPOSITE);
    ...
    bcm_gport_t * gport)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_allocate(unit, alloc_flags, DNX_ALGO_IPQ_QUEUE_TYPE_MULTICAST, numq, &base_fmq));

    /*
     * Map the allocated VOQ to system port
     * (is_flow is passed as TRUE since there is not need to map system port to VOQ base)
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_add_queue_map_set(unit, sysport, base_fmq, numq, TRUE, is_composite));
    /*
     * Set Queue profiles - credit request profile and rate class according to config info
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_add_queue_profile_set(unit, base_fmq, config));

    BCM_GPORT_MCAST_QUEUE_GROUP_CORE_QUEUE_SET(*gport, BCM_CORE_ALL, base_fmq);
    ...
    uint32 *flags)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_ipq_queue_num_cos_get(unit, base_voq, num_cos_levels));
    /*
     * Check if the Queue is mapped to a composite flow
     */
    base_voq_quartet = base_voq / min_bundle_size;
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_flow_map_is_composite_get
                    (unit, BCM_CORE_ALL, base_voq_quartet, &is_composite));
    /*
     * Get the mapping from queue quartet to system port
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_get(unit, base_voq_quartet, &sysport));

    if (is_mc && (sysport == dnx_data_device.general.invalid_system_port_get(unit)))
    ...
    uint32 *flags)
{
    ...
    *flags = 0;
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_get(unit, base_voq, is_mc, physical_port, num_cos_levels, flags));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_get(unit, sysport, &dest_base_queue, &is_dest_valid));
    ...
    int *is_empty)
{
    ...
    for (iter_idx = 0; iter_idx < nof_iterations; iter_idx++)
    {
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_occupancy_info_get(unit, core, base_voq, &queue_info));
    ...
    int queue_quartet)
{
    ...
    if (dnx_data_fabric.general.connect_mode_get(unit) == SOC_DNX_FABRIC_CONNECT_MODE_MESH)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_is_empty_get(unit, voq, DNX_COSQ_IPQ_QUEUE_EMPTY_ITERATIONS, &is_empty));
            if (!is_empty)
            {
     ...
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_sys_port_map_clear(unit, queue_quartet));
    ...
    int is_mc)
{
    ...
    uint32 sysport = 0, voq_quartet, base_voq_quartet, rate_class;
    uint32 min_bundle_size = dnx_data_ipq.queues.min_bundle_size_get(unit);
    bcm_cosq_ingress_queue_bundle_gport_config_t config;
    SHR_FUNC_INIT_VARS(unit);
     ...
     * get num cos levels from SW state
     */
    SHR_IF_ERR_EXIT(dnx_ipq_queue_num_cos_get(unit, base_voq, &numq));
    config.numq = numq;
    /*
    ...
    for (voq = base_voq; voq < base_voq + numq; voq += min_bundle_size)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_queue_quartet_to_flow_map_clear(unit, BCM_CORE_ALL, voq_quartet));
    }
    if (!is_mc)
    {
        base_voq_quartet = base_voq / min_bundle_size;
         ...
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_system_port_to_voq_base_map_clear(unit, sysport));
    }
    /*
     * Delete queue group
     */
    ...
    {
        voq_quartet = voq / min_bundle_size;
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_qrtt_unmap(unit, voq_quartet));
    }
    /*
     * free resources
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_deallocate(unit, base_voq));
    ...
    for (cosq = 0; cosq < numq; cosq++)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_voq_rate_class_get(unit, base_voq + cosq, &rate_class));
        /*
         * Decrement the rate class ref counter since the VOQ is about to be removed.
         */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.voq_rate_class.ref_count.dec(unit, rate_class, 1));
    ...
    for (cosq = 0; cosq < numq; cosq++)
    {
        SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_set(unit, base_voq + cosq, default_compensation));
    }
...
dnx_cosq_ipq_gport_delete_verify(
    ...
    uint32 base_voq)
{
    int voq = 0, numq = 0, core = BCM_CORE_ALL, flow_id;
    ...
    bcm_gport_t gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_delete_verify(unit, base_fmq));
exit:
    SHR_FUNC_EXIT;
}

/**
 * \brief - delete Multicast gport bundle
 *
 * \param [in] unit -The unit number.
 * \param [in] gport - gport of the base queue to be deleted
 *
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
    ...
    bcm_gport_t gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_delete(unit, base_voq, is_mc));
    ...
    bcm_gport_t gport)
{
    ...
    base_voq = BCM_GPORT_UCAST_QUEUE_GROUP_QID_GET(gport);
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_gport_delete_verify(unit, base_voq));
     ...
     *  base_queue/min_queue_size */
    base_voq_quartet = base_voq / min_bundle_size;
    SHR_IF_ERR_EXIT(dnx_ipq_db.base_queues.sys_port_ref_counter.get(unit, base_voq_quartet, &sysports_ref_counter));
...
/**
 * \brief - delete Unicast gport bundle
 *
 * \param [in] unit -The unit number.
 * \param [in] gport - gport of the base queue to be deleted
 *
    ...
    uint32 alloc_flags)
{
    ...
    res_tag_bitmap_create_info.grain_size = region_nof_quartets;
    /*
     * Tag is actually the queue type. so max tag value is [nof_queue_types - 1]
     */
    res_tag_bitmap_create_info.max_tag_value = NUM_DNX_ALGO_IPQ_QUEUE_TYPE - 1;
...
{
    uint8 num_cos = 0;
    uint32 nof_quartets_to_free, base_queue, region_id;
    int quartet_start, nof_quartets_in_region, nof_allocated_quartets;
     ...
     */
    dnx_ipq_db.base_queues.num_cos.get(unit, element, &num_cos);
    nof_quartets_to_free = num_cos / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
     ...
     */
    nof_quartets_in_region = DNX_IPQ_NOF_QUEUES_IN_REGION(unit) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    quartet_start = region_id * nof_quartets_in_region;
    SHR_IF_ERR_EXIT(sw_state_resource_nof_allocated_elements_in_range_get
                    (unit, module_id, res_tag_bitmap, quartet_start, nof_quartets_in_region, &nof_allocated_quartets));

    if (nof_allocated_quartets == 0)
    {
        /*
    ...
    int unit)
{
    ...
    create_data.nof_elements = DNX_IPQ_NOF_QUEUES(unit) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    create_data.advanced_algorithm = DNX_ALGO_RESOURCE_ADVANCED_ALGORITHM_IQM;
    sal_strncpy(create_data.name, "Ipq alloc mngr", SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
 ...
 */
shr_error_e
dnx_cosq_ipq_alloc_mngr_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    /*
     * Sw state is deinitialized in ipq module.
     */
...
/*
 * Get valid quartet range to alloc from, depending on queue type.
 */
static shr_error_e
dnx_cosq_ipq_queue_type_quartet_range_get(
    int unit,
    dnx_ipq_queue_type_e queue_type,
    int max_mc_queue,
    uint32 *start_quartet,
    uint32 *end_quartet)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (queue_type == DNX_ALGO_IPQ_QUEUE_TYPE_MULTICAST)
    {
        /*
         * FMQ enhance mode.
         * FMQ simple mode is handled through SW state and allocated as unicast in Alloc manager.
         */
        *start_quartet = 0;
        /** end quartet is the first invalid element */
        *end_quartet = (max_mc_queue + 1) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    }
    else
    {
        /*
         ...
         */
        *start_quartet = (max_mc_queue + 1) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
        /** end quartet is the first invalid element */
        *end_quartet = DNX_IPQ_NOF_QUEUES(unit) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    }
    ...
    int *element)
{
    ...
    int max_mc_queue = alloc_info_p->max_mc_queue;
    int tag, region_id, nof_quartets_in_region, nof_allocated_quartets;
    uint32 internal_flags = 0, start_quartet, end_quartet;
    SHR_FUNC_INIT_VARS(unit);

    /*
     * get quartet range from queue type
     */
    SHR_IF_ERR_EXIT(dnx_cosq_ipq_queue_type_quartet_range_get
                    (unit, queue_type, max_mc_queue, &start_quartet, &end_quartet));
    /*
    ...
    if (is_with_id)
    {
         ...
         */
        if (*element < start_quartet || *element > end_quartet)
        {
            SHR_ERR_EXIT(BCM_E_PARAM, "Requested queue quartet %d is out of bounds. Valid range is: %d - %d", *element,
                         start_quartet, end_quartet);
        }
        /*
    ...
    /*
     * Allocate element in a matching type region, or a new region
     */
    ...
    res_tag_bitmap_alloc_info.count = nof_quartets_to_alloc;
    res_tag_bitmap_alloc_info.tag = tag;
    res_tag_bitmap_alloc_info.range_start = start_quartet;
    res_tag_bitmap_alloc_info.range_end = end_quartet;

    SHR_IF_ERR_EXIT(sw_state_resource_tag_bitmap_alloc(unit, module_id, res_tag_bitmap, res_tag_bitmap_alloc_info,
                                                       element));
    /*
     * Check if we allocated a new region
     */
    nof_quartets_in_region = DNX_IPQ_NOF_QUEUES_IN_REGION(unit) / DNX_IPQ_MIN_BUNDLE_SIZE(unit);
    region_id = *element / nof_quartets_in_region;
    start_quartet = region_id * nof_quartets_in_region;
    SHR_IF_ERR_EXIT(sw_state_resource_nof_allocated_elements_in_range_get
                    (unit, module_id, res_tag_bitmap, start_quartet, nof_quartets_in_region, &nof_allocated_quartets));

    if (nof_allocated_quartets == nof_quartets_to_alloc)
    {
        /*
