    ...
    const dnx_oam_oamp_mep_db_args_t * entry_values)
{
    ...
    if (UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_RESET_DM_STATISTICS))
    {
        ...
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMM, INST_SINGLE, value_array8);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMR, INST_SINGLE, value_array8);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMM, INST_SINGLE, value_array8);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMR, INST_SINGLE, value_array8);
        /** Reset minimum must be the maximum value */
        sal_memset(value_array8, 0xFF, DBAL_FIELD_ARRAY_MAX_SIZE_IN_BYTES);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMM, INST_SINGLE, value_array8);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMR, INST_SINGLE, value_array8);
    }

    if (UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_COPY_DM_STATISTICS))
    {
        /** If we move the entry, we need to copy the values */
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.last_delay_dmm, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMM, INST_SINGLE, value_array8);
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.last_delay_dmr, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMR, INST_SINGLE, value_array8);
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.max_delay_dmm, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMM, INST_SINGLE, value_array8);
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.max_delay_dmr, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMR, INST_SINGLE, value_array8);
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.min_delay_dmm, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMM, INST_SINGLE, value_array8);
        sal_memcpy(value_array8, entry_values->exclusive.lm_dm.min_delay_dmr, ONE_WAY_DELAY_SIZE);
        dbal_entry_value_field_arr8_set(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMR, INST_SINGLE, value_array8);
    }
    ...
    uint32 entry_handle_id)
{
    ...
    /** Get field values   */
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMM, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.last_delay_dmm, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_LAST_DELAY_DMR, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.last_delay_dmr, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMM, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.max_delay_dmm, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_MAX_DELAY_DMR, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.max_delay_dmr, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMM, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.min_delay_dmm, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_MIN_DELAY_DMR, INST_SINGLE, value_array8);
    sal_memcpy(entry_values->exclusive.lm_dm.min_delay_dmr, value_array8, ONE_WAY_DELAY_SIZE);
    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_LAST_ENTRY, INST_SINGLE, &flag);
    UTILEX_SET_BIT(entry_values->flags, flag, OAMP_MEP_LAST_ENTRY);
    ...
    const dnx_oam_oamp_mep_db_args_t * entry_values)
{
    ...
    SHR_ALLOC_SET_ZERO(read_entry_values, sizeof(dnx_oam_oamp_mep_db_args_t), "Struct for MEP DB entry value verification",
                       "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    do
    {
        ...
        /** Value of UC/MC flag is ignored.  It's just there for future use */
        UTILEX_SET_BIT(read_entry_values->flags, UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_MC_UC_SEL), OAMP_MEP_MC_UC_SEL);

        /** Value of ECHO flag is ignored.  It's just there for future use */
        UTILEX_SET_BIT(read_entry_values->flags, UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_BFD_ECHO), OAMP_MEP_BFD_ECHO);

        /** Value of Down MEP Egress Injection is not a part of MEP DB */
        UTILEX_SET_BIT(read_entry_values->flags, UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_EGRESS_INJECTION), OAMP_MEP_EGRESS_INJECTION);
        ...
        read_entry_values->flags =
               ...
               (UTILEX_GET_BITS_RANGE(entry_values->flags, OAMP_MEP_COPY_DM_STATISTICS, OAMP_MEP_LM_ENABLE) << OAMP_MEP_LM_ENABLE);
        ...
        {
            UTILEX_SET_BIT(read_entry_values->flags, UTILEX_GET_BIT(entry_values->flags, OAMP_MEP_BFD_TUNNEL_MPLS_ENABLE), OAMP_MEP_BFD_TUNNEL_MPLS_ENABLE);
           ...
           (entry_values->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_Y1731_PWE))
        {
            ...
            read_entry_values->exclusive.oam_only.oam_inner_tpid_index = entry_values->exclusive.oam_only.oam_inner_tpid_index;
            read_entry_values->exclusive.oam_only.oam_outer_vid = entry_values->exclusive.oam_only.oam_outer_vid;
            read_entry_values->exclusive.oam_only.oam_outer_tpid_index = entry_values->exclusive.oam_only.oam_outer_tpid_index;
            read_entry_values->exclusive.oam_only.oam_sa_mac_lsb = entry_values->exclusive.oam_only.oam_sa_mac_lsb;
            read_entry_values->exclusive.oam_only.oam_sa_mac_msb_profile = entry_values->exclusive.oam_only.oam_sa_mac_msb_profile;
            read_entry_values->exclusive.oam_only.oam_inner_dei_pcp = entry_values->exclusive.oam_only.oam_inner_dei_pcp;
            read_entry_values->exclusive.oam_only.oam_outer_dei_pcp = entry_values->exclusive.oam_only.oam_outer_dei_pcp;
            read_entry_values->exclusive.oam_only.oam_nof_vlan_tags = entry_values->exclusive.oam_only.oam_nof_vlan_tags;
        }
        else
        {
