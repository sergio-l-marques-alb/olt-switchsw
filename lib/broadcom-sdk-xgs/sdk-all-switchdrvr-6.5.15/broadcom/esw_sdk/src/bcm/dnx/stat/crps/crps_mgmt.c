...
#include <include/soc/dnx/legacy/drv.h>
 ...
 */
 /** 
 * \brief
 * defines the default time interval in msec for the sequential timer
 */
#define CRPS_MGMT_SEQ_TIMER_INTERVAL_MSEC (20)
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, DNX_CRPS_DATABASE_RESOURCE, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    SHR_IF_ERR_EXIT(dnx_crps_db.database_res.alloc(unit, SW_STATE_ALGO_RES_NOF_CORES_GET(unit)));
    ...
    int unit)
{
    ...
    /** init bg thread and dma fifo */
    SHR_IF_ERR_EXIT(dnx_crps_eviction_init(unit));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    if (is_init == TRUE)
    {
        /** deinit bg thread and dma fifo */
        SHR_IF_ERR_CONT(dnx_crps_eviction_deinit(unit));
    ...
    bcm_stat_counter_interface_t * config)
{
    ...
    /** set default counter overflow mode */
    if (DNX_CRPS_MGMT_SOURCE_NO_EVICTION(unit, config->source))
    {
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    /** seq_timer_interval calculation. resault is in clocks resolution */
    time.time = CRPS_MGMT_SEQ_TIMER_INTERVAL_MSEC;
    time.time_units = DNXCMN_TIME_UNIT_MILISEC;
    SHR_IF_ERR_EXIT(dnxcmn_time_to_clock_cycles_get(unit, &time, &seq_timer_interval));
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.eviction.
                    sequential_timer_value.set(unit, engine->core_id, engine->engine_id, seq_timer_interval));
    ...
    int *valid)
{
    ...
    if ((conf_valid == TRUE) && (eviction_type == bcmStatEvictionDestinationLocalHost) &&
        (record_format == bcmStatEvictionRecordFormatPhysical) &&
        (DNX_CRPS_MGMT_SOURCE_NO_EVICTION(unit, engine_source) == FALSE))
    {
        *valid = TRUE;
    }
    else
    ...
    int *database_id)
{
    ...
    res_flags = (flags & BCM_STAT_DATABASE_CREATE_WITH_ID) ? SW_STATE_ALGO_RES_ALLOCATE_WITH_ID : 0;
