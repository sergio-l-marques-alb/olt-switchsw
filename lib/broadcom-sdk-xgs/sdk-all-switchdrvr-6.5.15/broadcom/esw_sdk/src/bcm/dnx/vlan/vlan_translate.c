    ...
    int vlan_translation_action_id)
{
     ...
     */
    nof_vlan_edit_action_ids = dnx_data_vlan.vlan_translate.nof_vlan_edit_action_ids_get(unit);
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(vlan_port_translation->gport))
    {
        ...
        /*
         * Verify the new_inner_vlan is INVALID since only new_outer_vlan can be set at EGRESS_DEFAULT_AC_PROF.
         */
        if (vlan_port_translation->new_inner_vlan != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Wrong setting. gport = 0x%08X is VIRTUAL EGERSS DEFUALT thus thus new_inner_vlan (=%d) can not be set!\n",
    ...
    if (!_SHR_IS_FLAG_SET(vlan_port_translation->flags, BCM_VLAN_ACTION_SET_INGRESS))
    {
        if (!BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(vlan_port_translation->gport)
            && !BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(vlan_port_translation->gport))
        {
            ...
            if (gport_hw_resources.outlif_dbal_table_id == DBAL_TABLE_EEDB_ARP)
            {
                SHR_IF_ERR_EXIT(dnx_lif_table_mngr_is_valid_field
                                (unit, _SHR_CORE_ALL, gport_hw_resources.local_out_lif, FALSE, DBAL_PHYSICAL_TABLE_NONE,
                                 DBAL_FIELD_VLAN_EDIT_VID_2, &is_2tag_valid));
            }
            /*
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Error! The specified gport does not support 2 tag vlan editing\n");
            }
        }
    }
    ...
    int *action_id)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_WITH_ID))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vlan_translation_action_id_verify(unit, *action_id));

        if (_SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS))
        {
            SHR_IF_ERR_EXIT(vlan_db.action_id_ingress.is_allocated(unit, *action_id, &is_allocated));
    ...
    bcm_vlan_translate_action_class_t * action_class)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_vlan_translation_action_id_verify(unit, action_class->vlan_translation_action_id));
 ...
 */
static shr_error_e
dnx_vlan_port_translation_set_egress_eedb(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    uint32 entry_handle_id;
    ...
    uint32 match_class_id)
{
    ...
    if (resultType == DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO)
    {
        if (vlan_translate_values_are_zero == FALSE)
        {
            ...
            dbal_entry_value_field32_set(unit, esem_handle_id, DBAL_FIELD_VLAN_EDIT_VID, INST_SINGLE,
                                         vlan_port_translation->new_outer_vlan);
    ...
    else if (resultType == DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO_VLANS)
    {
        ...
        if (vlan_translate_values_are_zero == FALSE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_translation_set_egress_data
                            (unit, esem_handle_id, vlan_port_translation, TRUE));
 ...
 * \brief
 * Internal function to configure EGRESS VLAN Translation
 * parameters per a virtual ESEM gport.
 * \see
 * bcm_dnx_vlan_port_translation_set
 */
static shr_error_e
dnx_vlan_port_translation_set_egress_esem(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    SHR_IF_ERR_EXIT(dnx_vlan_port_egress_virtual_ac_match_info_get
                    (unit, vlan_port_translation->gport, sw_table_handle_id));
    ...
    switch (entry_type)
    {
        ...
        case DBAL_RESULT_TYPE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW_ESEM_NAMESPACE_VSI:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_translation_set_egress_esem_namespace_vsi
                            (unit, esem_handle_id, vlan_port_translation, vsi, match_class_id));
        ...
        default:
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         ...
                         " resultType = %d is not supported.\n",
                         DBAL_TABLE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW, vlan_port_translation->gport, entry_type);
 ...
 * \brief
 * Internal function to configure EGRESS VLAN Translation
 * parameters per a virtual Egress Port Default gport
 * \see
 * bcm_dnx_vlan_port_translation_set
 */
static shr_error_e
dnx_vlan_port_translation_set_egress_port_default(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    uint32 entry_handle_id;
    ...
    /*
     * Decode the supplied virtual gport in order to receive the ac-profile
     */
    ac_profile = BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_GET(vlan_port_translation->gport);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_VID, INST_SINGLE,
                                 vlan_port_translation->new_outer_vlan);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_PROFILE, INST_SINGLE,
                                 vlan_port_translation->vlan_edit_class_id);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 * Translation parameters in the given ESEM entry:
 * - vlan_edit_class_id.
 * - new_outer_vlan.
 * - new_inner_vlan.
 * \see
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    /*
     * Not all entries support 2 tag vlan editing, get only if relevant
     */
    if (is_2tag_valid == TRUE)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_VID_2, INST_SINGLE,
                         &vlan_port_translation->new_inner_vlan));
 ...
 * \brief
 * Internal function to get the configuration of EGRESS VLAN
 * Translation parameters per gport (Out-LIF EEDB):
 * - vlan_edit_class_id.
 * - new_outer_vlan.
 * - new_inner_vlan.
 * \see
 ...
 */
static shr_error_e
dnx_vlan_port_translation_get_egress_eedb(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    uint32 entry_handle_id;
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, esem_handle_id, DBAL_FIELD_VLAN_EDIT_VID, INST_SINGLE,
                                                            &vlan_port_translation->new_outer_vlan));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
 ...
 * \brief
 * Internal function to get the configuration of EGRESS VLAN
 * Translation parameters per gport (Out-LIF, ESEM or Egress
 * Default):
 * - vlan_edit_class_id.
 * - new_outer_vlan.
 * - new_inner_vlan.
 ...
 */
static shr_error_e
dnx_vlan_port_translation_get_egress_esem(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    uint32 sw_table_handle_id;
    ...
    switch (entry_type)
    {
        case DBAL_RESULT_TYPE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW_ESEM_NATIVE_AC:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_translation_get_egress_data(unit, esem_handle_id, vlan_port_translation));
        ...
        case DBAL_RESULT_TYPE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW_ESEM_NAMESPACE_VSI:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vlan_port_egress_virtual_namespace_vsi_key_set
                            (unit, sw_table_handle_id, esem_handle_id));

            SHR_IF_ERR_EXIT(dnx_vlan_port_translation_get_egress_esem_namespace_vsi
                            (unit, esem_handle_id, vlan_port_translation));
 ...
 */
static shr_error_e
dnx_vlan_port_translation_get_egress_port_default(
    int unit,
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    uint32 entry_handle_id;
    ...
    /*
     * ESEM-Default has only one VID.
     */
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_VID, INST_SINGLE,
                                                        &vlan_port_translation->new_outer_vlan));
    vlan_port_translation->new_inner_vlan = 0;
    ...
    uint32 post_eve_vlan_membership)
{
    ...
    if ((dnx_data_qos.hw_bug.feature_get(unit, dnx_data_qos_hw_bug_fwd_eve_pcp_diff_encap)) &&
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    {
        if (BCM_GPORT_IS_VLAN_PORT(vlan_port_translation->gport)
            || BCM_GPORT_IS_EXTENDER_PORT(vlan_port_translation->gport))
        {
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(vlan_port_translation->gport))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_set_egress_esem(unit, vlan_port_translation));
            }
            else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(vlan_port_translation->gport))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_set_egress_port_default(unit, vlan_port_translation));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_set_egress_eedb(unit, vlan_port_translation));
            }
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Wrong GPort (0x%08x) type - vlan translation can only be set on a VLAN_PORT type GPort",
                         (vlan_port_translation->gport));
        }
    }
    ...
    bcm_vlan_port_translation_t * vlan_port_translation)
{
    ...
    else
    {
            ...
            || BCM_GPORT_IS_EXTENDER_PORT(vlan_port_translation->gport))
        {
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(vlan_port_translation->gport))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_get_egress_esem(unit, vlan_port_translation));
            }
            else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(vlan_port_translation->gport))
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_get_egress_port_default(unit, vlan_port_translation));
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_vlan_port_translation_get_egress_eedb(unit, vlan_port_translation));
            }
        }
        else
        {
    ...
    int *action_id)
{
    ...
    is_ingress = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_INGRESS) ? TRUE : FALSE;
    algo_flags = _SHR_IS_FLAG_SET(flags, BCM_VLAN_ACTION_SET_WITH_ID) ? SW_STATE_ALGO_RES_ALLOCATE_WITH_ID : 0;
    if (is_ingress)
    {
        /*
         * DNX SW Algorithm, allocate ACTION_ID_INGRESS.
         * sw_state_algo_res_allocate() will return an error if resource for specified ACTION_ID_INGRESS has already been allocated.
         */
    ...
    else
    {
         ...
         * sw_state_algo_res_allocate() will return an error if resource for specified ACTION_ID_EGRESS has already been allocated.
         */
    ...
    uint32 flags)
{
     ...
     */
    last_action_id = dnx_data_vlan.vlan_translate.nof_vlan_edit_action_ids_get(unit) - 1;
