    ...
    int unit)
{
    ...
    for (stif_instance_id = 0; stif_instance_id < dnx_data_stif.port.max_nof_instances_get(unit); stif_instance_id++)
    {
        ...
        if (dnx_data_stif.config.stif_idle_report_get(unit) == 0)
        {
            ...
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STIF_INSTANCE_OUTPUT_EXT_QSIZE_RPRT_PERIOD,
                                         INST_SINGLE, dnx_data_stif.report.qsize_idle_report_period_invalid_get(unit));
        }
        else
        {
    ...
    int unit)
{
    ...
    time.time = dnx_data_stif.config.stif_scrubber_rate_min_get(unit);
    ...
    time.time = dnx_data_stif.config.stif_scrubber_rate_max_get(unit);
    SHR_IF_ERR_EXIT(dnxcmn_time_to_clock_cycles_get(unit, &time, &nof_clock_cycles));
    ...
    uint32 init)
{
     ...
     *  init of output port - SW WA is to never inir output port
     *  0 (instance 0) */
    ...
    int unit)
{
    ...
    for (source_dbal = 0; source_dbal < DBAL_NOF_ENUM_STIF_QSIZE_SOURCE_VALUES; source_dbal++)
    {
        SHR_IF_ERR_EXIT(dnx_stif_mgmt_qsize_source_enable_set(unit, BCM_CORE_ALL, source_dbal, FALSE));
    }
    ...
    uint32 *is_connected)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_STIF_ACTIVE_SOURCE_PER_PORT, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_STIF_SOURCE, field_source);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_STIF_INSTANCE, instance_id);
    ...
    int fc_enable)
{
    uint32 entry_handle_id;
    uint32 flow_control_low, flow_control_high;
    int source_id, is_enq;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    /** in STIF_QSIZE_CONFIG table the source id keys does not
     *  include the scrubber source */
    for (source_id = 0; source_id < DBAL_ENUM_FVAL_STIF_QSIZE_SOURCE_ING_INT_SCRUB; source_id++)
    {
    ...
    int *shift)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /** size of bits that will be used in the record */
    *shift = msb_bit - lsb_bit + 1;
    /** now calculate whether this field should be shifted */
    /** count_lsb_zeroes holds how many 0 bits are there in the lsb */
    count_lsb_zeroes = lsb_bit;

    /** if (count_lsb_zeroes < 0) this means no 0 bits in the lsb -
     *  the field looks like that 0001111 for example meaning it
     *  will be shifted with the current shift place that was
     *  reached with the previous element */
    if (count_lsb_zeroes >= 0)
    {
        ...
        {
            /** means we need to add the zero difference to the size
             *  because it will be added to the report example: current
             *  shift place=2, field_mask = b'111100(00) - only 2 lsb 0s
             *  can be removed by the shift */
            *shift = *shift + (count_lsb_zeroes - *current_shift_place);
            *current_shift_place = 0;
        }
        else if ((current_shift_place - count_lsb_zeroes) == 0)
        {
            /** example: current shift place=4, field_mask = b'1111(0000) -
    ...
    dnx_stif_mgmt_report_format_element_with_shift_t * elements_array_get)
{
    ...
    /** obect_count has the number of objects and elements_count
     *  will  hold the count for all other elements */
    elements_count = 0;
    ...
    for (element = 0; element < DNX_STIF_INGRESS_MAX_RECORD_ELEMENTS; element++)
    {
        ...
        {
            /** get max mask */
            dbal_fields_predefine_value_get(unit, entry_handle_id, mask_field, FALSE, DBAL_PREDEF_VAL_MAX_VALUE,
                                            &mask_max);
    ...
    int *elements_size_array)
{
    ...
    dnx_stif_mgmt_report_format_element_with_shift_t elements_array_get[DNX_STIF_INGRESS_MAX_RECORD_ELEMENTS + 1];
    uint32 object_count, msb_bit, lsb_bit, mask_field_size;
    int element_id, record_size, elements_count = 0, object_id, elements_array_size = 0, current_shift_place;
    ...
    sal_memset(elements_array_get, 0,
               sizeof(dnx_stif_mgmt_report_format_element_with_shift_t) * (DNX_STIF_INGRESS_MAX_RECORD_ELEMENTS + 1));
    ...
    if (object_count != 0)
    {
        /** save objects in the elements array */
        for (object_id = 0; object_id < object_count; object_id++)
        {
            elements[elements_count].element_type = object_id;
            elements[elements_count].mask = BCM_STAT_FULL_MASK;
            elements_size_array[elements_count] = dnx_data_stif.report.stat_object_size_get(unit);
            elements_count++;
        }
    }
    /** get the shift values from the dbal and return the element
     *  count */
    SHR_IF_ERR_EXIT(dnx_stif_mgmt_ingress_format_elements_shift_values_get(unit, &elements_count, elements_array_get));
    ...
    /** include also the object count number to all elements
     *  count number */
    ...
    bcm_stat_stif_record_format_element_t * record_format_elements)
{
    ...
    for (element = 0; element < nof_elements; element++)
    {
            ...
            || record_format_elements[element].element_type == bcmStatStifRecordElementObj3)
        {
            ...
            else if (record_format_elements[element].mask == DNX_STIF_MGMT_EGRESS_STAT_OBJ_MASK_16_BITS)
            {
                shift = DNX_STIF_MGMT_EGRESS_STAT_OBJ_SHIFT_16_BITS;
            }
            else
            {
                shift = DNX_STIF_MGMT_EGRESS_STAT_OBJ_SHIFT_18_BITS;
            }
    ...
    int *elements_size_array)
{
    ...
    for (object_id = 0; object_id < object_count; object_id++)
    {
        ...
        else if (elements_size_array[object_id] == DNX_STIF_MGMT_EGRESS_STAT_OBJ_SHIFT_16_BITS)
        {
            elements[object_id].mask = DNX_STIF_MGMT_EGRESS_STAT_OBJ_MASK_16_BITS;
        }
        else
        {
            elements[object_id].mask = DNX_STIF_MGMT_EGRESS_STAT_OBJ_MASK_18_BITS;
        }
    }
