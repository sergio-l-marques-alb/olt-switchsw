 ...
 */
static shr_error_e
bcm_dnx_cosq_port_mapping_gport_verify(
    int unit,
    bcm_port_t port)
{
 ...
 */
static shr_error_e
bcm_dnx_cosq_mapping_set_verify(
    int unit,
    bcm_cos_t priority,
...
/*
 * brief - Verify function for bcm_dnx_cosq_port_mapping_set
 */
static shr_error_e
bcm_dnx_cosq_port_mapping_set_verify(
    int unit,
    bcm_port_t port,
    bcm_cos_t priority,
    bcm_cos_queue_t cosq)
{
     ...
     */
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_mapping_set_verify(unit, priority, cosq));

    /*
     * Verify Gport type
     */
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_port_mapping_gport_verify(unit, port));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_port_mapping_set_verify(unit, port, priority, cosq));
    ...
    if (BCM_COSQ_GPORT_IS_FMQ_ROOT(port))
    {
         ...
         * FMQ mapping
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_fmq_tc_to_voq_offset_map_set(unit, priority, cosq));
    }
    else
    {
        /*
         ...
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_mapping_set(unit, port, priority, cosq));
    }
 ...
 */
static shr_error_e
bcm_dnx_cosq_mapping_get_verify(
    int unit,
    bcm_cos_t priority,
    bcm_cos_queue_t * cosq)
{
...
/*
 * brief - verify function for bcm_dnx_cosq_port_mapping_get
 */
static shr_error_e
bcm_dnx_cosq_port_mapping_get_verify(
    int unit,
    bcm_port_t port,
    bcm_cos_t priority,
    bcm_cos_queue_t * cosq)
{
     ...
     * Verify TC and VOQ offset
     */
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_mapping_get_verify(unit, priority, cosq));
...
/*
 * Gep mapping of system TC (TC on the packet) to VOQ offset, for several queue types (FMQ, sysport, MC/UC)
 */
shr_error_e
    ...
    bcm_cos_queue_t * cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_port_mapping_get_verify(unit, port, priority, cosq));
    ...
    if (BCM_COSQ_GPORT_IS_FMQ_ROOT(port))
    {
         ...
         * FMQ mapping
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_fmq_tc_to_voq_offset_map_get(unit, priority, &voq_offset));
        *cosq = voq_offset;
    }
    else
    {
         ...
         * Queue group / sysport mapping
         */
        SHR_IF_ERR_EXIT(dnx_cosq_ipq_tc_mapping_get(unit, port, priority, cosq));
    }
    ...
    bcm_cos_queue_t voq_offset)
{
    ...
    if (is_flow)
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.profile_data_get(unit, _SHR_CORE_ALL, tc_profile, &ref_count, &tc_mapping));
        tc_mapping.tc[sys_tc] = voq_offset;
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_voq_flow_map.replace_data(unit, _SHR_CORE_ALL, tc_profile, &tc_mapping));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_sysport_map.profile_data_get(unit, _SHR_CORE_ALL, tc_profile, &ref_count, &tc_mapping));
        tc_mapping.tc[sys_tc] = voq_offset;
        SHR_IF_ERR_EXIT(dnx_ipq_alloc_mngr_db.
                        tc_sysport_map.replace_data(unit, _SHR_CORE_ALL, tc_profile, &tc_mapping));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_mapping_set_verify(unit, priority, cosq));
    ...
    bcm_cos_queue_t * cosq)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_mapping_get_verify(unit, priority, cosq));

    sys_tc = priority;
    is_flow = FALSE;
    tc_profile = dnx_data_ipq.tc_map.default_sysport_profile_idx_get(unit);

    SHR_IF_ERR_EXIT(dnx_cosq_ipq_dbal_tc_to_voq_offset_map_get(unit, sys_tc, tc_profile, is_flow, &voq_offset));
