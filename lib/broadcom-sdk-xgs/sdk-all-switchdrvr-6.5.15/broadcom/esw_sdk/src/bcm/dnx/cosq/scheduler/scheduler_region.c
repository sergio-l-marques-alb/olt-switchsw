    ...
    int unit)
{
    ...
    DBAL_FUNC_INIT_VARS(unit);
     ...
     /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SCH_REGION, &entry_handle_id));
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        for (region_index = dnx_data_sch.sch_alloc.type_con_reg_end_get(unit) + 1;
             region_index < dnx_data_sch.flow.nof_regions_get(unit); region_index++)
        {
            /*
             * translate region index (0-95 for Jer2) to 
             * index in last 32 regions (0-31 - Jer2)
             */
            k_idx = region_index - (dnx_data_sch.sch_alloc.type_con_reg_end_get(unit) + 1);
