    ...
    dnx_field_entry_t * entry_add_in_p)
{
    ...
    entry_add_in_p->core = SOC_CORE_ALL;
...
/**
* \brief
*  Call DBAL table and set the masked entry into HW
...
*/
static shr_error_e
dnx_field_entry_masked_hw_set(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_entry_t * entry_info_p,
    uint32 entry_handle_id)
{
    uint32 qual_idx = 0, action_idx = 0;
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, (DBAL_COMMIT | DBAL_COMMIT_OVERRIDE_DEFAULT)));
...
/**
* \brief
*  Call DBAL table and set the non-masked entry into HW
* \param [in] unit           - Device ID
* \param [in] field_stage    - Field Stage (iPMF1/2...)
* \param [in] entry_info_p   - Information that will be written into HW (QUALS & ACTIONS)
* \param [in] entry_handle_id- dbal entry handle id
...
*/
static shr_error_e
dnx_field_entry_not_masked_hw_set(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_entry_t * entry_info_p,
    uint32 entry_handle_id)
    ...
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_masked_hw_set(unit, field_stage, entry_info_p, entry_handle_id));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_not_masked_hw_set(unit, field_stage, entry_info_p, entry_handle_id));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
...
* \param [in] dbal_table_id  - DBAL table id that was created for Field group at fg_create()
* \param [in] entry_info_p   - Information that will be written into HW (QUALS & ACTIONS)
* \param [out] access_id_p   - access id received from the KBP hw
* \return
*   shr_error_e - Error Type
* \remark
*     The size of the qualifiers and actions array are terminated by DNX_FIELD_QUAL_TYPE_INVALID
...
/**
* \brief
* This procedure retrieves the content of a (masked ) TCAM entry from HW.
* \param [in] unit             -  Device ID
* \param [in] field_stage      -  Field Stage (iPMF1/2...)
* \param [in] fg_id            -  Identifier of the field group. Used for debugging.
* \param [in] entry_handle_id  -  DBAL entry id that we want to receive the information about
...
* \param [in] key - Key from which the qualifiers values will be extracted.
    ...
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_masked_hw_get(unit, field_stage, fg_id, entry_handle_id, entry_info_p));
    ...
    dnx_field_entry_key_t * entry_key_p)
{
    ...
    for (qual_idx = 0;
         entry_key_p->qual_info[qual_idx].dnx_qual != DNX_FIELD_QUAL_TYPE_INVALID &&
         qual_idx < DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG; qual_idx++)
    {
        SHR_IF_ERR_EXIT(dnx_field_map_dnx_to_dbal_qual
                        (unit, field_stage, entry_key_p->qual_info[qual_idx].dnx_qual, &field_id));
        dbal_entry_key_field_arr32_set(unit, entry_handle_id, field_id, entry_key_p->qual_info[qual_idx].qual_value);
    ...
    uint32 *entry_access_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_field_entry_tcam_access_id_allocate(unit, 0, &access_id));

    /*
     *   Encode internal TCAM entry id per Field Group ID and Access Id
     */
    *entry_access_id_p = DNX_FIELD_TCAM_ENTRY(fg_id, access_id, core);
    ...
    uint32 *entry_access_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_access_id_create(unit, core, DNX_DATA_MAX_FIELD_GROUP_NOF_FGS,
    ...
    uint32 *entry_access_id_p)
{
    ...
    if (fg_type == DNX_FIELD_GROUP_TYPE_TCAM)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_entry_tcam_access_id_create
                        (unit, core, fg_id, tcam_handler_id, priority, entry_access_id_p));
    }
    else
    {
...
dnx_field_ace_entry_key_allocate(
    int unit,
    dnx_field_ace_entry_add_flags_e flags,
    dnx_field_ace_id_t ace_id,
    dnx_field_ace_key_t * ace_key_p)
{
    ...
    if (flags & DNX_FIELD_ACE_ENTRY_ADD_FLAG_WITH_ID)
    {
...
/**
* \brief
*  Get the action types and qual types from TCAM FG sw state
* \param [in] unit             - Device ID
...
*/
static shr_error_e
dnx_field_entry_tcam_quals_actions_type_get(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_dnx_action_arr_get(unit, fg_id, dnx_actions, &nof_actions[0]));
    ...
    for (action_idx = 0; action_idx < nof_actions[0]; action_idx++)
    {
        
        entry_info_p->payload_info.action_info[action_idx].dnx_action = dnx_actions[0][action_idx];
    }
...
/**
* \brief
*  Get the action types and qual types from EXEM FG sw state
* \param [in] unit             - Device ID
* \param [in] fg_id            - Field Group ID
* \param [out] entry_info_p    - Holds the pointer to action and qualifier types
* \return
...
*/
static shr_error_e
dnx_field_entry_exem_quals_actions_type_get(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_entry_t * entry_info_p)
{
    ...
    int unit)
{
    ...
    for (address = 0; address <= dnx_data_field.state_table.address_max_get(unit); address++)
    {
        /*
         * Since the state table is a direct table, we don't need to check if an entry exists
         * before deleting it.
         */
...
* \param [in] entry_handle_p   - The entry handle ID to be loaded by the calling function.
* \return
*   shr_error_e - Error Type
* \remark
    ...
    uint32 *entry_handle_p)
{
    ...
    for (action_fg_ndx = 0;
         ...
         && actions_payload_info.actions_on_payload_info[0][action_fg_ndx].dnx_action != DNX_FIELD_ACTION_INVALID;
         action_fg_ndx++)
    {
        if (actions_payload_info.actions_on_payload_info[0][action_fg_ndx].dont_use_valid_bit == TRUE)
        {
            SHR_IF_ERR_EXIT(dnx_field_map_dnx_action_is_void
                            (unit, field_stage,
                             actions_payload_info.actions_on_payload_info[0][action_fg_ndx].dnx_action,
                             &action_is_void));
    ...
    uint32 *entry_handle_p)
{
    ...
    /** Verify the input.*/
    SHR_INVOKE_VERIFY_DNX(dnx_field_entry_tcam_verify(unit, fg_id, entry_info_p, entry_handle_p));

    *entry_handle_p = DNX_FIELD_ENTRY_ACCESS_ID_INVALID;

    if (entry_info_p->core != 0 && entry_info_p->core != 1 && entry_info_p->core != SOC_CORE_ALL)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Core ID %d is not valid. Legal values are 0, 1 or SOC_CORE_ALL \n",
                     entry_info_p->core);
    }

    if (entry_info_p->core == SOC_CORE_ALL)
    {
    ...
    /** Allocate entry access ID and find location for it (only for regular TCAM) */
    if (fg_type == DNX_FIELD_GROUP_TYPE_TCAM)
    {
        SHR_IF_ERR_EXIT(dnx_field_entry_pmf_access_id_create
                        (unit, entry_info_p->core, fg_type, fg_id, entry_info_p->priority, entry_handle_p));
    }
    else
    {
        uint32 key = 0;
        ...
        *entry_handle_p = DNX_FIELD_TCAM_ENTRY(fg_id, key, entry_info_p->core);

        /**
         * Inform TCAM manager about DT entry being add.
         * (This is needed for DT FGs with 'AUTO' bank allocation mode in order to allocate
         *  a bank on the first added entry)
         */
        SHR_IF_ERR_EXIT(dnx_field_group_tcam_handler_id_get(unit, fg_id, &tcam_handler_id));
        SHR_IF_ERR_EXIT(dnx_field_tcam_handler_dt_entry_add(unit, tcam_handler_id));
    }

    /**Set the HW with entry information*/
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_hw_set(unit, field_stage, dbal_table_id, *entry_handle_p, entry_info_p));
    ...
    uint32 *entry_handle_p)
{
    ...
    /**Set the HW with entry information*/
    SHR_IF_ERR_EXIT(dnx_field_entry_kbp_hw_set(unit, field_stage, dbal_table_id, entry_info_p, &access_id));
    /*
     * Only after hw_set we can get the  KBP entry (hw) access_id and encode the entry_id
     * It was allocated automatically by the KBP hw after the hw_set
     * */
    /*
     *   Encode external TCAM entry id per Field Group ID and Access Id
     */
    *entry_handle_p = DNX_FIELD_ENTRY(DNX_FIELD_TCAM_EXTERNAL, fg_id, access_id, 0);
    ...
    uint32 *data_p)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_STATE_TABLE_ADDRESS, address);

    /** Get the entry.*/
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));

    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_STATE_TABLE_DATA, INST_SINGLE, data_p);
    ...
    uint32 data)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_STATE_TABLE_DATA, INST_SINGLE, data);
...
/**
* \brief
*  Verifies that the given State Table address is in range
* 
* \param [in] unit    - Device ID
...
*/
static shr_error_e
dnx_field_entry_state_table_address_verify(
    int unit,
    uint32 address)
{
    SHR_FUNC_INIT_VARS(unit);

    if (address > dnx_data_field.state_table.address_max_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "State Table address out of range: %d\r\n", address);
    }
    ...
    uint32 *data_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_state_table_address_verify(unit, address));

    SHR_IF_ERR_EXIT(dnx_field_entry_state_table_hw_get(unit, address, data_p));
    ...
    uint32 address)
{
    ...
    /*
     * Delete is the same as setting the value to 0 in State Table
     */
    ...
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(rv, "EXEM entries must include values for all qualifiers in the field group they "
                             "belong to, since Exact Match doesn't use amask like TCAM.\r\n%s%s%s",
                             EMPTY, EMPTY, EMPTY);
             ...
             action_fg_ndx++)
        {
            if (actions_payload_info.actions_on_payload_info[0][action_fg_ndx].dont_use_valid_bit == TRUE)
            {
                ...
                if (action_is_void == FALSE)
                {
                    ...
                    for (action_entry_ndx = 0;
                         action_entry_ndx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP
                         && entry_info_p->payload_info.action_info[action_entry_ndx].dnx_action !=
    ...
    dnx_field_entry_t * entry_info_p)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_field_entry_exem_add_verify(unit, fg_id, entry_info_p));
    ...
    /**Set the HW with entry information*/
    SHR_IF_ERR_EXIT(dnx_field_entry_exem_hw_set(unit, field_stage, dbal_table_id, entry_info_p));
    ...
    dnx_field_ace_key_t * ace_entry_key_p)
{
    ...
    if ((DNX_FIELD_ACE_FORMAT_ID_IS_ACE(unit, ace_id) == FALSE) &&
        ((flags & DNX_FIELD_ACE_ENTRY_ADD_FLAG_UPDATE) == 0) && ((flags & DNX_FIELD_ACE_ENTRY_ADD_FLAG_WITH_ID) == 0))
    {
    ...
    if (flags & DNX_FIELD_ACE_ENTRY_ADD_FLAG_UPDATE)
    {
        /*
...
{
    dbal_tables_e dbal_table_id = DBAL_TABLE_EMPTY;
    dnx_field_group_t fg_id = DNX_FIELD_ENTRY_FG_ID(entry_handle);
    dnx_field_stage_e field_stage = DNX_FIELD_STAGE_NOF;
    ...
    dnx_field_entry_t * entry_info_p)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_quals_actions_type_get(unit, fg_id, entry_info_p));
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_hw_get(unit, field_stage, fg_id, dbal_table_id, entry_handle, entry_info_p));

    /*
     * Convert internal DNX_CORE_ALL to output BCM SOC_CORE_ALL 
     */
    if (entry_info_p->core == DNX_FIELD_TCAM_CORE_ALL)
    {
        entry_info_p->core = SOC_CORE_ALL;
    }
    ...
    dnx_field_entry_t * entry_info_p)
{
     ...
     * Get Actions/Quals values for the given entry
     */
    SHR_IF_ERR_EXIT(dnx_field_entry_exem_quals_actions_type_get(unit, fg_id, entry_info_p));
    SHR_IF_ERR_EXIT(dnx_field_entry_exem_hw_get(unit, field_stage, fg_id, dbal_table_id, entry_key_p, entry_info_p));
...
/**See H in field_entry.h*/
shr_error_e
dnx_field_entry_tcam_hit_per_core_get(
    int unit,
    ...
    uint8 *entry_hit_p)
{
    ...
    {
        dbal_table_id = DBAL_TABLE_FIELD_TCAM_TCAM_ACTION_HIT_INDICATION;
    ...
    {
        dbal_table_id = DBAL_TABLE_FIELD_TCAM_TCAM_ACTION_HIT_INDICATION_SMALL;
    ...
    uint8 *entry_hit_core_bmp_p)
{
    ...
    fg_id = DNX_FIELD_ENTRY_FG_ID(entry_handle);
    ...
    *entry_hit_core_bmp_p = 0;
    ...
    /**
     * Check if the given entry is relevant for both cores. In case no
     * it means that it is relevant only for one specific core,
     * which was retrieved from the given entry.
     */
    if (core_id == DNX_FIELD_TCAM_CORE_ALL)
    {
        ...
        /** Iterate over nof_cores and check whether we have a hit for the given entry. */
        for (core_id = 0; core_id < nof_cores; core_id++)
        {
            /** Get the hit indication for the given core. */
            SHR_IF_ERR_EXIT(dnx_field_entry_tcam_hit_per_core_get(unit, core_id, entry_handle, &hit_indication_value));

            /** Check whether we have a hit. */
            if (hit_indication_value)
            {
                /** In case we have a HIT for the given entry, set the relevant bit in the bitmap. */
                SHR_BITSET(entry_hit_core_bmp_p, core_id);
            }
        }
    }
    else
    ...
    uint32 entry_abs_location)
{
    ...
    if (entry_abs_location != DNX_FIELD_ENTRY_ACCESS_ID_INVALID)
    {
        /** Get the bank_id from entry absolute location. */
        bank_id = DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_ID(entry_abs_location);
        ...
        for (array_index = array_index_min; array_index <= array_index_max; array_index++)
        {
            ...
            for (index = index_min; index <= index_max; index++)
            {
                ...
                for (blk_index = blk_index_min; blk_index <= blk_index_max; blk_index++)
                {
                    ...
                    SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_WR_MASKr(unit, SOC_CORE_ALL, tcam_indirect_mask));
    ...
    uint32 entry_handle)
{
    ...
    /** Set entry absolute location to be invalid. */
    entry_abs_location = DNX_FIELD_ENTRY_ACCESS_ID_INVALID;

    if (flags & BCM_FIELD_ENTRY_HIT_FLUSH_ALL)
    {
        /** Clear hit info for all entries. */
        SHR_IF_ERR_EXIT(dnx_field_entry_tcam_hit_per_location_flush(unit, entry_abs_location));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
