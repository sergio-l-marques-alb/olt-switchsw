    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_LINK_PROFILES, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF1_CONTEXT_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF2_CONTEXT_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_IPMF3_CONTEXT_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_EPMF_CONTEXT_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy(data.name, DNX_ALGO_FIELD_KBP_USER_APPTYPE, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    int *context_alloc_id_p)
{
    ...
    if (alloc_flags & SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID)
    {
        ...
        if (stage == DNX_FIELD_STAGE_IPMF1)
        {
            ...
            SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf1_context_id.allocate_single
                            (unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, context_alloc_id_p));
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            ipmf2_context_id.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL,
                                                             context_alloc_id_p));
        }
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_field_info_sw.
                            ipmf1_context_id.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL,
                                                             context_alloc_id_p));
        }
        else if (stage != DNX_FIELD_STAGE_IPMF2)
        {
    ...
    dnx_field_context_t * context_id_p)
{
    ...
    if (alloc_flags & SW_STATE_ALGO_RES_ALLOCATE_WITH_ID)
    {
    ...
    uint8 *is_allocated_p)
{
    ...
    switch (stage)
    {
        ...
        case DNX_FIELD_STAGE_EXTERNAL:
            /*
             * No Verification needed. Contexts are automatically created (by PEMLA). 
             */
            /*
             * In the future (phase2) we will enable context creation from ACL. Now it is created for us by pemla
             * TBD
             */
            *is_allocated_p = TRUE;
            break;
