...
#include <soc/dnx/swstate/auto_generated/access/ppmc_access.h>
    ...
    int unit)
{
    ...
    sal_strncpy(create_data.name, DNX_ALGO_PPMC, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
 ...
 */
shr_error_e
dnx_algo_ppmc_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    /** Add multicast algo deinit actions*/
    SHR_FUNC_EXIT;
}
    ...
    int *mc_replication_id)
{
    ...
    alloc_flags =
        (_SHR_IS_FLAG_SET(flags, BCM_MULTICAST_ENCAP_EXTENSION_WITH_ID)) ? SW_STATE_ALGO_RES_ALLOCATE_WITH_ID : 0;
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT(ppmc_db.ppmc_res_manager.clear_all(unit));
exit:
    SHR_FUNC_EXIT;
}
