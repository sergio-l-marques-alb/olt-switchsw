    ...
    uint8 is_ingress)
{
    ...
    if (oam_lif != DNX_OAM_INVALID_LIF && mp_profile != BCM_OAM_PROFILE_INVALID)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_oam_sw_state_oam_lif_info_set(unit, DBAL_COMMIT_FORCE, oam_lif, is_ingress,
                                                          &oam_lif_info));
    }
    ...
    bcm_oam_profile_t * profile_index)
{
     ...
     */
    legal_flags |=
        BCM_OAM_PROFILE_WITH_ID | BCM_OAM_PROFILE_LM_TYPE_DUAL_ENDED | BCM_OAM_PROFILE_LM_TYPE_SLM |
        BCM_OAM_PROFILE_DM_TYPE_NTP | BCM_OAM_PROFILE_LM_TYPE_NONE;
     ...
     */
    if ((flags & BCM_OAM_PROFILE_LM_TYPE_DUAL_ENDED) && (flags & BCM_OAM_PROFILE_LM_TYPE_SLM))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error: unsupported flag combination of Dual ended LM and SLM\n");
    }
    if (((flags & BCM_OAM_PROFILE_LM_TYPE_DUAL_ENDED) || (flags & BCM_OAM_PROFILE_LM_TYPE_SLM))
    ...
    if ((flags & BCM_OAM_PROFILE_LM_TYPE_SLM) &&
        (oam_profile_type == bcmOAMProfileIngressAcceleratedEndpoint ||
         oam_profile_type == bcmOAMProfileEgressAcceleratedEndpoint))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error: BCM_OAM_PROFILE_LM_TYPE_SLM flag can be set only to non-accelerated profiles.\n");
    }
    ...
    bcm_oam_profile_t * profile_id)
{
    ...
    else if (flags & BCM_OAM_PROFILE_LM_TYPE_SLM)
    {
        if (!is_acc)
        {
            /*
             * SLM case: Disable counting of data packets and store in oam_sw_db.
             */
            SHR_IF_ERR_EXIT(dnx_oam_enable_data_counting(unit, profile_index, is_ingress, DNX_OAM_DISABLE_COUNTING));
        }

        SHR_IF_ERR_EXIT(dnx_oam_sw_db_lm_mode_set
                        (unit, profile_index, is_ingress, is_acc, DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_SLM));
    }
    ...
    bcm_oam_profile_t * enc_profile_id)
{
    ...
    *enc_profile_id = DNX_OAM_PROFILE_SET(profile_id, oam_profile_type);
    ...
    SHR_IF_ERR_EXIT(dnx_oam_sw_db_lm_mode_get(unit, profile_index, is_ingress, is_acc, &lm_type));

    *flags = 0;
    if (lm_type == DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_SLM)
    {
        *flags |= BCM_OAM_PROFILE_LM_TYPE_SLM;
    }
    else if (lm_type == DBAL_ENUM_FVAL_LM_MODE_LM_MODE_IS_DUAL_ENDED)
    {
    ...
    SHR_IF_ERR_EXIT(dnx_oam_sw_db_dm_mode_get(unit, profile_index, is_ingress, is_acc, &dm_type));
    ...
    bcm_oam_profile_t egress_oam_action_profile)
{
    ...
    /** Validate profile */
    if (ingress_oam_action_profile == BCM_OAM_PROFILE_INVALID && egress_oam_action_profile == BCM_OAM_PROFILE_INVALID)
    {
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_oam_lif_profile_set_verify(unit, flags, gport_hw_resources.global_in_lif,
                                                         gport_hw_resources.local_out_lif, ingress_oam_action_profile,
                                                         egress_oam_action_profile));

    /** Write profile to SW_STATE ing/eg */
    is_ingress = 1;
    SHR_IF_ERR_EXIT(dnx_oam_lif_profile_sw_update(unit, gport_hw_resources.global_in_lif,
                                                  DNX_OAM_PROFILE_IDX_GET(ingress_oam_action_profile), is_ingress));
    is_ingress = 0;
    SHR_IF_ERR_EXIT(dnx_oam_lif_profile_sw_update(unit, gport_hw_resources.local_out_lif,
                                                  DNX_OAM_PROFILE_IDX_GET(egress_oam_action_profile), is_ingress));
    ...
    for (prefix = PREFIX_HIGH; prefix < lif_analysis.lif_db.occupied_prefixes[DNX_OAM_CLASSIFIER_INGRESS]; prefix++)
    {
    ...
    for (prefix = PREFIX_HIGH; prefix < lif_analysis.lif_db.occupied_prefixes[DNX_OAM_CLASSIFIER_EGRESS]; prefix++)
    {
        oam_lif_db_key.oam_key_prefix = prefix;
    ...
    oam_action_content_t * action_content)
{
    ...
    else if (internal_opcode == DBAL_ENUM_FVAL_OAM_INTERNAL_OPCODE_ETH_LMM ||
    ...
    else if (internal_opcode == DBAL_ENUM_FVAL_OAM_INTERNAL_OPCODE_ETH_CCM)
    {
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    else if (oam_action_key->endpoint_type != bcmOAMMatchTypeMEP && oam_action_result->counter_increment != 0)
    {
    ...
    bcm_oam_action_result_t * oam_action_result)
{
    ...
    for (mp_type = 0; mp_type < DBAL_NOF_ENUM_MP_TYPE_VALUES; mp_type++)
    {
        if (DNX_OAM_ENDPOINT_TYPE_PER_MP_TYPE(mp_type) == oam_action_key->endpoint_type)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_oam_profile_counter_disable_calc(unit, profile_type, mp_type,
            ...
            if ((DNX_OAM_ENDPOINT_TYPE_PER_MP_TYPE(mp_type) == bcmOAMMatchTypeMEP) && (action_key.is_inject == 0))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_oam_profile_sub_type_calc
                                (unit, is_ingress, profile_id, action_key.oam_internal_opcode,
                                 oam_action_result->destination, &action_content));
