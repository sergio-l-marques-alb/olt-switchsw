...
/**
 * \brief
 * handler_entry_add_verify(unit, handler_id) - Verify function for
 * handler_entry_add, verifies that the handler is valid.
    ...
    uint32 *handler_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_save_params(unit, *handler_id, db));
    ...
    dnx_field_tcam_entry_t * entry)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_field_tcam_handler_entry_location_alloc_verify(unit, handler_id, entry));
    ...
    {
        LOG_DEBUG_EX(BSL_LOG_MODULE,
                     "TCAM Manager ignores DT entries, entry id: %d handler_id: %d%s%s\r\n",
                     entry->id, handler_id, EMPTY, EMPTY);
    ...
    for (core_iter = DNX_FIELD_TCAM_CORE_FIRST; core_iter < DNX_FIELD_TCAM_CORE_COUNT; core_iter++)
    {
        if (core == DNX_FIELD_TCAM_CORE_ALL || core == core_iter)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_tcam_location_alloc(unit, core_iter, handler_id, entry, &location));
            /*
    ...
    uint32 entry_id)
{
    ...
    if (fg_params.direct_table)
    {
        LOG_DEBUG_EX(BSL_LOG_MODULE,
                     "TCAM Manager ignores DT entries, entry id: %d handler_id: %d%s%s\r\n",
                     entry_id, handler_id, EMPTY, EMPTY);
    ...
    for (core_iter = DNX_FIELD_TCAM_CORE_FIRST; core_iter < DNX_FIELD_TCAM_CORE_COUNT; core_iter++)
    {
        if (core == DNX_FIELD_TCAM_CORE_ALL || core == core_iter)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_tcam_location_free(unit, core_iter, handler_id, entry_id));
            /*
...
shr_error_e
dnx_field_tcam_handler_dt_entry_add(
    int unit,
    uint32 handler_id)
{
    int bank_id;
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.fg_params.direct_table.get(unit, handler_id, &is_direct));

    if (!is_direct)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "This function can only be invoked for DT TCAM handlers\r\n");
    }
