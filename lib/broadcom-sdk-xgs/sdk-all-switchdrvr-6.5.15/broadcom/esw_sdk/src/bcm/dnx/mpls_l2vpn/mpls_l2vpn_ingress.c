    ...
    dnx_mpls_term_match_t * match)
{
    ...
    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_MPLS_LABEL, INST_SINGLE, &match->label);
    dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_SERVICE_TAGGED, INST_SINGLE,
                                        &match->service_tagged);
    ...
    dnx_mpls_termination_profile_t * term_profile_info)
{
    ...
    term_profile_info->check_bos = 0;
    term_profile_info->ttl_exp_label_index = TTL_EXP_CURRENT_MPLS_LABEL;
    if (_SHR_IS_FLAG_SET(l2vpn->flags, DNX_MPLS_L2VPN_FLAG_INGRESS_IML_WITH_ESI))
    {
        term_profile_info->labels_to_terminate = TWO_MPLS_LABELS_TO_TERMINATE;
    }
    else
    {
        bcm_mpls_port_t *info = l2vpn->mpls_port;
        if (info->vccv_type == bcmMplsPortControlChannelTtl)
        {
            term_profile_info->reject_ttl_0 = 1;
            term_profile_info->reject_ttl_1 = 1;
        }
        term_profile_info->has_cw = _SHR_IS_FLAG_SET(info->flags, BCM_MPLS_PORT_CONTROL_WORD) ? 1 : 0;
        if (_SHR_IS_FLAG_SET(info->flags, BCM_MPLS_PORT_ENTROPY_ENABLE))
        {
            term_profile_info->labels_to_terminate = TWO_MPLS_LABELS_TO_TERMINATE;
        }
        else
        ...
        if (info->criteria == BCM_MPLS_PORT_MATCH_LABEL_CONTEXT_LABEL)
        {
            /*
    ...
    dnx_mpls_l2vpn_info_t * l2vpn)
{
    ...
    if (labels_to_terminate_val == TWO_MPLS_LABELS_TO_TERMINATE)
    {
        ...
        else if (l2vpn->type == MPLS_L2VPN_TYPE_EVPN)
        {
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    else if (mpls_port->failover_mc_group)
    {
        ...
        mpls_port->failover_mc_group = gport_forward_port;
    }
    /*
     * Jericho mode beginning
     * {
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY)
    {
        ...
        {
            mpls_port->failover_mc_group = destination;
        }
    }
    /*
     * Jericho mode beginning
    ...
    else if (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_EEI)
    {
        ...
        {
            uint8 tmp;
            SHR_IF_ERR_EXIT(dnx_mpls_eei_enc_pointer_information_retrieve
                            (unit, forward_info.eei, &mpls_port->encap_id, &tmp));
    ...
    uint32 entry_handle_id)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PROTECTION_PATH, INST_SINGLE, info->protection_path);
    ...
    dnx_mpls_l2vpn_in_lif_info_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_inlif_info(unit, info->local_in_lif, entry_handle_id, &lif_info));
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    {
        mpls_port->flags2 |= BCM_MPLS_PORT2_INTERFACE_NAMESPACE;
    }
    ...
    dbal_tables_e inlif_dbal_table_id)
{
    ...
    /*
     * PWE tagged mode. Set InLIF profile with the right no. of service tags.
     */
    in_lif_profile_info_t_init(unit, &in_lif_profile_info);
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_INTERFACE_NAMESPACE))
    {
        in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags = DNX_IN_LIF_PROFILE_CS_PROFILE_PWE_LIF_SCOPED;
    }
...
{
    int gport_id, protection_pointer = 0;
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_ingress_lif_allocation_or_fill(unit, l2vpn_info, &pwe_lif_info));
    ...
    {
        DNX_FAILOVER_ID_GET(protection_pointer, l2vpn_info->mpls_port->ingress_failover_id);
        /** Encode IN-LIF protection pointer format from Failover ID */
        DNX_FAILOVER_ID_PROTECTION_POINTER_ENCODE(pwe_lif_info.protection_pointer, protection_pointer);
    ...
    if (l2vpn_info->type != MPLS_L2VPN_TYPE_EVPN)
    {
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_add(unit,
                _SHR_IS_FLAG_SET(l2vpn_info->mpls_port->flags, BCM_MPLS_PORT_REPLACE),
                l2vpn_info->mpls_port->mpls_port_id, &fwd_info));
    }

    /*
     * For the purposes of inlif profile, there is no current difference between
     * PWE and EVPN. Using IN_LIF_FORMAT_PWE as representative.
     */
    SHR_IF_ERR_EXIT(dnx_mpls_port_ingress_lif_profile_fill_and_allocation
                    (unit, l2vpn_info->mpls_port, orig_pwe_lif_info.in_lif_profile, &pwe_lif_info.in_lif_profile,
    ...
    SHR_IF_ERR_EXIT(dnx_mpls_l2vpn_in_lif_to_lif_table_manager(unit, &pwe_lif_info, entry_handle_id));
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_allocate_local_inlif_and_set_hw
                    (unit, entry_handle_id, &pwe_lif_info.local_in_lif, &lif_info));
    ...
    if (!is_replace)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_mpls_termination_lookup(unit, TERM_LOOKUP_SET, &term_match, &pwe_lif_info.local_in_lif));
    }
    ...
    dnx_mpls_l2vpn_info_t * l2vpn_info)
{
    ...
    BCM_ENCAP_ID_SET(l2vpn_info->mpls_port->encap_id, gport_hw_resources.global_in_lif);
    ...
    DNX_FAILOVER_ID_PROTECTION_POINTER_DECODE(l2vpn_info->mpls_port->ingress_failover_id, lif_info.protection_pointer);
    ...
    SHR_IF_ERR_EXIT(algo_mpls_db.
                    termination_profile.profile_data_get(unit, _SHR_CORE_ALL, lif_info.term_profile, &ref_count,
                                                         &term_profile_info));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_mpls_inlif_profile_to_mpls_port_info(unit, lif_info.in_lif_profile, l2vpn_info->mpls_port));
    ...
    bcm_gport_t mpls_l2vpn_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_mpls_db.termination_profile.free_single(unit, _SHR_CORE_ALL, lif_info.term_profile, &is_last));
