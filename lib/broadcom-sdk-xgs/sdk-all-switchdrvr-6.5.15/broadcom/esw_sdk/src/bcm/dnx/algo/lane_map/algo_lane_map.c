    ...
    int lane_id)
{
    ...
    dnx_algo_lane_map_direction_e direction;
     ...
     */
    if (DNX_ALGO_LANE_MAP_IS_NIF_SIDE(unit, lane_map_type) || is_over_fabric)
    {
        resolution = dnx_data_lane_map.nif.resolution_get(unit);
    }
    else
    {
     ...
     */
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit,port_type,1,1) && !is_over_fabric && (lane2serdes.rx_id != lane2serdes.tx_id))
    {
        /*
         * Get the current port type.
         */
        curr_port_is_ilkn = DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_type, 1);
        for (direction = DNX_ALGO_LANE_MAP_DIRECTION_RX; direction < DNX_ALGO_LANE_MAP_NOF_DIRECTIONS; direction++)
        {
            /*
             * Both serdes rx and tx should be checked.
             */
            serdes_id_to_check = (direction == DNX_ALGO_LANE_MAP_DIRECTION_RX) ? lane2serdes.rx_id : lane2serdes.tx_id;
            /*
             ...
             */
            lane_id_to_check = (direction == DNX_ALGO_LANE_MAP_DIRECTION_RX) ? serdes2lane.tx_id : serdes2lane.rx_id;
    ...
    soc_dnxc_lane_map_db_map_t * lane2serdes)
{
    ...
    for (lane = 0; lane < map_size; ++lane)
    {
        ...
        /** need to make sure that the mapping of active ports remains unchanged. */
        if (DNX_ALGO_LANE_MAP_IS_NIF_SIDE(unit, type))
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_nif_phy_active_get(unit, lane, &is_active));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_port_fabric_phy_active_get(unit, lane, &is_active));
        }
        if (is_active)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Lane map for active lane cannot be changed. Lane %d is active\n", lane);
            }
        }
    }
