    ...
    dnx_sat_init_t * sat_init)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CMIC_TOD_MODE, INST_SINGLE, sat_init->cmic_tod_mode);
    ...
    int unit)
{
    ...
    dnx_sat_init.cmic_tod_mode = DNX_SAT_CMIC_TOD_MODE;
    ...
    bcm_sat_config_t * conf)
{
    ...
    if (dnx_data_sat.tx_flow_param.feature_get(unit, dnx_data_sat_tx_flow_param_seq_number_wrap_around))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SEQ_NUM_WRAP_AROUND, INST_SINGLE, conf->seq_number_wrap_around);
        /** Preforming the action */
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    /*
     * (2) Timer config table
     */
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_SAT_TIMER_CONFIG, &entry_handle_id));

    /** Setting value fields */
    field = (conf->timestamp_format == bcmSATTimestampFormatNTP ? 2 : 1);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CMIC_TOD_MODE, INST_SINGLE, field);
    ...
    if (soc_property_get(unit, spn_SAT_ENABLE, 0))
    {
        ...
        for (type = 0; type < bcmSATEventCount; type++)
        {
            if (type == event_type)
            {
                ...
                /*
                 * dnx_sat_event_cb[unit][type] = cb; dnx_sat_event_ud[unit][type] = user_data;
                 */
            }
        }
    }
    ...
    if (soc_property_get(unit, spn_SAT_ENABLE, 0))
    {
        ...
        for (type = 0; type < bcmSATEventCount; type++)
        {
            if (type == event_type)
            {
                ...
                /*
                 * dnx_sat_event_cb[unit][type] = NULL; dnx_sat_event_ud[unit][type] = NULL;
                 */
            }
        }
    }
