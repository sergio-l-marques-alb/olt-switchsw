    ...
    int *bank_id)
{
    ...
    for (enum_index = 0; enum_index < field_info->nof_enum_values; enum_index++)
    {
        if (sal_strcasecmp(fec_resource_type_name, &field_info->enum_val_info[enum_index].name[str_len]) == 0)
        {
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate(unit, fec_index, flags, hierarchy, fec_resource_type, bank_id));
