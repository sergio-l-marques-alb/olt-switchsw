    ...
    int *trap_id_p)
{
    ...
    valid = dnx_rx_trap_type_map[type].valid;
    ...
    if (dnx_rx_trap_type_map[type].trap_block != DNX_RX_TRAP_BLOCK_INGRESS)
    {
        /** Encoding only done for none Ingress, since we return SW id*/
        *trap_id_p = DNX_RX_TRAP_ID_SET(dnx_rx_trap_type_map[type].trap_block,
                                        dnx_rx_trap_type_map[type].trap_class, dnx_rx_trap_type_map[type].trap_id);

    }
    else
    {
        /**Ingress block MUST be zero since we return HW trap_id, same as not encoding,
                * To differentiate between User and predfined the actual id is used
                */
        *trap_id_p = dnx_rx_trap_type_map[type].trap_id;
    }
    ...
    int *nof_traps_p)
{
    ...
    if ((trap_map_list = sal_alloc(bcmRxTrapCount * sizeof(dnx_rx_trap_map_type_t), "trap_map_list")) == NULL)
    {
    ...
    {
        if (dnx_rx_trap_type_map[i_trap].valid == TRUE)
        {
            if ((trap_block == dnx_rx_trap_type_map[i_trap].trap_block) &&
                (trap_class == dnx_rx_trap_type_map[i_trap].trap_class))
            {
                trap_map_list[nof_traps] = dnx_rx_trap_type_map[i_trap];
                nof_traps++;
            }
        }
    }

    *trap_map_list_p = trap_map_list;
    *nof_traps_p++ = nof_traps;

exit:
    if (SHR_FUNC_ERR() && (trap_map_list != NULL))
    {
    ...
    int *nof_traps_p)
{
    ...
    for (i_trap = 0; i_trap < bcmRxTrapCount; i_trap++)
    {
        if (dnx_rx_app_trap_type_map[i_trap].valid == TRUE && trap_block == dnx_rx_app_trap_type_map[i_trap].trap_block)
        {
            trap_map_list[nof_traps] = dnx_rx_app_trap_type_map[i_trap];
            nof_traps++;
        }
    }
    ...
    bcm_rx_trap_t * trap_type_p)
{
    ...
    for (i_trap = 0; i_trap < bcmRxTrapCount; i_trap++)
    {
        if (dnx_rx_app_trap_type_map[i_trap].valid == TRUE &&
            dnx_rx_app_trap_type_map[i_trap].trap_block == trap_block &&
            dnx_rx_app_trap_type_map[i_trap].trap_id == trap_id)
        {
            *trap_type_p = i_trap;
            break;
        }
    ...
    char *trap_name)
{
    ...
    if (dnx_rx_trap_type_map[trap_type].valid)
    {
        sal_strncpy(trap_name, dnx_rx_trap_type_map[trap_type].trap_name, DBAL_MAX_STRING_LENGTH);
    }
    else if (dnx_rx_app_trap_type_map[trap_type].valid)
    {
        sal_strncpy(trap_name, dnx_rx_app_trap_type_map[trap_type].trap_name, DBAL_MAX_STRING_LENGTH);
    }
    else
    {
    ...
    int *sw_trap_id_p)
{
    ...
    if (dnx_rx_trap_map_ingress_ud[hw_trap_id].valid)
    {
        *sw_trap_id_p = dnx_rx_trap_map_ingress_ud[hw_trap_id].alloc_index;
    }
    else
    {
    ...
    int *trap_id_p)
{
    ...
    {
        if (dnx_rx_trap_map_ingress_ud[ind].valid == TRUE)
        {
            if (*trap_id_p == dnx_rx_trap_map_ingress_ud[ind].alloc_index)
            {
                *trap_id_p = ind;
                break;
            }
    ...
    int trap_id)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Invalid trap type = %d \n", type);
    }
    ...
    if (is_oam_punt_or_protection == FALSE)
    {
        ...
        if (dnx_rx_trap_type_map[type].trap_class != DNX_RX_TRAP_CLASS_USER_DEFINED)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Trap bcmRxTrap%s was already created\n",
                             dnx_rx_trap_type_map[type].trap_name);
            }
        }
        ...
        if (flags & BCM_RX_TRAP_WITH_ID)
        {
            DNX_RX_TRAP_IS_LEGAL_INGRESS_TRAP_ID(trap_id);
            /** In case of user-defined WITH_ID, block is not encoded, retrieve it from type */
            trap_block = dnx_rx_trap_type_map[type].trap_block;
                    ...
                    && (!DNX_RX_TRAP_IS_ETPP_USER_DEFINED(DNX_RX_TRAP_ID_TYPE_GET(trap_id)))))
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG,
                             ...
                             dnx_rx_ud_trap_map[trap_block].block_name);
            }

            SHR_IF_ERR_EXIT(dnx_rx_trap_is_trap_id_allocated(unit, trap_id, &is_allocated));
            if (is_allocated == TRUE)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "Trap bcmRxTrapUserDefine id=%d was already created\n", trap_id);
            }
    ...
    int *trap_id_p)
{
    ...
    {
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            SHR_IF_ERR_EXIT(dnx_algo_rx_trap_id_etpp_allocate(unit, flags, trap_id_p));
    ...
    int *trap_id_p)
{
    ...
    if (is_punt_or_protection == FALSE)
    {
    ...
    switch (trap_block)
    {
        ...
        case DNX_RX_TRAP_BLOCK_ERPP:
            ...
            *trap_id_p = DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ERPP, DNX_RX_TRAP_CLASS_USER_DEFINED, *trap_id_p);
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_create(unit, flags, trap_class, trap_id_p));
            break;
        case DNX_RX_TRAP_BLOCK_OAMP:
            SHR_IF_ERR_EXIT(dnx_oam_error_trap_type_set(unit, type, *trap_id_p));
            *trap_id_p = DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_OAMP, DNX_RX_TRAP_CLASS_OAM, *trap_id_p);
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal trap_block=%d\n", trap_block);
            break;
    ...
    bcm_rx_trap_t * bcm_trap_type_p)
{
    ...
    switch (trap_class)
    {
        ...
        case DNX_RX_TRAP_CLASS_OAM:
            ...
            {
                if (dnx_rx_trap_type_map[i_trap].valid == TRUE &&
                    dnx_rx_trap_type_map[i_trap].trap_block == DNX_RX_TRAP_BLOCK_INGRESS)
                {
                    if (trap_type == dnx_rx_trap_type_map[i_trap].trap_id)
                    {
    ...
    bcm_rx_trap_t * bcm_trap_type_p)
{
    ...
    switch (trap_class)
    {
        ...
        case DNX_RX_TRAP_CLASS_OAM:
            ...
            {
                if (dnx_rx_trap_type_map[i_trap].valid == TRUE &&
                    dnx_rx_trap_type_map[i_trap].trap_block == DNX_RX_TRAP_BLOCK_ETPP &&
                    dnx_rx_trap_type_map[i_trap].trap_class == DNX_RX_TRAP_CLASS_OAM)
                {
                    if (trap_type == dnx_rx_trap_type_map[i_trap].trap_id)
                    {
                        *bcm_trap_type_p = i_trap;
...
*   \param [in] trap_class        -  Ingress trap type
...
{
    /*
     * User define, DA_NOT_FOUND, Unknown_dest and OAM are traps that being used by other applications as action entry,
     * hence dont have strength
     */
    if (trap_class != DNX_RX_TRAP_CLASS_USER_DEFINED && trap_class != DNX_RX_TRAP_CLASS_OAM &&
        DNX_RX_TRAP_IS_DA_NOT_FOUND(trap_type) == FALSE &&
        trap_type != DBAL_ENUM_FVAL_INGRESS_TRAP_ID_DEFAULT_DROP_TRAP &&
        trap_type != DBAL_ENUM_FVAL_INGRESS_TRAP_ID_DEFAULT_REDIRECT_TO_CPU_TRAP &&
        trap_type != DBAL_ENUM_FVAL_INGRESS_TRAP_ID_UNKNOWN_DESTINATION)
    {
        *is_strengthless = FALSE;
    }
    else
    {
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    switch (trap_block)
    {
        case DNX_RX_TRAP_BLOCK_INGRESS:
            ...
            /** Based on trap_block, we get the class from either encoding for EGRESS and dnx_rx_trap_ingress_class_get for INGRESS.*/
            SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_class_get(unit, trap_id, &trap_class));
            break;
        case DNX_RX_TRAP_BLOCK_ERPP:
            if (trap_type == BCM_RX_TRAP_EG_TRAP_ID_DEFAULT)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Default ERPP profile can't be set by user\n");
            }
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            if (trap_type == BCM_RX_TRAP_EG_TX_TRAP_ID_DEFAULT || trap_type == BCM_RX_TRAP_EG_TX_TRAP_ID_DROP)
            {
    ...
    dnx_rx_trap_ingress_strengthless_trap_check(unit, trap_type, trap_class, &is_strengthless);
    ...
    if (dest_ow == TRUE)
    {
        ...
        if (fwd_dest_valid == TRUE)
        {
            ...
            else
            {
                ...
                if (BCM_GPORT_IS_SYSTEM_PORT(gport_dest))
                {
                    dest_port = BCM_GPORT_SYSTEM_PORT_ID_GET(gport_dest);
                }
                else
                {
                    dest_port = gport_dest;
                }

                /** Retrieve dest_port according to core configuration */
                if (config_p->core_config_arr == NULL)
                {
                    config_p->dest_port = dest_port;
                }
                else
                {
                    config_p->core_config_arr[core_id].dest_port = dest_port;
                }
            }
        }
        else
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    for (stat_obj_to_ow = 0; stat_obj_to_ow < config_p->stat_obj_config_len; stat_obj_to_ow++)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].overwrite,
                                     INST_SINGLE, TRUE);
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].index,
                                     INST_SINGLE, config_p->stat_obj_config_arr[stat_obj_to_ow].counter_command_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].value,
                                     INST_SINGLE, config_p->stat_obj_config_arr[stat_obj_to_ow].stat_id);
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].offset_by_qual,
                                     INST_SINGLE,
                                     config_p->stat_obj_config_arr[stat_obj_to_ow].is_offset_by_qual_enable);
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].type,
                                     INST_SINGLE, config_p->stat_obj_config_arr[stat_obj_to_ow].stat_object_type);
        dbal_entry_value_field32_set(unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].is_meter,
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    for (stat_obj_to_ow = 0; stat_obj_to_ow < max_nof_stat_objs_to_ow; stat_obj_to_ow++)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                            dnx_rx_trap_map_stat_obj[stat_obj_to_ow].overwrite,
                                                            INST_SINGLE, &stat_obj_ow));

        if (stat_obj_ow)
        {
            config_p->stat_obj_config_len = stat_obj_to_ow + 1;
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                                                dnx_rx_trap_map_stat_obj[stat_obj_to_ow].index,
                                                                INST_SINGLE,
                                                                (uint32 *)
                                                                &config_p->stat_obj_config_arr
                                                                [stat_obj_to_ow].counter_command_id));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].value, INST_SINGLE,
                             &config_p->stat_obj_config_arr[stat_obj_to_ow].stat_id));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                            (unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].offset_by_qual,
                             INST_SINGLE, &config_p->stat_obj_config_arr[stat_obj_to_ow].is_offset_by_qual_enable));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].type, INST_SINGLE,
                             (uint32 *) &config_p->stat_obj_config_arr[stat_obj_to_ow].stat_object_type));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                            (unit, entry_handle_id, dnx_rx_trap_map_stat_obj[stat_obj_to_ow].is_meter, INST_SINGLE,
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    for (core_id = 0; core_id < nof_cores; core_id++)
    {
        ...
        /** Set action attribute to make desintaion equal to VSI */
        ...
        if (config_p->flags & BCM_RX_TRAP_UPDATE_FORWARDING_HEADER)
        {
            if (config_p->forwarding_header == bcmRxTrapForwardingHeaderOamBfdPdu)
            {
                SHR_ERR_EXIT(_SHR_E_UNAVAIL,
                             "bcmRxTrapForwardingHeaderOamBfdPdu type not supported,use cint_field_oam_wa_layer_index.c as WA in PMF\n");
            }
...
* \par DIRECT INPUT:
*   \param [in] unit      -  Unit Id
*   \param [in] trap_class  -  trap class (predefined, user-defined) to configure
*   \param [in] trap_type  -  ingress trap type (HW trap id value) to configure
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
        ...
        switch (trap_type)
        {
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_FER_SAME_INTERFACE:
                SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_same_interface_strength_hw_clear(unit,
                                                                                     DNX_RX_TRAP_SAME_INTERFACE_CONTEXT_PROFILE_DEFALT));
                break;
    ...
    int trap_type)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_strength_clear(unit, trap_class, trap_type));
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_clear(unit, trap_class, trap_type));

    /** Set forward and snoop strengths */
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_strength_set(unit, trap_class, trap_type, config_p));
    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    if (config_p->flags & BCM_RX_TRAP_UPDATE_ENCAP_ID)
    {
        ...
        if (BCM_L3_ITF_TYPE_IS_LIF(config_p->encap_id))
        {
            /*
    ...
    {
        if (config_p->mirror_profile >= nof_snoop_profiles)
        {
    ...
    if (config_p->flags & BCM_RX_TRAP_UPDATE_DEST)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                            (unit, config_p->dest_port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info));
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    if ((config_p->flags & BCM_RX_TRAP_UPDATE_DEST) && (config_p->dest_port == BCM_GPORT_BLACK_HOLE))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Drop action has a pre-configured profile(BCM_RX_TRAP_EG_TX_TRAP_ID_DROP), "
                     "please use it as Trap ID, instead of User Defined create and set");
    }
    ...
    etpp_config_p->action.is_recycle_high_priority = (config_p->is_recycle_high_priority == FALSE);
...
/**
* \brief
*  This function clears the MTU Profile in HW.
* \par DIRECT INPUT:
*   \param [in] unit         - unit id
    ...
    uint8 compressed_layer_type)
{
     ...
     * Keys
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MTU_PROFILE, mtu_profile);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LAYER_TYPE_COMPRESSED, compressed_layer_type);
...
/**
* \brief  
*  This function maps the MTU Profile and Compressed Protocol Type to an MTU Config.
*  It adds MTU Value and Trap-action-profile in the HW
* \par DIRECT INPUT:
*   \param [in] unit                    - unit id
*   \param [in] mtu_profile             - MTU Profile
*   \param [in] mtu_profile_config_p    - MTU PRofile config
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e -  Error Type
    ...
    dnx_rx_trap_mtu_profile_config_t * mtu_profile_config_p)
{
     ...
     * Keys
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LAYER_TYPE_COMPRESSED,
                               mtu_profile_config_p->compressed_layer_type);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_MTU_PROFILE, mtu_profile);

    /*
     * Values
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MTU, INST_SINGLE, mtu_profile_config_p->mtu);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_TRAP_ACTION_PROFILE, INST_SINGLE,
                                mtu_profile_config_p->trap_action_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TRAP_FWD_STRENGTH, INST_SINGLE,
                                 mtu_profile_config_p->fwd_strength);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_TRAP_SNP_STRENGTH, INST_SINGLE,
                                mtu_profile_config_p->snp_strength);
...
{
    uint32 entry_handle_id;
    dnx_rx_trap_map_etpp_profile_dbal_fields_t etpp_profile_dbal_fields = { 0 };
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_PEMLA_TRAP, &entry_handle_id));

    etpp_profile_dbal_fields = dnx_rx_trap_map_etpp_ud_profile[profile];
    ...
    int *new_recycle_cmd_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_recycle_cmd.exchange
                    (unit, _SHR_CORE_ALL, 0, recycle_cmd_config_p, old_recycle_cmd, NULL, new_recycle_cmd_p, &is_first,
                     &is_last));
...
*   \param [in] is_profile_clear  -  is profile clear (set to default values)
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    if (etpp_config_p->recycle_cmd_config.recycle_strength != 0 || is_profile_clear)
    {
    ...
    dnx_rx_trap_etpp_action_t * etpp_action_p)
{
    ...
    etpp_profile_dbal_fields = dnx_rx_trap_map_etpp_oam_profile[trap_type];
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    /** For OAMP the ingress trap id is a predefined value*/
    if (trap_type == DNX_RX_TRAP_ETPP_OAM_UP_MEP_OAMP)
    {
        etpp_config_p->action.ingress_trap_id = DBAL_ENUM_FVAL_INGRESS_TRAP_ID_TRAP_ETH_OAM;
    }
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    recycle_trap_id_block = DNX_RX_TRAP_ID_BLOCK_GET(etpp_config_p->recycle_cmd_config.ingress_trap_id);
    ...
    stamped_trap_id_block = DNX_RX_TRAP_ID_BLOCK_GET(etpp_config_p->action.ingress_trap_id);
    if (stamped_trap_id_block != DNX_RX_TRAP_BLOCK_INGRESS)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "stamped_trap_code can only be an INGRESS trap code!");
    }
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_set_verify(unit, etpp_config_p));

    switch (trap_class)
    {
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_user_defined_set(unit, trap_type, FALSE, etpp_config_p));
            break;
        case DNX_RX_TRAP_CLASS_OAM:
            oam_trap_type = DNX_RX_TRAP_ETPP_OAM_TRAP_TYPE_GET(trap_type);
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    switch (trap_block)
    {
        ...
        case DNX_RX_TRAP_BLOCK_ERPP:
            ...
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_config_from_config_fill(unit, config_p, &etpp_config));
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_strength_get(unit, trap_class, trap_type, config_p));
    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    /** Keys */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_TRAP_ACTION_PROFILE, (uint32) profile);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, (uint32) BCM_CORE_ALL);
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_DSP, INST_SINGLE, &(erpp_config_p->pp_dsp));
    ...
    dnx_rx_trap_etpp_config_t * etpp_config_p)
{
    ...
    switch (trap_class)
    {
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            switch (trap_type)
            {
                ...
                default:
                    ...
                    SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_recycle_cmd_hw_get(unit, BCM_CORE_ALL,
        ...
        case DNX_RX_TRAP_CLASS_OAM:
            ...
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_recycle_cmd_hw_get(unit, BCM_CORE_ALL, etpp_config_p->action.recycle_cmd,
                                                                &(etpp_config_p->recycle_cmd_config)));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal trap_class=%d\n", trap_class);
    ...
    int trap_id)
{
    ...
    if (is_allocated == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "trap_id was not created, call bcm_rx_trap_type_create() first \n");
    }
    ...
    int trap_type)
{
    ...
    switch (trap_class)
    {
        case DNX_RX_TRAP_CLASS_USER_DEFINED:
            ...
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_user_defined_set(unit, trap_type, TRUE, &etpp_config));
    ...
    bcm_gport_t gport)
{
    ...
    uint8 unknown_da_enabled = FALSE, ipv6_traps_enabled = FALSE;
    int system_headers_mode, trap_id;
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_rx_app_trap_type_map[trap_type].valid == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Trap type = %d , not supported \n", trap_type);
    }
    ...
    if ((system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE &&
         unknown_da_enabled == FALSE) && trap_type == bcmRxTrapEgUnknownDa)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "UnknowDA filter is not supported on this device in Jer2 System Headers mode.\n");
    }

    ipv6_traps_enabled = dnx_data_trap.erpp.feature_get(unit, dnx_data_trap_erpp_ipv6_traps_en);
    if (ipv6_traps_enabled == FALSE && trap_type == bcmRxTrapEgIpv6Error)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "bcmRxTrapEgIpv6Error is not supported on this device.\n");
    }
    ...
    if (BCM_GPORT_IS_MIRROR_MIRROR(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_action_profile_hw_get(unit, dnx_rx_app_trap_type_map[trap_type].trap_id,
                                                               &action_profile));
    ...
    else if (BCM_GPORT_IS_TRAP(gport))
    {
        ...
        {
            if ((dnx_rx_app_trap_type_map[trap_type].trap_block == DNX_RX_TRAP_BLOCK_ERPP) &&
 ...
 */
shr_error_e
dnx_rx_trap_action_profile_set(
    int unit,
    uint32 flags,
    bcm_rx_trap_t trap_type,
    bcm_gport_t gport)
{
    ...
    switch (dnx_rx_app_trap_type_map[trap_type].trap_block)
    {
        case DNX_RX_TRAP_BLOCK_ERPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_erpp_action_profile_hw_set(unit, dnx_rx_app_trap_type_map[trap_type].trap_id,
                                                                   &action_profile));
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_action_profile_hw_set(unit, dnx_rx_app_trap_type_map[trap_type].trap_id,
                                                                   &action_profile));
            break;
        default:
 ...
 */
shr_error_e
dnx_rx_trap_action_profile_get(
    int unit,
    bcm_rx_trap_t trap_type,
    bcm_gport_t * gport_p)
{
    ...
    {
        case DNX_RX_TRAP_BLOCK_ERPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_erpp_action_profile_hw_get(unit, dnx_rx_app_trap_type_map[trap_type].trap_id,
                                                                   &action_profile));
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_action_profile_hw_get(unit, dnx_rx_app_trap_type_map[trap_type].trap_id,
    ...
    if (action_profile.fwd_strength != 0 || action_profile.snp_strength == 0)
    {
        ...
        action_profile.profile = DNX_RX_TRAP_ID_SET(dnx_rx_app_trap_type_map[trap_type].trap_block,
                                                    DNX_RX_TRAP_CLASS_USER_DEFINED, action_profile.profile);
    }
    else if (action_profile.snp_strength != 0)
    {
        /** In case of snoop need to convert profile value */
        action_profile.profile = DNX_MIRROR_EGRESS_PROFILE_GET(action_profile.profile);
    }
    ...
    BCM_GPORT_TRAP_SET(*gport_p, action_profile.profile, action_profile.fwd_strength, action_profile.snp_strength);
 ...
 */
shr_error_e
dnx_rx_trap_action_profile_clear(
    int unit,
    bcm_rx_trap_t trap_type)
{
    dnx_rx_trap_action_profile_t action_profile = { 0 };
    ...
    switch (dnx_rx_app_trap_type_map[trap_type].trap_block)
    {
        case DNX_RX_TRAP_BLOCK_ERPP:
            ...
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_action_profile_hw_clear
                            (unit, dnx_rx_app_trap_type_map[trap_type].trap_id));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_eth_trap_vector_hw_set(unit, DBAL_ENUM_FVAL_LAYER_TYPES_IPV4,
                                                               DNX_RX_TRAP_VTT_MY_MAC_AND_IP_DISABLE_ACTION_PROFILE_IDX));
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_eth_trap_vector_hw_set(unit, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS,
                                                               DNX_RX_TRAP_VTT_MY_MAC_AND_MPLS_DISABLE_ACTION_PROFILE_IDX));
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_eth_trap_vector_hw_set(unit, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_UA,
                                                               DNX_RX_TRAP_VTT_MY_MAC_AND_MPLS_DISABLE_ACTION_PROFILE_IDX));
    ...
    int unit)
{
    ...
    /** Set keys */
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_ERPP_TRAP_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
...
/**
* \brief
*  Gets the Compressed ICMP type in the ICMP type compression table.
* \par DIRECT INPUT:
*   \param [in] unit                    - unit Id
*   \param [in] icmp_type               - ICMP type
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    if (protocol_trap_mode == DNX_RX_TRAP_PROTOCOL_MODE_IN_LIF)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange(unit, &in_lif_profile_info, old_in_lif_profile,
                                                    &new_in_lif_profile, LIF, gport_hw_resources.inlif_dbal_table_id));
    ...
    {
        /*
         * In case of In-PP-Port indexing
         */
        SHR_IF_ERR_EXIT(dnx_rx_trap_protocol_profiles_hw_set(unit, port, protocol_profiles_p));
    }
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    else
    {
         ...
         * In case of In-PP-Port indexing
         */
        SHR_IF_ERR_EXIT(dnx_rx_trap_protocol_profiles_hw_get(unit, port, protocol_profiles_p));
    }
...
/**
* \brief  
*  Set additional configuration of My-ARP/My-NDP protocol trap in HW.
* \par DIRECT INPUT: 
*   \param [in] unit      -  unit Id
*   \param [in] protocol_dbal_table  -  protocol type dbal table
...
*/
static shr_error_e
dnx_rx_trap_protocol_my_arp_ndp_hw_set(
    int unit,
    dbal_tables_e protocol_dbal_table,
    dnx_rx_trap_protocol_my_arp_ndp_config_t * config_p)
{
    ...
    if (protocol_dbal_table == DBAL_TABLE_PROTOCOL_TRAP_MY_ARP)
    {
 ...
 */
shr_error_e
dnx_rx_trap_protocol_my_arp_ndp_set(
    int unit,
    ...
    dnx_rx_trap_protocol_my_arp_ndp_config_t * config_p)
{
    ...
    {
        case bcmRxTrapArpMyIp:
            protocol_dbal_table = DBAL_TABLE_PROTOCOL_TRAP_MY_ARP;
            ...
            break;
        case bcmRxTrapNdpMyIp:
            protocol_dbal_table = DBAL_TABLE_PROTOCOL_TRAP_MY_NDP;
            if (config_p->ignore_da != 0)
            {
                SHR_ERR_EXIT(_SHR_E_CONFIG, "My-NDP trap doesn't have ignore_da configuration! \n");
                break;
            }
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Function only supports bcmRxTrapArpMyIp or bcmRxTrapNdpMyIp trap types!\n");
            break;
    }

    SHR_IF_ERR_EXIT(dnx_rx_trap_protocol_my_arp_ndp_hw_set(unit, protocol_dbal_table, config_p));
...
/**
* \brief  
*  Get additional configuration of My-ARP/My-NDP protocol trap from HW.
* \par DIRECT INPUT: 
*   \param [in] unit      -  unit Id
*   \param [in] protocol_dbal_table  -  protocol type dbal table
*   \param [in] config_p   -   My-ARP/My-NDP protocol traps additional configuration
...
*/
static shr_error_e
dnx_rx_trap_protocol_my_arp_ndp_hw_get(
    int unit,
    dbal_tables_e protocol_dbal_table,
    dnx_rx_trap_protocol_my_arp_ndp_config_t * config_p)
{
 ...
 */
shr_error_e
dnx_rx_trap_protocol_my_arp_ndp_get(
    int unit,
    bcm_rx_trap_t protocol_type,
    dnx_rx_trap_protocol_my_arp_ndp_config_t * config_p)
{
    ...
    SHR_NULL_CHECK(config_p, _SHR_E_PARAM, "config_p");
    sal_memset(config_p, 0, sizeof(dnx_rx_trap_protocol_my_arp_ndp_config_t));
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_protocol_my_arp_ndp_hw_get(unit, protocol_dbal_table, config_p));
    ...
    bcm_rx_trap_prog_config_t * prog_config_p)
{
    ...
    if (prog_index >= dnx_data_trap.ingress.nof_programmable_traps_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The Programmable trap index supplied(%d), "
                     "cannot be higher than the maximum number of programmable traps(8)!", prog_index);
    }
    ...
    bcm_rx_trap_prog_config_t * prog_config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_prog_set_verify(unit, prog_index, prog_config_p));
...
/**
* \brief
*  Retrieves the MTU profile which has been allocated for the supplied data(config).
* \par DIRECT INPUT:
*   \param [in] unit                    - unit Id
*   \param [in] mtu_profile_config_p    - MTU Profile Config data
    ...
    int *mtu_profile_p)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(mtu_profile_config_p, _SHR_E_PARAM, "mtu_profile_config_p");
    SHR_NULL_CHECK(mtu_profile_p, _SHR_E_PARAM, "mtu_profile_p");

    SHR_IF_ERR_EXIT_NO_MSG(algo_rx_db.
                           trap_mtu_profile.profile_get(unit, _SHR_CORE_ALL, mtu_profile_config_p, mtu_profile_p));
...
/**
* \brief
*  Clears the allocated profile from the template manager and frees it.
* \par DIRECT INPUT:
*   \param [in] unit           - unit Id
*   \param [in] mtu_profile    - MTU Profile Config data
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
    ...
    int mtu_profile)
{
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_mtu_profile.free_single(unit, _SHR_CORE_ALL, mtu_profile, &is_last));
...
*/
shr_error_e
dnx_rx_trap_mtu_profile_data_get(
    int unit,
    int mtu_profile,
    dnx_rx_trap_mtu_profile_config_t * mtu_profile_config_p)
{
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_mtu_profile.profile_data_get(unit, _SHR_CORE_ALL, mtu_profile, &ref_count,
                                                                 mtu_profile_config_p));
    ...
    uint8 *is_last)
{
    ...
    *is_last = TRUE;
    ...
    /** Taking a handle */
    DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_ETPP_MTU_PORT_INFO, &entry_handle_id);
...
/**
* \brief
*  Sets the MTU Profile in the Port table(DBAL_TABLE_ETPP_MTU_PORT_INFO),
*    for the supplied port. It also enables the MTU global config for ports.
* \par DIRECT INPUT:
*   \param [in] unit        - unit Id
*   \param [in] port        - Port (0-255)
*   \param [in] mtu_profile - MTU Profile
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
...
*/
static shr_error_e
dnx_rx_trap_port_mtu_profile_hw_set(
    int unit,
    bcm_gport_t port,
    int mtu_profile)
{
     ...
     */
    if (mtu_profile == DNX_RX_TRAP_MTU_DEFAULT_PROFILE)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_port_mtu_is_last(unit, port, &is_last));
        if (is_last == TRUE)
        {
            mtu_enable = FALSE;
        }
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_ETPP_MTU_PORT_INFO, entry_handle_id));

    /*
     * key construction - Port
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_PP_PORT, port);
...
/**
* \brief
*  Gets the MTU Profile from the Port table(DBAL_TABLE_ETPP_MTU_PORT_INFO),
*    for the supplied port.
* \par DIRECT INPUT:
*   \param [in] unit         - unit Id
*   \param [in] port         - Port (0-255)
*   \param [out] mtu_profile_p - MTU Profile
...
*/
static shr_error_e
dnx_rx_trap_port_mtu_profile_hw_get(
    int unit,
    bcm_gport_t port,
    int *mtu_profile_p)
{
...
/**
* \brief
*  This function allocates a MTU Profile according to given configuration.
* \par DIRECT INPUT:
*   \param [in] unit                    - unit id
*   \param [in] mtu_profile_config_p    - MTU Profile configuration
*   \param [out] new_mtu_profile_p      - new MTU Profile allocated
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e -  Error Type
...
*/
static shr_error_e
dnx_rx_trap_mtu_profile_set(
    int unit,
    dnx_rx_trap_mtu_profile_config_t * mtu_profile_config_p,
    int *new_mtu_profile_p)
{
    uint8 is_first = 0, is_last = 0;
    int old_mtu_profile;
    int rv;
    ...
    SHR_NULL_CHECK(mtu_profile_config_p, _SHR_E_PARAM, "mtu_profile_config_p");
    SHR_NULL_CHECK(new_mtu_profile_p, _SHR_E_PARAM, "new_mtu_profile_p");

    /**
     * Check if the data already exists, so we can get the old_profile, if not, we use the dafault one.
     */
    rv = dnx_rx_trap_mtu_profile_get(unit, mtu_profile_config_p, &old_mtu_profile);
    if (rv == _SHR_E_NOT_FOUND)
    {
        /** Set old_profile to be equal to default profile(0) */
        old_mtu_profile = DNX_RX_TRAP_MTU_DEFAULT_PROFILE;
    }
    else
    {
        SHR_IF_ERR_EXIT(rv);
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_mtu_profile.allocate_single
                    (unit, _SHR_CORE_ALL, SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID,
                     mtu_profile_config_p, NULL, new_mtu_profile_p, &is_first));

    /** Translate from Template manager MTU Profile(6b) to 3b MTU Profile */
    *new_mtu_profile_p = DNX_RX_MTU_PROFILE_FROM_INDEX_GET(*new_mtu_profile_p);
    old_mtu_profile = DNX_RX_MTU_PROFILE_FROM_INDEX_GET(old_mtu_profile);

    /** If this is the first time the MTU profile is allocated, set it in HW */
    if (is_first)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_mtu_profile_hw_set(unit, *new_mtu_profile_p, mtu_profile_config_p));
    }

    /** If this is the last trap pointing to this MTU profile, clear profile from HW */
    if (is_last)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_mtu_profile_hw_clear
                        (unit, old_mtu_profile, mtu_profile_config_p->compressed_layer_type));
    }
...
/**
* \brief
*  This function frees the MTU profile and clears the MTU profile per Port
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    /** Assign the DEFAULT Port FWD type */
    mtu_config_p->compressed_layer_type = DNX_RX_MTU_PORT_COMPRESSED_LAYER_TYPE;

    /**
     * Get the MTU profile from the PORT DBAL Table
     */
    SHR_IF_ERR_EXIT(dnx_rx_trap_port_mtu_profile_hw_get(unit, mtu_config_p->gport, &mtu_profile));

    /**
     * We pass 0(DEFAULT) to clear the MTU Profile from the PORT table.
     */
    SHR_IF_ERR_EXIT(dnx_rx_trap_port_mtu_profile_hw_set(unit, mtu_config_p->gport, DNX_RX_TRAP_MTU_DEFAULT_PROFILE));

    SHR_IF_ERR_EXIT(dnx_rx_trap_mtu_profile_hw_clear(unit, mtu_profile, mtu_config_p->compressed_layer_type));
    /** Translate from 3b MTU Profile, to Template manager MTU Profile(5b) */
    mtu_profile = DNX_RX_MTU_ENTRY_INDEX_SET(mtu_profile, mtu_config_p->compressed_layer_type);

    SHR_IF_ERR_EXIT(dnx_rx_mtu_profile_clear(unit, mtu_profile));
exit:
    SHR_FUNC_EXIT;
...
/**
* \brief
*  This function frees the MTU profile and clears the MTU profile per LIF
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    /**
    * We pass 0(DEFAULT) to clear the MTU Profile from the LIF table.
    */
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_mtu_profile_hw_set(unit,
                                                       ...
                                                       DNX_RX_TRAP_MTU_DEFAULT_PROFILE));
...
/**
* \brief
*  This function frees the MTU profile and clears the MTU profile per RIF
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    /**
     * Get the MTU profile from the RIF DBAL Table
     */
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_mtu_profile_hw_get(unit,
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_mtu_profile_hw_set(unit,
                                                       ...
                                                       DBAL_FIELD_OUT_RIF,
                                                       rif_hw_resources.outlif_dbal_result_type,
                                                       mtu_config_p->intf, DNX_RX_TRAP_MTU_DEFAULT_PROFILE));
...
/**
* \brief
*  This function frees the MTU profile and clears the MTU profile per LIF/RIF/Port
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_mtu_port_clear(unit, mtu_config_p));
    }
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_rx_mtu_rif_clear(unit, mtu_config_p));
        }
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_rx_mtu_lif_clear(unit, mtu_config_p));
        }
    }
...
/**
* \brief
*  This function allocates an MTU profile in the Template manager and sets
*  the relevant DBAL table for enabling PORT MTU Configuration.
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
...
dnx_rx_mtu_port_set(
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    dnx_rx_trap_mtu_profile_config_t mtu_profile_config;
    int trap_id;
    int new_mtu_profile = 0;

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(&mtu_profile_config, 0, sizeof(dnx_rx_trap_mtu_profile_config_t));

    /**
     * Set the relevant params in 'mtu_profile_config' for the template manager allocation.
     */
    mtu_profile_config.mtu = mtu_config_p->mtu;
    trap_id = BCM_GPORT_TRAP_GET_ID(mtu_config_p->trap_gport);
    mtu_profile_config.trap_action_profile = DNX_RX_TRAP_ID_TYPE_GET(trap_id);
    mtu_profile_config.fwd_strength = BCM_GPORT_TRAP_GET_STRENGTH(mtu_config_p->trap_gport);
    mtu_profile_config.snp_strength = BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(mtu_config_p->trap_gport);
    mtu_profile_config.is_profile_2b = FALSE;

    /**
     * Compressed forwarding layer type for PORT is 3b'1 = 0x7(DNX_RX_MTU_PORT_COMPRESSED_LAYER_TYPE).
     */
    mtu_profile_config.compressed_layer_type = DNX_RX_MTU_PORT_COMPRESSED_LAYER_TYPE;

    /**
     * Allocate an MTU Profile
     */
    SHR_IF_ERR_EXIT(dnx_rx_trap_mtu_profile_set(unit, &mtu_profile_config, &new_mtu_profile));

    /**
     *  Set the MTU Profile to the port config and enable Port MTU config
     */
    SHR_IF_ERR_EXIT(dnx_rx_trap_port_mtu_profile_hw_set(unit, mtu_config_p->gport, new_mtu_profile));
...
* \brief
*  This function allocates an MTU profile in the Template manager and sets
*  the relevant DBAL table for enabling LIF MTU Configuration.
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
    int trap_id;
    int new_mtu_profile = 0, profile_size = 0;
    ...
    mtu_profile_config.fwd_strength = BCM_GPORT_TRAP_GET_STRENGTH(mtu_config_p->trap_gport);
    mtu_profile_config.snp_strength = BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(mtu_config_p->trap_gport);
    mtu_profile_config.compressed_layer_type = mtu_config_p->compressed_layer_type;
    mtu_profile_config.is_profile_2b = FALSE;
    ...
    SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, mtu_config_p->gport,
                                                                   ...
                                                                   &gport_hw_resources), _SHR_E_NOT_FOUND,
                                _SHR_E_PARAM);
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, gport_hw_resources.outlif_dbal_table_id, DBAL_FIELD_MTU_PROFILE,
                                               FALSE, gport_hw_resources.outlif_dbal_result_type,
                                               INST_SINGLE, &profile_size));

    if (profile_size == 2)
    {
        mtu_profile_config.is_profile_2b = TRUE;
    }
    /**
     * Allocate an MTU Profile
     */
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_mtu_profile_hw_set(unit,
                                                       ...
                                                       DBAL_FIELD_OUT_LIF,
                                                       gport_hw_resources.outlif_dbal_result_type,
                                                       gport_hw_resources.local_out_lif, new_mtu_profile));
...
* \brief
*  This function allocates an MTU profile in the Template manager and sets
*  the relevant DBAL table for enabling RIF MTU Configuration.
*
* \par DIRECT INPUT:
*   \param [in] unit                 - Unit Id
*   \param [in] mtu_config_p         - MTU Configuration
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, rif_hw_resources.outlif_dbal_table_id, DBAL_FIELD_MTU_PROFILE,
                                               FALSE, rif_hw_resources.outlif_dbal_result_type,
                                               INST_SINGLE, &profile_size));
    if (profile_size == 2)
    {
        mtu_profile_config.is_profile_2b = TRUE;
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_mtu_profile_hw_set(unit,
                                                       ...
                                                       DBAL_FIELD_OUT_RIF,
                                                       rif_hw_resources.outlif_dbal_result_type,
                                                       mtu_config_p->intf, new_mtu_profile));
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    if (mtu_config_p->mtu > dnx_data_trap.etpp.max_mtu_value_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The supplied MTU value(%d) is bigger than the maximum allowed for the device: %d",
                     mtu_config_p->mtu, dnx_data_trap.etpp.max_mtu_value_get(unit));
    }

    if (!BCM_GPORT_IS_TRAP(mtu_config_p->trap_gport))
    {
        if (mtu_config_p->trap_gport != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "The Trap-Gport is not valid, should be 0 or valid Trap-Gport");
        }
    }

    if ((BCM_GPORT_TRAP_GET_STRENGTH(mtu_config_p->trap_gport) != 0) &&
        (BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(mtu_config_p->trap_gport) != 0))
    {
    ...
    if (mtu_config_p->compressed_layer_type > DNX_RX_MTU_PORT_COMPRESSED_LAYER_TYPE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid compressed forward layer type, %d is too big! Maximum value is %d",
                     mtu_config_p->compressed_layer_type, DNX_RX_MTU_PORT_COMPRESSED_LAYER_TYPE);
    }
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    /*
     * If the supplied MTU Value and trap_gport are 0,
     * it means its a structure meant for clearing a config.
     */
    if ((mtu_config_p->mtu == 0) && (mtu_config_p->trap_gport == 0))
    {
        SHR_IF_ERR_EXIT(dnx_rx_mtu_clear(unit, mtu_config_p));
        SHR_EXIT();
    }
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    /** Translate from 3b MTU Profile, to Template manager MTU Profile(6b) */
    mtu_profile = DNX_RX_MTU_ENTRY_INDEX_SET(mtu_profile, mtu_profile_config.compressed_layer_type);
    SHR_IF_ERR_EXIT(dnx_rx_trap_mtu_profile_data_get(unit, mtu_profile, &mtu_profile_config));

    mtu_config_p->mtu = mtu_profile_config.mtu;
    trap_id =
        DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_USER_DEFINED,
                           mtu_profile_config.trap_action_profile);
    BCM_GPORT_TRAP_SET(mtu_config_p->trap_gport, trap_id, mtu_profile_config.fwd_strength,
                       mtu_profile_config.snp_strength);
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_rx_mtu_config_t * mtu_config_p)
{
    ...
    sal_memset(&mtu_profile_config, 0, sizeof(mtu_profile_config));
    ...
    uint8 *is_rif_p)
{
    ...
    *is_rif_p = FALSE;
    ...
    switch (lif_config_p->lif_type)
    {
        ...
        case bcmRxTrapLifTypeOutLif:
            *is_ingress_lif_p = FALSE;
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(lif_config_p->lif_gport))
            {
                /** Egress Virtual Vlan ports (ESEM Native or non-Native) */
                is_esem_entry = TRUE;
                lif_hw_resources_p->global_out_lif = lif_config_p->lif_gport;
            }
            else if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(lif_config_p->lif_gport))
            {
                /** Egress ESEM Default */
                is_esem_entry = TRUE;
                lif_hw_resources_p->global_out_lif = lif_config_p->lif_gport;
                lif_hw_resources_p->outlif_dbal_table_id = DBAL_TABLE_ESEM_DEFAULT_RESULT_TABLE;
                lif_hw_resources_p->outlif_dbal_result_type = DBAL_RESULT_TYPE_ESEM_DEFAULT_RESULT_TABLE_ETPS_AC;
            }
            else
            {
                gport = lif_config_p->lif_gport;
    ...
    if (*is_rif_p == FALSE && is_esem_entry == FALSE)
    {
    ...
    bcm_gport_t action_gport)
{
    ...
    SHR_IF_ERR_EXIT(algo_rx_db.trap_lif_ingress.exchange
                    (unit, _SHR_CORE_ALL, SW_STATE_ALGO_TEMPLATE_EXCHANGE_IGNORE_NOT_EXIST_OLD_PROFILE, &action_gport,
    ...
    if (is_last)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_ingress_action_profile_hw_clear(unit, new_lif_trap_profile));
    }
...
/**
* \brief
*  Configure the key and result type of vlan port match (ESEM entry configuration)
*
* \par DIRECT INPUT:
*   \param [in] unit                       - Unit Id
*   \param [in] vlan_port_id            - contains the gport that has to be encoded 
*                                                  with at least a type and a subtype
*   \param [out] lif_trap_profile_p  - lif trap profile, if valid value treated as input, otherwise output
* \par INDIRECT INPUT:
*   * None
* \par DIRECT OUTPUT:
*   shr_error_e - Error type
...
*/
static shr_error_e
dnx_rx_trap_lif_match_esem_config(
    int unit,
    ...
    uint32 *lif_trap_profile_p)
{
    ...
    /** Write to EGRESS ESEM OUT AC table */
    ...
    if (*lif_trap_profile_p != DNX_RX_TRAP_LIF_PROFILE_INVALID)
    {
        /** Set the LIF trap profile */
    ...
    uint32 lif_trap_profile)
{
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(lif_hw_resources_p->global_out_lif))
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_match_esem_config(unit, lif_hw_resources_p->global_out_lif, &lif_trap_profile));
    }
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, lif_hw_resources_p->outlif_dbal_table_id, &entry_handle_id));

        /** key construction - check if ESEM default or not */
        if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_DEFAULT(lif_hw_resources_p->global_out_lif))
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ESEM_DEFAULT_RESULT_PROFILE,
                                       BCM_GPORT_SUB_TYPE_VIRTUAL_EGRESS_DEFAULT_GET
                                       (lif_hw_resources_p->global_out_lif));
        }
        else
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_OUT_LIF, lif_hw_resources_p->local_out_lif);
        }

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                     lif_hw_resources_p->outlif_dbal_result_type);
    ...
    uint32 *lif_trap_profile_p)
{
    ...
    if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_MATCH(lif_hw_resources_p->global_out_lif))
    {
        *lif_trap_profile_p = DNX_RX_TRAP_LIF_PROFILE_INVALID;
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_match_esem_config
                        (unit, lif_hw_resources_p->global_out_lif, lif_trap_profile_p));
    }
    else
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, lif_hw_resources_p->outlif_dbal_table_id, &entry_handle_id));
    ...
    bcm_gport_t action_gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_hw_get(unit, lif_hw_resources_p, &old_lif_trap_profile));
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_hw_set(unit, lif_hw_resources_p, new_lif_trap_profile));
    ...
    bcm_gport_t * action_gport_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_hw_get(unit, lif_hw_resources_p, &lif_trap_profile));
    ...
    bcm_rx_trap_lif_config_t * lif_config_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_set(unit, &lif_hw_resources, lif_config_p->action_gport));
    }
    ...
    bcm_rx_trap_lif_config_t * lif_config_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_get(unit, &lif_hw_resources, &lif_config_p->action_gport));
    }
