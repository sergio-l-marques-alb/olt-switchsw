    ...
    uint32 *mymac_prefix_profile)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGR_VSI_INFO_DB, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VSI, intf->l3a_vid);
    dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS);
    ...
    bcm_if_t * intf_id)
{
     ...
     */
    if (DNX_QOS_MAP_PROFILE_GET(ing_intf->qos_map_id) > DNX_QOS_L3_MAX_PROFILE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Qos map id %d is out of range", DNX_QOS_MAP_PROFILE_GET(ing_intf->qos_map_id));
    }
        /** Verify BCM_L3_INGRESS_IPMC_BRIDGE_FALLBACK flag validity */
    if (!BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && _SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_IPMC_BRIDGE_FALLBACK))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_l3_intf_t * intf)
{
     ...
     */
    dnx_l3_check_if_ingress_intf_exists(unit, intf->l3a_intf_id, &intf_exists);
    ...
    /*
     * Check if intf exists - egress side.
     */
    SHR_IF_ERR_EXIT(algo_l3_db.rif_alloc.rif_res_manager.is_allocated(unit, intf->l3a_intf_id, &is_allocated));

    if (replace && !is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Asked to replace rif id 0x%08x, but it doesn't exist.", intf->l3a_intf_id);
    }

    if (!replace && is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_EXISTS, "Rif id 0x%08x already exists, but replace flag wasn't given.", intf->l3a_intf_id);
    }
        ...
        (intf->ingress_qos_model.ingress_ttl > bcmQosIngressModelUniform))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     ...
                     intf->ingress_qos_model.ingress_ttl);
    }
    ...
    int old_mymac_prefix_profile)
{
    ...
    if (old_mymac_prefix_profile)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.free_single
                        (unit, _SHR_CORE_ALL, old_mymac_prefix_profile, &is_last));
    }
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.allocate_single
                    (unit, _SHR_CORE_ALL, 0, prefix, NULL, (int *) my_mac_prefix_index, &first_reference));
    ...
    uint8 *is_first_mymac_reference)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.allocate_single
                    (unit, _SHR_CORE_ALL, 0, &source_address_entry, NULL, mymac_prefix, is_first_mymac_reference));
    ...
    uint8 *is_last_mymac_reference)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.exchange
                    (unit, _SHR_CORE_ALL, 0, &source_address_entry, old_mymac_prefix, NULL, new_mymac_prefix,
    ...
    uint8 *last_ref)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.free_single
                    (unit, _SHR_CORE_ALL, src_mac_prefix, last_ref));
    ...
    bcm_l3_intf_t * intf)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vsi_table_default_set(unit, vsi_value));
        old_in_lif_profile = DNX_IN_LIF_PROFILE_DEFAULT;
    ...
    bcm_if_t * intf_id)
{
    ...
    {
        /*
         * get the current IN-LIF-PROFILE from LIF-TABLE
         */
        BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, *intf_id);
        SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                    (unit, gport, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS,
    ...
    if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && _SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_L3_MCAST_L2))
    {
        ...
        in_lif_profile_info.ingress_fields.l2_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_MC_FWD_TYPE_IPV4MC;
    }
    else if (BCM_L3_ITF_TYPE_IS_RIF(*intf_id) && !_SHR_IS_FLAG_SET(ing_intf->flags, BCM_L3_INGRESS_L3_MCAST_L2))
    {
        ...
        in_lif_profile_info.ingress_fields.l2_mc_fwd_type = DBAL_ENUM_FVAL_VSI_PROFILE_L2_MC_FWD_TYPE_BRIDGE;
    }
    /*
    ...
    uint8 print_error_if_not_found)
{
    ...
    if (old_prefix_profile)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.profile_data_get
                        (unit, _SHR_CORE_ALL, old_prefix_profile, &is_allocated, NULL));
        if (is_allocated)
        {
            SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.free_single
                            (unit, _SHR_CORE_ALL, old_prefix_profile, &is_last));
        }
    }
    ...
    bcm_l3_intf_t * intf)
{
     ...
     */
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.ingress_mymac_prefix_table.profile_data_get
                    (unit, _SHR_CORE_ALL, my_mac_prefix, &ref_count, (uint8 *) &prefix));
    /*
     * get mymac
     */
    ...
    bcm_l3_intf_t * intf)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.profile_data_get
                    (unit, _SHR_CORE_ALL, mymac_prefix, &is_allocated, NULL));
    if (is_allocated)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.free_single
                        (unit, _SHR_CORE_ALL, mymac_prefix, &last_ref));
    }
