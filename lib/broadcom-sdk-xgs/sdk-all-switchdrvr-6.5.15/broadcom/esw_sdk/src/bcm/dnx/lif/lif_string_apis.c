    ...
    lif_mngr_local_outlif_str_info_t *outlif_str_info)
{
    ...
    if (inlif_str_info != NULL)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_result_type_string_to_id(
                    unit, inlif_info.dbal_table_id, inlif_str_info->result_type_name, (uint32 *)(&inlif_info.dbal_result_type)));
    ...
    if (outlif_str_info != NULL)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_result_type_string_to_id(
                unit, outlif_info.dbal_table_id, outlif_str_info->result_type_name, (uint32 *)(&outlif_info.dbal_result_type)));
        SHR_IF_ERR_EXIT(dnx_lif_mngr_phase_string_to_id(unit, outlif_str_info->outlif_phase_name, &outlif_info.outlif_phase));
