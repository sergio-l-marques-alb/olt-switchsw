    ...
    dnx_algo_template_print_t * print_cb_data)
{
    ...
    switch (entry_ptr->address_type)
    {
        case source_address_type_ipv4:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_IPV4, "IP Address",
                                                entry_ptr->address.ipv4_address, "Address type: IPv4", NULL);
            break;
        case source_address_type_ipv6:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_IPV6, "IP Address",
                                                entry_ptr->address.ipv6_address, "Address type: IPv6", NULL);
            break;
        case source_address_type_mac:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_MAC, "MAC Address",
                                                entry_ptr->address.mac_address, "Address type: MAC", NULL);
            break;
        case source_address_type_mac_dual_homing:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_MAC, "MAC Address Dual Homing",
                                                entry_ptr->address.mac_address, "Address type: MAC", NULL);
            break;
        case source_address_type_full_mac:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_MAC, "MAC Address Full",
                                                entry_ptr->address.mac_address, "Address type: MAC", NULL);
            break;
        default:
            DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_STRING, NULL, "Invalid entry type.", NULL,
                                                NULL);
    }
    ...
    uint32 alloc_flags)
{
    ...
    res_tag_bitmap_create_info.low_id = create_data->first_profile;
    res_tag_bitmap_create_info.count = create_data->nof_profiles;
    res_tag_bitmap_create_info.grain_size = create_data->nof_profiles / 2;
    res_tag_bitmap_create_info.max_tag_value = 1;
    res_tag_bitmap_create_info.flags = SW_STATE_RESOURCE_TAG_BITMAP_CREATE_FLAGS_NONE;

    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.res_tag_bitmap.create(unit, res_tag_bitmap_create_info));
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.
                    res_tag_bitmap.tag_set(unit, tag, FALSE, create_data->first_profile, half_bank));

    tag = SOURCE_ADDRESS_TOP_HALF_TAG;
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.
                    res_tag_bitmap.tag_set(unit, tag, FALSE, create_data->first_profile + half_bank, half_bank));
    ...
    uint8 *first_reference)
{
    ...
    if (allocation_needed)
    {
        ...
        res_tag_bitmap_flags = with_id ? SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_WITH_ID : 0;
        res_tag_bitmap_flags |=
            SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_ALIGN_ZERO | SW_STATE_RESOURCE_TAG_BITMAP_ALWAYS_CHECK_TAG;
            ...
            (entry_ptr->address_type == source_address_type_ipv6))
        {
                  ...
                  && (*profile < dnx_data_l3.source_address.source_address_table_size_get(unit) / 2)))
            {
                ...
                rv = algo_l3_db.source_address_table_allocation.res_tag_bitmap.bitmap_alloc(unit,
                                                                                            res_tag_bitmap_alloc_info,
                                                                                            profile);
        ...
        if (!allocation_successful)
        {
            ...
            rv = algo_l3_db.source_address_table_allocation.res_tag_bitmap.bitmap_alloc(unit, res_tag_bitmap_alloc_info,
                                                                                        profile);
    ...
    uint8 *last_reference)
{
    ...
    if (*last_reference)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.
                        res_tag_bitmap.bitmap_free(unit, res_tag_bitmap_free_info, profile));
    }
    ...
    sw_state_multi_set_t multi_set_template)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.res_tag_bitmap.clear(unit));
...
dnx_algo_l3_source_address_table_init(
    int unit)
{
    sw_state_algo_template_create_data_t source_address_template_data;
     ...
     * Create template.
     */
    sal_memset(&source_address_template_data, 0, sizeof(sw_state_algo_template_create_data_t));

    source_address_template_data.flags = SW_STATE_ALGO_TEMPLATE_CREATE_USE_ADVANCED_ALGORITHM;
