    ...
    bcm_mirror_destination_t * mirror_dest)
{
    ...
    if (mirror_dest->flags & ~(BCM_MIRROR_DEST_WITH_ID |
                               ...
                               BCM_MIRROR_DEST_DROP_SNIFF_IF_FWD_DROPPED |
                               BCM_MIRROR_DEST_DROP_FWD_IF_SNIFF_DROPPED | BCM_MIRROR_DEST_UPDATE_EXT_COUNTERS))
    {
    ...
    void *user_data)
{
    ...
    {
        BCM_GPORT_MIRROR_MIRROR_SET(gport, action_profile_id);
        ...
        if (is_allocated)
        {
            ...
            if (ret)
            {
                SHR_ERR_EXIT(_SHR_E_FAIL, "callback function returned error value %d for mirror destination %u", ret,
                             (unsigned) action_profile_id);
            }
        }
    }
    ...
    bcm_mirror_profile_cmd_count_t * mirror_count_info)
{
     ...
     * this is not relevant for is_meter=true 
     */
    if (mirror_count_info->is_counted == 1 && mirror_count_info->is_meter != 1)
    {
