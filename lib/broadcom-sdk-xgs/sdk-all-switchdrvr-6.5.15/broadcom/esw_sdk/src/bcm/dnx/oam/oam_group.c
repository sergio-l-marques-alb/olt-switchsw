    ...
    if (name_type == DNX_OAMP_OAM_MA_NAME_TYPE_ICC)
    {
        ...
        if (create)
        {
            ...
            SHR_IF_ERR_EXIT(algo_oam_db.oam_group_icc_profile.allocate_single
                            (unit, _SHR_CORE_ALL, 0, icc_profile_data, NULL, &profile_index, &first_reference));
        ...
        else
        {
            SHR_IF_ERR_EXIT(algo_oam_db.
                            oam_group_icc_profile.profile_get(unit, _SHR_CORE_ALL, icc_profile_data, &profile_index));
            SHR_IF_ERR_EXIT(algo_oam_db.
                            oam_group_icc_profile.free_single(unit, _SHR_CORE_ALL, profile_index, &last_reference));
            hw_write_data->profile_index = profile_index;
    ...
    bcm_oam_group_info_t * group_info)
{
    ...
    /** Group replace is currently not supported */
    if (group_info->flags & BCM_OAM_GROUP_REPLACE)
    {
        SHR_ERR_EXIT(_SHR_E_UNAVAIL, "Group replace is currently not supported ");
    }

    /*
     * currently BCM_OAM_GROUP_WITH_ID and BCM_OAM_GROUP_FLEXIBLE_MAID_48_BYTE are
     * the only supported flags, need to return error for any other flag configuration
     */
