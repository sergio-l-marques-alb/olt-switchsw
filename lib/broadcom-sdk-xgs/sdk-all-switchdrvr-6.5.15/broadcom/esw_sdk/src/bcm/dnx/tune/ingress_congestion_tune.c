...
/*
 * brief - get the dram bound threshold depending on the rate and credit request profile off-to-normal threshold.
    ...
    int *dram_bound_th)
{
    ...
    *dram_bound_th = 0;
    ...
    /*
     * when slow is disabled, off_to_normal is < 0. we want to get a positive threshold so we multiply by -1. 
     */
    factor = is_slow_enabled ? 1 : -1;

    if (rate <= 50)
    {
        *dram_bound_th = 3 * off_to_normal_th * factor;
    }
    else if (rate <= 400)
    {
        /** DRAM bound thr = 2.5 * off_to_normal */
        *dram_bound_th = 25 * off_to_normal_th / 10 * factor;
    }
    else
    /** rate > 400G */
    {
        /** DRAM bound thr = 1.5 * off_to_normal */
        *dram_bound_th = 15 * off_to_normal_th / 10 * factor;
    }
    ...
    bcm_cosq_rate_class_create_info_t * create_info)
{
    ...
    /*
     * DRAM Bound Parameters are calculated externally according to formulas described in src/examples/dnx/ingress_tm/cint_dram_bound_thresh.c
    ...
    bcm_dnx_cosq_gport_discard_set(unit, gport, cosq, &ecn_params);
    ...
    bcm_dnx_cosq_gport_discard_set(unit, gport, cosq, &wred_params);
    ...
    SHR_IF_ERR_EXIT(dnx_tune_ingress_congestion_dram_bound_threshold_get
                    (unit, create_info->rate, off_to_normal_th, is_slow_enabled, &dram_bound_bytes_th));
    ...
    /** DRAM Recovery fail thr = 1.1 * DRAM bound thr */
    dram_recovery_fail_th = 11 * dram_bound_bytes_th / 10;
    sal_memset(&dram_bound_threshold, 0, sizeof(dram_bound_threshold));
    ...
    /** DRAM recovery thr = 1.3 * off_to_notmal th */
    recovery_threshold.value = 13 * off_to_normal_th / 10;
