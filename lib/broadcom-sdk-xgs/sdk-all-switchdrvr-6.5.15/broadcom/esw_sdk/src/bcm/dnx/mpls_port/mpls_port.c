...
static shr_error_e
bcm_dnx_mpls_port_l2_vpn_check(
    int unit,
    bcm_vpn_t vpn)
{
    SHR_FUNC_INIT_VARS(unit);
...
*/
static shr_error_e
bcm_dnx_mpls_port_vpn_port_verify(
    int unit,
    bcm_vpn_t vpn,
    bcm_gport_t mpls_port_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(bcm_dnx_mpls_port_l2_vpn_check(unit, vpn));
    }
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    /** Check the learning information if Protection is enabled */
    ...
    if (mpls_port->failover_port_id != 0)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "encap_id, failover_mc_group and egress_tunnel_if have to be zero when failover_port_id is set as FEC. encap_id = 0x%08x, egress_tunnel_if = 0x%08x, failover_mc_group = 0x%08x\n",
                         mpls_port->encap_id, mpls_port->egress_tunnel_if, mpls_port->failover_mc_group);
        }
    }
    else if (mpls_port->failover_mc_group != 0)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "encap_id, failover_port_id and egress_tunnel_if have to be zero when failover_mc_group is set as MC group. encap_id = 0x%08x, egress_tunnel_if = 0x%08x\n",
                         mpls_port->encap_id, mpls_port->egress_tunnel_if);
        }
    }
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "egress_tunnel_if have to be zero or FEC. its value is 0x%08x\n", mpls_port->egress_tunnel_if);
        }
        if (mpls_port->encap_id > (dnx_data_lif.global_lif.nof_global_out_lifs_get(unit) - 1))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    {
        /** PWE tagged mode. Make sure nof_service_tags parameter isn't set. */
        if (mpls_port->nof_service_tags != 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "mpls_port.nof_service_tags(%d) should not be set for egress port.",
                         mpls_port->nof_service_tags);
        }
    }

    if (_SHR_IS_FLAG_SET(mpls_port->flags, BCM_MPLS_PORT_ENCAP_WITH_ID) &&
        (mpls_port->encap_id > (dnx_data_lif.global_lif.nof_global_out_lifs_get(unit) - 1)))
    {
    ...
    if ((mpls_port->egress_label.encap_access < bcmEncapAccessInvalid)
        || (mpls_port->egress_label.encap_access > bcmEncapAccessArp))
    {
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    supported_flags2 =
        ...
        BCM_MPLS_PORT2_CROSS_CONNECT | BCM_MPLS_PORT2_INTERFACE_NAMESPACE;
    ...
    if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_INGRESS_ONLY))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Available Ingress qos model for remark(%d), phb(%d), ttl(%d) are Invalid(SDK default), Pipe, ShortPipe and Uniform.",
                         mpls_port->ingress_qos_model.ingress_remark, mpls_port->ingress_qos_model.ingress_phb, mpls_port->ingress_qos_model.ingress_ttl);
        }
    }
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "EXP model only support bcmQosEgressModelUniform, bcmQosEgressModelPipeMyNameSpace and bcmQosEgressModelPipeNextNameSpace.");
        }
        if ((mpls_port->egress_label.egress_qos_model.egress_qos != bcmQosEgressModelPipeNextNameSpace) &&
            (mpls_port->egress_label.egress_qos_model.egress_qos != bcmQosEgressModelPipeMyNameSpace))
        {
    ...
    if (system_headers_mode != DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
    {
        ...
        if (_SHR_IS_FLAG_SET(mpls_port->flags2, BCM_MPLS_PORT2_INGRESS_ONLY))
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "In JR2 mode, If create ingress mpls port, egress_label.label (%d) must be INVALID",
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_mpls_port_vpn_port_verify(unit, vpn, mpls_port->mpls_port_id));
...
*/
static shr_error_e
bcm_dnx_mpls_port_delete_verify(
    int unit,
    bcm_vpn_t vpn,
    bcm_gport_t mpls_port_id)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_mpls_port_vpn_port_verify(unit, vpn, mpls_port_id));
...
*   shr_error_e - Non-zero in case of an error.
* \see
*   * \ref bcm_dnx_mpls_port_traverese
*/
static shr_error_e
bcm_dnx_mpls_port_traverse_verify(
    int unit,
    ...
    bcm_mpls_port_t * mpls_port)
{
     ...
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_mpls_port_add_verify(unit, vpn, mpls_port));
    ...
    bcm_gport_t mpls_port_id)
{
     ...
     * BCM level verification
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_mpls_port_delete_verify(unit, vpn, mpls_port_id));
    ...
    bcm_mpls_port_t * mpls_port)
{
     ...
     * BCM level verification
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_mpls_port_get_verify(unit, vpn, mpls_port));
    ...
    void *user_data)
{
     ...
     * Verify
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_mpls_port_traverse_verify(unit, additional_info, trav_fn));
