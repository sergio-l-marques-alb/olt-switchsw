    ...
    int unit)
{
     ...
     */
    create_data.flags =
        SW_STATE_ALGO_RES_CREATE_DUPLICATE_PER_CORE | SW_STATE_ALGO_RES_CREATE_USE_ADVANCED_ALGORITHM |
        SW_STATE_ALGO_RES_CREATE_INDEXED_RESOURCE;
    create_data.first_element = 0;
    create_data.nof_resource_pool_indexes = dnx_data_sch.flow.nof_regions_get(unit);
    ...
    sal_strncpy(create_data.name, DNX_SCH_ALLOC_MNGR_RESOURCE, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);

    /*
     * Create queues bitmap
     */
    SHR_IF_ERR_EXIT(sch_alloc_db.sch_flow_alloc_mngr.alloc(unit, SW_STATE_ALGO_RES_NOF_CORES_GET(unit),
    ...
    for (core_id = 0; core_id < SW_STATE_ALGO_RES_NOF_CORES_GET(unit); core_id++)
    {
        ...
        {
            create_data.core_id = core_id;
            create_data.sub_resource_index = sub_resource_id;
            SHR_IF_ERR_EXIT(sch_alloc_db.sch_flow_alloc_mngr.create
                            (unit, core_id, sub_resource_id, &create_data, NULL));
        }
    }
    ...
    dnx_algo_sch_alloc_info_t * alloc_info)
{
    ...
    switch (flow_type)
    {
        ...
        case DNX_SCH_ALLOC_FLOW_TYPE_COMP_HR:
            ...
            alloc_info->length = dnx_data_sch.sch_alloc.composite_se_get(unit, flow_type, odd_even_mode)->pattern_size;
    ...
    int *flow_id)
{
    ...
    allocate_with_id = SW_STATE_ALGO_RES_ALLOCATE_WITH_ID & flags;

    if (allocate_with_id)
    {
        region_index = DNX_SCH_REGION_INDEX_FROM_FLOW_INDEX_GET(*flow_id);
        SHR_IF_ERR_EXIT(dnx_scheduler_region_verify(unit, core, is_non_contiguous, nof_remote_cores, num_cos,
                                                    flow_type, region_index, &is_type_available_in_region));
