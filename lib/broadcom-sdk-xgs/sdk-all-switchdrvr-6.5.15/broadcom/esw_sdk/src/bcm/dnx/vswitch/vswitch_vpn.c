    ...
    uint8 value)
{
    ...
    SHR_MAX_VERIFY(vsi, (dnx_data_l2.vsi.nof_vsis_get(unit) - 1), _SHR_E_PARAM, "vsi out of range!\n");
 ...
 * \param [in] unit -
 *      Relevant unit.
 * \param [in] flags - SW_STATE_ALGO_RES_ALLOCATE_xxx \n
    ...
    uint8 *was_replaced)
{
    ...
    /** allocate new VSI if SW_STATE_ALGO_RES_ALLOCATE_WITH_ID isn't set */
    ...
    if (!is_vsi_allocated)
    {
        /** allocate VSI if not allocate with flag SW_STATE_ALGO_RES_ALLOCATE_WITH_ID */
        SHR_IF_ERR_EXIT(vlan_db.vsi.allocate_single(unit, flags, NULL, (int *) vsi));
        /*
         * Write to HW VSI table
         */
    ...
    if (used)
    {
        ...
        {
            /** replace when VSI is allocated and set flag SW_STATE_ALGO_RES_ALLOCATE_WITH_ID */
            *was_replaced = 1;
        }
    }
    else
