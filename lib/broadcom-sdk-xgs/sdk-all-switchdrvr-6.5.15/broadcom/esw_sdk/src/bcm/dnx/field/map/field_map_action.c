    ...
    bcm_field_action_t bcm_action)
{
    ...
    {
        /** Predefined action.*/
        if (ISEMPTY(bcm_action_text[bcm_action]))
        {
            return "Invalid BCM action";
        }
        return bcm_action_text[bcm_action];
    }
    else if (dnx_field_action_get_int(unit, bcm_action, &action_info, &dbal_field_id) == _SHR_E_NONE)
    {
        /** User defined action.*/
 ...
 */
char *
dnx_field_dnx_action_type_text(
    int unit,
    dnx_field_action_t dnx_action)
{
    dbal_field_types_basic_info_t *field_info;
    dnx_field_map_action_info_t dnx_action_info;
    dnx_field_stage_e stage = DNX_ACTION_STAGE(dnx_action);

    if (dnx_field_map_action_info(unit, stage, dnx_action, &dnx_action_info) != _SHR_E_NONE)
    {
        return "Invalid DNX Qualifier";
    }
    if (dbal_fields_field_types_info_get(unit, dnx_field_map_stage_info[stage].action_field_id,
                                         &field_info) != _SHR_E_NONE)
    {
        return "Invalid DBAL ID for DNX Action";
    }
    if (field_info->enum_val_info == NULL)
    {
        return "No Action Type Info";
    }
    return field_info->enum_val_info[DNX_ACTION_ID(dnx_action)].name;
}
    ...
    const dnx_field_action_map_t ** action_map_entry_p)
{
    ...
    if (DNX_FIELD_BCM_ACTION_IS_USER(unit, bcm_action))
    {
        ...
        if (stage != action_info_in.stage)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_PARAM);
            SHR_EXIT();
        }

        dnx_action = DNX_ACTION(DNX_FIELD_ACTION_CLASS_USER, stage, action_id);
    }
    else
    {
        /*
         * Now any action should be only static one.
         */
        DNX_FIELD_BCM_ACTION_VERIFY(bcm_action);
        /*
        ...
        if (dnx_global_action_map[bcm_action].dnx_action != 0)
        {
            ...
            dnx_action |= (stage << DNX_QUAL_STAGE_SHIFT);
        }
        else
        {
            /*
            ...
            if ((dnx_field_map_stage_info[stage].static_action_id_map == NULL) ||
                (dnx_field_map_stage_info[stage].static_action_id_map[bcm_action].dnx_action == 0))
            {
                SHR_SET_CURRENT_ERR(_SHR_E_PARAM);
                SHR_EXIT();
            }

            action_map_entry = &dnx_field_map_stage_info[stage].static_action_id_map[bcm_action];
            dnx_action =
                DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, stage,
                           dnx_field_map_stage_info[stage].static_action_id_map[bcm_action].dnx_action);
        }
    }
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_action_bcm_to_dnx_int(unit, stage, bcm_action, dnx_action_p, NULL),
                             "BCM Action %s(%d) is not supported for stage:%s",
                             dnx_field_bcm_action_text(unit, bcm_action), bcm_action,
    ...
    dnx_field_entry_action_t * dnx_action_info)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_action_bcm_to_dnx_int(unit, stage, bcm_action_info->type,
                                                                 &dnx_action_info->dnx_action, &action_map_entry),
                             "BCM Action %s(%d) is not supported for stage:%s",
    ...
    else
    {
        SHR_IF_ERR_EXIT_WITH_LOG(action_map_entry->conversion_cb
                                 (unit, core, bcm_action_info->value, dnx_action_info->action_value),
    ...
    int *dnx_action_nof_p)
{
    ...
    for (i_info = 0; i_info < dnx_field_map_stage_info[stage].static_action_nof; i_info++)
    {
        if (dnx_field_map_stage_info[stage].static_action_info[i_info].field_id != DBAL_FIELD_EMPTY)
        {
            ...
            dnx_action_list[i_action++] = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, stage, i_info);
        }
    }
    ...
    dnx_field_action_type_t * action_type_p)
{
    ...
    if (dnx_action_info.action_type == DNX_FIELD_ACTION_ID_INVALID)
    {
    ...
    switch (dnx_action_class)
    {
        ...
        case DNX_FIELD_ACTION_CLASS_STATIC:
            ...
            SHR_IF_ERR_EXIT(dbal_field_struct_field_size_get(unit,
                                                 ...
                                                 dnx_field_map_stage_info[stage].static_action_info[action_id].field_id,
                                                 &size));
            break;
        case DNX_FIELD_ACTION_CLASS_SW:
            if (action_id >= DNX_FIELD_SW_ACTION_NOF)
    ...
    dnx_field_action_type_t * action_type_p)
{
    ...
    *action_type_p = dnx_field_map_stage_info[dnx_stage].static_action_nof - 1;
    ...
    int unit)
{
     ...
     */
    {
        ...
        for (i_info = DNX_FIELD_SW_ACTION_FIRST; i_info < DNX_FIELD_SW_ACTION_NOF; i_info++)
        {
            ...
            {
                LOG_WARN(BSL_LOG_MODULE, (BSL_META("No DBAL field ID for sw action: %d\n"), i_info));
            }
        }
    }
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        else
        {
            ...
            DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage)
            {
                ...
                /*
                 * Check if this enum is enabled for this stage if not act as if it is not supported
                 * Legitimate situation - in different devices actual support map may be different
                 * Assign nof - zero
                 */
                if (dbal_fields_enum_hw_value_get(unit, dnx_field_map_stage_info[dnx_stage].action_field_id,
                                                  action_id, &hw_value) != _SHR_E_NONE)
                {
                    LOG_WARN(BSL_LOG_MODULE, (BSL_META("BCM Action %s(%d) not supported for stage:%s device:%s\n"),
                                              dnx_field_bcm_action_text(unit, i_bcm_act), i_bcm_act,
                                              dnx_field_stage_text(unit, dnx_stage), soc_dev_name(unit)));
                    base_action_map->dnx_action = 0;
                    continue;
                }
