    ...
    uint32 enable)
{
    ...
    SHR_RANGE_VERIFY(enable, 0, 2, _SHR_E_PARAM, "enable");
    ...
    bcm_sat_ctf_packet_info_t * packet_info)
{
     ...
     * CRC_OFFSETf
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_CRC_OFFSET, INST_SINGLE,
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ADD_END_TLV, INST_SINGLE, 1);
    }
    /*
     * ADD_CRCf
     */
    if (packet_info->flags & BCM_SAT_CTF_PACKET_INFO_ADD_CRC)
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ADD_CRC, INST_SINGLE, 1);
    }
    ...
    bcm_sat_ctf_packet_info_t * packet_info)
{
    ...
    SHR_RANGE_VERIFY(packet_info->sat_header_type, bcmSatHeaderUserDefined, bcmSatHeadersCount, _SHR_E_PARAM,
                     "sat_header_type");
    SHR_RANGE_VERIFY(packet_info->payload.payload_type, bcmSatPayloadConstant8Bytes, bcmSatPayloadsCount, _SHR_E_PARAM,
                     "payload_type");
    ...
    bcm_sat_ctf_report_config_t * reports)
{
    ...
    {
        field =
            ((reports->report_sampling_percent / DNX_SAT_CTF_REPORT_SAMPLING_PER_MAX) * (uint32) max_report_shreshold);
    }
    ...
    bcm_sat_ctf_stat_t * stat)
{
    ...
    stat->last_packet_seq_number = field - 1;
    stat->sec_in_avail_state_counter = field_avail_cnt;
    stat->max_packet_delay = field_max_delay;
    stat->min_packet_delay = field_min_delay;
    stat->last_packet_delay = field_last_delay;
    ...
    uint32 trap_data_mask)
{
    ...
    sal_memset(&psat_data, 0, sizeof(dnx_sat_data_t));
    SHR_IF_ERR_EXIT(dnx_sat_data_get(unit, &psat_data));
    SHR_IF_ERR_EXIT(dnx_sat_ctf_trap_entry_find(unit, &psat_data, trap_data, trap_data_mask, &entry_idx));
