    ...
    int unit)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "First dynamic Apptype %d, last static Apptype %d.\r\n",
    ...
    bcm_field_AppType_t * apptype_p)
{
    ...
    /*
     * Verify that the source static Apptype is legal.
     */
    if (apptype_info_p->base_static_apptype < 0 || apptype_info_p->base_static_apptype >= bcmFieldAppTypeCount)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "When creating a context for for stage external, we are actually creating an apptye. "
                     "The apptype must be based on a predefined static apptype with value no more than %d. "
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_apptype_to_opcode_bcm_to_dnx
                             ...
                             "When creating a context for for stage external, we are actually creating an apptye. "
                             "The apptype must be based on a predefined static apptype."
                             "Value for base Apptype is (in cascaded from) %d.%s%s\r\n",
                             apptype_info_p->base_static_apptype, EMPTY, EMPTY);

    /*
     * Verify the Apptype value if given WITH_ID, to provide a clearer error message than resource menager.
     */
    if (flags & DNX_FIELD_CONTEXT_FLAG_WITH_ID)
    {
    ...
    bcm_field_AppType_t * apptype_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_apptype_to_opcode_bcm_to_dnx(unit, apptype_info_p->base_static_apptype, &base_static_opcode_id));
    ...
    SHR_IF_ERR_EXIT(dnx_algo_field_entry_kbp_opcode_cs_profile_id_allocate(unit, 0, &cs_profile_id));
    ...
    SHR_IF_ERR_EXIT(dnx_field_key_ifwd2_acl_ffc_update_for_new_context(unit, base_fwd_context, acl_context));
    ...
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.info.base_apptype.set(unit, apptype_index, apptype_info_p->base_static_apptype));
    ...
    bcm_field_AppType_t * base_static_apptype_p)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Received Apptype %d. Function only supportd dynamic Apptypes.\r\n", 
                     dynamic_apptype);
    }
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    hash_info_p->hash_config.crc_select = DNX_FIELD_CONTEXT_HASH_ACTION_NOF;
    ...
    dnx_field_context_hash_info_t * hash_info_p)
{
    ...
    if (hash_info_p->hash_config.action_key != DBAL_NOF_ENUM_FIELD_HASH_LB_KEY_VALUES)
    {
        ...
        if (hash_info_p->hash_config.crc_select != DNX_FIELD_CONTEXT_HASH_ACTION_NOF)
        {
...
/**
 * \brief
 *  This function sets the ACL_RESOLUTION_ENABLE for a given context.
 *
 * \param [in] unit        - Device Id
 * \param [in] context_id  - Context ID to detach
 * \param [in] acl_res_enable    - Value to set
 *
 * \return
 *   shr_error_e - Error Type
 * \remark
...
static shr_error_e
dnx_field_context_ifwd2_context_properties_hw_set(
    int unit,
    int context_id,
    uint32 acl_res_enable)
{
    uint32 entry_handle_id;
    dbal_tables_e table_id = DBAL_TABLE_INGRESS_IRPP_FWD2_CONTEXT_PROPERTIES;
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD2_CONTEXT_ID, context_id);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACL_RESOLUTION_ENABLE, INST_SINGLE, acl_res_enable);
...
/**
 * \brief
 *  This function sets the acl context properties for all contexts
 *
 * \param [in] unit        - Device Id
 *
 * \return
 ...
 */
static shr_error_e
dnx_field_context_ifwd2_kbp_acl_result_init(
    int unit)
{
    dnx_field_context_t context_id;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_context_ifwd2_context_properties_hw_set(unit, context_id, TRUE));
    }
    ...
    int unit)
{
    ...
    {
        /*
         * Enable the resolution (result) for acl
         */
        SHR_IF_ERR_EXIT(dnx_field_context_ifwd2_kbp_acl_result_init(unit));
        /*
