...
#define DNX_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES    (18)
    ...
    dnxc_sr_cell_link_list_t * sr_link_list)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_topology_status_connectivity_get,
                                         (unit, route->hop_ids[0], route->hop_ids[0], &connectivity_map)));
    ...
    switch (route->number_of_hops)
    {
        ...
        case 1:
        {
            if (connectivity_map.link_info[(route->hop_ids[0])].far_dev_type == SOC_DNX_FAR_DEVICE_TYPE_FE1)
            {
                sr_link_list->dest_entity_type = dnxcFabricDeviceTypeFE1;
            }
            else if (connectivity_map.link_info[(route->hop_ids[0])].far_dev_type == SOC_DNX_FAR_DEVICE_TYPE_FE2)
            {
                sr_link_list->dest_entity_type = dnxcFabricDeviceTypeFE2;
            }
            else if (connectivity_map.link_info[(route->hop_ids[0])].far_dev_type == SOC_DNX_FAR_DEVICE_TYPE_FAP)
            {
    ...
    is_multi_stage =
        (connectivity_map.link_info[(route->hop_ids[0])].far_dev_type == SOC_DNX_FAR_DEVICE_TYPE_FE2) ? FALSE : TRUE;
    ...
    uint32 *nof_lines)
{
    ...
    if (max_nof_lines < DNX_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Bigger table is required\n");
    }

    sal_memcpy(parse_table, temp_table,
               sizeof(soc_dnxc_fabric_cell_parse_table_t) * DNX_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES);

    *nof_lines = DNX_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES;
    ...
    dnxc_vsc256_sr_cell_t * cell)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_sr_cell_parse_table_get
                    (unit, SOC_DNXC_FABRIC_CELL_PARSE_TABLE_MAX_NOF_LINES, parse_table, &nof_lines));

    SHR_IF_ERR_EXIT(soc_dnxc_fabric_cell_parser(unit, entry, parse_table, nof_lines, &cell_info));
    ...
    dnxc_vsc256_sr_cell_t * data_cell_sent)
{
     ...
     */
    SHR_BITCOPY_RANGE(&output_link_in_5b, 0, (uint32 *) &(data_cell_sent->header.fip_link), 0,
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LINK_ID, INST_SINGLE, output_link_in_5b);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    /*
...
{
    uint32 entry_handle_id;
    int i_sr_instance, sr_instance_size;
    uint32 fifo_status;
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, DBAL_TABLE_FABRIC_SR_CELL_RECEIVE_CONTROL,
                                               DBAL_FIELD_SR_CELL_INSTANCE, TRUE, 0, 0, &sr_instance_size));
    /*
     * Find which MAC CPU-FIFO is not empty.
     */
    for (i_sr_instance = 0; i_sr_instance < SAL_BIT(sr_instance_size); i_sr_instance++)
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_SR_CELL_INSTANCE, i_sr_instance);
 ...
 */
static int
bcm_dnx_fabric_route_rx_verify(
    int unit,
    uint32 flags,
    uint32 data_out_max_size,
    uint32 *data_out,
    ...
    uint32 *data_out_size)
{
     ...
     * Verify
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_fabric_route_rx_verify(unit, flags, data_out_max_size, data_out, data_out_size));
 ...
 */
static int
bcm_dnx_fabric_route_tx_verify(
    int unit,
    uint32 flags,
    ...
    uint32 *data_in)
{
     ...
     * verify
     */
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_fabric_route_tx_verify(unit, flags, route, data_in_size, data_in));

    SHR_IF_ERR_EXIT(dnx_fabric_sr_cell_route_2_link_list(unit, route, &sr_link_list));

    SHR_IF_ERR_EXIT(dnx_fabric_sr_cell_write(unit, &sr_link_list, sizeof(uint32) * data_in_size, data_in));
