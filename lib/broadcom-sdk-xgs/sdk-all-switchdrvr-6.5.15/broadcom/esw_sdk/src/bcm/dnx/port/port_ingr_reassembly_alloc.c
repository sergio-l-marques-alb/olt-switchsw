    ...
    switch (action)
    {
        ...
        case DNX_PORT_INGR_REASSEMBLY_PORT_DEALLOC:
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_master_get(unit, port,
                                                     DNX_ALGO_PORT_MASTER_F_NON_TDM | DNX_ALGO_PORT_MASTER_F_NEXT,
                                                     &next_master_port));
    ...
    *is_required = 0;
    ...
    switch (port_type)
    {
        case DNX_ALGO_PORT_TYPE_NIF_ILKN:
            ...
            {
                /*
                 * Dedicated TDM feature is not currently supported in Jer2
        ...
        case DNX_ALGO_PORT_TYPE_EVENTOR:
            /** allocate/deallocate always for special interfaces */
            *is_required = 1;
            break;
        ...
        case DNX_ALGO_PORT_TYPE_RCY:
            ...
            switch (action)
            {
                case DNX_PORT_INGR_REASSEMBLY_ALLOC:
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_port_master_get(unit, port, DNX_ALGO_PORT_MASTER_F_NON_TDM, &master_port));
                    *is_required = (port == master_port);
                    break;

                case DNX_PORT_INGR_REASSEMBLY_PORT_DEALLOC:
                    /** deallocate  context if this is the last port */
                    flags = DNX_ALGO_PORT_MASTER_F_NON_TDM | DNX_ALGO_PORT_MASTER_F_NEXT;
                    SHR_IF_ERR_EXIT(dnx_algo_port_master_get(unit, port, flags, &next_master_port));
                    *is_required = (next_master_port == DNX_ALGO_PORT_INVALID);
                    break;
                default:
                    SHR_ERR_EXIT(_SHR_E_PARAM, "Unexpected action %d", action);
        ...
        case DNX_ALGO_PORT_TYPE_RCY_MIRROR:
            /** Mirror port should not go through this function */
        default:
    ...
    uint32 *reassembly_context)
{
    ...
    {
        /** need to allocaate new context -- always allocate 2 */
        for (temp_prio = 0; temp_prio < dnx_data_ingr_reassembly.priority.rcy_priorities_nof_get(unit); temp_prio++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.rcy.
                            reassembly_context.set(unit, core_id, temp_prio, temp_context));
        }
    }

    SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.rcy.reassembly_context.get(unit, core_id, priority, reassembly_context));
    ...
    if (*is_deallocated)
    {
        ...
        for (temp_prio = 0; temp_prio < dnx_data_ingr_reassembly.priority.rcy_priorities_nof_get(unit); temp_prio++)
        {
            SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.rcy.reassembly_context.get(unit,
                                                                                 core_id, temp_prio, &temp_context));
            ...
            SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.rcy.reassembly_context.set(unit, core_id, temp_prio,
                                                                                 invalid_context));
        }
    }
    ...
    switch (port_type)
    {
        ...
        /** other port types */
            ...
            else
            {
                ...
                SHR_IF_ERR_EXIT(dnx_port_ingr_reassembly_reassembly_and_port_termination_get(unit,
                                                                                             master_port, priority,
                                                                                             reassembly_context,
                                                                                             &dummy));
            }
    }
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_reassembly_tdm_context_alloc(
    int unit,
    bcm_port_t port,
    uint32 *reassembly_context)
{
    ...
    /*
     * currently there are 256 different contexts, so there is no need to allocate.
     * if required, use allocation manager here
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_tm_port_get(unit, port, &core_id, &tm_port));
    *reassembly_context = tm_port;
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_reassembly_tdm_context_dealloc(
    int unit,
    bcm_port_t port,
    uint32 reassembly_context)
{
    ...
    int unit)
{
    ...
    sal_strncpy(data.name, PORT_INGR_REASSEMBLY_CONTEXT_ALLOC_MNG_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    /*
     * data.desc = "Ingress Reassembly Context";
     */
    SHR_IF_ERR_EXIT(dnx_ingress_reassembly_db.alloc.alloc(unit, SW_STATE_ALGO_RES_NOF_CORES_GET(unit)));
 ...
 */
shr_error_e
dnx_port_ingr_reassembly_res_mngr_deinit(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
    /*
