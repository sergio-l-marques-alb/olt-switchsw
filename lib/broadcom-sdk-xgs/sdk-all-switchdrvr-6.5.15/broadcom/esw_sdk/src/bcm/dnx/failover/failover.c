/** \file bcm/dnx/failover/failover.c
 * $Id$
 *
 ...
 */
/**
 * \brief - Set FEC hierarchy of the bank in protection path select fec_protection_pointer belongs to.
 *
 * \param [in] unit -  Unit-ID
 * \param [in] fec_protection_pointer - protection pointer to table FEC path select
 * \param [in] fec_hierarchy - FEC hierarchy
 * \return
 *   See shr_error_e
 * \remark
 *   * None
    ...
    dbal_enum_value_field_hierarchy_level_e fec_hierarchy)
{
    ...
    /** bank_id = pointer / nof_pointers_in_bank */
    bank_id =
...
/**
 * \brief - Get FEC hierarchy of the bank in protection path select fec_protection_pointer belongs to.
 *
 * \param [in] unit -  Unit-ID
 * \param [in] fec_protection_pointer - protection pointer to table FEC path select
 * \param [out] fec_hierarchy - FEC hierarchy
 * \return
 *   See shr_error_e
 * \remark
 *   * None
...
/**
 * \brief - Verify that protection_ptr is in the allowed range
 *
 * \param [in] unit -  Unit-ID
...
/**
 * \brief - Verify that protection_ptr is in the allowed range and allocated correctly
 *
 * \param [in] unit -  Unit-ID
 * \param [in] failover_id - the encoded type return by bcm_dnx_failover_create()
 * \return
 *   See shr_error_e
 * \remark
 *   * None
...
/**
 * \brief - Initialize FEC protection HW defaults:
 * * Invalid FEC protection pointer value
 * * Facility protection pointer value
 * * Enable/disable accelerated facility protection
 * * Initialize protection resolution logic
 *
 * \param [in] unit -  Unit-ID
 * \return
 *   See shr_error_e
    ...
    int unit)
{
    ...
    /*
     * last bank needs to be marked as no hirarchy in to be available for ECMP usage.
     */
    ...
    /** set facility protection pointer value  */
    ...
    /** set protection logic resolution */
    /* *INDENT-OFF* */
    /*
     * initialize fec protection logic. This logic determines according to the following table which of the 2
     * entries within the super fec is chosen.
     *
     * The protection logic is determined according to the following table (x means "don't care"):
     *
     *  +-------------------+---------+---------+-----------+-------------------+-------------+-----------------+------------------+--------------------------+------------------+--------+
     *  |                   |  L.left | L.right | Protected | Facility only fec | fec_ptr_lsb | left is sysport | right is sysport | enable path and facility | protection state | output |
     *  +-------------------+---------+---------+-----------+-------------------+-------------+-----------------+------------------+--------------------------+------------------+--------+
     *  | Not protected     |         |         |           |                   |             |                 |                  |                          |                  |        |
     *  |                   | x       | x       |         0 |                 0 | 0           | x               | x                | x                        | x                |      0 |
     *  |                   | x       | x       |         0 |                 0 | 1           | x               | x                | x                        | x                |      1 |
    ...
    /* *INDENT-ON* */
    {
        /** This value is computed according to the 'fec protection logic table' described above. */
    ...
    int unit)
{
     ...
     */
    /** allocate predefined protection bitmaps */
    no_protection_element = dnx_data_failover.path_select.ing_no_protection_get(unit);
    SHR_IF_ERR_EXIT(algo_failover_db.ing_path_select.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL,
    ...
    SHR_IF_ERR_EXIT(algo_failover_db.egr_path_select.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL,
                                                                     &no_protection_element));
    ...
    SHR_IF_ERR_EXIT(algo_failover_db.fec_path_select.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, &tag,
                                                                     &no_protection_element));
    ...
    /*
     *  Configure special pointer for FEC PATH Select
     */
    ...
    bcm_failover_t failover_id)
{
    ...
    /** verify protection pointer range */
...
/**
 * \brief -
 *    Set protection path of the specified failover-id (which encodes protection-type and protection pointer)
 *
 * \param [in] unit -  Unit-ID
 * \param [in] failover_id - encoded ID of protection pointer and protection pointer allocated by bcm_failover_create()
 * \param [in] enable - 1 - for primary or 0 - for secondary.
 * \return
 *   See shr_error_e
 * \remark
 *   * None
...
/**
 * \brief -
 *    Get protection path of the specified failover-id (which encodes protection-type and protection pointer)
 *
 * \param [in] unit -  Unit-ID
 * \param [in] failover_id - encoded ID of protection pointer and protection pointer allocated by bcm_failover_create()
 * \param [out] enable - 1 - for primary or 0 - for secondary.
 * \return
 *   See shr_error_e
 * \remark
 *   * None
    ...
    int *enable)
{
    ...
    /** To align with legacy primary path should be 1 and secondary 0 */
...
/**
 * \brief - verify input parameters for bcm_dnx_failover_create().
 *             Refer to bcm_dnx_failover_create() for details about the input parameters.
 */
static shr_error_e
    ...
    bcm_failover_t * failover_id)
{
    ...
    /** Verify single type specified and get type*/
    ...
    /** Verify single hierarchy selected */
    if (is_fec)
    {
...
/**
 * \brief -
 *  Allocate failover ID (protection pointer) for ingress / egress / FEC Protection.
 *  failover_id will be set to encode both the protection pointer and protection type
 *
 * \param [in] unit -  Unit-ID
 * \param [in] flags -
 *                  Supported flags:
 *                  BCM_FAILOVER_FEC - Allocate protection pointer for FEC
 *                  protection. Not allowed in a case of ECMP extended mode.
 *                  BCM_FAILOVER_INGRESS - Allocate protection pointer for ingress protection.
 *                  BCM_FAILOVER_ENCAP - Allocate protection pointer for egress protection.
 *                  BCM_FAILOVER_WITH_ID - Allocate protection pointer with id.
 *                  BCM_FAILOVER_FEC_2ND_HIERARCHY - allocate protection to be used by stage one in hierarchical FEC
 *                  BCM_FAILOVER_FEC_3RD_HIERARCHY - allocate protection to be used by stage two in hierarchical FEC
 *
 * \param [in,out] failover_id -
 *                  Input: The required failover_id - relevant in a case flag BCM_FAILOVER_WITH_ID is set.
 *                  Output: allocated failover_id encoded with required failover type.
 * \return
 *   See shr_error_e
 * \remark
 *   * None
    ...
    bcm_failover_t * failover_id)
{
    ...
    /*
     *  Set FEC hierarchy
     *  Always set the hierarchy - even if it is not the first element in the bank.
     */
    if (is_fec)
    {
    ...
    /*
     * Set default protection path to primary
     */
...
/**
 * \brief -
 *  De-allocate failover-id and reset protection path select value.
 *
 * \param [in] unit -  Unit-ID
 * \param [in] failover_id - The failover_id returned by bcm_failover_create()
 *
 * \return
 *   See shr_error_e
 * \remark
    ...
    bcm_failover_t failover_id)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    /*
     * Verify: no need - bcm_dnx_failover_set() will verify
     */

    /*
     * Reset protection path
     */
    SHR_IF_ERR_EXIT(bcm_dnx_failover_set(unit, failover_id, 1));
    ...
    if (failover_type == DNX_FAILOVER_TYPE_FEC)
    {
        ...
        /*
         * in a case that an element from the last bank is freed, nof elements in the bank need to be checked to determine
         * whether the whole bank need to be marked with no hierarchy. This is must to ensure that FEC will not access this bank
         * and only ECMP
         */
        ...
        {
            /** reset protection pointer if it the last one in the bank */
            ...
            {
                SHR_IF_ERR_EXIT(dnx_failover_fec_hierarchy_set
                                                (unit, protection_ptr, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1 /* default value */));
            }
        }
    }
    else if (failover_type == DNX_FAILOVER_TYPE_INGRESS)
    ...
    dnx_failover_l2_protection_info_t * protection_info)
{
    ...
    /** parse values according to calling L2 object type */
    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    /** Verify FEC protection */
    if (protection_info.is_fec_protected)
    {
        /** Can't support both mc_ingress and fec protection since both require to update learn information */
        ...
        /** FEC can be supported only at ingress path */
        if (is_ingress == FALSE)
        {
    ...
    int enable)
{
    ...
    /** translate gport into system port */
