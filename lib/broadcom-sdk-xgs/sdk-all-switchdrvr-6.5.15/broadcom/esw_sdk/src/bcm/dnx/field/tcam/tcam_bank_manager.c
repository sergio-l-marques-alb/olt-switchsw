    ...
    dnx_field_tcam_access_fg_params_t * fg_params)
{
    ...
    if (bank_id >= DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS)
    {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error allocating bank with mode 'OPEN' for a handler with bank allocation mode 'SELECT', bank ID: %d, handler ID: %d\r\n",
                     bank_id, handler_id);
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error allocating bank with mode 'RESTRICTED' for a handler with bank allocation mode 'AUTO', bank ID: %d, handler ID: %d\r\n",
                     bank_id, handler_id);
    }

    /** DT/non-DT section */
    if (fg_params->direct_table && bank_mode == DNX_FIELD_TCAM_BANK_MODE_RESTRICTED)
    {
        /*
         * DT handlers can't be added on restricted non-DT banks
         */
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error allocating bank: Direct Table field group (%d) can't allocate bank (%d) with non-DT mode\r\n",
                     handler_id, bank_id);
    }
    ...
    int banks_to_alloc[])
{
    ...
    if (nof_banks_to_alloc > DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Can't allocate more than %d banks. Number requested: %d\r\n",
                     DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS, nof_banks_to_alloc);
    }
    ...
    uint32 handler_id)
{
    ...
    if (fg_params.bank_allocation_mode == DNX_FIELD_TCAM_BANK_ALLOCATION_MODE_AUTO)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_bank_sw.tcam_banks.bank_mode.set(unit, bank_id, DNX_FIELD_TCAM_BANK_MODE_OPEN));
    }
    else
    {
        if (fg_params.direct_table)
    ...
    uint32 handler_id)
{
    ...
    if (handler_node != SW_STATE_LL_INVALID)
    {
        ...
        /*
         * Delete handler's prefix from bank
         */
        if (!fg_params.direct_table)
        {
    ...
    uint32 handler_id)
{
    ...
    for (bank_id = 0; bank_id < dnx_data_field.tcam.nof_banks_get(unit); bank_id++)
    {
         ...
         */
        SHR_IF_ERR_EXIT_EXCEPT_IF(dnx_field_tcam_bank_remove_handler_from_bank(unit, bank_id, handler_id),
                                  _SHR_E_NOT_FOUND);
    }
