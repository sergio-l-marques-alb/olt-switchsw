    ...
    bcm_switch_reflector_data_t * data)
{
    ...
    /** Set DATA fields */
    lif_get_flags = DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS | DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS;
    BCM_L3_ITF_LIF_TO_GPORT_TUNNEL(gport, data->next_encap_id);
    SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, gport, lif_get_flags, &gport_hw_resources), _SHR_E_NOT_FOUND, _SHR_E_PARAM);
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NEXT_OUTLIF_POINTER, INST_SINGLE, gport_hw_resources.local_out_lif);
 ...
 * \brief
 *  Get outlif data
 *  The table and data to sgt depends in result_type
 *  side.
 * \param [in] unit  - unit #.
 * \param [in] local_lif - local LIF #
    ...
    bcm_switch_reflector_data_t * data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif(unit,
            ...
            _SHR_CORE_ALL, next_local_lif, &next_global_lif_gport));
    ...
    bcm_if_t encap_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(reflector_sw_db_info.uc.encap_id.set(unit, -1));
    ...
    for (table_id = 0; table_id < nof_tables; table_id++)
    {
        ...
        while (!is_end)
        {
            ...
            if (local_out_lif != 0)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif(unit,
                        DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_EGRESS|DNX_ALGO_GPM_GPORT_HW_RESOURCES_NON_STRICT,
                        _SHR_CORE_ALL, local_out_lif, &next_gport));
