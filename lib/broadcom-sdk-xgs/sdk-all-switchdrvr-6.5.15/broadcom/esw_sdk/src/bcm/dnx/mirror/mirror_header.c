 ...
 * \param [in] mirror_dest_id - Mirror destination gport
 * \param [in] mirror_header_info   - mirror header information
 * \param [in] total_len_in_bits  - Maximum supported system header size in bits.
 * \param [out] header_size - Constructed system header size in bytes.
 * \param [in/out] system_header - System header buffer.
 * \param [out] stacking_ext_offset - Point to the start of stacking extension in the buffer.
 * \param [out] ase_ext_offset   - Point to the start of ASE extension in the buffer.
 * \param [out] tsh_offset   - Point to the start of TSH in the buffer.
 * \param [in] fhei_stamp_en   - FHEI stamp enabled.
 * \param [out] fhei_offset   - Point to the end of FHEI in the buffer.
 *
 * \return
 *   \retval Non-zero in case of an error
 *   \retval Zero in case of NO ERROR
    ...
    uint32 *fhei_offset)
{
    ...
    /** FTMH.visibility */
    DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, ftmh, visibility, 1);
    ...
    add_dsp_ext = soc_property_get(unit, spn_STACKING_ENABLE, 0x0) || dnx_data_headers.ftmh.add_dsp_ext_get(unit);
    ...
    */
    ...
    while ((pp->out_vport_ext[lif_index] != 0) 
        && (lif_index < (sizeof(pp->out_vport_ext)/sizeof(pp->out_vport_ext[0]))))
    {
        ++lif_ext_type;
        ++lif_index;
    }
    ...
    DNX_MIRROR_SYSTEM_HEADER_FIELD_SET(unit, internal, parsing_start_type, DBAL_ENUM_FVAL_LAYER_TYPES_ETHERNET);
    ...
    if (lif_ext_type > 0)
    {
        ...
        switch (lif_ext_type)
        {
            case 3:
                ...
                SHR_IF_ERR_EXIT(utilex_bitstream_set_field(system_header, offset+(lif_ext_type-3)*lif_ext_size, lif_ext_size, gport_hw_resources.global_out_lif));
                /* coverity[fallthrough:FALSE] */
            case 2:
                /** Convert gport to outlif */
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                ...
                SHR_IF_ERR_EXIT(utilex_bitstream_set_field(system_header, offset+(lif_ext_type-2)*lif_ext_size, lif_ext_size, gport_hw_resources.global_out_lif));
                /* coverity[fallthrough:FALSE] */
            case 1:
                /** Convert gport to outlif. Outlif 1 in MSB */
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources
                ...
                SHR_IF_ERR_EXIT(utilex_bitstream_set_field(system_header, offset+(lif_ext_type-1)*lif_ext_size, lif_ext_size, gport_hw_resources.global_out_lif));
                break;
            default:
                break;
        }
    ...
    {    
        ...
        for (udh_index=0; udh_index<sizeof(mirror_header_info->udh)/sizeof(mirror_header_info->udh[0]); ++udh_index)
        {
    ...
    uint32 *fhei_offset)
{
    ...
    if (add_dsp_ext)
    {
        ...
        {
            SHR_IF_ERR_EXIT(utilex_bitstream_set_field(system_header, offset,
                                DNX_MIRROR_SYSTEM_HEADER_DATA_GET(unit, ftmh_legacy, dsp_ext_header_bits), dest));
        }
    }
    ...
    bcm_mirror_header_info_t * mirror_header_info)
{
    ...
    dbal_enum_value_field_ftmh_app_specific_ext_type_e ase_type = DBAL_ENUM_FVAL_FTMH_APP_SPECIFIC_EXT_TYPE_NONE;
    ...
    */
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ITPP_DELTA, INST_SINGLE, header_size);
    ...
    if (fhei_offset > 0)
    {
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_PPH_FHEI_EXT_OFFSET, INST_SINGLE,
                                    fhei_offset);
    }
    ...
    bcm_mirror_header_info_t * mirror_header_info)
{
    ...
    if (lif_ext_type > 0)
    {
        ...
        {
            case 3:
                SHR_IF_ERR_EXIT(utilex_bitstream_get_field(system_header, offset+(lif_ext_type-3)*lif_ext_size, lif_ext_size, &val));
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                (unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS, _SHR_CORE_ALL, val, &pp->out_vport_ext[2]));
                /* coverity[fallthrough:FALSE] */
            case 2:
                SHR_IF_ERR_EXIT(utilex_bitstream_get_field(system_header, offset+(lif_ext_type-2)*lif_ext_size, lif_ext_size, &val));
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                (unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS, _SHR_CORE_ALL, val, &pp->out_vport_ext[1]));
                /* coverity[fallthrough:FALSE] */
            case 1:
                SHR_IF_ERR_EXIT(utilex_bitstream_get_field(system_header, offset+(lif_ext_type-1)*lif_ext_size, lif_ext_size, &val));
                SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                                (unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS, _SHR_CORE_ALL, val, &pp->out_vport_ext[0]));
                break;
            default:
                break;
        }
