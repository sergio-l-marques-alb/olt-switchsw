...
#define DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX 0x3FF
#define DNX_FABRIC_RCI_UNINSTALL_THRESHOLD_MAX 0xFF
    ...
    switch (probability_info->probability_type)
    {
        case bcmFabricRciDevBiasingModeDiscard:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                ...
                threshold[rci_level + 1] =
                    UTILEX_DIV_ROUND_UP(probability[rci_level] * DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX, 100);
            }
            break;
        case bcmFabricControlRciThrottlingModeDeviceBiasing:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                 ...
                 */
                threshold[rci_level + 1] =
                    DNX_FABRIC_RCI_UNINSTALL_THRESHOLD_MAX -
                    UTILEX_DIV_ROUND_UP(probability[rci_level] * DNX_FABRIC_RCI_UNINSTALL_THRESHOLD_MAX, 100);
            }
            break;
        case bcmFabricRciFlowBiasingModeUninstall:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                 ...
                 */
                threshold[rci_level + 1] =
                    DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX -
                    UTILEX_DIV_ROUND_UP(probability[rci_level] * DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX, 100);
            }
            break;
    ...
    switch (probability_info->probability_type)
    {
        case bcmFabricRciDevBiasingModeDiscard:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                 ...
                 */
                probability[rci_level] =
                    UTILEX_DIV_ROUND(threshold[rci_level + 1] * 100, DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX);
            }
            break;
        case bcmFabricControlRciThrottlingModeDeviceBiasing:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                 ...
                 */
                probability[rci_level] =
                    100 - UTILEX_DIV_ROUND(threshold[rci_level + 1] * 100, DNX_FABRIC_RCI_UNINSTALL_THRESHOLD_MAX);
            }
            break;
        case bcmFabricRciFlowBiasingModeUninstall:
            for (rci_level = 0; rci_level < BCM_FABRIC_NUM_OF_RCI_LEVELS; rci_level++)
            {
                 ...
                 */
                probability[rci_level] =
                    100 - UTILEX_DIV_ROUND(threshold[rci_level + 1] * 100, DNX_FABRIC_RCI_DISCARD_THRESHOLD_MAX);
            }
            break;
