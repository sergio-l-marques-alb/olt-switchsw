...
#include <soc/dnx/swstate/auto_generated/access/meter_access.h>
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    if ((meter_profile_info->disable_eir == FALSE) || (meter_profile_info->is_coupling_enabled == TRUE) ||
        ((meter_profile_info->is_sharing_enabled == TRUE) && (global_sharing == TRUE)))
    {
    ...
    /*
     * In packet mode the values are artifically divided by _DPP_POLICER_PACKET_MODE_CORRECTION and so must be scaled
     * back to user values 
     */
    ...
    dnx_meter_profile_info_t * meter_profile_info)
{
    ...
    {
        meter_profile_info->cir = min_rate;
    ...
    {
        meter_profile_info->eir = min_rate;
    ...
    /** max_eir can exist also when disable_eir = TRUE, because if coupling/(sharing+global_sharing) is enabled the bucket gets extra credits from cir */
    if ((meter_profile_info->disable_eir == FALSE) || (meter_profile_info->is_coupling_enabled == TRUE) ||
        (pol_cfg_internal.mode == bcmPolicerModeCoupledCascade))
    {
    ...
    dnx_meter_profile_info_t * profile_info)
{
    ...
    data.flags = SW_STATE_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE | SW_STATE_ALGO_TEMPLATE_CREATE_DUPLICATE_PER_CORE;
    ...
    sal_strncpy(data.name, DNX_POLICER_INGRESS_DB0_TEMPLATE_NAME, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(dnx_meter_db.policer_templates.policer_ingress_db0.create(unit, &data, NULL));

    sal_strncpy(data.name, DNX_POLICER_INGRESS_DB1_TEMPLATE_NAME, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(dnx_meter_db.policer_templates.policer_ingress_db1.create(unit, &data, NULL));

    sal_strncpy(data.name, DNX_POLICER_INGRESS_DB2_TEMPLATE_NAME, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(dnx_meter_db.policer_templates.policer_ingress_db2.create(unit, &data, NULL));

    /** create 2 egress template managers. one for each database */
    sal_strncpy(data.name, DNX_POLICER_EGRESS_DB0_TEMPLATE_NAME, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(dnx_meter_db.policer_templates.policer_egress_db0.create(unit, &data, NULL));

    sal_strncpy(data.name, DNX_POLICER_EGRESS_DB1_TEMPLATE_NAME, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(dnx_meter_db.policer_templates.policer_egress_db1.create(unit, &data, NULL));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_policer_default(unit, &pol_cfg));
    SHR_IF_ERR_EXIT(dnx_policer_to_meter_inter_info(unit, &pol_cfg, &meter_profile_info));

    SHR_IF_ERR_EXIT(dnx_meter_global_init(unit, &meter_profile_info));
    SHR_IF_ERR_EXIT(dnx_meter_generic_init(unit, &meter_profile_info));
    ...
    dnx_algo_template_print_t * print_cb_data)
{
    ...
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_STRING, NULL, "meter profile info:", NULL, NULL);
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "disable_cir", profile_info->disable_cir, NULL,
                                        "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "disable_eir", profile_info->disable_eir, NULL,
                                        "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "cir", profile_info->cir, NULL, "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "eir", profile_info->eir, NULL, "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "max_cir", profile_info->max_cir, NULL,
                                        "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "max_eir", profile_info->max_eir, NULL,
                                        "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "cbs", profile_info->cbs, NULL, "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "ebs", profile_info->ebs, NULL, "0x%x");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "color_mode", profile_info->color_mode, NULL,
                                        "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "is_coupling_enabled",
                                        profile_info->is_coupling_enabled, NULL, "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "is_sharing_enabled",
                                        profile_info->is_sharing_enabled, NULL, "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "is_packet_mode",
                                        profile_info->is_packet_mode, NULL, "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "is_pkt_truncate",
                                        profile_info->is_pkt_truncate, NULL, "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "ir_rev_exp",
                                        profile_info->ir_rev_exp, NULL, "%d");
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "profile_data",
    ...
    bcm_policer_expansion_group_t * expansion_group)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id1, DBAL_FIELD_FWD_TYPE_FILTER_BITMAP, INST_SINGLE, &filter_bitmap);
    /** Preforming the action */
...
*   mode1: change_profile=FALSE, perfom the following steps:
...
*
*   mode2: change_profile=TRUE, perfom the following steps:
    ...
    int change_profile)
{
    ...
    /** change_profile only valid with new_profile !=0. user can't change profile 0 becuase it is the default one */
    if (change_profile && (new_profile == 0))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "change_profile is only valid in combination with new_profile\n");
    }

    /** get the profile id from hw that are used by the given meter id */
    SHR_IF_ERR_EXIT(dnx_meter_generic_profile_id_get(unit, core, is_ingress, database_id, meter_idx, &old_profile));

    /** User wants to change the profile data without changing the meters pointers */
    if (change_profile)
    {
        SHR_IF_ERR_EXIT(POLICER_MNGR_TM_CB
                        (is_ingress, database_id, replace_data(unit, core, new_profile, (void *) meter_profile_info)));
        first_reference = TRUE;
    }
    else
    {
        SHR_IF_ERR_EXIT(POLICER_MNGR_TM_CB(is_ingress, database_id,
                                           exchange(unit, core, 0, meter_profile_info, old_profile, NULL, &new_profile,
    ...
    bcm_policer_config_t * pol_cfg)
{
    ...
    int i, meter_incr, profile, nof_ref;
    ...
    {
        new_profile = pol_cfg->entropy_id; /** Expect non-zero only if  BCM_POLICER_REPLACE_SHARED flag*/
    ...
    bcm_policer_database_config_t * config)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_policer_mgmt_nof_expansion_groups_get(unit, core_id, &nof_expansion_groups));
    ...
    int *nof_expansion_groups)
{
    ...
    int offset[bcmPolicerFwdTypeMax] = { FALSE };
    int fwd_type_idx;
    int nof_groups = 0;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_policer_mgmt_expansion_groups_get(unit, 0, core_id, &expansion_groups));

    /** count for each meter offset, how many valid fwd-types use it */
    for (fwd_type_idx = bcmPolicerFwdTypeUc; fwd_type_idx < bcmPolicerFwdTypeMax; fwd_type_idx++)
    {
        if (expansion_groups.config[fwd_type_idx].valid == TRUE)
        {
            if (offset[expansion_groups.config[fwd_type_idx].offset] == FALSE)
            {
                nof_groups++;
            }
            offset[expansion_groups.config[fwd_type_idx].offset] = TRUE;
        }
    }
    ...
    bcm_policer_database_attach_config_t * config)
{
    ...
    DNXCMN_CORES_ITER(unit, database_core, core_idx)
    {
        ...
        if (single_bucket_bank == FALSE)
        {
            ...
            bank_id = DNX_CRPS_METER_SHMEM_BANK_ID_CALC(unit, engine->engine_id, section);
        ...
        dnx_meter_generic_engine_clear(unit, core_idx, engine, database_id, is_ingress, single_bucket_bank);
    }
    ...
    for (is_ingress_idx = FALSE; is_ingress_idx <= TRUE; is_ingress_idx++)
    {
        ...
        for (database_idx = 0; database_idx < nof_databases; database_idx++)
        {
            ...
            for (bank_idx = 0; bank_idx < nof_banks; bank_idx++)
            {
                ...
                calc_bank_id = DNX_CRPS_METER_SHMEM_BANK_ID_CALC
                    (unit, engine->engine_id, (dnx_crps_mgmt_shmem_section_t) engine->section);
            ...
            for (entry_ptr_map_table = 0; entry_ptr_map_table <= index_field_info.max_value; entry_ptr_map_table++)
            {
                ...
                if (bank_id_to_remove == hw_bank_id)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_meter_generic_ptr_map_hw_set(unit, core_idx, is_ingress, database_id,
                                                                     ...
                                                                     DNX_METER_PTR_MAP_INVALID_BANK));
                    break;
                }
            }
...
shr_error_e
dnx_policer_mgmt_ingress_port_compensation_delta_verify(
    int unit,
    int delta)
{
    ...
    int delta)
{
    ...
    /** verify delta value */
    SHR_INVOKE_VERIFY_DNX(dnx_policer_mgmt_ingress_port_compensation_delta_verify(unit, delta));
    ...
    void *user_data)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_idx)
    {
                    ...
                    if (single_bucket_bank == FALSE)
                    {
                        ...
                        for (bank_idx = 0; bank_idx < nof_banks; bank_idx++)
                        {
                            ...
                            for (meter_idx = object_stat_pointer_base;
                                 meter_idx < dnx_data_meter.mem_mgmt.object_stat_pointer_base_resolution_get(unit);
                                 meter_idx++)
                            {
        ...
        if (is_enabled == TRUE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_policer_mgmt_nof_expansion_groups_get(unit, core_idx, &nof_expansion_groups));
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_meter_global_fairness_set(unit, enable));
    SHR_IF_ERR_EXIT(dnx_meter_generic_fairness_set(unit, enable));
...
{
    SHR_FUNC_INIT_VARS(unit);
    /** get data only from core-0 and generic meter database-0. It should be the same for all meters. */
    SHR_IF_ERR_EXIT(dnx_meter_generic_fairness_get(unit, 0, 0, enable));
...
dnx_policer_mgmt_meter_index_get(
    ...
    int *meter_idx_size)
{
    uint32 db_created, expansion_enable;
    int i, valid = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (db_created == TRUE)
    {
        ...
        {
            /** get how many groups. the function can be called only if expansion is enabled */
            SHR_IF_ERR_EXIT(dnx_policer_mgmt_nof_expansion_groups_get(unit, core_id, meter_idx_size));
        }
