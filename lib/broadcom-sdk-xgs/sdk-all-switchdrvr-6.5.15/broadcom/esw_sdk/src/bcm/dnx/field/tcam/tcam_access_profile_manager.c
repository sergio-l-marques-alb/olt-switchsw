...
dnx_field_tcam_access_profile_intersect(
    ...
    uint8 *intersect)
{
    int bank_ids1[dnx_data_field.tcam.nof_banks_get(unit)];
    int banks_count1;
    int bank_ids2[dnx_data_field.tcam.nof_banks_get(unit)];
    int banks_count2;
    int ii = 0;
    int jj = 0;
    ...
    *intersect = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_banks_sorted_get(unit, handler_id1, bank_ids1, &banks_count1));
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_banks_sorted_get(unit, handler_id2, bank_ids2, &banks_count2));

    while (ii < banks_count1 && jj < banks_count2)
    {
        if (bank_ids1[ii] == bank_ids2[jj])
        {
            *intersect = TRUE;
            break;
        }
