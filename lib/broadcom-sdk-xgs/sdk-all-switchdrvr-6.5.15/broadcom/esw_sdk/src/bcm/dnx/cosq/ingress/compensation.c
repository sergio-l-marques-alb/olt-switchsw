    ...
    int delta)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_compensation_db.
                    compensation.exchange(unit, BCM_CORE_ALL, 0, &delta, old_comp_profile, NULL, &new_comp_profile,
                                          &is_first, &is_last));
    ...
    if (is_last)
    {
        /** configure old profile compensation to default compensation */
        SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_voq_delta_hw_set(unit, old_comp_profile, default_compensation));
    }
...
*/
shr_error_e
dnx_cosq_ingress_compensation_scheduler_per_port_enable_verify(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq)
{
    ...
    if (gport != 0)
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Invalid port %x. Only 0 is allowed.", gport);
    }

    if (cosq != 0 && cosq != -1)
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Invalid cosq %d. Only 0/-1 is allowed.", cosq);
    }
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(dnx_cosq_ingress_compensation_scheduler_per_port_enable_verify(unit, gport, cosq));
    ...
    int enable)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_cosq_ingress_compensation_scheduler_per_port_enable_set_verify(unit,
                                                                                             gport, cosq, enable));
    ...
    int *enable)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_cosq_ingress_compensation_scheduler_per_port_enable_verify(unit, gport, cosq));
...
/**
* \brief
* Get total Scheduler compensation for last enqueued packet
 * \param [in] unit -  Unit-ID 
 * \param [in] core_id - Core-ID
 * \param [in] delta - obtained last packet total compensation
 *             
 * \return
 *   See shr_error_e 
 * \remark
...
*/
shr_error_e
dnx_cosq_ingress_compensation_scheduler_last_packet_compensation_get(
    int unit,
    int core_id,
    int *delta)
{
    uint32 compensation_field_value;
    int field_size;
    ...
    /** Taking a handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_COMPENSATION_FOR_LAST_PACKET, &entry_handle_id));

    /** Set Key Field  */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);

    /** Setting pointers value to receive the fields */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TOTAL_COMPENSATION, INST_SINGLE,
                               &compensation_field_value);

    /** Performing the action */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, DBAL_TABLE_INGRESS_COMPENSATION_FOR_LAST_PACKET,
                                               DBAL_FIELD_TOTAL_COMPENSATION, 0, 0, 0, &field_size));

    *delta = UTILEX_TWO_COMPLEMENT_INTO_SIGNED_NUM(compensation_field_value, field_size);
    ...
    dnx_algo_template_print_t * print_cb_data)
{
    ...
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_INT, "Compensation Delta", *delta, NULL, NULL);
    ...
    int unit)
{
    ...
    sal_strncpy(create_data.name, "VOQ map Ingress Compensation profile",
