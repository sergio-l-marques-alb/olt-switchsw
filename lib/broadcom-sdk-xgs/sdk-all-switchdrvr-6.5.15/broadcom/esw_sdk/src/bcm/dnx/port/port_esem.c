    ...
    dnx_esem_cmd_data_t * esem_cmd_data)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.profile_data_get
                    (unit, _SHR_CORE_ALL, esem_cmd, &ref_count, esem_cmd_data));
    ...
    if (is_phy_port == FALSE)
    {
        ...
        else
        {
            ...
            if ((tag_bmp != DNX_ALGO_ESEM_ACC_CMD_RANGE_0_3) &&
                ...
                (tag_bmp != DNX_ALGO_ESEM_ACC_CMD_RANGE_60_63) &&
                (tag_bmp != DNX_ALGO_ESEM_ACC_CMD_RANGE_0_63))
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Calculated tag bitmap (0x%04X) is not valid. port 0x%08X, "
                             ...
                             tag_bmp, port, dbal_result_type_to_string(unit, gport_hw_resources.outlif_dbal_table_id, gport_hw_resources.outlif_dbal_result_type),
                             table_field_info.field_nof_bits, table_field_info.arr_prefix_size, table_field_info.arr_prefix);
            }
    ...
    *esem_if = ESEM_ACCESS_IF_1;
    ...
    switch (access_type)
    {
        ...
        case ESEM_ACCESS_TYPE_ETH_NATIVE_AC:
        {
             ...
             * If it is used for native AC, outer AC should be created explictly in EEDB,
 ...
 * \param [in] port - Physical port or out-lif-id is gport format.
 ...
 *
 * \param [in] unit - the relevant unit.
 * \param [in] flags - flags, SW_STATE_ALGO_TEMPLATE_ALLOCATE_*.
    ...
    uint8 *is_last)
{
    ...
    if (flags & SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID)
    {
        ...
        rv = algo_port_pp_db.esem.access_cmd.profile_get(unit, _SHR_CORE_ALL, esem_cmd_data, &tmp_esem_cmd);
    ...
    uint8 *is_last)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_port_esem_cmd_algo_exchange_verify
                          (unit, flags, esem_cmd_data, esem_cmd_tags, old_esem_cmd, new_esem_cmd, is_first, is_last));
    ...
    extra_alloc_info.resource_flags = DNX_ALGO_RES_SIMPLE_BITMAP_ALLOC_IN_RANGE;
    grain_size = dnx_data_esem.access_cmd.nof_cmds_get(unit) / DNX_ALGO_ESEM_CMD_RES_NOF_TAGS;
    ...
    if (with_id)
    {
        ...
        SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                        (unit, _SHR_CORE_ALL, flags, esem_cmd_data, tmp_esem_cmd, (void *)&extra_alloc_info, new_esem_cmd, is_first, is_last));
    }
    else
    {
        ...
        for (tag = 0; tag < nof_avail_tags; tag++)
        {
            extra_alloc_info.resource_alloc_info.range_start = sorted_tags[tag] * grain_size;
            extra_alloc_info.resource_alloc_info.range_end = sorted_tags[tag] * grain_size + grain_size;
    ...
    int unit)
{
     ...
     */
    flags = SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID;

    /** Allocate the cmd ID*/
    sal_memset(&esem_cmd_data, 0, sizeof(dnx_esem_cmd_data_t));
    esem_cmd_default = dnx_data_esem.access_cmd.no_action_get(unit);
    esem_cmd_alloc = esem_cmd_default;
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_algo_exchange(unit, flags, &esem_cmd_data, DNX_ALGO_ESEM_ACC_CMD_RANGE_0_3,
    ...
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_algo_exchange(unit, flags, &esem_cmd_data, DNX_ALGO_ESEM_ACC_CMD_RANGE_60_63,
                                                    esem_cmd_default, &esem_cmd_alloc, &is_first, &is_last));
    ...
    esem_cmd_data.esem[ESEM_ACCESS_IF_2].designated_offset = 0;
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_algo_exchange(unit, flags, &esem_cmd_data, DNX_ALGO_ESEM_ACC_CMD_RANGE_0_7,
                                                    esem_cmd_default, &esem_cmd_alloc, &is_first, &is_last));
    ...
    esem_cmd_data.esem[ESEM_ACCESS_IF_1].default_result_profile =
        dnx_data_esem.default_result_profile.default_ac_get(unit);
    esem_cmd_data.esem[ESEM_ACCESS_IF_1].designated_offset = 0;
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_algo_exchange(unit, flags, &esem_cmd_data, DNX_ALGO_ESEM_ACC_CMD_RANGE_0_63,
                                                    esem_cmd_default, &esem_cmd_alloc, &is_first, &is_last));
    ...
    uint8 *is_last)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_info_collect(unit, flags, port, esem_app_db, access_type,
                                                   esem_default_result_profile, old_esem_cmd, esem_cmd_data));
     ...
     */
    SHR_IF_ERR_EXIT(dnx_port_esem_cmd_algo_exchange(unit, 0, esem_cmd_data, esem_cmd_tags, old_esem_cmd,
    ...
    uint8 *is_last)
{
    ...
    *is_last = FALSE;
    if (free_needed == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_esem_cmd_data_sw_get(unit, tmp_esem_cmd, &esem_cmd_data));
        
        SHR_IF_ERR_EXIT(algo_port_pp_db.esem.access_cmd.exchange
                        (unit, _SHR_CORE_ALL, 0, &esem_cmd_data, esem_cmd, NULL, &tmp_esem_cmd, &is_first, is_last));
