    ...
    uint32 *ch0_out_remainder)
{
    ...
    pll_type = DNX_PLL_TYPE_NIF_0 + pll_index;
    ...
    SHR_IF_ERR_EXIT(imb_diag_phy_measure_pll_calc(unit, ref_clk, n_div, p_div, m0_div, ch0_out_int, ch0_out_remainder));
    ...
    uint32 *ch0_out_remainder)
{
    ...
    pll_type = DNX_PLL_TYPE_FABRIC_0 + pll_index;
    ...
    soc_dnxc_port_phy_measure_t * phy_measure)
{
     ...
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_synce_enable_get(unit, cdu_info.core, &synce_enable_default));
    synce_state_read = 1;
    /*
     * Enable syncE
     */
    SHR_IF_ERR_EXIT(soc_dnx_nif_synce_enable_set(unit, cdu_info.core, 1));
    /*
     * Save syncE configuration to restore at end of function 
     */
    SHR_IF_ERR_EXIT(imb_diag_synce_config_get(unit, cdu_info.core, cdu_info.cdu_id, &synce_default));
    synce_config_read = 1;
    /*
     * Config syncE
     */
    synce_config.clk_sel = swapped_lane % dnx_data_nif.eth.cdu_lanes_nof_get(unit);
    ...
    SHR_IF_ERR_EXIT(imb_diag_synce_config_set(unit, cdu_info.core, cdu_info.cdu_id, &synce_config));
    /*
     * Enable ECI Gtimer
     */
    SHR_IF_ERR_EXIT(dnx_gtimer_set(unit, IMB_DIAG_GTIMER_NOF_CYCLES, SOC_BLK_ECI, -1));
    /*
     * Wait Gtimer until it reset 
     */
    SHR_IF_ERR_EXIT(dnx_gtimer_wait(unit, SOC_BLK_ECI, -1));
     ...
     * Read syncE counter 
     */
    SHR_IF_ERR_EXIT(imb_diag_synce_counter_get(unit, cdu_info.core, &synce_cnt));
    /*
     ...
     * get VCO divider
     */
    vco_div = dnx_data_nif.phys.vco_div_get(unit);
    /*
...
exit:
    ...
    {
        if ((_func_rv = dnx_gtimer_clear(unit, SOC_BLK_ECI, -1)) < 0)
        {
            cli_out("write to ECI_GTIMER_CONFIGURATIONr failed\n");
        }
        if (synce_state_read)
        {
            if ((_func_rv = soc_dnx_nif_synce_enable_set(unit, cdu_info.core, synce_enable_default)) < 0)
            {
        ...
        {
            if ((_func_rv = imb_diag_synce_config_set(unit, cdu_info.core, cdu_info.cdu_id, &synce_default)) < 0)
            {
