...
dnx_algo_consistent_hashing_calendar_calculate(
    ...
    consistent_hashing_calendar_t * calendar)
{
    int current_members_array[max_nof_members_in_calendar];
    int wanted_members_array[max_nof_members_in_calendar];
    int delta_array[max_nof_members_in_calendar];
    int nof_needed_changes;
    SHR_FUNC_INIT_VARS(unit);
    ...
    sal_memset(wanted_members_array, 0, sizeof(wanted_members_array));
    /** fill calendar completely */
    SHR_IF_ERR_EXIT(dnx_algo_chc_fully_populate
                    (unit, nof_entries_in_calendar, max_nof_members_in_calendar, calendar, current_members_array));
    ...
    if (max_nof_members_in_calendar > current_nof_members_in_calendar)
    {
        ...
        else
        {
            ...
            for (int member = 0; member < current_nof_members_in_calendar; ++member)
            {
                ...
                wanted_members_array[wanted_member] = 1;
            }
        }
        /** get delta from ideal destribution */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_from_ideal_destribution_get
                        (unit, current_nof_members_in_calendar, current_members_array, wanted_members_array,
                         nof_entries_in_calendar, max_nof_members_in_calendar, delta_array, &nof_needed_changes));

        /** equilize calander */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_equilize
                        (unit, nof_needed_changes, max_nof_members_in_calendar, nof_entries_in_calendar,
                         wanted_members_array, delta_array, calendar));
    }
exit:
    SHR_FUNC_EXIT;
}
...
dnx_algo_consistent_hashing_calendar_member_add(
    ...
    consistent_hashing_calendar_t * new_calendar)
{
    int members_array[max_nof_members_in_calendar];
    int delta_array[max_nof_members_in_calendar];
    int nof_changes;
    uint32 nof_unique_members;
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_chc_count_entries_per_member
                        (unit, max_nof_members_in_calendar, current_calendar, nof_entries_in_calendar,
                         members_array, &nof_unique_members));
        /** verify member to add is not already found in calander */
        if (members_array[member_to_add] != 0)
        {
        ...
        *new_calendar = *current_calendar;
        ...
        /** overrun entries in original place with member_to_add */
        {
            ...
            {
                /** decrease count of entries in members_array */
                --members_array[new_calendar->lb_key_member_array[entry]];
            ...
            members_array[member_to_add] = nof_entries_to_overrun;
        }
        /** get delta from ideal entries to members destribution */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_from_ideal_destribution_get
                        (unit, nof_unique_members, members_array, members_array, nof_entries_in_calendar,
                         max_nof_members_in_calendar, delta_array, &nof_changes));

        /** equilize calander */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_equilize
                        (unit, nof_changes, max_nof_members_in_calendar, nof_entries_in_calendar,
                         members_array, delta_array, new_calendar));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    consistent_hashing_calendar_t * new_calendar)
{
    ...
    if (members_array[member_to_remove] == 0)
    {
    ...
    *new_calendar = *current_calendar;
    ...
    sal_memcpy(wanted_members_array, members_array, sizeof(members_array));
    wanted_members_array[member_to_remove] = 0;
    ...
    if (nof_unique_members != 0)
    {
        /** get delta from ideal entries to members destribution */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_from_ideal_destribution_get
                        (unit, nof_unique_members, members_array, wanted_members_array, nof_entries_in_calendar,
                         max_nof_members_in_calendar, delta_array, &nof_changes));
        /** equilize calendar */
        SHR_IF_ERR_EXIT(dnx_algo_chc_delta_equilize
                        (unit, nof_changes, max_nof_members_in_calendar, nof_entries_in_calendar,
                         wanted_members_array, delta_array, new_calendar));
    }
exit:
    SHR_FUNC_EXIT;
}
