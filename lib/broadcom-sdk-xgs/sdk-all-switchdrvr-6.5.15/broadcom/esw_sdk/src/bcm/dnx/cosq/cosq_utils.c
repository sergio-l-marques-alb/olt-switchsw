...
/* { */
  /*
   * Remove this procedure. Not required at this level.
   */
int
  dnx_fap_port_id_verify(
    int          unit,
    dnx_fap_port_id_t  port_id
  )
{
    SHR_FUNC_INIT_VARS(unit);
    if (port_id > DNX_MAX_FAP_PORT_ID)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
            "port_id (%d) is larger than max allowed (%d)", port_id, DNX_MAX_FAP_PORT_ID);
    }
exit:
    SHR_FUNC_EXIT;
}
