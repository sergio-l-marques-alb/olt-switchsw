    ...
    uint8 *is_first_sit_profile_reference)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.egress_pp_port_sit_profile.allocate_single
                    (unit, _SHR_CORE_ALL, 0, sit_profile_info, NULL, &value, is_first_sit_profile_reference));
    ...
    uint8 *is_last)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.egress_pp_port_sit_profile.free_single(unit, _SHR_CORE_ALL, sit_profile, is_last));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.egress_pp_port_sit_profile.allocate_single
                    (unit, _SHR_CORE_ALL, SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID, &sit_profile_info, NULL,
                     &sit_profile, &is_first));
    SHR_IF_ERR_EXIT(dnx_port_sit_profile_hw_set(unit, sit_profile, &sit_profile_info));
    ...
    *is_other_set = 0;
    ...
    PBMP_E_ITER(unit, port_i)
    {
        if (port_i != port)
        {
            ...
            else if (port_type == DNX_PORT_TYPE_PON)
            {
    ...
    uint32 *key)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_PRIORITY, INST_SINGLE, is_use));
    }
    if (pp_port)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field16_get
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, entry_handle_id, DBAL_FIELD_SYSTEM_PORT, INST_SINGLE, is_use));
    }
    if (key)
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    int value)
{
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_PP_PORT, INST_SINGLE, ptc);
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_VT_PTC_PROFILE, INST_SINGLE, ptc_profile);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    if (!enable)
    {
        SHR_IF_ERR_EXIT(dnx_port_sit_profile_is_used(unit, sit_profile, &is_sit_profile_used));
        if (!is_sit_profile_used)
        {
            SHR_IF_ERR_EXIT(dnx_port_sit_profile_free(unit, sit_profile, &is_last));
            SHR_IF_ERR_EXIT(dnx_port_sit_profile_hw_clear(unit, sit_profile));
        }
    }
    ...
    if (!enable)
    {
        ...
        BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
        {
            SHR_IF_ERR_EXIT(dnx_port_sit_virtual_port_access_id_get_by_port(unit, local_port, &access_id));
            /** clear TCAM entry */
