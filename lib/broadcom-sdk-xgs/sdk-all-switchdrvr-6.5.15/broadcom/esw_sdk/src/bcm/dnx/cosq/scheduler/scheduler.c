...
static int dnx_scheduler_port_hr_add(
    int unit,
    int port);
    ...
    int unit)
{
    ...
    {
        /** configure/add HR for the port */
        SHR_IF_ERR_EXIT(dnx_scheduler_port_hr_add(unit, port));
        ...
        SHR_IF_ERR_EXIT(dnx_scheduler_port_shapers_init(unit, port));
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
       ...
       /** This function is not invoked under warm boot */
        SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate(unit, core, nof_remote_cores, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID,
                                                    FALSE, DNX_COSQ_RESERVED_FLOW_NOF,
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_convert_se_to_flow_id(unit, dnx_data_sch.flow.hr_se_id_min_get(unit), &first_hr_flow_id));
    ...
    int unit)
{
    ...
    if (dnx_data_sch.hw_workarounds.feature_get(unit, dnx_data_sch_hw_workarounds_dlm))
    {
        /** HW bug WA for Scheduler DLM */
    ...
    int unit)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        for (region_indx = dnx_data_sch.sch_alloc.type_con_reg_end_get(unit) + 1;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_init_toggle(unit));
    ...
    bcm_gport_t * gport)
{
    uint32 allowed_flags =
        BCM_COSQ_GPORT_VOQ_CONNECTOR | BCM_COSQ_GPORT_COMPOSITE | BCM_COSQ_GPORT_NON_CONTIGUOUS_VOQ_CONNECTOR |
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate(unit, core, nof_remote_cores, res_flags,
                                                is_non_contiguous, numq, flow_type, src_modid, &flow_id));
    ...
    bcm_gport_t gport)
{
    ...
    int actual_flow_id[dnx_data_device.general.max_nof_cores_get(unit)];
    ...
    bcm_cos_queue_t cosq)
{
    ...
    else if (BCM_COSQ_GPORT_IS_COMPOSITE_SF2(gport))
    {
        ...
        is_connector = (_SHR_COSQ_GPORT_RESERVED_TYPE_GET(gport) == _SHR_COSQ_GPORT_RESERVED_VOQ_CONNECTOR);
 ...
 */
static shr_error_e
dnx_scheduler_gport_dettach_verify_config(
    int unit,
    int child_core,
    int child_flow_id,
    int parent_core,
 ...
 */
shr_error_e
dnx_scheduler_connector_gport_detach(
    int unit,
    bcm_gport_t sched_port,
    bcm_gport_t input_port,
    bcm_cos_queue_t cosq)
{
    ...
    /** Verify configuration */
    SHR_INVOKE_VERIFY_DNX(dnx_scheduler_gport_dettach_verify_config(unit, child_core, child_flow_id,
                                                                    core, parent_flow_id));
    ...
    bcm_gport_t * parent_port)
{
    ...
    if (parent_se_id == sw_state_default_se)
    {
        ...
        if (is_child_se)
        {
            ...
            {
                /** FMQ root */
                SHR_IF_ERR_EXIT(dnx_scheduler_is_flow_fmq_class(unit, core, child_flow_id,
                                                                &is_fmq_class, &fmq_class_indx));
                if (is_fmq_class)
                {
                    /*
                     * FMQ classes are 1-based
    ...
    int weight)
{
    ...
    /** update the data */
    cl_template_data.weight.weight[weight_ap] = weight;
    ...
    int *weight)
{
    ...
    *weight = cl_weight.weight[weight_ap];
    ...
    int max_burst)
{
    ...
    max_burst_in_credit_worth = UTILEX_DIV_ROUND_UP(max_burst, credit_worth);

    /** get flow id and core */
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_core_flow_id_get(unit, port, cosq, &core, &flow_id));

    /** 2 flows per remote core are written inside this function */
    ...
    int *max_burst)
{
    ...
    *max_burst = max_burst_in_credit_worth * credit_worth;
    ...
    uint32 round_up)
{
    ...
    max_rate_in_mega_bits_per_sec = (credit_worth * UTILEX_NOF_BITS_IN_BYTE) *
        DNXCMN_CORE_CLOCK_MHZ_GET(unit) / dnx_data_sch.general.cycles_per_credit_token_get(unit);
    ...
    else
    {
        ...
        if (((peak_rate_man & 0x1) == 1) && (peak_rate_exp == 15))
        {
            peak_rate_man = UTILEX_DIV_ROUND_UP(peak_rate_man, 2);
            peak_rate_exp--;
        }
    ...
    uint32 *rate_in_kbits_per_sec)
{
    ...
    else
    {
        ...
        if (!shaper_weight)
        {
            *rate_in_kbits_per_sec = max_rate_in_mega_bits_per_sec * 1000;
        }
        else
        {
            /*
             * intermediate_value = max_rate_in_mega_bits_per_sec * shaper_weight_denominator / shaper_weight_numerator, aka
             * shaper_weight_numerator / shaper_weight_denominator = max_rate_in_mega_bits_per_sec / intermediate_value
             *
             * intermediate_value is the rate represented by mantissa-exponent
             */

            utilex_u64_multiply_longs(max_rate_in_mega_bits_per_sec, shaper_weight_denominator, &intermediate_val);
            utilex_u64_devide_u64_long(&intermediate_val, shaper_weight, &intermediate_val);
            ...
            {
                *rate_in_kbits_per_sec = intermediate_val.arr[0] * 1000;
            }
        }
    }
    ...
    int slow_rate)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_slow_profile_rate_hw_set(unit, core, slow_rate_profile, level,
                                                     slow_rate_mantissa, slow_rate_exp));
exit:
    SHR_FUNC_EXIT;
    ...
    uint32 flags)
{
    ...
    int flow_id, actual_flow_id[dnx_data_device.general.max_nof_cores_get(unit)];
    ...
    bcm_gport_t * gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_flow_allocate(unit, core, 1, alloc_flags, FALSE, 1, flow_type, -1, &flow_id));
    ...
    uint32 *flags)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_scheduler_connector_gport_get_verify(unit, core, base_flow_id));
    ...
    BCM_GPORT_MODPORT_SET((*physical_port), remote_fap_id, 0);

    /** update parameters */
    (*flags) = BCM_COSQ_GPORT_VOQ_CONNECTOR;
    (*flags) |= (is_composite) ? BCM_COSQ_GPORT_COMPOSITE : 0;
    ...
    (*flags) |= (is_interdigitated) ? BCM_COSQ_GPORT_NON_CONTIGUOUS_VOQ_CONNECTOR : 0;
 ...
 */
static int
dnx_scheduler_gport_tcg_verify(
    int unit,
    bcm_gport_t gport,
    int tcg)
{
    ...
    if (tcg >= nof_priorities)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "cosq is out of range. Should be in range of 0-%d. port: %d cosq %d\n",
                     nof_priorities - 1, logical_port, tcg);
    }
...
/* 
 * \brief -
 * verify dnx_scheduler_gport_e2e_port_tcg_sched_set and get common parameters
 */
int
dnx_scheduler_gport_e2e_port_tcg_sched_verify(
    int unit,
    bcm_gport_t gport,
    bcm_cos_queue_t cosq)
{
    ...
    if (!BCM_COSQ_GPORT_IS_E2E_PORT_TCG(gport))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "E2E TCG gport is expected. gport: 0x%x\n", gport);
    }

    SHR_IF_ERR_EXIT(dnx_scheduler_gport_tcg_verify(unit, gport, cosq));
    ...
    int weight)
{
    ...
    SHR_IF_ERR_EXIT(dnx_scheduler_gport_e2e_port_tcg_sched_verify(unit, gport, cosq));
    ...
    uint32 flags)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid parameter: kbits_sec_max %d is bigger than maximal port rate %d\n",
    ...
    uint32 max_burst)
{
    ...
    if (max_burst > dnx_data_sch.ps.max_burst_get(unit))
    {
    ...
    int port)
{
    ...
    for (priority_i = 0; priority_i < nof_priorities; priority_i++)
    {
        ...
        if (port_type == DNX_ALGO_PORT_TYPE_ERP)
        {
            /** the HRs are pre-allocated; need to configure only */
            SHR_IF_ERR_EXIT(dnx_scheduler_element_gport_configure(unit, &params, flow_id));
        }
        else
        {
    ...
    uint32 flags)
{
    ...
    SHR_MASK_VERIFY(flags, BCM_COSQ_BW_NOT_COMMIT, _SHR_E_PARAM, "Unsupported flag provided\n");
    ...
    uint32 *hr_id)
{
    ...
    uint32 flags = 0;
    int flow_id[dnx_data_sch.general.nof_fmq_class_get(unit)];
