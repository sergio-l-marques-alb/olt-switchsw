...
#define L2_ADDR_SUPPORTED_BCM_L2_FLAGS  (BCM_L2_STATIC | BCM_L2_MCAST | BCM_L2_TRUNK_MEMBER | BCM_L2_DISCARD_DST | BCM_L2_HIT)
...
/** Trunk ID mask */
#define L2_TRUNK_ID_MASK 0xffffff
 /**
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (!DNX_VSI_VALID(unit, l2addr->vid))
    {
    ...
    {
        /*
         * Verify if the trunk ID value is valid and in range
         */
        if ((dnx_trunk_t_verify(unit, l2addr->tgid) != _SHR_E_NONE) && (dnx_trunk_t_verify(unit, l2addr->port) != _SHR_E_NONE))
        {
          SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid trunk member! port or tgid should have a valid trunk ID!");
        }
    }
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "table ID must be DBAL_TABLE_FWD_MACT or DBAL_TABLE_FWD_MACT_IVL.");
    }
    /*
     * Get key fields
     */
    ...
    if ((((result_type == DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_NO_OUTLIF) ||
        ...
        ||
        (((result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_NO_OUTLIF)
          || (result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST_STAT)
          || (result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_SINGLE_OUTLIF)
          || (result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST_LIF_STAT)
          || (result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_EEI_FEC))
         && table_id == DBAL_TABLE_FWD_MACT_IVL))
    {
        ...
        else if (dbal_sub_field == DBAL_FIELD_LAG_ID)
        {
            ...
            SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_from_forward_information(unit, &trunk_gport,
                                                                              ...
                                                                              &forward_info, 1));
            l2addr->tgid = (BCM_GPORT_TRUNK_GET(trunk_gport) & L2_TRUNK_ID_MASK);
    ...
    uint32 *eei)
{
    ...
    *destination = forward_info.destination;
        ...
        *outlif = forward_info.outlif;
        ...
        if (l2addr->modid)
        {
            *result_type = (l2addr->stat_pp_profile != STAT_PP_PROFILE_INVALID) ?
                DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST_LIF_STAT :
                DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_SINGLE_OUTLIF;
        }
        else
        {
            *result_type = (l2addr->stat_pp_profile != STAT_PP_PROFILE_INVALID) ?
    ...
    else if (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_ONLY)
    {
        ...
        {
            *result_type = (l2addr->stat_pp_profile != STAT_PP_PROFILE_INVALID) ?
                DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST_STAT :
                DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_NO_OUTLIF;
        }
        else
        {
            *result_type = (l2addr->stat_pp_profile != STAT_PP_PROFILE_INVALID) ?
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (l2addr->modid)
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FWD_MACT_IVL, &entry_handle_id));
    ...
    if (((l2addr->modid == 0) && ((result_type == DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_SINGLE_OUTLIF) ||
                                  (result_type == DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_DEST_LIF_STAT))) ||
        ((l2addr->modid != 0) &&
         ((result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_SINGLE_OUTLIF) ||
          (result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_DEST_LIF_STAT))))
    ...
    if ((result_type == DBAL_RESULT_TYPE_FWD_MACT_IVL_FWD_MACT_RESULT_EEI_FEC)
        || (result_type == DBAL_RESULT_TYPE_FWD_MACT_FWD_MACT_RESULT_EEI_FEC))
    {
     ...
     * Statistics enabled
     */
    if (((l2addr->modid == 0) && dnx_stat_pp_result_type_verify(unit, DBAL_TABLE_FWD_MACT, result_type) == _SHR_E_NONE)
        || ((l2addr->modid != 0)
            && dnx_stat_pp_result_type_verify(unit, DBAL_TABLE_FWD_MACT_IVL, result_type) == _SHR_E_NONE))
    {
    ...
    int *age_state_in_seconds)
{
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, learning_profile_id, &learning_profile_ref_count,
                     (void *) &learning_profile_info));

    SHR_IF_ERR_EXIT(algo_l2_db.vsi_aging_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, learning_profile_info.aging_profile, &aging_profile_ref_count,
    ...
    if (aging_profile_info.ageout_age != AGING_PROFILE_INVALID_AGE)
    {
        ...
        if ((nof_age_states_until_age_out < 0) || (nof_age_states_until_age_out >= MACT_MAX_AGE))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if (hit_bit)
    {
        /** If the hit bit indication is active then the flag BCM_L2_HIT needs to be set */
...
/**
 * \brief - Set the key fields in FWD_MACT/FWD_MACT_IVL tables.
 *
 * \param [in] unit - Relevant unit
