    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgGlemPpTrap, trap_gport_drop));
    SHR_IF_ERR_EXIT(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgGlemNonePpTrap, trap_gport_drop));
    ...
    SHR_IF_ERR_EXIT(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgIpv4Error, trap_gport_drop));
    SHR_IF_ERR_EXIT(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgL4Error, trap_gport_drop));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                    (unit, BCM_RX_TRAP_UPDATE_DEST, bcmRxTrapStpStateLearn,
                     dnx_data_trap.strength.default_trap_strength_get(unit), BCM_GPORT_BLACK_HOLE, 0, 0));
    /** In case of bcmRxTrapL2DiscardMacsaFwd, do nothing except for creating the trap type */
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init(unit, 0, bcmRxTrapL2DiscardMacsaFwd, 0, 0, 0, 0));
    /** In case of bcmRxTrapL2DiscardMacsaDrop, drop the packets. */
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                    (unit, BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_LEARN_DISABLE, bcmRxTrapL2DiscardMacsaDrop,
                     dnx_data_trap.strength.default_trap_strength_get(unit), BCM_GPORT_BLACK_HOLE, 0, 0));
    /** In case of bcmRxTrapL2DiscardMacsaTrap, redirect the packets to CPU */
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                    (unit, BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_LEARN_DISABLE, bcmRxTrapL2DiscardMacsaTrap,
                     dnx_data_trap.strength.default_trap_strength_get(unit), BCM_GPORT_BLACK_HOLE,
                     dnx_data_trap.strength.default_snoop_strength_get(unit), DNX_RX_SNOOP_CMD_TO_CPU));
    /** In case of bcmRxTrapL2DiscardMacsaSnoop, copy the packets to CPU*/
    SHR_IF_ERR_EXIT(dnx_tune_trap_ingress_configure_init
                    (unit, BCM_RX_TRAP_LEARN_DISABLE, bcmRxTrapL2DiscardMacsaSnoop,
                     0, 0, dnx_data_trap.strength.default_snoop_strength_get(unit), DNX_RX_SNOOP_CMD_TO_CPU));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_type_create(unit, 0, bcmRxTrap1588Discard, &trap_id));
    sal_memset(&config, 0, sizeof(config));
    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_get(unit, trap_id, &config));

    config.trap_strength = dnx_data_trap.strength.max_strength_get(unit);
    config.flags = BCM_RX_TRAP_UPDATE_DEST;
    config.dest_port = BCM_GPORT_BLACK_HOLE;

    SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_set(unit, trap_id, &config));
exit:
    SHR_FUNC_EXIT;
}
