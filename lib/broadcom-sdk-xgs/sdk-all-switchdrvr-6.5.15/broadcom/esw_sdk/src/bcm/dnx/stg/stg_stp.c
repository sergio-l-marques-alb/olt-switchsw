    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_stg_general_info_set(unit, &stg_min, &stg_max, NULL, NULL));
     ...
     */
    SHR_IF_ERR_EXIT(bcm_dnx_stg_create_id(unit, stg_defl));

    /*
     * Update the STG general information.
     */
    stg_count = 1;
    SHR_IF_ERR_EXIT(dnx_stg_general_info_set(unit, NULL, NULL, &stg_defl, &stg_count));

    /*
     * Check the legality of the definitions.
     */
    SHR_IF_ERR_EXIT(dnx_stg_general_info_get(unit, &stg_min, &stg_max, &stg_defl, &stg_count));
    assert(stg_defl >= stg_min && stg_defl <= stg_max && stg_min <= stg_max);

    /*
     * Add the default VLAN to the default Spanning Tree Group.
     */
    SHR_IF_ERR_EXIT(dnx_stg_vlan_map_add(unit, stg_defl, BCM_VLAN_DEFAULT));
