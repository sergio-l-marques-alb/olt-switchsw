    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    if (endpoint_info->ccm_period == 0)
    {
        ...
        dnx_oam_oamp_ccm_interval_to_ccm_period(entry_values->ccm_interval, (int *) &(ccm_period_ms));
    ...
    if (dnx_data_oam.oamp.rmep_db_non_zero_lifetime_units_limitation_get(unit))
    {
        ...
        if (lifetime_units != DBAL_ENUM_FVAL_LIFETIME_UNITS_SCAN_PERIOD_1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Non-zero lifetime units are not supported. Lower CCM period expected.\n");
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dnx_oam_rmep_punt_profile_temp_data_t * hw_write_data)
{
    ...
    SHR_IF_ERR_EXIT(algo_oam_db.oamp_punt_event_prof_template.free_single(unit, _SHR_CORE_ALL, punt_profile, &is_last));
    ...
    dnx_oam_rmep_punt_profile_temp_data_t * hw_write_data)
{
    ...
    if (is_update)
    {
        SHR_IF_ERR_EXIT(algo_oam_db.oamp_punt_event_prof_template.exchange
                        (unit, _SHR_CORE_ALL, 0, &punt_profile_data, old_profile_id, NULL, &profile_id, &write_hw,
                         &is_last));
    ...
    else
    {
        SHR_IF_ERR_EXIT(algo_oam_db.oamp_punt_event_prof_template.allocate_single
                        (unit, _SHR_CORE_ALL, 0, &punt_profile_data, NULL, &profile_id, &write_hw));
    }
    ...
    dnx_oam_oamp_rmep_db_entry_t * oamp_rmep_db_entry)
{
    ...
    /*
     * oamp_rmep_db_entry.punt_good_profile = 
     */
    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    dnx_oam_remote_endpoint_hw_entry_struct_set(unit, endpoint_info, punt_profile, &oamp_rmep_entry);

    /*
     * Do the punt profile HW configuration 
     */
