...
#define DNX_ALGO_GLOBAL_LIF_TAG_LIMITED_RANGE_IGNORE    1
    ...
    uint32 direction_flags)
{
    ...
    if (_SHR_IS_FLAG_SET(direction_flags, DNX_ALGO_LIF_EGRESS))
    {
        ...
        if (count >= dnx_data_lif.global_lif.nof_glem_lines_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "No more egress global lifs available. In use %d out of %d.",
                         count, dnx_data_lif.global_lif.nof_glem_lines_get(unit));
        }
    }
    ...
    int global_lif)
{
     ...
     */
    with_id = (!is_allocation) || _SHR_IS_FLAG_SET(allocation_flags, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID);
    ...
    int *global_lif)
{
    ...
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        egress_global_lif.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, global_lif));
    }

    if (_SHR_IS_FLAG_SET(direction_flags, DNX_ALGO_LIF_INGRESS))
    {
        SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.
                        ingress_global_lif.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, global_lif));
    }
    ...
    uint32 alloc_flags)
{
    ...
    res_tag_bitmap_create_info.count = create_data->nof_elements + dnx_data_l3.rif.nof_rifs_get(unit);
    res_tag_bitmap_create_info.grain_size = dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit);
    ...
    /*
     * Set the tags - tag 0 for limited range, tag 1 for everything else.
     */
    SHR_IF_ERR_EXIT(sw_state_resource_tag_bitmap_tag_set
                    (unit, module_id, *res_tag_bitmap, DNX_ALGO_GLOBAL_LIF_TAG_LIMITED_RANGE_USE, FALSE, 0,
                     dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit)));

    SHR_IF_ERR_EXIT(sw_state_resource_tag_bitmap_tag_set
                    (unit, module_id, *res_tag_bitmap, DNX_ALGO_GLOBAL_LIF_TAG_LIMITED_RANGE_IGNORE, FALSE,
                     dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit),
                     create_data->nof_elements - dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit)));
    ...
    int *element)
{
    ...
    {
        res_tag_bitmap_alloc_info.flags |=
            SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_WITH_ID | SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_IGNORE_TAG;
    }

    res_tag_bitmap_alloc_info.flags |= SW_STATE_RESOURCE_TAG_BITMAP_ALWAYS_CHECK_TAG;
...
dnx_algo_global_lif_allocation_init(
    int unit)
{
    sw_state_algo_res_create_data_t global_lif_data;
    ...
    else
    {
        ...
        resource_flags = SW_STATE_ALGO_RES_CREATE_USE_ADVANCED_ALGORITHM;
    }
     ...
     */
    sal_memset(&global_lif_data, 0, sizeof(sw_state_algo_res_create_data_t));
    ...
    sal_strncpy(global_lif_data.name, DNX_ALGO_GLOBAL_LIF_INGRESS_RESOURCE, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    /*
