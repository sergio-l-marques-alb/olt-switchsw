    ...
    dnx_sch_flow_credit_source_cos_data_t * hr_cos)
{
    ...
    if (temp_cos >= first_reserved_value || hr_mode == DBAL_ENUM_FVAL_HR_MODE_ENHANCED)
    {
        last_id_to_check =
        ...
        for (reserved_id = DBAL_ENUM_FVAL_COS_HR_EF1; reserved_id <= last_id_to_check; reserved_id++)
        {
            ...
            if (temp_cos == reserved_value)
            {
                temp_cos = dnx_sch_hr_reserved_mapping[reserved_id];
                break;
            }
        }
    }
    ...
    int *weight)
{
    ...
    switch (hr_cos->type)
    {
        ...
        case DBAL_RESULT_TYPE_SCH_FLOW_CREDIT_SRC_COS_HR_WFQ_WEIGHT:
            if (hr_mode == DBAL_ENUM_FVAL_HR_MODE_ENHANCED)
            {
