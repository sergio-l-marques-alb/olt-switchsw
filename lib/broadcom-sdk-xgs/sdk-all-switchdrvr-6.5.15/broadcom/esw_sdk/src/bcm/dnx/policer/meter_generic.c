    ...
    dnx_meter_profile_info_t * profile_info)
{
    ...
    if (profile_info->is_sharing_enabled)
    {
        ...
        if (profile_info->disable_cir && profile_info->disable_eir
            && min_rate == profile_info->max_cir && min_rate == profile_info->max_eir)
        {
        ...
        else
        {
            ...
            if (profile_info->max_cir != DNX_METER_UTILS_MAX_RATE_UNLIMITED)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_sand_break_complex_to_mnt_exp_round_down_2(profile_info->max_cir,
                                                                               max_ir_mant,
    ...
    SHR_IF_ERR_EXIT(dnx_sand_break_complex_to_mnt_exp_round_down_2(profile_info->cir,
                                                                   max_ir_mant,
                                                                   DNX_METER_UTILS_RATE_MANT_MIN_VAL,
                                                                   max_ir_exp,
                                                                   core_clock_freq,
    ...
    SHR_IF_ERR_EXIT(dnx_sand_break_complex_to_mnt_exp_round_down_2(profile_info->eir,
                                                                   max_ir_mant,
                                                                   DNX_METER_UTILS_RATE_MANT_MIN_VAL,
                                                                   max_ir_exp,
                                                                   core_clock_freq,
    ...
    {         /** not max eir unlimited */
        SHR_IF_ERR_EXIT(dnx_sand_break_complex_to_mnt_exp_round_down_2(max_eir_for_calc,
                                                                       max_ir_mant,
                                                                       DNX_METER_UTILS_RATE_MANT_MIN_VAL,
                                                                       max_ir_exp,
                                                                       core_clock_freq,
    ...
    dnx_meter_profile_info_t * profile_info)
{
    ...
    for (database_idx = 0; database_idx < nof_ingress_db; database_idx++)
    {
        ...
        /** set all default meters to the max rate */
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_idx)
        {
            SHR_IF_ERR_EXIT(dnx_meter_generic_profile_set(unit, core_idx, TRUE, database_idx,
                                                          DNX_METER_UTILS_DEFAULT_PROFILE_ID, profile_info));
        }
    ...
    for (database_idx = 0; database_idx < nof_egress_db; database_idx++)
    {
        ...
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_idx)
        {
            SHR_IF_ERR_EXIT(dnx_meter_generic_profile_set(unit, core_idx, FALSE, database_idx,
                                                          DNX_METER_UTILS_DEFAULT_PROFILE_ID, profile_info));
        }
        /** set the ptr map table to invalid banks */
        SHR_IF_ERR_EXIT(dnx_meter_generic_ptr_map_hw_set
    ...
    int single_bucket_bank)
{
    ...
    else
    {
        ...
        {
            bank_id = DNX_CRPS_METER_SHMEM_BANK_ID_CALC(unit, engine->engine_id, section_arr[section_idx]);
            /** Taking a handle */
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_METER_PROFILE_SET_FOR_BIG_ENGINE, &entry_handle_id));
            /** Setting key fields */
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
    ...
    int enable)
{
    ...
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_DATABASE_ID, DBAL_RANGE_ALL, DBAL_RANGE_ALL);
