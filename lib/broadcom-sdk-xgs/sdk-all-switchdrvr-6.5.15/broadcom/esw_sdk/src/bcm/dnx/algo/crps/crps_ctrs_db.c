    ...
    uint64 value)
{
    ...
    SHR_MASK_VERIFY(flags, DNX_CRPS_CTRS_OVERWRITE_COUNTER_VALUE, _SHR_E_PARAM, "Incorrect flags\n");
    SHR_IF_ERR_EXIT(dnx_crps_db.proc.counter.is_allocated(unit, core_id, engine_id, &is_alloc));
    ...
    DNX_CRPS_MGMT_ENGINE_VALID_GET(unit, core_id, engine_id, valid);
    ...
    uint64 *value)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.counter.set(unit, core_id, engine_id, (ofs + sub_counter), 0));
    }
    ...
    int engine_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_crps_db.proc.counter.set(unit, core_id, engine_id, counter_index, 0));
    }
