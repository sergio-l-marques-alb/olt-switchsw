    ...
    dnx_algo_rates_shaper_bounds_t * shaper_bounds)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_field_info_get
                    (unit, DBAL_TABLE_FABRIC_CGM_SLOW_START_CONFIG, DBAL_FIELD_CALENDAR, FALSE, 0, 0, &field_info));
    ...
    sal_memset(&field_info, 0, sizeof(field_info));
    SHR_IF_ERR_EXIT(dbal_tables_field_info_get
                    (unit, DBAL_TABLE_FABRIC_CGM_SLOW_START_CONFIG, DBAL_FIELD_DELAY, FALSE, 0, 0, &field_info));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_cgm_shaper_bounds_get(unit, &shaper_bounds));
    SHR_IF_ERR_EXIT(dnx_algo_rates_rate_to_credit_interval
                    (unit, shaper_rate, &shaper_bounds, interval_resolution, &shaper_interval, &shaper_credit));
    ...
    for (fmc_shaper = 0; fmc_shaper < bcmFabricMcShaperNof; ++fmc_shaper)
    {
        ...
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ING_TRANSMIT_SHAPER_ID, ing_transmit_shaper);
    ...
    int unit)
{
    ...
    for (ing_transmit_context = 0; ing_transmit_context < max_nof_pipes; ++ing_transmit_context)
    {
        for (pipe = 0; pipe < max_nof_pipes; ++pipe)
        {
            /** 1 to 1 mapping */
            enable = (pipe == ing_transmit_context && pipe < nof_pipes) ? 1 : 0;

            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_ING_TRANSMIT_CONTEXT, ing_transmit_context);
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PIPE, pipe);
    ...
    int unit)
{
    ...
    {
        /*
         * Set threshold to be 32 below the fifo size, or to be max for unused pipes
         */
        threshold =
            (pipe <
             nof_pipes) ? (egress_fifo_base_address_max_value / nof_pipes - 32) : (egress_fifo_base_address_max_value);
    ...
    dnx_fabric_cgm_control_access_info_t * access_info)
{
    ...
    else if (access_info->dbal_key1 != DBAL_FIELD_EMPTY)
    {
        if (access_info->index_type == DNX_FABRIC_CGM_CONTROL_INDEX_TYPE_PIPE)
        {
            ...
            {
                uint32 max_pipe = dnx_data_fabric.pipes.nof_pipes_get(unit) - 1;
                SHR_RANGE_VERIFY(control_id, 0, max_pipe, _SHR_E_PARAM, "pipe %u is out of range.", control_id);
            }
        }
        else
        {
    ...
    uint32 *shaper_rate)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_rates_credit_interval_to_rate
                    (unit, interval_resolution, shaper_interval, shaper_credit, shaper_rate));
    ...
    uint32 rate_percent)
{
    ...
    SHR_IF_ERR_EXIT(dnx_fabric_cgm_shaper_bounds_get(unit, &shaper_bounds));
    SHR_IF_ERR_EXIT(dnx_algo_rates_rate_to_credit_interval
                    (unit, slow_start_shaper_rate, &shaper_bounds, interval_resolution, &shaper_interval,
                     &shaper_credit));
    ...
    int value)
{
    ...
    for (key1 = keys_info.key1.start_index; key1 <= keys_info.key1.end_index; ++key1)
    {
        ...
        {
            /** Slow Start set */
            if (control_type == bcmFabricCgmSlowStartRateShaperPhaseParam)
            {
    ...
    int *value)
{
    ...
    if (control_type == bcmFabricCgmSlowStartRateShaperPhaseParam)
    {
        SHR_IF_ERR_EXIT(dnx_fabric_cgm_control_slow_start_hw_get
                        (unit, keys_info.key1.start_index, keys_info.key2.start_index, (uint32 *) value));
    }
    else
    {
