...
dnx_mirror_profile_probability_get(
    ...
    uint32 *prob_field)
{
    DNX_SAND_U64 tmp_result, result;
    uint32 field_mask = 0;
    ...
    {
        dnx_sand_u64_multiply_longs(dividend, field_mask, &tmp_result);

        dnx_sand_u64_devide_u64_long(&tmp_result, divisor, &result);

        SHR_IF_ERR_EXIT(dnx_sand_u64_to_long(&result, prob_field));
    }
    ...
    bcm_mirror_destination_t * mirror_dest)
{
    ...
    if (metadata_ow)
    {
        ...
        mirror_dest->packet_control_updates.metadata |= BCM_MIRROR_PKT_HEADER_UPDATE_METADATA;
    }
    ...
    int action_profile_id)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER_OW, INST_ALL, FALSE);
    ...
    int action_profile_id)
{
     ...
     */
    if ((soc_property_get(unit, spn_STACKING_ENABLE, 0x0) || dnx_data_headers.ftmh.add_dsp_ext_get(unit)) &&
    ...
    int action_profile_id)
{
    ...
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SNIF_CROP_ENABLE, INST_SINGLE, FALSE);
    }
    else if (mirror_dest->egress_packet_copy_size == dnx_data_snif.egress.crop_size_get(unit))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_SNIF_CROP_ENABLE, INST_SINGLE, TRUE);
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "cropping size=%d is invalid", mirror_dest->egress_packet_copy_size);
    }
...
{
    uint32 entry_handle_id;
    uint32 snif_type, action_profile_id, is_count_ow, type_ow, obj_id_ow, meter_ow;
    ...
    if (counter_cmd_id < dnx_data_meter.meter_db.nof_ingress_db_get(unit))
    {
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER_OW, counter_cmd_id,
                                   &meter_ow);
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER, counter_cmd_id,
                                   (uint32 *) &mirror_count_info->is_meter);
    }
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
    ...
    if (meter_ow == FALSE && counter_cmd_id < dnx_data_meter.meter_db.nof_ingress_db_get(unit))
    {
        mirror_count_info->is_meter = BCM_MIRROR_COUNT_AS_ORIG;
    }
    ...
    bcm_mirror_profile_cmd_count_t * mirror_count_info)
{
    ...
    /*
     * is meter field - valid only for ingress meter related commands
     */
    if (counter_cmd_id < dnx_data_meter.meter_db.nof_ingress_db_get(unit))
    {
        /*
         * verify that once is_meter is set to TRUE, is_counted is also TRUE in order for is-meter to be valid
         */
        if (mirror_count_info->is_meter == 1 && mirror_count_info->is_counted != 1)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "if is_meter is TRUE, is_counted should also be TRUE");
        }
        if (mirror_count_info->is_meter == BCM_MIRROR_COUNT_AS_ORIG && mirror_count_info->is_counted == 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "if is_meter is BCM_MIRROR_COUNT_AS_ORIG, is_counted should be set");
        }
        if (mirror_count_info->is_meter == BCM_MIRROR_COUNT_AS_ORIG)
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER_OW, counter_cmd_id,
                                         FALSE);
        }
        else
        {
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER_OW, counter_cmd_id,
                                         TRUE);
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_INT_STAT_OBJ_IS_METER, counter_cmd_id,
                                         mirror_count_info->is_meter);
        }
    }
