    ...
    uint8 *db_id)
{
    ...
    if (db_index == -1)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "table not found, table %s\n", name);
    }
    *db_id = db_index;
    ...
    kbp_mngr_key_segment_t * key_segments)
{
    ...
    if (nof_segments > DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Trying to set %d master key segments, which is more than the available %d\n",
                     nof_segments, DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP);
    }
    ...
    for (index = 0; index < nof_segments; index++)
    {
        ...
        SHR_IF_ERR_EXIT(kbp_mngr_key_segment_set
                        (unit, entry_handle_id, DNX_KBP_OPCODE_TABLE_INDICATION, index, &field_type,
                         &(key_segments[index].nof_bytes), key_segments[index].name));
    }

    /** Set the invalid master key segments */
    for (index = nof_segments; index < DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP; index++)
    {
        uint32 nof_bytes = 0;
        uint32 field_type = DBAL_ENUM_FVAL_KBP_KEY_FIELD_TYPE_INVALID;

        SHR_IF_ERR_EXIT(kbp_mngr_key_segment_set
                        (unit, entry_handle_id, DNX_KBP_OPCODE_TABLE_INDICATION, index, &field_type, &nof_bytes, ""));
    }
    ...
    kbp_mngr_key_segment_t * key_segments)
{
    ...
    uint32 master_key[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 };
    int index;
    ...
    for (index = 0; index < nof_valid_segments; index++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_MASTER_KEY, index, master_key));

        SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                        (unit, DBAL_FIELD_MASTER_KEY, DBAL_FIELD_SEGMENT_SIZE, &field_nof_bytes, master_key));
        if (field_nof_bytes == DNX_KBP_INVALID_SEGMENT_SIZE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "unexpected segment size, segment %d has invalid size but expected to be valid\n", index);
        }

        SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode
                        (unit, DBAL_FIELD_MASTER_KEY, DBAL_FIELD_SEGMENT_NAME, segment_name_u32, master_key));
        utilex_U32_to_U8(segment_name_u32, DNX_KBP_KEY_SEGMENT_NAME_SIZE_IN_BYTES,
                         (uint8 *) (key_segments[(*nof_key_segments)].name));
    ...
    kbp_mngr_key_segment_t * key_segments)
{
    ...
    uint32 nof_valid_segments = 0;
    int index;
    kbp_mngr_key_segment_t new_master_key[DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP];
    dbal_enum_value_field_kbp_device_status_e kbp_mngr_status;
    ...
    if (DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP - nof_valid_segments < nof_new_segments)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Not enough available segments in master key %d. New number of segments to add %d\n",
                     DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP - nof_valid_segments, nof_new_segments);
    }
    ...
    for (index = 0; index < nof_new_segments; index++)
    {
        ...
        SHR_IF_ERR_EXIT(kbp_mngr_key_segment_set
                        (unit, entry_handle_id, DNX_KBP_OPCODE_TABLE_INDICATION, nof_valid_segments + index,
                         &field_type, &(key_segments[index].nof_bytes), key_segments[index].name));
    }
    ...
    kbp_printable_entry_t * kbp_entry_print_info)
{
    ...
    *kbp_entry_print_num = 0;
    hitbit_index = 0;
    for (lookup_index = 0; lookup_index < DNX_KBP_MAX_NOF_RESULTS; lookup_index++)
    {
        uint8 result_id;
        uint8 lookup_db;
        ...
        if ((flags == DNX_KBP_ENTRY_PARSE_FWD_ONLY) &&
            ((lookup_type != DBAL_ENUM_FVAL_KBP_LOOKUP_TYPE_FWD) || (lookup_type != DBAL_ENUM_FVAL_KBP_LOOKUP_TYPE_RPF)))
        {
        ...
        {
            kbp_printable_entry_t * current_kbp_entry_print_info = kbp_entry_print_info + (*kbp_entry_print_num);
            ...
            *kbp_entry_print_num = *kbp_entry_print_num + 1;
        }
    }
    ...
    kbp_printable_entry_t * kbp_entry_print_info)
{
    ...
    uint32 current_key_offset_bytes = BITS2BYTES(key_sig_size);
    uint32 key_segment[DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY][DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { {0} };
    uint8 key_buffer[DNX_KBP_MAX_KEY_LENGTH_IN_BYTES] = { 0 };
    dbal_printable_field_t *printable_field;
    dbal_printable_entry_t *entry_print_info = &kbp_entry_print_info->entry_print_info;
    ...
    for (index = 0; index < nof_master_key_segments; index++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode(unit, DBAL_FIELD_MASTER_KEY, DBAL_FIELD_SEGMENT_SIZE,
                                                        &segment_bytes, key_segment[index]));

        SHR_IF_ERR_EXIT(utilex_U32_to_U8
                        (segment_name_u32, DNX_KBP_KEY_SEGMENT_NAME_SIZE_IN_BYTES, (uint8 *) segment_name));

        current_key_offset_bytes -= segment_bytes;

        printable_field = &entry_print_info->key_fields_info[entry_print_info->nof_key_fields];
        SHR_IF_ERR_EXIT(dbal_field_string_to_id(unit, segment_name, &printable_field->field_id));
        sal_strncpy(printable_field->field_name, segment_name, DBAL_MAX_LONG_STRING_LENGTH);
        SHR_IF_ERR_EXIT(utilex_U8_to_U32(&key_buffer[current_key_offset_bytes],
                                         segment_bytes, printable_field->field_value));
        SHR_IF_ERR_EXIT(dbal_field_printable_string_get
                        (unit, printable_field->field_id, printable_field->field_value, NULL, FALSE, DBAL_FIELD_EMPTY,
                         BYTES2BITS(segment_bytes), printable_field->field_print_value));
...
shr_error_e kbp_mngr_opcode_printable_entry_key_parsing(int unit,uint8 opcode_id,uint32 *key_sig_value,int key_sig_size,kbp_printable_entry_t * kbp_entry_print_info){DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_opcode_printable_entry_key_parsing")}
shr_error_e kbp_mngr_context_to_opcode_translate(int unit, uint8 fwd_context, uint8 acl_context, uint8 *opcode_id){DUMMY_PROTOTYPE_ERR_MSG("kbp_mngr_context_to_opcode_translate")}
