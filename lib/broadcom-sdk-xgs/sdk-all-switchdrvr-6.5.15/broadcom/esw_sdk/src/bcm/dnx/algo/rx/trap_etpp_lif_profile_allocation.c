    ...
    uint8 *first_reference)
{
     ...
     */
    rv = multi_set_template_profile_get(unit, module_id, multi_set_template, profile_data, profile);
        ...
        if (BCM_GPORT_IS_TRAP(*action_gport_p))
        {
                 ...
                 tmp_profile <= DNX_RX_TRAP_LIF_PROFILE_STACK_TRAP_2; tmp_profile++)
            {
                SHR_IF_ERR_EXIT(multi_set_template_profile_data_get
                                (unit, module_id, multi_set_template, tmp_profile, &tmp_nof_references,
                                 &tmp_action_gport));
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "No avaiable trap profile to allocate\n");
            }
        }
        else
        {
            ...
            if (tmp_nof_references != 0)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "No avaiable mirror profile to allocate\n");
            }
