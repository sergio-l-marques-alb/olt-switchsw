    ...
    uint8 *full)
{
    ...
    *full = TRUE;
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_tcam_bank_is_full(unit, core, bank_ids[bank_i], handler_id, full));
        if (!(*full))
        {
            /*
    ...
    uint32 *nof_free_backward)
{
    ...
    *nof_free_backward = 0;
    ...
    for (bank_i = 0; bank_i < bank_count; bank_i++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get(unit, core, bank_ids[bank_i], 0, &bank_nof_free));

        if (bank_ids[bank_i] < bank_id)
        {
    ...
    int bank_id)
{
    ...
    if (nof_free_forward > 0)
    {
        ...
        dnx_field_tcam_location_t last_location =
            { dnx_data_field.tcam.nof_banks_get(unit) - 1, dnx_data_field.tcam.nof_small_bank_lines_get(unit) - 1 };
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_algo_empty_location_find
    ...
    int bank_ids[])
{
    ...
    for (bank_idx = 0; bank_idx < nof_banks; bank_idx++)
    {
        for (core = 0; core < dnx_data_device.general.max_nof_cores_get(unit); core++)
        {
            ...
            else        /* Evacuation succeeded */
            {
                /** Remove bank from handler's access profile */
