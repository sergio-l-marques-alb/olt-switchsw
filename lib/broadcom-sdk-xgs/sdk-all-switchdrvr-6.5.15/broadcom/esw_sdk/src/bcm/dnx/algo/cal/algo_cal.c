    ...
    dnx_ofp_rates_cal_egq_t * calendar)
{
    ...
    uint32 port_credits[dnx_data_egr_queuing.params.nof_q_pairs_get(unit)];
    ...
    slot_idx = 0,
        port_idx = 0,
        sal_memset(port_credits, 0x0, (dnx_data_egr_queuing.params.nof_q_pairs_get(unit) * sizeof(uint32)));
    ...
    for (port_idx = 0; port_idx < dnx_data_egr_queuing.params.nof_q_pairs_get(unit); ++port_idx)
    {
        ...
        /*
         * Multiply to get 64-bits result:
         * u64_1 = ports_rates[port_idx] * (...)
         */
        utilex_u64_multiply_longs(ports_rates[port_idx],
                                  (calcal_length * calendar_len * DNX_EGQ_CAL_SLOT_TRAVERSE_IN_CLOCKS * egq_resolution),
                                  &u64_1);
        /*
         * Load u64_2 by 'u64_1 / (calcal_instances * DNXCMN_CORE_CLOCK_KHZ_GET(unit))'
         * Load 'rem' by the remainder of the division:
         * rem = 'u64_1 % (calcal_instances * DNXCMN_CORE_CLOCK_KHZ_GET(unit))'
         */
        rem = utilex_u64_devide_u64_long(&u64_1, (calcal_instances * DNXCMN_CORE_CLOCK_KHZ_GET(unit)), &u64_2);
        utilex_u64_to_long(&u64_2, &(port_credits[port_idx]));
...
dnx_algo_cal_simple_per_len_build(
    ...
    uint32 *deviation)
{
    uint32 obj_id, slots_rates[nof_objects], total_num_slots = 0, calc_deviation;
    uint32 rem;
    ...
    sal_memset(slots_rates, 0, sizeof(uint32) * nof_objects);
    ...
    else
    {
        ...
        for (obj_id = 0; obj_id < nof_objects; ++obj_id)
        {
            ...
            utilex_u64_multiply_longs(DNX_ALGO_CAL_RATE_DEVIATION, slots_rates[obj_id], &u64_1);
        ...
        *actual_len = total_num_slots;
        sal_memcpy(slots_per_object, slots_rates, sizeof(uint32) * nof_objects);
    ...
    uint32 *slots_per_object)
{
    ...
    uint32 slots_per_object_temp[nof_objects];
    ...
    for (tentative_len = 1; tentative_len <= max_calendar_len; ++tentative_len)
    {
        SHR_IF_ERR_EXIT(dnx_algo_cal_simple_per_len_build(unit,
                                                          ...
                                                          tentative_len, &actual_len, slots_per_object_temp,
                                                          &deviation));
    ...
    SHR_IF_ERR_EXIT(dnx_algo_cal_simple_per_len_build(unit,
                                                      ...
                                                      best_tentative_len, &actual_len, slots_per_object_temp,
                                                      &deviation));

    *actual_calendar_len = actual_len;
    sal_memcpy(slots_per_object, slots_per_object_temp, sizeof(uint32) * nof_objects);
...
dnx_algo_cal_simple_fixed_len_cal_build(
    ...
    uint32 *calendar)
{
    uint32 slot_idx, slots_per_object_temp[nof_objects];
    ...
    max_obj_idx = utilex_get_index_of_max_member_in_array(slots_per_object_temp, nof_objects);
    while (slots_per_object_temp[max_obj_idx] > 0)
    {
        /*
         * used as big hop size for the alternate algorithm 
         */
        hop_size = rem_cal_len / slots_per_object_temp[max_obj_idx];
        ...
        slots_per_object_temp[max_obj_idx] = 0;
...
dnx_algo_cal_simple_from_rates_to_calendar(
    ...
    uint32 *calendar_len)
{
    uint32 slots_per_object[nof_objects];
    ...
    SHR_IF_ERR_EXIT(dnx_algo_cal_simple_len_calculate(unit,
                                                      ...
                                                      nof_objects,
                                                      total_bandwidth,
                                                      max_calendar_len, calendar_len, slots_per_object));
