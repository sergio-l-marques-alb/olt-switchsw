    ...
    int numq)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.vsq_f_mapping.src_port.set(unit, core_id, pg_base,
                                                                                        src_port));
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.vsq_f_mapping.pg_numq.set(unit, core_id, pg_base, numq));
    ...
    int *vsq_e_id)
{
    ...
    {
        /** NIF port -- phy port is VSQ-E id */
        /** VSQ-E index is mapped 1-1 to NIF port */
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_first_phy_get(unit, src_port, 0, &phy_port));

        *vsq_e_id = phy_port % dnx_data_nif.phys.nof_phys_per_core_get(unit); /** VSQ-E id is local per core */
    ...
    int vsq_e_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_type_get(unit, src_port, &port_type));
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_type, FALSE, FALSE)) /** for NIF port, we didn't allocate, so no need to deallocate */
    {
    ...
    int *vsq_f_id)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_type, FALSE, FALSE))
    {
        ...
        local_phy_index = phy_port % dnx_data_nif.phys.nof_phys_per_core_get(unit); /** VSQ-E id is local per core */
    ...
    int unit)
{
     ...
     */
    data.first_element = dnx_data_nif.phys.nof_phys_per_core_get(unit);
    data.nof_elements = dnx_data_ingr_congestion.vsq.vsq_e_nof_get(unit) - data.first_element;

    /*
     * Support per core configuration 
     */
    data.flags = SW_STATE_ALGO_RES_CREATE_DUPLICATE_PER_CORE;

    sal_strncpy(data.name, INGRESS_CONGESTION_VSQE_RSRC_MNGR_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
     ...
     */
    sal_strncpy(data.name, INGRESS_CONGESTION_VSQF_RSRC_MNGR_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH - 1);
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    uint32 global_drop_pool_dp[dnx_data_ingr_congestion.vsq.pool_nof_get(unit)][DNX_COSQ_NOF_DP];
    ...
    bcm_gport_t vsq_gport)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.vsq_f_mapping.pg_numq.get(unit, core_id, pg_base, &numq));
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_port_based_vsq_sw_state_update(unit, core_id, DNX_ALGO_PORT_INVALID,
                                                                          src_port_vsq_index, pg_base, 0));

    for (cosq = 0; cosq < numq; cosq++)
    {
        SHR_IF_ERR_EXIT(dnx_vsq_rate_class_destroy(unit, core_id, pg_base + cosq, DNX_INGRESS_CONGESTION_VSQ_GROUP_PG));
    }

    /** All TCs are mapped to offset 0, by default*/
    sal_memset(&tc_pg_mapping, 0x0, sizeof(tc_pg_mapping));
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_pg_tc_profile_exchange_and_set(unit, src_local_port, &tc_pg_mapping));
    ...
    for (cosq = 0; cosq < numq; cosq++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_pg_use_port_guaranteed_set(unit, core_id, pg_base + cosq, FALSE));
    ...
    dnx_ingress_congestion_wred_info_t * wred_data)
{
    ...
    wred_data->min_avrg_th = discard->min_thresh;
    wred_data->max_avrg_th = discard->max_thresh;
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    discard->min_thresh = wred_data->min_avrg_th;
    discard->max_thresh = wred_data->max_avrg_th;
...
dnx_ingress_congestion_voq_rate_class_array_from_config_get(
    ...
    uint32 rate_class_nof_queues_to_add[dnx_data_ingr_congestion.voq.nof_rate_class_get(unit)])
{
    int cosq, rate_class;
    ...
    int unit)
{
    ...
    for (rate_class = 0; rate_class < nof_rate_class; ++rate_class)
    {
        for (int dp = 0; dp < DNX_COSQ_NOF_DP; ++dp)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_voq_rate_class_fadt_tail_drop_get
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    {
        /** Configure ECN drop */
        SHR_IF_ERR_EXIT(dnx_voq_rate_class_ecn_drop_set(unit, dbal_key.rate_class, discard->ecn_thresh));
    }
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    if (is_ecn)
    {
        SHR_IF_ERR_EXIT(dnx_voq_rate_class_ecn_drop_get(unit, dbal_key.rate_class, (uint32 *) &discard->ecn_thresh));
    }
    ...
    bcm_cosq_pfc_config_t * pfc_threshold)
{
    ...
    int vsq_index;
    dnx_ingress_congestion_vsq_group_e vsq_type;
    int pool[dnx_data_ingr_congestion.vsq.pool_nof_get(unit)];
    ...
    uint32 new_max_guaranteed)
{
    ...
    int pool_id;
    int nof_pools = dnx_data_ingr_congestion.vsq.pool_nof_get(unit);

    dnx_ingress_congestion_global_vsq_resource_alloc_params_t resource_alloc_hw_params[nof_pools];
