 ...
 *  - "Ingress Only" cannot be set with the Egress flags - "Egress Native" or "Egress IP Tunnel".
    ...
    uint32 flags)
{
    ...
    if ((is_ingress_only == TRUE) && ((is_egress_native == TRUE) || (is_egress_ip_tunnel == TRUE)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong flags setting. INGRESS_ONLY(0x%08X) is set togeter with NATIVE_EVE(0x%08X)/EGRESS_IP_TUNNEL(0x%08X)!!! flags = 0x%08X\n",
    ...
    uint32 flags)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Wrong flags setting. One of INGRESS_ONLY (0x%08X) and EGRESS_ONLY (0x%08X) must be are set!!! flags = 0x%08X\n",
                     BCM_PORT_TPID_CLASS_INGRESS_ONLY, BCM_PORT_TPID_CLASS_EGRESS_ONLY, flags);
    }
 ...
 *          - tpid1
 *          - tpid2
 * \param [out] tpid_class - Pointer to a TPID class structure
    ...
    bcm_port_tag_format_class_t * tag_format_class_id)
{
    ...
    flags = (is_with_id == TRUE) ? SW_STATE_ALGO_RES_ALLOCATE_WITH_ID : 0;
    ...
    if (is_symmetric == TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(port_tpid_db.tag_struct_allocation.bcm_tag_format_class_id_ingress.allocate_single
                        (unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, &tag_struct_sw_id));
    ...
    dnx_port_tpid_llvp_parser_info_t * llvp_parser_info)
{
     ...
     * Set key fields
     */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LLVP_PROFILE, llvp_profile);
    ...
    dnx_port_tpid_ingress_llvp_entry_t * dnx_port_tpid_ingress_llvp_entry)
{
     ...
     * Set key fields
     */
    dnx_port_tpid_class_utility_llvp_entry_dbal_keys_set(unit, entry_handle_id, llvp_profile, llvp_parser_info);
    ...
    dnx_port_tpid_egress_llvp_entry_t * egress_llvp_entry)
{
     ...
     * Read data fields
     */
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_C_TAG_OFFSET, INST_SINGLE,
    ...
    int llvp_profile)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LLVP_PROFILE, INST_SINGLE, llvp_profile);
    /*
     * write to HW and release handle
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    int *llvp_profile)
{
    ...
    if (is_physical_port == FALSE)
    {
        ...
        dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_LLVP_PROFILE, INST_SINGLE,
                                   (uint32 *) llvp_profile);
    ...
    acceptable_frame_type_template_t * acceptable_frame_type_template)
{
    ...
    for (outer_tpid_index = 0; outer_tpid_index < DNX_PORT_TPID_TPID_INDEX_SIZE; outer_tpid_index++)
    {
        for (inner_tpid_index = 0; inner_tpid_index < DNX_PORT_TPID_TPID_INDEX_SIZE; inner_tpid_index++)
        {
             ...
             * Set data fields
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION,
                                         INST_SINGLE,
    ...
    int acceptable_frame_type_profile)
{
     ...
     * Set the values
     */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION, INST_SINGLE,
                                 DBAL_ENUM_FVAL_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION_ACCEPT);
...
dnx_port_tpid_class_egress_acceptable_frame_type_get(
    ...
    int inner_tpid_index,
    int acceptable_frame_type_profile,
    dbal_enum_value_field_acceptable_frame_type_filter_action_e * acceptable_frame_type_filter_action)
{
    uint32 entry_handle_id;
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION, INST_SINGLE,
                              (uint8 *) acceptable_frame_type_filter_action);
...
{
    uint8 is_init;
    int ingress_native_llvp_profile, egress_ip_tunnel_llvp_profile;
    uint32 entry_handle_id;
    bcm_port_tag_format_class_t tag_format_class_id;
    soc_reg_above_64_val_t data;
    uint32 egress_arr_prefix;
    ...
    /*
     * SDK design assumption is that Last Egress profile (7) is used for Egress IP Tunnel -
     * Verify that ARR prefix value equals SDK assumption.
     * Note:
     * ARR prefix is 8 bits - 3 MSBs are the Egress LLVP profile.
     *
     */
    egress_ip_tunnel_llvp_profile = dnx_data_port_pp.general.default_egress_ip_tunnel_llvp_profile_id_get(unit);

    
    SOC_REG_ABOVE_64_CLEAR(data);
    SHR_IF_ERR_EXIT(soc_mem_read(unit, ETPPA_PRP_EES_ARR_PREFIX_TABLEm, IHP_BLOCK(unit, SOC_BLOCK_ALL), 2, data));
    egress_arr_prefix = soc_mem_field32_get(unit, ETPPA_PRP_EES_ARR_PREFIX_TABLEm, data, PREFIXf);

    /*
     * Only in case device image is standard_1, validate the egress_arr_prefix value.
     */
    {
        if (sal_strncmp("standard_1", dnx_data_pp.application.device_image_get(unit)->name, RHFILE_MAX_SIZE) == 0)
        {
            if (egress_ip_tunnel_llvp_profile != (egress_arr_prefix >> 5))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Error. ARR prefix = %d is different from SDK egress_ip_tunnel_llvp_profile = %d!!!\n",
                             egress_arr_prefix, egress_ip_tunnel_llvp_profile);
            }
        }
    }
    ...
    bcm_gport_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.allocate_single
                    (unit, _SHR_CORE_ALL, 0, default_template_data, NULL, &llvp_profile, &first_reference));
    ...
    bcm_gport_t logical_port)
{
    ...
    SHR_IF_ERR_EXIT(algo_port_pp_db.egress_acceptable_frame_type_profile.allocate_single
                    (unit, _SHR_CORE_ALL, 0, &default_acceptable_frame_type_template, NULL,
    ...
    bcm_port_tpid_class_t * tpid_class)
{
    ...
    else if (is_egress == TRUE)
    {
        ...
        {
            if (dnx_port_tpid_egress_llvp_entry.c_tag_offset == 0)
            {
                tpid_class->flags |= BCM_PORT_TPID_CLASS_OUTER_C;
            }
            else if (dnx_port_tpid_egress_llvp_entry.c_tag_offset == 1)
            {
        ...
        if (is_physical_port)
        {
            if (acceptable_frame_type_filter_action == DBAL_ENUM_FVAL_ACCEPTABLE_FRAME_TYPE_FILTER_ACTION_DROP)
            {
                tpid_class->flags |= BCM_PORT_TPID_CLASS_DISCARD;
            }
        }
    ...
    if ((is_ingress == TRUE) && (is_ingress_native == FALSE))
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.profile_data_get
                            (unit, _SHR_CORE_ALL, old_llvp_profile, &ref_count, tpid_class_buffer));
        }
        ...
        if (old_llvp_profile == ingress_native_ac_profile)
        {
             ...
             */
            SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.allocate_single
                            (unit, _SHR_CORE_ALL, 0, tpid_class_buffer, NULL, &new_llvp_profile, &first_reference));
            /*
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.port_pp_tpid_profile_ingress.exchange
                            (unit, _SHR_CORE_ALL, 0, tpid_class_buffer, old_llvp_profile, NULL, &new_llvp_profile,
    ...
    if ((is_egress == TRUE) && (is_egress_native == FALSE) && (is_egress_ip_tunnel == FALSE))
    {
        ...
        if (is_physical_port)
        {
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.egress_acceptable_frame_type_profile.profile_data_get
                            (unit, _SHR_CORE_ALL, old_acceptable_frame_type_profile, &ref_count,
            ...
            SHR_IF_ERR_EXIT(algo_port_pp_db.egress_acceptable_frame_type_profile.exchange
                            (unit, _SHR_CORE_ALL, 0, &acceptable_frame_type_template,
