    ...
    uint32 place)
{
    ...
    else
    {
        ...
        range.max = place;
        /*
         * Add entry after "iter" since entry_lookup returns the prev node to where the node is supposed to be if not
         * found 
         */
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.tcam_handlers_info.
                        priorities_range.add_after(unit, handler_id, core, iter, &key, &range));
...
dnx_field_tcam_location_algo_find(
    ...
    uint8 *found)
{
    int bank_ids[dnx_data_field.tcam.nof_banks_get(unit)];
    int bank_count;
    dnx_field_tcam_location_range_t insertion_range;
    uint32 insertion_place;
     ...
     */
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_profile_banks_sorted_get(unit, handler_id, bank_ids, &bank_count));
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_location_algo_search_in_range(unit, core, handler_id,
                                                                 &insertion_range,
                                                                 bank_ids, bank_count, &insertion_place, found));
...
shr_error_e
dnx_field_tcam_location_algo_priority_get(
    int unit,
    int core,
    uint32 handler_id,
    uint32 location,
    uint32 *priority)
{
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(iter))
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.tcam_handlers_info.
                            priorities_range.node_key(unit, handler_id, core, iter, &node_priority));
            break;
        }
        SHR_IF_ERR_EXIT(dnx_field_tcam_location_sw.tcam_handlers_info.
                        priorities_range.next_node(unit, handler_id, core, iter, &iter));
    ...
    if (!DNX_SW_STATE_LL_IS_NODE_VALID(iter))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "The requested entry was not found in priority list");
    }

    /*
     * Convert back entry's priority from node's priority.
     * See priority_list_entry_add for more information.
     */
    *priority = node_priority - 1;
    ...
    uint8 *found)
{
    ...
    uint32 key_size;
    uint32 forward_empty_place;
    uint32 backward_empty_place;
    ...
    *found = FALSE;
    ...
    {
        if (bank_ids[bank_i] >= location->bank_id)
        {
    ...
    else if (find_mode & DNX_FIELD_TCAM_LOCATION_FIND_MODE_FORWARD)
    {
        ...
        {
            if (key_size == dnx_data_field.tcam.key_size_double_get(unit) && bank_ids[bank_i] % 2)
            {
                /*
                 * Double sized key FGs can't start from odd bank numbers 
                 */
            ...
            search_range.min = FIELD_TCAM_LOCATION_ALGO_MIN_BANK_INDEX(bank_ids[bank_i]);
            search_range.max = FIELD_TCAM_LOCATION_ALGO_MAX_BANK_INDEX(bank_ids[bank_i]);

            if (location->bank_id == bank_ids[bank_i])
            {
