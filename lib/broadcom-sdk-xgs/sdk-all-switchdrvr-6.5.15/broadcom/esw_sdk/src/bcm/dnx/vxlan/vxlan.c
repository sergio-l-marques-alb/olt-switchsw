    ...
    bcm_vpn_t l2vpn)
{
     ...
     * Check L2VPN range
     */
    if (l2vpn >= (dnx_data_l2.vsi.nof_vsis_get(unit)))
    {
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    uint32 tunnel_term_dbal_result_type;
    uint32 learn_enable = 1;

    tunnel_term_dbal_table = tunnel_term_gport_hw_resources->inlif_dbal_table_id;
    tunnel_term_dbal_result_type = tunnel_term_gport_hw_resources->inlif_dbal_result_type;
    ...
    int out_lif_profile)
{
    ...
    if (((dbal_table_id == DBAL_TABLE_EEDB_IPV4_TUNNEL) && (dbal_result_type != DBAL_RESULT_TYPE_EEDB_IPV4_TUNNEL_ETPS_IPV4_TUNNEL)) ||
            ((dbal_table_id == DBAL_TABLE_EEDB_IPV6_TUNNEL) && (dbal_result_type != DBAL_RESULT_TYPE_EEDB_IPV6_TUNNEL_ETPS_IPV6_TUNNEL_RAW)))
    {
        SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get(unit,
                                                             tunnel_init_gport_hw_resources->outlif_dbal_table_id,
        ...
        SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get(unit,
                                                             tunnel_init_gport_hw_resources->outlif_dbal_table_id,
                                                             DBAL_FIELD_ESEM_NAME_SPACE,
                                                             FALSE, tunnel_init_gport_hw_resources->outlif_dbal_result_type, 0,
                                                             DBAL_PREDEF_VAL_MIN_VALUE,
                                                             &esem_name_space_min_val));
        if ((esem_namespace > esem_name_space_max_val) || (esem_namespace < esem_name_space_min_val))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                        "Invalid esem_namespace %d! Valid value in range from %d to %d!\n", esem_namespace, esem_name_space_min_val, esem_name_space_max_val);
        }
    }
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, dbal_table_id, &entry_handle_id));
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, dbal_result_type);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ESEM_NAME_SPACE, INST_SINGLE, esem_namespace);
    ...
    dnx_algo_gpm_forward_info_t * forward_info)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_l2_forward_info_add(unit,
            _SHR_IS_FLAG_SET(vxlan_port->flags, BCM_VXLAN_PORT_REPLACE), vxlan_port->vxlan_port_id, forward_info));
    ...
    bcm_vxlan_port_t * vxlan_port)
{
        ...
        (forward_info.fwd_info_result_type == DBAL_RESULT_TYPE_L2_GPORT_TO_FORWARDING_SW_INFO_DEST_OUTLIF))
    {
        ...
        else if (_SHR_GPORT_IS_SYSTEM_PORT(destination))
        {
            BCM_GPORT_SYSTEM_PORT_ID_SET(vxlan_port->match_port, destination);
        }
    }
    ...
    bcm_vxlan_port_t * vxlan_port)
{
    ...
    /** Update the tunnel outlif orientation based on the inputs*/
        SHR_IF_ERR_EXIT(dnx_vxlan_egress_lif_profile_update
                        (unit, vxlan_port, local_out_lif, tunnel_init_gport_hw_resources.outlif_dbal_table_id,
                         tunnel_init_gport_hw_resources.outlif_dbal_result_type, &out_lif_profile, 0));
    }
    /**
    ...
    SHR_IF_ERR_EXIT(dnx_vxlan_port_ingress_lif_set
                    (unit, &tunnel_term_gport_hw_resources, in_lif_profile, is_fec, &forward_info));
    ...
    if (is_outlif)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_tables_field_predefine_value_get(unit,
                                                             ...
                                                             DBAL_PREDEF_VAL_MIN_VALUE,
                                                             &esem_name_space));
        SHR_IF_ERR_EXIT(dnx_vxlan_port_egress_lif_set(unit, &tunnel_init_gport_hw_resources, esem_name_space, out_lif_profile));
    }
exit:
    SHR_FUNC_EXIT;
}
 ...
 *   \param [in] forwarding_domain - forwarding domain. Identify the network in which the VNI belongs to.
 *   \param [in] result_type - result type
 *   \param [out] qos_map_id - configured qos_map_id.
    ...
    uint32 *result_type)
{
    ...
    if (((*result_type) == DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO_VLANS)
        && (dnx_data_esem.hw_bug.feature_get(unit, dnx_data_esem_hw_bug_etps_properties_assignment)))
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_FWD_DOMAIN_NWK_NAME, INST_SINGLE, vni);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_REMARK_PROFILE, INST_SINGLE, qos_map_id);
        if (*qos_map_id != DNX_QOS_INITIAL_MAP_ID)
        {
    ...
    else if ((*result_type) == DBAL_RESULT_TYPE_ESEM_FORWARD_DOMAIN_MAPPING_DB_ETPS_L2_FODO)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_FWD_DOMAIN_NWK_NAME, INST_SINGLE, vni);
    }
...
dnx_vxlan_vpn_vsi_to_vni_result_type_get(
    ...
    uint32 *result_type)
{
    uint32 dummy_qos_map_id;
    uint32 vni;
    uint32 rv_vsi2vni;
    SHR_FUNC_INIT_VARS(unit);

    rv_vsi2vni =
        dnx_vxlan_vpn_vsi_to_vni_get(unit, vsi, forwarding_domain_namespace, &dummy_qos_map_id, &vni, result_type);
    ...
    bcm_vxlan_vpn_config_t * info)
{
    ...
    if (!update)
    {
        ...
        if (info->vnid != BCM_VXLAN_VNI_INVALID)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_vsi_to_vni_set
                            (unit, info->vpn, info->match_port_class, info->vnid, DNX_QOS_INITIAL_MAP_ID,
    ...
    if (update)
    {
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE)
        {
            ...
            rv_vsi2vni =
                dnx_vxlan_vpn_vsi_to_vni_get(unit, info->vpn, info->match_port_class, &qos_map_id, &old_vnid,
...
{
    uint32 vnid;
    uint32 network_domain, vxlan_vpn_create, qos_map_id;
    int rv_vsi2vni;
    uint8 is_vsi_used;
    uint32 result_type;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Trying to destroy mapping, which was created from vxlan_netwrok_domain_config_add API. \n");
    }
    rv_vsi2vni = dnx_vxlan_vpn_vsi_to_vni_get(unit, l2vpn, network_domain, &qos_map_id, &vnid, &result_type);
    SHR_IF_ERR_EXIT_EXCEPT_IF(rv_vsi2vni, _SHR_E_NOT_FOUND);
    ...
    bcm_vxlan_network_domain_config_t * config)
{
             ...
             && _SHR_IS_FLAG_SET(config->flags, BCM_VXLAN_NETWORK_DOMAIN_CONFIG_L2_MAPPING))
    {
        ...
        /** Update VSI <-> VNI mapping */
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_vsi_to_vni_set
                            (unit, config->vsi, config->network_domain, config->vni, config->qos_map_id,
                             vsi_to_vni_result_type, TRUE));
        }
        else
        {
             ...
             && _SHR_IS_FLAG_SET(config->flags, BCM_VXLAN_NETWORK_DOMAIN_CONFIG_L3_MAPPING))
    {
        if (_SHR_IS_FLAG_SET(config->flags, BCM_VXLAN_NETWORK_DOMAIN_CONFIG_REPLACE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_vrf_to_vni_set(unit, config->vrf, config->network_domain, config->vni, TRUE));
        }
        else
        {
            /** If we don't want to update - create new VRF to VNI mappings at egress*/
            SHR_IF_ERR_EXIT(dnx_vxlan_vpn_vrf_to_vni_set
                            (unit, config->vrf, config->network_domain, config->vni, FALSE));
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    bcm_vxlan_network_domain_config_t * config)
{
             ...
             && _SHR_IS_FLAG_SET(config->flags, BCM_VXLAN_NETWORK_DOMAIN_CONFIG_L2_MAPPING))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_vxlan_vpn_vsi_to_vni_get
                        (unit, config->vsi, config->network_domain, &qos_map_id, &config->vni, &result_type));
...
{
    uint32 entry_handle_id;
    uint32 dbal_tables[] = { 0, 0, 0 }, vsi, vni, vrf;
    ...
    else if (BCM_VXLAN_NETWORK_DOMAIN_CONFIG_EGRESS_ONLY)
    {
        dbal_tables[0] = DBAL_TABLE_ESEM_FORWARD_DOMAIN_MAPPING_DB;
    }
    else if (BCM_VXLAN_NETWORK_DOMAIN_CONFIG_INGRESS_ONLY)
    {
        dbal_tables[0] = DBAL_TABLE_VNI2VSI;
        ...
        while (!is_end)
        {
            ...
            else if (dbal_tables[table] == DBAL_TABLE_VNI2VRF)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                                (unit, entry_handle_id, DBAL_FIELD_NEXT_LAYER_NETWORK_DOMAIN, &config.network_domain));
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field_arr32_get
                                (unit, entry_handle_id, DBAL_FIELD_VRF, INST_SINGLE, &vsi));
                config.vrf = vsi;
            }
            ...
            else
            {
                ...
                config.vsi = vsi;
                SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id, DBAL_FIELD_VRF, &vrf));
                config.vrf = vrf;
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
