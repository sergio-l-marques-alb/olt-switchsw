...
/** See H file*/
shr_error_e
dnx_algo_field_stage_get(
    int unit,
    const bcm_field_aset_t * asset_p,
    dnx_field_stage_e * field_stage_p)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(asset_p, _SHR_E_PARAM, "asset_p");
    SHR_NULL_CHECK(field_stage_p, _SHR_E_PARAM, "field_stage_p");

    
    if (BCM_FIELD_ASET_TEST(*asset_p, bcmFieldActionStartPacketStrip))
    {
        *field_stage_p = DNX_FIELD_STAGE_IPMF3;
    }
    else
    {
        *field_stage_p = DNX_FIELD_STAGE_IPMF2;
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_field_ace_id_t * ace_id_p)
{
     ...
     */
    if (alloc_flags & SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID)
    {
        alloc_id = *ace_id_p;
             ...
             *ace_id_p <= dnx_data_field.ace.ace_id_pmf_alloc_last_get(unit); (*ace_id_p)++)
        {
            ...
            if (is_alloc == FALSE)
            {
                ...
                SHR_IF_ERR_EXIT(algo_field_info_sw.
                                ace_id.allocate_single(unit, (alloc_flags | SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID),
                                                       NULL, &alloc_id));
                found_alloc = TRUE;
                break;
            }
    ...
    dnx_field_ace_key_t * ace_key_p)
{
    ...
    /**In case WITH_ID flag was not set*/
    *ace_key_p = alloc_id;
    /**In case dnx_field_ace_key_t isn't large enough to contain the allocated key.*/
    if (*ace_key_p != (unsigned int) alloc_id)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Conversion error between unsigned int and dnx_field_ace_key_t, "
                     "converted between %u to %u, dnx_field_ace_key_t isn't large enough,\n",
                     (unsigned int) alloc_id, *ace_key_p);
    }
exit:
    SHR_FUNC_EXIT;
}
...
/**see H file: algo_field.h */
shr_error_e
dnx_algo_field_entry_kbp_opcode_cs_profile_id_allocate(
    int unit,
    int alloc_flags,
...
/**see H file: algo_field.h */
shr_error_e
dnx_algo_field_entry_kbp_opcode_cs_profile_id_deallocate(
    int unit,
    int cs_prodfile_id)
{
    ...
    dnx_algo_template_print_t * print_cb_data)
{
    ...
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_INT, "ACTION entry", fem_action_entry_p->fem_action,
    ...
    for (fem_bit_info_index = 0; fem_bit_info_index < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_BITS_IN_FEM_ACTION;
         fem_bit_info_index++)
    {
        DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_INT, "fem bit format",
                                            fem_action_entry_p->fem_bit_info[fem_bit_info_index].fem_bit_format, NULL,
                                            NULL);

        DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT8, "fem bit value(bit on key select)",
                                            fem_action_entry_p->fem_bit_info[fem_bit_info_index].
                                            fem_bit_value.bit_on_key_select, NULL, NULL);

        DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT8, "fem bit value(bit on map data)",
                                            fem_action_entry_p->fem_bit_info[fem_bit_info_index].
                                            fem_bit_value.bit_on_map_data, NULL, NULL);

        DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT8, "fem bit value(bit value)",
                                            fem_action_entry_p->fem_bit_info[fem_bit_info_index].
                                            fem_bit_value.bit_value, NULL, NULL);

        DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT8, "fem bit value(bit descriptor value)",
                                            fem_action_entry_p->fem_bit_info[fem_bit_info_index].
                                            fem_bit_value.bit_descriptor_value, NULL, NULL);
    }

    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_UINT32, "Fem adder", fem_action_entry_p->fem_adder,
                                        NULL, NULL);
    ...
    int unit)
{
    ...
    for (fem_id_index = 0; fem_id_index < fem_id_max; fem_id_index++)
    {
        sal_strncpy_s(data.name, dnx_algo_fem_action_template_id_get(unit, fem_id_index),
                      SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.nof_resource_pool_indexes = dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->nof_fem_id;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_PMF_A_FEM_PGM_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);

    SHR_IF_ERR_EXIT(algo_field_info_sw.ipmf_a_fem_pgm_id.alloc(unit, data.nof_resource_pool_indexes));
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_USER_QUAL_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_USER_ACTION_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_GROUP_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_SEXEM_APP_DB_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_LEXEM_APP_DB_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_ACE_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_ACE_KEY, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    sal_memset(&data, 0, sizeof(data));
    data.first_element = 0;
    data.nof_elements = (dnx_data_field.tcam.nof_big_banks_get(unit) *
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_ENTRY_TCAM_ACCESS_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
    ...
    int unit)
{
    ...
    data.flags = 0;
    sal_strncpy_s(data.name, DNX_ALGO_FIELD_KBP_OPCODE_CS_PROFILE_ID, SW_STATE_ALGO_RES_MNGR_MAX_NAME_LENGTH);
...
/**
* \brief
*  Init all required resources for Field module functionality
* \param [in] unit  - Device ID
* \return
*   shr_error_e - Error Type
* \remark
    ...
    int unit)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_field_action_sw_state_init(unit));
    /*
