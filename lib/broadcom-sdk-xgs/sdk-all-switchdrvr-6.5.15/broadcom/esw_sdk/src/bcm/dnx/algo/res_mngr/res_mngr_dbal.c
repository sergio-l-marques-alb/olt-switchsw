    ...
    uint8 *is_allocated)
{
    ...
    if (*is_allocated && !is_alloc_table_any)
    {
        ...
        int rv;
        rv = dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_get(unit, out_lif, &rec_table_id, &rec_result_type,
                                                                      NULL);
    ...
    uint8 *is_allocated)
{
    ...
    *is_allocated = 0;
    ...
    rv = algo_l3_db.source_address_table_allocation.egress_source_address_table.profile_data_get
        (unit, core_id, index, &ref_count, (void *) &source_address_entry);
    ...
    int *in_lif)
{
    ...
    {
        *in_lif = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
    }

    while ((*in_lif) != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT && (!is_found))
    {
        ...
        if ((*in_lif) != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
        {
    ...
    int *out_lif)
{
    ...
    while ((*out_lif) != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT && (!is_found))
    {
        int next_result_type = -1;
        is_found = FALSE;
        SHR_IF_ERR_EXIT(lif_mngr_db.egress_local_lif.get_next(unit, current_bank, out_lif));

        if ((*out_lif) != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
        {
        ...
        else
        {
            ...
            if (current_bank != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
            {
    ...
    int *ecmp_index)
{
     ...
     */
    if (*ecmp_index == SW_STATE_ALGO_RES_ILLEGAL_ELEMENT && is_extended == 0)
    {
    ...
    int *index)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.
                    egress_source_address_table.get_next(unit, core_id, &index_to_check));

    while (index_to_check != SW_STATE_ALGO_TEMPLATE_ILLEGAL_PROFILE)
    {
        switch (table_id)
        {
            case DBAL_TABLE_MAC_SOURCE_ADDRESS:
    ...
    (*index) = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
    ...
    int *index)
{
    ...
    /*
     * Indicate how many banks with bigger size are there before the sub_resource_index bank
     * Banks with bigger size are those which bank_ID % 5 = 0, they hold 2 more FEC addresses than the rest.
     */
    nof_big_banks_passed = ALGO_L3_NOF_BIG_BANKS_BEFORE_CURRENT(sub_resource_index);
    fec_index = (*index * 2) - (nof_big_banks_passed * dnx_data_l3.fec.max_fec_stage_map_size_get(unit) +
                                (sub_resource_index -
                                 nof_big_banks_passed) * dnx_data_l3.fec.fec_stage_map_size_get(unit));
    ...
    while (sub_resource_index < nof_fec_banks && is_found == 0)
    {
        ...
        {
            sub_resource_index++;
            fec_index = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
            continue;
        }
        /** Set valid value to fec_index before calling get_next. */
        if (fec_index == SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
        {
            fec_index = 0;
        }
    ...
    if (fec_index != SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
    {
        /** Recalculate the number of bigger banks that are found before the sub_resource_index bank. */
        nof_big_banks_passed = ALGO_L3_NOF_BIG_BANKS_BEFORE_CURRENT(sub_resource_index);
        *index = (fec_index + nof_big_banks_passed * dnx_data_l3.fec.max_fec_stage_map_size_get(unit) +
                  (sub_resource_index - nof_big_banks_passed) * dnx_data_l3.fec.fec_stage_map_size_get(unit)) / 2;
    }
    else
    {
        *index = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
    ...
    int is_alloc)
{
    ...
    if (is_alloc)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.allocate_single
                        (unit, _SHR_CORE_ALL, SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID, &source_address_entry, NULL,
                         index, &is_first));
    }
    else
    {
        SHR_IF_ERR_EXIT(algo_l3_db.source_address_table_allocation.egress_source_address_table.free_single
                        (unit, _SHR_CORE_ALL, (*index), &is_first));
    }
...
{
    int fec_id = *index * 2;
    int bank_id[ALGOL_L3_NOF_FEC_BANK_TYPES] = { -1, -1 };
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate
                    (unit, &fec_id, flags, hierarchy, DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_NO_PROT_NO_STAT, bank_id));
    /** If a valid bank ID was returned, then we need to set the hierarchy mapping for this bank */
    SHR_IF_ERR_EXIT(dnx_l3_fec_hierarchy_stage_map_set(unit, bank_id, hierarchy));
    ...
    int index)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_deallocate(unit, index * 2, bank_id));
    /** If a valid bank ID was returned, then we need to reset the hierarchy mapping for this bank */
    SHR_IF_ERR_EXIT(dnx_l3_fec_hierarchy_stage_map_set(unit, bank_id, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY));
...
dnx_algo_res_dbal_oamp_rmep_db_get_next(
    ...
    int *index)
{
    int self_contained_threshold, short_entries_in_self_cont = 2, index_in_full_pool;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_IF_ERR_EXIT(algo_oam_db.oam_rmep_id_below_threshold.get_next(unit, index));
    ...
    index_in_full_pool = ((*index) + 1 - self_contained_threshold) / short_entries_in_self_cont;

    /** Get next in this pool */
    SHR_IF_ERR_EXIT(algo_oam_db.oam_rmep_id_above_threshold.get_next(unit, &index_in_full_pool));

    if (index_in_full_pool == SW_STATE_ALGO_RES_ILLEGAL_ELEMENT)
    {
        *index = SW_STATE_ALGO_RES_ILLEGAL_ELEMENT;
        SHR_EXIT();
    }

    *index = self_contained_threshold + short_entries_in_self_cont * index_in_full_pool;
    ...
    int result_type)
{
    ...
    SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_set(unit, index, table_id, result_type));
    ...
    int result_type)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.rif_alloc.
                    rif_res_manager.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, (void *) &resource_info,
                                                    &index));
    ...
    switch (field_type)
    {
        ...
        case DBAL_FIELD_TYPE_DEF_QOS_PROFILE:
        {
            SHR_IF_ERR_EXIT(algo_qos_db.
                            ingress_qos.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, &index));
            break;
        }
        case DBAL_FIELD_TYPE_DEF_ECN_QOS_PROFILE:
        {
            SHR_IF_ERR_EXIT(algo_qos_db.
                            ingress_qos_ecn.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, &index));
            break;
        }
        case DBAL_FIELD_TYPE_DEF_RMEP_INDEX:
        {
            SHR_IF_ERR_EXIT(dnx_oam_remote_endpoint_id_alloc(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, &index));
            break;
        }
        case DBAL_FIELD_TYPE_DEF_OAMP_ENTRY_ID:
        {
