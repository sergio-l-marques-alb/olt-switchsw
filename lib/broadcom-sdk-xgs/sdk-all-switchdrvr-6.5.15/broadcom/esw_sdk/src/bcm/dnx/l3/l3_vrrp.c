...
/**
 * \brief - default protocl groups that are set on init.
 * Group 3 holds IPv6
 * Group 1 holds IPv4
 ...
 */
#define DNX_L3_VRRP_FFC_DATA_TOTAL_WIDTH 109

#define DNX_L3_VRRP_FFC_MAC_WIDTH 48
#define DNX_L3_VRRP_FFC_MAC_OFFSET (DNX_L3_VRRP_FFC_DATA_TOTAL_WIDTH - DNX_L3_VRRP_FFC_MAC_WIDTH)

#define DNX_L3_VRRP_FFC_VSI_WIDTH 18
#define DNX_L3_VRRP_FFC_VSI_OFFSET    (DNX_L3_VRRP_FFC_MAC_OFFSET - DNX_L3_VRRP_FFC_VSI_WIDTH)

#define DNX_L3_VRRP_FFC_IN_LIF_WIDTH 22
#define DNX_L3_VRRP_FFC_IN_LIF_OFFSET    (DNX_L3_VRRP_FFC_VSI_OFFSET - DNX_L3_VRRP_FFC_IN_LIF_WIDTH)

#define DNX_L3_VRRP_FFC_IN_PORT_WIDTH 8
#define DNX_L3_VRRP_FFC_IN_PORT_OFFSET    (DNX_L3_VRRP_FFC_IN_LIF_OFFSET - DNX_L3_VRRP_FFC_IN_PORT_WIDTH)

#define DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH 5
#define DNX_L3_VRRP_FFC_LAYER_PROTOCOL_OFFSET    (DNX_L3_VRRP_FFC_IN_PORT_OFFSET - DNX_L3_VRRP_FFC_LAYER_PROTOCOL_WIDTH)

#define DNX_L3_VRRP_FFC_GEN_VAR_WIDTH 8
#define DNX_L3_VRRP_FFC_GEN_VAR_OFFSET    (DNX_L3_VRRP_FFC_LAYER_PROTOCOL_OFFSET - DNX_L3_VRRP_FFC_GEN_VAR_WIDTH)
...
/**
 * \brief - Construct mapped port from pp_port and core
 */
#define DNX_L3_VRRP_EXEM_MAPPED_PORT_SET(mapped_port, core, pp_port, core_offset)  \
    (mapped_port = (core << core_offset) | pp_port)
/*
 * }
 */
/*
    ...
    uint32 *internal_flags)
{
    ...
    *internal_flags = 0;
    ...
    {
        SHR_BITSET(internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_ARP);
    }
    if (_SHR_IS_FLAG_SET(api_flags, BCM_SWITCH_L3_PROTOCOL_GROUP_MPLS))
    {
        SHR_BITSET(internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS);
    }
    if (_SHR_IS_FLAG_SET(api_flags, BCM_SWITCH_L3_PROTOCOL_GROUP_MIM))
    {
        SHR_BITSET(internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_ETHERNET);
    }
    if (_SHR_IS_FLAG_SET(api_flags, BCM_SWITCH_L3_PROTOCOL_GROUP_FCOE))
    {
        SHR_BITSET(internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_FCOE);
    }
    ...
    uint32 *api_flags)
{
    ...
    *api_flags = 0;
    ...
    if (SHR_BITGET(&internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_IPV6))
    {
        *api_flags |= BCM_SWITCH_L3_PROTOCOL_GROUP_IPV6;
    }
    if (SHR_BITGET(&internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_ARP))
    {
        *api_flags |= BCM_SWITCH_L3_PROTOCOL_GROUP_ARP;
    }
    if (SHR_BITGET(&internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_MPLS))
    {
        *api_flags |= BCM_SWITCH_L3_PROTOCOL_GROUP_MPLS;
    }
    if (SHR_BITGET(&internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_ETHERNET))
    {
        *api_flags |= BCM_SWITCH_L3_PROTOCOL_GROUP_MIM;
    }
    if (SHR_BITGET(&internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_FCOE))
    {
    ...
    uint32 *internal_flags)
{
    ...
    *internal_flags = 0;
    ...
    if (_SHR_IS_FLAG_SET(api_flags, BCM_L3_VRRP_IPV6))
    {
        SHR_BITSET(internal_flags, DBAL_ENUM_FVAL_LAYER_TYPES_IPV6);
    }
    ...
    uint32 members_bmp)
{
    ...
    for (ii = 0; ii < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); ii++)
    {
        ...
        curr_group_members &= ~members_bmp;

        if (ii == 0)
        {
            /** Set all old members of the group that are not in the group anymore to group 0. */
            curr_group_members |= (old_group_members & ~members_bmp);
        }
    ...
    /** Write to HW  */
    {
        ...
        SHR_BIT_ITER(&members_bmp, DBAL_NOF_ENUM_LAYER_TYPES_VALUES, member)
        {
...
{
    int i;
    uint32 valid_members;
    SHR_FUNC_INIT_VARS(unit);

    /** Initialize all valid protocols to be in group 0 in SW DB. HW is set by default. */
    DNX_L3_VRRP_PROTOCOL_GROUP_MEMBERS_ALL_VALID(&valid_members);
    ...
    int unit)
{
    ...
    /** set VRID_MY_MAC (VRRP protocol) */
    {
        ...
        /** set app db */
        SHR_IF_ERR_EXIT(dbal_tables_app_db_id_get(unit, DBAL_TABLE_EXEM_VRID_MY_MAC, &app_db_id, &app_id_size));
        ...
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(0, DNX_L3_VRRP_VRID_NOF_BITS - 1, DNX_L3_VRRP_FFC_MAC_OFFSET);
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(0, DNX_L3_VRRP_FFC_MAC_WIDTH - 1, DNX_L3_VRRP_FFC_MAC_OFFSET);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 0, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 0, TRUE);

        /** Set FFC 1 - source port */
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(DNX_L3_VRRP_FFC_MAC_WIDTH, DNX_L3_VRRP_FFC_IN_PORT_WIDTH - 1,
                                              DNX_L3_VRRP_FFC_IN_PORT_OFFSET);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 1, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 1, TRUE);

        /** Set FFC 2 - source port core (1 bit) */
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(DNX_L3_VRRP_FFC_MAC_WIDTH + DNX_L3_VRRP_FFC_IN_PORT_WIDTH, 1 - 1,
                                              DNX_L3_VRRP_FFC_GEN_VAR_OFFSET);
        ...
        /*
         * Workaround that indicates the core of the port.
    ...
    /** set PER_VSI_MULTIPLE_MY_MAC_MSB_0 (multiple my mac based on VSI and MAC address) */
    {
        ...
        exem_ffc = DNX_L3_VRRP_EXEM_FFC_BUILD(0, DNX_L3_VRRP_FFC_MAC_WIDTH - 2, DNX_L3_VRRP_FFC_MAC_OFFSET);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_FFC_INSTRUCTION, 0, exem_ffc);
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_VRRP_KBR_VALID, 0, TRUE);
    ...
    uint8 is_add)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vrrp_vsi_vrid_bitmap_get(unit, vlan, &vrid_bitmap));
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vrrp_vsi_vrid_bitmap_set(unit, vlan, vrid_bitmap));
    ...
    uint32 *group_mask)
{
    ...
    *group_mask = DNX_L3_VRRP_PROTOCOL_GROUP_MASK_FULL;
    ...
    for (i = 0; i < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); i++)
    {
         ...
         */
        if (internal_flags & members_flags)
        {
    ...
    if (exact_match && (existing_members != members_flags))
    {
    ...
    uint32 *tcam_index)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.vrrp_vsi_tcam_table.allocate_single
                    (unit, _SHR_CORE_ALL, 0, tcam_info, NULL, (int *) tcam_index, &is_first));
    ...
    if (is_exist == TRUE)
    {
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.
                        vrrp_vsi_tcam_table.free_single(unit, _SHR_CORE_ALL, *tcam_index, &is_last));
    ...
    algo_dnx_l3_vrrp_tcam_key_t * tcam_info)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.
                    vrrp_exem_tcam_table.profile_get(unit, _SHR_CORE_ALL, tcam_info, &tcam_index));
    /** deallocate the profile (tcam_index) */
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.
                    vrrp_exem_tcam_table.free_single(unit, _SHR_CORE_ALL, tcam_index, &is_last));
    ...
    int vlan)
{
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.
                    vrrp_vsi_tcam_table.profile_get(unit, _SHR_CORE_ALL, tcam_info, &tcam_index));
...
/**
 * \brief - Add handle for a VSI based VRRP
 */
static shr_error_e
dnx_l3_vrrp_vsi_based_vrid_add_remove(
    int unit,
    uint32 flags,
    bcm_vlan_t vlan,
    uint32 vrid,
    ...
    uint32 vrid)
{
    ...
    /** create handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EXEM_VRID_MY_MAC, &entry_handle_id));
    ...
    int index)
{
    ...
    /** clear HW EXEM entry */
    {
        ...
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_exem_multi_my_mac_get(unit, index, &result_type, &vlan, &core, &pp_port, mac_da));
    ...
    bcm_mac_t mac_da)
{
    ...
    *core = *pp_port = *vlan = 0;
    ...
    if (*result_type == DBAL_RESULT_TYPE_L3_VRRP_EXEM_DB_SW_VSI)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_VSI, INST_SINGLE, &vsi);
        *vlan = vsi;
    }
    else if (*result_type == DBAL_RESULT_TYPE_L3_VRRP_EXEM_DB_SW_PORT)
    {
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_PP_PORT, INST_SINGLE, pp_port);
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_CORE, INST_SINGLE, (uint32 *) core);
    }
    else
    {
    ...
    /** get mac address */
    dbal_entry_handle_value_field_arr8_get(unit, entry_handle_id, DBAL_FIELD_L2_MAC, INST_SINGLE, mac_da);
    ...
    uint32 vrid)
{
    ...
    /** Allocate TCAM profile, there is a dedicated tcam index for each protocol (ipv4, ipv6, or both) */
    if ((_SHR_IS_FLAG_SET(flags, BCM_L3_VRRP_IPV4) == TRUE) && (_SHR_IS_FLAG_SET(flags, BCM_L3_VRRP_IPV6) == TRUE))
    {
        tcam_index = dnx_data_l3.vrrp.exem_vrid_ip_tcam_index_get(unit);
    }
    else if (_SHR_IS_FLAG_SET(flags, BCM_L3_VRRP_IPV4) == TRUE)
    {
    ...
    SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.vrrp_exem_tcam_table.allocate_single
                    (unit, _SHR_CORE_ALL, SW_STATE_ALGO_TEMPLATE_ALLOCATE_WITH_ID, &tcam_data, NULL, &tcam_index,
                     &is_first));
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vrrp_exem_vrid_my_mac_add(unit, vlan, vrid));
...
/**
 * \brief - Delete handle for an EXEM based VRRP
 */
static shr_error_e
    ...
    uint32 vrid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l3_vrrp_exem_vrid_my_mac_remove(unit, vlan, vrid));
    ...
    uint32 vrid)
{
    ...
    {
        /** VSI based VRRP */
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_vsi_based_vrid_add_remove(unit, flags, vlan, vrid, TRUE));
    }
    ...
    uint32 vrid)
{
    ...
    {
        /** VSI based VRRP */
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_vsi_based_vrid_add_remove(unit, flags, vlan, vrid, FALSE));
    }
...
/**
 * \brief - Delete all handle for an VSI based VRRP
 */
static int
    ...
    bcm_vlan_t vlan)
{
    ...
    SHR_BIT_ITER(&vrid_bitmap, dnx_data_l3.vrrp.nof_vsi_tcam_entries_get(unit), tcam_index)
    {
        ...
        SHR_IF_ERR_EXIT(algo_l3_db.algo_l3_templates.vrrp_vsi_tcam_table.profile_data_get
                        (unit, _SHR_CORE_ALL, tcam_index, &ref_count, (void *) &tcam_data));
        ...
        if (is_vrrp == TRUE)
        {
            SHR_IF_ERR_EXIT(dnx_l3_vrrp_vsi_based_vrid_add_remove(unit, flags, vlan, tcam_data.mac_da[5], FALSE));
        }
    }
...
/**
 * \brief - Delete all handle for an EXEM based VRRP
 */
static int
    ...
    uint32 *count)
{
    ...
    if (alloc_size < 1 || alloc_size > dnx_data_l3.vrrp.nof_vsi_tcam_entries_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Given array size (%d) is invalid", alloc_size);
    }
...
/**
 * \brief - Get handle for an EXEM based VRRP
 */
static shr_error_e
    ...
    uint32 *count)
{
    ...
    {
        uint32 entry_handle_id, key_rule_value, key_rule_mask, vrid;
        int is_end;
...
/**
 * \brief - Get handle for an VSI based VRRP
 */
static shr_error_e
    ...
    uint32 *count)
{
    ...
    algo_dnx_l3_vrrp_tcam_key_t tcam_data;
    int tcam_index = 0;
    uint32 vrid_bitmap, members, api_flags;
    uint8 is_vrrp;
...
dnx_l3_vrrp_protocol_group_get(
    ...
    bcm_l3_protocol_group_id_t * group_id)
{
    uint32 internal_flags, requested_flags, group_mask = 0;
...
dnx_l3_vrrp_protocol_group_set_verify(
    ...
    bcm_l3_protocol_group_id_t group_id)
{
    uint32 internal_group_members, curr_group_members, count;
    int i;
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (i = DNX_L3_VRRP_L3_PROTOCOL_GROUP_ID_LOWEST_VALID; i < dnx_data_l3.vrrp.nof_protocol_groups_get(unit); i++)
    {
        ...
        if (internal_group_members & curr_group_members)
        {
            if (count > 0)
            {
