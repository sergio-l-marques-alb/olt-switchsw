    ...
    dbal_enum_value_field_meter_crps_smem_src_types_e src_type)
{
    ...
    else
    {
        ...
        {
            /** calcultae bank id - engine 16 > banks 0&6, engine 17 > banks 1&7, ...engine 21 > banks 5&11 */
            bank_id = DNX_CRPS_METER_SHMEM_BANK_ID_CALC(unit, engine_id, base_bank);
    ...
    dbal_enum_value_field_meter_crps_smem_src_types_e * src_type)
{
    ...
    else
    {
        ...
        /** calcultae bank id - engine 16 > banks 0&6, engine 17 > banks 1&7, ...engine 21 > banks 5&11 */
        bank_id = DNX_CRPS_METER_SHMEM_BANK_ID_CALC(unit, engine_id, section);
        /** Taking a handle */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_CRPS_METER_BIG_ENGINE_SHARED_MEM, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_CORE_ID, core_id);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_BANK_ID, bank_id);
