    ...
    uint32 alloc_flags)
{
    ...
    int region_index = sub_resource_index;

    sw_state_resource_tag_bitmap_create_info_t res_tag_bitmap_create_info;

    SHR_FUNC_INIT_VARS(unit);

    nof_remote_cores = dnx_data_sch.flow.nof_remote_cores_get(unit, core_id, region_index)->val;
    ...
    res_tag_bitmap_create_info.count = create_data->nof_elements / nof_remote_cores;
    res_tag_bitmap_create_info.grain_size = tag_grain;
    res_tag_bitmap_create_info.max_tag_value = max_tag_value;
    /*
     * we don't use allocation with ignore tag.
     * if this functionality is required, SW_STATE_RESOURCE_TAG_BITMAP_CREATE_ALLOW_IGNORING_TAG should be specified
     */
    ...
    int element)
{
    ...
    res_tag_bitmap_free_info.flags |= SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_SPARSE;
    ...
    uint8 *is_allocated)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_res_simple_bitmap_is_allocated(unit, module_id, res_tag_bitmap, base_element,
                                                            is_allocated));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *element)
{
    ...
    res_tag_bitmap_alloc_info.flags |= (with_id) ? SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_WITH_ID : 0;

    /** Ignore Tag */
    if (alloc_info_p->ignore_tag)
    {
        res_tag_bitmap_alloc_info.flags |= SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_IGNORE_TAG;
    }

    /** Check only */
    check_only = _SHR_IS_FLAG_SET(flags, SW_STATE_ALGO_RES_ALLOCATE_SIMULATION);
    if (check_only)
    {
        res_tag_bitmap_alloc_info.flags |= SW_STATE_RESOURCE_TAG_BITMAP_ALLOC_CHECK_ONLY;
    }
    ...
    rc = sw_state_resource_tag_bitmap_alloc(unit, module_id, res_tag_bitmap, res_tag_bitmap_alloc_info, element);
