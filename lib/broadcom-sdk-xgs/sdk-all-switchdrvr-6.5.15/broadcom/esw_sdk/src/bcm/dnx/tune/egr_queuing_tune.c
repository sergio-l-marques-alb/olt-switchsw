    ...
    int is_remove)
{
    ...
    egr_rate = if_speed / nof_channels;
    ...
    SHR_IF_ERR_EXIT(bcm_dnx_cosq_gport_bandwidth_set
                    (unit, gport, 0, 0, egr_rate, commit_changes ? 0 : BCM_COSQ_BW_NOT_COMMIT));
    ...
    for (cosq = 0; cosq < num_priorities; cosq++)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_dnx_cosq_gport_bandwidth_set(unit, gport, cosq, 0, egr_rate, 0));
    }
    ...
    if (num_priorities >= dnx_data_egr_queuing.params.tcg_min_priorities_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(bcm_dnx_cosq_gport_bandwidth_set(unit, gport, 0, 0, egr_rate, 0));
    }
