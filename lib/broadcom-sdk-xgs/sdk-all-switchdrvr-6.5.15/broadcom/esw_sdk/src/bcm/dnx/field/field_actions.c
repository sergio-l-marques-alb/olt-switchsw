    ...
    dnx_field_fes_chosen_mask_t chosen_mask)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FIELD_FES_VALID_BITS, INST_SINGLE,
    ...
    dnx_field_fes_chosen_mask_t * chosen_mask_p)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                    (unit, entry_handle_id, DBAL_FIELD_FIELD_FES_VALID_BITS, INST_SINGLE,
 ...
 * \param [in] action_input_select_p -
 ...
 *    The pointer would be loaded by the calling function with the identifier of the FES_KEY_SELECT which 
 *    carries this range of bits.
 * \param [in] required_shift_p -
 *    Pointer to uint, to be loaded by the calling function with the the number of shifts required on the
 *    FES_KEY_SELECT to get the action bits to the lsb location.
 * \param [in] found_p -
 *    Pointer to int to be loaded by the calling function with a boolean: If matching KEY_SELECT
 ...
 */
static shr_error_e
dnx_field_actions_fes_key_select_get_verify(
    int unit,
    ...
    int *found_p)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_field_actions_fes_key_select_get_verify
                          (unit, actions_block, field_stage, action_size, action_lsb, action_input_select_p,
                           action_input_select_p, found_p));
    ...
    *found_p = FALSE;
    ...
    if (action_size == 0)
    {
        switch (field_stage)
        {
            ...
            case DNX_FIELD_STAGE_EPMF:
            {
                *action_input_select_p =
                    dnx_data_field.stage.stage_info_get(unit, field_stage)->fes_key_select_for_zero_bit;
            ...
            case DNX_FIELD_STAGE_ACE:
            {
                ...
                /** Note ACE doesn't actually have a key select.*/
                *action_input_select_p = 0;
                
                *required_shift_p = dnx_data_field.ace.fes_shift_for_zero_bit_get(unit);
                break;
            }
            default:
            {
    ...
    else
    {
        ...
        for (key_select_ndx = 0; key_select_ndx < DNX_DATA_MAX_FIELD_EFES_MAX_NOF_KEY_SELECTS_PER_FIELD_IO;
             key_select_ndx++)
        {
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
         ...
         && actions_info[action_index].dnx_action != DNX_FIELD_ACTION_INVALID; action_index++)
    {
        ...
        fes_inst_info[inst_ndx].common_info.fes2msb_info[0].polarity = DNX_FIELD_ACTION_FES_VALID_BIT_POLARITY;
        /*
        ...
        SHR_IF_ERR_EXIT(dnx_field_actions_fes_key_select_get
                        (unit,
                         ...
                         action_size_with_valid,
                         actions_info[action_index].lsb, &action_input_select, &required_shift, &found));
        if (!found)
        {
            /*
            ...
            SHR_IF_ERR_EXIT_WITH_LOG(_SHR_E_PARAM,
                                     ...
                                     actions_info[action_index].lsb, action_size_with_valid);
        }
 ...
 */
shr_error_e
dnx_field_actions_single_fes_add(
    int unit,
    dnx_field_stage_e field_stage,
    ...
    dnx_field_actions_single_fes_add_t * fes_add_info_p)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(fes_add_info_p, _SHR_E_PARAM, "fes_add_info_p");

    /*
     * Initialize actions_info and actions_attach_info
     */
    for (action_ndx = 0; action_ndx < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP; action_ndx++)
    {
        SHR_IF_ERR_EXIT(dnx_field_action_in_group_info_t_init(unit, &(actions_info[action_ndx])));
        SHR_IF_ERR_EXIT(dnx_field_action_attach_info_t_init(unit, &(actions_attach_info[action_ndx])));
    }

    /*
     * Write the action to actions_info and actions_attach_info
     */
    actions_info[0].dnx_action = fes_add_info_p->dnx_action;
    actions_info[0].dont_use_valid_bit = fes_add_info_p->dont_use_valid_bit;
    actions_info[0].lsb = fes_add_info_p->lsb;
    actions_attach_info[0].priority = fes_add_info_p->priority;

    /*
     * Write the FES.
     */
    SHR_IF_ERR_EXIT(dnx_field_actions_calc_feses
                    (unit, field_stage, fes_add_info_p->field_io, actions_info, actions_attach_info, fes_inst_info));
    
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set(unit, field_stage, fg_id, context_id, TRUE, fes_inst_info));
 ...
 */
shr_error_e
dnx_field_actions_fes_data_2msb_feature_info(
    int unit,
    dnx_field_stage_e field_stage,
    dnx_field_group_t fg_id,
    dnx_field_context_t context_id,
...
{
    /**
     * List of the DATA qualifiers which are reserved for marking bits 62 and 63 of
     * the key_select FES_DATA.
     */
    static dnx_field_actions_feature_2msb_qualifier_map_info_t
        qualifiers_for_2msb_key_select_data[DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS] = {
        {DNX_FIELD_FES_DATA_2MSB_FEATURE_QUAL_BIT_1, DNX_FIELD_BASE_OF_2MSB},
        {DNX_FIELD_FES_DATA_2MSB_FEATURE_QUAL_BIT_2, DNX_FIELD_BASE_OF_2MSB + 1}
    };
    /*
     * Index for iterating over the fes_data_2msb qualifiers in info
     */
    int in_qual_ndx;
    /*
     * Index for iterating over the known fes_data_2msb qualifiers in qualifiers_for_2msb_key_select_data
     */
    int fd2msb_qual_ndx;
    uint8 valid_fd2msb_qual_found;
    uint8 invalid_fd2msb_qual_found;
    uint8 legal_qual_found;
    unsigned int bit_locations_2msb[DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS];
    unsigned int bit_width;
    unsigned int multiplier_for_2msb;
    uint32 bits_set;

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(feature_2msb_info_p, _SHR_E_PARAM, "feature_2msb_info_p");

    valid_fd2msb_qual_found = FALSE;
    invalid_fd2msb_qual_found = FALSE;
    bits_set = 0;
    /*
     * We only zero out the structure so that it will be predictable.
     */
    sal_memset(feature_2msb_info_p, 0x0, sizeof(*feature_2msb_info_p));
    /*
     * Check NULL qualifiers
     */
    for (in_qual_ndx = 0; in_qual_ndx < DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS; in_qual_ndx++)
    {
        legal_qual_found = FALSE;
        LOG_DEBUG_EX(BSL_LOG_MODULE, "Input qualifier for 2msb feature type: %d%s%s%s\r\n",
                     feature_2msb_quals[in_qual_ndx].qual_type, EMPTY, EMPTY, EMPTY);
        /*
         * Check if invalid
         */
        if (feature_2msb_quals[in_qual_ndx].qual_type == DNX_FIELD_QUAL_TYPE_INVALID)
        {
            invalid_fd2msb_qual_found = TRUE;
            legal_qual_found = TRUE;
        }
        else
        {
            /*
             ...
             */
            for (fd2msb_qual_ndx = 0; fd2msb_qual_ndx < DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS; fd2msb_qual_ndx++)
            {
                dnx_field_qual_t dnx_qual;
                dnx_qual =
                    DNX_QUAL(DNX_FIELD_QUAL_CLASS_USER, field_stage,
                             qualifiers_for_2msb_key_select_data[fd2msb_qual_ndx].qualifier_type);
                if (feature_2msb_quals[in_qual_ndx].qual_type == dnx_qual)
                {
                    valid_fd2msb_qual_found = TRUE;
                    legal_qual_found = TRUE;
                    /*
                     * If we found a valid qualifier, we set the bit at the quaax_
                     * lifier location shifted to the least
                     * significant bits (i.e. qualifier 62 will set bit 0, 63 will set bit 1)
                     */
                    bits_set |=
                        (((feature_2msb_quals[in_qual_ndx].val) & SAL_BIT(0)) <<
                         (qualifiers_for_2msb_key_select_data[fd2msb_qual_ndx].bit_on_fes_data -
                          DNX_FIELD_BASE_OF_2MSB));
                    break;
                }
            }
        }
        /*
         * If neither a known fes_data_2msb qualifier, nor invalid than an illegal qualifier type
         */
        if (legal_qual_found == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Illegal qualifier type for FES 2msb feature: %d\r\n",
                         feature_2msb_quals[in_qual_ndx].qual_type);
        }
    }
    /*
     * Checks if both valid and invalid qualifiers were found. Notice that the check is outside the loop as it is
     * assumed to be a very short loop
     */
    /*
     * NOTE: Possible to add support for only one bit (and then writing to two FES2msb instructions.
     */
    if (valid_fd2msb_qual_found == TRUE && invalid_fd2msb_qual_found == TRUE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Both qualifiers for FES 2msb feature must be either valid or invalid.\r\n");
    }
    /*
     * Now we check if we continue
     */
    if (valid_fd2msb_qual_found == FALSE)
    {
        /*
         * If not valid qualifiers found, return as such
         */
        feature_2msb_info_p->available = FALSE;
        /*
         * If we want to get rid of the argument feature_2msb_information_available we can set:
         */
        feature_2msb_info_p->bit_width = 0;
        LOG_DEBUG_EX(BSL_LOG_MODULE, "2msb feature information not found.%s%s%s%s\r\n", EMPTY, EMPTY, EMPTY, EMPTY);
    }
    else
    {
        /*
         * If we have qualifiers for 2msn feature
         */
        LOG_DEBUG_EX(BSL_LOG_MODULE, "2msb feature information found.%s%s%s%s\r\n", EMPTY, EMPTY, EMPTY, EMPTY);
        /*
         * Get the location on the key template to which the two qualifiers map
         */
        for (in_qual_ndx = 0; in_qual_ndx < DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS; in_qual_ndx++)
        {
            SHR_IF_ERR_EXIT(dnx_field_group_qualifier_info_get
                            (unit, fg_id, feature_2msb_quals[in_qual_ndx].qual_type,
                             &(bit_locations_2msb[in_qual_ndx]), &bit_width));
            if (bit_width != 1)
            {
                /*
                 * Each qualifier on '2msb feature' must be of one bit.
                 */
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Each qualifier on '2msb feature' must be of one bit. This qualifier (%d) is reported to "
                             "be %d bits wide.\r\n", feature_2msb_quals[in_qual_ndx].qual_type, bit_width);
            }

        }
        /*
         * Verifies that no two bits are not at the same place on the key. Technically covered by the next check
         * (sequentiality) still done to preserve the error message.
         */
        for (in_qual_ndx = 1; in_qual_ndx < DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS; in_qual_ndx++)
        {
            if (bit_locations_2msb[in_qual_ndx - 1] == bit_locations_2msb[in_qual_ndx])
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Bit locations (%d,%d) on qualifiers (%d,%d), assigned to '2msb', are the same.\r\n",
                             bit_locations_2msb[in_qual_ndx - 1], bit_locations_2msb[in_qual_ndx],
                             feature_2msb_quals[in_qual_ndx - 1].qual_type, feature_2msb_quals[in_qual_ndx].qual_type);
            }
        }
        /*
         * Verify that the bits are sequential. No longer a separate error for the case that they are equal. Needs to
         * be tested if the sequentiality requirement is canceled
         */
        for (in_qual_ndx = 1; in_qual_ndx < DNX_FIELD_NUM_2MSB_KEY_SELECT_QUALIFIERS; in_qual_ndx++)
        {
            if (bit_locations_2msb[in_qual_ndx - 1] + 1 != bit_locations_2msb[in_qual_ndx])
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Bit locations (%d,%d) on qualifiers (%d,%d), assigned to '2msb', are not sequential.\r\n",
                             bit_locations_2msb[in_qual_ndx - 1], bit_locations_2msb[in_qual_ndx],
                             feature_2msb_quals[in_qual_ndx - 1].qual_type, feature_2msb_quals[in_qual_ndx].qual_type);
            }
        }
        /*
         * Now verify the bits are located at one of the allowed MS bit locations.
         * Not a necessary check, at if it isn't it will later fail when allocating key select.
         * Assumes key select is chosen with DNX_FIELD_KEY_SELECT_BIT_GRANULARITY (32) bits granularity.
         */
        multiplier_for_2msb =
            (bit_locations_2msb[0] + DNX_FIELD_NUM_2MSB_BITS_ON_QUALIFIERS) / DNX_FIELD_KEY_SELECT_BIT_GRANULARITY;
        if ((multiplier_for_2msb * DNX_FIELD_KEY_SELECT_BIT_GRANULARITY)
            != (bit_locations_2msb[0] + DNX_FIELD_NUM_2MSB_BITS_ON_QUALIFIERS))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "Bit locations (%d,%d) on qualifiers, assigned to '2msb', are not located at the two ms "
                         "bits.\r\n", bit_locations_2msb[0], bit_locations_2msb[1]);
        }
        if (!((multiplier_for_2msb <= DNX_FIELD_NUM_LOCATIONS_OF_2MSB)
              && (multiplier_for_2msb >= DNX_FIELD_MIN_LOCATIONS_OF_2MSB)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "\r\n"
                         "Bit locations (%d,%d) on qualifiers, assigned to '2msb', are located too high on MS "
                         "direction.\r\n", bit_locations_2msb[0], bit_locations_2msb[1]);
        }
        /*
         * Returning the 2msb feature information
         */
        feature_2msb_info_p->available = TRUE;
        feature_2msb_info_p->lsb = bit_locations_2msb[0];
        feature_2msb_info_p->bit_width = DNX_FIELD_NUM_2MSB_BITS_ON_QUALIFIERS;
        feature_2msb_info_p->validity_condition = bits_set;
    }
    ...
    dnx_field_actions_fes_info_t fes_inst_info[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP])
{
    ...
    dnx_algo_field_action_fes_alloc_out_t alloc_result;
    ...
    /*
     * Perform verifications only after the pointers to have memory allocated for (current_state_p) are set to NULL. 
     */
    SHR_NULL_CHECK(fes_inst_info, _SHR_E_PARAM, "fes_inst_info");
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_convert_fes_instructions_to_alloc
                    (unit, field_stage, fg_id, context_id, fes_inst_info, &fes_fg_in));
    ...
    if (fes_fg_in.nof_fes_quartets > 0)
    {
        ...
        /*
         * Direct extraction does not perform sharing, since sharing is still idone by place in field group in 
         * direct extraction FES configuration can change between contexts for the same field group.
         */
        allow_fes_pgm_sharing = !is_dir_ext;
        ...
        SHR_IF_ERR_EXIT(dnx_field_actions_fes_array_write
                        (unit, field_stage, fg_id, context_id, fes_inst_info, &alloc_result));
    }
    ...
    unsigned int nof_fes_quartets)
{
    ...
    int found_an_all_invalid_element;
    dnx_field_action_type_t action_type_invalid;
    dnx_field_action_t dnx_action_invalidate_next;
    dnx_field_action_type_t invalidate_next_action_type;
    uint8 action_found[DNX_FIELD_ACTION_ID_NOF(DNX_FIELD_STAGE_ACE)];
    ...
    {
        action_found[action_type_ndx] = FALSE;
    }
     ...
     */
    SHR_IF_ERR_EXIT(dnx_field_map_field_id_dnx_action
                    (unit, DNX_FIELD_STAGE_ACE, DBAL_FIELD_INVALIDATE_NEXT, &dnx_action_invalidate_next));
    SHR_IF_ERR_EXIT(dnx_field_map_dnx_to_action_type
                    (unit, DNX_FIELD_STAGE_ACE, dnx_action_invalidate_next, &invalidate_next_action_type));
         ...
         && found_an_all_invalid_element == FALSE; fes_quartet_ndx++)
    {
        ...
        for (fes2msb_ndx = 0; fes2msb_ndx < nof_fes2msb; fes2msb_ndx++)
        {
            ...
            else if (action_found[fes_inst_info[fes_quartet_ndx].common_info.fes2msb_info[fes2msb_ndx].action_type])
            {
            ...
            {
                action_found[fes_inst_info[fes_quartet_ndx].common_info.fes2msb_info[fes2msb_ndx].action_type] = TRUE;
            }
            if (fes_inst_info[fes_quartet_ndx].common_info.fes2msb_info[fes2msb_ndx].action_type != action_type_invalid)
            {
                found_an_all_invalid_element = FALSE;
    ...
    dnx_field_context_t context_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_field_action_fes_dealloc
                    (unit, field_stage, fg_id, context_id, &current_state_dealloc, &dealloc_result));
    /*
     * Delete from HW.
     */
    ...
    dnx_field_actions_fes_context_group_get_info_t * fes_group_info_p)
{
    ...
    unsigned int nof_fes_quartets;
    int index_filled[DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP];
    ...
    /*
     * Verify that the number of FES quartets in the context does not exceed the maximum number of FESes (and thus the
     * array index). 
     */
    ...
    /*
     * Initialize the output variables, done only for predictability.
     */
    sal_memset(fes_group_info_p, 0x0, sizeof(*fes_group_info_p));

    /*
     * Initialize index_filled. Used only as a sanity check to ensure there are no "holes"
     * in the fes_group_info_p->fes_quartets and attach info (priorities) arrays.
     */
    ...
    {
        index_filled[fes_quartet_group_ndx] = FALSE;
    }
         ...
         fes_quartet_context_ndx++)
    {
        if (fes_context_info_p->fes_quartets[fes_quartet_context_ndx].fg_id == fg_id)
        {
             ...
             */
            if (place_in_fg >= DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
             ...
             */
            fes_group_info_p->fes_quartets[place_in_fg].fes_id =
                fes_context_info_p->fes_quartets[fes_quartet_context_ndx].fes_id;
            fes_group_info_p->fes_quartets[place_in_fg].fes_pgm_id =
                fes_context_info_p->fes_quartets[fes_quartet_context_ndx].fes_pgm_id;
            fes_group_info_p->fes_quartets[place_in_fg].priority =
                fes_context_info_p->fes_quartets[fes_quartet_context_ndx].priority;
            fes_group_info_p->fes_quartets[place_in_fg].key_select =
                fes_context_info_p->fes_quartets[fes_quartet_context_ndx].key_select;
            sal_memcpy(fes_group_info_p->fes_quartets[place_in_fg].fes2msb_info,
                       fes_context_info_p->fes_quartets[fes_quartet_context_ndx].fes2msb_info,
                       sizeof(fes_group_info_p->fes_quartets[place_in_fg].fes2msb_info));

            /*
             * Mark the place of the FES quartet as filled and increment nof_fes_quartets.
             */
            index_filled[place_in_fg] = TRUE;
            nof_fes_quartets++;
        }
    }
    /*
     ...
     */
    if (nof_fes_quartets > DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Number of FES quartets found belonging to the field group %d "
                     "exceeds the maximum number of actions per field group.\r\n", fg_id);
    }
    /*
     * If we found FES quartets, update the number of FES quartets found.
     */
    fes_group_info_p->nof_fes_quartets = nof_fes_quartets;
    /*
     * Sanity check, check that there are no uninitialized "holes" in the filled array.
     */
    for (fes_quartet_group_ndx = 0; fes_quartet_group_ndx < nof_fes_quartets; fes_quartet_group_ndx++)
    {
        if (index_filled[fes_quartet_group_ndx] == FALSE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Missing action %d in field group %d.\r\n", fes_quartet_group_ndx, fg_id);
        }
    }
    ...
    int *found_p)
{
    ...
    *found_p = FALSE;
    ...
    for (key_select_ndx = 0; key_select_ndx < DNX_DATA_MAX_FIELD_FEM_MAX_NOF_KEY_SELECTS_PER_FIELD_IO; key_select_ndx++)
    {
        if (field_io_key_select_info_p->num_bits[key_select_ndx] != 0)
        {
            required_shift = action_lsb - field_io_key_select_info_p->lsb[key_select_ndx];
    ...
    dnx_field_fem_action_entry_t * struct_p)
{
    ...
    struct_p->fem_adder = DNX_FIELD_FEM_ADDER_INVALID;
exit:
    SHR_FUNC_EXIT;
}
/**
    ...
    if (fem_id < dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->num_fems_with_short_action)
    {
        ...
        for (bit_in_fem_action_index = 0; bit_in_fem_action_index < bit_in_fem_action_min; bit_in_fem_action_index++)
        {
            ...
            if ((((unsigned int) fem_bit_format >= (unsigned int) (DNX_FIELD_FEM_BIT_FORMAT_NUM)) ||
                 ((unsigned int) fem_bit_format < (unsigned int) (DNX_FIELD_FEM_BIT_FORMAT_FIRST))) &&
        ...
        if (fem_action_entry_p->fem_adder != DNX_FIELD_FEM_ADDER_INVALID)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "\r\n"
                         "This is 'FEM id' %d which has only %d bits and no 'adder'. However, 'adder' is NOT marked as 'invalid' (%d). Quit.\r\n",
    ...
    {
        {
            ...
            for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
            {
                if ((ignore_actions & SAL_BIT(fem_map_index)) == 0)
                {
                    ...
                    SHR_IF_ERR_EXIT(algo_field_info_sw.action_template_per_fem.free_single(unit, fem_id, _SHR_CORE_ALL,
                                                                                           (int) fem_map_index,
                                                                                           &last_reference));
                }
            }
                                                                                      ...
                                                                                      DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index])
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_field_stage_get(unit, fg_id, &field_stage));
    {
         ...
         * Use this loop, as well, to initialize fem_encoded_actions_destination[] to
         * 'invalid' values.
         */
        dnx_field_fem_map_index_t fem_map_index_max, fem_map_index;
    ...
    }
    {
...

        {
             ...
             * ECO is implemented, since the result is now placed alinged to full TCAM payload size
             */
            if (dnx_data_field.minor_changes_j2_a0_b0.tcam_result_flip_eco_get(unit))
            {
                dnx_field_action_length_type_e action_length_type;
                ...
                {
                    /*
                     * Increase offset to sync with new payload place 
                     */
                    input_offset += dnx_data_field.tcam.action_size_half_get(unit);
                }
            }
        }
...
exit:
    {
        ...
        pending_error = SHR_GET_CURRENT_ERR();
        if (revert_hardware)
        {
            /*
            ...
            {
                if (pending_error == _SHR_E_NONE)
                {
                    pending_error = shr_error;
    ...
    uint8 ignore_actions)
{
    ...
    }
    {
        ...
        for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
        {
             ...
             * the value to be written (which indicates 'invalid action'). Actually,
             * the value written is DNX_FIELD_ACTION_ID_INVALID
             *
                                                                                      ...
                                                                                      DNX_FIELD_STAGE_IPMF1)->nof_fem_map_index])
{
    ...
    }
    {
        ...
        for (fem_map_index = 0; fem_map_index < fem_map_index_max; fem_map_index++)
        {
            ...
            /*
             * If 'fem_adder' is marked as 'invalid' then, since we need to load something,
             * we set it to '0'. This will only affect FEMs 2/23
             */
            if (fem_action_entry[fem_map_index].fem_adder == DNX_FIELD_FEM_ADDER_INVALID)
            {
                dnx_field_fem_action_info.fem_action_entry.fem_adder = 0;
            }
            else
            {
    ...
    dnx_field_fem_action_info_t * dnx_field_fem_action_info_p)
{
    ...
    LOG_DEBUG_EX(BSL_LOG_MODULE, "Writing action info for: fem_id %d, fem_map_index %d fem_action %d (%s)\r\n",
                 ...
                 dnx_field_fem_action_info_p->fem_action_entry.fem_action, dnx_field_dnx_action_type_text(unit,
                                                                                                          dnx_action));
