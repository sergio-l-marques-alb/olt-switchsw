    ...
    bcm_gport_t * sysport)
{
     ...
     * Make sure that indeed gport managment managed to reterive the system port
     */
    ...
    int enable)
{
    ...
    if (modid != my_modid)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid parameter modid");
    }

    

     /** check if ptr for dram shut down exist. If not, return erro before enable traffic */
    if (enable == TRUE)
    {
    ...
    bcm_stk_modid_config_t * modid)
{
    ...
    /** Get Core 0 modid */
    SHR_IF_ERR_EXIT(dnx_stk_sys_modid_get(unit, 0, (uint32 *) &(modid->modid)));
 ...
 */
static int
dnx_stk_modid_config_set_verify(
    int unit,
    bcm_stk_modid_config_t * modid)
{
    int device_enabled;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_BADID, "modid is invalid");
    }
    ...
    bcm_stk_modid_config_t * modid)
{
    ...
    /** Verify */
    SHR_INVOKE_VERIFY_DNX(dnx_stk_modid_config_set_verify(unit, modid));

    /** Set sequential modids for each core */
    for (core = 0; core < nof_cores; core++)
    {
        /** set my_modid to HW */
        SHR_IF_ERR_EXIT(dnx_stk_sys_modid_set(unit, core, modid->modid + core));

        /** set fmq dedicated system port */
    ...
    {
        bcm_module_t modid_array[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES] = { 0 };

        for (core = 0; core < nof_cores; core++)
        {
            modid_array[core] = modid->modid + core;
        }
        SHR_IF_ERR_EXIT(dnx_fabric_mesh_device_id_set(unit, -1, nof_cores, modid_array));
    }
    ...
    int *num_modid)
{
    ...
    /** Get number of cores */
    *num_modid = dnx_data_device.general.nof_cores_get(unit);
    ...
    bcm_port_t gport)
{
    ...
    if (BCM_GPORT_IS_MODPORT(gport))
    {
        ...
        SHR_RANGE_VERIFY(tm_port, 0, dnx_data_port.general.nof_tm_ports_get(unit), _SHR_E_PARAM,
                         "tm_port %d in modport is out of range\n", tm_port);

        modid = BCM_GPORT_MODPORT_MODID_GET(gport);
        SHR_RANGE_VERIFY(modid, 0, dnx_data_device.general.nof_faps_get(unit), _SHR_E_PARAM,
 ...
 * This API configures mapping relecant just for local ports and mapping relevant for remote ports. 
    ...
    bcm_port_t gport)
{
    ...
    /*
     * Extract modid + tm port from gport 
     */
    if (BCM_GPORT_IS_MODPORT(gport))
    {
        tm_port = BCM_GPORT_MODPORT_PORT_GET(gport);
    ...
    else if (BCM_GPORT_IS_LOCAL(gport))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_stk_sys_modid_get(unit, core, &modid));
    }
    else if (gport == -1)
    {
     ...
     */
    SHR_IF_ERR_EXIT(dnx_stk_sys_sysport_modport_map_set(unit, _SHR_CORE_ALL, sysport_id, modid, tm_port));

    /*
     * Delete existing port - sys_port mapping - if exist
     */
    SHR_IF_ERR_EXIT(dnx_stk_sys_sysport_map_clear(unit, sysport));
     ...
     */
    {
        ...
        if (local_core != -1)
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_tm_to_logical_get(unit, local_core, tm_port, &logical_port));
    ...
    bcm_gport_t * gport)
{
    ...
    sysport_id = BCM_GPORT_SYSTEM_PORT_ID_GET(sysport);
    SHR_IF_ERR_EXIT(dnx_stk_sys_sysport_modport_map_get(unit, sysport_id, &modid, &tm_port));

    if ((modid == dnx_data_device.general.nof_faps_get(unit) - 1) &&
        (tm_port == dnx_data_port.general.nof_tm_ports_get(unit) - 1))
    {
        *gport = -1;
    }
