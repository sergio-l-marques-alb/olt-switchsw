    ...
    int base_q_pair)
{
    ...
    int ps_interface;
    uint32 allocation_bmp, prio_mode, ps, already_allocated, prio;
    ...
    if (allocation_bmp == 0 || (!already_allocated && prio_mode == out_port_priority && port_interface == ps_interface))
    {
        SHR_BITSET_RANGE(&allocation_bmp, prio, out_port_priority);
        SHR_IF_ERR_EXIT(dnx_egr_db.ps.allocation_bmap.set(unit, core, ps, allocation_bmp));
    ...
    int *base_q_pair)
{
    ...
    *base_q_pair = -1;
    ...
    for (ps = 0; ps < DNX_EGR_NOF_PS; ps++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egr_db.ps.if_idx.get(unit, core, ps, &ps_interface));
        SHR_IF_ERR_EXIT(dnx_egr_db.ps.prio_mode.get(unit, core, ps, &prio_mode));
        if (allocation_bmp && prio_mode == out_port_priority && port_interface == ps_interface)
        {
            /*
            ...
            SHR_BITOR_RANGE(&allocation_bmp, &allocated_hrs, 0, DNX_EGR_NOF_Q_PAIRS_IN_PS, &allocation_bmp);
    ...
    if (*base_q_pair == -1)
    {
        for (ps = 0; ps < DNX_EGR_NOF_PS; ps++)
        {
            ...
            if (allocation_bmp == 0)
            {
                ...
                SHR_BITSET_RANGE(&allocation_bmp, 0, out_port_priority);
                SHR_IF_ERR_EXIT(dnx_egr_db.ps.allocation_bmap.set(unit, core, ps, allocation_bmp));
                SHR_IF_ERR_EXIT(dnx_egr_db.ps.if_idx.set(unit, core, ps, port_interface));
                SHR_IF_ERR_EXIT(dnx_egr_db.ps.prio_mode.set(unit, core, ps, out_port_priority));
                *base_q_pair = ps * DNX_EGR_NOF_Q_PAIRS_IN_PS;
                break;
            }
        }
    }
