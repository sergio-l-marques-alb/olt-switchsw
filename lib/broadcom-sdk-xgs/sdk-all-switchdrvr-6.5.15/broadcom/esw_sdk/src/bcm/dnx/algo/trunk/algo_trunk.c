    ...
    int unit)
{
    ...
    /** create data */
    sal_memset(&create_data, 0, sizeof(sw_state_algo_template_create_data_t));
    create_data.flags = 0;
    create_data.first_profile = 0;
    create_data.nof_profiles = dnx_data_trunk.egress_trunk.nof_get(unit);
    create_data.max_references = dnx_data_device.general.nof_cores_get(unit)
    ...
    sal_strncpy(create_data.name, DNX_ALGO_TRUNK_EGRESSTRUNK_TEMPLATE, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    ...
    uint8 *is_first)
{
    ...
    SHR_IF_ERR_EXIT(algo_trunk_db.egress_trunk.allocate_single
                    (unit, _SHR_CORE_ALL, 0, (void *) &trunk_id, NULL, egress_trunk_id, is_first));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int egress_trunk_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_trunk_db.egress_trunk.free_single(unit, _SHR_CORE_ALL, egress_trunk_id, &is_last));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int *profile_found)
{
    ...
    rv = algo_trunk_db.egress_trunk.profile_get(unit, _SHR_CORE_ALL, (void *) &trunk_id, egress_trunk_id);
    ...
    bcm_trunk_t * trunk_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_trunk_db.
                    egress_trunk.profile_data_get(unit, _SHR_CORE_ALL, egress_trunk_id, &ref_count, (void *) trunk_id));
exit:
    SHR_FUNC_EXIT;
}
    ...
    dnx_algo_template_print_t * print_cb_data)
{
    ...
    DNX_ALGO_TEMPLATE_PRINT_CB_ADD_LINE(TEMPLATE_MNGR_PRINT_TYPE_INT, "trunk_id", *trunk_id, "trunk id", "0x%x");
    ...
    dnx_algo_trunk_member_lb_key_ranges_t * lb_key_ranges_array)
{
    ...
    *lb_key_ranges_array_size = 0;
    ...
    {
        if (unique_members_array[unique_member].is_enabled == FALSE)
        {
            continue;
        }

        /** get range of lb-keys to update for each member  */
        range = range_per_member * unique_members_array[unique_member].nof_replications;
        if (remaining > unique_members_array[unique_member].nof_replications)
        {
            range += unique_members_array[unique_member].nof_replications;
            remaining -= unique_members_array[unique_member].nof_replications;
        }
        else
        {
            range += remaining;
        ...
        lb_key_ranges_array[*lb_key_ranges_array_size].system_port = unique_members_array[unique_member].system_port;
