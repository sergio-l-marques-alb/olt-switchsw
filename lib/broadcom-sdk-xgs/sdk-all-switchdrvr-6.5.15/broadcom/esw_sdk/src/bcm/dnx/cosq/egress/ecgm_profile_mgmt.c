...
* \see function descriptions in .h file
*/
shr_error_e
dnx_ecgm_profile_mgmt_port_handle_init(
    int unit,
    bcm_port_t port,
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    /*
     * setting initial SW profile values to 0.
     */
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_port_profile.allocate_single
                    (unit, port_profile_handle->core, 0, &port_profile_handle->info, NULL,
                     &port_profile_handle->profile_id, &first_reference));
    ...
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_tm_port_to_port_profile_set
                    (unit, port_profile_handle->core, port_profile_handle->tm_port, port_profile_handle->base_q_pair,
                     port_profile_handle->profile_id));
    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_port_profile.profile_data_get
                    (unit, port_profile_handle->core, port_profile_handle->profile_id, NULL,
    ...
    dnx_ecgm_profile_mgmt_port_handle_t * port_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_port_profile.free_single
                    (unit, port_profile_handle->core, port_profile_handle->profile_id, &last_reference));
    if (last_reference)
    {
        sal_memset(&port_profile_handle->info, 0, sizeof(dnx_ecgm_port_profile_info_t));
        SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_port_info_hw_set(unit, port_profile_handle));
    ...
    dnx_ecgm_threshold_data_t * data)
{
    ...
    dnx_ecgm_profile_mgmt_port_handle_t port_profile_handle;
    dnx_ecgm_port_profile_info_t old_profile_info;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_port_handle_get(unit, port, &port_profile_handle));

    old_profile_info = port_profile_handle.info;
    ...
    /*
     * if the profile has changed - set the new profile info to the port 
     */
    if (sal_memcmp(&old_profile_info, &port_profile_handle.info, sizeof(dnx_ecgm_port_profile_info_t)))
    {
        SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_port_info_set(unit, &port_profile_handle));
    }
 ...
 */
shr_error_e
dnx_ecgm_profile_mgmt_interface_handle_init(
    int unit,
    bcm_port_t port,
    dnx_ecgm_profile_mgmt_interface_handle_t * interface_profile_handle)
{
    ...
    /*
     * initiating interface profile only for the first interface occurence 
     */
...

    {
         ...
         * setting initial SW profile values to 0.
         */
        SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_interface_profile.allocate_single
                        (unit, interface_profile_handle->core, 0, &interface_profile_handle->info, NULL,
                         &interface_profile_handle->profile_id, &first_reference));
    ...
    dnx_ecgm_profile_mgmt_interface_handle_t * interface_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_interface_profile.profile_data_get
                    (unit, interface_profile_handle->core, interface_profile_handle->profile_id, NULL,
    ...
    dnx_ecgm_profile_mgmt_interface_handle_t * interface_profile_handle)
{
    ...
    if (nof_channels == 1)
    {
        SHR_IF_ERR_EXIT(algo_ecgm_db.ecgm_interface_profile.free_single
                        (unit, interface_profile_handle->core, interface_profile_handle->profile_id, &last_reference));
        if (last_reference)
        {
            sal_memset(&interface_profile_handle->info, 0, sizeof(dnx_ecgm_interface_profile_info_t));
            SHR_IF_ERR_EXIT(dnx_ecgm_profile_mgmt_interface_info_hw_set(unit, interface_profile_handle));
        ...
        SHR_IF_ERR_EXIT(dnx_ecgm_dbal_egq_if_to_interface_profile_map_set
                        (unit, interface_profile_handle->core, interface_profile_handle->egq_if, 0));
    }
    ...
    dnx_ecgm_threshold_data_t * data)
{
    int value;
    dnx_ecgm_profile_mgmt_interface_handle_t interface_profile_handle;
    dnx_ecgm_interface_profile_info_t old_profile_info;
    ...
    old_profile_info = interface_profile_handle.info;

    /*
     * choosing the right priority 
     */
    priority_ptr =
        (data->threshold->priority ==
         BCM_COSQ_HIGH_PRIORITY) ? &interface_profile_handle.info.high_priority : &interface_profile_handle.
        info.low_priority;
     ...
     * if the profile has changed - set the new profile info to the port 
     */
    if (sal_memcmp(&old_profile_info, &interface_profile_handle.info, sizeof(dnx_ecgm_interface_profile_info_t)))
    {
    ...
    dnx_ecgm_profile_mgmt_interface_handle_t * interface_profile_handle)
{
    ...
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_interface_multicast_thresholds_set
                    (unit, interface_profile_handle->core, ECGM_IF_LOW, interface_profile_handle->profile_id,
                     interface_profile_handle->info.low_priority.mc_interface_PDs_drop));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_interface_multicast_thresholds_set
                    (unit, interface_profile_handle->core, ECGM_IF_HIGH, interface_profile_handle->profile_id,
    ...
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_interface_unicast_thresholds_set
                    (unit, interface_profile_handle->core, ECGM_IF_LOW, interface_profile_handle->profile_id,
                     &interface_profile_handle->info.low_priority.uc_fc));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_interface_unicast_thresholds_set
                    (unit, interface_profile_handle->core, ECGM_IF_HIGH, interface_profile_handle->profile_id,
                     &interface_profile_handle->info.high_priority.uc_fc));
