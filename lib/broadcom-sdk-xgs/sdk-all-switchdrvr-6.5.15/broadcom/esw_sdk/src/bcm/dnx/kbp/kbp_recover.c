    ...
    int kbp_port_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read(&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PCS_EN_REG, &data));
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                    (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PCS_EN_REG, data & ~(1 << kbp_port_id)));
    sal_usleep(200000);
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write(&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PCS_EN_REG, data));
    ...
    int reset)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD4_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD5_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD6_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD7_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD8_REG, reg_val));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD0_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD1_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD2_REG, reg_val));
        SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_write
                        (&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PHASE_MATCHING_FIFO_CTRL_QUAD3_REG, reg_val));
    }

    /** Dummy read to avoid MDIO lock up */
    SHR_IF_ERR_EXIT(dnx_kbp_callback_mdio_read(&Kbp_app_data[unit]->user_data, 0, 2, DNX_KBP_TX_PCS_EN_REG, &reg_val));
exit:
    SHR_FUNC_EXIT;
}
