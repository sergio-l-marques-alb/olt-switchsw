    ...
    int core_all_valid)
{
    ...
    /** check for valid core - BCM_CORE_ALL is allowed for set/get
     *  APIs */
    ...
    bcm_stat_eviction_t * eviction)
{
    ...
    {
        /** configuration cannot be changed if engine is active */
    ...
    bcm_stat_counter_enable_t * enable)
{
    ...
    DNXCMN_CORES_ITER(unit, database->core_id, core_idx)
    {
        ...
        /** check for correct source - if no eviction, can't read counters (I-OAM/E-OAM)  */
        if (DNX_CRPS_MGMT_SOURCE_NO_EVICTION(unit, engine_source) && enable->enable_eviction == TRUE)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_stat_counter_output_data_t * stat_counter_output_data)
{
    ...
    /** check for correct source - if no eviction, can't read counters (I-OAM/E-OAM)  */
    if (DNX_CRPS_MGMT_SOURCE_NO_EVICTION(unit, engine_source))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    bcm_stat_counter_group_filter_t filter)
{
    ...
    if (filter < 0 || filter >= bcmStatCounterGroupFilterCount
        || (FALSE == dnx_data_crps.filter.feature_get(unit, dnx_data_crps_filter_latency_drop_is_supported)
            && filter == bcmStatCounterGroupFilterLatencyDrop))
    {
