    ...
    bcm_trunk_id_info_t * id_info)
{
    ...
    for (current_core = 0; current_core < nof_cores; ++current_core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_port_packet_processing_properties_remove_internal(unit, pp_port, current_core));
        /*
         * Free the pp port
         */
    ...
    bcm_trunk_pp_port_allocation_info_t * allocation_info)
{
    ...
    nof_cores = dnx_data_device.general.nof_cores_get(unit);
    SHR_BIT_ITER(&allocation_info->core_bitmap, nof_cores, core)
    {
    ...
    bcm_trunk_member_t * member_array)
{
    ...
    /** verify virtual trunk and master trunk match (members mapping, psc, amount of members, etc...) */
    {
        ...
        /** verify psc matches */
        if (trunk_info->psc != master_trunk_info.psc)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Virtual trunk can't have a different psc than in master trunk"
                         "Virtual trunk psc is %d but master trunk psc is %d", trunk_info->psc, master_trunk_info.psc);
        }
    ...
    bcm_trunk_member_t * added_member)
{
    ...
    algo_gpm_gport_verify_type_e allowed_port_types[] = { ALGO_GPM_GPORT_VERIFY_TYPE_SYSTEM_PORT };

    SHR_FUNC_INIT_VARS(unit);

    /** check validity of system port */
    SHR_IF_ERR_EXIT(algo_gpm_gport_verify(unit, added_member->gport, COUNTOF(allowed_port_types), allowed_port_types));
    ...
    bcm_trunk_member_t * member_array)
{
    ...
    uint32 pp_ports[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    uint32 pp_port;
    trunk_group_member_info_t member_info;
    dnx_algo_gpm_gport_phy_info_t phy_gport_info;
    ...
    for (member_index = 0; (member_index < max_nof_members_in_group) && (remaining_members); ++member_index)
    {
            ...
            ((trunk_flags & BCM_TRUNK_FLAG_VP) == 0))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_pp_mapping_set(unit, core, pp_port, port_tm, system_port_id, system_port_id));
    ...
    for (nof_enabled_members = 0, member_index = 0; member_index < member_count; ++member_index)
    {
                ...
                ((trunk_flags & BCM_TRUNK_FLAG_VP) == 0))
        {
            ...
            uint32 dspa;
            uint32 trunk_in_header_type;
            uint32 dummy_trunk_out_header_type;
            ...
            /** DSPA is encoded using the actual member in the LAG group  */
            /** SSPA is encoded only with the group and not with the
             *  member in the group - the reason for this is that the HW
             *  table is per PP port and not per TM port   */
            SHR_IF_ERR_EXIT(dnx_trunk_utils_spa_encode(unit, pool, group, pp_member_index, &dspa));
            SHR_IF_ERR_EXIT(dnx_trunk_utils_spa_encode(unit, pool, group, 0, &sspa));
            SHR_IF_ERR_EXIT(dnx_algo_port_pp_port_modify(unit, phy_gport_info.local_port, core, pp_ports[core]));
            SHR_IF_ERR_EXIT(dnx_port_pp_mapping_set(unit, core, pp_ports[core], port_tm, sspa, dspa));
            /** remove the old pp port properties of the added member, at this point in the code it is still available in
            ...
            SHR_IF_ERR_EXIT(dnx_port_packet_processing_properties_remove_internal
                            (unit, phy_gport_info.internal_port_pp_info.pp_port[0],
                             phy_gport_info.internal_port_pp_info.core_id[0]));
            SHR_IF_ERR_EXIT(dnx_algo_port_pp_port_free(unit, phy_gport_info.internal_port_pp_info.core_id[0], phy_gport_info.internal_port_pp_info.pp_port[0]));
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_header_types_get
                            (unit, pool, group, &trunk_in_header_type, &dummy_trunk_out_header_type));
    ...
    bcm_trunk_member_t * member)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "Virtual trunk can't have more members than in master trunk!"
                     "Virtual trunk has %d members but master has %d members",
                     (member_count + 1), master_member_count);
    }

    /** verify new member physical membership in master trunk */
    SHR_IF_ERR_EXIT(dnx_trunk_modport_membership_verify(unit, member->gport,
                                                        master_nof_members, master_member_array));
    ...
    bcm_trunk_member_t * member)
{
    ...
    SHR_NULL_CHECK(member, _SHR_E_PARAM, "member");
    /** check validity of system port */
    SHR_IF_ERR_EXIT(algo_gpm_gport_verify(unit, member->gport, COUNTOF(allowed_port_types), allowed_port_types));
    ...
    bcm_trunk_member_t * member)
{
    ...
    if (!member_found_in_trunk)
    {
            ...
            ((trunk_flags & BCM_TRUNK_FLAG_VP) == 0))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_sw_db_group_pp_port_get(unit, pool, group, core, &pp_port));
            SHR_IF_ERR_EXIT(dnx_algo_port_pp_port_modify(unit, phy_gport_info.local_port, core, pp_port));
            SHR_IF_ERR_EXIT(dnx_port_pp_mapping_set(unit, core, pp_port, port_tm, sspa, dspa));
            /** remove the pp properties of the added member, at this point in the code it is still available in
             *  phy_gport_info.internal_port_pp_info.pp_port[0] */
            SHR_IF_ERR_EXIT(dnx_port_packet_processing_properties_remove_internal
                            (unit, phy_gport_info.internal_port_pp_info.pp_port[0],
                             phy_gport_info.internal_port_pp_info.core_id[0]));
    ...
    bcm_trunk_t * trunk_id)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "given port (%u) is not a system port, "
                     "change port input to system port gport and try again\n", port);
    }
    ...
    dnx_algo_trunk_member_lb_key_ranges_t * lb_key_ranges_arr)
{
    ...
    for (int lb_key_range_entry = 0; lb_key_range_entry < lb_key_ranges_arr_size; ++lb_key_range_entry)
    {
        ...
        for (int core = 0; core < nof_cores; ++core)
        {
            ...
            {
                out_port = dnx_data_trunk.egress_trunk.invalid_out_port_get(unit);
            }
    ...
    int *egress_trunk_id)
{
    ...
    if (is_first)
    {
        ...
        int out_port = dnx_data_trunk.egress_trunk.invalid_out_port_get(unit);
    ...
    bcm_trunk_t trunk_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_trunk_egress_destroy(unit, egress_trunk_id));
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_pbmp_t * local_port_bmp)
{
    ...
    for (int member = 0; member < nof_unique_members; ++member)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_tm_port_get
                        (unit, phy_gport_info.local_port, (bcm_core_t *) & member_core_id, &port_tm));
        if ((phy_gport_info.flags == DNX_ALGO_GPM_GPORT_INFO_F_IS_LOCAL_PORT) && (member_core_id == core))
        {
