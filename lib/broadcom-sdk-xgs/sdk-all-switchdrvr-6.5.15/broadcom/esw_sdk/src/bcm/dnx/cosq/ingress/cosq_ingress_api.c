    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    else
    {
        ...
        allowed_flags = (BCM_COSQ_DISCARD_ENABLE | all_colors_flags | BCM_COSQ_DISCARD_BYTES);
    ...
    if (discard->flags & BCM_COSQ_DISCARD_MARK_CONGESTION)
    {
        SHR_MASK_VERIFY(discard->flags & all_colors_flags, 0, _SHR_E_PARAM,
                        "Color flag can be specified together with BCM_COSQ_DISCARD_MARK_CONGESTION.\n");
    ...
    bcm_cosq_gport_discard_t * discard)
{
    ...
    {
        allowed_flags |= BCM_COSQ_DISCARD_BYTES;
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    SHR_RANGE_VERIFY(config->numq, 1, BCM_COSQ_VSQ_NOF_PG,
                     _SHR_E_PARAM, "The numq %d value is out of range, valid range is 1-%d\n",
                     config->numq, BCM_COSQ_VSQ_NOF_PG - 1);
    ...
    if (!DNX_ALGO_PORT_TYPE_IS_TM(unit, port_type) || DNX_ALGO_PORT_TYPE_IS_ERP(unit, port_type))
    {
    ...
    for (pg = 0; pg < config->numq; pg++)
    {
        ...
        if (config->pg_attributes[pg].is_lossles != 0 && config->pg_attributes[pg].pool_id != lossless_pool)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         ...
                         config->pg_attributes[pg].pool_id, config->pg_attributes[pg].pool_id);
        }
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    for (cosq = 0; cosq < config->numq; cosq++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_dbal_pg_use_port_guaranteed_set(unit, core_id, pg_base + cosq, TRUE));
    }
 ...
 */
static int
bcm_dnx_cosq_src_vsqs_gport_get_verify(
    int unit,
    bcm_gport_t port,
    bcm_cosq_src_vsqs_gport_config_t * config,
    bcm_gport_t * src_port_vsq_gport,
    ...
    bcm_gport_t * pg_base_vsq_gport)
{
    ...
    SHR_INVOKE_VERIFY_DNX(bcm_dnx_cosq_src_vsqs_gport_get_verify(unit, port, config,
                                                                 src_port_vsq_gport, pg_base_vsq_gport));
    ...
    SHR_IF_ERR_EXIT(dnx_ingress_congestion_db.port_based_vsq.vsq_f_mapping.pg_numq.get(unit, core_id, pg_base,
                                                                                       (uint32 *) &(config->numq)));
