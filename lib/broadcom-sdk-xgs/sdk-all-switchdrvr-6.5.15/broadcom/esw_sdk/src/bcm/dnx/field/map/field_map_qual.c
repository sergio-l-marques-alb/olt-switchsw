    ...
    dnx_field_qual_info_t * dnx_qual_info_p)
{
    ...
    switch (dnx_qual_class)
    {
        ...
        case DNX_FIELD_QUAL_CLASS_META2:
        {
             ...
             */
            dnx_qual_info_p->offset = dnx_data_field.qual.params_get(unit,
                                                                     dnx_field_map_stage_info[stage].pp_stage,
                                                                     dnx_qual_id)->offset;
             ...
             */
            SHR_IF_ERR_EXIT(dnx_field_map_meta_qual_mutable(unit, meta_qual_info->field_id,
    ...
    /*
     * Verify DBAL field id in all cases besides USER one
     */
    ...
    bcm_field_qualify_t bcm_qual)
{
    ...
    if (bcm_qual >= 0 && bcm_qual < bcmFieldQualifyCount)
    {
        ...
        if (ISEMPTY(bcm_qual_text[bcm_qual]))
        {
            return "Invalid BCM qualifier";
        }
        return bcm_qual_text[bcm_qual];
    }
    else if (dnx_field_qual_get_int(unit, bcm_qual, &qual_in_info, &dbal_field_id) == _SHR_E_NONE)
    {
    ...
    dnx_field_qual_t * dnx_qual_p)
{
    ...
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_bcm_to_dnx_int(unit, dnx_stage, bcm_qual, dnx_qual_p, NULL),
                             "BCM Qualifier %s(%d) is not supported for stage:%s",
    ...
    dnx_field_entry_qual_t * dnx_qual_info)
{
     ...
     */
    SHR_IF_ERR_EXIT_WITH_LOG(dnx_field_map_qual_bcm_to_dnx_int(unit, dnx_stage, bcm_qual_info->type,
                                                               &dnx_qual_info->dnx_qual, &qual_map_entry),
                             "BCM Qualifier %s(%d) is not supported for stage:%s",
                             dnx_field_bcm_qual_text(unit, bcm_qual_info->type), bcm_qual_info->type,
     ...
     */
    if ((qual_map_entry == NULL) || (qual_map_entry->conversion_cb == NULL))
    {
    ...
    else
    {
        SHR_IF_ERR_EXIT_WITH_LOG(qual_map_entry->conversion_cb
                                 (unit, core, bcm_qual_info->value, dnx_qual_info->qual_value),
    ...
    dnx_field_header_qual_info_t * qual_info)
{
    ...
    if (SHR_FAILURE(sand_signal_field_params_get(unit, qual_info->name, &offset, &size)))
    {
        if (qual_info->size == 0)
        {
            /*
             ...
             */
            LOG_WARN(BSL_LOG_MODULE, (BSL_META("Qualifier:%s has zero size\n"), qual_info->name));
            qual_info->valid = FALSE;
        }
        else
        {
    ...
    int unit)
{
    ...
    {
        dnx_field_map_qual_fetch_params(unit, &dnx_header_qual_info[i_id]);
    ...
    {
        int i_info;
        for (i_info = 1; i_info < dnx_field_map_stage_info[stage].meta_qual_nof; i_info++)
        {
            dnx_field_meta_qual_info_t *meta_qual_info = &dnx_field_map_stage_info[stage].meta_qual_info[i_info];
            meta_qual_info->bcm_qual = DNX_BCM_ID_INVALID;
            if (meta_qual_info->field_id == DBAL_FIELD_EMPTY)
            {
                LOG_WARN(BSL_LOG_MODULE, (BSL_META("No DBAL field ID for metadata qualifier:%s %d\n"),
