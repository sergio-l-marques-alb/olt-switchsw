    ...
    int map_id)
{
    ...
    {
        if (DNX_QOS_MAP_IS_ECN(map_id))
        {
            dnx_qos_profile_is_allocated = algo_qos_db.ingress_qos_ecn.is_allocated;
            nof_profiles = dnx_data_qos.qos.nof_ingress_ecn_profiles_get(unit);
            qos_type = "Ingress ECN";
        }
        else if (DNX_QOS_MAP_IS_PHB(map_id) && DNX_QOS_MAP_IS_REMARK(map_id))
        {
    ...
    int map_opcode)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_opcode))
    {
        ...
        else if (DNX_QOS_MAP_IS_ECN(map_opcode))
        {
            dnx_qos_opcode_is_allocated = algo_qos_db.ingress_qos_ecn_opcode.is_allocated;
            nof_opcodes = dnx_data_qos.qos.nof_ingress_ecn_opcode_profiles_get(unit);
            opcode_type = "ingress ECN";
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "wrong ingress map type [%x]\n", map_opcode);
    ...
    int *map_id)
{
    ...
    if (flags & BCM_QOS_MAP_WITH_ID)
    {
        ...
        else if (flags & (BCM_QOS_MAP_EGRESS & BCM_QOS_MAP_POLICER))
        {
    ...
    bcm_qos_map_t * map_entry)
{
     ...
     * when add qos mapping for ingress phb/remark/ecn or egress remark/php,
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_id))
    {
        ...
        {
            /** Map qos profile to phb/nwk_qos/ecn for l2 untag or port default case.*/
            if (DNX_QOS_MAP_IS_OPCODE(map_id) ||
                (!DNX_QOS_MAP_IS_PHB(map_id) && !DNX_QOS_MAP_IS_REMARK(map_id) && !DNX_QOS_MAP_IS_ECN(map_id)))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "map_id [0x%08X] must be QOS profile for PHB/Remark/ECN\n", map_id);
            }
            else
            {
                SHR_IF_ERR_EXIT(dnx_qos_map_profile_verify(unit, map_id));
        ...
        else if (flags & (BCM_QOS_MAP_PHB | BCM_QOS_MAP_REMARK | BCM_QOS_MAP_ECN))
        {
            if ((flags & BCM_QOS_MAP_OPCODE))
            {
                ...
                if ((_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_PHB) && !DNX_QOS_MAP_IS_PHB(map_id)) ||
                    ...
                    (_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_ECN) && !DNX_QOS_MAP_IS_ECN(map_id)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "map_id [0x%08X] and flags [0x%08X] must match for PHB/Remark/ECN\n",
                                 map_id, flags);
                }
            ...
            {
                opcode = BCM_DNX_QOS_MAP_INVALID_ID;
            }
            else
            {
                opcode = map_id;
            ...
            if (opcode != BCM_DNX_QOS_MAP_INVALID_ID)
            {
                ...
                if (!DNX_QOS_MAP_IS_OPCODE(opcode) ||
                    (!DNX_QOS_MAP_IS_PHB(opcode) && !DNX_QOS_MAP_IS_REMARK(opcode) && !DNX_QOS_MAP_IS_ECN(opcode)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "%s [0x%08X] must be opcode for PHB/Remark/ECN\n", opcode_field, opcode);
                }
                else
                {
                ...
                if ((_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_PHB) && !DNX_QOS_MAP_IS_PHB(opcode)) ||
                    ...
                    (_SHR_IS_FLAG_SET(flags, BCM_QOS_MAP_ECN) && !DNX_QOS_MAP_IS_ECN(opcode)))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM, "%s [0x%08X] and flags [0x%08X] must match for PHB/Remark/ECN\n",
    ...
    bcm_qos_map_t * map_entry)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_id))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "flags must be PHB or REMARK or ECN[%x]", flags);
        }
        ...
        {
            DNX_QOS_ERROR_IF_OUT_OF_RANGE("QOS_MAP_ECN", map_entry->int_pri, dnx_data_qos.qos.packet_max_ecn_get(unit));
        }
    ...
    int map_opcode)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_opcode))
    {
        ...
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn_opcode.free_single(unit, opcode_index));
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "wrong ingress map type [%x]\n", map_opcode);
    ...
    int map_id)
{
    ...
    if (DNX_QOS_MAP_IS_INGRESS(map_id))
    {
        if (DNX_QOS_MAP_IS_ECN(map_id))
        {
            SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn.free_single(unit, profile_index));
    ...
    int *array_count)
{
    ...
    *array_count = 0;
    ...
    /** ingress ecn opcode profile*/
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_ecn_opcode_profiles_get(unit); idx++)
    {
        SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn_opcode.is_allocated(unit, idx, &is_allocated));
    ...
    int *array_count)
{
    ...
    *array_count = 0;
    ...
    /** ingress ecn profile*/
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_ecn_profiles_get(unit); idx++)
    {
        SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn.is_allocated(unit, idx, &is_allocated));
        if (is_allocated)
        {
            count++;
        }
    ...
    *array_count = 0;
    ...
    for (idx = 0; idx < dnx_data_qos.qos.nof_ingress_ecn_profiles_get(unit); idx++)
    {
        ...
        if (is_allocated)
        {
            ...
            DNX_QOS_INGRESS_MAP_SET(map_id);
            *(map_ids_array + count) = map_id;
            *(flags_array + count) = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_ECN;
            count++;
        }
    }
    /** ingress policer profile*/
...
/**
 * \brief 
 *  Verify the qos map ID for dnx_qos_control_mpls_ingress_swap_remark_profile_set.
 *
 * \param [in] unit - Relevant unit.
 * \param [in] egr_map - ID of the EGRESS MAP.
 *
 ...
 */
static int
dnx_qos_control_mpls_ingress_swap_remark_profile_verify(
    int unit,
    int egr_map)
{
    uint32 profile_index;
    ...
    /** check egr_map is valid for EGRESS remark */
    if ((egr_map == BCM_DNX_QOS_MAP_INVALID_ID) || !DNX_QOS_MAP_IS_EGRESS(egr_map) ||
        !DNX_QOS_MAP_IS_REMARK(egr_map) || (profile_index > dnx_data_qos.qos.nof_egress_profiles_get(unit) - 1))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "egr_map[%x] is not a valid egress QoS map ID!\n", egr_map);
    }

    /** check egr_map is allocated. */
    SHR_IF_ERR_EXIT(algo_qos_db.egress_qos.is_allocated(unit, profile_index, &is_allocated));
    if (is_allocated == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "egress REMARK map ID [%x] is not allocated\n", egr_map);
    }
 ...
 */
static int
dnx_qos_control_mpls_ingress_swap_remark_profile_set(
    int unit,
    int egr_map)
{
    ...
    /** check egr_map is valid for egress remark */
    SHR_INVOKE_VERIFY_DNX(dnx_qos_control_mpls_ingress_swap_remark_profile_verify(unit, egr_map));
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_REMARK,
                                 DBAL_ENUM_FVAL_FHEI_TYPE_MPLS_SWAP, profile_index);
 ...
 *
 * \param [in] unit - Relevant unit.
 * \param [out] egr_map - ID of the EGRESS MAP.
 *
 * \return
 *    \retval Negative in case of an error. 
 *    \retval Zero in case of NO ERROR
 ...
 */
static int
dnx_qos_control_mpls_ingress_swap_remark_profile_get(
    int unit,
    int *egr_map)
{
    uint32 entry_handle_id;
    ...
    int arg)
{
    ...
    switch (type)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_qos_control_mpls_ingress_swap_remark_profile_set(unit, arg));
            break;
        }
        default:
        {
    ...
    int *arg)
{
    ...
    switch (type)
    {
        case bcmQosControlMplsIngressSwapRemarkProfile:
        {
            SHR_IF_ERR_EXIT(dnx_qos_control_mpls_ingress_swap_remark_profile_get(unit, arg));
            break;
        }
        default:
        {
