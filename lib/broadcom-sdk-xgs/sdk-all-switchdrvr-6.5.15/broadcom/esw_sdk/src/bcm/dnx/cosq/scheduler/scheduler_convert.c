    ...
    int *se_id)
{
    ...
    int type0_end = (dnx_data_sch.sch_alloc.type_con_reg_end_get(unit) + 1) * dnx_data_sch.flow.region_size_get(unit);
    ...
    int *tc)
{
    bcm_pbmp_t ports_bm;
    uint32 port_ndx;
    int base_hr, hr_id_to_check, nof_priorities;
    ...
    *tc = 0;
    ...
    if (DNX_SCHEDULER_ELEMENT_IS_HR(unit, se_id))
    {
        ...
        hr_id_to_check = DNX_SCHEDULER_ELEMENT_HR_ID_GET(unit, se_id);

        /** Find match range of (base_hr <= hr_id_to_check < base_hr + nof_priorities) */
        SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get(unit, core, DNX_ALGO_PORT_LOGICALS_TYPE_TM_E2E_SCH, 0, &ports_bm));

        SOC_PBMP_ITER(ports_bm, port_ndx)
        {

            SHR_IF_ERR_EXIT(dnx_algo_port_base_hr_get(unit, port_ndx, &base_hr));

            SHR_IF_ERR_EXIT(dnx_algo_port_sch_priorities_nof_get(unit, port_ndx, &nof_priorities));

            if (UTILEX_IS_IN_RANGE(hr_id_to_check, base_hr, base_hr + nof_priorities - 1))
            {
                /** Match */
                *logical_port = port_ndx;
                *tc = hr_id_to_check - base_hr;
                break;
            }
        }
    }
