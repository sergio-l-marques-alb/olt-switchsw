    ...
    uint8 to_delete)
{
     ...
     *   * hierarchy
     */
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_super_fec_type_get(unit, fec_entry_info->fec_id, &super_fec_type));
    ...
    else
    {
         ...
         */
        dbal_fields_struct_field_encode(unit, fec_entry_info->fec_entry_type, DBAL_FIELD_DESTINATION,
        ...
        switch (fec_entry_info->fec_entry_type)
        {
            ...
            case DBAL_FIELD_FEC_DESTINATION_MPLS_POP:
            {
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                (unit, fec_entry_info->fec_entry_type, DBAL_FIELD_EEI, &fec_entry_info->eei,
                                 fec_entry));
                break;
            }
            case DBAL_FIELD_FEC_DESTINATION_MPLS_SWAP:
            {
                SHR_IF_ERR_EXIT(dbal_fields_struct_field_encode
                                (unit, fec_entry_info->fec_entry_type, DBAL_FIELD_EEI, &fec_entry_info->eei,
                                 fec_entry));
    ...
    l3_fec_entry_info_t * fec_entry_info)
{
    ...
    if (dnx_data_l3.hw_bug.feature_get(unit, dnx_data_l3_hw_bug_fec_hit_bit))
    {
            ...
            || (fec_entry_info->flags & L3_FEC_ENTRY_INFO_GET_CLEAR_HIT_BIT))
        {
            ...
            uint32 attr_type;
            /*
             * Operate on both 'primary' and 'secondary' since key is 'super_fec_id'.
             * In all cases, read the hit-bit.
             */
            attr_type = DBAL_ENTRY_ATTR_HIT_PRIMARY | DBAL_ENTRY_ATTR_HIT_SECONDARY | DBAL_ENTRY_ATTR_HIT_GET;
            if (fec_entry_info->flags & L3_FEC_ENTRY_INFO_GET_CLEAR_HIT_BIT)
            {
                /*
     ...
     */
    dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, fec_type_in_super_fec, instance, fec_entry_val);
    ...
    dbal_fields_e * fec_entry_type)
{
    ...
    {
        *fec_entry_type = DBAL_FIELD_FEC_DESTINATION_MPLS_POP;
    }
    else if (egr->mpls_label != BCM_MPLS_LABEL_INVALID)
    {
        ...
        {
            *fec_entry_type = DBAL_FIELD_FEC_DESTINATION_MPLS_SWAP;
        }
    }
     ...
     * {
     */
    else if (_SHR_IS_FLAG_SET(egr->flags, BCM_L3_ENCAP_SPACE_OPTIMIZED))
    {
        *fec_entry_type = DBAL_FIELD_FEC_JR1_DESTINATION_EEI;
    }
    /*
    ...
    bcm_if_t * if_id)
{
    ...
    if (_SHR_IS_FLAG_SET(flags, BCM_L3_WITH_ID))
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "FEC ID %d is out of range - should be positive and higher than %d\n",
                         BCM_L3_ITF_VAL_GET(*if_id), max_fec_id);
        }
    ...
    bcm_if_t * if_id)
{
    ...
    if (is_alloc_required == TRUE)
    {
        ...
        if (is_protected == TRUE)
        {
            super_fec_res_type =
                (is_stat_fec ==
                 TRUE) ? DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_W_PROT_W_STAT :
        ...
        else
        {
            super_fec_res_type =
                (is_stat_fec ==
                 TRUE) ? DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_NO_PROT_W_STAT :
                DBAL_ENUM_FVAL_FEC_RESOURCE_TYPE_NO_PROT_NO_STAT;
        }
        /** Allocate FEC (fec_index) */
        SHR_IF_ERR_EXIT(dnx_algo_l3_fec_allocate
                        (unit, &fec_index, alloc_flags, hierarchy, super_fec_res_type, bank_id));
        /** If a valid bank ID was returned, then we need to update the hierarchy mapping for this bank */
    ...
    int fec_index)
{
    ...
    if (is_protected)
    {
        ...
        if (fec_index % fec_per_super_fec == 1)
        {
            /** FEC is stand-by */
            fec_index = L3_FEC_ID_TO_EVEN_FEC_ID(fec_index);
            SHR_IF_ERR_EXIT(dnx_algo_l3_fec_deallocate(unit, fec_index, bank_id));
        }
    }
    else
    {
    ...
    bcm_if_t * if_id)
{
    ...
    {
        fec_entry_info.glob_out_lif = BCM_L3_ITF_VAL_GET(egr->intf);
        fec_entry_info.glob_out_lif_2nd = L3_FEC_ENCAP_ID_TO_EEI_POINTER(BCM_L3_ITF_VAL_GET(egr->encap_id));
    }
    else if (fec_entry_info.fec_entry_type == DBAL_FIELD_FEC_DESTINATION_MPLS_POP)
    {
        dbal_enum_value_field_model_type_e qos_model;
        dbal_enum_value_field_model_type_e ttl_model;

        qos_model =
            (_SHR_IS_FLAG_SET(egr->mpls_flags, BCM_MPLS_EGRESS_LABEL_EXP_SET)) ? DBAL_ENUM_FVAL_MODEL_TYPE_PIPE :
            DBAL_ENUM_FVAL_MODEL_TYPE_UNIFORM;
        ttl_model =
            (_SHR_IS_FLAG_SET(egr->mpls_flags, BCM_MPLS_EGRESS_LABEL_TTL_SET)) ? DBAL_ENUM_FVAL_MODEL_TYPE_PIPE :
            DBAL_ENUM_FVAL_MODEL_TYPE_UNIFORM;
        fec_entry_info.flags |= L3_FEC_ENTRY_INFO_EEI_PHP;
        SHR_IF_ERR_EXIT(dnx_mpls_eei_php_information_fill
                        (unit, egr->qos_map_id, qos_model, ttl_model, DBAL_ENUM_FVAL_JR_FWD_CODE_MPLS,
                         &fec_entry_info.eei));
    }
    /** BCM_L3_ENCAP_SPACE_OPTIMIZED is relevant only in Jericho mode */
    /*
    ...
    else if (fec_entry_info.fec_entry_type == DBAL_FIELD_FEC_JR1_DESTINATION_EEI)
    {
        if (egr->mpls_action == BCM_MPLS_EGRESS_ACTION_PUSH)
        {
            ...
            fec_entry_info.flags |= L3_FEC_ENTRY_INFO_EEI_PUSH_COMMAND;
            SHR_IF_ERR_EXIT(dnx_mpls_tunnel_create_push_entry_from_eei(unit, &label_entry, FALSE,
                                                                       _SHR_IS_FLAG_SET(flags, BCM_L3_REPLACE),
                                                                       &push_cmd));
     ...
     * }
     */
    else if (fec_entry_info.fec_entry_type == DBAL_FIELD_FEC_DESTINATION_MPLS_SWAP)
    {
        fec_entry_info.eei = egr->mpls_label;
    }
    ...
    bcm_l3_egress_t * egr)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_l3_fec_is_allocated(unit, fec_entry_info.fec_id, &is_allocated));
    if (is_allocated == 0)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "FEC with ID %d has not been allocated\n", intf);
    }
    /*
    ...
    else if (_SHR_IS_FLAG_SET(fec_entry_info.flags, L3_FEC_ENTRY_INFO_EEI_PHP))
    {
        /** in case of FHEI through FEC the model is always PIPE */
        dbal_enum_value_field_model_type_e qos_model;
        dbal_enum_value_field_model_type_e ttl_model;
        ...
        SHR_IF_ERR_EXIT(dnx_mpls_eei_php_information_retrieve
                        (unit, fec_entry_info.eei, &egr->qos_map_id, &qos_model, &ttl_model, &upper_layer_type));
        if (qos_model == DBAL_ENUM_FVAL_MODEL_TYPE_PIPE)
        {
        ...
        if (ttl_model == DBAL_ENUM_FVAL_MODEL_TYPE_PIPE)
        {
            egr->mpls_flags |= BCM_MPLS_EGRESS_LABEL_TTL_SET;
        }
        else
        {
            egr->mpls_flags |= BCM_MPLS_EGRESS_LABEL_TTL_COPY;
        }
    }
    /** BCM_L3_ENCAP_SPACE_OPTIMIZED is relevant only in Jericho mode */
    /*
     ...
     * }
     */
    else if ((fec_entry_info.eei) && (!_SHR_IS_FLAG_SET(fec_entry_info.flags, L3_FEC_ENTRY_INFO_EEI_PHP)))
    {
        system_headers_mode = dnx_data_headers.system_headers.system_headers_mode_get(unit);
        if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO2_MODE)
        {
 ...
 */
shr_error_e
dnx_l3_fec_hierarchy_stage_map_set(
    int unit,
    int *bank_ids,
    uint32 hierarchy)
{
    ...
    for (bank_iter = 0; bank_iter < 2; bank_iter++)
    {
        if (bank_ids[bank_iter] != -1)
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FEC_BANK_TO_STAGE_MAP, &entry_handle_id));
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_HIERARCHY_LEVEL, bank_ids[bank_iter],
                                         hierarchy);
            SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

            SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_hierarchy.
                            fec_hierarchy_map.set(unit, bank_ids[bank_iter], hierarchy));
            /*
             * Update the bank ID with a legal bank value, could be any one of the two if there are two. 
             */
            bank_id = bank_ids[bank_iter];
            bank_count++;
        }
    }

    /*
     * In case one or more than a single bank was found update the pair bits map
     */
    if (bank_count > 0)
    {
        int pair_to_clear = bank_id >> 1;
        /*
         * If the current hierarchy is none, then the matching pair are not a pair of same legal hierarchy,
         */
        if (hierarchy == DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY)
        {
            SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_res_mngr.
                            resource_is_part_of_pair.bit_clear(unit, pair_to_clear));
        }
        else
        {

            uint32 pair_bank = ALGO_L3_GET_FEC_SUB_RESOURCE_PAIR(bank_id);
            uint32 pair_hierarchy;
            SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_hierarchy.
                            fec_hierarchy_map.get(unit, pair_bank, &pair_hierarchy));
            /*
             * If two banks were found then both are of the same hierarchy and should be paired.
             * In case only one bank was found but the second one is already with the same hierarchy then they should be marked as a pair.
             */
            if ((bank_count == 2) || (pair_hierarchy == hierarchy))
            {
                SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_res_mngr.
                                resource_is_part_of_pair.bit_set(unit, pair_to_clear));
            }
        }
    }
 ...
 */
shr_error_e
dnx_l3_fec_mdb_table_resources_set(
    int unit,
    algo_dnx_l3_fec_banks_update_info_t * cluster_to_update)
{
    mdb_cluster_alloc_info_t new_clusters[ALGOL_L3_NOF_FEC_BANK_TYPES];
    algo_l3_fec_banks_group_type_e bank_group_type;
    int nof_big_banks_passed;
    int cluster_iter;
    ...
    {
        for (cluster_iter = 0; cluster_iter < ALGOL_L3_NOF_FEC_BANK_TYPES; cluster_iter++)
        {
            if (cluster_to_update->clusters_info[bank_group_type].
                cluster_to_update[cluster_iter].logical_start_address != ALGO_L3_INVALID_LOGICAL_ADDRESS)
            {
                nof_big_banks_passed =
                    (cluster_to_update->clusters_info[bank_group_type].bank_id[cluster_iter] + 4) / 5;
                /** Use the cluster_id_to_update as first FEC cluster in pair. */
                sal_memcpy(&new_clusters[0],
                           &(cluster_to_update->clusters_info[bank_group_type].cluster_to_update[cluster_iter]),
                           sizeof(mdb_cluster_alloc_info_t));
                sal_memcpy(&new_clusters[1],
                           &(cluster_to_update->clusters_info[bank_group_type].cluster_to_update[cluster_iter]),
                           sizeof(mdb_cluster_alloc_info_t));
                /** Update the logical start address for the pair of clusters. */
                new_clusters[0].logical_start_address =
                    ((cluster_to_update->clusters_info[bank_group_type].bank_id[cluster_iter] *
                      dnx_data_l3.fec.fec_stage_map_size_get(unit)) >> 1) + nof_big_banks_passed;
                new_clusters[1].logical_start_address = new_clusters[0].logical_start_address;
                new_clusters[1].cluster_idx++;

                SHR_IF_ERR_EXIT(mdb_init_add_table_resources(unit, 2, new_clusters));
    ...
    int unit)
{
    ...
    bank_id[1] = -1;
    /*
     * Iterate over all bank IDs in the hierarchy mapping and set them to NO_HIERARCHY by default.
     */
    for (idx = 0; idx < dnx_data_l3.fec.fec_max_nof_banks_get(unit); idx++)
    {
        bank_id[0] = idx;
        SHR_IF_ERR_EXIT(dnx_l3_fec_hierarchy_stage_map_set(unit, bank_id, DBAL_ENUM_FVAL_HIERARCHY_LEVEL_NO_HIERARCHY));
    }
