    ...
    dnx_oam_endpoint_info_t * sw_endpoint_info)
{
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TYPE, INST_SINGLE, &sw_endpoint_info->type);
 ...
 *        to which data will be written for subsequent use.
    ...
    uint8 is_update)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_OAM_ENDPOINT_SW_STATE_FLAGS, INST_SINGLE,
                                 sw_endpoint_info->sw_state_flags);
    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_TYPE, INST_SINGLE, sw_endpoint_info->type);
 ...
 *  \param [out] node -
 *    if value of EP is found in list than node gets the found
 *    node.
 *  \param [out] found -
 ...
 *  the linked list is the eps related to the endpoint
 * \param [in] unit  - 
 *   Relevant unit.
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected.
...
/**
 * \brief
 *   The function adds the OAM EP to relavent OAM endpoint link
 *   list
 * \param [in] unit  - 
 *   Relevant unit.
 * \param [in] endpoint_id- 
 *  ID of the OAM endpoint linked list
 * \param [in] rmep_id - 
 *  Id of EP to add to linked list
 * \return
 *   \retval Zero if no error was detected
...
/**
 * \brief
 *   The function removes the OAM RMEP from relavent OAM endpoint sw
 *   link list
 * \param [in] unit  - 
 *   Relevant unit.
 * \param [in] endpoint_id- 
