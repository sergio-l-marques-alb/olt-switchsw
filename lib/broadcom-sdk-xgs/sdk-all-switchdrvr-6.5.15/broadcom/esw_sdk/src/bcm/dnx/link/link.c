 ...
 */
STATIC int
_bcm_dnx_link_port_link_get(int unit, bcm_port_t port, int hw, int *up)
{
    int  rc;
 ...
 */
int
dnx_linkscan_port_init(int unit)
{
    bcm_port_t port;
 ...
 */
int
dnx_linkscan_port_remove(int unit)
{
    bcm_port_t port;
    dnx_algo_port_type_e port_type;
...
bcm_dnx_linkscan_init(int unit)
{
    ...
    SHR_ERR_EXIT(BCM_E_UNAVAIL,
                 "No need to call bcm_linkscan_init as linkscan is already initialized by the SDK.");
...
int
bcm_dnx_linkscan_detach(int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_ERR_EXIT(BCM_E_UNAVAIL,
                 "No need to call bcm_linkscan_detach as linkscan de-initialization is handled by SDK.");
    ...
    int mode)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get(unit, port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info));

    BCM_PBMP_ITER(gport_info.local_port_bmp, port_ndx) {
        if (!IS_SFI_PORT(unit, port)) {
            /*Statistic ports are not supported*/
            SHR_IF_ERR_EXIT(dnx_algo_port_type_get(unit, port_ndx, &port_type));
            ...
            if (IS_IL_PORT(unit, port_ndx) && mode == BCM_LINKSCAN_MODE_HW) {
