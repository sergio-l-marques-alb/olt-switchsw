    ...
    uint32 mask)
{
    ...
    allowed_data_mask = dnx_data_dram.hbm.mr_mask_get(unit);
    ...
    int channel)
{
    ...
    nof_channels_per_hbm = dnx_data_dram.hbm.nof_channels_get(unit);
    ...
    uint32 *data)
{
     ...
     */
    phy_reg_addr_mask = dnx_data_dram.hbm.phy_address_mask_get(unit);
    ...
    const hbmc_shmoo_bist_info_t * info)
{
    ...
    else
    {
        ...
        orig_nof_bist_actions_for_arethmetics = COMPILER_64_INIT(info->bist_num_actions[1], info->bist_num_actions[0]);
