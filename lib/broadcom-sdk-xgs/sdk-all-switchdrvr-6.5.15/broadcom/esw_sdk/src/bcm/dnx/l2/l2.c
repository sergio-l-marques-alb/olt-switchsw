...
#define MACT_DYNAMIC_THRESHOLD 1
...
/** Offset to the learn extension */
#define L2_LEARN_EXT_OFFSET 0x12

#define L2_LEARN_FORMAT_INDEX_FOR_COUNTING 0

/** Use learn format index 2 for a WA to the LEL bug for selecting FID profile */
#define L2_LEARN_FORMAT_INDEX_FOR_HW_WA 2
#define L2_LEARN_FID_OFFEST_HW_WA 118
 /*
  * }
  */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    /** IVL */
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FORMAT_INDEX, DBAL_DEFINE_MACT_APP_DB_IDX_IVL);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MDB_VMV, INST_SINGLE, vmv_value);
...
{
    uint32 entry_handle_id;
    uint32 size, ivl_offset, svl_offset, wa_offset;
    ...
    switch (learn_limit_mode)
    {
        case LEARN_LIMIT_MODE_VLAN:
             ...
             */
            ivl_offset = svl_offset = MACT_ENTRY_VSI_OFFSET;
        ...
        case LEARN_LIMIT_MODE_VLAN_PORT:
             ...
             */
            ivl_offset = svl_offset = MACT_ENTRY_OUTLIF_OFFSET;
    ...
    /** FID WA for HW bug in the LEL  */
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_MACT_APP_DB_INDEX, 0, 1);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_FORMAT_INDEX, L2_LEARN_FORMAT_INDEX_FOR_HW_WA);
    ...
    int unit)
{
    ...
    /** Disable ECC in case of ECC calculation bug */
    if (dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_fid_mgmt_ecc_error))
    {
    ...
    /** Offset to the learn extension */
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_LEARN_EXT_OFFSET, INST_SINGLE, L2_LEARN_EXT_OFFSET);

    if (dnx_data_l2.hw_bug.feature_get(unit, dnx_data_l2_hw_bug_appdb_id_for_olp))
    {
        /** Apply a WA for Key-DB-profile=0. */
    ...
    int *learn_limit_per_vsi)
{
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, vsi_learning_profile, &ref_count, (void *) &vsi_learning_profile_info));
    ...
    int unit)
{
    ...
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, DBAL_ENUM_FVAL_MAC_STRENGTH_0,
    ...
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, DBAL_ENUM_FVAL_MAC_STRENGTH_0,
                                     DBAL_ENUM_FVAL_MAC_STRENGTH_1);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RBD, 0);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_AGE_TYPE, MACT_MAX_AGE);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_AGE_PROFILE, MACT_DEFAULT_AGE_PROFILE);
    ...
    int aging_profile_id)
{
    ...
    dbal_entry_key_field32_range_set(unit, entry_handle_id, DBAL_FIELD_MAC_STRENGTH, 0, MACT_DYNAMIC_THRESHOLD);
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_RBD, 0);
    ...
    dnx_aging_profile_info_t * aging_profile_info)
{
    ...
    allocate_failed =
        algo_l2_db.vsi_aging_profile.allocate_single(unit, _SHR_CORE_ALL, 0, aging_profile_info, NULL,
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_aging_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, old_vsi_learning_profile_info.aging_profile,
    ...
    if (allocate_failed == _SHR_E_NONE)
    {
        ...
        if (!is_first)
        {
            SHR_IF_ERR_EXIT(algo_l2_db.
                            vsi_aging_profile.free_single(unit, _SHR_CORE_ALL, new_aging_profile, &last_reference));
        }
        else
        {
        ...
        {
            SHR_IF_ERR_EXIT(algo_l2_db.vsi_aging_profile.free_single(unit, _SHR_CORE_ALL,
                                                                     old_vsi_learning_profile_info.aging_profile,
                                                                     &last_reference));
    ...
    else
    {
        ...
        if (old_learning_ref_count == 1 && old_aging_ref_count == 1)
        {
            SHR_IF_ERR_EXIT(algo_l2_db.vsi_aging_profile.replace_data
                            (unit, _SHR_CORE_ALL, old_vsi_learning_profile_info.aging_profile, aging_profile_info));
        }
        else
        {
    ...
    int limit)
{
    ...
    /** Get the vsi learning profile info */
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, old_vsi_learning_profile, &ref_count, (void *) &vsi_learning_profile_info));

    vsi_learning_profile_info.limit = limit;
    SHR_IF_ERR_EXIT(algo_l2_db.vsi_learning_profile.exchange
                    (unit, _SHR_CORE_ALL, 0, &vsi_learning_profile_info, old_vsi_learning_profile,
                     NULL, &new_vsi_learning_profile, &is_first, &is_last));
...
/**
 * \brief - Init the age in which the refresh/delete/age out events are raised. Init the HW and template manager.
 *
 * \param [in] unit - unit id
 * \param [in] learn_mode - learn mode. Each learn mode has its default values for raising events.
 *
 * \return
 *   shr_error_e
 *
    ...
    dnx_learn_mode_t learn_mode)
{
    ...
    switch (learn_mode)
    {
        case LEARN_MODE_DISTRIBUTED:
            ...
            aging_profile_info.refresh_age = MACT_MAX_AGE;
            break;
    ...
    SHR_IF_ERR_EXIT(algo_l2_db.
                    vsi_aging_profile.allocate_single(unit, _SHR_CORE_ALL, alloc_flags, &aging_profile_info, NULL,
    ...
    int nof_meta_cycles_for_ageout)
{
    ...
    {
        age_for_delete = MACT_MAX_AGE - (nof_meta_cycles_for_ageout >> meta_cycles_mask_nof_bits) + 1;
    }
    ...
    SHR_IF_ERR_EXIT(dnx_aging_profile_update(unit, vsi, meta_cycles_mask, &aging_profile_info));
    ...
    int *nof_meta_cycles_for_ageout)
{
    ...
    if (aging_profile_info.ageout_age != AGING_PROFILE_INVALID_AGE)
    {
        ...
        nof_age_states_until_age_out = MACT_MAX_AGE - aging_profile_info.ageout_age + 1;
    ...
    int unit)
{
    ...
    DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_L2_AGE_SCAN, entry_handle_id);
    ...
    int vsi)
{
    ...
    SHR_IF_ERR_EXIT(dnx_l2_nof_age_cycles_before_ageout_set(unit, vsi, DEFAULT_NOF_AGE_CYCLES));

    /** Get the info of forwarding profile 0 (default profile) */
    SHR_IF_ERR_EXIT(algo_l2_db.event_forwarding_profile.profile_data_get
                    (unit, _SHR_CORE_ALL, 0, &ref_count, (void *) &event_forwarding_profile_info));
