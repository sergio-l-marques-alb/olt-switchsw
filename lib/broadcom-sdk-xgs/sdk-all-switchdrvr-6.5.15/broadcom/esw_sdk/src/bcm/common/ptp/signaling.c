        ...
        case bcmPTPTlvTypeRequestUnicastTransmission:
            {
                ...
                if (BCM_FAILURE(rv = _bcm_ptp_signaled_unicast_slave_exists(unit, ptp_id, &peer_ext, &i))) {
                    ...
                    /* Update slave's MAC address, just in case if this changed signaling will be seemless */
                    sal_memcpy(&signaled_slave_table.entry[i].slave.peer.peer_address.raw_l2_header[0], &peer_ext.peer.peer_address.raw_l2_header[0], peer_ext.peer.peer_address.raw_l2_header_length);
                }
