    ...
    bcm_xflow_macsec_subport_num_t *macsec_subport_num)
{
    ...
    if (soc_feature(unit, soc_feature_xflow_macsec)) {
        XFLOW_MACSEC_LOCK(unit);
        rv = xflow_macsec_subport_num_get(unit, id, macsec_subport_num);
        XFLOW_MACSEC_UNLOCK(unit);
    }
    return rv;
}
...
int bcm_common_xflow_macsec_station_mac_addr_get(
    int unit,
    bcm_mac_t mac_addr)
{
    int rv = BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_xflow_macsec)) {
        XFLOW_MACSEC_LOCK(unit);
        rv = xflow_macsec_station_mac_addr_get(unit, mac_addr);
        XFLOW_MACSEC_UNLOCK(unit);
    }
    return rv;
}

int bcm_common_xflow_macsec_station_mac_addr_set(
    int unit,
    bcm_mac_t mac_addr)
{
    int rv = BCM_E_UNAVAIL;
    ...
    if (soc_feature(unit, soc_feature_xflow_macsec)) {
        XFLOW_MACSEC_LOCK(unit);
        rv = xflow_macsec_station_mac_addr_set(unit, mac_addr);
        XFLOW_MACSEC_UNLOCK(unit);
    }
    return rv;
}
