	...
	if(i == BCM_PTP_MAX_UNICAST_MASTER_TABLE_ENTRIES) {
		...
		for (i=0; i<BCM_PTP_MAX_UNICAST_MASTER_TABLE_ENTRIES; i++) {
			if(master_table[i].in_use == 0) {
				...
				sal_memcpy(&master_table[i].master_portId, master_portId, sizeof(master_portId));
				master_table[i].in_use = 1;
				*inst_num = i;
				return BCM_E_NONE;
			}
    ...
    unsigned char                  clockClass)
{
    ...
    if (inst_num == PTP_EXT_SERVO_MSTR_INST_UNKNOWN) {
        if(PTP_CLOCK_IS_G8275P1_PROF(clock_info)) {
            ...
            if(BCM_FAILURE(rv = _bcm_ptp_ext_servo_master_table_entry_add(unit, ptp_id, clock_num, master_portId, master_portaddr,1, &instance))) {
    ...
    bcm_ptp_clock_port_address_t * master_portaddr)
{
    ...
    if (inst_num == PTP_EXT_SERVO_MSTR_INST_UNKNOWN) {
        if (BCM_FAILURE(rv = _bcm_ptp_ext_servo_master_table_entry_delete(unit, ptp_id, clock_num, master_portId, master_portaddr, 1, &instance))) {
    ...
    uint32                         int_usec)
{
    ...
    for (i=0; i<num_TS; i++) {
		ts_diff = _bcm_ptp_int64_read(ts_ptr +(i*8));
        LOG_DEBUG(BSL_LS_BCM_PTP,
            (BSL_META_U(unit, " [%d][0x%016llx/%lldnsec] "), i, (long long int)ts_diff, (long long int)ts_diff));
	}
    ...
    if (direction == _bcm_ptp_ts_type_downlink) {
        ...
        rv = IDTStackAdaptor_AddDownlinkTimeStampDifferences( &master, firstTS, num_TS, (T_osInt64 *)ts_ptr, int_usec );
        ...
        rv = IDTStackAdaptor_AddUplinkTimeStampDifferences( &master, firstTS, num_TS, (T_osInt64 *)ts_ptr, int_usec );
