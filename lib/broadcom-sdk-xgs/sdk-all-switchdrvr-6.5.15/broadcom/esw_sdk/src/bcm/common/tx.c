...
bcm_common_tx_init(int unit)
{
    ...
    if (tx_dv_done == NULL) {
        tx_dv_done = sal_sem_create("tx dv notify", sal_sem_BINARY, 0);
        if (tx_dv_done == NULL) {
            goto error;
        }
    }
...
_tx_pkt_desc_add(int unit, bcm_pkt_t *pkt, dv_t *dv, int pkt_idx)
{
    ...
    */
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        {
            /*
             * Module Header is not supported on cmodel
...
_bcm_tx_callback_thread(void *param)
{
    ...
    while (1) {
        ...
        while (dvdesc) {
            ...
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dvdesc->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, TRUE));
            _bcm_tx_desc_done(dvdesc->dv_unit, dvdesc, null_dcb);
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dvdesc->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, FALSE));
        ...
        while (dvrld) {
            ...
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dvrld->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, TRUE));
            _bcm_tx_reload_done(dvrld->dv_unit, dvrld);
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dvrld->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, FALSE));
        ...
        while (dv) {
            ...
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dv->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, TRUE));
            _bcm_tx_chain_done(dv->dv_unit, dv);
            DNXC_MTA(dnxc_multithread_analyzer_declare_api_in_play(dv->dv_unit, __FUNCTION__, MTA_FLAG_THREAD_MAIN_FUNC, FALSE));
