...
/* } */
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        if (SOC_IS_DNX(unit)) {
            ...
            if (queue_id < 0)
            {
                for (queue_id = 0; queue_id < 48; queue_id++)
                {
                    cfg = (queue_id < 32) ? 0x1:0x2;
                    queue_mask = (uint32)(1 << (queue_id % 32));
...
/* } */
        ...
        if (!SOC_DCB_RX_ERROR_GET(unit, dcb) && abort_cleanup_done) {
            ...
            if (SOC_IS_DNX(unit)) {
                ...
                {
                    if (channel_index == chan)
                    {
                        int base_q_pair = 0;
                        BCM_GPORT_LOCAL_SET(pkt->dst_gport, logical_port);
                        /** set queue_base to rx_port*/
                        dnx_algo_port_base_q_pair_get(unit, logical_port, &base_q_pair);
...
/* } */
        ...
        if (SOC_IS_TRIDENT2PLUS(unit) &&
            BCM_GPORT_IS_MPLS_PORT(pkt->src_gport) &&
            !_BCM_VPN_IS_SET(pkt->vlan)) {
...
/* } */
    ...
    if (soc_feature(unit, soc_feature_cmicx) && (!soc_feature(unit, soc_feature_no_ep_to_cpu))) {
        uint32  hdr_size = 0;
...
bcm_rx_trap_lif_config_t_init(bcm_rx_trap_lif_config_t *lif_config_p)
{
    ...
    {
        sal_memset(lif_config_p, 0, sizeof (bcm_rx_trap_lif_config_t));
...
/* } */
    ...
    if (SOC_IS_DNX(unit) && SOC_UNIT_VALID(unit)) {
         dnx_rx_packet_parse(unit, pkt);
    }
#endif
/* } */
