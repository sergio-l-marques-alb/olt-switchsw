...
#include <include/bcm_int/esw/tomahawk3.h>
#ifdef BCM_XGS3_SWITCH_SUPPORT

/*
 * Function:
 *      _bcm_th3_cache_gport_resolve
 * Purpose:
 ...
 */
STATIC int 
_bcm_th3_cache_gport_resolve(int unit, bcm_l2_cache_addr_t *l2caddr)
{
    bcm_port_t      port;
    bcm_module_t    modid;
    bcm_trunk_t     tgid;
    ...
    if (BCM_GPORT_IS_SET(l2caddr->dest_port)) {
        ...
        if (BCM_TRUNK_INVALID != tgid) {
            ...
            l2caddr->dest_modid = modid; /* This may be set to 0 - check */
        }
    }
...
/*
 * Function:
 *      _bcm_th3_cache_gport_construct
 * Purpose:
 ...
 */
STATIC int 
_bcm_th3_cache_gport_construct(int unit, bcm_l2_cache_addr_t *l2caddr)
{
    bcm_gport_t         gport;
    ...
    BCM_IF_ERROR_RETURN(_bcm_esw_gport_construct(unit, &gport_dst, &gport));

    l2caddr->dest_port = gport;

    gport_dst.gport_type = BCM_GPORT_TYPE_MODPORT;
    gport_dst.port = l2caddr->src_port;
    gport_dst.modid = SOC_BASE_MODID(unit);
                       ...
                       bcm_l2_cache_addr_t *l2caddr, l2u_entry_t *l2u_entry)
{
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        if (isGport) {
            BCM_IF_ERROR_RETURN(_bcm_th3_cache_gport_construct(unit, l2caddr));
        }
 ...
 *      BCM_E_XXX
 * Note:
 * BCM_L2_CACHE_L3, BCM_L2_CACHE_MIRROR are not supported
 */
STATIC int
_bcm_th3_l2_cache_to_l2u(int unit, 
                         l2u_entry_t *l2u_entry, bcm_l2_cache_addr_t *l2caddr)
{
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        if (BCM_GPORT_IS_SET(l2caddr->dest_port) ||
            ...
            BCM_IF_ERROR_RETURN(_bcm_th3_cache_gport_resolve(unit, l2caddr));
            isGport = 1;
        } else {
            isGport = 0;
        }
        ...
        _l2u_field32_set(unit, &l2u_mask_entry, KEY_TYPEf, 0);
        ...
        if (!((l2caddr->flags & BCM_L2_CACHE_TRUNK) ||
            ...
            if (!SOC_PORT_ADDRESSABLE(unit, port_in)) {
                return BCM_E_PORT;
            }
        ...
        if (l2caddr->flags & BCM_L2_CACHE_MULTICAST) {
            if (_BCM_MULTICAST_IS_SET(l2caddr->group)) {
                if (_BCM_MULTICAST_IS_L2(l2caddr->group)) {
                    port_field_len = soc_mem_field_length(unit, L2_USER_ENTRYm,
                                                          port_field);
                    /* If MACDA is MC, HW interprets value encoded in 
                     * PORT_FIELD as L2MC_INDEX
                     */
 ...
 */
int
bcm_th3_l2_cache_init(int unit)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
    int     skip_l2u;
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_th3_l2_cache_to_l2u(unit, &entry, &addr));
        SOC_IF_ERROR_RETURN(soc_l2u_insert(unit, &entry, -1, &index));

        soc_mem_field32_set(unit, L2_USER_ENTRYm, &entry, L2_PROTOCOL_PKTf, 1);
        soc_mem_field32_set(unit, L2_USER_ENTRYm, &entry, KEY_TYPEf, 0);
        SOC_IF_ERROR_RETURN(soc_l2u_insert(unit, &entry, -1, &index));
 ...
 */
int
bcm_tomahawk3_l2_cache_set(int unit, int index, bcm_l2_cache_addr_t *addr,
                           int *index_used)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
    int skip_l2u;
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_th3_l2_cache_to_l2u(unit, &l2u_entry,
                                                     &l2_addr));
 ...
 */
int
bcm_tomahawk3_l2_cache_get(int unit, int index, bcm_l2_cache_addr_t *addr)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
    int skip_l2u;
    l2u_entry_t l2u_entry;
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_th3_l2_cache_from_l2u(unit, addr, &l2u_entry));

#if defined(BCM_TRIDENT_SUPPORT)
        if (addr->flags & BCM_L2_CACHE_L3) {
            BCM_IF_ERROR_RETURN(bcm_td_l2cache_myStation_get(unit, index,
                                                             addr));
        }
...
/*
 * Function:
 *      bcm_tomahawk3_l2_cache_size_get
 * Purpose:
 ...
 */
int
bcm_tomahawk3_l2_cache_size_get(int unit, int *size)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
    int skip_l2u;
...
int
bcm_tomahawk3_l2_cache_init(int unit)
{
    return bcm_th3_l2_cache_init(unit);
}

/*
 * Function:
 *      bcm_tomahawk3_l2_cache_delete
 * Purpose:
 ...
 */
int
bcm_tomahawk3_l2_cache_delete(int unit, int index)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
    int skip_l2u, rv;
    ...
    if (soc_feature(unit, soc_feature_l2_user_table) && !skip_l2u) {
        ...
        if (BCM_SUCCESS(rv))  {
            /* TH3 doesn't have L3,RESERVED_0 fields, ignoring return value,
             * as done in legacy code
             */
 ...
 */
int
bcm_tomahawk3_l2_cache_delete_all(int unit)
{
#ifdef BCM_XGS3_SWITCH_SUPPORT
