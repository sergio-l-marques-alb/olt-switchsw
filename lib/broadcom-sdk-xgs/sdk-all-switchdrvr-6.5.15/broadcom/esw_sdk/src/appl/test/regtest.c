...
#endif /* BCM_TRIUMPH2_SUPPORT */
    {
        ...
        if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
           ...
           if (SOC_IS_DNX(unit)) {
                ...
                int init_steps[] =
                    { DNX_INIT_STEP_HARD_RESET, DNX_INIT_STEP_RING_CONFIG, DNX_INIT_STEP_SOFT_RESET,
                    DNX_INIT_STEP_PLL
                };
                r = dnx_init_step_list_convert(unit, dnx_init_deinit_seq, &utilex_list);
                if(r < 0) {
                    LOG_ERROR(BSL_LS_APPL_COMMON,
                              (BSL_META_U(unit,
                                          "ERROR: Unable to reinit unit %d: %s\n"), unit, soc_errmsg(r)));
                ...
                */
                for (i = 0; i < 4; i++)
                {
                    r = utilex_seq_run_step_by_id(unit, utilex_list, init_steps[i], 1, &step_found);
                    if(r < 0) {
                        LOG_ERROR(BSL_LS_APPL_COMMON,
                                (BSL_META_U(unit,
                                            "ERROR: Unable to reinit unit %d: %s\n"), unit, soc_errmsg(r)));
        ...
        for (i = 0; i < brdc_count; i++) {
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++) {
                ...
                /* Check the size of  the register by counting the fields and storing the number in reg_size */
                    ...
                    sal_strncpy(reg_name,"BRDC_", sizeof(char)*5);
            ...
            for (mem = 0; mem < NUM_SOC_MEM; mem++) {
                ...
                if(blk == blk_first_instance) {
                    ...
                    sal_strncpy(mem_name,"BRDC_", sizeof(char)*5);
