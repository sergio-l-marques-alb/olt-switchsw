        ...
        int copyno, int index, uint32 *entry_data)
{
...

    {
#if defined(BCM_ESW_SUPPORT) || defined (BCM_PETRA_SUPPORT) || defined (BCM_DFE_SUPPORT) || defined (BCM_DNX_SUPPORT) || defined (BCM_DNXF_SUPPORT)
#ifdef BCM_DNX_SUPPORT
        if (parm->mem == FQP_OTM_METER_TABLEm)
        {
            rv = dnx_dbal_egq_credit_mem_read_start(parm->unit, &egq_credit_read_status);
        }
#endif
        ...
        if (parm->mem == FQP_OTM_METER_TABLEm)
        {
            rv |= dnx_dbal_egq_credit_mem_read_end(parm->unit, &egq_credit_read_status);
        }
#endif
        if (rv != 0)
        {
...
mem_test_init(int u, args_t *a, void **p)
{
    ...
    if (!soc_mem_is_valid(u, parm->mem) ||
        soc_mem_is_readonly(u, parm->mem) 
#if defined(BCM_DFE_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT)
        || 
