...
td_test_data_init(int unit, int alloc)
{
    ...
    if (!td_test_data.init_done) {
         ...
         if (SOC_IS_DFE(unit) || SOC_IS_ARAD(unit) || SOC_IS_DNXF(unit) || SOC_IS_DNX(unit)) {
       /* no default for dfe */
        } else
#endif /* BCM_DFE_SUPPORT */
        {
...
int pstats_tr_init(int unit) {
    ...
    if (SOC_IS_HELIX5(unit)) {
        if (soc_helix5_mmu_pstats_mode_set(unit, 0) < 0) {
            return -1;
        }
    }
...
#if defined (BCM_HELIX5_SUPPORT)
    if (SOC_IS_HELIX5(unit)) {
        flag = _HX5_MMU_PSTATS_ENABLE |
               _HX5_MMU_PSTATS_SYNC |
               _HX5_MMU_PSTATS_HWM_MOD |
               _HX5_MMU_PSTATS_MOR_EN;
        if (soc_helix5_mmu_pstats_mode_set(unit, flag) < 0) {
            return -1;
        }
    }
#endif /* BCM_HELIX5_SUPPORT */
  ...
  uint32 *read_dma_buf)  /* input: DMA buffer fitting one array index of the memory */
{
    ...
    for (array_index = array_index_min; array_index <= array_index_max; array_index++) {
        ...
        SOC_MEM_BLOCK_ITER(unit, mem, blk) {
            ...
            for (index = index_max, p = read_dma_buf; index >= 0; --index, p += entry_words) {
    ...
    for (array_index = array_index_min; array_index <= array_index_max; array_index++) {
        ...
        SOC_MEM_BLOCK_ITER(unit, mem, blk) {
            ...
            for (index = index_max, p = read_dma_buf, q = write_dma_buf; index >= 0; --index, p += entry_words, q += entry_words) {
