...
int dnx_mdb_phymem_ser_verify(int unit, soc_mem_t mem, uint8 mdb_flag)
{
    ...
    sand_block_control_info_t*  sand_bloc_info;
    soc_mem_t   mdb_phy_name_list[] = {
        DDHA_MACRO_0_PHYSICAL_ENTRY_BANKm,
        DDHA_MACRO_1_PHYSICAL_ENTRY_BANKm,
        DDHB_MACRO_0_PHYSICAL_ENTRY_BANKm,
        DDHB_MACRO_1_PHYSICAL_ENTRY_BANKm,
        DHC_MACRO_PHYSICAL_ENTRY_BANKm
    };
    ser_test_params_t *ser_test_params = ser_parameters[unit];
    ...
    {
        for (ii=0; ii<COUNTOF(mdb_phy_name_list); ii++)
        {
            phy_mem = mdb_phy_name_list[ii];
            copyno  = 0;
...
int memory_ser_test_run_mem(int unit, args_t *a, void *p)
{
    ...
    if (mem == FQP_OTM_METER_TABLEm)
    {
        _SOC_IF_ERR_EXIT(dnx_dbal_egq_credit_mem_read_start(unit, &egq_credit_read_status));
    }
    ...
    if (mem == FQP_OTM_METER_TABLEm)
    {
        _SOC_IF_ERR_CONT(dnx_dbal_egq_credit_mem_read_end(unit, &egq_credit_read_status));
    }
...
int mem_ser_test_cb(int unit, soc_mem_t mem,void *p)
{
    ...
    if (SOC_BLOCK_TYPE(unit, SOC_MEM_BLOCK_ANY(unit, mem)) == SOC_BLK_IPSEC) {
        ...
        if(dev_id == BCM88272_DEVICE_ID || dev_id == BCM88273_DEVICE_ID ||dev_id == BCM88278_DEVICE_ID) {
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,"Skip on Mem:%s\n"),SOC_MEM_NAME(unit, mem)));
            ser_test_params->skipped_counter++;
            return rv;
        }
