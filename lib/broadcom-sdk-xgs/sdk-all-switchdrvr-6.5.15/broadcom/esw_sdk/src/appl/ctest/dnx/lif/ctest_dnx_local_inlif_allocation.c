    ...
    sh_sand_control_t * sand_control)
{
    int global_sbc_inlifs[NOF_INLIFS_FOR_TEST], global_dpc_all_cores_inlifs[NOF_INLIFS_FOR_TEST],
        global_dpc_inlifs[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES][NOF_INLIFS_FOR_TEST];
    int local_sbc_inlifs[NOF_INLIFS_FOR_TEST], local_dpc_all_cores_inlifs[NOF_INLIFS_FOR_TEST],
        local_dpc_inlifs[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES][NOF_INLIFS_FOR_TEST];
    int sbc_index, dpc_all_cores_index, dpc_index[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    lif_mngr_local_inlif_info_t sbc_inlif_info, dpc_all_cores_inlif_info,
        dpc_inlif_info[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    ...
    DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, current_core)
    {
        ...
        if (current_core % 2 == 0)
        {
            dpc_inlif_info[current_core].dbal_result_type =
                DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_VSI_MP;
        }
        else
        {
            dpc_inlif_info[current_core].dbal_result_type =
    ...
    ctest_dnx_local_inlif_free_all(unit, TRUE, &sbc_inlif_info, &sbc_index, global_sbc_inlifs, local_sbc_inlifs);
    ctest_dnx_local_inlif_free_all(unit, TRUE, &dpc_all_cores_inlif_info, &dpc_all_cores_index,
                                   global_dpc_all_cores_inlifs, local_dpc_all_cores_inlifs);

    DNXCMN_CORES_ITER(unit, _SHR_CORE_ALL, current_core)
    {
        ctest_dnx_local_inlif_free_all(unit, TRUE,
