...
/* Field Group ID legal value */
#define DNX_FIELD_GROUP_ID                  10
...
static dnx_field_action_id_t action_id_array[NUM_QUALS_AND_ACTIONS_PER_STAGE] = {
    DBAL_ENUM_FVAL_IPMF1_ACTION_GLOB_IN_LIF_0,  /* IPMF1 */
    DBAL_ENUM_FVAL_IPMF1_ACTION_DST_DATA,       /* IPMF2 */
    DBAL_ENUM_FVAL_IPMF3_ACTION_ACL_CONTEXT,    /* IPMF3 */
    DBAL_ENUM_FVAL_EPMF_ACTION_TC       /* EPMF */
};
...
static dnx_field_qual_t quals_array[NUM_QUALS_AND_ACTIONS_PER_STAGE] = {
    DNX_FIELD_IPMF1_QUAL_TC,    /* IPMF1 */
    DNX_FIELD_IPMF2_QUAL_PMF1_TCAM_ACTION_0,    /* IPMF2 */
    DNX_FIELD_IPMF3_QUAL_RPF_ECMP_MODE, /* IPMF3 */
    DNX_FIELD_EPMF_QUAL_PPH_NWK_QOS     /* EPMF */
};
    ...
    int test_mode)
{
    ...
    if (test_mode == TESTING_QUALIFIERS)
    {
         ...
         */
        fg_info->dnx_actions[0][VALID_INDEX] =
    ...
    else if (test_mode == TESTING_ACTIONS)
    {
         ...
         * Note that, in this test, currently, we are only referring to the first '2msb combination'
         */
        fg_info->dnx_actions[0][VALID_INDEX] = dnx_action;
    }
    fg_info->dnx_quals[INVALID_INDEX] = DNX_FIELD_QUAL_TYPE_INVALID;
    fg_info->dnx_actions[0][INVALID_INDEX] = DNX_FIELD_ACTION_INVALID;

    LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %s .Performing group add : %d \r\n",
                __FUNCTION__, __LINE__, dnx_field_stage_text(unit, fg_info->field_stage), fg_id);
    SHR_IF_ERR_EXIT(dnx_field_group_add(unit, (DNX_FIELD_GROUP_ADD_FLAG_WITH_ID), fg_info, &fg_id));
    ...
    int test_mode)
{
     ...
     * Note that, in this test, currently, we are only referring to the first '2msb combination'
     */
    attach_info.dnx_actions[0][VALID_INDEX] = fg_info->dnx_actions[0][VALID_INDEX];
    attach_info.dnx_actions[0][INVALID_INDEX] = fg_info->dnx_actions[0][INVALID_INDEX];

    /*
     * Due to the use of invalidate next, that requires the priority to demand a specific FES ID,
     * we provide a FES ID.
     */
    attach_info.action_info[0][VALID_INDEX].priority = BCM_FIELD_ACTION_POSITION(0, 5);

    LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %s. Performing context attach on conxtex : %d \r\n",
                __FUNCTION__, __LINE__, dnx_field_stage_text(unit, fg_info->field_stage), context_id);
    SHR_IF_ERR_EXIT(dnx_field_group_context_attach
                    (unit, DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE, fg_id, context_id, &attach_info));
...
/* } */
    if (field_qual_test_type_name == 1)
    {
         ...
         * dnx_field_group_attach_info_t attach_info;
         */
        dnx_field_qual_t *dnx_qual_list;
        ctest_dnx_field_qual_qual_info_t dnx_qual_info;
        dnx_field_group_t fg_id;
        dnx_field_context_t context_id;
        dnx_field_action_t *dnx_action_list;
        ...
        if (sal_strcasecmp(field_qual_test_mode_name, TEST_MODE_QUALIFIERS) == 0)
        {
            ...
            LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Options %d %s. Field Group add returned 'success' flag\r\n",
                        __FUNCTION__, __LINE__, field_qual_test_type_name, field_qual_test_mode_name);
            ...
            for (i_qual = 0; i_qual < dnx_field_qual_num; i_qual++)
            {
                ...
                if (sal_strcasecmp(dnx_qual_name, "CORE_ID") == 0)
                {
                    continue;
                }
                dnx_field_map_dnx_qual_size(unit, field_stage, dnx_qual_info.dnx_qual, &qual_size);
        ...
        else if (sal_strcasecmp(field_qual_test_mode_name, TEST_MODE_ACTIONS) == 0)
        {
