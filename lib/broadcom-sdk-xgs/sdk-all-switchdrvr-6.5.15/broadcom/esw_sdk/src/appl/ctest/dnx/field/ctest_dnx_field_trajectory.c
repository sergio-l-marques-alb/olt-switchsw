 ...
 * {
 */
/* The context ID in use by iPMF 1 and iPMF 2.*/
#define DNX_DIAG_FIELD_TRAJECTORY_CONTEXT_ID          5
    ...
    int unit)
{
    ...
    context_id = DNX_DIAG_FIELD_TRAJECTORY_CONTEXT_ID;

    /**
     * Create and configure the context
     */
    flags = 0;
    flags |= DNX_FIELD_CONTEXT_FLAG_WITH_ID;
    context_mode.context_ipmf1_mode.hash_mode = DNX_FIELD_CONTEXT_HASH_MODE_ENABLED;
    SHR_IF_ERR_EXIT(dnx_field_context_create(unit, flags, DNX_FIELD_STAGE_IPMF1, &context_mode, &context_id));
    ...
    /*
     * Use only first '2msb' combination.
     */
    a_info.action_info[0][0].priority = BCM_FIELD_ACTION_PRIORITY(0, 7);
    SHR_IF_ERR_EXIT(dnx_field_group_context_attach
                    (unit, DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE, fg_id, context_id, &a_info));
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_add(unit, fg_id, &entry_in_info, &entry_handle));
    /**
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strcasecmp(test_type_name, TEST_TYPE_INGRESS) == 0)
    {
        ...
        SHR_IF_ERR_EXIT(appl_dnx_trajectory_ingress(unit));
        LOG_INFO_EX(BSL_LOG_MODULE,
        ...
        if (test_clean == TRUE)
        {
            ...
            CTEST_DNX_FIELD_UTIL_ERR(dnx_field_context_hash_destroy
                                     (unit, DNX_FIELD_CONTEXT_HASH_FLAGS_NONE, DNX_FIELD_STAGE_IPMF1,
                                      DNX_DIAG_FIELD_TRAJECTORY_CONTEXT_ID));
