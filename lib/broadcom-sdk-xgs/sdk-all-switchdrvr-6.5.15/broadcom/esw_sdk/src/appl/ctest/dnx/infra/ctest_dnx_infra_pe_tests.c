...
static int
periodic_event_unit_test_callback(
    int unit,
    void *user_data)
{
...
static int
periodic_event_unit_test_error_callback(
    int unit,
    void *user_data)
{
    ...
    sh_sand_control_t * sand_control)
{
    ...
    periodic_event_handler_t my_handler;
    periodic_event_config_t config;
    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_INT32("start", start_operation);

    periodic_event_config_t_init(&config);
    config.name = "Periodic";
    config.interval = 100000;
    config.callback = &periodic_event_unit_test_callback;
     ...
     */
    SHR_CLI_EXIT_IF_ERR(periodic_event_create(unit, &config, &my_handler),
    ...
    if (!start_operation)
    {
        ...
        SHR_CLI_EXIT_IF_ERR(periodic_event_start(my_handler), "Test FAIL. Failed to re-start periodic event\n");
     ...
     */
    SHR_CLI_EXIT_IF_ERR(periodic_event_stop(my_handler, 1, 0), "Test FAIL. Failed to stop periodic event\n");
     ...
     * destroy 
     */
    SHR_CLI_EXIT_IF_ERR(periodic_event_destroy(&my_handler, 1), "Test FAIL. Failed to destroy periodic event\n");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (error_test)
    {
        ...
        config.callback = &periodic_event_unit_test_error_callback;
    }
    ...
    {
        SHR_CLI_EXIT_IF_ERR(periodic_event_trigger(my_handler), "Test FAIL. Failed to trigger event\n");
        sal_usleep(10000 * 10);
    }
    ...
    if (error_test)
    {
        int rv = periodic_event_trigger(my_handler);
...
static sh_sand_invoke_t diag_dnx_pe_periodic_tests[] = {
    {"periodic", "start=1"},
    {"create after start", "start=0"},
    {NULL}
};

static sh_sand_invoke_t diag_dnx_pe_trigger_tests[] = {
    {"trigger", "error_test=0"},
    {"Error Threashold", "error_test=1"},
    {NULL}
};
...
sh_sand_cmd_t dnx_infra_periodic_event_test_cmds[] = {
