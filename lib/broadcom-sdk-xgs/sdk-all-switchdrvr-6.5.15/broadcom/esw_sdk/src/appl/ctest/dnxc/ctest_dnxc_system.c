    ...
    int flags)
{
    ...
    {
        /*
         * Run invoke callback if exists
         */
        if (sh_sand_cmd->invoke_cb != NULL)
        {
    ...
    sh_sand_test_t * sh_sand_test)
{
     ...
     */
    sal_strncpy(teststr, "\tbcm shell \"", SH_SAND_MAX_TOKEN_SIZE - 1);
    ...
    int flags)
{
    ...
    SHR_SET_CURRENT_ERR(sh_sand_option_list_process(unit, leaf_args, sh_sand_cmd, &sand_ctr_m, sh_sand_cmd->flags));
    if (SHR_FUNC_VAL_IS(_SHR_E_NONE) &&
    ...
    int flags)
{
    ...
    RHITERATOR(sh_sand_test, test_list)
    {
        ...
        if (SOC_IS_DNX(unit) && ((sh_sand_cmd->flags & SH_CMD_ROLLBACK) || (sh_sand_test->flags & SH_CMD_ROLLBACK)))
        {
            ...
            cmd_ctest_run_test(unit, sh_sand_cmd, sh_sand_test, flex_ptr, flags);
