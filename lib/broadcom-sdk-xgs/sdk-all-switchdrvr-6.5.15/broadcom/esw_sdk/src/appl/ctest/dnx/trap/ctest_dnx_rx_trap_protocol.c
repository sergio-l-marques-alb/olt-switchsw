    ...
    bcm_gport_t port)
{
    ...
    result = dnx_rx_trap_protocol_profiles_set(unit, port, &protocol_profiles);
    ...
    result = dnx_rx_trap_protocol_profiles_get(unit, port, &protocol_profiles_get);
    if (BCM_FAILURE(result))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    result = dnx_rx_trap_protocol_profiles_set(unit, port, &protocol_profiles_reset);
    if (BCM_FAILURE(result))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "dnx_rx_trap_protocol_profiles_set failed. Error:%d (%s)\n"), result,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if(sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_MODE_LIF) == 0)
    {
        result = ctest_dnx_rx_trap_inlif_create_example(unit, &lif);
        if (BCM_FAILURE(result))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    for(kk = 0; kk < 2; kk++)
    {
        for (ii = 0; protocol_types[ii] != bcmRxTrapCount; ii++)
        {
            for (trap_arg = 0; trap_arg < nof_trap_args[ii]; trap_arg++)
            {
                for (protocol_trap_profile = 0; protocol_trap_profile < 4; protocol_trap_profile++)
                {
                    ...
                    result = dnx_rx_trap_protocol_set(unit, &protocol_key, trap_gport);
                    if (BCM_FAILURE(result))
                    {
                        LOG_ERROR(BSL_LS_APPL_COMMON,
                                  (BSL_META_U(unit, "dnx_rx_trap_protocol_set failed. Error:%d (%s)\n"), result,
                                   bcm_errmsg(result)));
                        return result;
                    }

                    trap_gport_get = 0;
                    result = dnx_rx_trap_protocol_get(unit, &protocol_key, &trap_gport_get);
                    if (BCM_FAILURE(result))
                    {
                        LOG_ERROR(BSL_LS_APPL_COMMON,
                                  (BSL_META_U(unit, "dnx_rx_trap_protocol_get failed. Error:%d (%s)\n"), result,
                                   bcm_errmsg(result)));
                        return result;
                    }
                    ...
                    result = dnx_rx_trap_protocol_clear(unit, &protocol_key);
                    if (BCM_FAILURE(result))
                    {
                        LOG_ERROR(BSL_LS_APPL_COMMON,
                                  (BSL_META_U(unit, "dnx_rx_trap_protocol_destroy failed. Error:%d (%s)\n"), result,
                                   bcm_errmsg(result)));
                        return result;
                    }
                }
     ...
     if(sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_MODE_LIF) == 0)
    {
        result = ctest_dnx_rx_trap_lif_destroy_example(unit, &lif);
        if (BCM_FAILURE(result))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_rx_trap_protocol_key_t protocol_key;
    bcm_gport_t trap_gport, port, lif, gport;
    bcm_rx_trap_t protocol_types[] = { bcmRxTrapL2cpPeer, bcmRxTrapIcmpRedirect, bcmRxTrap8021xFail,
        bcmRxTrapIgmpMembershipQuery, bcmRxTrapIgmpReportLeaveMsg, bcmRxTrapIgmpUndefined,
        bcmRxTrapDhcpv4Client, bcmRxTrapDhcpv4Server, bcmRxTrapDhcpv6Client,
        bcmRxTrapDhcpv6Server, bcmRxTrapArp, bcmRxTrapArpMyIp, bcmRxTrapCount
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_set NEGATIVE PROTOCOL TRAP CONFIGURATION START...\n")));
    ...
    if(sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_MODE_LIF) == 0)
    {
        ...
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_set should fail with BCM_E_PARAM! . Error:%d (%s)\n"), result,
                       bcm_errmsg(result)));
            return result;
        }
        gport = lif;
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_set NEGATIVE PROTOCOL TRAP CONFIGURATION END...\n")));

    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));

    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_get NEGATIVE PROTOCOL TRAP CONFIGURATION START...\n")));
    ...
    if(sal_strcasecmp(test_mode, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_MODE_LIF) == 0)
    {
        ...
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_get should fail with BCM_E_PARAM! . Error:%d (%s)\n"), result,
                       bcm_errmsg(result)));
            return result;
        }
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_profiles_get NEGATIVE PROTOCOL TRAP CONFIGURATION END...\n")));

    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "\n")));

    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_set NEGATIVE PROTOCOL TRAP CONFIGURATION START...\n")));
    ...
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "bcm_rx_trap_protocol_set should fail with BCM_E_PARAM! . Error:%d (%s)\n"), result,
                   bcm_errmsg(result)));
        return result;
    }
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_set NEGATIVE PROTOCOL TRAP CONFIGURATION END...\n")));

    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_get NEGATIVE PROTOCOL TRAP CONFIGURATION START...\n")));
    ...
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "bcm_rx_trap_protocol_get should fail with BCM_E_PARAM! . Error:%d (%s)\n"), result,
                   bcm_errmsg(result)));
        return result;
    }
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "dnx_rx_trap_protocol_set NEGATIVE PROTOCOL TRAP CONFIGURATION END...\n")));


    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_clear NEGATIVE PROTOCOL TRAP CONFIGURATION START...\n")));
    ...
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "bcm_rx_trap_protocol_clear should fail with BCM_E_PARAM! . Error:%d (%s)\n"), result,
                   bcm_errmsg(result)));
        return result;
    }
    ...
    LOG_INFO(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "bcm_rx_trap_protocol_clear NEGATIVE PROTOCOL TRAP CONFIGURATION END...\n")));
...
sh_sand_option_t sh_dnx_rx_trap_protocol_options[] = {
     ...
     * Default
     */
    {CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_TYPE, SAL_FIELD_TYPE_STR, "Type of test positive, negative or exhaustive", "positive"}
    ,
    {CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_MODE, SAL_FIELD_TYPE_STR, "Mode of Protocol traps, per In-Port or per In-LIF", "lif"}
    ,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if(sal_strcasecmp(test_type, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_TYPE_POSITIVE) == 0)
    {
        SHR_IF_ERR_EXIT(appl_dnx_rx_trap_protocol_pos(unit, args, sand_control));
    }
    else if(sal_strcasecmp(test_type, CTEST_DNX_RX_TRAPS_PROTOCOL_OPTION_TEST_TYPE_NEGATIVE) == 0)
    {
