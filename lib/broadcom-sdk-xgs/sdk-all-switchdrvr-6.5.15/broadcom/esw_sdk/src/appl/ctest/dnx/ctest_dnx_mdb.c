    ...
    mdb_test_mode_e mode)
{
     ...
     */
    SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, first_src_cluster.dbal_physical_table_id, mode));
    if (first_dst_cluster.dbal_physical_table_id != first_src_cluster.dbal_physical_table_id)
    {
        SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, first_dst_cluster.dbal_physical_table_id, mode));
    }
    ...
    mdb_test_mode_e mode)
{
    ...
    {
        SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, dbal_physical_table_id, mode));
    }
    else
    {
        SHR_IF_ERR_EXIT(mdb_direct_table_test(unit, dbal_physical_table_id, mode));
    }
    ...
    {
        SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, first_src_cluster.dbal_physical_table_id, mode));
        SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, second_src_cluster.dbal_physical_table_id, mode));
    }
    else
    {
        SHR_IF_ERR_EXIT(mdb_direct_table_test(unit, first_src_cluster.dbal_physical_table_id, mode));
        SHR_IF_ERR_EXIT(mdb_direct_table_test(unit, second_src_cluster.dbal_physical_table_id, mode));
    }
    ...
    int delete_cmd)
{
    ...
    if ((mode == MDB_TEST_CAPACITY) && (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM))
    {
    ...
    if ((mode == MDB_TEST_CAPACITY)
        && !((mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM) || (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_KAPS)))
    {
        SHR_ERR_EXIT(_SHR_E_NONE, "Capacity test is only available for LPM and EM.\n");
    }
    ...
    else if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_KAPS)
    {
        ...
        if (TRUE)
        {
            if (dynamic == 0)
            {
                SHR_IF_ERR_EXIT(mdb_lpm_test(unit, table_id, mode, delete_cmd));
            }
            else
            {
    ...
    else if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
    {
        ...
        if (dynamic == 0)
        {
            SHR_IF_ERR_EXIT(mdb_em_test(unit, table_id, mode, delete_cmd));
        }
        else
        {
            SHR_IF_ERR_EXIT(mdb_em_dynamic_table_test(unit, table_id, mode));
        }
    }
    ...
    else if (mdb_db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EEDB)
    {
        ...
        if (dynamic == 0)
        {
            SHR_IF_ERR_EXIT(mdb_eedb_table_test(unit, table_id, mode));
        }
        else if (dynamic == 1)
        {
            SHR_IF_ERR_EXIT(mdb_eedb_dynamic_cluster_test(unit, table_id, mode));
...
sh_sand_invoke_t dnx_mdb_tests[] = {
    ...
    ,
#if !defined(ADAPTER_SERVER_MODE)
    {"lpm_private_capacity", "table=KAPS1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"lpm_public_capacity", "table=KAPS2 mode=2", CTEST_POSTCOMMIT}
    ,
    {"ivsi_capacity", "table=IVSI mode=1", CTEST_POSTCOMMIT}
    ,
    {"isem_1_capacity", "table=ISEM1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"isem_2_capacity", "table=ISEM2 mode=2", CTEST_POSTCOMMIT}
    ,
    {"isem_3_capacity2", "table=ISEM3 mode=2", CTEST_POSTCOMMIT}
    ,
    {"inlif_1_capacity1", "table=INLIF1 mode=1", CTEST_POSTCOMMIT}
    ,
    {"inlif_2_capacity", "table=INLIF2 mode=1", CTEST_POSTCOMMIT}
    ,
    {"inlif_3_capacity", "table=INLIF3 mode=1", CTEST_POSTCOMMIT}
    ,
    {"ioem_1_capacity", "table=IOEM1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"ioem_2_capacity", "table=IOEM2 mode=2", CTEST_POSTCOMMIT}
    ,
    ...
    ,
    {"ppmc_capacity", "table=PPMC mode=2", CTEST_POSTCOMMIT}
    ,
    {"glem_1_capacity", "table=GLEM1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"glem_2_capacity", "table=GLEM2 mode=2", CTEST_POSTCOMMIT}
    ,
    {"eoem_1_capacity", "table=EOEM1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"eoem_2_capacity", "table=EOEM2 mode=2", CTEST_POSTCOMMIT}
    ,
    {"esem_capacity", "table=ESEM mode=2", CTEST_POSTCOMMIT}
    ,
    {"evsi_capacity", "table=EVSI mode=1", CTEST_POSTCOMMIT}
    ,
    {"sexem_1_capacity", "table=SEXEM1 mode=2", CTEST_POSTCOMMIT}
    ,
    {"sexem_2_capacity", "table=SEXEM2 mode=2", CTEST_POSTCOMMIT}
    ,
    {"sexem_3_capacity", "table=SEXEM3 mode=2", CTEST_POSTCOMMIT}
    ,
    {"lexem_capacity", "table=LEXEM mode=2", CTEST_POSTCOMMIT}
    ,
    {"rmep_capacity", "table=RMEP_EM mode=2", CTEST_POSTCOMMIT}
    ,
#endif
    {NULL}
};
...
sh_sand_option_t dnx_mdb_test_options[] = {
    ...
    ,
    {"MoDe", SAL_FIELD_TYPE_INT32, "The test mode: 0-fast (tests ~1/10 of the capacity), 1-full, 2-capacity test", "0"}
    ,
...
sh_sand_man_t dnx_mdb_test_man = {
    ...
    "ctest mdb [all] [table=DBAL_PHYSICAL_TABLE_NAME] [MoDe=0/1]",
    "all\n" "table=LEM mode=1"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (all != 0)
    {
        ...
        for (ii = DBAL_PHYSICAL_TABLE_NONE; ii < DBAL_NOF_PHYSICAL_TABLES; ii++)
        {
            ...
            rv_array[ii] = diag_mdb_test_physical_table(unit, ii, mode, dynamic, delete_cmd);
    ...
    else
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(diag_mdb_test_physical_table(unit, dbal_physical_table_id, mode, dynamic, delete_cmd));
        }
    }
#endif
