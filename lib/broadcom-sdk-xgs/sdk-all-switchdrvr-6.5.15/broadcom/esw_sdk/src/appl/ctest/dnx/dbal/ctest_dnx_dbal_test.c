...
extern shr_error_e dbal_hl_is_entry_default(
    int unit,
...
#ifdef INCLUDE_CTEST
extern cmd_result_t test_dnx_dbal_partial_access(
    int unit,
...
*/
uint8
table_is_exclude_from_adapter_mode(
    dbal_tables_e table_id)
{
    int ii, nof_tables;
    ...
    int *final_invalid_entries)
{
    ...
    uint32 egress_pp_port_arr[max_entries_in_test];
    uint32 egress_pp_port_core_id_arr[max_entries_in_test];
    uint32 pp_port;
    bcm_core_t core_id;
    ...
    sal_memset(egress_pp_port_arr, 0, sizeof(uint32) * max_entries_in_test);
    sal_memset(egress_pp_port_core_id_arr, 0, sizeof(uint32) * max_entries_in_test);
    ...
    BCM_PBMP_ITER(valid_ports, valid_port)
    {
        ...
        if (index < nof_entries)
        {
            ...
            egress_pp_port_arr[index] = pp_port;
            egress_pp_port_core_id_arr[index] = core_id;
            index++;
        }
        else
        {
    ...
    {
        /** Set pp_port */
        key_field_val[ii][jj][0] = egress_pp_port_arr[ii];
        /** Set core_id */
        key_field_val[ii][jj + 1][0] = egress_pp_port_core_id_arr[ii];
    ...
    uint8 is_error_recovery_overwritten_entry)
{
    ...
    if (table->table_type == DBAL_TABLE_TYPE_TCAM || table->table_type == DBAL_TABLE_TYPE_TCAM_DIRECT)
    {
        ...
        /** Committing for TCAM requires access id */
    ...
    for (jj = 0; jj < fields_comb.nof_fields; jj++)
    {
        if (fields_comb.fields_is_key[comb_index][jj])
        {
            ...
            {
                /** handle allocator */
                if (table->keys_info[jj].is_allocator && (!dbal_table_is_allcator_not_needed(unit, table_id)))
                {
        ...
        else
        {
            ...
            if (last_entry)
            {
                 ...
                 * This value is overwritten in the transcation by main algorithm.
                 */
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id,
                                      dbal_tables_is_non_direct(unit, table_id) ? DBAL_COMMIT_FORCE :
                                      DBAL_COMMIT));
    ...
    int index)
{
    ...
    LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "\n\n")));
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, table->has_result_type ? DBAL_GET_ALL_FIELDS : DBAL_COMMIT));
    ...
    int index)
{
    ...
    {
        /*
         * Clearing for TCAM requires access id
         */
        SHR_IF_ERR_EXIT(dbal_entry_handle_access_id_set(unit, entry_handle_id, access_id[ii]));
    }
    ...
    for (jj = 0; jj < table->nof_key_fields; jj++)
    {
        ...
        {
            dbal_entry_key_field_arr32_set(unit, entry_handle_id, table->keys_info[jj].field_id, key_field_val[ii][jj]);
        }
    }
    if ((dbal_physical_table_is_in_lif(table->physical_db_id[0])) ||
    ...
    uint32 flags)
{
    ...
    for (ii = 0; ii < nof_entries; ii++)
    {
        ...
        /*
         * don't add entry for tcam cornercases
         */
    ...
    uint32 flags)
{
     ...
     */
    if (_SHR_E_NONE != dnx_state_comparison_end_and_compare(unit))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Found system state difference when none was expected\n");
    }
    ...
    uint32 flags)
{
...
#else
    if (table_is_exclude_from_adapter_mode(table_id))
    {
        return _SHR_E_UNAVAIL;
    }
#endif
     ...
     * Don't check for error. If the device is already locked it should be OK.
     */
    if (table->access_method == DBAL_ACCESS_METHOD_KBP)
    {
        bcm_dnx_switch_control_set(unit, bcmSwitchExternalTcamSync, 0);
    }
    ...
    for (res_type_idx = 0; res_type_idx < nof_result_types_to_run; res_type_idx++)
    {
        ...
        if (dbal_logical_table_is_out_lif_allocator_eedb(table))
        {
            ...
            for (ii = 0; ii < nof_entries; ii++)
            {
                if (valid_key_value[ii] == TRUE)
                {
                    key_field_val[ii][0][0] = (outlif_base_val & ~(0x1FFF)) + (key_field_val[ii][0][0] & 0x1FFF);
                    if ((key_field_val[ii][0][0] & 0x1FFF) > 0x1FFB)
                    {
                    ...
                    /** check is outlif is already allocated (by SDK init procedure), in case is it, skip it */
                    {
                        ...
                        SHR_IF_ERR_EXIT(lif_mngr_db.
                                        egress_local_lif.is_allocated(unit, current_bank, key_field_val[ii][0][0],
                                                                      &is_allocated));
                        if (is_allocated)
                        {
                            valid_key_value[ii] = FALSE;
