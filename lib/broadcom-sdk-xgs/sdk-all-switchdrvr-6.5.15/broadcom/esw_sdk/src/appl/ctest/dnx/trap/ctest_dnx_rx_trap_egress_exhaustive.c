    ...
    sh_sand_control_t * sand_control)
{
    ...
    int trap_id, erpp_trap_id[nof_profiles], etpp_trap_id[nof_profiles];
    ...
    for (trap_iter = 0; trap_iter < nof_profiles; trap_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_create_and_set(unit, bcmRxTrapEgTxUserDefine, &etpp_trap_id[trap_iter],
                                                         &trap_config));

        bcm_rx_trap_config_t_init(&trap_config);
        ctest_dnx_rx_trap_erpp_fill(unit, &trap_config);
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_create_and_set(unit, bcmRxTrapEgUserDefine, &erpp_trap_id[trap_iter],
                                                         &trap_config));
    }
    ...
    for (trap_iter = 0; trap_iter < bcmRxTrapCount; trap_iter++)
    {
        ...
        if (dnx_rx_app_trap_type_map[trap_iter].valid && trap_iter != bcmRxTrapEgUnknownDa)
        {
            if (dnx_rx_app_trap_type_map[trap_iter].trap_block == DNX_RX_TRAP_BLOCK_ERPP)
            {
                trap_id = erpp_trap_id[sal_rand() % nof_profiles];
            }
            else
            {
                trap_id = etpp_trap_id[sal_rand() % nof_profiles];
            }
    ...
    for (trap_iter = 0; trap_iter < bcmRxTrapCount; trap_iter++)
    {
        if (dnx_rx_app_trap_type_map[trap_iter].valid)
        {
    ...
    for (trap_iter = 0; trap_iter < nof_profiles; trap_iter++)
    {
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_destroy(unit, etpp_trap_id[trap_iter]));
        SHR_IF_ERR_EXIT(ctest_dnx_rx_trap_destroy(unit, erpp_trap_id[trap_iter]));
    }
