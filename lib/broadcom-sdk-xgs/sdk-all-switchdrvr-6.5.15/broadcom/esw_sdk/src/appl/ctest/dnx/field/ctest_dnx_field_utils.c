...
ctest_dnx_field_util_extract_user_quals(
    ...
    uint8 qual_bitmap[CTEST_DNX_UTIL_MAX_NUM_USER_QUALS])
{
    int qual_iter;
...
ctest_dnx_field_util_extract_user_actions(
    ...
    uint8 action_bitmap[CTEST_DNX_UTIL_MAX_NUM_USER_ACTIONS])
{
    int action_iter;
    ...
    int unit)
{
    ...
    dnx_field_context_t context_id_arr[CTEST_DNX_UTIL_MAX_NUM_CONTEXT];
    uint8 action_bitmap[CTEST_DNX_UTIL_MAX_NUM_USER_ACTIONS];
    uint8 qual_bitmap[CTEST_DNX_UTIL_MAX_NUM_USER_QUALS];
    ...
    sal_memset(action_bitmap, FALSE, sizeof(action_bitmap));
    sal_memset(qual_bitmap, FALSE, sizeof(qual_bitmap));
    ...
    for (fg_id = 0; fg_id < dnx_data_field.group.nof_fgs_get(unit); fg_id++)
    {
        ...
        else
        {
            ...
            {
                if (context_id_arr[context_id_iter] == DNX_FIELD_CONTEXT_ID_INVALID)
                {
                    break;
                }
                /** Do not detach from the ITHM_PPH context.*/
                if (context_id_arr[context_id_iter] == dnx_data_field.context.default_itmh_pph_context_get(unit)
                    || context_id_arr[context_id_iter] == dnx_data_field.context.default_itmh_context_get(unit)
                    || context_id_arr[context_id_iter] == dnx_data_field.context.default_stacking_context_get(unit))
                {
                    attched_to_itmh_pph = TRUE;
                    continue;
                }
                /** Detach context if ID is valid */
                SHR_IF_ERR_EXIT(dnx_field_group_context_detach(unit, fg_id, context_id_arr[context_id_iter]));
            }
           ...
           /** Extract user defined actions for deletion, fill the action_bitmap  */
            SHR_IF_ERR_EXIT(ctest_dnx_field_util_extract_user_actions
                            (unit, fg_info.group_basic_info.dnx_actions[0], action_bitmap));
           /** Extract user defined qualifiers for deletion, fill the bitmap qual_bitmap  */
            SHR_IF_ERR_EXIT(ctest_dnx_field_util_extract_user_quals
                            (unit, fg_info.group_basic_info.dnx_quals, qual_bitmap));
            /*
    ...
    {
        if (action_bitmap[action_bitmap_iter] == TRUE)
        {
    ...
    {
        if (qual_bitmap[qual_bitmap_iter] == TRUE)
        {
    ...
    ctest_dnx_field_util_basic_tcam_fg_t * tcam_fg_util_p)
{
    ...
    for (ii = 0; ii < CTEST_DNX_FIELD_UTIL_TESTER_NUM_QUALS_ACTIONS; ii++)
    {
         ...
         */
        tcam_fg_util_p->bcm_actions[0][ii] = ctest_bcm_actions_for_init[ii];
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    for (qual_action_iter = 0; qual_action_iter < CTEST_DNX_FIELD_UTIL_TESTER_NUM_QUALS_ACTIONS; qual_action_iter++)
    {
        ...
        if (tcam_fg_util_p->bcm_actions[0][qual_action_iter] != bcmFieldActionCount)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_action_bcm_to_dnx
                            (unit, tcam_fg_util_p->field_stage, tcam_fg_util_p->bcm_actions[0][qual_action_iter],
                             &dnx_action));
            fg_info_p->dnx_actions[0][qual_action_iter] = dnx_action;
            action_count++;
        }
    }
    ...
    dnx_field_group_attach_info_t * attach_info_p)
{
    ...
    dnx_field_group_context_flags_e flags = DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE;
    ...
    dnx_field_entry_t * entry_info_p)
{
   ...
   /** Loop over the supplied arrays of BCM actions, which we limit to 3 */
    for (action_iter = 0; action_iter < action_count; action_iter++)
    {
       /** Setting action info and value to the entry */
        entry_info_p->payload_info.action_info[action_iter].dnx_action = attach_info_p->dnx_actions[0][action_iter];
   ...
   /** Loop over the supplied array of BCM qualifiers , which we limit to 3 */
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_add(unit, fg_id, entry_info_p, &entry_handle));
