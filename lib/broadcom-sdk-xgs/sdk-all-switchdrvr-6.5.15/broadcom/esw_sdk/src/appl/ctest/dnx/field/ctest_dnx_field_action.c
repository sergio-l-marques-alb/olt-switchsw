...
#include <appl/ctest/dnx/field/ctest_dnx_field_action.h>
...
sh_sand_man_t Sh_dnx_field_action_man = {
    "'Action' related test utilities",
    "Activate 'action' related test utilities as specified by 'type'\r\n"
    ...
    "ctest field action type=<database | fes_cfg | fem> stage=<ipmf1 | ipmf2 | ipmf3 | epmf> \r\n"
    ...
    dnx_field_stage_e field_stage)
{
    ...
    if ((field_stage == DNX_FIELD_STAGE_IPMF1) || (field_stage == DNX_FIELD_STAGE_IPMF2))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_actions_calc_feses
                        (unit, field_stage, field_io, actions_info_arr, actions_attach_info, fes_inst_info));
        /*
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    if (index_forward != 0)
    {
        ...
        for (action_index = 0; action_index < num_actions; action_index++)
        {
            ...
            fg_info_p->dnx_actions[0][action_index] = tcam_actions[action_index].dnx_action;
            if (fg_info_p->fg_type == DNX_FIELD_GROUP_TYPE_DIRECT_EXTRACTION
                || user_action_info.bcm_action == bcmFieldActionVoid)
            {
                fg_info_p->use_valid_bit[0][action_index] = FALSE;
            }
            else
            {
                fg_info_p->use_valid_bit[0][action_index] = TRUE;
            }
        }
    }
    else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fem_test_type)
    {
        case CTEST_DNX_FIELD_FEM_TEST_ACTION_INFO_TYPE:
        {
            ...
            {
                fem_action_info.fem_action_entry.fem_adder = DNX_FIELD_FEM_ADDER_INVALID;
            }
            else
            {
        ...
        case CTEST_DNX_FIELD_FEM_TEST_ACTIONS_FEM_TYPE:
        {
            ...
            dnx_field_fem_action_entry_t fem_action_entry[dnx_data_field.stage.stage_info_get(unit,
            ...
            for (fem_condition_index = 0; fem_condition_index < fem_condition_max; fem_condition_index++)
            {
                fem_condition_entry[fem_condition_index].fem_map_index = fem_condition_index % fem_map_index_max;
                /*
                 * Make map_data equal to 'condition'.
                 */
                fem_condition_entry[fem_condition_index].fem_map_data = fem_condition_index;
                /*
                 * Change fem_action_valid cyclically over all conditions. (Actually make it
                 * change from 0 to 1 and back as 'condition' progresses.)
                 */
            ...
            {
                fem_action_entry[fem_map_index].fem_action = DBAL_ENUM_FVAL_IPMF1_ACTION_LEARN_INFO_0 + fem_map_index;
                ...
                if (fem_id <
                    dnx_data_field.stage.stage_info_get(unit, DNX_FIELD_STAGE_IPMF1)->num_fems_with_short_action)
                {
                    fem_action_entry[fem_map_index].fem_adder = DNX_FIELD_FEM_ADDER_INVALID;
                }
                else
                {
            ...
            SHR_IF_ERR_EXIT(dnx_field_actions_fem_set
                            (unit, fem_id, fem_program, fem_condition_ms_bit, fem_condition_entry,
                             ignore_actions, fem_action_entry));
            break;
        }
...
/* *INDENT-ON* */
...

            {
                ...
                APPL_DNX_ACTION_SWSTATE_FEM_DISPLAY(unit, fem_id, "action_add", sand_control);
                /*
                 * Note that, currently, on Cmodel, we only have, effectively, 3 FEM programs per FEM
                 * (and not 4). See field_actions.h
                 */
#if (FIRST_AVAILABLE_FEM_PROGRAM == DEFAULT_FEM_PROGRAM)
/* { */
                /*
            ...
            }
            {
                ...
                for (attach_index = 0; attach_index < CTEST_DNX_FIELD_FEM_NUM_CONTEXTS_PER_FG_ON_ATTACH; attach_index++)
                {
                    ...
                    if (attach_index < 1)
                    {
                        ...
                        {
                            attach_info[attach_index].dnx_actions[0][action_index] =
                                fg_info[fg_index].dnx_actions[0][action_index];
                            attach_info[attach_index].action_info[0][action_index].priority =
    ...
    dnx_field_stage_e field_stage)
{
    ...
    if (context_fes_get_info.nof_fes_quartets != 0 && field_stage != DNX_FIELD_STAGE_IPMF3)
    {
    ...
    priority[2][1] = BCM_FIELD_ACTION_PRIORITY(0, 30);
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set(unit, field_stage, fg_id[0], context_id, is_dir_ext[0], fes_inst_info));
    fes_inst_info[0].alloc_info.priority = priority[1][0];
    fes_inst_info[1].alloc_info.priority = priority[1][1];
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set(unit, field_stage, fg_id[1], context_id, is_dir_ext[1], fes_inst_info));
    fes_inst_info[0].alloc_info.priority = priority[2][0];
    fes_inst_info[1].alloc_info.priority = priority[2][1];
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set(unit, field_stage, fg_id[2], context_id, is_dir_ext[2], fes_inst_info));
    ...
    for (fes_quartet_ndx = 0; fes_quartet_ndx < context_fes_get_info.nof_fes_quartets; fes_quartet_ndx++)
    {
        ...
        if (context_fes_get_info.fes_quartets[fes_quartet_ndx].fg_id >= dnx_data_field.group.nof_fgs_get(unit))
        {
            continue;
        }
        ...
        {
            if (context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[0].action_type !=
                context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[fes2msb_ndx].action_type
                || context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[0].valid_bits !=
                context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[fes2msb_ndx].valid_bits
                || context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[0].shift !=
                context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[fes2msb_ndx].shift
                || context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[0].type !=
                context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[fes2msb_ndx].type
                || context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[0].polarity !=
                context_fes_get_info.fes_quartets[fes2msb_ndx].fes2msb_info[fes2msb_ndx].polarity)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "After first run, the FES configuration was not as expected, "
                             "action %d in field group %d FES ID %d FES program ID %d"
                             "FES2msb ID %d has the same FES configuration as FES2msb ID 0.\r\n",
                             context_fes_get_info.fes_quartets[fes_quartet_ndx].place_in_fg,
                             context_fes_get_info.fes_quartets[fes_quartet_ndx].fg_id,
                             context_fes_get_info.fes_quartets[fes_quartet_ndx].fes_id,
    ...
    if (group_fes_get_info.nof_fes_quartets != CTEST_DNX_FIELD_ACTION_FES_CFG_NOF_ACTIONS ||
        group_fes_get_info.fes_quartets[0].priority != priority[0][0] ||
        group_fes_get_info.fes_quartets[1].priority != priority[0][1])
    {
    ...
    if (group_fes_get_info.nof_fes_quartets != CTEST_DNX_FIELD_ACTION_FES_CFG_NOF_ACTIONS ||
        group_fes_get_info.fes_quartets[0].priority != priority[1][0] ||
        group_fes_get_info.fes_quartets[1].priority != priority[1][1])
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected allocation first run field group %d.\r\n", fg_id[1]);
    }
    SHR_IF_ERR_EXIT(dnx_field_actions_context_fes_info_to_group_fes_info
    ...
    if (group_fes_get_info.nof_fes_quartets != CTEST_DNX_FIELD_ACTION_FES_CFG_NOF_ACTIONS ||
        group_fes_get_info.fes_quartets[0].priority != priority[2][0] ||
        group_fes_get_info.fes_quartets[1].priority != priority[2][1])
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected allocation first run field group %d.\r\n", fg_id[2]);
    }
    ...
    if (group_fes_get_info.nof_fes_quartets != CTEST_DNX_FIELD_ACTION_FES_CFG_NOF_ACTIONS
        || group_fes_get_info.fes_quartets[1].priority != priority[2][1]
        || group_fes_get_info.fes_quartets[1].key_select != key_select[1]
        || group_fes_get_info.fes_quartets[1].fes2msb_info[0].action_type != action_type[1]
        || group_fes_get_info.fes_quartets[1].fes2msb_info[0].valid_bits != valid_bits[1]
        || group_fes_get_info.fes_quartets[1].fes2msb_info[0].shift != shift[1]
        || group_fes_get_info.fes_quartets[1].fes2msb_info[0].type != type[1]
        || group_fes_get_info.fes_quartets[1].fes2msb_info[0].polarity != polarity[1])
    {
     ...
     */
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id[0], context_id));
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id[1], context_id));
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id[2], context_id));
    /*
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set
                    (unit, field_stage, fg_id[0], context_id_second, is_dir_ext[0], fes_inst_info));
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_set
                    (unit, field_stage, fg_id_extra, context_id, is_dir_ext[0], fes_inst_info));
    SHR_IF_ERR_EXIT(dnx_field_actions_context_fes_info_get(unit, field_stage, context_id, &context_fes_get_info));
    for (fes_quartet_ndx = 0; fes_quartet_ndx < context_fes_get_info.nof_fes_quartets; fes_quartet_ndx++)
    {
        if (context_fes_get_info.fes_quartets[fes_quartet_ndx].fg_id == fg_id[0] &&
    ...
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id[1], context_id));
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id[2], context_id));
    SHR_IF_ERR_EXIT(dnx_field_actions_fes_detach(unit, field_stage, fg_id_extra, context_id));
    /*
    ...
    if (nof_fes_array > 1)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_actions_fes_set
                        (unit, field_stage, fg_id[0], context_id, is_dir_ext[0], fes_inst_info));
        SHR_IF_ERR_EXIT(dnx_field_actions_context_fes_info_get(unit, field_stage, context_id, &context_fes_get_info));
        for (fes_quartet_ndx = 0; fes_quartet_ndx < context_fes_get_info.nof_fes_quartets; fes_quartet_ndx++)
        {
            if (context_fes_get_info.fes_quartets[fes_quartet_ndx].fg_id == fg_id[0] &&
