...
#include <include/appl/diag/dnx/field/diag_dnx_field.h>
#include <appl/diag/sand/diag_sand_framework.h>
#include <appl/diag/dnx/field/diag_dnx_field.h>

#include <include/appl/ctest/dnx/field/ctest_dnx_field_key.h>
...
/**
 * \brief
 * Input is the encoded handle, identifying an allocated half key.
 * This macro decodes the handle and displays its components.
 * The components are:
 * * stage
 * * field group type
 * * context ID
 * * containing key id (A-J)
 * * half key part indication (MSB or LSB)
 */
#define DNX_DIAG_KEY_DISPLAY_RECONSTRUCTED(_key_allocation_id) \
{  \
    dnx_field_group_type_e loc_fg_type;  \
    dnx_field_context_t loc_context_id;  \
    dnx_field_stage_e loc_field_stage;  \
    dbal_enum_value_field_field_key_e loc_key_id_0;  \
    dbal_enum_value_field_field_key_e loc_key_id_1;  \
    dnx_field_key_part_type_e loc_key_part_type;  \
    DECONSTRUCT_KEY_ALLOCATION_ID((_key_allocation_id), loc_fg_type, loc_field_stage,  \
                                  loc_context_id, loc_key_id_0, loc_key_id_1, loc_key_part_type);  \
    LOG_INFO_EX(BSL_LOG_MODULE, "Key reconstruction values:\r\n"  \
               "  field_stage       %d\r\n"  \
               "  fg_type           %d\r\n"  \
               "  context_id        %d\r\n"  \
               "  key_id_0          %d\r\n", loc_field_stage, loc_fg_type, loc_context_id, loc_key_id_0);  \
    LOG_INFO_EX(BSL_LOG_MODULE, "  key_id_1          %d\r\n"                                               \
                                "  key_part          %d %s%s\r\n", loc_key_id_1, (int) loc_key_part_type, EMPTY, EMPTY);  \
}
/**
 * \brief
...
{  \
    ...
    LOG_INFO_EX(BSL_LOG_MODULE, \
        "  key_part_type     %s (%d)\r\n" \
        "  key_allocation_id 0x%08lX %s\r\n", \
        dnx_field_key_part_type_e_get_name(key_id[alloc_test_step].key_part), \
        (int) key_id[alloc_test_step].key_part, \
        (unsigned long) key_allocation_id[alloc_test_step],EMPTY); \
}
/**
 * \brief
...
{  \
    LOG_INFO_EX(BSL_LOG_MODULE, _key_free_description \
        "  field_stage       %s (%d)\r\n"  \
        "  fg_type           %s (%d)\r\n", \
        dnx_field_stage_e_get_name(loc_field_stage), loc_field_stage, \
    ...
    LOG_INFO_EX(BSL_LOG_MODULE, "  key_id            %c (%d) \r\n"    \
        ...
        dnx_field_key_part_type_e_get_name(key_id[free_test_step].key_part), \
        (int) key_id[free_test_step].key_part); \
}
/*
 * }
 */
...
static sh_sand_enum_t Field_context_enum_table_for_test[] = {
    {"CONTEXT_LOWEST", DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS / 2},
    {"CONTEXT_HIGHEST", 0, NULL, "DNX_DATA.field.common_max_val.nof_contexts-1"},
    {NULL}
};
...
sh_sand_option_t Dnx_field_key_test_options[] = {
    ...
    {DNX_DIAG_KEY_OPTION_TEST_TYPE,        SAL_FIELD_TYPE_ENUM,    "Type of test (key/ffc/ffc-2/bit-range)",           NULL,                              (void *)Field_key_test_type_enum_table },
...
sh_sand_man_t Sh_dnx_field_key_test_man = {
    ...
    "ctest field key type=<key | ffc | bit-range> stage=<ipmf1 |ipmf2 |ipmf3 | epmf | ...> group_type=<TCAM | DIRECT_EXTRACTION | ...> context=<<low>-<high>> num_bits=<integer> clean=<YES | NO>",
    "type=ffc\r\n" "TY=KEY stage=ipmf3\r\n"
...
/*    diag_dnx_field_key_fill_quals(unit, DNX_DATA_MAX_FIELD_TCAM_SINGLE_KEY_SIZE, field_stage, &(field_database.qual_types);*/
     ...
     * Create Key template
     * */
    SHR_IF_ERR_EXIT(dnx_field_key_template_create(unit, field_group.field_stage, field_group.dnx_quals, DNX_FIELD_KEY_LOOKUP_STARTING_OFFSET, &key_template));
    ...
    for (context_idx = 0; context_idx < DNX_DIAG_KEY_CONTEXT_NOF; context_idx++)
    {
        SHR_IF_ERR_EXIT(dnx_field_key_attach(unit, context_id[context_idx], &key_in_info, &(key_id[context_idx])));
        /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_diag_display_key_occupation(unit, field_stage, context_id, sand_control));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (key_test_allocate)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_field_key_id_bit_range_allocate
                        (unit, field_stage, fg_type, context_lower, num_bits_in_bit_range, do_align,
                         (int16) bit_to_align, &key_id, &bit_range_offset_within_key));
        LOG_CLI_EX("Bit range allocation: Allocated %d bits at offset %d of key '%c' (half key %s)\r\n",
                   (int) num_bits_in_bit_range, (int) bit_range_offset_within_key, 'A' + key_id.id[0],
                   dnx_field_key_part_type_e_get_name(key_id.key_part));
    }
    else
    {
        /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 key_allocation_id[CTEST_DNX_FIELD_KEY_NUM_ALLOCATIONS_IN_KEY_TEST];
    ...
    for (context_index = context_lower; context_index <= context_upper; context_index++)
    {
        ...
        
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_field_key_id_allocate
                            (unit, 0, field_stage, fg_type[alloc_test_step], context_id[alloc_test_step],
                             ...
                             &key_allocation_id[alloc_test_step]));

            DNX_DIAG_KEY_DISPLAY_ALLOC_RESULTS();

            DNX_DIAG_KEY_DISPLAY_RECONSTRUCTED(key_allocation_id[alloc_test_step]);
        }
        {
            /*
        ...
        SHR_IF_ERR_EXIT(field_key_display_key_occupation_info
                        (unit, field_stage, context_id[alloc_test_step], sand_control));
        /*
        ...
        {
            {
                ...
                LOG_INFO_EX(BSL_LOG_MODULE, "Key test. Step 4 **************************************%s%s%s%s\r\n",
                            EMPTY, EMPTY, EMPTY, EMPTY);
                free_test_step = 0;
                DECONSTRUCT_KEY_ALLOCATION_ID(key_allocation_id[free_test_step], fg_type[free_test_step],
                                              loc_field_stage, context_id[free_test_step], key_id[free_test_step].id[0],
                                              key_id[free_test_step].id[1], key_id[free_test_step].key_part);

                DNX_DIAG_KEY_DISPLAY_FREE_PARAMS("Key free request - First-allocated half key:\r\n");

                SHR_IF_ERR_EXIT(dnx_algo_field_key_id_free(unit, key_allocation_id[free_test_step]));
            }
            {
                /*
        ...
        {
            {
                ...
                DNX_DIAG_KEY_DISPLAY_ALLOC_PARAMS("Key allocation request - Half key for direct extraction:\r\n");
            ...
            {
                {
                    ...
                    free_test_step = 1;
                    DECONSTRUCT_KEY_ALLOCATION_ID(key_allocation_id[free_test_step], fg_type[free_test_step],
                                                  loc_field_stage, context_id[free_test_step],
                                                  key_id[free_test_step].id[0], key_id[free_test_step].id[1],
                                                  key_id[free_test_step].key_part);
                ...
                }
                {
                    ...
                    DNX_DIAG_KEY_DISPLAY_FREE_PARAMS("Key free request - First allocated half key (DIR EXT):\r\n");
        ...
        if ((field_stage != DNX_FIELD_STAGE_IPMF1) && (field_stage != DNX_FIELD_STAGE_EPMF))
        {
                            ...
                            (unit, field_stage, context_id[alloc_test_step], sand_control));
            {
                ...
                }
                {
                    ...
                    DNX_DIAG_KEY_DISPLAY_FREE_PARAMS("Key free request - Fourth allocated half key (DIR EXT):\r\n");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (context_lower >= dnx_data_field.context.default_oam_context_get(unit))
    {
        context_lower = dnx_data_field.context.default_oam_context_get(unit) - 1;
    }
    if (context_upper >= dnx_data_field.context.default_oam_context_get(unit))
    {
        context_upper = dnx_data_field.context.default_oam_context_get(unit) - 1;
    }
    /*
    ...
    switch (key_test_type_name)
    {
        ...
        default:
        {
            LOG_INFO_EX(BSL_LOG_MODULE,
                        ...
                        __FUNCTION__, __LINE__, key_test_type_name, DNX_DIAG_KEY_OPTION_TEST_TYPE);
            break;
        }
    }
exit:
