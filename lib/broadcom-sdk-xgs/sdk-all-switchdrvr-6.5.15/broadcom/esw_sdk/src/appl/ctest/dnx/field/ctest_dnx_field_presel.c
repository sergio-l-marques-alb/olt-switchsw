...
/* *INDENT-OFF* */
     ...
     */
    bcm_field_presel_t presel_id[] =           { 5                        , 6                       , 5                      , 8                   , 9          };
    /*
     ...
     * Range: 0-63.
     */
    bcm_field_context_t context_id[] =       { 5                        , 6                       , 5                   , 4         , 7        };
    /*
...
/* *INDENT-ON* */
    ...
    for (presel_idx = 0; presel_idx < nof_presels; presel_idx++)
    {
        ...
        bcm_field_context_create(unit, BCM_FIELD_FLAG_WITH_ID, stage[presel_idx], &context_info,
                                 &context_id[presel_idx]);
    ...
    for (presel_idx = 0; presel_idx < nof_presels; presel_idx++)
    {
        ...
        if (entry_valid[presel_idx])
        {
            ...
            {
                if (entry_data.qual_data[int_qual_idx].qual_type != qual_type[qual_idx])
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "bcm_field_presel_get: Unexpected data returned.\n"
                                 "qualifier %d type expected %d received %d.\n",
                                 int_qual_idx, qual_type[qual_idx], entry_data.qual_data[int_qual_idx].qual_type);
                }
                ...
                if (entry_data.qual_data[int_qual_idx].qual_value != qual_value[qual_idx])
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "bcm_field_presel_get: Unexpected data returned.\n"
                                 "qualifier %d value expected %d received %d.\n",
                                 int_qual_idx, qual_value[qual_idx], entry_data.qual_data[int_qual_idx].qual_value);
                }
