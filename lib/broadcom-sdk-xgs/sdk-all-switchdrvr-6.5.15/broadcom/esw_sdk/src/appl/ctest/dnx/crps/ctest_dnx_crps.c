    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        SHR_IF_ERR_EXIT(dnx_crps_ctrs_db_counter_set
                        (unit, DNX_CRPS_CTRS_OVERWRITE_COUNTER_VALUE, core_idx, engine_id,
                         sw_counter_id / DNX_CRPS_MGMT_SUB_COUNT, (sw_counter_id & 0x1), value64));
    }
...
static sh_sand_man_t dnx_crps_set_man = {
    ...
    .synopsis = NULL,
    .examples = "engine=2 counter=222 value=22222 core=1\n" "engine=14 counter=444 value=444 core=0"
};

static sh_sand_invoke_t dnx_crps_set_tests[] = {
    {"crps_set", "engine=2 counter=102 value=502 core=1"},
    {"crps_clear", "engine=2 counter=102 value=0 core=1"},
    {NULL}
};
