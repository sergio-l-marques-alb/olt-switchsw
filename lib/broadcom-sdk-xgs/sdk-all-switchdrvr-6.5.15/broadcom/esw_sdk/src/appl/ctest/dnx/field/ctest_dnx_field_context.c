    ...
    dnx_field_context_mode_t * set_context_mode)
{
    ...
    if (field_stage == DNX_FIELD_STAGE_IPMF1 || field_stage == DNX_FIELD_STAGE_IPMF2)
    {
        ...
        LOG_INFO_EX(BSL_LOG_MODULE,
                    ...
                    __FUNCTION__, __LINE__, dnx_field_stage_e_get_name(field_stage), context_id);
    }
    else
    {
        LOG_INFO_EX(BSL_LOG_MODULE,
    ...
    bcm_field_context_info_t * set_context_info)
{
    ...
    if (field_stage == bcmFieldStageIngressPMF1 || field_stage == bcmFieldStageIngressPMF2)
    {
        ...
        LOG_INFO_EX(BSL_LOG_MODULE,
                    ...
                    __FUNCTION__, __LINE__, field_stage, context_id);
    }
    else
    {
        LOG_INFO_EX(BSL_LOG_MODULE,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (test_type == 0)
    {
        ...
        {
            bcm_context_info.cascaded_from = 4;
        ...
        if (rv == _SHR_E_NOT_FOUND)
        {
            LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %d .Context ID: %d does not exist  \r\n",
                        __FUNCTION__, __LINE__, bcm_field_stage, bcm_context_id);
        }
        else
        {
        ...
        /**
         *  ---------------------  CONTEXT LIMIT OVERFLOW TESTING SCENARIO ------------------------------
         */
        ...
        /** We want the device to allocate the context ID so we un-set the flags */
        bcm_context_flags = 0;
        /** We start the context iteration from 2 since we do not create the default context and the ITMH_PPH context.*/
        for (context_iter = 4; context_iter < CTEST_DNX_FIELD_CONTEXT_MAX_NUM_CONTEXTS + 1; context_iter++)
        {
            LOG_INFO_EX(BSL_LOG_MODULE, "Allocation/Deallocation try: %d \n %s%s%s", context_iter, EMPTY, EMPTY, EMPTY);

            /** Creating a context */
            rv = bcm_field_context_create(unit, bcm_context_flags, bcm_field_stage, &bcm_context_info, &bcm_context_id);
            if (rv != _SHR_E_NONE)
            {
                if (context_iter == CTEST_DNX_FIELD_CONTEXT_MAX_NUM_CONTEXTS && rv == _SHR_E_RESOURCE)
                {
                    LOG_INFO_EX(BSL_LOG_MODULE,
                                "SUCCESS ON A NEGATIVE TEST: %s(), line %d, Stage: %d .Context cannot be allocated, out of resources! %s \r\n",
                                __FUNCTION__, __LINE__, bcm_field_stage, EMPTY);
                }
                else
                {
                    SHR_SET_CURRENT_ERR(rv);
    ...
    else if (test_type == 1)
    {
        ...
        {
            dnx_context_mode.context_ipmf2_mode.cascaded_from = 4;
        ...
        if (rv == _SHR_E_NOT_FOUND)
        {
            LOG_INFO_EX(BSL_LOG_MODULE, "%s(), line %d, Stage: %s .Context ID: %d does not exist  \r\n",
                        __FUNCTION__, __LINE__, dnx_field_stage_text(unit, dnx_field_stage), dnx_context_id);
        }
        else
        {
            SHR_SET_CURRENT_ERR(rv);
        ...
        */
        ...
        if (dnx_field_stage == DNX_FIELD_STAGE_IPMF2)
        {
             ...
             */
            dnx_context_mode.context_ipmf2_mode.cascaded_from = 5;
            dnx_context_id = 5;
            dnx_context_flags = DNX_FIELD_CONTEXT_FLAG_WITH_ID;
            LOG_INFO_EX(BSL_LOG_MODULE,
             ...
             */
            dnx_context_mode.context_ipmf2_mode.cascaded_from = 25;
            dnx_context_id = 5;
            ...
            if (rv == _SHR_E_PARAM)
            {
                LOG_INFO_EX(BSL_LOG_MODULE,
                            ...
                            "IPMF1 context(%d)\r\n",
                            __FUNCTION__, __LINE__, dnx_field_stage_text(unit, dnx_field_stage),
                            dnx_context_mode.context_ipmf2_mode.cascaded_from);
            }
            else
            {
        ...
        dnx_context_mode.context_ipmf2_mode.cascaded_from = DNX_FIELD_CONTEXT_ID_DEFAULT(unit);
        /** We start the context iteration from 2 since we do not create the default context and the ITMH_PPH context.*/
        for (context_iter = 1; context_iter < CTEST_DNX_FIELD_CONTEXT_MAX_NUM_CONTEXTS - 4; context_iter++)
        {
            LOG_INFO_EX(BSL_LOG_MODULE, "Allocation/Deallocation try: %d \n %s%s%s", context_iter, EMPTY, EMPTY, EMPTY);

            if (context_iter == dnx_data_field.context.default_oam_context_get(unit))
            {
                context_iter += dnx_data_field.context.default_itmh_context_get(unit) + 1;
            }
            /** Creating a context */
            rv = dnx_field_context_create(unit, dnx_context_flags, dnx_field_stage, &dnx_context_mode, &dnx_context_id);
            if (rv != _SHR_E_NONE)
            {
                ...
                {
                    LOG_INFO_EX(BSL_LOG_MODULE,
                                "SUCCESS ON A NEGATIVE TEST: %s(), line %d, Stage: %s .Context cannot be allocated, out of resources! %s \r\n",
                                __FUNCTION__, __LINE__, dnx_field_stage_text(unit, dnx_field_stage), EMPTY);
                }
                else
                {
                    SHR_SET_CURRENT_ERR(rv);
