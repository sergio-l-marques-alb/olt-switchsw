    ...
    bcm_oam_group_t * group_id)
{
    ...
    bcm_oam_group_info_t_init(&group_info);
    sal_memcpy(group_info.name, &group_name, sizeof(group_name));
    SHR_CLI_EXIT_IF_ERR(bcm_oam_group_create(unit, &group_info),
                        "Test failed. Could not create group.\n");
    ...
    bcm_oam_endpoint_t * mep_id)
{
    ...
    *mep_id = endpoint_info->id;
    ...
    rmep_info->level = 2;
    SHR_CLI_EXIT_IF_ERR(bcm_oam_endpoint_create(unit, rmep_info),
                        "Test failed. Could not create remote endpoint.\n");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_CLI_EXIT_IF_ERR(bcm_rx_trap_set(unit, trap_code_cpu, &trap_config),
                        "Test failed. Could not configure trap.\n");
    ...
    /* Register events */
    SHR_CLI_EXIT_IF_ERR(diag_dnx_oam_register_events(unit),
                        "Test failed. Could not register events.\n");
    ...
    SHR_CLI_EXIT_IF_ERR(bcm_vlan_port_create(unit, &vlan_port),
                        "Test failed. Could not create AC LIF.\n");
    ...
    /* Create OAM group*/
    SHR_CLI_EXIT_IF_ERR(diag_dnx_oam_group_create(unit, &group_id),
                        "Test failed. Could not create group.\n");

    /* Create local accelerated endpoint */
    SHR_CLI_EXIT_IF_ERR(diag_dnx_oam_local_endpoint_create_with_rmep(unit, group_id, gport, ing_acc_profile_id, &mep_id),
    ...
    for (index = 0; index < recreate_limit; ++index)
    {
        ...
        SHR_CLI_EXIT_IF_ERR(bcm_oam_group_destroy(unit, group_id),
                            "Test failed. Could not destroy group.\n");
    ...
    for (index = 0; index < replace_limit; ++index)
    {
        ...
        SHR_CLI_EXIT_IF_ERR(diag_dnx_oam_endpoint_update(unit, mep_id),
                            "Test failed. Could not update endpoint.\n");
...
static sh_sand_man_t diag_dnx_oam_reconfiguration_sem_test_man = {
    ...
    /** Full */
    "Repeatedly create MEP and RMEP."
        "Configure snoop and cpu trap." "Replace MEP.",
    /** Synopsis   */
...
sh_sand_cmd_t dnx_oam_test_cmds[] = {
    ...
    {"reconfiguration", diag_dnx_oam_reconfiguration_semantic_test_cmd, NULL, diag_dnx_oam_reconfiguration_sem_test_options,
