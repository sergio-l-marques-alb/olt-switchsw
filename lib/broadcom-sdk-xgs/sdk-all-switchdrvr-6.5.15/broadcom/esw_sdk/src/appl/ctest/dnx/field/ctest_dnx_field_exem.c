...
static dnx_field_utils_packet_info_t Packet_info = {
    {"PTCH_2", "PTCH_2.PP_SSP", "200", "ETH1", "IPv4", "IPv4.SIP", "0.0.0.1", "IPv4.DIP", "0.0.0.2", ""}
};
    ...
    uint32 expected_value)
{
    ...
    do
    {
        ...
        return_value = NULL;
        rv = sand_signal_verify(unit, Packet_sig_info[field_stage].core,
                                ...
                                &(expected_value_local), Packet_sig_info[field_stage].size, NULL, return_value);
        num_retries++;
    }
    ...
    int exem_test_clean)
{
    ...
    for (action_ndx = 0; action_ndx < CTEST_DNX_FIELD_EXEM_NUM_ACTIONS; action_ndx++)
    {
        ...
        fg_info_in.dnx_actions[0][action_ndx] = Dnx_actions_array[stage_ndx][action_ndx];
    }
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_exem_add(unit, fg_id, &entry_info_in));
    ...
    for (action_ndx = 0; action_ndx < CTEST_DNX_FIELD_EXEM_NUM_ACTIONS; action_ndx++)
    {
        ...
        attach_info_in.dnx_actions[0][action_ndx] = Dnx_actions_array[stage_ndx][action_ndx];
        attach_info_in.action_info[0][action_ndx].priority = CTEST_DNX_FIELD_EXEM_ACTION_PRIORITY;
    }
    ...
    for (action_ndx = 0; action_ndx < CTEST_DNX_FIELD_EXEM_NUM_ACTIONS; action_ndx++)
    {
        ...
        if (fg_info_in.dnx_actions[0][action_ndx] != fg_info_out.group_basic_info.dnx_actions[0][action_ndx]
            || fg_info_in.dnx_actions[0][action_ndx] !=
            fg_info_out.group_full_info.actions_payload_info.actions_on_payload_info[0][action_ndx].dnx_action)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
    ...
    for (action_ndx = 0; action_ndx < CTEST_DNX_FIELD_EXEM_NUM_ACTIONS; action_ndx++)
    {
        ...
        if (attach_info_in.dnx_actions[0][action_ndx] != attach_info_out.attach_basic_info.dnx_actions[0][action_ndx]
            || attach_info_in.action_info[0][action_ndx].priority !=
            attach_info_out.attach_basic_info.action_info[0][action_ndx].priority)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Action %d returned from get differently from what was to be written.\r\n", action_ndx);
        }
    ...
    if (exem_test_traffic)
    {
        ...
        if (field_stage == DNX_FIELD_STAGE_EPMF)
        {
            ...
            attach_info_fec.dnx_actions[0][0] =
                DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, DNX_FIELD_STAGE_IPMF1, DBAL_ENUM_FVAL_IPMF1_ACTION_DST_DATA);
            attach_info_fec.action_info[0][0].priority = CTEST_DNX_FIELD_EXEM_ACTION_PRIORITY;
            ...
            SHR_IF_ERR_EXIT(dnx_field_entry_tcam_add(unit, fg_id_fec, &entry_info_fec, &entry_handle_fec));
        }
        /*
