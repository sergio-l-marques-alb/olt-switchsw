...
/**    in_lif_template_data_t in_lif_profile_template_data;*/
    int /**ref_count, in_lif_profile_counter,*/ use_pwe_lif_scoped, pwe_nof_eth_tags, new_in_lif_profile, array_counter,
        profiles_counter, intf_namespace, use_global_route, use_tni;
    ...
    /*
     * create all possible GRE8 in_lif_profiles
     */
    printf("creating all possible GRE8 cs_inlif_profiles\n");
    for (use_tni = 0; use_tni <= DNX_IN_LIF_PROFILE_CS_PROFILE_GRE8_USE_TNI;
         use_tni += DNX_IN_LIF_PROFILE_CS_PROFILE_GRE8_USE_TNI)
    {
             ...
             use_global_route += BCM_L3_INGRESS_GLOBAL_ROUTE)
        {
            ...
            inlif_dbal_table_id[profiles_counter] = DBAL_TABLE_IN_LIF_IPvX_TUNNELS;
            profiles_counter++;
        }
    }
     ...
     */
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_profile.clear(unit, _SHR_CORE_ALL));
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    in_lif_profile_info_t in_lif_profile_info_array[TOTAL_NOF_PROFILES];
     ...
     */
    SHR_IF_ERR_EXIT(algo_lif_profile_db.eth_rif_profile.clear(unit, _SHR_CORE_ALL));
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_profile.clear(unit, _SHR_CORE_ALL));
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_profile_l3_enablers.clear(unit, _SHR_CORE_ALL));
    SHR_IF_ERR_EXIT(algo_lif_profile_db.in_lif_da_not_found_destination.clear(unit, _SHR_CORE_ALL));
        ...
        for (pwe_nof_eth_tags = 0; pwe_nof_eth_tags < NOF_POSSIBLE_PWE_NOF_ETH_TAGS; pwe_nof_eth_tags++)
        {
            for (use_pwe_lif_scoped = 0; use_pwe_lif_scoped < NOF_POSSIBLE_PWE_MODES; use_pwe_lif_scoped++)
            {
                for (change_rpf_mode_flag = 0; change_rpf_mode_flag < 2; change_rpf_mode_flag++)
                {
                    ...
                    {
                        in_lif_profile_info_array[profiles_counter].ingress_fields.urpf_mode = bcmL3IngressUrpfLoose;
                    }
                    in_lif_profile_info_array[profiles_counter].ingress_fields.pwe_nof_eth_tags = pwe_nof_eth_tags;
                    profiles_counter++;
                }
    ...
    if (ref_count != dnx_data_lif.in_lif.nof_sbc_local_in_lifs_get(unit))
    {
    ...
    rc = dnx_in_lif_profile_exchange(unit, &in_lif_profile_info_array[array_counter], DNX_IN_LIF_PROFILE_DEFAULT,
                                     &new_in_lif_profile, ETH_RIF, inlif_dbal_table_id);
    if (rc == _SHR_E_NONE)
    {
    ...
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange
                    (unit, &in_lif_profile_info_array[array_counter], DNX_IN_LIF_PROFILE_DEFAULT, &new_in_lif_profile,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    out_lif_profile_info_t out_lif_profile_info_array[NOF_OUT_LIF_PROFILES];

    SHR_FUNC_INIT_VARS(unit);

    out_lif_profile_to_dealloc = 1;

    SHR_IF_ERR_EXIT(algo_lif_profile_db.out_lif_profile.clear(unit, _SHR_CORE_ALL));
         ...
         out_lif_orientation_val <= OUT_LIF_ORIENTATION_MAX_VALUE; out_lif_orientation_val++)
    {
        ...
        out_lif_profile_info_t_init(unit, &out_lif_profile_info_array[profiles_counter]);
    ...
    SHR_IF_ERR_EXIT(dnx_out_lif_profile_get_ref_count(unit, DNX_OUT_LIF_PROFILE_DEFAULT, &ref_count));
    if (ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "the default out_lif_profile has a wrong reference count: %d\n", ref_count);
    }
    ...
    {
        SHR_IF_ERR_EXIT(dnx_out_lif_profile_get_ref_count(unit, profiles_counter, &ref_count));
        if (profiles_counter == DNX_OUT_LIF_PROFILE_DEFAULT
            && ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) - 4)
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "wrong ref_count for out_lif_profile = %d,ref_count = %d,should be %d\n",
                         profiles_counter, ref_count, dnx_data_lif.out_lif.nof_local_out_lifs_get(unit) - 4);
        }
    ...
    for (profiles_counter = DNX_OUT_LIF_PROFILE_DEFAULT; profiles_counter < NOF_OUT_LIF_PROFILES; profiles_counter++)
    {
        SHR_IF_ERR_EXIT(dnx_out_lif_profile_get_ref_count(unit, profiles_counter, &ref_count));
        if (profiles_counter == DNX_OUT_LIF_PROFILE_DEFAULT
            && ref_count != dnx_data_lif.out_lif.nof_local_out_lifs_get(unit))
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "wrong ref_count for out_lif_profile = %d,ref_count = %d,should be %d\n",
                         profiles_counter, ref_count, dnx_data_lif.out_lif.nof_local_out_lifs_get(unit));
        }
