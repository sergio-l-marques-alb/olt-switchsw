    ...
    int unit)
{
    ...
    sw_state_algo_template_create_data_t data, tmp_data;
     ...
     */
    sal_strncpy(data.name, TEMPLATE_TEST_WITH_DEFAULT_CHECK_VALUES, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    ...
    sal_strncpy(tmp_data.name, TEMPLATE_TEST_WITHOUT_DEFAULT_CHECK_VALUES,
     ...
     */
    tmp_data.flags = SW_STATE_ALGO_TEMPLATE_CREATE_DUPLICATE_PER_CORE;
    sal_strncpy(tmp_data.name, TEMPLATE_TEST_PER_CORE_CHECK_VALUES, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);

    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_per_core_check_values.create(unit, &tmp_data, NULL));
    tmp_data.flags = SW_STATE_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE;
    /*
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default_check_values.allocate_single
                    (unit, _SHR_CORE_ALL, FLAGS_NONE, &profile_data, NULL, &profile, &first_reference));
    /*
     ...
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.allocate_single(unit, _SHR_CORE_ALL, FLAGS_NONE,
    ...
    /*
     * dnx_algo_template_allocate with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_test_without_default_check_values.allocate_single
        (unit, FIRST_CORE_ID - 1, FLAGS_NONE, &profile_data, NULL, &profile, &first_reference);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_allocate was successful\n");
    }
    /*
     * dnx_algo_template_allocate with core_id = nof_cores, should fail
     */
    rv = algo_temp_mngr_db.template_test_without_default_check_values.allocate_single
        (unit, dnx_data_device.general.nof_cores_get(unit), FLAGS_NONE, &profile_data, NULL, &profile,
         &first_reference);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores but dnx_algo_template_allocate was successful\n");
    }
    /*
     * dnx_algo_template_allocate with non existent name, should fail
     */
    rv = algo_temp_mngr_db.template_test_bad_values.allocate_single(unit, _SHR_CORE_ALL, FLAGS_NONE, &profile_data,
                                                                    NULL, &profile, &first_reference);
    if (rv == _SHR_E_NONE)
    {
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_with_default_check_values.exchange
                    (unit, _SHR_CORE_ALL, FLAGS_NONE, &profile_data, old_profile, NULL, &new_profile, &first_reference,
                     &is_last));
    ...
    rv = algo_temp_mngr_db.template_test_bad_values.exchange
        (unit, FIRST_CORE_ID, FLAGS_NONE, &profile_data, old_profile, NULL, &new_profile, &first_reference, &is_last);
    ...
    rv = algo_temp_mngr_db.template_with_default_check_values.exchange
        (unit, _SHR_CORE_ALL, FLAGS_NONE, &profile_data, NEGATIVE_PROFILE, NULL, &new_profile, &first_reference,
         &is_last);
    if (rv == _SHR_E_NONE)
    {
    ...
    /*
     * dnx_algo_template_exchange with old_profile = nof_profiles, should fail
     */
    
    rv = algo_temp_mngr_db.template_with_default_check_values.exchange
        (unit, _SHR_CORE_ALL, FLAGS_NONE, &profile_data, dnx_data_device.general.nof_cores_get(unit),
         NULL, &new_profile, &first_reference, &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "old_profile = nof_profiles but dnx_algo_template_exchange was successful\n");
    }

    /*
     * dnx_algo_template_exchange with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.exchange
        (unit, FIRST_CORE_ID - 1, FLAGS_NONE, &profile_data, old_profile, NULL, &new_profile, &first_reference,
         &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_exchange was successful\n");
    }
    /*
     * dnx_algo_template_exchange with core_id = nof_cores, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.exchange
        (unit, dnx_data_device.general.nof_cores_get(unit), FLAGS_NONE, &profile_data, old_profile,
         NULL, &new_profile, &first_reference, &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores but dnx_algo_template_exchange was successful\n");
    }
    /*
     * }
     */
     ...
     */
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_with_default_check_values.replace_data(unit, _SHR_CORE_ALL, new_profile,
                                                                                      &replace_profile_data));

    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_with_default_check_values.profile_data_get(unit, _SHR_CORE_ALL, new_profile, &ref_count,
                                                                        &compare_data));
     ...
     */
    rv = algo_temp_mngr_db.template_test_bad_values.replace_data(unit, _SHR_CORE_ALL, new_profile,
                                                                 &replace_profile_data);
    if (rv == _SHR_E_NONE)
    {
     ...
     */
    rv = algo_temp_mngr_db.template_test_bad_values.replace_data(unit, _SHR_CORE_ALL, NEGATIVE_PROFILE,
                                                                 &replace_profile_data);
    if (rv == _SHR_E_NONE)
    {
    ...
    rv = algo_temp_mngr_db.template_test_bad_values.replace_data
        (unit, _SHR_CORE_ALL, dnx_data_device.general.nof_cores_get(unit), &replace_profile_data);
    if (rv == _SHR_E_NONE)
    {
    ...
    /*
     * dnx_algo_template_replace_data with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_test_bad_values.replace_data
        (unit, FIRST_CORE_ID - 1, new_profile, &replace_profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_replace_data was successful\n");
    }
    /*
     * dnx_algo_template_replace_data with core_id = nof_cores, should fail
     */
    rv = algo_temp_mngr_db.template_test_bad_values.replace_data
        (unit, dnx_data_device.general.nof_cores_get(unit), new_profile, &replace_profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores but dnx_algo_template_replace_data was successful\n");
    }
     ...
     */
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_with_default_check_values.clear(unit, _SHR_CORE_ALL));
    /*
     * dnx_algo_template_clear with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.clear(unit, FIRST_CORE_ID - 1);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_clear was successful\n");
    }
    /*
     * dnx_algo_template_clear with core_id = nof_cores, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.clear(unit, dnx_data_device.general.nof_cores_get(unit));
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores but dnx_algo_template_clear was successful\n");
    }
    /*
     * dnx_algo_template_clear with non existent name, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.clear(unit, dnx_data_device.general.nof_cores_get(unit));
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "non existent name but dnx_algo_template_clear was successful\n");
    }
    /*
     * }
     */
     ...
     */
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_test_without_default_check_values.profile_get(unit, _SHR_CORE_ALL, &profile_data,
                                                                           &profile));
    /*
    ...
    /*
     * dnx_algo_template_profile_get with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.profile_get(unit, FIRST_CORE_ID - 1, &profile_data,
                                                                          &profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_profile_get was successful\n");
    }
    /*
     * dnx_algo_template_profile_get with core_id = nof_cores, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.profile_get(unit,
                                                                          dnx_data_device.general.nof_cores_get(unit),
                                                                          &profile_data, &profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores_id but dnx_algo_template_profile_get was successful\n");
    }
    /*
     * dnx_algo_template_profile_get with non existent name, should fail
     */
    rv = algo_temp_mngr_db.template_test_bad_values.profile_get(unit, _SHR_CORE_ALL, &profile_data, &profile);
    if (rv == _SHR_E_NONE)
    {
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_test_without_default_check_values.free_single(unit, _SHR_CORE_ALL, FIRST_PROFILE,
                                                                           &is_last));
    /*
    ...
    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit, _SHR_CORE_ALL, FIRST_PROFILE, &is_last);
    ...
    /*
     * dnx_algo_template_free with negative core_id, should fail
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit, FIRST_CORE_ID - 1, FIRST_PROFILE,
                                                                          &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_free was successful\n");
    }
    /*
     * dnx_algo_template_free with core_id = nof_cores, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit,
                                                                          dnx_data_device.general.nof_cores_get(unit),
                                                                          FIRST_PROFILE, &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "core_id = nof_cores but dnx_algo_template_free was successful\n");
    }
    /*
     * dnx_algo_template_free with non existent name, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit, _SHR_CORE_ALL, FIRST_PROFILE, &is_last);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "non existent name but dnx_algo_template_free was successful\n");
    }
    /*
     * dnx_algo_template_free with negative profile, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit, _SHR_CORE_ALL, FIRST_PROFILE - 1,
                                                                          &is_last);
     ...
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.free_single(unit, _SHR_CORE_ALL, data.nof_profiles,
                                                                          &is_last);
    if (rv == _SHR_E_NONE)
    {
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_with_default_check_values.profile_data_get(unit, _SHR_CORE_ALL, FIRST_PROFILE, &ref_count,
                                                                        &profile_data));
    /*
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_test_per_core_check_values.profile_data_get(unit, FIRST_CORE_ID, FIRST_PROFILE, &ref_count,
                                                                         &profile_data));
    /*
     * dnx_algo_template_profile_profile_data_get with negative core_id, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.profile_data_get(unit, FIRST_CORE_ID - 1, FIRST_PROFILE,
                                                                               &ref_count, &profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "negative core_id but dnx_algo_template_profile_profile_data_get was successful\n");
    }
    /*
     * dnx_algo_template_profile_profile_data_get with core_id = nof_cores, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.profile_data_get(unit,
                                                                               dnx_data_device.
                                                                               general.nof_cores_get(unit),
                                                                               FIRST_PROFILE, &ref_count,
                                                                               &profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "core_id = nof_cores_id but dnx_algo_template_profile_profile_data_get was successful\n");
    }
    /*
     * dnx_algo_template_profile_profile_data_get with non existent name, should fail
     */

    rv = algo_temp_mngr_db.template_test_bad_values.profile_data_get(unit, _SHR_CORE_ALL, FIRST_PROFILE, &ref_count,
                                                                     &profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "non existent name but dnx_algo_template_profile_profile_data_get was successful\n");
    }
    /*
     * dnx_algo_template_profile_profile_data_get with negative profile_id, should fail
     */

    rv = algo_temp_mngr_db.template_with_default_check_values.profile_data_get(unit, _SHR_CORE_ALL, FIRST_PROFILE - 1,
                                                                               &ref_count, &profile_data);
     ...
     */
    rv = algo_temp_mngr_db.template_with_default_check_values.profile_data_get(unit, _SHR_CORE_ALL, data.nof_profiles,
                                                                               &ref_count, &profile_data);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
    ...
    rv = algo_temp_mngr_db.template_test_bad_values.get_next(unit, _SHR_CORE_ALL, &current_profile);
    if (rv == _SHR_E_NONE)
    {
    ...
    rv = algo_temp_mngr_db.template_test_without_default_check_values.get_next(unit, _SHR_CORE_ALL, &current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "dnx_algo_template_get_next should fail because current_profile=SW_STATE_ALGO_TEMPLATE_ILLEGAL_PROFILE\n");
    }

    current_profile = FIRST_PROFILE;

    rv = algo_temp_mngr_db.template_test_without_default_check_values.get_next(unit, FIRST_CORE_ID - 1,
                                                                               &current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "dnx_algo_template_get_next should fail because core_id=FIRST_CORE_ID - 1\n");
    }

    rv = algo_temp_mngr_db.template_test_without_default_check_values.get_next(unit,
                                                                               dnx_data_device.
                                                                               general.nof_cores_get(unit),
                                                                               &current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "dnx_algo_template_get_next should fail because core_id=dnx_data_device.general.nof_cores_get(unit)\n");
    }
    ...
    rv = algo_temp_mngr_db.template_with_default_check_values.free_all(unit, _SHR_CORE_ALL, current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "dnx_algo_template_get_next should fail because name=TEMPLATE_TEST_WITH_DEFAULT\n");
    }

    rv = algo_temp_mngr_db.template_test_without_default_check_values.free_all(unit, FIRST_CORE_ID - 1,
                                                                               current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "dnx_algo_template_get_next should fail because name=TEMPLATE_TEST_WITHOUT_DEFAULT\n");
    }

    current_profile = SW_STATE_ALGO_TEMPLATE_ILLEGAL_PROFILE;

    rv = algo_temp_mngr_db.template_test_without_default_check_values.free_all(unit, _SHR_CORE_ALL, current_profile);
    if (rv == _SHR_E_NONE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL,
                     "dnx_algo_template_get_next should fail because current_profile=SW_STATE_ALGO_TEMPLATE_ILLEGAL_PROFILE\n");
...
{
    shr_error_e rv;
    sw_state_algo_template_create_data_t data_default, data;
     ...
     */
    sal_memset(&data_default, 0, sizeof(sw_state_algo_template_create_data_t));
    data_default.flags = SW_STATE_ALGO_TEMPLATE_CREATE_USE_DEFAULT_PROFILE;
     ...
     */
    sal_strncpy(data.name, TEMPLATE_TEST_WITHOUT_DEFAULT, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.create(unit, &data, NULL));

    sal_strncpy(data_default.name, TEMPLATE_TEST_WITH_DEFAULT, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_with_default.create(unit, &data_default, NULL));
    data.flags = data.flags | SW_STATE_ALGO_TEMPLATE_CREATE_DUPLICATE_PER_CORE;

    sal_strncpy(data.name, TEMPLATE_TEST_PER_CORE, SW_STATE_ALGO_TEMPLATE_MNGR_MAX_NAME_LENGTH - 1);
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_per_core.create(unit, &data, NULL));

    /*
     * test profile_get check that default profile was created and it has the right number of references
     */

    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_with_default.profile_get(unit, _SHR_CORE_ALL, &profile_data, &profile));
    if (profile != DEFAULT_PROFILE)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "default profile doesn't exist in index 0\n");
    }

    rv = algo_temp_mngr_db.template_test_without_default.profile_get(unit, _SHR_CORE_ALL, &profile_data, &profile);
    if (rv == _SHR_E_NONE)
    {
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                    template_with_default.profile_data_get(unit, _SHR_CORE_ALL, DEFAULT_PROFILE, &ref_count,
                                                           &profile_data));
    ...
    for (profile_counter = FIRST_PROFILE; profile_counter < data_default.max_references; profile_counter++)
    {
        ...
        SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.allocate_single(unit, _SHR_CORE_ALL, FLAGS_NONE,
    ...
    rv = algo_temp_mngr_db.template_test_without_default.allocate_single
        (unit, _SHR_CORE_ALL, FLAGS_NONE, &allocate_profile_data, NULL, &profile, &first_reference);
    ...
    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.clear(unit, _SHR_CORE_ALL));

    current_profile = FIRST_PROFILE;

    SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.get_next(unit, _SHR_CORE_ALL, &current_profile));
    ...
    while (TRUE)
    {
        ...
        SHR_IF_ERR_EXIT(algo_temp_mngr_db.
                        template_test_without_default.get_next(unit, _SHR_CORE_ALL, &current_profile));
        if (current_profile == SW_STATE_ALGO_TEMPLATE_ILLEGAL_PROFILE)
        {
            break;
        }

        SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.free_all(unit, _SHR_CORE_ALL, current_profile));
        current_profile++;
    }
    ...
    for (profile_counter = FIRST_PROFILE + 1; profile_counter < data_default.nof_profiles; profile_counter += 2)
    {
        ...
        SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.profile_data_get
                        (unit, _SHR_CORE_ALL, profile_counter - 1, &ref_count, &new_profile_value));

        SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.exchange
                        (unit, _SHR_CORE_ALL, FLAGS_NONE, &new_profile_value, profile_counter, NULL, &new_profile,
    ...
    for (profile_counter = FIRST_PROFILE; profile_counter < data_default.nof_profiles; profile_counter++)
    {
        ...
        SHR_IF_ERR_EXIT(algo_temp_mngr_db.template_test_without_default.profile_data_get
                        (unit, _SHR_CORE_ALL, profile_counter, &ref_count, &profile_data));
        /*
         * odd
         */
    ...
    /*
     * check that templates are separated per core when using flag SW_STATE_ALGO_TEMPLATE_CREATE_DUPLICATE_PER_CORE
     */
