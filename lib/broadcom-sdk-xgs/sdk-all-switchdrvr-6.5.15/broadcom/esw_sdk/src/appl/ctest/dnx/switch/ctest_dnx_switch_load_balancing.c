...
#include <appl/ctest/dnx/switch/ctest_dnx_switch_load_balancing.h>
...
sh_sand_invoke_t Sh_dnx_switch_load_balancing_bcm_tests[] = {
    {"lb_set_HashSeed", "type=LB_SET mode=HashSeed index=2 value=0x1234",     CTEST_PRECOMMIT}
    ,
    {"lb_get_HashSeed", "type=LB_GET mode=HashSeed index=2",                  CTEST_PRECOMMIT}
    ,
...
sh_sand_option_t Sh_dnx_switch_load_balancing_bcm_options[] = {
    ...
    {DNX_CTEST_LOAD_BALANCING_OPTION_BCM_INDEX,       SAL_FIELD_TYPE_UINT32,  "'index' of operation",                                       "0",            (void *)NULL},
...
sh_sand_man_t Sh_dnx_switch_load_balancing_bcm_man = {
    ...
    .examples = "type=LB_SET mode=HashSeed index=2 value=0x1234 \r\n" "type=LB_GET mode=HashSeed index=2"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    num_selects = sizeof(set_tcam_result.client_x_crc_16_sel) / sizeof(set_tcam_result.client_x_crc_16_sel[0]);
...
appl_dnx_lb_test_crc_seed(
    ...
    sh_sand_control_t * sand_control)
{
    dnx_switch_lb_crc_seed_t set_crc_seed, get_crc_seed;
    /**
     * This variables stores the severity of the load balancing on dnx
     */
    bsl_severity_t original_severity_lb;
     ...
     */
    SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_LB, original_severity_lb);
    set_crc_seed.lb_crc_function_index = 6;
    set_crc_seed.lb_crc_seed = 0x5AA5;
    SHR_IF_ERR_EXIT(dnx_switch_load_balancing_crc_seed_set(unit, &set_crc_seed));
    /*
     * Now retrieve the same entry.
     */
    get_crc_seed.lb_crc_function_index = set_crc_seed.lb_crc_function_index;
    SHR_IF_ERR_EXIT(dnx_switch_load_balancing_crc_seed_get(unit, &get_crc_seed));
    /*
     * Compare 'get' and 'set'.
     */
    if (sal_memcmp(&set_crc_seed, &get_crc_seed, sizeof(set_crc_seed)) != 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
    ...
    }
    {
         ...
         */
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LS_BCMDNX_LB, bslSeverityFatal);
        set_crc_seed.lb_crc_function_index = 26;
        rv = dnx_switch_load_balancing_crc_seed_set(unit, &set_crc_seed);
        if (rv != _SHR_E_NONE)
        {
            LOG_INFO_EX(BSL_LOG_MODULE,
