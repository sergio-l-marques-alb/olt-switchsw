        ...
        for (ii = 0; ii < num_db; ii++)
        {
            ...
            for (jj = 0; jj < num_entries / num_iter; jj++)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entry_location_alloc(unit, core, handler_ids[ii], &entry));
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_add(unit, 0, handler_ids[ii], &dbal_entry));
                    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_get(unit, 0, handler_ids[ii], &dbal_entry));
            ...
            if (!co_exist)
            {
                ...
                for (jj = 0; jj < num_entries / num_iter; jj++)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_delete(unit, 0, handler_ids[ii], &dbal_entry));
