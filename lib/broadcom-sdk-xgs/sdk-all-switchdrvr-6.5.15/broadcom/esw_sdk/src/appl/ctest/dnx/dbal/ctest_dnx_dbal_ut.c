...
extern shr_error_e dbal_tables_table_restore(
    int unit,
    ...
    uint8 is_rand)
{
    ...
    for (field_index = 0; field_index < table->nof_key_fields; field_index++)
    {
        ...
        if (table->keys_info[field_index].is_allocator && (!dbal_table_is_allcator_not_needed(unit, table_id)))
        {
    ...
    {
        /** in this case the table is not applicable to perform partial access test */
        SHR_EXIT();
    }
    /*
     * Commit table values
    ...
    dbal_tables_e table_id)
{
    ...
    /** Known result type valdations - getting the entry when the result type is known */
    ...
    for (cur_res_type = 0; cur_res_type < table->nof_result_types; cur_res_type++)
    {
        for (ii = 1; ii < table->multi_res_info[cur_res_type].nof_result_fields; ii++)
        {
            rv = dbal_entry_handle_value_field_arr32_get
                (unit, entry_handle_id, table->multi_res_info[cur_res_type].results_info[ii].field_id,
                 INST_SINGLE, ret_field_val);
    ...
    dbal_tables_e table_id)
{
    ...
    for (ii = 1; ii < table->multi_res_info[0].nof_result_fields; ii++)
    {
        uint32 field_value;
        uint32 field_value_arr[2] = { 0 };
        field_value = payload_start_offset + ii;
        if (table->multi_res_info[0].results_info[ii].max_value != 0)
        {
            field_value = UTILEX_MIN(table->multi_res_info[0].results_info[ii].max_value, payload_start_offset + ii);
        }
        field_value_arr[0] = field_value;
        dbal_entry_value_field_arr32_set(unit, entry_handle_id, table->multi_res_info[0].results_info[ii].field_id,
                                         INST_ALL, field_value_arr);
    }
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field
                    (buffer_to_read, table->multi_res_info[0].zero_padding, table->multi_res_info[0].entry_payload_size,
                     payload_buffer));

    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, table_id, &buffers_entry_handle_id));
    {
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_payload_update(unit, buffers_entry_handle_id, key_buffer, DBAL_CORE_ALL,
                                                             table->multi_res_info[0].entry_payload_size,
                                                             payload_buffer));
    }
    SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
    for (ii = 0; ii < table->nof_key_fields; ii++)
    ...
    dbal_tables_e table_id)
{
     ...
     */
    dbal_entry_key_field64_set(unit, entry_handle_wr, DBAL_FIELD_ROUTING_ENABLERS_PROFILE, key_val);
    ...
    dbal_tables_e table_id)
{
    ...
    dbal_entry_key_field_predefine_value_set(unit, entry_handle_id, key_field_1, DBAL_PREDEF_VAL_RESET_VALUE);
    dbal_entry_value_field_predefine_value_set(unit, entry_handle_id, res_field, INST_SINGLE,
    ...
    if (field_value_received != expected_val)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Wrong reset value, received = %d expected = %d\n", res_field, expected_val);
    }
    ...
    dbal_tables_e table_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_struct_field_decode_masked
                    (unit, field_id, field_in_struct_id, &field_in_struct_decoded_val,
                     &field_in_struct_ffs_decoded_mask, full_field_val, full_field_mask));
    /*
     * Compare encoded and decoded values
     */
    if ((field_in_struct_ffs_encoded_val != field_in_struct_decoded_val))
    ...
    dbal_tables_e in_table_id)
{
    ...
    if (!rv)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, " negative test passed, should return error - destroying  field\n");
    }
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM,
    ...
    if (!rv)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, " negative test passed, should return error - destroying unexisted table\n");
    }
    ...
    rv = dbal_tables_table_create(unit, DBAL_ACCESS_METHOD_MDB, DBAL_TABLE_TYPE_TCAM,
    ...
    for (iter = (first_table_id - DBAL_NOF_TABLES); iter < dnx_data_dbal.table.nof_dynamic_tables_get(unit); iter++)
    {
        SHR_IF_ERR_EXIT(dbal_tables_table_destroy(unit, table_id - iter));
    }
    ...
    dbal_tables_e table_id)
{
    ...
    int MAX_NOF_RES_TYPES_TO_TEST = 5;
    ...
    dbal_tables_e table_id)
{
    ...
    dbal_entry_key_field32_set(unit, entry_handle_id2, DBAL_FIELD_PP_PORT, 201);
    dbal_entry_key_field32_set(unit, entry_handle_id2, DBAL_FIELD_CORE_ID, 0);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id2, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id2, DBAL_FIELD_DEFAULT_LIF, INST_SINGLE, &default_lif));
    ...
    for(ii = 0; ii < nof_entries_to_add; ii+=2)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id2, DBAL_FIELD_RESULT_TYPE, INST_SINGLE, DBAL_RESULT_TYPE_IN_AC_INFO_DB_IN_LIF_AC_MP);
        dbal_entry_value_field32_set(unit, entry_handle_id2, DBAL_FIELD_FODO_ASSIGNMENT_MODE, INST_SINGLE, DBAL_ENUM_FVAL_FORWARD_DOMAIN_ASSIGNMENT_MODE_FORWARD_DOMAIN_FROM_LIF);
        ...
        sh_process_command(unit, "tx 1 PSRC=201 DATA=0x0011223344AA000000000001810000010800c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c");
    }
    ...
    while (!is_end)
    {
        entries_count++;
        SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get
                        (unit, entry_handle_id, DBAL_FIELD_VSI, &return_key_val));
    ...
    dbal_tables_e table_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(rv);
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Entry rewritten witout update flag.\n");
    }
    ...
    /*
     * Write entries of even and odd numbers and see that the iterator followes them.
     */
    ...
    for (ii = 0; ii < (sizeof(extra_entries) / sizeof(extra_entries[0])); ii++)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "interator didn't find entry %d.\n", extra_entries[ii]);
        }
    }
    ...
    dbal_tables_e table_id)
{
    ...
    /** Verify table id */
    if ((table_id != DBAL_TABLE_BIER_BFER_TERMINATION) &&
        (table_id != DBAL_TABLE_BRIDGE_IPV4_MULTICAST_SOURCE_SPECIFIC_IVL) &&
        (table_id != DBAL_TABLE_BFD_ENDPOINT_SW_INFO))
    {
...
#define TABLE_RESTORE_MAX_NUM_RESULTS 4
    ...
    for (ii = 0; ii < table->multi_res_info[0].nof_result_fields; ii++)
    {
        if ((results_info[ii].field_id != table->multi_res_info[0].results_info[ii].field_id) ||
            ...
            (results_info[ii].is_valid_indication_needed !=
             table->multi_res_info[0].results_info[ii].is_valid_indication_needed))
        {
...
#define MAX_NUM_RESULT_FIELDS 10
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit
                    (unit, partial_set_entry_handle_id, DBAL_COMMIT | DBAL_COMMIT_OVERRIDE_DEFAULT));
    ...
    dbal_tables_e table_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id,
                                      dbal_tables_is_non_direct(unit, table_id) ? DBAL_COMMIT_UPDATE :
                                      DBAL_COMMIT));
    ...
    dbal_tables_e table_id)
{
    ...
    multi_res_info_t *multi_res_info_saved = NULL;
    ...
    if (table->multi_res_info[result_type_index].zero_padding != (table->max_payload_size - total_fields_size))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected results. zero padding = %d, expected %d\n",
                     table->multi_res_info[result_type_index].zero_padding,
                     table->max_payload_size - total_fields_size);
    }
    total_fields_size = 0;
    for (ii = 0; ii < nof_result_fields; ii++)
    {
        ...
        if (table->multi_res_info[result_type_index].results_info[ii].bits_offset_in_buffer !=
            table->max_payload_size - total_fields_size)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unexpected results. bits_offset_in_buffer[%d] %d\n", ii,
                         table->multi_res_info[result_type_index].results_info[ii].bits_offset_in_buffer);
        }
    }
     ...
     * Before we perform restore, we save the current multiple result types.
     */
    SHR_ALLOC_SET_ZERO(multi_res_info_saved, (table->nof_result_types * sizeof(table->multi_res_info[0])),
                       "multiple results info allocation", "%s%s%s\r\n", table->table_name, EMPTY, EMPTY);
    sal_memcpy(multi_res_info_saved, table->multi_res_info,
               (table->nof_result_types * sizeof(table->multi_res_info[0])));
    ...
    for (result_type_index = first_dynamic_result_type; result_type_index < nof_result_type; result_type_index++)
    {
        ...
        if (table->multi_res_info[result_type_index].result_type_nof_hw_values !=
            ...
            table->multi_res_info[result_type_index].zero_padding !=
            table->multi_res_info[result_type_restored_index].zero_padding ||
            ...
            table->multi_res_info[result_type_index].refernce_field_id !=
            table->multi_res_info[result_type_restored_index].refernce_field_id ||
     ...
     */
    SHR_FREE(((dbal_logical_table_t *) table)->multi_res_info);
    ((dbal_logical_table_t *) table)->multi_res_info = multi_res_info_saved;
...
#define NOF_ITERATIONS 4
    ...
    if (table_id == DBAL_TABLE_EXAMPLE_FOR_HL_MULTIPLE_RESULT) {
        ...
        for (ii = 0; ii < NOF_ITERATIONS; ii++)
        {
            ...
            for (jj = 0; jj <= ii; jj++)
            {
                SHR_IF_ERR_EXIT(dbal_iterator_key_field_arr32_rule_add
                                (unit, entry_handle_id, rule_field_id[jj], rule_key_condition[jj], &rule_field_val[jj],
                                 NULL));
            }
            while (!is_end)
            {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ASSOCIATED_ENDPOINTS_LIST_HEAD, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        /** Entry 2 */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ASSOCIATED_ENDPOINTS_LIST_HEAD, INST_SINGLE, 2);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
        /** Entry 3 */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, table_id, entry_handle_id));
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_ASSOCIATED_ENDPOINTS_LIST_HEAD, INST_SINGLE, 3);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    dbal_tables_e table_id)
{
    ...
    {
        if (table->keys_info[field_id].is_allocator)
        {
    ...
    for (iter = 0; iter < expect_cnt; iter++)
    {
        ...
        for (ii = 0; ii < info.create_data.nof_elements; ii++)
        {
        ...
        switch (field_type)
        {
            case DBAL_FIELD_TYPE_DEF_QOS_PROFILE:
                SHR_IF_ERR_EXIT(algo_qos_db.
                                ingress_qos.allocate_single(unit, SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL, &ii));
                alloc_resource[iter] = ii;
                break;
            case DBAL_FIELD_TYPE_DEF_ECN_QOS_PROFILE:
                SHR_IF_ERR_EXIT(algo_qos_db.ingress_qos_ecn.allocate_single(unit,
                                                                            SW_STATE_ALGO_RES_ALLOCATE_WITH_ID, NULL,
                                                                            &ii));
                alloc_resource[iter] = ii;
                break;
            default:
    ...
    dbal_tables_e table_id)
{
    ...
    /*
     * Verify table ID, This test is a special test that can run only 
     * for table with result type per device
     */
    ...
    for (cur_result_type = 0; cur_result_type < table->nof_result_types; cur_result_type++)
    {
        ...
        for (field_index = 0; field_index < table->multi_res_info[cur_result_type].nof_result_fields; field_index++)
        {
             ...
             */
            if (table->multi_res_info[cur_result_type].results_info[field_index].permission == DBAL_PERMISSION_READONLY)
            {
                continue;
            }
            /*
