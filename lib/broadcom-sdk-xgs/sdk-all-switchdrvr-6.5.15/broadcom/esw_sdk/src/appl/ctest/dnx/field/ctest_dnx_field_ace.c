...
static dnx_field_utils_packet_info_t Packet_info = {
    {"PTCH_2", "PTCH_2.PP_SSP", "200", "ETH1", "IPv4", "IPv4.SIP", "0.0.0.2", "IPv4.DIP", "0.0.0.1", ""}
};
    ...
    int ace_test_clean)
{
    ...
    dnx_field_ace_entry_add_flags_e ace_entry_flags;
    ...
    attach_info.dnx_actions[0][0] =
        DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, DNX_FIELD_STAGE_EPMF,
                   DBAL_ENUM_FVAL_EPMF_ACTION_CUD_OUTLIF_OR_MCDB_PTR);
    attach_info.action_info[0][0].priority = BCM_FIELD_ACTION_PRIORITY(0, 3);
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_add(unit, fg_id_egress, &tcam_entry_info, &tcam_entry_handle_egress));
    ...
    if (ace_test_traffic)
    {
        ...
        SHR_IF_ERR_EXIT(sand_signal_verify(unit, Packet_sig_info.core, Packet_sig_info.block, Packet_sig_info.from,
                                           ...
                                           Packet_sig_info.size, NULL, NULL));
    }
    ...
    if (ace_test_update)
    {
        ...
        ace_entry_flags = DNX_FIELD_ACE_ENTRY_ADD_FLAG_UPDATE;
        SHR_IF_ERR_EXIT(dnx_field_ace_entry_add(unit, ace_entry_flags, ace_id, &payload_info_in, &ace_entry_key));
