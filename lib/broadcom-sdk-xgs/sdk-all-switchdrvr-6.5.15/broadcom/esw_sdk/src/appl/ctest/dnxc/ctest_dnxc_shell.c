    ...
    rhhandle_t * ctest_soc_property_set_h)
{
    ...
    for (; ctest_soc_property != NULL && !ISEMPTY(ctest_soc_property->property); ctest_soc_property++)
    {
        ...
        cur_value = sal_config_get(ctest_soc_property->property);
        /** We are not interested if current and to be set values are equal */
        if ((ISEMPTY(cur_value) && ISEMPTY(ctest_soc_property->value)) ||
            ((cur_value != NULL) && !sal_strcasecmp(cur_value, ctest_soc_property->value)))
            continue;

        if (ctest_soc_property_list == NULL)
        {       /** Generate list only if there is anything to modify */
            if ((ctest_soc_property_list = utilex_rhlist_create("SocPropertySet",
                                                                sizeof(ctest_soc_entry_t), 1)) == NULL)
            {
                SHR_CLI_EXIT(_SHR_E_MEMORY, "ERROR - No memory for soc property entries list\n");
            }
        }

        if (utilex_rhlist_entry_add_tail(ctest_soc_property_list, ctest_soc_property->property,
                                         RHID_TO_BE_GENERATED, &void_arg) != _SHR_E_NONE)
        {
            SHR_CLI_EXIT(_SHR_E_MEMORY, "ERROR - Cannot add soc property:%s to list", ctest_soc_property->property);
        }
        ...
        if (sal_config_set(RHNAME(property_entry), ctest_soc_property->value) != 0)
        {
            SHR_CLI_EXIT(_SHR_E_CONFIG, "Failed to set property:%s to %s\n",
                         RHNAME(property_entry), ctest_soc_property->value);
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (i_key = 0; sh_sand_keywords[i_key].keyword != NULL; i_key++)
    {
        ...
        if (full_flag == TRUE)
        {
            if (sal_strlen(sh_sand_keywords[i_key].short_key) == 1)
            {
    ...
    rhlist_t * test_list)
{
    ...
    SH_SAND_CMD_ITERATOR(sh_sand_cmd, sh_sand_device_a)
    {
