...
static dnx_ctest_dir_ext_semantic_actions_t Ctest_dnx_dir_ext_semantic_actions[] =
{
    ...
    {"",                   8,          24,           bcmFieldActionVoid,       {  BCM_FIELD_ACTION_POSITION(0, 5)  }},
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    {
        fg_info_p->dnx_actions[0][assignment_index] = actions_p[assignment_index];
        fg_info_p->use_valid_bit[0][assignment_index] = FALSE;
        ...
        attach_info_p->dnx_actions[0][assignment_index] = fg_info_p->dnx_actions[0][assignment_index];
        attach_info_p->action_info[0][assignment_index].priority =
    ...
    if (test_flag == CTEST_DNX_DIR_EXT_POSITIVE_TEST)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_group_context_attach
                        (unit, DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE, *fg_id_p, DNX_FIELD_CONTEXT_ID_DEFAULT(unit),
                         attach_info_p));
    }
    /**
    ...
    dnx_field_group_info_t * set_fg_info_p)
{
     ...
     */
    for (fg_iterator = 0; set_fg_info_p->dnx_actions[0][fg_iterator] != DNX_FIELD_ACTION_INVALID; fg_iterator++)
    {
        if (set_fg_info_p->dnx_actions[0][fg_iterator] != get_fg_info.group_basic_info.dnx_actions[0][fg_iterator])
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Set_fg action (%d) is not the same as Get_fg action (%d)\r\n",
                         set_fg_info_p->dnx_actions[0][fg_iterator],
                         get_fg_info.group_basic_info.dnx_actions[0][fg_iterator]);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    dnx_field_group_t * fg_id_p)
{
         ...
         negative_case_iterator < CTEST_DNX_DIR_EXT_NOF_TESTS; negative_case_iterator++)
    {
        ...
        switch (negative_case_iterator)
        {
            ...
            {
                sal_memcpy(negative_test_quals, quals_array, sizeof(negative_test_quals));
                negative_test_quals[CTEST_DNX_DIR_EXT_SEMANTIC_NOF_QUALS_AND_ACTIONS] = quals_array[0];
                sal_memcpy(negative_test_actions, actions_array, sizeof(negative_test_actions));
                 ...
                 */
                nof_iterations = CTEST_DNX_DIR_EXT_SEMANTIC_NOF_QUALS_AND_ACTIONS + 1;
                break;
            }
            /**
