...
{
                           /** IPMF1 Data */
    { bcmFieldActionDropPrecedence, bcmFieldQualifyVlanId, {bcmFieldInputTypeLayerAbsolute, CTEST_DNX_FIELD_CONTEXT_ATTACH_QUAL_INPUT_ARG_AND_OFFSET, CTEST_DNX_FIELD_CONTEXT_ATTACH_QUAL_INPUT_ARG_AND_OFFSET} },
                           /** IPMF2 Data */
    { bcmFieldActionAdmitProfile, bcmFieldQualifyHashValue, {bcmFieldInputTypeMetaData, BCM_FIELD_INVALID, BCM_FIELD_INVALID} },
                           /** IPMF3 Data */
    { bcmFieldActionSnoop, bcmFieldQualifyVlanAction, {bcmFieldInputTypeMetaData, BCM_FIELD_INVALID, BCM_FIELD_INVALID} },
                           /** EPMF Data */
    { bcmFieldActionMirrorEgress, bcmFieldQualifyMplsLabel, {bcmFieldInputTypeLayerAbsolute, CTEST_DNX_FIELD_CONTEXT_ATTACH_QUAL_INPUT_ARG_AND_OFFSET, CTEST_DNX_FIELD_CONTEXT_ATTACH_QUAL_INPUT_ARG_AND_OFFSET} }
};
    ...
    dnx_field_group_t * fg_id_p)
{
    ...
    fg_info_p->dnx_actions[0][0] = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, field_stage, qual_action_info->dnx_action);
    ...
    dnx_field_group_attach_info_t * attach_info_p)
{
    ...
    attach_info_p->action_info[0][0].priority = CTEST_DNX_FIELD_CONTEXT_ATTACH_ACTION_PRIORITY;
    ...
    dnx_field_group_attach_info_t * set_attach_info_p)
{
    ...
    if (set_attach_info_p->action_info[0][0].priority != get_attach_info.attach_basic_info.action_info[0][0].priority)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Set_attach_info action priority (%d) is not the same as Get_attach_info action priority(%d)\r\n",
                     set_attach_info_p->action_info[0][0].priority,
                     get_attach_info.attach_basic_info.action_info[0][0].priority);
    }

    /** Comparing the single DNX Action */
    if (set_attach_info_p->dnx_actions[0][0] != get_attach_info.attach_basic_info.dnx_actions[0][0])
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Set_attach_info dnx_action (%s) is not the same as Get_attach_info dnx_action (%s)\r\n",
                     dnx_field_dnx_action_text(unit, set_attach_info_p->dnx_actions[0][0]),
                     dnx_field_dnx_action_text(unit, get_attach_info.attach_basic_info.dnx_actions[0][0]));
    }
 ...
 */
    ...
    if (set_attach_info_p->compare_mode != get_attach_info.attach_basic_info.compare_mode)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Set_attach_info compare_mode (0x%08X) is not the same as Get_attach_info compare_mode (0x%08X)\r\n",
                     set_attach_info_p->compare_mode, get_attach_info.attach_basic_info.compare_mode);
    }
#endif
/*
 * }
    ...
    ctest_dnx_field_context_attach_info_per_stage_t * qual_action_info)
{
   ...
   /** Giving random mid-range priority, the value can be changed, it will not influence the test */
    dnx_attach_info.action_info[0][0].priority = CTEST_DNX_FIELD_CONTEXT_ATTACH_ACTION_PRIORITY;
    ...
    int unit)
{
    ...
    dnx_field_group_attach_info_t dnx_attach_info;
    dnx_field_group_info_t dnx_fg_info;
    dnx_field_group_context_flags_e attach_flags = 0;
    ...
    dnx_fg_info.dnx_actions[0][0] = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, stage, DBAL_ENUM_FVAL_EPMF_ACTION_TC);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_context_id = dnx_context_id = 7;
    bcm_context_flags = 0;
    dnx_context_flags = 0;
    ...
    if (test_type == 0)
    {
        ...
        bcm_context_flags |= BCM_FIELD_FLAG_WITH_ID;
       ...
       /** Convert DNX to BCM Field Stage */
        ...
        if (dnx_field_stage == DNX_FIELD_STAGE_IPMF2)
        {
            ...
            bcm_context_info.cascaded_from = DNX_FIELD_CONTEXT_ID_DEFAULT(unit);
        }
