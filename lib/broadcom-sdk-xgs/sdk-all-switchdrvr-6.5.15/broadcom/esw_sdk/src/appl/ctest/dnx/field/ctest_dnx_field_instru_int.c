    ...
    dnx_field_group_info_t * fg_info_p)
{
     ...
     */
    fg_info_p->dnx_actions[0][0] = CTEST_INSTRU_INT_IPMF3_ACTION_INT_COMMAND;
    ...
    dnx_field_context_t context)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_context_attach(unit, DNX_FIELD_GROUP_CONTEXT_FLAGS_NONE, fg_id, context, &a_info));
    ...
    uint32 *entry_handle_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_entry_tcam_add(unit, fg_id, &entry_in_info, entry_handle_p));
    ...
    dnx_field_group_info_t * fg_info_p)
{
     ...
     */
    fg_info_p->dnx_actions[0][0] = CTEST_INSTRU_INT_IPMF1_ACTION_TAIL_EDIT_PROFILE;
