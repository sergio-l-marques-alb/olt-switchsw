    ...
    uint32 *entry_id)
{
    ...
    int nof_counter_base_bits, max_counter_base, nof_mep_pe_profile_bits, max_mep_pe_profile;
    ...
    if (ccm_entry_data->mep_type == DBAL_ENUM_FVAL_OAMP_MEP_TYPE_ETH_OAM)
    {
        if (_SHR_IS_FLAG_SET(ccm_entry_data->flags, DNX_OAMP_OAM_CCM_MEP_IS_UPMEP))
        {
            ...
            ccm_entry_data->unified_port_access.ccm_eth_up_mep_port.port_core = UTILEX_GET_BIT(temp, CORE_BIT);
            test->gen_value += GEN_VALUE_DELTA;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_oam_oamp_scan_count_calc
                    (unit, endpoint_id, is_short_entry, ccm_rate, ccm_rx_without_tx, opcode, &mep_profile_data));
    ...
    exp_op0_cnt = 0;
    exp_op1_cnt = 0;
    rv = dnx_oam_oamp_scan_count_calc(unit, endpoint_id, is_short_entry, ccm_rate, ccm_rx_without_tx, opcode, &mep_profile_data);
