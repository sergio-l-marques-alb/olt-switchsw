...
static sh_sand_option_t Field_efes_list_options[] = {
    {DIAG_DNX_FIELD_OPTION_STAGE,  SAL_FIELD_TYPE_UINT32, "Field stage to present EFES info for.",   NULL,    (void *)Field_stage_enum_table},
    {NULL}
};
...
static sh_sand_option_t Field_efes_ctx_options[] = {
    {DIAG_DNX_FIELD_OPTION_STAGE,  SAL_FIELD_TYPE_UINT32, "Field stage to present EFES CTX info for.",   NULL,           (void *)Field_stage_enum_table},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!(DNX_FIELD_IS_FP_STAGE(field_stage)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "\r\n" "Given Stage (%s) is not a FP stage! FP stages can be between (%s) and (%s). %s\r\n\n",
        ...
        for (fes_pgm_iter = 0; fes_pgm_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_PROGRAMS; fes_pgm_iter++)
        {
                 ...
                 fes_mask_iter++)
            {
                    ...
                    && fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    sal_strncat(efes_masks_string, fes_mask_buff, sizeof(efes_masks_string) - 1);
                }
            }
            ...
            for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
            {
                ...
                if (utilex_bitstream_test_bit(fes_id_info->fes_pgm_info[fes_pgm_iter].context_bmp, ctx_iter))
                {
                    ...
                    sal_strncat(ctx_string, ctx_id_buff, sizeof(ctx_string) - 1);
                }
            }
            /**
             * Extract last 2 symbols of the constructed string
        ...
        for (fes_mask_iter = 0; fes_mask_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_MASKS_PER_FES; fes_mask_iter++)
        {
            ...
            for (fes_pgm_iter = 0; fes_pgm_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_FES_PROGRAMS; fes_pgm_iter++)
            {
                ...
                if (fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    sal_strncat(efes_pgm_string, fes_pgm_buff, sizeof(efes_pgm_string) - 1);
                }
            }
                ...
                if (fes_id_info->fes_pgm_info[fes_pgm_iter].masks_id_used[fes_mask_iter])
                {
                    ...
                    for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
                    {
                        ...
                        if (utilex_bitstream_test_bit(fes_id_info->fes_pgm_info[fes_pgm_iter].context_bmp, ctx_iter))
                        {
                            ...
                            sal_strncat(efes_ctx_string, fes_cs_buff, sizeof(efes_ctx_string) - 1);
                        }
                    }
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (fes_array < 0 || fes_array >= nof_fes_array)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "\r\n"
                     "Given array_id (%d) is invalid for FP stage (%s)! Array id can be between (%d) and (%d). \r\n\n",
                     fes_array, dnx_field_stage_text(unit, field_stage), 0, (nof_fes_array - 1));
    }
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Source");
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_get(unit, fes_id_info->fes_pgm_info[fes_pgm_id].field_group, fg_info));
    ...
    for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
    {
        ...
        if (utilex_bitstream_test_bit(fes_id_info->fes_pgm_info[fes_pgm_id].context_bmp, ctx_iter))
        {
                 ...
                 fes2msb_id_ndx++)
            {
                ...
                uint32 fg_action_index;
                uint32 non_void_act_index = 0;
                int action_is_void;
                /** Iterate over field group actions. */
                for (fg_action_index = 0; fg_action_index < DNX_DATA_MAX_FIELD_GROUP_NOF_ACTION_PER_FG &&
                     fg_info->group_basic_info.dnx_actions[0][fg_action_index] != DNX_FIELD_ACTION_INVALID;
                     fg_action_index++)
                {
                    /** Check if current FG action is a void action. */
                    SHR_IF_ERR_EXIT(dnx_field_map_dnx_action_is_void
                        (unit, field_stage, fg_info->group_basic_info.dnx_actions[0][fg_action_index], &action_is_void));
                    /** In case the action is not void action, then increase the non_void_act_index. */
                    if (!action_is_void)
                    {
                        /**
                         * In case the non_void_act_index becomes equal to fes_id_info->place_in_fg,
                         * it means that we are at the right place in the FG action array.
                         */
                        if (non_void_act_index == fes_id_info->place_in_fg)
                        {
                            break;
                        }
                        non_void_act_index++;
                ...
                SHR_IF_ERR_EXIT(dnx_field_map_dnx_action_size(unit, fg_info->group_basic_info.field_stage,
                                                              fg_info->group_full_info.
                                                              actions_payload_info.actions_on_payload_info[0]
                                                              [fg_action_index].dnx_action, &action_size));
                ...
                PRT_CELL_SET("%s",
                             dnx_field_dnx_action_text(unit,
                                                       fg_info->group_full_info.actions_payload_info.
                                                       actions_on_payload_info[0]
                                                       [fg_action_index].dnx_action));
                PRT_CELL_SET("%d", fes_id_info->key_select);

                /** Take the offset for the current action. */
                SHR_IF_ERR_EXIT(dnx_field_group_action_offset_get
                                (unit, fes_id_info->fes_pgm_info[fes_pgm_id].field_group,
                                 fg_info->group_full_info.actions_payload_info.
                                 actions_on_payload_info[0][fg_action_index].dnx_action, &action_offset));

                PRT_CELL_SET("%d", action_offset);

                if (fes_id_info->fes_pgm_info[fes_pgm_id].pgm_ctx_2msb_info[ctx_iter].
                    fes2msb_info[fes2msb_id_ndx].type == 0)
                {
                    PRT_CELL_SET("%d", (action_size + 1));
                    PRT_CELL_SET("Yes");
                }
                else
                {
                ...
                PRT_CELL_SET("%d",
                             fes_id_info->fes_pgm_info[fes_pgm_id].
                             pgm_ctx_2msb_info[ctx_iter].fes2msb_info[fes2msb_id_ndx].polarity);
                PRT_CELL_SET("%d",
                             fes_id_info->fes_pgm_info[fes_pgm_id].
                             pgm_ctx_2msb_info[ctx_iter].fes2msb_info[fes2msb_id_ndx].chosen_mask);
                PRT_CELL_SET("0x%08X",
                             fes_id_info->fes_pgm_info[fes_pgm_id].
                             pgm_ctx_2msb_info[ctx_iter].fes2msb_info[fes2msb_id_ndx].mask);
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ctx_iter = 0; ctx_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CONTEXTS; ctx_iter++)
    {
        ...
        for (fes_id_iter = 0; fes_id_iter < context_fes_get_info.nof_fes_quartets; fes_id_iter++)
        {
            ...
            /** Get info for current dnx action (size, field_id) and relevant bcm action type to it. */
