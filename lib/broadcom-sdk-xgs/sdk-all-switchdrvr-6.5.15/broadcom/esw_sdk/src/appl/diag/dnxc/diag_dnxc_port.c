...
#define DIAG_DNXC_MAX_PORT_CORES                      3
...
static uint64 phy_fecstat_val[SOC_MAX_NUM_DEVICES][DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS][SOC_MAX_NUM_PORTS];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(bcm_port_config_get(unit, port_config));
    SOC_PBMP_ASSIGN(pbm, port_config->nif);
    SOC_PBMP_OR(pbm, port_config->sfi);
    ...
    *serdes_rate = 0;
    switch (lane_speed)
    {
        ...
        case 50000:
            ...
            {
                if ((speed_config.fec == PORTMOD_PORT_PHY_FEC_NONE) || (speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_FEC))
                {
                    *serdes_rate = 51562;
                }
                else if ((speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_544) || (speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_544_2XN)
                         || (speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_272) || (speed_config.fec == PORTMOD_PORT_PHY_FEC_RS_272_2XN))
                {
                    *serdes_rate = 53125;
                }
                else
    ...
    sal_usecs_t start_time)
{
    ...
    int status, pow = 0;
    uint64 base = COMPILER_64_INIT(0, 0), total_bits, temp_status;
    ...
    switch (status)
    {
        ...
        default:
            ...
            for (pow = 0; pow <= DIAG_DNXC_PORT_BER_MAX_POW; pow++)
            {
                ...
                COMPILER_64_UMUL_32(temp_status, status);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /*
     * On Cmodel, CMICx operation will return TIMEOUT
     */
    if (!SAL_BOOT_PLISIM)
    {
        if (is_write)
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    ARG_NEXT(args);
    if (SHR_FAILURE(_phy_diag_berproj(unit, pbmp, args)))
    {
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "Error in _phy_diag_berproj! \n");
    }
exit:if ((_func_rv = dnxc_phy_diag_linkscan_recover(unit, interval)) < 0)
    {
        cli_out("Failed to recover linkscan, orginial linkscan interval is %d \n", interval);
    }
    SHR_FUNC_EXIT;
    ...
    uint32 *rate)
{
    ...
    {
#ifdef BCM_DNX_SUPPORT
        SHR_IF_ERR_EXIT(soc_jer2_mapping_stat_get(unit, port, counters, &array_size, sub_counters, &sub_array_size, counter_type, 1));
#endif
    }
    else
    {
#ifdef BCM_DNXF_SUPPORT
        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_mapping_stat_get,(unit, port, (int *)counters, &array_size, counter_type, 1)));
#endif
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_stat_val_t counter_type[DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS] = {snmpBcmRxFecCorrectable, snmpBcmRxFecUncorrectable};
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        for (i = 0; i < DIAG_DNXC_PHY_FECSTAT_NOF_COUNTERS; ++i)
        {
            ...
            else
            {
                ...
                COMPILER_64_SUB_64(counter_diff, phy_fecstat_val[unit][i][port]);
                ...
                COMPILER_64_COPY(phy_fecstat_val[unit][i][port], counter_val);
            }
        }
    }
    PRT_COMMITX;
    ...
    sh_sand_control_t * sand_control)
{
    bcm_pbmp_t logical_ports;
    bcm_port_t port;
    int pow, ber_int, ber_remainder;
    uint32 serdes_rate, rate;
    uint64 base = COMPILER_64_INIT(0, 0), temp_rate, total_serdes_rate;
    uint64 ber_int64, ber_remainder64;
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        else
        {
            ...
            for (pow = 0; pow <= DIAG_DNXC_PORT_BER_MAX_POW; pow++)
            {
                /** ber_int = rate * base / (serdes_rate * SOC_INFO(unit).port_num_lanes[port]) / 1000000; */
                if (pow == 0) {
                    COMPILER_64_SET(base, 0, 1);
                } else {
                    COMPILER_64_UMUL_32(base, 10);
                }
                COMPILER_64_COPY(temp_rate, base);
                COMPILER_64_UMUL_32(temp_rate, rate);

                COMPILER_64_COPY(ber_int64, temp_rate);
                COMPILER_64_UDIV_64(ber_int64, total_serdes_rate);
            ...
            /** ber_remainder = (rate * base * 100 / (serdes_rate * SOC_INFO(unit).port_num_lanes[port]) / 1000000) % 100; */
            COMPILER_64_UMUL_32(temp_rate, 100);
            COMPILER_64_COPY(ber_remainder64, temp_rate);
            COMPILER_64_UDIV_64(ber_remainder64, total_serdes_rate);
            COMPILER_64_TO_32_LO(ber_remainder, ber_remainder64);
            ber_remainder = ber_remainder % 100;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, port)
    {
        ...
        if (SOC_IS_DNX(unit))
        {
            ...
            if (!(DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_type) || DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_type)))
            {
                /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_pbmp_t phy_pbmp;
#ifdef BCM_DNX_SUPPORT
    int nof_nif_pms, nof_ilkn_units, nof_fabric_pms;
#endif
    ...
    for (pm = pm_id_start; pm <= pm_id_end; pm++)
    {
        ...
        {
            sal_memset(buf, 0, sizeof(buf));
            ports_str = format_bcm_pbmp(unit, buf, sizeof(buf), diag_info.ports);
        }
...
static sh_sand_cmd_t sh_dnxc_phy_prbs_cmds[] = {
    ...
    {"set",     cmd_dnxc_phy_prbs_set,         NULL,       dnxc_phy_prbs_set_options,      &dnxc_phy_prbs_set_man},
    {"get",     cmd_dnxc_phy_prbs_get,         NULL,       dnxc_phy_prbs_get_options,      &dnxc_phy_prbs_get_man},
    {"clear",   cmd_dnxc_phy_prbs_clear,       NULL,       dnxc_phy_prbs_clear_options,    &dnxc_phy_prbs_clear_man},
    {NULL}
};
...
static sh_sand_man_t dnxc_phy_raw_sbus_man = {
    .brief    = "Diagnostic for PHY raw sbus command",
    .full     = "Set or Get phy registers through sbus MDIO. "
                "If option \"data\" is not present, it a read register operation. "
                "This command is only available for DNX NIF PHY. \n"
                "\"lane\" and \"pll_index\" are used for accessing registers according to "
                "lane and PLL info.",
    .synopsis = "pm_id=<val> devad=<val> regad=<val> [lane=<val>] [pll_index=<val>] [data=<val>]",
    .examples = "pm_id=0 devad=1 regad=0x96 lane=0x1 pindx=0x1 \n"
                "pm_id=0 devad=1 regad=0x96 lane=0x1 pindx=0x1 data=0x1"
};
#endif
...
static sh_sand_man_t dnxc_phy_dsc_man = {
    .brief    = "Diagnostic for PHY dsc info",
    .full     = "Dump PHY DSC Info, only <pbmp> is mandatory. \n"
    ...
    .synopsis = "<pbmp> [<state|config|NULL string>]",
    .examples = "sfi state \n"
                "1 config \n"
                "13"
};
static sh_sand_option_t dnxc_phy_dsc_options[] = {
    /*name           type                  desc          default    ext*/
    {"pbmp",    SAL_FIELD_TYPE_PORT, "port # / logical port type / port name", NULL,   NULL, NULL, SH_SAND_ARG_FREE},
    {"state", SAL_FIELD_TYPE_BOOL,  "Detailed DSC dump, including lane info, core info and lane event info", "false", NULL},
...
static sh_sand_man_t dnxc_phy_fecstat_man = {
    ...
    .full     = "Diagnostic for PHY FECSTAT command",
};

static sh_sand_man_t dnxc_phy_fecstat_counter_man = {
    .brief    = "Diagnostic to show PHY fecstat counter",
...
static sh_sand_cmd_t sh_dnxc_phy_fecstat_cmds[] = {
    /*keyword,   action,                            command,    options,                            man*/
    {"counters", cmd_dnxc_phy_fecstat_counter,    NULL,       dnxc_phy_fecstat_options,     &dnxc_phy_fecstat_counter_man},
    {"ber",      cmd_dnxc_phy_fecstat_ber,        NULL,       dnxc_phy_fecstat_options,     &dnxc_phy_fecstat_ber_man},
    {NULL}
};
...
sh_sand_cmd_t sh_dnxc_phy_cmds[] = {
    ...
    {"measure",   cmd_dnxc_phy_measure,   NULL,                   dnxc_phy_measure_options,  &dnxc_phy_measure_man},
    {"cl72",      cmd_dnxc_phy_cl72,      NULL,                   dnxc_phy_cl72_options,     &dnxc_phy_cl72_man},
    {"raw",       NULL,                   sh_dnxc_phy_raw_cmds,   NULL,                      &dnxc_phy_raw_man},
    {"dsc",       cmd_dnxc_phy_dsc,       NULL,                   dnxc_phy_dsc_options,      &dnxc_phy_dsc_man},
    {"list",      cmd_dnxc_phy_list,      NULL,                   dnxc_phy_list_options,     &dnxc_phy_list_man},
    {"set",       cmd_dnxc_phy_set,       NULL,                   dnxc_phy_set_options,      &dnxc_phy_set_man},
    {"get",       cmd_dnxc_phy_get,       NULL,                   dnxc_phy_get_options,      &dnxc_phy_get_man},
    {"eyescan",   cmd_dnxc_phy_eyescan,   NULL,                   dnxc_phy_eyescan_options,  &dnxc_phy_eyescan_man},
#ifdef SERDES_API_FLOATING_POINT
    {"berproj",   cmd_dnxc_phy_ber_proj,  NULL,                   dnxc_phy_ber_proj_options, &dnxc_phy_ber_proj_man},
#endif
#ifdef PHYMOD_LINKCAT_SUPPORT
...
sh_sand_cmd_t sh_dnxc_port_cmds[] = {
     ...
     * keyword, action, command, options, man callback legacy
     */
    {"status",   cmd_dnxc_port_status,   NULL,   sh_dnxc_port_status_options,   &sh_dnxc_port_status_man},
