        ...
        for (i = 0; i < count; i++)
        {
            ...
            BCM_PBMP_ITER(list[i].port_bitmap, port)
            {
                ...
                if (port_name_buf_index + port_name_len < SOC_PBMP_FMT_LEN*BCM_TELEMETRY_PORT_NAME_SIZE-1)
                {
                    ...
                    sal_strncpy(port_name_buf+port_name_buf_index, ",", 1);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((sal_strcasecmp(action, "delete") != 0) &&
        ...
        (sal_strcasecmp(action, "clear") != 0) &&
        (sal_strcasecmp(action, "show") != 0)) {
        SHR_ERR_EXIT(_SHR_E_PARAM, "%s: ERROR: Invalid gport subcommand\n", action);
    }
    if ((sal_strcasecmp(action, "clear") != 0) &&
        (sal_strcasecmp(action, "show") != 0)) {
        ...
        if (nof_ports == 0)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "It is expected port to be configured!\n");
        }
    }
    if (sal_strcasecmp(action, "show") == 0) {
        ...
        if ((r = bcm_vlan_gport_get_all(unit, vlan, BCM_PBMP_PORT_MAX, port_array,
