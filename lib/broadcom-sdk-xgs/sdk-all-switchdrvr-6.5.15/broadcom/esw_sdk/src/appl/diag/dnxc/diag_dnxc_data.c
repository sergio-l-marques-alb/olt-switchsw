    ...
    sh_sand_control_t * sand_control)
{
     ...
     * Set '*' along the chain
     */
    if (module != NULL && !sal_strncasecmp("*", module, DNXC_DATA_UTILS_STRING_MAX))
    {
        submodule = "*";
        data = "*";
    }
    if (submodule != NULL && !sal_strncasecmp("*", submodule, DNXC_DATA_UTILS_STRING_MAX))
    {
        data = "*";
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (define_index = 0; define_index < submodule->nof_defines; define_index++)
        {
            if (!sal_strncasecmp("*", data, DNXC_DATA_UTILS_STRING_MAX) ||
    ...
    else
    {
        ...
        for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
        {
            if (!sal_strncasecmp(submodules[submodule_index].name, submodule, DNXC_DATA_UTILS_STRING_MAX) ||
                !sal_strncasecmp("*", submodule, DNXC_DATA_UTILS_STRING_MAX))
            {
                submodule_found = 1;
    ...
    else
    {
        ...
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            if (!sal_strncasecmp(modules[module_index].name, module, DNXC_DATA_UTILS_STRING_MAX) ||
                !sal_strncasecmp("*", module, DNXC_DATA_UTILS_STRING_MAX))
            {
                module_found = 1;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
        {
            if (!sal_strncasecmp("*", submodule, DNXC_DATA_UTILS_STRING_MAX) ||
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            if (!sal_strncasecmp("*", module, DNXC_DATA_UTILS_STRING_MAX) ||
...
static sh_sand_man_t dnxc_data_export_man = {
    .brief    = "Export changed data to an xml file (can also print all changed data to the consol)",
    ...
    .examples = "changed file=dnx_data.xml\n"
                "silence file=dnx_data.xml\n"
...
sh_sand_cmd_t sh_dnxc_data_cmds[] = {
    /*keyword,   action,                 cmd,  options,                    man                   */
    {"dump",     cmd_dnxc_data_dump,     NULL, dnxc_data_dump_options,     &dnxc_data_dump_man},
    ...
    {"property_manual",   cmd_dnxc_data_man,      NULL, dnxc_data_man_options,      &dnxc_data_man_man},
    {"property", cmd_dnxc_data_property, NULL, dnxc_data_property_options, &dnx_data_property_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnx},
    {"property", cmd_dnxc_data_property, NULL, dnxc_data_property_options, &dnxf_data_property_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnxf},
    {NULL}
};
