...
static sh_sand_man_t Field_port_list_man = {
    ...
    .examples = "type=PP stage=ipmf1",
};

static sh_sand_option_t Field_port_list_options[] = {
    {DIAG_DNX_FIELD_OPTION_TYPE,  SAL_FIELD_TYPE_UINT32, "Lowest-highest port type to get its info",    "LIF-TM",       (void *)Field_port_type_enum_table},
    {DIAG_DNX_FIELD_OPTION_STAGE,  SAL_FIELD_TYPE_UINT32, "Lowest-highest field stage to context key info for",   "IPMF1-EXTERNAL",    (void *)Field_stage_enum_table},
    {NULL}
};
    ...
    uint8 *do_display_p)
{
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        {
            /**
             * IF the table is DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION check the result_types
             * because not all result types has a field DBAL_FIELD_IN_LIF_PROFILE.
             */
            if (dbal_tables[table] == DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION)
            {
                ...
                if (result_type[0] == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_VSI_MP ||
                    result_type[0] == DBAL_RESULT_TYPE_INNER_ETH_VLAN_EDIT_CLASSIFICATION_IN_ETH_VLAN_EDIT_ONLY)
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (port_type = port_type_lower; port_type <= port_type_upper; port_type++)
    {
        ...
        for (stage_index = stage_lower; stage_index <= stage_upper && (DNX_FIELD_IS_FP_STAGE(stage_index));
             stage_index++)
        {
            /** Field Port diagnostic is not supported for ACE stage. */
            if (stage_index == DNX_FIELD_STAGE_ACE)
            {
                continue;
            }
            /** Fill the array for all pclasses for the given port_type and stage. */
            nof_pclasses = 0;
            diag_dnx_field_port_pclass_get(unit, stage_index, port_type, pclass_arr, &nof_pclasses);
            ...
            if (port_type == 0)
            {
                ...
                if (stage_index == DNX_FIELD_STAGE_IPMF1 ||
                    stage_index == DNX_FIELD_STAGE_IPMF2 || stage_index == DNX_FIELD_STAGE_IPMF3)
                {
            ...
            else
            {
                ...
                BCM_PBMP_ITER(port_config.all, port)
                {
                    ...
                    if (print_new_port && (cs_profile_value || profile_value || general_data_value))
                    {
                        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                        PRT_CELL_SET("%d", port);
                        PRT_CELL_SET("%s", dnx_field_stage_text(unit, Field_stage_enum_table[stage_index].value));
                        PRT_CELL_SET("0x%x", cs_profile_value);
                        if (stage_index != DNX_FIELD_STAGE_EXTERNAL)
                        {
                        ...
                        if (port_type == 1)
                        {
                            if (stage_index == DNX_FIELD_STAGE_IPMF1 || stage_index == DNX_FIELD_STAGE_IPMF2 ||
