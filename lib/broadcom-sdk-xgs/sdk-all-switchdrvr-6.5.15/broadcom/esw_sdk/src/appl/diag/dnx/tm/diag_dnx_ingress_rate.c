...
#define DNX_ING_COUNTER_RATE_PRINT(_index, _name, _type, _val, _ovf)                           \
{                                                                                              \
    uint32 _rate;                                                                              \
    ...
    {                                                                                          \
        _rate = (_val / 1000) * UTILEX_NOF_BITS_IN_BYTE;                                       \
        if (_ovf)                                                                              \
        {                                                                                      \
            PRT_CELL_SET("overflow!!!");                                                       \
        }                                                                                      \
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_IS_PRESENT("core", core_provided);
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        counter_idx = 0;
        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "NIF Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.nif_pkt_cnt, counter_info.nif_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "RCYH Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.rcyh_pkt_cnt, counter_info.rcyh_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "RCYL Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.rcyl_pkt_cnt, counter_info.rcyl_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "CPU Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.cpu_pkt_cnt, counter_info.cpu_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "OLP Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.olp_pkt_cnt, counter_info.olp_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "OAMP Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.oamp_pkt_cnt, counter_info.oamp_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "Register Interface Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
 ...
 */
sh_sand_option_t sh_dnx_ingress_ire_rate_options[] = {
/* *INDENT-OFF* */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        counter_idx = 0;
        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "EN-QUEUE Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.pkt_enq_cnt, counter_info.enq_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "DE-QUEUE Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
 ...
 */
sh_sand_option_t sh_dnx_ingress_sqm_rate_options[] = {
/* *INDENT-OFF* */
    /*name        type                   desc              default*/
    {"core",      SAL_FIELD_TYPE_INT32,  "core id",         "-17",      NULL},
    {NULL}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        counter_idx = 0;
        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "Packet from CGM Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.pkt_cgm_cnt, counter_info.cgm_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "Packet from SPB Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.pkt_spb_cnt, counter_info.spb_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "ITPP Discard Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
 ...
 */
sh_sand_option_t sh_dnx_ingress_ipt_rate_options[] = {
/* *INDENT-OFF* */
    /*name        type                   desc              default*/
    {"core",      SAL_FIELD_TYPE_INT32,  "core id",         "-17",      NULL},
    {NULL}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        counter_idx = 0;
        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Enqueued packets", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.sram_enq_pkt_cnt, counter_info.sram_enq_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Rejected packets", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.sram_disc_pkt_cnt, counter_info.sram_disc_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM to DRAM Dequeued packets", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.sram_to_dram_pkt_cnt, counter_info.sram_to_dram_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM to Fabric Dequeued packets", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.sram_to_fabric_pkt_cnt, counter_info.sram_to_fabric_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Deleted packets", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                   counter_info.sram_del_pkt_cnt, counter_info.sram_del_pkt_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Enqueued bytes", DNX_INGRESS_COUTNER_TYPE_BYTE,
                                   counter_info.sram_enq_byte_cnt, counter_info.sram_enq_byte_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Discarded bytes", DNX_INGRESS_COUTNER_TYPE_BYTE,
                                   counter_info.sram_disc_byte_cnt, counter_info.sram_disc_byte_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM Deleted bytes", DNX_INGRESS_COUTNER_TYPE_BYTE,
                                   counter_info.sram_del_byte_cnt, counter_info.sram_del_byte_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SRAM to DRAM Dequeued bytes", DNX_INGRESS_COUTNER_TYPE_BYTE,
                                   counter_info.sram_to_dram_byte_cnt, counter_info.sram_to_dram_byte_ovf);

        DNX_ING_COUNTER_RATE_PRINT(counter_idx, "SSRAM to fabric Dequeued bytes", DNX_INGRESS_COUTNER_TYPE_BYTE,
