/*
 * $Id: knetctrl.c,v 1.15 Broadcom SDK $
...
char cmd_knet_ctrl_usage[] =
    ...
    "  knetctrl debugpkt [on|off|show]\n"
    "        Enable/Disable debugging packet function.\n"
    "        Once enabling this function, there will be one packet copy being\n"
    "        sent/mirrored to RX API for debugging usage.\n"
#endif
    ;
...
_show_filter(int unit, bcm_knet_filter_t *filter)
{
    ...
    if (filter->dest_proto) {
        sal_sprintf(proto_str, "[0x%04x]", filter->dest_proto);
    }
    ...
    if (filter->mirror_type ==  BCM_KNET_DEST_T_BCM_RX_API) {
        ...
        if (filter->mirror_proto) {
            sal_sprintf(proto_str, "[0x%04x]", filter->mirror_proto);
        }
    ...
    if (filter->match_flags & BCM_KNET_FILTER_M_INGPORT) {
        cli_out(" ingport=%s", BCM_PORT_NAME(unit, filter->m_ingport));
    }
...
cmd_knet_ctrl(int unit, args_t *args)
{
    ...
    if (sal_strcasecmp(subcmd, "netif") == 0) {
    ...
    } else if (sal_strcasecmp(subcmd, "debugpkt") == 0) {
        if ((subcmd = ARG_GET(args)) == NULL) {
            cli_out("Requires string argument\n");
            return CMD_USAGE;
        }
        ...
        if (sal_strcasecmp(subcmd, "on") == 0) {
            if (soc_knet_debug_pkt_set(unit, TRUE) < 0) {
                return CMD_FAIL;
            }
        } else if (sal_strcasecmp(subcmd, "off") == 0) {
            if (soc_knet_debug_pkt_set(unit, FALSE) < 0) {
                return CMD_FAIL;
            }
        } else if (sal_strcasecmp(subcmd, "show") == 0) {
            int value = 0;
            if (soc_knet_debug_pkt_get(unit, &value) < 0) {
                return CMD_FAIL;
            }

            if (TRUE == value) {
                cli_out("Knet debugging packet function is on!\n");
            } else {
                cli_out("Knet debugging packet function is off!\n");
            }
        } else {
            cli_out("Subcommand not found: %s\n", subcmd);
            return CMD_USAGE;
