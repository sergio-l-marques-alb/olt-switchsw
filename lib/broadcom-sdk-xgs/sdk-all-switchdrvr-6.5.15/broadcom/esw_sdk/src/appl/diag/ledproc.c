...
ledproc_linkscan_cb(int unit, soc_port_t port, bcm_port_info_t *info)
{
    ...
    if (soc_feature(unit, soc_feature_cmicx)) {
        ...
        soc_led_port_speed_t led_speed;
        ...
        */
        ...
        if (led_link_sts.link_sts) {
            ...
            led_speed.port = led_link_sts.port;
            led_speed.speed = speed;
            if (led_speed.speed) {
#if defined (BCM_ESW_SUPPORT) || defined(BCM_DNXF_SUPPORT)
                soc_cmicx_led_speed(unit, &led_speed);
#endif /* BCM_ESW_SUPPORT || BCM_DNXF_SUPPORT */
            }
        }
 ...
 */
    ...
    if (ARG_CNT(a) == 0) {
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
                ...
                cli_out("LED M0 FW is %s Init DONE; "
                        "auto updating is %s.\n",
                        (SOC_CONTROL(unit)->led_fw_enabled) ? "ON" : "OFF",
                        auto_on ? "ON" : "OFF");
#endif /* CMICX_LED */
                return(CMD_OK);
            }
    ...
    if (!sal_strcasecmp(c, "status")) {
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            if (soc_feature(unit, soc_feature_cmicx)) {
#if CMICX_LED
                if (!SOC_CONTROL(unit)->led_fw_enabled) {
        ...
        if (soc_feature(unit, soc_feature_cmicx)) {
            ...
            int rv = SOC_E_NONE;
#if defined (BCM_ESW_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(BCM_DNX_SUPPORT)
            rv = soc_cmicx_led_enable(unit, LED_START);
#endif /* BCM_ESW_SUPPORT || BCM_DNXF_SUPPORT */
            if (rv != SOC_E_NONE) {
        ...
        if (soc_feature(unit, soc_feature_cmicx)) {
            ...
            int rv = SOC_E_NONE;
#if defined (BCM_ESW_SUPPORT) || defined(BCM_DNXF_SUPPORT) || defined(BCM_DNX_SUPPORT)
            rv = soc_cmicx_led_enable(unit, LED_STOP);
