    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
        ...
        {
            base_queue = BCM_GPORT_UCAST_QUEUE_GROUP_QID_GET(voq_gport);
            BCM_GPORT_UNICAST_QUEUE_GROUP_CORE_QUEUE_SET(voq_gport, core, queue);;
        }
         ...
         */
        PRT_CELL_SET("%u", voq_state_info.credit_balance);
    }
    PRT_COMMITX;
    ...
    sh_sand_control_t * sand_control)
{
    bcm_gport_t voq_gport;
    int rate_class;
    int max_mc_queue, base_queue = 0, num_cos = 0, cos;
...
/* *INDENT-OFF* */
    ...
    .full     = "Enable autocredit for a range of ingress queues, if no parametrs provided then current status is printed",
...
/* *INDENT-OFF* */
    ...
    {"enable",    SAL_FIELD_TYPE_BOOL,  "Option to enable/disable autocredit","0"},
    {"queue",     SAL_FIELD_TYPE_INT32, "Queue range",     "0", tm_queue_id_table, "0-max"},
    {NULL}
/* *INDENT-ON* */
};
...
dnx_iqs_debug_autocredit_info_print(
    ...
    sh_sand_control_t * sand_control)
{
    uint32 min_queue, max_queue, enable;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    /** get info */
    SHR_IF_ERR_EXIT(dnx_iqs_debug_autocredit_enable_get(unit, &min_queue, &max_queue, &enable));
    ...
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    PRT_CELL_SET("%s", enable ? "TRUE" : "FALSE");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        if (enable)
        {
            SHR_IF_ERR_EXIT(dnx_iqs_debug_autocredit_enable_set(unit, queue_min, queue_max, enable));
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_iqs_debug_autocredit_enable_set(unit, 0, 0, enable));
        }
    }
...
/* *INDENT-OFF* */
    ...
    {"last",        sh_dnx_tm_queue_last_cmd,       NULL,   sh_dnx_tm_queue_last_options,           &sh_dnx_tm_queue_last_man},
    {NULL}
/* *INDENT-ON* */
};
