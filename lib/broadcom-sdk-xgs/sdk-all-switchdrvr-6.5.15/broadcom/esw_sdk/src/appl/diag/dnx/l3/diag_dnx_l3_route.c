...
static sh_sand_man_t dnx_l3_route_add_man = {
    ...
    .synopsis =
        "(ip=<IPv4 IP> mask=<IPv4 mask>) | (ip6=<IPv6 IP> ip6mask=<IPv6 mask>), [vrf=<VRF>], [fec=<FEC>], [v6=<NO>], [raw=<NO>], [multicast=<NO>]",
    .examples =
...
static sh_sand_option_t dnx_l3_route_add_options[] = {
    ...
    {"ip6mask", SAL_FIELD_TYPE_IP6, "IPv6 mask. For 'multicast', this is source ip mask",
     "0000:0000:0000:0000:0000:0000:0000:0000"},
    {"fec", SAL_FIELD_TYPE_INT32, "FEC", "0x12345"},
    ...
    char *action)
{
    ...
    if (ip_version == 4)
    {
        SHR_IF_ERR_EXIT(dnx_sand_ip_addr_numeric_to_string(route_info->l3a_subnet, 1, buffer));
        PRT_CELL_SET("%s", buffer);
        SHR_IF_ERR_EXIT(dnx_sand_ip_addr_numeric_to_string(route_info->l3a_ip_mask, 1, buffer));
    }
    else
    {
