...
static sh_sand_option_t Field_group_list_options[] = {
    {DIAG_DNX_FIELD_OPTION_GROUP,  SAL_FIELD_TYPE_UINT32, "Lowest-highest field group to get its info",       "MIN-MAX",       (void *)Field_group_enum_table},
    {DIAG_DNX_FIELD_OPTION_STAGE,  SAL_FIELD_TYPE_UINT32, "Lowest-highest field stage to get key info for",   "IPMF1-EXTERNAL",    (void *)Field_stage_enum_table},
    {NULL}
};
...
static sh_sand_option_t Field_group_info_options[] = {
    {DIAG_DNX_FIELD_OPTION_GROUP,          SAL_FIELD_TYPE_UINT32, "Field group to get its info",           NULL   ,(void *)Field_group_enum_table},
    {NULL}
};
/*
 * }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (qual_action_iter = 0; qual_action_iter < max_num_qual_actions; qual_action_iter++)
    {
        ...
        if (qual_left)
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_group_info_qual_print(unit, qual_action_iter, fg_info_p, prt_ctr));
        }
        else
        {
            /**
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (qual_iter = 0, action_iter = 0; (qual_iter < max_num_qual_actions) && (action_iter < max_num_qual_actions);)
    {
        ...
        else if (qual_size > action_size && action_left)
        {
            ...
            {
                /** Print the qualifiers info. */
                SHR_IF_ERR_EXIT(diag_dnx_field_group_info_qual_print(unit, qual_iter, fg_info_p, prt_ctr));
 ...
 * \param [in] unit - The unit number.
 * \param [in] bank_entries_count_p - Pointer, which contains number of allocated entries per TCAM bank,
 ...
 * \param [out] bank_entries_count_p - Pointer, which which will store
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char bank_ids_string[DIAG_DNX_FIELD_UTILS_STR_SIZE] = { 0 };
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entries_info
                    (unit, core, tcam_handler_id, bank_entries_count_p, banks_bmp_p));
    ...
    for (bank_id_iter = 0; bank_id_iter < DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS; bank_id_iter++)
    {
        if (SHR_BITGET(banks_bmp_p, bank_id_iter))
        {
            ...
            DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(bank_ids_string, "%s", banks_buff);
            sal_strncat(bank_ids_string, banks_buff, sizeof(bank_ids_string) - 1);
        }
    }
    /**
     ...
     */
    bank_ids_string[sal_strlen(bank_ids_string) - 2] = '\0';
...
/**
 * \brief
 *   This function prepares and sets information regarding Access profile and Banks
 *   of a specific TCAM DirectTable FG.
 * \param [in] unit - The unit number.
 * \param [in] fg_id - FG ID to retrieve its dbal table.
 * \param [in] fg_info_p - Pointer, which contains information for the
 *  given FG, in this function we will retrieve the Access Profile from it.
 ...
 */
static shr_error_e
diag_dnx_field_group_dt_tcam_banks_info_print(
    int unit,
    dnx_field_group_t fg_id,
    dnx_field_group_full_info_t * fg_info_p,
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_tcam_handler_id_get(unit, dbal_table_id, &tcam_handler_id));
    /** Retrieve information about all entries, which are being add to the given FG, and used TCAM banks. */
    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entries_info(unit, core, tcam_handler_id, bank_entries_count, banks_bmp));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    unsigned int actions_size_per_comb[DNX_FIELD_NUM_2MSB_COMBINATIONS] = { 0 };
    ...
    SHR_IF_ERR_EXIT(dnx_field_group_calc_actions_bit_size(unit, fg_info_p->group_basic_info.field_stage,
                                                          fg_info_p->group_full_info.
                                                          actions_payload_info.actions_on_payload_info,
                                                          actions_size_per_comb, &payload_size_in_bits));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(diag_dnx_field_group_dt_tcam_banks_info_print(unit, fg_id, fg_info_p, sand_control));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (stage_index = stage_lower; stage_index <= stage_upper && (DNX_FIELD_IS_FP_STAGE(stage_index)); stage_index++)
    {
        ...
        for (fg_id_index = fg_id_lower; fg_id_index <= fg_id_upper; fg_id_index++)
        {
             ...
             */
            if (Field_stage_enum_table[stage_index].value != fg_info->group_basic_info.field_stage)
            {
                continue;
            }
