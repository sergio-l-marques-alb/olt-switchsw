...
#define DIAG_DNXC_SHOW_COUNTERS_SHARED_EXAMPLES      "\n" \
                                                     "full\n" \
                                                     "hex\n"
...
/* *INDENT-OFF* */
static sh_sand_man_t dnxc_show_voltage_man = {
    .brief    = "Shows device voltage",
    .full     = "Shows all device voltage monitors indications"
};
/* *INDENT-ON* */

static shr_error_e
diag_dnxc_pvt_change(
    int unit,
    int nof_pvt_monitors,
    int voltage)
{
    soc_reg_t pvt_control[] = { ECI_PVT_MON_A_CONTROL_REGr, ECI_PVT_MON_B_CONTROL_REGr, ECI_PVT_MON_C_CONTROL_REGr,
    ...
    for (ii = 0; ii < nof_pvt_monitors; ii++)
    {
        COMPILER_64_SET(controlVal, 0, 0);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, pvt_control[ii], REG_PORT_ANY, 0, controlVal));
        /*
         * power down 
         */
        COMPILER_64_SET(controlVal, val | 0x1, 0);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, pvt_control[ii], REG_PORT_ANY, 0, controlVal));
        sal_usleep(50);
        /*
         * power up 
         */
        COMPILER_64_SET(controlVal, val, 0);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, pvt_control[ii], REG_PORT_ANY, 0, controlVal));
        sal_usleep(100);
        /*
         * reset 
         */
        COMPILER_64_SET(controlVal, val | 0x2, 0);
        SHR_IF_ERR_EXIT(soc_reg_set(unit, pvt_control[ii], REG_PORT_ANY, 0, controlVal));
        sal_usleep(50);
    }
exit:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    soc_reg_t pvt_data[] = { ECI_PVT_MON_A_THERMAL_DATAr, ECI_PVT_MON_B_THERMAL_DATAr, ECI_PVT_MON_C_THERMAL_DATAr,
        ECI_PVT_MON_D_THERMAL_DATAr
    };
    soc_field_t pvt_current[] = { THERMAL_DATA_Af, THERMAL_DATA_Bf, THERMAL_DATA_Cf, THERMAL_DATA_Df };
    soc_field_t pvt_peak[] = { PEAK_THERMAL_DATA_Af, PEAK_THERMAL_DATA_Bf, PEAK_THERMAL_DATA_Cf, PEAK_THERMAL_DATA_Df };
    uint32 data;
    uint32 remainder[3];
     ...
     */
    PRT_TITLE_SET("VOLTAGE");
    PRT_COLUMN_ADD("Monitor");
    PRT_COLUMN_ADD("Current Voltage (mV)");
    PRT_COLUMN_ALIGN;
    PRT_COLUMN_ADD("Peak Voltage (mV)");
    PRT_COLUMN_ALIGN;

    SHR_IF_ERR_EXIT(diag_dnxc_pvt_change(unit, nof_pvt_monitors, 1));

    for (monitor_idx = 0; monitor_idx < nof_pvt_monitors; monitor_idx++)
    {
        SHR_IF_ERR_EXIT(soc_reg32_get(unit, pvt_data[monitor_idx], 0, 0, &data));
        temp_monitor[monitor_idx].curr = soc_reg_field_get(unit, pvt_data[monitor_idx], data, pvt_current[monitor_idx]);
        temp_monitor[monitor_idx].peak = soc_reg_field_get(unit, pvt_data[monitor_idx], data, pvt_peak[monitor_idx]);

        temp_int = (temp_monitor[monitor_idx].curr * 950 * 5) / (4 * 1024);
        temp_remainder = (temp_monitor[monitor_idx].curr * 950 * 5) % (4 * 1024);
        temp_remainder = (temp_remainder * 1000) / (4 * 1024);
        remainder[0] = temp_remainder % 10;
        remainder[1] = (temp_remainder / 10) % 10;
        remainder[2] = (temp_remainder / 100) % 10;

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%d", monitor_idx);
        PRT_CELL_SET("%d.%d%d%d", temp_int, remainder[0], remainder[1], remainder[2]);

        temp_int = (temp_monitor[monitor_idx].peak * 950 * 5) / (4 * 1024);
        temp_remainder = (temp_monitor[monitor_idx].peak * 950 * 5) % (4 * 1024);
        temp_remainder = (temp_remainder * 1000) / (4 * 1024);
        remainder[0] = temp_remainder % 10;
        remainder[1] = (temp_remainder / 10) % 10;
        remainder[2] = (temp_remainder / 100) % 10;
    ...
    SHR_IF_ERR_EXIT(diag_dnxc_pvt_change(unit, nof_pvt_monitors, 0));
    PRT_COMMITX;
...
static sh_sand_man_t dnx_show_counters_man = {
    ...
    if (!is_port_present)
    {
        ...
        if (SOC_IS_DNX(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                            (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF, 0, &selected_pbmp));
        }
        else
#endif /* BCM_DNX_SUPPORT */
        {
        ...
        PBMP_ITER(selected_pbmp, port)
        {
            ...
            if (cmap == NULL)
            {
                ...
                for (index = 0; soc->controlled_counters[index].controlled_counter_f != NULL; index++)
                {
                    ...
                    {
                        enable = 0;
                        rv = soc_jer2_stat_controlled_counter_enable_get(unit, port, index, &enable, &printable);
...
static sh_sand_man_t dnxf_clear_counters_man = {
    ...
    .synopsis = "Parameters: [Port=<integer|port_name|'fabric'|'all'>]",
    .examples = "port=sfi13\n"
                "port=sfi13-sfi16 \n"
                "port=fabric\n",
};
...
sh_sand_cmd_t sh_dnxc_show_cmds[] = {
    ...
    {"counter",   cmd_dnxc_show_counters, NULL,    dnxc_show_counters_options, &dnx_show_counters_man,   NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnx},
    {"counter",   cmd_dnxc_show_counters, NULL,    dnxc_show_counters_options, &dnxf_show_counters_man,  NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnxf},
    ...
    {"VOLTage",   cmd_dnxc_show_voltage,  NULL,    NULL,                       &dnxc_show_voltage_man},
    {NULL}
};
