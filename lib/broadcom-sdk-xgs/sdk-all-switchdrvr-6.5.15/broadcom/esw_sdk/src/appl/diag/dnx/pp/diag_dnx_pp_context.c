    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (; core_id < nof_cores; core_id++)
    {
        ...
        for (stage_id = stage_start; stage_id < nof_stages; stage_id++)
        {
             ...
             */
            if (context_val >= nof_values)
            {
                PRT_CELL_SET("%02x", context_val);
            }
            else
            {
                PRT_CELL_SET("%s", context_field_type->enum_val_info[context_val].name);
            }
    ...
    if (!ISEMPTY(stage_name))
    {
        ...
        if (table_names[stage_start] != 0 && context_val < nof_values)
        {
             ...
             */
            for (int iter = 0; iter < table->nof_key_fields; iter++)
            {
            ...
            for (int iter = 0; iter < table->multi_res_info[0].nof_result_fields; iter++)
            {
