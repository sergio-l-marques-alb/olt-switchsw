 ...
 * ITMH base header
 */
typedef union diag_soc_dnx_itmh_base_u
{
    struct
    {
        uint8 bytes[5];
...
#endif
} diag_soc_dnx_itmh_base_t;

typedef struct diag_soc_dnx_itmh_s
{
    /*
     * ITMH base header
     */
    diag_soc_dnx_itmh_base_t base;
} diag_soc_dnx_itmh_t;
    ...
    int unit)
{
    ...
    /*
     * clear out the ptch/itmh header contents
     */
...
{
    bcm_pkt_t *pkt_info = &xd->pkt_info;
    diag_soc_dnx_itmh_t itmh;
    uint32 itmh_tc = 0;

    sal_memset(&itmh, 0, sizeof(diag_soc_dnx_itmh_t));
    ...
    itmh.base.jr2_common.inbound_mirror_disable = xd->itmh_in_mirr_flag;
    itmh.base.jr2_common.snoop_profile_hi = xd->itmh_snoop_cmd >> 4 & 0x1;
    itmh.base.jr2_common.snoop_profile_lo = xd->itmh_snoop_cmd & 0xf;
    itmh.base.jr2_common.traffic_class = itmh_tc;
    itmh.base.jr2_common.drop_precedence = xd->itmh_dp;

    if (xd->itmh_fwd_type == DNX_HDR_ITMH_FWD_TYPE_UNICAST_DIRECT)
    {
        /*
        ...
        pkt_info->_dpp_hdr_type = DNX_HDR_itmh_base;
    }
    else if (xd->itmh_fwd_type == DNX_HDR_ITMH_FWD_TYPE_SYSTEM_MULTICAST)
    {
        /*
         * 11(2) + MC-ID(19)
         */
    ...
    else if (xd->itmh_fwd_type == DNX_HDR_ITMH_FWD_TYPE_UNICAST_FLOW)
    {
        /*
         * 10(2) + TM-Flow(19)
         */
    ...
    {
        LOG_ERROR(BSL_LS_APPL_TX, (BSL_META("Unsupported ITMH Destination type %d\n"), xd->itmh_fwd_type));
    }

    sal_memcpy(&(pkt_info->_dpp_hdr[0]), &itmh.base.raw.bytes[0], DNX_HDR_ITMH_BASE_LEN);
...
/**
 * \brief - Update Destination of ITMH
 */
static void
diag_dnx_tx_itmh_dest_port_set(
    ...
    uint32 dest_port)
{
    diag_soc_dnx_itmh_t *itmh = (diag_soc_dnx_itmh_t *) itmh_header;

    /*
     * 01100(5) + System_Port_Agg(16)
     */
    itmh->base.jr2_dest_info.destination_hi = 0x18 | ((dest_port >> 15) & 0x1);
    itmh->base.jr2_dest_info.destination_mi = (dest_port >> 7) & 0xff;
    itmh->base.jr2_dest_info.destination_lo = dest_port & 0x7f;
    return;
}
    ...
    int *incoming_header_type)
{
    ...
    rv = dnx_port_ptc_profile_get(unit, port, &prt_profile, &llr_profile);
    if (BCM_E_NONE != rv)
    {
    ...
    xd_t * xd)
{
    ...
    if (ptch_ssp == 0xFFFFFFFF)
    {
        ...
        /** Get device modid */
        rv = bcm_stk_modid_get(xd->xd_unit, &modid);
        if (BCM_E_NONE != rv)
        {
            LOG_ERROR(BSL_LS_APPL_SHELL, (BSL_META("TX failed to get device mod id %s\n"), bcm_errmsg(rv)));
            return (CMD_FAIL);
        }
        /** Switch local to sysport */
        rv = appl_dnx_logical_port_to_sysport_get(xd->xd_unit, modid, xd->xd_ptch_src_port, &sysport);
        if (BCM_E_NONE != rv)
        {
        ...
        rv = bcm_trunk_find(xd->xd_unit, modid, gport, &tid_find);
        if (BCM_E_NONE != rv)
        {
        ...
        (header_type_incoming == BCM_SWITCH_PORT_HEADER_TYPE_INJECTED_2))
    {
        if (BCM_PBMP_NOT_NULL(pkt_info->tx_pbmp))
        {
            ...
            xd->itmh_fwd_type = DNX_HDR_ITMH_FWD_TYPE_UNICAST_DIRECT;
            diag_dnx_tx_setup_itmh_tx(xd);
            header_size =
                ((DNX_HDR_itmh_base ==
                  pkt_info->_dpp_hdr_type) ? (DNX_HDR_ITMH_BASE_LEN + DNX_HDR_PTCH_TYPE2_LEN) : DNX_HDR_PTCH_TYPE2_LEN);
        }
        ...
        {
            pkt_info->_dpp_hdr_type = DNX_HDR_none;
            header_size = DNX_HDR_PTCH_TYPE2_LEN;
        }
        ...
        pkt_data_with_header =
            (uint8 *) soc_cm_salloc(xd->xd_unit, xd->xd_pkt_len + header_size, "TX packet with ptch");
        ...
        if (DNX_HDR_itmh_base == pkt_info->_dpp_hdr_type)
        {
            /*
            ...
            pkt_data_with_header[DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN] = pkt_info->_dpp_hdr[0];
            pkt_data_with_header[DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN + 1] = pkt_info->_dpp_hdr[1];
            pkt_data_with_header[DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN + 2] = pkt_info->_dpp_hdr[2];
            pkt_data_with_header[DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN + 3] = pkt_info->_dpp_hdr[3];
            pkt_data_with_header[DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN + 4] = pkt_info->_dpp_hdr[4];
        }
        else
        {
            /*
    ...
    payload_len = pkt_info->pkt_data[0].len - header_size - sizeof(enet_hdr_t) - sizeof(uint32);
    ...
    for (xd->xd_cur_cnt = 0; xd->xd_cur_cnt < xd->xd_tot_cnt; xd->xd_cur_cnt++)
    {
        ...
        BCM_PBMP_ITER(pkt_info->tx_pbmp, port_ndx)
        {
            ...
            diag_dnx_tx_itmh_dest_port_set(xd->xd_tx_unit,
                                           pkt_info->pkt_data[0].data + DNX_MODULE_HEADER_SIZE + DNX_HDR_PTCH_TYPE2_LEN,
