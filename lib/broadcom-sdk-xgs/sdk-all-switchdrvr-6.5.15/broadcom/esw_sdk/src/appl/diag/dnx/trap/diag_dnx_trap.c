    ...
    char *trap_id_str)
{
    ...
    if ((trap_id == BCM_RX_TRAP_EG_TX_TRAP_ID_DEFAULT) || (trap_id == BCM_RX_TRAP_EG_TRAP_ID_DEFAULT) || 0)
    {
        sal_strncpy(trap_id_str, "DEFAULT", DIAG_DNX_TRAP_ID_STRING_SIZE);
    }
    else if (trap_id == BCM_RX_TRAP_EG_TX_TRAP_ID_DROP || 1)
    {
...
sh_sand_cmd_t sh_dnx_trap_cmds[] = {
    ...
    ,
    {"MTU", sh_dnx_trap_mtu_cmd, NULL, NULL, &sh_dnx_trap_mtu_cmd_man}
    ,
