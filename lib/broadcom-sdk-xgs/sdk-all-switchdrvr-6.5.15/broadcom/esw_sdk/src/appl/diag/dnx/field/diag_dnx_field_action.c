...
static sh_sand_option_t Field_action_user_list_options[] = {
    {DIAG_DNX_FIELD_OPTION_ID,  SAL_FIELD_TYPE_UINT32, "Lowest-highest user defined action ids to get its info",       "MIN-MAX",       (void *)Field_action_enum_table},
    {DIAG_DNX_FIELD_OPTION_STAGE,  SAL_FIELD_TYPE_UINT32, "Lowest-highest field stage to get action info for",   "IPMF1-EXTERNAL",    (void *)Field_stage_enum_table},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_TITLE_SET("Action Map for:\"%s\"", dnx_field_stage_text(unit, stage));
...
#define DNX_FIELD_STAGE_ACTION_ITERATOR_COND(in_stage, out_stage)                                                    \
        for(out_stage = DNX_FIELD_STAGE_FIRST; out_stage < DNX_FIELD_STAGE_NOF; out_stage++)                         \
            if(((in_stage == DNX_FIELD_STAGE_NOF) && (dnx_field_map_stage_info[stage].static_action_id_map != NULL)) || \
                (in_stage == out_stage))
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 size;
    char *action_desc;
    dnx_field_action_t dnx_action_a[DNX_FIELD_STAGE_NOF];
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    PRT_COLUMN_ADD("Size");
    DNX_FIELD_STAGE_ACTION_ITERATOR_COND(in_stage, stage)
    {
    ...
    for (bcm_action = 0; bcm_action < bcmFieldActionCount; bcm_action++)
    {
        ...
        /*
         * Initiate action_set array
         */
        DNX_FIELD_STAGE_ACTION_ITERATOR_COND(in_stage, stage)
        {
            dnx_action_a[stage] = 0;
        }
        /*
         * Check if there is any dnx mapping for specific bcmAction
         */
        DNX_FIELD_STAGE_ACTION_ITERATOR_COND(in_stage, stage)
        {
            if (!SHR_FAILURE(dnx_field_map_action_bcm_to_dnx_int(unit, stage, bcm_action, &dnx_action_a[stage], NULL)))
            {
                bcm_action_n = dnx_field_bcm_action_text(unit, bcm_action);
                SHR_IF_ERR_EXIT(dnx_field_map_dnx_action_size(unit, stage, dnx_action_a[stage], &size));
            }
        }
        /*
        ...
        PRT_CELL_SET("%d", size);
        /*
         * Go through stage and print the names of dnx actions
         */
        DNX_FIELD_STAGE_ACTION_ITERATOR_COND(in_stage, stage)
        {
            if (dnx_action_a[stage] == 0)
            {
                PRT_CELL_SET("%s", "N/A");
            }
            else
            {
                PRT_CELL_SET("%s", dnx_field_dnx_action_text(unit, dnx_action_a[stage]));
            }
        }
    }
    ...
    prt_control_t * prt_ctr)
{
    ...
    dnx_field_action_t fg_action;
    uint8 do_row_sep_none_add;
    dnx_field_action_class_e dnx_action_class;
    ...
    for (fg_id_index = 0; fg_id_index < max_nof_fg; fg_id_index++)
    {
        ...
        for (fg_act_iter = 0; fg_act_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP &&
             fg_info_p->group_basic_info.dnx_actions[0][fg_act_iter] != DNX_FIELD_ACTION_INVALID; fg_act_iter++)
        {
            dnx_action_class = DNX_ACTION_CLASS(fg_info_p->group_basic_info.dnx_actions[0][fg_act_iter]);
            fg_action = DNX_ACTION_ID(fg_info_p->group_basic_info.dnx_actions[0][fg_act_iter]);
            /** Get information about the current Field action. */
            if ((fg_action == dnx_action_id) && (dnx_action_class == DNX_FIELD_ACTION_CLASS_USER))
            {
    ...
    if (!do_row_sep_none_add)
    {
        ...
        for (format_id_index = 1; format_id_index < max_num_format_ids; format_id_index++)
        {
            ...
            for (fg_act_iter = 0; fg_act_iter < DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_ACTION_PER_GROUP &&
                 ...
                 fg_act_iter++)
            {
                dnx_action_class = DNX_ACTION_CLASS(ace_format_id_info->format_basic_info.dnx_actions[fg_act_iter]);
                fg_action = DNX_ACTION_ID(ace_format_id_info->format_basic_info.dnx_actions[fg_act_iter]);
                /** Get information about the current Field action. */
                if ((fg_action == dnx_action_id) && (dnx_action_class == DNX_FIELD_ACTION_CLASS_USER))
                {
                    if (do_row_sep_none_add)
    ...
    for (stage_index = stage_lower; stage_index <= stage_upper && (DNX_FIELD_IS_FP_STAGE(stage_index)); stage_index++)
    {
             ...
             action_id <= (BCM_FIELD_FIRST_USER_ACTION_ID + action_id_upper); action_id++)
        {
           ...
           /**
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_action_list_fg_usage_cb(unit, dnx_action_id, prt_ctr));
