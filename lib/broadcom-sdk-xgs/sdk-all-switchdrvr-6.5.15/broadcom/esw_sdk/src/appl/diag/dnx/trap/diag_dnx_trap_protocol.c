...
static sh_sand_man_t dnx_trap_protocol_list_man = {
    .brief = "Show protocol traps profiles\n",
    ...
    prt_control_t *prt_ctr)
{
    ...
    for (protocol_index = 0; protocol_types[protocol_index] != bcmRxTrapCount; protocol_index++)
    {
        ...
        for(trap_arg_index = 0; trap_arg_index < nof_trap_args[protocol_index]; trap_arg_index++)
        {
            ...
            if(BCM_GPORT_TRAP_GET_STRENGTH(trap_gport) != 0 ||
               BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(trap_gport) != 0)
            {
                if(add_new_inlif)
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_iterator_key_field32_rule_add(unit, entry_handle_id, DBAL_FIELD_IN_PP_PORT, DBAL_CONDITION_BIGGER_THAN, field_val, 0));
        ...
        while (!is_end)
        {
            ...
            SHR_IF_ERR_EXIT(sh_dnx_trap_protocol_lines_print(unit, in_port_field, protocol_profiles, add_new_port, prt_ctr));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (table = 0; table < nof_tables; table++)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_iterator_init(unit, entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
        SHR_IF_ERR_EXIT(dbal_iterator_key_field32_rule_add(unit, entry_handle_id, DBAL_FIELD_IN_LIF, DBAL_CONDITION_BIGGER_THAN, field_val, 0));
        /** Receive first entry in table. */
        SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, entry_handle_id, &is_end));
        while (!is_end)
        {
            ...
            dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle);

            local_lif_info.core_id = entry_handle->core_id;
            ...
            if(global_lif != -1)
            {
                ...
                {
                    dnx_algo_gpm_gport_from_lif(unit,DNX_ALGO_GPM_GPORT_HW_RESOURCES_DPC_LOCAL_LIF_INGRESS, _SHR_CORE_ALL, in_lif_field, &gport);
                }
                else
                {
                    dnx_algo_gpm_gport_from_lif(unit,DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS, _SHR_CORE_ALL, global_lif, &gport);
                }

                rv = bcm_rx_trap_protocol_profiles_get(unit, gport, &protocol_profiles);
...
sh_sand_cmd_t sh_dnx_trap_protocol_cmds[] = {
     ...
     */
    {"PROFiles", sh_dnx_trap_protocol_profiles_cmd, NULL, dnx_trap_protocol_profiles_options, &dnx_trap_protocol_profiles_man}
    ,
