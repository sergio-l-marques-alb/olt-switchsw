...
static sh_sand_man_t dnx_trap_list_pd_man = {
    ...
    .full = "List info includes name,action index(HW id),snoop and forward strengths of configured predefined traps\n",
...
static sh_sand_man_t dnx_trap_list_ud_man = {
    ...
    .full = "List number of free and used user-defined traps\n",
...
static sh_sand_man_t dnx_trap_list_appl_man = {
    .brief = "List number of free and used application traps\n",
    .full = "List number of free and used application traps\n",
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_map_list
                    (unit, DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_OAM, &trap_map_list, &nof_traps));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int encoded_trap_id, trap_id = dnx_rx_ud_trap_map[trap_block].default_trap_id_list[0], trap_id_index = 0;
    ...
    {
        case 0:
            PRT_TITLE_SET("%s User-defined traps available", dnx_rx_ud_trap_map[trap_block].block_name);
            PRT_COLUMN_ADD("Trap id");
            PRT_COLUMN_ADD("Forward Action Profile");
            break;
        case 1:
            PRT_TITLE_SET("%s User-defined traps configured", dnx_rx_ud_trap_map[trap_block].block_name);
            PRT_COLUMN_ADD("Trap id");
            PRT_COLUMN_ADD("Forward Action Profile");
            while (trap_id != -1)
            {
                ...
                trap_id = dnx_rx_ud_trap_map[trap_block].default_trap_id_list[++trap_id_index];
            }
            break;
        case 2:
            PRT_TITLE_SET("%s User-defined traps", dnx_rx_ud_trap_map[trap_block].block_name);
            PRT_COLUMN_ADD("Trap id");
            PRT_COLUMN_ADD("Forward Action Profile");
            PRT_COLUMN_ADD("USED/FREE");
            while (trap_id != -1)
    ...
    for (trap_id = dnx_rx_ud_trap_map[trap_block].lower_bound_trap_id;
         trap_id <= dnx_rx_ud_trap_map[trap_block].upper_bound_trap_id; trap_id++)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_app_map_list(unit, trap_block, &trap_map_list, &nof_traps));
