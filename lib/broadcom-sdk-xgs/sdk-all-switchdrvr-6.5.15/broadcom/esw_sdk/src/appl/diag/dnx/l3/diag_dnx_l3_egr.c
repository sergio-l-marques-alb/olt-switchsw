...
static sh_sand_man_t Dnx_l3_egr_find_man = {
    .brief = "Find an L3 intface pointing to an Egress forwarding object - not supported",
    .full = "Not supported",
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_free_macro_a_type = 0;
    int nof_free_macro_b_type = 0;
    ...
    for (; start_id < end_id; start_id++)
    {
        ...
        nof_macro_a_type = 0;
        nof_macro_b_type = 0;
        nof_free_macro_a_type = 0;
        nof_free_macro_b_type = 0;
        ...
        for (idx = 0; idx < nof_clusters; idx++)
        {
             ...
             */
            for (cl_id = 0; cl_id < nof_cl_in_pair; cl_id++)
            {
                uint32 temp_hierarchy;
                bank_id =
                    DNX_ALGO_L3_PHYSICAL_FEC_GET(unit, clusters[idx + cl_id].logical_start_address * nof_cl_in_pair) /
                    dnx_data_l3.fec.nof_physical_fecs_per_bank_get(unit) + cl_id;

                SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_hierarchy.
                                fec_hierarchy_map.get(unit, bank_id, &temp_hierarchy));
             ...
             */
            if (cluster_is_free == nof_cl_in_pair)
            {
                if (clusters[idx].macro_type == MDB_MACRO_A)
                {
                    nof_free_macro_a_type++;
                }
                if (clusters[idx].macro_type == MDB_MACRO_B)
                {
                    nof_free_macro_b_type++;
                }
            }
            else if (addr_id != 0)
            {
        ...
        PRT_CELL_SET("%d (free %d)", nof_macro_a_type, nof_free_macro_a_type);
        PRT_CELL_SET("%d (free %d)", nof_macro_b_type, nof_free_macro_b_type);
        ...
        if (addr_id != 0)
        {
            for (idx = 0; idx < nof_clusters; idx++)
            {
                if (cluster_start_a[idx] != -1 || cluster_start_b[idx] != -1)
                {
                    ...
                    bank_id =
                        DNX_ALGO_L3_PHYSICAL_FEC_GET(unit, clusters[idx].logical_start_address * nof_cl_in_pair) /
                    ...
                    nof_elements_per_bank =
                        ALGO_L3_FEC_BANK_IS_BIG(bank_id) ? dnx_data_l3.
                        fec.max_fec_stage_map_size_get(unit) : dnx_data_l3.fec.fec_stage_map_size_get(unit);
                    if (bank_id % 5 != 0)
                    {
                        nof_free_elements -= 2;
                    }
                    ...
                    {
                        int nof_free_elem_in_pair;
                        int nof_elem_in_pair;
                        SHR_IF_ERR_EXIT(algo_l3_db.fec_alloc.l3_fec_res_mngr.
                                        fec_res_manager.nof_free_elements_get(unit, bank_id + 1,
                                                                              &nof_free_elem_in_pair));
                        nof_elem_in_pair =
                            ALGO_L3_FEC_BANK_IS_BIG(bank_id +
                                                    1) ? dnx_data_l3.fec.
                            max_fec_stage_map_size_get(unit) : dnx_data_l3.fec.fec_stage_map_size_get(unit) +
                            nof_elements_per_bank;
                        nof_free_elem_in_pair += nof_free_elements;
                        if ((bank_id + 1) % 5 != 0)
                        {
                            nof_free_elem_in_pair -= 2;
                        }
                        PRT_CELL_SET("Range %d to %d -> %d free", clusters[idx].logical_start_address * nof_cl_in_pair,
                                     clusters[idx].logical_start_address * nof_cl_in_pair + nof_elem_in_pair - 1,
                                     nof_free_elem_in_pair);
                        PRT_CELL_SKIP(1);
                    }
                    else
                    {
                        PRT_CELL_SKIP(1);
                        PRT_CELL_SET("Range %d to %d (%d free)", clusters[idx].logical_start_address * nof_cl_in_pair,
                                     clusters[idx].logical_start_address * nof_cl_in_pair + nof_elements_per_bank - 1,
                                     nof_free_elements);
                    }
                }
                idx++;
