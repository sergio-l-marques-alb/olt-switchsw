...
{
    int core, index;
    dnx_ingress_congestion_statistics_info_t stats[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int is_sys_red_enabled, sys_red_idx;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_cosq_fadt_info_t dram_bound_info, congestion_notification_info;
         ...
         resource++)
    {
        for (dp = 0; dp < DNX_COSQ_NOF_DP; dp++)
        {
            SHR_IF_ERR_EXIT(bcm_cosq_gport_color_size_get(unit, rate_class_gport, 0,
                                                          dp, resource_flags[resource],
    ...
    /** Get Dram Bounds */
    for (thresh = DNX_INGRESS_CONGESTION_DRAM_BOUND; thresh < DNX_INGRESS_CONGESTION_DRAM_BOUND_THR_TYPE_NOF; thresh++)
    {
        bcm_cosq_fadt_info_t_init(&dram_bound_info);
        dram_bound_info.gport = rate_class_gport;
        dram_bound_info.cosq = 0;
        dram_bound_info.thresh_type =
            (thresh == DNX_INGRESS_CONGESTION_DRAM_BOUND ? bcmCosqFadtDramBound : bcmCosqFadtDramBoundRecoveryFailure);

        dram_bound_info.resource_type = bcmResourceOcbBytes;
        SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_get(unit, 0, &dram_bound_info,
                                                          &rate_class_info->dram_bound_ocb_bytes_threshold[thresh]));

        dram_bound_info.resource_type = bcmResourceOcbBuffers;
        SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_get(unit, 0, &dram_bound_info,
                                                          &rate_class_info->dram_bound_ocb_buffers_threshold[thresh]));

        dram_bound_info.resource_type = bcmResourceOcbPacketDescriptors;
        SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_get(unit, 0, &dram_bound_info,
                                                          &rate_class_info->dram_bound_ocb_pds_threshold[thresh]));
    }

    bcm_cosq_threshold_t_init(&dram_bound_recovery_threshold);
    ...
    rate_class_info->dram_bound_recovery_threshold = dram_bound_recovery_threshold.value;
         ...
         resource++)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_cosq_gport_fadt_threshold_get(unit, 0, &congestion_notification_info,
                                                          &rate_class_info->congestion_notification_threshold[resource]));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (poll_idx = 0; poll_idx < poll_nof_times; poll_idx++)
    {
        ...
        DNXCMN_CORES_ITER(unit, core_id, core)
        {
            ...
            /** Skip all VOQs that have ZERO ocuupancy (field vlaues are
             *  0 by default) */
            ...
            /*
             * Go over all entries differnet than default (0) { 
             */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_vsq_programmable_counter_configuration_clear(unit, core));
    }
    ...
    prt_control_t * prt_ctr)
{
    ...
    else
    {
        ...
        for (resource = 0; resource < DNX_INGRESS_CONGESTION_RESOURCE_NOF; resource++)
        {
            PRT_CELL_SET("%u", current_vsq_occupancy_info.resources[resource].guaranteed);
            PRT_CELL_SET("%u", current_vsq_occupancy_info.resources[resource].shared);
            PRT_CELL_SET("%u", current_vsq_occupancy_info.resources[resource].headroom);
    ...
    for (poll_idx = 0; poll_idx < poll_nof_times; poll_idx++)
    {
        ...
        DNXCMN_CORES_ITER(unit, core_id, core)
        {
            ...
            else
            {
                ...
                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Guaranteed [Buffers]");
                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Shared [Buffers]");
                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Headroom [Buffers]");
                if (vsq_group == DNX_INGRESS_CONGESTION_VSQ_GROUP_PG)
                {
                    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Headroom Extension [Buffers]");
                }
                PRT_COLUMN_ADD("");

                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Guaranteed [PDS]");
                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Shared [PDS]");
                PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OCB Headroom [PDS]");
                if (vsq_group == DNX_INGRESS_CONGESTION_VSQ_GROUP_PG)
                {
                    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Headroom Extension [PDS]");
                }
                PRT_COLUMN_ADD("");
...
/**
 * \brief - Print a rate class info
 */
static shr_error_e
dnx_diag_scheduler_vsq_rate_class_print(
    int unit,
    int core,
    int vsq_group,
    int vsq_group_idx,
 ...
 */
static shr_error_e
dnx_diag_scheduler_source_vsq_rate_class_resource_allocation_info_print(
    int unit,
    int core,
             ...
             resource++)
        {
            for (color_idx = bcmColorGreen; color_idx < bcmColorCount - 1; color_idx++)
            {
                ...
                else
                {
                    ...
                    PRT_CELL_SET("%u", amounts.shared_pool_fadt.alpha);
 ...
 */
static shr_error_e
dnx_diag_scheduler_dest_vsq_rate_class_drop_info_print(
    int unit,
    int core,
    bcm_gport_t vsq_gport,
    int vsq_group,
 ...
 */
static shr_error_e
dnx_diag_scheduler_dest_vsq_rate_class_wred_info_print(
    int unit,
    int core,
    bcm_gport_t vsq_gport,
    int vsq_group,
 ...
 */
static shr_error_e
dnx_diag_scheduler_vsq_occupancy_info_print(
    int unit,
    int core,
    int vsq_group,
    int vsq_group_idx,
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_iterator_key_field32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_CORE_ID, DBAL_CONDITION_EQUAL_TO, (uint32) core, 0));
    SHR_IF_ERR_EXIT(dbal_iterator_key_field32_rule_add
                    (unit, entry_handle_id, DBAL_FIELD_VSQ_ID, DBAL_CONDITION_EQUAL_TO, (uint32) vsq_group_idx, 0));
    ...
    while (!is_end)
    {
        ...
        for (resource = 0; resource < DNX_INGRESS_CONGESTION_RESOURCE_NOF; resource++)
        {
            ...
            {
                /** For BOTH GROUP E and GROUP F */
                PRT_CELL_SET("%u", vsq_occupancy_info.resources[resource].guaranteed);
                PRT_CELL_SET("%u", vsq_occupancy_info.resources[resource].shared);
                PRT_CELL_SET("%u", vsq_occupancy_info.resources[resource].headroom);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        /** VSQ Rate Class Print   */
        SHR_IF_ERR_EXIT(dnx_diag_scheduler_vsq_rate_class_print(unit, core, vsq_group, vsq_group_idx, sand_control));

        /** Only for Source Based VSQs */
        if (vsq_group >= DNX_INGRESS_CONGESTION_VSQ_GROUP_SRC_PORT)
        {
            /**VSQ Resource Allocation info*/
            SHR_IF_ERR_EXIT(dnx_diag_scheduler_source_vsq_rate_class_resource_allocation_info_print
        ...
        else
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_diag_scheduler_dest_vsq_rate_class_drop_info_print
                            (unit, core, vsq_gport, vsq_group, vsq_group_idx, sand_control));
        }

        /** VSQ WRED info print */
        SHR_IF_ERR_EXIT(dnx_diag_scheduler_dest_vsq_rate_class_wred_info_print
                        (unit, core, vsq_gport, vsq_group, vsq_group_idx, sand_control));

        /** VSQ occupancy info print */
        SHR_IF_ERR_EXIT(dnx_diag_scheduler_vsq_occupancy_info_print
                        (unit, core, vsq_group, vsq_group_idx, sand_control));
    }
