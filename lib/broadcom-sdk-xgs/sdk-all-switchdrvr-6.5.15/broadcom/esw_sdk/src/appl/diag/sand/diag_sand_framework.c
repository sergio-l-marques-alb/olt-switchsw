    ...
    int flags)
{
    ...
    {
        /*
         * No Command line options provided, all arguments stay on default
         */
        SHR_EXIT();
    }
    /*
        ...
        switch (realtokens)
        {
            ...
            case 1:
                                                      ...
                                                      sand_ctr->dyn_args_list)) == NULL)
                {
                    ...
                    /** Found - great - no need to look any more */
                    if (sh_sand_arg != NULL)
                        break;
    ...
    rhlist_t * legacy_list)
{
    ...
    {   /** If not found in system list look in local one - not found - error   */
        ...
        if (!SHR_FUNC_VAL_IS(_SHR_E_NONE))
        {
            SHR_CLI_EXIT(_SHR_E_NOT_FOUND, "ERROR: command \"%s\" is not supported\n", cmd_name);
        }
    }
    else
    {   /* 
    ...
    else if (sh_sand_cmd->action)
    {
        ...
        SHR_SET_CURRENT_ERR(_SHR_E_NONE);
        SHR_CLI_EXIT_IF_ERR(sh_sand_option_list_process(unit, args, sh_sand_cmd, &sand_ctr_m, sh_sand_cmd->flags), "");
        SHR_CLI_EXIT_IF_ERR(sh_sand_cmd->action(unit, args, &sand_ctr_m), "");
    }
    else
    {
    ...
    int flags)
{
    ...
    {   /* Verify that every command is either leaf or branch */
        ...
        SHR_CLI_EXIT_IF_ERR(sh_sand_keyword_fetch(unit, command, sh_sand_cmd->keyword, &sh_sand_cmd->short_key,
                            ...
                            command);
        SH_SAND_CMD_ITERATOR(sh_sys_cmd, sh_sys_cmd_a)
        {
    ...
    args_t * args)
{
    ...
    SHR_SET_CURRENT_ERR(sh_sand_cmd_get(unit, cmd_name, sh_sand_cmd_a, &sh_sand_cmd, 1));
    /*
     * If not found check vs SKIP_ROOT commands
     */
    if (SHR_FUNC_VAL_IS(_SHR_E_NOT_FOUND))
    {
        for (sh_sand_cmd = sh_sand_cmd_a; sh_sand_cmd->keyword != NULL; sh_sand_cmd++)
        {
            ...
            {
                sh_sand_cmd_t *sh_sand_cmd_tmp = NULL;
                SHR_SET_CURRENT_ERR(sh_sand_cmd_get(unit, cmd_name, sh_sand_cmd->child_cmd_a, &sh_sand_cmd_tmp, 1));
                if (SHR_FUNC_VAL_IS(_SHR_E_NONE))
                {
                    /*
                     * Command found
                ...
                else if (sh_sand_cmd->legacy_list != NULL)
                {
                    /*
                     * Check in legacy list if exists
                     */
                    ARG_NEXT(args);
                    SHR_SET_CURRENT_ERR(sh_sand_legacy_fetch(unit, args, cmd_name, sh_sand_cmd->legacy_list));
                    if (SHR_FUNC_VAL_IS(_SHR_E_NONE))
                    {
                        SHR_EXIT();
                    }
    ...
    if (SHR_FUNC_VAL_IS(_SHR_E_NONE))
    {
        if (sh_sand_cmd->child_cmd_a != NULL)
        {
            SHR_SET_CURRENT_ERR(sh_sand_act
                                (unit, args, sh_sand_cmd->child_cmd_a, (sh_sand_cmd_t *) sh_sand_cmd->invokes,
                                 sh_sand_cmd->legacy_list));
        }
        else
        {
            SHR_CLI_EXIT(_SHR_E_INTERNAL, "ERROR: Root command:\"%s\" is not branch\n", cmd_name);
        }
    }
    else
    {
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "ERROR: Command:\"%s\" not found\n", cmd_name);
    }
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
#ifdef BCM_WARM_BOOT_API_TEST
    /* disabling wb regression so we don't perform reboot in the middle of a diagnostic
       doing reboot in middle of diagnostic may lead to loss of state that is required
       for the diagnostic to succeed */
    int wb_test_mode = 0;
    ...
    sh_sand_legacy_cmd_t * sh_sand_legacy_cmd_a)
{
    ...
    for (sh_sand_cmd = sh_root_commands; sh_sand_cmd->keyword != NULL; sh_sand_cmd++)
    {
        if (sh_sand_cmd->child_cmd_a == NULL)
        {
...
#ifdef INCLUDE_AUTOCOMPLETE
    {
        ...
        char option_text[AUTOCOMPLETE_MAX_STRING_LEN] = { 0 };
        autocomplete_node_t *access_read_node = autocomplete_find_node(unit, keywords);

#if defined(DNX_SW_STATE_LOGGING)
        int module_id;
        int sw_state_func_type;
        char *sw_state_log_start_keywords[5] = { "swstate", "logger", "start", "module_id=", NULL };
        char *sw_state_log_stop_keywords[5] = { "swstate", "logger", "stop", "module_id=", NULL };
        char *sw_state_log_start_by_function_keywords[5] = { "swstate", "logger", "start", "function_type=", NULL };
        char *sw_state_log_stop_by_function_keywords[5] = { "swstate", "logger", "stop", "function_type=", NULL };

        char *sw_state_log_add_keywords[5] = { "swstate", "logger", "add", "module_id=", NULL };
        char *sw_state_log_remove_keywords[5] = { "swstate", "logger", "remove", "module_id=", NULL };
        char *sw_state_log_add_by_function_keywords[5] = { "swstate", "logger", "add", "function_type=", NULL };
        char *sw_state_log_remove_by_function_keywords[5] = { "swstate", "logger", "remove", "function_type=", NULL };
        char *sw_state_log_print_keywords[5] = { "swstate", "logger", "print", "module_id=", NULL };
        char *sw_state_log_print_by_function_keywords[5] = { "swstate", "logger", "print", "function_type=", NULL };

        char opt_text[AUTOCOMPLETE_MAX_STRING_LEN] = { 0 };
        char sw_state_log_by_func_text[AUTOCOMPLETE_MAX_STRING_LEN] = { 0 };

        autocomplete_node_t *sw_state_log_start_node = autocomplete_find_node(unit, sw_state_log_start_keywords);
        autocomplete_node_t *sw_state_log_stop_node = autocomplete_find_node(unit, sw_state_log_stop_keywords);
        autocomplete_node_t *sw_state_log_func_start_node =
            autocomplete_find_node(unit, sw_state_log_start_by_function_keywords);
        autocomplete_node_t *sw_state_log_func_stop_node =
            autocomplete_find_node(unit, sw_state_log_stop_by_function_keywords);

        autocomplete_node_t *sw_state_log_add_node = autocomplete_find_node(unit, sw_state_log_add_keywords);
        autocomplete_node_t *sw_state_log_remove_node = autocomplete_find_node(unit, sw_state_log_remove_keywords);
        autocomplete_node_t *sw_state_log_func_add_node =
            autocomplete_find_node(unit, sw_state_log_add_by_function_keywords);
        autocomplete_node_t *sw_state_log_func_remove_node =
            autocomplete_find_node(unit, sw_state_log_remove_by_function_keywords);
        autocomplete_node_t *sw_state_log_print_node = autocomplete_find_node(unit, sw_state_log_print_keywords);
        autocomplete_node_t *sw_state_log_func_print_node =
            autocomplete_find_node(unit, sw_state_log_print_by_function_keywords);
        ...
        {
            sal_strncpy(opt_text, dnxc_module_id_e_get_name(module_id),
                        AUTOCOMPLETE_MAX_STRING_LEN - sal_strlen(opt_text) - 1);
            autocomplete_node_add(unit, sw_state_log_start_node, opt_text, 0);
            autocomplete_node_add(unit, sw_state_log_stop_node, opt_text, 0);
            autocomplete_node_add(unit, sw_state_log_add_node, opt_text, 0);
            autocomplete_node_add(unit, sw_state_log_remove_node, opt_text, 0);
            autocomplete_node_add(unit, sw_state_log_print_node, opt_text, 0);
        }

        if (sw_state_log_func_start_node == NULL)
        {
            cli_out("swstate logger start function_type= command's autocomplete node not found\n");
            goto exit;
        }

        if (sw_state_log_func_stop_node == NULL)
        {
            cli_out("swstate logger stop function_type= command's autocomplete node not found\n");
            goto exit;
        }

        for (sw_state_func_type = 0; sw_state_func_type < DNX_SW_STATE_COUNT_LOGGING; sw_state_func_type++)
        {
            sal_strncpy(sw_state_log_by_func_text, dnxc_fuction_type_e_get_name(sw_state_func_type),
                        AUTOCOMPLETE_MAX_STRING_LEN - sal_strlen(sw_state_log_by_func_text) - 1);
            autocomplete_node_add(unit, sw_state_log_func_start_node, sw_state_log_by_func_text, 0);
            autocomplete_node_add(unit, sw_state_log_func_stop_node, sw_state_log_by_func_text, 0);
            autocomplete_node_add(unit, sw_state_log_func_add_node, sw_state_log_by_func_text, 0);
            autocomplete_node_add(unit, sw_state_log_func_remove_node, sw_state_log_by_func_text, 0);
            autocomplete_node_add(unit, sw_state_log_func_print_node, sw_state_log_by_func_text, 0);
        }
#endif /* DNX_SW_STATE_LOGGING */
    }
#endif /* INCLUDE_AUTOCOMPLETE */
