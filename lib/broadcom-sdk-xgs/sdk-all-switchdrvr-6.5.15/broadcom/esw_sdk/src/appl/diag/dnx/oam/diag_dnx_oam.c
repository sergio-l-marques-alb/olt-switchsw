...
#define MAX_NOF_DATABASES 22
#define MAX_NOF_ENGINES 22
...
typedef struct
{
    ...
    int counter_base;
    int is_pcp;
    unsigned long long ing_counter_value[8];
    unsigned long long eg_counter_value[8];
} endpoint_results_t;
    ...
    oam_cf_inputs_t * oam_cf_inputs)
{
    ...
    sand_signal_list_get(unit, core, &match_m, dsig_list);

    RHITERATOR(signal_output, dsig_list)
    {
        debug_signal = signal_output->debug_signal;

        if (sal_strcmp(debug_signal->attribute, "OAM_LIF") == 0)
        {
            oam_cf_inputs->lif = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "VTT_OAM_LIF_0") == 0)
        {
            oam_cf_inputs->lif_1 = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "VTT_OAM_LIF_1") == 0)
        {
            oam_cf_inputs->lif_2 = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "VTT_OAM_LIF_2") == 0)
        {
            oam_cf_inputs->lif_3 = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "Mdl") == 0)
        {
            oam_cf_inputs->mdl = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "OAM_Opcode") == 0)
        {
            oam_cf_inputs->opcode = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "OAM_Your_Discr") == 0)
        {
            oam_cf_inputs->ydv = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "Pkt_is_BFD") == 0)
        {
            oam_cf_inputs->pkt_is_bfd = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "My_CFM_MAC") == 0)
        {
            oam_cf_inputs->mymac = signal_output->value[0];
        }
        if (sal_strcmp(debug_signal->attribute, "Pkt_is_Compatible_MC") == 0)
        {
            oam_cf_inputs->da_is_mc = signal_output->value[0];
        }
    }
    ...
    oam_cf_results_t * oam_cf_results)
{
    ...
    if (oam_cf_inputs.ydv == 0)
    {
        ...
        else
        {
            ...
            rv = dnx_lif_mngr_outlif_sw_info_table_id_and_result_type_get(unit, local_lif, &dbal_table_id,
    ...
    if ((oam_cf_inputs.ydv == 0) && (oam_cf_inputs.pkt_is_bfd == 0) && (dbal_table_id == DBAL_TABLE_IN_AC_INFO_DB))
    {
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_IN_AC_INFO_DB, &entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, local_lif);
        ...
        if (oam_cf_inputs.lif_2 != DNX_OAM_INVALID_LIF)
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, local_lif2);
        ...
        if (oam_cf_inputs.lif_3 != DNX_OAM_INVALID_LIF)
        {
            dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, local_lif3);
            SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_OAM_LIF_SET, INST_SINGLE, &temp_inlif_valid4lm));
            oam_cf_results->nof_lm_lifs += temp_inlif_valid4lm;
    ...
    oam_cf_results_t * oam_cf_results)
{
    ...
    if (oam_cf_results->lif_1_entry_found == 1)
    {
        ...
        oam_cf_results->oam_1_key = ((action_key.mp_type & 0x7) << 11) |
    ...
    else
    {
        ...
        oam_cf_results->oam_2_key = (packet_is_bfd & 0x1) |
            ((action_key.is_inject & 0x1) << 1) |
            ((action_key.is_my_mac & 0x1) << 2) |
            ((internal_opcode & 0xf) << 3) | ((action_key.mp_profile & 0xf) << 7) | ((action_key.da_is_mc & 0x1) << 11);
        SHR_IF_ERR_EXIT(dnx_oam_action_get(unit, flags, &action_key, &action_content));
        oam_cf_results->trap_strength = action_content.forwarding_strength;
        oam_cf_results->oam_subtype = action_content.oam_sub_type;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (oam_cf_inputs.last_run == 1)
    {
        SHR_IF_ERR_EXIT(diag_oam_cf_read_signals(unit, 0, &oam_cf_inputs));
    }
    /*
     *  Verify inputs
     */
 ...
 * \param [in] unit -     Relevant unit.
 * \param [in] sand_control -
 * \param [in] core_low -minimal core id
 * \param [in] core_high -maximal core id
 *
 * \retval Error indication
 *
 */
    ...
    int core_high)
{
    ...
    PRT_TITLE_SET("OAMP-PE last packet diagnotics:");
    ...
    for (core_idx = core_low; core_idx <= core_high; core_idx++)
    {
        ...
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("Last program selecated");
        PRT_CELL_SET("%s", prog_last_name);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("Last TCAM selecated");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Determine if the user passed a correct core value. If a specific correct core was asked, iterate on it only, otherwise on 0,max */
...
/* In cmodel force to use core_id = 0 */
    ...
    core_high = 0;
#else
    if (core_id >= 0 && core_id <= dnx_data_device.general.nof_cores_get(unit) - 1)
    {
        core_low = core_id;
        core_high = core_id;
    }
    ...
    if (last || all)
    {
         ...
         */
        SHR_IF_ERR_EXIT(diag_oamp_pe_last_packet(unit, sand_control, core_low, core_high));
    }
            ...
            case SHORT_TYPE:
            {
                ...
                else if (maid_type == TWO_BYTE_TYPE)
                {
                    ...
                    else
                    {
                         ...
                         */
                        mep_id =
                            ((nof_mep_db_short_entries * mep_index) / DNX_OAM_NUM_OF_SHORT_ENTRIES_IN_MEP_DB_LINE) + 1;
                    }
                }
                else
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_IN_HW)
    {
        ...
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("    %s", "Local diagnostic");
        ...
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("    %s", "Remote diagnostic");
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", "    UDP source port");
            PRT_CELL_SET("0x%x", oam_bfdep_results->endpoint_info.udp_src_port);

            if ((oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_MULTIHOP) == 0)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                ...
                {
                    PRT_CELL_SET(IPV4_PRINT_FMT, IPV4_PRINT_ARG(oam_bfdep_results->endpoint_info.src_ip_addr));
                }
            }
        }
        if (oam_bfdep_results->endpoint_info.type == bcmBFDTunnelTypeUdp)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", "    Destination IP address");
            PRT_CELL_SET(IPV4_PRINT_FMT, IPV4_PRINT_ARG(oam_bfdep_results->endpoint_info.dst_ip_addr));

            if (oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_MULTIHOP)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "    IP TTL");
                PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.ip_ttl);

                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("%s", "    IP TOS");
                PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.ip_tos);
            }
        }
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%s", "    Remote Flags");
        PRT_CELL_SET("0x%x", oam_bfdep_results->endpoint_info.remote_flags);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%s", "    Local Flags");
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", "    Remote detection explicit timeout");
            PRT_CELL_SET("%u", oam_bfdep_results->endpoint_info.bfd_detection_time);
        }
        else
        {
            PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            PRT_CELL_SET("%s", "    Remote detection multiplier");
    ...
    oam_cntrs_results_t * oam_cntrs_results)
{
     ...
     * Check all databases
     */
    for (database_idx = 0; database_idx < 22; database_idx++)
    {
        database.database_id = database_idx;
        database.core_id = core;
        oam_cntrs_results->database[database_idx].is_oam = 0;

        rv = bcm_stat_counter_interface_get(unit, flags, &database, &config);
        if (rv != _SHR_E_NOT_FOUND)
        {
            SHR_IF_ERR_EXIT(rv);
    ...
    if (is_acc)
    {
        ...
        if (rmep_index > 0)
        {
            ...
            for (rmep_no = 0; rmep_no < rmep_cnt; rmep_no++)
            {
                ...
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                PRT_CELL_SET("   ID");
                PRT_CELL_SET("%d\n   LOC %d\n   RDI %d\n   IF State %d\n   Port State %d", rmep_list[rmep_no], loc, rdi,
                             rmep_info.interface_state, rmep_info.port_state);
            }
        }
    }
    else
    ...
    oam_cntrs_results_t * oam_cntrs_results)
{
     ...
     */
    SHR_IF_ERR_EXIT(soc_mem_array_read(unit, counter_bank, ing_engine, MEM_BLOCK_ANY, ing_index, &ing_entry_above_64));
    SHR_IF_ERR_EXIT(soc_mem_array_read(unit, counter_bank, eg_engine, MEM_BLOCK_ANY, eg_index, &eg_entry_above_64));

    /*
     * Prepare values of pcp=0 counter. If no-pcp, this is the only value needed.
     */
    temp_64 = ing_entry_above_64[1];
    oam_cntrs_results->endpoint_results.ing_counter_value[0] = ing_entry_above_64[0] + ((temp_64 & 0x3) << 32);
    temp_64 = eg_entry_above_64[1];
    oam_cntrs_results->endpoint_results.eg_counter_value[0] = eg_entry_above_64[0] + ((temp_64 & 0x3) << 32);
    ...
    if (oam_cntrs_results->endpoint_results.is_pcp)
    {
        for (pcp_index = 1; pcp_index < 4; pcp_index++)
        {
            ...
            temp_64 = ing_entry_above_64[pcp_index + 1];
            oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index] =
                (ing_entry_above_64[pcp_index] >> (pcp_index * 2)) + ((temp_64 & mask) << 32);

            temp_64 = eg_entry_above_64[pcp_index + 1];
            oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index] =
                (eg_entry_above_64[pcp_index] >> (pcp_index * 2)) + ((temp_64 & mask) << 32);
        }
        SHR_IF_ERR_EXIT(soc_mem_array_read
                        (unit, counter_bank, ing_engine, MEM_BLOCK_ANY, ing_index + 1, &ing_entry_above_64));
        SHR_IF_ERR_EXIT(soc_mem_array_read
                        (unit, counter_bank, eg_engine, MEM_BLOCK_ANY, eg_index + 1, &eg_entry_above_64));
        for (pcp_index = 0; pcp_index < 4; pcp_index++)
        {
            ...
            temp_64 = ing_entry_above_64[pcp_index + 1];
            oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index + 4] =
                (ing_entry_above_64[pcp_index] >> (pcp_index * 2)) + ((temp_64 & mask) << 32);

            temp_64 = eg_entry_above_64[pcp_index + 1];
            oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index + 4] =
                (eg_entry_above_64[pcp_index] >> (pcp_index * 2)) + ((temp_64 & mask) << 32);
        }
    }
exit:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while (!is_ep_end)
    {
        ...
        if ((ep == id) || (id == -1))
        {
             ...
             */
            SHR_IF_ERR_EXIT(diag_oam_ep_display_single_endpoint_cntrs(unit, ep, oam_cntrs_results));
            ...
            PRT_CELL_SET("%llu", oam_cntrs_results->endpoint_results.ing_counter_value[0]);
            for (pcp_index = 1; pcp_index < 8; pcp_index++)
            {
                if (oam_cntrs_results->endpoint_results.is_pcp)
                {
                    PRT_CELL_SET("%llu", oam_cntrs_results->endpoint_results.ing_counter_value[pcp_index]);
                }
                else
                {
                    PRT_CELL_SET("--");
            ...
            PRT_CELL_SET("%d", oam_cntrs_results->endpoint_results.counter_if);
            PRT_CELL_SET("%d", oam_cntrs_results->endpoint_results.counter_base);
            PRT_CELL_SET("%llu", oam_cntrs_results->endpoint_results.eg_counter_value[0]);
            for (pcp_index = 1; pcp_index < 8; pcp_index++)
            {
                if (oam_cntrs_results->endpoint_results.is_pcp)
                {
                    PRT_CELL_SET("%llu", oam_cntrs_results->endpoint_results.eg_counter_value[pcp_index]);
                }
                else
                {
                    PRT_CELL_SET("--");
    ...
    sh_sand_control_t * sand_control)
{
     ...
     * Display all requested endpoints and gather statistics information
     */
    SHR_IF_ERR_EXIT(diag_oam_cntrs_display_all(unit, oam_cntrs_inputs.ep_id, &oam_cntrs_results, sand_control));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        PRT_TITLE_SET("Expected OAM_ID \033[91mDOESN'T MATCH\033[0m actual");
    }
    else
    {
        PRT_TITLE_SET("Expected OAM_ID \033[92mMATCH\033[0m actual");
    }
...
shr_error_e init_oam(int unit)
{
   ...
   vp3.flags = BCM_VLAN_PORT_CROSS_CONNECT;
...
static sh_sand_man_t sh_dnx_oam_cf_man =
{
    ...
    .full   = "Presents OAM Classifier outcomes for a given set of inputs.\n \
                * lif is mandatory input.\n \
                * All other inputs are optional. If optional input was not supplied, default value will be used.\n \
                * Keys of HW tables that result a HIT and its result fields will be lid.\n \
                * Counters that are enabled will be lid.",
    .synopsis = NULL,
    .examples = "lif=0x1000\n"
...
{
    .brief  = "OAM EndPoint Counters Diagnostic",
    .full   = "Presents Counters' IDs and values of configured OAM Endpoints.\n \
...
static sh_sand_man_t sh_dnx_oam_grp_man =
{
    ...
    .full   = "Presents Configured OAM Groups their properties, and the endpoints\n \
                * allocated on that group.\n \
...
sh_sand_cmd_t sh_dnx_oam_cmds[] =
{
    ...
    {"OAMP_CounTeRS", sh_dnx_oamp_ctrs_cmd,NULL,                    NULL,                       &sh_dnx_oamp_ctrs_man, NULL, NULL},
    {"OAMP_PE",     sh_dnx_oamp_pe_cmd,    NULL,                    dnx_oamp_pe_list_options,   &sh_dnx_oamp_pe_man, NULL, NULL},
    {"Oamp_Mep_DB", NULL,                  sh_dnx_oamp_mep_db_cmds, NULL,                       &sh_dnx_oamp_mep_db_man, NULL, NULL},
