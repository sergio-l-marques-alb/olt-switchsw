...
static sh_sand_man_t Field_entry_list_man = {
    ...
    .examples = "group=20 core=all",
};
static sh_sand_option_t Field_entry_list_options[] = {
    {DIAG_DNX_FIELD_OPTION_GROUP,  SAL_FIELD_TYPE_UINT32, "Field group to get its entries",       NULL,       (void *)Field_group_enum_table},
    {NULL}
};
...
static sh_sand_man_t Field_entry_info_man = {
    ...
    .examples = "group=50 entry=3276803",
};
static sh_sand_option_t Field_entry_info_options[] = {
    {DIAG_DNX_FIELD_OPTION_GROUP,          SAL_FIELD_TYPE_UINT32, "Field group to get its entry info",           NULL   ,(void *)Field_group_enum_table},
            ...
            if (SHR_BITGET(banks_bmp, bank_id_iter))
            {
                ...
                while (entry_iter != DNX_FIELD_ENTRY_ACCESS_ID_INVALID)
                {
                    ...
                    if ((entry_ids_string_length + tmp_buff_length) < RHSTRING_MAX_SIZE)
                    {
                        ...
                        sal_strncat(entry_ids_string, ent_id_buff, sizeof(entry_ids_string) - 1);
                    }
                    else
                    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(kbp_mngr_db_id_by_name_get(unit, (char *) table->table_name, &db_id));
    ...
    if (kbp_mngr_status == DBAL_ENUM_FVAL_KBP_DEVICE_STATUS_LOCKED)
    {
        ...
        while (!is_end)
        {
            ...
            DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL,
                                                  "0x%08X, ", DNX_FIELD_ENTRY(DNX_FIELD_TCAM_EXTERNAL, fg_id, access_id,
                                                                              0));
            sal_snprintf(ent_id_buff, sizeof(ent_id_buff), "0x%08X, ",
                         DNX_FIELD_ENTRY(DNX_FIELD_TCAM_EXTERNAL, fg_id, access_id, 0));
            entry_ids_string_length = sal_strlen(entry_ids_string);
            tmp_buff_length = sal_strlen(ent_id_buff);
            if ((entry_ids_string_length + tmp_buff_length) < RHSTRING_MAX_SIZE)
            {
        ...
        for (bank_id_iter = 0; bank_id_iter < DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS; bank_id_iter++)
        {
            ...
            if (SHR_BITGET(banks_bmp, bank_id_iter))
            {
                ...
                for (entry_iter = 0; entry_iter < max_nof_dt_entries; entry_iter++)
                {
                    ...
                    entry_handle = DNX_FIELD_TCAM_ENTRY(fg_id, entry_iter, core_index);
    ...
    prt_control_t * prt_ctr)
{
    ...
    if (qual_valid == TRUE)
    {
        ...
        for (qual_val_mask_iter = DNX_DATA_MAX_FIELD_ENTRY_NOF_QUAL_PARAMS_PER_ENTRY - 1; qual_val_mask_iter >= 0;
             qual_val_mask_iter--)
        {
            char qual_val_mas_buff[DIAG_DNX_FIELD_UTILS_STR_SIZE];
            if (entry_info_p->key_info.qual_info[qual_iter].qual_value[qual_val_mask_iter] == 0 &&
                entry_info_p->key_info.qual_info[qual_iter].qual_mask[qual_val_mask_iter] == 0)
            {
                continue;
            }
            ...
            sal_strncat(qual_val_string, qual_val_mas_buff, sizeof(qual_val_string) - 1);
        ...
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(entry_info_p->key_info.qual_info[qual_iter].qual_value,
                                                       *start_bit_p,
        ...
        SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
                        (entry_info_p->key_info.qual_info[qual_iter].qual_mask, *start_bit_p,
    ...
    prt_control_t * prt_ctr)
{
    ...
    if (do_cell_set)
    {
        ...
        for (act_val_iter = DNX_DATA_MAX_FIELD_ENTRY_NOF_ACTION_PARAMS_PER_ENTRY - 1; act_val_iter >= 0; act_val_iter--)
        {
            ...
            sal_strncat(action_val_string, act_val_buff, sizeof(action_val_string) - 1);
        }
    ...
    sh_sand_control_t * sand_control)
{
         ...
         qual_val_mask_iter >= 0; qual_val_mask_iter--)
    {
        ...
        sal_strncat(qual_raw_val_string, qual_val_mask_buff, sizeof(qual_raw_val_string) - 1);
        ...
        sal_strncat(qual_raw_mask_string, qual_val_mask_buff, sizeof(qual_raw_mask_string) - 1);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 entry_handle_id;
    int is_end;
    int key_field_iter, fg_qual_iter;
    ...
    while (!is_end)
    {
        ...
        for (key_field_iter = (entry_handle->nof_key_fields - 1); key_field_iter >= 0; key_field_iter--)
        {
            /** Set the qualifier type, in key info, as it is in the field group. */
            entry_key.qual_info[fg_qual_iter].dnx_qual = fg_info_p->group_basic_info.dnx_quals[fg_qual_iter];

            /** Get the value of the current qualifier. */
            SHR_IF_ERR_EXIT(dbal_entry_handle_key_field_arr32_get(unit, entry_handle_id,
                                                                  entry_handle->key_field_ids[key_field_iter],
                                                                  entry_key.qual_info[fg_qual_iter].qual_value));
            fg_qual_iter++;
        }

        /** Assign the last element of the key info qualifier array to be invalid. */
        if (fg_qual_iter < DNX_DATA_MAX_FIELD_GROUP_NOF_QUALS_PER_FG)
        {
            entry_key.qual_info[fg_qual_iter].dnx_qual = DNX_FIELD_QUAL_TYPE_INVALID;
        }

        SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
        SHR_IF_ERR_EXIT(dnx_field_entry_exem_get(unit, fg_id, &entry_key, &entry_info));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Get information about the given Entry. */
    SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
    SHR_SET_CURRENT_ERR(dnx_field_entry_tcam_get(unit, fg_id, entry_id, &entry_info));
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Get information about the given Entry. */
    SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
    SHR_SET_CURRENT_ERR(dnx_field_entry_kbp_get(unit, entry_id, &entry_info));
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
    {
        LOG_CLI_EX("\r\n" "The given entry %d (0x%08X) is invalid.\r\n%s%s", entry_id, entry_id, EMPTY, EMPTY);
        SHR_EXIT();
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fg_info->group_basic_info.fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_TCAM:
        {
            ...
            fg_id_returned = DNX_FIELD_ENTRY_FG_ID(entry_id);
