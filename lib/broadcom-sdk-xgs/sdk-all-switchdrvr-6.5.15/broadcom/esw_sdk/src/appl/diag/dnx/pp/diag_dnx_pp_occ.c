    ...
    char *ingress_bits)
{
    ...
    for (bit = 0; bit < NOF_IN_LIF_PROFILE_INGRESS_BITS + 1; bit++)
    {
        if (bit < NOF_IN_LIF_PROFILE_INGRESS_BITS)
        {
            ingress_bits[bit] = 'x';
        }
        else
    ...
    for (bit = 0; bit < IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS + 1; bit++)
    {
        if (bit < IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS)
        {
            egress_bits[bit] = 'x';
        }
        else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char buffer[DBAL_MAX_PRINTABLE_BUFFER_SIZE], acl_bits[ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS + 1],
        ingress_bits[NOF_IN_LIF_PROFILE_INGRESS_BITS + 1], egress_bits[IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS + 1];
    ...
    sh_dnx_pp_occ_ingress_profile_masked_bits_print(unit, acl_bits, egress_bits, ingress_bits);
    ...
    while (!is_end)
    {
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("%s%d%s", acl_bits, in_lif_profile, ingress_bits);
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    while (!is_end)
    {
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("%s%s%d", acl_bits, egress_bits, in_lif_profile);
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char acl_bits[ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS + 1], ingress_bits[NOF_IN_LIF_PROFILE_INGRESS_BITS + 1],
        egress_bits[IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS + 1];
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "MC forwarding type");
    ...
    while (!is_end)
    {
        ...
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("%s%s%d", acl_bits, egress_bits, vsi_profile);
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
            ...
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id, DBAL_FIELD_VSI_PROFILE_L2_MC_FWD_TYPE, INST_SINGLE, &fwd_type));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_TITLE_SET("ERPP Outlif Profile allocation");
    PRT_COMMITX;
exit:
    PRT_FREE;
    SHR_FUNC_EXIT;
