...
#include <appl/diag/dnx/fabric/diag_dnx_fabric.h>
...
#define DIAG_DNXC_LINK_CONFIG_FULL_MANUAL \
                "Displays the most important Fabric configuration information for all links or for a specific link \n" \
                "By default prints the configuration information of all links. \n" \
                "Filters: \n" \
                "\t <logical_port> - Free parameter for displaying the configuration information only for a specific logical_port"
...
#define DIAG_DNXC_LINK_STATUS_FULL_MANUAL \
                ...
                "Filters: \n" \
                "\t <logical_port> - Free parameter for displaying the status only for a specific logical_port"
...
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit) && !SOC_DNX_IS_MESH(unit))
    {
...
static sh_sand_option_t dnxc_fabric_link_status_options[] = {
    {"link",         SAL_FIELD_TYPE_INT32,     "link ID",         "-1", NULL, NULL, SH_SAND_ARG_FREE},
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SH_SAND_GET_INT32("link", logical_port);
     ...
     */
    if (!show_all && logical_port == DIAG_DNXC_OPTION_ALL)
    {
        show_all = 1;
    }
    ...
    if (show_all && logical_port != DIAG_DNXC_OPTION_ALL)
    {
        SHR_CLI_EXIT(_SHR_E_PARAM,
                     "Passing 'all' together with logical_port as parameters is now allowed! Please see 'fabric link status usage' \n");
    }
    ...
    else
    {
        ...
        BCM_PBMP_PORT_SET(diag_info_bmp, logical_port);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_port_t logical_port;
    int show_all, link_id, object_col_id = 0;
...
sh_sand_cmd_t sh_dnxc_fabric_link_cmds[] = {
    ...
    {"status", cmd_dnxc_fabric_link_status, NULL,    dnxc_fabric_link_status_options, &dnx_fabric_link_status_man,   NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnx},
    {"status", cmd_dnxc_fabric_link_status, NULL,    dnxc_fabric_link_status_options, &dnxf_fabric_link_status_man,  NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnxf},
