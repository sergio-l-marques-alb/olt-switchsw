...
#include <include/soc/dcmn/error.h>                                          /** for SOCDNX_EXIT_WITH_ERR() 										    */
...
static shr_error_e sh_dnx_per_service_pool_tc_resources_table_print(
    ...
    sh_sand_control_t * sand_control);
/**
* \brief -  Zeroes all rows in Max Values tables.
*/
static shr_error_e sh_dnx_max_values_clear(
    uint32 unit);
/**
...
/*    SHR_IF_ERR_EXIT(sh_dnx_per_interface_resources_table_print
    ...
    /** The CPU needs to zero all counters after reading them. */
    SHR_IF_ERR_EXIT(sh_dnx_max_values_clear(unit));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, &core_resource));
    ...
    dbal_interface_resources_get_t dbal_resource_get_func)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
    {
        for (uint32 interface = 0; interface < dnx_data_egr_queuing.params.nof_egr_interfaces_get(unit); interface++)
        {
            /** get uc_pd and uc_db values */
            SHR_IF_ERR_EXIT(dbal_resource_get_func
                            (unit, core_index, 0, ECGM_IF_LOW, interface, &if_pd_priority_low, &if_db_priority_low));
            SHR_IF_ERR_EXIT(dbal_resource_get_func
                            (unit, core_index, 0, ECGM_IF_HIGH, interface, &if_pd_priority_high, &if_db_priority_high));
            ...
            SHR_IF_ERR_EXIT(dbal_resource_get_func
                            (unit, core_index, 1, ECGM_IF_LOW, interface, &if_pd_priority_low, &if_db_priority_low));
            SHR_IF_ERR_EXIT(dbal_resource_get_func
                            (unit, core_index, 1, ECGM_IF_HIGH, interface, &if_pd_priority_high, &if_db_priority_high));
    ...
    dbal_port_resources_get_t dbal_resource_get_func)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
    {
        ...
        BCM_PBMP_ITER(ports_bmp, logical_port)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, 0, base_q_pair, &uc_pd, &uc_db));
            SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, 1, base_q_pair, &mc_pd, &mc_db));
    ...
    dbal_queue_resources_get_t dbal_resource_get_func)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
    {
        for (uint32 q_pair = 0; q_pair < dnx_data_egr_queuing.params.nof_q_pairs_get(unit); q_pair++)
        {
            SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, 0, q_pair, &uc_pd, &uc_db));
            SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, 1, q_pair, &mc_pd, &mc_db));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
    {
        for (int sp = 0; sp < dnx_data_ecgm.core_resources.nof_sp_get(unit); sp++)
        {
            SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, sp, &sp_resource));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
    {
        for (int sp = 0; sp < dnx_data_ecgm.core_resources.nof_sp_get(unit); sp++)
        {
            for (int tc = 0; tc < DNX_COSQ_NOF_TC; tc++)
            {
                SHR_IF_ERR_EXIT(dbal_resource_get_func(unit, core_index, sp, tc, &sp_tc_pd, &sp_tc_db));
...
static shr_error_e
sh_dnx_max_values_clear(
    uint32 unit)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_core_global_resources_statistics_max_clear(unit));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_interface_resources_statistics_max_clear(unit));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_port_resources_statistics_max_clear(unit));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_queue_resources_statistics_max_clear(unit));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_service_pool_resources_statistics_max_clear(unit));
    SHR_IF_ERR_EXIT(dnx_ecgm_dbal_service_pool_tc_resources_statistics_max_clear(unit));
