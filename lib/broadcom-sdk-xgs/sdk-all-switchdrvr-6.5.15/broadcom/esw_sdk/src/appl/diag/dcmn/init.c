...
appl_dpp_convert_modport_to_sysport_id(int unit, int modid_idx,int port)
{
    ...
    if (is_dnx_app)
    {
        ...
        rv = appl_sand_modid_to_sysport_convert(unit, modid_idx, port, &sys_port);
        if (rv < 0)
        {
            sys_port = -1;
        }
...
appl_dpp_convert_modport_to_base_voq_connector(int unit, int local_port, int core_id, int ingress_modid)
{
    ...
    if (is_dnx_app)
    {
        ...
        rv = appl_sand_modid_to_base_voq_connector_id_convert(unit, local_port, core_id, ingress_modid, g_dii.nof_devices, &voq_conn_id);
        if (rv < 0)
        {
            return -1;
        }
