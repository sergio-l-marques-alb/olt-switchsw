...
#define UINT64_HEXA_STR_MAX_SIZE 20
    ...
    for (index_loop = index_min; index_loop <= index_max; index_loop++) {
        if (SOC_REG_INFO(unit, reg).regtype == soc_portreg) {
            soc_reg_get(unit, reg, index, inst, &rval);
        } else {
            soc_reg_get(unit, reg, inst, index, &rval);
        }
    ...
    for(mem_idx = 0; mem_idx < mem_tot; mem_idx++) {
        ...
        if (mem != INVALIDm) {
            ...
            if (fld == &m->fields[m->nFields]) {
                ...
                SOC_IF_ERROR_RETURN(compare_mem_field_value (unit, mem,
...
th3_check_settings_from_file(int unit, char* file_name) {
    ...
    int arg_idx;
    int is_mem = -1, reg_mem_idx, exp_value;
...
th3_check_per_itm_health_regs (int unit, int itm) {
    ...
    static const struct {
        ...
        {MMU_TOQ_STATUSr, 0x1},
        {MMU_OQS_AGED_STATUS0r, 0x0},
        {MMU_OQS_AGED_STATUS1r, 0x0},
        {MMU_OQS_AGED_STATUS2r, 0x0},
         {INVALIDr, 0x0}
    };
    uint64 value;
    for (reg_idx = 0; reg_val_list[reg_idx].ref_reg != INVALIDr; reg_idx++) {
...
th3_check_idb_port_regs(int unit, int port) {
    ...
    for (reg_idx = 0; obm_usage_regs[reg_idx][0] != INVALIDr; reg_idx++) {
        ...
        compare_reg_value(unit, reg, subp, pipe, 0x0);
    }
    ...
    compare_reg_field_value(unit, reg, fld, 0, pipe, 0x1);
    ...
    compare_reg_field_value(unit, reg, fld, 0, pipe, 0x0);
...
th3_check_req_grant_per_port(int unit, int port) {
    ...
    if ((!IS_CPU_PORT(unit, port)) && (!IS_LB_PORT(unit, port))) {
        ...
        port_ep_req = soc_reg_field_get(unit, reg, rval, REQ_CNTf);
...
#define UINT64_HEXA_STR_MAX_SIZE 20
    ...
    int sync = 0;
    uint64 value, red_drop, yel_drop;
...
#define UINT64_HEXA_STR_MAX_SIZE 20
    ...
    soc_info_t *si = &SOC_INFO(unit);
    uint32 itm_map = si->itm_map;
    uint64 value64, sum, red_drop, yel_drop;
