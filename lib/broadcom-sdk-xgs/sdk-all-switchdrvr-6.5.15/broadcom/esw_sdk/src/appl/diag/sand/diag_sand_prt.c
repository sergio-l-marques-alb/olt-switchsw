...
#define     PRT_NOSAME_CELLS    1
#define     PRT_NOZERO_CELLS    2
    ...
    prt_column_t **prt_column_p)
{
    ...
    sal_memset(prt_column, 0, sizeof(prt_control_t));
    ...
    char *filename)
{
    ...
    if (enable == FALSE)
    {
        ...
        file_flags |= CONF_OPEN_CURRENT_LOC;
    }
    else
    {
        /*
         ...
         */
        if(sh_cmd_is_device(prt_ctr->unit, NULL) == _SHR_E_NONE)
        {
            sal_snprintf(output_file, RHFILE_MAX_SIZE, "gold/%s", filename);
        }
        else
        {
            sal_snprintf(output_file, RHFILE_MAX_SIZE, "gold/adapter/%s", filename);
        }
        file_flags |= CONF_OPEN_PER_DEVICE;
    }
     ...
     */
    if ((dbx_xml_property_get_str(top_node, "device", tmp_str, RHNAME_MAX_SIZE)) != _SHR_E_NONE)
    {
    ...
    char *columns_str)
{
    ...
    {
        SHR_CLI_EXIT(_SHR_E_INTERNAL, "Failed to parse column numbers list:%s\n", columns_str);
    }
    ...
    for (i_col = 0; i_col < realtokens; i_col++)
    {
        ...
        if (range_realtokens == 2)
        {
            ...
            {
                prt_ctr->columns[i_ran - 1]->dont_show = FALSE;
            }
            /*
        ...
        else
        {
             ...
             * Go through all numbers in list and assign appropriate status of don't show
             */
            ...
            {
                prt_ctr->columns[show_col_id - 1]->dont_show = FALSE;
            }
        }
        /*
    ...
    char *prt_filename)
{
    ...
    if (sand_control != NULL)
    {
        ...
        {
            /*
             * If columns are not all, assume all columns are invisible, so only the ones that do not match the pattern
             * or presented explicitly in columns list will turn to be visible.
             */
            PRT_ALL_COLUMN_ITERATOR(k, prt_column, prt_ctr)
            {
                prt_column->dont_show = TRUE;
            }

            if (!sal_strcasecmp(columns_str, "nosame"))
            {
                /*
                 * First column will be always visible even if matching all other criteria
                 */
                prt_ctr->columns[0]->dont_show = FALSE;
                column_control = PRT_NOSAME_CELLS;
            }
            else if (!sal_strcasecmp(columns_str, "nozero"))
            {
                 ...
                 */
                prt_ctr->columns[0]->dont_show = FALSE;
                column_control = PRT_NOZERO_CELLS;
            }
            else
            {
                /*
    ...
    {
        /*
         * Use here ALL_COLUMN because we may mark some of the columns as invisible, but here we may probably need to
         * make them visible
         */
        PRT_ALL_COLUMN_ITERATOR(k, prt_column, prt_ctr)
        {
             ...
             */
            if (column_control == PRT_NOSAME_CELLS)
            {
                /*
            ...
            else if ((column_control == PRT_NOZERO_CELLS))
            {
                /*
                 * Check if the token is actually full of zeroes in different formats
                 */
                if ((prt_column->dont_show == TRUE) && (prt_token_is_zero(prt_row_offset) == FALSE))
                {
                    prt_column->dont_show = FALSE;
                }
            }
    ...
    if (!ISEMPTY(prt_ctr->title))
    {
        ...
        if (title_width > (total_width - 4))
        {
            ...
            if (column != NULL)
            {
                sal_snprintf(column[last_visible_col_id].format, RHKEYWORD_MAX_SIZE - 1, "| %s-%ds", "%",
                             column[last_visible_col_id].width + 1 + (title_width + 4 - total_width));
            }
        ...
        /*
         * Info format is the same as title with 2 exptra spaces before
         */
        sal_snprintf(title_format, RHNAME_MAX_SIZE - 1, "|   %s-%ds |\n", "%", total_width - 6);
        RHITERATOR(info_entry, prt_ctr->info_list)
        {
    ...
    /*
     * Always separate general part from columns by '=' line
     */
    diag_sand_prt_row(total_width, '=');

    /*
     * Step 4 - Print Header Line from prt_ctr->col only if there is list of rows, otherwise skip,
    ...
    /*
     * Step 5 - Print cells, if there are any rows
     */
    i_rows = 0;
    RHITERATOR(prt_row_start, prt_ctr->list)
    {
        ...
        prt_printf("|\n");
        if ((i_rows == RHLNUM(prt_ctr->list) - 1) || (entry->mode == PRT_ROW_SEP_EQUAL))
        {
            diag_sand_prt_row(total_width, '=');
        }
