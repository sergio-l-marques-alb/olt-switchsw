...
static sh_sand_man_t access_read_man = {
    ...
    .examples = "name=VTT count=1\n"
        "name=RATE block=SCH wb count=8 print=on\n"
        "prp=mem.*signal name=PORT count=1 field\n"
        "name=VSI id=0 count=1 index=0 field overwrite\n"
        "prp=mem block=ETPP count=1 file=access_list.xml\n"
        "prp=reg.*port field separate\n"
        "block=IPP count=1 changed"
};
    ...
    int *fld_match_list)
{
    ...
    if (field_flag == TRUE)
    {
        ...
        for (i_fld = 0; i_fld < meminfo->nFields; i_fld++)
        {
            ...
            PRT_CELL_SET_SHIFT(1, "%s", value_str);
        }
    }
exit:
    SHR_FUNC_EXIT;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(help_filename, 0, 512);
        ...
        switch (reginfo->regtype)
        {
            ...
            default:
                ...
                {       /* 
                     ...
                     */
                    for (i_ind = reginfo->first_array_index; i_ind < numels + reginfo->first_array_index; i_ind++)
                    {
                        /*
