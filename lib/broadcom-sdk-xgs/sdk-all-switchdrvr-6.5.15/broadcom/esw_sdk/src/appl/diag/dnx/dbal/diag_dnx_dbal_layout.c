    ...
    if (to_print)
    {
        ...
        for (ii = 0; ii < DBAL_NOF_HL_ACCESS_TYPES; ii++)
        {
            ...
            for (jj = 0; jj < l2p_hl_info[ii].num_of_access_fields; jj++)
            {
                ...
                if (sal_strcmp(l2p_hl_info[ii].l2p_fields_info[jj].group_name, EMPTY) != 0)
                {
                    PRT_CELL_SET("%s%s", l2p_hl_info[ii].l2p_fields_info[jj].group_name, "g");
                }
                else
                {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        if (table->keys_info[ii].is_allocator)
        {
            PRT_CELL_SET("Yes");
        }
        else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii = 0; ii < table->multi_res_info[result_idx].nof_result_fields; ii++)
    {
        ...
        PRT_CELL_SET("%s", results_info->is_valid_indication_needed ? "V" : "X");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        LOG_CLI((BSL_META("\tTable is not active in current image!\n")));
    }
    ...
    if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        ...
        LOG_CLI((BSL_META("\tMax key size: %d\n"), key_buffer));
    }
    ...
    if (print_access)
    {
                ...
                if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
                {
                    ...
                    if (table->hl_mapping_multi_res[ii].l2p_hl_info[DBAL_HL_ACCESS_SW].num_of_access_fields > 0)
                    {
                        ...
                        LOG_CLI((BSL_META("sw state table size = %d, sw state entry length = %d bytes\n"),
                                 table->sw_state_nof_entries, table->sw_payload_length_bytes));
        ...
        else if (table->access_method == DBAL_ACCESS_METHOD_MDB)
        {
            ...
            for (jj = 0; jj < table->nof_physical_tables; jj++)
            {
                ...
                {
                    LOG_CLI((BSL_META("\tRESULT_TYPE is mapped Sw state!\n")));
                    LOG_CLI((BSL_META("\t\tsw state table size = %d, sw state entry length = %d bytes\n"),
                             table->sw_state_nof_entries, table->sw_payload_length_bytes));
                }
            ...
            if (table->table_type == DBAL_TABLE_TYPE_EM)
            {
                ...
                for (ii = 0; ii < table->nof_result_types; ii++)
                {
                    ...
                    SHR_IF_ERR_EXIT(mdb_em_get_entry_encoding(unit,
                                                              ...
                                                              table->key_size,
                                                              table->multi_res_info[ii].entry_payload_size,
                                                              table->app_id,
                                                              table->app_id_size,
                                                              &entry_encoding));
        ...
        {
            SHR_IF_ERR_EXIT(diag_dbal_tcam_cs_hard_coded_memory_dump(unit, table->dbal_stage));
            SHR_IF_ERR_EXIT(diag_dbal_hard_logic_tcam_hw_info_dump
                            (unit, &(table->hl_mapping_multi_res[0]), sand_control));
            LOG_CLI((BSL_META("\n")));
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_CELL_SET("%d", physical_table->nof_entries);
    PRT_CELL_SET("TBD");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** const value */
    if (field_type_info->const_value_valid)
    {
        if (field_type_info->print_type == DBAL_FIELD_PRINT_TYPE_HEX)
        {
    ...
    if (field_type_info->is_allocator)
    {
        PRT_CELL_SET("Yes");
    }
    else
