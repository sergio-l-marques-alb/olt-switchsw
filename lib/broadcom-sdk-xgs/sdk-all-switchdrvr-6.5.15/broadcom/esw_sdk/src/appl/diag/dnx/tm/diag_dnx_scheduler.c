...
typedef struct
{
     ...
     */
    int nof_conn[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    /**
    ...
    */
     ...
     */
    int nof_cl_ses[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    /**
     * Count of FQ Scheduler Elementss per core
     */
    int nof_fq_ses[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
    /**
     * Count of HR Scheduler Elementss per core
     */
    int nof_hr_ses[DNX_DATA_MAX_DEVICE_GENERAL_MAX_NOF_CORES];
...
/**
 * \brief - Return the actual credit rate of the current port hr
 */
static shr_error_e
...
/**
 * \brief - Return the actual fc status of a give port hr
 */
static shr_error_e
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_port_t port;
    bcm_gport_t local_gport, e2e_tc_gport;
    bcm_cosq_gport_info_t gport_info;
    uint32 flags = 0;
    ...
    PRT_COLUMN_ADD("EGQ Rate[Kbps]");
    PRT_COLUMN_ADD("EGQ Max Burst");
    ...
    SHR_IF_ERR_EXIT(bcm_cosq_gport_bandwidth_get(unit, local_gport, cos, &kbits_sec_min, &kbits_sec_max, &flags));
    SHR_IF_ERR_EXIT(bcm_cosq_control_get(unit, local_gport, cos, bcmCosqControlBandwidthBurstMax, &max_burst));
    PRT_CELL_SET("%u", kbits_sec_max);
    PRT_CELL_SET("%d", max_burst);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!is_flow)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_stk_sys_modid_local_core_get(unit, modid, &local_core));
        if (local_core == -1)
        {
    ...
    sh_sand_control_t * sand_control)
{
     ...
     *  mapping */
    if (num_priorities >= dnx_data_sch.ps.min_priority_for_tcg_get(unit))
    {
