...
_phy_prbs_dcmn(int unit, args_t *args)
{
    ...
    bcm_pbmp_t          pbmp;
    int                 rv, cmd, enable, mode = 0, lb_mode = BCM_PORT_LOOPBACK_PHY, interval=1; // mode_get = 0 /*PRBS clear variable unused*/
...
_phy_diag_dcmn(int unit, args_t *args)
{
    ...
    if ((cmd_str = ARG_GET(args)) == NULL) {
        cli_out("ERROR: Invalid format. Please include sub option: eyescan|LoopBack|berproj\n");
        return CMD_FAIL;
    }
