...
static sh_sand_man_t dnx_trap_prog_info_man = {
    ...
    .full = "Programmable trap info configuration per ID\n",
    .synopsis = "[id=0-7]",
    .examples = "id=4",
    .init_cb = diag_dnx_trap_prog_init_cb,
};
    ...
    int id)
{
    ...
    bcm_rx_trap_prog_config_t_init(&prog_config);
    SHR_IF_ERR_EXIT(dnx_rx_trap_prog_get(unit, id, &prog_config));
    ...
    PRT_CELL_SET("%d", BCM_GPORT_TRAP_GET_ID(prog_config.trap_gport));
    PRT_CELL_SET("N/A");
    PRT_CELL_SET("N/A");
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
