...
char cmd_gdpll_usage[] =
    ...
    "            <Channel_num+ChannelState>, <FB-TS>, <REF-TS>, <PERROR>, <NCO>, <LoopFilter>, <LockDetFilter>\n"
...
extern int _bcm_esw_gdpll_offset_set(int unit, int chan, uint64 Offset);
extern int _bcm_esw_gdpll_offset_get(int unit, int chan, uint64 *pOffset);
...
int gdpll_debug_cb(int unit, void *user_data, uint32 debug_buff, uint32 debug_buf_size)
{
    ...
    if (fp) {
        for (i=0; i<debug_buf_size; i++, rd_addr+=4) {
            val = soc_cm_iproc_read(unit, rd_addr);
...
{
    uint32 l, u;
    soc_iproc_getreg(unit, 0x0326c088/*NS_TIMESYNC_TS1_TIMESTAMP_UPPER_STATUSr*/, &u);
    soc_iproc_getreg(unit, 0x0326c084/*NS_TIMESYNC_TS1_TIMESTAMP_LOWER_STATUSr*/, &l);
...
int bcm_gdpll_cb_rsvd1(int unit, void *user_data, uint32 *cb_data, int cb_type)
{
    ...
    if (rsvd1count) {
        ...
        if (rsvd1count == 0) {
            rv = bcm_gdpll_cb_unregister(unit, BCM_GDPLL_CB_RSVD1);
            if (BCM_FAILURE(rv)) {
                return CMD_FAIL;
            }
        }
...
uint32 gdpll_config(bcm_gdpll_chan_t *pGdpllChan, char *pStr, char *pNumStr)
{
    ...
    /* DPLL config */
        ...
        pGdpllChan->chan_dpll_config.norm_phase_error_thres0 = num;
    else if (strcmp(pStr, "dpll_config_norm_phase_thres1") == 0)
        pGdpllChan->chan_dpll_config.norm_phase_error_thres1 = num;
...
cmd_result_t cmd_gdpll(int unit, args_t *args)
{
    ...
    if (parse_cmp("Flush", arg_string_p, 0)) {
        ...
        rsvd1count = 2;
        result = bcm_gdpll_cb_register(unit, BCM_GDPLL_CB_RSVD1, bcm_gdpll_cb_rsvd1, NULL);
        if (BCM_FAILURE(result)) {
            return CMD_FAIL;
        }
    } else if (parse_cmp("Rsvd1cbunreg", arg_string_p, 0)) {
        result = bcm_gdpll_cb_unregister(unit, BCM_GDPLL_CB_RSVD1);
        if (BCM_FAILURE(result)) {
            return CMD_FAIL;
        }
        ...
        if (parse_cmp("Set", arg_string_p, 0)) {
            ...
            _bcm_esw_gdpll_offset_set(unit, chan, Offset);
            ...
            _bcm_esw_gdpll_offset_get(unit, chan, &Offset);
        ...
        if (parse_cmp("Destroy", arg_string_p, 0)) {
            ...
            if (chan < 32) {
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE0r, REG_PORT_ANY, 0/*idx*/), &pair_avail);
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_IN_STATUS0r, REG_PORT_ANY, 0/*idx*/), &ia_in_status);
                pair_avail = pair_avail & (1 << chan);
                ia_in_status = ia_in_status & (1 << chan);
            } else if (chan < 64) {
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE1r, REG_PORT_ANY, 0/*idx*/), &pair_avail);
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_IN_STATUS1r, REG_PORT_ANY, 0/*idx*/), &ia_in_status);
                pair_avail = pair_avail & (1 << (chan-32));
                ia_in_status = ia_in_status & (1 << (chan-32));
            } else if (chan < 96) {
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE2r, REG_PORT_ANY, 0/*idx*/), &pair_avail);
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_IN_STATUS2r, REG_PORT_ANY, 0/*idx*/), &ia_in_status);
                pair_avail = pair_avail & (1 << (chan-64));
                ia_in_status = ia_in_status & (1 << (chan-64));
            } else if (chan < 128) {
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_TS_PAIR_AVAILABLE3r, REG_PORT_ANY, 0/*idx*/), &pair_avail);
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_IA_IN_STATUS3r, REG_PORT_ANY, 0/*idx*/), &ia_in_status);
                pair_avail = pair_avail & (1 << (chan-96));
                ia_in_status = ia_in_status & (1 << (chan-96));
            }

            /* Dump the debug info */
            if((chan>=0) && (chan < BCM_GDPLL_NUM_CHANNELS)) {
                soc_iproc_getreg(unit, soc_reg_addr(unit, NS_GDPLL_M7_ERROR_INFOr, REG_PORT_ANY, 0/*idx*/), &rval);

                cli_out("### GDPLL DIAG: Dump for chan:%d\n", chan);
                cli_out("    PAIR_AVAIL(%d): %d\n", chan, pair_avail);
                cli_out("    IA_IN_STATUS(%d): %d\n", chan, ia_in_status);
                cli_out("    M7 Trigger count: 0x%x\n", rval);
            }
            ...
            if (use_case_flag == 1)
            {
                ...
                if ((gdpll_chan.event_fb.input_event == bcmGdpllInputEventBS0HB) ||
                    ...
                    while(poll_period_uSec > 0) {
                        SOC_IF_ERROR_RETURN(READ_NS_MISC_EVENT_STATUSr(unit, &rval));
                        if (((ts_gpio == 0) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_0f)) ||
                            ((ts_gpio == 1) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_1f)) ||
                            ((ts_gpio == 2) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_2f)) ||
                            ((ts_gpio == 3) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_3f)) ||
                            ((ts_gpio == 4) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_4f)) ||
                            ((ts_gpio == 5) && soc_reg_field_get(unit, NS_MISC_EVENT_STATUSr, rval, TS_GPIO_5f))) {
