    ...
    int nof_valid_clusters)
{
    ...
    if (nof_valid_clusters != 0)
    {
        if ((dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
            || (dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM))
        {
    ...
    sh_sand_control_t * sand_control)
{
        ...
        for (ii = 1; ii < DBAL_NOF_PHYSICAL_TABLES; ii++)
        {
            ...
            if (nof_valid_clusters != 0)
            {
                if ((dnx_data_mdb.pdbs.pdb_info_get(unit, ii)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
                    || (dnx_data_mdb.pdbs.pdb_info_get(unit, ii)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM))
                {
                    PRT_CELL_SET("%d", clusters[0].entry_size);
                }
            }
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(mdb_diag_single_table_info_print
                        (unit, sand_control, dbal_physical_table_id, dbal_physical_table->physical_name,
                         dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type,
                         capacity, clusters, nof_valid_clusters));
        ...
        if (dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type == DBAL_ENUM_FVAL_MDB_DB_TYPE_EM)
        {
            ...
            soc_mem_info_t mem_info = SOC_MEM_INFO(unit, tid_atr);
            int nof_app_ids = mem_info.index_max + 1;
            ...
            for (ii = 0; ii < MDB_EM_ENTRY_ENCODING_EMPTY; ii++)
            {
                ...
                SHR_IF_ERR_EXIT(mdb_get_vmv_size_value(unit, dbal_physical_table_id, ii, &vmv_size, &vmv_value));
                if (vmv_size != 0)
                {
                    /*
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint8 is_end = FALSE;
    uint32 app_id = dnx_data_mdb.em.max_nof_tids_get(unit);
    PRT_INIT_VARS;
    ...
    SHR_IF_ERR_EXIT(mdb_lpm_iterator_get_next
                    (unit, dbal_physical_table_id, dnx_data_mdb.em.max_nof_tids_get(unit), &physical_entry_iterator,
                     &entry, &is_end));
    while (is_end == FALSE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("%d",
                     dnx_kbp_calculate_prefix_length(entry.k_mask, dnx_data_mdb.kaps.key_width_in_bits_get(unit)));
...
static sh_sand_option_t dnx_mdb_lpm_options[] = {
    ...
    {"dump", SAL_FIELD_TYPE_INT32, "Perform a dump operation, 1 = logical dump, 2 = HW dump.", "0"},
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_INT32("dump", dump_cmd);
#ifndef ADAPTER_SERVER_MODE
    ...
    {
        SHR_IF_ERR_EXIT(dnx_kbp_prefix_len_to_mask
                        (unit, prefix_length, dnx_data_mdb.kaps.key_width_in_bits_get(unit), &entry));
    ...
    if (hit_clr_cmd == TRUE)
    {
        entry.hitbit |= DBAL_PHYSICAL_KEY_HITBIT_CLEAR;
    }
#endif
...
static sh_sand_man_t dnx_mdb_direct_man = {
    ...
    .examples =
        ...
        "get force_hw table=IVSI key=100 payload_size=90\n"
        "dump table=IVSI\n" "dump force_hw table=IVSI\n"
...
static sh_sand_option_t dnx_mdb_direct_options[] = {
    ...
    {"CLEAR", SAL_FIELD_TYPE_BOOL, "Perform a table clear operation", "no"},
    {"Get", SAL_FIELD_TYPE_BOOL, "Perform a get operation", "no"},
    {"force_hw", SAL_FIELD_TYPE_BOOL, "Force HW interaction, skipping sw cache, relevant only for dump and get", "no"},
    {"DeLeTe", SAL_FIELD_TYPE_BOOL, "Perform a delete operation", "no"},
    ...
    sh_sand_control_t * sand_control)
{
    dbal_physical_tables_e dbal_physical_table_id;
    dbal_physical_entry_t entry;
    int add_cmd, get_cmd, delete_cmd, dump_cmd, clear_cmd, force_hw_cmd;
    char *table_name = NULL;
    /*
    ...
    SH_SAND_GET_BOOL("force_hw", force_hw_cmd);
    ...
    else
    {
        ...
        if (dnx_data_mdb.pdbs.pdb_info_get(unit, dbal_physical_table_id)->db_type != DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "The Table is not an MDB direct table.\n");
        }
        ...
        {
            SHR_IF_ERR_EXIT(mdb_diag_direct_dump(unit, dbal_physical_table_id, app_id, force_hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
...
static sh_sand_man_t dnx_mdb_eedb_man = {
    ...
    .examples = "add table=EEDB1 key=100 payload_size=120 payload=0x12345678abcdef01030507 eedb_ll=0\n"
        ...
        "get force_hw table=EEDB1 key=100 payload_size=120 eedb_ll=0\n" "dump table=EEDB1\n"
        "dump force_hw table=EEDB1\n"
...
static sh_sand_option_t dnx_mdb_eedb_options[] = {
    ...
    {"payload_size", SAL_FIELD_TYPE_UINT32, "The payload size", "0"},
    {"payload", SAL_FIELD_TYPE_ARRAY32, "Up to 150bits of the payload", "0"},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    dbal_physical_tables_e dbal_physical_table_id;
    dbal_physical_entry_t entry;
    int add_cmd, get_cmd, force_hw_cmd, delete_cmd, dump_cmd, clear_cmd;
    char *table_name = NULL;
    /*
     * app_id is unused in direct tables
     */
    ...
    sal_memcpy(entry.payload, array_uint32,
               BITS2WORDS(dnx_data_mdb.direct.max_payload_size_get(unit)) * sizeof(entry.payload[0]));
    SH_SAND_GET_BOOL("eedb_ll", entry.eedb_ll);
    ...
    else
    {
        ...
        if (add_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_eedb_table_entry_add(unit, dbal_physical_table_id, app_id, &entry));
        ...
        else if (get_cmd != 0)
        {
            ...
            {
                LOG_INFO(BSL_LOG_MODULE,
                         (BSL_META_U(unit, "entry.payload[%d]: 0x%08x %s.\n"), BITS2WORDS(entry.payload_size) - 1 - ii,
                          entry.payload[BITS2WORDS(entry.payload_size) - 1 - ii],
                          ((entry.hitbit & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED) != 0) ? "Hit" : "No Hit"));
            }
        }
        else if (delete_cmd != 0)
        {
        ...
        {
            SHR_IF_ERR_EXIT(mdb_diag_eedb_dump(unit, dbal_physical_table_id, app_id, force_hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (app_id = 0; app_id < nof_app_ids; app_id++)
    {
        ...
        while (is_end == FALSE)
        {
            ...
            {
                PRT_CELL_SET("0x%08x%08x%08x%08x", entry.payload[3], entry.payload[2], entry.payload[1],
                             entry.payload[0]);
            }
            else if (entry.payload_size <= 5 * SAL_UINT32_NOF_BITS)
            {
...
static sh_sand_man_t dnx_mdb_em_man = {
    ...
    .examples =
        "add table=ISEM1 key=0x12345678 key_size=32 payload_size=16 payload=0xabcd app_id=0\n"
        "get table=ISEM1 key=0x12345678 key_size=32 payload_size=16 app_id=0\n"
        "dump table=ISEM1\n"
        "delete table=ISEM1 key=0x12345678 key_size=32 app_id=0\n"
        "add table=LEM key=0x123456789abc key_size=49 payload_size=48 payload=0xabcdef0123 app_id=0\n"
        "get table=LEM key=0x123456789abc key_size=49 payload_size=48\n"
        "delete table=LEM key=0x123456789abc key_size=49 payload_size=48\n" "clear table=LEM"
};
    ...
    sh_sand_control_t * sand_control)
{
    dbal_physical_tables_e dbal_physical_table_id;
    dbal_physical_entry_t entry;
    int add_cmd, get_cmd, delete_cmd, clear_cmd, dump_cmd;
    char *table_name = NULL;
    uint32 app_id;
    uint32 *array_uint32;
    ...
    else
    {
        ...
        if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_em_dump(unit, dbal_physical_table_id, sand_control));
        }
        else if (clear_cmd != 0)
        {
        ...
        else
        {
            ...
            else if (get_cmd != 0)
            {
                ...
                entry.hitbit = DBAL_PHYSICAL_KEY_HITBIT_ACTION;
                if (MDB_INIT_EM_IS_EMP
