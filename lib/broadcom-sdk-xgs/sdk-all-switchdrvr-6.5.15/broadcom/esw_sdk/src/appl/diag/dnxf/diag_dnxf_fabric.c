    ...
    int * nof_link_ranges)
{
     ...
     */
    _SHR_PBMP_LAST(config.sfi, last_enabled_link);
    ...
    *nof_link_ranges = 0;
    ...
    {
        if (!BCM_PBMP_MEMBER(config.sfi, link))
        {
            /**
        ...
        {
            if (!BCM_PBMP_MEMBER(config.sfi, link_inner_index))
            {
                /**
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (link_selected != DIAG_DNXC_OPTION_ALL) {
        ...
        SHR_IF_ERR_EXIT(diag_dnxf_thresholds_profile_link_ranges_get(unit, config, profile_0_links_pbmp, profiles_link_ranges, &nof_link_ranges));
    }
    /*
        ...
        for (threshold_idx = 0; threshold_idx < dnxf_data_fabric.congestion.nof_thresholds_get(unit); threshold_idx++)
        {
                    ...
                    _shr_dnxf_cgm_index_type_pipe)
            {
                SHR_IF_ERR_EXIT(diag_dnxf_pipe_type_threshold_print(unit, threshold_idx, th_validity_bmp, link_selected, pipe_selected, profile_0_links_pbmp,
                         iteration_parameters, profiles_link_ranges, nof_link_ranges));
            }
