 ...
 */
extern int dnx_pemladrv_full_reg_write(
    int unit,
    const int reg_id,
    const int size,
    ...
    dbal_tables_e table_id)
{
    switch (table_id)
    {
        case DBAL_TABLE_LOCAL_SBC_INLIF_INFO_SW:
        case DBAL_TABLE_LOCAL_DPC_INLIF_INFO_SW:
        ...
        case DBAL_TABLE_LOCAL_OUT_LIF_INFO_SW:
        ...
        case DBAL_TABLE_LIF_TABLE_MANAGER_VALID_FIELDS_PER_OUT_LIF_TABLE:
        case DBAL_TABLE_LIF_TABLE_MANAGER_VALID_FIELDS_PER_IN_LIF_TABLE:
            return 1;
...
/**
 * \brief
 * this method is used to turn off the logger before performing a negative test that can dump information to consule.
 * after calling to this function API, ACCESS, DIAG severity logger will be FATAL and the current severities will be
 * saved. to restore the logger severiteies, call diag_dbal_post_action_logger_restore()
 */
cmd_result_t
    ...
    dbal_fields_e field_id)
{
    ...
    for (enum_val_index = 0; enum_val_index < field_info->nof_enum_values; enum_val_index++)
    {
        ...
        enum_entry_index++;
        dbal_test_enum_table[enum_entry_index].string = &(field_info->enum_val_info[enum_val_index].name[str_len]);
        dbal_test_enum_table[enum_entry_index].value = enum_val_index;
        dbal_test_enum_table[enum_entry_index].plugin_str = NULL;
        enum_entry_index++;
    }
    ...
    sh_sand_control_t * sand_control)
{
    int ii;
    dbal_labels_e label;
    char *label_name, *field_type_name, *print_type_name;
    CONST dbal_field_types_basic_info_t *field_type_info;
...
static sh_sand_man_t dnx_dbal_field_types_info_man = {
    .brief = "Print the field type info",
...
static sh_sand_man_t dnx_dbal_lables_dump_man = {
    .brief = "dump all available labels",
    .full = NULL,
    .synopsis = NULL,
    .examples = NULL
};
...
static sh_sand_man_t dnx_dbal_handles_status_man = {
    .brief = "dump entry handle status and basic info",
    .full = NULL,
    .synopsis = NULL,
    .examples = "ID=0"
};
    ...
    int unit)
{
    ...
    while (table_id != DBAL_TABLE_EMPTY)
    {
            ...
            (table->hl_mapping_multi_res[0].l2p_hl_info[DBAL_HL_ACCESS_REGISTER].num_of_access_fields > 0))
        {
			...
			SHR_IF_ERR_EXIT(dbal_tables_next_table_get
                        (unit, table_id, DBAL_LABEL_NONE, DBAL_ACCESS_METHOD_HARD_LOGIC, DBAL_PHYSICAL_TABLE_NONE, DBAL_TABLE_TYPE_NONE,
                         &table_id));
            continue;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_STR("HWElement", hw_element_name);
    ...
    {
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.table_status.get(unit, table_iter, &table_status));
                            ...
                            for (hl_iter = 0; hl_iter < l2p_hl_info->num_of_access_fields; hl_iter++)
                            {
                                ...
                                for (index = 0; index < DBAL_MAX_NUMBER_OF_HW_ELEMENTS; index++)
                                {
                                    if (l2p_hl_info->l2p_fields_info[hl_iter].reg[index] != INVALIDr)
                                    {
                                        if (strcaseindex
                                            (SOC_REG_NAME
                                             (unit, l2p_hl_info->l2p_fields_info[hl_iter].reg[index]),
                                             hw_element_name))
                                        {
                                            PRT_CELL_SET("%s", table->table_name);
                                            PRT_CELL_SET("%s",
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        ...
        PRT_CELL_SET("%d", table->sw_state_nof_entries);
        PRT_CELL_SET("%d", table->sw_payload_length_bytes);
        PRT_CELL_SET("%d", (table->sw_payload_length_bytes * table->sw_state_nof_entries) / 1000);
        ...
        {
            size_in_kb = (DBAL_HASH_TABLE_ENTRY_SIZE * table->sw_state_nof_entries);
            PRT_CELL_SET("%d", size_in_kb / 1000);
        }

        if (strcaseindex(table->table_name, "example"))
        {
            size_example_tables += table->sw_payload_length_bytes * table->sw_state_nof_entries;
            nof_example++;
        }
        else
        {
            size_real_tables += table->sw_payload_length_bytes * table->sw_state_nof_entries + size_in_kb;
        }
    }
...
exit:
    dnx_pemladrv_reg_info_de_allocate(pemla_reg_info, pemla_reg_info_size);
    PRT_FREE;
    SHR_FUNC_EXIT;
}
...
static sh_sand_option_t dnx_dbal_access_hl_status_options[] = {
    {"HWElement", SAL_FIELD_TYPE_STR, "dbal access HL HWElement", ""},
    {NULL}
};
...
static sh_sand_man_t dnx_dbal_access_mdb_man = {
    .brief = "MDB access diagnostic",
};

static sh_sand_man_t dnx_dbal_access_hl_status_man = {
    ...
    .full = "in-case that HW element was inserted dump all tables related to HW element\n"
		"otherwise dump all tables that has uniqe HW entity",
    .synopsis = "HWElement=<element_name>",
    .examples = "HWElement=IPPA_VTT_IN_PP_PORT_CONFIG\n" "HWElement=IPPE_PP_PORT_TO_VDm"
};
...
sh_sand_cmd_t sh_dnx_dbal_access_cmds[] = {
   ...
   *          *                            * CMD   *                                 *                               *
   *******************************************************************************************************************/
    {"HL", cmd_dbal_hl_info_dump, NULL, dnx_dbal_access_hl_status_options, &dnx_dbal_access_hl_status_man},
    {"MDB", NULL, sh_dnx_dbal_access_mdb_cmds, NULL, &dnx_dbal_access_mdb_man},
    {"SW", cmd_dbal_access_sw_info_dump, NULL, dnx_dbal_access_sw_status_options, &dnx_dbal_access_sw_status_man},
    {"PEMLA", NULL, diag_dbal_access_pemala_cmds, dnx_dbal_access_pemla_status_options,
     &dnx_dbal_access_pemla_status_man},
    {NULL}
};
/**********************************************************************************************************************
 *  DBAL DIAGNOSTIC PACK:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *table_param_names[] =
        { "key size", "nof key fields", "res size", "nof res fields per RT", "total nof res fields" };
    ...
    int table_max_val[] = { DBAL_PHYSICAL_KEY_SIZE_IN_BITS, DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS,
        DBAL_PHYSICAL_RES_SIZE_IN_BITS, DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE, 0
    };
    ...
    LOG_CLI((BSL_META("table properties SW state size (per table) %d\n"),
    ...
    for (table_id = 0; table_id < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_id++)
    {
        ...
        total_sw_alloc += table->sw_state_nof_entries * table->sw_payload_length_bytes;
        if (table->table_type != DBAL_TABLE_TYPE_DIRECT)
        {
            total_non_direct += table->sw_state_nof_entries * DBAL_HASH_TABLE_ENTRY_SIZE;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *table_name = NULL, *file_name = NULL;
    dbal_tables_e table_id = DBAL_NOF_TABLES;
    ...
    if (print_status == TRUE)
    {
        ...
        dbal_logger_info_get(unit, &logger_info);
        ...
        DIAG_DBAL_HEADER_DUMP("Logger status", "\0");

        if (logger_info->user_log_locked_table != DBAL_TABLE_EMPTY)
        {
            LOG_CLI((BSL_META(" Table %s is locked (only API prints related to this table will be dumped)\n"),
                     dbal_logical_table_to_string(unit, logger_info->user_log_locked_table)));
        }

        if (logger_info->internal_log_locked_table != DBAL_TABLE_EMPTY)
        {
            LOG_CLI((BSL_META(" Table %s is internally locked\n"),
                     dbal_logical_table_to_string(unit, logger_info->internal_log_locked_table)));
        }

        LOG_CLI((BSL_META(" Logger internal disable flag is %s\n"), logger_info->disable_logger ? "true" : "false"));

        LOG_CLI((BSL_META
                 ("\n Type API: severity %s\n Type Access: severity %s\n Type Diag: severity %s\n File mode: %s\n"),
                 bsl_severity2str(curr_api_severity), bsl_severity2str(curr_access_severity),
                 bsl_severity2str(curr_diag_severity), (logger_info->dbal_file != NULL) ? "ON" : "OFF"));
    ...
    {
        DIAG_DBAL_HEADER_DUMP("Example: dbal LoGger Type=API Severity=INFO", "\0");
        DIAG_DBAL_HEADER_DUMP("Example: dbal LoGger Type=ACCESS Severity=VERB", "\0");
        DIAG_DBAL_HEADER_DUMP("Example: dbal LoGger table=<table_name>", "\0");
    }
    else
    {
        if ((severity != DNX_DBAL_LOGGER_PRINT))
        {
            ...
            else if (dbal_logger_type == DNX_DBAL_LOGGER_TYPE_ACCESS)
            {
        ...
        {
            bsl_severity_t curr_severity = 0;

            if (dbal_logger_type == DNX_DBAL_LOGGER_TYPE_API)
            {
                curr_severity = bslenable_get(bslLayerSocdnx, bslSourceDbaldnx);
            }
            else if (dbal_logger_type == DNX_DBAL_LOGGER_TYPE_ACCESS)
            {
                curr_severity = bslenable_get(bslLayerSocdnx, bslSourceDbalaccessdnx);
            }
            else if (dbal_logger_type == DNX_DBAL_LOGGER_TYPE_DIAG)
            {
                curr_severity = bslenable_get(bslLayerSocdnx, bslSourceDiagdbaldnx);
            }

            LOG_CLI((BSL_META("Logger severity for type %s is %s \n"),
                     dbal_logger_type_to_string(unit, dbal_logger_type), bsl_severity2str(curr_severity)));
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(dbal_decode(unit, SOC_CORE_DEFAULT, field_name, data, size, &parsed_info));
...
static sh_sand_man_t dnx_dbal_logger_man = {
    ...
    .synopsis =
        "[Type=<API/ACCESS/DIAG>] [SeVerity=<level>] [table=<table_name/off>] [file=<file_name/close>] [status]",
    .examples =
        "File=dbal_log.txt\n" "File=close\n"
...
static sh_sand_man_t dnx_dbal_stats_man = {
    .brief = "This diagnostic returns statistics information regarding to the DBAL infrastructure.",
    .full = NULL,
    .synopsis = NULL,
    .examples = NULL
};
...
sh_sand_cmd_t sh_dnx_dbal_cmds[] = {
    ...
    ********************************************************************************************************************************/
    ...
    {"TYPes", NULL, sh_dnx_dbal_field_types_cmds, NULL, &dnx_dbal_field_types_man},
    ...
    {"STaTe", cmd_dbal_statistics_set, NULL, NULL, &dnx_dbal_stats_man, NULL, NULL},
    {"Decode", cmd_dbal_decode, NULL, dnx_dbal_dcd_options, &dnx_dbal_dcd_man},
    {NULL}
};
