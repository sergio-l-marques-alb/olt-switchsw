...
static sh_sand_option_t Field_tcam_bank_info_options[] = {
    {DIAG_DNX_FIELD_OPTION_BANK,          SAL_FIELD_TYPE_UINT32, "Tcam bank id to get its info",       "MIN-MAX"   , (void *)Field_tcam_bank_id_enum_table},
    {NULL}
};
...
static sh_sand_option_t Field_tcam_management_info_options[] = {
    {DIAG_DNX_FIELD_OPTION_HANDLER,          SAL_FIELD_TYPE_UINT32, "Tcam handler id to get its info",       NULL   , (void *)Field_tcam_handler_id_enum_table},
    {NULL}
};
/*
 * }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dnx_field_group_t fg_id_array[dnx_data_field.group.nof_fgs_get(unit)];
     ...
     */
    for (fg_id_iter = 0; fg_id_iter < dnx_data_field.group.nof_fgs_get(unit); fg_id_iter++)
    {
        /**
    ...
    for (bank_id_iter = bank_id_lower; bank_id_iter <= bank_id_upper; bank_id_iter++)
    {
        ...
        for (fg_id_iter = 0; fg_id_iter < allocated_fg_iter; fg_id_iter++)
        {
            ...
            if (do_display)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get
                                (unit, core, bank_id_iter, DNX_FIELD_TCAM_ENTRY_SIZE_SINGLE, &nof_available_entries));
                PRT_CELL_SET("%d", nof_available_entries);
                SHR_IF_ERR_EXIT(dnx_field_tcam_bank_available_entries_get
                                (unit, core, bank_id_iter, DNX_FIELD_TCAM_ENTRY_SIZE_DOUBLE, &nof_available_entries));
                PRT_CELL_SET("%d", nof_available_entries);
            ...
            PRT_CELL_SET("%d", bank_entries_count[bank_id_iter]);
        }
        if (!do_display)
        {
            PRT_COMMITX;
    ...
    char *table_name_p)
{
    ...
    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        ...
        if ((table->table_type == DBAL_TABLE_TYPE_TCAM) && (table->access_method == DBAL_ACCESS_METHOD_MDB))
        {
            ...
            {
                sal_strncpy(table_name_p, table->table_name, (DBAL_MAX_STRING_LENGTH - 1));
                goto exit;
            }
        }
    }
    ...
    if (!fg_params.direct_table)
    {
        ...
        for (bank_id_iter = 0; bank_id_iter < DNX_DATA_MAX_FIELD_TCAM_NOF_BANKS; bank_id_iter++)
        {
            ...
            if (SHR_BITGET(banks_bmp, bank_id_iter))
            {
                 ...
                 */
                total_num_of_entries += bank_entries_count[bank_id_iter];
            }
        }
