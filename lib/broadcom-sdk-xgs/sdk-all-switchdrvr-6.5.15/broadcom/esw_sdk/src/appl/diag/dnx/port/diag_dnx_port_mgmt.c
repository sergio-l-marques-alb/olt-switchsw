    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_type_str_get(unit, diag_info.port_type, port_type_str));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(logical_ports, logical_port)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_type_str_get(unit, diag_info.logical_port_info.port_type, port_type_str));
        ...
        if (diag_info.interface_info.valid)
        {
            ...
            PRT_CELL_SET("%d", diag_info.interface_info.interface_speed);
        ...
        if (diag_info.nif_info.valid)
        {
            ...
            /** ILKN info  */
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN_ELK(unit, diag_info.logical_port_info.port_type))
            {
        ...
        if (diag_info.tm_pp_info.valid)
        {
            ...
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("tm interface");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.tm_interface);
            ...
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("tdm master");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.tdm_master);
            DIAG_DNX_PORT_MGMT_DUMP_FULL_TYPE("non tdm master");
            PRT_CELL_SET("%d", diag_info.tm_pp_info.non_tdm_master);

            /**  Egress TM ports only */
            if (DNX_ALGO_PORT_TYPE_IS_EGR_TM(unit, diag_info.logical_port_info.port_type))
            {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_E2E_SCH(unit, diag_info.logical_port_info.port_type))
            {
            ...
            /**  PP ports only */
            if (DNX_ALGO_PORT_TYPE_IS_PP(unit, diag_info.logical_port_info.port_type))
            {
