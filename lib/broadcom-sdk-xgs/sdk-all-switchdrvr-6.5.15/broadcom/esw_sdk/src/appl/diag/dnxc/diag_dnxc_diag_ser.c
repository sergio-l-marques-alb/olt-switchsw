    ...
    int *blk)
{
    ...
    uint32 mdb_max_num_ddh_blks[] = {
        SOC_MAX_NUM_DDHA_BLKS,
        SOC_MAX_NUM_DDHA_BLKS,
        SOC_MAX_NUM_DDHB_BLKS,
        SOC_MAX_NUM_DDHB_BLKS,
        SOC_MAX_NUM_DHC_BLKS
    };

    SHR_FUNC_INIT_VARS(unit);

    /*
     * iter mdb_phy_name_list if CLI input is one of actually MDB physical entry memory 
     */
    for (ii = 0; ii < COUNTOF(mdb_phy_name_list); ii++)
    {
        n = sal_strlen(SOC_MEM_NAME(unit, mdb_phy_name_list[ii]));
        if (sal_strncasecmp(mem_name, SOC_MEM_NAME(unit, mdb_phy_name_list[ii]), n) == 0)
        {
            *phy_mem = mdb_phy_name_list[ii];
            if (copyno < mdb_max_num_ddh_blks[ii])
            {
...
static sh_sand_man_t Dnxc_diag_ser_mdb_read_man = {
    ...
    .examples = "macro=a-1 cluster=7 entry=0\n"
        "1 mem=DHC_MACRO_PHYSICAL_ENTRY_BANK block=4 array=7 entry=0\n" "2 table=ISEM_1"
};
...
static sh_sand_man_t Dnxc_diag_ser_mdb_write_man = {
    ...
    .examples = "macro=a-1 cluster=7 entry=0 data=0x12345678abcdef\n"
        "1 mem=DHC_MACRO_PHYSICAL_ENTRY_BANK block=4 array=7 entry=0 data=0x12345678abcdef ecc=7"
};
    ...
    char *pMemInfo)
{
    ...
    sal_strncpy(pBlockInfo, soc_block_name_lookup_ext(blk_type, unit), 32);
    sal_strncpy(pEntryInfo, srt_entry_info, sizeof(srt_entry_info));
    sal_strncpy(pMemInfo, str_mem_info, sizeof(str_mem_info));
    *pAddress = address;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int dynamic_flag, static_flag, alias_flag, export_flag;
    ...
    char str_mem_info[256] = { 0 };
    char str_entry_info[32] = { 0 };
    char str_block_info[32] = { 0 };
    uint32 address = 0;
    char *severity_flag;
...
static sh_sand_option_t dnxc_ser_memory_list_options[] = {
    ...
    {"Num_ENTries", SAL_FIELD_TYPE_UINT32, "The range of entry numbers of list memories", "0-max", (void*)sand_diag_num_entry_bits_enum, "0-max"},
    {"Num_BIts", SAL_FIELD_TYPE_UINT32, "The range of entry bit length of list memories", "0-max", (void*)sand_diag_num_entry_bits_enum, "0-max"},
...
sh_sand_cmd_t dnxc_ser_memory_cmds[] = {
     ...
     * Name | Leaf Action | Junction Array | Options for Leaf | Usage 
     */
    {"LIST", dnxc_ser_memory_list_cmd, NULL, dnxc_ser_memory_list_options, &dnxc_ser_memory_list_man},
...
static sh_sand_option_t dnxc_ser_intr_dump_options[] = {
    {"type", SAL_FIELD_TYPE_STR, "Interrupt output info (status/flags/storm/stat)", "status", NULL, NULL, SH_SAND_ARG_FREE},
...
sh_sand_cmd_t dnxc_ser_intr_cmds[] = {
     ...
     * Name | Leaf Action | Junction Array | Options for Leaf | Usage 
     */
    {"DuMP", dnxc_ser_intr_dump_cmd, NULL, dnxc_ser_intr_dump_options, &dnxc_ser_intr_dump_man},
