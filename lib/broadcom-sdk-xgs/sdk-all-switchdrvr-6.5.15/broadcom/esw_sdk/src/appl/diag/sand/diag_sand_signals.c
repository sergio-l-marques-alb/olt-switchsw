    ...
    sh_sand_control_t * sand_control)
{
    ...
    int all_flag, print_flags = SIGNALS_PRINT_VALUE, status_flag;
    char *show_str;
    int object_col_id = 0;
    char **btokens = NULL;
    uint32 btokens_num;
    ...
    if (tx_enable == TRUE)
    {
         ...
         */
        SHR_IF_ERR_EXIT(diag_sand_packet_tx(unit, 0, example_data_string, TRUE));
    }
    ...
    if (core == BCM_CORE_ALL)
    {
        ...
        {
            SHR_CLI_EXIT_IF_ERR(sand_signal_list_get(unit, core, &match_m, dsig_list), "");
        }
    }
    else
    {
        ...
        if (print_flags & SIGNALS_PRINT_ASIC)
        {
            ...
            SHR_CLI_EXIT_IF_ERR(sand_signal_list_get(unit, core, &match_m, adapter_dsig_list), "");
            match_m.flags |= SIGNALS_MATCH_ASIC;
            SHR_CLI_EXIT_IF_ERR(sand_signal_list_get(unit, core, &match_m, dsig_list), "");
        }
        else
...
#if defined(BCM_DNX_SUPPORT) && !defined(ADAPTER_SERVER_MODE)
            {
                ...
                if ((full_flag == FALSE) &&
                    ((debug_signal->block_id == DBAL_ENUM_FVAL_PP_ASIC_BLOCK_REGISTER) ||
                     (debug_signal->block_id == DBAL_ENUM_FVAL_PP_ASIC_BLOCK_TABLE) ||
...
static sh_sand_man_t signal_compare_man = {
    ...
    .examples = "name=Fwd_Action block=IRPP" "name=Port from=VTT" "\n"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_CLI_EXIT_IF_ERR(sand_signal_list_get(unit, 0, &match_m, dsig_list), "");
    ...
    rhlist_t ** sig_list)
{
     ...
     */
    SHR_IF_ERR_EXIT(sand_signal_list_get(unit, core, &match, *sig_list));
...
shr_error_e
compare_adapter_signal_values_inner(
    int unit,
    int in_port,
    int out_port,
    char *block,
    ...
    int fail_test_if_signals_mismatch)
{
    ...
    /*
     * temporarily disale warmboot regression so that bcm_port_get called from within this diag will not trigger rebbot
     */
    if (SOC_IS_DNX(unit))
    {
    ...
    /*
     * Resolve using in_port, out_port and if the signal is in Igress or Egress which Core to use
     */
...
#ifdef BCM_WARM_BOOT_API_TEST
    /*
     * temporarily disable warmboot regression so that bcm_info_get called from within this diag will not trigger reboot
     */
    if (SOC_IS_DNX(unit))
    {
        dnxc_wb_no_wb_test_set(unit, 1);
    ...
    {
        cli_out("Signal not found on device, ignoring\n");
        return _SHR_E_NONE;
    }
    else if (rc == _SHR_E_FAIL)
    {
        ...
        cli_out("*****************************************************************************\n");
        if (fail_test_if_signals_mismatch)
        {
            return _SHR_E_PARAM;
        }
    }
    ...
    {
        cli_out("Error in sand_signal_verify\n");
        return _SHR_E_FAIL;
    }
