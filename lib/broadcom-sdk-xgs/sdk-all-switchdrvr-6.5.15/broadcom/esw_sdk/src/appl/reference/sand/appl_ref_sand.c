    ...
    int *sysport)
{
    ...
    if (port >= APPL_SAND_NOF_SYS_PORTS_PER_MODID(unit))
    {
        *sysport = -1;
    ...
    *sysport = modid_idx * APPL_SAND_NOF_SYS_PORTS_PER_MODID(unit) + port;
    ...
    int *base_voq_connector)
{
    ...
    voq_conn_id = APPL_SAND_E2E_SCHEME_VOQ_CONNECTOR_START_OFFSET +
        (local_port * nof_devices + ingress_modid_idx) * APPL_SAND_E2E_SCHEME_COS;
