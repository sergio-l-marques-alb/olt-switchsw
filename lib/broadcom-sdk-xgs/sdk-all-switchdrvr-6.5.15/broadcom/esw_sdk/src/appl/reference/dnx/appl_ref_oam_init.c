    ...
    int trap_id)
{
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &downmep_reflector_fg_id));
    ...
    oam_traps_t * oam_traps)
{
    ...
    SHR_IF_ERR_EXIT(appl_dnx_field_trap_no_system_header(unit, 1, up_mep_ingress_trap_id, 0x1ff));
    ...
    /** Set PMF rule */
    SHR_IF_ERR_EXIT(appl_dnx_field_trap_no_system_header(unit, 2, up_mep_ingress_cpu_trap_id, 0x1ff));
    /**********************************************************************************/
...
appl_dnx_oam_single_punt_config(
    int unit,
    bcm_rx_trap_t trap_type,
    bcm_oamp_error_type_e trap_base,
    bcm_gport_t cpu_port)
{
    bcm_rx_trap_config_t trap_config;
    int trap_id = 0, rv;
    ...
    trap_id = (trap_base | DNX_OAM_PUNT_MSB);

    bcm_rx_trap_config_t_init(&trap_config);
    trap_config.flags = BCM_RX_TRAP_WITH_ID;
    ...
    bcm_gport_t cpu_port)
{
    ...
    /** punt config */
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config
                    (unit, bcmRxTrapOampChanTypeMissErr, CHAN_TYPE_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampTypeErr, TYPE_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampRmepErr, EM_CORE_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampMaidErr, MAID_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampFlexCrcMissErr, FLEX_CRC_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampMdlErr, MDL_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampCcmIntrvErr, CCM_INTRVL_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampMyDiscErr, MY_DISC_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampSrcIpErr, SRC_IP_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampYourDiscErr, YOUR_DISC_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampSrcPortErr, SRC_PORT_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config
                    (unit, bcmRxTrapOampRmepStateChange, RMEP_STATE_CHANGE_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampParityErr, PARITY_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampTimestampErr, TIMESTAMP_MISS_ERROR, cpu_port));
    SHR_IF_ERR_EXIT(appl_dnx_oam_single_punt_config(unit, bcmRxTrapOampTrapErr, TRAP_CODE_MISS_ERROR, cpu_port));
    ...
    bfd_traps_t * bfd_traps)
{
    ...
    else
    {
         ...
         */
        /*
         * Check if there is second OAMP port configured.
         */
        if (count < 2)
        {
            LOG_VERBOSE(BSL_LS_APPL_COMMON,
                        (BSL_META_U(unit, "Error: Oamp second port disabled, can't init BFD over IPv6.\n")));
        }
        trap_config.dest_port = oamp_gport[1];
        rv = appl_dnx_oam_trap_config(unit, bcmRxTrapOamBfdIpv6, &bfd_traps->ipv6_trap_id, &trap_config);
        if (BCM_FAILURE(rv))
        {
            LOG_ERROR(BSL_LS_APPL_COMMON,
                      (BSL_META_U(unit, "appl_dnx_bfd_trap_config failed. Error:%d (%s)\n"), rv, bcm_errmsg(rv)));
 ...
 */
static shr_error_e
appl_bcm_dnx_oam_configure_default_profile(
    int unit,
    uint8 is_ingress,
    oam_traps_t oam_traps)
{
 ...
 */
static shr_error_e
appl_bcm_dnx_oam_configure_default_acc_profile(
    int unit,
    uint8 is_ingress,
    oam_traps_t oam_traps)
{
    ...
    int unit)
{
    ...
    {
        LOG_ERROR(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "Skip appl dnx oam for emulation\n")));
        return 0;
    }
#endif
     ...
     */
    rv = appl_bcm_dnx_oam_configure_default_profile(unit, INGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "appl_bcm_dnx_oam_configure_default_profile for ingress failed. Error:%d (%s)\n"),
                   rv, bcm_errmsg(rv)));
        return rv;
    }
     ...
     */
    rv = appl_bcm_dnx_oam_configure_default_acc_profile(unit, INGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U
                   (unit, "appl_bcm_dnx_oam_configure_default_acc_profile  for ingress failed. Error:%d (%s)\n"), rv,
                   bcm_errmsg(rv)));
        return rv;
    }
     ...
     */
    rv = appl_bcm_dnx_oam_configure_default_profile(unit, EGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U(unit, "appl_bcm_dnx_oam_configure_default_profile for egress failed. Error:%d (%s)\n"),
                   rv, bcm_errmsg(rv)));
        return rv;
    }
     ...
     */
    rv = appl_bcm_dnx_oam_configure_default_acc_profile(unit, EGRESS_SIDE, oam_traps);
    if (BCM_FAILURE(rv))
    {
        LOG_ERROR(BSL_LS_APPL_COMMON,
                  (BSL_META_U
                   (unit, "appl_bcm_dnx_oam_configure_default_acc_profile for egress failed. Error:%d (%s)\n"), rv,
                   bcm_errmsg(rv)));
        return rv;
    }
