...
/** 
 * \brief
 *     This function initializes ingress ipv4 ecn default mapping
 *     default initialization profile 0 to 
 *     ECN eligible disable, CNI false
 */
static shr_error_e
appl_dnx_qos_ingress_ipv4_ecn_init(
    int unit)
{
    int ecn_map_id;
    uint32 flags;
    int map_opcode;
    uint32 tos;
    bcm_qos_map_t ipv4_ingress_ecn_map;

    SHR_FUNC_INIT_VARS(unit);

    /** profile 0 is allocated in qos moudle init  */
    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_ECN;
    SHR_IF_ERR_EXIT(bcm_qos_map_id_get_by_profile(unit, flags, 0, &ecn_map_id));

    /** allocate opcode */
    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_ECN | BCM_QOS_MAP_OPCODE;
    SHR_IF_ERR_EXIT(bcm_qos_map_create(unit, flags, &map_opcode));

    /** set opcode map  */
    bcm_qos_map_t_init(&ipv4_ingress_ecn_map);
    ipv4_ingress_ecn_map.opcode = map_opcode;

    flags = BCM_QOS_MAP_L3 | BCM_QOS_MAP_IPV4 | BCM_QOS_MAP_ECN | BCM_QOS_MAP_OPCODE;
    SHR_IF_ERR_EXIT(bcm_qos_map_add(unit, flags, &ipv4_ingress_ecn_map, ecn_map_id));
    ...
    {
        bcm_qos_map_t_init(&ipv4_ingress_ecn_map);
        ipv4_ingress_ecn_map.dscp = tos;
        /** default ECN eligible disable, CNI false */
        ipv4_ingress_ecn_map.int_pri = 0;
        SHR_IF_ERR_EXIT(bcm_qos_map_add(unit, flags, &ipv4_ingress_ecn_map, map_opcode));
    }
...
/** 
 * \brief
 *     This function initializes ingress IPv6 ecn default mapping
 *     default initialization profile 0 to 
 *     ECN eligible disable, CNI false
 */
static shr_error_e
appl_dnx_qos_ingress_ipv6_ecn_init(
    int unit)
{
    int ecn_map_id;
    uint32 flags;
    int map_opcode;
    uint32 tos;
    bcm_qos_map_t ipv6_ingress_ecn_map;
    ...
    /** set opcode map  */
    bcm_qos_map_t_init(&ipv6_ingress_ecn_map);
    ipv6_ingress_ecn_map.opcode = map_opcode;

    flags = BCM_QOS_MAP_L3 | BCM_QOS_MAP_IPV6 | BCM_QOS_MAP_ECN | BCM_QOS_MAP_OPCODE;
    SHR_IF_ERR_EXIT(bcm_qos_map_add(unit, flags, &ipv6_ingress_ecn_map, ecn_map_id));
    ...
    {
        bcm_qos_map_t_init(&ipv6_ingress_ecn_map);
        ipv6_ingress_ecn_map.dscp = tos;
        /** default ECN eligible disable, CNI false */
        ipv6_ingress_ecn_map.int_pri = 0;
        SHR_IF_ERR_EXIT(bcm_qos_map_add(unit, flags, &ipv6_ingress_ecn_map, map_opcode));
    }
    ...
    int unit)
{
    ...
    {
        SHR_EXIT_WITH_LOG(_SHR_E_NONE, "Not initializing QOS application in emulation to save startup time%s%s%s",
                          EMPTY, EMPTY, EMPTY);
    }
#endif
    ...
    /** init ingress ipv4 ecn mapping */
    SHR_IF_ERR_EXIT(appl_dnx_qos_ingress_ipv4_ecn_init(unit));

    /** init ingress ipv6 ecn mapping */
    SHR_IF_ERR_EXIT(appl_dnx_qos_ingress_ipv6_ecn_init(unit));
