...
static field_itmh_qual_info_t field_itmh_ipmf2_qual_info_array[ITMH_NOF_IPMF2_QUALIFIERS] = {
  /** qual_name  |  qual_id   |   qual_size     |  input_type    |    input_arg  |  qual_offset  */
    ...
    {"fwd_trap_code_zeros_qual", 0, TRAP_CODE_ZEROS_QUAL_LENGTH,
        {bcmFieldInputTypeConst, 0x0, TRAP_CODE_ZEROS_QUAL_OFFSET}},
    {"fwd_act_str_qual", 0, ITMH_FWD_STR_QUAL_LENGTH,
        {bcmFieldInputTypeConst, 0xF, ITMH_FWD_STR_QUAL_OFFSET}}
};
...
/** {action_name}   |  action_id  |    { stage | action_type | size | prefix_size | prefix_value | name  } */
    {NULL, bcmFieldActionPphPresentSet, {0}},
    {NULL, bcmFieldActionPrioIntNew, {0}},
    /**
     ...
     */
    {"snoop_data_act", 0, {0, bcmFieldActionSnoop, SNOOP_DATA_ACTION_LENGTH,
        15, 0x0007F, {0}}},
    {"destination_act", 0, {0, bcmFieldActionForward, DST_ACTION_LENGTH,
        11, 0, {0}}},
    {NULL, bcmFieldActionDropPrecedence, {0}},
    ...
    {"mirror_command_act", 0, {0, bcmFieldActionVoid, MIRROR_ACTION_LENGTH,
        31, 0x00000000, {0}}},
    {"fwd_act_str_act", 0, {0, bcmFieldActionTrap, ITMH_FWD_STR_ACTION_LENGTH,
        19, 0x0, {0}}},
};
...
/** nof_conditions | condition_indexes | nof_extractions | nof_map_bits | array_id | fem_position_in_array | fem_input | condition_msb | fem_condition_info */
    ...
    {8, {2, 3, 6, 7, 10, 11, 14, 15}, 1, 22, {1, 2},
       ...
       */
      {
        ...
        {{bcmFieldActionOutVport0, {
            ...
            {bcmFieldFemExtractionOutputSourceTypeKey, 22, 0}}, 0}}}},
    /**
     * MIRROR action
    ...
    /** SystemProfile action, in case ASE is injected we need to change System header profile so it will not rebuild it */
    {8, {1, 3, 5, 7, 9, 11, 13, 15}, 1, 4, {1, 4},
      {
            /*
             * bcm_field_fem_input_info_t fem_input:
             *   Set input_offset     to 32 (This is the offset within key template)
    ...
    bcm_field_context_t context_id)
{
    ...
    bcm_field_context_info_t_init(&context_info);
    sal_strncpy_s((char *) (context_info.name), "ITMH_IPMF3", sizeof(context_info.name));
    SHR_IF_ERR_EXIT(bcm_field_context_create
    ...
    field_itmh_fem_action_info_t * itmh_fem_action_info)
{
    ...
    {
        /*
         * Verify that requested setup is actually applied to HW and SWSTATE.
         */
        bcm_field_fem_action_info_t fem_action_info_image;

        SHR_IF_ERR_EXIT(bcm_field_fem_action_info_get(unit, fg_id, action_priority, &fem_action_info_image));

        if (fem_action_info.fem_input.input_offset != fem_action_info_image.fem_input.input_offset)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "\r\n"
                         "action_priority 0x%08X: Applied 'input_offset' (%d) is not the same as read back (%d)\r\n"
                         "Probably internal sw error\r\n",
                         action_priority,
                         fem_action_info.fem_input.input_offset, fem_action_info_image.fem_input.input_offset);
        }
        if (fem_action_info.condition_msb != fem_action_info_image.condition_msb)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "\r\n"
                         "action_priority 0x%08X: Applied 'input_offset' (%d) is not the same as read back (%d)\r\n"
        ...
        for (conditions_index = 0; conditions_index < itmh_fem_action_info->nof_conditions; conditions_index++)
        {
            if (fem_action_info.fem_condition[condition_id[conditions_index]].is_action_valid !=
                fem_action_info_image.fem_condition[condition_id[conditions_index]].is_action_valid)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "\r\n"
                             "Condition no. %d on action_priority 0x%08X: Applied 'is_action_valid' (%d) is not the same as read back (%d)\r\n"
                             "Probably internal sw error\r\n",
                             conditions_index, action_priority,
                             fem_action_info.fem_condition[condition_id[conditions_index]].is_action_valid,
                             fem_action_info_image.fem_condition[condition_id[conditions_index]].is_action_valid);
            }
            if (fem_action_info.fem_condition[condition_id[conditions_index]].is_action_valid)
            {
                if (fem_action_info.fem_condition[condition_id[conditions_index]].extraction_id !=
                    fem_action_info_image.fem_condition[condition_id[conditions_index]].extraction_id)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "\r\n"
                                 "Condition no. %d on action_priority 0x%08X: Applied 'extraction_id' (%d) is not the same as read back (%d)\r\n"
                                 "Probably internal sw error\r\n",
                                 conditions_index, action_priority,
                                 fem_action_info.fem_condition[condition_id[conditions_index]].extraction_id,
                                 fem_action_info_image.fem_condition[condition_id[conditions_index]].extraction_id);
        ...
        for (extractions_index = 0; extractions_index < itmh_fem_action_info->nof_extractions; extractions_index++)
        {
            if (fem_action_info.fem_extraction[extractions_index].action_type !=
                fem_action_info_image.fem_extraction[extractions_index].action_type)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "\r\n"
                             "Action no. %d on action_priority 0x%08X: Applied 'Action_type' (%d, %s) is not the same as read back (%d, %s)\r\n"
                             "Probably internal sw error\r\n",
                             extractions_index, action_priority,
                             fem_action_info.fem_extraction[extractions_index].action_type,
                             dnx_field_bcm_action_text(unit,
                                                       fem_action_info.fem_extraction[extractions_index].action_type),
                             fem_action_info_image.fem_extraction[extractions_index].action_type,
                             dnx_field_bcm_action_text(unit,
                                                       fem_action_info_image.
                                                       fem_extraction[extractions_index].action_type));
            }
            if (fem_action_info.fem_extraction[extractions_index].increment !=
                fem_action_info_image.fem_extraction[extractions_index].increment)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "\r\n"
                             "Action no. %d on action_priority 0x%08X: Applied 'increment' (%d) is not the same as read back (%d)\r\n"
                             "Probably internal sw error\r\n",
                             extractions_index, action_priority,
                             fem_action_info.fem_extraction[extractions_index].increment,
                             fem_action_info_image.fem_extraction[extractions_index].increment);
            }
            for (mapping_bits_index = 0; mapping_bits_index < itmh_fem_action_info->nof_mapping_bits;
                 mapping_bits_index++)
            {
                if (fem_action_info.fem_extraction[extractions_index].output_bit[mapping_bits_index].offset !=
                    fem_action_info_image.fem_extraction[extractions_index].output_bit[mapping_bits_index].offset)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "\r\n"
                                 "Action no. %d on action_priority 0x%08X Bit_index %d: Applied 'offset' (%d) is not the same as read back (%d)\r\n"
                                 "Probably internal sw error\r\n",
                                 extractions_index, action_priority, mapping_bits_index,
                                 fem_action_info.fem_extraction[extractions_index].
                                 output_bit[mapping_bits_index].offset,
                                 fem_action_info_image.fem_extraction[extractions_index].
                                 output_bit[mapping_bits_index].offset);
                }
                if (fem_action_info.fem_extraction[extractions_index].output_bit[mapping_bits_index].forced_value !=
                    fem_action_info_image.fem_extraction[extractions_index].output_bit[mapping_bits_index].forced_value)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "\r\n"
                                 "Action no. %d on action_priority 0x%08X Bit_index %d: Applied 'forced_value' (%d) is not the same as read back (%d)\r\n"
                                 "Probably internal sw error\r\n",
                                 extractions_index, action_priority, mapping_bits_index,
                                 fem_action_info.fem_extraction[extractions_index].
                                 output_bit[mapping_bits_index].forced_value,
                ...
                if (fem_action_info.fem_extraction[extractions_index].output_bit[mapping_bits_index].source_type !=
                    fem_action_info_image.fem_extraction[extractions_index].output_bit[mapping_bits_index].source_type)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "\r\n"
                                 "Action no. %d on action_priority 0x%08X Bit_index %d: Applied 'source_type' (%d) is not the same as read back (%d)\r\n"
                                 "Probably internal sw error\r\n",
                                 extractions_index, action_priority, mapping_bits_index,
                                 fem_action_info.fem_extraction[extractions_index].
                                 output_bit[mapping_bits_index].source_type,
    ...
    for (action_index = 0; action_index < fg_info.nof_actions; action_index++)
    {
        if (itmh_fg_info->itmh_action_info[action_index].name == NULL)
        {
            ...
            attach_info.payload_info.action_types[action_index] =
                itmh_fg_info->itmh_action_info[action_index].action_id;
        }
        else
        {
    ...
    int unit)
{
    ...
    {
        SHR_EXIT_WITH_LOG(_SHR_E_NONE, "Skipping initializing field ITMH in emulation to save time%s%s%s\n", EMPTY,
                          EMPTY, EMPTY);
    }
#endif
