...
/* Number of TAG-Struct that are created */
#define DNX_NOF_TAG_FORMAT_CREATED   5
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_port_tpid_class_create
                    (unit, BCM_PORT_TPID_CLASS_WITH_ID, bcmTagStructTypeSPrioCTag, &tag_format_class_id));
    ...
    int unit)
{
    ...
    int tag_format_array[DNX_NOF_TAG_FORMAT_CREATED] =
        { DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_NONE, DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_TAG,
        DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_C_TAG, DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_C_TAG,
        DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_PRIORITY_C_TAG
    };
    ...
    for (vlan_edit_profile = 0; vlan_edit_profile < (1 << 6); vlan_edit_profile++)
    {
         ...
         */
        for (tag_format_index = 0; tag_format_index < DNX_NOF_TAG_FORMAT_CREATED; tag_format_index++)
        {
    ...
    int unit)
{
    ...
    rep_array =
        (bcm_multicast_replication_t *) sal_alloc(sizeof(bcm_multicast_replication_t) * nof_port,
                                                  "dnx default_flooding_init ");
    SHR_NULL_CHECK(rep_array, _SHR_E_MEMORY, "failed to allocate memory ");
    sal_memset(rep_array, 0, sizeof(bcm_multicast_replication_t) * nof_port);
