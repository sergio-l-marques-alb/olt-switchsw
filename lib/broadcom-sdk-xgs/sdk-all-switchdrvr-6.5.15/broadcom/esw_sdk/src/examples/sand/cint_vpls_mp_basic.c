 ...
 * int in_port = 200;
 * int out_port = 201;
 * rv = vpls_basic_main(unit,ac_port,pwe_port);
 * print rv;
 *
 *
    ...
    int unit)
{
     ...
     */
    label_array[0].label = cint_vpls_basic_info.mpls_tunnel_label;
    if (is_device_or_above(unit, JERICHO2))
    {
        label_array[0].flags = BCM_MPLS_EGRESS_LABEL_WITH_ID;
        BCM_L3_ITF_SET(label_array[0].tunnel_id, BCM_L3_ITF_TYPE_LIF, cint_vpls_basic_info.mpls_tunnel_id);
         ...
         */
        label_array[0].l3_intf_id = cint_vpls_basic_info.core_arp_id;
    ...
    int unit)
{
    ...
    mpls_port.egress_label.label = cint_vpls_basic_info.pwe_label;
    ...
    bcm_vpn_t vpn)
{
    ...
    mpls_port.match_label = cint_vpls_basic_info.pwe_label;
    if (soc_property_get(unit, "mpls_termination_label_index_enable", 0))
    {
    ...
    bcm_gport_t port_id)
{
    ...
    vlan_port.egress_vlan = cint_vpls_basic_info.vpn;
    vlan_port.vsi = 0;
    vlan_port.flags = 0;
...
vpls_mp_basic_ive_swap(int unit, int port)
{
    ...
    if(is_device_or_above(unit, JERICHO2))
    {
        rv = bcm_port_class_set(unit, cint_vpls_basic_info.mpls_port_id_ingress, bcmPortClassIngress, 507);
        if (rv != BCM_E_NONE)
        {
