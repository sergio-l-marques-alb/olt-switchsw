 ...
 */
void l2_ivl_mac_table_init(int unit) {    

    /** Add IVL entries */
    l2_entry_extended_add(unit, mac1, vsi1, 0, 0, 0, vlan1, 0);
    l2_entry_extended_add(unit, mac1, vsi2, 0, 200, 0, vlan1, 0);
    l2_entry_extended_add(unit, mac1, vsi1, 0, 201, 0, vlan2, 0);
    l2_entry_extended_add(unit, mac2, vsi1, 0, 202, 0, vlan1, group1);
    l2_entry_extended_add(unit, mac2, vsi1, 0, 203, 0, vlan2, group2);

    /** Add SVL entries */
    l2_entry_extended_add(unit, mac1, vsi1, 0, 0, 0, 0, 0);
    l2_entry_extended_add(unit, mac2, vsi1, 0, 0, 201, 0, group1);
    l2_entry_extended_add(unit, mac2, vsi2, 0, 0, 202, 0, group2);
}
...
int l2_entry_extended_add(int unit, bcm_mac_t mac, bcm_vlan_t vlan, int dest_type, int dest_gport, int dest_mc_id, int modid, int group){
    return l2_entry_add_internal(unit, mac, vlan, dest_type, dest_gport, dest_mc_id, modid, group);
}

int l2_entry_add(int unit, bcm_mac_t mac, bcm_vlan_t vlan, int dest_type, int dest_gport, int dest_mc_id){
    return l2_entry_add_internal(unit, mac, vlan, dest_type, dest_gport, dest_mc_id, 0, 0);
}
 ...
 *              1 for MC Group
 */
int l2_entry_add_internal(int unit, bcm_mac_t mac, bcm_vlan_t vlan, int dest_type, int dest_gport, int dest_mc_id, int modid, int group){
    int rv = BCM_E_NONE;
    bcm_l2_addr_t l2_addr;
    ...
    int unit)
{
    ...
    rv = bcm_multicast_create(unit, BCM_MULTICAST_TYPE_L2 | BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, &l2_mact_table_info1.mc_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_multicast_add(unit, l2_mact_table_info1.mc_id, BCM_MULTICAST_TYPE_L2 | BCM_MULTICAST_INGRESS_GROUP, 2, replications);
    if (rv != BCM_E_NONE)
    {
