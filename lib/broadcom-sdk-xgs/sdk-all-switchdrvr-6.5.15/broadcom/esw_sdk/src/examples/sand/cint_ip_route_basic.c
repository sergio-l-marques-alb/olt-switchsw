    ...
    int out_port)
{
    ...
    if (rc != BCM_E_NONE)
    {
        printf("%s(): Error, bcm_vlan_port_create vlan_port %d\n",proc_name, vlan_port);
        return rc;
    }
 ...
 * This procedure is for JER2 only!
 *
 * Route:
 * packet to send:
 *  - in port = in_port
 ...
 * 'redir_encap_id' is the identifier of the allocated ARP-ID. Encap id for jer2 must be > 2k
 */
/*
 * example:
 int unit = 0;
    ...
    int redir_encap_id)
{
    ...
    {
        rv = BCM_E_PARAM;
        printf("%s(): Error, this procedure is for JER2 only!\n",proc_name);
        return rv;
    }
    /*
     * 1. Set In-Port to In ETh-RIF
