    ...
    int pweP)
{
    ...
    if (is_device_or_above(tx_unit, JERICHO2)) {
       BCM_L3_ITF_SET(fec_id_for_cross_connect, BCM_L3_ITF_TYPE_FEC, 0xa000);
    } else {
    ...
    /** 2nd FEC for tunnel **/
    if (is_device_or_above(tx_unit, JERICHO2)) {
       BCM_L3_ITF_SET(pwe_term->egress_tunnel_if, BCM_L3_ITF_TYPE_FEC, 0x10000);
    } else {
    ...
    int pweP)
{
    ...
    if (!vswitch_vpws_params_initialized)
    {
         ...
         * Init parameters
         */
        vswitch_vpws_params_init(tx_unit, rx_unit, &vswitch_vpws_params, acP, pweP);
    }

    if (has_cw) {
        /* JR2 does not support sequencing and supports only 0 in ControlWord */
        int control_word = is_device_or_above(rx_unit, JERICHO2) ? 0 : 0xffffffff;
