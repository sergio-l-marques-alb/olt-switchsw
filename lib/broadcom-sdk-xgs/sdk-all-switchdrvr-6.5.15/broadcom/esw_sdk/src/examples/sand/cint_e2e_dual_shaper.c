...
int nof_color_groups = *(dnxc_data_get(c6_unit, "sch", "se", "nof_color_group", NULL));
...
int e2e_scheduler_elements_color_set_get(int unit, bcm_gport_t gport, int group)
{
    int rv = BCM_E_NONE;
    int group_get, def_group;

    /** get current color */
    rv = bcm_cosq_control_get(c6_unit, gport, 0, bcmCosqControlGroup, &def_group);
    if (rv != BCM_E_NONE)
    {
         return(rv);
    }
    ...
    /** set requested color */
    rv = bcm_cosq_control_set(c6_unit, gport, 0, bcmCosqControlGroup, group);
    if (rv != BCM_E_NONE)
    {
         return(rv);
    }
    rv = bcm_cosq_control_get(c6_unit, gport, 0, bcmCosqControlGroup, &group_get);
    if (rv != BCM_E_NONE)
    {
         return(rv);
    }
    ...
    if (group_get != group)
    {
        printf("Color group set is %d but color group get is %d\n", group, group_get);
        return BCM_E_PARAM;
    }

    /** return to default color */
    rv = bcm_cosq_control_set(c6_unit, gport, 0, bcmCosqControlGroup, def_group);
    if (rv != BCM_E_NONE)
    {
         return(rv);
    }
...
int e2e_scheduler_elements_verify_color(int port_idx)
{
    int rv = BCM_E_NONE;
    int idx = port_idx;
    int group = sal_rand() % nof_color_groups;
    bcm_gport_t gport_list[] = {c6_gport_ucast_scheduler_fq0[idx], c6_gport_ucast_scheduler_fq1[idx], c6_gport_ucast_scheduler_cir[idx], c6_gport_ucast_scheduler_pir[idx]};
    int gport_i, nof_gports = 4;

    if (!is_dnx)
    {
        printf("Manual coloring of scheduler elements is only supported for DNX family\n");
        return BCM_E_UNAVAIL;
    }

    for (gport_i = 0; gport_i < nof_gports; ++gport_i)
    {
        rv = e2e_scheduler_elements_color_set_get(c6_unit, gport_list[gport_i], group);
        if (rv != BCM_E_NONE)
        {
             return(rv);
        }
