/*
 * $Id: cint_ipmc_route_examples.c, Exp $
    ...
    int vrf)
{
    ...
    if (is_device_or_above(unit, JERICHO2) &&
        (*dnxc_data_get(unit, "elk", "application", "ipv6", NULL)))
    {
        flags = BCM_IPMC_IP6 | BCM_IPMC_SOURCE_PORT_NOCHECK | BCM_IPMC_TCAM;
    }
    else
    {
        flags = BCM_IPMC_IP6 | BCM_IPMC_SOURCE_PORT_NOCHECK;
    }
    ...
    uint8 change_vlan)
{
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        mreps[0] = 0;
        mreps[1] = 1;
        rv = bcm_fabric_multicast_set(0, mc_id, 0, 2, mreps);
        if (rv != BCM_E_NONE) {
    ...
    int out_port3)
{
    ...
    rv = bcm_vlan_gport_add(unit, vid_in_drop, cint_ipmc_info.in_port + 1, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_vlan_gport_add \n");
        return rv;
    ...
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.port = cint_ipmc_info.in_port + 1;
    vlan_port.criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;
    ...
    rv = bcm_vlan_gport_add(unit, vid_in_flood, cint_ipmc_info.in_port + 1, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error in bcm_vlan_gport_add \n");
        return rv;
    ...
    set_multicast_replication(&replications[2], out_ports[2], -1);

    rv = bcm_multicast_add(unit, cint_ipmc_info.mc_group, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_TYPE_L2, 3, replications);
    if (rv != BCM_E_NONE)
    {
