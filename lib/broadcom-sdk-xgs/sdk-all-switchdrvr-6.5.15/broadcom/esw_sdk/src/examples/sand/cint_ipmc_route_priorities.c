    ...
    int out_port2)
{
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to TCAM table (full mask), \n");
        return rv;
    }
    rv = add_ipv6_ipmc(unit, dip_addr, all_valid, sip_addr, part_mask, cint_ipmc_info.intf_in, cint_ipmc_info.vrf,
                       mc_group[1], 0, flags);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to TCAM table (partial mask), \n");
        return rv;
    }
    ...
    rv = add_ipv6_ipmc(unit, dip_addr, all_valid, none_valid, none_valid, 0, cint_ipmc_info.vrf, mc_group[3], 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to LPM table, \n");
        return rv;
    }
    rv = add_ipv6_ipmc(unit, dip_addr, part_mask, none_valid, none_valid, 0, cint_ipmc_info.vrf, mc_group[4], 0, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to LPM table (prefix on G), \n");
        return rv;
    }
    rv = add_ipv6_ipmc(unit, dip_addr, none_valid, none_valid, none_valid, 0, cint_ipmc_info.vrf, 0, fec_id, 0);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to LPM table (prefix on G), \n");
        return rv;
