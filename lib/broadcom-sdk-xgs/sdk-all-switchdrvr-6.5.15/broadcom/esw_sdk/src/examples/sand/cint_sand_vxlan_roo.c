/*
 * $Id: cint_vxlan_roo_basic.c,v 1.0 Broadcom SDK $
 ...
 * 0x00000000CD1D000C000200008100800A080045280069000000003C111D1CA00F1011A10F10115000555500550000080000000013880001000000CD1D000C000200228100000F080045000035000000007F00B8C0010203047FFFFF03000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F20
 *
 *
 ...
 * Received packets on unit 0 should be: 
 *  0x00000000CD1D000C000200008100800A080045280069000000003C111D1CA00F1011A10F10115000555500550000080000000013880021000000CD1D000C000200228100000F080045000035000000007F00B8C3010203047FFFFF00000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F20
 * 
 * 
    ...
    int provider_port)
{
    ...
    cint_vxlan_basic_info.vxlan_egress_if_is_ecmp = TRUE;
    if (is_device_or_above(unit,JERICHO2)) {
        ip_tunnel_basic_info.encap_is_ecmp[0] = TRUE;
        ip_tunnel_basic_info.encap_is_hierarchy[0] = TRUE; 
    }
    ip_tunnel_basic_info.encap_is_ecmp[1] = TRUE;
    ...
    int unit)
{
     ...
     */
    int vrid = 0x77;
    rv = bcm_l3_vrrp_config_add(unit, BCM_L3_VRRP_IPV4 | BCM_L3_VRRP_IPV6, ip_tunnel_basic_info.access_eth_rif, vrid);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_l3_vrrp_config_add. \n");
        return rv;
    }

    rv = bcm_l3_vrrp_config_add(unit, BCM_L3_VRRP_IPV4 | BCM_L3_VRRP_IPV6, ip_tunnel_basic_info.provider_eth_rif, vrid);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_l3_vrrp_config_add. \n");
        return rv;
    }

    rv = bcm_l3_vrrp_config_add(unit, BCM_L3_VRRP_IPV4 | BCM_L3_VRRP_IPV6, ip_tunnel_basic_info.provider_native_eth_rif, vrid);
    if (rv != BCM_E_NONE) {
        printf("Error, in bcm_l3_vrrp_config_add. \n");
        return rv;
    }
...
int vxlan_roo_eve_action(int unit, int out_port, int out_vlan, int vsi, int is_native)
{
    ...
    if (is_adv_vt) {
        ...
        action.outer_tpid = 0x8100;
        action.priority = ip_tunnel_basic_info.egr_qos_id;
    ...
    int add_routing_table_entry)
{
    ...
    vxlan_roo_init(unit, access_port, provider_port);
