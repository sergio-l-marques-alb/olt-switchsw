/*
 * $Id: cint_ipmc_route_basic.c, Exp $
    ...
    int unit)
{
    ...
    rv = add_ipv6_ipmc(unit, cint_ipmc_info.ipv6_dip_addr, part_mask, no_sip, no_sip, 0,
                       cint_ipmc_info.vrf, cint_ipmc_info.mc_group, 0, 0);
    if (rv != BCM_E_NONE)
    {
    ...
    if (rv != BCM_E_NONE)
    {
        printf("Error, in function add_ipv6_ipmc to TCAM table, \n");
        return rv;
    }
    return rv;
}
