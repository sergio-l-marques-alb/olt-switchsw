...
int cint_dnx_cpu_pkts_config_fap(int unit, int fe_id)
{
    ...
    bshell(unit, "dbal entry commit table=IQS_CREDIT_REQUEST_PUSH_QUEUE PROFILE_ID=15 ENABLE=1");

    /* Configure profile 15 as a push queue (queue that doesn't need credits */
    ...
    credit_request_attr.credit_size = *(dnxc_data_get(unit, "iqs", "credit", "worth", NULL));
    credit_request_attr.flags = BCM_COSQ_DELAY_TOLERANCE_PRESET_ATTR_F_SLOW_ENABLED;
    credit_request_attr.rate = 10;
    rv = bcm_cosq_delay_tolerance_preset_get(unit, &credit_request_attr, &credit_request_profile);
    if (rv != BCM_E_NONE)
    {
        printf("Error in cint_dnx_cpu_pkts_config_fap - bcm_cosq_delay_tolerance_preset_get failed\n");
        return BCM_E_FAIL;
    }
    rv = bcm_cosq_delay_tolerance_level_set(unit, 15, &credit_request_profile);
    if (rv != BCM_E_NONE)
    {
        printf("Error in cint_dnx_cpu_pkts_config_fap - bcm_cosq_delay_tolerance_level_set failed\n");
        return BCM_E_FAIL;
    }
    ...
    {
        config.queue_atrributes[cosq].delay_tolerance_level = 15;
