    ...
    int p2_sys_port)
{
    ...
    /** Convert the sysport to local port */
    multi_dev_system_port_to_local(ingress_unit, ce1_sys_port, &ce1_port);
    multi_dev_system_port_to_local(ingress_unit, ce3_sys_port, &ce3_port);
    multi_dev_system_port_to_local(egress_unit, p1_sys_port, &p1_port);
    multi_dev_system_port_to_local(egress_unit, p2_sys_port, &p2_port);

    evpn_ingress_port_array[0] = cint_evpn_info.ce1_port = BCM_GPORT_LOCAL_GET(ce1_port);
    evpn_ingress_port_array[1] = cint_evpn_info.ce3_port = BCM_GPORT_LOCAL_GET(ce3_port); 
    evpn_egress_port_array[0] = cint_evpn_info.p1_port = BCM_GPORT_LOCAL_GET(p1_port);
    evpn_egress_port_array[1] = cint_evpn_info.p2_port = BCM_GPORT_LOCAL_GET(p2_port);
...
int multi_dev_evpn_create_egress_tunnels(int ingress_unit, int egress_unit, int config_wide_data)
{
    ...
    rv = multi_dev_evpn_set_egress_tunnel(ingress_unit, egress_unit, add_to_flooding_domain, 1,
                                ...
                                egress_action, cint_evpn_info.p1_sys_port, &cint_evpn_info.pe1_pe3_iml_no_esi_tunnel_id,cint_evpn_info.pe1_linker_layer_encap_id);
    ...
    rv = multi_dev_evpn_set_egress_tunnel(ingress_unit, egress_unit, add_to_flooding_domain, 1, \
                            ...
                            egress_action, cint_evpn_info.p1_sys_port, &cint_evpn_info.pe1_pe4_iml_no_esi_tunnel_id, cint_evpn_info.pe1_linker_layer_encap_id);
    ...
    rv = multi_dev_evpn_set_egress_tunnel(ingress_unit, egress_unit, add_to_flooding_domain, 1,  \
                            ...
                            egress_action, cint_evpn_info.p2_sys_port, &cint_evpn_info.pe1_pe2_iml_esi_tunnel_id, cint_evpn_info.pe2_linker_layer_encap_id);
    ...
    rv  = multi_dev_evpn_set_egress_tunnel(ingress_unit, egress_unit, add_to_flooding_domain, 0,  \ /* 3.3, 7.5 */
                            ...
                            egress_action, 0, &cint_evpn_info.pe1_pe3_evpn_lsp_tunnel_id, cint_evpn_info.pe1_linker_layer_encap_id);
    if (rv != BCM_E_NONE) {
        printf("Error: %d, in evpn_set_egress_tunnel\n", rv);
        return rv;
    }
...
int evpn_create_ingress_tunnels(int unit){
    ...
    evpn_is_indexed_termination = is_device_or_above(0, JERICHO2) ? 0 : soc_property_get(unit , "mpls_termination_label_index_enable",0);
...
int multi_dev_evpn_routing_into_evpn(int ingress_unit, int egress_unit)
{
    ...
    */
    ...
    if (ingress_unit != egress_unit) {
        ...
        rv = bcm_multicast_create(ingress_unit, BCM_MULTICAST_WITH_ID|BCM_MULTICAST_EGRESS_GROUP|BCM_MULTICAST_TYPE_L3, &mc_id); /* 8.7 */
        if (rv != BCM_E_NONE) {
...
int evpn_create_linker_layer_entries(int unit){
    ...
    if (evpn_add_frr_label && !is_device_or_above(unit,JERICHO2)) {
        ...
        /** JR2 use ARP_AC format, since no esem access can be used for outer AC.*/
    ...
    if (!is_device_or_above(unit,JERICHO2) &&
         is_device_or_above(unit,JERICHO_PLUS) && evpn_add_frr_label) { /* 3.7, 4.9, 7.9, 8.10 */
...
/** Get egress replication ports*/
int sand_erp_port_get(int unit, int nof_erp_ports, int *erp_sysport)
{
    int rv = BCM_E_NONE, count_erp = 0, port_id;
    bcm_gport_t erp_local_port_gport[nof_erp_ports], erp_sysport_gport;
    ...
    for (port_id = 0; port_id < nof_erp_ports; port_id++) {
        ...
        erp_sysport[port_id] = erp_sysport_gport;
        printf("count_erp=%d, erp_local_port_gport=0x%x, erp_sysport[%d]=0x%x.\n", count_erp, erp_local_port_gport[port_id], port_id, erp_sysport[port_id]);
    }
...
int multi_dev_evpn_add_routing_into_evpn_multicast_entry(int ingress_unit, int egress_unit)
{
    ...
    flags = BCM_MULTICAST_WITH_ID|BCM_MULTICAST_EGRESS_GROUP|BCM_MULTICAST_TYPE_L3;
    ...
    flags = BCM_MULTICAST_WITH_ID|BCM_MULTICAST_INGRESS_GROUP|BCM_MULTICAST_TYPE_L3;
    ...
    if (!is_device_or_above(ingress_unit, JERICHO2)) {
        ...
        BCM_GPORT_LOCAL_SET(reps[0].port, cint_evpn_info.ce3_port);
        reps[2].encap1 = cint_evpn_info.pe1_rif; /* OutRif */
    ...
    /** Get egress MC port.*/
    int egr_mc_sys_port[2] = { 0 };
    rv = sand_erp_port_get(egress_unit, 2, egr_mc_sys_port);
    reps[0].port = egr_mc_sys_port[0];
    reps[0].encap1 = is_device_or_above(egress_unit, JERICHO2) ? egr_mc_id : BCM_MULTICAST_L3_GET(egr_mc_id);

    reps[1].port = egr_mc_sys_port[1];
    ...
    rv = bcm_multicast_set(ingress_unit, cint_evpn_info.mc_id, flags, 3, reps); /* 8.7 */
    if (rv != BCM_E_NONE) {
...
int evpn_create_eve(int unit){
    ...
    if (is_device_or_above(unit,JERICHO_PLUS)) {
        ...
        rv = vlan_port_translation_set(unit, p1_outer_vlan, p1_inner_vlan, cint_evpn_info.p1_egr_vport_id, g_eve_edit_utils.edit_profile, 0);
        if (rv != BCM_E_NONE) {
            printf("Error, vlan_port_translation_set\n");
            return rv;
        }
        rv = vlan_port_translation_set(unit, p2_outer_vlan, p2_inner_vlan, cint_evpn_info.p2_egr_vport_id, g_eve_edit_utils.edit_profile, 0);
        if (rv != BCM_E_NONE) {
            printf("Error, vlan_port_translation_set\n");
            return rv;
        }
