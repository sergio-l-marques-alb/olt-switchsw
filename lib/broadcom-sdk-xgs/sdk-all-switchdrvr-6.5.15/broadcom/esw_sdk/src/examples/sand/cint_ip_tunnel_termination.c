/*
 * $Id: cint_ip_tunnel_termination.c,v 1.15 Broadcom SDK $
 $Copyright: (c) 2018 Broadcom.
 Broadcom Proprietary and Confidential. All rights reserved.$ File: cint_ip_tunnel_termination.c Purpose: Various examples for IPTunnels.
 */
 ...
 * ########################################################################################################################
 *
 * I BASIC exact-match/full-key lookups:
 * *************************************
 *  J2: <SIP, DIP, VRF, TunnelType> in dbal table IPV4_TT_P2P_EM_BASIC (VTT2)
 *  J1: <SIP, DIP, VRF, TunnelType> in TCAM database (TT Mode 6)
 *
 * II DIP/SIP Longest-prefix-match lookups with subnet masking:
  * ***********************************************************
 *  J2: <SIP, DIP, VRF, TunnelType> in dbal table IPV4_TT_TCAM_BASIC (VTT3)
 *  J1: <SIP, DIP, VRF, TunnelType> in TCAM database (TT Mode 6)
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming port (provider_port)
 *  2) Get the default's VSI per port
 *  3) Get the ETH-RIF, equal to VSI. (eth_rif_intf_provider)
 *  4) ETH-RIF is enabled for routing.
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination lif from VTT lookup with key: DIP, SIP, VRF.
 *  8) Tunnel Ipv4 is terminated.
 *  9) From the tunnel termination LIF, we get the VRF
 *  10) Packet is routed according to Ipv4 header
 *
 * Main function for ip tunnel termination of Ipv4 tunnel in Jericho 2
 ...
 * where the incomming packet arrives through a port and In-ETH-RIF configured for a different VRF 
 ...
 *  8) Tunnel Ipv4 is terminated.
 *  9) From the tunnel termination LIF, we get the VRF
 *  10) Packet is routed according to Ipv6 header
 *
 * Main function for termination of Gre4oIpv4 tunnel on IPv6 forwarded packets in Jericho 2
 * The function calls ip_tunnel_termination_open_route_interfaces for ports and logical interface
 * initialization, then creates the tunnel terminator with bcmTunnelTypeGreAnyIn4 type and adds
 * route entries for forwarding the inner Ipv6 headers.
 *
 * The configuration also sets up interfaces and adds forwarding entries for faulty case scenarios
 * where the incomming packet's tunnel SIP/DIP does not match the configured tunnel's ones and hence
 * the tunneled packet is forwarded onwards on the tunnel IPv4 DIP to a different next-hop-mac.
 *
 * Inputs: unit          - relevant unit;
 *         provider_port - incoming port;
 *         access_port   - outgoing port;
 ...
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination lif from VTT lookup with key: DIP, SIP, VRF in TCAM.
 *  8) Tunnel Gre4oIpv4 is terminated.
 *  9) From the tunnel termination LIF, we get the VRF
 *  10) Packet is routed according to MPLS header
 *
 * Main function for ip tunnel termination of Gre4oIpv4 tunnel in Jericho 2
    ...
    int access_port)
{
    ...
    else
    {
        ...
        /** Create FEC and configure its: Outlif (ARP), Destination port, Global Out-EthRIF */
        cint_ip_tunnel_basic_info.fec_id = 50003;
        rv = l3__egress_only_fec__create(unit, cint_ip_tunnel_basic_info.fec_id, mpls_tunnel_id, 0, access_port, 0);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, l3__egress_only_fec__create\n", proc_name);
 ...
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming port (200)
 *  2) Get the default's VSI per port
 *  3) Get the ETH-RIF, equal to VSI. (eth_rif_intf_provider)
 *  4) ETH-RIF is enabled for routing.
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination lif from VTT3 TCAM lookup with key: DIP, SIP, VRF, tunnel type.
 *  8) Tunnel UdpoIpv4 is terminated.
 *  9) From the tunnel termination LIF, we get the VRF
 *  10) Packet is routed according to Ipv4/Ipv6 header
 *
 ...
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming provider_port (200)
 *  2) Get the default's VSI per port
 *  3) Get the ETH-RIF, equal to VSI. (eth_rif_intf_provider)
 *  4) ETH-RIF is enabled for routing.
 *  5) Tunnel's ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) Get the tunnel termination LIF from VTT3 SEM lookup with key: DIP, VRF, tunnel type
 *  8) Tunnel UdpoIpv4 is terminated.
 *  9) From the tunnel termination LIF FoDo property, we get the VRF
 *  10) Packet is routed according to MPLS label resolved in FWD to FEC
 *
 * Main function for udp  tunnel termination of mpls-forwarded packet scenario in Jericho 2
...
/*
 * NO-TERMINATION:
 * Extra Interfaces initialisation function for faulty ip tunnel termination scenarios in Jericho 2
 * The function ip_tunnel_termination_open_route_interfaces implements the basic configuration, 
 * on top of witch unsuccsesful tunnel termination scenarios are built. Forwarding is to be done
