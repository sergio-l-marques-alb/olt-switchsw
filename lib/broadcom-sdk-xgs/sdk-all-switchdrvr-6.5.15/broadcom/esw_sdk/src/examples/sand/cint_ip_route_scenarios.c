    ...
    int out_sysport)
{
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        sand_utils_l3_arp_s_common_init(unit, 0, &l3_arp, 0, 0, encap_id[0], next_hop_mac, out_vlan);
        rv = sand_utils_l3_arp_create(unit, &l3_arp);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object ARP only\n");
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        /* Create ARP and set its properties */
        sand_utils_l3_arp_s_common_init(unit, 0, &l3_arp, 0, 0, encap_id[1], next_hop_mac2, out_vlan);
        rv = sand_utils_l3_arp_create(unit, &l3_arp);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create egress object ARP only\n");
    ...
    int out_port)
{
    ...
    rv = add_host_ipv4(unit, host+1, vrf, 0, encap_id, gport);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function add_host_ipv4(), \n",proc_name);
        return rv;
        ...
        if (i == 4)
        {
            ...
            if (rc != BCM_E_NONE)
            {
                if (lpm_raw_data_enabled) {
                    ...
                    rc = add_ipv6_ipmc(unit, route4 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[0], vrf);
                }
                if (rc != BCM_E_NONE) {
            ...
            if (second_unit>=0)
            {
                ...
                if (rc != BCM_E_NONE) {
                    if (lpm_raw_data_enabled) {
                        ...
                        rc = add_ipv6_ipmc(second_unit, route4 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[1], vrf);
                    }
                    if (rc != BCM_E_NONE) {
                        printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                        return BCM_E_FAIL;
        ...
        else if (i == 5)
        {
            ...
            if (rc != BCM_E_NONE)
            {
                if (lpm_raw_data_enabled) {
                    ...
                    rc = add_ipv6_ipmc(unit, route5 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[0], vrf);
                }
                if (rc != BCM_E_NONE) {
                    printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                    return BCM_E_FAIL;
            ...
            if (second_unit>=0)
            {
                ...
                if (rc != BCM_E_NONE) {
                    if (lpm_raw_data_enabled) {
                        ...
                        rc = add_ipv6_ipmc(second_unit, route5 /* MC DIP */, route0 /* UC SIP */, in_vlan, mc_id[1], vrf);
                    }
                    if (rc != BCM_E_NONE) {
                        printf("Error. add_ip6mc_entry failed: $rc\n", rc);
                        return BCM_E_FAIL;
                ...
                if (second_unit<0) {
                    if (i == 0) {
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route0, mask, vrf, 0, 0, egress_intf[0]);
                        ...
                        if (is_device_or_above(unit,JERICHO)) {
                            if (lpm_raw_data_enabled) {
                                ...
                                sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route0, mask, vrf_public, 0, 0, egress_intf[1]);
                                rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                            }
                        }
                    }
                        ...
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route1, mask, vrf, 0, 0, egress_intf[0]);
                            rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                        }
                    }
                    else if (i == 2) {
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route2, mask, vrf, 0, 0, egress_intf[0]);
                            rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                        }
                    }
                    else { /*i == 3*/
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route3, mask, vrf, 0, 0, egress_intf[0]);
                            rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                        }
                    }
                }
                else /* second_unit>=0 */
                {
                    if (i == 0) {
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route0, mask, vrf, 0, 0, egress_intf[i%MAX_PORTS]);
                            rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                        }
                    } else if (i == 1) {
                        if (!lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(unit, 0, &route_ipv6_structure, route1, mask, vrf, 0, 0, egress_intf[i%MAX_PORTS]);
                            rc = sand_utils_l3_route_ipv6_add(unit, &route_ipv6_structure);
                        }
                    } else if (i == 2) {
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(second_unit, 0, &route_ipv6_structure, route2, mask, vrf, 0, 0, egress_intf[i%MAX_PORTS]);
                        ...
                        if (lpm_raw_data_enabled) {
                            ...
                            sand_utils_l3_route_ipv6_s_common_init(second_unit, 0, &route_ipv6_structure, route3, mask, vrf, 0, 0, egress_intf[i%MAX_PORTS]);
                            rc = sand_utils_l3_route_ipv6_add(second_unit, &route_ipv6_structure);
                        }
                    }
                }
 ...
 *  - On JER2, mtu size is set via bcm_rx_mtu_set
 */
int
    ...
    int out_vlan)
{
    ...
    int ipv4_layer_protocol = 2;             /* value of DBAL_ENUM_FVAL_LAYER_TYPES_IPV4 */
    int ipv6_layer_protocol = 3;             /* value of DBAL_ENUM_FVAL_LAYER_TYPES_IPV6 */
    ...
    int redir_port)
{
     ...
     */
    rv = field_util_fg_context_attach(unit, 0, &fg_t_1, 0);
    if (rv != BCM_E_NONE)
    {
