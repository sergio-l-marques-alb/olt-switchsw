...
int bfd_ipv6_basic_param_init(int unit) {
    ...
    if(is_device_or_above(unit, JERICHO2)) {
        if(bfd_local_discr != 0)
        {
            local_disc = bfd_local_discr;
        }
        else
        {
            local_disc = 0x2008;
...
int bfd_ipv6_run_with_defaults(int unit, int endpoint_id, int is_acc, int is_single_hop, bcm_bfd_auth_type_t auth_type, int auth_index, int type, int is_trunk, int punt_rate, int use_my_bfd_dip) {
    ...
    if (is_device_or_above(unit,JERICHO2)) {
        rv = basic_example_ipv6(unit,bfd_in_port[0],bfd_out_port[0],0,use_my_bfd_dip);
    } else {
    ...
    if (is_trunk) {
        ...
        if(is_device_or_above(unit, JERICHO2)) {
            bfd_endpoint_info.tx_gport = 0x6C000000 + bfd_out_port[0];
        } else {
    ...
    if (is_device_or_above(unit,JERICHO2)) {
        bfd_endpoint_info.ipv6_extra_data_index = 8195;
    }

    if (is_device_or_above(unit,JERICHO2)) {
        bfd_endpoint_info.egress_if = 0x40000000 | 0x1384;
    } else {
    ...
    if(is_device_or_above(unit,JERICHO2) && use_my_bfd_dip) {
