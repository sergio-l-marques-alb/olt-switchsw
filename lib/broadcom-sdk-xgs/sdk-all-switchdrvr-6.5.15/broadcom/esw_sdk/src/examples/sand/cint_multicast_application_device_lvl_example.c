    ...
    bcm_vlan_t vlan_id)
{
    ...
    /*
     * Mapping from Local port to PP port. PP-Port for the PTCH header might be different than the local-port value 
     */
    ...
    /*
     * Add VLAN Gport for the In-port and VLAN ID 
     */
    pp_port = mapping_info.pp_port;
    rv = bcm_vlan_gport_add(unit, vlan_id, pp_port, 0);
    if (rv != BCM_E_NONE)
    {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
