    ...
    ip_compatible_mc_s * param)
{
    ...
    else
    {
        ...
        tpid_info.tpid_value = g_ip_compatible_mc.tpid_value.s_tag;
        ...
        tpid_info.tpid_value = g_ip_compatible_mc.tpid_value.c_tag;
        rv = bcm_switch_tpid_add(unit, 0, &tpid_info);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in bcm_switch_tpid_add(tpid_value=0x%04x)\n", tpid_info.tpid_value);
    ...
    else
    {
        ...
        if (*dnxc_data_get(unit, "l2", "hw_bug", "eth_qual_is_mc", NULL)) {
