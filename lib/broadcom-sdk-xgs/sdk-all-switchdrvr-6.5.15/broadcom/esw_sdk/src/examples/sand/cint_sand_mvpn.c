...
/*
 * Create a Entry, which will be built the recycle header for the copy going on the 2nd pass. (Relevant to JR2)
 * unit - relevant unit
 * recycle_entry_encap_id - returned encap id.
 */
int mvpn_dnx_create_recycle_entry (
    int unit,
    int *recycle_entry_encap_id)
{
    int rv = BCM_E_NONE;
    bcm_mac_t recycle_mac_address = {0x00, 0x0c, 0x00, 0x02, 0x00, 0x00};
    bcm_l2_egress_t recycle_entry;
    bcm_l2_egress_t_init(&recycle_entry);

    if (!is_device_or_above(unit, JERICHO2)) {
        printf("Unavailable, setting recycle entry relevant only to DNX device\n");
        return BCM_E_UNAVAIL;
    }

    recycle_entry.ethertype = 0x0800;
    recycle_entry.outer_tpid = 0x8100;
    recycle_entry.inner_tpid = 0x9100;
    recycle_entry.dest_mac = recycle_mac_address;
    ...
    int *tunnel_intf)
{
    ...
    if (!is_device_or_above(unit, JERICHO2))
    {
        /* For Jer2, vlan is an ARP property */
    ...
    int unit)
{
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        rv = mvpn_dnx_create_recycle_entry(unit, &cint_mvpn_info.recycle_entry_encap_id);
        if (rv != BCM_E_NONE)
        {
            printf("Error, create_recycle_entry \n");
            return rv;
        }
        printf("%s:\t recycle entry(0x%8.8) created \n", proc_name, cint_mvpn_info.recycle_entry_encap_id);
    }
    ...
    int unit )
{
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        ...
        printf("%s:\t ip_tunnel_terminator_create term_tunnel_id(0x%8.8x)\n", proc_name, term_tunnel_id);
    ...
    printf("%s:\t MC configured ipmc_index(0x%8.8x)\n", proc_name, ipmc_index);
    ...
    printf("%s:\t mc group entry(0x%8.8x) added to routing table (0x8.8x)\n", proc_name, ipmc_index, mc_ip);
    ...
    int unit)
{
    ...
    printf("%s:\t mvpn_ip_tunnel_initiator_create tunnel_encap_id(0x%8.8x)\n", proc_name, tunnel_encap_id);
