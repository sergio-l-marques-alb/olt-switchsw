...
/* $Id: cint_sand_l2_learn_event_parsing.c,v 1.0 2018/5/3 14:37:38 $
...
/* Added to be compatible with JER2 */
...
int show_mact_and_rpi(int unit){
    ...
    if (DEBUG_FLAG) {
        ...
        /* Remove this condition when this command is finished */
        if (!is_jericho2) {
...
int init_learn_event_parsing(int unit){
    ...
    if (is_jericho2) {        
        is_in_jericho2 = soc_property_get(unit , "system_headers_mode", 1);
    }
    ...
    /** Jericho 2 in Jericho 1 mode   */
    if (!is_in_jericho2 && is_jericho2) {
        valid_outlif_bit = 32;
        destination_system_port_flag = 0x10000;
        destination_multicast_flag = 0x40000;
        destination_flow_flag = 0x60000;
        destination_fec_flag = 0x20000;
        destination_shift = 0;
        destination_index_in_payload_buffer = 0;
...
int construct_buffer(int unit, int payload_type, int valid_outlif, int outlif, int valid_sid, int sid, int eei_cmd, int eei_data, int entry){
    ...
    if (is_jericho2) {
        is_in_jericho2 = *(dnxc_data_get(0,  "headers", "system_headers", "system_headers_mode", NULL));
    }

    if (is_in_jericho2) {
        lpkgv_shift = *(dnxc_data_get(0,  "l2", "olp", "lpkgv_shift", NULL));
        lpkgv_mask = *(dnxc_data_get(0,  "l2", "olp", "lpkgv_mask", NULL));
        dnx_dbal_fields_enum_value_get(0, "IRPP_FWD_STAGE_FORMATS", "FWD_MACT_RESULT_SINGLE_OUTLIF", &lpkgv_with_outlif);
        dnx_dbal_fields_enum_value_get(0, "IRPP_FWD_STAGE_FORMATS", "FWD_MACT_RESULT_NO_OUTLIF", &lpkgv_wo_outlif);
    }
