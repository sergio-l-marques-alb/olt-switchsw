/*
 * $Id: cint_sand_vswitch_vpls.c, Exp $
 ...
 * cint examples/sand/cint_sand_vswitch_vpls.c
 * cint
    ...
    int pwe_port_2)
{
    ...
    pwe_enc->mpls_port_id = 0x2001;
    /** In JR1, encap_id is needed in case of EGRESS_ONLY*/
    if (!is_device_or_above(unit, JERICHO2)) {
        pwe_enc->mpls_port_id = 0x2011;
        pwe_enc->encap_id = 0x2011;
    ...
    pwe_term->egress_tunnel_if = is_device_or_above(unit, JERICHO2) ? 50001 : 10001; /* learned FEC */
    ...
    pwe_enc->label = 3457;
    pwe_enc->label_flags |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
    pwe_enc->mpls_port_id = 0x2002;
    /** In JR1, encap_id is needed in case of EGRESS_ONLY*/
    if (!is_device_or_above(unit, JERICHO2)) {
        pwe_enc->mpls_port_id = 0x2012;
        pwe_enc->encap_id = 0x2012;
        pwe_enc->flags |= BCM_MPLS_PORT_ENCAP_WITH_ID;
    }
    pwe_enc->encap_access = bcmEncapAccessTunnel1;
    if (params->test_split_horizon)
    ...
    pwe_term->egress_tunnel_if = is_device_or_above(unit, JERICHO2) ? 50003 : 10003; /* learned FEC */
    pwe_term->forwarding_domain = params->vpn;
    ...
    vswitch_vpls_pwe_info_s * pwe_param)
{
    ...
    if (pwe_param->pwe_term.egress_tunnel_if)
    {
        ...
        {
            /*
             *In JR1, FEC carry pwe info which then links to tunnel encap, since
             *system header can't carry pwe and tunnel lif at the same time.
             */
            BCM_L3_ITF_SET(outlif_in_fec, BCM_L3_ITF_TYPE_LIF, pwe_param->pwe_encap.encap_id);
        }
        rv = vpls_create_fec_entry(unit, pwe_param->pwe_term.egress_tunnel_if,
                                   outlif_in_fec, pwe_param->pwe_port.port);
        if (rv != BCM_E_NONE)
        {
    ...
    if (!is_device_or_above(unit, JERICHO2))
    {
        pwe_param->pwe_encap.egress_tunnel_if = pwe_param->mpls_encap_tunnel[0].tunnel_id;
    }
    ...
    bcm_vpn_t vpn_id)
{
     ...
     */
    int mc_id = vpn_id;
    rv = multicast__open_mc_group(unit, &mc_id, BCM_MULTICAST_TYPE_L2);
    if (rv != BCM_E_NONE)
    {
    ...
    vswitch_vpls_pwe_info_s * pwe_param)
{
    ...
    int cuds[2];
    uint32 flags = BCM_MULTICAST_INGRESS_GROUP;
