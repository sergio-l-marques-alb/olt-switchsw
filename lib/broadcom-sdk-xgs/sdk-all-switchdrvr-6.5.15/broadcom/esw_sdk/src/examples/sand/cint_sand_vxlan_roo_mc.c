    ...
    int provider_port2)
{
    ...
    ip_tunnel_basic_info.term_bud = (is_device_or_above(unit,JERICHO2));
    ...
    int unit)
{
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        recycle_entry.dest_mac = g_vxlan_roo_mc.recycle_mac_address;
...
int vxlan_roo_mc_add_native_mc(int unit){
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ...
        mreps[0] = 0;
        mreps[1] = 1;
        rv = bcm_fabric_multicast_set(0, g_vxlan_roo_mc.native_ipmc, 0, 2, mreps);
        if (rv != BCM_E_NONE) {
 ...
 * add gport of type vlan-port to the multicast
 */
int vxlan_roo_mc_add_overlay_mc(int unit){
    int rv;
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        rv = bcm_multicast_add(unit, g_vxlan_roo_mc.overlay_ipmc, BCM_MULTICAST_INGRESS_GROUP, 1, &replications);
        if (rv != BCM_E_NONE)
        {
    ...
    int provider_port2)
{
    ...
    vxlan_roo_mc_init(unit, access_port1, access_port2, provider_port1, provider_port2);
    ...
    rv = vxlan_roo_mc_add_overlay_mc(unit);
    if (rv != BCM_E_NONE)
    {
