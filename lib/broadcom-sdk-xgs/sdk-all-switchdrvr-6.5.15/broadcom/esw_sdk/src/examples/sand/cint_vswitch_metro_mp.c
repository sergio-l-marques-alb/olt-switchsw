...
vswitch_metro_mp_multi_device_arad_run(int *units_ids, int nof_units, int known_mac_lsb, int known_vlan){
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        sysport_is_local = 0;
        rv = system_port_is_local(unit, vswitch_metro_mp_info.sysports[0], &local_port, &sysport_is_local);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in sysport_is_local().\n");
            return rv;
        }
        if(sysport_is_local == 1 || !is_device_or_above(unit,JERICHO2)) {
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = vswitch_add_port_multi_dev(unit, vsi1,vswitch_metro_mp_info.sysports[0], gport1);
        if (rv != BCM_E_NONE) {
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        sysport_is_local = 0;
        rv = system_port_is_local(unit, vswitch_metro_mp_info.sysports[1], &local_port, &sysport_is_local);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in sysport_is_local().\n");
            return rv;
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        rv = vswitch_add_port_multi_dev(unit, vsi1,vswitch_metro_mp_info.sysports[1], gport2);
        if (rv != BCM_E_NONE) {
            printf("Error, vswitch_add_port_multi_dev\n");
            return rv;
        }
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        sysport_is_local = 0;
        rv = system_port_is_local(unit, vswitch_metro_mp_info.sysports[2], &local_port, &sysport_is_local);
        if (rv != BCM_E_NONE)
        {
            printf("Error, in sysport_is_local().\n");
            return rv;
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        rv = vswitch_add_port_multi_dev(unit, vsi1,vswitch_metro_mp_info.sysports[2], gport3);
        if (rv != BCM_E_NONE) {
            printf("Error, vswitch_add_port_multi_dev\n");
            return rv;
        }
...
int vswitch_add_port_multi_dev(int unit, bcm_vlan_t  vsi, bcm_port_t phy_port, bcm_gport_t gport){
    int rv;
    int sysport_is_local = 0;
    int local_port = 0;
    
    rv = system_port_is_local(unit, phy_port, &local_port, &sysport_is_local);
    if (rv != BCM_E_NONE)
    {
        printf("Error, in sysport_is_local().\n");
        return rv;
...
vswitch_metro_mp_run_with_defaults_multi_device (int *units_ids, int nof_units , int sysport_1, int sysport_2, int sysport_3){
    ...
    if (nof_units > 1 && !is_device_or_above(units_ids[0],JERICHO2)){
        rv = l2_learning_run_with_defaults(units_ids, nof_units);
        if (rv != BCM_E_NONE){
