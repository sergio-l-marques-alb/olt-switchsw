    ...
    sand_device_type_t * device_type)
{
    ...
    *device_type =
        ...
        device_type_qax : (info.device == 0x8690) ? device_type_jericho2 : -1;
    return rv;
}
    ...
    int unit)
{
    ...
    bcm_gport_t mirr_dest_ids[max_dest_id - min_dest_id + 1];   /* will contain gport represantation of mirror
                                                                 * destinations */
    ...
    int nof_mc_groups = soc_property_get(unit,
                                         "multicast_egress_group_id_range_max", default_nof_mc_groups);
    ...
    unsigned int prob_shift, egr_prob_shift;
    int core, tm_port;
    uint16 egress_packet_copy_size = 128;
    uint16 packet_copy_size = (dev_type == device_type_jericho2 || dev_type == device_type_qax) ? 256 : 128;

    if (dev_type == device_type_jericho2)
    {
        uint32 *max_val = dnxc_data_get(unit, "snif", "ingress",
                                        "prob_max_val", NULL);
        prob_shift = *max_val;
    ...
    /** for jericho2 use system ports only */
    if (dev_type == device_type_jericho2)
    {
    ...
    for (i = min_dest_id; i <= max_dest_id; ++i)
    {
        ...
        dest.egress_sample_rate_dividend = (egr_prob_shift / max_dest_id) * i;
    ...
    for (i = min_dest_id; i <= max_dest_id; ++i)
    {
        ...
        /*
         * Jericho2 does not uses modport 
         */
        if (dev_type != device_type_jericho2)
        {
