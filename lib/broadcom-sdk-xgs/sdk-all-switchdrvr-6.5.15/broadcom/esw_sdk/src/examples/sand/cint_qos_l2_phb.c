...
/*
 * Main application example to run QOS L2 PHB
 * Use the application example of Cross-connect and set its apprporiate QOS settings
 */
int
qos_map_l2_run_with_tagged (int unit, int port_1, int port_2){
    int rv;
	int index = 0;
    ...
    /*
     * Bring-up vswitch-metro application example
     */
    rv = basic_example(unit, port_1, port_2);
    if (rv != BCM_E_NONE) {
        return rv;
    }

    rv = qos_map_l2_example(unit,&ingress_qos_id,&egress_qos_id);

    if (rv != BCM_E_NONE) {
        return rv;
    }

    rv = qos_map_gport(unit, in_vlan_port_id,ingress_qos_id,-1);

    if (rv != BCM_E_NONE) {
        return rv;
    }

    rv = qos_map_gport(unit, out_vlan_port_id,-1,egress_qos_id);
