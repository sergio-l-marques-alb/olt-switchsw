    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (is_ingress)
    {
        ...
        mpls_port_ing.egress_tunnel_if = mpls_port->egress_tunnel_if;
        ...
        mpls_port_ing.encap_id = mpls_port->encap_id;
        ...
        if(verbose >= 2) {
            printf("mpls port add ingress objects complete, mpls_port_id = 0x%08X, encap_id = 0x%08X\n", mpls_port->mpls_port_id, mpls_port->encap_id);
        }
    }
