...
int example_ip_to_tunnel(int *units_ids, int nof_units, int in_sysport, int out_sysport){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (!is_device_or_above(unit, JERICHO2)){
            ...
            l3eg1.fec_id = 0xA00B;
             rv = l3__egress_only_fec__create(unit,&l3eg1);
             if (rv != BCM_E_NONE) {
                printf("Error, l3__egress_only_fec__create\n");
                return rv;
            }
...
int example_ip_to_tunnel_ext_label(int *units_ids, int nof_units, int in_sysport, int out_sysport, int encap_mode){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        intf.qos_map_id = ing_qos_profile;
 ...
 */
int example_ip_to_tunnel_qos_preserve(int unit, int encap_mode, int qos_profile)
{
    int rv =BCM_E_NONE;
    int qos_map_id;
    uint32 flags;
    ...
    if ((preserve_dscp_per_lif & 0x2) && !is_device_or_above(unit, JERICHO2)) {
        ...
        rv = bcm_port_control_set(unit, route_qos_lif, bcmPortControlPreserveDscpEgress, 1);
        if (rv != BCM_E_NONE) {
