...
int system_red_rsrc_thrs_example(int unit, int destination)
{
    ...
    for (i=0; i < 12; i++) {
        ...
        if (i < 4) {
            threshold.type = (is_dnx) ? bcmCosqThresholdPacketDescriptorBuffers : bcmCosqThresholdDbuffs;
        } else if (i < 8) {
            threshold.type = (is_dnx) ? bcmCosqThresholdOcbDbuffs : bcmCosqThresholdFullDbuffs;
        } else if (i < 12) {
