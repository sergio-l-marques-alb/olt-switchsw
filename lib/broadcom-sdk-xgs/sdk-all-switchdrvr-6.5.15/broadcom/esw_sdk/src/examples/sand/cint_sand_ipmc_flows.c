    ...
    int outP3)
{
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        ipmc_index_offset = 0;
        ipmc_index_upper_bound = *(dnxc_data_get(0,  "multicast", "params", "max_ing_mc_groups", NULL));
    }
...
int ipmc_force_bridge(int unit,int inport, int outP1, int outP2, int outP3 ) {
    ...
    /** QUX nad QAX has a different IPMC group range*/
    ipmc_group_start = (is_device_or_above(unit, JERICHO_PLUS) && !is_device_or_above(unit, JERICHO2)) ? 0x3000 : 0x5000;
    if (is_device_or_above(unit, JERICHO2) )
    {
