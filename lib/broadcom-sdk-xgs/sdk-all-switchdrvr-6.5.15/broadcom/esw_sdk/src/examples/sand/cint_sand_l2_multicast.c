...
int l2_multicast_flood__main_config__start_run(int unit,  l2_multicast_flood_s *param, int step){
    ...
    else
    {
        ...
        tpid_info.tpid_value = g_l2_multicast_flood.tpid_value.s_tag;
        ...
        tpid_info.tpid_value = g_l2_multicast_flood.tpid_value.c_tag;
        rv = bcm_switch_tpid_add(unit, 0, &tpid_info);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_switch_tpid_add(tpid_value=0x%04x)\n", tpid_info.tpid_value);
            return rv;
    ...
    /*
     * Creates Out LIFs (port x VLAN) for signals verification
     */
    bcm_vlan_port_t_init(&vlan_port);

    vlan_port.flags = BCM_VLAN_PORT_CREATE_EGRESS_ONLY;
    vlan_port.criteria = BCM_VLAN_PORT_MATCH_NONE;
    vlan_port.match_vlan = g_l2_multicast_flood.outer_vid;
    vlan_port.vsi = 0;
    ...
    flags = (BCM_MULTICAST_TYPE_L2 | BCM_MULTICAST_WITH_ID | ((g_l2_multicast_flood.type == known_multicast)?BCM_MULTICAST_EGRESS_GROUP:BCM_MULTICAST_INGRESS_GROUP));
    ...
    flags = BCM_MULTICAST_TYPE_L2 | ((g_l2_multicast_flood.type == known_multicast)?BCM_MULTICAST_EGRESS_GROUP:BCM_MULTICAST_INGRESS_GROUP);
    ...
    if (g_l2_multicast_flood.type == known_multicast) {
        ...
        if (is_device_or_above(unit, JERICHO2)) {
            ...
            mreps[0] = 0;
            mreps[1] = 1;
            rv = bcm_fabric_multicast_set(0, g_l2_multicast_flood.mcid, 0, 2, mreps);
            if (rv != BCM_E_NONE) {
