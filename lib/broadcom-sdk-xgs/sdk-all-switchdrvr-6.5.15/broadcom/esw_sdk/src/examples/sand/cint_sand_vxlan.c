...
int vxlan_split_horizon_set(int unit, int vpn, int vxlan_network_group_id) 
{   
    int rv;
    ...
    int is_ecmp)
{
    ...
    BCM_L3_ITF_SET(vxlan_port_add.egress_if, BCM_L3_ITF_TYPE_FEC, ip_tunnel_basic_info.encap_fec_id[tunnel_idx]); 
    ...
    int vdc_enable)
{
     ...
     */
    rv = vxlan_split_horizon_set(unit, cint_vxlan_basic_info.vpn_id, cint_vxlan_basic_info.vxlan_network_group_id);
    if (rv != BCM_E_NONE)
    {
