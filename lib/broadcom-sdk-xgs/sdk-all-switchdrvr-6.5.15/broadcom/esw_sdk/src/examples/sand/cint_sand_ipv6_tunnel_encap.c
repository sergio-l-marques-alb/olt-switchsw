/* $Id: cint_sand_ipv6_tunnel_encap.c,v 1.6 Broadcom SDK $
 ...
 *
 *  Packet walk:
 *  1) Packet arrives at the incoming port (access_port)
 *  2) Get the default's VSI per port
 *  3) Get the ETH-RIF, equal to VSI. (access_eth_rif)
 *  4) ETH-RIF is enabled for routing.
 *  5) ETH is terminated.
 *  6) Get the VRF from ETH-RIF.
 *  7) FWD lookup to get the FEC
 *  8) FEC to be resolved as: Outlif, Destination port and Global Out-EthRIF.
 *  9) Packet of the egress pipeline to be encapsulated as IPv4oIPv6oETH1/IPv6oIPv6oETH1
*/ 

/*
 * Function that  add an IPv6/IPv4 Route entry using <vrf, addr, mask> that will result to an intf.
