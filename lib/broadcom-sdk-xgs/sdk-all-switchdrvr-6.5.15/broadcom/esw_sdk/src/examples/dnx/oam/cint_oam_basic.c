    ...
    int port2)
{
     ...
     */
    rv = set_counter_resource(unit, port1, counter_if_0, &counter_base_0);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in set_counter_resource.\n", rv);
        return rv;
    }
    rv = set_counter_resource(unit, port1, counter_if_1, &counter_base_1);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in set_counter_resource.\n", rv);
        return rv;
    }
    rv = set_counter_resource(unit, port1, counter_if_2, &counter_base_2);
    if (rv != BCM_E_NONE)
    {
        printf("Error no %d, in set_counter_resource.\n", rv);
        return rv;
    ...
    int counter_if)
{
   ...
   if (rv != BCM_E_NONE)
   {
     ...
     */
    raw_ingress_profile_id = 6;
    rv = bcm_oam_profile_id_get_by_type(unit, raw_ingress_profile_id, bcmOAMProfileIngressLIF, &flags, &ingress_profile_id);
    egress_profile_id = BCM_OAM_PROFILE_INVALID;
 ...
 * 
 * This function returns a batch of 8 counters for optional PCP based counting. 
 *  
 ...
 *  
 * Output: 
 * counter_base: pointer to index of the first counters out of 8 
 *  
 */
/** Next engine to be added to database */
int next_engine = 0;
/** Next counter_base to be set for counter_if */
int counter_id[6]={0,0,0,0,0,0};
int nof_counters[6]={0,0,0,0,0,0};

int set_counter_resource(int unit, int port, int counter_if, int *counter_base)
{
    /*
     * CRPS - General
     */
    ...
    */
   database_id[0] = counter_if;
   database_id[1] = counter_if + 3;
   ...
   if (counter_id[database_id[i]] == 0) 
   {
       ...
       for (i=0 ; i<2 ; i++) 
       {
           ...
           counter_id[database_id[i]] = 8;
           nof_counters[database_id[i]] -=8;
       }
   }
    ...
    * counting. 
    */
   if (counter_id[database_id[0]] + 8 >= nof_counters[database_id[0]]) 
   {
       /*
   ...
   /* Assign new counter_base. Allocate 8 counters to support PCP based counting. */   
   *counter_base = counter_id[database_id[0]];
   counter_id[database_id[0]] += 8;
 ...
 * @return int
 */ 
int oam_up_mep_over_vpls_run_with_defaults_dnx(int unit, int port, int port2) {
   ...
   */ 
    ...
    */
   counter_if = 0;
   rv = set_counter_resource(unit, port, counter_if, &lm_counter_base_id);
   if (rv != BCM_E_NONE)
   {
       printf("Error no %d, in set_counter_resource.\n", rv);
       return rv;
...
int dnx_oamp_bfd_trap_snoop_config(int unit, bcm_gport_t dest_snoop_port, int is_ipv6) {
    ...
    if ($is_ipv6)
    {
        trap_config_snoop.dest_port = oamp_gports[1];
    }
    else
    {
