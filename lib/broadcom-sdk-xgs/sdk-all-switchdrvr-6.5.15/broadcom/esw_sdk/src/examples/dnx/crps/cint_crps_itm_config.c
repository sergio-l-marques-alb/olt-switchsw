    ...
    crps_itm_config_database_t create_db)
{
    int rv,i;
    int nof_engines = 6;
    int engine_id[nof_engines] = {8,9,10,11,12,13};
    int engine_id_remove[3] = {21,13,12}; /** as removing is made from last, we assume that add range was called before remove range*/
    uint32* engine_size;
    ...
    else if (create_db == CRPS_ITM_ADD_NEW_RANGE)
    {
        ...
        /** add engine id 21 */
        engine_size = dnxc_data_1d_get(unit, "crps", "engine", "engines_info", "nof_counters", 21);
        rv = crps_itm_voq_data_base_range_add(unit, core_id, database_id, engine_id[nof_engines-1], ((*engine_size)/4), 1, 21);
        if (rv != BCM_E_NONE)
        {
