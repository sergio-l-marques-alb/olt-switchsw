 ...
 * bcmFieldQualifyRangeCheck for L4 SRC/DST ports.
    ...
    bcm_field_stage_t stage)
{
    ...
    */
    ...
    /**
     * We are using range 0 thats why the value of the qualifier is 0.
     */
    ...
    bcm_field_context_t context_id)
{
     ...
     */
    rv = cint_field_range_fg_create_context_attach_entry_add(unit, bcmFieldQualifyPacketLengthRangeCheck, context_id, stage);
    if (rv != BCM_E_NONE)
    {
    ...
    bcm_field_context_t context_id)
{
     ...
     * Create a FG, attach it to a context and add an entry.
     */
    rv = cint_field_range_fg_create_context_attach_entry_add(unit, bcmFieldQualifyRangeCheck, context_id, stage);
    if (rv != BCM_E_NONE)
    {
        printf("%s Error (%d), in cint_field_range_fg_create_context_attach_entry_add\n", proc_name, rv);
        return rv;
