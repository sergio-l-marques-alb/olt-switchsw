 ...
 * cint
 * cint_field_group_const_main(0, stage,context_id,action_type,action_value);
 * cint_field_group_const_desrtoy(0,context_id);
 *
 *
         ...
         bcm_field_action_t action_type, int action_value,char * name)
{
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &cint_const_fg_id);
    if (rv != BCM_E_NONE)
    {
...
/**
* \brief
*  Exmaple to always generate IPT action in iPMF3
