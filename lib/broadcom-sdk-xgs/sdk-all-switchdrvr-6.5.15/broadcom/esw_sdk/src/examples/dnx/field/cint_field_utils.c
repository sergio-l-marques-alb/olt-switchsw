 ...
 * \param [int] compare_id               - Comparision ID
 *
 * \return
 *   shr_error_e - Type of Error
 * \remark
    ...
    int err_reason)
{
    ...
    rv = field_util_fg_context_attach(unit, 0, &fg_utlil, 0);
    ...
    bcm_field_group_t * fg_id_p)
{
     ...
     * Create the FG
     */
    rv = bcm_field_group_add(unit, flags | BCM_FIELD_FLAG_MSB_RESULT_ALIGN , &fg_info, fg_id_p);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
        return rv;
