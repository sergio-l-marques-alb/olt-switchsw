...
int Cint_field_dt_multi_entries_nof_entries = 2048;
    ...
    cint_field_dt_multi_fg_info_t * dt_fg_info)
{
    ...
    Cint_field_dt_multi_entries_entry_info_array[entry_index].priority = 1;

    if (field_stage == bcmFieldStageEgress)
    {
        Cint_field_dt_multi_entries_entry_info_array[entry_index].entry_qual[0].type = bcmFieldQualifyOutPort;
    }
    else
    {
        Cint_field_dt_multi_entries_entry_info_array[entry_index].entry_qual[0].type = bcmFieldQualifyInPort;
    }
    BCM_GPORT_LOCAL_SET(gport, port);

    Cint_field_dt_multi_entries_entry_info_array[entry_index].entry_qual[0].value[0] = gport;
    ...
    /** Print every 256 entries. */
    if ((entry_index + 1) % 256 == 0)
    {
    ...
    bcm_field_stage_t field_stage)
{
    ...
    rv = cint_field_dt_multi_entries_fg_create(unit, field_stage, Cint_field_dt_multi_entries_fg_info_array[0]);
    if (rv != BCM_E_NONE)
    {
    ...
    for (entry_index = 0; entry_index < Cint_field_dt_multi_entries_nof_entries; entry_index++)
    {
        ...
        if (((entry_index+1) % 500) == 0)
        {
            rv = cint_field_dt_multi_entries_entry_create(unit, entry_index, port, field_stage, Cint_field_dt_multi_entries_fg_info_array[0]);
            if (rv != BCM_E_NONE)
            {
        ...
        {
            rv = cint_field_dt_multi_entries_entry_create(unit, entry_index, dummy_port, field_stage, Cint_field_dt_multi_entries_fg_info_array[0]);
            if (rv != BCM_E_NONE)
            {
                printf("Error (%d), in cint_field_dt_multi_entries_entry_create\n", rv);
                return rv;
...
/**
 * \brief
 *  Destroys first 1024 entries, to remove the two valid entries and
 *  verify that their actions are not going to be performed.
 *
 * \param [in] unit         - Device id
 * \return
 *   int - Error Type
 ...
 */
int
cint_field_dt_multi_entries_half_entries_destroy(
    int unit)
{
    int rv = BCM_E_NONE;
    int entry_index;

    for (entry_index = 0; entry_index < (Cint_field_dt_multi_entries_nof_entries / 2); entry_index++)
    {
        rv = bcm_field_entry_delete(unit, Cint_field_dt_multi_entries_fg_info_array[0].fg_id,
            Cint_field_dt_multi_entries_entry_info_array[entry_index].entry_qual, Cint_field_dt_multi_entries_fg_info_array[0].entry_handle);
        if (rv != BCM_E_NONE)
        {
    ...
    bcm_field_stage_t field_stage)
{
    ...
    for (entry_index = ((Cint_field_dt_multi_entries_nof_entries / 2) - 1); entry_index < Cint_field_dt_multi_entries_nof_entries; entry_index++)
    {
        rv = bcm_field_entry_delete(unit, Cint_field_dt_multi_entries_fg_info_array[0].fg_id,
            Cint_field_dt_multi_entries_entry_info_array[entry_index].entry_qual, Cint_field_dt_multi_entries_fg_info_array[0].entry_handle);
        if (rv != BCM_E_NONE)
