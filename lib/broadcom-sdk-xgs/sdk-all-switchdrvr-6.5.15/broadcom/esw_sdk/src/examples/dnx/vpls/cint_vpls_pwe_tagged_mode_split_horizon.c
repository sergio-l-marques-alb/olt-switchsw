 ...
 * cint examples/sand/cint_qos_l2_phb.c
 * cint examples/sand/cint_mpls_qos.c
 * cint examples/sand/cint_vpls_pwe_tagged_mode_split_horizon.c
 * cint
    ...
    int unit)
{
    ...
    */
    ...
    vswitch_vpls_params.pwe_params[0].pwe_term.flags2 |= BCM_MPLS_PORT2_INTERFACE_NAMESPACE;
    /* pwe qos */
    ...
    vswitch_vpls_params.pwe_params[0].pwe_encap.flags |= BCM_MPLS_PORT_SERVICE_TAGGED;
    ...
    */
    ...
    vswitch_vpls_params.pwe_params[1].pwe_term.flags2 |= BCM_MPLS_PORT2_INTERFACE_NAMESPACE;
    ...
    pwe_tagged_mode_split_horizon_vlan_params_s* vlan_params)
{
    ...
    tpid_info.tpid_value = vlan_params->outer_tpid;
    rv = bcm_switch_tpid_add(unit, 0, &tpid_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_switch_tpid_add for 0x$x\n", vlan_params->outer_tpid);
        return rv;
    }

    /* add TPID to global array */
    tpid_info.tpid_value = vlan_params->inner_tpid;
    rv = bcm_switch_tpid_add(unit, 0, &tpid_info);
    if (rv != BCM_E_NONE)
    {
        printf("Error, bcm_switch_tpid_add for 0x%x\n", vlan_params->inner_tpid);
        return rv;
    }
