...
int cint_bcm_mgmt_remote_credit_size_set(int fap_unit, int dest_fap_unit, int credit_size)
{
    ...
    bcm_stk_modid_get(dest_fap_unit, &dest_modid);
    if (BCM_FAILURE(rv))
    {
    ...
    for (core = 0; core < nof_modids; core++)
    {
        rv = bcm_cosq_dest_credit_size_set(fap_unit, dest_modid + core, credit_size);
        if (BCM_FAILURE(rv))
        {
           printf("Error, bcm_cosq_dest_credit_size_set (dest_fap_id=%d, credit_size=%d) \n", (dest_modid + core), credit_size);
           return rv;
        }
    }
