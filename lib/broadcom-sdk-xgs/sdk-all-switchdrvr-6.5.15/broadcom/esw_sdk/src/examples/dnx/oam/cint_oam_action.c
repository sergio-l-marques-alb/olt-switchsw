 ...
 *     - for single ended, dual ended and slm,
 *     - for ETH,MPLS,PWE 
 ...
 *       4        || oam_profile_eth_slm           ||   ETH    |   SLM
 *       5        || oam_profile_eth_none          ||   ETH    |   NONE
 *       6        || oam_profile_mpls_single_ended ||   MPLS   |   Single Ended
 *       7        || oam_profile_mpls_dual_ended   ||   MPLS   |   Dual Ended
 *       8        || oam_profile_mpls_slm          ||   MPLS   |   SLM
 *       9        || oam_profile_mpls_none         ||   MPLS   |   NONE
 *      10        || oam_profile_pwe_single_ended  ||   PWE    |   Single Ended
 *      11        || oam_profile_pwe_dual_ended    ||   PWE    |   Dual Ended
 *      12        || oam_profile_pwe_slm           ||   PWE    |   SLM
 *      13        || oam_profile_pwe_none          ||   PWE    |   NONE
 *
 *
 ...
 */
static const int ETH = 0;
static const int MPLS = 1;
static const int PWE = 2;
    ...
    int is_dual_ended)
{
     ...
     * 1. Set loss type: Configure action for lm, slm or dual ended 
    ...
    for (dest_mac_type = 0; dest_mac_type < nof_mac_types; dest_mac_type++)
    {
        ...
        for (opcode_index = 0; opcode_index < nof_supported_opcodes; opcode_index++)
        {
            ...
            for (endpoint_type = 0; endpoint_type < nof_endpoint_types; endpoint_type++)
            {
                 ...
                 * (Escept for CCM packets in LM-MEPs and 
                ...
                switch (endpoint_type)
                {
                    case bcmOAMMatchTypeMEP:
                    {
                         ...
                         */
                        if (((is_slm_action == 0) && (is_dual_ended == 0) && (action_key.opcode == bcmOamOpcodeCCM)) ||
                            ((is_slm_action == 1) && (action_key.opcode == bcmOamOpcodeSLM)))
                        {
                            /*
                             * For SLM action count only OAM-SLM packets 
    ...
    int is_dual_ended)
{
     ...
     */
    rv = oam_lm_action(unit, is_ingress, ingress_profile_id, is_slm_action, is_dual_ended);
    if (rv != BCM_E_NONE)
    {
    ...
    oam_destinations_s * oam_destinations)
{
    ...
    /*
     * Get the trap code dedicated for OAM Level. 
     */
    ...
    int lm_type)
{
    ...
    if (is_acc || (lif_type == MPLS) || (lif_type == PWE))
    {
        /*
            ...
            for (endpoint_type = endpoint_type_start; endpoint_type <= endpoint_type_end; endpoint_type++)
            {
                ...
                switch (endpoint_type)
                {
                    case bcmOAMMatchTypeMEP:
                    {
                        ...
                        if (!is_acc)
                        {
                            if (((lm_type == SINGLE_ENDED) && (action_key.opcode == bcmOamOpcodeCCM)) ||
                                ((lm_type == SLM) && (action_key.opcode == bcmOamOpcodeSLM)))
                            {
                                /*
                                 * For SLM action count only OAM-SLM packets 
                                 * For LM action count only OAM-CCM packets 
    ...
    oam_destinations_s oam_destinations)
{
    ...
    for (lif_type = ETH; lif_type <= PWE; lif_type++)
    {
        for (lm_type = SINGLE_ENDED; lm_type <= NONE; lm_type++)
        {
            /*
             * Create OAM ingress profile
             */
            ...
            else if (lm_type == DUAL_ENDED)
            {
                flags |= BCM_OAM_PROFILE_LM_TYPE_DUAL_ENDED;
            }
            else if (lm_type == NONE)
            {
    ...
    oam_destinations_s oam_destinations)
{
    ...
    for (lif_type = ETH; lif_type <= PWE; lif_type++)
    {
        for (lm_type = SINGLE_ENDED; lm_type <= NONE; lm_type++)
        {
           /* Accelerated SLM is not supported */
           if (lm_type != SLM) 
           {
               /*
                * Create OAM ingress profile
