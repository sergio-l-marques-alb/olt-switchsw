...
{ 0, 0,
    /*
     * l2_remote_arp_intf[3] - filled according to allocation
     */
    {0, 0, 0},
    /*
    ...
    int unit)
{
    ...
    uint8 system_headers_jr1_mode = (soc_property_get(unit, "system_headers_mode", 1) == 0);
    ...
    if (!system_headers_jr1_mode)
    {
        mirror_dest.packet_control_updates.valid = BCM_MIRROR_PKT_HEADER_UPDATE_FABRIC_HEADER_EDITING;
    }
    else 
    {
        mirror_dest.encap_id = BCM_L3_ITF_VAL_GET(l2_external_cpu_info.l2_remote_arp_intf[2]);
    ...
    int qos_egress)
{
    ...
    rv = l3__egress_only_encap__create_inner(unit, 0, &l2_external_cpu_info.l2_remote_arp_intf[tunnel_index], dst_mac,
