    ...
    int presel_index)
{
     ...
     * Set the presel.
     */
    rv = bcm_field_presel_set(unit, BCM_FIELD_FLAG_WITH_ID, &p_id, &p_data);
    if (rv != BCM_E_NONE)
    {
    ...
    int address_prefix)
{
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &Cint_field_st_multi_fg_id[address_prefix]);
    if (rv != BCM_E_NONE)
    {
