    ...
    int nof_tpids)
{
    ...
    for(tpid_idx = 0; tpid_idx < nof_tpids; tpid_idx++)
    {
        if(tpids[tpid_idx].valid)
        {
            ...
            {
                tpid_info.tpid_value = tpids[tpid_idx].tpid1;
            ...
            {
                /* add TPID to global array */
                tpid_info.tpid_value = tpids[tpid_idx].tpid2;
                rv = bcm_switch_tpid_add(unit, 0, &tpid_info);
                if (rv != BCM_E_NONE)
                {
    ...
    int nof_vpls_term)
{
    ...
    for(vpls_idx = 0; vpls_idx < nof_vpls_term; vpls_idx++)
    {
        if(mpls_is_valid_lable(port_term[vpls_idx].match_label))
        {
            ...
            rv = bcm_vlan_create(unit, *port_term[vpls_idx].forwarding_domain);
            if ((rv != BCM_E_NONE) && (rv != BCM_E_EXISTS))
            {
                printf("Failed (%d) to create VLAN %d\n", rv, *port_term[vpls_idx].forwarding_domain);
                return rv;
            }
    ...
    int nof_vlan)
{
    ...
    for(vlan_idx = 0; vlan_idx < nof_vlan; vlan_idx++)
    {
        if(vlans[vlan_idx].criteria != BCM_VLAN_PORT_MATCH_INVALID)
        {
            ...
            vlan_port.egress_vlan = vlans[vlan_idx].egress_vlan;
            vlan_port.vsi = 0;
