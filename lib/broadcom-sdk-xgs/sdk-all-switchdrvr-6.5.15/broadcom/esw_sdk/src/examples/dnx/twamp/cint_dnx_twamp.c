    ...
    int *recycle_entry_encap_id)
{
    ...
    bcm_mac_t recycle_mac_address = {0x00, 0x0c, 0x00, 0x11, 0x22, 0x33};
 ...
 *
 * \param [in] unit - The unit number.  
 * \param [in] in_vlan - The in vlan.
 * \param [in] in_sysport - The system port.
 * \param [in] in_vlan2 - The in vlan 2.
 * \param [in] in_sysport2 - The system port 2.
 * \param [in] sat_port - The SAT port.
 * \param [in] rcy_port - The recycle port.
 *
 * \return
 ...
 *
 */
int twamp_reflector_example(int unit, int in_port, int out_port, int rcy_port) {
    int rv = BCM_E_NONE;    
    int trap_id = 0;    
    ...
    /*** create RCH encap ***/
    rv = twamp_create_recycle_entry(unit, &recycle_entry_encap_id);
    if (rv != BCM_E_NONE) {
        printf("twamp_create_recycle_entry $rv");
    }

    /*** create TWAMP encap ***/
    rv = twamp_create_reflector(unit, recycle_entry_encap_id, &twamp_encap_id);
    if (rv != BCM_E_NONE) {
    ...
    /*** create PMF rule ***/
    rv = cint_field_twamp_reflector_rule(unit, gport_trap, local_ip_addr, remote_ip_addr);
    if (rv != BCM_E_NONE) {
 ...
 *
 */
int twamp_tx_rx_example(int unit, int in_port, int out_port, int trap_port) {
    int rv = BCM_E_NONE;    
    int trap_id = 0;    
    bcm_rx_trap_config_t config;
    int trap_strength = 7;
    ...
    /*** create PMF rule ***/
    rv = cint_field_twamp_tx_rx_rule(unit, gport_trap, local_ip_addr, remote_ip_addr);
    if (rv != BCM_E_NONE) {
