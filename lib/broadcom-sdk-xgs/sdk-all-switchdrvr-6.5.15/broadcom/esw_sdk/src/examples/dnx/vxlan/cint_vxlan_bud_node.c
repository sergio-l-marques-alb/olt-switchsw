 ...
 * Packet Walk:
 * --------------------- First Pass --------------------
 * 1) VTT1: Resolve VSI from AC-LIF, taken from {In-PP-Port.Default-LIF}. ETH is MC compatible - terminate it.
 * 2) VTT2-VTT4: FAIL to terminate tunnel
 * 3) FWD1: {DIP, VRF} -> MC group 
 * 4) Ingress TM: packet is duplicated: to outlif + port, outlif + RCH outlif + recycle port
 * 5) ENC5: Encapsulate ETH Header according to outrif
 * 6) TRAP: Encapsulate RCH header and go to 2nd pass
 *
 * --------------------- Second Pass -------------------
 * 1) Port Termination: RCH header resolution
 * 2) VTT1: recycle port default inlif. (Special inlif, indicate to get the fodo from RCH header)
 * 3) VTT2: VTT lookup: {Fwd-Domain,SIP, DIP, Tunnel-Type}-> Tunnel in-LIF, no hit
 * 4) VTT3: VTT lookup : {Fwd-Domain, DIP, Tunnel-Type }->Tunnel In-LIF; VRF is resolved by Tunnel In-Lif FoDo property
 * 5) VTT4: VNI2VSI resolution
 * 6) FWD: MACT lookup: DA->MC
 * 7) Ingress TM: packet is duplicated: to vxlan outlif + port, vxlan outlif + port, port
 * 8) ERPP Filter: Filter the same interface packet
 * 9) ENC2: Encapsulate VXLAN header on vxlan outlif + port packet
 * 10) ENC5: EVE on the port packet and eth encapsulation on the vxlan packet
 */
...
struct cint_vxlan_tunnel_bud_node_info_s
{
    ...
    bcm_mac_t recycle_mac_address;              /* recycle next hop MAC DA */
...
cint_vxlan_tunnel_bud_node_info_s cint_vxlan_tunnel_bud_node_info = {
    ...
    /*
     * recycle_mac_address | route_mc_mac_address
     */
    {0x00, 0x0c, 0x00, 0x02, 0x00, 0x00}, {0x00, 0x0c, 0x00, 0x02, 0x00, 0xf1},
    /*
     * tunnel_init_dip
     */
    ...
    int recycle_port)
{
    ...
    rv = set_multicast_replication(&replications_1st_pass[0], cint_vxlan_tunnel_bud_node_info.out_ports[1], (recycle_entry_encap_id & 0x3fffff));
    if (rv != BCM_E_NONE)
    {
