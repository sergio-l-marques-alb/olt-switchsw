                             ...
                             /* in */ bcm_gport_t snoop_trap_gport_id)
{
	...
	*/
	...
	fg_info.qual_types[2] = bcmFieldQualifyRangeCheck;
    ...
    */
    ...
    ent_info.entry_qual[2].value[0] = 0x1;
    ent_info.entry_qual[2].mask[0] = 0xFFFFFF;
...
int cint_field_ip_snoop_main(int unit,bcm_field_context_t context_id, bcm_port_t in_port, int dest_local_port, int udp_port_min, int udp_port_max)
{
    ...
    rv = cint_field_ip_snoop_set(unit, dest_local_port, &snoop_trap_gport_id);
    if (BCM_E_NONE != rv) {
