...
int cint_field_cascading_fg_add(int unit, bcm_field_stage_t stage, bcm_field_group_type_t fg_type,
        bcm_field_group_t *fg_id_p)
{
    ...
    switch (stage)
    {
        case bcmFieldStageIngressPMF1:
        {
            ...
            sal_strncpy_s(dest_char, "TEST_CASCADED_IPMF1", sizeof(fg_info.name));
        ...
        case bcmFieldStageIngressPMF2:
        {
            ...
            fg_info.qual_types[1] = bcm_udf_qual;

            /* Third qualifier - same as in ipmf1 group.
             * We add it here just to enlarge the key size to the >80b key,
             * because currently the prefix id is not implemented for 80b entry.
             * So technically it is only a placeholder in the key. */
            fg_info.qual_types[2] = bcmFieldQualifyDstMac;
            ...
            /* Set FG name */
            dest_char = &(fg_info.name[0]);
            sal_strncpy_s(dest_char, "TEST_CASCADED_IPMF2", sizeof(fg_info.name));
        ...
        bcm_field_group_attach_info_t * attach_info_p)
{
    ...
    switch (stage)
    {
        ...
        {
            int action_offset;
            bcm_field_group_info_t fg1_info;
            ...
            /*
             * Get the action we want to cascaded from in iPMF1 fg
             */
            bcm_field_group_info_get( unit, ipmf1_cascaded_fg_id, &fg1_info);
            /*
             * Get the offset for the action
             */
            rv = bcm_field_group_action_offset_get(unit, 0, ipmf1_cascaded_fg_id,
                    fg1_info.action_types[1], &action_offset);
            if (rv != BCM_E_NONE)
            {
        ...
        bcm_field_group_t ipmf1_cascaded_fg_id)
{
    ...
    rv = cint_field_cascading_set_attach_info(unit, stage, ipmf1_cascaded_fg_id, &attach_info);
    if (rv != BCM_E_NONE)
    {
        ...
        bcm_field_entry_t *entry_handle_p)
{
    ...
    switch (stage)
    {
        ...
        {
            /* SIP */
            entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];   /* bcmFieldQualifySrcIp */
            entry_in_info.entry_qual[qual_idx].mask[0] = 0xFFFFFFFF;
            entry_in_info.entry_qual[qual_idx].value[0] = 0x0a0b0c0d;               /* 10.11.12.13 */
            qual_idx++;
            /* DIP */
            entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];   /* bcmFieldQualifyDstIp */
            entry_in_info.entry_qual[qual_idx].mask[0] = 0xFFFFFFFF;
            entry_in_info.entry_qual[qual_idx].value[0] = 0x0a0b0c0e;               /* 10.11.12.14 */
            qual_idx++;
            /* DA */
            entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];   /* bcmFieldQualifyDstMac */
            entry_in_info.entry_qual[qual_idx].mask[0] = 0xFFFFFFFF;
            entry_in_info.entry_qual[qual_idx].value[0] = 0x12341234;               /* 00.00.12.34.12.34 */
            qual_idx++;
            /* TTL Action */
            entry_in_info.entry_action[action_idx].type = fg_info.action_types[action_idx + 1];
        ...
        {
            /* SIP */
            entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];   /* bcmFieldQualifySrcMac */
            entry_in_info.entry_qual[qual_idx].mask[0] = 0xFFFFFFFF;
            entry_in_info.entry_qual[qual_idx].value[0] = 0xabcdab;               /* 0xabcdab*/
            qual_idx++;

            /* User qualifier for cascading */
            entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];   /* CASCADED_ACTION_QUAL */
            ...
            /* DP Action */
            entry_in_info.entry_action[action_idx].type = fg_info.action_types[action_idx];
        ...
        default:
        {
            for (qual_idx = 0; qual_idx < fg_info.nof_quals; qual_idx ++)
            {
                entry_in_info.entry_qual[qual_idx].type = fg_info.qual_types[qual_idx];
...
int cint_field_cascading_main(int unit, bcm_field_context_t ipmf1_context_id, bcm_field_context_t ipmf2_context_id)
{
    ...
    rv = cint_field_cascading_fg_add(unit, bcmFieldStageIngressPMF1, bcmFieldGroupTypeTcam, &ipmf1_fg_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_field_cascading_fg_attach(unit, bcmFieldStageIngressPMF1, ipmf1_context_id, ipmf1_fg_id, 0 /* Not relevant for IPMF1*/);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_field_cascading_fg_add(unit, bcmFieldStageIngressPMF2, bcmFieldGroupTypeTcam, &ipmf2_fg_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_field_cascading_fg_attach(unit, bcmFieldStageIngressPMF2, ipmf2_context_id, ipmf2_fg_id, ipmf1_fg_id);
    if (rv != BCM_E_NONE)
    {
    ...
    /******************Entry Configuration* *******/
    /**********************************************/
    rv = cint_field_cascading_entry_add(unit, bcmFieldStageIngressPMF1, ipmf1_context_id, ipmf1_fg_id, cascaded_action_value, 2, &ipmf1_entry_handle);
    if (rv != BCM_E_NONE)
    {
        printf("%s Error (%d), in cint_field_cascading_entry_add  for IngressPMF1\n", proc_name, rv);
        return rv;
    }
    rv = cint_field_cascading_entry_add(unit, bcmFieldStageIngressPMF2, ipmf2_context_id, ipmf2_fg_id, cascaded_action_value, 0, &ipmf2_entry_handle);
    if (rv != BCM_E_NONE)
    {
