...
int field_state_table_main(int unit, bcm_field_presel_t presel_id, bcm_field_layer_type_t fwd_layer)
{
    ...
    printf("Presel (%d) was configured context(%d) fwd_layer(%d) \n",
        presel_id, state_table_context_id, fwd_layer);
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &state_table_fg_state_table_id);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = bcm_field_group_add(unit, BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &state_table_fg_de_id);
    if (rv != BCM_E_NONE)
    {
        printf("Error (%d), in bcm_field_group_add\n", rv);
    }
