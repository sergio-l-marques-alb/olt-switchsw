...
int l2_sa_discard_config_traps(int unit, int dest_port) {
    ...
    int flags = 0;
    int is_jericho2 = 0;
    int trap_id_mac_sa_trap_JER2 = 142;
    int trap_id_mac_sa_drop_JER2 = 141;
    ...
    if(drop_trap_id == 0){
        if (!is_jericho2) {
            ...
            /* Only OAMP Error traps and bcmRxTrapUserDefine traps can be created using BCM_RX_TRAP_WITH_ID */
            /* the initial trap id for bcmRxTrapL2DiscardMacsaTrap is 142 and it's already created in dnx_tune_trap_ingress_configure_init */ 
            rv = bcm_rx_trap_type_destroy(unit, trap_id_mac_sa_trap_JER2);
            if (rv != BCM_E_NONE) {
                printf("Error, in trap destroy bcmRxTrapL2DiscardMacsaTrap 142 \n");
                return rv;
            }
    ...
    if(redirect_trap_id == 0){
        if (!is_jericho2) {
            ...
            if (rv != BCM_E_NONE) {
                printf("Error, in trap create, trap id $redirect_trap_id \n");
                return rv;
            }
        } else {
            /* Only OAMP Error traps and bcmRxTrapUserDefine traps can be created using BCM_RX_TRAP_WITH_ID */
            /* the initial trap id for bcmRxTrapL2DiscardMacsaDrop is 141 and it's already created in dnx_tune_trap_ingress_configure_init */
            rv = bcm_rx_trap_type_destroy(unit, trap_id_mac_sa_drop_JER2);
            if (rv != BCM_E_NONE) {
                printf("Error, in trap destroy bcmRxTrapL2DiscardMacsaDrop 141 \n");
                return rv;
            }
