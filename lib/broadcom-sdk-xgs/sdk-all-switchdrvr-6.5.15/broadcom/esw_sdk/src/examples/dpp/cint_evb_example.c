/* $Id: cint_evb_example.c,v 1.6 Broadcom SDK $
 ...
 *  6. ARAD workaround FP: enable same-interface filter (learn_data_rebuild_example example function).
 * 
 * 
 *  How-to-run:
...
int main_evb_example(int unit, int uplink_port, int bridge_port) {
    ...
    if (!is_device_or_above(unit,ARAD_PLUS)) {
      /* 
       * Workaround for ARAD to enable flexible same interface filter per Incoming Logical Interface
       */
