...
mpls_mc_test3_explicit_flags_example(int unit, int in_port, int out_port){
   ...
   if(is_device_or_above(unit, JERICHO2))
   {
       label_array[0].egress_qos_model.egress_ttl= bcmQosEgressModelPipeNextNameSpace;
   }
   else
   {
       label_array[0].flags |= BCM_MPLS_EGRESS_LABEL_TTL_SET;
   ...
   label_array[1].flags = (BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT|BCM_MPLS_EGRESS_LABEL_ACTION_VALID);  
   if(is_device_or_above(unit, JERICHO2))
   {
       label_array[1].egress_qos_model.egress_ttl= bcmQosEgressModelPipeNextNameSpace;
   }
   else
   {
       label_array[1].flags |= BCM_MPLS_EGRESS_LABEL_TTL_SET;
