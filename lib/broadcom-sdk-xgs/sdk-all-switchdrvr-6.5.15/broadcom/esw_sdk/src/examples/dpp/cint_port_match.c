...
int port_match_run_ingress_match(int *units_ids, int nof_units, int no_initial_match, int is_asymmetric_lifs){
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        sysport_is_local = 0;
        rv = system_port_is_local(unit, port_match_info_1.ports[0], &local_port, &sysport_is_local);
        if (rv != BCM_E_NONE)
        {
        ...
        if(sysport_is_local == 1) {
            if (no_initial_match) {
                ...
                if (is_device_or_above(unit, JERICHO2)) {
                    match_criteria = BCM_VLAN_PORT_MATCH_PORT_CVLAN;
                }else{
                    match_criteria = BCM_VLAN_PORT_MATCH_PORT_VLAN;
                }
                rv = vlan_port_create(unit, 
    ...
    if (is_device_or_above(unit, JERICHO2)) {
        match_criteria = BCM_PORT_MATCH_PORT_CVLAN;
    }else{
        match_criteria = BCM_PORT_MATCH_PORT_VLAN;
    }
    if (no_initial_match) {
...
port_match_run_egress_match(int *units_ids, int nof_units, int is_asymmetric_lifs){
    ...
    for (j = 0 ; j < nof_eg_ports ; j++){
        ...
        for (i = 0 ; i < nof_units ; i++){
            unit = units_ids[i];
            sysport_is_local = 0;
            rv = system_port_is_local(unit, port, &local_port, &sysport_is_local);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in system_port_is_local(%d,%d).\n", unit, port);
                return rv;
            }
            if(sysport_is_local == 0) {
                continue;
            }
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Error, in system_port_is_local(%d,%d).\n", unit, port_match_info_1.ports[0]);
            return rv;
        }
        if(sysport_is_local ==0) {
            continue;
        ...
        for (i = 0 ; i < nof_units ; i++){
            ...
            if (j < (nof_eg_ports - nof_dropped_ports)) {
                if (is_device_or_above(unit, JERICHO2)) {
                    ...
                    sysport_is_local = 0;
                    rv = system_port_is_local(unit, port_match_info_1.ports[1], &local_port, &sysport_is_local);
                    if (rv != BCM_E_NONE)
                    {
                        printf("Error, in system_port_is_local(%d,%d).\n", unit, port);
                        return rv;
                    ...
                    if(sysport_is_local ==1 ) {
                        ...
                        rv = vlan_port_create(unit, 
                                              port_match_info_1.ports[1], 
                                              BCM_VLAN_PORT_MATCH_PORT_CVLAN, 
                ...
                if (is_device_or_above(unit, JERICHO2)) {
                    ...
                    if(sysport_is_local ==1 ) {
                        ...
                        rv = vlan_port_create(unit,
                                              ...
                                              port_match_info_1.eg_vids[1],
                                              0,
                                              flags | BCM_VLAN_PORT_DEFAULT | BCM_VLAN_PORT_VLAN_TRANSLATION);
                    }
                }else{
                    rv = port_match_add_remove(unit, 
                                   BCM_PORT_MATCH_PORT, 
...
port_match_cleaup(int *units_ids, int nof_units, int ingress){
    ...
    for (j = 0 ; j < nof_eg_ports ; j++){
        ...
        for (i = 0 ; i < nof_units ; i++){
            ...
            if (is_device_or_above(unit, JERICHO2) && !ingress){
                continue;
            }
            if (!drop_match) {
                if (is_device_or_above(unit, JERICHO2)) {
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (gport_additional != -1) {
            sysport_is_local = 0;
            rv = gport_is_local(unit, gport_additional, &local_port, &sysport_is_local);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in gport_is_local(0x%x,0x%x).\n", unit, gport_additional);
                return rv;
            }
            if(sysport_is_local ==1) {
            ...
            sysport_is_local = 0;
            rv = gport_is_local(unit, port_match_info_1.vlan_port_egress_default, &local_port, &sysport_is_local);
            if (rv != BCM_E_NONE)
            {
                printf("Error, in gport_is_local(0x%x,0x%x).\n", unit, port_match_info_1.vlan_port_egress_default);
                return rv;
            }
            if(sysport_is_local ==1) {
