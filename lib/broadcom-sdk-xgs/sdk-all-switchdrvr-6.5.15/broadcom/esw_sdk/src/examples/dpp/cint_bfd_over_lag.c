...
int bfd_server_example(int server_unit, int client_unit) {
     ...
     *  configured above */
    rv = l3_interface_init(client_unit, bfd_endpoint_over_lag_1.system_ports[0], bfd_endpoint_over_lag_1.eg_port, &next_hop_mac, &tunnel_id, 0, 0);
    if (rv != BCM_E_NONE) {                                         
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;                                     
    }
    ...
    rv = l3_interface_init(server_unit, bfd_endpoint_over_lag_1.system_ports[1], bfd_endpoint_over_lag_1.eg_port, &next_hop_mac, &tunnel_id, 0, 0);
    if (rv != BCM_E_NONE) {                                         
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    }
