...
/* It's for injected packets which use to test non bfd forwarding with two different core*/
int non_bfd_forwarding_two_cores = 0;
...
int bfd_ipv4_single_hop_field_action(int unit)
{
    ...
    result = oam__device_type_get(unit, &device_type);
	...
	if(!non_bfd_forwarding_two_cores){
    ...
    {
        result = 	bcm_field_qualify_AppType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL | presel_flags, bcmFieldAppTypeBfdIp4SingleHop);
        if (BCM_E_NONE != result) {
    ...
    BCM_FIELD_PRESEL_ADD(presel_set, presel_id);
    	...
    	stage = bcmFieldStageIngress;
        result = bcm_field_qualify_Stage(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL | presel_flags, stage);
    	if (BCM_E_NONE != result) {
    		printf("Error in bcm_field_qualify_Stage Err %x\n",result);
    		return result;
    	}
    ...
    result = bcm_field_qualify_ForwardingType(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL | presel_flags, bcmFieldForwardingTypeIp4Ucast);
    if (BCM_E_NONE != result) {
          printf("Error in bcm_field_qualify_ForwardingType\n");
          return result;
    }
    ...
    if((device_type <= device_type_arad_plus) || (remote_detect_compare_mode == 1))
    {
        result = bcm_field_qualify_AppType(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL | presel_flags, bcmFieldAppTypeBfdIp4SingleHop);
        if (BCM_E_NONE != result) {
            printf("Error in bcm_field_qualify_AppType\n");
            return result;
        }
