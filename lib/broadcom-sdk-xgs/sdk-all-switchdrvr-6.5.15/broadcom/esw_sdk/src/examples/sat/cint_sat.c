 ...
 *
 * Background:
 *     Service Activation Test (SAT) feature is basically a validation of an Ethernet Service for a customer 
 *     by a service provider operator before activating theservice. The customer may be another operator, 
 *     upstream or downstream, or an end-point customer. It is usually done with an Ethernet Test Equipment and involves a Truck Roll to 
 *     the premises where the network endpoint or the intermediate point resides. 
 *     The industry is moving towards the use of embedded test application within the network switch element.
 * 
 *     The SAT implementation borrows concept of MEP points from Ethernet OAM. Each SAT MEP has an Rx and Tx point. 
 *     These SAT MEP Tx/Rx points in SAT context are defined as Service Activation Measurement Points (SAMP). 
 *     SAT MEP Rx point is where a SAT packet coming from wire is trapped and sent to OLP. 
 *     SAT MEP Tx point is where a SAT packet is injected from OLP before going out on wire.
 * 
 *     A SAT MEP (SAMP) can perform one or all of the functions defined below:
 *     Generator Function (GTF):  SAT logic that injects test frames for OVC/EVC at specified rate and 
 *                                Test frames processed like customer service frames.
 *     Collector Function (CTF):  SAT Test frames are processed like customerservice frames and 
 *                                logic has instrumentation counters for SAT.
 *     Reflector Function (Latching Loopback Function): Loops back customerservice frames 
 *                                at remote end for two-way measurement with GTF and CTF at the same end.
 * 
 * Purpose: 
 ...
 *    FWD_DP                     2               28:27
 *    FWD_DEST_INFO              19              26:8
 *    SNOOP_CMD                  4               7:4
 *    FWD_TRAFFIC_CLASS          3               3:1
 *    ITMH_DESTINATION_INFO      1               0
 *    EXTENSION_EXISTS
 *
 *    Jericho-style FWD_DEST_INFO(19bits)
 *    1 1        Flow-ID(17)
 *    1 0      Multicast-ID(17)
 *    0 1     FEC-Pointer-ID(17)
 *    0 0 1   Dest-System-Port-Agr(16)
 */
...
*  The workaround is avariable  if the sequence number is enabled.
*/
