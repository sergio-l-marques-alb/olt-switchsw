...
#define CL37_W_BAM_HG     6
#define CL37_HR2SPM_W_10G 5
#define CL37_HR2SPM       4
#define CL37_W_10G        3
#define CL37_WO_BAM       2
#define CL37_W_BAM        1
#define CL37_DISABLE      0

#define CL73_HG                0x6
#define CL73_CL37              0x5
#define CL73_HPAM              0x4
#define CL73_HPAM_VS_SW        0x8
#define CL73_WO_BAM            0x2
#define CL73_W_BAM             0x1
#define CL73_DISABLE           0x0
...
soc_esw_portctrl_core_config_update(int unit, int lport, int pport, uint32_t xphy_id, int phy_idx) {
    ...
    xphy_core_info.force_fw_load = 0;
    if (fw_ld_method & 0x1) xphy_core_info.force_fw_load = 1;
 ...
 * Parameters:
 *      unit             - (IN)  Unit number.
 *      phy_ports       - (IN)  bit map of physical ports of the phy.
 * Returns:
 *      BCM_E_XXX
 */
int
soc_esw_portctrl_setup_ext_phy_add(int unit, soc_pbmp_t phy_ports, portmod_dispatch_type_t type)
{
        ...
        while((ext_phy_addr != 0xFF) || (phy_chain_length != 0)) {
           ...
           */
            ...
            /* only for the iner most external phys we have an alternate config */
                                xphy_lane_connection.ls_lane = -1;
                                xphy_lane_connection.ls_lane = -1;
            if (topology.is_default && (chain_idx == 1)) {
                ...
                if (is_100G_port) {
                    xphy_lane_connection.ss_lane = pport - first_port;
                    xphy_lane_connection.ls_lane = pport - first_port;
                    switch (lane_config) {
                        case SOC_LANE_CONFIG_100G_4_4_2:
                            if ((pport - first_port) == 10 || 
                                (pport - first_port) == 11) {
                                xphy_lane_connection.ss_lane = -1;
                                xphy_lane_connection.ls_lane = -1;    
                            }
                            /* Temp fix for sesto gearbox */
                            if ((((pport - first_port) > 3) &&
                               ((pport - first_port) < 8))) {
                                xphy_lane_connection.ls_lane = (pport - first_port)-4;
                            } else {
                                xphy_lane_connection.ls_lane = -1;
                            }
                        ...
                        case SOC_LANE_CONFIG_100G_3_4_3:
                            ...
                            if (((pport - first_port) > 3) &&  
                                ((pport - first_port) < 11)) {
                                xphy_lane_connection.ss_lane = (pport - first_port) - 1; 
                                xphy_lane_connection.ls_lane = (pport - first_port) - 1;   
                            }
                            /* Temp fix for sesto gearbox */
                            if ((((pport - first_port) > 3) &&
                               ((pport - first_port) < 8))) {
                        ...
                        case SOC_LANE_CONFIG_100G_2_4_4:
                            ...
                            if ((pport - first_port) > 3) {
                                xphy_lane_connection.ss_lane = (pport - first_port) - 2;
                                xphy_lane_connection.ls_lane = (pport - first_port) - 2;   
                            }
                            /* Temp fix for sesto gearbox */
                    ...
                    xphy_lane_connection.ss_lane = ((pport - first_port) % 4);
                    xphy_lane_connection.ls_lane = ((pport - first_port) % 4);
                }
                ...
                xphy_lane_connection.ss_lane = topology.sys_lane;
                xphy_lane_connection.ls_lane = topology.line_lane;
            }
...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag)
{
    ...
    if (init_flag == PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD)
    {
        PORTMOD_PORT_ADD_F_INIT_UNTIL_FW_LOAD_SET(&add_info);
    } 

    if (init_flag == PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD)
    { 
        PORTMOD_PORT_ADD_F_INIT_RESUME_AFTER_FW_LOAD_SET(&add_info);
    }
                             ...
                             soc_phy_control_t phy_ctrl, uint32 value)
{
    ...
    */
    ...
    if(!is_legacy_ext_phy_present) {
        ...
        SOC_IF_ERROR_RETURN(
            portmod_port_ext_phy_control_set(unit, port, phy_ctrl, value));
    }
                             ...
                             soc_phy_control_t phy_ctrl, uint32 *value)
{
    ...
    */
    ...
    if (!is_legacy_ext_phy) {
        ...
        SOC_IF_ERROR_RETURN(
            portmod_port_ext_phy_control_get(unit, port, phy_ctrl, value));
    }
