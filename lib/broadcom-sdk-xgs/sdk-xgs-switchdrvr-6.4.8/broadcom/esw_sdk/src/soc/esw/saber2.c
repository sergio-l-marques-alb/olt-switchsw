...
const static soc_port_details_t sb2_port_details_56262_2[SB2_MAX_PHYSICAL_PORTS+1]={
    {1, 4, 4, XE_PORT, 10000},
    {5, 8, 4, XE_PORT, 10000},
    {9,12, 4, 0, 0},
    {13,16,4, 0, 0},
    ...
    for (port = SB2_CMIC; port < SB2_MAX_LOGICAL_PORTS; port += port_incr) {
        ...
        if (IS_CPU_PORT(unit, port)) {
            ...
            if((eagle_n4s_mode) && ((port == 26) || (port == 28))) {
                ...
                start_threshold = 7;
                valid = TRUE;
            } else if(((eagle_n4s_mode || eagle_n2_mode) && (port == 27)) ||
                    (!eagle_n4s_mode && !eagle_n2_mode)){
                egr_fifo_depth = (IS_EXT_MEM_PORT(unit, port)) ? 53 : 10;
                threshold = (IS_EXT_MEM_PORT(unit, port)) ? 49 : 1;
                padding = 0;
                port_bw = 325;
            ...
            if(!SOC_WARM_BOOT(unit)) {
                if(port == 25) {
                    if(sb2_selected_port_details[loop_index+1].port_type != 0) {
                        ...
                        eagle_n4s_mode = 1;
                    } else if(sb2_selected_port_details[loop_index].port_type == 2) {
