...
tdm_vmap_chk_sister_1D(tdm_mod_t *_tdm)
{
    ...
    TDM_PRINT0("TDM: Check VMAP sister spacing (1-D)\n\n");
...
tdm_vmap_chk_sister_xy(tdm_mod_t *_tdm, unsigned short **vmap, int idx_x, int idx_y)
{
    ...
    if ( !(idx_x>=0 && idx_x<param_vmap_wid && idx_y>=0 && idx_y<param_vmap_len)){
        ...
        if (port_token>=param_phy_lo &&
            ...
            port_tsc = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
            for (i=0; i<param_vmap_wid; i++){
                for (k=1; k<param_space_sister; k++){
                    ...
                    if (port_tmp>=param_phy_lo &&
                        ...
                        tsc_tmp = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                        if (tsc_tmp==port_tsc){
                            result = FAIL;
                            break;
                        }
...
tdm_vmap_chk_sister_2D(tdm_mod_t *_tdm, unsigned short **vmap)
{
    ...
    TDM_PRINT0("TDM: Check VMAP sister spacing (2-D)\n\n");
...
{
    int i, j, rotate_pass, chk_result, result_i, result_j, result=PASS;
    int param_vmap_wid;
...
/*             if (rotate_pass==BOOL_TRUE){
            ...
            for (j=i+1; j<param_vmap_wid; j++){
                if(tdm_vmap_switch_col_xy(_tdm, vmap, i, j)==PASS){
                    result_i = tdm_vmap_chk_sister_col(_tdm, vmap, i);
                    result_j = tdm_vmap_chk_sister_col(_tdm, vmap, j);
                    if (result_i==PASS && result_j==PASS){
                        rotate_pass = BOOL_TRUE;
                        TDM_PRINT2("\t\t (2) switch column %d with column %d\n",
                                  i, j);
                    }
                    else if ( (result_i!=PASS && result_j==PASS)  ||
                              (result_i!=PASS && result_j!=PASS ) ){
            ...
            if (param_cal_main[i]==port_token){
                ...
                for (k=1; k<param_space_sister; k++){
                    ...
                    if (param_cal_main[j]>=param_phy_lo &&
                        ...
                        _tdm->_core_data.vars_pkg.port = param_cal_main[j];
                        tsc_tmp = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                        if (tsc_tmp==port_tsc){
                            k_dn = k;
                            break;
                ...
                if (k_dn>0){
                    for (j=0; j<param_cal_len; j++){
                        if (param_cal_main[j]==port_token){
                            ...
                            _tdm->_core_data.vars_pkg.port = param_cal_main[k];
                            tsc_tmp = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                            if (tsc_tmp!=port_tsc){
                                x1 = j;
...
/**
@name: tdm_vmap_get_speed_slot
@param:

**/
int
tdm_vmap_get_speed_slot(int port_speed)
{
    int port_spd_slot;
...
tdm_vmap_alloc(tdm_mod_t *_tdm)
{
    ...
    int a0, a1, a2, a3, b, token_vmap_idle, 
        ...
        for (i=0; i<param_vmap_wid ; i++){
            ...
            if (port_phynum>=param_phy_lo && port_phynum<=param_phy_hi){
                ...
                if (port_state==PORT_STATE__LINERATE    ||
                    ...
                    port_slot_num = tdm_vmap_get_speed_slot(port_speed);
    ...
    if (lr_en==BOOL_TRUE && result==PASS){
        ...
        for (i=0; i<param_vmap_wid; i++){
            ...
            if (port_phynum>=param_phy_lo && port_phynum<=param_phy_hi){
                _tdm->_core_data.vars_pkg.port = port_phynum;
                port_pm = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                ...
                if (pm_port_cnt[i]==2){
                    ...
                    if (port_0>=param_phy_lo && port_0<=param_phy_hi &&
                        ...
                        if ((spd_0/1000)!=(spd_1/1000)){
                            ...
                            slot_req = tdm_vmap_get_speed_slot(spd_1);
                ...
                if (port_token>=param_phy_lo && port_token<=param_phy_hi){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            ...
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum_curr += tdm_vmap_get_speed_slot(port_speed);
                ...
                for (i=0; i<(param_num_phy_pm-1); i++){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            port_phynum<=param_phy_hi){
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum += tdm_vmap_get_speed_slot(port_speed);
                        }
                    }
                ...
                for (i=0; i<(param_num_phy_pm-1); i++){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            port_phynum<=param_phy_hi){
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum_tmp += tdm_vmap_get_speed_slot(port_speed);
                        }
                    }
                    ...
                    if (pm_slot_sum>0 && empty_slot_left>0){
                        ...
                        if ((pm_slot_sum/lr_2_os_r>=pm_slot_sum_tmp) ||
                            (pm_slot_sum_tmp>0 && pm_pmap[i][0]==token_vmap_idle) ){
        ...
        for (i=0; i<param_num_phy_pm && i<pm_cnt; i++){
            ...
            if (port_token==token_vmap_idle && port_cnt==1){
                ...
                for (j=0; j<port_cnt && j<param_num_pm_lanes; j++){
                    ...
                    if (port_phynum>=param_phy_lo && 
                        port_phynum<=param_phy_hi){
                        port_speed = param_speeds[port_phynum];
                        slot_req += tdm_vmap_get_speed_slot(port_speed);
                    }
                }
                                ...
                                if(vmap[vmap_idx][j]==port_token){
                                    switch(k){
                                        ...
                                        case 1:
                                            ...
                                            break;
                                        case 3:
                                            vmap[vmap_idx][j]=token_vmap_idle;
                                            break;
                                        default:
                                            break;
                                    }
...
tdm_core_vmap_alloc(tdm_mod_t *_tdm)
{
    ...
    if (tdm_vmap_chk_sister_1D(_tdm)!=PASS){
        TDM_PRINT0("TDM: Adjust sister port space\n\n");
