...
#define _BCM_VXLAN_MATCH_PORT_INVALID(criteria) \
           ((criteria == BCM_VXLAN_PORT_MATCH_VN_ID) || \
            (criteria == BCM_VXLAN_PORT_MATCH_SHARE) || \
            (criteria == BCM_VXLAN_PORT_MATCH_NONE))
#endif
...
_tr2_vlan_vp_match_add(int unit, bcm_vlan_port_t *vlan_vp, int vp)
{
    ...
    if ((vlan_vp->criteria == BCM_VLAN_PORT_MATCH_PORT_PON_TUNNEL) ||
        ...
        soc_VLAN_XLATEm_field32_set(unit, &vent, LLTAG__NEW_OVIDf,
                                vlan_vp->egress_vlan);
        soc_VLAN_XLATEm_field32_set(unit, &vent, LLTAG__NEW_IVIDf,
                                vlan_vp->egress_inner_vlan);
        soc_VLAN_XLATEm_field32_set(unit, &vent, LLTAG__TAG_ACTION_PROFILE_PTRf,
                                profile_idx);
                           ...
                           int flags)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(gport)) {
        ...
        if (BCM_GPORT_IS_MODPORT(vlan_vp.port) && is_vfi) {
    ...
    if (BCM_GPORT_IS_VXLAN_PORT(gport)) {
        ...
        if (_BCM_VXLAN_MATCH_PORT_INVALID(vxlan_port.criteria)) {
            phy_port_trunk = BCM_GPORT_INVALID;
        } else {
        ...
        mim_port.mim_port_id = gport;
        BCM_IF_ERROR_RETURN(bcm_tr2_mim_port_get(unit, vlan, &mim_port));
        phy_port_trunk = mim_port.port;
...
#ifdef BCM_TRIUMPH3_SUPPORT
    if (soc_feature(unit, soc_feature_vlan_vfi) && BCM_GPORT_IS_VLAN_PORT(gport) && is_vfi) {
...
#ifdef BCM_TRIUMPH3_SUPPORT
    if (soc_feature(unit, soc_feature_vlan_vfi) &&
        BCM_GPORT_IS_VLAN_PORT(gport) && is_vfi) {
    ...
    *flags = 0;
    ...
    if (soc_feature(unit, soc_feature_vlan_vfi_membership)) {
        ...
        if (eg_vp_group_found) {
            BCM_IF_ERROR_RETURN(bcm_td2p_vp_group_state_get(unit, gport,
                                vlan_vfi, TRUE, &state_flags));
            if (state_flags & BCM_VLAN_GPORT_ADD_UNTAGGED) {
