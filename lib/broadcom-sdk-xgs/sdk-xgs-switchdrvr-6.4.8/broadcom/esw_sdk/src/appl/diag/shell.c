...
_sh_parse_units(int unit, const char *ustr, int *umap_ptr, int *ulen_ptr)
{
    ...
    /* calculating the maximal device id */
    for(dev_idx = 0; dev_idx < soc_ndev; ++dev_idx) {
        if(SOC_NDEV_IDX2DEV(dev_idx) > maximal_dev_id) {
            maximal_dev_id = SOC_NDEV_IDX2DEV(dev_idx);
        }
    }
...
#   define    CMD_PER_LINE    5
    ...
    if (cur_cmd_list[i].c_help[0] != '.') {
        cli_out("%-15s%s", cur_cmd_list[i].c_cmd,
                (words % CMD_PER_LINE) == (CMD_PER_LINE - 1) ? "\n" : "");
        words++;
    }
    }
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
static char *
...
appl_warmboot_storage_requirements(int unit)
{
    ...
    if (SOC_IS_PETRAB(unit)) {
        cli_out("Total storage used: %d bytes\n", total);
    } else {
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT) || defined(BCM_SBX_SUPPORT) || defined(BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT))
#if defined(BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT)
...
sh_warmboot(int u, args_t *a)
{
    ...
    if (0 == ARG_CNT(a)) {
        ...
        if (!sal_strcasecmp(c, "on")) {
            ...
            uint32 left = 0;
            uint32 in_use = 0;
            shr_sw_state_sizes_get(unit, &in_use, &left);
            cli_out("total: %dMB.\n", (left+in_use)/1000000);
            cli_out("in use: %dMB.\n", in_use/1000000);
            cli_out("left: %dMB.\n", left/1000000);
        }
		   ...
		   if (0 == ARG_CNT(a)) {
		    ...
		    if (!sal_strcasecmp(test_status, "on")) {
			     ...
			     soc_dcmn_wb_test_mode_set(unit,_DCMN_BCM_WARM_BOOT_API_TEST_MODE_AFTER_EVERY_API);
                        ...
                        soc_dcmn_wb_test_mode_set(unit,_DCMN_BCM_WARM_BOOT_API_TEST_MODE_END_OF_DVAPIS);
			}else if (!sal_strcasecmp(test_status, "off")){
                        soc_dcmn_wb_test_mode_set(unit,_DCMN_BCM_WARM_BOOT_API_TEST_MODE_NONE);
                        ...
                        soc_dcmn_wb_test_skip_multiple_wb_tests_set(unit,skip);
			}else {
				return(CMD_USAGE);
			}
		}
            ...
            cli_out("\ncalculating WarmBoot file sizes: may take a few seconds to complete.\n");

            rv = soc_scache_commit(unit);
            if (rv != _SHR_E_NONE) {
                cli_out("\nError: failed to sync Scache\n");
                sal_fclose(status);
                return CMD_FAIL;
            }
#if defined(BCM_88675_A0) && defined(INCLUDE_KBP) && !defined(BCM_88030)
            ...
            wb = sal_fopen( "warmboot_data", "r" );
            if (wb == NULL) {
                cli_out("\nError: Unable to open wb file\n");
            }
            else {
                sal_fseek((FILE *)wb, 0L, SEEK_END);
                wb_sz = sal_ftell((FILE *)wb);
            }

            kaps_wb = sal_fopen( "warmboot_data_kaps", "r" );
            if (kaps_wb == NULL) {
                cli_out("\nError: Unable to open kaps wb file\n");
            }
            else{
                sal_fseek((FILE *)kaps_wb, 0L, SEEK_END);
                kaps_wb_sz = sal_ftell((FILE *)kaps_wb);
            }
            ...
            sal_printf("-----\n");
            sal_printf("wb file size:         %d MB\n", (wb_sz/1000000));
            sal_printf("kaps wb file size:    %d MB\n", (kaps_wb_sz/1000000));
            sal_printf("-----\n");
            sal_printf("memory + WB storage:  %d MB\r\n\n", ((kaps_wb_sz+wb_sz+(values[0]*page_size))/1000000));

            sal_fclose((FILE *)status);
            sal_fclose((FILE *)wb);
            sal_fclose((FILE *)kaps_wb);
...
#endif /*(defined(BCM_DPP_SUPPORT) && defined(BCM_PETRA_SUPPORT)) || defined(BCM_DFE_SUPPORT)*/
#endif /* (BCM_XGS3_SWITCH_SUPPORT || BCM_SBX_SUPPORT || BCM_DPP_SUPPORT) || defined(BCM_DFE_SUPPORT)*/
#endif /* BCM_WARM_BOOT_SUPPORT */
...
#endif /* BCM_DPP_SUPPORT && BCM_PETRA_SUPPORT */
