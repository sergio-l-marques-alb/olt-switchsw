...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-r") || !strcmp(argv[i], "--reload")) {
        ...
        } else if (!strcmp(argv[i], "-w") || !strcmp(argv[i], "--warmboot")) {
            sal_boot_flags_set(sal_boot_flags_get() | BOOT_F_WARM_BOOT);
            ...
            if (sysconf_ltsw_keep_ha_file_set(1) < 0) {
                printf("Can not keep HA file upon exit\n");
                exit(1);
            }
        } else if (!strcmp(argv[i], "-q")) {
            if (sysconf_ltsw_ha_sync_mode_set(1) < 0) {
                printf("Can not set HA sync mode\n");
                exit(1);
            }
#endif
        }
