                          ...
                          int banks, uint8 op, uint32 *index, uint32 *result)
{
    ...
    reg_addr = soc_reg_addr_get(unit, _ism_table_bank_cfg_reg[0], REG_PORT_ANY,
                                0, SOC_REG_ADDR_OPTION_NONE, &sblk, &at),
    soc_internal_extended_read_reg(pcid_info, sblk, at, reg_addr, tmp_hs);
...
{   
    ...
    if (rc) {
        ...
        if (opres == SCHAN_GEN_RESP_TYPE_FULL) {
            ...
            sblk = SOC_BLOCK2SCH(unit, blk);
            genresp->index = index;
            mem_addr = soc_mem_addr_get(unit, mem, 0, blk, index, &at);
            soc_internal_extended_write_mem(pcid_info, sblk,
                                            at, mem_addr, (uint32 *)entry);
                                 ...
                                 void *entry, uint32 *result)
{
    ...
    if (rc) {
        ...
        sblk = SOC_BLOCK2SCH(unit, blk);
        mem_addr = soc_mem_addr_get(unit, mem, 0, blk, index, &at);
        soc_internal_extended_read_mem(pcid_info, sblk,
                                       at, mem_addr, (uint32 *)entry);
        if (soc_mem_field_valid(unit, mem, VALIDf)) {
