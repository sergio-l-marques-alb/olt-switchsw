...
soc_internal_l2x2_hash(pcid_info_t * pcid_info, l2x_entry_t *entry, int dual)
{
    ...
    if (dual) {
        reg_addr = soc_reg_addr_get(pcid_info->unit, L2_AUX_HASH_CONTROLr,
                                    REG_PORT_ANY, 0, SOC_REG_ADDR_OPTION_NONE,
                                    &blk, &at);
        soc_internal_extended_read_reg(pcid_info, blk, at, reg_addr, tmp_hs);
        hash_sel = soc_reg_field_get(pcid_info->unit, L2_AUX_HASH_CONTROLr,
                                     tmp_hs[0], HASH_SELECTf);
    } else {
        reg_addr = soc_reg_addr_get(pcid_info->unit, HASH_CONTROLr,
                                    REG_PORT_ANY, 0, SOC_REG_ADDR_OPTION_NONE,
                                    &blk, &at);
        soc_internal_extended_read_reg(pcid_info, blk, at, reg_addr, tmp_hs);
                             ...
                             int *dual, int *slot_min, int *slot_max)
{
    ...
    reg_addr = soc_reg_addr_get(pcid_info->unit, L2_AUX_HASH_CONTROLr,
                                REG_PORT_ANY, 0, SOC_REG_ADDR_OPTION_NONE,
                                &blk, &at);
    soc_internal_extended_read_reg(pcid_info, blk, at, reg_addr, tmp);
